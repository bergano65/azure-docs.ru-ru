...
- Классический Azure CLI — это наш интерфейс командной строки для классической модели развертывания и модели развертывания с помощью Resource Manager (в этой статье).
- [Azure CLI](../articles/virtual-machines/linux/cli-manage.md) — это интерфейс командной строки нового поколения для модели развертывания в режиме управления ресурсами

## <a name="basic-azure-resource-manager-commands-in-azure-classic-cli"></a>Основные команды Azure Resource Manager в классическом Azure CLI

В этой статье рассматриваются основные команды в классическом Azure CLI для управления ресурсами (в основном виртуальными машинами) и взаимодействия с ними в подписке Azure.  Чтобы получить подробные сведения о конкретных параметрах командной строки, можно использовать интерактивную справку по командам, к которой можно получить доступ с помощью команд `azure <command> <subcommand> --help` или `azure help <command> <subcommand>`.
...
| Список виртуальных машин |`azure  vm list [options]` |
...
| Создание виртуальной машины из специализированного диска |`azure  vm create [options] –d <os-disk-vhd> <resource-group> <name> <location> <os-type>` |
