...
1. <a name="getnet"></a>Получите имя сетевого ресурса кластера.
...
1. <a name = "dependencyGroup"></a>Сделайте SQL Server ресурс группы доступности зависимым от точки доступа клиента.
...
1. <a name="listname"></a>Сделайте ресурс точки доступа клиента зависимым от IP-адреса.
   ...
   ![Ресурс IP-адреса](./media/virtual-machines-ag-listener-configure/98-dependencies.png) 
    ...
    c. Перейдите на вкладку **зависимости** . Убедитесь, что IP-адрес является зависимостью. В противном случае укажите IP-адрес в качестве зависимости. Если в списке несколько ресурсов, убедитесь, что IP-адреса имеют зависимости OR, а не AND. Нажмите кнопку **ОК**. 
...
1. <a name="setparam"></a>Задайте параметры кластера в PowerShell.
...
1. Получите имя IP-адреса для кластера WSFC. В **Диспетчер отказоустойчивости кластеров** в разделе **ресурсы ядра кластера**выберите **имя сервера**.
...
1. <a name="setwsfcparam"></a>Задайте параметры кластера в PowerShell.
...
>[!WARNING]
>Порт пробы работоспособности прослушивателя группы доступности должен отличаться от порта пробы работоспособности ядра кластера IP-адреса. В этих примерах порт прослушивателя — 59999, а порт пробы работоспособности IP-адреса основного кластера — 58888. Оба порта требуют правило разрешения брандмауэра для входящих подключений.
