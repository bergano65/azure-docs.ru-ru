...
На следующих скриншотах показано, как обновить учетную запись хранилища Classic в учетную запись хранения ресурсов Azure Manager с помощью портала Azure:
1. Войдите на [портал Azure](https://portal.azure.com).
2. Войдите в свою учетную запись хранения.
3. В разделе **Настройки** нажмите **«Мигрировать» в ARM.**
4. Нажмите на **Validate,** чтобы определить целесообразность миграции.
5. Если проверка проходит, нажмите на **Prepare** для создания мигрированной учетной записи хранения.
6. Введите **да,** чтобы подтвердить миграцию и нажмите **Commit,** чтобы закончить миграцию.

    ![Проверка учетной записи хранилища](../includes/media/storage-account-upgrade-classic/storage-migrate-resource-manager-1.png)
    
    ![Подготовка учетной записи хранилища](../includes/media/storage-account-upgrade-classic/storage-migrate-resource-manager-2.png)
    
    ![Завершение миграции учетной записи хранилища](../includes/media/storage-account-upgrade-classic/storage-migrate-resource-manager-3.png)
...
| Служба | Параметр Configuration | Рекомендация |
| --- | --- | --- |
| Resource Manager |Контроль доступа на основе ролей (RBAC) для классических ресурсов |Так как после миграции универсальные коды ресурсов (URI) меняются, рекомендуется спланировать обновления политики RBAC, которые должны быть применены после миграции. |
...
| Службы вычислений | VM с расширением восстановления сайта Azure | Эти расширения устанавливаются на виртуальную машину, настроенную с помощью службы восстановления сайта Azure. В то время как миграция хранилища, используемого при восстановлении сайта, будет работать, текущее воспроизведение будет затронуто. Необходимо отключить и включить репликацию VM после миграции хранилища. |
