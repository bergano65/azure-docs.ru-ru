...
---
Если возникнут проблемы при выполнении операций с коллекциями общих образов, определениями и версиями образов, выполните команду, которая дала сбой, в режиме отладки. Режим отжута активируется путем передачи **переключателя -отладки** с CLI и **переключателем -Debug** с PowerShell. Обнаружив ошибку, следуйте инструкциям в этом документе для ее устранения.
...
Допустимыми знаками для имени коллекции являются прописные или строчные буквы, цифры и точки. Название галереи не может содержать тире. Измените имя коллекции и повторите попытку. 
...
Разрешенные символы для определения изображения являются верхними или нижними буквами, цифрами, точками, тире и периодами. Измените имя определения образа и повторите попытку.
...
*Список целевых регионов не включает регион исходного кода.*
...
Совместное использование общих изображений, определение изображений и ресурсов версии изображений между подписями включено с помощью [Role-Based Control](https://docs.microsoft.com/azure/role-based-access-control/rbac-and-directory-admin-roles) (RBAC). 
