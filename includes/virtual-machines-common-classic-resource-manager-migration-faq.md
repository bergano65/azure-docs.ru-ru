...
---
## <a name="what-is-the-time-required-for-migration"></a>Сколько времени требуется для миграции?

Планирование и выполнение миграции сильно зависит от сложности архитектуры и может занять несколько месяцев.  

## <a name="what-is-the-definition-of-a-new-customer-on-iaas-vms-classic"></a>Каково определение нового клиента на виртуальных машинах IaaS (классическая модель)?

Клиенты, у которых нет виртуальных машин IaaS (классические) в своих подписках в течение месяца Фебрауари 2020 (месяц до начала срока действия), считаются новыми клиентами. 

## <a name="what-is-the-definition-of-an-existing-customer-on-iaas-virtual-machines-classic"></a>Каково определение существующего клиента на виртуальных машинах IaaS (классическая модель)?

Клиент, который был активным или остановлен, но выделил виртуальные машины IaaS (классические) в своих подписках в течение февраля 2020, считается существующим клиентом. Только эти клиенты получают данные до 1 марта 2023, чтобы перенести их виртуальные машины из Service Manager Azure в Azure Resource Manager. 

## <a name="why-am-i-getting-an-error-stating-newclassicvmcreationnotallowedforsubscription"></a>Почему я получаю сообщение об ошибке "Невклассиквмкреатионноталловедфорсубскриптион"?

В рамках процесса прекращения использования виртуальная машина IaaS (классическая) больше не будет доступна для новых клиентов. Мы определили вас в качестве новых клиентов, поэтому ваша операция не была авторизации. Мы настоятельно рекомендуем использовать [виртуальные машины Azure с помощью ARM](https://docs.microsoft.com/azure/virtual-machines/windows/quick-create-powershell). Если вы не можете использовать виртуальные машины Azure с помощью ARM, обратитесь в службу поддержки для список разрешений подписки.
...
Не до 1 марта 2023 для виртуальных машин IaaS (классическая модель). Виртуальные машины IaaS (классическая модель) являются полностью поддерживаемыми службами в общедоступной доступности. Можно продолжать использовать эти ресурсы, чтобы расширить занимаемый объем в Microsoft Azure. 1 марта 2023 эти виртуальные машины будут полностью прекращены, а все активные или выделенные виртуальные машины будут остановлены & освобождены. Другие классические ресурсы, такие как облачные службы (классические), учетные записи хранения (классические) и т. д., не будут затронуты.   
...
1 марта 2023 виртуальные машины IaaS (классические) будут полностью удалены, а все активные или выделенные виртуальные машины будут остановлены & освобождены. Чтобы предотвратить влияние на бизнес, мы очень перекомментируем, чтобы начать планирование миграции сегодня и выполнить его до 1 марта 2023. Мы не рекомендуем использовать существующие классические API, облачные службы и модель ресурсов. Мы хотим максимально упростить миграцию с помощью расширенных функций, доступных в модели развертывания с помощью Resource Manager. Рекомендуется приступить к планированию миграции этих ресурсов в Azure Resource Manager. 
...
Прервать миграцию при сбое фиксации нельзя. Все операции миграции, включая фиксацию, являются идемпотентными. Поэтому рекомендуется повторить операцию спустя некоторое время. Если вы по-прежнему сталкиваетесь с ошибкой, создайте запрос в службу поддержки.
