...
В Azure сетевому интерфейсу можно назначить следующие типы [IP-адресов](../articles/virtual-network/public-ip-addresses.md).
...
- **[Перенаправление портов.](../articles/load-balancer/tutorial-load-balancer-port-forwarding-portal.md)** Эти правила определяют, как входящий трафик проходит через внешний IP-адрес и передается внутреннему IP-адресу с использованием правил NAT для входящего трафика.
...
- **[Правило для исходящего трафика.](../articles/load-balancer/load-balancer-outbound-rules-overview.md)** Позволяет настроить NAT для отображения в интерфейсе всех виртуальных машин или экземпляров, которые входят в серверный пул Load Balancer (цен. категория "Стандартный").

В таблице ниже перечислены методы создания балансировщика нагрузки с выходом в Интернет.

| Метод | Описание |
| ------ | ----------- |
| Портал Azure |  Вы можете [настроить балансировку интернет-трафика на виртуальных машинах с помощью портала Azure](../articles/load-balancer/tutorial-load-balancer-standard-manage-portal.md). |
...
| Метод | Описание |
| ------ | ----------- |
| Портал Azure | Вы можете [настроить балансировку нагрузки внутреннего трафика с помощью подсистемы балансировки нагрузки на портале Azure](../articles/load-balancer/tutorial-load-balancer-standard-internal-portal.md). |
...
- Дополнительные сведения о [пропускной способности сетевых ресурсов виртуальной машины](../articles/virtual-network/virtual-machine-network-throughput.md).
