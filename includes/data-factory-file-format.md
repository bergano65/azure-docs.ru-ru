...
| encodingName |Имя кодировки. |Допустимое имя кодировки. Ознакомьтесь с описанием свойства [Encoding.EncodingName](/dotnet/api/system.text.encoding). Пример: windows-1250 или shift_jis. **По умолчанию** используется **UTF-8**. |Нет  |
...
| encodingName |Имя кодировки. Список допустимых имен кодировок приведен в описании свойства [Encoding.EncodingName](/dotnet/api/system.text.encoding). Например: windows-1250 или shift_jis. По **умолчанию** используется **UTF-8**. |Нет  |
...
- **Тип инструкции: setOfObjects**
    ...
    * **Пример JSON с разбивкой на строки**
...
- `structure` раздел определяет настраиваемые имена столбцов и соответствующие типы данных во время преобразования в табличных данных. Этот раздел является **необязательным**, если вам не нужно сопоставлять столбцы. Дополнительные сведения см. в разделе об указании определения структуры для прямоугольных наборов данных.
- `jsonPathDefinition` Указывает путь для каждого столбца, откуда следует извлекать данные из JSON. Чтобы скопировать данные из массива, с помощью **array[X].property** извлеките значение нужного свойства из объекта X или с помощью **array[*].property** найдите нужное значение в любом объекте с таким свойством.
...
- `structure` раздел определяет настраиваемые имена столбцов и соответствующие типы данных во время преобразования в табличных данных. Этот раздел является **необязательным**, если вам не нужно сопоставлять столбцы. Дополнительные сведения см. в разделе об указании определения структуры для прямоугольных наборов данных.
- `jsonNodeReference` обозначает итерацию и извлечение данных из объектов с одинаковым шаблоном в разделе **массива** orderlines.
- `jsonPathDefinition` Указывает путь для каждого столбца, откуда следует извлекать данные из JSON. В этом примере значения ordernumber, orderdate и city находятся в корневом объекте. Путь JSON к этому объекту начинается с "$." а значения order_pd и order_price определяются с помощью пути, производного от элемента массива без "$.".
