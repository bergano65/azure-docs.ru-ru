...
ms.locfileid: "89644264"
---
### <a name="is-bgp-supported-on-all-azure-vpn-gateway-skus"></a>VPN-шлюзы Azure поддерживают BGP для всех классов SKU?
Протокол BGP поддерживается для всех ценовых категорий VPN-шлюза Azure, за исключением категории "Базовый".
...
* маршруты, полученные от других сеансов пиринга BGP с подключением к VPN-шлюзу Azure, **за исключением маршрутов по умолчанию и маршрутов, пересекающихся с префиксами любой виртуальной сети**.
...
VPN-шлюз Azure выделяет из диапазона GatewaySubnet один IP-адрес для VPN-шлюзов в режиме "активный — резервный" или два IP-адреса для VPN-шлюзов в режиме "активный — активный". Вы можете узнать фактически выделенные IP-адреса BGP с помощью PowerShell (командлет Get-AzVirtualNetworkGateway возвращает свойство bgpPeeringAddress) или на портале Azure (в разделе "Настройка BGP ASN" на странице "Конфигурация шлюза").
