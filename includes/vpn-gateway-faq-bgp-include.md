...
BGP поддерживается на всех SkUs Azure VPN Gateawy, кроме Basic SKU.
...
### <a name="can-i-use-32-bit-4-byte-asns-autonomous-system-numbers"></a>Могу ли я использовать 32-разрядные (4-байт) ASNs (автономные номера системы)?
Да, Azure VPN Gateways теперь поддерживает 32-битные (4-байт) ASNs. Пожалуйста, используйте PowerShell/CLI/SDK для настройки с помощью ASN в десятичном формате.
...
### <a name="what-private-asns-can-i-use"></a>Какие частные ASNs я могу использовать?
Используемый диапазон частных ASNs, которые могут быть использованы являются:

* 64512-65514, 65521-65534

Эти ASN не зарезервированы IANA или Azure для использования и поэтому могут быть использованы для присвоения вашему VPN шлюзу Azure.
...
Vpn шлюз Azure выделит единый IP-адрес из диапазона GatewaySubnet для шлюзов VPN с активным режимом ожидания или два IP-адреса для активных VPN шлюзов. Вы можете получить фактический IP-адрес BGP (es), выделенный с помощью PowerShell (Get-AzVirtualNetworkGateway, ищите свойство "bgpPeeringAddress") или на портале Azure (под свойством "Наладить BGP ASN" на странице конфигурации шлюза).
