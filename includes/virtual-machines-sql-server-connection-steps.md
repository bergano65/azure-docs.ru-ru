...
8. В диалоговом окне **Профиль** выберите **Открытый**, **Частный** и **Доменный**. Нажмите **Далее**.
...
2. Management Studio отображает диалоговое окно **Подключение к серверу** . В поле **Имя сервера** введите имя виртуальной машины для подключения к ядру СУБД с помощью обозревателя объектов (вместо имени виртуальной машины можно также использовать **(local)**, а вместо **имени сервера** — одну точку). Выберите **аутентификацию Windows**и оставьте ***your_VM_name your_local_administrator*** в поле **имени пользователя.** Нажмите кнопку **Подключить**.
...
2. Справа щелкните папку **безопасности,** укажите на **Новый**, и выберите **Логин ...**.
