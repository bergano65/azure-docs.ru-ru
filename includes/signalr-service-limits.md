---
title: Таблица ограничений службы Azure SignalR
description: Описывает ограничения системы для службы SignalR Azure.
services: signalr
documentationcenter: signalr
author: sffamily
manager: cfowler
editor: ''
ms.service: signalr
ms.devlang: NA
ms.topic: article
ms.tgt_pltfrm: NA
ms.workload: TBD
ms.date: 09/24/2018
ms.author: zhshang
ms.openlocfilehash: 1d87439b7afeb946fd37c468a0853adcb674ee5a
ms.sourcegitcommit: cd70273f0845cd39b435bd5978ca0df4ac4d7b2c
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 09/18/2019
ms.locfileid: "67185688"
---
| Resource | Ограничение по умолчанию | Максимальное ограничение | 
| --- | --- | --- |
| Количество единиц обслуживания Azure SignalR на экземпляр для уровня "бесплатный" |1 |1 |
| Количество единиц службы SignalR Azure на экземпляр уровня "Стандартный" |100 |100 |
| Количество единиц обслуживания Azure SignalR на подписку в регионе для уровня "бесплатный"|5 |5 |
| Общее число единиц службы SignalR Azure для каждой подписки на регион |150 |Без ограничений |
| Подключения на единицу в день, уровень "Бесплатный" |20 |20 |
| Подключения на единицу в день, уровень "Стандартный" |1000 |1000|
| Включенные сообщения на единицу в день, уровень "Бесплатный"|20 000 |20 000 |
| Включенные сообщения на единицу в день, уровень "Стандартный"|1 000 000 |1 000 000 |

Запросите обновление стандартных ограничений для подписки. Для этого отправьте запрос в службу поддержки. 
