...
- В настоящее время поддерживается только в [подмножестве регионов](#regional-availability).
- В настоящее время поддерживается только Azure Resource Manager или поддержка пакета SDK. 
...
Общие твердотельные накопители уровня "Премиум" поддерживаются только в следующих регионах:
