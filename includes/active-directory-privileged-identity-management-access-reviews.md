...
1. Нажмите **новый,** чтобы создать новый обзор доступа.
...
1. Установите **дату начала**. По умолчанию проверка доступа выполняется однократно, начинается в то же самое время, когда она создается, и завершается через один месяц. Можно изменить даты начала и окончания таким образом, чтобы проверка доступа началась в будущем и длилась необходимое количество дней.
    ...
    ![Дата начала, частота, продолжительность, конец, количество раз и дата окончания](./media/active-directory-privileged-identity-management-access-reviews/start-end-dates.png)

1. Чтобы сделать обзор доступа повторяющимся, измените настройки **частоты** от **Одного раза** до **еженедельных,** **ежемесячных,** **ежеквартальных, ежегодных**или **полугодовых.** **Quarterly** Используйте ползунок **Duration** или текстовый ящик, чтобы определить, сколько дней каждый обзор повторяющихся серий будет открыт для ввода от рецензентов. Например, для ежемесячной проверки вы можете настроить продолжительность не более 27 дней, что позволяет избежать перекрытия интервалов проверки.
...
1. В разделе **«Пользователи»** выберите одну или несколько ролей, которые вы хотите просмотреть членство.

    ![Область охвата пользователей для рассмотрения роли членства](./media/active-directory-privileged-identity-management-access-reviews/users.png)

    > [!NOTE]
    > Выбор более чем одной роли создаст несколько обзоров доступа. Например, при выборе пяти ролей будет создано пять отдельных обзоров доступа.

    При создании обзора ролей Azure AD для доступа приведен пример списка участников Обзора.

    ![Просмотрите панель членства с указанием ролей Azure AD, которые вы можете выбрать](./media/active-directory-privileged-identity-management-access-reviews/review-membership.png)

    При создании обзора ролей ресурсов Azure в доступе приведен пример списка участников Обзора.

    ![Просмотрите панель членства с указанием ролей ресурса Azure, которые вы можете выбрать](./media/active-directory-privileged-identity-management-access-reviews/review-membership-azure-resource-roles.png)
...
1. В разделе **Рецензенты** выберите одного или нескольких пользователей для просмотра всех пользователей. Или можно указать, чтобы участники сами проверяли собственный доступ.
    ...
    ![Рецензенты список выбранных пользователей или членов (самостоятельно)](./media/active-directory-privileged-identity-management-access-reviews/reviewers.png)

    - **Выбранные пользователи** - Используйте эту опцию, когда вы не знаете, кому нужен доступ. С помощью этого параметра можно назначить выполнение проверки владельцу ресурса или руководителю группы.
    - **Члены (самостоятельно)** - Используйте эту опцию, чтобы пользователи рассмотрели свои собственные назначения ролей.
...
1. Чтобы указать, что происходит после завершения проверки, разверните раздел **Настройки после завершения**.
    ...
    ![При завершении настройки автоматического применения и должны пересмотреть не отвечать](./media/active-directory-privileged-identity-management-access-reviews/upon-completion-settings.png)
...
1. Чтобы настроить дополнительные параметры, разверните раздел **Дополнительные параметры**.
    ...
    ![Расширенные настройки для рекомендаций по показу требуют обоснования утверждения, уведомлений по почте и напоминаний](./media/active-directory-privileged-identity-management-access-reviews/advanced-settings.png)
