...
|Определение образа|Издатель|Предложение|Номер SKU|
|---|---|---|---|
|myImage1|Contoso|Финансы|Сервер|
|myImage2|Contoso|Финансы|Интерфейсный сервер|
|myImage3|Тестирование|Финансы|Интерфейсный сервер|
...
* Состояние операционной системы. Вы можете задать для ОС состояние "обобщенный" или "специализированный", но в настоящее время поддерживается только обобщенная версия. Образы должны создаваться на виртуальных машинах, обобщенных с помощью Sysprep для Windows `waagent -deprovision` или Linux.
...
* Дата окончания срока действия. Присоедините конечную дату к определению образа, чтобы иметь возможность использовать автоматизацию для удаления старых определений образов.
...
| Исходные регионы |
|---------------------|-----------------|------------------|-----------------|
| Центральная Австралия   | Центральная часть США (EUAP) | Центральная Корея    | Центрально-западная часть США |
| Центральная Австралия 2 | Восточная Азия       | Республика Корея, южный регион      | Западная Европа     |
| Восточная Австралия      | East US         | Центрально-северная часть США | Западная Индия      |
| Юго-восточная Австралия | Восточная часть США 2       | Северная Европа     | Западная часть США         |
| Южная Бразилия        | Восточная часть США 2 (EUAP)  | Центрально-южная часть США | Западная часть США 2       |
| Центральная Канада      | Центральная Франция  | Южная Индия      |                 |
| Восточная Канада         | Южная Франция    | Юго-Восточная Азия   |                 |
| Центральная Индия       | Восточная Япония      | Южная часть Соединенного Королевства         |                 |
| Центральная часть США          | Западная Япония      | Западная часть Соединенного Королевства          |                 |
...
Дополнительные сведения см. в разделе [Проверка использования ресурсов в соответствии](https://docs.microsoft.com/azure/networking/check-usage-against-limits) с ограничениями в качестве примера проверки текущего использования.
...
- Для каждых 20 виртуальных машин, которые вы создаете параллельно, рекомендуется использовать одну реплику. Например, если вы создаете виртуальные машины 120 параллельно с помощью одного и того же образа в регионе, мы рекомендуем разместить по крайней мере 6 реплик образа. 
- Для каждого развертывания масштабируемого набора, в котором до 600 экземпляров рекомендуется разместить по крайней мере одну реплику. Например, если вы создаете 5 наборов масштабирования одновременно, каждый с экземплярами виртуальных машин 600, использующих один образ в одном регионе, мы рекомендуем разместить по крайней мере 5 реплик образа. 
...
| Доступ предоставлен пользователю     | Общедоступная коллекция образов | Определение образа | Изображение |
|----------------------|----------------------|--------------|----------------------|
| Общедоступная коллекция образов | Да                  | Да          | Да                  |
...
**Вопрос.** Как создать список всех ресурсов коллекции общих образов в разных подписках? 
 ...
 A. Чтобы получить список всех ресурсов коллекции общих образов в разных подписках, к которым вы имеете доступ на портале Azure, выполните следующие действия:
...
**Вопрос.** Можно ли перенести существующий образ в коллекцию общих образов?
 ...
 A. Да. Существует 3 сценария, в зависимости от типов образов, которые у вас могут быть.
...
**Вопрос.** Можно ли реплицировать версии образов между облаками — 21Vianet Azure для Китая, Azure для Германии и облако Azure для государственных организаций? 
 ...
 A. Нет, реплицировать версии образов между облаками невозможно.

**Вопрос.** Можно ли реплицировать версии образов между подписками? 

 A. Нет, можно реплицировать версии образов между регионами в рамках подписки и использовать их в других подписках с помощью RBAC.
...
**Вопрос.** Можно ли создать коллекцию общих образов в расположении, отличном от того, где планируется создание определения образа и версии образа?
 ...
 A. Да, это возможно. Однако рекомендуется размещать в одном расположении группу ресурсов, коллекцию общих образов, определение образа и версию образа.


**Вопрос.** Какова стоимость использования коллекции общих образов?

 A. За использование службы "Коллекция общих образов" плата не взимается, за исключением расходов на хранение версий образов и на исходящий трафик для репликации версий образов из исходного региона в целевые регионы.
...
**Вопрос.** Какую версию API следует использовать для создания коллекции общих образов, определения образа, версии образа и виртуальной машины или масштабируемого набора виртуальных машин из версии образа?
 ...
 A. Для развертываний виртуальной машины и масштабируемого набора виртуальных машин с помощью версии образа рекомендуется использовать API версии 2018-04-01 или более поздней. Для работы с коллекцией общих образов, определениями образов и версиями образов рекомендуется использовать API версии 2018-06-01. Для хранилища, избыточного в виде зоны (ZRS), требуется версия 2019-03-01 или более поздняя.
