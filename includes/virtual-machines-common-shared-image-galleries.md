...
-   Hyper-V generation (Поколение Hyper-V): вы можете указать, был ли этот образ создан на основе виртуального диска 1-го или [2-го поколения](../articles/virtual-machines/generation-2.md) Hyper-V. Значение по умолчанию — поколение 1.
...
Дополнительные сведения и примеры проверки текущего уровня потребления см. в статье [Проверка использования ресурсов в соответствии с ограничениями](../articles/networking/check-usage-against-limits.md).
...
Кроме того, для каждого целевого региона можно выбрать тип учетной записи. По умолчанию используется тип учетной записи хранения Standard_LRS, однако для регионов с Зонами доступности можно выбирать тип Standard_ZRS. Доступность службы ZRS в различных регионах можно уточнить [здесь](../articles/storage/common/storage-redundancy.md).
...
- Если это виртуальный жесткий диск виртуальной машины Windows, см. раздел об [отправке универсального диска VHD](../articles/virtual-machines/windows/upload-generalized-managed.md).
- Если это виртуальный жесткий диск виртуальной машины Linux, см. раздел [Создание виртуальной машины Linux на основе пользовательского диска с помощью Azure CLI](../articles/virtual-machines/linux/upload-vhd.md#option-1-upload-a-vhd).
