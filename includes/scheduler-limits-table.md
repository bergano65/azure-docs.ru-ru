...
| **Время ожидания** | Время ожидания запроса является статическим, то есть не может быть настроено. и 60 секунд для действий HTTP. Для выполнения более длительных операций используйте асинхронные протоколы HTTP. Например, можно немедленно получить код состояния 202, но продолжить работу в фоновом режиме. | 
