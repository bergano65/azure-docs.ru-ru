...
| Исходные IP-адреса | Тег службы источника | Исходные порты | Место назначения | Конечные порты | Протокол | Действие |
| --- | --- | --- | --- | --- | --- | --- |
| Н/Д | `BatchNodeManagement` [Тег службы](../articles/virtual-network/security-overview.md#service-tags) | * | Все | 29876–29877 | TCP | Allow |
| Пользователь IP-адреса источника для удаленного доступа к вычислительных узлов и (или) подсети узла вычислений для задач с несколькими экземплярами Linux, если это необходимо. | Н/Д | * | Все | 3389 (Windows), 22 (Linux) | TCP | Allow |

**Правила безопасности для исходящего трафика**

| `Source` | Исходные порты | Место назначения | Тег службы назначения | Протокол | Действие |
| --- | --- | --- | --- | --- | --- |
| Все | 443 | [Тег службы](../articles/virtual-network/security-overview.md#service-tags) | `Storage` (в одном регионе с вашей учетной записи пакетной службы и виртуальной сети)  | Все | Allow |
...
| --- | --- | --- | --- | --- | --- |
Все <br /><br />Хотя фактически это разрешает доступ со всех адресов, пакетная служба применяет правило списка управления доступом на уровне каждого узла, которое отфильтровывает все IP-адреса, не относящиеся к пакетной службе. | * | Все | 10100, 20100, 30100 | TCP | Allow |
| (Необязательно) чтобы предоставить доступ RDP на вычислительных узлах. | * | Все | 3389 | TCP | Allow |

**Правила безопасности для исходящего трафика**

| `Source` | Исходные порты | Место назначения | Конечные порты | Протокол | Действие |
| --- | --- | --- | --- | --- | --- |
| Все | * | Все | 443  | Все | Allow |
