...
* Частные конечные точки - Примечание: DNS должны управляться отдельно, а не с помощью частных зон Azure DNS.
...
* Приложение не может устраять адреса в частных зонах Azure DNS.
