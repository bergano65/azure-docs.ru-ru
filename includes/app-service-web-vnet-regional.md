...
Эти параметры будут отсылать все исходящие вызовы из приложения в виртуальную сеть. Кроме того, оно позволит приложению использовать Azure DNS, запрашивая зону Частная зона DNS на уровне рабочей роли. Эта функция используется, когда работающее приложение обращается к зоне Частная зона DNS.
