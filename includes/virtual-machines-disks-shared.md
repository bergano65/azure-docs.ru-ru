...
Виртуальные машины в кластере могут выполнять чтение или запись на подключенный диск в соответствии с резервированием, выбранным кластеризованным приложением, с помощью [постоянного резервирования SCSI](https://www.t10.org/members/w_spc3.htm) (SCSI PR). SCSI PR — это промышленный стандарт, используемый приложениями, работающими в локальных сетях хранения данных (SAN). Включение SCSI PR на управляемом диске позволяет перенести эти приложения в Azure "как есть".

Общий доступ к управляемым дискам обеспечивает общее блочное хранилище, доступ к которому можно получить из нескольких виртуальных машин. Последние предоставляются в виде логических номеров устройств (LUN). Затем LUN представляются инициатору (виртуальной машине) из целевого объекта (диска). Эти LUN представляются виртуальной машине дисками с прямым подключением (DAS) или локальными дисками.
...
Большинство кластеров на основе Windows использует платформу WSFC, которая обеспечивает всю основную инфраструктуру для обмена данными с узлами кластера, позволяя приложениям использовать шаблоны параллельного доступа. WSFC поддерживает решения на основе CSV и других форматов в зависимости от используемой версии Windows Server. Дополнительные сведения см. в разделе [Создание отказоустойчивого кластера](https://docs.microsoft.com/windows-server/failover-clustering/create-failover-cluster).
...
Сведения о том, как настроить высокий уровень доступности Ubuntu с помощью Corosync и Pacemaker на общих дисках Azure, можно найти в [Обсуждениях сообщества Ubuntu](https://discourse.ubuntu.com/t/ubuntu-high-availability-corosync-pacemaker-shared-disk-environments/14874).
...
:::image type="content" source="media/virtual-machines-disks-shared-disks/ultra-reservation-table.png" alt-text="Изображение таблицы, в которой показан доступ только для чтения и доступ на чтение и запись для владельца резервирования, зарегистрированного пользователя и прочих.":::

## <a name="performance-throttles"></a>Регулировки производительности

### <a name="premium-ssd-performance-throttles"></a>Регулирование производительности SSD уровня "Премиум"
При использовании SSD уровня "Премиум" дисковые операции ввода-вывода и пропускная способность фиксируются, например, число операций ввода-вывода P30 — 5000. Это значение остается, если диск является общим для двух виртуальных машин или 5 виртуальных машин. Ограничения на доступ к диску можно получить с одной виртуальной машины или разделить на две или несколько виртуальных машин. 
...
|DiskIOPSReadOnly*     |Общее разрешенное количество операций ввода-вывода в секунду на всех виртуальных машинах, подключенных к общему диску с доступом только для чтения.         |
|DiskMBpsReadOnly*     |Общая разрешенная пропускная способность (МБ/с) на всех виртуальных машинах, подключенных к общему диску с доступом только для чтения.         |
