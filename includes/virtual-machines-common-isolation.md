...
<sup>*</sup>Гарантия изоляции будет прекращена до 15 мая 2020 г.

## <a name="retiring-d15_v2ds15_v2-isolation-on-may-15-2020"></a>Снятие изоляции D15_v2 и DS15_v2 на 15 мая 2020 г.
**Обновление 10 февраля 2020: временная шкала "изоляция" была расширена до 15 мая 2020 "**

Выделенный узел Azure теперь является общедоступным, что позволяет запускать виртуальные машины Linux и Windows на физических серверах с одним клиентом. Мы планируем полностью заменить изолированные виртуальные машины Azure выделенным узлом Azure. После **15 мая 2020** виртуальные машины Azure D15_v2/DS15_v2 больше не будут изолированы от оборудования.

## <a name="how-does-this-affect-me"></a>Как это повлияет на работу?
После 15 мая 2020 мы больше не будем предоставлять гарантию изоляции для виртуальных машин Azure D15_v2 и DS15_v2. 
...
Если требуется изоляция, то до 15 мая 2020 необходимо выполнить одно из следующих действий.

• [Перенос](https://azure.microsoft.com/blog/introducing-azure-dedicated-host) рабочей нагрузки на выделенный узел Azure.
...
### <a name="q-is-the-size-dds15_v2-going-to-get-retired"></a>Вопрос. размер D/DS15_v2 будет снят с учета?
Ответ **. нет**, будет снята только функция "изоляция". Если изоляция не требуется, никаких действий предпринимать не нужно.

### <a name="q-is-the-size-dds15i_v2-going-to-get-retired"></a>Вопрос. размер D/DS15i_v2 будет снят с учета?
Ответ **. Да**, размер доступен только до 15 мая 2021. Для клиентов, которые приобрели 3-летний RIs на D/DS15_v2 до 18 ноября, 2019 будет иметь доступ к D/DS15i_v2 до 17 ноября 2022.
...
| --- | --- | --- |
|   D15_v2  |   Выключено     |   D15_v2 и D15i_v2 |    
|   D15_v2  |   Включено  |   D15_v2 ряд и D15i_v2 получат преимущество RI. |    
|   D14_v2  |   Включено  |   D15_v2 ряд и D15i_v2 получат преимущество RI. |    
 
Аналогично для ряда Dsv2.
 
### <a name="q-i-want-to-purchase-additional-reserved-instances-for-dv2-which-one-should-i-choose"></a>Вопрос. я хочу приобрести дополнительные зарезервированные экземпляры для Dv2. Что из них выбрать?
Ответ **. все**службы RIS, приобретенные после 18 ноября 2019, имеют следующее поведение. 

| RI |  Гибкость размера экземпляра | Права на преимущества |   
| --- | --- | --- |
| D15_v2 |  Выключено |   Только D15_v2  
| D15_v2 |  Включено |    D15_v2 ряд получит преимущество RI. Новый D15i_v2 не будет иметь права на использование RI из этого типа RI. | 
| D15i_v2 |     Выключено | Только D15i_v2 |  
| D15i_v2 |     Включено  | Только D15i_v2 | 
...
Ответ. это действие не является **обязательным, поскольку**преимущество будет применено к изолированным и неизолированным размерам. Но Azure будет поддерживать изменение существующих зарезервированных экземпляров D15_v2 и DS15_v2 для D15i_v2 и DS15i_v2. Для всех остальных зарезервированных экземпляров Dv2/Dsv2 используйте существующий зарезервированный экземпляр или купите новые зарезервированные экземпляры для изолированных размеров.
