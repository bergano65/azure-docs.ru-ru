...
1. Запустите Internet Explorer и перейдите по [http://catalog.update.microsoft.com](https://catalog.update.microsoft.com)адресу.
...
4. Нажмите **Загрузить**. Укажите или **выберите** локальное расположение, в которое хотите скачать файлы. Выберите файлы для загрузки в указанное расположение и папку. Этот каталог также можно скопировать в сетевую папку, которая доступна с вашего устройства.
...
8. Повторите шаги 3–5, чтобы установить обновления второго типа. **Для обновления второго порядка можно установить несколько обновлений, просто запустив `Start-HcsHotfix cmdlet` и указав папку, где находятся обновления второго порядка. Командлет выполнит все обновления, доступные в папке.** Если обновление уже установлено, логика обновления обнаружит это и не установит его. 
...
1. Переведите устройство в режим обслуживания. **Обратите внимание, что при подключении к устройству в режиме обслуживания не следует использовать удаленное взаимодействие Windows PowerShell. Вместо этого запустите этот командлет на контроллере устройства при подключении через последовательную консоль устройства.** Тип:
