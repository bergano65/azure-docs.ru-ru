...
1. Запустите Internet Explorer [http://catalog.update.microsoft.com](https://catalog.update.microsoft.com)и перейдите к .
...
3. В поле поиска каталога Центра обновления Майкрософт введите номер необходимого исправления в базе знаний, например **4011839**, а затем нажмите кнопку **Найти**.
   
    Появится список исправлений, например **Совокупное обновление программного обеспечения версии 4.0 для StorSimple серии 8000**.
   
    ![Поиск в каталоге](./media/storsimple-install-update2-hotfix/HCS_SearchCatalog1-include.png)

4. Нажмите кнопку **Загрузить**. Укажите или **выберите** локальное расположение, в которое хотите скачать файлы. Выберите файлы для загрузки в указанное расположение и папку. Этот каталог также можно скопировать в сетевую папку, которая доступна с вашего устройства.
5. Найдите дополнительные исправления, перечисленные в приведенной выше таблице (**4011841**), и скачайте соответствующие файлы в папки, указанные в этой таблице.
...
Выполните следующие действия для установки и проверки обычных исправлений. Если вы уже установили их с помощью классического портала Azure, перейдите к [установке и проверке исправлений режима обслуживания](#to-install-and-verify-maintenance-mode-hotfixes).
...
2. Выберите вариант 1, **Log in with full access** (Вход с правами полного доступа). Рекомендуем сначала установить исправление на пассивный контроллер.
3. Чтобы установить исправление, в командной строке введите следующую команду:
    ...
    Ниже приведен пример выходных данных для установки обновлений первого типа. Для обновления первого типа необходимо указать определенный файл.
...
6. Когда установка обновления будет завершена, проверьте версии программного обеспечения системы. Тип:
...
8. Повторите шаги 3–5, чтобы установить обновления второго типа. **Для обновления второго заказа, несколько обновлений могут быть установлены, просто `Start-HcsHotfix cmdlet` запустив и указывая на папку, где второй заказ обновления расположены. Cmdlet будет выполнять все обновления, доступные в папке.** Если обновление уже установлено, логика обновления обнаружит это и не установит его. 
...
Используйте статью KB4011837, чтобы установить обновления встроенного ПО дисков. Эти обновления прерывают работу, и на их завершение может потребоваться около 30 минут. Данные обновления можно установить в период планового техобслуживания, подключившись к последовательной консоли устройства.
...
1. Переведите устройство в режим обслуживания. **Обратите внимание, что при подключении к устройству в режиме обслуживания не следует использовать ремотивирование Windows PowerShell. Вместо этого запустите этот cmdlet на контроллере устройства при подключении через серийную консоль устройства.** Тип:
...
4. После завершения установки контроллер, на котором установлено исправление режима обслуживания, будет перезагружен. Выберите вариант 1, **Log in with full access** (Вход с правами полного доступа), и проверьте версию встроенного ПО диска. Тип:
