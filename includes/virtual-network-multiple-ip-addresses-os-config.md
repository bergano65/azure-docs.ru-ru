...
5. Выберите **Использовать следующий IP-адрес** и введите следующие значения:
    ...
    * **Маска подсети**: Набор, в зависимости от своей подсети. Например, если подсеть — /24, то маска подсети — 255.255.255.0.
    ...
    * Щелкните **Использовать следующие адреса DNS-серверов** и введите следующие значения:
        * **Основной DNS-сервер**: Если вы не используете собственный DNS-сервер, введите 168.63.129.16.  В противном случае введите IP-адрес этого DNS-сервера.
...
Мы рекомендуем, просмотрев последнюю версию документации по дистрибутиву Linux. 
