...
**Docker**. Для выполнения действий, описанных в этом учебнике, требуется базовое понимание таких основных понятий Docker, как контейнеры и образы контейнеров, а также знание основных команд `docker`. См. [общие сведения о Docker и контейнерах][docker-get-started].

**Модуль Docker**. Для работы с этим учебником требуется модуль Docker, установленный локально. Docker предоставляет пакеты, которые настраивают среду с Docker для [macOS][docker-mac], [Windows][docker-windows] и [Linux][docker-linux].
