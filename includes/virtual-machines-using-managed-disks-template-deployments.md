...
Чтобы создать виртуальную машину с управляемыми дисками, вам больше не нужно создавать ресурс учетной записи хранения. Ссылка на приведенный ниже пример шаблона содержит некоторые отличия от предыдущих примеров неуправляемого диска, которые следует отметить:

- `apiVersion` — это версия, поддерживающая управляемые диски.
- `osDisk` и `dataDisks` больше не ссылаются на конкретный универсальный код ресурса (URI) для виртуального жесткого диска.
...
- Если имя диска не указано, оно принимает формат `<VMName>_OsDisk_1_<randomstring>` для диска ОС и `<VMName>_disk<#>_<randomstring>` для каждого диска данных.
