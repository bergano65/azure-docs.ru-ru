...
|траккингстореконнектионстрингнаме||Имя строки подключения, используемой для таблиц журнала и экземпляров. Если значение не указано, `azureStorageConnectionStringName` используется соединение.|
|траккингсторенамепрефикс||Префикс, используемый для таблиц журнала и экземпляров, если `trackingStoreConnectionStringName` указан параметр. Если значение не задано, то префиксом по умолчанию будет `DurableTask` . Если `trackingStoreConnectionStringName` параметр не указан, то таблицы журнала и экземпляров будут использовать значение в `hubName` качестве префикса, а все параметры для `trackingStoreNamePrefix` будут пропущены.|
