...
|траккингстореконнектионстрингнаме||Имя строки подключения, используемой для таблиц журнала и экземпляров. Если значение `azureStorageConnectionStringName` не указано, используется соединение.|
|траккингсторенамепрефикс||Префикс, используемый для таблиц журнала и экземпляров, если `trackingStoreConnectionStringName` указан параметр. Если значение не задано, то префиксом по `DurableTask`умолчанию будет. Если `trackingStoreConnectionStringName` параметр не указан, то таблицы журнала и экземпляров будут использовать `hubName` значение в качестве префикса, а все параметры для `trackingStoreNamePrefix` будут пропущены.|
