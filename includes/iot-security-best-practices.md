...
* **Сохраняйте безопасность ключей аутентификации:** во время развертывания каждому устройству требуются идентификации устройств и связанные с ними ключи аутентификации, генерируемые облачной службой. Эти ключи следует хранить в физически безопасном месте даже после развертывания. Вредоносное устройство может использовать любой скомпрометированный ключ для маскировки под существующее устройство.
...
* **Физически защитить инфраструктуру IoT**: Наихудшие атаки безопасности на инфраструктуру IoT запускаются с использованием физического доступа к устройствам. Очень важно обеспечить защиту USB-портов и других физически доступных компонентов от атак злоумышленников. Одним из ключей к обнаружению брешей является ведение журналов физического доступа, например использования USB-портов. Опять же, в Windows 10 (IoT и другие SKU) доступна возможность подробного ведения журнала таких событий.

* **Защита учетных данных облака:** учетные данные проверки подлинности облака, используемые для настройки и работы с развертыванием IoT, возможно, являются самым простым способом получить доступ и скомпрометировать систему IoT. Чтобы защитить учетные данные, следует часто изменять пароль и не использовать их на общедоступных компьютерах.
