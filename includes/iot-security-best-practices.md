...
* **Обеспечьте безопасность ключей проверки подлинности**: во время развертывания для каждого устройства требуются идентификаторы устройств и связанные ключи проверки подлинности, созданные облачной службой. Эти ключи следует хранить в физически безопасном месте даже после развертывания. Вредоносное устройство может использовать любой скомпрометированный ключ для маскировки под существующее устройство.
...
* **Физическая защита инфраструктуры IOT**. наиболее подходящими атаками безопасности для инфраструктуры IOT можно воспользоваться с помощью физического доступа к устройствам. Очень важно обеспечить защиту USB-портов и других физически доступных компонентов от атак злоумышленников. Одним из ключей к обнаружению брешей является ведение журналов физического доступа, например использования USB-портов. Опять же, в Windows 10 (IoT и другие SKU) доступна возможность подробного ведения журнала таких событий.

* **Защита облачных учетных данных**. учетные данные для облачной проверки подлинности, используемые для настройки и работы развертывания IOT, возможно, являются самым простым способом получить доступ к системе IOT Чтобы защитить учетные данные, следует часто изменять пароль и не использовать их на общедоступных компьютерах.
