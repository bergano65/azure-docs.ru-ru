...
> [!NOTE]
> Не забудьте [синхронизировать учетные данные DS DS с Azure AD,](../articles/active-directory/hybrid/how-to-connect-install-roadmap.md) если вы планируете использовать свой предприимченные DD для проверки подлинности. Синхронизация хэша паролей от AD DS до Azure AD не является обязательной. Разрешение уровня общего наодиные данные будет предоставлено идентатору Azure AD, синхронизированному с вашим a-premises AD DS.
...
1. Откройте Windows File Explorer и нажмите право на файл/каталог и выберите **Свойства.**
2. Перейдите на вкладку **Безопасность**.
3. Выберите **edit..** изменить разрешения.
4. Вы можете изменить разрешения существующих пользователей или выбрать **Добавить...**
5. В оперативном окне для добавления новых пользователей введите имя целевого пользователя, которое вы хотите предоставить в **Введите имена объектов, чтобы выбрать** окно, и выберите Проверить **имена,** чтобы найти полное имя пользователя UPN.
7.    Щелкните **ОК**.
...
Восвай в VM, используя иденцию Azure AD, на которую вы предоставили разрешения, как показано на следующем изображении. Если вы включили предпосылок проверки подлинности AD DS для файлов Azure, используйте учетные данные AD DS. Для проверки подлинности Azure AD DS впишитесь в систему с учетными данными Azure AD.
...
Используйте следующую команду для установки общего файла Azure. Не забудьте заменить значения заполнителей собственными значениями. Поскольку вы прошли аутентификации, вам не нужно предоставлять ключ учетной записи хранилища, учетные данные AD DS в помещениях или учетные данные Azure AD DS. Единый опыт регистрации поддерживается для проверки подлинности либо с помощью на месте AD DS, либо с Azure AD DS. Если вы столкнулись с проблемами, нарастающим с учетными данными AD DS, обратитесь к [проблемам Troubleshoot Azure Files в Windows](https://docs.microsoft.com/azure/storage/files/storage-troubleshoot-windows-file-connection-problems) для руководства.
