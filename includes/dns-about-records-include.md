...
ms.locfileid: "73832054"
---
### <a name="record-names"></a>Имена записей

В Azure DNS записи указываются с помощью относительных имен. *Полностью квалифицированное* доменное имя (ФЗДН) включает имя зоны, в то время как *относительное* имя не включает в себя. Например, относительное `www` имя записи в зоне `contoso.com` `www.contoso.com`дает полностью квалифицированное имя записи.

Запись *вершины* — это запись DNS в корне (или *вершины*) зоны DNS. Например, в зоне `contoso.com`DNS запись вершины также имеет `contoso.com` полностью квалифицированное имя (это иногда называют *голым* доменом).  По соглашению относительное имя \@ используется для предоставления записей вершины.
