...
ms.locfileid: "86050281"
---
### <a name="record-names"></a>Имена записей

В Azure DNS записи указываются с помощью относительных имен. Полное *доменное* имя включает имя зоны, а *относительное* имя — нет. Например, относительное имя записи `www` в зоне `contoso.com` дает полное имя записи `www.contoso.com` .

Запись *вершины* — это запись DNS в корне (или *вершины*) зоны DNS. Например, в зоне DNS `contoso.com` запись вершине также имеет полное имя `contoso.com` (это иногда называется доменом с именем *naked* ).  По соглашению относительное имя \@ используется для предоставления записей вершины.
