...
В Azure DNS записи указываются с помощью относительных имен. *Полное доменное* имя включает в себя имя зоны, а *относительное* имя — нет. Например, относительное имя записи `www` в зоне `contoso.com` предоставляет полное имя записи `www.contoso.com`.

Запись *вершины* — это запись DNS в корне (или *вершины*) зоны DNS. Например, в `contoso.com`зоны DNS в записи вершине также содержится полное имя `contoso.com` (это иногда называется доменом с именем *naked* ).  По соглашению относительное имя \@ используется для предоставления записей вершины.
