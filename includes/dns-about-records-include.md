...
В Azure DNS записи указываются с помощью относительных имен. Полное *доменное* имя включает имя зоны, а *относительное* имя — нет. Например, относительное имя `www` записи в зоне `contoso.com` дает полное имя `www.contoso.com`записи.

Запись *вершины* — это запись DNS в корне (или *вершины*) зоны DNS. Например, в зоне `contoso.com`DNS запись вершине также имеет полное имя `contoso.com` (это иногда называется доменом с именем *naked* ).  По соглашению относительное имя \@ используется для предоставления записей вершины.
