...
> [!NOTE]
> Иногда проблема, которая выглядит как недостаточная производительность дисков, на самом деле вызвана узким местом в сети. В таких ситуациях следует улучшить [производительность сети](../articles/virtual-network/virtual-network-optimize-network-bandwidth.md).
> Если виртуальная машина поддерживает ускоренную сеть, необходимо включить эту функцию. Если она не включена, ее можно включить на уже развернутых виртуальных машинах под управлением [Windows](../articles/virtual-network/create-vm-accelerated-networking-powershell.md#enable-accelerated-networking-on-existing-vms) и [Linux](../articles/virtual-network/create-vm-accelerated-networking-cli.md#enable-accelerated-networking-on-existing-vms).

Если вы не знакомы с хранилищем класса "Премиум", перед началом работы ознакомьтесь со статьями о [выборе типа диска Azure для виртуальных машин IaaS](../articles/virtual-machines/windows/disks-types.md) и [целевых показателях масштабируемости и производительности службы хранилища Azure](../articles/storage/common/storage-scalability-targets.md).
...
Следующие операции плоскости управления на управляемых дисках может потребоваться перемещение диска из одного расположения хранения в другую. Это осуществляется с помощью копия данных, может занять несколько часов, обычно менее 24 часов в зависимости от объема данных на дисках в фоновом режиме. В течение этого времени приложение может столкнуться с больше, чем обычные задержки при чтении, так как некоторые операции чтения перенаправлялись в исходное расположение и может занять больше времени. В этот период нет влияния на задержку записи.

- Обновите тип хранилища.
- Отсоединения и присоединения диска с одной виртуальной Машины на другой.
- Создание управляемого диска из виртуального жесткого диска.
- Создание управляемого диска из моментального снимка.
- Преобразованию неуправляемых дисков на управляемые диски.
...
Основные факторы, влияющие на производительность приложения, работающего в хранилище класса Premium являются запросов характера ввода-ВЫВОДА, размер виртуальной Машины, размер диска, количество дисков, кэширование диска, многопоточность и длина очереди. Некоторыми из этих факторов можно управлять с помощью средств, предоставляемых системой. В большинстве приложений не предусмотрена возможность изменения объема операций ввода-вывода и длины очереди напрямую. Например, при использовании SQL Server нельзя выбрать значения этих показателей. SQL Server выбирает оптимальные значения объема операций ввода-вывода и длины очереди для получения лучшей производительности. Важно понимать, какое влияние оба показателя оказывают на производительность приложения, чтобы выделить объем ресурсов в соответствии с потребностями в производительности.

Определить нужный уровень оптимизации производительности приложения можно с помощью созданного ранее контрольного списка требований приложения. Исходя из этого, можно определить, какие показатели из этого раздела необходимо настроить. Чтобы убедиться во влиянии каждого из показателей на производительность приложения, запустите инструменты тестирования производительности на установленном приложении. См. в разделе "измерение производительности" в конце этой статьи для действия для запуска общих инструментов тестирования производительности в Windows и виртуальных машин Linux.

### <a name="optimize-iops-throughput-and-latency-at-a-glance"></a>Оптимизация операций ввода-ВЫВОДА, пропускной способности и задержки с первого взгляда

В таблице ниже приведены факторы производительности и шаги, необходимые для оптимизации операций ввода-ВЫВОДА, пропускной способности и задержки. В следующих разделах более подробно описан каждый из показателей.
...
| **Чередование дисков** |Используйте несколько дисков и чередуйте их, чтобы повысить общее ограничение количества операций ввода-вывода и пропускной способности. Общее ограничение для каждой виртуальной Машины должно быть больше, чем общие ограничения подключенных дисков. | &nbsp; | &nbsp; |
| **Размер блока чередования** |Меньший размер блока чередования для случайных небольших запросов ввода-вывода в приложениях OLTP. Например используйте размер блока чередования 64 КБ для приложения SQL Server OLTP. |Больший размер блока чередования для последовательных объемных запросов ввода-вывода в приложениях хранилища данных. Например используйте размер блока чередования на 256 КБ для приложения хранилища данных SQL Server. | &nbsp; |
...
Запрос ввода-вывода — это единица операции ввода-вывода, которую будет выполнять приложение. Определив характер запроса ввода-вывода (случайный или последовательный, чтение или запись, большого или небольшого размера), можно определить требования к производительности приложения. Важно понимать характер запросов ввода-ВЫВОДА, для принятия правильных решений при проектировании инфраструктуры приложений.

Один из наиболее важных факторов — объем ввода-вывода. Это объем запроса на операцию ввода-вывода, созданного приложением. Объем ввода-вывода оказывает существенное влияние на производительность, особенно на показатели количества операций ввода-вывода и пропускной способности, которых может достигнуть приложение. Следующая формула показана связь между операций ввода-ВЫВОДА, объем ввода-ВЫВОДА и пропускной способности.  
...
* Приложения OLTP генерируют миллионы небольших случайных запросов ввода-вывода. Для обработки этих типов запросов ввода-ВЫВОДА, необходимо разработать инфраструктуру приложения, чтобы получить более операций ввода-ВЫВОДА.  
* Приложения хранилища данных генерируют крупные последовательные запросы ввода-вывода. Для обработки этих типов запросов ввода-ВЫВОДА, необходимо разработать инфраструктуру приложения, чтобы получить более пропускной способности или пропускная способность.
...
Чтобы убедиться во влиянии объема ввода-вывода на производительность приложения, можно запустить инструменты тестирования производительности на виртуальной машине и дисках. Выполните несколько тестовых запусков и используйте для каждого из них разный объем ввода-вывода, чтобы увидеть влияние. См. в разделе "измерение производительности" в конце этой статьи для получения дополнительных сведений.
...
Высокомасштабируемые виртуальные машины доступны в разных размеров с количеством ядер ЦП, памяти, ОС и размер временного диска. Для каждого размера виртуальной машины также установлено максимальное количество дисков данных, которые можно подключить к виртуальной машине. Таким образом, выбранный размер виртуальной машины влияет на мощность обработки, объем памяти и хранилища, которые доступны для приложения. Он также влияет на затраты на вычисление и хранение. Например, ниже представлены спецификации виртуальной машины максимального размера серий DS, DSv2 и GS:

| Размер виртуальной машины | Ядра ЦП | Память | Размеры диска виртуальной машины | Макс. количество дисков данных | Объем кэша | IOPS | Ограничения количества операций ввода-вывода в секунду и пропускной способности кэша |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
В качестве примера рассмотрим приложение, требующее выполнения 16 000 операций ввода-вывода в секунду. Для достижения такой производительности требуется стандартный\_D14 виртуальных Машин Azure IaaS, который может предоставить максимальное число IOPS для 16 000, используя 32 диска емкостью 1 ТБ хранилища класса standard. Каждый диск хранилища класса standard с объемом 1 ТБ может выполнить до 500 операций ввода-вывода. Оценочная стоимость этой виртуальной машины в месяц составляет 1570 долл. США. Ежемесячная стоимость 32 дисков хранилища класса «Стандартный» составляет 1638 долл. США. Оценочная общая ежемесячная стоимость составляет 3208 долл. США.
...
| **Стоимость дисков в месяц** |$1,638.40 (диски объемом 32 x 1 ТБ) |544,34 долл. США (4 диска P30) |
...
Хранилище Azure класса Premium предлагает восемь размеры дисков общедоступной версии и трех размеров диска, которые в настоящее время находятся в предварительной версии. Каждый из них имеет различные ограничения масштабирования для операций ввода-ВЫВОДА, пропускной способности и хранилища. Выберите диск хранилища класса «Премиум» правильного размера в соответствии с требованиями приложения и размером высокомасштабируемой виртуальной машины. В следующей таблице показаны размеров 11 дисков и их возможности. Размеры P4, P6, P15, P60, P70 и P80 сейчас поддерживаются только на управляемых дисках.
...
> [!WARNING]
> Кэширование диска не поддерживается для дисков размером более 4 Тиб. Если несколько дисков уже подключены к виртуальной Машине каждого диска, который является 4 Тиб или более мелкие поддержку кэширования.
>
...
Выберите подходящий размер блока чередования в зависимости от типа рабочей нагрузки приложения. Для случайных небольших запросов ввода-вывода следует использовать меньший размер блока чередования, В то время как для больших последовательных операций ввода-ВЫВОДА запросов используйте больший размер блока чередования. Узнайте больше о рекомендациях по размеру блока чередования для приложения, которое будет запущено в хранилище класса «Премиум». Для SQL Server настройте размер блока чередования 64 КБ для рабочих нагрузок OLTP и 256 КБ для рабочих нагрузок хранилища данных. в статье [Рекомендации по оптимизации производительности SQL Server на виртуальных машинах Azure](../articles/virtual-machines/windows/sql/virtual-machines-windows-sql-performance.md#disks-guidance) .
...
Длина очереди "или" Длина очереди "или" размер очереди — это количество ожидающих запросов ввода-ВЫВОДА в системе. Значение длины очереди определяет, сколько операций ввода-ВЫВОДА, приложение можно выровнять, который их обработки дисками хранилища. Это влияет на все три показателя производительности приложения, которые мы обсуждали в этой статье а именно на операции, операций ввода-ВЫВОДА, пропускной способности и задержки.
...
Как правило, приложение может достигать максимальной пропускной способности при наличии 8–16 и более ожидающих выполнения операций ввода-вывода на подключенный диск. Если длина очереди, приложение не отправляет достаточное количество операций ввода-вывода в системе, и он будет обрабатывать меньше операций в течение указанного периода. Другими словами, пропускная способность будет меньше.
