...
> [!NOTE]
> Иногда то, что можно принять за проблему с производительностью, на самом деле является узким местом в сети. В таких ситуациях следует улучшить [производительность сети](../articles/virtual-network/virtual-network-optimize-network-bandwidth.md).
> Также убедитесь в том, что ваша виртуальная машина поддерживает ускорение работы в сети. Если поддерживает, вы можете включить эту функцию на виртуальных [Windows](../articles/virtual-network/create-vm-accelerated-networking-powershell.md#enable-accelerated-networking-on-existing-vms) и [Linux](../articles/virtual-network/create-vm-accelerated-networking-cli.md#enable-accelerated-networking-on-existing-vms) даже после развертывания.
