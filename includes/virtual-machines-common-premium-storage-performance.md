...
Следующие операции плоскости управления на управляемых дисках могут включать перемещение диска из одного места хранения в другое. Это организовано через фоновую копию данных, которые могут занять несколько часов, как правило, менее 24 часов в зависимости от объема данных на дисках. За это время ваше приложение может испытывать более высокую, чем обычно, задержку чтения, так как некоторые чтения могут быть перенаправлены в исходное местоположение и могут занять больше времени. В этот период нет влияния на задержку записи.

- Обновление типа хранилища.
- Отсоедините и прикрепите диск от одного VM к другому.
- Создайте управляемый диск с VHD.
- Создайте управляемый диск из снимка.
- Преобразование неуправляемых дисков в управляемые диски.
...
| **Задержки** |Общее время завершения запроса ввода-вывода на диске. |Средняя скорость чтения с диска в секунду  <br>  Средняя скорость записи на диск в секунду |await  <br>  svctm |
...
| **Макс. Памяти** |Объем памяти, необходимый для стабильной работы приложения |Использование выделенной памяти (в байтах), % |Использование vmstat |
| **Макс. Процессора** |Объем ЦП, необходимый для стабильной работы приложения |Загруженность процессора, % |%util |
...
Основными факторами, влияющими на производительность приложения, работающего на Premium Storage, являются Nature of IO-запросы, размер VM, размер диска, количество дисков, кэширование дисков, многопоточность и глубина очереди. Некоторыми из этих факторов можно управлять с помощью средств, предоставляемых системой. В большинстве приложений не предусмотрена возможность изменения объема операций ввода-вывода и длины очереди напрямую. Например, при использовании SQL Server нельзя выбрать значения этих показателей. SQL Server выбирает оптимальные значения объема операций ввода-вывода и длины очереди для получения лучшей производительности. Важно понимать, какое влияние оба показателя оказывают на производительность приложения, чтобы выделить объем ресурсов в соответствии с потребностями в производительности.

Определить нужный уровень оптимизации производительности приложения можно с помощью созданного ранее контрольного списка требований приложения. Исходя из этого, можно определить, какие показатели из этого раздела необходимо настроить. Чтобы убедиться во влиянии каждого из показателей на производительность приложения, запустите инструменты тестирования производительности на установленном приложении. Обратитесь к статье Benchmarking, связанной в конце, для шагов для запуска общих инструментов бенчмаркинга на Windows и Linux VMs.

### <a name="optimize-iops-throughput-and-latency-at-a-glance"></a>Оптимизация IOPS, пропускной их пропускной столик, и задержка с первого взгляда

В приведенной ниже таблице кратко излагаются факторы производительности и шаги, необходимые для оптимизации IOPS, пропускной способности и задержки. В следующих разделах более подробно описан каждый из показателей.
...
| &nbsp; | **ОПЕРАЦИЙ ВВОДА-ВЫВОДА** | **Пропускная способность** | **Задержки** |
| --- | --- | --- | --- |
...
| **Размер VM** |Используйте размер виртуальной машины, который обеспечивает более высокую скорость выполнения операций ввода-вывода, чем указано в требованиях приложения. |Используйте размер виртуальной машины с большей пропускной способностью, чем требует приложение. |Используйте размер виртуальной машины с более широкими пределами масштабируемости, чем требует приложение. |
...
| **Чередование дисков** |Используйте несколько дисков и чередуйте их, чтобы повысить общее ограничение количества операций ввода-вывода и пропускной способности. Совокупный лимит на VM должен быть выше, чем совокупные лимиты прикрепленных премиальных дисков. | &nbsp; | &nbsp; |
| **Размер блока чередования** |Меньший размер блока чередования для случайных небольших запросов ввода-вывода в приложениях OLTP. Например, используйте размер полосы 64 КБ для приложения S'L Server OLTP. |Больший размер блока чередования для последовательных объемных запросов ввода-вывода в приложениях хранилища данных. Например, используйте размер полосы 256 КБ для приложения для склада серверных данных S'L Server Data. | &nbsp; |
...
Запрос ввода-вывода — это единица операции ввода-вывода, которую будет выполнять приложение. Определив характер запроса ввода-вывода (случайный или последовательный, чтение или запись, большого или небольшого размера), можно определить требования к производительности приложения. Важно понимать характер запросов обработки данных, принимать правильные решения при проектировании инфраструктуры приложений. IOs должны распределяться равномерно для достижения наилучшей производительности возможно.

Один из наиболее важных факторов — объем ввода-вывода. Это объем запроса на операцию ввода-вывода, созданного приложением. Объем ввода-вывода оказывает существенное влияние на производительность, особенно на показатели количества операций ввода-вывода и пропускной способности, которых может достигнуть приложение. Следующая формула показывает взаимосвязь между IOPS, размером IO и пропускной способностью/пропускной способностью.  
...
* Приложения OLTP генерируют миллионы небольших случайных запросов ввода-вывода. Для обработки этих типов запросов на обработку данных необходимо разработать инфраструктуру приложений, чтобы получить более высокий ioPS.  
* Приложения хранилища данных генерируют крупные последовательные запросы ввода-вывода. Для обработки этих типов запросов на обработку данных необходимо разработать инфраструктуру приложений, чтобы получить более высокую пропускную способность или пропускную способность.
...
Чтобы убедиться во влиянии объема ввода-вывода на производительность приложения, можно запустить инструменты тестирования производительности на виртуальной машине и дисках. Выполните несколько тестовых запусков и используйте для каждого из них разный объем ввода-вывода, чтобы увидеть влияние. Для более подробной информации обратитесь к статье Benchmarking, связанной в конце.

## <a name="high-scale-vm-sizes"></a>Размеры высокомасштабируемых виртуальных машин

При разработке приложения нужно в первую очередь выбрать виртуальную машину для размещения приложения. Хранилище класса «Премиум» предусматривает использование высокомасштабируемых виртуальных машин, на которых могут выполняться приложения, требующие высокой вычислительной мощности и повышенной производительности операций ввода-вывода локального диска. Эти виртуальные машины отличаются более быстрыми процессорами, более высоким соотношением «память — ядро» и использованием твердотельного накопителя (SSD) в качестве локального диска. Примерами высокомасштабных ВМ, поддерживающих Премиум-хранилище, являются VM серии DS и GS.

Высокомасштабные VMs доступны в различных размерах с разным количеством ядер процессора, памяти, ОС и временного размера диска. Для каждого размера виртуальной машины также установлено максимальное количество дисков данных, которые можно подключить к виртуальной машине. Таким образом, выбранный размер виртуальной машины влияет на мощность обработки, объем памяти и хранилища, которые доступны для приложения. Он также влияет на затраты на вычисление и хранение. Например, ниже приведены спецификации самого большого размера VM в серии DS и серия GS:
...
В качестве примера рассмотрим приложение, требующее выполнения 16 000 операций ввода-вывода в секунду. Для достижения этой производительности вам\_понадобится стандартный D14 Azure IaaS VM, который может дать максимальный IOPS 16 000 с помощью 32 стандартных дисков хранения 1 ТБ. Каждый 1-ТБ стандартный диск хранения может достигать максимум 500 IOPS. Оценочная стоимость этой виртуальной машины в месяц составляет 1570 долл. США. Ежемесячная стоимость 32 дисков хранилища класса «Стандартный» составляет 1638 долл. США. Оценочная общая ежемесячная стоимость составляет 3208 долл. США.
...
| **Стоимость дисков в месяц** |$1,638.40 (32 х 1-ТБ дисков) |544,34 долл. США (4 диска P30) |
...
Azure Premium Storage предлагает различные размеры, так что вы можете выбрать тот, который наилучшим образом соответствует вашим потребностям. Каждый размер диска имеет различные ограничения масштаба для IOPS, пропускной способности и хранения. Выберите диск хранилища класса «Премиум» правильного размера в соответствии с требованиями приложения и размером высокомасштабируемой виртуальной машины. В таблице ниже показаны размеры дисков и их возможности. Размеры P4, P6, P15, P60, P70 и P80 сейчас поддерживаются только на управляемых дисках.
...
> [!WARNING]
> Кэширование диска не поддерживается для дисков 4 TiB и больше. Если к VM прикреплено несколько дисков, каждый диск, который меньше 4 TiB, будет поддерживать кэширование.
>
...
| **Настройка кэширования диска** | **рекомендация о том, когда использовать этот параметр** |
| --- | --- |
...
*Нет*  
В настоящее время **ни один из** них поддерживается только на дисках данных. Он не поддерживается на дисках ОС. Если вы установите **None** на дискЕ ОС, он переопределяет это внутренне и установит его на **ReadOnly**.
...
Для SSD дисков ценовой категории "Премиум" можно использовать следующие дистрибутивы Linux. Для повышения производительности и стабильности с помощью премиальных SSD-наделок мы рекомендуем обновить вневые вводы в свои штаты до одной из этих версий или новых. 
...
| Distribution | Версия | Поддерживаемое ядро | Сведения |
| --- | --- | --- | --- |
| Ubuntu | 12.04 или новее| 3.2.0-75.110 или более поздней версии | &nbsp; |
| Ubuntu | 14.04 или новее| 3.13.0-44.73 или более поздней версии  | &nbsp; |
| Debian | 7.x, 8.x или более новый| 3.16.7-ckt4-1 или более поздней версии | &nbsp; |
| SUSE | SLES 12 или новее| 3.12.36-38.1 или более поздней версии | &nbsp; |
| SUSE | SLES 11 SP4 или новее| 3.0.101-0.63.1 или более поздней версии | &nbsp; |
| CoreOS | 584.0.0 " или более новые| 3.18.4 или более поздней версии | &nbsp; |
| CentOS | 6.5, 6.6, 6.7, 7.0, или более новые| &nbsp; | [Требуется LIS4](https://www.microsoft.com/download/details.aspx?id=55106) <br> *См. примечание ниже* |
| CentOS | 7.1 " или более новые| 3.10.0-229.1.2.el7 или более поздней версии | [Рекомендуется LIS4](https://www.microsoft.com/download/details.aspx?id=55106) <br> *См. примечание ниже* |
| Red Hat Enterprise Linux (RHEL) | 6,8 ", 7,2" или более новые | &nbsp; | &nbsp; |
| Oracle; | 6,0 ", 7,2" или более новые | &nbsp; | UEK4 или RHCK |
| Oracle; | 7.0-7.1 или более новые | &nbsp; | UEK4 или RHCK w/[LIS4](https://www.microsoft.com/download/details.aspx?id=55106) |
| Oracle; | 6.4-6.7 или более новые | &nbsp; | UEK4 или RHCK w/[LIS4](https://www.microsoft.com/download/details.aspx?id=55106) |
...
В некоторых случаях приведенная выше команда также обновит ядро. Если требуется обновление ядра, то вам может понадобиться запустить вышеперечисленные команды снова после перезагрузки, чтобы полностью установить пакет Microsoft-hyper-V.
...
Важно! С помощью пользовательского интерфейса диспетчера сервера для чередующегося тома можно задать 8 столбцов. При подключении более восьми дисков используйте PowerShell для создания громкости. С помощью PowerShell можно задать такое же количество столбцов, как и количество дисков. Например, если в одном чередующемся наборе 16 дисков, укажите 16 столбцов для параметра *NumberOfColumns* командлета PowerShell *New-VirtualDisk*.
...
Выберите подходящий размер блока чередования в зависимости от типа рабочей нагрузки приложения. Для случайных небольших запросов ввода-вывода следует использовать меньший размер блока чередования, В то время как для больших последовательных запросов IO используются большие размеры полосы. Узнайте больше о рекомендациях по размеру блока чередования для приложения, которое будет запущено в хранилище класса «Премиум». Для сервера S'L, настроить размер полосы 64 КБ для рабочих нагрузок OLTP и 256 КБ для складирования данных рабочих нагрузок. в статье [Рекомендации по оптимизации производительности SQL Server на виртуальных машинах Azure](../articles/virtual-machines/windows/sql/virtual-machines-windows-sql-performance.md#disks-guidance) .
...
Глубина очереди, длина очереди или размер очереди — это количество ожидающих запросов на иум в системе. Значение глубины очереди определяет, сколько операций обработки io-код может выстраиваться в линию, какие диски хранения будут обрабатываться. Это влияет на все три индикатора производительности приложений, которые мы обсуждали в этой статье viz., IOPS, пропускную способность и задержку.
...
Как правило, приложение может достигать максимальной пропускной способности при наличии 8–16 и более ожидающих выполнения операций ввода-вывода на подключенный диск. Если глубина очереди одна, приложение не подталкивает достаточное количество ИТ в систему, и оно будет обрабатывать меньшее количество в заданный период. Другими словами, пропускная способность будет меньше.
