...
ms.locfileid: "55757579"
---
# <a name="azure-premium-storage-design-for-high-performance"></a>Хранилище Azure класса Premium. Проектирование для обеспечения высокой производительности
...
Если вы не знакомы с хранилищем класса Premium, перед началом работы ознакомьтесь со статьями [Хранилище класса Premium: высокопроизводительная служба хранилища для рабочих нагрузок виртуальных машин Azure](../articles/virtual-machines/windows/premium-storage.md) и [Целевые показатели масштабируемости и производительности службы хранилища Azure](../articles/storage/common/storage-scalability-targets.md).
...
Для выполнения следующих операций уровня управления для управляемых дисков может потребоваться перемещение диска из одного расположения хранения в другое. Это осуществляется с помощью фоновой копии данных, которая может занять несколько часов, обычно менее чем 24 часа в зависимости от объема данных на дисках. В течение этого времени ваше приложение может столкнуться с большей, чем обычно, задержкой чтения, так как некоторые операции чтения могут быть перенаправлены в исходное местоположение, а завершение может занять больше времени. В этот период нет влияния на задержку записи.  

1.  [Обновление типа хранилища](../articles/virtual-machines/windows/convert-disk-storage.md)
2.  [Отсоединение диска от одной виртуальной машины и присоединение к другой](../articles/virtual-machines/windows/attach-disk-ps.md)
3.  [Создание управляемого диска на основе VHD](../articles/virtual-machines/scripts/virtual-machines-windows-powershell-sample-create-managed-disk-from-vhd.md)
4.  [Создание управляемого диска из моментального снимка с помощью PowerShell](../articles/virtual-machines/scripts/virtual-machines-windows-powershell-sample-create-managed-disk-from-snapshot.md)
5.  [Переключение с неуправляемых на управляемые](../articles/virtual-machines/windows/convert-unmanaged-to-managed-disks.md)
...
1. Запустите тест Iometer, чтобы инициализировать диск с кэшем со следующими параметрами. Используйте три рабочих потока для целевого тома и задайте значение «128» для длины очереди. На вкладке "Test Setup" (Настройка теста) для параметра "Run time" (Время выполнения) установите длительность 2 ч.

   | Сценарий | Целевой том | ИМЯ | Duration |
   | --- | --- | --- | --- |
   | Инициализация диска с кэшем |CacheReads |RandomWrites\_1MB |2 ч |
1. Запустите тест Iometer, чтобы подготовить кэш на диске со следующими параметрами. Используйте три рабочих потока для целевого тома и задайте значение «128» для длины очереди. На вкладке "Test Setup" (Настройка теста) для параметра "Run time" (Время выполнения) установите длительность 2 ч.
...
* [Хранилище класса Premium: высокопроизводительное хранилище для рабочих нагрузок виртуальных машин Azure](../articles/virtual-machines/windows/premium-storage.md)  
