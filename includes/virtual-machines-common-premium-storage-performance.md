...
| **Максимальной. Свободной** |Объем памяти, необходимый для стабильной работы приложения |Использование выделенной памяти (в байтах), % |Использование vmstat |
| **Максимальной. ЗАГРУЗКИ** |Объем ЦП, необходимый для стабильной работы приложения |Загруженность процессора, % |%util |
...
Определить нужный уровень оптимизации производительности приложения можно с помощью созданного ранее контрольного списка требований приложения. Исходя из этого, можно определить, какие показатели из этого раздела необходимо настроить. Чтобы убедиться во влиянии каждого из показателей на производительность приложения, запустите инструменты тестирования производительности на установленном приложении. Инструкции по запуску общих средств тестирования производительности на виртуальных машинах Windows и Linux см. в статье о тестировании производительности, связанной в конце.
...
При использовании приложения, в котором нельзя изменять объем ввода-вывода, следуйте приведенным здесь рекомендациям для оптимизации ключевых показателей эффективности для производительности, необходимых приложению. Например, примененная к объекту директива
...
Высокомасштабируемые виртуальные машины доступны в разных размерах с различным количеством ядер ЦП, памяти, ОС и временного диска. Для каждого размера виртуальной машины также установлено максимальное количество дисков данных, которые можно подключить к виртуальной машине. Таким образом, выбранный размер виртуальной машины влияет на мощность обработки, объем памяти и хранилища, которые доступны для приложения. Он также влияет на затраты на вычисление и хранение. Например, ниже приведены спецификации наиболее крупного размера виртуальной машины в серии DS и GS:
...
Хранилище Azure класса Premium предоставляет разнообразные размеры, что позволяет выбрать подходящий вариант. Размер каждого диска отличается от предельного масштаба для операций ввода-вывода, пропускной способности и хранения. Выберите диск хранилища класса «Премиум» правильного размера в соответствии с требованиями приложения и размером высокомасштабируемой виртуальной машины. В таблице ниже приведены размеры дисков и их возможности. Размеры P4, P6, P15, P60, P70 и P80 сейчас поддерживаются только на управляемых дисках.
...
В высокомасштабируемых виртуальных машинах, которые используют хранилище Azure класса «Премиум», применяется технология многоуровневого кэширования под названием BlobCache. BlobCache использует сочетание ОЗУ узла и локального SSD для кэширования. Это кэширование доступно для постоянных дисков хранилища класса «Премиум» и локальных дисков виртуальной машины. По умолчанию для параметра кэширования задано значение Read/Write для дисков операционной системы и ReadOnly для дисков данных, размещенных в хранилище класса «Премиум». Если активировать функцию кэширования диска для дисков хранилища класса «Премиум», можно добиться чрезвычайно высокого уровня производительности высокомасштабируемых виртуальных машин, который превысит базовый уровень производительности диска.
...
| **Тип диска** | **Параметр кэша по умолчанию** |
| --- | --- |
| Диск ОС |ReadWrite |
...
| --- | --- |
| Нет |Задайте для кэша узла значение None для дисков с высокой интенсивностью операций записи и дисков только для записи. |
...
*Нет*  
В настоящее время **ни один** из них не поддерживается на дисках данных. Он не поддерживается на дисках ОС. Если на диске ОС задать значение **None** , это будет переопределено внутренним образом и установлено в значение **ReadOnly**.
...
Для SSD дисков ценовой категории "Премиум" можно использовать следующие дистрибутивы Linux. Для повышения производительности и стабильности благодаря расширенным твердотельным накопителям рекомендуется обновить виртуальные машины до одной из этих версий или более новой. 
...
| --- | --- | --- | --- |
| Ubuntu | 12,04 или более поздней версии| 3.2.0-75.110 или более поздней версии | &nbsp; |
| Ubuntu | 14,04 или более поздней версии| 3.13.0-44.73 или более поздней версии  | &nbsp; |
| Debian | 7. x, 8. x или более поздней версии| 3.16.7-ckt4-1 или более поздней версии | &nbsp; |
| SUSE | SLES 12 или более поздней версии| 3.12.36-38.1 или более поздней версии | &nbsp; |
| SUSE | SLES 11 SP4 или более поздней версии| 3.0.101-0.63.1 или более поздней версии | &nbsp; |
| CoreOS | 584.0.0 + или более поздняя версия| 3.18.4 или более поздней версии | &nbsp; |
| CentOS | 6,5, 6,6, 6,7, 7,0 или более поздней версии| &nbsp; | [Требуется LIS4](https://www.microsoft.com/download/details.aspx?id=55106) <br> *См. примечание ниже* |
| CentOS | 7.1 + или более поздняя версия| 3.10.0-229.1.2.el7 или более поздней версии | [Рекомендуется LIS4](https://www.microsoft.com/download/details.aspx?id=55106) <br> *См. примечание ниже* |
| Red Hat Enterprise Linux (RHEL) | 6.8 +, 7,2 + или более поздней версии | &nbsp; | &nbsp; |
| Oracle; | 6.0 +, 7,2 + или более поздняя версия | &nbsp; | UEK4 или RHCK |
| Oracle; | 7.0 — 7.1 или более поздняя версия | &nbsp; | UEK4 или RHCK w/[LIS4](https://www.microsoft.com/download/details.aspx?id=55106) |
| Oracle; | 6.4 — 6.7 или более поздняя версия | &nbsp; | UEK4 или RHCK w/[LIS4](https://www.microsoft.com/download/details.aspx?id=55106) |
...
В некоторых случаях команда выше обновит ядро. Если требуется обновление ядра, то после перезагрузки может потребоваться выполнить приведенные выше команды, чтобы полностью установить пакет Microsoft-Hyper-v.
