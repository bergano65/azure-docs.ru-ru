...
- **Рынки**. Рынки, по которым возвращаются результаты. Например, en-us (английский, США). По умолчанию выбраны все рынки. Обратите внимание, что рынок en-WW — это рынок, который использует Bing, если вызов не указывает рынок и Bing не может определить рынок пользователя.  
...
- **Распределение по запрошенным ответам**. API веб-поиска отвечает на запросы, указанные в параметре запроса `responseFilter`.  
