...
Для управляемого диска можно создать подписанный URL-адрес (SAS) с доступом только для чтения, с помощью которого содержимое диска можно скопировать в частную учетную запись хранения или в локальное хранилище. URI SAS можно использовать с помощью портал Azure, Azure PowerShell, Azure CLI или [AzCopy](../articles/storage/common/storage-use-azcopy.md)
...
Образы поколения 1 могут использовать секционирование GPT только на дисках данных, а не на дисках ОС. На дисках ОС необходимо использовать стиль разделов MBR.

[Образы поколения 2](https://docs.microsoft.com/azure/virtual-machines/linux/generation-2) могут использовать секционирование GPT на диске ОС, а также на дисках данных.
...
Резервирование дисков — это возможность приобретения одного года дискового хранилища заранее, что снижает общую стоимость. Дополнительные сведения о резервировании дисков Azure см. в нашей статье: [сведения о том, как скидка на резервирование применяется к диску Azure](../articles/cost-management-billing/reservations/understand-disk-reservations.md).
...
Резервирование выполняется в виде дисков, а не емкости. Иными словами, при резервировании диска P80 (32 Тиб) вы получаете один диск P80, после чего вы не сможете разделить это резервирование на два небольших диска P70 (16 ТиБ). Конечно, можно зарезервировать столько дисков, сколько вам нравится, включая два отдельных диска P70 (16 ТиБ).
...
Нет, поддерживается только для управляемых дисков SSD уровня "Премиум".

**Какие регионы поддерживают общие диски?**

В настоящее время Западная Центральная часть США.
...
Только твердотельные накопители уровня "Премиум", P15 или более поздней версии, поддерживают общие диски.

**Если у меня есть SSD уровня "Премиум", можно ли включить на нем общие диски?**

Все управляемые диски, созданные с помощью API версии 2019-07-01 или более поздней, могут включать общие диски. Для этого необходимо отключить диск от всех виртуальных машин, к которым он подключен. Затем измените `maxShares` свойство на диске.

**Если я больше не хочу использовать диск в общем режиме, как отключить его?**

Отключите диск от всех виртуальных машин, к которым он подключен. Затем измените свойство Максшаре на диске на 1.

**Можно ли изменить размер общего диска?**

Да.

**Можно ли включить ускоритель записи на диск, на котором также включены общие диски?**
...
**Включено ли шифрование на стороне сервера при создании управляемого диска?**

Да. Управляемые диски шифруются с помощью шифрования на стороне сервера с помощью управляемых платформой ключей. 
...
Управляемые ключи платформы управляются корпорацией Майкрософт. Вы также можете использовать собственные ключи, хранящиеся в Azure Key Vault, и управлять ими. 

**Можно ли отключить шифрование на стороне сервера для моих управляемых дисков?**

Нет.

**Шифрование на стороне сервера доступно только в конкретных регионах?**

Нет. Шифрование на стороне сервера с управляемыми ключами платформы и клиента доступно во всех регионах, где доступны управляемые диски. 

**Поддерживает ли Azure Site Recovery шифрование на стороне сервера с помощью управляемого клиентом ключа в сценариях аварийного восстановления из локальной среды в Azure и Azure.**

Да. 

**Можно ли создать резервную копию управляемых дисков, зашифрованных с помощью шифрования на стороне сервера, с использованием управляемого клиентом ключа Azure Backup службы?**

Да.

**Шифруются ли управляемые моментальные снимки и образы?**

Да. Все управляемые моментальные снимки и изображения шифруются автоматически. 
...
В настоящее время возможность разбивки поддерживается во всех регионах общедоступного облака Azure с поддержкой независимых Cloud. 

**Какие регионы имеют размер 4/8/16 гиб управляемых дисков (P1/P2/P3, E1/E2/E3), поддерживаемых в?**

В настоящее время эти новые размеры дисков поддерживаются во всех регионах общедоступного облака Azure. Поддержка облаков независимых скоро появится. 
...
Самый крупный размер диска, поддерживаемый Azure Backup, — 32 Тиб (4 Тиб для зашифрованных дисков). Самый крупный размер диска, поддерживаемый Azure Site Recovery, — 8 тиб. Поддержка дисков большего размера до 32 Тиб еще недоступна в Azure Site Recovery.
...
Для достижения пропускной способности диска SSD (цен. категория "Стандартный") и HDD (цен. категория "Стандартный") дисков большого размера (>4 Тиб) свыше 500 операций ввода-вывода и 60 MiB/с мы рекомендуем развернуть новую виртуальную машину на основе одного из следующих размеров виртуальных машин, чтобы оптимизировать производительность: B-Series, серии DSv2, серии Dsv3, ESv3-Series, серии FS, серии серия fsv2, серии M, серии a, серии м, и Ls. Присоединение больших дисков к существующим виртуальным машинам или виртуальным машинам, которые не используют рекомендованные размеры, могут снизить производительность.
