...
В Azure существует три роли основной диск: диск данных, диск операционной системы и временный диск. Эти роли сопоставляются диски, подключенные к виртуальной машине.

![Диск ролей в действии](media/virtual-machines-managed-disks-overview/disk-types.png)

### <a name="data-disk"></a>Диск с данными

Диск данных — управляемый диск, подключенный к виртуальной машине для хранения данных приложений или других необходимых данных. Диски данных регистрируются как диски SCSI и обозначаются любой указанной буквой. Каждый диск данных имеет максимальную емкость 32 767 гибибайтах (Гиб). Размер виртуальной машины определяет, сколько дисков данных можно подключить и какой тип хранилища можно использовать для размещения дисков.
...
* [Как записать управляемый образ универсальной виртуальной машины в Azure](../articles/virtual-machines/windows/capture-image-resource.md)
* [Как обобщить и записать образ виртуальной машины Linux с помощью Azure CLI](../articles/virtual-machines/linux/capture-image.md)
