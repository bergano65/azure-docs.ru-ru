...
В локальных решениях для устройств могут применяться политики трафика для передачи трафика между несколькими туннелями в Azure.
...
2. Локальные маршруты через внутренний концентратор.
3. Статические маршруты по протоколу BGP.
4. ExpressRoute (ER) через VPN.
5. Длина пути AS.

Переход между ER и ER всегда выполняется через Global Reach, поэтому если запрос поступает через ER в один концентратор и в удаленном концентраторе действуют VPN и ER, VPN будет выбран в качестве предпочтительного способа передачи данных из удаленного концентратора для доступа к конечной точке, подключенной через VPN или ER в удаленном концентраторе.
