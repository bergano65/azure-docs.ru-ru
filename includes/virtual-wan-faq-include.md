...
У вас есть два варианта добавления DNS-серверов для клиентов P2S.

1. Вы можете отправить запрос в службу поддержки Майкрософт с просьбой добавить DNS-сервер на концентратор.
...
У каждого VPN-шлюза для подключения пользователей ("точка — сеть") есть два экземпляра, каждый из которых имеет ограничение по числу поддерживаемых пользователей в зависимости от единиц масштабирования. Единица масштабирования 1–3 поддерживает 500 подключений, единица масштабирования 4–6 поддерживает 1000 подключений, единица масштабирования 7–12 поддерживает 5000 подключений, а единица масштабирования 13-20 поддерживает до 10 000 подключений. Для примера предположим, что пользователь выбрал единицу масштабирования 1. Каждая единица масштабирования подразумевает развертывание шлюза с активными экземплярами (в нашем примере их 2), каждый из которых поддерживает до 500 подключений. Но возможность получить 500×2 подключений на каждом шлюзе не должна служить основанием учитывать для этой единицы масштабирования плановую нагрузку в 1000 пользователей вместо 500, так как при возможном обслуживании экземпляров все избыточные подключения сверх 500 будут разорваны.
...
* Если у вас есть периферийные виртуальные сети, подключенные к концентратору, плата за пиринг в таких сетях будет по-прежнему взиматься. 
...
Да. Виртуальная глобальная сеть цен. категории "Стандартный" поддерживает транзитивное подключение между виртуальными сетями через концентратор виртуальной глобальной сети, к которому они подключены. В терминологии виртуальных глобальных сетей такие пути также называются "локальным транзитом виртуальной глобальной сети" для виртуальных сетей, подключенных к Концентратору виртуальной глобальной сети в пределах одного региона, и "глобальным транзитом виртуальной глобальной сети" для виртуальных сетей, подключенных через несколько Концентраторов виртуальной глобальной сети в двух или более регионах. Во время общедоступной предварительной версии транзит виртуальной сети поддерживает пропускную способность до 3 Гбит/с. В общедоступной версии пропускная способность увеличится.

Примечание. Сейчас для использования предварительной версии функции передачи данных V2V нужно развернуть VPN-шлюз в виртуальном концентраторе, чтобы активировать запуск элементов маршрутизации. Этот VPN-шлюз не используется для передачи данных V2V. Это известное ограничение, которое будет удалено в общедоступной версии V2V. После полного запуска VPN-шлюз можно удалить в концентраторах, так как он не требуется для передачи данных V2V. 

В некоторых сценариях виртуальные сети периферийных зон также можно напрямую объединить в одноранговую с помощью [пиринга между виртуальными сетями](../articles/virtual-network/virtual-network-peering-overview.md) в дополнение к локальному или глобальному транзиту Виртуальной глобальной сети. В этом случае пиринг виртуальной сети имеет приоритет над транзитивным подключением через концентратор виртуальной глобальной сети. 
...
Если виртуальный концентратор получает один и тот же маршрут из нескольких удаленных концентраторов, то оптимальный путь определяется в таком порядке:
1) Источник маршрута: 1) сетевые маршруты — префиксы виртуальной сети, непосредственно полученные шлюзами виртуального концентратора; 2) таблица маршрутов концентратора (статически настроенные маршруты); 3) BGP; 4) внутренние маршруты концентратора.
2)  Метрика маршрута: виртуальная глобальная сеть отдает предпочтение ExpressRoute перед VPN. У узла ExpressRoute приоритет будет выше, чем у VPN-узла.
3)  Длина пути AS.

### <a name="is-there-support-for-ipv6-in-virtual-wan"></a>Поддерживается ли IPv6 в Virtual WAN?

IPv6 не поддерживается в концентраторе виртуальной глобальной сети и его шлюзах. Если у вас есть виртуальная сеть с поддержкой IPv6, и вы хотите подключить ее к виртуальной глобальной сети, этот сценарий также не поддерживается. 
