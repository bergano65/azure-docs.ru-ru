...
| Виртуальная сеть {virtual-network-name} не существует. |Это может произойти, если вы создали виртуальную сеть на новом портале Azure. Фактическое название виртуальной сети следует \<шаблону "Группа и имя VNET>" |
| Виртуальная машина {vm-name} в размещенной службе {hosted-service-name} содержит расширение {extension-name}, которое не поддерживается в Azure Resource Manager. Рекомендуется удалить его из виртуальной машины перед продолжением миграции. |XML расширения, такие как BGInfo 1. \* не поддерживаются в менеджере ресурсов Azure. Таким образом их невозможно перенести. Если оставить эти расширения на виртуальной машине, они автоматически удаляются перед выполнением миграции. |
...
| Миграция запрещена для размещенной службы {размещенных service-name}, так как она содержит виртуальную машину {vm-name} в состоянии RoleStateUnknown. Миграцию можно выполнить, только если виртуальная машина находится в одном из следующих состояний: Running, Stopped, Stopped Deallocated. |VM может проходить через переход состояния, который обычно происходит, когда во время операции обновления на HostedService, таких как перезагрузка, установка расширения и т.д. Перед попыткой миграции рекомендуется завершить операцию обновления на HostedService. |
...
| Миграция запрещена для развертывания {имя_развертывания} в размещенной службе {имя_облачной_службы}, так как она не входит в область для региона. Пожалуйста, обратитесь\/к https: /aka.ms/regionalscope для перемещения этого развертывания в региональный охват. | В 2014 г. было объявлено, что сетевые ресурсы в Azure будут перемещены из области работ для кластера на региональный уровень. Смотрите [https://aka.ms/regionalscope](https://aka.ms/regionalscope) для более подробной информации . Эта ошибка возникает при миграции развертывания без выполнения обновления, которое автоматически перемещает такое развертывание в область для региона. Лучшим решением будет добавить для виртуальной машины конечную точку или диск данных, а затем повторить попытку миграции. <br> См. дополнительные сведения о [настройке конечных точек для классической виртуальной машины Windows в Azure](/previous-versions/azure/virtual-machines/windows/classic/setup-endpoints#create-an-endpoint) и [подключении диска данных к виртуальной машине Windows, созданной с помощью классической модели развертывания](../articles/virtual-machines/windows/classic/attach-disk.md).|
