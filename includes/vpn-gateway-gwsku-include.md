...
| **номер SKU**| **Компоненты**|
| ---    | ---         |
|**Базовый** (* *)   | **VPN на основе маршрутов**: 10 туннелей для S2S и подключений; без аутентификации RADIUS для подключения типа "точка — сеть"; без протокола IKEv2 для подключения типа "точка — сеть".<br>**VPN на основе политики**: (IKEv1): 1 туннель S2S и тоннель подключения; без P2S.|
...
###  <a name="gateway-skus---production-vs-dev-test-workloads"></a><a name="workloads"></a>Номера SKU шлюзов — рабочие нагрузки и разработки — тестирование рабочих нагрузок
