...
* Сразу же вызвать `AcquireTokenAsync`. Этот вызов приводит к появлению запроса на вход пользователя. Этот шаблон используется в интерактивных приложениях, где пользователю недоступно автономное содержимое. Пример, созданный в этом руководстве, следует этому шаблону. Вы увидите его в действии при первом запуске примера.
* Продемонстрировать пользователям визуальное оповещение о том, что требуется интерактивный вход. Вызовите `AcquireTokenAsync`, когда пользователь будет готов совершить вход.
...
В этом примере приложения метод `callGraphAPI()` использует `getAccessToken()`, чтобы получить новый маркер доступа.  Приложение использует маркер в запросе HTTP `GET` к API Microsoft Graph. 
<!--end-collapse-->
