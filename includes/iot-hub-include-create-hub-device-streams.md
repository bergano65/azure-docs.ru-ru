...
3. Нажмите кнопку **Центр Интернета вещей** в списке справа. Появится первый экран для создания Центра Интернета вещей.
   ...
   **Регион**. Выберите регион, в котором будет расположен Центр Интернета вещей. Выберите регион, который поддерживает функцию "Потоки устройств" Центра Интернета вещей (предварительная версия). Это Центральная часть США или Центральная часть США (EUAP).
