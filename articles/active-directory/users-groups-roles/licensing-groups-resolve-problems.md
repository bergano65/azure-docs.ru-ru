---
title: Устранение проблем с назначением лицензий группы - Активный каталог Azure (ru) Документы Майкрософт
...
## <a name="find-license-assignment-errors"></a>Поиск ошибок назначения лицензии

### <a name="to-find-users-in-an-error-state-in-a-group"></a>Поиск пользователей в состоянии ошибки в группе

1. Откройте группу на странице обзора и выберите **Лицензии.** Уведомление отображается, если есть какие-либо пользователи в состоянии ошибки.

   ![Сообщение об уведомлениях группы и ошибок](./media/licensing-groups-resolve-problems/group-error-notification.png)
...
1. Чтобы открыть список всех пользователей с состоянием, описанным выше, щелкните уведомление. Для просмотра дополнительных сведений можно выбрать отдельного пользователя.
   ...
   ![список пользователей в состоянии ошибки группового лицензирования](./media/licensing-groups-resolve-problems/list-of-users-with-errors.png)
...
1. Чтобы найти все группы, в которых есть по крайней мере одна ошибка, в колонке **Azure Active Directory** выберите **Лицензии**, а затем — **Обзор**. Если есть группы, требующие внимания, отображается поле сведений.
   ...
   ![Обзор и информация о группах в состоянии ошибки](./media/licensing-groups-resolve-problems/group-errors-widget.png)
...
Чтобы узнать, сколько лицензий доступно, перейдите в **Azure Active Directory** > **Licenses** > **Все продукты.**
...
Чтобы решить эту проблему, удалите пользователей из неподдерживаемых мест из лицензированной группы. Кроме того, если текущие значения места использования не соответствуют фактическому местонахождению пользователя, вы можете изменить эти значения для последующего правильного назначения лицензий (в случае, если новое расположение поддерживается).
...
## <a name="azure-ad-mail-and-proxyaddresses-attribute-change"></a>Изменение атрибутов Azure AD Mail и ProxyAddresses

**Проблема:** При обновлении назначения лицензии для пользователя или группы можно увидеть, что атрибут Azure AD Mail и ProxyAddresses некоторых пользователей изменяется.

Обновление назначения лицензии для пользователя приводит к срабатыванию расчета прокси-адреса, что может изменить атрибуты пользователя. Чтобы понять точную причину изменения и решить проблему, смотрите в этой статье о том, [как атрибут proxyAddresses заполняется в Azure AD.](https://support.microsoft.com/help/3190357/how-the-proxyaddresses-attribute-is-populated-in-azure-ad)

## <a name="licenseassignmentattributeconcurrencyexception-in-audit-logs"></a>ЛицензияНазначениеАтрибутАковлизм в журналах аудита

**Проблема:** Пользователь имеет ЛицензиюНазначениеАтрибутАклизваем для назначения лицензии в журналах аудита.
Когда групповое лицензирование пытается обработать одноодное назначение лицензии той же лицензии пользователю, это исключение записывается на пользователя. Это обычно происходит, когда пользователь является членом более чем одной группы с одинаковой лицензией. A'ure AD повторно пропытается обработку пользовательской лицензии и решит проблему. Клиентне не требует никаких действий для устранения этой проблемы.

## <a name="more-than-one-product-license-assigned-to-a-group"></a>Лицензия на продукт, назначенная группе
...
## <a name="manage-licenses-for-products-with-prerequisites"></a>Управление лицензиями для продуктов с предварительными условиями
...
Если мы попытаемся самостоятельно назначить этот продукт группе, портал возвращает сообщение об уведомлении. Если мы выберем детали элемента, он показывает следующее сообщение об ошибке:

  "Лицензионная операция не удалась. Убедитесь, что группа содержит нужные службы перед тем, как добавить или удалить зависимую службу. **Служба Microsoft Workplace Analytics требует, чтобы Exchange Online (план 2) также был включен"**
...
Можно также создать изолированную группу, содержащую только минимальный требуемый набор продуктов, обеспечивающий работу надстройки. Он может использоваться для лицензирования только выбранных пользователей для надстройки продукта. На основе предыдущего примера можно назначить следующую продукцию одной и той же группе:
...
## <a name="force-group-license-processing-to-resolve-errors"></a>Обработка лицензии группы форс-групп для устранения ошибок
...
## <a name="force-user-license-processing-to-resolve-errors"></a>Принудить обработку лицензии пользователя для устранения ошибок
