...
> [!NOTE]
> Портал администрирования Azure AD пока не поддерживает добавление разрешений, перечисленных в этой статье, в пользовательское определение роли каталога. Для создания пользовательской роли каталога с разрешениями, перечисленными в этой статье, необходимо [использовать Azure AD PowerShell](roles-create-custom.md#create-a-role-using-powershell).
