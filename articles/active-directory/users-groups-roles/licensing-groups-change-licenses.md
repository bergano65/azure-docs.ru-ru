...
ms.locfileid: "60471201"
---
# <a name="change-the-license-for-a-single-user-in-a-licensed-group-in-azure-active-directory"></a>Изменить лицензию для одного пользователя в лицензированной группы в Azure Active Directory
...
**Шаг 2**. Пользователь добавляется в целевую группу, а лицензирование на основе групп обрабатывает это изменение. Теперь у пользователя есть *исходная* и *целевая лицензии*, унаследованные от групп:
...
**Шаг 3**. Пользователь удаляется из исходной группы, а лицензирование на основе группы обрабатывает это изменение. Теперь у пользователя есть только *целевая лицензия*:
...
Цель переноса — использование лицензирования на основе групп для замены лицензий пользователя с *исходной лицензии* (например, Office 365 корпоративный E1) в *целевую лицензию* (например, Office 365 корпоративный E3). В этом сценарии эти два продукта содержат конфликтующие планы обслуживания, поэтому нам потребуется обойти эту проблему, чтобы прозрачно перенести пользователей. Дополнительные сведения об ошибках см. в разделе [Конфликтующие планы обслуживания](https://docs.microsoft.com/azure/active-directory/active-directory-licensing-group-problem-resolution-azure-portal#conflicting-service-plans). Ни на одном этапе переноса пользователи не должны утратить доступ к службам или данным. Перенос выполняется небольшими "пакетами". Поэтому имеется возможность проверить результат каждого пакета и свести к минимуму масштаб проблем, которые могут возникнуть во время этого процесса. Ниже приведено общее описание процесса.
...
**Шаг 2**. Пользователь добавляется в целевую группу, а лицензирование на основе групп обрабатывает это изменение. Так как у пользователя по-прежнему есть *исходная лицензия*, *целевая лицензия* находится в состоянии ошибки из-за конфликта:
...
**Шаг 3**. Пользователь удаляется из исходной группы, а лицензирование на основе группы обрабатывает это изменение. Пользователю назначается *целевая лицензия*:
