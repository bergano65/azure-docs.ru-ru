...
После включения этой функции "классические" классификации для групп будут отображаться только существующие группы и сайты. их следует использовать только для новых групп, если создание групп в приложениях, которые не поддерживают метки чувствительности. При необходимости администратор может впоследствии преобразовать их в метки чувствительности. Классические классификации — это старые классификации, которые можно настроить, определив значения параметров `ClassificationList` в Azure AD PowerShell. Если эта функция включена, эти классификации не будут применяться к группам.
