...
Пользователи с этой ролью могут устанавливать или сбрасывать учетные данные, не являющиеся паролями, для некоторых пользователей и могут обновлять пароли для всех пользователей. Администраторы проверки подлинности могут требовать от пользователей, не являющихся администраторами, или назначать некоторые роли для повторной регистрации в существующих учетных данных, не являющихся паролями (например, MFA или FIDO), а также отменять **запоминание MFA на устройстве**, при этом при следующем входе запрашиваются mfa. Эти действия применяются только к пользователям, не являющимся администраторами или которым назначена одна или несколько из следующих ролей:
...
Пользователи с этой ролью добавляют или удаляют настраиваемые атрибуты, доступные для всех потоков пользователей в Организации Azure AD.Таким образом, пользователи с этой ролью могут изменять или добавлять новые элементы в схему конечного пользователя и влиять на поведение всех потоков пользователей и косвенно вносить изменения в то, какие данные могут быть запрошены конечными пользователями и в конечном счете в качестве утверждений для приложений.Эта роль не может изменять потоки пользователей.
...
Пользователи с этой ролью могут создавать, читать, обновлять и удалять все пользовательские политики в Azure AD B2C и, таким образом, иметь полный контроль над инфраструктурой процедур идентификации в соответствующей Azure AD B2C Организации. Изменяя политики, этот пользователь может установить прямую федерацию с внешними поставщиками удостоверений, изменить схему каталога, изменить все содержимое, доступное пользователю (HTML, CSS, JavaScript), изменить требования для выполнения проверки подлинности, создать новых пользователей, отправить данные пользователя во внешние системы, включая полные миграции, и изменить все сведения о пользователе, включая пароли и номера телефонов. И наоборот, эта роль не может изменить ключи шифрования или изменить секреты, используемые для Федерации в Организации.

> [!IMPORTANT]
> Администратор политики B2 инфраструктура процедур идентификации — это роль с высокой степенью секретности, которая должна быть назначена на очень ограниченном уровне для организаций в рабочей среде.Действия этих пользователей должны быть тщательно проверены, особенно для организаций в рабочей среде.
...
В | Может
----- | ----------
[Центр соответствия требованиям Microsoft 365](https://protection.office.com) | Защита данных Организации и управление ими в службах Microsoft 365 Services<br>Управление оповещениями по соответствию.
...
[Диспетчер соответствия](https://docs.microsoft.com/office365/securitycompliance/meet-data-protection-and-regulatory-reqs-using-microsoft-cloud) | Отслеживание, назначение и проверка деятельности организации на соответствие требованиям.
[Центр безопасности и соответствия требованиям Office 365](https://support.office.com/article/About-Office-365-admin-roles-da585eea-f576-4f55-a1e0-87090b6aaa9d) | Управление системой управления данными.<br>Выполнение юридического расследование и анализа данных.<br>Управление запросом субъекта данных.<br><br>Эта роль имеет те же разрешения, что и [администратор данных о соответствии группы ролей](https://docs.microsoft.com/microsoft-365/security/office-365-security/permissions-in-the-security-and-compliance-center#permissions-needed-to-use-features-in-the-security--compliance-center) в центре безопасности Office 365 Security & обеспечения контроля доступа на основе ролей.
[Intune](https://docs.microsoft.com/intune/role-based-access-control) | Просмотр всех данных проверки Intune.
[Cloud App Security](https://docs.microsoft.com/cloud-app-security/manage-admins) | Обладает разрешением только для чтения и может управлять оповещениями.<br>Может создавать и изменять файловые политики и давать разрешение на управление файлом.<br>Позволяет просматривать все встроенные отчеты в разделе "Управление данными".
...
пользователи с этой ролью имеют глобальные разрешения в Microsoft Dynamics 365 Online (если служба используется), а также возможность управлять запросами в службу поддержки и отслеживать работоспособность службы. Дополнительные сведения об [использовании роли администратора службы для управления организацией Azure AD](https://docs.microsoft.com/dynamics365/customer-engagement/admin/use-service-admin-role-manage-tenant).

> [!NOTE]
> В Microsoft Graph API и Azure AD PowerShell эта роль определяется как "Администратор службы Dynamics 365". Это "Администратор Dynamics 365" в [портал Azure](https://portal.azure.com).

### <a name="exchange-administrator"></a>[Администратор Exchange](#exchange-service-administrator-permissions)

пользователи с этой ролью имеют глобальные разрешения в Microsoft Exchange Online (если служба используется). У них также есть возможность создавать все группы Office 365 и управлять ими, а также управлять запросами в службу поддержки и отслеживать работоспособность служб. Дополнительные сведения [о ролях администраторов Office 365](https://support.office.com/article/About-Office-365-admin-roles-da585eea-f576-4f55-a1e0-87090b6aaa9d).

> [!NOTE]
> В Microsoft Graph API и Azure AD PowerShell эта роль определяется как "Администратор службы Exchange". Это «администратор Exchange» в [портал Azure](https://portal.azure.com). Это «администратор Exchange Online» в [центре администрирования Exchange](https://go.microsoft.com/fwlink/p/?LinkID=529144).
...
Этот администратор управляет интеграцией между организациями Azure AD и внешними поставщиками удостоверений.С помощью этой роли пользователи могут добавлять новых поставщиков удостоверений и настраивать все доступные параметры (например, путь проверки подлинности, идентификатор службы, назначенные контейнеры ключей).Этот пользователь может позволить Организации Azure AD доверять проверкам подлинности от внешних поставщиков удостоверений.Полученное влияние на взаимодействие с конечными пользователями зависит от типа организации:

* Организации Azure AD для сотрудников и партнеров. Добавление Федерации (например, Gmail) немедленно повлияет на все приглашения гостей, которые еще не были активированы. См. статью [Добавление Google в качестве поставщика удостоверений для гостевых пользователей B2B](https://docs.microsoft.com/azure/active-directory/b2b/google-federation).
* Azure Active Directory B2C Организации. Добавление Федерации (например, с помощью Facebook или другой организации Azure AD) не влияет на потоки конечных пользователей, пока поставщик удостоверений не будет добавлен как вариант в потоке пользователей (также называемом встроенной политикой). Пример см. [в разделе настройка учетная запись Майкрософт в качестве поставщика удостоверений](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-setup-msa-app) .Чтобы изменить потоки пользователей, требуется ограниченная роль "B2C User Flow Administrator".
...
пользователи с этой ролью имеют доступ ко всем административным функциям в Azure Active Directory, а также к службам, использующим идентификаторы Azure Active Directory, например Центр безопасности Microsoft 365, Центр соответствия требованиям Microsoft 365, Exchange Online, SharePoint Online и Skype для бизнеса Online. Пользователь, который подписывается на организацию Azure AD, станет глобальным администратором. В компании может быть несколько глобальных администраторов. Глобальные администраторы могут сбросить пароль любого пользователя и администратора.
...
>- [Центр безопасности Office & соответствие требованиям](https://sip.protection.office.com/homepage) . глобальный модуль чтения не может читать журналы аудита SCC, выполнять поиск содержимого или просматривать данные оценки безопасности.
...
### <a name="hybrid-identity-administrator"></a>[Гибридный администратор удостоверений](#hybrid-identity-administrator-permissions)

Пользователи с этой ролью могут включать, настраивать и администрировать службы и параметры, связанные с включением гибридной идентификации в Azure AD. Эта роль предоставляет возможность настроить Azure AD для использования одного из трех поддерживаемых методов проверки подлинности, синхронизации хэшей паролей (PHS), сквозной проверки подлинности (PTA) или Федерации (AD FS или стороннего поставщика Федерации), а также для развертывания связанной локальной инфраструктуры, чтобы включить их. Локальная инфраструктура включает в себя подготовку и PTA агентов. Эта роль предоставляет возможность простого единого входа (S-SSO) для обеспечения простой проверки подлинности на устройствах, отличных от Windows 10, или на компьютерах, отличных от Windows Server 2016. Кроме того, эта роль предоставляет возможность просмотра журналов входа и доступа к работоспособности и аналитике для целей мониторинга и устранения неполадок. 
...
Эта роль может создавать все группы безопасности и управлять ими. Однако у администратора Intune нет прав администратора на группы Office. Это означает, что администратор не может обновить владельцев или членство всех групп Office в Организации. Тем не менее, он может управлять группой Office, которую он создает, которая входит в состав своих привилегий для конечных пользователей. Таким образом, любая группа Office (не группа безопасности), которую он создает, должна быть рассчитана на его квоту 250.

> [!NOTE]
> В Microsoft Graph API и Azure AD PowerShell эта роль определяется как "Администратор службы Intune". Это "Администратор Intune" в [портал Azure](https://portal.azure.com).
...
### <a name="network-administrator"></a>[Сетевой администратор](#network-administrator-permissions)

Пользователи этой роли могут просматривать рекомендации по архитектуре периметра сети от корпорации Майкрософт, основанные на данных телеметрии сети из их расположений. Производительность сети для Office 365 полагается на тщательную архитектуру периметра сети клиентов предприятия, которая обычно зависит от местонахождения пользователя. Эта роль позволяет изменять обнаруженные расположения пользователей и настраивать сетевые параметры для этих расположений, чтобы упростить улучшенные измерения телеметрии и рекомендации по проектированию. 
...
> [!NOTE]
> В Microsoft Graph API и Azure AD PowerShell эта роль определяется как "Администратор службы Power BI". Это «Power BI Администратор» в [портал Azure](https://portal.azure.com).
...
### <a name="printer-technician"></a>[Специалист по печати](#printer-technician-permissions)

Пользователи с этой ролью могут регистрировать принтеры и управлять состоянием принтера в универсальном решении печати (Майкрософт). Они также могут читать все сведения о соединителе. Основная задача. специалист по печати не может сделать это, устанавливая разрешения пользователя на принтерах и совместное использование принтеров.
...
Пользователи с этой ролью могут устанавливать или сбрасывать учетные данные, не являющиеся паролями, для всех пользователей, включая глобальных администраторов, и могут обновлять пароли для всех пользователей. Администраторы привилегированной проверки подлинности могут принудительно выполнить повторную регистрацию существующих учетных данных, не являющихся паролями (например, MFA или FIDO), и отозвать параметр "запомнить MFA на устройстве", запрашивая MFA при следующем входе всех пользователей. Роль [администратора проверки подлинности](#authentication-administrator) может принудительно выполнять повторную регистрацию и MFA только для пользователей, не являющихся администраторами, и назначаться следующим РОЛЯМ Azure AD:
...
[Управление привилегированными пользователями (PIM)](https://docs.microsoft.com/azure/active-directory/privileged-identity-management/pim-configure) | Все разрешения роли читателя сведений о безопасности.<br>**Невозможно** управлять назначениями ролей или параметрами Azure AD
...
Пользователи с этой ролью могут управлять оповещениями и иметь глобальный доступ только для чтения к функциям, связанным с безопасностью, включая всю информацию в центре безопасности Microsoft 365, Azure Active Directory, защиту идентификации, управление привилегированными пользователями и центр обеспечения безопасности & Microsoft Office 365. Дополнительные сведения о разрешениях в Office 365 см. в статье [Разрешения в Центре безопасности и соответствия требованиям Office 365](https://docs.microsoft.com/office365/securitycompliance/permissions-in-the-security-and-compliance-center).
...
> [!NOTE]
> Ранее эта роль называлась "Администратор службы" в [портал Azure](https://portal.azure.com) и [Microsoft 365 центре администрирования](https://admin.microsoft.com). Мы переименовали его в "администратор поддержки службы", чтобы согласовать с именем существующими в Microsoft Graph API, Azure AD API Graph и Azure AD PowerShell.
...
> [!NOTE]
> В Microsoft Graph API и Azure AD PowerShell эта роль определяется как "Администратор служб SharePoint". Это "администратор SharePoint" в [портал Azure](https://portal.azure.com).
...
Может создавать регистрации приложений независимо от параметра "пользователи могут регистрировать приложения".
...
| microsoft.office365.exchange/allEntities/allTasks | Управление всеми аспектами Exchange Online. |
| Microsoft. Office 365. сеть/производительность/Аллпропертиес/чтение | Чтение страниц производительности сети в центре администрирования M365. |
| microsoft.office365.serviceHealth/allEntities/allTasks | Чтение и настройка работоспособности служб Office 365. |
| microsoft.office365.supportTickets/allEntities/allTasks | Создание запросов в службу поддержки Office 365 и управление ими. |
| microsoft.office365.usageReports/allEntities/read | Чтение отчетов об использовании Office 365. |
| microsoft.office365.webPortal/allEntities/basic/read | Чтение базовых свойств для всех ресурсов в microsoft.office365.webPortal. |
...
| **Действия** | **Описание** |
| --- | --- |
| Microsoft. Commerce. Billing/Аллентитиес/Read    | Чтение всех аспектов выставления счетов. |
| Microsoft. Directory/administrativeUnits/Basic/Read    | Чтение стандартных свойств для объектов administrativeUnit в Azure Active Directory. |
| Microsoft. Directory/administrativeUnits/Members/Read    | Чтение свойства administrativeUnits.members в Azure Active Directory. |
| Microsoft. Directory/Applications/Basic/Read    | Чтение стандартных свойств приложений в Azure Active Directory. |
| Microsoft. Directory/Applications/Owners/Read    | Чтение свойства applications.owners в Azure Active Directory. |
...
### <a name="groups-administrator-permissions"></a>Группирует разрешения администратора
...
Может приглашать гостевых пользователей независимо от параметра "участники могут приглашать гостей".
...
### <a name="hybrid-identity-administrator-permissions"></a>Разрешения администратора гибридной идентификации

Включение, развертывание, Настройка, мониторинг и устранение неполадок в облачной подготовке и службах проверки подлинности. 
...
| Microsoft. Directory/приложения/разрешения/обновление | Изменение свойства applications.permissions в Azure Active Directory. |
| Microsoft. Directory, приложения, политики или обновление | Изменение свойства applications.policies в Azure Active Directory. |
| Microsoft. Directory/Аппликатионтемплатес/экземпляр | Создание экземпляров приложений из коллекции из шаблонов приложений. |
| Microsoft. Directory/auditLogs/Аллпропертиес/Read | Чтение всех свойств (включая привилегированные) в auditLogs в Azure Active Directory. |
| Microsoft. Directory/Клаудпровисионинг/Аллпропертиес/Аллтаскс | Чтение и настройка всех свойств службы подготовки облачных служб Azure AD. |
| Microsoft. Directory/federatedAuthentication/Аллпропертиес/Аллтаскс | Управление всеми аспектами Active Directory федеративных служб (ADFS) или стороннего поставщика Федерации в Azure AD. |
| Microsoft. Directory/организация, dirSync/обновление | Изменение свойства organization.dirSync в Azure Active Directory. |
| Microsoft. Directory/Пассвордхашсинк/Аллпропертиес/Аллтаскс | Управление всеми аспектами синхронизации хэшей паролей (PHS) в Azure AD. |
| Microsoft. Directory/Пасссраугхаусентикатион/Аллпропертиес/Аллтаскс | Управление всеми аспектами сквозной проверки подлинности (PTA) в Azure AD. |
| Microsoft. Directory/Сеамлессссо/Аллпропертиес/Аллтаскс | Управляйте всеми аспектами простого единого входа (SSO) в Azure AD. |
| Microsoft. Directory/свойства serviceprincipals/аудитория/обновление | Обновление свойства servicePrincipals.owners в Azure Active Directory. |
| Microsoft. Directory/свойства serviceprincipals, проверка подлинности или обновление | Обновление свойства servicePrincipals.authentication в Azure Active Directory. |
| Microsoft. Directory/свойства serviceprincipals/Basic/Update | Обновление базовых свойств для объектов ServicePrincipal в Azure Active Directory. |
| Microsoft. Directory/свойства serviceprincipals/создать | Создание объектов servicePrincipal в Azure Active Directory. |
...
| Microsoft. Directory/свойства serviceprincipals/разрешения/обновление | Обновление свойства servicePrincipals.permissions в Azure Active Directory. |
| Microsoft. Directory/свойства serviceprincipals/политики/обновление | Изменение свойства servicePrincipals.policies в Azure Active Directory. |
| Microsoft. Directory/свойства serviceprincipals/Синчронизатионжобс/управление | Управление всеми аспектами заданий синхронизации в Azure AD. |
| Microsoft. Directory/свойства serviceprincipals/Синчронизатионсчема/управление | Управление всеми аспектами схемы синхронизации в Azure AD. |
| Microsoft. Directory/свойства serviceprincipals/Синчронизатионкредентиалс/управление | Управление всеми аспектами учетных данных синхронизации в Azure AD. |
| Microsoft. Directory/свойства serviceprincipals/Tag/обновление | Обновите свойство свойства serviceprincipals. Tag в Azure Active Directory. |
| Microsoft. Directory/Сигнинрепортс/Аллпропертиес/Read | Чтение всех свойств (включая привилегированные) в signInReports в Azure Active Directory. |
| microsoft.office365.messageCenter/messages/read | Чтение сообщений в microsoft.office365.messageCenter. |
| microsoft.office365.serviceHealth/allEntities/allTasks | Чтение и настройка работоспособности служб Office 365. |
| microsoft.office365.supportTickets/allEntities/allTasks | Создание запросов в службу поддержки Office 365 и управление ими. |
...
Может управлять сетевыми расположениями и просматривать аналитические данные по проектированию корпоративной сети для Microsoft 365 программного обеспечения как приложений служб.
...
| --- | --- |
| Microsoft. Office 365. сеть/производительность/Аллпропертиес/чтение | Чтение страниц производительности сети в центре администрирования M365.  |
| Microsoft. Office 365. Network/Locations/Аллпропертиес/Аллтаскс | Чтение и Настройка свойств сетевых расположений для каждого расположения. |
...
### <a name="printer-administrator-permissions"></a>Разрешения администратора принтера

Может управлять всеми аспектами принтеров и соединителей принтеров.
...
| **Действия** | **Описание** |
| --- | --- |
| Microsoft. Azure. Print/Аллентитиес/Аллпропертиес/Аллтаскс | Создание и удаление принтеров и соединителей, а затем чтение и обновление всех свойств в Microsoft Print. |

### <a name="printer-technician-permissions"></a>Разрешения специалиста по печати

Может регистрировать и отменять регистрацию принтеров и обновлять состояние принтера.
...
| **Действия** | **Описание** |
| --- | --- |
| Microsoft. Azure. Print/Connectors/Аллпропертиес/Read | Чтение всех свойств соединителей в Microsoft Print. |
| Microsoft. Azure. Print/Printers/Аллпропертиес/Read | Чтение всех свойств принтеров в Microsoft Print. |
...
| Microsoft. Azure. Print/Printers/Отмена регистрации | Отмена регистрации принтеров в Microsoft Print. |
...
| microsoft.intune/allEntities/allTasks | Управление всеми аспектами Intune. |
| microsoft.office365.securityComplianceCenter/allEntities/allTasks | Прочтите и настройте центр безопасности & соответствия требованиям. |
...
Администратор атрибута потока пользователя B2C | Администратор атрибута потока пользователя B2C | 0f971eea-41eb-4569-a71e-57bb8a3eff1e
Администратор набора ключей B2C инфраструктура процедур идентификации | Администратор набора ключей B2C инфраструктура процедур идентификации | aaf43236-0c0d-4d5f-883a-6955382ac081
Администратор политики B2C инфраструктура процедур идентификации | Администратор политики B2C инфраструктура процедур идентификации | 3edaf663-341e-4475-9f94-5c398ef6c070
...
Учетные записи синхронизации службы каталогов | Не показано, так как его не следует использовать | d29b2b05-8046-44ba-8758-1e26182fcf32
Создатели каталогов | Не показано, так как его не следует использовать | 9360feb5-f418-4baa-8175-e2a00bac4301
...
Гибридный администратор удостоверений | Гибридный администратор удостоверений | 8ac3fc64-6eca-42ea-9e69-59f4c7b60eb2
...
Сетевой администратор | Сетевой администратор | d37c8bed-0711-4417-ba38-b4abe66ce4c2
Администратор приложений Office | Администратор приложений Office | 2b745bdf-0803-4d80-aa65-822c4493daac
Служба поддержка партнеров уровня 1 | Не показано, так как его не следует использовать | 4ba39ca4-527c-499a-b93d-d9b492c50246
Служба поддержка партнеров уровня 2 | Не показано, так как его не следует использовать | e00e864a-17c5-4a4b-9c06-f5b95a8d5bd8
...
Администратор принтера | Администратор принтера | 644ef478-e28f-4e28-b9dc-3fdde9aa0b1f
Специалист по печати | Специалист по печати | e8cef6f1-e4bd-4ea8-bc07-4b8d950f4477
...
Пользователь | Не показано, так как его нельзя использовать | a0b1b346-4d3e-4e8b-98f8-753987be4970
Администратор учетных записей | Администратор пользователей | fe930be7-5e62-47db-91af-98c3a49a38b1
Присоединение устройства к рабочей области | Не рекомендуется | c34f683f-4d5a-4403-аффд-6615e00e3a7f
...
Не все роли, возвращаемые PowerShell или MS API Graph, отображаются в портал Azure. Эти различия организованы в следующей таблице.
...
Администратор службы CRM | Администратор Dynamics 365 | [Отражается текущая фирменная символика продукта](directory-assign-admin-roles.md#role-template-ids)
Device Join (Присоединение устройства) | Не рекомендуется | [Документация по устаревшим ролям](directory-assign-admin-roles.md#deprecated-roles)
Диспетчеры устройств | Не рекомендуется | [Документация по устаревшим ролям](directory-assign-admin-roles.md#deprecated-roles)
Device Users (Пользователи устройства) | Не рекомендуется | [Документация по устаревшим ролям](directory-assign-admin-roles.md#deprecated-roles)
Учетные записи синхронизации службы каталогов | Не показано, так как его не следует использовать | [Документация по учетным записям синхронизации каталогов](directory-assign-admin-roles.md#directory-synchronization-accounts)
Создатели каталогов | Не показано, так как его не следует использовать | [Документация по модулям записи каталога](directory-assign-admin-roles.md#directory-writers)
Гостевой пользователь | Не показано, так как его нельзя использовать  | Н/Д
Администратор службы Lync | администратор Skype для бизнеса; | [Отражается текущая фирменная символика продукта](directory-assign-admin-roles.md#role-template-ids)
"Partner Tier 1 Support" (Поддержка для партнеров уровня 1); | Не показано, так как его не следует использовать | [Документация по службе поддержки партнеров 1 класса](directory-assign-admin-roles.md#partner-tier1-support)
"Partner Tier 2 Support" (Поддержка для партнеров уровня 2); | Не показано, так как его не следует использовать | [Документация по службе поддержки партнеров партнеров](directory-assign-admin-roles.md#partner-tier2-support)
Ограниченный гостевой пользователь | Не показано, так как его нельзя использовать | Н/Д
Пользователь | Не показано, так как его нельзя использовать | Н/Д
Присоединение устройства к рабочей области | Не рекомендуется | [Документация по устаревшим ролям](directory-assign-admin-roles.md#deprecated-roles)

## <a name="next-steps"></a>Следующие шаги

* Дополнительные сведения о назначении пользователя в качестве администратора подписки Azure см. в статье [Управление доступом с помощью ролей Azure (Azure RBAC)](../../role-based-access-control/role-assignments-portal.md) .
* Дополнительные сведения об управлении доступом к ресурсам в Microsoft Azure см. в разделе [Общие сведения о различных ролях](../../role-based-access-control/rbac-and-directory-admin-roles.md) .
