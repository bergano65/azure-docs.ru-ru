...
Пользователи с этой ролью могут устанавливать или сбросить учетные данные без пароля для некоторых пользователей и обновлять пароли для всех пользователей. Администраторы аутентификации могут требовать от пользователей, не являющихся администраторами или назначенных на некоторые роли, перерегистрироваться против существующих учетных данных, не относящих паролей (например, МИД или FIDO), а также отозвать **запоминающееся мидна на устройстве,** которое подсказывает МИД при следующем входе в систему. Эти действия применяются только к пользователям, не являющиеся администраторами или которым назначена одна или несколько из следующих ролей:
...
[Привилегированная роль администратора проверки подлинности](#privileged-authentication-administrator) имеет разрешение, может вынудить перерегистрацию и многофакторную аутентификацию для всех пользователей.
...
Пользователи с этой ролью могут устанавливать или сбросить учетные данные без пароля для всех пользователей, включая глобальных администраторов, и могут обновлять пароли для всех пользователей. Привилегированные администраторы аутентификации могут заставить пользователей перерегистрироваться против существующих учетных данных, не относящих пароль (например, MFA или FIDO) и отозвать «запомнить МИД на устройстве», что побудило МИД при следующем входе всех пользователей. Роль [администратора аутентификации](#authentication-administrator) может принудить к перерегистрации и МИД только для не-администраторов и пользователей, назначенных для следующих ролей Azure AD:
...
| microsoft.office365.exchange/allEntities/allTasks | Управление всеми аспектами Exchange Online. |
| microsoft.office365.network/performance/allProperties/read | Прочитайте страницы производительности сети в M365 Admin Center. |
| microsoft.office365.serviceHealth/allEntities/allTasks | Чтение и настройка работоспособности служб Office 365. |
| microsoft.office365.supportTickets/allEntities/allTasks | Создание запросов в службу поддержки Office 365 и управление ими. |
| microsoft.office365.usageReports/allEntities/read | Чтение отчетов об использовании Office 365. |
| microsoft.office365.webPortal/allEntities/basic/read | Чтение базовых свойств для всех ресурсов в microsoft.office365.webPortal. |
...
### <a name="hybrid-identity-administrator-permissions"></a>Разрешения администратора гибридных идентификационных данных

Включить, развернуть, настроить, управлять, контролировать и устранить проблемы облачных служб подготовки и аутентификации. 
...
| microsoft.directory/applications/permissions/update | Изменение свойства applications.permissions в Azure Active Directory. |
| microsoft.directory/applications/policies/update | Изменение свойства applications.policies в Azure Active Directory. |
| microsoft.directory/applicationTemplates/instantiate | Мгновенное приложение галереи из шаблонов приложений. |
| microsoft.directory/auditLogs/allProperties/read | Чтение всех свойств (включая привилегированные) в auditLogs в Azure Active Directory. |
| microsoft.directory/cloudProvisioning/allProperties/allTasks | Прочитайте и настройте все свойства службы облачного обеспечения Azure AD. |
| microsoft.directory/federatedAuthentication/allProperties/allTasks | Управление всеми аспектами службы активных federated Services (ADFS) или поставщика 3-й стороны федерации в Azure AD. |
| microsoft.directory/organisation/dirSync/update | Изменение свойства organization.dirSync в Azure Active Directory. |
...
| microsoft.directory/passThroughAuthentication/allProperties/allTasks | Управление всеми аспектами пассивной аутентификации (PTA) в Azure AD. |
| microsoft.directory/seamlessSSO/allProperties/allTasks | Управление всеми аспектами бесшовных единого входного (SSO) в Azure AD. |
| microsoft.directory/servicePrincipals/audience/update | Обновление свойства servicePrincipals.owners в Azure Active Directory. |
| microsoft.directory/servicePrincipals/authentication/update | Обновление свойства servicePrincipals.authentication в Azure Active Directory. |
| microsoft.directory/servicePrincipals/basic/update | Обновление базовых свойств для объектов ServicePrincipal в Azure Active Directory. |
| microsoft.directory/servicePrincipals/create | Создание объектов servicePrincipal в Azure Active Directory. |
...
| microsoft.directory/servicePrincipals/tag/update | Обновление свойства службыPrincipals.tag в Active Directory Azure. |
| microsoft.directory/signInReports/allProperties/read | Чтение всех свойств (включая привилегированные) в signInReports в Azure Active Directory. |
| microsoft.office365.messageCenter/messages/read | Чтение сообщений в microsoft.office365.messageCenter. |
| microsoft.office365.serviceHealth/allEntities/allTasks | Чтение и настройка работоспособности служб Office 365. |
| microsoft.office365.supportTickets/allEntities/allTasks | Создание запросов в службу поддержки Office 365 и управление ими. |
...
Можно управлять местоположениями сетей и просматривать идеи проектирования корпоративных сетей для Microsoft 365 Software в качестве приложения службы.
...
| --- | --- |
| microsoft.office365.network/performance/allProperties/read | Прочитайте страницы производительности сети в M365 Admin Center.  |
| microsoft.office365.network/locations/allProperties/allTasks | Прочитайте и назначайте свойства местоположений сети для каждого местоположения. |
...
Можно управлять всеми аспектами принтеров и разъемов принтеров.
...
| **Действия** | **Описание** |
| --- | --- |
| microsoft.azure.print/allEntities/allProperties/allTasks | Создавайте и удаляйте принтеры и разъемы, а также считывайте и обновляйте все свойства в Microsoft Print. |

### <a name="printer-technician-permissions"></a>Разрешения техников принтера

Можно зарегистрировать и отменить принтеры и обновить статус принтера.
...
| **Действия** | **Описание** |
| --- | --- |
| microsoft.azure.print/connectors/allProperties/read | Ознакомьтесь со всеми свойствами разъемов в Microsoft Print. |
| microsoft.azure.print/printers/allProperties/read | Ознакомьте все свойства принтеров в Microsoft Print. |
...
| microsoft.azure.print/printers/unregister | Не регистрируйте принтеры в Microsoft Print. |
...
Администратор гибридной идентификации | Администратор гибридных идентификационных данных | 8ac3fc64-6eca-42ea-9e69-59f4c7b60eb2
...
Техник принтера | Техник принтера | e8cef6f1-e4bd-4ea8-bc07-4b8d950f4477
