...
Пользователи с этой ролью могут считывать параметры и административные данные в службах Microsoft 365 Services, но не могут выполнять действия по управлению. Глобальный читатель является аналогом глобального администратора, предназначенным только для чтения. Назначьте глобальному считывателю, а не глобальному администратору планирование, аудит или расследование. Используйте глобальное средство чтения в сочетании с другими ограниченными ролями администрирования, такими как администратор Exchange, чтобы упростить работу без вызова роли глобального администратора. Глобальный модуль чтения работает с новым центром администрирования Microsoft 365, центром администрирования Exchange, центром администрирования группы, центром безопасности, центром соответствия, центром администрирования Azure AD и центром администрирования управления устройствами. 

> [!NOTE]
> Роль глобального читателя теперь имеет следующие ограничения —
>* Центр администрирования SharePoint — центр администрирования SharePoint не поддерживает роль глобального читателя. Поэтому вы не увидите "SharePoint" в левой области в разделе "центры администрирования" [центра администрирования M365](https://admin.microsoft.com/Adminportal/Home#/homepage). При переходе по адресу https://{имя клиента}-admin.sharepoint.com будет получен отказ в доступе. 
>* [Портал Azure AD](https://portal.azure.com/#blade/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/AllApps/menuId/) : глобальный считыватель не может читать режим подготовки для корпоративного приложения.
>* [Центр администрирования M365](https://admin.microsoft.com/Adminportal/Home#/homepage) — глобальный модуль чтения не может читать запросы к защищенному хранилище кусомтер. На вкладке поддержка в левой области центра администрирования M365 не будет найдено ни одной вкладки запросов на защищенный клиент.
>* [M365 Security Center](https://security.microsoft.com/homepage) — глобальный модуль чтения не может считывать метки чувствительности и хранения. Вы не найдете метки чувствительности, метки хранения и вкладки аналитики меток в левой области в разделе классификация в центре безопасности M365.
>* [Центр администрирования группы](https://admin.teams.microsoft.com) — глобальный модуль чтения не может читать жизненный цикл команд, отчеты & аналитика вызовов, Управление устройствами с IP-телефонами и каталог приложений.
>* [Privileged Access Management (PAM)](https://docs.microsoft.com/en-us/office365/securitycompliance/privileged-access-management-overview) не поддерживает глобальное средство чтения.
> 
> Поддержка этих функций будет происходить со временем.
>
...
| Microsoft. Directory/Сервицеактион/Активатесервице | Может выполнять действие службы Activateservice в Azure Active Directory |
| Microsoft. Directory/Сервицеактион/Дисабледиректорифеатуре | Может выполнять действие службы Disabledirectoryfeature в Azure Active Directory |
| Microsoft. Directory/Сервицеактион/Енабледиректорифеатуре | Может выполнять действие службы Enabledirectoryfeature в Azure Active Directory |
| Microsoft. Directory/Сервицеактион/Жетаваилабликстентионпропертиес | Может выполнять действие службы Getavailableextentionproperties в Azure Active Directory |
...
Администратор атрибутов потоков пользователей B2C | Администратор атрибутов потоков пользователей B2C | 0f971eea-41eb-4569-a71e-57bb8a3eff1e
...
Пользовательская | Пользовательская | a0b1b346-4d3e-4e8b-98f8-753987be4970
