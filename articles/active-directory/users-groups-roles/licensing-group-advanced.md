---
title: Дополнительные сценарии лицензирования на основе группы - Azure AD Документы Майкрософт
...
Некоторые службы Майкрософт недоступны во всех расположениях. Прежде чем назначать лицензию, администратор должен указать для пользователя свойство **Место использования**. На [портале Azure](https://portal.azure.com)можно указать местоположение использования в **настройках** **профиля** &gt; **пользователя.** &gt;

Если лицензии назначаются группам, все пользователи, для которых не указано расположение, наследуют расположение каталога. Если у вас есть пользователи в нескольких местах, убедитесь, что правильно отразить в ваших пользовательских ресурсов, прежде чем добавлять пользователей в группы с лицензиями.
...
Лицензирование на основе группы можно использовать с любой группой безопасности, а это значит, что ее можно использовать с динамическими группами Azure AD. Динамические группы заражают правила против атрибутов ресурса пользователя для автоматического добавления и удаления пользователей из групп.
...
- При выборе лицензии *E3* отображаются более подробные сведения, в ключая информацию о том, какие службы включены для пользователя по назначению групповой лицензии.
...
Первоначально пользователь наследует лицензию только из группы *базовых служб Е3,* которая позволяет выполнять четыре плана обслуживания.

1. Выберите **назначить** пользователю лицензию E3 напрямую. В этом случае вы собираетесь отключить все планы службы, за исключением Yammer корпоративный.

    В результате пользователь по-прежнему использует только одну лицензию на продукт E3, но при прямом назначении служба Yammer корпоративный активируется только для этого пользователя. Вы можете увидеть, какие службы включены членством в группе по сравнению с прямым назначением.

1. При использовании прямого назначения разрешены следующие операции:

   - Yammer Enterprise может быть отключен на пользовательском ресурсе напрямую. В отличие от других переключателей службы переключатель **Вкл./Выкл.** активирован, так как эта служба включена непосредственно для пользователя, и поэтому ее можно изменить.
...
Когда корпорация Майкрософт добавляет новую службу в план лицензирования продукта, она включается по умолчанию во всех группах, которым вы присвоили лицензию продукта. Пользователи в клиенте, которые подписаны на уведомления об изменениях продукта, заранее получат сообщения электронной почты с уведомлением о предстоящих дополнениях.
...
4. Щелкните группу, чтобы просмотреть ее (в этом случае *O365 E5 - Exchange only*). Это откроет вкладку Лицензии. Нажатие на лицензию E5 откроет лезвие, перечисляющее все включенные **службы.**
   > [!NOTE]
...
3. Выберите элемент в списке, чтобы увидеть детали того, что изменилось. В разделе *Измененные свойства* отображаются старые и новые значения для назначения лицензии.
...
2. Аналогичным образом, чтобы узнать, когда была завершена обработка групп, используйте значение фильтра *Finish Applying Group Based License To Users* (Завершить назначать пользователям лицензию, назначенную группе).
   > [!TIP]
   > В этом случае в поле *Измененные свойства* содержится сводка по результатам — это позволяет быстро проверить, возникали ли ошибки при обработке. Образец вывода:
   > ```
   > Modified Properties
   > ...
   > Name : Result
...
- В настоящее время центр управления Microsoft [365](https://admin.microsoft.com) не поддерживает групповое лицензирование. Если пользователь наследует лицензию от группы, эта лицензия отображается на портале администрирования Office как обычная лицензия пользователя. При попытке изменить или удалить эту лицензию на портале появится сообщение об ошибке. Наследуемые от группы лицензии невозможно изменить непосредственно для пользователя.
