...
2. Выберите **группы**, а затем выберите **истечения срока действия** чтобы открыть параметры срока действия.
  
   ![Параметры срока действия для групп](./media/groups-lifecycle/expiration-settings.png)

3. В колонке **Срок действия** можно сделать следующее.
...
![Уведомления по электронной почте для удаления группы](./media/groups-lifecycle/deletion-notification.png)
...
Приведенные ниже командлеты можно использовать для более точной настройки политики. Дополнительные сведения см. в разделе [документации PowerShell](https://docs.microsoft.com/powershell/module/azuread/?view=azureadps-2.0-preview&branch=master#groups).
