...
Действия фоллоинг приведут к автоматическому продлению группы:

- SharePoint — просмотр, изменение, скачивание, перемещение, совместное использование и отправка файлов
- Outlook — объединение сообщений группы, чтение и запись группы и как сообщение
- Группы — посетите канал команд
...
> [!NOTE]
> При первой настройке срока действия все группы, срок действия которых истек, задаются в течение 35 дней до истечения срока действия, если только группа не будет обновлена или владелец не пополнит ее. 
>
...
1. Удалите существующую политику с помощью Remove-AzureADMSGroupLifecyclePolicy. Этот командлет удаляет параметры срока действия группы Office 365, но требует указать идентификатор политики. При этом будет отключен срок действия групп Office 365.
