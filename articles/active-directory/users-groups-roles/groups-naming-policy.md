---
title: Принудительная политика именования групп в Active Directory Azure (ru) Документы Майкрософт
description: Как настроить политику именования для групп Office 365 в каталоге Active
...
ms.locfileid: "79497876"
---
# <a name="enforce-a-naming-policy-on-office-365-groups-in-azure-active-directory"></a>Принудить политику именования в группах Office 365 в каталоге Azure Active
...
> [!IMPORTANT]
> Использование политики именования Azure AD для групп Office 365 требует, чтобы вы обладали, но не обязательно присваивали лицензию Azure Active Directory Premium P1 или лицензию Azure AD Basic EDU для каждого уникального пользователя, который является членом одной или нескольких групп Office 365.

Политика именования применяется при создании или изменении групп, созданных для рабочих нагрузок (например, Outlook, Microsoft Teams, SharePoint, Exchange или службы "Планировщик"). Она применяется к имени и псевдониму группы. Если вы настроили политику именования в Azure AD и у вас есть существующая политика именования группы Exchange, в вашей организации обеспечивается политика именования Azure AD.

При настройке политики группового именования политика будет применяться к новым группам Office 365, созданным конечными пользователями. Политика именования не применяется к определенным ролям каталога, таким как Глобальный администратор или Администратор пользователей (см. ниже полный список ролей, исключенных из политики именования группы). Для существующих групп Office 365 политика не будет применяться сразу во время настройки. После того, как владелец группы отодвинет название группы для этих групп, политика именования будет приведена в исполнение.

## <a name="naming-policy-features"></a>Возможности политики именования

Вы можете применять политику именования для групп двумя различными способами:
...
Для настройки политики именования требуется одна из ролей folowing:
...
1. Войти на [администратора Azure Ad](https://aad.portal.azure.com) с учетной записью администратора группы.
...
### <a name="view-or-edit-the-prefix-suffix-naming-policy"></a>Просмотр или отображее политики именования префикса-суффикса
...
1. Сохраните изменения для вхуса в действие новой политики, выбрав **Сохранить.**

### <a name="edit-custom-blocked-words"></a>Отображете пользовательские заблокированные слова
...
3. Установите последнюю версию AzureADPreview.
   ...
   Если вам предлагается получить доступ к недоверчивому репозиторию, введите **Y**. Установка нового модуля может занять несколько минут.
...
1. Сохраните настройки для вхуса новой политики в силу, например в следующем примере.
...
## <a name="export-or-import-custom-blocked-words"></a>Экспортные или импортные пользовательские заблокированные слова

Для получения дополнительной информации смотрите в статье [Azure Active Directory cmdlets для настройки параметров группы](groups-settings-cmdlets.md).
...
## <a name="experience-across-office-365-apps"></a>Опыт работы в приложениях Office 365
...
Центр администрирования Microsoft 365 | Центр управления microsoft 365 соответствует политике именования. При пользователь создает или изменяет имена групп, политика именования применяется автоматически, и пользователи будут видеть соответствующие сообщения об ошибках при вводе настраиваемых запрещенных слов. Центр microsoft 365 пока не показывает предварительный просмотр политики именования и не возвращает пользовательские заблокированные ошибки слов, когда пользователь вводит имя группы.
