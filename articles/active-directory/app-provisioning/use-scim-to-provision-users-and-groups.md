...
Схема, определенная выше, будет представлена с помощью полезных данных Json, приведенных ниже. Обратите внимание, что в дополнение к атрибутам, необходимым для приложения, представление JSON содержит обязательные атрибуты "id", "externalId" и "meta".
...
|/Schemas|Набор атрибутов, поддерживаемых каждым клиентом и поставщиком служб, может различаться. Хотя один поставщик услуг может включать "name", "title," и "emails", в то время как другой поставщик услуг использует "name", "title" и "phoneNumbers". Конечная точка schemas позволяет обнаруживать поддерживаемые атрибуты.|
...
В настоящее время служба подготовки Azure AD может опперате в любой диапазон IP-адресов Azure. Работа выполняется для консолидации набора диапазонов IP-адресов, с которыми работает служба. Этот документ будет обновлен после консолидации списка диапазонов IP-адресов. 
...
Azure Active Directory запрашивает службу пользователя, у которого значение атрибута externalId совпадает со значением атрибута mailNickname пользователя Azure AD. Запрос имеет вид HTTP-запроса, пример которого приведен ниже. Значение jyoung в этом запросе обозначает пример mailNickname пользователя Azure Active Directory.
...
В примере запроса для пользователя с заданным значением для атрибута externalId, значения аргументов, переданных в метод QueryAsync будут:
