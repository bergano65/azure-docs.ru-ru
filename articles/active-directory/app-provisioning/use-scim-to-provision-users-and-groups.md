---
title: Разработка конечной точки SCIM для подготовки пользователей к работе с приложениями из Azure AD
...
ms.locfileid: "91967058"
---
# <a name="build-a-scim-endpoint-and-configure-user-provisioning-with-azure-ad"></a>Создание конечной точки SCIM и настройка подготовки пользователей с Azure AD
...
Схема, определенная выше, будет представлена с помощью полезных данных JSON ниже. Обратите внимание, что в дополнение к атрибутам, необходимым для приложения, представление JSON включает обязательные `id` `externalId` атрибуты, и `meta` .
...
|/Schemas|Набор атрибутов, поддерживаемых каждым клиентом и поставщиком служб, может различаться. Один поставщик услуг может включать `name` , `title` и `emails` , тогда как другой поставщик услуг использует `name` , `title` и `phoneNumbers` . Конечная точка schemas позволяет обнаруживать поддерживаемые атрибуты.|
...
* Поддержка изменения пользователей и групп с помощью запросов PATCH, как описано [в разделе 3.5.2 протокола SCIM](https://tools.ietf.org/html/rfc7644#section-3.5.2). Поддержка гарантирует, что группы и пользователи подготавливаются к работе по принципу выполнения. 
...
* Поддерживает обратимое удаление пользователя `active=false` и восстановление пользователя `active=true` (объект User должен возвращаться в запросе независимо от того, является ли пользователь активным). Единственный момент, когда пользователь не должен возвращаться, — это момент, когда он твердо удаляется из приложения. 
...
Сейчас служба подготовки Azure AD работает в диапазонах IP-адресов для AzureActiveDirectory, как указано [здесь](https://www.microsoft.com/download/details.aspx?id=56519&WT.mc_id=rss_alldownloads_all). Вы можете добавить диапазоны IP-адресов, перечисленные в теге AzureActiveDirectory, чтобы разрешить трафик из службы подготовки Azure AD в приложение. Обратите внимание, что необходимо внимательно просмотреть список диапазонов IP-адресов для вычислено. Адрес, например "40.126.25.32", может быть представлен в списке диапазонов IP-адресов как "40.126.0.0/18". Можно также [программно](https://docs.microsoft.com/rest/api/virtualnetwork/servicetags/list)получить список диапазонов IP-адресов с помощью следующего API.
...
Azure Active Directory запрашивает у службы пользователя `externalId` значение атрибута, совпадающее со значением атрибута mailNickname пользователя в Azure AD. Запрос имеет вид HTTP-запроса, пример которого приведен ниже. Значение jyoung в этом запросе обозначает пример mailNickname пользователя Azure Active Directory.

>[!NOTE]
> Это только пример. Не все пользователи будут иметь атрибут mailNickname, а значение пользователя может не быть уникальным в каталоге. Кроме того, атрибут, используемый для сопоставления (в данном случае это `externalId` ), можно настроить в [сопоставлениях АТРИБУТОВ Azure AD](customize-application-attributes.md).
...
В образце запроса для пользователя с заданным значением `externalId` атрибута значениями аргументов, передаваемых в метод QueryAsync, являются:
...
Если ответ на запрос к веб-службе для пользователя со `externalId` значением атрибута, совпадающим со значением атрибута mailNickname пользователя, не возвращает никаких пользователей, то Azure Active Directory запрашивает, что служба подготавливает пользователя, соответствующего этой службе, в Azure Active Directory.  Ниже приведен пример такого запроса. 
...
9. Выберите **Проверить подключение**. Azure Active Directory попробует подключиться к конечной точке SCIM. Если подключиться не удастся, отобразятся сведения об ошибке.  
...
> * Поддержка по крайней мере 25 запросов в секунду для каждого клиента, чтобы обеспечить подготовку пользователей и групп и их отмену без задержки (обязательно).
...
|Предоставление кода авторизации OAuth|Срок действия маркеров доступа намного короче, чем у паролей, поэтому они имеют механизм автоматического обновления, который не имеет долгосрочных маркеров носителя.  Реальный пользователь должен присутствовать во время начальной авторизации для добавления уровня ответственности. |Требуется наличие пользователя. Если пользователь покидает организацию, маркер является недействительным, и авторизацию следует выполнять заново.|Поддерживается для приложений коллекции, но не для приложений, не относящихся к коллекции. Однако можно предоставить маркер доступа в пользовательском интерфейсе в качестве секретного маркера для краткосрочных целей тестирования. Поддержка предоставления кода OAuth в не коллекции находится в невыполненной работе.|
...
Шаги в потоке предоставления кода OAuth:
1. Пользователь входит в портал Azure > корпоративные приложения > выбрать приложение > подготовки > щелкните Авторизовать.
2. Портал Azure перенаправляет пользователя на URL-адрес авторизации (страница входа для стороннего приложения).
3. Администратор предоставляет учетные данные для приложения стороннего производителя. 
4. Стороннее приложение перенаправляет пользователя в портал Azure и предоставляет код предоставления 
5. Службы подготовки Azure AD вызывают URL-адрес маркера и предоставляют код предоставления. Стороннее приложение реагирует на маркер доступа, маркер обновления и дату истечения срока действия
6. Когда начинается цикл подготовки, служба проверяет, является ли текущий маркер доступа допустимым, и при необходимости обменивается с новым токеном. Маркер доступа предоставляется в каждом запросе, сделанном в приложении, и срок действия запроса проверяется перед каждым запросом.

> [!NOTE]
> Пока невозможно настроить OAuth в приложении, не являющемся коллекцией, можно вручную создать маркер доступа на сервере авторизации и ввести данные в поле Секретный токен приложения не из коллекции. Это позволяет проверить совместимость сервера SCIM с клиентом Azure AD SCIM перед подключением к коллекции приложений, которая поддерживает предоставление кода OAuth.  

**Долгосрочные токены носителя OAuth:** Если приложение не поддерживает поток предоставления кода авторизации OAuth, можно также создать долгосрочный токен носителя OAuth, который может использоваться администратором для настройки интеграции подготовки. Маркер должен быть бессрочным, иначе задание подготовки будет [помещено в карантин](application-provisioning-quarantine-status.md) по истечении срока действия маркера. Размер этого маркера должен быть меньше 1 КБ.  
