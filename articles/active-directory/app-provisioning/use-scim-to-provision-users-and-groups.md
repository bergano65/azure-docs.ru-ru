...
* Поддержка изменения пользователей и групп с помощью запросов PATCH, как описано [в разделе 3.5.2 протокола SCIM](https://tools.ietf.org/html/rfc7644#section-3.5.2).  
...
Служба подготовки Azure AD в настоящее время работает в диапазонах IP-адресов для AzureActiveDirectory и Азуреактиведиректоридомаинсервицес, как указано [здесь](https://www.microsoft.com/download/details.aspx?id=56519&WT.mc_id=rss_alldownloads_all). Работа выполняется для консолидации только диапазонов IP-адресов в AzureActiveDirectory. 
...
Дополнительные сведения об использовании HTTPS в ASP.NET Core см. в статье: [Принудительное применение HTTPS в ASP.NET Core](https://docs.microsoft.com/aspnet/core/security/enforcing-ssl)
...
> [!div class="checklist"]
> * Поддержка конечной точки [SCIM 2.0](https://docs.microsoft.com/azure/active-directory/app-provisioning/use-scim-to-provision-users-and-groups#step-2-understand-the-azure-ad-scim-implementation) пользователя и группы (необходим только один, однако рекомендуются оба)
> * Поддержка не менее 25 запросов в секунду для каждого клиента (обязательно)
