...
![Схема, на которой показан процесс создания пользователей или групп из локального приложения H R с помощью службы подготовки Azure A D в целевом приложении S a S.](./media/plan-auto-user-provisioning/cloudprovisioning.png)
...
Расширьте развертывание на более крупные группы пользователей, увеличив область целевых групп. Это можно сделать с помощью [динамического членства в группе](../enterprise-users/groups-dynamic-membership.md)или вручную путем добавления пользователей в целевые группы (-ы).
...
1. [Создайте запрос](../develop/v2-howto-app-gallery-listing.md) для предварительно интегрированного соединителя подготовки пользователей. Наша группа будет работать с вами и разработчиком приложения, чтобы подключить приложение к нашей платформе, если оно поддерживает SCIM.
...
1. Если приложение может использовать соединитель БЙОА SCIM, см. [руководство по интеграции БЙОА scim](../app-provisioning/use-scim-to-provision-users-and-groups.md) , чтобы настроить соединитель бйоа scim для приложения.
...
* Затем при необходимости используйте [назначения пользователей и групп](../manage-apps/assign-user-or-group-access-portal.md) для дополнительной фильтрации.
...
Дополнительные сведения см. [в разделе Настройка подготовки пользователей Attribute-Mappings для приложений SaaS в Azure Active Directory](../app-provisioning/customize-application-attributes.md).
...
Для настройки службы подготовки пользователей Azure AD и управления ею можно также использовать [API Microsoft Graph](/graph/api/resources/synchronization-overview).
...
* Служба останавливается вручную, а новый начальный цикл запускается с помощью [портал Azure](https://portal.azure.com/)или с помощью соответствующей команды [API Microsoft Graph](/graph/api/resources/synchronization-overview) .
