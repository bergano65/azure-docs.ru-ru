...
* Пользователь является обратимо удален в Azure AD (перезапускается в свойстве корзины/AccountEnabled, для которого задано значение false).
    Через 30 дней после удаления пользователя из Azure AD он будет окончательно удален из клиента. На этом этапе служба подготовки отправит запрос DELETE для окончательного удаления пользователя из приложения. В любое время в течение 30-дневного периода можно [вручную удалить пользователя](../fundamentals/active-directory-users-restore.md), что приведет к отправке запроса на удаление в приложение.
