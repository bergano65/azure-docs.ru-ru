...
7. Определите предложение, выбрав исходные параметры **Имя атрибута**, **Оператор** и **Значение атрибута** для сопоставления. Поддерживаются следующие операторы:
   ...
   а. **ЭКВАЛС**. Предложение возвращает значение true, если оцениваемый атрибут точно совпадает со значением входной строки (с учетом регистра).

   b. **НЕ АУКВАЛС**. Предложение возвращает значение true, если оцениваемый атрибут не совпадает со значением входной строки (с учетом регистра).
   ...
   д) **ЯВЛЯЕТСЯ НЕДЕЙСТВИТЕЛЬНЫМ**. Предложение возвращает значение true, если вычисленный атрибут имеет пустое значение.
   ...
   i. **Greater_Than.** Клаузе возвращает "истинно", если оцененный атрибут больше значения. Значение, указанное на фильтре скопирования, должно быть бесразмереным, а атрибут на пользователе должен быть бесразмеренным (0,1,2,...). 
   
   j. **Greater_Than_OR_EQUALS.** Клаузе возвращает "истинно", если оцененный атрибут больше или равен значению. Значение, указанное на фильтре скопирования, должно быть бесразмереным, а атрибут на пользователе должен быть бесразмеренным (0,1,2,...). 
   
   k. **Включает.** Клаузе возвращает "истинно", если оцененный атрибут содержит значение строки (дело чувствительное), как описано [здесь.](https://docs.microsoft.com/dotnet/api/system.string.contains?view=netframework-4.8) 


>[!IMPORTANT] 
> - Фильтр IsMemberOf в настоящее время не поддерживается.
> - АУКВАЛС и НЕ АВЕКТЕС не поддерживаются для мультицено-ценных атрибутов
...
>[!IMPORTANT] 
> Сохранение нового фильтра области активирует полную синхронизацию для приложения. Все пользователи в исходной системе будут повторно оценены с применением нового фильтра области. Если ранее пользователь приложения сначала соответствовал области для подготовки, а затем перестал соответствовать, его учетная запись в приложении будет отключена или отозвана. Чтобы переопределить это поведение по умолчанию, обратитесь к [удалению Skip для учетных записей пользователей, которые выходят за рамки.](../app-provisioning/skip-out-of-scope-deletions.md)


## <a name="common-scoping-filters"></a>Общие фильтры для скопирования
| Целевой атрибут| Оператор | Значение | Описание|
|----|----|----|----|
|userPrincipalName|РЕГЕКС МАТЧ|.\*@domain.com |Все пользователи с пользователемPrincipal, который имеет домен @domain.com будет в области подготовки|
|userPrincipalName|НЕ REGEX МАТЧ|.\*@domain.com|Все пользователи с пользователемPrincipal, который имеет домен @domain.com будет вне сферы для подготовки|
|department|EQUALS|sales|Все пользователи из отдела продаж находятся в возможности подготовки|
|workerID|РЕГЕКС МАТЧ|(1[0–9][0–9][0–9][0–9][0–9][0–9]).| Все сотрудники с рабочими идентизацией между 1000000 и 2000000 находятся в возможности подготовки.|

## <a name="related-articles"></a>Связанные статьи
* [Автоматизация подготовки пользователей и деобеспечения приложений SaaS](../app-provisioning/user-provisioning.md)
