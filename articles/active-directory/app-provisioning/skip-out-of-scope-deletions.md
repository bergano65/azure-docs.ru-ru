...
* Если для ***скипаутофскопеделетионс*** задано значение 1 (true), то учетные записи, выходящие за пределы области, не будут отключены в целевом объекте. Этот флаг задается на уровне *приложения подготовки* и может быть настроен с помощью API Graph. 
