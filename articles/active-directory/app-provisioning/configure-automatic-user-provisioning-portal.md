...
В этой статье описаны общие действия по управлению автоматической подготовкой учетных записей пользователей и их отзыву для приложений, которые его поддерживают. *Подготовка учетной записи пользователя* — это создание, обновление или отключение записей для учетных записей пользователя в локальном хранилище профилей пользователей. Большинство облачных и SaaS-приложений сохраняют роль пользователей и разрешения в собственном хранилище профиля пользователя, и наличие такой записи пользователя в локальном хранилище пользователя *требуется* для единого входа и доступа к работе. Дополнительные сведения об автоматической подготовке учетных записей пользователей см. в статье [Автоматизация подготовки пользователей и ее отмене в приложениях SaaS с помощью Azure Active Directory](user-provisioning.md).

> [!IMPORTANT]
> Azure Active Directory (Azure AD) содержит коллекцию, содержащую тысячи предварительно интегрированных приложений, для которых включена автоматическая подготовка с помощью Azure AD. Сначала необходимо найти руководство по настройке подготовки, относящееся к вашему приложению, в [списке руководств по интеграции приложений SaaS с Azure Active Directory](https://azure.microsoft.com/documentation/articles/active-directory-saas-tutorial-list/). Скорее всего, вы найдете пошаговое руководство по настройке приложения и Azure AD для создания подключения для подготовки.
...
**Очистка текущего состояния и перезапуск синхронизации** запускает начальный цикл. Затем служба будет оценивать всех пользователей в исходной системе и определить, находятся ли они в области для подготовки. Это может быть полезно, если приложение находится в карантине или необходимо внести изменения в сопоставления атрибутов. Обратите внимание, что начальный цикл занимает больше времени, чем типичный инкрементный цикл, из-за количества объектов, которые необходимо вычислить. Дополнительные сведения о производительности начального и добавочного циклов см. [здесь](application-provisioning-when-will-provisioning-finish-specific-user.md). 
