...
| --- | --- | --- | --- |
| **source** |Обязательно |Строковый тип |Как правило, имя атрибута из исходного объекта. |
| **suffix** |Обязательно |Строковый тип |Строка, которую необходимо присоединить к концу исходного значения. |
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **source1  … sourceN** | Обязательно | Строковый тип |Обязательно, переменное число раз. Как правило, имя атрибута из исходного объекта. |
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **source** |Обязательно |Строковый тип |Строка, которая должна быть преобразована в Base 64|
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **source** |Обязательно |Строковый тип |Строка для преобразования в шестнадцатеричное значение UTF8|
...
| --- | --- | --- | --- |
| **source** |Обязательно |Строковый тип |Как правило, имя атрибута из исходного объекта. |
| **инпутформат** |Обязательно |Строковый тип |Предполагаемый формат исходного значения. Поддерживаемые форматы см. в разделе [/дотнет/Стандард/Басе-типес/кустом-дате-Анд-Тиме-формат-стрингс](/dotnet/standard/base-types/custom-date-and-time-format-strings). |
| **outputFormat** |Обязательно |Строковый тип |Формат вывода даты. |
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **value1** |Обязательно |Строковый тип |Строка для поиска |
| **value2** |Обязательно |Строковый тип |Строка для поиска |
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **separator**язательно, количество |Обязательно |Строковый тип |Строка, используемая для разделения исходных значений, когда они объединяются в одну строку. Если разделитель не требуется, может не иметь значения (""). |
| **source1  … sourceN** |Обязательно, количество повторов может меняться. |Строковый тип |Строковые значения для объединения. |
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **source** |Обязательно |Строковый тип |Как правило, имя атрибута. |
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **source** |Обязательно |Строковый тип | Обычно атрибут имени или фамилии. |
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **value** |Обязательно | Строковый тип | Строка даты и времени в поддерживаемом формате. Поддерживаемые форматы см. по адресу https://msdn.microsoft.com/library/8kb3ddd4%28v=vs.110%29.aspx. |
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **source** |Обязательно |Строковый тип |Обычно это имя атрибута из **исходного** объекта. |
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **uniqueValueRule1... uniqueValueRuleN** |Требуется не менее 2, нет верхней границы |Строковый тип | Список правил формирования уникальных значений для оценки. |
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **[appRoleAssignments]** |Обязательно |Строковый тип |Объект **[appRoleAssignments]**. |
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **source** |Обязательно |Строковый тип |**источник** — обновляемое значение. |
| **разделитель** |Обязательно |Строковый тип |Задает символ, который будет использоваться для разделения строки (к примеру, ",") |
...
| Имя | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **source** |Обязательно |Строковый тип |Значение **source**, которое необходимо обновить |
...
| **key** |Обязательно |Строковый тип |Параметр **key**, с которым сравнивается значение **source** |
| **value** |Обязательно |Строковый тип |Значение, которым заменяется значение **source** , соответствующее ключу. |
...
| **разделители** |Обязательно |Строковый тип| Строка, представляющая разделители, которые должны использоваться для поиска слов|
