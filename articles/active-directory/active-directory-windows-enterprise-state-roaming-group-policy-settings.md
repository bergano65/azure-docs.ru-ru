...
Параметры политики MDM применяются к Windows 10 и Windows 10 Mobile.  Поддержка Windows 10 Mobile существует только для перемещения на основе учетной записи Майкрософт через учетную запись пользователя OneDrive.  Дополнительные сведения о том, для каких устройств поддерживается синхронизация на основе Azure AD, см. в разделе [Устройства и конечные точки](active-directory-windows-enterprise-state-roaming-windows-settings-reference.md).
