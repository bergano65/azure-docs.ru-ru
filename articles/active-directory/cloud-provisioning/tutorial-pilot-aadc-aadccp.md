...
3. Убедитесь, что для объектов в области пилотного развертывания заполнен параметр ms-ds-consistencyGUID, чтобы при подготовке облачных служб происходило строгое сопоставление этих объекты. Обратите внимание, что служба синхронизации Azure AD Connect по умолчанию не заполняет ms-ds-consistencyGUID для групповых объектов.
...
Версия службы [Azure AD Connect](https://www.microsoft.com/download/details.aspx?id=47594) не должна быть ниже 1.4.32.0. Чтобы обновить службу синхронизации Azure AD Connect, выполните шаги из статьи [Azure AD Connect: обновление до последней версии](../hybrid/how-to-upgrade-previous-version.md).  Этот шаг предоставляется на случай, если в тестовой среде отсутствует последняя версия Azure AD Connect.
...
## <a name="scope-azure-ad-connect-sync-to-exclude-the-pilot-ou"></a>Исключение подразделения пилотного проекта из области службы синхронизации Azure AD Connect
Теперь в Azure AD Connect вам нужно исключить ранее созданное подразделение пилотного проекта.  Синхронизацию этих пользователей будет выполнять агент подготовки облака.  Для настройки области Azure AD Connect выполните следующие действия.
