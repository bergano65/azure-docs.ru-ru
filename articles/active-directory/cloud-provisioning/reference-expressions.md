...
|[IIF](#iif)|Функция IIF возвращает один из набора возможных значений на основе заданного условия.|
...
|[Join](#join) |Функция Join() схожа с функцией Append() за исключением того, что она может объединять несколько строковых значений аргумент в одну строку. При этом каждое значение разделяется строкой **separator**.| 
...
|[Not](#not) |Обращает логическое значение переданного аргумента. Если значение аргумента равно *True*, возвращается значение *False*. В противном случае возвращает значение*True*.| 
...
|[SelectUniqueValue](#selectuniquevalue)|Требует как минимум два аргумента, которые являются правилами формирования уникальных значений, определенными с помощью выражений. Функция вычисляет каждое правило, а затем проверяет сформированное значение на уникальность в целевом приложении/каталоге.| 
...
|[Split](#split)|Разделяет строку на массив подстрок с помощью определенного символа разделителя.|
...
**Синтаксис**  
`str ConvertFromBase64(str source)` — здесь предполагается кодирование Unicode.  
...
**Описание.**  
Функция ConvertToBase64 преобразует строку в строку в кодировке base64.  
...
**Описание.**  
Функция ConvertToUTF8Hex преобразует значение в шестнадцатеричную кодировку UTF8.
...
**Описание.**  
Функция DNComponent возвращает значение указанного DN компонентa, начиная с левого края.

**Синтаксис**  
`str DNComponent(ref dn, num ComponentNumber)`

* dn: ссылочный атрибут для интерпретации.
* ComponentNumber: компонент в DN для возврата.

**Пример.**  
`DNComponent(CRef([dn]),1)`  
Если значение DN равно cn=Joe,ou=…, функция возвращает Joe.
...
**Примечания:**  
Для атрибута это значение True, если атрибут отсутствует или присутствует, но является пустой строкой.  
...
**Примечание:**  
Функция Item полезна вместе с функцией Contains, так как последняя функция возвращает индекс элемента в атрибуте с несколькими значениями.
...
**Параметры:**<br> 
   ...
   | ИМЯ | Обязательно/повторяется | Тип | Примечания |
   | --- | --- | --- | --- |
   | **separator** |Обязательно |Строка |Строка, используемая для разделения исходных значений, когда они объединяются в одну строку. Если разделитель не требуется, может не иметь значения (""). |
...
**Параметры:**<br> 
   ...
   | **length** |Обязательно |целое число |Длина подстроки. Если длина превышает размер **исходной** строки, функция возвращает подстроку из **начала** индекса и до конца **исходной** строки. |
...
**Параметры:**<br> 
   ...
   | ИМЯ | Обязательно/повторяется | Тип | Примечания |
   | --- | --- | --- | --- |
   | **source** |Обязательно |Строка |**source** , которое необходимо разделить |
...
**Параметры:**<br> 
   ...
   | ИМЯ | Обязательно/повторяется | Тип | Примечания |
   | --- | --- | --- | --- |
   | **source** |Обязательно |Строка |**source**, которое необходимо обновить. |
...
**Параметры:**<br> 
   ...
   | --- | --- | --- | --- |
   | **source** |Обязательно |Строка |**Source** , которое необходимо обновить. |
   | **defaultValue** |Необязательно |Строка |Значение по умолчанию, которое необходимо использовать, если  не соответствует ни одному ключу. Может быть пустой строкой (""). |
