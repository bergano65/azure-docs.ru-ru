---
title: Tutorial - Интеграция одного леса с одним арендатором Azure AD
...
---
# <a name="tutorial-integrate-a-single-forest-with-a-single-azure-ad-tenant"></a>Учебник: Интеграция одного леса с одним арендатором Azure AD

Этот учебник позволяет вам создать гибридную среду идентификации с помощью облачного каталога Azure Active Directory (Azure AD) Connect.

![Создание](media/tutorial-single-forest/diagram1.png)

Вы можете использовать среду, создаваемую в этом учебнике, для тестирования или для более подробного ознакомления с подготовкой облака.
...
2. Если между серверами и Azure AD настроен брандмауэр, необходимо настроить указанные ниже элементы.
   ...
   - Если брандмауэр или прокси позволяет указать безопасные суффиксы, добавьте соединения t to ** \*.msappproxy.net** и ** \*.servicebus.windows.net**. Если нет, разрешите доступ к [диапазонам IP-адресов центра обработки данных Azure](https://www.microsoft.com/download/details.aspx?id=41653). Список диапазонов IP-адресов обновляется еженедельно.
...
3. Слева выберите **Active Directory Azure,** щелкните **Azure AD Connect,** а в центре выберите **управление подготовкой (предварительным просмотром)**.

   ![Портал Azure](media/how-to-install/install6.png)

4. Нажмите **Скачать агента**.
5. Выполнить агент по подготовке Azure AD Connect.
...
10. Как только эта операция завершится, вы должны увидеть уведомление: **Конфигурация агента была успешно проверена.**  Здесь можно щелкнуть **Выйти**.</br>
...
2.  Нажмите **активный каталог Azure**
3.  Щелкните **Azure AD Connect**.
4.  Выберите **Подготовка управления (Предварительный просмотр)**
![](media/how-to-configure/manage1.png)
5.  Нажмите **новую конфигурацию**
...
1. Просмотр[https://myapps.microsoft.com](https://myapps.microsoft.com)
