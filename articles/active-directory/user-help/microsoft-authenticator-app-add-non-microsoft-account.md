---
title: Добавление личных учетных записей, кроме предоставленных корпорацией Майкрософт, в приложение Microsoft Authenticator (Azure Active Directory) | Документация Майкрософт
description: Сведения о добавлении личных учетных записей сторонних поставщиков, таких как Google, Facebook или GitHub, в приложение Microsoft Authenticator для двухфакторной проверки подлинности.
...
ms.locfileid: "55459151"
---
# <a name="add-your-personal-non-microsoft-accounts"></a>Добавление личных учетных записей сторонних поставщиков
Вы можете добавить личные учетные записи сторонних поставщиков, таких как Google, Facebook или GitHub, в приложение Microsoft Authenticator для двухфакторной проверки подлинности.
...
1. Войдите в учетную запись и включите для нее двухфакторную проверку подлинности, используя компьютер или мобильное устройство.

2. Добавьте учетную запись в приложение Microsoft Authenticator.
...
1. На компьютере перейдите по ссылке https://myaccount.google.com/signinoptions/two-step-verification/enroll-welcome, выберите **Приступить к работе** и пройдите аутентификацию.
...
1. Откройте на компьютере страницу Google, перейдите в раздел **Set up alternative second step** (Настроить альтернативный второй шаг) и выберите **Set up** (Настройка) из раздела **Authenticator app** (Приложение Authenticator).
...
4. С помощью камеры своего устройства отсканируйте QR-код со страницы **Set up Authenticator** (Настройка приложения Authenticator), которая открыта на вашем компьютере.
...
6. Выберите **Next** (Далее) на странице **Set up Authenticator** (Настройка приложения Authenticator), которая открыта на вашем компьютере, введите полученный в приложении шестизначный код проверки для своей учетной записи Google, а затем щелкните **Verify** (Проверить).
...
1. На компьютере откройте Facebook, выберите раскрывающееся меню в правом верхнем углу и перейдите к разделу **Settings** > **Security and Login** (Настройки > Безопасность и вход).
...
1. На компьютере откройте страницу Facebook, перейдите к разделу **Add a backup** (Добавление резервного метода) и выберите **Setup** (Установка) из области **Authentication app** (Приложение Authentication).
...
3. С помощью камеры своего устройства отсканируйте QR-код со страницы **Two-Factor Authentication** (Двухфакторная проверка подлинности), которая открыта на вашем компьютере.
...
5. Выберите **Next** (Далее) на странице **Two-Factor Authentication** (Двухфакторная проверка подлинности), которая открыта на вашем компьютере, введите полученный в приложении шестизначный код проверки для своей учетной записи Facebook.
...
1. На компьютере откройте GitHub, щелкните изображение в правом верхнем углу и выберите **Settings** (Параметры).
...
4. На странице **Two-Factor Authentication** (Двухфакторная проверка подлинности), которая открыта на вашем компьютере, введите полученный в приложении шестизначный код проверки для своей учетной записи GitHub, а затем щелкните **Enable** (Включить).
...
1. Откройте на компьютере сайт Amazon, щелкните раскрывающееся меню **Account & Lists** (Учетная запись и списки), а затем выберите **Your Account** (Ваша учетная запись).
...
6. С помощью камеры своего устройства отсканируйте QR-код со страницы **Choose how you'll receive codes** (Выберите, как вы будете получать коды), которая открыта на вашем компьютере.
...
6. На странице **Choose how you'll receive codes** (Выберите, как вы будете получать коды), которая открыта на вашем компьютере, введите полученный в приложении шестизначный код проверки для своей учетной записи Amazon, а затем щелкните **Verify code and continue** (Проверить код и продолжить).
...
8. На странице **Add a backup verification method** (Добавление резервного метода проверки подлинности), которая открыта на вашем компьютере, введите полученный в приложении шестизначный код проверки для резервного метода проверки подлинности учетной записи Amazon, а затем щелкните **Verify code and continue** (Проверить код и продолжить).

9. На странице Almost done (Почти готово) выберите, нужно ли назначить компьютер доверенным устройством, а затем щелкните **Got it. Turn on Two-Step Verification** (Понятно. Включите двухфакторную проверку подлинности).
