---
title: Настройка сведений о безопасности (Предварительная версия) для использования текстовых сообщений — Azure Active Directory | Документация Майкрософт
...
5. Введите код из текстового сообщения, отправленного на ваше мобильное устройство, а затем щелкните **Далее**.
    ...
    ![Уведомление об успешном выполнении, подключение номера телефона, возможность получения текстовых сообщений и учетной записи](media/security-info/securityinfo-myprofile-phonetext-success.png)
...
Если требуется, чтобы текстовые сообщения были методом по умолчанию, используемым при входе в рабочую или учебную учетную запись с помощью двухфакторной проверки подлинности или для запросов на сброс пароля, его можно задать на странице **сведений о безопасности** .
...
2. Выберите вариант **Phone - text (Телефон — текстовое сообщение) ( *_номер_телефона_* )** из раскрывающегося списка доступных методов, а затем щелкните **Подтвердить**.
    ...
    ![Выбор метода входа по умолчанию](media/security-info/securityinfo-myprofile-phonetext-changeddefault.png)
