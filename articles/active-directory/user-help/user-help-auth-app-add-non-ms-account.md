---
title: Добавление учетных записей, не относясь от Microsoft, в приложение Microsoft Authenticator - Azure AD
description: Добавьте учетные записи, не относявшиеся к Microsoft, например, для Google или Facebook в приложение Microsoft Authenticator для проверки вашей личности при использовании двухфакторной проверки.
...
ms.locfileid: "77063957"
---
# <a name="add-non-microsoft-accounts-to-the-microsoft-authenticator-app"></a>Добавление учетных записей, не относясь от Microsoft, в приложение Microsoft Authenticator
...
1. На компьютере откройте Facebook, выберите выпадающее меню в правом верхнем углу, а затем перейдите в **Настройки Безопасности** > **и Входа.**
...
2. Прокрутите вниз до варианта **Use two-factor authentication** (Использовать двухфакторную проверку подлинности) в разделе **Two-Factor Authentication** (Двухфакторная проверка подлинности), а затем щелкните **Edit** (Изменить).
    ...
    Появляется **двухфакторная страница аутентификации.**
...
8. На странице **Choose how you'll receive codes** (Выберите, как вы будете получать коды) на компьютере введите полученный в приложении шестизначный код проверки для своей учетной записи Amazon, а затем щелкните **Verify code and continue** (Проверить код и продолжить).
...
11. На **странице Почти сделано,** решить, следует ли сделать ваш компьютер доверенным устройством, а затем выбрать **Got его. Включите двухступенчатую проверку.**
