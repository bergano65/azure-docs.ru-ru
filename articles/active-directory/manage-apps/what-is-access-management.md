---
title: Управление доступом к приложениям с помощью Azure AD.
...
Если назначение пользователя *не требуется*, неназначенные пользователи не видят приложение в своих приложениях, но они могут входить в само приложение (также называется входом, инициированным поставщиком услуг) или могут использовать **URL-адрес доступа пользователя** на странице **свойств** приложения (также известной как IDP вход).
...
Вы можете определить, могут ли пользователи, назначенные корпоративному приложению, просматривать их в разделе "Мои приложения" и в средстве запуска приложений Office 365.
...
В этом случае все назначенные пользователи будут автоматически подготовлены для Salesforce; поскольку они добавляются в разные группы, их назначения ролей в Salesforce будут обновлены. Пользователи смогут обнаруживать и получать доступ к Salesforce через мои приложения, веб-клиенты Office или даже путем перехода на страницу входа в систему Salesforce. Администраторы смогут легко просмотреть состояние использования и назначения с помощью отчетов Azure AD.
...
Пользователи могут получать доступ к приложениям Office 365 через порталы Office 365. Вы также можете показать или скрыть приложения Office 365 в папке "Мои приложения" с помощью [переключателя видимости office 365](hide-application-from-user-portal.md) в **параметрах пользователя**каталога. 
