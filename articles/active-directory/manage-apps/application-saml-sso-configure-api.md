...
> [!NOTE] 
> Вы можете использовать API Аппликатионтемплате для создания экземпляров [приложений, не относящихся к коллекции](add-non-gallery-app.md). Используйте Аппликатионтемплатеид `8adf8e6e-67b2-4cf2-a259-e3dc5476c621` .

> [!NOTE]
> Подождите некоторое время, пока приложение будет подготовлено в клиенте Azure AD. Это не мгновенно. Одной из стратегий является выполнение запроса GET для объекта приложения или субъекта-службы каждые 5-10 секунд до тех пор, пока запрос не будет успешным.
...
Дополнительные сведения см. [в разделе Настройка утверждений, выпущенных в токене](https://docs.microsoft.com/azure/active-directory/develop/active-directory-claims-mapping).

> [!NOTE]
> Некоторые ключи в политике сопоставления утверждений чувствительны к регистру (например, "версия"). Если появляется сообщение об ошибке "свойство имеет недопустимое значение", это может быть проблемой с учетом регистра.
...
Для целей тестирования можно создать самозаверяющий сертификат с помощью приведенных ниже команд PowerShell. Затем необходимо будет обрабатывать и извлекать значения, необходимые вручную с помощью других средств. При создании сертификата для подписи для рабочей среды примените все самые передовые практики безопасности, используемые в вашей компании.
...
Убедитесь, что значение keyId для keyCredential, для которого указано назначение Sign (подпись), соответствует значению keyId для passwordCredential. Вы можете создать `customkeyIdentifier`, получив хэш отпечатка сертификата. См. справочный код C# выше.

#### <a name="request"></a>Запрос

> [!NOTE]
> Значение key в свойстве keyCredentials сокращено для удобства чтения. Значение — Base 64 в кодировке. Для закрытого ключа свойство `usage` имеет значение "Sign". Для открытого ключа свойство `usage` — «Verify».
