---
title: Соображения топологии сети для прокси-приложений Azure AD
...
При регистрации на арендатора Azure AD регион арендатора определяется страной/регионом, указанной вами. Когда вы включаете прокси приложения, экземпляры службы прокси приложения для вашего клиента выбираются или создаются в том же регионе, где находится клиент Azure AD, или в ближайшем к нему регионе.

Например, если страна или регион арендатора Azure AD — это Соединенное Королевство, все разъемы приложения Proxy используют экземпляры службы в европейских центрах обработки данных. Когда ваши пользователи обращаются к опубликованным приложениям, этот трафик проходит через экземпляры службы прокси приложения в этом расположении.
...
Мало что можно сделать для управления подключением между пользователями и службой прокси приложения. Пользователи могут получить доступ к вашим приложениям из домашней сети, кафе или другой страны/региона. Вместо этого можно оптимизировать подключения между службой прокси приложения, соединителями прокси приложения и приложениями. Рекомендуется реализовать в вашей среде указанные ниже шаблоны.
...
### <a name="pattern-2-take-advantage-of-expressroute-with-microsoft-peering"></a>Шаблон 2: Воспользуйтесь преимуществами ExpressRoute с помощью пиринга Майкрософт
...
![Диаграмма, поключававую пользователей, прокси, разъем и приложение находятся в США](./media/application-proxy-network-topology/application-proxy-pattern1.png)
...
![Пользователи распространяются по всему миру, но все остальное находится в США](./media/application-proxy-network-topology/application-proxy-pattern2.png)
...
![Соединение в центре обработки данных Azure, ExpressRoute между разъемом и приложением](./media/application-proxy-network-topology/application-proxy-pattern4.png)

### <a name="use-case-5"></a>Вариант использования 5

**Сценарий:** Приложение находится в сети организации в Европе, с экземпляром Application Proxy и большинством пользователей в США.
...
![Диаграмма показывает пользователей и прокси в США, разъем и приложение в Европе](./media/application-proxy-network-topology/application-proxy-pattern5b.png)

Также в этой ситуации можно применить еще один вариант. Если большинство пользователей организации находятся в США, возможно ваша сеть уже "расширена" в этот регион. Поместите разъем в США и используйте специальную внутреннюю корпоративную сетевую линию для приложения в Европе. Этот позволит оптимизировать прыжки 2 и 3.

![Диаграмма показывает пользователям, прокси, и разъем в США, приложение в Европе](./media/application-proxy-network-topology/application-proxy-pattern5c.png)
