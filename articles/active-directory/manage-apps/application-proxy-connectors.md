...
Чтобы успешно развернуть прокси приложения, необходим по крайней мере один соединитель, но мы рекомендуем использовать не меньше двух, чтобы повысить гибкость. Установите разъем на машину под управлением Windows Server 2012 R2 или позже. Соединителю нужен доступ к службе Application Proxy и к локальным приложениям, которые вы публикуете.
...
Прежде чем установить этот соединитель, на сервере Windows Server нужно включить протокол TLS 1.2. Для включения TLS 1.2 на сервере:
...
Обязательно проверьте, что между соединителями существует достаточная производственная мощность для обработки ожидаемого объема трафика. Мы рекомендуем, чтобы каждая группа разъема имеет по крайней мере два разъема, чтобы обеспечить высокую доступность и масштаб. Наличие трех разъемов является оптимальным в случае, если вам может понадобиться обслуживание машины в любой момент.

В общем, чем больше пользователей, тем больше компьютер необходим. Ниже приведена таблица, дающая наброски объема и ожидаемой задержки различных машин может обрабатывать. Обратите внимание, что все данные основаны на ожидаемом числе транзакций в секунду (TPS), а не на числе пользователей. Причина в том, что шаблоны использования различаются, и на их основании нельзя спрогнозировать нагрузку. В зависимости от размера ответов и времени отклика серверного приложения также будут наблюдаться некоторые различия. Больший размер ответов и большее время отклика приведут к снижению максимального числа транзакций в секунду (макс. TPS). Мы также рекомендуем иметь дополнительные машины, чтобы распределенная нагрузка между машинами всегда обеспечивает достаточно буфера. Дополнительные емкости обеспечат вам высокую доступность и отказоустойчивость.
...
\*Эта машина использовала пользовательские настройки, чтобы поднять некоторые из ограничений соединения по умолчанию за пределы рекомендуемых параметров .NET. Рекомендуем запустить тест с параметрами по умолчанию, прежде чем обратиться в службу поддержки, чтобы изменить это ограничение для своего клиента.
...
>
> В этой таблице также основное внимание уделяется ожидаемой производительности разъема в зависимости от типа установленной на нем машины. Это отдельно от границ службы прокси-приложений, см. [Ограничения и ограничения службы.](https://docs.microsoft.com/azure/active-directory/users-groups-roles/directory-service-limits-restrictions)

## <a name="security-and-networking"></a>Безопасность и сеть

Соединители можно устанавливать в любом месте в сети. Поэтому они могут отправлять запросы к службе прокси приложения. Вам достаточно лишь обеспечить доступ к вашим приложениям с компьютера, на котором выполняется соединитель. Можно установить соединители в корпоративной сети или на виртуальной машине, работающей в облаке. Соединительные соединения могут работать в сети периметра, также известной как демилитаризованная зона (ДМЗ), но это не обязательно, потому что весь трафик исходящий, так что ваша сеть остается безопасной.
...
Производительность соединителя связана с ЦП и сетью. Производительность процессора необходима для шифрования и расшифровки TLS, в то время как сеть важна для быстрого подключения к приложениям и онлайн-сервису в Azure.
...
- **Контроллеры домена:** Если разъемы выполняют одиночный входе (SSO) с помощью Kerberos Constrained Delegation, они связываются с контроллерами домена перед отправкой запроса в бэкэнд. Соединители содержат кэш билетов Kerberos, но в среде с высокой нагрузкой скорость реагирования контроллеров домена может снизить производительность. Такая проблема наиболее характерна для ситуации, когда соединители выполняются в Azure, но взаимодействуют с контроллерами домена в локальной среде.
...
После первого успешного обновления сертификата служба Proxy Connector Application (Network Service) не имеет разрешения на удаление старого сертификата из локального магазина машин. Если срок действия сертификата истек или он больше не будет использоваться службой, его можно безопасно удалить.

Чтобы избежать проблем с обновлением сертификата, убедитесь, что сетевая связь от разъема к [задокументированным направлениям](https://docs.microsoft.com/azure/active-directory/manage-apps/application-proxy-add-on-premises-application#prepare-your-on-premises-environment) включена.
...
```
Чтобы узнать больше о том, как проверить сертификат и проблемы устранения неполадок, смотрите [поддержку компонентов Verify Machine и бэкэнда для сертификата доверия Application Proxy.](application-proxy-connector-installation-problem.md#verify-machine-and-backend-components-support-for-application-proxy-trust-certificate)
...
Состояние службы можно проверить в окне "Служба". Соединитель состоит из двух служб Windows: собственно соединитель и служба обновления. Они должны работать постоянно.
 ...
 ![Пример: окно служб, отображающее локальные службы Azure AD](./media/application-proxy-connectors/aad-connector-services.png)
