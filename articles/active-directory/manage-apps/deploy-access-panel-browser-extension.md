...
---
# <a name="how-to-deploy-the-access-panel-extension-for-internet-explorer-using-group-policy"></a>Как: Развертывание расширения панели доступа для Internet Explorer с использованием групповой политики
...
* У вас есть разрешение на изменение параметров для редактирования объекта групповой политики. По умолчанию такое разрешение имеют члены следующих групп безопасности: «Администраторы домена», «Администраторы предприятия» и «Владельцы-создатели групповой политики». [См. дополнительные сведения.](https://technet.microsoft.com/library/cc781991%28v=ws.10%29.aspx)

## <a name="step-1-create-the-distribution-point"></a>Шаг 1: Создание точки распределения
...
1. Подпишитесь на сервер в качестве администратора.
...
1. Перейдите на вкладку **Акции.** Затем нажмите **Задачи** > **Новая доля ...**

    ![Скриншот показывает, где найти новую долю с экрана задач](./media/deploy-access-panel-browser-extension/shares.png)
...
1. Скопируйте пакет установщика в выбранное расположение в общем ресурсе.
    ...
    ![Копирование файла .msi на акцию](./media/deploy-access-panel-browser-extension/copy-package.png)
...
1. Войти на сервер, на котором размещена установка Active Directory Domain Services (AD DS).
1. В серверном менеджере перейдите на**управление политикой группы инструментов.** **Tools** > 
...
1. В левой части окна **Управление групповой политикой** изучите иерархию организационного подразделения и определите, для какой области действия следует применить групповую политику. Например, вы можете выбрать небольшое организационное подразделение для тестового развертывания на нескольких компьютерах или организационное подразделение верхнего уровня для развертывания во всей организации.
   ...
   > [!NOTE]
   > Если вы хотите создать или отсвазать свои подразделения Организации (OUs), переключитесь на серверный менеджер и перейдите на **инструменты** > **активных пользователей каталогов и компьютеров.**
...
1. Выбрав организационное подразделение, щелкните его правой кнопкой мыши и выберите пункт **Создать объект групповой политики в этом домене и связать его...**
    ...
    ![Скриншот показывает Создать новую опцию GPO](./media/deploy-access-panel-browser-extension/create-gpo.png)
...
## <a name="step-3-assign-the-installation-package"></a>Шаг 3: Назначить пакет установки
...
1. Щелкните правой кнопкой мыши **Список надстроек** и выберите пункт **Изменить**.
    ...
    ![Нажмите право "Список добавления" и выберите "Edit"](./media/deploy-access-panel-browser-extension/edit-add-on-list.png)
...
1. В окне **Вывод содержания** выполните следующие действия.
   ...
   1. Нажмите кнопку **ОК**, чтобы закрыть окно **Вывод содержания**.
      ...
      ![Заполните значения, указанные на предыдущем этапе](./media/deploy-access-panel-browser-extension/show-contents.png)
...
## <a name="step-5-optional-disable-remember-password-prompt"></a>Шаг 5 (необязательно): Отключить "Помните пароль" запрос
...
![Показывает "Хотите сохранить свой пароль..." Строке](./media/deploy-access-panel-browser-extension/remember-password-prompt.png)
...
1. Найдите параметр **Включить автозаполнение для имен пользователей и паролей в формах**.
   ...
   > В предыдущих версиях Active Directory этот параметр может иметь имя **Запретить функции автоматического заполнения сохранять пароли**. Настройка для этого параметра отличается от параметра, описанного в этом учебнике.
    ...
    ![Не забудьте найти это в настройках пользователя](./media/deploy-access-panel-browser-extension/disable-auto-complete.png)
...
1. В окне **Включить автозаполнение для имен пользователей и паролей в формах** выберите **Отключено**.
    ...
    ![Выберите опцию "Инвалид" для функции "Включить функцию автоматического завершения"](./media/deploy-access-panel-browser-extension/disable-passwords.png)
...
1. Для полного обновления с помощью этой машины может потребоваться несколько встыков для изменения групповой политики. Для принудительного обновления откройте окно **командной строки** и выполните следующую команду: `gpupdate /force`.
...
1. В окне **Управление надстройками** убедитесь, что **расширение панели доступа** установлено и для параметра **Состояние** задано значение **Включено**.
   ...
   ![Удостоверьте, что расширение панели доступа установлено и включено](./media/deploy-access-panel-browser-extension/verify-install.png)
