---
title: Проблемы, вопиявввв в негалерей федеративный однозначный приложение
...
*Ошибка AADSTS70001: Приложение `https://contoso.com` с идентификатором не было найдено в каталоге*.
...
8. После загрузки приложения щелкните **Базовая конфигурация SAML**. Убедитесь, что значение в текстовом ящике идентификатора соответствует значению значения идентификатора, отображаемого в ошибке.
...
*Ошибка AADSTS50011: адрес `https://contoso.com` ответа не соответствует адресам ответа, настроенным для приложения* 
...
8. После загрузки приложения щелкните **Базовая конфигурация SAML**. Проверьте или обновите значение в `AssertionConsumerServiceURL` текстовом ящике URL-адреса Reply в соответствии с значением в запросе SAML.    
...
*Ошибка AADSTS50105: Подпись `brian\@contoso.com` в пользователе не назначена роль для приложения*
...
9. Выберите срок действия. Затем щелкните **Сохранить.**

10. Проверьте **Сделать новый сертификат активным,** чтобы переопределить активный сертификат. Нажмите кнопку **Сохранить** в верхней части области и активируйте сертификат возобновления.

11. В разделе **Сертификат подписи SAML** нажмите **кнопку «Удалить** **неиспользованный** сертификат».
