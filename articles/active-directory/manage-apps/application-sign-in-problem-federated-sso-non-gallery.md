...
Для устранения проблем со входом ниже, мы рекомендуем выполнить эти предложения, чтобы получить лучшую диагностику и автоматизировать действия по разрешению проблемы:

- Установка [приложений Secure браузер расширение пространства имен My](access-panel-extension-problem-installing.md) для Azure Active Directory (Azure AD) для предоставления лучше диагностики и способы их устранения при использовании тестирования взаимодействия на портале Azure.
- Воспроизведите ошибку, с помощью интерфейса пользователя для тестирования на странице настройки приложения на портале Azure. Дополнительные сведения о [отладка SAML единого входа приложений на основе](../develop/howto-v1-debug-saml-sso-issues.md)
...
Убедитесь, что `Issuer` атрибут в запросе SAML совпадает со значением идентификатора, настроенный в Azure AD. Если вы используете [тестирование работы](../develop/howto-v1-debug-saml-sso-issues.md) на портале Azure с помощью приложения Secure браузер расширения пространства имен My, не нужно вручную выполните следующие действия.
...
8. После загрузки приложения откройте **конфигурации основные SAML**. Убедитесь, что значение в текстовом поле идентификатора соответствует значению для значения идентификатора, указанного в ошибке.
...
8. После загрузки приложения откройте **конфигурации основные SAML**. Проверьте или обновите значение в текстовое поле URL-адрес ответа в соответствии с `AssertionConsumerServiceURL` значение в запросе SAML.    
    
После обновления URL-адрес ответа, в Azure AD, и он совпадает со значением, отправленный приложением в запросе SAML, можно выполнить вход в приложение.

## <a name="user-not-assigned-a-role"></a>Не назначена роль пользователя

*Ошибка AADSTS50105: Пользователю, выполнившему вход `brian\@contoso.com` не назначена роль для приложения*
...
Чтобы напрямую назначить одного или нескольких пользователей к приложению, выполните следующие действия. Если вы используете [тестирование работы](../develop/howto-v1-debug-saml-sso-issues.md) на портале Azure с помощью приложения Secure браузер расширения пространства имен My, не нужно вручную выполните следующие действия.
...
Поставщику приложения следует проверить, что они поддерживают реализацию Azure AD SAML для единого входа.

## <a name="misconfigured-application"></a>Доставить неверно настроенное приложение

*Ошибка AADSTS650056: Доставить неверно настроенное приложение. Это может быть вызвано одной из следующих причин: Клиент не имеет указываются все разрешения для «AAD Graph» в запрошенные разрешения в регистрации клиентского приложения. Или администратор не предоставил в клиенте. Или проверьте идентификатор приложения в запросе, чтобы убедиться, что он соответствует идентификатору приложения настроенный клиент. Обратитесь к администратору, чтобы исправить конфигурацию или предоставлять согласие от лица клиента.* .
...
1.  Тип **«Azure Active Directory»** в поле фильтра поиска и выберите **Azure Active Directory** элемента.

1.  Выберите **корпоративные приложения** Azure Active Directory меню навигации слева.
...
1.  После загрузки приложения откройте **конфигурации основные SAML**. Убедитесь, что значение в текстовом поле идентификатора соответствует значению для значения идентификатора, указанного в ошибке.
...
Azure AD не удалось определить запрос SAML в параметрах URL-адреса в HTTP-запросе. Это может произойти, если приложение не использует HTTP-перенаправление привязки при отправке запроса SAML в Azure AD.

**Способы устранения:**

Приложение должно отправить запрос SAML, закодированного в заголовке location, с помощью протокола HTTP перенаправления привязки. Дополнительные сведения о реализации см. в разделе о HTTP перенаправлении привязки в документе спецификации протокола SAML [здесь](https://docs.oasis-open.org/security/saml/v2.0/saml-bindings-2.0-os.pdf).

## <a name="azure-ad-is-sending-the-token-to-an-incorrect-endpoint"></a>Azure AD отправляет токен неверная конечная точка

**Возможная причина**

Во время единого входа если запрос на вход в не содержит явного URL-адрес ответа (утверждение потребителя URL-адрес службы), то Azure AD будет выбрать любой из настроенных используют URL-адреса для этого приложения. Даже если приложение имеет явные URL-адрес ответа настроен, пользователь может быть перенаправление https://127.0.0.1:444. 
...
Удалите неиспользуемые ответа URL-адреса, настроенного для приложения.

1.  Откройте [**портал Azure**](https://portal.azure.com/) и войдите в систему как **глобальный администратор** или **соадминистратор**.

2.  Откройте **расширение Azure Active Directory** , выбрав **все службы** в верхней части главного меню навигации слева.

3.  Тип **«Azure Active Directory»** в поле фильтра поиска и выберите **Azure Active Directory** элемента.

4.  Выберите **корпоративные приложения** Azure Active Directory меню навигации слева.
...
7.  После загрузки приложения откройте **конфигурации основные SAML**. В **URL-адрес ответа (утверждение потребителя URL-адрес службы)**, удалить неиспользуемые или URL-адреса ответа по умолчанию, созданный системой. Например, `https://127.0.0.1:444/applications/default.aspx`.
