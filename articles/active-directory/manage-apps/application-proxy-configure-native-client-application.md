...
![Связь между конечными пользователями, Azure AD и публикации приложений](./media/application-proxy-configure-native-client-application/richclientflow.png)
...
1. На боковой панели, выберите **Azure Active Directory**. **Azure Active Directory** отображается на странице "Обзор".
1. На боковой панели Обзор Azure AD, выберите **регистрация приложений**. Отобразится список всех зарегистрированных приложений.
1. Выберите **Новая регистрация**. **Зарегистрировать приложение** появится страница.

   ![Создание регистрации приложения на портале Azure](./media/application-proxy-configure-native-client-application/create.png)

1. В **имя** заголовок, укажите имя отображения пользовательского интерфейса для приложения.
1. В разделе **поддерживаемые типы учетных записей** выберите уровень доступа, с помощью следующих правил:
...
1. В **URI перенаправления** щелкните **общедоступным клиентом (мобильных и настольных систем)** , а затем введите URI перенаправления для приложения.
1. Выберите и прочитайте **политики платформы Microsoft**, а затем выберите **зарегистрировать**. На странице "Обзор" для регистрация нового приложения создаются и отображаются.
...
1. Выберите **Добавить разрешение**. **Разрешения запроса API** появится страница.
1. В разделе **Выбор API** выберите **API, организации**. Откроется список приложений в каталоге, которые предоставляют API-интерфейсы.
1. Введите в поле поиска или прокрутки, чтобы найти прокси приложения, которое вы опубликовали в [шаг 1: Опубликуйте приложение прокси](#step-1-publish-your-proxy-application), а затем выберите приложение прокси-сервера.
1. В **какой тип разрешения требует приложения?** щелкните тип разрешения. Если собственное приложение требуется доступ к прокси-сервера приложения API от имени выполнившего вход пользователя, выберите **делегированные разрешения**. Если собственное приложение выполняется как фоновая служба или управляющая программа без вошедшего пользователя, выберите **разрешения приложения**.
1. В **Выбор разрешений** , выберите нужные разрешения и выберите **добавить разрешения**. **Разрешения API** страницы, теперь в собственном приложении показано прокси приложения и разрешение API, который вы добавили.
