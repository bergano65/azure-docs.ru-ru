...
- Число **пользователей** и **групп** в хранилище данных соединителя. Число увеличивается каждый раз, когда объект добавляется в область подготовки. Счетчик не будет передвигаться, если пользователь обратимо удален или жестко удален, так как он не удаляет объект из хранилища данных соединителя. После [сброса](https://docs.microsoft.com/graph/api/synchronization-synchronizationjob-restart?view=graph-rest-beta&tabs=http) компакт-дисков счетчик будет повторно вычисленные с первой синхронизацией 
