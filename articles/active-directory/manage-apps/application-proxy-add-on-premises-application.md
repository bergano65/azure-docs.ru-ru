...
> [!IMPORTANT]
> Чтобы обеспечить лучшее в своем классе шифрование для наших клиентов, мы обновлением прокси-службу приложения для предоставления доступа только по протоколам TLS 1.2. Исходя из готовности клиентов, изменения будут постепенно развертываться для тех, кто использует только протоколы TLS 1.2, чтобы избежать негативных последствий из-за этого изменения. Поддержка протоколов TLS 1.0 и 1.1 прекратиться 31 августа 2019 г., и клиенты заранее получат уведомления, чтобы подготовиться к этому изменению. Для подготовки к этому изменению убедитесь, что все сочетания "клиент-сервер" и "браузер-сервер" обновлены для использования протокола TLS 1.2, чтобы обеспечить подключение к прокси-службе приложения. Это также касается клиентов, с помощью которых ваши пользователи получают доступ к приложениям, опубликованным через прокси приложения. Полезные ссылки и ресурсы см. в статье [Подготовка к использованию TLS 1.2 в Office 365](https://support.microsoft.com/help/4057306/preparing-for-tls-1-2-in-office-365).
...
1. Просмотрите соединитель, чтобы проверить сведения о нем. Соединители должны быть развернуты по умолчанию. Если соединитель, который вы хотите просмотреть, не развернут, разверните его, чтобы просмотреть сведения. Активная зеленая метка указывает на то, что соединитель может подключиться к службе. Тем не менее, несмотря на то, что метка зеленая, проблема с сетью по-прежнему может помешать соединителю получать сообщения.
    ...
    ![Соединители Azure AD Application Proxy](./media/application-proxy-connectors/app-proxy-connectors.png)
...
1. В области навигации слева выберите **Azure Active Directory**.
1. Перейдите в колонку **Корпоративные приложения** и выберите **Новое приложение**.
1. Выберите **Локальное приложение**.  
1. В разделе **Добавление локального приложения** укажите следующие сведения о приложении.
...
1. При необходимости настройте **дополнительные параметры**. Для большинства приложений следует сохранить значения этих параметров по умолчанию. 
...
1. Щелкните **Приступая к работе** и выберите **Assign a user for testing** (Назначение пользователя для тестирования).
1. В разделе **Пользователи и группы** выберите **Добавление пользователя**.
1. В разделе **Добавление назначения** выберите **Пользователи и группы**. Появится раздел **Пользователи и группы**.
1. Выберите учетную запись, которую вы хотите добавить.
1. Щелкните **Выбрать**, а затем выберите **Назначить**.

### <a name="test-the-sign-on"></a>Проверка единого входа

Для тестирования единого входа в приложение:

1. Откройте в браузере внешний URL-адрес, который вы указали на этапе публикации. Вы должны увидеть начальный экран.
1. Выполните вход от имени пользователя, созданного в предыдущем разделе.
