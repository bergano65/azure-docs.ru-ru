...
Так как добавленное приложение является локальным, то вам также потребуется сервер под управлением Windows Server 2012 R2 или более поздней версии, на котором можно установить соединитель прокси приложения. Серверу соединителя нужна возможность подключаться к службам прокси приложения в Azure, а также к локальным приложениям, которые вы планируете публиковать.
...
Если приложение использует для подключения WebSocket, убедитесь, что вы установили последнюю версию соединителя.  WebSocket поддерживается соединителями версии 1.5.612.0 или более поздней.
...
5. При необходимости настройте дополнительные параметры. Для большинства приложений следует сохранить значения этих параметров по умолчанию. 
    ...
    | **Использовать безопасный файл cookie**| Установите значение **Да**, чтобы файлы cookie передавались только по защищенному каналу, как например зашифрованный HTTPS-запрос.
...
Чтобы проверить, что приложение было добавлено правильно, добавьте учетную запись пользователя в приложение и попробуйте войти. 
...
2. Вы должны увидеть экран приветствия. Войдите в приложение, используя настроенную для тестирования учетную запись.
...
В этом руководстве вы подготовили локальную среду для работы с прокси приложения, а затем установили и зарегистрировали соединитель прокси приложения. Затем добавили приложение в клиент Azure AD и убедились, что в приложение можно войти с учетной записью Azure AD.
...
Теперь все готово для настройки единого входа. Есть несколько способов единого входа, и выбор лучшего зависит от того, как ваше приложение выполняет проверку подлинности. Используйте следующую ссылку, чтобы найти соответствующее руководство по единому входу для вашего приложения.
