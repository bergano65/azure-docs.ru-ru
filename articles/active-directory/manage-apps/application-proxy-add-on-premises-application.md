...
4. В колонке **Добавление локального приложения** укажите следующие сведения о приложении.
    ...
    ![Настройка локального приложения](./media/application-proxy-add-on-premises-application/add-on-premises-app-with-application-proxy-updated.png)
...
5. При необходимости настройте **дополнительные параметры**. Для большинства приложений следует сохранить значения этих параметров по умолчанию. 
    ...
    | **Использовать файл cookie, предназначенный только для HTTP** | Задайте для этого параметра значение **Да**, чтобы файлы cookie прокси приложения содержали флаг HTTPOnly в заголовке ответа HTTP. Если используете службы удаленных рабочих столов, задайте значение **Нет**.|
    | **Использование безопасного файла cookie**| Установите значение **Да**, чтобы файлы cookie передавались по защищенному каналу, как например зашифрованный HTTPS-запрос.
    | **Использование постоянных файлов cookie**| Для этого параметра оставьте значение **Нет**. Этот параметр нужно использовать только для приложений, в которых файлы cookie не используются совместно между процессами. Дополнительные сведения о параметрах файлов cookie см. в статье [Параметры файлов cookie для доступа к локальным приложениям в Azure Active Directory](https://docs.microsoft.com/azure/active-directory/manage-apps/application-proxy-configure-cookie-settings).
    | **Translate URLs in Headers (Преобразовывать URL-адреса в заголовках)** | Оставьте значением этого параметра **Да**, если только приложению не требуется исходный заголовок узла в запросе на аутентификацию. |
