...
6. При необходимости настройте **дополнительные параметры**. Для большинства приложений следует сохранить значения этих параметров по умолчанию. 
    ...
    | **Сохранять файлы сookie**| Для этого параметра оставьте значение **Нет**. Этот параметр нужно использовать только для приложений, в которых файлы cookie не используются совместно между процессами. Дополнительные сведения о параметрах файлов cookie см. в статье [Cookie settings for accessing on-premises applications in Azure Active Directory](./application-proxy-configure-cookie-settings.md) (Параметры файлов cookie для доступа к локальным приложениям в Azure Active Directory).
