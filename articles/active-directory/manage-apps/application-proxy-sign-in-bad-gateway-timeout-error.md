...
Чтобы найти код, просмотрите текст в поле "Код состояния", находящемся в нижнем левом углу сообщения об ошибке. Кроме того, ознакомьтесь с дополнительными советами в самом конце страницы.
   ...
   ![Ошибка при истечении времени ожидания шлюза](./media/application-proxy-sign-in-bad-gateway-timeout-error/connection-problem.png)
...
В качестве первого шага перепроверьте и исправьте внутренний URL-адрес, открыв приложение через функцию **Корпоративные приложения**, а затем выбрав меню **Прокси приложения**. Убедитесь, что внутренний URL-адрес будет использоваться из локальной сети, доступ к приложению.
...
2. Взгляните на поле "Группа соединителей". Если в группе нет активных соединителей, отображается предупреждение. Если вы не видите все предупреждения, перейти к проверить, разрешены все необходимые порты.
...
4. Здесь работу можно продолжить несколькими способами:
   ...
   * Проверьте неактивный соединитель. Если соединитель отображается как неактивный, значит ему не удалось подключиться к службе. Обычно эта ошибка возникает, потому что заблокированы некоторые необходимые порты. Чтобы решить эту проблему, перейти к проверить, разрешены все необходимые порты.
...
## <a name="check-all-required-ports-are-open"></a>Проверьте все необходимые порты открыты.
