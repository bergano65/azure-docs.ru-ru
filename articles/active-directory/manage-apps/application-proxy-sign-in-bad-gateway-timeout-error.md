---
title: Не удается получить доступ к этой ошибке корпоративного приложения с помощью приложения прокси приложения
...
![Пример: ошибка времени ожидания шлюза](./media/application-proxy-sign-in-bad-gateway-timeout-error/connection-problem.png)
...
1. Здесь работу можно продолжить несколькими способами:
   ...
   - Проверка неактивного соединителя: если соединитель отображается как неактивный, значит ему не удалось подключиться к службе. Обычно эта ошибка возникает, потому что заблокированы некоторые необходимые порты. Чтобы устранить эту проблему, проведите проверку, чтобы убедиться, что все необходимые порты разрешены.
