...
- Локальное приложение, [опубликованное через прокси приложение](application-proxy-add-on-premises-application.md).
...
9. Убедитесь, что запись DNS настроена правильно с помощью команды [nslookup](https://social.technet.microsoft.com/wiki/contents/articles/29184.nslookup-for-beginners.aspx) , чтобы узнать, доступен ли внешний URL-адрес, а домен msapproxy.NET отображается как псевдоним.
...
Сертификаты, выданные собственной инфраструктурой открытых ключей (PKI), можно использовать, если цепочка сертификатов установлена на клиентских устройствах. Intune можно использовать для развертывания этих сертификатов на управляемых устройствах. Для неуправляемых устройств эти сертификаты должны быть установлены вручную.
...
* [Включите условный доступ](https://docs.microsoft.com/en-us/azure/active-directory/conditional-access/technical-reference#cloud-apps-assignments) к опубликованным приложениям.
