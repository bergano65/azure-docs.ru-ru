...
Эта диаграмма объясняет поток, когда пользователь пытается получить доступ к приложению на помещение, которое использует IWA.
...
4. Прокси-сервер приложения проверяет токен и извлекает из него главное имя пользователя (UPN), а затем разъем вытягивает UPN, а главное имя службы (SPN) — через двойно проверенный защищенный канал.
5. Соединитель выполняет kerberos Constrained Delegation (KCD) переговоры с на территории AD, выдавая себя за пользователя, чтобы получить маркер Kerberos в приложение.
...
* Все ваши приложения имеют [основные имена службы.](https://social.technet.microsoft.com/wiki/contents/articles/717.service-principal-names-spns-setspn-syntax-setspn-exe.aspx)
...
1. В Active Directory перейдите к **пользователям инструментов** > **и компьютерам.**
2. Выберите сервер, на котором работает соединитель.
3. Нажмите правой кнопкой мыши и выберите **Свойства** > **делегации**.
4. Выберите **Доверять компьютеру делегирование указанных служб**. 
5. Выберите **Используйте любой протокол проверки подлинности.**
...
2. Используйте `principalsallowedtodelegateto` свойство учетной записи службы (компьютер или учетную запись пользователя из гостевого дома) веб-приложения, чтобы включить делегацию аутентификации Kerberos из приложения Proxy (разъем). Сервер приложения работает в контексте `webserviceaccount` и делегирования `connectorcomputeraccount`сервера . Выполнить команды ниже на контроллер домена (запуск Windows Server 2012 `webserviceaccount`R2 или позже) в домене . Используйте плоские имена (не UPN) для обеих учетных записей.

   Если `webserviceaccount` это компьютерная учетная запись, используйте следующие команды:
...
* Используйте различные псевдонимы в помещениях и в облаке. Например, joe-johns@contoso.com и joej@contoso.com.  
...
При возникновении ошибки в процессе SSO она появляется в журнале событий машины разъема, как это объясняется в [Устранении проблем.](application-proxy-back-end-kerberos-constrained-delegation-how-to.md)
