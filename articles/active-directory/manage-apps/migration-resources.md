...
|[Руководство по решению. Перенос приложений из служб федерации Active Directory (AD FS) в Azure AD](./migrate-adfs-apps-to-azure.md) | В этом руководстве по решению последовательно разбираются четыре этапа планирования и выполнения проекта по переносу приложений, которые в общей форме описываются в техническом документе по миграции. В нем вы узнаете, как использовать эти этапы для достижения конкретной цели — переноса приложения из служб федерации Active Directory (AD FS) в Azure AD.|
|[Руководство разработчика. AD FS для разработчиков сборник тренировочных заданий миграции приложений Azure AD](https://aka.ms/adfsplaybook) | Этот набор примеров кода ASP.NET и сопутствующих учебников поможет вам узнать, как безопасно и безопасно перенести приложения, интегрированные с службы федерации Active Directory (AD FS) (AD FS), в Azure Active Directory (Azure AD). Это руководство предназначено для разработчиков, которые не только настраивают приложения как в AD FS, так и в Azure AD, но также могут быть осведомлены и уверены в том, что их базовая база кода будет требоваться в этом процессе.|
