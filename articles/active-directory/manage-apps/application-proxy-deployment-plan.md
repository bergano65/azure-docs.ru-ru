...
AD Application Proxy Azure также может поддерживать приложения, разработанные для использования [библиотеки проверки подлинности Майкрософт (MSAL)](https://docs.microsoft.com/azure/active-directory/develop/v2-overview). Он поддерживает собственные клиентские приложения, используя созданные токены Azure AD, полученные в заголовке запроса клиента для выполнения предварительной проверки подлинности от имени пользователей.
