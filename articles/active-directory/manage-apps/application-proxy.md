...
Есть два компонента, необходимые для работы прокси приложения: соединитель и конечная точка. 
...
Конечной точкой может быть URL-адрес или [пользовательский портал](end-user-experiences.md). Пользователи за пределами вашей сети могут получить доступ к приложениям, используя внешний URL-адрес. Пользователи в вашей сети могут получить доступ к приложениям, используя URL-адрес или пользовательский портал. При переходе к одной из этих конечных точек пользователи проходят аутентификацию в Azure AD и через соединитель направляются к локальному приложению.
...
2. После успешного входа создается токен и передается на клиентское устройство пользователя.
...
1. [Включение прокси приложения и настройка соединителя](application-proxy-add-on-premises-application.md).    
2. [Публикация приложений](application-proxy-add-on-premises-application.md) — легко и быстро опубликуйте локальные приложения и предоставьте удаленный доступ к ним с помощью мастера.
