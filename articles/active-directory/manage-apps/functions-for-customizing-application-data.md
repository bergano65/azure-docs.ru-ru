---
title: Написание выражений для сопоставлений атрибутов в Azure AD
...
| имя | Обязательно/повторяется | введите | Примечания |
| --- | --- | --- | --- |
| **источник** |обязательные |Строка, |Как правило, имя атрибута из исходного объекта. |
| **суффикс** |обязательные |Строка, |Строка, которую необходимо присоединить к концу исходного значения. |
...
| --- | --- | --- | --- |
| **источник** |обязательные |Строка, |Как правило, имя атрибута из исходного объекта. |
| **входной_формат** |обязательные |Строка, |Предполагаемый формат исходного значения. Поддерживаемые форматы см. по адресу [https://msdn.microsoft.com/library/8kb3ddd4%28v=vs.110%29.aspx](https://msdn.microsoft.com/library/8kb3ddd4%28v=vs.110%29.aspx). |
| **выходной_формат** |обязательные |Строка, |Формат вывода даты. |
...
| имя | Обязательно/повторяется | введите | Примечания |
| --- | --- | --- | --- |
| **разделитель** |обязательные |Строка, |Строка, используемая для разделения исходных значений, когда они объединяются в одну строку. Если разделитель не требуется, может не иметь значения (""). |
...
| имя | Обязательно/повторяется | введите | Примечания |
| --- | --- | --- | --- |
| **источник** |обязательные |Строка, |Как правило, имя атрибута. |
| **start** |обязательные |целое число |Индекс положения в строке **source**, откуда должна начинаться подстрока. Первый символ в строке будет иметь индекс 1, второй символ — индекс 2 и т. д. |
| **длина** |обязательные |целое число |Длина подстроки. Если длина превышает размер **исходной** строки, функция возвращает подстроку из **начала** индекса и до конца **исходной** строки. |
...
| имя | Обязательно/повторяется | введите | Примечания |
| --- | --- | --- | --- |
| **источник** |обязательные |Строка, | Обычно атрибут имени или фамилии. |
...
| имя | Обязательно/повторяется | введите | Примечания |
| --- | --- | --- | --- |
| **источник** |обязательные |Логическая строка |Предполагаемые значения **source**: True или False. |
...
| имя | Обязательно/повторяется | введите | Примечания |
| --- | --- | --- | --- |
| **источник** |обязательные |Строка, |Обычно это имя атрибута из **исходного** объекта. |
...
| имя | Обязательно/повторяется | введите | Примечания |
| --- | --- | --- | --- |
| **[appRoleAssignments]** |обязательные |Строка, |Объект **[appRoleAssignments]** . |
...
| имя | Обязательно/повторяется | введите | Примечания |
| --- | --- | --- | --- |
| **источник** |обязательные |Строка, |**источник** , которое необходимо обновить. |
| **разделитель** |обязательные |Строка, |Задает символ, который будет использоваться для разделения строки (к примеру, ",") |
...
| имя | Обязательно/повторяется | введите | Примечания |
| --- | --- | --- | --- |
| **источник** |обязательные |Строка, |**Source** , которое необходимо обновить. |
...
| **key** |обязательные |Строка, |Параметр **key**, с которым сравнивается значение **source**. |
| **значение** |обязательные |Строка, |Значение, которым заменяется значение **source** , соответствующее ключу. |
