...
| Название | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **источник** |Обязательное значение |Строковое |Как правило, имя атрибута из исходного объекта. |
| **суффикс** |Обязательное значение |Строковое |Строка, которую необходимо присоединить к концу исходного значения. |
...
| --- | --- | --- | --- |
| **источник** |Обязательное значение |Строковое |Как правило, имя атрибута из исходного объекта. |
| **входной_формат** |Обязательное значение |Строковое |Предполагаемый формат исходного значения. Поддерживаемые форматы см. по адресу [https://msdn.microsoft.com/library/8kb3ddd4%28v=vs.110%29.aspx](https://msdn.microsoft.com/library/8kb3ddd4%28v=vs.110%29.aspx). |
| **выходной_формат** |Обязательное значение |Строковое |Формат вывода даты. |
...
| Название | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **разделитель** |Обязательное значение |Строковое |Строка, используемая для разделения исходных значений, когда они объединяются в одну строку. Если разделитель не требуется, может не иметь значения (""). |
| **источник1… источникN** |Обязательно, количество повторов может меняться |Строковое |Строковые значения для объединения. |
...
| Название | Обязательно/повторяется | Type | Примечания |
| --- | --- | --- | --- |
| **источник** |Обязательное значение |Строковое |Как правило, имя атрибута. |
| **start** |Обязательное значение |целочисленный |Индекс положения в строке **source**, откуда должна начинаться подстрока. Первый символ в строке будет иметь индекс 1, второй символ — индекс 2 и т. д. |
| **длина** |Обязательное значение |целочисленный |Длина подстроки. Если длина превышает размер **исходной** строки, функция возвращает подстроку из **начала** индекса и до конца **исходной** строки. |
...
| Название | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **источник** |Обязательное значение |Строковое | Обычно атрибут имени или фамилии. |
...
| Название | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **источник** |Обязательное значение |Логическая строка |Предполагаемые значения **source**: True или False. |
...
| Название | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **источник** |Обязательное значение |Строковое |Обычно это имя атрибута из **исходного** объекта. |
| **старое_значение** |Необязательный |Строковое |Значение для замены в **source** или **template**. |
| **шаблон_регулярного_выражения** |Необязательный |Строковое |Шаблон регулярного выражения для значения, заменяемого в **source**. Или, если используется **replacementPropertyName** , шаблон для извлечения значения из **replacementPropertyName**. |
| **имя_группы_регулярного_выражения** |Необязательный |Строковое |Имя группы в **regexPattern**. Если используется **replacementPropertyName** , то значение этой группы будет извлечено как **replacementValue** из **replacementPropertyName**. |
| **значение_замены** |Необязательный |Строковое |Новое значение для замены старого. |
| **имя_атрибута_замены** |Необязательный |Строковое |Имя атрибута, используемого для заменяющего значения |
| **шаблон** |Необязательно |Строковое |Если указано значение **шаблона** , мы будем искать в шаблоне значение **OldValue** и заменять его **исходным** значением. |
...
>2. Эта функция не может применяться к атрибутам с соответствующим приоритетом.  
>3. Эта функция предназначена только для создания записей. При использовании ее с атрибутом задайте для свойства **Apply Mapping** (Применить сопоставление) значение **Только в ходе создания объектов**.
...
| Название | Обязательно/повторяется | Тип | Примечания |
| --- | --- | --- | --- |
| **uniqueValueRule1... uniqueValueRuleN** |Требуется не менее 2, нет верхней границы |Строковое | Список правил формирования уникальных значений для оценки. |
...
| Название | Обязательно/повторяется | Type | Примечания |
| --- | --- | --- | --- |
| **[appRoleAssignments]** |Обязательное значение |Строковое |Объект **[appRoleAssignments]** . |
...
| Название | Обязательно/повторяется | Type | Примечания |
| --- | --- | --- | --- |
| **источник** |Обязательное значение |Строковое |**источник** , которое необходимо обновить. |
| **разделитель** |Обязательное значение |Строковое |Задает символ, который будет использоваться для разделения строки (к примеру, ",") |
...
| Название | Обязательно/повторяется | Type | Примечания |
| --- | --- | --- | --- |
| **источник** |Обязательное значение |Строковое |**Source** , которое необходимо обновить. |
| **defaultValue** |Необязательный |Строковое |Значение по умолчанию, которое необходимо использовать, если источник не соответствует ни одному ключу. Может быть пустой строкой (""). |
| **key** |Обязательное значение |Строковое |Параметр **key**, с которым сравнивается значение **source**. |
| **значение** |Обязательное значение |Строковое |Значение, которым заменяется значение **source** , соответствующее ключу. |
