...
1. На странице **Регистрация приложения** выполните следующие действия.
   ...
   1. В разделе **URL-адрес перенаправления**раскрывающийся список и выберите **общедоступный клиент (мобильный & Desktop)**, а затем введите *https:\//апппроксинативеапп*. 
...
- Вставьте в `<add key="ida:Tenant" value="" />` поле **идентификатор каталога (клиента)** . Это значение (GUID) можно найти и скопировать на странице **Обзор** любого из ваших приложений. 
  
- Вставьте в `<add key="ida:ClientId" value="" />` поле **идентификатор приложения (клиента)** апппроксинативеаппсампле. Это значение (GUID) можно найти и скопировать на странице **обзора** апппроксинативеаппсампле.
