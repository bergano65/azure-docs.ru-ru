...
![Внутренний URL-адрес, используйте формат HTTP (s) :/ / *. \<домена >](./media/application-proxy-wildcard/22.png)

При настройке **внешнего URL-адреса** необходимо использовать следующий формат: `https://*.<custom domain>`.

![Для внешнего URL-адреса используйте формат https://*. \<личного домена >](./media/application-proxy-wildcard/21.png)
