...
| Учебники | [Список руководств по интеграции приложений SaaS с Azure AD](../saas-apps/tutorial-list.md) |
| | [Учебник. Настройка Workday для автоматической подготовки пользователей](../saas-apps/workday-inbound-tutorial.md#frequently-asked-questions-faq) |
| Часто задаваемые вопросы | [Автоматическая подготовка пользователей](user-provisioning.md#what-applications-and-systems-can-i-use-with-azure-ad-automatic-user-provisioning) |
...
Используйте [фильтры области](define-conditional-rules-for-provisioning-user-accounts.md) , чтобы определить правила на основе атрибутов, которые определяют, какие пользователи должны быть подготовлены из облачного приложения отдела кадров для Active Directory или Azure AD.
...
С помощью подготовки вы получаете возможность сопоставлять существующие учетные записи между исходной и целевой системами. При интеграции облачного приложения отдела кадров с помощью службы подготовки Azure AD можно [настроить сопоставление атрибутов](configure-automatic-user-provisioning-portal.md#mappings) , чтобы определить, какие данные пользователя должны передаваться из облачного приложения отдела кадров в Active Directory или Azure AD.
...
В зависимости от требований вы можете настроить логику сопоставления с помощью [выражений Azure AD](functions-for-customizing-application-data.md) , чтобы учетная запись Active Directory была включена или отключена на основе сочетания точек данных.
...
Функция Azure AD [селектуникуевалуес](functions-for-customizing-application-data.md#selectuniquevalue) оценивает каждое правило, а затем проверяет значение, созданное для уникальности в целевой системе. Пример см. в разделе [Создание уникального значения для атрибута userPrincipalName (UPN)](functions-for-customizing-application-data.md#generate-unique-value-for-userprincipalname-upn-attribute).
...
Используйте функцию [switch ()](functions-for-customizing-application-data.md#switch) для настройки бизнес-логики назначения подразделения и сопоставьте ее с Active Directory атрибутом **парентдистингуишеднаме**.
...
При запуске процесса объединения необходимо задать и доставить временный пароль для новых учетных записей пользователей. С облачным персоналом для подготовки пользователей в Azure AD вы можете развернуть возможность [самостоятельного сброса пароля](../authentication/quickstart-sspr.md) Azure AD (SSPR) для пользователя в первый день.
...
Когда вы удовлетворены результатами начального цикла для тестовых пользователей, запустите [добавочные обновления](how-provisioning-works.md#incremental-cycles).
...
После успешного выполнения [начального цикла](how-provisioning-works.md#initial-cycle)служба подготовки Azure AD продолжит выполнять добавочные обновления на неопределенное время с интервалами, определенными в руководствах для каждого приложения, до тех пор, пока не произойдет одно из следующих событий:
...
Чтобы просмотреть эти события и все другие действия, выполняемые службой подготовки, Узнайте, [как просматривать журналы и получать отчеты о действиях по подготовке](check-status-user-account-provisioning.md).
