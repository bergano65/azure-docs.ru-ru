...
Доступность назначения на основе групп определяется лицензионным соглашением. Назначение на основе групп поддерживается только для групп безопасности. Членство в вложенных группах и группы Office 365 сейчас не поддерживаются.

## <a name="configure-the-application-to-require-assignment"></a>Настройка приложения для обязательного назначения

Приложение можно настроить так, чтобы оно затребовало назначение до того, как оно будет доступно. Чтобы затребовать назначение:

1. Войдите в портал Azure с помощью учетной записи администратора или владельца приложения в разделе **корпоративные приложения**.
...
1.  Откройте [**портал Azure**](https://portal.azure.com/) и войдите в систему как **глобальный администратор или как владелец приложения без прав администратора.**
...
1.  Откройте [**портал Azure**](https://portal.azure.com/) и войдите в систему как **глобальный администратор** или как владелец приложения без прав администратора с назначенной лицензией Azure AD Premium.
...
По истечении короткого периода времени пользователи в выбранных группах смогут запускать эти приложения с помощью методов, описанных в разделе Описание решения. Если используются динамические группы, назначения для пользователей в назначенных группах могут отображаться с некоторой задержкой.
...
13. **Необязательно.** **для приложений, которые предоставляют роли**. Если вы хотите назначить роль с утвержденными пользователями, щелкните селектор рядом с **ролью, которой должны быть назначены пользователи в этом приложении?** для выбора роли, которой должны быть назначены эти пользователи.
