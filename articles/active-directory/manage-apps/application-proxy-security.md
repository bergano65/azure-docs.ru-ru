...
С [помощью условия «Условного доступа»](../conditional-access/overview.md)можно определить ограничения на доступ к бэк-энд-приложениям. Вы можете создать политики, ограничивающие вход на основе расположения, надежности аутентификации и профиля риска пользователя.

Можно также использовать Условный доступ для настройки политик мультифакторной аутентификации, добавляя еще один уровень безопасности в аутентификацию пользователя. Кроме того, ваши приложения также могут быть перенаправлены в Microsoft Cloud App Security через Azure AD Conditional Access для обеспечения мониторинга и управления в режиме реального времени с помощью политик [доступа](https://docs.microsoft.com/cloud-app-security/access-policy-aad) и [сеансов](https://docs.microsoft.com/cloud-app-security/session-policy-aad)
...
Являясь частью Azure Active Directory, прокси приложения может использовать [защиту идентификации Azure AD](../active-directory-identityprotection.md) с данными из Microsoft Security Response Center и Digital Crimes Unit. Вместе мы активно выявляем скомпрометированные учетные записи и предлагаем защиту от вхливых в систему с высоким риском. Мы принимаем во внимание множество факторов, чтобы определить, какие попытки входной связи являются высоким риском. К этим факторам относятся помеченные флагами зараженные устройства, анонимизация сетей и нетипичные или маловероятные расположения.
...
#### <a name="azure-ddos-protection-service"></a>Служба защиты DDoS Azure

Приложения, опубликованные через Application Proxy, защищены от атак Distributed Denial of Service (DDoS). **Защита DDoS Azure** — это служба, предлагаемая платформой Azure для защиты ресурсов Azure от атак типа «отказ в обслуживании». **Базовый** уровень обслуживания автоматически включается, обеспечивая постоянное мониторинг трафика и смягчение общих атак сетевого уровня в режиме реального времени. Также доступен **стандартный** уровень, предлагающий дополнительные возможности по смягчению последствий, настроенные специально на ресурсы виртуальной сети Azure. Для получения подробной информации смотрите [обзор стандарта DDoS-стандарта Azure DDoS.](https://docs.microsoft.com/azure/virtual-network/ddos-protection-overview)
...
>[!NOTE]
>Все сообщения происходят через TLS, и они всегда происходят на разъеме для службы приложения Прокси. В службе используются только исходящие соединения.
...
1. Прокси-сервер приложения проверяет любые требования политики условного доступа к конкретному приложению. Этот шаг позволяет гарантировать, что пользователь назначен приложению. Если требуется двухфакторная проверка подлинности, появляется запрос на второй метод проверки подлинности.
...
#### <a name="2-the-service-places-a-request-in-the-connector-queue"></a>2. Служба помещает запрос в очередь разъема
...
#### <a name="3-the-connector-processes-the-request-from-the-queue"></a>3. Разъем обрабатывает запрос из очереди. 
...
#### <a name="4-the-connector-waits-for-a-response"></a>4. Разъем ждет ответа.
...
#### <a name="5-the-service-streams-data-to-the-user"></a>5. Служба транслирует данные пользователю. 
