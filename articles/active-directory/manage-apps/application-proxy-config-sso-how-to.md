---
title: Как настроить одиночный вписаться в приложение App Proxy
...
Чтобы настроить единый вход, убедитесь, что для приложения настроена предварительная проверка подлинности с использованием Azure Active Directory. Для этого можно перейти к **приложениям Azure Active Directory**  - &gt; **Enterprise Applications**  - &gt; **Все приложения**  - &gt; Приложения ** - &gt; Прокси.** На этой странице найдите параметр "Предварительная проверка подлинности" и убедитесь, что для него установлено значение "Azure Active Directory". 
...
-   **SAML одиночный вписать :** С одним вданным в систему SAML, Azure AD проверяет подлинность приложения с помощью учетной записи Azure AD пользователя. Azure AD передает приложению данные для входа через протокол соединения. Если используется единый вход на основе SAML, вы можете сопоставлять пользователей с конкретными ролями приложений согласно правилам, определенным в утверждениях SAML. Для получения информации о настройке ОДНОГО входного ввоза SAML, см [SAML для одного входного с приложением Proxy](application-proxy-configure-single-sign-on-on-premises-apps.md).

Каждый из этих вариантов входа можно выбрать, открыв свое приложение в разделе "Корпоративные приложения" и выбрав страницу **Единый вход** в меню слева. Обратите внимание, что если ваше приложение было создано на старом портале, вы можете не увидеть все эти параметры.
...
- [SAML для одного входного с приложением Прокси](application-proxy-configure-single-sign-on-on-premises-apps.md).
