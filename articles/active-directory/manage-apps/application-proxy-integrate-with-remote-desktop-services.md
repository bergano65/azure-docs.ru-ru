...
- Текущие клиенты, использующие службы удаленных рабочих столов, которые хотят сократить поверхность атаки своего развертывания с помощью прокси приложения Azure AD. Этот сценарий предоставляет ограниченный набор двухфакторную проверку подлинности и элементов управления условным доступом к RDS.
...
- Для потока предварительной проверки подлинности Azure AD, пользователи могут подключаться только к ресурсам, опубликованные для них в **RemoteApp и рабочим столам** области. Пользователи не смогут подключиться к рабочего стола с помощью **подключение к удаленным ПК** области.
...
5. В разделе **управление**выберите **фирменной символики**.
6. Обновление **URL-адрес домашней страницы** поле, чтобы к удаленным рабочим Столам конечную точку веб-(например `https://\<rdhost\>.com/RDWeb`).
...
Поток предварительной аутентификации предлагает больше преимуществ с точки зрения безопасности, чем сквозной режим. С помощью предварительной проверки подлинности можно использовать функции проверки подлинности Azure AD, как единый вход, условный доступ и двухфакторную проверку подлинности для локальных ресурсов. Также гарантируется, что сети достигает только прошедший аутентификацию трафик.
