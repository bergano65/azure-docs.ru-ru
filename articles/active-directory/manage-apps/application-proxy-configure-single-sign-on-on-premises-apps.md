...
1. В **задать вверх единого входа с помощью SAML** странице, изменить **базовой конфигурации SAML** данных и выполните действия, описанные в [ввод базовой конфигурации SAML](configure-single-sign-on-non-gallery-applications.md#saml-based-single-sign-on) для настройки на основе SAML Проверка подлинности для приложения.
   ...
   * Убедитесь, что **URL-адрес ответа** или совпадает с путем под **внешний URL-адрес** для локального приложения, которые опубликованы через прокси приложения. 
   * Если приложению требуется другой **URL-адрес ответа** для конфигурации SAML, добавьте его в виде **дополнительных** URL-адрес в списке и Марк флажок рядом с ним, чтобы обозначить его как основной  **URL-адрес ответа** для отправки ответов инициированном поставщиком Удостоверений SAML.
   * Для потока, инициированный поставщиком услуг убедитесь, что приложение также указан правильный **URL-адрес ответа** или URL-адрес службы обработчика утверждений использовать для получения маркера проверки подлинности.
