---
title: Настройка введома автоматического ускорения с помощью Home Realm Discovery
description: Узнайте, как настроить политику Home Realm Discovery для проверки подлинности Active Directory Для пользователей с федеративными пользователями, включая автоматическое ускорение и подсказки домена.
...
В этой статье содержится введение в настройку поведения аутентификации Активных каталогов Azure для федеративных пользователей. В нем рассказывается о настройке ограничений автоматического ускорения входа и проверки подлинности для пользователей в федеративных доменах.
...
Для создания и управления политикой управления персоналом можно использовать cmdlets Azure Active Directory PowerShell.
...
Чтобы увидеть новую политику и получить ее **ObjectID,** запустите следующую команду:
...
Поскольку вы используете PowerShell, вы можете использовать следующие cmdlet, чтобы перечислить принципы службы и их идентионные идеи.
...
- Для получения дополнительной информации о едином всхотливке пользователя смотрите [в каталоге Active Azure Single-sign-on.](what-is-single-sign-on.md)
- Посетите [идентификационную платформу Майкрософт](../develop/v2-overview.md) для получения обзора всего контента, связанного с разработчиком.
