...
- Общие проблемы для первоначальной проверки
  ...
  * Убедитесь, что учетная запись пользователя **включена** для входе в [систему.](#problems-with-the-users-account)
  ...
  * Убедитесь, что **политика условного доступа** или политика **защиты идентификации** не блокирует доступ пользователей. [Проверьте конкретную политику условного доступа](#problems-with-conditional-access-policies) или [проверьте политику условного доступа или](#check-a-specific-applications-conditional-access-policy) отключить [конкретную политику условного доступа](#disable-a-specific-conditional-access-policy)
...
- Для приложений **Майкрософт, ** **которым лицензия не нужна**, можно проверить следующее:
  ...
  * Если приложение запрашивает **разрешения на уровне пользователя** (например, "Доступ к почтовому ящику этого пользователя"), убедитесь, что пользователь вошел в приложение и выполнил **операцию согласия пользователя,** чтобы позволить приложению получить доступ к своим данным.
...
-   [Сбросить пароль пользователя](#reset-a-users-password)
...
-   [Назначить пользователю лицензию](#assign-a-user-a-license)
...
1.  Откройте [**портал Azure**](https://portal.azure.com/) и вопием в качестве **глобального администратора.**
...
4.  щелкните **пользователей и группы** в меню навигации.

5.  нажмите **Все пользователи**.
...
Для проверки или проверки одной политики условного доступа:
...
4. нажмите **корпоративные приложения** в меню навигации.

5. щелкните элемент **навигации «Условный доступ».**

6. Щелкните политику, которую хотите проверить.

7. Просмотрите, что нет никаких конкретных условий, назначений или других параметров, которые могут блокировать доступ пользователей.

   >[!NOTE]
   >Возможно, вы захотите временно отключить эту политику, чтобы убедиться, что она не влияет на знак ins. Для этого установите **политику включить** кнопку **«Нет»** и нажмите кнопку **«Сохранить».**
   >
   >
...
Для проверки или проверки настроенной в настоящее время политики условного доступа одного приложения:
...
6.  Поиск интересующего вас приложения или попытки пользователя войти в систему по имени дисплея приложения или идентификатору приложения.
