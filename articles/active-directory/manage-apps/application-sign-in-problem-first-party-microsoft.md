...
description: Устранение распространенных проблем при входе в сторонние приложения Майкрософт с помощью Azure AD (например, Microsoft 365).
...
ms.locfileid: "99260017"
---
# <a name="problems-signing-in-to-a-microsoft-application"></a>Проблемы при входе в приложение Майкрософт

Приложения Майкрософт (например, Exchange, SharePoint, Yammer и т. д.) назначаются и управляются немного иначе, чем сторонние приложения SaaS или другие приложения, которые вы интегрируете с Azure AD для единого входа.
...
-   Для приложений в Microsoft 365 или других платных комплектах пользователям предоставляется доступ по **назначению лицензии** либо непосредственно к учетной записи пользователя, либо через группу с помощью возможности назначения лицензий на основе группы.

-   Для приложений, которые Майкрософт или сторонний разработчик публикуют для свободного использования, пользователи могут получать доступ через **согласие пользователя**. Это означает, что пользователь входит в приложение с рабочей или учебной учетной записью Azure AD, разрешая приложению доступ к ограниченному набору данных в этой учетной записи.
...
Ниже приведен список общих проблемных областей, которые можно изучить более подробно, если вы поняли, где именно нужно искать. Однако для быстрого начала работы мы рекомендуем прочитать статью Проблемы при входе в приложение Майкрософт.
...
- Общие проблемы для первоначальной проверки
  ...
  * Убедитесь, что учетная запись пользователя **включена** для входа в систему. [Проверка состояния учетной записи пользователя](#problems-with-the-users-account)

  * Удостоверьтесь в том, что **пароль действителен и пользователь не забыл его**. [Сброс пароля пользователя](#reset-a-users-password) или [Включение самостоятельного сброса пароля](../authentication/tutorial-enable-sspr.md)
  ...
  * Убедитесь, что **политика условного доступа** или политика **защиты идентификации** не блокирует доступ пользователей. [Проверьте определенную политику условного доступа](#problems-with-conditional-access-policies) или [Проверьте политику условного доступа конкретного приложения](#check-a-specific-applications-conditional-access-policy) или [Отключите определенную политику условного](#disable-a-specific-conditional-access-policy) доступа.
...
- Для приложений **Майкрософт,** **которым лицензия не нужна**, можно проверить следующее:
  ...
  * Если приложение запрашивает **разрешения уровня пользователя** (например, "доступ к почтовому ящику этого пользователя"), убедитесь, что пользователь выполнил вход в приложение и выполнил **операцию согласия на уровне пользователя** , чтобы предоставить приложению доступ к своим данным.
...
Чтобы проверить или проверить одну политику условного доступа, выполните следующие действия.
...
4. в меню навигации выберите пункт **корпоративные приложения** .

5. Щелкните элемент навигации **условного доступа** .

6. Щелкните политику, которую хотите проверить.

7. Проверьте, что отсутствуют определенные условия, назначения или другие параметры, которые могут блокировать доступ пользователей.

   >[!NOTE]
   >Вы можете временно отключить эту политику, чтобы убедиться, что она не влияет на входы. Для этого задайте для переключателя **включить политику** значение **нет** и нажмите кнопку **сохранить** .
   >
   >

### <a name="check-a-specific-applications-conditional-access-policy"></a>Проверка политики условного доступа конкретного приложения

Чтобы проверить или проверить настроенную в настоящее время политику условного доступа для одного приложения, выполните следующие действия.
...
6.  Найдите приложение, которое вас интересует, или пользователь пытается войти в по отображаемому имени приложения или ИДЕНТИФИКАТОРу приложения.
...
7.  Щелкните элемент навигации **условного доступа** .
...
### <a name="disable-a-specific-conditional-access-policy"></a>Отключение определенной политики условного доступа
...
-   Если вы хотите сделать это программными средствами, см. статью [Запрос на получение согласия одного пользователя](../develop/v2-permissions-and-consent.md#requesting-individual-user-consent).
...
-   Вы также можете принудительно реализовать согласие на уровне администратора, выполнив инструкции из раздела **Запрос разрешений у администратора каталога** статьи [Использование конечной точки предоставления согласия администратора](../develop/v2-permissions-and-consent.md#using-the-admin-consent-endpoint).

## <a name="next-steps"></a>Дальнейшие действия
[Использование конечной точки предоставления согласия администратора](../develop/v2-permissions-and-consent.md#using-the-admin-consent-endpoint)