...
**Очистка текущего состояния и перезапуск синхронизации** запускает начальный цикл. Затем служба будет оценивать всех пользователей в исходной системе и определить, находятся ли они в области для подготовки. Это может быть полезно, если приложение находится в карантине или необходимо внести изменения в сопоставления атрибутов. Это не следует использовать для активации запроса Delete или Disable, так как эти события могут быть удалены при срабатывании очистки состояния и перезапуске. Начальный цикл также занимает больше времени, чем при обычном инкрементном цикле, из-за количества объектов, которые необходимо вычислить. Дополнительные сведения о производительности начального и добавочного циклов можно получить [здесь.](https://docs.microsoft.com/azure/active-directory/manage-apps/application-provisioning-when-will-provisioning-finish-specific-user) 
