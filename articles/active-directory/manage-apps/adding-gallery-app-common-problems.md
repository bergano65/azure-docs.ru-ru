---
title: Проблема при добавлении приложения из коллекции Azure AD | Документация Майкрософт
...
Если вам нужна помощь в изучении приложений, [список учебников по интеграции приложений SaaS со](https://docs.microsoft.com/azure/active-directory/active-directory-saas-tutorial-list) статьей Active Directory Azure является хорошим местом для начала.
...
6.  Нажмите **значок** **копии** справа от текстового ящика **ошибки Copy,** чтобы скопировать все сведения уведомлений, чтобы поделиться с инженером группы поддержки или группой продуктов.
...
- **Внутренний идентификатор транзакций** — внутренний идентификатор, который мы можем использовать для поиска ошибки в наших системах

  -   Пример: **71a2f329-ca29-402f-aa72-bc00a7aca603**.

- **UPN** — пользователь, который выполнил операцию.

  -   Пример – **f128.info\@tperkins**
...
-   **Идентификатор объекта** – **(может быть пустым)** идентификатор объекта, против которого была выполнена операция

    -   Пример: **8e08161d-f2fd-40ad-a34a-a9632d6bb599**.

-   **Подробные сведения** — подробное описание результата операции.

    -   Пример - **Внутренний URL `https://bing.com/` является недействительным, поскольку он уже используется**
