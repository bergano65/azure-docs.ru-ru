...
- В облачных приложениях может использоваться единый вход на основе SAML или паролей либо связанный способ. Единый вход также можно отключить. SAML — это наиболее безопасный способ единого входа.
...
| :------ | :------- | :----- |
| [SAML](#saml-sso) | Только в облаке | По мере возможности используйте SAML. SAML работает, если приложения настроены на использование одного из протоколов SAML.|
| [На основе пароля](#password-based-sso) | В облаке и локальной среде | Используйте этот способ, если в приложении выполняется проверка подлинности с помощью имени пользователя и пароля. Единый вход на основе пароля позволяет безопасно хранить пароли приложений и воспроизводить его с помощью расширения веб-браузера или мобильного приложения. Этот способ использует существующий процесс входа, предоставляемый приложением, однако позволяет администраторам управлять паролями. |
...
| [Встроенная проверка подлинности Windows (IWA)](#integrated-windows-authentication-iwa-sso) | Только в локальной среде | Используйте этот способ единого входа для приложений, использующих [встроенную проверку подлинности Windows (IWA)](/aspnet/web-api/overview/security/integrated-windows-authentication) или поддерживающих утверждения. Соединители прокси приложений используют ограниченное делегирование Kerberos (KCD) для проверки подлинности пользователей в приложении. | 
| [На основе заголовков](#header-based-sso) | Только в локальной среде | Используйте единый вход на основе заголовков, если приложение использует заголовки для проверки подлинности. Для единого входа на основе заголовков требуется PingAccess для Azure Active Directory. Прокси приложения использует Azure AD для проверки подлинности пользователя, а затем передает трафик через службу соединителей.  | 
...
- безопаснее, чем единый вход на основе паролей и все остальные способы единого входа;
- является рекомендуемым нами способом единого входа.
