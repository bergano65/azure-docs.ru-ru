...
![Диаграмма потока решений для метода единого ввоза](./media/what-is-single-sign-on/choose-single-sign-on-method-040419.png)
...
| [Связанный](#linked-sign-on) | В облаке и локальной среде | Выберите связанную регистрацию, когда приложение настроено для единого входного в другой службе поставщика идентификационных данных. При выборе этого варианта единый вход для приложения не настраивается. Однако для приложения может уже быть настроен единый вход с использованием другой службы, например служб федерации Active Directory.|
| [Отключен](#disabled-sso) | В облаке и локальной среде | Отключайте единый вход, если приложение не готово к настройке для единого входа. Этот режим является по умолчанию при создании приложения.|
...
При разработке новых приложений используйте современные протоколы, такие как OpenID Connect и OAuth, чтобы оптимально использовать единый вход для своего приложения на различных платформах устройств. OAuth позволяет пользователям или адм-имам [предоставлять согласие](configure-user-consent.md) на защищенные ресурсы, такие как [Microsoft Graph.](/graph/overview) Мы предоставляем простые в использовании [SDK](../develop/reference-v2-libraries.md) для вашего приложения, и, кроме того, ваше приложение будет готово к использованию [Microsoft Graph.](/graph/overview)

Дополнительные сведения см. в разделе:

- [OAuth 2.0](../develop/v2-oauth2-auth-code-flow.md)
- [OpenID Connect 1.0](../develop/v2-protocols-oidc.md)
- [Руководство разработчика идентификационных платформ Майкрософт.](https://docs.microsoft.com/azure/active-directory/develop/active-directory-developers-guide)
...
Чтобы настроить приложение SaaS для ОДНОГО знака на основе SAML, [см.](configure-single-sign-on-non-gallery-applications.md) Кроме того, многие приложения SaaS имеют руководства [для определенных приложений](../saas-apps/tutorial-list.md), которые позволяют пошагово выполнить настройку единого входа на основе SAML.

Чтобы настроить приложение для WS-Federation, следуйте тем же рекомендациям для настройки приложения для ОДНОГО знака на основе SAML, [см.](configure-single-sign-on-non-gallery-applications.md) На этапе настройки приложения для использования Azure AD необходимо заменить URL-адрес входа Azure AD `https://login.microsoftonline.com/<tenant-ID>/wsfed`для конечных точек WS-Federation.

Чтобы настроить предварительное приложение для SAML-единственного знака, см. [SAML одноразовый для местных приложений с application Proxy.](application-proxy-configure-single-sign-on-on-premises-apps.md)
...
Для настройки облачного приложения для единого входного входного знака на основе пароля [см.](configure-password-single-sign-on-non-gallery-applications.md)
...
Связанный единый вход позволяет службе Azure AD добавлять возможность единого входа в приложение, которое уже настроено для единого входа в другой службе. Связанное приложение может быть видно пользователям на портале Office 365 или портале Azure AD MyApps. Например, пользователь может запустить приложение, настроенное для единого входа в службах федерации Active Directory 2.0 (AD FS) на портале Office 365. Для связанных приложений, запускаемых с портала Office 365 или Azure AD MyApps, также доступна дополнительная отчетность. Для настройки приложения для связанного входиного, [см. Настроить связанные знаки.](configure-linked-sign-on.md)

### <a name="linked-sign-on-for-application-migration"></a>Связанный всхот для миграции приложений

Связанные ссылки могут обеспечить согласованный пользовательский опыт во время переноса приложений в течение определенного периода времени. Если вы переносите приложения в Active Directory Azure, можно использовать связанные ввести в эксплуатацию для быстрого опубликования ссылок на все приложения, которые вы собираетесь перенести.  Пользователи могут найти все ссылки на [портале MyApps](../user-help/active-directory-saas-access-panel-introduction.md) или в [средстве запуска приложений Office 365](https://support.office.com/article/meet-the-office-365-app-launcher-79f12104-6fed-442f-96a0-eb089a3f476a). Пользователи не знают, получают ли они доступ к связанному или перенесенному приложению.  
...
Обратите внимание, что если вы настроили приложение для одного вхождения на основе SP SAML и вы измените режим SSO, чтобы отключить, это не остановит пользователей от подписания в приложение за пределами портала MyApps. Для достижения этой цели необходимо [отключить возможность для пользователей войти в систему](disable-user-sign-in-portal.md)
...
Выберите интегрированный режим проверки подлинности Windows, чтобы обеспечить единое вхождения в предварительное приложение, которое проверяет подлинность с помощью IWA.
...
![Диаграмма потока потока проверки подлинности Microsoft Azure AD](./media/application-proxy-configure-single-sign-on-with-kcd/AuthDiagram.png)

1. Пользователь вводит URL-адрес, чтобы получить доступ к приложению в помещении через приложение Proxy.
...
1. Разъем использует Kerberos Constrained Delegation (KCD) переговоры с на территории AD, выдавая себя за пользователя, чтобы получить маркер Kerberos в приложение.
...
Выберите одиночный вход на основе заголовка, когда приложение Proxy и PingAccess настроены для приложения.
...
* [Настройка одного знака на основе SAML](configure-single-sign-on-non-gallery-applications.md)
* [Настройка одного знака на основе пароля на](configure-password-single-sign-on-non-gallery-applications.md)
* [Настройка связанного входиного знака](configure-linked-sign-on.md)
