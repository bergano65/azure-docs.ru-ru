...
![Показывает, Подготовка из Azure AD для приложения или хранилища удостоверений][0]<br/>
*Рис. 1. Подготовка из Azure Active Directory для приложения или хранилища удостоверений, реализующий SCIM*
...
1. Введите имя для вашего приложения и выберите **добавить** для создания объекта приложения. Новое приложение добавляется в список корпоративных приложений и открывает его экран управления приложения.
   ...
   ![Снимок экрана показывает в коллекцию приложений Azure AD][1]<br/>
...
1. В меню **Режим подготовки к работе** выберите **Автоматический**.
   ...
   ![Пример. Страница подготовки приложения на портале Azure][2]<br/>
...
![Показывает, пользователь при подготовке и отзыве последовательности][4]<br/>
...
![Показывает группу при подготовке и отзыве последовательности][5]<br/>
...
* Библиотеки Common Language Infrastructure (CLI) для использования с языками, основанными на этой инфраструктуре, например C#. Одна из этих библиотек, Microsoft.SystemForCrossDomainIdentityManagement.Service, объявляет интерфейс, Microsoft.SystemForCrossDomainIdentityManagement.IProvider, показанный на следующем рисунке. Разработчик, использующий эти библиотеки, может реализовать класс с этим интерфейсом и обращаться к нему как к поставщику службы. Эти библиотеки позволяют разработчику развернуть веб-службы, которая соответствует спецификации SCIM. Веб-служба может размещаться в службах IIS, или в любой исполняемой сборке CLI. Запрос преобразовывается в вызовы методов поставщика, которые программирует разработчик для управления хранилищем удостоверений.
   ...
   ![Подразделение: Запрос, преобразуются в вызовы методов поставщика][3]
...
1. К этому методу, чтобы любой запрос на любой из конечных точек службы, проверку подлинности как выпустившая маркер, выданный службой Azure Active Directory для указанного клиента для доступа к веб-службы Azure AD Graph, добавьте следующий код: 
...
1. Если ответ на запрос в веб-службу для пользователя со значением атрибута externalId, который совпадает со значением атрибута mailNickname пользователя не возвращает всех пользователей, затем Azure Active Directory запрашивает этой службе на подготовку пользователя, соответствующего столбцу в Azure Active Directory.  Ниже приведен пример такого запроса. 
   ...
   ```
    ...
    Content-type: application/scim+json
    {
      ...
      "manager":null}
   ```
   Библиотеки CLI, предоставляемые корпорацией Майкрософт для реализации служб SCIM преобразуют этот запрос в вызов метода Create поставщика службы.  Подпись метода Create будет выглядеть так: 
   ```csharp
    // System.Threading.Tasks.Tasks is defined in mscorlib.dll.  
    // Microsoft.SystemForCrossDomainIdentityManagement.Resource is defined in 
    // Microsoft.SystemForCrossDomainIdentityManagement.Schemas.  
...
1. Для обновления пользователя, уже существующего в хранилище удостоверений с интерфейсом SCIM, Azure Active Directory запросит у службы текущее состояние этого пользователя, отправив запрос следующего вида. 
   ```
    ...
    Authorization: Bearer ...
   ```
   Если служба создана с помощью библиотеки CLI, предоставляемые корпорацией Майкрософт для реализации служб SCIM такой запрос будет преобразован в вызов метода Retrieve поставщика услуги.  Подпись метода Retrieve будет выглядеть так: 
   ```csharp
    // System.Threading.Tasks.Tasks is defined in mscorlib.dll.  
...
1. Если ссылочный атрибут должен быть обновлен, Azure Active Directory запрашивает службу, чтобы определить, соответствует ли текущее значение атрибута ссылки в хранилище удостоверений интерфейсом для которого служба уже значение этого атрибута в Azure Active Каталог. Для пользователей таким способом запрашивается только значение атрибута manager. Ниже приведен пример запроса на проверку значения атрибута manager для определенного пользователя. 

   Если служба построена с использованием библиотеки CLI, предоставляемые корпорацией Майкрософт для реализации служб SCIM, то запрос будет преобразован в вызов метода Query поставщика услуги. В нем передается аргумент parameters, значением которого будет объект со следующими свойствами. 
...
1. Ниже приведен пример запроса от Azure Active Directory к службе SCIM для обновления пользователя. 
   ...
   ```
   Библиотеки Microsoft CLI для реализации служб SCIM преобразуют такой запрос в вызов метода Update поставщика услуги. Подпись метода Update выглядит следующим образом. 
   ```csharp
...
Определенные приложения разрешать входящий трафик для своих приложений. Службу подготовки Azure AD могла функционировать должным образом необходимо разрешить IP-адреса. Список IP-адресов для каждой службы тега или региона, см. в файле JSON - [диапазоны IP-адресов Azure и теги служб — общедоступное облако](https://www.microsoft.com/download/details.aspx?id=56519). Можно загрузить и программировать эти IP-адреса в брандмауэре, при необходимости. Зарезервированный IP-адресов для подготовки Azure AD можно найти в разделе «AzureActiveDirectoryDomainServices.»
