...
4. Сервисный автобус проверяет, использовалось ли соединение ранее существующего разъема в группе разъемов. Если это так, он повторно использует соединение. Если коннектор еще не подключен к соединению, он выбирает доступный разъем случайным образом для сигнала. Затем разъем забирает запрос от Service Bus.
   ...
   - Соединение восстановлено только в том случае, если соединение нарушено или происходит простой период в 10 минут. Например, соединение может быть нарушено при перезагрузке службы связи или перезапуске службы развязок или сбое в работе сети.
...
8. Последующие запросы из того же соединения повторяют вышеперечисленные шаги до тех пор, пока это соединение не будет нарушено или не будет простаивать в течение 10 минут.
