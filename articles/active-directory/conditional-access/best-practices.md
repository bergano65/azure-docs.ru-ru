---
title: Советы и рекомендации для условного доступа в Azure Active Directory | Документация Майкрософт
description: Узнайте о том, которые следует знать, и что не нужно делать при настройке политики условного доступа.
...
ms.locfileid: "67509413"
---
# <a name="best-practices-for-conditional-access-in-azure-active-directory"></a>Советы и рекомендации для условного доступа в Azure Active Directory
...
- Что это следует делать при настройке политики условного доступа. 

В этой статье предполагается, что вам знакомы основные понятия и терминология описанные в [что такое условный доступ в Azure Active Directory?](../active-directory-conditional-access-azure-portal.md)
...
| Что           | Как                                  | Назначение |
| :--            | :--                                  | :-- |
| **Облачные приложения** |Выберите одно или несколько приложений.  | Цель политики условного доступа — позволяют управлять как авторизованные пользователи могут получить доступ к облачным приложениям.|
| **Пользователи и группы** | Выберите по меньшей мере одного пользователя или группу, которая имеет права для доступа к выбранным облачным приложениям. | Политики условного доступа, которая имеет не назначены пользователи и группы, не активируется. |
...
### <a name="how-are-conditional-access-policies-applied"></a>Применение политики условного доступа?

Несколько политик условного доступа могут применяться при доступе к облачному приложению. В этом случае условия всех применяемых политик должны быть выполнены. Например, если для одной политики требуется MFA, а для другой требуется соответствующее устройство, необходимо пройти MFA и использовать соответствующее устройство. 
...
- На **втором** этапе вам будет предложено удовлетворить невыполненные требования. Если ни одной из политик заблокировать доступ, являются заблокирован и не предлагается для удовлетворения другие элементы управления политиками. Если политики блокируют вы, вам будет предложено удовлетворяют другие элементы управления политики в следующем порядке:

   ![Порядок](./media/best-practices/06.png)
    
   Внешних поставщиков многофакторной проверки Подлинности и условия использования будут следующие.
...
Если вас заблокирован на портале Azure AD, из-за неправильной настройки политики условного доступа:
...
- Если ни один из администраторов в организации не может обновить политику, необходимо отправить запрос в службу поддержки. Технической поддержки Майкрософт можно просмотреть и обновить политики условного доступа, которые препятствуют доступу.
...
Да, вы можете использовать Exchange ActiveSync в политике условного доступа с некоторыми [ограничения](block-legacy-authentication.md). 
...
Очень важно для политик, требующих взаимодействия с пользователем, например многофакторной проверки подлинности, когда управление доступом осуществляется в начале сеанса или задачи. Если этого не сделать, пользователи не смогут выполнить некоторые задачи в приложении. Например если требуется многофакторная проверка подлинности на неуправляемых устройствах для доступа к SharePoint, но не к электронной почте, работа в электронной почте пользователи не смогут для вложения файлов SharePoint в сообщение. Дополнительные сведения можно найти в статье [Каковы зависимости служб в Azure Active Directory условного доступа?](service-dependencies.md).

## <a name="what-you-should-avoid-doing"></a>Чего следует избегать

Платформа условного доступа предоставляет гибкость конфигурации. Тем не менее исключительная гибкость также означает, что во избежание нежелательных результатов необходимо внимательно просмотреть каждую политику конфигурации, прежде чем выпускать ее. В этом контексте следует уделить особое внимание назначениям, которые влияют на полные наборы, такие как **все пользователи / группы / облачные приложения**.
...
- Как настроить политику условного доступа, см. в разделе [требования MFA для конкретных приложений, с помощью Azure Active Directory условного доступа](app-based-mfa.md).
- Как для планирования политик условного доступа, см. в разделе [способы планирования развертывания условного доступа в Azure Active Directory](plan-conditional-access.md).
