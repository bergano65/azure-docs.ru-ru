...
Количество именованных расположений, которые можно настроить, ограничено размером соответствующего объекта в Azure AD. Можно настроить один из следующих:
...
При оценке политики используется общедоступный IP-адрес пользователя. Для устройств в частной сети используется не клиентский IP-адрес устройства пользователя в интрасети, а адрес, применяемый в сети для подключения к Интернету. Если на устройстве только IPv6-адрес, Настройка условия расположения не поддерживается.
