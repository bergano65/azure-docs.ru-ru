---
title: Требовать от МИД ненадежных сетей - Активный каталог Azure
description: Узнайте, как настроить политику условного доступа в Active Directory Azure (Azure AD) для попыток доступа из ненадежных сетей.
services: active-directory
ms.service: active-directory
ms.subservice: conditional-access
ms.topic: article
ms.date: 11/21/2019
ms.author: joflore
author: MicrosoftGuyJFlo
manager: daveba
ms.reviewer: calebb
ms.collection: M365-identity-device-management
ms.openlocfilehash: 4cedec7f0bd51460796d8138f8d481d2982098f4
ms.sourcegitcommit: 2ec4b3d0bad7dc0071400c2a2264399e4fe34897
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 03/27/2020
ms.locfileid: "74379991"
---
# <a name="how-to-require-mfa-for-access-from-untrusted-networks-with-conditional-access"></a>Как: Требуйте МИД для доступа от ненадежных сетей с условным доступом   

Azure Active Directory (Azure AD) обеспечивает единый вход на устройства, в приложения и службы откуда угодно. Пользователи могут получать доступ к облачным приложениям не только из сети вашей организации, но и из любого ненадежного расположения в Интернете. Общая рекомендация для предоставления доступа из ненадежных сетей — требовать Многофакторную идентификацию (MFA).

В этой статье содержится информация, необходимая для настройки политики условного доступа, которая требует от МИД доступа из ненадежных сетей. 

## <a name="prerequisites"></a>Предварительные требования

В данной статье предполагается, что вы знакомы с: 

- [Основные концепции](overview.md) условного доступа Azure AD 
- [Лучшие методы](best-practices.md) настройки политик условного доступа на портале Azure

## <a name="scenario-description"></a>Описание сценария

Чтобы управлять балансом между безопасностью и производительностью, при входе из сети вашей организации может быть достаточно требовать ввода пароля. Однако при доступе из расположения в ненадежной сети повышается риск входа незаконными пользователями. Чтобы устранить эту проблему, можно заблокировать доступ из ненадежных сетей. В качестве альтернативы также можете задать требование многофакторной проверки подлинности (MFA), чтобы получить дополнительную уверенность в том, что попытка была сделана законным владельцем учетной записи. 

С помощью условия Azure AD Условный доступ можно решить с помощью единой политики, которая предоставляет доступ: 

- к выбранным облачным приложениям;
- к выбранным пользователям и группам;  
- по требованию многофакторной проверки подлинности; 
- При доступе из: 
   - ненадежного расположения.

## <a name="implementation"></a>Реализация

Задача этого сценария заключается в переводе *доступа из ненадежного местоположения сети* в условие условного доступа. В политике условного доступа можно настроить [состояние местоположений](location-condition.md) для решения сценариев, связанных с местоположениями сети. Условие расположения позволяет выбрать именованные расположения, которые представляют собой логическое группирование диапазонов IP-адресов, стран и регионов.  

Как правило, ваша организация владеет одним или несколькими диапазонами адресов, например, 199.30.16.0 - 199.30.16.15.
Чтобы настроить именованное расположение, сделайте следующее:

- Указание этого диапазона (199.30.16.0/28) 
- Назначьте описательное имя, например **Корпоративная сеть**. 

Вместо того, чтобы пытаться определить ненадежные расположения, вы можете сделать следующее:

- Включить все расположения 

   ![Условный доступ](./media/untrusted-networks/02.png)

- Исключить все надежные расположения. 

   ![Условный доступ](./media/untrusted-networks/01.png)

## <a name="policy-deployment"></a>Развертывание политики

При подходе, изложенном в этой статье, теперь можно настроить политику условного доступа для ненадежных местоположений. Чтобы убедиться, что политика работает должным образом, рекомендуется протестировать ее перед развертыванием в рабочей среде. В идеале для проверки правильности работы новой политики следует использовать тестовый клиент. Дополнительные сведения см. в разделе [Как развертывать новую политику?](best-practices.md#how-should-you-deploy-a-new-policy) 

## <a name="next-steps"></a>Дальнейшие действия

Если вы хотите узнать больше об [What is Conditional Access in Azure Active Directory?](../active-directory-conditional-access-azure-portal.md) условном доступе, см.
