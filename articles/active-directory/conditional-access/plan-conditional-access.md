...
|-|-|
|Выполняется попытка доступа:<br>— К облачному приложению*<br>— Пользователями и группами*<br>Вместе с этим:<br>— Условие 1 (например, за пределами корпоративной сети)<br>— Условие 2 (например, риск входа)|Блокировать доступ к приложению|
|Выполняется попытка доступа:<br>— К облачному приложению*<br>— Пользователями и группами*<br>Вместе с этим:<br>— Условие 1 (например, за пределами корпоративной сети)<br>— Условие 2 (например, риск входа)|Предоставить доступ с ограничениями (условное "И"):<br>— Требование 1 (например, многофакторная проверка подлинности)<br>— Требование 2 (например, соответствие устройств)|
|Выполняется попытка доступа:<br>— К облачному приложению*<br>— Пользователями и группами*<br>Вместе с этим:<br>— Условие 1 (например, за пределами корпоративной сети)<br>— Условие 2 (например, риск входа)|Предоставить доступ с ограничениями (условное "ИЛИ"):<br>— Требование 1 (например, многофакторная проверка подлинности)<br>— Требование 2 (например, соответствие устройств)|
...
Кроме активных политик, следует определить некоторые отключенные политики, которые будут служить дополнительным [средством контроля доступа при сбоях или в аварийном режиме](../authentication/concept-resilient-controls.md). Такое назначение должно быть учтено в стандарте именования, чтобы было проще включить политики во время сбоя. Например: 
...
### <a name="block-access"></a>Блокирование доступа
...
### <a name="require-mfa"></a>Запрос на Многофакторную идентификацию
...
4. Оцените смоделированный вход.

5. Протестируйте политику.
