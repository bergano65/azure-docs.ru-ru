---
title: Блокировка проверки подлинности в Azure Active Directory (Azure AD) с помощью условного доступа | Документация Майкрософт
description: Узнайте, как повысить уровень безопасности, блокируя проверки подлинности с помощью условного доступа Azure AD.
...
ms.locfileid: "67509036"
---
# <a name="how-to-block-legacy-authentication-to-azure-ad-with-conditional-access"></a>Практическое руководство: Блок устаревшей проверки подлинности Azure AD с помощью условного доступа   
...
Если вашей среды будет готов к блок устаревшей проверки подлинности, чтобы улучшить защиту вашего клиента, этой цели можно решить с условным доступом. В этой статье объясняется, как можно настроить политики условного доступа, которые блокируют устаревшей проверки подлинности для вашего клиента.
...
Политики условного доступа применяются после завершения первой двойной проверки подлинности. Таким образом условного доступа не является первой линии защиты для сценариев, таких как атаки типа "отказ в обслуживании" (DoS), но можно использовать сигналы, поступающие из этих событий (например, уровень риска входа в систему, расположение данного запроса и т. д.) для определения доступа.

## <a name="implementation"></a>Реализация

В этом разделе объясняется, как настроить политику условного доступа для проверки подлинности блока. 
...
Azure имеет функцию безопасности, которая блокирует создание этого, так как эта конфигурация нарушает [рекомендации](best-practices.md) для политик условного доступа.
...
Можно выбрать все доступные элементы управления для **других клиентов** условия; тем не менее, взаимодействие с пользователем остается неизменным - заблокировала доступ.

При блокировании проверки подлинности с помощью **других клиентов** условие, можно также задать условие платформы и расположение устройства. Например, если вы только хотите заблокировать устаревшую аутентификацию для мобильных устройств, задайте условие **платформ устройств**, выбрав:
...
- Если вы не знакомы с еще настройке политики условного доступа, см. в разделе [требовать многофакторную Идентификацию для конкретных приложений с Azure Active Directory условным доступом](app-based-mfa.md) пример.
- Дополнительные сведения о поддержке современной проверки подлинности см. в разделе [применение современной проверки подлинности для клиентских приложений Office 2013 и Office 2016](https://docs.microsoft.com/office365/enterprise/modern-auth-for-office-2013-and-2016) 
