...
1. **Добавление фильтров**  >  > **клиентского приложения** выберите все устаревшие протоколы проверки подлинности. Выберите вне диалогового окна фильтрации Блокс, чтобы применить выбранные параметры, и закройте диалоговое окно.
...
Использование [режима "только отчет"](concept-conditional-access-report-only.md) при включении политики для блокировки устаревших методов аутентификации предоставляет организации возможность отслеживать возможное влияние политики.
...
Если заблокировать устаревшие методы аутентификации с помощью условия **Другие клиенты**, вы можете также задать условие платформы и расположения устройства. Например, если вы только хотите заблокировать устаревшую аутентификацию для мобильных устройств, задайте условие **платформ устройств**, выбрав:
...
- [Определение влияния с помощью условного доступа в режиме "только отчет"](howto-conditional-access-report-only.md)
- Если вы плохо знакомы с настройкой политик условного доступа, ознакомьтесь со статьей [Руководство. Защита событий входа с помощью Многофакторной идентификации Azure](app-based-mfa.md).
