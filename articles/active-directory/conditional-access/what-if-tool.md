...
**Условный доступ, что если средство политики** позволяет понять влияние политики условного доступа на рабочую среду. Вместо того, чтобы тестировать политики, выполняя несколько входов вручную, можно воспользоваться данным инструментом, чтобы имитировать вход пользователя. При имитации оценивается влияние входа на политики и создается отчет об имитации. Отчет не включает только примененные политики условного доступа, но также [классических политик](policy-migration.md#classic-policies) если они существуют.    
...
В **инструмент what if условного доступа**, сначала необходимо настроить параметры сценария входа в систему требуется имитировать. К числу этих параметров относятся следующие:
