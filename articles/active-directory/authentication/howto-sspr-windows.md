...
Когда пользователи сбросили свой пароль с экрана входа устройства Windows 10, `defaultuser1` создается временная учетная запись с низкой привилегией. Эта учетная запись используется для обеспечения безопасности процесса сброса пароля. Учетная запись имеет случайно сгенерированный пароль, который не отображается при входе в устройство и будет автоматически удален после сброса пользователем пароля. Несколько `defaultuser` профилей могут существовать, но могут быть безопасно проигнорированы.
