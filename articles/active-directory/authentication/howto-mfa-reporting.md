...
| SUCCESS_PIN_EXPIRED | ПИН-код просрочен и не изменен | Срок действия ПИН-кода пользователя истек, и ему было предложено изменить его, но изменение ПИН-кода не было выполнено. |
...
| SUCCESS_SMS_AUTHENTICATED | Текстовое сообщение с проверкой подлинности | Для двустороннего тестового сообщения пользователь правильно ответил с помощью одноразового секретного кода (OTP) или OTP + PIN. |
...
| SUCCESS_FALLBACK_OATH_CODE_VERIFIED | Резервный OATH-код проверен | Пользователю отказано в проверке подлинности с помощью основного метода многофакторной проверки подлинности, а затем предоставлен допустимый OATH-код для отката. |
| SUCCESS_FALLBACK_SECURITY_QUESTIONS_ANSWERED | Ответы на секретные вопросы безопасности | Пользователю было отказано в проверке подлинности с помощью основного метода многофакторной проверки подлинности, а затем правильно получены ответы на контрольные вопросы. |
| FAILED_PHONE_BUSY | Проверка подлинности уже выполняется | Многофакторная проверка подлинности уже обрабатывает проверку подлинности для этого пользователя.  Часто это вызвано тем, что клиенты RADIUS, отправляющие несколько запросов проверки подлинности во время одного входа. |
...
| FAILED_USER_HUNGUP_ON_US | Пользователь зависла в телефоне | Пользователь ответил на телефон, но затем зависла без нажатия каких либо кнопок. |
| FAILED_INVALID_EXTENSION | Недопустимое расширение | Расширение содержит недопустимые символы.  Допускаются только цифры, запятые, * и #.  Можно также использовать префикс @. |
| FAILED_FRAUD_CODE_ENTERED | Код мошенничества занесен | Пользователь решил сообщить о мошенничестве во время вызова, что привело к запрещенной проверке подлинности и заблокированному номеру телефона.| 
| FAILED_SERVER_ERROR | Не удается разместить вызов | Службе многофакторной идентификации не удалось разместить вызов. |
...
| FAILED_SMS_OTP_INCORRECT | Недопустимое текстовое сообщение OTP | Пользователь указал неправильный одноразовый пароль (OTP) на основе полученного текстового сообщения.  Проверка подлинности запрещена. |
...
| FAILED_PHONE_APP_DENIED | Мобильное приложение отклонено | Пользователь отклонил проверку подлинности в мобильном приложении, нажав кнопку "запретить". |
| FAILED_PHONE_APP_INVALID_PIN | Недопустимый ПИН-код мобильного приложения | При проверке подлинности в мобильном приложении пользователь указал недопустимый ПИН-код. |
| FAILED_PHONE_APP_PIN_NOT_CHANGED | ПИН-код мобильного приложения не изменен | Пользователь не успешно выполнил требуемое изменение ПИН-кода в мобильном приложении. |
| FAILED_FRAUD_REPORTED | Отчет о мошенничестве | Пользователь сообщил о мошенничестве в мобильном приложении. |
...
| FAILED_PHONE_APP_NOTIFICATION_FAILED | Не удалось уведомить мобильное приложение | При попытке отправить уведомление в мобильное приложение на устройстве пользователя произошла ошибка. |
| FAILED_PHONE_APP_INVALID_RESULT | Недопустимый результат мобильного приложения | Мобильное приложение вернуло недопустимый результат. |
...
| FAILED_OATH_CODE_OLD | Неактуальный OATH-код | Пользователь указал OATH-код, который предшествует ранее использованному OATH-коду.  Проверка подлинности запрещена. |
