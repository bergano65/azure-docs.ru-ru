...
Чтобы настроить список разрешенных IP-адресов, `HKLM\SOFTWARE\Microsoft\AzureMfa` перейдите к следующему значению реестра и настройте его:

| Название | Type | Значение по умолчанию | Описание |
| ---- | ---- | ------------- | ----------- |
| IP_WHITELIST | строка | Очистить | Предоставьте список разделенных точкой с запятой IP-адресов. Включите в него IP-адреса компьютеров, где создаются запросы к службе, например NAS- или VPN-сервер. Диапазоны IP-адресов и подсети не поддерживаются. <br><br> Например, *10.0.0.1;10.0.0.2;10.0.0.3*.
