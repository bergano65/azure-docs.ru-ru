---
title: VPN с Azure MFA с использованием расширения NPS - Активный каталог Azure
...
![Вкладка Dial-in в свойствах пользователей Active Directory и Computers](./media/howto-mfa-nps-extension-vpn/image1.png)
...
В этом разделе подробно описаны предпосылки, которые должны быть завершены, прежде чем вы сможете интегрировать МИД с VPN. Прежде чем приступить к работе, следует подготовить приведенные ниже необходимые компоненты:
...
3. В консоли сервера политики сети щелкните правой кнопкой мыши **Сервер сетевых политик (локальный)**, а затем щелкните **Зарегистрировать сервер в Active Directory**. Дважды нажмите кнопку **ОК**.
    ...
    ![Регистрация сервера в опции меню Active Directory](./media/howto-mfa-nps-extension-vpn/image2.png)
...
2. В разделе **Стандартная конфигурация** выберите **RADIUS-сервер для подключений удаленного доступа или VPN**, а затем щелкните **Настройка VPN или удаленного доступа**.
    ...
    ![Настройка сервера RADIUS для Dial-Up или VPN-соединений](./media/howto-mfa-nps-extension-vpn/image3.png)
...
3. В окне **Выберите тип подключения - удаленный доступ или виртуальная частная сеть** выберите **Virtual Private Network Connections** (Подключения к виртуальной частной сети) и нажмите кнопку **Далее**.
    ...
    ![Настройка виртуальных частных сетевых соединений](./media/howto-mfa-nps-extension-vpn/image4.png)
...
5. В окне **Новый RADIUS-клиент** введите понятное имя, разрешаемое имя или IP-адрес VPN-сервера и общий секретный пароль. Этот секретный пароль должен быть длинным и сложным. Запишите его, так как он понадобится в следующем разделе.
    ...
    ![Создание нового окна клиента RADIUS](./media/howto-mfa-nps-extension-vpn/image5.png)

6. Выберите **OK,** а затем выберите **Следующий**.
...
13. В окне **Завершение создания подключений удаленного доступа или виртуальной частной сети и RADIUS-клиентов** нажмите кнопку **Готово**.
    ...
    ![Завершено окно конфигурации](./media/howto-mfa-nps-extension-vpn/image10.png)
...
4. На сервере политики сети в консоли "Сервер сетевых политик (локальный)" разверните элемент **Политики** и выберите **Политики запросов на подключение**. Отобразится политика VPN-подключений, как показано на следующем рисунке.
    ...
    ![Политика запроса на подключение, показывающая политику VPN-соединения](./media/howto-mfa-nps-extension-vpn/image12.png)

5. В **соответствии**с политиками выберите **сетевые политики.** Вы должны увидеть политику VPN-подключений, напоминающую политику, показанную на рисунке ниже.

    ![Сетевые политики, показывающие политику виртуальных частных сетевых подключений](./media/howto-mfa-nps-extension-vpn/image13.png)
...
4. В ** \<названии сервера> (локальное)** окно свойств выберите вкладку **безопасности.**
...
5. На вкладке **Безопасность** в разделе **Поставщик проверки подлинности** щелкните **Проверка подлинности RADIUS**, а затем — **Настройка**.
    ...
    ![Настройка поставщика аутентификации RADIUS](./media/howto-mfa-nps-extension-vpn/image15.png)
...
7. В окне **Добавление RADIUS-сервера** выполните следующие действия.
    ...
    c. В поле **Тайм-аут (секунды)** введите значение **30**.  
...
7. Щелкните **Изменение параметров адаптера**.
    ...
    ![Сеть и центр обмена - Изменение настройки адаптера](./media/howto-mfa-nps-extension-vpn/image18.png)
...
12. В окне **Параметры** выберите **Подключить**.  
    Успешное соединение отображается в журнале безопасности на сервере RADIUS, как Event ID 6272, как показано здесь:

    ![Окно Event Properties, показывающее успешное подключение](./media/howto-mfa-nps-extension-vpn/image21.png)
...
![Зритель событий, показывающий события NPAS](./media/howto-mfa-nps-extension-vpn/image22.png)

## <a name="configure-multi-factor-authentication"></a>Настройка Многофакторной идентификации

Для настройки пользователей для мультифакторной аутентификации см. статьи [Планирование развертывания многофакторной аутентификации Azure на основе облака](howto-mfa-getstarted.md#create-conditional-access-policy) и [настройка учетной записи для двухэтапной проверки](../user-help/multi-factor-authentication-end-user-first-time.md)
...
Создайте новое значение строки, названное _REQUIRE_USER_MATCH в HKLM-SOFTWARE-Microsoft-AzureMfa_, и установите значение *True* или *False.*
...
2. В меню портала Azure выберите **Active Directory Azure**или ищите и выберите **Active Directory Azure** на любой странице.

3. Выберите **Свойства**.

4. Чтобы скопировать идентификатор Azure AD, нажмите кнопку **Копировать**.

    ![Идентификатор каталога Azure AD на портале Azure](./media/howto-mfa-nps-extension-vpn/azure-active-directory-id-in-azure-portal.png)

### <a name="install-the-nps-extension"></a>Установка расширения NPS

Расширение NPS должно быть установлено на сервере, на котором установлена роль "Службы политики сети и доступа" и который действует как сервер RADIUS в инфраструктуре. Не *not* устанавливайте расширение NPS на VPN-сервере.

1. Скачайте расширение NPS из [Центра загрузки Майкрософт](https://aka.ms/npsmfa).

2. Копируйте исполняемый файл *(NpsExtnForAzureMfaInstaller.exe)* на сервер NPS.
...
2. В запросе команды PowerShell введите **cd "c: 'Программные файлы)-Microsoft-AzureMfa-Config"** и выберите Enter.

3. В следующем запросе команды введите **.'AzureMfaNpsExtnConfigSetup.ps1**, а затем выберите Enter. Сценарий проверяет, установлен ли модуль PowerShell для Azure Active Directory. Если он не установлен, сценарий автоматически установит этот модуль.

    ![Запуск сценария конфигурации AzureMfsNpsExtnConfigSetup.ps1](./media/howto-mfa-nps-extension-vpn/image38.png)
...
4. Введите учетные данные администратора Azure AD, включая пароль, и щелкните **Вход**.
    ...
    ![Аутентификации для Azure AD PowerShell](./media/howto-mfa-nps-extension-vpn/image39.png)

5. Вставьте идентификатор клиента, скопированный ранее, в командную строку и нажмите клавишу ВВОД.

    ![Ввод идентификатор каталога Azure AD, скопированный ранее](./media/howto-mfa-nps-extension-vpn/image40.png)
    ...
    ![Окно PowerShell, отображающее сертификат Self-signed](./media/howto-mfa-nps-extension-vpn/image41.png)
...
![Пример запроса МИД на Windows Phone](./media/howto-mfa-nps-extension-vpn/image43.png)
...
Для просмотра успешных событий входа в журналы Windows Event Viewer запишите журнал безопасности Windows на сервере NPS, введя следующую команду PowerShell:
...
![Пример журнала сервера сетевой политики](./media/howto-mfa-nps-extension-vpn/image45.png)
...
![Пример панели журналов «Обозреватель Аут»](./media/howto-mfa-nps-extension-vpn/image46.png)
...
![Пример приложения Shareware IAS parser](./media/howto-mfa-nps-extension-vpn/image49.png)
...
![Анализатор сообщений Майкрософт, показывающий отфильтрованный трафик](./media/howto-mfa-nps-extension-vpn/image50.png)
