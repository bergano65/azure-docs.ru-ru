...
5. Многофакторная идентификация Azure взаимодействует с Azure Active Directory, получает сведения о пользователе и выполняет вторичную проверку подлинности с помощью метода, настроенного пользователем (вызов сотового телефона, текстового сообщения или мобильного приложения).
...
5. В разделе **политики**выберите **политики сети**. Вы должны увидеть политику VPN-подключений, напоминающую политику, показанную на рисунке ниже.
...
4. В окне ** \<свойства> (локальный) имя сервера** выберите вкладку **Безопасность** .
...
2. Скопируйте исполняемый файл программы установки (*NpsExtnForAzureMfaInstaller. exe*) на сервер политики сети.
...
Чтобы использовать этот сценарий, укажите в расширении свои учетные данные администратора Azure Active Directory и идентификатор клиента Azure Active Directory, скопированный ранее. Учетная запись должна находиться в том же клиенте Azure AD, что и вы хотите включить расширение для. Запустите сценарий на каждом NPS-сервере, на котором установлено расширение NPS.

1. Запустите Windows PowerShell от имени администратора.

2. В командной строке PowerShell введите **CD "C:\Program Files\Microsoft\AzureMfa\Config"**, а затем нажмите клавишу ВВОД.
...
3. В следующей командной строке введите **.\AzureMfaNpsExtnConfigSetup.ps1**, а затем нажмите клавишу ВВОД. Сценарий проверяет, установлен ли модуль PowerShell для Azure Active Directory. Если он не установлен, сценарий автоматически установит этот модуль.
    ...
    Если возникла ошибка безопасности из-за TLS, включите TLS 1,2 с помощью `[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12` команды в командной строке PowerShell.
