...
3. Щелкните **Добавить**.
   ...
   ![Окно проверки подлинности IIS сервера многофакторной проверки Подлинности](./media/howto-mfaserver-adfs-2/setup1.png)
...
10. Установите флажок **Требуется сопоставление пользователей многофакторной проверки подлинности**, если все пользователи были или будут импортированы на сервер и пройдут двухфакторную проверку подлинности. Если значительное число пользователей еще не импортировано на сервер или их исключат из двухфакторной проверки подлинности, не устанавливайте флажок.
    ...
    ![Добавьте сервер многофакторной проверки Подлинности веб-сайт на основе форм](./media/howto-mfaserver-adfs-2/manual.png)
...
2. На вкладке "Параметры" установите переключатель **Использовать определенную конфигурацию LDAP**.
   ...
   ![Настройка параметров протокола LDAP для определенных параметров, LDAP](./media/howto-mfaserver-adfs-2/ldap1.png)
...
5. Проверьте LDAP-подключение. Для этого нажмите кнопку **Проверка**.
   ...
   ![Проверить конфигурацию LDAP на сервере MFA](./media/howto-mfaserver-adfs-2/ldap2.png)
...
Убедитесь, что пользователи импортированы из Active Directory на сервер Azure Multi-Factor Authentication. См. в разделе [надежные IP-адреса](#trusted-ips) Если вы хотите разрешить внутренний IP-адресов для двухфакторной проверки подлинности не требуется при входе на веб-сайт с этих расположений.

![Редактор реестра, чтобы настроить параметры компании](./media/howto-mfaserver-adfs-2/reg.png)
...
Убедитесь, что пользователи импортированы из Active Directory на сервер Azure Multi-Factor Authentication. Если вы хотите разрешить внутренних IP-адресов двухфакторная проверка подлинности не требуется, при входе на веб-сайт с этих расположений, см. раздел надежных IP-адресов.
...
4. Введите IP-адрес, диапазон IP-адреса или подсети, в которой должно быть разрешено. Указывая подсеть, выберите соответствующую маску сети и нажмите кнопку **ОК**.

![Настроить надежные IP-адреса для сервера MFA](./media/howto-mfaserver-adfs-2/trusted.png)
