...
16. Нажмите на вкладку **Native Module** и выберите сервер, веб-сайт, под которым работает прокси AD FS (например, "По умолчанию web Site"), или прокси-приложение AD FS (например, "ls" под "adfs"), чтобы включить плагин IIS на нужном уровне.
...
3. Если пользователи вводят свое имя пользователя в формате "domain-username", сервер должен иметь возможность лишить домена от имени пользователя при его создается запрос LDAP. Это можно сделать с помощью параметра реестра.
4. Откройте редактор реестра и перейдите в раздел HKEY_LOCAL_MACHINE/SOFTWARE/Wow6432Node/Positive Networks/PhoneFactor на 64-разрядном сервере. Если на 32-битном сервере, возьмите "Wow6432Node" с пути. Создайте ключ реестра DWORD под названием "UsernameCxz_stripPrefixDomain" и установите значение до 1. После этого Многофакторная идентификация Azure будет защищать прокси-сервер AD FS.
...
8. Нажмите кнопку **ОК**.
9. Нажмите на вкладку **Native Module** и выберите сервер, веб-сайт (например, "Узел по умолчанию") или приложение AD FS (например, "ls" под "adfs"), чтобы включить плагин IIS на нужном уровне.
