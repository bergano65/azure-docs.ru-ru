...
* Все компьютеры, на которых размещается служба прокси-сервера, должны иметь сетевой доступ к следующим конечным точкам:
