...
Если доступна более новая версия программного обеспечения агента DC для защиты паролем Azure AD, то обновление выполняется с помощью последней версии `AzureADPasswordProtectionDCAgentSetup.msi` пакета программного обеспечения. Удаление текущей версии программного обеспечения агента контроллера домена не требуется — установщик выполняет обновление на месте. При обновлении программного обеспечения агента контроллера домена всегда требуется перезагрузка. это связано с поведением Windows в основном режиме. 
