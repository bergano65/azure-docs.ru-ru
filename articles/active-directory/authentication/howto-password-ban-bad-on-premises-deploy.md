...
### <a name="install-the-dc-agent-service"></a>Установка службы агента контроллера домена
   ...
   > [!WARNING]
   > Пример команды msiexec вызывает немедленную перезагрузку. Чтобы избежать этого, используйте `/norestart` флаг.
