---
title: Как запретить ненадежные пароли в Azure AD — Azure Active Directory
...
> [!NOTE]
> Список запрещенных паролей может содержать не более 1000 условий. Он не предназначен для блокирования чрезвычайно больших списков паролей. Чтобы полностью воспользоваться преимуществами настраиваемого списка запрещенных паролей, корпорация Майкрософт рекомендует сначала изучить и понять предполагаемую структуру и использование настраиваемого списка запрещенных паролей (см. [список пользовательских запрещенных](concept-password-ban-bad.md#custom-banned-password-list)паролей), а также алгоритм оценки пароля (см. раздел [как оцениваются пароли](concept-password-ban-bad.md#how-are-passwords-evaluated)).
...
Когда пользователь пытается сбросить пароль на что-то, что было бы заблокировано, он увидит одно из следующих сообщений об ошибке:

* К сожалению, ваш пароль содержит слово, фразу или сочетание символов, которые легко угадать. Попробуйте ввести другой пароль.
* К сожалению, вы не можете использовать этот пароль, так как он содержит слова или символы, заблокированные администратором. Попробуйте ввести другой пароль.
