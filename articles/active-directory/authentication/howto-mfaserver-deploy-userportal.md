---
title: Пользовательский портал для сервера Azure MFA — Azure Active Directory
...
![Страница входа на пользовательский портал сервера MFA](./media/howto-mfaserver-deploy-userportal/portal.png)
...
4. Привяжите сертификат TLS/SSL к сайту в службах IIS.

Если у вас возникли вопросы по настройке сертификата TLS/SSL на сервере IIS, см. статью [Настройка SSL в IIS](https://docs.microsoft.com/iis/manage/configuring-security/how-to-set-up-ssl-on-iis).

Пакет SDK веб-службы должен быть защищен с помощью сертификата TLS/SSL. Для этой цели подходит самозаверяющий сертификат. Импортируйте сертификат в хранилище "Доверенные корневые центры сертификации" учетной записи локального компьютера на веб-сервере пользовательского портала, чтобы он доверял этому сертификату при инициации подключения TLS.
...
Следующие предварительные требования необходимы для установки пользовательского портала на том **же сервере** , что и сервер многофакторной идентификации Azure.
...
* Защитите пользовательский портал с помощью сертификата TLS/SSL.
* Защитите пакет SDK веб-службы многофакторной идентификации Azure с помощью сертификата TLS/SSL.
...
3. Привязка сертификата TLS/SSL к сайту в службах IIS

   > [!NOTE]
   > Этот сертификат TLS/SSL обычно является открытым подписанным сертификатом TLS/SSL.

4. Откройте веб-браузер с любого компьютера и перейдите по URL-адресу, на котором был установлен пользовательский портал `https://mfa.contoso.com/MultiFactorAuth`(например,). Убедитесь, что не отображаются предупреждения или ошибки сертификата.
...
Если сервер, на котором работает сервер Многофакторной идентификации Azure, не подключен к Интернету, необходимо установить пользовательский портал на **отдельном сервере с выходом в Интернет**.
...
* Защитите пользовательский портал с помощью сертификата TLS/SSL.
* Защитите пакет SDK веб-службы многофакторной идентификации Azure с помощью сертификата TLS/SSL.
* Убедитесь, что пользовательский портал может подключаться к пакету SDK веб-службы многофакторной идентификации Azure по протоколу TLS/SSL.
...
3. Привяжите сертификат TLS/SSL к сайту в службах IIS.
...
5. Измените файл Web.Config в блокноте
    ...
    * Найдите значение **https://www.contoso.com/MultiFactorAuthWebServiceSdk/PfWsSdk.asmx** и измените этот URL-адрес заполнителя на URL-адрес пакета SDK Web Service, установленный на шаге 2.
    * Сохраните файл Web.Config и закройте Блокнот.

6. Откройте веб-браузер с любого компьютера и перейдите по URL-адресу, на котором был установлен пользовательский портал `https://mfa.contoso.com/MultiFactorAuth`(например,). Убедитесь, что не отображаются предупреждения или ошибки сертификата.
...
3. Определите, кто должен быть администратором на вкладке **Администраторы** . Можно создать детализированные административные разрешения с помощью флажков и раскрывающихся списков в полях добавить/изменить.
...
![Управление учетной записью сервера MFA с помощью пользовательского портала](./media/howto-mfaserver-deploy-userportal/portalsettings.png)

### <a name="self-service-user-enrollment"></a>Самостоятельная регистрация пользователей

Если вы хотите, чтобы пользователи вошли и зарегистрировались, необходимо выбрать параметр **Разрешить пользователям входить** и **разрешить регистрацию пользователей** на вкладке Параметры. Помните, что выбранные параметры влияют на процесс входа пользователя в систему.
...
Если при проверке подлинности пользователю нужно использовать ПИН-код, ему предлагается создать ПИН-код. После ввода номеров телефонов и ПИН-кода (если используется) пользователь нажимает кнопку **Позвоните мне для проверки подлинности**. Многофакторная идентификация Azure выполняет проверку телефонного звонка для основного номера телефона пользователя. Пользователь должен ответить на звонок, ввести свой ПИН-код (если используется) и нажать кнопку # для перехода к следующему этапу процесса самостоятельной регистрации.

Если в качестве метода проверки подлинности выбрана или предварительно настроена проверка подлинности с помощью текстовых сообщений, пользователю предлагается ввести номер мобильного телефона. Если пользователь должен использовать ПИН-код при проверке подлинности, ему также предлагается ввести ПИН-код.  После ввода номера телефона и ПИН-кода (если используется) пользователь нажимает кнопку **Отправьте мне SMS для проверки подлинности**. Многофакторная идентификация Azure выполняет проверку SMS на мобильном телефоне пользователя. Пользователь получит текстовое сообщение с одноразовым паролем. Затем он отвечает на сообщение, указав этот пароль и ПИН-код (если используется).

![Проверка пользовательского портала с помощью SMS](./media/howto-mfaserver-deploy-userportal/text.png)
...
После завершения активации пользователь нажимает кнопку **Проверить мою подлинность**. Многофакторная идентификация Azure выполняет проверку мобильного приложения пользователя. Пользователь должен ввести свой ПИН-код (если используется) и нажать кнопку «Выполнить проверку подлинности» в своем мобильном приложении для перехода к следующему этапу процесса самостоятельной регистрации.
...
- [Развертывание веб-службы мобильного приложения Azure сервер Многофакторной идентификации](howto-mfaserver-deploy-mobileapp.md)
