---
title: Пользовательский портал для Сервера Azure MFA - Активный каталог Azure
...
![MFA Server Пользовательский портал войти в страницу](./media/howto-mfaserver-deploy-userportal/portal.png)
...
4. Привязать сертификат TLS/SSL к сайту IIS.

Если у вас есть вопросы о настройке сертификата TLS/SSL на сервере IIS, смотрите статью [Как настроить SSL на IIS.](https://docs.microsoft.com/iis/manage/configuring-security/how-to-set-up-ssl-on-iis)

SDK веб-службы должен быть защищен сертификатом TLS/SSL. Для этой цели подходит самозаверяющий сертификат. Импортсертификата в хранилище "Доверенные органы по сертификации корней" на веб-сервере User Portal, чтобы он доверял этому сертификату при интонивании подключения TLS.
...
Для установки пользовательского портала на том **же сервере,** что и на многофакторном сервере аутентификации Azure, требуются следующие предварительные требования:
...
* Защищайте пользовательский портал сертификатом TLS/SSL.
* Защищайте многофакторную аутентификацию Azure SDK с сертификатом TLS/SSL.
...
3. Привязать сертификат TLS/SSL к сайту iIS

   > [!NOTE]
   > Этот сертификат TLS/SSL обычно является публично подписанным сертификатом TLS/SSL.

4. Откройте веб-браузер с любого компьютера и перейдите на `https://mfa.contoso.com/MultiFactorAuth`URL-адрес, где был установлен пользовательский портал (Пример: ). Убедитесь, что не отображаются предупреждения или ошибки сертификата.
...
Если сервер, на котором работает Azure Multi-Factor Authentication Server, не облицовается интернетом, необходимо установить пользовательский портал на **отдельном интернет-сервере.**
...
* Защищайте пользовательский портал сертификатом TLS/SSL.
* Защищайте многофакторную аутентификацию Azure SDK с сертификатом TLS/SSL.
* Убедитесь, что пользовательский портал может подключиться к веб-службе Azure Multi-Factor Authentication Web SDK через TLS/SSL.
...
5. Измените файл Web.Config в блокноте
    ...
    * Найдите **https://www.contoso.com/MultiFactorAuthWebServiceSdk/PfWsSdk.asmx** значение и измените этот URL-адрес заполнителя на URL-адрес Веб-службы SDK, который мы установили в шаге 2.
    * Сохраните файл Web.Config и закройте Блокнот.

6. Откройте веб-браузер с любого компьютера и перейдите на `https://mfa.contoso.com/MultiFactorAuth`URL-адрес, где был установлен пользовательский портал (Пример: ). Убедитесь, что не отображаются предупреждения или ошибки сертификата.
...
3. Определите, кто должен быть администратором на вкладке **Администраторов.** Вы можете создавать гранулированные административные разрешения с помощью флажков и выпадающих в коробках Add/Edit.
...
![Управление учетной записью MFA Server с помощью пользовательского портала](./media/howto-mfaserver-deploy-userportal/portalsettings.png)

### <a name="self-service-user-enrollment"></a>Самостоятельная регистрация пользователей

Если вы хотите, чтобы ваши пользователи зарегистрировались и зарегистрировались, необходимо выбрать **опционы «Разрешить пользователям войти»** и разрешить параметры **регистрации пользователей** под вкладкой «Настройки».
...
Если при проверке подлинности пользователю нужно использовать ПИН-код, ему предлагается создать ПИН-код. После ввода номеров телефонов и ПИН-кода (если используется) пользователь нажимает кнопку **Позвоните мне для проверки подлинности**. Azure Multi-Factor Authentication выполняет проверку телефонного звонка на основной номер телефона пользователя. Пользователь должен ответить на звонок, ввести свой ПИН-код (если используется) и нажать кнопку # для перехода к следующему этапу процесса самостоятельной регистрации.

Если в качестве метода проверки подлинности выбрана или предварительно настроена проверка подлинности с помощью текстовых сообщений, пользователю предлагается ввести номер мобильного телефона. Если пользователь должен использовать ПИН-код при проверке подлинности, ему также предлагается ввести ПИН-код.  После ввода номера телефона и ПИН-кода (если используется) пользователь нажимает кнопку **Отправьте мне SMS для проверки подлинности**. Multi-Factor Authentication выполняет SMS-проверку мобильного телефона пользователя. Пользователь получит текстовое сообщение с одноразовым паролем. Затем он отвечает на сообщение, указав этот пароль и ПИН-код (если используется).

![Проверка портала пользователя с помощью SMS](./media/howto-mfaserver-deploy-userportal/text.png)
...
После завершения активации пользователь нажимает кнопку **Проверить мою подлинность**. Multi-Factor Authentication выполняет проверку мобильного приложения пользователя. Пользователь должен ввести свой ПИН-код (если используется) и нажать кнопку «Выполнить проверку подлинности» в своем мобильном приложении для перехода к следующему этапу процесса самостоятельной регистрации.
...
- [Развертывание многофакторной аутентификации Azure Server Mobile App Web Service](howto-mfaserver-deploy-mobileapp.md)
