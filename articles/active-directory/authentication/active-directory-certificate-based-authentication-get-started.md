---
title: Проверка подлинности на основе сертификатов - Активный каталог Azure
...
- Проверка подлинности на основе сертификатов (CBA) поддерживается только для федеративных сред для браузерных приложений, нативных клиентов, использующих современную аутентификацию (ADAL) или MSAL библиотеки. Единственным исключением является решение Exchange Active (EAS) для Exchange Online (EXO), которое можно использовать для федеративных и управляемых учетных записей.
...
>[!IMPORTANT]
>Максимальный размер CRL для успешной загрузки и кэша Azure составляет 20 МБ, а время, необходимое для загрузки CRL, не должно превышать 10 секунд.  Если Azure Active Directory не может загрузить CRL, аутентификация на основе сертификатов с использованием сертификатов, выданных соответствующим ЦС, выйдет из строя. Рекомендации по обеспечению того, чтобы файлы CRL находятся в пределах ограничений по размеру, должны держать срокслужбы сертификата в разумных пределах и очищать сертификаты с истекшим сроком действия. 
