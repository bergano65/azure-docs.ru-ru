...
![Отчетность о SSPR с использованием журналов аудита в Azure AD][Reporting]
...
4. Выберите **пользователей** из раздела «Управление».
5. Выберите **журналы аудита** из лезвия **пользователей.** Здесь отображаются все события аудита, выполняемые в отношении всех пользователей в каталоге. Чтобы просмотреть все связанные с паролем события, это представление можно фильтровать.
6. Из меню **фильтра** в верхней части панели выберите список **выпадающих** служб и измените его на тип **службы управления паролем самообслуживания.**
...
Если вы участвуете в публичном предварительном просмотре конвергентной регистрации, информация о активности пользователей в журналах аудита будет найдена в**методах проверки** **безопасности.** > 
...
* [Разблокировка учетной записи пользователя (самообслуживания)](#activity-type-unlock-a-user-account-self-service): указывает на то, что пользователь успешно разблокировал свою учетную запись Active Directory без сброса пароля [с портала сброса пароля Azure AD](https://passwordreset.microsoftonline.com) с помощью функции Active Directory разблокировки учетной записи без сброса.
...
* **Субъект действия**. Администратор, сбросивший пароль от имени другого пользователя или администратора. Должен быть администратором паролей, администратором пользователя или администратором службы поддержки.
