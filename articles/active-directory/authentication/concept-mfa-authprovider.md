...
Поставщики проверки подлинности можно найти в **портал Azure** > **Azure Active Directory** > **Безопасность** > **MFA** > **поставщиков**. Щелкните список поставщиков, чтобы просмотреть сведения и конфигурации, связанные с этим поставщиком.
...
Серверы Azure MFA, связанные с поставщиками, потребуется активировать повторно с помощью учетных данных, созданных в разделе **портал Azure** > **Azure Active Directory** > **Security** > **MFA** > **Параметры сервера**. Перед повторной активацией необходимо удалить следующие файлы из каталога `\Program Files\Multi-Factor Authentication Server\Data\` на серверах Azure MFA в вашей среде:
...
Убедившись в том, что все параметры перенесены, можно перейти к **портал Azure** > **Azure Active Directory** > **Безопасность** > **MFA** > **providers** и нажать кнопку с многоточием **...** и выбрать **Удалить**.
