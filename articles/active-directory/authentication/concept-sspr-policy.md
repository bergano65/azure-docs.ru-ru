...
| Истечение срока действия пароля (разрешить срок действия пароля никогда не истекает) |<ul><li>Значение по умолчанию: **false** (указывает, что у пароля есть Дата окончания срока действия).</li><li>Это значение можно настроить для каждой учетной записи пользователя с помощью командлета `Set-MsolUser`.</li></ul> |
