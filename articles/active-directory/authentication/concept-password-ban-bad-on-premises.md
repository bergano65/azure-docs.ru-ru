...
* Служба агента контроллера домена защиты паролем Azure AD получает запросы на подтверждение пароля из библиотеки DLL фильтрации паролей агента контроллера домена, обрабатывает их с помощью доступной на данный момент политики паролей и возвращает результат (выполнено/сбой). Эта служба отвечает за периодический (ежечасный) вызов прокси-службы защиты паролем Azure AD для получения новых версий политики паролей. Связь между агентом контроллера домена службы защиты паролем Azure AD и службой прокси-сервера защиты паролем Azure AD осуществляется с помощью RPC (удаленного вызова процедур) по протоколу TCP. После получения новые политики хранятся в папке sysvol, где они реплицируются с другими контроллерами домена. Служба агента контроллера домена также отслеживает папку sysvol на предмет изменений в случае записи в другие контроллеры домена новых политик паролей. Если уже имеется подходящая недавняя политика,новые запросы на скачивание политики будут пропущены.
