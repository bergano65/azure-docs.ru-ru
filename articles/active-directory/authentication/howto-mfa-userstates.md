...
> [!IMPORTANT]
> Включение Azure MFA через политику условного доступа не изменит состояние пользователя. Не выводятся оповещения о недоступности пользователей. Условный доступ не изменяет состояние. **Организации не должны включать и не применять пользователей, если они используют политики условного доступа.**

| Состояние | Description | Затронутые приложения, не использующие браузер | Затронутые приложения, использующие браузер | Затронутая современная аутентификация |
|:---:| --- |:---:|:--:|:--:|
