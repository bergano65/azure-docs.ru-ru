...
Не переводите пользователей непосредственно в состояние *Применено*. Если сделать это, приложения, не использующие браузер, перестанут работать, так как пользователь не прошел регистрацию в службе "Многофакторная идентификация Azure" и не получил [пароль приложения](howto-mfa-app-passwords.md).
