...
> [!IMPORTANT]
>
> Убедитесь, что пользователи успешно зарегистрировались для многофакторной аутентификации Azure. Если ранее пользователи зарегистрировались только для сбросить паролем самообслуживания (SSPR), *StrongAuthenticationMethods* включен для их учетной записи. Многофакторная аутентификация Azure обеспечивается при настройке *StrongAuthenticationationMethods,* даже если пользователь зарегистрировался только для SSPR.
>
> Комбинированная регистрация безопасности может быть включена, что настраивает SSPR и Azure Multi-Фактор аутентификации в то же время. Для получения дополнительной информации в [каталоге Azure Active Directory см.](howto-registration-mfa-sspr-combined.md)
>
> Вы также можете [заставить пользователей перерегистрировать методы проверки подлинности,](howto-mfa-userdevicesettings.md#manage-user-authentication-options) если они ранее только включены SSPR.
