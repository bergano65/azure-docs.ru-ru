...
1. Алгоритм шифрования пароля для обмена данными между клиентом RADIUS (VPN, сервером Netscaler или другим) и серверами NPS.
   ...
   - УВЕДОМЛЕНИЯ о телефонных звонках и мобильном приложении **CHAPV2** и **EAP.**
      ...
      >
      > Кроме того, если ваш VPN клиент UX поддерживает поле ввода и вы настроили политику сетевого доступа - аутентификация может быть успешной, однако ни один из атрибутов RADIUS, настроенных в сетевой политике, не будет применяться ни к устройству сетевого доступа, как сервер RRAS, ни VPN клиента. В результате, VPN клиент может иметь больше доступа, чем хотелось бы или меньше, чтобы не получить доступ.
      >
