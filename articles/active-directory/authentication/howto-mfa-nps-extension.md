...
4. **Azure MFA** взаимодействует с Azure Active Directory для получения сведений о пользователе и выполняет дополнительную проверку подлинности по тому методу, который настроен для этого пользователя.
...
Кроме того, для выполнения [установки адаптера с помощью предоставленного скрипта PowerShell](#run-the-powershell-script) требуется подключение к следующим URL-адресам.
...
1. Алгоритм шифрования пароля для обмена данными между клиентом RADIUS (VPN, сервером Netscaler или другим) и серверами NPS.
   - **PAP** поддерживает все методы проверки подлинности Azure MFA в облаке: телефонный звонок, одностороннее текстовое сообщение, уведомление мобильного приложения, аппаратный токен OATH и код проверки мобильного приложения.
   - **CHAPV2** и **EAP** поддерживают телефонный звонок или уведомление мобильного приложения.
      ...
      >
      > Кроме того, если ваш пользовательский интерфейс VPN-клиента поддерживает поле ввода и настроена политика сетевого доступа, проверка подлинности может быть выполнена успешно, однако ни один из атрибутов RADIUS, настроенных в политике сети, не будет применен ни к устройству сетевого доступа, такому как сервер RRAS, ни к VPN-клиенту. В результате VPN-клиент может получить больший уровень доступа, чем это необходимо, или меньший, вплоть до отсутствия доступа.
      >
...
3. [Создайте политику условного доступа](howto-mfa-getstarted.md#create-conditional-access-policy) для включения многофакторной проверки подлинности в тестовой учетной записи.
...
Чтобы при обновлении существующей установки расширения NPS не выполнялась перезагрузка базового сервера, выполните следующие действия.
...
### <a name="microsoft-azure-government-additional-steps"></a>Дополнительные действия для пользователей Microsoft Azure для государственных организаций

Для клиентов, использующих облако Azure для государственных организаций, на каждом сервере NPS необходимо выполнить следующие дополнительные действия по настройке.

> [!IMPORTANT]
> Настраивайте эти параметры реестра, только если вы являетесь клиентом Azure для государственных организаций.

1. Если вы являетесь клиентом Azure для государственных организаций, откройте **редактор реестра** на сервере NPS.
1. Перейдите на страницу `HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\AzureMfa`. Задайте следующие значения разделов реестра.
...
В выпуске 1.0.1.32 расширения NPS теперь поддерживается чтение нескольких сертификатов. Эта возможность упрощает обновление сертификатов до истечения срока их действия. Если в вашей организации работает предыдущая версия расширения NPS, следует выполнить обновление до версии 1.0.1.32 или более поздней версии.

Сертификаты, созданные скриптом `AzureMfaNpsExtnConfigSetup.ps1`, действительны в течение 2 лет. ИТ-специалисты организации должны отслеживать срок действия сертификатов. Сертификаты для расширения NPS размещаются в хранилище сертификатов локального компьютера в личной папке и выдаются идентификатору клиента, предоставленному в скрипт.
...
- Расширение NPS использует имя участника-пользователя из локального каталога Active Directory и по нему определяет пользователя MFA Azure, для которого будет выполняться дополнительная проверка подлинности. Для этого расширения нельзя настроить использование других идентификаторов, например альтернативного имени для входа или настраиваемых полей Active Directory. Дополнительные сведения см. в статье [Параметры расширенной конфигурации расширения NPS для Многофакторной идентификации](howto-mfa-nps-extension-advanced.md).
...
Самозаверяющие сертификаты, созданные скриптом *AzureMfaNpsExtnConfigSetup.ps1*, тоже имеют срок действия в два года. При проверке установки сертификата также следует убедиться, что срок действия сертификата не истек.
...
Проверьте, не истек ли срок действия пароля. Расширение NPS не поддерживает изменение паролей при входе в систему. Обратитесь за дополнительной помощью к ИТ-специалистам вашей организации.
...
Дополнительные инструкции по устранению неполадок и возможные решения см. в статье [Устранение ошибок, связанных с расширением NPS для многофакторной проверки подлинности Azure](howto-mfa-nps-extension-errors.md).

## <a name="next-steps"></a>Дальнейшие действия

- [Общие сведения о сервере политики сети и его настройка в Windows Server](https://docs.microsoft.com/windows-server/networking/technologies/nps/nps-top)
