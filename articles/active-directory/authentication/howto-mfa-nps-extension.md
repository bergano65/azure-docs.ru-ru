...
4. **Azure MFA** взаимодействует с Azure Active Directory для получения сведений о пользователе и выполняет дополнительную проверку подлинности с помощью метода проверки, настроенного для пользователя.
...
1. Алгоритм шифрования пароля для обмена данными между клиентом RADIUS (VPN, сервером Netscaler или другим) и серверами NPS.
   ...
   - **CHAPV2** и **EAP** поддерживают телефонные звонки и уведомления мобильного приложения.
      ...
      >
      > Кроме того, если ваш интерфейс VPN-клиента поддерживает поле ввода и настроена политика сетевого доступа, проверка подлинности может быть выполнена успешно, однако ни один из атрибутов RADIUS, настроенных в политике сети, не будет применяться ни к сетевому устройству, например к серверу RRAS, ни к VPN-клиенту. В результате VPN-клиент может иметь больше доступа, чем требуется или меньше, чем без доступа.
      >
...
1. Войдите в систему [https://aka.ms/mfasetup](https://aka.ms/mfasetup) с помощью тестовой учетной записи.
...
> [!IMPORTANT]
>
> Убедитесь, что пользователи успешно зарегистрировались для многофакторной идентификации Azure. Если пользователи ранее регистрировались только для самостоятельного сброса пароля (SSPR), для их учетной записи включена *стронгаусентикатионмесодс* . Многофакторная идентификация Azure применяется при настройке *стронгаусентикатионмесодс* , даже если пользователь зарегистрирован только для SSPR.
>
> Можно включить объединенную регистрацию безопасности, которая одновременно настраивает SSPR и многофакторную идентификацию Azure. Дополнительные сведения см. [в разделе Включение Объединенной регистрации сведений о безопасности в Azure Active Directory](howto-registration-mfa-sspr-combined.md).
>
> Вы также можете [принудительно заставить пользователей повторно регистрировать методы проверки подлинности](howto-mfa-userdevicesettings.md#manage-user-authentication-options) , если они ранее включали только SSPR.
...
- Предоставьте доступ к закрытому ключу сертификата сетевому пользователю.
...
### <a name="microsoft-azure-government-additional-steps"></a>Microsoft Azure для государственных организаций дополнительных действий

Для клиентов, использующих облако Azure для государственных организаций, необходимо выполнить следующие дополнительные действия по настройке на каждом сервере политики сети.

1. Откройте **редактор реестра** на сервере NPS.
1. Перейдите на страницу `HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\AzureMfa`. Задайте следующие значения ключей:
...
1. Перезапустите службу NPS для каждого сервера NPS.
    ...
    Для минимального воздействия переведите каждый сервер политики сети из ротации балансировки сетевой нагрузки по одному за раз и дождитесь завершения всех соединений.
...
- [Обзор и настройка сервера политики сети в Windows Server](https://docs.microsoft.com/windows-server/networking/technologies/nps/nps-top)
