...
Кроме того, для выполнения требуется подключение к следующим URL-адресам [установки адаптера, используя предоставленный сценарий PowerShell](#run-the-powershell-script)
...
1. Алгоритм шифрования пароля для обмена данными между клиентом RADIUS (VPN, сервером Netscaler или другим) и серверами NPS.
   ...
   - **CHAPV2** и **EAP** поддерживают телефонный звонок или уведомление мобильного приложения.
      ...
      >
      > Кроме того Если VPN-клиента UX поддерживает поле ввода и вы настроили политику доступа к сети - проверки подлинности может быть выполнена успешно, однако ни один из атрибутов RADIUS, настроенные в политике сети будут применяться к ни сетевое устройство, как и RRAS-сервере, а также VPN-клиента. Таким образом VPN-клиент может иметь более высокий уровень доступа, чем требуемой или меньше, нет доступа.
      >
...
3. [Создание политики условного доступа](howto-mfa-getstarted.md#create-conditional-access-policy) требовать многофакторную проверку подлинности для тестовой учетной записью.
...
При обновлении существующего расширения NPS установить, чтобы избежать перезагрузки базового сервера выполните следующие действия:
...
Если истек срок действия предыдущего сертификата компьютера, и был создан новый сертификат, следует удалить просроченные сертификаты. Наличие просроченные сертификаты могут вызвать проблемы с расширением NPS запуск.
...
С выпуском теперь поддерживается 1.0.1.32 расширения NPS, чтение несколько сертификатов. Эта возможность поможет упростить последовательного обновления сертификатов до их истечения. Если ваша организация работает под управлением предыдущей версии расширения NPS, следует обновить до версии 1.0.1.32 или более поздней версии.

Сертификаты, созданные `AzureMfaNpsExtnConfigSetup.ps1` скрипт действительны в течение 2 года. ИТ-организациям необходимо отслеживать сертификаты для истечения срока действия. Сертификаты для расширения NPS, помещаются в хранилище сертификатов локального компьютера в разделе Personal и выдан для идентификатора клиента предоставляются в скрипт.

Когда сертификат приближается Дата истечения срока действия, заменить его должен будет создан новый сертификат.  Этот процесс выполняется, выполнив `AzureMfaNpsExtnConfigSetup.ps1` еще раз и сохранение один и тот же идентификатор клиента при появлении запроса. Этот процесс следует повторить на каждом NPS-сервере в вашей среде.
...
Следующий скрипт можно найти в коллекции TechNet для выполнения основных работоспособности Проверьте действия при устранении неполадок расширения NPS.
...
Самозаверяющие сертификаты, созданные с *AzureMfaNpsExtnConfigSetup.ps1* скрипт также иметь срок действия два года. При проверке, что сертификат установлен, следует также проверить, что сертификат не истек.
...
Проверьте, не истек ли срок действия пароля. Расширение NPS не поддерживает изменение паролей при входе в систему. За дополнительной помощью обратитесь к ИТ-специалистам вашей организации.
...
### <a name="why-is-authentication-not-working-despite-a-valid-certificate-being-present"></a>Почему проверка подлинности не работает, несмотря на действительный сертификат, они отсутствуют?

Если истек срок действия предыдущего сертификата компьютера, и был создан новый сертификат, следует удалить просроченные сертификаты. Наличие просроченные сертификаты могут вызвать проблемы с расширением NPS запуск.

Чтобы проверить, если у вас есть действительный сертификат, проверьте Store сертификатов локального компьютера учетной записи, с помощью консоли MMC и убедитесь, что сертификат не прошел ее Дата окончания срока действия. Для создания вновь действительный сертификат, повторно выполните шаги в разделе "[запустите сценарий PowerShell](#run-the-powershell-script)"
...
### <a name="additional-troubleshooting"></a>Дополнительные настройки

Дополнительные рекомендации и возможные решения для устранения можно найти в статье [Устранение ошибок с расширением NPS для многофакторной идентификации Azure](howto-mfa-nps-extension-errors.md).
