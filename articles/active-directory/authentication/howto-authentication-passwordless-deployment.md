...
ms.locfileid: "77505604"
---
# <a name="complete-a-passwordless-authentication-deployment-in-azure-active-directory"></a>Завершение развертывания проверки подлинности с незащищенным паролем в Azure Active Directory

Большая часть кибератакам начинается с имени пользователя и пароля, заимствованных у пользователя в Организации. Организации пытаются оценить угрозу, попросив пользователей использовать один из следующих подходов:
...
- Улучшенное взаимодействие с пользователем. Предоставление пользователям удобного способа доступа к данным из любого места и предоставление простого доступа к приложениям и службам, таким как Outlook, OneDrive или Office, на мобильных устройствах.
- Надежная аналитика. Получайте ценные сведения о действиях без пароля пользователей с надежным протоколированием и аудитом.

При использовании без пароля пароль заменяется на то, что у вас есть, или что-нибудь известно. Например, в Windows Hello для бизнеса — биометрический жест, например лицо или отпечаток, или ПИН-код для конкретного устройства, который не передается по сети.
...
- Вход с использованием ключа безопасности с ключами [безопасности FIDO2](https://docs.microsoft.com/azure/security/fundamentals/ad-passwordless) особенно удобен для пользователей, которые входят на общие компьютеры, например киоски, в ситуациях, когда использование телефонов ограничено и для привилегированных удостоверений.
...
Чтобы включить вход в Windows 10 с помощью ключей безопасности FIDO2, необходимо включить функцию поставщика учетных данных в Windows 10 одним из следующих способов:
...
| Пользователь может получить доступ к приложению с помощью входа на телефоне | Пользователь проходит по телефонному потоку входа и приступает к назначенному приложению. |
