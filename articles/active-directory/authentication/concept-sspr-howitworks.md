---
title: Самообслуживание пароль сбросить глубокое погружение - Azure Активный каталог
...
1. Пользователь открывает ссылку **Не удается получить доступ к своей учетной записи** или переходит непосредственно по адресу [https://aka.ms/sspr](https://passwordreset.microsoftonline.com).
   ...
   * Для просмотра портала сбросить пароля на другом локализованном языке приложение "?mkt" в конце URL-адреса [https://passwordreset.microsoftonline.com/?mkt=es-us](https://passwordreset.microsoftonline.com/?mkt=es-us)сбросить пароль с примером, который следует за локализацией на испанский .
...
![Выбор методов аутентификации на портале Azure][Authentication]
...
Пользователи не имеют возможности зарегистрировать свое мобильное приложение при регистрации для сбросить пароль самообслуживания с [https://aka.ms/ssprsetup](https://aka.ms/ssprsetup). Пользователи могут зарегистрировать [https://aka.ms/mfasetup](https://aka.ms/mfasetup)свое мобильное приложение по адресу, или в новом предварительном просмотре регистрации безопасности по адресу [https://aka.ms/setupsecurityinfo](https://aka.ms/setupsecurityinfo).
...
> [!IMPORTANT]
> Приложение аутентификаторнейте не может быть выбрано в качестве единственного метода проверки подлинности при настройке политики 1-gate. Аналогичным образом, приложение аутентификатора и только один дополнительный метод не могут быть выбраны при настройке политики 2-гейта.
> Затем при настройке политикSSPR, которые включают приложение аутентификатора в качестве метода, при настройке политики «1-ворота» следует выбрать по крайней мере два дополнительных метода, и при настройке политики «2-ворота» следует выбрать по крайней мере два дополнительных метода.
> Причина этого требования заключается в том, что текущий опыт регистрации SSPR не включает в себя возможность регистрации приложения аутентификатора. Возможность регистрации приложения аутентификатора включена в новую [конвергентную регистрацию для сбросить пароля самообслуживания и многофакторную аутентификацию Azure (публичный предварительный просмотр).](concept-registration-mfa-sspr-converged.md)
> Разрешение политик, которые используют только приложение аутентификатора (для политик 1-ворот) или приложение аутентификатора и только один дополнительный метод (для политик 2-гейтников), может привести к тому, что пользователи будут заблокированы от регистрации для SSPR до тех пор, пока они не будут настроены на использование нового опыт регистрации.
...
Если обязательная регистрация отключена, пользователи могут выполнить регистрацию вручную. Они могут [https://aka.ms/ssprsetup](https://aka.ms/ssprsetup) посетить или выбрать **Регистр для сбросить пароль** ссылку под вкладкой **профиля** в панели доступа.
...
>
> Если они уже выполнили вход, то это прерывание не нарушает подключение пользователей.
...
![Проверка списания пароля включена и работает][Writeback]
...
* Если установить **нет,** то пользователи могут выполнять только комбинированный сбросить пароль и разблокировать учетную запись операции.

### <a name="on-premises-active-directory-password-filters"></a>Фильтры паролей локальной службы Active Directory

Самостоятельный сброс пароля в Azure AD выполняется аналогично сбросу пароля, инициированному администратором в Active Directory. Используя фильтр пароля сторонних производителей для обеспечения пользовательских правил для паролей и имея необходимость его проверки во время самостоятельного сброса пароля в Azure AD, убедитесь, что стороннее решение для фильтрации паролей настроено на применение в сценарии сброса пароля администратором. [Защита паролем Azure AD для Windows Server Active Directory](concept-password-ban-bad-on-premises.md) поддерживается по умолчанию.
