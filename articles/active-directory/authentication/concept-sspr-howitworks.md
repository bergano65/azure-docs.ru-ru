...
1. Пользователь открывает ссылку **Не удается получить доступ к своей учетной записи** или переходит непосредственно по адресу [https://aka.ms/sspr](https://passwordreset.microsoftonline.com).
   ...
   * Чтобы просмотреть портал сброса паролей на другом локализованном языке, добавьте "? МКТ =" в конец URL-адреса сброса пароля, используя следующий пример: локализация на Испанский [https://passwordreset.microsoftonline.com/?mkt=es-us](https://passwordreset.microsoftonline.com/?mkt=es-us) .
...
Пользователи не имеют возможности регистрировать свое мобильное приложение при регистрации для самостоятельного сброса пароля из [https://aka.ms/ssprsetup](https://aka.ms/ssprsetup) . Пользователи могут зарегистрировать свое мобильное приложение по адресу [https://aka.ms/mfasetup](https://aka.ms/mfasetup) или в новой предварительной версии регистрации сведений о безопасности по адресу [https://aka.ms/setupsecurityinfo](https://aka.ms/setupsecurityinfo) .
...
> Разрешение политик, которые используют только приложение для проверки подлинности (для политик с 1 шлюзом) или приложение для проверки подлинности и только один дополнительный метод (для политик с 2 шлюзами), может привести к тому, что пользователи будут заблокированы для SSPR до тех пор, пока они не будут настроены для использования новой функции регистрации.
...
Если обязательная регистрация отключена, пользователи могут выполнить регистрацию вручную. Они могут либо посетить, [https://aka.ms/ssprsetup](https://aka.ms/ssprsetup) либо выбрать ссылку **Регистрация для сброса пароля** на вкладке **профиль** на панели доступа.
