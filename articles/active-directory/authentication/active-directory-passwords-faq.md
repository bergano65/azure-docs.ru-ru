...
* [Вопросы о регистрации сна пароля](#password-reset-registration)
...
* **Вопрос. Если включить сброс пароля для группы, а затем понадобится включить сброс пароля для всех пользователей, придется ли повторно регистрировать пользователей?**
  ...
  > **A:** Нет. Для пользователей, которые заполнили данные для проверки подлинности, повторная регистрация не требуется.
  >
  >
* **В: Могу ли я определить данные сброса пароля от имени пользователей?**

  > **A:** Да, вы можете сделать это с Помощью Azure AD Connect, PowerShell, [портала Azure](https://portal.azure.com)или [в центре админ-центра Microsoft 365.](https://admin.microsoft.com) Дополнительные сведения см. в статье [Развертывание сброса пароля без регистрации пользователя](howto-sspr-authenticationdata.md).
  >
  >
...
* **Вопрос. Могут ли пользователи зарегистрировать данные так, чтобы другие пользователи эти данные не видели?**
  ...
  > **A:** Да. Когда пользователь регистрирует данные на портале регистрации для сброса паролей, эти данные сохраняются в специальных частных полях, которые могут просматривать только глобальные администраторы и сам пользователь.
  >
  >
* **В: Должны ли мои пользователи быть зарегистрированы, прежде чем они смогут использовать сброс пароля?**

  > **A:** Нет. Если вы от их имени укажете достаточно сведений для проверки подлинности, пользователям не придется регистрировать эти сведения самостоятельно. Для работы функции сброса паролей требуется, чтобы правильно отформатированные данные хранились в соответствующих полях каталога.
  >
  >
* **В: Могу ли я синхронизировать или установить телефон проверки подлинности, электронную почту аутентификации или альтернативные поля телефона проверки подлинности от имени моих пользователей?**
...
* **Вопрос. Как на портале регистрации определяется, что именно нужно показывать для того или иного пользователя?**
  ...
  > **Ответ.** На портале регистрации данных для сброса паролей отображаются только те данные, которые вы настроили для пользователей Эти параметры можно найти в разделе **Политика снаустановки пароля пользователя** на вкладке **Настройка** каталога. Например, если вы не включаете вопросы безопасности, пользователи не смогут зарегистрироваться для этой опции.
  >
  >
...
* **Вопрос. Есть ли ограничения на количество попыток сброса пароля за короткий промежуток времени?**
  ...
  >
  >
* **В: Как долго я должен ждать, чтобы получить электронную почту, SMS или телефонный звонок от сбросить пароль?**
  ...
  > * Убедитесь, что данные аутентификации в каталоге правильно отформатированы, например, No1 4255551234 или *contoso.com пользователя.\@* 
* **В: Какие языки поддерживаются сбросить пароль?**
  ...
  >
  >
* **В: Какие части опыта сброса пароля заклеймены, когда я устанавливаю организационные элементы брендинга в настройке вкладки каталога?**

  > **Ответ.** Логотип вашей организации будет отображаться на портале сброса паролей. Вы также сможете настроить ссылку для связи с администратором, указав нужный вам электронный или URL-адрес. Любое письмо, отправленное сбросом пароля, включает логотип, цвета и имя организации в теле письма и настраивается из настроек для данного конкретного имени.
  >
  >
* **Вопрос. Куда именно нужно направлять пользователей для сброса паролей?**

  > **A:** Попробуйте некоторые из предложений в нашей статье [о развертывании SSPR.](howto-sspr-deployment.md#plan-communications)
  >
  >
* **Вопрос. Могу ли я пользоваться этой страницей на мобильном устройстве?**
  ...
  >
  >
* **В: Поддерживаете ли вы разблокировку локальных учетных записей Active Directory при сбросить пароли?**

  > **A:** Да. Когда пользователь сбрасывает свой пароль, если запись пароля была развернута через Azure AD Connect, учетная запись этого пользователя автоматически разблокируется при сбросе пароля.
  >
  >
* **В: Как я могу интегрировать сброс пароля непосредственно в настольный интерфейс пользователя?**
...
* **Вопрос. Какова допустимая длина ответов на контрольные вопросы?**
  ...
  >
  >
* **В: Отклонены ли дублирующиеся ответы на вопросы безопасности?**
...
* **Вопрос. Может ли пользователь зарегистрировать несколько одинаковых контрольных вопросов?**
  ...
  > **A:** Нет. Если пользователь зарегистрировал определенный вопрос, он не сможет его еще раз зарегистрировать.
  >
  >
* **В: Можно ли установить минимальный лимит вопросов безопасности для регистрации и сбросить?**
...
* **Вопрос. Как долго действует одноразовый секретный код, полученный по электронной почте или в SMS?**
  ...
  > **Ответ.** Длительность сеанса сброса пароля — 15 минут. С начала операции сброса пароля у пользователя есть 15 минут на выполнение этой операции. Электронная почта и SMS одноразовый пароль действительны в течение 5 минут во время сеанса сбросить пароль.
  >
  >
...
* **Вопрос. Где пользователи могут изменять свои пароли?**
  ...
  >
  >
* **В: Могут ли мои пользователи быть уведомлены на портале Office по истечении срока действия их собственного пароля?**
...
* **В: Сколько времени требуется для того, чтобы данные отображались в отчетах по управлению паролями?**
  ...
  >
  >
* **В: Как отфильтровать отчеты об управлении паролями?**
...
* **Вопрос. Каково максимальное число событий, которые хранятся в отчетах об управлении паролями?**
  ...
  >
  >
* **В: Как далеко заходят отчеты по управлению паролями?**
...
* **Вопрос. Есть ли максимальное количество строк, которые могут отображаться в отчетах об управлении паролями?**
  ...
  > **A:** Да. Каждый отчет может вмещать не более 75 000 строк — как в пользовательском интерфейсе, так и в скачанном файле.
  >
  >
...
* **Вопрос. Что происходит во время обратной записи паролей?**
  ...
  >
  >
* **В: Как долго работает списание пароля? Есть ли задержка синхронизации, как есть с синхронизацией хэша паролей?**
...
* **Вопрос. Для каких типов учетных записей работает обратная запись паролей?**
  ...
  >
  >
* **В: Обеспечивает ли списание пароля соблюдение политики паролей моего домена?**

  > **A:** Да. Компонент обратной записи паролей соблюдает настройки срока действия пароля, журнала, сложности, фильтров и другие ограничения, установленные для паролей в локальном домене.
  >
  >
* **В: Безопасно ли списание пароля?  Как я могу быть уверен, что меня не взломают?**
