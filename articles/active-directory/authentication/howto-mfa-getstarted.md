---
title: Рекомендации по развертыванию для многофакторной идентификации Azure
...
В этом руководство по развертыванию показано, как спланировать и затем протестировать развертывание многофакторной идентификации Azure.

Чтобы быстро просмотреть многофакторную идентификацию Azure в действии, а затем вернитесь к анализу дополнительных рекомендаций по развертыванию:
...
6. Выберите, следует ли определять расположение с помощью *диапазонов IP-адресов* или *стран или регионов* .
   1. При использовании *диапазонов IP-адресов*
      1. Решите, следует ли *помечать как надежное расположение*. Вход из надежного расположения снижает риски входа пользователя. Пометьте это расположение как доверенное, если известно, что введенные диапазоны IP-адресов установлены и достоверных в вашей организации.
...
1. На новой вкладке, открытой, перейдите к **параметрам службы**.
...
Запустите эту оболочку PowerShell в окне интегрированной среды сценариев `.PS1` или сохраните в виде файла для локального запуска.
...
1. Выберите **создать политику**.
   ![Создание политики условного доступа для включения MFA для пользователей портал Azure в пилотной группе](media/howto-mfa-getstarted/conditionalaccess-newpolicy.png)
1. Введите понятное имя для политики.
1. В разделе **Пользователи и группы**:
...
1. В разделе **облачные приложения**выберите переключатель **все облачные приложения** .
...
1. Пропустите раздел **Session** .
1. Задайте для переключателя **включить политику** значение **вкл**.
...
Выберите, что произойдет, если пользователи, не зарегистрированные с помощью MFA, попытаются пройти проверку подлинности. Используйте параметр `REQUIRE_USER_MATCH` реестра в пути `HKLM\Software\Microsoft\AzureMFA` реестра, чтобы управлять поведением функции. Этот параметр имеет один параметр конфигурации.
...
| Параметры | Состояние пользователя MFA | Произведенный эффект |
| --- | --- | --- |
...
1. Отправка пользователей и получение пользователям регистрации[https://aka.ms/mfasetup](https://aka.ms/mfasetup)
...
Чтобы увидеть многофакторную идентификацию Azure в действии, выполните следующие действия.
