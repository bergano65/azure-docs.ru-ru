---
title: Интеграция списания с помощью сносов с помощью сноверия паролем ССПР - Активный каталог Azure
description: Получить облачные пароли обратно в локонную инфраструктуру AD
...
* **Отраслевой стандартный протокол TLS**
   ...
   1. Зашифрованный пароль помещается в сообщение HTTPS, которое отправляется по зашифрованному каналу с помощью сертификатов Microsoft TLS/SSL для ретранслятора вашего сервисного автобуса.
...
* **Поддерживаемые операции администрирования:**
   ...
   * Любой сбросить пароля с пароля, инициированного администратором, с [портала Azure](https://portal.azure.com)
...
* **Неподдерживаемые операции пользователей:**
   * Любой конечный пользователь сбросив свой пароль с помощью версии PowerShell 1, версии 2 или API Microsoft Graph
* **Неподдерживаемые операции администрирования:**
   * Любой инициированный администратором пароль с валидса PowerShell версии 1, версии 2 или API Microsoft Graph
   * Любой сбросить пароля с конечным пользователем, инициированный администратором, из [центра администратора Microsoft 365](https://admin.microsoft.com)

> [!WARNING]
> Использование флажка "Пользователь должен изменить пароль при следующем логове" в находящихся в помещениях активных пользователей каталога и компьютеров или Вадминистративном центре Active Directory поддерживается как функция предварительного просмотра Azure AD Connect. Для получения дополнительной информации смотрите статью [«Реализация синхронизации хэша паролей с синхронизацией Azure AD Connect».](../hybrid/how-to-connect-password-hash-synchronization.md)
