...
Можно получить доступ к параметрам, связанных с многофакторной идентификации Azure на портале Azure, перейдя по адресу **Azure Active Directory** > **MFA**.
...
| [Блокировка и разблокировка пользователей](#block-and-unblock-users) | Используется, чтобы заблокировать определенным пользователям возможность получать запросы многофакторной проверки подлинности. Любые попытки выполнить аутентификацию заблокированных пользователей отклоняются автоматически. Блокировка пользователей действует в течение 90 дней. |
| [Предупреждение о мошенничестве](#fraud-alert) | Настройте параметры, относящиеся к пользователям возможность сообщить о мошеннической проверке запросов |
...
>[!NOTE]
>Пароли приложений не работают с политиками условного доступа на основе многофакторной проверки подлинности и современные методы проверки подлинности.
...
> [!NOTE]
> MFA доверенных IP-адресов и условный доступ — именованные расположения работают только с IPv4-адреса.
...
1. Войдите на [портале Azure](https://portal.azure.com).
2. С левой стороны экрана выберите **Azure Active Directory** > **условного доступа** > **именованные расположения**.
