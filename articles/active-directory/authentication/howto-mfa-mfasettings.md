...
## <a name="fraud-alert"></a>Оповещение о мошенничестве
...
2. Выберите **Azure Active Directory** > **входе в систему**. Отчет о мошенничестве теперь является частью стандартного отчета о входе в Azure AD.
...
| --- | --- |
| Проверка подлинности выполнена успешно | Вход успешно проверен. До свидания. |
| Запрос расширения | Благодарим за использование системы проверки входа Майкрософт. Для продолжения нажмите клавишу решетки. |
...
| Повторная проверка подлинности запрещена | Проверка отклонена. |
...
| Запрос расширения после цифр | Если это расширение уже существует, нажмите решетку, чтобы продолжить. |
| Проверка подлинности запрещена | К сожалению, в настоящее время не удается выполнить вход. Повторите попытку позже. |
...
|:--- |:--- |
| Управляемое |**Specific range of IP addresses** (Определенные диапазоны IP-адресов). Администраторы могут задать диапазон IP-адресов, которые могут обходить двухфакторную проверку подлинности применительно к пользователям, которые выполняют вход из интрасети компании.|
