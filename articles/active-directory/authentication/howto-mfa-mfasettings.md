---
title: Настройка многофакторная Идентификация Azure — Azure Active Directory
...
Доступ к параметрам, имеющим отношение к многофакторной проверке подлинности, вы можете получить на портале Azure в разделе **Azure Active Directory** > **MFA**.
...
| [Блокировка и разблокировка пользователей](#block-and-unblock-users) | Блокирует определенных пользователей на (локальном) сервере MFA, чтобы они не получали запросы Многофакторной идентификации. Любые попытки выполнить аутентификацию заблокированных пользователей отклоняются автоматически. Блокировка пользователей действует в течение 90 дней. |
...
Возможность _блокировки и разблокировки пользователей_ используется, чтобы помешать пользователям получать запросы на аутентификацию. Любые попытки выполнить аутентификацию заблокированных пользователей отклоняются автоматически. Блокировка пользователей действует в течение 90 дней. Эта функция относится только к серверу MFA (локальному)
...
4. Выберите **Группа репликации**. Введите имя пользователя для заблокированного пользователя в качестве **username\@domain.com**. Введите комментарий в поле **Причина**.
...
Настройте _предупреждение о мошенничестве_ таким образом, чтобы пользователи могли сообщать о мошенничестве при попытках получить доступ к их ресурсам. Пользователи могут сообщать о попытках мошенничества с помощью мобильного приложения или через свой телефон. Эта функция относится только к серверу MFA (локальному)
...
5. Введите имя пользователя в формате **username\@domain.com**. Введите количество секунд, в течение которых будет работать обход проверки, и причину обхода.
...
|:--- |:--- |
| Управляемые |**Specific range of IP addresses** (Определенный диапазон IP-адресов). Администраторы могут задать диапазон IP-адресов, которые могут обходить двухфакторную проверку подлинности применительно к пользователям, которые выполняют вход из интрасети компании.|
