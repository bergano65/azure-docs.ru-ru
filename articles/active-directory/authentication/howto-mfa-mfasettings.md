...
Доступ к параметрам, имеющим отношение к многофакторной идентификации, можно получить на портале Azure в разделе **Azure Active Directory** > **Безопасность** > **MFA**.
...
| [Блокировка и разблокировка пользователей](#block-and-unblock-users) | Блокирует определенных пользователей, чтобы они не получали запросы многофакторной идентификации. Любые попытки выполнить аутентификацию заблокированных пользователей отклоняются автоматически. Блокировка пользователей действует в течение 90 дней. |
| [Предупреждение о мошенничестве](#fraud-alert) | Настройте возможности, которые будут доступны пользователям для информирования о мошеннических запросах проверки от сервера многофакторной идентификации. |
...
2. Перейдите к разделу **Azure Active Directory** > **Безопасность** > **MFA** > **Блокировка и разблокировка пользователей**.
3. Выберите **Добавить**, чтобы заблокировать пользователя.
4. Выберите **Группа репликации**. Введите имя заблокированного пользователя в формате **имя_пользователя\@domain.com**. Введите комментарий в поле **Причина**.
...
1. Войдите на [портал Azure](https://portal.azure.com) с использованием учетной записи администратора.
2. Перейдите к разделу **Azure Active Directory** > **Безопасность** > **MFA** > **Предупреждение о мошенничестве**.
...
2. Выберите **Azure Active Directory** > **События входа** > **Сведения о проверке подлинности**. Отчет о мошенничестве теперь является частью стандартного отчета о входе в Azure AD и отображается в разделе **Сведения о результатах** с пометкой "Многофакторная идентификация не пройдена, введен код мошенничества".
 
## <a name="notifications"></a>Уведомления

Настройте адреса электронной почты пользователей, которые будут получать по электронной почте предупреждения, в разделе **Azure Active Directory** > **Безопасность** > **Многофакторная идентификация** > **Уведомления**.
...
В США, если не настроен ИД вызывающего абонента MFA, голосовые вызовы от корпорации Microsoft поступают со следующих номеров: + 1 (866) 539 4191, + 1 (855) 330 8653 и + 1 (877) 668 6536. При использовании фильтров нежелательной почты следует исключить эти номера.
...
1. Войдите на [портал Azure](https://portal.azure.com) с использованием учетной записи администратора.
2. Перейдите в раздел **Azure Active Directory** > **Безопасность** > **MFA** > **Однократный обход**.
3. Выберите **Добавить**.
4. При необходимости выберите группу репликации для этого обхода.
5. Введите имя пользователя в формате **имя_пользователя\@domain.com**. Введите количество секунд, в течение которых будет работать обход проверки, и причину обхода.
...
1. Войдите на [портал Azure](https://portal.azure.com) с использованием учетной записи администратора.
2. Перейдите в раздел **Azure Active Directory** > **Безопасность** > **MFA** > **Кеширование правил**.
3. Выберите **Добавить**.
...
Такие настройки, как пароли приложений, надежные IP-адреса, параметры проверки. Кроме того, многие настройки многофакторной проверки подлинности для службы Многофакторной идентификации Azure можно найти в параметрах службы. Параметры службы можно отрыть на портале Azure в разделе **Azure Active Directory** > **Безопасность** > **MFA** > **Приступая к работе** > **Настройка** > **Дополнительные параметры облачной MFA**.
...
Диапазоны доверенных IP-адресов могут быть частными или общедоступными.
...
> [!NOTE]
> Доверенные IP-адреса MFA и именованные расположения для условного доступа могут использоваться только с IPv4-адресами.
...
1. Войдите на [портал Azure](https://portal.azure.com).
2. В левой части экрана выберите **Azure Active Directory** > **Безопасность** > **Условный доступ** > **Именованные расположения**.
