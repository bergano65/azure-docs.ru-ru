...
| Компонент | Описание |
| --- | --- |
| Сервер |Укажите имя узла или IP-адрес сервера, содержащего каталог LDAP.  Также можно указать резервный сервер через точку с запятой. <br>Примечание. Если тип привязки — SSL (TLS), требуется полное имя узла. |
