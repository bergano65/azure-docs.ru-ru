---
title: Интеграция сервера Azure Multi-factor Authentication и Active Directory — Azure Active Directory
...
| Функция | ОПИСАНИЕ |
| --- | --- |
| Использовать Active Directory |Выберите этот параметр для использования Active Directory для импорта и синхронизации.  Это значение по умолчанию. <br>Примечание. Для правильной интеграции с Active Directory присоедините компьютер к домену и войдите с помощью учетной записи домена. |
...
| Использовать заданную конфигурацию LDAP |Выберите параметр «Использовать заданную конфигурацию LDAP» для использования заданных настроек LDAP для импорта и синхронизации. Примечание. Если выбрано использование LDAP, пользовательский интерфейс меняет ссылки с Active Directory на LDAP. |
...
![Настройка фильтрации каталог на сервере MFA](./media/howto-mfaserver-dir-ad/dirint2.png)
...
| Различающееся имя |Введите имя атрибута, который содержит различающееся имя для каждой записи.  В Active Directory это обычно distinguishedName. В других реализациях LDAP может использоваться entryDN или что-либо подобное.  По умолчанию используется distinguishedName. <br><br>Если не существует атрибута, который содержит только различающееся имя, может использоваться атрибут path рекламы.  Фрагмент пути LDAP://\<сервер\>/ будет автоматически удален, и останется только различающееся имя объекта. |
...
![Изменить сопоставление атрибутов каталога на сервере MFA](./media/howto-mfaserver-dir-ad/dirint4.png)
...
![Синхронизацию объектов каталога, сервер многофакторной проверки Подлинности](./media/howto-mfaserver-dir-ad/dirint5.png)
...
## <a name="multi-factor-authentication-servers"></a>Серверы многофакторной проверки подлинности

Дополнительные серверы многофакторной проверки подлинности может быть настроен для использования в качестве резервных RADIUS-прокси, прокси-сервера LDAP или для проверки подлинности IIS. Настройки синхронизации доступны всем агентам. Однако только один из этих агентов может быть запущена служба сервера многофакторной проверки подлинности. На этой вкладке можно выбрать сервера многофакторной проверки подлинности, который должен быть включен для синхронизации.
