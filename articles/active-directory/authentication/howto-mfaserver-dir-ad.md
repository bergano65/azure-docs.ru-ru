...
| Функция | ОПИСАНИЕ |
| --- | --- |
| Server |Укажите имя узла или IP-адрес сервера, содержащего каталог LDAP.  Также можно указать резервный сервер через точку с запятой. <br>Примечание. Если задан тип привязки SSL, требуется указать полное имя узла. |
