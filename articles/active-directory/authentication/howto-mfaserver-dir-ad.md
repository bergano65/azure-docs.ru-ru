---
title: Сервер Azure MFA и активный каталог - Активный каталог Azure
...
![Оторитная конфигурация LDAP в сервере MFA](./media/howto-mfaserver-dir-ad/dirint.png)
...
| Компонент | Описание |
| --- | --- |
| Использовать Active Directory |Выберите этот параметр для использования Active Directory для импорта и синхронизации.  Это параметр по умолчанию. <br>Примечание. Для правильной интеграции с Active Directory компьютер должен быть присоединен к домену, а вы должны войти с помощью учетной записи домена. |
...
| Компонент | Описание |
| --- | --- |
| Сервер |Укажите имя узла или IP-адрес сервера, содержащего каталог LDAP.  Также можно указать резервный сервер через точку с запятой. <br>Примечание: Когда тип Bind является SSL (TLS), требуется полностью квалифицированное хост-имя. |
...
![Настройка фильтрации каталога в сервере MFA](./media/howto-mfaserver-dir-ad/dirint2.png)
...
Атрибуты для каталога при необходимости можно изменить.  Это позволит добавлять настраиваемые атрибуты и выполнять тонкую настройку синхронизации, оставляя только те атрибуты, которые необходимы. Используйте имя атрибута, определенное в схеме каталога, для значения каждого поля атрибута. В таблице ниже приведены дополнительные сведения о каждой функции.

Атрибуты можно вводить вручную и не обязательно выбирать из списка.

![Настройка атрибутов интеграции каталогов в сервере MFA](./media/howto-mfaserver-dir-ad/dirint3.png)
...
| Другой |Введите имя атрибута, который содержит номер другого телефона в записи пользователя.  Значение по умолчанию — пусто. |
...
Чтобы отсеять атрибуты, нажмите **«Оторите»** на вкладке «Атрибуты».  Это создает окно, где можно отсылать атрибуты. Щелкните **...** рядом с любым атрибутом, чтобы открыть окно, в котором можно выбрать, какие атрибуты следует отобразить.

![Отображение атрибутов каталога в MFA Server](./media/howto-mfaserver-dir-ad/dirint4.png)
...
## <a name="multi-factor-authentication-servers"></a>Многофакторные серверы аутентификации

Дополнительные многофакторные серверы аутентификации могут быть настроены в качестве резервного прокси RADIUS, прокси-сервера LDAP или для проверки подлинности IIS. Настройки синхронизации доступны всем агентам. Однако только один из этих агентов может работать с сервером Multi-Factor Authentication. Эта вкладка позволяет выбрать многофакторный сервер аутентификации, который должен быть включен для синхронизации.

![Похожие многофакторные серверы аутентификации](./media/howto-mfaserver-dir-ad/dirint6.png)
