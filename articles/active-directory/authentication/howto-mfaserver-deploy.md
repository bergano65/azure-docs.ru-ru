---
title: Начало работы Azure MFA Server - Активный каталог Azure
...
![Начало работы с MFA Server на месте](./media/howto-mfaserver-deploy/server2.png)</center>
...
2. Найдите и выберите *Azure Active Directory*. Выберите**МИД** **безопасности** > .
3. Под **сервером Manager MFA**, выберите **настройки сервера.**
4. Выберите **Загрузить** и следуйте инструкциям на странице скачивания, чтобы сохранить установщик. 

   ![Скачать MFA Server с портала Azure](./media/howto-mfaserver-deploy/downloadportal.png)
...
4. На экране приветствия мастера настройки установите флажок **Пропустить использование мастера настройки проверки подлинности** и нажмите кнопку **Далее**. Мастер закроется, и сервер запустится.
   ...
   ![Пропустить с помощью мастера конфигурации аутентификации](./media/howto-mfaserver-deploy/skip2.png)
...
> [!NOTE]
> Только глобальные администраторы могут генерировать учетные данные активации на портале Azure.
...
![Шаблоны электронной почты MFA Server в консоли](./media/howto-mfaserver-deploy/email2.png)
...
4. Выделите всех пользователей справа и нажмите кнопку **Импортировать**. Должно появиться всплывающее окно с сообщением о том, что импорт выполнен успешно. Закройте окно импорта.
   ...
   ![Импорт пользователя MFA Server из Active Directory](./media/howto-mfaserver-deploy/import2.png)
