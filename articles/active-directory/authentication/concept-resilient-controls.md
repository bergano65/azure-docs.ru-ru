...
3. Включение политики 2. Убедитесь в том, что пользователи группы SalesContigency не могут получить доступ к Salesforce со своих ноутбуков Windows/Mac, но могут получать доступ со своих мобильных устройств. Убедитесь в том, что SalesAdmin может получить доступ к Salesforce с любого устройства.
...
Вы должны отменить изменения, внесенные вами как часть активированного плана на непредвиденные случаи, после восстановления службы, которая вызвала сбой. 
...
6. Отмените все токены обновления, выпущенные [с помощью PowerShell](https://docs.microsoft.com/powershell/module/azuread/revoke-azureaduserallrefreshtoken?view=azureadps-2.0) для целевого набора пользователей. Отмена всех токенов обновления особо важна для привилегированных учетных записей, используемых во время сбоя, и это заставит их повторно пройти проверку подлинности и получить контроль над восстановленными политиками.
...
1. Если у вас есть исходящий IP-адрес корпоративной сети, вы можете добавить его в качестве доверенных IP-адресов, чтобы включить проверку подлинности только в корпоративной сети.
 2. Если у вас нет перечня исходящих IP-адресов или вам нужно включить доступ внутри и за пределами корпоративной сети, вы можете добавить все адресное пространство IPv4 в качестве доверенных IP-адресов в нотации CIDR (например, 192.168.1.1/24).
