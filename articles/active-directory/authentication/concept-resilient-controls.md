...
* Реализуйте MFA с помощью условного доступа, а не для многопользовательского MFA.
* Устранение блокировок пользователей с помощью нескольких элементов управления условным доступом.
...
1. Подготовьте для каждого пользователя несколько методов проверки подлинности, которые используют разные коммуникационные каналы, например приложение Microsoft Authenticator (онлайн), OATH-токен (генерированный на устройстве) и SMS (телефонный). Приведенный ниже сценарий PowerShell поможет заранее определить, какие дополнительные методы следует зарегистрировать: [скрипт для анализа метода проверки подлинности Azure AD MFA](/samples/azure-samples/azure-mfa-authentication-method-analysis/azure-mfa-authentication-method-analysis/).
...
3. Используйте доверенные устройства через [гибридное присоединение устройств к Azure AD](../devices/overview.md) или [управляемые устройства Microsoft Intune](/intune/planning-guide). Доверенные устройства улучшат взаимодействие с пользователем, поскольку само доверенное устройство может соответствовать строгим требованиям проверки подлинности к политике без вызова MFA для пользователя. Тогда MFA потребуется при регистрации нового устройства и при доступе к приложениям или ресурсам с ненадежных устройств.
...
5. Если вы защищаете доступ VPN с помощью расширения NPS Azure AD MFA, рассмотрите возможность объединения решения VPN в качестве [приложения SAML](../manage-apps/view-applications-portal.md) и определите категорию приложения, как описано ниже. 
...
* Разверните [Самостоятельный сброс пароля (SSPR) в Azure AD](./tutorial-enable-sspr.md) и [Защита паролем Azure AD](./howto-password-ban-bad-on-premises-deploy.md), чтобы убедиться, что пользователи не используют простой пароль и условия, которые вы запретили.
...
Если вы защищаете доступ VPN с помощью расширения NPS Azure AD MFA, рассмотрите возможность объединения решения VPN в качестве [приложения SAML](../manage-apps/view-applications-portal.md) и определите категорию приложения, как описано ниже. 

Если вы развернули расширение NPS для Azure AD MFA для защиты локальных ресурсов, таких как VPN и шлюз удаленный рабочий стол, с MFA, следует заранее рассмотреть, если вы готовы отключить MFA в случае аварии.
...
Для обеспечения большей устойчивости вашей организации следует [включить синхронизацию хэша пароля](../hybrid/choose-ad-authn.md), поскольку она позволяет вам [переключиться на использование синхронизации хэша пароля](../hybrid/plan-connect-user-signin.md), если локальные системы идентификации не работают.
...
4. [Рассмотрение всех обнаружений рисков, о которых сообщается](../reports-monitoring/concept-sign-ins.md) в этом окне.
...
* [Что такое условия в Azure Active Directory условного доступа?](../conditional-access/concept-conditional-access-conditions.md)
* [Что такое элементы управления доступом в Azure Active Directory условном доступе?](../conditional-access/controls.md)
* [Что такое режим "только отчет с условным доступом"?](../conditional-access/concept-conditional-access-report-only.md)
