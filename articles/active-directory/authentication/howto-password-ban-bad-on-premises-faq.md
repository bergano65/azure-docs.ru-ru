...
**Вопрос. Почему при использовании повторяющиеся пароль отклонения события, регистрируемые при попытке установить ненадежный пароль Active Directory — пользователи и компьютеры оснастки управления?**

Оснастку управления компьютерами и Active Directory — пользователи сначала попытается задать новый пароль, с помощью протокола Kerberos. В случае сбоя в оснастке сделает секунды попытка задать пароль с помощью устаревших протоколов (SAM RPC) (определенные протоколы, используемые не важны). Если новый пароль считается слабой защитой паролей Azure AD, это приведет к в двух наборах сброса пароля: журнал событий отклонения.
