---
title: RADIUS и Azure MFA Server - Активный каталог Azure
...
> [!IMPORTANT]
> Эта статья предназначена только для пользователей Azure MFA Server. Если вы используете облачный Azure MFA, вместо этого посмотреть, как [интегрировать с radiUS аутентификации для Azure MFA](howto-mfa-nps-extension.md).
>
...
![Радиус аутентификации в МИД Сервер](./media/howto-mfaserver-dir-radius/radius.png)
...
1. Нажмите на вкладку **«Цель».**
   * Если сервер Azure MFA установлен на сервере, работаемом на домене в среде Active Directory, выберите **домен Windows.**
   * Если пользователи должны быть проверены на отношении каталога LDAP, выберите **LDAP bind**.
      Щелкните значок интеграции каталогов и измените конфигурацию LDAP на вкладке "Параметры", чтобы можно было привязать сервер к каталогу. Инструкции по настройке LDAP можно найти в [руководстве по конфигурации LDAP Proxy.](howto-mfaserver-dir-ldap.md)
   * Если пользователи должны быть проверены на другом сервере RADIUS, выберите **сервер RADIUS (ы)**.
