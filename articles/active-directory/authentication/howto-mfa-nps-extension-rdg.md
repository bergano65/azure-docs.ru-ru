---
title: Интеграция RDG с расширением NPS Azure MFA - Активный каталог Azure
...
Расширение Network Policy Server (NPS) для Azure позволяет клиентам защищать удаленную аутентификацию Dial-In User Service (RADIUS) клиентскую аутентификацию с помощью облачной [мультифакторной аутентификации Azure (MFA).](multi-factor-authentication.md) Это решение предоставляет двухфакторную проверку подлинности в качестве второго уровня безопасности для входа пользователей в систему и транзакций.
...
До получения расширения NPS для Azure клиенты, пожелавшие внедрить двухэтапную проверку интегрированных сред NPS и Azure MFA, должны были настроить и поддерживать отдельный mFA Server в предварительной среде, как это описано в [удаленном настольном шлюзе и многофакторном сервере аутентификации Azure с помощью RADIUS.](howto-mfaserver-nps-rdg.md)
...
1. Azure MFA общается с Azure AD, получает данные пользователя и выполняет вторичную аутентификацию с помощью поддерживаемых методов.
...
Необходима рабочая инфраструктура служб удаленных рабочих столов (RDS). Если вы этого не сделаете, то вы можете быстро создать эту инфраструктуру в Azure, используя следующий шаблон quickstart: [Создание удаленного развертывания коллекции сеансов рабочего стола.](https://github.com/Azure/azure-quickstart-templates/tree/ad20c78b36d8e1246f96bb0e7a8741db481f957f/rds-deployment)
...
**Подробнее:** [Развертывание RDS с помощью быстрого запуска Azure](https://docs.microsoft.com/windows-server/remote/remote-desktop-services/rds-in-azure) и [базового развертывания инфраструктуры RDS.](https://docs.microsoft.com/windows-server/remote/remote-desktop-services/rds-deploy-infrastructure)
...
1. В меню портала Azure выберите **Active Directory Azure**или ищите и выберите **Active Directory Azure** на любой странице.
1. Выберите **Свойства**.
1. В колонке "Свойства" рядом с идентификатором каталога щелкните значок **Копировать**, как показано ниже, чтобы скопировать идентификатор в буфер обмена.
   ...
   ![Получение идентификатора каталога с портала Azure](./media/howto-mfa-nps-extension-rdg/azure-active-directory-id-in-azure-portal.png)
...
* Предоставляет доступ к личному ключу сертификата пользователю сети
...
1. Введите `.\AzureMfaNpsExtnConfigSetup.ps1` и нажмите клавишу **ВВОД**. Сценарий проверяет, установлен ли модуль Azure Active Directory для PowerShell. Если он не установлен, сценарий автоматически установит этот модуль.
   ...
   ![Запуск AzureMfaNpsExtnConfigSetup.ps1 в Azure AD PowerShell](./media/howto-mfa-nps-extension-rdg/image4.png)
...
1. Проверив установку модуля PowerShell, сценарий отображает диалоговое окно модуля Azure Active Directory для PowerShell. В этом диалоговом окне введите учетные данные администратора Azure AD и пароль, затем щелкните **Вход**.
   ...
   ![Проверка подлинности Azure AD в PowerShell](./media/howto-mfa-nps-extension-rdg/image5.png)

1. При запросе вставьте идентификатор каталога, который вы скопировали на буфер обмена ранее, и нажмите **ENTER**.

   ![Ввод идентификатора каталога в PowerShell](./media/howto-mfa-nps-extension-rdg/image6.png)
...
1. Этот сценарий создает самозаверяющий сертификат и выполняет другие изменения конфигурация. На рисунке ниже показан пример выходных данных.
   ...
   ![Выход PowerShell, показывающий самоподписанный сертификат](./media/howto-mfa-nps-extension-rdg/image7.png)
...
Поток проверки подлинности требует обмена сообщениями RADIUS между шлюзом удаленного рабочего стола и сервером NPS, на котором установлено расширение NPS. Это означает, что необходимо настроить параметры клиента RADIUS на шлюзе удаленных рабочих столов и NPS-сервере, на котором установлено расширение NPS.
...
1. В менеджере RD Gateway, право-нажмите ** \[имя\] сервера (местный)** и нажмите **Свойства**.
1. В диалоговом поле Properties выберите вкладку **RD CAP Store.**
...
1. В поле **Введите имя или IP-адрес сервера политики сети** введите IP-адрес или имя сервера, на котором установлено расширение NPS.
   ...
   ![Введите имя или IP-адрес вашего NPS Server](./media/howto-mfa-nps-extension-rdg/image10.png)
...
1. В диалоговом окне **Общий секрет** введите общий секрет и нажмите кнопку **ОК**. Обязательно запишите этот общий секрет и сохраните в безопасном месте.
   ...
   ![Создание общей тайны для установления доверия](./media/howto-mfa-nps-extension-rdg/image11.png)
...
Для того чтобы у нас было время для проверки учетных данных пользователей, проведения двухэтапной проверки, получения ответов и ответа на сообщения RADIUS, необходимо скорректировать значение тайм-аута RADIUS.
...
1. В консоли **Сервер сетевых политик (локальный)** разверните элемент **RADIUS-клиенты и серверы** и выберите **Remote RADIUS Server** (Удаленный сервер RADIUS).
   ...
   ![Консоль управления сервером сетевой политики с изображением удаленного сервера RADIUS](./media/howto-mfa-nps-extension-rdg/image12.png)
...
1. В диалоговом окне **TS GATEWAY SERVER GROUP Properties** (Свойства группы серверов шлюза службы терминалов) выберите IP-адрес или имя NPS-сервера, настроенного для хранения политик авторизации подключений к удаленным рабочим столам, а затем нажмите кнопку **Изменить**.
   ...
   ![Выберите IP или имя сервера NPS, настроенного ранее](./media/howto-mfa-nps-extension-rdg/image13.png)
...
1. В поле **Число секунд между запросами, после которого сервер считается недоступным** измените значение по умолчанию, равное 30 секундам, на значение, которое равно или больше значения, указанного на предыдущем шаге.
   ...
   ![Отменять настройки тайм-аута Radius Server на вкладке балансировки нагрузки](./media/howto-mfa-nps-extension-rdg/image14.png)
...
1. На вкладке **Параметры** в разделе "Пересылка запроса на подключение" щелкните **Проверка подлинности**. Клиент RADIUS настроен для пересылки запросов на аутентификацию.
   ...
   ![Настройка настроек аутентификации с указанием группы серверов](./media/howto-mfa-nps-extension-rdg/image15.png)

1. Щелкните **Отмена**.

>[!NOTE]
> Для получения дополнительной информации о создании политики запроса на подключение смотрите статью, [нанастройка политик подключения запросить](https://docs.microsoft.com/windows-server/networking/technologies/nps/nps-crp-configure#add-a-connection-request-policy) документацию для того же. 
...
1. На NPS-сервере, на котором установлено расширение NPS, в консоли **Сервер сетевых политик (локальный)** щелкните правой кнопкой мыши **RADIUS-клиенты** и выберите **Создать**.
   ...
   ![Создание нового клиента RADIUS на консоли NPS](./media/howto-mfa-nps-extension-rdg/image17.png)

1. В новом диалоговом окне **клиента RADIUS** укажите дружественное имя, такое как _Gateway,_ и IP-адрес или DNS-имя сервера удаленного настольного шлюза.
1. В полях **Общий секрет** и **Подтвердите общий секрет** введите тот же секрет, который использовался ранее.

   ![Настройка дружественного имени и ip или DNS-адреса](./media/howto-mfa-nps-extension-rdg/image18.png)

1. Нажмите **OK,** чтобы закрыть новый диалоговый ящик клиента RADIUS.
...
1. Щелкните правой кнопкой мыши **Подключения к другим серверам доступа** и щелкните **Повторяющаяся политика**.
   ...
   ![Дублирование подключения к политике других серверов доступа](./media/howto-mfa-nps-extension-rdg/image19.png)
...
1. В диалоговом окне **Copy of Connections to other access servers** (Копировать подключения к другим серверам доступа) в поле **Имя политики** введите подходящее имя, например _RDG_CAP_. Проверка **включенной политики**и выберите **доступ к Гранту.** При необходимости в разделе **Тип сервера доступа к сети** выберите **Шлюз удаленных рабочих столов** или оставьте значение **Не указано**.

   ![Назовите политику, включите и предоставите доступ](./media/howto-mfa-nps-extension-rdg/image21.png)
...
1. Щелкните вкладку **Ограничения** и установите флажок **Разрешить подключение клиентов без согласования метода проверки подлинности**.
   ...
   ![Изменение методов проверки подлинности, чтобы позволить клиентам подключиться](./media/howto-mfa-nps-extension-rdg/image22.png)
...
1. При необходимости щелкните вкладку **Условия** и добавьте условия, которые должны быть выполнены для авторизации подключения, например членство в определенной группе Windows.
   ...
   ![Дополнительно указать условия соединения](./media/howto-mfa-nps-extension-rdg/image23.png)
...
1. Убедитесь, что новая политика находится вверху списка, включена и предоставляет доступ.
   ...
   ![Переместите политику в верхнюю часть списка](./media/howto-mfa-nps-extension-rdg/image24.png)
...
![Тестирование в удаленном веб-доступе к рабочему столу](./media/howto-mfa-nps-extension-rdg/image25.png)
...
![Удаленное подключение к рабочему столу, исражающее удаленное соединение](./media/howto-mfa-nps-extension-rdg/image26.png)

В следующем примере приложение Authenticator в Windows Phone используется для дополнительной аутентификации.

![Пример windows Phone Authenticator приложение, показывающее проверку](./media/howto-mfa-nps-extension-rdg/image27.png)

После успешного прохождения дополнительной аутентификации вы вошли в шлюз удаленных рабочих столов в обычном режиме. Однако, поскольку вы должны использовать вторичный метод аутентификации с помощью мобильного приложения на надежном устройстве, в процессе ввечили более безопасный, чем это было бы иначе.
...
![просмотр политики авторизации соединения с помощью PowerShell](./media/howto-mfa-nps-extension-rdg/image29.png)
...
![Пример успешных событий в журнале](./media/howto-mfa-nps-extension-rdg/image30.png)
...
![Телезритель событий сетевой политики и услуг доступа](./media/howto-mfa-nps-extension-rdg/image31.png)
...
![Журналы приложений Event Viewer Auth'](./media/howto-mfa-nps-extension-rdg/image32.png)
...
![Пример события неудачного логона](./media/howto-mfa-nps-extension-rdg/image33.png)

Ниже приведено связанное событие из журналов Azure MFA.

![Пример входа в журнал Azure MFA в Viewer event](./media/howto-mfa-nps-extension-rdg/image34.png)
...
![Пример приложения Shareware IAS parser](./media/howto-mfa-nps-extension-rdg/image35.png)
...
На рисунке ниже из Microsoft Message Analyzer показан сетевой трафик, отфильтрованный на протоколе RADIUS, который содержит имя пользователя **CONTOSO-AliceC.**

![Анализатор сообщений Майкрософт, показывающий отфильтрованный трафик](./media/howto-mfa-nps-extension-rdg/image36.png)

## <a name="next-steps"></a>Дальнейшие действия

[Как получить службу Многофакторной идентификации Azure](concept-mfa-licensing.md)

[Шлюз удаленных рабочих столов и сервер Многофакторной идентификации, использующие проверку подлинности RADIUS](howto-mfaserver-nps-rdg.md)
