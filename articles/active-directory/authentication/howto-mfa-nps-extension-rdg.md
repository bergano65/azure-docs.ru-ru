---
title: Интегрированным удаленным шлюза рабочих столов с помощью расширения NPS для MFA Azure — Azure Active Directory
...
**Дополнительные сведения**. [Развертывание служб удаленных рабочих СТОЛОВ с помощью быстрого запуска Azure](https://docs.microsoft.com/windows-server/remote/remote-desktop-services/rds-in-azure) и [развертывание инфраструктуры служб удаленных рабочих СТОЛОВ основные](https://docs.microsoft.com/windows-server/remote/remote-desktop-services/rds-deploy-infrastructure).
...
1. В колонке "Свойства" рядом с идентификатором каталога щелкните значок **Копировать**, как показано ниже, чтобы скопировать идентификатор в буфер обмена.
   ...
   ![Получение идентификатора каталога на портале Azure](./media/howto-mfa-nps-extension-rdg/image1.png)
...
1. Введите `.\AzureMfaNpsExtnConfigSetup.ps1` и нажмите клавишу **ВВОД**. Сценарий проверяет, установлен ли модуль Azure Active Directory для PowerShell. Если он не установлен, сценарий автоматически установит этот модуль.
   ...
   ![Под управлением AzureMfaNpsExtnConfigSetup.ps1 в Azure AD PowerShell](./media/howto-mfa-nps-extension-rdg/image4.png)
...
1. При появлении вставьте идентификатор каталога, скопированный в буфер обмена ранее и нажмите клавишу **ввод**.
...
1. Этот сценарий создает самозаверяющий сертификат и выполняет другие изменения конфигурация. На рисунке ниже показан пример выходных данных.
   ...
   ![Выходные данные PowerShell, показывающий самозаверяющий сертификат](./media/howto-mfa-nps-extension-rdg/image7.png)
...
1. В поле **Введите имя или IP-адрес сервера политики сети** введите IP-адрес или имя сервера, на котором установлено расширение NPS.
   ...
   ![Введите имя или IP-адрес сервера политики сети](./media/howto-mfa-nps-extension-rdg/image10.png)
...
1. В диалоговом окне **Общий секрет** введите общий секрет и нажмите кнопку **ОК**. Обязательно запишите этот общий секрет и сохраните в безопасном месте.
   ...
   ![Создание общего секрета для установления доверительных отношений](./media/howto-mfa-nps-extension-rdg/image11.png)
...
1. В консоли **Сервер сетевых политик (локальный)** разверните элемент **RADIUS-клиенты и серверы** и выберите **Remote RADIUS Server** (Удаленный сервер RADIUS).
   ...
   ![Консоль управления сервера политики сети, показывающий удаленного RADIUS-сервера](./media/howto-mfa-nps-extension-rdg/image12.png)
...
1. В диалоговом окне **TS GATEWAY SERVER GROUP Properties** (Свойства группы серверов шлюза службы терминалов) выберите IP-адрес или имя NPS-сервера, настроенного для хранения политик авторизации подключений к удаленным рабочим столам, а затем нажмите кнопку **Изменить**.
   ...
   ![Выберите IP-адрес или имя NPS-сервера, настроенного ранее](./media/howto-mfa-nps-extension-rdg/image13.png)
...
1. В поле **Число секунд между запросами, после которого сервер считается недоступным** измените значение по умолчанию, равное 30 секундам, на значение, которое равно или больше значения, указанного на предыдущем шаге.
   ...
   ![Изменение параметров времени ожидания Radius-сервера на вкладке балансировки нагрузки](./media/howto-mfa-nps-extension-rdg/image14.png)
...
1. На вкладке **Параметры** в разделе "Пересылка запроса на подключение" щелкните **Проверка подлинности**. Клиент RADIUS настроен для пересылки запросов на аутентификацию.
   ...
   ![Настроить параметры проверки подлинности, указав группу серверов](./media/howto-mfa-nps-extension-rdg/image15.png)
...
1. Дважды нажмите кнопку **ОК**.
   ...
   ![Регистрация сервера политики сети в Active Directory](./media/howto-mfa-nps-extension-rdg/image16.png)
...
1. На NPS-сервере, на котором установлено расширение NPS, в консоли **Сервер сетевых политик (локальный)** щелкните правой кнопкой мыши **RADIUS-клиенты** и выберите **Создать**.
   ...
   ![Создать новый RADIUS-клиент в консоли сервера политики сети](./media/howto-mfa-nps-extension-rdg/image17.png)
...
1. В полях **Общий секрет** и **Подтвердите общий секрет** введите тот же секрет, который использовался ранее.
   ...
   ![Настройте понятное имя и IP-адрес или DNS-адрес](./media/howto-mfa-nps-extension-rdg/image18.png)
...
1. Щелкните правой кнопкой мыши **Подключения к другим серверам доступа** и щелкните **Повторяющаяся политика**.
   ...
   ![Дублировать подключение на других серверах политики доступа](./media/howto-mfa-nps-extension-rdg/image19.png)
...
1. В диалоговом окне **Copy of Connections to other access servers** (Копировать подключения к другим серверам доступа) в поле **Имя политики** введите подходящее имя, например _RDG_CAP_. Установите флажок **Политика включена** и щелкните **Разрешить доступ**. При необходимости в разделе **Тип сервера доступа к сети** выберите **Шлюз удаленных рабочих столов** или оставьте значение **Не указано**.
   ...
   ![Имя политики, активация и предоставление доступа](./media/howto-mfa-nps-extension-rdg/image21.png)
...
1. При необходимости щелкните вкладку **Условия** и добавьте условия, которые должны быть выполнены для авторизации подключения, например членство в определенной группе Windows.
   ...
   ![При необходимости укажите условия соединения](./media/howto-mfa-nps-extension-rdg/image23.png)

1. Последовательно выберите **ОК**. При появлении предложения просмотреть соответствующий раздел справки нажмите кнопку **Нет**.
1. Убедитесь, что новая политика находится вверху списка, включена и предоставляет доступ.

   ![Переместить политике в верхней части списка](./media/howto-mfa-nps-extension-rdg/image24.png)
...
![Тестирование доступа к удаленному рабочему столу](./media/howto-mfa-nps-extension-rdg/image25.png)
...
![Подключение удаленного рабочего стола, инициализация удаленного подключения](./media/howto-mfa-nps-extension-rdg/image26.png)

В следующем примере приложение Authenticator в Windows Phone используется для дополнительной аутентификации.

![Пример Windows Phone структуры проверки подлинности, показывающий подтверждение в мобильном приложении](./media/howto-mfa-nps-extension-rdg/image27.png)

После успешного прохождения дополнительной аутентификации вы вошли в шлюз удаленных рабочих столов в обычном режиме. Тем не менее так как вы должны использовать дополнительную аутентификацию с помощью мобильного приложения на доверенном устройстве, процесс входа защищен лучше, чем было бы в противном случае.
...
![Примеры событий успешного входа в систему](./media/howto-mfa-nps-extension-rdg/image30.png)
...
![Политики сети и просмотра событий служб доступа](./media/howto-mfa-nps-extension-rdg/image31.png)
...
![Журналы событий приложений AuthZ средства просмотра](./media/howto-mfa-nps-extension-rdg/image32.png)
...
![Пример на событие logon Failed](./media/howto-mfa-nps-extension-rdg/image33.png)

Ниже приведено связанное событие из журналов Azure MFA.

![Пример журнала Azure MFA в средстве просмотра событий](./media/howto-mfa-nps-extension-rdg/image34.png)
...
![Пример средства синтаксического анализа для условно-бесплатных приложений IAS](./media/howto-mfa-nps-extension-rdg/image35.png)
...
![Microsoft Message Analyzer с фильтрацией трафика](./media/howto-mfa-nps-extension-rdg/image36.png)
