...
- Поиск `C:\Program Files\Multi-Factor Authentication Server\Logs\MultiFactorAuthSvc.log` и все резервные копии для "\<имя пользователя >» (включая кавычки для поиска) для поиска всех экземпляров записи пользователя добавленных или измененных.
