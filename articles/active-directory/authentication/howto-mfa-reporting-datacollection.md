---
title: Сбор данных пользователей Azure MFA - Активный каталог Azure
...
Изменения (используется для синхронизации изменений пользователя в MFA Server или Azure AD):
...
- Поиск `C:\Program Files\Multi-Factor Authentication Server\Logs\MultiFactorAuthSvc.log` и любые резервные копирования для "имя\<пользователя>" (включите цитаты в поиск), чтобы найти все экземпляры записи пользователя добавляются или изменены.
   - Эти записи могут быть ограничены (но не устранены) путем отсеивания **"Изменения пользователя"** в разделе MFA Server UX, разделе журнала, вкладке "Файлы журнала".
   - Если слог настроен, а **"Изменения пользователя"** проверяются в разделе MFA Server UX, разделе журнала, вкладке Syslog, то записи в журнале могут быть собраны из слога.
