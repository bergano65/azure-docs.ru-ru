---
title: Мониторинг защиты паролем Azure AD
description: Узнайте, как отслеживать и просматривать журналы для защиты паролем Azure AD для среды штатных служб домена Active Directory
...
---
# <a name="monitor-and-review-logs-for-on-premises-azure-ad-password-protection-environments"></a>Мониторинг и обзор журналов для находинных сред защиты паролем Azure AD
...
Мониторинг и отчетность выполняются либо с помощью сообщений журнала событий, либо с помощью запуска cmdlets PowerShell. Агент постоянного тока и прокси-сервисы регистрирующие сообщения журнала событий. Все cmdlets PowerShell, описанные ниже, доступны только на прокси-сервере (см. модуль AzureADPasswordProtection PowerShell). Программное обеспечение агента ПОСТОЯННОГО ТОКа не устанавливает модуль PowerShell.
...
Область отчетности cmdlet может влиять с помощью одного из параметров -Forest, -Domain, или -DomainController. Если этот параметр не указан, используется параметр -Forest.

Командлет `Get-AzureADPasswordProtectionSummaryReport` запрашивает журнал событий администратора агента контроллера домена, а затем подсчитывает общее количество событий, соответствующих каждой отображаемой категории результатов. Следующая таблица содержит отображение между каждым результатом и соответствующим идентификатором событий:
...
> [!NOTE]
> Это cmdlet работает, удаленно задавая запрос в журнал событий admin службы dc. Если журналы событий содержат большое количество событий, выполнение командлета может занять много времени. Кроме того, пакетные сетевые запросы для передачи больших наборов данных могут повлиять на производительность контроллера домена. Поэтому данный командлет следует осмотрительно использовать в рабочих средах.
...
Область запроса cmdlet может зависеть от использования параметров «Лес» или «Домен».
...
Если значение PasswordPolicyDateUTC становится устаревшим, это может быть симптомом того, что агент Azure AD Password Protection НА этой машине не работает должным образом.

## <a name="dc-agent-newer-version-available"></a>DC агент новая версия доступна

Служба агента ПОСТОЯННОГО ТОК зарегистрирует событие предупреждения 30034 для оперативного журнала при обнаружении, например, новая версия программного обеспечения агента ПОСТОЯННОГО ТОКа:
...
В приведенном выше событии не указана версия нового программного обеспечения. Для получения этой информации необходимо перейти к ссылке в сообщении события.

> [!NOTE]
> Несмотря на ссылки на "автообновление" в вышеупомянутом сообщении события, программное обеспечение агента DC в настоящее время не поддерживает эту функцию.
...
Кроме того, большинство смлетов Azure AD Password Protection PowerShell будут записываться в текстовый журнал, расположенный под:
...
## <a name="proxy-agent-newer-version-available"></a>Прокси-агент новая версия доступна

Служба прокси будет регистрировать 20002 предупреждение событие оперативного журнала при обнаружении, что новая версия прокси программного обеспечения доступна, например:
...
Это событие будет испущено, даже если агент Proxy настроен с включенным автоматическим обновлением.
