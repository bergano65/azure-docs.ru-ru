---
title: Сбросить пароли самообслуживания - Azure Active Directory
...
| Пользователю не назначена лицензия Azure AD. | Это может произойти, если у администратора, выполняющего операцию, не будет назначена лицензия Azure AD. <br> <br> Эту проблему можно устранить, назначив лицензию для соответствующей учетной записи администратора. Следуйте инструкциям в разделе [Шаг 1. Назначение необходимых лицензий](../users-groups-roles/licensing-groups-assign.md#step-1-assign-the-required-licenses). |
...
| Пароль не удовлетворяет политике нечеткого соответствия | Использованный пароль отображается в [списке запрещенных паролей](https://docs.microsoft.com/azure/active-directory/authentication/concept-password-ban-bad#how-are-passwords-evaluated) и не может быть использован. |
...
| Журнал событий компьютера Azure AD Connect содержит ошибку 32002, выданную службой PasswordResetService. <br> <br> Текст сообщения об ошибке: "Ошибка подключения к служебной шине. Поставщику маркеров не удалось предоставить маркер безопасности." | Локальной среде не удалось подключиться к конечной точке служебной шины Azure в облаке. Эту ошибку обычно вызывает правило брандмауэра, блокирующее исходящее подключение к конкретному порту или веб-адресу. См предпосылки подключения для получения дополнительной [информации.](../hybrid/how-to-connect-install-prerequisites.md) После обновления этих правил перезагрузите компьютер Azure AD Connect, и обратная запись паролей должна снова начать работать. |
...
* [Отключить и повторно включить функцию записи пароля](#disable-and-re-enable-the-password-writeback-feature)
...
> [!NOTE]
> SSPR также может выйти из строя, если настройки "Password никогда не истекает" или "Пользователь не может изменить пароль" настройки настроены на учетную запись в AD DS на месте. 
...
1. Щелкните запись службы правой кнопкой мыши, выберите команду **Перезапустить** и дождитесь завершения операции.
   ...
   ![Перезапустите службу синхронизации Azure AD Sync с помощью графического интерфейса][Service restart]
...
1. На вкладке **Соединители** выберите локальный соединитель **доменных служб Active Directory**, а затем щелкните **Свойства**.  
   ![Менеджер службы синхронизации показывает, как отсеивать свойства](./media/active-directory-passwords-troubleshoot/checkpermission01.png)  
...
1. Во всплывающем окне выберите вкладку **Connect to Active Directory Forest** (Подключение к лесу Active Directory) и запишите свойство **Имя пользователя**. Это свойство является учетной записью AD DS, используемой службой Azure AD Connect для выполнения синхронизации каталогов. Чтобы Azure AD Connect выполнила обратную запись паролей, учетной записи AD DS нужно иметь разрешение на сброс пароля.  
   ...
   ![Поиск службы синхронизации Active Directory учетной записи пользователя](./media/active-directory-passwords-troubleshoot/checkpermission02.png) 
...
1. Выберите **Просмотр** и убедитесь, что параметр **Advanced Features** (Дополнительные функции) включен.  
   ...
   ![Активные пользователи каталога и компьютеры показывают расширенные функции](./media/active-directory-passwords-troubleshoot/checkpermission03.png) 
...
1. Щелкните **Выбрать пользователя** и выберите учетную запись AD DS, используемую службой Azure AD Connect (см. шаг 3), а затем выберите **Просмотреть действующие разрешения**.
   ...
   ![Вкладка «Эффективный доступ», показывающая учетную запись синхронизации](./media/active-directory-passwords-troubleshoot/checkpermission06.png) 
...
1. Прокрутите список вниз и найдите **Сброс пароля**. Если возле записи установлен флажок, учетная запись AD DS имеет разрешение на сброс пароля выбранной учетной записи пользователя Active Directory.  
   ...
   ![Проверка того, что учетная запись синхронизации имеет разрешение пароля Спроза](./media/active-directory-passwords-troubleshoot/checkpermission07.png)  
...
* **Идентификатор пользователя.** Какой пользователь увидел ошибку? Примером является *contoso.com пользователей\@*.
...
* **Лицензирование**: Имеет ли пользователь назначенную лицензия Azure AD?
...
[Service restart]: ./media/active-directory-passwords-troubleshoot/servicerestart.png "Перезапуск службы синхронизации AD Azure"
