...
Предупреждения Если у вас есть синхронизированные учетные записи, для которых в Azure AD требуются пароли с неистекшим сроком `DisablePasswordExpiration` действия, необходимо явно добавить значение в атрибут PasswordPolicies объекта User в Azure AD.  Это можно сделать, выполнив следующую команду.
...
#### <a name="public-preview-of-synchronizing-temporary-passwords-and-force-password-on-next-logon"></a>Общедоступная Предварительная версия синхронизации временных паролей и "принудительный пароль при следующем входе"
...
Для поддержки временных паролей в Azure AD для синхронизированных пользователей можно включить функцию *форцепассвордресетонлогонфеатуре* , выполнив следующую команду на сервере Azure AD Connect, заменив <AAD Connector Name> именем соединителя. для конкретной среды:
...
Предупреждения  Принудительное изменение пароля пользователем при следующем входе в систему требует смены пароля.  AD Connect не установит флаг принудительной смены пароля самостоятельно, он дойдет до обнаруженной смены пароля, которая происходит во время синхронизации хэша паролей.
