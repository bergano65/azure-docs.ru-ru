...
Рекомендуется включить Енфорцеклаудпассвордполицифорпассвордсинцедусерс, прежде чем включать синхронизацию хэша паролей, чтобы начальная синхронизация хэшей паролей не применяет значение `DisablePasswordExpiration` к атрибуту PasswordPolicies для пользователей.
...
Предостережение. Если у вас есть синхронизированные учетные записи, для которых в Azure AD требуются пароли с неистекшим сроком действия, необходимо явно добавить значение `DisablePasswordExpiration` в атрибут PasswordPolicies объекта User в Azure AD.  Это можно сделать, выполнив следующую команду.
...
Для поддержки временных паролей в Azure AD для синхронизированных пользователей можно включить функцию *форцепассвордресетонлогонфеатуре* , выполнив следующую команду на сервере Azure AD Connect, заменив <AAD Connector Name> именем соединителя, относящимся к Ваша среда:
