...
>*«Приготовленный объект слишком велик. Обрезать число значений атрибутов на этом объекте. Операция будет повторена в следующем цикле синхронизации..."*
...
2. Настройте фильтры поиска со следующими значениями:
    ...
    | Тип объекта соединителя |**Пользователя** |
...
2. На вкладке **Описание** укажите следующую конфигурацию:
    ...
    | --- | --- | --- |
    | name | *Укажите имя* | Например, *Из внешней среды в AAD — пользовательское переопределение для userCertificate*. |
    | Описание | *Предоставить описание* | Например, *если атрибут userCertificate содержит более 15 значений, экспортировать NULL.* |
    | Подключенная система | *Выберите соединитель Azure AD* |
    | Тип объекта подключенной системы | **Пользователя** | |
    | Тип объекта метавселенной | **Человек** | |
...
7. Нажмите на **разъем Azure AD** и выберите **Run...**
...
1. Откройте вкладку **Connectors** (Соединители) в Synchronization Service Manager.
2. Нажмите на **разъем Azure AD** и выберите **Run...**
