...
- После успешного единого входа пользователю не предоставляется возможность выбрать вариант **Оставаться в системе**. Из-за такого поведения [сценарии отображения SharePoint и OneDrive](https://support.microsoft.com/help/2616712/how-to-configure-and-to-troubleshoot-mapped-network-drives-that-connec) не работают.
...
- Добавление URL-адреса службы`https://autologon.microsoftazuread-sso.com`Azure AD () в зону доверенных сайтов вместо локальной зоны интрасети *блокирует входе пользователей в*систему.
- Seamless SSO поддерживает AES256_HMAC_SHA1, AES128_HMAC_SHA1 и RC4_HMAC_MD5 типы шифрования для Kerberos. Для дополнительной безопасности рекомендуется, чтобы тип шифрования для учетной записи AzureADSSOAcc$ был установлен для AES256_HMAC_SHA1 или одного из типов AES против RC4. Тип шифрования хранится на атрибуте msDS-SupportedEncryptionTypes учетной записи в вашем Active Directory.  Если тип шифрования учетной записи AzureADSSOAcc$установлен на RC4_HMAC_MD5, и вы хотите изменить его на один из типов шифрования AES, пожалуйста, убедитесь, что вы сначала пролонгируете ключ расшифровки Kerberos учетной записи AzureADSSOAcc$, как поясняется в [документе часто задаваемых вопросов](how-to-connect-sso-faq.md) в соответствии с соответствующим вопросом, в противном случае бесшовных SSO не произойдет.
...
- Убедитесь, что URL-адрес`https://autologon.microsoftazuread-sso.com`Azure AD () является частью настроек зоны Intranet пользователя.
