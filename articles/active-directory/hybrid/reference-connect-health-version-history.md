...
ms.locfileid: "66298989"
---
# <a name="azure-ad-connect-health-version-release-history"></a>Azure AD Connect Health. Журнал выпуска версий
...
> [!NOTE]
> Connect Health, агенты обновляются автоматически при выпуске новой версии. Убедитесь, что параметры автоматического обновления включена на портале Azure. 
>
...
* Агент Azure AD Connect Health для AD FS (версия 3.1.51.0) 
   1. Исправление ошибки для различения нескольких операциях входа, которые совместно используют же client-request-id.
   2. Исправление ошибки синтаксического разбора ввода неправильного имени пользователя и пароля на языке локализации серверов.   

## <a name="april-2019"></a>Апрель 2019 г.
**Обновление агента:** 
* Агент Azure AD Connect Health для AD FS (версия 3.1.46.0) 
   1. Устранить предупреждения процесса проверки повторяющееся имя участника-службы для служб федерации Active Directory

## <a name="march-2019"></a>Марта 2019 г.
**Обновление агента:** 
* Агент Azure AD Connect Health для AD DS (версия 3.1.41.0)  
   1. Коллекции версии .NET
   2. Для улучшения сбора данных счетчиков производительности при отсутствии определенных категорий
   3. Исправление ошибки, предотвращению, запускающая нескольких экземпляров Monitoring Agent

* Агент Azure AD Connect Health для AD FS (версия 3.1.41.0) 
   1. Интеграция и обновление с помощью ADFSToolBox скриптов тестирования AD FS
   2. Реализовать коллекцию версии .NET
   3. Для улучшения сбора данных счетчиков производительности при отсутствии определенных категорий
   4. Исправление ошибки, предотвращению, запускающая нескольких экземпляров Monitoring Agent
...
* Агент Azure AD Connect Health для AD FS (версия 3.1.24.0).  
   ...
   3. Решить проблему с блокировкой EventHandler диагностики агента
