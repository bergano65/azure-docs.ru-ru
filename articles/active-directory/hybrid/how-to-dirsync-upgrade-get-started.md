...
![Обновление заблокировано](./media/how-to-dirsync-upgrade-get-started/analysisblocked.png)

В таких случаях рекомендуется установить новый сервер Azure AD Connect в [промежуточном режиме](how-to-connect-sync-staging-server.md), а также проверить старую конфигурацию DirSync и новую конфигурацию Azure AD Connect. Повторно примените изменения с помощью пользовательской конфигурации, как описано в разделе [Пользовательская конфигурация службы синхронизации Azure AD Connect](how-to-connect-sync-whatis.md).
...
2. Просмотрите и примите условия лицензионного соглашения и заявления о конфиденциальности.  
   ![Вас приветствует Azure AD](./media/how-to-dirsync-upgrade-get-started/Welcome.png)
3. Нажмите кнопку "Далее" для анализа существующей установки DirSync.  
   ![Анализ существующей установки службы синхронизации каталогов](./media/how-to-dirsync-upgrade-get-started/Analyze.png)
4. По завершении анализа вы получите рекомендации по дальнейшим действиям.  
   ...
   * При использовании полной версии SQL Server для DirSync вы увидите такую страницу:  
     ![снимок экрана, показывающий существующий используемый сервер базы данных SQL.](./media/how-to-dirsync-upgrade-get-started/AnalysisReadyFullSQL.png)  
   ...
   * При наличии более 50 000 объектов отобразится следующий экран:  
     ![Снимок экрана, на котором отображается экран с более чем 50 000 объектами для обновления.](./media/how-to-dirsync-upgrade-get-started/AnalysisRecommendParallel.png)  
...
5. Введите пароль для учетной записи, используемой в настоящее время для подключения к Azure AD. Это должна быть учетная запись, используемая в настоящее время с DirSync.  
   ...
   Если вы получаете сообщение об ошибке и испытываете проблемы с подключением, см. статью [Устранение неполадок подключения в Azure AD Connect](tshoot-connect-connectivity.md).
6. Укажите учетную запись администратора предприятия для Active Directory.  
   ![Введите учетные данные ADDS.](./media/how-to-dirsync-upgrade-get-started/ConnectToADDS.png)
7. Теперь все готово для настройки. Нажмите кнопку **Обновить**, чтобы удалить DirSync и начать настройку и синхронизацию Azure AD Connect.  
   ![Теперь все готово для настройки.](./media/how-to-dirsync-upgrade-get-started/ReadyToConfigure.png)
...
![Анализ завершен.](./media/how-to-dirsync-upgrade-get-started/AnalysisRecommendParallel.png)
...
![Снимок экрана, показывающий параметр экспорта параметров для переноса параметров в новую установку Azure AD Connect.](./media/how-to-dirsync-upgrade-get-started/forceexport.png)
...
4. Из папки установки Azure AD Connect (по умолчанию: C:\Program Files\Microsoft Azure Active Directory Connect) выполните следующую команду: `AzureADConnect.exe /migrate`.
   ...
   ![Снимок экрана, на котором показано, где импортировать файл параметров при обновлении.](./media/how-to-dirsync-upgrade-get-started/ImportSettings.png)
...
> [!NOTE]
> Между Windows Server Active Directory и Azure Active Directory начнется синхронизация, но изменения не будут экспортированы в Azure AD. В каждый момент времени активно экспортировать изменения может только одно средство синхронизации. Это состояние называется [промежуточным режимом](how-to-connect-sync-staging-server.md).
...
В приложении перейдите на вкладку **операции** . На этой вкладке убедитесь, что выполнены следующие операции:
...
![Импорт и синхронизация завершены](./media/how-to-dirsync-upgrade-get-started/importsynccompleted.png)

Просмотрите результат этих операций и убедитесь, что отсутствуют ошибки.

Если вы хотите просмотреть, какие изменения будут экспортированы в Azure AD, прочитайте, как проверить конфигурацию в [промежуточном режиме](how-to-connect-sync-staging-server.md). Внесите необходимые изменения конфигурации, пока не увидите что-нибудь непредвиденное.
...
![Дополнительные задачи](./media/how-to-dirsync-upgrade-get-started/AdditionalTasks.png)

* Выберите **Настроить промежуточный режим**.
* Отключите промежуточный режим, сняв флажок **Промежуточный режим включен** .

![Снимок экрана, на котором показан параметр включения промежуточного режима.](./media/how-to-dirsync-upgrade-get-started/configurestaging.png)
