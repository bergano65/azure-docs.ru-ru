---
title: 'Синхронизация Azure AD Connect: Внести изменение конфигурации в синхронизацию Azure AD Connect'
...
Цель этой статьи состоит в том, чтобы провести время, как внести изменения в конфигурацию по умолчанию в синхронизации Active Directory Azure (Azure AD). Он предоставляет шаги для некоторых распространенных сценариев. Ознакомившись с этими сведениями, вы сможете вносить изменения в конфигурацию в соответствии с собственными бизнес-правилами.

> [!WARNING]
> Если вы внесете изменения в правила синхронизации по умолчанию, то эти изменения будут перезаписаны при обновлении Azure AD Connect, что приведет к неожиданным и вероятным нежелательным результатам синхронизации.
>
> Правила синхронизации по умолчанию имеют отпечаток пальца. Если внести в правила изменения, отпечаток также изменится. Тогда при следующей попытке установить новый выпуск Azure AD Connect могут возникнуть проблемы. Выполняйте изменения только так, как описано в этой статье.
...
3. Присвойте правилу имя и описание. Выберите экземпляр локального каталога Active Directory и соответствующие типы объектов. В поле **Тип связи** выберите **Join** (Присоединение). Для **Precedence**выберите число, которое не используется другим правилом. Приоритет стандартных правил начинается со 100, поэтому в этом примере используется значение 50.
...
- Azure AD не разрешает службе Azure AD Connect изменять атрибут UserType для существующих пользователей Azure AD. Он может быть установлен только во время создания Azure AD пользователей и [изменен с помощью Powershell](/powershell/module/azuread/set-azureaduser?view=azureadps-2.0).
...
- Кроме того, можно получить значение атрибута UserType из других свойств. Например, необходимо синхронизировать всех пользователей в качестве **гостя,** если их атрибут ADPrincipalName заканчивается частью <em>@partners.fabrikam123.org</em>домена. 
...
Во избежание экспорта непреднамеренных изменений в Azure AD, изменяя правила синхронизации, убедитесь, что синхронизация не выполняется. Чтобы отключить встроенный планировщик синхронизации, сделайте следующее:
 ...
 3. Откройте менеджер службы синхронизации, отправившись в**службу синхронизации.** **Start** > 
...
1. Откройте редактор правил синхронизации, отправившись в редакцию **"Правил** > **синхронизации".**
...
6. Перейдите на вкладку **Преобразования** и реализуйте нужное правило преобразования. Предположим, вы назначили неиспользуемый локальный атрибут AD (например, extensionAttribute1) в качестве исходного атрибута для UserType. Вы можете реализовать непосредственный поток атрибутов.
    ...
    Еще один пример: требуется получить значение атрибута UserType из других свойств. Например, необходимо синхронизировать всех пользователей в качестве гостя, если их атрибут ADPrincipalName заканчивается частью <em>@partners.fabrikam123.org</em>домена. Вы можете реализовать такое выражение:
...
6. Перейдите на вкладку **«Трансформация»** и реализуйте следующее правило трансформации:
