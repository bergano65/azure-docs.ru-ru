...
| --- | --- |
| Использование существующего SQL Server |Позволяет указать имя SQL Server и имя экземпляра. Выберите этот параметр, если у вас уже есть сервер базы данных AD, который вы хотите использовать. Если для SQL Server не разрешен просмотр, в поле **Имя экземпляра** следует ввести имя экземпляра и через запятую номер порта.  Затем укажите имя базы данных Azure AD Connect.  Ваши привилегии s'L определяют, будет ли создана новая база данных или ваш администратор S'L должен создать базу данных заранее.  Если у вас есть разрешения S'L SA, [см., как установить с помощью существующей базы данных.](how-to-connect-install-existing-database.md)  Если вам были делегированы разрешения (DBO), [см. Install Azure AD Connect с разрешениями администратора S'L.](how-to-connect-install-sql-delegation.md) |
| Использование существующей учетной записи службы |По умолчанию Azure AD Connect использует учетную запись виртуальной службы для служб синхронизации. Если вы используете удаленный сервер S'L или прокси, требующее проверки подлинности, вам необходимо использовать **учетную запись управляемой службы** или знать учетную запись службы в домене и знать пароль. В этом случае укажите учетную запись для использования. Убедитесь, что пользователь, выполняющий установку, использует имя для входа SA в SQL, чтобы создать имя для входа в учетную запись службы.  См. статью [Azure AD Connect: учетные записи и разрешения](reference-connect-accounts-permissions.md#adsync-service-account). </br></br>В последней сборке внешнюю подготовку базы данных может выполнять администратор SQL, а установку — администратор Azure AD Connect с правами владельца базы данных.  Дополнительные сведения см. в статье [Install Azure AD Connect using SQL delegated administrator permissions](how-to-connect-install-sql-delegation.md) (Установка Azure AD Connect с использованием делегированных разрешений администратора SQL).|
...
| --- | --- |
| Синхронизация хэша паролей |Пользователи могут войти в облачные службы (Майкрософт), например Office 365, используя тот же пароль, что и в локальной сети. Пароли пользователей синхронизируются в Azure путем хэширования, а аутентификация выполняется в облаке. Дополнительные сведения см. в статье [Реализация синхронизации паролей с помощью Azure AD Connect Sync](how-to-connect-password-hash-synchronization.md). |
...
| Федерация с PingFederate|Пользователи могут войти в облачные службы (Майкрософт), например Office 365, используя тот же пароль, что и в локальной сети.  Во время входа пользователи перенаправляются в локальный экземпляр PingFederate и аутентификация выполняется локально. |
...
| Использовать существующую учетную запись | Выберите этот параметр, если вы хотите предоставить имеющуюся учетную запись AD DS, которая будет использоваться Azure AD Connect для подключения к лесу AD во время синхронизации каталогов. Вы можете указать имя домена в формате NetBios либо ввести полное доменное имя, т. е. FABRIKAM\syncuser или fabrikam.com\syncuser. Эта учетная запись может принадлежать обычному пользователю, так как стандартных разрешений для чтения вполне достаточно. Однако для некоторых сценариев могут потребоваться дополнительные разрешения. Для получения дополнительной информации смотрите [учетные записи и разрешения Azure AD Connect.](reference-connect-accounts-permissions.md#create-the-ad-ds-connector-account) |

![Подключение каталога](./media/how-to-connect-install-custom/connectdir02.png)

#### <a name="enterprise-admin-and-domain-admin-accounts-not-supported"></a>Учетные записи корпоративного админа и домена Admin не поддерживаются
По состоянию на сборку 1.4.18.0 он больше не поддерживается для использования корпоративного админ или учетной записи Domain Admin в качестве учетной записи AD DS Connector.  Если вы попытаетесь ввести учетную запись, которая является корпоративным админом или админ-аминтом при указании **использования существующей учетной записи,** вы получите следующую ошибку:

  **"Использование учетной записи администратора предприятия или домена для вашей учетной записи AD forest не допускается.  Позволяет Azure AD Connect создать учетную запись для вас или указать учетную запись синхронизации с правильными разрешениями.  &lt;Узнать&gt;больше "**
...
>
>Дополнительные сведения см. в статье о [миграции из Службы контроля доступа Azure](../azuread-dev/active-directory-acs-migration.md)
>
...
| --- | --- |
| Гибридное развертывание Exchange |Функция гибридного развертывания Exchange обеспечивает сосуществование почтовых ящиков Exchange в локальной среде и в службе Office 365. Azure AD Connect синхронизирует определенный набор [атрибутов](reference-connect-sync-attributes-synchronized.md#exchange-hybrid-writeback) из Azure AD с локальным каталогом. |
...
| Обратная запись групп |Если вы используете функцию **Группы Office 365**, эти группы могут быть доступны в вашей локальной службе Active Directory. Эта возможность доступна только при наличии Exchange в вашей локальной службе Active Directory. Для получения дополнительной [Group writeback](how-to-connect-preview.md#group-writeback)информации см. |
| Обратная запись устройств |Позволяет записывать объекты устройства в Azure AD в предпосылок Active Directory для сценариев условного доступа. Дополнительные сведения см. в статье [Azure AD Connect: включение обратной записи устройств](how-to-connect-device-writeback.md). |
...
Чтобы клиент автоматически входил в зону интрасети, необходимо убедиться, что URL является частью зоны интрасети. Таким образом при подключении к корпоративной сети присоединенный к домену компьютер автоматически отправляет билет Kerberos в Azure AD.
...
>[!NOTE]
>Вы можете обновить сертификат TLS/SSL для фермы AD FS с помощью Azure AD Connect, даже если вы не используете его для управления доверием федерации.
...
Вы можете использовать существующую ферму AD FS или создать новую. Если вы решите создать новый сертификат TLS/SSL. Если сертификат TLS/SSL защищен паролем, вам предлагается пароль.
...
Для получения дополнительной [Staging mode](how-to-connect-sync-staging-server.md)информации см.
...
При пользовательском настройке Azure AD Connect и выборе опции **Используйте существующий сервер S'L** на странице **обязательных компонентов Install,** вы можете столкнуться с ошибкой, которая гласит, что **база данных ADSync уже содержит данные и не может быть перезаписана. Пожалуйста, удалите существующую базу данных и повторите попытку.**
