...
| --- | --- |
| Использование существующего SQL Server |Позволяет указать имя SQL Server и имя экземпляра. Выберите этот параметр, если у вас уже есть сервер базы данных AD, который вы хотите использовать. Если для SQL Server не разрешен просмотр, в поле **Имя экземпляра** следует ввести имя экземпляра и через запятую номер порта.  Затем укажите имя базы данных Azure AD Connect.  Ваши привилегии SQL определяют, будет ли создана новая база данных, или администратор SQL должен создать базу данных заранее.  Если у вас есть разрешения SA SQL, ознакомьтесь с разрешениями [по установке с использованием существующей базы данных](how-to-connect-install-existing-database.md).  Если вы были делегированными разрешениями (DBO), см. [инструкции по установке Azure AD Connect с делегированными полномочиями администратора SQL](how-to-connect-install-sql-delegation.md). |
| Использование существующей учетной записи службы |По умолчанию Azure AD Connect использует учетную запись виртуальной службы для служб синхронизации. Если используется удаленный сервер SQL Server или используется прокси-сервер, требующий проверки подлинности, необходимо использовать **управляемую учетную запись службы** или использовать учетную запись службы в домене и знать пароль. В этом случае укажите учетную запись для использования. Убедитесь, что пользователь, выполняющий установку, использует имя для входа SA в SQL, чтобы создать имя для входа в учетную запись службы.  См. статью [Azure AD Connect: учетные записи и разрешения](reference-connect-accounts-permissions.md#adsync-service-account). </br></br>В последней сборке внешнюю подготовку базы данных может выполнять администратор SQL, а установку — администратор Azure AD Connect с правами владельца базы данных.  Дополнительные сведения см. в статье [Install Azure AD Connect using SQL delegated administrator permissions](how-to-connect-install-sql-delegation.md) (Установка Azure AD Connect с использованием делегированных разрешений администратора SQL).|
...
| Использовать существующую учетную запись | Выберите этот параметр, если вы хотите предоставить имеющуюся учетную запись AD DS, которая будет использоваться Azure AD Connect для подключения к лесу AD во время синхронизации каталогов. Вы можете указать имя домена в формате NetBios либо ввести полное доменное имя, т. е. FABRIKAM\syncuser или fabrikam.com\syncuser. Эта учетная запись может принадлежать обычному пользователю, так как стандартных разрешений для чтения вполне достаточно. Однако для некоторых сценариев могут потребоваться дополнительные разрешения. Дополнительные сведения см. в статье [Azure AD Connect учетные записи и разрешения](reference-connect-accounts-permissions.md#create-the-ad-ds-connector-account). |

![Подключение каталога](./media/how-to-connect-install-custom/connectdir02.png)

#### <a name="enterprise-admin-and-domain-admin-accounts-not-supported"></a>Учетные записи администратора и домена предприятия не поддерживаются.
Начиная с Build 1.4.18.0, в качестве учетной записи соединителя AD DS больше не поддерживается использование учетной записи администратора предприятия или администратор домена.  При попытке ввести учетную запись, которая является администратором предприятия или администратора домена при указании параметра **использовать существующую учетную запись**, появится следующая ошибка:

  **"Использование учетной записи администратора предприятия или домена для учетной записи леса AD запрещено.  Позвольте Azure AD Connect создать учетную запись или указать учетную запись синхронизации с правильными разрешениями.  &lt;Дополнительные сведения&gt;"**
...
| Обратная запись групп |Если вы используете функцию **Группы Office 365**, эти группы могут быть доступны в вашей локальной службе Active Directory. Эта возможность доступна только при наличии Exchange в вашей локальной службе Active Directory. Дополнительные сведения см. в разделе [обратная запись групп](how-to-connect-preview.md#group-writeback). |
| Обратная запись устройств |Позволяет выполнять обратную запись объектов устройств в Azure AD в локальную Active Directory для сценариев условного доступа. Дополнительные сведения см. в статье [Azure AD Connect: включение обратной записи устройств](how-to-connect-device-writeback.md). |
...
Чтобы обеспечить автоматический вход клиентов в зону интрасети, необходимо убедиться, что URL-адрес входит в зону интрасети. Таким образом при подключении к корпоративной сети присоединенный к домену компьютер автоматически отправляет билет Kerberos в Azure AD.
...
Вы можете использовать существующую ферму AD FS или создать новую. Если вы решили создать новый, необходимо предоставить сертификат TLS/SSL. Если сертификат TLS/SSL защищен паролем, вам будет предложено ввести пароль.
...
![Прокси](./media/how-to-connect-install-custom/adfs4.png)
...
Если пользовательская установка Azure AD Connect и выбрать параметр **использовать существующий SQL Server** на странице " **необходимые компоненты установки** ", может возникать ошибка, указывающая, что **база данных ADSync уже содержит данные и не может быть перезаписана. Удалите существующую базу данных и повторите** попытку.

![Ошибка](./media/how-to-connect-install-custom/error1.png)
...
![Ошибка](./media/how-to-connect-install-custom/error2.png)
