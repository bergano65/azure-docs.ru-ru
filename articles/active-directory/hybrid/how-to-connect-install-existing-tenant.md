...
### <a name="admin-role-considerations"></a>Соображения роли админа
Чтобы не допустить соответствия недоверчивых пользователей с пользователем облака, который имеет какую-либо роль админ, Azure AD Connect не будет сопоставлять объекты пользователя с объектами, которые играют роль админ. Это выполняется по умолчанию. Для обхода этого поведения вы можете сделать следующее:

1.  Удалите роли каталога из объекта пользователя только для облака.
2.  Если была неудачная попытка синхронизации пользователя, трудно удалить объект Quarantined в облаке.
3.  Триггер синхронизации.
4.  Дополнительно добавляйте роли каталога обратно в объект пользователя в облаке после сопоставления.
