...
### <a name="admin-role-considerations"></a>Рекомендации по роли администратора
Чтобы предотвратить недоверенных локальных пользователей с любой ролью администратора пользователям облака, Azure AD Connect не будет соответствовать локальных объектов пользователя с объектами, имеющими роль администратора. Это по умолчанию. Чтобы обойти это поведение, можно сделать следующее:

1.  Удалить роли каталога из объекта пользователя только для облака
2.  Запуск синхронизации
3.  При необходимости добавьте роли каталога объект пользователя в облаке после соответствующего произошла.
