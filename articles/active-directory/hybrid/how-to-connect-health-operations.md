...
>[!NOTE] 
> При наличии проблем с обработкой запросов на синхронизацию в нашей службе бэкэнда эта служба отправляет уведомление с информацией об ошибке на адрес электронной почты административного контакта (es) вашего клиента. Мы слышали отзывы клиентов о том, что в некоторых случаях объем этих сообщений непомерно велик, поэтому мы меняем способ отправки этих сообщений. 
>
> Вместо того, чтобы отправлять сообщение для каждой ошибки синхронизации каждый раз, когда это происходит, мы отправим ежедневный дайджест всех ошибок, которые вернула служба бэкэнда. Это позволяет клиентам обрабатывать эти ошибки более эффективно и уменьшает количество дубликатов сообщений об ошибках.
>
> Мы планируем, что это изменение будет осуществлено 15 января 2020 года.
...
4. Введите имя или идентификатор целевого пользователя или целевой группы. Одновременно можно выбрать сразу несколько пользователей или групп. Нажмите **Выберите**.
