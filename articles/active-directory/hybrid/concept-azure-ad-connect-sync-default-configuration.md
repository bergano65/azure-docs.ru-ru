...
* При наличии нескольких лесов атрибуты используются в следующем порядке:
  ...
  5. Если есть несколько лесов, соответствующих одному из этих правил, для определения леса, предоставляющего атрибуты, используется порядок создания (дата и время) соединителей (лесов). Первый подключенный лес будет первым лесом для синхронизации. 
...
ВЫПОЛНЯЮТСЯ — это средство набора ресурсов, которое устанавливается вместе с Azure AD Connect синхронизацией. Чтобы его можно было запустить, необходимо быть членом группы ADSyncAdmins. При запуске редактора вы увидите примерно следующее:
...
Вы также можете увидеть, что это правило синхронизации используется для синхронизации паролей. Если пользователь находится в области действия этого правила синхронизации, пароль синхронизируется из локальной среды в облако (при условии, что включена функция синхронизации паролей).
...
![Снимок экрана, показывающий раздел "фильтр области" в окне "изменение правила синхронизации для входящего трафика".](./media/concept-azure-ad-connect-sync-default-configuration/syncrulescopingfilter.png)
...
| Поступление из AD — учетная запись включена |Атрибуты, необходимые для входа в Azure AD и Microsoft 365. Эти атрибуты нужно перенести из включенной учетной записи. |
