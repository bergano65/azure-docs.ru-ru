...
- Для исходящего трафика агентов аутентификации, передаваемого в Azure AD, используются только стандартные порты (80 и 443). Не требуется открывать входящие порты в брандмауэре. 
  ...
  - Полный список требований к сети см. в [кратком руководстве по сквозной проверке подлинности Azure Active Directory ](how-to-connect-pta-quick-start.md#step-1-check-the-prerequisites).
...
- **Azure Service Bus**. Обеспечивает обмен данными через облако с корпоративными службами обмена сообщениями и ретрансляции. Такая конфигурация позволяет подключаться к локальным решениям с помощью облака.
- **Azure AD Connect Authentication Agent**. Локальный компонент, который ожидает передачи запросов на проверку пароля и отвечает на них.
- **Azure SQL Database**. Содержит сведения об агентах аутентификации клиента, включая их метаданные и ключи шифрования.
...
12. Агент аутентификации получает результат от службы Active Directory: проверка прошла успешно, неправильное имя пользователя или пароль, срок действия пароля истек.
   ...
   > [!NOTE]
   > Во время процесса входа в случае сбоя агента проверки подлинности, удаляется весь запрос на вход. Нет, не силами входа в систему запросов из одного агента проверки подлинности другой агент проверки подлинности на локальный. Эти агенты могут обмениваться данными только с облаком, а не с друг с другом.
...
>[!NOTE]
>Если в клиенте зарегистрировано несколько агентов аутентификации, то Azure AD не выполняет одновременное обновление этих агентов или их сертификатов. Вместо этого Azure AD выполняет одна за раз, чтобы обеспечить высокий уровень доступности запросов на вход.
>
...
- [Принцип работы](how-to-connect-pta-how-it-works.md). Основные принципы работы сквозной аутентификации Azure AD.
