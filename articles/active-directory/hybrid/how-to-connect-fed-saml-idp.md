---
title: 'Подключение Azure AD: Используйте поставщика идентификационных данных SAML 2.0 для одного знака - Azure'
...
Все остальные клиенты не поддерживаются в сценарии единого входа с помощью поставщика удостоверений SAML 2.0. Например, настольный клиент Lync 2010 не может войти в службу с вашим поставщиком идентификации SAML 2.0, настроенным для одного входного.

## <a name="azure-ad-saml-20-protocol-requirements"></a>Требования Azure AD к протоколу SAML 2.0
В этом документе подробно описываются требования к протоколу и формату сообщений, которые поставщик удостоверений SAML 2.0 должен выполнять для федерации с Azure AD с целью обеспечения единого входа в одну или несколько облачных служб Майкрософт (таких как Office 365). Проверяющей стороной SAML 2.0 (SP-STS) для облачной службы Майкрософт в этом сценарии является Azure AD.
...
Метаданные Azure AD можно загрузить с [https://nexus.microsoftonline-p.com/federationmetadata/saml20/federationmetadata.xml](https://nexus.microsoftonline-p.com/federationmetadata/saml20/federationmetadata.xml)этого URL: .
Для клиентов в Китае, использующих китай-специфический экземпляр Office 365, следует использовать следующую конечную точку федерации: [https://nexus.partner.microsoftonline-p.cn/federationmetadata/saml20/federationmetadata.xml](https://nexus.partner.microsoftonline-p.cn/federationmetadata/saml20/federationmetadata.xml).
...
2.  Azure AD потребует HTTP POST для представления токенов во время входиного.
3.  Azure AD будет использовать HTTP POST для запроса на аутентификацию поставщику идентификационных данных и REDIRECT для сообщения о вывески поставщику идентификационных данных.
...
Ниже приведен пример ответного сообщения, отправляемого поставщиком удостоверений, совместимым с SAML 2.0, в Azure AD или Office 365.
...
Для получения дополнительной информации о "Set-MsolDomainAuthentication", см.: [https://technet.microsoft.com/library/dn194112.aspx](https://technet.microsoft.com/library/dn194112.aspx).

>[!NOTE]
>Вы должны `$ecpUrl = "https://WS2012R2-0.contoso.com/PAOS"` запустить использование только в том случае, если вы настроили расширение ECP для поставщика идентификационных данных. Клиенты Exchange Online, исключая Outlook Web Application (OWA), используют активную конечную точку на основе метода POST. Если служба токенов безопасности SAML 2.0 реализует активную конечную точку, аналогичную реализации активной конечной точки с помощью расширения ECP для Shibboleth, эти полнофункциональные клиенты могут взаимодействовать со службой Exchange Online.

После настройки федерации можно вернуться к управляемой реализации (без поддержки федерации), однако на применение этого изменения требуется до двух часов, а каждому пользователю необходимо назначить новый случайный пароль для входа в облачные службы. Переключение к управляемой реализации может потребоваться в некоторых сценариях для сброса ошибочных параметров. Для получения дополнительной информации [https://msdn.microsoft.com/library/windowsazure/dn194122.aspx](https://msdn.microsoft.com/library/windowsazure/dn194122.aspx)о преобразовании домена см.: .

## <a name="provision-user-principals-to-azure-ad--office-365"></a>Подготовка субъектов-пользователей для Azure AD и Office 365
Чтобы пользователи могли проходить аутентификацию в Office 365, сначала нужно подготовить в Azure AD субъектов-пользователей, соответствующих утверждению в SAML 2.0. Если эти субъекты-пользователи заранее не известны службе Azure AD, их нельзя использовать для федеративного входа. Подготовить субъектов-пользователей можно с помощью Azure AD Connect или Windows PowerShell.
...
Для получения дополнительной информации о выезде "New-MsolUser",[https://technet.microsoft.com/library/dn194096.aspx](https://technet.microsoft.com/library/dn194096.aspx)
...
1. Скачать Connectivity Analyzer [https://testconnectivity.microsoft.com/?tabid=Client](https://testconnectivity.microsoft.com/?tabid=Client)от , .
...
5.  Анализатор подключения откроет ваш SAML 2.0 IDP для вас, чтобы войти в систему, ![ввести учетные данные для пользователя основной вы тестируете: SAML](./media/how-to-connect-fed-saml-idp/saml1.png)
...
1. На компьютере, связанном с доменом, вопийте в облачном сервисе, используя то же имя, которое используется для корпоративных учетных данных.
