...
**В: Как затвердеть сервер Azure AD Connect, чтобы уменьшить поверхность атаки безопасности?**
...
* Развертывание Azure AD Connect на сервере, подключенном к домену, и ограничение административного доступа к администраторам доменов или другим строго контролируемым группам безопасности
...
**В: Соединяет ли AD Azure поддержку синхронизации с двух доменов в Azure AD?**  
...
**В: Можете ли вы иметь несколько разъемов для одного и того же домена Active Directory в Azure AD Connect?**  
Нет, несколько соединителей для одного домена AD не поддерживаются. 

**В: Могу ли я перенести базу данных Azure AD Connect из локальной базы данных в удаленный экземпляр S'L Server?**   
...
Для этого проще всего использовать SQL Server Management Studio, установленный на том же компьютере, что и Azure AD Connect. Подключитесь к *(LocalDb).*
...
3. Установите Azure AD Connect для существующей [удаленной Базы данных SQL](how-to-connect-install-existing-database.md).
   В этой статье показано, как перейти на использование локальной базы данных SQL. При переносе для использования удаленной базы данных SQL на шаге 5 этого процесса также нужно ввести имеющуюся учетную запись службы, от имени которой будет запущена служба синхронизации Windows. Ниже описана учетная запись модуля синхронизации.
      ...
      **Используйте существующую учетную запись службы:** По умолчанию Azure AD Connect использует виртуальную учетную запись службы синхронизации для использования служб синхронизации. Если вы используете удаленный экземпляр SQL Server или требующий проверки подлинности прокси-сервер, вам необходима управляемая учетная запись службы или учетная запись службы в домене и пароль. В этом случае укажите учетную запись для использования. Убедитесь, что пользователи, которые запускают установку, являются системными администраторами SQL, чтобы можно было создать учетные данные для учетной записи службы. Дополнительные сведения см. в разделе [Учетная запись службы синхронизации Azure AD Connect](reference-connect-accounts-permissions.md#adsync-service-account). 
...
**Вопрос: Каковы некоторые из лучших практик на местах?**  

Ниже приводится информационный документ, в который представлены некоторые из лучших практик, которые инженерные, поддержки и наши консультанты разработали на протяжении многих лет.  Это представлено в списке пуль, на который можно быстро ссылаться.  Хотя этот список пытается быть всеобъемлющим, могут быть дополнительные передовые практики, которые, возможно, еще не сделали его в списке еще.

- При использовании полного S'L, то он должен оставаться локальным и удаленным
    - Меньше хмеля
    - Легче устранить неполадки
    - Меньше сложности
    - Необходимо обозначить ресурсы в S'L и разрешить накладные расходы для подключения Azure AD и ОС
- Обойти прокси, если это вообще возможно, если вы не в состоянии обойти прокси, то вам нужно убедиться, что значение тайм-аута больше, чем 5 минут.
- Если требуется прокси, то вы должны добавить прокси в файл machine.config
- Будьте в курсе локальных заданий и технического обслуживания S'L и о том, как они повлияют на Azure AD Connect, в частности, переиндексацию
- Убедитесь, что, чем DNS может решить внешне
- Убедитесь, что [спецификации сервера](how-to-connect-install-prerequisites.md#hardware-requirements-for-azure-ad-connect) являются рекомендацией независимо от того, используете ли вы физические или виртуальные серверы
- Убедитесь, что если вы используете виртуальный сервер, необходимые ресурсы
- Убедитесь, что конфигурация диска и диска соответствует рекомендациям для сервера S'L
- Установка и настройка Azure AD Connect Для мониторинга
- Используйте порог удаления, встроенный в Azure AD Connect.
- Тщательно просмотрите обновления релизов, чтобы быть готовыми ко всем изменениям и новым атрибутам, которые могут быть добавлены
- Резервное копирование все
    - Ключи резервного копирования
    - Правила синхронизации резервного копирования
    - Конфигурация сервера резервного копирования
    - Резервное копирование базы данных SQL
- Убедитесь, что нет агентов резервного копирования третьей стороны, которые резервного копирования S'L без S'L VSS Writer (обычно в виртуальных серверах с 3-й стороной снимки)
- Ограничьте количество пользовательских правил синхронизации, которые используются по мере их добавления сложности
- Относитесь к серверам подключения Azure AD как к серверам уровня 0
- Будьте хитрыми в изменении правил синхронизации облаков без глубокого понимания воздействия и правильных бизнес-драйверов
- Убедитесь, что корректные порты URL и Firewall открыты для поддержки Azure AD Connect и Azure AD Connect Health
- Использование отфильтрованного облака атрибута для устранения неполадок и предотвращения фантомных объектов
- С помощью staging Server убедитесь, что вы используете документирование конфигурации Azure AD Connect для согласованности между серверами
- Постановка серверов должна находиться в отдельных центрах обработки данных (физические места
- Постановка серверов не предназначена для решения высокой доступности, но вы можете иметь несколько промежуточных серверов
- Введение промежуточных серверов "Lag" может смягчить некоторые потенциальные простои в случае ошибки
- Сначала проверьте и проверите все обновления на промежуточного сервере
- Всегда проверяйте экспорт перед переходом на промежуточный сервер.  Используйте промежуточный сервер для полного импорта и полной синхронизации для снижения влияния на бизнес
- Сохраняйте согласованность версий между серверами подключения Azure AD как можно больше 

**В: Могу ли я разрешить подключение Azure AD для создания учетной записи Azure AD Connector на машине рабочей группы?**
Нет.  Для того, чтобы разрешить подключение Azure AD к автоматическому созданию учетной записи Azure AD Connector, машина должна быть соединена доменом.  

## <a name="network"></a>Сеть
**В: У меня есть брандмауэр, сетевое устройство или что-то еще, что ограничивает время, когда соединения могут оставаться открытыми в моей сети. Каким должен быть порог тайм-аута в клиентской стороне при использовании Azure AD Connect?**  
...
**В: Я имеют многолесной среды и сеть между двумя лесами использует NAT (Сетевой адрес перевод). Поддерживается ли использование Azure AD Connect между этими двумя лесами?**</br>
...
**В: У меня есть "Автоматические обновления полагаться участника" набор для Office 365 полагаться на участника. Должен ли я предпринимать какие-либо действия, когда мой сертификат подписи токенов автоматически переворачивается?**  
...
**В: Поддерживаются ли правила синхронизации нового поколения (NGC) на машине с поддержкой FIPS?**  
Нет.  Такая возможность не поддерживается.

**В. Если я отключил синхронизированное устройство (например: HAADJ) на портале Azure, почему оно включено повторно?**<br>
Синхронизированные устройства могут быть авторизированы или освоены в помещениях. Если синхронизированное устройство включено в помещении, оно может быть повторно включено в порталAz, даже если ранее было отключено администратором. Чтобы отключить синхронизированное устройство, используйте в ней active Directory для отработки учетной записи компьютера.

**В. Если я блокирую запись пользователя на портале Office 365 или Azure AD для синхронизированных пользователей, почему он разблокирован при повторном входе?**<br>
Синхронизированные пользователи могут быть авторами или осваиваться в помещениях. Если учетная запись включена в помещении, она может разблокировать блок ввне, размещенный администратором.
...
**В: Страница ввне Azure отображает имена пользователей, которые ранее успешно зарегистрировались. Можно ли отключить такое поведение?**  
В настоящее время изменение HTML атрибутов поля ввода **Password,** включая тег автозаполнения, не поддерживается. Мы работаем над функцией, которая позволит добавлять любой атрибут в поле **пароля**, используя пользовательский код Javascript.
...
**Вопрос: Я получил письмо, в котором говорилось, что мой авто-ап больше не работает, и мне нужно установить новую версию. Зачем мне это нужно?**  
...
**Вопрос: Я получил письмо, в котором говорилось, что я должен перейти на последнюю версию для повторного включения автоматического обновления. Я использую версию 1.1.654.0. Нужно ли обновлять?**  
...
**Вопрос: Я получил письмо, в котором говорилось, что я должен перейти на последнюю версию для повторного включения автоматического обновления. Если я использовал PowerShell для автоматического обновления, нужно ли мне установить последнюю версию?**  
...
**В: Я хочу перейти на новую версию, но не знаю, кто установил Azure AD Connect, и у нас нет имени пользователя и пароля. Нужна ли нам это?**
...
Чтобы проверить, какая версия Azure AD Connect установлена на вашем сервере, перейдите на панель управления и найдите установленную версию Microsoft Azure AD Connect, выбрав **программы** > **и функции,** как показано здесь:
...
Чтобы узнать, как это сделать, см. статью [Azure AD Connect: обновление до последней версии](how-to-upgrade-previous-version.md). 

**В: В прошлом году мы уже перешли на последнюю версию Azure AD Connect. Нужно ли нам обновить еще раз?**  
...
**В: Я считаю, что я перейдён до Azure AD Connect, но на портале Office по-прежнему упоминается DirSync. Почему это происходит?**  
...
**В: Мой статус автоматического обновления говорит: "Приостановлено". Почему он приостанавливается? Должен ли я включить его?**  
...
**Вопрос: Моя компания имеет строгие требования к управлению изменениями, и я хочу контролировать, когда она вытеснена. Могу ли я контролировать при запуске автоматического обновления?**  
Нет, сейчас такой возможности нет. Но мы рассматриваем возможность реализации этой функции в будущих выпусках.

**В: Получу ли я электронное письмо, если автоматическое обновление не удалось? Как я узнаю, что он был успешным?**  
...
**В: Я не уверен, каковы риски при переходе на новую версию Azure AD Connect. Можете ли вы позвонить мне, чтобы помочь мне с обновлением?**  
...
**В: Почему я вижу события 6311 и 6401 происходят после ошибок шага синхронизации?**

События 6311 - **Сервер столкнулся с неожиданной ошибкой при выполнении обратного вызова** и 6401 - **Контроллер агента управления столкнулся с неожиданной ошибкой** - всегда регистрируются после ошибки шага синхронизации. Чтобы устранить эти ошибки, необходимо очистить ошибки шагов синхронизации.  Для получения дополнительной информации см. [Ошибки при синхронизации](tshoot-connect-sync-errors.md) и [синхронизации объектов Troubleshoot с синхронизацией Azure AD Connect](tshoot-connect-objectsync.md)
