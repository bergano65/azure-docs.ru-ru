...
**Вопрос. как защитить сервер Azure AD Connect для снижения уязвимой зоны безопасности?**
...
**Вопрос. поддерживает ли Azure AD Connect синхронизацию из двух доменов в Azure AD?**  
...
**Вопрос. можно ли использовать несколько соединителей для одного домена Active Directory в Azure AD Connect?**  
Нет, несколько соединителей для одного домена AD не поддерживаются. 

**Вопрос. можно ли переместить базу данных Azure AD Connect из локальной базы данных в удаленный экземпляр SQL Server?**   
...
Для этого проще всего использовать SQL Server Management Studio, установленный на том же компьютере, что и Azure AD Connect. Подключитесь к *(LocalDb) .\адсинк*, а затем создайте резервную копию базы данных ADSync.
...
3. Установите Azure AD Connect для существующей [удаленной Базы данных SQL](how-to-connect-install-existing-database.md).
   В этой статье показано, как перейти на использование локальной базы данных SQL. При переносе для использования удаленной базы данных SQL на шаге 5 этого процесса также нужно ввести имеющуюся учетную запись службы, от имени которой будет запущена служба синхронизации Windows. Ниже описана учетная запись модуля синхронизации.
      ...
      **Использовать существующую учетную запись службы**. по умолчанию Azure AD Connect использует учетную запись виртуальной службы для использования службами синхронизации. Если вы используете удаленный экземпляр SQL Server или требующий проверки подлинности прокси-сервер, вам необходима управляемая учетная запись службы или учетная запись службы в домене и пароль. В этом случае укажите учетную запись для использования. Убедитесь, что пользователи, которые запускают установку, являются системными администраторами SQL, чтобы можно было создать учетные данные для учетной записи службы. Дополнительные сведения см. в разделе [Учетная запись службы синхронизации Azure AD Connect](reference-connect-accounts-permissions.md#adsync-service-account). 
...
**Вопрос. Каковы рекомендации из этого поля?**  

Ниже приведен информационный документ, в котором представлены некоторые рекомендации по проектированию, поддержке и нашим консультантам, разработанным в течение многих лет.  Это представлено в виде маркированного списка, на который можно быстро ссылаться.  Хотя этот список пытается быть исчерпывающим, могут возникнуть дополнительные рекомендации, которые еще не сделали этого в списке.

- При использовании полного SQL он должен оставаться локальным и удаленным
    - Меньше прыжков
    - Упрощение устранения неполадок
    - Меньше сложности
    - Необходимо назначить ресурсы SQL и разрешить дополнительные издержки на Azure AD Connect и ОС
- По возможности обходить прокси-сервер. Если не удается обойти прокси-сервер, необходимо убедиться, что время ожидания превышает 5 минут.
- Если требуется прокси-сервер, необходимо добавить прокси-сервер в файл Machine. config.
- Помните о локальных заданиях SQL и обслуживании, а также о том, как они влияют Azure AD Connect — особенно повторное индексирование
- Убедитесь, что служба DNS может разрешать внешние
- Убедитесь, что [спецификации сервера](how-to-connect-install-prerequisites.md#hardware-requirements-for-azure-ad-connect) используются для каждой рекомендации, независимо от того, используете ли вы физические или виртуальные серверы.
- Убедитесь, что при использовании виртуального сервера выделенные ресурсы выделены
- Убедитесь, что конфигурация диска и диска соответствует рекомендациям для SQL Server
...
- Резервное копирование все
    ...
    - Правила синхронизации резервных копий
    - Настройка резервного сервера
    - Резервное копирование базы данных SQL
- Убедитесь, что отсутствуют сторонние агенты резервного копирования, выполняющие резервное копирование SQL Server без модуля записи VSS SQL (Общие в виртуальных серверах с моментальными снимками сторонних разработчиков).
- Ограничьте объем настраиваемых правил синхронизации, которые используются для повышения сложности
- Рассматривать Azure AD Connect серверы как серверы уровня 0
- Будьте Лири, как изменять правила синхронизации в облаке без хорошего понимания влияния и правильных бизнес-факторов.
- Убедитесь, что правильные порты URL и брандмауэра открыты для поддержки Azure AD Connect и Azure AD Connect Health
- Использование атрибута Cloud Filtered для устранения неполадок и предотвращения фантомных объектов
- При использовании промежуточного сервера убедитесь, что для обеспечения согласованности между серверами используется Azure AD Connect документ конфигурации.
- Промежуточные серверы должны находиться в разных центрах обработки данных (физические расположения
- Промежуточные серверы не предназначены для решения высокой доступности, но могут иметь несколько промежуточных серверов.
- Введение промежуточных серверов задержки может снизить вероятность простоя в случае ошибки
- Сначала протестируйте и проверяйте все обновления на промежуточном сервере.
- Всегда проверяйте экспорты перед переключением на промежуточный сервер.  Используйте промежуточный сервер для полного импорта и полной синхронизации, чтобы снизить влияние на бизнес
...
**Вопрос. у меня есть брандмауэр, сетевое устройство или что-то другое, которое ограничивает время, в течение которого подключения могут оставаться открытыми в моей сети. Что должно иметь пороговое значение времени ожидания на стороне клиента при использовании Azure AD Connect?**  
...
**ВОПРОС в среде с несколькими лесами и в сети между двумя лесами используется NAT (преобразование сетевых адресов). Поддерживает ли Azure AD Connect между этими двумя лесами?**</br>
...
**Вопрос. у меня есть параметр "автоматически обновлять проверяющую сторону" для проверяющей стороны Office 365. Нужно ли предпринимать какие бы то ни было действия, когда сертификат для подписи маркера автоматически переносится?**  
...
**Вопрос. являются ли правила синхронизации следующего поколения Cryptographic (NGC), поддерживаемые на компьютере с поддержкой FIPS?**  
Нет.  Такая возможность не поддерживается.

**Вопрос. Если я отключил Синхронизированное устройство (например: ХААДЖ) в портал Azure, почему оно снова включено?**<br>
Синхронизированные устройства могут быть созданы или настроены локально. Если Синхронизированное устройство включено локально, оно может быть снова включено в портал Azure, даже если оно ранее было отключено администратором. Чтобы отключить Синхронизированное устройство, используйте локальную Active Directory для отключения учетной записи компьютера.

**В. Если я заблокирую вход пользователя на портале Office 365 или Azure AD для синхронизации пользователей, почему он разблокирован при повторном входе?**<br>
Синхронизированные пользователи могут быть созданы или настроены локально. Если учетная запись включена локально, она может разблокировать блок входа, размещенный администратором.
...
**Вопрос. на странице входа в Azure отображаются имена пользователей, которые ранее выполнили вход. Можно ли отключить это поведение?**  
В настоящее время изменение атрибутов HTML поля ввода **пароля** , включая тег автозаполнения, не поддерживается. Мы работаем над функцией, которая позволит добавлять любой атрибут в поле **пароля**, используя пользовательский код Javascript.
...
**Вопрос. я получил сообщение электронной почты с сообщением о том, что автоматическое обновление больше не работает, и мне нужно установить новую версию. Зачем это нужно?**  
...
**Вопрос. я получил сообщение электронной почты с сообщением об обновлении до последней версии для повторного включения автоматического обновления. Я использую версию 1.1.654.0. Требуется ли обновление?**  
...
**Вопрос. я получил сообщение электронной почты с сообщением об обновлении до последней версии для повторного включения автоматического обновления. Если я использовал PowerShell для включения автоматического обновления, нужно ли мне по-прежнему устанавливать последнюю версию?**  
...
**Вопрос. я хочу выполнить обновление до более новой версии, но я не уверен, что установил Azure AD Connect, и у нас нет имени пользователя и пароля. Требуется ли это?**
...
Чтобы проверить, какая версия Azure AD Connect установлена на сервере, перейдите на панель управления и найдите установленную версию Microsoft Azure AD подключить, выбрав **программы** > программы**и компоненты**, как показано ниже:
...
**Вопрос. Мы уже обновили последнюю версию Azure AD Connect прошлого года. Нужно ли выполнить обновление еще раз?**  
...
**Вопрос. я полагаю, что я обновился до Azure AD Connect, но портал Office по-прежнему упоминает DirSync. Почему это так?**  
...
**Вопрос. состояние автоматического обновления говорит "suspended". Почему он приостанавливается? Следует ли включать его?**  
...
**Вопрос. Моя компания имеет ограниченные требования к управлению изменениями, и я хочу контролировать, когда она оттуда. Можно ли контролировать, когда запускается автоматическое обновление?**  
Нет, сейчас такой возможности нет. Но мы рассматриваем возможность реализации этой функции в будущих выпусках.

**Вопрос. будет ли получено сообщение электронной почты, если не удалось выполнить автоматическое обновление? Как узнать, что оно было успешным?**  
...
**Вопрос. я не уверен, что риски возникают при обновлении до более новой версии Azure AD Connect. Можно ли позвонить мне для помощи в обновлении?**  
...
[Получение поддержки для Azure AD](https://docs.microsoft.com/azure/active-directory/active-directory-troubleshooting-support-howto)
