...
Если вам нужно вручную запустить цикл синхронизации, затем из выполнения PowerShell `Start-ADSyncSyncCycle -PolicyType Delta`.
...
Под управлением цикл полной синхронизации может занять очень много времени, в следующем разделе для чтения способы оптимизации этого процесса.

### <a name="sync-steps-required-for-different-configuration-changes"></a>Синхронизировать действия, необходимые для изменения конфигурации
Изменения конфигурации требуют действия синхронизации, чтобы убедиться, что изменения правильно применяются ко всем объектам.

- Добавление объектов или атрибутов для импорта из исходного каталога (путем добавления или изменения правила синхронизации)
    - Требуется полный импорт в соединителе для этого каталога источника
- Изменения в правилах синхронизации
    - Требуется полная синхронизация в соединителе для измененного правила синхронизации
- Изменение [фильтрации](how-to-connect-sync-configure-filtering.md), в результате которого изменяется число включаемых объектов.
    - Полный импорт необходимый для соединителя для каждого соединителя AD только при использовании фильтрации на основе атрибута на основе атрибутов, которые уже импортируется в модуль синхронизации

### <a name="customizing-a-sync-cycle-run-the-right-mix-of-delta-and-full-sync-steps"></a>Настройка цикл синхронизации, выполните нужные варианты изменений и полный шаги синхронизации
Чтобы избежать выполнения цикл полной синхронизации можно пометить специальные соединители, чтобы запустить полный шаг, с помощью следующих командлетов.
...
Пример:  При внесении изменений в правила синхронизации для соединителя «AD лесу А», не требующих любые новые атрибуты, должны импортироваться необходимо выполнить следующие командлеты для запуска синхронизации изменений цикл также полной синхронизации этап для этого соединителя.

`Set-ADSyncSchedulerConnectorOverride -ConnectorName “AD Forest A” -FullSyncRequired $true`

`Start-ADSyncSyncCycle -PolicyType Delta`

Пример:  При внесении изменений в правила синхронизации для соединителя «AD лесу А» таким образом, теперь требовать новый атрибут должен быть импортирован будет запустите следующие командлеты, чтобы запустить цикл синхронизации изменений, который также был полный импорт, шаг полной синхронизации для этого соединителя.
