...
В этой теме описывается встроенный планировщик синхронизации Azure AD Connect (синхронизация двигателя).
...
* **Задачи по техническому обслуживанию.** Обновление ключей и сертификатов для сброса паролей и службы регистрации устройств (DRS). Удаление старых записей из журнала операций.
...
* **ИндивидуальныеSyncCycleInterval**. Этот параметр позволяет настроить планировщик таким образом, чтобы синхронизация выполнялась чаще или реже, чем каждые 30 минут. На представленном выше снимке экрана планировщик настроен на ежечасное выполнение. Если значение этого параметра меньше, чем значение параметра AllowedSyncInterval, то используется последний.
* **NextSyncCyclePolicyType**. Изменение или исходное значение. Определяет, должен ли следующий запуск обрабатывать только изменения дельты, или если следующий запуск должен выполнять полный импорт и синхронизацию. Последние также будут перерабатывать любые новые или измененные правила.
...
### <a name="delta-sync-cycle"></a>Цикл синхронизации изменений
...
### <a name="full-sync-cycle"></a>Цикл полной синхронизации
...
Если вам нужно вручную запустить цикл синхронизации, `Start-ADSyncSyncCycle -PolicyType Delta`то от PowerShell запустить .
...
Запуск полного цикла синхронизации может занять очень много времени, прочитайте следующий раздел, чтобы прочитать, как оптимизировать этот процесс.

### <a name="sync-steps-required-for-different-configuration-changes"></a>Шаги синхронизации, необходимые для различных изменений конфигурации
...
- Добавлено больше объектов или атрибутов, которые будут импортированы из исходного каталога (путем добавления/изменения правил синхронизации)
    - Полный импорт требуется на коннектору для этого исходного каталога
- Изменения в правилах синхронизации
    - Для измененных правил синхронизации в коннекторе требуется полная синхронизация
- Изменение [фильтрации](how-to-connect-sync-configure-filtering.md), в результате которого изменяется число включаемых объектов.
    - Полный импорт требуется на разъеме для каждого AD-разъема UNLESS вы используете атрибут основе фильтрации на основе атрибутов, которые уже импортируются в движок синхронизации

### <a name="customizing-a-sync-cycle-run-the-right-mix-of-delta-and-full-sync-steps"></a>Настройка цикла синхронизации работает правильное сочетание шагов Delta и Full sync
Чтобы избежать полного цикла синхронизации, можно отметить определенные соединилы для выполнения полного шага, используя следующие cmdlets.
...
Пример: Если вы внесли изменения в правила синхронизации для разъема "AD Forest A", которые не требуют импорта новых атрибутов, вы запустите следующие cmdlets для выполнения цикла синхронизации дельты, который также сделал шаг полной синхронизации для этого разъема.

`Set-ADSyncSchedulerConnectorOverride -ConnectorName “AD Forest A” -FullSyncRequired $true`

`Start-ADSyncSyncCycle -PolicyType Delta`

Пример: Если вы внесли изменения в правила синхронизации для разъема "AD Forest A", так что теперь требуется импортировать новый атрибут, вы запустите следующие cmdlets для запуска цикла синхронизации дельты, который также сделал полный импорт, полный шаг синхронизации для этого разъема.
...
2. При использовании сборки до версии 1.1.281 остановка планировщика не означает, что действующий соединитель прекратит выполнение своей текущей задачи. Чтобы принудительно остановить соединитель, выполните следующие действия. ![StopAConnector](./media/how-to-connect-sync-feature-scheduler/stopaconnector.png)
   * Запустите **службу синхронизации** из меню «Старт». Перейдите в **коннектора,** выделите соединитель с состоянием **Бег,** и выберите **Остановка** из действий.
