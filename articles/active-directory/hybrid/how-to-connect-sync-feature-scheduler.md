...
* **NextSyncCyclePolicyType**. Изменение или исходное значение. Определяет, следует ли выполнять следующий запуск только для обработки изменений или если следующий запуск должен выполнить полный импорт и синхронизацию. В последнем случае также будут повторно обработаны все новые или измененные правила.
...
### <a name="delta-sync-cycle"></a>Цикл разностной синхронизации
...
Если необходимо запустить цикл синхронизации вручную, то из `Start-ADSyncSyncCycle -PolicyType Delta`запуска PowerShell.
