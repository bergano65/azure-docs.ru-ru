...
2. Подробное сообщение об ошибке, связанное с конфликтом UPN, показывает неправильное свойство displayName пользователя, имя UPN которого изменено или помещено на карантин. Например: 
    ...
    4.3. Сообщение об ошибке для **пользователя B** должно содержать информацию о том, что **пользователь A** уже использует **User@contoso.com** в качестве имени участника-пользователя, но вместо этого в нем содержится собственное отображаемое имя (свойство displayName) **пользователя B**.
