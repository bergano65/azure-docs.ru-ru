...
![Снимок экрана, на котором показаны ошибки синхронизации каталогов в центре администрирования Microsoft 365.](./media/how-to-connect-syncservice-duplicate-attribute-resiliency/1234.png "Активные пользователи")
...
Вот как выглядит уведомление электронной почты о конфликте ProxyAddress:   
    ![Снимок экрана, на котором показан пример уведомления по электронной почте о конфликте ProxyAddress.](./media/how-to-connect-syncservice-duplicate-attribute-resiliency/6.png "Активные пользователи")  
...
1. Объекты с определенными конфигурациями атрибутов по-прежнему вызывают ошибки экспорта вместо помещения в карантин повторяющихся атрибутов.  
   Пример:
    ...
    Б. Свойства этого объекта конфликтуют с существующей группой, где ProxyAddress — это **SMTP: джо \@ contoso.com**.
...
2. Подробное сообщение об ошибке, связанное с конфликтом UPN, показывает неправильное свойство displayName пользователя, имя UPN которого изменено или помещено на карантин. Пример:
    ...
    Б. Попытка синхронизации **пользователя б** выполняется следующим с **именем участника-пользователя (UPN = user \@ contoso.com** ).
...
Он должен указывать на [https://aka.ms/duplicateattributeresiliency]() .
