...
В настоящее время существует два метода для идентификации объектов, содержащие эти ошибки из-за конфликтов повторяющихся свойств, Azure Active Directory PowerShell и [Центр администрирования Microsoft 365](https://admin.microsoft.com). В будущем планируется добавить к этим средствам создание отчетов на портале.
...
1. **MaxResults \<Int >** может использоваться для ограничения запроса определенным количеством значений.
...
Ошибки синхронизации каталогов можно просмотреть в центре администрирования Microsoft 365. Отчет в Microsoft 365 admin center отображение **пользователя** объекты, содержащие эти ошибки. Этот отчет не содержит сведений о конфликтах между такими объектами, как **Группы** и **Контакты**.
...
Различные стратегии устранения неполадок описаны в статье [Duplicate or invalid attributes prevent directory synchronization in Office 365](https://support.microsoft.com/kb/2647098) (Запрет синхронизации службы каталогов в Office 365 из-за повторяющихся или недопустимых атрибутов).
...
1. Объекты с определенными конфигурациями атрибутов по-прежнему вызывают ошибки экспорта вместо помещения в карантин повторяющихся атрибутов.  
   Пример:
    ...
    a. Пользователь будет создан в AD с именем участника-пользователя **Джо\@contoso.com** и ProxyAddress **smtp:Joe\@contoso.com**
   
    2\. Свойства этого объекта конфликтуют с существующей группой, где ProxyAddress **SMTP:Joe\@contoso.com**.
...
2. Подробное сообщение об ошибке, связанное с конфликтом UPN, показывает неправильное свойство displayName пользователя, имя UPN которого изменено или помещено на карантин. Пример:
    ...
    a. **Пользователь А** синхронизируется до интеграции с **имени участника-пользователя = пользователь\@contoso.com**.
   
    2\. **Пользователь Б** является попытка синхронизировать время с **имени участника-пользователя = пользователь\@contoso.com**.
   
    c. **Пользователь Б** имени участника-пользователя меняется на **User1234\@contoso.onmicrosoft.com** и **пользователя\@contoso.com** добавляется **DirSyncProvisioningErrors** .
   
    d. Сообщение об ошибке для **пользователь Б** следует указать, что **пользователь A** уже **пользователя\@contoso.com** как имя участника-пользователя, но он показывает **пользователя B** собственные displayName.
