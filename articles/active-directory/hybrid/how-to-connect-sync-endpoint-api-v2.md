...
> [!NOTE]
> В настоящее время новая конечная точка не имеет настроенного ограничения размера группы для групп Office 365, которые записываются обратно. Это может повлиять на Active Directory и задержки цикла синхронизации.  Рекомендуется увеличивать размеры групп постепенно.  
...
>[!NOTE]
>Сейчас эта общедоступная предварительная версия доступна только в глобальном облаке Azure и недоступна для [национальных облаков](https://docs.microsoft.com/azure/active-directory/develop/authentication-national-cloud).
...
2. После завершения проверки переключите сервер **версии 2** в режим активного сервера, а сервер**версии 1** в режим промежуточного сервера. На этом этапе большие группы, которые находятся в области для синхронизации, будут подготовлены к работе в Azure AD, а также будут подготовлены к работе в AD большие единые группы Office 365, если включена обратная запись группы.
...
4. Нажмите кнопку **Изменить**, затем ![Изменить правило синхронизации](media/how-to-connect-sync-endpoint-api-v2/endpoint2.png).

6. Нажмите кнопку **Да**, чтобы отключить правило по умолчанию и создать редактируемую копию.
 ![Изменение правила синхронизации](media/how-to-connect-sync-endpoint-api-v2/endpoint3.png)

7. Во всплывающем окне на странице **Описание** задайте для приоритета ![Изменить правило синхронизации](media/how-to-connect-sync-endpoint-api-v2/endpoint4.png) доступное значение от 1 до 99.
...
>[!NOTE]
> Если у вас есть единые группы Office 365, насчитывающие более 50 000 членов, эти группы будут считываться в Azure AD Connect, а если включена обратная запись групп, они будут записаны в локальную службу AD. 
...
>[!NOTE]
> При переключении с конечных точек версии 2 на версию 1 синхронизированные группы, в которых более чем 50 000 членов, будут удалены после выполнения полной синхронизации. Это касается как групп AD, подготовленных к работе с Azure AD, так и единых групп Office 365, подготовленных к работе с AD. 
