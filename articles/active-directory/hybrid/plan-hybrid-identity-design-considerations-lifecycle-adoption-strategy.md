...
Azure AD RBAC позволяет компании ограничить количество операций, которые может выполнять человек после того, как получит доступ к порталу Azure. Используя RBAC для управления доступом к порталу, ИТ-администраторы могут применить такие методы управления доступом:

* **Назначение ролей на основе группы:** Вы можете назначить доступ к группам Azure AD, которые могут быть синхронизированы с локальным Active Directory. Это позволит с пользой применить средства и процессы, которые уже созданы в организации для управления группами. Можно также использовать функцию делегирования управления группой, которая есть в Azure AD Premium.
...
* **Гранулярный доступ к ресурсам:** Вы можете назначить роли пользователям и группам для определенной подписки, группы ресурсов или отдельного ресурса Azure, например веб-сайта или базы данных. Это позволит гарантировать, что пользователи имеют доступ ко всем нужным ресурсам и не имеют доступа к тем ресурсам, которыми не должны управлять.
...
* Администраторы могут предоставлять условный доступ на основе ресурса приложения, устройства и идентификации пользователя, местоположения сети и многофакторной аутентификации.
