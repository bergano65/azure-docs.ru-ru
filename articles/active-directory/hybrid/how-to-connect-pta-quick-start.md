...
![Azure AD Connect выполняет следующие функции: Вход пользователя](./media/how-to-connect-pta-quick-start/sso3.png)
...
![Azure AD Connect выполняет следующие функции: Изменение параметров входа пользователя](./media/how-to-connect-pta-quick-start/changeusersignin.png)
...
## <a name="step-3-test-the-feature"></a>Шаг 3. Тестирование компонента
...
![Центр администрирования Azure Active Directory: область Azure AD Connect](./media/how-to-connect-pta-quick-start/pta7.png)

![Центр администрирования Azure Active Directory: область сквозной аутентификации](./media/how-to-connect-pta-quick-start/pta8.png)
...
## <a name="step-4-ensure-high-availability"></a>Шаг 4. Обеспечение высокой доступности
...
Для большинства клиентов трех агентов аутентификации, достаточно высокий уровень доступности и емкость. Рекомендуется устанавливать агенты аутентификации как можно ближе к контроллерам домена, чтобы сократить задержку входа.

Чтобы начать, выполните эти инструкции, чтобы загрузить программное обеспечение агента аутентификации.
...
![Центр администрирования Azure Active Directory: область агента скачивания](./media/how-to-connect-pta-quick-start/pta10.png)
...
>[!IMPORTANT]
>Если на виртуальной машине установлен агент аутентификации, нельзя клонировать виртуальную машину, чтобы настроить другой агент проверки подлинности. Этот метод является **неподдерживаемый**.

## <a name="step-5-configure-smart-lockout-capability"></a>Шаг 5. Настройте возможность интеллектуальной блокировки

Смарт-блокировка помогает блокировки недобросовестных сторон, пытающимися угадать паролями пользователей или с помощью подбора методы для получения. Настроив параметры смарт-блокировка в Azure AD и (или) соответствующие блокировки учетных записей в Active Directory на предприятии, атак отфильтровывать прежде чем они достигнут Active Directory. Чтение [в этой статье](../authentication/howto-password-smart-lockout.md) Дополнительные сведения о том, как настроить параметры смарт-блокировка на клиенте для защиты учетных записей пользователей.
