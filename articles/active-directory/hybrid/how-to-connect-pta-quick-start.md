---
title: Azure AD сквозной аутентификации - Быстрый запуск (ru) Документы Майкрософт
...
3. Укажите один или несколько дополнительных серверов (под управлением Windows Server 2012 R2 или более поздней версии с включенным протоколом TLS 1.2), на которых необходимо запустить изолированные агенты аутентификации. Эти дополнительные серверы нужны для обеспечения высокого уровня доступности запросов на вход. Добавьте эти серверы в лес Active Directory, в котором размещены пользователи, пароли которых требуется проверить.
    ...
    >[!IMPORTANT]
    >В рабочих средах рекомендуется наличие как минимум 3 агентов аутентификации на клиенте. Существует системное ограничение в 40 агентов проверки подлинности для каждого клиента. Все серверы, на которых запущены агенты аутентификации, рекомендуется считать системами уровня 0 (ознакомьтесь со [справочником](https://docs.microsoft.com/windows-server/identity/securing-privileged-access/securing-privileged-access-reference-material)).
...
4. Если между серверами и Azure AD настроен брандмауэр, необходимо настроить указанные ниже элементы.
   ...
   - Если ваш брандмауэр или прокси позволяет DNS белый список, добавить соединения на ** \*.msappproxy.net** и ** \*.servicebus.windows.net**. Если нет, разрешите доступ к [диапазонам IP-адресов центра обработки данных Azure](https://www.microsoft.com/download/details.aspx?id=41653). Список диапазонов IP-адресов обновляется еженедельно.
...
### <a name="azure-government-cloud-prerequisite"></a>Предпосылки для облаков правительства Azure
Прежде чем включить проверку подлинности через Azure AD Connect с помощью Шага 2, загрузите последний релиз агента PTA с портала Azure.  Вы должны убедиться, что ваш агент версии **x.x.xxx.x** или позже.  Для проверки [Upgrade authentication agents](how-to-connect-pta-upgrade-preview-authentication-agents.md) вашего агента см.

После загрузки последнего релиза агента приступайте к приведенным ниже инструкциям по настройке проверки подлинности Pass-Through через Azure AD Connect.
...
1. Вопийте в [центре администратора active Directory Azure](https://aad.portal.azure.com) с глобальными учетными данными администратора для вашего клиента.
...
Установка нескольких агентов аутентификации обеспечивает высокую доступность, но не детерминированную балансировку нагрузки между агентами аутентификации. Чтобы определить, сколько агентов аутентификации вам нужно для вашего арендатора, рассмотрим пик овсянность и среднюю нагрузку запросов на входе, которые вы ожидаете увидеть на арендаторе. В качестве измерения производительности один агент аутентификации может обрабатывать от 300 до 400 операций аутентификации в секунду на стандартном сервере с четырехъядерным ЦП и 16 ГБ ОЗУ.

Чтобы оценить объем трафика, используйте следующие рекомендации по выбору размеров:
- Каждый запрос имеет размер полезной нагрузки (0,5 k и 1K num_of_agents) байтов, то есть данные из Azure AD в агент аутентификации. Здесь "число_агентов" указывает количество агентов аутентификации, зарегистрированных в клиенте.
- Каждый ответ имеет размер полезной нагрузки 1K байт, то есть данные от агента аутентификации до Azure AD.

Для большинства клиентов, три аутентификации агентов в общей сложности достаточно для высокой доступности и емкости. Рекомендуется устанавливать агенты аутентификации как можно ближе к контроллерам домена, чтобы сократить задержку входа.

Для начала следуйте этим инструкциям для загрузки программного обеспечения агента аутентификации:
...
2. Агент аутентификации можно зарегистрировать в службе с помощью Windows PowerShell. Создайте объект учетных данных PowerShell `$cred`, содержащий имя пользователя и пароль глобального администратора для вашего клиента. Выполнить следующую команду, заменив * \<имя\> пользователя* и * \<пароль:\>*
...
>[!IMPORTANT]
>Если агент аутентификации установлен на виртуальной машине, вы не можете клонировать виртуальную машину для настройки другого агента аутентификации. Этот метод **не поддерживается.**

## <a name="step-5-configure-smart-lockout-capability"></a>Шаг 5: Настройка возможности смарт-локаута
