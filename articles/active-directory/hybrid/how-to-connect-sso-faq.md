...
**Вопрос: С какими методами ввоза бесшовные SSO**
...
**В: Является ли Бесшовная SSO бесплатной функцией?**

Это бесплатный компонент, и для его использования не требуются платные выпуски Azure AD.

**В: Доступна ли бесшовная SSO в [облаке Microsoft Azure в Германии](https://www.microsoft.de/cloud-deutschland) и [в облаке правительства Microsoft Azure?](https://azure.microsoft.com/features/gov/)**

Нет. Компонент "Простой единый вход" доступен только в международном экземпляре Azure AD.

**В: Какие приложения `domain_hint` используют `login_hint` или параметр ную стьи в возможности Бесшовной SSO?**
...
| Порталы Office 365 | https:\//portal.office.com?domain_hint'contoso.com, https:\//www.office.com?domain_hint |

Кроме того, пользователи получают бесшумный опыт регистрации, если приложение отправляет запросы на ввоза в конечные точки\/Azure AD, настроенные как арендаторы, т.е. https: /login.microsoftonline.com/contoso.com/<..> или https:\//login.microsoftonline.com/<tenant_ID>/<.. > - вместо общей конечной точки Azure AD -\/то есть https: /login.microsoftonline.com/common/<...>. Ниже приведен неполный список приложений, которые выполняют такие типы запросов на вход.
...
**В: Поддерживается ли `Alternate ID` Бесшовная Поддержка `userPrincipalName`SSO в качестве имени пользователя, а не?**
...
**В: В чем разница между одним опытом вступления в систему, предоставленным [Azure AD Join](../active-directory-azureadjoin-overview.md) и Seamless SSO?**
...
**В: Я хочу зарегистрировать устройства без Windows 10 с Помощью Azure AD без использования AD FS. Могу ли я использовать Seamless SSO вместо этого?**
...
**В: Как пролонгировать ключ расшифровки `AZUREADSSOACC` компьютера Kerberos?**
...
Выполните следующие действия на локальном сервере, на котором выполняется Azure AD Connect:
   ...
   **Шаг 1. Получить список aD лесов, где бесшовные SSO была включена**
   ...
   **Шаг 2. Обновление ключа расшифровки Kerberos на каждом лесу н.э., что он был настроен на**
...
**В: Как отключить Бесшовную SSO?**

   **Шаг 1. Отключить функцию на арендаторе**
   ...
   **Шаг 2. Получить список aD лесов, где бесшовные SSO была включена**
   ...
   **Шаг 3. Вручную удалите учетную `AZUREADSSOACCT` запись компьютера из каждого леса AD, который вы видите в списке.**

## <a name="next-steps"></a>Дальнейшие действия

- [**Быстрый старт**](how-to-connect-sso-quick-start.md) - Вставать и запускать Azure AD Бесшовные SSO.
