...
Сбой входа в действие клиентских IP-адресов, собираются через серверы прокси веб-приложения. Каждый элемент в отчете о ненадежном IP-адресе показывает статистические данные о неудачных действиях входа AD FS, превышающих указанный порог. В отчете представлены следующие данные. ![Портал Azure AD Connect Health](./media/how-to-connect-health-adfs/report4a.png)
