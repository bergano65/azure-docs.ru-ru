...
* **Настройка сервера Azure AD Connect**. Если в качестве метода входа вы используете [сквозную аутентификацию](how-to-connect-pta.md), никакие дополнительные проверки предварительных требований не нужны. Если вы используете [синхронизацию хэша паролей](how-to-connect-password-hash-synchronization.md) в качестве метода входного вввоза, и если между Azure AD Connect и Azure AD существует брандмауэр, убедитесь, что:
   - Используется Azure AD Connect версии 1.1.644.0 или более поздней версии. 
   - Если ваш брандмауэр или прокси позволяет DNS белый список, белый список соединений ** \*к .msappproxy.net** URL-адресов над портом 443. Если нет, разрешите доступ к [диапазонам IP-адресов центра обработки данных Azure](https://www.microsoft.com/download/details.aspx?id=41653). Список диапазонов IP-адресов обновляется еженедельно. Это предварительное условие применяется только в том случае, если вы включаете функцию. Его выполнение не обязательно для фактического входа пользователя.

    >[!NOTE]
    >В Azure AD Connect версий 1.1.557.0, 1.1.558.0, 1.1.561.0 и 1.1.614.0 есть проблема, связанная с синхронизацией хэшей паролей. Если вы _не_ собираетесь использовать синхронизацию хэша паролей в сочетании с pass-through Authentication, прочитайте [примечания к выпуску Azure AD Connect,](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-version-history#116470) чтобы узнать больше.
...
Продолжайте работать через мастера, пока не доберетесь до **одного знака Enable на** странице. Укажите учетные данные администратора домена для каждого леса Active Directory, который:
...
>[!IMPORTANT]
> Бесшовная SSO создает `AZUREADSSOACC` компьютерную учетную запись, названную в вашем локальном Active Directory (AD) в каждом лесу AD. Учетная `AZUREADSSOACC` запись компьютера должна быть сильно защищена по соображениям безопасности. Управлять учетной записью компьютера должны только доменные аналитики. Убедитесь, что делегация Kerberos в учетной записи компьютера отключена, и `AZUREADSSOACC` что ни одна другая учетная запись в Active Directory не имеет разрешений на делегацию в учетной записи компьютера. Храните учетную запись компьютера в организационном подразделении (OU), где они защищены от случайных удаляний и где только домены имеют доступ.

>[!NOTE]
> Если вы используете архитектуру «Pass-the-Hash» и «Устранение учетных данных» `AZUREADSSOACC` в предварительной среде, внедь соответствующих изменений, чтобы убедиться, что учетная запись компьютера не окажется в контейнере quarantine. 
...
3. Просмотр **пользовательской политики настройки** > **административного** > **шаблонов** > Windows**Компоненты** > Internet**Explorer** > Internet Control**Панель** > **безопасности страница**панели . Выберите **Список назначений зоны для веб-сайтов**.
...
6. Просмотр**пользовательской политики** >  **конфигурации** > **административных шаблонов** > **Windows Компоненты** > **Internet Explorer** > **Internet Control Панель** > **безопасности Страница** > **Интранет зоны**. Выберите **Разрешить обновление строки состояния в сценарии**.
...
1. Откройте редактор "Управление групповыми политиками".
2. Измените групповую политику, которая применяется к некоторым или всем пользователям. В этом примере используется **политика домена по умолчанию**.
3. Просмотр**настройки** >  **конфигурации** > пользователя**Настройки Windows Настройки** > **Реестра** > **Новый** > **элемент реестра**.
...
Если вы переопределили в своей среде url-адрес [AuthNegotiateDelegateAllowlist](https://docs.microsoft.com/DeployEdge/microsoft-edge-policies#authnegotiatedelegateallowlist) или параметры политики [AuthServerAllowlist,](https://docs.microsoft.com/DeployEdge/microsoft-edge-policies#authserverallowlist) убедитесь, что вы добавите URL-адрес Azure AD ()`https://autologon.microsoftazuread-sso.com`к ним.

#### <a name="microsoft-edge-based-on-chromium-macos-and-other-non-windows-platforms"></a>Microsoft Edge на основе Chromium (macOS и других не-Windows платформ)

Для Microsoft Edge, основанного на Chromium на Mac OS и других платформах, не связанных с Windows, обратитесь к [Microsoft Edge, основанному на списке политик и дополнений,](https://docs.microsoft.com/DeployEdge/microsoft-edge-policies#authserverallowlist) чтобы узнайте о том, как добавить URL-адрес Azure AD для интегрированной аутентификации в свой список разрешений.
...
Простой единый вход не работает в конфиденциальном режиме просмотра в браузерах Firefox и Microsoft Edge, а также в Internet Explorer, если браузер работает в режиме повышенной защиты. Для следующей версии Microsoft Edge на основе Chromium, он не будет работать в режиме InPrivate и Гость дизайн.
...
Для тестирования сценария, когда пользователю не нужно вводить имя пользователя или пароль, выполните один из следующих шагов. 
   - Войдите в `https://myapps.microsoft.com/contoso.onmicrosoft.com` в новом частном сеансе браузера. Замените *contoso* с именем вашего арендатора.
