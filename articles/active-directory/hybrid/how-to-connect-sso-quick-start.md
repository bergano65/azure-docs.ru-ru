...
>[!NOTE]
> Если вы используете в локальной среде архитектуру защиты от имени и кражи учетных данных, внесите соответствующие изменения, чтобы убедиться, что учетная запись `AZUREADSSOACC` компьютера не находится в карантинном контейнере. 
...
3. Перейдите к**политике** >  **конфигурации** > пользователей**Административные шаблоны** > **компоненты** > **Internet Explorer** > **Страница Безопасность****панели** > управления Интернет. Выберите **Список назначений зоны для веб-сайтов**.
...
6. Перейдите к**политике** >  **конфигурации** > пользователей**Административные шаблоны** > **компоненты** > **Windows Internet Explorer** > Интернет**панель** > управления**страница** > безопасность**зона интрасети**. Выберите **Разрешить обновление строки состояния в сценарии**.
...
1. Откройте редактор "Управление групповыми политиками".
2. Измените групповую политику, которая применяется к некоторым или всем пользователям. В этом примере используется **политика домена по умолчанию**.
3. Перейдите**к параметрам** >  **конфигурации** > пользователя**Параметры** > **Реестр** > **Новый** > **элемент реестра**.
...
Сведения о том, как добавить URL-адрес Azure AD для встроенной проверки подлинности в список разрешений, см. в [статье Microsoft ребр](https://docs.microsoft.com/DeployEdge/microsoft-edge-policies#authserverallowlist) на основе Chromium на Mac OS и других платформах, отличных от Windows.
