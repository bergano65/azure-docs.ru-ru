---
title: 'Azure AD Connect выполняет следующие функции: простой единый вход — быстрый запуск | Документация Майкрософт'
...
* **Используйте поддерживаемую топологию Azure AD Connect**. Убедитесь, что вы используете одну из поддерживаемых топологий Azure AD Connect, описанных [здесь](plan-connect-topologies.md).
...
* **Настройте учетные данные администратора домена**. Необходимо иметь учетные данные администратора домена для каждого леса Active Directory, который:
    * синхронизируется с Azure AD через Azure AD Connect;
    * содержит пользователей, для которых нужно включить простой единый вход.
    
* **Активируйте современную аутентификацию**. Для работы этой функции необходимо включить [современную аутентификацию](https://docs.microsoft.com/office365/enterprise/modern-auth-for-office-2013-and-2016) на клиенте.
...
![Azure AD Connect выполняет следующие функции: Вход пользователя](./media/how-to-connect-sso-quick-start/sso8.png)
...
![Azure AD Connect выполняет следующие функции: изменение параметров входа пользователя](./media/how-to-connect-sso-quick-start/changeusersignin.png)
...
3. Перейдите к**политике** >  **конфигурации пользователей** **Административные шаблоны**компонентыWindows > **Internet Explorer**Интернет >  >  >  **Страница «Безопасность» панели** > управления. Выберите **Список назначений зоны для веб-сайтов**.
...
6. Перейдите к**политике** >  **конфигурации пользователей** **Административные шаблоны**компонентыWindows > **Internet Explorer**Интернет >  >  >  > **Зона** **безопасности на странице** > безопасность панели управления. Выберите **Разрешить обновление строки состояния в сценарии**.
...
На шаге 2 Azure AD Connect создает учетные записи компьютеров (представляющие Azure AD) во всех лесах Active Directory, для которых включен простой единый вход. Дополнительные сведения см. в статье [Простой единый вход Azure Active Directory: подробное техническое руководство](how-to-connect-sso-how-it-works.md).
...
- [Техническое руководство](how-to-connect-sso-how-it-works.md). Поймите, как работает функция простого единого входа.
