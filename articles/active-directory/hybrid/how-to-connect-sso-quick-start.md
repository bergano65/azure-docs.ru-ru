...
>[!IMPORTANT]
> Простой единый вход создает учетную запись компьютера с именем `AZUREADSSOACC` в локальной Active Directory (AD) в каждом лесу AD. Учетная запись компьютера `AZUREADSSOACC` должна быть строго защищена по соображениям безопасности. Только администраторы домена должны иметь возможность управлять учетной записью компьютера. Убедитесь, что делегирование Kerberos для учетной записи компьютера отключено и что другие учетные записи в Active Directory не имеют разрешений на делегирование учетной записи компьютера `AZUREADSSOACC`. Храните учетную запись компьютера в подразделении (OU), где они защищены от случайного удаления и когда доступ имеют только администраторы домена.

>[!NOTE]
> Если вы используете в локальной среде архитектуру для защиты от кражи и хэширования учетных данных, внесите соответствующие изменения, чтобы убедиться, что учетная запись `AZUREADSSOACC` компьютера не находится в контейнере карантина. 
...
| Вариант | Решение администратора | Возможности для пользователя |
| --- | --- | --- |
...
3. Перейдите к **пользовательской конфигурации** > **политика** > **Административные шаблоны** > **компоненты Windows** > **Internet Explorer** > веб- **Панель управления Интернет** > **Страница "безопасность**". Выберите **Список назначений зоны для веб-сайтов**.
...
6. Перейдите к **пользовательской конфигурации** > **политика** > **Административные шаблоны** > **компоненты Windows** > **internet Explorer** > **Панель управления Интернет** > **Страница Безопасность** > **зона интрасети**. Выберите **Разрешить обновление строки состояния в сценарии**.
