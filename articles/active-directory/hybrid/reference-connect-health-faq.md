...
**В: Я управляю несколькими каталогами Azure AD. Как перейти на тот, который имеет Azure Active Directory Premium?**
...
1. Открытый Internet Explorer > **Настройки** > **Интернет Параметры** > **подключения** > **LAN Настройки**.
2. Выберите **прокси-сервер для своей сети.**
...
* Агент не может подключиться к необходимым конечным точкам, так как трафик блокируется брандмауэром. Это особенно часто происходит на прокси-серверах веб-приложений. Убедитесь, что исходящие подключения к необходимым конечным точкам и портам разрешены. Подробнее о [разделе требований.](how-to-connect-health-agent-install.md#requirements)
* Исходящие связи подвергаются проверке TLS сетевым слоем. В результате сертификат, используемый агентом, заменяется проверяющим сервером или сущностью, что приводит к ошибке регистрации агента.
...
**Вопрос: Меня предупреждают, что «данные Службы здравоохранения не обновляются». Как устранить проблему?**

Расширение Azure AD Connect Health создает оповещение, если не получает все точки данных от сервера за последние два часа. [Читать далее](how-to-connect-health-data-freshness.md).
...
**В: Меня предупреждают о том, что "Запрос на проверку подлинности (Synthetic Transaction) не получил токен". Как устранить проблему?**
...
**Вопрос: Я получил письмо с указанием мои машины не исправлены для недавних атак вымогателей. Почему я получил это письмо?**
...
Включите журналы аудита с помощью командлета PowerShell <i>Get-AdfsProperties -AuditLevel</i>. Узнайте больше о [журналах аудита ADFS](https://docs.microsoft.com/windows-server/identity/ad-fs/technical-reference/auditing-enhancements-to-ad-fs-in-windows-server#auditing-levels-in-ad-fs-for-windows-server-2016). Обратите внимание: если расширенные параметры аудита отправляются на сервер AD FS, все изменения, внесенные в файл auditpol.exe, перезаписываются (даже если созданное приложение не настроено). В этом случае настройте локальную политику безопасности для ведения журнала сбоев и успешных выполнений созданного приложения.

**В: Когда сертификат агента будет автоматически продлен до истечения срока действия?**
Сертификация агента будет автоматически продлена за **6 месяцев** до истечения срока его действия. Если он не будет продлен, пожалуйста, убедитесь, что сетевое соединение агента является стабильным. Перезапуск служб агента или обновление до последней версии также может решить проблему.


## <a name="related-links"></a>Связанные ссылки
* [Azure AD Connect Health](whatis-hybrid-identity-health.md)
* [Установка агента по охране здоровья Azure AD Connect](how-to-connect-health-agent-install.md)
