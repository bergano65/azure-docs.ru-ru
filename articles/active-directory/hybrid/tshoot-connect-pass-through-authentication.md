...
>[!IMPORTANT]
>Если вы столкнулись с проблемами в работе сквозной аутентификации, которые касаются входа пользователей, не отключайте эту функцию и не удаляйте агенты сквозной аутентификации, если у вас нет облачной учетной записи глобального администратора, на которую можно переключиться. Узнайте о [добавлении учетной записи Глобального администратора только](../active-directory-users-create-azure-portal.md)для облака. Этот шаг очень важен для того, чтобы не потерять доступ к клиенту.
...
Перейдите в**систему Active** **Directory** -> В в [центре Azure Active Directory](https://aad.portal.azure.com/) и нажмите на активность ввходе конкретного пользователя. Найдите поле **КОД ОШИБКИ ВХОДА**. Сопоставьте значение этого поля c причиной сбоя и способом разрешения с помощью следующей таблицы:
...
>[!IMPORTANT]
>Агенты проверки подлинности Azure AD, удостоверяющие подлинность пользователей Azure AD, проверяет их имена пользователей и пароли в отношении Active Directory, позвонив в [API Win32 LogonUser.](https://msdn.microsoft.com/library/windows/desktop/aa378184.aspx) В результате, если вы установили настройку "Logon To" в Active Directory для ограничения доступа к входу в журнал на рабочих станциях, вам также придется добавлять серверы, худеяря которых размещают агенты по аутентификации через Pass-Through. В противном случае пользователи могут войти в Azure AD.
