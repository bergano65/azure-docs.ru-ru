...
- Компьютерная учетная запись ()`AZUREADSSOACC`создается в вашем предпосылок Active Directory (AD) в каждом лесу AD, который вы синхронизируете с Azure AD (с помощью Azure AD Connect).
- Кроме того, создается ряд основных имен службы Kerberos (SPNs), которые будут использоваться в процессе вхинга Azure AD.
- С помощью Azure AD безопасным образом предоставляется ключ расшифровки Kerberos учетной записи компьютера. Если существует несколько лесов АД, каждый компьютерный счет будет иметь свой собственный уникальный ключ расшифровки Kerberos.

>[!IMPORTANT]
> Учетная `AZUREADSSOACC` запись компьютера должна быть сильно защищена по соображениям безопасности. Управлять учетной записью компьютера должны только доменные аналитики. Убедитесь, что делегация Kerberos в учетной записи компьютера отключена, и `AZUREADSSOACC` что ни одна другая учетная запись в Active Directory не имеет разрешений на делегацию в учетной записи компьютера. Храните учетную запись компьютера в организационном подразделении (OU), где они защищены от случайных удаляний и где только домены имеют доступ. Ключ расшифровки Kerberos на учетной записи компьютера также следует рассматривать как чувствительный. Настоятельно рекомендуется, чтобы вы [меняли ключ расшифровки Kerberos](how-to-connect-sso-faq.md) для учетной записи компьютера `AZUREADSSOACC` хотя бы раз в 30 дней.
