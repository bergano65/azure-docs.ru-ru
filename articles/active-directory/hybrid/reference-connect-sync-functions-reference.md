...
[цертверсион](#certversion) |[IsCert](#iscert) | | | |
| **Преобразование** | | | | |
...
**Примечания:**  
Если результатом вычисления выражения является ненулевое значение, CBool возвращает значение true, в противном случае возвращается значение false.
...
* Значение: строка с датой, временем и (необязательно) часовым поясом.
...
* Casetype: CaseInsensitive (без учета регистра) или CaseSensitive (с учетом регистра).
...
* Источник: строка в двухбайтовой кодировке UTF8.
...
* dn: ссылочный атрибут для интерпретации.
* ComponentNumber: компонент в различающемся имени для возврата.
* Параметры: DC — игнорировать все компоненты с "dc=".
...
**Примечания:**  
Возможные значения для формата можно найти здесь: [Custom date and time formats for the FORMAT function](https://docs.microsoft.com/dax/custom-date-and-time-formats-for-the-format-function) (Настраиваемые форматы даты и времени для функции форматирования).
...
* : атрибут с несколькими значениями, содержащий строки для объединения.
...
* padCharacter: строка, состоящая из одного знака, используемого в качестве символа заполнения.
