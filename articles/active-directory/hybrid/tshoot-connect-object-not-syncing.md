...
* **CS:** Пространство для подключения, таблица в базе данных
...
1. **Импорт из АД:** Объекты active Directory вносятся в Active Directory CS.

2. **Импорт из Azure AD:** Объекты Azure AD вносятся в CS Azure AD.

3. **Синхронизация:** Правила входиной синхронизации и правила исходящих синхронизации работают в порядке номера приоритета, от нижнего к более высокому. Чтобы просмотреть правила синхронизации, перейдите в редактор правил синхронизации из приложений рабочего стола. Правила входящей синхронизации перемещают данные из CS в MV. Правила исходящей синхронизации перемещают данные из MV в CS.

4. **Экспорт в АД:** После синхронизации объекты экспортируются из Active Directory CS в Active Directory.
...
Начните с выбора строки ошибки. (В предыдущем рисунке строка ошибки **синхронизирована с функциями синхронизации с ошибкой-функциями.)** Сначала вам предоставляется обзор объекта. Чтобы просмотреть фактическую ошибку, щелкните **Stack Trace** (Трассировка стека). Эта трассировка отображает информацию об ошибке уровня отладки.
...
Вкладка **Connectors** отображает все пространства разъема, которые имеют представление объекта. 
