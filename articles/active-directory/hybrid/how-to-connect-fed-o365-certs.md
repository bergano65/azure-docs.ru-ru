...
### <a name="renewal-notification-from-the-microsoft-365-admin-center-or-an-email"></a>Уведомление о возобновлении действия в центре администрирования Microsoft 365 или по электронной почте
> [!NOTE]
...
* Если она могла успешно опроса метаданных федерации и получать новые сертификаты, не уведомления по электронной почте или предупреждение в центре администрирования Microsoft 365 выдается пользователю.
* Если не удалось получить новые сертификаты для подписи маркеров, либо потому, что метаданные федерации недоступен или автоматическая смена сертификатов не включена, Azure AD выдает уведомление по электронной почте "и" warning "в центре администрирования Microsoft 365.
...
**1. AD FS свойства AutoCertificateRollover должно быть присвоено значение True.** Это указывает на то, что службы AD FS будут автоматически создавать сертификаты для подписи маркеров и их расшифровки до окончания срока действия старых.

**2. Метаданные федерации AD FS является общедоступным.**  Убедитесь, что метаданные федерации доступны для общего пользования через Интернет (за пределами корпоративной сети), перейдя со своего компьютера по следующему URL-адресу.

https://(ваше_имя_FS)/federationmetadata/2007-06/federationmetadata.xml

где `(your_FS_name)` заменяется на имя узла службы федерации, ваша организация использует, например fs.contoso.com.  Если проверка двух этих параметров оказалась успешной, вам больше не нужно ничего делать.  
...
**Метаданные федерации не является общедоступным**
