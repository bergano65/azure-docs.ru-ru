...
### <a name="renewal-notification-from-the-microsoft-365-admin-center-or-an-email"></a>Уведомление о продлении от центра microsoft 365 по электронной почте или по электронной почте
> [!NOTE]
...
* Если он может успешно опросить метаданные федерации и получить новые сертификаты, пользователю не выдается уведомление по электронной почте или предупреждение в центре админирования Microsoft 365.
* Если он не может получить новые сертификаты подписи токенов, либо потому, что метаданные федерации недоступны или автоматический опрокидывание сертификата не включен, Azure AD выдает уведомление по электронной почте и предупреждение в центре админирования Microsoft 365.
...
где `(your_FS_name)` заменяется имя узла службы федерации, которое использует ваша организация, например fs.contoso.com.  Если проверка двух этих параметров оказалась успешной, вам больше не нужно ничего делать.  
...
3. Выполнить Connect-MsolService - $cred. Этот cmdlet соединяет вас с облачным сервисом. Перед выполнением любых дополнительных командлетов, установленных с помощью средства, необходимо создать контекст, который подключит вас к облачной службе.
