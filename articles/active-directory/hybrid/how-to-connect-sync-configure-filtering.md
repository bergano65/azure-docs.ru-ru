...
Существует два способа синхронизации доменов:
...
#### <a name="select-the-domains-to-be-synchronized-using-the-azure-ad-connect-wizard"></a>Выберите домены для синхронизации с помощью мастера Azure AD Connect
Настройка фильтрации по доменам

1.  Запустите мастер-разъем Azure AD Connect
2.  Щелкните **Настройка**.
3.  Выберите **параметры синхронизации** и нажмите **кнопку «Следующий».**
4.  Введите учетные данные Azure AD.
5.  На **экране подключенных каталогов** нажмите **кнопку Next**.
6.  На **странице фильтрации доменов и OU** нажмите **Refresh**.  Новые домены плохо теперь появляются и удаленные домены исчезнут.
...
3. Для каждого профиля настройте **добавленные** и **удаленные** домены.
    1. Выполните следующие действия для каждого из пяти профилей всех **добавленных** доменов:
        ...
        2. На странице **Configure Step** (Настройка шага) в раскрывающемся меню **Тип** выберите тип шага с тем же именем, что и у настраиваемого профиля. Нажмите **Далее**.  
...
5.  Для завершения конфигурации необходимо запустить **полный импорт** и **синхронизацию Delta.** Продолжить чтение раздела [Применить и проверить изменения](#apply-and-verify-changes).
...
8. Для завершения конфигурации необходимо запустить **полный импорт** и **синхронизацию Delta.** Продолжить чтение раздела [Применить и проверить изменения](#apply-and-verify-changes).
...
7. Щелкните **Add Transformation** (Добавить преобразование), для параметра **FlowType** (Тип потока) выберите значение **Константа**, а для параметра **Целевой атрибут** — значение **cloudFiltered**. В текстовом поле **Источник** введите значение **True**. Нажмите кнопку **Добавить**, чтобы сохранить правило.  
   ![Преобразование входящего объекта 3](./media/how-to-connect-sync-configure-filtering/inbound3.png)
8. Для завершения конфигурации необходимо запустить **полную синхронизацию.** Продолжить чтение раздела [Применить и проверить изменения](#apply-and-verify-changes).
...
11. Щелкните **Add Transformation** (Добавить преобразование), выберите значение **Константа** в качестве параметра **FlowType** (Тип потока) и **cloudFiltered** для параметра **Целевой атрибут**. В поле **Источник** введите значение **True**. Нажмите кнопку **Добавить**, чтобы сохранить правило.  
    ![Преобразование входящего объекта 3](./media/how-to-connect-sync-configure-filtering/inbound3.png)  
12. Для завершения конфигурации необходимо запустить **полную синхронизацию.** Продолжить чтение раздела [Применить и проверить изменения](#apply-and-verify-changes).
...
7. В области навигации слева щелкните **Scoping filter** (Фильтр области), а затем щелкните **Добавить предложение**. В разделе **Атрибут** выберите **mail**. В разделе **Оператор** выберите **ENDSWITH**. В **значение**, введите ** \@contoso.com,** а затем нажмите **Добавить положение**. В разделе **Атрибут** выберите **userPrincipalName**. В разделе **Оператор** выберите **ENDSWITH**. В **значении**, введите ** \@contoso.com**.
8. Нажмите **Сохранить**.
9. Для завершения конфигурации необходимо запустить **полную синхронизацию.** Продолжить чтение раздела [Применить и проверить изменения](#apply-and-verify-changes).
