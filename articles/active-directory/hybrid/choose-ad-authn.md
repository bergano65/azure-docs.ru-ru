---
title: Аутентификация для гибридных решений azure AD
...
Identity — это новая система управления ИТ-безопасностью, поэтому аутентификация — это охранник доступа организации к новому облачному миру. Организации должны определить плоскость управления удостоверениями, которая повышает безопасность и защищает облачные приложения от несанкционированного доступа.

> [!NOTE]
> Изменение метода проверки подлинности требует планирования, тестирования и потенциально простоя. [Поэтапное развертывание](https://docs.microsoft.com/azure/active-directory/hybrid/how-to-connect-staged-rollout) — это отличный способ тестирования и постепенного перехода от проверки подлинности облака к облаку.
...
**Синхронизация хэша паролей Azure AD**. Это самый простой способ включить аутентификацию объектов локального каталога в Azure AD. Пользователи могут использовать те же имя пользователя и пароль, которые они вводят в локальной среде, без необходимости развертывания какой-либо дополнительной инфраструктуры. Некоторые премиум-функции Azure AD, такие как Identity Protection и [Azure AD Domain Services,](../../active-directory-domain-services/active-directory-ds-getting-started-password-sync.md)требуют синхронизации хэш-пароля, независимо от того, какой метод проверки подлинности вы выберете.
...
4. Способы входа, не поддерживаемые Azure AD:
   ...
   * Востретите с помощью решения для проверки подлинности третьих сторон.
   * универсальное локальное решение проверки подлинности.
5. Защита идентификации AD Azure требует синхронизации хэша паролей независимо от того, какой метод вхастывается, чтобы предоставить пользователям отчет об *утечке учетных данных.* Организации могут выйти из системы «Синхронизация хэша паролей», если их основной метод вхлаженного регистра не удается и он был настроен до сбоя события.
...
* **Расширенные сценарии**. При необходимости организации могут использовать аналитические сведения из удостоверений в отчетах службы защиты идентификации Azure AD с Azure AD Premium P2. Например, в отчете об утечке учетных данных. Windows Hello for Business имеет [специфические требования при использовании синхронизации хэша паролей.](https://docs.microsoft.com/windows/access-protection/hello-for-business/hello-identity-verification) [Службы домена Azure AD](../../active-directory-domain-services/active-directory-ds-getting-started-password-sync.md) требуют синхронизации хэша паролей для предоставления пользователям своих корпоративных учетных данных в управляемом домене.

    Организации, требующие многофакторной аутентификации с синхронизацией хэш-паролей, должны использовать многофакторную аутентификацию Azure AD или [пользовательские элементы управления условным доступом.](../../active-directory/conditional-access/controls.md#custom-controls-preview) Эти организации не могут использовать сторонние или внутренние методы многофакторной аутентификации, которые полагаются на федерацию.

> [!NOTE]
> Условный доступ Azure AD требует лицензий [Azure AD Premium P1.](https://azure.microsoft.com/pricing/details/active-directory/)
...
* **Расширенные сценарии**. Пропускная аутентификация обеспечивает соблюдение политики в отношении учетных записей на момент входе в систему. Например, доступ отклоняется, когда состояние учетной записи в помещении пользователя отключено, заблокировано, или [истечет его пароль,](../../active-directory/hybrid/how-to-connect-pta-faq.md#what-happens-if-my-users-password-has-expired-and-they-try-to-sign-in-by-using-pass-through-authentication) или попытка входа в систему выходит за нерабочее время, когда пользователю разрешено войти в систему.

    Организации, которым требуется многофакторная аутентификация с помощью сквозной аутентификации, должны использовать пользовательские элементы контроля Azure Multi-Factor Authentication (MFA) или [условного доступа.](../../active-directory/conditional-access/controls.md#custom-controls-preview) Эти организации не могут использовать сторонний или внутренний метод многофакторной аутентификации, который опирается на федерацию. Для использования расширенных функций требуется, чтобы синхронизация хэша паролей была развернута независимо от того, реализуется ли сквозная аутентификация. Например, это требуется для получения отчетов об утечке учетных данных службы "Защита идентификации".
...
* **Рассмотрение**. Синхронизацию хэша паролей можно использовать в качестве резервного метода аутентификации для сквозной аутентификации на случай, если агентам не удается проверить учетные данные пользователя из-за значительного сбоя в локальной среде. Сбой в синхронизации хэша паролей не происходит автоматически, и вы должны использовать Azure AD Connect для переключения метода регистрации вручную.
...
Ссылка на [реализацию сквозной аутентификации](../../active-directory/hybrid/how-to-connect-pta.md) для этапов развертывания.
...
* **Расширенные сценарии**. Решение для федеративной проверки подлинности обычно требуется, когда требования клиентов к аутентификации изначально не может удовлетворить служба Azure AD. Ознакомьтесь с подробными сведениями, чтобы [выбрать подходящий вариант входа в систему](https://blogs.msdn.microsoft.com/samueld/2017/06/13/choosing-the-right-sign-in-option-to-connect-to-azure-ad-office-365/). Рассмотрим наиболее распространенные требования:

  * Аутентификация, требующая наличия смарт-карт или сертификатов.
  * На местах серверы МИД или сторонних многофакторных провайдеров, требующих федеративного поставщика удостоверений личности.
  ...
  * Войти в этом, что требуетs sAMAccountName, например, DOMAIN-username, а user@domain.comне имя пользователя (UPN), например, .
...
* Требования агента сквозной аутентификации, используя два агента для избыточности:
...
|Существует ли требование к сертификату TLS/SSL?|нет|нет|Да|
...
|Какие типы входа поддерживаются?|UserPrincipalName и пароль<br><br>Интегрированная аутентификация Windows с помощью [бесшовной SSO](../../active-directory/hybrid/how-to-connect-sso.md)<br><br>[Альтернативное имя пользователя](../../active-directory/hybrid/how-to-connect-install-custom.md)|UserPrincipalName и пароль<br><br>Интегрированная аутентификация Windows с помощью [бесшовной SSO](../../active-directory/hybrid/how-to-connect-sso.md)<br><br>[Альтернативное имя пользователя](../../active-directory/hybrid/how-to-connect-pta-faq.md)|UserPrincipalName и пароль<br><br>sAMAccountName и пароль<br><br>Интегрированная аутентификация Windows<br><br>[Аутентификация с использованием сертификатов и смарт-карт](https://docs.microsoft.com/windows-server/identity/ad-fs/operations/configure-user-certificate-authentication)<br><br>[Альтернативное имя пользователя](https://docs.microsoft.com/windows-server/identity/ad-fs/operations/configuring-alternate-login-id)|
...
|Что такое параметры условного доступа?|[Условный доступ Azure AD с премией Azure AD Premium](../../active-directory/conditional-access/overview.md)|[Условный доступ Azure AD с премией Azure AD Premium](../../active-directory/conditional-access/overview.md)|[Условный доступ Azure AD с премией Azure AD Premium](../../active-directory/conditional-access/overview.md)<br><br>[Правила утверждений AD FS](https://adfshelp.microsoft.com/AadTrustClaims/ClaimsGenerator)|
|Поддерживается ли блокировка устаревших протоколов?|[Да](../../active-directory/conditional-access/overview.md)|[Да](../../active-directory/conditional-access/overview.md)|[Да](https://docs.microsoft.com/windows-server/identity/ad-fs/operations/access-control-policies-w2k12)|
...
> [!NOTE]
> Пользовательские элементы управления в Azure AD Conditional Access в настоящее время не поддерживают регистрацию устройств.
...
2. **Обработка локальных сбоев**.  Последствия локальных сбоев из-за кибератаки или аварии могут быть существенными, начиная от ущерба репутации бренда до неспособности организации справиться с атакой. В последнее время многие организации стали жертвами вредоносных атак, в том числе целевых вымогателей, которые вызвали их на территории серверов, чтобы пойти вниз. Помогая клиентам справиться с такими видами атак, корпорация Майкрософт выделила две категории организаций:
   ...
   * Организации, которые ранее не разрешали синхронизацию хэша паролей, вынуждены были прибегать к ненадежным внешним системам электронной почты потребителей для связи для решения проблем. В этих случаях им понадобилось несколько недель, чтобы восстановить свою инфраструктуру идентификации, прежде чем пользователи смогли снова войти в облачные приложения.

3. **Защита идентификации**. Одним из лучших способов защиты пользователей в облаке является "Защита идентификации Azure Active Directory" с Azure AD Premium P2. Корпорация Майкрософт постоянно ищет в Интернете списки пользователей и паролей, которые злоумышленники продают и предоставляют на теневых веб-сайтах. Azure AD может использовать эту информацию, чтобы проверить, были ли скомпрометированы какие-либо имена пользователей и пароли в организации. Поэтому крайне важно обеспечить синхронизацию хэша паролей независимо от того, какой метод проверки подлинности вы используете, будь то федеративная или сквозная аутентификация. Сведения об утечке учетных данных представляются в виде отчета. Эти сведения можно использовать, чтобы блокировать пользователей или вынудить их сменить пароль при попытке выполнить вход с помощью скомпрометированного пароля.
...
Рассмотрите каждый метод аутентификации. Удовлетворяет ли усилия по развертыванию решения и опыт пользователя процесса вхинга в соответствии с вашими бизнес-требованиями? Оцените, нужны ли вашей организации расширенные сценарии и возможности обеспечения непрерывности бизнес-процессов каждого метода аутентификации. Наконец, учтите рекомендации для каждого метода аутентификации. Возможно, какая-либо из них изменит ваше решение.
...
Если вы планируете перейти с федеративной аутентификации на облачную, узнайте больше об [изменении метода входа](../../active-directory/hybrid/plan-connect-user-signin.md). Чтобы помочь вам спланировать и реализовать миграцию, используйте [эти планы развертывания или](https://aka.ms/deploymentplans) рассмотрите возможность использования новой функции [поэтапного развертывания](../../active-directory/hybrid/how-to-connect-staged-rollout.md) для переноса федеративных пользователей на использование проверки подлинности облаков в поэтапном подходе.
