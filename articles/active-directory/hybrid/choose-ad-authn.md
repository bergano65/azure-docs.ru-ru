...
> [!NOTE]
> Для изменения метода проверки подлинности требуется планирование, тестирование и потенциальное время простоя. [Промежуточное развертывание](https://docs.microsoft.com/azure/active-directory/hybrid/how-to-connect-staged-rollout) — это отличный способ тестирования и постепенного перехода с Федерации на облачную аутентификацию.
...
4. Способы входа, не поддерживаемые Azure AD:
   ...
   * Вход с использованием стороннего решения проверки подлинности.
   * универсальное локальное решение проверки подлинности.
5. Защита идентификации Azure AD требуется синхронизация хэша паролей независимо от выбранного метода входа, чтобы предоставить *пользователям отчет с утечкой учетных данных* . Организации могут выполнить отработку отказа в синхронизацию хэша паролей, если их основной метод входа завершается сбоем и был настроен перед событием сбоя.
...
* **Расширенные сценарии**. Сквозная проверка подлинности применяет локальную политику учетных записей во время входа. Например, доступ запрещается, когда состояние учетной записи локального пользователя отключено, заблокировано или [срок его действия истек](../../active-directory/hybrid/how-to-connect-pta-faq.md#what-happens-if-my-users-password-has-expired-and-they-try-to-sign-in-by-using-pass-through-authentication) , или попытка входа в систему находилась не в часы, когда пользователю разрешено выполнять вход.
...
Инструкции по развертыванию см. в статье [Реализация сквозной проверки подлинности](../../active-directory/hybrid/how-to-connect-pta.md) .
...
* **Расширенные сценарии**. Решение федеративной проверки подлинности требуется, когда клиенты имеют требование проверки подлинности, которое Azure AD не поддерживает изначально. Ознакомьтесь с подробными сведениями, чтобы [выбрать подходящий вариант входа в систему](https://blogs.msdn.microsoft.com/samueld/2017/06/13/choosing-the-right-sign-in-option-to-connect-to-azure-ad-office-365/). Рассмотрим наиболее распространенные требования:
  ...
  * Войдите в систему, для которой требуется sAMAccountName, например домен \ имя_пользователя, а не имя участника-пользователя (UPN), user@domain.comнапример.
...
Для домена без поддержки маршрутизации, который не может быть проверен в Azure AD, требуется дополнительные настройки для реализации входа с ИДЕНТИФИКАТОРом пользователя. Это требование называется поддержкой альтернативного имени для входа. Ознакомьтесь со статьей о [настройке альтернативного имени для входа](https://docs.microsoft.com/windows-server/identity/ad-fs/operations/configuring-alternate-login-id), чтобы узнать об ограничениях и требованиях. Если вы решили использовать сторонний поставщик многофакторной проверки подлинности с федерацией, убедитесь, что он поддерживает WS-Trust для присоединения устройств к Azure AD.
...
|Требуется ли потребность в сертификатах TLS/SSL?|Нет|Нет|Да|
...
Рассмотрите каждый метод аутентификации. Предпринимает ли усилия по развертыванию решения, и пользовательский интерфейс входа в систему будет решать ваши бизнес-требования? Оцените, нужны ли вашей организации расширенные сценарии и возможности обеспечения непрерывности бизнес-процессов каждого метода аутентификации. Наконец, учтите рекомендации для каждого метода аутентификации. Возможно, какая-либо из них изменит ваше решение.
