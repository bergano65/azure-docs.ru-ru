---
title: 'Azure AD Connect: Архив журнала выпусков версий | Документация Майкрософт'
description: В этой статье перечислены все архивные выпуски Azure AD Connect и Azure AD Sync
...
ms.locfileid: "94836093"
---
# <a name="azure-ad-connect-version-release-history-archive"></a>Azure AD Connect: Архив журнала выпусков версий
...
>[!NOTE] 
> В этой статье содержатся справочные сведения по всем архивным версиям Azure AD-1.3.20.0 и более ранним версиям.  Сведения о текущих выпусках см. в [журнале выпусков версий Azure AD Connect](reference-connect-version-history.md)
...
- Теперь доступна общедоступная версия интеграции Ping Federate в Azure AD Connect. [Узнайте больше о федерации Azure AD с Ping Federate](./plan-connect-user-signin.md#federation-with-pingfederate).
- Azure AD Connect теперь создает резервную копию отношения доверия Azure AD в AD FS при каждом обновлении и сохраняет его в отдельный файл для простого восстановления в случае необходимости. [Узнайте больше о новых функциях и управлении доверием Azure AD в Azure AD Connect](./how-to-connect-azure-ad-trust.md).
...
- Исправлена ошибка, из-за которой Azure AD Connect серверу отображалась высокая загрузка ЦП после обновления до .NET 4.7.2
...
-  Добавлена поддержка предоставления разрешений на обратную запись групп в мастере установки Azure Адконнект.
...
Дополнительные сведения см. в статье [Microsoft Security Advisory 4056318](/security-updates/securityadvisories/2017/4056318) (Советы по безопасности корпорации Майкрософт №4056318).
...
* Устранена проблема с обновлением Azure AD Connect, которая затрагивала клиентов, у которых включен [простой единый вход](./how-to-connect-sso.md). После обновления Azure AD Connect функция простого единого входа отображалась как отключенная в мастере Azure AD Connect, хотя она оставалась включенной и полностью работоспособной. Теперь благодаря этому исправлению эта функция отображается в мастере как включенная.
...
> [!NOTE]
> Примечание. Служба синхронизации предоставляет интерфейс WMI, который позволяет разрабатывать собственный пользовательский планировщик. Этот интерфейс больше не рекомендуется использовать. Он будет удален из будущих версий Azure AD Connect, которые будут выпущены после 30 июня 2018 г. Клиентам, которым необходимо настроить расписание синхронизации, следует использовать [встроенный планировщик](./how-to-connect-sync-feature-scheduler.md).
#### <a name="fixed-issues"></a>Исправленные проблемы
...
#### <a name="new-features-and-improvements"></a>Новые функции и внесенные улучшения
* Задача проверки имени входа Azure AD Connect в систему ADFS была обновлена, чтобы она проверяла имена входа в Microsoft Online, а не только получение токенов из ADFS.
* Когда вы настраиваете новую ферму ADFS с помощью Azure AD Connect, страница, запрашивающая учетные данные ADFS, была перемещена так, чтобы она находилась до того, как пользователь получит запрос на предоставление серверов ADFS и WAP.  Это позволяет Azure AD Connect проверить, имеет ли указанная учетная запись правильные разрешения.
* Во время обновления Azure AD Connect обновление больше не будет выполняться, если не удается обновить доверие Azure AD ADFS.  В этом случае пользователь получит соответствующее предупреждающее сообщение и продолжит сбросить доверие с помощью Azure AD Connect дополнительной задачи.
...
* Ранее функция [ms-DS-ConsistencyGuid в качестве функции привязки к источнику](./plan-connect-design-concepts.md#using-ms-ds-consistencyguid-as-sourceanchor) была доступна только для новых развертываний. Теперь она доступна для существующих развертываний. В частности:
...
* На основе атрибута **userCertificate** объектов-устройств Azure AD Connect теперь ищет значения сертификатов, необходимых для [подключения присоединенных к домену устройств к Azure AD для работы в Windows 10](../devices/hybrid-azuread-join-plan.md) и отфильтровывает остальные перед синхронизацией с Azure AD. Чтобы включить это поведение, стандартное правило синхронизации Out to AAD - Device Join SOAInAD было обновлено.
...
* Исправлена проблема, которая приводит к тому, что атрибут TrackingId, возвращенный Azure AD, будет пропущен в журналах событий сервера Azure AD Connect. Эта проблема возникала, когда служба Azure AD Connect получала перенаправленное сообщение от Azure AD и Azure AD Connect не удавалось подключиться к указанной конечной точке. TrackingId используется специалистами службы поддержки для сопоставления с журналами службы при устранении неполадок.
...
* Изменения правил синхронизации. Реализованы следующие изменения правил синхронизации:
  ...
  * Добавлена **preferredDataLocation** в схему метавселенной и схему СОЕДИНИТЕЛЯ Azure AD. Клиенты, которым требуется обновить какие-либо атрибуты в Azure AD, могут использовать для этого настраиваемые правила синхронизации. 
  * Добавлен **userType** в схему метавселенной и схему СОЕДИНИТЕЛЯ Azure AD. Клиенты, которым требуется обновить какие-либо атрибуты в Azure AD, могут использовать для этого настраиваемые правила синхронизации.
...
Сброс паролей
* Исправлена проблема, которая может привести к тому, что сервер Azure AD Connect Azure не попытается повторно подключиться, если подключение было прервано брандмауэром или прокси-сервером.
...
* Поддержка глобального администратора с помощью многофакторной идентификации Azure AD и управление привилегированными пользователями в мастере установки.
...
* Поддерживает несколько локальных организаций Exchange. Дополнительные сведения см. в статье [Гибридные развертывания в нескольких лесах Active Directory](/previous-versions/exchange-server/exchange-150/jj873754(v=exchg.150)).
...
* Атрибут ms-DS-ExternalDirectoryObjectID записывается обратно в Active Directory. Эта функция добавляет поддержку для Microsoft 365 приложений. Она использует OAuth2 для доступа к сетевым и локальным почтовым ящикам в гибридном развертывании Exchange.
