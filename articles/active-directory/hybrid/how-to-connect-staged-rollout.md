...
С помощью промежуточного развертывания вы можете перейти из федеративной проверки подлинности на облачную. В этой статье обсуждается, как выполнить такой переход. Однако перед началом промежуточного развертывания следует принять во внимание последствия, которые могут возникнуть, если выполняется одно или несколько из следующих условий:
...
-   Принято решение об использовании одного из двух вариантов:
    - **Вариант A** - *синхронизация хэша паролей (sync)*  + *простой единый вход (SSO)* .
...
-   Если вы планируете использовать Многофакторную идентификацию Azure, рекомендуем [конвергентную регистрацию для самостоятельного сброса пароля (SSPR) и Многофакторной идентификации](../authentication/concept-registration-mfa-sspr-combined.md), чтобы пользователи регистрировали свои методы проверки подлинности один раз.
...
- Вам все равно нужно выполнить окончательный переход из федеративной проверки подлинности в облачную с помощью Azure AD Connect или PowerShell. Промежуточное развертывание не преобразовывает федеративные домены в управляемые.
...
>[!NOTE]
> Так как клиентские конечные точки не отправляют подсказки для входа, они не поддерживаются для промежуточного развертывания.  Приложения SAML используют клиентские конечные точки и также не поддерживаются для промежуточного развертывания.
