...
ms.locfileid: "82112326"
---
# <a name="migrate-to-cloud-authentication-using-staged-rollout-preview"></a>Переход на облачную аутентификацию с помощью промежуточного развертывания (Предварительная версия)
...
-   Было принято решение перейти на один из двух вариантов:
    - **Параметр,** -  + *простой единый вход* (*Sync) для синхронизации хэшей паролей*(SSO)
    - **Вариант B** - .*простой единый вход* для*сквозной проверки подлинности* + 
...
5. Вызов `Get-AzureADSSOStatus | ConvertFrom-Json`метода. Эта команда отображает список лесов Active Directory (см. список "домены"), в которых включена эта функция. По умолчанию для него задано значение false на уровне клиента.

   ![Пример выходных данных Windows PowerShell](./media/how-to-connect-staged-rollout/sr3.png)

6. Вызов `$creds = Get-Credential`метода. В командной строке введите учетные данные администратора домена для предполагаемого Active Directory леса.

7. Вызовите процедуру `Enable-AzureADSSOForest -OnPremCredentials $creds`. Эта команда создает учетную запись компьютера AZUREADSSOACC из локального контроллера домена для Active Directory леса, необходимого для *простого единого входа*.
...
- **Параметр для** -  + *простого единого входа* *синхронизации хэша паролей*
- **Вариант B** - .*простой единый вход* для*сквозной проверки подлинности* + 
- **Неподдерживаемая** -  + *сквозная проверка подлинности* + *синхронизации хэша паролей**простой единый вход*
