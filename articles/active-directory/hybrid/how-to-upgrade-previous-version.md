...
ms.locfileid: "98729996"
---
# <a name="azure-ad-connect-upgrade-from-a-previous-version-to-the-latest"></a>Azure AD Connect: обновление до последней версии
В этой статье описываются различные варианты обновления установленного экземпляра Azure Active Directory (Azure AD) Connect до последней версии.  Действия, описанные в разделе [Обновление со сменой сервера](#swing-migration), можно также использовать при значительных изменениях конфигурации.

>[!NOTE]
> Важно, чтобы серверы были актуальными с использованием последних выпусков Azure AD Connect. Мы постоянно обновляем AADConnect, и эти обновления включают исправления проблем безопасности и ошибок, а также возможность обслуживания, повышения производительности и масштабируемости. Чтобы узнать, что такое последняя версия, и узнать, какие изменения были внесены между версиями, см. [Журнал версий выпуска](./reference-connect-version-history.md) .
