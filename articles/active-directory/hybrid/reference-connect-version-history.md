...
>[!NOTE]
>Выпуск новой версии Azure AD Connect — это процесс, который требует нескольких шагов контроля качества, чтобы обеспечить функциональность работы службы, и пока мы проходим через этот процесс, номер версии нового выпуска, а также статус релиза будут обновлены для отражения самого последнего состояния.
...
>
>Если вы запустите амортизированную версию Azure AD Connect, у вас может не быть последних исправлений безопасности, улучшений производительности, устранения неполадок и диагностических средств и улучшений службы, а если вам требуется поддержка, мы не сможем предоставить вам уровень обслуживание, необходимое вашей организации.
>
...
>[!IMPORTANT]
>В связи с изменением внутренней схемы в этом выпуске Azure AD Connect, если вы управляете настройками конфигурации доверительных отношений ADFS с помощью MSOnline PowerShell, то вы должны обновить модуль MSOnline PowerShell до версии 1.1.183.57 или выше
### <a name="fixed-issues"></a>Исправленные проблемы
...
- Использование админ-регистратора предприятия или домена в качестве учетной записи разъема больше не поддерживается в новых развертываниях AAD Connect. Текущие развертывания AAD Connect с помощью админа Enterprise или Domain в качестве учетной записи разъема не будут затронуты этим выпуском.
...
- Добавлено предупреждение об амортизацию для менеджера службы синхронизации на странице свойств разъема. Это предупреждение уведомляет пользователя о том, что изменения должны быть внесены через мастера AADC.
...
>
> Для решения этой проблемы необходимо импортировать модуль`Set-ADSyncDirSyncConfiguration` **AdSync,** а затем запустить смдлет powershell на сервере Azure AD Connect.  Вы можете использовать следующие действия:
>
>1. Open Powershell в режиме администратора
>2. Выполнить `Import-Module "ADSync"`
>3. Выполнить `Set-ADSyncDirSyncConfiguration -AnchorAttribute ""`
...
* [Предотвращение случайных удалений](how-to-connect-sync-feature-prevent-accidental-deletes.md)
