...
Пока мы проходим через этот процесс, номер версии релиза будет отображаться с "X" в позиции minor release number, как в "1.3.X.0" - это означает, что примечания к выпуску в этом документе действительны для всех версий, начиная с "1.3.". Как только мы завершаем процесс выпуска, номер версии релиза будет обновлен до самой недавно выпущенной версии, а статус релиза будет обновлен до "Выпущен для загрузки и автоматического обновления".
...
>[!IMPORTANT]
> Начиная с 1 ноября 2020 года, мы приступим к внедрению процесса амортизации, в рамках которого будут амортизированы версии Azure AD Connect, выпущенные более 18 месяцев назад. В это время мы начнем этот процесс, унижая все релизы Azure AD Connect с версией 1.3.20.0 (которая была выпущена на 4/24/2019) и старше, и мы приступим к оценке амортизации старых версий Azure AD Connect каждый раз, когда новая версия выпускает.
>
> Для получения оптимального обслуживания необходимо убедиться, что вы работаете с последней версией Azure AD Connect. 
>
>При запуске амортизированной версии Azure AD Connect у вас может не быть последних исправлений безопасности, улучшений производительности, устранения неполадок и диагностических средств и улучшений службы, а если вам требуется поддержка, мы не сможем предоставить вам уровень обслуживания, в который нуждается ваша организация.
>
>Если вы включили Azure AD Connect для синхронизации, вы скоро автоматически начнете получать уведомления о здоровье, которые предупреждают вас о предстоящих изысканиях при запуске одной из старых версий.
>
...
04/02/2020: Выпущен для скачивания
...
>[!NOTE]
>Старый инструмент CSDelete.exe был удален и заменен новым cmdlet Remove-ADSyncCSObject 

### <a name="fixed-issues"></a>Исправленные проблемы

- Исправлена ошибка в селекторе списания группы списания/OU при повторном запуске мастера Azure AD Connect после отключения функции. 
- Введена новая страница ошибки, которая будет отображаться, если требуемые значения реестра DCOM отсутствуют с новой ссылкой справки. Информация также записываются в журнал файлов. 
- Исправлена проблема с созданием учетной записи синхронизации Active Directory Azure, где включение расширений каталога или PHS может вывести из строя, поскольку учетная запись не распространилась по всем репликам службы до попытки использования. 
- Исправлена ошибка в утилите сжатия ошибок синхронизации, которая не была правильно йокерации суррогатных символов. 
- Исправлена ошибка в автоматическом обновлении, которая оставила сервер в состоянии приостановленного посевов. 

## <a name="14380"></a>1.4.38.0
### <a name="release-status"></a>Состояние выпуска
12/9/2019: Выпуск для скачивания. Недоступно через автоматическое обновление.
### <a name="new-features-and-improvements"></a>Новые функции и внесенные улучшения
- Мы обновили синхронизацию паролей для служб доменов Azure AD, чтобы правильно учесть обивку в хэшах Kerberos.  Это обеспечит повышение производительности при синхронизации паролей от AAD до служб домена Azure AD.
- Мы добавили поддержку надежных сеансов между агентом аутентификации и сервисным автобусом.
- Этот релиз обеспечивает TLS 1.2 для связи между агентом аутентификации и облачными службами.
- Мы добавили кэш DNS для подключения веб-сокет между агентом аутентификации и облачными службами.
- Мы добавили возможность таргетинга на конкретный агент из облака для тестирования на подключение агента.

### <a name="fixed-issues"></a>Исправленные проблемы
- Выпуск 1.4.18.0 имел ошибку, в которой cmdlet PowerShell для DSSO использовал учетные данные окон входа вместо учетных данных админ, предоставленных во время работы ps. В результате этого не удалось включить DSSO в нескольких лесах через пользовательский интерфейс AADConnect. 
- Исправление было сделано для включения DSSO одновременно во всех лесах через пользовательский интерфейс AADConnect

## <a name="14320"></a>1.4.32.0
### <a name="release-status"></a>Состояние выпуска
11/08/2019: Выпущен для скачивания. Недоступно через автоматическое обновление.

>[!IMPORTANT]
>В связи с изменением внутренней схемы в этом выпуске Azure AD Connect, если вы управляете настройками конфигурации доверительных отношений AD FS с помощью MSOnline PowerShell, то вы должны обновить модуль MSOnline PowerShell до версии 1.1.183.57 или выше

### <a name="fixed-issues"></a>Исправленные проблемы

Эта версия исправляет проблему с существующими устройствами, присоединенными к гибридным Azure AD. Этот релиз содержит новое правило синхронизации устройства, которое исправляет эту проблему.
Обратите внимание, что это изменение правила может привести к удалению устаревших устройств из Azure AD. Это не вызывает беспокойства, так как эти объекты устройства не используются Azure AD во время авторизации условного доступа. Для некоторых клиентов количество устройств, которые будут удалены в результате этого изменения правила, может превышать порог удаления. Если в Azure AD удаление объектов устройств превышает порог удаления экспорта, рекомендуется разрешить удаление. [Как разрешить поток удаленпри к потоку, когда они превышают порог удаления](how-to-connect-sync-feature-prevent-accidental-deletes.md)

## <a name="14250"></a>1.4.25.0

### <a name="release-status"></a>Состояние выпуска
9/28/2019: Выпущен для автоматического обновления для выбора арендаторов. Недоступен для скачивания.

Эта версия исправляет ошибку, когда некоторые серверы, которые были автоматически обновлены с предыдущей версии до 1.4.18.0 и испытывали проблемы с сбросить пароля самообслуживания (SSPR) и пароль Списания.

### <a name="fixed-issues"></a>Исправленные проблемы

При определенных обстоятельствах серверы, которые были автоматически обновлены до версии 1.4.18.0, не перевносили сбросить пароли самообслуживания и списать пароль после завершения обновления. Этот автоматический релиз обновления исправляет эту проблему и повторно позволяет самообслуживания сбросить паролем и пароль списания.

Мы исправили ошибку в утилите сжатия ошибок синхронизации, которая не была правильно йокерации суррогатных символов.

## <a name="14180"></a>1.4.18.0

>[!WARNING]
>Мы расследуем инцидент, когда некоторые клиенты испытывают проблемы с существующими подключенными устройствами Hybrid Azure AD после обновления до этой версии Azure AD Connect. Мы советуем клиентам, развернувшим гибридную Azure AD, отложить обновление до тех пор, пока не будет полностью понята и устранена первопричина этих проблем. Более подробная информация будет предоставлена как можно скорее.

>[!IMPORTANT]
>С этой версией Azure AD Connect некоторые клиенты могут увидеть, как некоторые или все их устройства Windows исчезают из Azure AD. Это не вызывает беспокойства, так как эти идентификаторы устройств не используются Azure AD во время авторизации условия. Для получения дополнительной информации [см. Понимание Azure AD Connect 1.4.xx.x исчезновение устройства](reference-connect-device-disappearance.md)
...
### <a name="new-features-and-improvements"></a>Новые функции и внесенные улучшения
- Новый инструмент для устранения неполадок помогает устранить неполадки в сценариях "пользователь не синхронизирует", "группа не синхронизирует" или "член группы не синхронизирует".
- Добавить поддержку национальных облаков в сценарий устранения неполадок AAD Connect 
- Клиенты должны быть проинформированы о том, что в настоящее время удалены обесточенные конечные точки WMI для MIIS_Service. Любые операции WMI теперь должны быть сделаны через PS cmdlets.
- Улучшение безопасности путем сброса ограниченной делегации на объекте АЗУРЕССОАКК
- При добавлении/редактировании правила синхронизации, если в правиле используются какие-либо атрибуты, которые находятся в схеме разъема, но не добавляются в разъем, атрибуты автоматически добавляются в разъем. То же самое относится и к типу объекта, на который влияет правило. Если что-то будет добавлено в разъем, разъем будет помечен для полного импорта в следующем цикле синхронизации.
- Использование админа корпоративного или доменного средства в качестве учетной записи разъема больше не поддерживается в новых развертываниях Azure AD Connect. Текущие развертывания AAD Connect с помощью админа Enterprise или Domain в качестве учетной записи разъема не будут затронуты этим выпуском.
- В менеджере синхронизации полная синхронизация работает на создании правил/отсечении/удалении. Всплывающее окно появится на любом изменении правила, уведомляющем пользователя, если будет запущен полный импорт или полная синхронизация.
- Добавлены шаги по смягчению последствий ошибок паролей на страницу «разъемы > свойства> подключения»
- Добавлено предупреждение об амортизацию для менеджера службы синхронизации на странице свойств разъема. Это предупреждение уведомляет пользователя о том, что изменения должны быть внесены через мастер-класс Azure AD Connect.
- Добавлена новая ошибка для проблем с политикой паролей пользователя.
- Предотвращение неправильной настройки групповой фильтрации по домену и фильтрам OU. Фильтрация группы покажет ошибку, когда домен/OU вводной группы уже отфильтрован, и не позволит пользователю двигаться вперед до тех пор, пока проблема не будет решена.
- Пользователи больше не могут создавать разъем для служб доменов Active Directory или Active Directory в пользовательской группе менеджеров синхронизации.
- Фиксированная доступность пользовательских элементов управления пользовательским использованием в менеджере службы синхронизации.
- Включено шесть задач управления федеративностью для всех методов вхлажи в Azure AD Connect.  (Раньше для всех всфов.) была доступна только задача "Обновление Сертификата AD FS TLS/SSL".
- Добавлено предупреждение при изменении метода вхламывания из федерации в PHS или PTA о том, что все домены Azure AD и пользователи будут преобразованы в управляемую аутентификацию.
- Удалены сертификаты подписи токенов из задачи «Перезагрузка Azure AD и AD FS trust» и добавлена отдельная подзадача для обновления этих сертификатов.
- Добавлена новая задача управления федерацией под названием "Управление сертификатами", которая имеет подзадачи для обновления TLS или сертификатов подписи токенов для фермы AD FS.
- Добавлена новая подзадача управления федеративное управление под названием "Указать основной сервер", которая позволяет администраторам указывать новый основной сервер для фермы AD FS.
- Добавлена новая задача управления федеративной системой под названием "Управлять серверами", которая имеет подзадачи для развертывания сервера AD FS, развертывания сервера Web Application Proxy и указания основного сервера.
- Добавлена новая задача управления федеративностью под названием "Посмотреть конфигурацию федерации", которая отображает текущие настройки AD FS.  (Из-за этого дополнения настройки AD FS были удалены со страницы «Обзор решения».

### <a name="fixed-issues"></a>Исправленные проблемы
- Разрешенная проблема ошибки синхронизации для сценария, в которой объект пользователя, захватив свой соответствующий контактный объект, имеет самоссылку (например, пользователь является их собственным менеджером).
- Справка всплывающие окно теперь показывают на клавиатуре фокус.
- Для обновления Auto, если любое конфликтующее приложение работает от 6 часов, убить его и продолжить обновление.
- Ограничьте количество атрибутов, которые клиент может выбрать до 100 на объект при выборе расширений каталога. Это предотвратит ошибку во время экспорта, так как Azure имеет максимум 100 атрибутов расширения на объект.
- Исправлена ошибка, чтобы сделать сценарий Подключения AD более надежным
- Исправлена ошибка, чтобы сделать AADConnect установить на машине с помощью существующей службы Named Pipes WCF более надежной.
- Улучшенная диагностика и устранение неполадок вокруг групповых политик, которые не позволяют службе ADSync запускать службу ADSync при первоначальном установке.
- Исправлена ошибка, при которой имя отображения компьютера Сwindows было написано неправильно.
- Исправить ошибку, когда тип ОС для компьютера Windows был написан неправильно.
- Исправлена ошибка, при которой компьютеры, не связанные с Windows 10, синхронизировались неожиданно. Обратите внимание, что эффект этого изменения заключается в том, что компьютеры без Windows-10, которые ранее были синхронизированы, теперь будут удалены. Это не влияет на какие-либо функции, поскольку синхронизация компьютеров Windows используется только для соединения гибридного aure AD, который работает только для устройств Windows-10.
- Добавлено несколько новых (внутренних) cmdlets в модуль ADSync PowerShell.


## <a name="13210"></a>1.3.21.0
>[!IMPORTANT]
>Существует известная проблема с обновлением Azure AD Connect с более ранней версии до 1.3.21.0, где портал O365 не отражает обновленную версию, даже несмотря на то, что Azure AD Connect успешно обновлен.
>
> Для решения этой проблемы необходимо импортировать модуль`Set-ADSyncDirSyncConfiguration` **AdSync,** а затем запустить cmdlet PowerShell на сервере Azure AD Connect.  Вы можете использовать следующие действия:
>
>1. Open PowerShell в режиме администратора.
>2. Выполните `Import-Module "ADSync"`.
>3. Выполните `Set-ADSyncDirSyncConfiguration -AnchorAttribute ""`.
 
### <a name="release-status"></a>Состояние выпуска 

05/14/2019: Выпущен для скачивания

### <a name="fixed-issues"></a>Исправленные проблемы 

- Исправлена уязвимость уязвимости привилегий, которая существует в сборке Microsoft Azure Active Directory Connect 1.3.20.0.  Эта уязвимость при определенных условиях может позволить злоумышленнику выполнить два cmdlets PowerShell в контексте привилегировой учетной записи и выполнять привилегированные действия.  Это обновление безопасности решает проблему, отключая эти cmdlets. Для получения дополнительной [информации см.](https://portal.msrc.microsoft.com/security-guidance/advisory/CVE-2019-1000)

## <a name="13200"></a>1.3.20.0 

### <a name="release-status"></a>Состояние выпуска 

04/24/2019: Выпущен для скачивания

### <a name="new-features-and-improvements"></a>Новые функции и внесенные улучшения 

- Добавление поддержки для обновления домена 
- Обмен почта общественных Фоллеры функция идет GA 
- Улучшение обработки ошибок мастера для сбоев службы 
- Добавлена предупреждающая ссылка на uI-менеджера службы синхронизации на странице свойств разъема. 
- Функция списания объединенных групп теперь GA 
- Улучшенное сообщение об ошибке SSPR, когда DC отсутствует элемент управления LDAP 
- Добавлена диагностика ошибок реестра DCOM при установке  
- Улучшенная отслеживание ошибок PHS RPC 
- Разрешить EA creds из детского домена 
- Разрешить вводить имя базы данных во время установки (имя ADSync по умолчанию)
- Обновление до ADAL 3.19.8, чтобы подобрать исправление WS-Trust для Ping и добавить поддержку для новых экземпляров Azure 
- Изменение правил групповой синхронизации для потока samAccountName, DomainNetbios и DomainF-DN в облако - необходимо для претензий 
- Модифицированные правила синхронизации по умолчанию Обработка - подробнее [здесь](how-to-connect-fix-default-rules.md).
- Добавлен новый агент, работающий в качестве службы windows. Этот агент, названный «Агент администратора», позволяет более глубокой удаленной диагностики сервера Azure AD Connect, чтобы помочь инженерам Майкрософт устранить неполадки при открытии дела поддержки. Этот агент не устанавливается и включен по умолчанию.  Для получения дополнительной информации о том, как установить и включить агента, смотрите [Что такое AD Azure Connect Admin Agent?](whatis-aadc-admin-agent.md). 
- Обновлено соглашение о лицензии конечных пользователей (EULA) 
- Добавлена поддержка автоматического обновления для развертываний, которые используют AD FS в качестве типа входа.  Это также устранила требование об обновлении AD FS Azure AD Relying Party Trust в рамках процесса обновления. 
- Добавлена задача управления доверием Azure AD, которая предоставляет два варианта: анализ/обновление доверия и сбросить доверие. 
- Изменило поведение AD FS Azure AD Relying Party, чтобы он всегда использует переключатель -SupportMultipleDomain (включает в себя обновления домена Доверия и Azure AD). 
- Изменилось поведение фермы AD FS таким образом, чтобы для этого потребовался сертификат .pfx, удалив опцию использования предустановленного сертификата.
- Обновлено установка нового рабочего процесса фермы AD FS, чтобы он развязал только 1 AD FS и 1 сервер WAP.  Все дополнительные серверы будут сделаны после первоначальной установки. 

### <a name="fixed-issues"></a>Исправленные проблемы 

- Исправление логики повторного подключения для службы ADSync 
- Исправление, чтобы позволить очистить установку с помощью пустого DB S'L AOA 
- Исправление скрипта PS Permissions для уточнения разрешений GWB 
- Исправление ошибок VSS с помощью LocalDB  
- Исправление вводящего в заблуждение сообщения об ошибке, когда тип объекта не находится в области 
- Исправлена проблема, при которой установка Azure AD PowerShell на сервере потенциально может привести к конфликту сборки с Подключением КОН. 
- Исправлена ошибка PHS на промежуточном сервере при обновлении учетных данных connector в uI-uI менеджера синхронизации. 
- Исправлено некоторые утечки памяти 
- Разные исправления автообновления 
- Различные исправления для экспорта и неподтвержденной обработки импорта 
- Исправлена ошибка при обработке подстычки в фильтрации домена и OU 
- Исправлена проблема, при которой служба ADSync останавливается более 2 минут и вызывает проблему во время обновления. 
...
12/18/2018: Выпущен для скачивания
...
### <a name="release-status"></a>Состояние выпуска
12/11/2018: Выпущен для скачивания
...
11/30/2018: Выпущен для скачивания
...
Версия для скачивания выпущена 19.11.2018
...
- В мастер Azure AD Connect добавлена диагностика для исследования и выявления проблем с подключением к Azure AD. Эти же диагностики также могут быть запущены непосредственно через PowerShell с помощью теста- AdSyncAzureServiceConnectivity Cmdlet. 
- В мастер Azure AD Connect добавлена диагностика для исследования и выявления проблем с подключением к AD. Эти же диагностики также могут быть запущены непосредственно через PowerShell с помощью функции Start-ConnectivityValidation в модуле ADConnectivityTools PowerShell.  Дополнительные сведения см. в разделе [Что такое модуль PowerShell ADConnectivityTools?](how-to-connect-adconnectivitytools.md)
...
- Azure AD Connect теперь создает резервную копию отношения доверия Azure AD в AD FS при каждом обновлении и сохраняет его в отдельный файл для простого восстановления в случае необходимости. [Узнайте больше о новой функциональности и доверительном управлении Azure AD в Azure AD Connect.](https://aka.ms/fedtrustinaadconnect)
...
- Исправлена ошибка, при которой сервер AAD Connect показал бы высокое использование процессора после обновления до .NET 4.7.2
...
- Исправлена ошибка, чтобы предотвратить ошибку, допущенную из-за неправильной многопоточной передачи в мастере
...
* На странице **Вход пользователя** параметр "Синхронизация паролей" изменен на "Синхронизация хэша паролей".  Azure AD Connect синхронизирует хэши паролей, а не пароли, что и отражено в этом изменении.  Для получения дополнительной информации [см. синхронизацию хэша паролей «Реализация» с синхронизацией Azure AD Connect](how-to-connect-password-hash-synchronization.md)
...
> [!NOTE]
> Примечание. Служба синхронизации имеет интерфейс WMI, который позволяет разрабатывать собственный пользовательский планировщик. Этот интерфейс больше не рекомендуется использовать. Он будет удален из будущих версий Azure AD Connect, которые будут выпущены после 30 июня 2018 г. Клиенты, желающие настроить график синхронизации, должны использовать [встроенный планировщик.](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnectsync-feature-scheduler)
...
* Есть известная проблема с обновлением Azure AD Connect, которая затрагивает клиентов, у которых включен [простой единый вход](how-to-connect-sso.md). После обновления Azure AD Connect компонент отображается как отключенный в мастере, несмотря на то что он остается включенным. Эта проблема будет решена в следующем выпуске. Клиенты, которые обеспокоены этой проблемой отображения, могут вручную исправить ее, включив простой единый вход в мастере.
...
### <a name="ad-fs-management"></a>Управление AD FS
#### <a name="fixed-issues"></a>Исправленные проблемы
* Инициализация-ADSyncNGKeysWriteBack cmdlet в модуле AD prep PowerShell неправильно применяла ACLs к контейнеру регистрации устройства и, следовательно, только наследует существующие разрешения.  Это было обновлено, чтобы учетная запись службы синхронизации имела правильные разрешения.
...
* Существует проблема, которая затрагивает клиентов, использующих [фильтрацию OU с](how-to-connect-sync-configure-filtering.md#organizational-unitbased-filtering) синхронизацией Azure AD Connect. При переходе на [страницу фильтрации домена и OU](how-to-connect-install-custom.md#domain-and-ou-filtering) в мастере Azure AD Connect ожидается следующее поведение:
...
* Когда Azure AD Connect получает ошибку LargeObject от Azure AD, Azure AD Connect генерирует событие с EventID 6941 и сообщение *"Подготовленный объект слишком велик. Обрезать количество значений атрибутов на этом объекте.* В то же время Azure AD Connect также создает вводящее в заблуждение событие с идентификатором EventID 6900 и сообщением *"Microsoft.Online.Coexistence.ProvisionRetryException: Не удается установить связь со службой Windows Azure Active Directory".* Во избежание путаницы Azure AD Connect больше не создает такое событие при возникновении ошибки LargeObject.
...
* Azure AD Connect теперь автоматически обеспечивает использование атрибута ConsistencyGuid в качестве атрибута привязки к источнику для локальных объектов AD. Более того, Azure AD Connect заполняет атрибут ConsistencyGuid значением атрибута objectGuid, если оно пустое. Эта функция применима только к новому развертыванию. Дополнительные сведения об этой функции см. в [разделе "Использование ms-DS-ConsistencyGuid в качестве sourceAnchor" статьи "Azure AD Connect: принципы проектирования"](plan-connect-design-concepts.md#using-ms-ds-consistencyguid-as-sourceanchor).
...
* Ранее, если вы изменили пароль учетной записи службы синхронизации Azure AD Connect, служба синхронизации не могла правильно запуститься, пока вы не сбросите ключ шифрования и не выполните повторную инициализацию пароля учетной записи этой службы. Теперь этот процесс не требуется.
...
* Исправлена проблема, которая вызывает ошибку "Изображение имеет якорь, который отличается от изображения", чтобы произойти на сервере Azure AD Connect в промежуточном режиме, после того как вы временно исключили находящийся объект AD из синхронизации, а затем включил его снова для синхронизации.
...
Управление AD FS
* Добавлена поддержка для обновления сертификата AD FS Farm TLS/SSL.
...
* Когда вы настраиваете [расширения каталога](how-to-connect-sync-feature-directory-extensions.md) в мастере Azure AD Connect, вы можете выбрать атрибут Azure AD, относящийся к типу "строка Teletex".
...
* Службу Azure AD Connect теперь можно устанавливать на FIPS-совместимом сервере.
  * Дополнительные сведения о синхронизации паролей см. в разделе [Синхронизация хэшированных паролей и FIPS](how-to-connect-password-hash-synchronization.md#password-hash-synchronization-and-fips).
...
* Добавлена поддержка облака [Microsoft Cloud Germany](reference-connect-instances.md#microsoft-cloud-germany).
* Добавлена поддержка последней инфраструктуры для [облака Microsoft Azure для государственных организаций](reference-connect-instances.md#microsoft-azure-government) с новым перечнем URL-адресов.
...
* Добавлена поддержка многозначных атрибутов для [расширений каталогов](how-to-connect-sync-feature-directory-extensions.md).
* Добавлена поддержка большего числа вариантов конфигурации для соответствия условиям [автоматического обновления](how-to-connect-install-automatic-upgrade.md).
* Добавлены некоторые командлеты для [настраиваемого планировщика](how-to-connect-sync-feature-scheduler.md#custom-scheduler).
...
* [Automatic upgrade](how-to-connect-install-automatic-upgrade.md) для тех, кто использует стандартные параметры.
...
* Разрешите [фильтрацию доменов и подразделений](how-to-connect-install-custom.md#domain-and-ou-filtering) в мастере установки. Это также позволяет подключиться к лесам, в которых доступны не все домены.
* [Планировщик](how-to-connect-sync-feature-scheduler.md) встроен в модуль синхронизации.

**Функции, повышенные с предварительной версии до общедоступной версии:**

* [Устройство списания](how-to-connect-device-writeback.md).
* [Расширения каталога](how-to-connect-sync-feature-directory-extensions.md).

**Новые функции предварительной версии:**

* По умолчанию интервал синхронизации цикла составляет 30 минут. Для всех более ранних версий он составлял 3 часа. Добавлена поддержка изменения поведения [планировщика](how-to-connect-sync-feature-scheduler.md).
...
* [Azure AD подключите здоровье для синхронизации](how-to-connect-health-sync.md).
* Поддержка синхронизации паролей [доменных служб Azure AD](../user-help/active-directory-passwords-update-your-own-password.md).

**Новый поддерживаемый сценарий:**

* Поддерживает несколько локальных организаций Exchange. Дополнительные сведения см. в статье [Гибридные развертывания в нескольких лесах Active Directory](https://docs.microsoft.com/previous-versions/exchange-server/exchange-150/jj873754(v=exchg.150)).
...
* Функция [обратной записи пользователя](how-to-connect-preview.md#user-writeback) предварительной версии была временно удалена на основе отзывов клиентов этой версии. Она будет повторно добавлена позже, когда мы изучим полученные отзывы.
...
* Установка [настроек экспресса](how-to-connect-install-express.md)
* возможность [настройки AD FS](how-to-connect-install-custom.md#configuring-federation-with-ad-fs);
* возможность [обновления из DirSync](how-to-dirsync-upgrade-get-started.md);
* [Предотвращение случайного удаления](how-to-connect-sync-feature-prevent-accidental-deletes.md)
* Добавлен [промежуточный режим](how-to-connect-sync-staging-server.md)

**Новые функции предварительной версии:**

* [Обратная запись пользователей](how-to-connect-preview.md#user-writeback)
* [Обратная запись групп](how-to-connect-preview.md#group-writeback)
* [Списание устройств](how-to-connect-device-writeback.md)
* [Расширения каталогов](how-to-connect-preview.md)
...
* Теперь поддерживается синхронизация паролей с фильтрацией на основе атрибутов. Дополнительные сведения см. в статье [Службы синхронизации Azure AD Connect: настройка фильтрации](how-to-connect-sync-configure-filtering.md).
* Атрибут ms-DS-ExternalDirectoryObjectID записывается обратно в Active Directory. Эта функция добавляет поддержку приложений Office 365. Он использует OAuth2 для доступа к онлайн и на территории почтовых ящиков в гибридной разложениExchange.
...
1. Удалите правило синхронизации.
1. Найдите новое правило синхронизации, созданное службой Azure AD Sync, и повторно внесите изменения.
