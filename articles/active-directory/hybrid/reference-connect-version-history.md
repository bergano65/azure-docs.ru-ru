...
>[!IMPORTANT]
>Имеется известная проблема с обновлением Azure AD Connect из более ранней версии до 1.3.21.0, где на портале Office 365 не отражает обновленные версии, несмотря на то, что Azure AD Connect обновлен успешно.
>
> Чтобы устранить это необходимо импортировать **AdSync** модуля, а затем выполните`Set-ADSyncDirSyncConfiguration` командлет powershell на сервере Azure AD Connect.  Можно использовать следующие действия:
>
...
- Исправлена повышения привилегий устраняет уязвимость в Microsoft Azure Active Directory Connect сборки 1.3.20.0.  Этой уязвимости, при определенных условиях может позволить злоумышленнику выполнить два командлета powershell в контексте привилегированной учетной записи и выполнения привилегированных действий.  Данное обновление устраняет проблему, отключив эти командлеты. Дополнительные сведения см. в разделе [обновление безопасности](https://portal.msrc.microsoft.com/en-US/security-guidance/advisory/CVE-2019-1000).
...
- Добавлен новый агент запущен как служба windows. Этот агент, с именем «Агент-администратор», позволяет подробной диагностики, удаленного сервера Azure AD Connect для устранения неполадок при открытии обращение в службу поддержки инженерами корпорации Майкрософт. Этот агент не установлен и включен по умолчанию.  Дополнительные сведения о том, как установить и включить агент см. в разделе [что такое агент Azure AD Connect администратор?](whatis-aadc-admin-agent.md). 
...
* Исправлена проблема, которая вызывает ошибку «образ имеет привязку, отличный от изображение» на сервере Azure AD Connect в промежуточном режиме, после временного исключения локально объекта из синхронизации AD, а затем его включения синхронизации.
