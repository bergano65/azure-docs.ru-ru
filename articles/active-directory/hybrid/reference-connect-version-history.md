...
>[!IMPORTANT]
>Компьютеры Windows, зарегистрированные как гибридные присоединенные к Azure AD, представлены в Azure AD как объекты устройств. Эти объекты устройств можно использовать для условного доступа. Компьютеры Windows 10 синхронизируются с облаком с помощью Azure AD Connect, компьютеры Windows нижнего уровня регистрируются напрямую с помощью AD FS или простого единого входа.
>
>Только компьютеры Windows 10 с указанным значением атрибута userCertificate, настроенным с помощью гибридного подключения Azure AD, должны быть синхронизированы с облаком Azure AD Connect.  В предыдущих версиях Azure AD Connect это требование не было строго применено, что привело к ненужным объектам устройств в Azure AD. Такие устройства в Azure AD всегда неизменности в состоянии "ожидание", так как эти компьютеры не были предназначены для регистрации в Azure AD.
>
>Эта версия Azure AD Connect синхронизирует только компьютеры Windows 10, которые правильно настроены для гибридного присоединения к Azure AD. Azure AD Connect никогда не должны синхронизировать [устройства Windows нижнего уровня](../../active-directory/devices/hybrid-azuread-join-plan.md#windows-down-level-devices).  Все устройства в Azure AD, которые ранее были синхронизированы неправильно, теперь будут удалены из Azure AD.  Однако это изменение не приведет к удалению устройств Windows, которые были правильно зарегистрированы в Azure AD для гибридного присоединение к Azure AD. 
>
>Некоторые клиенты могут увидеть некоторые или все их устройства с Windows из Azure AD. Это не является причиной проблемы, так как эти удостоверения устройств не используются Azure AD во время авторизации условного доступа. Некоторым клиентам может потребоваться перейти [к следующим инструкциям: Спланируйте реализацию](../../active-directory/devices/hybrid-azuread-join-plan.md) гибридного подключения Azure Active Directory, чтобы убедиться, что компьютеры Windows зарегистрированы правильно, и убедитесь, что такие устройства могут полностью участвовать в условном доступе на основе устройств. Если Azure AD Connect пытается удалить [устройства Windows нижнего уровня](../../active-directory/devices/hybrid-azuread-join-plan.md#windows-down-level-devices) , это значит, что устройство не было создано [корпорацией Майкрософт Workplace Join для MSI-файла компьютеров не на основе Windows 10](https://www.microsoft.com/download/details.aspx?id=53554) и не может использоваться другими функциями Azure AD.  Если вы видите, что удаление объектов компьютеров или устройств в Azure AD превышает пороговое значение удаления экспорта, рекомендуется, чтобы клиент пропускал эти удаления.

### <a name="release-status"></a>Состояние выпуска
9/10/2019: Выпущено только для автоматического обновления
...
- Использование администратора предприятия или домена, так как учетная запись соединителя больше не поддерживается.
...
- Исправление ошибки, при которой отображаемое имя компьютера Windows было неправильно написано.
- Исправление ошибки, при которой тип ОС для компьютера Windows был неправильно записан.
...
05/14/2019: выпущена версия для скачивания.
...
04/24/2019: выпущена версия для скачивания.
...
Allow    | Прошедшие проверку подлинности пользователи           | Вывод списка содержимого        | этому объекту  |
Allow    | Прошедшие проверку подлинности пользователи           | Чтение всех свойств  | этому объекту  |
Allow    | Прошедшие проверку подлинности пользователи           | Разрешения на чтение     | этому объекту  |
...
* Добавлен следующий набор функций, совместимых с сертификатом X509Certificate2, чтобы создать выражения правила синхронизации для обработки значений сертификата в атрибуте userCertificate:
    ...
    |CertVersion|CertSignatureAlgorithmOid|Выбор|
