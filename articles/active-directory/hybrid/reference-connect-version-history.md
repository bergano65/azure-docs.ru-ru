...
Пока мы проверим этот процесс, номер версии выпуска будет отображаться как "X" в дополнительной версии номера выпуска, как в "1.3. X. 0" — это означает, что заметки о выпуске в этом документе действительны для всех версий, начинающихся с "1,3". Как только мы завершаем процесс выпуска, номер версии выпуска будет обновлен до последней выпущенной версии, а состояние выпуска будет изменено на "выпущено для скачивания и автоматического обновления".
...
>[!IMPORTANT]
> Начиная с 1 ноября 2020, мы начнем реализовать процесс устаревания, при котором версии Azure AD Connect, которые были выпущены более 18 месяцев назад, будут считаться устаревшими. В это время мы начнем этот процесс, не выполнив все выпуски Azure AD Connect с версией 1.3.20.0 (которая была выпущена в 4/24/2019) и более ранних версий, и мы будем продолжать оценивать устаревшие версии Azure AD Connect каждый раз при выпуске новых версий.
>
> Необходимо убедиться, что вы используете последнюю версию Azure AD Connect для получения оптимального интерфейса поддержки. 
>
...
- Исправлена проблема, при которой невыбранные домены были неправильно выбраны в пользовательском интерфейсе мастера.
- Исправлена проблема в модуле PowerShell Адсинкконфиг, где вызов команды DSACLS, используемой во всех командлетах разрешений Set-ADSync *, вызовет одну из следующих ошибок:
...
### <a name="fixed-issues"></a>Исправленные проблемы
Эта сборка исправления устраняет проблему в сборке 1.5.20.0, если вы выполнили клонирование правила **объединения из AD-Group** и не выполнили клонирование **общего правила в из группы AD** .
...
### <a name="fixed-issues"></a>Исправленные проблемы
Эта сборка исправления устраняет проблему с построением 1.5.18.0, если включена функция фильтрации групп и в качестве привязки к источнику используется mS-DS-ConsistencyGuid.
...
- Добавлена поддержка функции mS-DS-ConsistencyGuid для групповых объектов. Это позволяет перемещать группы между лесами или повторно подключать группы в AD к Azure AD, где изменился objectID группы AD, например, когда сервер AD перестраивается после бедствия. Дополнительные сведения см. в статье [Перемещение групп между лесами](how-to-connect-migrate-groups.md).
...
- Удален параметр Get-Адсинкрунпрофиле, так как он больше не используется. 
...
- Добавлен новый командлет для удаления объектов из пространства соединителя. старое средство Ксделете. exe удаляется и заменяется новым командлетом Remove-Адсинкксобжект. Командлет Remove-Адсинкксобжект принимает в качестве входных данных Ксобжект. Этот объект можно получить с помощью командлета Get-Адсинкксобжект.

>[!NOTE]
>Старое средство Ксделете. exe было удалено и заменено новым командлетом Remove-Адсинкксобжект 
...
- Исправлена проблема с созданием учетной записи синхронизации Azure Active Directory, в которой Включение расширений каталогов или PHS может завершиться неудачей, так как учетная запись не распространяется на все реплики службы перед попыток использования. 
- Исправлена ошибка в служебной программе сжатия ошибок синхронизации, в которой неправильно обработаны суррогатные символы. 
- Исправлена ошибка в автоматическом обновлении, которая оставляет сервер в состоянии приостановлена планировщиком. 
- Исправлена ошибка на странице фильтрации доменов и подразделений, которая удалит профили запуска домена, просто выполнив частичное расширение дерева доменов без внесения каких-либо изменений.
...
- В этом выпуске для обмена данными между агентом проверки подлинности и облачными службами применяется протокол TLS 1,2.
- Мы добавили кэш DNS для подключений WebSocket между агентом аутентификации и облачными службами.
- Мы добавили возможность ориентироваться на конкретный агент из облака, чтобы проверить наличие подключения к агенту.

### <a name="fixed-issues"></a>Исправленные проблемы
- В выпуске 1.4.18.0 возникла ошибка, при которой командлет PowerShell для ДССО использовал учетные данные Windows для входа вместо учетных данных администратора, предоставленных при выполнении PS. В результате невозможно включить ДССО в нескольких лесах с помощью пользовательского интерфейса AADConnect. 
- Было сделано исправление, позволяющее одновременно включить ДССО во всем лесу с помощью пользовательского интерфейса AADConnect
...
>[!IMPORTANT]
>Из-за внутреннего изменения схемы в этом выпуске Azure AD Connect, если вы управляете AD FS параметрами конфигурации отношений доверия с помощью MSOnline PowerShell, необходимо обновить модуль PowerShell MSOnline до версии 1.1.183.57 или выше.
...
Обратите внимание, что это изменение правила может привести к удалению устаревших устройств из Azure AD. Это не является причиной проблемы, так как эти объекты устройств не используются Azure AD во время авторизации условного доступа. Для некоторых клиентов количество устройств, которые будут удалены с помощью этого правила, может превышать пороговое значение удаления. Если вы видите, что удаление объектов устройств в Azure AD превышает пороговое значение удаления экспорта, рекомендуется разрешить удаление. [Как разрешить удаление в поток, если они превышают пороговое значение удаления](how-to-connect-sync-feature-prevent-accidental-deletes.md)

## <a name="14250"></a>1.4.25.0

### <a name="release-status"></a>Состояние выпуска
9/28/2019: выпущено автоматическое обновление для выбора клиентов. Недоступно для скачивания.

Эта версия устраняет ошибку, из-за которой некоторые серверы, которые были автоматически обновлены с предыдущей версии, 1.4.18.0 и испытывают проблемы с самостоятельным сбросом пароля (SSPR) и обратной записью паролей.
...
>[!WARNING]
>Мы изучаем инцидент, в котором у некоторых клиентов возникла проблема с существующими гибридными устройствами, присоединенными к Azure AD, после обновления до этой версии Azure AD Connect. Мы советуем клиентам, которые развернули гибридное присоединение к Azure AD, отложить обновление до этой версии, пока не будут полностью понятны и устранены основные причины этих проблем. Дополнительные сведения будут предоставлены как можно скорее.

>[!IMPORTANT]
>В этой версии Azure AD Connect некоторые клиенты могут увидеть некоторые или все их устройства с Windows, которые исчезают из Azure AD. Это не является причиной проблемы, так как эти удостоверения устройств не используются Azure AD во время авторизации условного доступа. Дополнительные сведения см. в статье [об Azure AD Connect 1.4. XX. x дисаппеарнце устройства](reference-connect-device-disappearance.md)


### <a name="release-status"></a>Состояние выпуска
9/25/2019: выпущено только для автоматического обновления.

### <a name="new-features-and-improvements"></a>Новые функции и внесенные улучшения
- Новые средства устранения неполадок помогают устранять неполадки в сценариях "пользователь не синхронизируется", "Группа не синхронизирована" или "член группы не синхронизирован".
- Добавление поддержки национальных облаков в сценарии устранения неполадок в AAD Connect 
- Клиентам следует сообщить о том, что устаревшие конечные точки WMI для MIIS_Service были удалены. Все операции WMI теперь следует выполнять с помощью командлетов PS.
- Улучшение безопасности за счет сброса ограниченного делегирования для объекта AZUREADSSOACC
- При добавлении или изменении правила синхронизации при наличии каких-либо атрибутов, используемых в правиле, которые находятся в схеме соединителя, но не добавляются в соединитель, атрибуты автоматически добавляются в соединитель. То же справедливо и для типа объекта, на который влияет правило. Если к соединителю добавлены какие-либо данные, соединитель будет помечен для полного импорта в следующем цикле синхронизации.
- Использование администратора предприятия или домена, так как учетная запись соединителя больше не поддерживается в новых Azure AD Connect развертываниях. Текущие развертывания AAD Connect, использующие администратора предприятия или домена, так как учетная запись соединителя не будут затронуты этим выпуском.
...
- Добавлены действия по устранению ошибок паролей для "соединители > свойства > подключения"
- Добавлено предупреждение об устаревании для диспетчера службы синхронизации на странице свойств соединителя. Это предупреждение уведомляет пользователя о том, что изменения должны быть сделаны с помощью мастера Azure AD Connect.
- Добавлена новая ошибка для проблем с политикой паролей пользователя.
- Предотвращение ненужной настройки фильтрации групп по фильтрам доменов и подразделений. При фильтрации групп будет отображаться сообщение об ошибке, когда домен или подразделение указанной группы уже отфильтровано, и пользователь не сможет передвигаться вперед до устранения проблемы.
- Пользователи больше не могут создавать соединитель для домен Active Directory служб или Windows Azure Active Directory в пользовательском интерфейсе Synchronization Service Manager.
- Исправлена доступность пользовательских элементов управления пользовательского интерфейса в Synchronization Service Manager.
- Включены шесть задач управления Федерации для всех методов входа в Azure AD Connect.  (Ранее только задача "обновление AD FS TLS/SSL-сертификат" была доступна для всех входов в систему.)
- Добавлено предупреждение при изменении метода входа с Федерации на PHS или PTA, что все домены и пользователи Azure AD будут преобразованы в управляемую проверку подлинности.
- Удалены сертификаты для подписи маркера из задачи "Сброс Azure AD и AD FS Trust" и добавлена отдельная подзадача для обновления этих сертификатов.
- Добавлена новая задача управления Федерации под названием "Управление сертификатами", которая содержит подзадачи для обновления сертификатов TLS или подписи маркеров для AD FS фермы.
- Добавлена новая подзадача управления Федерации с именем "Указание основного сервера", которая позволяет администраторам указать новый сервер-источник для фермы AD FS.
- Добавлена новая задача управления Федерации под названием "Управление серверами", которая содержит подзадачи для развертывания AD FS сервера, развертывания прокси-сервера и указания основного сервера.
- Добавлена новая задача управления Федерации с именем "Просмотр конфигурации Федерации", которая отображает текущие параметры AD FS.  (Из-за этого добавления AD FS параметры были удалены со страницы "Проверка решения".)

### <a name="fixed-issues"></a>Исправленные проблемы
- Устранена проблема с ошибкой синхронизации для сценария, в котором объект пользователя, передаваемый по соответствующему контактному объекту, имеет собственную ссылку (например, пользователь является собственным руководителем).
- Всплывающие окна справки теперь отображаются в фокусе клавиатуры.
- Для автоматического обновления, если любое конфликтующее приложение работает в течение 6 часов, удалите его и продолжайте обновление.
- Ограничьте число атрибутов, которые клиент может выбрать в 100 для каждого объекта при выборе расширений каталога. Это предотвратит возникновение ошибки во время экспорта, так как Azure имеет максимум 100 атрибутов расширения на объект.
- Исправлена ошибка, которая делает сценарий подключения Active Directory более надежным
- Исправлена ошибка для установки AADConnect на компьютере с использованием существующей службы WCF именованных каналов, которая стала более надежной.
- Улучшена диагностика и устранение неполадок групповых политик, которые не допускают запуск службы ADSync при первоначальной установке.
- Исправлена ошибка, при которой отображаемое имя компьютера Windows было написано неправильно.
- Исправление ошибки, при которой тип ОС для компьютера Windows был неправильно записан.
- Исправлена ошибка, при которой компьютеры, не относящиеся к Windows 10, были непредвиденно синхронизированы. Обратите внимание, что результат этого изменения состоит в том, что компьютеры, не являющиеся компьютерами Windows-10, которые были ранее синхронизированы, теперь будут удалены. Это не влияет на функции, так как синхронизация компьютеров Windows используется только для гибридного присоединение к домену Azure AD, которое работает только для устройств Windows 10.
...
- Исправлена уязвимость повышения привилегий, которая существует в Microsoft Azure Active Directory Connect сборки 1.3.20.0.  Эта уязвимость при определенных условиях может позволить злоумышленнику выполнить два командлета PowerShell в контексте привилегированной учетной записи и выполнить привилегированные действия.  Это обновление для системы безопасности устраняет эту ошибку, отключая эти командлеты. Дополнительные сведения см. в разделе [Обновление безопасности](https://portal.msrc.microsoft.com/security-guidance/advisory/CVE-2019-1000).
...
- Поддержка общих папок почты Exchange 
- Улучшение обработки ошибок мастера при сбоях служб 
- Добавлена ссылка на предупреждение в Synchronization Service Manager пользовательском интерфейсе на странице свойств соединителя. 
- Функция обратной записи Объединенных групп теперь является общедоступной 
- Улучшенное сообщение об ошибке SSPR, когда в контроллере домена отсутствует элемент управления LDAP 
- В ходе установки Добавлена диагностика ошибок реестра DCOM  
- Улучшенная трассировка ошибок RPC PHS 
- Разрешить учетные данными EA из дочернего домена 
- Разрешить вход в базу данных при установке (имя по умолчанию ADSync)
- Выполните обновление до ADAL 3.19.8, чтобы получить исправление WS-Trust для проверки связи и добавить поддержку для новых экземпляров Azure. 
- Изменение правил синхронизации групп для потока samAccountName, DomainNetbios и DomainFQDN в Cloud — требуется для заявок 
...
- Добавлен новый агент, работающий как служба Windows. Этот агент с именем "агент администратора" позволяет более глубокую удаленную диагностику сервера Azure AD Connect, чтобы помочь специалистам Майкрософт в устранении неполадок при открытии обращения в службу поддержки. Этот агент не установлен и не включен по умолчанию.  Дополнительные сведения об установке и включении агента см. в разделе [что такое Агент администрирования Azure AD Connect?](whatis-aadc-admin-agent.md). 
- Обновлено лицензионное соглашение (EULA) 
- Добавлена поддержка автоматического обновления для развертываний, в которых в качестве типа входа используется AD FS.  При этом также было удалено требование обновить отношение доверия AD FS с проверяющей стороной Azure AD в рамках процесса обновления. 
- Добавлена задача управления доверием Azure AD, которая предоставляет два варианта: анализ/обновление доверия и сброс доверия. 
- Изменен AD FS поведение доверия проверяющей стороны Azure AD, чтобы оно всегда использовало параметр-SupportMultipleDomain (включает в себя отношения доверия и обновления домена Azure AD). 
- Изменилось поведение "установить новое AD FS ферму", чтобы ему требовался сертификат PFX, удалив вариант использования предварительно установленного сертификата.
- Обновлен рабочий процесс "Установка нового AD FS фермы", позволяющий развертывать только 1 AD FS и 1 сервер WAP.  Все дополнительные серверы будут выполнены после первоначальной установки. 

### <a name="fixed-issues"></a>Исправленные проблемы 

- Исправление логики повторного подключения SQL для службы ADSync 
- Исправление для разрешения чистой установки с использованием пустой базы данных SQL AOA DB 
- Исправление сценария разрешений PS для уточнения разрешений ГВБ 
- Исправление ошибок VSS с помощью LocalDB  
- Исправлять сообщение об ошибке в заблуждение, если тип объекта не находится в области 
...
- Прочие исправления для обновления 
- Прочие исправления для экспорта и неподтвержденной обработки импорта 
- Исправлена ошибка с обработкой обратной косой черты при фильтрации доменов и подразделений. 
- Исправлена проблема, из-за которой служба ADSync длится более 2 минут и вызывает проблему во время обновления. 
...
11/30/2018: выпущено для скачивания
...
- В мастер Azure AD Connect добавлена диагностика для исследования и выявления проблем с подключением к Azure AD. Эти же диагностические данные также можно запустить непосредственно через PowerShell с помощью командлета Test-Адсинказуресервицеконнективити. 
- В мастер Azure AD Connect добавлена диагностика для исследования и выявления проблем с подключением к AD. Эти же диагностические данные также можно запустить непосредственно через PowerShell с помощью функции start-Коннективитивалидатион в модуле PowerShell Адконнективититулс.  Дополнительные сведения см. в разделе [Что такое модуль PowerShell ADConnectivityTools?](how-to-connect-adconnectivitytools.md)
...
- Azure AD Connect теперь создает резервную копию отношения доверия Azure AD в AD FS при каждом обновлении и сохраняет его в отдельный файл для простого восстановления в случае необходимости. [Узнайте больше о новых возможностях и управлении доверием Azure AD в Azure AD Connect](https://aka.ms/fedtrustinaadconnect).
...
- Исправлена ошибка, при которой сервер AAD Connect покажет высокую загрузку ЦП после обновления до .NET 4.7.2
...
- Исправлена ошибка, предотвращающая возникновение ошибки из-за неправильной обработки нескольких потоков в мастере.
...
* На странице **Вход пользователя** параметр "Синхронизация паролей" изменен на "Синхронизация хэша паролей".  Azure AD Connect синхронизирует хэши паролей, а не пароли, что и отражено в этом изменении.  Дополнительные сведения см. [в статье реализация синхронизации хэшей паролей с Azure AD Connect синхронизацией](how-to-connect-password-hash-synchronization.md) .
...
### <a name="ad-fs-management"></a>Управление AD FS
#### <a name="fixed-issues"></a>Исправленные проблемы
* Командлет Initialize-Адсинкнгккэйсвритебакк в модуле PowerShell для подготовки AD неправильно применяет списки ACL к контейнеру регистрации устройств и поэтому наследует только существующие разрешения.  Это было обновлено, чтобы учетная запись службы синхронизации имела правильные разрешения.
...
* Существует проблема, влияющая на клиентов, использующих [фильтрацию по подразделениям](how-to-connect-sync-configure-filtering.md#organizational-unitbased-filtering) с Azure AD Connect синхронизацией. При переходе на [страницу фильтрации доменов и ПОДразделений](how-to-connect-install-custom.md#domain-and-ou-filtering) в мастере Azure AD Connect ожидается следующее поведение:
...
* Когда Azure AD Connect получает ошибку LargeObject из Azure AD, Azure AD Connect создает событие с кодом EventID 6941 и сообщением *"подготовленный объект слишком велик. Сократите число значений атрибутов для этого объекта ".* В то же время Azure AD Connect также создает вводящее в заблуждение событие с идентификатором EventID 6900 и сообщением *"Microsoft.Online.Coexistence.ProvisionRetryException: Не удается установить связь со службой Windows Azure Active Directory".* Во избежание путаницы Azure AD Connect больше не создает такое событие при возникновении ошибки LargeObject.
...
* Исправлена проблема, которая приводит к ошибке "у изображения есть привязка, которая отличается от изображения" на Azure AD Connect сервере в промежуточном режиме, после того как вы временно исправили локальный объект AD из синхронизации и снова включили его для синхронизации.
...
Управление AD FS
* Добавлена поддержка обновления сертификата TLS или SSL AD FS фермы.
...
* [Обратная запись устройства](how-to-connect-device-writeback.md).
* [Расширения каталогов](how-to-connect-sync-feature-directory-extensions.md).
...
* Установка стандартных [параметров](how-to-connect-install-express.md)
...
* [Обратная запись устройств](how-to-connect-device-writeback.md)
...
* Атрибут ms-DS-ExternalDirectoryObjectID записывается обратно в Active Directory. Эта функция добавляет поддержку приложений Office 365. Он использует OAuth2 для доступа к сетевым и локальным почтовым ящикам в гибридном развертывании Exchange.
