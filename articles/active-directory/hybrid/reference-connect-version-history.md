...
>[!IMPORTANT]
>Ранее компьютеры Windows нижнего уровня, присоединенные к локальной службе AD, были неправильно синхронизированы с облаком в некоторых обстоятельствах. Например, значение атрибута userCertificate для устройств нижнего уровня Windows в AD заполняется. Но такие устройства в Azure AD всегда неизменности в состоянии "ожидание", так как эти версии ОС не были предназначены для регистрации в Azure AD с помощью AAD Sync. В этой версии Azure AD Connect AAD Sync останавливает синхронизацию компьютеров Windows нижнего уровня с Azure AD и также удалит ранее неправильно синхронизированные устройства Windows нижнего уровня из Azure AD. Обратите внимание, что это изменение не приведет к удалению устройств Windows нижнего уровня, которые были правильно зарегистрированы в Azure AD с помощью пакета MSI. Эти устройства будут работать должным образом в целях условного доступа на основе устройств. Некоторые клиенты могут увидеть некоторые или все устройства Windows нижнего уровня, которые не будут отображаться в Azure AD. Это не является причиной проблемы, так как эти удостоверения устройств никогда не использовались Azure AD во время авторизации условного доступа. Таким клиентам может потребоваться повторное посещение https://docs.microsoft.com/azure/active-directory/devices/hybrid-azuread-join-plan устройств Windows нижнего уровня и их получение, чтобы гарантировать, что такие устройства могут полностью участвовать в условном доступе на основе устройств. Обратите внимание, что если вы видите, что эти удаления объектов компьютеров или устройств нижнего уровня в Azure AD превышают пороговое значение удаления экспорта, рекомендуется разрешить этим удаленным клиентам выполнять эти удаления.
...
Тип     | Название                          | Access               | Применяется к
---------|-------------------------------|----------------------|--------------|
...
РАЗРЕШИТЬ    | Прошедшие проверку подлинности пользователи           | Вывод списка содержимого        | этому объекту  |
РАЗРЕШИТЬ    | Прошедшие проверку подлинности пользователи           | Чтение всех свойств  | этому объекту  |
РАЗРЕШИТЬ    | Прошедшие проверку подлинности пользователи           | Разрешения на чтение     | этому объекту  |
...
* Атрибут ms-DS-ExternalDirectoryObjectID записывается обратно в Active Directory. Эта функция добавляет поддержку приложений Office 365. Она использует OAuth2 для доступа к сетевым и локальным почтовым ящикам в гибридном развертывании Exchange.
