...
Да. Все возможности условного доступа, включая многофакторную аутентификацию Azure, работают с пропускной проверкой.
...
Войдите в систему использования значения, не связанного с UPN, например альтернативной электронной почты, в настоящее время тестируется в частном предварительном просмотре как для сквозной аутентификации (PTA), так и для синхронизации хэша паролей (PHS).
...
## <a name="what-happens-when-i-switch-from-password-hash-synchronization-to-pass-through-authentication"></a>Что происходит при переходе от синхронизации хэша паролей к пропускной проверке подлинности?

При использовании Azure AD Connect для переключения метода синхронизации с момента синхронизации паролей на проверку подлинности пропускной связи, проверка подлинности прохода становится основным методом вступления в систему для пользователей в управляемых доменах. Обратите внимание, что все хэши паролей пользователей, которые ранее были синхронизированы синхронизацией с помощью синхронизации паролей, хранятся в Azure AD.
...
- Агенты аутентификации делают запросы HTTP по порту 80 для загрузки списков аннулирования сертификатов TLS/SSL (CRLs).
...
Обмен данными между каждым агентом сквозной проверки подлинности и Azure AD защищен с помощью проверки подлинности на основе сертификата. Эти [сертификаты автоматически обновляются каждые несколько месяцев Azure AD.](how-to-connect-pta-security-deep-dive.md#operational-security-of-the-authentication-agents) Нет необходимости вручную продлевать их действие. Старые просроченные сертификаты при необходимости можно удалить.
...
Да. Мультилесные среды поддерживаются, если между лесами Active Directory существуют лесные трасты (двусторонние) и правильно настраивается суффикс наятерна.
...
Рекомендуется включать или отключать сквозную аутентификацию, используя облачную учетную запись глобального администратора. Узнайте о [добавлении учетной записи Глобального администратора только](../active-directory-users-create-azure-portal.md)для облака. Выполните эти инструкции, чтобы не потерять доступ к клиенту.
