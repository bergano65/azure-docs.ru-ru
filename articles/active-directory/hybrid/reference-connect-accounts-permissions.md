...
![Обзор счетов](media/reference-connect-accounts-permissions/account5.png)
...
>[!IMPORTANT]
> По состоянию на 1.4.  Если вы попытаетесь ввести учетную запись, которая является корпоративным админом или админ-аминтом при указании **использования существующей учетной записи,** вы получите ошибку.

> [!NOTE]
> Он поддерживается для управления административными учетными записями, используемыми в Azure AD Connect из административного леса ESAE (также известного как "Красный лес").
...
> Чтобы узнать больше о выделенных административных лесов, пожалуйста, обратитесь к [ESAE административного лесного дизайна подход](https://docs.microsoft.com/windows-server/identity/securing-privileged-access/securing-privileged-access-reference-material#esae-administrative-forest-design-approach).

> [!NOTE]
> Роль Глобального администратора не требуется после первоначальной настройки, и единственной необходимой учетной записью будет ролевая учетная запись **синхронизации учетных записей директоров.** Это не обязательно означает, что вы хотите просто удалить учетную запись с ролью Глобального администратора. Лучше изменить роль на менее мощную роль, так как полное удаление учетной записи может привести к проблемам, если вам когда-нибудь понадобится повторно запустить мастера снова. Уменьшая привилегию роли, вы всегда можете повторно повысить привилегии, если вам придется снова использовать мастера Azure AD Connect. 
...
| --- | --- | --- | --- |
| Недоступно |Пользователь, запускающий мастер установки |Администратор локального сервера |<li>Создание учетной записи службы ADSync, используемой для запуска службы синхронизации. |
...
| Серверы AD FS |Для каждого сервера в списке мастер собирает учетные данные, когда учетных данных пользователя, управляемого мастером, недостаточно для подключения |Администратор домена |Установка и настройка роли сервера AD FS. |
| Прокси-серверы веб-приложений |Для каждого сервера в списке мастер собирает учетные данные, когда учетных данных пользователя, управляемого мастером, недостаточно для подключения |Локальный администратор на целевом компьютере |Установка и настройка роли сервера WAP. |
...
| Учетная запись службы AD FS (параметр "Использовать учетную запись пользователя домена") |Учетные данные пользователя AD |Пользователь домена |Учетная запись пользователя Azure AD, учетные данные которой предоставляются, используется в качестве учетной записи ввне службы AD FS. |
...
| --- | --- |
| функция ms-DS-ConsistencyGuid |Напишите разрешения на атрибут ms-DS-ConsistencyGuid, задокументированный в [концепции дизайна - Использование ms-DS-ConsistencyGuid](plan-connect-design-concepts.md#using-ms-ds-consistencyguid-as-sourceanchor)в качестве источника. | 
...
| **Контроллер домена** | **Учетная запись домена** | *gMSA*</br>**Учетная запись домена**</br>sMSA| *gMSA*</br>**Учетная запись домена**|
...
>[!NOTE]
>Прежде чем использовать вышеперечисленные команды PowerShell, необходимо установить [модуль Azure Active Directory PowerShell для графического модуля](https://docs.microsoft.com/powershell/azure/active-directory/install-adv2?view=azureadps-2.0#installing-the-azure-ad-module) и подключиться к экземпляру Azure AD с помощью [Connect-AzureAD](https://docs.microsoft.com/powershell/module/azuread/connect-azuread?view=azureadps-2.0)

Для получения дополнительной информации о том, как управлять или сбросить пароль для учетной записи Azure AD Connect, можно получить [информацию об управлении учетной записью Azure AD Connect](how-to-connect-azureadaccount.md)
