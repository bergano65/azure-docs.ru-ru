---
title: 'Azure AD Connect: Настройка предпочтительного расположения данных для ресурсов Office 365'
...
В этом разделе описано, как настроить атрибут для расположения предпочтительных данных в службе синхронизации Azure Active Directory (Azure AD) Connect. Когда кто-то пользуется несколькими географическими возможностями в Office 365, этот атрибут используется для обозначения географического расположения данных пользователя Office 365. (Термины *регион* и *геообъект* являются взаимозаменяемыми.)
...
> [!IMPORTANT]
> В настоящее время поддерживается поддержка нескольких географических пользователей с активной Соглашение Enterprise и минимум 500 подписок на службы Office 365. Обратитесь к представителю корпорации Майкрософт, чтобы получить дополнительные сведения.
>
>

Список всех геообъектов для Office 365 можно найти в разделе [где находятся ваши данные?](https://aka.ms/datamaps).
...
По умолчанию **preferredDataLocation** не включен для синхронизации. Эта функция предназначена для более крупных организаций. Схема Active Directory в Windows Server 2019 имеет атрибут **msDS-preferredDataLocation** , который следует использовать для этой цели. Если вы не обновили схему Active Directory и не можете сделать это, то необходимо будет задать атрибут для хранения географических пользователей Office 365. Этот атрибут будет индивидуальным для каждой организации.
