---
title: 'Azure AD Connect выполняет следующие функции: Настройка предпочтительного расположения данных для ресурсов Office 365'
...
| ЮАР | ЗАФ |
| Объединенные Арабские Эмираты | МЕЩАЮТСЯ |
...
## <a name="step-3-add-preferreddatalocation-to-the-azure-ad-connector-schema"></a>Шаг 3. Добавьте **preferredDataLocation** в схему соединителя Azure AD.
...
4. На вкладке **Описание** укажите следующую конфигурацию.
    ...
    | --- | --- | --- |
    | Название | *Укажите имя* | Например, In from AD – User PreferredDataLocation. |
...
6. Перейдите на вкладку **Преобразование** и реализуйте следующее правило преобразования.
    ...
    | --- | --- | --- | --- | --- |
    |Прямой | preferredDataLocation | Выберите исходный атрибут. | Флажок снят. | Обновление |
...
4. На вкладке **Описание** укажите следующую конфигурацию.
    ...
    | Атрибут | Значение | Подробнее |
    | ----- | ------ | --- |
    | Название | *Укажите имя* | Например, Out to AAD – User PreferredDataLocation. |
...
5. Перейдите на вкладку **Scoping filter** (Фильтр области) и добавьте отдельную группу фильтра области с двумя предложениями.
    ...
    | --- | --- | --- |
    | sourceObjectType | EQUAL | пользователя |
...
6. Перейдите на вкладку **Преобразование** и реализуйте следующее правило преобразования.
    ...
    | Flow type (Тип потока) | Целевой атрибут | Source | Применить однократно | Merge Type (Тип объединения) |
    | --- | --- | --- | --- | --- |
    | Прямой | preferredDataLocation | preferredDataLocation | Флажок снят. | Обновление |
...
## <a name="step-6-run-full-synchronization-cycle"></a>Шаг 6. Запуск цикла полной синхронизации
...
5. Проверьте **ожидающие операции экспорта** в Azure AD.
   ...
   2. В диалоговом окне **Search Connector Space** (Поиск пространства соединителя) сделайте следующее.
        ...
        1\. Для параметра **Scope** (Область) укажите значение **Pending Export** (Ожидающая операция экспорта).<br>
        2\. Установите все три флажка: **"Добавить", "Изменить" и "Удалить"** .<br>
...
## <a name="step-7-re-enable-sync-scheduler"></a>Шаг 7. Повторно включите планировщик синхронизации
...
## <a name="step-8-verify-the-result"></a>Шаг 8. Проверьте результат.
