---
title: 'Подключение Azure AD: Переход из федерации в PHS для Azure AD Документы Майкрософт'
...
> [!NOTE]
> Изменение метода проверки подлинности требует планирования, тестирования и потенциально простоя. [Поэтапное развертывание](how-to-connect-staged-rollout.md) предоставляет альтернативный способ тестирования и постепенного перехода от федеративной системы к облачной аутентификации с помощью синхронизации хэш-паролей.
...
Вы можете настроить Azure AD Connect с помощью стандартных параметров или выборочной установки. При использовании опции пользовательской установки [необходимые разрешения](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-accounts-permissions) на синхронизацию хэша паролей могут оказаться ненаместо.
...
Чтобы гибридное соединение продолжало работать для любых устройств, которые соединяются с доменом после того, как ваши домены преобразуются в синхронизацию хэш-пароля, для клиентов Windows 10 необходимо использовать параметры устройства Azure AD Connect для синхронизации компьютера Active Directory учетные записи в Azure AD. 
...
### <a name="step-1-enable-password-hash-synchronization"></a>Шаг 1: Включить синхронизацию хэша паролей
...
2. Выберите **параметры синхронизации,** а затем выберите **Следующий.**
...
### <a name="step-2-prepare-for-seamless-sso"></a>Шаг 2: Подготовка к бесшовной SSO
...
### <a name="step-3-change-the-sign-in-method-to-password-hash-synchronization-and-enable-seamless-sso"></a>Шаг 3: Измените метод синхронизации хэша паролей и включите бесшовную SSO

Доступны два варианта изменения метода входа на синхронизацию хэша паролей и включения простого единого входа.

#### <a name="option-a-switch-from-federation-to-password-hash-synchronization-by-using-azure-ad-connect"></a>Вариант A: Переключение от федерирования к синхронизации хэша пароля с помощью Azure AD Connect
...
3. На странице **Подключение к Azure AD** введите имя пользователя и пароль глобального администратора.
4. На странице **входной** системы пользователя выберите **кнопку синхронизации хэша Password.** Обязательно установите флажок **Не преобразовывать учетные записи пользователей**. Этот параметр является устаревшим. Выберите **Включить единый вход** и щелкните **Далее**.
...
6. Убедитесь, что на странице **Готово к настройке** установлен флажок **Start the synchronization process when configuration completes** (Начать синхронизацию после завершения настройки). Затем выберите **налаживание.**
...
Перейдите к разделу [Тестирование и дальнейшие действия](#testing-and-next-steps).
   ...
   > [!IMPORTANT]
   > Пропустить раздел **Вариант B: Переключитесь с федерирования на синхронизацию хэша пароля с помощью Azure AD Connect и PowerShell.** Если вы выбрали вариант A для изменения метода входа на синхронизацию хэша паролей и включения простого единого входа, действия из этого раздела не применяются.

#### <a name="option-b-switch-from-federation-to-password-hash-synchronization-using-azure-ad-connect-and-powershell"></a>Вариант B: Переключение от федерирования к синхронизации хэша паролей с помощью Azure AD Connect и PowerShell
...
4. На странице **входа пользователей** нажмите кнопку **Синхронизация хэша паролей**. Выберите **Включить единый вход** и щелкните **Далее**.
   ...
   Перед включением синхронизации хэша ![паролей: скриншот, на который показана опция «Не настраивать» на странице входной вариант пользователя](media/plan-migrate-adfs-password-hash-sync/migrating-adfs-to-phs_image12.png)<br />

   После включения синхронизации хэша ![паролей: скриншот, отображающий новые опции на странице входной системы пользователя](media/plan-migrate-adfs-password-hash-sync/migrating-adfs-to-phs_image13.png)<br />
...
[Синхронизация хэш-ходовых ходов и хэш-ходов для работы с неполадкой Azure Active Directory](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization)
