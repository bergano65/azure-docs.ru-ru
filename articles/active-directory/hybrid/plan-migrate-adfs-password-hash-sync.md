...
Вы можете настроить Azure AD Connect с помощью стандартных параметров или выборочной установки. Если вы использовали параметр выборочной установки, [необходимые разрешения](./reference-connect-accounts-permissions.md) для синхронизации хэша паролей могут отсутствовать.
...
Утверждение **InsideCorporateNetwork** недоступно после преобразования доменов для использования синхронизации хэша паролей. Вместо этой функции вы можете использовать [именованные расположения в Azure AD](../reports-monitoring/quickstart-configure-named-locations.md).
...
Чтобы обеспечить работу гибридного соединения для всех устройств, присоединенных к домену после преобразования доменов в синхронизацию хэшей паролей, для клиентов Windows 10 необходимо использовать параметры Azure AD Connect устройства для синхронизации учетных записей компьютеров Active Directory в Azure AD. 
...
> [!NOTE]
> Фирменная символика организации доступна только при приобретении лицензии Premium или Basic для Azure Active Directory или при наличии лицензии на Microsoft 365.
...
После развертывания синхронизации хэша паролей и простого единого входа пользователь получает доступ к Microsoft 365 и другим ресурсам, которые проходят проверку подлинности с помощью изменений Azure AD. Для пользователей за пределами вашей сети будет отображаться только страница входа в Azure AD. Они не будут перенаправляться на страницу на основе форм, предоставляемую интерфейсными прокси-серверами веб-приложения.
...
Дополнительные сведения см. в статье [Устранение неполадок синхронизации хэшированных паролей в службе синхронизации Azure AD Connect](./tshoot-connect-password-hash-synchronization.md).
...
3. На странице **Подключение к Azure AD** введите имя пользователя и пароль глобального администратора.
4. На странице **входа пользователя** нажмите **кнопку Синхронизация хэша паролей**. Обязательно установите флажок **Не преобразовывать учетные записи пользователей**. Этот параметр является устаревшим. Выберите **Включить единый вход** и щелкните **Далее**.
...
[Устранение неполадок Azure Active Directory синхронизации хэшей паролей](./tshoot-connect-password-hash-synchronization.md)

[Устранение неполадок с простым единым входом Azure Active Directory](./tshoot-connect-sso.md)
