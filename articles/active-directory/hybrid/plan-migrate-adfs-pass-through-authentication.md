---
title: 'Подключение Azure AD: Переход из федерации в PTA для Azure AD'
...
> [!NOTE]
> Изменение метода проверки подлинности требует планирования, тестирования и потенциально простоя. [Поэтапное развертывание](how-to-connect-staged-rollout.md) предоставляет альтернативный способ тестирования и постепенного перехода от федерации к проверке подлинности облака с помощью сквозной аутентификации.
...
Чтобы обновить Azure AD Connect, выполните этапы в [Azure AD Connect: Обновление до последней версии.](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-upgrade-previous-version)
...
1. На сервере Azure AD Connect откройте Azure AD Connect. Выберите **настройку.**
...
3. В рамках **дополнительных задач > Федерации управления**прокрутите в **Службу Федерации активных каталогов (AD FS).**<br />

   * Если в этом разделе есть конфигурация AD FS, можно считать, что службы AD FS были изначально настроены с помощью Azure AD Connect. Вы можете преобразовать федеративные домены в управляемые с помощью параметра **Смена имени пользователя для входа** в Azure AD Connect. Для получения дополнительной информации о процессе смотрите раздел **Вариант A: Настройка сквозной аутентификации с помощью Azure AD Connect**.
   * Если службы федерации Active Directory не указаны в текущих параметрах, необходимо вручную перевести домены с федеративных удостоверений на управляемые с помощью PowerShell. Для получения дополнительной информации об этом процессе смотрите раздел **Вариант B: Переход от федерации к сквозной аутентификации с помощью Azure AD Connect и PowerShell.**
...
| Вы используете AD FS, чтобы заблокировать старые версии клиентов проверки подлинности.| Рассмотрите возможность замены элементов управления AD FS, которые блокируют более ранние версии клиентов аутентификации, используя комбинацию [элементов управления условным доступом](https://docs.microsoft.com/azure/active-directory/conditional-access/conditions) и [правил доступа к клиентам Exchange Online.](https://aka.ms/EXOCAR) |
...
| Вы применяете политики управления доступом (правила AuthZ) в AD FS для управления доступом к Office 365.| Рассмотрите возможность замены политик и эквивалентными [политиками условного доступа](https://docs.microsoft.com/azure/active-directory/active-directory-conditional-access-azure-portal) Azure AD и [правилами доступа к клиентам Exchange Online.](https://aka.ms/EXOCAR)|
...
После настройки названных местоположений необходимо обновить все политики условного доступа, которые были настроены либо на включение или исключение сети **Все доверенные местоположения** или значения **IFA Trusted IPs,** чтобы отразить новые названные местоположения.

Для получения дополнительной информации о состоянии **местоположения** в [условных точках](https://docs.microsoft.com/azure/active-directory/active-directory-conditional-access-locations)доступа см.

#### <a name="hybrid-azure-ad-joined-devices"></a>Устройства, использующие гибридное присоединение к Azure AD

При присоединении к устройству в Azure AD можно создать правила условного доступа, которые обеспечивают соответствие устройств стандартам доступа к безопасности и соответствию требованиям. Кроме того, пользователи могут входить на устройство с помощью рабочей или учебной учетной записи организации вместо личной. Использование гибридного присоединения к Azure AD позволяет присоединять к Azure Active Directory устройства, которые присоединены к домену Azure AD. Эта функция могла быть настроена в старой федеративной среде.
...
### <a name="step-1-prepare-for-seamless-sso"></a>Шаг 1: Подготовка к бесшовной SSO
...
По умолчанию веб-браузеры автоматически вычисляют соответствующую зону (Интернет или интрасеть) по URL-адресу. Например, **http:\/\/contoso/maps** to the intranet zone и **http:\/\/intranet.contoso.com** maps to the internet zone (потому что URL содержит период). Браузеры отправляют билеты Kerberos в облачную конечную точку (например, на URL-адрес Azure AD), только если ее URL-адрес добавлен явным образом в зону интрасети в браузере.
...
### <a name="step-2-change-the-sign-in-method-to-pass-through-authentication-and-enable-seamless-sso"></a>Шаг 2: Изменить метод входной системы для сквозной аутентификации и включить бесшовные SSO

Вы можете изменить метод входа на сквозную проверку подлинности и включить эффективный единый вход двумя способами.

#### <a name="option-a-configure-pass-through-authentication-by-using-azure-ad-connect"></a>Вариант A: Настройка сквозной аутентификации с помощью Azure AD Connect
...
2. Выберите **Изменение пользователя ввесение,** а затем выберите **Следующий**. 
...
6. Убедитесь, что на странице **Готово к настройке** установлен флажок **Start the synchronization process when configuration completes** (Начать синхронизацию после завершения настройки). Затем выберите **налаживание.**<br />
...
1. На портале Azure перейдите на **Azure Active Directory** > **Azure AD Connect,** а затем выберите **проверку подлинности Pass-through.**
...
> [!IMPORTANT]
> Пропустить раздел **Вариант B: Переключитесь из федерации на сквозную аутентификацию с помощью Azure AD Connect и PowerShell.** Действия из этого раздела не применяются, если вы выбрали вариант 1 для изменения метода входа на синхронизацию хэша паролей и включения эффективного единого входа. 

#### <a name="option-b-switch-from-federation-to-pass-through-authentication-by-using-azure-ad-connect-and-powershell"></a>Вариант B: Переход от федерации к сквозной аутентификации с помощью Azure AD Connect и PowerShell
...
8. Выберите **проверку подлинности Pass-through** и убедитесь, что статус **является активным.**<br />
...
2. Перейти в офис 365 войти[https://portal.office.com](https://portal.office.com/)в страницу ().
...
* Выберите [правильную аутентификацию.](https://docs.microsoft.com/azure/security/fundamentals/choose-ad-authn)
