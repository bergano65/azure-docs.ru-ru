...
Чтобы узнать текущие настройки федерации, выполните командлет **Get-MsolDomainFederationSettings**:
...
* Сведения о том, кто и когда будет создавать и отправлять эти сообщения.
...
### <a name="remove-the-relying-party-trust"></a>Удаление отношений доверия проверяющей стороны
...
Обновления атрибута **userPrincipalName**, использующего службы синхронизации из локальной среды, традиционно блокируются, если не выполняются сразу оба следующих условия:

* пользователь размещен в управляемом домене (нефедеративном);
