---
title: 'Azure AD Connect: миграция из Федерации в PTA для Azure AD'
...
Чтобы обновить Azure AD Connect, выполните действия, описанные в разделе [Azure AD Connect: обновление до последней версии](./how-to-upgrade-previous-version.md).
...
3. В разделе **Дополнительные задачи > управление Федерации** прокрутите до **службы федерации Active Directory (AD FS) (AD FS)**.<br />
   ...
   * Если службы федерации Active Directory не указаны в текущих параметрах, необходимо вручную перевести домены с федеративных удостоверений на управляемые с помощью PowerShell. Дополнительные сведения об этом процессе см. в разделе **вариант б. Переключение из Федерации в сквозную аутентификацию с помощью Azure AD Connect и PowerShell**.
...
>
> Вместо этого используйте облачную службу многофакторной идентификации Azure AD для выполнения той же функции. Внимательно проанализируйте требования многофакторной проверки подлинности, прежде чем действовать дальше. Прежде чем преобразовывать домены, убедитесь, что вы понимаете, как использовать многофакторную идентификацию Azure AD, влияние на лицензирование и процесс регистрации пользователей.

#### <a name="back-up-federation-settings"></a>Резервное копирование параметров федерации

В ходе процесса, описанного в этой статье, другие проверяющие стороны на ферме AD FS не подвергаются изменениям. Но мы в любом случае рекомендуем создать актуальную резервную копию фермы AD FS, чтобы обеспечить возможность восстановления. Это можно сделать с помощью бесплатного инструмента [AD FS Rapid Restore Tool](/windows-server/identity/ad-fs/operations/ad-fs-rapid-restore-tool) корпорации Майкрософт. Он позволяет выполнить резервное копирование AD FS, восстановить существующую ферму или создать новую.

Если вы решили не использовать средство быстрого восстановления AD FS, как минимум следует экспортировать Microsoft 365 отношение доверия с проверяющей стороной платформы удостоверений и все добавленные настраиваемые правила утверждений. Это можно сделать с помощью следующего примера PowerShell:
...
Прежде чем преобразовывать федеративные удостоверения в управляемые, внимательно ознакомьтесь с тем, как в настоящее время используется AD FS для Azure AD, Microsoft 365 и других приложений (отношения доверия с проверяющей стороной). В частности, рассмотрите описанные в приведенной ниже таблице сценарии.

| If | Следующее действие |
|-|-|
| Вы планируете использовать AD FS с другими приложениями (кроме Azure AD и Microsoft 365). | После преобразования доменов вы будете использовать одновременно AD FS и Azure AD. Необходимо учитывать взаимодействие с пользователем. В некоторых сценариях пользователям может потребоваться выполнить проверку подлинности дважды: один раз в Azure AD (когда пользователь получает единый вход для доступа к другим приложениям, например Microsoft 365), и снова для всех приложений, которые все еще привязаны к AD FS в качестве отношения доверия с проверяющей стороной. |
...
| Вы используете AD FS, чтобы заблокировать старые версии клиентов проверки подлинности.| Рассмотрите возможность замены элементов управления AD FS, блокирующих предыдущие версии клиентов проверки подлинности, с помощью сочетания [элементов управления условным доступом](../conditional-access/concept-conditional-access-conditions.md) и [правил клиентского доступа Exchange Online](/exchange/clients-and-mobile-in-exchange-online/client-access-rules/client-access-rules). |
| Вы хотите, чтобы при проверке подлинности в AD FS пользователи проходили Многофакторную идентификацию (MFA) на локальном сервере MFA.| У вас нет возможности внедрить запрос MFA, выполняемый через локальное решение MFA, в поток проверки подлинности для управляемого домена. Однако вы можете использовать службу многофакторной идентификации Azure AD для многофакторной идентификации после преобразования домена.<br /><br /> Если пользователи в настоящее время не используют многофакторную идентификацию Azure AD, необходимо выполнить OneTime регистрацию пользователя. Нужно будет подготовить этот дополнительный шаг и сообщить о нем пользователям. |
| В настоящее время вы используете политики управления доступом (правила AuthZ) в AD FS для управления доступом к Microsoft 365.| Попробуйте заменить политики эквивалентными [политиками условного доступа](../conditional-access/overview.md) Azure AD и [правилами клиентского доступа Exchange Online](/exchange/clients-and-mobile-in-exchange-online/client-access-rules/client-access-rules).|
...
В службах AD FS создается утверждение **InsideCorporateNetwork** при проверке подлинности пользователя, который находится в корпоративной сети. Это утверждение можно передать в Azure AD. Оно используется для обхода многофакторной проверки подлинности в зависимости от расположения сети пользователя. Сведения о том, как определить, доступна ли эта функция в AD FS, см. в статье [Защита облачных ресурсов с помощью Многофакторной идентификации Azure и AD FS](../authentication/howto-mfa-adfs.md).

Утверждение **InsideCorporateNetwork** недоступно после преобразования доменов для использования сквозной проверки подлинности. Вместо этой функции вы можете использовать [именованные расположения в Azure AD](../reports-monitoring/quickstart-configure-named-locations.md).
...
Дополнительные сведения о условии **расположения** в условном доступе см. в разделе [Active Directory расположение условного доступа](../conditional-access/location-condition.md).
...
> [!NOTE]
> Фирменная символика организации доступна, только если вы приобрели лицензию Premium или Basic для Azure Active Directory или если у вас Microsoft 365 лицензия.
...
Дополнительные сведения см. в статье [Смарт-блокировка Azure Active Directory](../authentication/howto-password-smart-lockout.md).
...
> [!IMPORTANT]
> Не завершайте работу среды AD FS или удаляйте Microsoft 365 отношения доверия с проверяющей стороной, пока не будет проверено, что все пользователи могут пройти проверку подлинности с помощью облачной аутентификации.
...
### <a name="step-1-prepare-for-seamless-sso"></a>Шаг 1. Подготовка к эффективному единый вход
...
Выполните соответствующие действия, чтобы [развернуть](./how-to-connect-sso-quick-start.md) необходимые изменения на устройствах.
...
### <a name="step-2-change-the-sign-in-method-to-pass-through-authentication-and-enable-seamless-sso"></a>Шаг 2. изменение метода входа на сквозную проверку подлинности и включение простого единого входа
...
2. Выберите **изменить вход пользователя** и нажмите кнопку **Далее**. 
...
8. Проверьте настройку следующих параметров:
   ...
   * Для параметра **Сквозная проверка подлинности** должно быть установлено значение **Включено**.<br />
   ...
   ![Снимок экрана с параметрами в разделе входа пользователей](media/plan-migrate-adfs-pass-through-authentication/migrating-adfs-to-pta_image9.png)<br />
...
> [!IMPORTANT]
> Пропустите **параметр раздела б. Переключитесь с Федерации на сквозную аутентификацию с помощью Azure AD Connect и PowerShell**. Действия из этого раздела не применяются, если вы выбрали вариант 1 для изменения метода входа на синхронизацию хэша паролей и включения эффективного единого входа. 

#### <a name="option-b-switch-from-federation-to-pass-through-authentication-by-using-azure-ad-connect-and-powershell"></a>Вариант б. Переключение из Федерации в сквозную аутентификацию с помощью Azure AD Connect и PowerShell
...
3. Затем пользователь будет перенаправлен на панель доступа и успешно войдет в нее.
   ...
   > [!NOTE]
   > Простой единый вход работает на Microsoft 365 службах, поддерживающих Указание домена (например, myapps.microsoft.com/contoso.com). В настоящее время Microsoft 365 портал (portal.office.com) не поддерживает указания домена. Пользователю потребуется ввести имя участника-пользователя. После ввода имени участника-пользователя служба эффективного единого входа получает билет Kerberos от имени пользователя. Пользователь выполняет вход без ввода пароля.

   > [!TIP]
   > Рекомендуем развернуть [службы гибридного присоединения к Azure AD в Windows 10](../devices/overview.md), чтобы расширить возможности единого входа.

### <a name="remove-the-relying-party-trust"></a>Удаление отношений доверия с проверяющей стороной

После проверки успешной проверки подлинности всех пользователей и клиентов с помощью Azure AD можно удалить отношение доверия с проверяющей стороной Microsoft 365.
...
Инструкции о том, как проверить и (или) включить эту функцию, см. в статье [Функции службы синхронизации Azure AD Connect](./how-to-connect-syncservice-features.md).
...
Дополнительные сведения см. в разделе о [смене ключе расшифровки Kerberos компьютерной учетной записи AZUREADSSOACC](./how-to-connect-sso-faq.md).
