...
На уровне данных содержится несколько интерфейсных служб, обеспечивающих возможность чтения и записи. На приведенной ниже диаграмме показано, как компоненты раздела с одним каталогом поставляются по всему географически распределенному центру обработки данных.

  ![Диаграмма раздела с одним каталогом](./media/active-directory-architecture/active-directory-architecture.png)
...
Все считываникаталогобслуживаются из *вторичных реплик,* которые находятся в центрах обработки данных, которые физически расположены в разных географических регионах. *reads* Данные реплицируются асинхронно, поэтому вы можете иметь большое количество вторичных реплик. Считывателькаталог, например, запросы на аутентификацию, обслуживаются в центрах обработки данных, близких к клиентам. Вторичные реплики отвечают за масштабируемость операций чтения.
...
Доступность (или время непрерывной работы) определяет возможность системы работать без перебоя. Ключом к высокой доступности Azure AD является то, что службы могут быстро перемещать трафик через несколько географически распределенных центров обработки данных. Каждый центр обработки данных независим, что позволяет выполнять режимы сбоев, связанные с декорированными. Благодаря такой конструкции высокой доступности Azure AD не требует простоев для работы по техническому обслуживанию.
...
Запись прочно обязуется, по крайней мере, два центра обработки данных до его признания. Это происходит, сначала совершая запись на первичном, а затем немедленно реплицируя запись, по крайней мере, в один другой центр обработки данных. Это действие записи гарантирует, что потенциальная катастрофическая потеря центра обработки данных, хозяйничающая первичная, не приведет к потере данных.
...
Реплики Azure AD хранятся в центрах обработки данных, расположенных по всему миру. Для получения дополнительной [Azure global infrastructure](https://azure.microsoft.com/global-infrastructure/)информации см.

Azure AD работает в центрах обработки данных со следующими характеристиками:

* Службы проверки подлинности, Graph и другие службы AD находятся за службой шлюза. Шлюз управляет балансировкой нагрузки этих служб. При обнаружении неработоспособных серверов с использованием транзакционных проб работоспособности он автоматически выполняет отработку отказа. Основываясь на этих зондах работоспособности, Gateway динамически направляет трафик в здоровые центры обработки данных.
* Для *чтения*каталог имеет вторичные реплики и соответствующие фронтовые службы в активной активной конфигурации, работающей в нескольких центрах обработки данных. В случае сбоя всего центра обработки данных трафик будет автоматически направляться в другой центр обработки данных.
 «Для *записей—* каталог сведет на нет первичную (мастер-) реплику через центры обработки данных через запланированные (новый первичный синхронизируется со старыми первичными) или процедурами аварийного сбоя. Долговечность данных достигается путем репликации любого коммита по крайней мере в двух центрах обработки данных.
...
Записи приложений с использованием API-известь Microsoft Graph API Azure AD абстрагируются от сохранения сродства к реплике каталога для согласованности чтения. Служба Microsoft Graph API поддерживает логическую сессию, которая имеет сродство к вторичной реплике, используемой для чтения; сродство фиксируется в "маркере реплики", который служба кэширует с помощью распределенного кэша во вторичном центре обработки данных. Затем этот токен используется в последующих операциях в том же логическом сеансе. Чтобы продолжить использование одного и того же логического сеанса, последующие запросы должны быть направлены в тот же центр обработки данных Azure AD. Невозможно продолжить логический сеанс, если запросы клиентов каталога направляются в несколько центров обработки данных Azure AD; если это произойдет, то клиент имеет несколько логических сессий, которые имеют независимые консистенции чтения записи.
