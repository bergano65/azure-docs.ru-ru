...
См. также документацию по [разработчикам приложений](https://aka.ms/azureadresilience/developer) и [Azure AD B2Cным системам](resilience-b2c.md).
...
При каждом вызове системы проверки подлинности происходит сбой, если какой-либо компонент в цепочке вызовов к Azure AD завершается сбоем. Это означает, что если какая-либо часть инфраструктуры может быть прервана, так как пользователи не могут получить доступ к нужным им приложениям. Таким образом, уменьшение числа вызовов проверки подлинности и количества зависимостей в этих вызовах важно для обеспечения устойчивости. Разработчики приложений могут утверждать некоторый контроль над тем, как часто запрашиваются маркеры. Например, обратитесь к разработчикам, чтобы убедиться, что они используют управляемые удостоверения Azure AD для своих приложений везде, где это возможно. 
...
По истечении срока действия маркера, представленного для приложения, приложение отклоняет маркер, и клиент должен получить новый маркер из Azure AD. Получение нового маркера потенциально требует вмешательства пользователя, например запросов учетных данных. Уменьшение частоты вызовов проверки подлинности с долгосрочными маркерами снижает этот риск. Однако необходимо сбалансировать жизненный цикл маркера с риском, созданным меньшим числом оценок политики. Дополнительные сведения об управлении жизненным циклом маркеров см. в этой статье, посвященной [оптимизации запросов на проверку подлинности](https://docs.microsoft.com/azure/active-directory/authentication/concepts-azure-multi-factor-authentication-prompts-session-lifetime).
