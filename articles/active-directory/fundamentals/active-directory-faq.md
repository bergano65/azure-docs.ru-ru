...
**Вопрос. Почему при попытке получить доступ к Azure AD в портал Azure выводится "подписки не найдены"?**

**Ответ.** Для получения доступа к порталу Azure каждому пользователю требуются разрешения в рамках подписки Azure. Если у вас нет платной подписки Office 365 или Azure AD, необходимо активировать бесплатную [учетную запись Azure](https://azure.microsoft.com/free/
...
---
**Вопрос. Какова взаимосвязь между Azure AD, Office 365 и Azure?**
...
Дополнительные сведения см. [в статье интеграция локальных удостоверений с Azure Active Directory](../hybrid/whatis-hybrid-identity.md).
...
Реализовать единый вход из локальной среды можно с помощью таких решений Федерации, как службы федерации Active Directory (AD FS) (AD FS) или путем настройки синхронизации хэшей паролей. Оба варианта можно легко развернуть с помощью мастера настройки Azure AD Connect.
...
**Ответ.** Да, в Azure AD есть [панель доступа к Azure AD](https://myapps.microsoft.com) для самообслуживания пользователей и доступа к приложениям. Если вы являетесь клиентом Office 365, вы можете найти многие возможности на [портале office 365](https://portal.office.com).
...
**Вопрос. можно ли использовать обратную запись пароля Azure AD без синхронизации паролей? (В этом случае можно использовать самостоятельный сброс пароля Azure AD (SSPR) с обратной записью пароля и не хранить пароли в облаке?)**
...
Ответ **.** Для такого типа ситуации существует несколько вариантов.  Используйте самостоятельный сброс пароля, если он доступен.  Функционирование самостоятельного сброса пароля зависит от настройки этой функции.  Дополнительные сведения см. [в статье как работает портал сброса паролей](../authentication/howto-sspr-deployment.md).
...
**Вопрос. Некоторые (Общие) пароли отклоняются с сообщениями "Этот пароль использовался во много раз", что означает, что это относится к паролям, используемым в текущем каталоге Active Directory?**

Это относится к глобально общим паролям, например к любым разновидностям "Password" и "123456".
...
**Ответ.** С помощью Azure AD Premium вы можете добавить и настроить любое приложение. В зависимости от возможностей приложения и настроек вы можете настроить единый вход и автоматическую подготовку.  
...
* [Active Directory протоколы проверки подлинности](https://msdn.microsoft.com/library/azure/dn151124.aspx)
* [Единый вход для приложений в Azure AD](../manage-apps/what-is-single-sign-on.md)

---
**Вопрос. можно ли добавлять приложения, работающие локально?**
...
Ответ **.** С помощью условного доступа Azure AD можно назначить уникальную политику доступа для каждого приложения. В политике вы можете настроить обязательное выполнение многофакторной идентификации для всех пользователей или только для пользователей, не подключенных к локальной сети.  
...
Дополнительные сведения см. [в разделе Автоматизация подготовки пользователей и ее отмене в приложениях SaaS с помощью Azure Active Directory](../app-provisioning/user-provisioning.md).

---
**Вопрос. Можно ли установить безопасное подключение LDAP к Azure AD? **

Ответ **.**  Нет. Azure AD не поддерживает протокол LDAP или защищенный протокол LDAP напрямую. Тем не менее можно включить экземпляр доменных служб Azure AD (Azure AD DS) в клиенте Azure AD с правильной настройкой групп безопасности сети через сеть Azure для обеспечения подключения LDAP. Дополнительные сведения см. в статье [Настройка защищенного протокола LDAP для управляемого домена Azure Active Directory доменных служб](../../active-directory-domain-services/tutorial-configure-ldaps.md) .
