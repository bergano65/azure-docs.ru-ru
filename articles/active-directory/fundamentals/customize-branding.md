---
title: Добавление брендинга на страницу ввне вашей организации - Azure AD
...
Пользовательский брендинг не будет отображаться сразу, когда ваши\.пользователи переходят на такие сайты, как www office.com. Для появления настраиваемой фирменной символики пользователю необходимо выполнить вход. После того, как пользователь вписался, клеймо может занять 15 минут или больше. 

> [!NOTE]
> Все элементы для фирменной символики являются необязательными. Например, при указании баннера с логотипом без фонового изображения на странице входа отобразится логотип и фоновое изображение целевого сайта по умолчанию (например, Office 365).<br><br>Кроме того, брендинг страницы, вспять, не переносится на личные учетные записи Майкрософт. Если пользователи или бизнес-гости выполнят вход с помощью личных учетных записей Майкрософт, фирменная символика компании на этих страницах входа отображаться не будет.
...
3. На странице **Настройка корпоративной фирменной символики** укажите всю или часть следующей информации.
    ...
    - **Общие настройки**
        ...
        - **Логотип баннера.** Укажите ваш логотип в формате PNG или JPG, который будет отображаться на странице входа после того, как пользователь введет имя пользователя, а также на странице портала **Мои приложения**.
            ...
            Изображение не может быть выше 60 пикселей или шире 280 пикселей. Рекомендуется использовать прозрачное изображение, так как цвет фона может не совпасть с фоном логотипа. Также рекомендуется не добавлять поля вокруг изображения, чтобы ваш логотип не выглядел слишком маленьким.
    ...
    - **Расширенные настройки**
        ...
        - **Ввесенний фоновый цвет страницы.** Укажите шестнадцатеричный код цвета (например, #FFFFFF для белого), который будет отображаться вместо фонового изображения в случае низкой пропускной способности подключения. Мы рекомендуем использовать основной цвет баннера с логотипом или фирменный цвет вашей организации.

        - **Квадратный логотип.** Выберите изображение логотипа организации .png (предпочтительно) или .jpg, чтобы они появились пользователям в процессе настройки новых устройств Windows 10 Enterprise. Это изображение используется только во время проверки подлинности Windows и отображается только на клиентах, использующих [Windows Autopilot]( https://docs.microsoft.com/windows/deployment/windows-autopilot/windows-10-autopilot) для развертывания или для страниц ввода пароля в других службах Windows 10. В некоторых случаях он также может отображаться в диалоге согласия.
        
            Изображение не может быть больше, чем 240x240 пикселей в размерах и должны иметь размер файла менее 10 кБ. Рекомендуется использовать прозрачное изображение, так как цвет фона может не совпасть с фоном логотипа. Также рекомендуется не добавлять поля вокруг изображения, чтобы ваш логотип не выглядел слишком маленьким.
    
        - **Квадратный логотип для темной темы.** Формат совпадает с квадратным логотипом в предыдущем пункте. Это изображение с логотипом отображается вместо обычного квадратного логотипа, когда используется темный фон, например на экранах Windows 10, присоединенных к Azure AD, во время запуска при первом включении компьютера (OOBE).  Если ваш логотип хорошо смотрится на белом, темно-синем и черном фоне, вам не нужно добавлять это изображение. 
        
        - **Отображение параметра, позволяющего остаться в системе.** Вы можете позволить пользователям оставаться в сети в Azure AD до явного выхода из системы. Если вы выберете **Нет,** эта опция скрыта, и пользователи должны войти в каждый раз, когда браузер закрыт и вновь открыт.
...
**Пользовательский URL::**`https://account.activedirectory.windowsazure.com/proofup.aspx?whr=contoso.com`

**Исходный URL-адрес:** https://aka.ms/SSPR<br>
**Пользовательский URL::**`https://passwordreset.microsoftonline.com/?whr=contoso.com`
