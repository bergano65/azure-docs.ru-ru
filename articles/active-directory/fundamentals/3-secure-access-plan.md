...
Сейчас вы можете [Применить многофакторную проверку подлинности для пользователей B2B в вашем клиенте](../external-identities/b2b-tutorial-require-mfa.md). 
...
Можно использовать [политики защиты идентификации на основе рисков](../conditional-access/howto-conditional-access-policy-risk.md) . Однако проблемы должны быть устранены в домашнем клиенте пользователя.

Для [сетевых расположений](../conditional-access/howto-conditional-access-policy-location.md)можно ограничить доступ любым ДИАПАЗОНОМ IP-адресов. Вы можете использовать его, если хотите, чтобы только внешние партнеры обращались к приложению, только если они находятся на сайте в вашей организации.

Дополнительные [сведения о политиках условного доступа](../conditional-access/overview.md).
...
* Проверки **доступа для управления обслуживанием**. Используйте функции управления назначением для
   ...
   * [Автоматически истекает срок действия пакетов доступа](../governance/entitlement-management-access-package-lifecycle-policy.md), и таким образом внешний пользователь получает доступ к добавленным ресурсам.

   * Задайте [обязательную частоту проверки](../governance/entitlement-management-access-reviews-create.md) для проверок доступа.

   * Если вы используете [подключенные Организации](../governance/entitlement-management-organization.md) для группирования всех пользователей одного партнера, запланируйте регулярную проверку с помощью владельца компании и партнера.

* **Группы Microsoft 365**. Задайте [политику срока действия группы](/microsoft-365/solutions/microsoft-365-groups-expiration-policy?view=o365-worldwide) для групп Microsoft 365, к которым приглашены внешние пользователи. 

* **Другие параметры**. Если у внешних пользователей есть доступ за пределами пакетов доступа управления назначением или групп Microsoft 365, настройте бизнес-процесс для проверки того, когда учетные записи следует сделать неактивными или удаленными. Например:
...
Некоторые функции, например [Управление](../governance/entitlement-management-overview.md)назначениями, доступны только с лицензиями Azure AD Premium 2 (P2). Лицензии "Microsoft 365" и "Office 365", включают лицензии Azure AD P2. 

Другие сочетания Microsoft 365, Office 365 и Azure AD также обеспечивают некоторые функции для управления внешними пользователями. Дополнительные сведения см. в разделе [Information Protection](/office365/servicedescriptions/microsoft-365-service-descriptions/microsoft-365-tenantlevel-services-licensing-guidance/microsoft-365-security-compliance-licensing-guidance) .
...
[Пакеты доступа для управления](../governance/entitlement-management-access-package-create.md) назначением позволяют подготавливать и предоставлять доступ к группам и командам, приложениям и сайтам SharePoint. Можно определить, к каким подключенным организациям разрешен доступ, разрешены ли запросы на самостоятельную обслуживание и какие рабочие процессы утверждения требуются (если они есть) для предоставления доступа. Чтобы гарантировать, что доступ не будет продолжаться дольше, чем необходимо, можно определить политики срока действия и проверки доступа для каждого пакета доступа. 
