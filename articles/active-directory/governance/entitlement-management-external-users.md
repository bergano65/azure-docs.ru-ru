...
1. Когда внешний пользователь теряет свое последнее назначение в любом из пакетов доступа, если вы хотите удалить учетную запись гостевого пользователя в каталоге, установите для параметра **Удалить внешний пользователь** значение **Да**.

    > [!NOTE]
    > Управление назначениями позволяет удалять только те учетные записи, которые были приглашены с помощью управления назначениями. Кроме того, обратите внимание, что пользователь будет заблокирован для входа и удален из каталога, даже если этот пользователь был добавлен к ресурсам в каталоге, не имеющему доступа к назначениям пакетов. Если гостевая учетная запись присутствовала в вашем каталоге, прежде чем получать доступ к назначениям пакета, они останутся. Однако если гостевая виртуальная машина была приглашена с помощью назначения пакета Access, а после приглашения была назначена на сайт OneDrive для бизнеса или SharePoint Online, они все равно будут удалены.

1. Если вы хотите удалить гостевую учетную запись в каталоге, можно задать число дней до его удаления. Если вы хотите удалить учетную запись гостя, как только она потеряет свое последнее назначение пакетам доступа, задайте **число дней перед удалением внешнего пользователя из этого каталога** в значение **0**.
...
- [Для пользователей, не наличныхся в вашем каталоге](entitlement-management-access-package-create.md#for-users-not-in-your-directory)
