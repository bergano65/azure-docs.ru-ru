---
title: Просмотр запросов для пакета Access в управлении назначениями Azure AD — Azure Active Directory
description: Узнайте, как просматривать запросы к пакету Access в Azure Active Directory управлении назначением.
services: active-directory
documentationCenter: ''
author: msaburnley
manager: daveba
editor: ''
ms.service: active-directory
ms.workload: identity
ms.tgt_pltfrm: na
ms.devlang: na
ms.topic: conceptual
ms.subservice: compliance
ms.date: 10/15/2019
ms.author: ajburnle
ms.reviewer: ''
ms.collection: M365-identity-device-management
ms.openlocfilehash: addc7db2aa2f0f9ba16438d2a31d6c7cc6bfa49e
ms.sourcegitcommit: 509b39e73b5cbf670c8d231b4af1e6cfafa82e5a
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 03/05/2020
ms.locfileid: "78376592"
---
# <a name="view-requests-for-an-access-package-in-azure-ad-entitlement-management"></a>Просмотр запросов для пакета Access в управлении назначениями Azure AD

В разделе "Управление назначениями Azure AD" можно увидеть, кто запросил пакеты доступа, их политику и состояние. В этой статье описывается, как просматривать запросы пакетов Access.

## <a name="view-requests"></a>Просмотр запросов

**Предварительная роль:** Глобальный администратор, администратор пользователей, владелец каталога или диспетчер пакетов Access

1. На портале Azure щелкните **Azure Active Directory** и выберите **Управление удостоверениями**.

1. В меню слева щелкните **доступ к пакетам** и откройте пакет Access.

1. Щелкните **запросы**.

1. Щелкните конкретный запрос, чтобы просмотреть дополнительные сведения.

    ![Список запросов для пакета Access](./media/entitlement-management-access-package-requests/requests-list.png)

## <a name="next-steps"></a>Следующие шаги

- [Изменение параметров запроса и утверждения для пакета Access](entitlement-management-access-package-request-policy.md)
- [Просмотр, Добавление и удаление назначений для пакета Access](entitlement-management-access-package-assignments.md)
- [Устранение неполадок запросов](entitlement-management-troubleshoot.md#requests)
