...
- Никто из пользователей не может быть уверен, что все конкретные пользователи в каталогах других организаций могут пригласить их.
...
Управление назначениями Azure AD может помочь решить эти проблемы.  Чтобы узнать больше о том, как клиенты используют управление назначением Azure AD, вы можете ознакомиться с примером [использования в Avanade](https://customers.microsoft.com/story/avanade-professional-services-azure-canada) и [практическом примере](https://customers.microsoft.com/story/757467-centrica-energy-azure).  В этом видео представлен обзор управления назначением и его ценности.
...
Специализированные облака, такие как Azure для Германии и Azure для Китая, в настоящее время недоступны для использования.

### <a name="how-many-licenses-must-you-have"></a>Сколько лицензий необходимо?

Убедитесь, что в каталоге должно быть не менее столько лицензий Azure AD Premium P2, сколько вы используете:

- Пользователи участников, которые **могут** запрашивать пакет Access.
- Участники и гостевые пользователи, запрашивающие пакет Access.
- Участники и гостевые пользователи, которые утверждают запросы на пакет Access.
- Пользователи-участники и гости, которые имеют прямое назначение пакету доступа.

Лицензии Azure AD Premium P2 **не** требуются для следующих задач:

- Для пользователей с ролью глобального администратора, которая настраивает исходные каталоги, доступ к пакетам и политикам, а также делегирование административных задач другим пользователям, лицензии не требуются.
- Лицензии не требуются для пользователей, которым были делегированы административные задачи, такие как создатель каталога, владелец каталога и диспетчер пакетов Access.
- Для гостей, которые **могут** запрашивать пакеты доступа, не требуются лицензии, но **не** запрашивают пакет Access.
...
| --- | --- | --- |
| Глобальный администратор в банке Woodgrove Bank создает исходные каталоги и делегирует административные задачи 6 другим пользователям. Одна из политик указывает, что **все сотрудники** (2 000 сотрудников) могут запрашивать конкретный набор пакетов доступа. 150. сотрудники запрашивают пакеты доступа. | 2 000. сотрудники, которые **могут** запрашивать пакеты доступа | 2 000 |
| Глобальный администратор в банке Woodgrove Bank создает исходные каталоги и делегирует административные задачи 6 другим пользователям. Одна из политик указывает, что **все сотрудники** (2 000 сотрудников) могут запрашивать конкретный набор пакетов доступа. Другая политика указывает, что некоторые пользователи из **партнерской компании Contoso** (гости) могут запрашивать те же пакеты доступа, которые подчиняются утверждению. Contoso имеет 30 000 пользователей. 150 сотрудники запрашивают пакеты доступа и 10 500 пользователей из Contoso по запросу доступа. | 2 000 сотрудников + 500 гостевых пользователей из компании Contoso, превышающих соотношение 1:5 (10 500-(2 000 * 5)) | 2500 |
