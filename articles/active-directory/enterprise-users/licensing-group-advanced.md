---
title: Дополнительные сценарии лицензирования на основе групп — Azure AD | Документация Майкрософт
...
Лицензирование на основе группы можно использовать с любой группой безопасности, а это значит, что ее можно использовать с динамическими группами Azure AD. Динамические группы выполняют правила для атрибутов пользовательских ресурсов, чтобы автоматически добавлять и удалять пользователей из групп.
...
Пользователь может быть участником нескольких групп с лицензиями. Следует учесть ряд факторов:
...
- При выборе лицензии *E3* отображаются дополнительные сведения, включая сведения о том, какие службы включены для пользователя по назначению лицензии группы.
...
Изначально пользователь наследует лицензию только из группы *E3 Basic Services* , которая включает четыре плана обслуживания.

1. Выберите **назначить** , чтобы непосредственно назначить пользователю лицензию E3. В этом случае вы собираетесь отключить все планы службы, за исключением Yammer корпоративный.

    В результате пользователь по-прежнему использует только одну лицензию на продукт E3, но при прямом назначении служба Yammer корпоративный активируется только для этого пользователя. Вы можете видеть, какие службы включены по членству в группе, а не по прямому назначению.

1. При использовании прямого назначения разрешены следующие операции:

   - Yammer Enterprise может быть отключено непосредственно для ресурса пользователя. В отличие от других переключателей службы переключатель **Вкл./Выкл.** активирован, так как эта служба включена непосредственно для пользователя, и поэтому ее можно изменить.
...
Когда корпорация Майкрософт добавляет новую службу в план лицензирования продукта, она включается по умолчанию во всех группах, которым назначена лицензия продукта. Пользователи в вашей организации, подписанные на уведомления об изменениях продукта, будут получать сообщения электронной почты с уведомлением о предстоящих дополнениях к службе.
...
2. Вы получили уведомление от корпорации Майкрософт, в котором сообщается, что продукт E5 будет дополнен новой службой — *Microsoft Stream*. Когда служба станет доступной в Организации, можно выполнить следующие действия.
...
1. Запустите `connect-msolservice` командлет для проверки подлинности и подключения к вашей организации.

2. `Get-MsolAccountSku` можно использовать для обнаружения всех подготовленных лицензий на продукт в Организации Azure AD.
...
3. Выберите элемент в списке, чтобы просмотреть сведения об изменениях. В разделе *Измененные свойства* отображаются старые и новые значения для назначения лицензии.
...
2. Аналогичным образом, чтобы узнать, когда была завершена обработка групп, используйте значение фильтра *Finish Applying Group Based License To Users* (Завершить назначать пользователям лицензию, назначенную группе).
   > [!TIP]
   > В этом случае в поле *Измененные свойства* содержится сводка по результатам — это позволяет быстро проверить, возникали ли ошибки при обработке. Пример результатов выполнения:
   > ```
   > Modified Properties
   > ...
   > Name : Result
...
- Эту функцию можно использовать только с группами безопасности и Microsoft 365 группами с securityEnabled = TRUE.

- [Центр администрирования Microsoft 365](https://admin.microsoft.com) в настоящее время не поддерживает групповые лицензии. Если пользователь наследует лицензию от группы, эта лицензия отображается на портале администрирования Office как обычная лицензия пользователя. При попытке изменить или удалить эту лицензию на портале появится сообщение об ошибке. Наследуемые от группы лицензии невозможно изменить непосредственно для пользователя.
...
- Если вы управляете членством пользователей с помощью динамических групп, убедитесь, что пользователь входит в группу, которая необходима для назначения лицензии. Если нет, [проверьте состояние обработки для правила членства](groups-create-rule.md) динамической группы.
