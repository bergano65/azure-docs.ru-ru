...
| **Почему я получу это оповещение?** | Если пользователям назначены привилегированные роли, которые им не нужны, это увеличивает вероятность атаки. Это также позволяет злоумышленникам оставаться незамеченными в учетными записях, которые не используются активно. |
...
| --- | --- |
| **Уровень серьезности** | Низкий |
| **Почему я получу это оповещение?** | Без MFA скомпрометированные пользователи могут активировать привилегированные роли. |
...
| --- | --- |
| **Уровень серьезности** | Низкий |
| **Почему я получу это оповещение?** | В текущем клиенте нет Azure AD Premium P2. |
...
| **Почему я получу это оповещение?** | Обнаружены учетные записи с привилегированной ролью, у которых не менялся пароль в течение последних 90 дней. Это могут быть неиспользуемые учетные записи служб или общие учетные записи, которые уязвимы к атакам злоумышленников. |
...
| **Почему я получу это оповещение?** | Назначения привилегированных ролей за пределами PIM не отслеживаются надлежащим образом и могут свидетельствовать об активной атаке. |
...
| --- | --- |
| **Уровень серьезности** | Низкий |
| **Почему я получу это оповещение?** | Глобальный администратор является высшей привилегированной ролью. Если глобальный администратор скомпрометирован, злоумышленник получает доступ ко всем разрешениям, что подвергает риску всю систему. |
...
| --- | --- |
| **Уровень серьезности** | Низкий |
| **Почему я получу это оповещение?** | Множественные активации одной привилегированной роли тем же пользователем являются признаком атаки. |
