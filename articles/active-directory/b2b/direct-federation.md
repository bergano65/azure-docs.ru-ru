---
title: Настройка прямой Федерации с поставщиком удостоверений для B2B-Azure Active Directory | Документация Майкрософт
...
Прямая Федерация разрешена только для доменов, которые ***не*** проверены DNS в Azure AD. Прямая Федерация разрешена для неуправляемых (проверенных по электронной почте или "вирусных") клиентов Azure AD, так как они не проверены DNS.
...
Azure AD B2B можно настроить для создания Федерации с поставщиками удостоверений, которые используют протокол WS-подач с некоторыми конкретными требованиями, указанными ниже. В настоящее время два поставщика WS-подач были протестированы на совместимость с Azure AD, включая AD FS и Shibboleth. Дополнительные сведения о создании отношения доверия между проверяющей стороной между поставщиком WS-подачи и Azure AD [см. в](https://www.microsoft.com/download/details.aspx?id=56843)документации по интеграции STS с помощью протоколов WS.
