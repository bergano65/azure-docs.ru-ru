---
title: Часто задаваемые вопросы о совместной работе B2B - Активный каталог Azure (ru) Документы Майкрософт
...
Эта функция в настоящее время не поддерживается. Если доступ к ресурсам организации требует многофакторной аутентификации, организации-партнеру необходимо зарегистрироваться для многофакторной аутентификации в вашей (приглашающей) организации.
...
Да. Объекты гостевых не видны в глобальном списке адресов организации (GAL) по умолчанию, но можно использовать Azure Active Directory PowerShell, чтобы сделать их видимыми. [Можно ли сделать объекты гостевых видимыми в глобальном списке адресов?](https://docs.microsoft.com/office365/admin/create-groups/manage-guest-access-in-groups#add-guests-to-the-global-address-list)
...
Если пользователю не будет отведена роль ограниченного администратора, пользователям Совместной работы B2B не потребуется доступ к порталу Azure. Тем не менее, пользователи B2B-сотрудничества, которым отведена роль ограниченного администратора, могут получить доступ к порталу. Кроме того, если гостевой пользователь, которому не назначены эти роли, получает доступ к порталу, он может воспользоваться частью возможностей, так как у роли гостевого пользователя есть некоторые разрешения в каталоге.
...
### <a name="can-i-block-access-to-the-azure-portal-for-guest-users"></a>Можно ли заблокировать доступ к порталу Azure для гостевых пользователей?
 ...
 Да! Можно создать политику условного доступа, которая блокирует доступ всех гостевых и внешних пользователей к порталу Azure. Но будьте осторожны при настройке этой политики, чтобы случайно не заблокировать доступ для участников и администраторов.
...
2. На портале Azure выберите **Active Directory Azure.** 
3. Под **управлением**, выберите **безопасность**.
4. Под **защитой,** выберите **Условный доступ**. Выберите **новую политику**.
5. На **новой** странице, в текстовом ящике **Name,** введите имя для политики (например, "Блокировать гостей от доступа к порталу").
6. В разделе **Назначения** выберите **Пользователи и группы**.
7. На вкладке **«Включить»** выберите **избранных пользователей и групп,** а затем выберите **всех гостевых и внешних пользователей (Preview).**
9. Выберите **готовое**.
10. На **новой** странице в разделе **«Назначения»** выберите **приложения или действия облака.**
11. На странице **приложений или действий облака** выберите **приложения,** а затем **выберите .**
12. На странице **выбора** выберите **управление Microsoft Azure**, а затем нажмите кнопку **Выбрать**.
13. На странице **приложений или действий облака** выберите **Done**.
...
Все интегрированные приложения Azure AD могут поддерживать гостевых пользователей Azure B2B, но они должны использовать конечную точку, настроенную в качестве арендатора для проверки подлинности гостевых пользователей. Кроме того, может потребоваться [настроить утверждения](claims-mapping.md) в токене SAML, который создается, когда гостевой пользователь проходит проверку подлинности в приложении. 
...
Да. Для получения дополнительной информации [см.](conditional-access.md)
