---
title: Добавление Google в качестве поставщика идентификационных данных для B2B - Azure AD
...
Настроив федерацию с помощью Google, вы можете разрешить приглашенным пользователям входить в общие приложения и ресурсы с помощью собственных учетных записей Gmail без необходимости создавать учетные записи Майкрософт (MSA). 

> [!NOTE]
> Федерация Google разработана специально для пользователей Gmail. Чтобы использовать домены G Suite с помощью доменов G Suite, используйте [функцию прямой федерации.](direct-federation.md)
...
![Скриншот, показывающий знак Google на странице](media/google-federation/google-sign-in.png)

## <a name="limitations"></a>Ограничения

Команды полностью поддерживают гостевых пользователей Google на всех устройствах. Пользователи Google могут войти в команды `https://teams.microsoft.com`из общей точки, как .

Общие конечные точки других приложений могут не поддерживать пользователей Google. Гость google пользователи должны войти в систему с помощью ссылки, которая включает в себя информацию о вашем арендаторе. Примеры:
  * `https://myapps.microsoft.com/?tenantid=<your tenant id>`
  * `https://portal.azure.com/<your tenant id>`
  * `https://myapps.microsoft.com/<your verified domain>.onmicrosoft.com`

   Если гостевые пользователи `https://myapps.microsoft.com` Google `https://portal.azure.com`попытаются использовать ссылку, такую как или, они получат ошибку.

Вы также можете предоставить гостевым пользователям Google прямую ссылку на приложение или `https://myapps.microsoft.com/signin/Twitter/<application ID?tenantId=<your tenant ID>`ресурс, если эта ссылка включает в себя информацию о вашем арендаторе, например. 
...
2. Создайте новый проект: на панели мониторинга выберите **Создать проект**, а затем нажмите кнопку **Создать**. На странице "Создание проекта" введите **имя проекта**, а затем нажмите кнопку **Создать**.
   ...
   ![Скриншот, показывающий новую страницу проекта для Google](media/google-federation/google-new-project.png)

3. Убедитесь, что в меню проектов выбран ваш новый проект. Затем под **AIS & услуг**, выберите **OAuth согласие экрана**.

4. Выберите **Внешний,** а затем выберите **Создать**. 
5. На **экране согласия OAuth**введите **имя приложения.** (Оставьте другие параметры без изменений.)

   ![Скриншот, показывающий вариант экрана согласия Google OAuth](media/google-federation/google-oauth-consent-screen.png)

6. Прокрутите страницу до раздела **Авторизованные домены** и введите microsoftonline.com.

   ![Скриншот с разделом Авторизованных доменов](media/google-federation/google-oauth-authorized-domains.png)

7. Нажмите кнопку **Сохранить**.

8. Выберите **учетные данные**. В меню **Создать учетные данные** выберите **Идентификатор клиента OAuth**.

   ![Скриншот, показывающий, что AIS Google создают опцию учетных данных](media/google-federation/google-api-credentials.png)
...
9. В разделе **Тип приложения** выберите **Веб-приложение**, а затем в поле **Разрешенные URI перенаправления** ведите следующие URI:
   ...
   ![Скриншот, показывающий авторизованный раздел URIs](media/google-federation/google-create-oauth-client-id.png)

10. Выберите **Создать**. Скопируйте идентификатор и секрет клиента, которые будут использоваться при добавлении поставщика удостоверений на портале Azure AD.

   ![Скриншот, показывающий идентификатор клиента OAuth и секрет клиента](media/google-federation/google-auth-client-id-secret.png)
...
4. Введите имя. Затем введите идентификатор и секрет клиента, полученные ранее. Нажмите кнопку **Сохранить**. 
   ...
   ![Скриншот, показывающий страницу поставщика идентификационных данных Add Google](media/google-federation/google-identity-provider.png)
...
4. На строке **Google** выберите контекстное меню **(...)** и затем выберите **Удалить**. 
   
   ![Скриншот, показывающий опцию «Удалить» для поставщика социальных идентификаций](media/google-federation/google-social-identity-providers.png)
