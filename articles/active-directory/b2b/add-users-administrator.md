---
title: Добавление пользователей совместной работы B2B на портал Azure - Azure AD
...
Как пользователю, которому присваивается какая-либо из ограниченных ролей каталога администратора, можно использовать портал Azure, чтобы пригласить пользователей совместной работы B2B. Вы можете приглашать гостевых пользователей в каталог, группу или приложение. После приглашения пользователя (любым из способов) его учетная запись добавляется в Azure Active Directory (Azure AD) с типом пользователя *Гость*. Затем для доступа к ресурсам гостевой пользователь должен активировать приглашение. Приглашение пользователя не истекает.
...
Убедитесь, что внешние настройки совместной работы вашей организации настроены таким образом, чтобы вы могли приглашать гостей. По умолчанию все пользователи и админы могут приглашать гостей. Но внешние политики совместной работы вашей организации могут быть настроены для того, чтобы не допустить приглашения гостей определенными типами пользователей или админ-миминов. Чтобы узнать, как просматривать и устанавливать эти политики, [см.](delegate-invitations.md)
...
1. Вопиюсь на [портал Azure](https://portal.azure.com) в качестве пользователя, которому назначена ограниченная роль каталога администратора или роль приглашенного гостя.
2. Поиск и выберите **Активный каталог Azure** с любой страницы.
...
5. На странице **Новый пользователь** выберите **Пригласить пользователя**, а затем добавьте сведения о пользователе-госте. 
   ...
   - **Имя.** Имя и фамилия пользователя-гостя.
   ...
   - **Группы**: Вы можете добавить гостевого пользователя в одну или несколько существующих групп, или вы можете сделать это позже.
   - **Роль каталога:** Если пользователю требуются административные разрешения Azure AD, их можно добавить в роль Azure AD. 
...
Если вам нужно вручную добавить пользователей совместной работы B2B в группу, выполните следующие действия:
...
Чтобы добавить пользователей совместной работы B2B в приложение, выполните следующие действия:
