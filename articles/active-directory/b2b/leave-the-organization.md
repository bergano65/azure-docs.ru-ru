...
> [!NOTE]
> Гостевой пользователь не может уходят из организации, если эта учетная запись отключена в главном клиенте или в клиенте ресурса. Если эта учетная запись отключена, гостевой пользователь потребуется обратиться к администратору клиента, который можно удалить учетную запись гостя или включить учетную запись гостя, поэтому пользователь может покинуть организацию.
...
После выхода пользователя из организации учетная запись пользователя обратимо удаляется в каталоге. По умолчанию, перемещение объекта пользователя **удаленных пользователей** области в Azure AD, но не удалены без возможности восстановления в течение 30 дней. Такое обратимое удаление позволяет администратору восстановить учетную запись пользователя (включая группы и разрешения), если пользователь отправит запрос на восстановление учетной записи в течение 30-дневного периода.
