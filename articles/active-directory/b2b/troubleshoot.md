---
title: Работа по устранению проблем B2B - Активный каталог Azure (ru) Документы Майкрософт
...
Если вы уведомлены о том, что у вас нет разрешений на приглашение пользователей, убедитесь, что ваша учетная запись пользователя уполномочена приглашать внешних пользователей в соответствии с настройками Azure Active Directory > > внешними пользователями > управлять внешними настройками совместной работы:

![Скриншот, показывающий настройки внешних пользователей](media/troubleshoot/external-user-settings.png)
...
![Ошибка, заявляя, что арендатор не разрешает пользователям, проверяемым по электронной почте](media/troubleshoot/allow-email-verified-users.png)
...
## <a name="you-receive-an-aadsts65005-error-when-you-try-to-log-in-to-an-azure-resource"></a>При попытке входа в ресурс Azure вы получаете ошибку "AADSTS65005"

Пользователь, у которого есть гостевая учетная запись, не может войти в систему и получает следующее сообщение об ошибке:

    AADSTS65005: Using application 'AppName' is currently not supported for your organization contoso.com because it is in an unmanaged state. An administrator needs to claim ownership of the company by DNS validation of contoso.com before the application AppName can be provisioned.

Пользователь имеет учетную запись пользователя Azure и является вирусным арендатором, который был заброшен или неуправляемым. Кроме того, в арендаторе нет глобальных или фирменных администраторов.

Чтобы решить эту проблему, необходимо взять на себя брошенных арендаторов. Обратитесь к [неуправляемому каталогу в качестве администратора в Active Directory Azure.](https://docs.microsoft.com/azure/active-directory/users-groups-roles/domains-admin-takeover) Вы также должны получить доступ к Интернету перед DNS для домена суффикс в вопросе, с тем чтобы обеспечить прямые доказательства того, что вы контролируете пространство имен. После того, как арендатор возвращается в управляемое состояние, пожалуйста, обсудите с клиентом, является ли оставление пользователей и проверенное доменное имя лучшим вариантом для их организации.
...
## <a name="a-guest-user-is-unable-to-use-the-azuread-powershell-v1-module"></a>Гость не может использовать модуль AzureAD PowerShell V1

По состоянию на 18 ноября 2019 года гостевые пользователи в вашем каталоге (определяемые как учетные записи пользователей, где свойство **userType** равно **гостевому)** заблокированы от использования модуля AzureAD PowerShell V1. В будущем пользователь должен быть либо пользователем-участником (где **userType** равен **члену),** либо использовать модуль AzureAD PowerShell V2.

## <a name="in-an-azure-us-government-tenant-i-cant-invite-a-b2b-collaboration-guest-user"></a>В арендаторе Правительства Azure в США я не могу пригласить приглашенного приглашенного пользователя B2B

В облаке Правительства Сша ВС США сотрудничество B2B в настоящее время поддерживается только между арендаторами, которые находятся в облаке правительства Azure, и поддерживают сотрудничество B2B. Если вы приглашаете пользователя в арендатора, который не входит в облако правительства Azure В США или еще не поддерживает совместное взаимодействие B2B, вы получите ошибку. Для получения подробной информации и ограничений [см.](https://docs.microsoft.com/azure/azure-government/documentation-government-services-securityandidentity#azure-active-directory-premium-p1-and-p2)
