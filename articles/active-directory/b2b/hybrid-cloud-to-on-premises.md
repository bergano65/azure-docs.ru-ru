---
title: Grant B2B пользователи получают доступ к вашим приложениям - Azure AD
description: Показывает, как предоставить пользователям облачных B2B доступ к приложениям в помещениях с помощью совместной работы Azure AD B2B.
...
- Вы можете настроить для гостевого пользователя политику многофакторной проверки подлинности (MFA), чтобы она обязательно использовалась при аутентификации через прокси приложений. Для получения дополнительной информации [см.](conditional-access.md)
- К локальным пользователям применяются любые правила спонсорства, проверки доступа, проверки учетных записей и пр., которые определены для облачного пользователя B2B. Например, если пользователь облака удаляется через политики управления жизненным циклом, он также удаляется MIM Sync или через синхронизацию Azure AD Connect. Для получения дополнительной информации смотрите [Управление доступом для гостей с обзорами доступа Azure AD.](../governance/manage-guest-access-with-access-reviews.md)
