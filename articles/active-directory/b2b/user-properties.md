---
title: Свойства гостевого пользователя B2B - Активный каталог Azure (ru) Документы Майкрософт
description: Свойства гостевых пользователей Azure Active Directory B2B и состояния до и после выкупа приглашений
...
В этой статье описаны свойства и состояния объекта пользователя гостевых B2B в каталоге Azure Active Directory (Azure AD) до и после искупления приглашений. Пользователь совместной работы Azure AD (B2B) является пользователем user with UserType и Guest. Обычно такой пользователь является представителем партнерской организации и по умолчанию имеет ограниченные права в каталоге приглашающей организации.
...
- Состояние 4: Размещено в Azure AD организации с UserType и учетными данными, которыми управляет унистовая организация.

  ![Диаграмма с изображением четырех состояний пользователя](media/user-properties/redemption-diagram.png)
...
![Скриншот, показывающий свойства пользователя перед предложением выкупа](media/user-properties/before-redemption.png)
...
- Гость: Это значение указывает на пользователя, который не считается внутренним для компании, например, внешнего сотрудника, партнера или клиента. Такой пользователь не должен, например, получать служебных записок от генерального директора или пользоваться социальным пакетом компании.
...
- Внешний активный каталог Azure: Этот пользователь развилась во внешней организации и проверяет подлинность с помощью учетной записи Azure AD, принадлежащей другой организации. Этот тип входа соответствует состоянию 1.
...
![Скриншот, показывающий фильтр для гостевых пользователей](media/user-properties/filter-guest-users.png)
...
![Скриншот, показывающий опцию Внешних пользователей в настройках пользователя](media/user-properties/remove-guest-limitations.png)
