...
## <a name="setup-required-for-single-page-apps"></a>Настройка одностраничных приложений

Поток кода авторизации для одностраничных приложений требует некоторой дополнительной настройки.  При [создании](howto-create-service-principal-portal.md) приложения необходимо отметить URI перенаправления для приложения в качестве URI перенаправления `spa`. В результате сервер входа разрешает использовать CORS (общий доступ к ресурсам независимо от источника) для приложения.  Это необходимо для активации кода с помощью XHR.
