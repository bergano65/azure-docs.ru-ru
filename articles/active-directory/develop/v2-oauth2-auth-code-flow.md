...
| `scope`  | обязательно    | Разделенный пробелами список [областей](v2-permissions-and-consent.md) , для которого необходимо согласие пользователя.  Для `/authorize` части запроса это может охватывать несколько ресурсов, позволяя приложению получить согласие на несколько веб-интерфейсов API, которые требуется вызвать. |
...
| `token_type`    | Указывает значение типа токена. Единственный тип, поддерживаемый Azure AD, — носитель |
