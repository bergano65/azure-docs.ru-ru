...
| `scope`  | обязательные    | Разделенный пробелами список [областей](v2-permissions-and-consent.md) , на которые пользователь должен согласиться.  `/authorize` Для части запроса это может охватывать несколько ресурсов, позволяя приложению получить согласие на несколько веб-API, которые требуется вызвать. |
...
| `code_verifier` | необязательно  | Это тот же параметр code_verifier, который использовался для получения кода авторизации (authorization_code). Требуется, если в запросе на предоставление кода авторизации использовался ключ PKCE. Дополнительные сведения см. в описании [PKCE RFC](https://tools.ietf.org/html/rfc7636). |
