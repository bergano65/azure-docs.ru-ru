...
|`response_mode`| Рекомендуется | Определяет метод, который следует использовать для отправки созданного маркера запрашивающему приложению. По умолчанию принимает значение `query` только для кода авторизации, но `fragment` Если запрос содержит id_token `response_type` .|
