...
| `code_verifier` | необязательный  | Это тот же параметр code_verifier, который использовался для получения кода авторизации (authorization_code). Требуется, если в запросе на предоставление кода авторизации использовался ключ PKCE. Дополнительные сведения см. в описании [PKCE RFC](https://tools.ietf.org/html/rfc7636). |
