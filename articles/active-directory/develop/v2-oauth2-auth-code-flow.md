...
description: Создание веб-приложений с помощью реализации платформы удостоверений Microsoft протокола проверки подлинности OAuth 2.0.
...
Код авторизации OAuth 2.0 может использоваться в приложениях, установленных на устройстве, для получения доступа к защищенным ресурсам, таким как веб-API. С помощью реализации платформы удостоверений Microsoft OAuth 2.0, можно добавить вход и доступ к API в мобильные и настольные приложения. Для выполнения задач этого руководства не предусмотрено использование конкретного языка. Здесь объясняется, как отправлять и получать сообщения HTTP без применения [библиотек аутентификации Azure с открытым кодом](active-directory-authentication-libraries.md).

> [!NOTE]
> Не все сценарии Azure Active Directory и функции поддерживаются конечной точкой платформа удостоверений Майкрософт. Чтобы определить, если необходимо использовать конечную точку платформы Microsoft identity, ознакомьтесь [ограничения платформы удостоверений Microsoft](active-directory-v2-limitations.md).

Описание потока кода авторизации OAuth 2.0 см. в [разделе 4.1 спецификации OAuth 2.0](https://tools.ietf.org/html/rfc6749). Он используется для проверки подлинности и авторизации в большинстве типов приложений, включая [веб-приложения](v2-app-types.md#web-apps) и [изначально установленных приложений](v2-app-types.md#mobile-and-native-apps). Этот поток обеспечивает приложениям безопасно получать маркеры доступа, который может использоваться для доступа к ресурсам, защищенным конечной точкой платформа удостоверений Майкрософт.
...
| `client_id`   | Требуется    | **Идентификатор приложения (клиент)** , [портал Azure — регистрация приложений](https://go.microsoft.com/fwlink/?linkid=2083908) опыт, полученный вашим приложением.  |
...
| `domain_hint`  | дополнительный    | Может использоваться значение `consumers` или `organizations`.<br/><br/>Если включен, пропускается процесс обнаружения на основе электронной почты который проходит пользователь на странице входа в систему, что приводит к несколько упрощает взаимодействие с пользователем. Обычно этот параметр применяется в приложениях при повторной аутентификации. Для этого значение утверждения `tid` извлекается из предыдущего сеанса входа. Если значение утверждения `tid` — `9188040d-6c67-4c5b-b112-36a304b66dad`, следует использовать `domain_hint=consumers`. Или используйте `domain_hint=organizations`.  |
| `code_challenge_method` | дополнительный    | Метод, используемый для кодирования `code_verifier` в параметре `code_challenge`. Может принимать одно из следующих значений:<br/><br/>- `plain` <br/>- `S256`<br/><br/>Если этот параметр не указан, для `code_challenge` принимается формат открытого текста, если задано значение `code_challenge`. Поддерживает платформы удостоверений Microsoft `plain` и `S256`. Дополнительные сведения см. в описании [PKCE RFC](https://tools.ietf.org/html/rfc7636). |
| `code_challenge`  | дополнительный | Используется, чтобы защитить разрешения кода авторизации из собственного клиента при помощи ключа проверки для обмена кодом (PKCE). Является обязательным, если указан параметр `code_challenge_method`. Дополнительные сведения см. в описании [PKCE RFC](https://tools.ietf.org/html/rfc7636). |

На текущем этапе пользователю придется ввести учетные данные и завершить проверку подлинности. Конечная точка платформы удостоверений Microsoft гарантирует, что пользователь предоставил разрешения, указанные в `scope` параметр запроса. Если пользователь не предоставил какие-либо из этих разрешений, конечная точка запросит их у пользователя. Подробные сведения о [разрешениях, согласии на предоставление и мультитенантных приложениях можно найти здесь](v2-permissions-and-consent.md).
...
| Параметр | Описание  |
|-----------|--------------|
| `code` | Запрашиваемый приложением код авторизации. Приложение может использовать код авторизации для запроса маркера доступа для целевого ресурса. Мал и обычно истекает через 10 минут. |
...
| `unauthorized_client` | Клиентское приложение не разрешено запрашивать код авторизации. | Эта ошибка обычно возникает, когда клиентское приложение не зарегистрировано в Azure AD или не добавляется к клиенту Azure AD пользователя. Приложение может отобразить пользователю запрос с инструкцией по установке приложения и его добавлению в Azure AD. |
| `access_denied`  | Владелец ресурса отказал в использовании  | Клиентское приложение может уведомить пользователя, который не может продолжить работу без согласия пользователя. |
...
| `invalid_resource`  | Целевой ресурс является недопустимым, поскольку он не существует, Azure AD не удается найти или он настроен неправильно. | Это означает, что ресурс не существует или не настроен в клиенте. Приложение может отобразить пользователю запрос с инструкцией по установке приложения и его добавлению в Azure AD. |
| `login_required` | Слишком много пользователей или пользователи не найдены | Клиент запросил автоматическую аутентификацию (`prompt=none`), но одного пользователя не удалось найти. Это может означать, что в сеансе активно несколько пользователей или пользователи отсутствуют. Принимая во внимание выбранного клиента (например, в том случае, если имеется два активных записей Azure AD и одна учетная запись Майкрософт, и `consumers` выбирается, автоматическую проверку подлинности будут работать). |
...
> [!TIP]
> Попытайтесь выполнить этот запрос в Postman. (Не забудьте заменить `code`.) [![Run in Postman](./media/v2-oauth2-auth-code-flow/runInPostman.png)](https://app.getpostman.com/run-collection/f77994d794bab767596d) (Выполнить в Postman)

| Параметр  | Обязательный/необязательный | Описание     |
|------------|-------------------|----------------|
| `tenant`   | Требуется   | Значение `{tenant}` в пути запроса можно использовать для того, чтобы контролировать, кто может входить в приложение. Допустимые значения: `common`, `organizations`, `consumers`, а также идентификаторы клиента. Дополнительные сведения см. в [описании протоколов](active-directory-v2-protocols.md#endpoints).  |
| `client_id` | Требуется  | Идентификатор приложения (клиента), [портал Azure — регистрация приложений](https://go.microsoft.com/fwlink/?linkid=2083908) страницы, назначенный вашему приложению. |
| `grant_type` | Требуется   | Должен быть `authorization_code` для потока кода авторизации.   |
| `scope`      | Требуется   | Список областей с разделителями-пробелами. Области, запрашиваемые на этом участке, должны быть эквивалентны областям, запрашиваемым на первом участке, или являться их подмножеством. Если области, указанные в этом запросе, охватывают несколько серверов ресурсов, конечная точка платформы Microsoft identity будет вернуть маркер для ресурса, указанного в первой области. Более подробное описание областей можно найти в разделе, посвященном [разрешениям, согласию на их предоставление и областям](v2-permissions-and-consent.md). |
| `code`          | Требуется  | Код авторизации, полученный на первом участке потока. |
| `redirect_uri`  | Требуется  | Значение redirect_uri, которое использовалось для получения кода authorization_code. |
| `client_secret` | необходим для веб-приложений | Секрет приложения, созданный на портале регистрации для приложения. Не следует использовать секрет приложения в собственном приложении, так как client_secrets невозможно хранении на устройствах. Это обязательное поле для веб-приложений и веб-API-интерфейсы, которые имеют возможность возможностью безопасного хранения на стороне сервера.  Секрет клиента должен быть преобразован в формат URL-адреса перед отправкой.  |
...
| `unauthorized_client` | Прошедший проверку подлинности клиент не авторизована для использования этого типа предоставления авторизации. | Обычно это происходит, когда клиентское приложение не зарегистрировано в Azure AD или не добавляется к клиенту Azure AD пользователя. Приложение может отобразить пользователю запрос с инструкцией по установке приложения и его добавлению в Azure AD. |
| `invalid_client` | Сбой проверки подлинности клиента.  | Учетные данные клиента являются недопустимыми. Чтобы устранить проблему, администратору приложения необходимо обновить учетные данные.   |
...
| `invalid_resource` | Целевой ресурс является недопустимым, поскольку он не существует, Azure AD не удается найти или он настроен неправильно. | Это означает, что ресурс, если он существует, не настроен в клиенте. Приложение может отобразить пользователю запрос с инструкцией по установке приложения и его добавлению в Azure AD.  |
...
> [!TIP]
> Выполните этот запрос в Postman. (Сначала замените заголовок `Authorization`.) [![Run in Postman](./media/v2-oauth2-auth-code-flow/runInPostman.png)](https://app.getpostman.com/run-collection/f77994d794bab767596d) (Выполнить в Postman)
...
Несмотря на то, что маркеры обновления не отозван, при использовании для получения новых маркеров доступа, вы должны удалить старый маркер обновления. [Спецификации OAuth 2.0](https://tools.ietf.org/html/rfc6749#section-6) говорит: «Сервер авторизации может выдать новый маркер обновления, в котором случае клиент должен отменить старый токен обновления и замените его новый маркер обновления. Сервер авторизации может отозвать старый токен обновления после выдачи нового маркера обновления для клиента.»  
...
> [!TIP]
> Попытайтесь выполнить этот запрос в Postman. (Не забудьте заменить `refresh_token`.) [![Run in Postman](./media/v2-oauth2-auth-code-flow/runInPostman.png)](https://app.getpostman.com/run-collection/f77994d794bab767596d) (Выполнить в Postman)
> 
...
| `scope`         | Требуется    | Список областей с разделителями-пробелами. Области, запрашиваемые на этом участке, должны быть эквивалентны областям, запрашиваемым на исходном участке запроса кода авторизации, или являться их подмножеством. Если области, указанные в этом запросе, охватывают несколько серверов ресурсов, конечная точка платформы Microsoft identity будет вернуть маркер для ресурса, указанного в первой области. Более подробное описание областей можно найти в разделе, посвященном [разрешениям, согласию на их предоставление и областям](v2-permissions-and-consent.md). |
| `refresh_token` | Требуется    | Токен обновления, полученный на втором участке потока. |
| `client_secret` | необходим для веб-приложений | Секрет приложения, созданный на портале регистрации для приложения. Его не следует в собственном приложении, так как client_secrets невозможно хранении на устройствах. Это обязательное поле для веб-приложений и веб-API-интерфейсы, которые имеют возможность возможностью безопасного хранения на стороне сервера. |
