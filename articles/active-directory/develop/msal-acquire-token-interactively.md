...
Вы также можете очистить кэш токенов, что осуществляется путем удаления учетных записей из кэша. Файл cookie сеанса, находящийся в браузере, не будет удален.

## <a name="scopes-when-acquiring-tokens"></a>Области при получении токенов
[Области](v2-permissions-and-consent.md) — это разрешения, предоставляемые веб-API для запроса доступа к клиентским приложениям. Клиентские приложения запрашивают согласие пользователя на эти области при отправке запросов на аутентификацию для получения токенов, предоставляющих доступ к веб-API. MSAL позволяет получить токены для доступа к Azure AD для разработчиков (версии 1.0) и API-интерфейсов платформы удостоверений Майкрософт (версии 2.0). В запросах протокола версии 2.0 вместо ресурсов используются области. Дополнительные сведения о сравнении версий 1.0 и 2.0 см. [здесь](active-directory-v2-compare.md). В зависимости от конфигурации веб-API принимаемой версии токена конечная точка версии 2.0 возвращает маркер доступа в MSAL.

Для некоторых методов получения токенов MSAL требуется параметр *scopes*. Этот параметр представляет собой простой список строк, которые объявляют нужные разрешения и запрашиваемые ресурсы. Хорошо известными областями являются [разрешения Microsoft Graph](/graph/permissions-reference).
...
Когда приложению необходимо запросить токены с определенными разрешениями для API ресурса, вам понадобится передать области, содержащие URI идентификатора приложения API-интерфейса, в следующем формате: *&lt;URI идентификатора приложения&gt;/&lt;область&gt;*.
...
Только для API Microsoft Graph значение области `user.read` сопоставляется с форматом `https://graph.microsoft.com/User.Read` и они могут быть взаимозаменяемыми.
...
При создании приложений с использованием версии 1.0 вам нужно было зарегистрировать полный набор разрешений (статические области), необходимых приложению для того, чтобы пользователь дал согласие во время входа. В версии 2.0 вы можете запросить дополнительные разрешения по мере необходимости, используя параметр области. Они называются динамическими областями и разрешают пользователю предоставлять добавочное согласие для областей.

Например, вы можете изначально выполнить вход для пользователя и запретить ему какой-либо доступ. Позже вы сможете предоставить ему возможность чтения календаря другого пользователя, запросив область календаря в методах получения токенов, и получить согласие пользователя.
...
MSAL хранит кэш токенов (или два кэша для конфиденциальных клиентских приложений) и кэширует токен после его получения.  Во многих случаях при попытке получить токен автоматически будет получен еще один токен с дополнительными областями в зависимости от токена в кэше. При этом токен также может быть обновлен, если его срок действия скоро истекает (так как кэш токенов также содержит токен обновления).
...
Тем не менее есть два потока, перед которыми **не следует** предпринимать попытку автоматически получить токен:
...
- [Поток с использованием кода авторизации](msal-authentication-flows.md#authorization-code) в веб-приложениях, так как он активирует код, получаемый приложением при входе пользователя в систему, и содержит его согласие на дополнительные области. Так как код передается в качестве параметра, а не учетной записи, метод не может выполнять поиск в кэше до активации кода, для чего в любом случае требуется вызов к службе.
...
Для веб-приложений, использующих [поток кода авторизации OpenID Connect](v2-protocols-oidc.md), рекомендуемый шаблон в контроллерах подразумевает следующие действия:
...
Для общедоступных клиентских приложений (классическое или мобильное приложение):
...
Когда клиент запрашивает маркер доступа, Azure AD также возвращает результат аутентификации, содержащий некоторые метаданные о маркере доступа. Эти сведения включают время окончания срока действия маркера доступа и области, для которых он действителен. Благодаря этим данным приложение может выполнять интеллектуальное кэширование маркеров доступа без анализа самого маркера.  В результате аутентификации содержится:

- [Маркер доступа](access-tokens.md) для доступа веб-API к ресурсам. Это строка, обычно JWT в кодировке Base64, но клиенту не следует просматривать содержимое маркера доступа. Стабильность формата не гарантируется, и его можно зашифровать для ресурса. Люди, пишущие код на основе содержимого маркера доступа на стороне клиента, являются одним из основных источников ошибок и разрывов логики клиента.
- [Токен идентификации](id-tokens.md) для пользователя (это JWT).
- Срок действия токена, который указывает дату и время истечения срока действия токена.
- Идентификатор клиента содержит сведения о клиенте, в котором был найден пользователь. Для гостевых пользователей (сценарии Azure AD B2B) идентификатор клиента — это гостевой, а не уникальный клиент. Когда токен предоставляется в имени пользователя, результат аутентификации также содержит сведения об этом пользователе. Для потоков конфиденциальных клиентов, где токены запрашиваются без пользователя (для приложения), эта информация пользователя имеет значение null.
