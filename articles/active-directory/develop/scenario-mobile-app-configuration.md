...
Когда MSAL.NET вызывает брокер, брокер выполняет обратный вызов к приложению. Он вызывает обратную передачу с `AppDelegate.OpenUrl` помощью метода. Так как MSAL ожидает ответа от брокера, приложение должно взаимодействовать для вызова MSAL.NET назад. Это поведение настраивается путем обновления `AppDelegate.cs` файла для переопределения метода, как показано в следующем коде.
...
1. В `AppDelegate.cs`задайте `App.RootViewController` новое `UIViewController()`значение. Этот параметр обеспечивает включение `UIViewController`в вызов брокера. Если оно задано неправильно, может появиться следующее сообщение об ошибке:
...
MSAL.NET использует URL-адреса для вызова брокера, а затем возвращает ответ брокера обратно в приложение. Чтобы завершить цикл обработки, зарегистрируйте в `Info.plist` файле схему URL-адресов приложения. 

Чтобы зарегистрировать схему URL-адресов приложения, выполните следующие действия.

1. С `CFBundleURLSchemes` `msauth`префиксом. 
1. Добавьте `CFBundleURLName` в конец. Используйте следующий шаблон: 

   `$"msauth.(BundleId)"`

   Здесь `BundleId` уникально идентифицирует ваше устройство. Например, если `BundleId` имеет значение `yourcompany.xforms`, то схема URL- `msauth.com.yourcompany.xforms`адреса будет иметь значение.
...
> [!NOTE]
> Если вы приняли `UISceneDelegate` в iOS 13 или более поздней версии, поставьте обратный вызов `scene:openURLContexts:` MSAL `UISceneDelegate` в вместо. MSAL `handleMSALResponse:sourceApplication:` необходимо вызывать только один раз для каждого URL-адреса.
>
...
1. Добавьте в префикс настраиваемой схемы `msauth`URL-адреса. 
