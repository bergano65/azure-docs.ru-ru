...
Для первого входа пользователя в приложение, можно отправить [OpenID Connect](v2-protocols-oidc.md) запрос на авторизацию и get `id_token` из конечной точки платформы Microsoft identity.
...
| `client_id` | обязательно | Идентификатор приложения (клиента), [Azure портал регистрации приложений](https://go.microsoft.com/fwlink/?linkid=2083908) страницы, назначенный вашему приложению. |
...
| `prompt` | необязательный |Указывает требуемый тип взаимодействия с пользователем. На текущий момент единственные допустимые значения — login, none, select_account и consent. `prompt=login` будет предложить пользователю ввести свои учетные данные по запросу, не сработает единого входа. `prompt=none` является противоположным — оно гарантирует, что пользователю не предоставлены все запросы на экран ни при каких обстоятельствах. Если запрос не удастся завершить автоматически с помощью единого входа, конечная точка платформы удостоверений Microsoft вернет ошибку. `prompt=select_account` отправляет пользователя средство выбора учетной записи, где отображаются все учетные записи, хранится в сеансе. `prompt=consent` Активирует диалоговое окно согласия OAuth после входа пользователя, запросом на предоставление разрешений для приложения. |
