...
> [!IMPORTANT]
> После прослуха клиентов во время предварительной версии мы реализовали [возможности управления сеансом проверки](https://go.microsoft.com/fwlink/?linkid=2083106) подлинности в условном доступе Azure AD. С помощью этой новой функции можно настроить время существования маркера обновления, установив частоту входа. После 1 ноября 2019 вы не сможете использовать настраиваемую политику времени существования маркеров для настройки маркеров сеанса и обновления. Вы по-прежнему можете настроить время существования маркера доступа после его устаревания.
...
### <a name="id-tokens"></a>Токен идентификатора
...
| Свойство | Строка свойства политики | Область применения | Значение по умолчанию | Минимальная | Максимум |
| --- | --- | --- | --- | --- | --- |
...
* <sup>2</sup> Чтобы обеспечить работу веб-клиента Microsoft Teams, рекомендуется задать для Microsoft Teams Акцесстокенлифетиме более 15 минут.
...
>
> Пользователь хочет получить доступ к двум веб-приложениям: веб-приложению A и веб-приложению Б.
> 
> Факторы:
...
**Область применения.** Маркеры доступа, маркеры идентификации
...
**Описание.** Эта политика определяет, как долго пользователь может применять маркер сеанса для получения новых маркеров сеанса и идентификации после последней успешной аутентификации с использованием однофакторных методов. Когда пользователь выполнит проверку подлинности и получит новый маркер сеанса, он сможет и дальше использовать поток маркеров сеанса в течение указанного периода времени. (До того момента, пока маркер текущего сеанса не будет отозван или не истечет его срок действия.) После указанного периода времени пользователь должен повторно выполнить проверку подлинности, чтобы получить новый маркер сеанса.
...
**Описание.** Эта политика определяет, как долго пользователь может применять маркер сеанса для получения новых маркеров сеанса и идентификации после последней успешной аутентификации с использованием многофакторных методов. Когда пользователь выполнит проверку подлинности и получит новый маркер сеанса, он сможет и дальше использовать поток маркеров сеанса в течение указанного периода времени. (До того момента, пока маркер текущего сеанса не будет отозван или не истечет его срок действия.) После указанного периода времени пользователь должен повторно выполнить проверку подлинности, чтобы получить новый маркер сеанса.
