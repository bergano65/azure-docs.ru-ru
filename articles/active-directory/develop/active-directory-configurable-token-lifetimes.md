...
Токены SAML используются многими веб-приложениями SAAS и получаются с помощью конечной точки протокола SAML2 Azure Active Directory.  Они также используются приложениями, использующими WS-Federation.    Время существования маркера по умолчанию составляет 1 час. После и с точки зрения приложений срок действия маркера определяется значением Нотонорафтер условий <... > элемент в токене.  По истечении срока действия маркера клиент должен инициировать новый запрос проверки подлинности, который, как правило, будет удовлетворен без интерактивного входа в результате маркера сеанса единого входа (SSO).

Значение Нотонорафтер можно изменить с помощью параметра Акцесстокенлифетиме в TokenLifetimePolicy.  Для него будет задано время существования, настроенное в политике, если таковое имеется, а также коэффициент наклона часов, равный пяти минутам.
