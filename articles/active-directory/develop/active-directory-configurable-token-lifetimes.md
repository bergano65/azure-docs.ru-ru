---
title: Настраиваемые сроки службы маркеров Azure AD
...
> [!IMPORTANT]
> Выслушав от клиентов во время предварительного просмотра, мы внедрили [возможности управления сеансами проверки подлинности](https://go.microsoft.com/fwlink/?linkid=2083106) в Azure AD Conditional Access. Эту новую функцию можно использовать для настройки сроков обновления маркеров, установив знак частоты. После 1 мая 2020 года вы не сможете использовать политику Configurable Token Lifetime для настройки сеанса и обновления токенов. Вы все еще можете настроить сроки доступа к токену после амортизм.
...
> [!NOTE]
> Политика в отношении сроковых действия токенов применяется только к мобильным и настольным клиентам, которые получают доступ к ресурсам SharePoint Online и OneDrive для бизнеса, и не распространяется на сеансы веб-браузера.
> Для управления сроком службы сеансов веб-браузера для SharePoint Online и OneDrive для бизнеса используйте функцию [«Условная продолжительность сеанса доступа».](../conditional-access/howto-conditional-access-session-lifetime.md) Ознакомьтесь с [блогом о SharePoint Online](https://techcommunity.microsoft.com/t5/SharePoint-Blog/Introducing-Idle-Session-Timeout-in-SharePoint-and-OneDrive/ba-p/119208), чтобы получить дополнительные сведения о настройке времени ожидания бездействующих сеансов.

## <a name="token-types"></a>Типы маркеров

Политики срока службы токенов можно установить для маркеров обновления, токенов доступа, токенов SAML, токенов сеансов и токенов и идентификаторов.
...
Токены SAML используются многими веб-приложениями SAAS и доявляютсяся с помощью конечной точки протокола SAML2 протокола Azure Active. Они также потребляются приложениями с использованием WS-Federation. Срок службы токена по умолчанию составляет 1 час. С точки зрения приложения, срок действия токена указан значением NotOnOrAfter `<conditions …>` элемента в токене. После окончания срока действия токена клиент должен инициировать новый запрос на проверку подлинности, который часто удовлетворяется без интерактивного ввоза в результате токена «Один знак на» (SSO).

Значение NotOnOrAfter может быть изменено с помощью `AccessTokenLifetime` параметра в `TokenLifetimePolicy`. Он будет установлен на срок службы настроен в политике, если таковые имеется, плюс фактор перекоса часов в пять минут.

Обратите внимание, что подтверждение объекта NotOnOrAfter, указанное в `<SubjectConfirmationData>` элементе, не зависит от конфигурации Token Lifetime. 
...
Общедоступные клиенты не могут обеспечить безопасное хранение клиентского пароля (секрета). Например, приложение iOS или Android не может замаскировать секрет от владельца ресурса, и поэтому считается общедоступным клиентом. Для ресурсов можно определить политики, которые запрещают маркерам обновления из общедоступных клиентских приложений, которые старше указанного периода, получать новую пару маркеров доступа и обновления. (Для этого используйте свойство "Обновить маркер`MaxInactiveTime`Макс" ().) Можно также использовать политики для установки периода, после которого токены обновления больше не принимаются. (Для этого используйте свойство Refresh Token Max Age.) Вы можете настроить срок службы маркера обновления, чтобы контролировать, когда и как часто пользователю требуется повторно вводить учетные данные, вместо того, чтобы молча повторно reauthenticated, при использовании приложения для публичных клиентов.

> [!NOTE]
> Свойство Max Age — это время использования одного токена. 
...
Когда пользователь выполняет аутентификацию в Azure AD, для пользовательского браузера и Azure AD создается сеанс единого входа. Этот сеанс представлен маркером сеанса единого входа — в виде файла cookie. Токен сеанса SSO не привязан к конкретному приложению ресурса/клиента. Маркеры сеанса единого входа могут быть отозваны, а при каждом использовании проверяется их действительность.

В Azure AD используется два вида маркеров сеанса единого входа: постоянные и временные. Постоянные маркеры сеанса хранятся в постоянных файлах cookie, а временные — хранятся в виде файлов cookie сеанса. (Сессионные файлы cookie уничтожаются при закрытии браузера.) Обычно сохраняется маркер сеанса неустойчивого. Но когда пользователь при аутентификации устанавливает флажок **Оставаться в системе**, сохраняется постоянный маркер сеанса.

Срок действия временных маркеров сеанса составляет 24 часа, а постоянных маркеров — 180 дней. Всякий раз, когда токен сеанса SSO используется в течение срока действия, срок действия продлевается еще на 24 часа или 180 дней, в зависимости от типа токена. Если маркер сеанса единого входа не используется в течение срока его действия, он считается истекшим и не будет приниматься.

С помощью политик можно задать период времени после выдачи первого маркера сеанса, по прошествии которого такой маркер не будет приниматься. (Для этого используйте свойство Session Token Max Age.) Можно настроить срок службы маркера сеанса для управления, когда и как часто пользователю требуется повторно вводить учетные данные, вместо того, чтобы молча аутентифицироваться при использовании веб-приложения.
...
* Если принцип службы не назначен, организация или объект приложения не назначены, значения по умолчанию не выполняются. (См. таблицу в разделе [Свойства для настройки времени жизни маркера](#configurable-token-lifetime-properties).)
...
**Влияет на:** Токены доступа, токены ID, токены SAML

**Описание:** эта политика определяет, как долго продолжают действовать маркеры доступа и маркеры идентификации для определенного ресурса. Чем меньше значение свойства времени жизни маркера доступа, тем ниже риск использования маркера доступа или маркера идентификатора вредоносным субъектом в течение длительного времени. (Эти токены не могут быть отменены.) Компромисс заключается в том, что на производительность отрицательно сказывается, поскольку токены должны заменяться чаще.
...
**Описание:** эта политика определяет, как долго пользователь может применять маркер обновления для получения новых пар маркеров доступа и обновления после последней успешной проверки подлинности с использованием многофакторных методов. Когда пользователь выполнит проверку подлинности и получит новый маркер обновления, он сможет и дальше использовать маркер обновления в течение указанного периода времени. (Это верно до тех пор, пока ток маркер обновления не отменяется, и он не используется дольше, чем неактивное время.) В этот момент пользователи вынуждены повторно аутентифицировать, чтобы получить новый токен обновления.
...
**Описание:** эта политика определяет, как долго пользователь может применять маркер сеанса для получения новых маркеров сеанса и идентификатора после последней успешной проверки подлинности с использованием однофакторных методов. Когда пользователь выполнит проверку подлинности и получит новый маркер сеанса, он сможет и дальше использовать поток маркеров сеанса в течение указанного периода времени. (Это верно до тех пор, пока маркер текущей сессии не будет аннулирован и не истек.) По истечении указанного периода времени пользователь вынужден повторно аутентифицироваться, чтобы получить новый токен сеанса.
...
В этом примере создается политика, которая позволяет пользователям реже вскакивать по всей организации. Для этого создайте политику времени жизни для однофакторных маркеров обновления и примените ее к организации. Эта политика будет распространяться на все приложения в организации и на все субъекты-службы, для которых не установлены отдельные политики.
...
В этом примере вы создаете несколько политик, чтобы узнать, как работает система приоритетов. Вы также узнаете, как управлять несколькими политиками, которые применяются к нескольким объектам.
...
| Параметры | Описание | Пример |
| --- | --- | --- |
| <code>&#8209;Id</code> (необязательный параметр) |**ObjectId (ID)** политики, который вы хотите. |`-Id <ObjectId of Policy>` |
...
| Параметры | Описание | Пример |
| --- | --- | --- |
| <code>&#8209;Id</code> |**ObjectId (ID)** политики, который вы хотите. |`-Id <ObjectId of Policy>` |
...
| Параметры | Описание | Пример |
| --- | --- | --- |
| <code>&#8209;Id</code> |**ObjectId (ID)** приложения. | `-Id <ObjectId of Application>` |
