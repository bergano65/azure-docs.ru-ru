...
> [!IMPORTANT]
> После 30 января 2021 клиенты больше не смогут настраивать время существования маркера обновления и сеанса, а Azure Active Directory прекращает учитывать существующую конфигурацию маркера обновления и сеанса в политиках после этой даты. Вы по-прежнему можете настроить время существования маркера доступа после выхода из эксплуатации.
> Мы реализовали [возможности управления сеансом проверки подлинности](../conditional-access/howto-conditional-access-session-lifetime.md)   в условном доступе Azure AD. С помощью этой новой функции можно настроить время существования маркера обновления, установив частоту входа. Условный доступ — это функция Azure AD Premium P1, и вы можете оценить, подходит ли Premium на органзатион на [странице цен Premium](https://azure.microsoft.com/en-us/pricing/details/active-directory/). 
> 
> Для клиентов, которые не используют управление сеансами проверки подлинности в условном доступе после даты выбытия, они могут рассчитывать на то, что Azure AD будет учитывать конфигурацию по умолчанию, описанную в следующем разделе.
...
На конфигурацию обновления и токена сеанса влияют следующие свойства и соответствующие им значения. После прекращения настройки обновления и конфигурации маркера сеанса Azure AD будет учитывать только значение по умолчанию, описанное ниже, независимо от того, настроены ли для политик пользовательские значения.  
...
|Максимальный возраст многофакторного маркера обновления  |MaxAgeMultiFactor  |Маркеры обновления (для всех пользователей) |180 дней  |
...
|Максимальный возраст многофакторного маркера сеанса  |MaxAgeSessionMultiFactor  |Маркеры сеанса (постоянные и временные)  |180 дней |

Для определения политик времени жизни маркера, значения свойств которых отличаются от значений по умолчанию Azure AD, можно использовать командлет [Get-AzureADPolicy](/powershell/module/azuread/get-azureadpolicy?view=azureadps-2.0-preview&preserve-view=true) .

Чтобы еще больше понять, как используются политики в клиенте, можно использовать командлет [Get-AzureADPolicyAppliedObject](/powershell/module/azuread/get-azureadpolicyappliedobject?view=azureadps-2.0-preview&preserve-view=true) , чтобы определить, какие приложения и субъекты-службы связаны с вашими политиками. 

Если у клиента есть политики, определяющие пользовательские значения для свойств обновления и конфигурации токена сеанса, корпорация Майкрософт рекомендует обновить эти политики в области до значений, отражающих значения по умолчанию, описанные выше. Если изменения не вносятся, Azure AD будет автоматически учитывать значения по умолчанию.  
