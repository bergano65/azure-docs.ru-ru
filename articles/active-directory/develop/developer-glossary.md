---
title: Глоссарий платформы идентификации Microsoft Azure
description: Список терминов для широко используемых концепций и функций разработчика платформы идентификационных данных Майкрософт.
...
---
# <a name="microsoft-identity-platform-developer-glossary"></a>Глоссарий платформы идентификации Майкрософт

Эта статья содержит определения для некоторых основных концепций разработчика и терминологии, которые полезны при изучении разработки приложений с помощью платформы идентификации Microsoft.
...
Для получения более подробной информации можно узнать подробную [информацию о платформе токенов корпорации Майкрософт.][AAD-Tokens-Claims]

## <a name="application-id-client-id"></a>идентификатор приложения (идентификатор клиента)

Уникальный идентификатор, который служба Azure AD присваивает зарегистрированному приложению. Идентификатор определяет конкретное приложение и связанные с ним конфигурации. Этот идентификатор приложения[(идентификатор клиента)](https://tools.ietf.org/html/rfc6749#page-15)используется при выполнении запросов на аутентификацию и предоставляется библиотекам аутентификации во время разработки. Идентификатор приложения (идентификационный код клиента) не является секретом.
...
* Брокерский доступ к [защищенным ресурсам](#resource-server) [клиентскими приложениями](#client-application)через [сервер авторизации](#authorization-server) OAuth 2.0
...
В случае интеграции приложений платформы идентификационных данных Майкрософт идентификационная платформа Майкрософт реализует роль сервера авторизации для приложений Azure AD и ADI службы Майкрософт, например [AA, майконную графику.][Microsoft-Graph]
...
Для получения более подробной информации можно узнать [ссылку на токены платформы майкрософт.][AAD-Tokens-Claims]
...
[Токен безопасности](#security-token) [OpenID Connect,][OpenIDConnect-ID-Token] предоставляемый [конечной точкой авторизации](#authorization-endpoint) [сервера авторизации,](#authorization-server) который содержит [утверждения,](#claim) относящиеся к аутентификации [владельца конечного пользователя ресурса.](#resource-owner) Подобно маркеру доступа, маркеры идентификации также представлены в виде маркеров [JSON Web Token (JWT)][JWT] с цифровой подписью. Тем не менее в отличие от маркера доступа утверждения маркера идентификации не используются для целей, связанных с доступом к ресурсам и контролем доступа.
...
Запросы разрешений настраиваются на странице **разрешений API** для приложения на [портале Azure,][AZURE-portal]выбирая желаемые "Делегированные разрешения" и "Разрешения приложений" (последнее требует членства в глобальной роли админирования). Так как [общедоступный клиент](#client-application) не может обеспечить безопасное хранение учетных данных, он может запросить только делегированные разрешения, в то время как [конфиденциальный клиент](#client-application) может запросить и делегированные разрешения, и разрешения приложения. [Объект приложения](#application-object) клиента сохраняет объявленные разрешения в [свойстве requiredResourceAccess][Graph-App-Resource].
...
Сервер ресурсов предоставляет API-интерфейсы и обеспечивает доступ к размещенным на нем защищенным ресурсам на основе [областей](#scopes) и [ролей](#roles), используя OAuth 2.0 Authorization Framework. В качестве примеров можно привести [API Microsoft Graph,][Microsoft-Graph] предоставляющий доступ к данным о клиентах Azure AD, и API Office 365, предоставляющие доступ к таким данным, как почта и календарь. 

Как и в клиентском приложении, конфигурация удостоверения приложения ресурсов устанавливается путем [регистрации](#application-registration) в клиенте Azure AD, при этом предоставляется и объект приложения, и объект субъекта-службы. Некоторые aPI, предоставленные корпорацией Майкрософт, такие как Microsoft Graph API, имеют предварительно зарегистрированные принципы обслуживания, доступные во всех клиентах во время подготовки.
...
Для подробного обсуждения ролей приложений, выставленных на API Графика Майкрософт, [см.][Graph-Perm-Scopes] Пример пошаговой реализации см. в статье [Управление доступом с помощью RBAC и портала Azure][AAD-RBAC].
...
Рекомендуется использовать такой формат именования: resource.operation.constraint. Для подробного обсуждения областей, выставленных На примере Microsoft Graph API, [см.][Graph-Perm-Scopes] Сведения об областях, предоставляемых службами Office 365, см. в статье [Microsoft Graph permission scopes][O365-Perm-Ref] (Области действия разрешений Microsoft Graph).
...
При регистрации/обновлении приложения на [портале Azure][AZURE-portal]портал создает/обновляет как [объект приложения,](#application-object) так и соответствующий основной объект службы для этого клиента. Объект приложения глобально *определяет* конфигурацию удостоверения для приложения (для всех клиентов, где связанному приложению предоставлен доступ) и шаблон, *на основе* которого создаются соответствующие объекты субъектов-служб для локального использования во время выполнения (в конкретном клиенте).
...
Аналогично объекту субъекта-службы, который представляет экземпляр приложения, объект субъекта-пользователя является другим типом субъекта безопасности, который представляет пользователя. [Тип ресурса][Graph-User-Resource] Microsoft Graph User определяет схему для объекта пользователя, включая связанные с пользователем свойства, такие как имя и фамилия, имя пользователя, членство в роли каталога и т.д. Это обеспечивает конфигурацию идентификации пользователя для Azure AD для определения принципала пользователя во время выполнения. Субъект-пользователь представляет прошедшего проверку подлинности пользователя при едином входе, записи делегирования права [согласия](#consent), принятии решений по контролю доступа и т. п.
...
[Руководство разработчика идентификационной платформы Майкрософт][AAD-Dev-Guide] — это целевая страница для всех тем, связанных с разработкой платформы идентификации Майкрософт, включая обзор [интеграции приложений][AAD-How-To-Integrate] и основ [аутентификации платформы майкрософт и поддерживаемых сценариев аутентификации.][AAD-Auth-Scenarios] Руководства и примеры кода для быстрого начала работы можно также найти на сайте [GitHub](https://github.com/azure-samples?utf8=%E2%9C%93&q=active%20directory&type=&language=).
