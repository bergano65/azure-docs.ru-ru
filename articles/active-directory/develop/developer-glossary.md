...
Функция, предоставляемая [портал Azure][AZURE-portal], которая создает JSON-представление конфигурации удостоверения приложения, используемое в качестве механизма обновления связанных сущностей [приложения][Graph-App-Resource] и [ServicePrincipal][Graph-Sp-Resource] . Дополнительные сведения см. [в статье основные сведения о манифесте приложения Azure Active Directory][AAD-App-Manifest] .
...
Роли — это определенные ресурсом строки (например, "утверждающий расходы", "только для чтения", "Directory. ReadWrite. ALL"), управляемые в [портал Azure][AZURE-portal] с помощью [манифеста приложения](#application-manifest)ресурса и хранятся в [свойстве appRoles][Graph-Sp-Resource]ресурса. С помощью портала Azure можно также назначать пользователям роль "пользователь" и настраивать [разрешения приложений](#permissions) клиента для доступа к роли "приложение".

Подробное описание ролей приложений, предоставляемых Microsoft Graph API, см. в разделе [API Graph области разрешений][Graph-Perm-Scopes]. Пример пошаговой реализации см. [в разделе Управление доступом с помощью RBAC и портал Azure][AAD-RBAC].
...
Области — это определенные ресурсом строки (например, «mail. Read», «Directory. ReadWrite. ALL»), управляемые в [портал Azure][AZURE-portal] с помощью [манифеста приложения](#application-manifest)ресурса и хранимые в [свойстве oauth2Permissions][Graph-Sp-Resource]ресурса. Портал Azure также используется для настройки [делегированных разрешений](#permissions) клиентского приложения для доступа к области.

Рекомендуется использовать такой формат именования: resource.operation.constraint. Подробное описание областей, предоставляемых Microsoft Graph API, см. в разделе [API Graph области разрешений][Graph-Perm-Scopes]. Сведения об областях, предоставляемых службами Office 365, см. в [справочнике по разрешениям API для office 365][O365-Perm-Ref].
