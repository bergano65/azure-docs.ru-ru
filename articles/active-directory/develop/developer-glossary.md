---
title: Глоссарий разработчика платформы удостоверений Microsoft | Azure
description: Список терминов для часто используемых понятий для разработчиков платформы Microsoft identity и компоненты.
...
ms.locfileid: "66235354"
---
# <a name="microsoft-identity-platform-developer-glossary"></a>Глоссарий разработчика платформы удостоверений Microsoft

Эта статья содержит определения некоторых основных понятий для разработчиков и терминов, которые могут быть полезны при изучении разработки приложений с помощью платформы Microsoft identity.
...
См. в разделе [платформой Microsoft identity справочнике по маркерам] [ AAD-Tokens-Claims] для получения дополнительных сведений.

## <a name="application-id-client-id"></a>Идентификатор приложения (идентификатор клиента)

Уникальный идентификатор, который служба Azure AD присваивает зарегистрированному приложению. Идентификатор определяет конкретное приложение и связанные с ним конфигурации. Этот идентификатор приложения ([идентификатор клиента](https://tools.ietf.org/html/rfc6749#page-15)) используется при проверке подлинности запрашивает и предоставленные библиотеки проверки подлинности во время разработки. Идентификатор приложения (идентификатор клиента) не секрет.
...
* Доступ через брокер к [защищенным ресурсам](#resource-server) по [клиентских приложений](#client-application), с помощью OAuth 2.0 [сервера авторизации](#authorization-server)
...
При интеграции приложений платформы Microsoft identity, платформы удостоверений Microsoft реализует роль сервера авторизации для приложения Azure AD и API, службы Майкрософт, например [API-интерфейсов Microsoft Graph] [Microsoft-Graph].
...
См. в разделе [справочнике по маркерам платформы Microsoft identity] [ AAD-Tokens-Claims] для получения дополнительных сведений.
...
[Руководства разработчика платформы Microsoft identity] [ AAD-Dev-Guide] — целевая страница для все Microsoft identity платформы разработки темы, включая Обзор [приложения Интеграция] [ AAD-How-To-Integrate] и основы [проверки подлинности платформы удостоверений Microsoft и поддерживаемые сценарии проверки подлинности][AAD-Auth-Scenarios]. Руководства и примеры кода для быстрого начала работы можно также найти на сайте [GitHub](https://github.com/azure-samples?utf8=%E2%9C%93&q=active%20directory&type=&language=).
