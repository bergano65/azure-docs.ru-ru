...
В этом руководстве описано, как реализовать вход через учетную запись Майкрософт, используя решение ASP.NET MVC (Модель — представление — контроллер) с традиционным браузерным приложением, используя OpenID Connect. Вы узнаете, как настроить вход с помощью рабочих и учебных учетных записей в приложении ASP.NET.
...
* Установите Visual Studio 2015 с обновлением 3 или более позднюю версию. У вас его нет? [Скачайте Visual Studio 2019 бесплатно](https://www.visualstudio.com/downloads/).
...
Браузер обращается к веб-сайту ASP.NET и предлагает пользователю пройти проверку подлинности с помощью кнопки входа. В этом сценарии большая часть работы по отображению веб-страницы происходит на стороне сервера.

В этом руководстве показано, как обеспечить вход пользователей в веб-приложении ASP.NET, начиная с пустого шаблона. Здесь описываются такие действия, как добавление кнопки входа, контроллеров и методов, а также обсуждаются некоторые основные понятия, связанные с этими задачами. Вы также можете создать проект для входа пользователей Azure AD (с использованием рабочих и учебных учетных записей). Для этого воспользуйтесь [веб-шаблоном Visual Studio](https://docs.microsoft.com/aspnet/visual-studio/overview/2013/creating-web-projects-in-visual-studio#organizational-account-authentication-options), выберите **Учетные записи в организации** и укажите одно из облачных решений. Этот вариант предлагает расширенный шаблон с дополнительными контроллерами, методами и представлениями.
...
2. Для параметра **Тип проекта** выберите **Веб**, а затем выберите **Веб-приложение ASP.NET (.NET Framework)** .
3. Присвойте имя приложению и нажмите кнопку **Создать**.
4. Выберите **Пустой**, а затем в разделе **Добавить папки и основные ссылки** выберите **MVC**, чтобы добавить ссылки на MVC.
...
3. Перейдите на страницу [Регистрация приложений](https://go.microsoft.com/fwlink/?linkid=2083908) Платформы удостоверений Майкрософт для разработчиков.
...
6. В разделе **Поддерживаемые типы учетных записей** выберите **Accounts in any organizational directory and personal Microsoft accounts** (Учетные записи в любом каталоге организации и личные учетные записи Майкрософт).
7. Для платформы выберите тип **Интернет** в разделе **URI перенаправления** и установите значение для *URL-адреса SSL* проекта Visual Studio (расположение, в которое Azure AD будет возвращать маркеры).
78. По завершении щелкните **Зарегистрировать**. На странице приложения **Обзор** скопируйте **идентификатор приложения (клиента)** .
9. Вернитесь в Visual Studio и в `web.config` замените `Enter_the_Application_Id_here` идентификатором зарегистрированного приложения.
...
Приложение можно настроить так, чтобы разрешить вход только пользователям, принадлежащим к одному экземпляру Azure AD организации, или пользователям, принадлежащим к любой организации. Выберите один из следующих вариантов:
...
Выполните следующие действия, если требуется принимать операции входа с рабочими и учебными учетными записями из любой компании или организации, интегрированной с Azure AD. Этот сценарий используется для *приложений SaaS*:

1. Вернитесь на [портал регистрации приложений Microsoft Azure](https://portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/RegisteredApps) и найдите зарегистрированное приложение.
...
Другой вариант — реализовать пользовательский метод для проверки издателей с помощью параметра *IssuerValidator*. См. подробнее о `TokenValidationParameters` в этой [статье MSDN](https://msdn.microsoft.com/library/system.identitymodel.tokens.tokenvalidationparameters.aspx "Статья о классе TokenValidationParameters на сайте MSDN").
...
Вошедший пользователь перенаправляется на домашнюю страницу веб-сайта по URL-адресу для протокола HTTPS, которой указан в сведениях о регистрации приложения на портале регистрации приложений Майкрософт. Теперь на этой странице отображается *приветствие*, ссылка, чтобы выйти из приложения, и ссылка, чтобы увидеть утверждения пользователя. Она является ссылкой на ранее созданный контроллер авторизации.
