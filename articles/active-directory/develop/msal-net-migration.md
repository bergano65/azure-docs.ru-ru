...
- Можно проверить подлинность более широкого набора идентификационных данных Майкрософт (идентификаторы Azure AD и учетные записи Майкрософт, а также учетные записи социальных и локальных с помощью Azure AD B2C) при использовании конечной точки платформы идентификации Майкрософт,
- ваши пользователи получат лучший опыт с одним подписанием.
- Ваше приложение может включить поэтапное согласие, а поддержка условного доступа проще
- вы получаете выгоду от инноваций.

**MSAL.NET теперь рекомендуется Auth библиотека для использования с платформой microsoft identity.** Новые функции ADAL.NET не будут реализованы. Усилия направлены на улучшение MSAL.

В этой статье описаны различия между Библиотекой аутентификации Майкрософт для .NET (MSAL.NET) и Библиотекой аутентификации Azure AD для .NET (ADAL.NET) и позволяют перейти на MSAL.
...
Но ADAL.NET пока остается обязательной для тех случаев, когда приложению нужно обрабатывать вход пользователей из более ранних версий [служб федерации Active Directory (ADFS)](/windows-server/identity/active-directory-federation-services). Для получения дополнительной [ADFS support](https://aka.ms/msal-net-adfs-support)информации см.
...
- Приобретение токенов: ADAL.NET и MSAL.NET имеют`AcquireTokenAsync` одинаковые вызовы `AcquireTokenInteractive` `AcquireTokenSilent` аутентификации (и `AcquireTokenSilentAsync` для ADAL.NET, и в MSAL.NET), но с различными параметрами требуется. В числе отличий важно отметить, что в MSAL.NET уже не нужно передавать в приложение `ClientID` при каждом вызове AcquireTokenXX. Теперь `ClientID` задается только один раз при создании `IPublicClientApplication` или `IConfidentialClientApplication`.
...
- Работающее с ADAL.NET общедоступное клиентское приложение, которое получит такое исключение, должно вызывать переопределение `AcquireTokenInteractive` с параметром утверждения. Это переопределение `AcquireTokenInteractive` никогда не обращается к кэшу, так как в этом нет необходимости. Дело в том, что маркер в кэше наверняка не имеет правильных утверждений (иначе не возникло бы исключение `AdalClaimChallengeException`). Это означает, что проверять кэш не нужно. Обратите внимание, что `ClaimChallengeException` может быть получен в WebAPI `AcquireTokenInteractive` делать OBO, в то время как необходимо вызвать в публичном клиентском приложении вызова этого веб-API.
...
Встроенная проверка подлинности Windows | [Встроенная аутентификация Windows (Kerberos)](https://github.com/AzureAD/azure-activedirectory-library-for-dotnet/wiki/AcquireTokenSilentAsync-using-Integrated-authentication-on-Windows-(Kerberos)) | [Встроенная проверка подлинности Windows](msal-authentication-flows.md#integrated-windows-authentication)
...
Вот гранты, поддерживаемые в ADAL.NET и MSAL.NET для веб-приложений, веб-AA и daemon приложений:

Тип приложения | Предоставить | ADAL.NET | MSAL.NET
----- | ----- | ----- | -----
Веб-приложение, веб-aPI, daemon | Учетные данные клиента | [Потоки учетных данных клиента в ADAL.NET](https://github.com/AzureAD/azure-activedirectory-library-for-dotnet/wiki/Client-credential-flows) | [Потоки учетных данных клиента в MSAL.NET](msal-authentication-flows.md#client-credentials)
...
Тем не менее, конечная точка v2.0 (используется MSAL) испускает версию маркера, который принимает веб-API. Свойство манифеста приложения web API позволяет разработчикам выбирать, какая версия токена принимается. См. описание `accessTokenAcceptedVersion` в справочной документации [о манифесте приложения](reference-app-manifest.md).
...
Если вы хотите приобрести токены для приложения, принимающего токены v1.0 (например, Microsoft Graph API, который необходимо https://graph.microsoft.com)создать, `scopes` сопереев нужный идентификатор ресурса с желаемым разрешением OAuth2 для этого ресурса.

Например, для доступа во имя пользователя v1.0 веб-aPI, `ResourceId`который App ID URI , вы хотите использовать:

```csharp
var scopes = new [] {  ResourceId+"/user_impersonation"};
```

Если вы хотите читать и писать с помощью MSAL.NET Activehttps://graph.microsoft.com/)Directory Azure с помощью API Microsoft Graph (, вы создадите список областей, как в следующем фрагменте:
...
#### <a name="warning-should-you-have-one-or-two-slashes-in-the-scope-corresponding-to-a-v10-web-api"></a>Предупреждение: Если у вас есть один или два слэши в области, соответствующей v1.0 веб-API
...
- Для конечной точки MSAL (версии 2.0), которая запрашивает маркер доступа для ресурса, принимающего маркеры версии 1.0 (как в примере выше), AAD выполняет синтаксический анализ параметра audience из запрошенной области, используя все символы до последней косой черты в качестве идентификатора ресурса. Поэтому если https:\//database.windows.net ожидает для audience значение "https://database.windows.net/", следует запросить область https:\//database.windows.net//.default. См. также вопрос[No 747](https://github.com/AzureAD/microsoft-authentication-library-for-dotnet/issues/747): Ресурс URL в задней черта опущена, что вызвало квл-хл auth сбой #747
...
Если используется клиентский поток учетных данных, следует передать область `/.default`. Эта область говорит AD Azure: «Все разрешения на уровне приложения, на которые поставщик дал согласие при регистрации приложения.
...
MSAL.NET не предоставляет маркеры обновления по соображениям безопасности: MSAL обрабатывает освежающие токены для вас.

К счастью, у MSAL.NET теперь есть API, который позволяет перенести предыдущие `IConfidentialClientApplication`токены обновления (приобретенные с ADAL) в:
