---
title: Единый вход между приложениями ADAL и MSAL в iOS и macOS — платформа идентификации Майкрософт
description: ''
...
ms.locfileid: "71268925"
---
# <a name="how-to-sso-between-adal-and-msal-apps-on-macos-and-ios"></a>Практическое руководство. Единый вход между приложениями ADAL и MSAL в macOS и iOS
...
При получении `MSALAccount` объекта в MSAL результат он содержит идентификатор учетной записи `identifier` в свойстве. Приложение должно использовать этот идентификатор для последующих запросов в автоматическом режиме.

Кроме `identifier`того, `MSALAccount` объект содержит отображаемый Идентификатор с именем `username`. Это преобразуется `userId` в ADAL. `username`не считается уникальным идентификатором и может изменяться в любое время, поэтому его следует использовать только для сценариев обратной совместимости с ADAL. MSAL поддерживает запросы кэша с помощью `username` или `identifier` `identifier` , где рекомендуется использовать запрос.
...
| Идентификатор учетной записи не известен               | Запрос всех учетных `allAccounts:` записей через API в`MSALPublicClientApplication` | Н/Д             | Н/Д                            |
...
`homeAccountId`в ADAL является аналогом `identifier` в MSAL. Этот идентификатор можно сохранить для использования в MSAL для поиска учетных записей с `accountForIdentifier:error:` помощью API.

#### <a name="adals-userid"></a>ADAL`userId`

Если `homeAccountId` параметр недоступен или имеется только отображаемый Идентификатор, можно использовать `userId` ADAL для поиска учетной записи в MSAL.

В MSAL сначала найдите учетную запись с помощью `username` или `identifier`. Всегда используйте `identifier` для запросов, если он имеется и используется `username` только в качестве резервного. Если учетная запись найдена, используйте учетную запись в вызовах acquireTokenSilent.
...
Так `homeAccountId` как она недоступна в более старых версиях ADAL, вам потребуется выполнить поиск учетных записей `username`с помощью:
