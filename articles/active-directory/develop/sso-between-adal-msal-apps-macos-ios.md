...
Когда вы получаете объект `MSALAccount` в результате MSAL, он содержит идентификатор учетной записи в свойстве `identifier`. Приложение должно использовать этот идентификатор для последующих запросов в автоматическом режиме.

Кроме `identifier`, `MSALAccount` объект содержит отображаемый Идентификатор, который называется `username`. Это преобразуется в `userId` в ADAL. `username` не считается уникальным идентификатором и может изменяться в любое время, поэтому его следует использовать только в сценариях обратной совместимости с ADAL. MSAL поддерживает запросы кэша с помощью `username` или `identifier`, где рекомендуется использовать запрос `identifier`.
...
| Идентификатор учетной записи не известен               | Запрос всех учетных записей через API `allAccounts:` в `MSALPublicClientApplication` | Н/Д             | Н/Д                            |
...
Если у вас есть приложение MSAL и ADAL и пользователь впервые подписывается в приложение на основе MSAL, вы можете получить единый вход в приложении ADAL, сохранив `username` из объекта `MSALAccount` и передав его в приложение на основе ADAL как `userId`. ADAL может затем найти сведения об учетной записи автоматически с помощью API `acquireTokenSilentWithResource:clientId:redirectUri:userId:completionBlock:`.
...
#### <a name="adals-userid"></a>@No__t ADAL-0

Если `homeAccountId` недоступен или имеется только отображаемый Идентификатор, можно использовать ADAL `userId` для поиска учетной записи в MSAL.

В MSAL сначала найдите учетную запись, `username` или `identifier`. Всегда используйте `identifier` для запроса, если он есть, и использовать `username` в качестве резервного. Если учетная запись найдена, используйте учетную запись в вызовах `acquireTokenSilent`.
