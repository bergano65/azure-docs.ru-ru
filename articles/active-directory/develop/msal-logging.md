---
title: Ведение журнала в приложениях библиотеки проверки подлинности Майкрософт (MSAL) | Службы
...
- `tag` — это строка, передаваемая обратному вызову в библиотеке. Он связан с записью журнала и может использоваться для сортировки сообщений журнала.
...
- `containsPII` указывает, записываются ли в журнал сообщения, содержащие персональные данные или данные Организации. По умолчанию задано значение false, чтобы приложение не заносить персональные данные в журнал. Если `containsPII` равно `true`, этот метод получит сообщения дважды: один раз с параметром `containsPII`, равным `false`, и `message` без персональных данных, а второй раз с параметром `containsPii`, имеющим значение `true`, и сообщение может содержать персональные данные. В некоторых случаях (если сообщение не содержит персональные данные) сообщения будут одинаковыми.
