...
- `containsPII`Указывает, заносятся ли в журнал сообщения, содержащие персональные данные или данные Организации. По умолчанию задано значение false, чтобы приложение не заносить персональные данные в журнал. Если `containsPII` имеет `true`значение, этот метод получит сообщения дважды: один раз с `containsPII` параметром, `false` для которого `message` задано значение, и без персональных данных, а `containsPii` второй раз с `true` параметром, для которого задано значение, а сообщение может содержать персональные данные. В некоторых случаях (если сообщение не содержит персональные данные) сообщения будут одинаковыми.
...
## <a name="javascript"></a>[JavaScript](#tab/javascript)
 ...
 Включите ведение журнала в MSAL. js (JavaScript), передав объект средства ведения журнала во время настройки для `UserAgentApplication` создания экземпляра. Этот объект средства ведения журнала имеет следующие свойства.
