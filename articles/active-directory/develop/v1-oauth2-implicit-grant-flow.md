...
Неявное предоставление предоставляет больше рисков, чем другие гранты, и области, с которыми необходимо обратить внимание, хорошо документированы (например, [неправильное использование маркера доступа для олицетворения владельца ресурса при неявном потоке][OAuth2-Spec-Implicit-Misuse] и [модели угроз OAuth 2,0 и рекомендации по безопасности][OAuth2-Threat-Model-And-Security-Implications]). Но высокий риск связан преимущественно с тем, что предоставление позволяет использовать приложения с активным кодом, который отправляется из удаленного ресурса в браузер. Если у вас нет серверных компонентов приложения, если вы намерены использовать архитектуру безопасной проверки пароля или вызывать веб-API через JavaScript, мы рекомендуем использовать неявный поток получения маркеров.
...
* Полный список ресурсов для разработчиков, включая справочные сведения по протоколам и потокам предоставления авторизации OAuth2, см. в [Руководстве разработчика Azure AD][AAD-Developers-Guide] .
* Дополнительные сведения о процессе интеграции приложений см. в статье [интеграция приложения с Azure AD][ACOM-How-To-Integrate] .
