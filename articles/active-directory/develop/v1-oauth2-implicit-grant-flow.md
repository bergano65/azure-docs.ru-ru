...
Неявное предоставление предоставляет больше рисков, чем другие гранты, и области, с которыми необходимо обратить внимание, хорошо документированы (например, неправильное [Использование маркера доступа для олицетворения владельца ресурса при][OAuth2-Spec-Implicit-Misuse] неявном потоке и [модели угроз OAuth 2,0 и безопасности Рекомендации][OAuth2-Threat-Model-And-Security-Implications]). Но высокий риск связан преимущественно с тем, что предоставление позволяет использовать приложения с активным кодом, который отправляется из удаленного ресурса в браузер. Если у вас нет серверных компонентов приложения, если вы намерены использовать архитектуру безопасной проверки пароля или вызывать веб-API через JavaScript, мы рекомендуем использовать неявный поток получения маркеров.
