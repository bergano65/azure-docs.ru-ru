...
1. Не регистрировать несколько URI перенаправления, если только порт отличается. Сервер входа будет выбирать один произвольным образом и использовать поведение, связанное с этим URI перенаправления (например, является ли он `web` -, `native` -или `spa` -типом перенаправление).
1. Если необходимо зарегистрировать несколько URI перенаправления на localhost для тестирования различных потоков во время разработки, следует отличать их с помощью компонента *пути* URI. Например, `http://127.0.0.1/MyWebApp` не соответствует `http://127.0.0.1/MyNativeApp` .
