...
* Если необходимо зарегистрировать несколько URI перенаправления на localhost для тестирования различных потоков во время разработки, следует отличать их с помощью компонента *пути* URI. Например, `http://localhost/MyWebApp` не соответствует `http://localhost/MyNativeApp` .
