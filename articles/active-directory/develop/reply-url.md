...
* Чтобы предотвратить невозможность нарушения работы приложения с помощью неправильно настроенных брандмауэров или переименованных сетевых интерфейсов, используйте IP-адрес замыкания на себя `127.0.0.1` в URI перенаправления вместо `localhost` .

    Чтобы использовать `http` схему с адресом обратной связи IP-литерала `127.0.0.1` , в настоящее время необходимо изменить атрибут [реплюрлсвистипе](reference-app-manifest.md#replyurlswithtype-attribute) в [манифесте приложения](reference-app-manifest.md).
