...
Для ^ требуетсяhttp://localhost"" URI перенаправления
...
- **Внедренный WebView.** Чтобы использовать только внедренные WebView в MSAL.NET, `AcquireTokenInteractively` построитель параметров содержит `WithUseEmbeddedWebView()` метод.
