...
Как правило, рекомендуется использовать платформу по умолчанию, и обычно это системный браузер. Системный браузер лучше запоминает пользователей, выполнивших вход ранее. Если необходимо изменить это поведение, используйте метод `WithUseEmbeddedWebView(bool)`
...
MSAL.NET пока еще не поддерживает брокеры Android и iOS. Поэтому если необходимо обеспечить единый вход (SSO), системный браузер, скорее всего, все равно будет лучшим вариантом. Обеспечение поддержки брокеров при использовании встроенного веб-браузера находится в списке запланированных функций для будущих версий MSAL.NET.
