...
|`scope`        | Обязательный      | Определяет набор разрешений, запрашиваемых приложением. Это может быть либо статическая (с помощью/.дефаулт), либо Динамическая область.  Это могут быть области OIDC (`openid`, `profile`, `email`). |
