...
1. Войдите в свою учетную запись Azure с помощью [портал Azure](https://portal.azure.com).
...
В вашей подписке Azure учетная запись должна `Microsoft.Authorization/*/Write` иметь доступ, чтобы назначить роль для приложения AD. Это разрешение предоставляется ролью [владельца](../../role-based-access-control/built-in-roles.md#owner) или [администратора доступа пользователей](../../role-based-access-control/built-in-roles.md#user-access-administrator). Если вашей учетной записи назначена роль **участника** , у вас нет соответствующего разрешения. При попытке назначить роль субъекта-службы возникает ошибка.
