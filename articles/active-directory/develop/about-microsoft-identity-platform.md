...
С единой платформой удостоверений Майкрософт (версии 2.0) вам будет достаточно один раз написать код, который позволит проверять подлинность любых удостоверений Майкрософт в вашем приложении. Для нескольких платформ рекомендуется на конечных точках платформы удостоверений использовать полностью поддерживаемую библиотеку проверки подлинности Майкрософт (MSAL) с открытым кодом. Она разработана на базе жизненного цикла безопасной разработки (SDL) Майкрософт, проста в применении, предоставляет вашим пользователям удобные возможности единого входа (SSO) и обеспечивает высокий уровень надежности и производительности. При вызове API-интерфейсов вы можете настроить в своем приложении добавочное согласие, которое будет запрашиваться для более уязвимых областей, возникающих во время использования приложения в среде выполнения. Библиотека MSAL также поддерживает Azure Active Directory B2C, поэтому ваши клиенты смогут использовать удобные им удостоверения из социальных сетей, корпоративных или локальных учетных записей для единого входа в ваши приложения и API.
...
Интерфейс **API приложения в Microsoft Graph** сейчас доступен в предварительной версии. Он позволяет программно настраивать проверку подлинности любого удостоверения Майкрософт в приложениях, интегрированных с платформой удостоверений Майкрософт. Однако пока этот API не станет общедоступным, необходимо использовать API Azure AD Graph 1.6 и манифест приложения.
