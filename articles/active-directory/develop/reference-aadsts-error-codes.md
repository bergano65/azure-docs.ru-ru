...
| AADSTS50005 | DevicePolicyError - пользователь пытался войти систему на устройство из платформу, которая в настоящее время не поддерживается с помощью политики условного доступа. |
...
| AADSTS50011 | InvalidReplyTo — обратный адрес отсутствует, неправильно настроен или не соответствует обратным адресам, настроенным для приложения.  Решения проблемы убедитесь, чтобы добавить этот отсутствует адрес ответа приложения Azure Active Directory или кого-нибудь с разрешениями для управления приложением в Active Directory это для вас.|
...
| AADSTS50131 | ConditionalAccessFailed - указывает различных ошибках условного доступа, например неверный состояние устройства Windows, запрос заблокирован из-за подозрительной активности, политики доступа или принятия решений политики безопасности. |
...
| AADSTS53000 | DeviceNotCompliant - политики условного доступа требуется соответствующее устройство, и устройство не соответствует требованиям. Пользователю нужно зарегистрировать свое устройство у утвержденного поставщика MDM, например Intune. |
| AADSTS53001 | DeviceNotDomainJoined - политики условного доступа требуется присоединенном к домену устройстве, и устройство не присоединено к домену. Потребуйте, чтобы пользователь использовал присоединенное к домену устройство. |
| AADSTS53002 | ApplicationUsedIsNotAnApprovedApp - приложения, не утвержденного приложения для условного доступа. Чтобы получить доступ, нужно использовать приложение, входящее в список утвержденных. |
| AADSTS53003 | BlockedByConditionalAccess - доступа была заблокирована политиками условного доступа. Политика доступа не разрешает выдачу маркеров. |
