...
[Спецификация OAuth2.0](https://tools.ietf.org/html/rfc6749#section-5.2) содержит рекомендации о том, как `error` обрабатывать ошибки при проверке подлинности с помощью части ответа на ошибку. 

Вот пример ответа на ошибку:
...
| Параметр         | Описание    |
|-------------------|----------------|
| `error`       | Строка кода ошибки, которая может быть использована для классификации типов ошибок, которые происходят, и должна использоваться для реагирования на ошибки. |
| `error_description` | Конкретное сообщение об ошибке, с помощью которого разработчик может определить причину возникновения ошибки проверки подлинности. Никогда не используйте это поле, чтобы отреагировать на ошибку в коде. |
...
| `error_uri` |  Ссылка на страницу поиска ошибок с дополнительной информацией об ошибке.  Это только для использования разработчиком, не преподнести его пользователям.  Только присутствует, когда система поиска ошибок имеет дополнительную информацию об ошибке - не все ошибки имеют дополнительную информацию.|

Поле `error` имеет несколько возможных значений - просмотреть ссылки на протокольную документацию и спецификации OAuth 2.0, чтобы узнать больше о конкретных ошибках (например, `authorization_pending` в [потоке кода устройства)](v2-oauth2-device-code.md)и о том, как на них реагировать.  Некоторые общие из них перечислены здесь:
...
| `invalid_grant`    | Некоторые материалы проверки подлинности (код, токен обновления, токен доступа, задача PKCE) были недействительными, неотделимы, отсутствующими или иным образом непригодными для использования | Попробуйте новый `/authorize` запрос в конечную точку, чтобы получить новый код авторизации.  Рассмотрите и проверите использование протоколов этого приложения. |
...
| `invalid_resource` | Целевой ресурс недействителен, поскольку он не существует, Azure AD не может найти его, или он неправильно настроен. | Это означает, что ресурс, если он существует, не настроен в клиенте. Приложение может отобразить пользователю запрос с инструкцией по установке приложения и его добавлению в Azure AD.  Во время разработки это обычно указывает на неправильно настроенного тест-арендатора или опечатку в названии запрашиваемого объема. |
| `interaction_required` | Для запроса требуется взаимодействие с пользователем. К примеру, требуется дополнительный шаг проверки подлинности. | Повторяйте запрос с помощью того же ресурса, взаимодействуйте, чтобы пользователь мог выполнить любые необходимые задачи.  |
...
| AADSTS50003 | MissingSigningKey: не удалось войти из-за отсутствия ключа или сертификата для подписи. Возможно, в приложении не настроен ключ подписывания. Проверьте резолюции, [https://docs.microsoft.com/azure/active-directory/application-sign-in-problem-federated-sso-gallery#certificate-or-key-not-configured](https://docs.microsoft.com/azure/active-directory/application-sign-in-problem-federated-sso-gallery#certificate-or-key-not-configured)изложенные на . Если проблемы не удалось решить, обратитесь к владельцу или администратору приложения. |
| AADSTS50005 | DevicePolicyError - Пользователь пытался войти на устройство с платформы, которая в настоящее время не поддерживается через политику условного доступа. |
...
| AADSTS50011 | InvalidReplyTo — обратный адрес отсутствует, неправильно настроен или не соответствует обратным адресам, настроенным для приложения.  Поскольку разрешение гарантирует добавление этого отсутствующих ответных адресов в приложение Azure Active Directory или чтобы кто-то с разрешениями управлять приложением в Active Directory сделал это за вас.|
