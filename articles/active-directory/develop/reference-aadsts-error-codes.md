...
[Спецификация OAuth 2.0](https://tools.ietf.org/html/rfc6749#section-5.2) содержит рекомендации по обработке ошибок во время проверки подлинности с `error` помощью части ответа на ошибку. 
...
`error` Поле имеет несколько возможных значений. Ознакомьтесь со ссылками на документацию по протоколам и спецификациями OAuth 2,0, чтобы узнать больше о `authorization_pending` конкретных ошибках (например, в [потоке кода устройства](v2-oauth2-device-code.md)) и о том, как реагировать на них.  Ниже перечислены некоторые распространенные из них.
...
| `invalid_grant`    | Некоторые материалы для проверки подлинности (код проверки подлинности, маркер обновления, маркер доступа, запрос PKCE) являются недопустимыми, недоступными для синтаксического анализа, отсутствующими или непригодными для использования. | Попробуйте новый запрос к `/authorize` конечной точке, чтобы получить новый код авторизации.  Рассмотрите возможность просмотра и проверки использования протоколов приложением. |
...
| `invalid_resource` | Целевой ресурс недопустим, так как он не существует, Azure AD не может найти его или неправильно настроен. | Это означает, что ресурс, если он существует, не настроен в клиенте. Приложение может отобразить пользователю запрос с инструкцией по установке приложения и его добавлению в Azure AD.  Во время разработки это обычно свидетельствует о неправильной установке тестового клиента или опечатке в имени запрашиваемой области. |
| `interaction_required` | Для запроса требуется взаимодействие с пользователем. К примеру, требуется дополнительный шаг проверки подлинности. | Повторите запрос с тем же ресурсом интерактиевли, чтобы пользователь мог выполнить все необходимые проблемы.  |
...
Коды ошибок и сообщения могут изменяться.  Для получения самых актуальных сведений ознакомьтесь со `https://login.microsoftonline.com/error` страницей, чтобы найти описания ошибок, исправления и некоторые возможные обходные пути аадстс.  
...
| AADSTS50003 | MissingSigningKey: не удалось войти из-за отсутствия ключа или сертификата для подписи. Возможно, в приложении не настроен ключ подписывания. Ознакомьтесь с разрешениями, изложенными [https://docs.microsoft.com/azure/active-directory/application-sign-in-problem-federated-sso-gallery#certificate-or-key-not-configured](https://docs.microsoft.com/azure/active-directory/application-sign-in-problem-federated-sso-gallery#certificate-or-key-not-configured)в. Если проблемы не удалось решить, обратитесь к владельцу или администратору приложения. |
...
| AADSTS70012 | MsaServerError — при проверке подлинности пользователя (объекта-получателя) MSA произошла ошибка сервера. Повторите попытку. Если проблема не будет устранена, отправьте запрос [в службу поддержки](../fundamentals/active-directory-troubleshooting-support-howto.md) . |
...
| AADSTS530032 | Блоккедбикондитионалакцессонсекуритиполици — администратор клиента настроил политику безопасности, которая блокирует этот запрос. Проверьте политики безопасности, определенные на уровне клиента, чтобы определить, соответствует ли запрос требованиям политики. |
...
| AADSTS7000222| Инвалидклиентсекретекспиредкэйспровидед — срок действия предоставленных секретных ключей клиента истек. Посетите портал Azure, чтобы создать новые ключи для приложения, или используйте учетные данные сертификата для повышения безопасности:https://aka.ms/certCreds |
