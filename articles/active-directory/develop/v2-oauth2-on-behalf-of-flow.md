---
title: Платформа идентификации Майкрософт и OAuth2.0 On-Behalf-Of потока Azure
...
ms.locfileid: "81309377"
---
# <a name="microsoft-identity-platform-and-oauth-20-on-behalf-of-flow"></a>Платформа идентификации Майкрософт и OAuth 2.0 On-Behalf-Of потока
...
> [!NOTE]
>
> - Конечная точка платформы майкрософт не поддерживает все сценарии и функции. Чтобы определить, следует ли использовать конечную точку платформы идентификации Майкрософт, прочитайте об [ограничениях платформы идентификации Майкрософт.](active-directory-v2-limitations.md) 
...
Предположим, что пользователь был проверен на приложении с помощью [потока выдачи кода авторизации OAuth 2.0](v2-oauth2-auth-code-flow.md) или другого потока входа. На этом этапе приложение содержит маркер доступа для *API А* (маркер A) с утверждениями пользователя и разрешением на доступ к веб-API среднего уровня (API A). Теперь API A должен выполнить запрос к веб-API нижнего уровня (API B) с проверкой подлинности.

Последующие шаги образуют поток OBO. Эти шаги поясняются на следующей схеме.

![Показывает поток OAuth2.0 On-Behalf-Of](./media/v2-oauth2-on-behalf-of-flow/protocols-oauth-on-behalf-of-flow.png)

1. Клиентское приложение отправляет запрос к API A с маркером A (с утверждением `aud` API А).
1. API A authenticatata к точке выдачи токенов платформы Майкрософт и запрашивает токен для доступа к API B.
1. Конечная точка выпуска токенов платформы идентификационных данных Майкрософт проверяет учетные данные API A вместе с токеном А и выдает токен доступа для API B (токен B) aPI A.
1. Токен B устанавливается API A в заголовке авторизации запроса в API B.
...
Чтобы запросить токен доступа, сделайте HTTP POST на конечную точку маркера платформы Microsoft с следующими параметрами.
...
| `client_id` | Обязательно | Id-идентификатор приложения (клиента), который страница [регистраций Azure - App](https://go.microsoft.com/fwlink/?linkid=2083908) присвоила вашему приложению. |
| `client_secret` | Обязательно | Секрет клиента, который вы создали для вашего приложения на портале Azure - страница регистрации приложений. |
| `assertion` | Обязательно | Значение токена, используемого в запросе.  Этот маркер должен иметь аудиторию приложения, делающие этот запрос OBO (приложение, обозначаемое полем). `client-id` |
| `scope` | Обязательно | Список областей для запроса токена, разделенный пробелами. Для получения дополнительной [информации](v2-permissions-and-consent.md)см. |
...
| `requested_token_use` | Обязательно | Указывает, как должен быть обработан запрос. В потоке OBO нужно указать значение `on_behalf_of`. |
| `scope` | Обязательно | Список областей для запроса маркера, разделенный пробелами. Для получения дополнительной [информации](v2-permissions-and-consent.md)см.|
...
| Параметр | Описание |
| --- | --- |
| `token_type` | Указывает значение типа маркера. Единственный тип, который поддерживает `Bearer`идентификационная платформа Майкрософт, — это. Для получения дополнительной информации о токенах предъявителя см. [OAuth 2.0 Authorization Framework: Bearer Token Usage (RFC 6750)](https://www.rfc-editor.org/rfc/rfc6750.txt) |
...
> [!NOTE]
> Маркер доступа выше имеет формат версии 1.0. Это происходит потому, что токен предоставляется на основе доступа **к ресурсу.** Microsoft Graph настраивается на прием токенов v1.0, поэтому идентификационная платформа Майкрософт производит токены доступа v1.0, когда клиент запрашивает токены для Microsoft Graph. Только приложениям следует просматривать маркеры доступа. Клиенты **не должны** их проверять.

### <a name="error-response-example"></a>Пример ответа с сообщением об ошибке

Ответ на ошибку возвращается конечным топором при попытке приобрести токен доступа для API ниже по течению, если на амебистерте ниже по течению установлена политика условного доступа (например, многофакторная аутентификация). Служба среднего уровня должна обнаружить эту ошибку в клиентском приложении, чтобы клиентское приложение может обеспечить взаимодействие с пользователем для удовлетворения политики условного доступа.
...
В зависимости от архитектуры или использования приложения можно рассмотреть различные стратегии для обеспечения успешного потока OBO. Во всех случаях конечная цель состоит в том, чтобы обеспечить надлежащее согласие, чтобы клиентское приложение мог вызвать приложение среднего уровня, а приложение среднего уровня имеет разрешение на вызов резервного ресурса. 

> [!NOTE]
> Ранее система учетных записей Майкрософт (личные счета) не поддерживала поле "Известные приложения клиента" и не могла показать комбинированного согласия.  Это было добавлено, и все приложения в платформе идентификации Майкрософт могут использовать известный подход клиентского приложения для получения согласия на вызовы OBO. 

### <a name="default-and-combined-consent"></a>Область /.default и объединенное согласие

Приложение среднего уровня добавляет клиент в список известных клиентских приложений в своем манифесте, после чего клиент может активировать поток объединенного согласия как для себя, так и для приложения среднего уровня. На конечную точку платформы идентификации Майкрософт это делается с использованием [ `/.default` области.](v2-permissions-and-consent.md#the-default-scope) При запуске экрана согласия с `/.default`использованием известных клиентских приложений и экрана согласия будут отображаться разрешения **как** для клиента на API среднего уровня, так и запрашивать любые разрешения, требуемые API среднего уровня. Пользователь дает согласие для обоих приложений, после чего выполняется поток OBO.

### <a name="pre-authorized-applications"></a>Предварительно авторизованные приложения

Ресурсы могут указывать, что данное приложение всегда имеет разрешение на получение определенных областей. Это особенно удобно для упрощения подключений между интерфейсным клиентом и внутренним ресурсом. Ресурс может объявить несколько предварительно авторизованных приложений. Любое из них может запросить соответствующие разрешения в потоке OBO и получить их без согласия пользователя.
...
Если клиент использует неявный поток, чтобы получить id_token, и что клиент также имеет подстановочные знаки в URL-адресе ответа, id_token не может быть использован для потока OBO.  Тем не менее, конфиденциальный клиент может активировать маркеры доступа, полученные с помощью потока неявного предоставления, даже если для инициирующего клиента зарегистрирован URL-адрес ответа с подстановочными знаками.
...
* [OAuth 2.0 грант на учетные данные клиентов в платформе идентификации Майкрософт](v2-oauth2-client-creds-grant-flow.md)
* [Поток кода OAuth 2.0 в платформе идентификации Майкрософт](v2-oauth2-auth-code-flow.md)
