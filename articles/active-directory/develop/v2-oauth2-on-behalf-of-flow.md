...
| `client_id` | Обязательное значение | Идентификатор приложения (клиента), назначенный вашему приложению на странице [регистрации приложений на портале Azure](https://go.microsoft.com/fwlink/?linkid=2083908). |
| `client_secret` | Обязательное значение | Секрет клиента, созданный для приложения на портале Azure — страница регистрации приложений. |
| `assertion` | Обязательное значение | Значение токена, используемого в запросе.  Этот маркер должен иметь аудиторию приложения, так что это становится запросом OBO (приложение, обозначенное полем `client-id`). |
| `scope` | Обязательное значение | Список областей для запроса токена, разделенный пробелами. Дополнительные сведения см. в разделе [Области](v2-permissions-and-consent.md). |
| `requested_token_use` | Обязательное значение | Указывает, как должен быть обработан запрос. В потоке OBO нужно указать значение `on_behalf_of`. |
