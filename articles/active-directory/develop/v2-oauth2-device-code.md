...
| `scope` | Рекомендуемая | Разделенный пробелами список [областей](v2-permissions-and-consent.md) , на которые пользователь должен согласиться.  |
...
> [!NOTE]
> Поле `verification_uri_complete` ответа в настоящее время не включено или не поддерживается.  Мы упомянули это, поскольку если вы прочитаете [стандартный стандарт](https://tools.ietf.org/html/rfc8628) , который `verification_uri_complete` указан как необязательная часть стандарта потока кода устройства.
...
| `grant_type` | Обязательный | Должен содержать значение `urn:ietf:params:oauth:grant-type:device_code`.|
| `client_id`  | Обязательный | Должен соответствовать `client_id`, используемому в первоначальном запросе. |
| `device_code`| Обязательный | `device_code`, возвращаемый в запросе авторизации устройства.  |
