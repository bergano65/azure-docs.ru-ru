...
>
> В качестве дополнительного примечания `verification_uri_complete` поле ответа в настоящее время не включено или не поддерживается.  Мы упомянули это, поскольку если вы прочитаете Стандартный `verification_uri_complete` стандарт, который указан как необязательная часть стандарта потока кода устройства.
...
| --- | --- | --- |
| `tenant` | Обязательное значение |Клиент каталога, из которого необходимо запросить разрешение. Его можно указать в виде GUID или понятного имени.  |
...
| ---              | --- | --- |
|`device_code`     | Строковое | Длинная строка, используемая для проверки сеанса между клиентом и сервером авторизации. Клиент использует этот параметр для запроса маркера доступа с сервера авторизации. |
|`user_code`       | Строковое | Короткая строка, отображаемая для пользователя, который используется для задания сеанса на вторичном устройстве.|
...
| `message`        | Строковое | Понятная пользователю строка с инструкциями. Ее можно локализовать, включив **параметр запроса** в запросе формы `?mkt=xx-XX`, заменив соответствующий код языка и региональных параметров. |
...
| -------- | -------- | ---------- |
| `grant_type` | Обязательное значение | Должен содержать значение `urn:ietf:params:oauth:grant-type:device_code`.|
| `client_id`  | Обязательное значение | Должен соответствовать `client_id`, используемому в первоначальном запросе. |
| `device_code`| Обязательное значение | `device_code`, возвращаемый в запросе авторизации устройства.  |
...
| Параметр | Формат | Описание |
| --------- | ------ | ----------- |
| `token_type` | Строковое| Всегда "Bearer". |
