...
1. Пространство имен из предыдущего шага и перейдите к `https://<namespace>.accesscontrol.windows.net`

    Например если один из пространства имен — contoso-test, перейдите к `https://contoso-test.accesscontrol.windows.net`
...
| Service | Руководство |
| ------- | -------- |
| Azure Service Bus | [Перенос в подписи общего доступа](https://docs.microsoft.com/azure/service-bus-messaging/service-bus-migrate-acs-sas) |
| Ретрансляция служебной шины Azure | [Перенос в подписи общего доступа](https://docs.microsoft.com/azure/service-bus-relay/relay-migrate-acs-sas) |
| Управляемый кэш Azure | [Перенос в Azure кэша для Redis](https://docs.microsoft.com/azure/azure-cache-for-redis/cache-faq#which-azure-cache-offering-is-right-for-me) |
| Azure DataMarket | [Перенос в интерфейсах API Cognitive Services](https://docs.microsoft.com/azure/machine-learning/studio/datamarket-deprecation) |
| Службы BizTalk | [Миграция в компонент Logic Apps службы приложений Azure](https://docs.microsoft.com/azure/machine-learning/studio/datamarket-deprecation) |
| Службы мультимедиа Azure | [Миграция с проверкой подлинности Azure AD](https://azure.microsoft.com/blog/azure-media-service-aad-auth-and-acs-deprecation/) |
| Служба архивации Azure | [Обновление агента службы Azure Backup](https://docs.microsoft.com/azure/backup/backup-azure-file-folder-backup-faq) |
...
SharePoint 2013, 2016 г., и клиенты SharePoint Online давно используют ACS для проверки подлинности в облаке, на локальные и гибридные сценарии. Прекращение использования службы контроля доступа повлияет на некоторые функции и варианты использования SharePoint. В таблице ниже приведены краткие инструкции по миграции для некоторых из наиболее популярных компонентов SharePoint, использующих службу контроля доступа.

| Функция | Руководство |
| ------- | -------- |
| Аутентификация пользователей из Azure AD | Ранее в Azure AD не поддерживались маркеры SAML 1.1, необходимые SharePoint для аутентификации, и служба контроля доступа использовалась в качестве посредника, обеспечивающего совместимость SharePoint с форматами маркеров Azure AD. Теперь вы можете [непосредственно подключить SharePoint к Azure AD, используя SharePoint коллекции приложений Azure AD в локальной среде приложение](https://docs.microsoft.com/azure/active-directory/saas-apps/sharepoint-on-premises-tutorial). |
| [Аутентификация приложений и проверки подлинности между серверами в SharePoint на локальном компьютере](https://technet.microsoft.com/library/jj219571(v=office.16).aspx) | Не зависит от прекращения использования службы контроля доступа. Какие-либо изменения не требуются. | 
| [Авторизация с низким уровнем доверия для надстроек SharePoint (размещении у поставщика и размещении в SharePoint)](https://docs.microsoft.com/sharepoint/dev/sp-add-ins/three-authorization-systems-for-sharepoint-add-ins) | Не зависит от прекращения использования службы контроля доступа. Какие-либо изменения не требуются. |
...
| **Протоколы и совместимости пакета SDK** | | |
...
| ![Проверка связи](./media/active-directory-acs-migration/rsz_ping.png) | [Ping Identity](https://www.pingidentity.com) предлагает два решения, аналогичные возможностям ACS. PingOne — это облачная служба удостоверений, которая поддерживает многие из тех же функций, что ACS, и PingFederate похож на локального удостоверения продукта, которая обеспечивает большую гибкость. Дополнительные сведения по их использованию см. в [этом руководстве](https://www.pingidentity.com/en/company/blog/2017/11/20/migrating_from_microsoft_acs_to_ping_identity.html). |
...
- [Пример кода управляющей программы с помощью учетных данных клиента простой пароль](https://github.com/Azure-Samples/active-directory-dotnet-daemon)
- [Пример кода управляющей программы с помощью учетных данных сертификата клиента](https://github.com/Azure-Samples/active-directory-dotnet-daemon-certificate-credential)
