...
1. Введите **URL-адрес выхода** `https://localhost:3000`.
1. В разделе "Неявное предоставление" установите флажок **Маркеры идентификаторов**, так как в этом примере требуется включить [поток "Неявное предоставление"](./v2-oauth2-implicit-grant-flow.md), чтобы предоставить пользователю вход в систему.
