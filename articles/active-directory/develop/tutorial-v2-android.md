...
В этом примере приложение авторизует пользователя и получает данные от его имени.  Доступ к этим данным будет осуществляться через защищенный API (API Microsoft Graph), требующий авторизации.
...
6. В разделе **Хэш подписи** на странице **Настройка приложения Android** щелкните **Generating a development Signature Hash** (Создания хэша подписи для разработки) и скопируйте команду KeyTool для использования с вашей платформой. Обратите внимание, что KeyTool.exe устанавливается как часть пакета средств разработки Java (JDK). Вам также понадобится установить средство OpenSSL, чтобы выполнить команду KeyTool.
