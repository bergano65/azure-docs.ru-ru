...
4. Вы перейдете на главную страницу регистрации приложения, в которой содержится страница **Параметры** приложения. Чтобы добавить учетные данные для веб-приложения, сделайте следующее:
   ...
   1. Чтобы добавить сертификат, сделайте следующее:
   ...
   1. Чтобы добавить пароль, сделайте следующее:
...
- Возможность входа для пользователей. Предполагается, что пользователю будет предоставлена кнопка "Регистрация", которая перенаправит браузер в конечную точку `/authorize` Azure AD OAuth 2.0 или в конечную точку `/userinfo` OpenID Connect. Эти конечные точки позволяют приложению получить сведения о новом пользователе путем проверки id_token. На следующем этапе регистрации пользователя появится запрос на продолжение, как показано в разделе с кратким описанием платформы проверки согласия.
...
- [Краткое руководство по добавлению фирменной символики организации на страницу входа в Azure AD](../fundamentals/customize-branding.md)
