...
Стратегии сериализации кэша токенов различаются в зависимости от того, пишете ли вы публичное клиентское приложение (Desktop) или конфиденциальное клиентское приложение (Web App, Web API или приложение Daemon).
...
Для Web-приложений или Web-aI можно использовать сеанс, кэш Redis или базу данных для хранения кэша маркеров. На одну учетную запись должен быть один кэш маркеров (на учетную запись), чтобы обеспечить сериализацию кэша маркеров на учетную запись.
