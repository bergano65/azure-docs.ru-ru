...
- **Получение маркеров безопасности** для защищенного веб-API. Корпорация Майкрософт рекомендует использовать [библиотеки проверки подлинности](reference-v2-libraries.md#microsoft-supported-client-libraries) для получения маркеров, в частности, для семейства библиотек проверки подлинности Майкрософт (MSAL).
...
Дополнительные сведения см. в статье [Scenario: Web app that signs in users](scenario-web-app-sign-user-overview.md) (Сценарий. Веб-приложение, которое поддерживает вход пользователей).
...
В веб-приложении для **вызова веб-API** от имени пользователя будет использоваться `ConfidentialClientApplication` MSAL. Вы будете использовать поток кода авторизации, сохранив полученный маркер в кэше маркеров. Затем контроллер будет автоматически получать токены из кэша при необходимости. MSAL обновляет маркер, если это необходимо.

Дополнительные сведения см. в статье [Scenario: Web app that calls web APIs](scenario-web-app-call-api-overview.md) (Сценарий. Веб-приложение, которое вызывает веб-API)
...
Чтобы вызвать веб-API из классического приложения, которое выполняет вход пользователей, вы будете использовать интерактивные методы получения маркера PublicClientApplication MSAL. Эти интерактивные методы позволяют управлять пользовательским интерфейсом входа в систему. Чтобы реализовать это взаимодействие, MSAL использует веб-браузер.

![Классические приложения](media/scenarios/desktop-app.svg)

Для размещенных в Windows приложений, работающих на компьютерах, присоединенных к домену Windows или к присоединенных к AAD, есть другая возможность. Они могут получать маркер автоматически, используя [встроенную проверку подлинности Windows](https://aka.ms/msal-net-iwa).
...
Дополнительные сведения см. в статье [Scenario: Desktop app that calls web APIs](scenario-desktop-overview.md) (Сценарий. Классическое приложение, которое вызывает веб-API).
...
Как и в классических приложениях, для получения маркера для вызова веб-API мобильное приложение будет использовать интерактивные методы получения маркера PublicClientApplication MSAL. В iOS и Android MSAL по умолчанию использует системный веб-браузер. Но вы можете настроить использование встроенного веб-представления. В зависимости от мобильной платформы существуют определенные особенности: (UWP, iOS, Android).
...
Дополнительные сведения см. в статье [Scenario: Mobile application that calls web APIs](scenario-mobile-overview.md) (Сценарий. Мобильное приложение, вызывающее веб-API).
...
Дополнительные сведения см. в статье [Scenario: Protected web API](scenario-protected-web-api-overview.md) (Сценарий. Защищенный веб-API).
...
Дополнительные сведения см. в статье [Scenario: Web API that calls web APIs](scenario-web-api-call-api-overview.md) (Сценарий. Веб-API, которые вызывают веб-API).
