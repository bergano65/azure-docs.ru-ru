...
- [С пользователями или без пользователей](#with-users-or-without-users): в некоторых сценариях участвует пользователь, выполнивший вход, но другие, например, сценарии управляющей программы не используют пользователя.
- [Одностраничные, общедоступные клиенты и конфиденциальные клиентские приложения](#single-page-public-client-and-confidential-client-applications): три категории типов приложений. Каждая из них используется с разными библиотеками и объектами.
...
- **Получение маркеров безопасности для защищенного веб-API**. Корпорация Майкрософт рекомендует использовать [библиотеки проверки подлинности](reference-v2-libraries.md#microsoft-supported-client-libraries) для получения маркеров, в частности, в семействе библиотеки проверки подлинности Майкрософт (MSAL).
...
Чтобы защищенный веб-API ASP.NET или ASP.NET Core вызывал другой веб-API от имени пользователя, приложению нужно получить маркер для подчиненного веб-API. Для этого оно вызывает метод **AcquireTokenOnBehalfOf** класса [ConfidentialClientApplication](https://aka.ms/msal-net-on-behalf-of). Такой механизм известен как вызовы между службами. Веб-API, вызывающие другие веб-API, должны реализовать настраиваемую сериализацию кэша.
...
Вы можете создать приложение (управляющую программу), которое получает маркер для вызывающего приложения с помощью методов получения **учетных данных клиента** из класса [ConfidentialClientApplication](https://aka.ms/msal-net-client-credentials) библиотеки MSAL. Для этих методов нужно, чтобы вызывающее приложение зарегистрировало секрет в Azure AD. Затем приложение передает этот секрет управляющей программе, которую оно вызывает. В качестве секретов могут использоваться пароли приложений, утверждение сертификата, утверждение клиента и т. д.
