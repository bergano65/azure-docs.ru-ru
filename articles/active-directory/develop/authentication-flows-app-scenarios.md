...
В некоторых сценариях, например при условном доступе, связанном с ИДЕНТИФИКАТОРом устройства или регистрацией устройства, необходимо установить брокер на устройстве. К таким брокерам относятся корпоративный портал Майкрософт (на Android) и Microsoft Authenticator (Android и iOS). MSAL теперь может взаимодействовать с брокерами. Дополнительные сведения см. в статье Использование [брокеров в Android и iOS](https://github.com/AzureAD/azure-activedirectory-library-for-dotnet/wiki/leveraging-brokers-on-Android-and-iOS).
...
Чтобы защищенный веб-API ASP.NET или ASP.NET Core вызывал другой веб-API от имени пользователя, приложению нужно получить маркер для подчиненного веб-API. Чтобы получить маркер, приложение вызывает метод [аккуиретокенонбехалфоф](https://aka.ms/msal-net-on-behalf-of) класса **конфидентиалклиентаппликатион** . Такие вызовы также называются вызовами *между службами* . Веб-API, вызывающие другие веб-API, должны реализовать настраиваемую сериализацию кэша.
