...
- `acquireTokenSilent`не требуется параметр учетной записи.  Если вы предоставляете учетную запись, а указанная учетная запись не соответствует текущей учетной записи, отслеживающей `MsalClientException` MSAL, создается исключение.
...
Используйте одну или несколько учетных записей в приложении, `acquireToken` вызвав один или несколько раз.  
...
Удалите учетную запись, `removeAccount` вызвав ее с помощью идентификатора учетной записи.
