...
MSAL специализирует поверхность API `PublicClientApplication` для упрощения и уточнения процесса разработки для приложений, которые позволяют одновременно использовать только одну учетную запись. `PublicClientApplication` является подклассом `SingleAccountPublicClientApplication` и `MultipleAccountPublicClientApplication`.  На следующей схеме показана связь между этими классами.
...
Класс `SingleAccountPublicClientApplication` позволяет создать приложение на основе MSAL, которое разрешает вход только в одну учетную запись за раз. `SingleAccountPublicClientApplication` имеет следующие отличия от `PublicClientApplication` :
...
- `acquireTokenSilent` не требует параметра учетной записи.  Если вы предоставляете учетную запись, а указанная учетная запись не соответствует текущей учетной записи, отслеживающей MSAL, создается `MsalClientException`.
...
- `getCurrentAccount` возвращает результирующий объект, который предоставляет следующие сведения:
  ...
  - Куррентаккаунт.
- `signOut` удаляет из устройства все маркеры, связанные с клиентом.  

Если на устройстве установлен брокер проверки подлинности Android, например Microsoft Authenticator или Корпоративный портал Intune, а приложение настроено для использования брокера, то `signOut` не удалит учетную запись с устройства.
...
Если приложение настроено для использования брокера и на устройстве установлен брокер, учетная запись не будет удалена из брокера при вызове `removeAccount`.  Удаляются только маркеры, связанные с клиентом.
