...
- IWA не обходит MFA (многофакторная проверка подлинности). Если настроено MFA, IWA может завершиться ошибкой, если требуется запрос MFA, так как для MFA требуется вмешательство пользователя.
  > [!NOTE]
  > Это очень просто. IWA не является интерактивным, но 2FA требует взаимодействия с пользователем. Вы не контролируете, когда поставщик удостоверений запрашивает выполнение 2FA, а администратор клиента выполняет. Из наших наблюдений 2FA требуется при входе из другой страны, если он не подключен через VPN к корпоративной сети, а иногда даже при подключении через VPN. Не следует рассчитывать на детерминированный набор правил, Azure Active Directory использует AI для непрерывного изучения того, является ли 2FA обязательным. Если IWA завершается неудачей, следует выполнить откат к приглашению пользователя (Интерактивная проверка подлинности или поток кода устройства).
...
Дополнительные сведения о согласии см. в разделе [разрешения и согласие версии 2.0](https://docs.microsoft.com/azure/active-directory/develop/v2-permissions-and-consent) .
...
> [!IMPORTANT]
> MSAL.NET создает кэши маркеров для вас и обеспечивает `IToken` кэш при вызове свойств `UserTokenCache` приложения и `AppTokenCache` . Вы не должны реализовывать интерфейс самостоятельно. При реализации пользовательской сериализации для кэша маркеров в вашу зону ответственности входит следующее:
>
