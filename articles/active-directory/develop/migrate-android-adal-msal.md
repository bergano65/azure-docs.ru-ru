...
- Новая модель для доступа к маркерам:
  - ADAL предоставляет доступ к маркерам через `AuthenticationContext`объект, который представляет сервер. MSAL предоставляет доступ к маркерам через `PublicClientApplication`объект, который представляет клиент. Разработчикам клиентов не нужно создавать новый `PublicClientApplication` экземпляр для каждого центра, с которым им нужно взаимодействовать. Требуется только `PublicClientApplication` одна конфигурация.
...
- Изменения API-интерфейса маркеров:
  - В ADAL `AcquireToken()` сначала выполняет автоматический запрос. В таком случае он выполняет интерактивный запрос. Это привело к тому, что некоторые разработчики полагается `AcquireToken`только на, что привело к непредвиденному получению учетных данных у пользователя. MSAL требует, чтобы разработчики предпринимали намерение о том, когда пользователь получит запрос пользовательского интерфейса.
...
- MSAL поддерживает объекты параметров для `AcquireToken` вызовов `AcquireTokenSilent` и.
...
> [!CAUTION]
> Невозможно задать обе области и идентификатор ресурса. Попытка задать оба значения приведет к `IllegalArgumentException`вызову.
...
В ADAL существует один тип исключения `AuthenticationException`, который включает метод для получения значения `ADALError` перечисления.
