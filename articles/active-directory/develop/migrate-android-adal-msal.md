...
ADAL работает с конечной точкой Azure Active Directory v 1.0. Библиотека проверки подлинности Microsoft (MSAL) работает с платформой идентификации Майкрософт, которая ранее называлась конечной точкой Azure Active Directory v 2.0.

Платформа Microsoft Identity отличается от Azure Active Directory v 1.0:
...
Общедоступный API MSAL содержит важные изменения для удобства использования, включая:
...
- Новая модель для доступа к маркерам:
  ...
  - Поддержка добавочного согласия. Разработчики могут запрашивать области, включая те, которые не были добавлены во время регистрации приложения.
  - Проверка центра — > известные центры
...
- Изменения API-интерфейса маркеров:
  - В ADAL `AcquireToken` сначала пытается выполнить неудачный запрос, а затем выполняет интерактивный запрос. Это поведение привело к тому, что некоторые разработчики полагается только на `AcquireToken`, что иногда означает, что взаимодействие с пользователем будет происходить в непредвиденный момент. MSAL требует, чтобы разработчики предпринимали намерение о том, когда пользователь получит запрос пользовательского интерфейса.
    - `AcquireTokenSilent` всегда приводит к автоматическому запросу, который либо завершается успешно, либо завершается сбоем.
    - `AcquireToken` всегда приводит к интерактивному запросу (пользовательский интерфейс с запросом пользовательского интерфейса).
- MSAL поддерживает взаимодействие пользовательского интерфейса входа из браузера по умолчанию или встроенного веб-представления.
...
- Новая модель исключений:
  - Исключения ясно определяют тип возникшего исключения и действия, которые разработчик должен выполнить для его устранения.
...
Если вы используете ADAL и не хотите использовать добавочное согласие, самый простой способ начать использовать MSAL — сделать запрос `acquireToken`, используя новый объект `AcquireTokenParameter`, и задать значение идентификатора ресурса.
...
Чтобы воспользоваться преимуществами добавочного согласия, необходимо создать список разрешений (областей), которые приложение использует при регистрации приложения, а затем распределить их по двум спискам на основе:
...
После того как вы организовали области, необходимо организовать каждый список ресурсов (API), для которых требуется запросить маркер. А также любые другие области, которые пользователь должен авторизовать в то же время.
...
Если вы попытаетесь использовать центр сертификации, который не известен корпорации Майкрософт и не входит в конфигурацию, вы получите `UnknownAuthorityException`.

### <a name="logging"></a>Ведение журнала
Теперь можно декларативно настроить ведение журнала как часть конфигурации, как показано ниже.
...
После успешного вызова `acquireToken` вы увидите ссылку на объект `IAccount`, который можно использовать в последующих запросах `acquireTokenSilent`.

### <a name="imultitenantaccount"></a>имултитенантаккаунт

Если у вас есть приложение, которое обращается к заявкам из каждого из клиентов, в которых представлена учетная запись, можно привести `IAccount` к `IMultiTenantAccount`. Этот интерфейс предоставляет карту `ITenantProfiles`, с ключом по ИДЕНТИФИКАТОРу клиента, которая позволяет получить доступ к утверждениям, принадлежащим учетной записи в каждом из клиентов, которые запросили маркер, относительно текущей учетной записи.
...
В ADAL существует один тип исключения, `AuthenticationException`, который включает метод для получения значения перечисления `ADALError`.
