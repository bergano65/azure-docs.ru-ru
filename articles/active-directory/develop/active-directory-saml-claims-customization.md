...
* Приложению требуется утверждение `NameIdentifier` или NameID, отличное от имени пользователя (или имени участника-пользователя), хранящегося в Azure AD.
...
| **Не указан** | Azure AD будет использовать в качестве формата NameID значение не указано. |
...
3. Чтобы применить несколько преобразований, щелкните **добавить преобразование**. К утверждению можно применить не более двух преобразований. Например, можно сначала извлечь префикс электронной почты для `user.mail`. Затем вставьте строку в верхнем регистре.
...
Во-первых, Azure AD проверяет, имеет ли тип пользователя `All guests`Брита значение. Так как это так, Azure AD назначает источник для утверждения `user.extensionattribute1`. Во-вторых, Azure AD проверяет, имеет `AAD guests`ли тип пользователя Брита значение, так как это также верно, а Azure AD назначает источник для утверждения. `user.mail` Наконец, заявка создается со значением `user.email` для Брита.
