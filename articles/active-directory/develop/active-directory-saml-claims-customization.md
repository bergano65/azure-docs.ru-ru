...
* Приложению требуется `NameIdentifier` утверждение или NameID, отличное от имени пользователя (или имени участника-пользователя), хранящегося в Azure AD.
...
| **Регулярные** | Azure AD будет использовать временный формат в качестве формата NameID. |
...
Дополнительные сведения см [. в таблице 3. Допустимые значения идентификатора на](active-directory-claims-mapping.md#table-3-valid-id-values-per-source)источник.
