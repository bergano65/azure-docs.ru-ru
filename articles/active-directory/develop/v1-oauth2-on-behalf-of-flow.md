---
title: Проверка подлинности между службами Azure Active Directory, использующая проект спецификации OAuth 2.0 On-Behalf-Of | Документация Майкрософт
...
1. Выберите **регистрация приложений** и затем **Регистрация нового**.
...
1. Значение URI перенаправления базовый URL-адрес.
1. Выберите **Зарегистрировать**, чтобы создать приложение.
1. Создайте секрет клиента, прежде чем выйти с портала Azure.
1. На портале Azure выберите свое приложение и выберите **сертификаты и секреты**.
1. Выберите **новый секрет клиента** и добавление секрета с длительностью один год или два года.
1. При сохранении этой страницы, портал Azure отображает значение секрета. Скопируйте и сохраните значение секрета в безопасном месте.

> [!IMPORTANT]
> Требуется секретный код для настройки параметров приложения в вашей реализации. Это секретное значение больше не отобразится, и не доступны другими средствами. Запишите его, как только он появится на портале Azure.
...
1. Значение URI перенаправления базовый URL-адрес.
1. Выберите **Зарегистрировать**, чтобы создать приложение.
1. Настройте разрешения для приложения. В **разрешения API**выберите **добавить разрешение** и затем **Мои API**.
1. Введите имя службы среднего уровня в текстовом поле.
1. Выберите **Выбор разрешений** , а затем выберите **доступа <service name>** .
...
Конечная точка маркера Azure AD выдает сообщение об ошибке при попытке получить маркер доступа для API нижнего уровня, который задается с помощью политики условного доступа (например, многофакторная проверка подлинности). Служба среднего уровня должна передать эту ошибку в клиентское приложение, чтобы клиентское приложение может обеспечивать взаимодействие с пользователем в соответствии с политикой условного доступа.
