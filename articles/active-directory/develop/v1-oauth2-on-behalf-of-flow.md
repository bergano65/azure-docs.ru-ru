...
1. Нажмите элемент **Регистрация приложений** и выберите пункт **Регистрация нового приложения**.
1. Введите понятное имя для приложения и укажите тип приложения.
    1. В зависимости от типа приложения, задайте базовый URL-адрес в качестве URL-адреса входа или URL-адреса перенаправления.
    1. Выберите **Создать**, чтобы создать приложение.
1. Создайте секрет клиента, прежде чем выйти с портала Azure.
   1. На портале Azure выберите свое приложение и нажмите **Параметры**.
   1. Выберите пункт **Ключи** в меню "Параметры" и добавьте ключ, срок действия которого составляет один или два года.
   1. При сохранении этой страницы на портале Azure отображается значение ключа. Скопируйте и сохраните значение ключа в безопасном месте.

      > [!IMPORTANT]
      > Ключ необходим для настройки параметров приложений в вашей реализации. Это значение ключа больше не будет отображаться, и его невозможно получить каким-либо другим способом. Запишите его, как только он появится на портале Azure.
...
1. Настройте разрешения для приложения.
   1. В меню "Параметры" выберите раздел **Необходимые разрешения**, а затем нажмите **Добавить** и **Выбрать API**.
   1. Введите имя службы среднего уровня в текстовом поле.
   1. Нажмите **Выбор разрешений**, а затем выберите **Доступ к имени службы**.
