...
1. Войдите на [портал Azure](https://portal.azure.com) с помощью личной учетной записи Майкрософт либо рабочей или учебной учетной записи. Alternatively, sign in to the Azure portal of choice for the national cloud.
...
1. После появления страницы **Регистрация приложения** введите сведения о регистрации приложения:
   ...
   1. For **Redirect URI**, add the type of application and the URI destination that will accept returned token responses after successful authentication. Например, введите **https://localhost:44321** . Затем щелкните **Регистрация**.
...
1. In the **Advanced settings** section, set **Logout URL** to **http://localhost:8080/msal4jsample/sign_out** . Затем нажмите кнопку **Сохранить**.
...
1. Выберите **Зарегистрировать**, чтобы создать приложение.
1. На странице приложения **Обзор** найдите **идентификатор приложения (клиента)** и запишите его, чтобы использовать позже. You'll need it to configure the Visual Studio configuration file for this project.
1. Select the **Authentication** section. In **Advanced settings**, set **Logout URL** to **http://localhost:5000/logout** . Затем нажмите кнопку **Сохранить**.
...
1. In the **Client Secrets** section, select **New client secret**, and then:
   ...
   1. Выберите **Добавить**.
   1. When the key value appears, copy it. Он понадобится вам позднее.
---
