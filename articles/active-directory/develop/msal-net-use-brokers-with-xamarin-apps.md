...
Поддержка брокера включена для каждого Публикклиентаппликатион. Она отключена по умолчанию. Используйте параметр `WithBroker()` (по умолчанию задано значение true) при создании `IPublicClientApplication` с помощью `PublicClientApplicationBuilder`.
...
Для этого необходимо предоставить действие (обычно это MainActivity) `WithParentActivityOrWindow(object parent)` в качестве родительского объекта. 
...
Однако на этапе разработки приложения, использующего Visual Studio, при отладке кода без подписи apk с помощью определенного сертификата Visual Studio подпишет APK для вас в целях отладки, предоставив APK уникальную подпись для компьютер, на котором он построен. Таким же, каждый раз при сборке приложения на другом компьютере необходимо обновить URI перенаправления в коде приложения и регистрацию приложения в портал Azure для проверки подлинности с помощью MSAL. 
