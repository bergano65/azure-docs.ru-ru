...
Необходимо включить поддержку брокера для отдельных экземпляров `PublicClientApplication`. По умолчанию поддержка отключена. При создании `PublicClientApplication` с помощью `PublicClientApplicationBuilder`используйте `WithBroker()` параметр, как показано в следующем примере. По `WithBroker()` умолчанию параметр имеет значение true.
...
Когда библиотека проверки подлинности Microsoft для .NET (MSAL.NET) вызывает брокер, брокер выполняет обратный вызов к приложению с `OpenUrl` помощью метода `AppDelegate` класса. Так как MSAL ожидает ответа брокера, приложение должно взаимодействовать для вызова MSAL.NET назад. Чтобы обеспечить взаимодействие, обновите `AppDelegate.cs` файл, чтобы переопределить следующий метод.
...
По-прежнему `AppDelegate.cs` в файле необходимо задать окно объекта. Как правило, для Xamarin iOS не нужно задавать окно объекта. Но для отправки и получения ответов от брокера требуется окно объекта. 

Чтобы настроить окно объекта, выполните следующие действия. 
1. В `AppDelegate.cs` файле задайте новое `App.RootViewController` `UIViewController()`значение. Это назначение гарантирует, что вызов брокера включает `UIViewController`. Если этот параметр назначен неправильно, может появиться следующее сообщение об ошибке:
...
`CFBundleURLSchemes` Имя должно включать `msauth.` в качестве префикса. После префикса используйте `CFBundleURLName`. 

В схеме URL-адресов `BundleId` уникальным образом идентифицирует приложение: `$"msauth.(BundleId)"`. Поэтому, `BundleId` если `com.yourcompany.xforms`имеет значение, используется схема URL `msauth.com.yourcompany.xforms`-адресов.
...
Обратите внимание, что URI перенаправления совпадает с `CFBundleURLSchemes` именем, которое `Info.plist` вы включили в файл.
...
Поддержка брокера включена для каждого Публикклиентаппликатион. Она отключена по умолчанию. Используйте `WithBroker()` параметр (по умолчанию задано значение true) при создании `IPublicClientApplication` с помощью `PublicClientApplicationBuilder`.
