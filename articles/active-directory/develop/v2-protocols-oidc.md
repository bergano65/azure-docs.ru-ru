...
Простое получение id_token не всегда достаточно для проверки подлинности пользователя; Вам также может потребоваться проверить подпись id_token и проверить утверждения в маркере на соответствие требованиям приложения. Как и во всех OIDC платформах, конечная точка платформы идентификации Майкрософт использует [токены JSON Web tokens (JWT)](https://tools.ietf.org/html/rfc7519) и шифрование с открытым ключом для подписи маркеров идентификации и проверки их допустимости.
...
| Параметр | Описание |
| --- | --- |
| `token` | Токен, который будет использоваться для вызова конечной точки UserInfo.|
