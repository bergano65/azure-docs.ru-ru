...
![Протокол OpenID Connect. Вход](./media/v2-protocols-oidc/convergence_scenarios_webapp.png)
...
Как правило, этот документ метаданных используется для настройки библиотеки OpenID Connect или пакета SDK. Метаданные будут использоваться для работы библиотеки. Тем не менее если вы не используете готовой библиотеки OpenID Connect, необходимо выполнить действия, описанные в оставшейся части этой статьи, для выполнения входа в веб-приложения с помощью конечной точки версии 2.0.
...
| область |Обязательно |Список областей с разделителями-пробелами. При использовании протокола OpenID Connect он должен содержать область `openid`, что преобразуется в разрешение "Вход" в пользовательском интерфейсе предоставления согласия. Для предоставления согласия этот запрос может также включать в себя другие области. |
...
| domain_hint |Необязательно | Область пользователя использовании федеративного каталога.  Пропускает процесс обнаружения на основе электронной почты, который проходит пользователь на версии 2.0-странице входа, несколько упрощает взаимодействие с пользователем. Для клиентов, которые входят в федерацию через локальный каталог, например служб федерации Active Directory это часто приводит простой вход в систему из-за существующего сеанса входа. |
...
| Параметр | Описание |
| --- | --- |
| ошибка |Строка кода ошибки, которую можно использовать для классификации типов возникших ошибок и реагирования на них. |
...
Для выхода пользователя из приложения недостаточно очистить файлы cookie или каким-то другим образом завершить сеанс пользователя. Чтобы пользователь вышел, его также необходимо перенаправить в конечную точку версии 2.0. Если этого не сделать, то пользователь сможет проходить повторную проверку подлинности без ввода учетных данных, так как для него сохранится действительный сеанс единого входа на конечной точке версии 2.0.
...
| состояние |Если запрос содержит параметр "state", в ответе должно отображаться то же значение. Приложение должно проверить, совпадают ли значения параметра "state" в запросе и ответе. |
