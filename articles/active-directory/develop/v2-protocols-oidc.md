...
> [!IMPORTANT]
> Чтобы успешно запросить маркер идентификации, в регистрации приложения на [портале регистрации](https://apps.dev.microsoft.com) для веб-клиента должно быть включено **[неявное предоставление разрешения](v2-oauth2-implicit-grant-flow.md)**. Если его не включить, будет возвращена следующая ошибка `unsupported_response`: "The provided value for the input parameter 'response_type' is not allowed for this client. Expected value is 'code'" (Указанное значение параметра response_type запрещено для данного клиента. Ожидаемое значение: code).
