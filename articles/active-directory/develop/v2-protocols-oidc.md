...
![Протокол OpenID Connect: вход в систему](./media/v2-protocols-oidc/convergence_scenarios_webapp.png)
...
> [!IMPORTANT]
> Чтобы успешно запросить токен идентификатора, в регистрации приложения на [портале регистрации](https://apps.dev.microsoft.com) для веб-клиента должно быть включено **[неявное предоставление](v2-oauth2-implicit-grant-flow.md)**. Если эта функция не включена, будет возвращена ошибка `unsupported_response`: "The provided value for the input parameter 'response_type' is not allowed for this client. Expected value is 'code'" (Указанное значение параметра response_type запрещено для данного клиента. Ожидаемое значение: code).
...
![Протокол OpenID Connect: получение маркера](./media/v2-protocols-oidc/convergence_scenarios_webapp_webapi.png)
