...
- **центр сертификации**: необязательный. URL-адрес, указывающий каталог, из которого MSAL может запрашивать маркеры. Значение по умолчанию: `https://login.microsoftonline.com/common`.
    * В Azure AD это форма https://&lt;instance&gt;/&lt;аудитории&gt;, где &lt;экземпляр&gt; является доменом поставщика удостоверений (например, `https://login.microsoftonline.com`), а &lt;аудиторией&gt; — идентификатором, представляющим аудиторию входа. Это могут быть следующие значения:
    ...
    * В Azure AD B2C это форма `https://<instance>/tfp/<tenant>/<policyName>/`, где экземпляр — это Azure AD B2C домен, т. е. {имя вашего клиента}. b2clogin. com, клиент — это имя клиента Azure AD B2C, т. е. {имя вашего клиента}. onmicrosoft. com, policyName — это имя применяемой политики B2C.
