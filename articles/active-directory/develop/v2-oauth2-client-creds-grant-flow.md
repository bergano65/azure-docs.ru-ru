---
title: Использование платформы удостоверений для доступа к защищенным ресурсам без участия пользователя | Azure
...
> [!NOTE]
> Конечная точка платформы удостоверений Microsoft не поддерживает все сценарии Azure AD и компоненты. Чтобы определить, следует ли использовать конечную точку платформы Microsoft identity, ознакомьтесь [ограничения платформы удостоверений Microsoft](active-directory-v2-limitations.md).
...
2. Перейдите в приложение на взаимодействие с приложениями регистрации (Предварительная версия). Перейдите к **сертификаты и секреты** и добавьте **новый секрет клиента**, так как вам потребуется по крайней мере один секрет клиента для запроса маркера.
...
Если вход пользователя в приложении, можно определить организацию, к которой принадлежит пользователь, прежде чем запрашивать у пользователя утверждение разрешений приложения. Хотя это не обязательно, таким образом можно сделать пользовательский интерфейс более интуитивно понятным. Для входа пользователя, выполните наши [учебниках по протоколу платформы удостоверений Microsoft](active-directory-v2-protocols.md).
...
На этом этапе Azure AD принудительно указывает, что только администратора клиента могут входить в полный запрос. Администратору будет предложено утвердить все непосредственные разрешения, запрошенные для приложения на портале регистрации приложений.
...
| `scope` | Обязательно для заполнения | Значение, передаваемое для параметра `scope` в этом запросе, должно быть идентификатором требуемого ресурса (универсальным кодом ресурса (URI) идентификатора приложения) с суффиксом `.default`. В примере Microsoft Graph используется значение `https://graph.microsoft.com/.default`. <br/>Это значение сообщает конечной точкой платформа удостоверений Майкрософт, что все непосредственные разрешения, которые вы настроили для своего приложения, она должна выдать маркер для тех, которые связаны с ресурсом, который вы хотите использовать. Дополнительные сведения об области `/.default` можно найти в документации о согласии [здесь](v2-permissions-and-consent.md#the-default-scope). |
...
| `token_type` | Указывает значение типа маркера. Единственный тип поддерживает платформу удостоверений Microsoft `bearer`. |
...
> [!NOTE]
> В порядке для вашего приложения иметь возможность получать маркер v2 можно обновлять файл манифеста приложения из портала azure. Вы можете добавить атрибут `accessTokenAcceptedVersion` и задайте значение 2 в качестве `"accessTokenAcceptedVersion": 2`. Проверьте статью [манифест приложения](https://docs.microsoft.com/en-us/azure/active-directory/develop/reference-app-manifest#manifest-reference) больше об этом знать. По умолчанию приложение в настоящее время получит маркер v1. Если оно не определено в манифесте приложения или веб-API, его значение для этого атрибута в манифесте по умолчанию равно 1, и таким образом, приложение будет получать маркер v1.  
