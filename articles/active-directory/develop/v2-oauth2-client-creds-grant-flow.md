...
1. Создайте и зарегистрируйте приложение с помощью [портала регистрации приложений](quickstart-v2-register-an-app.md) или нового [интерфейса Регистрации приложений (предварительная версия)](quickstart-register-app.md).
1. Перейдите к приложению на портале, который использовался для регистрации или создания приложения. При создании приложения потребуется использовать по крайней мере один секрет приложения.
1. Найдите раздел **Разрешения API** и добавьте **разрешения приложения**, необходимые для приложения.
1. **Сохраните** регистрацию приложения.
...
| `client_secret` | Обязательно для заполнения | Секрет приложения, созданный для приложения на портале регистрации приложений. Секрет клиента должен быть преобразован в формат URL-адреса перед отправкой. |
