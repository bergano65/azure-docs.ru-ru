...
Поставщик ресурсов может принудительно применять проверку авторизации на основе списка известных идентификаторов приложений (клиента), а также предоставлять определенный уровень доступа на основе этих идентификаторов. Когда ресурс получает маркер от конечной точки платформы идентификации Майкрософт, он может декодировать маркер и извлечь идентификатор приложения клиента из `appid` `iss` утверждений и. Затем он проверяет наличие приложения в действующем списке управления доступом. Детализация и методы использования списка управления доступом для разных ресурсов могут значительно отличаться.
...
> [!NOTE]
> Чтобы включить эту модель авторизации на основе ACL, Azure AD не требует, чтобы приложения были авторизованы для получения маркеров для другого приложения, поэтому маркеры только для приложений могут быть выданы без `rules` утверждения. Приложения, которые предоставляют API, должны реализовывать проверки разрешений для принятия маркеров.
...
Дополнительные сведения о разрешениях приложений см. в [документации согласие и разрешения](v2-permissions-and-consent.md#permission-types).
...
| --- | --- | --- |
| `tenant` | Обязательное значение | Клиент каталога, который будет использовать приложение, в формате GUID или доменного имени. |
| `client_id` | Требуется |Идентификатор приложения (клиента), который назначен приложению. |
...
| `client_assertion_type` | Требуется | Его значением должно быть `urn:ietf:params:oauth:client-assertion-type:jwt-bearer`. |
