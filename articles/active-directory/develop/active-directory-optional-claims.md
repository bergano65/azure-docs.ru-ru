...
Разработчикам приложений можно использовать необязательные утверждения в свои приложения Azure AD, чтобы указать утверждения, которые нужно поместить в токены, отправляемые в приложение. 
...
## <a name="v10-and-v20-optional-claims-set"></a>версия 1.0 и версии 2.0 набора необязательных утверждений
...
**Таблица 2: 1.0 и версии 2.0 необязательный набор утверждений**
...
**Таблица 3: только для версии 2.0 необязательных утверждений**
...
| `family_name` | Фамилия                       | Предоставляет последний имя, Фамилия или фамилию пользователя, как определено в объекте пользователя. <br>"family_name":"Miller" | Поддерживается в MSA и Azure AD   |
| `given_name`  | Имя                      | Указывает на имя или «заданное» имя пользователя, которое было задано в объекте пользователя.<br>"given_name": "Frank"                   | Поддерживается в MSA и Azure AD  |
...
> [!Note]
> - Расширения схемы каталогов являются Azure AD — уникальная функция, поэтому если ваш манифест приложения запрашивает пользовательское расширение, а пользователь MSA регистрируется в вашем приложении, эти расширения не возвращается.
> - Необязательные утверждения Azure AD работают только с помощью расширения Azure AD и не работает работы с расширением directory Microsoft Graph. Оба API требуют `Directory.ReadWriteAll` разрешение, которое может быть использовано только администраторами.
...
> [!IMPORTANT]
> См. в разделе [настроить групповые заявки для приложений с Azure AD](../hybrid/how-to-connect-fed-group-claims.md) Дополнительные сведения, включая важные предупреждения для общедоступной предварительной версии групповые заявки от локальных атрибутов.
