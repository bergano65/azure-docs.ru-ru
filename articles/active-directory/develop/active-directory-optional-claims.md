...
> - Необязательные утверждения Azure AD работают только с помощью расширения Azure AD и не работает с расширением directory Microsoft Graph. Оба API требуют `Directory.ReadWriteAll` разрешение, которое может быть использовано только администраторами.
