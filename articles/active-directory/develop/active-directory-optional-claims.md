...
ms.locfileid: "58882807"
---
# <a name="how-to-provide-optional-claims-to-your-azure-ad-app-preview"></a>Практическое руководство: Предоставлять дополнительные утверждения для приложения Azure AD (Предварительная версия)
...
Список стандартных утверждениях и их использовании в токенах, см. в разделе [основами маркеры, выданные Azure AD](v1-id-and-access-tokens.md).
...
| `sid`                      | Идентификатор сеанса, используемый для каждого сеанса пользователя на выход. | JWT        |           |         |
...
**Таблица 3. Необязательные утверждения только для версии 2.0**
...
|                | `include_externally_authenticated_upn_without_hash` | Аналогично указанному выше, за исключением того, что отмечает хэш (`#`) заменяются символом подчеркивания (`_`), например `foo_hometenant.com_EXT_@resourcetenant.com` |
