...
ms.locfileid: "68835326"
---
# <a name="how-to-provide-optional-claims-to-your-azure-ad-app"></a>Практическое руководство. Предоставление необязательных утверждений для приложения Azure AD
...
| Название                       |  Описание   | Тип маркера | Тип пользователя | Примечания  |
|----------------------------|----------------|------------|-----------|--------|
...
| Утверждение JWT     | Название                            | Описание                                | Примечания |
|---------------|---------------------------------|-------------|-------|
...
> [!IMPORTANT]
> Маркеры доступа **всегда** создаются с помощью манифеста ресурса, а не клиента.  Так что в запросе `...scope=https://graph.microsoft.com/user.read...` ресурс является графом.  Таким же маркер доступа создается с помощью манифеста графа, а не манифеста клиента.  Изменение манифеста для приложения никогда не приведет к тому, что маркеры для Graph будут выглядеть иначе.  Чтобы проверить, что `accessToken` изменения вступают в действие, запросите маркер для вашего приложения, а не другого приложения.  
...
> - Дополнительные утверждения Azure AD работают только с расширением Azure AD и не работают с расширением Microsoft Graph Directory. Для обоих API требуется `Directory.ReadWriteAll` разрешение, которое может быть предоставлено только администраторам.
