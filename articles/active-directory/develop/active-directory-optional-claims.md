...
| Утверждение JWT     | Имя                            | Описание | Примечания |
|---------------|---------------------------------|-------------|-------|
|`aud`          | Аудитория | Всегда имеется в JWT, но в маркерах доступа v1 его можно создавать различными способами, что может быть трудно программировать при проверке маркера.  Используйте [Дополнительные свойства для этого утверждения](#additional-properties-of-optional-claims) , чтобы убедиться, что для него всегда задано значение GUID в маркерах доступа v1. | только маркеры доступа JWT v1|
...
| `aud`          |                          | В маркерах доступа v1 этот параметр используется для изменения формата `aud` утверждения.  Это не влияет на токены v2 или маркеры идентификации, где `aud` утверждение всегда является идентификатором клиента. Используйте этот способ, чтобы обеспечить более простое выполнение проверки аудитории API. Как и все необязательные утверждения, влияющие на маркер доступа, ресурс в запросе должен установить это необязательное утверждение, так как ресурсы владеют маркером доступа.|
|                | `use_guid`               | Выдает идентификатор клиента ресурса (API) в формате GUID в качестве `aud` утверждения вместо URI AppID или GUID. Таким образом, если идентификатор клиента ресурса — `bb0a297b-6a42-4a55-ac40-09a501456577` , любое приложение, запрашивающее маркер доступа для этого ресурса, получит маркер доступа со `aud` следующим: `bb0a297b-6a42-4a55-ac40-09a501456577` .|
