...
ms.locfileid: "55077258"
---
# <a name="how-to-provide-optional-claims-to-your-azure-ad-app-public-preview"></a>Практическое руководство: Предоставление дополнительных утверждений для приложения Azure Active Directory (общедоступная предварительная версия)
...
Дополнительные сведения о стандартных утверждениях и их использовании в токенах см. в статье [Справочник по токенам в Azure AD](v1-id-and-access-tokens.md). 
...
| Учетная запись Azure AD          | Поддерживаются                          | Поддержка с предупреждениями |

> [!IMPORTANT]
> В приложениях, в которых поддерживаются как личные учетные записи, так и Azure AD, (зарегистрированных с помощью [портала регистрации приложений](https://apps.dev.microsoft.com)) нельзя использовать необязательные утверждения. Тем не менее приложения, которые были зарегистрированы, только чтобы использовать Azure AD и конечную точку версии 2.0, могут получить необязательные утверждения, запрашиваемые в манифесте. Для изменения необязательных утверждений на портале Azure можно использовать редактор манифестов приложений в интерфейсе **Регистрация приложений**. Но эта возможность пока что недоступна в редакторе манифестов приложений в интерфейсе **Регистрация приложений (предварительная версия)**.

## <a name="standard-optional-claims-set"></a>Набор стандартных необязательных утверждений

Набор необязательных утверждений доступен по умолчанию для использования приложениями, перечисленными ниже. Чтобы добавить настраиваемые необязательные утверждения для приложения, ознакомьтесь с разделом [Расширения каталога](active-directory-optional-claims.md#Configuring-custom-claims-via-directory-extensions) ниже. Обратите внимание, что процесс добавления утверждений в **маркер доступа** приведет к их применению в запрашиваемых *маркерах доступа* приложения (веб-интерфейса API), а не в *самом* приложении. Это гарантирует, что независимо от того, получил ли клиент доступ к вашему API, в маркере доступа, используемом при аутентификации с помощью API, будут присутствовать правильные данные.

> [!NOTE]
> Большую часть этих утверждений можно включить в JWT для токенов версии 1.0 и 2.0, кроме токенов SAML, за исключением оговоренных в столбце типов токенов. Кроме того, в текущее время необязательные утверждения поддерживаются только для пользователей AAD, поддержка MSA будет добавлена. Когда в MSA появится поддержка необязательных утверждений в конечной точке версии 2.0, в столбце типа пользователя будет указано, доступно ли утверждение для пользователя AAD или MSA. 
...
| `upn`                      | Утверждение UserPrincipalName. | JWT, SAML  |           | Несмотря на то, что это утверждение автоматически включается, можно указать его в качестве необязательного утверждения для присоединения дополнительных свойств, чтобы изменить его поведение в варианте использования гостевым пользователем. <br> Дополнительные свойства: <br> `include_externally_authenticated_upn` <br> `include_externally_authenticated_upn_without_hash` |

### <a name="v20-optional-claims"></a>Необязательные утверждения версии 2.0

Эти утверждения всегда включаются в токены версии 1.0, но не включаются в токены версии 2.0, если они не запрошены. Эти утверждения применяются только для JWT (токены идентификации и маркер доступа). 
...
| `in_corp`     | В корпоративной сети        | Посылает сигнал, если клиент входит в корпоративную сеть. В противном случае это утверждение не добавляется.   |       |
...
|---------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|-------------|
| `upn`                                                 |                                                                                                                                      |  Может использоваться для ответов SAML и JWT.        |
...
С помощью изменения манифеста приложения можно настроить необязательные утверждения (см. пример ниже). Дополнительные сведения см. в статье [Манифест приложения Azure Active Directory](reference-app-manifest.md).
