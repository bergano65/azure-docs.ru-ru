---
title: Создайте итог приложения Azure (PowerShell) Azure
...
Вы можете задать область действия на уровне подписки, группы ресурсов или ресурса. Разрешения наследуют более низкие уровни области действия. Например, добавление приложения к роли *Reader* для группы ресурсов означает, что оно может читать группу ресурсов и любые ресурсы, которые она содержит. Чтобы позволить приложению выполнять такие действия, как перезагрузка, запускать и останавливать экземпляры, выберите роль *автора.*

## <a name="create-service-principal-with-self-signed-certificate"></a>Создание субъекта-службы с самозаверяющим сертификатом

Ниже приводится простой пример сценария. Он использует [New-AzADServicePrincipal](/powershell/module/az.resources/new-azadserviceprincipal) для создания директора службы с самоподписанным сертификатом и использует [New-AzRoleAssignment](/powershell/module/az.resources/new-azroleassignment) для присвоения роли [читателя](/azure/role-based-access-control/built-in-roles#reader) директору службы. Назначение ролей ограничивается текущей выбранной подпиской Azure. Чтобы выбрать другую подписку, выполните командлет [Set-AzContext](/powershell/module/Az.Accounts/Set-AzContext).

> [!NOTE]
> Смдлет New-SelfSignedCertificate и модуль PKI в настоящее время не поддерживаются в PowerShell Core. 
...
В приведенном ниже примере создается субъект-служба с сертификатом, выданным центром сертификации. Назначение ограничено указанной подпиской Azure. Он добавляет основной сервис к роли [Читателя.](../../role-based-access-control/built-in-roles.md#reader) Если при назначении ролей возникнет ошибка, назначение выполняется повторно.
...
* **"Authentication_Unauthorized"** (Аутентификация не выполнена) или **"No subscription found in the context"** (В контексте не удалось найти подписку). Эта ошибка может отобразиться, если учетная запись не имеет [необходимых разрешений](#required-permissions) в Azure AD для регистрации приложения. Как правило, вы видите эту ошибку, когда только пользователи-админы в вашем active Directory Azure могут зарегистрировать приложения, а ваша учетная запись не является админ- Попросите администратора либо назначить вас на роль администратора, либо предоставить пользователям возможность зарегистрировать приложения.
