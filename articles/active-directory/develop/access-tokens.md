...
|`idp`|Строка, обычно — URI STS | Фиксирует поставщика удостоверений, который проверил подлинность субъекта маркера. Это значение идентично значению утверждения Issuer за исключением случаев, когда учетная запись пользователя и издатель принадлежат разным клиентам. Если утверждение отсутствует, это означает, что взамен можно использовать значение `iss`.  Для пользовательских учетных записей, используемых в контексте orgnizational (например, приглашение пользовательской учетной записи для клиента Azure AD), утверждение `idp` может быть "live.com" или URI STS, содержащим клиента учетной записи Microsoft `9188040d-6c67-4c5b-b112-36a304b66dad`. |  
...
* Если вы запросили в маркере доступа утверждения `roles` или `groups`, убедитесь, что пользователь входит в группу, которая может выполнять это действие.
  * В случае с маркерами, получаемых через неявную последовательность, для получения этих данных обычно нужно запрашивать [Microsoft Graph](https://developer.microsoft.com/graph/), ведь их размер часто превышает допустимый для маркера. 
