...
В маркер включаются только те утверждения, для которых существуют значения. Приложение не должно зависеть от утверждения. Примеры: `pwd_exp` (не каждый клиент требует, чтобы срок действия паролей истек), а `family_name` потоки ([учетные данные клиента] (v2-OAuth2-Client-creds-Grant-Flow.md) — от имени приложений, у которых нет имен). Утверждения, используемые для проверки маркера доступа, присутствуют всегда.
