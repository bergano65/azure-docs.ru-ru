...
description: Узнайте, как принимать маркеры доступа, создаваемых конечными точками Azure AD версий 1.0 и 2.0.
...
| `x5t` | Строка | Действует и указывается точно так же, как `kid`. `x5t` создается, только в версии 1.0 маркеры доступа для обеспечения совместимости устаревших утверждения. |
...
* MaxInactiveTime: если токен обновления не использовался в течение времени, указанного в параметре MaxInactiveTime, этот токен обновления считается недопустимым. 
