---
title: Маркеры доступа платформы удостоверений Майкрософт | Службы
...
ms.locfileid: "82926805"
---
# <a name="microsoft-identity-platform-access-tokens"></a>Маркеры доступа платформы удостоверений Майкрософт

Маркеры доступа позволяют клиентам безопасно вызывать защищенные интерфейсы API. Маркеры доступа платформы идентификации Майкрософт — это [JWT](https://tools.ietf.org/html/rfc7519)объекты JSON в кодировке Base64, подписанные платформой идентификации Майкрософт. Клиентам следует обрабатывать маркеры как непрозрачные строки, так как их содержимое предназначено только для ресурса. В целях проверки и отладки разработчики могут декодировать JWT (веб-токены JSON) с помощью сайта, например [JWT.MS](https://jwt.ms). Клиент может получить маркер доступа либо из конечной точки версии 1.0, либо из конечной точки версии 2.0, используя различные протоколы.

Когда клиент запрашивает маркер доступа, платформа Microsoft Identity также возвращает некоторые метаданные о маркере доступа для использования приложением. Эти сведения включают срок действия маркера доступа и области, для которых он действителен. Благодаря этим данным приложение может выполнять интеллектуальное кэширование маркеров доступа, не анализируя сам маркер.
...
> [!IMPORTANT]
> Маркеры доступа создаются на основе *аудитории* маркера, то есть к приложению, владеющему областями в маркере.  Таким образом, параметр `accessTokenAcceptedVersion` ресурса в [манифесте приложения](reference-app-manifest.md#manifest-reference) `2` позволяет клиенту, вызывающему конечную точку версии 1.0, получить маркер доступа версии 2.0.  Аналогичным образом, поэтому изменение [необязательных утверждений](active-directory-optional-claims.md) маркера доступа для клиента не изменяет маркер доступа `user.read`, полученный при запросе маркера, который принадлежит ресурсу.
...
JWT (веб-маркеры JSON) делятся на три части:
...
В маркер включаются только те утверждения, для которых существуют значения. Поэтому приложение не должно зависеть от утверждения. Примеры: `pwd_exp` (не каждый клиент требует, чтобы срок действия паролей истек `family_name` ), а потоки (учетные данные клиента ([v 1.0](../azuread-dev/v1-oauth2-client-creds-grant-flow.md), [v 2.0](v2-oauth2-client-creds-grant-flow.md)) — от имени приложений, у которых нет имен). Утверждения, используемые для проверки маркера доступа, присутствуют всегда.
...
| `azpacr` | 0, 1 или 2 | Только в маркерах версии 2.0, замена для `appidacr`. Указывает на способ проверки подлинности клиента. Для общедоступного клиента значение равно 0. При использовании идентификатора и секрета клиента значение равно 1. Если для аутентификации использовался сертификат клиента, значение равно 2. |
...
| `wids` | Массив идентификаторов GUID [RoleTemplateID](https://docs.microsoft.com/azure/active-directory/users-groups-roles/directory-assign-admin-roles#role-template-ids) | Обозначает роли на уровне клиента, назначенные этому пользователю, из раздела ролей, присутствующего на [странице роли администратора](https://docs.microsoft.com/azure/active-directory/users-groups-roles/directory-assign-admin-roles#role-template-ids).  Это утверждение настраивается отдельно для каждого приложения с помощью `groupMembershipClaims` свойства [манифеста приложения](reference-app-manifest.md).  Для него необходимо задать значение "ALL" или "DirectoryRole".  Может отсутствовать в токенах, полученных через неявный поток из-за проблем с длиной токена. |
...
Вы можете использовать `BulkCreateGroups.ps1` предоставленный в папке [скриптов создания приложений](https://github.com/Azure-Samples/active-directory-aspnetcore-webapp-openidconnect-v2/tree/master/5-WebApp-AuthZ/5-2-Groups/AppCreationScripts) , чтобы помочь вам протестировать более длительные сценарии.
...
| `in_corp`| boolean | Посылает сигнал, если клиент входит в корпоративную сеть. В противном случае утверждение не включается. |
...
`alg` Утверждение указывает алгоритм, который использовался для подписания маркера, а `kid` утверждение указывает конкретный открытый ключ, который использовался для проверки маркера.
...
> [!TIP]
> Попробуйте использовать этот [URL-адрес](https://login.microsoftonline.com/common/v2.0/.well-known/openid-configuration) в браузере!
...
* Проверьте утверждение `scp` или `roles` , чтобы убедиться, что все существующие области соответствуют предоставленным API, и разрешите клиенту выполнить запрошенное действие.
* С помощью утверждения `appid` убедитесь, что вызывающий клиент имеет право вызывать API.
* Проверка состояния проверки подлинности вызывающего клиента `appidacr` с помощью команды. она не должна быть равна 0, если общедоступные клиенты не могут вызывать API.
* Проверьте список прошлых `nonce` утверждений, чтобы убедиться, что маркер не воспроизводится.
...
* Если вы запросили `roles` утверждения `groups` или в маркере доступа, убедитесь, что пользователь входит в группу, которой разрешено выполнять это действие.
...
* Токены только для приложений не будут иметь `scp` утверждения, а вместо этого могут иметь `roles` утверждение. Именно в нем будут указываться разрешения приложения (в отличие от делегированных разрешений). Дополнительные сведения о делегированных и разрешениях приложений см. в разделе разрешение и согласие ([v 1.0](../azuread-dev/v1-permissions-consent.md), [v 2.0](v2-permissions-and-consent.md)).
* Многие заявки, связанные с человеком, будут отсутствовать, `name` например `upn`или.
...
| Пользователь отменяет свои маркеры обновления [с помощью PowerShell](https://docs.microsoft.com/powershell/module/azuread/revoke-azureadsignedinuserallrefreshtoken) | Отозвано | Отозвано | Отозвано | Отозвано | Отозвано |
| Администратор отменяет все маркеры обновления для пользователя с [помощью PowerShell](https://docs.microsoft.com/powershell/module/azuread/revoke-azureaduserallrefreshtoken) | Отозвано | Отозвано |Отозвано | Отозвано | Отозвано |
| Единый выход ([v 1.0](../azuread-dev/v1-protocols-openid-connect-code.md#single-sign-out), [v 2.0](v2-protocols-oidc.md#single-sign-out) ) в Интернете | Отозвано | Остается активным | Отозвано | Остается активным | Остается активным |
...
* Дополнительные сведения об [ `id_tokens` Azure AD](id-tokens.md).
