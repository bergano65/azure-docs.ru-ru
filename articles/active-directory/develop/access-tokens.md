...
Приложение может получать маркеры от имени пользователя (обычный поток) или непосредственно из приложения через поток учетных данных клиента ([v1.0](../azuread-dev/v1-oauth2-client-creds-grant-flow.md), [v2.0](v2-oauth2-client-creds-grant-flow.md)). Маркеры для приложений указывают, что вызов поступает от самого приложения, а не по запросу пользователя. Эти маркеры обрабатываются в целом как обычно, но с некоторыми отличиями:

* У маркеров для приложений нет утверждения `scp`. Вместо него есть утверждение `roles`. Именно в нем будут указываться разрешения приложения (в отличие от делегированных разрешений). Дополнительные сведения о делегированных разрешениях и разрешениях приложения см. в статьях о разрешениях и согласии (для [версии 1.0](../azuread-dev/v1-permissions-consent.md) и [версии 2.0](v2-permissions-and-consent.md)).
* Также в нем отсутствуют многие утверждения, касающиеся пользователя, например `name` или `upn`.
