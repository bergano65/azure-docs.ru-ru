...
>
> По той же причине, при тестировании клиентского приложения с помощью API Microsoft, поддерживающего личную учетную запись (например, hotmail.com или outlook.com), вы обнаружите, что полученный клиентом маркер доступа является непрозрачной строкой. Это связано с тем, что доступ к ресурсу использует зашифрованные токены и не может быть понят клиентом.  Это ожидаемое значение, и это не должно быть проблемой для приложений. клиентские приложения никогда не должны иметь зависимости от формата маркера доступа. 
