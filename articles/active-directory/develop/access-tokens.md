...
| Утверждение | Формат | ОПИСАНИЕ |
|-----|--------|-------------|
| `aud` | Строка "URI идентификатора приложения". | Определяет целевого получателя маркера. В маркерах идентификации аудиторией является идентификатор приложения, назначенный приложению в портал Azure. Приложение должно проверить это значение и отклонить маркер, если он ему не соответствует. |
...
| `aio` | Непрозрачная строка | Внутреннее утверждение, используемое Azure AD для записи данных для повторного использования маркеров. Ресурсы не должны использовать это утверждение. |
...
| `hasgroups` | Логическое значение. | Если он присутствует, всегда имеет значение `true`. Это обозначает, что пользователь входит как минимум в одну группу. Используется вместо утверждения `groups` для JWT в неявных потоках предоставления, если утверждение полной группы расширяет фрагмент URI за пределы ограничения длины URL-адреса (в настоящее время 6 или более групп). Указывает на то, что клиент должен использовать Graph для определения групп пользователя (`https://graph.windows.net/{tenantID}/users/{userID}/getMemberObjects`). |
...
| `in_corp`| логическое значение | Посылает сигнал, если клиент входит в корпоративную сеть. В противном случае утверждение не включается. |
