...
Приложение может получить маркеры для пользователя (обычно он обсуждается) или напрямую из приложения (через [поток учетных данных клиента](v1-oauth2-client-creds-grant-flow.md)). Маркеры для приложений указывают, что вызов поступает от самого приложения, а не по запросу пользователя. Эти маркеры обрабатываются практически так же:
