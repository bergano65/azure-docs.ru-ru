...
В маркер включаются только те утверждения, для которых существуют значения. Поэтому приложение не должно зависеть от утверждения. В качестве примеров можно назвать `pwd_exp` (не каждому клиенту нужен срок действия паролей) или `family_name` (потоки [учетных данных клиента](v1-oauth2-client-creds-grant-flow.md) выполняются от лица приложений, которым не присвоены имена). Утверждения, используемые для проверки маркера доступа, присутствуют всегда.
