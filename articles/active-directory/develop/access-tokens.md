...
> [!IMPORTANT]
> Маркеры доступа создаются на основе *аудитории* маркера, то есть к приложению, владеющему областями в маркере.  Таким образом, параметр `accessTokenAcceptedVersion` ресурса в [манифесте](reference-app-manifest.md#manifest-reference) `2` приложения позволяет клиенту, вызывающему конечную точку версии 1.0, получить маркер доступа версии 2.0.  Аналогичным образом, поэтому изменение [необязательных утверждений](active-directory-optional-claims.md) маркера доступа для клиента не изменяет маркер доступа `user.read`, полученный при запросе маркера, который принадлежит ресурсу MS Graph.  
...
| `nonce` | Строковое | Уникальный идентификатор для защиты от атак с повторением маркеров. Ваш ресурс может сохранять это значение для защиты от повторов. |
| `alg` | Строковое | Обозначает алгоритм, с помощью которого был подписан маркер, например RS256. |
| `kid` | Строковое | Содержит отпечаток для открытого ключа, который использовался для подписания этого маркера. Выпускаются в маркерах доступа версий 1.0 и 2.0. |
| `x5t` | Строковое | Действует и указывается точно так же, как `kid`. `x5t` — устаревшее утверждение, которое выпускается только в маркерах доступа версии 1.0 для обеспечения совместимости. |
...
| `name` | Строковое | Предоставляет удобное для восприятия значение, которое идентифицирует субъект маркера. Это значение не обязательно должно быть уникальным. Оно изменяемое и предназначено только для отображения. Чтобы получить это утверждение, необходимо указать область `profile` . |
...
| `hasgroups` | логический | Если он присутствует, всегда имеет значение `true`. Это обозначает, что пользователь входит как минимум в одну группу. Используется вместо утверждения `groups` для JWT в неявных потоках предоставления, если утверждение полной группы расширяет фрагмент URI за пределы ограничения длины URL-адреса (в настоящее время 6 или более групп). Указывает на то, что клиент должен использовать Graph для определения групп пользователя (`https://graph.windows.net/{tenantID}/users/{userID}/getMemberObjects`). |
...
| `unique_name` | Строковое | Присутствует только в маркерах версии 1.0. Предоставляет удобное для восприятия значение, которое идентифицирует субъект маркера. Это значение не обязательно должно быть уникальным в пределах клиента. Оно используется только для отображения. |
...
| Утверждение | Формат | Описание |
|-----|--------|-------------|
| `ipaddr`| Строковое | IP-адрес, с которого пользователь прошел аутентификацию. |
...
| `pwd_url`| Строковое | URL-адрес, на который можно направить пользователя для сброса пароля. |
| `in_corp`| boolean | Посылает сигнал, если клиент входит в корпоративную сеть. В противном случае утверждение не включается. |
| `nickname`| Строковое | Дополнительное имя для пользователя, отдельное от имени или фамилии.|
| `family_name` | Строковое | Указывает фамилию пользователя, которая определена в объекте пользователя. |
| `given_name` | Строковое | Указывает личное имя пользователя, которое задано в объекте пользователя. |
| `upn` | Строковое | Имя пользователя. Может содержать номер телефона, адрес электронной почты или любую неформатированную строку. Должно использоваться только для отображения и подсказки имени пользователя в сценариях повторной аутентификации. |
...
Утверждение указывает алгоритм, который использовался для подписания маркера, `kid` а утверждение указывает конкретный открытый ключ, который использовался для проверки маркера. `alg`
...
* Проверьте утверждение `roles` или, чтобы убедиться, что все существующие области соответствуют предоставленным API, и разрешите клиенту выполнить запрошенное действие. `scp`
