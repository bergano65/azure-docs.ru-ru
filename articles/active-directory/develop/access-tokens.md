...
ms.locfileid: "99575760"
---
# <a name="microsoft-identity-platform-access-tokens"></a>Маркеры доступа платформы удостоверений Майкрософт

Маркеры доступа позволяют клиентам безопасно вызывать защищенные веб-API и использовать их для проверки подлинности и авторизации в веб-API. В соответствии со спецификацией OAuth маркеры доступа являются непрозрачными строками без формата набора. Некоторые поставщики удостоверений (поставщиков удостоверений) используют идентификаторы GUID, а другие используют зашифрованные большие двоичные объекты. Платформа Microsoft Identity использует различные форматы маркеров доступа в зависимости от конфигурации API, который принимает маркер. [Пользовательские API, зарегистрированные разработчиками](quickstart-configure-app-expose-web-apis.md) на платформе Microsoft Identity, могут выбирать из двух разных форматов веб-маркеров JSON (JWT), именуемых "v1" и "v2", и разработанных корпорацией Майкрософт API, таких как Microsoft Graph или API в Azure, имеют дополнительные форматы маркеров. Эти частные форматы могут быть зашифрованными токенами, JWT или специальными маркерами, похожими на JWT, которые не будут проверяться.

Клиенты должны рассматривать маркеры доступа как непрозрачные строки, так как содержимое токена предназначено только для ресурсов (API). *Только* в целях проверки и отладки разработчики могут декодировать JWT с помощью сайта, например [JWT.MS](https://jwt.ms). Однако имейте в виду, что токены, получаемые для Microsoft API, могут не всегда быть JWT, и их не всегда можно декодировать.

Чтобы получить подробные сведения о том, что находится внутри маркера доступа, клиенты должны использовать данные ответа маркера, возвращаемые с маркером доступа, клиенту. Когда клиент запрашивает маркер доступа, платформа Microsoft Identity также возвращает некоторые метаданные о маркере доступа для использования приложением. Эти сведения включают срок действия маркера доступа и области, для которых он действителен. Благодаря этим данным приложение может выполнять интеллектуальное кэширование маркеров доступа, не анализируя сам маркер.

В следующих разделах содержатся сведения о том, как ваш API может проверять и использовать утверждения внутри маркера доступа.  

> [!NOTE]
> Вся документация на этой странице, за исключением указанных здесь, относится только к маркерам, выпущенным для зарегистрированных API.  Он не применяется к маркерам, выданным для интерфейсов API, принадлежащих корпорации Майкрософт, и не может использоваться для проверки того, как платформа Microsoft Identity будет выдавать маркеры для создаваемого API.  

## <a name="token-formats-and-ownership"></a>Форматы токенов и владение

### <a name="v10-and-v20"></a>v 1.0 и v 2.0 

В платформе Microsoft Identity поддерживается две версии маркеров доступа: v 1.0 и v 2.0.  Эти версии определяют, какие утверждения входят в маркер, гарантируя, что веб-API может управлять тем, как выглядят их маркеры. Веб-API во время регистрации по умолчанию — v 1.0 для приложений, предназначенных только для Azure AD, и версия 2.0 для приложений, поддерживающих учетные записи потребителей.  Это может контролироваться приложениями, использующими `accessTokenAcceptedVersion` параметр в [манифесте приложения](reference-app-manifest.md#manifest-reference), где и, в своюмся случае, приводить к `null` `1` токенам версии 1.0 и `2` приводит к маркерам версии 2.0.

### <a name="what-app-is-a-token-for"></a>Какое приложение является маркером для?

В запросе маркера доступа участвуют две стороны: Клиент, который запрашивает маркер, и ресурс (API), который принимает маркер при вызове API. `aud`Утверждение в маркере Указывает ресурс, для которого предназначен маркер (его *аудитория*). Клиенты используют маркер, но не должны понимать его или пытаться его проанализировать. Ресурсы принимают токен.  

Платформа удостоверений Майкрософт поддерживает выдачу любой версии токена из любой конечной точки версии. они не связаны. Именно поэтому параметр ресурса `accessTokenAcceptedVersion` `2` означает, что клиент, вызывающий конечную точку v 1.0 для получения маркера для этого API, получит маркер доступа версии 2.0.  Ресурсы всегда владеют своими маркерами (с их `aud` утверждением) и являются единственными приложениями, которые могут изменять сведения об их токенах. Именно поэтому изменение [необязательных утверждений](active-directory-optional-claims.md) маркера доступа для *клиента* не приводит к изменению маркера доступа, полученного при запросе маркера `user.read` , который принадлежит ресурсу Microsoft Graph.

### <a name="sample-tokens"></a>Примеры маркеров

Токены версий 1.0 и 2.0 похожи друг на друга и содержат много одинаковых утверждений. Ниже представлены примеры каждого из них. Однако эти маркеры не будут [проверяться](#validating-tokens), так как ключи повернуты перед публикацией, а персональные данные удалены из них.
...
В маркер включаются только те утверждения, для которых существуют значения. Приложение не должно зависеть от утверждения. Примеры: `pwd_exp` (не каждый клиент требует, чтобы срок действия паролей истек) и `family_name` (потоки[учетных данных клиента](v2-oauth2-client-creds-grant-flow.md) находятся от имени приложений, у которых нет имен). Утверждения, используемые для проверки маркера доступа, присутствуют всегда.
...
| Утверждение | Формат | Описание |
|-----|--------|-------------|
| `aud` | Строка, URI идентификатора приложения или GUID | Определяет предполагаемого получателя маркера — его аудитория.  Ваш API должен проверить это значение и отклонить токен, если значение не совпадает. В маркерах версии 2.0 это всегда идентификатор клиента API, а в маркерах версии 1.0 это может быть идентификатор клиента или URI ресурса, используемый в запросе, в зависимости от того, как клиент запросил маркер.|
| `iss` | Строка с URI службы токенов безопасности | Определяет службу маркеров безопасности (STS), которая создает и возвращает маркер, а также клиент Azure AD, в котором пользователь прошел аутентификацию. Если выданный маркер является маркером версии 2.0 (см. утверждение `ver`), URI заканчивается на `/v2.0`. GUID, который указывает, что пользователь является потребителем с учетной записью Майкрософт: `9188040d-6c67-4c5b-b112-36a304b66dad`. Приложение может использовать часть идентификатора, соответствующую идентификатору GUID, чтобы ограничить набор клиентов, которые могут входить в приложение, если это применимо. |
...
| `preferred_username` | Строка | Основное имя пользователя, представляющее пользователя. Это может быть адрес электронной почты, телефонный номер или универсальное имя пользователя без определенного формата. Его значение не является неизменяемым и может меняться с течением времени. Так как это значение является изменяемым, его нельзя использовать для принятия решений об авторизации.  Тем не менее, его можно использовать для указания имени пользователя, а в пользовательском интерфейсе для чтения в качестве имени пользователя. Чтобы получить это утверждение, необходимо указать область `profile`. Имеется только в маркерах версии 2.0. |
...
| `wids` | Массив GUID [RoleTemplateID](../roles/permissions-reference.md#role-template-ids) | Обозначает роли на уровне клиента, назначенные этому пользователю, из раздела ролей, имеющихся в [странице роли администратора](../roles/permissions-reference.md#role-template-ids).  Это утверждение настраивается на уровне приложения с использованием свойства `groupMembershipClaims` [манифеста приложения](reference-app-manifest.md).  Для него необходимо задать значение ALL или DirectoryRole.  Может отсутствовать в маркерах, полученных через неявный поток из-за проблем с длиной маркера. |
...
Чтобы размер токена не превышал ограничения на размер заголовка HTTP, Azure AD ограничивает количество идентификаторов объектов, включаемых в утверждение групп. Если пользователь является членом более групп, чем превышен предел (150 для токенов SAML, 200 для токенов JWT и только 6 при выдаче через неявный поток), то Azure AD не выдает утверждение групп в токене. Вместо этого в маркер включается утверждение избытка, которое указывает приложению выполнить запрос к API Microsoft Graph для получения групп, в которых состоит пользователь.
...
Не все приложения должны проверять маркеры. Только в конкретных сценариях приложения должны проверять маркер:

* [Веб-API](quickstart-configure-app-expose-web-apis.md) должны проверять маркеры доступа, отправленные им клиентом.  Они должны принимать только маркеры, содержащие их `aud` утверждение.
* Конфиденциальные веб-приложения, такие как ASP.NET Core, должны проверять маркеры идентификации, отправленные им через браузер пользователя в гибридном потоке, прежде чем разрешить доступ к данным пользователя или установить сеанс.
...
Для тех, кто хочет понять базовый процесс, предоставляются следующие сведения. ПО промежуточного слоя Azure AD содержит встроенные возможности для проверки маркеров доступа. Вы можете найти пример для выбранного вами языка в нашем наборе [примеров](sample-v2-code.md). Существует также несколько сторонних библиотек с открытым исходным кодом, доступных для проверки JWT. существует по крайней мере один вариант почти для каждой платформы и языка. Дополнительные сведения о библиотеках проверки подлинности Azure AD и примеры кода см. в разделе [библиотеки проверки подлинности](reference-v2-libraries.md).
...
> [!NOTE]
> Мы рекомендуем использовать утверждение `kid` для проверки маркера. Хотя токены версии 1.0 содержат как `x5t` утверждения, так и `kid` маркеры версии 2.0 содержат только `kid` утверждение.

Выполнение проверки подписи выходит за рамки этого документа. Если вам требуется помощь в этом вопросе, воспользуйтесь всевозможными библиотеками с открытым исходным кодом.  Однако платформа Microsoft Identity имеет одно расширение подписывания маркеров для стандартных ключей подписывания.
...
* С помощью утверждения `amr` проверьте, прошел ли пользователь многофакторную проверку подлинности. Это требование нужно предварительно применять при помощи [Условного доступа](../conditional-access/overview.md).
...
* Используйте `roles` для просмотра разрешений, предоставленных субъекту маркера.
...
| Change | Файл cookie на основе пароля | Токен на основе пароля | Файл cookie без использования пароля | Токен без использования пароля | Конфиденциальный маркер клиента |
|---|-----------------------|----------------------|---------------------------|--------------------------|---------------------------|
