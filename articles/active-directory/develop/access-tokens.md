...
| `hasgroups` | логический | Если он присутствует, всегда имеет значение `true`. Это обозначает, что пользователь входит как минимум в одну группу. Используется вместо утверждения `groups` для JWT в неявных потоках предоставления, если утверждение полной группы расширяет фрагмент URI за пределы ограничения длины URL-адреса (в настоящее время 6 или более групп). Указывает, что клиент должен использовать Microsoft Graph API для определения групп пользователя (`https://graph.microsoft.com/v1.0/users/{userID}/getMemberObjects`). |
...
| Администратор отменяет все маркеры обновления для пользователя с [помощью PowerShell](https://docs.microsoft.com/powershell/module/azuread/revoke-azureaduserallrefreshtoken) | Отменен | Отменен |Отменен | Отменен | Отменен |
