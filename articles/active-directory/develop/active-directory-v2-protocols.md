...
ms.locfileid: "82926822"
---
# <a name="oauth-20-and-openid-connect-protocols-on-microsoft-identity-platform"></a>Протоколы OAuth 2,0 и OpenID Connect Connect на платформе Microsoft Identity

Конечная точка платформы Microsoft Identity для идентификации как услуги с протоколами отраслевых стандартов, OpenID Connect Connect (OIDC) и OAuth 2,0. Хотя эта служба соответствует стандартам, между любыми двумя реализациями этих протоколов могут быть незначительные различия. Информация здесь будет полезна, если вы решили написать код путем непосредственной отправки и обработки HTTP-запросов или использования сторонней библиотеки с открытым исходным кодом вместо использования одной из наших [библиотек с открытым исходным](reference-v2-libraries.md)кодом.
...
* **Клиент OAuth** — это ваше приложение, определяемое по идентификатору приложения. Обычно именно с этой стороной взаимодействует конечный пользователь. Клиент OAuth также запрашивает маркеры у сервера авторизации. Владелец ресурса должен предоставить клиенту разрешение на доступ к ресурсу.
...
* **Идентификатор приложения** , который уникальным образом идентифицирует ваше приложение.
