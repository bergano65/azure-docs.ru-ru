...
Когда приложение пытается получить доступ к службе с помощью политики условного доступа, может возникнуть запрос условного доступа. Этот запрос кодируется в `claims` параметр, который поставляется в ответе из Azure AD. Ниже приведен пример этого параметра запроса: 
...
В следующих разделах рассматриваются более сложные сценарии. Основным принципом является условного доступа, которые оцениваются во время запроса маркера для службы, которая применяется политика условного доступа.

## <a name="scenario-app-performing-the-on-behalf-of-flow"></a>Сценарий: приложение, выполняющее поток On-Behalf-Of

В этом сценарии рассматривается вариант, при котором собственное приложение вызывает веб-службу или API. В свою очередь эта служба выполняет поток «on-behalf-of» для вызова подчиненной службы. В нашем случае применяется политика условного доступа к подчиненной службе (веб-API 2) и используется собственное приложение, а не серверное приложение или управляющая программа. 
...
Если приложение использует библиотеку ADAL, при сбое получения маркера всегда осуществляется повторная попытка в интерактивном режиме. Во время такого интерактивного запроса пользователь может выполнить условный доступ. Это верно, если запрос не `AcquireTokenSilentAsync` или `PromptBehavior.Never` в этом случае приложение должно выполнить интерактивный ```AcquireToken``` запрос, чтобы предоставить конечному пользователю возможность обеспечить соответствие политике.
