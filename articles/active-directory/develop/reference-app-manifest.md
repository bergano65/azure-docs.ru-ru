...
| `displayName` | Строка | Отображаемое имя приложения. <br><br> _Примечание. Эта возможность доступна только в регистрации (для прежних версий) взаимодействие с приложениями. Заменены `name` в [регистрация приложений](https://go.microsoft.com/fwlink/?linkid=2083908) столкнуться._ | `"MyRegisteredApp"` |
| `errorUrl` | Строка | Не поддерживается. | |
...
| `homepage` | Строка | URL-адрес домашней страницы приложения. <br><br> _Примечание. Эта возможность доступна только в регистрации (для прежних версий) взаимодействие с приложениями. Заменены `signInUrl` в [регистрация приложений](https://go.microsoft.com/fwlink/?linkid=2083908) столкнуться._ | `"https://MyRegisteredApp"` |
| `objectId` | Строка | Уникальный идентификатор для приложения в каталоге. <br><br> _Примечание. Эта возможность доступна только в регистрации (для прежних версий) взаимодействие с приложениями. Заменены `id` в [регистрация приложений](https://go.microsoft.com/fwlink/?linkid=2083908) столкнуться._ | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
...
| `publisherDomain` | Строка | Домен проверенного издателя приложения. Только для чтения. | https://www.contoso.com |
...
> [!NOTE]
> В случае, если попытаться добавить записи более чем 1200 в манифесте приложения, может появиться сообщение об ошибке **«не удалось обновить xxxxxx приложения. Описание ошибки: Превышен предел для размера манифеста. Уменьшите количество значений и повторите свой запрос.»**
