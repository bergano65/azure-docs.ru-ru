...
| `availableToOtherTenants` | Boolean | значение true, если приложение используется совместно с другими клиентами; в противном случае — значение false. <br><br> _Примечание. Оно заменяется `signInAudience` в [регистрация приложений (Предварительная версия)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) столкнуться._ | |
...
| `displayName` | String | Отображаемое имя приложения. <br><br> _Примечание. Оно заменяется `name` в [регистрация приложений (Предварительная версия)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) столкнуться._ | `"MyRegisteredApp"` |
...
| `homepage` | String | URL-адрес домашней страницы приложения. <br><br> _Примечание. Оно заменяется `signInUrl` в [регистрация приложений (Предварительная версия)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) столкнуться._ | `"https://MyRegisteredApp"` |
| `objectId` | String | Уникальный идентификатор для приложения в каталоге. <br><br> _Примечание. Оно заменяется `id` в [регистрация приложений (Предварительная версия)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) столкнуться._ | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
...
| `publicClient` | Boolean | Указывает, является ли это приложение общедоступным клиентом (например, установленное приложение, работающее на мобильном устройстве). <br><br> _Примечание. Оно заменяется `allowPublicClient` в [регистрация приложений (Предварительная версия)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) столкнуться._ | |
| `publisherDomain` | String | Домен проверенного издателя приложения. Только для чтения. | https://www.contoso.com |
| `replyUrls` | Массив строк | Это свойство с несколькими значениями хранит список зарегистрированных значений redirect_uri, которые Azure AD будет принимать в качестве назначений при возвращении маркеров. <br><br> _Примечание. Оно заменяется `replyUrlsWithType` в [регистрация приложений (Предварительная версия)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) столкнуться._ | |
...
Манифест приложения представляет схему базовой моделью приложения в Azure AD. По мере развития базовая схема редакторе манифеста будет обновляться в соответствии с новой схемой время от времени. Таким образом вы можете заметить новые атрибуты, которые отображаются в манифесте приложения. В редких случаях вы можете заметить изменение синтаксиса или семантики в существующих атрибутов или для них атрибут, который ранее существовал больше не поддерживаются. Например, вы увидите новые атрибуты в [регистрация приложений (Предварительная версия)](https://developer.microsoft.com/graph/blogs/new-app-registration/) которой известны с другим именем в регистрации (GA) взаимодействие с приложениями.


| Регистрация приложений (GA)    | Регистрация приложений (Предварительная версия) |
|---------------------------|-----------------------------|
...
1. Если рабочему процессу требуется сохранить манифесты в исходном репозитории для последующего использования, мы рекомендуем перемещения изменений из сохраненного манифесты в репозитории с одним в **регистрация приложений (Предварительная версия)** столкнуться.
