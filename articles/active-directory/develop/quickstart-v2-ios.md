...
>      - Введите **_идентификатор пакета_* _ своего приложения. Идентификатор пакета — это просто уникальная строка, однозначно идентифицирующая ваше приложение, например `com.<yourname>.identitysample.MSALMacOS`. Запишите значение, которое используете.
>      - Обратите внимание, что конфигурация iOS также применима к приложениям macOS.
> 1. Щелкните элемент `Configure` и сохраните сведения о _*_конфигурации MSAL_*_ для последующего использования в рамках этого краткого руководства.
...
> 1. Измените _ *ViewController.swift** и замените строку, начинающуюся с "let kClientID", приведенным ниже фрагментом кода. Не забудьте обновить значение `kClientID`, указав идентификатор клиента, который вы ранее сохранили при регистрации приложения на портале в этом кратком руководстве:
>    ```swift
