...
В этом кратком руководстве показано, как добавить и зарегистрировать приложение на портале Azure, чтобы, таким образом, его можно было интегрировать с платформой удостоверений Майкрософт.
...
1. В области навигации слева выберите службу **Azure Active Directory**, а затем выберите **App registrations (Preview) (Регистрация приложений (предварительная версия)) и щелкните "Новая регистрация"**.
1. После появления страницы **Регистрация приложения** введите сведения о регистрации приложения:
    ...
    - **Поддерживаемые типы учетных записей** — выберите учетные записи, которые должно поддерживать приложение.
        ...
        |-------------------------|-------------|
        | **Accounts in this organizational directory only** (Учетные записи только в этом каталоге организации) | Выберите этот вариант, если вы создаете бизнес-приложение. Этот параметр недоступен, если вы не регистрируете приложение в каталоге.<br><br>Этот параметр сопоставляется с Azure AD только одного клиента.<br><br>Это параметр по умолчанию, если вы не регистрируете приложение вне каталога. Если приложение зарегистрировано за пределами каталога, по умолчанию используются личные учетные записи Майкрософт и учетные записи с несколькими клиентами Azure AD. |
...
AAD присвоит вашему приложению уникальный идентификатор, а затем откроется страница **Обзор** приложения. Чтобы добавить дополнительные возможности в свое приложение, вы можете выбрать другие параметры конфигурации, включая фирменную символику, сертификаты и секреты, разрешения API и прочее.
