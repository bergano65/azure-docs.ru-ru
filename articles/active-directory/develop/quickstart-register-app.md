...
Сертификаты, иногда называемые *открытым ключом*, — это рекомендуемый тип учетных данных, так как они предоставляют более высокий уровень надежности, чем секрет клиента. Дополнительные сведения об использовании сертификата в качестве метода проверки подлинности в приложении см. в статье [Учетные данные сертификата проверки подлинности приложения платформы удостоверений Майкрософт](active-directory-certificate-credentials.md).
...
**Примечание.** Идентификатор, созданный вместе со значением секрета, является идентификатором секрета и отличается от идентификатора приложения.
