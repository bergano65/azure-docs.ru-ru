...
При создании конфиденциального клиента с секретами клиента файл конфигурации будет выглядеть так, как это делается из примера [confidential_client_secret_sample. копировать # 4-L15](https://github.com/AzureAD/microsoft-authentication-library-for-python/blob/0.6.1/sample/confidential_client_secret_sample.py#L4-L15) в GitHub.
...
При создании конфиденциального клиента с сертификатами он будет выглядеть так, как в примере [confidential_client_certificate_sample. Корректировка уровня 4 – L15](https://github.com/AzureAD/microsoft-authentication-library-for-python/blob/4b34fd660e5574eb876cfba63a1b927ae375efd6/sample/confidential_client_certificate_sample.py#L4-L15) в GitHub.
...
Вы предоставляете CONFIDENTIAL_CLIENT_ID или CONFIDENTIAL_CLIENT_SECRET. Оба параметра являются эксклюзивными.
...
Дополнительные сведения см. в разделе MSAL. Справочная документация по Python для [конфидентиалклиентаппликатион](https://msal-python.readthedocs.io/en/latest/#msal.ClientApplication.__init__)

# <a name="javatabjava"></a>[Java](#tab/java)

msal4j находится в общедоступной предварительной версии. Подписанные утверждения пока не поддерживаются.
