...
По умолчанию MSAL. js использует `sessionStorage` , что не позволяет совместно использовать сеансы вкладок. Чтобы обеспечить единый вход между вкладками, убедитесь, что в `cacheLocation` MSAL. js задано `localStorage` значение, как показано ниже.
...
> [!Note]
> Идентификатор безопасности может использоваться только с необъявляемыми запросами проверки `acquireTokenSilent` подлинности, выполняемыми вызовом в MSAL. js.
...
Если вы не настроили заявку SID или не хотите обойти запрос выбора учетной записи в интерактивных вызовах проверки подлинности `login_hint` , это можно сделать, `domain_hint` предоставив в параметрах `extraQueryParameters` запроса и, при необходимости, как в`loginPopup`интерактивных методах MSAL. js ( `loginRedirect`, `acquireTokenPopup` и `acquireTokenRedirect`). Пример:
...
Передайте `sid` значение, если доступно `login_hint` (или, `domain_hint`при необходимости) параметры запроса в вызов MSAL. `acquireTokenSilent` js следующим образом:
...
Чтобы воспользоваться преимуществами единого входа (SSO) при обновлении из ADAL. js, необходимо убедиться, что библиотеки используются `localStorage` для кэширования маркеров. Присвойте `cacheLocation` параметру значение `localStorage` в конфигурации MSAL. js и ADAL. js при инициализации следующим образом:
