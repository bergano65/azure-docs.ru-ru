...
| Свойство | Тип данных | Обязательно для заполнения | Примечания |
|-----------|------------|-------------|-------|
| `client_id` | String | Да | Идентификатор клиента приложения на [странице регистрации приложения](https://ms.portal.azure.com/#blade/Microsoft_AAD_RegisteredApps/ApplicationsListBlade) |
| `redirect_uri`   | String | Да | URI перенаправления приложения со [страницы регистрации приложения](https://ms.portal.azure.com/#blade/Microsoft_AAD_RegisteredApps/ApplicationsListBlade) |
...
| Свойство | Тип данных  | Обязательно для заполнения | Примечания |
|-----------|-------------|-----------|--------|
| `type` | String | Да | Отражает аудиторию или тип учетной записи, для которой предназначено приложение. Возможные значения: `AAD`, `B2C` |
...
| `authority_url` | String | Да | Требуется, только если Type =`B2C`. Указывает URL-адрес центра или политику, которые должно использовать приложение  |
...
| Свойство | Тип данных  | Обязательно для заполнения | Примечания |
|-----------|-------------|------------|-------|
| `type` | String | Да | Указывает аудиторию, для которой нужно назначить приложение. Возможные значения: `AzureADandPersonalMicrosoftAccount`, `PersonalMicrosoftAccount`, `AzureADMultipleOrgs`, `AzureADMyOrg` |
| `tenant_id` | String | Да | Требуется только при `"type":"AzureADMyOrg"`. Необязательно для других `type` значений. Это может быть домен клиента, например `contoso.com`, или идентификатор клиента, например `72f988bf-86f1-41af-91ab-2d7cd011db46`). |
