---
title: Общие сведения о файле конфигурации библиотеки проверки подлинности Microsoft Android (MSAL) | Службы
...
| Свойство | Тип данных | Обязательное значение | Примечания |
|-----------|------------|-------------|-------|
| `client_id` | Строковое | Да | Идентификатор клиента приложения на [странице регистрации приложения](https://ms.portal.azure.com/#blade/Microsoft_AAD_RegisteredApps/ApplicationsListBlade) |
| `redirect_uri`   | Строковое | Да | URI перенаправления приложения со [страницы регистрации приложения](https://ms.portal.azure.com/#blade/Microsoft_AAD_RegisteredApps/ApplicationsListBlade) |
| `authorities` | List @ no__t-0Authority > | Нет | Список органов, необходимых для приложения |
...
| `logging` | логгингконфигуратион | Нет | Задает уровень детализации журнала. К дополнительным конфигурациям относятся: `pii_enabled`, принимающая логическое значение, и `log_level`, которая принимает `ERROR`, `WARNING`, `INFO` или `VERBOSE`. |
...
|------|------------|------------|----------------|----------------------|---------|
| AAD | азуреадандперсоналмикрософтаккаунт | | | https://login.microsoftonline.com/common | `common` является псевдонимом клиента для учетной записи. Например, определенного клиента Azure Active Directory или учетная запись Майкрософт системы. |
...
> Если MSAL получает запрос маркера на неизвестный центр сертификации, `MsalClientException` типа `UnknownAuthority`.

#### <a name="authority-properties"></a>Свойства центра

| Свойство | Тип данных  | Обязательное значение | Примечания |
|-----------|-------------|-----------|--------|
| `type` | Строковое | Да | Отражает аудиторию или тип учетной записи, для которой предназначено приложение. Возможные значения: `AAD`, `B2C` |
| `audience` | Object | Нет | Применяется только при типе = `AAD`. Указывает удостоверение, для которого предназначено ваше приложение. Использовать значение из регистрации приложения |
| `authority_url` | Строковое | Да | Требуется, только если Type = `B2C`. Указывает URL-адрес центра или политику, которые должно использовать приложение  |
| `default` | boolean | Да | Если указан один или несколько центров, требуется одиночная `"default":true`. |

#### <a name="audience-properties"></a>Свойства аудитории

| Свойство | Тип данных  | Обязательное значение | Примечания |
|-----------|-------------|------------|-------|
| `type` | Строковое | Да | Указывает аудиторию, для которой нужно назначить приложение. Возможные значения: `AzureADandPersonalMicrosoftAccount`, `PersonalMicrosoftAccount`, `AzureADMultipleOrgs`, `AzureADMyOrg` |
| `tenant_id` | Строковое | Да | Требуется только при `"type":"AzureADMyOrg"`. Необязательно для других значений `type`. Это может быть домен клиента, например `contoso.com`, или идентификатор клиента, например `72f988bf-86f1-41af-91ab-2d7cd011db46`). |
...
| `log_level`   | boolean | Нет | Какие сообщения журнала должны выводиться |
