...
В самых распространенных случаях условный доступ не изменяет поведение приложения и не требует внесения изменений разработчиком. В некоторых случаях, когда приложение косвенно или автоматически запрашивает маркер для службы, приложению требуется изменение кода для обработки задач условного доступа.  Это может быть так же просто, как выполнение запроса на интерактивный вход.

В частности, в следующих сценариях требуется код для решения проблем условного доступа:
...
В зависимости от сценария корпоративный клиент может в любое время применять и удалять политики условного доступа. Чтобы приложение продолжало работать при применении новой политики, реализуйте обработку запроса. В следующих примерах показана обработка запроса.
...
* Дополнительные сведения о пакете SDK MSAL и доступе к справочной документации см. в [статье Общие сведения о библиотеке проверки подлинности Майкрософт](msal-overview.md).
