...
Сначала MSAL использует привязанную службу брокера, так как для вызова этой службы не требуются разрешения Android. При сбое привязки к привязанной службе MSAL будет использовать API AccountManager для Android. MSAL делает это только в том случае, `"READ_CONTACTS"` если приложению уже предоставлено разрешение.

Если получен `MsalClientException` код `"BROKER_BIND_FAILURE"`ошибки, есть два варианта:
