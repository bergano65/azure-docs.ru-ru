...
|`scope`        | Обязательный        | Определяет набор разрешений, запрашиваемых приложением. Это могут быть либо статические (с использованием [`/.default`](#the-default-scope) ), либо динамические области.  Это могут быть области OIDC ( `openid` , `profile` , `email` ). Если требуются разрешения приложения, необходимо использовать `/.default` для запроса статически настроенного списка разрешений.  |
