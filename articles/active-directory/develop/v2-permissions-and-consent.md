...
> [!NOTE]
>Разрешения на доступ к [`/.default`](#the-default-scope) приложениям могут запрашиваться только с помощью. Если приложению требуются разрешения приложения, убедитесь, что они перечислены на портале регистрации приложений.
...
|:--------------|:--------------|:-----------------------------------------------------------------------------------------|
| `tenant` | Обязательный | Клиент каталога, из которого необходимо запросить разрешение. Может предоставляться в формате GUID или понятном имени, а также в универсальных ссылках на организации, как показано в примере. Не используйте "Common", так как личные учетные записи не могут предоставить согласие администратора, за исключением контекста клиента. Чтобы обеспечить лучшую совместимость с личными учетными записями, которые управляют клиентами, по возможности используйте идентификатор клиента. |
...
| `redirect_uri` | Обязательный |Универсальный код ресурса (URI) перенаправления, на который нужно направлять ответ для обработки в приложении. Он должен в точности соответствовать одному из универсальных кодов ресурсов (URI) перенаправления, зарегистрированных на портале регистрации приложений. |
...
|`scope`        | Обязательный        | Определяет набор разрешений, запрашиваемых приложением. Это могут быть либо статические ( [`/.default`](#the-default-scope)с использованием), либо динамические области.  Это могут быть области OIDC (`openid`, `profile`, `email`). Если требуются разрешения приложения, необходимо использовать `/.default` для запроса статически настроенного списка разрешений.  |
