---
title: Области платформы удостоверений Microsoft, разрешения и согласие | Документация Майкрософт
description: Описание авторизации в конечной платформы удостоверений Microsoft, включая области, разрешения и согласие.
...
ms.locfileid: "67702704"
---
# <a name="permissions-and-consent-in-the-microsoft-identity-platform-endpoint"></a>Разрешения и согласие для конечной точки платформы удостоверений Microsoft

[!INCLUDE [active-directory-develop-applies-v2](../../../includes/active-directory-develop-applies-v2.md)]

Приложения, которые интегрируются с платформой удостоверений Майкрософт, следуют модели авторизации, при которой пользователи и администраторы контролируют доступ к данным. Реализация модели авторизации была обновлена в конечной точке платформы удостоверений Microsoft и их изменения, механизм взаимодействия приложения с платформой Microsoft identity. В этой статье рассматриваются основные понятия этой модели авторизации, включая области, разрешения и согласие на их предоставление.

> [!NOTE]
> Конечная точка платформы удостоверений Microsoft не поддерживает все сценарии и компоненты. Чтобы определить, следует ли использовать конечную точку платформы Microsoft identity, ознакомьтесь [ограничения платформы удостоверений Microsoft](active-directory-v2-limitations.md).
...
Приложения чаще всего запросов конечной точки авторизации эти разрешения, указав области в запросах к платформе Microsoft identity. Тем не менее, некоторые разрешения с высоким уровнем привилегий можно только предоставлены через согласие администратора и запроса или предоставить с помощью [конечной точки предоставления согласия администратора](v2-permissions-and-consent.md#admin-restricted-permissions). Ознакомьтесь с дополнительными сведениями, чтобы узнать больше.
...
_Действующие разрешения_ — это разрешения, которые есть у приложений при запросе целевого ресурса. Важно понимать разницу между делегированные разрешения приложения, которые предоставлены приложения и действующими разрешениями, при выполнении вызовов к целевому ресурсу.
...
После того как пользователь введет свои учетные данные, конечная точка платформы удостоверений Microsoft проверяет наличие соответствующей записи *согласия пользователя*. Если пользователь не предоставил к любому из запрашиваемых разрешений в прошлом, и не администратор предоставил эти разрешения от имени всей организации, конечная точка платформы удостоверений Microsoft пользователю предлагается предоставить запрошенные разрешения.
...
![Снимок экрана с примером показано согласия работы учетной записи](./media/v2-permissions-and-consent/work_account_consent.png)
...
Хотя пользователь, являющийся потребителем, и может предоставить приложению доступ к таким данным, корпоративным пользователям нельзя предоставлять доступ к таким же конфиденциальным данным компании. Если приложение запрашивает доступ к одному из этих разрешений у корпоративного пользователя, пользователь получает сообщение об ошибке, говорящее, что они не права на согласие на разрешения для вашего приложения.
...
Если приложение запрашивает разрешения приложения и администратор предоставит эти разрешения через администратор конечной точки предоставления согласия, делается это право не от имени конкретного пользователя. а *напрямую* клиентскому приложению. Разрешения такого типа используются только службы управляющих программ и других неинтерактивных приложений, которые выполняются в фоновом режиме.

## <a name="using-the-admin-consent-endpoint"></a>Использование конечной точки предоставления согласия администратора

Когда администратор компании, используя ваше приложение, переходит к конечной точке авторизации, платформа удостоверений Майкрософт определяет роль пользователя и предлагает ему предоставлять согласие на запрошенные вами разрешения от имени всего клиента. Если вы хотите, чтобы администратор предоставлял разрешения от имени всего клиента, можно использовать выделенную конечную точку предоставления согласия администратора. С помощью этой конечной точки также необходимый для запроса разрешения приложения (который не может быть запрошен с помощью конечной точки авторизации).
...
Согласие администратора не принимает параметр области, поэтому любые запрошенные разрешения нужно статически определить при регистрации приложения. Как правило рекомендуется обеспечить надмножество разрешений, динамически/постепенно запрашивать разрешения, статически определенные для данного приложения.

#### <a name="to-configure-the-list-of-statically-requested-permissions-for-an-application"></a>Чтобы настроить список статически запрошенные разрешения для приложения

1. Перейдите к приложению в [портал Azure — регистрация приложений](https://go.microsoft.com/fwlink/?linkid=2083908) возникают, или [Создание приложения](quickstart-register-app.md) Если у вас еще нет.
2. Найдите **разрешения API** , при этом в рамках API разрешений нажмите кнопку Добавить разрешение.
3. Выберите **Microsoft Graph** из списка доступных интерфейсов API и добавьте разрешения, необходимые для приложения.
...
При входе пользователя в приложение можно определить организацию, к которой принадлежит администратор, прежде чем запрашивать у него утверждение необходимых разрешений. Хотя это не обязательно, таким образом можно сделать пользовательский интерфейс более интуитивно понятным для сотрудников организации. Для входа пользователя, выполните наши [учебниках по протоколу платформы удостоверений Microsoft](active-directory-v2-protocols.md).
...
После того как пользователь предоставит разрешения для приложения, оно может получать маркеры доступа, представляющие разрешение приложения на определенный уровень доступа к ресурсу. Маркер доступа можно использовать только для отдельного ресурса, но в этом маркере закодированы все разрешения, предоставленные приложению для данного ресурса. Чтобы получить маркер доступа, приложение может отправить запрос в Microsoft identity платформы конечную точку маркера, следующим образом:
...
Дополнительные сведения о протоколе OAuth 2.0 и способах получения маркеров доступа, см. в разделе [по протоколу конечной точки для платформы Microsoft identity](active-directory-v2-protocols.md).

## <a name="the-default-scope"></a>Область /.default

Можно использовать `/.default` область переноса приложений из конечная точка версии 1.0 в конечную точку платформы Microsoft identity. Это встроенная область для каждого приложения, которая ссылается на статический список разрешений, настроенных в регистрации приложения. Значение `scope` параметра `https://graph.microsoft.com/.default` обеспечивает те же функции, что и параметр`resource=https://graph.microsoft.com` конечных точек версии 1.0: запрашивается маркер с областями в Microsoft Graph, зарегистрированными для приложения на портале Azure.

/.Default области можно использовать в любой поток OAuth 2.0, но необходим в [поток On-Behalf-Of](v2-oauth2-on-behalf-of-flow.md) и [поток учетных данных клиента](v2-oauth2-client-creds-grant-flow.md).  

> [!NOTE]
> Клиенты, не может сочетать статический (`/.default`) и динамического согласия в одном запросе. Поэтому `scope=https://graph.microsoft.com/.default+mail.read` приведет к ошибке из-за объединения типов области.

### <a name="default-and-consent"></a>Область /.default и согласие

Область `/.default` также активирует поведение конечной точки версии 1.0 для `prompt=consent`. Она запрашивает согласие для всех разрешений, зарегистрированных для приложения, вне зависимости от ресурса. Если рассматривается как часть запроса, `/.default` область возвращает маркер, содержащий области для запрошенный ресурс.
...
В результате отображается экран предоставления согласия для всех зарегистрированных разрешений (если применимо в соответствии с приведенным выше описанием согласия и `/.default`), а затем возвращается маркер id_token, а не маркер доступа.  Это поведение для определенных устаревших клиентов, перемещение из ADAL в MSAL существует и не должен использоваться новым клиентам, предназначенных для конечной точки платформы удостоверений Microsoft.  
