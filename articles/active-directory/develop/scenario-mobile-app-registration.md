...
Если приложение использует только проверку подлинности с помощью пароля пользователя, вам не нужно регистрировать универсальный код ресурса (URI) перенаправления для приложения. Этот поток выполняет циклическое обращение к платформе Microsoft Identity. Приложение не будет вызываться обратно по любому конкретному URI.

Тем не менее, приложение следует обозначать как общедоступное клиентское приложение. Для этого сделайте следующее:

1. По-прежнему в <a href="https://portal.azure.com/" target="_blank">портал Azure</a>выберите свое приложение в **Регистрация приложений**, а затем выберите **Проверка подлинности**.
