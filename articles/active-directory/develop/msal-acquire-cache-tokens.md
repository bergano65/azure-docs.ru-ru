---
title: Получение маркеров кэша & с помощью MSAL | Службы
...
Если приложению требуется запросить маркеры с конкретными разрешениями для API ресурсов, необходимо передать области, содержащие URI идентификатора приложения API, в следующем формате: * &lt;область URI&gt;/&lt;идентификатора приложения&gt;*
...
- [поток кода авторизации](msal-authentication-flows.md#authorization-code) в веб-приложениях, так как он активирует код, полученный приложением при входе пользователя, и имеет согласие на дополнительные области. Так как код передается в качестве параметра, а не учетной записи, метод не может выполнять поиск в кэше до активации кода, для которой в любом случае требуется вызов к службе.
