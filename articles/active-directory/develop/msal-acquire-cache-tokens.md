---
title: Приобретайте & токены кэша с помощью MSAL Azure
...
ms.locfileid: "81534590"
---
# <a name="acquire-and-cache-tokens-using-the-microsoft-authentication-library-msal"></a>Приобретение и кэширование токенов с помощью библиотеки проверки подлинности Майкрософт (MSAL)

[Маркеры доступа](access-tokens.md) позволяют клиентам безопасно вызывать веб-API, защищенные платформой Azure. Получить токен с использованием библиотеки аутентификации Майкрософт (MSAL) можно многими способами. Для некоторых способов пользователю требуется выполнить действия в веб-браузере, а для других участие пользователя не требуется. Как правило, способ приобретения токена зависит от того, является ли приложение публичным клиентским приложением (настольное или мобильное приложение) или конфиденциальным клиентским приложением (веб-приложение, веб-aPI или приложение daemon, как служба Windows).
...
Когда приложению необходимо запрашивать токены с определенными разрешениями для API ресурса, необходимо пройти области, содержащие идентификатор приложения URI API в нижем формате: * &lt;&gt;/&lt;область URI идентификатора приложения&gt;*
...
- [поток кода авторизации](msal-authentication-flows.md#authorization-code) в веб-приложениях, так как он высылает код, который приложение получило, войдя в пользователя, и имея их согласие на больше областей. Так как код передается в качестве параметра, а не учетной записи, метод не может выполнять поиск в кэше до активации кода, для которой в любом случае требуется вызов к службе.
...
Для конфиденциальных клиентских приложений (веб-приложения, веб-приложения или приложения daemon, такие как служба Windows), вы:
