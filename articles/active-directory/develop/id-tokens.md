---
title: Справочник по маркерам ИДЕНТИФИКАТОРов платформ Microsoft Identity
...
|`name` | Строка | Утверждение `name` предоставляет удобное для восприятия значение, определяющее субъект маркера. Значение не обязательно должно быть уникальным, оно является изменяемым и предназначено для использования только в целях показа. Для `profile` получения этого утверждения требуется область. |
