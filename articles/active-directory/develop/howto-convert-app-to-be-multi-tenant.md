...
Если у вас есть приложение, которое имеет собственную систему учетных записей или поддерживает вход с помощью других поставщиков облачных служб, добавить вход в Azure AD с любого клиента не составит труда. Просто зарегистрируйте приложение, добавьте код входа через OAuth2, OpenID Connect или SAML и поместить [кнопку «Войти в Майкрософт»][AAD-App-Branding] в приложении.

> [!NOTE]
> В этой статье предполагается, что вы уже знакомы с процессом создания однотенантного приложения для Azure AD. Если вы не являетесь, начните с одним из кратких руководств на [Домашняя страница руководства разработчика][AAD-Dev-Guide].
...
Давайте рассмотрим каждый из этих шагов подробнее. Вы также можете перейти непосредственно к [этот список примеров кода для нескольких клиентов][AAD-Samples-MT].

## <a name="update-registration-to-be-multi-tenant"></a>Обновление регистрации с изменением ее на мультитенантную

По умолчанию регистрация веб-приложения или API в Azure AD предусматривает работу только с одним клиентом, то есть является однотенантной. Вы можете сделать регистрацию мультитенантной, поиск **поддерживаемые типы учетных записей** переключиться **проверки подлинности** области регистрации приложения в [портала Azure][AZURE-portal] и установив значение **учетных записей в любой организации directory**.
...
> [!NOTE]
> Регистрации собственных клиентов в том числе в [приложений платформы удостоверений Microsoft](./active-directory-appmodel-v2-overview.md) являются мультитенантными по умолчанию. Не нужно предпринимать никаких действий, чтобы зарегистрировать приложение как мультитенантное.
...
Когда платформы удостоверений Microsoft получает запрос на "/ Common" конечной точки, он выполняет вход пользователя и, как следствие, выясняет клиента, которые этот пользователь. / Общая конечная точка работает со всеми протоколами проверки подлинности, поддерживаемых Azure AD:  OpenID Connect, OAuth 2.0, SAML 2.0 и WS-Federation.
...
Веб-приложений и веб-API получать и проверять токены платформы Microsoft identity.

> [!NOTE]
> Хотя собственные клиентские приложения запрашивают и получают маркеры из платформой Microsoft identity, они делать для отправки их в интерфейсах API, где они проходят проверку. Собственные приложения не проверяют маркеры и воспринимают их как непрозрачные.

Давайте взглянем на том, как приложение проверяет маркеры, получаемые от платформой Microsoft identity. Однотенантное приложение обычно принимает такое значение конечной точки:
...
Администратор клиента может отключить возможность обычным пользователям предоставлять согласие для приложений. Если эта возможность отключена, то для использования приложения в клиенте всегда требуется согласие администратора. Если вы хотите протестировать приложение с согласия конечного пользователя отключена, можно найти соответствующий параметр в [портала Azure][AZURE-portal] в **[параметры пользователя](https://portal.azure.com/#blade/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/UserSettings/menuId/)** раздела **Корпоративные приложения**.
...
> [!NOTE]
> В некоторых приложениях применяется следующая практика: сначала свое согласие имеют возможность предоставить обычные пользователи, а затем приложение может привлечь администратора и запросить разрешения, для которых требуется его согласие. Нет способа это сделать с помощью Регистрация приложения версии 1.0 в Azure AD на сегодняшний день; Тем не менее использование конечной точки Microsoft identity платформы (версии 2.0) позволяет приложениям запрашивать разрешения во время выполнения, а не во время регистрации, которая поддерживает такие сценарии. Дополнительные сведения см. в разделе [конечная точка платформы удостоверений Microsoft][AAD-V2-Dev-Guide].
...
Это может быть проблемой, если ваше логические приложение состоит из двух или более регистраций приложения, например, отдельно для клиента и ресурса. Как сначала поместить ресурс в клиент пользователя? На этот случай в Azure AD предусмотрена возможность получения согласия для клиента и ресурса за один шаг. Пользователь видит общее число разрешений, запрошенных клиентом и ресурсом на странице получения согласия. Чтобы включить это поведение, Регистрация приложения ресурса должен включать идентификатор клиентского приложения как `knownClientApplications` в его [манифест приложения][AAD-App-Manifest]. Пример:
...
![Иллюстрирует согласия для известного многоуровневого приложения][Consent-Multi-Tier-Known-Client]

#### <a name="multiple-tiers-in-multiple-tenants"></a>Несколько уровней в нескольких клиентов
...
Если это API, созданный организацией, отличными от Майкрософт, разработчик API должен предоставить позволяет клиентам получить согласие на приложение в их клиентом. Рекомендуемая конфигурация предназначена для независимых разработчиков для создания API, таким образом, что он также может работать как веб-клиента для реализации регистрации. Для этого:
...
2. Помимо предоставления области интерфейса API и роли, убедитесь, что регистрация включает в себя «вход и чтение профиля пользователя» разрешение (предоставляется по умолчанию).
...
![Иллюстрирует согласия для многоуровневого приложения c несколькими участниками][Consent-Multi-Tier-Multi-Party]
...
* Пользователи отозвать доступ к отдельным приложениям, удалив их из их [приложения панели доступа][AAD-Access-Panel] списка.
* Администраторы могут отозвать доступ к приложениям, удалив их с помощью [корпоративные приложения](https://portal.azure.com/#blade/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/AllApps) раздел [портала Azure][AZURE-portal].
...
* [Объекты приложения и объектами субъекта-службы][AAD-App-SP-Objects]
* [Интеграция приложений с Azure Active Directory][AAD-Integrating-Apps]
* [Обзор платформы согласия][AAD-Consent-Overview]
* [Области разрешений Microsoft Graph API][MSFT-Graph-permission-scopes]
* [Области разрешений Azure AD Graph API][AAD-Graph-Perm-Scopes]
