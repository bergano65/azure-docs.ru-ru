---
title: Создание приложений, которые регистрировались в пользователях Azure AD
titleSuffix: Microsoft identity platform
description: Показывает, как создать мультитенантное приложение, которое может войти в систему пользователя из любого арендатора Active Directory Azure.
...
Если вы предлагаете приложение по схеме "программное обеспечение как сервис" (SaaS) для многих организаций, вы можете настроить его так, чтобы оно поддерживало вход из любого клиента Azure AD (AD). Эта конфигурация называется *сделать приложение мультитенантным.* Пользователи из любого клиента Azure AD смогут входить в приложение после того, как согласятся использовать свою учетную запись с вашим приложением.

Если у вас есть приложение, которое имеет собственную систему учетных записей или поддерживает вход с помощью других поставщиков облачных служб, добавить вход в Azure AD с любого клиента не составит труда. Просто зарегистрируйте свое приложение, добавьте код для ввода через OAuth2, OpenID Connect или SAML и поместите в приложение [кнопку "Войти в приложение".][AAD-App-Branding]

> [!NOTE]
> В этой статье предполагается, что вы уже знакомы с процессом создания однотенантного приложения для Azure AD. В противном случае ознакомьтесь с [одним из кратких руководств][AAD-Dev-Guide].
...
Давайте рассмотрим каждый из этих шагов подробнее. Вы также можете перейти прямо к примеру [Построить мульти-арендаторS веб-приложение, которое вызывает Microsoft Graph с помощью Azure AD и OpenID Connect.](https://github.com/Azure-Samples/active-directory-aspnetcore-webapp-openidconnect-v2/blob/master/2-WebApp-graph-user/2-3-Multi-Tenant/README.md)

## <a name="update-registration-to-be-multi-tenant"></a>Обновление регистрации с изменением ее на мультитенантную

По умолчанию регистрация веб-приложения или API в Azure AD предусматривает работу только с одним клиентом, то есть является однотенантной. Вы можете сделать регистрацию мультиарендатором, найдя, что **типы поддерживаемых учетных записей** включают панель **аутентификации** [вашего портала приложения на портале Azure][AZURE-portal] и устанавливая его **на учетные записи в любом каталоге организации.**
...
> [!NOTE]
> Регистрации коренных клиентов, а также [приложения платформы идентификации Майкрософт](./active-directory-appmodel-v2-overview.md) по умолчанию являются мультитенантными. Не нужно предпринимать никаких действий, чтобы зарегистрировать приложение как мультитенантное.
...
Когда платформа идентификации Майкрософт получает запрос на /общей конечную точку, она подписывает пользователя и, как следствие, обнаруживает, от какого арендатора пользователь. Конечная точка /общая работа работает со всеми протоколами аутентификации, поддерживаемыми Azure AD: OpenID Connect, OAuth 2.0, SAML 2.0 и WS-Federation.
...
Веб-приложения и веб-AA получают и проверяют токены с платформы идентификации Майкрософт.

> [!NOTE]
> В то время как приложения для натисевших клиентов запрашивают и получают токены от платформы идентификации Майкрософт, они делают это, чтобы отправить их в AI, где они проверяются. Собственные приложения не проверяют маркеры и воспринимают их как непрозрачные.

Давайте посмотрим, как приложение проверяет токены, которые оно получает от платформы идентификации Майкрософт. Однотенантное приложение обычно принимает такое значение конечной точки:
...
Администратор клиента может отключить возможность обычным пользователям предоставлять согласие для приложений. Если эта возможность отключена, то для использования приложения в клиенте всегда требуется согласие администратора. Если вы хотите протестировать приложение с отключенным согласием конечных пользователей, вы можете найти переключатель конфигурации на [портале Azure][AZURE-portal] в разделе **[настроек пользователя](https://portal.azure.com/#blade/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/UserSettings/menuId/)** в **приложениях Enterprise.**
...
> [!NOTE]
> В некоторых приложениях применяется следующая практика: сначала свое согласие имеют возможность предоставить обычные пользователи, а затем приложение может привлечь администратора и запросить разрешения, для которых требуется его согласие. Сегодня в Azure AD это невозможно сделать с регистрацией заявки v1.0; однако использование конечной точки идентификации Майкрософт (v2.0) позволяет приложениям запрашивать разрешения во время выполнения, а не во время регистрации, что позволяет использовать этот сценарий. Для получения дополнительной [Microsoft identity platform endpoint][AAD-V2-Dev-Guide]информации см.
...
![Иллюстрирует согласие на многоуровневое известное клиентское приложение][Consent-Multi-Tier-Known-Client]
...
Если это API, созданный другой организацией, кроме Microsoft, разработчик API должен предоставить своим клиентам возможность дать согласие на предоставление приложения арендаторам своих клиентов. Рекомендуемая конструкция предназначена для стороннего разработчика для создания API таким образом, чтобы он мог также функционировать в качестве веб-клиента для реализации регистрации. Для этого:
...
2. Помимо раскрытия областей/ролей API, убедитесь, что регистрация включает в себя разрешение «Войти и прочитать профиль пользователя» (предоставленное по умолчанию).
...
![Иллюстрирует согласие на многоуровневое многопартийное приложение][Consent-Multi-Tier-Multi-Party]
...
* Администраторы аннулируют доступ к приложениям, удаляя их с помощью раздела [приложений Enterprise](https://portal.azure.com/#blade/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/AllApps) [портала Azure.][AZURE-portal]
...
* [Многотенантная выборка приложений](https://github.com/Azure-Samples/active-directory-aspnetcore-webapp-openidconnect-v2/blob/master/2-WebApp-graph-user/2-3-Multi-Tenant/README.md)
