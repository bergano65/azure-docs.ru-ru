...
Для запуска примера кода, приведенного в этой статье, требуется:
...
В этом разделе описано, как зарегистрировать веб-API на портале **Регистрация приложений**.
...
1. Когда откроется страница **Регистрация приложения**, введите сведения о регистрации приложения.
   ...
   a. В разделе **Имя** введите понятное имя приложения, которое будет отображаться его пользователям. Например, введите **AppModelv2-NativeClient-DotNet-TodoListService**.  
   b. Для параметра **Поддерживаемые типы учетных записей** выберите **Учетные записи в любом каталоге организации**.  
   c. Выберите **Зарегистрировать**, чтобы создать приложение.
...
1. В разделе **Предоставление API** выберите **Добавить группу**, примите предложенный код URI идентификатора приложения (api://{clientId}), выбрав **Сохранить и продолжить**, а затем введите следующие сведения:
   ...
   a. В поле **Имя области** введите **access_as_user**.  
   b. Убедитесь, что для параметра **Кто может давать согласие** выбран вариант **Admins and users** (Администраторы и пользователи).  
   c. В поле **Отображаемое имя согласия администратора** введите **Access TodoListService as a user** (Доступ к TodoListService в качестве пользователя).  
   d. В поле **Описание согласия администратора** введите **Accesses the TodoListService web API as a user** (Доступ к веб-API TodoListService в качестве пользователя).  
   д) В поле **Отображаемое имя согласия пользователя** введите **Access TodoListService as a user** (Доступ к TodoListService в качестве пользователя).  
   е) В поле **Описание согласия пользователя** введите **Accesses the TodoListService web API as a user** (Доступ к веб-API TodoListService в качестве пользователя).  
   ж. Для параметра **Состояние** оставьте значение **Включено**.  
   h. Выберите **Добавить область**.
...
1. Когда откроется страница **Регистрация приложения**, введите сведения о регистрации приложения.
   ...
   a. В разделе **Имя** введите понятное имя приложения, которое будет отображаться пользователям приложения (например, **NativeClient-DotNet-TodoListClient**).  
   b. Для параметра **Поддерживаемые типы учетных записей** выберите **Учетные записи в любом каталоге организации**.  
   c. Выберите **Зарегистрировать**, чтобы создать приложение.
...
1. На странице **Обзор** выберите **Проверка подлинности** и выполните следующие действия.
   ...
   a. В разделе **Конфигурации платформ** нажмите кнопку **Добавить платформу**.  
   b. Для параметра **Мобильные и классические приложения** выберите значение **Мобильные и классические приложения**.  
   c. Для параметра **URI перенаправления** установите флажок **https://login.microsoftonline.com/common/oauth2/nativeclient** .  
   d. Нажмите кнопку **Настроить**.   
1. Выберите **Разрешения API**, а затем выполните следующие действия.

   a. Нажмите кнопку **Add a permission** (Добавить разрешение).  
   b. Выберите вкладку **Мои API**.  
   c. В списке интерфейсов API выберите **ppModelv2-NativeClient-DotNet-TodoListService API** или имя, введенное для веб-API.  
   d. Установите флажок для разрешения **access_as_user**, если он еще не установлен. При необходимости используйте поле поиска.  
   д) Нажмите кнопку **Add permissions** (Добавить разрешения).
