---
title: Настройка единого входа для macOS и iOS | Платформа Microsoft Identity
...
ms.locfileid: "71268938"
---
# <a name="how-to-configure-sso-on-macos-and-ios"></a>Практическое руководство. Настройка единого входа для macOS и iOS
...
URI перенаправления APP1: `msauth.com.contoso.mytestapp1://auth`URI перенаправления для в. `msauth.com.contoso.mytestapp2://auth`URI перенаправления App3:`msauth.com.contoso.mytestapp3://auth`
...
После правильной настройки прав `entitlements.plist` вы увидите в каталоге проекта файл, который содержит примерно следующий пример:
...
1. Зарегистрируйте формат URI перенаправления, совместимого с брокером, для приложения в файле info. plist приложения. Формат URI для перенаправления, совместимый с брокером: `msauth.<app.bundle.id>://auth`. Замените "< App. пучок. ID >" "ИДЕНТИФИКАТОРом пакета вашего приложения. Пример:
...
1. Добавьте следующий `AppDelegate.m` объект в файл для управления обратными вызовами:
...
Если поддерживаются Уисценеделегате и Уиаппликатионделегате для совместимости с более старыми версиями iOS, то обратный вызов MSAL потребуется поместить в оба файла.
