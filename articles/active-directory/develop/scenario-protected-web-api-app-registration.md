...
1. Убедитесь, `id` что это уникальный идентификатор GUID.
1. Убедитесь, `displayName` `value` что не содержат пробелы.
...
1. Если вы хотите, чтобы Azure AD разрешит доступ к веб-API только определенным клиентам, задайте для параметра **Назначение пользователей** значение **Да**.
   ...
   > [!IMPORTANT]
   >
   > Если вы настроили **Назначение пользователя?** в качестве значения **Да**, Azure AD проверяет назначение роли приложения клиента при запросе маркера доступа к веб-API. Если клиент не назначен ни одной роли приложения, Azure AD возвратит сообщение об ошибке "invalid_client: \<AADSTS501051: имя\> приложения приложения не назначено роли для \<веб-API\>".
   >
