---
title: Защищенный веб-API — регистрация приложений
...
Общие шаги регистрации приложения см. в разделе [Краткое руководство. Регистрация приложения на платформе удостоверений Майкрософт](quickstart-register-app.md) .
...
Области обычно имеют форму `resourceURI/scopeName`. Для Microsoft Graph области имеют такие ярлыки, как `User.Read`. Эта строка является ярлыком для `https://graph.microsoft.com/user.read`.
...
1. Измените манифест, найдя параметр `appRoles` и добавив одну или несколько ролей приложения. Определение роли представлено в следующем образце блока JSON. Оставьте `allowedMemberTypes` установленным только `"Application"`. Убедитесь, что `id` является уникальным идентификатором GUID, а `displayName` и `value` не содержат пробелов.
