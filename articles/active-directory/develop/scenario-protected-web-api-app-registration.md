...
Конечная точка платформы идентификации Майкрософт может выдавать маркеры версии 1.0 и токены версии 2.0. Дополнительные сведения об этих токенах см. в разделе [маркеры доступа](access-tokens.md).

Принятая версия токена зависит от значения **поддерживаемых типов учетных записей** , выбранных при создании приложения.

- Если значения **поддерживаемых типов учетных записей** являются **учетными записями в любом каталоге Организации и личных учетных записях Майкрософт (например, Skype, Xbox, Outlook.com)** , принятая версия токена — v 2.0.
- В противном случае принятая версия токена — v 1.0.
...
1. Найдите свойство **акцесстокенакцептедверсион** в манифесте. Значение свойства по умолчанию — 2.
...
> [!NOTE]
> Веб-API указывает, какую версию токена он принимает. Когда клиент запрашивает маркер для веб-API из конечной точки Microsoft Identity Platform (v 2.0), клиент получает маркер, указывающий, какая версия токена принимает веб-API.
...
Другими параметрами, специфичными для веб-API, являются предоставляемые API и предоставленные области.
...
По умолчанию портал регистрации приложений рекомендует использовать универсальный код ресурса (URI) `api://{clientId}`. Этот URI уникален, но не читается человеком. При изменении универсального кода ресурса убедитесь, что новое значение уникально.
...
Области также отображаются в окне согласия, которое отображается для пользователей приложения. Поэтому необходимо предоставить соответствующие строки, описывающие область:
...
1. Чтобы изменить манифест, найдите параметр `appRoles` и добавьте роли приложения. Определения ролей приведены в следующем примере блока JSON.
1. Оставьте `allowedMemberTypes` установленным только для `"Application"`.
1. Убедитесь, что `id` является уникальным идентификатором GUID.
...
1. Если вы хотите, чтобы Azure AD разрешит доступ к веб-API только определенным клиентам, задайте для параметра **Назначение пользователей** значение **Да**.
   ...
   > [!IMPORTANT]
   >
   > Если вы настроили **Назначение пользователя?** в качестве значения **Да**, Azure AD проверяет назначение роли приложения клиента при запросе маркера доступа к веб-API. Если клиент не назначен ни одной роли приложения, Azure AD возвратит сообщение об ошибке "invalid_client: AADSTS501051: приложение \<имя приложения\> не назначено роли для \<веб-API\>".
   >
