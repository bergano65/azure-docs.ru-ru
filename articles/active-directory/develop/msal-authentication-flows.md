...
- Если вы пишете приложение ASP.NET или ASP.NET Core, это может произойти, если вы не сообщитее платформе, что вы уже активировали код авторизации. Для этого необходимо вызвать `context.HandleCodeRedemption()` метод `AuthorizationCodeReceived` обработчика событий.
...
Поток IWA включен для приложений .NET для настольных систем, .NET Core и универсальной платформы Windows. В .NET Core доступно только перегрузка, принимающая имя пользователя. Платформе .NET Core не удается запросить имя пользователя для операционной системы.
