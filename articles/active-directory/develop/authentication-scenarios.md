...
В этой статье рассматриваются многие основные понятия проверки подлинности, которые необходимо знать для создания защищенных веб-приложений, веб-API или приложений, вызывающих защищенные веб-API. Если вы видите термин, который вы не знакомы с, попробуйте наш [Глоссарий](developer-glossary.md).
...
Маркеры безопасности содержат сведения о пользователях и приложениях. Azure AD использует токены на основе JSON (JWT), которые содержат утверждения.

Утверждение предоставляет утверждения об одной сущности, например [клиентском приложении](https://docs.microsoft.com/azure/active-directory/develop/developer-glossary#client-application) или [владельцу ресурса](https://docs.microsoft.com/azure/active-directory/develop/developer-glossary#resource-owner), к другой сущности, такой как [сервер ресурсов](https://docs.microsoft.com/azure/active-directory/develop/developer-glossary#resource-server).

Утверждения — это пары "имя-значение", которые ретранслировать факты о субъекте маркера. Например, утверждение может содержать факты о субъекте безопасности, проверенном [сервером авторизации](https://docs.microsoft.com/azure/active-directory/develop/developer-glossary#authorization-server). Утверждения, представленные в заданном маркере, зависят от многих вещей, в том числе от типа маркера, типа учетных данных, используемых для проверки подлинности субъекта, конфигурации приложения и т. д.
...
После регистрации приложению будет предоставлен уникальный идентификатор, который приложение использует совместно с Azure AD при запросе токенов. Если приложение является [конфиденциальным клиентским приложением](https://docs.microsoft.com/azure/active-directory/develop/developer-glossary#client-application), оно также будет совместно использовать секрет или открытый ключ *-в зависимости от того, использовались ли сертификаты или секреты.
...
В платформе Microsoft Identity платформа [объект приложения](https://docs.microsoft.com/azure/active-directory/develop/developer-glossary#application-object) описывает приложение. Во время развертывания платформа Microsoft Identity использует объект приложения в качестве схемы для создания [субъекта-службы](https://docs.microsoft.com/azure/active-directory/develop/developer-glossary#service-principal-object), который представляет конкретный экземпляр приложения в каталоге или клиенте. Субъект-служба определяет, что приложение может реально выполнять в определенном целевом каталоге, кто может его использовать, какие ресурсы он имеет доступ и т. д. Платформа Microsoft Identity создает субъект-службу из объекта приложения с помощью **согласия**.
...
MSAL использует браузер для получения маркеров. Как и в случае с веб-приложениями, проверка подлинности делегирована в Azure AD.
