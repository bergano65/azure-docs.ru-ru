...
|Поток | Необходима | id_token | токен доступа | маркер обновления | Код авторизации | 
|-----|----------|----------|--------------|---------------|--------------------|
|[Поток кода авторизации](v2-oauth2-auth-code-flow.md) | | п | п | п | п|  
|[Неявный поток](v2-oauth2-implicit-grant-flow.md) | | п        | п    |      |                    |
|[Гибридный поток OIDC](v2-protocols-oidc.md#get-access-tokens)| | п  | |          |            п   |
|[Активация токена обновления](v2-oauth2-auth-code-flow.md#refresh-the-access-token) | маркер обновления | п | п | п| |
|[Поток On-Behalf-Of](v2-oauth2-on-behalf-of-flow.md) | токен доступа| п| п| п| |
|[Поток кода устройства](v2-oauth2-device-code.md) | | п| п| п| |
...
| --- | --- |
| ИД приложения | Идентифицирует приложение, которое использует маркер. |
...
| Способ проверки подлинности | Указывает способ проверки подлинности субъекта маркера (с помощью пароля, сертификата и т. д.). |
...
| Название | Предоставляет удобное для восприятия значение, которое идентифицирует субъект маркера. |
| ИД объекта | Содержит неизменяемый, уникальный идентификатор субъекта в Azure AD. |
| Роли | Содержит простые имена ролей приложения в Azure AD, которые предоставлены пользователю. |
| `Scope` | Указывает разрешения, предоставленные клиентскому приложению. |
| Subject | Указывает участника, сведения о котором утверждает маркер. |
