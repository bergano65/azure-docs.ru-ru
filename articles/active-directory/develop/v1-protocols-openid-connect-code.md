...
После авторизации `code` и `id_token` вы можете выполнить вход и получить [маркеры доступа](access-tokens.md) от имени пользователя. Для входа пользователя необходимо проверить маркер `id_token` , как описано выше. Чтобы получить маркеры доступа, необходимо выполнить действия, описанные в разделе "Использование кода авторизации для запроса маркера доступа" [документации по последовательности кода OAuth](v1-protocols-oauth-code.md#use-the-authorization-code-to-request-an-access-token).
