...
```
Метаданные — это простой документ JSON. В качестве примера ниже приведен фрагмент кода. Его содержимое полностью описано в [спецификации OpenID Connect](https://openid.net). Учтите, что предоставление клиента, отличного от `common`, вместо {tenant} приведет к получению URI определенных клиентов в возвращаемом объекте JSON.
