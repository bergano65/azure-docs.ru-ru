...
Вы определяете роли приложения с помощью [портал Azure](https://portal.azure.com). Когда пользователь входит в приложение, Azure AD выдает `roles` заявку для каждой роли, которую пользователь предоставил пользователю и в отношении членства в группе.
...
1. В области **Создание роли приложения** введите параметры роли. В таблице, следующей за изображением, описывается каждый параметр и их параметры.
    ...
    | **Вы хотите включить эту роль приложения?** | Указывает, включена ли роль приложения. Чтобы удалить роль приложения, снимите этот флажок и примените изменения, прежде чем пытаться выполнить операцию удаления. | *Отмечен* |
