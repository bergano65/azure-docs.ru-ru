---
title: Добавляйте роли приложения и получайте их из маркера Azure
...
1. В списке **избранное** или **все каталоги** выберите арендатора Active Directory, где вы хотите зарегистрировать свою заявку.
...
1. Выберите приложение, в котором требуется определить роли приложения. Затем выберите **Manifest**.
1. Измените манифест приложения. Для этого найдите параметр `appRoles` и добавьте все имеющиеся роли приложения.

     > [!NOTE]
     > Каждое определение роли приложения в этом манифесте должно иметь различные `id` действительные GUID в контексте манифеста для свойства.
     >
     > Свойство `value` определения роли каждого приложения должно точно соответствовать строкам, которые используются в коде в приложении. Свойство `value` не может содержать пробелы. Если это произойдет, вы получите ошибку, когда вы сохраните манифест.
...
> [!NOTE]
>Не `displayName` может содержать пробелы.
...
Количество определенных ролей влияет на пределы, которые имеет манифест приложения. Они подробно обсуждались на странице [манифеста пределов.](https://docs.microsoft.com/azure/active-directory/develop/reference-app-manifest#manifest-limits)
...
1. Выберите **панель пользователей и групп** в меню левой навигации приложения.
