...
description: Введение в новые возможности регистрации приложений портал Azure
...
Формат URI перенаправления изменился. Они должны быть связаны с типом приложения — Web или public. По соображениям безопасности подстановочные знаки `http://` и схемы не поддерживаются, *http://localhost*за исключением.
...
- Страница **проверки подлинности** имеет **неявный поток Grant** (`oauth2AllowImplicitFlow`). В отличие от устаревшей среды, можно включить **маркеры доступа** или **маркеры идентификации**или и то, и другое.
- Страница " **предоставление интерфейса API** " содержит **области, определенные этим API** (`oauth2Permissions`) и **полномочными клиентскими приложениями** (`preAuthorizedApplications`). Дополнительные сведения о настройке приложения в качестве веб-API и предоставлении разрешений и областей см. в разделе [Краткое руководство. Настройка приложения для предоставления веб-API](quickstart-configure-app-expose-web-apis.md).
