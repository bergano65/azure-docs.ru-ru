...
Утверждения, предоставляемые для объекта Account, всегда являются утверждениями от/{аусорити} "домашний клиент" для учетной записи. Если эта учетная запись не использовалась для запроса маркера для своего домашнего клиента, MSAL не может предоставить утверждения через объект Account.  Например:
