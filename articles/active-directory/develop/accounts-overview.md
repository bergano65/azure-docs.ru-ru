...
---
# <a name="accounts--tenant-profiles-android"></a>Учетные записи & Профили клиентов (Android)

В этой статье приводятся общие сведения о том, что `account` находится на платформе Microsoft Identity.
...
- Платформа Microsoft Identity позволяет использовать одну учетную запись для доступа к ресурсам, принадлежащим нескольким организациям (Azure Active Directory клиентов).
  ...
  - MSAL предоставляет эту локальную запись как `Tenant Profile`.
...
- Сведения о Tom и Bob в других клиентах могут отличаться от данных в системе записи. Они могут различаться по атрибутам, таким как должность, расположение офиса и т. д. Они могут быть членами групп и (или) ролей в каждой организации (Azure Active Directory клиенте). Мы будем называть эти сведения как профиль клиента bob@contoso.com.
...
Маркер идентификации содержит список утверждений. `Claims` — пары "имя-значение" для учетной записи и используются для выполнения запроса.
...
> [!TIP]
> Чтобы включить дополнительные утверждения в id_token, обратитесь к документации по дополнительным утверждениям в [How: Предоставление необязательных утверждений для приложения Azure AD @ no__t-0
