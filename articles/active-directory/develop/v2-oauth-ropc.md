---
title: Использование платформы удостоверений для входа пользователей с помощью предоставления учетных данных (ROPC) пароль владельца ресурса | Azure
...
Поддерживает платформа Microsoft identity [предоставить учетные данные пароля владельца ресурса (ROPC)](https://tools.ietf.org/html/rfc6749#section-4.3), который позволяет приложению вход пользователя непосредственно обрабатывая свой пароль. ROPC потока требуется высокий уровень доверия и пользователя экспозиции, и этот поток следует использовать только в случае, когда потоки других, более безопасные, нельзя использовать.
...
| `scope` | Рекомендуется | Разделенный пробелами список [областей](v2-permissions-and-consent.md) или разрешений, которые нужны приложению. В интерактивный поток администратор или пользователь должен дать согласие для этих областей, заранее. |

### <a name="successful-authentication-response"></a>Успешный ответ аутентификации

В следующем примере показано успешный ответ маркера:
