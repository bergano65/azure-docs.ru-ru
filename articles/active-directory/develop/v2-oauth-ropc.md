...
| --- | --- | --- |
| `tenant` | Обязательное значение | Клиент каталога, в который пользователь выполняет вход. Его можно указать в виде GUID или понятного имени. Этот параметр не может иметь значение `common` или `consumers`, но может быть равен `organizations`. |
| `client_id` | Обязательное значение | Идентификатор приложения (клиента), которому назначена страница [портал Azure регистрация приложений](https://go.microsoft.com/fwlink/?linkid=2083908) приложения. | 
| `grant_type` | Обязательное значение | Нужно задать значение `password`. |
| `username` | Обязательное значение | Адрес электронной почты пользователя. |
| `password` | Обязательное значение | Пароль пользователя. |
...
| --------- | ------ | ----------- |
| `token_type` | Строковое | Всегда имеет значение `Bearer`. |
...
| `invalid_request` | Запрос был неправильно сформирован | Тип предоставления не поддерживается в `/common` контекстах проверки подлинности или. `/consumers`  Вместо `/organizations` этого используйте или идентификатор клиента. |
