...
| 401 — недостаточно прав | unknown_source | Неизвестный исходный * \<код ресурса\>* | Убедитесь, что универсальный код ресурса (URI) запроса HTTP GET имеет правильный формат. Часть `scheme:host/resource-path` должна быть указана как `http://localhost:50342/oauth2/token`. В качестве примера см. "Пример запроса" в предыдущем разделе для REST.|
