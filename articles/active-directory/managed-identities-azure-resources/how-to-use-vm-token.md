---
title: Используйте управляемые идентификаторы на виртуальной машине для получения токена доступа - Azure AD
...
| `mi_res_id` | (Необязательно) Параметр строки запроса, указывающий на mi_res_id (Идентификатор ресурсов Azure) управляемого идентификатора, для которому вы хотите маркер. Требуется, если виртуальная машина имеет несколько управляемых удостоверений, назначаемых пользователем. |
...
| error_description | Подробное описание ошибки. **Описания ошибок могут меняться в любое время. Не пишите код, который ветвится на основе значений в описании ошибки.**|
...
|           | unauthorized_client | Клиент не авторизован для запроса маркера доступа с помощью этого метода. | Вызвано запросом, в котором не используется локальная задняя часть для вызова расширения, или на VM, который не управлял идентификаторами для ресурсов Azure, настроенных правильно. Если вам нужна помощь с конфигурацией виртуальной машины, ознакомьтесь с разделом [Настройка управляемых удостоверений для ресурсов Azure на виртуальной машине с помощью портала Azure](qs-configure-portal-windows-vm.md). |
...
| **Стратегия повторов** | **Параметры** | **Значения** | **Как это работает** |
| --- | --- | --- | --- |
