...
3. В окне терминала выполните запрос к локальным управляемым удостоверениям для конечной точки ресурсов Azure с помощью CURL, чтобы получить маркер доступа для Azure Key Vault.  
 
