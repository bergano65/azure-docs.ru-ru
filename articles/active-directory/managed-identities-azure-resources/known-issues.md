...
Нет, управляемые удостоверения для ресурсов Azure еще не интегрированы с библиотеками ADAL и MSAL. Подробнее о приобретении токена для управляемых идентификаторов для ресурсов Azure с помощью конечных точек REST можно [узнать, как использовать управляемые идентификаторы для ресурсов Azure в VM Azure для получения токена доступа.](how-to-use-vm-token.md)
...
### <a name="should-i-use-the-managed-identities-for-azure-resources-imds-endpoint-or-the-vm-extension-endpoint"></a>Должен ли я использовать управляемые идентификаторы для конечных точек IMDS ресурсов Azure или конечную точку расширения VM?

При использовании управляемых идентификаторов для ресурсов Azure с помощью VMs рекомендуется использовать конечную точку IMDS. Служба метаданных экземпляров Azure является конечной точкой REST, доступной всем виртуальным машинам IaaS, созданным с помощью Azure Resource Manager. 
...
- 1000 управляемых идентификаторов, назначенных пользователем, могут быть назначены одному VM. 

Управляемые идентификаторы для расширения ресурсов Azure VM по-прежнему доступны; однако, мы больше не разрабатываем новую функциональность на нем. Мы рекомендуем переключиться на использование конечной точки IMDS. 

Некоторые из ограничений использования конечной точки расширения VM:
- Ограниченная поддержка дистрибутивов Linux: CoreOS Stable, CentOS 7.1, Red Hat 7.2, Ubuntu 15.04, Ubuntu 16.04
- Только 32 управляемых пользователя идентификаторы могут быть назначены VM.


Примечание: Управляемые идентификаторы для расширения ресурсов Azure VM не будут поддерживаться в январе 2019 года. 
...
### <a name="what-azure-rbac-permissions-are-required-to-managed-identity-on-a-resource"></a>Какие разрешения Azure RBAC необходимы для управления идентаторами на ресурсе? 

- Система назначенных управляемых идентаторов: Вам нужно написать разрешения на ресурс. Например, для виртуальных машин необходимо разрешение Microsoft.Compute/virtualMachines/write. Это действие включено в встроенные роли, такие как [Virtual Machine Contributor.](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#virtual-machine-contributor)
- Назначенные пользователем управляемые идентаторы: Вам нужно написать разрешения на ресурс. Например, для виртуальных машин необходимо разрешение Microsoft.Compute/virtualMachines/write. В дополнение к назначению роли [оператора управляемой идентификации](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#managed-identity-operator) над управляемой идентификацией.
...
Обход для управляемых идентификационных данных в подписке, которая была перемещена в другой каталог:

 - Управляемые удостоверения, назначаемые системой, нужно отключить и снова включить. 
 - Управляемые удостоверения, назначаемые пользователем, нужно удалить, повторно создать и присоединить к необходимым ресурсам (например, виртуальным машинам).

### <a name="moving-a-user-assigned-managed-identity-to-a-different-resource-groupsubscription"></a>Перемещение управляемого удостоверения пользователя в другую группу ресурсов/подписку

Перемещение управляемого удостоверения, назначаемого пользователем, в другую группу ресурсов приведет к повреждению удостоверения. В результате ресурсы (например, VM), использующие эту идентификацию, не смогут запрашивать для него токены. 
