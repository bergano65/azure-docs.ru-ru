...
Нет, управляемые удостоверения для ресурсов Azure еще не интегрированы с библиотеками ADAL и MSAL. Дополнительные сведения о получении маркера для управляемых удостоверений для ресурсов Azure с помощью конечной точки REST, см. в разделе [способы использования управляемых удостоверений для ресурсов Azure на виртуальной Машине Azure для получения маркеров доступа](how-to-use-vm-token.md).
...
### <a name="should-i-use-the-managed-identities-for-azure-resources-imds-endpoint-or-the-vm-extension-endpoint"></a>Использовать управляемые удостоверения для конечной точки IMDS ресурсов Azure или конечную точку расширения виртуальной Машины?
...
- 1000 назначаемого пользователем управляемого удостоверения могут быть назначены одной виртуальной Машины. 

Управляемые удостоверения для расширения виртуальной Машины Azure ресурсы по-прежнему доступна; Тем не менее мы больше не разрабатываете новые функциональные возможности на нем. Рекомендуется переключиться на использование конечной точки IMDS. 

Ниже приведены некоторые ограничения использования конечной точки расширения виртуальной Машины.
- Ограниченная поддержка для дистрибутивов Linux: Стабильная версия CoreOS, CentOS 7.1, 7.2 с Red Hat, Ubuntu 15.04, Ubuntu 16.04
- К виртуальной Машине можно назначить только 32 назначаемого пользователем управляемого удостоверения.


Примечание. Управляемые удостоверения для расширения виртуальной Машины Azure ресурсы поддержка будет прекращена в января 2019. 
...
№ Если вы переместили подписку в другой каталог, нужно вручную воссоздать эти удостоверения и снова назначить роли Azure RBAC.
...
- Назначенный системой управляемого удостоверения: Вам требуются разрешения на запись ресурса. Для exampl для виртуальных машин необходимо Microsoft.Compute/virtualMachines/write. Это действие включено в ресурсе, определенные встроенные роли, такие как [участник виртуальных машин](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#virtual-machine-contributor).
- Назначаемое пользователем управляемое удостоверение: Вам требуются разрешения на запись ресурса. Например для виртуальных машин необходимо Microsoft.Compute/virtualMachines/write. В дополнение к [оператор управляемого удостоверения](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#managed-identity-operator) назначение роли за управляемым удостоверением.
...
Обходное решение для управляемых удостоверений в подписке, которая была перенесена в другой каталог.

 - Управляемые удостоверения, назначаемые системой, нужно отключить и снова включить. 
 - Управляемые удостоверения, назначаемые пользователем, нужно удалить, повторно создать и присоединить к необходимым ресурсам (например, виртуальным машинам).

### <a name="moving-a-user-assigned-managed-identity-to-a-different-resource-groupsubscription"></a>Переход на другой ресурс группы или подписки назначаемого пользователем управляемого удостоверения

Перемещение управляемого удостоверения, назначаемого пользователем, в другую группу ресурсов приведет к повреждению удостоверения. Таким образом ресурсы (например, виртуальной Машины), через этот идентификатор будет невозможно для запроса маркеров для него. 
