---
title: Настройка управляемых идентификаторов на наборе виртуального масштаба машины - Azure CLI - Azure AD
description: Пошаговые инструкции по настройке системы и назначенных пользователем управляемых идентификаторов в наборе виртуальной машины Azure с использованием Azure CLI.
...
В этой статье вы узнаете, как выполнять следующие управляемые идентификаторы для операций ресурсов Azure в наборе виртуальной машины Azure с помощью Azure CLI:
...
В этом разделе вы узнаете, как включить и отключить установленную системой управляемую идентификацию для виртуального набора машин Azure с использованием Azure CLI.
...
2. Чтобы сохранить и развернуть масштабируемый набор виртуальных машин и связанные с ним ресурсы, создайте [группу ресурсов](../../azure-resource-manager/management/overview.md#terminology) с помощью команды [az group create](/cli/azure/group/#az-group-create). Если вы уже создали группу ресурсов, которую можно использовать, этот шаг можно пропустить:
...
Этот раздел рассказывает о создании виртуального набора масштабов машины и назначении управляемого удостоверения, назначенного пользователем, в набор виртуальной шкалы машин. Если у вас уже есть набор виртуальной шкалы машины, который вы хотите использовать, пропустите этот раздел и перейдите к следующему.
...
3. Создайте виртуальный набор масштабов машины с помощью [az vmss создать.](/cli/azure/vmss/#az-vmss-create) Следующий пример создает набор виртуальной шкалы машины, связанный с новым `--assign-identity` управляемым имитающим удостоверением, назначенным пользователем, как указано в параметре. Не забудьте заменить значения параметров `<RESOURCE GROUP>`, `<VMSS NAME>`, `<USER NAME>`, `<PASSWORD>` и `<USER ASSIGNED IDENTITY>` своими значениями. 
...
2. Назначьте назначенный пользователем управляемый итог вашему набору виртуальной шкалы машины с помощью [набора идентификационных данных az vmss.](/cli/azure/vmss/identity) Не забудьте заменить значения параметров `<RESOURCE GROUP>` и `<VIRTUAL MACHINE SCALE SET NAME>` собственными. `<USER ASSIGNED IDENTITY>` — свойство ресурса `name` удостоверения, назначаемого пользователем, созданное на предыдущем шаге.
