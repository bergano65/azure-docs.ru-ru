...
1. В разделе **Домены и URL-адреса приложения SpaceIQ** сделайте следующее:
    ...
    2. В текстовом поле **URL-адрес ответа** введите URL-адрес в следующем формате: `https://api.spaceiq.com/saml/<instanceid>/callback`.
...
1. В диалоговом окне **единого входа** сделайте следующее.
    ...
    2. Скопируйте значение **SAML CallBack Endpoint URL (read-only)** (URL-адрес конечной точки обратного вызова SAML (только для чтения)) и вставьте его в поле **URL-адрес ответа** на портале Azure в разделе **Домены и URL-адреса приложения SpaceIQ**.
