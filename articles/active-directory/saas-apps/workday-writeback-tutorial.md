...
1. В раскрывающемся списке **область** выберите **синхронизировать все пользователи и группы**. С помощью этого параметра приложение обратной записи будет записывать сопоставленные атрибуты всех пользователей из Azure AD в Workday в соответствии с правилами области, определенными в разделе **сопоставления**  ->  **область исходного объекта**. 

   > [!div class="mx-imgBorder"]
   > ![Выбрать область обратной записи](./media/sap-successfactors-inbound-provisioning/select-writeback-scope.png)

   > [!NOTE]
   > Приложение подготовки обратной записи Workday не поддерживает параметр **синхронизировать только назначенные пользователи и группы**.
...
4. В любое время проверьте вкладку **журналы подготовки** в портал Azure, чтобы узнать, какие действия выполняла служба подготовки. В журналах аудита перечислены все индивидуальные события синхронизации, выполняемые службой подготовки, например, какие пользователи импортируются из источника и экспортируются в целевое приложение.  
...
* Приложение обратной записи использует предварительно заданное значение для параметров **Communication_Usage_Type_ID** и **Phone_Device_Type_ID**. Если клиент Workday использует для этих атрибутов другое значение, операция обратной записи не будет выполнена. Предлагаемое решение — обновление Type_IDs в Workday. 
* Если приложение обратной записи настроено для обновления вторичных телефонных номеров, оно не заменяет существующий дополнительный номер телефона в Workday. Он добавляет к записи рабочей роли еще один дополнительный номер телефона. Это поведение не предусмотрено. 
