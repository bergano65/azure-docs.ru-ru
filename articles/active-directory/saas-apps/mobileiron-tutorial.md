...
1. Чтобы настроить приложение в режиме, инициированном  **IDP**, в разделе **MobileIron Domain and URLs** (Домен и URL-адреса приложения MobileIron) сделайте следующее:
    ...
    1. В текстовом поле **Идентификатор** введите URL-адрес в следующем формате: `https://www.mobileiron.com/<key>`

    1. В текстовом поле **URL-адрес ответа** введите URL-адрес в следующем формате: `https://<host>.mobileiron.com/saml/SSO/alias/<key>`.
...
1. На странице диалогового окна **Single User** (Один пользователь) выполните инструкции ниже.
    ...
    1. В текстовое поле **Email address** (Адрес электронной почты) введите адрес электронной почты пользователя, например brittasimon@contoso.com.
