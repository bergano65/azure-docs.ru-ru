...
* Вы можете включить автоматический вход пользователей в Workday (единый вход) с помощью учетных записей Azure AD.
...
* Workday поддерживает единый вход, инициируемый **поставщиком услуг** и **поставщиком удостоверений**.
...
4. В поле поиска введите **Workday**, выберите **Workday** на панели результатов и нажмите кнопку **Добавить**, чтобы добавить это приложение.
     ...
     ![Workday в списке результатов](common/search-new-app.png)
...
Чтобы единый вход функционировал, необходимо установить связь между пользователем Azure AD и одноименным пользователем в Workday.
...
2. **[Настройка единого входа в Workable](#configure-workday-single-sign-on)** необходима, чтобы настроить параметры единого входа на стороне приложения.
...
1. На [портале Azure](https://portal.azure.com/) на странице интеграции с приложением **Workday** щелкните **Единый вход**.
...
5. Щелкните **Задать дополнительные URL-адреса** и выполните следующие действия.
    ...
    ![Сведения о домене и URL-адресах для единого входа для приложения Workday](./media/workday-tutorial/reply.png)
...
8. Требуемые URL-адреса вы можете скопировать из раздела **Настройка Workday**.
...
### <a name="configure-workday-single-sign-on"></a>Настройка единого входа в Workday
...
3. В разделе **URL-адреса перенаправления** выполните следующие действия.
    ...
    b. В текстовых полях **Login Redirect URL** (URL-адрес перенаправления для входа) и **Mobile Redirect URL** (URL-адрес перенаправления для мобильных устройств) введите значение **URL-адрес выхода**, которое было введено в разделе **Базовая конфигурация SAML** на портале Azure.

    c. На портале Azure в разделе **Настройка Workday** скопируйте значение **URL-адрес выхода** и вставьте его в текстовое поле **Logout Redirect URL** (URL-адрес перенаправления для выхода).
...
1. На портале Azure выберите **Корпоративные приложения**, **Все приложения**, а затем **Workday**.

    ![Колонка "Корпоративные приложения"](common/enterprise-applications.png)

2. Из списка приложений выберите **Workday**.

    ![Ссылка на Workday в списке "Приложения"](common/all-applications.png)
