...
3. В разделе **Домены и URL-адреса приложения Sansan** выполните следующие действия.
    ...
    > [!NOTE] 
    > Эти значения приведены для примера. Замените эти значения фактическим URL-адресом для входа. Чтобы получить эти значения, обратитесь к [группе поддержки клиентов Sansan](https://www.sansan.com/form/contact). 
...
7. Чтобы настроить несколько **идентификаторов** и **URL-адресов ответа** для приложения Sansan с помощью скрипта PowerShell, выполните следующие действия:
    ...
    4.3. Выполните команду `Connect-AzureAD` и войдите в систему с помощью учетных данных глобального администратора.
