...
6. В разделе **Утверждения пользователя** диалогового окна **Атрибуты пользователя** настройте атрибут токена SAML, как показано на рисунке выше, и выполните следующие действия.
    ...
    >[!NOTE]
    >Дополнительные сведения о ролях в Azure AD см. [здесь](../develop/howto-add-app-roles-in-azure-ad-apps.md#app-roles-ui--preview).
...
1. В другом окне откройте [песочницу Microsoft Graph](https://developer.microsoft.com/graph/graph-explorer).
    ...
    1. Войдите на сайт песочницы Microsoft Graph с учетными данными глобального администратора или соадминистратора вашего арендатора.

    1. У вас должно быть достаточно разрешений для создания ролей. Щелкните **Изменить разрешения**, чтобы получить требуемые разрешения.

        ![Диалоговое окно песочницы Microsoft Graph 1](./media/aws-multi-accounts-tutorial/graph-explorer-new9.png)

    1. Выберите следующие разрешения из списка (если у вас их еще нет) и щелкните "Изменить разрешения". 

        ![Диалоговое окно песочницы Microsoft Graph 2](./media/aws-multi-accounts-tutorial/graph-explorer-new10.png)

    1. Вам необходимо снова войти в систему и принять запрос. После предоставления согласия вы снова войдете в песочницу Microsoft Graph.

    1. В раскрывающемся списке измените версию на **Бета**. Чтобы получить все субъекты-службы клиента, используйте следующий запрос: `https://graph.microsoft.com/beta/servicePrincipals`. При использовании нескольких каталогов можно применить следующий шаблон, в котором есть основной домен: `https://graph.microsoft.com/beta/contoso.com/servicePrincipals`.

        ![Диалоговое окно песочницы Microsoft Graph 3](./media/aws-multi-accounts-tutorial/graph-explorer-new1.png)

    1. В полученном списке субъектов-служб найдите ту, в которую нужно внести изменения. Также для поиска приложений в списке субъектов-служб можно использовать сочетание клавиш CTRL+F. Используя **идентификатор объекта субъекта-службы**, который был скопирован со страницы свойств Azure AD, выполните следующий запрос для получения соответствующего субъекта-службы.
    ...
    1. Теперь вам следует создать роли для приложения. 

    1. Расположенный ниже JSON является примером объекта appRoles. Создайте похожий объект, чтобы добавить необходимые роли в приложение.
    ...
    1. Вернитесь к песочнице Microsoft Graph и измените метод с **GET** на **PATCH**. Примените к объекту субъекта-службы нужные роли, обновив свойство appRoles, как показано в примере выше. Щелкните **Выполнить запрос**, чтобы выполнить операцию исправления. Сообщение об успешном выполнении подтвердит создание роли для приложения Amazon Web Services
