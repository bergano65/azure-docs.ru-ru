...
![На схеме показано, как Azure A D с приложением W, подключенным к трем учетным записям с помощью I D P, инициировало S O.](./media/aws-multi-accounts-tutorial/amazonwebservice.png)
...
5. Приложение Amazon Web Services (AWS) ожидает утверждения SAML в определенном формате. Настройте следующие утверждения для этого приложения. Управлять значениями этих атрибутов можно в разделе **User Attributes & Claims** (Атрибуты пользователя и утверждения) на странице интеграции приложения. На странице **Настройка единого Sign-On с SAML** нажмите кнопку **изменить** , чтобы открыть диалоговое окно **атрибуты пользователя & утверждения** .

    ![На снимке экрана показаны пользовательские атрибуты с вызываемым элементом управления Edit.](common/edit-attribute.png)
...
6. В разделе **Утверждения пользователя** диалогового окна **Атрибуты пользователя** настройте атрибут токена SAML, как показано на рисунке выше, и выполните следующие действия.
    ...
    ![На снимке экрана показаны утверждения пользователей с добавлением нового утверждения и сохранением вызываемых.](common/new-save-attribute.png)

    ![На снимке экрана показано управление утверждениями пользователей, в которых можно ввести значения, описанные в этом шаге.](common/new-attribute-details.png)
...
1. Найдите **права администратора** на панели поиска и установите флажок **администраторакцесс** , а затем нажмите кнопку **Далее: Теги**.
    ...
    ![На снимке экрана показан Администраторакцесс, выбранный в качестве имени политики.](./media/aws-multi-accounts-tutorial/administrator-access.png)

1. В разделе **Добавление тегов (необязательно)** выполните следующие действия.

    ![На снимке экрана показана панель Добавление тегов, в которой можно добавлять пары "ключ — значение".](./media/aws-multi-accounts-tutorial/config2.png)
...
1. В диалоговом окне **Review** (Обзор) выполните следующие действия.
    ...
    a. В текстовом поле **Role name (имя роли** ) введите значение в следующем шаблоне `accountname-aws-admin` .
...
1. Запишите идентификатор учетной записи для этой учетной записи AWS либо из свойств EC2, либо с панели мониторинга IAM, как показано ниже:
    ...
    ![На снимке экрана показано, где в окне W S отображается учетная запись D.](./media/aws-multi-accounts-tutorial/aws-accountid.png)
...
1. Создайте новые группы с тем же именем, что и у созданных ранее ролей IAM, и запишите **идентификаторы объектов** этих новых групп.
    ...
    ![На снимке экрана показано, куда ввести учетную запись D в панели "Обзор". ](./media/aws-multi-accounts-tutorial/copy-objectids.png)
...
1. После создания всех ролей в учетных записях они отображаются в соответствующем списке **ролей**.
    ...
    ![На снимке экрана показан список ролей с именем роли, описанием и доверенными сущностями.](./media/aws-multi-accounts-tutorial/tutorial-amazonwebservices-listofroles.png)
...
1. Щелкните роли, чтобы скопировать значения **ARN роли** и **доверенных сущностей**. Эти значения нужны всем ролям, которые необходимо создать в Azure AD.
    ...
    ![На снимке экрана показана панель Сводка с выбранной вкладкой отношения доверия.](./media/aws-multi-accounts-tutorial/tutorial-amazonwebservices-role-summary.png)
...
1. Откройте [проводник Microsoft Graph](https://developer.microsoft.com/graph/graph-explorer) в другом окне.
    ...
    ![На снимке экрана показано окно проверки подлинности обозревателя графов с ссылкой изменение разрешений.](./media/aws-multi-accounts-tutorial/graph-explorer-new9.png)

    c. Выберите следующие разрешения из списка (если у вас их еще нет) и щелкните "Изменить разрешения". 

    ![На снимке экрана показаны три выбранных разрешения: Directory. Акцессасусер. ALL, Directory. Read. ALL и Directory. ReadWrite. ALL.](./media/aws-multi-accounts-tutorial/graph-explorer-new10.png)
    ...
    ![На снимке экрана показан выбранный запрос GET, бета-версия и запуск.](./media/aws-multi-accounts-tutorial/graph-explorer-new1.png)
    ...
    ![Снимок экрана, показывающий использование запроса для получения объекта субъекта-службы.](./media/aws-multi-accounts-tutorial/graph-explorer-new2.png)

    ж. Извлеките свойство appRoles из объекта субъекта-службы.

    ![На снимке экрана показаны сведения об объекте субъекта-службы.](./media/aws-multi-accounts-tutorial/graph-explorer-new3.png)
    ...
    ![На снимке экрана показан проводник Graph с выбранным методом patch.](./media/aws-multi-accounts-tutorial/graph-explorer-new11.png)
...
1. После создания групп выберите группу и присвойте ее приложению.
    ...
    ![На снимке экрана показано, как добавить назначение выбранным пользователям и группам, чтобы открыть панель пользователи и группы.](./media/aws-multi-accounts-tutorial/graph-explorer-new5.png)
...
1. Чтобы присвоить роль группе, выберите роль и нажмите кнопку **Присвоить** в нижней части страницы.
    ...
    ![На снимке экрана показано добавление назначения с одной выбранной группой.](./media/aws-multi-accounts-tutorial/graph-explorer-new6.png)
...
![На снимке экрана показана страница приложения с W S, на которой можно выбрать роль.](./media/aws-multi-accounts-tutorial/tutorial-amazonwebservices-test-screen.png)

Также можно проверить ответ SAML, чтобы увидеть роли, передаваемые как утверждения.

![Снимок экрана показывает часть ответа SAML со значениями атрибутов.](./media/aws-multi-accounts-tutorial/tutorial-amazonwebservices-test-saml.png)
