...
description: Узнайте, как настроить единый вход между Azure AD и Amazon Web Services (AWS) (руководство по устаревшей версии).
...
В этом руководстве описано, как интегрировать Azure Active Directory (Azure AD) с Amazon Web Services (AWS) (руководство по устаревшей версии).
...
> [!NOTE]
> Учтите, что подключение одного приложения AWS ко всем учетным записям AWS не рекомендуется. Вместо этого мы рекомендуем использовать [этот](https://docs.microsoft.com/azure/active-directory/saas-apps/amazon-web-service-tutorial) подход, чтобы настроить несколько экземпляров учетной записи AWS для нескольких экземпляров приложений AWS в Azure AD. Этот подход следует использовать только в том случае, если в нем есть несколько учетных записей и ролей AWS. Эта модель не масштабируется, так как учетные записи AWS и роли внутри этих учетных записей увеличиваются. Этот подход не использует функцию импорта роли AWS с помощью подготовки пользователей Azure AD, поэтому необходимо вручную добавить, обновить или удалить роли. Другие ограничения по этому подходу см. в приведенных ниже сведениях.

**Обратите внимание на то, что мы не рекомендуем использовать этот подход по следующим причинам.**

* Для исправления всех ролей в приложении необходимо использовать подход Microsoft Graph Explorer. Мы не рекомендуем использовать файлы манифестов.
...
* Amazon Web Services (AWS) поддерживает службы **SP и IDP,** ИНИЦИИРУЮЩИЕ единый вход
...
1. После добавления приложения перейдите на страницу **свойств** и скопируйте **идентификатор объекта**.
...
6. В разделе **Утверждения пользователя** диалогового окна **Атрибуты пользователя** настройте атрибут токена SAML, как показано на рисунке выше, и выполните следующие действия.
    ...
    c. В текстовом поле **namespace (пространство имен** ) введите значение пространства имен, отображаемое для этой строки.
...
1. Найдите **права администратора** на панели поиска и установите флажок **администраторакцесс** , а затем нажмите кнопку **Далее: Теги**.

    ![Выбор доступа администратора](./media/aws-multi-accounts-tutorial/administrator-access.png)

1. В разделе **Добавление тегов (необязательно)** выполните следующие действия.

    ![Выбор доступа администратора](./media/aws-multi-accounts-tutorial/config2.png)

    a. В текстовом поле **Key (ключ** ) введите имя ключа для ex: азуреадтест.

    b. В текстовом поле **value (необязательно)** введите значение ключа, используя следующий формат `accountname-aws-admin`. Имя учетной записи должно быть в нижнем регистре.

    c. Нажмите кнопку **Далее: Проверка**.

1. В диалоговом окне **Review** (Обзор) выполните следующие действия.

    ![Проверка настройки единого входа][34]

    a. В текстовом поле **Role name (имя роли** ) введите значение в следующем `accountname-aws-admin`шаблоне.

    b. В текстовом поле **Описание роли** введите то же значение, которое использовалось для имени роли.
    ...
    > [!NOTE]
    > Аналогичным образом создайте оставшиеся другие роли, такие как AccountName-Finance-Admin, имя_учетной_записи-Read-only-пользователь, имя_учетной_записи-devops-пользователь, имя_учетной_записи-TPM-пользователь с разными политиками для присоединения. Кроме того, эти политики ролей можно изменить в соответствии с требованиями для каждой учетной записи AWS, но лучше хранить одинаковые политики для каждой роли в учетных записях AWS.

1. Запишите идентификатор учетной записи для этой учетной записи AWS либо из свойств EC2, либо с панели мониторинга IAM, как показано ниже:

    ![Выбор доступа администратора](./media/aws-multi-accounts-tutorial/aws-accountid.png)

1. Теперь войдите в [портал Azure](https://portal.azure.com/) и перейдите к **группам**.
...
1. Создайте новые группы с тем же именем, что и у созданных ранее ролей IAM, и запишите **идентификаторы объектов** этих новых групп.
    ...
    ![Выбор доступа администратора](./media/aws-multi-accounts-tutorial/copy-objectids.png)
...
1. Откройте [проводник Microsoft Graph](https://developer.microsoft.com/graph/graph-explorer) в другом окне.
    ...
    a. Войдите на сайт Microsoft Graph Explorer, используя учетные данные глобального администратора или соадминистратора для вашего клиента.
    ...
    ![Диалоговое окно "обозреватель Microsoft Graph"](./media/aws-multi-accounts-tutorial/graph-explorer-new9.png)

    c. Выберите следующие разрешения из списка (если у вас их еще нет) и щелкните "Изменить разрешения". 

    ![Диалоговое окно "обозреватель Microsoft Graph"](./media/aws-multi-accounts-tutorial/graph-explorer-new10.png)

    d. Вам необходимо снова войти в систему и принять запрос. После принятия согласия вы снова вошли в обозреватель Microsoft Graph.
    ...
    ![Диалоговое окно "обозреватель Microsoft Graph"](./media/aws-multi-accounts-tutorial/graph-explorer-new2.png)

    ж. Извлеките свойство appRoles из объекта субъекта-службы.

    ![Диалоговое окно "обозреватель Microsoft Graph"](./media/aws-multi-accounts-tutorial/graph-explorer-new3.png)
    ...
    > [!Note]
    > Вы можете добавлять новые роли только после **msiam_access** для операции исправления. Вы можете добавить столько ролей, сколько нужно вашей организации. Azure AD будет передавать **значения** этих ролей в качестве значения утверждения в ответе SAML.

    j. Вернитесь в обозреватель Microsoft Graph и измените метод с " **получить** на **исправление**". Примените к объекту субъекта-службы нужные роли, обновив свойство appRoles, как показано в примере выше. Щелкните **Выполнить запрос**, чтобы выполнить операцию исправления. Сообщение об успешном выполнении подтвердит создание роли для приложения Amazon Web Services

    ![Диалоговое окно "обозреватель Microsoft Graph"](./media/aws-multi-accounts-tutorial/graph-explorer-new11.png)
...
* [Настройка подготовки с помощью API-интерфейсов MS Graph](https://docs.microsoft.com/azure/active-directory/manage-apps/application-provisioning-configure-api)
