...
1. В разделе **Домены и URL-адреса приложения UserVoice** выполните следующие действия:
    ...
    > [!NOTE] 
    > Эти значения приведены для примера. Замените эти значения фактическим URL-адресом для входа и идентификатором. Чтобы получить их, обратитесь в [службу поддержки клиентов UserVoice](https://www.uservoice.com/).
...
1. На странице **Изменение проверки подлинности пользователя** выполните следующие действия.
    ...
    4.3. В текстовое поле **Current certificate SHA1 fingerprint** (Отпечаток текущего сертификата SHA1) вставьте значение **Thumbprint**, которое вы скопировали на портале Azure.
