...
> [!NOTE]
> Соединитель подготовки синхронизации удостоверений глобального ретранслятора использует метод авторизации SCIM, который больше не поддерживается из-за проблем безопасности. Все усилия выполняются с помощью глобальной ретрансляции, чтобы переключиться на более безопасный метод авторизации.
