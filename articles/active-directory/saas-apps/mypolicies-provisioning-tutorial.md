...
5. В разделе **учетные данные администратора** введите `https://<myPoliciesCustomDomain>.mypolicies.com/scim` в поле **URL-адрес клиента** , где `<myPoliciesCustomDomain>` является пользовательским доменом myPolicies. Вы можете получить домен клиента myPolicies из URL-адреса.
