...
## <a name="configure-and-test-azure-ad-sso-for-sap-cloud-for-customer"></a>Настройка и проверка единого входа Azure AD для SAP Cloud for Customer
...
1. На портале Azure на странице интеграции с приложением **SAP Cloud for Customer** найдите раздел **Управление** и выберите **Единый вход**.
...
1. В разделе **Атрибуты пользователя** в диалоговом окне **User Attributes & Claims** (Атрибуты пользователя и утверждения) выполните следующие действия.
    ...
    ![Снимок экрана: страница User Attributes & Claims (Атрибуты пользователя и утверждения) с выделенным значком Edit (Изменить).](./media/sap-customer-cloud-tutorial/tutorial_usermail.png)
...
2. В левой части меню щелкните **Identity Providers (Поставщики удостоверений)**  > **Corporate Identity Providers (Поставщики корпоративных удостоверений)**  > **Add (Добавить)** , а во всплывающем окне добавьте имя поставщика удостоверений, например **Azure AD**, нажмите кнопку **Save** (Сохранить), а затем щелкните **SAML 2.0 Configuration** (Настройка SAML 2.0).
    ...
    ![Снимок экрана: страница Identity Providers (Поставщики удостоверений) с выделенным текстовым диалоговым окном Add Identity Provider (Добавить поставщика удостоверений) и кнопкой Save (Сохранить).](./media/sap-customer-cloud-tutorial/configure01.png)

3. В разделе **SAML 2.0 Configuration** (Настройка SAML 2.0) выполните следующие действия:

    ![Снимок экрана: диалоговое окно SAML 2.0 Configuration (Настройка SAML 2.0) с выбранной кнопкой Browse (Обзор).](./media/sap-customer-cloud-tutorial/configure02.png)
...
2. В левой части меню щелкните **Users & Authorizations (Пользователи и авторизация)**  > **User Management (Управление пользователями)**  > **Add User (Добавить пользователя)** .
    ...
    ![Снимок экрана: страница User Management (Управление пользователями) с выбранной кнопкой Add User (Добавить пользователя).](./media/sap-customer-cloud-tutorial/configure03.png)
...
* Выберите **Тестировать приложение** на портале Azure. Вы будете перенаправлены по URL-адресу для входа в SAP Cloud for Customer, где можно инициировать поток входа. 

* Вручную введите URL-адрес для входа в SAP Cloud for Customer и инициируйте поток входа.

* Вы можете использовать портал "Мои приложения" корпорации Майкрософт. Щелкнув плитку SAP Cloud for Customer в разделе "Мои приложения", вы автоматически будете перенаправлены по URL-адресу для входа в приложение SAP Cloud for Customer. Дополнительные сведения о портале "Мои приложения" см. в [этой статье](../user-help/my-apps-portal-end-user-access.md).


## <a name="next-steps"></a>Дальнейшие действия

После настройки SAP Cloud for Customer вы можете применять элементы управления сеансами, которые в реальном времени защищают конфиденциальные данные вашей организации от хищения и несанкционированного доступа. Элементы управления сеансом являются расширением функции условного доступа. [Узнайте, как применять управление сеансами с помощью Microsoft Cloud App Security](/cloud-app-security/proxy-deployment-aad).