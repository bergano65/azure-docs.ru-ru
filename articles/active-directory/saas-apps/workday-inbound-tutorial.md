...
* **Январь 2020 г. Возможность задать атрибут accountExpires в AD.** Теперь с помощью функции [NumFromDate](../app-provisioning/functions-for-customizing-application-data.md#numfromdate) можно сопоставлять поля дат Workday, например *EndContractDate* или *StatusTerminationDate*. 
...
1. В поле поиска введите **Domain Security Configuration**, а затем перейдите по ссылке **Domain Security Configuration Report** (Отчет о конфигурации безопасности домена).  
   >[!div class="mx-imgBorder"]
   >![Снимок экрана с текстом "Domain Security Configuration" в поле поиска и надписью "Domain Security Configuration Report — Report" (Отчет о конфигурации безопасности домена — Отчет) в результатах поиска.](./media/workday-inbound-tutorial/wd_isu_06.png "Политики безопасности домена")  
2. С помощью текстового поля **Domain** (Домен) найдите указанные ниже домены и добавьте их к фильтру по одному.  
   ...
   * *Учетные записи Workday*
     ...
     >[!div class="mx-imgBorder"]
     >![Снимок экрана, на котором показан отчет о конфигурации безопасности домена и External Account (Внешняя учетная запись) в текстовом поле Domain (Домен).](./media/workday-inbound-tutorial/wd_isu_07.png "Политики безопасности домена")  

     >[!div class="mx-imgBorder"]
     >![Снимок экрана, на котором показан отчет о конфигурации безопасности домена с выбранным списком доменов.](./media/workday-inbound-tutorial/wd_isu_08.png "Политики безопасности домена") 
...
4. Прокрутите страницу **Edit Domain Security Policy Permissions** (Изменение разрешений политики безопасности для домена) до раздела **Integration Permissions** (Разрешения интеграции). Нажмите значок "+", чтобы добавить группу системы интеграции в список групп безопасности с разрешениями интеграции **Get** и **Put**.
   >[!div class="mx-imgBorder"]
   >![Снимок экрана, на котором показан выделенный раздел Integration Permissions (Разрешения интеграции).](./media/workday-inbound-tutorial/wd_isu_10.png "Изменить разрешение")  
...
Определив сервер Windows, на котором будет размещен агент подготовки, войдите на узел сервера с учетными данными локального администратора или администратора домена. Процесс установки агента создаст файлы учетных данных защищенного хранилища ключей и обновит конфигурацию профиля службы на сервере размещения. Для этого требуется доступ с правами администратора для сервера, на котором размещен агент. 
...
Выполните следующие действия, чтобы настроить учетную запись службы, которую можно использовать для операций агента подготовки. 
1.  На контроллере домена AD откройте оснастку *Пользователи и компьютеры Active Directory*. 
2.  Создайте нового пользователя домена (пример, *provAgentAdmin*).  
3.  Щелкните правой кнопкой мыши имя подразделения или домена и выберите *Делегирование управления*. Откроется *мастер делегирования управления*. 

> [!NOTE] 
> Если вы хотите, чтобы с помощью агента подготовки можно было только создавать пользователей и считывать их данные из определенного подразделения для целей тестирования, рекомендуем при тестовых запусках делегировать управление на уровне соответствующего подразделения.

4. Нажмите кнопку **Далее** на экране приветствия. 
5. На экране **Выбор пользователей или групп** добавьте пользователя домена, созданного на шаге 2. Нажмите кнопку **Далее**.
   >[!div class="mx-imgBorder"]
   >![Экран добавления](./media/workday-inbound-tutorial/delegation-wizard-01.png "Экран добавления")

6. На экране **Делегируемые задачи** выберите следующие задачи: 
...
7. Нажмите кнопки **Далее** и **Сохранить** для сохранения конфигурации.
...
1. Пройдите проверку подлинности в экземпляре Azure AD, используя учетные данные администратора гибридных удостоверений.
...
1. Проверьте установку агента и убедитесь, что он работает, открыв оснастку "Службы" и обнаружив службу под именем "Microsoft Azure AD Connect Provisioning Agent"
   ...
   >[!div class="mx-imgBorder"]
   >![Снимок экрана, на котором в оснастке "Службы" показан запущенный агент подготовки Microsoft Azure AD Connect.](./media/workday-inbound-tutorial/services.png)
...
1. Чтобы сохранить сопоставления, щелкните **Сохранить** в верхней части раздела "Сопоставление атрибутов".
   >[!div class="mx-imgBorder"]
   >![Снимок экрана, на котором показана страница "Сопоставление атрибутов" (Параметры) с выбранным действием "Сохранить".](./media/workday-inbound-tutorial/wd_2.png)
...
8.    Щелкните ссылку **configure** (Настроить) под областями Request/Response (Запрос/Ответ), чтобы задать учетные данные Workday. Установите флажок **Authentication** (Аутентификация), а затем введите имя пользователя и пароль учетной записи системы интеграции Workday. Убедитесь, что имя пользователя отформатировано как "имя\@клиент", и оставьте флажок возле параметра **WS-Security UsernameToken**.
   ![Снимок экрана, на котором показана вкладка Security (Безопасность) с заполненными полями Username (Имя пользователя) и Password (Пароль), и выбран параметр WS-Security Username Token.](./media/workday-inbound-tutorial/wdstudio2.png)
...
13. В командной строке Workday Studio последовательно выберите **File (Файл) > Open File (Открыть файл)** и откройте только что сохраненный XML-файл. После этого в редакторе XML Workday Studio откроется файл.
    ...
    ![Снимок экрана редактора XML Workday Studio с открытым в нем XML-файлом.](./media/workday-inbound-tutorial/wdstudio3.png)
...
5. Выберите ссылку **Изменить список атрибутов для Workday**.
    ...
    ![Снимок экрана, на котором показана страница Workday to Azure AD User Provisioning — Provisioning (Подготовка пользователей Workday в Azure AD) с выделенным действием "Изменить список атрибутов для Workday".](./media/workday-inbound-tutorial/wdstudio_aad1.png)
