...
Цель этого руководства состоит в том, чтобы показать шаги, которые необходимо выполнить для импорта профилей работников из рабочего дня в Active Directory и Azure Active Directory, с дополнительным сносом адреса электронной почты и имени пользователя в Workday.
...
* **Напишите обратно адрес электронной почты и имя пользователя в Workday** - Служба предоставления услуг по предоставлению услуг по предоставлению услуг Azure AD может записывать адреса электронной почты и имя пользователя из Azure AD обратно в Workday.
...
Это решение для подготовки пользователей Workday идеально подходит для:
...
* **Поток записи электронной почты и имен пользователей — от предпосылок Active Directory до рабочего дня:** После завершения создания учетной записи в Active Directory она синхронизируется с Azure AD через Azure AD Connect, а атрибут ы и адреса электронной почты и имени пользователя может быть записан обратно в Workday.
...
7. Если разъем записи рабочего дня настроен, он записывает обратно атрибут электронной почты и имя пользователя в Workday, на основе используемого атрибута соответствия.
...
* Действительная лицензия Azure AD Premium P1 или более высокая лицензия подписки для каждого пользователя, которая будет получена из Рабочего дня и подготовлена либо в предприимчивом Active Directory, либо в Active Directory.
* Глобальный доступ администратора Azure AD для настройки агента подготовки
...
* **Рабочий день для обеспечения пользователей Azure AD** - В то время как Azure AD Connect — это инструмент, который следует использовать для синхронизации пользователей Active Directory с Active Directory, это приложение может быть использовано для облегчения подготовки пользователей только для облаков от Рабочего дня до одного арендатора Active Directory Azure.

* **Workday Writeback** - Это приложение облегчает списание адресов электронной почты пользователя из Active Directory Azure в Workday.
...
Воспользуйтесь приведенной ниже схемой принятия решений, чтобы определить, какие приложения подготовки Workday подходят для вашего сценария.
    ![Решение Flowchart](./media/workday-inbound-tutorial/wday_app_flowchart.png "Решение Flowchart")
...
#### <a name="deployment-scenario-1--single-workday-tenant---single-ad-domain"></a>Сценарий развертывания #1: один рабочий день арендатора -> один aD домена
...
#### <a name="deployment-scenario-2--single-workday-tenant---multiple-child-ad-domains"></a>Сценарий развертывания #2: Один рабочий день Арендатор -> несколько доменов ад-низом ребенка
...
#### <a name="deployment-scenario-3--single-workday-tenant---disjoint-ad-forests"></a>Сценарий развертывания #3 : Единый рабочий день Арендатор -> разрозненных aD лесов
...
* **Каким образом необходимо сопоставить пользователей Workday и Active Directory?**
  ...
  * *Пример: Пользователи с определенным значением "Worker_ID" рабочего дня сопоставляются с пользователями Active Directory, где "employeeID" имеет такое же значение. Если значение Worker_ID не найдено в Active Directory, создайте нового пользователя.*
  
* **Содержит ли лес Active Directory идентификаторы пользователей, необходимые для работы логики сопоставления?**

  * *Пример: Если эта настройка является новым развертыванием рабочего дня, рекомендуется предварительно заселить Active Directory с правильным испугом Worker_ID значения (или уникальное значение идентификатора выбора), чтобы сохранить логику соответствия как можно более простой.*
...
2. Выполните задачу **группы безопасности.** 
   ...
   * Есть два типа групп безопасности в Workday:
     ...
     * **Ограничено:** Все участники группы безопасности имеют контекстный доступ к подмноженому набору экземпляров данных (строк), к которым может получить доступ группа безопасности.
...
* [Настройка подключения к рабочему дню и активному каталогу](#part-3-in-the-provisioning-app-configure-connectivity-to-workday-and-active-directory)
...
6. После **добавления**приложения и отображаемого экрана деталей приложения, выберите Подготовка .

7. Измените **режим** **обеспечения** на **автоматический.**

8. Нажмите на информационный баннер, отображаемый для загрузки Агента По предоставлению услуг. 

   ![Скачать агент](./media/workday-inbound-tutorial/pa-download-agent.png "Скачать экран агента")
...
### <a name="part-3-in-the-provisioning-app-configure-connectivity-to-workday-and-active-directory"></a>Часть 3: В приложении для подготовки настраивается на подключение к workday и Active Directory
На этом этапе мы устанавливаем связь с Workday и Active Directory на портале Azure. 

1. На портале Azure вернитесь к рабочему дню в приложение "Активный каталог", созданное в [части 1](#part-1-add-the-provisioning-connector-app-and-download-the-provisioning-agent)
...
1. В поле **Область исходного объекта** можно выбрать, какие наборы пользователей в Workday должны учитываться при подготовке в AD, определив набор фильтров на основе атрибутов. Область по умолчанию — "Все пользователи в Workday". Примеры фильтров
   ...
   * Пример: Область для пользователей с идентимативами работы между 1000000 и 2000000 (за исключением 2000000)
...
1. Щелкните существующее сопоставление атрибута, чтобы его обновить, или **Добавить новое сопоставление** в нижней части экрана, чтобы добавить новые сопоставления. Отдельное сопоставление атрибутов поддерживает следующие свойства:
      ...
      * **Тип сопоставления**
         ...
         * **Прямая** - Записывает значение атрибута рабочего дня атрибуту AD, без изменений
...
* [Существует документация по написанию выражений здесь](../app-provisioning/functions-for-customizing-application-data.md). В этот раздел входят примеры, демонстрирующие удаление специальных символов.
...
| **Мобильные**  |    mobile       |     |       Создание и обновление |
| **LocalReference** |  preferredLanguage  |     |  Создание и обновление |                                               
| **Коммутатор\](Муниципалитет\[, "ОУ-Стандартные пользователи,OU-users,OU-Default,OU-Locations,DC-contoso,DC'com", "Даллас", "ОУ-Стандартные пользователи,OU-пользователи,OU-Даллас,OU-Locations,DC-contoso,DC "Остин", "ОУ-Стандартные пользователи,OU-пользователей, OU-Остин, OU-Locations,DC-contoso,DC'com", "Сиэтл", "OU-Стандартные пользователи, OU-пользователей, ОУ-Сиэтл, OU-Locations,DC-contoso,DC'com", "Лондон", "OU-Standard users,OU**  | parentDistinguishedName     |     |  Создание и обновление |
...
8. В разделе **Учетные данные администратора** заполните поля следующим образом.
   ...
   * Если тест соединения удается, нажмите кнопку **Сохранить** в верхней части. В случае неудачи дважды проверьте правильность URL-адреса и учетных данных Workday в приложении Workday.

### <a name="part-2-configure-workday-and-azure-ad-attribute-mappings"></a>Часть 2: Настройка рабочего дня и атрибута Azure AD атрибута отображения
...
2. В поле **области исходного объекта** можно выбрать, какие наборы пользователей в Workday должны быть в области подготовки к Azure AD, определив набор фильтров на основе атрибутов. Область по умолчанию — "Все пользователи в Workday". Примеры фильтров
...
3. В поле **Действия с целевыми объектами** можно в глобальном масштабе отфильтровать, какие действия выполняются в Azure AD. **Создание** и **обновление** являются наиболее распространенными.
...
6. Чтобы сохранить отображение, нажмите **«Сохранить»** в верхней части раздела «Отображение атрибутов».

После завершения настройки сопоставления атрибутов [можно включить и запустить службу подготовки пользователей](#enable-and-launch-user-provisioning).

## <a name="configuring-azure-ad-attribute-writeback-to-workday"></a>Настройка атрибута Azure AD к workday

Следуйте этим инструкциям, чтобы настроить списание адресов электронной почты пользователя и имени пользователя из Active Directory Azure в Workday.
...
### <a name="part-1-adding-the-writeback-connector-app-and-creating-the-connection-to-workday"></a>Часть 1: Добавление приложения для разъема списания и создание соединения с рабочим днем
...
8. В разделе **Учетные данные администратора** заполните поля следующим образом.
   ...
   * **Имя пользователя Admin** - Введите имя пользователя учетной записи системы интеграции Workday с придативаемым доменным именем клиента. Должно выглядеть что-то вроде: *имя\@пользователя contoso4*

   * **Админ пароль -** Введите пароль учетной записи системы интеграции Workday

   * **URL-адрес арендатора -** Введите URL-адрес в конечную точку веб-службрабочей рабочего дня для вашего клиента. Значение должно иметь следующий вид: https://wd3-impl-services1.workday.com/ccx/service/contoso4/Human_Resources, где *contoso4* заменяется правильным именем клиента, а *wd3-impl* — правильной строкой среды (если необходимо).

   * **Уведомление по электронной почте -** Введите свой адрес электронной почты и проверьте флажок "Отправить электронную почту в случае сбоя".

   * Нажмите кнопку **Проверить подключение**. Если тест соединения удается, нажмите кнопку **Сохранить** в верхней части. В случае неудачи дважды проверьте правильность URL-адреса и учетных данных Workday в приложении Workday.

### <a name="part-2-configure-writeback-attribute-mappings"></a>Часть 2: Настройка отображения атрибутов списания

В этом разделе описывается настройка потока атрибутов обратной записи из Azure AD в Workday. В настоящее время разъем поддерживает только списание адреса электронной почты и имени пользователя в Workday.
...
2. В поле **области исходного объекта** можно дополнительно фильтровать, какие наборы пользователей в Active Directory Azure должны быть записаны на рабочий день. Область по умолчанию — "Все пользователи в Azure AD".
...
4. Обычно вы отображаете атрибут Azure AD *с* атрибутом *Workday UserID* и отображаете атрибут *azure* AD с атрибутом *EmailAddress* рабочего дня. Чтобы сохранить отображение, нажмите **«Сохранить»** в верхней части раздела «Отображение атрибутов».
...
* **Вопросы по работе агента подготовки**
  ...
  * [Могу ли я установить агент по обеспечению на том же сервере под управлением Azure AD Connect?](#can-i-install-the-provisioning-agent-on-the-same-server-running-azure-ad-connect)
...
* **Вопросы по конфигурации и сопоставлению атрибутов Workday и AD**
  ...
  * [У меня есть пользовательские атрибуты в Workday и Active Directory. Как настроить решение для работы с пользовательскими атрибутами?](#i-have-custom-attributes-in-workday-and-active-directory-how-do-i-configure-the-solution-to-work-with-my-custom-attributes)
...
Сейчас такая возможность не поддерживается. Рекомендуемое решение заключается в развертывании сценария PowerShell, который запрашивает конечную точку Microsoft GraphPI для [данных журнала аудита](https://docs.microsoft.com/graph/api/resources/azure-ad-auditlog-overview?view=graph-rest-beta) и использовать его для запуска таких сценариев, как групповое назначение. Этот сценарий PowerShell можно присоединить к планировщику задач и развернуть в том же окне, где запущен агент подготовки.  
...
* Войти на сервер Windows, где установлен агент по обеспечению.
* Перейти к **панели** -> управления**Удалить или изменить** меню программы
...
#### <a name="can-i-install-the-provisioning-agent-on-the-same-server-running-azure-ad-connect"></a>Могу ли я установить агент по обеспечению на том же сервере под управлением Azure AD Connect?

Да, вы можете установить агент по обеспечению на том же сервере, который работает под ключ Azure AD Connect.
...
Вы также можете проверить, есть ли у вас все необходимые порты открыты, открыв [соединительный портовый тестовый инструмент](https://aadap-portcheck.connectorporttest.msappproxy.net/) из вашей сети помещений. Большее число зеленых флажков означает большую устойчивость.

Чтобы получить правильные результаты, необходимо:

* Откройте средство в браузере на сервере, где установлен агент подготовки.
* Убедитесь, что все прокси-серверы или брандмауэры, которые применяются к агенту подготовки, также применяются к этой странице. Это можно сделать в Internet Explorer, перейдя в **настройки-> Параметры Интернета -> подключения - > LAN Настройки**. На этой странице появится флажок "Использовать прокси-сервер для локальной сети". Установите этот флажок и укажите адрес прокси-сервера в поле "Адрес".
...
* Войти на сервер Windows под управлением агента подготовки.
* Open PowerShell в качестве администратора Windows.
...
* Из списка агентов, которые появляются `id` - скопировать значение поля из этого ресурса, чей *ресурсName* равна вашему доменному имени AD.
...
Обычно требуется настроить атрибут *displayName* в AD таким образом, чтобы он также предоставил информацию о отделе пользователя и стране/регионе. Например, если Иван Воронков работает в отделе маркетинга в США, можно настроить так, чтобы его значение *displayName* отображалось как *Воронков, Иван (Маркетинг-Россия)*.

Вот как вы можете обрабатывать такие требования для построения *CN* или *displayName* включить атрибуты, такие как компания, бизнес-подразделение, город или страна / регион.
...
* Чтобы создать правильное выражение отображения атрибутов, определите, какой атрибут Workday «авторитетно» представляет имя пользователя, фамилию, страну/регион и отдел. Предположим, у нас есть атрибуты *PreferredFirstName*, *PreferredLastName*, *CountryReferenceTwoLetter* и *SupervisoryOrganization* соответственно. Их можно использовать, чтобы создать приведенное ниже выражение для атрибута AD *displayName* и получить отображаемое имя, например *Воронков, Иван (Маркетинг-Россия)*.

    ```
     Append(Join(", ",[PreferredLastName],[PreferredFirstName]), Join(""," (",[SupervisoryOrganization],"-",[CountryReferenceTwoLetter],")"))
    ```
    После того, как у вас есть правильное выражение, отодемонстрируем таблицу ![атрибутов и измените отображение атрибута *displayName,* как показано ниже: DisplayName Mapping](./media/workday-inbound-tutorial/wd_displayname_map.png)
...
В этом разделе приведены рекомендации по устранению неполадок подготовки, возникающих во время интеграции Workday, с помощью журналов аудита Azure AD и журналов средства просмотра событий Windows Server. Он строится на вершине общих шагов по устранению неполадок и концепций, отраженных в [учебнике: Отчетность об автоматическом подготовке учетной записи пользователя](../app-provisioning/check-status-user-account-provisioning.md)
...
* [Понимание журналов для операций обновления менеджера](#understanding-logs-for-manager-update-operations)
...
* Войти на машину Windows Server, где развернут агент по обеспечению
...
При обнаружении нового сотрудника в Workday (например, с идентификатором *21023*) служба подготовки Azure AD пытается создать новую учетную запись пользователя AD для него и в процессе создает 4 записи журнала аудита, как описано ниже.
  ...
  [![Создание журналов аудита](media/workday-inbound-tutorial/wd_audit_logs_02.png)](media/workday-inbound-tutorial/wd_audit_logs_02.png#lightbox)
...
* Запись **действий правила синхронизации:** Эта запись журнала отображает результаты правил отображения атрибутов и настраивает фильтры scoping вместе с действием подготовки, которое будет принято для обработки входящего события рабочего дня. Используйте информацию, приведенную в разделе *Дополнительные сведения* записи журнала, чтобы устранить неполадки, связанные с действием синхронизации. Ниже показан пример записи вместе с указателями того, как интерпретировать каждое поле.
...
* Запись **экспорта AD:** Эта запись журнала отображает результат операции создания учетной записи AD вместе с значениями атрибутов, которые были установлены в процессе. Используйте информацию, приведенную в разделе *Дополнительные сведения* записи журнала, чтобы устранить неполадки, связанные с операцией создания учетной записи. Ниже показан пример записи вместе с указателями того, как интерпретировать каждое поле. В разделе "Дополнительные сведения" для параметра EventName задано значение EntryExportAdd, для параметра JoiningProperty устанавливается значение атрибута идентификатора сопоставления, для параметра SourceAnchor — значение WorkdayID (WID), связанное с записью, а для параметра TargetAnchor — значение атрибута ObjectGuid AD созданного пользователя. 
  ...
  [![SCIM Добавить](media/workday-inbound-tutorial/wd_event_viewer_05.png)](media/workday-inbound-tutorial/wd_event_viewer_05.png#lightbox)
  ...
  [![SCIM Добавить](media/workday-inbound-tutorial/wd_event_viewer_06.png)](media/workday-inbound-tutorial/wd_event_viewer_06.png#lightbox)
...
Атрибут руководителя является ссылочным атрибутом в AD. Служба подготовки не устанавливает атрибут руководителя как часть операции создания пользователя. Скорее, атрибут руководителя устанавливается как часть операции *обновления* после создания учетной записи AD для пользователя. Предположим, что новый сотрудник с идентификатором 21451 активирован в Workday, а у руководителя нового сотрудника (*21023*) уже есть учетная запись AD. В этом случае в результате поиска пользователя 21451 в журналах аудита отобразится 5 записей.
  ...
  [![Обновление менеджера](media/workday-inbound-tutorial/wd_audit_logs_03.png)](media/workday-inbound-tutorial/wd_audit_logs_03.png#lightbox)
...
|--|---|---|---|
|1.| Ошибка установки агента подготовки с сообщением об ошибке: *Служба 'Microsoft Azure AD Connect Provisioning Agent' (AADConnectProvisioningAgent) не сработала. Убедитесь, что у вас достаточно привилегий для запуска системы.* | Эта ошибка обычно появляется при попытке установить агент подготовки на контроллере домена, а групповая политика запрещает запуск службы.  Она также отображается, если у вас работает предыдущая версия агента и вы не удалили ее перед началом новой установки.| Установите агент подготовки на сервере, отличном от сервера контроллера домена. Перед установкой нового агента убедитесь, что предыдущие версии агента удалены.|
|2.| Служба Windows "Агент подготовки Microsoft Azure AD Connect" находится в состоянии *Запускается* и не переходит в состояние *Выполняется*. | В рамках установки мастер агента создает на сервере локальную учетную запись **(NT Service\\AADConnectServiceingService)** и это учетная запись logon, используемая для запуска службы. Эта ошибка возникнет, если политика безопасности на сервере Windows запрещает локальным учетным записям запускать службы. | Откройте *консоль служб*. Нажмите на службу Windows 'Microsoft Azure AD Connect ProvisionIng Agent' и во вкладке Logon укажите учетную запись администратора домена для запуска службы. Перезапустите службу. |
|3.| При настройке агента подготовки с вашим доменом AD в шаге *Connect Active Directory*мастер уводит много времени, пытаясь загрузить схему AD и в конечном итоге время выходит из нее. | Эта ошибка обычно отображается, если мастеру не удается связаться с сервером контроллера домена AD из-за проблем брандмауэра. | На экране мастера *Connect Active Directory,* предоставляя учетные данные для вашего домена AD, есть опция под названием *Выберите приоритет контроллера домена.* Используя этот параметр, выберите контроллер домена, который находится на том же сайте, что и сервер агента, и убедитесь в отсутствии правил брандмауэра, блокирующих связь. |
...
|#|Сценарий ошибки |Возможные причины|Рекомендуемое решение|
|--|---|---|---|
|1.| Сбои операции экспорта в журнале аудита с *сообщением Ошибка: ОперацииОшибка-SvcErr: Произошла ошибка операции. Превосходная ссылка не была настроена для службы каталога. Таким образом, служба каталогов не может выдавать рефералы объектам за пределами этого леса.* | Эта ошибка обычно проявляется, если *Active Directory Container* OU настроен неправильно или если есть проблемы с картой выражения, используемой для *parentDistinguishedName.* | Проверьте параметр подразделения *контейнера Active Directory* на наличие опечаток. Если вы используете атрибут *parentDistinguishedName* в сопоставлении атрибутов, убедитесь, что он всегда соответствует известному контейнеру в домене AD. Проверьте событие *экспорта* в журналах аудита, чтобы увидеть созданное значение. |
|2.| Сбои операций экспорта в журнале аудита с кодом ошибки: *SystemForCrossDomainIdentityBadResponse* и *ошибка сообщения: ОграничениеНарушение-AtrErr: Значение в запросе является недействительным. Значение для атрибута не было в приемлемом диапазоне значений. Подробности: CONSTRAINT_ATT_TYPE - компания*. | Хотя эта ошибка относится только к атрибуту *company*, она может возникнуть и для других атрибутов, таких как *CN*. Эта ошибка появляется из-за принудительного ограничения схемы AD. По умолчанию такие атрибуты, как *company* и *CN* в AD, имеют верхний предел длины в 64 знака. Если длина значения из Workday превышает 64 знака, вы увидите это сообщение об ошибке. | Проверьте событие *экспорта* в журналах аудита, чтобы увидеть значение атрибута, указанное в сообщении об ошибке. Попробуйте обрезать значение, полученное из Workday, с помощью функции [Mid](../app-provisioning/functions-for-customizing-application-data.md#mid), или изменить сопоставления на атрибут AD, который не имеет аналогичных ограничений длины.  |
...
Ссылка на [конфигурацию подготовки](../app-provisioning/export-import-provisioning-configuration.md) по статье Экспорт и импорт
