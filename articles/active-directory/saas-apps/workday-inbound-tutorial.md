...
Цель этого учебника — показать, какие действия необходимо выполнить для импорта профилей работников из Workday в Active Directory и Azure Active Directory с необязательной обратной записью электронного адреса и именем пользователя в Workday.
...
* **Обратная запись адресов электронной почты и имени пользователя в Workday** — служба подготовки пользователей Azure AD может выполнять обратную запись адресов электронной почты и имен пользователей из Azure AD в Workday.
...
* **Поток обратной записи электронной почты и имени пользователя — из локального каталога Active Directory в Workday.** По завершении создания учетной записи в Active Directory она синхронизируется с Azure AD через Azure AD Connect, а атрибут электронной почты и имени пользователя можно записать обратно в Workday.
...
7. Если настроен соединитель Workday Writeback, то выполняется обратная запись атрибута электронной почты и имени пользователя в Workday в соответствии с используемым атрибутом.
...
* Действующая лицензия на подписку Azure AD Premium P1 или более поздней версии для каждого пользователя, который присоединится из Workday и будет подготовлен для локальной среды Active Directory или Azure Active Directory.
...
1. В разделе **Учетные данные администратора** заполните поля следующим образом.
   ...
   * **URL-адрес Workday Web Services API** — введите URL-адрес конечной точки веб-служб Workday для вашего клиента. Значение должно иметь следующий вид: `https://wd3-impl-services1.workday.com/ccx/service/contoso4`, где *contoso4* заменяется правильным именем клиента, а *wd3-impl* — правильной строкой среды.

     > [!NOTE]
     > По умолчанию приложение использует веб-службы Workday (WWS) версии 21.1, если в URL-адресе не указана другая версия. Для использования определенной версии API WWS используйте такой формат URL-адреса: https://####.workday.com/ccx/service/tenantName/Human_Resources/v##.# <br>
     > Например, `https://wd3-impl-services1.workday.com/ccx/service/contoso4/Human_Resources/v31.0`. <br>
     
     > [!NOTE]
     > Если вы используете API WWS версии 30.0 и более поздней, прежде чем включать задание подготовки, обновите **выражения API XPATH** в разделе **Сопоставление атрибутов > Дополнительные параметры > Изменить список атрибутов для Workday** в соответствии с разделами [Управление конфигурацией](#managing-your-configuration) и [Описание атрибутов Workday](../app-provisioning/workday-attribute-reference.md#xpath-values-for-workday-web-services-wws-api-v30).  
...
8. В разделе **Учетные данные администратора** заполните поля следующим образом.
   ...
   * **URL-адрес Workday Web Services API** — введите URL-адрес конечной точки веб-служб Workday для вашего клиента. Значение должно иметь следующий вид: `https://wd3-impl-services1.workday.com/ccx/service/contoso4`, где *contoso4* заменяется правильным именем клиента, а *wd3-impl* — правильной строкой среды. Если этот URL-адрес неизвестен, свяжитесь с вашим партнером по интеграции Workday или представителем службы поддержки, чтобы определить правильный URL-адрес для использования.

     > [!NOTE]
     > По умолчанию приложение использует веб-службы Workday версии 21.1, если в URL-адресе не уточнены другие сведения о версии. Чтобы использовать определенную версию WWS API, используйте URL-адрес в таком формате: https://####.workday.com/ccx/service/tenantName/Human_Resources/v##.# <br>
...
В этом разделе описывается настройка потока атрибутов обратной записи из Azure AD в Workday. В настоящее время соединитель поддерживает только обратную запись адреса электронной почты и имени пользователя в Workday.
...
4. Обычно сопоставляется атрибут Azure AD *userPrincipalName* с атрибутом Workday *UserID*, а атрибут *mail* Azure AD с атрибутом Workday *EmailAddress*. Чтобы сохранить сопоставления, щелкните **Сохранить** в верхней части раздела "Сопоставление атрибутов".
