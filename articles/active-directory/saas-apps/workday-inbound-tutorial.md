...
Один соединитель подготовки взаимодействует с API одной исходной системой и помогает подготавливать данные в одной целевой системе. Большинство соединителей подготовки, поддерживаемых Azure AD, предназначены для одной исходной и целевой систем (например, Azure AD в ServiceNow), и их можно настроить, просто добавив нужное приложение из коллекции приложений Azure AD (например, ServiceNow).
...
1. Войдите в клиент Workday с помощью учетной записи администратора. В области **Workday Workbench** введите в поле поиска "create user", а затем щелкните ссылку **Create Integration System User** (Создать пользователя системы интеграции).
    ...
    ![Создание пользователя](./media/workday-inbound-tutorial/IC750979.png "Создание пользователя")
 ...
 * Для параметра **Session Timeout Minutes** (Время ожидания сеанса в минутах) оставьте значение по умолчанию 0, что не позволит сеансам пользователя завершаться раньше времени.
    ...
    ![Создание пользователя системы интеграции](./media/workday-inbound-tutorial/IC750980.png "Создание пользователя системы интеграции")
...
1. Введите в поле поиска текст "создать группу безопасности", а затем щелкните **Создать группу безопасности**.
    ...
    ![Создание группы безопасности](./media/workday-inbound-tutorial/IC750981.png "Создание группы безопасности")
...
4. Создайте группу безопасности, в которую явно будут добавлены участники.
    ...
    ![Создание группы безопасности](./media/workday-inbound-tutorial/IC750982.png "Создание группы безопасности")
...
1. Введите в поле поиска текст "изменить группу безопасности", а затем щелкните **Изменить группу безопасности**.
    ...
    ![Изменение группы безопасности](./media/workday-inbound-tutorial/IC750983.png "Изменение группы безопасности")
1. Найдите и выберите новую группу безопасности интеграции по имени.

    ![Изменение группы безопасности](./media/workday-inbound-tutorial/IC750984.png "Изменение группы безопасности")
2. Добавьте нового пользователя системы интеграции в новую группу безопасности. 

    ![Группа безопасности системы](./media/workday-inbound-tutorial/IC750985.png "Группа безопасности системы")  
...
1. В поле поиска введите **политики безопасности домена**, а затем щелкните ссылку **Domain Security Policies for Functional Area** (Политики безопасности домена для функциональной области).  
    ...
    ![Политики безопасности домена](./media/workday-inbound-tutorial/IC750986.png "Политики безопасности домена")  
2. Найдите систему и выберите функциональную область **Система** .  Последовательно выберите **ОК**.  

    ![Политики безопасности домена](./media/workday-inbound-tutorial/IC750987.png "Политики безопасности домена")  
3. В списке политик безопасности для функциональной области системы разверните узел **Администрирование безопасности** и выберите политику безопасности домена **External Account Provisioning**.  
    ...
    ![Политики безопасности домена](./media/workday-inbound-tutorial/IC750988.png "Политики безопасности домена")  
1. Нажмите кнопку **Изменить разрешения**, а затем на странице диалогового окна **Изменение разрешений** добавьте новую группу безопасности в список групп безопасности с разрешениями интеграции **Get** и **Put**.

    ![Изменение разрешения](./media/workday-inbound-tutorial/IC750989.png "Изменение разрешения")  

1. Повторите шаги 1–4 выше для каждой из оставшихся политик безопасности:
...
1. Введите "активировать" в поле поиска и затем нажмите ссылку **Активировать ожидающие изменения политики безопасности**.
    ...
    ![Активация](./media/workday-inbound-tutorial/IC750992.png "Активация") 
2. Начните выполнять задачу активации ожидающих изменений политики безопасности: введите комментарий для проведения аудита и нажмите кнопку **ОК**. 
    ...
    ![Активация ожидающих изменений безопасности](./media/workday-inbound-tutorial/IC750993.png "Активация ожидающих изменений безопасности")  
1. Завершите задачу на следующем экране, установив флажок **Confirm** (Подтверждаю) и нажав кнопку **ОК**.

    ![Активация ожидающих изменений безопасности](./media/workday-inbound-tutorial/IC750994.png "Активация ожидающих изменений безопасности")  
...
>[!TIP]
> Причиной появления сообщения об ошибке "Сбой связи между основным и доверенным доменами" может быть расположение локального компьютера в среде, в которой настроены несколько лесов или доменов Active Directory, и по крайней мере одно настроенное отношение доверия неисправно или не работает. Чтобы устранить проблему, исправьте или удалите неисправное отношение доверия.
...
![Журналы событий](./media/workday-inbound-tutorial/Windows_Event_Logs.png) 
