...
6. Повторите указанные выше этапы 3–5 для каждой из оставшихся политик безопасности:
   ...
   | Получить | Рабочие данные: рабочие роли |
...
2. В текстовом поле **тип бизнес-процесса** найдите *контакт* и выберите **Рабочий контакт изменить** бизнес-процесс и нажмите кнопку **ОК**.
...
3. На странице **изменение политики безопасности бизнес-процессов** перейдите к разделу **изменение контактной информации (веб-служба)** .
...
* [Настройка подключения к Workday и Active Directory](#part-3-in-the-provisioning-app-configure-connectivity-to-workday-and-active-directory)
...
6. После добавления приложения и отображения экрана сведения о приложении выберите **Подготовка**.

7. Измените режим **подготовки** **Mode** на **Автоматический**.

8. Щелкните баннер сведений, отображающийся, чтобы скачать агент подготовки. 

   ![Скачать агент](./media/workday-inbound-tutorial/pa-download-agent.png "Экран загрузки агента")
...
### <a name="part-3-in-the-provisioning-app-configure-connectivity-to-workday-and-active-directory"></a>Часть 3. Настройка подключения к Workday и Active Directory в приложении подготовки
На этом шаге мы устанавливаем подключение к Workday и Active Directory в портал Azure. 

1. В портал Azure вернитесь к Workday, чтобы Active Directory приложение подготовки пользователей, созданное в [части 1](#part-1-add-the-provisioning-connector-app-and-download-the-provisioning-agent) .
...
* [Здесь есть документация по написанию выражений](../app-provisioning/functions-for-customizing-application-data.md). В этот раздел входят примеры, демонстрирующие удаление специальных символов.
...
| **Коммутатор (\[орган\]государственной власти) OU = Standard Users, ou = users, OU = по умолчанию, OU = Locations, DC = contoso, DC = com "," ДАЛЛАСЕ "," OU = Standard Users, ou = users, OU = Далласе, OU = Locations, DC = contoso, DC = com "," Остин ", OU = Locations, DC = contoso, DC = com", "Сиэтл", "OU = Standard Users, ou = users, OU = Сиэтл, OU = Locations, DC = contoso, DC = com", "Лондон", "OU = Standard Users, ou = users = Contoso, DC = com")**  | parentDistinguishedName     |     |  Создание и обновление |
...
8. В разделе **Учетные данные администратора** заполните поля следующим образом.
   ...
   * **Имя пользователя Workday** — введите имя пользователя учетной записи системы интеграции Workday с добавленным доменным именем клиента. Оно должно иметь примерно следующий вид: username@contoso4
   ...
   * Если проверка подключения будет выполнена, нажмите кнопку **сохранить** вверху. В случае неудачи дважды проверьте правильность URL-адреса и учетных данных Workday в приложении Workday.
...
2. В поле **область исходного объекта** можно выбрать, какие наборы пользователей в Workday должны находиться в области для подготовки в Azure AD, определив набор фильтров на основе атрибутов. Область по умолчанию — "Все пользователи в Workday". Примеры фильтров
...
3. В поле **Действия с целевыми объектами** можно в глобальном масштабе отфильтровать, какие действия выполняются в Azure AD. Наиболее часто **создаются** и **обновляются** .
...
6. Чтобы сохранить сопоставления, щелкните **сохранить** в верхней части раздела сопоставления атрибутов.
...
8. В разделе **Учетные данные администратора** заполните поля следующим образом.
   ...
   * **Имя пользователя администратора** — введите имя пользователя учетной записи системы интеграции Workday с добавленным доменным именем клиента. Должен выглядеть примерно так: *username\@contoso4*
   ...
   * Нажмите кнопку **Проверить подключение**. Если проверка подключения будет выполнена, нажмите кнопку **сохранить** вверху. В случае неудачи дважды проверьте правильность URL-адреса и учетных данных Workday в приложении Workday.
...
2. В поле **область исходного объекта** можно дополнительно отфильтровать, какие наборы пользователей в Azure Active Directory должны иметь свои адреса электронной почты, записанные в Workday. Область по умолчанию — "Все пользователи в Azure AD".
...
4. Обычно вы сопоставляете атрибут *userPrincipalName* Azure AD с атрибутом workday *UserID* и сопоставляете атрибут *почты* Azure AD с атрибутом *EmailAddress* . Чтобы сохранить сопоставления, щелкните **сохранить** в верхней части раздела сопоставления атрибутов.
...
Сейчас такая возможность не поддерживается. Рекомендуемый обходной путь заключается в развертывании скрипта PowerShell, который запрашивает у конечной точки Microsoft Graph API [данные журнала аудита](https://docs.microsoft.com/graph/api/resources/azure-ad-auditlog-overview?view=graph-rest-beta) и использует их для запуска таких сценариев, как назначение групп. Этот сценарий PowerShell можно присоединить к планировщику задач и развернуть в том же окне, где запущен агент подготовки.  
...
* Формат **URL-адреса API веб-служб Workday** , используемый в разделе **учетных данных администратора** , определяет версию API, используемую для Get_Workers
  * Если URL-адрес имеет формат:\#\#\#\#\.HTTPS://\.workday com/КККС/Service/tenantName, то используется API v 21.1. 
  * Если URL-адрес имеет формат:\#\#\#\#\.HTTPS://\.workday com/КККС/Service/tenantName/\_людские ресурсы, то используется API v 21.1 
  * Если URL-адрес имеет формат:\#\#\#\#\.HTTPS://\.workday com/КККС/Service/tenantName/\_HID/v\# \# \. \# , то используется указанная версия API. (Пример: Если указана версия 34.0, используется.)  
   
* Функция обратной записи электронной почты Workday использует Change_Work_Contact_Information (v 30,0) 
* Функция обратной записи имени пользователя Workday использует Update_Workday_Account (v 31,2) 
...
* Выберите **панель** -> управления**Удаление или изменение меню программы** .
...
* В списке агентов, которые отображаются, скопируйте значение `id` поля из этого ресурса, *resourceName* которого РАВНЯЕТСЯ доменному имени AD.
...
* Чтобы создать правильное выражение сопоставления атрибута, необходимо выяснить, какой атрибут Workday представляет собой имя пользователя, фамилию, страну, регион и отдел. Предположим, у нас есть атрибуты *PreferredFirstName*, *PreferredLastName*, *CountryReferenceTwoLetter* и *SupervisoryOrganization* соответственно. Их можно использовать, чтобы создать приведенное ниже выражение для атрибута AD *displayName* и получить отображаемое имя, например *Воронков, Иван (Маркетинг-Россия)*.
    ...
    ```
    После получения правого выражения измените таблицу сопоставления атрибутов и измените сопоставление атрибутов *DisplayName* , как показано ниже: сопоставление DisplayName. ![](./media/workday-inbound-tutorial/wd_displayname_map.png)
...
* [Основные сведения о журналах для операций обновления диспетчера](#understanding-logs-for-manager-update-operations)
...
При обнаружении нового сотрудника в Workday (например, с идентификатором *21023*) служба подготовки Azure AD пытается создать новую учетную запись пользователя AD для него и в процессе создает 4 записи журнала аудита, как описано ниже.
  ...
  [![Журнал аудита создание операций](media/workday-inbound-tutorial/wd_audit_logs_02.png)](media/workday-inbound-tutorial/wd_audit_logs_02.png#lightbox)
...
|2.| Служба Windows "Агент подготовки Microsoft Azure AD Connect" находится в состоянии *Запускается* и не переходит в состояние *Выполняется*. | В ходе установки мастер создания агента создает локальную учетную запись (**NT Service\\аадконнектпровисионингажент**) на сервере, а это учетная запись входа, используемая для запуска службы. Эта ошибка возникнет, если политика безопасности на сервере Windows запрещает локальным учетным записям запускать службы. | Откройте *консоль служб*. Щелкните правой кнопкой мыши службу Windows "Microsoft Azure AD подключить агент подготовки" и на вкладке "вход в систему" укажите учетную запись администратора домена для запуска службы. Перезапустите службу. |
|3.| При настройке агента подготовки с доменом AD на шаге *Connect Active Directory*мастер предпринимает слишком много времени, пытаясь загрузить схему AD и в конечном итоге истечет время ожидания. | Эта ошибка обычно отображается, если мастеру не удается связаться с сервером контроллера домена AD из-за проблем брандмауэра. | На экране *Подключение к Active Directory* мастеру, в ходе которого предоставляются учетные данные для домена AD, можно *выбрать вариант Выбор приоритета контроллера домена*. Используя этот параметр, выберите контроллер домена, который находится на том же сайте, что и сервер агента, и убедитесь в отсутствии правил брандмауэра, блокирующих связь. |
...
2. Скачайте файл Workday **Human_Resources** WSDL, относящийся к версии API WWS, которую вы планируете использовать из [каталога веб-служб Workday](https://community.workday.com/sites/default/files/file-hosting/productionapi/index.html) .
