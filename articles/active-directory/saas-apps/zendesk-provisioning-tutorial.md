...
1. Войдите в [портала Azure](https://portal.azure.com) и выберите **корпоративные приложения**выберите **все приложения**, а затем выберите **Zendesk**.
...
6. **Маркер секрета** для учетной записи Zendesk можно найти в разделе **Администрирование > API > Параметры**.
   Параметру **Token Access** (Доступ по маркеру) должно быть задано состояние **Enabled** (Включено).
    ...
    ![Подготовка Zendesk](./media/zendesk-provisioning-tutorial/ZenDesk4.png)

    ![Подготовка Zendesk](./media/zendesk-provisioning-tutorial/ZenDesk2.png)
