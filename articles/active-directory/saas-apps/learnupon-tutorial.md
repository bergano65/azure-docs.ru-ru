---
title: Руководство. Интеграция Azure Active Directory с LearnUpon | Документация Майкрософт
...
1. В разделе **General Settings** (Общие параметры) выполните следующие действия.
    ...
    4.3. В текстовое поле **SAML Token Post param name** (Имя параметра POST для токена SAML) введите имя параметра POST, передаваемого в запросе на указанный выше URL-адрес клиента SAML (в нем содержится требующее проверки и аутентификации утверждение SAML, например **SAMLResponse**).
...
В этом разделе никакие действия с вашей стороны не требуются. Пользователь будет создан при попытке получить доступ к приложению LearnUpon (если он еще не создан). Продолжите настройку единого входа в Azure AD.
