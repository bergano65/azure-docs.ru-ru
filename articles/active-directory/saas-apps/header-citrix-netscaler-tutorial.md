...
Чтобы создать виртуальный сервер, выполните следующие действия:
...
1. В поле **Имя** введите имя сервера SAML для проверки подлинности.

1. В разделе **Экспорт метаданных SAML** выполните следующее.

   1. Установите флажок **Импортировать метаданные**.

   1. Введите URL-адрес метаданных федерации из пользовательского интерфейса SAML Azure, скопированного ранее.
    
1. В поле **Имя поставщика** введите соответствующий URL-адрес.

1. Щелкните **Создать**.

![Настройка Citrix NetScaler — панель "Создание сервера SAML для проверки подлинности"](./media/header-citrix-netscaler-tutorial/server01.png)

### <a name="create-an-authentication-virtual-server"></a>Создание виртуального сервера проверки подлинности

Чтобы создать виртуальный сервер проверки подлинности, выполните следующие действия.

1.  Выберите **Безопасность** > **AAA- трафик приложений** > **Политики** > **Проверка подлинности** > **Виртуальные серверы проверки подлинности**.

1.  Выберите **Добавить** и выполните следующие действия.

    1. В поле **Имя** введите имя для виртуального сервера проверки подлинности.

    1. Установите флажок **Без адресации**.

    1. Для параметра **Протокол** выберите значение **SSL**.
...
1.  На панели **Расширенные политики проверки подлинности** выберите **Без политики проверки подлинности**.
...
1. На панели **Привязка политики** выберите политику проверки подлинности, а затем выберите **Привязка**.

    ![Настройка Citrix NetScaler — панель привязки политики](./media/header-citrix-netscaler-tutorial/virtual02.png)

1. На панели **Виртуальные серверы на основе формы** выберите **Виртуальный сервер без балансировки нагрузки**.
...
1. В поле **Полное доменное имя для проверки подлинности** введите полное доменное имя (FQDN) (обязательно).
...
1. Выберите **Привязка**.
    ...
    > [!NOTE]
    > Не забудьте выбрать **Готово** на панели **Конфигурация виртуального сервера проверки подлинности**.

1. Чтобы проверить изменения, в браузере перейдите по URL-адресу приложения. Вы должны увидеть страницу входа клиента вместо доступа без проверки подлинности, который вы видели ранее.
...
1. В списке виртуальных серверов выберите виртуальный сервер, к которому требуется привязать политику перезаписи, а затем выберите **Открыть**.

1. В области **Виртуальный сервер балансировки нагрузки** в разделе **Расширенные параметры** выберите **Политики**. Все политики, настроенные для экземпляра NetScaler, отображаются в списке.
...
1.  Установите флажок рядом с именем политики, которую вы хотите связать с этим виртуальным сервером.
    ...
    ![Конфигурация Citrix NetScaler — панель привязки политики трафика виртуального сервера для балансировки нагрузки](./media/header-citrix-netscaler-tutorial/header08.png)

1. В диалоговом окне **Выбор типа** выполните следующее:

    1. Для параметра **Выбор политики** выберите **Трафик**.
