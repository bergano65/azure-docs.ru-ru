...
ms.locfileid: "70165527"
---
# <a name="tutorial-azure-active-directory-single-sign-on-sso-integration-with-f5"></a>Руководство по интеграции единого входа Azure Active Directory с F5
...
1. Необходимо импортировать сертификат метаданных в F5 (приложение на основе заголовков), который будет использоваться позже в процессе настройки. Выберите **System > Certificate Management > Traffic Certificate Management >> SSL Certificate List** (Система > Управление сертификатами > Управление сертификатами трафика > Список SSL-сертификатов). Нажмите кнопку **Import** (Импортировать) в правом верхнем углу.

    ![Настройка F5 (приложение на основе заголовков)](./media/headerf5-tutorial/configure12.png)
 
1. Кроме того, вам потребуется **SSL-сертификат** для имени узла (`headerapp.superdemo.live`). В этом примере мы использовали групповой сертификат.

    ![Настройка F5 (приложение на основе заголовков)](./media/headerf5-tutorial/configure13.png)

1. Выберите **F5 (Header Based) BIG-IP Click Access > Guided Configuration > Federation > SAML Service Provider** (BIG-IP F5 (на основе заголовков) > Доступ > Интерактивная конфигурация > > Поставщик службы SAML).
...
1. Создайте виртуальный сервер, укажите **адрес назначения**. **Порт перенаправления** указывать необязательно. Выберите **групповой сертификат** (или **сертификат**, отправленный для приложения), который мы отправляли ранее, и **связанный закрытый ключ**.
...
1. В разделе единого входа выберите **HTTP header-based** (На основе заголовков HTTP). Можно добавить другие заголовки на основе приложения. Список переменных сеанса SAML см. в приложении.

    ![Настройка F5 (приложение на основе заголовков)](./media/headerf5-tutorial/configure07.png) 

1. Обратитесь к [группе поддержки клиентов приложения F5](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45) (на основе заголовков) для **получения сведений о свойствах проверки конечной точки**.

    ![Настройка F5 (приложение на основе заголовков)](./media/headerf5-tutorial/configure08.png)

1. Обратитесь к [группе поддержки клиентов F5](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45) (на основе заголовка) для **получения сведений о свойствах проверки конечной точки**.
