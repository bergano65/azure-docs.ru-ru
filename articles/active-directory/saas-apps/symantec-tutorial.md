...
4. В диалоговом окне **Базовая конфигурация SAML** выполните следующие действия.
    ...
    > [!NOTE]
    > Если по какой-либо причине предложенные значения [Идентификатора](https://www.symantec.com/contact-us) и **URL-адреса ответа** не работают, обратитесь в **группу поддержки Symantec Web Security Service (WSS)** . Можно также посмотреть шаблоны в разделе **Базовая конфигурация SAML** на портале Azure.
