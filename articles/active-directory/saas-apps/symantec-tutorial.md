...
2. **[Настройка единого входа для Symantec Web Security Service (WSS)](#configure-symantec-web-security-service-(wss)-single-sign-on)** необходима, чтобы настроить параметры единого входа на стороне приложения.
...
4. В диалоговом окне **Базовая конфигурация SAML** выполните следующие действия.
    ...
    > [!NOTE]
    > Если по какой-либо причине предложенные значения **Идентификатора** и **URL-адреса ответа** не работают, обратитесь в [группу поддержки Symantec Web Security Service (WSS)](https://www.symantec.com/contact-us). Можно также обратиться к шаблонам, указанным в разделе **Базовая конфигурация SAML** на портале Azure.
