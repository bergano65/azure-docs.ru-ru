---
title: 'Учебник: Налаживайте Mixpanel для автоматического подготовки пользователей с помощью Active Directory Azure (ru) Документы Майкрософт'
description: Узнайте, как автоматически предоставлять и депредоставлять учетные записи пользователей от Azure AD до Mixpanel.
...
---
# <a name="tutorial-configure-mixpanel-for-automatic-user-provisioning"></a>Учебник: Налаживание Mixpanel для автоматического подготовки пользователей

В этом уроке описаны шаги, которые необходимо выполнить как в Mixpanel, так и в Active Directory Azure (Azure AD) для настройки автоматической подготовки пользователей. При настройке Azure AD автоматически провизии и де-положения пользователей и групп [к Mixpanel](https://mixpanel.com/pricing/) с помощью службы обеспечения Azure AD. Подробные сведения о том, что делает эта служба, как она работает, и часто задаваемые вопросы см. в статье [Автоматическая подготовка пользователей и ее отзыв для приложений SaaS в Azure Active Directory](../manage-apps/user-provisioning.md). 


## <a name="capabilities-supported"></a>Возможности, поддерживаемые
> [!div class="checklist"]
> * Создание пользователей в Mixpanel
> * Удалите пользователей в Mixpanel, если они больше не требуют доступа
> * Сохраняйте синхронизацию атрибутов пользователя между Azure AD и Mixpanel
> * Группы по предоставлению и членству в группах в Mixpanel
> * [Однократный знак на](https://docs.microsoft.com/azure/active-directory/saas-apps/mixpanel-tutorial) Mixpanel (рекомендуется)
...
* Организация микспанель корпоративного уровня
* Учетная запись mixpanel с привилегиями админа на указанной орге
* SSO включен в микспанель с заявленным доменом
...
3. Определите, какие данные следует [сопоставить между Azure AD и Mixpanel.](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes) 

## <a name="step-2-configure-mixpanel-to-support-provisioning-with-azure-ad"></a>Шаг 2. Налаживание Mixpanel для поддержки подготовки с Azure AD
1. Для настройки SSO и утверждая, домен относятся [к этому](https://help.mixpanel.com/hc/articles/360036428871-Single-Sign-On).
2. После этого вам нужно будет создать токен SCIM во вкладке SCIM в разделе безопасности доступа настроек вашей организации.
![Токен Mixpanel](./media/mixpanel-provisioning-tutorial/mixpanelscim.png)


## <a name="step-3-add-mixpanel-from-the-azure-ad-application-gallery"></a>Шаг 3. Добавьте Mixpanel из галереи приложений Azure AD

Добавьте Mixpanel из галереи приложений Azure AD, чтобы начать управление подготовкой к Mixpanel. Если у вас есть ранее настройка Mixpanel для SSO вы можете использовать то же приложение. Однако рекомендуется создать отдельное приложение при тестировании интеграции на начальном этапе. Подробнее о добавлении приложения из галереи [можно](https://docs.microsoft.com/azure/active-directory/manage-apps/add-gallery-app)здесь. 
...
## <a name="step-5-configure-automatic-user-provisioning-to-mixpanel"></a>Шаг 5. Настройка автоматического пользовательского обеспечения для Mixpanel 
...
### <a name="to-configure-automatic-user-provisioning-for-mixpanel-in-azure-ad"></a>Для настройки автоматического пользовательского обеспечения Для Mixpanel в Azure AD:
...
5. В разделе **Admin Credentials** ввейте **URL-адрес** Mixpanel Tenant и **секретный токен.** Нажмите **на тестовое соединение,** чтобы обеспечить подключение Azure AD к Mixpanel. Если соединение не удается, убедитесь, что ваша учетная запись Mixpanel имеет разрешения Admin и повторите попытку.
...
8. В разделе **Картпинги** выберите **синхронизацию активных пользователей каталога Azure mixpanel.**

9. Просмотрите атрибуты пользователя, синхронизированные с Azure AD с Mixpanel в разделе **Атрибут-Картирование.** Атрибуты, выбранные в качестве свойств **соответствия,** используются для сопоставления учетных записей пользователей в Mixpanel для операций обновления. Если вы решите изменить [соответствующий целевой атрибут,](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)необходимо убедиться, что API Mixpanel поддерживает фильтрацию пользователей на основе этого атрибута. Нажмите кнопку **Сохранить**, чтобы зафиксировать все изменения.
...
10. В разделе **Картпинги** выберите **синхронизацию активных групп каталогов Azure в Mixpanel.**

11. Просмотрите атрибуты группы, синхронизированные с Azure AD с Mixpanel в разделе **Атрибут-Картирование.** Атрибуты, выбранные в качестве свойств **соответствия,** используются для соответствия группам в Mixpanel для операций обновления. Нажмите кнопку **Сохранить**, чтобы зафиксировать все изменения.
...
13. Чтобы включить службу предоставления Услуг Azure AD для Mixpanel, измените **статус подготовки** **в** разделе **«Настройки».**

    ![Состояние подготовки "Включено"](common/provisioning-toggle-on.png)

14. Определите пользователей и/или группы, которые вы хотели бы предоставить Mixpanel, выбрав желаемые значения в **разделе Область** в разделе **Параметры.**
