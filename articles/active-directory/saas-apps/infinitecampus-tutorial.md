...
- подписка Azure AD;
- Бесконечный Campus единого входа подписка с поддержкой
...
- Как минимум необходимо иметь права администратора Azure Active Directory и иметь роль безопасности продукта Campus из «Student сведения системы копий (SIS)» для завершения настройки.
...
3. **[Создание тестового пользователя бесконечный Campus](#creating-an-infinite-campus-test-user)**  — требуется для создания пользователя Britta Simon в бесконечную Campus, связанного с одноименным пользователем в Azure AD.
...
4. На **базовой конфигурации SAML** раздел, если у вас есть **файл метаданных поставщика службы** экспортирован из бесконечного Campus, выполните шаги 4.a через 4.d, а затем переходите к шагу 11.c. Если у вас нет файла метаданных поставщика служб, перейдите к шагу 5.
...
5. Если у вас нет **файла метаданных поставщика службы**, выполните следующие действия (обратите внимание, что домен зависит от модели размещения):
...
7. В разделе **Настройка Infinite Campus** используйте следующие значения для проверки при отправке или использовании URL-адреса или файла метаданных Azure.
...
11. На странице **SSO Service Provider Configuration** (Настройки поставщика службы единого входа) выполните следующие действия:
    ...
    2. Изменить **необязательное имя атрибута** должен содержать **имя**
    
    c. На **выберите параметр, чтобы получить данные сервера поставщика удостоверений (IDP)** выберите **URL-адрес метаданных**, вставьте **URL-адрес метаданных федерации приложения** (из шага 6 выше) в поле, а затем щелкните **синхронизации**.

    d. Щелкните ссылку **Service Provider Metadata** (Метаданные поставщика служб), чтобы сохранить **файл метаданных поставщика услуг** на своем компьютере, и отправьте его в раздел **Basic SAML Configuration** (Базовая конфигурация SAML), чтобы автоматически заполнить значения **идентификатора** и **URL-адреса ответа** на портале Azure (см. шаг 4 для отправки и автоматического заполнения значений или шаг 5 для ввода вручную).

    д. После нажатия элемента **Синхронизация** значения автоматически заполнятся на странице **SSO Service Provider Configuration** (Настройка поставщика услуг единого входа).

    Е. Выберите команду **Сохранить**.

### <a name="creating-an-azure-ad-test-user"></a>Создание тестового пользователя Azure AD

Цель этого раздела — создать на портале Azure _одного_ тестового пользователя с именем Britta Simon.
...
### <a name="creating-an-infinite-campus-test-user"></a>Создание тестового пользователя бесконечный Campus
...
Щелкнув плитку Infinite Campus на панели доступа, вы должны автоматически войти в свое приложение Infinite Campus. Если вход в приложение бесконечный Campus в том же браузере, при администрировании Azure AD, убедитесь, что вы вошли в Azure AD в качестве тестового пользователя. Дополнительные сведения о панели доступа см. в статье с [общими сведениями о панели доступа](../user-help/active-directory-saas-access-panel-introduction.md).
