...
## <a name="configure-and-test-azure-ad-sso-for-qlik-sense-enterprise"></a>Настройка и проверка единого входа Azure AD для Qlik Sense Enterprise
...
Чтобы настроить и проверить единый вход Azure AD в Qlik Sense Enterprise, сделайте следующее.
...
1. **[Настройка единого входа в Qlik Sense Enterprise](#configure-qlik-sense-enterprise-sso)** необходима, чтобы настроить параметры единого входа на стороне приложения.
    1. **[Создание тестового пользователя Qlik Sense Enterprise](#create-qlik-sense-enterprise-test-user)** требуется, чтобы в Qlik Sense Enterprise существовал пользователь Britta Simon, связанный с одноименным пользователем в Azure AD.
...
1. На портале Azure на странице интеграции с приложением **Qlik Sense Enterprise** найдите раздел **Управление** и выберите **Единый вход**.
...
1. Подготовьте XML-файл метаданных федерации для отправки на сервер Qlik Sense.
    ...
    ![Снимок экрана: окно Visual Studio Code, в котором содержится XML-файл метаданных федерации.][qs24]
...
3. В QMC щелкните пункт меню **Virtual Proxies** (Виртуальные прокси-серверы).
    ...
    ![Снимок экрана: выбранный в пункте CONFIGURE SYSTEM (Настройка системы) параметр "Виртуальные прокси-серверы".][qs6]

4. В нижней части страницы нажмите кнопку **Create new** (Создать).

    ![Снимок экрана: параметр Create new (Создать).][qs7]
...
5. Появится экран Virtual proxy edit (Редактирование виртуального прокси-сервера).  В правой части экрана расположено меню для отображения параметров конфигурации.
    ...
    ![Снимок экрана: выбранный в меню "Свойства" пункт "Идентификация".][qs9]
...
6. Установите галочку возле пункта меню Identification (Идентификация) и введите идентифицирующие сведения для конфигурации виртуального прокси-сервера Azure.
    ...
    ![Снимок экрана: раздел Edit virtual proxy (Изменение виртуального прокси-сервера) для параметра Identification (Идентификация), в котором можно ввести указанные значения.][qs8]  
...
7. Щелкните пункт меню Authentication (Проверка подлинности), чтобы сделать его видимым.  Появится экран Authentication (Проверка подлинности).
    ...
    ![Снимок экрана: раздел Edit virtual proxy (Изменение виртуального прокси-сервера) для параметра Authentication (Аутентификация), в котором можно ввести указанные значения.][qs10]
...
8. Щелкните пункт меню **Load balancing** (Балансировка нагрузки), чтобы сделать его видимым.  Появится экран Load balancing (Балансировка нагрузки).
    ...
    ![Снимок экрана: экран Virtual proxy edit (Изменение виртуального прокси-сервера) для пункта LOAD BALANCING (Балансировка нагрузки), в котором можно выбрать пункт Add new server node (Добавить узел сервера).][qs11]
...
9. Нажмите кнопку **Add new server node** (Добавить новый узел сервера), выберите узел или узлы обработчика, на которые Qlik Sense будет отправлять сеансы для балансировки нагрузки, а затем нажмите кнопку **Add** (Добавить).
    ...
    ![Снимок экрана: диалоговое окно Add server nodes to load balance on (Добавить узлы сервера для распределения нагрузки), в котором можно добавить серверы.][qs12]

10. Щелкните пункт меню Advanced (Дополнительно), чтобы сделать его видимым. Откроется экран Advanced (Дополнительно).

    ![Снимок экрана: экран Edit virtual proxy (Изменение виртуального прокси-сервера) для пункта Advanced (Расширенный).][qs13]
...
11. Нажмите кнопку **Применить**.
    ...
    ![Снимок экрана: кнопка Apply (Применить).][qs14]
...
12. Нажмите кнопку OK (ОК), чтобы принять предупреждение о перезапуске прокси-серверов, связанных с виртуальным прокси-сервером.
    ...
    ![Снимок экрана: сообщение с подтверждением Apply changes to virtual proxy (Применить изменения к виртуальному прокси-серверу).][qs15]
...
13. В правой части экрана отображается меню Associated items (Связанные элементы).  Щелкните пункт меню **Proxies** (Прокси-серверы).
    ...
    ![Снимок экрана: выбранный пункт Proxies (Прокси-серверы) меню Associated items (Связанные элементы).][qs16]
...
14. Появится экран прокси-сервера.  Щелкните кнопку **Link** (Связать) внизу, чтобы связать прокси-сервер с виртуальным прокси-сервером.
    ...
    ![Снимок экрана: кнопка Link (Связать).][qs17]
...
15. Выберите узел прокси-сервера, который будет поддерживать подключение к этому виртуальному прокси-серверу, и щелкните кнопку **Link** (Связать).  После установки связи прокси-сервер отобразится в списке связанных прокси-серверов.
    ...
    ![Снимок экрана: окно Select proxy services (Выбор служб прокси-серверов).][qs18]
  
    ![Снимок экрана: подпункт Associated proxies (Связанные прокси-серверы) пункта Virtual proxy associated items (Связанные элементы виртуального прокси-сервера).][qs19]

16. Через 5–10 секунд появится сообщение об обновлении QMC.  Нажмите кнопку **Refresh QMC** (Обновить QMC).

    ![Снимок экрана: сообщение Your session has ended (Сеанс завершен).][qs20]
...
17. После обновления QMC щелкните пункт меню **Virtual proxies** (Виртуальные прокси-серверы). Новая запись виртуального прокси-сервера SAML указана в таблице на экране.  Выделите запись виртуального прокси-сервера.
    ...
    ![Снимок экрана: экран Virtual proxies (Виртуальные прокси-серверы) с одной записью.][qs51]
...
18. В нижней части экрана станет активной кнопка Download SP metadata (Скачать метаданные SP).  Щелкните **Download SP metadata** (Скачать метаданные SP), чтобы сохранить метаданные в файл.
    ...
    ![Снимок экрана: кнопка Download SP metadata (Скачать метаданные SP).][qs52]
...
19. Откройте файл метаданных SP.  Обратите внимание на записи **entityID** и **AssertionConsumerService**.  Значения этих записей должны быть эквивалентны **идентификатору**, **URL-адресу для входа** и **URL-адресу ответа** в конфигурации приложения Azure AD. Вставьте эти значения в разделе **Домены и URL-адреса приложения Qlik Sense Enterprise** в параметрах конфигурации приложения в Azure AD. Если они не совпадают, измените их через мастер настройки приложения Azure AD.
    ...
    ![Снимок экрана: редактор обычного текста с вызванными EntityDescriptor с entityID и AssertionConsumerService.][qs53]
...
* Выберите **Тестировать приложение** на портале Azure. Вы будете перенаправлены по URL-адресу для входа в Qlik Sense Enterprise, где можно инициировать поток входа. 
