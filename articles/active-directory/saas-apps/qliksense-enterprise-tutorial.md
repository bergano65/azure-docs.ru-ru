---
title: Руководство по Интеграция Azure Active Directory с Qlik Sense Enterprise | Документация Майкрософт
...
1. **[Настройка единого входа в Qlik Sense Enterprise](#configure-qlik-sense-enterprise-sso)** необходима, чтобы настроить параметры единого входа на стороне приложения.
    * **[Создание тестового пользователя Qlik Sense Enterprise](#create-qlik-sense-enterprise-test-user)** требуется, чтобы в Qlik Sense Enterprise существовал пользователь Britta Simon, связанный с одноименным пользователем в Azure AD.
...
1. На странице **Базовая конфигурация SAML** введите значения для следующих полей.
    ...
    а. В текстовом поле **URL-адрес для входа** введите URL-адрес в следующем формате: `https://<Fully Qualified Domain Name>:443{/virtualproxyprefix}/hub`
...
1. Подготовьте XML-файл метаданных федерации для отправки на сервер Qlik Sense.
    ...
    а. Откройте в текстовом редакторе файл FederationMetaData.xml, скачанный с портала Azure.
...
6. Установите галочку возле пункта меню Identification (Идентификация) и введите идентифицирующие сведения для конфигурации виртуального прокси-сервера Azure.
    ...
    а. В поле **Description** (Описание) укажите понятное имя для конфигурации виртуального прокси-сервера.  Введите значение в поле Description (Описание).
    ...
    d. **Session cookie header name** (Имя заголовка сеанса в файле cookie) — имя cookie, которое содержит идентификатор для сеанса Qlik Sense, полученный пользователем после успешной проверки подлинности.  Имя должно быть уникальным.
...
Qlik Sense Enterprise поддерживает **JIT-подготовку**: пользователи автоматически добавляются в репозиторий USERS (ПОЛЬЗОВАТЕЛИ) в Qlik Sense Enterprise по мере использования функции единого входа. Кроме того, клиенты могут использовать QMC и создать UDC (соединитель каталога пользователей) для предварительного заполнения списка пользователей в Qlik Sense Enterprise из выбранного решения для передачи по протоколу LDAP, например Active Directory и прочих.
