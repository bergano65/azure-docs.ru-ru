...
6. Для получения клиента, URL-адрес и маркер секрета, перейдите к **администрирования** > **параметры проверки подлинности** в Zscaler One пользовательском Интерфейсе портала. В разделе **Authentication Type** (Тип проверки подлинности) выберите **SAML**.
