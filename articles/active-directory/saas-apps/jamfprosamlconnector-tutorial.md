...
7. Прокрутите эту страницу до элемента **IDENTITY PROVIDER** (Поставщик удостоверений) в разделе **Single Sign-On** (Единый вход) и выполните следующие действия:
    ...
    4.3. Скопируйте значение **Entity ID** (Идентификатор сущности) и вставьте его в текстовое поле **Идентификатор (сущности)** в разделе **Домены и URL-адреса приложения Jamf Pro** на портале Azure.
...
6. В диалоговом окне **Новая учетная запись** сделайте следующее:
    ...
    4.3. В текстовом поле **EMAIL ADDRESS** (Электронная почта) введите адрес электронной почты для учетной записи Britta Simon.
