---
title: Справочник по перемещаемым параметрам Windows 10 — Azure Active Directory
description: Параметры, которые будут перемещены или архивированы в Windows 10 с помощью ESR
...
ms.locfileid: "85252973"
---
# <a name="windows-10-roaming-settings-reference"></a>Справочник по перемещаемым параметрам в Windows 10

Ниже приведен список параметров, которые будут перемещены или архивированы в Windows 10. 
...
| Тип учетной записи и операция | Классические приложения | Мобильные службы |
| --- | --- | --- |
...
Следующие группы параметров доступны конечным пользователям для включения и отключения синхронизации параметров на устройствах Windows 10.
...
* Параметр браузера Microsoft ребра: Избранное Microsoft ребр, список для чтения и другие параметры
...
В следующей таблице другие записи в столбце Группа параметров ссылаются на параметры, которые можно отключить, выбрав параметры > учетные записи > синхронизации параметров > других параметров Windows. 
...
| Параметры | Классические приложения | Мобильные службы | Группа |
| --- | --- | --- | --- |
| **Учетные записи**: аватар |sync |X |Тема |
| **Учетные записи**: другие параметры учетных записей |X |X | |
| **Расширенное мобильное широкополосное**подключение: сетевое имя общего доступа к подключению к Интернету (включает автоматическое обнаружение точек подключения Wi-Fi мобильных устройств через Bluetooth) |X |X |Пароли |
...
| **Список приложений**: список установленных приложений |X |резервная копия |Другое |
...
| **Устройства**: общие принтеры, к которым подключен компьютер |X |X |др. |
...
| **Мышь**: изменение размера указателя мыши |sync |X |др. |
| **Мышь**: изменение цвета указателя мыши |sync |X |др. |
