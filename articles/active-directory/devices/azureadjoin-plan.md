---
title: Как спланировать присоединение к реализации активного каталога Azure
...
Чтобы спланировать реализацию присоединения Azure AD, следует ознакомиться с:
...
>[!NOTE]
> В настоящее время соединение Azure AD не работает с [AD FS 2019, настроенным с внешними поставщиками аутентификации в качестве основного метода проверки подлинности.](/windows-server/identity/ad-fs/operations/additional-authentication-methods-ad-fs#enable-external-authentication-methods-as-primary) Azure AD соединяет по умолчанию для проверки подлинности паролей в качестве основного метода, что приводит к сбоям в проверке подлинности в этом сценарии
...
Управление устройствами для устройств, объединенных Azure AD, основано на платформе MDM, такой как Intune и MDM CSPs. В Windows 10 есть встроенный агент MDM, работающий со всеми совместимыми решениями MDM.

> [!NOTE]
> Политики группы не поддерживаются в подключенных к Azure AD устройствах, так как они не подключены к предприимчивому Active Directory. Управление устройствами, объединенными Azure AD, возможно только через MDM
...
При совместном управлении можно использовать SCCM для управления определенными аспектами устройств, в то время как политики доставляются через платформу MDM. Microsoft Intune предоставляет возможность совместного управления с SCCM. Для получения дополнительной информации о совместном управлении для устройств Windows 10, [см.](/configmgr/core/clients/manage/co-management-overview) Если вы используете продукт MDM, отличный от Intune, обратитесь к поставщику MDM, чтобы узнать о применимых сценариях совместного управления.
...
Эту реализацию можно использовать для того, чтобы [требовать управляемых устройств для доступа к облачным приложениям с помощью условия условного доступа.](../conditional-access/require-managed-devices.md)

## <a name="next-steps"></a>Дальнейшие действия

> [!div class="nextstepaction"]
> [Присоединиться к новому устройству Windows 10 с Azure AD во время первого запуска](azuread-joined-devices-frx.md)
> [Присоединяйтесь к рабочему устройству в сети вашей организации](/azure/active-directory/user-help/user-help-join-device-on-network)
