...
description: Узнайте, как пользователи могут настроить присоединение к Azure AD во время первого запуска.
...
1. Ваше устройство найдет соответствующий арендатор в Azure AD. Если ваше устройство находится в федеративном домене, то вы будете перенаправлены на локальный сервер службы токенов безопасности, например в службы федерации Active Directory (AD FS).
