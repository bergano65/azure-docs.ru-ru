...
Чтобы управлять устройствами с помощью портала Azure, эти устройства нужно [зарегистрировать в Azure AD или присоединить](overview.md#getting-devices-under-the-control-of-azure-ad) к Azure AD. Администратор может точно настроить процесс регистрации и присоединения устройств, задав параметры устройства. 
...
- **Пользователи могут присоединять устройства к Azure AD**. Этот параметр позволяет выбрать пользователей, которые могут [присоединять устройства](overview.md#azure-ad-joined-devices) к Azure AD. Значение этого параметра по умолчанию — **Все**. Этот параметр применяется только для присоединения к Azure AD в ОС Windows 10.
...
- администраторы службы технической поддержки;
- администраторы безопасности;
- читатели сведений о безопасности;
- администраторы службы Intune.
