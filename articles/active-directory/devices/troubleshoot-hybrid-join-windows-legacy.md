---
title: Устранение проблем устаревшего гибридного каталога Azure Active
...
- AD FS или Azure AD, или сеть настроены неправильно
   ...
   - Возможно, многофакторная аутентификация (MFA) включена/настроена для пользователя, а WIAORMULTIAUTHN не настроена на сервере AD FS. 
   ...
   - **Autoworkplace.exe** требует, чтобы клиент имеет прямую линию видимости от клиента к контроллеру домена AD организации, что означает, что гибридное соединение Azure AD успешно удается только тогда, когда клиент подключен к интрасети организации.
