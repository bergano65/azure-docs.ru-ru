...
| ![Проверить][1] | Проверка управляемой проверки гибридного присоединение к Azure AD |
| ![Проверить][1] | Выбор сценария на основе инфраструктуры идентификации |
| ![Проверить][1] | Проверка поддержки локального имени участника-пользователя AD для гибридного присоединение к Azure AD |
...
- Если вы используете [Объединенный фильтр записи](https://docs.microsoft.com/windows-hardware/customize/enterprise/unified-write-filter) и аналогичные технологии, которые очищают изменения на диске при перезагрузке, они должны быть применены после гибридного устройства, присоединенного к Azure AD. Включение таких технологий до завершения гибридного присоединение к Azure AD приведет к тому, что устройство будет получать отсоединение при каждой перезагрузке.
