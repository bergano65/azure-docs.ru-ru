---
title: Корпоративный государственный роуминг часто задаваемые вопросы - Активный каталог Azure
...
* *Языковые предпочтения,* которые включают настройки для макетов клавиатуры, системного языка, даты и времени и многое другое.
...
**Данные приложения** — универсальные приложения для Windows могут записывать данные параметров в папку Roaming. Все данные, записанные в эту папку, будут автоматически синхронизироваться. Такую возможность можно предусмотреть для приложения еще на этапе его проектирования. Для получения дополнительной информации о том, как создать [appdata storage API](https://msdn.microsoft.com/library/windows/apps/mt299098.aspx) приложение Universal Windows, [Windows 8 appdata roaming developer blog](https://blogs.msdn.com/b/windowsappdev/archive/2012/07/17/roaming-your-app-data.aspx)использующем роуминг, см.

## <a name="what-account-is-used-for-settings-sync"></a>Какая учетная запись используется для синхронизации параметров?

В Windows 8.1 при синхронизации параметров всегда использовались учетные записи Майкрософт. Корпоративные пользователи имели возможность подключить учетную запись Майкрософт к своей учетной записи домена Active Directory, чтобы получить доступ к синхронизации настроек. В Windows 10 эта связанная функциональность учетной записи Майкрософт заменяется основной/вторичной инфраструктурой учетной записи.
...
* данные приложения будут отнесены к учетной записи, используемой для получения приложения. Синхронизация будет синхронизирована только приложения, отмеченные основной учетной записью. Пометка владения приложениями определяется, когда приложение загружается по боговому устройству через Магазин Windows или управление мобильными устройствами (MDM).
...
## <a name="can-i-store-synced-settings-and-data-on-premises"></a>Можно ли хранить синхронизированные настройки и данные в помещениях?
...
* UE-V роуминг для настроек Windows и современных данных приложений UWP должен быть отключен при использовании политик и дентиц UE-V. Эти функции уже обеспечивает служба Enterprise State Roaming.
