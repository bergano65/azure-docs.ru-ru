...
4. В разделе одноразовый **секретный код по электронной почте для гостей** выберите **Отключить однократный секретный код для гостей**.
   ...
   > [!NOTE]
   > Если вместо параметров секретного кода на один раз отображается следующий переключатель, это означает, что вы ранее включили, отключили или выбрали предварительную версию функции. Выберите **нет** , чтобы отключить эту функцию.
   >
...
Если вы ранее включили в общедоступную предварительную версию одноразового секретного кода для отправки по электронной почте, 2021 Дата выпуска для автоматического включения функций не применяется, поэтому связанные бизнес-процессы не будут затронуты. Кроме того, в портал Azure в поле **секретный код одноразового пароля для гостей** вы не увидите возможность **автоматического включения одноразового секретного кода для гостей в 2021 октября**. Вместо этого вы увидите следующий переключатель **Да** или **нет** :
