...
Да. Если домен не проверен, а клиент не прошел [смену администратором](../users-groups-roles/domains-admin-takeover.md), вы можете настроить прямую федерацию с таким доменом. Неуправляемые (проверяемые по электронной почте) клиенты создаются, когда пользователь активирует приглашение для совместной работы B2B или выполняет самостоятельную регистрацию в Azure AD, используя домен, который в настоящее время не существует. Вы можете настроить прямую федерацию с такими доменами. Попытка настроить прямую федерацию с проверенным DNS доменом на портале Azure либо с помощью PowerShell приведет к ошибке.
