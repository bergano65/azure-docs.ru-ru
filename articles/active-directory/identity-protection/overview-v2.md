---
title: Что такое Защита идентификации Azure Active Directory (обновленная)? | Документы Майкрософт
...
Azure AD применяет машинное обучение для обнаружения аномалий и подозрительной активности, используя как сигналы, обнаруженные в режиме реального времени при входе в систему, так и сигналы не в реальном времени, связанные с пользователями и их действиями при входе. Используя эти данные, защиты идентификации вычисляет риска в реальном времени входа в систему каждый раз, когда пользователь проходит проверку подлинности, а также определить общий уровень риска пользователя для каждого пользователя. Она позволяет автоматически выполнять действия при обнаружении рисков, настраивая политики защиты идентификации для рисков пользователя и при входе.  
...
5. Сбой многофакторной проверки Подлинности строки появления события риска записывать, вызов их риска пользователя для последующих входов в систему. 
...
| --- | --- | --- | --- |
| Политика риска пользователя | Да | Нет | Нет |
| Политика риска входа | Да | Нет | Нет |
