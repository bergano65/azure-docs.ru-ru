...
* Заполнение данных в среде защиты идентификации путем имитации обнаружения и уязвимостей рисков
* Настройка политик условного доступа на основе рисков и проверка влияния этих политик

## <a name="simulating-risk-detections"></a>Имитация обнаружения рисков

В этом разделе содержатся инструкции по моделированию следующих типов обнаружения рисков.
...
Дополнительные сведения об этом обнаружении рисков см. [в статье входы с анонимных IP-адресов](../reports-monitoring/concept-risk-events.md#sign-ins-from-anonymous-ip-addresses). 
...
### <a name="sign-ins-from-unfamiliar-locations"></a>Входы в систему из необычного расположения

Дополнительные сведения об этом обнаружении рисков см. [в статье входы в систему из](../reports-monitoring/concept-risk-events.md#sign-in-from-unfamiliar-locations)незнакомых расположений. 
...
Дополнительные сведения об этом обнаружении рисков см. [в разделе невозможное перемещение в нетипичное расположение](../reports-monitoring/concept-risk-events.md#impossible-travel-to-atypical-locations). 

Моделирование условия невозможности перемещения — достаточно сложная задача, так как соответствующий алгоритм использует машинное обучение, чтобы отсеять ложные срабатывания, например, когда невозможно переместиться и используется знакомое устройство или вход выполнен из VPN, которую используют другие пользователи в каталоге. Кроме того, для алгоритма требуется журнал входа 14 дней и 10 имен входа пользователя, прежде чем он начнет создавать обнаружение рисков. Из-за сложных моделей машинного обучения и правил, приведенных выше, существует вероятность того, что следующие шаги не приведут к обнаружению рисков. Возможно, потребуется выполнить репликацию этих шагов для нескольких учетных записей Azure AD, чтобы опубликовать это обнаружение риска.
...
2. Введите учетные данные учетной записи, для которой вы хотите создать невозможность обнаружения риска в пути.
...
6. Повышайте риск для пользователя тестовой учетной записи, например, несколько раз имитируя одно из обнаруженных рисков.
7. Подождите несколько минут и убедитесь, что для пользователя отображается уровень "Средний". В противном случае имитируйте обнаружение рисков для пользователя.
8. Для **политики применения** выберите значение **Вкл.**
9. Теперь вы можете протестировать условный доступ на основе рисков, войдя с помощью пользователя с повышенным уровнем риска.
...
8. Теперь вы можете проверить условный доступ на основе риска входа, выполнив вход с помощью рискованного сеанса (например, с помощью браузера Tor). 
