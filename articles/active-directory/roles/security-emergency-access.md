---
title: Управление учетными записями администратора аварийного доступа — Azure AD
...
- Администраторы регистрируются с помощью многофакторной идентификации Azure AD, и все их отдельные устройства недоступны или служба недоступна. Пользователям, возможно, не удается завершить многофакторную проверку подлинности для активации роли. Например, из-за сбоя сотовой сети они не могут отвечать на телефонные звонки или получать текстовые сообщения (два механизма аутентификации, зарегистрированные для устройства).
...
- Механизм проверки подлинности, используемый для учетной записи аварийного доступа, должен отличаться от алгоритмов, используемых другими административными учетными записями, включая другие учетные записи для аварийного доступа.  Например, если обычным входом администратора является локальная служба MFA, то Azure AD MFA будет другим механизмом.  Однако если Azure AD MFA является основной частью проверки подлинности для административных учетных записей, то следует рассмотреть другой подход, например использование условного доступа с поставщиком MFA сторонних поставщиков с помощью пользовательских элементов управления.
...
Некоторые организации используют доменные службы AD и ADFS или аналогичный поставщик удостоверений для Федерации с Azure AD. [Нет локальных учетных записей с правами администратора](../fundamentals/protect-m365-from-on-premises-attacks.md). При использовании проверки подлинности и использовании источников для учетных записей с правами администратора за пределами Azure AD добавляется ненужный риск в случае сбоя или компрометации этих систем.
