...
![Действие входа](./media/reference-sign-ins-error-codes/06.png "Действия при входе")
...
![Действие входа](./media/reference-sign-ins-error-codes/05.png "Действия при входе")
...
|50078|Истек срок действия представленной многофакторной проверки подлинности. для доступа необходимо обновить многофакторную проверку подлинности.|
...
|50131|Используется в различных ошибках условного доступа. Пример: При недопустимом состоянии устройства Windows запрос блокируется из-за подозрительной активности, политики доступа и решений, связанных с политикой безопасности.|
...
|50148|Значение code_verifier не соответствует значению code_challenge, указанному в запросе на авторизацию для PKCE. Обратитесь к разработчику приложения. |
...
|50181|Ошибка, связанная с OTP при входе в систему. |
|50194|Приложение "{appId}" ({appName}) не настроено в качестве приложения с несколькими клиентами. Использование конечной точки "/Common" не поддерживается для таких приложений, созданных после "{Time}". Используйте конечную точку конкретного клиента или настройте приложение для использования нескольких клиентов.|
...
|53032|Запись в заблокирована из-за политик защита идентификации Azure AD.|
...
|70043|Управление сеансом условного доступа Azure приводит к истечению срока действия сеанса|
|70044|Управление сеансом условного доступа Azure приводит к истечению срока действия сеанса|
...
|80002|Внутренняя ошибка. Истекло время ожидания запроса на проверку пароля. Не удалось отправить запрос проверки подлинности во внутреннюю службу гибридной идентификации. Чтобы получить дополнительные сведения об ошибке, [отправьте запрос в службу поддержки](../fundamentals/active-directory-troubleshooting-support-howto.md).|
...
|80014|Истекло время ожидания агента аутентификации. Чтобы получить дополнительные сведения об этой ошибке, отправьте запрос в [службу поддержки](../fundamentals/active-directory-troubleshooting-support-howto.md) с кодом ошибки, идентификатором корреляции и DateTime.|
...
|90072| Учетную запись необходимо сначала добавить как внешнего пользователя в клиент. Выполните выход и снова войдите, используя другую учетную запись Azure AD.|
|90094| Приложение запросило разрешения, на которые пользователь, выполнивший вход, не может принять согласие, а пользователь был заблокирован. |
|90095| Приложение запросило разрешения, на которые пользователь, выполнивший вход, не может принять согласие, а пользователь показал форму [запроса согласия администратора](../manage-apps/configure-admin-consent-workflow.md) . |
|130500|Вход с телефоном заблокирован из-за политики учетных данных пользователя.|
|500011| Субъект ресурса с именем <site address> не найден в клиенте с именем <tenant ID>. Это может произойти, если приложение установил не администратор клиента или если пользователь в клиенте не предоставил к нему разрешение. Возможно, вы отправили запрос на проверку подлинности в другой клиент.|
|500014|Ресурс "{identifier}" отключен.|
...
|500172|Сертификат "{Name}", выданный "{Issuer}", является недопустимым. Текущее время: "{Куртиме}". NotBefore сертификата: "{startTime}". NotAfter сертификата: "{endTime}".|
|501291|Клиентское приложение — это приложение MAM, устройство не зарегистрировано и запрос отправлен с помощью брокера. Необходимо выполнить присоединение к рабочему месту, чтобы зарегистрировать устройство, прежде чем можно будет получить доступ к приложению.|
|530003|Устройство должно быть управляемым для доступа к этому ресурсу.|
...
|700051|Response_type "token" не включен для приложения. Приложение запросило неподдерживаемый тип ответа по следующим причинам: response_type "токен" не включен для приложения. Владелец приложения должен перейти на портал Azure или вызвать MS Graph, чтобы разрешить предоставление маркера неявного доступа.|
|900432|Конфиденциальный клиент не поддерживается в запросе между облаками.|
|5000811|Не удалось проверить подпись токена SAML. Идентификатор ключа подписывания не соответствует ни одному из допустимых зарегистрированных ключей.|
|7000215|Указан недопустимый секрет клиента.|
