...
|80001|Агенту проверки подлинности не удалось подключиться к Active Directory. Убедитесь, что агент проверки подлинности установлен на присоединенном к домену компьютере, в зоне прямой видимости которого есть контроллер домена для обработки запросов пользователей на вход.|
...
|80003|Агент проверки подлинности получил недопустимый ответ. Неизвестная ошибка при попытке выполнить проверку подлинности Active Directory в локальной среде. Чтобы получить дополнительные сведения об ошибке, [отправьте запрос в службу поддержки](../fundamentals/active-directory-troubleshooting-support-howto.md).|
