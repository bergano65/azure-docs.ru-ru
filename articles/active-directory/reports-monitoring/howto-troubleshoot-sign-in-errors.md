---
title: Как устранить неполадки в отчетах об ошибках в всходных в систему (ru) Документы Майкрософт
...
* Пользователь, который находится в **глобальном администраторе,** **администратор безопасности,** **считыватель безопасности**или отчет о роли **читателя** для арендатора. а также любой пользователь, который может получить доступ к своим собственным событиям входов. 
...
3. Используйте предоставленные фильтры, чтобы сузить область поиска сбоя по имени пользователя, идентификатору объекта, имени приложения или дате. Кроме того, выберите **Сбой** из выпадения **статуса,** чтобы отобразить только неудавшееся входе. 

    ![Фильтрация результатов](./media/howto-troubleshoot-sign-in-errors/filters.png)
        
4. Определите неудавшийся входе, который вы хотите исследовать. Выберите его, чтобы открыть окно дополнительных деталей с более подробной информацией о неудавшемся входе. Запишите значения параметров **Код ошибки входа** и **Причина сбоя**. 
...
6. Причина сбоя описывает ошибку. Например, в приведенном выше сценарии причиной сбоя является **имя пользователя или пароль Invalid или Invalid-** имя пользователя или пароль. Исправление — просто войти еще раз с правильными именем пользователя и паролем.
