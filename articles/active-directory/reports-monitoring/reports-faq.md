...
**Вопрос. в настоящее время я `https://graph.windows.net/<tenant-name>/reports/` использую интерфейсы API конечных точек, чтобы получать отчеты об аудите Azure AD и интегрированных отчетах по использованию приложений в системы отчетов программным способом. Что следует переключиться на?**
...
Ответ **.** Используйте [API управления office 365](https://docs.microsoft.com/office/office-365-management-api/office-365-management-apis-overview) для доступа к журналам действий Office 365 через API.
