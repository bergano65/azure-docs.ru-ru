---
title: Azure Active Directory журналов действий в Azure Monitor | Документация Майкрософт
description: Общие сведения о Azure Active Directory журналах действий в Azure Monitor
...
Вы можете направить журналы действий Azure Active Directory (Azure AD) в несколько конечных точек для долгосрочного хранения и аналитики данных. Эта функция позволяет следующее:
...
* Учетная запись хранения Azure, для которой у вас есть разрешения *ListKeys* . Рекомендуется использовать общую учетную запись хранения вместо учетной записи хранилища BLOB-объектов. Информацию о ценах на хранилище см. [здесь](https://azure.microsoft.com/pricing/calculator/?service=storage). 
...
Следующая таблица содержит оценочные затраты в месяц для базового концентратора событий в западной части США, в зависимости от объема данных события, который может отличаться от клиента к клиенту, например по многим факторам, например по способу входа пользователя и т. д. Чтобы рассчитать точную оценку объема данных, который вы планируете использовать для вашего приложения, используйте [Калькулятор цен на концентраторы событий](https://azure.microsoft.com/pricing/details/event-hubs/).
...
| Категория журнала | Количество пользователей | Событий в день | Событий в месяц (30 дней) | Стоимость за месяц в долларах США (EST) |
|:-|--|--|--|-:|
...
**Вопрос. что происходит, если администратор изменяет срок хранения параметра диагностики?**
...
**Ответ:** Сначала [направьте журналы действий Azure AD в концентратор событий](./tutorial-azure-monitor-stream-logs-to-event-hub.md), а затем выполните следующие действия, чтобы [интегрировать журналы действий со Splunk](./howto-integrate-activity-logs-with-splunk.md).
...
**Ответ**. Да. Для доступа к журналам из пользовательского приложения можно использовать [API Центров событий](../../event-hubs/event-hubs-dotnet-standard-getstarted-send.md). 
...
* [Интеграция журналов действий с Azure Monitor](howto-integrate-activity-logs-with-log-analytics.md)