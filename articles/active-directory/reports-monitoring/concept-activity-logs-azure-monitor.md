---
title: Активные журналы активности каталогов Azure в Azure Monitor Документы Майкрософт
description: Введение в журналы активности активных каталогов Azure в Azure Monitor
...
Можно направить журналы активности Azure Active Directory (Azure AD) в несколько конечных точек для долгосрочного хранения и анализа данных. Эта функция позволяет следующее:
...
Следующая таблица содержит сметные затраты в месяц для базового концентратора событий в Западной США, в зависимости от объема данных о событиях, которые могут варьироваться от арендатора к арендатору в зависимости от многих факторов, таких как поведение пользователя, вслусывающего в систему и т.д. Для расчета точной оценки объема данных, которые вы ожидаете для приложения, используйте [калькулятор ценообразования Event Hubs.](https://azure.microsoft.com/pricing/details/event-hubs/)
...
| Категория журнала       | Количество пользователей | Событий в день | События в месяц (30 дней) | Стоимость в месяц в долларах США (est.) |
| :--                | ---             | ---            | ---                        | --:                          |
| Аудит и входе | 100 000         | 16,500,000     | 495,000,000                |  $1093.00                       |
...
**В: Что произойдет, если Администратор изменит период хранения диагностической настройки?**

**A:** Новая политика удержания будет применена к журналам, собранным после изменения. Журналы, собранные до изменения, не будут затронуты.
...
**A:** Да. Для доступа к журналам из пользовательского приложения можно использовать [API Центров событий](../../event-hubs/event-hubs-dotnet-standard-getstarted-receive-eph.md). 
