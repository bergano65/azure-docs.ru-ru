...
|Key|Gets the type of tenant (Получение сведений о типе клиента)|
|Key|Verify if B2C feature is enabled (Проверка того, включен ли компонент B2C)|
|Key|Проверка того, включен ли компонент|
|Key|Добавление партнера для компании|
|Key|Add unverified domain (Добавление непроверенного домена)|
|Key|Add verified domain (Добавление проверенного домена)|
|Key|Create company (Создание организации)|
|Key|Создание параметров организации.|
|Key|Удаление параметров организации.|
|Key|Demote partner (Изменение типа партнера)|
|Key|Directory deleted (Каталог удален)|
...
|Категория аудита|Действие|
|---|---|
|Другое|Создание утверждения запроса|
|Другое|Удаление проверки доступа|
...
|Управление пользователями|Просмотр членства в группе|
