---
title: Анализ журналов активности с помощью журналов Azure Monitor Документы Майкрософт
description: Узнайте, как анализировать журналы активности активных каталогов Azure с помощью журналов Azure Monitor
...
После [интеграции журналов действий Azure AD с журналами Azure Monitor](howto-integrate-activity-logs-with-log-analytics.md) вы можете использовать возможности журналов Azure Monitor, чтобы получить полезные сведения о своей среде. Вы также можете установить [представления аналитики журнала для журналов деятельности Azure AD,](howto-install-use-log-analytics-views.md) чтобы получить доступ к заранее построенным отчетам вокруг аудиторских и регистрационных событий в вашей среде.
...
* Следующие роли в Active Directory Azure (если вы получаете доступ к журналу Analytics через портал Active Directory Azure)
...
2. Разверните раздел **Управление журналами**, а затем разверните **AuditLogs** или **SignInLogs**, чтобы просмотреть схему журнала.
    ![Аудит журналов](./media/howto-analyze-activity-logs-log-analytics/auditlogschema.png) ![Signin журналы](./media/howto-analyze-activity-logs-log-analytics/signinlogschema.png)
