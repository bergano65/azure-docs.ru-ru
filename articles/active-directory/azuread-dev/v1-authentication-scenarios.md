---
title: Azure AD для разработчиков (v1.0) Azure
description: Изучите основы аутентификации для Azure AD для разработчиков (v1.0), такие как модель приложения, API, подготовка и наиболее распространенные сценарии аутентификации.
...
Активный каталог Azure для разработчиков (v1.0) (Azure AD) упрощает аутентификацию для разработчиков приложений, предоставляя идентификационные данные как услуги, с поддержкой отраслевых стандартных протоколов, таких как OAuth 2.0 и OpenID Connect, а также библиотеки с открытым исходным кодом для различных платформ, чтобы помочь вам начать кодирование быстро.
...
## <a name="authentication-basics-in-azure-ad"></a>Основы аутентификации в Azure AD
...
Вот что вам нужно знать о различных компонентах, показанных на диаграмме:

* Azure AD является поставщиком удостоверений. Поставщик идентификационных данных отвечает за проверку идентификации пользователей и приложений, которые существуют в каталоге организации, и выдает маркеры безопасности при успешной аутентификации этих пользователей и приложений.
* Приложение, которое хочет передать аутентификацию на аутсорсинг в Azure AD, должно быть зарегистрировано в Active Directory Azure (Azure AD). Azure AD регистрирует и уникальным образом идентифицирует приложение в каталоге.
...
* После проверки подлинности пользователя приложение должно проверить маркер безопасности пользователя, чтобы убедиться, что аутентификация была успешной. Можно найти краткие руководства, учебники и примеры кода на различных языках и платформах, в которых показано, что должно делать приложение.
...
* Поток запросов и ответов при аутентификации определяется применяемым протоколом аутентификации, например OAuth 2.0, OpenID Connect, WS-Federation или SAML 2.0. Для получения дополнительной информации о протоколах смотрите раздел **протокола «Концепции > аутентификации».**
...
|[Обновление выкупа токенов](v1-protocols-oauth-code.md#refreshing-the-access-tokens) | обновить токен | x | x | x| |
...
1. Azure AD использует объект приложения в арендаторе A в качестве чертежа для создания основного обслуживания в арендаторе B
...
В таблице ниже приведено краткое описание каждого типа утверждений, которые создает Azure AD. Более подробную информацию можно получить в формате [маркеров доступа](../develop/access-tokens.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json) и [идентификаторов, выпущенных](../develop/id-tokens.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json) Azure AD.
