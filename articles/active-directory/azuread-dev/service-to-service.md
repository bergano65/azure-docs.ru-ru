...
1. Сначала серверное приложение должно пройти проверку подлинности в Azure AD от своего имени, без участия пользователя (то есть без интерактивного диалогового окна входа). Он делает запрос в точку маркера Azure AD, предоставляя учетные данные, идентификатор приложения и URI.
...
Просмотрите образцы кода для Daemon или Server Application to Web API сценариев: [Сервер или Приложение Daemon к Web API](sample-v1-code.md#daemon-applications-accessing-web-apis-with-the-applications-identity)

## <a name="app-registration"></a>Регистрация приложения

* Однотенантный: и для удостоверения приложения, и для делегированного удостоверения пользователя управляющая программа или серверное приложение должны быть зарегистрированы в одном и том же каталоге Azure AD. Веб-API можно настроить для выявления набора разрешений, которые используются для ограничения доступа daemon или сервера к его ресурсам. При использовании делегированного типа идентификации пользователя серверное приложение должно выбрать нужные разрешения. На странице **Разрешения API** для регистрации приложения после того, как вы выбрали **добавление разрешения** и выбрали семейство API, выберите **делегированные разрешения,** а затем выберите разрешения. Этот шаг не является обязательным, если используется тип удостоверения приложения.
