...
- **[Одностраничное приложение (SPA)](single-page-application.md)**: Пользователю необходимо войти в одностраничное приложение, защищенное Azure AD.
- **[Веб-браузер для веб-приложений:](web-app.md)** Пользователю необходимо войти в веб-приложение, защищенное Azure AD.
- **[Родное приложение для Web API:](native-app.md)** Нативное приложение, которое работает на телефоне, планшете или ПК, должно проверить подлинность пользователя для получения ресурсов от web API, защищенного Azure AD.
- **[Веб-приложение для Web API:](web-api.md)** Веб-приложению необходимо получать ресурсы из веб-API, защищенного Azure AD.
- **[Daemon или серверное приложение для Web API:](service-to-service.md)** приложение daemon или серверное приложение без веб-интерфейса пользователя должны получать ресурсы от веб-API, защищенного Azure AD.
...
Любое приложение, передающее функцию проверку подлинности системе Azure AD, должно быть зарегистрировано в каталоге. Этот шаг включает в себя сообщение Azure AD о вашем приложении, включая URL-адрес, где оно находится, URL-адрес для отправки ответов после проверки подлинности, URI для идентификации приложения и многое другое. Эта информация необходима по нескольким основным причинам:
...
При разработке мультитенантного приложения вместо однотенантного необходимо учесть некоторые дополнительные соображения. Например, если приложение доступно пользователям в нескольких каталогах, необходим механизм для определения того, в каком арендаторе они находятся. Однотенантное приложение должно присутствовать только в собственном каталоге для данного пользователя, тогда как мультитенантное приложение должно идентифицировать конкретного пользователя из любого каталога в Azure AD. Для выполнения этой задачи система Azure AD предоставляет общую конечную точку проверки подлинности, в которую мультитенантное приложение может направлять запросы на вход, вместо конечной точки для конкретного клиента. Для всех каталогов в Azure AD эта конечная точка — `https://login.microsoftonline.com/common`, тогда как конечной точкой для конкретного клиента может быть `https://login.microsoftonline.com/contoso.onmicrosoft.com`. Общая конечная точка особенно важна для рассмотрения при разработке приложения, потому что вам понадобится необходимая логика для обработки нескольких арендаторов во время регистрации, входе в систему и проверки токенов.
...
Каждый сценарий, рассматриваемый в этом документе, содержит подраздел, где описываются соответствующие требования по подготовке. Для получения более подробной информации о подготовке приложения в Azure AD и [Integrating applications with Azure Active Directory](../develop/single-and-multi-tenant-apps.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json) различиях между приложениями-одиночками и несколькими арендаторами см. Продолжим рассмотрение типичных сценариев работы приложений в Azure AD.
