---
title: Разрешить доступ к веб-приложениям с помощью OpenID Connect & AD Azure (ru) Документы Майкрософт
...
```
Метаданные — это простой документ JSON. В качестве примера ниже приведен фрагмент кода. Его содержимое полностью описано в [спецификации OpenID Connect](https://openid.net). Обратите внимание, что предоставление `common` идентификатора клиента, а не вместо вышеуказанного «арендатора», приведет к возвращению URL-изосновныых ОБЪЕКТОв для конкретного клиента в объекте JSON.
...
Если в приложении есть пользовательские ключи подписи в результате использования функции [отображения утверждений,](../develop/active-directory-claims-mapping.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json) необходимо прижать параметр `appid` запроса, содержащий идентификатор приложения, чтобы получить указание на ключевую `jwks_uri` информацию вашего приложения. Например: `https://login.microsoftonline.com/{tenant}/.well-known/openid-configuration?appid=6731de76-14a6-49ae-97bc-6eba6914391e` содержит `jwks_uri` `https://login.microsoftonline.com/{tenant}/discovery/keys?appid=6731de76-14a6-49ae-97bc-6eba6914391e`a .
...
| client_id |обязательно |Идентификатор приложения, назначенный вашему приложению при регистрации в Azure AD. Его значение можно найти на портале Azure. Нажмите **Azure Active Directory,** нажмите **регистрацию приложений,** выберите приложение и найдите идентификатор приложения на странице приложения. |
...
| область | рекомендуется | Спецификация OpenID Connect `openid`требует области, что переводится как разрешение "Подписать вас" в uI-разъема согласия. Эта и другие области OIDC игнорируются на конечных точках v1.0, но по-прежнему является наилучшей практикой для клиентов, совместимых со стандартами. |
...
| redirect_uri | рекомендуется |URI перенаправления приложения, на который можно отправлять ответы проверки подлинности для их получения приложением. Он должен в точности соответствовать одному из URI перенаправления, зарегистрированных на портале, но иметь форму закодированного URL-адреса. В случае отсутствия агент пользователя будет отправлен обратно в один из перенаправить URIs, зарегистрированных для приложения, наугад. Максимальная длина составляет 255 байтов |
...
Образец ответа, отправленный `redirect_uri` указанному в запросе регистрации после проверки подлинности пользователя, может выглядеть следующим образом:
...
| state |Значение, включенное в запрос, которое также возвращается в ответе маркера. Случайно сгенерированное уникальное значение обычно используется для [предотвращения атак подделки запросов на кросс-сайт.](https://tools.ietf.org/html/rfc6749#section-10.12) Параметр "state" также используется для кодирования информации о состоянии пользователя в приложении перед созданием запроса на проверку подлинности, например информации об открытой на тот момент странице или представлении. |
...
* Обеспечение пользователя имеет надлежащую авторизацию/привилегии с использованием или `wids` `roles` претензий. 
* Обеспечение определенного уровня проверки подлинности, например Многофакторной Идентификации.

После проверки маркера `id_token` вы можете начать сеанс с пользователем, используя утверждения из маркера `id_token` для получения сведений о пользователе в приложении. Эта информация может быть использована для отображения, записи, персонализации и т.д. Для получения `id_tokens` дополнительной информации о и претензий, прочитайте [AAD id_tokens](../develop/id-tokens.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json).

## <a name="send-a-sign-out-request"></a>Отправка запроса на выход

Для выхода пользователя из приложения недостаточно очистить файлы cookie или каким-то другим образом завершить сеанс пользователя. Вы также должны перенаправить `end_session_endpoint` пользователя на регистрацию. Если вы этого не сделаете, пользователь сможет повторно подтвердить подлинность в ашего приложении, не введя его учетные данные снова, потому что они будут иметь действительный один сеанс входа с конечным пунктом Azure AD.
...
| Параметр |  | Описание |
| --- | --- | --- |
| post_logout_redirect_uri |рекомендуется |URL-адрес, на который пользователь должен быть перенаправлен после успешного выхода.  Этот URL-адрес должен соответствовать одному из перенаправления URIs, зарегистрированных для вашего приложения на портале регистрации приложений.  Если *post_logout_redirect_uri* не включен, пользователю отображается общее сообщение. |
...
Успешный ответ, отправленный на `redirect_uri` использование, `response_mode=form_post`выглядит следующим образом:
...
* Узнайте больше о [ `id_token` претензиях и претензиях.](../develop/id-tokens.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json)
