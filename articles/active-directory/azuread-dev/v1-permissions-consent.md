...
* **Делегированные разрешения**. Они используются приложениями с авторизованным пользователем. Для этих приложений пользователь или администратор соглашается с разрешениями, которые запрашивает приложение. Приложение получает разрешение выполнять роль авторизованного пользователя при вызове API. В зависимости от API пользователь может быть не в состоянии дать согласие на API напрямую и вместо этого [потребует от администратора предоставить "согласие администратора".](/azure/active-directory/develop/active-directory-devhowto-multi-tenant-overview)
* **Разрешения приложений**. Они используются в приложениях, работающих без авторизованного пользователя (например, приложения, работающие как фоновые службы или управляющие программы). Разрешения приложений могут быть согласованы только [администраторами,](/azure/active-directory/develop/active-directory-v2-scopes#requesting-consent-for-an-entire-tenant) поскольку они, как правило, являются мощными и позволяют получить доступ к данным через границы пользователя, или к данным, которые в противном случае были бы ограничены администраторами. Пользователям, которые определяются как владельцы приложения ресурса (т.е. API, который публикует разрешения), также разрешается предоставлять разрешения на применение API, которыми они владеют.
