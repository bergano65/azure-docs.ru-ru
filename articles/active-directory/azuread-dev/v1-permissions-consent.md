...
* **Делегированные разрешения**. Они используются приложениями с авторизованным пользователем. Для этих приложений пользователь или администратор соглашается с разрешениями, которые запрашивает приложение. Приложение получает разрешение выполнять роль авторизованного пользователя при вызове API. В зависимости от API пользователь может не иметь разрешения на доступ к API напрямую, а вместо этого [требовать от администратора предоставить согласие](../develop/howto-convert-app-to-be-multi-tenant.md)администратора.
* **Разрешения приложений**. Они используются в приложениях, работающих без авторизованного пользователя (например, приложения, работающие как фоновые службы или управляющие программы). Разрешения на приложения могут быть предоставлены только [администраторам](../develop/v2-permissions-and-consent.md#requesting-consent-for-an-entire-tenant) , так как они обычно являются мощными и разрешают доступ к данным через границы пользователей или данные, которые в противном случае были бы ограничены администраторами. Пользователям, которые определены как владельцы приложения ресурсов (т. е. API, который публикует разрешения), также разрешено предоставлять разрешения приложения для интерфейсов API, которыми они владеют.
...
* **Динамическое согласие пользователя**. Представляет собой функцию модели приложения Azure AD версии 2. В этом случае приложение запрашивает набор разрешений, необходимых ему в [потоке авторизации OAuth 2.0 для приложений версии 2](../develop/v2-permissions-and-consent.md#requesting-individual-user-consent). Пользователю будет предложено дать согласие, если он этого еще не сделал. Дополнительные сведения см. в разделе [Добавочное и динамическое согласие](./azure-ad-endpoint-comparison.md#incremental-and-dynamic-consent).
