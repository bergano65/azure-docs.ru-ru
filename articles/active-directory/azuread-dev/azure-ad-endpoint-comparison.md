...
Дополнительные сведения о OAuth 2,0, `refresh_tokens`и `access_tokens`см. в [справочнике по протоколу платформы идентификации Майкрософт](../develop/active-directory-v2-protocols.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json).
...
* Вместо параметра `scope` теперь поддерживается параметр `resource`.  
