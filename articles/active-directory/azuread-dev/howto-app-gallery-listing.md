...
* У гостевых пользователей есть свои риски. В этом случае администратор выполнит сброс пароля (создание временного пароля). Для этого не требуется защита идентификации. Администратор гостевого пользователя может войти в https://aka.ms/RiskyUsers систему и щелкнуть "Сброс пароля".
...
Для любых укрупнений отправьте сообщение электронной почты [группе интеграции единого входа Azure AD](mailto:SaaSApplicationIntegrations@service.microsoft.com) по SaaSApplicationIntegrations@service.microsoft.comадресу, и мы будем отвечать как можно скорее.