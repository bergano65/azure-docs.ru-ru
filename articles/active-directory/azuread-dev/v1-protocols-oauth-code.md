...
> [!NOTE]
>  Если вы не сообщите серверу, какой ресурс вы планируете вызвать, то сервер не будет запускать политики условного доступа для этого ресурса. Таким образом, для того, чтобы спусковой крючок МИД, вам нужно будет включить ресурс в свой URL. 
>

В Azure Active Directory (Azure AD) используется протокол OAuth 2.0, чтобы обеспечивать авторизацию доступа к веб-приложениям и веб-API в клиенте Azure AD. Это руководство является независимым от языка и описывает, как отправлять и получать сообщения HTTP без использования каких-либо из наших [библиотек с открытым исходным кодом.](active-directory-authentication-libraries.md)
...
Во-первых, зарегистрируйте приложение в арендаторе Azure Active Directory (Azure AD). После этого вашему приложению будет присвоен идентификатор, и оно сможет получать маркеры.

1. Войдите на [портал Azure](https://portal.azure.com).
   
1. Выберите клиента Azure AD, выбрав учетную запись в правом верхнем углу страницы, после чего выберите навигацию **Switch Directory** и выбрав подходящего клиента. 
   - Пропустите этот шаг, если у вас есть только один арендатор Azure AD под вашей учетной записью, или если вы уже выбрали подходящего арендатора Azure AD.
   
1. На портале Azure найдите и выберите **Azure Active Directory**.
   
1. В **Azure Active Directory** влево меню выберите **регистрацию приложений,** а затем выберите **новую регистрацию.**
   
1. Следуйте инструкциям на экране, а затем создайте новое приложение. Это не имеет значения, если это веб-приложение или публичный клиент (мобильный & настольный) приложение для этого учебника, но если вы хотите конкретные примеры для веб-приложений или приложений для публичных клиентов, проверить наши [quickstarts](v1-overview.md).
   ...
   - Обеспечить **перенаправление URI**. Для веб-приложений это базовый URL вашего приложения, в котором пользователи могут войти в систему.  Например, `http://localhost:12345`. Для общедоступных клиентов (мобильных & настольных компьютеров) Azure AD использует его для возврата ответов токенов. Укажите значение, специфичное для вашего приложения.  Например, `http://MyFirstAADApp`.
   <!--TODO: add once App ID URI is configurable: The **App ID URI** is a unique identifier for your application. The convention is to use `https://<tenant-domain>/<app-name>`, e.g. `https://contoso.onmicrosoft.com/my-first-aad-app`-->  
   
1. После завершения регистрации Azure AD присвоит вашему приложению уникальный идентификатор клиента **(идентификатор приложения).** Это значение вам понадобится в следующих разделах, поэтому не забудьте скопировать его на странице приложения.
   
1. Чтобы найти приложение на портале Azure, выберите **регистрацию приложений,** а затем выберите **Просмотр всех приложений.**
...
| redirect_uri |обязательно | Зарегистрированная `redirect_uri`в клиентской заявке. |
...
| code_verifier | необязательный | Это тот же параметр code_verifier, который использовался для получения кода авторизации (authorization_code). Является обязательным, если в запросе на код авторизации использовался PKCE. Для получения дополнительной [информации](https://tools.ietf.org/html/rfc7636) см.   |
...
| Параметр | Описание |
| --- | --- |
| access_token |Запрашиваемый маркер доступа.  Это непрозрачная строка - она зависит от того, что ресурс ожидает получить, и не предназначен для клиента, чтобы посмотреть. Приложение может использовать этот маркер для проверки подлинности на защищенном ресурсе, таком как веб-API. |
...
| 401 |Ошибка проверки подлинности. Например, в запросе отсутствует параметр client_secret. |
...
Чтобы узнать больше о концепта Azure AD v1.0 и о том, как добавить аутентификацию и авторизацию в веб-приложения и веб-аДИ, смотрите [пример приложений.](sample-v1-code.md)
