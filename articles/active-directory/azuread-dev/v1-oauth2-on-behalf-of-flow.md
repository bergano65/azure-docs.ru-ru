---
title: Аутентификация от службы к обслуживанию с OAuth2.0 от имени потока Документы Майкрософт
...
Поток OBO начинается после того, как пользователь пройдет проверку подлинности для приложения с использованием [потока для предоставления кода проверки подлинности в OAuth 2.0](v1-protocols-oauth-code.md). В этот момент приложение отправляет токен доступа (токен A) на веб-API среднего уровня (API A), содержащий требования пользователя и согласие на доступ к API A. Далее API A делает аутентифицированный запрос на аиП(API B).

Эти шаги представляют собой поток On-Behalf-Of: ![Показывает шаги в потоке OAuth2.0 On-Behalf-Of](./media/v1-oauth2-on-behalf-of-flow/active-directory-protocols-oauth-on-behalf-of-flow.png)
...
1. Выберите **регистрацию приложений,** а затем **новую регистрацию.**
...
1. Установите перенаправление URI на базовый URL.
1. Выберите **Зарегистрировать**, чтобы создать приложение.
1. Создайте секрет клиента, прежде чем выйти с портала Azure.
1. На портале Azure выберите приложение и выберите **Сертификаты & секреты.**
1. Выберите **новый секрет клиента** и добавьте секрет продолжительностью один год или два года.
1. При сохранении этой страницы портал Azure отображает секретное значение. Копировать и сохранять секретное значение в безопасном месте.

> [!IMPORTANT]
> Для настройки настроек приложения в реализации нужен секрет. Это секретное значение не отображается снова, и оно не может быть извлечено никакими другими средствами. Запишите его, как только он появится на портале Azure.
...
1. Установите перенаправление URI на базовый URL.
1. Выберите **Зарегистрировать**, чтобы создать приложение.
1. Настройте разрешения для приложения. В **разрешениях API**выберите **Добавление разрешения,** а затем **мои API.**
1. Введите имя службы среднего уровня в текстовом поле.
1. Выберите **разрешения,** а затем выберите имя **службы \<доступа>. **
...
| ресурс |обязательно | URI идентификатора приложения принимающей службы (защищенный ресурс). Чтобы узнать URI идентификатора приложения на портале Azure, нажмите **Active Directory** и выберите каталог. Щелкните имя приложения, выберите **Все параметры**, а затем выберите пункт **Свойства**. |
...
| область |обязательно | Список областей для запроса токена, разделенный пробелами. Для OpenID Connect необходимо указать область **openid**.|
