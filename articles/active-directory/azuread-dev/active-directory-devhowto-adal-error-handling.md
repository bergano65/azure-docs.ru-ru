---
title: ADAL клиент ское приложение ошибка обработки передовых методов Azure
...
- [Приложения для коренных клиентов](../develop/developer-glossary.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json#native-client) (iOS, Android, .NET Desktop или Xamarin)
...
При создании собственного клиентского приложения есть несколько типов обработки ошибок, связанных с сетевыми проблемами, временными сбоями и другими ошибками на платформе. В большинстве случаев приложение не должно выполнять немедленные повторы, а должно ждать взаимодействия с конечным пользователем, которое подсказывает ввоза. 
...
|  |  |
|------|-------------|
| **Тип 1.**<br>Ошибка без возможности повторной попытки (в большинстве случаев) | 1. Не пытайтесь немедленно повторить попытку. Представляйте пользовательский интерфейс конечных пользователей на основе конкретной ошибки, которая вызывает повторную попытку (например, "Попробуй войти снова" или "Загрузить приложение брокера Azure AD"). |
| **Тип 2.**<br>Ошибка с возможностью повтора | 1. Выполните одну повторную попытку, поскольку конечный пользователь может войти в состояние, которое приводит к успеху.<br><br>2. Если повторная попытка не удается, представить пользовательского интерфейса конечных пользователей на основе конкретной ошибки, которая вызывает повтор ("Попробуйте войти снова", "Скачать приложение Azure AD брокера" и т.д.). |
...
|  |  |
|------|-------------|
| **Тип 1.**<br>Ошибка устраняется с помощью интерактивного запроса | 1. Если логин () не удается, не выполняйте немедленной повторной попытки. Выполняйте повтор только после запроса пользователя.|
| **Тип 2.**<br>Ошибка не устраняется с помощью интерактивного запроса Ошибка предусматривает возможность повтора. | 1. Выполните одну повторную попытку, поскольку конечный пользователь майор вошел в состояние, которое приводит к успеху.<br><br>2. Если повторная попытка не удается, представить конечному пользователю с действием, основанным на конкретной ошибке, которая может вызвать повторную попытку ("Попробуйте войти снова"). |
| **Дело 3**:<br>Ошибка не устраняется с помощью интерактивного запроса Ошибка не предусматривает возможность повтора. | 1. Не пытайтесь немедленно повторить попытку. Предоставьте пользователю действие на основе конкретной ошибки, которое может вызвать повторную попытку ("Повторите попытку входа"). |
...
### <a name="logging-personal-identifiable-information--organizational-identifiable-information"></a>Регистрация личной идентифицируемой информации & организационной идентифицируемой информации 
По умолчанию журнал ADAL не захватывает и не регистрирует личную идентифицируемую информацию или организационную идентифицируемую информацию. Библиотека позволяет разработчикам приложений включить их запись с помощью метода задания в классе Logger. Регистрируя личную идентифицируемую информацию или организационную идентифицируемую информацию, приложение берет на себя ответственность за безопасную обработку высокочувствительных данных и соблюдение любых нормативных требований.
...
* [Интеграция приложений с активным каталогом Azure][AAD-Integrating-Apps]

Оставляйте свои замечания и пожелания в разделе ниже. Они помогают нам улучшать содержимое веб-сайта.

[![Отображаеткнопку "Войти в систему с помощью Microsoft"][AAD-Sign-In]][AAD-Sign-In]
