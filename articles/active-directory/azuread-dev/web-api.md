...
1. Чтобы веб-приложение могло выполнить проверку подлинности для веб-интерфейса API и получить нужный ресурс, это приложение должно получить маркер доступа. Он делает запрос в точку маркера Azure AD, предоставляя учетные данные, идентификатор приложения и идентификатор приложения URI.
...
1. Через HTTPS веб-приложение использует возвращенный токен доступа JWT для добавления строки JWT с обозначением "Bearer" в заголовке авторизации запроса к веб-API. Затем веб-интерфейс API проверяет этот маркер JWT, и, если проверка будет выполнена успешно, возвращает требуемый ресурс.
...
1. Используя код авторизации, выпущенный Azure AD, веб-приложение отправляет запрос в токенную конечную точку Azure AD, которая включает код авторизации, сведения о клиентском приложении (идентификатор приложения и перенаправление URI) и желаемый ресурс (идентификатор приложения URI для веб-API).
...
Просмотрите примеры кода для сценариев "из веб-приложения в веб-интерфейс API". Регулярно возвращайтесь к этому разделу — мы часто добавляем новые примеры. [Веб-приложение для Web API](sample-v1-code.md#web-applications-signing-in-users-calling-microsoft-graph-or-a-web-api-with-the-users-identity).
...
* Приложение с одним клиентом. Как для варианта с удостоверением приложения, так и с делегированным удостоверением пользователя, веб-приложение и веб-интерфейс API нужно зарегистрировать в одном и том же каталоге в Azure AD. API веб-страниц может быть настроен для выявления набора разрешений, которые используются для ограничения доступа веб-приложения к его ресурсам. Если используется делегированный тип идентификации пользователя, веб-приложению необходимо выбрать желаемые разрешения из **меню «Разрешения» в меню** отсева других приложений на портале Azure. Этот шаг не является обязательным, если используется тип удостоверения приложения.
