...
Прежде чем продолжить работу с этой статьей, убедитесь, что вы установили Фармбеатс и собрали исторические данные с устройств IoT.
...
6. Убедитесь, что для среды задано значение **PowerShell**. По умолчанию он имеет значение bash.

    ![Настройка панели инструментов PowerShell](./media/get-sensor-data-from-sensor-partner/power-shell-new-1.png)

7. Отправьте файл из шага 1 в экземпляре Cloud Shell.

    ![Кнопка "отправить панель инструментов"](./media/get-sensor-data-from-sensor-partner/power-shell-two-1.png)

8. Перейдите в каталог, в который был отправлен файл. По умолчанию файлы передаются в корневой каталог по имени пользователя.

9. Выполните следующий сценарий. Сценарий запрашивает идентификатор клиента, который можно получить на странице обзора Azure Active Directory->.
...
10. Следуйте инструкциям на экране, чтобы записать значения для **конечных точек API**, **идентификаторов** **клиента, идентификаторов**клиентов, **секретов клиента**и **строки подключения EventHub**.
## <a name="create-device-or-sensor-metadata"></a>Создание метаданных устройства или датчика

 Теперь, когда у вас есть необходимые учетные данные, можно определить устройство и датчики. Для этого создайте метаданные, вызвав интерфейсы API Фармбеатс. Обратите внимание, что необходимо вызвать API в качестве клиентского приложения, созданного в предыдущем разделе.

 Фармбеатс Датахуб содержит следующие API, позволяющие создавать метаданные устройства или датчика и управлять ими. Обратите внимание, что в качестве партнера, у которого есть доступ только для чтения, создания и обновления метаданных; **Удаление запрещено партнером.**
