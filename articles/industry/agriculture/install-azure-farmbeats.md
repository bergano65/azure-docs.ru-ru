...
Вам понадобятся следующие разрешения в клиенте Azure, на который вы хотите установить Azure Фармбеатс —

- Клиент — создатель приложения AAD
- Подписка — владелец
- Группа ресурсов, в которой устанавливается Фармбеатс — владелец

Первые два разрешения необходимы для создания шага [приложения AAD](#create-an-aad-application) . При необходимости вы можете получить у человека соответствующие разрешения на создание приложения AAD. Пользователь, устанавливающий Фармбеатс, должен быть владельцем группы ресурсов, в которой устанавливается Фармбеатс.
...
4. Сценарий AAD занимает около 2 минут для запуска и вывода значений на экран, а также для JSON-файла в том же каталоге. Если вы еще не работали с этим сценарием, попросите его поделиться этим выходом.
...
Теперь все готово для установки Фармбеатс. Чтобы начать установку, выполните следующие действия.
...
3. Откроется новое окно с обзором Azure Фармбеатс. Нажмите кнопку **Создать**.
...
5. Укажите адрес электронной почты, который должен получить все оповещения службы, связанные с Azure Фармбеатс, в разделе " **оповещения службы фармбеатс** ". Нажмите кнопку Далее в нижней части страницы, чтобы перейти на вкладку **зависимости** . ![вкладка "основы"](./media/install-azure-farmbeats/create-azure-farmbeats-basics.png)
...
7. Введите имя пользователя и пароль [учетной записи Sentinel](#create-sentinel-account) в разделе Учетная запись Sentinel. Нажмите кнопку Далее, чтобы перейти на вкладку **Проверка и создание** ![вкладке зависимости](./media/install-azure-farmbeats/create-azure-farmbeats-dependencies.png)

8. После проверки указанных сведений нажмите кнопку **ОК**. Откроется страница Условия использования. Ознакомьтесь с условиями и выберите **создать** , чтобы начать установку. Вы автоматически будете перенаправлены на страницу, на которой можно выполнить ход установки.
...
Путь к входному JSON-файлу является необязательным. Если этот параметр не указан, сценарий запросит все необходимые входные данные. Обновление должно завершиться примерно через 30 минут.
