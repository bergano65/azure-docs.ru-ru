...
| План службы приложений   | Ускоритель & датахуб  |
...
Вы можете проверить разрешения на доступ в портал Azure, следуя инструкциям в статье [Управление доступом на основе ролей в Azure](https://docs.microsoft.com/azure/role-based-access-control/check-access).
...
4. Сценарий запрашивает следующие три входа:
    ...
    - **Имя веб-сайта фармбеатс**: это уникальный префикс URL-адреса для вашего приложения фармбеатс. Если префикс уже используется, сценарий выдаст ошибку. После установки развертывание Фармбеатс будет доступно из https:// \<FarmBeats-website-name> . azurewebsites.NET, а API Swagger будут находиться в HTTPS:// \<FarmBeats-website-name> -API.azurewebsites.NET.
...
3. Откроется новое окно с обзором Azure Фармбеатс. Нажмите кнопку **создания**.
...
После завершения установки можно проверить установку и начать использовать портал Фармбеатс, перейдя по имени веб-сайта, которое вы указали во время установки: https:// \<FarmBeats-website-name> . azurewebsites.NET. Вы должны увидеть пользовательский интерфейс Фармбеатс с возможностью создания ферм.

**Датахуб** можно найти по адресу https:// \<FarmBeats-website-name> -API.azurewebsites.NET/Swagger. Здесь вы увидите различные объекты API Фармбеатс и выполняют операции RESTFUL с API.
...
Путь к input.jsу в файле является необязательным. Если не указано, сценарий запросит все необходимые входные данные. Обновление должно завершиться примерно через 30 минут.
