...
ms.locfileid: "86536629"
---
# <a name="get-weather-data-from-weather-partners"></a>Получение данных о погоде от партнеров по погоде

Azure Фармбеатс поможет вам перенести данные о погоде из поставщиков данных погоды с помощью инфраструктуры соединителя на основе DOCKER. С помощью этой платформы поставщики данных Weather реализуют DOCKER, который можно интегрировать с Фармбеатс. В настоящее время поддерживается только поставщик погодных данных [ДТН](https://www.dtn.com/dtn-content-integration/) .
...
2. Выполните запрос GET/{ID} в API/Веасердатамодел для **веасердатамоделид** , как было сделано ранее. Модель данных Weather отображает все метаданные и сведения о полученных данных о погоде. Например, в объекте модели данных Weather мера Weather содержит сведения о том, какие сведения о погоде поддерживаются и в каких типах и единицах. Вот несколько примеров:
