...
2. Выполните запрос GET/{ID} в API/Веасердатамодел для **веасердатамоделид** , как было сделано ранее. Модель данных Weather отображает все метаданные и сведения о полученных данных о погоде. Например, в объекте модели данных Weather мера Weather содержит сведения о том, какие сведения о погоде поддерживаются и в каких типах и единицах. Например:
