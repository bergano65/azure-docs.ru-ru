...
ms.locfileid: "73797690"
---
# <a name="sensor-partner-integration"></a>Интеграция с партнером датчика
В этой статье содержатся сведения о компоненте Azure Фармбеатс **Translator** .
...
Когда клиенты приобрели и развернули устройства и датчики, они могут получить доступ к данным и телеметрии устройства на портале SaaS (программное обеспечение как услуга). Партнеры устройств должны позволить клиентам связать свою учетную запись с экземпляром Фармбеатс в Azure. Для заполнения клиентом/SI необходимы следующие учетные данные:
...
Клиент может редактировать интеграцию Фармбеатс. Основным сценарием для редактирования является изменение секрета клиента или строки подключения из-за истечения срока действия. в этом случае клиент может изменять только следующие поля.
   ...
   > [!NOTE]
   > Редактирование не должно прерывать создание объектов метаданных.
...
парентдевицеид | Идентификатор родительского устройства, к которому подключено это устройство. Предположим, Узел, подключенный к шлюзу; узел будет иметь Парентдевицеид в качестве шлюза |
  ...
  **Модель датчика** | Сенсормодел соответствует метаданным датчика, таким как производитель, тип датчика: аналоговый или цифровой, датчик датчика, например окружающая температура, давление и т. д. |
  ...
  Имя > Сенсормеасурес  | Имя меры датчика. Поддерживается только нижний регистр. Для показателя с различной глубиной укажите глубину. Например, soil_moisture_15cm это имя должно соответствовать данным телеметрии. |
 ...
 Сведения о каждом из объектов и их свойствах см. в разделе [Swagger](httpa://aka.ms/FarmBeatsDatahubSwagger).
