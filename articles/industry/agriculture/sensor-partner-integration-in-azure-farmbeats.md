...
С помощью этого компонента партнеры могут интегрироваться с Фармбеатс с помощью API-интерфейсов концентратора данных Фармбеатс и передавать данные и телеметрию устройства клиента в центр данных Фармбеатс. После того, как данные доступны в Фармбеатс, они выявляются визуализацией с помощью ускорителя Фармбеатс и могут использоваться для создания данных, а также для разработки моделей машинного обучения и искусственного интеллекта.
...
--- | ---
Content-Type | Формат запроса (Content-Type: Application/<format>). Для интерфейсов API концентратора данных Фармбеатс используется формат JSON. Content-Type: application/json
Авторизация | Указывает маркер доступа, необходимый для выполнения вызова API. Авторизация: носитель < доступ — токен >
Принять | Формат ответа. Для интерфейсов API концентратора данных Фармбеатс используется формат JSON. Принять: приложение/JSON
...
Партнеры устройств могут позволить клиентам разорвать связь с существующей интеграцией Фармбеатс. Удаление связи Фармбеатс не должно удалять метаданные устройства или датчика, созданные в концентраторе данных Фармбеатс. Отмена связи выполняет следующие действия.
...
Дополнительные сведения о REST API см. в разделе [REST API](references-for-azure-farmbeats.md#rest-api).
