...
- Имя группы ресурсов
- Прикрепите файл журнала к сбою развертывания, используя приведенные ниже шаги:

    1. Перейдите к группе ресурсов на портале Azure.
    2. Выберите развертывания под разделом «Настройки» на левой стороне.
    3. Для каждого развертывания, которое показывает "Failed", нажмите до сведения и загрузите детали развертывания. Прикрепите этот файл к почте.
...
**Исправление действий**: Проверьте идентификатор электронной почты, для которого вы пытаетесь добавить назначение роли. Идентификатор электронной почты должен точно совпадать с идентификатором, который зарегистрирован для этого пользователя в Active Directory. Если ошибка сохраняется, свяжитесь с нами с сообщением об ошибке / журналами.
...
Если ошибка сохраняется, свяжитесь с нами с сообщением об ошибке / журналами.
...
- Обновление FarmBeats с правильным именем пользователя / пароль, используя шаги ниже и повторить работу.

    *Обновление Имя пользователя Sentinel*
    1. Войти на [портал Azure](https://portal.azure.com).
    2. В поле **поиска** ищите группу ресурсов FarmBeats Datahub.
    3. Выберите хранилище учетной записи хранения хранилища -> контейнеры -> пакет-prep-files -> to_vm -> config.ini
    4. Нажмите кнопку "Правка"
    5. Обновление имени пользователя в разделе sentinel_account

    *Обновление Sentinel Пароль*
    1. Войти на [портал Azure](https://portal.azure.com).
    2. В поле **поиска** ищите группу ресурсов FarmBeats Datahub.
    3. Выберите keyvault-'
    4. Выберите политики доступа в настройках
    5. Нажмите кнопку "Политика добавления доступа"
    6. Используйте "Секретное управление" для настройки из шаблона и добавить себя в главный
    7. Нажмите Добавить, а затем нажмите Сохранить на странице политики доступа
    8. Нажмите Секреты под настройками
    9. Нажмите Sentinel-пароль
    10. Создание новой версии значения и включение
