...
## <a name="use-managed-disks-to-improve-data-reliability-temporarily-disabled"></a>Использование управляемых дисков для повышения надежности данных (временно отключенных)
...
[Агент подключенного компьютера Azure](https://docs.microsoft.com/azure/azure-arc/servers/manage-agent) регулярно обновляется с исправлениями ошибок, улучшениями стабильности и новыми функциями. Мы определили ресурсы, которые не работают с последней версией агента компьютера, и эта рекомендация Advisor предложит вам обновить агент до последней версии для получения лучшего интерфейса ARC в Azure.

## <a name="do-not-override-hostname-to-ensure-website-integrity"></a>Не переопределять имя узла для обеспечения целостности веб-сайта
Помощник рекомендует не переопределять имя узла при настройке шлюза приложений. Наличие другого домена на интерфейсном шлюзе приложений, отличном от того, который используется для доступа к серверной части, потенциально может привести к нарушению работы файлов cookie или перенаправления URL-адресов. Обратите внимание, что это может быть не так во всех ситуациях, а определенные категории незавершенных операций (например REST API) в целом менее чувствительны к этому. Убедитесь, что серверная часть может работать с этим или обновить конфигурацию шлюза приложений, чтобы имя узла не перезаписано в серверную часть. При использовании со службой приложений присоедините пользовательское доменное имя к веб-приложению и Избегайте использования *имени узла azurewebsites.NET в серверной части.* Дополнительные [сведения о пользовательском домене](https://aka.ms/appgw-advisor-usecustomdomain).
