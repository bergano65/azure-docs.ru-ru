...
В конфигурации "активный — активный" оба экземпляра VPN-шлюза будут устанавливать VPN-туннели S2S на локальное VPN-устройство. Когда событие запланированного обслуживания или незапланированное событие происходит с одним экземпляром шлюза, трафик автоматически переключается на другой активный туннель IPsec. Помощник по Azure будет идентифицировать VPN-шлюзы, которые не настроены на режим "активный — активный", и предложит настроить их для обеспечения высокой доступности.
...
Помощник Azure проверяет наличие VPN-шлюзов, являющихся базовыми номерами SKU, и рекомендует вместо этого использовать номер SKU рабочей среды. Номер SKU "базовый" предназначен для целей разработки и тестирования. Производственные номера SKU предлагают большее количество туннелей, поддержку BGP, параметры конфигурации "активный — активный", настраиваемую политику IPSec/IKE и повышенную стабильность и доступность.
...
## <a name="configure-consistent-indexing-mode-on-your-cosmos-db-collection"></a>Настройка режима последовательного индексирования для коллекции Cosmos DB

Azure Cosmos DB контейнеры, настроенные с отложенным режимом индексирования, могут повлиять на актуальность результатов запроса. Помощник обнаружит контейнеры, настроенные таким образом, и порекомендует переключиться в режим согласованности. [Дополнительные сведения о политиках индексирования в Cosmos DB](https://aka.ms/cosmosdb/how-to-manage-indexing-policy)
...
Помощник по Azure выявляет Azure Cosmos DB несекционированные коллекции, которые приближается к квотам подготовленного хранилища. Рекомендуется перенести эти коллекции в новые коллекции с определением ключа секции, чтобы они могли автоматически масштабироваться службой. [Дополнительные сведения о выборе ключа секции](https://aka.ms/cosmosdb/choose-partitionkey)
...
Помощник по Azure определит учетные записи Azure Cosmos DB, которые используют старые версии пакета SDK для .NET, и рекомендует обновление до последней версии с NuGet для последних исправлений, повышения производительности и новых возможностей. [Дополнительные сведения о пакете SDK для Cosmos DB .NET](https://aka.ms/cosmosdb/sql-api-sdk-dotnet)
...
Помощник Azure определит учетные записи Azure Cosmos DB, которые используют старые версии соединителя Cosmos DB Spark, и рекомендует обновление до последней версии с Maven для последних исправлений, повышения производительности и новых возможностей. [Дополнительные сведения о соединителе Cosmos DB Spark](https://aka.ms/cosmosdb/spark-connector)

## <a name="enable-virtual-machine-replication"></a>Включение репликации виртуальных машин
Виртуальные машины, для которых не включена репликация в другой регион, не являются устойчивыми к региональным простоям. Репликация виртуальных машин сокращает любые неблагоприятные последствия для бизнеса во время сбоя региона Azure. Advisor обнаружит виртуальные машины, на которых не включена репликация, и рекомендует включить репликацию, чтобы в случае сбоя можно было быстро подключить виртуальные машины в удаленном регионе Azure. [Дополнительные сведения о репликации виртуальных машин](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-quickstart)
