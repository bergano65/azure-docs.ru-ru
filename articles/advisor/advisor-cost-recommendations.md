...
Чтобы сэкономить 60 % стоимости, рекомендуем хранить моментальные снимки в Службе хранилища (цен. категория 'Стандартный') независимо от типа хранилища родительского диска. Этот параметр используется по умолчанию для моментальных снимков управляемых дисков. Помощник Azure определит моментальные снимки, которые хранятся в хранилище класса Premium, и порекомендует перенести моментальный снимок с уровня "Премиум" в хранилище уровня "Стандартный". [Дополнительные сведения о ценах на управляемые диски](https://aka.ms/aa_manageddisksnapshot_learnmore)

## <a name="utilize-lifecycle-management"></a>Использование управления жизненным циклом
Помощник Azure будет использовать аналитику, касающуюся числа объектов хранилища BLOB-объектов Azure, общего размера и транзакций, чтобы определить, будет ли одна или несколько учетных записей хранения лучше всего подходят для управления жизненным циклом данных на уровне. Будет предложено создать правила управления жизненным циклом, чтобы автоматически распределять данные по уровням, чтобы оптимизировать затраты на хранение, сохранив данные в хранилище BLOB-объектов Azure для совместимости приложений.

## <a name="create-an-ephemeral-os-disk-recommendation"></a>Создание рекомендации по диску временного ОС
Благодаря [эфемерному диску ОС](https://docs.microsoft.com/azure/virtual-machines/windows/ephemeral-os-disks)клиенты получают следующие преимущества: экономия ресурсов хранения для диска ОС. Уменьшите задержку чтения и записи на диск ОС. Ускоренная операция повторного создания образа виртуальной машины путем сброса операционной системы (и временного диска) в исходное состояние. Более рекомендуемое использовать временный диск ОС для кратковременных виртуальных машин IaaS или виртуальных машин с рабочими нагрузками без отслеживания состояния. Помощник имеет рекомендации по ресурсам, которые могут использовать преимущества временного диска ОС. 
