...
## <a name="improve-azure-cache-for-redis-performance-and-reliability"></a>Повышение надежности и производительности кэша Redis для Azure

Помощник выявляет экземпляры кэша Redis для Azure, на производительность которых может отрицательно влиять использование большого объема памяти, нагрузка на сервер, пропускная способность сети или большое количество клиентских подключений. Он также предоставляет практические рекомендации, позволяющие избежать возможных проблем. Дополнительные сведения см. в статье с рекомендациями по использованию [Помощника по кэшу Redis для Azure](https://azure.microsoft.com/documentation/articles/cache-configure/#redis-cache-advisor).
