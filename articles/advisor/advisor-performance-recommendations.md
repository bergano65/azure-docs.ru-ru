...
Дополнительные сведения о SQL Помощник по базам данных см. в разделе [sql помощник по базам данных](https://azure.microsoft.com/documentation/articles/sql-database-advisor/).

## <a name="upgrade-your-storage-client-library-to-the-latest-version-for-better-reliability-and-performance"></a>Обновление клиентской библиотеки для службы хранилища до последней версии для повышения надежности и производительности

Последняя версия клиентской библиотеки для службы хранилища и соответствующего пакета SDK содержит исправления проблем, о которых сообщили клиенты и которые мы заблаговременно выявили благодаря контролю качества. Кроме того, в последней версии оптимизирована надежность и производительность, а также добавлены новые функции, которые в целом делают службу хранилища Azure удобнее. Advisor предоставляет рекомендации и инструкции по обновлению до последней версии пакета SDK, если вы используете устаревшую версию. Рекомендации для поддерживаемых языков — C++ и .NET.
...
## <a name="consider-increasing-the-size-of-your-vnet-gateway-sku-to-adress-high-p2s-use"></a>Попробуйте увеличить размер SKU шлюза виртуальной сети, чтобы он P2S использование.

Каждый SKU шлюза может поддерживать только указанное число одновременных подключений P2S. Если число подключений приближается к ограничению шлюза, то может произойти сбой дополнительных попыток подключения. Увеличение размера шлюза позволит поддерживать больше одновременных пользователей P2S. Помощник предоставляет рекомендации и действия, которые необходимо выполнить для этого.

## <a name="consider-increasing-the-size-of-your-vnet-gateway-sku-to-address-high-cpu"></a>Рассмотрите возможность увеличения размера SKU шлюза виртуальной сети для решения высокой загрузки ЦП.

При интенсивной нагрузке трафика VPN-шлюз может удалять пакеты из-за высокой загрузки ЦП. Рекомендуется обновить SKU VPN-шлюза, так как VPN-подключение постоянно выполняется в. Увеличение размера VPN-шлюза гарантирует, что подключения не будут отброшены из-за высокой загрузки ЦП. Advisor провдес рекомендует решить эту задачу заранее. 

## <a name="increase-batch-size-when-loading-to-maximize-load-throughput-data-compression-and-query-performance"></a>Увеличение размера пакета при загрузке, чтобы обеспечить максимальную пропускную способность, сжатие данных и производительность запросов

Помощник может определить, что можно увеличить производительность и пропускную способность, увеличив размер пакета при загрузке в базу данных. Можно использовать инструкцию COPY. Если вы не можете использовать инструкцию COPY, рассмотрите возможность увеличения размера пакета при использовании таких служебных программ, как API SQLBulkCopy или BCP. хорошим правилом для параметра Thumb является размер пакета в диапазоне от 100 до 1 млн строк. Это позволит повысить пропускную способность, сжатие данных и производительность запросов.

## <a name="co-locate-the-storage-account-within-the-same-region-to-minimize-latency-when-loading"></a>Совместное размещение учетной записи хранения в том же регионе для снижения задержки при загрузке

Помощник может определить, что загрузка выполняется из региона, отличного от пула SQL. Для снижения задержки при загрузке данных следует рассмотреть возможность загрузки из учетной записи хранения, расположенной в том же регионе, что и пул SQL. Это поможет снизить задержку и повысить производительность загрузки.

## <a name="unsupported-kubernetes-version-is-detected"></a>Обнаружена неподдерживаемая версия Kubernetes

Advisor может определить, обнаружена ли неподдерживаемая версия Kubernetes. Рекомендации помогут обеспечить работу кластера Kubernetes с поддерживаемой версией.
