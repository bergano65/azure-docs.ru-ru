...
## <a name="update-your-current-compute-management-sdk-version-to-the-most-recent-version"></a>Обновите текущую версию пакета SDK для управления вычислениями до самой последней.
Advisor определяет подписки, которые имеют операции с устаревшими версиями пакета SDK для управления вычислением. Это может повлиять на безопасность и производительность рабочих нагрузок, поэтому помощник рекомендует перейти на последнюю версию пакета SDK для управления COMPUTE. 
