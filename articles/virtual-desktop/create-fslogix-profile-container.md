...
3.  По завершении выберите **Next: Протокол \> @ no__t-1 @ no__t-2, чтобы открыть вкладку Протокол и настроить параметры доступа к тому.
...
3. В файле перейдите на **x64** > **выпуски** и запустите **фслогиксаппссетуп. exe**. Откроется меню установки.
...
7. Перейдите в раздел **C: \\Program Files @ no__t-2FSLogix @ no__t-3Apps** , чтобы подтвердить установку агента.

8. В меню Пуск запустите **regedit** от имени администратора.

9. Перейдите в раздел **Computer @ no__t-1HKEY_LOCAL_MACHINE @ no__t-2software @ no__t-3FSLogix**.
...
1. Откройте Интернет-браузер и перейдите в @no__t — 0.
...
6. Подключитесь напрямую к любой виртуальной машине из пула узлов с помощью удаленный рабочий стол и откройте **Проводник.** Затем перейдите к **пути подключения** (в следующем примере путь подключения — @no__t -1\\anf-SMB-3863.gt1107.onmicrosoft.com @ no__t-3ANF-VOL).
