...
Профиль пользователя содержит элементы данных об отдельных пользователей, включая сведения о конфигурации, такие как параметры рабочего стола, постоянные сетевые подключения и параметры приложения. По умолчанию Windows создает профиль локального пользователя, который тесно интегрирован с операционной системой.

Профиль удаленного пользователя предоставляет секции между данными пользователя и операционной системы. Он позволяет операционной системе заменять или изменить, не влияющие на пользовательские данные. В узла сеансов удаленных рабочих столов (RDSH) и инфраструктуры виртуальных рабочих столов (VDI) операционная система может быть заменен по следующим причинам:
...
19 ноября 2018 года [Корпорация Майкрософт приобрела FSLogix](https://blogs.microsoft.com/blog/2018/11/19/microsoft-acquires-fslogix-to-enhance-the-office-365-virtualization-experience/). FSLogix адреса, которые являются профиля контейнера сложностями, ключей между ними:
