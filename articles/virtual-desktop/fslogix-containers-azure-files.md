...
| ---------- | :-------------: | :------------: | :---------: | --------: | :---------------------: | :-----------------------: | :--------------------------: | :-------------: | :---------------------: |-----|
| **Диски профилей пользователей (UPD)** | Yes | Yes | Yes | Yes | Yes | Нет | Yes | Win 7 + | Yes | |
| **Перемещаемый профиль пользователя (RUP), режим обслуживания** | Нет | Yes | Yes | Yes | Yes| Нет | Yes | Win 7 + | Нет | |
| **Enterprise State Roaming (ESR)** | Yes | Нет | Yes | Нет | См. примечания | Yes | Нет | Windows 10 | Нет | Функции на SKU сервера, но не поддерживают пользовательский интерфейс |
| **Виртуализация взаимодействия с пользователем (UE-V)** | Yes | Yes | Yes | Нет | Yes | Нет | Yes | Win 7 + | Нет |  |
| **Облачные файлы OneDrive** | Нет | Нет | Нет | Yes | См. примечания | См. примечания  | См. примечания | Win 10 RS3 | Нет | Не тестировался на сервере SKU. Хранилище серверной части в Azure зависит от клиента синхронизации. Для хранения серверной части локального хранилища требуется клиент синхронизации. |
