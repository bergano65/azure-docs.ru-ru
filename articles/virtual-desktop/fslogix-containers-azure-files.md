---
title: Файлы контейнеров профилей Фслогикс виртуальных рабочих столов Windows — Azure
...
ms.locfileid: "99225921"
---
# <a name="fslogix-profile-containers-and-azure-files"></a>Контейнеры профилей FSLogix и Файлы Azure

Служба виртуальных рабочих столов Windows рекомендует Фслогикс контейнеры профилей в качестве решения для профилей пользователей. Набор решений FSLogix предназначен для перемещения профилей в удаленные вычислительные среды, например Виртуальный рабочий стол Windows. Он хранит полный профиль пользователя в одном контейнере. При входе в систему этот контейнер динамически вкладывается в вычислительную среду с помощью встроенного виртуального жесткого диска (VHD) и виртуального жесткого диска Hyper-V (VHDX). Профиль пользователя доступен сразу и отображается в системе точно так же, как собственный профиль пользователя. В этой статье описывается, как контейнеры профилей Фслогикс используются с функциями службы файлов Azure в виртуальном рабочем столе Windows.
...
| Технология | Современные параметры | Параметры Win32 | Параметры ОС | Пользовательские данные | Поддерживается на SKU сервера | Хранилище серверной части в Azure | Локальное хранилище серверной части | Поддерживаемые версии | Время следующего входа |Примечания|
| ---------- | :-------------: | :------------: | :---------: | --------: | :---------------------: | :-----------------------: | :--------------------------: | :-------------: | :---------------------: |-----|
...
UPD требует [Локальные дисковые пространства (S2D)](/windows-server/remote/remote-desktop-services/rds-storage-spaces-direct-deployment/) для устранения требований к производительности. UPD использует протокол SMB. Он копирует профиль на виртуальную машину, в которой регистрируется пользователь.
...
- **Производительность:** [Контейнеры профилей фслогикс](/fslogix/configure-profile-container-tutorial/) — это высокая производительность и устранение проблем с производительностью, в которых кэшированный режим обмена с предысторией заблокирован.
- **OneDrive:** Без контейнеров профилей Фслогикс OneDrive для бизнеса не поддерживается в непостоянных средах удаленных рабочих столов или в среде VDI. На [странице поддержки OneDrive для VDI](/onedrive/sync-vdi-support) вы узнаете, как они взаимодействуют. Дополнительные сведения см. в статье [Использование клиента синхронизации на виртуальных рабочих столах](/deployoffice/rds-onedrive-business-vdi/).
...
- Разрешения для файлов Azure должны соответствовать разрешениям, описанным в разделе [требования — контейнеры профилей](/fslogix/fslogix-storage-config-ht).
- Каждая виртуальная машина пула узлов должна быть создана на основе одного и того же типа и размера виртуальной машины, основанной на одном и том же главном образе.
...
- Чтобы приступить к созданию решения для виртуализации настольных систем, см. раздел [Создание клиента в виртуальном рабочем столе Windows](./virtual-desktop-fall-2019/tenant-setup-azure-active-directory.md).
