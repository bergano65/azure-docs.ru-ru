...
4. После этого перейдите в раздел **данные** > **счетчики производительности Windows** и добавьте следующие счетчики:
    ...
    -   Логический диск (C\\:), СР. Disk Queue Length
...
Дополнительные сведения о счетчиках производительности в [источниках данных производительности Windows и Linux см. в Azure Monitor](/articles/azure-monitor/platform/data-sources-performance-counters.md).
...
8. Теперь перейдите к ресурсам Azure, выберите ресурс служб приложений Azure с именем, указанным в шаблоне, и перейдите по URL-адресу, связанному с ним. (Например, если имя приложения, использованное в шаблоне, имело `contosoapp45`значение, связанный URL-адрес — <https://contosoapp45.azurewebsites.net>).
...
-  Пользователям также требуется доступ на чтение для клиента виртуальных рабочих столов Windows (роль RDS Reader). Дополнительные сведения см. [в разделе делегированный доступ в Windows Virtual Desktop Preview](delegated-access-virtual-desktop.md).
...
- Логический диск\*(\|)% свободного места:
    ...
    - Порог: Менее 20% помечены как неработоспособные.

- Логический диск (C\\:), СР. Длина очереди диска:

    - Представляет условия системы хранения.
    - Порог: Выше 5 помечается как неработоспособное.

- Доступная\*память\\(МБ):

    - Доступная память для системы.
    - Порог: Менее 500 мегабайтов помечены как неработоспособные.

- Сведения о процессоре\\(\*) время процессора:

    - Порог: Более 80% помечается как неработоспособное.

- [Максимальная задержка ввода данных пользователем на\*сеанс\\()](https://docs.microsoft.com/windows-server/remote/remote-desktop-services/rds-rdsh-performance-counters):

    - Порог: Более 2000 мс помечены как неработоспособные.
