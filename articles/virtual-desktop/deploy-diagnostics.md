...
4. После этого перейдите к разделу **данные** > **счетчики производительности Windows** и добавьте следующие счетчики:

    -   Логический диск (\*)\\% свободного места
    -   Логический диск (C:)\\средняя длина очереди диска
    -   Память (\*)\\доступных МБ
    -   Сведения о процессоре (\*)\\время процессора
    -   Задержка ввода данных пользователем на сеанс (\*)\\максимальной задержки ввода
...
4. Убедитесь, что следующие счетчики предварительно настроены:
   ...
   - Память (\*)\\доступных МБ: объем доступной памяти для системы в мегабайтах.
