---
title: Автоматическое масштабирование узлов сеансов предварительной версии виртуальных рабочих столов Windows — Azure
description: Описывает, как настроить скрипт автоматического масштабирования для узлов сеансов предварительной версии виртуальных рабочих столов Windows.
...
4. Выполните следующий командлет:
    ...
    Например, **Set-Variable-Name ключевое имя области действия Global-Value "c\\: масштабирования-HostPool1"**
5. Выполните командлет **New-сторедкредентиал- \$ключевое ключевое** значение. При появлении запроса введите учетные данные виртуального рабочего стола Windows с разрешениями на запрос пула узлов (пул узлов указан в **файле config. XML**).
    - Если вы используете разные субъекты-службы или учетную запись Standard, запустите командлет **сторедкредентиал- \$ключевой ключевой путь** один раз для каждой учетной записи, чтобы создать локальные сохраненные учетные данные.
...
7. В диалоговом окне **Создание действия** введите **PowerShell. exe** в поле **Program/Script** , а затем введите **C:\\rescaling\\басикскале. ps1** в поле **Добавить аргументы (необязательно)** .
