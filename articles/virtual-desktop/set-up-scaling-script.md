---
title: Динамическое масштабирование узлов сеансов виртуальных рабочих столов Windows — Azure
...
2. Создайте папку на виртуальной машине с масштабируемым классом для хранения скрипта масштабирования и его конфигурации (например, **C:\\Scale-HostPool1**).
3. Скачайте файлы **басикскале. ps1**, **config. JSON**и **Функтионс-пссторедкредентиалс. ps1** и папку **повершеллмодулес** из [репозитория сценариев масштабирования](https://github.com/Azure/RDS-Templates/tree/master/wvd-sh/WVD%20scaling%20script) и скопируйте их в папку, созданную на шаге 2. Существует два основных способа получения файлов перед их копированием на виртуальную машину с масштабируемым классом:
...
4. Выполните следующий командлет:
    ...
    Например, **Set-Variable-Name ключевое имя области действия Global-Value "c:\\rescaling-HostPool1"**
5. Выполните командлет **New-сторедкредентиал-ключевой \$ключевого пути** . При появлении запроса введите учетные данные виртуального рабочего стола Windows с разрешениями на запрос пула узлов (пул узлов указан в **файле config. JSON**).
    - Если вы используете разные субъекты-службы или учетную запись Standard, выполните командлет **New сторедкредентиал-ключевой \$ключевой путь** один раз для каждой учетной записи, чтобы создать локальные сохраненные учетные данные.
6. Выполните команду **Get-сторедкредентиал-List** , чтобы подтвердить, что учетные данные были успешно созданы.

### <a name="configure-the-configjson-file"></a>Настройка файла config. JSON

Введите соответствующие значения в следующие поля, чтобы обновить параметры сценария масштабирования в файле config. JSON:
...
| рдброкер                      | URL-адрес службы ВВД, значение по умолчанию HTTPS:\//rdbroker.wvd.microsoft.com             |
...
После настройки JSON-файла конфигурации необходимо настроить планировщик задач для запуска файла Басикскалер. ps1 через равные промежутки времени.
...
7. В диалоговом окне **Создание действия** введите **PowerShell. exe** в поле **Program/Script** , а затем введите **C:\\масштабирования\\басикскале. ps1** в поле **Добавить аргументы (необязательно)** .
...
Этот скрипт масштабирования считывает параметры из файла config. JSON, включая начало и конец пикового периода использования в течение дня.

Во время пикового использования скрипт проверяет текущее число сеансов и текущее количество выполняемых ресурсов для каждого пула узлов. Он вычисляет, достаточно ли емкости работающих виртуальных машин узла сеансов для поддержки существующих сеансов на основе параметра Сессионсрешолдперкпу, определенного в файле config. JSON. В противном случае скрипт запускает дополнительные виртуальные машины узла сеансов в пуле узлов.

Во время непикового использования Скрипт определяет, какие виртуальные машины узла сеансов должны завершить работу в соответствии с параметром Минимумнумберофрдш в файле config. JSON. Сценарий установит для виртуальных машин узла сеанса режим стока, чтобы предотвратить подключение новых сеансов к узлам. Если для параметра **лимитсекондстофорцелогоффусер** в файле config. JSON задано ненулевое положительное значение, то сценарий будет уведомлять всех пользователей, выполнивших вход в систему, сохранить работу, подождать настроенное время, а затем вынудить пользователей выйти из нее. После того как все пользовательские сеансы будут отключены на виртуальной машине узла сеансов, сценарий завершит работу сервера.

Если для параметра **лимитсекондстофорцелогоффусер** в файле config. JSON задано значение 0, скрипт разрешит параметр конфигурации сеанса в свойствах пула узлов, чтобы выполнять выход из сеансов пользователя. Если на виртуальной машине узла сеансов находятся какие-либо сеансы, виртуальная машина узла сеансов будет оставлена работающей. Если сеансов нет, сценарий завершит работу виртуальной машины узла сеансов.
