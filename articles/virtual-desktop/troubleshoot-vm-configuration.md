...
## <a name="remote-licensing-model-is-not-configured"></a>Удаленный модель лицензирования не настроена

Если вы войдете Windows 10 Корпоративная нескольких сеансу, с помощью учетной записи администратора, может появиться уведомление об ошибке: «режима лицензирования удаленных рабочих столов не настроен, службы удаленных рабочих столов перестанет работать через X дней. На сервере посредника подключений используйте диспетчер сервера Чтобы задать режим лицензирования удаленного рабочего стола.» Если вы видите это сообщение, значит, необходимо вручную настроить режим лицензирования для **на пользователя**.

Чтобы вручную настроить режим лицензирования:  

1. Перейдите к **меню "Пуск"** поле поиска, а затем найти и открыть **gpedit.msc** для доступа к редакторе локальной групповой политики. 
2. Перейдите к **Конфигурация компьютера** > **административные шаблоны** > **компоненты Windows**  >   **Службы удаленных рабочих столов** > **узла сеансов удаленных рабочих столов** > **лицензирования**. 
3. Выберите **Установка режима лицензирования удаленного рабочего стола** и измените его на **на пользователя**.

Мы рассматриваем в настоящее время проблемы времени ожидания уведомлений и льготный период и план для их устранения в будущем обновлении. 
