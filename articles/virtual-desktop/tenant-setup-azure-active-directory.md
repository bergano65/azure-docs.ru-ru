...
ms.locfileid: "75459506"
---
# <a name="tutorial-create-a-tenant-in-windows-virtual-desktop"></a>Руководство. Создание клиента в Виртуальном рабочем столе Windows
...
2. В разделе **Корпоративные приложения** найдите **Windows Virtual Desktop** (Виртуальный рабочий стол Windows). Отобразятся два приложения, для которых вы предоставили согласие на предыдущем этапе. Из этих двух приложений выберите **Windows Virtual Desktop** (Виртуальный рабочий стол Windows).
   ...
   ![Снимок экрана: результаты поиска "Виртуальный рабочий стол Windows" в разделе "Корпоративные приложения". Приложение "Виртуальный рабочий стол Windows" выделено.](media/tenant-enterprise-app.png)
3. Выберите **Пользователи и группы**. Вы можете заметить, что администратор, которому предоставлено согласие для приложения, уже присутствует в списке с назначенной ролью**Доступ по умолчанию**. Этого недостаточно, чтобы создать клиента виртуального рабочего стола Windows. Далее назначьте пользователю роль **TenantCreator**, выполнив указания ниже.
   ...
   ![Снимок экрана: пользователи и группы, назначенные для управления корпоративным приложением "Виртуальный рабочий стол Windows". На снимке экрана показано только одно назначение для "Доступ по умолчанию".](media/tenant-default-access.png)
4. Выберите **Добавить пользователя**, а затем в колонке **Добавление назначения** щелкните **Пользователи и группы**.
...
7. На странице **виртуального рабочего стола Windows — пользователи и группы**, убедитесь, что вы видите новую запись с ролью **TenantCreator**, назначенной пользователю, который создаст клиента виртуального рабочего стола Windows.
   ...
   ![Снимок экрана: пользователи и группы, назначенные для управления корпоративным приложением "Виртуальный рабочий стол Windows". Снимок экрана теперь отображает вторую запись пользователя с ролью TenantCreator.](media/tenant-tenant-creator-added.png)
...
1. В том же сеансе [портала Azure](https://portal.azure.com) найдите и выберите **Azure Active Directory**.
   ...
   ![Снимок экрана: результаты поиска для "Azure Active Directory" на портале Azure. Результат поиска под разделом "Службы" выделен.](media/tenant-search-azure-active-directory.png)
...
3. Найдите **Идентификатор каталога**, а затем щелкните значок буфера обмена. Вставьте его в удобное расположение, чтобы можно было использовать его позже как значение **AadTenantId**.
   ...
   ![Снимок экрана: свойства Azure Active Directory. Указатель мыши наведен на значок буфера обмена "Идентификатор каталога" для копирования и вставки.](media/tenant-directory-id.png)

Поиск идентификатора подписки Azure:
1. В том же сеансе [портала Azure](https://portal.azure.com) найдите и выберите **Подписки**.
   
   ![Снимок экрана: результаты поиска для "Azure Active Directory" на портале Azure. Результат поиска под разделом "Службы" выделен.](media/tenant-search-subscription.png)
...
3. Найдите **Идентификатор подписки**, а затем наведите указатель мыши над значением пока не появится значок буфера обмена. Выберите значок буфера обмена и вставьте его в удобное расположение, чтобы его можно было использовать позже как значение **AzureSubscriptionId**.
   ...
   ![Снимок экрана: свойства подписки Azure. Указатель мыши наведен на значок буфера обмена "Идентификатор подписки" для копирования и вставки.](media/tenant-subscription-id.png)
