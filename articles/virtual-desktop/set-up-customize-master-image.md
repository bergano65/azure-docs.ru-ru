...
1. Установка Office 365, следуя инструкциям в [Подготовка образа Office](set-up-customize-master-image.md#office-image-preparation).
2. Установить дополнительные приложения, следуя инструкциям в [Настройка профиля пользователя (FSLogix)](set-up-customize-master-image.md#user-profile-setup-fslogix), [Защитник Windows](set-up-customize-master-image.md#windows-defender), и [другие приложения и конфигурации реестра](set-up-customize-master-image.md#other-applications-and-registry-configuration).
...
Чтобы включить контейнер FSLogix как часть изображения, следуйте инструкциям в [Настройка общей папки профиля пользователя в пуле узлов](create-host-pools-user-profile.md#configure-the-fslogix-profile-container).

При настройке реестра файл общий ресурс, использовать файловый ресурс, созданный в [настроить разрешения для файлового сервера](set-up-customize-master-image.md#configure-permissions-for-the-file-server) где планируется хранить контейнеры профиля. Также можно проверить функциональные возможности с помощью этого контейнера FSLogix [быстрого запуска](https://docs.fslogix.com/display/20170529/Profile+Containers+-+Quick+Start).
