...
- Если вы предпочитаете PowerShell, см. статью [создание log Analytics рабочей области с помощью PowerShell](../azure-monitor/platform/powershell-workspace-configuration.md).
...
Дополнительные сведения об образце интерфейса запроса см. в [Azure Monitor log Analytics](../azure-monitor/log-query/example-queries.md).

Следующий список запросов позволяет просматривать сведения о подключении или проблемы для одного пользователя. Эти запросы можно выполнить в [редакторе запросов log Analytics](../azure-monitor/log-query/log-analytics-tutorial.md#write-a-query). Для каждого запроса замените `userupn` именем участника-пользователя, которого требуется найти.
