...
6. По завершении процесса он создаст ресурс с именем **AzureRunAsConnection** в указанной учетной записи службы автоматизации Azure. Выберите **учетную запись запуска от имени Azure**. Этот ресурс содержит идентификаторы приложения, клиента и подписки, а также отпечаток сертификата. Запишите идентификатор приложения, он понадобится вам позже. Эти же сведения можно найти на странице **подключения** . Чтобы открыть эту страницу, на панели в левой части окна выберите **подключения** в разделе **Общие ресурсы** и щелкните ресурс подключения с именем **AzureRunAsConnection**.
...
Далее необходимо создать назначение ролей, чтобы **AzureRunAsConnection** мог взаимодействовать с виртуальным рабочим столом Windows. Обязательно войдите в учетную запись в PowerShell, у которой есть разрешения на создание назначений ролей.
...
При обнаружении клиента с пулами узлов, которые необходимо масштабировать, следуйте инструкциям в статье [Создание учетной записи запуска от имени для службы автоматизации Azure](#create-an-azure-automation-run-as-account) , чтобы получить идентификатор приложения **AzureRunAsConnection** и использовать имя клиента виртуальных рабочих столов Windows, полученное из предыдущего командлета, для создания назначения ролей.
...
5. Выполните следующий сценарий PowerShell, чтобы создать приложение логики Azure и расписание выполнения для пула узлов.
