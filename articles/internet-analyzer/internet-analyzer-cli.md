...
description: Из этой статьи вы узнаете, как создать тест Анализатора Интернета с помощью Azure CLI.
...
2. [Установка Azure CLI](/cli/azure/install-azure-cli?view=azure-cli-latest).
...
5. Выберите идентификатор подписки, которому был предоставлен доступ к общедоступной предварительной версии Анализатора Интернета.
    ...
    После входа в систему появится список подписок, связанных с учетной записью Azure. В сведениях о подписке с `isDefault: true` указана текущая активная подписка, отображаемая после входа. Чтобы выбрать другую подписку, укажите идентификатор подписки с помощью команды [az account set](/cli/azure/account#az-account-set). См. дополнительные сведения о [выборе нужной подписки при использовании нескольких подписок Azure](/cli/azure/manage-azure-subscriptions-azure-cli?view=azure-cli-latest).

    Вы также можете войти в автономном режиме, как описано в руководстве по [входу с помощью Azure CLI](/cli/azure/authenticate-azure-cli?view=azure-cli-latest).
...
12. Чтобы начать создание измерений, в веб-приложение нужно встроить файл JavaScript, на который указывает объект **scriptFileUri** теста. Специальные инструкции можно найти на странице [Embed the Internet Analyzer client](internet-analyzer-embed-client.md) (Внедрение клиента Анализатора Интернета).
...
* Полный список поддерживаемых команд и примеры использования см. в справочнике по [интерфейсу командной строки службы "Анализатор Интернета"](/cli/azure/ext/internet-analyzer/internet-analyzer?view=azure-cli-latest).
