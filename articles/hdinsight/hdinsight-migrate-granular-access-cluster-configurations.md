...
Вызовы `GET /configurations` `GET /configurations/{configurationName}` и `POST /configurations/gateway` больше не будут возвращать никаких сведений, и вызов больше не будет возвращать конфиденциальные параметры, такие как ключи учетной записи хранения или пароль кластера. То же самое относится к соответствующим методам SDK и командлетам PowerShell.
