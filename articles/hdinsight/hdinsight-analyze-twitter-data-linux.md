...
Узнайте, как использовать [Apache Hive](https://hive.apache.org/) для обработки данных Twitter. Результатом является список пользователей Twitter, отправивших большинство твитов, которые содержат определенное слово.
...
Twitter позволяет получать данные для каждого твита в виде документа JSON с помощью интерфейса REST API. [OAuth](https://oauth.net) .

### <a name="create-a-twitter-application"></a>Создание приложения Twitter

1. Из веб-браузера, [https://developer.twitter.com/apps/](https://developer.twitter.com/apps/)войти на . Выберите ссылку **Регистрация,** если у вас нет учетной записи Twitter.
...
4. Выберите **Да, я согласен,** а затем выберите **Создать приложение Twitter**.

5. Выберите вкладку **«Разрешения».** Разрешение по умолчанию **читается только**.

6. Откройте вкладку **Ключи и токены доступа** .

7. Выберите **Создайте токен доступа**.

8. Выберите **Test OAuth** в правом верхнем углу страницы.
...
1. Используйте следующие команды для установки [Tweepy,](https://www.tweepy.org/) [панели Прогресса](https://pypi.python.org/pypi/progressbar/2.2)и других необходимых пакетов:
...
1. Создайте файл **gettweets.py** с помощью следующей команды:

   ```bash
   nano gettweets.py
   ```

1. Отоверьте код ниже, `Your consumer secret` `Your consumer key`заменив , , `Your access token`и `Your access token secret` с соответствующей информацией из вашего приложения Twitter. Затем вставьте отредактированный код как содержимое **gettweets.py** файла.
...
1. Нажмите клавиши **Ctrl + X**, а затем **Y** (Да) для сохранения файла.

1. Чтобы запустить файл и скачать твиты, выполните следующую команду:
   ...
   > [!NOTE]  
   > Если индикатор хода выполнения перемещается очень медленно, то следует изменить фильтр, чтобы отслеживать популярные темы. Когда есть много твитов на эту тему в фильтре, вы можете быстро получить 100 твитов необходимо.
...
1. Используйте следующую команду для создания файла, содержащего операторы [Hive-L:](https://cwiki.apache.org/confluence/display/Hive/LanguageManual)
...
1. Выполните приведенную ниже команду, чтобы запустить код HiveQL в файле:
...
1. Используйте следующий запрос в командной строке Beeline, чтобы убедиться, что данные импортированы:
...
Вы узнали, как преобразовать неструктурированный набор данных JSON в структурированную таблицу [Apache Hive.](https://hive.apache.org/) Дополнительные сведения о Hive в HDInsight см. в следующих документах:

* [Приступая к работе с HDInsight](hadoop/apache-hadoop-linux-tutorial-get-started.md)
* [Анализ данных о задержке рейсов с помощью HDInsight](/azure/hdinsight/interactive-query/interactive-query-tutorial-analyze-flight-data)
