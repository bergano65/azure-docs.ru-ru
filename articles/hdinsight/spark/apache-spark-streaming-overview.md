...
description: Как использовать приложения потоковой передачи Apache Spark в кластерах HDInsight Spark.
...
Подождите около 30 секунд после запуска приложения выше.  Затем можно периодически запросить DataFrame, чтобы увидеть текущий набор значений, присутствующих в пакете, например, с помощью этого запроса S'L:
