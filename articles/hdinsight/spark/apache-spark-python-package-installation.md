...
В HDInsight имеется две встроенные установки Python в кластере Spark, Anaconda Python 2,7 и Python 3,5. Клиентам может потребоваться настроить среду Python. Например, установка внешних пакетов Python или другой версии Python. Здесь мы покажем лучшую методику безопасного управления окружениями Python для Apache Spark кластеров в HDInsight.
...
2. При необходимости установите внешние пакеты Python в созданную виртуальную среду. Выполните действия сценария в кластере для всех узлов со следующим скриптом, чтобы установить внешние пакеты Python. Для записи файлов в папку виртуальной среды необходимо иметь права sudo.
    ...
    Найдите в [индексе пакета](https://pypi.python.org/pypi) полный список доступных пакетов. Его также можно получить из других источников. Например, можно установить пакеты, предоставляемые посредством [conda-forge](https://conda-forge.org/feedstocks/).
...
Существует известная ошибка для версии `4.7.11`Anaconda, `4.7.12`и. `4.8.0` Если вы видите, что действия сценария зависает при `"Collecting package metadata (repodata.json): ...working..."` сбое. `"Python script has been killed due to timeout after waiting 3600 secs"` Вы можете скачать [этот скрипт](https://gregorysfixes.blob.core.windows.net/public/fix-conda.sh) и запустить его как действия сценария на всех узлах, чтобы устранить проблему.
