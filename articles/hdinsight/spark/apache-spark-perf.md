...
ms.locfileid: "73177364"
---
# <a name="optimize-apache-spark-jobs-in-hdinsight"></a>Оптимизация заданий Apache Spark в HDInsight

Узнайте, как оптимизировать конфигурацию кластера [Apache Spark](https://spark.apache.org/) для определенной рабочей нагрузки.  Наиболее распространенной проблемой является неправильное использование памяти из-за неправильных конфигураций (особенно неправильного размера исполнительных исполнителей), длительных операций и задач, которые приводят к появлению операций Декарт. Ускорить выполнение заданий можно с помощью соответствующего кэширования и разрешения [неравномерного распределения данных](#optimize-joins-and-shuffles). Для повышения производительности мы рекомендуем отслеживать и просматривать выполнения длительных и ресурсоемких заданий Spark.
...
| Хранилище BLOB-объектов Azure (безопасное) | **wasbs:** //УРЛ/ | **Стандартный** | ДА | Промежуточный кластер |
| Azure Data Lake Storage Gen 2| **абфс:** //УРЛ/ | **Более быстрая** | ДА | Промежуточный кластер |
...
* Кэширование на уровне хранилища (рекомендуется)
    * Можно реализовать с помощью [Alluxio](https://www.alluxio.io/).
