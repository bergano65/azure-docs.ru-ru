...
Более ранние версии Spark использовать наборы RDD для абстракции данных Spark 1.3 и 1.6, к которому представляет кадры и наборы данных, соответственно. Рассмотрим следующие относительные характеристики:
...
При создании кластера Spark вы можете выбрать хранилище BLOB-объектов Azure или Azure Data Lake Storage как хранилище кластера по умолчанию. Оба варианта предоставляют возможность долговременного хранения промежуточных кластеров, что означает, что данные не будут автоматически удалены при удалении кластера. Вы можете повторно создать промежуточный кластер и по-прежнему иметь доступ к данным.

| Тип хранилища данных | Файловая система | Скорость | Промежуточный | Варианты использования |
| --- | --- | --- | --- | --- |
| Хранилище BLOB-объектов Azure | **wasb [s]:**//url/ | **Стандартные** | Да | Промежуточный кластер |
| Общего хранилища Озера данных Azure 2| **abfs [s]:**//url/ | **Быстрее** | Да | Промежуточный кластер |
| Хранилище Azure Data Lake поколения 1| **adl:**//url/ | **Быстрее** | Да | Промежуточный кластер |
| Локальная система HDFS | **hdfs:**//url/ | **Самый быстрый** | Нет | Интерактивный постоянно доступный кластер |
...
Если вы используете [Apache Hadoop YARN](https://hadoop.apache.org/docs/current/hadoop-yarn/hadoop-yarn-site/YARN.html), эта платформа управляет максимальным объемом памяти, используемой всеми контейнерами на всех узлах Spark.  На схеме ниже показаны ключевые объекты и их связи.
...
* `--executor-memory` Задает объем памяти для каждого исполнителя, который управляет размером кучи в YARN. Необходимо предусмотреть лимит переполнения памяти при выполнения заданий.
...
* [Использование Apache Spark REST API для отправки удаленных заданий в кластер Apache Spark](apache-spark-livy-rest-interface.md)
* [Помощник по настройке Apache Spark](https://spark.apache.org/docs/latest/tuning.html)
* [Как фактически настраивать приложение Apache Spark заданий таким образом они работают](https://www.slideshare.net/ilganeli/how-to-actually-tune-your-spark-jobs-so-they-work)
* [Сериализация Kryo](https://github.com/EsotericSoftware/kryo)
