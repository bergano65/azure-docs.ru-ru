...
1. На [портале Azure](https://portal.azure.com/)выберите кластер Spark.  Инструкции см. в разделе [Отображение кластеров](../hdinsight-administer-use-portal-linux.md#showClusters). Открывается представление **"Обзор".**

2. Из представления **Обзор,** в поле **панели мониторинга кластера,** выберите **ноутбук Jupyter**. При появлении запроса введите учетные данные администратора для кластера.

    ![Jupyter ноутбук на Apache Spark](./media/apache-spark-jupyter-notebook-kernels/hdinsight-spark-open-jupyter-interactive-spark-sql-query.png "Jupyter ноутбук на Spark")
...
3. Выберите **новый**, а затем выберите либо **Pyspark**, **PySpark3**, или **Spark** для создания ноутбука. Для приложений Scala используйте ядро Spark, для приложений Python2 — ядро PySpark, а для приложений Python3 — ядро PySpark3.

    ![Ядра для ноутбука Jupyter на Spark](./media/apache-spark-jupyter-notebook-kernels/kernel-jupyter-notebook-on-spark.png "Ядра для ноутбука Jupyter на Spark")
...
- **Предустановленные контексты**. С **PySpark,** **PySpark3**или ядрами **Spark** вам не нужно четко устанавливать контексты Spark или Hive, прежде чем начать работать с приложениями. Они доступны по умолчанию. а именно:

  - **sc** для контекста Spark;
  - **sqlContext** для контекста Hive.

    Таким образом, для настройки контекстов **не** нужно запускать такие инструкции:
...
- **Волшебные команды.** Ядро PySpark предоставляет некоторые предопределенные "магии", которые являются специальными командами, которые вы можете вызвать с `%%` (например, `%%MAGIC` `<args>`). Волшебная команда должна быть первым словом в ячейке кода и может состоять из нескольких строк содержимого. Волшебное слово должно быть первым словом в ячейке. Любые другие слова перед магической командой, даже комментарии, приведут к ошибке.     Дополнительные сведения о волшебных командах см. [здесь](https://ipython.readthedocs.org/en/stable/interactive/magics.html).
   ...
   | local |`%%local`<br>`a=1` |Весь код в последующих строках выполняется локально. Код должен быть действительным кодом Python2, даже независимо от ядра, который вы используете. Таким образом, даже если вы выбрали **ядра PySpark3** или **Spark** `%%local` при создании ноутбука, если вы используете магию в ячейке, эта ячейка должна иметь только действительный код Python2. |
   | журналы |`%%logs` |Выводит журналы для текущего сеанса Livy. |
   | удалить |`%%delete -f -s <session number>` |Удаляет указанный сеанс для текущей конечной точки Livy. Нельзя удалять сеанс, инициированный для самого ядра. |
...
| -Q |`-q` |Позволяет отключить визуализации для ячейки. Если вы не хотите, чтобы автоматизировать содержимое ячейки и просто хотите, `-q -o <VARIABLE>`чтобы захватить его в качестве кадра данных, а затем использовать . Если вы хотите отключить визуализацию, не записывая результаты (например, для выполнения запроса SQL, такого как инструкция `CREATE TABLE`), то используйте параметр `-q` без аргумента `-o`. |
...
| -n |`-n <MAXROWS>` |**MAXROWS** должно быть выражено целым числом. Число выходных строк для параметра **MAXROWS** ограничивается ядром. Если **MAXROWS** является отрицательным числом, таким как **-1,** то количество строк в наборе результатов не ограничено. |
...
- Потому что мы используем -q, он выключает автовизуализацию.
...
Какое бы ядро вы ни использовали, работающие объекты Notebook потребляют ресурсы кластера.  С этими ядрами, потому что контексты предустановлены, просто выход из ноутбуков не убивает контекст и, следовательно, ресурсы кластера продолжают использоваться. Хорошая практика заключается в использовании опции **«Закрыть и остановить»** из меню **«Файла»** ноутбука, когда вы закончите использовать блокнот, который убивает контекст, а затем выходит из ноутбука.
...
Новые ядра находятся в стадии развития и будут улучшаться со временем. Кроме того, это может означать, что по мере развития этих ядер API могут измениться. Мы будем признательны вам за любые отзывы о работе с новыми ядрами. Ваши комментарии помогут нам оформить финальную версию этих ядер. Вы можете оставить свои комментарии / отзывы в разделе **Обратная связь** в нижней части этой статьи.
