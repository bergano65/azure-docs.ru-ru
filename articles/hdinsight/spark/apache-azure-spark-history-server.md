...
+ Цвет фона каждой задачи соответствует тепловой карте.
   ...
   ![Тепловая схема на странице с графиком задания & приложения Spark.](./media/apache-azure-spark-history-server/sparkui-graph-heatmap.png)
    ...
    |Orange|Не удалось выполнить задачу, но это не повлияет на окончательный результат задания. Эти задачи имеют дубликаты или экземпляры повторных попыток, которые могут быть выполнены позже.|
    ...
    ![пропущенную задачу на странице граф заданий & приложения Spark.](./media/apache-azure-spark-history-server/sparkui-graph-color-skip.png)
...
Если вы хотите обновить исправление, используйте следующий сценарий, который обновит `spark-enhancement.jar*`.
...
Если у вас возникли какие-либо отзывы или возникли проблемы при использовании этого средства, отправьте сообщение по адресу ([hdivstool@microsoft.com](mailto:hdivstool@microsoft.com)).