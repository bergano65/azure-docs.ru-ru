...
![Данные, перетекающие из приложения Insights в хранилище капли, а затем Spark](./media/apache-spark-analyze-application-insight-logs/application-insights.png)
...
Выполните последующие действия в [настройке непрерывного экспорта,](../../azure-monitor/app/export-telemetry.md) чтобы настроить анализ приложений для экспорта телеметрической информации в blob Хранилища Azure.

## <a name="configure-hdinsight-to-access-the-data"></a>Настройка доступа к данным из HDInsight

Если вы создаете кластер HDInsight, добавьте учетную запись хранилища во время создания кластера.
...
1. Из веб-браузера `https://CLUSTERNAME.azurehdinsight.net/jupyter` перейдите туда, где clusterNAME — это название кластера.
...
5. Под первой ячейкой будет создана ячейка. Введите в новую ячейку следующие данные. `CONTAINER` Замените `STORAGEACCOUNT` и с именем учетной записи Хранилища Azure и названием контейнера blob, который содержит данные Application Insights.
   ...
   ```
    ...
    Возвращается путь wasbs — это расположение телеметрических данных Application Insights. Измените `hdfs dfs -ls` линию в ячейке, чтобы использовать возвращенный путь wasbs, а затем используйте **SHIFT-ENTER** для повторного запуска ячейки. На этот раз в результате выполнения должны отобразиться каталоги, содержащие данные телеметрии.
...
8. Чтобы зарегистрировать таблицу данных в качестве временной таблицы и выполнить запрос на основе этих данных, используйте следующую команду:
   ...
   ```
    ...
    Этот запрос возвращает информацию о городе для 20 лучших записей, где context.location.city не является недействительным.
...
5. Под первой ячейкой будет создана ячейка. Введите в новую ячейку следующие данные. `CONTAINER` Замените `STORAGEACCOUNT` и с именем учетной записи Хранилища Azure и названиеконтейнера капли, которое содержит журналы Application Insights.
