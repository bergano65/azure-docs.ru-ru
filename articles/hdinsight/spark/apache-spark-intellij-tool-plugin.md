...
В этой статье показано, как разрабатывать приложения Apache Spark на Azure HDInsight с помощью плагина **Azure Toolkit** для IntelliJ IDE. [Azure HDInsight](../hdinsight-overview.md) — это управляемый аналитический сервис с открытым исходным кодом в облаке. Услуга позволяет использовать такие платформы с открытым исходным кодом, как Hadoop, Apache Spark, Apache Hive и Apache Kafka.

Плагин **Azure Toolkit** можно использовать несколькими способами:

* Разработайте и отправьте приложение Scala Spark в кластер HDInsight Spark.
...
> * Отправить заявку в кластер Azure HDInsight

## <a name="prerequisites"></a>Предварительные требования

* Кластер Apache Spark в HDInsight. Инструкции см. в статье [Начало работы. Создание кластера Apache Spark в HDInsight на платформе Linux и выполнение интерактивных запросов с помощью SQL Spark](apache-spark-jupyter-spark-sql.md).

* [Комплект Oracle Java Development](https://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html).  В этой статье используется Java-версия 8.0.202.

* Среда IntelliJ IDEA. В этой статье используется [IntelliJ IDEA Сообщества ver.  2018.3.4](https://www.jetbrains.com/idea/download/).
...
Шаги по установке плагина Scala:

1. Откройте IntelliJ IDEA.

2. На экране приветствия перейдите на **настройку** > **плагинов,** чтобы открыть окно **Plugins.**

    ![IntelliJ IDEA позволяет scala плагин](./media/apache-spark-intellij-tool-plugin/enable-scala-plugin1.png)

3. Выберите **Install** (Установить) в области подключаемого модуля Scala в новом окне.  

    ![IntelliJ IDEA устанавливает плагин scala](./media/apache-spark-intellij-tool-plugin/install-scala-plugin.png)
...
4. Из списка выпадающих **инструментов Build** выберите один из следующих вариантов:
...
6. В окне **New Project** (Новый проект) укажите следующую информацию:  
    ...
    |Project&nbsp;location (Расположение проекта)| Введите местоположение, чтобы сохранить ваш проект.|
    |Project SDK (Пакет SDK проекта)| Это поле может быть пустым на ваше первое использование IDEA.  Выберите **New...** (Создать...) и перейдите к JDK.|
...
8. Проект Spark автоматически создает артефакт. Чтобы просмотреть артефакт, выполните указанные ниже действия.
   ...
   а. Из меню бар, перейдите к **файлу** > **структуры проекта ...**.
...
9. Добавьте исходный код приложения, выполнив следующие действия:
    ...
    а. От проекта, перейдите на **myApp** > **src** > **основной** > **scala**.  
...
Пользователь может войти [в подписку Azure](#sign-in-to-your-azure-subscription)или [связать кластер HDInsight.](#link-a-cluster) Используйте имя пользователя/пароль Ambari или доступ к домену, объединяемый для подключения к кластеру HDInsight.

### <a name="sign-in-to-your-azure-subscription"></a>Войдите в свою подписку Azure.

1. Из панели меню перейдите к **View** > **Tool Windows** > **Azure Explorer.**

   ![IntelliJ IDEA показывает лазурный исследователь](./media/apache-spark-intellij-tool-plugin/show-azure-explorer1.png)
...
3. В диалоговом окне **Вход в Azure** выберите **Имя пользователя устройства**, а затем — **Войти**.
    ...
    !['IntelliJ IDEA лазурный вход в систему устройства войти'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer2.png)

4. В диалоговом окне **Azure Device Login** (Вход в систему устройства Azure) щелкните **Copy&Open** (Копировать и открыть).

   !['IntelliJ IDEA лазурное устройство Войти'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer5.png)

5. В интерфейсе браузера вставьте код и нажмите кнопку **Далее**.

   !['Microsoft ввести диалог кода для ИРЧП'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer6.png)

6. Введите учетные данные Azure и закройте браузер.

   !['Microsoft ввести диалог электронной почты для ИРЧП'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer7.png)
...
1. От проекта, перейдите на **myApp** > **src** > **основной** > **scala** > **myApp**.  Щелкните правой кнопкой мыши папку **myApp** и выберите **Submit Spark Application** (Запустить приложение Spark). Этот параметр скорее всего будет расположен в нижней части списка.
...
2. В окне диалога **приложения Отправка spark** выберите **1. Искра на HDInsight**.
...
Мы также рекомендуем еще один способ отправки приложения Spark в кластер. Он заключается в задании параметров **конфигураций запуска и отладки** в интегрированной среде разработки. Смотрите [приложения Debug Apache Spark локально или удаленно в кластере HDInsight с Azure Toolkit для IntelliJ через SSH.](apache-spark-intellij-tool-debug-remotely-through-ssh.md)
...
Вы можете выполнять различные операции с помощью Azure Toolkit для IntelliJ.  Большая часть операций начинается с **Azure Explorer**.  Из панели меню перейдите к **View** > **Tool Windows** > **Azure Explorer.**

### <a name="access-the-job-view"></a>Доступ к представлению задания

1. От Azure Explorer перейдите на **HDInsight** > \<Ваш кластер> > **вакансий.**
...
5. Вы можете просмотреть uI истории Spark и uI YARN (на уровне приложения). Выберите ссылку в верхней части окна.
...
3. На панели мониторинга сервера журнала Spark вы сможете найти приложение, выполнение которого только что было завершено, по его имени. В приведенном выше коде имя приложения было указано с помощью `val conf = new SparkConf().setAppName("myApp")`. Ваше имя приложения Spark **myApp**.
...
Убедитесь, что вы удовлетворены WINUTILS. Предпосылок EXE.

1. Из меню бар, перейдите к **Run** > **Edit Конфигурации ...**.
...
3. Из основного окна **`Locally Run`** выберите вкладку.
...
5. От проекта, перейдите на **myApp** > **src** > **основной** > **scala** > **myApp**.  

6. Из меню бар, перейдите к **Tools** > **Spark Консоль** > **Run Spark местной консоли (Scala)**.
...
8. Консоль должна выглядеть примерно так, как показано ниже. В окне консоли введите `sc.appName` и нажмите клавиши CTRL+ВВОД.  Отобразится результат. Вы можете закончить локальную консоль, нажав красную кнопку.
...
3. Из основного окна **`Remotely Run in Cluster`** выберите вкладку.
...
6. Из меню бар, перейдите к **Tools** > **Spark Консоль** > **Run Spark Livy Интерактивная сессия консоли (Scala)**.

7. Консоль должна выглядеть примерно так, как показано ниже. В окне консоли введите `sc.appName` и нажмите клавиши CTRL+ВВОД.  Отобразится результат. Вы можете закончить локальную консоль, нажав красную кнопку.
...
### <a name="connect-to-your-hdinsight-esp-cluster-with-id-broker-hib"></a>Подключитесь к кластеру HDInsight ESP с помощью ID Broker (HIB)

Вы можете следовать обычным шагам, чтобы войти в подписку Azure, чтобы подключиться к кластеру HDInsight ESP с ID Broker (HIB). После входе в систему список кластеров можно увидеть в Azure Explorer. Указания см. в разделе [Подключение к кластеру HDInsight](#connect-to-your-hdinsight-cluster).

### <a name="run-a-spark-scala-application-on-an-hdinsight-esp-cluster-with-id-broker-hib"></a>Запуск приложения Spark Scala в кластере HDInsight ESP с ID Broker (HIB)

Вы можете следовать обычным шагам, чтобы отправить задание в кластер HDInsight ESP с ID Broker (HIB). Указания см. в разделе [Запуск приложения Spark Scala в кластере HDInsight Spark](#run-a-spark-scala-application-on-an-hdinsight-spark-cluster).

Мы загружаем необходимые файлы в папку с вашей учетной записью, и вы можете увидеть путь загрузки в файле конфигурации.

   ![путь отправки в конфигурации](./media/apache-spark-intellij-tool-plugin/upload-path-in-the-configuration.png)

### <a name="spark-console-on-an-hdinsight-esp-cluster-with-id-broker-hib"></a>Консоль Spark в кластере HDInsight ESP с ID Broker (HIB)

Вы можете запустить Местную консоль Spark (Scala) или запустить Консоль Spark Livy Interactive Session Console (Scala) в кластере HDInsight ESP с ID Broker (HIB). Дополнительные инструкции см. в разделе [Консоль Spark](#spark-console).
...
2. В **Azure Explorer** разверните **HDInsight**, чтобы просмотреть кластеры HDInsight в своей подписке. Кластеры с пометкой **Role:Reader**, имеют только разрешение роли только для чтения.
    ...
    !['IntelliJ Azure Explorer Роль:Reader'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer15.png)
...
2. Щелкните **Link this cluster** (Связать этот кластер), чтобы связать кластер.
    ...
    ![кластерный доступ к работе отказано в диалоге](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer9.png)
...
2. Выберите кластер, у которого есть разрешение роли только для чтения для **Spark clusters(Linux only)** (Кластеры Spark (только Linux)). Предупреждающее сообщение отображается. Вы можете нажать **ссылку этот кластер,** чтобы связать кластер.
...
* Для кластеров с разрешением роли только для чтения щелкните узел **Учетные записи хранения** и появится всплывающее окно **Storage Access Denied** (Доступ к хранилищу запрещен). Выберите **Открыть обозреватель службы хранилища Azure**, чтобы открыть Обозреватель службы хранилища.
   ...
   !['IntelliJ IDEA Доступ к хранению отказано'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer14.png)
...
* Для связанных кластеров щелкните узел **Учетные записи хранения** и появится всплывающее окно **Storage Access Denied** (Доступ к хранилищу запрещен). Выберите **Открыть обозреватель службы хранилища Azure**, чтобы открыть Обозреватель службы хранилища.
   ...
   !['IntelliJ IDEA Доступ к хранению отказано2'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer13.png)
...
1. Для существующего приложения Spark Scala, созданного с помощью `.iml` IntelliJ IDEA, откройте связанный файл.

2. На корневом уровне находится **элемент модуля,** такой как следующий текст:

        ```
        <module org.jetbrains.idea.maven.project.MavenProjectsManager.isMavenModule="true" type="JAVA_MODULE" version="4">
        ```

   Отспособите элемент `UniqueKey="HDInsightTool"` для добавления так, чтобы элемент **модуля** выглядел следующим текстом:
...
1. Выберите **Удалить**. Выберите **Да**.
...
В этой статье вы узнали, как использовать Azure Toolkit для плагина IntelliJ для разработки приложений Apache Spark, написанных в [Scala.](https://www.scala-lang.org/) Затем они были представлены кластеру HDInsight Spark непосредственно из интегрированной среды разработки IntelliJ (IDE). Теперь переходите к следующей статье, в которой объясняется, как перенести зарегистрированные в Apache Spark данные в средство бизнес-аналитики, например в Power BI.
