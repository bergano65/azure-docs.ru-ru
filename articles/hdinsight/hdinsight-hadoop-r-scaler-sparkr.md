...
Теперь мы готовы к созданию модели. Чтобы увидеть влияние данных по погоде на задержку времени прибытия, мы используем подпрограмму логистической регрессии ScaleR. С ее помощью мы смоделируем, зависит ли задержка прибытия более чем на 15 минут от погоды в аэропортах отправления и прибытия.
