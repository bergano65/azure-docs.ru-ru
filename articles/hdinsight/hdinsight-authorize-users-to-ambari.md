...
Пользователи Active Directory могут войти в кластерные узлы, используя свои учетные данные домена. Они также могут использовать свои учетные данные домена для проверки подлинности взаимодействия кластеров с другими утвержденными конечными точками, такими как [Hue,](https://gethue.com/)Ambari Views, ODBC, JDBC, PowerShell и REST AIS.
...
Чтобы открыть **страницу управления Ambari** в [пользовательском веб-интерфейсе Apache Ambari](hdinsight-hadoop-manage-ambari.md), перейдите по адресу `https://CLUSTERNAME.azurehdinsight.net`. Введите имя пользователя и пароль администратора кластера, определенные при создании кластера. Затем на панели мониторинга Ambari выберите **Manage Ambari** (Управление Ambari) в меню **admin** (Администрирование).

![Управление приборной панелью Apache Ambari](./media/hdinsight-authorize-users-to-ambari/manage-apache-ambari.png)

## <a name="add-users"></a>Добавление пользователей

### <a name="add-users-through-the-portal"></a>Добавление пользователей через портал

1. На странице управления выберите **пользователей.**
...
1. Выберите **и создайте локального пользователя**.

1. Укажите **имя пользователя** и **пароль**. Выберите **сохранить**.

### <a name="add-users-through-powershell"></a>Добавление пользователей через PowerShell

Отодеть переменные ниже, заменив, `CLUSTERNAME` `NEWUSER`и `PASSWORD` с соответствующими значениями.
...
### <a name="add-users-through-curl"></a>Добавить пользователей через Curl

Отобразите переменные ниже, `ADMINPASSWORD` `NEWUSER`заменив, `USERPASSWORD` `CLUSTERNAME`и с соответствующими значениями. Скрипт предназначен для выполнения с помощью bash. Для запроса команды Windows потребуются небольшие изменения.
...
1. На странице управления щелкните ссылку **Views** (Представления) под заголовком меню **Views** (Представления) в левой части экрана.
    ...
    ![Apache Ambari просматривает ссылки](./media/hdinsight-authorize-users-to-ambari/apache-ambari-views-link.png)
...
2. На странице "Views" (Представления) разверните строку **HIVE**. При добавлении службы Hive в кластер создается представление по умолчанию Hive. При необходимости можно также создать дополнительные экземпляры представлений Hive. Выберите представление Hive.
    ...
    ![HDInsight Просмотров - Apache Hive зрения](./media/hdinsight-authorize-users-to-ambari/views-apache-hive-view.png)
...
1. Чтобы добавить пользователя, нажмите кнопку **Add User** (Добавить пользователя).
   ...
   * Начните вводить имя пользователя, и вы увидите раскрывающийся список ранее определенных имен.
     ...
     ![Apache Ambari пользователь авто завершает](./media/hdinsight-authorize-users-to-ambari/ambari-user-autocomplete.png)
   ...
   * Чтобы сохранить изменения, щелкните **синий флажок**.
     ...
     ![Apache Ambari предоставляет разрешения пользователей](./media/hdinsight-authorize-users-to-ambari/user-entered-permissions.png)
...
1. Чтобы добавить группу, нажмите кнопку **Add Group** (Добавить группу).
   ...
   * Чтобы сохранить изменения, щелкните **синий флажок**.
     ...
     ![Apache Ambari дает разрешения](./media/hdinsight-authorize-users-to-ambari/ambari-group-entered.png)
...
![HDInsight Просмотров - Apache Tez зрения](./media/hdinsight-authorize-users-to-ambari/views-apache-tez-view.png)
...
![Apache Ambari роли меню ссылки](./media/hdinsight-authorize-users-to-ambari/cluster-roles-menu-link.png)
...
![Разрешения на ссылку ролей Apache Ambari](./media/hdinsight-authorize-users-to-ambari/roles-menu-permissions.png "Разрешения на ссылку ролей Apache Ambari")
...
![Роли Apache Ambari блокируют представление](./media/hdinsight-authorize-users-to-ambari/ambari-roles-block-view.png)
...
* В категории "Users" (Пользователи) представления "List" (Список) отображается список всех пользователей, в котором из раскрывающегося списка можно выбрать роль для каждого пользователя.
    ...
    ![Представление ролей Apache Ambari - пользователи](./media/hdinsight-authorize-users-to-ambari/roles-list-view-users.png)
...
* Категория "Groups" (Группы) представления "List" (Список) содержит все группы, а также роли, назначенные каждой группе. В нашем примере список групп синхронизирован с группами Azure AD, указанными в свойстве **Access user group** (Группа доступа пользователей) параметров домена кластера. См. [Создание кластера HDInsight с корпоративным пакетом безопасности](./domain-joined/apache-domain-joined-configure-using-azure-adds.md#create-an-hdinsight-cluster-with-esp).
    ...
    ![Представление ролей Apache Ambari - группы](./media/hdinsight-authorize-users-to-ambari/roles-list-view-groups.png)
...
![Пользователь Apache Ambari с только видами](./media/hdinsight-authorize-users-to-ambari/ambari-user-views-only.png)
...
![Дисплей приборной панели Apache Ambari](./media/hdinsight-authorize-users-to-ambari/user-cluster-user-role.png)
