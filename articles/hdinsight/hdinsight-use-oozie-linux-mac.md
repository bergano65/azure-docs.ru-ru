...
* **Кластер Hadoop на HDInsight**. Ознакомьтесь со статьей [Краткое руководство. Использование Apache Hadoop и Apache Hive в Azure HDInsight с шаблоном Resource Manager](hadoop/apache-hadoop-linux-tutorial-get-started.md).

* **Клиент SSH**. Смотрите [Подключитесь к HDInsight (Apache Hadoop) с помощью SSH](hdinsight-hadoop-linux-use-ssh-unix.md).

* **База данных Azure S'L**.  На [портале Azure см.](../sql-database/sql-database-get-started.md)  В этой статье используется база данных под названием **oozietest**.
...
![Диаграмма рабочего процесса HDInsight oozie](./media/hdinsight-use-oozie-linux-mac/oozie-workflow-diagram.png)

1. Действие Hive запускает скрипт Hive-L для `hivesampletable` извлечения записей из записей, включенных в HDInsight. Каждая строка данных описывает посещение с определенного мобильного устройства. Формат записи таков:
...
2. Действие Sqoop экспортирует содержимое новой таблицы Hive в таблицу, созданную в базе данных SQL Azure. Дополнительные сведения о Sqoop см. в статье, [посвященной использованию Apache Sqoop в HDInsight][hdinsight-use-sqoop].
...
1. Отспособите ниже `sshuser` код, чтобы заменить имя пользователя `CLUSTERNAME` SSH для кластера, и заменить имя кластера.  Затем введите код для подключения к кластеру HDInsight [с помощью SSH.](hdinsight-hadoop-linux-use-ssh-unix.md)  
...
2. Чтобы создать каталог, выполните следующую команду:
    ...
    > [!NOTE]  
    > Параметр `-p` означает, что будут созданы все каталоги для пути. Каталог `data` используется для хранения данных, `useooziewf.hql` используемых скриптом.

3. Отспособьте ниже код, чтобы заменить `sshuser` имя пользователя SSH.  Выполните следующую команду, чтобы убедиться, что Oozie может олицетворять учетную запись пользователя:
...
> [!IMPORTANT]  
> Проверить фактический драйвер JDBC, который существует в `/usr/share/java/`.
...
Для создания сценария языка запросов Hive (HiveQL), определяющего запрос, сделайте следующее. Запрос будет использоваться в рабочем процессе Oozie позже в этом документе.
...
1. Открыв редактор GNU nano, используйте следующий запрос в качестве содержимого файла:
   ...
   * `${hiveDataFolder}`: содержит расположения файлов данных для таблицы.
     ...
     Файл определения рабочего процесса, workflow.xml в этой статье, передает эти значения в этот скрипт Hive-L во время выполнения.

1. Чтобы сохранить файл, выберите **Ctrl'X,** введите **Y,** а затем выберите **Enter**.  

1. Используйте следующую `useooziewf.hql` команду `wasbs:///tutorials/useoozie/useooziewf.hql`для копирования:
...
2. Открыв редактор nano, введите следующий код XML в качестве содержимого файла:
   ...
   * `RunSqoopExport`. Это действие экспортирует созданные данные из сценария Hive в базу данных SQL, используя Sqoop. Это действие будет выполнено только после успешного завершения действия `RunHiveScript`.
     ...
     В рабочем процессе есть несколько записей, таких как `${jobTracker}`. Вы замените эти записи значениями, которые вы используете в определении задания. Определение задания будет создано позднее в этом документе.
...
3. Чтобы сохранить файл, выберите **Ctrl'X,** введите **Y,** а затем выберите **Enter**.  
...
2. Отспособьте ниже код, чтобы заменить `<serverName>` имя сервера Azure S'L, а `<sqlLogin>` также логин сервера Azure S'L.  Введите команду для подключения к предварительной базе данных S'L.  Введите пароль в запросе.
...
3. В командной строке `1>` введите следующее:
    ...
    Если вводится инструкция `GO`, то оцениваются предыдущие инструкции. Эти операторы создают `mobiledata`таблицу с именем , которая используется рабочим процессом.
...
4. Выйдите из утилиты `exit` tsql, введя в подсказку. `1>`
...
2. Оторите xml ниже следующим образом:

    |Значение заполнителя| Замененное значение|
    |---|---|
    |wasbs://mycontainer\@mystorageaccount.blob.core.windows.net| Значение, полученное от шага 1.|
    |admin| Ваше имя входа для кластера HDInsight, если не админ.|
    |serverName| Имя сервера базы данных Azure S'L.|
    |sqlLogin| Логин сервера базы данных Azure S'L.|
    |sqlPassword| Пароль для входа в базу данных сервера Лазурного налажи.|
    ...
    Большая часть информации в этом файле используется для заполнения значений, используемых в файлах workflow.xml или ooziewf.hql, таких как `${nameNode}`.  Если используется путь `wasbs`, его необходимо указать полностью. Не сокращайте его `wasbs:///`до просто . Запись `oozie.wf.application.path` определяет, где можно найти файл workflow.xml. Этот файл содержит рабочий процесс, запущенный этим заданием.
...
4. После открытия наноредактора вставьте отредактированный XML в качестве содержимого файла.

5. Чтобы сохранить файл, выберите **Ctrl'X,** введите **Y,** а затем выберите **Enter**.
...
2. Отспособите код, чтобы заменить URL-адрес, с тем, который вы получили ранее. Используйте следующую команду для создания переменной среды для URL-адреса, чтобы не вводить его в каждой команде:
...
3. Чтобы отправить задание, воспользуйтесь следующей командой:
    ...
    Эта команда загружает `job.xml` информацию о заданиях и отправляет ее Oozie, но не заражает ее.
...
4. Отспособите код `<JOBID>` ниже, чтобы заменить идентификатор, возвращенный на предыдущем этапе.  Чтобы просмотреть состояние задания, используйте следующую команду:
...
5. Отспособите код `<JOBID>` ниже, чтобы заменить идентификатор, возвращенный ранее.  Для запуска задания выполните следующую команду:
...
6. Отспособьте ниже код, чтобы заменить `<serverName>` имя сервера Azure S'L, а `<sqlLogin>` также логин сервера Azure S'L.  *После успешного завершения задачи* можно проверить, что данные были сгенерированы и экспортированы в таблицу базы данных S'L, используя следующую команду.  Введите пароль в запросе.
...
2. После создания туннеля откройте веб-интерфейс Ambari в `http://headnodehost:8080`веб-браузере с помощью URI.

3. С левой стороны страницы, выберите **Oozie** > **Быстрый Ссылки** > **Oozie веб-интерфейс.**

    ![Apache Ambari oozie веб-ui шаги](./media/hdinsight-use-oozie-linux-mac/hdi-oozie-web-ui-steps.png)
...
4. По умолчанию в веб-интерфейсе Oozie отображаются запущенные задания рабочих процессов. Чтобы просмотреть все задания рабочего процесса, выберите **All Jobs** (Все задания).
    ...
    ![Задания рабочего процесса веб-консолей Oozie](./media/hdinsight-use-oozie-linux-mac/hdinsight-oozie-jobs.png)

5. Чтобы просмотреть дополнительные сведения о задании, выберите это задание.

    ![HDInsight Apache Oozie информация о работе](./media/hdinsight-use-oozie-linux-mac/hdinsight-oozie-job-info.png)
...
6. На вкладке **Job Info** (Сведения о задании) можно просмотреть базовую информацию о задании, а также отдельные действия в рамках задания. С помощью вкладок вверху можно просмотреть **определение задания**, **конфигурацию задания**, обратиться к **журналу задания** или просмотреть направленный ациклический граф (DAG) задания в разделе **Job DAG** (Направленный ациклический граф задания).
   ...
   * **Job Log** (Журнал задания). Нажмите кнопку **Get Logs** (Получить журналы) для просмотра всех журналов задания или воспользуйтесь полем **Enter Search Filter** (Введите фильтр поиска) для выбора журналов с помощью фильтра.
       ...
       ![HDInsight Apache Oozie журнал работы](./media/hdinsight-use-oozie-linux-mac/hdinsight-oozie-job-log.png)
   ...
   * **Job DAG** (Направленный ациклический граф задания). DAG представляет собой графическое отображение путей данных рабочего процесса.
       ...
       ![HDInsight Apache Oozie работу даг](./media/hdinsight-use-oozie-linux-mac/hdinsight-oozie-job-dag.png)
...
7. Выбрав одно из действий на вкладке **Job Info** (Сведения о задании), вы увидите информацию об этом действии. Например, выберите действие **RunSqoopExport**.
    ...
    ![HDInsight oozie информация о работе действий](./media/hdinsight-use-oozie-linux-mac/oozie-job-action-info.png)
...
1. Выполните следующую команду для создания файла с именем **coordinator.xml**:
    ...
    > * `${coordTimezone}`: задания координатора задаются для конкретного часового пояса без летнего времени (обычно представляется в виде времени в формате UTC). Этот часовой пояс называется *часовой пояс oozie.*
    > * `${wfPath}`: путь к файлу workflow.xml.

2. Чтобы сохранить файл, выберите **Ctrl'X,** введите **Y,** а затем выберите **Enter**.
...
4. Чтобы изменить `job.xml` созданный ранее файл, используйте следующую команду:
   ...
   * Чтобы определить начало, окончание и частоту выполнения для координатора, добавьте следующий код XML:
       ...
       Эти значения установили время начала до 12:00 10 мая 2018 года, а время окончания - 12 мая 2018 года. Интервал запуска этого задания — "ежедневно". Интервал задается в минутах, то есть 24 часа x 60 минут = 1440 минут. И наконец, часовой пояс устанавливается в формате UTC.

5. Чтобы сохранить файл, выберите **Ctrl'X,** введите **Y,** а затем выберите **Enter**.

6. Чтобы отправить и начать задание, используйте следующую команду:
...
7. Если вы зайдете в веб-интерфейс Oozie и выберете вкладку **Coordinator Jobs** (Задания координатора), то увидите похожую информацию:
    ...
    ![Вкладка задания координатора веб-консолей Oozie](./media/hdinsight-use-oozie-linux-mac/coordinator-jobs-tab.png)
...
8. Как и для предыдущих заданий рабочего процесса, если вы выберете это задание в веб-интерфейсе, то увидите информацию о нем:
    ...
    ![Apache Oozie координатор работу информация](./media/hdinsight-use-oozie-linux-mac/coordinator-job-info.png)
    ...
    ![OOzie веб-консоли работу информационной вкладке](./media/hdinsight-use-oozie-linux-mac/coordinator-action-job.png)
...
С помощью пользовательского интерфейса Oozie можно просмотреть журналы Oozie. Пользовательский интерфейс Oozie также содержит ссылки на журналы JobTracker для заданий MapReduce, запущенных рабочим процессом. Действия по решению проблемы должны подчиняться следующему шаблону:
   ...
   2. Если есть ошибка или сбой для конкретного действия, выберите действие, чтобы увидеть, если поле **Сообщения об ошибке** предоставляет дополнительную информацию о сбое.
...
### <a name="ja002-oozie-is-not-allowed-to-impersonate-ltusergt"></a>JA002: Oozie не имеет права &lt;выдавать себя за USER&gt;
...
**Решение.** Oozie может работать от имени пользователей из группы **users**. Для просмотра групп, в которые входит данный пользователь, воспользуйтесь командой `groups USERNAME` . Если пользователь не является членом группы **пользователей,** используйте следующую команду, чтобы добавить пользователя в группу:
...
1. Копирование `mssql-jdbc-7.0.0.jre8.jar` файла в **каталог /tutorials/useoozie:**
...
В этой статье вы узнали, как определить рабочий процесс Oozie и как запустить задание Oozie. Дополнительные сведения о работе с HDInsight приведены в следующих статьях:
