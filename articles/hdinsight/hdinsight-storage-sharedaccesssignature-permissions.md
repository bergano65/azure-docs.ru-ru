...
* Кластер HDInsight под управлением Linux ИЛИ [Azure PowerShell][powershell]. Если у вас есть кластер под управлением Linux, можно добавить подписанный URL-адрес в кластер с помощью Apache Ambari. Если нет, создайте кластер с помощью Azure PowerShell и добавьте подписанный URL-адрес в процессе создания кластера.
...
1. Откройте файл SASToken.py и измените следующие значения:
   ...
   * storage\_account\_name. Имя учетной записи хранения.
   ...
   * example\_file\_path. Путь к файлу, который будет отправляться в контейнер.
...
4. Разверните раздел **Custom core-site** (Настройка файла core-site), прокрутите экран до конца и щелкните ссылку **Add property...** (Добавить свойство...). В полях **Key** (Ключ) и **Value** (Значение) укажите следующие значения.
   ...
   * **Value** (Значение): SAS, выданный выполненным ранее приложением C# или Python.
