...
| Причина: | Разрешение |
| --- | --- |
| Превышено максимальное число одновременных запросов, обслуживаемых WebHCat в минуту (по умолчанию 20) |Сократите рабочую нагрузку, чтобы не отправлять больше, чем максимальное количество параллельных запросов, или увеличьте ограничение параллельных запросов, изменив `templeton.exec.max-procs`. Для получения дополнительных сведений ознакомьтесь с разделом [Изменение конфигурации](#modifying-configuration). |
...
| Выполнена попытка получить все задания вызовом [GET /Jobs](https://cwiki.apache.org/confluence/display/Hive/WebHCat+Reference+Jobs) при `Fields` со значением `*`. |Не *извлекать сведения* о задании. Вместо этого `jobid` используйте для получения сведений только о заданиях, превышающих определенный идентификатор задания. Или не используйте`Fields` |
