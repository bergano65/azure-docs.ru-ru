...
* Для кластеров **Windows**: используйте сценарий действия, чтобы настроить значение во время создания кластера. Для получения дополнительных сведений обратитесь к разделу [Разработка сценариев Action Script](hdinsight-hadoop-script-actions-linux.md).

* Для кластеров **Linux**: используйте Ambari (онлайн-версию или API-Интерфейс REST) для изменения значения. Дополнительные сведения см. в разделе [Управление кластерами HDInsight с помощью веб-интерфейса Ambari](hdinsight-hadoop-manage-ambari.md).
...
| Выполнена попытка получить все задания вызовом [GET /Jobs](https://cwiki.apache.org/confluence/display/Hive/WebHCat+Reference+Jobs) при `Fields` со значением `*`. |Не пытайтесь получить сведения о *всех* заданиях. Вместо этого используйте `jobid` получить сведения для задания только больше, чем идентификатор определенного задания. Или не используйте `Fields` |
