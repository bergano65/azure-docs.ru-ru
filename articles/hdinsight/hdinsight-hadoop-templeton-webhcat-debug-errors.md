...
| Причина | Решение |
| --- | --- |
| Вы превысили максимальные одновременные запросы, подаваемые WebHCat в минуту (по умолчанию 20) |Уменьшите рабочую нагрузку, чтобы убедиться, что вы не отправляете больше, чем `templeton.exec.max-procs`максимальное количество одновременных запросов или увеличиваете лимит одновременных запросов путем изменения. Для получения дополнительных сведений ознакомьтесь с разделом [Изменение конфигурации](#modifying-configuration). |
...
| Использовался идентификатор недействительной работы |Проверьте, является ли идентификатор задания правильным |
...
| Выполнена попытка получить все задания вызовом [GET /Jobs](https://cwiki.apache.org/confluence/display/Hive/WebHCat+Reference+Jobs) при `Fields` со значением `*`. |Не извлекайте *все* детали работы. Вместо `jobid` этого используйте для получения деталей для заданий только больше, чем определенный идентификатор задания. Или не используйте`Fields` |
