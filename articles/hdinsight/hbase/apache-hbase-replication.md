...
В этой статье вы настраиваете репликацию исходного пункта назначения. Другие топологии кластеров см. в [справочном руководстве по Apache HBase](https://hbase.apache.org/book.html#_cluster_replication).
...
Прежде чем начать эту статью, необходимо иметь подписку Azure. См. страницу о [получении бесплатной пробной версии Azure](https://azure.microsoft.com/documentation/videos/get-azure-free-trial-for-testing-hadoop-in-hdinsight/).
...
3. Нанастройка Bind для переадресовывания запросов на разрешение имен на ваш сервер DNS. Чтобы сделать это, в качестве содержимого файла `/etc/bind/named.conf.options` добавьте следующий текст:
...
> [!NOTE]
> Если вы хотите воспроизвести таблицы из пользовательского пространства имен, необходимо убедиться, что соответствующие пользовательские пространства имен также определены в кластере назначения.
>
...
5. Выберите или введите следующие сведения.
   ...
   4. **Параметры**: Параметры в следующем примере позволяют включить репликацию для всех существующих таблиц, а затем копировать все данные из исходного кластера в целевой.
      ...
      >
      > Это пошаговое поство предполагает hn1 как активный headnode. Пожалуйста, проверьте свой кластер, чтобы определить активный головной узлы.
...
|-m, --machine | Указывает головной узел, на котором будет выполняться действие сценария. Значение должно быть выбрано на основе которого является активным головным узлам. Используйте этот параметр при запуске скрипта $0 как действия сценария из портала HDInsight или Azure PowerShell.|
...
- **Включить репликацию в определенных таблицах и скопировать существующие данные.** Чтобы включить репликацию для table1, table2 и table3, используйте следующие параметры:
...
> [!NOTE]
> Если вы намереваетесь удалить кластер назначения, убедитесь, что вы удалите его из списка одноранговых узлов исходного кластера. Это можно сделать, запустив команду remove_peer '1' на оболочке hbase на исходном кластере. В противном случае исходный кластер может не функционировать должным образом.
>
