...
> * **Виртуальное название сети**: CLUSTERNAME-vnet
> * **Виртуальное пространство адреса сети**: 10.0.0.0/16
> * **Название подсети**: subnet1
> * **Диапазон адресов подсети**: 10.0.0.0/24
>
> `CLUSTERNAME`заменяется названием кластера, которое вы предоставляете при использовании шаблона.
...
1. Из диалога **custom deployment** выберите **шаблон Edit.**

1. На строке 165 `Standard_A3` `Standard_A4_V2`измените значение на . Затем выберите **Сохранить**.

1. Заполните оставшийся шаблон со следующей информацией:
    ...
    |Кластер Войти Имя пользователя и пароль|Имя пользователя по умолчанию — **это админ.** Предоставьте пароль.|
    |Имя пользователя ssh и пароль|Имя пользователя по умолчанию **sshuser**.  Предоставьте пароль.|

    Выберите **Я согласен с условиями и условиями, указанными выше**.

1. Нажмите кнопку **Купить**. Процесс создания кластера занимает около 20 минут. После создания кластера можно выбрать кластер в портале, чтобы открыть его.
...
* **Виртуальная сеть**: CLUSTERNAME-vnet
* **Субнет**: поднет1

> [!IMPORTANT]  
> Замените `CLUSTERNAME` имя, используемое при создании кластера HDInsight в предыдущих шагах.
...
### <a name="obtain-fully-qualified-domain-name"></a>Получить полностью квалифицированное доменное имя
...
* С помощью веб-браузера вызовите [Apache Ambari](https://ambari.apache.org/).
    ...
    Перейдите по адресу `https://CLUSTERNAME.azurehdinsight.net/api/v1/clusters/CLUSTERNAME/hosts?minimal_response=true`. Он возвращает файл JSON с суффиксами DNS.

* Используйте веб-сайт Ambari.

    1. Перейдите по адресу `https://CLUSTERNAME.azurehdinsight.net`.
    2. Выберите **Хостов** из верхнего меню.
...
### <a name="verify-communication-inside-virtual-network"></a>Проверка связи внутри виртуальной сети
...
* [Виртуальный обзор сети](../../virtual-network/virtual-networks-overview.md)
