...
- Старайтесь не `memstore` блокировать очистку из-за ограничения хранилища. Чтобы предоставить этот буфер, увеличьте `Hbase.hstore.blockingStoreFiles` значение параметра равным **100**.
...
- Подключитесь [@AzureSupport](https://twitter.com/azuresupport)к. Это официальная учетная запись Microsoft Azure для улучшения качества взаимодействия с клиентами. Он подключает сообщество Azure к нужным ресурсам: ответы, поддержка и эксперты.
