...
Используйте [учетную запись хранилища блочных BLOB-объектов категории "Премиум"](https://azure.microsoft.com/blog/azure-premium-block-blob-storage-is-now-generally-available/) в качестве удаленного хранилища, чтобы значительно повысить эффективность операций чтения. Это возможно, только если включена функция журналов упреждающего протоколирования.
...
1. Увеличьте размер `memstore` с 128 по умолчанию на 256 МБ — этот параметр обычно рекомендуется для интенсивного сценария записи.
...
1. Старайтесь не блокировать `memstore`, так как это ограничение хранилища. `Hbase.hstore.blockingStoreFiles` можно увеличить до 100, чтобы предоставить этот буфер.

1. Для контроля очистки значения по умолчанию можно устранить следующим образом:

    1. `Hbase.regionserver.maxlogs` можно увеличила до 140 из 32 (избегая очистки из-за ограничений WAL).
...
1. Другие конфигурации, относящиеся к Phoenix:
    ...
    1. `Phoenix.rpc.index.handler.count` можно задать равным 50 при наличии большого или большого количества уточняющих запросов индекса.

    1. `Phoenix.stats.updateFrequency` — может быть увеличила до 1 часа со времени по умолчанию (15 минут).
...
1. Истечение времени ожидания RPC — время ожидания RPC для HBase, время ожидания для клиентского сканера HBase и время ожидания запроса в Финиксе могут быть увеличены до 3 минут. Важно отметить, что для параметра `hbase.client.scanner.caching` задано значение, совпадающее со стороны сервера и клиентской стороны. В противном случае этот параметр приводит к ошибкам, связанным с `OutOfOrderScannerException` на стороне клиента. Для этого параметра следует задать низкое значение для больших просмотров. Это значение устанавливается равным 100.
