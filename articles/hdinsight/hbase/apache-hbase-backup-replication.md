---
title: Резервное копирование & репликации для Apache HBase, Феникс - Azure HDInsight
...
* В хранилище озер данных `hbase` Azure находится под корневым путем, указанным при подготовке кластера. Обычно этот путь к корневому каталогу имеет папку `clusters` с вложенной папкой, имя которой совпадает с кластером HDInsight:
...
В исходном кластере HDInsight используйте [утилиту Экспорт](https://hbase.apache.org/book.html#export) (включаемый hBase) для экспорта данных из исходной таблицы в прикрепленное по умолчанию хранилище. Затем можно скопировать экспортируемую папку в место хранения назначения и запустить [утилиту Импорта](https://hbase.apache.org/book.html#import) в кластере назначения HDInsight.

Для экспорта данных таблицы сначала SSH в головной узлы `hbase` вашего кластера HDInsight источника, а затем запустите следующую команду:

    hbase org.apache.hadoop.hbase.mapreduce.Export "<tableName>" "/<path>/<to>/<export>"

Каталог экспорта уже не должен существовать. Название таблицы является чувствительным к делу.

Чтобы импортировать данные таблицы, SSH в головной узлы `hbase` кластера HDInsight назначения, а затем запустите следующую команду:
...
[Утилита CopyTable](https://hbase.apache.org/book.html#copy.table) копирует данные из исходной таблицы, строки за строкой, в существующую таблицу назначения с той же схемой, что и источник. Целевая таблица может находиться в том же или другом кластере HBase. Имена таблиц чувствительны к случаям.
...
Когда оба кластера HDInsight находятся в той же виртуальной сети, как описано выше, разрешение имен внутренних узлов выполняется автоматически. Чтобы использовать кластеры CopyTable для HDInsight в двух отдельных виртуальных сетях, соединенных VPN Gateway, необходимо предоставить IP-адреса узлов зоозащитника в кворуме.
...
[Снимки](https://hbase.apache.org/book.html#ops.snapshots) позволяют осуществлять резервное копирование данных в хранилище данных HBase. Они имеют минимальные издержки и создаются в течение нескольких секунд. Во время этой операции, которая, по сути, является операцией с метаданными, записываются имена всех файлов, сохраненных на этот момент в хранилище. При создании моментального снимка фактические данные не копируются. Моментальные снимки зависят от постоянных данных, хранящихся в HDFS, где операции обновления, удаления и вставки представляют собой новые данные. Вы можете восстановить (*клонировать*) моментальный снимок в том же кластере или экспортировать его в другой кластер.
...
Моментальные снимки предоставляют полную резервную копию таблицы на момент выполнения команды `snapshot`. Снимки не предоставляют возможность выполнять дополнительные снимки окнами времени, а также не указывают подмножества семейств столбцов для включения в моментальный снимок.

## <a name="replication"></a>Репликация

[Репликация HBase](https://hbase.apache.org/book.html#_cluster_replication) автоматически толкает транзакции из исходного кластера в кластер назначения, используя асинхронный механизм с минимальными накладными затратами на исходный кластер. В HDInsight репликацию можно настроить между кластерами, где:
...
* [Работа с импортно-экспортным предприятием HBase](https://blogs.msdn.microsoft.com/data_otaku/2016/12/21/working-with-the-hbase-import-and-export-utility/)
