...
Не удается завершить операцию перезапуска на сервере региона Apache HBase. В `region-server.log` в каталоге `/var/log/hbase` на рабочих узлах, где происходит сбой запуска сервера Region, может появиться сообщение об ошибке, похожее на следующее:
...
1. Из-за этого внезапного завершения работы, хотя процесс сервера региона завершается, порт, связанный с процессом, может не освободиться, что в конечном итоге приводит к `AddressBindException`у.
