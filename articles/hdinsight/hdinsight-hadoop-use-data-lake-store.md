...
Для анализа данных в кластере HDInsight можно хранить данные либо в [Azure Storage,](../storage/common/storage-introduction.md) [Azure Data Lake Storage Gen 1,](../data-lake-store/data-lake-store-overview.md)либо в [Azure Data Lake Storage Gen 2.](../storage/blobs/data-lake-storage-introduction.md) Все варианты хранилищ позволяют безопасно и без потери пользовательских данных удалять используемые для расчетов кластеры HDInsight.
...
|------------------------|------------------------------------|---------------------------------------|------|
| HDInsight версия 4.0 | нет | нет |ADLS Gen1 не поддерживается HDInsight 4.0 |
...
Использование Data Lake Storage Gen1 в качестве дополнительной учетной записи хранилища не влияет на производительность или возможность чтения или записи в хранилище Azure из кластера.
...
Если вы хотите настроить Azure Data Lake Storage 1 в качестве хранилища по умолчанию для нового кластера, а сертификат для основного обслуживания хранится в Хранилище ключей Azure, необходимо провести несколько дополнительных шагов для преобразования сертификата в правильный формат. В приведенном ниже фрагменте кода показано, как выполнить преобразование.
...
Data Lake Storage 1-го поколения можно использовать в качестве дополнительного хранилища кластера. В таких случаях хранилищем кластера по умолчанию может быть Azure Storage Blob или учетная запись Data Lake Storage. Если вы используете задания HDInsight против данных, хранящихся в хранилище Data Lake Storage в качестве дополнительного хранилища, необходимо использовать полностью квалифицированный путь к файлам. Пример:

    adl://mydatalakestore.azuredatalakestore.net/<file_path>

Обратите внимание, что в этом URL-адресе нет **cluster_root_path**. Это потому, что Data Lake Storage не является хранилищем по умолчанию в этом случае, так что все, что вам нужно сделать, это обеспечить путь к файлам.
...
Для получения дополнительной информации об основах модели управления доступом для Data Lake Storage Gen1 можно [ознакомиться на канале Azure Data Lake Storage Gen1.](../data-lake-store/data-lake-store-access-control.md)
...
Примеры основаны на [съеме Ssh](./hdinsight-hadoop-linux-use-ssh-unix.md) к головному узлам кластера. В примерах используются все три схемы URI. `DATALAKEACCOUNT` Заменить `CLUSTERNAME` и с соответствующими значениями.
...
Чтобы определить полный путь к настроенной магазину по умолчанию, перейдите на **HDFS** > **Configs** и введите `fs.defaultFS` в поле ввода фильтра.
