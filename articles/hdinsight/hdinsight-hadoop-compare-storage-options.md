...
|---|---|---|---|---|---|---|---|
|Azure Data Lake Storage 2-го поколения| Общего назначения версии 2 | Иерархическая (файловая система) | Большой двоичный объект | Стандартный | Горячий, холодное, архивная | 3.6+ | Все, кроме Spark 2,1 и 2,2|
|Хранилище Azure| Общего назначения версии 2 | Объект | Большой двоичный объект | Стандартный | Горячий, холодное, архивная | 3.6+ | Все |
|Хранилище Azure| Общего назначения версии 1 | Объект | Большой двоичный объект | Стандартный | Н/Д | Все | Все |
|Хранилище Azure| Хранилище BLOB-объектов * * | Объект | Блочный BLOB-объект | Стандартный | Горячий, холодное, архивная | Все | Все |
...
Вы можете создать кластер, используя разные сочетания служб для основного и дополнительного хранилища. В следующей таблице перечислены конфигурации хранилища кластера, которые в настоящее время поддерживаются в HDInsight.
...
| 3,6 & 4,0 | Общего назначения v1, общего назначения v2 | Data Lake Storage 1-го поколения | Да |
...
* **Экономичность:** Data Lake Storage 2-го поколения предлагает недорогие ресурсы хранения и транзакции. Такие функции, как жизненный цикл хранилища BLOB-объектов Azure, помогают снизить затраты, регулируя тарифные ставки при перемещении данных в течение жизненного цикла.
...
* **Открытые контейнеры или общедоступные большие двоичные объекты в учетных записях хранения, которые *не* подключены к кластеру:** У вас есть разрешение только на чтение для больших двоичных объектов в контейнерах.
...
* **Стоимость хранения данных:** Хранение данных в DFS в долгосрочной перспективе является более дорогостоящим, чем хранение данных в службе хранилища Azure, так как стоимость вычислений кластера выше, чем стоимость хранилища Azure. Кроме того, так как данные не требуется повторно загружать при создании каждого вычислительного кластера, вы экономите также на загрузке данных.
