...
* Они должны храниться в универсальном коде ресурса (URI), доступном из кластера HDInsight. Возможные места хранения:
    ...
    * Общедоступная служба общего доступа к файлам. Примеры: служба больших двоичных объектов Azure, GitHub, OneDrive и Dropbox.

        Примеры URI см. в разделе [Пример сценариев действий сценария](#example-script-action-scripts).

        > [!WARNING]  
        > HDInsight поддерживает большие двоичные объекты только в учетных записях службы хранилища Azure с уровнем производительности "Стандартный". 
