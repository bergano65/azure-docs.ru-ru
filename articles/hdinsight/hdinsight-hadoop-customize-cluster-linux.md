...
- Они должны храниться в универсальном коде ресурса (URI), доступном из кластера HDInsight. Возможные места хранения:
    ...
    - Для обычных кластеров (не ESP):
      - Data Lake Storage 1-го поколения/Gen2: служба HDInsight субъекта-службы использует для доступа к Data Lake Storage должен иметь доступ для чтения к скрипту. URI для скриптов, находящихся в Data Lake Storage 1-го поколения, имеет формат `adl://DATALAKESTOREACCOUNTNAME.azuredatalakestore.net/path_to_file`. Формат URI для Data Lake Storage 2-го поколения скриптов: `abfs://<FILE_SYSTEM_NAME>@<ACCOUNT_NAME>.dfs.core.windows.net/<PATH>`
      ...
      - Общедоступная служба общего доступа к файлам, доступная через `http://` пути. Примеры: BLOB-объект Azure, GitHub или OneDrive. Примеры URI см. в разделе [Пример сценариев действий сценария](#example-script-action-scripts).
    - Для кластеров с ESP `wasb://` `wasbs://` `http[s]://` поддерживаются URI или или.

- Действия можно ограничить выполнением только на определенных типах узлов, например головных или рабочих.
- Может быть сохраненным или *нерегламентированным*.
    ...
    - *Нерегламентированные* сценарии не сохраняются. Действия скриптов, используемые при создании кластера, автоматически сохраняются. Они не применяются на рабочих узлах, добавленных в кластер после выполнения сценария. Затем можно преобразовать *нерегламентированный* сценарий в сохраненный скрипт или понизить сохраненный скрипт до *нерегламентированного* сценария. Сценарии, в которых произошла ошибка, не сохраняются, даже если вы отдельно указали, что они должны быть сохранены.
...
Вы можете настроить действие сценария для запуска при первом создании кластера или запустить его в существующем кластере.
...
В этом разделе описывается использование командлета [Add-аздинсигхтскриптактион](/powershell/module/az.hdinsight/add-azhdinsightscriptaction) для вызова сценариев для настройки кластера. Перед началом работы убедитесь, что установили и настроили Azure PowerShell. Для использования этих команд PowerShell необходим [модуль AZ](/powershell/azure/).
...
В этом разделе объясняется, как применять действия сценария в работающем кластере.
...
Для использования этих команд PowerShell необходим [модуль AZ](/powershell/azure/). Следующий пример демонстрирует применение действия сценария к работающему кластеру:
...
Перед началом работы убедитесь, что установили и настроили Azure CLI. Убедитесь, что у вас установлена последняя версия. Дополнительные сведения см. в статье [Установка Azure CLI](/cli/azure/install-azure-cli).
...
Ознакомьтесь со статьей [Cluster REST API in Azure HDInsight](/rest/api/hdinsight/hdinsight-cluster) (REST API кластера в Azure HDInsight).
...
| [`az hdinsight script-action list`](/cli/azure/hdinsight/script-action#az-hdinsight-script-action-list) |Список всех сохраненных действий скрипта для указанного кластера. |
|[`az hdinsight script-action list-execution-history`](/cli/azure/hdinsight/script-action#az-hdinsight-script-action-list-execution-history)|Список всех журналов выполнения сценариев для указанного кластера.|
|[`az hdinsight script-action promote`](/cli/azure/hdinsight/script-action#az-hdinsight-script-action-promote)|Повышает заданное нерегламентированное выполнение скрипта до сохраненного скрипта.|
|[`az hdinsight script-action show-execution-details`](/cli/azure/hdinsight/script-action#az-hdinsight-script-action-show-execution-details)|Возвращает сведения о выполнении скрипта для заданного идентификатора выполнения скрипта.|
