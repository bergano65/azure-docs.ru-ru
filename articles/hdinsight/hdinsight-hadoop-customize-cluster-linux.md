...
description: Добавление пользовательских компонентов в кластеры HDInsight с помощью действий скрипта. Действия сценариев — это сценарии Bash, которые можно использовать для настройки конфигурации кластера или добавления дополнительных служб и служебных программ, таких как Hue, Solr или R.
...
* Действия сценария могут быть сохраняемыми или специализированными.
    ...
    Специализированные сценарии не сохраняются. Действия скриптов, используемые при создании кластера, автоматически сохраняются. Они не применяются на рабочих узлах, добавленных в кластер после выполнения сценария. Затем вы можете повысить уровень специализированного сценария до сохраняемого (и наоборот). Сценарии, в которых произошла ошибка, не сохраняются, даже если вы отдельно указали, что они должны быть сохранены.
...
Ошибка в выполняющемся сценарии в работающем кластере не приводит автоматически к сбою в работе кластера. После завершения сценария кластер должен вернуться в рабочее состояние. Даже если кластер запущен, ошибка сценария может привести к проблемам. К примеру, сценарий может удалить необходимые для работы кластера файлы.
...
1. Вы также можете использовать многоточие **…** справа от записей в разделе "Действия сценария" для выполнения действий.
...
| командлет | Компонент |
| --- | --- |
| `Get-AzHDInsightPersistedScriptAction` |Получение сведений о действиях сохраняемого сценария Это cmdlet не отменяет действия, выполняемые скриптом, оно только удаляет сохраняющийся флаг.|
...
| Команда | Описание |
| --- | --- |
| [az hdinsight сценарий действия удаления](https://docs.microsoft.com/cli/azure/hdinsight/script-action?view=azure-cli-latest#az-hdinsight-script-action-delete) |Удаляет указанное сохраняемые действия скрипта кластера. Эта команда не отменяет действия, выполняемые скриптом, она удаляет только сохраняющийся флаг.|
...
| [az hdinsight сценарий действия список](https://docs.microsoft.com/cli/azure/hdinsight/script-action?view=azure-cli-latest#az-hdinsight-script-action-list) |Перечисляет все сохраняющиеся действия скрипта для указанного кластера. |
|[az hdinsight сценарий-действие список-исполнение-история](https://docs.microsoft.com/cli/azure/hdinsight/script-action?view=azure-cli-latest#az-hdinsight-script-action-list-execution-history)|Перечисляет историю выполнения всех скриптов для указанного кластера.|
|[az hdinsight сценарий действия содействовать](https://docs.microsoft.com/cli/azure/hdinsight/script-action?view=azure-cli-latest#az-hdinsight-script-action-promote)|Усваивает указанное выполнение специального скрипта до сохраняемого скрипта.|
|[az hdinsight сценарий действия шоу-исполнение-детали](https://docs.microsoft.com/cli/azure/hdinsight/script-action?view=azure-cli-latest#az-hdinsight-script-action-show-execution-details)|Получает деталь выполнения скрипта для данного идентификатора выполнения скрипта.|
...
## <a name="support-for-open-source-software"></a>Поддержка программного обеспечения с открытым исходным кодом
...
1. **Представление о работе**. В кластере можно отправлять задания Hadoop или другие типы заданий, выполняющие или использующие настраиваемые компоненты.
...
3. **Образцы**. Корпорация Майкрософт и другие компании могут предоставлять примеры использования популярных настраиваемых компонентов в кластерах HDInsight. Эти примеры представляются без поддержки.
...
1. Найдите записи, для которых в столбце **Операции** указано **run\_customscriptaction**. Такие записи создаются при выполнении действий сценариев.
    ...
    ![Операции действий по сценарию Apache Ambari](./media/hdinsight-hadoop-customize-cluster-linux/ambari-script-action.png)
...
* Журналы хранилища находятся в `\STORAGE_ACCOUNT_NAME\DEFAULT_CONTAINER_NAME\custom-scriptaction-logs\CLUSTER_NAME\DATE`.
    ...
    ![Журналы действий по сценарию](./media/hdinsight-hadoop-customize-cluster-linux/script-action-logs-in-storage.png)
