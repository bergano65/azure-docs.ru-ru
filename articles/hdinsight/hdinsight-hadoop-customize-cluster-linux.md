---
title: Настройка кластеров Azure HDInsight с помощью действий скриптов
description: Добавление пользовательских компонентов в кластеры HDInsight с помощью действий скрипта. Действия скрипта являются скриптами Bash, которые могут быть использованы для настройки конфигурации кластера. Или добавьте дополнительные услуги и утилиты, такие как Hue, Solr или R.
...
ms.locfileid: "81770738"
---
# <a name="customize-azure-hdinsight-clusters-by-using-script-actions"></a>Настройка кластеров Azure HDInsight с помощью действий скриптов

Azure HDInsight предоставляет метод конфигурации, называемый **действиями скрипта,** который вызывает пользовательские скрипты для настройки кластера. Эти скрипты используются для установки дополнительных компонентов и изменения параметров конфигурации. Действия скрипта можно использовать во время или после создания кластера.
...
* **АМБАРИ. \_ВЫПОЛНИТЬ\_ПОЛЬЗОВАТЕЛЬСКИЕ КОМАНДЫ.** Это разрешение по умолчанию назначено для роли администратора Ambari.
* **КЛАСТЕР. \_ВЫПОЛНИТЬ\_ПОЛЬЗОВАТЕЛЬСКИЕ КОМАНДЫ.** Это разрешение по умолчанию назначено для администратора кластера HDInsight и администратора Ambari.
...
Человек, имеющий хотя бы доступ к подписке Azure, должен ранее зарегистрировать поставщика. Регистрация поставщика происходит, когда пользователь с доступом к подписке создает ресурс. Для без создания ресурса, [см. зарегистрировать поставщика с помощью REST](https://msdn.microsoft.com/library/azure/dn790548.aspx).
...
* Они должны храниться в универсальном коде ресурса (URI), доступном из кластера HDInsight. Возможные места хранения:
    ...
    * Для регулярных кластеров:

      * ADLS Gen1: Основной сервис, который использует HDInsight для доступа к хранилищу озера Data, должен иметь доступ к скрипту. URI для скриптов, находящихся в Data Lake Storage 1-го поколения, имеет формат `adl://DATALAKESTOREACCOUNTNAME.azuredatalakestore.net/path_to_file`.
      ...
      * Большой двоичный объект в учетной записи хранения Azure, которая служит основной или дополнительной учетной записью хранения для кластера HDInsight. При создании кластера HDInsight получает доступ к обоим типам учетных записей хранения.
        ...
        > [!IMPORTANT]  
        > Не поворачивайте ключ хранения в этой учетной записи Хранилища Azure, так как это приведет к тому, что последующие действия скрипта с сохраненными там скрипами сбой.

      * Общедоступная служба обмена файлами, доступная через http:// пути. Примерами являются Azure Blob, GitHub, OneDrive. Примеры URI см. в разделе [Пример сценариев действий сценария](#example-script-action-scripts).

     * Для кластеров с ESP поддерживаются wasb:// или wasbs:// или http's://URIS.

* Действия можно ограничить выполнением только на определенных типах узлов, например головных или рабочих.

* Может быть сохраняется или `ad hoc`.
    ...
    `Ad hoc`скрипты не сохраняются. Действия скриптов, используемые при создании кластера, автоматически сохраняются. Они не применяются на рабочих узлах, добавленных в кластер после выполнения сценария. Затем можно продвинуть `ad hoc` сценарий к упорствуемому скрипту `ad hoc` или понизить упорный сценарий до скрипта. Сценарии, в которых произошла ошибка, не сохраняются, даже если вы отдельно указали, что они должны быть сохранены.
...
* Можно использовать через портал Azure, Azure PowerShell, Azure CLI или HDInsight .NET SDK.
...
Вы можете выполнять такие операции, как остановка и запуск служб, включая услуги, связанные с Apache Hadoop. Если вы прекратите службы, убедитесь, что Ambari и другие службы, связанные с Hadoop, заработают до завершения скрипта. Эти необходимые службы определяют работоспособность и состояние кластера во время его создания.
...
> [!IMPORTANT]  
> Действия по сценарию должны завершиться в течение 60 минут, или они тайм-аут. Во время кластерной подготовки скрипт выполняется одновременно с другими процессами настройки и конфигурации. Конкуренция за ресурсы, такие как ЦП или пропускная способность сети, может привести к затягиванию выполнения сценария по сравнению со временем его выполнения в среде разработки.
>
...
Сбой скрипта в уже работаемом кластере автоматически не приводит к изменению кластера в неисходиное состояние. После завершения сценария кластер должен вернуться в рабочее состояние. Даже если кластер запущен, ошибка сценария может привести к проблемам. К примеру, сценарий может удалить необходимые для работы кластера файлы.

Действия сценариев выполняются с использованием привилегий суперпользователя. Убедитесь, что вы понимаете, что делает скрипт, прежде чем применить его к кластеру.
...
## <a name="script-action-during-cluster-creation"></a>Действие сценария при создании кластера
...
1. Начните создавать кластер, описанный в [создании кластеров на основе Linux в HDInsight, используя портал Azure.](hdinsight-hadoop-create-linux-clusters-portal.md) Из вкладки **«Конфигурация и цены»** выберите **действие скрипта .**

    ![Действие кластерного скрипта портала Azure](./media/hdinsight-hadoop-customize-cluster-linux/azure-portal-cluster-configuration-scriptaction.png)
...
1. Используйте запись __Выберите сценарий__, чтобы выбрать готовый сценарий. Чтобы использовать настраиваемый сценарий, выберите __Настраиваемый__. Затем укажите __имя__ и __универсальный код ресурса (URI) Bash-сценария__ своего сценария.
    ...
    | Голова/Рабочий/Зоозащитник |Укажите узлы, на которых запущен скрипт: **Head,** **Worker**или **zooKeeper**. |
...
1. Чтобы сохранить скрипт, нажмите кнопку __Создать__. Чтобы добавить еще один сценарий, вы можете использовать элемент __+ Submit new__ (+ Отправить новый).
    ...
    ![HDInsight несколько действий скрипта](./media/hdinsight-hadoop-customize-cluster-linux/multiple-scripts-actions.png)

    Когда вы закончите добавлять скрипты, вы возвращаетесь к вкладке **«Конфигурация и цены».**

1. Заполните оставшиеся этапы создания кластера в обычном режиме.
...
В этом разделе используется cmdlet [Add-AzHDInsightScriptAction](https://docs.microsoft.com/powershell/module/az.hdinsight/add-azhdinsightscriptaction) для вызывать скрипты для настройки кластера. Перед началом работы убедитесь, что установили и настроили Azure PowerShell. Для использования этих команд PowerShell необходим [модуль a .](https://docs.microsoft.com/powershell/azure/overview)
...
Пакет SDK HDInsight для .NET предоставляет клиентские библиотеки, которые упрощают работу с кластерами HDInsight из приложения .NET. Для примера кода [см.](https://docs.microsoft.com/dotnet/api/overview/azure/hdinsight?view=azure-dotnet#script-actions)

## <a name="script-action-to-a-running-cluster"></a>Действие сценария для запущенного кластера
...
1. Войдите на [портал Azure](https://portal.azure.com) и найдите кластер.
...
1. Используйте запись __Выберите сценарий__, чтобы выбрать готовый сценарий. Чтобы использовать настраиваемый сценарий, выберите __Настраиваемый__. Затем укажите __имя__ и __универсальный код ресурса (URI) Bash-сценария__ своего сценария.
    ...
    | Свойство | Значение |
    | --- | --- |
    | Выберите скрипт | Чтобы использовать свой собственный скрипт, выберите __пользовательский__. В противном случае выберите предоставленный скрипт. |
    | Имя |Укажите имя для действия сценария. |
    | URI bash-скрипта |Укажите URI сценария. |
    | Головной, рабочий или Zookeeper |Укажите узлы, на которых запущен скрипт: **Head,** **Worker**или **zooKeeper**. |
    | Параметры |Укажите параметры, если они требуются для сценария. |
...
Для использования этих команд PowerShell необходим [модуль a .](https://docs.microsoft.com/powershell/azure/overview) Следующий пример демонстрирует применение действия сценария к работающему кластеру:
...
Перед началом работы убедитесь, что установили и настроили Azure CLI. Убедитесь, что у вас есть последняя версия. Дополнительные сведения см. в статье [Установка Azure CLI](https://docs.microsoft.com/cli/azure/install-azure-cli).
...
1. Примените действие сценария в работающем кластере.
    ...
    Действительные `headnode` `workernode`роли, `zookeepernode` `edgenode`, . Если скрипт следует применять к нескольким типам узлов, разделите роли на пространство. Например, `--roles headnode workernode`.
...
1. Журнал сценариев для этого кластера отображается в разделе "Действия сценария". Эти сведения включают в себя список сохраняемых скриптов. На следующем снимке экрана показано, что в этом кластере был выполнен сценарий Solr. Сохраняемых сценариев на нем не видно.
    ...
    ![Действия портного скрипта представляют историю](./media/hdinsight-hadoop-customize-cluster-linux/script-action-history.png)
...
1. При выборе сценария в журнале отображается соответствующий раздел **Панель свойств**. В верхней части экрана можно повторно запустить скрипт или изменить его тип.
    ...
    ![Свойства действий скрипта способствуют](./media/hdinsight-hadoop-customize-cluster-linux/promote-script-actions.png)

1. Вы также можете выбрать эллипсис, **...**, справа от записи в разделе действий скрипта, чтобы делать действия.

    ![Упорные действия скрипта удаляют](./media/hdinsight-hadoop-customize-cluster-linux/hdi-delete-promoted-sa.png)

### <a name="azure-powershell"></a>Azure PowerShell

| командлет | Функция |
| --- | --- |
| `Get-AzHDInsightPersistedScriptAction` |Получение сведений о действиях сохраняемого сценария Это cmdlet не отменяет действия, выполненные скриптом, он только удаляет сохраняющийся флаг.|
...
| `Set-AzHDInsightPersistedScriptAction` |Содействие `ad hoc` действию скрипта к упорствуению действия скрипта. |
| `Remove-AzHDInsightPersistedScriptAction` |Снизите упорный `ad hoc` действие скрипта в действие. |
...
| Команда | Описание |
| --- | --- |
| [`az hdinsight script-action delete`](https://docs.microsoft.com/cli/azure/hdinsight/script-action?view=azure-cli-latest#az-hdinsight-script-action-delete) |Удаляет указанное сохраняемые действия скрипта кластера. Эта команда не отменяет действия, выполненные скриптом, она удаляет только сохраняющийся флаг.|
...
| [`az hdinsight script-action list`](https://docs.microsoft.com/cli/azure/hdinsight/script-action?view=azure-cli-latest#az-hdinsight-script-action-list) |Перечисляет все сохраняющиеся действия скрипта для указанного кластера. |
|[`az hdinsight script-action list-execution-history`](https://docs.microsoft.com/cli/azure/hdinsight/script-action?view=azure-cli-latest#az-hdinsight-script-action-list-execution-history)|Перечисляет историю выполнения всех скриптов для указанного кластера.|
|[`az hdinsight script-action promote`](https://docs.microsoft.com/cli/azure/hdinsight/script-action?view=azure-cli-latest#az-hdinsight-script-action-promote)|Усваивает указанное выполнение специального скрипта до сохраняемого скрипта.|
|[`az hdinsight script-action show-execution-details`](https://docs.microsoft.com/cli/azure/hdinsight/script-action?view=azure-cli-latest#az-hdinsight-script-action-show-execution-details)|Получает деталь выполнения скрипта для данного идентификатора выполнения скрипта.|
...
* [Действия скрипта устранения неполадок](troubleshoot-script-action.md)
