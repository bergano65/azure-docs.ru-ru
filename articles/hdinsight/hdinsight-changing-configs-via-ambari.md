...
1. Сохраните изменения, нажав зеленую кнопку **Save** (Сохранить) в верхней части экрана конфигурации.
    ...
    ![Амбари Амбари сохранить конфигурации](./media/hdinsight-changing-configs-via-ambari/ambari-save-changes1.png)
...
* `pig.cachedbag.memusage`: объем памяти, выделяемой для контейнера. Контейнер представляет собой набор кортежей. Кортеж — это упорядоченный набор полей, а поле — это фрагмент данных. Если данные в сумке не подходят к выделенной памяти, они разливаются на диск. Значение по умолчанию — 0,2. Это означает 20 процентов доступной памяти. Эта память используется совместно всеми контейнерами в приложении.
...
Если эта функция включена, небольшие файлы объединяются, чтоб уменьшить число задач сопоставления. Это повышает эффективность заданий с множеством небольших файлов. Чтобы включить эту функцию, задайте для параметра `pig.noSplitCombination` значение true. Значение по умолчанию — false.
...
1. Измените значение по умолчанию на 5000 МБ.
    ...
    ![Apache Ambari HBase памяти кучи](./media/hdinsight-changing-configs-via-ambari/ambari-hbase-heapsize.png)
...
![Apache HBase HRegion макс файлов](./media/hdinsight-changing-configs-via-ambari/hbase-hregion-max-filesize.png)
...
Использование локального буфера выделения Memstore определяет свойство `hbase.hregion.memstore.mslab.enabled`. Если эта функция включена (параметр имеет значение true), она предотвращает фрагментацию кучи во время операций записи большого объема данных. Значение по умолчанию — true.
