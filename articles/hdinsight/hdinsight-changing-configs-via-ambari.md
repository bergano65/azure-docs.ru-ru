...
1. Чтобы включить промежуточное сжатие, перейдите на вкладку **Configs** (Конфигурации) Hive, а затем установите для параметра `hive.exec.compress.intermediate` значение true. Значение по умолчанию — false.
    ...
    !["Средний уровень сжатия файла Hive"](./media/hdinsight-changing-configs-via-ambari/hive-exec-compress-intermediate.png)
...
1. Вот как можно добавить пользовательский параметр.
    ...
    a. Последовательно выберите **Hive** > **Конфигурация** > **Hive дополнительно** > **Пользовательский Hive — сайт**.
    ...
    ![' Apache Hive пользовательское свойство Add '](./media/hdinsight-changing-configs-via-ambari/hive-custom-property.png)
...
* Чтобы включить спекулятивное выполнение, перейдите на вкладку **Configs** (Конфигурации) Hive, а затем установите для параметра `hive.mapred.reduce.tasks.speculative.execution` значение true. Значение по умолчанию — false.
    ...
    ![' Hive mapred уменьшить задачи, выполняющие упреждающее выполнение "](./media/hdinsight-changing-configs-via-ambari/hive-mapred-reduce-tasks-speculative-execution.png)
...
Если эта функция включена, небольшие файлы объединяются, чтоб уменьшить число задач сопоставления. Этот параметр повышает эффективность заданий с большим количеством небольших файлов. Чтобы включить эту функцию, задайте для параметра `pig.noSplitCombination` значение true. Значение по умолчанию — false.
...
1. Измените значение по умолчанию на 5000 МБ.
    ...
    !["Apache Ambari HBase Memory хеапсизе"](./media/hdinsight-changing-configs-via-ambari/ambari-hbase-heapsize.png)
