---
title: Apache Ambari для оптимизации конфигураций кластера — Azure HDInsight
...
1. Сохраните изменения, нажав зеленую кнопку **Save** (Сохранить) в верхней части экрана конфигурации.
    ...
    ![Ambari Ambari сохранение конфигураций](./media/hdinsight-changing-configs-via-ambari/ambari-save-changes1.png)
...
1. На вкладке **Configs** (Конфигурации) Hive в поле фильтра введите **execution engine**.
    ...
    ![Механизм выполнения поиска Apache Ambari](./media/hdinsight-changing-configs-via-ambari/ambari-search-execution.png)

1. Свойство **Optimization** (Оптимизация) по умолчанию имеет значение **Tez**.

    ![Оптимизация — Apache Tez Engine](./media/hdinsight-changing-configs-via-ambari/optimization-apache-tez.png)
...
1. Задайте для обоих параметров значение **33 554 432** байтов (32 МБ).
    ...
    ![Размеры группирования Tez Apache Ambari](./media/hdinsight-changing-configs-via-ambari/apache-tez-grouping-size.png)
...
1. Чтобы изменить этот параметр, перейдите на вкладку **Configs** (Конфигурации) Hive и найдите параметр **Data per Reducer** (Данные на модуль сжатия) на странице "Settings" (Параметры).
    ...
    ![Данные Apache Ambari для каждого из них](./media/hdinsight-changing-configs-via-ambari/ambari-data-per-reducer.png)
...
1. Выберите **Edit** (Изменить), чтобы изменить значение параметра на 128 МБ (134 217 728 байтов), и нажмите клавишу **ВВОД**, чтобы сохранить изменение.
    ...
    ![Ambari данные для каждого уменьшения](./media/hdinsight-changing-configs-via-ambari/data-per-reducer-edited.png)
...
1. Чтобы ограничить число заданий, выполняемых параллельно, измените свойство `hive.exec.parallel.thread.number`. Значение по умолчанию: 8.

    ![Параллельное отображение Apache Hive Exec](./media/hdinsight-changing-configs-via-ambari/apache-hive-exec-parallel.png)
...
1. Чтобы включить векторизированное выполнение сжимаемой части запроса, задайте для параметра `hive.vectorized.execution.reduce.enabled` значение true. Значение по умолчанию — false.
    ...
    ![Apache Hive векторное выполнение](./media/hdinsight-changing-configs-via-ambari/hive-vectorized-execution.png)
...
Чтобы включить статистические, перейдите к разделу > **конфигурации** **Hive** > параметры и найдите **параметр** **включить Оптимизатор на основе затрат**, а затем установите переключатель в положение **вкл**.

![Оптимизатор на основе затрат HDInsight](./media/hdinsight-changing-configs-via-ambari/hdinsight-cbo-config.png)
...
* `hive.compute.query.using.stats`
    ...
    ![Apache Hive вычисление запроса с помощью статистики](./media/hdinsight-changing-configs-via-ambari/hive-compute-query-using-stats.png)
...
* `hive.stats.fetch.column.stats`
    ...
    ![Статистика по столбцам набора Apache Hive статистики](./media/hdinsight-changing-configs-via-ambari/hive-stats-fetch-column-stats.png)
...
1. Вот как можно добавить пользовательский параметр.
    ...
    а. Перейдите в раздел > **конфигурации** **Hive** > **Дополнительно** > **Пользовательский Hive — сайт**.

    б. Щелкните **Добавить свойство...** в нижней части области пользовательский Hive — сайт.

    в. В окне "Add Property" (Добавление свойства) введите ключ `mapred.map.output.compression.codec` и значение `org.apache.hadoop.io.compress.SnappyCodec`.

    . Выберите **Добавить**.

    ![Добавление Apache Hive пользовательского свойства](./media/hdinsight-changing-configs-via-ambari/hive-custom-property.png)
...
1. Чтобы выбрать кодек для сжатия выходных данных, добавьте пользовательское свойство `mapred.output.compression.codec` в область "Custom hive-site" (Настраиваемый сайт Hive), как описано на шаге 3 в предыдущем разделе.
    ...
    ![Apache Hive пользовательское свойство ADD2](./media/hdinsight-changing-configs-via-ambari/hive-custom-property2.png)
...
1. Чтобы ограничить число создаваемых динамических секций, измените параметр `hive.exec.max.dynamic.partitions`. Значение по умолчанию — 5000.
...
![Локальный автоматический режим Apache Hive Exec](./media/hdinsight-changing-configs-via-ambari/hive-exec-mode-local-auto.png)
...
| Тип объединения | Когда | Способ | Параметры Hive | Comments |
| -- | -- | -- | -- | -- |
...
1. Щелкните **сохранить** в правой верхней части окна, чтобы сохранить новое значение. Изменение некоторых свойств может потребовать перезапуска службы.
    ...
    ![Дополнительные свойства Apache Pig](./media/hdinsight-changing-configs-via-ambari/advanced-pig-properties.png)
...
* `pig.cachedbag.memusage`: объем памяти, выделяемой для контейнера. Контейнер представляет собой набор кортежей. Кортеж — это упорядоченный набор полей, а поле — это фрагмент данных. Если данные в контейнере выходят за пределы выделенной памяти, они сбрасываются на диск. Значение по умолчанию — 0,2. Это означает 20 процентов доступной памяти. Эта память используется совместно всеми контейнерами в приложении.
...
Конфигурация [Apache HBase](https://hbase.apache.org/) изменяется на вкладке **конфигурации HBase** . В следующих разделах описаны некоторые важные параметры конфигурации, влияющие на производительность HBase.
...
1. Измените значение по умолчанию на 5000 МБ.
    ...
    ![Хеапсизе памяти Apache Ambari HBase](./media/hdinsight-changing-configs-via-ambari/ambari-hbase-heapsize.png)
...
1. Чтобы изменить этот параметр, перейдите на вкладку **Settings** (Параметры) на вкладке **Configs** (Конфигурации) HBase, а затем найдите параметр **% of RegionServer Allocated to Read Buffers** (% памяти сервера региона, выделяемой для буферов чтения).
    ...
    ![Размер кэша блоков памяти Apache HBase](./media/hdinsight-changing-configs-via-ambari/hbase-block-cache-size.png)
...
![Выборка количества строк Apache HBase](./media/hdinsight-changing-configs-via-ambari/hbase-num-rows-fetched.png)
...
![Apache HBase Хрегион Max размер файла](./media/hdinsight-changing-configs-via-ambari/hbase-hregion-max-filesize.png)
...
* Множитель блока HBase определяется `hbase.hregion.memstore.block.multiplier`. Значение по умолчанию — 4. Максимально допустимое значение равно 8.
...
![Множитель блока для Apache HBase](./media/hdinsight-changing-configs-via-ambari/hbase-hregion-memstore-block-multiplier.png)
