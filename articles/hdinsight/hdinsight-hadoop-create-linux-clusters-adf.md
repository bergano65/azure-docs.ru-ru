...
- Установите Azure PowerShell. Инструкции см. в статье [Установка и настройка Azure PowerShell](https://docs.microsoft.com/powershell/azure/install-az-ps).
...
В этом разделе создайте учетную запись хранилища, которая будет использоваться в качестве хранилища по умолчанию, для кластера HDInsight, который создается по требованию. Эта учетная запись хранения также содержит пример скрипта [HiveQL](https://cwiki.apache.org/confluence/display/Hive/LanguageManual) **(hivescript.hql)**, который позволяет моделировать образец задания [Apache Hive](https://hive.apache.org/), выполняющегося в кластере.
...
1.  В разделе **Рекомендованные** выберите **Фабрика данных**.
...
1. Введите или выберите значения, как показано на следующем снимке экрана:
    ...
    |**Версия**     | Выберите **V2**. |
    |**Местоположение.**     | Автоматически задается то расположение, которое было указано при предыдущем создании группы ресурсов. В этом руководстве расположение установлено как **восточная часть США**. |
    

1. Нажмите кнопку **Создать**. Создание фабрики данных может занять от 2 до 4 минут.


1. После создания фабрики данных вы получите уведомление **Развертывание прошло успешно** с кнопкой **Перейти к ресурсу**.  Нажмите кнопку **Перейти к ресурсу**, чтобы открыть представление фабрики данных по умолчанию.
