...
7. Откройте папку, она должна содержать пример файла сценария **partitionweblogs.hql**.
...
4. В окне **New Linked Service** (Новая связанная служба) введите следующие значения и оставьте остальные по умолчанию.
    ...
    |Выбор региона | Выберите регион из раскрывающегося списка.|
    ...
    | Тип ОС/Пароль пользователя кластера | Укажите пароль для пользователя кластера. |
...
3. Убедитесь, что выбрано действие Hive, выберите вкладку **HDI Cluster** (Кластер HDI) и в раскрывающемся списке **HDInsight Linked Service** (Связанная служба HDInsight) выберите связанную службу **HDinightLinkedService**, созданную ранее для HDInsight.
...
4. Выберите вкладку **Скрипт** и затем сделайте следующее:
    ...
    1. В разделе **Расширенный** > **Параметры** выберите **Автозаполнение из скрипта**. Этот параметр ищет любые параметры в сценарии Hive, которым требуются значения во время выполнения. Использованный сценарий (**partitionweblogs.hql**) имеет параметр **Вывод**. Укажите **значение** в формате `wasb://adfgetstarted@<StorageAccount>.blob.core.windows.net/outputfolder/` для указания имеющейся папки в службе хранилища Azure. Путь следует вводить с учетом регистра. Это путь, где будут храниться выходные данные сценария.
