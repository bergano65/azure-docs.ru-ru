...
* PowerShell [модуль Az](https://docs.microsoft.com/powershell/azure/overview) установлен.
...
## <a name="create-preliminary-azure-objects"></a>Создание предварительных объекты Azure

В этом разделе создайте различных объектов, которые будут использоваться для кластера HDInsight, создаваемых по запросу. Созданная учетная запись хранения будет содержать образец [HiveQL](https://cwiki.apache.org/confluence/display/Hive/LanguageManual) сценария (`hivescript.hql`) использовать для имитации образец [Apache Hive](https://hive.apache.org/) задания, выполняемого в кластере.
...
3. Выберите имя группы ресурсов, созданной в сценарии PowerShell. Если отображается слишком много групп ресурсов, используйте фильтр.
...
2. В меню слева, перейдите к **+ создать ресурс** > **Analytics** > **фабрики данных**.
...
3. Введите или выберите следующие значения для **новая фабрика данных** плитки:
...
5. После создания фабрики данных, вы получите **развертывание прошло успешно** уведомление с **перейти к ресурсу** кнопки.  Нажмите кнопку **Перейти к ресурсу**, чтобы открыть представление фабрики данных по умолчанию.
...
1. В области слева от **приступим к работе** выберите **автор** значок.
...
4. Введите следующие значения для связанной службы хранилища:
    ...
    |Подписка Azure. |Выберите подписку из раскрывающегося списка.|
    |Имя учетной записи хранения |Выберите созданную учетную запись хранилища Azure как часть скрипта PowerShell.|
...
2. В **новая связанная служба** выберите **вычислений** вкладки.

3. Выберите **Azure HDInsight**, а затем выберите **Продолжить**.
...
4. В **новая связанная служба** окно, введите следующие значения и оставьте для остальных по умолчанию:

    | Свойство | Value |
    | --- | --- |
    | ИМЯ | Укажите `HDinisghtLinkedService`.|
    | type | Выберите **HDInsight по запросу**. |
    ...
    | Идентификатор субъекта-службы | Укажите идентификатор приложения субъекта-службы Azure Active Directory, созданной при выполнении предварительных требований. |
    | Ключ субъект-службы | Укажите ключ проверки подлинности для субъекта-службы Azure Active Directory. |
    | Префикс имени кластера | Укажите значение, которое будет иметь префикс для всех типов кластеров, создаваемых фабрикой данных. |
    |Подписка |Выберите подписку из раскрывающегося списка.|
    | Выбор группы ресурсов | Выберите группу ресурсов, созданной как часть сценария PowerShell, который использовался ранее.|
    |Выберите регион | Выберите регион из раскрывающегося списка.|
    | Тип операционной системы или кластера имя пользователя SSH | Введите имя пользователя SSH, обычно `sshuser`. |
    | Тип операционной системы или кластера пароль SSH | Укажите пароль пользователя SSH. |
    | Имя пользователя или в кластере тип ОС | Введите имя пользователя кластера, обычно `admin`. |
    | Тип операционной системы или кластера пароль пользователя | Укажите пароль для пользователя кластера. |
...
3. Убедитесь, что у вас есть действие Hive, который выбран, выберите **кластера HDI** вкладки и из **связанной службы HDInsight** стрелку раскрывающегося списка выберите связанные службы, созданные ранее,  **HDinightLinkedService**, для HDInsight.
...
4. Выберите вкладку **Скрипт** и затем сделайте следующее:
    ...
    1. Для **связанная служба скрипта**выберите **HDIStorageLinkedService** из раскрывающегося списка. Это — связанная служба хранилища, созданная ранее.
    ...
    1. В разделе **Расширенный** > **Параметры** выберите **Автозаполнение из скрипта**. Этот параметр ищет любые параметры в сценарии Hive, которым требуются значения во время выполнения. Использованный сценарий (**hivescript.hql**) имеет параметр **Вывод**. Укажите **значение** в формате `wasb://adfgetstarted@<StorageAccount>.blob.core.windows.net/outputfolder/` указывал на существующую папку в службе хранилища Azure. Путь следует вводить с учетом регистра. Это путь, где будут храниться выходные данные сценария.
...
1. На панели инструментов в области конструктора, выберите **добавить триггер** > **триггер теперь**.
...
С помощью создания кластера HDInsight по запросу вы не обязательно должны явно удалить кластер HDInsight. Удаление кластера заложено в основе конфигурации, которая указывается при создании конвейера. Тем не менее даже после удаления кластера, учетные записи хранения, связанные с кластером, продолжают существовать. Это сделано намеренно, чтобы сохранить данные без изменений. Однако, если вы не хотите сохранять данные, можно удалить созданную учетную запись хранения.
...
В этой статье вы узнали, как использовать фабрику данных Azure для создания кластера HDInsight по требованию и выполнения заданий [Apache Hive](https://hive.apache.org/). Перейдите к следующей статье, чтобы узнать, как создавать кластеры HDInsight с помощью пользовательской конфигурации.
