...
1. Убедитесь, что выбрано действие Hive, выберите вкладку **HDI Cluster** (Кластер HDI) и в раскрывающемся списке **HDInsight Linked Service** (Связанная служба HDInsight) выберите связанную службу **HDInsightLinkedService**, созданную ранее для HDInsight.
...
1. Выберите вкладку **Скрипт** и затем сделайте следующее:
    ...
    1. В разделе **Расширенный** > **Параметры** выберите **Автозаполнение из скрипта**. Этот параметр ищет любые параметры в сценарии Hive, которым требуются значения во время выполнения.
