...
4. Дважды щелкните кластер в списке, чтобы просмотреть подробные сведения о нем.
   ...
   * **Secure Shell**. Отображает сведения о подключении к кластеру по протоколу SSH.
   ...
   * **Представления Ambari**. Ссылки на веб-интерфейс Ambari.
...
5. Щелкните **Параметры**.
   ...
   * **Изменить масштаб кластера**. Увеличение и уменьшение количества рабочих узлов кластера.
...
* **Редактор Hive**. Графический пользовательский веб-интерфейс для отправки заданий Hive.  См. дополнительные сведения о [выполнении запросов Apache Hive с помощью консоли запросов](hadoop/apache-hadoop-use-hive-query-console.md).

    ![Редактор кустов на портале HDInsight](./media/hdinsight-administer-use-management-portal/hdinsight-hive-editor.png)
* **Журнал заданий**. Мониторинг заданий Hadoop.  
...
2. На рабочем столе дважды щелкните элемент **Командная строка Hadoop**.
    ...
    См. дополнительные сведения о [командах Apache Hadoop](https://hadoop.apache.org/docs/current/hadoop-project-dist/hadoop-common/CommandsManual.html).
