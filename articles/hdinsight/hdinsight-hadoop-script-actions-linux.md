...
description: Узнайте, как с помощью сценариев Bash для настройки кластеров HDInsight. Действия сценария позволяют выполнять сценарии во время или после создания кластера, чтобы изменить параметры конфигурации кластера или установки дополнительного программного обеспечения.
...
### <a name="bPS1"></a>Выбор целевой версии Apache Hadoop
...
### <a name="bps10"></a> Целевая версия операционной системы
...
Для большинства операций не требуется указывать файловую систему. Например ниже копирует файл hadoop common.jar из локальной файловой системы в хранилище кластера.
...
*Причина*. Эта ошибка возникает, когда для завершения строк в сценарии используется CRLF. В системах UNIX для завершения строк допускается только символ LF.
...
*Решение*. Если в вашем текстовом редакторе есть функция выбора формата Unix или использования LF для завершения строк, воспользуйтесь ею. Кроме того, в системе Unix вы можете использовать следующие команды изменения CRLF на LF.
