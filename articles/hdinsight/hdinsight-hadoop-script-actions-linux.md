---
title: Разработка действий по сценарию для настройки кластеров Azure HDInsight
description: Узнайте, как использовать скрипты Bash для настройки кластеров HDInsight. Действия скрипта позволяют запускать скрипты во время или после создания кластера, чтобы изменить настройки конфигурации кластера или установить дополнительное программное обеспечение.
...
* [Настройка пользовательских компонентов для использования хранилища Azure Blob](#bPS6)
...
### <a name="target-the-operating-system-version"></a><a name="bps10"></a>Целевой вариант операционной системы
...
Например, образцы, предоставленные корпорацией Майкрософт, хранятся в учетной записи [https://hdiconfigactions.blob.core.windows.net/](https://hdiconfigactions.blob.core.windows.net/) хранилища. Это открытый и доступный только для чтения контейнер, который поддерживает команда HDInsight.
...
Например, скрипт, изменяющие файлы конфигурации, не должен добавлять дубликаты записей, если он работал несколько раз.
...
В кластерах HDInsight под управлением Linux есть два головных узла, активных в пределах кластера. Действия сценария выполняются на обоих узлах. Если компоненты, которые вы устанавливаете, ожидают только один головной узлы, не устанавливайте компоненты на оба узла головы.
...
Для большинства операций не требуется указывать файловую систему. Например, следующие копии файла hadoop-common.jar из локальной файловой системы в кластерное хранилище:
...
### <a name="use-retry-logic-to-recover-from-transient-errors"></a><a name="bps9"></a>Используйте логику повтора для восстановления от временных ошибок

При загрузке файлов, установке пакетов с использованием apt-get или других действиях, передающих данные через Интернет, действие может выйти из строя из-за временных сетевых ошибок. Например, удаленный ресурс, с помощью сообщения с ним, может находиться в процессе сбоя в резервном узлах.
...
Вспомогательные методы действий сценариев — это служебные программы, которые можно использовать при создании пользовательских сценариев. Эти методы содержатся [https://hdiconfigactions.blob.core.windows.net/linuxconfigactionmodulev01/HDInsightUtilities-v01.sh](https://hdiconfigactions.blob.core.windows.net/linuxconfigactionmodulev01/HDInsightUtilities-v01.sh) в скрипте. Чтобы скачать и использовать их как часть сценария, используйте следующую команду:
...
| --- | --- |
| `download_file SOURCEURL DESTFILEPATH [OVERWRITE]` |Скачивает файл из исходного универсального кода ресурса (URI) и сохраняет его в указанное расположение. По умолчанию он не перезаписывает существующий файл. |
...
* __Учетная запись Azure Data Lake Storage__, связанная с кластером HDInsight. Для получения дополнительной [информации](../storage/data-lake-storage/quickstart-create-connect-hdi-cluster.md)об использовании хранилища озер данных Azure с помощью HDInsight см.
...
Корпорация Майкрософт предоставляет примеры сценариев для установки компонентов в кластере HDInsight. В качестве примера примера примера можно [просмотреть «Установку» и использовать «Hue» в кластерах HDInsight.](hdinsight-hadoop-hue-linux.md)

## <a name="troubleshooting"></a>Устранение неполадок

Ниже приведены ошибки, с которые вы можете столкнуться при использовании разработанных сценариев:
...
*Разрешение*: Если это опция в текстовом редакторе, выберите формат Unix или LF для окончания строки. Кроме того, в системе Unix вы можете использовать следующие команды изменения CRLF на LF.
