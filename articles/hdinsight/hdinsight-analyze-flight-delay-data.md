---
title: Анализ данных о задержке рейсов с помощью Apache Hadoop в HDInsight в Azure
...
> [!IMPORTANT]  
> Действия, описанные в этом документе, требуют наличия кластера HDInsight на основе Windows. Linux — это единственная операционная система, используемая для работы с HDInsight 3.4 или более поздних версий. Дополнительные сведения см. в разделе [Приближается дата прекращения сопровождения HDI версии 3.3](hdinsight-component-versioning.md#hdinsight-windows-retirement). Инструкции по работе с кластерами под управлением Linux см. в статье [Анализ данных о задержке рейсов с помощью Apache Hive в HDInsight в Linux](hdinsight-analyze-flight-delay-data-linux.md).
...
> [!NOTE]  
> Действия, описанные в этом документе, относятся только к кластерам HDInsight для Windows. Инструкции по работе с кластерами под управлением Linux см. в статье [Анализ данных о задержке рейсов с помощью Apache Hive в HDInsight (Linux)](hdinsight-analyze-flight-delay-data-linux.md).
...
|Файлы|ОПИСАНИЕ|  
|----|----|   
|wasb://flightdelay@hditutorialdata.blob.core.windows.net/flightdelays.hql|Для запуска задания Hive используется скрипт HiveQL. Этот сценарий был загружен в учетную запись хранилища больших двоичных объектов Azure с общими правами доступа. В <a href="#appendix-b">приложении Б</a> содержатся инструкции о том, как подготовить и отправить файл в учетную запись хранения больших двоичных объектов Azure.|
|wasb://flightdelay@hditutorialdata.blob.core.windows.net/2013Data|Входные данные для задания Hive. Эти данные были отправлены в учетную запись хранилища больших двоичных объектов Azure с общим доступом. В <a href="#appendix-a">приложении А</a> содержатся инструкции о том, как получить данные и отправить их в учетную запись хранения больших двоичных объектов Azure.|
|\tutorials\flightdelays\output|Выходной путь для задания Hive. Для хранения результирующих данных используется контейнер по умолчанию.|
|\tutorials\flightdelays\jobstatus|Папка состояния задания Hive в контейнере по умолчанию.|
...
Дополнительные сведения о создании кластера HDInsight и выполнении заданий Hive см. в статьях [Создание кластеров Apache Hadoop в HDInsight][hdinsight-provision] и [Использование Apache Hive с HDInsight][hdinsight-use-hive].
...
Отправление файла данных и файлов скриптов [HiveQL](https://cwiki.apache.org/confluence/display/Hive/LanguageManual) (см. [приложение Б](#appendix-b)) нужно спланировать. Суть заключается в том, чтобы сохранить файлы данных и файл HiveQL до создания кластера HDInsight и запуска задания Hive. Существует два варианта.
...
* **Использование учетной записи хранения Azure, отличной от той, которая используется кластером HDInsight в качестве файловой системы по умолчанию.** В этом случае необходимо изменить относящуюся к созданию часть сценария Windows PowerShell, которую можно найти в разделе, посвященном созданию кластера HDInsight и выполнению заданий Apache Hive и Sqoop, чтобы привязать эту учетную запись хранения как дополнительную. Инструкции см. в статье о [создании кластера Apache Hadoop в HDInsight][hdinsight-provision]. После этого кластеру HDInsight станет известен ключ доступа для этой учетной записи хранения.
...
2. На странице выберите следующие значения:
    ...
    |Поля|*Year*, *FlightDate*, *UniqueCarrier*, *Carrier*, *FlightNum*, *OriginAirportID*, *Origin*, *OriginCityName*, *OriginState*, *DestAirportID*, *Dest*, *DestCityName*, *DestState*, *DepDelayMinutes*, *ArrDelay*, *ArrDelayMinutes*, *CarrierDelay*, *WeatherDelay*, *NASDelay*, *SecurityDelay*, *LateAircraftDelay* (очистите остальные поля)|
...
1. Задайте следующие параметры.
    ...
    |Имя переменной|Примечания|
    |---|---|
    |$storageAccountName|Учетная запись хранения Azure, в которую необходимо отправить данные.|
    |$blobContainerName|Контейнер больших двоичных объектов, в который необходимо отправить данные.|
...
Полный список команд HiveQL см. в разделе о [языке описания данных Apache Hive][hadoop-hiveql]. В конце каждой команды [HiveQL](https://cwiki.apache.org/confluence/display/Hive/LanguageManual) должна стоять точка с запятой.
...
1. Задайте следующие параметры.
    ...
    |Имя переменной|Примечания|
    |---|---|
    |$storageAccountName|Учетная запись хранения Azure, в которую требуется отправить скрипт HiveQL.|
    |$blobContainerName|Контейнер больших двоичных объектов, в который необходимо отправить сценарий HiveQL.|
...
1. Задайте следующие параметры.
    ...
    |Имя переменной|Примечания|
    |---|---|
    |$sqlDatabaseServerName|Имя сервера базы данных SQL Azure. Чтобы создать новый сервер, не указывайте ничего.|
    |$sqlDatabaseUsername|Имя для входа на сервер базы данных SQL Azure. Если значение переменной $sqlDatabaseServerName представляет собой существующий сервер, для выполнения аутентификации на сервере используются имя пользователя и пароль для входа. В противном случае они используются для создания нового сервера.|
    |$sqlDatabasePassword|Пароль для входа на сервер базы данных SQL Azure.|
    |$sqlDatabaseLocation|Это значение используется только при создании нового сервера базы данных SQL Azure.|
    |$sqlDatabaseName|База данных SQL, которая используется для создания таблицы AvgDelays для задания Sqoop. Если это значение не задано, то будет создана база данных с именем HDISqoop. Имя таблицы для выходных данных задания Sqoop&#160;— AvgDelays.|
...
3. Скопируйте следующий скрипт и вставьте его в область скриптов:  
   ...
   > [!NOTE]  
   > В этом скрипте, чтобы получить внешний IP-адрес, используется служба передачи репрезентативного состояния (REST): https://bot.whatismyipaddress.com. Этот IP-адрес используется для создания правила брандмауэра для сервера базы данных SQL.

    В данном сценарии используются следующие переменные:

   * **$ipAddressRestService.** Значение по умолчанию — https://bot.whatismyipaddress.com. Это общедоступный IP-адрес службы REST для получения внешнего IP-адреса. При необходимости можно использовать и другие службы. Внешний IP-адрес, полученный с помощью данной службы, используется при создании правила брандмауэра для сервера базы данных SQL Azure, поэтому доступ к этой базе данных можно получить с рабочей станции (с помощью скрипта Windows PowerShell).
...
Теперь вы знаете, как отправить файл в хранилище BLOB-объектов Azure, как заполнить таблицу Apache Hive, используя данные из этого хранилища, как выполнять запросы Hive и как использовать Sqoop для экспорта данных из [Apache Hadoop HDFS](https://hadoop.apache.org/docs/r1.2.1/hdfs_design.html) в базу данных SQL Azure. Для получения дополнительных сведений ознакомьтесь со следующими статьями:
