...
Можно развернуть кластер, чтобы справиться с пиковыми нагрузками, а затем уменьшить его масштаб, когда эти дополнительные узлы уже будут не нужны. [Функция автоматического масштабирования](hdinsight-autoscale-clusters.md) позволяет автоматически масштабируемого кластера на основе метрик и временных интервалов, предопределенные видеодрайвером. Дополнительные сведения о масштабировании кластеров вручную, см. в разделе [кластеры HDInsight масштабирования](hdinsight-scaling-best-practices.md).
...
1. Выберите **Справка и поддержка** в нижней части страницы.
1. Выберите на **новый запрос на поддержку**.
...
1. На **сведения** странице введите описание проблемы, выберите уровень серьезности проблемы, предпочтительный способ контакта и остальные обязательные поля.
1. По завершении выберите **Next: Просмотр и создание >>** .
1. На **проверки + создать** выберите **создать**.
