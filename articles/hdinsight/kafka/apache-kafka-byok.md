...
## <a name="setup-the-key-vault-and-keys"></a>Настройка Key Vault и ключей
   ...
   3. Создание ключей
        ...
        б) Задайте **Параметры**, чтобы **Создать** и присвоить имя для ключа.

        ![Имя ключа для создания Apache Kafka](./media/apache-kafka-byok/apache-kafka-create-key.png "Создать имя ключа")
    ...
    4. Добавьте управляемое удостоверение в политику доступа хранилища ключей.
        ...
        б) В поле **Выбор субъекта** выберите назначаемое пользователем управляемое удостоверение, которое вы создали.
...
## <a name="create-hdinsight-cluster"></a>Создание кластера HDInsight
   ...
   Во время создания кластера укажите полный URL-адрес ключа, включая версию ключа. Пример: `https://contoso-kv.vault.azure.net/keys/kafkaClusterKey/46ab702136bc4b229f8b10e8c2997fa4`. Кроме того, необходимо назначить кластеру управляемое удостоверение и указать URI ключа.
