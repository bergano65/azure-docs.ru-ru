...
![Создание назначаемого пользователем управляемого удостоверения на портале Azure](./media/apache-kafka-byok/azure-portal-create-managed-identity.png)
...
1. Чтобы создать новое хранилище ключей, выполните краткое руководство по [Azure Key Vault](../../key-vault/quick-create-cli.md). Дополнительные сведения об импорте существующих ключей см. в статье [Сведения о ключах, секретах и сертификатах](../../key-vault/about-keys-secrets-and-certificates.md).
...
1. Создайте ключи.
    ...
    ![Создание нового ключа в Azure Key Vault](./media/apache-kafka-byok/kafka-create-new-key.png "Создание нового ключа в хранилище ключей Azure")

    b. Задайте **Параметры**, чтобы **Создать** и присвоить имя для ключа.

    ![Apache Kafka создает имя ключа](./media/apache-kafka-byok/apache-kafka-create-key.png "Создать имя ключа")

    c. Выберите ключ, созданный из списка ключей.

    ![Список ключей хранилища ключей Apache Kafka](./media/apache-kafka-byok/kafka-key-vault-key-list.png)

    d. Используя собственный ключ для шифрования кластера Kafka, необходимо предоставить ключ URI. Скопируйте **идентификатор ключа** и сохраните его, пока не будете готовы к созданию кластера.

    ![Идентификатор ключа получения Apache Kafka](./media/apache-kafka-byok/kafka-get-key-identifier.png)
...
1. Добавьте управляемое удостоверение в политику доступа хранилища ключей.
    ...
    ![Задание выбранного субъекта для политики доступа Azure Key Vault](./media/apache-kafka-byok/azure-portal-add-access-policy.png)
...
![Kafka поворачивать ключ шифрования диска](./media/apache-kafka-byok/apache-kafka-change-key.png)
...
![Оповещение о Ambari доступа к ключу Apache Kafka](./media/apache-kafka-byok/kafka-byok-ambari-alert.png)
...
* Дополнительные сведения об Azure Key Vault см. в статье [Что такое Azure Key Vault?](../../key-vault/key-vault-overview.md)
