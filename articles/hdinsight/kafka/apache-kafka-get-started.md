...
ms.locfileid: "51014522"
---
# <a name="quickstart-create-an-apache-kafka-on-hdinsight-cluster"></a>Краткое руководство по созданию Apache Kafka в кластере HDInsight
...
6. В колонке **Хранилище** выберите или создайте учетную запись хранения. Для действий, описанных в этом документе, в других полях этой колонки оставьте значения по умолчанию. Нажмите кнопку __Далее__, чтобы сохранить конфигурацию хранилища. Дополнительные сведения об использовании Data Lake Storage Gen2 см. в статье [Краткое руководство по установке кластеров в HDInsight](../../storage/data-lake-storage/quickstart-create-connect-hdi-cluster.md).

   ![Настройка параметров учетной записи хранения для HDInsight](./media/apache-kafka-get-started/storage-configuration.png)

7. Чтобы продолжить, оставив значения параметров по умолчанию, в колонке __Приложения (необязательно)__ щелкните __Далее__.

8. Чтобы продолжить, оставив значения параметров по умолчанию, в колонке __Размер кластера__ щелкните __Далее__.
...
9. Чтобы продолжить, оставив значения параметров по умолчанию, в колонке __Дополнительные параметры__ щелкните __Далее__.

10. В колонке **Сводка** проверьте конфигурацию для кластера. Используйте ссылки __Изменить__, чтобы изменить неправильные параметры. Наконец, нажмите кнопку "Создать" для создания кластера.
...
Для работы с Kafka необходимы сведения об узле *Zookeeper* и узле *брокера*. Эти узлы используются API Apache Kafka и многими другими служебными программами, поставляемыми с платформой Kafka.
...
2. Чтобы задать переменную среды с именем кластера, выполните следующую команду.
    ...
    При появлении запроса введите имя кластера Apache Kafka.
...
5. Чтобы задать переменную среды со сведениями об узле брокера Apache Kafka, выполните следующую команду.
...
* **Чтобы создать раздел**, в сеансе SSH-подключения выполните следующую команду.
    ...
    * Каждая секция реплицируются на три рабочих узла в кластере.
        ...
        Для обеспечения высокой доступности используйте [средство перераспределения секций Kafka](https://github.com/hdinsight/hdinsight-kafka-tools). Этот инструмент следует запускать из сеанса SSH-подключения на головном узле кластера Apache Kafka.
