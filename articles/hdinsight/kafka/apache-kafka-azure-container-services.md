...
В этой статье предполагается, что вы ознакомились с [руководством по службе Azure Kubernetes](../../aks/tutorial-kubernetes-prepare-app.md). В этой статье создается служба контейнеров, создается кластер Kubernetes, реестр контейнеров и настраивается служебная программа `kubectl`.
...
![HDInsight в одной виртуальной сети, AKS в другой, с помощью пиринга](./media/apache-kafka-azure-container-services/kafka-aks-architecture.png)
...
> [!IMPORTANT]  
> AKS создает виртуальную сеть во время установки в **дополнительной** группе ресурсов. Дополнительная группа ресурсов соответствует соглашению об именовании **MC_resourceGroup_AKSclusterName_location**.  
...
### <a name="identify-preliminary-information"></a>Выявление предварительных сведений

1. В [портал Azure](https://portal.azure.com)выберите дополнительную **группу ресурсов** , содержащую виртуальную сеть для кластера AKS.

2. В группе ресурсов выберите ресурс __виртуальной сети__ . Запишите имя для использования в дальнейшем.

3. В разделе **Параметры**выберите __адресное пространство__. Запишите полученное адресное пространство.

### <a name="create-virtual-network"></a>Создание виртуальной сети

1. Чтобы создать виртуальную сеть для HDInsight, перейдите в раздел __+ создать ресурс__ > __сети__ > __Виртуальная сеть__.

1. Создайте сеть, следуя приведенным ниже рекомендациям по определенным свойствам.

    |Свойство | Value |
    |---|---|
    |Пространство адресов|Необходимо использовать адресное пространство, которое не пересекается с используемой сетью кластера AKS.|
    |Location|Используйте то же __расположение__ виртуальной сети, что и в кластере AKS.|
...
1. Выберите __+ Добавить__ и заполните форму, используя следующие значения:
    ...
    |Свойство |Value |
    |---|---|
    |Имя пиринга из \<этого > VN в удаленную виртуальную сеть|Введите уникальное имя этой конфигурации пиринга.|
    |Виртуальная сеть|Выберите виртуальную сеть для **кластера AKS**.|
    |Имя пиринга из \<AKS VN > для \<этого VN >|Укажите уникальное имя.|
...
1. Откройте веб-браузер и перейдите по адресу `https://CLUSTERNAME.azurehdinsight.net`. Замените ИМЯ_КЛАСТЕРА именем Kafka в кластере HDInsight.
...
3. Чтобы просмотреть конфигурацию Kafka, выберите пункт __Configs__ (Конфигурации) в верхней части окна.
    ...
    ![Настройка служб Apache Ambari Services](./media/apache-kafka-azure-container-services/select-kafka-config1.png)
...
8. Нажмите кнопку __Save__ (Сохранить), чтобы сохранить изменения в конфигурации. Введите текст, описывающий изменения. После сохранения изменений нажмите кнопку __ОК__.
    ...
    ![Конфигурация сохранения для Apache Ambari](./media/apache-kafka-azure-container-services/save-configuration-button.png)
