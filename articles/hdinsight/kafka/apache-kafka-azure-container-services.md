...
В этой статье предполагается, что вы ознакомились с [руководством по службе Azure Kubernetes](../../aks/tutorial-kubernetes-prepare-app.md). Эта статья создает контейнерную службу, создает кластер Kubernetes, `kubectl` реестр контейнеров и настраивает утилиту.
...
![HDInsight в одной виртуальной сети, AKS в другой, используя пиринг](./media/apache-kafka-azure-container-services/kafka-aks-architecture.png)
...
> [!IMPORTANT]  
> AKS создает виртуальную сеть во время установки в **дополнительную** группу ресурсов. Дополнительная группа ресурсов следует конвенции о наименовании **MC_resourceGroup_AKSclusterName_location.**  
...
### <a name="identify-preliminary-information"></a>Определить предварительную информацию

1. На [портале Azure](https://portal.azure.com)найдите дополнительную **группу ресурсов,** содержащую виртуальную сеть для кластера AKS.

2. Из группы ресурсов выберите ресурс __Виртуальной сети.__ Запишите имя для использования в дальнейшем.

3. Под **настройками**выберите __адресное пространство.__ Запишите полученное адресное пространство.

### <a name="create-virtual-network"></a>Создание виртуальной сети

1. Чтобы создать виртуальную сеть для HDInsight, перейдите на __и создайте__ > __ресурсСети__ > __Виртуальная сеть.__

1. Создание сети с использованием следующих рекомендаций для определенных свойств:

    |Свойство | Значение |
    |---|---|
    |Пространство адресов|Необходимо использовать адресное пространство, которое не перекрывает то, что используется кластерной сетью AKS.|
...
1. Выберите __+ Добавить__ и заполните форму, используя следующие значения:
    ...
    |Свойство |Значение |
    |---|---|
    |Название пиринга из \<этого VN> к удаленной виртуальной сети|Введите уникальное имя этой конфигурации пиринга.|
    |Виртуальная сеть|выберите виртуальную сеть для **кластера AKS.**|
    |Название пиринга от \<AKS VN \<> к этому VN>|Укажите уникальное имя.|

    Оставьте во всех остальных полях значения по умолчанию, а затем нажмите кнопку __ОК__, чтобы настроить пиринг.

## <a name="create-apache-kafka-cluster-on-hdinsight"></a>Создание кластера Apache Kafka на HDInsight
...
3. Чтобы просмотреть конфигурацию Kafka, выберите пункт __Configs__ (Конфигурации) в верхней части окна.
    ...
    ![Конфигурация служб Apache Ambari](./media/apache-kafka-azure-container-services/select-kafka-config1.png)
...
8. Нажмите кнопку __Save__ (Сохранить), чтобы сохранить изменения в конфигурации. Введите текст, описывающий изменения. После сохранения изменений нажмите кнопку __ОК__.
    ...
    ![Apache Ambari сохранить конфигурацию](./media/apache-kafka-azure-container-services/save-configuration-button.png)
...
2. Скачать пример [https://github.com/Blackmist/Kafka-AKS-Test](https://github.com/Blackmist/Kafka-AKS-Test)приложения из .
...
11. Откройте браузер и введите внешний IP-адрес службы. Вы должны перейти на страницу, аналогичную показанной ниже:
    ...
    ![Apache Кафка тест веб-страницы изображения](./media/apache-kafka-azure-container-services/test-web-page-image1.png)
