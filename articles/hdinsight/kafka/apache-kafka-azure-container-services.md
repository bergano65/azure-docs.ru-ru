...
В этой статье предполагается, что вы ознакомились с [руководством по службе Azure Kubernetes](../../aks/tutorial-kubernetes-prepare-app.md). В этой статье создается служба контейнеров, создается кластер Kubernetes, реестр контейнеров и настраивается `kubectl` программа.
...
1. Чтобы создать виртуальную сеть для HDInsight, перейдите в раздел __+ создать ресурс__ > __Сетевые подключения__ > __Виртуальная сеть__.
...
1. Выберите __+ Добавить__ и заполните форму, используя следующие значения:
    ...
    |Имя пиринга из \<AKS VN> к \<этому VN>|Укажите уникальное имя.|
