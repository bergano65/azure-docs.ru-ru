...
Кластеры HDInsight предоставляют доступ к веб-интерфейсу Apache Ambari через Интернет, но для некоторых компонентов требуется туннель SSH. Например, веб-интерфейс службы Apache Oozie не может быть доступен через Интернет без туннеля SSh.
...
* **q** — тихий режим;
* **T** - Отключить псевдо-tty распределения, так как вы просто переадресации порта.
* **n** - Предотвратить чтение STDIN, так как вы просто пересылки порта.
* **N** - Не выполняйте удаленную команду, так как вы просто пересылаете порт.
