...
Кластеры HDInsight предоставляют доступ к веб-интерфейсу Apache Ambari через Интернет, но для некоторых компонентов требуется туннель SSH. Например, веб-интерфейс для службы Apache Oozie не может быть доступен через Интернет без туннеля SSh.
...
Используйте следующую команду для создания туннеля SSH с помощью команды `ssh` . Замените `sshuser` на пользователя SSH для кластера HDInsight и замените `CLUSTERNAME` именем кластера HDInsight:
...
* **q** — тихий режим;
* **T** — отключение выделения псевдо-TTY, так как выполняется только пересылка порта.
* **n** — предотвращение чтения stdin, так как выполняется только пересылка порта.
* **N** — не выполнять удаленную команду, так как выполняется только пересылка порта.
...
1. Введите следующую информацию в форме **Параметры, управляющие перенаправлением портов SSH** :
    ...
    |Место назначения|SSH-адрес для кластера HDInsight. Например, **mycluster-ssh.azurehdinsight.net**.|
    |Динамические|Включает динамическую маршрутизацию прокси-сервера SOCKS.|
...
4. Откроется страница, аналогичная следующей:
    ...
    > [!NOTE]  
    > Обратите внимание на URL-адрес этой страницы. оно должно быть похоже на `http://hn1-CLUSTERNAME.randomcharacters.cx.internal.cloudapp.net:8088/cluster`. Этот URI использует полное внутреннее доменное имя узла (FQDN), и он доступен только при использовании туннеля SSH.
