...
* Прямой доступ к службам Apache Hadoop, которые недоступны в Интернете. Например, возможность напрямую работать с API Apache Kafka или использовать API Java для Apache HBase.
...
* Необходимо ли ограничить/перенаправить входящий или исходящий трафик в HDInsight?
    ...
    Для HDInsight требуется неограниченная связь с определенными IP-адресами в центре обработки данных Azure. Существует несколько портов, которые следует разрешить в брандмауэрах для взаимодействия с клиентами. Дополнительные сведения см. в разделе [Управление сетевым трафиком](./control-network-traffic.md).
...
> - Виртуальная сеть и создаваемый кластер должны находиться в одной подписке.
...
* Дополнительные сведения об управлении трафиком, включая интеграцию брандмауэра, см. в разделе [Управление сетевым трафиком](./control-network-traffic.md).
