...
Вы можете управлять минимальными версиями TLS, поддерживаемыми на узлах шлюза для кластера HDInsight, с помощью свойства *минсуппортедтлсверсион* в шаблоне Resource Manager во время развертывания. Пример шаблона см. в статье шаблон быстрого запуска для [HDInsight минимум TLS 1,2](https://github.com/Azure/azure-quickstart-templates/tree/master/101-hdinsight-minimum-tls). Это свойство поддерживает три значения: "1,0", "1,1" и "1,2", которые соответствуют TLS 1.0 +, TLS 1.1 + и TLS 1.2 + соответственно. По умолчанию, не указывая это свойство, кластеры Azure HDInsight принимают TLS 1,2 подключений на общедоступных конечных точках HTTPS, а также старые версии для обратной совместимости. В итоге HDInsight будет применять TLS 1,2 или более поздней версии на всех подключениях к узлам шлюза.
