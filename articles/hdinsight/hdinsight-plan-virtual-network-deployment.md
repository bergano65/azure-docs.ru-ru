...
3. Найдите назначенный Azure DNS-суффикс для виртуальной сети. Это значение аналогично `0owcbllr5hze3hxdja3mqlrhhe.ex.internal.cloudapp.net`. Дополнительные сведения о поиске DNS-суффиксов см. в разделе [Пример. Пользовательский DNS-сервер](hdinsight-create-virtual-network.md#example-dns).
...
* **Сетевые виртуальные устройства** (NVA) может использоваться только с исходящим трафиком. NVA реплицирует функциональность таких устройств, как брандмауэры и маршрутизаторы. Дополнительные сведения см. в документе [Сетевые устройства](https://azure.microsoft.com/solutions/network-appliances).
