...
Если вы планируете использовать **брандмауэр** и получить доступ к кластеру за пределами определенных портов, вам следует разрешить трафик на этих портах, необходимых для вашего сценария. По умолчанию специальный белый список портов не требуется до тех пор, пока трафик управления Azure, объясняемый в предыдущем разделе, не допускается к кластеру в порту 443.
