...
ms.locfileid: "75922663"
---
# <a name="operationalize-a-data-analytics-pipeline"></a>Ввод в эксплуатацию конвейера аналитики данных

*Конвейеры данных* лежат в основе многих решений для анализа данных. Как следует из названия, конвейер данных принимает необработанные данные, очищает и изменяет их по мере необходимости, а затем обычно выполняет вычисления или агрегации перед хранением обработанных данных. Обработанные данные используются клиентами, отчетами или API-интерфейсами. Конвейер данных должен возвращать повторяемые результаты по расписанию или во время активации с помощью новых данных.
...
![Oozie Полет пример данных трубопровода](./media/hdinsight-operationalize-data-pipeline/pipeline-overview-oozie.png)

## <a name="provision-azure-resources"></a>Подготовка ресурсов Azure

Для работы этого конвейера требуются база данных SQL Azure и кластер HDInsight Hadoop в одном расположении. База данных Azure S'L хранит как сводные данные, полученные конвейером, так и магазин метаданных Oozie.

### <a name="provision-azure-sql-database"></a>Подготовка базы данных SQL Azure

1. Создайте Базу данных SQL Azure. На [портале Azure см.](../sql-database/sql-database-single-database-get-started.md)

1. Чтобы убедиться, что кластер HDInsight может получить доступ к подключенной базе данных Azure S'L, назначьте правила брандмауэра базы данных Azure S'L, чтобы позволить службам и ресурсам Azure получить доступ к серверу. Эту опцию можно включить на портале Azure, выбрав **брандмауэр Set server**и выбрав **ON** под **allow Azure сервисы и ресурсы для доступа к этому серверу** для сервера или базы данных Базы данных Azure s'L. Дополнительные сведения см. в разделе о [создании правил брандмауэра для IP-адресов и управление ими](../sql-database/sql-database-firewall-configure.md#use-the-azure-portal-to-manage-server-level-ip-firewall-rules).

1. Используйте [редактор запроса](../sql-database/sql-database-single-database-get-started.md#query-the-database) для выполнения следующих `dailyflights` инструкций s'L для создания таблицы, в которую будут храниться обобщенные данные с каждого запуска конвейера.
...
### <a name="provision-an-apache-hadoop-cluster"></a>Предоставление кластера Apache Hadoop

Создайте кластер Apache Hadoop с пользовательским метамагазином. Во время создания кластера с портала, из вкладки **Хранения,** убедитесь, что вы выбираете базу данных S'L в **настройках Metastore.** Для получения дополнительной информации о выборе метамагазина, [см. Выберите пользовательский метамагазин во время создания кластера.](./hdinsight-use-external-metadata-stores.md#select-a-custom-metastore-during-cluster-creation) Для получения дополнительной информации о создании кластера, смотрите [Начало с HDInsight на Linux](hadoop/apache-hadoop-linux-tutorial-get-started.md).
...
1. Запустите следующую команду, чтобы открыть туннель `CLUSTERNAME` SSH для кластера, где находится название кластера:
...
1. Чтобы получить доступ к **веб-консоли Oozie** из Амбари, перейдите на **Oozie** > **Быстрые ссылки** > «Активный сервер» > **интерфейсу веб Oozie Web**.
...
1. Скопируйте этот CSV-файл в учетную запись хранения Azure, подключенную к кластеру HDInsight, и разместите в папке `/example/data/flights`.
    ...
    1. От сеанса ssh используйте команду HDFS для копирования файла из локального хранилища головного узла в Хранилище Azure.
...
2. В списке служб выберите **Hive**.
    ...
    ![Список услуг Apache Ambari, выбрав Hive](./media/hdinsight-operationalize-data-pipeline/hdi-ambari-services-hive.png)

3. Выберите параметр **Перейти к представлению** рядом с меткой Hive View 2.0.

    ![Амбари Апач Хив](./media/hdinsight-operationalize-data-pipeline/hdi-ambari-services-hive-summary.png)
...
5. Нажмите кнопку **Выполнить**, чтобы создать таблицу.
    ...
    ![hdi ambari услуги улья запрос](./media/hdinsight-operationalize-data-pipeline/hdi-ambari-services-hive-query.png)

6. Чтобы создать таблицу `flights`, замените текст в текстовом поле запроса инструкциями ниже. Таблица `flights` представляет собой таблицу, управляемую Hive, в которую загружаются данные разделов по году, месяцу и дню месяца. Эта таблица будет содержать данные всех рейсов с наименьшей степенью детализации в исходных данных одной записи за вылет.
...
1. С локальной рабочей станции `job.properties`создайте файл под названием . Используйте текст ниже в качестве исходного содержимого файла.
Затем обновите значения для конкретной среды. В таблице ниже текста обобщается каждое из свойств и указывается, где можно найти значения для вашей собственной среды.
...
1. С локальной рабочей станции `hive-load-flights-partition.hql`создайте файл под названием . Используйте приведенный ниже код в качестве содержимого файла.
    ...
    Переменные Oozie используют синтаксис `${variableName}`. Эти переменные устанавливаются `job.properties` в файле. Oozie подставляет фактические значения во время выполнения.

1. С локальной рабочей станции `hive-create-daily-summary-table.hql`создайте файл под названием . Используйте приведенный ниже код в качестве содержимого файла.
...
1. С локальной рабочей станции `workflow.xml`создайте файл под названием . Используйте приведенный ниже код в качестве содержимого файла. Эти приведенные выше действия выражены как отдельные действия в файле рабочего процесса Oozie.
...
Два запроса Hive доступны по пути в Хранилище Azure, а остальные переменные значения предоставляются `job.properties` файлом. Этот файл настраивает рабочий процесс для выполнения даты 3 января 2017 года.

## <a name="deploy-and-run-the-oozie-workflow"></a>Развертывание и запуск рабочего процесса Oozie

Используйте SCP из сеанса Bash для`workflow.xml`развертывания рабочего процесса`hive-load-flights-partition.hql` Oozie (), запросов Hive (и), `hive-create-daily-summary-table.hql`а также конфигурации заданий ().`job.properties`  В Oozie в локальном хранилище головного узла может располагаться только файл `job.properties`. Остальные файлы должны храниться в HDFS, в этом случае — в службе хранилища Azure. Действие Sqoop, используемое рабочим процессом, зависит от драйвера JDBC для обмена данными с базой данных SQL, которую нужно скопировать из головного узла в HDFS.

1. Создайте подпапку `load_flights_by_day` ниже пути пользователя в локальном хранилище головного узла. Из открытой сессии ssh выполните следующую команду:

    ```bash
    mkdir load_flights_by_day
    ```

1. Скопируйте все файлы в текущем каталоге (файлы `workflow.xml` и `job.properties`) в подпапку `load_flights_by_day`. С локальной рабочей станции выполните следующую команду:

    ```cmd
    scp ./* sshuser@CLUSTERNAME-ssh.azurehdinsight.net:load_flights_by_day
    ```

1. Скопируйте файлы рабочих процессов в HDFS. Из открытой сессии ssh выполните следующие команды:
...
1. Копирование `mssql-jdbc-7.0.0.jre8.jar` из локального головного узла в папку рабочего процесса в HDFS. Пересмотрите команду по мере необходимости, если кластер содержит другой файл банки. Пересмотрите `workflow.xml` по мере необходимости, чтобы отразить другой файл банки. Из открытой сессии ssh выполните следующую команду:

    ```bash
    hdfs dfs -put /usr/share/java/sqljdbc_7.0/enu/mssql-jdbc*.jar /oozie/load_flights_by_day
    ```

1. Запустите рабочий процесс. Из открытой сессии ssh выполните следующую команду:
...
1. Проследите его состояние с помощью веб-консоли Oozie. В Ambari выберите **Oozie**, **Быстрые ссылки**, **Oozie Web Console** (Веб-консоль Oozie). На вкладке **заданий рабочего процесса** выберите **Все задания**.
    ...
    ![hdi oozie веб-консоли рабочие процессы](./media/hdinsight-operationalize-data-pipeline/hdi-oozie-web-console-workflows.png)

1. Когда статус SUCCEEDED, задавайте запрос в таблицу базы данных S'L для просмотра вставленных строк. С помощью портала Azure перейдите в область базы данных SQL, нажмите кнопку **Инструменты** и откройте **редактор запросов**.
...
* Точка 2. В рамках диапазона дат рабочего процесса элемент `dataset` указывает, где в HDFS искать данные для определенного диапазона дат, и настраивает способ определения готовности данных к обработке в Oozie.
    ...
    Путь к данным в HDFS создается динамически в соответствии с выражением в элементе `uri-template`. В этом координаторе для набора данных также используется частота в один день. Тогда как начальная и конечная даты в элементе координатора управляют временем планирования запуска действий (и определяют их номинальное время), `initial-instance` и `frequency` в наборе данных управляют расчетом даты, используемой в создании `uri-template`. В этом случае значение для исходного экземпляра рекомендуется задать в один день, а затем запустить координатор, чтобы обеспечить сбор данных в соответствии с указанным интервалом (1 января 2017 года). Расчет даты набора данных переносится `initial-instance` на значение (12/31/2016), продвигающегося с шагом частоты набора данных (один день), пока не найдирует самую позднюю дату, которая не проходит номинальное время, установленное координатором (2017-01-01T00:00 GMT для первого действия).

    Пустой элемент `done-flag` указывает, что когда Oozie проверяет наличие входных данных в назначенное время, Oozie выполняет поиск данных, доступность которых определяется наличием каталога или файла. В этом случае, это наличие файла csv. Если CSV-файл имеется, Oozie предполагает, что данные готовы и запускает экземпляр рабочего процесса для обработки файла. Если нет файла csv, Oozie предполагает, что данные еще не готовы, и что запуск рабочего процесса переходит в состояние ожидания.
