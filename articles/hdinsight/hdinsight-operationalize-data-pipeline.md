...
1. Используйте [Редактор запросов](../sql-database/sql-database-single-database-get-started.md#query-the-database) для выполнения следующих инструкций SQL, чтобы создать `dailyflights` таблицу, в которой будут храниться обобщенные данные из каждого выполнения конвейера.
...
1. Скопируйте `mssql-jdbc-7.0.0.jre8.jar` из локального головного узла в папку рабочего процесса в HDFS. При необходимости измените команду, если кластер содержит другой JAR-файл. Внесите необходимые `workflow.xml` изменения, чтобы отразить другой JAR-файл. В открытом сеансе SSH выполните следующую команду:
