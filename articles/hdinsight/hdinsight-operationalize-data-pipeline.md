...
1. Создайте Базу данных SQL Azure. См. статью [Создание базы данных SQL Azure на портал Azure](../sql-database/sql-database-single-database-get-started.md).

1. Чтобы обеспечить доступ кластера HDInsight к подключенной базе данных SQL Azure, настройте правила брандмауэра базы данных SQL Azure, чтобы предоставить службам и ресурсам Azure доступ к серверу. Этот параметр можно включить в портал Azure, выбрав **задать брандмауэр сервера**и выбрав **в** списке **Разрешить службам и ресурсам Azure доступ к серверу** для сервера базы данных SQL Azure или базы данных. Дополнительные сведения см. в разделе о [создании правил брандмауэра для IP-адресов и управление ими](../sql-database/sql-database-firewall-configure.md#use-the-azure-portal-to-manage-server-level-ip-firewall-rules).

1. Используйте [Редактор запросов](../sql-database/sql-database-single-database-get-started.md#query-the-database) для выполнения следующих инструкций SQL, чтобы создать `dailyflights` таблицу, в которой будут храниться сводные данные каждого запуска конвейера.
...
1. Чтобы получить доступ к **веб-консоли Oozie** из Ambari, перейдите к разделу **Oozie** > **быстрые ссылки** > [Active Server] > **веб-интерфейс Oozie**.
...
1. Войдите в Ambari, перейдя по адресу `http://headnodehost:8080`.
...
6. Чтобы создать таблицу `flights`, замените текст в текстовом поле запроса инструкциями ниже. `flights`ная таблица представляет собой управляемую Hive таблицу, которая разделяет данные, загруженные в него, по годам, месяцам и дням месяца. Эта таблица будет содержать данные всех рейсов с наименьшей степенью детализации в исходных данных одной записи за вылет.
...
Затем обновите значения для конкретной среды. В таблице под текстом перечислены все свойства и указано, где можно найти значения для своей среды.
    ...
    | год | Компонент года дня, для которого вычисляются суммарные результаты данных о рейсах. Оставьте значение по умолчанию. |
...
1. На локальной рабочей станции создайте файл с именем `hive-load-flights-partition.hql`. Используйте приведенный ниже код в качестве содержимого для файла.
    ...
    Переменные Oozie используют синтаксис `${variableName}`. Эти переменные задаются в файле `job.properties`. Oozie подставляет фактические значения во время выполнения.
...
Доступ к двум запросам Hive осуществляется по пути в службе хранилища Azure, а остальные значения переменных предоставляются файлом `job.properties`. Этот файл настраивает рабочий процесс для выполнения в течение 3 января 2017 года.
...
1. Скопируйте `mssql-jdbc-7.0.0.jre8.jar` из локального головного узла в папку рабочего процесса в HDFS. При необходимости измените команду, если кластер содержит другой JAR-файл. При необходимости измените `workflow.xml`, чтобы отразить другой JAR-файл. В открытом сеансе SSH выполните следующую команду:
