...
## <a name="create-an-hdinsight-cluster-with-data-lake-storage-gen2"></a>Создание кластера HDInsight с Gen2 хранилища Озера данных
...
1. В учетной записи хранения назначьте управляемое удостоверение пользователю с ролью **Владелец для данных больших двоичных объектов хранилища (предварительная версия)**. Дополнительные сведения см. в разделе [Управление правами доступа к данным в BLOB-объектах и очередях Azure с помощью RBAC (предварительная версия)](../storage/common/storage-auth-aad-rbac.md).
    ...
    1. Щелкните **Сохранить**. Назначаемое пользователем удостоверение, которое вы выбрали, теперь отображается в роли **участника**.
...
### <a name="use-an-azure-resource-manager-template-deployed-with-the-azure-cli"></a>Использование шаблона Azure Resource Manager, развернутых с помощью Azure CLI
