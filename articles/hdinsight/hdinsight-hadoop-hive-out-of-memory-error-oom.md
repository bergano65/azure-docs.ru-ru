...
* T1 является псевдонимом большой таблицы, TABLE1, которая имеет много типов столбцов STRING.
* Другие таблицы не настолько велики, но имеют много столбцов.
...
"Когда hive.auto.convert.join.noconditionaltask - правда, мы проверяем noconditionaltask.size, и если сумма размеров таблиц в соединении карты меньше, чем noconditionaltask.size план будет генерировать соединение Карты, проблема с этим заключается в том, что расчет не принимается во внимание накладные расходы, введенные различными реализациями HashTable в качестве результатов, если сумма входных размеров меньше, чем размер noconditionaltask, по небольшой марже запросов ударит OOM ".
...
Скорее всего, соединение карты стало причиной ошибки в памяти Java Heap Space. Как описано в записи блога [Hadoop Yarn memory settings in HDInsight](https://blogs.msdn.com/b/shanyu/archive/2014/07/31/hadoop-yarn-memory-settings-in-hdinsigh.aspx) (Параметры памяти Hadoop Yarn в HDInsight), при использовании модуля Tez используемое пространство кучи на самом деле принадлежит контейнеру Tez. Описание памяти контейнера Tez см. на рисунке ниже.
