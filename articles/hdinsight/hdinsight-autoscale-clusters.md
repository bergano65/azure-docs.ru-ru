...
> [!Important]
> Функция автоматического масштабирования работает только для кластеров Spark, Hive и MapReduce, созданных после 8 мая 2019 г. 
...
### <a name="choosing-load-based-or-schedule-based-scaling"></a>Выбор на основе нагрузки или расписанию масштабирование

Прежде чем решение, какой режим следует выбрать, учитывайте следующие факторы:

* Загрузить отклонение: нагрузка кластера выполните согласованный шаблон в определенное время, в определенные дни. В противном случае планирования на основе нагрузки является лучшим вариантом.
* Требования соглашения об уровне ОБСЛУЖИВАНИЯ: Масштабирование автомасштабирования реактивного вместо прогнозирования. Будет ли достаточно задержку между при запуске нагрузки увеличение и когда должен быть размер целевого кластера? Если существуют строгие требования соглашения об уровне ОБСЛУЖИВАНИЯ, а нагрузка фиксированный шаблон известных, «по расписанию» является лучшим вариантом.

### <a name="consider-the-latency-of-scale-up-or-scale-down-operations"></a>Рассмотрим задержку масштабирования вверх и уменьшать операций

Может занять 10 – 20 минут для завершения операции масштабирования. При задании настраиваемого расписания, спланируйте этой задержки. Например если требуется, чтобы размер кластера равным 20, в 9:00, присвоено триггер расписания более раннее время, например 8:30 по таким образом, с 9:00 до завершения операции масштабирования.

### <a name="preparation-for-scaling-down"></a>Подготовка для уменьшения масштаба

Во время процесса масштабирование кластера, автомасштабирования будет вывод узлов из эксплуатации в соответствии с целевой размер. Если на этих узлах выполняются задачи, автомасштабирования будет ждать завершения задач. Так как каждый рабочий узел также выполняет роль в HDFS, временные данные будут смещены на оставшихся узлах. Поэтому следует убедиться, что имеется достаточно места на оставшихся узлах, чтобы разместить все временные данные. 

Запущенные задания будут продолжать запуска и окончания. Ожидающие задания будут ожидать размещения как обычно с меньшим числом доступных рабочих узлов.
...
|---|---|
| Выполнение | Кластер работает нормально. Все предыдущие действия автомасштабирования успешно завершена. |
