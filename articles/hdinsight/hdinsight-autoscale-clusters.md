...
При использовании кластеров LLAP с включенным автомасштабированием событие автомасштабирования с увеличением и уменьшением также увеличивает или уменьшает число управляющих LLAP до количества активных рабочих узлов. Но это изменение в количестве управляющих программ не сохраняется в **num_llap_nodes** config в Ambari. Если службы Hive перезапускаются вручную, то число управляющих программ LLAP будет сброшено согласно конфигурации в Ambari.

Возьмем следующий сценарий:
1. Кластер с включенным автомасштабированием LLAP создается с 3 рабочими узлами, а автомасштабирование на основе загрузки включается с минимальными рабочими узлами (3 и максимум рабочих узлов), равной 10.
2. Конфигурация счетчика управляющих программ LLAP в соответствии с конфигурацией LLAP, а Ambari — 3, так как кластер создан с 3 рабочими узлами.
3. После этого запускается Автомасштабирование из-за нагрузки на кластер. Теперь кластер масштабируется до 10 узлов.
4. Проверка автомасштабирования, выполняемая с регулярным интервалом, заметит, что число управляющих программ LLAP равно 3, но число активных рабочих узлов равно 10, процесс автомасштабирования теперь увеличит число управляющих объектов LLAP до 10, но это изменение не сохраняется в Ambari config-num_llap_nodes.
5. Автомасштабирование теперь отключено.
6. Кластер теперь содержит 10 рабочих узлов и 10 управляющих LLAP.
7. Служба LLAP перезапускается вручную.
8. Во время перезагрузки он проверяет файл конфигурации num_llap_nodes в конфигурации LLAP и записывает значение 3, что позволяет запустить 3 экземпляра управляющих программ, но число рабочих узлов равно 10. Теперь существует несоответствие между ними.

В этом случае необходимо вручную изменить **конфигурацию num_llap_node (число узлов для запуска управляющей программы Hive llap) в разделе расширенный куст-Interactive-env** в соответствии с текущим числом активных рабочих узлов.

**Примечание**.

События автомасштабирования не изменяют **Максимальное число одновременных запросов** в конфигурации Hive в Ambari. Это означает, что интерактивная служба Hive Server 2 **может обслуживать только заданное количество параллельных запросов в любой момент времени, даже если счетчик управляющих программ LLAP масштабируется вверх и вниз в зависимости от нагрузки и расписаний**. Общая рекомендация заключается в том, чтобы задать эту конфигурацию для сценария пикового использования, чтобы можно было избежать ручного вмешательства. Однако следует учитывать, что **Установка большого значения для параметра Максимальное число одновременных запросов может привести к сбою при перезапуске службы Hive Server 2, если минимальное количество рабочих узлов не может соответствовать заданному числу Tez AMS (равной конфигурации максимального количества одновременных запросов)** .
