...
3. Чтобы запустить задание Pig, использующее приложение .NET Framework, введите следующую команду.
    ...
    `DEFINE` Инструкция создает псевдоним `streamer` для приложения *PigUDF. exe* и `CACHE` загружает его из хранилища по умолчанию для кластера. `streamer` Позже используется с `STREAM` оператором для обработки отдельных строк, содержащихся в `LOG` , и возврата данных в виде последовательности столбцов.

    > [!NOTE]
    > Имя приложения, используемое для потоковой передачи, должно быть заключено \` в escape-символ (Обратная кавычка) при использовании псевдонима, а также символом "( `SHIP`одинарная цитата), если используется с.
