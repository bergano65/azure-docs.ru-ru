---
title: Миграция локальных кластеров Apache Hadoop в HDInsight Azure — рекомендации по использованию хранилища
...
## <a name="choose-the-right-storage-system-for-hdinsight-clusters"></a>Выбор правильной системы хранения для кластеров HDInsight
...
### <a name="azure-data-lake-storage-gen1"></a>Хранилище Azure Data Lake Gen1
...
### <a name="azure-data-lake-storage-gen2-preview"></a>Azure Data Lake Storage 2-го поколения (предварительная версия)

Azure Data Lake Storage 2-го поколения — это последнее предложение хранилища, которое пребывает на этапе предварительной версии на момент написания настоящего документа. Оно объединяет основные возможности Azure Data Lake Storage первого поколения с конечной точкой файловой системы, совместимой с Hadoop, непосредственно интегрированной в хранилище BLOB-объектов Azure. Это усовершенствование сочетает в себе преимущества масштабирования и экономии хранения объектов с высокой надежностью и производительностью, которые обычно связаны только с локальными файловыми системами.
...
- **Экономичность.** Data Lake Storage Gen2 отличается низкой стоимостью приобретения емкости хранилища и выполнения транзакций. За счет таких встроенных функций, как  [Жизненный цикл хранилища BLOB-объектов Azure](../../storage/common/storage-lifecycle-managment-concepts.md), в ходе жизненного цикла данных тарифные ставки максимально снижаются.
...
- [Общие сведения о хранилище Data Lake Storage 2-го поколения (предварительная версия)](../../storage/data-lake-storage/introduction.md)
...
## <a name="protect-azure-storage-key-visibility-within-the-on-premises-hadoop-cluster-configuration"></a>Защита видимости ключа к службе хранилища Azure в локальной конфигурации кластера Hadoop
...
## <a name="restrict-access-to-azure-storage-data-using-sas-signatures"></a>Ограничение доступа к данным службы хранилища Azure с помощью подписанных URL-адресов
...
1. Откройте файл [SASToken.py](https://github.com/Azure-Samples/hdinsight-dotnet-python-azure-storage-shared-access-signature/blob/master/Python/SASToken.py) и измените следующие значения:
    ...
    - storage_account_key: ключ вашей учетной записи хранения.
    - storage_container_name: контейнер в учетной записи хранения, доступ к которому необходимо ограничить.
    - example_file_path: путь к файлу, который передается в контейнер.
...
6. Используйте следующие значения для полей  **Key** (Ключ) и **Value** (Значение) :
...
7. Нажмите кнопку  **Add** (Добавить), чтобы сохранить этот ключ и значение, а затем кнопку  **Save** (Сохранить), чтобы сохранить изменения в конфигурации. При появлении запроса добавьте описание внесенного изменения (например, "Добавление доступа к хранилищу SAS") и нажмите кнопку  **Save** (Сохранить).
...
- [Хранилище, избыточное в пределах зоны (ZRS)](../../storage/common/storage-redundancy-zrs.md)
...
## <a name="attach-additional-azure-storage-accounts-to-the-cluster"></a>Подключение дополнительных учетных записей хранения Azure к кластеру
...
- [Подключение дополнительных учетных записей хранения Azure к кластеру](https://blogs.msdn.microsoft.com/ashish/2016/08/25/hdinsight-attach-additional-azure-storage-accounts/)
