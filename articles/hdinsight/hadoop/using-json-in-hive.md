...
![Apache Hive получить объект Json UDF](./media/using-json-in-hive/hdinsight-get-json-object.png)
...
Напоследок следует заметить, что тип оператора JSON в Hive, который нужно выбрать, зависит от ситуации. Если у вас есть простой документ JSON и у вас есть только одно поле, чтобы посмотреть вверх, вы можете использовать Get_json_object Ude **UDF.** Если у вас есть более чем один ключ, чтобы посмотреть на, то вы можете использовать **json_tuple**. Если у вас есть вложенный документ, то вы должны использовать **JSON SerDe**.
