...
* `STORED AS TEXTFILE LOCATION`. Сообщает Hive, где хранятся данные (каталог `example/data`) и хранится в виде текста. Данные могут храниться в одном файле или быть распределенными по нескольким файлам в каталоге.
