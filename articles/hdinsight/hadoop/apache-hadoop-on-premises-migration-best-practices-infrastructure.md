---
title: 'Инфраструктура: На территории Apache Hadoop к Azure HDInsight'
...
Начальное количество рабочих узлов можно определить с помощью смоделированных рабочих нагрузок. Кластер можно масштабировать позже, добавляя больше рабочих узлов для соответствия потребностям пиковых нагрузок. Кластер позже может быть уменьшен, когда дополнительные узлы рабочего не требуются.

Для получения дополнительной информации см. [Capacity planning for HDInsight clusters](../hdinsight-capacity-planning.md)
...
Каждая версия HDInsight представляет собой облачное распределение набора компонентов эко-системы Hadoop. Дополнительные сведения о компонентах HDInsight и их текущих версиях см. в статье [Что представляют собой различные компоненты Hadoop, доступные в HDInsight](../hdinsight-component-versioning.md).
...
|NiFi|IaaS 
|Presto|Узлы края IaaS или HDInsight
|Python 2;|PaaS 
|Python 3|PaaS 
|R|PaaS 
|SAS|IaaS 
...
|Tableau|IaaS 
|Waterline|Узлы края HDInsight
|Наборы потоков|Край HDInsight 
|Palantir|IaaS 
|Sailpoint|IaaS 
...
Изменения в конфигурациях в файлах конфигурации, таких как `core-site.xml`, `hive-site.xml` и `oozie-env.xml`, можно сделать с помощью начальной загрузки. Следующий скрипт является примером использования [модуля](https://docs.microsoft.com/powershell/azure/new-azureps-module-az) Powershell АЗ cmdlet [New-AzHDInsightClusterConfig](https://docs.microsoft.com/powershell/module/az.hdinsight/new-azhdinsightcluster):
...
Дополнительные сведения см. в статье [Настройка кластеров HDInsight с помощью начальной загрузки](../hdinsight-hadoop-customize-cluster-bootstrap.md).  Смотрите также, [Управляйте кластерами HDInsight с помощью Apache Ambari REST API.](../hdinsight-hadoop-manage-ambari-rest-api.md)
...
* [Масштабируйте кластеры HDInsight.](../hdinsight-scaling-best-practices.md)
...
HDInsight можно добавить в новую или существующую виртуальную сеть Azure. Если HDInsight добавляется в существующую виртуальную сеть, необходимо обновить существующие группы безопасности сети и определенные пользователем маршруты, чтобы обеспечить неограниченный доступ к [нескольким IP-адресам](../hdinsight-management-ip-addresses.md) в центре обработки данных Azure. Кроме того, убедитесь, что не блокировать трафик в [порты](../hdinsight-plan-virtual-network-deployment.md#hdinsight-ports), которые используются службами HDInsight.
...
HDInsight поддерживает [виртуальные конечные точки сетевого обслуживания,](../../virtual-network/virtual-network-service-endpoints-overview.md)которые позволяют безопасно подключаться к базам данных Azure Blob Storage, Azure Data Lake Storage Gen2, Cosmos DB и S'L. Включив конечную точку службы для Azure HDInsight, вы обеспечите защищенный маршрут трафика из центра обработки данных Azure. Этот повышенный уровень безопасности на сетевом уровне позволяет заблокировать большие учетные записи хранилища данных для их конкретных виртуальных сетей (VNET) и по-прежнему использовать кластеры HDInsight для доступа и обработки этих данных.

Дополнительные сведения см. в следующих статьях:

- [Конечные точки виртуального сетевого обслуживания](../../virtual-network/virtual-network-service-endpoints-overview.md)
...
Прочитайте следующую статью в этой серии: [Хранение рекомендаций для на местах для миграции Azure HDInsight Hadoop](apache-hadoop-on-premises-migration-best-practices-storage.md).
