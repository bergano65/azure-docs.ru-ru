...
Каждая версия HDInsight является облачным распределением набора компонентов Hadoop корректно. Дополнительные сведения о компонентах HDInsight и их текущих версиях см. в статье [Что представляют собой различные компоненты Hadoop, доступные в HDInsight](../hdinsight-component-versioning.md).
