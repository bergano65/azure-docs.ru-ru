...
|Пример |Описание |
|---|---|
|агрегированный счет слов|Подсчитывает слова в файлах ввода.|
|aggregatewordhist|Вычисляет гистограмму слов в файлах ввода.|
|Bbp|Использует Бейли-Борвейн-Плуфф для вычисления точных цифр Пи.|
|dbcount|Подсчитывает журналы просмотра страниц, хранящиеся в базе данных.|
|distbbp|Использует формулу типа BBP для вычисления точных битов Pi.|
|grep|Подсчитывает совпадения регеса в входе.|
|join|Выполняет соединение над отсортированными, одинаково разделенными наборами данных.|
|multifilewc|Подсчитывает слова из нескольких файлов.|
|пентомино|Программа укладки плитки, чтобы найти решения проблем pentomino.|
|pi|Оценки Пи с помощью квази-Монте-Карло метод.|
|случайнотекстера|Записывает 10 ГБ случайных текстовых данных на узла.|
|случайного писателя|Записывает 10 ГБ случайных данных на узла.|
|вторичныйсорт|Определяет вторичный сорт к фазе уменьшения.|
|sort|Сортирует данные, написанные случайным писателем.|
|Судоку|программа решения судоку.|
|тераген|создание данных для программы TeraSort.|
|terasort|выполнение программы TeraSort.|
|teravalidate|проверка результатов выполнения программы TeraSort.|
|счет слов|Подсчитывает слова в файлах ввода.|
|wordmean|Подсчитывает среднюю длину слов в файлах ввода.|
|wordmedian|Подсчитывает среднюю длину слов в файлах ввода.|
|wordstandarddeviation|Подсчитывает стандартное отклонение длины слов в файлах ввода.|

## <a name="run-the-wordcount-example"></a>Запуск примера для подсчета слов

1. Подключитесь к HDInsight с помощью протокола SSH. Замените `CLUSTER` имя кластера, а затем введите следующую команду:

    ```cmd
    ssh sshuser@CLUSTER-ssh.azurehdinsight.net
    ```

2. Из сеанса SSH используйте следующую команду, чтобы перечислить образцы:
...
4. Используйте следующие, чтобы подсчитать все слова в тетрадях Леонардо да Винчи, которые предоставляются в качестве выборочных данных с вашим кластером:
...
## <a name="10-gb-graysort-example"></a>10 ГБ Серого Сорта пример
...
В этом примере используется небольшой объем данных, 10 ГБ, чтобы можно было выполнить сортировку достаточно быстро. Используются приложения MapReduce, разработанные Оуэном О'Мэлли (Owen O'Malley) и Аруном Мёрти (Arun Murthy). Эти заявки выиграли ежегодный общий ("Daytona") терабайт сортировки ориентир в 2009 году, со скоростью 0,578 ТБ / мин (100 ТБ в 173 минут). Для получения дополнительной информации об этом и других сортировки критериев, [см.](https://sortbenchmark.org/)
