...
Значение по `alert_grace_period` умолчанию — 5 секунд. Этот параметр можно настроить в/ЕТК/Амбари-ажент/конф/Амбари-ажент.ини.. Для узлов, на которых просроченные оповещения происходят через регулярные интервалы, попробуйте увеличить значение до 10. Затем перезапустите агент Ambari.
...
* [@AzureSupport](https://twitter.com/azuresupport) Подключение к Twitter. Это официальная учетная запись Microsoft Azure для улучшения качества взаимодействия с клиентами. Он подключает сообщество Azure к нужным ресурсам: ответы, поддержка и эксперты.
