---
title: 'Миграция данных: На территории Apache Hadoop к Azure HDInsight'
...
* Передача данных по сети с помощью протокола TLS
    * Интернет. Данные можно перенести в службу хранилища Azure, используя обычное интернет-подключение и любое из средств, таких как Обозреватель службы хранилища Azure, AzCopy, Azure Powershell и Azure CLI. Для получения дополнительной информации [см. Перемещение данных в и из хранилища Azure](../../storage/common/storage-moving-data.md).

    * ExpressRoute. ExpressRoute — это служба Azure, которая позволяет создавать частные подключения между центрами обработки данных Microsoft и инфраструктурой вашей локальной среды или среды для совместной работы. Соединения ExpressRoute не проходят через общедоступный Интернет, и предлагают более высокую безопасность, надежность и скорость с более низкими опозданием, чем типичные соединения через Интернет. Дополнительные сведения см. в руководстве по [созданию и изменению канала ExpressRoute](../../expressroute/expressroute-howto-circuit-portal-resource-manager.md).
...
|Количество данных|Пропускная способность сети||||
|---|---|---|---|---|
...
* Наименьшая степень детализации DistCp — один файл. Указать количество Mappers больше, чем количество исходных файлов не поможет и будет тратить доступные ресурсы кластера.
...
1. Создайте Hive DDLs из помещения Hive metastore. Это действие можно выполнить, используя [оболочку скрипта Bash](https://github.com/hdinsight/hdinsight.github.io/blob/master/hive/hive-export-import-metastore.md).
...
- Преобразование на конкретных HDFS-пути на основе помещений на WASB/ADLS с помощью инструмента, такого как XSLT.
