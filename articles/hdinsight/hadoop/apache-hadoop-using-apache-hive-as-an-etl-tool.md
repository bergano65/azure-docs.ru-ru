...
* Магазин документов JSON, такой как [Azure Cosmos DB.](https://azure.microsoft.com/services/cosmos-db/)
...
* Для составления регулярно обновляемых отчетов и визуализаций. Например, если создание отчета занимает слишком много времени и не может выполняться в течение дня, вы можете генерировать его по расписанию в ночное время. Чтобы автоматически запускать запрос Hive, можно использовать [приложения Azure Logic Apps](../../logic-apps/logic-apps-overview.md) и PowerShell.

Если цель для данных не является базой данных, можно создать файл в соответствующем формате в запросе, например CSV. Затем этот файл можно затем в Microsoft Excel или Power BI.
