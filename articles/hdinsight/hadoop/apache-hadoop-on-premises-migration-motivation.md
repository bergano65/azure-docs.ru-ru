---
title: 'Преимущества: Мигрировать на месте Apache Hadoop в Azure HDInsight'
...
- **Глобальная доступность** - HDInsight доступен в [большем количестве регионов,](https://azure.microsoft.com/regions/services/) чем любое другое предложение аналитики больших данных. Служба Azure HDInsight также доступна в Azure для государственных организаций, Китая и Германии, что позволяет обеспечить соответствие требованиям организации в основных независимых регионах.
...
- **Упрощенное управление версиями** - Azure HDInsight управляет версией компонентов экосистемы Hadoop и поддерживает их в актуальном состоянии. Обновления программного обеспечения обычно представляют собой сложный процесс для локальных развертываний.

- **Меньшие кластеры, оптимизированные для конкретных рабочих нагрузок с меньшим количеством зависимостей между компонентами** - типичная настройка на месте Hadoop использует единый кластер, который служит многим целям. С помощью Azure HDInsight можно создавать кластеры, зависящие от рабочей нагрузки. Создание кластеров для конкретных рабочих нагрузок устраняет возрастающую сложность обслуживания одного кластера.
...
- **Простое управление, администрирование и мониторинг** - Azure HDInsight интегрируется с [журналами](../hdinsight-hadoop-oms-log-analytics-tutorial.md) Azure Monitor, чтобы обеспечить единый интерфейс, с помощью которого можно контролировать все кластеры.
...
### <a name="on-premises-deployment-questionnaire"></a>Вопросник о развертывании на территории
...
|Число кластеров|4|
|Количество мастер-узлов|2|
|Количество рабочих узлов|100|
|Количество краевых узлов| 5|
...
|Аварийное восстановление / резервное копирование|Нужно ли резервное копирование кластера?|  
...
|Количество метамагазинов Hive|2||
|Количество таблиц Hive|100||
|Количество полисов Рейнджера|20||
...
|Навыки|Hadoop, Spark||
|Количество имеющихся ресурсов для усилий по миграции|2||
|**Раздел**: **ограничения**|||
...
|Количество пользователей AD для синхронизации?|          100||
