...
- **Простота управления, администрирования и мониторинга** . Azure HDInsight интегрируется с [журналами Azure Monitor](../hdinsight-hadoop-oms-log-analytics-tutorial.md) TO предоставляет единый интерфейс, с помощью которого можно отслеживать все кластеры.
