...
* **Совместное возникновение**: Джо, Алиса, и Боб все *любили "Звездных войн",* *Империя наносит ответный удар*, , и *Возвращение джедаев*. Mahout определяет, что пользователям, которым нравится любой из этих фильмов, также нравятся другие два.
...
* **Рекомендация сходства**: Потому что Джо понравилось первые три фильма, Mahout смотрит на фильмы, которые другие с аналогичными предпочтениями любил, но Джо не смотрел (любил / рейтинг). В этом случае Mahout рекомендует посмотреть фильмы *Призрачная угроза*, *Атака клонов* и *Месть ситхов*.
...
Данные, `user-ratings.txt` содержащиеся в `userID` `movieID`имеет `userRating`структуру , , и , который `timestamp`показывает, насколько высоко каждый пользователь оценил фильм. Вот пример данных:
...
4. Запустите скрипт Python. Следующая команда предполагает, что вы находитесь в каталоге со всеми скачанными файлами.
   ...
   * Файл **moviedb.txt** используется для получения имен фильмов.

   * **Рекомендации.txt** используются для получения рекомендаций фильма для этого пользователя.
...
Задания Mahout не удаляют временные данные, создаваемые при обработке задания. В примере задания указан параметр `--tempDir` , чтобы выделить временные файлы в отдельный каталог для простоты последующего их удаления. Чтобы удалить временные файлы, используйте следующую команду:
...
Теперь, когда вы узнали, как использовать Mahout, откройте для себя другие способы работы с данными на HDInsight:
