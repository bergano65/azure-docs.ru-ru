...
1. После разработки, компиляции и тестирования программы MapReduce используйте команду `scp`, чтобы передать JAR-файл в головного узла.

    ```cmd
    scp mycustomprogram.jar sshuser@CLUSTERNAME-ssh.azurehdinsight.net
    ```

    Замените ИМЯ_КЛАСТЕРА именем кластера. Если для защиты учетной записи SSH использовался пароль, вам будет предложено ввести пароль. Если используется сертификат, может потребоваться использовать параметр `-i` , чтобы указать соответствующий файл закрытого ключа.
