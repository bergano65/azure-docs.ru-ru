...
2. Задайте для `JAVA_HOME` переменной среды каталог, содержащий Java.

3. Задайте переменную `PATH` среды, чтобы включить `%JAVA_HOME%\bin` каталог.
...
3. Откройте файл *Splitter.cs*. По умолчанию у него есть только один `Execute`метод:. `Execute` Метод вызывается, когда молния получает кортеж для обработки. Здесь можно считывать и обрабатывать входящие кортежи и создавать исходящие кортежи.
...
Откройте *Program.CS*. Важным методом является `GetTopologyBuilder`, который используется для определения топологии, которая отправляется в самое большое. Замените содержимое `GetTopologyBuilder` следующим кодом, чтобы реализовать топологию, описанную ранее.
...
* **Подтверждение** — каждый элемент bolt в топологии может вызвать `this.ctx.Ack(tuple)`, чтобы подтвердить успешную обработку кортежа. Когда все винты подтверждают кортеж, вызывается `Ack` метод spout. Метод `Ack` позволяет воронке удалить кэшированные данные для воспроизведения.
...
1. Откройте *Program.CS*и добавьте в `Main` метод следующий код:
