...
> ](./media/migrate-storm-to-spark/storm-migration-path.png) пути миграции ![HDInsight
...
> [!div class="mx-imgBorder"]
> ![ное взаимодействие компонентов](./media/migrate-storm-to-spark/apache-storm-components.png)
...
|Контролер|Прослушивает работу, назначенную компьютеру, и запускает и останавливает рабочие процессы на основе директив из Nimbus. Каждый рабочий процесс выполняет подмножество топологии. Логика приложения пользователя (выкрутка и молния) выполняется здесь.|
...
> [!div class="mx-imgBorder"]
> ![путь потоковой передачи Spark к выходным](./media/migrate-storm-to-spark/spark-streaming-to-output.png)
...
> [!div class="mx-imgBorder"]
> ![пакетах обработки потоковой передачи Spark](./media/migrate-storm-to-spark/spark-streaming-batches.png)
...
Время, когда данные обрабатываются из входной таблицы, контролируется с помощью интервала триггера. По умолчанию интервал триггера равен нулю, поэтому структурированная потоковая передача пытается обрабатывать данные по мере их поступления. На практике это означает, что как только структурированная потоковая передача завершает обработку выполнения предыдущего запроса, она запускает следующую обработку новых полученных данных. Можно настроить запуск триггера с интервалом, что позволит обрабатывать данные потоковой передачи в пакетах на основе времени.

> [!div class="mx-imgBorder"]
> ![обработки данных в структурированной потоковой передаче](./media/migrate-storm-to-spark/structured-streaming-data-processing.png)
...
1. **Перестает потребляться в старом кластере.** В существующем выходе, закройте использование данных из источника потоковых данных и дождитесь завершения записи данных в целевой приемник.
    ...
    > [!div class="mx-imgBorder"]
    > ![прерывать использование текущего кластера](./media/migrate-storm-to-spark/stop-consuming-current-cluster.png)
...
* [Общие сведения о Apache Spark структурированной потоковой передаче](../spark/apache-spark-structured-streaming-overview.md)