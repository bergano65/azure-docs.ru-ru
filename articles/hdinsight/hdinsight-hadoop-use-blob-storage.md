...
|----------------------|--------------------|-----------------------------|------------------------|
| StorageV2 (учетная запись общего назначения версии 2)  | BLOB-объекты     | Стандарт                    | Горячий, крутой, архивный \*   |
| Хранилище (общего назначения v1)   | BLOB-объекты     | Стандарт                    | Н/Д                    |
| блобстораже                    | BLOB-объекты     | Стандарт                    | Горячий, крутой, архивный \*   |
...
![Кластеры Hadoop используют API HDFS для доступа к данным и их хранения в хранилище BLOB-объектов](./media/hdinsight-hadoop-use-blob-storage/storage-architecture.png "Архитектура хранилища HDInsight")
...
* **Затраты на хранение данных.** Хранение данных в файловой системе DFS в долгосрочной перспективе является более затратным, чем хранение данных в службе хранилища Azure, так как стоимость вычислительного кластера превышает стоимость службы хранилища Azure. Кроме того, поскольку данные не должны перегружаться для каждого поколения вычислений, вы также экономите затраты на загрузку данных.
...
@No__t_0 идентифицирует имя контейнера больших двоичных объектов в службе хранилища Azure.
@No__t_0 идентифицирует имя учетной записи хранения Azure. Обязательно использовать полное доменное имя (FQDN).

Если не указаны ни `<BlobStorageContainerName>`, ни `<StorageAccountName>`, используется файловая система по умолчанию. Для файлов в файловой системе по умолчанию можно использовать относительный или абсолютный путь. Например, для ссылки на файл *hadoop-mapreduce-examples.jar* , который поставляется с кластерами HDInsight, можно использовать один из следующих вариантов:
...
Путь представляет собой имя пути HDFS файла или каталога. Так как контейнеры службы хранилища Azure содержат данные типа "ключ — значение", подлинная иерархическая файловая система в них отсутствует. Знак косой черты "/" в ключе BLOB-объекта интерпретируется как разделитель каталогов. Например, имя BLOB-объекта для файла *hadoop-mapreduce-examples.jar* выглядит следующим образом:
