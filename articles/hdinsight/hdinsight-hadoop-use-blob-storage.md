...
|----------------------|--------------------|-----------------------------|------------------------|
| StorageV2 (учетная запись общего назначения версии 2)  | Blob     | Стандарт                    | Горячий, холодное, архивная\*   |
| Хранилище (общего назначения v1)   | Blob     | Стандарт                    | Н/Д                    |
| блобстораже                    | Blob     | Стандарт                    | Горячий, холодное, архивная\*   |
...
Кроме того, HDInsight позволяет получить доступ к данным, содержащимся в службе хранилища Azure. Синтаксис выглядит следующим образом.
...
Учетные записи хранения, определенные в процессе создания и их ключи, хранятся `%HADOOP_HOME%/conf/core-site.xml` на узлах кластера. По умолчанию HDInsight будет использовать учетные записи хранения, описанные в файле core-site.xml. Этот параметр можно изменить с помощью [Apache Ambari](./hdinsight-hadoop-manage-ambari.md).
...
Если не указан `<StorageAccountName>` ни параметр, ни,используетсяфайловаясистемапоумолчанию.`<BlobStorageContainerName>` Для файлов в файловой системе по умолчанию можно использовать относительный или абсолютный путь. Например, для ссылки на файл *hadoop-mapreduce-examples.jar* , который поставляется с кластерами HDInsight, можно использовать один из следующих вариантов:
