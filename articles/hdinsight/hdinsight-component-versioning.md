...
| Apache Hive            | 3.1.0         | 2.1.0 (кластеры, отличные от ESP), 1.2.1 (кластеры ESP)                |
...
**Срок действия поддержки** означает, что корпорация Майкрософт больше не будет предоставлять поддержку для указанной версии HDInsight, и она больше не будет доступная через портал Azure для создания кластера. Тем не менее эти версии можно создавать с помощью Azure CLI или различных пакетов SDK. 

**Прекращение использования версии** HDInsight означает, что существующие кластеры будут продолжать работать как есть. Однако новые кластеры этой версии не могут быть созданы с помощью каких-либо средств (включая CLI и пакеты SDK). Другие функции плоскости управления (например, ручное масштабирование и автоматическое масштабирование) также могут не работать после выхода версии. Поддержка непредусмотренных версий недоступна.
