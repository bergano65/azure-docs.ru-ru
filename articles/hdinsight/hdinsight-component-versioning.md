...
| Apache Kafka | 1,1 |1.1, 1.0 * (см. примечание ниже) | 0.10.0 | 0.9.0 |-|-|-|-|
...
> [!NOTE]
> Из-за вопросы производительности системы поддержка Kafka версии 0.10 истек срок действия в марта 2019 г.
...
Ниже перечислены версии HDInsight, доступные в портале Azure, а также другие методы развертывания, например PowerShell и пакета SDK для .NET.
...
> [!NOTE]  
> По истечении срока поддержки для версии она может стать недоступной на портале Microsoft Azure. Тем не менее, версии кластера продолжают быть доступны с помощью `Version` параметр в Windows PowerShell [New AzHDInsightCluster](https://docs.microsoft.com/powershell/module/az.hdinsight/new-azhdinsightcluster) команда и пакета SDK .NET до даты вывода.
>

### <a name="retired-versions"></a>Устаревшие версии

В следующей таблице перечислены версии HDInsight, которые **не** доступны на портале Azure.
...
|Тип кластера|Hadoop|hbase|Interactive Query|Storm|Spark|ML Server|Kafka|
|---|---|---|---|---|---|---|---|
...
|||A8 v2||A4 v2|||D3 v2|
|||A2m v2||A8 v2|||E8 v3|
|Граничный узел: размер виртуальной машины по умолчанию||||||D4 v2||
|Граничный узел: рекомендуемый размер виртуальной машины||||||D4 v2||
|||||||D12 v2||
|||||||D13 v2||
|||||||D14 v2||
|||||||E16 v3||
