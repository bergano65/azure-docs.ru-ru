...
1. В строке меню перейдите к **файлу** > **Открыть папку...**  > **к:\хд\хдексампле**, а затем нажмите кнопку **выбрать папку** . Папка отобразится в представлении **обозревателя** слева.
...
3. Присвойте новому файлу имя, используя либо `.hql` (запросы Hive), либо расширение файла `.py` (Spark script). В этом примере используется файл **HelloWorld.hql**.

## <a name="set-the-azure-environment"></a>Настройка среды Azure

Для пользователя национального облака выполните следующие действия, чтобы сначала настроить среду Azure, а затем использовать **Azure: Чтобы войти в Azure, выполните команду @ no__t-0:

1. Перейдите в раздел **файл** > **настройки**@no__t-**3.**
2. Выполните поиск по следующей строке: **Общие сведения Cloud @ no__t-0.
...
1. В строке меню перейдите к **просмотру** > **Палитра команд...** и введите **Azure: Вход в @ no__t-0:
...
1. В строке меню перейдите к **просмотру** > **Палитра команд...** и введите **Spark/Hive: Связывание кластера**.
...
3. Введите универсальную конечную точку Livy. Например: HTTP @ no__t-0//10.172.41.42:18080.
...
1. В строке меню перейдите к **просмотру** > **Палитра команд...** и введите **Spark/Hive: List Cluster** (HDInsight: перечисление кластеров).
...
5. Щелкните правой кнопкой мыши редактор скриптов и выберите **Hive: Interactive @ no__t-0 для отправки запроса или используйте сочетание клавиш CTRL + ALT + I.  Выберите **Hive: Batch @ no__t-0 для отправки сценария или используйте сочетание клавиш CTRL + ALT + H.  
...
5. Выделите весь код, щелкните правой кнопкой мыши редактор скриптов и выберите **Spark: PySpark Interactive @ no__t-0, чтобы отправить запрос. Или используйте сочетание клавиш CTRL + ALT + I.
...
5. Щелкните правой кнопкой мыши редактор скриптов и выберите **Spark: PySpark Batch @ no__t-0 или используйте сочетание клавиш CTRL + ALT + H.
...
2. В поле **Поиск параметров** введите @no__t — отправка задания 1HDInsight: Livy Conf** (Отправка задания HDInsight: конфигурация Livy).  
...
3. В левой области разверните узел **AZURE: HDINSIGHT @ NO__T-0. В списке перечислены доступные подписки и кластеры.
...
В строке меню перейдите к**палитре команд** **View** > , а затем введите **Azure: Выйдите из функции @ no__t-0.
