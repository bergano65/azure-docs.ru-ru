...
description: Узнайте, как использовать Spark & Hive Tools (Azure HDInsight) для visual Studio Code. Используйте инструменты для создания и отправки запросов и скриптов.
...
Узнайте, как использовать Apache Spark & Hive Tools для визуального кода studio. Используйте инструменты для создания и отправки рабочих мест apache Hive, интерактивных запросов Hive и скриптов PySpark для Apache Spark. Сначала мы опишите, как установить Spark & Hive Tools в Visual Studio Code. Затем мы будем ходить через как представить рабочих мест Spark & Hive Tools.  

Spark & Hive Tools могут быть установлены на платформах, которые поддерживаются Visual Studio Code. Обратите внимание на следующие предпосылки для различных платформ.
...
1. Из меню бар, перейдите к **файл** > **уоткрытию Folder ...**  >  **C: HD-HDexample**, а затем выберите кнопку **Select Folder.** Папка отображается в представлении **проводника** слева.
...
Прежде чем отправить скрипты в кластеры из Code Studio Code, необходимо либо подключиться к учетной записи Azure, либо связать кластер. Используйте имя пользователя и учетные данные apache Ambari или учетную запись, связанную с доменом. Выполните следующие действия, чтобы подключиться к Azure:
...
Метод 2 Отправить файл и `.vscode` заметить, что папка автоматически добавляется в рабочую папку. Вы можете увидеть конфигурацию Livy, выбрав **.vscode'settings.json.**
...
- ПАНЕЛЬ MESSAGES
   ...
   2. Когда количество строк в таблице меньше или равно 100, вы видите следующее сообщение: "60 строк отображаются для таблицы Hive".
   3. Когда нет содержания в таблице, вы видите`0 rows are displayed for Hive table.`следующее сообщение: "
...
Выберите исследователь Azure HDInsight, чтобы расширить учетную запись Data Lake Storage Gen2. Вам предлагается ввести ключ доступа к хранилищам, если ваша учетная запись Azure не имеет доступа к хранению Gen2. После проверки ключа доступа учетная запись Data Lake Storage Gen2 автоматически расширяется.
...
Отправьте задание в кластер HDInsight с помощью Data Lake Storage Gen2. Вам предлагается ввести ключ доступа к хранилищам, если у учетной записи Azure нет доступа к записи к хранению Gen2. После проверки ключа доступа задание будет успешно отправлено.
