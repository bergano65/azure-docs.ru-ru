...
7. Используйте командную строку, чтобы выполнить команду **PIP Install NumPy = = 1.19.3**, а затем снова загрузите окно VSCode.

8. В строке меню последовательно выберите **Вид** > **Палитра команд...** или воспользуйтесь сочетанием клавиш **SHIFT+CTRL+P** и введите **Python: Select Interpreter to start Jupyter Server** (Выбрать интерпретатор для запуска сервера Jupyter).
...
9. Выберите параметр python ниже.

   ![выбор параметра ниже](./media/hdinsight-for-vscode/choose-the-below-option.png)
    
10. В строке меню последовательно выберите **Вид** > **Палитра команд...** или воспользуйтесь сочетанием клавиш **SHIFT+CTRL+P** и введите **Developer: Reload Window** (Разработчик: перезагрузить окно).

    ![перезагрузка окна](./media/hdinsight-for-vscode/reload-window.png)

11. [Подключитесь](#connect-to-an-azure-account) к учетной записи Azure или свяжите кластер, если вы еще не сделали этого.

12. Выделите весь код, щелкните правой кнопкой мыши редактор скриптов и выберите **Spark: PySpark Interactive/синапсе: PySpark Interactive** , чтобы отправить запрос. 

    ![контекстное меню интерактивной среды pyspark](./media/hdinsight-for-vscode/pyspark-interactive-right-click.png)

13. Выберите кластер, если вы не указали кластер по умолчанию. Через несколько секунд **Интерактивные результаты Python** отобразятся на новой вкладке. Щелкните PySpark, чтобы переключить ядро в **PySpark/синапсе PySpark**, и код будет выполнен успешно. Если вы хотите переключиться на ядро синапсе Pyspark, рекомендуется отключить автоматические параметры в портал Azure. В противном случае для активации кластера и установки ядра synapse при первом использовании может потребоваться много времени. Если средства также позволяют отправить блок кода вместо всего файла скрипта с помощью контекстного меню:
...
14. Введите **%% info**, а затем нажмите клавиши SHIFT + ВВОД, чтобы просмотреть сведения о задании (необязательно):
