...
Spark & Hive Tools можно установить на платформах, поддерживаемых Visual Studio Code, включая Windows, Linux и macOS. Обратите внимание на следующие компоненты для различных платформ.
...
1. В строке меню перейдите к **файлу** > **Открыть папку...**  > **к:\хд\хдексампле**, а затем нажмите кнопку **выбрать папку** . Папка отображается в представлении **проводника** слева.
...
3. Присвойте новому файлу имя, используя либо `.hql` (запросы Hive), либо расширение файла `.py` (скрипт Spark). В этом примере используется файл **HelloWorld.hql**.
...
1. Последовательно выберите **файл** > **Параметры** > **Параметры**.
...
1. В строке меню перейдите к **просмотру** > **Палитра команд...** и введите **Azure: вход**:
...
Вы можете связать нормальный кластер с помощью имени пользователя, управляемого [Apache Ambari](https://ambari.apache.org/), или связать свой корпоративный пакет безопасности в кластере Hadoop с помощью имени пользователя домена (например, `user1@contoso.com`).

1. В строке меню перейдите к **просмотру** > **Палитра команд...** и введите **Spark/Hive: Свяжите кластер**.
...
1. В строке меню перейдите к **просмотру** > **Палитра команд...** и введите **Spark/Hive: List Cluster**.
...
+ Поддерживаемые конфигурации Livy:
    ...
    **Опубликовать/батчес** Текст запроса
    ...
    | proxyUser | Пользователь, олицетворяемый при выполнении задания | String |
    | className | Класс main приложения в Java/Spark | String |
    ...
    | driverMemory | Объем памяти, используемый для процесса драйвера | String |
    | driverCores | Число ядер, используемых для процесса драйвера | Int |
    | executorMemory | Объем памяти, используемый для каждого процесса исполнителя | String |
    ...
    | state |Пакетное состояние | String |
...
- Панель сообщений
   ...
   2. Если число строк в таблице меньше или равно 100, вы увидите примерно следующее сообщение: "60 строки отображаются для таблицы Hive".
   3. Если в таблице нет содержимого, появится следующее сообщение: "0 строк отображается для таблицы Hive".
...
- **Автозавершение IntelliSense**. Всплывающие предложения для ключевых слов, методов, переменных и других программных элементов. Разные значки обозначают разные объекты:
...
- **Маркер ошибок IntelliSense**. Языковая служба подчеркивает редактирование ошибок в скрипте Hive.     
- **Выделение синтаксиса**. Языковая служба использует разные цвета для различения переменных, ключевых слов, типов данных, функций и других программных элементов.
...
> [!NOTE]
>
> Ключ доступа для учетной записи хранения можно получить из портал Azure. Дополнительные сведения см. в статье [Управление ключами доступа учетной записи хранения](../storage/common/storage-account-keys-manage.md).
