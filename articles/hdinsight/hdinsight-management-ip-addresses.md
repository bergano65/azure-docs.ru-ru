...
description: Узнайте, с каких IP-адресов необходимо разрешить входящий трафик, чтобы правильно настроить группы сетевой безопасности и маршруты, определяемые пользователем, для виртуальных сетей с Azure HDInsight.
...
> [!Important]
> В большинстве случаев теперь можно использовать функцию [тега обслуживания](hdinsight-service-tags.md) для групп сетевой безопасности, вместо того, чтобы вручную добавлять IP-адреса. Новые регионы будут добавлены только для тегов обслуживания, а статические IP-адреса в конечном итоге будут унижаться.

Если для управления входящим трафиком в кластер HDInsight используются группы сетевой безопасности (НСГ) или маршруты с определенными пользователями (UDR), необходимо убедиться, что кластер может общаться с критически важными службами работоспособности и управления Azure.  Некоторые IP-адреса этих служб являются специфическими для регионов, а некоторые — во всех регионах Azure. Если у вас нет пользовательской службы DNS, вам, возможно, нужно будет разрешить трафик из службы Azure DNS.

В следующих разделах обсуждаются конкретные IP-адреса, которые должны быть разрешены.

## <a name="azure-dns-service"></a>Служба Azure DNS

Если вы используете службу DNS, предоставляемую Azure, разрешите доступ с __168.63.129.16__ в порту 53. Для получения дополнительной информации смотрите [разрешение имен для вс-а и примеров ролей.](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md) Если вы используете пользовательские DNS, пропустите этот шаг.

## <a name="health-and-management-services-all-regions"></a>Услуги в области здравоохранения и управления: все регионы

Разрешить трафик со следующих IP-адресов для служб здоровья и управления Azure HDInsight, которые применяются ко всем регионам Azure:
...
## <a name="health-and-management-services-specific-regions"></a>Услуги в области здравоохранения и управления: конкретные регионы

Разрешить трафик с IP-адресов, перечисленных для служб ы и управления azure HDInsight, в определенном регионе Azure, где находятся ваши ресурсы:

> [!IMPORTANT]  
> Если область Azure, которую вы используете, не указана, используйте функцию [тега службы](hdinsight-service-tags.md) для групп сетевой безопасности.
...
Для получения дополнительной [информации](hdinsight-plan-virtual-network-deployment.md#networktraffic) см.

Если вы используете маршруты, определяемые пользователями (UDRs), следует указать маршрут и разрешить исходящий трафик из виртуальной сети в вышеуказанные I-е исправные приложения со следующим набором hop в "Интернет".

## <a name="next-steps"></a>Дальнейшие действия

* [Создание виртуальных сетей для кластеров Azure HDInsight](hdinsight-create-virtual-network.md)
* [Группа сетевой безопасности (NSG) теги служб для Azure HDInsight](hdinsight-service-tags.md)
