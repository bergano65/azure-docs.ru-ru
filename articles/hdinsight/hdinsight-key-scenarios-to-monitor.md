...
1. Просмотрите выходные данные и проверьте наличие больших файлов в папке `mnt` или других папках. Обычно папки `usercache`и `appcache` (МНТ/Resource/Hadoop/Yarn/Local/усеркаче/Hive/аппкаче/) содержат большие файлы.
...
1. Если эта команда указывает конкретное задание, можно завершить задание с помощью команды, которая выглядит следующим образом:
    ...
    Замените `application_id` ИДЕНТИФИКАТОРом приложения. Если конкретные задания не указаны, перейдите к следующему шагу.
