...
В некоторых случаях может возникнуть низкая производительность из-за нехватки дискового пространства в кластере. Изучите следующие шаги:
...
1. Просмотрите выходные данные и проверьте наличие больших файлов в `mnt` папке или других папках. Как правило, `usercache`папки, `appcache` и (МНТ/Resource/Hadoop/Yarn/Local/усеркаче/Hive/аппкаче/) содержат файлы большого размера.

1. При наличии больших файлов текущее задание приведет к увеличению размера файла или сбою предыдущего задания, которое могло быть связано с этой проблемой. Чтобы проверить, вызвано ли это поведение текущим заданием, выполните следующую команду:
...
1. Если эта команда указывает конкретное задание, можно завершить задание с помощью команды, которая выглядит следующим образом:
    ...
    Замените `application_id` на идентификатор приложения. Если конкретные задания не указаны, перейдите к следующему шагу.
