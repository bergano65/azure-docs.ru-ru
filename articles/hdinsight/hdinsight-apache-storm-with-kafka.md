...
* Опыт создания и развертывания решений Storm (топологий). В частности топологий, в которых используется платформа [Flux](https://storm.apache.org/releases/current/flux.html). Дополнительные сведения см. в статье [Создание топологии Apache Storm на языке Java](./storm/apache-storm-develop-java-topology.md).
