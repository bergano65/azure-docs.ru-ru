...
С помощью [портала Azure](https://portal.azure.com), вы можете управлять [Apache Hadoop](https://hadoop.apache.org/) кластеров в Azure HDInsight. Используйте другие вкладки выше, чтобы перейти к статьям об управлении кластерами Hadoop в HDInsight с помощью других инструментов.
...
**Меню слева:**  
  ...
  - **Меню "Мониторинг"**
    ...
    |metrics|Мониторинг метрик кластера в журналах Azure Monitor.|
...
|Конечная точка Private|Частную конечную точку для кластера.|
...
|Размеры узлов рабочей роли|Выбранный размер виртуальной машины для рабочих узлов.|
...
См. в разделе [кластеры HDInsight масштабирования](./hdinsight-scaling-best-practices.md) полные сведения.
...
4. Из **действия скрипта** выберите **отправить новое**.
5. Из **Отправка действия скрипта** введите следующие сведения:
...
**Размер кластера** плитку [кластера Домашняя страница](#homePage) отображает количество ядер, выделенных для этого кластера, и как они выделяются для узлов на этом кластере.
