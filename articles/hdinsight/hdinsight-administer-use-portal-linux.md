...
|Обновление|Обновляет представление.|
...
**Меню слева:**  
  ...
  - **Меню параметров**  
    ...
    |Элемент Property|Просмотр [свойств кластера](#properties).|
...
### <a name="change-the-ssh-user-password-or-public-key"></a>Изменение пароля пользователя SSH или открытого ключа
...
5. На странице **Отправка действия скрипта** введите следующие сведения.
   ...
   | Параметры |Введите имя пользователя SSH и новый пароль. Между именем пользователя и паролем должен быть один пробел. В паролях SSH не поддерживаются следующие символы: "' '/\ <% ~ | $ &
