...
Большинство заданий Hadoop представляют собой пакетные задания, выполняемые от случая к случаю. Для большинства кластеров Hadoop существует большой период времени, в течение которого кластер не используется для обработки. В HDInsight ваши данные хранятся в службе хранилища Azure, поэтому вы можете безопасно удалить кластер, когда он не используется.
Вы также платите за кластер HDInsight, даже если он не используется. Так как плата за кластеры много раз больше, чем плата за хранилище, имеет смысл удалить кластеры, если они не используются.
...
В этой статье вы узнали некоторые основные административные функции. Для получения дополнительных сведений ознакомьтесь со следующими статьями:
