...
description: Узнайте, как создавать и управлять кластерами Azure HDInsight с помощью портала Azure.
...
С помощью [портала Azure](https://portal.azure.com)можно управлять кластерами [Apache Hadoop](https://hadoop.apache.org/) в Azure HDInsight. Используйте другие вкладки выше, чтобы перейти к статьям об управлении кластерами Hadoop в HDInsight с помощью других инструментов.
...
Войти [https://portal.azure.com](https://portal.azure.com)на ввесенку .

## <a name="list-and-show-clusters"></a><a name="showClusters"></a>Список и показ кластеров
...
Выберите имя кластера на странице [**кластеров HDInsight.**](#showClusters)  Откроется представление **Обзор**, которое будет выглядеть следующим образом.
...
**Левое меню:**  
  ...
  - **Меню параметров**  
    ...
    |SSH - Кластерный логин|Отображение сведений о подключении к кластеру по протоколу SSH. Для получения дополнительной информации [см.](hdinsight-hadoop-linux-use-ssh-unix.md)|
    |Data Lake Storage 1-го поколения|Настройка доступа к Data Lake Storage 1-го поколения.  Дополнительные сведения см. в статье [Краткое руководство по установке кластеров в HDInsight](../storage/data-lake-storage/quickstart-create-connect-hdi-cluster.md).|
    ...
    |Внешние метамагазины|просмотр метахранилищ [Apache Hive](https://hive.apache.org/) и [Apache Oozie](https://oozie.apache.org/). Метахранилища можно настроить только во время создания кластера.|
  ...
  - **Меню "Мониторинг"**
    ...
    |Метрики|Мониторинг метрик кластеров в журналах Azure Monitor.|
    |Настройки диагностики|Настройки места хранения для метрик диагностики.|
...
|---|---|
|Узла|Имя кластера.|
|КЛАСТЕРНЫЙ URL|URL-адрес веб-интерфейса Ambari.|
|Частная конечная точка|Частная конечная точка для кластера.|
|Безопасная оболочка (SSH)|Имя пользователя и имя узла используются при доступе к кластеру по протоколу SSH.|
|Состояние|Один из: Прерванный, Принятый, ClusterStorageProvisioned, AzureVMConfiguration, HDInsightConfiguration, Оперативная, Бег, Ошибка, Удаление, Удаление, Удаление, Delete'd, DeleteTimedout, DeleteError, Patch'ud, CertRollover'ud, Resize'ud, или Кластернаяикикализация.|
...
|Виртуальная сеть|Название виртуальной сети, которую развертывает кластер, если он был выбран во время развертывания.|
...
Для получения полной информации можно ознакомиться с [кластерами Scale HDInsight.](./hdinsight-scaling-best-practices.md)

## <a name="pauseshut-down-clusters"></a>Приостановка и завершение работы кластеров

Большинство заданий Hadoop представляют собой пакетные задания, выполняемые от случая к случаю. Для большинства кластеров Hadoop существует большой период времени, когда кластер не используется для обработки. В случае с HDInsight ваши данные хранятся в службе хранилища Azure, что позволяет безопасно удалить неиспользуемый кластер.
...
- С помощью интерфейса командной строки Azure. Смотрите [кластеры Управления Azure HDInsight с помощью Azure CLI.](hdinsight-administer-use-command-line.md)
...
1. Выберите **Панели мониторинга кластера**.
    ...
    ![Кластерное меню HDInsight Apache Hadoop](./media/hdinsight-administer-use-portal-linux/hdinsight-azure-portal-cluster-menu2.png)
...
3. На [главной странице кластера](#homePage)выберите **действия скрипта** в **настройках.**
4. На странице **действий скрипта** выберите **Отправить новый**.
5. Со страницы **действий скрипта «Отправить»** введите следующую информацию:

   | Поле | Значение |
   | --- | --- |
   | Тип скрипта | Выберите **— Настраиваемый** из раскрывающегося списка.|
   | name |"Изменить пароль SSH" |
   | URI bash-скрипта |Универсальный код ресурса (URI) файла changepassword.sh. |
   | Тип узла (ы): (голова, рабочий, Нимбус, Руководитель или Зоозащитник.) |✓ для всех перечисленных типов узлов. |
...
6. Выберите **Создать**, чтобы применить сценарий. После завершения скрипта можно подключиться к кластеру с помощью SSH с помощью нового пароля.
...
Плитка **размера кластера** из [главной страницы кластера](#homePage) отображает количество ядер, выделенных этому кластеру, и способих их выделения для узлов в этом кластере.

> [!IMPORTANT]  
> Для мониторинга служб, предоставляемых кластером HDInsight, необходимо использовать интерфейс Ambari Web или API REST Ambari. Для получения дополнительной информации об использовании Ambari, [см. Управление HDInsight кластеров с помощью Apache Ambari](hdinsight-hadoop-manage-ambari.md)
...
В этой статье вы изучили некоторые основные административные функции. Дополнительные сведения см. в следующих статьях:
