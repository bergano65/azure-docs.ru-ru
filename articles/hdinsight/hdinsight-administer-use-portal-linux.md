...
HDInsight работает со множеством компонентов Hadoop. Список проверенных и поддерживаемых компонентов см. в статье [Что представляют собой компоненты и версии Apache Hadoop, доступные в HDInsight?](hdinsight-component-versioning.md) Общие сведения о создании кластеров см. в статье [Установка кластеров в HDInsight с использованием Hadoop, Spark, Kafka и других технологий](hdinsight-hadoop-provision-linux-clusters.md).
...
- Использование существующей группы ресурсов:
    ...
    4. Щелкните **Управление доступом (IAM)** и **Назначения ролей** и проверьте, есть ли у вас (или у группы, к которой вы принадлежите) доступ к группе ресурсов, соответствующий роли не ниже участника.
...
4. Щелкните кластер в списке, чтобы просмотреть страницу с общими сведениями:
    ...
    * **Панель мониторинга**. Открывает веб-интерфейс Ambari для кластера.
...
**Меню слева:**
    * **Журналы действий**. Отображение и запрашивание журналов действий.
...
* **Data Lake Store 1-го поколения**. Настройка доступа к Data Lake Store 1-го поколения.  См. [Quickstart: Set up clusters in HDInsight](../storage/data-lake-storage/quickstart-create-connect-hdi-cluster.md) (Краткое руководство. Настройка кластеров в HDInsight).
...
**Мониторинг**
* **Изменения**. Управление оповещениями и действиями.
    * **Метрики**. Мониторинг метрик кластера в Azure Log Analytics.
* **Параметры диагностики**. Настройки места хранения для метрик диагностики **Поддержка и устранение неполадок**.
