...
Выберите имя кластера из [ **кластеры HDInsight** ](#showClusters) страницы.  Откроется представление **Обзор**, которое будет выглядеть следующим образом.
...
|Delete (Удалить)|Удаляет кластер. |
...
**Меню слева:**  
  - **Меню сверху слева**
    ...
    |Быстрый старт|Отображение сведений, помогающих начать работу с HDInsight.|
    |Средства|Справочные сведения об инструментах, связанных с HDInsight.|
  ...
  - **Меню параметров**  
    ...
    |SSH и вход в кластер|Отображение сведений о подключении к кластеру по протоколу SSH. Дополнительные сведения см. в статье [Использование SSH с Hadoop на основе Linux в HDInsight из Linux, Unix или OS X](hdinsight-hadoop-linux-use-ssh-unix.md).|
    ...
    |properties|Просмотр [свойств кластера](#properties).|
...
> [!NOTE]  
> Поддерживаются только кластеры HDInsight версии 3.1.3 или более поздней. Если вы не знаете версию кластера, см. страницу «Свойства».  Ознакомьтесь с разделом "Отображение кластеров".
...
* С помощью интерфейса командной строки Azure. См. в разделе [управлять Azure HDInsight кластеров под управлением Windows с помощью Azure CLI](hdinsight-administer-use-command-line.md).
...
Кластер HDInsight может иметь две учетные записи пользователя. Учетная запись пользователя кластера HDInsight (учетная запись пользователя HTTP) и учетной записи пользователя SSH создаются во время создания. С помощью портала можно изменить имя и пароль учетной записи, а также действия сценария для изменения учетной записи SSH.
...
3. Из [кластера Домашняя страница](#homePage)выберите **действия скрипта** под **параметры**.
...
По умолчанию эти службы предоставляются для доступа. Вы можете отменить или предоставить доступ с помощью [Azure PowerShell](hdinsight-administer-use-powershell.md#grantrevoke-access).
...
В колонке кластера HDInsight в разделе **Использование** отображаются сведения о количестве ядер, доступных для HDInsight в рамках вашей подписки, а также о количестве ядер, выделенных для этого кластера, и их распределении между узлами в кластере. Ознакомьтесь с разделом "Отображение кластеров".
