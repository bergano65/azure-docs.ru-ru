...
> [!IMPORTANT]  
> Параметр `Spark2Defaults` может потребоваться использовать с [AzHDInsightConfigValues добавить](https://docs.microsoft.com/powershell/module/az.hdinsight/add-azhdinsightconfigvalues). Пустые значения можно передать в параметр, как показано в следующем примере кода.
