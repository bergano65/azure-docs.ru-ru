...
2. В окне **Обозреватель серверов** щелкните правой кнопкой мыши запись **HDInsight**, а затем выберите **Connect to HDInsight Emulator** (Подключиться к эмулятору HDInsight).
    ...
    ![обозреватель сервера с выделенным эмулятором подключения к HDInsight](./media/hdinsight-hadoop-emulator-visual-studio/connect-hdinsight-emulator.png)
...
4. Кроме того, можно выполнять задания Hive интерактивно. Для этого нужно изменить значение поля **Пакет** на **Интерактивный**, а затем нажать кнопку **Выполнить**.
    ...
    ![Снимок экрана выходных данных HiveServer2](./media/hdinsight-hadoop-emulator-visual-studio/hiveserver2-output-box.png)
...
2. В списке проектов разверните список **Шаблоны**, раздел **Azure Data Lake**, а затем выберите **HIVE (HDInsight)**. В списке шаблонов выберите **Hive Sample** (Пример Hive). Введите имя и расположение, а затем нажмите кнопку **ОК**.
    ...
    ![Окно "новый проект" с Azure Data Lake, примером Hive и ОК](./media/hdinsight-hadoop-emulator-visual-studio/new-apache-hive-project.png)
...
1. Откройте Visual Studio и выберите **файл**, **создать**, а затем **проект**. В списке проектов разверните список **Шаблоны**, разверните **Azure Data Lake**, а затем выберите **Pig (HDInsight)**. В списке шаблонов выберите **Pig Application** (Приложение Pig). Введите имя и расположение, а затем нажмите кнопку **ОК**.
...
2. Если развернуть таблицу, отобразятся все столбцы этой таблицы. Чтобы быстро просмотреть данные, щелкните таблицу правой кнопкой мыши и выберите пункт **Просмотреть первые 100 строк**.
    ...
    ![обозреватель сервера с развернутой таблицей и просмотром первых 100 строк](./media/hdinsight-hadoop-emulator-visual-studio/hdi-view-top-100-rows.png)
