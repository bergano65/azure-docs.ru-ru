---
title: Инструменты Data Lake для Visual Studio & Hortonworks - Azure HDInsight
...
2. В окне **Обозреватель серверов** щелкните правой кнопкой мыши запись **HDInsight**, а затем выберите **Connect to HDInsight Emulator** (Подключиться к эмулятору HDInsight).
    ...
    ![Исследователь сервера, с подключением к HDInsight эмулятор подчеркнул](./media/hdinsight-hadoop-emulator-visual-studio/connect-hdinsight-emulator.png)

3. В диалоговом окне **Подключиться к эмулятору HDInsight** введите пароль, настроенный для Ambari.

    ![Скриншот диалогового окна, с амбари пароль текстовое окно выделено](./media/hdinsight-hadoop-emulator-visual-studio/enter-ambari-password.png)
...
4. В поле **Пароль** введите пароль, настроенный для учетной записи `root`. Оставьте в остальных полях значения по умолчанию.
    ...
    ![Скриншот диалогового окна с выделенным корневым текстовым полем пароля](./media/hdinsight-hadoop-emulator-visual-studio/enter-root-password1.png)
...
2. В новом окне создания запроса введите следующую команду:
    ...
    ![Скриншот отправки сценария диалог окно улей](./media/hdinsight-hadoop-emulator-visual-studio/advanced-apache-hive.png)
...
4. Кроме того, можно выполнять задания Hive интерактивно. Для этого нужно изменить значение поля **Пакет** на **Интерактивный**, а затем нажать кнопку **Выполнить**.
    ...
    ![Скриншот выхода HiveServer2](./media/hdinsight-hadoop-emulator-visual-studio/hiveserver2-output-box.png)
...
2. В списке проектов разверните список **Шаблоны**, раздел **Azure Data Lake**, а затем выберите **HIVE (HDInsight)**. В списке шаблонов выберите **Hive Sample** (Пример Hive). Введите имя и расположение, а затем нажмите кнопку **ОК**.
    ...
    ![Новое окно проекта с озером данных Azure, образцом Улей и OK](./media/hdinsight-hadoop-emulator-visual-studio/new-apache-hive-project.png)
...
1. Открыть Visual Studio, и выберите **файл**, **Новый**, а затем **проекта**. В списке проектов разверните список **Шаблоны**, разверните **Azure Data Lake**, а затем выберите **Pig (HDInsight)**. В списке шаблонов выберите **Pig Application** (Приложение Pig). Введите имя и расположение, а затем нажмите кнопку **ОК**.
...
2. Введите в качестве содержимого файла **script.pig**, созданного с помощью этого проекта, следующий текст.
    ...
    ![Скриншот Отправка сценарий диалог окно свиньи](./media/hdinsight-hadoop-emulator-visual-studio/advanced-apache-pig1.png)
...
2. Если развернуть таблицу, отобразятся все столбцы этой таблицы. Чтобы быстро просмотреть данные, щелкните таблицу правой кнопкой мыши и выберите пункт **Просмотреть первые 100 строк**.
    ...
    ![Исследователь серверов, с расширенной таблицей и посмотреть Топ 100 строк выбран](./media/hdinsight-hadoop-emulator-visual-studio/hdi-view-top-100-rows.png)
...
![Снимок экрана: обозреватель сервера с выделенным пунктом "Создать таблицу"](./media/hdinsight-hadoop-emulator-visual-studio/server-explorer-create-table.png)
