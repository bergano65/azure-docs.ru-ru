...
* **Группы безопасности сети** (NSG) позволяют фильтровать входящий и исходящий трафик в сети. Дополнительные сведения см. в документе [Фильтрация сетевого трафика с помощью групп безопасности сети](../virtual-network/security-overview.md).
    ...
    > [!WARNING]  
    > HDInsight не поддерживает ограничение исходящего трафика. Весь исходящий трафик должен быть разрешен.
...
Как управляемая служба HDInsight требуется неограниченный доступ к HDInsight работоспособности и управления служб, как для входящего и исходящего трафика из виртуальной сети. При использовании NSG и UDR необходимо убедиться, что эти службы могут взаимодействовать с кластером HDInsight.
...
3. Создайте или измените группы безопасности сети или определяемые пользователем маршруты для подсети, в которой планируется установить HDInsight.
    ...
    * __Группы безопасности сети__: разрешите __входящий__ трафик через порт __443__ с диапазона IP-адресов. Это позволит гарантировать, что службы управления HDI могут обращаться к кластеру за пределами виртуальной сети.
    * __Определяемые пользователем маршруты__. Если вы планируете использовать определяемые пользователем маршруты, создайте маршрут для каждого IP-адреса и задайте для параметра __Тип следующего прыжка__ значение __Интернет__. Следует также разрешить другой исходящий трафик из виртуальной сети без ограничений. Например вы можете перенаправлять весь трафик в брандмауэр или сетевого виртуального устройства Azure (размещенный в Azure) для наблюдения, но не блокируйте исходящий трафик.
...
Принудительное туннелирование — это определяемая пользователем конфигурация маршрутизации, где весь трафик из подсети принудительно направляется в определенную сеть или расположение, например в локальную сеть. HDInsight __не__ поддерживает Принудительное туннелирование для локальных сетей. Если вы используете брандмауэр Azure или сетевой виртуальный модуль, размещенной в Azure, можно использовать определяемые пользователем маршруты для маршрутизации трафика к нему для наблюдения и разрешить весь исходящий трафик.
...
1. Всегда должен быть разрешен трафик для следующих IP-адресов.
    ...
    | Исходный IP-адрес | Конечный порт | Direction |
    | ---- | ----- | ----- |
...
2. Если ваш кластер HDInsight находится в одном из следующих регионов, необходимо разрешить трафик для IP-адресов, указанных для этого региона.
    ...
    | Страна | Регион | Разрешенные исходные IP-адреса | Разрешенный порт назначения | Direction |
    | ---- | ---- | ---- | ---- | ----- |
    | Азия | Восточная Азия | 23.102.235.122</br>52.175.38.134 | 443 | Вход. |
    | &nbsp; | Юго-Восточная Азия | 13.76.245.160</br>13.76.136.249 | 443 | Вход. |
    | Австралия | Восточная часть Австралии | 104.210.84.115</br>13.75.152.195 | 443 | Вход. |
    | &nbsp; | Юго-Восточная часть Австралии | 13.77.2.56</br>13.77.2.94 | 443 | Вход. |
    | Бразилия | Южная часть Бразилии | 191.235.84.104</br>191.235.87.113 | 443 | Вход. |
    ...
    | Япония | Восточная часть Японии | 13.78.125.90</br>13.78.89.60 | 443 | Вход. |
    | &nbsp; | Западная часть Японии | 40.74.125.69</br>138.91.29.150 | 443 | Вход. |
    | Корея | Центральная Корея | 52.231.39.142</br>52.231.36.209 | 433 | Вход. |
    | &nbsp; | Южная Корея | 52.231.203.16</br>52.231.205.214 | 443 | Вход.
    | Соединенное королевство | Западная часть Великобритании | 51.141.13.110</br>51.141.7.20 | 443 | Вход. |
    | &nbsp; | Южная часть Великобритании | 51.140.47.39</br>51.140.52.16 | 443 | Вход. |
    | США | Центральный регион США | 13.67.223.215</br>40.86.83.253 | 443 | Вход. |
    ...
    | &nbsp; | Западно-центральная часть США | 52.161.23.15</br>52.161.10.167 | 443 | Вход. |
    | &nbsp; | Запад США | 13.64.254.98</br>23.101.196.19 | 443 | Вход. |
    | &nbsp; | Западный регион США 2 | 52.175.211.210</br>52.175.222.222 | 443 | Вход. |
