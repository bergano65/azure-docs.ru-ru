...
> [!IMPORTANT]  
> Создав кластер HDInsight в виртуальной сети, создайте сетевые ресурсы, такие как сетевые адаптеры и подсистемы балансировки нагрузки. Сделать **не** удалить эти сетевые ресурсы, так как они требуются для кластера, чтобы правильно с виртуальной сети.
>
> Через 28 февраля 2019 г. эти сетевые ресурсы (например, сетевые адаптеры, LBs и т. д), для НОВЫХ кластеров, созданных в виртуальной сети будет подготовлена к работе в той же группе ресурсов кластера HDInsight. Ранее эти ресурсы были подготовлены в группе ресурсов виртуальной сети. Нет изменений для текущей работающие кластеры и кластеры создана без виртуальной сети.
...
### <a name="hdinsight-with-network-security-groups"></a>HDInsight с помощью групп безопасности сети
...
Если вы используете группы безопасности сети или определяемые пользователем маршруты для управления трафиком, необходимо разрешить трафик с IP-адресов для служб Azure, работоспособности и управления таким образом, чтобы они могли взаимодействовать с кластером HDInsight. Некоторые IP-адресов, зависят от региона, а некоторые из них применяются во всех регионах Azure. Может также потребоваться разрешить трафик из службы Azure DNS, если вы не используете пользовательский DNS-сервер. Кроме того, вам необходимо разрешить трафик между виртуальными машинами внутри подсети. Чтобы найти IP-адреса, которые нужно разрешить, выполните следующие действия.
...
1. Если вы используете службу DNS, предоставленной Azure, разрешите доступ из __168.63.129.16__ через порт 53. Дополнительные сведения см. в документе [Разрешение имен для виртуальных машин и экземпляров ролей](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md). Если вы используете пользовательский DNS-сервер, пропустите этот шаг.

2. Разрешите трафик от следующих IP-адресов для Azure службы работоспособности и управления, которые применяются во всех регионах Azure:
...
3. Разрешите трафик с IP-адресов для служб Azure работоспособности и управления в определенном регионе, где находятся ресурсы:

    > [!IMPORTANT]  
    > Если используемый регион Azure не указан в списке, то используйте только четыре IP-адреса, приведенные на шаге 1.

    | Country | Регион | Разрешенные исходные IP-адреса | Допустимые назначения | Direction |
    | ---- | ---- | ---- | ---- | ----- |
    ...
    | США | Центральный регион США | 13.89.171.122</br>13.89.171.124 | \*:443 | Входящий трафик |
...
> [!IMPORTANT]  
> Изменить IP-адреса для `hdirule1` и `hdirule2` в этом примере в соответствии с регионом Azure, вы используете. Эти сведения представлены в разделе [HDInsight с группами безопасности сети и определяемыми пользователем маршрутами](#hdinsight-ip).
...
В этом примере показано, как добавить правила, чтобы разрешить входящий трафик на требуемые IP-адреса. В нем не содержится правило, ограничивающее входящий доступ из других источников. Следующий код демонстрирует, как включить доступ по протоколу SSH из Интернета:
...
Следующий код демонстрирует, как включить доступ по протоколу SSH из Интернета:
...
* Настройка кластеров Apache HBase в виртуальных сетях Azure, см. в разделе [Создание Apache кластеров HBase в HDInsight в виртуальной сети Azure](hbase/apache-hbase-provision-vnet.md).
