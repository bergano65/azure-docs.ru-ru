...
3. Создайте или измените группы безопасности сети или определяемые пользователем маршруты для подсети, в которой планируется установить HDInsight.
    ...
    * __Определяемые пользователем маршруты__. Если вы планируете использовать определяемые пользователем маршруты, создайте маршрут для каждого IP-адреса и задайте для параметра __Тип следующего прыжка__ значение __Интернет__. Следует также разрешить другой исходящий трафик из виртуальной сети без ограничений. Например, можно направлять весь остальной трафик на брандмауэр Azure или виртуальный сетевой модуль (размещенный в Azure) для мониторинга, но исходящий трафик не должен блокироваться.
...
Принудительное туннелирование — это определяемая пользователем конфигурация маршрутизации, где весь трафик из подсети принудительно направляется в определенную сеть или расположение, например в локальную сеть. HDInsight __не__ поддерживает принудительное туннелирование в локальных сетях. Если вы используете Брандмауэр Azure или виртуальный сетевой модуль, размещенный в Azure, то можете использовать UDR, чтобы направить трафик к нему для мониторинга, и разрешить весь исходящий трафик.
...
Если вы используете группы безопасности сети или определяемые пользователем маршруты, то необходимо разрешить передачу трафика службы работоспособности и службы управления в HDInsight. Кроме того, вам необходимо разрешить трафик между виртуальными машинами внутри подсети. Чтобы найти IP-адреса, которые нужно разрешить, выполните следующие действия.
...
Если вы планируете использовать **брандмауэр** для защиты виртуальной сети и получать доступ к кластеру через определенные порты, вам следует разрешить трафик через порты, необходимые для вашего сценария. По умолчанию вам не нужно будет добавлять в список разрешений эти порты:
