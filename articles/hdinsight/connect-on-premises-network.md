...
Эти конфигурации обеспечивают следующее поведение:
...
## <a name="create-virtual-network-configuration"></a>Создание конфигурации виртуальной сети

Дополнительные сведения о том, как создать виртуальную сеть Azure, подключенную к локальной сети, см. в следующих документах:

* [Использование портал Azure](../vpn-gateway/tutorial-site-to-site-portal.md)
...
1. На вкладке __Основные сведения__ задайте следующие параметры.  
    ...
    |Region | Выберите тот же регион, что и для созданной ранее виртуальной сети.  Не во всех регионах доступны виртуальные машины всех размеров.  |
    ...
    |Образ — | Оставьте в **Ubuntu Server 18,04 LTS**. |
...
После создания виртуальной машины вы получите уведомление об **успешном развертывании** с кнопкой " **Переход к ресурсу** ".  Выберите **Перейти к ресурсу**, чтобы перейти к новой виртуальной машине.  Из представления по умолчанию для новой виртуальной машины выполните следующие действия, чтобы определить связанные IP-адреса.
...
1. Используйте SSH для подключения к __общедоступному IP-адресу__ виртуальной машины. Замените `sshuser` учетной записью пользователя SSH, указанной при создании виртуальной машины. В следующем примере устанавливается подключение к виртуальной машине по адресу 40.68.254.142:
...
## <a name="configure-virtual-network-to-use-the-custom-dns-server"></a>Настройка виртуальной сети для использования настраиваемого DNS-сервера
...
1. В меню слева перейдите к разделу **все службы**  >  **Сетевые подключения**  >  **виртуальные сети**.
...
Дополнительные сведения об использовании DNS в **Windows Server 2016** см. в документации по [Add-DnsServerConditionalForwarderZone](/powershell/module/dnsserver/add-dnsserverconditionalforwarderzone).

После настройки локального DNS-сервера можно использовать `nslookup` из локальной сети, чтобы проверить возможность разрешения имен в виртуальной сети. Следующий пример: 
...
2. Для IP-адресов, определенных на шаге 1, необходимо разрешить входящий трафик.
   ...
   * Если вы используете __NSG__: Разрешите __входящий__ трафик через порт __443__ для IP-адресов.
   * Если вы используете __UDR__: задайте для IP-адресов тип __следующего прыжка__ маршрута к __Интернету__ .
...
В большей части документации по HDInsight предполагается, что у вас есть доступ к кластеру через Интернет. Например, что вы можете подключиться к кластеру по адресу `https://CLUSTERNAME.azurehdinsight.net`. Этот адрес использует общедоступный шлюз, который недоступен, если вы использовали группы безопасности сети или определяемые пользователем маршруты для ограничения доступа из Интернета.

В некоторых документах также указывается `headnodehost` при подключении к кластеру из сеанса SSH. Этот адрес доступен только на узлах в кластере и не может использоваться на клиентах, подключенных через виртуальную сеть.
