---
title: Подключение Azure HDInsight к вашей предварительной сети
...
1. Создать пользовательский DNS-сервер в виртуальной сети Azure.
1. Настроить виртуальную сеть для использования пользовательского DNS-сервера вместо рекурсивного сопоставителя Azure по умолчанию.
1. Настроить переадресацию между пользовательским и локальным DNS-сервером.

Эти конфигурации позволяют следующее поведение:
...
![Диаграмма того, как dNS-запросы решаются в конфигурации](./media/connect-on-premises-network/on-premises-to-cloud-dns.png)
...
## <a name="create-virtual-network-configuration"></a>Создание виртуальной конфигурации сети
...
* [Использование Azure CLI](../vpn-gateway/vpn-gateway-howto-site-to-site-resource-manager-cli.md)
...
1. Выберите **компьютерную** > **виртуальную машину,** чтобы перейти на страницу **«Создать виртуальную машину».**

1. На вкладке __Основные сведения__ задайте следующие параметры.  
    ...
    |Общедоступные входящие порты|Выберите **Разрешить выбранные порты**. Затем выберите **SSH (22)** из списка выпадающих **портов Select.**|
...
4. На вкладке **Сеть** введите следующие сведения.
    ...
    ![HdInsight Виртуальные настройки сети](./media/connect-on-premises-network/virtual-network-settings.png)
...
1. Используйте SSH для подключения к __общедоступному IP-адресу__ виртуальной машины. Замените `sshuser` указанную при создании VM учетную запись пользователя SSH, указанную при создании VM. В следующем примере устанавливается подключение к виртуальной машине по адресу 40.68.254.142:
...
3. Для настройки Bind для отправки запросов на разрешение имен на ваш сервер `/etc/bind/named.conf.options` DNS помещения используйте следующий текст в качестве содержимого файла:
...
4. В сеансе SSH используйте следующую команду:
    ...
    Текст `icb0d0thtw0ebifqt0g1jycdxd.ex.internal.cloudapp.net` — это __DNS-суффикс__ для виртуальной сети. Сохранить это значение, как это используется позже.
...
## <a name="configure-virtual-network-to-use-the-custom-dns-server"></a>Настройка виртуальной сети для использования пользовательского DNS-сервера
...
1. Из левого меню, перейдите на **все услуги** > **Сети Виртуальные** > **сети**.
...
## <a name="configure-on-premises-dns-server"></a>Настройка внутреннего DNS-сервера
...
После настройки локального DNS-сервера можно использовать `nslookup` из локальной сети, чтобы убедиться, что вы можете решить имена в виртуальной сети. Следующий пример: 
...
1. Найдите IP-адреса расположения, которое содержит виртуальную сеть. Список требуемых IP-адресов по расположениям см. в [этом разделе](./hdinsight-management-ip-addresses.md).

2. Для IP-адресов, определенных на шаге 1, необходимо разрешить входящий трафик.

   * Если вы используете __NSG:__ Разрешить __входящий__ трафик на порт __443__ для IP-адресов.
   * Если вы используете __UDR:__ Установите следующий тип __hop__ маршрута в __Интернет__ для IP-адресов.

Примеры использования Azure PowerShell и Azure CLI для создания групп безопасности см. в статье [Расширение возможностей HDInsight с помощью виртуальной сети Azure](hdinsight-create-virtual-network.md#hdinsight-nsg).
...
В большей части документации по HDInsight предполагается, что у вас есть доступ к кластеру через Интернет. Например, что вы можете подключиться к кластеру по адресу `https://CLUSTERNAME.azurehdinsight.net`. Этот адрес использует общедоступный шлюз, который недоступен, если вы использовали НСК или UdR для ограничения доступа из Интернета.

В некоторых документах также указывается `headnodehost` при подключении к кластеру из сеанса SSH. Этот адрес доступен только из узлов в кластере и не доступен для клиентов, подключенных к виртуальной сети.
...
* Для получения дополнительной информации об использовании [Plan a virtual network deployment for Azure HDInsight clusters](./hdinsight-plan-virtual-network-deployment.md)HDInsight в виртуальной сети см.
