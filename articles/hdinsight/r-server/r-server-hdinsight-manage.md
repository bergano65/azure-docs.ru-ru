...
В этой статье вы узнаете, как управлять существующим кластером ML Services на Azure HDInsight для выполнения таких задач, как добавление нескольких одновременных пользователей, удаленное подключение к кластеру услуг ML, изменение контекста вычислений и т.д.
...
![Параметры входа в портал HDI Azure](./media/r-server-hdinsight-manage/hdi-concurrent-users1.png)
...
- **Безопасное имя пользователя Shell (SSH):** пользователь SSH для доступа к кластеру через безопасную оболочку. Этот пользователь применяется в системе Linux для всех головных, рабочих и граничных узлов. Таким образом, с помощью безопасной оболочки вы можете получить доступ к любому узлу в удаленном кластере.
...
![скриншот вывода одновременных пользователей](./media/r-server-hdinsight-manage/hdi-concurrent-users2.png)
...
Контекст вычислений позволяет определить, где будут выполнены вычисления — на локальном компьютере или пограничном узле, или же они будут распределены между узлами в кластере HDInsight.  Например, для настройки контекста вычислений с RStudio Server [см. Выполнить R-скрипт в кластере ML Services в Azure HDInsight с помощью RStudio Server.](machine-learning-services-quickstart-job-rstudio.md)
...
3. В разделе **Отправка действия скрипта** введите следующие сведения.
   ...
   * Установите флажок **Persist this script action** (Сохранить это действие скрипта).  
   ...
   >    Для получения дополнительной информации [Script Action development](../hdinsight-hadoop-script-actions-linux.md)о разработке действий по сценарию см.

   ![Портал Azure отправляет действие сценария](./media/r-server-hdinsight-manage/submit-script-action.png)
