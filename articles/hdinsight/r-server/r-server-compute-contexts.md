...
[Службы машинного обучения в Azure HDInsight](r-server-overview.md) предоставляют новейшие возможности для анализа на основе R. Он может использовать данные, которые хранятся в контейнере Apache Hadoop HDFS в вашей учетной записи хранения [Azure Blob,](../../storage/common/storage-introduction.md "Хранилище BLOB-объектов Azure") магазине data Lake Store или локальной файловой системе Linux. Поскольку ML Services построена на базе R с открытым исходным кодом, созданные на основе R приложения могут применяться к любому из 8000 пакетов R с открытым исходным кодом. Также вы можете использовать подпрограммы [RevoScaleR](https://docs.microsoft.com/machine-learning-server/r-reference/revoscaler/revoscaler) — пакета аналитики больших данных от корпорации Майкрософт, который предоставляется вместе со Службами машинного обучения.  
...
Выбор варианта распараллеленного выполнения зависит от характера задач анализа, а также размера и местонахождения данных. Нет простой формулы, которая говорит вам, какой контекст для использования. Однако есть некоторые базовые принципы, которые помогут вам определиться или хотя бы сузить выбор еще до запуска теста производительности. К ним относятся следующие:
...
- Если объем данных для анализа невелик и не требует повторного анализа, то поток его непосредственно в режим анализа с помощью *локального* или *локального*пара.
...
- Используйте контекст вычислений Map Reduce только в том случае, если вы столкнетесь с непреодолимой проблемой с контекстом вычислений Spark, так как он, как правило, медленнее.  
