...
description: Обзор Apache Феникс
author: ashishthaps
ms.author: ashishth
ms.reviewer: jasonh
...
---
# <a name="apache-phoenix-in-azure-hdinsight"></a>Apache Феникс в Azure HDInsight
...
Такой подход может повысить производительность при выполнении одноиндексных запросов. Этот тип вторичного индекса является **охватывающим индексом**, содержащим все столбцы, включенные в запрос. Таким образом, поиск таблицы не требуется, и индекс удовлетворяет весь запрос.
...
Сканирование с пропуском использует перечисление `SEEK_NEXT_USING_HINT` фильтра HBase. С помощью `SEEK_NEXT_USING_HINT` при сканировании с пропуском отслеживается набор ключей или диапазон ключей, выполняемый в каждом столбце. Затем сканирование пропуска принимает ключ, который был передан ему во время оценки фильтра, и определяет, является ли это одной из комбинаций. Если нет, оценивается следующий ключ с самым высоким значением для перехода.
...
*Обнаружение доступа серверов региона* может произойти при написании записей с последовательными ключами к HBase. Не смотря на то, что у вас может быть несколько региональных серверов в кластере, ваши записи обрабатываются только на одном. Эта концентрация создает проблему "гиперобъекта". Только один сервер обрабатывает всю рабочую нагрузку вместо ее распределения по всем доступным региональным серверам. Поскольку каждый регион имеет предопределенный максимальный размер, когда область достигает этого предела размера, он делится на два небольших региона. После этого один из новых регионов принимает все новые записи, становясь новым гиперобъектом.
...
2. Из списка служб в меню слева выберите **HBase**, а затем выберите вкладку **Конфигурация**.
    ...
    ![Конфигурации Apache Ambari HBase](./media/hdinsight-phoenix-in-hdinsight/ambari-hbase-config1.png)
...
* [Используйте Apache Цеппелин для запуска запросов Apache Phoenix над Apache HBase в Azure HDInsight](./hbase/apache-hbase-phoenix-zeppelin.md)
