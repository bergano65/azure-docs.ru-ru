...
description: Общие сведения о Apache Phoenix
...
Такой подход может повысить производительность при выполнении одноиндексных запросов. Этот тип вторичного индекса является **охватывающим индексом**, содержащим все столбцы, включенные в запрос. Поэтому поиск в таблице не требуется, и индекс удовлетворяет всему запросу.
...
*Гиперобъекта сервера регионов* может выполняться при записи записей с последовательными ключами в HBase. Не смотря на то, что у вас может быть несколько региональных серверов в кластере, ваши записи обрабатываются только на одном. Эта концентрация создает проблему "гиперобъекта". Только один сервер обрабатывает всю рабочую нагрузку вместо ее распределения по всем доступным региональным серверам. Так как каждый регион имеет стандартный максимальный размер, при достижении этого предельного размера он разбивается на два маленьких региона. После этого один из новых регионов принимает все новые записи, становясь новым гиперобъектом.
...
* [Использование Apache Zeppelin для запуска запросов Apache Phoenix через Apache HBase в Azure HDInsight](./hbase/apache-hbase-phoenix-zeppelin.md)
