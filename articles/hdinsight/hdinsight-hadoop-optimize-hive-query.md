...
В Azure HDInsight существует несколько типов кластеров и технологий, которые могут выполнять запросы Apache Hive. Выберите подходящий тип кластера, чтобы оптимизировать производительность для потребностей рабочей нагрузки.

Например, выберите тип кластера `ad hoc` **Интерактивный запрос** для оптимизации для интерактивных запросов. Выберите тип кластера Apache **Hadoop**, чтобы оптимизировать его для запросов Hive, используемых в качестве пакетной обработки. Типы кластера **Spark** и **HBase** также могут выполнять запросы Hive. Дополнительные сведения о выполнении запросов Hive кластерами различных типов см. в статье [Обзор Apache Hive и HiveQL в Azure HDInsight](hadoop/hdinsight-use-hive.md).

Кластеры кластеров HDInsight типа кластера Hadoop не оптимизированы для производительности по умолчанию. В этой статье описывается несколько наиболее распространенных методов оптимизации производительности Hive, которые можно применить к отправке запросов.

## <a name="scale-out-worker-nodes"></a>Горизонтальное масштабирование рабочих узлов

Увеличение количества рабочих узлов в кластере HDInsight позволяет выполнять работу параллельно с большим количеством картографов и редукторов. Горизонтальное масштабирование в HDInsight можно выполнить двумя способами:

* При создании кластера можно указать количество узлов работы с помощью портала Azure, интерфейса Azure PowerShell или интерфейса командной строки.  Дополнительные сведения см. в статье о [создании кластеров HDInsight](hdinsight-hadoop-provision-linux-clusters.md). На следующем снимке экрана показана рабочая конфигурация узла на портале Azure:
  
    ![Узлы размера кластера портала Azure](./media/hdinsight-hadoop-optimize-hive-query/azure-portal-cluster-configuration.png "scaleout_1")
...
* После создания кластера можно также изменить количество рабочих узлов, чтобы масштабировать кластер без необходимости его повторного создания.
    ...
    ![Размер кластера кластера портала Azure](./media/hdinsight-hadoop-optimize-hive-query/azure-portal-settings-nodes.png "scaleout_2")
...
[Apache Tez](https://tez.apache.org/) — это альтернативный двигатель исполнения двигателя MapReduce. При подготовке кластеров HDInsight под управлением Linux платформа Tez включена по умолчанию.

![Диаграмма обзора HDInsight Apache Tez](./media/hdinsight-hadoop-optimize-hive-query/hdinsight-tez-engine.png)

Tez работает быстрее, так как:

* **Выполняет задания, представленные в виде направленных ациклических графов (DAG), как единое целое при работе с платформой MapReduce.** DAG требует, чтобы набору модулей сопоставления соответствовал набор модулей сжатия. Это требование приводит к тому, что для каждого запроса Hive отключается несколько заданий MapReduce. Tez не имеет таких ограничений и может обрабатывать сложные DAG, как одно задание минимизации работы запуска накладных расходов.
* **Позволяет избежать ненужных операций записи.** Чтобы обработать тот же запрос Hive в механизме MapReduce, используются несколько заданий. Выходные данные каждого задания MapReduce записываются в HDFS для промежуточных данных. Так как Tez сводит к минимуму количество заданий для каждого запроса Hive, он может избежать ненужных записей.
...
* **Повторно использует контейнеры.** Всякий раз, когда это возможно Tez будет повторно использовать контейнеры, чтобы гарантировать, что задержка от запуска контейнеров уменьшается.
...
![HDInsight Apache Hive раздел](./media/hdinsight-hadoop-optimize-hive-query/hdinsight-partitioning.png)

Некоторые рекомендации для выполнения секционирования:

* **Не под разделом** - Раздел на столбцах с несколькими значениями может привести к нескольким перегородок. Например, раздел по полу создает только две части для создания (мужской и женский), поэтому уменьшить задержку максимум наполовину.
* **Не переутомляйте** - С другой стороны, создание раздела на столбце с уникальным значением (например, userid) вызывает несколько разделов. что увеличит нагрузку на узел имен кластера из-за необходимости обрабатывать множество каталогов.
...
* **Динамическое секционирование** означает, что Hive автоматически создаст секции для вас. Поскольку таблица раздела уже создана из таблицы, все, что вам нужно сделать, это вставить данные в разделительную таблицу:
