...
Следующие сведения о кластере полезны для сбора информации в вашей стратегии управления журналами. Соберите эту информацию из всех созданных кластеров HDInsight в определенной учетной записи Azure.
...
Вы можете получить большую часть этих сведений верхнего уровня, используя портал Azure.  Кроме того, вы можете использовать [Azure CLI](https://docs.microsoft.com/cli/azure/?view=azure-cli-latest) для получения информации о кластере HDInsight(ы):
...
Apache Ambari упрощает конфигурацию кластера HDInsight, управление им и его мониторинг за счет пользовательского веб-интерфейса и интерфейса REST API. Ambari есть в кластерах HDInsight на основе Linux. Выберите панель **кластерной панели на** странице портала Azure HDInsight, чтобы открыть страницу ссылки **на панель кластеров.**  Выберите область **Панель мониторинга кластера HDInsight**, чтобы открыть пользовательский интерфейс Ambari.  Вам предложено настроить учетные данные кластера для входа в систему.
...
![Apache Ambari Админ Стек и версии](./media/hdinsight-log-management/ambari-stack-versions.png)
...
![Apache Ambari скачать принимающей клиент конфигурации](./media/hdinsight-log-management/download-client-configs.png)
...
### <a name="view-ambari-alerts-status-logs"></a>Просмотр журналов оповещений о состоянии амбари

Apache Ambari пишет изменения `ambari-alerts.log`статуса оповещения . Полный путь `/var/log/ambari-server/ambari-alerts.log`. Для включения отладки для журнала `/etc/ambari-server/conf/log4j.properties.` измените свойство в изменении, а затем ввейте под: `# Log alert state changes`
...
Следующий шаг заключается в анализе файлов журнала выполнения заданий для различных служб,  таких как Apache HBase, Apache Spark и т. д. Кластер Hadoop производит большое количество многословных журналов, поэтому определение того, какие журналы полезны (а какие нет), может занять много времени.  Важно понимать систему ведения журналов для целевого управления файлами журналов.  Следующее изображение — это пример файла журнала.

![Выход примера файла файла HDInsight выходной](./media/hdinsight-log-management/hdi-log-file-example.png)

### <a name="access-the-hadoop-log-files"></a>Доступ к файлам журналов Hadoop

HDInsight хранит свои файлы журнала как в файловой системе кластера, так и в Хранилище Azure. Вы можете изучить файлы журнала в кластере, открыв соединение [SSH](hdinsight-hadoop-linux-use-ssh-unix.md) с кластером и просматривая файловую систему, или с помощью портала Hadoop YARN Status на удаленном сервере головного узла. Вы можете изучить файлы журнала в Хранилище Azure с помощью любого из инструментов, которые могут получить доступ к данным и загрузить из Azure Storage. Примерами являются [AzCopy,](../storage/common/storage-use-azcopy.md) [CloudXplorer](https://clumsyleaf.com/products/cloudxplorer)и Visual Studio Server Explorer. Кроме того, можно использовать PowerShell и библиотеки клиента хранилища Azure или пакеты SDK .NET Azure для доступа к данным в хранилище BLOB-объектов.

Hadoop выполняет работу заданий в виде *попыток завершения задач* на различных узлах в кластере. HDInsight может инициировать спекулятивные попытки задач, прекращая любые другие попытки задачи, которые не выполняются в первую очередь. Это создает множество операций, которые регистрируются в файлах журнала контроллера, stderr и syslog в режиме реального времени. Кроме того, несколько попыток выполняются одновременно, но файл журнала может отображать результаты только линейно.
...
Служба YARN объединяет журналы со всех контейнеров на рабочем узле и хранит их как один сводный файл журнала на рабочем узле. Этот журнал хранится в файловой системе по умолчанию после завершения приложения. Ваше приложение может использовать сотни или тысячи контейнеров, но журналы для всех контейнеров, выполненных на одном рабочем узле, всегда объединяются в один файл. В приложении используется только один журнал на рабочий узла. Объединение журналов включено по умолчанию на кластерах HDInsight версии 3.0 и более поздних версий. Объединенные журналы находятся в хранилище по умолчанию для кластера.

```
/app-logs/<user>/logs/<applicationId>
```

Агрегированные журналы не читаемы напрямую, так как они записаны в двоичном формате TFile, индексируемом контейнером. Чтобы отобразить эти журналы для интересующих вас приложений или контейнеров в виде обычного текста, используйте журналы YARN ResourceManager или средства CLI.
...
3. В раскрывающемся списке "Быстрые ссылки" выберите один из главных узлов кластера, а затем щелкните **ResourceManager logs** (Журналы Resource Manager). Вам представлен список ссылок на журналы YARN.
...
Теперь у вас достаточно информации для создания стратегии управления ключевыми журналами.  Используйте электронную таблицу (или другой инструмент) для прогнозирования роста объема журналов и затрат на хранение в Azure.  Рассмотрим также все требования к удержанию журналов для набора журналов, которые вы изучаете.  Теперь можно перепрогнозировать будущие затраты на хранение журналов, определив, какие файлы журнала могут быть удалены (если таковые имеется) и какие журналы следует сохранить и архивировать в менее дорогие хранилища Azure.
...
Для некоторых файлов журнала можно использовать более экономный подход архивирования файлов журналов. Для журналов деятельности менеджера ресурсов Azure можно изучить этот подход с помощью портала Azure.  Навеяйте архивирование журналов «Менеджер ресурсов», выбрав ссылку **журнала activity Log** на портале Azure для экземпляра HDInsight.  В верхней части страницы поиска журналов действий выберите пункт меню **Экспорт**, чтобы открыть область **Экспорт журнала действий**.  Укажите подписку, регион, необходимость экспорта в учетную запись хранения и количество дней хранения журналов. В этой же области можно также указать, следует ли экспортировать журналы в концентратор событий.

![Предварительный просмотр экспортной активности портала Azure](./media/hdinsight-log-management/hdi-export-log-files.png)
...
### <a name="accessing-azure-storage-metrics"></a>Доступ к метрикам хранения Azure

Хранилище Azure можно настроить для операций хранения и доступа к журналу. Вы можете использовать эти подробные журналы для мониторинга и планирования емкости, а также для аудита запросов на хранение. Зарегистрированная информация включает в себя детали задержки, позволяющие отслеживать и точно настраивать производительность ваших решений.
Для изучения файлов журнала, генерируемых для хранилища Azure, вмещающие данные для кластера HDInsight, можно использовать SDK .NET для Hadoop.
...
* [Доступ к журналу приложений Apache Hadoop YARN в Linux-hdInsight](hdinsight-hadoop-access-yarn-app-logs-linux.md)
