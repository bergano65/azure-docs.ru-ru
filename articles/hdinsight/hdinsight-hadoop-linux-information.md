...
* [прилистывание](https://curl.haxx.se/) — используется для связи со службами, основанными на веб-службах.
* **JQ**, процессор командной строки JSON.  См. раздел [https://stedolan.github.io/jq/](https://stedolan.github.io/jq/).
...
Замените `CLUSTERNAME` именем кластера. При появлении запроса введите пароль для учетной записи администратора. Эта команда возвращает документ JSON, который содержит список узлов в кластере. [JQ](https://stedolan.github.io/jq/) используется для извлечения значения `host_name` элемента для каждого узла.
...
* **2.6.5.3009-43**: имя каталога — это версия платформы Hadoop, используемая HDInsight. Это число для вашего кластера может отличаться от указанного здесь.
* **текущий**: этот каталог содержит ссылки на подкаталоги в каталоге **2.6.5.3009-43** . Он позволяет вам не запоминать номер версии.
...
В большинстве дистрибутивов Hadoop данные хранятся в HDFS. HDFS поддерживается локальным хранилищем на компьютерах в кластере. Использование локального хранилища может быть дорогостоящим для облачного решения, при котором плата за ресурсы для вычислений наследуется ежечасно или по минуте.

При использовании HDInsight файлы данных хранятся в гибком и устойчивом виде в облаке с помощью хранилища BLOB-объектов Azure и при необходимости Azure Data Lake Storage. Эти службы предоставляют следующие преимущества:
...
* Емкость больших файлов и большие адаптируемые хранилища.
...
Если вы используете службу хранилища Azure или Data Lake Storage, для доступа к данным не требуется никаких специальных действий в HDInsight. Например, следующая команда выводит список файлов в папке `/example/data` , независимо от того, хранится ли он в службе хранилища Azure или Data Lake Storage:

    hdfs dfs -ls /example/data

В HDInsight ресурсы хранилища данных (хранилище BLOB-объектов Azure и Azure Data Lake Storage) отделены от вычислительных ресурсов. Кластеры HDInsight можно создавать для выполнения вычислений по мере необходимости, а позднее удалять кластер после завершения работы. В то же время поддержание сохранности файлов данных в облачном хранилище при необходимости.

### <a name="uri-and-scheme"></a><a name="URI-and-scheme"></a>Схема и URI
...
При использовании службы [**хранилища Azure**](./hdinsight-hadoop-use-blob-storage.md)используйте одну из следующих схем URI:
...
При использовании [**Azure Data Lake Storage 2-го поколения**](./hdinsight-hadoop-use-data-lake-storage-gen2.md)используйте следующую схему URI:
...
При использовании [**Azure Data Lake Storage 1-го поколения**](./hdinsight-hadoop-use-data-lake-store.md)используйте одну из следующих схем URI:
...
* [blobxfer.py](https://github.com/Azure/blobxfer)— cценарий Python для работы с большими двоичными объектами в хранилище Azure.
* Различные пакеты SDK:
    ...
    * [API REST хранилища](https://msdn.microsoft.com/library/azure/dd135733.aspx)
...
Масштабирование кластера позволяет динамически изменить число узлов данных в кластере. Операции масштабирования можно выполнять во время выполнения других заданий или процессов в кластере.  См. раздел [Масштабирование кластеров](./hdinsight-scaling-best-practices.md).

## <a name="how-do-i-install-hue-or-other-hadoop-component"></a>Как установить Hue (или другой компонент Hadoop)?

HDInsight является управляемой службой. В случае обнаружения проблем с кластером Azure может удалить неисправный узел и создать новый для его замены. При установке вручную в кластере они не сохраняются при выполнении этой операции. Вместо этого используйте [действия скрипта HDInsight](hdinsight-hadoop-customize-cluster-linux.md). Действие скрипта может использоваться для внесения следующих изменений:
...
Некоторые технологии Hadoop предоставляют автономные JAR-файлы. Эти файлы содержат функции, используемые в составе задания MapReduce или внутри Pig или Hive. Они часто не требуют какой-либо настройки и могут передаваться в кластер после его создания для непосредственного использования. Если требуется, чтобы компонент сохранил повторное создание образа кластера, сохраните JAR-файл в хранилище кластера по умолчанию.
