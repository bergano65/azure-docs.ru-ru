...
* [cURL](https://curl.haxx.se/) - используется для связи с веб-сервисами.
...
Полностью квалифицированное доменное имя (ФЗДН) для использования при `CLUSTERNAME.azurehdinsight.net` `CLUSTERNAME-ssh.azurehdinsight.net` подключении к кластеру из Интернета является или (только для SSH).
...
Замените `CLUSTERNAME` именем кластера. При появлении запроса введите пароль для учетной записи администратора. Эта команда возвращает документ JSON, который содержит список узлов в кластере. [jq](https://stedolan.github.io/jq/) используется для `host_name` извлечения значения элемента для каждого узла.
...
Эта команда возвращает документ JSON, описывающий службу, а `host_name` затем [jq](https://stedolan.github.io/jq/) вытягивает только значение для узлов.

## <a name="remote-access-to-services"></a>Удаленный доступ к службам

* **Амбари (веб)** - `https://CLUSTERNAME.azurehdinsight.net`

    Authenticate с помощью пользователя кластера администратора и пароль, а затем войти в Ambari.
...
* **Амбари (REST)** - `https://CLUSTERNAME.azurehdinsight.net/ambari`
...
* **WebHCat (Темплтон)** - `https://CLUSTERNAME.azurehdinsight.net/templeton`
...
* **2.6.5.3009-43**: Название каталога — это версия платформы Hadoop, используемая HDInsight. Это число для вашего кластера может отличаться от указанного здесь.
* **текущий**: Этот каталог содержит ссылки на субдиректора в соответствии **с каталогом 2.6.5.3009-43.** Он позволяет вам не запоминать номер версии.
...
В большинстве дистрибутивов Hadoop данные хранятся в распределенной файловой системе Hadoop, реализованной на базе локального хранилища на компьютерах в кластере. Использование локального хранилища может быть дорогостоящим для облачного решения, где вы будете платить почасовую или поминутную плату за вычислительные ресурсы.
...
Если вы используете службу хранилища Azure или Data Lake Storage, для доступа к данным не требуется никаких специальных действий в HDInsight. Например, следующие команды перечисляют файлы в `/example/data` папке независимо от того, хранится ли она в Хранилище Azure или Хранилище озер данных:
...
При использовании [**Хранилища Azure**](./hdinsight-hadoop-use-blob-storage.md)используйте одну из следующих схем URI:
...
При использовании [**Azure Data Lake Storage Gen2**](./hdinsight-hadoop-use-data-lake-storage-gen2.md)используйте следующую схему URI:
...
При использовании [**Azure Data Lake Storage Gen1**](./hdinsight-hadoop-use-data-lake-store.md)используйте одну из следующих схем URI:
...
* Различные пакеты SDK:
    ...
    * [Руби](https://github.com/Azure/azure-sdk-for-ruby)
...
* [Лазурный CLI](../data-lake-store/data-lake-store-get-started-cli-2.0.md)
* [REST API WebHDFS](../data-lake-store/data-lake-store-get-started-rest-api.md);
* [Инструменты озера данных для визуальной студии](https://www.microsoft.com/download/details.aspx?id=49504)
...
Масштабирование кластера позволяет динамически изменить число узлов данных в кластере. Операции масштабирования можно выполнять параллельно с другими заданиями и процессами, выполняющимися в кластере.  См. также, [Масштабируйте кластеры HDInsight](./hdinsight-scaling-best-practices.md)
...
* **Storm.** После завершения операции масштабирования следует запустить повторную балансировку для всех запущенных топологий Storm. Она позволяет настроить параметры параллелизма для топологий на основе нового количества узлов в кластере. Чтобы повторно выполнить балансировку выполняющихся топологий, используйте один из следующих вариантов.
    ...
    * **Пользовательский интерфейс Storm.** Чтобы запустить балансировку для топологии из интерфейса Storm, выполните следующие действия.
        ...
        1. Откройте `https://CLUSTERNAME.azurehdinsight.net/stormui` в вашем `CLUSTERNAME` веб-браузере, где находится название кластера Storm. При появлении запроса введите имя администратора кластера HDInsight (admin) и пароль, указанный при создании кластера.
...
HDInsight является управляемой службой. В случае обнаружения проблем с кластером Azure может удалить неисправный узел и создать новый для его замены. Если вы вручную установите вещи в кластер, они не сохраняются при возникновении этой операции. Вместо этого используйте [действия скрипта HDInsight](hdinsight-hadoop-customize-cluster-linux.md). Действие скрипта может использоваться для внесения следующих изменений:
