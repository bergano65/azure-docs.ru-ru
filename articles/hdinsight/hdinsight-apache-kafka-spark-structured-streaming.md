...
1. Выберите данные и начните потоковую передачу. В приведенной ниже команде показано, как извлечь данные из Kafka с помощью пакетного запроса и записать результаты в HDFS в кластере Spark. В этом примере `select` получает сообщение (значение поля) из Kafka и применяет схему к нему. Затем данные записываются в HDFS (WASB или ADL) в формате Parquet. Введите команду в следующую ячейку Jupyter.
