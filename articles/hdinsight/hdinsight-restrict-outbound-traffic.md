...
1. Добавьте следующие правила:
    ...
    1. Если в кластере реализуется WASB и вы не используете выше конечные точки службы, добавьте правило WASB:
        1. В **полные доменные имена целевых** укажите **имя**и задайте **исходные адреса** для `*`.
        1. Введите `http` или `https` в зависимости от того, если вы используете wasb: / / или wasbs: / / в разделе **протокола: порт** и URL-адрес учетной записи хранения в разделе **полные доменные ИМЕНА целевых**.
1. Щелкните **Добавить**.
...
1. Создайте следующие правила:
    ...
    1. Если вы используете хранилище Azure Data Lake, можно добавить правило сети для решения проблемы SNI с ADLS Gen1 и Gen2. Этот параметр будет направлять трафик к брандмауэру, что может привести к более высокими затратами для больших объемов данных, но и будет журнал и доступны для аудита.
        ...
        1. Задайте **порты назначения** для `*`.
    1. (Необязательно) Если вы используете Log Analytics, создайте правило сети для обеспечения взаимодействия с рабочей областью Log Analytics.
        1. В следующей строке в **правила** укажите **имя** и выберите **любой** из **протокола** раскрывающегося списка.
        1. Задайте **исходные адреса** `*`.
