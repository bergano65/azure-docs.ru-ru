...
ms.locfileid: "65762431"
---
# <a name="configure-outbound-network-traffic-restriction-for-azure-hdinsight-clusters"></a>Настройка ограничения исходящего сетевого трафика для кластеров Azure HDInsight
...
Решение для защиты исходящих адресов является использование устройства брандмауэра, можно контролировать исходящего трафика на основе имен домена. Брандмауэр Azure может ограничить исходящий трафик HTTP и HTTPS на основе FQDN назначения.
...
1. Добавьте следующие правила:
    ...
    1. Если в кластере реализуется WASB и вы не используете выше конечные точки службы, добавьте правило WASB:
        1. В **полные доменные имена целевых** укажите **имя**и задайте **исходные адреса** для `*`.
        1. Введите `wasb` под **протокола: порт** и `*` под **целевой полные доменные ИМЕНА**.
1. Щелкните **Добавить**.
