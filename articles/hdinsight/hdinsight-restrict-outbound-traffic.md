...
1. Добавьте следующие правила:
    ...
    1. Если WASB для кластера, затем добавьте правило для WASB.
        1. В **полные доменные имена целевых** укажите **имя**и задайте **исходные адреса** для `*`.
        1. Введите `http:80,https:443` под **протокола: порт** и URL-адрес учетной записи хранения в разделе **полные доменные ИМЕНА целевых**. Формат должен быть аналогичен < storage_account_name.blob.core.windows.net >. Для использования только протокола https подключений убедитесь, что [«требуется безопасное перемещение»](https://docs.microsoft.com/azure/storage/common/storage-require-secure-transfer) с включенной учетной записи хранения.
1. Щелкните **Добавить**.
...
1. Создайте следующие правила:
    1. Сетевые правила в разделе IP-адресов, который позволяет кластеру выполните синхронизацию времени с помощью NTP.
        1. В **правила** укажите **имя** и выберите **UDP** из **протокола** раскрывающегося списка.
        1. Задайте **исходных адресов** и **адресов назначения** для `*`.
        1. Задайте **порты назначения** на 123.
    1. Если вы используете пакет безопасности предприятия (ESP), затем добавьте правило сети в разделе IP-адресов, позволяющем связываться с доменных Служб AAD для кластеров ESP.
    ...
    1. Если вы используете хранилище Azure Data Lake, можно добавить правило сети в разделе IP-адресов для решения проблемы SNI с ADLS Gen1 и Gen2. Этот параметр будет направлять трафик к брандмауэру, что может привести к более высокими затратами для больших объемов данных, но и будет журнал и доступны для аудита в журналах брандмауэра.
        ...
        1. В следующей строке в **правила** укажите **имя** и выберите **TCP** из **протокола** раскрывающегося списка.
        1. Задайте **исходные адреса** `*`.
        1. Введите IP-адрес для учетной записи хранения **адресов назначения**.
        1. Задайте **порты назначения** для `*`.
    1. (Необязательно) Если вы используете Log Analytics, создайте правило сети в разделе IP-адресов для обеспечения взаимодействия с рабочей областью Log Analytics.
        1. В следующей строке в **правила** укажите **имя** и выберите **TCP** из **протокола** раскрывающегося списка.
        1. Задайте **исходные адреса** `*`.
        1. Задайте **адресов назначения** для `*`.
        1. Задайте **порты назначения** для `12000`.
    1. Настройка правила сети в разделе "теги службы" для SQL, который позволит вам регистрирование и аудит трафик SQL, если вы не настроили конечные точки службы для SQL Server в HDInsight подсети, в которой будут обходить брандмауэр.
        1. В следующей строке в **правила** укажите **имя** и выберите **TCP** из **протокола** раскрывающегося списка.
        1. Задайте **исходные адреса** `*`.
        1. Задайте **адресов назначения** для `*`.
