...
В этой статье приведены шаги для защиты исходящего трафика из кластера HDInsight с помощью Azure Firewall. Приведенные ниже шаги предполагают, что вы настраиваете брандмауэр Azure для существующего кластера. Если вы развертываете новый кластер за брандмауэром, сначала создайте кластер и подсеть HDInsight. Затем выполните шаги в этом руководстве.

## <a name="background"></a>Историческая справка

Кластеры HDInsight обычно развертываются в виртуальной сети. Кластер имеет зависимость от служб за пределами этой виртуальной сети.
...
Зависимость от исходящих трафика HDInsight почти полностью определена с помощью F-DN. Которые не имеют статических IP-адресов за ними. Отсутствие статических адресов означает, что группы сетевой безопасности (НСГ) не могут блокировать исходящий трафик из кластера. Адреса меняются достаточно часто, нельзя настроить правила, основанные на текущем разрешении и использовании имен.

Безопасные исходящие адреса с брандмауэром, который может управлять исходящим трафиком на основе доменных имен. Azure Firewall ограничивает исходящий трафик на основе F-DN назначения или [теги F-DN.](../firewall/fqdn-tags.md)
...
1. На экране **сбора правил приложения Add** укажите следующую информацию:
    ...
    **Целевой раздел ФЗН**
    ...
    | Имя | Адреса исходных источников | `Protocol:Port` | Целевая ПЗДНС | Примечания |
    | --- | --- | --- | --- | --- |
...
1. На экране **сбора правил сети Add** укажите следующую информацию:
    ...
    **Раздел "Теги обслуживания"**
    ...
    | --- | --- | --- | --- | --- | --- |
    | Rule_7 | TCP | * | SQL-код | 1433 | Настройте сетевое правило в разделе Теги обслуживания для S'L, которое позволит вам регистрировать и проверять трафик S'L. Если только вы не настроили конечные точки службы для сервера S'L в подсети HDInsight, которая обойдет брандмауэр. |
...
1. На экране **подсети Associate** выберите виртуальную сеть, в которую был создан кластер. И **Поднет,** который вы использовали для вашего кластера HDInsight.
...
После завершения настройки журналов, если вы используете Log Analytics, можно просмотреть заблокированный трафик с помощью такого запроса, как:

```Kusto
AzureDiagnostics | where msg_s contains "Deny" | where TimeGenerated >= ago(1h)
```

Интеграция брандмауэра Azure с журналами Azure Monitor полезна при первом заработке приложения. Особенно, когда вы не знаете обо всех зависимостях приложений. См. дополнительные сведения о журналах Azure Monitor в статье [Анализ данных журнала в Azure Monitor](../azure-monitor/log-query/log-query-overview.md)
...
Чтобы использовать публичную`https://CLUSTERNAME.azurehdinsight.net`конечную точку (`CLUSTERNAME-ssh.azurehdinsight.net`) или ssh конечную точку ( ), убедитесь, что у вас есть правильные маршруты в таблице маршрутов и правила NSG, чтобы избежать асимметричной проблемы маршрутизации объяснил [здесь](../firewall/integrate-lb.md). В частности, в этом случае необходимо разрешить IP-адрес клиента в правилах Inbound NSG, а также `internet`добавить его в таблицу маршрутов, определяемую пользователем, со следующим набором hop как. Если настройка реуктора не установлена правильно, вы увидите ошибку тайм-аута.
...
| IPs, [опубликованные здесь](hdinsight-management-ip-addresses.md) | Эти ит-изображения являются службой HDInsight |
