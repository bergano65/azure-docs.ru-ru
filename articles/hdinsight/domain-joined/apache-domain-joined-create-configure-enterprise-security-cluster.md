...
1. На странице **Создание виртуальной машины Azure с помощью нового леса AD** выполните следующие действия.
    ...
    1. В остальных полях шаблона введите следующие сведения:
        ...
        * **Имя пользователя администратора**: хдифабрикамадмин
...
1. Подключитесь к контроллеру домена с помощью удаленный рабочий стол.
    ...
    1. В портал Azure выберите **группы ресурсов** > **онпремадврг** > **адвм** > **Подключиться**.
...
1. Создайте два новых пользователя: **хдиадмин** и **HDIUser**. Эти два пользователя будут входить в кластеры HDInsight.
    ...
    1. На странице **Active Directory пользователи и компьютеры** выберите **действие** > **Новый** **пользователь** > .
...
1. Если вы хотите изменить пароль только что созданного пользователя \<fabrikamazureadmin@hdifabrikam.com>, используйте удостоверение для входа в портал Azure. Затем вам будет предложено изменить пароль.
...
1. Разверните узел **Сертификаты (локальный компьютер)**  > **Сертификаты** **личных** > . В **личном** хранилище должен присутствовать новый сертификат. Этот сертификат выдается полному имени узла.
...
![Создание назначаемого пользователем управляемого удостоверения](./media/apache-domain-joined-create-configure-enterprise-security-cluster/hdinsight-image-0117.png)
