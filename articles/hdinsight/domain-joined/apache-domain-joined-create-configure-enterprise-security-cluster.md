...
1. На странице **Создание виртуальной машины Azure с помощью нового леса AD** укажите следующие сведения.
    ...
    |Свойство | Значение |
    |---|---|
    |Subscription|Выберите подписку, в которой требуется развернуть ресурсы.|
...
1. Подключитесь к контроллеру домена с помощью удаленный рабочий стол.
    1. В портал Azure перейдите к **группе ресурсов** > **онпремадврг** > **адвм** > **Connect**.
    1. В раскрывающемся списке **IP-адрес** выберите общедоступный IP-адрес.
    1. Выберите **скачать RDP-файл**, а затем откройте файл.
    1. В качестве имени пользователя используйте `HDIFabrikam\HDIFabrikamAdmin`.
    1. Введите пароль, выбранный для учетной записи администратора.
    1. Нажмите кнопку **ОК**.

1. На панели мониторинга **Диспетчер сервера** контроллер домена перейдите в раздел **средства** > **Active Directory пользователи и компьютеры**.
...
1. Создайте два новых пользователя: **хдиадмин** и **HDIUser**. Эти два пользователя будут входить в кластеры HDInsight.
    ...
    1. На странице **Active Directory пользователи и компьютеры** щелкните правой кнопкой мыши `HDIFabrikam.com`, а затем перейдите к **новому** > **пользователя**.
    ...
    1. На странице **новый объект-пользователь** введите `HDIUser` для **имени** и **имени входа пользователя**. Другие поля будут заполнены автоматически. Выберите **Далее**.
    ...
    1. Повторите описанные выше действия, чтобы создать `HDIAdmin`пользователя.
...
1. Создайте глобальную группу безопасности.
    ...
    1. В **Active Directory пользователи и компьютеры**щелкните правой кнопкой мыши `HDIFabrikam.com`, а затем перейдите к **новой** **группе** > .
...
1. В поле **первоначальное доменное имя**введите `HDIFabrikamoutlook`.
...
1. Выберите **+ Добавить личный домен**.
...
1. Перейдите к разделу **управление** > **пользователей** > **нового пользователя**.
...
1. На странице **Подключение к домен Active Directory службам** введите имя пользователя и пароль для учетной записи администратора предприятия. Используйте имя пользователя `HDIFabrikam\HDIFabrikamAdmin` и пароль, созданный ранее. Выберите **Далее**.
...
1. На странице **Настройка входа в Azure AD** нажмите кнопку **Далее**.
   ![страницу "Конфигурация входа в Azure AD"](./media/apache-domain-joined-create-configure-enterprise-security-cluster/hdinsight-image-0062.png)
...
1. На странице **Конфигурация завершена** выберите **Выйти**.
   ![страницу «Настройка завершена»](./media/apache-domain-joined-create-configure-enterprise-security-cluster/hdinsight-image-0078.png)
...
1. Войдите на портал Azure.
1. Выберите **создать ресурс** и введите `managed identity`. Выберите **назначенное пользователем управляемое удостоверение** > **создать**.
1. В поле **имя ресурса**введите `HDIFabrikamManagedIdentity`.
...
1. Нахождение IP-адресов пользовательских DNS-серверов.
    1. Выберите ресурс `HDIFabrikam.com` Azure AD DS.
...
> [!NOTE]  
> Для формирования запроса SSL-сертификата можно использовать любую программу или приложение, которое создает допустимый запрос PKCS \#10.
...
1. Включите LDAPs для управляемого домена Azure AD DS.
    1. В портал Azure выберите `HDIFabrikam.com`домена.
...
1. Настройте пользовательский DNS-сервер в виртуальной сети **хдифабрикам-ааддсвнет** .
    1. Перейдите в раздел портал Azure > **группы ресурсов** > **онпремадврг** > **Хдифабрикам-ааддсвнет** > **DNS-серверы**.
