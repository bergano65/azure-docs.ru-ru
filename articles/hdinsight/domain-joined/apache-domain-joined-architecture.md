...
description: Узнайте, как спланировать безопасность Azure HDInsight с помощью пакета корпоративной безопасности.
...
- Домен Active Directory (под управлением Azure AD DS). **Доменное имя должно быть 39 символов или меньше для работы с Azure HDInsight.**
...
Если вы используете федерацию с помощью служб Ытвейра (AD FS), необходимо включить синхронизацию хэша паролей. (Для рекомендуемой установки, [см. это видео](https://youtu.be/qQruArbu2Ew).) Синхронизация хэша паролей помогает при аварийном восстановлении в случае сбоя инфраструктуры AD FS, а также обеспечивает защиту от утечки учетных данных. Дополнительные сведения см. в статье [Реализация синхронизации хэшированных паролей в службе синхронизации Azure AD Connect](../../active-directory/hybrid/how-to-connect-password-hash-synchronization.md).
...
Если используется федерация и хэши паролей синхронизируются правильно, но вы получаете сбои аутентификации, проверьте, включена ли проверка подлинности пароля в облаке для принципаслужбы службы PowerShell. Если нет, вы должны установить [Политику обнаружения домашней области (HRD)](../../active-directory/manage-apps/configure-authentication-for-federated-users-portal.md) для вашего клиента Azure AD. Чтобы проверить и установить политику HRD, выполните следующие действия.

1. Установите [предварительный модуль Azure AD PowerShell.](https://docs.microsoft.com/powershell/azure/active-directory/install-adv2)

   ```powershell
   Install-Module AzureAD
   ```

2. Подключение с помощью учетных данных глобального администратора (администратора арендатора).
...
4. Если его не существует, создайте принцип службы.
