...
Кластеры Azure HDInsight обеспечивают многопользовательский доступ к кластерам Корпоративный пакет безопасности (ESP). Кластеры HDInsight с ESP подключены к домену. Это подключение позволяет пользователям домена использовать учетные данные домена для проверки подлинности в кластерах и выполнения заданий обработки больших данных.
...
В следующем примере создается самозаверяющий сертификат. Доменное имя *contoso100.onmicrosoft.com* находится в обоих `Subject` полях (имя субъекта `DnsName` ) и (альтернативное имя субъекта).
...
Назначение роли **участника доменных служб HDInsight** гарантирует, что это удостоверение имеет`on behalf of`надлежащий () доступ для выполнения операций доменных служб в домене Azure AD DS. Эти операции включают создание и удаление подразделений.
