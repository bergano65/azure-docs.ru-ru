...
* По умолчанию у пользователей нет доступа к папке **/** в HDFS (для успешности доступа они должны быть включены в роль владельца BLOB-объектов хранилища).
* Для промежуточного каталога для MapReduce и других пользователей пользовательский каталог создается и предоставляется `sticky _wx` разрешений. Пользователи могут создавать файлы и папки под, но не могут просматривать другие элементы.
...
* для федеративных пользователей требуется политика `AllowCloudPasswordValidation`. Так как HDInsight использует имя пользователя и пароль непосредственно для получения маркеров из Azure AD, эта политика должна быть включена для всех федеративных пользователей.
...
* Создайте экземпляр с `.onmicrosoft.com domain`. Таким образом, не будет нескольких DNS-серверов, обслуживающих домен.
