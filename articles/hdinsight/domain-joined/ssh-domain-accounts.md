...
Чтобы изменить доступ по протоколу SSH к конкретным пользователям или `/etc/ssh/sshd_config` группам, обновите на каждом из узлов.
...
1. При необходимости `sshd_config` измените файл. Если ограничить пользователей определенными группами, локальные учетные записи не смогут получить доступ к этому узлу по протоколу SSH. Ниже приведен пример синтаксиса.
