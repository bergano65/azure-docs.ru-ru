...
При выполнении запроса, такого как `insert into table1 partition(a,b) select a,b,c from table2` план запроса запускает кучу модулей сжатия, но данные из каждой секции переходят к одному элементу reduce. Это приводит к тому, что запрос будет работать так же, как и в случае с более крупным уменьшением секции.
...
Если секции во входной таблице меньше (скажем, меньше 10), и, так, это количество выходных секций, а переменная имеет значение `true`, то данные будут глобально отсортированы и записаны с использованием одного уменьшения на секцию. Даже если число доступных модулей сжатия больше, модулей сжатия может быть задержкой за пределами данных, и не удается достичь максимального параллелизма. При изменении на `false`более одного из них может обрабатывать одну секцию, и будет записано несколько меньших файлов, что приведет к более быстрой вставке. Это может повлиять на последующие запросы из-за наличия файлов меньшего размера.
