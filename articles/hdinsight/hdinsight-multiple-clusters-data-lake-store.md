---
title: Несколько кластеров HDInsight & одной учетной записи хранения озер данных Azure
...
Data Lake Storage поддерживает неограниченное пространство для хранения данных, поэтому оно идеально подходит не только для хранения больших объемов данных, но и для размещения нескольких кластеров HDInsight, которые совместно используют одну и ту же учетную запись Data Lake Storage. Для получения инструкций о том, как создать кластер HDInsight с хранением данных озера в качестве хранилища, [см.](../storage/data-lake-storage/quickstart-create-connect-hdi-cluster.md)
...
- После того, как структура папки и разрешения на месте, процесс создания кластера HDInsight создает кластерное место хранения под **/кластерами/финансами/.** Например, в качестве расположения для кластера с именем fincluster01 может использоваться папка **/clusters/finance/fincluster01**. В таблице ниже показаны владельцы и разрешения для папок, созданных для кластера HDInsight.
...
Входные и выходные данные задания рекомендуется размещать в папке за пределами папки **/clusters**. Это гарантирует, что даже если папка для кластера будет удалена для восстановления некоторого пространства для хранения данных, входные данные и выводы по-прежнему доступны для использования в будущем. В этом случае убедитесь, что иерархия папок для хранения входных и выходных данных задания обеспечивает соответствующий уровень доступа для субъекта-службы.
...
При создании субъекта-службы с доступом именованного пользователя (как показано в таблице выше) рекомендуется **не** добавлять именованного пользователя со списком управления доступом по умолчанию. При подготовке доступа именованного пользователя со списками управления доступом по умолчанию назначаются разрешения 770 для пользователя-владельца, группы-владельца и остальных пользователей. В то время как это значение по умолчанию 770 не отнимает разрешения у владеющих пользователей (7) или владеющей группой (7), оно отнимает все разрешения для других (0). Это приводит к известной проблеме, которая подробно описана в разделе [Известные проблемы и их решения](#known-issues-and-workarounds).
...
Как указано в статье YARN JIRA, ссылка на которую приведена выше, при локализации общедоступных ресурсов средство локализации проверяет, что все запрошенные ресурсы действительно являются общедоступными, анализируя их разрешения в удаленной файловой системе. Любой LocalResource, который не соответствует этому условию, отклоняется для локализации. Проверка разрешений включает проверку доступа к файлу на чтение для остальных пользователей. Этот сценарий не работает нестандартно при размещении кластеров HDInsight на озере данных Azure, так как Azure Data Lake лишает доступ к "другим" на уровне корневой папки.
