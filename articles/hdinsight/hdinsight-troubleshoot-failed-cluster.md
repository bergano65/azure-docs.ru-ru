---
title: Устранение проблем медленной или неудачной работы в кластере Azure HDInsight
description: Диагностировать и устранить медленное или неудачное задание в кластере Azure HDInsight.
...
Если данные об обработке приложений в кластере HDInsight либо работают медленно, либо не работают с кодом ошибки, у вас есть несколько вариантов устранения неполадок. Если задания выполняются дольше ожидаемого времени или вы в целом наблюдаете малое время отклика, могут присутствовать неполадки с вышестоящими компонентами кластера, например, со службами, в которых он работает. Тем не менее самой распространенной причиной снижения производительности является недостаточное масштабирование. При создании нового кластера HDInsight выберите соответствующие [размеры виртуальных машин.](hdinsight-component-versioning.md#default-node-configuration-and-virtual-machine-sizes-for-clusters)
...
* Шаг 1: Сбор данных о проблеме.
* Шаг 2: Проверка кластерной среды HDInsight.
* Шаг 3: Просмотр работоспособности кластера.
* Шаг 4: Просмотрите стек среды и версии.
* Шаг 5: Изучите файлы журналов кластеров.
* Шаг 6: Проверьте настройки конфигурации.
* Шаг 7: Воспроизвести сбой на другом кластере.
...
Вы также можете использовать [Azure CLI:](https://docs.microsoft.com/cli/azure/?view=azure-cli-latest)
...
* Проверьте наличие перебоев в [работе службы Azure](https://azure.microsoft.com/status/).  
* Проверьте лимиты использования службы Azure. 
...
![Обзор приборной панели Apache Ambari](./media/hdinsight-troubleshoot-failed-cluster/apache-ambari-overview.png)
...
[Панель мониторинга пользовательского пульam ()](#view-cluster-configuration-settings-with-the-ambari-ui) `https://<clustername>.azurehdinsight.net`предоставляет обзор работоспособности кластеров, таких как время простоя, память, использование сети и процессора, использование дисков HDFS и так далее. Используйте раздел Ambari об узлах, чтобы просмотреть ресурсы на уровне узла. Можно также остановить и перезапустить службы.
...
Этот код является общим сообщением из узлов шлюза и является наиболее распространенным кодом состояния сбоя. Одна из возможных причин этой ошибки — служба WebHCat вышла из строя на активном головном узле. Чтобы проверить это, используйте команду CURL ниже:
...
![Apache Ambari перезапустить WebHCat Сервер](./media/hdinsight-troubleshoot-failed-cluster/restart-webhcat-server.png)
...
1. Отправка задания YARN может занять достаточно времени, чтобы вызвать истечение времени ожидания.
    ...
    ![HDInsight Работа пусковой очереди](./media/hdinsight-troubleshoot-failed-cluster/hdi-job-launcher-queue.png)
...
![Apache Ambari Стек и версии](./media/hdinsight-troubleshoot-failed-cluster/ambari-stack-versions.png)
...
4. Если первый шаг выполнен успешно, запустите следующий. При наличии ошибки проанализируйте ее в файлах журнала. Если это была ошибка в коде, сделайте коррекцию и перезапустите шаг.
...
* [Доступ к знаку приложения Apache Hadoop YARN в HDInsight на основе Linux](hdinsight-hadoop-access-yarn-app-logs-linux.md)
