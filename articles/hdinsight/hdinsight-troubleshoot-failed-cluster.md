---
title: Устранение задержки работы или сбоя задания в кластере HDInsight — Azure HDInsight
...
ms.locfileid: "65203625"
---
# <a name="troubleshoot-a-slow-or-failing-job-on-a-hdinsight-cluster"></a>Устранение задержки работы или сбоя задания в кластере HDInsight

Если приложение обработки данных в кластере HDInsight выполняется медленно, или произошел сбой с кодом ошибки, у вас есть несколько вариантов устранения неполадок. Если задания выполняются дольше ожидаемого времени или вы в целом наблюдаете малое время отклика, могут присутствовать неполадки с вышестоящими компонентами кластера, например, со службами, в которых он работает. Тем не менее самой распространенной причиной снижения производительности является недостаточное масштабирование. При создании нового кластера HDInsight, выберите соответствующий [размеры виртуальных машин](hdinsight-component-versioning.md#default-node-configuration-and-virtual-machine-sizes-for-clusters).
...
* Шаг 1. Соберите данные о проблеме.
* Шаг 2. Проверка среды кластера HDInsight.
* Шаг 3. Просмотрите состояние работоспособности кластера.
...
## <a name="step-1-gather-data-about-the-issue"></a>Шаг 1. Сбор данных о проблеме
...
* Поиск [простоев службы Azure](https://azure.microsoft.com/status/).  
...
2. Обработка YARN может занять длительное время, что может привести к истечению времени ожидания.
    ...
    * Получение списка заданий, выполненных более семи дней назад: на сервере HDInsight YARN JobHistoryServer настроено хранение сведений о завершенных заданиях в течение 7 дней (значение `mapreduce.jobhistory.max-age-ms`). Попытка перечислить результаты очищенных заданий может вызвать истечение времени ожидания.
...
## <a name="step-4-review-the-environment-stack-and-versions"></a>Шаг 4. Проверка версий и стека среды.
