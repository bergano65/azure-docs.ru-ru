...
Эти полные доменные имена будут иметь `<node-type-prefix><instance-number>-<abbreviated-clustername>.<unique-identifier>.cx.internal.cloudapp.net`форму.

Будет использоваться *HN* для головных узлах, *WN* для рабочих узлов и зн для Zookeeper узлов. *zn* `<node-type-prefix>`
...
- Конечная точка HTTPS в виртуальной сети `CLUSTERNAME-int.azurehdinsight.net`. Обратите внимание`-int`на "" в этом URL-адресе. Эта конечная точка будет разрешаться в частный IP-адрес в этой виртуальной сети и недоступна из общедоступного Интернета.
