...
От времени до времени из сеанса SSH с кластером может появиться сообщение о том, что обновление доступно. Сообщение может выглядеть примерно так:
...
- [Использование действия скрипта при создании кластера](hdinsight-hadoop-customize-cluster-linux.md#script-action-during-cluster-creation)
