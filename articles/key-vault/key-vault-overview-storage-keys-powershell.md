...
Учетная запись хранения Azure использует учетные данные, включающие имя учетной записи и ключ. Ключ является автоматически генерируемым и служит паролем, а не криптографическим ключом. Key Vault управляет ключами учетной записи хранилища, храним их в качестве [секретов Key Vault.](/azure/key-vault/about-keys-secrets-and-certificates#key-vault-secrets) 
