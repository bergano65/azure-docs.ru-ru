...
- [Установка и получение секрета из Azure Key Vault с помощью Azure CLI](quick-create-cli.md)
...
Добавьте необходимый код для обращения к хранилищу ключей и получения значения секрета. Сначала необходимо добавить следующую инструкцию `using`:
...
> [!IMPORTANT]
> Модули Runbook службы автоматизации Azure по-прежнему нуждаются в использовании модуля `AzureRM`.
...
3. Выберите **создать ресурс** > **Корпоративная интеграция** > **Service Bus**, а затем введите необходимые сведения.
...
После создания приложения-функции Azure перейдите к нему и выберите сценарий **таймера** и **\#C** для этого языка. Затем выберите **создать эту функцию**.
...
[Создайте приложение логики](../logic-apps/quickstart-create-first-logic-app-workflow.md) , выбрав **создать ресурс** > **Интеграция** > **приложение логики**.
