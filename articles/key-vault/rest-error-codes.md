...
Токены кодируются в кодировке Base64, и значения могут быть декодированы на веб-сайтах, например [http://jwt.calebb.net](http://jwt.calebb.net). Ниже приведена декодированная лексема:
...
Сначала включите ведение журнала. Инструкции о том, как это сделать, см. в разделе [ведение журнала Azure Key Vault](key-vault-logging.md).
...
В общем случае запросы к Key Vault ограничены 2000 запросами/10 секунд. Исключения являются ключевыми операциями, как описано в разделе [ограничения службы Key Vault](key-vault-service-limits.md)
...
- При выполнении регулирования Key Vault адаптировать запрашивающий код для использования экспоненциальной задержки при повторной попытке. Алгоритм описан здесь: [регулирование приложения](key-vault-ovw-throttling.md#how-to-throttle-your-app-in-response-to-service-limits)
...
Подробное руководство, в том числе запрос на увеличение ограничений, можно найти здесь: [Key Vault руководство по регулированию](key-vault-ovw-throttling.md)
