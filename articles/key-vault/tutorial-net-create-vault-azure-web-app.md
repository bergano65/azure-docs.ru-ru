...
При активации MSI для службы Azure (например, Виртуальных машин, Службы приложений или Функций) Azure создает [субъект-службу](key-vault-whatis.md#basic-concepts). MSI делает это для экземпляра службы в Azure Active Directory (Azure AD) и добавляет учетные данные для субъекта-службы в этот экземпляр.
