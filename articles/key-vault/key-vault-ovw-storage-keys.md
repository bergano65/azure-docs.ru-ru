...
- Задайте `<YourSASDefinitionName>`определения подписанного URL для общего доступа. Определение задается в Key Vault управляемой учетной записью хранения, `<YourStorageAccountName>` в хранилище ключей `<YourKeyVaultName>`.
