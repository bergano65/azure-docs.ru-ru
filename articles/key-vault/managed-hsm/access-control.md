...
---
# <a name="managed-hsm-access-control"></a>Управляемый контроль доступа HSM
...
Azure Key Vault управляемый HSM — это облачная служба, которая защищает ключи шифрования. Так как эти данные являются конфиденциальными и являются критически важными для бизнеса, необходимо обеспечить безопасный доступ к управляемым HSM, разрешая доступ к ней только пользователям, прошедшим проверку подлинности. В этой статье представлен обзор управляемой модели контроля доступа HSM. В нем объясняется проверка подлинности и авторизация, а также описывается, как защитить доступ к управляемой HSM.
...
- Руководство по началу работы для администратора см. в статье [что такое управляемый HSM?](overview.md).
- Руководство по управлению ролями см. в разделе [Managed HSM Local RBAC](role-management.md)
- Дополнительные сведения о ведении журналов использования управляемого модуля HSM см. в разделе [управляемое](logging.md)протоколирование HSM.
