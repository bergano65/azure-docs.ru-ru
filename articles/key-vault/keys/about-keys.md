...
Azure Key Vault поддерживает программные и аппаратные ключи.
...
- **Аппаратные ключи**. Ключи, которые обрабатываются непосредственно в HSM. Такие ключи защищены одной из систем безопасности HSM Key Vault (в каждом регионе есть системы безопасности для обеспечения изоляции). Клиенты могут импортировать ключ RSA или EC (программными средствами либо путем экспорта с совместимого устройства HSM). Клиенты могут также запросить создание ключа в Key Vault. Этот тип ключа добавляет атрибут key_hsm к JWK, позволяющий получить материал ключа HSM.
