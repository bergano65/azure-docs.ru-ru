...
Функция «мягкое удаление» изначально доступна через интерфейсы [REST,](/rest/api/keyvault/) [CLI,](key-vault-soft-delete-cli.md) [PowerShell](key-vault-soft-delete-powershell.md) и [.NET/C.](/dotnet/api/microsoft.azure.keyvault?view=azure-dotnet)
...
При создании нового хранилища ключей по умолчанию происходит удаление. Вы можете создать хранилище ключей без мягкого удаления через [Azure CLI](key-vault-soft-delete-cli.md) или [Azure Powershell.](key-vault-soft-delete-powershell.md) После включения мягкого удаления в хранилище ключей он не может быть отключен
...
Защита очистки является дополнительным поведением Key Vault и **не включена по умолчанию.** Он может быть включен через [CLI](key-vault-soft-delete-cli.md#enabling-purge-protection) или [Powershell](key-vault-soft-delete-powershell.md#enabling-purge-protection).
