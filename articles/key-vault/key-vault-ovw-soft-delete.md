...
Обратимое удаление — это дополнительная функция Key Vault, которая **не включена по умолчанию** в этом выпуске. Ее можно включить с помощью [интерфейса командной строки](key-vault-soft-delete-cli.md) или [PowerShell](key-vault-soft-delete-powershell.md).

### <a name="purge-protection"></a>Очистка защиты 

Если включена защита от удаления, хранилище или объект в удаленном состоянии нельзя очистить до тех пор, пока не истечет срок хранения в 90 дней. Эти хранилища и объекты по-прежнему можно восстановить, гарантируя клиентам, что политика хранения будет соблюдена. 
