...
Функция обратимого удаления изначально доступна через интерфейсы [RESTful](/rest/api/keyvault/), [CLI](key-vault-soft-delete-cli.md), [PowerShell](key-vault-soft-delete-powershell.md) и [.NET/C# ](/dotnet/api/microsoft.azure.keyvault?view=azure-dotnet) .
...
Обратимое удаление теперь включено по умолчанию для вновь созданных хранилищ ключей. Его можно отключить с помощью [Azure CLI](key-vault-soft-delete-cli.md) или [Azure PowerShell](key-vault-soft-delete-powershell.md).

По умолчанию срок хранения составляет 90 дней, но можно задать для интервала политики хранения значение от 7 до 90 дней после портал Azure. Политика хранения с очисткой защиты использует тот же интервал. 

После настройки обратимого удаления в хранилище ключей его нельзя отключить, и изменить интервал политики хранения невозможно. 
...
Очистка защиты — это необязательное поведение Key Vault и **не включено по умолчанию**. Ее можно включить с помощью [интерфейса командной строки](key-vault-soft-delete-cli.md#enabling-purge-protection) или [PowerShell](key-vault-soft-delete-powershell.md#enabling-purge-protection).
...
- Если в самом хранилище включен флаг--enable-очистить-Protection. В этом случае Key Vault будет ожидать 90 дней с момента, когда исходный объект секрета был помечен для удаления, прежде чем окончательно удалить объект.
