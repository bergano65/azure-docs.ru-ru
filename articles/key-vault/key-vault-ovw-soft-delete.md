...
Функция обратимого удаления изначально доступна через [REST](/rest/api/keyvault/), [CLI](key-vault-soft-delete-cli.md), [PowerShell](key-vault-soft-delete-powershell.md) и [.NET /C# ](/dotnet/api/microsoft.azure.keyvault?view=azure-dotnet) интерфейсов.
...
Обратимое удаление — это дополнительная функция Key Vault, которая **не включена по умолчанию** в этом выпуске. Его можно включить с помощью [CLI](key-vault-soft-delete-cli.md) или [Powershell](key-vault-soft-delete-powershell.md).

### <a name="purge-protection"></a>Очистка защиты 

Если очистка защита включена, хранилище, или не удается очистить объект в удаленном состоянии, пока не истечет срок 90 дней. Эти хранилища и объекты можно восстановить, обеспечение клиентам, что вы будете соблюдать политику хранения. 

Очистка защиты — это дополнительный режим работы хранилища ключей и является **не включена по умолчанию**. Его можно включить с помощью [CLI](key-vault-soft-delete-cli.md#enabling-purge-protection) или [Powershell](key-vault-soft-delete-powershell.md#enabling-purge-protection).
...
Не распространяется.
- Когда подписка Azure была помечена как *неудаляемая*. В этом случае только служба может выполнить фактическое удаление в рамках запланированной процедуры. 
- Когда enable Очистка protection флаг включен на само хранилище. В этом случае Key Vault будет ожидать 90 дней с момента, когда исходный объект секрета был помечен для удаления, прежде чем окончательно удалить объект.
...
После удаления объекта key vault, например ключ, служба переведет его в удаленном состоянии, сделав ее недоступной для любой операции извлечения. В этом состоянии объект хранилища ключей можно только внести в список, восстановить, а также принудительно или окончательно удалить. 
