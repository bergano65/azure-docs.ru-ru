...
* Набор инструментов задает свойства ключа клиента, который привязывает ваш ключ к системе безопасности хранилища ключей Azure. В связи с этим после, как только аппаратные модули безопасности хранилища ключей Azure получат и расшифруют ваш ключ, использовать его смогут только они. Экспортировать ключ нельзя. Эта привязка применяется по nCipher аппаратные модули безопасности.
* Ключ обмена ключами, используемый для шифрования вашего ключа, создается в аппаратных модулях безопасности хранилища ключей Azure и не может быть экспортирован. Аппаратные модули безопасности не допускают существования ключа обмена ключами в чистом виде за пределами аппаратных модулей безопасности. Кроме того набор инструментов включает аттестацию из nCipher, что ключ шифрования КЛЮЧЕЙ, не может быть экспортирован и был создан в подлинном аппаратном модуле безопасности, произведенная nCipher.
* Набор инструментов включает аттестацию из nCipher, что в систему безопасности Azure Key Vault также была создана в подлинном аппаратном модуле безопасности производства nCipher. Эта аттестация является доказательством того, что корпорация Майкрософт использует подлинное оборудование.
...
## <a name="more-information-about-ncipher-hsms-and-microsoft-services"></a>Дополнительные сведения о nCipher аппаратные модули безопасности и служб Майкрософт

nCipher безопасности — ведущий глобальный поставщик шифрования данных и решений по обеспечению безопасности электронных финансовых услуг, высоких технологий, производства, для государственных организаций и технологии секторов. 40 лет финансовое положение защиты корпоративной и государственной информации криптографических решений по обеспечению безопасности nCipher используются в четырех из пяти крупнейших энергетических и аэрокосмических компаний. Свои решения также используются программой 22 странах НАТО и регионах и защищают более 80 процентов по всему миру платежных транзакций.

Корпорация Майкрософт сотрудничает с nCipher безопасности, чтобы расширить возможности развития аппаратных модулей безопасности. Эти улучшения позволят вам пользоваться стандартными преимуществами размещаемых служб, не теряя контроль над ключами, а корпорации Майкрософт — управлять аппаратными модулями безопасности за вас. Поскольку хранилище ключей Azure является облачной службой, она легко масштабируется в соответствии с потребностями вашей организации. В то же время ваш ключ защищен в аппаратных модулях безопасности Майкрософт: вы получаете контроль над жизненным циклом ключа, так как создаете ключ и перемещаете его в аппаратные модули безопасности Майкрософт.
...
### <a name="step-11-install-azure-powershell"></a>Шаг 1.1. Установите Azure PowerShell
...
Хранилище ключей BYOK-Tools-SouthAfrica.zip
...
### <a name="step-21-prepare-the-disconnected-workstation-with-ncipher-nshield-hsm"></a>Шаг 2.1. Подготовка отключенной рабочей станции с nCipher nShield HSM
...
Дополнительные сведения см. в руководстве пользователя, включенном nShield HSM.
...
Если вы используете nCipher nShield Edge, чтобы изменить режим: 1. Используйте кнопку "Режим", чтобы выделить необходимый режим. 2. Через несколько секунд нажмите и удерживайте пару секунд кнопку "Очистить". Если режим изменится, светодиодный индикатор нового режима перестанет мигать и начнет светиться. Индикатор состояния может беспорядочно мигать в течение нескольких секунд. Затем, когда устройство будет готово, он станет мигать регулярно. В противном случае устройство останется в текущем режиме. При этом загорится соответствующий индикатор режима.

### <a name="step-32-create-a-security-world"></a>Шаг 3.2. Создание системы безопасности

Откройте командную строку и запустите программу nCipher новый мир.
...
* Ключ обмена ключами, включенный в набор средств, создан на основе genuine nCipher nShield HSM.
* Хэш системы безопасности, включенный в набор средств, создан в подлинном nCipher nShield HSM.
...
1. Запустите сценарий verifykeypackage.py, указав одно из приведенных ниже значений в зависимости от вашего географического региона или экземпляра Azure.
   ...
   * Для Южно-Африканская Республика:
   ...
   * Для Соединенного Королевства:
     ...
     > [!TIP]
     > Программное обеспечение nCipher nShield включает python в %NFAST_HOME%\python\bin
     >
     >
2. Должен появиться следующий текст, показывающий, что проверка пройдена. **Результат: успешно**

Этот сценарий проверяет цепочку подписавшего вплоть до корневого ключа nShield. Хэш этого корневого ключа встроен в сценарий и должен иметь значение **59178a47 de508c3f 291277ee 184f46c4 f1d9c639**. Вы можете также подтвердить это значение отдельно, посетив [nCipher веб-сайт](https://www.ncipher.com/products/key-management/cloud-microsoft-azure/validation).

Теперь можно создать новый ключ.

### <a name="step-35-create-a-new-key"></a>Шаг 3.5 Создание ключа

Создайте ключ с помощью nCipher nShield **generatekey** программы.
...
> [!IMPORTANT]
> После передачи ключа в хранилище ключей Azure корпорация Майкрософт не сможет экспортировать ключ обратно, поэтому крайне важно сделать резервную копию ключа и системы безопасности. Обратитесь к [nCipher](https://www.ncipher.com/about-us/contact-us) инструкции и рекомендации по резервному копированию ключа.
>
...
Теперь ключ, защищенный с помощью аппаратного модуля безопасности, можно использовать в хранилище ключей. Дополнительные сведения см. в разделе этого цен и функций [сравнения](https://azure.microsoft.com/pricing/details/key-vault/).
