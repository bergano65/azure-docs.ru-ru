...
- *Идентификатор (ID)* может использоваться для идентификации ресурса при восстановлении или удалении. 
...
Функция удаления используется для окончательного удаления объекта хранилища ключей или всего хранилища ключей, которое было ранее обратимо удалено. Как показано в предыдущем разделе, объекты, содержащиеся в хранилище ключей со включенной функцией обратимого удаления, могут быть в нескольких состояниях:
...
## <a name="enabling-purge-protection"></a>Включение защиты от удаления

При включении защиты от удаления хранилище или его объект в удаленном состоянии не могут быть удалены до истечения срока в 90 дней. Такое хранилище или его объект все еще подлежат восстановлению. Этот параметр обеспечивает для клиентов более высокий уровень защиты, гарантируя, что хранилище или его объект не смогут быть необратимо удалены до истечения срока хранения.

Защиту от удаления можно включить, только если включена функция soft-delete (обратимое удаление). 

Чтобы включить защиту с обратимым удалением и очисткой при создании хранилища, используйте командлет[New-AzKeyVault](/powershell/module/az.keyvault/new-azkeyvault?view=azps-1.5.0):

```powershell
New-AzKeyVault -Name ContosoVault -ResourceGroupName ContosoRG -Location westus -EnableSoftDelete -EnablePurgeProtection
```

Чтобы добавить защиту от удаления в существующем хранилище (для которого уже включено обратимое удаление), используйте командлеты [Get-AzKeyVault](/powershell/module/az.keyvault/Get-AzKeyVault?view=azps-1.5.0), [Get-AzResource](/powershell/module/az.resources/get-azresource?view=azps-1.5.0) и [Set-AzResource](/powershell/module/az.resources/set-azresource?view=azps-1.5.0):
...
- Общий обзор использования Хранилища Azure Key Vault см. в разделе [Что такое хранилище Azure Key Vault?](key-vault-overview.md).ate=Успешно}
