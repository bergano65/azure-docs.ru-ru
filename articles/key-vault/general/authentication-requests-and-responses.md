...
Здесь рассматривается служба Azure Key Vault. Общие сведения об использовании интерфейсов Azure REST, в том числе об аутентификации и авторизации, а также о том, как получить маркер доступа, см. в [справочнике по Azure REST API](/rest/api/azure).
...
> [!NOTE]
> Key Vault клиенты пакета SDK для секретов, сертификатов и ключей при первом вызове Key Vault не предоставляют маркер доступа для получения сведений о клиенте. Предполагается, что вы получаете HTTP 401 с помощью клиента Key Vault SDK, где Key Vault отображается в приложении WWW-Authenticate заголовок, содержащий ресурс, и клиент, где он должен пройти, и запросить маркер. Если все настроено правильно, второй вызов из приложения в Key Vault будет содержать действительный токен и будет успешно выполнен. 
