...
description: Сведения о том, как с помощью Azure PowerShell использовать функцию обратимого удаления Azure Key Vault, которая позволяет восстанавливать хранилища ключей и их объекты.
services: key-vault
...
- Очистка сертификата в удаленном состоянии: 

  > [!IMPORTANT]
  > Очистка сертификата приведет к его окончательному удалению, то есть восстановить его будет невозможно.
...
Если защита от очистки включена, хранилище или удаленный из него объект нельзя удалить безвозвратно, пока не пройдет период хранения. Такое хранилище или его объект все еще подлежат восстановлению. Эта функция обеспечивает дополнительную гарантию того, что хранилище или его объект не будут безвозвратно удалены до истечения срока хранения. Период хранения по умолчанию составляет 90 дней. Но при создании хранилища ключей можно задать для интервала политики хранения значение от 7 до 90 дней. Для политики хранения защиты от очистки используется тот же интервал. После установки интервал политики хранения нельзя изменить.
...
Чтобы включить обратимое удаление и защиту от очистки при создании хранилища, используйте командлет [New-AzKeyVault](/powershell/module/az.keyvault/new-azkeyvault?view=azps-1.5.0):

```powershell
New-AzKeyVault -Name ContosoVault -ResourceGroupName ContosoRG -Location westus -EnableSoftDelete -EnablePurgeProtection
```

Чтобы добавить защиту от очистки в существующем хранилище (для которого уже включено обратимое удаление), используйте командлеты [Get-AzKeyVault](/powershell/module/az.keyvault/Get-AzKeyVault?view=azps-1.5.0), [Get-AzResource](/powershell/module/az.resources/get-azresource?view=azps-1.5.0) и [Set-AzResource](/powershell/module/az.resources/set-azresource?view=azps-1.5.0):
