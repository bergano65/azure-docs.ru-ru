...
Традиционно все настройки конфигурации веб-приложений сохраняются в файлах конфигурации, таких как Web.config. Эта практика приводит к проверке в секретных настройках, таких как учетные данные облака, для систем управления открытым исходным источником, таких как GitHub. В свою очередь безопасность может оказаться под угрозой, так как для ее обеспечения требуются дополнительные расходы на изменение исходного кода и перенастройку параметров разработки.

Чтобы убедиться в безопасности процесса разработки, создаются инструментарии и библиотеки фреймворков для надежного сохранения секретных параметров приложений при минимальных изменениях исходного кода или без такого изменения.
...
Если у вас уже есть веб-приложение, предоставьте веб-приложению доступ к Key Vault, чтобы он мог получить доступ к хранилищу ключей без хранения секретной конфигурации в настройках приложений или файлах. Поиск веб-приложения по его имени и добавить его так же, как вы предоставляете пользователям доступ.

    ![Добавление политики доступа к хранилищу Key Vault](../media/vs-secure-secret-appsettings/add-keyvault-access-policy.png)

3. Добавьте свой секрет в Key Vault на портале Azure. Чтобы имя секрета Key Vault было допустимым, замените ":" на "--" в параметрах вложенной конфигурации. ":" является недопустимым символом в имени секрета Key Vault.

    ![Добавление секрета Key Vault](../media/vs-secure-secret-appsettings/add-keyvault-secret.png)

    > [!NOTE]
    > До Visual Studio 2017 V15.6 мы рекомендовали установить расширение аутентификации Azure Services для Visual Studio. Но это deprecated сейчас, как функциональность интегрирована в Visual Studio . Следовательно, если вы находитесь на старой версии визуальной студии 2017 , мы предлагаем вам обновить, по крайней мере VS 2017 15.6 или выше, так что вы можете использовать эту функциональность наместе и получить доступ к key-vault от использования Visual Studio регистрации идентификации себя.
    >
...
2. Создайте файл, похожий на следующий файл. Сохраните его за пределами папки проекта.
...
4. Укажите в разделе appSettings, что используется построитель конфигурации секрета. Убедитесь, что есть запись для секретной настройки с фиктивным значением.
...
2. Определите строитель конфигурации Key Vault в Web.config. Поместите этот раздел в раздел *appSettings.* Замените имя Key Vault на *vaultName*, если ваше хранилище размещено в общедоступном облаке Azure, или укажите полный универсальный код ресурса (URI), если вы используете независимое облако.
