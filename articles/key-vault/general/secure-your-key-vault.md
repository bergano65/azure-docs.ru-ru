...
- **Пользователь плюс доступ к приложению:** Приложение получает доступ к Key Vault от имени пользователя, вписаваемого в нее. Такой доступ осуществляется с помощью Azure PowerShell и портала Azure. Есть два способа предоставления доступа пользователям. Пользователи могут получить доступ к Key Vault из любого приложения или только при использовании конкретного приложения (называемого _составным идентификатором_).
- **Доступ только для приложений**: Приложение выполняется как служба daemon или фоновая работа. Доступ к хранилищу ключей предоставляется по удостоверению приложения.
...
В этом примере мы разрабатываем приложение, использующее сертификат для TLS/SSL, Хранилище Azure для хранения данных и 2048-разрядный ключ RSA для операций знаков. Это приложение выполняется на виртуальной машине Azure (или в масштабируемом наборе виртуальных машин). Мы можем использовать хранилище ключей, чтобы хранить секреты приложения. Мы можем сохранить сертификат начальной загрузки, используемый приложением для аутентификации в Azure AD.

Нам нужен доступ к следующим сохраненным ключам и секретам:
- **Сертификат TLS/SSL:** Используется для TLS/SSL.
- **Ключ хранения:** Используется для доступа к учетной записи хранилища.
- **RSA 2048-разрядный ключ:** Используется для операций знака.
- **Сертификат Bootstrap**: Используется для проверки подлинности с помощью Azure AD. После предоставления доступа можно получить ключ к хранилищу данных и использовать ключ RSA для подписывания.
...
- **Команда безопасности**: ИТ-персонал из офиса ОГО (главный сотрудник по безопасности) или аналогичные участники. Служба безопасности несет ответственность за надлежащее хранение секретов. Секреты могут включать сертификаты TLS/SSL, ключи RSA для подписания, строки подключения и ключи учетной записи хранилища.
- **Разработчики и операторы**: Сотрудники, разрабатывающие приложение и развертывающие его в Azure. Участники этой команды не являются сотрудниками службы безопасности. Они не должны иметь доступ к конфиденциальным данным, таким как сертификаты TLS/SSL и ключи RSA. Только развертываемое ими приложение должно иметь доступ к конфиденциальным данным.
- **Аудиторы**: Эта роль предназначена для участников, которые не являются членами разработки или общего ИТ-персонала. Они проверяют надлежащее использование и обслуживание сертификатов, ключей и секретов, а также обеспечивают соответствие стандартам безопасности данных.
...
- Получите ссылки от группы безопасности для сертификатов bootstrap и TLS/SSL (отпечатки пальцев), ключа хранения (секретный URI) и ключа RSA (ключ URI) для подписания.
...
| Разработчики и&nbsp;операторы | Разрешение на развертывание Key Vault<br><br> **Примечание**: Это разрешение позволяет развернутые VMs для получения секретов из хранилища ключей. | None |
| Аудиторы | None | Ключи: перечисление<br>Секреты: перечисление<br><br> **Примечание**: Это разрешение позволяет аудиторам проверять атрибуты (теги, даты активации, даты истечения срока действия) для ключей и секретов, не исключаемых в журналах. |
...
- Скачать [образцы клиентов Azure Key Vault.](https://www.microsoft.com/download/details.aspx?id=45343) В них показано, как с помощью сертификата начальной загрузки выполнить аутентификацию в Azure AD и получить доступ к хранилищу ключей.
...
- Администратор Azure AD создал группы безопасности для представления трех ролей: Contoso Security Team, Contoso App DevOps и Contoso App Auditors. Администратор также добавил пользователей в группы, к которым они относятся.
...
Команда безопасности создает хранилище ключей и настраивает ведение журнала и права доступа.
...
Для наших сотрудников DevOps назначение пользовательских ролей для разрешения `deploy/action` хранилища ключей ограничивается группой ресурсов. Доступ к секретам разрешен только вдыхать в группу ресурсов **ContosoAppRG** (сертификаты TLS/SSL и bootstrap). Виртуальные машины, созданные в других группах ресурсов участником DevOps, не могут получить доступ к этим секретам, даже если у виртуальной машины есть URI секретов.
...
* [Ключевые Vault управления REST APIs](https://msdn.microsoft.com/library/azure/mt620024.aspx)
...
* [Ключевые Vault REST APIs](https://msdn.microsoft.com/library/azure/dn903609.aspx)
...
Для получения начала учебник для администратора, [см. Что такое Azure Key Vault?](overview.md)

Для получения дополнительной информации о регистрации использования для Key Vault см. [журнал Azure Key Vault).](logging.md)

Для получения дополнительной информации об использовании ключей и секретов с Azure Key Vault, [см.](https://msdn.microsoft.com/library/azure/dn903623.aspx)
