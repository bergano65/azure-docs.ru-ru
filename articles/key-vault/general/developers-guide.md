...
Для защищенных приложений рекомендуется использовать управляемое удостоверение для приложений, развернутых в Azure. Если службы Azure, которые не поддерживают управляемое удостоверение или приложения, развернутые локально, [субъект-служба с сертификатом](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal) является возможной альтернативой. Сертификат должен храниться в Key Vault и поворачиваться часто. 

Субъект-службу с секретом можно использовать для сред разработки и тестирования, а также для локальной среды или в Cloud Shell субъекта пользователя.

Для разработки приложений пакет SDK для удостоверений Azure можно использовать в разных средах и платформах без изменения кода. Удостоверение Azure интегрируется с Azure CLI, Visual Studio, Visual Studio Code и другими. 
