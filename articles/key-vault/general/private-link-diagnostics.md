...
### <a name="symptoms-covered-by-this-article"></a>Симптомы, описываемые в этой статье

- Запросы DNS по-прежнему возвращают общедоступный IP-адрес для хранилища ключей, а не частный IP-адрес, с помощью которого вы ожидаете использовать функцию private Link.
...
### <a name="symptoms-not-covered-by-this-article"></a>Симптомы, не описываемые в этой статье
...
- Вы используете и продукт Azure, поддерживающий BYOK (создание собственных ключей) или CMK (управляемые пользователем ключи), и этот продукт не может получить доступ к вашему хранилищу ключей. *Ознакомьтесь с другой документацией по продукту. Убедитесь, что он явно указывает на поддержку хранилищ ключей с включенным брандмауэром. При необходимости обратитесь в службу технической поддержки для конкретного продукта.*
...
Это руководство по устранению неполадок применимо к подключениям к хранилищу ключей, которые берутся из кода приложения. Примеры — это приложения и сценарии, выполняемые на виртуальных машинах, кластерах Service Fabric Azure, службе приложений Azure, службе Azure Kubernetes (AKS) и других.
...
Если псевдоним не отображается `privatelink` , это означает, что в хранилище ключей нет подключений к частным конечным точкам в `Approved` состоянии. Продолжайте чтение этой статьи.
...
Обычно этот ресурс создается автоматически при создании частной конечной точки с помощью типичного метода. Но бывают случаи, когда этот ресурс не создается автоматически, и вам потребуется выполнить его вручную. Этот ресурс также мог быть случайно удален.
...
>[!NOTE]
> При каждом удалении или изменении `A` записи компьютер по-прежнему может разрешить доступ к старому IP-адресу, так как срок жизни значения TTL (срок жизни) еще не истек. Рекомендуется всегда указывать значение TTL не менее 60 секунд (одна минута) и не больше 600 секунд (10 минут). Если указано слишком большое значение, клиенты будут иметь проблемы с восстановлением после простоев.
...
### <a name="fact-the-user-controls-dns-resolution"></a>Факт. пользователь управляет разрешением DNS.

Если вы являетесь счоларом сети или интересного человека, вы, вероятно, поняли, как работает разрешение DNS. Как описано в [предыдущем разделе](#key-vault-with-private-link-resolving-from-arbitrary-internet-machine), хранилище ключей с частными ссылками будет иметь псевдоним `{vaultname}.privatelink.vaultcore.azure.net` в *общедоступной* регистрации. DNS-сервер, используемый виртуальной сетью, будет проверять все псевдонимы для регистрации *закрытого* имени, и если оно будет найдено, оно перестанет отвечать за псевдонимы общедоступной регистрации.

Например, рассмотрим, что виртуальная сеть связана с Частная зона DNS зоной с именем `privatelink.vaultcore.azure.net` , а общедоступная регистрация DNS для хранилища ключей имеет псевдоним `fabrikam.privatelink.vaultcore.azure.net` . Обратите внимание, что суффикс полностью соответствует имени зоны Частная зона DNS. Это означает, что разрешение сначала будет выглядеть для `A` записи с именем `fabrikam` в зоне частная зона DNS. Если `A` запись найдена, ее IP-адрес будет возвращен в запросе DNS. И этот IP-адрес просто является частным IP-адресом хранилища ключей.

Как видите, все разрешение имен находится под контролем пользователя.
