...
> [!NOTE]
> В этом руководстве нет инструкций по созданию хранилищ ключей, ключей и секретов. Дополнительные сведения см. в [обзорной статье об Azure Key Vault](overview.md). Инструкции по кроссплатформенному интерфейсу Azure CLI см. в [этом руководстве](manage-with-cli2.md).
>
...
Общие сведения о Key Vault см. в статье [Что такое хранилище ключей Azure?](overview.md) Сведения о регионах доступности Key Vault можно узнать на [странице цен](https://azure.microsoft.com/pricing/details/key-vault/).
...
В [руководстве по началу работы](../secrets/quick-create-cli.md) использовалось имя **ContosoKeyVault** для хранилища ключей. Мы сохраним это же имя и сохраним его в переменной с именем **kv**:
...
Руководство по смене ключей и аудиту журналов с помощью Azure Key Vault см. в статье [о настройке полной смены ключей и аудита в Key Vault](../secrets/key-rotation-log-monitoring.md).
