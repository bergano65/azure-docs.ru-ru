...
В этом списке вопросов и ответов от Майкрософт приведены часто задаваемые вопросы об Azure Monitor.
...
### <a name="why-am-i-am-getting-no-access-error-message-when-opening-log-analytics-from-a-vm"></a>Почему я получаю сообщение об ошибке "Нет доступа" при входе в Log Analytics на виртуальной машине? 
...
Правило действия позволяет изменить поведение набора оповещений, которые соответствуют определенным критериям. Это позволяет выполнять такие задачи, как отключение действий оповещений на период обслуживания. Вы также можете применить группу действий к набору оповещений, а не напрямую к правилам генерации оповещений. См. статью [Правила действий](platform/alerts-action-rules.md).
...
*Сколько ресурсов Application Insights мне следует развернуть?*
...
Параметр **Другие процессы** позволяет выяснить причину чрезмерного потребления ресурсов на узле. Благодаря ему вы сможете отличать потребление, связанное с контейнерными и неконтейнерными процессами.
...
Измените запросы, добавив свойства Image и ImageTag из таблицы ```ContainerInventory``` и выполнив объединение с ней по свойству ContainerID. Вы можете добавить свойство Name (так как ранее оно присутствовало в таблице ```ContainerLog```) из поля ContaineName таблицы KubepodInventory, настроив объединение с ней по свойству ContainerID, и мы рекомендуем использовать этот вариант.
...
### <a name="which-ports-and-domains-do-i-need-to-openwhitelist-for-the-agent"></a>Какие порты и домены нужно открыть или включить в список разрешений для агента?
