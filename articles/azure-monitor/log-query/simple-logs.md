...
Перейдите к любому Cosmos DB или Key Vault в подписке Azure с [параметрами диагностики, настроенными для получения журналов в рабочей области log Analytics](../platform/diagnostic-logs-stream-log-store.md). Щелкните **журналы** в меню **мониторинг** , чтобы открыть простой журнал.
