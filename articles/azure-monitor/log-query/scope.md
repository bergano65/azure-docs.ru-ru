...
![Область, отображаемая на портале](media/scope/scope.png)
...
> [!IMPORTANT]
> Если вы используете приложение на основе рабочей области в Application Insights, его данные хранятся в Log Analytics рабочей области со всеми остальными данными журнала. Для обеспечения обратной совместимости вы получите классическую Application Insightsную работу при выборе приложения в качестве области. Чтобы просмотреть эти данные в рабочей области Log Analytics, задайте для области значение Рабочая область.
...
| групп ресурсов; | Записи, созданные всеми ресурсами в группе ресурсов. Может включать данные из нескольких рабочих областей Log Analytics. | Выберите **журналы** в меню Группа ресурсов. | Не удается изменить область.|
...
Диапазон времени определяет набор записей, которые оцениваются для запроса на основе времени создания записи. Это определяется стандартным столбцом для каждой записи в рабочей области или приложении, как указано в следующей таблице.

| Расположение | Столбец |
|:---|:---|
...
![Управляющий элемент выбора времени](media/scope/time-picker.png)
...
При использовании [рабочей области](workspace-expression.md) или команды [приложения](app-expression.md) для получения данных из другой рабочей области или приложения средство выбора времени может вести себя по-разному. Если область является Log Analytics рабочей областью и используется **приложение**или если область является Application Insights приложением и используется **Рабочая область**, то log Analytics может не понять, что столбец, используемый в фильтре, должен определить фильтр времени.

В следующем примере область задается в Log Analytics рабочей области.  Запрос использует **рабочую область** для получения данных из другой log Analytics рабочей области. Выбор времени изменяется для **установки в запросе** , так как он видит фильтр, использующий ожидаемый столбец **timegenerated** .

![Запрос с рабочей областью](media/scope/query-workspace.png)

Если запрос использует **приложение** для извлечения данных из Application Insights приложения, log Analytics не распознает столбец **отметок времени** в фильтре, а средство выбора времени остается неизменным. В этом случае применяются оба фильтра. В этом примере в запрос включаются только записи, созданные за последние 24 часа, хотя в предложении **WHERE** указывается 7 дней.
