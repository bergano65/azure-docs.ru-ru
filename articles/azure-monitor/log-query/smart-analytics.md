...
![Выходные данные анализа когорт](media/smart-analytics/cohorts.png)
...
![Обработка ежемесячных выходных данных пользователей](media/smart-analytics/rolling-mau.png)

В следующем примере приведенный выше запрос преобразовывается в повторно используемую функцию и применяется для вычисления обработки данных закрепления пользователей. Активными в этом запросе считаются пользователи, которые выполняли извлечения по крайней мере один раз в конкретный день.
...
![Выходные данные закрепления пользователей](media/smart-analytics/user-stickiness.png)
