...
Полезным применением `mvexpand` является необходимость заполнения значений по умолчанию для отсутствующих ячеек. Например, предположим, что вы ищете время простоя конкретной машины, исследуя ее сердцебиение. Также вы хотите просмотреть источник пульса, который находится в столбце _категории_. Как правило, мы используем простой оператор суммирования следующим образом:
