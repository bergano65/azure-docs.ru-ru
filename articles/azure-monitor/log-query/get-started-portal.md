...
Запросы могут начинаться с имени таблицы или команды **search**. Пример выше начинается с имени таблицы _Event_, которое определяет область запроса. Символ вертикальной черты (|) отделяет команды, поэтому выходные данные первой служит в качестве входных данных следующей команды. Вы можете добавить любое количество команд в один запрос.
