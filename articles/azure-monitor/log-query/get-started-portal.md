...
ms.locfileid: "67296518"
---
# <a name="get-started-with-log-analytics-in-azure-monitor"></a>Начало работы с Log Analytics в Azure Monitor

[!INCLUDE [log-analytics-demo-environment](../../../includes/log-analytics-demo-environment.md)]

В этом руководстве вы узнаете, как использовать Log Analytics на портале Azure для написания запросов к журналу Azure Monitor. Вы научитесь:

- Использование Log Analytics, чтобы создать простой запрос
...
Учебник по написанию запросов журнала, см. в разделе [приступить к работе с помощью запросов к журналу в Azure Monitor](get-started-queries.md).<br>
Дополнительные сведения о журнале запросов, см. в разделе [Общие сведения о журнале запросов в Azure Monitor](log-query-overview.md).
...
Этот запрос осуществляет _событий_ таблицу для записи, содержащие термин _ошибка_ в любом свойстве.

Запросы могут начинаться с имени таблицы или команды [search](/kusto/query/searchoperator). Приведенный выше пример начинается с имени таблицы _событий_, который получает все записи из таблицы событий. Символ вертикальной черты (|) отделяет команды, поэтому выходные данные первой служит в качестве входных данных следующей команды. Вы можете добавить любое количество команд в один запрос.
...
В этом примере **поиска** ограничивается _событий_ таблицу и все записи в этой таблице осуществляется поиск термина _ошибка_.
...
- Разрывы строк: Разрыв одного облегчает чтение запроса. Несколько разрывов строк разделяют его на отдельные запросы.
...
Схема представляет собой коллекцию таблиц, визуально сгруппированных в логической категории. Некоторые из категорий относятся к решениям мониторинга. _LogManagement_ категория содержит общие данные, такие как Windows и системного журнала событий, данные о производительности и периодических сигналов агента.
