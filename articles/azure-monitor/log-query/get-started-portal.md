...
- Разрывы строк. единый разрыв делает запрос более удобным для чтения. Несколько разрывов строк разделяют его на отдельные запросы.
...
![SCHEMA (Схема)](media/get-started-portal/schema.png)
...
>[!NOTE]
>Поддерживаются следующие символы: `a–z, A–Z, 0-9, -, _, ., <space>, (, ), |` в поле **имени** при сохранении или изменении сохраненного запроса.
