...
Чтобы отреагировать на событие и запустить runbook из результатов поиска по журналам, выполните поиск по журналам и на основе результатов вызовите runbook по запросу.  Это можно сделать через функцию поиска по журналам или на [портале Azure](../../azure-monitor/log-query/log-query-overview.md).  В этом примере мы выполняем поиск по журналам на портале Azure, демонстрируя основные возможности этой функции.
