...
- **Ограничение скорости** — клиент может иметь более высокие ограничения скорости приема только для выделенного кластера.
...
Выделенные кластеры поддерживаются только для рабочих областей, использующих планы за ГБ с уровнями резервирования емкости или без них. У выделенных кластеров нет дополнительной платы за клиентов, которые зафиксированы на получение более 1 ТБ для такого кластера. "Фиксация в приеме" означает, что им назначена категория резервирования емкостью не менее 1 ТБ/день на уровне кластера. Хотя резервирование емкости прикрепляется на уровне кластера, существует два варианта фактической зарядки данных:

- *Кластер* (по умолчанию). затраты на резервирование емкости кластера приводятся к ресурсу *кластера* .
- *Рабочие области* . затраты на резервирование ресурсов для кластера настраиваются пропорционально рабочим областям кластера. За ресурсы *кластера* взимается плата за использование, если общий объем принимаемых данных за день находится под резервированием емкости. Дополнительные сведения о модели ценообразования кластера см. в разделе [log Analytics выделенные кластеры](../platform/manage-cost-storage.md#log-analytics-dedicated-clusters) .

Дополнительные сведения о выставлении счетов за выделенные кластеры см. в разделе [log Analytics выставления счетов за выделенный кластер](../platform/manage-cost-storage.md#log-analytics-dedicated-clusters)
...
### <a name="link-operations"></a>Операции связывания
...
Выделенный ресурс кластера можно удалить. Необходимо разорвать связь всех рабочих областей с кластером, прежде чем удалять их. После удаления ресурса кластера физический кластер переходит в процесс очистки и удаления. При удалении кластера удаляются все данные, хранящиеся в кластере. Данные могут быть из рабочих областей, которые были связаны с кластером в прошлом.
