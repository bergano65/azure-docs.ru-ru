...
## <a name="cross-resource-query-limits"></a>Ограничения межресурсных запросов 

* Количество ресурсов Application Insights и рабочих областей анализа журналов, которые можно включить в один запрос, ограничено 100.
* Кросс-ресурсный запрос не поддерживается в «Квью-конструктор». Вы можете заставить запрос в журнале Analytics и прикрепить его к панели мониторинга Azure, чтобы [визуализировать запрос журнала.](../learn/tutorial-logs-dashboards.md) 
* Запрос между разными ресурсами в оповещениях журнала поддерживается в новом API [правил запросов по расписанию](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules). По умолчанию Azure Monitor использует [устаревшие API оповещения Log Analytics](../platform/api-alerts.md) для создания любого нового правила генерации оповещений на портале Azure, пока вы не переключаетесь с [устаревших API оповещений журнала](../platform/alerts-log-api-switch.md#process-of-switching-from-legacy-log-alerts-api). После переключения новый API используется по умолчанию для новых правил генерации оповещений на портале Azure и позволяет создавать правила генерации оповещений журнала запроса между разными ресурсами. Можно создать правила оповещения о перепозренных запросах, не делая переключатель, используя [шаблон менеджера ресурсов Azure для запланированного API,ruryRules](../platform/alerts-log.md#log-alert-with-cross-resource-query-using-azure-resource-template) - но это правило оповещения является управляемым, хотя [запланированный API,](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules) а не с портала Azure.
...
* Квалифицированное имя - это "полное имя" рабочего пространства, состоящее из названия подписки, группы ресурсов и имени компонента в этом формате: *subscriptionName/resourceGroup/componentName*. 
...
* Имя ресурса — это имя приложения в удобном для восприятия формате, которое иногда называется *именем компонента*.  
    ...
    >[!NOTE]
    >Определение приложения по имени предполагает уникальность всех доступных подписок. Если у вас есть несколько приложений с таким именем, запрос не выполнится из-за неоднозначности. В этом случае необходимо воспользоваться другим идентификатором.
