...
По умолчанию команда **search** проверит все столбцы в наборе данных. Для поиска только определенного столбца (названного *Источником* в приведенном ниже примере) используйте этот синтаксис:
...
> [!TIP]
> При использовании `==` вместо `:` результаты будут включать записи, в которых столбец *Source* имеет точное значение "error". Использование ':' будет включать записи, где *Источник* имеет значения, такие как "код ошибки 404" или "Ошибка".
