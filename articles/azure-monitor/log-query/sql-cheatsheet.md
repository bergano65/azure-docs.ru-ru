...
С помощью следующей таблицы пользователи, знакомые с SQL, смогут изучить язык запросов Kusto для составления запросов журнала в Azure Monitor. Осмотрите команду T-S'SL для решения общих сценариев и эквивалентв в запросе журнала Azure Monitor.
...
Топ n записей по меркам                |`SELECT TOP 100 name, COUNT(*) as Count FROM dependencies GROUP BY name ORDER BY Count asc`        |<code>dependencies <br>&#124; summarize Count=count() by name <br>&#124; top 100 by Count asc</code>
...
- Пройдите уроки [по написанию запросов журнала в Azure Monitor](get-started-queries.md).
