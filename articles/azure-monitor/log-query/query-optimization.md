...
- [Объем данных](#data-volume): общие данные, к которым был получен доступ для обработки запроса. Зависит от размера целевой таблицы, используемого диапазона времени, примененных фильтров и количества столбцов, на которые имеются ссылки.

- [Диапазон времени](#time-range): разрыв между последними и самыми старыми данными, которые были доступны для обработки запроса. Влияет на явный диапазон времени, указанный для запроса.

- [Время существования данных](#age-of-data): разрыв между Now и самыми старыми данными, к которым был получен доступ для обработки запроса. Это сильно влияет на эффективность выборки данных.
...
Обычно такая ошибка возникает, когда [arg_max ()](/azure/kusto/query/arg-max-aggfunction) используется для поиска последнего вхождения. Например:
...
> [!IMPORTANT]
> Этот индикатор недоступен для запросов между регионами.

## <a name="age-of-data"></a>Возраст данных
...
Azure Monitor журналы используют большие кластеры обозреватель данных Azure для выполнения запросов, и эти кластеры различаются в масштабе. Система автоматически масштабирует кластеры в соответствии с логикой и емкостью размещения рабочей области.
