...
### <a name="avoid-full-xml-and-json-parsing-when-string-parsing-works"></a>Избегайте полного разбора XML и JSON при разборе строк
Полное разбор объекта XML или JSON может потреблять большой процессор и ресурсы памяти. Во многих случаях, когда требуется только один или два параметра и объекты XML или JSON просты, их легче разбирать в виде строк с помощью [оператора анализа](/azure/kusto/query/parseoperator) или других [методов разбора текста.](/azure/azure-monitor/log-query/parse-text) Повышение производительности будет более значительным по мере увеличения числа записей в объекте XML или JSON. Это важно, когда количество записей достигает десятков миллионов.

Например, следующий запрос будет возвращать точно такие же результаты, как и вышеуказанные запросы, без выполнения полного xML-разбора. Обратите внимание, что он делает некоторые предположения о структуре файлов XML, таких как элемент FilePath приходит после FileHash и ни один из них не имеет атрибутов. 
