...
> - При установке значения `identity` `type` "нет" для кластера также отменяется доступ к данным, но этот подход не рекомендуется, так как вы не сможете отменить отзыв при переуказании `identity` в кластере, не открывая запрос на поддержку.
...
Хранилище кластера периодически проверяет Key Vault, чтобы попытаться расшифровать ключ шифрования и после доступа к нему возобновить прием данных и запрос в течение 30 минут.
...
## <a name="customer-managed-key-for-saved-queries"></a>Ключ, управляемый клиентом для сохраненных запросов
