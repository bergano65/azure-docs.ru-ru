---
title: Настройка агента Azure Log Analytics для гибридных компьютеров Windows | Документация Майкрософт
description: Из этого краткого руководства вы узнаете, как развернуть агент Log Analytics для компьютеров Windows, которые работают вне среды Azure, и включить сбор данных с помощью Log Analytics.
...
ms.locfileid: "65602979"
---
# <a name="configure-the-log-analytics-agent-for-windows-computers-in-a-hybrid-environment"></a>Настройка агента Log Analytics для компьютеров Windows в гибридной среде
[Azure Log Analytics](../../azure-monitor/platform/agent-windows.md) может собирать данные напрямую c физических компьютеров или виртуальных машин Windows в один репозиторий для подробного анализа и сопоставления. Log Analytics может собирать данные из центра обработки данных и других облачных окружений. В этом кратком руководстве показано, как настроить и собирать данные c компьютера Windows с помощью нескольких простых действий.  Сведения о виртуальных машинах Windows в Azure см. в статье [Сбор данных о виртуальных машинах Azure](../../azure-monitor/learn/quick-collect-azurevm.md).  
...
1. На портале Azure щелкните **Все службы**. В поле поиска введите **Log Analytics**. По мере ввода символов список отфильтруется соответствующим образом. Выберите **Log Analytics**.

    ![Портал Azure](media/quick-collect-windows-computer/azure-portal-01.png)
  
2. Щелкните **Создать** и укажите следующие сведения:

   * Введите имя новой **рабочей области Log Analytics**. Например **DefaultLAWorkspace**.
   * Выберите **подписку**, к которой она будет привязана. Если по умолчанию предлагается не та подписка, которую вы хотите использовать, выберите из списка другую.
   * В разделе **Группа ресурсов** выберите имеющуюся группу ресурсов, в которой присутствует одна или несколько виртуальных машин Azure.  
   * Выберите **Расположение**, в котором развернуты виртуальные машины. Список регионов, где доступна служба Azure Log Analytics, вы найдете [здесь](https://azure.microsoft.com/regions/services/).  
   * Если вы создаете рабочую область в подписке, созданной после 2 апреля 2018 г., для нее будет автоматически настроен тарифный план с оплатой **за ГБ**. Вы не можете выбирать ценовую категорию. Если вы создаете рабочую область в подписке, созданной до 2 апреля 2018 г. или привязанной к существующей регистрации EA, выберите ценовую категорию по своему усмотрению. Дополнительные сведения о ценовых категориях см. на [странице цен](https://azure.microsoft.com/pricing/details/log-analytics/).

        ![Создание ресурса Log Analytics](media/quick-collect-windows-computer/create-loganalytics-workspace-02.png)<br>  

3. Завершив ввод обязательных сведений на панели **Рабочая область Log Analytics**, щелкните **OK**.  

Пока проверяются данные и создается рабочая область, вы можете контролировать прогресс в разделе меню **Уведомления**.

## <a name="get-the-workspace-id-and-key"></a>Получение идентификатора и ключа рабочей области
Перед установкой Microsoft Monitoring Agent для Windows следует получить идентификатор и ключ для рабочей области Log Analytics. Эта информация необходима мастеру настройки для правильной настройки агента и организации взаимодействия с Log Analytics.  

1. Щелкните **Все службы** в левом верхнем углу на портале Azure. В поле поиска введите **Log Analytics**. По мере ввода символов список отфильтруется соответствующим образом. Выберите **Log Analytics**.
...
3. Выберите **Дополнительные параметры**.
    ...
    ![Дополнительные параметры Log Analytics](media/quick-collect-windows-computer/log-analytics-advanced-settings-01.png)
...
5. На странице **Параметры установки агента** подключите агент к Azure Log Analytics и щелкните **Далее**.
6. На странице **Azure Log Analytics** выполните следующие шаги.
...
7. Добавьте все параметры конфигурации и снова щелкните **Далее**:

    ![Установка Microsoft Monitoring Agent](media/quick-collect-windows-computer/log-analytics-mma-setup-laworkspace.png)

8. На странице **Все готово для установки** просмотрите выбранные параметры, а затем выберите **Установить**.
9. На странице **Настройка успешно завершена** щелкните **Готово**.

После завершения установки и настройки Microsoft Monitoring Agent появится на панели управления. Вы можете просмотреть конфигурацию и проверить, подключен ли агент к Log Analytics. При подключении агент отображает на вкладке **Azure Log Analytics** такое сообщение: **Microsoft Monitoring Agent успешно подключен к службе Microsoft Log Analytics**.<br><br> ![Состояние подключения MMA](media/quick-collect-windows-computer/log-analytics-mma-laworkspace-status.png)
...
1. Щелкните **Другие службы** в левом нижнем углу на портале Azure. В поле поиска введите **Log Analytics**. По мере ввода символов список отфильтруется соответствующим образом. Выберите **Log Analytics**.
...
1. На портале Azure выберите плитку **Журналы** в выбранной рабочей области.  
2. На панели **Поиск по журналам** введите в поле запроса строку **Perf** и щелкните **Запуск** над полем запроса:
 
    ![Поиск по журналам Log Analytics](media/quick-collect-windows-computer/log-analytics-portal-queryexample.png)

    Так, запрос на этом рисунке вернул 735 записей о производительности.

    ![Результат поиска по журналам Log Analytics](media/quick-collect-windows-computer/log-analytics-search-perf.png)
...
![Удаление рабочей области Log Analytics](media/quick-collect-windows-computer/log-analytics-portal-delete-resource.png)
