...
ms.locfileid: "72900485"
---
# <a name="start-monitoring-your-aspnet-web-application"></a>Запуск мониторинга веб-приложения ASP.NET

С помощью Azure Application Insights можно легко отслеживать доступность, производительность и использование своего веб-приложения. Вы также можете быстро идентифицировать и диагностировать в нем ошибки, не дожидаясь, пока пользователь сообщит о них. На основе сведений о производительности и эффективности, получаемых из Insights, вы можете эффективно принимать решения по поддержке и улучшению приложения.

В этом руководстве показано, как добавить Application Insights в имеющееся веб-приложение ASP.NET и начать анализировать статистику в реальном времени с помощью одного из различных методов. Если у вас нет веб-приложения ASP.NET, его можно создать, выполнив инструкции из статьи [Создание веб-приложения ASP.NET Framework в Azure](../../app-service/app-service-web-get-started-dotnet-framework.md)
...
2. В меню **Проект** выберите **Настроить Application Insights**. С помощью Visual Studio пакет SDK Application Insights добавляется в приложение.
    ...
    > [!IMPORTANT]
    > Процесс добавления Application Insights зависит от типа шаблона ASP.NET. Если вы используете **Пустой** шаблон или шаблон **Мобильного приложения Azure**, выберите **Проект** > **Добавить телеметрию Application Insights**. Для работы со всеми другими шаблонами ASP.NET ознакомьтесь с инструкциями на предыдущем шаге. 
...
5. Выберите **Проект** > **Управление пакетами NuGet** > **Источник пакета: nuget.org** > **Обновить** и обновите пакеты SDK для Application Insights до последней стабильной версии.
...
1. В **Обозревателе решений** откройте папку **Подключенные службы**, нажав значок с облаком и вилкой, правой кнопкой мыши щелкните папку **Application Insights**, а затем — **Открыть портал Application Insights**. Появятся некоторые сведения о приложении и разнообразные параметры.


    ![Схема приложений](media/quick-monitor-portal/04-overview.png)
...
2. Щелкните **Схема сопоставления приложений**, чтобы получить визуальный макет отношений зависимости между компонентами приложения.  Каждый компонент показывает ключевой показатель эффективности, такие как производительность, сбои и оповещения.
    ...
    ![Схема приложений](media/quick-monitor-portal/05-appmap.png)

3. Щелкните в **Аналитике приложений** значок ![значок схемы приложений](media/quick-monitor-portal/app-viewinlogs-icon.png) **Просмотреть в журналах (аналитика)** на одном из компонентов приложений. Откроется окно **Журналы (аналитика)**, которое предоставляет многофункциональный язык запросов для анализа всех данных, собираемых Application Insights. В этом случае автоматически создается запрос, который преобразует число запросов для просмотра в виде диаграммы. Вы можете писать собственные запросы для анализа других данных.
...
> [Поиск и диагностика исключений времени выполнения с помощью Azure Application Insights](tutorial-runtime-exceptions.md)
