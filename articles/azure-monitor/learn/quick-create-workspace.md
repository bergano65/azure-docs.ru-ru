...
2. Щелкните **Добавить** и выберите параметры для следующих элементов:
   ...
   * Введите имя для новой **pабочей области Log Analytics**, например *DefaultLAWorkspace*. Это имя должно быть глобально уникальным для всех Azure Monitor подписок.
   ...
   * В качестве **группы ресурсов** выберите существующую и уже настроенную группу ресурсов или создайте новую.  
   * Выберите доступное **Pасположение**.  Дополнительные сведения см. на странице [Доступность продуктов по регионам](https://azure.microsoft.com/regions/services/), введя "Azure Monitor" в поле **Поиск продукта**.  
