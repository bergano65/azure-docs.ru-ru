...
1. На портале Azure щелкните **все сервисы**. В списке ресурсов введите **log Analytics**. Как только вы начнете вводить символы, список отфильтруется соответствующим образом. Выберите **Рабочие области Log Analytics**.
...
2. Щелкните **Добавить** и выберите параметры для следующих элементов:
   ...
   * Выберите доступное **расположение**.  Для получения дополнительной информации узнайте, в [каких регионах доступна аналитика журналов,](https://azure.microsoft.com/regions/services/) и ищите azure Monitor из поля **поиска продукта.**  
