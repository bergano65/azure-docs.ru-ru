...
> [!NOTE]
> Динамические пороговые значения могут обнаруживать сезонности для почасовых, ежедневных или еженедельных шаблонов. Другие закономерности, такие как BI-Hour или сезонности, могут не быть обнаружены. Для обнаружения еженедельных сезонности требуется по крайней мере три недели исторических данных. 
...
При первом создании правила генерации оповещений пороговые значения, отображаемые на диаграмме, рассчитываются на основе достаточного количества исторических данных для вычисления ежедневных или дневных сезонных шаблонов (10 дней). После создания правила генерации оповещений динамические пороговые значения используют все необходимые исторические данные, которые будут постоянно изучены и адаптированы на основе новых данных, чтобы сделать пороги более точными. Это означает, что после этого вычисления на диаграмме также будут отображаться еженедельные шаблоны.

## <a name="how-much-data-is-needed-to-trigger-an-alert"></a>Сколько данных необходимо для активации предупреждения?

Если у вас есть новый ресурс или отсутствуют данные метрики, то динамические пороговые значения не будут активировать предупреждения до трех дней, и по крайней мере 30 выборок данных метрик будут доступны для обеспечения точных порогов.
Для существующих ресурсов с достаточными данными метрики динамические пороговые значения могут немедленно активировать предупреждения.

## <a name="dynamic-thresholds-best-practices"></a>Рекомендации по динамическим порогам

Динамические пороговые значения могут применяться к большинству платформ и пользовательских метрик в Azure Monitor и также настроены для общих метрик приложений и инфраструктуры.
...
1. В [портал Azure](https://portal.azure.com)щелкните **монитор**. В представлении "Монитор" объединены все параметры мониторинга и все данные.
