...
ms.locfileid: "100623025"
---
# <a name="optimizing-log-alert-queries"></a>Оптимизация запросов предупреждений журнала
В этой статье описывается написание и преобразование запросов [предупреждений журнала](../platform/alerts-unified-log.md) для достижения оптимальной производительности. Оптимизированные запросы уменьшают задержку и нагрузку на предупреждения, которые выполняются часто.
...
>[!NOTE]
> [Запросы между ресурсами](../log-query/cross-workspace-query.md) поддерживаются в новом [API счедуледкуерирулес](/rest/api/monitor/scheduledqueryrules). Если вы по-прежнему используете [устаревший API предупреждений log Analytics](../platform/api-alerts.md) для создания оповещений журнала, см. сведения о переключении [здесь](../alerts/alerts-log-api-switch.md).
