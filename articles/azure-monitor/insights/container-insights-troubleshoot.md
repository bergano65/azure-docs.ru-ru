...
Azure Monitor для модулей Pod агента контейнеров используют конечную точку cAdvisor на агенте узла для сбора метрик производительности. Убедитесь, что контейнерный агент на узле настроен так, чтобы `cAdvisor port: 10255` его можно было открывать на всех узлах в кластере для сбора метрик производительности.
