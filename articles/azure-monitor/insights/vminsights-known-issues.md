...
- После отключение мониторинга виртуальной машины с помощью поддерживаемых методов повторная попытка подключения должна выполняться для той же рабочей области.  Если используется новая рабочая область, состояние работоспособности этой виртуальной машины может отображаться как аномальное поведение.
...
- Сводка оповещений, предоставляемая Azure Monitor для виртуальных машин, касается оповещений, вызванных проблемами работоспособности, которые были обнаружены на отслеживаемых виртуальных машинах Azure.
