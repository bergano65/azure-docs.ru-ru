...
|Версия ОС |Производительность |Карты |
|-----------|------------|-----|
...
|:--|:--|:--|
| Агенты Windows | Yes | Вместе с [агентом log Analytics для Windows](../../azure-monitor/platform/log-analytics-agent.md)агентам Windows требуется агент зависимостей. Дополнительные сведения см. в разделе [Поддерживаемые операционные системы](#supported-operating-systems). |
| Агенты Linux | Yes | Вместе с [агентом log Analytics для Linux](../../azure-monitor/platform/log-analytics-agent.md)агентам Linux требуется агент зависимостей. Дополнительные сведения см. в разделе [Поддерживаемые операционные системы](#supported-operating-systems). |
| Группа управления System Center Operations Manager | Нет | |

Вы можете скачать агент зависимостей из следующих расположений:

| Файл | ОС | Версия | SHA-256 |
|:--|:--|:--|:--|
...
>[!NOTE]
>Приведенный ниже список счетчиков производительности, включенных Azure Monitor для виртуальных машин, не ограничивается включением дополнительных счетчиков, которые необходимо выполнить для получения отчетов из виртуальных машин в рабочую область. Кроме того, если отключить эти счетчики, набор диаграмм производительности, включенных в функцию производительности, не будет показывать использование ресурсов на виртуальных машинах.
