...
```
>[!IMPORTANT]
>В следующих запросах используются значения заполнителей \<>-Cluster-Name и \<имя-контроллера > для представления кластера и контроллера. При настройке оповещений замените их значениями, характерными для вашей среды.
...
7. В разделе **условие** выберите **каждый раз, когда пользовательский поиск по журналам будет \<логика не определена >** заранее определенного условия пользовательского журнала. Тип сигнала **поиска пользовательских журналов** выбирается автоматически, так как мы создаем правило оповещения непосредственно на странице журналов Azure Monitor.  
