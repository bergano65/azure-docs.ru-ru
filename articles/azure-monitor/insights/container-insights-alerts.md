---
title: Создание предупреждений производительности для Azure Monitor для контейнеров | Документация Майкрософт
...
```
>[!IMPORTANT]
>В следующих запросах используются значения \<заполнителей. имя кластера> и \<имя контроллера> для представления кластера и контроллера. При настройке оповещений замените их значениями, характерными для вашей среды.
...
Выполните следующие действия, чтобы создать оповещение журнала в Azure Monitor с помощью одного из указанных выше правил поиска журналов. Сведения о создании с помощью шаблона ARM см. в разделе [Пример создания оповещений журнала с помощью шаблона ресурсов Azure](../platform/alerts-log.md#sample-log-alert-creation-using-azure-resource-template).
...
2. В портал Azure найдите и выберите **log Analytics рабочие области**.
3. В списке рабочих областей Log Analytics выберите рабочую область, поддерживающую Azure Monitor для контейнеров. 
4. На панели слева выберите **журналы** , чтобы открыть страницу Azure Monitor журналы. Эта страница используется для создания и выполнения запросов Log Analytics Azure.
...
7. В разделе **условие** выберите **каждый раз, когда пользовательский поиск по журналам \<является логикой неопределенного>** заранее определенного условия пользовательского журнала. Тип сигнала **поиска пользовательских журналов** выбирается автоматически, так как мы создаем правило оповещения непосредственно на странице журналов Azure Monitor.  
