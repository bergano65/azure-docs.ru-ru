---
title: Настройка мониторинга в Azure Monitor для виртуальных машин работоспособности гостей с помощью правил сбора данных (Предварительная версия)
...
ms.locfileid: "94687166"
---
# <a name="configure-monitoring-in-azure-monitor-for-vms-guest-health-using-data-collection-rules-preview"></a>Настройка мониторинга в Azure Monitor для виртуальных машин работоспособности гостей с помощью правил сбора данных (Предварительная версия)
...
|:---|:---|:---|
| Активировано | Статистическое<br>Единицы | Если значение — true, монитор состояния вычисляется и влияет на работоспособность виртуальной машины. Он может активировать оповещение о включении предупреждений. |
| Оповещение | Статистическое<br>Единицы | Если задано значение true, то при переходе в неработоспособное состояние для монитора запускается оповещение. Если задано значение false, состояние монитора будет по-прежнему влиять на работоспособность виртуальной машины, что может вызвать оповещение. |
| Предупреждение | Единицы | Критерии для состояния предупреждения. Если нет, то монитор никогда не введет состояние предупреждения. |
| Критически важно | Единицы | Критерии для критического состояния. Если нет, то монитор никогда не будет переходить в критическое состояние. |
| Периодичность оценки | Единицы | Частота оценки состояния работоспособности. |
| лукбакк | Единицы | Размер окна лукбакк в секундах. Подробное описание см. в [элементе мониторконфигуратион](#monitorconfiguration-element) . |
| Тип оценки | Единицы | Определяет, какое значение следует использовать из набора выборки. Подробное описание см. в [элементе мониторконфигуратион](#monitorconfiguration-element) . |
| Пример min | Единицы | Минимальное число значений, используемых для вычисления значения. |
| Пример Max | Единицы | Максимальное число значений, которое будет использоваться для вычисления значения. |
...
| Azure Monitor | Активировано | Оповещение | Предупреждение | Критически важно | Периодичность оценки | лукбакк | Тип оценки | Пример min | Максимальное число выборок |
|:---|:---|:---|:---|:---|:---|:---|:---|:---|:---|
| загрузка ЦП;  | Верно | Неверно | Отсутствуют | \> 90%    | 60 с | 240 с | Min | 2 | 3 |
| Объем доступной памяти | Верно | Неверно | Отсутствуют | \< 100 МБ | 60 с | 240 с | Макс. | 2 | 3 |
| Файловая система      | Верно | Неверно | Отсутствуют | \< 100 МБ | 60 с | 120 с | Макс. | 1 | 1 |
...
Переопределения определяются в [правиле сбора данных (ДКР)](../platform/data-collection-rule-overview.md). Вы можете создать несколько DCR с разными наборами переопределений и применить их к нескольким виртуальным машинам. Вы применяете ДКР к виртуальной машине, создав Ассоциацию, как описано в разделе [Настройка сбора данных для агента Azure Monitor (Предварительная версия)](../platform/data-collection-rule-azure-monitor-agent.md#dcr-associations).
...
В следующем примере правила сбора данных показан пример переопределения для настройки мониторинга.
...
- Узнайте больше о [правилах сбора данных](../platform/data-collection-rule-overview.md).