...
>[!NOTE]
>Для работы этой функции требуется доступ **участника** к ресурсу кластера.
>
...
- **URI перенаправления (необязательно)** : Это тип **веб-** приложения, и значение базового URL-адреса должно `https://afd.hosting.portal.azure.net/monitoring/Content/iframe/infrainsights.app/web/base-libs/auth/auth.html`быть.
...
| Просмотр | Тип ресурса | Журнал или событие | Представленные данные |
|------|---------------|--------------|----------------|
| Узлы | Узел | Событие | Если узел выбран, события не фильтруются и не отображаются события Kubernetes на уровне кластера. В заголовке панели отображается имя кластера. |
| Узлы | Pod | Событие | При выборе типа POD события фильтруются по его пространству имен. В заголовке области отображается пространство имен Pod. | 
| Controllers; | Pod | Событие | При выборе типа POD события фильтруются по его пространству имен. В заголовке области отображается пространство имен Pod. |
| Controllers; | Controller | Событие | Если контроллер выбран, события фильтруются по его пространству имен. В заголовке панели показано пространство имен контроллера. |
