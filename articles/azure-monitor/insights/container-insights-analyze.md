...
* **Неправильно** -Azure Monitor для контейнеров не было правильно настроено в указанной рабочей области.
...
Вы можете использовать клавиши со стрелками влево и вправо для циклического просмотра каждой точки данных на диаграмме и клавиши со стрелками вверх или вниз для циклического просмотра строк процентиля. Щелкнув значок булавки в правом верхнем углу любого из диаграмм будет закрепить выбранная диаграмма последнюю панель мониторинга Azure, которая просматривалась последней. На панели мониторинга можно изменять и перемещать диаграмму. Выбор диаграммы на панели мониторинга перенаправить вас на Azure Monitor для контейнеров и загрузить правильную область и представления.
...
Workbooks объединяют текста, [журнал запросов](../log-query/query-language.md), [метрики](../platform/data-platform-metrics.md)и параметры в подробные интерактивные отчеты. Их могут редактировать другие участники команды с доступом к тому же ресурсу Azure.

Azure Monitor для контейнеров включает книги, чтобы приступить к работе, **емкость диска**.  Эта книга представляет диаграммы использования интерактивных диска для каждого диска, представленных на узел в контейнере с точки зрения следующих:

- Использование диска % для всех дисков
- Свободного места на диске для всех дисков
- Таблица для каждого диска, узлы, его процент используемого места, тренд % используется пространства, свободного места на диске (ГБ) и тренд свободного дискового пространства (Гиб). При выборе строки в таблице используемое место, %, а ниже приведен свободного места на диске (ГБ) 

Доступ к этой книге, выбрав **емкость диска** из **книгами View** стрелку раскрывающегося списка.  

![Просмотр книг с раскрывающимся списком](./media/container-insights-analyze/view-workbooks-dropdown-list.png)
