...
Состояние работоспособности вычисляет общее состояние кластера как *наихудшее из* трех состояний с одним исключением. Если какое-либо из трех состояний неизвестно, общее состояние кластера отображается какнеизвестное. 
...
В **рабочем наборе памяти** отображается как резидентная память, так и виртуальная память (кэш), а также общее число используемых приложений. В **RSS-канале памяти** отображается только основная память, которая является резидентной памятью. Эта метрика показывает фактическую емкость доступной памяти.
