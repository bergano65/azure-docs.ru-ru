...
Используйте команду [​az aks disable-addons](https://docs.microsoft.com/cli/azure/aks?view=azure-cli-latest#az-aks-disable-addons), чтобы отключить Azure Monitor для контейнеров. Команда удаляет агента из узлов кластера, она не удаляет решение или данные, уже собранные и хранящиеся в ресурсе Azure Monitor.  
...
>[!NOTE]
>Шаблон должен быть развернут в той же группе ресурсов кластера. Если вы опустите какие-либо другие свойства или надстройки при использовании этого шаблона, это может привести к удалению их из кластера. Например, *включитьRBAC* для политик RBAC, реализованных в кластере, или *aksResourceTagValues,* если метки указаны для кластера AKS.  
>
...
Если рабочая область была создана только для мониторинга кластера и больше не требуется, ее необходимо удалить вручную. Если вам неизвестно, как удалить рабочую область, прочитайте раздел [Удаление рабочей области Azure Log Analytics на портале Azure](../../log-analytics/log-analytics-manage-del-workspace.md). Не забывайте об **идентификаторе ресурсов Workspace,** скопированный ранее в шаге 4, вам это понадобится.
