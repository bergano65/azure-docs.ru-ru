...
Просмотр компонентов обнаруженных приложений на Windows и Linux виртуальных машин, работающих в Azure, в вашей среде можно наблюдать, одним из двух способов, с помощью Azure Monitor для виртуальных машин из виртуальной машины напрямую или через группы виртуальных машин из Azure Monitor. 

Благодаря этой статье вы поймете разницу между этими двумя способами и узнаете, как использовать функцию "Схема". Сведения о настройке Azure Monitor для виртуальных машин, см. в [этой статье](vminsights-enable-overview.md).
...
## <a name="view-map-directly-from-a-virtual-machine-scale-set"></a>Просмотр карты напрямую от масштабируемого набора виртуальных машин набора
...
Карта визуализирует всех экземпляров в масштабируемом наборе, как узел группы вместе с зависимостей группы. Развернутый узел список экземпляров в масштабируемом наборе, который можно прокручивать десять за раз. Чтобы загрузить карту для конкретного экземпляра, укажите, что экземпляр на карте и затем нажмите кнопку с многоточием в него правильный, а выберите **загрузить карту сервера**. Он загрузит карте для этого экземпляра, что позволяет видеть группы процессов и процессов с активными сетевыми подключениями через определенного диапазона времени. По умолчанию на схеме показаны последние 30 минут. С помощью **TimeRange** селектор вы можете запрашивать исторические временные диапазоны до одного часа, чтобы получать представление о зависимостях в прошлом (например, во время инцидента или перед изменением).  

![Общие сведения о непосредственной схеме виртуальной машины](./media/vminsights-maps/map-direct-vmss-01.png)

>[!NOTE]
>Карты для конкретного экземпляра можно также войти через представление экземпляров масштабируемого набора виртуальных машин. Перейдите к **экземпляров** под **параметры** раздела, а затем выберите **Insights (Предварительная версия)** .
...
В селекторе **Рабочая область** в верхней части страницы (если у вас несколько рабочих областей Log Analytics) выберите рабочую область, которая интегрирована с решением и в которой имеются виртуальные машины, передающие данные в нее. **Группы** селектора возвращает подписки, группы ресурсов, [групп компьютеров](../../azure-monitor/platform/computer-groups.md)и масштабируемые наборы виртуальных машин из компьютеров, связанных с выбранной рабочей области. Выбор применяется только к функции карты и не применяется к производительности.
