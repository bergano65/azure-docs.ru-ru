...
## <a name="why-am-i-not-seeing-data-if-log-analytics-workspace-is-configured-with-the-free-pricing-tier"></a>Почему я не вижу данные в рабочей области Log Analytics, для которой настроена бесплатная ценовая категория? 

Возможно, уже достигнуто стандартное ограничение в 500 МБ или вы указали ежедневное ограничение на объем собираемых данных. Чтобы проверить и изменить параметры использования данных, воспользуйтесь статьей [Управление использованием и затратами для Log Analytics](../platform/manage-cost-storage.md). 
...
Кластеры AKS с управлением доступом на основе ролей в настоящее время не поддерживаются этим решением. На странице сведений о решении может неправильно отображаться информация в колонках с данными о таких кластерах.
...
Сбор журналов из контейнеров в пространстве имен kube-system по умолчанию отключен. Вы можете включить сбор журналов, задав переменную среды для агента OMS. Дополнительные сведения см. на странице GitHub, посвященной [Azure Monitor для контейнеров](https://github.com/helm/charts/tree/master/incubator/azuremonitor-containers). 
...
Когда вы запросите эти данные, в журналах Azure Monitor они будут выглядеть примерно так:
...
Вы можете получать следующую ошибку: **The reply url specified in the request does not match the reply urls configured for the application (указанный в запросе URL-адрес ответа не соответствует URL-адресам ответа, настроенным для приложения): '60b4dec7-5a69-4165-a211-12c40b5c0435'**. Метод ее устранения описан в разделе [Настройка AKS с Azure Active Directory](container-insights-live-logs.md#configure-aks-with-azure-active-directory). 
