...
Возможность наблюдения за сетями с помощью узлов на основе Linux в настоящее время доступна в предварительной версии. Соответствующий [агент.](../../virtual-machines/extensions/oms-linux.md) Дополнительные сведения вы можете получить у своего менеджера. Агенты Linux поддерживают при работе с решением "Монитор производительности сети" только функцию мониторинга производительности. Мониторинг подключений службы и (или) ExpressRoute не поддерживается.
...
Возможности монитора производительности и подключения службы поддерживают узлы [, подключенные как прямые агенты](../platform/agent-windows.md) и [подключенные с помощью Operations Manager](../platform/om-agents.md).
...
* Выполните на этом узле скрипт Powershell [EnableRules.ps1](https://aka.ms/npmpowershellscript). Дополнительные сведения см. в разделе [инструкций](./network-performance-monitor.md#configure-log-analytics-agents-for-monitoring).
...
В настоящее время создание предупреждений из пользовательского интерфейса NPM завершается сбоем из-за известной проблемы. [Создайте оповещения вручную](../platform/alerts-log.md).
...
Монитор ExpressRoute сравнивает значения производительности сети (потери, задержка и использование пропускной способности), сообщаемые агентами или службой с пороговыми значениями, заданными во время настройки. Для канала, если переданное значение использования пропускной способности больше порогового значения, установленного в конфигурации, цепь помечается как неработоспособная. Для пиринга, если значение параметра "потери, задержка или пропускная способность" больше порогового значения, установленного в конфигурации, пиринг помечается как неработоспособный. NPM не использует метрики или другие формы данных для деикде состояния работоспособности.

### <a name="why-does-expressroute-monitorbandwidth-utilisation-report-a-value-differrent-from-metrics-bits-inout"></a>Почему при Монитор'бандвидсе ExpressRoute в отчете о значении дифферрентся из метрик бит/с
Для монитора ExpressRoute пропускная способность утилиатион — это среднее значение входящей и исходящей пропускной способности за последние 20 минут, которое выражается в битах/сек. Для метрик выпусков Express Route бит/с — это точка данных в минуту. На внутреннем уровне набор данных используется одинаково, но Статистическая обработка валиес между метриками NPM и ER. Для наблюдения за минутуми и быстрыми оповещениями рекомендуется устанавливать оповещения непосредственно на метриках ER.
