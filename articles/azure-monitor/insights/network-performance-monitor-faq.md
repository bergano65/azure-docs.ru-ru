...
description: В этой статье часто задаваемые вопросы о сетевом мониторе производительности в Azure. Network Performance Monitor (NPM) помогает отслеживать производительность сетей практически в режиме реального времени и обнаруживать и находить узкие места в производительности сети.
ms.subservice: logs
ms.topic: conceptual
author: vinynigam
ms.author: vinigam
...
- Возможности NPM По мониторингу производительности и сервисного мониторинга подключения поддерживают как сервер Windows, так и настольные/клиентские операционные системы Windows. Версии Windows Server OS поддерживается 2008 SP1 или позже. Windows настольных /клиентверсий поддерживается Windows 10, Windows 8.1, Windows 8, и Windows 7. 
...
Возможность мониторинга сетей с использованием узлов на базе Linux в настоящее время находится в предварительном просмотре. Дополнительные сведения вы можете получить у своего менеджера. Агенты Linux поддерживают при работе с решением "Монитор производительности сети" только функцию мониторинга производительности. Мониторинг подключений службы и (или) ExpressRoute не поддерживается.
...
Чтобы решение "Монитор производительности сети" работало на виртуальных машинах узла и выполняло мониторинг сети, этот узел должен располагать как минимум 500 МБ памяти и одним ядром. Вам не нужно использовать отдельные узлы для запуска NPM. Это решение может выполняться на узлах, которые выполняют другие рабочие нагрузки. Решение имеет возможность остановить процесс мониторинга, если он использует более 5% процессора.
...
Как монитор производительности, так и возможности Service Connectivity Monitor поддерживают узлы, [подключенные как прямые агенты](../../azure-monitor/platform/agent-windows.md) и [подключенные через менеджера операций.](../../azure-monitor/platform/om-agents.md)

Для функции мониторинга ExpressRoute узлы Azure должны быть обязательно подключены как прямые агенты. Эта функция не поддерживает узлы Azure, подключенные через Operations Manager. Для локтевых узлов узлы, подключенные в качестве прямых агентов и через менеджера по операциям, поддерживаются для мониторинга схемы ExpressRoute.
...
Если вы отслеживаете свою сеть с помощью узлов на базе серверов Windows, мы рекомендуем использовать TCP в качестве протокола мониторинга, поскольку он обеспечивает лучшую точность. 

ICMP рекомендуется для настольных компьютеров под управлением Windows и узлов на базе клиентской операционной системы. Эта платформа не позволяет отправлять данные TCP по необработанным розетке, которые NPM использует для обнаружения топологии сети.
...
### <a name="what-is-the-maximum-number-of-agents-i-can-use-or-i-see-error--youve-reached-your-configuration-limit"></a>Каково максимальное количество агентов, которые я могу использовать или я вижу ошибку ".... вы достигли предела конфигурации"?
NPM ограничивает количество IP-адресов до 5000 IP-адресов на каждую рабочую область. Если у узла есть IP-адреса версий 4 и 6, то считается, что для этого узла задано два IP-адреса. Таким образом, это ограничение в 5000 IP-адресов определяет максимально допустимое число агентов. Неактивных агентов можно удалить на вкладке "Узлы" в разделе NPM > "Настройка". NPM также ведет историю всех IP-адресов, которые когда-либо были назначены VM хостинг агента и каждый засчитывается как отдельный IP способствует, что верхний предел 5000 IP-адресов. Чтобы освободить I-страницы для рабочего пространства, можно использовать страницу узлов для удаления неисправных ипотечники.
...
### <a name="what-are-the-default-log-analytics-queries-for-alerts"></a>Каковы запросы аналитики журналов по умолчанию для предупреждений
Запрос мониторинга производительности

    NetworkMonitoring 
     | where (SubType == "SubNetwork" or SubType == "NetworkPath") 
     | where (LossHealthState == "Unhealthy" or LatencyHealthState == "Unhealthy") and RuleName == "<<your rule name>>"
    
Запрос мониторинга подключения служб

    NetworkMonitoring                 
     | where (SubType == "EndpointHealth" or SubType == "EndpointPath")
     | where (LossHealthState == "Unhealthy" or LatencyHealthState == "Unhealthy" or ServiceResponseHealthState == "Unhealthy" or LatencyHealthState == "Unhealthy") and TestName == "<<your test name>>"
    
Запросы монитора ExpressRoute: Запрос схем
...
Для получения информации о уровне ms peering используйте упомянутый ниже запрос в журнале Search

    NetworkMonitoring 
     | where SubType == "ERMSPeeringUtilization"
     | project  CircuitName,PeeringName,PrimaryBytesInPerSecond,PrimaryBytesOutPerSecond,SecondaryBytesInPerSecond,SecondaryBytesOutPerSecond
    
Для получения информации об уровне вников в частные области используйте упомянутый ниже запрос в журнале Search

    NetworkMonitoring 
     | where SubType == "ERVNetConnectionUtilization"
     | project  CircuitName,PeeringName,PrimaryBytesInPerSecond,PrimaryBytesOutPerSecond,SecondaryBytesInPerSecond,SecondaryBytesOutPerSecond
  
Для получения информации об уровне схемы используйте упомянутый ниже запрос в журнале Search
...
Когда какая-либо из конечных точек находится в Azure, трасса отображает неопознанные прыжки, поскольку инфраструктура Azure не показывает идентификационный маршрут. 

### <a name="i-get-alerts-for-unhealthy-tests-but-i-do-not-see-the-high-values-in-npms-loss-and-latency-graph-how-do-i-check-what-is-unhealthy"></a>Я получаю оповещения для нездоровых тестов, но я не вижу высоких значений в графике потери и задержки NPM. Как проверить, что нездорово?
NPM повышает оповещение, если задержка от конца до конца между источником и пунктом назначения пересекает порог для любого пути между ними. Некоторые сети имеют несколько путей, соединяющих один и тот же источник и пункт назначения. NPM поднимает оповещение: любой путь неработоспособен. Потеря и задержка, видимые на графиках, являются средним значением для всех путей, следовательно, оно может не показывать точное значение одного пути. Чтобы понять, где был нарушен порог, ищите столбец "SubType" в предупреждении. Если проблема вызвана траекторией, значение SubType будет NetworkPath (для тестов монитора производительности), EndpointPath (для тестов service Connectivity Monitor) и ExpressRoutePath (для тестов ExpressRotue Monitor). 

Пример запроса, чтобы найти это путь является нездоровым:
...
### <a name="why-does-my-test-show-unhealthy-but-the-topology-does-not"></a>Почему мой тест показывает нездоровый, но топология не 
NPM отслеживает сквозные потери, задержки и топологию с разными интервалами. Потери и задержки измеряются один раз в 5 секунд и агрегируются каждые три минуты (для монитора производительности и экспресс-монитора маршрута), в то время как топология рассчитывается с помощью трассы один раз в 10 минут. Например, между 3:44 и 4:04 топология может обновляться три раза (3:44, 3:54, 4:04), но потери и задержка обновляются примерно в семь раз (3:44, 3:47, 3:50, 3:53, 3:56, 3:59, 4:02). Топология, созданная в 3:54, будет отображаться для потери и задержки, которая рассчитывается в 3:56, 3:59 и 4:02. Предположим, вы получите предупреждение о том, что ваша схема ER была неработоспособной в 3:59. Вы входите в NPM и пытаетесь установить время топологии до 3:59. NPM сделает топологию, сгенерированную в 3:54. Чтобы понять последнюю известную топологию вашей сети, сравните поля TimeProcessed (время, в которое была рассчитана потеря и задержка) и TracerouteCompletedTime (время, в которое была рассчитана топология). 

### <a name="what-is-the-difference-between-the-fields-e2emedianlatency-and-avghoplatencylist-in-the-networkmonitoring-table"></a>В чем разница между полями E2EMedianLatency и AvgHopLatencyList в таблице сетевого мониторинга
E2EMedianLatency — это задержка, обновляемая каждые три минуты после агрегирования результатов тестов tcp ping, в то время как AvgHopLatencyList обновляется каждые 10 минут на основе трассировки. Чтобы понять точное время расчета E2EMedianLatency, используйте поле TimeProcessed. Чтобы понять точное время завершения и обновления трассы AvgHopLatencyList, используйте поле TracerouteCompletedTime

### <a name="why-does-hop-by-hop-latency-numbers-differ-from-hoplatencyvalues"></a>Почему номера задержки hop-by-hop отличаются от HopLatencyValues 
HopLatencyValues являются источником конечных точек.
Например: Хмель - A,B,C. AvgHopLatency - 10,15,20. Это означает источник задержки 10, источник b задержки 15 и источник c задержки составляет 20. UI вычислит задержку а-B-хопа как 5 в топологии
