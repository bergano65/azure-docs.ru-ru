---
title: Решение vMware Мониторинг в Azure Monitor (ru) Документы Майкрософт
...
---
# <a name="vmware-monitoring-deprecated-solution-in-azure-monitor"></a>Решение VMware Monitoring (Deprecated) в Azure Monitor
...
Решение VMware Monitoring в Azure Monitor — это решение, которое помогает создать централизованный подход к ведению и мониторингу для больших журналов VMware. В статье описано, как с помощью этого решения централизованно устранять неполадки, связанные с узлами ESXi, а также регистрировать их и управлять ими. В решении можно увидеть подробные данные для всех узлов ESXi в одном расположении. В журналах узла ESXi вы можете просматривать счетчики основных событий, а также состояния и тенденции ВМ и узлов ESXi. Вы также можете устранять неполадки, просматривая централизованные журналы узлов ESXi и выполняя поиск в них. И наоборот, можно создавать оповещения на основе поисковых запросов по журналам.

В решении для отправки данных на целевую виртуальную машину, где установлен агент Log Analytics, используются встроенные функции системного журнала узла ESXi. Однако решение не предусматривает запись файлов в системный журнал целевой виртуальной машины. Агент Log Analytics открывает порт 1514 и прослушивает его. Как только они получают данные, агент Log Analytics передвигает данные в Azure Monitor.
...
1. Откройте брандмауэр узла ESXi для системного журнала. **ESXi Хост Конфигурация** > **программного обеспечения** > **безопасности профиля** > **брандмауэра** и **открытые свойства**.  
...
1. На портале Azure выполните запрос `VMware_CL`журнала для . Когда Azure Monitor собирает данные syslog, он сохраняет формат syslog. На портале регистрируются некоторые поля, включая *Hostname* и *ProcessName*.  
...
Отсюда можно отсеить запрос журнала, чтобы изменить его на что-то конкретное. Для получения подробной информации о создании журналов запросов [см. Поиск данных с помощью запросов журнала в Azure Monitor.](../log-query/log-query-overview.md)
...
Сохранение журналов является стандартной функцией в Azure Monitor и может помочь вам сохранить любые запросы, которые вы нашли полезными. Создав запрос, который вы считаете полезным, сохраните его, щелкнув **Избранное**. Сохранение запроса позволяет без труда использовать его позже на странице [Моя панель мониторинга](../learn/tutorial-logs-dashboards.md), где вы можете создавать собственные настраиваемые панели мониторинга.
