---
title: Решение для мониторинга VMware в Azure Monitor | Документация Майкрософт
...
Решение для мониторинга VMware в Azure Monitor — это решение, которое помогает создавать централизованное ведение журнала и подход к наблюдению больших журналов VMware. В статье описано, как с помощью этого решения централизованно устранять неполадки, связанные с узлами ESXi, а также регистрировать их и управлять ими. В решении можно увидеть подробные данные для всех узлов ESXi в одном расположении. В журналах узла ESXi вы можете просматривать счетчики основных событий, а также состояния и тенденции ВМ и узлов ESXi. Вы также можете устранять неполадки, просматривая централизованные журналы узлов ESXi и выполняя поиск в них. И наоборот, можно создавать оповещения на основе поисковых запросов по журналам.

В решении для отправки данных на целевую виртуальную машину, где установлен агент Log Analytics, используются встроенные функции системного журнала узла ESXi. Однако решение не предусматривает запись файлов в системный журнал целевой виртуальной машины. Агент Log Analytics открывает порт 1514 и прослушивает его. При получении данных, агент Log Analytics отправляет данные в Azure Monitor.
...
1. На портале Azure, выполните запрос по журналу для `VMware_CL`. Собирая данные системного журнала Azure Monitor, сохраняет формат syslog. На портале регистрируются некоторые поля, включая *Hostname* и *ProcessName*.  
...
Здесь можно изменить, изменить ее для определенного запроса к журналу. Дополнительные сведения о создании запросов к журналу, см. в разделе [найти данные с помощью журнала запросов в Azure Monitor](../log-query/log-query-overview.md).
...
#### <a name="common-log-queries"></a>Общие запросы журнала
...
Сохранение журнала запросов — это стандартная функция в Azure Monitor и помогает сохранять запросы, оказавшиеся полезными. Создав запрос, который вы считаете полезным, сохраните его, щелкнув **Избранное**. Сохранение запроса позволяет без труда использовать его позже на странице [Моя панель мониторинга](../learn/tutorial-logs-dashboards.md), где вы можете создавать собственные настраиваемые панели мониторинга.
...
№ Для этого процесса перезапуск не требуется. В некоторых случаях vSphere не обновляет системный журнал надлежащим образом. В таком случае войдите на узел ESXi и перезагрузите системный журнал. Опять же, узел перезапускать не нужно, поэтому этот процесс не нарушает работу в среде.
