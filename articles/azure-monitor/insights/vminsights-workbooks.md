...
В следующей таблице перечислены книги, которые Azure Monitor для виртуальных машин включают, чтобы начать работу.
...
Каждый раздел имеет свои собственные дополнительные параметры, доступ к которым можно получить с ![помощью значка "Параметры](media/vminsights-workbooks/006-settings.png) " в разделе "Редактирование элементов управления", расположенного справа от кнопки " **Добавить параметры** ".
