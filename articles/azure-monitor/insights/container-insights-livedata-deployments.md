...
При использовании Azure Monitor для контейнеров функция представления развертывания (Предварительная версия) эмулирует прямой доступ к объектам развертывания Kubernetes в режиме реального времени, предоставляя `kubeclt get deployments` `kubectl describe deployment {your deployment}` команды и.
