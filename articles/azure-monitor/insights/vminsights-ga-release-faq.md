...
>[!NOTE]
>При наличии правил генерации оповещений, ссылающихся на эти `Perf` счетчики в таблице, необходимо обновить их, чтобы они ссылались на `InsightsMetrics` новые данные, хранящиеся в таблице. См. документацию по запросам к журналу, которые можно использовать для обращения к этой таблице.
>
...
Если вы создали [оповещения журнала](../platform/alerts-unified-log.md) , которые запрашивают `Perf` в таблице целевые счетчики производительности, которые были включены в рабочей области, следует обновить эти правила, чтобы `InsightsMetrics` они ссылались на таблицу. Это руководство также применяется к любым правилам поиска журналов с `ServiceMapComputer_CL` помощью `ServiceMapProcess_CL`и, поскольку эти наборы данных перемещаются `VMComputer` в `VMProcess` таблицы и.
...
Нет, `VMComputer` два решения совместно используют наборы данных карт, которые хранятся в (ранее ServiceMapComputer_CL), `VMProcess` (ранее ServiceMapProcess_CL) `VMConnection`, и. `VMBoundPort` Если в вашей рабочей области есть оба решения, вы не будете использовать двойную оплату за эти данные.
...
Чтобы получить доступ к функции, можно добавить в URL-адрес `feature.vmhealth=true` [https://portal.azure.com](https://portal.azure.com)портал Azure следующий флаг компонента. Пример `https://portal.azure.com/?feature.vmhealth=true`.
