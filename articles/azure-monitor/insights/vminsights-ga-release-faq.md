...
В этом обновлении Azure Monitor для виртуальных машин наборы данных производительности теперь хранятся в той же таблице `InsightsMetrics`, что и [Azure Monitor для контейнеров](container-insights-overview.md), и упрощают выполнение запросов к этим двум наборам. Кроме того, вы можете хранить более разнообразные наборы данных, которые не могли бы храниться в ранее использованной таблице.  Наши представления производительности также будут обновлены для использования этой новой таблицы.

Мы переходим к новым типам данных для наших наборов данных подключения. Данные, которые в настоящее время хранятся в `ServiceMapComputer_CL` и `ServiceMapProcess_CL`, которые используют пользовательские таблицы журнала, будут переходить на выделенные типы данных с именами `VMComputer` и `VMProcess`.  Переходя к выделенным типам данных, можно предоставить эти приоритеты для приема данных, и схема таблицы будет стандартизована для всех клиентов.
...
Если вы решите включить счетчики производительности, плата будет взиматься за данные, полученные и сохраняемые в таблице производительности на основе [цены на Log Analytics [(https://azure.microsoft.com/pricing/details/monitor/).

## <a name="how-will-this-change-affect-my-alert-rules"></a>Как это изменение повлияет на мои правила генерации оповещений?

Если вы создали [оповещения журнала](../platform/alerts-unified-log.md) , которые запрашивают в таблице `Perf` целевые счетчики производительности, которые были включены в рабочей области, следует обновить эти правила, чтобы они ссылались на таблицу `InsightsMetrics`. Это руководство также применяется к любым правилам поиска журналов, использующим `ServiceMapComputer_CL` и `ServiceMapProcess_CL`, так как эти наборы данных перемещаются в таблицы `VMComputer` и `VMProcess`.
...
Изменения в перемещении данных с `ServiceMapComputer_CL` и `ServiceMapProcess_CL` влияют на Сопоставление служб и Azure Monitor для виртуальных машин, поэтому вам все равно нужно спланировать это обновление.
...
Если используются оба решения, наборы данных не будут дублироваться. Оба предложения совместно используют наборы данных, которые будут храниться в таблицах `VMComputer` (ранее ServiceMapComputer_CL), `VMProcess` (ранее ServiceMapProcess_CL), `VMConnection` и `VMBoundPort` для хранения собранных наборов данных карт.  

Таблица `InsightsMetrics` будет использоваться для хранения собранных наборов данных ВМ, процессов и служб, которые будут заполнены только при использовании Azure Monitor для виртуальных машин.
...
Для доступа к функции можно добавить следующий флаг компонента `feature.vmhealth=true` в URL-адрес портала [https://portal.azure.com](https://portal.azure.com). Пример `https://portal.azure.com/?feature.vmhealth=true`.

Вы также можете использовать этот короткий URL-адрес, который устанавливает флаг компонента автоматически: [https://aka.ms/vmhealthpreview](https://aka.ms/vmhealthpreview).
