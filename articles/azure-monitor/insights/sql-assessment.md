---
title: Оптимизируйте свою среду сервера СЗЛ с помощью Azure Monitor Документы Майкрософт
description: С помощью Azure Monitor вы можете использовать решение для проверки здоровья S'L, чтобы регулярно оценивать риск и работоспособность окружающей среды.
ms.subservice: logs
ms.topic: conceptual
author: bwren
ms.author: bwren
...
---
# <a name="optimize-your-sql-environment-with-the-sql-server-health-check-solution-in-azure-monitor"></a>Оптимизируйте свою среду с помощью решения для проверки работоспособности сервера S'L в Azure Monitor
...
* Для проверки работоспособности s'L требуется поддерживаемая версия .NET Framework 4.6.2, установленная на каждом компьютере с установленным агентом мониторинга Майкрософт (MMA).  Агент MMA используется решением System Center 2016 Operations Manager и Operations Manager 2012 R2, а также Azure Monitor.  
...
Для выполнения проверки работоспособности на серверах серверов S'L Server требуется агент и подключение к Azure Monitor с помощью одного из следующих поддерживаемых методов:
...
2. Если он контролируется с помощью System Center 2016 - Менеджер операций или менеджер по операциям 2012 R2 и группа управления не интегрирована с Azure Monitor, сервер может быть многодоменным с Log Analytics для сбора данных и перенаправить в службу и при этом быть контролируется операционным менеджером.  
...
Агент на вашем сервере S'L, который отчитывается перед группой управления менеджером операций, собирает данные, пересылает на назначенный сервер управления, а затем отправляется непосредственно с сервера управления на Azure Monitor.  Данные не записываются в базы данных Operations Manager.  

Если сервер SQL Server отслеживается Operations Manager, необходимо настроить учетную запись запуска от имени Operations Manager. Для получения дополнительной информации смотрите отчеты менеджера по работе с [операциями, как учетные записи Для Azure Monitor](#operations-manager-run-as-accounts-for-log-analytics) ниже.
...
Прежде чем использовать решение оценки в Azure Monitor, необходимо установить решение.  После установки можно просмотреть резюме рекомендаций, используя плитку проверки работоспособности s'L на странице **«Обзор»** для Azure Monitor на портале Azure.
...
3. В разделе **Исследования** меню выберите **Подробнее**.  
...
1. В меню Azure Monitor нажмите **«Журналы».**
2. Выполните следующий запрос, чтобы получить список рекомендаций, не выполненных на компьютерах в вашей среде.
    ...
    ```
    Вот скриншот, показывающий запрос журнала:<br><br> ![невыполненные рекомендации](./media/sql-assessment/sql-assess-failed-recommendations.png)<br>
...
*Какие проверки выполняются решением s'L Assessment?*
...
* [Запросы в журнал,](../log-query/log-query-overview.md) чтобы узнать, как анализировать подробные данные и рекомендации по проверке здоровья s'L.
