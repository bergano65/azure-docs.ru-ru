---
title: Оценка менеджера по работе системного центра с помощью Azure Monitor
...
Решение работает с Microsoft System Center 2012 Менеджер по работе службы управления 1, Microsoft System Center 2012 R2 Менеджер по операциям, Microsoft System Center 2016 Операционный менеджер, Microsoft System Center 2016 Менеджер операций и Microsoft System Менеджер по операциям центра 1807. На каждом сервере управления должна быть установлена поддерживаемая версия .NET Framework 4.6.2.
...
Решение «Менеджер по проверке здоровья менеджера системного центра» собирает данные из следующих источников:
...
5. Имя профиля должно быть: *Менеджер по операциям Microsoft System Center Менеджер по проверке работы, как профиль.*
...
Пакет управления решением для управления решениям системного центра Operations Manager Manager Check включает в себя правило, названное microsoft *System Center Operations Manager Run Health Check Rule.* Это правило отвечает за проверку работоспособности. Чтобы включить правило и настроить частоту, используйте приведенные ниже процедуры.

По умолчанию правило проверки работы операционного центра Microsoft System Center отключено. Чтобы выполнить проверку работоспособности, необходимо включить это правило на сервере управления. Выполните следующие действия.

#### <a name="enable-the-rule-for-a-specific-management-server"></a>Включение правила для определенного сервера управления

1. В рабочей области **панели «Авторинг»,** найме консоли «Операционный менеджер операций» (The Authoring workspace, ) ищите правило Microsoft *System Center Operations Manager Run Health Check Rule* в панели **правил.**
2. В результатах поиска выберите тот, который содержит текст *Тип: сервер управления*.
3. Нажмите справа на правило, а затем нажмите **Overrides** > **для конкретного объекта класса: Management Server.**
...
1. В рабочей области **на консоли** «Менеджер операций» ищите правило, в разделе *«Менеджер по эксплуатации системы Майкрософт» выполнить правило проверки работоспособности* в разделе **«Правила».**
2. В результатах поиска выберите тот, который содержит текст *Тип: сервер управления*.
3. Нажмите правой кнопкой мыши правило, а затем нажмите **Переопределить правило** > **для всех объектов класса: Management Server**.
...
**Обновление, миграция и развертывание** - В этой области фокусировки представлены рекомендации, которые помогут вам обновить, перенести и развернуть сервер S'L в существующую инфраструктуру.
...
Чтобы использовать решение для проверки работоспособности в Log Analytics, его необходимо установить. Дополнительные сведения об установке решений см. в разделе [Установка решения по управлению](../../azure-monitor/insights/solutions.md). После установки можно просматривать сводку рекомендаций с помощью плитки проверки работоспособности System Center Operations Manager на странице **Обзор** в рабочей области на портале Azure.
...
1. Войти на портал Azure [https://portal.azure.com](https://portal.azure.com)по адресу .
2. На портале Azure щелкните **Другие службы** в нижнем левом углу. В списке ресурсов введите **log Analytics**. Как только вы начнете вводить символы, список отфильтруется соответствующим образом. Выберите **аналитику журнала**.
...
2. Выполните следующий запрос, чтобы получить список рекомендаций, не выполненных на компьютерах в вашей среде.
    ...
    Вот скриншот, показывающий запрос поиска журнала:<br><br> ![Поиск в журналах](./media/scom-assessment/scom-log-search.png)<br>
...
4. На сервере управления операциями - *SystemDrive*: »Программные файлы »Microsoft System Center 2012 R2»Операционный менеджер.Сервер.
...
*Я добавил решение «Проверка здоровья» в рабочее пространство журнала Analytics. Но я не вижу рекомендаций. Почему бы нет?* Добавив решение, используйте следующие шаги и просмотрите рекомендации на панели мониторинга Log Analytics.  
...
*Если другой сервер обнаружен после того, как я добавил решение «Менеджер по работе с системой менеджеров по работе с системой», будет ли он проверен?* Да, после обнаружения он будет проверяться в дальнейшем. По умолчанию это будет происходить каждые семь дней.

*Как называется процесс сбора данных?* AdvisorAssessment.exe
...
*Существует сообщение `Failed to connect to the SQL Instance (….).` в предпосылке сбоев. В чем проблема?* AdvisorAssessment.exe (процесс, собирающий данные) выполняется в рамках процесса службы работоспособности на сервере управления. В ходе проверки работоспособности этот процесс пытается подключиться к экземпляру SQL Server, на котором размещена база данных Operations Manager. Эта ошибка может возникать, когда правила брандмауэра блокируют подключение к экземпляру SQL Server.

*Какой тип данных собирается?* Собираются данные следующих типов: данные WMI, данные реестра, данные журнала событий, данные Operations Manager посредством Windows PowerShell, SQL-запросов и сборщика сведений о файлах.
