...
С полным списком кодов состояния можно ознакомиться в статье [Коды состояния HTTP в Azure Cosmos DB](https://docs.microsoft.com/rest/api/cosmos-db/http-status-codes-for-cosmosdb).
...
Вы можете закрепить любой раздел метрик на [панели мониторинга Azure](https://docs.microsoft.com/azure/azure-portal/azure-portal-dashboards), щелкнув значок канцелярской кнопки в правом верхнем углу раздела.
...
* Настройте [уведомления о метриках](../platform/alerts-metric.md) и [работоспособности служб](../../service-health/alerts-activity-log-service-notifications.md), чтобы получать автоматизированные оповещения для обнаружения проблем.

* Узнайте о том, где поддерживаются книги, как создавать новые и изменять существующие отчеты, а также многое другое, ознакомившись со статьей [Создание интерактивных отчетов с помощью книг Azure Monitor](../platform/workbooks-overview.md).
