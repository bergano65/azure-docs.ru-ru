...
Итак, в приведенном ниже примере показано `eastus-billingint` , что в основном получают запросы на чтение, но с небольшим количеством Upsert и запросов на создание. `westeurope-billingint` В то время как объект доступен только для чтения с точки запроса, по крайней мере за последние четыре часа, на которые в настоящий момент находится рабочая область, с помощью параметра интервала времени.
