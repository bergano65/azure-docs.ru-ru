---
title: Мониторинг Azure Cosmos DB с помощью Azure Monitor для Cosmos DB | Документация Майкрософт
...
Итак, в приведенном ниже примере показано, что в `eastus-billingint` основном получают запросы на чтение, но с небольшим количеством Upsert и запросов на создание. В то время как объект доступен `westeurope-billingint` только для чтения с точки запроса, по крайней мере за последние четыре часа, на которые в настоящий момент находится рабочая область, с помощью параметра интервала времени.
...
## <a name="customize-azure-monitor-for-azure-cosmos-db"></a>Настройка Azure Monitor для Azure Cosmos DB
