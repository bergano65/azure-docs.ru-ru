...
В книге **Обзор** для выбранной подписки в таблице отображаются интерактивные метрики хранилищ и состояние доступности службы для 10 учетных записей хранения, сгруппированных в пределах подписки. Результаты можно фильтровать по параметрам, которые вы выбираете в следующих раскрывающихся списках:
...
### <a name="what-happens-when-i-click-on-a-recently-pinned-tile-in-the-dashboard"></a>Что произойдет, если щелкнуть недавно закрепленную плитку на панели мониторинга?

* Если щелкнуть в любом месте плитки, откроется вкладка, на которой была закреплена плитка. Например, если закрепить граф на вкладке "Обзор учетной записи хранения", когда вы щелкните плитку на панели мониторинга, откроется представление по умолчанию, однако если закрепить граф из собственной сохраненной копии, откроется представление сохраненной копии.
...
* При сохранении книги вы можете создать копию книги, внеся в нее изменения и изменив заголовок. При сохранении книга не перезаписывается, текущая книга будет всегда отображаться в представлении по умолчанию.
* **Несохраненная** книга — это просто представление по умолчанию.


### <a name="why-dont-i-see-all-my-subscriptions-in-the-portal"></a>Почему на портале не отображаются все мои подписки?

При запуске портала на нем будут показаны данные только для выбранных подписок. Чтобы изменить выбор подписок, перейдите в верхний правый угол и щелкните записную книжку со значком фильтра. Отобразится вкладка "Каталог и подписки".
...
1. Затем измените значение "Число разбиений".
...
### <a name="what-is-time-granularity-time-grain"></a>Что такое степень детализации времени (интервал времени)?
...
### <a name="what-is-the-time-granularity-once-we-pin-any-part-of-the-workbooks-to-a-dashboard"></a>Какая степень детализации времени используется после закрепления какой-либо части книг на панели мониторинга?

По умолчанию для степени детализации времени задано значение "Автоматически", в настоящее время его невозможно изменить.

### <a name="how-do-i-change-the-timespan-time-range-of-the-workbook-step-on-my-dashboard"></a>Как изменить интервал и диапазон времени шага в книге на панели мониторинга?
...
![В параметре "Настройка параметров плитки" выберите раскрывающийся список для диапазона времени, чтобы изменить его.](./media/storage-insights-overview/fqa-timespan.png)

### <a name="how-do-i-change-the-title-of-the-workbook-or-a-workbook-step-i-pinned-to-a-dashboard"></a>Как изменить заголовок или шаг книги, закрепленной на панели мониторинга?
...
Чтобы изменить имя шага в сохраненной книге, щелкните "Изменить" под шагом, а затем — значок шестеренки в нижней части параметров.
