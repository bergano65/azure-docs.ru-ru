...
Служба "Аналитика SQL Azure" — это расширенное облачное решение для наблюдения за производительностью баз данных SQL Azure, эластичных пулов и управляемых экземпляров в нужном масштабе и в нескольких подписках на одной панели. Эта служба собирает и отображает важные метрики производительности Базы данных SQL Azure благодаря встроенным средствам аналитики, которые помогают устранять соответствующие неполадки.
...
Аналитика SQL Azure — это облачное решение для мониторинга с поддержкой потоковой передачи диагностических данных телеметрии для баз данных SQL Azure: отдельных, в пуле и для баз данных Управляемых экземпляров. Так как это решение не использует агенты для подключения к Log Analytics, оно не поддерживает мониторинг серверов SQL Server, размещенных в локальной среде или на виртуальных машинах (сведения о совместимости см. в следующей таблице).

| Подключенный источник | Поддерживаются | ОПИСАНИЕ |
| --- | --- | --- |
| [Система диагностики Azure](../../azure-monitor/platform/collect-azure-metrics-logs.md) | **Да** | Данные метрик и журнала Azure отправляются в Log Analytics непосредственно из Azure. |
| [Учетная запись хранения Azure](../../azure-monitor/platform/collect-azure-metrics-logs.md) | Нет  | Log Analytics не считывает данные из учетной записи хранения. |
| [Агенты Windows](../../azure-monitor/platform/agent-windows.md) | Нет  | Решение не использует прямые агенты Windows. |
| [Агенты Linux](../../azure-monitor/learn/quick-collect-linux-computer.md) | Нет  | Решение не использует прямые агенты Linux. |
| [Группа управления System Center Operations Manager](../platform/om-agents.md) | Нет  | Решение не использует прямое подключение агента Operations Manager к Log Analytics. |
...
2. На портале Azure щелкните **+Создать ресурс** и выберите **Аналитика SQL Azure**.  
    ![Мониторинг и управление](./media/azure-sql/monitoring-management.png)
...
4. В области **Службы анализа SQL Azure (предварительная версия)** щелкните **Создать**.  
    ![Создание](./media/azure-sql/portal-create.png)
5. В области **Создание решения** выберите или создайте рабочую область, в которую необходимо добавить решение, а затем нажмите кнопку **Создать**.
    ...
    ![Добавление в рабочую область](./media/azure-sql/add-to-workspace.png)
...
![Элемент служб анализа SQL Azure](./media/azure-sql/azure-sql-sol-tile.png)
...
Обратите внимание: если потоковая передача некоторых метрик или журналов в службу Azure Log Analytics не выполняется, плитки в решении не будут заполнены данными мониторинга.
...
[Оповещения можно легко создать](../../azure-monitor/platform/alerts-metric.md) с помощью данных, поступающих из ресурсов службы "База данных SQL Azure". Вот несколько полезных [запросов журналов](../../azure-monitor/log-query/log-query-overview.md), которые можно использовать с оповещениями журналов:
...
- Используйте [поиск по журналам](../../azure-monitor/log-query/log-query-overview.md) в Log Analytics для просмотра подробных данных SQL Azure.
- [Создавайте пользовательские панели мониторинга](../../azure-monitor/platform/dashboards.md), отображающие данные SQL Azure.
- [Создавайте оповещения](../../azure-monitor/platform/alerts-overview.md), предупреждающие о возникновении определенных событий SQL Azure.
