...
* [Мониторинг подключения службы](network-performance-monitor-service-endpoint.md): вы можете отслеживать подключение своих пользователей к нужным службам, определять инфраструктуру в пути, а также узкие места в сети. Вы можете узнавать о простоях, прежде чем с ними столкнутся пользователи, а также точно определять проблемные области в сетевом пути. 
...
Базовая процедура установки агентов приведена в статьях [Подключение компьютеров Windows к службе Log Analytics](../../log-analytics/log-analytics-om-agents.md) и [Подключение Operations Manager к Log Analytics](../../log-analytics/log-analytics-om-agents.md).
