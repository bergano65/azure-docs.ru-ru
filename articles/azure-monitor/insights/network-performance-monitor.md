...
Используйте основные процессы для установки агентов на странице [Подключение компьютеров Windows к Azure Monitor](../platform/agent-windows.md) и [подключение Operations Manager к Azure Monitor](../platform/om-agents.md).
...
* **Протокол TCP**. Если вы выбрали TCP как протокол мониторинга, откройте порт брандмауэра для агентов, используемых для мониторинга производительности сети и мониторинга ExpressRoute, чтобы обеспечить подключение между агентами. Чтобы открыть порт, запустите скрипт PowerShell [EnableRules.ps1](https://aka.ms/npmpowershellscript), не задавая каких-либо параметров, в окне PowerShell с правами администратора.
    ...
    >[!NOTE]
    > Для монитора подключения службы не нужно запускать скрипт PowerShell [EnableRules. ps1](https://aka.ms/npmpowershellscript ) .
...
* **Присоединиться к когорте наших клиентов**. Мы всегда заинтересованы в новых клиентах. Став одним из них, вы получаете доступ к новым функциям и возможностям, а также помогаете нам улучшить монитор производительности сети. Если вы хотите присоединиться, заполните этот [краткий опрос](https://aka.ms/npmcohort). 
