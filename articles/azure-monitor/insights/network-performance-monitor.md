...
* [Монитор подключений службы](network-performance-monitor-service-connectivity.md). Вы можете отслеживать подключение своих пользователей к нужным службам, определять инфраструктуру в пути, а также узкие места в сети. Вы можете узнавать о простоях, прежде чем с ними столкнутся пользователи, а также точно определять проблемные области в сетевом пути. 
...
* **Протокол TCP**. Если вы выбрали TCP как протокол мониторинга, откройте порт брандмауэра для агентов, используемых для мониторинга производительности сети и мониторинга ExpressRoute, чтобы обеспечить подключение между агентами. Чтобы открыть порт, запустите скрипт PowerShell [EnableRules.ps1](https://aka.ms/npmpowershellscript), не задавая каких-либо параметров, в окне PowerShell с правами администратора.
    ...
    >[!NOTE]
    > Нет необходимости запускать [EnableRules.ps1](https://aka.ms/npmpowershellscript ) сценарий PowerShell для мониторинга подключения службы.
