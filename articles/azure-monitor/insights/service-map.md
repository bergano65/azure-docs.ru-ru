...
Эта статья описывает подключение и использование Сопоставления служб. Для получения информации о настройке предпосылок для этого решения можно найти обзор [Azure Monitor для обзора свмов.](vminsights-enable-overview.md#prerequisites) Подводя итоги, необходимо:

* Рабочее пространство для включения этого решения.

* Агент Log Analytics, установленный на компьютере Windows или сервере Linux, настроенном для отчета о том же рабочем пространстве, с которым вы включили решение.

* Агент зависимости, установленный на компьютере Windows или сервере Linux.

>[!NOTE]
>Если Сопоставление служб уже развернуто, вы можете также просматривать свои карты в Azure Monitor для виртуальных машин, которая включает дополнительные возможности для мониторинга работоспособности и производительности виртуальной машины. Дополнительные сведения см. в разделе [Общие сведения об Azure Monitor для виртуальных машин](../../azure-monitor/insights/vminsights-overview.md). Чтобы узнать о различиях между решением карты обслуживания и функцией [FAQ](../faq.md#azure-monitor-for-vms)Azure Monitor для карты VMs, см.
...
1. Включите решение карты обслуживания с [рынка Azure](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.ServiceMapOMS?tab=Overview) или используя процесс, описанный в [решениях мониторинга Add из галереи решений.](solutions.md)
1. [Установите агент зависимости на Windows](vminsights-enable-hybrid-cloud.md#install-the-dependency-agent-on-windows) или [установите агент зависимости на Linux](vminsights-enable-hybrid-cloud.md#install-the-dependency-agent-on-linux) на каждом компьютере, где вы хотите получить данные. Агент зависимостей может отслеживать ближайшие соседние подключения, поэтому необязательно выполнять установку агента на каждом компьютере.
...
Если вы используете Azure Site Recovery и вам нужна помощь с определением последовательности восстановления среды приложения, служба схемы услуги может автоматически показать взаимосвязь между системами. Таким образом вы обеспечите надежный план восстановления. Выбрав важный сервер или группу и просмотрев их клиенты, можно определить внешние системы, которые следует восстановить после восстановления и обеспечения доступности сервера. И наоборот, глядя на критические зависимости серверов, можно определить, какие системы восстановить до восстановления систем фокусировки.
...
Агенты карты обслуживания собирают информацию обо всех процессах, подключенных к TCP, на сервере, где они установлены, и сведения о входящих и исходящих соединениях для каждого процесса.
...
После создания некоторых групп можно просмотреть их, выбрав вкладку Группы.
...
Панель **Process Summary** предоставляет дополнительную информацию о подключении процесса, включая связанные порты, входящие и исходящие соединения и неисправные соединения.
...
Агент зависимости *обычно* не требует перезагрузки при установке или удалении. Однако в некоторых редких случаях для продолжения установки Windows Server требуется перезагрузить. Это происходит, когда зависимость, как правило, Библиотека Реистримобовая Microsoft Visual C' требует перезагрузки из-за заблокированного файла.
...
Установщики библиотек среды выполнения создают журналы в папке %LOCALAPPDATA%\temp. `dd_vcredist_arch_yyyymmddhhmmss.log`Файл , где *арка* `amd64` `x86` или и *yyyymmdhmmss* является дата и время (24-часовые часы), когда журнал был создан. В журнале содержатся подробные сведения о проблеме, из-за которой блокируется установка.

Возможно, было бы полезно сначала установить [последние библиотеки времени выполнения.](https://support.microsoft.com/help/2977003/the-latest-supported-visual-c-downloads)

В таблице ниже приведены некоторые номера кодов и рекомендации по устранению проблем.

| Код | Описание | Решение |
|:--|:--|:--|
| 0x17 | Установщику библиотек требуется обновление Windows, которое не было установлено. | Проверьте сведения в последнем журнале установщика библиотек.<br><br>Если за `Windows8.1-KB2999226-x64.msu` ссылкой следует `Error 0x80240017: Failed to execute MSU package,` строка, у вас нет предпосылок для установки KB2999226. Следуйте инструкциям в разделе предварительных условий в [статье Universal C Runtime в Windows.](https://support.microsoft.com/kb/2999226) Может потребоваться запустить Центр обновления Windows и несколько раз выполнить перезагрузку, чтобы установить необходимые компоненты.<br><br>Запустите установщик Microsoft Dependency Agent повторно. |
...
Если установка агента зависимости прошла успешно, но вы не видите свою машину в решении Карты обслуживания:
* Успешно ли установлен Dependency Agent? Для этого проверьте, установлена и запущена ли служба.<br><br>
**Windows**: Ищите службу, названную **агентом зависимости Майкрософт.**
**Linux**: Ищите запущенный процесс **Microsoft-зависимость-агент**.

* Вы на [журнал Analytics бесплатно уровня?](https://azure.microsoft.com/pricing/details/monitor/) Бесплатный план позволяет до пяти уникальных машин Сервисная карта. Любые последующие машины не будут отображаться в карте обслуживания, даже если предыдущие пять больше не отправляют данные.

* Отправляет ли ваш сервер данные журнала и перф в журналы Azure Monitor? Перейдите в журналы Azure Monitor и запустите следующий запрос для компьютера: 

    ```kusto
    Usage | where Computer == "admdemo-appsvr" | summarize sum(Quantity), any(QuantityUnit) by DataType
    ```

Вы получили множество событий в результатах? Это последние данные? Если это так, ваш агент Log Analytics работает правильно и общается с рабочим пространством. Если нет, проверьте агента на вашей машине: [Log Analytics агент для устранения неполадок Windows](../platform/agent-windows-troubleshoot.md) или журнал Analytics агента для [устранения неполадок Linux.](../platform/agent-linux-troubleshoot.md)
...
Если вы видите вашу машину на карте обслуживания, но у нее нет данных процесса или соединения, это указывает на то, что агент зависимости установлен и работает, но драйвер ядра не загружается. 

Проверьте `C:\Program Files\Microsoft Dependency Agent\logs\wrapper.log file` (Windows) `/var/opt/microsoft/dependency-agent/log/service.log file` или (Linux). В последних строках файла должно быть указано, почему не удалось загрузить ядро. Например, если вы обновили ядро, оно может не поддерживаться в Linux.
