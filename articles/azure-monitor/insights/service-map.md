...
ms.locfileid: "75402598"
---
# <a name="using-service-map-solution-in-azure"></a>Использование решения "Сопоставление служб" в Azure

Функция "Сопоставление служб" автоматически обнаруживает компоненты приложений в системах Windows и Linux и сопоставляет взаимодействие между службами. Это решение позволяет рассматривать серверы как взаимосвязанные системы, обеспечивающие работу важных служб. Функция "Сопоставление служб" отображает сведения о подключениях между серверами, процессах, задержках во входящих и исходящих подключениях, а также портах в любой подключенной по протоколу TCP архитектуре без дополнительной настройки. Пользователям требуется только установить агент.
...
* Рабочая область Log Analytics для включения этого решения.

* Агент Log Analytics, установленный на компьютере Windows или на сервере Linux, настроенный для передачи той же рабочей области, с которой вы включили решение.

* Агент зависимостей, установленный на компьютере Windows или на сервере Linux.

>[!NOTE]
>Если Сопоставление служб уже развернуто, вы можете также просматривать свои карты в Azure Monitor для виртуальных машин, которая включает дополнительные возможности для мониторинга работоспособности и производительности виртуальной машины. Дополнительные сведения см. в разделе [Общие сведения об Azure Monitor для виртуальных машин](../../azure-monitor/insights/vminsights-overview.md). Дополнительные сведения о различиях между Сопоставление служб решением и функцией сопоставления Azure Monitor для виртуальных машин см. в следующих [часто задаваемых вопросах](vminsights-faq.md#how-is-azure-monitor-for-vms-map-feature-different-from-service-map).
...
1. Включите решение Сопоставление служб из [Azure Marketplace](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.ServiceMapOMS?tab=Overview) или с помощью процесса, описанного в разделе [Добавление решений мониторинга из коллекция решений](solutions.md).
1. [Установите агент зависимостей в Windows](vminsights-enable-hybrid-cloud.md#install-the-dependency-agent-on-windows) или [установите агент зависимостей в Linux](vminsights-enable-hybrid-cloud.md#install-the-dependency-agent-on-linux) на каждом компьютере, на котором требуется получить данные. Агент зависимостей может отслеживать ближайшие соседние подключения, поэтому необязательно выполнять установку агента на каждом компьютере.
...
### <a name="discovery"></a>Представление
...
Установщики библиотек среды выполнения создают журналы в папке %LOCALAPPDATA%\temp. Файл `dd_vcredist_arch_yyyymmddhhmmss.log`, где *Arch* — `x86` или `amd64`, а *YYYYMMDDHHMMSS* — Дата и время (24-часовой формат) при создании журнала. В журнале содержатся подробные сведения о проблеме, из-за которой блокируется установка.
...
Вы получили множество событий в результатах? Это последние данные? Если это так, то агент Log Analytics работает правильно и взаимодействует с рабочей областью. В противном случае проверьте агент на компьютере: [log Analytics агент для устранения неполадок в Windows](../platform/agent-windows-troubleshoot.md) или [Агент log Analytics для Linux](../platform/agent-linux-troubleshoot.md).
