...
>[!NOTE]
>Минимальная версия агента, поддерживаемая для бракованной Prometheus метрик, — ciprod07092019 или более поздней версии, а версия агента, поддерживаемая для записи ошибок `KubeMonAgentEvents` конфигурации и агента в таблице, — ciprod10112019. Для Azure Red Hat OpenShift и Red Hat OpenShift V4, версия агента ciprod04162020 или более поздняя. 
>
...
| | `monitor_kubernetes_pods` | логический | true или false | Если задано `true` значение в параметрах на уровне кластера, Azure Monitor для агента контейнеров будет наделять модули Pod Kubernetes по всему кластеру на наличие следующих заметок Prometheus:<br> `prometheus.io/scrape:`<br> `prometheus.io/scheme:`<br> `prometheus.io/path:`<br> `prometheus.io/port:` |
| | `prometheus.io/scrape` | логический | true или false | Включает брак/отход модуля Pod. Для параметра `monitor_kubernetes_pods` нужно задать значение `true`. |
...
2. Измените файл YAML ConfigMap, дополнив настройки, чтобы забракировать метрики Prometheus.
    ...
    - Чтобы настроить отходы метрик Prometheus, указав аннотацию Pod, выполните следующие действия.
       ...
       2. Укажите следующую конфигурацию для заметок Pod:
          ...
          Если вы хотите ограничить мониторинг определенными пространствами имен для модулей Pod с заметками, например, включайте только модули памяти, выделенные для рабочих `monitor_kubernetes_pod` нагрузок `true` , установите для параметра значение в ConfigMap и `monitor_kubernetes_pods_namespaces` добавьте фильтр пространства имен, указывающий пространства имен, из которых следует избавиться. Например `monitor_kubernetes_pods_namespaces = ["default1", "default2", "default3"]`.
