...
|Область | Ключ | Тип данных | Значение | Description (Описание) |
|------|-----|-----------|-------|-------------|
...
| | `monitor_kubernetes_pods` | Логический | Значение true или false | Если задано значение `true` в параметрах на уровне кластера, Azure Monitor для агента контейнеров будет Kubernetes модули Pod для всего кластера для следующих заметок Prometheus:<br> `prometheus.io/scrape:`<br> `prometheus.io/scheme:`<br> `prometheus.io/path:`<br> `prometheus.io/port:` |
...
| На уровне узла или на уровне кластера | `interval` | string | 60 | Интервал сбора по умолчанию равен одной минуте (60 секунд). Можно изменить коллекцию для *[prometheus_data_collection_settings. Node]* и/или *[prometheus_data_collection_settings. Cluster]* на единицы времени, такие как s, m, h. |
| На уровне узла или на уровне кластера | `fieldpass`<br> `fielddrop`| string | Массив с разделителями-запятыми | Можно указать определенные метрики для сбора или не из конечной точки, установив список Allow (`fieldpass`) и запретить (`fielddrop`). Сначала необходимо задать список разрешений. |
...
2. Измените файл YAML ConfigMap, дополнив настройки, чтобы забракировать метрики Prometheus.
    ...
    - Чтобы настроить отходы метрик Prometheus из набора управляющих программ агента для каждого отдельного узла в кластере, настройте следующие параметры в ConfigMap:
        ...
        >[!NOTE]
        >$NODE _IP — это конкретная Azure Monitor для параметра Containers, которую можно использовать вместо IP-адреса узла. Оно должно быть в верхнем регистре. 
