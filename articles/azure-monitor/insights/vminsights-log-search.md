...
>[!NOTE]
>Azure Monitor для виртуальных машин не поддерживает сбора и записи данных порта в рабочей области Log Analytics в следующих регионах:  
>- Восточная часть США  
>- Западная Европа
>
> Сбор таких данных включена в другом [поддерживаемые регионы](vminsights-enable-overview.md#log-analytics) для Azure Monitor для виртуальных машин. 

Каждая запись в VMBoundPort идентифицируется по следующим полям: 

| Свойство | Description |
|:--|:--|
|Процесс | Удостоверение процесса (или группы процессов), с которыми порт будет связано с.|
...
| Имя пользователя: | Учетная запись, с которой выполняется процесс |
