...
|Ip | IP-адрес порта (может иметь подстановочный знак IP, *0.0.0.0*) |
|Порт |Номер порта |
...
|Время создания | Метка времени записи (UTC) |
|Компьютер | Полное доменное имя компьютера | 
...
### <a name="vmprocess-record"></a>Запись Вмпроцесс
...
|Role | Роль *процесса:* *выборе*, *databaseServer*, *лдапсервер*, *смбсервер* |
