...
| Имя элемента | обязательные | ОПИСАНИЕ |
|:--|:--|:--|
| enabled       | Yes | Указывает, включено ли оповещение при его создании. |
...
| `name` | Yes | Отображаемое имя оповещения.  Это имя, которое отображается в консоли для правила генерации оповещений. |
...
| Имя элемента | обязательные | ОПИСАНИЕ |
|:--|:--|:--|
| `Operator` | Yes | Оператор сравнения может содержать следующие значения:<br><br>**gt — больше;<br>lt — меньше.** |
| `Value` | Yes | Значение для сравнения с результатами. |
...
| `Operator` | Yes | Оператор сравнения может содержать следующие значения:<br><br>**gt — больше;<br>lt — меньше.** |
| `Value` | Yes | Определяет, сколько раз должны быть соблюдены условия для активации оповещения. |
