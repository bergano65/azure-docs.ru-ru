...
description: Решения по управлению обычно включают в себя сохраненные запросы журналов для анализа данных, собираемых решением. В этой статье описывается, как определить Log Analytics сохраненные поисковые запросы в шаблоне диспетчер ресурсов.
ms.subservice: ''
ms.topic: conceptual
author: bwren
ms.author: bwren
...
> [!NOTE]
> В примерах, приведенных в этой статье, используются параметры и переменные, которые являются обязательными или общими для решений управления и описаны в статье [проектирование и создание решения для управления в Azure](solutions-creating.md) .
...
Имя рабочей области указывается в имени каждого ресурса Log Analytics. Это выполняется в решении с параметром **Workspace** , как показано в следующем примере ресурса SavedSearch.
...
Существуют два типа ресурса действия, которые задает свойство **Type**. Для расписания требуется одно действие **оповещения** , которое определяет сведения о правиле генерации оповещений и действия, выполняемые при создании оповещения. Ресурсы действия имеют тип `Microsoft.OperationalInsights/workspaces/savedSearches/schedules/actions`.
...
| `severity` | Да | Серьезность записи оповещения состоит из следующих значений.<br><br> **критическое**<br>**warning**<br>**извещен**
