---
title: Сохраненные поиски в управленческих решениях Документы Майкрософт
description: Решения для управления обычно включают сохраненные запросы журнала для анализа данных, собранных решением. В этой статье описывается, как определить сохраненные поиски Log Analytics в шаблоне менеджера ресурсов.
ms.subservice: ''
ms.topic: conceptual
author: bwren
ms.author: bwren
...
> [!IMPORTANT]
> Как [было объявлено ранее,](https://azure.microsoft.com/updates/switch-api-preference-log-alerts/)рабочее пространство для журналов(s), созданное после *1 июня 2019* г., сможет управлять правилами **оповещения,** используя только Azure scheduled'eryRules [REST API,](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules/) [Azure Resource Manager Template](../../azure-monitor/platform/alerts-log.md#managing-log-alerts-using-azure-resource-template) и [PowerShell cmdlet.](../../azure-monitor/platform/alerts-log.md#managing-log-alerts-using-powershell) Клиенты могут легко [переключить свои предпочтительные средства управления правилами оповещения](../../azure-monitor/platform/alerts-log-api-switch.md#process-of-switching-from-legacy-log-alerts-api) для старых рабочих пространств, чтобы использовать azure Monitor запланированных queryRules по умолчанию и получить много новых [преимуществ,](../../azure-monitor/platform/alerts-log-api-switch.md#benefits-of-switching-to-new-azure-api) таких как возможность использования родной PowerShell cmdlets, увеличение периода времени поиска в правилах, создание правил в отдельной группе ресурсов или подписки и многое другое.
...
> [!NOTE]
> В образцах в этой статье используются параметры и переменные, которые являются обязательными или общими для управленческих решений и описаны в [проекте и создании решения для управления в Azure](solutions-creating.md)

## <a name="prerequisites"></a>Предварительные требования
В этой статье предполагается, что вы уже знаете, как [создать решение по управлению](solutions-creating.md), а также знакомы со структурой [шаблона ARM](../../azure-resource-manager/templates/template-syntax.md) и файла решения.
...
Имя рабочей области указывается в имени каждого ресурса Log Analytics. Это делается в решении с параметром **рабочей области,** как в следующем примере ресурса SavedSearch.
...
| Свойство | Описание |
|:--- |:--- |
| категория | Категория для сохраненного поиска.  Сохраненные поиски в одном решении часто будут принадлежать одной категории, поэтому они группируются в консоли. |
...
Правила оповещения в устаревшем решении управления состоят из следующих трех различных ресурсов.
...
- **Действия оповещения.** У каждого правила генерации оповещений есть один ресурс группы действий или ресурс действий (устарел) типа **Alert**, который определяет параметры оповещения, например условия создания записи оповещения и его серьезность. Для ресурса [Группа действий](../../azure-monitor/platform/action-groups.md) может быть настроена группа действий, выполняемых при возникновении оповещения, например голосовой звонок, отправка SMS или сообщения по электронной почте, срабатывание веб-перехватчика, инструмента ITSM, runbook службы автоматизации, приложения логики и т. д.
...
Существуют два типа ресурса действия, которые задает свойство **Type**. Расписание требует одного действия **оповещения,** которое определяет детали правила оповещения и какие действия принимаются при создании оповещения. Ресурсы действия имеют тип `Microsoft.OperationalInsights/workspaces/savedSearches/schedules/actions`.
...
| `severity` | Да | Серьезность записи оповещения состоит из следующих значений.<br><br> **Критических**<br>**warning**<br>**Информационных**
