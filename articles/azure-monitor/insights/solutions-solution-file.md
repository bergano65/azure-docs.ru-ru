...
Ссылки на значения параметров в других элементах решения создаются с помощью синтаксиса **parameters('имя_параметра')**.  Например, чтобы получить доступ к имени рабочей области, используйте **Параметры (' workspaceName ')** .
...
Обращаться к значениям переменных в рамках решения можно с помощью синтаксиса **variables('имя_переменной')**.  Например, для доступа к переменной имяРешения можно использовать **переменные ("имя_решения")**.
...
| Свойство | Описание |
|:--- |:--- |
| workspaceResourceId |Идентификатор рабочей области log Analytics в форме * \<идентификатор группы ресурсов> имя\<\>рабочей области/провидерс/Микрософт.оператионалинсигхтс/воркспацес/*. |
