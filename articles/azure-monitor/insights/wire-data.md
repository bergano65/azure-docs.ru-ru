...
>[!NOTE]
>Решение "данные передачи" было заменено [сопоставление служб решением](service-map.md).  Оба компонента используют агент Log Analytics и агент зависимостей для получения данных сетевого подключения в Azure Monitor. 
> 
...
>
>Новые клиенты должны установить [сопоставление служб решение](service-map.md) или [Azure Monitor для виртуальных машин](vminsights-overview.md).  Сопоставление служб набор данных сравним с данными передачи.  Azure Monitor для виртуальных машин включает набор данных Сопоставление служб с дополнительными данными о производительности и функциями для анализа. 
...
| --- | --- | --- |
| Агенты Windows | Да | Решение "Данные передачи" анализирует и собирает данные из компьютеров агентов Windows. <br><br> В дополнение к [агенту log Analytics для Windows для](../platform/agent-windows.md)агентов Windows требуется агент зависимостей Майкрософт. Полный список версий операционной системы см. в разделе [Поддерживаемые операционные системы](vminsights-enable-overview.md#supported-operating-systems) . |
| Агенты Linux | Да | Решение "Данные передачи" анализирует и собирает данные из компьютеров агентов Linux.<br><br> В дополнение к [агенту log Analytics для Linux для](../learn/quick-collect-linux-computer.md)агентов Linux требуется агент зависимостей Майкрософт. Полный список версий операционной системы см. в разделе [Поддерживаемые операционные системы](vminsights-enable-overview.md#supported-operating-systems) . |
...
1. Включите решение Аналитика журнала действий из [Azure Marketplace](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.WireData2OMS?tab=Overview) или с помощью процесса, описанного в разделе [Добавление решений мониторинга из коллекция решений](./solutions.md).
...
1. Установите агент Log Analytics, выполнив действия, приведенные в статье [Подключение компьютеров Windows к службе Log Analytics в Azure](../platform/agent-windows.md).
...
1. Установите агент Log Analytics, выполнив действия, приведенные в статье [Настройка агента Log Analytics для компьютеров Linux в гибридной среде](../learn/quick-collect-linux-computer.md#obtain-workspace-id-and-key).
...
- Добавьте решение "данные передачи" в рабочую область Log Analytics с помощью процесса, описанного в разделе [Добавление решений мониторинга из коллекция решений](solutions.md). Дополнительная настройка не требуется.
...
![Снимок экрана: колонка "сетевой трафик" — "агенты" на панели мониторинга Wire Data 2.0, в которой отображается пропускная способность сети, используемая каждым компьютером.](./media/wire-data/log-search-example01.png)
...
![Снимок экрана: колонка "локальные подсети" на панели мониторинга Wire Data 2.0, показывающая пропускную способность сети, используемую каждым Локалсубнет.](./media/wire-data/log-search-example02.png)
...
![Снимок экрана: колонка протоколов уровня приложения на панели мониторинга Wire Data 2.0, показывающая пропускную способность сети, используемую каждым протоколом.](./media/wire-data/log-search-example03.png)
...
| Статус | Опасность потенциально вредоносной программы |
...
- [поиск в журналах](../log-query/log-query-overview.md) , чтобы просмотреть подробные записи поиска данных передачи.
