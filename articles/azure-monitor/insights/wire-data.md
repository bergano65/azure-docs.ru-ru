---
title: Решение для данных о проводах в Azure Monitor Документы Майкрософт
...
ms.locfileid: "77670650"
---
# <a name="wire-data-20-preview-solution-in-azure-monitor"></a>Решение Wire Data 2.0 (Preview) в Azure Monitor
...
>[!NOTE]
>Если вы уже развернули карту обслуживания или рассматриваете карту обслуживания или [мониторинг Azure для VMs,](../../azure-monitor/insights/vminsights-overview.md)существует новый набор данных метрик соединения, который они собирают и хранят в Azure Monitor, который предоставляет сопоставимую информацию с wire Data.

По умолчанию Azure Monitor регистрирует данные для процессора, памяти, диска и сетевых данных производительности с счетчиков, встроенных в Windows и Linux, а также других счетчиков производительности, которые можно указать. Процесс сбора сетевых и других данных выполняется в режиме реального времени для каждого агента, включая протоколы подсетей и протоколы уровня приложений, используемые компьютером.  Решение Wire Data ориентировано на сетевые данные на уровне приложения, а не на транспортном уровне TCP. Это решение также не ориентировано на отдельные пакеты ACK и SYN. Когда подтверждение будет завершено, подключение будет считаться активным и сторона будет помечена как подключенная. Подключение будет оставаться активным, пока на обеих сторонах сокет будет открытым и данные будут передаваться между этими сторонами. Как только обе стороны закрывают соединение, оно помечается как отключенное.  Таким образом, учитывается только пропускная способность успешно переданных пакетов. Данные о повторно отправленных и непереданных пакетах не записываются.
...
Однако просмотр метаданных не всегда полезен для проведения глубокой диагностики. Данные проводов в Azure Monitor не являются полным захватом сетевых данных.  Они не предназначены для расширенного устранения неполадок на уровне пакетов. Преимущество использования агента, по сравнению с другими методами сбора, заключается в том, что вам не нужно устанавливать приборы, перенастраивать сетевые коммутаторы или выполнять сложные конфигурации. Данные передачи основаны на использовании агента — установите агент на компьютере, и он будет отслеживать свой собственный сетевой трафик. Еще одно преимущество проявляется в случае, если нужно отслеживать рабочие нагрузки, выполняющиеся у поставщиков облачных служб, поставщиков услуг размещения или в Microsoft Azure, где пользователь не является владельцем уровня структуры.

## <a name="connected-sources"></a>Подключенные источники

Решение "Данные передачи" получает данные от агента зависимостей Майкрософт. Агент зависимости зависит от агента аналитики журналов для его подключения к Монитору Azure. Это означает, что сначала на сервере нужно установить и настроить агент Log Analytics, после чего можно будет установить Dependency Agent. В приведенной ниже таблице описаны подключенные источники, которые поддерживаются решением "Данные передачи".

| **Подключенный источник** | **Поддерживается** | **Описание** |
| --- | --- | --- |
| Агенты Windows | Да | Решение "Данные передачи" анализирует и собирает данные из компьютеров агентов Windows. <br><br> В дополнение к [агенту Log Analytics для Windows,](../platform/agent-windows.md)агенты Windows требуют агента зависимости Майкрософт. Ознакомьтесь [с поддерживаемыми операционными системами](vminsights-enable-overview.md#supported-operating-systems) для получения полного списка версий операционных систем. |
| Агенты Linux | Да | Решение "Данные передачи" анализирует и собирает данные из компьютеров агентов Linux.<br><br> В дополнение к [агенту Log Analytics для Linux,](../learn/quick-collect-linux-computer.md)агенты Linux требуют агента зависимости Майкрософт. Ознакомьтесь [с поддерживаемыми операционными системами](vminsights-enable-overview.md#supported-operating-systems) для получения полного списка версий операционных систем. |
| Группа управления System Center Operations Manager | Да | Решение "Данные передачи" анализирует и собирает данные из агентов Windows и Linux в подключенной [группе управления System Center Operations Manager](../platform/om-agents.md). <br><br> Требуется прямое подключение с компьютера-агента диспетчера операций Системного центра на Azure Monitor. |
...
В Windows агент мониторинга Майкрософт (MMA) используется как диспетчером системного центра, так и Azure Monitor для сбора и отправки данных. В зависимости от контекста этот агент называется агентом System Center Operations Manager, агентом Log Analytics, MMA или Direct Agent. Менеджер по работе с системными центрами и Azure Monitor предоставляют несколько иные версии ММА. Каждый из этих версий может отчитываться перед менеджером по операциям ВЦЦентра, Azure Monitor или обоим.

На Linux агент журналной аналитики для Linux собирает и отправляет данные на Azure Monitor. Данные проводов можно использовать на серверах с агентами, непосредственно подключенными к Azure Monitor, или на серверах, подключенных к Azure Monitor через группы управления операциями System Center.

Агент зависимости не передает никаких данных и не требует каких-либо изменений в брандмауэрах или портах. Данные в Wire Data всегда передаются агентом Log Analytics в Azure Monitor напрямую или через шлюз Log Analytics.

![Схема передачи данных агентами](./media/wire-data/agents.png)

Если вы являетесь пользователем управления системным центром с группой управления, подключенной к Azure Monitor:

- Дополнительная конфигурация не требуется, когда агенты диспетчера операций ВАшего центра могут получить доступ к Интернету для подключения к Azure Monitor.
- Вам необходимо настроить шлюз Log Analytics для работы с менеджером по операциям System Center, когда агенты диспетчера операций System Center не могут получить доступ к монитору Azure через Интернет.

Если компьютеры Windows или Linux не могут напрямую подключиться к службе, необходимо настроить агент Log Analytics для подключения к Azure Monitor с помощью шлюза Log Analytics. Шлюз Log Analytics можно скачать в [Центре загрузки Майкрософт](https://www.microsoft.com/download/details.aspx?id=52666).
...
- Привилегии администратора необходимы для установки или удаления агента зависимости.
- Агент зависимости должен быть установлен на компьютере с 64-битной операционной системой.

### <a name="operating-systems"></a>Операционные системы

В следующих разделах перечисляютподдерживаемые операционные системы для агента зависимости. Решение "Данные передачи" не поддерживает 32-разрядные архитектуры операционных систем.
...
В следующих разделах перечисляютподдерживаемые операционные системы для агента зависимости на Linux.  
...
1. Включите решение «Аналитика активности» на [рынке Azure](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.WireData2OMS?tab=Overview) или с помощью процесса, описанного в [решениях мониторинга Add из Галереи решений.](../../azure-monitor/insights/solutions.md)
2. Установите агент зависимости на каждом компьютере, где вы хотите получить данные. Агент зависимости может отслеживать соединения с непосредственными соседями, поэтому может не понадобиться агент на каждом компьютере.
...
Агент зависимости устанавливается на компьютерах, работающих под управлением Windows через InstallDependencyAgent-Windows.exe. Если запустить этот исполняемый файл без параметров, запустится мастер для установки в интерактивном режиме.

Используйте следующие шаги для установки агента зависимости на каждом компьютере под управлением Windows:
...
2. Скачать агент зависимости Windows, используя ссылку в предыдущем разделе, а затем запустить его с помощью следующей команды:`InstallDependencyAgent-Windows.exe`
...
Файлы для агента зависимости Windows по умолчанию размещаются в C: «Файлы программы»-Агент зависимости Microsoft.
...
Агент зависимости устанавливается на компьютерах Linux через InstallDependencyAgent-Linux64.bin, скрипт оболочки с самоизвечимым двоичным файлом. Вы можете запустить этот файл с помощью _sh_ или добавить разрешения на выполнение в сам файл.
...
Чтобы легко развернуть агент зависимости на многих серверах одновременно, это помогает использовать скрипт. Вы можете использовать следующие примеры скриптов для загрузки и установки агента зависимости на Windows или Linux.
...
Для развертывания агента зависимости через конфигурацию «Желаемое состояние» можно использовать модуль xPSDesiredStateConfiguration и немного кода, следующего:
...
### <a name="uninstall-the-dependency-agent"></a>Установить агент зависимости

Используйте следующие разделы, чтобы помочь вам удалить агент зависимости.

#### <a name="uninstall-the-dependency-agent-on-windows"></a>Удаление агента зависимости в Windows
...
#### <a name="uninstall-the-dependency-agent-on-linux"></a>Установить агент зависимости на Linux

Чтобы полностью удалить агент зависимости из Linux, необходимо удалить сам агент и разъем, который устанавливается автоматически с агентом. Удалить и то, и другое одновременно можно с помощью одной команды.
...
- Добавьте решение Wire Data в рабочее пространство журнала Analytics, используя процесс, описанный в [решениях мониторинга Add из галереи Решений.](solutions.md) Дополнительная настройка не требуется.
...
| RemoteIPCountry | Страна/регион удаленного IP-адреса |
