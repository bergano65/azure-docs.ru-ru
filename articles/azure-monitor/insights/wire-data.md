...
>[!NOTE]
>Если вы уже развернули Сопоставление служб или просматриваете Сопоставление служб или [Azure Monitor для виртуальных машин](../../azure-monitor/insights/vminsights-overview.md), то набор данных метрик подключения, который они собирает и сохраняет в Azure Monitor, который предоставляет сравнимую информацию для передачи данных.

По умолчанию Azure Monitor записывает данные о производительности ЦП, памяти, диска и сети из счетчиков, встроенных в Windows и Linux, а также другие счетчики производительности, которые можно указать. Процесс сбора сетевых и других данных выполняется в режиме реального времени для каждого агента, включая протоколы подсетей и протоколы уровня приложений, используемые компьютером.  Решение Wire Data ориентировано на сетевые данные на уровне приложения, а не на транспортном уровне TCP.  Решение не рассматривает отдельные подтверждения и SYN.  После завершения подтверждения оно рассматривается как активное подключение и помечается как подключенное. Подключение будет оставаться активным, пока на обеих сторонах сокет будет открытым и данные будут передаваться между этими сторонами.  Когда любая из сторон закрывает подключение, она помечается как отключенная.  Таким образом, учитывается только пропускная способность успешно переданных пакетов. Данные о повторно отправленных и непереданных пакетах не записываются.
...
1. Включите решение Аналитика журнала действий из [Azure Marketplace](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.WireData2OMS?tab=Overview) или с помощью процесса, описанного в разделе [Добавление решений мониторинга из коллекция решений](../../azure-monitor/insights/solutions.md).
...
Агент зависимостей устанавливается на компьютерах под управлением Windows через файла installdependencyagent-Windows. exe. Если запустить этот исполняемый файл без параметров, запустится мастер для установки в интерактивном режиме.
...
| Серьезность | Опасность потенциально вредоносной программы |
