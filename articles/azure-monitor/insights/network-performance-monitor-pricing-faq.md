...
**Как было использование мониторинга производительности, оплата осуществляется в старой модели?**
...
* **Данные**. Результаты различных тестов сети хранятся в рабочей области Log Analytics.

В прежней модели счет выставлялся на основе количества узлов и объема генерируемых данных. 

**Как оплачивается использование мониторинга производительности в новой модели?**
...
**У меня две подсети (A и B), и у меня есть несколько агентов в каждой подсети. Системный монитор отслеживает возможность подключения из всех агентов в подсети A ко всем агентам в подсети B. Будет ли взиматься плата на основе числа подключений между подсетями?**

№ В целях выставления счетов все подключения из подсети А к подсети В группируются вместе в одно соединение подсети. Плата будет взиматься за одно подключение. Монитор производительности продолжает отслеживать подключение между различными агентами в каждой подсети.

**Каковы затраты для мониторинга соединения подсети?**
...
**Как выставляются счета для данных, монитор производительности формирует?**

Плата за прием (отправку данных в рабочей области Log Analytics в Azure Monitor, обработку и индексирование) доступна на [странице с ценами](https://azure.microsoft.com/pricing/details/log-analytics/) для Log Analytics, в разделе приема данных. Плата за хранение данных (то есть, данные, сохраняемые по желанию клиента по истечении первого месяца использования) также доступна на [странице цен](https://azure.microsoft.com/pricing/details/log-analytics/) в разделе о приеме данных.
...
**Я использую монитор ExpressRoute для мониторинга нескольких каналов ExpressRoute. Зависит ли тариф от числа отслеживаемых каналов?**
...
**Каков объем данных, создаваемый при ExpressRoute отслеживает один канал?**
...
**Каковы причины для изменения объема данных?**
...
**Как выставляются счета за использование мониторинга конечной точки службы?**
...
**Каковы затраты на мониторинг конечной точки службы?**
