...
В этой статье объясняется, как настроить Azure Monitor для виртуальных машин, чтобы отслеживать работоспособность операционной системы на виртуальных машинах Azure, масштабируемых наборах виртуальных машин, виртуальных машинах в среде, а также обнаруживать и сопоставлять зависимости размещенных на них приложений.  
...
* Установите решения **ServiceMap** и **InfrastructureInsights**. Это можно сделать только с помощью шаблона Azure Resource Manager, предоставленного в этой статье.   
* Настройте рабочую область Log Analytics для сбора данных счетчиков производительности.

Чтобы настроить рабочую область для этого сценария масштабирования, обратитесь к разделу [Настройка рабочей области Log Analytics](#setup-log-analytics-workspace).
...
Функция схемы в Azure Monitor для виртуальных машин получает данные от Microsoft Dependency Agent. Dependency Agent использует агент Log Analytics для подключения к Log Analytics, поэтому в системе необходимо установить агент Log Analytics и настроить его для взаимодействия с агентом зависимостей. При включении решения "Azure Monitor для виртуальных машин" для отдельной виртуальной машины или при использовании методов для масштабируемого развертывания расширение Dependency Agent для виртуальных машин Azure используется для установки агента как часть этого процесса подключения. В гибридной среде Dependency Agent можно скачать и установить вручную или с помощью метода автоматического развертывания на виртуальных машинах, размещенных за пределами Azure.  
...
Чтобы пользователь мог включить решение "Azure Monitor для виртуальных машин" и получить доступ к его функциям, ему должен быть предоставлен следующий доступ.  
  
- Чтобы включить это решение, необходимо быть участником роли участника Log Analytics.  

- Чтобы иметь возможность просматривать данные о производительности, работоспособности и сопоставлении, пользователь должен быть участником роли читателя мониторинга для виртуальной машины Azure и рабочей области Log Analytics, настроенной с помощью Azure Monitor для виртуальных машин.   

Дополнительные сведения о том, как управлять доступом к рабочей области Log Analytics, см. в статье [Управление рабочими областями](../../log-analytics/log-analytics-manage-access.md).
...
5. На странице **Аналитические сведения (предварительная версия)** выберите **Попробовать**.
...
Этот раздел содержит инструкции по развертыванию Azure Monitor для виртуальных машин в нужном масштабе с помощью Политики Azure или Azure PowerShell.  

Ниже приведены общие шаги, которые необходимо выполнить для предварительной настройки рабочей области Log Analytics, прежде чем приступать к подключению виртуальных машин.

1. Создание рабочей области (если она еще не существует), которая может использоваться для поддержки Azure Monitor для виртуальных машин. Прежде чем создать рабочую область, ознакомьтесь со статьей [Управление рабочими областями](../../log-analytics/log-analytics-manage-access.md?toc=/azure/azure-monitor/toc.json), чтобы узнать о связанных затратах, методах управления и рекомендациях по соответствию требованиям.       
2. Включение счетчиков производительности в рабочей области для сбора данных на виртуальных машинах Windows и Linux.
3. Установка и включение решения **ServiceMap** и **InfrastructureInsights** в рабочей области.  
...
Если указанная в решении рабочая область Log Analytics еще не настроена для сбора данных от необходимых решению счетчиков производительности, их необходимо включить отдельно. Это можно сделать вручную, как описано [здесь](../../azure-monitor/platform/data-sources-performance-counters.md), или скачав и выполнив скрипт PowerShell, размещенный в [коллекции Powershell Azure](https://www.powershellgallery.com/packages/Enable-VMInsightsPerfCounters/1.1).
...
Ознакомьтесь с требованиями и методами развертывания для [агента Log Analytics в Linux и Windows](../../azure-monitor/platform/log-analytics-agent.md).  
