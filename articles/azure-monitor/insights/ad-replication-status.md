---
title: Мониторинг состояния ререпликации активного каталога
...
Решение AD Replication Status регулярно отслеживает среду Active Directory на наличие сбоев репликации.
...
* Для решения AD Replication Status требуется поддерживаемая версия .NET Framework 4.6.2 или выше, установленная на каждом компьютере с установленным агентом Log Analytics для Windows (также именуемым агентом мониторинга Майкрософт (MMA)...  Агент используется System Center 2016 - Менеджером по операциям, операционным менеджером 2012 R2 и Azure Monitor.
...
3. На этом компьютере настройте следующий раздел реестра.<br>Раздел: **HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\HealthService\Parameters\Management Groups\<ManagementGroupName>\Solutions\ADReplication**<br>Значение: **IsTarget**.<br>Данные значения: **true**

   > [!NOTE]
   > Эти изменения не всменяются до тех пор, пока вы не перезапустите службу агента по мониторингу Майкрософт (HealthService.exe).
...
**В: Я не хочу добавлять контроллеры доменов в рабочее пространство Log Analytics. Могу ли я использовать решение AD Replication Status?**
