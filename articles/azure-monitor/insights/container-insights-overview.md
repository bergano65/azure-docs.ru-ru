...
ms.locfileid: "73477056"
---
# <a name="azure-monitor-for-containers-overview"></a>Обзор службы "Azure Monitor для контейнеров"

Azure Monitor для контейнеров — это функция, предназначенная для наблюдения за производительностью рабочих нагрузок контейнеров, развернутых в экземплярах контейнеров Azure, управляемых кластерах Kubernetes, размещенных в службе Kubernetes Azure (AKS) или на самостоятельно управляемых Kubernetes кластерах. на Azure Stack. Мониторинг контейнеров крайне важен, особенно если вы управляете рабочим кластером в нужном масштабе с несколькими приложениями.
...
* Настройте оповещения для упреждающего уведомления или записи, когда загрузка ЦП и памяти на узлах или контейнерах превышает пороговые значения, или когда в кластере происходит изменение состояния работоспособности в инфраструктуре, узлах или свертке работоспособности рабочей нагрузки.
...
- Чтобы начать наблюдение за ядром AKS в Azure Stack или Kubernetes, развернутом локально, см. раздел [Настройка гибридных кластеров Kubernetes с Azure Monitor для контейнеров](container-insights-hybrid-setup.md).  
