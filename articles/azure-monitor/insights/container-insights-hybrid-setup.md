...
* Возможным 
    ...
    * Подсистема AKS в Azure и Azure Stack. Дополнительные сведения см. [в разделе AKS Engine on Azure Stack](https://docs.microsoft.com/azure-stack/user/azure-stack-kubernetes-aks-engine-overview?view=azs-1908)
...
* Среда выполнения контейнера: DOCKER, значок Кита и CRI совместимые среды выполнения, такие как CRI-O и контейнеры.

* Выпуск ОС Linux для главного и работающего узла: Ubuntu (18,04 LTS и 16,04 LTS) и Red Hat Enterprise Linux CoreOS 43,81.
...
Вы можете развернуть решение с помощью указанного шаблона Azure Resource Manager, используя командлет `New-AzResourceGroupDeployment` Azure PowerShell или с помощью Azure CLI.
...
Чтобы сначала указать полный идентификатор ресурса Log Analytics рабочей области, необходимой для значения `workspaceResourceId` параметра в файле **контаинерсолутионпарамс. JSON** , выполните следующие действия, а затем выполните командлет PowerShell или Azure CLI команду, чтобы добавить решение.
...
Можно указать надстройку в файле JSON спецификации кластера AKS Engine, которая также называется моделью API. В этой надстройке укажите версию `WorkspaceGUID` и `WorkspaceKey` log Analytics рабочей области в кодировке Base64, в которой хранятся собранные данные мониторинга.

Поддерживаемые определения API для кластера Azure Stack Hub можно найти в этом примере — [kubernetes-Container-monitoring_existing_workspace_id_and_key. JSON](https://github.com/Azure/aks-engine/blob/master/examples/addons/container-monitoring/kubernetes-container-monitoring_existing_workspace_id_and_key.json). В частности, найдите свойство **addons** в **kubernetesConfig**:
...
Если при попытке включить мониторинг для гибридного кластера Kubernetes возникает ошибка, скопируйте сценарий PowerShell [TroubleshootError_nonAzureK8s. ps1](https://raw.githubusercontent.com/microsoft/OMS-docker/ci_feature/Troubleshoot/TroubleshootError_nonAzureK8s.ps1) и сохраните его в папку на своем компьютере. Этот сценарий предназначен для обнаружения и устранения обнаруженных проблем. Проблемы, с которыми он предназначен для обнаружения и попыток исправления, приведены ниже.
...
* Проверьте, что все рабочие узлы Linux `kubernetes.io/role=agent` имеют метку, чтобы запланировать модуль RS. Если он не существует, добавьте его.
