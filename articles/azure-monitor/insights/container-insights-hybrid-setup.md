...
- Возможным
    ...
    - Подсистема AKS в Azure и Azure Stack. Дополнительные сведения см. [в разделе AKS Engine on Azure Stack](/azure-stack/user/azure-stack-kubernetes-aks-engine-overview?view=azs-1908&preserve-view=true)
...
7. Измените значения для **воркспацересаурцеид** , используя значение, скопированное на шаге 3, а для **Воркспацерегион** скопируйте значение **региона** после выполнения команды Azure CLI [AZ Monitor журнала-Analytics Workspace](/cli/azure/monitor/log-analytics/workspace?view=azure-cli-latest#az-monitor-log-analytics-workspace-list&preserve-view=true).
