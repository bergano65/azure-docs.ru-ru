...
Следующие конфигурации официально поддерживаются Azure Monitor для контейнеров.

- Возможным

    - Kubernetes в локальной среде
    
    - Подсистема AKS в Azure и Azure Stack. Дополнительные сведения см. [в разделе AKS Engine on Azure Stack](/azure-stack/user/azure-stack-kubernetes-aks-engine-overview?view=azs-1908)
...
7. Измените значения для **воркспацересаурцеид** , используя значение, скопированное на шаге 3, а для **Воркспацерегион** скопируйте значение **региона** после выполнения команды Azure CLI [AZ Monitor журнала-Analytics Workspace](/cli/azure/monitor/log-analytics/workspace?view=azure-cli-latest#az-monitor-log-analytics-workspace-list).
...
Например: `omsagent.proxy=http://user01:password@proxy01.contoso.com:8080`
