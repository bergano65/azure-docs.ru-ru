...
| диск, средн. — время чтения с диска (с) | 5 | 25 | > | 0,05 s | Critical | Sev1 | Linux | 
| диск, средн. — время обращения к диску (с) | 5 | 25 | > | 0,05 s | Critical | Sev1 | Linux | 
| диск, средн. — время записи на диск (с) | 5 | 25 | > | 0,05 s | Critical | Sev1 | Linux | 
| Состояние диска | 5 | 25 | <> | 1 (true) | Critical | Sev1 | Linux | 
| Общий процент загруженности процессора операционной системой | 5 | 10 | >= | 95% | Critical | Sev1 | Linux | 
...
| |Неизвестный |Невозможно вычислить состояние по нескольким причинам. В следующем разделе приведены дополнительные сведения и возможные решения. |
...
- Зависимости решения удалены. Чтобы повторно включить решения ServiceMap и Инфраструктуреинсигхтс в рабочей области Log Analytics, переустановите эти решения с помощью [шаблона Azure Resource Manager](vminsights-enable-at-scale-powershell.md#install-the-servicemap-and-infrastructureinsights-solutions). Или используйте параметр настроить рабочую область, расположенный на вкладке Приступая к работе.
...
|Resource |Выбор ресурса. В представление включены только те оповещения, целью которых выбраны ресурсы. Этот столбец доступен только после указания типа ресурса. |
