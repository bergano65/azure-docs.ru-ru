...
### <a name="advantages"></a>Преимущества
- Глубокая интеграция в Azure. Визуализации можно закреплять на панелях мониторинга из нескольких страниц Azure, включая [Обозреватель метрик](essentials/metrics-charts.md), [log Analytics](logs/log-analytics-overview.md)и [Application Insights](app/app-insights-overview.md).
- Поддержка метрик и журналов.
- Объедините данные из нескольких источников, включая выходные данные [Обозреватель метрик](essentials/metrics-charts.md), [запросы журнала](logs/log-query-overview.md), а также [карты](app/app-map.md) и доступность в [Application Insights](app/app-insights-overview.md).
...
[Grafana](https://grafana.com/) — это открытая платформа, которая отличается операционными панелями мониторинга. Она особенно полезна для обнаружения, изоляции и приоритизации производственных инцидентов. Добавьте [подключаемый модуль источника данных Grafana для Azure Monitor](visualize/grafana-plugin.md) в подписку Azure, чтобы с его помощью визуализировать данные метрик Azure.
...
> [!IMPORTANT]
> Представления скоро будут считаться устаревшими. Инструкции по преобразованию представлений в книги см. в разделе [Руководство по переходу от конструктора представлений к книгам Azure Monitor](visualize/view-designer-conversion-overview.md).

[Представления в Azure Monitor](visualize/view-designer.md) позволяют создавать пользовательские визуализации на основе данных журналов. Они используются в [решениях для мониторинга](insights/solutions.md) для представления собираемых данных.
...
- [Фильтры](visualize/view-designer-filters.md) для пользовательских параметров.
...
- Дополнительные сведения об [импорте данных журналов в Power BI](./visualize/powerbi.md).
- Дополнительные сведения о [подключаемом модуле источника данных Grafana для Azure Monitor](./visualize/grafana-plugin.md).
- Дополнительные сведения о [представлениях в Azure Monitor](visualize/view-designer.md).
