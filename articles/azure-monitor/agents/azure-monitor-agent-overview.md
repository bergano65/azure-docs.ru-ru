...
- [Агент log Analytics](../platform/log-analytics-agent.md) — отправляет данные в log Analytics рабочую область и поддерживает решения Azure Monitor для виртуальных машин и мониторинга.
- [Расширение диагностики](../platform/diagnostics-extension-overview.md) — отправляет данные в метрики Azure Monitor (только для Windows), концентраторы событий Azure и хранилище Azure.
- [Агент Telegraf](../platform/collect-custom-metrics-linux-telegraf.md) — отправляет данные в метрики Azure Monitor (только для Linux).
