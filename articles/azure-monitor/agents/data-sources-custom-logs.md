...
> [!IMPORTANT]
> В этой статье рассматривается сбор пользовательских журналов с помощью [агента log Analytics](../platform/log-analytics-agent.md) , который является одним из агентов, используемых Azure Monitor. Другие агенты собираются разные данные и настраиваются по-разному. Список доступных агентов и данных, которые они могут собираются, см. в разделе [Обзор агентов Azure Monitor](../agents/agents-overview.md) .
...
- Отправьте данные непосредственно в Azure Monitor с помощью [API сборщика данных HTTP](../platform/data-collector-api.md). 
