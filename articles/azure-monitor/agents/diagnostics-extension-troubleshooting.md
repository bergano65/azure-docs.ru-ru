...
- Убедитесь, что скомпилированный код на самом деле содержит **строки Diagnostics. Trace** (для проверки используйте Reflector, Ildasm или ILSpy). Команды **Diagnostics. Trace** удаляются из скомпилированного двоичного файла, если не используется символ условной компиляции TRACE. Если для сборки проекта используется msbuild, это обычная проблема.   
...
**2. данные счетчиков производительности доступны в хранилище, но не отображаются на портале.**
