...
## <a name="important-configuration-files"></a>Важные файлы конфигурации
 ...
 >[!NOTE]
 >Измененные файлы конфигурации для счетчиков производительности и системный журнал будут перезаписаны при настройке сбора в [меню "Данные" в дополнительных параметрах Log Analytics](../agents/agent-data-sources.md#configuring-data-sources) для рабочей области на портале Azure. Чтобы отключить конфигурацию для всех агентов, отключите сбор в разделе **Дополнительные параметры** Log Analytics, а для одного агента выполните следующую команду:  
...
| 4 | Скрипту omsadmin предоставлена недопустимый прокси-сервер. Проверьте прокси-сервер и изучите [документацию по использованию прокси-сервера HTTP](../platform/log-analytics-agent.md#firewall-requirements). |
...
3. Дважды убедитесь, что конечные точки, перечисленные в списке [требований к сетевому брандмауэру](../platform/log-analytics-agent.md#firewall-requirements) Azure Monitor, добавлены в список разрешений правильно. При использовании службы автоматизации Azure также связаны необходимые шаги по настройке сети.
