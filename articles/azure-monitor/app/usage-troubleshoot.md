---
title: Инструменты анализа проблем пользователей - Azure Application Insights
...
**Инструменты анализа поведения пользователей показывают, что мое приложение имеет одного пользователя/сессию, но я знаю, что мое приложение имеет много пользователей/сессий. Как я могу исправить эти неправильные подсчеты?**
...
**Мое приложение имеет тысячи различных просмотров страниц и пользовательских имен событий. Трудно провести различие между ними, и инструменты аналитики поведения пользователей часто становятся безответными. Как я могу исправить эти проблемы с именованием?**

Представления страниц и имена пользовательских событий используются с помощью средств для анализа поведения пользователей. Оптимальное именование событий имеет решающее значение при получении выгоды от этих инструментов. Целью является баланс между наличием слишком мало, слишком общие имена ("Кнопка нажал") и имеющие слишком\/много, слишком конкретные имена ("Edit кнопка нажата на http: / www.contoso.com/index").
...
* Если имена представления страницы указаны вручную в коде [ `trackPageView` ](https://github.com/Microsoft/ApplicationInsights-JS/blob/master/API-reference.md)с помощью API, измените имя на менее конкретное. Избегайте распространенных ошибок, например, помещения URL-адреса в имя представления страницы. Вместо этого используйте параметр URL-адреса API `trackPageView`. Переместите другие данные из имени представления страницы в пользовательские свойства.
