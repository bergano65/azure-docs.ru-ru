...
* Для некоторых платформ приложений или заданных параметров необходимо выполнить дополнительные шаги, чтобы перехватывать дополнительные исключения:
  ...
  * [WCF](#wcf)
  ...
  Эта статья конкретно посвящена приложениям .NET Framework с точки зрения примера кода. Некоторые из методов, которые работают для .NET Framework, устарели в SDK .NET Core. Если у вас есть приложение .NET Core, обратитесь к [документации .NET Core SDK.](https://docs.microsoft.com/azure/azure-monitor/app/asp-net-core)
...
*Никаких исключений? Посмотреть [исключения захвата](#exceptions).*
...
Отобразится полноэкранное представление с тенденциями частоты сбоев для запросов, количеством невыполненных запросов и числом затронутых пользователей. Справа вы увидите некоторые из наиболее полезных дистрибутивов, характерных для выбранной операции, включая три верхних кода ответа, три верхних типа исключений и три типа неисправных типов зависимостей.

![Рассмотрение ошибок на вкладке "Операции"](./media/asp-net-exceptions/failures0719.png)

Одним щелчком мыши можно просмотреть репрезентативные образцы для каждого из этих подмножества операций. В частности, для диагностики исключений можно нажать на количество конкретного исключения, представленное вкладкой «Информация о транзакциях «Конец-конец», например, на этой:

![Вкладка сведения о транзакциях в конце](./media/asp-net-exceptions/end-to-end.png)

**Кроме того,** вместо того, чтобы рассматривать исключения из конкретной операции сбоя, можно начать с общего представления исключений, переключившись на вкладку Исключений в верхней части. Здесь вы увидите все исключения, собранные для отслеживаемого приложения.

*Никаких исключений? Посмотреть [исключения захвата](#exceptions).*


## <a name="custom-tracing-and-log-data"></a>Пользовательская трассировка и данные журналов
Для получения диагностических данных своего приложения вставьте код для отправки собственных данных телеметрии. Это отображается в диагностическом поиске вместе с запросом, представлением страницы и другими автоматически собранными данными.
...
Чтобы увидеть эти события, откройте [Поиск](../../azure-monitor/app/diagnostic-search.md) из левого меню, выберите **типы событий**выпадающих событий, а затем выберите пользовательские события, трассировку или исключение.

![Детализация](./media/asp-net-exceptions/customevents.png)

> [!NOTE]
> Если приложение генерирует много телеметрических данных, модуль адаптивной выборки автоматически сокращает объем отправляемых на портал данных, пересылая только репрезентативную часть событий. События, составляющие часть той же операции, отбираются как группа, что позволяет перемещаться между связанными событиями. [Узнайте о выборке.](../../azure-monitor/app/sampling.md)
>
>
...
Параметры свойств и измерений являются необязательными, но полезны для [фильтрации и добавления](../../azure-monitor/app/diagnostic-search.md) дополнительной информации. Например, если у вас есть приложение, которое запускает несколько игр, вы можете просматривать все отчеты об исключениях для каждой конкретной игры. Вы можете добавить в каждый словарь столько элементов, сколько вам нужно.

## <a name="browser-exceptions"></a>Исключения браузера
Большинство исключений браузера регистрируются.

Если на вашей веб-странице есть файлы скриптов из сетей доставки контента или других доменов, убедитесь, что тег скрипта имеет атрибут, ```crossorigin="anonymous"```и что сервер отправляет [заголовки CORS.](https://enable-cors.org/) Это позволит вам извлекать трассировки стека и регистрировать необработанные исключения JavaScript из этих ресурсов.

## <a name="reuse-your-telemetry-client"></a>Повторное использование телефонного клиента

> [!NOTE]
> TelemetryClient рекомендуется мгновенно использовать один раз и повторно использовать в течение всего срока службы приложения.

Ниже приведен пример правильного использования TelemetryClient.
...
Если для конфигурации [CustomErrors](https://msdn.microsoft.com/library/h0hfz6fc.aspx) установлено значение `Off`, исключения будут доступны для сбора в [HTTP-модуле](https://msdn.microsoft.com/library/ms178468.aspx). Тем не менее, если это значение `RemoteOnly` (по умолчанию) или `On`, то исключение будет удалено и недоступно для автоматического сбора в Application Insights. Вы можете исправить это, переопределив [класс System.Web.Mvc.HandleErrorAttribute](https://msdn.microsoft.com/library/system.web.mvc.handleerrorattribute.aspx)и применяя переопределяемый класс, как показано для различных версий MVC ниже ([источник GitHub](https://github.com/AppInsightsSamples/Mvc2UnhandledExceptions/blob/master/MVC2App/Controllers/AiHandleErrorAttribute.cs)):
...
Откройте вкладку Metric Explorer, добавьте новую диаграмму и выберите **коэффициент исключения,** указанный в таблицах производительности.
