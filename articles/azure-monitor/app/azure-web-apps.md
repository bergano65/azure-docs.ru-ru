...
| Данные | Коллекция .NET Basic | Рекомендуемая коллекция для .NET |
| --- | --- | --- |
...
Поддерживаются следующие версии .NET Core: ASP.NET Core 2,1, ASP.NET Core 2,2, ASP.NET Core 3,0, ASP.NET Core 3,1

Для мониторинга на основе .NET Core, автономного развертывания и приложений на основе Linux в настоящее время **не поддерживаются** мониторинг с помощью агентов и расширений. ([Инструментирование вручную](./asp-net-core.md) с помощью кода будет работать во всех предыдущих сценариях.)
...
2. Указав, какой ресурс следует использовать, можно выбрать способ, с помощью которого Application Insights собирает данные для каждой платформы приложения. .NET Core предлагает **рекомендованную коллекцию** или **отключена** для ASP.NET Core 2,1, 2,2, 3,0 и 3,1.
...
Веб-приложения на основе службы приложений Java в настоящее время не поддерживают автоматическое отслеживание на основе агентов и расширений. Чтобы включить мониторинг для приложения Java, необходимо [вручную выполнить инструментирование приложения](./java-get-started.md).
...
|`AppContainsDiagnosticSourceAssembly**:true`|Это значение указывает, что расширение обнаружило ссылки на `System.Diagnostics.DiagnosticSource` приложение и будет отключаться.| Удаление ссылки.
