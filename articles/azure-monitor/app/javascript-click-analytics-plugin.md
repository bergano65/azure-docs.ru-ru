...
Щелкните подключаемый модуль автосбора в аналитике для Application Insights пакета SDK для JavaScript, который позволяет автоматически отслеживать события щелчка на веб-страницах на основе `data-*` тегов META. Этот подключаемый модуль использует `data-*` Глобальные атрибуты для записи событий щелчка и заполнения данных телеметрии.

## <a name="getting-started"></a>Начало работы

Пользователи могут настроить подключаемый модуль автосбора в меню "аналитика" с помощью NPM.
...
| Имя                      | Тип    | По умолчанию   | Описание                                                                                       |
|---------------------------|---------|-----------|---------------------------------------------------------------------------------------------------|
| уседефаултконтентнамеорид | Логическое | false     | Если определенный элемент не помечен по умолчанию Кустомдатапрефикс или Кустомдатапрефикс не предоставлен пользователем, этот флаг используется для получения стандартного атрибута HTML для contentName. |
| кустомдатапрефикс          | строка  | `data-`   | Автоматически записывать имя и значение элементов, помеченных указанным префиксом.       |
| аиблобаттрибутетаг        | строка  | `ai-blob` | Подключаемый модуль поддерживает добавление тегов метаданных содержимого BLOB-объектов JSON вместо отдельных `data-*` атрибутов. |
| метадатапрефикс            | строка  | null      | Автоматически записать имя и содержимое элемента meta заголовка HTML с указанным префиксом. |
| каптуреаллметадатаконтент | строка  | null      | Автоматически захватить все имена и содержимое элементов meta заголовка HTML. Значение по умолчанию — false. Если этот флажок включен, переопределяет предоставленные Метадатапрефикс. |
| парентдататаг             | строка  | null      | Прекращает обход модели DOM для записи имени содержимого и значения элементов при обнаружении этого тега.|
| днтдататаг                | строка  | `ai-dnt`  | Элементы HTML с этим атрибутом будут игнорироваться подключаемым модулем для сбора данных телеметрии.|

### <a name="behaviorvalidator"></a>бехавиорвалидатор

Функцию Бехавиорвалидатор можно использовать, если требуется обеспечить согласованность данных, хотя автоматическая проверка поведений с тегами в коде соответствует заранее определенному списку известных и принятых таксономий в Организации. Это не обязательно или ожидается, что большинство Azure Monitorных клиентов будут использовать его, но оно доступно для расширенных сценариев. Существует три различные функции обратного вызова Бехавиорвалидатор, предоставляемые в рамках этого расширения. Тем не менее пользователи могут использовать собственные функции обратного вызова, если предоставляемые функции не решают ваше требование. Цель заключается в том, чтобы использовать собственную структуру данных о поведении, подключаемый модуль использует эту функцию проверяющего элемента управления при извлечении поведений из тегов данных.
