...
| context.location.city |string |Производный от `clientip` , если известен |
...
| context.operation.id |string |Элементы, имеющие одинаковые, отображаются на `operation id` портале как связанные элементы. Обычно `request id` . |
...
| context.session.id |string |`Id`группы операций из одного источника. 30-минутный период без операций указывает на завершение сеанса. |
...
| internal.data.id |string | `Unique id`, который назначается при приеме элемента в Application Insights |
...
| запрос [0]`id` |string |`Operation id` |
