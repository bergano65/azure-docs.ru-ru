...
* [**Запросы экспортной аналитики**](#export-analytics-queries). Это является предпочтительным методом. Можно создать нужный запрос и экспортировать его в Power BI. Его можно разместить на панели мониторинга вместе с другими данными.
...
Установка [power BI Desktop](https://powerbi.microsoft.com/en-us/desktop/).
...
4. В Power BI Desktop выберите Получить**пустой запрос** **данных** > . Затем в редакторе запросов в разделе **Вид**выберите **Расширенный редактор**.
...
5. Чтобы разрешить Power BI доступ к Azure, может потребоваться указать учетные данные. Используйте **организационную учетную запись** для вху-си в учетную запись Майкрософт.
   
    ![Снимок экрана диалогового окна "Параметры запроса" в Power BI](./media/export-power-bi/power-bi-import-sign-in.png)

    Если вам необходимо проверить учетные данные, используйте команду меню **параметров исхода данных** в редакторе запроса. Укажите учетные данные, используемые для Azure. Они могут отличаться от учетных данных для Power BI.
...
1. [Сделайте воронку.](../../azure-monitor/app/usage-funnels.md)
2. Выберите **Power BI**.

   ![Снимок экрана с кнопкой Power BI](./media/export-power-bi/button.png)

3. В Power BI Desktop выберите Получить**пустой запрос** **данных** > . Затем в редакторе запросов в разделе **Вид**выберите **Расширенный редактор**.
...
1. Вопием на порталaz и убедитесь, что вы можете получить доступ к ресурсу.
2. Попробуйте обновить учетные данные для панели мониторинга.
3. Попробуйте очистить кэш от вашего Рабочего стола PowerBI.
...
В зависимости от объема данных, отправляемых вашим приложением, может потребоваться использовать функцию адаптивной выборки, которая отправляет только часть данных телеметрии. То же произойдет, если выборка настроена вручную в пакете SDK или на приеме. [Подробнее о выборке](../../azure-monitor/app/sampling.md).
...
1. Войти на [Power BI](https://app.powerbi.com/).
