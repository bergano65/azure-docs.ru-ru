...
Если вам требуется более гибкая альтернатива `DisableIpMasking` , чем запись всех IP-адресов или их части, можно использовать [инициализатор телеметрии](./api-filtering-sampling.md#addmodify-properties-itelemetryinitializer) , чтобы скопировать весь IP-адрес или его часть в настраиваемое поле. 
