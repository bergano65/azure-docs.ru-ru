...
Вы можете открыть диагностический поиск из вкладки Application Insights Overview вашего приложения (расположен в верхней панели) или под расследованием слева.

![Вкладка "Поиск"](./media/diagnostic-search/view-custom-events.png)

Перейдите в меню выпадения типов событий, чтобы увидеть список телеметрических элементов- запросов сервера, просмотров страниц, пользовательских событий, которые вы закодировали, и так далее. В верхней части списка результатов находится сводный график, показывающий количество событий с течением времени.

Нажмите из выпадающего меню или обновите, чтобы получить новые события.
...
Откройте меню выпадения типов событий и выберите типы событий, которые вы хотите увидеть. (Если позже требуется восстановить фильтры, нажмите «Сбросить».)

Ниже приведены типы событий.

* **Trace** - [Диагностические журналы](../../azure-monitor/app/asp-net-trace-logs.md) Trace, включая вызовы TrackTrace, log4Net, NLog и System.Diagnostic.Trace.
...
* **Телеметрия просмотра** - страниц,[отправленная веб-клиентом,](../../azure-monitor/app/javascript.md)используется для создания отчетов о просмотре страниц.
...
* **Исключение** - [Непойманные исключения на сервере](../../azure-monitor/app/asp-net-exceptions.md)и те, которые вы регистрируете с помощью TrackException().
* **Звонки зависимости** - [от серверного приложения](../../azure-monitor/app/asp-net-dependencies.md) к другим службам, таким как REST AIS или базы данных, и AJAX звонки из [вашего клиентского кода.](../../azure-monitor/app/javascript.md)
...
Можно фильтровать события по значениям их свойств. Доступные свойства зависят от выбранных типов событий. Нажмите на значок фильтра ![Значок фильтра](./media/diagnostic-search/filter-icon.png) для начала.
...
Чтобы найти все элементы с одинаковым значением свойства, введите его в панель поиска или нажмите на флажок, просматривая свойства во вкладке фильтра.

![Нажмите на флажок свойства во вкладке фильтра](./media/diagnostic-search/filter-property.png)

## <a name="search-the-data"></a>Поиск данных

> [!NOTE]
> Чтобы написать более сложные запросы, откройте [**журналы (Аналитика)**](../../azure-monitor/log-query/get-started-portal.md) из верхней части лезвия поиска.
>
...
Если приложение генерирует большое количество телеметрии (а вы используете ASP.NET версию SDK 2.0.0-beta3 или позже), модуль адаптивной выборки автоматически уменьшает объем, отправляемый на портал, отправив только репрезентативную часть событий. При этом связанные с тем же запросом события отбираются как группа, что позволяет перемещаться между связанными событиями.
...
Перейдите к сквозному представлению деталей транзакции, нажав на любой элемент телеметрии, затем выберите **Элемент «Создать работу».**
...
(Вы также можете настроить ссылку на вкладке «Рабочие элементы».)
