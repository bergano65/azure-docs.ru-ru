...
**clr!JITutil\_MonContention** или **clr!JITutil\_MonEnterWorker** указывает, что текущий поток ожидает снятия блокировки. Этот текст обычно отображается при выполнении команды **LOCK** языка C# путем вызова метода **Monitor.Enter** или метода с атрибутом **MethodImplOptions.Synchronized**. Конфликты блокировки обычно возникают, когда поток _а_ получает блокировку, а поток _б_ пытается получить ту же блокировку до освобождения потоком _a_ .
