---
title: Azure Monitor - Application Insights для Kubernetes со службой mesh Istio | Документация Майкрософт
description: Application Insight для Kubernetes — это мониторинга решение, которое позволяет собирать данные телеметрии Application Insights, относящиеся к входящих и исходящих запросов и из модулями, запущенными в кластере Kubernetes, используя технологию сетки службы Istio.
...
ms.locfileid: "64870692"
---
# <a name="application-insights-for-kubernetes-with-service-mesh"></a>Application Insights для кластера Kubernetes с помощью слоя взаимодействия между службами

> [!IMPORTANT]
> Application Insights для Kubernetes с помощью слоя взаимодействия между службами в настоящее время находится в общедоступной предварительной версии.
...
С помощью Application Insights для Kubernetes размещенного приложения, можно использовать:
...
### <a name="deploy-application-insights-for-kubernetes"></a>Развертывание Application Insights для Kubernetes

1. Загрузите и извлеките [ *Application Insights для Kubernetes* выпуска](https://github.com/Microsoft/Application-Insights-Istio-Adapter/releases/).
...
### <a name="verify-application-insights-for-kubernetes-deployment"></a>Проверка Application Insights для развертывания Kubernetes

- Убедитесь, что будет развернуто Application Insights для адаптера Kubernetes:
...
5. Представление *Application Insights для Kubernetes* адаптера трассировок.
