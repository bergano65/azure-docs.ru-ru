...
B. Отдельно плата взимается за [многошаговые веб-тесты](../../azure-monitor/app/monitor-web-app-availability.md#multi-step-web-tests). (К ним не относятся простые тесты доступности, плата за которые взимается при выставлении счетов за используемый объем данных.)  
...
Можно создать сценарий для настройки тарифного плана с помощью службы управления ресурсами Azure. [Подробнее](powershell.md#price).
...
## <a name="legacy-enterprise-pricing-plan"></a>Устаревший корпоративный тарифный план

Для первых пользователей Azure Application Insights по-прежнему существует два возможных тарифных плана: "Базовый" и "Корпоративный". По умолчанию используется базовый тарифный план, который соответствует описанному выше. Он включает в себя все возможности плана "Корпоративный", не требуя дополнительных затрат. В плане "Базовый" плата в основном взимается за объем принимаемых данных. 
