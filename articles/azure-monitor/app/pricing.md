...
Чтобы [изменить ежедневное ограничение с помощью Azure Resource Manager](../../azure-monitor/app/powershell.md), нужно изменить свойство `dailyQuota`.  С помощью Azure Resource Manager можно также установить `dailyQuotaResetTime` и ежедневное ограничение `warningThreshold`. 
