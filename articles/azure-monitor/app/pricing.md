...
Параметр Application Insights для [включения предупреждений для пользовательских измерений метрик](https://docs.microsoft.com/azure/azure-monitor/app/pre-aggregated-metrics-log-metrics#custom-metrics-dimensions-and-pre-aggregation) может также формироваться за дополнительные затраты, поскольку это может привести к созданию дополнительных метрик предварительной статистической обработки. Дополнительные [сведения](https://docs.microsoft.com/azure/azure-monitor/app/pre-aggregated-metrics-log-metrics) об основах на журналах и предварительно агрегированных метриках см. в Application Insights и [ценах](https://azure.microsoft.com/pricing/details/monitor/) на Azure Monitor пользовательских метрик.

### <a name="workspace-based-application-insights"></a>Application Insights на основе рабочей области

Для Application Insightsных ресурсов, которые отправляют свои данные в рабочую область Log Analytics, называемую [ресурсами Application Insights на основе рабочей области](create-workspace-resource.md), выставление счетов за прием и хранение данных осуществляется рабочей областью, в которой находятся данные Application Insights. Это позволяет клиентам использовать все варианты [ценовой модели](https://docs.microsoft.com/azure/azure-monitor/platform/manage-cost-storage#pricing-model) log Analytics, включающие резервирование емкости в дополнение к оплате по мере использования. Log Analytics также имеет больше возможностей для хранения данных, включая [хранение по типу данных](https://docs.microsoft.com/azure/azure-monitor/platform/manage-cost-storage#retention-by-data-type). Application Insights типы данных в рабочей области получают 90 дней хранения без оплаты. Использование веб-тестов и включение оповещений для пользовательских измерений метрик по-прежнему выводятся с помощью Application Insights. Узнайте, как отслеживанию приема и удержания данных в Log Analytics с использованием [и оценка затрат](https://docs.microsoft.com/azure/azure-monitor/platform/manage-cost-storage#understand-your-usage-and-estimate-costs), [управления затратами Azure, выставления счетов](https://docs.microsoft.com/azure/azure-monitor/platform/manage-cost-storage#viewing-log-analytics-usage-on-your-azure-bill) и [log Analytics запросов](#data-volume-for-workspace-based-application-insights-resources). 
...
Если вы включили функцию "Оценка объема данных на основе действия приложения" в Application Insights калькуляторе цен на Azure Monitoring, вы можете предоставить входные данные о приложении (запросы в месяц и Просмотры страниц в месяц, если вы собираетесь собирать данные телеметрии на стороне клиента), а затем калькулятор сообщит, как медиана и 90 процентиль объем данных, собираемых аналогичными приложениями. Эти приложения охватывают диапазон конфигураций Application Insights (например, некоторые из них имеют [выборку](../../azure-monitor/app/sampling.md)по умолчанию, некоторые из них не имеют выборки и т. д.), поэтому у вас по-прежнему есть элемент управления, позволяющий уменьшить объем данных, которые вы занимали гораздо меньше медианы, Но это отправная точка для понимания того, что видят другие похожие клиенты.
...
Чтобы получить дополнительные сведения о томах данных, выберите **метрики** для ресурса Application Insights и добавьте новую диаграмму. Для метрики диаграммы в разделе **метрики на основе журнала**выберите **том точек данных**. Щелкните **Применить разделение**и выберите Группировать по ** `Telemetryitem` типу**.
...
Существует два подхода к исследованию томов данных для Application Insights. В первой используется агрегированная информация в `systemEvents` таблице, а во второй `_BilledSize` — свойство, доступное для каждого полученного события. `systemEvents`не будет содержать сведения о размере данных для [Application-Insights на основе рабочей области](#data-volume-for-workspace-based-application-insights-resources).

#### <a name="using-aggregated-data-volume-information"></a>Использование собранных сведений о томе данных

Например, можно использовать `systemEvents` таблицу для просмотра объема данных, полученного за последние 24 часа с помощью запроса:
...
Чтобы получить дополнительные сведения об источнике томов данных, можно использовать `_BilledSize` свойство, которое имеется в каждом полученном событии.
...
#### <a name="data-volume-for-workspace-based-application-insights-resources"></a>Объем данных для ресурсов Application Insights на основе рабочих областей

Чтобы просмотреть тенденции объема данных для всех [ресурсов Application Insights на основе рабочей области](create-workspace-resource.md) за последнюю неделю, перейдите в рабочую область log Analytics и выполните запрос:
...
Чтобы запросить тенденции объема данных по типу для конкретного ресурса Application Insights на основе рабочей области, в рабочей области Log Analytics используйте:
...
Чтобы [изменить ежедневное ограничение с помощью Azure Resource Manager](../../azure-monitor/app/powershell.md), изменяемым свойством является `dailyQuota` .  С помощью Azure Resource Manager можно также задать `dailyQuotaResetTime` и ежедневное ограничение `warningThreshold` .

### <a name="create-alerts-for-the-daily-cap"></a>Создание оповещений для ежедневного ограничения

Application Insights ежедневное ограничение создает событие в журнале действий Azure, когда полученные тома данных достигают уровня предупреждения или ежедневного ограничения.  Вы можете [создать оповещение на основе этих событий журнала действий](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-activity-log#create-with-the-azure-portal). Ниже приведены названия сигналов для этих событий.
...
Если срок хранения уменьшится, то перед удалением самых старых данных будет льготный период в несколько дней.

Кроме того, удержание можно [настроить программно с помощью PowerShell](powershell.md#set-the-data-retention) с `retentionInDays` параметром. Если задать срок хранения данных 30 дней, можно немедленно запустить немедленную очистку старых данных с помощью `immediatePurgeDataOn30Days` параметра, который может быть полезен для сценариев, связанных с соответствием. Эта функция очистки доступна только через Azure Resource Manager и должна использоваться с крайней осторожностью. Время ежедневного сброса для ограничения объема данных можно настроить с помощью Azure Resource Manager, чтобы задать `dailyQuotaResetTime` параметр.
...
* Вы платите за каждый узел, который отправляет данные телеметрии для любых приложений на уровне каждого узла.
  * *Узел* — это физический или виртуальный сервер, или экземпляр роли платформы как услуги, в которой размещено ваше приложение.
