...
Триггеры поддерживаются только в версии 2,6 или более поздней версии агента профилировщика. Если вы используете службу приложений Azure, агент будет обновлен автоматически. Вы можете увидеть, какая версия агента выполняется, если вы перейдете на URL-адрес KUDU для веб-сайта и добавите \Диагностиксервицес к концу, например: https://yourwebsite.scm.azurewebsites.net/diagnosticservices. Веб-задание Application Insights Profiler должно иметь версию 2,6 или более позднюю. Вы можете принудительно выполнить обновление, перезапустив веб-приложение. 
...
Продолжительность | Задает период времени, в течение которого профилировщик будет запускаться при запуске.
...
Метка времени | Время, когда был захвачен профиль.
...
### <a name="step-1-generate-traffic-to-your-web-app-by-starting-a-web-performance-test"></a>Шаг 1. Создание входящего трафика для веб-приложения путем запуска веб-теста производительности
