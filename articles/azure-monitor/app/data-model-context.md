...
Анонимный идентификатор пользователя. Представляет интересы конечного пользователя приложения. Когда данные телеметрии отправляются из службы, контекст пользователя указывает на пользователя, который инициировал операцию в службе.
...
Аутентифицированный идентификатор пользователя. Противоположность анонимного идентификатора пользователя, это поле представляет пользователя с дружественным именем. Так как это персональные данные, они не собираются по умолчанию большинством пакетов SDK.
...
В мультитенантных приложениях это идентификатор или имя учетной записи, в которой работает пользователь. Примерами могут быть идентификатор подписки на портал Azure или название блога для блог-платформы.
...
Версия пакета SDK. Смотрите [эту статью](https://github.com/microsoft/ApplicationInsights-Home/blob/master/EndpointSpecs/SDK-VERSIONS.md) для получения информации.
