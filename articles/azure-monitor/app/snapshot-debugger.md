...
2. Чтобы открыть файл `.diagsession`, установите компонент VS Snapshot Debugger. Snapshot Debugger — это необходимый компонент рабочей нагрузки ASP.net в VS. Его можно выбрать в списке отдельных компонентов в установщике VS. Если вы используете версию Visual Studio 2017, предшествующие 15.5 необходимо установить расширение из [VS marketplace](https://aka.ms/snapshotdebugger).
...
Срок хранения данных по умолчанию составляет 7 дней. Для каждого экземпляра Application Insights максимальное число 50 снимков допускается в день.
