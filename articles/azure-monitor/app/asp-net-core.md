...
В качестве примера мы будем использовать [приложение MVC](https://docs.microsoft.com/aspnet/core/tutorials/first-mvc-app) , предназначенное для `netcoreapp3.0` . Эти инструкции можно применить ко всем ASP.NET Coreным приложениям. Если вы используете [рабочую службу](https://docs.microsoft.com/aspnet/core/fundamentals/host/hosted-services#worker-service-template), воспользуйтесь инструкциями, приведенными [здесь](./worker-service.md).
...
Используйте [инициализаторы телеметрии](https://docs.microsoft.com/azure/azure-monitor/app/api-filtering-sampling#addmodify-properties-itelemetryinitializer) , если хотите расширить телеметрию с дополнительной информацией.
...
Дополнительные сведения о настраиваемых отчетах данных в Application Insights см. в разделе [Справочник по API настраиваемых метрик Application Insights](https://docs.microsoft.com/azure/azure-monitor/app/api-custom-events-metrics/). Аналогичный подход можно использовать для отправки пользовательских метрик на Application Insights с помощью API- [интерфейса](./get-metric.md).
