...
* [Тесты доступности пользовательской трассировки](/dotnet/api/microsoft.applicationinsights.telemetryclient.trackavailability). Если вы решили создать пользовательское приложение для выполнения тестов доступности, `TrackAvailability()` метод можно использовать для отправки результатов в Application Insights.

**Для каждого Application Insights ресурса можно создать до 100 тестов доступности.**

> [!IMPORTANT]
> И [Проверка связи по URL-адресу](#create-a-url-ping-test) , и [многошаговый веб-тест](availability-multistep.md) используют общедоступную инфраструктуру DNS Интернета для разрешения доменных имен проверяемых конечных точек. Это означает, что если используется Частная зона DNS, необходимо либо убедиться, что каждое доменное имя теста также может быть разрешено серверами общедоступного доменного имени, либо, если это невозможно, можно использовать [тесты доступности пользовательской трассировки](/dotnet/api/microsoft.applicationinsights.telemetryclient.trackavailability) .
...
Помимо необработанных результатов, можно также просмотреть две метрики доступности в [Обозреватель метрик](../essentials/metrics-getting-started.md).
