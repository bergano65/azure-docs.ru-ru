...
Анализ изменений в настоящее время интегрирован в веб-приложение службы приложений для **диагностики и устранения проблем** , а также доступна как отдельная вкладка в портал Azure.
Дополнительные сведения см. в разделе *Просмотр изменений для всех ресурсов в Azure* для доступа к колонке анализ изменений и *анализ изменений для компонента веб-приложений* , который будет использоваться на портале веб-приложения далее в этой статье.
...
Такие параметры, как правило IP-конфигурации, параметры SSL и версии расширений, еще недоступны в аргументе ARG, поэтому запросы на анализ изменений и повторное вычисление этих изменений позволяют получить дополнительные сведения о том, что изменилось в приложении. Эти сведения пока недоступны в графе ресурсов Azure, но скоро будут доступны.
...
1. Включите **анализ изменений** и нажмите кнопку **сохранить**. Средство отображает все веб-приложения в рамках плана службы приложений. Чтобы включить анализ изменений для всех веб-приложений в рамках плана, можно использовать параметр уровень плана.

    ![Снимок экрана: Пользовательский интерфейс "Включение анализа изменений"](./media/change-analysis/change-analysis-on.png)


1. Чтобы получить доступ к анализу изменений, выберите **Диагностика и решение проблем** > **доступности и производительности** > **сбои приложений**. Вы увидите граф, который суммирует тип изменений со временем, а также сведения об этих изменениях. По умолчанию за последние 24 часа отображаются изменения, которые помогут устранить немедленные проблемы.
