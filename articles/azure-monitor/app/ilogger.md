...
Хотя выше подход можно использовать в приложении ASP.NET Core, наиболее распространенный подход будет объединить обычное приложение мониторинга (запросы, зависимости и т.д.) с помощью функций записи ILogger, как показано ниже.
...
*Я вижу что некоторые журналы ILogger отображаются два раза в Application Insights?*

* Это возможно в том случае, если у вас есть более старой версии (устаревшая) `ApplicationInsightsLoggerProvider` включена, вызвав `AddApplicationInsights` на `ILoggerFactory`. Установите флажок, если ваш `Configure` имеет следующие метод и удалите его.
...
* При отладке из Visual Studio при возникновении double ведения журнала, измените код, используемый для включения ApplicationInsights следующим образом, задав `EnableDebugLogger` равным false. Это относится только при отладке приложения.
