...
Следующими шагами являются [просмотр метрик из приложения,](../../azure-monitor/platform/metrics-charts.md) [запрос данных с помощью Analytics.](../../azure-monitor/app/analytics.md) 
...
1. В Visual Studio Solution Explorer, под  >  ** \<YourCloudService>** **roles,** откройте свойства каждой роли.
...
1. Чтобы настроить пакет SDK для отправки данных в ресурс Application Insights, сделайте следующее:
    ...
    * [Роль работника](https://github.com/Microsoft/ApplicationInsights-Home/blob/master/Samples/AzureEmailService/WorkerRoleA/WorkerRoleA.cs#L232)
...
## <a name="set-up-status-monitor-to-collect-full-sql-queries-optional"></a>Настройка монитора статуса для сбора полных запросов s'L (необязательно)

Этот шаг необходим только в том случае, если вы хотите запечатлеть полные запросы s'L на рамочном этапе .NET. 

1. В `\*.csdef` [задаче запуска](https://docs.microsoft.com/azure/cloud-services/cloud-services-startup-tasks) файла Добавить для каждой роли, аналогичной 
...
2. Скачать [InstallAgent.bat](https://github.com/microsoft/ApplicationInsights-Home/blob/master/Samples/AzureEmailService/WorkerRoleA/AppInsightsAgent/InstallAgent.bat) и [InstallAgent.ps1](https://github.com/microsoft/ApplicationInsights-Home/blob/master/Samples/AzureEmailService/WorkerRoleA/AppInsightsAgent/InstallAgent.ps1) `AppInsightsAgent` , положить их в папку на каждом ролевой проект. Убедитесь в том, чтобы скопировать их в каталог вывода через visual Studio свойства файла или построить скрипты.

3. Во всех ролях рабочей группы добавляйте переменные среды: 
...
1. Откройте созданные ранее ресурсы Application Insights.
   ...
   В разделе [Поиск][diagnostic] отображаются отдельные точки данных, а в [обозревателе метрик](../../azure-monitor/platform/metrics-charts.md) — агрегированные данные.
...
Чтобы просмотреть счетчики производительности и счетчики событий, откройте [обозреватель метрик](../../azure-monitor/platform/metrics-charts.md) и добавьте следующую диаграмму:
...
Вы можете указать дополнительные пользовательские или другие счетчики производительности Windows, редактируя *ApplicationInsights.config,* [как показано в этом примере.](https://github.com/Microsoft/ApplicationInsights-Home/blob/master/Samples/AzureEmailService/WorkerRoleA/ApplicationInsights.config#L14)
