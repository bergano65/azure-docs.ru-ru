...
Эта функция работает для любого веб-приложения, размещенного в облаке или на собственных серверах, которое создает данные телеметрии запросов или зависимостей, например, если у вас есть рабочая роль, вызывающая [TrackRequest ()](../../azure-monitor/app/api-custom-events-metrics.md#trackrequest) или [TrackDependency ()](../../azure-monitor/app/api-custom-events-metrics.md#trackdependency).
...
## <a name="failure-anomalies-v2"></a>Аномалии сбоев v2
Теперь доступна новая версия правила оповещения "аномалии ошибок". Эта новая версия работает на новой платформе оповещений Azure и предоставляет разнообразные улучшения по сравнению с существующей версией.

### <a name="whats-new-in-this-version"></a>Новые возможности в этой версии
- Более быстрое обнаружение проблем
- Расширенный набор действий. правило генерации оповещений создается с соответствующей [группой действий](https://docs.microsoft.com/azure/azure-monitor/platform/action-groups) с именем "Application Insights интеллектуального обнаружения", которая содержит действия электронной почты и веб-перехватчика, и может быть расширена для активации дополнительных действий при срабатывании оповещения.
- Более ориентированные уведомления. уведомления по электронной почте, отправленные из этого правила оповещения, теперь по умолчанию отправляются пользователям, связанным с ролями отслеживания мониторинга и мониторинга участников подписки. Дополнительные сведения об этом можно найти [здесь](https://docs.microsoft.com/azure/azure-monitor/app/proactive-email-notification).
- Более простая настройка с помощью шаблонов ARM — см. пример [здесь](https://docs.microsoft.com/azure/azure-monitor/app/proactive-arm-config).
- Поддержка общей схемы предупреждений. уведомления, отправленные из этого правила оповещения, соответствуют [общей схеме предупреждений](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-common-schema).
- Шаблон единой почты — уведомления по электронной почте от этого правила генерации оповещений имеют согласованный вид & с другими типами оповещений. После этого изменения можно получить оповещения об аномалиях сбоев с подробными сведениями диагностики, которые больше не доступны.

### <a name="how-do-i-get-the-new-version"></a>Разделы справки получить новую версию?
- Вновь созданные Application Insightsные ресурсы теперь подготавливаются с помощью новой версии правила генерации оповещений об аномалиях сбоя.
- Существующие ресурсы Application Insights с классической версией правила генерации оповещений об аномалиях при сбоях будут получать новую версию после переноса их подписки на размещение на новую платформу предупреждений в рамках процесса прекращения [действия классических оповещений](https://docs.microsoft.com/azure/azure-monitor/platform/monitoring-classic-retirement).

> [!NOTE]
> Новая версия правила оповещения "аномалии ошибок" остается бесплатной. Кроме того, действия электронной почты и веб-перехватчика, активируемые связанной группой действий "интеллектуальное обнаружение Application Insights", также доступны бесплатно.
> 
> 
...
![Настройка](./media/proactive-failure-diagnostics/032.png)
