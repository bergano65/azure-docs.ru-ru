...
## <a name="failure-anomalies-v2"></a>Сбой аномалии v2
Теперь доступна новая версия аномалии в сбоях правило генерации оповещений. Эта новая версия работает на новой платформе Azure предупреждений и дает ряд улучшений по сравнению с существующей версией.

### <a name="whats-new-in-this-version"></a>Новые возможности в этой версии?
- Быстрое обнаружение проблем
- Широкий набор действий — правило генерации оповещений создается со связанным [группы действий](https://docs.microsoft.com/azure/azure-monitor/platform/action-groups) с именем «Интеллектуального обнаружения Application Insights» содержит действия электронной почты и веб-перехватчика, который может быть расширен для активации дополнительных действий при оповещения активируется.
- Больше занимаетесь уведомлений — уведомления по электронной почте, отправляемых из этого правила генерации оповещений теперь отправляются по умолчанию для пользователей, связанные с ролями Monitoring Reader и Monitoring Contributor подписки. Дополнительные сведения об этом можно найти [здесь](https://docs.microsoft.com/azure/azure-monitor/app/proactive-email-notification).
- Упрощенную конфигурацию с помощью шаблонов ARM - см. пример [здесь](https://docs.microsoft.com/azure/azure-monitor/app/proactive-arm-config).
- Поддержка распространенных предупреждений схемы - уведомлений, отправляемых из этого правила генерации оповещений выполните [общей схеме оповещений](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-common-schema).
- Единая шаблона сообщения электронной почты - уведомления из этого правила генерации оповещений, иметь согласованный вид & вы с помощью других типов оповещений по электронной почте. Благодаря этому изменению параметр для получения оповещений аномальных сбоев с помощью Подробная диагностическая информация больше не доступен.

### <a name="how-do-i-get-the-new-version"></a>Как получить новую версию?
- Вновь созданные ресурсы Application Insights теперь подготавливаются с новой версией аномалии в сбоях правила генерации оповещений.
- Существующие Application Insights, ресурсами с помощью классической версии аномальных сбоев оповещения правило будет получить новую версию после размещения подписки переносится на новую платформу предупреждений как часть [классических оповещений процесс прекращения использования ](https://docs.microsoft.com/azure/azure-monitor/platform/monitoring-classic-retirement).

> [!NOTE]
> Новая версия правила генерации оповещений аномалии в сбоях останется бесплатной. Кроме того, действия электронной почты и веб-перехватчик активируется в соответствующих «Application Insights Интеллектуальное обнаружение» группа действий также предоставляются бесплатно.
> 
> 

## <a name="benefits-of-smart-detection"></a>Преимущества интеллектуального обнаружения
Обычные [оповещения метрик](../../azure-monitor/app/alerts.md) сообщают о возможном наличии проблемы. Однако система интеллектуального обнаружения начинает самостоятельно осуществлять диагностику, выполняя множество операций анализа, которые в противном случае вам пришлось бы проводить самостоятельно. Вы получаете тщательно подобранные результаты, что помогает быстро выявить первопричину проблемы.
...
Как и в случае [оповещений, настраиваемых вручную](../../azure-monitor/app/alerts.md), вы можете проверить состояние оповещения и настроить его в соответствующей колонке ресурса Application Insights. В отличие от других оповещений, здесь не требуется настройка интеллектуального обнаружения. При желании адреса электронной почты для отправки оповещений можно отключить или изменить.
...
![На странице "Обзор" щелкните плитку оповещения. Либо на любой странице метрики нажмите кнопку "Оповещения".](./media/proactive-failure-diagnostics/021.png)

Щелкните оповещение, чтобы его настроить.

![Параметр Configuration](./media/proactive-failure-diagnostics/032.png)
...
Для дальнейшего изучения воспользуйтесь ссылками в каждом разделе, которые укажут на [страницу поиска](../../azure-monitor/app/diagnostic-search.md) , относящуюся к соответствующим запросам, исключениям, зависимостям или трассировкам. Вы также можете открыть [портал Azure](https://portal.azure.com), выбрав ресурс Application Insights для своего приложения и открыв колонку "Сбои".
...
![Поиск по журналу диагностики](./media/proactive-failure-diagnostics/051.png)
...
![Общие данные оповещения](./media/proactive-failure-diagnostics/070.png)
...
* № У каждого приложения, которое отправляет данные телеметрии запросов, имеется правило генерации оповещений интеллектуального обнаружения.
