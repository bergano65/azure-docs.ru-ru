---
title: Отслеживайте пользовательские операции с помощью Azure Application Insights .NET SDK
...
Хотя есть [Контекст W3C Trace Context](https://www.w3.org/TR/trace-context/) и http Protocol для [корреляции для](https://github.com/dotnet/corefx/blob/master/src/System.Diagnostics.DiagnosticSource/src/HttpCorrelationProtocol.md) передачи деталей корреляции с запросом HTTP, каждый протокол очереди должен определить, как одни и те же детали передаются по сообщению очереди. Некоторые протоколы очереди (например, AMQP) разрешают передавать дополнительные метаданные. Другим протоколам (например, очереди службы хранилища Azure) требуется контекст для кодирования в полезных данных сообщения.

> [!NOTE]
> * **Кросс-компонентная трассировка пока не поддерживается для очередей** С помощью HTTP, если ваш производитель и потребитель отправляют телеметрию на различные ресурсы Application Insights, опыт диагностики транзакций и карта приложений отображают транзакции и карту сквозной. В случае очередей это еще не поддерживается. 
...
Он настроен по умолчанию на ASP.NET и ASP.NET основных приложений, с другими видами приложений, вы можете обратиться к [консоли приложений документации](../../azure-monitor/app/console.md)
...
С операцией `Dequeue` все не так просто. Пакет SDK Application Insights автоматически отслеживает HTTP-запросы. Тем не менее ему не известен контекст корреляции, пока выполняется анализ сообщения. Невозможно соотнести запрос HTTP, чтобы получить сообщение с остальной частью телеметрии, особенно когда получено более одного сообщения.
...
Application Insights использует тип зависимости для обработки опыта uI. Для очередей он распознает `DependencyTelemetry` следующие типы, которые улучшают [опыт диагностики транзакций:](/azure/azure-monitor/app/transaction-diagnostics)
- `Azure queue`для очередей хранения Azure
- `Azure Event Hubs`для концентраторов событий Azure
- `Azure Service Bus`для сервисного автобуса Azure

### <a name="batch-processing"></a>Пакетная обработка
Некоторые очереди поддерживают вывод из очереди нескольких сообщений с помощью одного запроса. Обработка таких сообщений предположительно является независимой и относится к разным логическим операциям. Невозможно соотнести операцию `Dequeue` с определенным обработанным сообщением.
...
## <a name="applicationinsights-operations-vs-systemdiagnosticsactivity"></a>ApplicationInsights операции против System.Diagnostics.Activity
`System.Diagnostics.Activity`представляет контекст распределенного отслеживания и используется рамками и библиотеками для создания и распространения контекста внутри и вне процесса и корреляции элементов телеметрии. Деятельность работает `System.Diagnostics.DiagnosticSource` вместе с - механизм омик между рамкой/ библиотекой для уведомления об интересных событиях (входящих или исходящих запросах, исключениях и т.д.).

Мероприятия первоклассных граждан в Application Insights и автоматической зависимости и `DiagnosticSource` сбора запросов в значительной степени зависит от них вместе с событиями. Если вы создаете активность в приложении - это не приведет к созданию телеметрии Application Insights. Application Insights необходимо получать события DiagnosticSource и знать имена событий и полезные нагрузки для перевода активности в телеметрию.

Каждая операция Application Insights (запрос `Activity` или `StartOperation` зависимость) включает в себя - когда называется, она создает активность внизу. `StartOperation`является рекомендуемым способом отслеживания телеметрии запроса или зависимости вручную и обеспечения того, чтобы все коррелировалось.

## <a name="next-steps"></a>Дальнейшие действия

- Изучите основы [корреляции данных телеметрии](correlation.md) в Application Insights.
- Проверьте, как коррелированные данные полномочия [трансопераций Диагностика Опыт](../../azure-monitor/app/transaction-diagnostics.md) и [приложение Карта](../../azure-monitor/app/app-map.md).
