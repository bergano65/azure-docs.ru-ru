...
* [Включите ведение журнала](#debug-data-from-the-sdk) , добавив элемент `<SDKLogger />` в корневой узел в файле ApplicationInsights. XML (в папке ресурсов проекта) и проверьте наличие записей, начинающихся с AI: info/warn/Error для любых подозрительных журналов. 
...
* Откройте страницу данных об использовании и предполагаемых расходов и проверьте, не выполняется ли операция [выборки](../../azure-monitor/app/sampling.md). (100% передача означает, что выборка не выполняется.) Служба Application Insights может быть настроена на принятие только части телеметрии, поступающих из приложения. Это помогает не выходить за пределы месячной квоты.
...
Чтобы включить ведение журнала SDK в приложениях с пружинной загрузкой с помощью Application Insights пружинной загрузки, добавьте в файл `application.properties` следующее:
