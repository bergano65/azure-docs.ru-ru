...
* Если в элементе зависимости `<version>` используется шаблон с подстановочными знаками (например, (Maven) `<version>[2.0,)</version>` или (Gradle) `version:'2.+'`), попробуйте указать конкретную версию, например `2.6.2`.
