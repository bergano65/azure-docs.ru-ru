...
* [Включите журнал,](#debug-data-from-the-sdk) добавив элемент под корневой `<SDKLogger />` узло в файл ApplicationInsights.xml (в папке ресурсов в проекте) и проверьте записи, предваряемые ИИ: INFO/WARN/ERROR для любых подозрительных журналов. 
...
* Вы достигли месячной квоты точек данных? Открытые настройки / Квота и ценообразование, чтобы узнать. Если это так, вы можете обновить свой план, или платить за дополнительную емкость. См. [таблицу цен](https://azure.microsoft.com/pricing/details/application-insights/).
...
* Откройте страницу данных об использовании и предполагаемых расходов и проверьте, не выполняется ли операция [выборки](../../azure-monitor/app/sampling.md). (100% передача означает, что выборка не работает.) Служба Application Insights может быть настроена на принятие только части телеметрии, поступаюей из вашего приложения. Это помогает не выходить за пределы месячной квоты.
...
**Как изменить ресурс Azure, в который отправляет данные ресурсAzра?**
...
### <a name="spring-boot-starter"></a>Весенняя загрузка Стартер

Чтобы включить sDK в журнал с помощью приложений Spring Boot `application.properties` Apps с помощью spring Boot Starter Application Insights, добавьте в файл следующее:
...
или распечатать стандартную ошибку:
...
Для включения в систему JVM Agent Logging обновлен [файл AI-Agent.xml:](java-agent.md)
...
### <a name="java-command-line-properties"></a>Свойства командной линии Java
_С версии 2.4.0_

Для включения в систему с использованием параметров командной строки без изменения файлов конфигурации:
...
**Я смотрю на [портал Azure](https://portal.azure.com). Карта говорит мне что-то о моем приложении?**
