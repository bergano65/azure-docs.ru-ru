...
Корреляция [HTTP протокол, также называемый Запрос-Id](https://github.com/dotnet/corefx/blob/master/src/System.Diagnostics.DiagnosticSource/src/HttpCorrelationProtocol.md), в настоящее время удаляется. Этот протокол определяет два заголовка:
...
[Application Insights SDK для](../../azure-monitor/app/java-get-started.md) Java-версии 2.0.0 или позже поддерживает автоматическую корреляцию телеметрии. Он автоматически `operation_id` заполняется для всех телеметрии (например, следов, исключений и пользовательских событий), выданных в рамках запроса. Он также распространяет заголовки корреляций (описанные ранее) для вызовов службы в службу через HTTP, если настроен [агент Java SDK.](../../azure-monitor/app/java-agent.md)

> [!NOTE]
> Поддерживаются только звонки, сделанные через Apache HttpClient для функции корреляции. Оба Весенний RestTemplate и Feign могут быть использованы с Apache HttpClient под капотом.

В настоящее время автоматическое распространение контекста в технологиях обмена сообщениями (таких как Kafka, RabbitM и Azure Service Bus) не поддерживается. Закодировать такие сценарии можно вручную, используя методы `trackDependency` и `trackRequest` методы. В этих методах телеметрия зависимости представляет собой сообщение, окабиваемые производителем. Запрос представляет собой сообщение, обрабатываемые потребителем. В этом случае и `operation_id`, и `operation_parentId` должны быть указаны в свойствах сообщений.

### <a name="telemetry-correlation-in-asynchronous-java-applications"></a>Телеметрия корреляции в асинхронных Java-приложениях

Чтобы узнать, как соотнести телеметрию в асинхронном приложении Spring Boot, [см.](https://github.com/Microsoft/ApplicationInsights-Java/wiki/Distributed-Tracing-in-Asynchronous-Java-Applications) Эта статья содержит руководство для инструментирования [Весна ThreadPoolTaskExcutor](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/scheduling/concurrent/ThreadPoolTaskExecutor.html) и [ThreadPoolTaskПланировщикr](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/scheduling/concurrent/ThreadPoolTaskScheduler.html).
