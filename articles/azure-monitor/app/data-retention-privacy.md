...
[Отладочные моментальные снимки](../../azure-monitor/app/snapshot-debugger.md) хранятся в течение пятнадцати дней. Такая политика хранения задается для каждого приложения отдельно. Если нужно изменить этот параметр, вы можете запросить изменения, открыв окно Службы Поддержки на портале Microsoft Azure.
...
| --- | --- |
| [Добавление пакета SDK для Application Insights в веб-проект .NET][greenbrown] |ServerContext<br/>Выводимые<br/>Счетчики производительности<br/>Запрошено<br/>**Исключения**<br/>Сеанс<br/>пользователей |
...
| metrics |Имя и значение метрики |
...
| Запрошено |URL-адрес, длительность, код ответа |
