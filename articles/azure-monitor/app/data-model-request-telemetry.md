...
Веб-пакет SDK Application Insights отправляет имя запроса "как есть" с учетом регистра. Группирование в пользовательском интерфейсе учитывает регистр, поэтому `GET /Home/Index` регистрируется отдельно от `GET /home/INDEX`, хотя они часто дают одинаковый контроллер и выполняемое действие. Причина заключается в том, что в общем случае URL-адреса [учитывают регистр](https://www.w3.org/TR/WD-html40-970708/htmlweb.html). Вы можете захотеть просмотреть, возникли ли все события `404` для URL-адресов, введенных прописными буквами. Можно прочитать дополнительные о сборе имен запросов по ASP.NET Web SDK в [блога](https://apmtips.com/blog/2015/02/23/request-name-and-url/).
...
Указание того, был ли вызов успешным. Это поле является обязательным. Если явно не задано значение `false`, запрос считается успешным. Задайте значение `false`, если операция была прервана из-за исключения или возвратила код ошибки.
