---
title: Модель данных для телеметрии запросов - Azure Application Insights
...
Веб-пакет SDK Application Insights отправляет имя запроса "как есть" с учетом регистра. Группирование в пользовательском интерфейсе учитывает регистр, поэтому `GET /Home/Index` регистрируется отдельно от `GET /home/INDEX`, хотя они часто дают одинаковый контроллер и выполняемое действие. Причина заключается в том, что в общем случае URL-адреса [учитывают регистр](https://www.w3.org/TR/WD-html40-970708/htmlweb.html). Вы можете захотеть просмотреть, возникли ли все события `404` для URL-адресов, введенных прописными буквами. Вы можете прочитать больше о коллекции имя запроса ASP.NET Web SDK в [блоге](https://apmtips.com/blog/2015/02/23/request-name-and-url/).
