...
Уведомления интеллектуального обнаружения по умолчанию включены и отправляются тем, кто имеет доступ к подписке[Monitoring Reader](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-reader) и [Monitoring Contributor](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-contributor), в которой размещается ресурс Application Insights. Чтобы изменить этот режим, либо щелкните **Настройка** в уведомлении по электронной почте, либо откройте параметры интеллектуального обнаружения в Application Insights. 
