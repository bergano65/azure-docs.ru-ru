...
Уведомления содержат диагностические сведения. Пример:
...
Уведомления интеллектуального обнаружения по умолчанию включены и отправляются тем, [кто имеет доступ](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-reader) к подписке, в которой размещается ресурс Application Insights. [Monitoring Contributor](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-contributor) Чтобы изменить этот режим, либо щелкните **Настройка** в уведомлении по электронной почте, либо откройте параметры интеллектуального обнаружения в Application Insights. 
...
* *Можно ли создавать собственные правила обнаружения аномалий или настраивать имеющиеся правила?*
  ...
  * Пока еще нет, но вы можете:
    * [Настройте оповещения](../../azure-monitor/app/alerts.md) , которые сообщают, когда метрика пересекает пороговое значение.
    * [Экспортируйте данные телеметрии](../../azure-monitor/app/export-telemetry.md) в [базу данных](../../azure-monitor/app/code-sample-export-sql-stream-analytics.md) или [в Power BI](../../azure-monitor/app/export-power-bi.md ), где их можно проанализировать самостоятельно.
...
* *Так что это заменяет [оповещения метрик](../../azure-monitor/app/alerts.md)?*
...
* Ссылки, которые помогут при диагностике проблемы:
  ...
  * Отчеты о сбоях. Если значение счетчика > 1, это означает, что в этой операции возникли сбои, которые могли повлиять на снижение производительности.

## <a name="dependency-duration-degradation"></a>Увеличение времени отклика зависимостей

Современные приложения еще больше и используют подход к проектированию микрослужб, который во многих случаях приводит к высокой надежности внешних служб. Например, если приложение зависит от какой-либо платформы для управления данными или вы разрабатываете собственную службу программ-роботов, вероятно, вы воспользуетесь услугами поставщика служб когнитивных вычислений, чтобы ваши программы-роботы вели себя человекоподобно, а также воспользуетесь службой банка данных, из которой программы-роботы будут извлекать ответы.  
...
* [Аналитика](../../azure-monitor/log-query/get-started-portal.md)
