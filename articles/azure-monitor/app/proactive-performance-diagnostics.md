...
1. **Триаж**. Уведомление показывает, скольких пользователей или сколько операций затрагивает проблема. Это поможет назначить приоритет проблемы.
2. **Область действия**. Проблема влияет на весь трафик или только на некоторые страницы? Она связана с определенными браузерами или расположениями? Эти сведения можно получить из уведомления.
...
Уведомления Smart Detection включены по умолчанию и отправляются тем, у кого [есть](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-reader) [доступ](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-contributor) к подписке, в которой находится ресурс Application Insights. Чтобы изменить этот режим, либо щелкните **Настройка** в уведомлении по электронной почте, либо откройте параметры интеллектуального обнаружения в Application Insights. 
...
* *Можно ли создавать собственные правила обнаружения аномалий или настраивать имеющиеся правила?*
  ...
  * Пока еще нет, но вы можете:
    * [Насете оповещения,](../../azure-monitor/app/alerts.md) которые укажут вам, когда метрика пересекает порог.
    * [Экспорт телеметрии](../../azure-monitor/app/export-telemetry.md) в [базу данных](../../azure-monitor/app/code-sample-export-sql-stream-analytics.md) или [в Power BI](../../azure-monitor/app/export-power-bi.md ), где вы можете проанализировать его самостоятельно.
...
* *Так это заменяет [метрические оповещения?](../../azure-monitor/app/alerts.md)*
...
* Ссылки, которые помогут при диагностике проблемы:
  ...
  * Отчеты о сбоях - Если считать > 1, это означает, что в этой операции были сбои, которые могли бы способствовать снижению производительности.

## <a name="dependency-duration-degradation"></a>Увеличение времени отклика зависимостей

Современные приложения все чаще принимают подход к проектированию микроуслуг, что во многих случаях приводит к высокой надежности внешних услуг. Например, если приложение зависит от какой-либо платформы для управления данными или вы разрабатываете собственную службу программ-роботов, вероятно, вы воспользуетесь услугами поставщика служб когнитивных вычислений, чтобы ваши программы-роботы вели себя человекоподобно, а также воспользуетесь службой банка данных, из которой программы-роботы будут извлекать ответы.  
...
* [Моментальный отладчик](../../azure-monitor/app/snapshot-debugger.md)
