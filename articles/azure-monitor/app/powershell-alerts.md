...
| Имя показателя | Имя экрана | Описание |
| --- | --- | --- |
...
| `clientPerformance.clientProcess.value` |Время обработки на стороне клиента |Время с момента получения последнего байта документа до загрузки модели DOM. Обработка асинхронных запросов может продолжаться. |
...
| `request.duration` |времени ответа сервера; |Время с момента получения HTTP-запроса до завершения отправки ответа. |
