...
Предположим, что вы тестируете инструмент, который получает зависящие от времени данные (например, запасы) от внешнего источника. При записи веб-теста вам необходимо будет использовать определенное время, которое задается в качестве параметров теста: время начала и время окончания.
...
1. Добавьте подключаемый модуль веб-теста для каждого нужного значения параметра переменной. На панели инструментов веб-теста выберите **Добавить подключаемый модуль веб-теста**.
...
2. Откройте свойства каждого подключаемого модуля. Присвойте ему имя и настройте его для использования текущего времени. Для другого задайте значение «–15» для параметра «Добавление минут».
...
Теперь можно передать тест на портал. Во время каждого запуска теста будут использоваться динамические значения.
...
| Не после | Интервал времени, для которого токен будет допустимым.  Значение по умолчанию равно 5 минутам. |
