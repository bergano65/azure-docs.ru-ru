...
Настройте [оповещение](../../azure-monitor/platform/alerts-log.md) для **времени ответа от сервера**. Пороговое значение может быть в пределах от 1 до 2 секунд.

![Снимок экрана, показывающий, как настроить оповещение о времени ответа сервера.](./media/how-do-i/030-server.png)
...
2. [Установите оповещение](../../azure-monitor/platform/alerts-log.md) об исключении подсчета метрики

### <a name="email-on-an-event-in-my-app"></a>Уведомлять меня по электронной почте о событиях в приложении
Предположим, что вы хотите получать уведомления по электронной почте при возникновении определенных событий. Application Insights не предоставляет эту функцию напрямую, но позволяет [отправлять оповещение, если метрика превысит пороговое значение](../../azure-monitor/platform/alerts-log.md).
...
![Снимок экрана, показывающий, как создать диаграмму в обозревателе метрик для просмотра оповещения.](./media/how-do-i/010-alarm.png)
...
![Снимок экрана, показывающий, как настроить оповещение на срабатывание, если метрика превышает среднее значение в течение короткого периода.](./media/how-do-i/020-threshold.png)
...
* Отдельные этапы разработки, тестирования и выпуска версий. Используйте разные ресурсы Application Insights. Получите ключи инструментирования из файла web.config. [Дополнительные сведения](../../azure-monitor/app/separate-resources.md)
...
![Снимок экрана, на котором показаны параметры поиска по указанным идентификаторам пользователей.](./media/how-do-i/110-search.png)

* выполнить фильтрацию метрики для анонимных или прошедших проверку подлинности пользователей.

![Снимок экрана, на котором показано, как фильтровать метриксс пользователей как анонимных, так и прошедших проверку.](./media/how-do-i/115-metrics.png)
...
Не рекомендуется использовать отдельную базу данных `TelemetryConfiguration.Active` в консоли и приложениях ASP.NET Core.
Если вы создали экземпляр `TelemetryConfiguration` самостоятельно, задайте для `DisableTelemetry` значение `true`.

В приложениях ASP.NET Core доступ к экземпляру `TelemetryConfiguration` можно получить с помощью [внедрения зависимостей ASP.NET Core](/aspnet/core/fundamentals/dependency-injection/). Дополнительные сведения см. в статье [ApplicationInsights для приложений ASP.NET Core](../../azure-monitor/app/asp-net-core.md).

## <a name="disable-selected-standard-collectors"></a>Отключение выбранных стандартных сборщиков
Можно отключить стандартные сборщики (например, счетчики производительности, HTTP-запросы или зависимости)

* **Приложения ASP.NET**. Удалите или закомментируйте соответствующие строки в файле [ApplicationInsights. config](../../azure-monitor/app/configuration-with-applicationinsights-config.md)
* **Приложения ASP.NET Core**. Используйте параметры конфигурации модулей телеметрии в [ApplicationInsights ASP.NET Core](../../azure-monitor/app/asp-net-core.md#configuring-or-removing-default-telemetrymodules)
