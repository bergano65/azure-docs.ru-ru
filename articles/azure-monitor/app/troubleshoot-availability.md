...
|Симптом/сообщение об ошибке| Возможные причины|
|----|---------|
после нарушения протокола CR должен следовать перевод строки | Это происходит при обнаружении неправильно сформированных заголовков. В частности, некоторые заголовки могут не использовать CRLF для обозначения конца строки, что нарушает спецификацию HTTP и поэтому не проходит проверку на уровне WebRequest .NET.
 || Это также может быть вызвано подсистемами балансировки нагрузки или CDN.
