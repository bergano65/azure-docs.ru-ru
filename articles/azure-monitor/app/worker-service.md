...
> [!NOTE]
> Эта статья посвящена новому пакету из пакета SDK для Application Insights для служб Worker. Этот пакет доступен в качестве бета-версии уже сегодня. Этот документ будет обновлен, когда будет доступен стабильный пакет.
...
[Евенткаунтер](https://github.com/dotnet/corefx/blob/master/src/System.Diagnostics.Tracing/documentation/EventCounterTutorial.md)— это кросс-платформенный метод для публикации и использования счетчиков в .NET и .NET Core. Хотя эта функция существовала раньше, встроенные поставщики, которые опубликовали эти счетчики, отсутствовали. Начиная с .NET Core 3,0, несколько счетчиков публикуются из окна, например счетчики CLR, ЦП и т. д.

По умолчанию пакет SDK собирает следующие счетчики (доступные только в .NET Core 3,0 или более поздней версии), и эти счетчики можно запрашивать либо в обозреватель метрик, либо с помощью запроса аналитики, предназначенного для таблицы PerformanceCounter. Имена счетчиков будут иметь вид "Category | Счетчик ".
