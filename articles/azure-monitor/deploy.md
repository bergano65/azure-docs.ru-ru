...
Необходимо создать ресурс в Application Insights для каждого приложения, которое вы будете отслеживать. Данные журнала, собранные Application Insights, хранятся в журналах Azure Monitor, но отделены от рабочей области Log Analytics, как описано в разделе [как данные в журналах Azure Monitor структурированы?](platform/data-platform-logs.md#how-is-data-in-azure-monitor-logs-structured). В настоящее время предварительная версия — это возможность хранить данные приложения непосредственно в Log Analytics рабочей области с другими данными. Это упрощает настройку и позволяет приложению воспользоваться всеми возможностями Log Analytics рабочей области.

 При создании приложения необходимо выбрать, следует ли использовать классическую версию или на основе рабочей области (Предварительная версия). См. раздел [создание Application Insights ресурса](app/create-new-resource.md) для создания классического приложения. Чтобы создать приложение на основе рабочей области, см. раздел [ресурсы Application Insights на основе рабочих областей (Предварительная версия)](app/create-workspace-resource.md) .
