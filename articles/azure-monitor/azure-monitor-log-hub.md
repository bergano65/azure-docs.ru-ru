---
layout: HubPage
hide_bc: false
title: Документация по данным журнала Azure Monitor | Документация Майкрософт
description: Azure Monitor может собирать данные различных источников в виде данных журнала и хранить их для корреляции и анализа, используя свой язык запросов.
services: azure-monitor
author: mgoedtel
manager: carmonm
ms.assetid: ''
ms.service: azure-monitor
ms.tgt_pltfrm: na
ms.devlang: na
ms.topic: hub-page
ms.date: 01/29/2019
ms.author: magoedte
ms.openlocfilehash: a48e252d1dce2d9142249642a6221e8913c1d92d
ms.sourcegitcommit: 95822822bfe8da01ffb061fe229fbcc3ef7c2c19
ms.translationtype: HT
ms.contentlocale: ru-RU
ms.lasthandoff: 01/29/2019
ms.locfileid: "55223431"
---
<div id="main" class="v2">
    <div class="container">
        <h1>Документация по данным журнала Azure Monitor</h1>
        <p>Azure Monitor может собирать данные различных источников в виде данных журнала и хранить их для корреляции и анализа, используя свой язык запросов.</p>
        <p>Изучите наше самое популярное содержимое о том, как планировать, собирать и анализировать данные журналов.</p>
        <hr style="margin: 30px 0;" />
        <ul class="pivots">
            <li>
                <a href="#products"></a>
                <ul id="products">
                    <li>
                        <a class="singlePanelNavItem selected" style="display: none" href="#indexA" data-linktype="self-bookmark"></a>
                        <ul class="panelContent singlePanelContent" id="indexA" style="border: medium; border-image: none; margin-top: 0px; display: flex; float: left;">
                            <li class="fullSpan">
                                <a href="#index1" data-linktype="self-bookmark"></a>
                                <ul class="cardsF cols cols4" id="index1" style="float: left; display: flex; width: 100%; border-bottom: 1px var(--grey-lighter) solid;">  
                                    <li>
                                        <ul class="cardsB panelContent" id="cardtypes-B" style="float: left; display: flex; width: 100%;">
                                            <li>
                                                <!-- <a href="">-->
                                                    <div class="cardSize">
                                                        <div class="cardPadding">
                                                            <div class="card">
                                                                <div class="cardImageOuter">
                                                                    <div class="cardImage">
                                                                        <img alt="" src="https://docs.microsoft.com/media/common/i_learn-about.svg" data-linktype="external">
                                                                    </div>
                                                                </div>
                                                                <div class="cardText" style="padding-left: 0px">
                                                                    <h3>Изучение основ</h3> 
                                                                    <p> 
                                                                        <a href="/azure/azure-monitor/overview">Общие сведения об Azure Monitor</a><br/>
                                                                        <a href="/azure/azure-monitor/azure-monitor-rebrand">Изменения фирменной символики Azure Monitor</a><br/>
                                                                        <a href="/azure/azure-monitor/platform/data-sources">Источники данных в Azure Monitor</a><br/>
                                                                        <a href="/azure/azure-monitor/platform/service-providers">Log Analytics для поставщиков служб</a><br/>
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                <!-- </a>-->
                                            </li>
                                            <li>
                                                <!-- <a href="">-->
                                                    <div class="cardSize">
                                                        <div class="cardPadding">
                                                            <div class="card">
                                                                <div class="cardImageOuter">
                                                                    <div class="cardImage">
                                                                        <img alt="" src="https://docs.microsoft.com/media/common/i_security-management.svg" data-linktype="external">
                                                                    </div>
                                                                </div>
                                                                <div class="cardText" style="padding-left: 0px">
                                                                    <h3>Общие сведения о безопасности</h3> 
                                                                    <p>
                                                                        <a href="/azure/azure-monitor/platform/data-security">Данные журнала</a><br/>
                                                                        <a href="/azure/azure-monitor/platform/personal-data-mgmt">Обработка персональных данных</a><br/>
                                                                        <a href="/azure/azure-monitor/platform/roles-permissions-security">Приступая к работе с ролями, разрешениями и системой безопасности с помощью Azure Monitor</a><br/>
                                                                    </p>
                                                                    <br>
                                                                     <h3>Управление рабочей областью</h3>
                                                                    <p>
                                                                    <a href="/azure/azure-monitor/learn/quick-create-workspace">Создание рабочей области Log Analytics на портале Azure</a><br/>
                                                                    <a href="/azure/azure-monitor/learn/quick-create-workspace-cli">Создание рабочей области Log Analytics с помощью Azure CLI 2.0</a><br/>
                                                                   <a href="/azure/azure-monitor/learn/quick-create-workspace-posh">Создание рабочей области Log Analytics с помощью Azure PowerShell</a><br/>
                                                                  <a href="/azure/azure-monitor/platform/delete-workspace">Удаление рабочей области Azure Log Analytics на портале Azure</a><br/>
                                                                 </p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                <!-- </a>-->
                                            </li>
                                        </ul>
                                    </li>
                                    <li>
                                        <div class="cardSize">
                                            <div class="cardPadding">
                                                <div class="card">
                                                    <div class="cardText">
                                                    <h3>Мониторинг</h3>
                                                        <p>
                                                            <a href="/azure/azure-monitor/platform/data-collection">Данные мониторинга, собираемые службой Azure Monitor</a><br/>
                                                            <a href="/azure/azure-monitor/platform/data-ingestion-time">Время приема данных в Log Analytics</a><br/>
                                                        </p>
                                                        <br>
                                                        <h3>Источники данных</h3>
                                                        <p>
                                                            <a href="/azure/azure-monitor/platform/data-sources">Источники данных в Azure Monitor<br/>
                                                            <a href="/azure/azure-monitor/platform/data-sources-windows-events">События Windows</a><br/>
                                                            <a href="/azure/azure-monitor/platform/data-sources-performance-counters">Источники данных о производительности Windows и Linux в Log Analytics</a><br/>
                                                            <a href="/azure/azure-monitor/platform/data-sources-linux-applications">Производительность приложений Linux</a><br/>
                                                            <a href="/azure/azure-monitor/platform/data-sources-json">Настраиваемые данные JSON</a><br/>
                                                            <a href="/azure/azure-monitor/platform/data-sources-collectd">Сбор данных о производительности</a><br/>
                                                            <a href="/azure/azure-monitor/platform/data-sources-alerts-nagios-zabbix">Оповещения Nagios и Zabbix</a><br/>
                                                            <a href="/azure/azure-monitor/platform/data-sources-syslog">Системный журнал</a><br/>
                                                            <a href="/azure/azure-monitor/platform/data-sources-iis-logs">Журналы IIS</a><br/>
                                                            <a href="/azure/azure-monitor/platform/data-sources-custom-logs">Пользовательские журналы</a><br/>
                                                            <a href="/azure/azure-monitor/platform/custom-fields">Настраиваемые поля</a><br/>
                                                            <a href="/azure/azure-monitor/platform/data-collector-api">Отправка данных в Log Analytics c помощью API сборщика данных HTTP (общедоступная предварительная версия)</a><br/>
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="cardSize">
                                            <div class="cardPadding">
                                                <div class="card">
                                                    <div class="cardImageOuter">
                                                                    <div class="cardImage">
                                                                        <img alt="" src="https://docs.microsoft.com/media/common/i_search.svg" data-linktype="external">
                                                                    </div>
                                                                </div>
                                                    <div class="cardText">
                                                        <h3>Анализ данных</h3>
                                                        <p>
                                                            <a href="/azure/azure-monitor/log-query/get-started-queries">Начало работы с запросами в Log Analytics</a><br/>
                                                            <a href="/azure/azure-monitor/log-query/search-queries">Поисковые запросы в Log Analytics</a><br/>
                                                            <a href="/azure/azure-monitor/log-query/portals">Просмотр и анализ данных в Log Analytics</a><br/>
                                                            <a href="/azure/azure-monitor/log-query/query-language">Язык запросов Log Analytics</a><br/>
                                                            <a href="/azure/azure-monitor/log-query/cross-workspace-query">Выполнение поиска по журналам нескольких ресурсов в Log Analytics</a><br/>
                                                        </p>
                                                        <br>
                                                        <h3>реагирование на инциденты.</h3>
                                                        <p>
                                                            <a href="/azure/azure-monitor/platform/alerts-overview">Обзор оповещений в Microsoft Azure</a><br/>
                                                            <a href="/azure/azure-monitor/platform/autoscale-overview">Autoscale</a><br/>
                                                            <a href="/azure/azure-monitor/platform/alerts-unified-log">Оповещения журнала в Azure Monitor</a><br/>
                                                            <a href="/azure/azure-monitor/platform/alerts-activity-log">Создание, просмотр и управление оповещениями журнала действий с помощью Azure Monitor</a><br/>
                                                            <a href="/azure/azure-monitor/platform/action-groups">Создание групп действий и управление ими на портале Azure</a><br/>
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </li>
                                </ul>
                            </li>
                        </ul>
                    </li>
                </ul>
            </li>
        </ul>
    </div>
</div>
