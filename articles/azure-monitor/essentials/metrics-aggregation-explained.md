...
Например, [Microsoft. ApiManagement/Service](../platform/metrics-supported.md#microsoftapimanagementservice) имеет *Расположение* в виде измерения для многих метрик. 
...
Дополнительные сведения о каждой метрике и доступных измерениях см. в статье [Поддерживаемые метрики](../platform/metrics-supported.md) Azure Monitor. Кроме того, документация для каждого поставщика ресурсов и типа может предоставить дополнительные сведения об измерениях и их измерении.
