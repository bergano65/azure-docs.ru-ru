---
title: Получение журналов действий Azure в рабочую область Log Analytics в разных клиентах Azure | Документация Майкрософт
description: Использование концентраторов событий и Logic Apps для сбора данных из журнала действий Azure и отправить его в рабочую область Log Analytics в Azure Monitor в другом клиенте.
...
---
# <a name="collect-azure-activity-logs-into-azure-monitor-across-azure-active-directory-tenants"></a>Получение журналов действий Azure в Azure Monitor в разных клиентах Azure Active Directory

Этой статье рассматривается метод сбора журналов действий Azure в рабочую область Log Analytics в Azure Monitor с помощью соединителя сборщик данных Azure Log Analytics для Logic Apps. Следуйте приведенной здесь процедуре, когда вам нужно отправить журналы в рабочее пространство другого арендатора Azure Active Directory. Например, если вы являетесь поставщиком управляемых служб, вы можете собирать журналы действий из подписки клиента и хранить их в рабочей области Log Analytics в своей собственной подписке.

Если в рабочей области Log Analytics находится в той же подписке Azure или в другой подписке, но в том же Azure Active Directory, следуйте инструкциям в [сбор и анализ журналов действий Azure в рабочую область Log Analytics в Azure Monitor](activity-log-collect.md)действия Azure сбор журналов.
...
![изображение потока данных из журнала действий в рабочую область Log Analytics](media/collect-activity-logs-subscriptions/data-flow-overview.png)

Преимущества этого подхода:
- Низкая задержка, так как журнал действий Azure передается потоком в концентратор событий.  Приложение логики, затем запускается и отправляет данные в рабочей области. 
...
3. Создайте приложение логики для чтения из концентратора событий и отправки событий в рабочей области Log Analytics.
...
- Рабочая область Log Analytics может находиться в другой подписке и клиенте Azure Active Directory, нежели приложение логики, но для простоты мы рекомендуем, чтобы они принадлежали к одной подписке. Приложение логики отправляет в рабочую область, используя идентификатор рабочей области Log Analytics и ключ.
...
Когда журналы действий записываются в концентратор событий, вы создадите приложение логики для сбора журналов из концентратора событий и записи их в рабочую область Log Analytics.
...
- Объект [Log Analytics соединитель отправки данных](https://docs.microsoft.com/connectors/azureloganalyticsdatacollector/) для публикации данных в рабочей области Log Analytics.
...
2. Введите параметры в таблице, приведенной ниже.
   ...
   | Log Analytics  | Выберите, если вы хотите регистрировать состояние каждого запуска приложения логики в рабочей области Log Analytics.  |
...
Выходные данные в концентратора событий содержат полезные данные JSON с массивом записей. [Анализ JSON](../../logic-apps/logic-apps-content-type.md) действие используется для извлечения только массива записей для отправки в рабочей области Log Analytics.
...
4. После создания подключения измените параметры в таблице ниже. 
   ...
   | Имя пользовательского журнала | AzureActivity | Имя настраиваемой таблицы журналов для создания в рабочей области Log Analytics для хранения импортированных данных. |
...
>[!NOTE]
> При первом новый настраиваемый журнал отправляется в рабочую область Log Analytics он может занять до одного часа для настраиваемого журнала для включения в поиск.
...
В этой статье вы создали приложение логики для чтения журналов действий Azure из концентратора событий и отправки их в рабочую область Log Analytics для анализа. Дополнительные сведения о визуализации данных в рабочей области, включая создание панелей мониторинга, просмотрите руководство по визуализации данных.
