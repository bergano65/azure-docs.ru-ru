---
title: Журналы действий Azure для разных клиентов в Azure Monitor
...
![изображение потока данных из журнала действий в Log Analytics рабочую область](media/collect-activity-logs-subscriptions/data-flow-overview.png)

Преимущества этого подхода:
- Низкая задержка, так как журнал действий Azure передается потоком в концентратор событий.  Затем приложение логики активируется и отправляет данные в рабочую область. 
...
- Рабочая область Log Analytics может находиться в другой подписке и клиенте Azure Active Directory, нежели приложение логики, но для простоты мы рекомендуем, чтобы они принадлежали к одной подписке. Приложение логики отправляет в рабочую область, используя идентификатор и ключ рабочей области Log Analytics.
...
1. В портал Azure выберите **создать ресурс**  >  **"Интернет вещей"**  >  **концентраторы событий**.
...
1. В портал Azure выберите **мониторинг**  >  **Журнал действий**.
...
- [Соединитель отправки данных log Analytics](/connectors/azureloganalyticsdatacollector/) для публикации данных в рабочую область log Analytics.
   ...
   ![Снимок экрана конструктора приложений логики, в котором показаны шаги для получения журналов действий из концентратора событий и их записи в рабочую область Log Analytics.](media/collect-activity-logs-subscriptions/log-analytics-logic-apps-activity-log-overview.png)
...
1. В портал Azure выберите **создать ресурс**  >  **Корпоративная интеграция**  >  **приложение логики**.
...
1. В конструкторе приложений логики введите в поле поиска текст фильтра *Центры событий*. Выберите триггер Центров событий с именем **При наличии событий в концентраторе событий**.
   ...
   ![Снимок экрана конструктора приложений логики с триггером "концентраторы событий — когда события доступны в концентраторе событий", выбранном для службы концентраторов событий.](media/collect-activity-logs-subscriptions/logic-apps-event-hub-add-trigger.png)
...
Выходные данные в концентратора событий содержат полезные данные JSON с массивом записей. Действие [анализ JSON](../../logic-apps/logic-apps-content-type.md) используется для извлечения только массива записей для отправки в log Analytics рабочую область.
...
В конструкторе Logic Apps щелкните **Запуск**, чтобы протестировать приложение. На каждом шаге в приложении логики отображается значок состояния с белой галочкой в ​​зеленом круге, что указывает на успешное выполнение.
   ...
   ![Снимок экрана конструктора приложений логики после выполнения теста. На каждом шаге приложения логики устанавливается галочка, указывающая на успешное выполнение.](media/collect-activity-logs-subscriptions/test-logic-app.png)
...
![Снимок экрана поиска AzureActivity_CL на панели поиска по журналам, в котором отображается таблица результатов с одним результатом, отображаемым для отображения сведений о действиях.](media/collect-activity-logs-subscriptions/log-analytics-results.png)
