---
title: Кросс-арендаторные журналы активности Azure в Azure Monitor
description: Используйте концентраторы событий и логические приложения для сбора данных из журнала активности Azure и отправки их в рабочее пространство analytics log в Azure Monitor в другом арендаторе.
ms.subservice: logs
ms.topic: conceptual
author: bwren
ms.author: bwren
...
---
# <a name="collect-azure-activity-logs-into-azure-monitor-across-azure-active-directory-tenants-legacy"></a>Сбор журналов деятельности Azure в Azure Monitor через активных каталогов Azure (наследие)

> [!NOTE]
> В этой статье описывается устаревший метод настройки журнала деятельности Azure в разложении клиентов Azure для сбора в рабочей области analytics журнала.  Теперь можно собирать журнал activity в рабочее пространство Log Analytics с помощью диагностической настройки, аналогичной тому, как вы собираете журналы ресурсов. Смотрите [журналы активности Azure и анализируйте в рабочей области журналов log Analytics в Azure Monitor.](activity-log-collect.md)


В этой статье используется метод сбора журналов активности Azure в рабочее пространство для анализа журналов в Azure Monitor с помощью разъема для сбора данных Azure Log Analytics collector для логических приложений. Следуйте приведенной здесь процедуре, когда вам нужно отправить журналы в рабочее пространство другого арендатора Azure Active Directory. Например, если вы являетесь поставщиком управляемых служб, вы можете собирать журналы действий из подписки клиента и хранить их в рабочей области Log Analytics в своей собственной подписке.

Если рабочее пространство журнала Analytics находится в той же подписке Azure или в другой подписке, но в том же каталоге Active Azure, используйте шаги в [журналах деятельности «Сбор» и проанализируйте журналы активности Azure в рабочей области журнала Analytics в Azure Monitor](activity-log-collect.md) для сбора журналов деятельности Azure.
...
![изображение потока данных из журнала активности в рабочее пространство Log Analytics](media/collect-activity-logs-subscriptions/data-flow-overview.png)

Преимущества этого подхода:
- Низкая задержка, так как журнал действий Azure передается потоком в концентратор событий.  Затем срабатывает Logic App и публикует данные в рабочее пространство. 
...
3. Создайте logic App для чтения из концентратора событий и отправьте события в рабочее пространство Log Analytics.
...
- Рабочая область Log Analytics может находиться в другой подписке и клиенте Azure Active Directory, нежели приложение логики, но для простоты мы рекомендуем, чтобы они принадлежали к одной подписке. Logic App отправляет в рабочее пространство с помощью идентификатора рабочего пространства log Analytics и ключа.
...
1. На портале Azure выберите **«Создайте ресурс** > Интернет**концентраторов****событий».** > 
...
7. В колонке **Политики общего доступа** щелкните **RootManageSharedAccessKey**.
   ...
   ![изображение политик общего доступа к концентраторам событий](media/collect-activity-logs-subscriptions/create-event-hub7.png)
   
8. Нажмите кнопку копирования, чтобы скопировать строку подключения **RootManageSharedAccessKey** в буфер обмена. 

   ![изображение общего ключа доступа к концентраторам событий](media/collect-activity-logs-subscriptions/create-event-hub8.png)
...
1. На портале Azure выберите журнал **мониторинга** > **активности**.
3. Нажмите кнопку **Экспорт** в верхней части страницы.

   ![изображение Azure Monitor в области навигации](media/collect-activity-logs-subscriptions/activity-log-blade.png)
...
7. Щелкните **Пространство имен служебной шины** и выберите **подписку** с концентратором событий, **пространство имен концентратора событий** и **имя политики концентратора событий**.
    ...
    ![изображение страницы экспорта в концентратор событий](media/collect-activity-logs-subscriptions/export-activity-log2.png)
...
После того, как журналы активности пишут сярют в концентратор событий, вы создаете Logic App для сбора журналов из концентратора событий и записывания их в рабочее пространство Log Analytics.
...
- [Аналитика журнала отправляет разъем данных](https://docs.microsoft.com/connectors/azureloganalyticsdatacollector/) для размещения данных в рабочее пространство Log Analytics.
...
1. На портале Azure выберите **создать ресурс** > **Enterprise Integration** > **Logic App.**

    ![Новое приложение логики в Marketplace](media/collect-activity-logs-subscriptions/marketplace-new-logic-app.png)
...
2. Введите параметры в таблице, приведенной ниже.
   ...
   | Log Analytics  | Выберите, хотите ли вы войти в состояние каждого запуска приложения логики в рабочей области Log Analytics.  |
...
3. После создания подключения измените параметры триггера. Для начала выберите **insights-operational-logs** из раскрывающегося списка **Имя концентратора событий**.
   ...
   ![Диалоговое окно "При наличии событий в концентраторе событий"](media/collect-activity-logs-subscriptions/logic-apps-event-hub-read-events.png)

5. Разверните **Показать дополнительные параметры** и измените **тип содержимого** на *приложение/json*

   ![Диалоговое окно конфигурации концентратора событий](media/collect-activity-logs-subscriptions/logic-apps-event-hub-configuration.png)

### <a name="add-parse-json-action"></a>Добавление действия "Анализ JSON"

Выходные данные в концентратора событий содержат полезные данные JSON с массивом записей. Действие [Parse JSON](../../logic-apps/logic-apps-content-type.md) используется для извлечения только массива записей для отправки в рабочее пространство Log Analytics.

1. Нажмите **Новый шаг** > **Добавить действие**
2. В поле поиска для фильтрации введите *анализ json*. Выберите действие **Операции с данными: анализ JSON**.

   ![Добавление действия анализа json в приложениях для логики](media/collect-activity-logs-subscriptions/logic-apps-add-parse-json-action.png)
...
4. Скопируйте следующую схему и вставьте ее в поле **Схема**:  Эта схема совпадает с результатом действия концентратора событий.  
   ...
   ![Диалоговое окно анализа JSON](media/collect-activity-logs-subscriptions/logic-apps-parse-json-configuration.png)
...
2. Введите *compose* в качестве фильтра и затем выберите действие **Операции с данными: compose**.
    ...
    ![Действие "Добавить compose"](media/collect-activity-logs-subscriptions/logic-apps-add-compose-action.png)
...
Действие [«Коллектор данных analytics Log](https://docs.microsoft.com/connectors/azureloganalyticsdatacollector/) Analytics» выводит объект из действия «Сочиние» и отправляет его в рабочее пространство анализа журналов.
...
4. После создания подключения измените параметры в таблице ниже. 
   ...
   | Имя пользовательского журнала | AzureActivity | Название пользовательской таблицы журнала для создания в рабочей области Log Analytics для хранения импортируемых данных. |
   | time-generated-field | time | Не выбирайте поле **время** JSON — просто введите слово time. Если вы выберете поле JSON, конструктор поместит действие **отправить данные** в цикл *For Each*, что не требуется. |
...
В конструкторе Logic Apps щелкните **Запуск**, чтобы протестировать приложение. На каждом шаге в приложении логики отображается значок состояния с белой галочкой в ​​зеленом круге, что указывает на успешное выполнение.
   ...
   ![Тестирование приложения логики](media/collect-activity-logs-subscriptions/test-logic-app.png)
...
Теперь следует проверить рабочую область Log Analytics, чтобы убедиться, что сбор данных выполняется, как ожидалось.
...
>[!NOTE]
> Первый раз, когда новый пользовательский журнал отправляется в рабочее пространство Log Analytics, может потребоваться до часа для поиска пользовательского журнала.
...
В этой статье вы создали логическое приложение для чтения журналов активности Azure из концентратора событий и отправки их в рабочее пространство analytics журнала для анализа. Чтобы узнать больше о визуализации данных в рабочей области, включая создание панелей мониторинга, просмотрите учебник для визуализации данных.
