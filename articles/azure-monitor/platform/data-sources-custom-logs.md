...
>[!IMPORTANT]
>Пользовательский сбор журналов требует, чтобы приложение, пишущая файл журнала, периодически произворавало содержимое журнала к диску. Это связано с тем, что пользовательская коллекция журналов опирается на уведомления об изменении файловой системы для отслеживаемого файла журнала.
...
2. Нажмите на **пользовательские** > **журналы данных**.
...
1. Введите имя для журнала.  Суффикс ** \_CL** предоставляется автоматически.
...
Файлы журналов будут храниться в каталоге *C:\MyApp\Logs*.  Каждый день создается файл с именем, которое включает в себя дату, в формате *приложение_ГГГГММДД.log*.  Достаточным шаблоном для этого журнала было бы *C: «MyApp»Logs\\\*.logs*.
