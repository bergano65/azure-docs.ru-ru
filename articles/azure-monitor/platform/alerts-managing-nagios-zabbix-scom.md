---
title: Управление оповещениями от менеджера по операциям Системного центра, Заббикса и Нагиоса в Azure Monitor
description: Управление оповещениями от менеджера по операциям Системного центра, Заббикса и Нагиоса в Azure Monitor
ms.topic: conceptual
ms.date: 09/24/2018
...
---
# <a name="manage-alerts-from-system-center-operations-manager-zabbix-and-nagios-in-azure-monitor"></a>Управление оповещениями от менеджера по операциям Системного центра, Заббикса и Нагиоса в Azure Monitor
...
1. Для предупреждений **Nagios** и **zabbix** [настройте эти серверы](https://docs.microsoft.com/azure/log-analytics/log-analytics-linux-agents) для [отправки оповещений](https://docs.microsoft.com/azure/azure-monitor/platform/data-sources-alerts-nagios-zabbix?toc=%2Fazure%2Fazure-monitor%2Ftoc.json) в журнал Analytics.
1. Для оповещений менеджера **по операциям System Center** [подключите группу управления менеджером операций к рабочему пространству Log Analytics.](https://docs.microsoft.com/azure/log-analytics/log-analytics-om-agents) После этого разверните [решение "Управление оповещениями" в Azure Log Analytics](https://docs.microsoft.com/azure/azure-monitor/platform/alert-management-solution) из решений Azure Marketplace. После этого все оповещения, созданные в System Center Operations Manager, импортируются в Log Analytics.
...
> [!NOTE]
>  1. Это решение позволяет просматривать только операционные центры Системного центра/Заббикс/Наджос, выпущенные экземплярами оповещения в Azure Monitor. Вы можете просмотреть или изменить конфигурацию правила генерации оповещений только в соответствующих средствах мониторинга. 
...
>  1. Все оповещения от менеджера по операциям Системного центра, Заббикса и Нагиоса имеют тип сигнала «Неизвестный», так как базовый тип телеметрии недоступен.
