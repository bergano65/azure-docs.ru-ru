...
| `Color palette` | Цветовая палитра, используемая в диаграмме. Игнорируется, `Split by` если используется параметр | Синий, зеленый, красный и т. д. |
...
8. В **поле плитки**задайте:
    * Заголовок: `name`
    * Left: `Requests`, модуль подготовки отчетов `Big Number`:, цветовая `Green to Red`палитра:, минимальное значение:`0`
    * Нижний`appName`
