...
Рекомендации:
1. SKU концентратора событий "базовый" поддерживает более низкий [предел](https://docs.microsoft.com/azure/event-hubs/event-hubs-quotas#basic-vs-standard-tiers) размера событий, и некоторые журналы в рабочей области могут превыситься и быть удалены. В качестве назначения экспорта рекомендуется использовать концентратор событий "Стандартный" или "выделенный".
