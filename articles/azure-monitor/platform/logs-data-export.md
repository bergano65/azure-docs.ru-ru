...
- Поддерживаемые таблицы в настоящее время ограничиваются указанными в разделе (#supported-табес) ниже. Если правило экспорта данных включает неподдерживаемую таблицу, операция будет выполнена, но данные для этой таблицы не будут экспортированы. Если правило экспорта данных включает таблицу, которая не существует, она завершится ошибкой, но *Таблица ошибок <tableName> не будет существовать в рабочей области.*
...
Если правило экспорта данных включает таблицу, которая не существует, она завершится ошибкой, но *Таблица ошибок <tableName> не будет существовать в рабочей области*.
...
| Предупреждение | Некоторые данные из этой таблицы принимаются через учетную запись хранения. Эта часть в настоящее время отсутствует в экспорте. |
...
| CommonSecurityLog | |
| ComputerGroup | |
| ConfigurationData | Некоторые данные принимаются через внутренние службы, которые не поддерживаются для экспорта. Эта часть в настоящее время отсутствует в экспорте. |
...
| днсинвентори | |
| Dynamics365Activity | |
| Событие | Некоторые данные из этой таблицы принимаются через учетную запись хранения. Эта часть в настоящее время отсутствует в экспорте. |
| ексчанжеассессментрекоммендатион | |
| ексчанжеассессментрекоммендатион | |
| фаилединжестион | |
| функтионапплогс | |
| Пульс | Поддерживаются: | |
| хунтингбукмарк | |
| InsightsMetrics | Некоторые данные принимаются через внутренние службы, которые не поддерживаются для экспорта. Эта часть в настоящее время отсутствует в экспорте. |
| интунеаудитлогс | |
...
| OfficeActivity | Некоторые данные для приема через веб-перехватчики из Office 365 в LA. Эта часть в настоящее время отсутствует в экспорте. |
| Операция | Некоторые данные принимаются через внутренние службы, которые не поддерживаются для экспорта. Эта часть в настоящее время отсутствует в экспорте. |
| Perf | Поддерживаются: | |
...
| сукцеедединжестион | |
| Системный журнал |Partial | Некоторые данные из этой таблицы принимаются через учетную запись хранения. Эта часть в настоящее время отсутствует в экспорте. |
| ThreatIntelligenceIndicator | |
| Update |Partial | Некоторые данные принимаются через внутренние службы, которые не поддерживаются для экспорта. Эта часть в настоящее время отсутствует в экспорте. |
| UpdateRunProgress | |
| UpdateSummary | |
| Использование | |
| усеракцессаналитикс | |
...
| виндовсевент | |
| WindowsFirewall | |
| Данные передачи |Partial | Некоторые данные принимаются через внутренние службы, которые не поддерживаются для экспорта. Эта часть в настоящее время отсутствует в экспорте. |
