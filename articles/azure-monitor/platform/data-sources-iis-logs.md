...
Azure Monitor собирает записи журналов IIS от каждого агента каждый раз, когда изменяется метка времени журнала. Бревно читается каждые **5 минут.** Если по какой-либо причине IIS не обновляет метку времени до момента создания нового файла, записи будут собраны после создания нового файла. Частота создания нового файла контролируется настройками **Расписания rollover Logover** Для сайта IIS, который один раз в день по умолчанию. Если параметр **почасовой,** Azure Monitor собирает журнал каждый час. Если параметр **ежедневный,** Azure Monitor собирает журнал каждые 24 часа.
...
| RemoteIPCountry |Страна/регион IP-адреса клиента. |
...
| W3CIISLog &#124; где csHost»"www\.contoso.com" &#124; суммировать кол() csuriStem |Подсчет записей журнала IIS по URL\.для хостов, contoso.com. |
