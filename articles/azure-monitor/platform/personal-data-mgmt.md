...
* *Пользовательские данные*. Log Analytics позволяет собирать данные различным образом: пользовательские журналы и настраиваемые поля, [API-интерфейс сборщика данных HTTP](../../azure-monitor/platform/data-collector-api.md) и пользовательские данные, собранные как часть журналов системных событий. Все они восприимчивы к содержанию частных данных и должны быть просмотрены, чтобы проверить, существуют ли такие данные.
...
* *IP-адреса*. По умолчанию Application Insights маскирует все значения в полях IP-адресов, заменяя их строкой 0.0.0.0, но для сохранения сведений о сеансах в них часто сохраняются реальные значения клиентских IP-адресов. Приведенный ниже запрос Analytics позволяет найти все таблицы, в которых содержатся поля IP-адресов со значениями, отличными от 0.0.0.0, за последние 24 часа:
    ...
    ```
* *Идентификаторы пользователей*. По умолчанию Application Insights создает случайные идентификаторы для отслеживания пользователей и сеансов. Но эти поля в реальных приложениях часто переопределяются так, чтобы сохранять реальные идентификаторы, имеющие смысл для приложения. Например: имена пользователей, GUID AAD и т.д. Эти идентионные данные часто считаются в области персональных данных, и поэтому должны быть обработаны надлежащим образом. Мы рекомендуем всегда маскировать или анонимизировать эти идентификаторы. Чаще всего такие значения сохраняются в полях с такими именами, как session_Id, user_Id, user_AuthenticatedId, user_AccountId или даже customDimensions.
...
> [!IMPORTANT]
>  В то время как подавляющее большинство операций по очистке может завершиться гораздо быстрее, чем SLA, **официальное SLA для завершения операций по очистке установлен о 30 дней** из-за их сильного воздействия на используемую платформу данных. Это автоматизированный процесс; нет никакого способа запросить более быструю обработку операции.
...
> [!IMPORTANT]
> Для управления системными ресурсами запросы на очистку замедляются по 50 запросам в час. Вы должны пакет исполнения запросов очистки, отправив одну команду, предикат которой включает в себя все идентификаторы пользователей, которые требуют очистки. Используйте [оператора](/azure/kusto/query/inoperator) для указания нескольких идентификационных данных. Необходимо выполнить запрос перед выполнением запроса на очистку, чтобы убедиться, что результаты ожидаются. 
...
> [!IMPORTANT]
>  Хотя мы ожидаем, что подавляющее большинство операций по очистке завершить гораздо быстрее, чем наши SLA, из-за их тяжелого воздействия на платформу данных, используемую Log Analytics, **формальный SLA для завершения операций по очистке установлен на 30 дней**. 
...
> [!IMPORTANT]
>  В то время как подавляющее большинство операций по очистке может завершиться гораздо быстрее, чем SLA, из-за их сильного влияния на платформу данных, используемую Application Insights, **официальное SLA для завершения операций по очистке установлен о 30 дней.**
