...
Тело ответа от операции содержит информацию об операции, а свойство *статуса* указывает ее состояние. Асинхронные операции в этой конфигурации процедуры и их статусы:

**Создание *кластерного* ресурса**
* ПодготовкаСчета - Кластер ADX находится в подготовке 
* Успешно - Подготовка кластера ADX завершена

**Предоставление разрешений на ваше убежище ключа**
* Обновление - Обновление деталей идентификатора ключевых данных продолжается
* Успешно - Обновление завершено

**Связывание рабочих областей анализа журналов**
* Связь - Ассоциация рабочего пространства с кластером продолжается
* Успешно - Ассоциация завершена
...
200 OK и заголовок, когда принято.
>[!Important]
...
> 1. Копирование значения *URL-адреса Azure-AsyncOperation* из ответа и использование его для проверки состояния операции в [асинхронных операциях](#asynchronous-operations-and-status-check)
...
> [!IMPORTANT]
> Копировать и сохранять ответ, так как вам понадобится его подробная информация в последующих шагах
...
200 OK и заголовок, когда принято.
>[!Important]
> Распространение идентификатора Key занимает несколько минут. Вы можете проверить состояние подготовки двумя способами:
> 1. Копирование значения *URL-адреса Azure-AsyncOperation* из ответа и использование его для проверки состояния операции в [асинхронных операциях](#asynchronous-operations-and-status-check)
...
200 OK и заголовок, когда принято.
>[!Important]
> Он может работать до 90 минут. Данные, попаданные в рабочие области, хранятся в зашифрованном виде с помощью управляемого ключа только после успешной ассоциации рабочих пространств.
> Чтобы проверить состояние ассоциации рабочего пространства, скопируйте значение *URL-адреса Azure-AsyncOperation* из ответа и используйте его для проверки состояния операции в [асинхронных операциях](# asynchronous-operations-and-status-check)

Вы можете проверить ресурс *кластера,* связанный с рабочим пространством, отправив запрос GET в [рабочие области - Получить](https://docs.microsoft.com/rest/api/loganalytics/workspaces/get) и наблюдать ответ. *КластерResourceId* указывает на идентификатор ресурса *кластера.*
...
- *Ассоциация* кластерных ресурсов в рабочее пространство должна быть перенесена только после того, как вы подтвердили, что подготовка кластера ADX была выполнена. Данные, отправляемые до этого подготовки, будут удалены и не будут восстановлены.
...
- Удалите ресурс *кластера* - выполняется операция "мягкого удаления", позволяющая восстановить ресурс кластера, данные и связанные с ними рабочие области в течение 14 дней, независимо от того, было ли удаление случайным или преднамеренным. Имя ресурса *кластера* остается зарезервированным в период смягче-удаляемого, и вы не можете создать новый кластер с этим именем. После периода «мягкого *удаления»* ресурс кластера и данные не могут быть восстановлены. Связанные рабочие области де-ассоциируются с ресурсом *кластера,* а новые данные подаются в совместное хранилище и шифруются ключом Майкрософт.
...
>[!Important]
> Подготовка недостаточного кластера ADX занимает несколько минут. Вы можете проверить состояние подготовки при выполнении вызова GET REST API на ресурсе *кластера* и при поиске значения *положения.* Это *ProvisioningAccount* в то время как подготовка и "Успешно" при завершении.
