...
Когда вы [перенесете собственное хранилище](https://docs.microsoft.com/azure/azure-monitor/platform/private-storage) (BYOS) и связываете его с рабочей областью, служба отправляет запросы на сохранение в вашу учетную запись хранения, а также *выполняет операции сохранения* и *оповещения журналов* . Это означает, что вы управляете учетной записью хранения и [политикой шифрования неактивных](https://docs.microsoft.com/azure/storage/common/encryption-customer-managed-keys) данных с помощью того же ключа, который используется для шифрования log Analytics кластера или другого ключа. Тем не менее, вы несете ответственность за затраты, связанные с этой учетной записью хранения. 
