...
>[!Important]
> Для завершения требуется некоторое время подготовки недостаточного кластера ADX. Вы можете проверить состояние подготовки при выполнении вызова GET REST API на ресурсе *кластера* и при поиске значения *положения.* Это *ProvisioningAccount* в то время как подготовка и *успешно,* когда завершено.
...
Этот шаг применяется в исходных и будущих ключевых версиях в вашем Ключе Vault. Он информирует Azure Monitor Storage о ключевой версии, которая будет использоваться для шифрования данных. При обновлении новый ключ используется для обертывания и разворачивания ключа хранения (AEK).
...
Объединение рабочего пространства выполняется с помощью асинхронных операций Resource Manager, которые могут занять до 90 минут. Следующий шаг показывает, как можно проверить состояние ассоциации рабочего пространства. После объединения рабочих пространств данные, попаданные в рабочие области, хранятся в зашифрованном виде с помощью управляемого ключа.

### <a name="workspace-association-verification"></a>Проверка ассоциации рабочего пространства
Вы можете проверить, связано ли рабочее пространство с ресурсом *кластера,* взглянув на [рабочие области - Получить](https://docs.microsoft.com/rest/api/loganalytics/workspaces/get) ответ. Связанные рабочие области будут иметь свойство clusterResourceId с идентификатором ресурса *кластера.*
