...
> [!NOTE]
> Кластеры поддерживают два [управляемых типа удостоверений](../../active-directory/managed-identities-azure-resources/overview.md#managed-identity-types): назначенные системой и назначенные пользователем, и каждый из них может быть основан в зависимости от вашего сценария. Управляемое системой удостоверение является более простым и создается автоматически при создании кластера, когда для Identity `type` задано значение "*SystemAssigned*". это удостоверение можно использовать позже, чтобы предоставить кластеру доступ к Key Vault. Если вы хотите создать кластер, а ключ, управляемый клиентом, определен во время создания кластера, необходимо заранее определить ключ и назначить ему Key Vault удостоверение, а затем создать кластер со следующими параметрами: Identity `type` как "*усерассигнед*", `UserAssignedIdentities` указав идентификатор ресурса удостоверения и `keyVaultProperties` сведения о ключе.

> [!IMPORTANT]
> В настоящее время вы не можете определить ключ, управляемый клиентом, с управляемым удостоверением, назначенным пользователем, если Key Vault находится в Private-Link (vNet), и в этом случае можно использовать управляемое системой удостоверение.
...
- Максимальное число рабочих областей, которые могут быть связаны с кластером: 1000
...
- Azure Key Vault, кластер и рабочие области должны находиться в одном и том же регионе и в одном клиенте Azure Active Directory (Azure AD), но они могут находиться в разных подписках.
