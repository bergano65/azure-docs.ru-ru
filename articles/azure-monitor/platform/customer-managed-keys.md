...
> [! СВЕДЕНИЯ] кластеры поддерживают два [управляемых типа удостоверений](../../active-directory/managed-identities-azure-resources/overview.md#managed-identity-types). Управляемое системой удостоверение с помощью кластера создается при вводе `SystemAssigned` типа удостоверения. его можно использовать позже для предоставления доступа к Key Vault. Если вы хотите создать кластер, настроенный для управляемого клиентом ключа при создании, создайте кластер с назначенным пользователем управляемым удостоверением, который предоставляется в Key Vault--обновите кластер с `UserAssigned` типом удостоверения, идентификатором ресурса удостоверения в `UserAssignedIdentities` и предоставьте сведения о ключах в `keyVaultProperties` .
