...
В течение периода раннего доступа функции кластер ADX подготавливается группой разработчиков вручную после выполнения предыдущих шагов. Используйте канал с корпорацией Майкрософт, чтобы предоставить сведения о ресурсах *кластера* . Ответ JSON можно получить с помощью инструкции GET REST API:
