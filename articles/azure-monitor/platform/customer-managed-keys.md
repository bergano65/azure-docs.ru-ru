...
- [Защита от очистки](../../key-vault/general/overview-soft-delete.md#purge-protection) защищает секрет и данные хранилища от принудительного удаления даже после обратимого удаления.
...
> [!NOTE]
> CMK для запросов, используемых в книгах и на панелях мониторинга Azure, пока не поддерживается. Эти запросы остаются зашифрованными с помощью ключа Microsoft Key.  

Когда вы [перенесете собственное хранилище](./private-storage.md) (BYOS) и связываете его с рабочей областью, служба отправляет запросы на сохранение в вашу учетную запись хранения, а также *выполняет операции сохранения* и *оповещения журналов* . Это означает, что вы управляете учетной записью хранения и [политикой шифрования неактивных](../../storage/common/encryption-customer-managed-keys.md) данных с помощью того же ключа, который используется для шифрования log Analytics кластера или другого ключа. Тем не менее, вы несете ответственность за затраты, связанные с этой учетной записью хранения. 
...
- Azure Key Vault должны быть настроены как восстанавливаемые. Эти свойства не включены по умолчанию и должны быть настроены с помощью интерфейса командной строки или PowerShell:<br>
  - [обратимое удаление](../../key-vault/general/overview-soft-delete.md)
  - Чтобы защититься от принудительного удаления секрета или хранилища даже после обратимого удаления, следует включить [защиту](../../key-vault/general/overview-soft-delete.md#purge-protection) от удаления.
...
- Сообщения об ошибках
  ...
  -  400-ключ не может быть восстановлен. Для Key Vault должно быть задано обратимое удаление и защита от удаления. См. [документацию по Key Vault](../../key-vault/general/overview-soft-delete.md)
