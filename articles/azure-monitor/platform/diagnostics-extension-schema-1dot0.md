...
|Атрибут  |Type   |Обязательное значение| Значение по умолчанию | Описание|  
|-----------|-------|--------|---------|------------|  
|**configurationChangePollInterval**|длительность|Необязательный | PT1M| Указывает интервал, с которым монитор диагностики опрашивает наличие изменений конфигурации диагностики.|  
|**overallQuotaInMB**|unsignedInt|Необязательный| 4000 МБ. Если указать значение, оно не должно превышать эту величину. |Общий объем хранилища файловой системы, выделенный для всех буферов ведения журнала.|  
...
|Атрибут|Тип|Описание|  
|---------------|----------|-----------------|  
|**path**|строка|Обязательный. Абсолютный путь к отслеживаемому каталогу.|  
|**expandEnvironment**|Логический|Обязательный. Если задано значение **true**, то переменные среды в пути раскрываются.|  
...
|Атрибут|Тип|Описание|  
|---------------|----------|-----------------|  
|**name**|строка|Обязательный. Имя локального ресурса, который содержит каталог для отслеживания.|  
|**relativePath**|строка|Обязательный. Путь относительно отслеживаемого локального ресурса.|  
...
|---------------|----------|-----------------|  
|**bufferQuotaInMB**|unsignedInt|Необязательный элемент. Указывает максимальный объем хранилища файловой системы, который доступен для указанных данных.<br /><br /> Значение по умолчанию — 0.|  
|**scheduledTransferPeriod**|duration|Необязательный. Указывает интервал между запланированными передачами данных, округленный с точностью до ближайшей минуты.<br /><br /> По умолчанию используется значение PT0S.|  
...
|Атрибут|Тип|Описание|  
|---------------|----------|-----------------|  
|**counterSpecifier**|строка|Обязательный. Путь к счетчику производительности, данные которого будут собираться.|  
|**sampleRate**|duration|Обязательный. Частота сбора данных счетчика производительности.|  
...
|---------------|----------|-----------------|  
|**bufferQuotaInMB**|unsignedInt|Необязательный элемент. Указывает максимальный объем хранилища файловой системы, который доступен для указанных данных.<br /><br /> Значение по умолчанию — 0.|  
|**scheduledTransferLogLevelFilter**|строка|Необязательный. Указывает минимальный уровень серьезности для передаваемых записей журнала. По умолчанию используется значение **Undefined**. Другие возможные значения: **Verbose**, **Information**, **Warning**, **Error** и **Critical**.|  
|**scheduledTransferPeriod**|duration|Необязательный элемент. Указывает интервал между запланированными передачами данных, округленный с точностью до ближайшей минуты.<br /><br /> По умолчанию используется значение PT0S.|  
...
|Атрибут|Тип|Описание|  
|---------------|----------|-----------------|  
|**name**|строка|Обязательный. Выражение XPath, задающее журнал для сбора.|  
