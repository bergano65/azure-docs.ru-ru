...
Правила генерации оповещений отделены от оповещений и действий, которые создаются, когда срабатывает оповещение. 
...
Кроме того, вы можете [программно перечислить экземпляров предупреждения, созданные на ваши подписки с помощью интерфейсов API REST](#manage-your-alert-instances-programmatically).
...
| столбец | Описание |
|:---|:---|
| Подписка | Выбор подписок Azure, для которых вы хотите просмотреть оповещения. При необходимости вы можете выбрать все подписки. Только предупреждения, что имеется доступ к в выбранных подписках включаются в представление. |
...
На странице сведений о предупреждении состоит из следующих разделов.
...
| Диагностика | Сведения об смарт-группах, в которые включены оповещения. *Счетчик оповещений* отображает количество оповещений, которые включены в смарт-группу. Включает в себя другие предупреждения в той же смарт-группе, которые были созданы за последние 30 дней, независимо от фильтра времени на странице списка оповещений. Чтобы получить дополнительные сведения, щелкните оповещение. |

## <a name="role-based-access-control-rbac-for-your-alert-instances"></a>Элемент управления доступом на основе ролей (RBAC) для оповещения экземпляров

Потребление и управление экземпляры предупреждений об изменении требует от пользователя имеют встроенные роли RBAC либо [участник мониторинга](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-contributor) или [роль monitoring reader](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-reader). Эти роли поддерживаются в любой области диспетчера ресурсов Azure, от уровня подписки на уровне ресурса детальные назначениям. Например если пользователь имеет только доступ «участник мониторинга» для виртуальной машины «ContosoVM1», затем он может использовать и управлять только предупреждения, созданные на «ContosoVM1».

## <a name="manage-your-alert-instances-programmatically"></a>Программно управлять вашей экземпляров предупреждений

Существует множество сценариев, где следует программно запросить для предупреждений, созданных для вашей подписки. Это может быть, для создания пользовательских представлений за пределами портала Azure, или чтобы анализировать предупреждения, чтобы определить закономерности и тенденции.

Вы можете запрашивать предупреждений, созданных для вашей подписки с помощью [REST API оповещений управления](https://aka.ms/alert-management-api) или с помощью [Graph REST API ресурсов Azure для оповещений](https://docs.microsoft.com/rest/api/azureresourcegraph/resources/resources).

[Graph REST API ресурсов Azure для оповещений](https://docs.microsoft.com/rest/api/azureresourcegraph/resources/resources) позволяет запрашивать экземпляры предупреждений об изменении в нужном масштабе. Это рекомендуется для сценариев, где необходимо управлять предупреждения, созданные в нескольких подписках. 

Следующий пример запроса к API возвращает количество оповещений в рамках одной подписки:
...
```
Оповещения можно запрашивать их [«основные»](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-common-schema-definitions#essentials-fields) поля.

[API REST управления предупреждение](https://aka.ms/alert-management-api) может использоваться для получения дополнительных сведений об определенных оповещений, включая их [«контекст предупреждения»](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-common-schema-definitions#alert-context-fields) поля.
