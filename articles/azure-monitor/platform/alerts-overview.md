...
Правила генерации оповещений отделены от предупреждений и действий, выполняемых при срабатывании оповещения. 
...
Кроме того, можно [программно перечислить экземпляры предупреждений, созданные в ваших подписках, с помощью интерфейсов API](#manage-your-alert-instances-programmatically).

> [!NOTE]
   >  Только оповещения, созданные за последние 30 дней, могут быть доступны в интерфейсе UX или через API-интерфейсы RESTFUL.
...
## <a name="manage-alert-rules"></a>Управление правилами оповещения
...
| Resource | Выбор ресурса. В представление включены только те оповещения, целью которых выбраны ресурсы. Этот столбец становится доступным только после указания типа ресурсов. |
...
![Сведения о предупреждении](media/alerts-overview/alert-detail2.png)
...
[Граф ресурсов Azure REST API для оповещений](https://docs.microsoft.com/rest/api/azureresourcegraph/resources/resources) позволяет запрашивать экземпляры предупреждений в масштабе. Это рекомендуется для сценариев, в которых необходимо управлять предупреждениями, созданными для нескольких подписок. 
...
```
Оповещения могут запрашиваться по их [ключевым](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-common-schema-definitions#essentials-fields) полям.

[Управление оповещениями REST API](https://aka.ms/alert-management-api) можно использовать для получения дополнительных сведений о конкретных предупреждениях, включая поля ["контекст предупреждения"](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-common-schema-definitions#alert-context-fields) .
