...
Если агент обменивается данными через прокси-сервер или брандмауэр, возможны ограничения, препятствующие обмену данными с исходного компьютера и службы Azure Monitor. Если обмен данными заблокирован, ошибка конфигурации, регистрация в рабочей области может завершиться ошибкой при попытке установить агент, настроить агент после установки для отправки отчета в дополнительную рабочую область или после успешной регистрации произойдет сбой связи с агентом. В этом разделе описываются методы устранения неполадок, связанных с этим типом, с агентом Windows. 
...
- Выполните следующий запрос, чтобы убедиться, что агент отправляет пульс в рабочую область, для которой настроен отчет. Замените `<ComputerName>` на фактическое имя компьютера.
...
- Фильтрация *Operations Manager* журнала событий по **источникам** - событий*Служба работоспособности модулям*, *HealthService*и *соединителю службы* , а также фильтрация по *предупреждениям* и *ошибкам* на **уровне событий** для подтверждения того, есть ли у него события, записанные в следующей таблице. Если это так, ознакомьтесь с действиями по устранению, которые включены для каждого возможного события.
...
1. Откройте командную строку с повышенными привилегиями на компьютере и перезапустите службу агента `net stop healthservice && net start healthservice`, введя.
