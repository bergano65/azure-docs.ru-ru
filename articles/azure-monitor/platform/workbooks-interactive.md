...
10. Использование редактора запросов для ввода ККЛ для анализа
    ...
    ```
11. `Run query`для просмотра результатов.
12. Изменить _визуализацию_ на`Area chart`
...
7. В разделе _столбцы_ задайте:
    1. _Sample_ Модуль подготовки к просмотру столбцов `Link`:, представление для открытия `Cell Details`:, метка ссылки:`Sample`
...
1. Выполните действия, описанные `Setting up interactivity on grid row click` в разделе, чтобы настроить два интерактивных элемента управления.
