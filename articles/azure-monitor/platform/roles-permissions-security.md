...
Многим командам необходимо строго регулировать доступ к данным и параметрам мониторинга. Например, если у вас есть члены команды, работающие исключительно с мониторингом (инженеры службы поддержки, инженеры DevOps) или если вы используете управляемый поставщик услуг, вам может потребоваться предоставить им доступ только к данным мониторинга, не снижая их возможности создавать, изменять и удалять ресурсы. В этой статье показано, как быстро применить к пользователю в Azure встроенные роли RBAC для мониторинга или создать собственную настраиваемую роль для пользователя, которому нужен ограниченный набор разрешений для мониторинга. Далее в статье рассматриваются вопросы безопасности ресурсов, связанных с Azure Monitor, и способы ограничения доступа к данным, которые они содержат.
...
* Получите схему поиска в Log Analytics рабочей области.
* Вывод списка пакетов мониторинга в Log Analytics рабочей области.
* Получение и выполнение сохраненных поисков в Log Analytics рабочей области.
...
* Включение или отключение пакетов мониторинга в Log Analytics рабочей области.
* Создание и удаление и выполнение сохраненных поисков в Log Analytics рабочей области.
* Создание и удаление конфигурации хранилища рабочей области Log Analytics.

\*пользователю также должно быть предоставлено отдельное разрешение ListKeys на целевой ресурс (учетная запись хранения или пространство имен концентратора событий), чтобы задать профиль журнала или параметр диагностики.
...
2. Журналы ресурсов, которые представляют собой журналы, генерируемые ресурсом.
...
![Диалоговое окно](./media/roles-permissions-security/secured-storage-example.png) "Параметры службы хранилища Azure" Azure Monitor считается одной из следующих "доверенных служб Майкрософт". Если вы разрешите доверенным службам Майкрософт доступ к защищенному хранилищу, Azure Monitor получит доступ к защищенной учетной записи хранения. Включение записи Azure Monitor журналов ресурсов, журнала действий и метрик в учетную запись хранения с этими защищенными условиями. Это также позволит службе Log Analytics осуществлять чтение журналов из защищенного хранилища.   
