...
Многим командам необходимо строго регулировать доступ к данным и параметрам мониторинга. Например, если у вас есть члены группы, которые работают исключительно над мониторингом (инженеры поддержки, инженеры DevOps) или если вы используете управляемого поставщика услуг, вы можете предоставить им доступ только к данным мониторинга, ограничивая их способность создавать, изменять или удалять ресурсы. В этой статье показано, как быстро применить к пользователю в Azure встроенные роли RBAC для мониторинга или создать собственную настраиваемую роль для пользователя, которому нужен ограниченный набор разрешений для мониторинга. Далее в статье рассматриваются вопросы безопасности ресурсов, связанных с Azure Monitor, и способы ограничения доступа к данным, которые они содержат.
...
* Извинайте схему поиска в рабочей области Log Analytics.
* Список пакетов мониторинга в рабочей области Log Analytics.
* Изыгайте и выполняйте сохраненные поиски в рабочей области Log Analytics.
* Изытайте конфигурацию рабочего пространства анализа Log Analytics.
...
* Установите [диагностические настройки](diagnostic-settings.md) ресурса.\*
* Установите [профиль журнала](activity-log-export.md) для подписки.\*
...
* Включить или отключить пакеты мониторинга в рабочей области Log Analytics.
* Создание и удаление и выполнение сохраненных поисков в рабочей области Log Analytics.
* Создайте и удалите конфигурацию рабочего пространства Log Analytics.

\*Пользователю также должно быть предоставлено разрешение ListKeys на целевом ресурсе (учетная запись хранения или пространство имен центра событий) для установки профиля журнала или диагностической настройки.
...
2. журналы ресурсов, которые являются журналами, испускаемыми ресурсом.
...
![Настройки хранения данных](./media/roles-permissions-security/secured-storage-example.png) Azure Dialog Azure Monitor считаются одним из таких «доверенных служб Майкрософт», если вы позволите надежным службам Майкрософт получить доступ к защищенной учетной записи хранилища, монитор Azure получит доступ к защищенной учетной записи хранения; позволяет записывать журналы ресурсов Azure Monitor, журнал активности и метрики в учетную запись хранения в этих защищенных условиях. Это также позволит службе Log Analytics осуществлять чтение журналов из защищенного хранилища.   
