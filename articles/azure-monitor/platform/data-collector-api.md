...
| Log-Type |Укажите тип записи для отправляемых данных. Может содержать только буквы, цифры и подчеркивание (яп. ) и не может превышать 100 символов. |
| x-ms-date |Дата обработки запроса в формате RFC 1123. |
| x-ms-AzureResourceId | Идентификатор ресурса ресурса ресурса Azure, с помощью чего следует ассоциироваться. Это нанимает [свойство _ResourceId](log-standard-properties.md#_resourceid) и позволяет включать данные в запросы [ресурсно-контекстного контекста.](design-logs-deployment.md#access-mode) Если это поле не указано, данные не будут включены в запросы ресурсно-контекстного контекста. |
...
Текст сообщения должен иметь формат JSON. Он должен включать одну или несколько записей с именем свойства и парами значений в следующем формате. Имя свойства может содержать только буквы, цифры и подчеркивание.
...
Следующие свойства зарезервированы и не должны использоваться в пользовательском типе записи. Вы получите ошибку, если ваша полезная нагрузка включает в себя любой из этих имен свойств.
...
```
## <a name="alternatives-and-considerations"></a>Альтернативы и соображения
Хотя API-коллекционирование данных должно охватывать большую часть ваших потребностей в сборе данных свободной формы в журналы Azure, есть случаи, когда может потребоваться альтернатива для преодоления некоторых ограничений API. Все ваши варианты следующим образом, основные соображения включены:

| Альтернатива | Описание | Лучше всего подходит для |
|---|---|---|
| [Пользовательские события](https://docs.microsoft.com/azure/azure-monitor/app/api-custom-events-metrics?toc=%2Fazure%2Fazure-monitor%2Ftoc.json#properties): Родная SDK на основе приема в приложения Исследования | Application Insights, обычно инструментированный через SDK в приложении, предлагает возможность для вас отправлять пользовательские данные через пользовательские события. | <ul><li> Данные, генерируемые в приложении, но не подхваченные SDK через один из типов данных по умолчанию (запросы, зависимости, исключения и так далее).</li><li> Данные, которые чаще всего соотносятся с другими данными приложения в Application Insights </li></ul> |
| API-иНОС для сбора данных в журналах Azure Monitor | API-разработок в журналах Azure Monitor — это полностью открытый способ глотать данные. Любые данные, отформатированные в объекте JSON, могут быть отправлены здесь. После отправки он будет обработан и доступен в журналах, которые будут соотнесены с другими данными в журналах или с другими данными Application Insights. <br/><br/> Загружать данные в файлы в Azure Blob blob довольно просто, откуда эти файлы будут обрабатываться и загружаться в Log Analytics. Пожалуйста, [ознакомьтесь](https://docs.microsoft.com/azure/log-analytics/log-analytics-create-pipeline-datacollector-api) с этой статьей для примера реализации такого конвейера. | <ul><li> Данные, которые не обязательно генерируются в приложении, инструментируемом в Application Insights.</li><li> Примеры включают таблицы поиска и фактов, справочные данные, предварительно агрегированную статистику и так далее. </li><li> Предназначен для данных, которые будут перекрестно сопомнены с другими данными Azure Monitor (Application Insights, другие типы данных журналов, Центр безопасности, Azure Monitor для контейнеров/ВМ и так далее). </li></ul> |
| [Azure Data Explorer](https://docs.microsoft.com/azure/data-explorer/ingest-data-overview) | Azure Data Explorer (ADX) — платформа данных, которая позволяет аналитике Application Insights и журналам мониторинга Azure. Теперь общедоступные ("GA"), используя платформу данных в сыром виде обеспечивает полную гибкость (но требующие накладных расходов на управление) над кластером (RBAC, скорость удержания, схема и так далее). ADX предоставляет множество [вариантов приема,](https://docs.microsoft.com/azure/data-explorer/ingest-data-overview#ingestion-methods) включая [CSV, TSV и JSON](https://docs.microsoft.com/azure/kusto/management/mappings?branch=master) файлы. | <ul><li> Данные, которые не будут соотноситься с другими данными в приложениях Insights или Logs. </li><li> Данные, требующие расширенных возможностей приема или обработки, не доступны сегодня в журналах Azure Monitor. </li></ul> |
