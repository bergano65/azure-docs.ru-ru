...
> [!NOTE]
> Обнаружение сезоне задается интервал час, день или неделю. Это означает, что другие шаблоны, такие как шаблон bihourly или могут не определяться semiweekly.
...
- Низкий — пороги будут достаточно "вольными", то есть располагаются дальше от ожидаемых значений метрик. Правило генерации оповещений будет активировать только на большие отклонения, приводит к меньшим числом оповещений.
...
- Диаграмма периода, в котором оповещение было триггера, содержащего динамического пороговые значения, используемые в определенный момент времени.
...
Пороговые значения, отображаемых на диаграмме, перед созданием правила генерации оповещений на основе метрики, вычисляются на основе достаточно исторических данных, для которого требуется вычислить час или день сезонным (10 дн.). После создания правила генерации оповещений, динамическими порогами будут выполнять все необходимые исторических данных, которая доступна и узнаете, постоянно и опыт, на основе новых данных обеспечивают более высокую точность пороговые значения. Это означает, что после этой диаграммы вычисления также отображает еженедельные закономерности.
