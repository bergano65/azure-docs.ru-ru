...
### <a name="custom-logs"></a>Пользовательские журналы
 ...
 Вы не можете предоставить доступ к отдельным пользовательским журналам, но вы можете предоставить доступ ко всем пользовательским журналам. Чтобы создать роль с доступом ко всем пользовательским журналам, создайте пользовательскую роль, используя следующие действия:
...
```
Альтернативным подходом к управлению доступом к пользовательским журналам является присвоение их ресурсу Azure и управление доступом с помощью парадигмы ресурсного контекста. Чтобы использовать этот метод, необходимо включить идентификатор ресурса, указав его в заголовке [x-ms-AzureResourceId](data-collector-api.md#request-headers) при попарении данных в log Analytics через [API http Data Collector.](data-collector-api.md) Идентификатор ресурса должен быть действительным и применять к нему правила доступа. После того, как журналы попадает, они доступны для тех, кто читает доступ к ресурсу, как поясняется здесь.

Иногда пользовательские журналы поступают из источников, которые напрямую не связаны с определенным ресурсом. В этом случае создайте группу ресурсов только для управления доступом к этим журналам. Группа ресурсов не несет никаких затрат, но дает вам действительный идентификатор ресурса для управления доступом к пользовательским журналам. Например, если определенный брандмауэр отправляет пользовательские журналы, создайте группу ресурсов под названием "MyFireWallLogs" и убедитесь, что запросы API содержат идентификатор ресурсов "MyFireWallLogs". Записи журнала брандмауэра доступны только пользователям, которым был предоставлен доступ либо к MyFireWallLogs, либо к тем, у кого есть полный доступ к рабочей области.          
