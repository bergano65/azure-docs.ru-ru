...
Значение `False` означает, что Рабочая область настроена с использованием режима доступа контекста рабочей области.  Значение `True` означает, что Рабочая область настроена с режимом доступа к контексту ресурса.
...
| `Microsoft.OperationalInsights/*` | Добавление, обновление и удаление конфигурации для рабочих областей Log Analytics. Чтобы изменить дополнительные параметры рабочей области, необходимо `Microsoft.OperationalInsights/workspaces/write`пользователя. |
...
1. Чтобы предоставить пользователю доступ к данным журнала из своих ресурсов, выполните следующие действия.
    ...
    * Предоставьте пользователям `*/read` или `Microsoft.Insights/logs/*/read` разрешения на доступ к их ресурсам. Если им уже назначена роль [модуля чтения log Analytics](../../role-based-access-control/built-in-roles.md#reader) в рабочей области, это достаточно.
...
4. Чтобы предоставить пользователю доступ к данным журнала из своих ресурсов и прочитать все данные журнала Управление обновленияминого решения в Azure AD и прочитать их из рабочей области, выполните следующие действия.
    ...
    * Предоставьте пользователям следующие разрешения для ресурсов: `*/read`, назначенные роли читателя или `Microsoft.Insights/logs/*/read`. 
...
### <a name="custom-logs"></a>Пользовательские журналы
 ...
 В настоящее время вы не можете предоставить доступ к отдельным пользовательским журналам, но можете предоставить доступ ко всем настраиваемым журналам. Чтобы создать роль с доступом ко всем настраиваемым журналам, создайте настраиваемую роль, выполнив следующие действия.
...
* Если пользователю предоставлено глобальное разрешение на чтение для стандартных ролей читателя или участников, включающих действие _\*/Реад_ , оно переопределит управление доступом на уровне таблицы и предоставит им доступ ко всем данным журнала.
