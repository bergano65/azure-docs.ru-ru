...
Для метрик платформы, которые могут быть экспортированы через диагностические настройки, есть несколько метрик, для которых Azure Monitor интерпретирует '0s' как 'Nulls'. Это вызвало некоторую путаницу между реальными '0s' (излучаемые ресурсом) и интерпретируемыми '0s' (Nulls). Вскоре произойдет изменение, и метрики платформы, экспортируемой через диагностические настройки, больше не будут экспортировать '0s', если они действительно не были испущены базовым ресурсом. Изменение было запланировано на 1 апреля 2020 года, но было отложено из-за приоритетных сдвигов из-за COVID-19. 
...
| Экспортируемый через диагностические настройки?  | Уже излучают NULL |  ResourceType  |  Метрика  |  MetricDisplayName  |  Единицы  |  AggregationType | 
|---|---| ---- | ----- | ------ | ---- | ---- | 
