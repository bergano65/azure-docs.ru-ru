...
Путь должен отвечать в течение двух минут с кодом состояния от 200 до 299 (включительно). Если путь не отвечает в течение двух минут или возвращает код состояния за пределами диапазона, то экземпляр считается неработоспособным. Проверка работоспособности интегрируется с функциями проверки подлинности и авторизации службы приложений, когда система подйдет к конечной точке, даже если эти функции Security включены. При использовании собственной системы проверки подлинности путь проверки работоспособности должен разрешить анонимный доступ. Если на сайте включено только HTTP**s**, запрос Healthcheck будет отправлен через HTTP**s**.
