---
title: Подключение компьютеров Windows к монитору Azure Документы Майкрософт
description: В этой статье описывается, как подключить компьютеры Windows, размещенные в других облаках или на месте, к Azure Monitor с помощью агента Log Analytics для Windows.
ms.subservice: logs
ms.topic: conceptual
author: bwren
ms.author: bwren
...
>[!NOTE]
>Центр безопасности Azure (ASC) зависит от агента мониторинга Майкрософт (также называемого агентом Windows Log Analytics) и устанавливает и настраивает его для отчета в рабочее пространство Log Analytics в рамках развертывания. ASC включает в себя опцию автоматического подготовки, которая позволяет автоматическую установку агента Log Analytics Windows на все ВМ в подписке и настраивает его на отчет в определенное рабочее пространство. Для получения дополнительной информации об этой опции [см.](../../security-center/security-center-enable-data-collection.md#auto-provision-mma)
>
...
3. Выберите **Расширенные настройки.**<br><br> ![Дополнительные параметры Log Analytics](media/agent-windows/log-analytics-advanced-settings-01.png)<br><br>  
...
>[!NOTE]
>Если вы настраиваете VM под управлением Windows Server 2008 SP2 x64 для использования TLS 1.2, сначала необходимо установить следующее [обновление подписи кода SHA-2](https://support.microsoft.com/help/4474419/sha-2-code-signing-support-update) перед выполнением следующих шагов. 
>
...
2. Создание подключаемого ключа в соответствии с **протоколами** для TLS 1.2 **HKLM-System/CurrentControlSet/Control/SecurityProviders/SCHANNEL-Protocols-TLS 1.2**
3. Создайте подраздел **Client** в созданном ранее подразделе версии протокола TLS 1.2. Например, **HKLM-System/CurrentControlSet/Control-SecurityProviders-SCHANNEL-Protocols-TLS 1.2"Клиент**.
4. Создайте следующие значения DWORD в соответствии с **HKLM-System-CurrentControlSet/Control/SecurityProviders-SCHANNEL-Протоколы/TLS 1.2 'Клиент:**
...
4. На странице **Destination Folder** измените или сохраните папку установки по умолчанию, а затем нажмите **далее.**
...
2. Чтобы автоматически установить агент и настроить его для отправки отчетов в рабочую область коммерческого облака Azure, из папки, в которую вы извлекли файлы установки, выполните следующую команду: 
   ...
   Чтобы настроить агент для отправки отчетов в облако Azure для US Gov организаций, выполните следующую команду: 
    ...
    ```
    >[!NOTE]
    >Значения строки для параметров *OPINSIGHTS_WORKSPACE_ID* и *OPINSIGHTS_WORKSPACE_KEY* должны быть инкапсулированы в двойных котировках, чтобы поручить установке Windows вставить в качестве действительных вариантов для пакета. 
...
1. На портале Azure ищите и выберите **Monitor.**
1. Выберите **журналы** в меню.
1. На панели **журналов** в типе поля запроса:  
...
- Просмотрите [проблему с агентом Windows,](agent-windows-troubleshoot.md) если вы столкнулись с проблемами при установке или управлении агентом.
