...
ms.locfileid: "86505760"
---
# <a name="connect-windows-computers-to-azure-monitor"></a>Подключение компьютеров Windows к Azure Monitor

Чтобы отслеживать виртуальные машины или физические компьютеры и управлять ими в локальном центре обработки данных или другой облачной среде с помощью Azure Monitor, разверните агент Log Analytics (также называемый Microsoft Monitoring Agent (MMA)) и настройте его таким образом, чтобы он отправлял отчеты в одну или несколько рабочих областей Log Analytics. Агент также поддерживает гибридную рабочую роль Runbook для службы автоматизации Azure.  

На отслеживаемом компьютере Windows агент отображается как служба Microsoft Monitoring Agent. Служба Microsoft Monitoring Agent собирает события из файлов журнала, журнала событий Windows, данных о производительности и других данных телеметрии. Даже если агент не может взаимодействовать с Azure Monitor, в которую он отправляет отчеты, он продолжает запускать и добавлять в очередь собранные данные на диске отслеживаемого компьютера. При восстановлении подключения служба Microsoft Monitoring Agent отправляет собранные данные в службу.
...
>[!NOTE]
>Центр безопасности Azure (ASC) зависит от Microsoft Monitoring Agent (также называется агентом Log Analytics Windows) и устанавливает и настраивает его для передачи отчетов в рабочую область Log Analytics в рамках развертывания. ASC включает параметр автоматической подготовки, который позволяет автоматически устанавливать агент Log Analytics Windows на всех виртуальных машинах в подписке и настраивает его для передачи отчетов в определенную рабочую область. Дополнительные сведения об этом параметре см. в разделе [Включение автоматической подготовки агента Log Analytics](../../security-center/security-center-enable-data-collection.md#auto-provision-mma).
>
...
Перед установкой агента Log Analytics для Windows требуется получить идентификатор и ключ для рабочей области Log Analytics.  При любом методе установки эта информация необходима для правильной настройки агента и обеспечения его взаимодействия с Azure Monitor в Azure для коммерческих организаций и облаке для государственных организаций США. 
...
Чтобы настроить использование протокола [TLS 1.2](/windows-server/security/tls/tls-registry-settings#tls-12) для обмена данными между агентом Windows и службой Log Analytics, можно выполнить следующие действия для включения протокола до установки агента на виртуальной машине или после.

>[!NOTE]
>Если вы настраиваете виртуальную машину под управлением Windows Server 2008 с пакетом обновления 2 (SP2) x64 для использования TLS 1.2, сначала необходимо установить следующее [обновление поддержки подписывания кода SHA-2](https://support.microsoft.com/help/4474419/sha-2-code-signing-support-update), прежде чем выполнять описанные ниже действия. 
>
...
Чтобы извлечь код продукта из пакета установщика агента напрямую, вы можете воспользоваться Orca.exe (компонент пакета разработки программного обеспечения для Windows), который можно найти на странице [компонентов Windows SDK для разработчиков установщика Windows](/windows/win32/msi/platform-sdk-components-for-windows-installer-developers). Кроме того, вы можете использовать PowerShell с [примером сценария](https://www.scconfigmgr.com/2014/08/22/how-to-get-msi-file-information-with-powershell/), написанного специалистом с рангом Microsoft Valuable Professional (MVP).  В обоих случаях сначала нужно извлечь файл**MOMagent.msi** из пакета установки MMASetup.  Эта процедура рассматривается выше в разделе [Установка агента с помощью командной строки](#install-the-agent-using-the-command-line).  
