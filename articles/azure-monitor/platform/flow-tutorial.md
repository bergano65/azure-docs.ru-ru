...
1. В поле **Имя потока** добавьте имя потока и нажмите кнопку **Создать поток**.<br><br>![Сохранение последовательности](media/flow-tutorial/flow06.png)
2. Теперь поток создан и будет выполняться через день, как задано в расписании. 
3. Чтобы проверить поток, щелкните **Запустить сейчас**, а затем — **Запустить поток**.<br><br>![Запуск последовательности](media/flow-tutorial/flow07.png)
