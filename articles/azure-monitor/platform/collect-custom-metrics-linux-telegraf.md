---
title: Сбор пользовательских метрик для Linux VM с агентом InfluxData Телеграф
description: Инструкции о том, как развернуть агент InfluxData Telegraf на Linux VM в Azure и настроить агента для публикации метрик в Azure Monitor.
...
1. Выберите **опцию «Создать ресурс»** из левой навигационной панели. 
1. Введите в поле поиска **Виртуальная машина**.  
1. Выберите **Ubuntu 16.04 LTS** и нажмите кнопку **Создать**. 
1. Предоставьте имя VM, как **MyTelegrafVM**.  
1. Сохраните тип диска **SSD**. Затем укажите **имя пользователя,** например **лазурный.** 
1. Для **типа аутентификации**выберите **пароль**. Затем введите пароль, который будет использоваться позже для SSH в этой виртуальной машине. 
1. Выберите **новую группу ресурсов.** Затем укажите имя, например **myResourceGroup**. Выберите свое **местоположение**. Затем выберите **OK**. 
...
1. На странице **«Настройки»** **в** >  > **группе сетевой безопасности****Выберите открытые входяные порты,** выберите **HTTP** и **SSH (22).** Сохраните остальные значения по умолчанию и нажмите кнопку **ОК**. 
...
1. В панели VM перейдите на вкладку **Identity.** **On** 
...
На странице **подключения к виртуальной машине** сохраните значения по умолчанию, чтобы использовать подключение по DNS-имени через порт 22. В **Журнале Входа с помощью локальной учетной записи VM**отображается команда соединения. Нажмите кнопку, чтобы скопировать эту команду. Ниже представлен пример команды подключения по SSH: 
...
```
Файл конфигурации Telegraf определяет операции Telegraf. Пример файла конфигурации по умолчанию установлен по пути **/etc/telegraf/telegraf.conf**. Файл конфигурации примера перечисляет все возможные входные и выходные плагины. Тем не менее, мы создадим пользовательский файл конфигурации и поможем агенту использовать его, запустив следующие команды: 
...
1. Перейдите к новой вкладке **Monitor.** Затем выберите **метрики**.  
...
Предыдущее пошаговое представление содержит информацию о том, как настроить агент Телеграф для сбора метрик из нескольких основных плагинов ввода. Агент Telegraf имеет поддержку более 150 входных плагинов, с некоторыми дополнительными опциями конфигурации. Компания InfluxData опубликовала [список поддерживаемых подключаемых модулей](https://docs.influxdata.com/telegraf/v1.7/plugins/inputs/) и инструкции по [их настройке](https://docs.influxdata.com/telegraf/v1.7/administration/configuration/).  
...
Если группа ресурсов, виртуальная машина и все связанные с ними ресурсы вам больше не требуются, их можно удалить. Для этого выберите группу ресурсов для виртуальной машины и выберите **Delete**. Затем подтвердите имя удаляемой группы ресурсов. 
