...
См. дополнительные сведения о [безопасной отправке данных с помощью TLS 1.2](./data-security.md#sending-data-securely-using-tls-12). 
...
1. На странице со сведениями об **адресе порта и прокси-сервера**:
   ...
   a. Введите номер TCP-порта, который будет использоваться для шлюза. Программа установки использует этот номер порта для настройки правила входящего трафика в брандмауэре Windows.  По умолчанию используется значение 8080.
...
[Добавьте источники данных](./agent-data-sources.md) для получения данных из подключенных источников и сохраните их в рабочей области log Analytics.