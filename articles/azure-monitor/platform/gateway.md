...
1. На странице со сведениями об **адресе порта и прокси-сервера**:
   ...
   1\. Введите номер TCP-порта, который будет использоваться для шлюза. Программа установки использует этот номер порта для настройки правила входящего трафика в брандмауэре Windows.  По умолчанию используется значение 8080.
...
|ИМЯ ПОЛЬЗОВАТЕЛЯ | Идентификатор пользователя для проверки подлинности с помощью прокси-сервера |
...
1. На панели инструментов Operations Manager выберите **переопределения** , а затем переопределите **руле\фор для конкретного объекта класса: Служба работоспособности** и выберите объект из списка.  Или создайте пользовательскую группу, содержащую объект службы работоспособности серверов, для которых вы хотите применить это переопределение. Затем примените переопределение к пользовательской группе.
...
Если в вашей среде есть гибридные рабочие роли Runbook службы автоматизации, выполните следующие действия, чтобы настроить шлюз OMS для поддержки рабочих ролей вручную.

Чтобы выполнить действия, описанные в этом разделе, необходимо знать регион Azure, в котором находится учетная запись службы автоматизации. Чтобы найти это расположение, сделайте следующее:
...
1. Просмотрите ее регион в разделе **Расположение**.
   ...
   ![Снимок экрана: расположение учетной записи службы автоматизации в портал Azure](./media/gateway/location.png)

Используйте следующие таблицы, чтобы указать URL-адрес для каждого расположения.

**URL-адреса службы данных среды выполнения заданий**
...
| Восточная часть США 2 |eus2-jobruntimedata-prod-su1.azure-automation.net |
...
| Восточная часть США 2 |eus2-agentservice-prod-1.azure-automation.net |
...
Если компьютер зарегистрирован в качестве гибридной рабочей роли Runbook автоматически, используйте Управление обновлениями решение для управления исправлением. Выполните следующие действия.
...
Ошибка на шаге 3 означает, что модуль не был импортирован. Эта ошибка может возникать, когда PowerShell не удается найти модуль. Этот модуль можно найти в пути установки шлюза OMS: *C:\Program Files\Microsoft OMS Gateway\PowerShell\OmsGateway*.
...
| 105 |Ошибка TCP-подключения — недопустимый сертификат клиента: CN = Gateway. <br><br> Убедитесь, что вы используете шлюз OMS версии 1.0.395.0 или более поздней. Также убедитесь, что агент MMA на сервере шлюза OMS и агенты, взаимодействующие с шлюзом OMS, подключены к одной рабочей области Log Analytics. |
