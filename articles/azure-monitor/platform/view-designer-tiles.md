...
> [!NOTE]
> Плитки в представлениях основаны на [запросах журналов](../log-query/log-query-overview.md) в рабочей области log Analytics. В настоящее время они не поддерживают [запросы перекрестных ресурсов](../log-query/cross-workspace-query.md) для получения данных из Application Insights.
...
| query |Выполняемый запрос. Отобразится счетчик количества записей, возвращенных запросом. |
...
| query |Запрос, позволяющий определить, доступны ли данные для представления. Если запрос не возвращает результат, то вместо значения из основного запроса отображается соответствующее сообщение. |
...
| Описание |Текст, отображаемый под именем плитки. |
| **Кольцо** | |
| query |Запрос на выполнение для кольцевой диаграммы. Первое свойство — это текстовое значение, а второе — числовое. Обычно этот запрос использует ключевое слово *measure* для суммирования результатов. |
| **Кольцо** |**> Центр** |
...
| Единицы измерения |Укажите единицы измерения для значений, возвращенных запросом. Эти сведения используются для отображения на графике подписей, в которых указаны типы значений, и при необходимости для преобразования значений. **Тип единицы** определяет категорию единицы измерения и определяет доступные значения **текущего типа единицы** . Если выбрать значение в поле **преобразовать в** , то числовые значения будут преобразованы из **текущего типа единиц измерения** в тип **Convert to** . |
...
* Сведения о [запросах журналов](../log-query/log-query-overview.md) для поддержки запросов на плитках.
