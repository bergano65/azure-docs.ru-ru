---
title: История схемы схемы схемы схемы расширения Windows Azure (WAD)
...
ms.locfileid: "77672248"
---
# <a name="windows-azure-diagnostics-extension-wad-configuration-schema-versions-and-history"></a>Версии и история схемы схемы расширения Windows Azure Diagnostics (WAD)
В этой статье приводится история истории расширения [Azure Diagnostics для](diagnostics-extension-overview.md) версий схемы Windows (WAD), поставляемой в рамках SDK Microsoft Azure.  
...
Для разных версий системы диагностики Azure используются разные схемы конфигурации. Схема 1.0 и 1.2 были унижаются. Для получения дополнительной информации о версии 1.3 и позже, см [Диагностика 1.3 и более поздняя схема конфигурации](diagnostics-extension-schema-windows.md)  
...
Можно указать токен SAS вместо ключа учетной записи хранилища в частной конфигурации. При предоставлении токена SAS ключ учетной записи хранилища игнорируется.
...
* Выходные данные пакета Visual Studio содержат, среди прочего, общедоступный XML-файл конфигурации расширения диагностики для каждой роли. Это позволяет упростить процесс настройки расширения с помощью PowerShell. С помощью строки подключения диагностики Visual Studio получает из общедоступной конфигурации данные учетной записи хранения. Файлы публичной конфигурации создаются в папке `PaaSDiagnostics.<RoleName>.PubConfig.xml`расширения и следуют шаблону. Все развертывания с использованием PowerShell могут использовать этот шаблон имени для сопоставления каждой конфигурации с ролью.
* Строка соединения в файле .cscfg также используется порталом Azure для доступа к данным диагностики, чтобы она может отображаться во вкладке **Мониторинг.** Строка соединения необходима для настройки службы для отображать данные многословного мониторинга на портале.
