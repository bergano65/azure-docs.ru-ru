...
Вы также можете [задать ценовую категорию через Azure Resource Manager](template-workspace-configuration.md#configure-a-log-analytics-workspace) с помощью параметра `sku` (`pricingTier` в шаблоне Azure Resource Manager). 
...
2. На странице **использование и оценка затрат** щелкните **срок хранения данных** в верхней части страницы.
...
Ежедневное ограничение можно настроить с помощью ARM, задав `dailyQuotaGb` параметр в `WorkspaceCapping` соответствии с описанием в разделе [рабочие области — создание или обновление](https://docs.microsoft.com/rest/api/loganalytics/workspaces/createorupdate#workspacecapping). 
...
| Ваша рабочая область достигла [скорости приема данных](https://docs.microsoft.com/azure/azure-monitor/service-limits#log-analytics-workspaces) | Пороговое значение скорости приема по умолчанию, равное 500 МБ (сжатое), применяется к рабочим областям, то есть примерно **6 Гб/мин** без сжатия — фактический размер может различаться в зависимости от длины журнала и его коэффициента сжатия. Это пороговое значение применяется ко всем полученным данным независимо от того, отправлены ли из ресурсов Azure с помощью [параметров диагностики](diagnostic-settings.md), [API сборщика данных](data-collector-api.md) или агентов. При отправке данных в рабочую область с частотой тома выше 80% от порогового значения, настроенного в рабочей области, событие передается в таблицу *операций* в рабочей области каждые 6 часов, пока не будет превышено пороговое значение. Если частота принимаемого тома выше порогового значения, некоторые данные отбрасываются, и событие отправляется в таблицу *операций* в рабочей области каждые 6 часов, пока не будет превышено пороговое значение. Если скорость приема громкости по-тому превышает порог, или ожидается, что она будет достигнута вскоре, можно запросить его увеличение в рабочей области, открыв запрос в службу поддержки. Чтобы получать уведомления о таком событии в рабочей области, создайте [правило генерации оповещений журнала](alerts-log.md) , используя следующий запрос с базой логики оповещений на основе числа результатов больше нуля, периода оценки 5 минут и частоты 5 минут. Достигнута скорость приема, 80% от порогового значения: `Operation | where OperationCategory == "Ingestion" | where Detail startswith "The data ingestion volume rate crossed 80% of the threshold"` . Достигнуто пороговое значение скорости приема: `Operation | where OperationCategory == "Ingestion" | where Detail startswith "The data ingestion volume rate crossed the threshold"` . |
