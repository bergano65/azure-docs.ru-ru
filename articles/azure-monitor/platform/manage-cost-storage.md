...
| Ваша рабочая область достигла [скорости приема данных](https://docs.microsoft.com/azure/azure-monitor/service-limits#log-analytics-workspaces) | Пороговое значение объема принимаемых данных по умолчанию, применяемое к рабочим областям, составляет 500 МБ (в сжатом виде), что приблизительно равно **6 ГБ/мин** для несжатых данных. Фактический размер может отличаться в зависимости от размера и степени сжатия журнала. Это пороговое значение применяется ко всем принимаемым данным независимо от того, каким образом они отправляются из ресурсов Azure: с помощью [параметров диагностики](diagnostic-settings.md), [API сборщика данных](data-collector-api.md) или агентов. Если в одну рабочую область вы отправляете данные, объем которых превышает 80 % от настроенного порога, каждые 6 часов, в течение которых пороговое значение превышено, в таблицу *Операция* в рабочей области будет отправляться событие. Если объем принимаемых данных превышает пороговое значение, часть данных будет удалена, а событие будет отправляться в таблицу *Операция* в рабочей области каждые 6 часов, пока пороговое значение будет превышено. Если ограничение на объем принимаемых за день данных продолжает превышаться или будет превышено в ближайшее время, вы можете запросить увеличение ограничения для рабочей области, отправив запрос на поддержку. Чтобы получать уведомления о таком событии в рабочей области, создайте [правило генерации оповещений журнала](alerts-log.md) , используя следующий запрос с логикой оповещений, основанной на количестве результатов, превышающих 0, период оценки, равный 5 минутам, и частотой 5 минут. Достигнута скорость приема, 80% от порогового значения: `Operation | where OperationCategory == "Ingestion" | where Detail startswith "The data ingestion volume rate crossed 80% of the threshold"` . Достигнуто пороговое значение скорости приема: `Operation | where OperationCategory == "Ingestion" | where Detail startswith "The data ingestion volume rate crossed the threshold"` . |
