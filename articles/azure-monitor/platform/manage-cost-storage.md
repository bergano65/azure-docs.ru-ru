...
Цены на Log Analytics основаны на принятом объеме данных и, возможно, для более длительного хранения данных. Каждая Рабочая область Log Analytics оплачивается как отдельная служба и участвует в выставлении счетов за подписку Azure. Объем приема данных может быть значительным в зависимости от следующих факторов. 

  - число включенных решений по управлению;
  - Использование решений с собственной моделью выставления счетов для экземпляра [центра безопасности Azure](https://azure.microsoft.com/en-us/pricing/details/security-center/)
  - Число отслеживаемых виртуальных машин
  - Тип данных, собираемых с каждой отслеживаемой виртуальной машины 

> [!NOTE]
> Недавно объявленные ценовые категории резервирования емкости будут доступны для Log Analytics 1 ноября 2019. Дополнительные сведения см. в [https://azure.microsoft.com/en-us/pricing/details/monitor/](Azure Monitor pricing page).
...
Сохранение также можно задать с помощью [ARM](https://docs.microsoft.com/azure/azure-monitor/platform/template-workspace-configuration#configure-a-log-analytics-workspace) , используя параметр `dataRetention`. Кроме того, если задать срок хранения данных 30 дней, можно немедленно запустить немедленную очистку старых данных с помощью параметра `immediatePurgeDataOn30Days`, который может быть полезен для сценариев, связанных с соответствием. Эта функция доступна только через ARM. 
...
[Ценовую категорию](https://docs.microsoft.com/azure/azure-monitor/platform/template-workspace-configuration#configure-a-log-analytics-workspace) можно также задать с помощью ARM, используя параметр `ServiceTier`. 
