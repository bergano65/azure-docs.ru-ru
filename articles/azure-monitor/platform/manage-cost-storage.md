...
По умолчанию для Log Analytics используется модель оплаты **по мере использования**, основанная на объеме полученных данных. При необходимости эта модель может использоваться для более длительного хранения данных. Объем данных оценивается по размеру данных для хранения. Каждая рабочая область Log Analytics оплачивается как отдельная служба и включается в счет за подписку Azure. Объем принятых данных может быть значительным в зависимости от следующих факторов: 
...
Во всех ценовых категориях объем данных вычисляется на основе строкового представления типа данных по мере подготовки к их хранению. Некоторые [свойства, общие для всех типов данных](https://docs.microsoft.com/azure/azure-monitor/platform/log-standard-properties), не включены в вычисление размера события, в том числе `_ResourceId`, `_ItemId`, `_IsBillable` и `_BilledSize`.
...
Так как выставление счетов за принимаемые данные выполняется на уровне кластера, рабочие области, связанные с кластером, больше не имеют ценовой категории. Полученные объемы данных из каждой рабочей области, связанной с кластером, суммируются для вычисления ежедневного счета за кластер. Обратите внимание, что распределения по отдельным узлам из [Центра безопасности Azure](https://docs.microsoft.com/azure/security-center/) применяются на уровне рабочей области перед этой статистической обработкой агрегированных данных во всех рабочих областях в кластере. Плата за хранение данных по-прежнему взимается на уровне рабочей области. Обратите внимание, что плата за кластер начисляется при его создании независимо от того, связаны ли рабочие области с кластером. 
...
По достижении ежедневного ограничения сбор платных типов данных прекращается до конца дня. (Задержка, связанная с применением ежедневного ограничения, может означать, что ограничение не применяется в качестве точно указанного уровня ежедневного ограничения.) В верхней части страницы для выбранной рабочей области Log Analytics появится предупреждающий баннер и событие операции будет отправлено в таблицу *Операция* в категории **LogManagement**. Сбор данных возобновится по наступлении времени сброса, определенного в разделе *Daily limit will be set at* (Ежедневное ограничение будет установлено в). Рекомендуем определить правило генерации оповещений на основе этого события операции, настроенного для уведомления о достижении ежедневного ограничения по сбору данных. 

> [!WARNING]
> Ежедневное ограничение не останавливает сбор данных из Центра безопасности Azure, за исключением рабочих областей, в которых Центр безопасности Azure был установлен до 19 июня 2017 г. 
...
Обратите внимание, что предложение `where IsBillable = true` отсеивает типы данных из определенных решений, для которых не взимается плата за прием данных. 
...
Для данных из узлов, размещенных в Azure, можно определить **размер** полученных данных __за каждую подписку Azure__. Проанализируйте свойство `_ResourceId`:
...
При изменении `subscriptionId` на `resourceGroup` отобразится объем принятых данных, подлежащих оплате, по группам ресурсов Azure. 
