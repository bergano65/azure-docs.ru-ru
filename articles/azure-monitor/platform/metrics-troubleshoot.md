...
**Решение.** При использовании обозревателя метрик требуется выбрать подписки и группы ресурсов до перечисления доступных ресурсов. Если ресурс не отображается, сделайте следующее:
...
**Решение.** Задайте более широкий диапазон времени для диаграммы. Вы можете начать с "последних 30 дней", используя более крупную гранулярность времени (или полагаться на параметр "Автоматическая детализация времени").
...
**Решение.** Если расширение "Диагностика Azure" включено, но вы все еще не видите свои метрики, выполните шаги из [руководства по устранению неполадок с расширением "Диагностика Azure"](diagnostics-extension-troubleshooting.md#metric-data-doesnt-appear-in-the-azure-portal). См. также шаги по устранению неполадок для [не удается выбрать пространство имен и метрики гостевой ОС](metrics-troubleshoot.md#cannot-pick-guest-os-namespace-and-metrics) .
...
**Решение.** Обновите неработающую плитку, выбрав альтернативную метрику для диаграммы на панели мониторинга. См. [список доступных метрик для служб Azure](metrics-supported.md).
...
1. Убедитесь, что учетная запись хранения не защищена брандмауэром. Портал Azure требуется доступ к учетной записи хранения для получения данных метрик и построения диаграмм.
