---
title: Создание и управление классическими оповещениями метрик с помощью Azure Monitor
...
Классические оповещения метрик в Azure Monitor позволяют получать уведомления, когда одна из метрик достигает порогового значения. Классические оповещения метрик — это более старая функциональность, которая позволяет оповещать только метрики без измерений. Существует более новая функциональность, называемая "оповещения метрик", которая имеет улучшенные функциональные возможности по сравнению с классическими оповещениями метрики. Подробнее о новой функциональности оповещений метрик можно узнать в [обзоре оповещений метрик](./alerts-metric-overview.md). В этой статье мы расскажем, как создавать, просматривать классические правила генерации оповещений метрик и управлять ими через портал Azure, Azure CLI и PowerShell.
