...
4. Ниже приведен гипотетический сценарий масштабирования, который используется в данном пошаговом руководстве.
   ...
   * **Размер очереди сообщений** — я использую очередь служебной шины для входящих сообщений, поступающих в приложение. Я использую число сообщений очереди и ЦПУ% и настроил профиль по умолчанию для активации действия масштабирования, если количество сообщений или ЦП достигает порогового значения.\*
   * **День недели и время суток** — мне нужен профиль для еженедельного повторения в определенное время дня, который называется "Утренние часы в рабочие дни". На основе исторических данных я знал, что лучше иметь определенное количество экземпляров виртуальных машин, чтобы обрабатывалась нагрузка приложения в течение этого времени.\*
   * **Особые даты** — добавлен профиль "День запуска продукта". Я планирую впереди конкретные даты, чтобы мое приложение было готово к обработке рекламных объявлений, связанных с загрузкой, и при помещении нового продукта в приложение.\*
   * *Последние два профиля могут также иметь в них другие правила на основе метрик производительности. В этом случае я решил не иметь одного и вместо этого использовать правила на основе метрик производительности по умолчанию. Правила являются необязательными для повторяющихся профилей и профилями на основе даты.*
