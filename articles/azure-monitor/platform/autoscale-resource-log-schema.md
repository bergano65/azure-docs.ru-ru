---
title: Схема событий журнала автоматического масштабирования Azure
description: Формат журналов для мониторинга и устранения неполадок действий автомасштабирования
ms.topic: conceptual
ms.date: 11/14/2019
...
Ниже приведены общие форматы для автомасштабирования журналов ресурсов, включая примеры данных. Не все приведенные ниже примеры имеют правильный формат JSON, так как они могут содержать несколько значений, допустимых для данного поля. 

Используйте события этого типа для устранения проблем, с которыми может столкнуться Автомасштабирование. Дополнительные сведения см. в разделе [Устранение неполадок автоматического масштабирования](autoscale-troubleshoot.md).


## <a name="profile-evaluation"></a>Оценка профиля

Записывается, когда Автомасштабирование сначала просматривает профиль автомасштабирования
...
## <a name="profile-cooldown-evaluation"></a>Cooldownная Оценка профиля

Записывается, если Автомасштабирование оценивается, если не следует выполнять масштабирование из-за длительного периода. 
...
## <a name="rule-evaluation"></a>Оценка правил

Записывается, когда Автомасштабирование сначала начинает оценивать конкретное правило масштабирования. 
...
## <a name="metric-evaluation"></a>Оценка метрики

Записывается, когда Автомасштабирование оценивает метрику, используемую для запуска действия масштабирования. 
...
## <a name="instance-count-evaluation"></a>Оценка числа экземпляров

Записывается, когда Автомасштабирование оценивает количество экземпляров, уже запущенных при подготовке, чтобы решить, должно ли оно начать больше, завершить часть или ничего не делать. 
...
## <a name="scale-action-evaluation"></a>Оценка действия масштабирования

Записывается, когда Автомасштабирование начинает оценку, если необходимо выполнить действие масштабирования. 
...
## <a name="instance-update-evaluation"></a>Оценка обновления экземпляра

Записывается, когда Автомасштабирование обновляет число выполняющихся экземпляров вычислений, либо вверх, либо вниз.
...
## <a name="scale-action"></a>Действие масштабирования

Записывается, когда Автомасштабирование инициирует действие масштабирования: вверх или вниз. 
```json
{
  "time": "2018-09-10 18:12:00.6132593",
  "resourceId": "/SUBSCRIPTIONS/BA13C41D-C957-4774-8A37-092D62ACFC85/RESOURCEGROUPS/AUTOSCALETRACKING12042017/PROVIDERS/MICROSOFT.INSIGHTS/AUTOSCALESETTINGS/DEFAULTSETTING",
...
## <a name="scale-action-tracking"></a>Отслеживание действий масштабирования

Записывается с разными интервалами действия масштабирования экземпляра.
