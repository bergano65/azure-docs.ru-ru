...
| Экспорт | [Перенаправление метрик в журналы](diagnostic-logs-stream-log-store.md) для анализа данных в Azure Monitor метриках вместе с данными в журналах Azure Monitor и для хранения значений метрик дольше 93 дней.<br>Потоковая передача метрик в [концентратор событий](stream-monitoring-data-event-hubs.md) для их направления на внешние системы. |
...
> [!NOTE]
> Вы можете [Отправить метрики платформы для Azure Monitor ресурсов в рабочую область log Analytics](diagnostic-logs-stream-log-store.md) для долгосрочного прогнозирования.
