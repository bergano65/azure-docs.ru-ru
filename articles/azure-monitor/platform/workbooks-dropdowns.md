...
В примерах до сих пор явно задан параметр для выбора только одного значения в раскрывающемся списке. Параметры раскрывающегося списка `multiple selection` также поддерживаются, так же как и `Allow multiple selection` при проверке параметра. 

Пользователь также может указать формат результирующего набора с `delimiter` помощью параметров и. `quote with` Значение по умолчанию просто возвращает значения как коллекцию в этой форме: "a", "b", "c". Они также имеют возможность ограничить количество вариантов выбора.
