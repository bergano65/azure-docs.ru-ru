...
Службе группы действий потребуется разрешение на получение маркеров проверки подлинности из приложения AAD для проверки подлинности с помощью службы. Чтобы предоставить эти разрешения, необходимо создать субъект-службу для службы группы действий в клиенте.
Эти [команды PowerShell](./action-groups.md#secure-webhook-powershell-script) можно использовать для этой цели. (Требуются права администратора клиента).
В качестве дополнительного шага можно определить роль приложения в манифесте созданного приложения, что позволит дополнительно ограничить доступ таким образом, чтобы сообщения могли быть отправлены только определенным приложениям с этой конкретной ролью. Эта роль должна быть назначена субъекту-службе группы действий. \
Этот шаг можно выполнить с помощью одних и тех же [команд PowerShell](./action-groups.md#secure-webhook-powershell-script).
