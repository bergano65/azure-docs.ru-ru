...
- Агрегат времени (Статистический показатель, который выполняется в необработанных значениях метрик. [Поддерживаемые агрегаты времени](metrics-charts.md#aggregation) : min, Max, AVG, Total, Count) — Average.
