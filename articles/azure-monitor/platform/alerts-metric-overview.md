...
- Агрегат времени (Статистический показатель, который выполняется в необработанных значениях метрик. Поддерживаемые агрегации времени: Мин, Макс, Авг, Всего, Граф): Средний
- Период (Окно оглядываться назад, над которым проверяются метрические значения): За последние 5 минут
- Частота (частота, с которой метрика предупреждает, если условия выполнены): 1 мин
- Оператор: Больше, чем
...
Когда вы используете несколько условий в одном правиле, правило "и" условия вместе.  То есть, предупреждение сравнивается, когда все условия в оповещении оцениваются как истинные и разрешаются, когда одно из условий уже не соответствует действительности. И пример этого типа оповещения будет бдительным, когда "ЦП выше 90%" и "длина очереди составляет более 300 элементов". 
...
- Целевой ресурс (ресурс Azure, который требуется отслеживать): myVM.
- Метрика: загрузка ЦП
- Тип состояния: Динамический
- Агрегат времени (Статистический показатель, который выполняется в необработанных значениях метрик. Поддерживаемые агрегации времени: Мин, Макс, Авг, Всего, Граф): Средний
- Период (Окно оглядываться назад, над которым проверяются метрические значения): За последние 5 минут
- Частота (частота, с которой метрика предупреждает, если условия выполнены): 1 мин
- Оператор: Больше, чем
- Чувствительность: Средний
- Периоды оглянуться назад: 4
...
На данный момент вы узнали, как одно предупреждение метрики можно использовать для отслеживания одного или нескольких метрических временных рядов, связанных с одним ресурсом Azure. Во многих случаях вы можете захотеть, чтобы одно и то же правило генерации оповещений применялось ко многим ресурсам. Azure Monitor также поддерживает мониторинг нескольких ресурсов (одного и того же типа) с помощью одного правила предупреждения метрики для ресурсов, которые существуют в том же регионе Azure. 

Эта функция в настоящее время поддерживается для метрик платформы (не пользовательских метрик) для следующих служб в следующих облаках Azure:

| Служба | Public Azure (Общедоступные Azure) | Государственные организации | Китай |
|:--------|:--------|:--------|:--------|
| Виртуальные машины  | **Да** | нет | нет |
| Базы данных серверов S'L | **Да** | **Да** | нет |
| Эластичные пулы серверов S'L | **Да** | **Да** | нет |
| Устройства края коробки данных | **Да** | **Да** | нет |

Можно указать область мониторинга одним правилом предупреждения метрики одним из трех способов. Например, с помощью виртуальных машин можно указать область действия как:  
...
- [Узнайте, как создавать, просматривать и управлять метрическими оповещениями в Azure](alerts-metric.md)
...
- [Подробнее о группах действий](action-groups.md)
