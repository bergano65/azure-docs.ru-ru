...
| Категория |Нет |Разделенный запятыми список категорий журналов, которые будут включены. |
...
| RetentionInDays |Нет |Число дней, для которых будут храниться события от 1 до 365. Нулевое значение означает, что журналы хранятся неограниченно долго. |
...
Когда вы настроите архивацию, в учетной записи хранения будет создан контейнер хранилища, как только в одной из включенных категорий журналов возникнет событие журнала. Большие двоичные объекты в контейнере выполните то же соглашение об именовании в журналы действий и журналов диагностики, как показано ниже.
