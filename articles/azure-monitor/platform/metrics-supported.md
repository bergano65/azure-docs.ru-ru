...
|Производительность|Производительность|Percent|Average|Индикатор загрузки экземпляра управления API для принятия обоснованных решений о том, следует ли масштабировать экземпляр в соответствии с большей нагрузкой.|Местоположение|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|Загрузка ЦП|Загрузка ЦП|Percent|Average|Процент выделенных единиц вычисления, которые сейчас используются виртуальными машинами.|Нет измерений|
...
|Скорость чтения с диска (байт/с)|Чтение с диска|BytesPerSecond|Average|Среднее число байтов, считанных с диска за период мониторинга.|Нет измерений|
|Скорость записи на диск (байт/с)|Запись на диск|BytesPerSecond|Average|Среднее число байтов, записанных на диск за период мониторинга.|Нет измерений|
|Операций чтения с диска/с|Чтение с дисков (операций/с)|CountPerSecond|Average|Операций ввода-вывода с чтением с диска/с.|Нет измерений|
|Операций записи на диск/с|Запись на диски (операций/с)|CountPerSecond|Average|Операций ввода-вывода с записью на диск/с.|Нет измерений|

## <a name="microsoftclassiccomputedomainnamesslotsroles"></a>Microsoft.ClassicCompute/domainNames/slots/roles

|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|Загрузка ЦП|Загрузка ЦП|Percent|Average|Процент выделенных единиц вычисления, которые сейчас используются виртуальными машинами.|RoleInstanceId|
...
|Скорость чтения с диска (байт/с)|Чтение с диска|BytesPerSecond|Average|Среднее число байтов, считанных с диска за период мониторинга.|RoleInstanceId|
|Скорость записи на диск (байт/с)|Запись на диск|BytesPerSecond|Average|Среднее число байтов, записанных на диск за период мониторинга.|RoleInstanceId|
|Операций чтения с диска/с|Чтение с дисков (операций/с)|CountPerSecond|Average|Операций ввода-вывода с чтением с диска/с.|RoleInstanceId|
|Операций записи на диск/с|Запись на диски (операций/с)|CountPerSecond|Average|Операций ввода-вывода с записью на диск/с.|RoleInstanceId|

## <a name="microsoftcognitiveservicesaccounts"></a>Microsoft.CognitiveServices/accounts

|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|TotalCalls|Total Calls|Count|Всего|Общее количество вызовов.|ApiName, Имя_операции, регион|
|SuccessfulCalls|Successful Calls|Count|Всего|Количество успешных вызовов.|ApiName, Имя_операции, регион|
|TotalErrors|Total Errors|Count|Всего|Общее количество вызовов, завершившихся ошибкой (код ответа HTTP 4xx или 5xx).|ApiName, Имя_операции, регион|
|BlockedCalls|Blocked Calls|Count|Всего|Количество вызовов, которое превысило ограничение тарифа или квоты.|ApiName, Имя_операции, регион|
|ServerErrors|Server Errors|Count|Всего|Количество вызовов с внутренней ошибкой в службе (код ответа HTTP 5xx).|ApiName, Имя_операции, регион|
|ClientErrors|Client Errors|Count|Всего|Количество вызовов с ошибкой на стороне клиента (код ответа HTTP 4xx).|ApiName, Имя_операции, регион|
...
|Задержка|Задержка|MilliSeconds|Average|Задержка в миллисекундах.|ApiName, Имя_операции, регион|
|CharactersTranslated|Символов расшифровано|Count|Всего|Общее число символов во входящем текстовом запросе.|ApiName, Имя_операции, регион|
...
|Операций чтения с диска/с|Чтение с дисков (операций/с)|CountPerSecond|Average|Операций ввода-вывода с чтением с диска/с|Нет измерений|
|Операций записи на диск/с|Запись на диски (операций/с)|CountPerSecond|Average|Операций ввода-вывода с записью на диск/с|Нет измерений|
|Оставшиеся кредиты ЦП|Оставшиеся кредиты ЦП|Count|Average|Общее число кредитов, доступных для пакетной передачи|Нет измерений|
|Использованные кредиты ЦП|Использованные кредиты ЦП|Count|Average|Общее число кредитов, использованных виртуальной машиной|Нет измерений|
|Скорость чтения в расчете на диск (байт/с)|Скорость чтения с диска данных (байт/с) (нерекомендуется)|CountPerSecond|Average|Скорость чтения (байт/с) с одного диска во время мониторинга|ИД слота|
|Скорость записи в расчете на диск (байт/с)|Скорость записи на диск данных (байт/с) (нерекомендуется)|CountPerSecond|Average|Скорость записи (байт/с) на один диск во время мониторинга|ИД слота|
|Скорость чтения в расчете на диск (операций/с)|Скорость чтения с диска данных (операций/с) (нерекомендуется)|CountPerSecond|Average|Операции ввода-вывода в секунду при чтении с одного диска во время мониторинга|ИД слота|
|Скорость записи в расчете на диск (операций/с)|Скорость записи на диск данных (операций/с) (нерекомендуется)|CountPerSecond|Average|Операции ввода-вывода в секунду при записи на один диск во время мониторинга|ИД слота|
|Длина очереди в расчете на диск|Длина очереди диска данных (нерекомендуется)|Count|Average|Длина очереди дисков данных|ИД слота|
|Скорость чтения с ОС на диск (байт/с)|Скорость чтения с диска ОС (байт/с) (нерекомендуется)|CountPerSecond|Average|Скорость чтения (байт/с) с одного диска во время мониторинга для диска ОС|Нет измерений|
|Скорость записи на ОС на диск (байт/с)|Скорость записи на диск ОС (байт/с) (нерекомендуется)|CountPerSecond|Average|Скорость записи (байт/с) на один диск во время мониторинга для диска ОС|Нет измерений|
|Скорость чтения с ОС на диск (операций/с)|Скорость чтения с диска ОС (операций/с) (нерекомендуется)|CountPerSecond|Average|Операции ввода-вывода в секунду при чтении с одного диска во время мониторинга для диска ОС|Нет измерений|
|Скорость записи на ОС на диск (операций/с)|Скорость записи на диск ОС (операций/с) (нерекомендуется)|CountPerSecond|Average|Операции ввода-вывода в секунду при записи на один диск во время мониторинга для диска ОС|Нет измерений|
|Длина очереди ОС на диск|Длина очереди диска ОС (нерекомендуется)|Count|Average|Длина очереди дисков ОС|Нет измерений|
|Скорость чтения с диска данных (байт/с)|Скорость чтения с диска данных, байт/с (предварительная версия)|CountPerSecond|Average|Скорость чтения (байт/с) с одного диска во время мониторинга|LUN|
|Скорость записи на диск данных (байт/с)|Скорость записи на диск данных, байт/с (предварительная версия)|CountPerSecond|Average|Скорость записи (байт/с) на один диск во время мониторинга|LUN|
|Скорость чтения с диска данных (операций/с)|Скорость чтения с диска данных, операций/с (предварительная версия)|CountPerSecond|Average|Операции ввода-вывода в секунду при чтении с одного диска во время мониторинга|LUN|
|Скорость записи на диск данных (операций/с)|Скорость записи на диск данных, операций/с (предварительная версия)|CountPerSecond|Average|Операции ввода-вывода в секунду при записи на один диск во время мониторинга|LUN|
|Длина очереди диска данных|Длина очереди диска данных (предварительная версия)|Count|Average|Длина очереди дисков данных|LUN|
|Скорость чтения с диска ОС (байт/с)|Скорость чтения с диска ОС, байт/с (предварительная версия)|CountPerSecond|Average|Скорость чтения (байт/с) с одного диска во время мониторинга для диска ОС|Нет измерений|
|Скорость записи на диск ОС (байт/с)|Скорость записи на диск ОС, байт/с (предварительная версия)|CountPerSecond|Average|Скорость записи (байт/с) на один диск во время мониторинга для диска ОС|Нет измерений|
|Скорость чтения с диска ОС (операций/с)|Скорость чтения с диска ОС, операций/с (предварительная версия)|CountPerSecond|Average|Операции ввода-вывода в секунду при чтении с одного диска во время мониторинга для диска ОС|Нет измерений|
|Скорость записи на диск ОС (операций/с)|Скорость записи на диск ОС, операций/с (предварительная версия)|CountPerSecond|Average|Операции ввода-вывода в секунду при записи на один диск во время мониторинга для диска ОС|Нет измерений|
|Длина очереди диска ОС|Длина очереди диска ОС (предварительная версия)|Count|Average|Длина очереди дисков ОС|Нет измерений|
|Входящие потоки|Входящие потоки (предварительная версия)|Count|Average|Входящие потоки — это число текущих входящих потоков (с трафиком по направлению к виртуальной машине)|Нет измерений|
|Исходящие потоки|Исходящие потоки (предварительная версия)|Count|Average|Исходящие потоки — это число текущих исходящих потоков (с трафиком по направлению от виртуальной машины)|Нет измерений|
|Максимальная скорость создания входящих потоков|Максимальная скорость создания входящих потоков (предварительная версия)|CountPerSecond|Average|Максимальная скорость создания входящих потоков (входящий трафик виртуальной машины)|Нет измерений|
|Максимальная скорость создания исходящих потоков|Максимальная скорость создания исходящих потоков (предварительная версия)|CountPerSecond|Average|Максимальная скорость создания исходящих потоков (исходящий трафик виртуальной машины)|Нет измерений|
|Попадание при чтении в кэше для диска данных категории "Премиум"|Попадание при чтении в кэше для диска данных категории "Премиум" (предварительная версия)|Percent|Average|Попадание при чтении в кэше для диска данных категории "Премиум"|LUN|
|Промах чтения в кэше для диска данных категории "Премиум"|Промах чтения в кэше для диска данных категории "Премиум" (предварительная версия)|Percent|Average|Промах чтения в кэше для диска данных категории "Премиум"|LUN|
|Попадание при чтении в кэше для диска ОС категории "Премиум"|Попадание при чтении в кэше для диска ОС категории "Премиум" (предварительная версия)|Percent|Average|Попадание при чтении в кэше для диска ОС категории "Премиум"|Нет измерений|
|Промах чтения в кэше для диска ОС категории "Премиум"|Промах чтения в кэше для диска ОС категории "Премиум" (предварительная версия)|Percent|Average|Промах чтения в кэше для диска ОС категории "Премиум"|Нет измерений|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|Загрузка ЦП|Загрузка ЦП|Percent|Average|Процент выделенных единиц вычисления, которые сейчас используются виртуальными машинами|VMName|
...
|Операций чтения с диска/с|Чтение с дисков (операций/с)|CountPerSecond|Average|Операций ввода-вывода с чтением с диска/с|VMName|
|Операций записи на диск/с|Запись на диски (операций/с)|CountPerSecond|Average|Операций ввода-вывода с записью на диск/с|VMName|
|Оставшиеся кредиты ЦП|Оставшиеся кредиты ЦП|Count|Average|Общее число кредитов, доступных для пакетной передачи|Нет измерений|
|Использованные кредиты ЦП|Использованные кредиты ЦП|Count|Average|Общее число кредитов, использованных виртуальной машиной|Нет измерений|
|Скорость чтения в расчете на диск (байт/с)|Скорость чтения с диска данных (байт/с) (нерекомендуется)|CountPerSecond|Average|Скорость чтения (байт/с) с одного диска во время мониторинга|ИД слота|
|Скорость записи в расчете на диск (байт/с)|Скорость записи на диск данных (байт/с) (нерекомендуется)|CountPerSecond|Average|Скорость записи (байт/с) на один диск во время мониторинга|ИД слота|
...
|Скорость записи на ОС на диск (операций/с)|Скорость записи на диск ОС (операций/с) (нерекомендуется)|CountPerSecond|Average|Операции ввода-вывода в секунду при записи на один диск во время мониторинга для диска ОС|Нет измерений|
|Длина очереди ОС на диск|Длина очереди диска ОС (нерекомендуется)|Count|Average|Длина очереди дисков ОС|Нет измерений|
|Скорость чтения с диска данных (байт/с)|Скорость чтения с диска данных, байт/с (предварительная версия)|CountPerSecond|Average|Скорость чтения (байт/с) с одного диска во время мониторинга|LUN, VMName|
|Скорость записи на диск данных (байт/с)|Скорость записи на диск данных, байт/с (предварительная версия)|CountPerSecond|Average|Скорость записи (байт/с) на один диск во время мониторинга|LUN, VMName|
|Скорость чтения с диска данных (операций/с)|Скорость чтения с диска данных, операций/с (предварительная версия)|CountPerSecond|Average|Операции ввода-вывода в секунду при чтении с одного диска во время мониторинга|LUN, VMName|
|Скорость записи на диск данных (операций/с)|Скорость записи на диск данных, операций/с (предварительная версия)|CountPerSecond|Average|Операции ввода-вывода в секунду при записи на один диск во время мониторинга|LUN, VMName|
|Длина очереди диска данных|Длина очереди диска данных (предварительная версия)|Count|Average|Длина очереди дисков данных|LUN, VMName|
|Скорость чтения с диска ОС (байт/с)|Скорость чтения с диска ОС, байт/с (предварительная версия)|CountPerSecond|Average|Скорость чтения (байт/с) с одного диска во время мониторинга для диска ОС|VMName|
|Скорость записи на диск ОС (байт/с)|Скорость записи на диск ОС, байт/с (предварительная версия)|CountPerSecond|Average|Скорость записи (байт/с) на один диск во время мониторинга для диска ОС|VMName|
|Скорость чтения с диска ОС (операций/с)|Скорость чтения с диска ОС, операций/с (предварительная версия)|CountPerSecond|Average|Операции ввода-вывода в секунду при чтении с одного диска во время мониторинга для диска ОС|VMName|
|Скорость записи на диск ОС (операций/с)|Скорость записи на диск ОС, операций/с (предварительная версия)|CountPerSecond|Average|Операции ввода-вывода в секунду при записи на один диск во время мониторинга для диска ОС|VMName|
|Длина очереди диска ОС|Длина очереди диска ОС (предварительная версия)|Count|Average|Длина очереди дисков ОС|VMName|
|Входящие потоки|Входящие потоки (предварительная версия)|Count|Average|Входящие потоки — это число текущих входящих потоков (с трафиком по направлению к виртуальной машине)|VMName|
|Исходящие потоки|Исходящие потоки (предварительная версия)|Count|Average|Исходящие потоки — это число текущих исходящих потоков (с трафиком по направлению от виртуальной машины)|VMName|
|Максимальная скорость создания входящих потоков|Максимальная скорость создания входящих потоков (предварительная версия)|CountPerSecond|Average|Максимальная скорость создания входящих потоков (входящий трафик виртуальной машины)|VMName|
|Максимальная скорость создания исходящих потоков|Максимальная скорость создания исходящих потоков (предварительная версия)|CountPerSecond|Average|Максимальная скорость создания исходящих потоков (исходящий трафик виртуальной машины)|VMName|
|Попадание при чтении в кэше для диска данных категории "Премиум"|Попадание при чтении в кэше для диска данных категории "Премиум" (предварительная версия)|Percent|Average|Попадание при чтении в кэше для диска данных категории "Премиум"|LUN, VMName|
|Промах чтения в кэше для диска данных категории "Премиум"|Промах чтения в кэше для диска данных категории "Премиум" (предварительная версия)|Percent|Average|Промах чтения в кэше для диска данных категории "Премиум"|LUN, VMName|
|Попадание при чтении в кэше для диска ОС категории "Премиум"|Попадание при чтении в кэше для диска ОС категории "Премиум" (предварительная версия)|Percent|Average|Попадание при чтении в кэше для диска ОС категории "Премиум"|VMName|
|Промах чтения в кэше для диска ОС категории "Премиум"|Промах чтения в кэше для диска ОС категории "Премиум" (предварительная версия)|Percent|Average|Промах чтения в кэше для диска ОС категории "Премиум"|VMName|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|CpuUsage|Загрузка ЦП|Count|Average|Использование ЦП на всех ядрах (в миллиядрах).|containerName|
|MemoryUsage|Использование памяти|Байт|Average|Общее использование памяти (в байтах).|containerName|
|NetworkBytesReceivedPerSecond|Получено байтов по сети в секунду|Байт|Average|Получено байтов по сети в секунду.|Нет измерений|
|NetworkBytesTransmittedPerSecond|Передано байт по сети в секунду|Байт|Average|Передано байт по сети в секунду.|Нет измерений|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|никреадсраугхпут|Пропускная способность чтения (сеть)|BytesPerSecond|Average|Пропускная способность чтения сетевого интерфейса на устройстве в отчетный период для всех томов в шлюзе.|InstanceName|
|никвритесраугхпут|Пропускная способность записи (сеть)|BytesPerSecond|Average|Пропускная способность записи сетевого интерфейса на устройстве в отчетный период для всех томов в шлюзе.|InstanceName|
|клаудреадсраугхпутпершаре|Пропускная способность скачивания для облака (общий ресурс)|BytesPerSecond|Average|Пропускная способность скачивания в Azure из общего ресурса в отчетный период.|Общий доступ|
|клаудуплоадсраугхпутпершаре|Пропускная способность отправки для облака (общий ресурс)|BytesPerSecond|Average|Пропускная способность отправки в Azure из общего ресурса в отчетный период.|Общий доступ|
|битесуплоадедтоклаудпершаре|Отправлено байт для облака (общий ресурс)|Байт|Average|Общее число байтов, отправляемых в Azure из общего ресурса в отчетный период.|Общий доступ|
|Общая емкость составляет|Общая емкость|Байт|Average|Общая емкость|Нет измерений|
|AvailableCapacity|Доступная емкость|Байт|Average|Доступная емкость в байтах в отчетный период.|Нет измерений|
...
|битесуплоадедтоклауд|Отправлено байт для облака (устройство)|Байт|Average|Общее число байтов, отправляемых в Azure с устройства в отчетный период.|Нет измерений|
|хиперввиртуалпроцессорутилизатион|Пограничные вычисления — процент загрузки ЦП|Percent|Average|Процент загрузки ЦП|InstanceName|
|хипервмеморютилизатион|Пограничные вычисления — использование памяти|Percent|Average|Используемый объем ОЗУ|InstanceName|
...
|TotalRequests|Всего запросов|Count|Count|Количество выполненных запросов|DatabaseName, CollectionName, Region, StatusCode, OperationType|
...
|ServerErrors|Ошибки на сервере.|Count|Всего|Ошибки на сервере для Microsoft.EventHub.|EntityName, |
...
|CaptureBacklog|Невыполненная работа сбора.|Count|Всего|Невыполненная работа сбора для Microsoft.EventHub.|EntityName|
|CapturedMessages|Собранные сообщения.|Count|Всего|Собранные сообщения для Microsoft.EventHub.|EntityName|
|CapturedBytes|Собранные байты.|Байт|Всего|Собранные байты для Microsoft.EventHub.|EntityName|
|Size|Size|Байт|Average|Размер концентратора событий в байтах.|EntityName|
...
|EHINMSGS|Входящих сообщений (устаревшая)|Count|Всего|Всего входящих сообщений для пространства имен (не рекомендуется)|Нет измерений|
...
|EHOUTMSGS|Исходящих сообщений (устаревшая)|Count|Всего|Всего исходящих сообщений для пространства имен (не рекомендуется)|Нет измерений|
...
|EHINBYTES|Входящих байт (устаревшая)|Байт|Всего|Пропускная способность входящего сообщения концентратора событий для пространства имен (не рекомендуется)|Нет измерений|
...
|EHOUTBYTES|Исходящих байт (устаревшая)|Байт|Всего|Пропускная способность исходящего сообщения концентратора событий для пространства имен (не рекомендуется)|Нет измерений|
...
|ServerErrors|Ошибки на сервере. (Предварительная версия)|Count|Всего|Ошибки на сервере для Microsoft.EventHub. (Предварительная версия)|Нет измерений|
...
|CaptureBacklog|Невыполненная работа сбора. (Предварительная версия)|Count|Всего|Невыполненная работа сбора для Microsoft.EventHub. (Предварительная версия)|Нет измерений|
|CapturedMessages|Собранные сообщения. (Предварительная версия)|Count|Всего|Собранные сообщения для Microsoft.EventHub. (Предварительная версия)|Нет измерений|
...
|ЦП|CPU (Preview) (ЦП (предварительная версия))|Percent|Максимум|Загрузка ЦП для кластера концентратора событий в процентах|Role|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|ObservedMetricValue|Наблюдаемое значение метрики|Count|Average|Значение, вычисленное при выполнении автомасштабирования|MetricTriggerSource|
|MetricThreshold|Пороговое значение метрики|Count|Average|Настроенное при запуске автомасштабирования пороговое значение.|MetricTriggerRule|
|ObservedCapacity|Наблюдаемая емкость|Count|Average|Емкость, обнаруженная при выполнении автомасштабирования.|Нет измерений|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|availabilityResults/Аваилабилитиперцентаже|Доступность|Percent|Average|Процент пройденных тестов доступности.|Аваилабилитиресулт/Name, Аваилабилитиресулт/Location|
...
|browserTimings/totalDuration|Время загрузки страницы браузера|MilliSeconds|Average|Время от запроса пользователя до загрузки модели DOM, таблиц стилей, скриптов и образов.|Нет измерений|
|dependencies/count|Вызовы зависимостей|Count|Count|Число вызовов, сделанных приложением к внешним ресурсам.|dependency/type, dependency/performanceBucket, dependency/success, operation/synthetic, cloud/roleInstance, cloud/roleName|
|dependencies/duration|Длительность зависимости|MilliSeconds|Average|Длительность вызовов, сделанных приложением во внешние ресурсы.|dependency/type, dependency/performanceBucket, dependency/success, operation/synthetic, cloud/roleInstance, cloud/roleName|
|dependencies/failed|Сбои при вызове зависимости|Count|Count|Число завершившихся сбоем вызовов зависимостей, сделанных приложением к внешним ресурсам.|dependency/type, dependency/performanceBucket, operation/synthetic, cloud/roleInstance, cloud/roleName|
|pageViews/count|Просмотры страниц|Count|Count|Число просмотров страницы.|operation/synthetic|
|pageViews/duration|Время загрузки страницы|MilliSeconds|Average|Время загрузки страницы|operation/synthetic|
|performanceCounters/requestExecutionTime|Время выполнения HTTP-запроса|MilliSeconds|Average|Время выполнения последнего запроса.|cloud/roleInstance|
|performanceCounters/requestsInQueue|HTTP-запросы в очереди приложений|Count|Average|Длина очереди запроса приложения.|cloud/roleInstance|
|performanceCounters/requestsPerSecond|Скорость HTTP-запроса|CountPerSecond|Average|Частота всех запросов в приложение в секунду из ASP.NET.|cloud/roleInstance|
|performanceCounters/exceptionsPerSecond|Частота исключений|CountPerSecond|Average|Счетчик обработанных и необработанных исключений, о которых сообщается в Windows, включая исключения .NET и неуправляемые исключения, которые преобразованы в исключения .NET.|cloud/roleInstance|
|performanceCounters/processIOBytesPerSecond|Скорость ввода-вывода процесса|BytesPerSecond|Average|Общее число прочитанных и записанных байт в секунду в файле, сети и на устройствах.|cloud/roleInstance|
...
|performanceCounters/memoryAvailableBytes|Доступная память|Байт|Average|Физическая память, доступная для немедленного выделения для процесса или использования системой.|cloud/roleInstance|
|performanceCounters/processPrivateBytes|Байты исключительного пользования процесса|Байт|Average|Память, исключительно назначенная отслеживаемым процессам приложения.|cloud/roleInstance|
|requests/duration|Время ответа сервера|MilliSeconds|Average|Время между получением HTTP-запроса и завершением отправки ответа.|request/performanceBucket, request/resultCode, operation/synthetic, cloud/roleInstance, request/success, cloud/roleName|
...
|requests/failed|Неудачные запросы|Count|Count|Число HTTP-запросов, помеченных как невыполненные. В большинстве случаев это запросы с кодами отклика >= 400, кроме 401.|request/performanceBucket, request/resultCode, operation/synthetic, cloud/roleInstance, cloud/roleName|
...
|exceptions/browser|Исключения браузера|Count|Count|Число неперехваченных исключений в браузере.|Нет измерений|
|exceptions/server|Исключения сервера|Count|Count|Число неперехваченных исключений, вызванных в приложении сервера.|cloud/roleName, cloud/roleInstance|
...
|IngestionResult|Ingestion Result (Результат приема данных)|Count|Count|Число операций приема данных|Сообщение о состоянии|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|RunsStarted|Runs Started|Count|Всего|Число запущенных запусков рабочих процессов.|Нет измерений|
|RunsCompleted|Runs Completed|Count|Всего|Число выполненных запусков рабочих процессов.|Нет измерений|
|RunsSucceeded|Runs Succeeded|Count|Всего|Число успешных запусков рабочих процессов.|Нет измерений|
|RunsFailed|Runs Failed|Count|Всего|Число неудачных запусков рабочих процессов.|Нет измерений|
|RunsCancelled|Выполнение отменено|Count|Всего|Число отмененных запусков рабочего процесса.|Нет измерений|
|RunLatency|Run Latency|Секунды|Average|Задержка выполненных запусков рабочих процессов.|Нет измерений|
|RunSuccessLatency|Run Success Latency|Секунды|Average|Задержка успешных запусков рабочих процессов.|Нет измерений|
|RunThrottledEvents|Run Throttled Events|Count|Всего|Число событий регулировки для действий рабочих процессов или триггеров.|Нет измерений|
|RunFailurePercentage|Run Failure Percentage|Percent|Всего|Процент неудачных запусков рабочих процессов.|Нет измерений|
|ActionsStarted|Actions Started |Count|Всего|Число запущенных действий рабочих процессов.|Нет измерений|
|ActionsCompleted|Actions Completed |Count|Всего|Число выполненных действий рабочих процессов.|Нет измерений|
|ActionsSucceeded|Actions Succeeded |Count|Всего|Число успешных действий рабочих процессов.|Нет измерений|
|ActionsFailed|Actions Failed|Count|Всего|Число неудачных действий рабочих процессов.|Нет измерений|
|ActionsSkipped|Actions Skipped |Count|Всего|Число пропущенных действий рабочих процессов.|Нет измерений|
|ActionLatency|Action Latency |Секунды|Average|Задержка выполненных действий рабочих процессов.|Нет измерений|
|ActionSuccessLatency|Action Success Latency |Секунды|Average|Задержка успешных действий рабочих процессов.|Нет измерений|
|ActionThrottledEvents|Action Throttled Events|Count|Всего|Число событий регулировки для действий рабочих процессов.|Нет измерений|
|TriggersStarted|Triggers Started |Count|Всего|Число запущенных триггеров рабочих процессов.|Нет измерений|
|TriggersCompleted|Triggers Completed |Count|Всего|Число выполненных триггеров рабочих процессов.|Нет измерений|
|TriggersSucceeded|Triggers Succeeded |Count|Всего|Число успешных триггеров рабочих процессов.|Нет измерений|
|TriggersFailed|Triggers Failed |Count|Всего|Число неудачных триггеров рабочих процессов.|Нет измерений|
|TriggersSkipped|Triggers Skipped|Count|Всего|Число пропущенных триггеров рабочих процессов.|Нет измерений|
|TriggersFired|Triggers Fired |Count|Всего|Число активированных триггеров рабочих процессов.|Нет измерений|
|TriggerLatency|Trigger Latency |Секунды|Average|Задержка выполненных триггеров рабочих процессов.|Нет измерений|
|TriggerFireLatency|Trigger Fire Latency |Секунды|Average|Задержка активированных триггеров рабочих процессов.|Нет измерений|
|TriggerSuccessLatency|Trigger Success Latency |Секунды|Average|Задержка успешных триггеров рабочих процессов.|Нет измерений|
|TriggerThrottledEvents|Trigger Throttled Events|Count|Всего|Число событий регулировки для триггеров рабочих процессов.|Нет измерений|
|BillableActionExecutions|Billable Action Executions|Count|Всего|Число выполнений действий рабочих процессов, за которые будет выставлен счет.|Нет измерений|
|BillableTriggerExecutions|Billable Trigger Executions|Count|Всего|Число выполнений триггеров рабочих процессов, за которые будет выставлен счет.|Нет измерений|
|TotalBillableExecutions|Total Billable Executions|Count|Всего|Число выполнений рабочих процессов, за которые будет выставлен счет.|Нет измерений|
|BillingUsageNativeOperation|Выставление счетов за внутренние операции|Count|Всего|Число внутренних операций, за которое выставляется счет.|Нет измерений|
|BillingUsageStandardConnector|Выставление счетов за операции стандартного соединителя|Count|Всего|Число операций стандартного соединителя, за которое выставляется счет.|Нет измерений|
|BillingUsageStorageConsumption|Выставление счетов за операции с использованием хранилища|Count|Всего|Число операций с использованием хранилища, за которое выставляется счет.|Нет измерений|
|BillingUsageNativeOperation|Выставление счетов за внутренние операции|Count|Всего|Число внутренних операций, за которое выставляется счет.|Нет измерений|
|BillingUsageStandardConnector|Выставление счетов за операции стандартного соединителя|Count|Всего|Число операций стандартного соединителя, за которое выставляется счет.|Нет измерений|
|BillingUsageStorageConsumption|Выставление счетов за операции с использованием хранилища|Count|Всего|Число операций с использованием хранилища, за которое выставляется счет.|Нет измерений|
...
|TriggerSuccessLatency|Trigger Success Latency |Секунды|Average|Задержка успешных триггеров рабочих процессов.|Нет измерений|
|TriggerThrottledEvents|Trigger Throttled Events|Count|Всего|Число событий регулировки для триггеров рабочих процессов.|Нет измерений|
|интегратионсервицеенвиронментворкфловпроцессорусаже|Загрузка процессора рабочего процесса для среды службы интеграции|Percent|Average|Загрузка процессора рабочего процесса для среды службы интеграции.|Нет измерений|
|интегратионсервицеенвиронментворкфловмеморюсаже|Использование памяти рабочего процесса для среды службы интеграции|Percent|Average|Использование памяти рабочего процесса для среды службы интеграции.|Нет измерений|
|интегратионсервицеенвиронментконнекторпроцессорусаже|Загрузка процессора соединителя для среды службы интеграции|Percent|Average|Загрузка процессора соединителя для среды службы интеграции.|Нет измерений|
|интегратионсервицеенвиронментконнектормеморюсаже|Использование памяти соединителя для среды службы интеграции|Percent|Average|Использование памяти соединителя для среды службы интеграции.|Нет измерений|
...
|TotalRequests|Всего запросов|Count|Всего|Число успешных запросов, обработанных шлюзом приложений|BackendSettingsPool|
...
|registration.create|Операции создания регистрации|Count|Всего|Число всех успешных регистраций.|Нет измерений|
|registration.update|Операции обновления регистрации|Count|Всего|Число всех успешных обновлений регистрации.|Нет измерений|
|registration.get|Операции чтения регистрации|Count|Всего|Число всех успешных запросов регистрации.|Нет измерений|
|registration.delete|Операции удаления регистрации|Count|Всего|Число всех успешных удалений регистрации.|Нет измерений|
...
|outgoing.allpns.success|Успешные уведомления|Count|Всего|Число всех успешных уведомлений.|Нет измерений|
|outgoing.allpns.invalidpayload|Ошибки полезных данных|Count|Всего|Число push-уведомлений со сбоем из-за того, что система PNS вернула ошибку неверной полезной нагрузки.|Нет измерений|
|outgoing.allpns.pnserror|Ошибки внешней системы уведомлений|Count|Всего|Число push-уведомлений со сбоем из-за ошибки взаимодействия с PNS (за исключением проблем с проверкой подлинности).|Нет измерений|
|outgoing.allpns.channelerror|Ошибки канала|Count|Всего|Число push-уведомлений со сбоем из-за того, что канал недопустим, связан не с тем регулируемым приложением или срок его действия истек.|Нет измерений|
|outgoing.allpns.badorexpiredchannel|Ошибки из-за поврежденного или просроченного канала|Count|Всего|Число push-уведомлений со сбоем из-за того, что канал, токен или идентификатор регистрации недопустим или срок его действия истек.|Нет измерений|
|outgoing.wns.success|Успешные уведомления WNS|Count|Всего|Число всех успешных уведомлений.|Нет измерений|
|outgoing.wns.invalidcredentials|Ошибки авторизации WNS (недопустимые учетные данные)|Count|Всего|Число push-уведомлений со сбоем из-за того, что система PNS не приняла указанные учетные данные или учетные данные заблокированы. (Windows Live не распознает учетные данные.)|Нет измерений|
...
|outgoing.wns.tokenproviderunreachable|Ошибки авторизации WNS (нет доступа)|Count|Всего|Служба Windows Live недоступна.|Нет измерений|
...
|outgoing.wns.dropped|Пропущенные уведомления WNS|Count|Всего|Уведомление было удалено, так как ChannelURI в регистрации отрегулирован (X-WNS-NotificationStatus: удалено, но не X-WNS-DeviceConnectionStatus: отключено).|Нет измерений|
|outgoing.wns.pnserror|Ошибки WNS|Count|Всего|Уведомление не доставлено из-за ошибок связи с WNS.|Нет измерений|
|outgoing.wns.authenticationerror|Ошибки проверки подлинности WNS|Count|Всего|Уведомление не доставлено из-за ошибок связи, вызванных недопустимыми учетными данными Windows Live или неправильным токеном.|Нет измерений|
|outgoing.apns.success|Успешные уведомления APNS|Count|Всего|Число всех успешных уведомлений.|Нет измерений|
|outgoing.apns.invalidcredentials|Ошибки авторизации APNS|Count|Всего|Число push-уведомлений со сбоем из-за того, что система PNS не приняла указанные учетные данные или учетные данные заблокированы.|Нет измерений|
...
|outgoing.apns.expiredchannel|Ошибка из-за просроченного канала APNS|Count|Всего|Число токенов, признанных недопустимыми каналом обратной связи APNS.|Нет измерений|
...
|outgoing.apns.pnserror|Ошибки APNS|Count|Всего|Число push-уведомлений со сбоем из-за ошибок связи с APNS.|Нет измерений|
|outgoing.gcm.success|Успешные уведомления GCM|Count|Всего|Число всех успешных уведомлений.|Нет измерений|
|outgoing.gcm.invalidcredentials|Ошибки авторизации GCM (недопустимые учетные данные)|Count|Всего|Число push-уведомлений со сбоем из-за того, что система PNS не приняла указанные учетные данные или учетные данные заблокированы.|Нет измерений|
...
|outgoing.gcm.pnserror|Ошибки GCM|Count|Всего|Число push-уведомлений со сбоем из-за ошибок связи с GCM.|Нет измерений|
...
|outgoing.mpns.success|Успешные уведомления MPNS|Count|Всего|Число всех успешных уведомлений.|Нет измерений|
|outgoing.mpns.invalidcredentials|Недопустимые учетные данные MPNS|Count|Всего|Число push-уведомлений со сбоем из-за того, что система PNS не приняла указанные учетные данные или учетные данные заблокированы.|Нет измерений|
...
|outgoing.mpns.invalidnotificationformat|Недопустимый формат уведомления MPNS|Count|Всего|Число push-уведомлений со сбоем из-за слишком высокой полезной нагрузки уведомления.|Нет измерений|
...
|outgoing.mpns.pnserror|Ошибки MPNS|Count|Всего|Число push-уведомлений со сбоем из-за ошибок связи с MPNS.|Нет измерений|
|outgoing.mpns.authenticationerror|Ошибки проверки подлинности MPNS|Count|Всего|Число push-уведомлений со сбоем из-за того, что система PNS не приняла указанные учетные данные или учетные данные заблокированы.|Нет измерений|
|notificationhub.pushes|Все исходящие уведомления|Count|Всего|Все исходящие уведомления в центре уведомлений|Нет измерений|
|incoming.all.requests|Все входящие запросы|Count|Всего|Всего входящих запросов для концентратора уведомлений|Нет измерений|
|Incoming.all.failedrequests|Все неудачные входящие запросы|Count|Всего|Всего невыполненных входящих запросов для концентратора уведомлений|Нет измерений|
...
|Average_Disk Write Bytes/sec|Скорость записи (байт/с)|Count|Average|Average_Disk Write Bytes/sec|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Average_Bytes Received/sec|Получено байт/с|Count|Average|Average_Bytes Received/sec|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
|Average_Bytes Sent/sec|Отправлено байт/с|Count|Average|Average_Bytes Sent/sec|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|ListenerConnections — выполненные|ListenerConnections — выполненные|Count|Всего|Выполненные ListenerConnections для Microsoft.Relay.|EntityName|
...
|SenderConnections — выполненные|SenderConnections — выполненные|Count|Всего|Выполненные SenderConnections для Microsoft.Relay.|EntityName|
...
|ServerErrors|Ошибки на сервере. (Предварительная версия)|Count|Всего|Ошибки на сервере для Microsoft.ServiceBus. (Предварительная версия)|EntityName|
...
|деадлеттередмессажес|Число недоставленных сообщений в очереди или разделе. (Предварительная версия)|Count|Average|Число недоставленных сообщений в очереди или разделе. (Предварительная версия)|EntityName|
...
|CPUXNS|CPU usage per namespace|Percent|Максимум|Показатели загрузки ЦП пространством имен категории "Премиум" служебной шины|Нет измерений|
|WSXNS|Memory size usage per namespace|Percent|Максимум|Показатели использования памяти пространством имен категории "Премиум" служебной шины|Нет измерений|
...
|dtu_consumption_percent|Процент DTU|Percent|Average|Процент DTU|Нет измерений|
...
|взаимоблокировка|Взаимоблокировки|Count|Всего|Взаимоблокировки|Нет измерений|
|storage_percent|Процент использования пространства данных|Percent|Максимум|Размер базы данных в процентах|Нет измерений|
...
|cpu_limit|Ограничение ЦП|Count|Average|Ограничение ЦП|Нет измерений|
|cpu_used|Используемый ЦП|Count|Average|Используемый ЦП|Нет измерений|
|dwu_limit|Лимит DWU|Count|Максимум|Лимит DWU|Нет измерений|
|dwu_consumption_percent|DWU percentage|Percent|Максимум|DWU percentage|Нет измерений|
|dwu_used|DWU used|Count|Максимум|DWU used|Нет измерений|
...
|dw_physical_data_read_percent|Процент операций ввода-вывода данных на уровне узла DW|Percent|Average|Процент операций ввода-вывода данных на уровне узла DW|DwLogicalNodeId|
    |cache_hit_percent|Cache hit percentage (Процент попаданий в кэш)|Percent|Максимум|Cache hit percentage (Процент попаданий в кэш)|Нет измерений|
|cache_used_percent|Cache used percentage (Процент использования кэша)|Percent|Максимум|Cache used percentage (Процент использования кэша)|Нет измерений|
...
|app_cpu_billed|Выставление счетов за использование ЦП приложениями|Count|Всего|Выставление счетов за использование ЦП приложениями|Нет измерений|
|app_cpu_percent|Процент загрузки ЦП приложения|Percent|Average|Процент загрузки ЦП приложения|Нет измерений|
...
|Исходящий трафик|Исходящий трафик|Байт|Всего|Объем входящих данных в байтах. Это количество включает исходящий трафик из внешнего клиента в службу хранилища Azure, а также исходящий трафик в пределах Azure. В результате это количество не отражает исходящий трафик для тарификации.|GeoType, ApiName, Authentication|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|BlobCapacity|Емкость BLOB-объектов|Байт|Average|Объем используемой службой BLOB-объектов учетной записи хранения памяти (в байтах).|BlobType, уровень|
|BlobCount|Число BLOB-объектов|Count|Всего|Число BLOB-объектов в службе BLOB-объектов учетной записи хранения.|BlobType|       |BlobCount|Число BLOB-объектов|Count|Average|Число BLOB-объектов в службе BLOB-объектов учетной записи хранения.|BlobType, уровень|
|ContainerCount|Число контейнеров BLOB-объектов|Count|Average|Число контейнеров в службе BLOB-объектов учетной записи хранения.|Нет измерений|
|IndexCapacity|Емкость индекса|Байт|Average|Объем хранилища, используемый для индекса ADLS 2-го поколения (иерархического), в байтах.|Нет измерений|
|Транзакции|Транзакции|Count|Всего|Количество запросов к службе хранения или указанной операции API. Это число включает успешные и неудачные запросы, а также запросы, вызвавшие ошибку. Используйте измерение ResponseType для различных типов ответа.|ResponseType, GeoType, ApiName, Authentication|
|Входящий трафик|Входящий трафик|Байт|Всего|Объем входящих данных в байтах. Это количество включает входящий трафик из внешнего клиента в службу хранилища Azure, а также входящий трафик в пределах Azure.|GeoType, ApiName, Authentication|
|Исходящий трафик|Исходящий трафик|Байт|Всего|Объем входящих данных в байтах. Это количество включает исходящий трафик из внешнего клиента в службу хранилища Azure, а также исходящий трафик в пределах Azure. В результате это количество не отражает исходящий трафик для тарификации.|GeoType, ApiName, Authentication|
|SuccessServerLatency|Серверная задержка успешного запроса|мс|Average|Средняя задержка, используемая службой хранилища Azure для обработки успешного запроса, в миллисекундах. Это значение не включает сетевую задержку, указанную в метрике AverageE2ELatency.|GeoType, ApiName, Authentication|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|FileCapacity|Емкость файлов|Байт|Average|Объем используемой службой файлов учетной записи хранения памяти (в байтах).|Нет измерений|
|FileCount|Количество файлов|Count|Average|Число файлов в службе файлов учетной записи хранения.|Нет измерений|
|FileShareCount|Число общих папок|Count|Average|Число общих папок в службе файлов учетной записи хранения.|Нет измерений|
|Транзакции|Транзакции|Count|Всего|Количество запросов к службе хранения или указанной операции API. Это число включает успешные и неудачные запросы, а также запросы, вызвавшие ошибку. Используйте измерение ResponseType для различных типов ответа.|ResponseType, GeoType, ApiName, Authentication|
|Входящий трафик|Входящий трафик|Байт|Всего|Объем входящих данных в байтах. Это количество включает входящий трафик из внешнего клиента в службу хранилища Azure, а также входящий трафик в пределах Azure.|GeoType, ApiName, Authentication|
|Исходящий трафик|Исходящий трафик|Байт|Всего|Объем входящих данных в байтах. Это количество включает исходящий трафик из внешнего клиента в службу хранилища Azure, а также исходящий трафик в пределах Azure. В результате это количество не отражает исходящий трафик для тарификации.|GeoType, ApiName, Authentication|
|SuccessServerLatency|Серверная задержка успешного запроса|мс|Average|Средняя задержка, используемая службой хранилища Azure для обработки успешного запроса, в миллисекундах. Это значение не включает сетевую задержку, указанную в метрике AverageE2ELatency.|GeoType, ApiName, Authentication|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|QueueCapacity|Емкость очереди|Байт|Average|Объем используемого службой очередей учетной записи хранения хранилища (в байтах).|Нет измерений|
|QueueCount|Счетчик очередей|Count|Average|Число очередей в службе очередей учетной записи хранения.|Нет измерений|
|QueueMessageCount|Число сообщений в очереди|Count|Average|Приблизительное число сообщений очереди в службе очередей учетной записи хранения.|Нет измерений|
|Транзакции|Транзакции|Count|Всего|Количество запросов к службе хранения или указанной операции API. Это число включает успешные и неудачные запросы, а также запросы, вызвавшие ошибку. Используйте измерение ResponseType для различных типов ответа.|ResponseType, GeoType, ApiName, Authentication|
|Входящий трафик|Входящий трафик|Байт|Всего|Объем входящих данных в байтах. Это количество включает входящий трафик из внешнего клиента в службу хранилища Azure, а также входящий трафик в пределах Azure.|GeoType, ApiName, Authentication|
|Исходящий трафик|Исходящий трафик|Байт|Всего|Объем входящих данных в байтах. Это количество включает исходящий трафик из внешнего клиента в службу хранилища Azure, а также исходящий трафик в пределах Azure. В результате это количество не отражает исходящий трафик для тарификации.|GeoType, ApiName, Authentication|
|SuccessServerLatency|Серверная задержка успешного запроса|мс|Average|Средняя задержка, используемая службой хранилища Azure для обработки успешного запроса, в миллисекундах. Это значение не включает сетевую задержку, указанную в метрике AverageE2ELatency.|GeoType, ApiName, Authentication|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|TableCapacity|Емкость таблицы|Байт|Average|Объем используемого службой таблиц учетной записи хранения хранилища (в байтах).|Нет измерений|
|TableCount|Число таблиц|Count|Average|Число таблиц в службе таблиц учетной записи хранения.|Нет измерений|
|TableEntityCount|Число сущностей таблиц|Count|Average|Число сущностей таблиц в службе таблиц учетной записи хранения.|Нет измерений|
|Транзакции|Транзакции|Count|Всего|Количество запросов к службе хранения или указанной операции API. Это число включает успешные и неудачные запросы, а также запросы, вызвавшие ошибку. Используйте измерение ResponseType для различных типов ответа.|ResponseType, GeoType, ApiName, Authentication|
|Входящий трафик|Входящий трафик|Байт|Всего|Объем входящих данных в байтах. Это количество включает входящий трафик из внешнего клиента в службу хранилища Azure, а также входящий трафик в пределах Azure.|GeoType, ApiName, Authentication|
|Исходящий трафик|Исходящий трафик|Байт|Всего|Объем входящих данных в байтах. Это количество включает исходящий трафик из внешнего клиента в службу хранилища Azure, а также исходящий трафик в пределах Azure. В результате это количество не отражает исходящий трафик для тарификации.|GeoType, ApiName, Authentication|
|SuccessServerLatency|Серверная задержка успешного запроса|мс|Average|Средняя задержка, используемая службой хранилища Azure для обработки успешного запроса, в миллисекундах. Это значение не включает сетевую задержку, указанную в метрике AverageE2ELatency.|GeoType, ApiName, Authentication|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|ResourceUtilization|Использование единиц потоковой передачи (%)|Percent|Максимум|Использование SU (%)|LogicalName, PartitionId|
...
|Метрика|Отображаемое имя метрики|Единица измерения|Тип статистической обработки|Описание|Размеры|
|---|---|---|---|---|---|
|дискреадбитесперсеконд|Скорость чтения с диска (байт/с)|BytesPerSecond|Average|Средняя пропускная способность диска из-за операций чтения за период выборки.|Нет измерений|
|дисквритебитесперсеконд|Скорость записи на диск (байт/с)|BytesPerSecond|Average|Средняя пропускная способность диска из-за операций записи за период выборки.|Нет измерений|
...
|Операций чтения с диска/с|Чтение с дисков (операций/с)|CountPerSecond|Average|Среднее число операций чтения ввода-вывода в предыдущем периоде выборки. Обратите внимание, что эти операции могут иметь переменный размер.|Нет измерений|
|Операций записи на диск/с|Запись на диски (операций/с)|CountPerSecond|Average|Среднее число операций записи ввода-вывода в предыдущем периоде выборки. Обратите внимание, что эти операции могут иметь переменный размер.|Нет измерений|
