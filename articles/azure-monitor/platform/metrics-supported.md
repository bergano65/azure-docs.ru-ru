...
Эта статья представляет собой полный список всех метрик платформы (т.е. автоматически собранных) метрик, доступных в настоящее время в консолидированном метрическом конвейере Azure Monitor. Последний раз список обновлялся 27 марта 2020 года. Метели изменены или добавлены после этой даты не могут отображаться ниже. Чтобы запросить и получить доступ к списку метрик программно, пожалуйста, используйте [апи-версию 2018-01-01](https://docs.microsoft.com/rest/api/monitor/metricdefinitions)

Другие метрики могут быть доступны на портале или при использовании интерфейсов API прежних версий. Метрики для гостевой операционной системы (гостевой os), которая работает в Azure Virtual Machines, Service Fabric и Облачных службах, **не** перечислены здесь. Они должны быть собраны через одного или нескольких агентов, которые работают на или как часть операционной системы. Вы можете отправить метрики агента в базу данных метрик платформы, используя [пользовательские метрики](metrics-custom-overview.md) API, которые в настоящее время находятся в открытом предварительном просмотре. Затем вы можете наметить, оповещение и иным образом использовать гостевые показатели os, такие как метрики платформы. Для получения дополнительной [Monitoring Agents Overview](agents-overview.md)информации см.    

Метрики организованы по пространству имен. Список служб и зон имен, принадлежащих им, можно найти [в ресурсах для служб Azure.](../../azure-resource-manager/management/azure-services-resource-providers.md) 
...
|sqlserver_process_core_percent<sup>1</sup> |Основные проценты основных процессов сервера Сервера|Процент|Максимальная|Процент использования процессора для процесса S'L Server, измеряемый операционной системой.|None|
|sqlserver_process_memory_percent<sup>1</sup> |Процент памяти процесса обработки серверов S'L|Процент|Максимальная|Процент использования памяти для процесса S'L Server, измеряемый операционной системой.|None|
|tempdb_data_size<sup>2</sup> |Темпб данных файл размер килобайт|Count|Максимальная|Темп файла данных Размер килобайт.|None|
|tempdb_log_size<sup>2</sup> |Tempdb журнал файл размер килобайт|Count|Максимальная|Tempdb журнал файл размер килобайт.|None|
|tempdb_log_used_percent<sup>2</sup> |Tempdb Процент Журнала Используется|Процент|Максимальная|Tempdb Процент Журнала Используется.|None|
...
|sqlserver_process_core_percent<sup>1</sup>|Основные проценты основных процессов сервера Сервера|Процент|Максимальная|Процент использования процессора для процесса S'L Server, измеряемый операционной системой. Применяется к эластичным бассейнам. |None|
|sqlserver_process_memory_percent<sup>1</sup>|Процент памяти процесса обработки серверов S'L|Процент|Максимальная|Процент использования памяти для процесса S'L Server, измеряемый операционной системой. Применяется к эластичным бассейнам. |None|
|tempdb_data_size<sup>2</sup>|Темпб данных файл размер килобайт|Count|Максимальная|Темп файла данных Размер килобайт.|None|
|tempdb_log_size<sup>2</sup>|Tempdb журнал файл размер килобайт|Count|Максимальная|Tempdb журнал файл размер килобайт. |None|
|tempdb_log_used_percent<sup>2</sup>|Tempdb Процент Журнала Используется|Процент|Максимальная|Tempdb Процент Журнала Используется.|None|
