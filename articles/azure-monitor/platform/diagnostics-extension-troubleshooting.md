...
| **Файлы журнала** | C:\Logs\Plugins\Microsoft.Azure.Diagnostics.PaaSDiagnostics\<версия>\ |
...
| **Пакет расширения Azure Diagnostics** | %SystemDrive%\Packages\Plugins\Microsoft.Azure.Diagnostics.PaaSDiagnostics\<версия> |
...
| **Пакет расширения Azure Diagnostics** | C:\Packages\Plugins\Microsoft.Azure.Diagnostics.IaaSDiagnostics\<версия_системы_диагностики>|
...
Если нет данных по конкретной метрике, проверьте, **Diagnostics Configuration** > **PerformanceCounter** включена ли метрика (счетчик производительности). Следующие счетчики включены по умолчанию.
...
- \.NET CLR Память\% _(Глобальный_) Время в GC
...
## <a name="azure-diagnostics-is-not-starting"></a>Azure Диагностика не начинается
...
Если учетная запись хранения настроена правильно, удаленный доступ в машину и убедитесь, что *DiagnosticsPlugin.exe* и *MonAgentCore.exe* работают. Если они не работают, следуйте за шагами в [Azure Diagnostics не запускается.](#azure-diagnostics-is-not-starting)
...
1. Удаление агента
2. Удалить каталог C: »WindowsAzure-Logs-Plugins-Microsoft.Azure.Diagnostics.IaaSДиагностика
3. Установка агента снова
...
| Событие | Имя таблицы |
| --- | --- |
| provider'"prov1" &lt;Событие id'"1" /&gt; |ВАДВЕнт-МД5 ("prov1") |
| provider'"prov1" &lt;Событие id'"2" eventDestination'"dest1" /&gt; |WADdest1 |
...
- Убедитесь, что **DiagnosticMonitorTraceListener** настроен в web.config или app.config.  Это настроено по умолчанию в проектах облачных сервисов. но некоторые клиенты могут закомментировать его, в результате чего система диагностики не собирает трассировочные операторы.

- Если журналы не записываются с метода **OnStart** или **Run,** убедитесь, что **DiagnosticMonitorTraceListener** находится в app.config.  По умолчанию он находится в web.config, но это относится только к коду работает в рамках w3wp.exe. Поэтому этот класс также нужно определить в файле app.config, чтобы собирать трассировки, выполненные в средстве WaIISHost.exe.
...
- Убедитесь, что компилированный код на самом деле имеет **линии Diagnostics.Trace** (использовать Reflector, ildasm или ILSpy для проверки). **Команды Diagnostics.Trace** удаляются из компилированного двоичного файла, если только вы не используете условный символ компиляции TRACE. Если для сборки проекта используется msbuild, это обычная проблема.   
...
**2. Данные счетчиков производительности доступны в хранилище, но не отображаться на портале**
...
- Указаны ли имена счетчиков в хранилище на английском. В противном случае диаграмма метрик на портале не сможет распознать их. **Предотвращение.** Изменить язык системных учетных записей виртуальной машины на английский. Для этого выберите параметры**административной** > **копирования** **Контрольной панели** > **региона.** >  Далее отмените выделение параметра **Экран приветствия и системные учетные записи**, чтобы пользовательский язык не применялся к системной учетной записи.

- Если вы используете подстановочные знаки (\*) в именах счетчиков производительности, портал не сможет отслеживать настроенные и собранные показатели счетчика, отправляемые в приемник службы хранилища Azure. **Смягчение**: Чтобы убедиться, что вы можете использовать\*подстановочные знаки и иметь портал расширить ( , маршрут ваши счетчики производительности к раковине Azure Monitor.
