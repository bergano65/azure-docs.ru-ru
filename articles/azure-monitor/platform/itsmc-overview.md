...
Узнайте больше о [юридических терминах и политике конфиденциальности.](https://go.microsoft.com/fwLink/?LinkID=522330&clcid=0x9)
...
3. В разделе **Рабочая область OMS** выберите рабочую область Azure Log Analytics, где нужно установить решение.
   ...
   > * Соединение ITSM может быть установлено только в рабочих пространствах Log Analytics в следующих регионах: Восточная ЧАСТЬ США, Западная US2, South Central US, West Central US, Центральная Канада, Западная Европа, южная Великобритания, юго-восточная Азия, Восточная Япония, Центральная Индия, юго-восточная Австралия.
...
При развертывании ресурса решения в правом верхнем правом верхнем окне появляется уведомление.
...
В зависимости от подключаемого к продукту ITSM, используйте следующие действия:
...
Служба схемы услуги автоматически обнаруживает компоненты приложений в системах Windows и Linux и сопоставляет взаимодействие между службами. Это решение позволяет рассматривать серверы как взаимосвязанные системы, предоставляющие важные службы. Схема услуги отображает сведения о подключениях между серверами, процессами и портами в любой подключенной по протоколу TCP архитектуре без дополнительной настройки. Пользователям требуется только установить агент. Ознакомьтесь с [дополнительными сведениями](../../azure-monitor/insights/service-map.md).
...
1. Если сбой подключения происходит из пользовательского интерфейса подключенного источника с сообщением **Ошибка при сохранении подключения**, сделайте следующее:
   ...
   - убедитесь, что веб-приложение успешно развернуто и создано гибридное подключение. Чтобы проверить, что подключение успешно установлено с помощью машины менеджера службы, посетите URL-адрес веб-приложения, как подробно описано в документации для создания [гибридного соединения.](../../azure-monitor/platform/itsmc-connections.md#configure-the-hybrid-connection)  
...
3. Если поступают оповещения Log Analytics, но рабочие элементы не создаются в продукте ITSM либо элементы конфигурации не создаются или не связываются с рабочими элементами или же вам нужны другие сведения общего характера, просмотрите следующие компоненты:
   -  ITSMC: Решение показывает резюме соединений / рабочих элементов / компьютеров и т.д. Нажмите на плитку, показывающую **статус соединителя**, который приведет вас к **поиску журнала** с соответствующим запросом. Чтобы получить дополнительные сведения, просмотрите записи журнала, где для параметра ERROR задано значение LogType_S.
...
Для получения любых запросов или отзывов на итоговом коннектору управления иТ-сервисами свяжитесь с нами по телефону [omsitsmfeedback@microsoft.com](mailto:omsitsmfeedback@microsoft.com).
