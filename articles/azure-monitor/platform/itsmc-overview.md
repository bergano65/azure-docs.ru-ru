...
3. В разделе **Рабочая область OMS** выберите рабочую область Azure Log Analytics, где нужно установить решение.
   ...
   > * Соединитель ITSM можно установить только в Log Analytics рабочих областях в следующих регионах: Восточная часть США, Западная Европа, Юго-Восточная Азия, Юго-Восточная Австралия, Юго-Центральный регион США, Восточная Япония, Южная часть Соединенного Королевства, Центральная Индия, Центральная Канада.
...
Служба схемы услуги автоматически обнаруживает компоненты приложений в системах Windows и Linux и сопоставляет взаимодействие между службами. Это решение позволяет рассматривать серверы как взаимосвязанные системы, предоставляющие важные службы. Схема услуги отображает сведения о подключениях между серверами, процессами и портами в любой подключенной по протоколу TCP архитектуре без дополнительной настройки. Пользователям требуется только установить агент. [Дополнительные сведения](../../azure-monitor/insights/service-map.md)
...
2.  Если во время выполнения **сценария** отображается ошибка [Ссылка на объект не указывает на экземпляр объекта](itsmc-service-manager-script.md), проверьте значения в разделе **Конфигурация пользователя**.
