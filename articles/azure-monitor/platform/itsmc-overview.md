...
3. В разделе **Рабочая область OMS** выберите рабочую область Azure Log Analytics, где нужно установить решение.
   ...
   > * Соединитель ITSM можно установить только в Log Analytics рабочих областях в следующих регионах: Восточная часть США, Западная США 2, Юго-Центральный регион США, Западная Центральная часть США, US Gov (Аризона), US Gov (Вирджиния), Центральная Канада, Западная Европа, Южная Великобритания, Юго-Восточная Азия, Восточная Япония, Центральная Индия, Юго-Восточная Австралия.
...
1. В портал Azure щелкните  **оповещения** .
2. В верхней области щелкните  **Управление действиями** . Откроется окно **Добавить группу действий** .

    [![Группы действий](media/itsmc-overview/action-groups-selection.png)](media/itsmc-overview/action-groups-selection-big.png)

3. Выберите **подписку** и **группу ресурсов** , в которых нужно создать группу действий. Укажите **имя группы действий** и **Отображаемое имя** для группы действий. Щелкните **Далее: уведомления** .

    ![Сведения о группах действий](media/itsmc-overview/action-groups-details.png)

4. В списке уведомлений щелкните **вложения: действия** .
5. В списке действий выберите **ITSM** из раскрывающегося меню **Тип действия** . Укажите **имя** действия и щелкните перо, представляющее **сведения об изменении** .
6. Выберите **подписку** , в которой расположена рабочая область Log Analytics. Выберите имя **подключения** (имя соединителя ITSM), за которым следует имя рабочей области. Например, MyITSMMConnector(MyWorkspace).

    ![Сведения о действиях ITSM](media/itsmc-overview/itsm-action-configuration.png)

7. Выберите тип **рабочего элемента** из раскрывающегося меню.
...
9. Установив флажок **создать отдельные рабочие элементы для каждого элемента конфигурации** , у каждого элемента конфигурации будет свой собственный рабочий элемент. Это означает, что для каждого элемента конфигурации будет существовать один рабочий элемент, который будет обновлен в соответствии с создаваемыми оповещениями.
Если отменить выбор флажка **создать отдельные рабочие элементы для каждого элемента конфигурации** каждое предупреждение создаст новый рабочий элемент, то есть для каждого элемента конфигурации может быть создано более одного предупреждения.
...
![Снимок экрана, на котором показана панель мониторинга соединитель ITSM.](media/itsmc-overview/itsmc-overview-sample-log-analytics.png)
