...
При создании правила генерации оповещений метрика имя метрики проверяется с помощью [API определений метрик](https://docs.microsoft.com/rest/api/monitor/metricdefinitions/list) , чтобы убедиться, что он существует. В некоторых случаях вы хотите создать правило генерации оповещений для пользовательской метрики, даже прежде чем она будет выдаваться. Например, при создании (с использованием шаблона ARM) Application Insights ресурс, который будет выдавать пользовательскую метрику, а также правило генерации оповещений, отслеживающее эту метрику.

Чтобы избежать сбоя развертывания при попытке проверить определения пользовательской метрики, можно использовать параметр *скипметриквалидатион* в разделе критериев правила оповещения, что приведет к пропуску проверки метрики. См. пример ниже, чтобы использовать этот параметр в шаблоне ARM (для получения подробных примеров шаблонов ARM для создания правил генерации оповещений метрик см. [здесь]( https://docs.microsoft.com/azure/azure-monitor/platform/alerts-metric-create-templates)).
