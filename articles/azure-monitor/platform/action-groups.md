...
1. Включите группы действий для использования приложения Azure AD.
    ...
    - Измените переменную $myAzureADApplicationObjectId в сценарии PowerShell, чтобы использовать свой идентификатор объекта приложения Azure AD.
...
Повторные вызовы веб-перехватчиков выполняются по следующим правилам. Вызов веб-перехватчика повторяется (но не более 2 раз), если получены такие коды состояния HTTP: 408, 429, 503, 504 (или если конечная точка HTTP не отвечает). Первый повторный вызов происходит через 10 секунд, второй — через 100 секунд. После двух сбоев никакая группа действий не будет вызывать конечную точку в течение 30 минут. 
