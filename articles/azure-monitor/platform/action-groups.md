...
* **Тип действия**: выполняемое действие. Примеры включают отправку голосового звонка, текстовое сообщение, сообщение электронной почты или активацию различных типов автоматических действий. Типы приведены далее в этой статье.
...
1. В [портал Azure](https://portal.azure.com)найдите и выберите **Monitor (монитор**). Панель **монитор** объединяет все параметры мониторинга и данные в одном представлении.
...
1. Введите имя в поле **имя группы действий** и введите имя в поле **Краткое имя** . Короткое имя используется вместо полного имени группы действий при отправке уведомлений с помощью этой группы.

      ![Диалоговое окно Add action group (Добавить группу действий)](./media/action-groups/action-group-define.png)

1. Поле " **Подписка** " автоматически заполняется текущей подпиской. В этой подписке сохраняются группы действий.

1. Выберите **группу ресурсов** , в которой будет сохранена группа действий.
...
1. Определите список действий. Для каждого действия укажите следующие сведения.
    ...
    1. **Тип действия**: выберите модуль Runbook службы автоматизации, функция Azure, электронная почта Azure Resource Manager роль, электронная почта, SMS, Push/Voice, ITSM, приложение логики, безопасный веб-перехватчик, веб-перехватчик.
    ...
    1. **Общая схема предупреждений**. Вы можете включить [общую схему предупреждений](https://aka.ms/commonAlertSchemaDocs), которая предоставляет преимущества единого расширяемого и Объединенного полезных данных оповещений во всех службах предупреждений в Azure Monitor.

1. Нажмите кнопку **ОК**, чтобы создать группу действий.

## <a name="manage-your-action-groups"></a>Управление группами действий

После создания группы действий можно просмотреть **группы действий** , выбрав **Управление действиями** на странице " **оповещения** " на панели " **монитор** ". Выберите группу действий, которой необходимо управлять:
...
Отправьте сообщение электронной почты членам роли подписки. Сообщение электронной почты будет отправлено только членам роли **пользователя Azure AD** . Электронная почта не будет отправляться в группы или субъекты-службы Azure AD.
...
> [!NOTE]
> Если пользовательский интерфейс группы действий портал Azure не позволяет выбрать код страны, то SMS не поддерживается в вашей стране.  Если код страны недоступен, можно проголосовать за добавление вашей страны в [Пользовательский Голос](https://feedback.azure.com/forums/913690-azure-monitor/suggestions/36663181-add-more-country-codes-for-sms-alerting-and-voice). В то же время, обойти это, чтобы Группа действий вызывала веб-перехватчик стороннего поставщика SMS с поддержкой в вашей стране.  

Цены на поддерживаемые страны перечислены на [странице цен на Azure Monitor](https://azure.microsoft.com/pricing/details/monitor/).
  

### <a name="voice"></a>Голосовая связь
Дополнительные важные действия см. в статье [сведения о ограничении скорости](./../../azure-monitor/platform/alerts-rate-limiting.md) .

В группе действий может быть ограниченное количество голосовых действий.

> [!NOTE]
> Если пользовательский интерфейс группы действий портал Azure не позволяет выбрать код страны, то голосовой звонок не будет поддерживаться в вашей стране. Если код страны недоступен, можно проголосовать за добавление вашей страны в [Пользовательский Голос](https://feedback.azure.com/forums/913690-azure-monitor/suggestions/36663181-add-more-country-codes-for-sms-alerting-and-voice).  В то же время, обойти это, чтобы Группа действий вызывала веб-перехватчик стороннего поставщика услуг передачи голоса с поддержкой в вашей стране.  
...
* Дополнительные сведения о [ограничении скорости](../../azure-monitor/platform/alerts-rate-limiting.md) для оповещений.
