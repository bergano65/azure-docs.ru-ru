...
**Веб-перехватчик.** Можно иметь не более 10 действий веб-перехватчика в группе действий.
Логика повторных попыток: период ожидания ответа — 10 секунд. Вызов веб-перехватчика будет повторен (но не более 2 раз), если будут получены такие коды состояния HTTP: 408, 429, 503, 504 (или если конечная точка HTTP не отвечает). Первый повторный вызов происходит через 10 секунд, второй — через 100 секунд. После двух отказов вызовы к конечной точке не будут отправляться из любой группы действий в течение 30 минут.
