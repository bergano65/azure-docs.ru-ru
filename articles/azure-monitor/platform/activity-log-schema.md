...
| Description (Описание) |Статическое текстовое описание события оповещения. |
...
| correlationId | Глобальный уникальный идентификатор (GUID) в строковом формате. |
| Description (Описание) |Статическое текстовое описание события автомасштабирования. |
...
| correlationId | Глобальный уникальный идентификатор (GUID) в строковом формате. |
| Description (Описание) |Статическое текстовое описание события безопасности. |
...
| каналов | Всегда значение Operation. |
| correlationId | Глобальный уникальный идентификатор (GUID) в строковом формате. |
| Description (Описание) |Статическое текстовое описание события рекомендации |
...
| claims | Токен JWT, который используется Active Directory для аутентификации пользователя или приложения при выполнении этой операции в Resource Manager. |
| correlationId | Обычно GUID в строковом формате. События, которые совместно используют идентификатор correlationId, принадлежат к одному общему действию. |
| Description (Описание) | Это поле будет пустым для событий Политики. |
| eventDataId | Уникальный идентификатор события. |
...
## <a name="mapping-to-resource-logs-schema"></a>Сопоставление со схемой журналов ресурсов

При потоковой передаче журнала действий Azure в учетную запись хранения или пространство имен концентраторов событий данные следуют за [схемой журналов ресурсов Azure](./diagnostic-logs-schema.md). Ниже приведено сопоставление свойств схемы, приведенной выше, к схеме журналов ресурсов.

| Свойство схемы журналов ресурсов | Свойство схемы журнала действий REST API | Заметки |
| --- | --- | --- |
| time | eventTimestamp |  |
...
| resultDescription | Description (Описание) |  |
