...
Эта категория содержит записи всех событий, связанных с работоспособностью службы, которые произошли с ресурсами Azure. Пример события такого типа из этой категории может указывать на то, что состояние работоспособности виртуальной машины изменилось на "Отключено". События работоспособности ресурсов соответствуют одному из четырех состояний: Available (Доступно), Unavailable (Недоступно), Degraded (Понижено) и Unknown (Неизвестно). Кроме того, события работоспособности ресурсов можно классифицировать по свойствам PlatformInitiated (Инициировано платформой) и UserInitiated (Инициировано пользователем).
