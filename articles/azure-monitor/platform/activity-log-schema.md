...
description: Описывает схему событий для каждой категории в журнале деятельности Azure.
author: bwren
...
ms.locfileid: "79472747"
---
# <a name="azure-activity-log-event-schema"></a>Схема событий журнала действий Azure
[В журнале Деятельности Azure](platform-logs-overview.md) содержится информация о любых событиях уровня подписки, произошедших в Azure. В этой статье описывается схема события для каждой категории. 

Приведенные ниже примеры показывают схему при доступе к журналу активности с портала, PowerShell, CLI и REST API. Схема отличается при [потоковой передаче журнала activity в хранилище или концентраторы событий.](resource-logs-stream-event-hubs.md) В конце статьи обеспечивается отображение свойств в [схеме журналов ресурсов.](diagnostic-logs-schema.md)
...
| eventName | Дружественное название Административного мероприятия. |
...
| тип_ресурса | Тип ресурса, на который повлияло административное событие. |
...
Эта категория содержит запись о любых новых рекомендациях, которые создаются для служб. Примером рекомендации будет "использование группы доступности для повышения отказоустойчивости". Можно сгенерировать четыре типа событий Рекомендации: высокая доступность, производительность, безопасность и оптимизация затрат. 
...
Эта категория содержит записи всех операций действия эффекта, выполняемых [Политикой Azure](../../governance/policy/overview.md). Примеры типов событий, которые вы увидите в этой категории, включают _аудит_ и _отрицание_. Каждое действие, выполняемое Политикой, моделируется как операция для ресурса.
...
При потоковой передаче журнала Azure Activity в учетную запись хранилища или концентратор событий данные следуют за [схемой журнала ресурсов.](diagnostic-logs-schema.md) В приведенной ниже таблице представлено отображение свойств от вышеуказанной схемы для журналов ресурсов.

> [!IMPORTANT]
> Формат данных журнала Activity, записанных на учетную запись хранения, изменен на JSON Lines 1 ноября 2018 года. [Просрнируйте для изменения формата журналы ресурсов Azure Monitor, архивированные в учетную запись хранения](diagnostic-logs-append-blobs.md) для сведения об этом изменении формата.
...
| Level | Level |  |
| location | Недоступно | Расположение, где было обработано событие. *Это не расположение ресурса, а место обработки события. Это свойство будет удалено в будущем обновлении.* |
...
Ниже приводится пример события с использованием этой схемы.
...
* [Создание диагностического параметра для отправки журнала активности в рабочее пространство журнала Analytics, хранилище Azure или концентраторы событий](diagnostic-settings.md)
