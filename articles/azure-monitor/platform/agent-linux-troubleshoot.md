...
description: Описание симптомов, причин и решений для наиболее распространенных проблем с агентом Log Analytics для Linux.
...
Эта статья содержит информацию по распространенным ошибкам, которые могут возникнуть в работе агента Log Analytics для Linux, и возможные решения для их устранения.
...
| 55 | Неподдерживаемая версия openssl, ИЛИ не удается подключиться к службе Log Analytics, ИЛИ заблокирован dpkg, ИЛИ отсутствует программа curl. | |
...
| 5 | Получена ошибка HTTP 403 от службы Log Analytics. Изучите выходные данные скрипта omsadmin, чтобы получить подробные сведения. |
| 6 | От службы Log Analytics получен статус HTTP, отличный от кода 200. Изучите выходные данные скрипта omsadmin, чтобы получить подробные сведения. |
| 7 | Не удалось подключиться к службе Log Analytics. Изучите выходные данные скрипта omsadmin, чтобы получить подробные сведения. |
...
Ведение журнала отладки позволяет просматривать групповые отправки данных в службу Log Analytics с разбивкой по типу, количеству элементов данных и времени, затраченному на отправку.
...
1. Просмотрите раздел [Обновление параметров прокси-сервера](agent-manage.md#update-proxy-settings), чтобы убедиться в правильности настройки агента для обмена данными через прокси-сервер.    
1. Убедитесь, что в список разрешенных включены следующие конечные точки службы Log Analytics:
...
### <a name="resolution"></a>Способы устранения:
1. Убедитесь, что подключение к Log Analytics установлено успешно, проверив наличие файла `/etc/opt/microsoft/omsagent/<workspace id>/conf/omsadmin.conf`. Если его нет, выполните одно из следующих действий:  
...
1. Установите агент в соответствии с [этими инструкциями](../../azure-monitor/learn/quick-collect-linux-computer.md).
1. Перезапустите агент, выполнив следующую команду: `sudo /opt/microsoft/omsagent/bin/service_control restart`.
1. Подождите несколько минут, пока для состояния не отобразится значение **Provisioning succeeded** (Подготовка выполнена успешно).
