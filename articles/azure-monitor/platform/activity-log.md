...
[Создайте параметр диагностики](diagnostic-settings.md) для отправки журнала действий в рабочую область log Analytics. Журнал действий можно отправить из любой одной подписки в до пяти рабочих областей. Для сбора журналов между клиентами требуется [Azure лигхсаусе](../../lighthouse/index.yml).
...
> [!IMPORTANT]
> В некоторых случаях значения в этих столбцах могут быть в верхнем регистре. При наличии запроса, содержащего эти столбцы, следует использовать [оператор = ~](/azure/kusto/query/datatypes-string-operators) для сравнения без учета регистра.
