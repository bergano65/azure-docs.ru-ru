...
### <a name="guest-os-metrics-for-resource-manager-based-windows-vms"></a>Метрика Гостевой ОС для Windows VMs на базе менеджера ресурсов
...
| «PhysicalDisk(_Total)»Avg. Длина чтения диска |Count |
| «PhysicalDisk(_Total)»Avg. Диск Написать Длина очереди |Count |
...
## <a name="commonly-used-app-service-server-farm-metrics"></a>Обычно используемые метрики службы приложений (Server Farm)
Можно также выполнить автомасштабирование на основе общих метрик веб-сервера, таких как длина очереди HTTP. Его метрическое название **http'elength**.  В следующем разделе перечислены доступные метрики фермы серверов (App Service).
