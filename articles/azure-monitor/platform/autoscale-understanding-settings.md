...
- Один профиль. 
- В этом профиле есть два правила метрик. Одно правило задает горизонтальное увеличение масштаба, а другое — горизонтальное уменьшение масштаба.
...
| Section | Имя элемента | Description |
| --- | --- | --- |
...
| Профиль | Capacity.default | Если возникнут проблемы чтения метрик ресурсов (в этом случае — ЦП "vmss1") и текущий объем будет меньше, чем по умолчанию, механизм автомасштабирования выполнит масштабирование до значения по умолчанию. Это необходимо, чтобы обеспечить доступность ресурса. Если текущая емкость превышает емкость по умолчанию, масштаб не будет горизонтально уменьшен в результате автомасштабирования. |
...
| scaleAction | direction | "Increase" — для горизонтального увеличения масштаба, а "Decrease" — для горизонтального уменьшения масштаба.|
...
- **Регулярный профиль.** Самый распространенный. Регулярный профиль можно использовать, если не нужно масштабировать ресурс по дню недели или определенному дню. Этот профиль можно настроить, используя правила метрик, определяющие, когда горизонтально увеличивать масштаб ресурса, а когда горизонтально уменьшать масштаб. Регулярный профиль должен быть только один.
