...
| profile | name | Имя профиля. Для него можно указать любое имя, позволяющее отличить профиль от других. |
| profile | Capacity.maximum | Максимальный разрешенный объем. Обеспечивает, чтобы во время автомасштабирования при выполнении этого профиля объем ресурса не превышал это число. |
| profile | Capacity.minimum | Минимальный разрешенный объем. Обеспечивает, чтобы во время автомасштабирования при выполнении этого профиля объем ресурса не снижался ниже этого числа. |
| profile | Capacity.default | Если возникнут проблемы чтения метрик ресурсов (в этом случае — ЦП "vmss1") и текущий объем будет меньше, чем по умолчанию, механизм автомасштабирования выполнит масштабирование до значения по умолчанию. Это необходимо, чтобы обеспечить доступность ресурса. Если текущий объем превышает значение по умолчанию, количество экземпляров автомасштабирования не уменьшается. |
| profile | правила | Механизм автомасштабирования автоматически уменьшает и увеличивает емкость путем использования правил в профиле. В профиле может быть несколько правил. Обычно имеется два правила: одно применяется, когда необходимо горизонтальное масштабирование, а другое — когда нужно уменьшить масштаб. |
| rule | metricTrigger | Определяет условие метрики для правила. |
...
| rule | scaleAction | Действие, выполняемое при активации параметра metricTrigger правила. |
...
* [Обзор автомасштабирования](../../azure-monitor/platform/autoscale-overview.md)
