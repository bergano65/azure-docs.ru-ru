...
ms.locfileid: "56106416"
---
# <a name="log-alerts-in-azure-monitor"></a>Оповещения журнала в Azure Monitor
В этой статье рассматриваются оповещения журнала. Это один из типов оповещений, которые поддерживаются в системе [оповещений Azure](../platform/alerts-overview.md) и позволяют пользователям применять платформу аналитики Azure в качестве основы для оповещений.

Оповещение журнала состоит из правил запроса по журналам, созданных для [Azure Monitor](../learn/tutorial-viewdata.md) или [Application Insights](../app/cloudservices.md#view-azure-diagnostics-events). Дополнительные сведения о его использовании см. в сведениях о [создании оповещений журнала в Azure](../platform/alerts-log.md).

> [!NOTE]
> Распространенные данные журнала из [Azure Monitor](../learn/tutorial-viewdata.md) теперь также доступны на платформе метрик в Azure Monitor. Более подробную информацию см. в статье [Create Metric Alerts for Logs in Azure Monitor](../platform/alerts-metric-logs.md) (Создание оповещений метрик для журналов в Azure Monitor).
...
- **Запрос к журналу.**  Это запрос, который будет запускаться каждый раз при срабатывании правила генерации оповещений.  Определить, следует ли активировать оповещение, можно с помощью записей, возвращаемых этим запросом. Запрос аналитики может предназначаться для конкретной рабочей области Log Analytics или приложения Application Insights и даже охватывать [несколько ресурсов Log Analytics и Application Insights](../../azure-monitor/log-query/cross-workspace-query.md#querying-across-log-analytics-workspaces-and-from-application-insights) при условии, что у пользователя есть доступ ко внешним приложениям. Определенные аналитические команды и сочетания не подходят для использования в оповещениях журнала. Дополнительные сведения см. в статье [Запросы оповещений журналов в Azure Monitor](../../azure-monitor/platform/alerts-log-query.md).

    > [!IMPORTANT]
    > В целях безопасности оповещение журнала **не** поддерживает использование [функций](../log-query/functions.md). Чтобы пользователь мог создать правило генерации оповещений журнала, ему необходимо указать полный запрос аналитики и обладать правами на полный доступ и выполнение.
...
Правила запроса по журналам могут быть двух типов (как для [Azure Monitor](../learn/tutorial-viewdata.md), так и для [Application Insights](../app/cloudservices.md#view-azure-diagnostics-events)). Каждый из этих типов подробно описан в последующих разделах.
...
Если результат запроса отобразить на графике, он будет выглядеть следующим образом.

![Результаты выполнения примера запроса](media/alerts-unified-log/metrics-measurement-sample-graph.png)

В этом примере показан средний объем использования процессоров для каждого из трех компьютеров в ячейках 5 минут, вычисляемый в течение 5 минут. Пороговое значение 90 нарушено srv01 только один раз в ячейке 1:25. Для сравнения, srv02 превышает пороговое значение 90 в ячейках 1:10, 1:15 и 1:25; а srv03 превышает пороговое значение 90 в 1:10, 1:15, 1:20 и 1:30. Поскольку оповещение настроено, чтобы активировать общую брешь в системе безопасности (более двух нарушений), только srv02 и srv03 соответствуют этому критерию. Поэтому для srv02 и srv03 будут созданы отдельные оповещения, так как за указанный период они 2 раза нарушили пороговое значение в 90 %.  Если вместо параметра *Активировать оповещение на основе:* было настроено параметр *Continuous breaches* (Последовательные нарушения), тогда оповещение будет срабатывать **только** для srv03, так как он нарушил пороговое значение в ячейках времени с 1:10 до 1:20 последовательно три раза. Однако оно **не** будет срабатывать для srv02, так как он нарушил пороговое значение в ячейках времени с 1:10 до 1:15 последовательно два раза.
...
- В следующей итерации в 13:10, когда правило Contoso-Log-Alert было выполнено оповещениями Azure, в результате поиска по журналу было получено 5 записей. Это выше порога, поэтому сработало оповещение вскоре после запуска связанной [группы действий](../platform/action-groups.md). 
- В 13:15, когда правило Contoso-Log-Alert было выполнено оповещениями Azure, в результате поиска по журналу было получено 2 записи. Порог был превышен, из-за чего сработало оповещение вскоре после запуска связанной [группы действий](../platform/action-groups.md).
...
- Для оповещений журнала Log Analytics указываются имя оповещения в формате `<WorkspaceName>|<savedSearchId>|<scheduleId>|<ActionId>`, группа ресурсов и свойства оповещения, если создание выполняется с помощью [устаревшего API Log Analytics](api-alerts.md) или с использованием портала Azure **без** добровольного перехода на новый API-интерфейс.

    > [!NOTE]
    > Если имеются недопустимые знаки, такие как `<, >, %, &, \, ?, /`, в счете они будут заменены знаком `_`. Чтобы удалить ресурсы scheduleQueryRules, созданные для выставления счетов за правила генерации оповещения с помощью [устаревшего API Log Analytics](api-alerts.md), пользователю необходимо удалить исходное расписание и действие оповещения с помощью [устаревшего API Log Analytics](api-alerts.md).

## <a name="next-steps"></a>Дополнительная информация
* Дополнительные сведения о [создании оповещений журнала в Azure](../platform/alerts-log.md).
* Информация о [веб-перехватчиках в оповещениях журналов в Azure](alerts-log-webhook.md).
* Сведения об [оповещениях Azure](../platform/alerts-overview.md).
* Дополнительные сведения об [Application Insights](../app/analytics.md).
* Дополнительные сведения о запросах по журналам в Azure Monitor см. в [этой статье](../log-query/log-query-overview.md).    
