...
| Проблема | Рабочая область — контекст | Ресурс — контекст |
|:---|:---|:---|
...
| Что требуется пользователю для просмотра журналов? | Разрешения для рабочей области. См. раздел **разрешения рабочей области** в разделе [Управление доступом с помощью разрешений рабочей области](manage-access.md#manage-access-using-workspace-permissions). | Доступ для чтения к ресурсу. См. раздел **разрешения ресурсов** в разделе [Управление доступом с помощью разрешений Azure](manage-access.md#manage-access-using-azure-permissions). Разрешения могут наследоваться (например, из содержащей их группы ресурсов) или напрямую назначаться ресурсу. Разрешение на доступ к журналам для ресурса будет автоматически назначено. |
...
* **Использовать разрешения ресурса или рабочей области**. Этот режим управления разрешает детализированный RBAC. Пользователям можно предоставить доступ только к данным, связанным с ресурсами, которые они могут просматривать, назначая `read` разрешение Azure. 
...
Чтобы узнать, как изменить режим управления доступом на портале с помощью PowerShell или шаблона диспетчер ресурсов, см. раздел [Настройка режима управления доступом](manage-access.md#configure-access-control-mode).

## <a name="ingestion-volume-rate-limit"></a>Предел скорости приема

Azure Monitor — это высокомасштабируемая служба, которая обслуживает тысячи клиентов, ежемесячно отправляющих стремительными темпами терабайты данных. Ограничение скорости тома позволяет защитить Azure Monitor клиентов от внезапных пиков приема в среде с многоаренды. Пороговое значение скорости приема по умолчанию, равное 500 МБ (сжатое), применяется к рабочим областям, то есть примерно **6 Гб/мин** без сжатия — фактический размер может различаться в зависимости от длины журнала и его коэффициента сжатия. Это пороговое значение применяется ко всем полученным данным независимо от того, отправлены ли из ресурсов Azure с помощью [параметров диагностики](diagnostic-settings.md), [API сборщика данных](data-collector-api.md) или агентов.

При отправке данных в рабочую область с частотой тома выше 80% от порогового значения, настроенного в рабочей области, событие передается в таблицу *операций* в рабочей области каждые 6 часов, пока не будет превышено пороговое значение. Если частота принимаемого тома выше порогового значения, некоторые данные отбрасываются, и событие отправляется в таблицу *операций* в рабочей области каждые 6 часов, пока не будет превышено пороговое значение. Если скорость приема громкости по-тому превышает порог, или ожидается, что она будет достигнута вскоре, можно запросить его увеличение в рабочей области, открыв запрос в службу поддержки. 

Чтобы получать уведомления о таком событии в рабочей области, создайте [правило генерации оповещений журнала](alerts-log.md) , используя следующий запрос с базой логики оповещений на основе числа результатов больше нуля, периода оценки 5 минут и частоты 5 минут.

Достигнуто количество порогового объема приема, 80% от порога:
```Kusto
Operation
...
Достигнуто пороговое значение скорости приема:
```Kusto
Operation
|where OperationCategory == "Ingestion"
