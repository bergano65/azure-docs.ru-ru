...
Azure Monitor — это высокомасштабируемая служба, которая обслуживает тысячи клиентов, ежемесячно отправляющих стремительными темпами терабайты данных. Ограничение скорости тома позволяет изолировать Azure Monitor клиентов от внезапных пиков приема в среде с многоаренды. В рабочих областях определяется пороговое значение скорости приема по умолчанию, равное 500 МБ (сжатое), и оно преобразуется в примерно **6 Гб/мин** без сжатия. фактический размер может различаться в зависимости от длины журнала и его коэффициента сжатия. Ограничение скорости тома применяется ко всем принимаемым данным независимо от того, отправлены ли из ресурсов Azure с помощью [параметров диагностики](diagnostic-settings.md), [API сборщика данных](data-collector-api.md) или агентов.
...
Чтобы получать уведомления о приближении или достижении предельного количества скорости приема в рабочей области, создайте [правило генерации оповещений журнала](alerts-log.md) , используя следующий запрос с базой логики оповещений на основе числа результатов, превышающих 0, период оценки 5 минут и частоту в 5 минут.

Скорость приема громкости превысила пороговое значение
```Kusto
Operation
...
Скорость приема громкости превысила 80% порогового значения
```Kusto
Operation
| where Category == "Ingestion"
| where OperationKey == "Ingestion rate limit"
...
Скорость приема громкости превысила 70% порогового значения
```Kusto
Operation
| where Category == "Ingestion"
| where OperationKey == "Ingestion rate limit"
