...
Чтобы сохранить текущие инвестиции в [System Center Operations Manager](https://docs.microsoft.com/system-center/scom/key-concepts?view=sc-om-1807) и использовать расширенные возможности в Azure Monitor, Operations Manager можно интегрировать с рабочей областью Log Analytics. Это позволяет использовать возможности журналов в Azure Monitor, продолжая при этом работать Operations Manager для:
...
Интеграция с System Center Operations Manager добавляет значение в стратегии операций службы с помощью скорость и эффективность Azure Monitor в сбора, хранения и анализа данных журнала из Operations Manager. Azure Monitor журнала запросов справки корреляции и работы, определять ошибки проблем и сообщать о повторном возникновении для поддержки существующего процесса управления проблемами. Гибкость ядро запросов для проверки производительности, событий и оповещений надежность Azure Monitor демонстрирует данные, с помощью многофункциональных панелей мониторинга и отчетности для предоставления этих данных эффективными способами, переводит свидетельствуют Operations Manager.

Агенты, отправляющие отчеты в группу управления Operations Manager собирать данные с серверов на основе [источников данных Log Analytics](agent-data-sources.md) и решения, которые вы включили в рабочей области. В зависимости от включенного решения, данные отправляются непосредственно из сервера управления Operations Manager к службе, или из-за объема данных, собранных в управляемой агентом системе, отправляются непосредственно из агента в рабочую область Log Analytics. Сервер управления перенаправляет данные непосредственно в службу. Они никогда не записываются в рабочую базу данных или хранилище данных. Если сервер управления теряет связь с Azure Monitor, он кэширует данные локально до повторного установления. Если сервер управления отключен из-за планового обслуживания или сбоя, другой сервер управления в группе управления возобновляет подключение с помощью Azure Monitor.  

В примере ниже показан подключения между серверами управления и агентов в группу управления System Center Operations Manager и Azure Monitor, включая направление и порты.   

![oms-operations-manager-integration-diagram](./media/om-agents/oms-operations-manager-connection.png)

Если политики ИТ-безопасности запрещают подключение компьютеров в сети к Интернету, серверы управления можно настроить для подключения к шлюзу Log Analytics, чтобы получать сведения о конфигурации и отправлять собранные данные в зависимости от включенного решения. Дополнительные сведения и инструкции по настройке группы управления Operations Manager для взаимодействия через шлюз Log Analytics в Azure Monitor, см. в разделе [подключение компьютеров к Azure Monitor с помощью Log Analytics шлюза](../../azure-monitor/platform/gateway.md).  
...
>[!NOTE]
>Последние изменения в API-интерфейсы Azure позволит клиентам, чтобы успешно настроить интеграцию между их группе управления и Azure Monitor в первый раз. Если вы уже интегрировали свою группу управления со службой, на вас это не воздействует, пока вам не потребуется перенастроить существующее подключение.  
...
Ниже приводятся сведения о прокси-сервера и брандмауэра конфигурации необходим для агента Operations Manager, серверов управления и консоль управления для взаимодействия с Azure Monitor. Трафик из каждого компонента является исходящим из локальной сети к Azure Monitor.   
...
|\*.ods.opinsights.azure.com| 443| Yes|  
|*.azure-automation.net | 443| Yes|  
|**Консоль Operations Manager с Azure Monitor**|||  
...
После этого выполните следующие действия для интеграции с Azure Monitor, можно удалить конфигурацию, выполнив `netsh winhttp reset proxy` , а затем использовать **Настройка прокси-сервера** параметр в консоли управления, чтобы указать прокси-сервер или журнала Сервер шлюза Analytics. 
...
1. На странице **Мастер подключения Operations Management Suite: проверка подлинности** введите адрес электронной почты или номер телефона и пароль учетной записи администратора, связанной с вашей подпиской OMS, и щелкните **Войти**.
   ...
   >[!NOTE]
   >Было удалено имя Operations Management Suite. 
...
1. После успешной аутентификации на странице **Мастер подключения Operations Management Suite: выбор рабочей области** вам предлагается выбрать клиент Azure, подписку и рабочую область Log Analytics. При наличии нескольких рабочих областей выберите в раскрывающемся списке рабочую область, которую необходимо зарегистрировать в группе управления Operations Manager, и нажмите кнопку **Далее**.
   ...
   > [!NOTE]
   > Одновременно в Operations Manager поддерживается только одна рабочая область Log Analytics. Подключение и компьютеры, которые были зарегистрированы в Azure Monitor с предыдущей рабочей областью, удаляются из Azure Monitor.
   > 
   > 
...
1. В диалоговом окне **Поиск компьютера** можно выполнить поиск компьютеров или групп, отслеживаемых с помощью Operations Manager. Выберите компьютеры или группы для подключения к Azure Monitor, нажмите кнопку **добавить**, а затем нажмите кнопку **ОК**.
...
Если внутренний прокси-сервер между группой управления и Azure Monitor, выполните следующие действия. Эти параметры являются централизованно управлять из группы управления и распространить на управляемых агентами системах, которые входят в область для сбора данных журнала для Azure Monitor.  Это дает преимущества, если определенные решения обходят сервер управления и отправляют данные непосредственно в службу.
...
Если прокси-сервер требует проверки подлинности, выполните следующие действия, чтобы настроить учетные данные и параметры, которые необходимо распространить на управляемые компьютеры, которые в Azure Monitor в отчете в группе управления.
...
После создания подключения и настройки агентов для сбора и отчетов данных журнала в Azure Monitor, в группе управления, не обязательно в порядке применяется следующая конфигурация:
...
После завершения настройки группы управления Operations Manager устанавливает подключение с помощью Azure Monitor. Сервер управления синхронизируется с веб-службой и получает обновленные сведения о конфигурации в виде пакетов управления для решений, которые вы включили для интеграции с Operations Manager. Operations Manager проверяет наличие обновлений для этих пакетов управления, автоматически скачивает и импортирует их. Существует два правила для управления этими действиями:

* **Microsoft.SystemCenter.Advisor.MPUpdate** -обновляет базовые пакеты управления для Azure Monitor. По умолчанию выполняется каждые 12 часов.
...
Вы можете переопределить эти два правила, чтобы предотвратить автоматическое скачивание, путем их отключения или изменить частоту как часто сервер управления синхронизируется с Azure Monitorto определить, доступен ли новый пакет управления и должны быть загружены. Чтобы изменить значение в секундах параметра **Частота** для изменения расписания синхронизации или изменить параметр **Включено** для отключения правил, выполните действия, описанные в статье [Переопределение правила или монитора состояния](https://technet.microsoft.com/library/hh212869.aspx). Примените переопределенные значения для всех объектов класса группы управления Operations Manager.

Если изменять существующий процесс управления изменениями для контроля выпусков пакетов управления в рабочей группе управления не требуется, отключите правила и включайте их только тогда, когда требуются обновления. При наличии в среде группы управления разработкой и контролем качества и подключения к Интернету, для поддержки этого сценария эту группу управления можно настроить с рабочей областью Log Analytics. Это позволяет просматривать и оценивать последовательные выпуски пакетов управления, Azure Monitor перед их выпуском в вашей рабочей группе управления.
...
## <a name="validate-operations-manager-integration-with-azure-monitor"></a>Проверка интеграции Operations Manager с помощью Azure Monitor
Существует несколько разных способов, чтобы узнать, что Azure Monitor для интеграции Operations Manager выполнена успешно.
...
Если интеграция группы управления Operations Manager и рабочей области Log Analytics больше не требуется, для надлежащего удаления подключения и конфигурации в группе управления необходимо выполнить несколько действий. В следующей процедуре содержаться вам обновить рабочую область Log Analytics, удалив ссылку на группу управления, удалить соединителя Azure Monitor и затем удалить пакеты управления, поддерживающие интеграцию со службой.  

Пакеты управления для решений, которые вы включили для интеграции с Operations Manager и пакеты управления, необходимые для поддержки интеграции с Azure Monitor, непросто удалить из группы управления. Это обусловлено тем, некоторые пакеты управления монитора Azure имеют зависимости от других связанных пакетов управления. Чтобы удалить пакеты управления, зависимые от других таких пакетов, скачайте сценарий для [удаления пакета управления с зависимостями](https://gallery.technet.microsoft.com/scriptcenter/Script-to-remove-a-84f6873e) из центра сценариев TechNet.  
...
Чтобы добавить функциональные возможности и сбора данных, см. в разделе [решения добавьте Azure Monitor из коллекции решений](../../azure-monitor/insights/solutions.md).
