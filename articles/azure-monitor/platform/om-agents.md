...
>[!NOTE]
>Недавние изменения программных интерфейсов Azure не позволяют клиентам настроить интеграцию между своей группой управления и Log Analytics в первый раз. Если вы уже интегрировали свою группу управления со службой, на вас это не воздействует, пока вам не потребуется перенастроить существующее подключение.  
...
|\*.ods.opinsights.azure.com| 443| Yes|  
|*.azure-automation.net | 443| Yes|  
|**Консоль Operations Manager с Log Analytics**|||  
...
Выполнив интеграцию с Log Analytics, можно удалить конфигурацию, запустив команду `netsh winhttp reset proxy`, а затем щелкните **Настройка прокси-сервера** в консоли управления, чтобы указать прокси-сервер или сервер шлюза Log Analytics. 
...
1. В диалоговом окне **Поиск компьютера** можно выполнить поиск компьютеров или групп, отслеживаемых с помощью Operations Manager. Выберите компьютеры или группы для подключения к службе Log Analytics, нажмите кнопку **Добавить**, а затем — **ОК**.
...
Если внутренний прокси-сервер расположен между группой управления и службой Log Analytics, выполните действия, указанные ниже. Этими параметрами можно централизовано управлять из группы управления, и они распределены между управляемыми агентами системами, которые используются при сборе данных для Log Analytics.  Это дает преимущества, если определенные решения обходят сервер управления и отправляют данные непосредственно в службу.
