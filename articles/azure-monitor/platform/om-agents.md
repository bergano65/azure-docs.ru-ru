...
Чтобы выполнить интеграцию с Azure Monitor, выполните следующие действия, чтобы удалить конфигурацию, запустив `netsh winhttp reset proxy`, а затем используйте параметр **настроить прокси-сервер** в консоли управления для указания прокси-сервера или log Analytics шлюза.
...
1. В мастере профиля запуска от имени нажмите кнопку Добавить, чтобы использовать учетную запись запуска от имени. Можно создать [учетную запись запуска от имени](https://technet.microsoft.com/library/hh321655.aspx) или использовать существующую. Эта учетная запись должна иметь достаточные разрешения для передачи данных через прокси-сервер.
...
Эти два правила можно переопределить, чтобы запретить автоматическую загрузку, отключив их, или изменить частоту синхронизации сервера управления с Azure Monitor, чтобы определить, доступен ли новый пакет управления и следует ли его загрузить. Чтобы изменить значение в секундах параметра [Частота](https://technet.microsoft.com/library/hh212869.aspx) для изменения расписания синхронизации или изменить параметр **Включено** для отключения правил, выполните действия, описанные в статье **Переопределение правила или монитора состояния**. Примените переопределенные значения для всех объектов класса группы управления Operations Manager.
