...
Можно использовать метрические оповещения в популярных журналах Log Analytics, извлеченных в качестве метрик в качестве части метрик из журналов, включая ресурсы в Azure или на месте. Поддерживаемые решения Log Analytics перечислены ниже.
...
> [!NOTE]
> Для просмотра поддерживаемой метрики, извлеченной из рабочего пространства log Analytics через [Azure Monitor - Metrics,](../../azure-monitor/platform/metrics-charts.md)необходимо создать метрика для журнала на этой конкретной метрике. Размеры, выбранные в метрикации для журналов, будут отображаться только для исследования с помощью Azure Monitor - Metrics.
...
2. **Агент настроен для рабочего пространства аналитики журнала:** агент должен быть настроен для VMs-м и vMs-наместных новизнах Azure для отправки данных в рабочее пространство анализа журналов, используемое на более раннем этапе. Дополнительные сведения см. в статье [Обзор агентов Azure для мониторинга виртуальных машин Azure](../../azure-monitor/platform/agents-overview.md).
3. **Поддерживается журнал Analytics Решения установлен:** Журнал Analytics решение должно быть настроено и отправки данных в журнал Analytics рабочее пространство - поддерживаемые решения [счетчики производительности для Windows & Linux](../../azure-monitor/platform/data-sources-performance-counters.md), [Heartbeat записи для агента здравоохранения](../../azure-monitor/insights/solution-agenthealth.md), управление [обновлением](../../automation/automation-update-management.md), и [данные событий](../../azure-monitor/platform/data-sources-windows-events.md).
...
> [!NOTE]
> При создании метрического оповещения для рабочего пространства Log Analytics через портал Azure - соответствующее правило для преобразования данных журнала в метрику через [Azure Monitor - Правила планового запроса](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules) автоматически создаются в фоновом режиме, *без необходимости вмешательства или действий пользователя.* Для создания оповещений о метриках для журналов с использованием средств, отличных от портала Azure, см. раздел [Шаблон ресурса для оповещений о метриках для журналов](#resource-template-for-metric-alerts-for-logs) с примером средства создания правила преобразования журнала на основе ScheduledQueryRule в метрику до создания оповещения о метрике — иначе для созданного оповещения о метрике для журналов не будет данных.
...
- Узнайте о [предупреждениях в Azure](alerts-overview.md).
