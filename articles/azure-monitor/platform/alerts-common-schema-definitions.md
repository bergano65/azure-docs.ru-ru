---
title: Общие определения схемы оповещения для Azure Monitor
description: Понимание общих определений схемы оповещения для Azure Monitor
author: ofirmanor
ms.topic: conceptual
...
ms.locfileid: "80672904"
---
# <a name="common-alert-schema-definitions"></a>Определения общей схемы оповещений

В этой статье [описаны общие определения схемы оповещения](https://aka.ms/commonAlertSchemaDocs) для Azure Monitor, в том числе для веб-крючков, приложений логики Azure, функций Azure и runbooks Azure Automation. 

В любом экземпляре оповещения описывается затронутый ресурс и причина оповещения. Эти экземпляры описаны в общей схеме в следующих разделах:
* **Основы**: Набор стандартизированных полей, распространенных во всех типах оповещения, которые описывают, на каком ресурсе находится оповещение, наряду с дополнительными общими метаданными оповещения (например, серьезность или описание). 
* **Контекст оповещения**: Набор полей, описывающий причину оповещения, с полями, которые различаются в зависимости от типа оповещения. Например, метрика включает такие области, как имя метрики и значение метрики в контексте оповещения, в то время как оповещение журнала активности содержит информацию о событии, генерируемом оповещение. 
...
| Поле | Описание|
|:---|:---|
| alertId | GUID однозначно идентифицирует экземпляр оповещения. |
| alertRule | Имя правила оповещения, генерируемого экземпляром оповещения. |
| Severity | Тяжесть предупреждения. Возможные значения: Sev0, Sev1, Sev2, Sev3, или Sev4. |
| сигналТип | Определяет сигнал, на котором было определено правило оповещения. Возможные значения: Метрика, журнал или журнал активности. |
| мониторУсловие | При возгорании оповещения состояние монитора устанавливается на **выполнение.** Когда основное условие, вызвавшее оповещение о пожаре, очищается, условие монитора устанавливается для **разрешения.**   |
| мониторингСервис | Служба мониторинга или решение, генерируемое оповещением. Поля для контекста оповещения диктуются службой мониторинга. |
| alertTargetIds | Список идов идов управления ресурсами Azure, затронутых целями оповещения. Для оповещения о журнале, определяемого в рабочей области журнала Analytics или экземпляре Application Insights, это соответствующее рабочее пространство или приложение. |
| происхождениеAlertId | Идентификатор экземпляра оповещения, генерируемый службой мониторинга. |
| уволенДатаВремя | Дата и время, когда экземпляр оповещения был выпущен в скоординированном универсальном времени (UTC). |
| решенноеВремя | Дата и время, когда условие монитора для экземпляра оповещения установлено для **решения** в UTC. В настоящее время применяется только для метрических оповещений.|
| description | Описание, как определено в правиле оповещения. |
|essentialsVersion| Номер версии для раздела essentials.|
|alertКонтекст | Номер версии `alertContext` для раздела. |
...
> [!NOTE]
> Для журналов оповещений, которые имеют пользовательский объект электронной почты и / или JSON полезной нагрузки определены, что позволяет общей схеме возвращает тему электронной почты и / или полезной нагрузки схемы в тот, описанный следующим образом. Оповещения с общей схемой включен имеют верхний предел размера 256 кБ на оповещение. Результаты поиска не встроены в полезную нагрузку оповещения журналов, если они вызывают перечеркив этот порог. Вы можете определить это, `IncludeSearchResults`проверив флаг. Если результаты поиска не включены, следует использовать поисковый запрос в сочетании с [API log Analytics.](https://docs.microsoft.com/rest/api/loganalytics/query/get) 
