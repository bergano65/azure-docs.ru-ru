...
[Параметры диагностики](./diagnostic-settings.md) собираются журналы ресурсов и метрики из ресурсов Azure в несколько расположений, обычно в log Analytics рабочей области, которая позволяет анализировать данные с помощью [запросов журнала](../log-query/log-query-overview.md) и [оповещений журнала](alerts-log.md). Используйте политику для автоматического создания параметров диагностики при каждом создании ресурса.
...
![Назначение инициативы](media/deploy-scale/initiative-assignment.png)
...
## <a name="azure-monitor-for-vms-and-virtual-machine-agents"></a>Azure Monitor для виртуальных машин и агенты виртуальных машин
[Azure Monitor для виртуальных машин](../insights/vminsights-overview.md) является основным средством Azure Monitor для наблюдения за виртуальными машинами и масштабируемыми наборами виртуальных машин. Чтобы включить Azure Monitor для виртуальных машин необходимо установить агент Log Analytics и агент зависимостей на каждом клиенте. Вы также можете установить собственный агент Log Analytics для поддержки других сценариев мониторинга. Вместо выполнения этих задач вручную используйте политику Azure, чтобы обеспечить настройку каждой виртуальной машины при ее создании.
...
![Назначение инициативы](media/deploy-scale/virtual-machine-scale-set-assign-initiative.png)
