...
> [!IMPORTANT]
> Представления в Azure Monitor выводятся и заменяются [книгами](workbooks-overview.md) , которые предоставляют дополнительные функциональные возможности. Дополнительные сведения о преобразовании существующих представлений в книги см. в разделе [Конструктор представлений Azure Monitor. книга переходов](view-designer-conversion-overview.md) .
...
| Параметр | Описание |
|:--|:--|
| Обновление   | Обновляет представление с помощью самых актуальных данных. | 
| Журналы      | Открывает [log Analytics](../log-query/portals.md) для анализа данных с помощью запросов журналов. |
| Правка       | Открывает представление в конструкторе представлений для изменения его содержимого и конфигурации.  |
| Clone      | Создает представление и открывает его в конструкторе представлений. Для нового представления используется такое же имя, как и у исходного, с добавлением в конце слова *Copy* (Копия). |
...
![Меню «Изменить»](media/view-designer/edit-menu.png)
