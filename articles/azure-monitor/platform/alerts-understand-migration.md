...
- Классические правила генерации оповещений для гостевых метрик виртуальной машины (Windows и Linux). См. [руководство по повторному созданию таких правил генерации оповещений в новых оповещениях о метриках](#guest-metrics-on-virtual-machines) далее в этой статье.
...
> [!NOTE]
> Кроме перечисленных выше исключений, если классические правила генерации оповещений недопустимы, т. е. они находятся в [нерекомендуемых метриках](#classic-alert-rules-on-deprecated-metrics) или ресурсах, которые были удалены, они не будут перенесены и станут недоступны после прекращения использования службы.
...
Средство миграции преобразует классические правила генерации оповещений в эквивалентные новые правила генерации оповещений и группы действий. Для большинства классических правил генерации оповещений эквивалентные новые правила генерации оповещений находятся в той же метрике с такими же свойствами, как `windowSize` и `aggregationType`. Однако некоторые классические правила генерации оповещений относятся к метрикам, имеющим разные эквивалентные метрики в новой системе. Следующие принципы применяются к переносу классических оповещений, если они не указаны в разделе ниже.

- **Frequency**: определяет, как часто будет проверяться наличие в классическом или новом правиле генерации оповещений для условия. `frequency` в классических правилах оповещений не было настроено пользователем и всегда было 5 минут для всех типов ресурсов, кроме Application Insights компонентов, для которых он был 1 мин. Таким образом, частота эквивалентных правил также равна 5 минутам и 1 минутам соответственно.
- **Тип статистической обработки**: определяет, как Статистическая обработка метрики осуществляется по интересующему окну. Кроме того, эта `aggregationType` одинакова между классическими оповещениями и новыми оповещениями для большинства метрик. В некоторых случаях, поскольку метрика отличается между классическими предупреждениями и новыми оповещениями, используется эквивалентная `aggregationType` или `primary Aggregation Type`, определенных для метрики.
...
- **Размер окна**: определяет окно, по которому объединяются данные метрики для сравнения с пороговым значением. Для стандартных значений `windowSize`, таких как 5mins, 15mins, 30mins, 1hour, 3hours, 6 часов, 12 часов, 1 день, для эквивалентного нового правила генерации оповещений не вносится никаких изменений. Для других значений выбирается ближайшее `windowSize` для использования. Большинство клиентов не влияют на это изменение. Для небольшого количества клиентов может потребоваться настроить пороговое значение, чтобы добиться точно такого же поведения.
