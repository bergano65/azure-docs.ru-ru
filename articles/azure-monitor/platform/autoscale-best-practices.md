...
* Все ошибки автомасштабирования записываются в журнал действий. Затем можно настроить [оповещение журнала действий](./../../azure-monitor/platform/activity-log-alerts.md), чтобы в случае сбоя автоматического масштабирования получать уведомления при помощи электронной почты, SMS, или веб-перехватчиков.
...
Если вы используете только одну часть комбинации, автомасштаб будет действовать только в одном направлении (масштабирование или внутри), пока не достигнет максимума или минимального количества экземпляра, определенного в профиле. Это не является оптимальным. В идеале нужный ресурс для масштабирования в периоды высокой нагрузки должен обеспечивать доступность. Аналогичным образом в некоторых случаях с низким уровнем использования для экономии средств необходимо, чтобы ресурс уменьшался.
...
Мы *не рекомендуем* настройки автомасштаба, такие как приведенные ниже примеры с одинаковыми или аналогичными пороговыми значениями для вне и в условиях:

* Увеличьте экземпляры на 1 счет, когда отсчет потоков >600 евро
* Уменьшение экземпляров на 1 счет, когда отсчет потоков <600 евро
...
4. Перед уменьшением масштаба служба автомасштабирования пытается оценить, какое конечное состояние будет после сворачивания. Например, 575 x 3 (текущее количество экземпляров) = 1725 / 2 (итоговое количество экземпляров после уменьшения масштаба) = 862,5 потока. Это означает, что автомасштабу придется немедленно масштабировать снова даже после того, как он масштабируется, если среднее количество потоков остается прежним или даже падает лишь небольшое количество. Однако после повторного разворачивания весь опять процесс повторится, что приведет к бесконечному циклу.
5. Чтобы избежать такой нестабильности, параметр автомасштабирования вообще не уменьшает масштаб. Вместо этого она пропускает, а затем повторно оценивает условие при следующем выполнении задания службы. Состояние хлопания может запутать многих людей, потому что автомасштаб не будет работать, когда средний отсчет потока был 575.
...
4. Третий экземпляр очереди хранения добавляется из-за действия масштабирования.  Следующее развертывание не будет выполнено, пока общее количество сообщений в очереди не достигнет 150, так как 150/3 = 50.
...
На следующем рисунке показан параметр автомасштабирования с профилем по умолчанию с минимальным числом экземпляров, равным 2, и максимальным числом экземпляров, равным 10. В этом примере правила настраиваются для масштабирования, когда количество сообщений в очереди превышает 10, а масштабирование, когда количество сообщений в очереди меньше трех. Итак, теперь ресурс может масштабироваться в пределах от двух до десяти экземпляров.
...
Бывают случаи, когда требуется задать несколько правил в профиле. Следующие правила автомасштабирования используются автоматическим двигателем при установке нескольких правил.

На *масштабе,* автомасштаб работает, если какое-либо правило выполнено.
...
* Если процессор < 30%, масштабирование на 1
...
* Если процессор 76% и память 50%, мы масштабируется.
* Если процессор 50%, а память 76%, мы масштабируется.
...
Количество экземпляров по умолчанию важно, потому что автомасштаб масштабирование службы к этому счету, когда метрики недоступны. Поэтому выберите число экземпляров по умолчанию, являющееся безопасным для рабочих нагрузок.
...
* Autoscale выдает операцию масштабирования.
* Сервис Autoscale успешно завершает действие масштаба.
...
- [Создайте оповещение о журнале активности для мониторинга всех операций автоматического двигателя по подписке.](https://github.com/Azure/azure-quickstart-templates/tree/master/monitor-autoscale-alert)
