...
В этой статье описывается, как настроить в Log Analytics связь между продуктами или службами ITSM и соединителем управления ИТ-службами (ITSM), чтобы централизованно управлять рабочими элементами ITSM. Дополнительные сведения о ITSMC см. в разделе [Обзор](../../azure-monitor/platform/itsmc-overview.md).
...
- ITSMC установлен. Дополнительные сведения: [Добавление решения соединитель управления ИТ-услугами](../../azure-monitor/platform/itsmc-overview.md#adding-the-it-service-management-connector-solution).
...
| **Создание нового элемента конфигурации в решении ITSM** | Выберите этот параметр, если вы хотите создать элементы конфигурации в продукте ITSM. После выбора этого параметра Log Analytics создаст зависимые элементы как элементы конфигурации (в случае, если эти элементы еще не созданы) в поддерживаемой системе ITSM. **По умолчанию**: отключено. |
...
Дополнительные сведения: [Создание рабочих элементов ITSM из оповещений Azure](../../azure-monitor/platform/itsmc-overview.md#create-itsm-work-items-from-azure-alerts).
...
- **Настройте гибридное подключение** - , чтобы[настроить это подключение](#configure-the-hybrid-connection)вручную.
...
Сценарий создаст веб-приложение, используя имя, которое вы указали (и несколько дополнительных строк для обеспечения его уникальности). При этом создается **URL-адрес приложения**, **идентификатор клиента** и **секрет клиента**.

Сохраните эти значения. Они вам понадобятся при создании подключения к ITSMC.

**Подтверждение установки веб-приложения**

1. Перейдите к **портал Azure** > **ресурсам**.
2. Выберите веб-приложение, щелкните **Параметры** > параметры**приложения**.
...
2. Щелкните **Параметры** > **Сетевые подключения**.
...
> [!NOTE]
> ITSMC поддерживает только официальное предложение SaaS из службы. Закрытые развертывания службы сейчас не поддерживаются. 

**Администраторы ServiceNow должны выполнить следующие действия в своем экземпляре ServiceNow**:
- Создать идентификатор и секрет клиента для продукта ServiceNow. Дополнительные сведения о том, как создать идентификатор и секрет клиента, см. в соответствующих разделах:
    ...
    - [Настройка OAuth для Нью Йорк](https://docs.servicenow.com/bundle/newyork-platform-administration/page/administer/security/task/t_SettingUpOAuth.html)
    ...
    - [Настройка OAuth для Лондона](https://docs.servicenow.com/bundle/london-platform-administration/page/administer/security/task/t_SettingUpOAuth.html)
...
> [!NOTE]
> В качестве части определения "Настройка OAuth" Мы рекомендуем:
>
> 1) **Обновите срок действия маркера обновления до 90 дней (7 776 000 секунд):** В рамках [настройки OAuth](https://nam06.safelinks.protection.outlook.com/?url=https%3A%2F%2Fdocs.servicenow.com%2Fbundle%2Fnewyork-platform-administration%2Fpage%2Fadminister%2Fsecurity%2Ftask%2Ft_SettingUpOAuth.html&data=02%7C01%7CNoga.Lavi%40microsoft.com%7C2c6812e429a549e71cdd08d7d1b148d8%7C72f988bf86f141af91ab2d7cd011db47%7C1%7C0%7C637208431696739125&sdata=Q7mF6Ej8MCupKaEJpabTM56EDZ1T8vFVyihhoM594aA%3D&reserved=0) в фазе 2. [Создайте конечную точку для доступа клиентов к экземпляру](https://nam06.safelinks.protection.outlook.com/?url=https%3A%2F%2Fdocs.servicenow.com%2Fbundle%2Fnewyork-platform-administration%2Fpage%2Fadminister%2Fsecurity%2Ftask%2Ft_CreateEndpointforExternalClients.html&data=02%7C01%7CNoga.Lavi%40microsoft.com%7C2c6812e429a549e71cdd08d7d1b148d8%7C72f988bf86f141af91ab2d7cd011db47%7C1%7C0%7C637208431696749123&sdata=hoAJHJAFgUeszYCX1Q%2FXr4N%2FAKiFcm5WV7mwR2UqeWA%3D&reserved=0) после определения конечной точки, в колонке ServiceNow Поиск по системному OAuth, а не выберите реестр приложений. Выберите имя OAuth, которое было определено, и обновите поле срок существования маркера обновления до 7 776 000 (90 дней в секундах).
> В конце щелкните Обновить.
> 2) **Рекомендуется установить внутреннюю процедуру, чтобы убедиться, что подключение остается активным:** В соответствии с сроком существования маркера обновления для обновления токена. Убедитесь, что выполнены следующие операции, предшествующие ожидаемому сроку действия маркера обновления (несколько дней до истечения срока действия маркера обновления):
>
>>  1) [Завершение процесса синхронизации вручную для конфигурации соединителя ITSM](https://docs.microsoft.com/azure/azure-monitor/platform/itsmc-resync-servicenow)
 >> 2) Отозвать старый маркер обновления, так как не рекомендуется использовать старые ключи из соображений безопасности. В колонке ServiceNow Поиск системы OAuth по сравнению с выберите Управление токенами. Выберите Старый токен из списка в соответствии с именем OAuth и датой окончания срока действия. Щелкните отменять доступ и отменять.
...
| **Пароль**   | Введите пароль, связанный с этим именем пользователя. **Примечание**. имя пользователя и пароль используются только для создания маркеров проверки подлинности и не хранятся в службе ITSMC.  |
...
| **Создание нового элемента конфигурации в решении ITSM** | Выберите этот параметр, если вы хотите создать элементы конфигурации в продукте ITSM. После выбора этого параметра ITSM создаст необходимые элементы (если они еще не созданы) как элементы конфигурации в поддерживаемой системе ITSM. **По умолчанию**: отключено. |
...
| **Область синхронизации данных**   | Выберите рабочие элементы Provance, которые вы хотите синхронизировать с Azure Log Analytics через ITSMC.  Эти рабочие элементы импортируются в службу Log Analytics.   **Параметры:**   Инциденты, запросы на изменение.|
| **Синхронизация данных** | Введите число прошедших дней, которые будут учитываться при синхронизации. **Максимальное ограничение**: 120 дней. |
| **Создание нового элемента конфигурации в решении ITSM** | Выберите этот параметр, если вы хотите создать элементы конфигурации в продукте ITSM. После выбора этого параметра ITSM создаст необходимые элементы (если они еще не созданы) как элементы конфигурации в поддерживаемой системе ITSM. **По умолчанию**: отключено.|
...
2. Щелкните **Безопасность** > **изменить REST API параметры клиента**.
3. Выберите **создать новый** > **секрет клиента**клиента.
