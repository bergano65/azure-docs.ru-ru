---
title: Соединитель управления ИТ-службами в Azure Monitor
...
- ITSMC установлен. Дополнительная информация: [Добавление решения для подключения к управлению ИТ-службами](../../azure-monitor/platform/itsmc-overview.md#adding-the-it-service-management-connector-solution).
...
- **Назначай гибридное соединение** - [Нанастройку это соединение](#configure-the-hybrid-connection)вручную.
...
Сценарий создаст веб-приложение, используя имя, которое вы указали (и несколько дополнительных строк для обеспечения его уникальности). Он генерирует **URL-адрес веб-приложения,** **идентификатор клиента** и **секрет клиента.**
...
2. Выберите веб-приложение, нажмите **Настройки Настройки** > **Настройки**.
...
Выполните указанные ниже предварительные требования.
- ITSMC установлен. Дополнительная информация: [Добавление решения для подключения к управлению ИТ-службами](../../azure-monitor/platform/itsmc-overview.md#adding-the-it-service-management-connector-solution).
- ServiceNow поддерживает версии: Нью-йорк, Мадрид, Лондон, Кингстон, Джакарта, Стамбул, Хельсинки,
> [!NOTE]
> ITSMC поддерживает только официальное предложение SaaS от Service Now. Частные развертывания Service Now не поддерживаются. 

**ServiceNow Админы должны сделать следующее в своем экземпляре ServiceNow:**
- Создать идентификатор и секрет клиента для продукта ServiceNow. Дополнительные сведения о том, как создать идентификатор и секрет клиента, см. в соответствующих разделах:
    ...
    - [Настройка OAuth для Нью-йорка](https://docs.servicenow.com/bundle/newyork-platform-administration/page/administer/security/task/t_SettingUpOAuth.html)
    - [Настройка OAuth для Мадрида](https://docs.servicenow.com/bundle/madrid-platform-administration/page/administer/security/task/t_SettingUpOAuth.html)
...
| **Область синхронизации данных**   | Выберите рабочие элементы Provance, которые вы хотите синхронизировать с Azure Log Analytics через ITSMC.  Эти рабочие элементы импортируются в службу Log Analytics.   **Варианты:**   Инциденты, Запросы на изменение.|
| **Синхронизация данных** | Введите число прошедших дней, которые будут учитываться при синхронизации. **Максимальное ограничение**: 120 дней. |
| **Создание нового элемента конфигурации в решении ITSM** | Выберите этот параметр, если вы хотите создать элементы конфигурации в продукте ITSM. После выбора этого параметра ITSM создаст необходимые элементы (если они еще не созданы) как элементы конфигурации в поддерживаемой системе ITSM. **Параметр по умолчанию**: отключено.|
...
2. Нажмите настройки клиента **Security** > **Edit REST API под ими.**
3. Выберите **Создать новый клиент** > **секрет**.
