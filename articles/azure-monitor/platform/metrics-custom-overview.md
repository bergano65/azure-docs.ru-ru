...
## <a name="methods-to-send-custom-metrics"></a>Методы отправки пользовательских метрик
...
- Отправка пользовательских метрик [непосредственно в API REST Azure Monitor](../../azure-monitor/platform/metrics-store-custom-rest-api.md). `https://<azureregion>.monitoring.azure.com/<AzureResourceID>/metrics`

## <a name="pricing-model"></a>Модель ценообразования

В хранилище метрик Azure Monitor не стоит глотать стандартные метрики (метрики платформы). Пользовательские метрики, попадаемые в хранилище метрик Azure Monitor, будут выставлены счета за MByte с каждой пользовательской метиковой точкой данных, написанной как 8 байтов по размеру. Все проглоченные метрики сохраняются в течение 90 дней.

Метрические запросы будут взиматься в зависимости от количества стандартных вызовов API. Стандартный вызов API — это вызов, который анализирует 1440 точек данных (1440 — это также общее количество точек данных, которые могут храниться на метрику в день). Если вызов API анализирует более 1440 точек данных, то он будет засчитываться как несколько стандартных вызовов API. Если вызов API анализирует менее 1440 точек данных, он будет считаться менее чем одним вызовом API. Количество стандартных вызовов API рассчитывается каждый день, так как общее количество точек данных, анализируемых в день, делится на 1440.

Конкретные данные о ценах для пользовательских метрик и метрических запросов доступны на [странице ценообразования Azure Monitor.](https://azure.microsoft.com/pricing/details/monitor/)

> [!NOTE]  
> Метрики, отправленные в Azure Monitor через Application Insights SDK, будут выставлены на счет как данные журнала, и понесут дополнительные расходы только в том случае, если функция Application Insights [Включить оповещение о пользовательских измерениях метрик](https://docs.microsoft.com/azure/azure-monitor/app/pre-aggregated-metrics-log-metrics#custom-metrics-dimensions-and-pre-aggregation) и была выбрана. Узнайте больше о модели ценообразования и [ценах Вашего региона](https://azure.microsoft.com/pricing/details/monitor/)Application [Insights.](https://docs.microsoft.com/azure/azure-monitor/app/pricing#pricing-model)

> [!NOTE]  
> Проверьте [страницу ценообразования Azure Monitor,](https://azure.microsoft.com/pricing/details/monitor/) чтобы узнать, когда будет включено выставление счетов для пользовательских запросов метрик и метрик. 

## <a name="how-to-send-custom-metrics"></a>Как отправлять пользовательские метрики
...
Каждая точка данных, отправляемая в Azure Monitor, должна быть отмечена временной меткой. Эта метка времени содержит дату и время измерения или получения метрики. Azure Monitor принимает данные метрик с метками времени до 20 минут назад и до 5 минут вперед. Отметка времени должна быть в формате ISO 8601.

### <a name="namespace"></a>Пространство имен
При помощи пространств имен можно классифицировать или группировать похожие метрики. Они позволяют обеспечить изоляцию между группами метрик, которые смогут собирать разные аналитические сведения или индикаторы производительности. Например, у вас может быть пространство имен под названием **contosomemorymetrics,** которое отслеживает метрики использования памяти, которые профилют ваше приложение. Другое пространство имен, называемое **contosoapptransaction,** может отслеживать все метрики транзакций пользователей в вашем приложении.
...
Размеры неявляются, не все метрики могут иметь размеры. Пользовательская метрика может иметь до 10 измерений.
...
Размеры неявляются, не все метрики могут иметь размеры. Если столб метрики определяет ключи измерения, соответствующие значения измерения являются обязательными.
...
Например, если в приложении в течение данной минуты было четыре транзакции в входе, то измеренные просечения для каждой из них могут быть следующими:
...
> [!NOTE]  
> Azure Monitor пока не поддерживает определение **единиц** для пользовательской метрики.
...
> [!NOTE]
> Вы должны быть читателем или роль вкладчика для просмотра пользовательских метрик.
...
|Центральная Канада | https:\//canadacentral.monitoring.azure.comc
...
Используйте пользовательские метрики из различных служб: 
 ...
 - [Классические облачные сервисы](collect-custom-metrics-guestos-vm-cloud-service-classic.md)
