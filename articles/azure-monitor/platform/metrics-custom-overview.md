...
Затраты на прием стандартных метрик (метрик платформы) в хранилище метрик Azure Monitor не взимается. Плата за пользовательские метрики, принимаемые в хранилище метрик Azure Monitor, будет взиматься за МБ, при этом каждая Пользовательская точка данных метрики считается размером 8 байт. Все полученные метрики хранятся в течение 90 дней.

На запросы метрик будут начисляться номера стандартных вызовов API. Стандартный вызов API — это вызов, который анализирует 1 440 точек данных (1 440 также является общим количеством точек данных, которые могут храниться на метрику в день). Если вызов API анализирует более 1 440 точек данных, то он будет считаться несколькими стандартными вызовами API. Если вызов API анализирует менее 1 440 точек данных, он будет считаться менее чем одним вызовом API. Количество стандартных вызовов API вычисляется каждый день, так как общее количество проанализированных точек данных в день делится на 1 440.

Конкретные сведения о ценах для настраиваемых метрик и запросов метрик доступны на [странице цен на Azure Monitor](https://azure.microsoft.com/pricing/details/monitor/).

> [!NOTE]  
> Метрики, отправленные Azure Monitor через пакет SDK для Application Insights, будут оплачиваться как полученные данные журнала и взимать дополнительные метрики только в том случае, если выбрана функция Application Insights [Включить оповещения для пользовательских измерений метрик](https://docs.microsoft.com/azure/azure-monitor/app/pre-aggregated-metrics-log-metrics#custom-metrics-dimensions-and-pre-aggregation) . Дополнительные сведения о [модели ценообразования Application Insights](https://docs.microsoft.com/azure/azure-monitor/app/pricing#pricing-model) и [ценах в вашем регионе](https://azure.microsoft.com/pricing/details/monitor/).

> [!NOTE]  
> Дополнительные сведения о включении выставления счетов для настраиваемых метрик и запросов метрик см. на [странице цен на Azure Monitor](https://azure.microsoft.com/pricing/details/monitor/) . 
