...
description: В этой статье описаны разные задачи управления, которые обычно выполняются во время жизненного цикла Windows Analytics журнала или агента Linux, развернутых на компьютере.
...
После первоначального развертывания агента Linux в Azure Monitor или Log Analytics Windows может потребоваться перенастроить агент, обновить или удалить его с компьютера, он стадия прекращения использования жизненного цикла. Вы можете легко управлять этими задачами обслуживания вручную или автоматизировать этот процесс, сократив при этом эксплуатационные расходы и количество ошибок.
...
Действия, описанные в этом разделе необходимы, если требуется не только перенастроить агент Windows для отправки отчетов в другую рабочую область или удалить рабочую область из его конфигурации, но и при необходимости настроить агент для отправки отчетов в несколько рабочих областей (обычно называется множественной). Настройка агента для отправки отчетов в несколько рабочих областей Windows может выполняться только после начальной установки агента и с помощью описанных ниже методов.    
...
Следующие шаги демонстрируют, как перенастроить агент для Linux, если принято решение для его регистрации в другой рабочей области или удаление рабочей области из его конфигурации.
...
4. Чтобы проверить внесенные изменения действует, выполните следующую команду:
...
Воспользуйтесь одним из следующих процедур для удаления агента Windows или Linux, с помощью командной строки или мастера установки.
...
- Просмотрите [Устранение неполадок агента Linux](agent-linux-troubleshoot.md) при возникновении проблем во время установки или настройки агента Linux.

- Просмотрите [Устранение неполадок агента Windows](agent-windows-troubleshoot.md) при возникновении проблем во время установки или настройки агента Windows.
