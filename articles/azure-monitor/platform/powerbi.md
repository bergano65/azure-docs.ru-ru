...
2. Если вы используете портал поиска по журналам, щелкните **Power BI**.  Если вы используете портал Analytics, выберите **Экспорт** > **Power BI Query (M)**.  Оба этих параметра экспортируют запрос в текстовый файл с именем **PowerBIQuery.txt**. 
    ...
    ![Экспорт поиска по журналам](media/powerbi/export-logsearch.png) ![Экспорт поиска по журналам](media/powerbi/export-analytics.png)
...
* Узнайте больше о [запросах поиска по журналам](../../azure-monitor/log-query/log-query-overview.md) , чтобы создавать запросы, которые можно экспортировать в Power BI.
