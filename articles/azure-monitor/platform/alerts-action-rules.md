...
* **Уровень серьезности**. Возможность выбора одного или нескольких серьезности оповещений. **Severity = Sev1** означает, что правило действия применимо ко всем предупреждениям, установленным в Sev1.
* **Служба мониторинга**: Фильтр, основанный на исходной службе мониторинга. Этот фильтр также является множественным выделением. Например, **Monitor Service = "Application Insights"** означает, что правило действия применимо ко всем оповещениям на основе Application Insights.
...
* **Идентификатор правила оповещения**: Параметр для фильтрации конкретных правил генерации оповещений с помощью диспетчер ресурсов идентификатора правила генерации оповещений.
...
* **С Now (всегда)** : Подавляет все уведомления в течение неограниченного времени.
...
### <a name="scenario-1-suppression-of-alerts-based-on-severity"></a>Сценарий 1. Подавление предупреждений на основе серьезности
...
### <a name="scenario-2-suppression-of-alerts-based-on-alert-context-payload"></a>Сценарий 2. Подавление предупреждений на основе контекста предупреждения (полезные данные)
...
* Пересечение: Например, определяемое вами правило действия находится в **VM1** и **VM2**, а перекрытие правила действия — на **VM2** и **VM3**.
...
* Пересечение: Например, определяемое вами правило генерации оповещений находится в **VM1** и **VM2**, а правило действия — в **VM2** и **VM3**.
