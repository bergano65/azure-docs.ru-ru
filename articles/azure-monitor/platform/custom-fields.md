...
1. Выделите текст в примере записи, для которой требуется заполнить настраиваемое поле.  Затем откроется диалоговое окно, в котором можно указать имя и тип данных для поля и выполнить начальное извлечение.  Символы ** \_CF** будут автоматически добавлены.
