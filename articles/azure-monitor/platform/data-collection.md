...
**Метрики платформы** создаются ресурсами Azure и наглядно отображают работоспособность и производительность этих ресурсов. Каждый тип ресурсов создает [отдельный набор метрик](../../azure-monitor/platform/metrics-supported.md), который не нужно настраивать. 
...
- С помощью [обозревателя метрик](../../azure-monitor/platform/metrics-charts.md) можно анализировать собранные метрики и отображать их на диаграмме. Отслеживать производительность ресурса (например, виртуальной машины, веб-сайта или логического приложения), закрепляя диаграммы на [панели мониторинга Azure](../../azure-portal/azure-portal-dashboards.md).
...
- С помощью [автомасштабирования](../../azure-monitor/platform/autoscale-overview.md) можно увеличивать или уменьшать ресурсы, когда метрика переходит пороговое значение.
...
- Выполнять потоковую передачу метрик в [концентратор событий](../../azure-monitor/platform/stream-monitoring-data-event-hubs.md), чтобы маршрутизировать их в службу [Azure Stream Analytics](../../stream-analytics/stream-analytics-introduction.md) или во внешние системы.
...
- Получать доступ к значениям метрик из командной строки или пользовательского приложения с помощью [командлетов PowerShell](https://docs.microsoft.com/powershell/module/azurerm.insights/?view=azurermps-6.7.0) или [REST API](../../azure-monitor/platform/rest-api-walkthrough.md).
...
Как было сказано выше, данные метрик используется различными способами. С помощью [обозревателя метрик](../../azure-monitor/platform/metrics-charts.md) можно анализировать данные непосредственно в хранилище метрик и отображать на диаграмме изменение значений нескольких метрик во времени. Вы можете просматривать графики в интерактивном режиме или закрепить их на панели мониторинга, чтобы просматривать их вместе с другими визуализациями. Кроме того, вы можете извлечь метрики с помощью [REST API мониторинга Azure](../../azure-monitor/platform/rest-api-walkthrough.md).

![Обозреватель метрик](media/data-collection/metrics-explorer.png)
...
- [Журналы действий](collect-activity-logs.md) из ресурсов Azure, которые включают информацию о конфигурации и работоспособности ресурсов, и [журналы диагностики](../../azure-monitor/platform/diagnostic-logs-stream-log-store.md), предоставляющие аналитические данные о работе ресурсов.
...
Azure Monitor получает все данные журнала с помощью [запроса журнала](../log-query/log-query-overview.md), написанного на [языке запросов Data Explorer](../log-query/get-started-queries.md), который позволяет быстро получить, консолидировать и проанализировать собранные данные. В [Log Analytics](../log-query/portals.md) на портале Azure можно создавать и тестировать запросы. Вы можете работать с результатами в интерактивном режиме или закрепить их на панели мониторинга, чтобы просматривать их вместе с другими визуализациями. Кроме того, вы можете извлечь журналы с помощью [REST API мониторинга Azure](../../monitoring-and-diagnostics/monitoring-rest-api-walkthrough.md).

> [!IMPORTANT]
> Данные из Application Insights хранятся в отдельном разделе, а не вместе с другими данными журнала в Azure Monitor. Это позволяет использовать такие же функции, что и для других данных журнала, но для доступа к этим данным необходимо использовать [консоль Application Insights](/azure-monitor/app/analytics.md) или [API Application Insights](https://dev.applicationinsights.io/). Для анализа данных приложений вместе с другими данными журналов можно использовать [межресурсный запрос](../log-query/cross-workspace-query.md).
...
Описание этой возможности приведено в статье [Создание оповещений о метриках для журналов в Azure Monitor](../../azure-monitor/platform/alerts-metric-logs.md). Со списком поддерживаемых значений можно ознакомиться в статье [Метрики, поддерживаемые Azure Monitor](../../azure-monitor/platform/metrics-supported.md#microsoftoperationalinsightsworkspaces).
...
Сведения о различных типах данных мониторинга см. в статье [Потоковая передача данных мониторинга Azure в концентратор событий для потребления внешним инструментом](../../azure-monitor/platform/stream-monitoring-data-event-hubs.md).
