...
Средство выбора области ресурсов позволяет просматривать метрики для одного и нескольких ресурсов. Ниже приведены инструкции по использованию средства выбора области ресурсов. 

### <a name="selecting-a-single-resource"></a>Выбор одного ресурса
Выберите **Метрики** в меню **Azure Monitor** или в разделе **Мониторинг** в меню ресурса. Нажмите кнопку "выбрать область", чтобы открыть средство выбора области, которое позволит выбрать ресурсы, для которых нужно просмотреть метрики. Этот параметр уже должен быть заполнен, если обозреватель метрик был открыт из меню ресурса. 

![Снимок экрана: выбор области ресурсов](./media/metrics-charts/scope-picker.png)

Для некоторых ресурсов одновременно можно просматривать только метрики одного ресурса. Эти ресурсы находятся в разделе "все типы ресурсов" в раскрывающемся списке "типы ресурсов".

![Снимок экрана: один ресурс](./media/metrics-charts/single-resource-scope.png)

Щелкнув нужный ресурс, вы увидите все подписки и группы ресурсов, содержащие этот ресурс.

![Снимок экрана доступных ресурсов](./media/metrics-charts/available-single-resource.png)

> [!TIP]
> Если вы хотите одновременно просмотреть метрики нескольких ресурсов или метрики в подписке или группе ресурсов, нажмите кнопку проголосует.

Выбрав нужный вариант, нажмите кнопку "Применить".

### <a name="viewing-metrics-across-multiple-resources"></a>Просмотр метрик в нескольких ресурсах
Некоторые типы ресурсов позволяют запрашивать метрики по нескольким ресурсам, если они находятся в одной подписке и расположении. Эти типы ресурсов можно найти в верхней части раскрывающегося списка "типы ресурсов". Чтобы получить дополнительные сведения о просмотре метрик по нескольким ресурсам, просмотрите [этот документ](metrics-dynamic-scope.md#selecting-multiple-resources).

![Снимок экрана с перекрестными типами ресурсов](./media/metrics-charts/multi-resource-scope.png)

Для типов, совместимых с несколькими ресурсами, можно также запросить метрики в подписке или нескольких группах ресурсов. Чтобы узнать, как это сделать, ознакомьтесь с [этой статьей](metrics-dynamic-scope.md#selecting-a-resource-group-or-subscription)
...
![Снимок экрана: сумма запроса](./media/metrics-charts/request-sum.png)
...
![Снимок экрана: количество запросов](./media/metrics-charts/request-count.png)
...
![Снимок экрана: минимальный запрос](./media/metrics-charts/request-min.png)

**Max** — наибольшее значение, захваченное за интервал агрегирования

![Снимок экрана запроса Max](./media/metrics-charts/request-max.png)
...
3. Выберите значения измерения, которые нужно включить при создании диаграммы (в этом примере показаны отфильтрованные успешные транзакции с хранилищем):
   ...
   ![Снимок экрана, на котором показана фильтрация из успешных транзакций хранилища.](./media/metrics-charts/029.png)
...
2. Выберите измерения, по которому необходимо сегментировать диаграммы:
   ...
   ![Снимок экрана, на котором показано выбранное измерение, на котором разсегментирована диаграмма.](./media/metrics-charts/031.png)

   Теперь на диаграмме отображаются несколько строк, по одной для каждого сегмента измерения:

   ![Снимок экрана, на котором показаны несколько линий, по одному для каждого сегмента измерения.](./media/metrics-charts/032.png)
...
Для управления диапазоном оси Y используйте меню диаграммы меню диаграммы и выберите **Параметры диаграммы** для доступа к дополнительным параметрам диаграммы.

![Снимок экрана, посвященный параметру "Параметры диаграммы".](./media/metrics-charts/033.png)

 Измените значения в разделе "Диапазон оси Y" или нажмите кнопку **Автоматически**, чтобы вернуть значения по умолчанию.
...
После настройки диаграммы щелкните **закрепить на панели мониторинга** в правом верхнем углу диаграммы.

![Снимок экрана, показывающий, как закрепить на диаграмме](./media/metrics-charts/036.png)
...
![Кнопка "Создать правило генерации оповещений", выделенная красным цветом](./media/metrics-charts/042.png)
...
![Создание правила генерации оповещений](./media/metrics-charts/041.png)
...
## <a name="next-steps"></a>Дальнейшие действия
  ...
  Дополнительные сведения о рекомендациях по созданию готовых к работе панелей мониторинга с метриками см. в статье [Create custom KPI dashboards using Azure Application Insights](../learn/tutorial-app-dashboards.md) (Создание пользовательских панелей мониторинга KPI с помощью Azure Application Insights).
