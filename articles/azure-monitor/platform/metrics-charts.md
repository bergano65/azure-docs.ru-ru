...
description: Узнайте о расширенных функциях Azure Monitor Metrics Explorer
author: vgorbenko
...
ms.locfileid: "80371602"
---
# <a name="advanced-features-of-azure-metrics-explorer"></a>Расширенные возможности обозревателя метрик Azure

> [!NOTE]
> Эта статья предполагает, что вы знакомы с основными функциями Metrics Explorer. Если вы являетесь новым пользователем и хотите узнать, как создать свою первую метрическую диаграмму, [см.](metrics-getting-started.md)

## <a name="metrics-in-azure"></a>Метрики в Azure

[Метрики в Azure Monitor](data-platform-metrics.md) — это ряд измеренных значений и счетчиков, которые собираются и сохраняются в течение определенного времени. Существуют стандартные метрики (или "метрики платформы") и настраиваемые метрики. Стандартные метрики предоставляет платформа Azure. Стандартные метрики показывают работоспособность и статистику потребления ресурсов Azure. В то время как пользовательские метрики отправляются в Azure вашими приложениями с помощью [API Application Insights для пользовательских событий и метрик,](https://docs.microsoft.com/azure/application-insights/app-insights-api-custom-events-metrics) [расширения Windows Azure Diagnostics (WAD)](https://docs.microsoft.com/azure/azure-monitor/platform/diagnostics-extension-overview)или [API Azure Monitor REST.](https://docs.microsoft.com/azure/azure-monitor/platform/metrics-store-custom-rest-api)

## <a name="create-views-with-multiple-metrics-and-charts"></a>Создание представлений с несколькими метриками и диаграммами

Можно создать диаграммы, которые населяют несколько строк метрик или показывают несколько метрических диаграмм одновременно. Эта функция позволяет:

- соотнести связанные метрики на том же графике, чтобы увидеть, как одно значение связано с другим
- отображение метрик с различными единицами измерения в непосредственной близости
- визуально ежектом и сравнению метрик из нескольких ресурсов

Например, если у вас есть 5 учетных записей хранения и вы хотите знать, сколько общего пространства потребляется между ними, можно создать (сложенную) диаграмму области, которая показывает индивидуальность и сумму всех значений в определенных точках времени.

### <a name="multiple-metrics-on-the-same-chart"></a>Несколько метрик на одной диаграмме

Во-первых, [создать новый график](metrics-getting-started.md#create-your-first-metric-chart). Нажмите **Добавить метрики** и повторить шаги, чтобы добавить другую метрику на той же диаграмме.
...
### <a name="multiple-charts"></a>Несколько графиков

Нажмите **на диаграмму Добавить** и создайте другую диаграмму с другой метрикой.

### <a name="order-or-delete-multiple-charts"></a>Заказать или удалить несколько диаграмм

Чтобы заказать или удалить несколько диаграмм, нажмите на эллипсы **(...)** символ, чтобы открыть меню диаграммы и выбрать соответствующий пункт меню **Переместить вверх**, **Переместить вниз**, или **Удалить**.
...
## <a name="apply-splitting-to-a-chart"></a>Применить расщепление на диаграмму
...
1. Щелкните **Применить разделение** над диаграммой.
   ...
   > [!NOTE]
   > Разделение не может быть использовано с диаграммами, которые имеют несколько метрик. Кроме того, вы можете иметь несколько фильтров, но только одно измерение разделения применяется к любой одной диаграмме.
...
## <a name="change-colors-of-chart-lines"></a>Изменение цветов линий диаграммы

После настройки диаграмм строки диаграммы автоматически присваиваются цвету из палитры по умолчанию. Вы можете изменить эти цвета.

Чтобы изменить цвет линии диаграммы, нажмите на цветную панель в легенде, которая соответствует диаграмме. Диалог сборщика цветов откроется. Используйте сборщик цветов для настройки цвета для линии.

После настройки цветов диаграммы они останутся такими, когда вы прикрепите диаграмму к панели мониторинга. В следующем разделе показано, как закрепить диаграмму.

> [!NOTE]
> Из-за ограничений нашего графика выпуска и публикации, изменение цветов линий диаграммы временно требует прохождения [https://portal.azure.com/?feature.colorpicker=true](https://portal.azure.com/?feature.colorpicker=true)специального параметра **?feature.colorpicker-true** при запуске портала Azure . В ближайшее время эти ограничения будут устранены. 
