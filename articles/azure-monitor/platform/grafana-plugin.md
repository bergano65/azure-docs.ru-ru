...
Чтобы настроить локальный сервер Grafana, [скачайте и установите Grafana в локальной среде](https://grafana.com/grafana/download). Чтобы использовать интеграцию Azure Monitor плагина, установите версию Grafana 5.3 или выше.
...
## <a name="sign-in-to-grafana"></a>Войти на Грагану
...
2. Вопинье с *администратором* имени пользователя и паролем администратора сервера Grafana, созданным ранее. Если вы используете локальную настройку, пароль по умолчанию будет *admin*. При первом входе в систему его нужно будет изменить.
...
4. Передайте сведения о подключении в интерфейсы API, которые вы хотите использовать. Можно подключиться ко всем или некоторым из них. 
    * При подключении к метрикам и журналам в Azure Monitor можно повторно использовать одни и те же учетные данные, выбрав те же данные, что и **API Azure Monitor.**
...
4. Выберите источник данных Azure Monitor, который вы настроили.
   ...
   * Сбор данных журнала Azure Monitor - выберите **аналитику журналов Azure** в службе. Выберите рабочую область, в которой вы хотите выполнять запросы и настраивать текст запроса. Вы можете скопировать здесь любой запрос журнала у вас уже есть или создать новый. При вводе запроса будет отображаться IntelliSense с предлагаемыми вариантами автозаполнения. Выберите тип визуализации, **Time series** **Table** (Таблица временных рядов), а затем выполните запрос.
