...
- Просмотрите журнал действий в портал Azure и доступе к событиям из PowerShell и CLI. Дополнительные сведения см. [в разделе Просмотр журнала действий](activity-log.md#view-the-activity-log) . 
...
| Назначение | Описание |
|:---|:---|
| Рабочая область Log Analytics | Проанализируйте журналы всех ресурсов Azure вместе и воспользуйтесь всеми возможностями, доступными для [Azure Monitor журналов](data-platform-logs.md) , включая [запросы журналов](../log-query/log-query-overview.md) и [оповещения журналов](alerts-log.md). Закрепите результаты запроса журнала на панели мониторинга Azure или включите ее в книгу как часть интерактивного отчета. |  |
