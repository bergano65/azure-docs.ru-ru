...
### <a name="azure-activity-logs-resource-logs-and-metrics"></a>Журналы активности Azure, журналы ресурсов и метрики
Отправка данных Azure для обработки в точку приема Log Analytics занимает некоторое время:

- Данные из журналов ресурсов занимают 2-15 минут, в зависимости от службы Azure. Ознакомьтесь с [запросом ниже](#checking-ingestion-time), чтобы изучить задержку в своей среде.
...
В различных условиях время приема может меняться для разных ресурсов. Запросы журналов можно использовать для идентификации конкретного поведения среды. В следующей таблице указывается, как можно определить разное время для записи по мере ее создания и отправки в Azure Monitor.

| Шаг | Свойство или функция | Комментарии |
|:---|:---|:---|
| Запись, созданная в источнике данных | [TimeGenerated](log-standard-properties.md#timegenerated-and-timestamp) <br>Если источник данных не устанавливает это значение, то оно будет установлено в то же время, что и _TimeReceived. |
| Запись, полученная конечным пунктом azure Monitor | [_TimeReceived](log-standard-properties.md#_timereceived) | |
| Запись, хранящаяся в рабочем пространстве и доступная для запросов | [ingestion_time()](/azure/kusto/query/ingestiontimefunction) | |

### <a name="ingestion-latency-delays"></a>Задержки приема данных
Можно измерить задержку определенной записи, сравнив результат [ingestion_time()](/azure/kusto/query/ingestiontimefunction) функции с свойством _TimeGenerated._ Эти данные можно использовать в различных агрегатах, чтобы определить поведение при задержке приема данных. Изучите некоторый процентиль времени приема для получения аналитических сведений по большому объему данных. 

Например, следующий запрос покажет вам, какие компьютеры имели наибольшее время приема за предыдущие 8 часов: 
...
Предыдущие проверки процентиля хороши для поиска общих тенденций в задержке. Для определения краткосрочного всплеска задержки, использование`max()`максимума ( ) может быть более эффективным.

Если вы хотите просверлить время приема для определенного компьютера в течение определенного периода времени, используйте следующий запрос, который также визуализирует данные за прошедший день на графике: 
...
Используйте следующий запрос, чтобы показать время приема компьютера по стране/региону, в котором они расположены на основе их IP-адреса: 
