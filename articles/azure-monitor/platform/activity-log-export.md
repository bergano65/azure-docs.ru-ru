...
**Как долго журнал действий должен храниться в учетной записи хранения.** Срок хранения 0 дней означает, что журналы хранятся неограниченно долго. В противном случае укажите количество дней в диапазоне от 1 до 2 147 483 647.
...
3. Создайте профиль журнала с помощью `Add-AzLogProfile`:
    ...
    | RetentionInDays |Да |Число дней, в течение которых события должны храниться в учетной записи хранения в диапазоне от 1 до 2147483647. Нулевое значение означает, что журналы хранятся неограниченно долго. |
    | Category |Нет |Разделенный запятыми список категорий событий, которые будут собираться. Возможные значения: _Write_, _Delete_и _Action_. |
...
3. Создайте профиль журнала с помощью `az monitor log-profiles create`:
   ...
   ```
    ...
    | дн. |Да |Число дней, в течение которых должны храниться события, от 1 до 365. Нулевое значение означает, что журналы будут храниться неограниченно долго, то есть всегда.  Если значение равно нулю, для включенного параметра нужно установить значение true. |
    ...
    | categories |Да |Разделенный пробелами список категорий событий, которые будут собираться. Возможные значения: Write, Delete или Action. |
...
| уровень |Уровень события. Одно из следующих значений: _Критическая_, _Ошибка_, _предупреждение_, _информационное_и _подробное_ сообщение |
| расположение |Регион, к которому принадлежит расположение (или глобальное местоположение). |
