---
title: Справочный справочник по частям посмотреть конструктор атакжем в Azure Monitor (ru) Документы Майкрософт
description: Используя View Designer в Azure Monitor, можно создать пользовательские представления, которые отображаются на портале Azure и содержат различные визуализации данных в рабочей области Log Analytics. В этой статье содержится справка по параметрам элементов визуализации, доступных в пользовательских представлениях.
ms.subservice: logs
ms.topic: conceptual
author: bwren
ms.author: bwren
...
ms.locfileid: "77658563"
---
# <a name="reference-guide-to-view-designer-visualization-parts-in-azure-monitor"></a>Справочный справочник по визуализации деталей для просмотра дизайнера в Azure Monitor
Используя View Designer в Azure Monitor, можно создать различные пользовательские представления на портале Azure, которые помогут вам визуализировать данные в рабочем пространстве Log Analytics. В этой статье содержится справка по параметрам элементов визуализации, доступных в пользовательских представлениях.

Дополнительные сведения о конструкторе представлений см. в следующих ресурсах:

* [Использование конструктора представлений для создания пользовательских представлений Log Analytics](view-designer.md) — обзор конструктора представлений и процедур создания и редактирования пользовательских представлений.
...
|:--- |:--- |
| [список запросов.](#list-of-queries-part) |Отображает список запросов журналов. Вы можете выбрать каждый запрос, чтобы отобразить его результаты. |
| [Число и список](#number-and-list-part) |Заголовок отображает одно число, которое показывает количество записей из запроса журнала. В списке отображаются десять самых показательных результатов запросов. На графике представлено относительное значение числового столбца или его изменение за некоторый промежуток времени. |
| [Два числа и список](#two-numbers-and-list-part) |Заголовок отображает два числа, которые показывают количество записей из отдельных запросов журнала. В списке отображаются десять самых показательных результатов запросов. На графике представлено относительное значение числового столбца или его изменение за некоторый промежуток времени. |
...
| [Информация](#information-part) |В верхней части содержится статический текст и необязательная ссылка. В списке отображается один или несколько элементов со статическим названием и текстом. |
...
> [!NOTE]
> Части в представлениях основаны на [запросах журналов](../log-query/log-query-overview.md) в рабочем пространстве журнала Analytics. В настоящее время они не поддерживают [перекрестные запросы ресурсов](../log-query/cross-workspace-query.md) для получения данных из Application Insights.

## <a name="list-of-queries-part"></a>Представление "Список запросов"
В части списка запросов отображается список запросов журнала. Вы можете выбрать каждый запрос, чтобы отобразить его результаты. Представление содержит один запрос по умолчанию. Выберите **+ Query** (+ Запрос), чтобы добавить дополнительные запросы.
...
|:--- |:--- |
| **Общие** | |
| Заголовок |Текст, отображаемый в верхней части представления. |
...
Заголовок отображает одно число, которое показывает количество записей из запроса журнала. В списке отображаются десять самых показательных результатов запросов. На графике представлено относительное значение числового столбца или его изменение за некоторый промежуток времени.
...
| Color |Цвет линий или спарклайнов. |
...
| name |Текст, отображаемый в верхней части первого столбца. |
...
Заголовок имеет два номера, отображающие количество записей из отдельных запросов журнала. В списке отображаются десять самых показательных результатов запросов. На графике представлено относительное значение числового столбца или его изменение за некоторый промежуток времени.
...
| Значок использования |Щелкните эту ссылку, чтобы отобразить значок. |
| **Заголовка** | |
| Заголовок |Текст, отображаемый в верхней части заголовка. |
...
| Навигация по щелчку | Действие, выполняемое при щелчке заголовка.  Дополнительные сведения см. в разделе [Общие параметры](#click-through-navigation). |
| **Пончик** |**> Центр** |
| Text |Текст, отображаемый под значением на кольцевой диаграмме. |
...
| Color |Цвет, используемый для столбцов в ряду. |
...
| Имя группы |Текст, отображаемый в верхней части плитки. |
| Новая группа |Щелкните эту ссылку, чтобы создать группу в представлении, начиная с текущего. |
| Color |Цвет фона для заголовка. |
| **Заголовка** | |
| Изображение |Файл изображения, отображаемый в верхней части. |
| Метка |Текст, отображаемый в верхней части. |
| **Заголовка** |**> Ссылка** |
| Метка |Текст ссылки. |
| Url |URL-адрес для ссылки. |
| **Элементы информации** | |
| Заголовок |Текст, отображаемый в качестве имени для каждого элемента. |
...
![Стек графиков](media/view-designer-parts/view-stack-line-charts.png)
...
| Заголовок |Текст, отображаемый в верхней строке диаграммы. |
...
### <a name="name-and-value-separator"></a><a name="name-value-separator"></a>Разделитель имени и значения
Разделитель имени и значения — это односимвольный разделитель, разбивающий свойства текста из запроса списка на несколько значений. Если вы указываете делимитер, можно указать имена для каждого поля, разделенного тем же делимитетом в поле **Name.**
...
### <a name="click-through-navigation"></a><a name="click-through-navigation"></a>Навигация по щелчку
Навигация по щелчку позволяет определить, какое действие будет выполнено, если щелкнуть заголовок или элемент списка в представлении.  Это либо откроет запрос в [журнале Analytics,](../../azure-monitor/log-query/portals.md) либо запустит другое представление.

В следующей таблице описаны параметры для навигации по щелчку:

| Параметр           | Описание |
|:--|:--|
| Поиск по журналам (автоматически) | Запрос входа для запуска при выборе элемента заголовка.  Это тот же запрос журнала, на который основан элемент.
| Поиск по журналам        | Запрос входа для запуска при выборе элемента в списке.  Введите запрос в поле **Запрос навигации**.   Используйте *{выбранный элемент}*, чтобы включить синтаксис для элемента, выбранного пользователем.  Например, если запрос содержит столбец *Computer*, а запрос навигации — *{выбранный элемент}*, тогда при выборе компьютера выполняется запрос *Computer="MyComputer"*. Если запрос навигации имеет форму запроса *Type=Event {выбранный элемент}*, выполняется запрос в форме *Type=Event Computer="MyComputer"*. |
| Представление              | Представление, которое открывается при выборе элемента заголовка или элемента в списке.  В поле **Имя представления** укажите имя представления в рабочей области. |



### <a name="sparklines"></a><a name="sparklines"></a>Спарклайны
...
### <a name="thresholds"></a><a name="thresholds"></a>Пороговые значения
...
| name |Имя порогового значения. |
| Порог |Пороговое значение. Для цвета работоспособности каждого элемента списка задается цвет наибольшего порогового значения, превышенного значением элемента. Если пороговые значения не превышены, используется стандартный цвет. |
| Color |Цвет, определяющий пороговое значение. |

## <a name="next-steps"></a>Дальнейшие действия
* Узнайте о [запросах журнала](../log-query/log-query-overview.md) для поддержки запросов в частях визуализации.
