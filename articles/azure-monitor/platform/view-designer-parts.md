...
|:--- |:--- |
| [список запросов.](#list-of-queries-part) |Отображает список запросов к журналу. Вы можете выбрать каждый запрос, чтобы отобразить его результаты. |
| [Число и список](#number-and-list-part) |В заголовке отображается одно число, показывающее количество записей в запросе журнала. В списке отображаются десять самых показательных результатов запросов. На графике представлено относительное значение числового столбца или его изменение за некоторый промежуток времени. |
| [Два числа и список](#two-numbers-and-list-part) |В заголовке отображаются два числа, отображающие количество записей из отдельных запросов журнала. В списке отображаются десять самых показательных результатов запросов. На графике представлено относительное значение числового столбца или его изменение за некоторый промежуток времени. |
...
| [Данные](#information-part) |В верхней части содержится статический текст и необязательная ссылка. В списке отображается один или несколько элементов со статическим названием и текстом. |
...
> [!NOTE]
> Части в представлениях основаны на [запросах журналов](../log-query/log-query-overview.md) в рабочей области log Analytics. В настоящее время они не поддерживают [запросы перекрестных ресурсов](../log-query/cross-workspace-query.md) для получения данных из Application Insights.

## <a name="list-of-queries-part"></a>Представление "Список запросов"
В разделе Список запросов отображается список запросов к журналу. Вы можете выбрать каждый запрос, чтобы отобразить его результаты. Представление содержит один запрос по умолчанию. Выберите **+ Query** (+ Запрос), чтобы добавить дополнительные запросы.
...
В заголовке отображается одно число, показывающее количество записей в запросе журнала. В списке отображаются десять самых показательных результатов запросов. На графике представлено относительное значение числового столбца или его изменение за некоторый промежуток времени.
...
| query |Запрос на выполнение для верхней части. Отобразится счетчик количества записей, возвращенных запросом. |
...
Заголовок содержит два числа, которые отображают число записей из отдельных запросов журнала. В списке отображаются десять самых показательных результатов запросов. На графике представлено относительное значение числового столбца или его изменение за некоторый промежуток времени.
...
| **Кольцо** | |
| query |Запрос на выполнение для кольцевой диаграммы. Первое свойство — это текстовое значение, а второе — числовое. |
| Навигация по щелчку | Действие, выполняемое при щелчке заголовка.  Дополнительные сведения см. в разделе [Общие параметры](#click-through-navigation). |
| **Кольцо** |**> Центр** |
...
| **Список** | |
| query |Запрос на выполнение для списка. Отобразится счетчик количества записей, возвращенных запросом. |
| Скрытие графика |Щелкните эту ссылку, чтобы отключить график справа от числового столбца. |
| Включение спарклайнов |Щелкните эту ссылку, чтобы отобразить спарклайн вместо горизонтальной линии. Дополнительные сведения см. в разделе [Общие параметры](#sparklines). |
| Цвет |Цвет линий или спарклайнов. |
| Операция |Операция, которую нужно выполнить для спарклайна. Дополнительные сведения см. в разделе [Общие параметры](#sparklines). |
...
| query |Запрос на выполнение для первого ряда. Счетчик записей за каждый временной интервал представлен в виде столбцов диаграммы. |
...
Разделитель имени и значения — это односимвольный разделитель, разбивающий свойства текста из запроса списка на несколько значений. При указании разделителя можно указать имена для каждого поля, разделяя их одним разделителем в поле **имя** .
...
Навигация по щелчку позволяет определить, какое действие будет выполнено, если щелкнуть заголовок или элемент списка в представлении.  Это приведет к открытию запроса в [log Analytics](../../azure-monitor/log-query/portals.md) или запуску другого представления.

В следующей таблице описаны параметры для навигации по щелчку:

| Параметр           | Описание |
|:--|:--|
| Поиск по журналам (автоматически) | Запрос журнала для запуска при выборе элемента заголовка.  Это тот же запрос журнала, на котором основан элемент.
| Поиск по журналам        | Запрос журнала для запуска при выборе элемента в списке.  Введите запрос в поле **Запрос навигации**.   Используйте *{выбранный элемент}*, чтобы включить синтаксис для элемента, выбранного пользователем.  Например, если запрос содержит столбец *Computer*, а запрос навигации — *{выбранный элемент}*, тогда при выборе компьютера выполняется запрос *Computer="MyComputer"*. Если запрос навигации имеет форму запроса *Type=Event {выбранный элемент}*, выполняется запрос в форме *Type=Event Computer="MyComputer"*. |
...
* Сведения о [запросах журналов](../log-query/log-query-overview.md) для поддержки запросов в частях визуализации.
