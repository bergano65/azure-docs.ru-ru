---
title: Справочное руководство по части конструктора представлений в Azure Monitor | Документация Майкрософт
...
ms.locfileid: "61341929"
---
# <a name="reference-guide-to-view-designer-visualization-parts-in-azure-monitor"></a>Справочник по элементам визуализации конструктора представлений в Azure Monitor
С помощью конструктора представлений в Azure Monitor, можно создать различные пользовательские представления на портале Azure, которые могут помочь визуализировать данные в рабочую область Log Analytics. В этой статье содержится справка по параметрам элементов визуализации, доступных в пользовательских представлениях.
...
* [Справочник по плиткам](view-designer-tiles.md). Справочник по параметрам каждой плитки, доступной в пользовательских представлениях.
...
|:--- |:--- |
| [Список запросов](#list-of-queries-part) |Отображает список журнала запросов. Вы можете выбрать каждый запрос, чтобы отобразить его результаты. |
| [Число и список](#number-and-list-part) |В заголовке отображается одно число, представляющее количество записей из запроса журнала. В списке отображаются десять самых показательных результатов запросов. На графике представлено относительное значение числового столбца или его изменение за некоторый промежуток времени. |
| [Два числа и список](#two-numbers-and-list-part) |В заголовке отображается два числа, представляющие количество записей из отдельных запросов по журналу. В списке отображаются десять самых показательных результатов запросов. На графике представлено относительное значение числового столбца или его изменение за некоторый промежуток времени. |
...
| Создать группу |Щелкните эту ссылку, чтобы создать группу в представлении, начиная с текущего. |
...
| Скрыть диаграмму |Щелкните эту ссылку, чтобы отключить график справа от числового столбца. |
...
| Включить пороговые значения |Щелкните эту ссылку, чтобы включить пороговые значения. Дополнительные сведения см. в разделе [Общие параметры](#thresholds). |
...
| Значения результатов, используемые при работе центра |При необходимости щелкните знак "плюс" (+), чтобы добавить одно или несколько значений. Результаты запроса ограничены записями со значениями свойств, которые вы указали. Если значения не добавлены, в запрос включаются все записи. |
...
| **Верхняя часть** | |
| Image |Файл изображения, отображаемый в верхней части. |
...
| Параметр | Описание |
|:--- |:--- |
| Включить спарклайны |Щелкните эту ссылку, чтобы отобразить спарклайн вместо горизонтальной линии. |
...
| Параметр | Описание |
|:--- |:--- |
| Включить пороговые значения |Щелкните эту ссылку, чтобы отобразить слева от каждого значения цветной значок. Значок указывает значение работоспособности относительно указанных пороговых значений. |
