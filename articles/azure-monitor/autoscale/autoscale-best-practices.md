...
* Все ошибки автомасштабирования записываются в журнал действий. Затем можно настроить [оповещение журнала действий](../alerts/activity-log-alerts.md), чтобы в случае сбоя автоматического масштабирования получать уведомления при помощи электронной почты, SMS, или веб-перехватчиков.
...
> [!NOTE]
> Если модуль автомасштабирования обнаруживает неустойчивый в результате масштабирования до целевого количества экземпляров, он также пытается масштабироваться на разное количество экземпляров между текущим числом и целевым числом. Если неустойчивый не встречается в этом диапазоне, автомасштабирование продолжит операцию масштабирования с новым целевым объектом.
...
* Автомасштабирование обнаруживает неустойчивый и прерывает попытки масштабирования. В этой ситуации вы увидите тип журнала `Flapping` . Если вы видите это, определите, являются ли пороговые значения слишком узкими.
* Автомасштабирование обнаруживает неустойчивый, но по-прежнему может успешно масштабироваться. В этой ситуации вы увидите тип журнала `FlappingOccurred` . Если вы видите это, подсистема автомасштабирования попыталась масштабировать (например, с 4 экземпляров до 2), но определила, что это вызовет неустойчивый. Вместо этого модуль автомасштабирования масштабируется до различного количества экземпляров (например, при использовании 3 экземпляров вместо 2), что больше не вызывает неустойчивый, поэтому масштабируется до этого числа экземпляров.
