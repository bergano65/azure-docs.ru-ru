...
Конфиденциальные вычисления — это отраслевой термин, определенный [Консорциумом конфиденциальных вычислений (Confidential Computing Consortium, CCC)](https://confidentialcomputing.io/), который создан специально для определения и развития этой концепции. Конфиденциальные вычисления защищают используемые данные при выполнении вычислений. Такие вычисления выполняются в доверенной среде выполнения (TEE) с аппаратной поддержкой.
...
### <a name="enclaves-and-trusted-execution-environments"></a>Анклавы и доверенные среды выполнения

В контексте конфиденциальных вычислений среды TEE часто называются *анклавами* или *защищенными анклавами*. Анклав — это защищенный раздел аппаратного обеспечения: процессора и памяти. Даже с помощью отладчика невозможно просматривать данные, размещенные в анклаве. Если недоверенный код попытается изменить содержимое памяти анклава, среда будет заблокирована, а любые операции отклонены.
...
**Основное приложение** — это приложение анклава, которое выполняется в недоверенной среде. Код основного приложения не имеет доступа к коду, загруженному в анклав. 

**Анклав** — это место выполнения кода и обработки данных в реализации TEE. Все защищенные вычисления нужно выполнять только в анклаве, чтобы обеспечить защиту секретов и конфиденциальных данных. 

При разработке приложения с поддержкой анклава необходимо правильно определить, какой код и какие данные нуждаются в защите. Код, выбранный для переноса в доверенный компонент, изолируется от остальной части приложения. После инициализации анклава и загрузки кода в память этот код становится недоступным для чтения или изменения за пределами защищенной среды.
