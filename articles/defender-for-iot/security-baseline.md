---
title: Базовый план безопасности Azure для защитника Azure для IoT
description: Базовый план безопасности защитника Azure для IoT содержит практические руководства и ресурсы для реализации рекомендаций по безопасности, указанных в статье о производительности системы безопасности Azure.
author: msmbaldwin
...
ms.locfileid: "94974973"
---
# <a name="azure-security-baseline-for-azure-defender-for-iot"></a>Базовый план безопасности Azure для защитника Azure для IoT

Этот базовый план безопасности применяет рекомендации из [контрольного показателя безопасности Azure версии 2,0](../security/benchmarks/overview.md) к Microsoft Azure Защитнику для IOT. Azure Security Benchmark содержит рекомендации по обеспечению безопасности облачных решений в Azure. Содержимое группируются по **элементам управления безопасностью** , определенным в ходе тестирования системы безопасности Azure, и связанным рекомендациям, применимым к службе "защитник Azure для Интернета вещей". **Элементы управления** , неприменимые к службе "защитник Azure для Интернета вещей", были исключены.

Чтобы увидеть, как защитник Azure для IoT полностью сопоставлен с тестовым показателем безопасности Azure, см. [полный базовый файл сопоставления Azure Defender для безопасности IOT](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Руководство**. Служба "защитник Azure для Интернета вещей" интегрирована с Azure Active Directory (Azure AD), службой управления удостоверениями и доступом Azure по умолчанию. Необходимо стандартизировать Azure AD, чтобы управлять удостоверениями и управлением доступом организации в.
...
**Руководство**. Служба "защитник Azure для Интернета вещей" использует Azure Active Directory, поддерживающие управление строгими проверками подлинности с помощью многофакторной идентификации (MFA), и надежные методы безпарольного ввода.
...
- Проверка подлинности с паролем: доступны три варианта проверки подлинности, не защищенные паролем. Windows Hello для бизнеса, Microsoft Authenticatorное приложение и локальные методы проверки подлинности, такие как смарт-карты.
...
**Руководство**. Служба "защитник Azure для Интернета вещей" интегрирована с Azure Active Directory, в которой предоставляются следующие источники данных:
...
**Руководство**. Служба "защитник Azure для Интернета вещей" использует Azure RBAC для изоляции доступа к критически важным бизнес-системам путем определения того, какие учетные записи получают привилегированный доступ к подпискам и группам управления, в которых они находятся.
...
**Руководство**. Служба "защитник Azure для Интернета вещей" интегрирована с Azure Active Directory для управления ресурсами. Используйте функции управления обслуживанием Azure AD для автоматизации рабочих процессов запросов на доступ, включая назначения доступа, обзоры и срок действия. Поддерживается также два или несколько промежуточных утверждений.
...
**Рекомендации**: обнаружение, классификация и маркировка конфиденциальных данных, позволяющая разрабатывать соответствующие элементы управления для обеспечения хранения, обработки и передачи конфиденциальных данных в технологические системы Организации. 
...
**Руководство**. Установка или обновление политик безопасности, которые устраняют процессы управления жизненным циклом активов для потенциально высоких изменений. К таким изменениям относятся: поставщики удостоверений и доступ, чувствительность к данным, конфигурация сети и назначение прав администратора.
