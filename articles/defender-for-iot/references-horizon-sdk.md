...
| Метка параметра | Описание | Type |
|--|--|--|
| **ID** | Имя протокола. Удалите значение по умолчанию и добавьте имя протокола в том виде, в каком оно отображается. | Строка |
| **архитектуре** | Определяет, как кодируются многобайтовые данные. Используйте только слово "маленький" или "крупный". Берется из спецификации протокола или записи трафика | Строка |
| **sanity_failure_codes** | Это коды, возвращаемые средством синтаксического анализа при конфликте работоспособности в отношении идентификатора кода. См. раздел Проверка волшебных номеров в разделе C++. | Строка |
| **malformed_codes** | Это коды, которые были правильно идентифицированы, но обнаружена ошибка. Например, если длина поля слишком короткая или длинная, или значение недопустимо. | Строка |
...
|--|--|--|
| **ID** | Представляет один идентификатор предупреждения. Он должен быть уникальным в этом контексте. | Числовое значение 0-10000 |
...
Можно извлечь сведения о встроенном по, определить значения индексов и активировать предупреждения встроенного по для протокола подключаемого модуля. Например, примененная к объекту директива
...
|--|--|
| key | Указывает ключ. |
| value | Указывает поле реализации, используемое для предоставления данных. |
