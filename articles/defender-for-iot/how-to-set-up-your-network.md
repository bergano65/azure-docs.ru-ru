...
| **Назначение** | **протокол**; | **Транспорт** | **Входящий или исходящий** | **порт**. | **Категория** |
| ----------- | ----------- | ------------ | ---------- | -------- | ------------ |
| **Доступ к веб-консоли** | HTTPS | TCP | Входящий или исходящий | 443 | Локальная консоль управления для платформы "защитник для Интернета вещей" |
| **Доступ к интерфейсу командной строки** | SSH | TCP | Входящий или исходящий | 22 | CLI |
| **Подключение между защитником для платформы IoT и локальной консолью управления** | SSL | TCP | Входящий или исходящий | 443 | Консоль управления датчиком и локальной средой|
| **Локальная консоль управления, используемая в качестве NTP для датчика** | NTP | UDP| В CM | 123 | Синхронизация времени | 
| **Датчик, подключенный к внешнему NTP-серверу (если применимо)** | NTP | UDP | Входящий или исходящий| 123 | Синхронизация времени |
| **Подключение между защитником и платформой управления для платформы IoT и почтовым сервером (если это уместно)** | SMTP | TCP | Вне управления датчиками | 25 | Адрес электронной почты |
| **Журналы, которые отправляются из локальной консоли управления на сервер системного журнала (если это уместно)** | Системный журнал | UDP | Вне управления датчиками| 514 | лиф |
| **Порт DNS-сервера (если применимо)** | DNS | Недоступно | Входящий или исходящий| 53 | DNS |
| **Подключение между защитником для платформы IoT и локальной консолью управления к Active Directory (если применимо)** | LDAPS | TCP | Входящий или исходящий | 636 <br />389 | Active Directory |
| **Удаленные собирающие SNMP (если применимо)** | SNMP | UDP | Вне управления датчиками| 161 | Мониторинг |
| **Мониторинг конечных точек Windows (если применимо)** | WMI | UDP | Вне управления датчиками| 135 | Мониторинг |
| **Мониторинг конечных точек Windows (если применимо)** | WMI | TCP | Вне управления датчиками| 1024 и выше | Мониторинг |
| **Туннелирование (если применимо)** | Туннелирование | TCP | В CM | 9000<br />в дополнение к порту 443<br />От конечного пользователя до локальной консоли управления <br />Порт 22 от датчика к локальной консоли управления | Мониторинг |
| **Исходящий трафик в защитнике для центра Интернета вещей** | HTTPS | TCP | Вне управления датчиками| **URL-адрес**<br />*. azure-devices.net:443<br />или, если подстановочные знаки не поддерживаются<br />{имя центра Интернета вещей}. Azure-devices.net:443 |
