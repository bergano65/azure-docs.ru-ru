...
| **name** | String | Нет | - |
| **type** | String | Нет | Неизвестный, Инженерная станция, ХМИ, historian, контроллер домена, сервер базы данных, точка беспроводного доступа, маршрутизатор, коммутатор, сервер, Рабочая станция, IP-камера, принтер, брандмауэр, станция терминала, шлюз VPN, Интернет или многоадресная рассылка и широковещательная рассылка |
...
| Имя | Тип | Допускает значения NULL | Список значений |
|--|--|--|--|
| **подключение** | String | Нет | Н/д или фактическое значение |
| **model** | String | Нет | Н/д или фактическое значение |
| **FirmwareVersion** | Double | Нет | Н/д или фактическое значение |
| **аддитионалдата** | String | Нет | Н/д или фактическое значение |
| **модулеаддресс** | String | Нет | Н/д или фактическое значение |
| **оптимизирован** | String | Нет | Н/д или фактическое значение |
| **резервирован** | String | Нет | Н/д или фактическое значение |
| **address** | String | Нет | Н/д или фактическое значение |
...
| **понять** | String | Нет | 0,0-10,0 |
| **аттакквектор** | String | Нет | Сеть, смежная сеть, локальная или физическая |
...
| **title** | String | Нет | - |
| **message** | String | Нет | - |
| **severity** | String | Нет | Предупреждение, дополнительный, основной или критический |
| **ядре** | String | Нет | Нарушение протокола, нарушение политики, вредоносная программа, аномалия или работоспособность |
...
| **title** | String | Нет | - |
| **severity** | String | Нет | СВЕДЕНИЯ, уведомление или ОПОВЕЩЕНие |
| **владелец** | Строка | Да | Если событие было создано вручную, в это поле будет включено имя пользователя, создавшего событие. |
| **content** | String | Нет | - |
...
| **password** | String | Нет | - |
| **protocol** | String | Нет | - |
| **сложность** | String | Нет | Очень слабый, слабый, средний или строгий |
...
|--|--|--|--|
| **port** | Числовой | Нет | - |
| **перемещения** | String | Нет | TCP или UDP |
| **компьютера** | String | Нет | IP-адрес |
| **клиентсофтваре** | String | Нет | SSH, VNC, удаленный рабочий стол или средство просмотра команд |
...
| Имя поля | Тип | Список значений |
| ---------- | ---- | -------------- |
| **server** | String | IP-адрес |
| **компьютера** | String | IP-адрес |
| **port** | Числовой | - |
| **перемещения** | String | TCP, UDP или ICMP |
...
| **разработчика** | String | Имя поставщика |
| **IP** | String | IP-адрес или н/д |
| **name** | String | Имя устройства или н/д |
| **подключение** | String | Нет, не имеет подозрений или Да |
...
| Имя поля | Тип | Список значений |
| ---------- | ---- | -------------- |
| **интерналаддресс** | String | IP-адрес |
...
| Имя поля | Тип | Список значений |
|--|--|--|
| **source** | String | IP-адрес |
| **назначение** | String | IP-адрес |
| **port** | Числовой | - |
| **перемещения** | String | TCP или UDP |
| **баккупмаксималинтервал** | String | - |
...
|--|--|--|
| **адрес** s | Числовой | - |
| **сети** | String | IP-адрес |
| **виде** | String | Маска подсети |
...
|--|--|--|
| protocol | String | - |
| том | String | "номер тома МБ" |

**отключения**

| Имя поля | Тип | Список значений |
|--|--|--|
| **ассетаддресс** | String | IP-адрес |
| **assetName** | String | - |
...
| **имя**; | **Тип** | **Допускает значения NULL** |
|--|--|--|
| **admin_username** | String | Нет |
| **admin_password** | String | Нет |
| **username** | String | Нет |
| **new_password** | String | Нет |
...
| Имя | Тип | Допускает значения NULL | Список значений |
|--|--|--|--|
| **action** | String | Нет | Handle или Хандлеандлеарн |
...
|--|--|--|--|
| **содержимое/ошибка** | String | Нет | Если запрос выполнен успешно, отображается свойство Content. В противном случае отображается свойство Error. |
...
| Тип | API-интерфейсы | Пример |
|--|--|--|
| ОТПРАВКА | фигурная-k-X размещение-d "{" Action ":" <ACTION> "} '-H" Авторизация: <AUTH_TOKEN> "https://<IP_ADDRESS>/External/v1/Alerts/<UUID> | фигурная-k-X размещение-d "{" Action ":" Handle "} '-H" Авторизация: 1234b734a9244d54ab8d40aedddcabcd "https:/ <span> /127.0.0.1/External/v1/Alerts/1-1594550943000 |
...
| Тип | API-интерфейсы | Пример |
|--|--|--|
| ПОМЕСТИТЬ | фигурная-k-d "{" имя_пользователя ":" <USER_NAME> "," Password ":" пароль "}" ' https://<IP_ADDRESS>/екстернал/аусентикатион/валидатион " | фигурная-k-d "{" имя_пользователя ":" myUser "," Password ":" 1234@abcd "}" "https:/ <span> /127.0.0.1/External/Authentication/Validation" |
