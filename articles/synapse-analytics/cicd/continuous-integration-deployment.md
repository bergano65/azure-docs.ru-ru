...
В синапсе все виды артефактов не являются ресурсами ARM, которые отличаются от ADF. Невозможно использовать задачу развертывания шаблона ARM для развертывания артефактов синапсе
 
### <a name="unexpected-token-error-in-release"></a>Непредвиденная ошибка токена в выпуске

Если в файле параметров есть значения параметров, которые не экранированы, конвейер выпуска не сможет проанализировать файл с ошибкой непредвиденного маркера. Мы рекомендуем переопределить параметры или KeyVault для получения параметров. Можно также дважды escape-последовательность в качестве обходного пути.