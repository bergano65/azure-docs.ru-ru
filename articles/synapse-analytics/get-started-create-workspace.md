...
1. Для создания рабочей области вам потребуется учетная запись ADLSGEN2 и контейнер в ней. Проще всего создать новую учетную запись. Если вы хотите повторно использовать существующую учетную запись, вам потребуется выполнить дополнительную настройку. 
    1. Рабочая область Synapse будет использовать этот контейнер как расположение по умолчанию для хранения журналов Spark и данных для таблиц Spark.
