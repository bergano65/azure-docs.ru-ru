...
При запуске экземпляра Spark создается новая виртуальная среда с использованием этой установки в качестве основы. Кроме того, для обновления виртуальной среды можно использовать файл *требований. txt* (выходные данные `pip freeze` команды). Пакеты, перечисленные в этом файле для установки или обновления, загружаются из PyPi во время запуска кластера. Этот файл требований используется при каждом создании экземпляра Spark из пула Spark.
