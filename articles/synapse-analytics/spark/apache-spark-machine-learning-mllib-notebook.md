---
title: Создание приложения машинного обучения с помощью Apache Spark MLlib и Azure Synapse Analytics
...
1. Выполните следующие строки, чтобы создать кадр данных Spark, вставив код в новую ячейку. В первом разделе выполняется назначение переменным сведений о доступе к хранилищу Azure. Второй раздел позволяет Spark удаленно считывать данные из хранилища BLOB-объектов. Последняя строка кода считывает Parquet, но в этот момент данные не загружаются.
...
2. Получение всех этих данных приводит к созданию порядка 1 500 000 000 строк. В зависимости от размера пула Spark (предварительная версия), необработанные данные могут быть слишком большими или занимать слишком много времени для обработки. Эти данные можно отфильтровать до меньшего объема. При необходимости добавьте следующие строки, чтобы отфильтровать данные примерно до 2 000 000 строк для более простой обработки. Используйте эти параметры, чтобы получить данные за неделю.
...
4. Теперь можно просмотреть данные, чтобы узнать, что было считано. Обычно лучше рассмотреть данные с подмножеством, а не с полным набором, в зависимости от размера набора данных. В следующем коде представлены два способа просмотра данных: первый из них является базовым, а второй предоставляет более широкие возможности за счет сетки, а также возможность визуализировать данные в графическом виде.
...
5. В зависимости от созданного размера набора данных и необходимости поэкспериментировать или запустить записную книжку может быть целесообразно кэшировать набор данных локально в рабочую область. Существует три способа явного кэширования.
...
- Добавление меток. Поскольку выполняется двоичная классификация (будут ли чаевые или нет для той или иной поездки), сумму чаевых необходимо преобразовать в значение 0 или 1.
...
В результате создается новый кадр данных со всеми столбцами в правильном формате для обучения модели.
