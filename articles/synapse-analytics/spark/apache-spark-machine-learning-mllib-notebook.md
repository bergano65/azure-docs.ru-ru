---
title: Создание приложения машинного обучения с помощью Apache Spark MLlib
...
4. В зависимости от размера созданного размера набора данных и необходимости в поэксперименте или запуске записной книжки может быть целесообразно кэшировать набор данных локально в рабочей области. Существует три способа выполнения явного кэширования:
...
И последняя задача — преобразуйте данные с метками в такой формат, который удобен для анализа методом логистической регрессии. Входные данные для алгоритма логистической регрессии должны представлять собой *набор пар "метка — вектор признаков"* , где *вектор признаков* содержит числа, характеризующие точку входных данных. Поэтому необходимо преобразовать столбцы категорий в числа. `trafficTimeBins`Столбцы и `weekdayString` должны преобразовываться в целочисленные представления. Существует несколько подходов к преобразованию, однако подход, применяемый в этом примере, *OneHotEncoding*, является общепринятым подходом.
