...
> [!NOTE]  
> Альтернативой загрузке является [заявление COPY,](https://docs.microsoft.com/sql/t-sql/statements/copy-into-transact-sql?view=azure-sqldw-latest) наиболее публичное предварительное просмотр.  Заявление COPY обеспечивает максимальную гибкость. Чтобы предоставить обратную связь по заявлению COPY, sqldwcopypreview@service.microsoft.comотправьте электронное письмо в следующий список рассылки: .
>
> [!div class="checklist"]
...
* Учетная запись хранения озера данных. Смотрите [Начало с Azure хранения озера данных](../../data-lake-store/data-lake-store-get-started-portal.md). Для этой учетной записи хранилища необходимо настроить или указать один из следующих учетных данных для загрузки: ключ учетной записи хранилища, пользователь приложения Каталог Azure или пользователь AAD, который имеет соответствующую роль RBAC для учетной записи хранилища. 
...
Чтобы подключиться к хранилищам озер данных с помощью основ службы, необходимо **сначала** создать приложение Active Directory, создать ключ доступа и предоставить приложению доступ к учетной записи Data Lake Storage. Для получения инструкций [см. Authenticate to Azure Data Lake Storage с помощью active Directory.](../../data-lake-store/data-lake-store-service-to-service-authenticate-using-active-directory.md)
...
Используйте команду [CREATE EXTERNAL DATA SOURCE](/sql/t-sql/statements/create-external-data-source-transact-sql), чтобы сохранить расположение данных. Если вы проверяете подлинность с помощью aAD-проходной, параметр CREDENTIAL не требуется. 
...
Для загрузки данных из Data Lake Storage используйте заявление [CREATE TABLE AS SELECT (Transact-S'L).](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse) 
