...
[Пул СИНАПСЕ SQL](sql-data-warehouse-overview-what-is.md#dedicated-sql-pool-in-azure-synapse) представляет коллекцию подготовленных аналитических ресурсов. Аналитические ресурсы определяются как сочетание ЦП, памяти и операций ввода-вывода.
...
- Скорость, с которой Стандартный выделенный запрос пула SQL может сканировать большое количество строк, а затем выполнять комплексное агрегирование. Это требует интенсивного использования операций ввода-вывода и ресурсов ЦП.
- Скорость, с которой выделенный пул SQL может принимать данные из больших двоичных объектов службы хранилища Azure или Azure Data Lake. Это сетевая операция, которая требует интенсивного использования ресурсов ЦП.
...
Цель уровня обслуживания — это параметр масштабируемости, определяющий стоимость и уровень производительности выделенного пула SQL. Уровни обслуживания для выделенного пула SQL Gen2 измеряются в единицах хранилища данных (DWU), например DW2000c.
...
Выделенный пул SQL — это масштабируемая система, позволяющая подготавливать большие объемы вычислений и запрашивать немалым количество данных.
...
Это динамическое административное представление возвращает сведения о различных операциях управления в выделенном пуле SQL, таких как операция, и состояние операции, которое либо IN_PROGRESS, либо ЗАВЕРШЕНо.
