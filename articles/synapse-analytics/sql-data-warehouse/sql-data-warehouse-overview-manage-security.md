...
Правила брандмауэра используются как [логическим сервером SQL Server](../../azure-sql/database/logical-servers.md) , так и его базами данных для отклонения попыток подключения с IP-адресов, которые не были явно список разрешений. Чтобы разрешить подключение из своего приложения или с клиентского компьютера с общедоступным IP-адресом, сначала необходимо создать правило брандмауэра уровня сервера. Для этого воспользуйтесь порталом Azure, REST API или PowerShell.
