...
description: Советы по защите выделенного пула SQL и разработки решений в Azure синапсе Analytics.
author: julieMSFT
manager: craigg
...
ms.locfileid: "93317699"
---
# <a name="secure-a-dedicated-sql-pool-in-azure-synapse-analytics"></a>Защита выделенного пула SQL в Azure синапсе Analytics
...
В этой статье рассматриваются основы защиты выделенного пула SQL. В частности, эта статья посвящена началу работы с ресурсами для ограничения доступа, защиты данных и мониторинга действий с помощью выделенного пула SQL.
...
Рекомендуется ограничить диапазоны IP-адресов, разрешенные через брандмауэр уровня сервера, насколько это возможно.  Чтобы получить доступ к выделенному пулу SQL с локального компьютера, убедитесь, что брандмауэр в сети и локальный компьютер допускают исходящие подключения через TCP-порт 1433.  
...
Подключения к выделенному пулу SQL шифруются по умолчанию.  Изменение параметров подключения для отключения шифрования игнорируется.

## <a name="authentication"></a>Проверка подлинности

В процессе аутентификации предлагается подтвердить личность пользователя при подключении к базе данных. Выделенный пул SQL в настоящее время поддерживает SQL Server проверку подлинности с помощью имени пользователя и пароля, а также с Azure Active Directory.
...
Затем подключитесь к **выделенной базе данных пула SQL** с именем для входа администратора сервера и создайте пользователя базы данных на основе созданного имени входа сервера.
...
Дополнительные сведения и примеры подключения к хранилищу с различными протоколами см. в разделе [Подключение к выделенному пулу SQL](../sql/connect-overview.md).
