...
description: Рекомендации по проектированию хэш-распределенных таблиц и таблиц с циклическим распределением в пуле Synapse SQL.
...
ms.locfileid: "85214015"
---
# <a name="guidance-for-designing-distributed-tables-in-synapse-sql-pool"></a>Руководство по проектированию распределенных таблиц в пуле Synapse SQL

Рекомендации по проектированию хэш-распределенных таблиц и таблиц с циклическим распределением в пулах Synapse SQL.

В этой статье предполагается, что вы знакомы с основными понятиями, связанными с распределением данных и перемещением данных в пуле Synapse SQL.  Дополнительные сведения см. в статье [Архитектура Azure Synapse Analytics (ранее — хранилище данных SQL)](massively-parallel-processing-mpp-architecture.md).
...
- Существуют ли в пуле Synapse SQL таблицы фактов и измерений?
...
Так как хэш-функция всегда назначает одинаковые значения в одно и то же распределение, хранилище данных содержит встроенные сведения о расположении строк. В пуле Synapse SQL эти сведения используются для того, чтобы свести к минимуму перемещение данных во время выполнения запросов, что повышает их производительность.
...
В руководстве [Загрузка набора данных о такси в Нью-Йорке](load-data-from-azure-blob-storage-using-polybase.md#load-the-data-into-your-data-warehouse) приведен пример загрузки данных в промежуточную таблицу с циклическим распределением.
...
Данные, хранимые в столбце распределения, можно обновлять, что может вызывать смешение данных.
...
- [CREATE TABLE (пул Synapse SQL)](/sql/t-sql/statements/create-table-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
- [CREATE TABLE AS SELECT (пул Synapse SQL)](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
