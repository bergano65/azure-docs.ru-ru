...
description: Рекомендации по проектированию распределенных хэш-таблиц и кластеров с циклическим перебором с помощью выделенного пула SQL в Azure синапсе Analytics.
...
Рекомендации по проектированию распределенных хэш-распределенных таблиц и циклического перебора в выделенных пулах SQL.

В этой статье предполагается, что вы знакомы с концепциями распределения данных и перемещения данных в выделенном пуле SQL.  Дополнительные сведения см. в статье [архитектура Azure синапсе Analytics](massively-parallel-processing-mpp-architecture.md).
...
- Есть ли у меня таблицы фактов и измерений в выделенном пуле SQL?
...
Так как хэш-функция всегда назначает одинаковые значения в одно и то же распределение, хранилище данных содержит встроенные сведения о расположении строк. В выделенном пуле SQL эти знания используются для уменьшения перемещения данных во время запросов, что повышает производительность запросов.
