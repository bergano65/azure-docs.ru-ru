---
title: Руководство по проектированию распределенных таблиц
description: Рекомендации по проектированию распределенных хэш-распределенных и круговых распределенных таблиц в пуле Synapse S'L.
...
ms.locfileid: "80742867"
---
# <a name="guidance-for-designing-distributed-tables-in-synapse-sql-pool"></a>Руководство по разработке распределенных таблиц в бассейне Synapse S'L

Рекомендации по проектированию распределенных хэш-распределенных и круговых распределенных таблиц в бассейнах Synapse S'L.

В этой статье предполагается, что вы знакомы с концепциями распределения данных и движения данных в пуле Synapse S'L.Для получения дополнительной информации см. [Azure Synapse Analytics массивно параллельной обработки (MPP) архитектуры](massively-parallel-processing-mpp-architecture.md).
...
- Есть ли у меня таблицы фактов и измерений в пуле Synapse S'L?
...
![Распределенная таблица](./media/sql-data-warehouse-tables-distribute/hash-distributed-table.png "Распределенная таблица")  

Так как хэш-функция всегда назначает одинаковые значения в одно и то же распределение, хранилище данных содержит встроенные сведения о расположении строк. В пуле Synapse S'L эти знания используются для минимизации движения данных во время запросов, что повышает производительность запроса.
...
- Если нет хорошего столбца кандидата для распределения хэша
...
Учебник [Load New York taxicab данные](load-data-from-azure-blob-storage-using-polybase.md#load-the-data-into-your-data-warehouse) дает пример загрузки данных в круговой плацдарм.
...
- **Не является столбецом даты**. Все данные с одинаковой датой попадают в одно распределение. Если несколько пользователей использует фильтр по одной дате, то только 1 из 60 распределений выполняет всю обработку.
...
- *Не* используется `WHERE` в положениях. Это может ограничить выполнение запроса, в результате его он будет выполнен не для всех распределений.
...
После завершения проектирования хэш-распределенной таблицы следующим шагом является загрузка данных в эту таблицу.  Руководство по загрузке представлено в [обзоре загрузки](design-elt-data-loading.md).
...
- [CREATE TABLE (Бассейн Synapse S'L)](/sql/t-sql/statements/create-table-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
- [CREATE TABLE AS SELECT (Бассейн Synapse S'L)](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
