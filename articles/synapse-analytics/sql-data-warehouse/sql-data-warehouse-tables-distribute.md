...
description: Рекомендации по проектированию распределенных хэш-таблиц и распределенных циклов с циклическим перебором в пуле SQL синапсе.
...
---
# <a name="guidance-for-designing-distributed-tables-in-synapse-sql-pool"></a>Руководство по проектированию распределенных таблиц в пуле синапсе SQL

Рекомендации по проектированию распределенных хэш-таблиц и распределенных циклов с циклическим перебором в пулах синапсе SQL.

В этой статье предполагается, что вы знакомы с концепциями распределения данных и перемещения данных в пуле синапсе SQL.Дополнительные сведения см. в статье [архитектура Azure синапсе Analytics с массовой параллельной обработкой (MPP)](massively-parallel-processing-mpp-architecture.md).
...
- Есть ли у меня таблицы фактов и измерений в пуле SQL синапсе?
...
Так как хэш-функция всегда назначает одинаковые значения в одно и то же распределение, хранилище данных содержит встроенные сведения о расположении строк. В пуле синапсе SQL эти знания используются для уменьшения перемещения данных во время запросов, что повышает производительность запросов.
...
В руководстве [Загрузка данных в Нью-Йорке такси](load-data-from-azure-blob-storage-using-polybase.md#load-the-data-into-your-data-warehouse) приводится пример загрузки данных в промежуточную таблицу с циклическим перебором.
...
- **Не является столбцом дат**. Все данные с одинаковой датой попадают в одно распределение. Если несколько пользователей использует фильтр по одной дате, то только 1 из 60 распределений выполняет всю обработку.
...
- *Не* используется в `WHERE` предложениях. Это может ограничить выполнение запроса, в результате его он будет выполнен не для всех распределений.
...
- [CREATE TABLE (пул SQL синапсе)](/sql/t-sql/statements/create-table-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
- [CREATE TABLE как SELECT (пул SQL синапсе)](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
