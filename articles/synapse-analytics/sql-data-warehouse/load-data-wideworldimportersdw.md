---
title: 'Учебник: Загрузка данных с помощью портала Azure & SSMS'
description: Учебник использует портал Azure и студию управления серверами S'L для загрузки хранилища данных WideWorldImportersDW из глобального кабы Azure в пул Azure Synapse Analytics S'L.
...
1. Выберите **базы данных** из **новой** страницы и выберите **аналитику Azure Synapse** под **функцией** на **новой** странице.
...
Все готово для начала процесса загрузки данных в новое хранилище данных. Для получения последующих ссылок, чтобы узнать, как получить данные в хранилище Azure Blob или загрузить их непосредственно из вашего источника в пул S'L, см. [loading overview](design-elt-data-loading.md)
...
7. Создайте внешние таблицы. Определения таблицы хранятся в базе данных, но таблицы ссылаются на данные, которые хранятся в хранилище Azure blob. Выполните следующие команды T-SQL, чтобы создать несколько внешних таблиц, и при этом все они будут указывать на большой двоичный объект Azure, определенный нами ранее во внешнем источнике данных.
...
8. В Object Explorer расширьте SampleDW, чтобы увидеть созданные внешние таблицы.

    ![Просмотр внешних таблиц](./media/load-data-wideworldimportersdw/view-external-tables.png)

## <a name="load-the-data-into-sql-pool"></a>Загрузка данных в пул S'L

В этом разделе используются внешние таблицы, определяемые для загрузки выборочных данных с Azure Blob в пул S'L.  
...
2. Просмотрите данные при загрузке. Вы загружаете несколько ГБ данных и сжимаете их в высокоэффективные кластерные индексы столбцов. Откройте новое окно запроса на SampleDW и выполните следующий запрос для отображения состояния загрузки. После запуска запроса, возьмите кофе и закуску, в то время как бассейн S'L делает некоторую тяжелую работу.
...
В этом разделе создаются таблицы wwi.dimension_Date и wwi.fact_Sale. Он также создает сохраненные процедуры, которые могут генерировать миллионы строк в таблицах wwi.dimension_Date и wwi.fact_Sale.
...
2. Запустите эту процедуру, чтобы заполнить wwi.fact_Sale с 100000 строк в день за каждый день в 2000 году.
...
2. Если вы хотите сохранить данные в хранилище, то можете приостановить работу вычислительных ресурсов, когда не используете хранилище данных. Приоостанавливая вычисления, вы будете взимать плату только за хранение данных, и вы можете возобновить вычисление, когда вы будете готовы работать с данными. Чтобы приостановить работу вычислительных ресурсов, нажмите кнопку **Приостановить**. Если работа хранилища данных приостановлена, то отобразится кнопка **Запуск**.  Чтобы возобновить работу вычислительных ресурсов, нажмите кнопку **Запуск**.
...
Заранее пересмотрите обзор разработки, чтобы узнать, как перенести существующую базу данных в пул Azure Synapse S'L.

> [!div class="nextstepaction"]
>[Проектные решения о переносе существующей базы данных в пул S'L](sql-data-warehouse-overview-develop.md)
