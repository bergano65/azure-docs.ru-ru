...
description: Просмотрите ограничения памяти и параллелизма, выделенные для различных уровней производительности и классов ресурсов в Azure синапсе Analytics.
...
---
# <a name="memory-and-concurrency-limits-for-azure-synapse-analytics"></a>Ограничения памяти и параллелизма для Azure синапсе Analytics

Просмотрите ограничения памяти и параллелизма, выделенные для различных уровней производительности и классов ресурсов в Azure синапсе Analytics.  
...
Чтобы каждый запрос получил достаточно ресурсов для эффективного выполнения, SQL Analytics в Azure синапсе отслеживает использование ресурсов, назначая слоты параллелизма каждому запросу. Система помещает запросы в очередь на основе важности и слотов выдачи. Запросы ожидают в очереди, пока не будет доступно достаточное число слотов. Слоты [важности](sql-data-warehouse-workload-importance.md) и параллелизма ОПРЕДЕЛЯЮТ приоритеты ЦП. Дополнительные сведения см. в статье [Анализ рабочей нагрузки](analyze-your-workload.md).
