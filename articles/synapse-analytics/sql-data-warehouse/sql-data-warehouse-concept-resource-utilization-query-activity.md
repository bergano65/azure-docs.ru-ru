...
| Успешные подключения  | Количество успешных подключений (логинов) к базе данных | Сумма, граф       |
| Неудачные подключения      | Количество сбойных подключений (логинов) в базе данных | Сумма, граф       |
...
| Размер хранилища данных (GB) | Общий размер базы данных. Это включает в себя используемое, зарезервированное и нераспределенное пространство. Нераспределенное пространство хранится для базы данных для оптимизации производительности запроса и загрузки. | SUM |
...
Для просмотра списка DMV, которые применяются к Synapse S'L, обратитесь к этой [документации](../sql/reference-tsql-system-views.md#sql-pool-dynamic-management-views-dmvs). 
...
В следующем руководстве How-to описывается общие сценарии и случаи использования при мониторинге и управлении хранилищем данных:
