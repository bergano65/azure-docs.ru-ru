...
![underutilized-wg.png](./media/sql-data-warehouse-workload-management-portal-monitor/underutilized-wg.png) Диаграмма показывает, что при изоляции рабочих нагрузок в 25 % в среднем используется только 10 %.  В этом случае значение параметра `MIN_PERCENTAGE_RESOURCE` можно уменьшить в диапазоне от 10 до 15 и разрешить другим рабочим нагрузкам в системе использовать ресурсы.
