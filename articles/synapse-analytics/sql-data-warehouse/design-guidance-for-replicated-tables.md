---
title: Руководство по проектированию для реплицированных таблиц
description: Рекомендации по проектированию реплицированных таблиц в синапсе SQL
...
---
# <a name="design-guidance-for-using-replicated-tables-in-sql-analytics"></a>Руководство по проектированию для использования реплицированных таблиц в SQL Analytics

В этой статье приводятся рекомендации по проектированию реплицируемых таблиц в схеме аналитики SQL. Данные рекомендации позволяют повысить производительность запросов за счет уменьшения перемещения данных и упрощения запросов.

> [!VIDEO https://www.youtube.com/embed/1VS_F37GI9U]

## <a name="prerequisites"></a>Предварительные условия

В этой статье предполагается, что вы знакомы с концепциями распределения данных и перемещения данных в SQL Analytics.Дополнительные сведения см. в статье об [архитектуре](massively-parallel-processing-mpp-architecture.md).
...
- Есть ли у меня таблицы фактов и измерений в базе данных SQL Analytics?

## <a name="what-is-a-replicated-table"></a>Что такое реплицированная таблица?

Реплицированная таблица содержит полную копию таблицы, которая доступна на каждом вычислительном узле. Репликация таблицы устраняет необходимость передавать данные между вычислительными узлами перед операциями соединения или агрегирования. Поскольку в таблице имеется несколько копий, реплицированные таблицы наиболее эффективны, когда размер таблицы в сжатом состоянии менее 2 ГБ.  2 ГБ не являются жестким ограничением.  Если данные являются статическими и не меняются, можно реплицировать большие таблицы.

На схеме ниже показана реплицированная таблица, которая доступна на каждом вычислительном узле. В SQL Analytics реплицированная таблица полностью копируется в базу данных распространителя на каждом кластерном узле.

![Реплицированная таблица](./media/design-guidance-for-replicated-tables/replicated-table.png "Реплицированная таблица")  

Реплицированные таблицы хорошо подходят для таблиц измерений в схеме типа «звезда». Таблицы измерений обычно соединяются с таблицами фактов, которые распределяются иначе, чем таблица измерения.  Размеры обычно имеют размер, позволяющий хранить и поддерживать несколько копий. В измерениях хранятся описательные данные, которые изменяются редко, например имя и адрес клиента, а также сведения о продукте. Медленно изменяющаяся природа данных приводит к уменьшению обслуживания реплицированной таблицы.
...
- Таблица содержит частые операции вставки, обновления и удаления.Для операций языка обработки данных DML требуется перестроение реплицированной таблицы.Зачастую перестроение приводит к снижению производительности.
- База данных аналитики SQL масштабируется часто. Масштабирование базы данных SQL Analytics изменяет число узлов вычислений, что влечет за собой перестроение реплицированной таблицы.
- Таблица содержит много столбцов, однако операции с данными обычно обращаются только к небольшому числу столбцов. В этом случае вместо репликации всей таблицы лучше всего использовать распределение таблицы, а затем создать индекс для часто используемых столбцов. Если запрос требует перемещения данных, SQL Analytics перемещает только данные для запрошенных столбцов.
...
Например, в этом запросе содержится сложный предикат.  Он выполняется быстрее, если данные находятся в распределенной таблице, а не в реплицированной таблице. В этом примере данные могут быть распределены с циклическим перебором.
...
Если у вас уже есть таблицы с циклическим перебором, рекомендуется преобразовать их в реплицируемые таблицы, если они соответствуют критериям, описанным в этой статье. Реплицированные таблицы эффективнее по сравнению с циклическими таблицами, поскольку они исключают необходимость в перемещении данных.  Для соединений в циклической таблице всегда требуется перемещение данных.
...
SQL Analytics реализует реплицированную таблицу, сохраняя главную версию таблицы. Она копирует основную версию в первую базу данных распространителя на каждом кластерном узле. При изменении SQL Analytics сначала обновляет основную версию, а затем перестраивает таблицы на каждом из вычислений. Перестроение реплицированной таблицы включает в себя копирование таблицы на каждый вычислительный узел и последующее перестроение индексов.  Например, реплицированная таблица в DW2000c содержит 5 копий данных.  Мастер-копия и полные копии на каждом вычислительном узле.  Все данные хранятся в базах данных распространителя. SQL Analytics использует эту модель для поддержки более быстрых инструкций изменения данных и гибких операций масштабирования.

Перестроение требуется после следующего:

- Загрузка или изменение данных
- Экземпляр SQL синапсе масштабируется до другого уровня
...
В отношении реплицированных таблиц применяется стандартный подход к индексированию. SQL Analytics перестраивает каждый индекс реплицированной таблицы в процессе перестроения. Индексы следует использовать только в том случае, если для повышения производительности целесообразно перестроить индексы.

### <a name="batch-data-load"></a>Загрузка пакетных данных
...
- [CREATE TABLE как SELECT (SQL Analytics)](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
