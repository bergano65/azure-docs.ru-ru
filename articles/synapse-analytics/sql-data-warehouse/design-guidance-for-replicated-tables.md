...
Аналитика S'L реализует реплицированную таблицу, поддерживая мастер-версию таблицы. Он копирует мастер-версию к первой базе данных распределения на каждом вычислительном узлах. При изменении, S'L Analytics сначала обновляет мастер-версию, а затем восстанавливает таблицы на каждом вычислительном узлах. Перестроение реплицированной таблицы включает в себя копирование таблицы на каждый вычислительный узел и последующее перестроение индексов.  Например, реплицированная таблица на DW2000c имеет 5 копий данных.  Мастер-копия и полные копии на каждом вычислительном узле.  Все данные хранятся в базах данных распространителя. Эта модель используется для поддержки более быстрых отчетов о модификации данных и гибких операций масштабирования.
