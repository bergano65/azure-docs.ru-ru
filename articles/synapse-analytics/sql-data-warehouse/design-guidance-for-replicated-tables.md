...
description: Рекомендации по проектированию реплицированных таблиц в пуле Synapse SQL.
...
В этой статье предполагается, что вы знакомы с основными понятиями, связанными с распределением данных и перемещением данных в пуле SQL.  Дополнительные сведения см. в статье об [архитектуре](massively-parallel-processing-mpp-architecture.md).
...
- Существуют ли в пуле SQL таблицы фактов и измерения?
...
На схеме ниже показана реплицированная таблица, которая доступна на каждом вычислительном узле. Для пула SQL реплицируемая таблица полностью копируется в базу данных распространителя на каждом вычислительном узле.
...
- База данных пула SQL масштабируется часто. Масштабирование базы данных пула SQL изменяет число вычислительных узлов, что влечет за собой перестроение реплицированной таблицы.
...
Асинхронные пересборки активируются первым запросом к реплицированной таблице после:
...
### <a name="batch-data-load"></a>Пакетная загрузка данных
