...
ms.locfileid: "80582242"
---
# <a name="design-guidance-for-using-replicated-tables-in-synapse-sql"></a>Руководство по разработке для использования реплицированных таблиц в Synapse S'L

В этой статье содержатся рекомендации по разработке реплицированных таблиц в схеме Synapse S'L. Данные рекомендации позволяют повысить производительность запросов за счет уменьшения перемещения данных и упрощения запросов.

> [!VIDEO https://www.youtube.com/embed/1VS_F37GI9U]

## <a name="prerequisites"></a>Предварительные требования

В этой статье предполагается, что вы знакомы с концепциями распределения данных и движения данных в Synapse S'L.Дополнительные сведения см. в статье об [архитектуре](massively-parallel-processing-mpp-architecture.md). 
...
- Есть ли у меня таблицы фактов и измерений в базе данных Synapse S'L?   
...
На схеме ниже показана реплицированная таблица, которая доступна на каждом вычислительном узле. В Synapse S'L реплицированная таблица полностью скопирована в базу данных дистрибутива на каждом вычислительном узло. 
...
Реплицированная таблица реализуется путем сохранения основной версии таблицы. Для этого оно копирует главную версию в одну базу данных распространителя на каждый вычислительный узел. При изменении сначала обновляется мастер-таблица. Затем таблица на каждом вычислительном узлах перестраивается. Перестроение реплицированной таблицы включает в себя копирование таблицы на каждый вычислительный узел и последующее перестроение индексов.  Например, у реплицированной таблицы на DW400 имеются 5 копий данных.  Мастер-копия и полные копии на каждом вычислительном узле.  Все данные хранятся в базах данных распределения для поддержки более быстрых заявлений об изменении данных и гибких операций масштабирования. 
...
В отношении реплицированных таблиц применяется стандартный подход к индексированию. Каждый реплицированный индекс таблицы перестраивается в рамках восстановления индекса. Индексы следует использовать только в том случае, если для повышения производительности целесообразно перестроить индексы.  
...
- [СОЗДАТЬ ТАБЛИЦУ](/sql/t-sql/statements/create-table-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
- [СОЗДАТЬ ТАБЛИЦУ ПО ВЫБОРУ](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
