...
description: Руководство по использованию классов ресурсов для управления параллелизмом и вычисления ресурсов для запросов в Azure Synapse Analytics.
...
Руководство по использованию классов ресурсов для управления памятью и параллелизмом для запросов S'L Analytics в Azure Synapse.  

## <a name="what-are-resource-classes"></a>Что такое классы ресурсов

Производительность запроса определяется классом ресурсов пользователя.  Классы ресурсов являются заранее определенными ограничениями ресурсов в S'L Analytics, которые регулируют вычислительные ресурсы и параллелиризируют выполнение запросов. Классы ресурсов могут помочь настроить ресурсы для запросов, установив ограничения на количество запросов, которые работают одновременно, и на вычислительные ресурсы, назначенные каждому запросу.  Есть компромисс между памятью и параллелизмом.

- Небольшие классы ресурсов уменьшают максимальный объем памяти для каждого запроса, но обеспечивают больший параллелизм.
- Большие классы ресурсов увеличивают максимальную память на запрос, но уменьшают параллелизм.
...
- Динамические классы ресурсов, которые хорошо подходят для наборов данных, которые увеличиваются в размерах и нуждаются в повышении производительности по мере расширения уровня обслуживания.
...
Распределение памяти для каждого класса ресурсов следующим образом. 
...
| DW500c         | 5 %                | 10 %                    | 22 %                    | 70 %                    |
| DW1000c к<br> DW30000c | 3 %       | 10 %                    | 22 %                    | 70 %                    |
...
Класс ресурсов администратора службы является фиксированным в smallrc и его нельзя изменить.  Администратором службы является пользователь, созданный во время процесса подготовки.  Администратором службы в этом контексте является логин, указанный для входа администратора сервера при создании нового экземпляра S'L Analytics с новым сервером.
...
- ИЗМЕНИТЬ ТАБЛИЦУ ... SWITCH, SPLIT, или MERGE PARTITION
- ALTER INDEX DISABLE
- DROP INDEX
...
Только запросы, управляемые ресурсами, используют слоты выдачи. Системные запросы и некоторые тривиальные запросы не потребляют слотов. Точное число используемых слотов выдачи определяется классом ресурсов запроса.
...
>[!NOTE]
>Рассмотрите возможность управления рабочей нагрузкой[(изоляция рабочей нагрузки,](sql-data-warehouse-workload-isolation.md) [классификация](sql-data-warehouse-workload-classification.md) и [важность)](sql-data-warehouse-workload-importance.md)для большего контроля над рабочей нагрузкой и предсказуемой производительности.  
>
>

Мы рекомендуем создать пользователя, предназначенного для выполнения определенного типа операции запроса или загрузки. Предоставь этому пользователю постоянный класс ресурсов вместо того, чтобы часто менять класс ресурсов. Статические классы ресурсов позволяют более полно контролировать рабочую нагрузку, поэтому мы предлагаем использовать классы статических ресурсов перед рассмотрением динамических классов ресурсов.

### <a name="resource-classes-for-load-users"></a>Классы ресурсов для пользователей, выполняющих загрузку

`CREATE TABLE` использует кластеризованные индексы columnstore по умолчанию. Сжатие данных в индекс columnstore представляет собой операцию, требующую интенсивного использования памяти, а нехватка памяти может повлиять на качество индекса. Давление памяти может привести к необходимости более высокого класса ресурсов при загрузке данных. Чтобы убедиться, что для загрузок используется достаточно памяти, вы можете создать пользователя, предназначенного для выполнения загрузки и назначить этого пользователя более высокому классу ресурсов.
...
- Используйте динамический класс ресурсов при наличии широкого ряда требований к памяти таблицы. Для загрузок может потребоваться больше памяти, чем обеспечивают текущие уровни DWU или cDWU. Масштабирование хранилища данных добавляет больше памяти для загрузки операций, что позволяет нагрузкам выполняться быстрее.

### <a name="resource-classes-for-queries"></a>Классы ресурсов для запросов

Некоторые запросы являются вычислительными, а некоторые нет.  

- Выберите динамический класс ресурсов, когда запросы сложны, но не нуждаются в высокой параллелизме.  Например, при создании ежедневных или еженедельных отчетов потребность в ресурсах эпизодическая. Если отчеты обрабатывают большие объемы данных, масштабирование хранилища данных обеспечит больший объем памяти для имеющегося класса ресурса пользователя.
- Выберите статический класс ресурса, когда спрос на ресурсы непостоянный в течение дня. Например, статический класс ресурса эффективно работает, когда хранилище данных запрашивается многими пользователями. При масштабировании хранилища данных объем памяти, выделяемый пользователю, не изменяется. Следовательно, в системе можно выполнить больше параллельных запросов.

Надлежащие гранты памяти зависят от многих факторов, таких как объем запрашиваемых данных, характер схем таблицы и различные соединения, выбор и групповые предикаты. С общей точки зрения, выделение большего объема памяти позволит ускорить выполнение запросов, но снизит общий уровень параллелизма. Если низкий уровень параллелизма не является проблемой, чрезмерное выделение памяти не нанесет вреда пропускной способности.
...
Вы можете использовать следующую указанную процедуру хранения, чтобы выяснить, что параллель и грант памяти на класс ресурсов в данном классе SLO и лучший класс ресурсов для интенсивных операций CCI памяти на неразделенной таблице CCI в данном классе ресурсов:
...
2. Для просмотра наилучшего класса ресурсов для операций CCI с интенсивными памятью (загрузка, таблица копирования, индекс восстановления и т.д.) на таблице CCI, не разделенной в данном классе ресурсов. Хранимая процедура использует схему таблицы, чтобы определить требуемый объем временно предоставляемого буфера памяти.

### <a name="dependencies--restrictions"></a>Ограничения зависимостей &

- Эта сохраненная процедура не предназначена для расчета требования к памяти для раздельной таблицы cci.
- Эта сохраненная процедура не учитывает требования к памяти для части SELECT CTAS/INSERT-SELECT и предполагает, что это SELECT.
...
- Эта сохраненная процедура зависит от текущих предложений (например, конфигурация оборудования, DMS-конфигурация), и если что-либо из этого изменяется, то этот сохраненный proc не будет работать правильно.  
- Эта сохраненная процедура зависит от существующих предложений лимита параллелизма, и если эти изменения, то эта сохраненная процедура не будет работать правильно.  
- Эта сохраненная процедура зависит от существующих предложений класса ресурсов, и если они изменятся, то эта сохраненная процедура не будет работать правильно.  
...
>
>Например, на DW100c самый высокий доступный грант памяти составляет 1 ГБ, и если схема таблицы достаточно широка, чтобы пересечь требование 1 ГБ.
...
1. @DWU:Либо предоставить параметр NULL для извлечения текущего DWU из DW DB или предоставить любой поддерживаемый DWU в виде 'DW100c'
