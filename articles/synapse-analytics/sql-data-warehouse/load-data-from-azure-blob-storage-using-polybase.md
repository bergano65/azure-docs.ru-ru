---
title: 'Учебник: Загрузите данные New York Taxicab'
description: В учебном центре используется портал Azure и студия управления серверами S'L для загрузки данных New York Taxicab из глобального капли Azure для аналитики S'L.
...
Пул пула S'L создается с определенным набором [вычислительных ресурсов.](memory-concurrency-limits.md) База данных создается в пределах [группы ресурсов Azure](../../azure-resource-manager/management/overview.md) и [логического сервера SQL Azure](../../sql-database/sql-database-features.md). 

Выполните следующие действия, чтобы создать пустую базу данных. 

1. Щелкните **Создать ресурс** в верхнем левом углу окна портала Azure.

2. Выберите **базы данных** из **новой** страницы и выберите **аналитику Azure Synapse** под **функцией** на **новой** странице.
...
3. Заполните форму, указав следующую информацию. 
   ...
   | ------------------ | --------------------- | ------------------------------------------------------------ |
   | *Имя**            | mySampleDataWarehouse | Для действительных имен баз данных [см.](/sql/relational-databases/databases/database-identifiers) |
...
12. На панели инструментов выберите **Уведомления** для мониторинга процесса развертывания.
...
5. Выберите **настройки брандмауэра**Show. Откроется страница **параметров брандмауэра** для сервера Базы данных SQL. 
...
> [!IMPORTANT]
> По умолчанию доступ через брандмауэр базы данных SQL включен для всех служб Azure. Выберите **OFF** на этой странице, а затем выберите **Сохранить,** чтобы отключить брандмауэр для всех служб Azure.
...
1. В Object Explorer выберите меню **Connect** drop down и выберите **движок базы данных.** Откроется диалоговое окно **Соединение с сервером** .
...
В этом разделе используются только что определенные внешние таблицы для загрузки выборочных данных из Azure Storage Blob.  
...
1. Войдите на [портал Azure,](https://portal.azure.com)выберите хранилище данных.
...
3. Чтобы удалить хранилище данных, чтобы не взимать плату за вычисление или хранение данных, выберите **Delete.**
...
Перейти к обзору разработки, чтобы узнать, как перенести существующую базу данных в Azure Synapse Analytics.

> [!div class="nextstepaction"]
> [Проектные решения о переносе существующей базы данных в Azure Synapse Analytics](sql-data-warehouse-overview-develop.md)
