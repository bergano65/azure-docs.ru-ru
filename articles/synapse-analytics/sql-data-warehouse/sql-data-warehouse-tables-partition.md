---
title: Таблицы разгородки
description: Рекомендации и примеры использования теджей в аналитике S'L
...
ms.locfileid: "80351231"
---
# <a name="partitioning-tables-in-sql-analytics"></a>Таблицы разделов в аналитике S'L
Рекомендации и примеры использования теджейных разделов в S'L Analytics.

## <a name="what-are-table-partitions"></a>Что такое секции таблицы
Секции таблицы позволяют разделять данные на более мелкие группы данных. В большинстве случаев секции таблицы создаются по столбцу с датами. Раздел поддерживается во всех типах таблиц S'L Analytics; включая кластерный столбец, кластерный индекс и кучи. Секционирование также поддерживается для всех типов распределения, включая распределение с помощью хэша или методом циклического перебора.  
...
При секционировании таблиц с **кластеризованными индексами columnstore** важно учитывать, сколько строк будет содержать каждая секция. Для оптимального сжатия и производительности таких таблиц требуется как минимум 1 миллион строк на одно распределение и одну секцию. Перед созданием разделов s'L Analytics уже делит каждую таблицу на 60 распределенных баз данных. Таким образом, секционирование таблицы выступает дополнением к распределениям, созданным в фоновом режиме. Используя этот пример, если таблица фактов продаж содержала 36 ежемесячных разделов, и учитывая, что база данных S'L Analytics содержит 60 дистрибутивов, то таблица фактов продаж должна содержать 60 миллионов строк в месяц, или 2,1 миллиарда строк, когда все месяцы населены. Если в таблице намного меньше строк, чем рекомендуемый минимум на одну секцию, уменьшите количество секций, чтобы на каждую из них приходилось больше строк. Дополнительные сведения см. в статье [Индексирование таблиц в хранилище данных SQL](sql-data-warehouse-tables-index.md). В ней рассматриваются запросы, с помощью которых можно оценить качество кластеризованных индексов columnstore.

## <a name="syntax-differences-from-sql-server"></a>Отличия синтаксиса от SQL Server
Аналитика S'L вводит способ определения разделов, который проще, чем сервер S'L. Функции и схемы раздела не используются в s'L Analytics, как это имеется в сервере S'L. Вам нужно только определить секционированный столбец и граничные точки. Хотя синтаксис секционирования может немного отличаться от синтаксиса SQL Server, основные понятия одинаковые. Сервер и аналитика S'L поддерживают одну колонку разделов на стол, которая может быть разлительной. Дополнительные сведения о секционировании см. в статье [Секционированные таблицы и индексы](/sql/relational-databases/partitions/partitioned-tables-and-indexes).
...
Для переноса определений раздела сервера S'L в аналитику S'L просто:
...
Если вы переносите секционированную таблицу из экземпляра SQL Server, количество строк в каждой секции можно запросить с помощью приведенного ниже алгоритма SQL. Имейте в виду, что если та же гранулированная часть раздела используется в S'L Analytics, то количество строк на раздел уменьшается в 60 раз.  
...
Аналитика s'L поддерживает разделение, слияние и переключение разделов. Для каждой из этих функций используется инструкция [ALTER TABLE](/sql/t-sql/statements/alter-table-transact-sql).
...
### <a name="load-new-data-into-partitions-that-contain-data-in-one-step"></a>Загрузите новые данные в разделы, содержащие данные за один шаг
Загрузка данных в разделы с переключением разделов является удобным способом этапа новых данных в таблице, которая не видна пользователям переключатель в новых данных.  Это может быть сложной задачей для занятых систем для решения блокировки спора, связанного с переключением разделов.  Чтобы очистить существующие данные в разделе, `ALTER TABLE` используется для переключения данных.  Затем `ALTER TABLE` для переключения новых данных требовался еще один.  В s'L `TRUNCATE_TARGET` Analytics опция `ALTER TABLE` поддерживается в команде.  С `TRUNCATE_TARGET` `ALTER TABLE` помощью команды перезаписываетсуществующие данные в разделе с новыми данными.  Ниже приведен пример, `CTAS` который использует для создания новой таблицы с существующими данными, вставляет новые данные, а затем переключает все данные обратно в целевую таблицу, перезаписывая существующие данные.
