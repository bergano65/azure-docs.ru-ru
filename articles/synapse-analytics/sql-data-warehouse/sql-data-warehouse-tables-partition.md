...
description: Рекомендации и примеры использования теджей в пуле Synapse S'L
...
При секционировании таблиц с **кластеризованными индексами columnstore** важно учитывать, сколько строк будет содержать каждая секция. Для оптимального сжатия и производительности таких таблиц требуется как минимум 1 миллион строк на одно распределение и одну секцию. Перед созданием разделов пул Synapse S'L уже делит каждую таблицу на 60 распределенных баз данных. Таким образом, секционирование таблицы выступает дополнением к распределениям, созданным в фоновом режиме. Используя этот пример, если таблица фактов продаж содержала 36 ежемесячных разделов, и учитывая, что пул Synapse S'L содержит 60 дистрибутивов, то таблица фактов продаж должна содержать 60 миллионов строк в месяц, или 2,1 миллиарда строк, когда все месяцы заселены. Если в таблице намного меньше строк, чем рекомендуемый минимум на одну секцию, уменьшите количество секций, чтобы на каждую из них приходилось больше строк. Дополнительные сведения см. в статье [Индексирование таблиц в хранилище данных SQL](sql-data-warehouse-tables-index.md). В ней рассматриваются запросы, с помощью которых можно оценить качество кластеризованных индексов columnstore.
