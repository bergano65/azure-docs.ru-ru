...
description: Рекомендации и примеры для индексирующих таблиц в пуле Synapse S'L.
...
По умолчанию пул Synapse S'L создает кластерный индекс столбцов, когда параметры индекса не указаны на столе. Кластеризованные таблицы Columnstore обеспечивают максимальную производительность запросов, а также самый высокий уровень сжатия данных.  Обычно эти таблицы более эффективны, чем таблицы с кластеризованными индексами и таблицы без кластеризованных индексов, и их рекомендуется использовать в больших таблицах.  Таким образом, если вы не уверены, какой метод индексации применить к вашей таблице, используйте кластеризованные индексы Columnstore.  
...
При временной посадке данных в пуле Synapse S'L вы можете обнаружить, что использование таблицы кучи ускоряет общий процесс. Это связано с тем, что загрузка в кучи выполняется быстрее, чем в таблицы с кластеризованными индексами, и в некоторых случаях из кэша можно выполнять последующее считывание.  Загрузка таблицы в таблицу без кластеризованных индексов перед выполнением преобразования данных выполняется намного быстрее, чем загрузка данных в таблицу с кластеризованными индексами Columnstore. Кроме того, загрузка данных во [временную таблицу](sql-data-warehouse-tables-temporary.md) выполняется быстрее, чем загрузка таблицы в постоянное хранилище.  После загрузки данных можно создать индексы в таблице для более быстрой производительности запроса.  

Таблицы кластерных столбцов начинают достигать оптимального сжатия, как только насто появляется более 60 миллионов строк.  Для небольших таблиц поиска, менее 60 миллионов строк, рассмотрите возможность использования ИНДЕКСа HEAP или кластерного индекса для более высокой производительности запроса. 
...
Небольшие нагрузки, впадающие в бассейн Synapse S'L, также иногда называются струйками. Обычно они представляют собой практически постоянный поток данных, принимаемых системой. Однако поскольку этот поток является не совсем непрерывным, объем строк невелик. Чаще всего количество данных значительно ниже порогового значения, необходимого для непосредственной загрузки в формате columnstore.
