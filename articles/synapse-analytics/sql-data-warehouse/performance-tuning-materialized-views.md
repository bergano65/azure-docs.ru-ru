---
title: Настройка производительности с материализованными представлениями
description: Сведения о рекомендациях и вопросах, которые следует знать при использовании материализованных представлений для повышения производительности запросов.
...
Материализованные представления в пуле SQL Azure синапсе обеспечивают низкий уровень обслуживания сложных аналитических запросов, чтобы обеспечить высокую производительность без изменения запроса. В этой статье рассматриваются общие рекомендации по использованию материализованных представлений.

## <a name="materialized-views-vs-standard-views"></a>Материализованные представления и стандартные представления

Пул SQL в Azure синапсе поддерживает стандартные и материализованные представления.  И те, и другие являются виртуальными таблицами, созданными с помощью выражений SELECT и представленными для запросов в виде логических таблиц.  Представления инкапсулируют сложность общих вычислений данных и добавляют уровень абстракции к вычислительным изменениям, поэтому нет необходимости переписывать запросы.  

Стандартное представление рассчитывает данные при каждом использовании представления.  Данные не хранятся на диске. Обычно пользователи используют стандартные представления в качестве средства, помогающего упорядочить логические объекты и запросы в пуле SQL.  Чтобы использовать стандартное представление, запрос должен явно сослаться на него.
...
|Содержимое представления                    | Создается каждый раз при использовании представления.   | Обрабатывается предварительно и сохраняется в пуле SQL при создании представления. Обновляется по мере добавления данных в базовые таблицы.
...
- Требует минимального обслуживания представлений.  Все добавочные изменения данных из базовых таблиц автоматически добавляются в материализованные представления синхронным образом.  Такая схема позволяет запросам из материализованных представлений возвращать те же данные, что и при непосредственном обращении к базовым таблицам.
...
Каждое материализованное представление связано с затратами на хранение данных и обслуживание представления.  При изменении данных в базовых таблицах размер материализованных представлений увеличивается, а также изменяется их физическая структура.  Чтобы избежать снижения производительности запросов, каждое материализованные представление сохраняется отдельно модулем SQL Analytics.  
...
Этот запрос можно выполнить для списка материализованных представлений в пуле SQL:
...
Оптимизатор SQL Analytics может автоматически использовать развернутые материализованные представления для повышения производительности запросов.  Эта поддержка прозрачна для запросов, которые не ссылаются на представления, и запросов, использующих статистические выражения, не поддерживаемые при создании материализованных представлений.  Изменение запроса не требуется. Вы можете просмотреть предполагаемый план выполнения запроса, чтобы проверить использование материализованных представлений.  

**Мониторинг материализованных представлений**

Материализованные представления хранятся в пуле SQL точно так же, как таблица с кластеризованным индексом columnstore (CCI).  Чтение данных из материализованного представления предусматривает сканирование сегментов индекса CCI и применение всех добавочных изменений из базовых таблиц. Если число добавочных изменений слишком велико, разрешение запроса из материализованного представления может занять больше времени, чем непосредственное выполнение запроса к базовым таблицам.  

Чтобы избежать снижения производительности запросов, рекомендуется выполнять запрос [DBCC PDW_SHOWMATERIALIZEDVIEWOVERHEAD](/sql/t-sql/database-console-commands/dbcc-pdw-showmaterializedviewoverhead-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json) для отслеживания коэффициента_дополнительных_затрат представления (строк_всего/максимум (1, строк_в_базовом_представлении)).  Пользователи должны перестроить материализованные представления, если коэффициент_дополнительных_затрат слишком велик.

**Материализованные представления и кэширование результирующего набора**

Эти две функции появились в SQL Analytics одновременно для настройки производительности запросов.  Кэширование результирующего набора используется для достижения высокой степени параллелизма и быстрого ответа на повторяющиеся запросы к статическим данным.  
...
Проверьте еще раз план выполнения исходного запроса.  Теперь количество соединений изменилось с 17 до 5, перемешивания и вовсе отсутствуют.  Щелкните значок операции фильтрации в плане. Список выходных данных показывает, что данные считываются из материализованных представлений, а не из базовых таблиц.  
 ...
 ![Список выходных данных плана с материализованными представлениями](./media/performance-tuning-materialized-views/output-list.png)
...
Дополнительные советы по разработке приведены в [обзоре разработки пула Synapse SQL](sql-data-warehouse-overview-develop.md).
