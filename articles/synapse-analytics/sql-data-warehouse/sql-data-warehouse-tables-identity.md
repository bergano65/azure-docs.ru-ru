...
description: Рекомендации и примеры использования свойства IDENTITY для создания суррогатных ключей на столах в пуле Synapse S'L.
...
---
# <a name="using-identity-to-create-surrogate-keys-in-synapse-sql-pool"></a>Использование IDENTITY для создания суррогатных ключей в пуле Synapse S'L

Рекомендации и примеры использования свойства IDENTITY для создания суррогатных ключей на столах в пуле Synapse S'L.
...
Свойство IDENTITY предназначено для масштабирования всех дистрибутивов в пуле Synapse S'L без влияния на производительность нагрузки. Поэтому реализация IDENTITY ориентирована на достижение этих целей.
...
Свойство IDENTITY не гарантирует порядок, в котором распределяются суррогатные значения, что отражает поведение SQL Server и базы данных SQL Azure. Тем не менее, в бассейне Synapse S'L отсутствие гарантии является более выраженным.
...
Бассейн Synapse S'L поддерживает `SET IDENTITY_INSERT <your table> ON|OFF` синтаксис. Этот синтаксис позволяет явно вставить значения в столбец IDENTITY.
...
Для получения дополнительной информации о данных о загрузке [см.](design-elt-data-loading.md) [Loading best practices](guidance-for-loading-data.md)
...
Следующие связанные функции не поддерживаются в пуле Synapse S'L:

- [ИДЕНТИЧНОСТЬ()](/sql/t-sql/functions/identity-function-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
