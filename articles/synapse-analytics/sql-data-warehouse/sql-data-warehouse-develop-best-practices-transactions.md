...
description: Узнайте, как оптимизировать производительность вашего транзакционного кода в S'L Analytics, минимизируя при этом риск длительных откатов.
...
ms.locfileid: "80350585"
---
# <a name="optimizing-transactions-in-sql-analytics"></a>Оптимизация транзакций в аналитике S'L
Узнайте, как оптимизировать производительность вашего транзакционного кода в S'L Analytics, минимизируя при этом риск длительных откатов.

## <a name="transactions-and-logging"></a>Транзакции и ведение журнала
Транзакции представляют собой важную часть ядра реляционной СУБД. При модификации данных используется транзакции. Эти транзакции могут быть явными или неявными. Одиночные инструкции INSERT, UPDATE и DELETE являются примерами неявных транзакций. В явных транзакциях используются инструкции BEGIN TRAN, COMMIT TRAN или ROLLBACK TRAN. Явные транзакции обычно используются, когда несколько инструкций изменения нужно связать в одну атомарную единицу. 

СЗЛ Аналитика фиксирует изменения в базе данных с помощью журналов транзакций. У каждого распределения есть свой журнал транзакций. Запись в журналы транзакций выполняется автоматически. Настраивать что-либо не требуется. Этот процесс гарантирует запись, но в то же время увеличивает нагрузку на систему. Чтобы минимизировать этот эффект, нужно написать эффективный код транзакции. Такой код можно условно разделить на две категории.
...
| --- | --- | --- |
| Куча |Любой |**Небольшие** |
| Кластеризованный индекс |Пустая целевая таблица |**Небольшие** |
| Кластеризованный индекс |Загруженные строки не перекрывают существующие страницы в целевом объекте |**Небольшие** |
| Кластеризованный индекс |Загруженные строки перекрывают существующие страницы в целевом объекте |Полное |
| Кластеризованный индекс columnstore |Размер пакета >= 102 400 для распределения с выравниванием по разделам |**Небольшие** |
...
> [!NOTE]
> Повторное создание больших таблиц может принести пользу с помощью функций управления рабочей нагрузкой S'L Analytics. Дополнительные сведения см. в разделе [Классы ресурсов для управления рабочими нагрузками](resource-classes-for-workload-management.md).
> 
> 
...
Аналитика s'L позволяет [приостановить, возобновить и масштабировать](sql-data-warehouse-manage-compute-overview.md) свой пул по требованию. При приостановке или масштабировании пула S'L важно понимать, что любые транзакции в полете немедленно прекращаются; что приводит к откату любых открытых транзакций. Если в рамках рабочей нагрузки выполнено длительное и незавершенное изменение данных перед операцией приостановки или масштабирования, это изменение нужно отменить. Это отмена может повлиять на время, необходимое для приостановки или масштабирования пула S'L. 
...
Подробнее об уровнях изоляции и транзакционных ограничениях читайте в информации о [транзакциях.](sql-data-warehouse-develop-transactions.md)  Рекомендации по использованию хранилища данных SQL Azure см. в статье [Рекомендации по использованию хранилища данных SQL Azure](sql-data-warehouse-best-practices.md).
