...
description: Узнайте, как оптимизировать производительность транзакционного кода в Synapse S'L при минимизации риска для длительных откатов.
...
---
# <a name="optimizing-transactions-in-synapse-sql"></a>Оптимизация транзакций в Synapse S'L

Узнайте, как оптимизировать производительность транзакционного кода в Synapse S'L при минимизации риска для длительных откатов.

## <a name="transactions-and-logging"></a>Транзакции и ведение журнала

Транзакции представляют собой важную часть ядра реляционной СУБД. Транзакции используются при модификации данных. Эти транзакции могут быть явными или неявными. Одиночные инструкции INSERT, UPDATE и DELETE являются примерами неявных транзакций. В явных транзакциях используются инструкции BEGIN TRAN, COMMIT TRAN или ROLLBACK TRAN. Явные транзакции обычно используются, когда несколько инструкций изменения нужно связать в одну атомарную единицу.

Изменения в базе данных отслеживаются с помощью журналов транзакций. У каждого распределения есть свой журнал транзакций. Запись в журналы транзакций выполняется автоматически. Настраивать что-либо не требуется. Этот процесс гарантирует запись, но в то же время увеличивает нагрузку на систему. Чтобы минимизировать этот эффект, нужно написать эффективный код транзакции. Такой код можно условно разделить на две категории.
...
* СОЗДАЙТЕ ТАБЛИЦУ КАК ВЫБЕРИТЕ[(CTAS](sql-data-warehouse-develop-ctas.md))
* INSERT..SELECT
* CREATE INDEX
* ALTER INDEX REBUILD
* DROP INDEX
...
> [!NOTE]
> Повторное создание больших таблиц может принести пользу с помощью функций управления рабочей нагрузкой пула Synapse s'L. Дополнительные сведения см. в разделе [Классы ресурсов для управления рабочими нагрузками](resource-classes-for-workload-management.md).
...
Синапсе S'L позволяет [приостановить, возобновить и масштабировать](sql-data-warehouse-manage-compute-overview.md) свой пул по требованию. При приостановке или масштабировании пула S'L важно понимать, что любые транзакции в полете немедленно прекращаются; что приводит к откату любых открытых транзакций. Если в рамках рабочей нагрузки выполнено длительное и незавершенное изменение данных перед операцией приостановки или масштабирования, это изменение нужно отменить. Это отмена может повлиять на время, необходимое для приостановки или масштабирования пула S'L.
...
* Переписать длительные операции с помощью [CTAS](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
* Разбейте операцию на блоки для работы с подмножествами строк.

## <a name="next-steps"></a>Дальнейшие действия

[См. Транзакции в Synapse S'L,](sql-data-warehouse-develop-transactions.md) чтобы узнать больше об уровнях изоляции и лимитах транзакций.  Рекомендации по использованию хранилища данных SQL Azure см. в статье [Рекомендации по использованию хранилища данных SQL Azure](sql-data-warehouse-best-practices.md).
