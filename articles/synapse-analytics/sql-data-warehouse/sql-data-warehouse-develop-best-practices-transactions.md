...
description: Узнайте, как оптимизировать производительность транзакционного кода в выделенном пуле SQL и свести к минимуму риск длительных откатов.
...
Изменения в пуле SQL отправляются с помощью журналов транзакций. У каждого распределения есть свой журнал транзакций. Запись в журналы транзакций выполняется автоматически. Настраивать что-либо не требуется. Этот процесс гарантирует запись, но в то же время увеличивает нагрузку на систему. Чтобы минимизировать этот эффект, нужно написать эффективный код транзакции. Такой код можно условно разделить на две категории.
...
> [!NOTE]
> Для повторного создания больших таблиц можно воспользоваться выделенными функциями управления рабочей нагрузкой пула SQL. Дополнительные сведения см. в разделе [Классы ресурсов для управления рабочими нагрузками](resource-classes-for-workload-management.md).
...
Дополнительные сведения об уровнях изоляции и ограничениях транзакций см. [в разделе транзакции в выделенном пуле SQL](sql-data-warehouse-develop-transactions.md) .  Общие сведения о других рекомендациях см. в разделе рекомендации по [выделению пула SQL](sql-data-warehouse-best-practices.md).
