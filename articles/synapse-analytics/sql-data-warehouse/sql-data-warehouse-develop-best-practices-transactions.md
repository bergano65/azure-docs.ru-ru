...
description: Узнайте, как оптимизировать производительность транзакционного кода в синапсе SQL и свести к минимуму риск длительных откатов.
...
ms.locfileid: "80633563"
---
# <a name="optimizing-transactions-in-synapse-sql"></a>Оптимизация транзакций в синапсе SQL

Узнайте, как оптимизировать производительность транзакционного кода в синапсе SQL и свести к минимуму риск длительных откатов.

## <a name="transactions-and-logging"></a>Транзакции и ведение журнала

Транзакции представляют собой важную часть ядра реляционной СУБД. Транзакции используются во время изменения данных. Эти транзакции могут быть явными или неявными. Одиночные инструкции INSERT, UPDATE и DELETE являются примерами неявных транзакций. В явных транзакциях используются инструкции BEGIN TRAN, COMMIT TRAN или ROLLBACK TRAN. Явные транзакции обычно используются, когда несколько инструкций изменения нужно связать в одну атомарную единицу.

Изменения в базе данных записываются с помощью журналов транзакций. У каждого распределения есть свой журнал транзакций. Запись в журналы транзакций выполняется автоматически. Настраивать что-либо не требуется. Этот процесс гарантирует запись, но в то же время увеличивает нагрузку на систему. Чтобы минимизировать этот эффект, нужно написать эффективный код транзакции. Такой код можно условно разделить на две категории.
...
* CREATE TABLE как SELECT ([CTAS](sql-data-warehouse-develop-ctas.md))
* INSERT..SELECT
* CREATE INDEX
* ALTER INDEX REBUILD
* DROP INDEX
...
UPDATE является полностью регистрируемой операцией.  Если необходимо обновить большое количество строк в таблице или секции, то часто может быть гораздо эффективнее использовать минимально регистрируемую операцию, например [CTAS](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest) .
...
> [!NOTE]
> Для повторного создания больших таблиц можно воспользоваться функциями управления рабочей нагрузкой пула SQL синапсе. Дополнительные сведения см. в разделе [Классы ресурсов для управления рабочими нагрузками](resource-classes-for-workload-management.md).
...
Синапсе SQL позволяет [приостанавливать, возобновлять и масштабировать](sql-data-warehouse-manage-compute-overview.md) пул SQL по запросу. При приостановке или масштабировании пула SQL важно понимать, что все выполняющиеся транзакции немедленно прерываются; Это приводит к откату всех открытых транзакций. Если в рамках рабочей нагрузки выполнено длительное и незавершенное изменение данных перед операцией приостановки или масштабирования, это изменение нужно отменить. Это может повлиять на время, затрачиваемое на приостановку или масштабирование пула SQL.
...
* Перезапись длительно выполняемых операций с помощью [CTAS](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
* Разбейте операцию на блоки для работы с подмножествами строк.

## <a name="next-steps"></a>Дальнейшие шаги

Дополнительные сведения об уровнях изоляции и ограничениях транзакций см. [в разделе транзакции в СИНАПСЕ SQL](sql-data-warehouse-develop-transactions.md) .  Рекомендации по использованию хранилища данных SQL Azure см. в статье [Рекомендации по использованию хранилища данных SQL Azure](sql-data-warehouse-best-practices.md).
