...
>[!NOTE]
>Рекомендуется создавать управляемые частные конечные точки для подключения ко всем источникам данных Azure. В будущем весь исходящий трафик из виртуальных сетей управляемых рабочих областей будет блокироваться.
...
Управляемая частная конечная точка, предназначенная для пула SQL, будет называться *synapse-ws-sql--\<имя рабочей области\>* , а для SQL по запросу — *synapse-ws-sqlOnDemand--\<имя рабочей области\>* .
