...
- доступ плоскости данных к Базам данных SQL (как для выделенных пулов SQL, так и для бессерверного пула SQL);
- создание учетных данных для баз данных бессерверного пула SQL через учетную запись хранения.
...
> Для чтения и записи напрямую из Spark в выделенный пул SQL или из него пользователю Spark требуется разрешение *db_owner*.

Создав пользователей, проверьте, может ли бессерверный пул SQL запрашивать учетную запись хранения.
...
> [!IMPORTANT]
> Удостоверению рабочей области необходимо предоставить доступ к пулу SQL напрямую для успешного выполнения конвейеров, включающих в себя наборы данных или действия, которые ссылаются на выделенный пул SQL.

Выполните следующие команды в каждом выделенном пуле SQL, чтобы разрешить управляемому удостоверению рабочей области выполнять конвейеры в базе данных пула SQL:
