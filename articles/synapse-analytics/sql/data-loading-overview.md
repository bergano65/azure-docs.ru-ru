---
title: Вместо ETL, дизайн ELT для бассейна Synapse S'L (ru) Документы Майкрософт
description: Вместо ETL разработайте процесс экстракта, загрузки и трансформации (ELT) для загрузки данных или пула S'L.
...
ms.locfileid: "81429594"
---
# <a name="designing-a-polybase-data-loading-strategy-for-azure-synapse-sql-pool"></a>Разработка стратегии загрузки данных PolyBase для пула Azure Synapse S'L

Традиционные хранилища данных SMP используют процесс извлечения, преобразования и загрузки (ETL) для загрузки данных. Бассейн Azure S'L — это массивно параллельная обработка (MPP), которая использует масштабируемость и гибкость ресурсов вычислений и хранения данных. Применяя процесс извлечения, загрузки и преобразования (ELT), можно воспользоваться преимуществами MPP и исключить ресурсы, необходимые для преобразования данных перед загрузкой.

В то время как пул S'L поддерживает многие методы загрузки, включая не-Полибазы варианты, такие как BCP и S'L BulkCopy API, самый быстрый и масштабируемый способ загрузки даты через PolyBase.  PolyBase — это технология, которая обращается к внешним данным, хранящимся в хранилище BLOB-объектов Azure или Azure Data Lake Storage, с помощью языка T-SQL.
...
Основными шагами для реализации PolyBase ELT для пула S'L являются:
...
4. Загрузите данные в промежуточные таблицы пула СЗЛ с помощью PolyBase.
...
Если вы выполняете экспорт из SQL Server, можно воспользоваться [программой командной строки bcp](/sql/tools/bcp-utility?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json&view=azure-sqldw-latest) для экспорта данных в текстовые файлы с разделителями. Картирование типа данных типа «Паркет» для Типа DW — это следующее:
...
- [СОЗДАНИЕ ВНЕШНЕЙ ТАБЛИЦЫ](/sql/t-sql/statements/create-external-table-transact-sql?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json&view=azure-sqldw-latest)

Пример создания внешних объектов см. в разделе [Создание внешних таблиц для примеров данных](../sql-data-warehouse/load-data-from-azure-blob-storage-using-polybase.md?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json#create-external-tables-for-the-sample-data) в руководстве по загрузке.
...
- Формат данных в текстовом файле, чтобы привести их в соответствие с столбцов и типами данных в таблице назначения пула S'L. Неполное соответствие между типами данных во внешних текстовых файлах и таблице хранилища данных вызовет отклонение строк во время загрузки.
...
## <a name="4-load-the-data-into-sql-pool-staging-tables-using-polybase"></a>4. Загрузите данные в промежуточные таблицы пула СЗЛ с помощью PolyBase
...
- [PolyBase со службами SSIS](/sql/integration-services/load-data-to-sql-data-warehouse?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json&view=azure-sqldw-latest) является оптимальным выбором, когда исходные данные хранятся в SQL Server — локально или в облаке. Службы SSIS определяют сопоставления исходной и целевой таблиц, а также управляют загрузкой. При наличии пакетов служб SSIS можно изменить пакеты для работы с новым назначением хранилища данных.
...
- [PolyBase с Azure Databricks](../../azure-databricks/databricks-extract-load-sql-data-warehouse.md?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json) передает данные из таблицы хранилища данных в кадр данных Databricks и/или записывает данные с кадрового фрейма Databricks в таблицу хранилища данных с помощью PolyBase.

### <a name="non-polybase-loading-options"></a>Варианты загрузки, отличные от PolyBase

Если данные несовместимы с PolyBase, можно использовать программу [bcp](/sql/tools/bcp-utility?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json&view=azure-sqldw-latest) или [API-интерфейс SQLBulkCopy](/dotnet/api/system.data.sqlclient.sqlbulkcopy?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json). bcp загружается непосредственно в пул S'L, не проходя через хранилище Azure Blob, и предназначен только для небольших нагрузок. Обратите внимание, что производительность загрузки этих вариантов значительно ниже, чем у PolyBase.
