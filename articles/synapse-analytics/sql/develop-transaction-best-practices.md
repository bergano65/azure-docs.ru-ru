---
title: Оптимизация транзакций для пула S'L
description: Узнайте, как оптимизировать производительность вашего транзакционного кода в пуле S'L (хранилище данных) при минимизации риска для длительных откатов.
...
---
# <a name="optimizing-transactions-in-sql-pool"></a>Оптимизация транзакций в пуле S'L

Узнайте, как оптимизировать производительность транзакционного кода в пуле S'L, минимизируя при этом риск длительных откатов.

## <a name="transactions-and-logging"></a>Транзакции и ведение журнала

Транзакции представляют собой важную часть ядра реляционной СУБД. В пуле S'L используются транзакции при модификации данных. Эти транзакции могут быть явными или неявными. Одиночные инструкции INSERT, UPDATE и DELETE являются примерами неявных транзакций. В явных транзакциях используются инструкции BEGIN TRAN, COMMIT TRAN или ROLLBACK TRAN. Явные транзакции обычно используются, когда несколько инструкций изменения нужно связать в одну атомарную единицу.

Пулс S'L фиксирует изменения в базе данных с помощью журналов транзакций. У каждого распределения есть свой журнал транзакций. Запись в журналы транзакций выполняется автоматически. Настраивать что-либо не требуется. Этот процесс гарантирует запись, но в то же время увеличивает нагрузку на систему. Чтобы минимизировать этот эффект, нужно написать эффективный код транзакции. Такой код можно условно разделить на две категории.
...
* СОЗДАЙТЕ ТАБЛИЦУ КАК ВЫБЕРИТЕ[(CTAS](../sql-data-warehouse/sql-data-warehouse-develop-ctas.md?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json)
* INSERT..SELECT
* CREATE INDEX
* ALTER INDEX REBUILD
* DROP INDEX
...
UPDATE является полностью регистрируемой операцией.  Если вам нужно обновить большое количество строк в таблице или раздел, часто может быть гораздо более эффективным использовать минимально зарегистрированные операции, такие как [CTAS,](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse) чтобы сделать это.
...
> [!NOTE]
> Повторное создание больших таблиц может принести пользу с помощью функций управления рабочей нагрузкой пула S'L. Дополнительные сведения см. в разделе [Классы ресурсов для управления рабочими нагрузками](../sql-data-warehouse/resource-classes-for-workload-management.md?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json).
...
Azure Synapse Analytics позволяет [приостановить, возобновить и масштабировать](../sql-data-warehouse/sql-data-warehouse-manage-compute-overview.md?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json) пул S'L по требованию. 

При приостановке или масштабировании пула S'L важно понимать, что любые транзакции в полете немедленно прекращаются; что приводит к откату любых открытых транзакций. 

Если в рамках рабочей нагрузки выполнено длительное и незавершенное изменение данных перед операцией приостановки или масштабирования, это изменение нужно отменить. Это отмена может повлиять на время, необходимое для приостановки или масштабирования пула S'L. 
...
Наилучшим сценарием является выполнение транзакций по изменению данных о полетах до приостановки или масштабирования пула S'L. Но это не всегда удобно. Чтобы снизить риск длительного отката, выполните одно из следующих действий.

* Переписать длительные операции с помощью [CTAS](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse)
* Разбейте операцию на блоки для работы с подмножествами строк.

## <a name="next-steps"></a>Дальнейшие действия

Просмотрите [транзакции в пуле S'L,](develop-transactions.md) чтобы узнать больше об уровнях изоляции и лимитах транзакций.  Для получения обзора других рекомендаций [по](best-practices-sql-pool.md)наилучшей практике см.
