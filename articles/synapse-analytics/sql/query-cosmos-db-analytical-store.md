...
- Несерверный пул SQL сейчас не поддерживает [Azure Cosmos DB схему полной точности](../../cosmos-db/analytical-store-introduction.md#schema-representation). Используйте неиспользуемый пул SQL Server только для доступа к четко определенной схеме Cosmos DB.
