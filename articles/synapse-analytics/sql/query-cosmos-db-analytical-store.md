...
> [!IMPORTANT]
> Убедитесь, что используются параметры сортировки базы данных UTF-8 (например,), `Latin1_General_100_CI_AS_SC_UTF8` так как строковые значения в Cosmos DB аналитическом хранилище кодируются как текст в кодировке UTF-8.
> Несоответствие между кодировкой текста в файле и параметрами сортировки может привести к непредвиденным ошибкам преобразования текста.
...
- Несерверный пул SQL не возвращает ошибку времени компиляции, если `OPENROSET` Параметры сортировки столбца не имеют кодировки UTF-8. Можно легко изменить параметры сортировки по умолчанию для всех `OPENROWSET` функций, выполняемых в текущей базе данных, с помощью следующей инструкции T-SQL: `alter database current collate Latin1_General_100_CI_AI_SC_UTF8`
