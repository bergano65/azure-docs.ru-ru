...
> [!IMPORTANT]
> В этом руководстве используется контейнер с [четко определенной схемой Azure Cosmos DB](../../cosmos-db/analytical-store-introduction.md#schema-representation) , которая предоставляет возможности запросов, которые будут поддерживаться в будущем. Работа запросов, которая обеспечивается в бессерверном пуле SQL для [Azure Cosmos DB полной схеме точности](#full-fidelity-schema) , является временным поведением, которое будет изменено на основе отзывов о предварительной версии. Не полагайтесь на схему, `OPENROWSET` предоставляемую функцией для полных контейнеров во время общедоступной предварительной версии, так как запрос експеринеце может быть изменен и согласован с четко определенной схемой. Обратитесь к [группе разработчиков по ссылке синапсе](mailto:cosmosdbsynapselink@microsoft.com) , чтобы отправить отзыв.
...
> [!IMPORTANT]
> Схема с полной точностью представляет оба значения с ожидаемыми типами и значениями с неправильными вводом типов.
...
При запросе полной схемы точности необходимо явно указать тип SQL и ожидаемое Cosmos DB тип свойства в `WITH` предложении. В следующем примере предполагается, что `string` имеет правильный тип для `geo_id` Свойства и `int32` правильный тип для `cases` Свойства:
...
Значения с другими типами не будут возвращаться в `geo_id` `cases` столбцах и, а запрос возвратит `NULL` значение в этих ячейках. Этот запрос будет ссылаться только на объект `cases` с указанным типом в выражении ( `cases.int32` ). Если имеются значения с другими типами ( `cases.int64` , `cases.float64` ), которые представляют невозможность очистки в контейнере Cosmos DB, необходимо явно ссылаться на них в `WITH` предложении и объединить результаты. Следующий запрос объединяет `int32` `int64` и, и `float64` хранится в `cases` столбце:
...
- Работа запросов, которая обеспечивается в бессерверном пуле SQL для [Azure Cosmos DB полной схеме точности](#full-fidelity-schema) , является временным поведением, которое будет изменено на основе отзывов о предварительной версии. Не полагайтесь на схему, которую `OPENROWSET` функция предоставляет во время общедоступной предварительной версии, так как запросы могут быть согласованы с четко определенной схемой. Обратитесь к [группе разработчиков по ссылке синапсе](mailto:cosmosdbsynapselink@microsoft.com) , чтобы отправить отзыв.
