...
> [!NOTE]
> Пулы SQL не поддерживают перекрестные запросы к базам данных любого типа. Поэтому необходимо будет пересмотреть реализации аналитики, использующие этот шаблон. SQL по запросу (Предварительная версия) поддерживает запросы между базами данных.
