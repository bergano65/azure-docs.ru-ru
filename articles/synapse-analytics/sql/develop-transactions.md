...
Также следует отметить, что пул SQL не реализует и не поддерживает функцию ERROR_LINE(). Если эта функция имеется в коде, ее необходимо удалить, чтобы она соответствовала пулу SQL. Вместо этого используйте в коде метки запросов, чтобы реализовать эквивалентную функциональность. Дополнительные сведения см. в статье [Метка](develop-label.md) .
...
В пуле SQL есть несколько ограничений, относящихся к транзакциям. Вот они:
