---
title: Рекомендации по использованию несерверного пула SQL (Предварительная версия)
description: Рекомендации и рекомендации по работе с бессерверным пулом SQL (Предварительная версия).
...
ms.locfileid: "94638875"
---
# <a name="best-practices-for-serverless-sql-pool-preview-in-azure-synapse-analytics"></a>Рекомендации по бессерверному пулу SQL (Предварительная версия) в Azure синапсе Analytics

В этой статье вы найдете набор рекомендаций по использованию бессерверного пула SQL (Предварительная версия). Бессерверный пул SQL — это ресурс в Azure синапсе Analytics.
...
## <a name="colocate-your-azure-storage-account-and-serverless-sql-pool"></a>Совместное размещение учетной записи хранения Azure и бессерверного пула SQL
...
- Преобразуйте CSV и JSON в Parquet. Parquet — это формат столбцов. Благодаря сжатию файлы в этом формате меньше по размеру, чем CSV- или JSON-файлы с теми же данными. Для бессерверного пула SQL потребуется меньше времени и меньше запросов на хранение.
