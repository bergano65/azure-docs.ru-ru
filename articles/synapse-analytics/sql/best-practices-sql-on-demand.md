...
- Преобразование CSV и JSON в Parquet-Parquet имеет формат столбцов. Так как он сжимается, размер файлов меньше CSV-или JSON-файлов с теми же данными. SQL по запросу требует меньше времени и запросов на хранение для чтения.
...
> [!NOTE]
> Функции, используемые для удаления секций FilePath и FileInfo, в настоящее время не поддерживаются для внешних таблиц, отличных от созданных автоматически для каждой таблицы, созданной в синапсе Spark.
...
## <a name="use-parser_version-20-for-querying-csv-files"></a>Использование PARSER_VERSION 2,0 для запроса CSV-файлов

При запросе CSV-файлов можно использовать средство синтаксического анализа, оптимизированное для производительности. Дополнительные сведения см. в [PARSER_VERSION](develop-openrowset.md) .
...
## <a name="aad-pass-through-performance"></a>Производительность сквозной передачи AAD

SQL по запросу позволяет получать доступ к файлам в хранилище, используя сквозные учетные данные AAD или SAS. Вы можете столкнуться с низкой производительностью благодаря сравнению с SAS. 

Если вам нужна лучшая производительность, попробуйте использовать учетные данные SAS для доступа к хранилищу, пока не будет улучшена производительность передачи данных AAD.
