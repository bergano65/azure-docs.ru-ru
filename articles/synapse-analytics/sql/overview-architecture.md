...
В синапсе SQL механизм распределенных запросов выполняется на управляющем узле для оптимизации и координации параллельных запросов. При отправке запроса T-SQL в пул SQL управляющий узел преобразует его в запросы, которые будут выполняться на каждом вычислительном узле в параллельном режиме.
