...
**Вариант 2:** Вы также можете вызвать [сохраненную процедуру в рамках действия копирования.](#invoke-a-stored-procedure-from-a-sql-sink) Этот подход выполняет каждую партию `writeBatchSize` (в ведении свойств) в исходной таблице вместо использования массовой вставки в качестве подхода по умолчанию в деятельности копирования.
...
При копировании данных в базу данных Azure S'L, вы также можете настроить и вызвать указанную пользователем процедуру хранения с дополнительными параметрами на каждой партии исходной таблицы. Функция сохраненной процедуры использует [параметры, оцениваемые таблицей.](https://msdn.microsoft.com/library/bb675163.aspx)
