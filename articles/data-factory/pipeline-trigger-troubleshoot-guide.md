---
title: Устранение неполадок оркестрации конвейера и триггеров в ADF
description: Использование различных методов для устранения неполадок, связанных с триггерами конвейера в ADF
author: ssabat
...
ms.locfileid: "97521851"
---
# <a name="troubleshoot-pipeline-orchestration-and-triggers-in-adf"></a>Устранение неполадок оркестрации конвейера и триггеров в ADF
...
В некоторых контекстах у вас есть ADF-файл и Azure приложение-функция, работающие в частной конечной точке. Вы пытаетесь получить конвейер, который взаимодействует с приложение-функция Azure для работы. Вы предпринимали три разных метода, но один возвращает ошибку `Bad Request` , два других метода возвращают `103 Error Forbidden` .

#### <a name="cause"></a>Причина 
Сейчас ADF не поддерживает закрытый соединитель конечной точки для Azure приложение-функция. Это должна быть причина, по которой Azure приложение-функция отклоняет вызовы, так как она настроена на разрешение только подключений с частного канала.
...
#### <a name="issue"></a>Проблема  
Если файлы в определенной копируемой папке содержат файлы с разными схемами, такими как переменное число столбцов, различные разделители, параметры символов кавычек или некоторые проблемы с данными, конвейер ADF завершит работу в этой ошибке:
...
#### <a name="resolution"></a>Решение
При создании действия Копирование данных выберите параметр "двоичное копирование". Таким образом, при копировании или переносе данных с одного Data Lake на другой, с **двоичным** параметром, ADF не открывает файлы для чтения схемы, а просто обрабатывает все файлы как двоичные и копирует их в другое расположение.
...
#### <a name="issue"></a>Проблема
Часто приходится отслеживать конвейеры ADF за интервалы, скажем 5 минут. Вы можете запрашивать и фильтровать запуски конвейера из фабрики данных с помощью конечной точки. 
...
- [Внешняя отправка уведомлений из ADF](https://www.mssqltips.com/sqlservertip/5962/send-notifications-from-an-azure-data-factory-pipeline--part-2/)
