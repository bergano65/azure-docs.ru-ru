...
1.  Когда группа готова к развертыванию изменений в тестовой или UAT фабрике, команда переходит к выпуску Azure Pipelines и развертывает нужную версию фабрики для разработки в UAT. Это развертывание выполняется как часть задачи Azure Pipelines и использует параметры шаблона Resource Manager, чтобы применить соответствующую конфигурацию.
