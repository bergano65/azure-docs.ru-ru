...
-    Автоматизированное развертывание с помощью интеграции Фабрики данных с [Azure Pipelines](/azure/devops/pipelines/get-started/what-is-azure-pipelines?view=azure-devops).
...
1. Для активации выпуска нажмите кнопку **Создать выпуск** . Сведения об автоматизации создания выпусков см. в статье [Триггеры выпуска](/azure/devops/pipelines/release/triggers?view=azure-devops).
...
Чтобы переопределить шаблон параметризации по умолчанию, перейдите в центр управления и выберите **шаблон параметризации** в разделе Система управления версиями. Выберите **изменить шаблон** , чтобы открыть редактор кода шаблона параметризации. 

![Управление настраиваемыми параметрами](media/author-management-hub/management-hub-custom-parameters.png)

При создании пользовательского шаблона параметризации создается файл с именем **arm-template-parameters-definition.js** в корневой папке ветви Git. Необходимо использовать точное имя файла.
...
* Установка значения свойства в виде строки указывает, что вы хотите параметризовать свойство. Используйте следующий формат: `<action>:<name>:<stype>`.
   ...
   * `<stype>` тип параметра. Если `<stype>` параметр пуст, используется тип по умолчанию `string` . Поддерживаются такие значения: `string`, `bool`, `number`, `object` и `securestring`.
...
- Группа фабрики данных Azure не рекомендует назначать элементы управления Azure RBAC отдельным сущностям (конвейеры, наборы данных и т. д.) в фабрике данных. Например, если у разработчика есть доступ к конвейеру или набору данных, они должны иметь возможность доступа ко всем конвейерам или наборам данных в фабрике. Если вы считаете, что вам нужно реализовать множество ролей Azure в фабрике данных, ознакомьтесь с разворачиванием второй фабрики данных.
