...
| partitionOptions | Задает параметры секционирования данных, используемые для загрузки данных из Teradata. <br>Допустимые значения: **Нет** (по умолчанию), **hash** и **динамикранже**.<br>Если параметр секции включен (то есть не `None`), степень параллелизма для параллельной загрузки данных из базы данных Teradata управляется [`parallelCopies`](copy-activity-performance.md#parallel-copy) параметром действия копирования. | Нет |
