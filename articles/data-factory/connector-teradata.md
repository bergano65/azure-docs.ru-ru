...
При включании раздела копии Data Factory выполняет параллельные запросы против источника Teradata для загрузки данных разделами. Параллельная степень контролируется настройкой [`parallelCopies`](copy-activity-performance-features.md#parallel-copy) на активности копирования. Например, если `parallelCopies` вы установите четыре, Data Factory одновременно генерирует и запускает четыре запроса на основе указанного параметра раздела и настроек, и каждый запрос получает часть данных из тераданных.
...
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| Полная нагрузка с большого стола.                                   | **Вариант раздела**: Хэш. <br><br/>Во время выполнения Data Factory автоматически обнаруживает столбец основного индекса, применяет хэш против него и копирует данные по разделам. |
