---
title: Копирование данных из API API Azure Cosmos DB для MongoDB
...
Следующие свойства поддерживаются в разделе **раковина активности** копирования:

| Свойство | Описание | Обязательно |
|:--- |:--- |:--- |
| type | Свойство **type** приемника действия копирования должно иметь значение **CosmosDbMongoDbApiSink**. |Да |
| writeBehavior |Описывает способ записи данных в Azure Cosmos DB. Допустимые значения: **insert** и **upsert**.<br/><br/>Поведение **upsert** заключается в замене документа, `_id` если документ с тем же уже существует; в противном случае вставьте документ.<br /><br />**Примечание**: Data Factory `_id` автоматически генерирует документ, если `_id` он не указан ни в исходном документе, ни в картографии столбца. Это означает, что для правильной работы **upsert** у документа должен быть идентификатор. |нет<br />(По умолчанию используется **insert**.) |
...
| writeBatchTimeout | Время ожидания операции пакетной вставки, чтобы закончить, прежде чем раз выходит. Разрешенное значение составляет промежуток времени. | нет<br/>(по умолчанию используется **00:30:00** — 30 минут) |
...
* Импортируйте документы JSON из различных источников в Azure Cosmos DB, в том числе из Хранилища MongoDB, Azure Blob, магазина Azure Data Lake Store и других хранилищ, которые поддерживает Azure Data Factory.
