...
Следующие свойства поддерживаются `translator` в  ->  `mappings` объекте-> с помощью `source` и `sink`:
...
Следующие `translator` свойства поддерживаются `mappings` `sink`в дополнение к объекту с помощью `source` и:  -> 
...
|:--- |:--- |:--- |
| type | Свойство типа преобразователя действия копирования должно иметь следующее значение. **TabularTranslator** | Да |
