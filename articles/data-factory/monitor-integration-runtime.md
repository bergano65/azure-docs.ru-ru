...
| State | Состояние среды выполнения интеграции Azure. | 
...
| Description | Описание среды выполнения интеграции.  |
...
| ------ | ------------------ |
| Online | Среда выполнения интеграции Azure подключена к сети и готова к использованию. | 
| Не в сети | Среда выполнения интеграции Azure находится в автономном режиме из-за внутренней ошибки. |
...
| Status | Состояние локальной среды выполнения интеграции и каждого узла. Пример: "В сети", "Автономно", "Ограничено" и т. д. Дополнительные сведения об этих состояниях указаны в следующем разделе. | 
...
| Доступная память | Объем доступной памяти на узле среды выполнения интеграции. Это значение соответствует моментальному снимку в режиме, близком к реальному времени. | 
...
| Status | Description |
| ------ | ------------------ | 
| Online | Узел подключен к службе фабрики данных. |
| Не в сети | Узел находится в автономном режиме. |
...
| Неактивен | Конфигурация узла отличается от конфигурации большинства других узлов. |
...
| Online | Все узлы подключены к сети. |
| Не в сети | Узлы не подключены к сети. |
...
| Свойство или состояние | Description |
| --------------- | ----------- |
...
| State | Общее состояние ("Начальное", "Запуск", "Запущено", "Остановка", "Остановлено") среды выполнения интеграции SSIS Azure. |
...
| Subnet | Имя подсети, к которой нужно присоединить среду выполнения интеграции SSIS Azure. |
...
| Description | Описание среды выполнения интеграции SSIS Azure. |
...
| Status | Description |
| ------ | ----------- | 
| Идет запуск | Этот узел будет подготовлен. |
...
| Недоступно | Этот узел не готов к развертыванию и выполнения пакетов служб SSIS и в нем обнаружены ошибки или проблемы, которые можно устранить. |

### <a name="status-overall-azure-ssis-integration-runtime"></a>Состояние (общее для среды выполнения интеграции SSIS Azure)

| Общее состояние | Description | 
| -------------- | ----------- | 
| Исходный | Узлы среды выполнения интеграции SSIS Azure не выделены или не подготовлены. | 
| Идет запуск | Узлы среды выполнения интеграции SSIS Azure выделяются или подготавливаются, выставления счетов запущено. |
| Запущен | Узлы среды выполнения интеграции SSIS Azure выделены или подготовлены и готовы к развертыванию и выполнению пакетов служб SSIS. |
| Идет остановка  | Узлы среды выполнения интеграции SSIS Azure освобождаются. |
