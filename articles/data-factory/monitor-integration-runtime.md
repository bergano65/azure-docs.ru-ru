---
title: Мониторинг времени выполнения интеграции на фабрике данных Azure
...
Вычислительным ресурсом для среды выполнения интеграции Azure полностью управляет Azure в гибком режиме. В следующей таблице приведены описания свойств, возвращенных командой **Get-AzDataFactoryV2IntegrationRuntime:**
...
| Штат | Состояние среды выполнения интеграции Azure. | 
...
В этом разделе приведены описания свойств, возвращенных cmdlet Get-AzDataFactoryV2IntegrationRuntime. 
...
| Состояние | Состояние локальной среды выполнения интеграции и каждого узла. Пример: Онлайн/Оффлайн/Ограниченное/и т.д. Для получения информации об этих статусах, см. | 
...
| Concurrent Jobs (Running/Limit) (Одновременные задания (выполняемые и ограниченные)) | **Бег**. Число заданий или задач, выполняющихся на каждом узле. Это значение соответствует моментальному снимку в режиме, близком к реальному времени. <br/><br/>**Лимит**. Ограничение означает максимальное количество одновременно выполняемых заданий для каждого узла. Это значение определяется на основе размера компьютера. Ограничение можно увеличить, чтобы увеличить масштаб параллельных заданий в сложных сценариях, где действия превышают время ожидания, даже когда ЦП, память или сеть мало используются. Эта возможность доступна также для среды выполнения интеграции с одним узлом. |
...
Вы можете переопределить полученное значение по умолчанию на портале Azure. Выберите "Создать" > "Подключения" > "Среды выполнения интеграции" > "Изменить" > "Узлы" > "Изменить число параллельных заданий на каждом узле". Вы также можете использовать команду [обновления PowerShell-Azdatafactoryv2integrationruntimenode.](https://docs.microsoft.com/powershell/module/az.datafactory/update-Azdatafactoryv2integrationruntimenode#examples)
...
Используйте **Get-AzDataFactoryV2IntegrationTimeMetriccmMetric** cmdlet для получения полезной нагрузки JSON, содержащей подробные собственные свойства интеграции, и их значения моментального снимка во время выполнения cmdlet.
...
| Штат | Общее состояние ("Начальное", "Запуск", "Запущено", "Остановка", "Остановлено") среды выполнения интеграции SSIS Azure. |
...
| Запущено | Узлы среды выполнения интеграции SSIS Azure выделены или подготовлены и готовы к развертыванию и выполнению пакетов служб SSIS. |
...
- [Быстрый запуск: создать фабрику данных.](quickstart-create-data-factory-dot-net.md)
