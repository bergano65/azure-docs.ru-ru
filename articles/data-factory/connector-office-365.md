...
Если вы настраиваете `startTime`параметр `endTime` `dateFilterColumn`,, `userScopeFilterUri` и в наборе данных, он по-прежнему поддерживается как есть, хотя в дальнейшем предлагается использовать новую модель в источнике действия.
