...
Пользовательская установка среды выполнения интеграции Azure-SQL Server Integration Services (Azure-SSIS IR) позволяет добавить собственные шаги установки во время настройки или перенастройки Azure-SSIS IR. 

С помощью пользовательской установки можно изменить рабочую конфигурацию или среду по умолчанию, например, запустить дополнительные службы Windows, сохранить учетные данные для доступа к общим папкам или использовать стойкое шифрование или более безопасный сетевой протокол (TLS 1.2). Также можно установить дополнительные компоненты, такие как сборки, драйверы или расширения, на каждом узле Azure-SSIS IR.
...
Вы можете установить как бесплатные нелицензированные компоненты, так и платные лицензированные компоненты с помощью быстрой или стандартной пользовательской установки. Если вы являетесь независимым поставщиком ПО (ISV), ознакомьтесь со статьей [Разработка платных или лицензионных пользовательских компонентов для среды выполнения интеграции Azure-SSIS](how-to-develop-azure-ssis-ir-licensed-components.md).
...
1. Если вы хотите установить или перенастроить Azure-SSIS IR с помощью PowerShell, скачайте и установите [Azure PowerShell](/powershell/azure/install-az-ps). Если выполняется быстрая пользовательская установка, перейдите к шагу 4.
...
1. При установке или повторной настройке Azure-SSIS IR с помощью пользовательского интерфейса фабрики данных можно добавить или удалить пользовательскую установку, установив флажок **Настроить для Azure-SSIS Integration Runtime дополнительные конфигурации системы и (или) установки компонентов** в разделе **Дополнительные параметры** области **Настройка среды выполнения интеграции**. 

   Если вы хотите добавить стандартную пользовательскую установку, введите URI SAS контейнера в поле **Пользовательская настройка контейнера URI SAS**. 
   
   Если вы хотите добавить быструю пользовательскую установку, выберите **Создать**, чтобы открыть область **Добавление быстрой пользовательской установки**, а затем выберите тип в раскрывающемся списке **Тип быстрой пользовательской установки**.
   ...
   * Если выбран тип **Установка лицензированного компонента**, можно выбрать интегрированный компонент от наших партнеров независимых поставщиков программных продуктов в раскрывающемся списке **Название компонента**:
   ...
   Добавленные быстрые пользовательские установки отобразятся в разделе **Дополнительные параметры**. Чтобы удалить их, установите соответствующие флажки, а затем выберите **Удалить**.
...
1. При установке или повторной настройке Azure-SSIS IR с помощью PowerShell можно добавить или удалить пользовательские установки, выполнив командлет `Set-AzDataFactoryV2IntegrationRuntime` перед запуском Azure-SSIS IR.
   ...
   После завершения стандартной пользовательской установки и запуска Azure-SSIS IR можно найти стандартные выходные данные скрипта *main.cmd* и другие журналы выполнения в папке *main.cmd.log* контейнера хранилища.

1. Чтобы просмотреть примеры стандартных пользовательских установок, подключитесь к нашему контейнеру общедоступной предварительной версии с помощью Обозревателя службы хранилища Azure.
   ...
   f. Чтобы попробовать эти примеры пользовательской установки, скопируйте и вставьте содержимое из выбранной папки в свой контейнер.
   
      При установке или повторной настройке Azure-SSIS IR с помощью пользовательского интерфейса фабрики данных установите флажок **Настроить для Azure-SSIS Integration Runtime дополнительные конфигурации системы и (или) установки компонентов** в разделе **Дополнительные параметры** и введите URI SAS вашего контейнера в поле **Пользовательская настройка контейнера URI SAS**.
   
      При установке или перенастройке Azure-SSIS IR с помощью PowerShell выполните командлет `Set-AzDataFactoryV2IntegrationRuntime` с URI SAS контейнера в качестве значения параметра `SetupScriptContainerSasUri`.

## <a name="next-steps"></a>Дальнейшие действия

- [Установка Enterprise Edition среды выполнения интеграции Azure-SSIS](how-to-configure-azure-ssis-ir-enterprise-edition.md)
