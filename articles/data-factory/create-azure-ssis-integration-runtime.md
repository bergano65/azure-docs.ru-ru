...
2. На странице **Общие параметры** в разделе **Настройка среды выполнения интеграции** выполните следующие действия:
   ...
   ![Общие параметры](./media/tutorial-create-azure-ssis-runtime-portal/general-settings.png)
    ...
    г) В поле **размер узла**выберите размер узла в кластере среды выполнения интеграции. Отображаются только поддерживаемые размеры узла. Если вы планируете использовать пакеты с высокими требованиями к количеству компьютеров или объему памяти, выберите большой размер узла (увеличение масштаба).
    ...
    е) Для **выпуска/лицензии**выберите SQL Server выпуск для среды выполнения интеграции: Standard или Enterprise. Если в среде выполнения интеграции вы будете использовать расширенные функции, выберите SQL Server Enterprise.

    ж. Для параметра **сохранить деньги**выберите параметр преимущество гибридного использования Azure для среды выполнения интеграции: **Да** или **нет**. Если для экономии вы намерены использовать гибридный режим с собственной лицензией SQL Server по программе Software Assurance, выберите **Да**.

    h. Щелкните **Далее**.

3. На странице **Параметры SQL** выполните следующие шаги.

   ![Раздел "SQL Settings" (Параметры SQL)](./media/tutorial-create-azure-ssis-runtime-portal/sql-settings.png)

   а) Установите флажок в поле **Создать каталог SSIS**, чтобы выбрать модель развертывания для запуска пакетов в Azure-SSIS IR. Вам нужно выбрать модель развертывания проектов, при которой пакеты развертываются в SSISDB, размещенной на сервере базы данных, или модель развертывания пакетов, при которой пакеты развертываются в файловых системах, на общих файловых ресурсах или в службе "Файлы Azure". 
   ...
   б) В раскрывающемся списке **Подписка** выберите подписку Azure, которая соответствует серверу базы данных узла SSISDB. 
...
5. Установите флажок **Выбрать виртуальную среду для присоединения Azure-SSIS Integration Runtime и создания определенных сетевых ресурсов в Фабрике данных Azure**, чтобы присоединить среду выполнения интеграции к виртуальной сети. 
   ...
   в) В поле **тип**выберите тип виртуальной сети: классический или Azure Resource Manager. Рекомендуется выбрать виртуальную сеть Azure Resource Manager, так как классические виртуальные сети скоро будут считаться устаревшими.
...
6. Установите флажок **Настроить автономную среду выполнения интеграции в качестве прокси-сервера для Azure-SSIS Integration Runtime**, если вы хотите использовать автономную среду выполнения интеграции в качестве прокси-сервера для Azure-SSIS IR. См. подробнее о [настройке автономной среды выполнения интеграции в качестве прокси-сервера](https://docs.microsoft.com/azure/data-factory/self-hosted-integration-runtime-proxy-ssis). 
   ...
   б) Для **связанной службы промежуточного хранилища**выберите существующую связанную службу хранилища BLOB-объектов Azure. Или создайте новый для промежуточного хранения.

   в) В качестве **промежуточного пути**укажите контейнер больших двоичных объектов в выбранной учетной записи хранилища BLOB-объектов Azure. Или оставьте его пустым, чтобы использовать значение по умолчанию для промежуточного хранения.

7. Выберите **Проверка виртуальной сети** > **Далее**. 

8. На странице **Сводка** проверьте все параметры подготовки, закладку рекомендуемых ссылок на документацию и щелкните **Готово**, чтобы создать среду выполнения интеграции.
...
- [Управление средой выполнения интеграции Azure SSIS](manage-azure-ssis-integration-runtime.md). В этой статье показано, как приступить к работе, запускать или удалять Azure-SSIS IR. Здесь также показано, как масштабировать Azure-SSIS IR, добавляя дополнительные узлы.
