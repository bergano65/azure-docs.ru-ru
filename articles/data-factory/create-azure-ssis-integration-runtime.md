...
- **Сервер базы данных SQL Azure или управляемый экземпляр SQL (необязательно)**. Если у вас еще нет сервера базы данных или управляемого экземпляра, создайте его в портал Azure перед началом работы. Фабрика данных, в свою очередь, создаст экземпляр SSISDB на этом сервере базы данных. 
  ...
  - Убедитесь, что для сервера базы данных включен параметр **Разрешить доступ к службам Azure**. Этот параметр неприменим при использовании сервера базы данных SQL Azure с правилами брандмауэра IP-адресов, конечными точками службы виртуальной сети или управляемым экземпляром SQL с частной конечной точкой для размещения SSISDB. Дополнительные сведения см. в разделе [Создание правила брандмауэра](../sql-database/sql-database-security-tutorial.md#create-firewall-rules). Сведения о включении этого параметра с помощью PowerShell см. в статье [New-AzSqlServerFirewallRule](/powershell/module/az.sql/new-azsqlserverfirewallrule).
...
На странице **Общие параметры** на панели **Настройка среды выполнения интеграции** выполните описанные ниже действия.
   ...
   > [!NOTE]
   > Если требуется [изоляция вычислений](https://docs.microsoft.com/azure/azure-government/azure-secure-isolation-guidance#compute-isolation), выберите размер узла **Standard_E64i_v3** . Этот размер узла представляет изолированные виртуальные машины, использующие их весь физический узел, и обеспечивает необходимый уровень изоляции, необходимый для определенных рабочих нагрузок, например для отдела США от рабочих нагрузок уровня защиты 5 (IL5).
...
На панели **Добавление хранилища данных** выполните описанные ниже действия.
   ...
   1. Для параметра **Связанная служба хранилища пакетов** выберите имеющуюся связанную службу, в которой хранятся сведения о доступе к файловой системе, службе "Файлы Azure" и Управляемому экземпляру Azure SQL, где развернуты ваши пакеты, или создайте новую, выбрав **Создать**. На панели **Новая связанная служба** выполните описанные ниже действия.
      ...
      > [!NOTE]
      > Для доступа к службе "файлы Azure" можно использовать **хранилище файлов Azure** или связанные службы **файловой системы** . Если вы используете связанную службу **хранилища файлов Azure** , Azure-SSIS IR хранилище пакетов в настоящее время поддерживает только **базовый** метод проверки подлинности (без **ключа учетной записи** или **URI SAS**). Чтобы использовать **обычную** проверку подлинности для связанной службы **хранилища файлов Azure** , можно добавить `?feature.upgradeAzureFileStorage=false` к URL-адресу портала ADF в браузере. Кроме того, можно использовать связанную службу **файловой системы** для доступа к файлам Azure. 
