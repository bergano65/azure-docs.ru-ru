---
title: Обработка больших наборов данных с помощью фабрики данных и пакетной службы
...
Дополнительно, чтобы узнать больше о пакете, [см.](https://docs.microsoft.com/azure/batch/)
...
* [Основные сведения о Фабрике данных](data-factory-introduction.md)
...
[![Масштабная диаграмма обработки данных](./media/data-factory-data-processing-using-batch/image1.png)](https://go.microsoft.com/fwlink/?LinkId=717686)
...
* **Создайте пользовательскую активность .NET в конвейере Data Factory.** Действие является пользовательским кодом, который выполняется в пуле пакетной службы.

* **Храните большие объемы входных данных в качестве капли в хранилище Azure.** Данные при этом разделяются на логические срезы (обычно по времени).
...
Учетная запись хранения используется в этом руководстве для хранения данных. Если у вас нет учетной записи хранения, [создайте ее](../../storage/common/storage-account-create.md). В примере решения используется хранилище BLOB-объектов.

#### <a name="azure-batch-account"></a>Учетная запись пакетной службы Azure
Создайте учетную запись пакетной службы с помощью [портала Azure](https://portal.azure.com/). Дополнительные сведения см. в [этой статье](../../batch/batch-account-create-portal.md). Запишите ключ и имя учетной записи пакетной службы. Вы также можете использовать [cmdlet New-AzBatchAccount](https://docs.microsoft.com/powershell/module/az.batch/new-azbatchaccount) для создания учетной записи Пакета. Инструкции по использованию этого командлета см. в статье [Управление ресурсами пакетной службы с помощью командлетов PowerShell](../../batch/batch-powershell-cmdlets-get-started.md).
...
1. В колонке **Pools** (Пулы) нажмите кнопку **Добавить** на панели инструментов, чтобы добавить пул.
   ...
   а. Введите идентификатор для пула (**идентификатор пула**). Запишите идентификатор пула. Он понадобится при создании решения фабрики данных.

   b. Укажите **Windows Server 2012 R2** для настройки **семейства операционной системы.**
...
Для проверки и изменения данных в проектах службы хранилища используется [Обозреватель службы хранилища Azure 6](https://azurestorageexplorer.codeplex.com/) или [CloudXplorer](https://clumsyleaf.com/products/cloudxplorer) (от ClumsyLeaf Software). Кроме того, с помощью этих средств можно проверять и изменять данные в журналах облачных приложений.

1. Создайте контейнер под названием **mycontainer** с частным доступом (без анонимного доступа).
...
* Метод принимает четыре параметра:
  ...
  * **деятельности**. этот параметр представляет собой текущую вычислительную сущность. В этом примере — пакетную службу.
  * **регистратор**. средство ведения журнала, позволяющее записывать комментарии отладки, которые отображаются в виде пользовательского журнала для конвейера.
...
1. Создайте проект библиотеки классов .NET в Visual Studio.
   ...
   а. Запустите Visual Studio 2012, 2013 или 2015.

   b. Выберите **файл** > **Новый** > **проект**.
   ...
   ж. Чтобы создать проект, щелкните **ОК**.

1. Выберите **Инструменты** > **NuGet пакет менеджер** > **менеджер консоли**.
...
1. Код для работы с набором больших двоичных объектов логически находится в цикле do-while. В методе **Execute** цикл do-while передает список больших двоичных объектов в метод **Calculate**. Этот метод возвращает строковую переменную с именем **output** , которая является результатом итерации всех больших двоичных объектов в сегменте.
   ...
   Он возвращает количество случаев возникновения поискового термина "Microsoft" в каплей, передаваемых методу **расчета.**
...
1. После входа на [портал Azure](https://portal.azure.com/) сделайте следующее:
   ...
   а. В меню слева выберите **Создать**.
...
1. В сценарии JSON выполните следующее:
   ...
   а. Замените **account name** именем учетной записи пакетной службы.
   ...
   d. Введите URI пакета для свойства JSON **batchUri** .
      ...
      > [!IMPORTANT]
      > URL-адрес из лезвия **счета** \<пакета\>находится в следующем формате: имя учетной записи . \<регион\>.batch.azure.com. В свойстве **batchUri** в скрипте JSON необходимо удалить a88"accountname."** из URL-адреса. Например, `"batchUri": "https://eastus.batch.azure.com"`.
      >
      >
...
1. Замените скрипт JSON в правой области на следующий фрагмент кода JSON:
    ...
    | **Ломтик** | **Время начала**          | **Входная папка**  |
    |-----------|-------------------------|-------------------|
...
1. Замените скрипт JSON в правой области на следующий фрагмент кода JSON:
    ...
    | **Ломтик** | **Время начала**          | **Выходной файл**       |
    |-----------|-------------------------|-----------------------|
...
1. В папке вывода удалите выходной файл **2015-11-16-01.txt**.
...
1. В примере решения метод **Execute** вызывает метод **Calculate**, который обрабатывает входной срез данных для создания выходного среза данных. Можно написать свой собственный метод для обработки входных данных и заменить вызов метода **вычисления** в методе **Выполнения** вызовом к вашему методу.
