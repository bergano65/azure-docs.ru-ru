...
Дополнительные сведения о пакете см. в [документации по пакетной](https://docs.microsoft.com/azure/batch/)службе.
...
[![Диаграмма обработки крупномасштабных данных](./media/data-factory-data-processing-using-batch/image1.png)](https://go.microsoft.com/fwlink/?LinkId=717686)
...
* **Создайте настраиваемое действие .NET в конвейере фабрики данных.** Действие является пользовательским кодом, который выполняется в пуле пакетной службы.

* **Храните большие объемы входных данных в виде больших двоичных объектов в службе хранилища Azure.** Данные при этом разделяются на логические срезы (обычно по времени).
...
Создайте учетную запись пакетной службы с помощью [портала Azure](https://portal.azure.com/). Дополнительные сведения см. в [этой статье](../../batch/batch-account-create-portal.md). Запишите ключ и имя учетной записи пакетной службы. Для создания учетной записи пакетной службы можно также использовать командлет [New-азбатчаккаунт](https://docs.microsoft.com/powershell/module/az.batch/new-azbatchaccount) . Инструкции по использованию этого командлета см. в статье [Управление ресурсами пакетной службы с помощью командлетов PowerShell](../../batch/batch-powershell-cmdlets-get-started.md).
...
1. В колонке **Pools** (Пулы) нажмите кнопку **Добавить** на панели инструментов, чтобы добавить пул.
   ...
   b. Укажите **Windows Server 2012 R2** для параметра **семейства операционных систем** .
...
1. Создайте контейнер с именем **MyContainer** с закрытым доступом (без анонимного доступа).
...
* Метод принимает четыре параметра:
  ...
  * **действие**. этот параметр представляет собой текущую вычислительную сущность. В этом примере — пакетную службу.
  * **средство ведения журнала**. средство ведения журнала, позволяющее записывать комментарии отладки, которые отображаются в виде пользовательского журнала для конвейера.
...
1. Код для работы с набором больших двоичных объектов логически находится в цикле do-while. В методе **Execute** цикл do-while передает список больших двоичных объектов в метод **Calculate**. Этот метод возвращает строковую переменную с именем **output** , которая является результатом итерации всех больших двоичных объектов в сегменте.
   ...
   Он возвращает количество вхождений условия поиска Microsoft в большой двоичный объект, переданный методу **Calculate** .
...
1. Замените скрипт JSON в правой области на следующий фрагмент кода JSON:
   ...
   * Аргументу **AssemblyName** присваивается имя библиотеки DLL **MyDotNetActivity. dll**.
...
1. В выходной папке удалите выходной файл **2015-11-16 01. txt**.
...
1. В примере решения метод **Execute** вызывает метод **Calculate**, который обрабатывает входной срез данных для создания выходного среза данных. Вы можете написать собственный метод для обработки входных данных и заменить вызов метода **Calculate** в методе **EXECUTE** вызовом метода.
