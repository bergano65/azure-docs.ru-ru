...
Дополнительные сведения о пакете см. в [документации](https://docs.microsoft.com/azure/batch/)по пакетной службе.
...
1. В колонке **Pools** (Пулы) нажмите кнопку **Добавить** на панели инструментов, чтобы добавить пул.
   ...
   1\. Введите идентификатор для пула (**идентификатор пула**). Запишите идентификатор пула. Он понадобится при создании решения фабрики данных.

   2\. В качестве параметра **семейства операционных систем** укажите **Windows Server 2012 R2**.

   В. Выберите **Ценовая категория для узлов**.

   Г. Для параметра **Выделенный целевой объект** укажите значение **2**.
...
1. Создайте проект библиотеки классов .NET в Visual Studio.
   ...
   В. Разверните раздел **Шаблоны** и выберите **Visual C#\#** . В этом руководстве используется язык C\#, но для создания настраиваемого действия вы можете использовать любой язык .NET.

   Г. Выберите тип **Библиотека классов** из списка типов проектов справа.
...
1. После входа на [портал Azure](https://portal.azure.com/) сделайте следующее:
   ...
   2\. В колонке **Создание** выберите **Данные+аналитика**.

   В. Выберите **Фабрика данных** в колонке **Аналитика данных**.
...
1. Чтобы развернуть связанную службу, выберите **Развернуть** на панели команд.
   ...
   ![Развернуть](./media/data-factory-data-processing-using-batch/image8.png)
...
1. В сценарии JSON выполните следующее:
   ...
   1\. Замените **account name** именем учетной записи пакетной службы.

   2\. Замените **access key** ключом доступа к учетной записи пакетной службы.

   В. Введите идентификатор пула для свойства **poolName**. Для этого свойства можно задать имя или идентификатор пула.

   Г. Введите URI пакета для свойства JSON **batchUri** .
