---
title: Копирование данных в/из хранилища данных Azure S'L
...
2. Создание **взаимосвязанных служб** для связи входных и выходных данных с фабрикой данных. Например, при копировании данных из хранилища BLOB-объектов Azure в хранилище данных Azure SQL создайте две связанные службы, чтобы связать учетную запись хранения Azure и хранилище данных Azure SQL с фабрикой данных. Сведения о свойствах связанной службы, относящихся к хранилищу данных Azure SQL, см. в разделе [Свойства связанной службы](#linked-service-properties). 
3. Создайте **наборы данных**, которые представляют входные и выходные данные для операции копирования. В примере, упомянутом на последнем этапе, создайте набор данных, чтобы указать контейнер BLOB-объектов и папку, содержащую входные данные. И создайте другой набор данных, чтобы указать таблицу в хранилище данных Azure SQL, содержащую данные, скопированные из хранилища BLOB-объектов. Сведения о свойствах набора данных, относящихся к хранилищу данных Azure SQL, см. в разделе [Свойства набора данных](#dataset-properties).
...
| connectionString |Укажите сведения, необходимые для подключения к экземпляру хранилища данных SQL Azure, для свойства connectionString. Поддерживается только обычная проверка подлинности. |Да |
...
Раздел typeProperties во всех типах наборов данных разный. В нем содержатся сведения о расположении данных в хранилище данных. Раздел **typeProperties** для набора данных типа **AzureSqlDWTable** имеет следующие свойства:
...
```
**Определение сохраненной процедуры:**
...
| rejectValue |Указывает количество или процент строк, которые могут быть отклонены, прежде чем запрос завершится с ошибкой. <br/><br/>Подробнее о вариантах отклонения PolyBase читайте в разделе **Аргументы** в разделе [CREATE EXTERNAL TABLE (Transact-S'L).](https://msdn.microsoft.com/library/dn935021.aspx) |0 (по умолчанию), 1, 2, ... |нет |
...
Использование **[PolyBase](https://docs.microsoft.com/sql/relational-databases/polybase/polybase-guide)** является эффективным способом загрузки большого объема данных в хранилище данных Azure s'L с высокой пропускной стоимостью. Используя PolyBase вместо стандартного механизма BULKINSERT, можно значительно увеличить пропускную способность. Подробное сравнение см. в разделе [Базовые показатели производительности](data-factory-copy-activity-performance.md#performance-reference). Пошаговое руководство и пример использования см. в статье [Загрузка 1 ТБ в хранилище данных SQL Azure с помощью фабрики данных Azure [мастер копирования] менее чем за 15 минут](data-factory-load-sql-data-warehouse.md).
...
1. **Служба, связанная с исходным** сообщением, имеет тип: **AzureStorage** или **AzureDataLakeStore с основной аутентификацией службы.**
2. Тип **входного набора данных** — **AzureBlob** или **AzureDataLakeStore**, тип формата в свойствах `type` — **OrcFormat**, **ParquetFormat** или **TextFormat** со следующими конфигурациями.
   ...
   5. `compression`может быть **никакого сжатия,** **ГЗип**, или **deflate**.
...
Если исходные данные не соответствуют критериям, введенным в предыдущем разделе, можно включить копирование данных с помощью промежуточной постановки Azure Blob Storage (не может быть Premium Storage). В таком случае в фабрике данных Azure данные будут преобразованы и приведены в соответствие с требованиями к формату PolyBase, переданы в хранилище данных SQL с помощью PolyBase и, наконец, временные данные будут очищены из хранилища BLOB-объектов. Подробные сведения о том, как работает копирование данных через промежуточное хранилище BLOB-объектов Azure, см. в разделе [Промежуточное копирование](data-factory-copy-activity-performance.md#staged-copy).

> [!NOTE]
> При копировании данных из хранилища данных в помещениях в хранилище данных Azure S'L с использованием PolyBase и постановки, если версия шлюза управления данными ниже 2,4, JRE (Java Runtime Environment) требуется на вашем шлюзе, который используется для преобразования исходных данных в надлежащий формат. Чтобы избежать такой зависимости, мы рекомендуем обновить шлюз до последней версии.
>
...
В следующих разделах приведены практические рекомендации дополнительно к указанным в разделе [Рекомендации по использованию хранилища данных SQL Azure](../../synapse-analytics/sql-data-warehouse/sql-data-warehouse-best-practices.md).

### <a name="required-database-permission"></a>Необходимые разрешения базы данных
Чтобы отправить данные в хранилище данных SQL с помощью PolyBase, пользователь, используемый для этого процесса, должен иметь [разрешение CONTROL](https://msdn.microsoft.com/library/ms191291.aspx) в целевой базе данных. Это разрешение можно получить, добавив этого пользователя как участника роли db_owner. Дополнительные сведения см. в [этом разделе](../../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-manage-security.md#authorization).
...
Отображение такое же, как и [картирование типа данных сервера s'L для ADO.NET.](https://msdn.microsoft.com/library/cc716729.aspx)
...
Пример предполагает, что вы создали таблицу "MyTable" в хранилище данных Azure s'L и содержит столбец под названием "timestampcolumn" для данных временных рядов.
...
Данные берутся из нового BLOB-объекта каждый час (frequency: hour, interval: 1). Путь к папке с BLOB-объектом и имя файла вычисляются динамически на основе времени начала обрабатываемого среза. В пути к папке используется год, месяц и день начала, а в имени файла — час начала. "внешний": "истинная" настройка информирует службу Data Factory о том, что эта таблица является внешней для фабрики данных и не производится в связи с деятельностью на фабрике данных.
...
Образец копирует данные в таблице под названием "MyTable" в хранилище данных Azure S'L. Таблицу в хранилище данных SQL Azure необходимо создать с таким количеством столбцов, которое должно быть в CSV-файле большого двоичного объекта. Новые строки добавляются в таблицу каждый час.
