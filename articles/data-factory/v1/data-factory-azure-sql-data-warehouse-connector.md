---
title: Копирование данных в хранилище данных SQL Azure и из него
...
2. Создайте **связанные службы** , чтобы связать входные и выходные данные хранилища с фабрикой данных. Например, при копировании данных из хранилища BLOB-объектов Azure в хранилище данных Azure SQL создайте две связанные службы, чтобы связать учетную запись хранения Azure и хранилище данных Azure SQL с фабрикой данных. Сведения о свойствах связанной службы, относящихся к хранилищу данных Azure SQL, см. в разделе [Свойства связанной службы](#linked-service-properties). 
...
2. Тип **входного набора данных** — **AzureBlob** или **AzureDataLakeStore**, тип формата в свойствах `type` — **OrcFormat**, **ParquetFormat** или **TextFormat** со следующими конфигурациями.
   ...
   5. `compression`не может иметь **Сжатие**, **gzip**или **Deflate**.
...
Сопоставление аналогично [SQL Serverному сопоставлению типов данных для ADO.NET](https://msdn.microsoft.com/library/cc716729.aspx).
...
В этом примере предполагается, что вы создали таблицу MyTable в хранилище данных SQL Azure и она содержит столбец с именем «timestampcolumn» для данных временных рядов.
...
В примере данные копируются в таблицу с именем MyTable в хранилище данных SQL Azure. Таблицу в хранилище данных SQL Azure необходимо создать с таким количеством столбцов, которое должно быть в CSV-файле большого двоичного объекта. Новые строки добавляются в таблицу каждый час.
