...
3. В разделе `skipHeaderLineCount`BlobSource**или**AzureDataLakeStore**для действия копирования в конвейере отсутствует параметр**.
4. В разделе `sliceIdentifierColumnName`SqlDWSink**для действия копирования в конвейере отсутствует параметр**. PolyBase гарантирует, что за один цикл выполнения либо все данные будут обновлены, либо ничего не будет обновлено. Чтобы обеспечить **воспроизводимость**, используйте свойство `sqlWriterCleanupScript`).
...
> [!NOTE]
> Если вы копируете данные из локального хранилища данных в хранилище данных SQL Azure с помощью Polybase и промежуточного хранения, если версия шлюза Управление данными ниже 2,4, то на компьютере шлюза, который используется для преобразования источника, требуется наличие JRE (среда выполнения Java). данные в правильном формате. Чтобы избежать такой зависимости, мы рекомендуем обновить шлюз до последней версии.
>
