...
7. На странице **File format settings** (Параметры формата файла) отображаются разделители и схема, которые мастер определяет автоматически при анализе файла.
    1. Проверьте следующие параметры:  
        1\. Убедитесь, что для **формата файла** задано значение **Текстовый формат**. Все поддерживаемые форматы находятся в раскрывающемся списке. Пример: JSON, Avro, ORC, Parquet.
       ...
       В. Убедитесь, что в качестве **разделителя строк** используется `Carriage Return + Line feed (\r\n)`. Другие разделители строк, поддерживаемые фабрикой данных, находятся в раскрывающемся списке. Вы также можете указать пользовательский разделитель.
       Г. Убедитесь, что для **пропуска указанного количества строк** задано значение **0**. Если требуется пропустить несколько строк в верхней части файла, введите их количество здесь.
