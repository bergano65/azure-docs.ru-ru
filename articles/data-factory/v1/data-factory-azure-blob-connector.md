---
title: Копирование данных в хранилище BLOB-объектов Azure и из него
...
> [!IMPORTANT]
> Действие копирования поддерживает копирование данных как в учетные записи хранения Azure общего назначения и в хранилище горячих и холодных BLOB-объектов, так и из них. Действие поддерживает **чтение из блоков, добавления или страничных BLOB-объектов**, но поддерживает **запись только для блочных больших двоичных объектов**. Хранилище Azure класса Premium нельзя использовать в качестве приемника, так как оно поддерживается страничными BLOB-объектами.
>
...
2. Создайте **наборы данных**, которые представляют входные и выходные данные для операции копирования. В примере, упомянутом на последнем этапе, создайте набор данных, чтобы указать контейнер BLOB-объектов и папку, содержащую входные данные. Кроме того, вы создаете другой набор данных, чтобы указать таблицу SQL в базе данных SQL Azure, которая содержит данные, скопированные из хранилища BLOB-объектов. Сведения о свойствах набора данных, относящихся к хранилищу BLOB-объектов Azure, см. в разделе [Свойства набора данных](#dataset-properties).
...
| Свойство | Описание | Допустимые значения | Обязательно |
| --- | --- | --- | --- |
| copyBehavior |Это свойство определяет поведение функции копирования, когда в качестве источника используется BlobSource или FileSystem. |<b>PreserveHierarchy</b>: сохраняет иерархию файлов в целевой папке. Относительный путь исходного файла в исходной папке идентичен относительному пути целевого файла в целевой папке.<br/><br/><b>FlattenHierarchy</b>: все файлы из исходной папки находятся на первом уровне целевой папки. Целевые файлы имеют автоматически сформированное имя. <br/><br/><b>MergeFiles</b>: объединяет все файлы из исходной папки в один файл. Если указано имя файла или большого двоичного объекта, именем объединенного файла будет указанное имя; в противном случае имя файла будет автоматически сформировано. |Нет |
...
1. Создайте **учетную запись хранения Azure** общего назначения, если у вас ее нет. В этом пошаговом руководстве хранилище больших объектов используется как **исходное** и **целевое** хранилище данных. Если у вас нет учетной записи хранения Azure, создайте ее, как описано в статье [Создание учетной записи хранения](../../storage/common/storage-account-create.md) .
...
3. После завершения создания отобразится колонка **фабрика данных** , как показано на следующем рисунке:  ![ Домашняя страница фабрики данных.](./media/data-factory-azure-blob-connector/data-factory-home-page.png)
...
5. Вот что нужно сделать на странице **Choose the input file or folder** (Выбор входного файла или папки).
    ...
    2. Выберите **input** и нажмите кнопку **Выбрать**. В этом пошаговом руководстве вы выберете папку input. Кроме того, можно выбрать файл emp.txt в папке.
        ![Средство копирования — выберите входной файл или папку 1](./media/data-factory-azure-blob-connector/copy-tool-choose-input-file-or-folder.png)
6. Вот что нужно сделать на странице **Choose the input file or folder** (Выбор входного файла или папки).
    ...
    5. Нажмите кнопку **Далее**.
    ![Средство копирования — выбор входного файла или папки 2](./media/data-factory-azure-blob-connector/chose-input-file-folder.png)
7. На странице **File format settings** (Параметры формата файла) отображаются разделители и схема, которые мастер определяет автоматически при анализе файла.
    1. Проверьте следующие параметры:  
        а. Убедитесь, что для **формата файла** задано значение **Текстовый формат**. Все поддерживаемые форматы находятся в раскрывающемся списке. Например, JSON, Avro, ORC, Parquet.
...
10. Вот что нужно сделать на странице **Choose the output file or folder** (Выбор выходного файла или папки).  
    1. В поле **Путь к папке** укажите **adfblobconnector/output/{год}/{месяц}/{день}**. Нажмите клавишу **TAB**.
    1. В качестве **года** выберите **yyyy**.
    1. Убедитесь, что для **месяца** задано значение **ММ**.
    1. Убедитесь, что для **дня** задано значение **Д**.
    1. Убедитесь, что для **типа сжатия** задано значение **нет**.
    1. Убедитесь, что для параметра **Copy behavior** (Поведение копирования) задано значение **Merge files** (Объединение файлов). Если выходной файл с тем же именем уже существует, новое содержимое добавляется в тот же файл в конце.
...
2. На отдельной вкладке вы увидите **приложение мониторинг и управление** .  ![Мониторинг приложения и управление им](media/data-factory-azure-blob-connector/monitor-manage-app.png)
...
Данные берутся из нового BLOB-объекта каждый час (frequency: hour, interval: 1). Путь к папке с BLOB-объектом и имя файла вычисляются динамически на основе времени начала обрабатываемого среза. В пути к папке используется год, месяц и день начала, а в имени файла — час начала. параметр "External": "true" информирует фабрику данных о том, что таблица является внешней по отношению к фабрике данных и не создается действием в фабрике данных.
...
В примере данные копируются в таблицу с именем MyTable в базе данных SQL Azure. Создайте таблицу в базе данных SQL с тем же количеством столбцов, которое будет содержать CSV-файл большого двоичного объекта. Новые строки добавляются в таблицу каждый час.
...
Установка значения "External": "true" информирует службу фабрики данных о том, что таблица является внешней по отношению к фабрике данных и не создается действием в фабрике данных.
