...
description: Как копировать данные BLOB-объектов в фабрике данных Azure. Воспользуйтесь нашим примером, как копировать данные в базу данных SQL Azure и хранилище BLOB-объектов Azure и обратно.
...
2. Создайте **наборы данных**, которые представляют входные и выходные данные для операции копирования. В примере, упомянутом в последнем шаге, создайте набор данных, чтобы указать контейнер BLOB-объектов и папку, содержащую входные данные. Также создайте другой набор данных, чтобы указать таблицу SQL в базе данных SQL Azure, содержащую данные, скопированные из хранилища BLOB-объектов. Сведения о свойствах набора данных, относящихся к хранилищу BLOB-объектов Azure, см. в разделе [Свойства набора данных](#dataset-properties).
...
## <a name="walkthrough-use-copy-wizard-to-copy-data-tofrom-blob-storage"></a>Пошаговое руководство Копирование данных из хранилища BLOB-объектов и в него с помощью мастера копирования
...
7. На странице **File format settings** (Параметры формата файла) отображаются разделители и схема, которые мастер определяет автоматически при анализе файла.
    1. Проверьте следующие параметры:  
       ...
       2. Убедитесь, что в качестве **разделителя столбцов** используется `Comma (,)`. Другие разделители столбцов, поддерживаемые фабрикой данных, находятся в раскрывающемся списке. Вы также можете указать пользовательский разделитель.
...
10. Вот что нужно сделать на странице **Choose the output file or folder** (Выбор выходного файла или папки).  
    ...
    1. Убедитесь, что для параметра **Compression type** (Тип сжатия) задано значение **Нет**.
...
```
### <a name="json-example-copy-data-from-azure-sql-to-azure-blob"></a>Пример JSON. Копирования данных из SQL Azure в BLOB-объект Azure
В примере ниже используется следующее:
