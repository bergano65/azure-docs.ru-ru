---
title: Копирование данных в/из хранилища Azure Blob
...
> [!IMPORTANT]
> Действие копирования поддерживает копирование данных как в учетные записи хранения Azure общего назначения и в хранилище горячих и холодных BLOB-объектов, так и из них. Действие поддерживает **чтение из блока, приложения или страницы капли**, но поддерживает **написание только блокировать капли.** Хранилище Azure класса Premium нельзя использовать в качестве приемника, так как оно поддерживается страничными BLOB-объектами.
>
...
1. Создайте **учетную запись хранения Azure** общего назначения, если у вас ее нет. В этом пошаговом руководстве хранилище больших объектов используется как **исходное** и **целевое** хранилище данных. Если у вас нет учетной записи хранения Azure, см. статью [учетной записи хранилища](../../storage/common/storage-account-create.md) для шагов по ее созданию.
...
3. После завершения создания вы увидите лезвие **фабрики данных,** указанное на следующем изображении: ![главная страница фабрики данных](./media/data-factory-azure-blob-connector/data-factory-home-page.png)
...
10. Вот что нужно сделать на странице **Choose the output file or folder** (Выбор выходного файла или папки).  
    ...
    1. Подтвердите, что **тип сжатия** установлен на **none.**
...
2. Приложение Monitor **and Manage** следует видеть в отдельной вкладке.  ![Приложение Monitor and Manage](media/data-factory-azure-blob-connector/monitor-manage-app.png)
...
1. Связанная служба типа [AzureSqlDatabase](data-factory-azure-sql-connector.md#linked-service-properties).
2. Связанная служба типа [AzureStorage](#linked-service-properties).
3. Набор входных [данных](data-factory-create-datasets.md) типа [AzureSqltable](data-factory-azure-sql-connector.md#dataset-properties).
