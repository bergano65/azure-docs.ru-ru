---
title: Перемещение данных на сервер и с сервера S'L
...
В этой статье рассказывается, как с помощью действия копирования в фабрике данных Azure перемещать данные в локальную базу данных SQL Server и обратно. Он основывается на статье [«Деятельность движения данных»,](data-factory-data-movement-activities.md) в которой представлен общий обзор движения данных с активностью копирования.
...
2. Создание **взаимосвязанных служб** для связи входных и выходных данных с фабрикой данных. Например, при копировании данных из базы данных SQL Server в хранилище BLOB-объектов Azure создайте две связанные службы, чтобы связать базу данных SQL Server и учетную запись хранения Azure с фабрикой данных. Сведения о свойствах связанной службы, относящихся к базе данных SQL Server, см. в разделе [Свойства связанной службы](#linked-service-properties).
...
Вы можете шифровать учетные данные с помощью cmdlet **New-AzDataFactoryEncryptValue** и использовать их в строке соединения, как показано в следующем примере **(свойство EncryptedCredential):**
...
5. [Конвейер](data-factory-create-pipelines.md) с активностью Copy, используюм [BlobSource](data-factory-azure-blob-connector.md#copy-activity-properties) и SqlSink.
...
```
**Набор вхотливых данных Azure Blob**
...
4. Переключитесь на вкладку **IP-адресов.** Прокрутите вниз, чтобы просмотреть раздел **IPAll.** Обратите внимание на **порт TCP**(по умолчанию **1433).**
...
Как указано в статье о [движении данных,](data-factory-data-movement-activities.md) активность Copy выполняет автоматические преобразования типов из типов источников в типы раковин со следующим 2-ступенчатым подходом:
