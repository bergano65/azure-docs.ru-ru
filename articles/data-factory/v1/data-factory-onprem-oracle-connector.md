...
| gatewayName | Имя шлюза, который используется для подключения к локальному серверу Oracle. |Yes |

**Пример. Использование драйвера Microsoft**
...
Данные записываются в новый большой двоичный объект каждый час (**frequency**: **hour**, **interval**: **1**). Путь к папке с большим двоичным объектом и имя файла вычисляются динамически на основе времени начала обработки среза. В пути к папке используется год, месяц, день и час времени запуска.
...
Данные берутся из нового большого двоичного объекта каждый час (**frequency**: **hour**, **interval**: **1**). Путь к папке с большим двоичным объектом и имя файла вычисляются динамически на основе времени начала обработки среза. В пути к папке используется год, месяц и день начала. В имени файла используется часть времени начала, обозначающая час. Если для параметра **external** задать значение **true**, то Фабрика данных воспримет эту таблицу как внешнюю, которая создана не в результате какого-либо действия в этой службе.
