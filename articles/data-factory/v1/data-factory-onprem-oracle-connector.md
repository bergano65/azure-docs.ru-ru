---
title: Копирование данных в Oracle или от oracle с помощью фабрики данных
...
Вы также можете использовать один из следующих инструментов для создания конвейера: **Visual Studio**, **Azure PowerShell**, **шаблон менеджера ресурсов Azure,** **API .NET**или **REST API.** Пошаговые инструкции по созданию конвейера с действием копирования см. в [руководстве по действию копирования](data-factory-copy-data-from-azure-blob-storage-to-sql-database.md).
...
2. Создание **взаимосвязанных служб** для связи входных и выходных данных с фабрикой данных. Например, при копировании данных из базы данных Oracle в хранилище BLOB-объектов Azure создайте две связанные службы, чтобы связать базу данных Oracle и учетную запись хранения Azure с фабрикой данных. Сведения о свойствах связанной службы, относящихся к Oracle, см. в разделе [свойств связанной службы](#linked-service-properties).
...
| --- | --- | --- |
| type |Свойство **типа** должно быть установлено **на OnPremisesOracle.** |Да |
...
Раздел **typeProperties** отличается для каждого типа набора данных и предоставляет информацию о местоположении данных в хранилище данных. Раздел **typeProperties** для набора данных типа **OracleTable** имеет следующие свойства:
...
Данные записываются в новую каплю каждый час **(частота**: **час,** **интервал**: **1**). Путь к папке с большим двоичным объектом и имя файла вычисляются динамически на основе времени начала обработки среза. В пути к папке используется год, месяц, день и час времени запуска.
...
Данные собираются из новой капли каждый час **(частота**: **час,** **интервал**: **1**). Путь к папке с большим двоичным объектом и имя файла вычисляются динамически на основе времени начала обработки среза. В пути к папке используется год, месяц и день начала. В имени файла используется часть времени начала, обозначающая час. Если для параметра **external** задать значение **true**, то Фабрика данных воспримет эту таблицу как внешнюю, которая создана не в результате какого-либо действия в этой службе.
...
* Если сообщение об ошибке появляется даже после установки поставщика, выполните следующие действия:
    ...
    2. Найдите **поставщика данных Oracle для .NET**, Вы должны быть в состоянии найти запись, как показано в следующем примере под **system.data** > **DbProviderFactories**:`<add name="Oracle Data Provider for .NET" invariant="Oracle.DataAccess.Client" description="Oracle Data Provider for .NET" type="Oracle.DataAccess.Client.OracleClientFactory, Oracle.DataAccess, Version=2.112.3.0, Culture=neutral, PublicKeyToken=89b483f429c47342" />`
* Копируйте эту запись в файле machine.config в следующей папке .NET 4.0: <системный диск:\>«Windows»Microsoft.NET-Framework64-v4.0.30319-Config-machine.config.. Затем измените версию на 4.xxx.x.x.
