---
title: Загрузка терабайт данных в хранилище данных S'L
...
[Хранилище данных Azure S'L](../../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is.md) — это облачная база данных, способная обрабатывать огромные объемы данных, как реляционных, так и несвязанных.  Благодаря реализованной архитектуре вычислений с массовым параллелизмом (MPP) хранилище данных SQL оптимизировано для рабочих нагрузок хранилища корпоративных данных.  Оно предоставляет эластичность облака и гибкие возможности масштабирования хранилища и вычислительной мощности независимо друг от друга.
...
>
> Вы также можете построить трубопроводы с помощью Visual Studio, PowerShell и т.д. [См. Учебник: Копирование данных из базы данных Azure Blob в базу данных Azure S'L](data-factory-copy-data-from-azure-blob-storage-to-sql-database.md) для быстрого прохождения с пошаговыми инструкциями для использования копирования активности в фабрике данных Azure.  
>
>
...
3. На панели **Новая фабрика данных** сделайте следующее:
   ...
   1. В поле **Имя** введите **LoadIntoSQLDWDataFactory**.
       Имя фабрики данных Azure должно быть глобально уникальным. Если вы получили ошибку: **название фабрики данных "LoadIntoSLDWDataFactory" недоступно,** измените название фабрики данных (например, yournameLoadIntoSLDWDataFactory) и попробуйте создать еще раз. Ознакомьтесь с разделом [Фабрика данных — правила именования](data-factory-naming-rules.md) , чтобы узнать о правилах именования артефактов фабрики данных.  
...
4. После завершения создания вы увидите лезвие **фабрики данных,** указанное на следующем изображении:
...
4. После нажатия кнопки **Далее** параметры формата файлов определяются автоматически.  Проверьте, чтобы убедиться, что ограничители столбца ''' вместо запятой по умолчанию ','.  Просмотрев данные, нажмите кнопку **Далее**.
...
2. После завершения развертывания щелкните `Click here to monitor copy pipeline`, чтобы отслеживать ход выполнения копирования. Выберите конвейер копирования, созданный при работе со списком **Окна действий**.
    ...
    Как вы можете видеть на следующем скриншоте, копирование 1 ТБ из хранилища Azure Blob в хранилище данных S'L заняло 14 минут, эффективно достигнув пропускной памяти 1,22 GBps!
...
Дополнительные сведения см. в разделе [Рекомендации по использованию хранилища данных SQL Azure](../../synapse-analytics/sql-data-warehouse/sql-data-warehouse-best-practices.md).
