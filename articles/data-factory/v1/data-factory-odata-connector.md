---
title: Перемещение данных из источников OData
...
В этой статье описывается, как с помощью действия копирования в фабрике данных Azure перемещать данные из источника OData. Он основывается на статье [«Деятельность движения данных»,](data-factory-data-movement-activities.md) в которой представлен общий обзор движения данных с активностью копирования.
...
| --- | --- | --- |
| type |Свойство типа должно быть установлено: **OData** |Да |
...
| username |При использовании обычной проверки подлинности укажите имя пользователя. |Да (только при использовании обычной проверки подлинности) |
...
| gatewayName |Имя шлюза, который следует использовать службе фабрики данных для подключения к локальной службе OData. Укажите только, копируете данные из помещения источника OData. |нет |
...
Раздел **typeProperties** отличается для каждого типа набора данных и предоставляет информацию о местоположении данных в хранилище данных. Раздел typeProperties набора данных типа **ODataResource** (который включает в себя набор данных OData) содержит следующие свойства.
...
Как указано в статье о [движении данных,](data-factory-data-movement-activities.md) активность Copy выполняет автоматические преобразования типов из типов источников в типы раковин со следующим двухэтапным подходом.
