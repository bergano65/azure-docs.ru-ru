...
| gatewayName |Имя шлюза, который следует использовать службе фабрики данных для подключения к локальной службе OData. Укажите только, если при копировании данных из источника OData в локальной среде. |Нет |
...
**Связанная служба OData.** В этом примере используется анонимная проверка подлинности. Сведения о различных типах проверки подлинности, которые можно использовать, см. в разделе [Свойства связанной службы OData](#linked-service-properties).
