---
title: Перемещение данных из MyS'L с помощью фабрики данных Azure
...
В этой статье рассказывается, как с помощью действия копирования в фабрике данных Azure перемещать данные из локальной базы данных MySQL. Он основывается на статье [«Деятельность движения данных»,](data-factory-data-movement-activities.md) в которой представлен общий обзор движения данных с активностью копирования.

Вы можете скопировать данные из локального хранилища данных MySQL в любой поддерживаемый приемник данных. Для списка хранилиров данных, поддерживаемых как [Supported data stores](data-factory-data-movement-activities.md#supported-data-stores-and-formats) раковины активностью копирования, см. Сейчас фабрика данных поддерживает только перемещение данных из локального хранилища данных MySQL в другие хранилища данных, но не наоборот. 
...
Для подключения шлюза управления данными к базе данных MyS'L необходимо установить [разъем myS'L-NET для Microsoft Windows](https://dev.mysql.com/downloads/connector/net/) (версия между 6.6.5 и 6.10.7) на той же системе, что и шлюз управления данными. Это 32-разрядная версия драйвера совместима с 64-разрядной версией шлюза управления данными. Поддерживается MySQL версии 5.1 и более.

> [!TIP]
> Если вы нажали на ошибку на "Authentication не удалось, потому что удаленная сторона закрыла транспортный поток.", рассмотреть вопрос об обновлении MyS'L Разъем / NET до более высокой версии.
...
Раздел **typeProperties** отличается для каждого типа набора данных и предоставляет информацию о местоположении данных в хранилище данных. Раздел typeProperties набора данных с типом **RelationalTable** (который включает набор данных MySQL) содержит следующие свойства.
...
Когда источник в активности копирования имеет тип **RelationalSource** (который включает В.
