...
- Сейчас можно настроить четыре узла физического шлюза для одного логического шлюза. Если требуется более четырех узлов по соображениям производительности, отправьте сообщение электронной почты по [DMGHelp@microsoft.com](mailto:DMGHelp@microsoft.com)адресу.
...
- Если прокси-сервер HTTP необходим для всех узлов шлюза, настройте прокси-сервер в файлах diahost. exe. config и diawp. exe. config и используйте Диспетчер серверов, чтобы убедиться, что все узлы имеют одинаковые файлах diahost. exe. config и диавип. exe. config. Дополнительные сведения см. в разделе [Настройка параметров прокси-сервера](data-factory-data-management-gateway.md#configure-proxy-server-settings) . 
