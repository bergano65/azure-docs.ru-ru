---
title: Высокая доступность шлюза управления данными на фабрике данных Azure
...
Шлюзы управления данными, установленные на нескольких локальных компьютерах, можно связать с одним логическим шлюзом с портала. Эти машины называются **узлами.** С логическим шлюзом можно связать **четыре узла**. Ниже перечислены преимущества связывания нескольких узлов (локальных компьютеров с установленным шлюзом) с логическим шлюзом.  
...
Шлюз с несколькими узлами синхронизирует учетные данные хранилища данных на всех узлах. При наличии проблемы подключения узлов к узлам учетные данные могут быть не синхронизированы. Когда вы устанавливаете учетные данные для предварительного хранилища данных, использующего шлюз, он сохраняет учетные данные на узе диспетчера/рабочего. Узел-диспетчер синхронизируется с другими рабочими узлами. Этот процесс известен как **синхронизация учетных данных.** Канал связи между узлами может быть **зашифрован** общедоступным сертификатом SSL/TLS. 
...
Concurrent Jobs (Running/Limit) (Одновременные задания (выполняемые и ограниченные)) | Число заданий или задач, выполняющихся на каждом узле. Это значение соответствует моментальному снимку в режиме, близком к реальному времени. Ограничение означает максимальное количество одновременно выполняемых заданий для каждого узла. Это значение определяется на основе размера компьютера. Можно увеличить лимит для увеличения одновременного выполнения задания в продвинутых сценариях, где процессор/память/сеть недостаточно используются, но действия отходят от времени. Эта возможность также доступна с одноузловой шлюзом (даже если функция масштабируемости и доступности не включена). Дополнительные сведения см. в разделе о [рекомендациях по масштабированию](#scale-considerations). 
...
- Сейчас можно настроить четыре узла физического шлюза для одного логического шлюза. Если вам нужно более четырех узлов по причинам [DMGHelp@microsoft.com](mailto:DMGHelp@microsoft.com)производительности, отправьте электронное письмо.
...
- Если прокси HTTP требуется для всех узлов шлюза, установите прокси в diahost.exe.config и diawp.exe.config, и используйте менеджер сервера, чтобы убедиться, что все узлы имеют одинаковые diahost.exe.config и diawip.exe.config. Для получения подробной информации смотрите раздел [настройки параметров прокси.](data-factory-data-management-gateway.md#configure-proxy-server-settings) 
...
- Используйте официальный сертификат TLS, если вы решите зашифровать канал связи узла к узну. Самозаверяющий сертификат может вызвать проблемы с подключением, так как тот же сертификат не может быть доверенным в списке центров сертификации на других компьютерах. 
