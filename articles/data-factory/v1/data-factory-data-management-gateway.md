...
1. Разработчик создает шлюз для фабрики данных Azure, используя [портал Azure](https://portal.azure.com) или [командлет PowerShell](https://docs.microsoft.com/powershell/module/az.datafactory/).
...
> 2. Для некоторых облачных баз данных (например, [базы данных SQL Azure](https://docs.microsoft.com/azure/sql-database/sql-database-configure-firewall-settings), [Azure Data Lake](https://docs.microsoft.com/azure/data-lake-store/data-lake-store-secure-data#set-ip-address-range-for-data-access)и т. д.) может потребоваться разрешить IP-адрес компьютера шлюза в конфигурации брандмауэра.
>
>
...
> [!NOTE]
> Если брандмауэр блокирует исходящий порт 1433, то шлюз не сможет напрямую получить доступ к SQL Azure. В этом случае вы можете использовать [промежуточное копирование](https://docs.microsoft.com/azure/data-factory/data-factory-copy-activity-performance#staged-copy) в базу данных SQL или SQL Управляемый экземпляр/SQL Azure DW. Тогда для перемещения данных понадобится только протокол HTTPS (порт 443).
>
>
...
5. Не закрывайте **диспетчер конфигурации шлюза управления данными** .
    ...
    ![диспетчер конфигураций](./media/data-factory-data-management-gateway/ConfigurationManager.png)
...
Если вы ищете подход на основе API для шифрования учетных данных, вы можете использовать командлет PowerShell [New-аздатафакторенкриптвалуе](https://docs.microsoft.com/powershell/module/az.datafactory/new-azdatafactoryencryptvalue) для шифрования учетных данных. Командлет шифрует учетные данные с помощью сертификата, который настроен в шлюзе. Зашифрованные учетные данные добавляются в элемент **EncryptedCredential** в **connectionString** в JSON. Используйте JSON с командлетом [New-аздатафакторилинкедсервице](https://docs.microsoft.com/powershell/module/az.datafactory/new-azdatafactorylinkedservice) или в редакторе фабрики данных.
