...
2. Нажмите кнопку **загрузить**выберите **64-разрядных** версии (32-разрядных больше не поддерживается) и нажмите кнопку **Далее**.
...
* **Без прокси**. Шлюз явно не использует никакой прокси-сервер, чтобы подключиться к облачным службам.
...
* **Использовать настраиваемый прокси**. Шлюз использует параметры HTTP-прокси вместо конфигурации, настроенной в файлах diahost.exe.config и diawp.exe.config. Необходимо указать адрес и порт. Имя пользователя и пароль являются необязательными и указываются в зависимости от параметров аутентификации прокси-сервера. Все параметры шифруются с помощью сертификата учетных данных шлюза и сохраняются локально на хост-компьютере шлюза.
...
1. В проводнике, сделать копию *C:\\\\Program Files\\шлюз управления данными Майкрософт\\2.0\\Shared\\файлах diahost.exe.config* для резервную копию исходного файла.
2. Запустите Notepad.exe с правами администратора и откройте текстовый файл *C:\\\\Program Files\\шлюз управления данными Майкрософт\\2.0\\Shared\\ файле diahost.exe.config*. Найдите тег по умолчанию для system.net, как показано в коде ниже.
...
2. Переключиться в режим *C:\\\\Program Files\\Microsoft Integration Runtime\\3.0\\PowerShellScript\\*  папки.
...
2. Запустите исполняемый файл *ConfigManager.exe* в папке: *C:\\\\Program Files\\шлюз управления данными Майкрософт\\2.0\\общих*.
...
Status | Состояние логического шлюза и узлов шлюза, Пример: "В сети", "Автономно", "Ограничено" и т. д. Сведения об этих состояниях см. в разделе [Состояние шлюза](#gateway-status).
...
Если вы ищете подход на основе API для шифрования учетных данных, можно использовать [New AzDataFactoryEncryptValue](https://docs.microsoft.com/powershell/module/az.datafactory/new-azdatafactoryencryptvalue) командлет PowerShell для шифрования учетных данных. Командлет шифрует учетные данные с помощью сертификата, который настроен в шлюзе. Зашифрованные учетные данные добавляются в элемент **EncryptedCredential** в **connectionString** в JSON. Используйте JSON с помощью [New AzDataFactoryLinkedService](https://docs.microsoft.com/powershell/module/az.datafactory/new-azdatafactorylinkedservice) командлета или в редакторе фабрики данных.
