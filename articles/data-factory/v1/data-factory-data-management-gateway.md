...
* **Использовать системный прокси-сервер**: шлюз использует параметр прокси-сервера, настроенный в файлах diahost. exe. config и diawp. exe. config. Если прокси-сервер не настроен в файлах diahost. exe. config и diawp. exe. config, шлюз подключается к облачной службе напрямую, не пробегая через прокси-сервер.
* **Использовать настраиваемый прокси-сервер**. Настройте параметр прокси-сервера HTTP, который будет использоваться для шлюза, вместо использования конфигураций в файлах diahost. exe. config и diawp. exe. config. Необходимо указать адрес и порт. Имя пользователя и пароль являются необязательными и указываются в зависимости от параметров аутентификации прокси-сервера. Все параметры шифруются с помощью сертификата учетных данных шлюза и сохраняются локально на хост-компьютере шлюза.
...
При выборе параметра **использовать системные прокси** -серверы для прокси-сервера HTTP шлюз использует параметр прокси-сервера в файлах diahost. exe. config и diawp. exe. config. Если прокси-сервер не указан в файлах diahost. exe. config и diawp. exe. config, шлюз подключается к облачной службе напрямую, не пробегая через прокси-сервер. Ниже приводится процедура обновления файла конфигурации diahost.exe.config.

1. В проводнике Создайте надежную копию *\\\\диска C: Program Files\\Microsoft Управление данными Gateway\\2,0\\Shared\\файлах diahost. exe. config* , чтобы создать резервную копию исходного файла.
2. Запустите программу Notepad. exe с правами администратора и откройте текстовый файл *C:\\\\Program\\Files Microsoft Управление данными\\Gateway\\2,0\\Shared файлах diahost. exe. config*. Вы найдете тег по умолчанию для system.net, как показано в следующем коде:
...
> [!IMPORTANT]
> Не забудьте обновить как файлах diahost. exe. config, **так** и diawp. exe. config.
...
Offline | Узел находится в автономном режиме.
...
Offline | Узлы не подключены к сети.
