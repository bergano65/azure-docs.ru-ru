---
title: Шлюз управления данными для фабрики данных
...
* Когда хост-компьютер переходит в спящий режим, шлюз не может отвечать на запросы данных. Таким образом, настроить соответствующий **план питания** на компьютере перед установкой шлюза. Если компьютер настроен на использование режима гибернации, во время установки шлюза отобразится соответствующее сообщение.
...
2. Нажмите **Скачать,** выберите **64-битную** версию (32-бит больше не поддерживается), и нажмите **Далее**.
...
Например, для копирования из хранилища данных в **хранилище данных Azure s'L или в хранилище данных Azure S'L,** сделайте следующие шаги:

* Разрешить исходящие **связи TCP** на порт **1433** для брандмауэра Windows и корпоративного брандмауэра.
...
Шлюз использует прокси-сервер для подключения к облачной службе. Нажмите **Ссылка Изменения** во время начальной настройки. Отобразится диалоговое окно **Proxy setting** (Настройка прокси-сервера).
...
* **Используйте систему прокси**: Gateway использует настройку прокси, настроенную в diahost.exe.config и diawp.exe.config. Если прокси не настроен в diahost.exe.config и diawp.exe.config, шлюз подключается к облачному сервису напрямую, не проходя через прокси.
* **Используйте пользовательский прокси:** Настройте настройки прокси HTTP для использования для шлюза, вместо использования конфигураций в diahost.exe.config и diawp.exe.config. Требуется адрес и порт. Имя пользователя и пароль являются необязательными и указываются в зависимости от параметров аутентификации прокси-сервера. Все параметры шифруются с помощью сертификата учетных данных шлюза и сохраняются локально на хост-компьютере шлюза.
...
Если вы выберете настройки **прокси-сервера Системы использования** для прокси HTTP, шлюз использует настройки прокси в diahost.exe.config и diawp.exe.config. Если в diahost.exe.config и diawp.exe.config прокси-сервере не указан ни один прокси, шлюз подключается к облачному сервису напрямую, не проходя через прокси. Ниже приводится процедура обновления файла конфигурации diahost.exe.config.

1. В File Explorer сделайте безопасную копию *\\\\C:\\Program\\Files\\\\Microsoft Data Management Gateway 2.0 Shared diahost.exe.config* для резервного копирования исходного файла.
2. Запуск Notepad.exe работает в качестве администратора, и открытый текстовый файл *C:\\\\Программные\\файлы Microsoft Data Management Gateway\\2.0\\Общие\\diahost.exe.config*. Вы найдете тег по умолчанию для system.net, как показано в следующем коде:
...
> [!IMPORTANT]
> Не забудьте **both** обновить diahost.exe.config и diawp.exe.config.
...
2. Переключитесь на *папку\\\\C: Программные\\файлы\\ \\Microsoft Integration Runtime\\3.0 PowerShellScript* папку.
...
2. Выполнить исполняемый *ConfigManager.exe* в папке: *C:\\\\\\Программные\\файлы\\Microsoft Data Management Gateway 2.0 Общие*.
...
* **Сертификат Просмотра SSL** используется для связи TLS/SSL между порталом и шлюзом для набора учетных данных для источников данных.
...
Состояние | Состояние логического шлюза и узлов шлюза, Пример: Онлайн/Оффлайн/Ограниченное/и т.д. Для получения информации об этих статусах, [См.](#gateway-status)
...
Concurrent Jobs (Running/Limit) (Одновременные задания (выполняемые и ограниченные)) | Число заданий или задач, выполняющихся на каждом узле. Это значение соответствует моментальному снимку в режиме, близком к реальному времени. Ограничение означает максимальное количество одновременно выполняемых заданий для каждого узла. Это значение определяется на основе размера компьютера. Можно увеличить лимит для увеличения одновременного выполнения задания в продвинутых сценариях, где процессор/память/сеть недостаточно используются, но действия отходят от времени. Эта возможность также доступна с одноузловой шлюзом (даже если функция масштабируемости и доступности не включена).
...
3. На странице **шлюза Data** нажмите **Скачать и установить шлюз данных.**
...
Если вы ищете подход на основе API для шифрования учетных данных, вы можете использовать [New-AzDataFactoryValuePower](https://docs.microsoft.com/powershell/module/az.datafactory/new-azdatafactoryencryptvalue) PowerShell cmdlet для шифрования учетных данных. Командлет шифрует учетные данные с помощью сертификата, который настроен в шлюзе. Зашифрованные учетные данные добавляются в элемент **EncryptedCredential** в **connectionString** в JSON. Вы используете JSON с cmdlet [New-AzDataFactoryLinkedService](https://docs.microsoft.com/powershell/module/az.datafactory/new-azdatafactorylinkedservice) или в редакторе фабрики данных.
...
2. Войдите в учетную запись Azure, выполнив следующую команду и введя свои учетные данные Azure.
    ...
    Connect-AzAccount
    ```
3. Используйте смдlet **New-AzDataFactoryGateway** для создания логического шлюза следующим образом:
...
1. В Azure PowerShell переключитесь на папку: *\\\\C: Программные\\файлы\\\\Microsoft Integration Runtime\\3.0 PowerShellScript*. Выполните сценарий *RegisterGateway.ps1*, связанный с локальной переменной **$Key**, как показано в следующей команде. Этот сценарий регистрирует агент клиента, установленный на вашем компьютере с логическим шлюзом, созданным ранее.
    ...
    ```
2. Для получения списка шлюзов на фабрике данных можно использовать cmdlet **Get-AzDataFactoryGateway.** Если в поле **Состояние** указано значение **Подключено**, то шлюз готов к использованию.

    ```powershell        
    Get-AzDataFactoryGateway -DataFactoryName <dataFactoryName> -ResourceGroupName ADF
    ```
   Вы можете удалить шлюз с помощью cmdlet **Remove-AzDataFactoryGateway** и обновить описание шлюза с помощью cmdlets **Set-AzDataFactoryGateway.** Дополнительную информацию о синтаксисе и другую информацию об этих командлетах см. в "Справочных материалах по командлетам фабрики данных".  
