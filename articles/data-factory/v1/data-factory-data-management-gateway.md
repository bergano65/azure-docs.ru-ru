---
title: Шлюз Управление данными для фабрики данных
description: Используйте шлюз управления данными в фабрике данных Azure для перемещения данных.
...
Эта статья дополняет пошаговое руководство, приведенное в статье [Перемещение данных между локальными источниками и облаком с помощью шлюза управления данными](data-factory-move-data-between-onprem-and-cloud.md) . В этом пошаговом руководстве вы создадите конвейер, который использует шлюз для перемещения данных из базы данных SQL Server в большой двоичный объект Azure. В этой статье содержатся подробные сведения об использовании шлюза управления данными.
...
* Эффективное перемещение данных — данные передаются параллельно, и благодаря логике автоматических повторов перемещение не зависит от временных сетевых проблем.
...
1. Разработчик создает шлюз для фабрики данных Azure, используя [портал Azure](https://portal.azure.com) или [командлет PowerShell](/powershell/module/az.datafactory/).
...
* Требуется .NET Framework 4.5.1 или более поздней версии. При установке шлюза на компьютере под управлением Windows 7 установите .NET Framework 4.5 или более поздней версии. Дополнительные сведения см. в разделе [Требования к системе для .NET Framework](/dotnet/framework/get-started/system-requirements).
...
* Когда хост-компьютер переходит в спящий режим, шлюз не может отвечать на запросы данных. Поэтому перед установкой шлюза настройте на компьютере соответствующую **схему управления питанием** . Если компьютер настроен на использование режима гибернации, во время установки шлюза отобразится соответствующее сообщение.
...
2. Щелкните **скачать**, выберите **64-разрядную** версию (32-бит больше не поддерживается) и нажмите кнопку **Далее**.
...
4. На странице **Настройка** щелкните **Повторное создание ключа**. Нажмите кнопку "Да" в окне предупреждающего сообщения, внимательно его прочитав.
    ...
    ![Кнопка повторного создания ключа](media/data-factory-data-management-gateway/recreate-key-button.png)
...
> [!NOTE]
> 1. В зависимости от источника и приемников, может потребоваться разрешить дополнительные домены и исходящие порты в брандмауэре организации или в Windows.
> 2. Для некоторых облачных баз данных (например, [базы данных SQL Azure](../../azure-sql/database/firewall-configure.md), [Azure Data Lake](../../data-lake-store/data-lake-store-secure-data.md#set-ip-address-range-for-data-access)и т. д.) может потребоваться разрешить IP-адрес компьютера шлюза в конфигурации брандмауэра.
>
>
...
Например, чтобы скопировать данные из **локального хранилища данных в приемник базы данных SQL Azure или приемник аналитики Azure синапсе**, выполните следующие действия.

* Разрешите исходящий трафик **TCP** через порт **1433** для брандмауэра Windows и корпоративного брандмауэра.
* Настройте параметры брандмауэра логического сервера SQL Server, чтобы добавить IP-адрес компьютера шлюза в список разрешенных IP-адресов.

> [!NOTE]
> Если брандмауэр блокирует исходящий порт 1433, то шлюз не сможет напрямую получить доступ к SQL Azure. В этом случае вы можете использовать [промежуточное копирование](./data-factory-copy-activity-performance.md#staged-copy) в базу данных SQL или SQL Управляемый экземпляр/SQL Azure DW. Тогда для перемещения данных понадобится только протокол HTTPS (порт 443).
>
>
...
Шлюз использует прокси-сервер для подключения к облачной службе. Во время начальной настройки щелкните ссылку **изменить** . Отобразится диалоговое окно **Proxy setting** (Настройка прокси-сервера).

![Настройка прокси-сервера с помощью диспетчера конфигурации 1](media/data-factory-data-management-gateway/SetProxySettings.png)
...
* **Использовать системный прокси-сервер**: шлюз использует параметр прокси-сервера, настроенный в diahost.exe.config и diawp.exe.config. Если прокси-сервер не настроен в diahost.exe.config и diawp.exe.config, шлюз подключается к облачной службе напрямую, не пробегая через прокси-сервер.
* **Использовать настраиваемый прокси-сервер**: Настройте параметр прокси-сервера HTTP, который будет использоваться для шлюза, вместо использования конфигураций в diahost.exe.config и diawp.exe.config. Необходимо указать адрес и порт. Имя пользователя и пароль являются необязательными и указываются в зависимости от параметров аутентификации прокси-сервера. Все параметры шифруются с помощью сертификата учетных данных шлюза и сохраняются локально на хост-компьютере шлюза.
...
![Настройка прокси-сервера с помощью диспетчера конфигурации 2](media/data-factory-data-management-gateway/SetProxyConfigManager.png)
...
Если для прокси-сервера HTTP выбран параметр **использовать системный прокси** -сервер, шлюз использует параметр прокси-сервера в diahost.exe.config и diawp.exe.config. Если прокси-сервер не указан в diahost.exe.config и diawp.exe.config, шлюз подключается к облачной службе напрямую, не пробегая через прокси-сервер. Ниже приводится процедура обновления файла конфигурации diahost.exe.config.

1. В проводнике Создайте надежную копию файла *C: \\ \\ Program Files \\ Microsoft Управление данными Gateway \\ 2,0 \\ Shared \\diahost.exe.config* , чтобы создать резервную копию исходного файлов.
2. Запустите Notepad.exe с правами администратора и откройте текстовый файл *C: \\ \\ Program Files \\ Microsoft Управление данными Gateway \\ 2,0 \\ Shared \\diahost.exe.config*. Вы найдете тег по умолчанию для system.net, как показано в следующем коде:
    ...
    Внутри тега прокси-сервера можно указать дополнительные свойства, чтобы задать требуемые параметры, например scriptLocation. Сведения о синтаксисе см. в статье [Элемент <proxy> (параметры сети)](/dotnet/framework/configure-apps/file-schema/network/proxy-element-network-settings).
...
> [!IMPORTANT]
> Не забывайте обновлять как diahost.exe.config, **так** и diawp.exe.config.

Помимо этих точек необходимо также убедиться, Microsoft Azure находится в списке разрешенных компаний. Список допустимых IP-адресов Microsoft Azure можно скачать из [Центра загрузки Майкрософт](https://www.microsoft.com/download/details.aspx?id=41653).
...
2. При открытии диспетчера конфигурации отображается состояние "Отключено" или "Подключение". В средстве просмотра событий Windows в разделе "Журналы приложения и служб" > "Шлюз управления данными" отображается следующее сообщение об ошибке: `Unable to connect to the remote server`
...
2. Перейдите в папку *C: \\ \\ Program Files \\ Microsoft Integration Runtime \\ 3,0 \\ повершеллскрипт \\* .
...
2. Запустите исполняемый файл *ConfigManager.exe* в папке: *C: \\ \\ Program Files \\ Microsoft Управление данными Gateway \\ 2,0 \\ Shared*.
...
* Просмотр **SSL-сертификата** используется для связи TLS/SSL между порталом и шлюзом для настройки учетных данных для источников данных.
...
Concurrent Jobs (Running/Limit) (Одновременные задания (выполняемые и ограниченные)) | Число заданий или задач, выполняющихся на каждом узле. Это значение соответствует моментальному снимку в режиме, близком к реальному времени. Ограничение означает максимальное количество одновременно выполняемых заданий для каждого узла. Это значение определяется на основе размера компьютера. Можно увеличить предельное значение для увеличения масштаба параллельного выполнения заданий в сложных сценариях, в которых ЦП, память и сеть не используются, но время ожидания действий истекло. Эта возможность также доступна для шлюза с одним узлом (даже если функция масштабируемости и доступности не включена).
Роль | В многоузловом шлюзе есть два типа ролей — рабочая роль и диспетчер. Все узлы являются рабочими, то есть их можно использовать для выполнения заданий. Только один узел выполняет роль диспетчера, который используется для извлечения задачи или задания из облачных служб и отправки в разные рабочие узлы (включая сам узел-диспетчер).
...
3. На странице **шлюз данных** щелкните **скачать и установить шлюз данных**.
...
6. На странице **Настройка** на портале щелкните **Повторное создание ключа** на панели команд и щелкните **Да** в окне с предупреждением. Скопируйте ключ в буфер обмена с помощью **кнопки копирования** рядом с текстом ключа. Шлюз на старом компьютере прекращает работу сразу же, как только будет повторно создан ключ.
    ...
    ![Повторное создание ключа 2](./media/data-factory-data-management-gateway/RecreateKey.png)
...
1. Запустите веб-браузер на **компьютере шлюза** и перейдите на [портал Azure](https://portal.azure.com). Найдите нужную фабрику данных и откройте ее на странице **Фабрика данных**, а затем выберите **Author & Deploy** (Создать и развернуть), чтобы запустить редактор фабрики данных.
...
Если вы ищете подход на основе API для шифрования учетных данных, вы можете использовать командлет PowerShell [New-аздатафакторенкриптвалуе](/powershell/module/az.datafactory/new-azdatafactoryencryptvalue) для шифрования учетных данных. Командлет шифрует учетные данные с помощью сертификата, который настроен в шлюзе. Зашифрованные учетные данные добавляются в элемент **EncryptedCredential** в **connectionString** в JSON. Используйте JSON с командлетом [New-аздатафакторилинкедсервице](/powershell/module/az.datafactory/new-azdatafactorylinkedservice) или в редакторе фабрики данных.
...
2. Войдите в учетную запись Azure, выполнив следующую команду и введя свои учетные данные Azure.
    ...
    Connect-AzAccount
    ```
3. Используйте командлет **New-аздатафакторигатевай** , чтобы создать логический шлюз следующим образом:
...
1. В Azure PowerShell перейдите в папку: *C: \\ \\ Program Files \\ Microsoft Integration Runtime \\ 3,0 \\ повершеллскрипт \\*. Выполните сценарий *RegisterGateway.ps1*, связанный с локальной переменной **$Key**, как показано в следующей команде. Этот сценарий регистрирует агент клиента, установленный на вашем компьютере с логическим шлюзом, созданным ранее.
    ...
    ```
2. Для получения списка шлюзов в фабрике данных можно использовать командлет **Get-аздатафакторигатевай** . Если в поле **Состояние** указано значение **Подключено**, то шлюз готов к использованию.

    ```powershell        
    Get-AzDataFactoryGateway -DataFactoryName <dataFactoryName> -ResourceGroupName ADF
    ```
   Удалить шлюз можно с помощью командлета **Remove-аздатафакторигатевай** и описания обновления для шлюза с помощью командлетов **Set-аздатафакторигатевай** . Дополнительную информацию о синтаксисе и другую информацию об этих командлетах см. в "Справочных материалах по командлетам фабрики данных".  
...
* В разделе [Перемещение данных между локальными источниками и облаком с помощью шлюза управления данными](data-factory-move-data-between-onprem-and-cloud.md) . В этом пошаговом руководстве вы создадите конвейер, который использует шлюз для перемещения данных из базы данных SQL Server в большой двоичный объект Azure.