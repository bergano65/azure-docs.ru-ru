...
| Описание |Текст, описывающий, для чего используется действие или конвейер | Нет  |
...
| Свойство | ОПИСАНИЕ | Обязательно для заполнения | значение по умолчанию |
| --- | --- | --- | --- |
| frequency |Указывает единицу времени, которая определяет частоту создания среза данных.<br/><br/><b>Поддерживаемая частота</b>: Minute (минута), Hour (час), Day (день), Week (неделя), Month (месяц). |Yes |Нет данных |
...
| name | ОПИСАНИЕ | Обязательно для заполнения | По умолчанию |
| --- | --- | --- | --- |
...
| Свойство | ОПИСАНИЕ | Обязательно для заполнения |
|:--- |:--- |:--- |
| Тип | Свойству type необходимо задать значение **AzureDataLakeStore** | Yes |
...
| порт |TCP-порт, используемый сервером MongoDB для прослушивания клиентских подключений |Необязательно, по умолчанию — 27017 |
| authenticationType |Укажите тип Basic или Anonymous |Yes |
...
| host |Задает корневой путь к папке, которую необходимо скопировать. Чтобы указать специальные знаки в строке, используйте escape-символ "\". Примеры приведены в разделе "Примеры определений связанной службы и набора данных". |Yes |
...
| authenticationType |Anonymous или Windows. <br><br> Чтобы использовать **аутентификацию Kerberos** для соединителя HDFS, настройте локальную среду, как описано здесь. |Yes |
...
| authenticationType |Укажите тип аутентификации. Допустимые значения: **Basic**, **SshPublicKey**. <br><br> Описание свойств и примеры JSON для каждого типа см. ниже в разделах "Использование обычной аутентификации" и [Использование аутентификации с открытым ключом SSH](#using-ssh-public-key-authentication) соответственно. |Yes |
...
| index |Индекс таблицы в ресурсе. Дополнительные сведения см. в разделе "Получение индекса таблицы на HTML-странице". |Yes |
...
webServiceOutputs | Наборы данных, назначенные в качестве входных данных для веб-службы Студии машинного обучения Azure. Веб-служба возвращает выходные данные в этом наборе данных. | Yes |
...
| Свойство | Описание | Обязательно для заполнения |
|:--- |:--- |:--- |
| AssemblyName | Имя сборки. В нашем примере поисковый запрос будет выглядеть так: **MyDotnetActivity.dll**. | Yes |
| EntryPoint |Имя класса, реализующего интерфейс IDotNetActivity. В нашем примере поисковый запрос будет выглядеть так: В примере это **MyDotNetActivityNS.MyDotNetActivity**, где MyDotNetActivityNS — это пространство имен, а MyDotNetActivity — класс.  | Yes |
| PackageLinkedService | Имя связанной службы хранилища Azure, указывающей на хранилище BLOB-объектов с ZIP-файлом настраиваемого действия. В нашем примере поисковый запрос будет выглядеть так: **AzureStorageLinkedService**.| Yes |
