...
В следующем примере конвейера содержится одно действие типа **Copy** in the **действий** . В этом примере [активность Copy](data-factory-data-movement-activities.md) копирует данные из хранилища Azure Blob в базу данных Azure S'L.
...
* Раздел **определяет,** что для определения параметров времени выполнения, которые передаются скрипту улья в `${hiveconf:inputtable}` `${hiveconf:partitionedtable}`качестве значений конфигурации Hive (например, ).
...
В следующей таблице описаны свойства, которые можно использовать в разделе **доступности:**
...
| anchorDateTime |Определяет момент времени, на основе которого планировщик вычисляет границы среза набора данных. <br/><br/><b>Примечание</b>: Если AnchorDateTime имеет детали даты, которые являются более гранулированными, чем частота, то более гранулированные части игнорируются. <br/><br/>Например, если <b>интервал</b> <b>ежечасно</b> (частота: час и интервал: 1) и <b>AnchorDateTime</b> содержит <b>минуты и секунды,</b> то <b>минутные и секундные</b> части AnchorDateTime игнорируются. |нет |01/01/0001 |
| offset |Интервал времени, на который сдвигаются начало и конец всех срезов данных. <br/><br/><b>Примечание:</b>Если указаны как anchorDateTime, так и смещение, результатом является комбинированный сдвиг. |нет |Н/Д |
...
| minimumRows |Проверяет, содержат ли данные в **базе данных SQL Azure** или **таблице Azure** минимально необходимое количество строк. |<ul><li>База данных SQL Azure</li><li>таблице Azure</li></ul> |нет |Н/Д |
...
|:--- |:--- |
| **Azure** |[Хранение Azure Blob](#azure-blob-storage) |
| &nbsp; |Хранилище озера данных Azure |
| &nbsp; |[Azure Космос DB](#azure-cosmos-db) |
...
| &nbsp; |[Хранение таблицы Azure](#azure-table-storage) |
| **Баз данных** |[Амазонка Редсифт](#amazon-redshift) |
| &nbsp; |[IBM DB2](#ibm-db2) |
| &nbsp; |[Mysql](#mysql) |
| &nbsp; |[Oracle](#oracle) |
...
| compression | Укажите тип и уровень сжатия данных. Поддерживаемые типы: **ГЗип,** **Дефлировать**, **B'ip2**, и **цифферлат**. Поддерживаемые уровни: **Оптимальный** и **Быстрый**. Узнайте больше о [форматах файлов и сжатия данных в фабрике данных Azure](data-factory-supported-file-and-compression-formats.md#compression-support). |нет |
...
Если вы копируете данные из хранилища Azure Blob, установите **тип исхода** действия копии **на BlobSource**и укажите следующие свойства в **разделе исходный:**
...
| Свойство | Описание | Допустимые значения | Обязательно |
| --- | --- | --- | --- |
| copyBehavior |Это свойство определяет поведение функции копирования, когда в качестве источника используется BlobSource или FileSystem. |<b>PreserveHierarchy</b>: сохраняет иерархию файлов в папке-мишени. Относительный путь исходного файла в исходной папке идентичен относительному пути целевого файла в целевой папке.<br/><br/><b>FlattenHierarchy</b>: все файлы из исходной папки находятся в папке первого уровня целевой. Целевые файлы имеют автоматически сформированное имя. <br/><br/><b>MergeFiles (по умолчанию):</b> объединяет все файлы из исходной папки в один файл. Если указано имя файла или большого двоичного объекта, именем объединенного файла будет указанное имя; в противном случае имя файла будет автоматически сформировано. |нет |
...
| Свойство | Описание | Обязательно |
|:--- |:--- |:--- |
| type | Свойство типа должно быть установлено: **AzureDataLakeStore** | Да |
...
| subscriptionId | Идентификатор подписки Azure, к которому принадлежит Data Lake Store. | Необходимо для приемника |
...
| Свойство | Описание | Допустимые значения | Обязательно |
| --- | --- | --- | --- |
| copyBehavior |Определяет поведение копирования. |<b>PreserveHierarchy</b>: сохраняет иерархию файлов в папке-мишени. Относительный путь исходного файла в исходной папке идентичен относительному пути целевого файла в целевой папке.<br/><br/><b>FlattenHierarchy</b>: все файлы из исходной папки создаются в папке первого уровня целевой. Целевые файлы создаются с автоматически создаваемыми именами.<br/><br/><b>MergeFiles</b>: объединяет все файлы из исходной папки в один файл. Если указано имя файла или большого двоичного объекта, именем объединенного файла будет указанное имя; в противном случае имя файла будет автоматически сформировано. |нет |
...
| writeBatchSize |Число параллельных запросов к службе Azure Cosmos DB для создания документов.<br/><br/>При копировании данных из или в Azure Cosmos DB с помощью этого свойства можно оптимизировать производительность. Если увеличить значение свойства writeBatchSize, то производительность повышается, потому что в Azure Cosmos DB начинает поступать больше параллельных запросов. Однако необходимо избежать регулирования, которое может вызвать сообщение об ошибке: "Высокая частота запросов".<br/><br/>Регулирование определяется рядом факторов, в том числе размером документов, количеством терминов в документах, политикой индексации целевого сбора и т.д. Для операций копирования можно использовать улучшенную коллекцию (например, S3), чтобы иметь максимальную доступную пропускную выемку (2500 единиц запроса в секунду). |Целое число |Нет (значение по умолчанию — 5) |
...
| --- | --- | --- | --- |
| sqlWriterCleanupScript |Укажите запрос на выполнение действия копирования, позволяющий убедиться в том, что данные конкретного среза очищены. |Инструкция запроса. |нет |
| allowPolyBase |Указывает, следует ли использовать PolyBase (если применимо) вместо механизма BULKINSERT. <br/><br/> **Использование PolyBase — рекомендуемый способ загрузки данных в хранилище данных SQL.** |True <br/>False (по умолчанию) |нет |
...
### <a name="linked-service"></a>Связанные службы
Чтобы определить связанную с Azure функцию когнитивного поиска, установите **тип** связанной службы с **AzureSearch**и укажите следующие свойства в разделе **typeProperties:**
...
Для получения дополнительной информации смотрите статью [Azure Cognitive Search.](data-factory-azure-search-connector.md#linked-service-properties)

### <a name="dataset"></a>Dataset
Чтобы определить набор данных Azure Cognitive Search, установите **тип** набора данных в **AzureSearchIndex**и укажите следующие свойства в разделе **typeProperties:**
...
Для получения дополнительной информации смотрите статью [Azure Cognitive Search.](data-factory-azure-search-connector.md#dataset-properties)

### <a name="azure-cognitive-search-index-sink-in-copy-activity"></a>Azure когнитивный поисковый индекс раковина в копировании деятельности
Если вы копируете данные в индекс поиска, установите тип действия **копирования** на **AzureSearchIndexSink**и укажите следующие свойства в разделе **раковины:**
...
| WriteBatchSize | Загружает данные в индекс поиска, когда размер буфера достигает writeBatchSize. | 1–1000. Значение по умолчанию — 1000. | нет |
...
Для получения дополнительной информации смотрите статью [Azure Cognitive Search.](data-factory-azure-search-connector.md#copy-activity-properties)
...
| Свойство | Описание | Допустимые значения | Обязательно |
| --- | --- | --- | --- |
| azureTableDefaultPartitionKeyValue |Значение ключа раздела по умолчанию, которое может использоваться приемником. |Значение типа string. |нет |
...
### <a name="linked-service"></a>Связанные службы
Можно связать локтую файловую систему с фабрикой данных Azure с службой, связанной с **file Server.** В следующей таблице приведены описания элементов JSON, специфичными для службы, связанной с файловым сервером On-premises.
...
| format | Поддерживаются следующие типы формата: **TextFormat**, **JsonFormat**, **AvroFormat**, **OrcFormat**, **ParquetFormat**. Свойству **type** в разделе format необходимо присвоить одно из этих значений. Дополнительные сведения см. в разделах о [текстовом формате](data-factory-supported-file-and-compression-formats.md#text-format), [формате Json](data-factory-supported-file-and-compression-formats.md#json-format), [формате Avro](data-factory-supported-file-and-compression-formats.md#avro-format), [формате Orc](data-factory-supported-file-and-compression-formats.md#orc-format) и [ формате Parquet](data-factory-supported-file-and-compression-formats.md#parquet-format). <br><br> Если вы хотите **копировать файлы как между** файлохранилищами (двоичная копия), пропустите раздел формата как в водах, так и в определениях набора выходных данных. |нет |
| compression | Укажите тип и уровень сжатия данных. Поддерживаемые типы: **ГЗип,** **Дефлировать**, **B'ip2**, и **Цифферлат**; и поддерживаемые уровни: **Оптимальный** и **Быстрый**. См. сведения о [форматах файлов и сжатия данных в фабрике данных Azure](data-factory-supported-file-and-compression-formats.md#compression-support). |нет |
...
| enableServerCertificateValidation |Укажите, следует ли включать проверку сертификата сертификата TLS/SSL при использовании FTP по каналу SSL/TLS |нет |Да |
...
| format | Поддерживаются следующие типы формата: **TextFormat**, **JsonFormat**, **AvroFormat**, **OrcFormat**, **ParquetFormat**. Свойству **type** в разделе format необходимо присвоить одно из этих значений. Дополнительные сведения см. в разделах о [текстовом формате](data-factory-supported-file-and-compression-formats.md#text-format), [формате Json](data-factory-supported-file-and-compression-formats.md#json-format), [формате Avro](data-factory-supported-file-and-compression-formats.md#avro-format), [формате Orc](data-factory-supported-file-and-compression-formats.md#orc-format) и [ формате Parquet](data-factory-supported-file-and-compression-formats.md#parquet-format). <br><br> Если вы хотите **копировать файлы как между** файлохранилищами (двоичная копия), пропустите раздел формата как в водах, так и в определениях набора выходных данных. |нет |
| compression | Укажите тип и уровень сжатия данных. Поддерживаемые типы: **ГЗип,** **Дефлировать**, **B'ip2**, и **Цифферлат**; и поддерживаемые уровни: **Оптимальный** и **Быстрый**. Узнайте больше о [форматах файлов и сжатия данных в фабрике данных Azure](data-factory-supported-file-and-compression-formats.md#compression-support). |нет |
...
| Свойство | Описание | Обязательно |
| --- | --- | --- |
| type |Свойство типа должно быть установлено на: **Hdfs** |Да |
...
| encryptedCredential |[Новый-AzDataFactoryEncryptValue](https://docs.microsoft.com/powershell/module/az.datafactory/new-azdatafactoryencryptvalue) выход учетных данных доступа. |нет |
...
#### <a name="example-basic-authentication-with-encrypted-credential"></a>Пример: **Базовая аутентификация с зашифрованными учетными данными**
...
#### <a name="using-ssh-public-key-authentication"></a>**Использование общедоступной аутентификации ключей SSH:**
...
| enableServerCertificateValidation | Укажите, следует ли включить проверку сертификата Сертификата TLS/SSL, если источником является HTTPS Web Server | Нет. Значение по умолчанию — true. |
...
3. Нажмите справа на сертификат из личного магазина и выберите **все задачи**->**Управления частными ключами...**
...
Вы можете шифровать учетные данные с помощью смдлета [New-AzDataFactoryEncryptValue.](https://docs.microsoft.com/powershell/module/az.datafactory/new-azdatafactoryencryptvalue)
...
| --- | --- |
| [Кластер HDInsight по запросу](#on-demand-azure-hdinsight-cluster) или [собственный кластер HDInsight](#existing-azure-hdinsight-cluster) |[.NET пользовательская активность](#net-custom-activity), [Активность Hive,](#hdinsight-hive-activity) [Активность свиньи,](#hdinsight-pig-activity) [Активность MapReduce,](#hdinsight-mapreduce-activity)активность Хадуп, [активность Spark](#hdinsight-spark-activity) |
| [Пакетная служба Azure](#azure-batch) |[пользовательская деятельность .NET](#net-custom-activity) |
| [Машинное обучение Azure](#azure-machine-learning) | [Машинное обучение пакет исполнения деятельности](#machine-learning-batch-execution-activity), [машинное обучение Обновление ресурсной активности](#machine-learning-update-resource-activity) |
| [Аналитика озер данных Azure](#azure-data-lake-analytics) |[Аналитика озера данных U-SQL](#data-lake-analytics-u-sql-activity) |
| [База данных Azure SQL](#azure-sql-database), [хранилище данных Azure SQL](#azure-sql-data-warehouse), [SQL Server](#sql-server-stored-procedure) |[Сохраненная процедура](#stored-procedure-activity) |

## <a name="on-demand-azure-hdinsight-cluster"></a>Кластер Azure HDInsight по требованию
Для обработки данных служба фабрики данных Azure автоматически создает кластер HDInsight под управлением Windows/Linux по запросу. Кластер создается в том же регионе, что и учетная запись хранения (свойство linkedServiceName в JSON), связанная с кластером. Вы можете запустить следующие действия преобразования на этом связанном сервисе: [.NET пользовательская активность](#net-custom-activity), [активность Hive,](#hdinsight-hive-activity) [Активность Свиньи,](#hdinsight-pig-activity) [Активность MapReduce,](#hdinsight-mapreduce-activity)активность потоковой передачи Hadoop, [активность Spark.](#hdinsight-spark-activity)
...
Чтобы зарегистрировать собственный кластер HDInsight в фабрике данных, вы можете создать связанную службу Azure HDInsight. Вы можете запустить следующие действия преобразования данных на этом связанном сервисе: [.NET пользовательская активность](#net-custom-activity), [активность Hive,](#hdinsight-hive-activity) [активность Свиньи,](#hdinsight-pig-activity) [Активность MapReduce,](#hdinsight-mapreduce-activity)активность потоковой передачи Hadoop, [активность Spark.](#hdinsight-spark-activity)
...
## <a name="sql-server-stored-procedure"></a>Процедура хранения сервера S'L
...
Действие | Описание
-------- | -----------
[HDInsight Hive деятельности](#hdinsight-hive-activity) | Действие Hive HDInsight в конвейере фабрики данных выполняет запросы Hive к вашему собственному кластеру HDInsight или кластеру HDInsight по запросу под управлением Windows или Linux.
[HDInsight Свинья деятельности](#hdinsight-pig-activity) | Действие Pig HDInsight в конвейере фабрики данных выполняет запросы Pig к вашему собственному кластеру HDInsight или кластеру HDInsight по запросу под управлением Windows или Linux.
...
[Действие обновления ресурса в службе Машинного обучения](#machine-learning-update-resource-activity) | Со временем прогнозные модели в оценивающих экспериментах машинного обучения потребуют повторного обучения с помощью новых входных наборов данных. Когда повторное обучение будет завершено, вам потребуется обновить веб-службу оценки на основании обновленной модели машинного обучения. Чтобы обновить веб-службу с помощью заново обученной модели, можно использовать действие обновления ресурса Машинного обучения Azure.
...
[Деятельность Data Lake Analytics U-S'L](#data-lake-analytics-u-sql-activity) | Действие U-SQL Data Lake Analytics запускает сценарий U-SQL для кластера Azure Data Lake Analytics.
[пользовательская деятельность .NET](#net-custom-activity) | Если вам нужно преобразовать данные способом, который не поддерживается фабрикой данных Azure, то можно создать настраиваемое действие с собственной логикой обработки данных и использовать это действие в конвейере. Можно настроить запуск настраиваемого действия .NET с помощью пакетной службы Azure или кластера HDInsight.
...
- Свойство **типа** настроено на **HDInsightSpark.**
- **RootPath** установлен на **adfspark\\pyFiles,** где adfspark является контейнерazо Blob и pyFiles является тонкой папкой в этом контейнере. В этом примере хранилище BLOB-объектов Azure связано с кластером Spark. Файл можно отправить в другое хранилище Azure. Чтобы сделать это, создайте связанную службу хранилища Azure, которая свяжет эту учетную запись хранения с фабрикой данных. Затем укажите название связанного сервиса как значение для свойства **sparkJobLinkedService.** Сведения об этом свойстве и других свойствах, поддерживаемых действием Spark, см. в этом разделе.
- **ЗаписьFilePath** устанавливается на **test.py,** который является файлом python.
- Свойство **getDebugInfo** настроено **на always,** что означает, что файлы журнала всегда генерируются (успех или неудача).

    > [!IMPORTANT]
    > Если вы не устраняете неполадки, мы советуем не устанавливать для этого свойства значение Always в рабочей среде.
- Раздел **выводов** имеет один набор выходных данных. Вы должны определить выходной набор данных, даже если программа Spark не выдает выходные данные. На основе этого набора настраивается расписание конвейера (ежечасно, ежедневно и т. д.).
...
Вы можете указать следующие свойства в определении пакета пакетных действий По исполнению пакетов Пакетной деятельности Azure Machine Learning. Свойству type действия присваивается значение **AzureMLBatchExecution**. Сначала необходимо создать службу связанного с Azure Machine Learning службы и указать ее имя как значение для свойства **linkedServiceName.** В разделе **typeProperties** при установке AzureMLBatchExecution в качестве типа действия поддерживаются следующие свойства:

Свойство | Описание | Обязательно
-------- | ----------- | --------
webServiceInput | Набор данных, передаваемый в качестве входных данных для веб-службы Студии машинного обучения Azure. Его также следует включить в качестве входных данных для действия. |Используйте webServiceInput или webServiceInputs. |
webServiceInputs | Укажите наборы данных, передаваемые в качестве входных данных для веб-службы Студии машинного обучения Azure. Если веб-служба принимает несколько входных наборов данных, используйте свойство webServiceInputs вместо webServiceInput. Наборы данных, на которые ссылается свойство **webServiceInputs**, также должны быть включены в раздел **inputs** действия. | Используйте webServiceInput или webServiceInputs. |
webServiceOutputs | Наборы данных, назначенные в качестве входных данных для веб-службы Студии машинного обучения Azure. Веб-служба возвращает выходные данные в этом наборе данных. | Да |
...
Вы можете указать следующие свойства в студии машинного обучения Azure Update Resource Activity JSON. Свойству type действия присваивается значение **AzureMLUpdateResource**. Сначала необходимо создать службу связанного с Azure Machine Learning службы и указать ее имя как значение для свойства **linkedServiceName.** В разделе **typeProperties** при установке AzureMLUpdateResource в качестве типа действия поддерживаются следующие свойства:
