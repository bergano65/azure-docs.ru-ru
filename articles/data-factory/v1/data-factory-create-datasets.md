---
title: Создание наборов данных в фабрике данных Azure
...
| type |Тип набора данных. Укажите один из типов, которые поддерживает фабрика данных (например: AzureBlob, AzureSqlTable). <br/><br/>Для получения подробной информации [см.](#Type) |Да |Н/Д |
| structure |Схема набора данных.<br/><br/>Для получения подробной информации [см.](#Structure) |нет |Н/Д |
| typeProperties | Свойства каждого типа отличаются (например, свойства большого двоичного объекта Azure и таблицы SQL Azure). Для получения подробной информации об [Dataset type](#Type)поддерживаемых типах и их свойствах см. |Да |Н/Д |
...
В следующем примере набор данных представляет таблицу под названием **MyTable** в базе данных S'L.
...
* **тип** установлен на AzureSqlTable.
* Свойство типа **tableName** (используется только для типа AzureSqlTable) имеет значение MyTable.
* **linkedServiceName** относится к связанной службе типа AzureSqlDatabase, которая определена в следующем фрагменте JSON.
...
* **тип** установлен на AzureSqlDatabase.
...
В примере в предыдущем разделе задан тип набора данных **AzureSqlTable**. Аналогичным образом, для набора данных Azure Blob тип набора данных устанавливается в **AzureBlob,** как показано в следующем JSON:
...
| anchorDateTime |Определяет момент времени, на основе которого планировщик вычисляет границы среза набора данных. <br/><br/>Обратите внимание, что если это свойство имеет детали даты, которые являются более гранулированными, чем указанная частота, более гранулированные части игнорируются. Например, если для **интервала** задано значение **ежечасно** (frequency = Hour, interval = 1), а свойство **anchorDateTime** содержит **минуты и секунды**, то минуты и секунды свойства **anchorDateTime** не учитываются. |нет |01/01/0001 |
...
- [Мониторинг и управление конвейерами с помощью лопастей портала Azure](data-factory-monitor-manage-pipelines.md)
