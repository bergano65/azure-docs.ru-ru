---
title: Перемещение данных — шлюз Управление данными
...
3. На странице **Новая фабрика данных** введите **ADFTutorialOnPremDF** в поле "Имя".
   ...
   > [!IMPORTANT]
   > Имя фабрики данных Azure должно быть глобально уникальным. Если возникает ошибка: **имя фабрики данных "ADFTutorialOnPremDF" недоступно**, измените имя фабрики данных (например, йоурнамеадфтуториалонпремдф) и повторите попытку создания. Выполняя оставшиеся действия, описанные в этом руководстве, вместо ADFTutorialOnPremDF используйте именно это имя.
   >
   > В будущем имя фабрики данных может быть зарегистрировано как **DNS-** имя, поэтому оно становится общедоступным.
   >
   >
...
2. В редакторе фабрики данных щелкните **... Дополнительно** на панели инструментов и выберите **Новый шлюз данных**. Кроме того, в представлении в виде дерева можно щелкнуть правой кнопкой мыши **Шлюзы данных** и выбрать пункт **Новый шлюз данных**.
...
4. На странице **Настройка** щелкните **Установка непосредственно на этот компьютер**. Это позволит скачать пакет установки для шлюза, а также установить, настроить и зарегистрировать шлюз на компьютере.  
   ...
   >
   > То же самое (установку надстройки) сделайте и в случае с браузером Firefox. Нажмите кнопку **Открыть меню** на панели инструментов (**три горизонтальные линии** в правом верхнем углу), щелкните **надстройки**, выполните поиск с помощью ключевого слова "ClickOnce", выберите одно из расширений ClickOnce и установите его.    
   >
   >
   ...
   > [!NOTE]
   > Для успешной установки шлюза управления данными и его настройки вы должны обладать правами администратора на локальном компьютере. Вы можете добавить дополнительных пользователей в локальную группу Windows " **Пользователи шлюза управление данными** ". Участники этой группы могут использовать диспетчер конфигурации шлюза управления данными для настройки шлюза.
   >
   >
...
1. В **редакторе фабрики данных** щелкните **... Дополнительно**, нажмите кнопку **создать набор данных** на панели команд и выберите **SQL Server таблица**.
...
1. В редакторе фабрики данных щелкните **... Дополнительно** и щелкните **создать конвейер**.
2. Замените сценарий JSON в области справа на следующий текст:    
   ...
   * В разделе **typeProperties** в качестве **типа источника** указано **SqlSource** , а в качестве **типа приемника**— **BlobSink** .
...
На этом шаге вы используете портал Azure, чтобы отслеживать, что происходит в фабрике данных Azure. Вы также можете использовать командлеты PowerShell для мониторинга наборов данных и конвейеров. Дополнительные сведения о мониторинге см. в статье [Мониторинг конвейеров фабрики данных Azure и управление ими](data-factory-monitor-manage-pipelines.md).
...
4. Щелкните любой срез данных в списке, чтобы отобразить страницу **Срез данных**. Для этого среза отобразится раздел "Запуски операции". Как правило, в этом разделе отображается только одно действие.  
    ...
    Если срез не находится в состоянии " **Готово** ", можно увидеть неготовые неготовые срезы и заблокировать работу текущего среза в **неготовом списке неготовых срезов** .
