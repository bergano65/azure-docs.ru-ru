---
title: Перемещение данных - Шлюз управления данными
...
* **Учетная запись хранения Azure**. В этом руководстве в качестве **назначения и приемника** будет использоваться хранилище BLOB-объектов. Если у вас нет учетной записи хранения Azure, см. статью [учетной записи хранилища](../../storage/common/storage-account-create.md) для шагов по ее созданию.
* **Сервер S'L**. В этом руководстве используйте локальную базу данных SQL Server в качестве **исходного** хранилища данных.
...
3. На странице **Новая фабрика данных** введите **ADFTutorialOnPremDF** в поле "Имя".
   ...
   >
   > Название фабрики данных может быть зарегистрировано как имя **DNS** в будущем и, следовательно, стать общедоступным.
   >
   >
...
2. В редакторе Data Factory нажмите **... Подробнее** о панели инструментов, а затем нажмите **Новый шлюз данных**. Кроме того, в представлении в виде дерева можно щелкнуть правой кнопкой мыши **Шлюзы данных** и выбрать пункт **Новый шлюз данных**.
...
4. На странице **Настройка** щелкните **Установка непосредственно на этот компьютер**. Это позволит скачать пакет установки для шлюза, а также установить, настроить и зарегистрировать шлюз на компьютере.  
   ...
   > [!NOTE]
   > Для успешной установки шлюза управления данными и его настройки вы должны обладать правами администратора на локальном компьютере. Вы можете добавить дополнительных пользователей в локальную группу Windows **для пользователей шлюза управления данными.** Участники этой группы могут использовать диспетчер конфигурации шлюза управления данными для настройки шлюза.
   >
   >
...
8. Переключитесь на вкладку **«Настройки».** Сертификат, указанный в разделе **Сертификат,** используется для шифрования/расшифровки учетных данных для портаального хранилища данных, указанного на портале. Щелкните **Изменить** , чтобы использовать собственный сертификат (необязательно). По умолчанию шлюз использует сертификат, который автоматически создан службой фабрики данных.
...
2. В **редакторе JSON** справа сделайте следующее:
   ...
   2. В **connectionString** сделайте следующее:    
      ...
      5. Закройте пустую вкладку браузера, запустившую диалоговое окно, если она не закрылась автоматически, и вернитесь на вкладку портала Azure.
         ...
         В компьютере шлюза учетные данные **зашифрованы** с помощью сертификата, который принадлежит службе фабрики данных. Если вы хотите использовать вместо него сертификат, связанный со шлюзом управления данными, см. раздел, посвященный безопасной настройке учетных данных.    
...
1. В **редакторе data Factory**, нажмите **... Подробнее,** нажмите **новый набор данных** на панели команд и щелкните **таблицу сервера S'L.**
...
2. Замените сценарий JSON в области справа на следующий текст:
   ...
   * **Доступность** устанавливается **на почасовую** **(частота** установлена в **час** и **интервал** установлен до **1**).  Фабрика данных каждый час создает срез выходных данных в таблице **emp** в базе данных SQL Azure.

   Если вы не указали **fileName** для **таблицы вывода,** генерируемые файлы в `Data.<Guid>.txt` **папкеPath** названы в следующем формате: (например: Data.0a405f8a-93ff-4c6f-b3be-f69616f1df7a.txt.).
...
1. В редакторе Data Factory нажмите **кнопку ... Подробнее**, и нажмите **Новый конвейер**.
2. Замените сценарий JSON в области справа на следующий текст:    
   ...
   * В разделе **typeProperties,** **SqlSource** указан как **тип источника,** а **BlobSink** — **как тип раковины.**
...
**Поздравляю!** Вы успешно создали фабрику данных Azure, связанные службы, наборы данных и конвейер, а также выполнили планирование конвейера.
...
4. Щелкните любой срез данных в списке, чтобы отобразить страницу **Срез данных**. Для этого среза отобразится раздел "Запуски операции". Как правило, в этом разделе отображается только одно действие.  
    ...
    Если срез не находится в состоянии **готового,** можно увидеть неготовые срезы вверх по течению и блокируют текущий срез от выполнения в **неготовых списках Upstream.**
