...
1. Создайте проект **библиотеки классов .NET** .
   <ol type="a">
     ...
     <li>выберите тип <b>Библиотека классов</b> в списке типов проектов справа; В Visual Studio выберите <b>Библиотека классов (.NET Framework)</b> </li>
...
12. Создайте ZIP-файл **MyDotNetActivity.zip**, который содержит все двоичные файлы в \<папке проекта\>\bin\Debug. Добавьте файл **MyDotNetActivity.pdb**, чтобы в случае сбоя получить дополнительные сведения, например номер строки в исходном коде, вызвавшем ошибку.
    ...
    > [!IMPORTANT]
    > Все файлы в ZIP-архиве пользовательского действия должны размещаться на **верхнем уровне** без вложенных папок.
...
3. На панели инструментов щелкните **Развернуть**, чтобы создать и развернуть **InputDataset**. Убедитесь, что на панели заголовка в редакторе отображается сообщение **ТАБЛИЦА УСПЕШНО СОЗДАНА** .
...
2. Замените сценарий JSON в правой области на следующий:
    ...
    ```
     ...
     Выходное расположение — **adftutorial/customactivityoutput/** , а имя выходного файла — ГГГГ-ММ-ДД-ЧЧ.txt (где ГГГГ, ММ, ДД и ЧЧ — год, месяц, день и час создания среза). Дополнительные сведения см. в [справочнике разработчика][adf-developer-reference] .
   ...
   | 5 |2016-11-16T04:00:00 |2016-11-16-04.txt |
    ...
    Помните, что все файлы во входной папке являются частью среза со значениями времени начала, указанными выше. Во время обработки этого среза пользовательское действие сканирует каждый файл и создает строку в выходном файле с количеством вхождений условия поиска (Microsoft). Если в папке входных данных есть три файла, в выходном файле для каждого ежечасного среза есть три строки: 2016-11-16 00. txt, 2016-11-16:01:00:00. txt и т. д.
...
5. Если вы откроете выходной файл, вы увидите выходные данные, похожие на следующие:
    ...
    2 occurrences(s) of the search term "Microsoft" were found in the file inputfolder/2016-11-16-00/file.txt.
    ```
6. Используйте командлеты [портал Azure][azure-preview-portal] или Azure PowerShell, чтобы отслеживать фабрику данных, конвейеры и наборы данных. Вы можете просматривать сообщения **ActivityLogger** о настраиваемом действии в журналах (а именно — user-0.log), которые можно скачать на портале или с помощью командлетов.
...
5. Все файлы в ZIP-архиве пользовательского действия должны размещаться на **верхнем уровне** без вложенных папок.
