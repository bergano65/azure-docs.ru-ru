...
- **действие**. Это свойство представляет текущее действие. Его можно использовать для доступа к расширенным свойствам, связанным с пользовательским действием. Дополнительные сведения см. в разделе [Доступ к расширенным свойствам](#access-extended-properties).
- **средство ведения журнала**. Этот объект позволяет записывать комментарии отладки, которые отображаются в виде пользовательского журнала для конвейера.
...
1. Создайте проект **библиотеки классов .NET** .
   <ol type="a">
     ...
     <li>В меню <b>файл</b>выберите пункт <b>создать</b>и щелкните <b>проект</b>.</li>
     <li>Разверните узел <b>шаблоны</b>и выберите <b>Visual C#</b>. В этом руководстве используется язык C#, но для создания настраиваемого действия вы можете использовать любой язык .NET.;</li>
...
3. Если входной срез не имеет значение **Ready**, убедитесь, что структура входных папок верна, а **файл. txt** существует во входных папках.
