...
- [Деятельность движения данных](data-factory-data-movement-activities.md) для перемещения данных между [поддерживаемыми источниками и хранилищами данных sink.](data-factory-data-movement-activities.md#supported-data-stores-and-formats)
- [Мероприятия по преобразованию данных](data-factory-data-transformation-activities.md) с помощью вычислительных служб, таких как Azure HDInsight, Azure Batch и Azure Machine Learning.
...
* Визуальная студия 2012/2013/2015/2017
...
В этом руководстве вы запустите свои настраиваемые действия .NET с помощью пакетной службы Azure как вычислительные ресурсы. **Пакетная служба Azure** — это служба платформы, которая позволяет эффективно работать с приложениями для крупномасштабных параллельных и высокопроизводительных вычислений (HPC) в облаке. Azure Batch планирует интенсивно работать на управляемой **коллекции виртуальных машин**и может автоматически масштабировать вычислительные ресурсы для удовлетворения потребностей ваших рабочих мест. Подробные сведения о пакетной службе Azure см. в [этой статье][batch-technical-overview].

Для этого руководства создайте учетную запись пакетной службы Azure с пулом виртуальных машин. Ниже приводятся шаги:

1. Создайте **учетную запись Azure Batch** с помощью [портала Azure.](https://portal.azure.com) Инструкции см. в статье [Создание учетной записи пакетной службы Azure на портале Azure][batch-create-account].
...
- **деятельности**. Это свойство представляет текущее действие. Его можно использовать для доступа к расширенным свойствам, связанным с пользовательским действием. Дополнительные сведения см. в разделе [Доступ к расширенным свойствам](#access-extended-properties).
- **регистратор**. Этот объект позволяет записывать комментарии отладки, которые отображаются в виде пользовательского журнала для конвейера.
...
1. Создайте проект **библиотеки классов .NET** .
   <ol type="a">
     ...
     <li>Нажмите <b>файл,</b>укажите на <b>новый</b>, и нажмите <b>проекта</b>.</li>
     <li>Расширяйте <b>шаблоны</b>и выберите <b>Visual C .</b> В этом руководстве используется язык C#, но для создания настраиваемого действия вы можете использовать любой язык .NET.;</li>
     <li>Выберите тип <b>Библиотека классов</b> из списка типов проектов справа. В визуальной студии выберите <b>классбиблиотеку (.NET Framework)</b> </li>
     <li>В поле <b>Имя</b> введите <b>MyDotNetActivity</b>.</li>
     <li>Выберите <b>C: ADFGetStarted</b> для <b>местоположения</b>.</li>
...
3. В консоли менеджера пакетов выполните следующую команду для импорта **Microsoft.Azure.Management.DataFactories.**
...
6. Измените имя **пространства имен** на **MyDotNetActivityNS**.
    ...
    ```
7. Измените название класса на **MyDotNetActivity** и вычеркните его из интерфейса **IDotNetActivity,** показанного в следующем фрагменте кода:
...
11. Запуск **Windows Explorer**и переходите в папку **«бин-дебуг»** или **«бин-релиз»** в зависимости от типа сборки.
...
4. Создайте **конвейер,** в который используется пользовательская деятельность.
...
4. Убедитесь, что вы используете правильную **подписку** и **регион,** где вы хотите, чтобы фабрика данных была создана.
5. В колонке **Создание фабрики данных** нажмите кнопку **Создать**.
6. Вы видите, как создается фабрика данных, создаваемый на **панели мониторинга** портала Azure.
...
2. Нажмите **Новый хранилище данных** в панели команд и выберите **хранилище Azure.** В редакторе отобразится сценарий JSON для создания связанной службы хранилища Azure.
...
1. В редакторе Data Factory нажмите **... Подробнее** о панели команд нажмите **«Новый вычисление»,** а затем выберите **Azure Batch** из меню.
...
1. В **редакторе** для фабрики данных, нажмите **... Подробнее** о панели команд нажмите **«Новый набор данных»,** а затем выберите **хранилище Azure Blob** из меню выпадающих данных.
...
1. В **редакторе Data Factory**, нажмите **... Подробнее** о панели команд нажмите **«Новый набор данных»,** а затем выберите **хранилище Azure Blob.**
2. Замените сценарий JSON в правой области на следующий:
   ...
   | 5 |2016-11-16T04:00:00 |2016-11-16-04.txt |
    ...
    Помните, что все файлы во входной папке являются частью среза со значениями времени начала, указанными выше. Во время обработки этого среза пользовательское действие сканирует каждый файл и создает строку в выходном файле с количеством вхождений условия поиска (Microsoft). Если в папке ввода есть три файла, то в файле вывода есть три строки для каждого почасового среза: 2016-11-16-00.txt, 2016-11-16:01:00:00.txt и т.д.
...
1. В редакторе Data Factory нажмите **... Больше**, а затем выберите **новый конвейер** на панели команд.
2. Замените сценарий JSON в правой области приведенным ниже.
   ...
   * **AssemblyName** устанавливается на название DLL: **MyDotnetActivity.dll**.
   ...
   * **isPaused** свойство устанавливается **ложно** по умолчанию. В этом примере конвейер запускается незамедлительно, потому что срезы приходятся на прошлое. Для этого свойства можно задать значение true, чтобы приостановить работу конвейера. Чтобы перезапустить его, нужно снова установить значение false.
   * Время **начала** и **время окончания** пять часов друг от друга и ломтики производятся ежечасно, так что пять ломтиков производятся трубопровода. **five**
3. Чтобы развернуть конвейер, на панели команд нажмите кнопку **Развернуть**.

### <a name="monitor-the-pipeline"></a>Мониторинг конвейера
1. В лезвии фабрики данных на портале Azure нажмите **Диаграмма**.
...
3. Если срез ввода не настроен **на готовую,** подтвердите, что структура папки ввода является правильной и **file.txt** существует в папках ввода.
3. В методе **Execute** настраиваемого действия используйте объект **IActivityLogger**, чтобы записывать в журнал сведения, которые помогут устранить неполадки. Сообщения, записываемые в журналы, появятся в файлах журнала пользователя (один или несколько файлов с именами user-0.log, user-1.log, user-2.log и т. д.).
   ...
   В списке файлов журнала щелкните **user-0.log**. На панели справа отображаются результаты использования метода **IActivityLogger.Write** . Если вы не видите все сообщения, проверьте, есть ли у вас больше файлов журнала по имени: user_1.log, user_2.log и т.д. В противном случае код может вывести из строя после последнего зарегистрированного сообщения.
...
8. Настраиваемое действие не использует файл **app.config** из пакета. Поэтому, если код считывает какие-либо строки подключения из файла конфигурации, это не сработает во время выполнения. Наилучшей практикой при использовании Azure Batch является хранить какие-либо секреты в **Azure KeyVault,** использовать принцип службы на основе сертификатов для защиты **ключа**и распространять сертификат в пул Azure Batch. В этом случае пользовательское действие .NET в среде выполнения сможет использовать секреты из хранилища ключей. Это общее решение, которое можно использовать для любого типа секретов, а не только строк подключения.
