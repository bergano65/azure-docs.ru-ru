...
3. Используйте следующий скрипт SQL, чтобы создать таблицу **emp** в базе данных SQL Azure.  
    ...
    **Если на компьютере установлено SQL Server 2012/2014:** следуйте инструкциям по [управлению базой данных SQL Azure с помощью SQL Server Management Studio](../../sql-database/sql-database-manage-azure-ssms.md) , чтобы подключиться к серверу и запустить скрипт SQL.

    Если клиенту не разрешен доступ к логическому серверу SQL Server, необходимо настроить брандмауэр для сервера, чтобы разрешить доступ с компьютера (IP-адрес). Инструкции по настройке брандмауэра для сервера см. в [этой статье](../../sql-database/sql-database-configure-firewall-settings.md) .
