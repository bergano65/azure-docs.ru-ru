---
title: Копирование данных в/из файловой системы с помощью Фабрики данных Azure
...
> * [Версия 1](data-factory-onprem-file-system-connector.md)
> * [Версия 2 (текущая)](../connector-file-system.md)

> [!NOTE]
> В этой статье рассматривается служба "Фабрика данных Azure" версии 1. Если вы используете текущую версию Фабрики данных, см. статью о [соединителе файловой системы в службе "Фабрика данных Azure" версии 2](../connector-file-system.md).


В этой статье рассказывается, как с помощью действия копирования в фабрике данных Azure копировать данные в локальную файловую систему и обратно. Он основывается на статье [«Деятельность движения данных»,](data-factory-data-movement-activities.md) в которой представлен общий обзор движения данных с активностью копирования.
...
2. Создание **взаимосвязанных служб** для связи входных и выходных данных с фабрикой данных. Например, при копировании данных из хранилища BLOB-объектов Azure в локальную файловую систему создайте две связанные службы, чтобы связать локальную файловую систему и учетную запись хранения Azure с фабрикой данных. Сведения о свойствах связанной службы, относящихся к локальной файловой системе, см. в разделе [Свойства связанной службы](#linked-service-properties).
...
Можно связать локтую файловую систему с фабрикой данных Azure с службой связи **On-Premises File Server.** В таблице ниже приведены описания элементов JSON, которые относятся к связанной службе локального файлового сервера.
...
| encryptedCredential |Укажите зашифрованные учетные данные, которые вы можете получить, запустив смдлет New-AzDataFactoryEncryptValue. |Нет (если имя пользователя и пароль указываются в виде обычного текста) |
...
| format | Поддерживаются следующие типы формата: **TextFormat**, **JsonFormat**, **AvroFormat**, **OrcFormat**, **ParquetFormat**. Свойству **type** в разделе format необходимо присвоить одно из этих значений. Дополнительные сведения см. в разделах о [текстовом формате](data-factory-supported-file-and-compression-formats.md#text-format), [формате Json](data-factory-supported-file-and-compression-formats.md#json-format), [формате Avro](data-factory-supported-file-and-compression-formats.md#avro-format), [формате Orc](data-factory-supported-file-and-compression-formats.md#orc-format) и [ формате Parquet](data-factory-supported-file-and-compression-formats.md#parquet-format). <br><br> Если вы хотите **копировать файлы как между** файлохранилищами (двоичная копия), пропустите раздел формата как в водах, так и в определениях набора выходных данных. |нет |
| compression | Укажите тип и уровень сжатия данных. Поддерживаемые типы: **ГЗип,** **Дефлировать**, **B'ip2**, и **цифферлат**. Поддерживаемые уровни: **Оптимальный** и **Быстрый**. См. сведения о [форматах файлов и сжатия данных в фабрике данных Azure](data-factory-supported-file-and-compression-formats.md#compression-support). |нет |
...
| Свойство | Описание | Допустимые значения | Обязательно |
| --- | --- | --- | --- |
| copyBehavior |Это свойство определяет поведение функции копирования, когда в качестве источника используется BlobSource или FileSystem. |**СохранениеИерархия:** Сохраняет иерархию файлов в папке целевой. То есть относительный путь исходного файла в исходной папке идентичен относительному пути целевого файла в целевой папке.<br/><br/>**FlattenHierarchy**: все файлы из исходной папки создаются на первом уровне в целевой папке. Целевые файлы создаются с автоматически сформированными именами.<br/><br/>**MergeFiles:** Слияние всех файлов из исходной папки в один файл. Если указано имя большого двоичного объекта или имя файла, то оно присваивается объединенному файлу. В противном случае присваивается автоматически созданное имя файла. |нет |
...
* Набор ввода [данных](data-factory-create-datasets.md) типа [FileShare.](#dataset-properties)
...
* [Конвейер](data-factory-create-pipelines.md) с копированием деятельности, который использует [FileSystemSource](#copy-activity-properties) и [BlobSink](data-factory-azure-blob-connector.md#copy-activity-properties).
...
Конвейер содержит действие копирования, которое использует входной и выходной наборы данных и выполняется каждый час. В определении JSON тип **источника** устанавливается на **FileSystemSource,** а тип **раковины** устанавливается на **BlobSink.**
...
* Связанная служба типа [OnPremisesFileServer](#linked-service-properties).
* Набор входных данных типа [AzureSqltable](data-factory-azure-sql-connector.md#dataset-properties).
* Набор выходных данных типа [FileShare](#dataset-properties).
* Конвейер с копированием, который использует [SqlSource](data-factory-azure-sql-connector.md#copy-activity-properties) и [FileSystemSink.](#copy-activity-properties)
...
**Связанная служба базы данных Azure S'L:**
