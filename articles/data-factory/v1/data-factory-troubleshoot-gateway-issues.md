---
title: Проблемы управления данными
...
#### <a name="resolution"></a>Решение
Удалите существующий шлюз на машине и нажмите **на установку непосредственно на этом компьютере** ссылку снова.
...
#### <a name="cause"></a>Причина
Сертификат TLS/SSL мог быть потерян на шлюзе. Компьютер шлюза не может загрузить сертификат, используемый в настоящее время для шифрования TLS. Кроме того, в журнале событий также может появиться сообщение об ошибке примерно следующего вида.
...
3. Нажмите кнопку **«Изменение»,** чтобы изменить сертификат TLS/SSL.
...
2. Найдите журналы в папке «Системы > управления**данными»** **приложений и служб.**
