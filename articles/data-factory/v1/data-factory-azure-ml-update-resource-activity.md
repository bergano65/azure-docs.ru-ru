...
В следующей таблице описаны веб-службы, используемые в данном примере.  Дополнительные сведения см. в разделе [программное переобучение моделей машинное обучение Azure Studio (классическая модель)](../../machine-learning/studio/retrain-machine-learning-model.md) .
...
Если веб-служба оценки является **классической веб-службой**, создайте вторую, **не используемую по умолчанию и обновляемую конечную точку** , используя портал Azure. Инструкции см. в статье [Создание конечных точек](../../machine-learning/studio/create-endpoint.md). Создав обновляемую конечную точку не по умолчанию, выполните следующие действия:
