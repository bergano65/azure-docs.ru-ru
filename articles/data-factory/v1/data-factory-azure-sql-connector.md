---
title: Копирование данных в/из базы данных Azure S'L
...
В этой статье рассказывается, как с помощью действия копирования в Фабрике данных Azure перемещать данные в базу данных SQL Azure и обратно. Он основывается на статье [«Деятельность движения данных»,](data-factory-data-movement-activities.md) в которой представлен общий обзор движения данных с активностью копирования.
...
2. Создание **взаимосвязанных служб** для связи входных и выходных данных с фабрикой данных. Например, при копировании данных из хранилища BLOB-объектов Azure в базу данных SQL Azure создайте две связанные службы, чтобы связать учетную запись хранения Azure и базу данных SQL Azure с фабрикой данных. Сведения о свойствах связанной службы, относящихся к базе данных SQL Azure, см. в разделе [Свойства связанной службы](#linked-service-properties).
3. Создайте **наборы данных**, которые представляют входные и выходные данные для операции копирования. В примере, упомянутом на последнем этапе, создайте набор данных, чтобы указать контейнер BLOB-объектов и папку, содержащую входные данные. Также создайте другой набор данных, чтобы указать таблицу SQL в базе данных SQL Azure, содержащую данные, скопированные из хранилища BLOB-объектов. Сведения о свойствах набора данных, относящихся к Azure Data Lake Store, см. в разделе [Свойства набора данных](#dataset-properties).
...
| --- | --- | --- |
| type |Свойство типа должно быть настроено на: **AzureSqlDatabase** |Да |
...
3. Набор входных [данных](data-factory-create-datasets.md) типа [AzureSqltable](#dataset-properties).
...
При копировании данных в базу данных SQL Server действие копирования по умолчанию добавляет данные в таблицу приемника. Вместо этого, чтобы выполнить UPSERT, смотрите [Повторяемое запись в статью SqlSink.](data-factory-repeatable-copy.md#repeatable-write-to-sqlsink)
