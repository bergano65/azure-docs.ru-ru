---
title: Копирование данных в и из Azure Data Lake Storage Gen1
...
| **ресурсGroupName** | Имя группы ресурсов Azure, к которой принадлежит учетная запись Data Lake Store. | Необходимо для приемника |
...
>- **Чтобы использовать Data Lake Store**в качестве источника, предоставите, по крайней мере, разрешение на доступ к **данным,** чтобы перечислить и скопировать содержимое папки, или **Прочитайте** разрешение на копирование одного файла. Управление доступом на уровне учетной записи не требуется.
>- **Чтобы использовать Data Lake Store**в качестве раковины, предоставите, по крайней **мере, разрешение на** доступ к данным для создания элементов детского возраста в папке. Если для расширения возможностей копирования используется среда IR Azure (источник и приемник находятся в облаке), чтобы фабрика данных смогла определить регион Data Lake Store, назначьте в Системе управления идентификацией и доступом (IAM) роль по крайней мере **читателя**. Чтобы не назначать эту роль IAM, определите [executionLocation](data-factory-data-movement-activities.md#global) с расположением Data Lake Store в действии копирования.
>- Если вы **используете Copy Wizard для авторов конвейеров,** предоставите по крайней мере роль **Читателя** в управлении доступом к учетной записи (IAM). Кроме того, предоставьте разрешение по крайней мере на **чтение и выполнение** в корневом каталоге Data Lake Store (/) и его дочерних элементах. Иначе может появиться сообщение "Указаны недопустимые учетные данные".
...
2. Убедитесь, что пользователю или субъекту-службе предоставлена по крайней мере роль **читателя** в учетной записи Data Lake. Вот как это можно сделать:
    ...
    1. Перейдите на портал Azure -> учетную запись Магазина озера данных
    2. В колонке Data Lake Store щелкните **Управление доступом (IAM)**.
    3. Нажмите **Добавить ролевые назначения**
...
3. Если вы не хотите предоставлять роль **Reader** пользователю или директору службы, альтернативой является [явное указание местоположения выполнения](data-factory-data-movement-activities.md#global) в активности копирования с местоположением вашего магазина Data Lake Store. Пример
...
| Свойство | Описание | Обязательно |
|:--- |:--- |:--- |
| **папкаPath** |Путь к контейнеру и папке в Data Lake Store. |Да |
| **Имени файла** |Имя файла в Azure Data Lake Store. Свойство **fileName** является необязательным и в нем учитывается регистр символов. <br/><br/>Если указать значение **fileName**, то действие (включая копирование) работает с определенным файлом.<br/><br/>Если значение **fileName** не указано, то копируются все файлы в **folderPath** для входного набора данных.<br/><br/>Когда **fileName** не указан для набора данных вывода и **сохраненияИерархия** не указана в `Data._Guid_.txt`поглотителе активности, имя генерируемого файла находится в формате . Например: Data.0a405f8a-93ff-4c6f-b3be-f69616f1df7a.txt. |нет |
...
| --- | --- | --- | --- |
| **рекурсивные** |Указывает, следует ли читать данные рекурсивно из вложенных папок или только из указанной папки. |True (значение по умолчанию), False |нет |
