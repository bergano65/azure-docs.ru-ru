---
title: Перемещение данных в/из Azure Cosmos DB
...
* Экспорт документов JSON из коллекции Cosmos DB в различные файлообменники.
...
```
Azure Cosmos DB — это хранилище NoSQL для JSON-документов, в которых разрешено использовать вложенные структуры. Azure Data Factory позволяет пользователю обозначать иерархию через **nestingSeparator,** который является ". которым в этом примере является точка. Благодаря этому разделителю действие копирование создаст объект Name с тремя дочерними элементами, First, Middle и Last, в соответствии с элементами Name.First, Name.Middle и Name.Last в определении таблицы.
