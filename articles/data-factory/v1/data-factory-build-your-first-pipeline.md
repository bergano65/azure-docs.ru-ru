---
title: 'Учебник Data Factory: Первый конвейер данных '
...
Эта статья содержит общие сведения и описание необходимых компонентов для работы с учебником. После выполнения предварительных задач, вы можете сделать учебник, используя один из следующих инструментов / SDKs: Visual Studio, PowerShell, шаблон менеджер ресурсов, REST API. Выберите один из вариантов в раскрывающемся списке в начале (или) ссылки в конце этой статьи, чтобы пройти учебник, используя один из этих вариантов.    
...
3. Создание наборов входных и **выходных данных.** Входной набор данных представляет входные данные для действия в конвейере, а выходной набор данных — выходные данные для действия.
...
Visual Studio предоставляет способ графического интерфейса для создания фабрик данных. PowerShell, шаблон Resource Manager и интерфейс REST API, в свою очередь, дают возможность сделать это с помощью средств создания сценариев и программирования.
