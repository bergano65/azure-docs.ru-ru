...
2. Создайте следующую **хранимую процедуру**, вставляющую данные в таблицу **sampletable**.
   ...
   > [!IMPORTANT]
   > **Имя** и **регистр** параметра (в этом примере — DateTime) должны соответствовать имени и регистру параметра, указанного в конвейере или действии JSON. В определении сохраненной **\@** процедуры убедитесь, что это используется в качестве префикса для параметра.
...
3. В колонке **Новая фабрика данных** введите **SProcDF** в поле "Имя". Имена фабрики данных Azure **являются уникальными во всем мире.** Для успешного создания фабрики добавьте свое имя в качестве префикса имени фабрики.

   ![Новая фабрика данных](media/data-factory-stored-proc-activity/new-data-factory-blade.png)
4. Выберите **подписку На Azure.**
...
6. Выберите **место для** фабрики данных.
...
1. Нажмите **... Подробнее** о панели инструментов нажмите **на новый набор данных**и нажмите **Azure S'L.** **Новый набор данных** на панели команд и выберите **Azure SQL**.
...
1. Нажмите **... Подробнее** о панели команд и нажмите **Новый конвейер**.
...
Теперь передайте параметр **сценария** и значение из действия сохраненной процедуры. Раздел **typeProperties** в предыдущем образце выглядит следующим фрагментом:
