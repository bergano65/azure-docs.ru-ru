...
> [!NOTE]
> Статья относится к версии 1 фабрики данных Azure, которая является общедоступной версией. Если вы используете текущую версию службы "Фабрика данных", см. статью о [преобразовании данных с помощью действия Apache Spark в службе "Фабрика данных Azure"](../transform-data-using-spark.md).
...
1. Создайте учетную запись хранения общего назначения, следуя указаниям в [этом разделе](../../storage/common/storage-quickstart-create-account.md).
...
1. Выберите **Создать** > **Данные+аналитика** > **Фабрика данных**.
...
1. Выберите **Новое хранилище данных**, а затем **Служба хранилища Azure**.
...
1. Чтобы развернуть связанную службу, выберите **Развертывание** на панели команд. После развертывания связанной службы окно Draft-1 исчезнет. Экземпляр **AzureStorageLinkedService** отобразится в представлении в виде дерева слева.
...
1. Вставьте следующий фрагмент в окно Draft-1. В редакторе JSON выполните следующие действия.
    ...
    г) Укажите связанную службу хранилища, которая связана с кластером HDInsight Spark. (В этом примере это AzureStorageLinkedService.)
...
1. Замените скрипт в окне Draft-1 следующим скриптом:
    ...
    а. Свойству **type** присваивается значение **HDInsightSpark**.
    ...
    г) Свойству **getDebugInfo** присваивается значение **Always**. Так файлы журналов будут создаваться постоянно (успешные и неудачные события).
...
1. Щелкните **New** (Создать) > **PySpark**, чтобы создать записную книжку.
...
| Путь | Description (Описание) | Обязательно | Тип |
| ---- | ----------- | -------- | ---- |
