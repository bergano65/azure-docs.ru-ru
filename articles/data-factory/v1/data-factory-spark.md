...
> * [Свинья деятельность](data-factory-pig-activity.md)
> * [Действие MapReduce](data-factory-map-reduce.md)
> * [Действие потоковой передачи Hadoop](data-factory-hadoop-streaming-activity.md)
> * [Активность искры](data-factory-spark.md)
> * [Деятельность по выполнению пакета машинного обучения](data-factory-azure-ml-batch-execution-activity.md)
> * [Деятельность по обновлению ресурсов машинного обучения](data-factory-azure-ml-update-resource-activity.md)
> * [Сохраненная процедура деятельности](data-factory-stored-proc-activity.md)
> * [Деятельность Data Lake Analytics U-S'L](data-factory-usql-activity.md)
> * [пользовательская деятельность .NET](data-factory-use-custom-activities.md)
...
1. Скачайте и просмотрите файл сценария Python **test.py**, расположенный по адресу [https://adftutorialfiles.blob.core.windows.net/sparktutorial/test.py](https://adftutorialfiles.blob.core.windows.net/sparktutorial/test.py).

1. Отправьте файл **test.py** в папку **pyFiles** контейнера **adfspark** в хранилище BLOB-объектов. Создайте контейнер и папку, если их нет.
...
1. Выберите **новую** > **фабрику данных и** > **аналитики**.
...
1. Выберите имеющуюся группу ресурсов Azure или создайте новую.
...
1. Выберите **Создать**.
   ...
   > [!IMPORTANT]
   > Для создания экземпляров Data Factory необходимо быть участником роли [вкладчика Data Factory](../../role-based-access-control/built-in-roles.md#data-factory-contributor) на уровне группы подписок/ресурсов.

1. Созданная фабрика данных появится на панели мониторинга портала Azure.

1. После успешного создания фабрики данных ее содержимое отобразится на странице **Фабрика данных**. Если страница **Фабрика данных** не отображается, щелкните плитку вашей фабрики данных на панели мониторинга.
...
1. Скрипт JSON, используемый для создания связанной службы хранилища, отобразится в редакторе.

   ![AzureStorageLinkedService](./media/data-factory-build-your-first-pipeline-using-editor/azure-storage-linked-service.png)

1. Замените **имя учетной записи** и **ключ учетной записи** значениями имени и ключа учетной записи хранения. Сведения о том, как получить ключ доступа к хранилищу, см. в статье [Управление ключами доступа к хранилищу](../../storage/common/storage-account-keys-manage.md).
...
1. В редакторе Фабрики данных выберите **новый** > **вычислительный** > **кластер HDInsight.**
...
1. Чтобы развернуть связанную службу, выберите **Развертывание** на панели команд.
...
1. В редакторе фабрики данных выберите **больше** > **новых наборов** > данных,**наносимых Azure Blob.**

1. Вставьте следующий фрагмент в окно Draft-1. Этот фрагмент кода JSON определяет набор данных с именем **OutputDataset**. Кроме того, нужно указать, что результаты будут храниться в контейнере больших двоичных объектов **adfspark** и в папке **pyFiles/output**. Как упоминалось ранее, это фиктивный набор данных. В этом примере программа Spark не создает никаких выходных данных. В разделе **availability** указывается частота, с которой ежедневно будет создаваться выходной набор данных.
    ...
    ```
1. Чтобы развернуть набор данных, щелкните **Развертывание** на панели команд.
...
1. В редакторе Фабрики данных выберите **новый** > **конвейер.**

1. Замените скрипт в окне Draft-1 следующим скриптом:
...
1. Вверху укажите для фильтра **Start time** (Время начала) дату **01.02.2017** и щелкните **Применить**.

1. Отобразится только одно окно действия, так как между временем начала (01.02.2017) и окончания (02.02.2017) конвейера всего один день. Убедитесь, что срез данных находится в состоянии **Готово**.

    ![Мониторинг конвейера](media/data-factory-spark/monitor-and-manage-app.png)

1. Выберите выполнение действия в списке **Activity windows** (Окна действий), чтобы просмотреть сведения о нем. В случае ошибки в области справа будут отображаться детали.
...
1. Выберите **новый** > **PySpark,** чтобы запустить новый ноутбук.

    ![Новая записная книжка Jupyter](media/data-factory-spark/jupyter-new-book.png)

1. Выполните следующую команду, скопировав и вставив текст и нажав SHIFT+ENTER в конце второй инструкции.
    ...
    ```
1. Убедитесь, что вы видите данные из таблицы hvac.

    ![Результаты запроса Jupyter](media/data-factory-spark/jupyter-notebook-results.png)

<!-- Removed bookmark #run-a-hive-query-using-spark-sql since it doesn't exist in the target article -->
 См. подробные инструкции о [выполнении SQL-запроса Spark](../../hdinsight/spark/apache-spark-jupyter-spark-sql.md).
...
1. Щелкните **Logs** (Журналы) для одной из попыток выполнения.

    ![Страница приложения](media/data-factory-spark/yarn-applications.png)

1. Отобразятся дополнительные сведения об ошибке на странице журнала.
