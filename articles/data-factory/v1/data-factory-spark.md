...
1. Вставьте следующий фрагмент в окно Draft-1. В редакторе JSON выполните следующие действия.
    ...
    1. Укажите имя пользователя, имеющего доступ к кластеру Spark.

    1. Укажите пароль для пользователя.

    1. Укажите связанную службу хранилища, которая связана с кластером HDInsight Spark. (В этом примере это AzureStorageLinkedService.)
...
1. Замените скрипт в окне Draft-1 следующим скриптом:
    ...
    1. Для свойства **Type** задано значение **HDInsightSpark**.

    1. Свойству **rootPath** присваивается значение **adfspark\\pyFiles**, где adfspark — контейнер больших двоичных объектов, а pyFiles — папка с файлами в этом контейнере. В этом примере хранилище BLOB-объектов связано с кластером Spark. Файл можно отправить в другую учетную запись хранения. Чтобы сделать это, создайте связанную службу хранилища, которая свяжет эту учетную запись хранения с фабрикой данных. Затем укажите имя связанной службы в качестве значения для свойства **sparkJobLinkedService** . Сведения об этом и других свойствах, поддерживаемых действием Spark, см. в разделе [Свойства действия Spark](#spark-activity-properties).

    1. Свойству **entryFilePath** присваивается значение **test.py**, которое является файлом Python.

    1. Свойство **getDebugInfo** имеет значение **Always**, что означает, что файлы журнала создаются всегда (успешно или неудачно).

       > [!IMPORTANT]
       > Не рекомендуется устанавливать это свойство `Always` в рабочей среде, если только не устранена проблема.

    1. Раздел **Outputs** имеет один выходной набор данных. Вы должны определить выходной набор данных, даже если программа Spark не выдает выходные данные. На основе этого набора настраивается расписание конвейера (ежечасно, ежедневно).
