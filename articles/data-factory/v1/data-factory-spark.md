...
> * [Действие выполнения пакета Машинное обучение](data-factory-azure-ml-batch-execution-activity.md)
...
1. Выберите **Новая** > **данные и аналитика** > **фабрика данных**.
...
1. Нажмите кнопку **создания**.
   ...
   > [!IMPORTANT]
   > Для создания экземпляров фабрики данных необходимо быть членом роли " [участник фабрики данных](../../role-based-access-control/built-in-roles.md#data-factory-contributor) " на уровне подписки или группы ресурсов.
...
1. В редакторе фабрики данных выберите **больше** > **новых вычислений** > **кластер HDInsight**.
...
1. В редакторе фабрики данных выберите **больше** > **нового набора данных** > **хранилище BLOB-объектов Azure**.
...
1. В редакторе фабрики данных выберите **больше** > **нового конвейера**.
...
1. Замените скрипт в окне Draft-1 следующим скриптом:
    ...
    а. Для свойства **Type** задано значение **HDInsightSpark**.
    ...
    d. Свойство **getDebugInfo** имеет значение **Always**, что означает, что файлы журнала создаются всегда (успешно или неудачно).

    > [!IMPORTANT]
    > Пока вы не устраните неполадки, советуем не устанавливать для этого свойства значение `Always` в рабочей среде.

    д) Раздел **Outputs** имеет один выходной набор данных. Вы должны определить выходной набор данных, даже если программа Spark не выдает выходные данные. На основе этого набора настраивается расписание конвейера (ежечасно, ежедневно).
...
1. Запустите Jupyter Notebook для кластера HDInsight Spark, перейдя по `https://CLUSTERNAME.azurehdinsight.net/jupyter`. Можно также сначала открыть панель мониторинга для кластера HDInsight Spark, а затем запустить записную книжку Jupyter.

1. Выберите **создать** > **PySpark** , чтобы начать новую записную книжку.
