---
title: Функции фабрики данных и системные переменные
...
1. Указание запросов выбора данных (см. статьи о соединителях, на которые ссылается статья [Действия перемещения данных](data-factory-data-movement-activities.md)).
   ...
   Синтаксис для использования функции фабрики ** $$ \<** данных: функция>для запросов выбора данных и других свойств в действиях и наборах данных.  
...
| Time |AddMinutes(X,Y) |X: DateTime  <br/><br/>Y: int |Добавляет Y минут к Х.<br/><br/>Например, `9/15/2013 12: 00:00 PM + 15 minutes = 9/15/2013 12: 15:00 PM`. |
| Time |StartOfHour(X) |X: DateTime  |Получает то время, когда начинался час, отображаемый компонентом часа в параметре X. <br/><br/>Например, `StartOfHour of 9/15/2013 05: 10:23 PM is 9/15/2013 05: 00:00 PM`. |
...
| Дата |DayOfWeek(X) |X: DateTime  |Возвращает значение дня недели для X.<br/><br/>Например, `DayOfWeek of 9/15/2013 12:00:00 PM is Sunday`. |
...
| Дата |DaysInMonth(X) |X: DateTime  |Получает количество дней в месяце, представленном значением месяца параметра X.<br/><br/>Например, `DaysInMonth of 9/15/2013 are 30 since there are 30 days in the September month`. |
| Дата |EndOfDay(X) |X: DateTime  |Получает значение даты и времени, которое соответствует окончанию суток (значения дня) в параметре X.<br/><br/>Например, `EndOfDay of 9/15/2013 05:10:23 PM is 9/15/2013 11:59:59 PM`. |
...
| Дата |StartOfDay(X) |X: DateTime  |Получает начало суток, представленных значением дня в параметре X.<br/><br/>Например, `StartOfDay of 9/15/2013 05:10:23 PM is 9/15/2013 12:00:00 AM`. |
...
| Text |Format(X) |X: String (переменная) |Форматирует текст (используйте комбинацию `\\'` для экранирования символа `'`).|

> [!IMPORTANT]
> При использовании функции в другой функции, вам не нужно использовать **$$** префикс для внутренней функции. Например, $$Text.Format('PartitionKey eq \\'my_pkey_filter_value\\' and RowKey ge \\'{0:yyyy-MM-dd HH:mm:ss}\\'', Time.AddHours(SliceStart, -6)). В этом примере **$$** обратите внимание, что префикс не используется для функции **Time.AddHours.** 
