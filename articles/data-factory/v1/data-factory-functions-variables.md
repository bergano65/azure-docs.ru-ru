...
| --- | --- | --- | --- |
| WindowStart |Начало интервала времени для текущего окна запуска действия. |операция |<ol><li>Укажите запросы на выбор данных. Ознакомьтесь со статьями о соединителях, упомянутых в статье [Действия перемещения данных](data-factory-data-movement-activities.md).</li> |
| WindowEnd |Конец интервала времени для текущего окна запуска действия. |действие |то же, что и WindowStart. |
| SliceStart |Начало интервала времени для формируемого среза данных. |операция<br/>dataset |<ol><li>Укажите динамические пути к папкам и имена файлов при работе с [большими двоичными объектами Azure](data-factory-azure-blob-connector.md) и [наборами данных файловой системы](data-factory-onprem-file-system-connector.md).</li><li>Укажите входные зависимости с помощью функций фабрики данных в коллекции входных данных действия.</li></ol> |
| SliceEnd |Конец интервала времени для текущего среза данных. |операция<br/>dataset |То же, что и для SliceStart. |
...
| Time |AddMinutes(X,Y) |X: DateTime <br/><br/>Y: int |Добавляет Y минут к Х.<br/><br/>Пример: `9/15/2013 12: 00:00 PM + 15 minutes = 9/15/2013 12: 15:00 PM` |
| Time |StartOfHour(X) |X: DateTime |Получает то время, когда начинался час, отображаемый компонентом часа в параметре X. <br/><br/>Пример: `StartOfHour of 9/15/2013 05: 10:23 PM is 9/15/2013 05: 00:00 PM` |
...
| DateTime |From(X) |X: Строковое |Разбор строки X в значение даты и времени. |
...
| Текст |Format(X) |X: Строковая переменная |Форматирует текст (используйте комбинацию `\\'` для экранирования символа `'`).|
