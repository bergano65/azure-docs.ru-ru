...
Раздел **typeProperties** отличается для каждого типа набора данных и предоставляет информацию о местоположении данных в хранилище данных. Раздел typeProperties для набора данных типа **RelationalTable** содержит следующие свойства.
...
* **Образец СОЗЛ**:`$$Text.Format('SELECT Id, Name, BillingCity FROM Account WHERE LastModifiedDate >= {0:yyyy-MM-ddTHH:mm:ssZ} AND LastModifiedDate < {1:yyyy-MM-ddTHH:mm:ssZ}', WindowStart, WindowEnd)`
* **Образец СЗЛ**:
...
* Чтобы задать запрос только удаленные записи, укажите "выберите из **MyTable__c, где IsDeleted" 1**"
* Чтобы задать запрос на все записи, включая существующие и удаленные, укажите "выберите из **MyTable__c, где IsDeleted - 0 или IsDeleted No 1**"
...
* [Конвейер](data-factory-create-pipelines.md) с копированием деятельности, который использует [РелиляционныйИсточник](#copy-activity-properties) и [BlobSink](data-factory-azure-blob-connector.md#copy-activity-properties)
