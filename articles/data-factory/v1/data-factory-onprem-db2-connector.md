---
title: Перемещение данных из DB2 с помощью Azure Data Factory
...
- Для создания конвейера также можно использовать инструменты, включая Visual Studio, Azure PowerShell, шаблон менеджера ресурсов Azure, API .NET и API REST. Для пошаговых инструкций по созданию конвейера с копированием, [см.](data-factory-copy-data-from-azure-blob-storage-to-sql-database.md) 
...
Если вы используете мастер копирования, то определения JSON для сущностей фабрики данных (связанных служб, наборов данных и конвейера) создаются автоматически. При использовании инструментов или интерфейсов API (за исключением API .NET) вы самостоятельно определяете сущности фабрики данных в формате JSON. В разделе Пример JSON. Копирование данных из DB2 в хранилище BLOB-объектов Azure показаны определения JSON для сущностей фабрики данных, которые используются для копирования данных из локального хранилища данных DB2.
...
| --- | --- | --- |
| **тип** |Этому свойству необходимо задать значение **OnPremisesDB2**. |Да |
| **Сервера** |Имя сервера DB2. |Да |
| **Базы данных** |Имя базы данных DB2. |Да |
| **Схемы** |Имя схемы в базе данных DB2. Это свойство чувствительно к регистру. |нет |
| **аутентификацияType** |Тип аутентификации, используемый для подключения к базе данных DB2. Возможные значения: "Анонимная", "Обычная" и "Windows". |Да |
| **Пользователя** |Имя для учетной записи пользователя, если используемая аутентификация имеет тип "Обычная" или "Windows". |нет |
| **пароль** |Пароль для учетной записи пользователя. |нет |
| **шлюзИмя** |Имя шлюза, который следует использовать службе фабрики данных для подключения к локальной базе данных DB2. |Да |
...
Раздел **typeProperties** отличается для каждого типа набора данных и предоставляет информацию о местоположении данных в хранилище данных. Раздел **typeProperties** набора данных с типом **RelationalTable**, который включает набор данных DB2, имеет следующее свойство:
...
| --- | --- | --- | --- |
| **Запроса** |Используйте пользовательский запрос для чтения данных. |Строка запроса SQL. Например: `"query": "select * from "MySchema"."MyTable""` |Нет (если для свойства **tableName** задано значение dataset). |
...
- Набор входных [данных](data-factory-create-datasets.md) типа [RelationalTable](data-factory-onprem-db2-connector.md#dataset-properties)
...
Сначала установите и настройте шлюз данных. Инструкции находятся в [перемещение данных между местами расположения и облачной](data-factory-move-data-between-onprem-and-cloud.md) статьей.
