---
title: Нажмите на данные в индекс поиска с помощью фабрики данных
description: Узнайте о том, как перенайдить данные в индекс когнитивного поиска Azure с помощью Azure Data Factory.
...
ms.locfileid: "79281565"
---
# <a name="push-data-to-an-azure-cognitive-search-index-by-using-azure-data-factory"></a>Нажмите данные на индекс когнитивного поиска Azure с помощью Azure Data Factory
> [!div class="op_single_selector" title1="Выберите используемую версию службы "Фабрика данных":"]
> * [Версия 1](data-factory-azure-search-connector.md)
> * [Версия 2 (текущая)](../connector-azure-search.md)

> [!NOTE]
> В этой статье рассматривается служба "Фабрика данных Azure" версии 1. Если вы используете текущую версию службы Data Factory, см. [разъем Azure Cognitive Search в V2.](../connector-azure-search.md)

В этой статье описывается, как использовать активность копирования для передачи данных из поддерживаемого хранилища исходных данных в индекс когнитивного поиска Azure. Поддерживаемые исходные хранилища данных перечислены в столбце "Источник" таблицы [поддерживаемых источников и приемников](data-factory-data-movement-activities.md#supported-data-stores-and-formats). Эта статья основывается на [статье,](data-factory-data-movement-activities.md) в которой представлены общие обзоры движения данных с комбинациями копировальных и поддерживаемых комбинаций хранилища данных.
...
Можно создать конвейер с активностью копирования, которая передвигает данные из хранилища исходных данных в индекс поиска с помощью различных инструментов/AIS.
...
Если вы используете мастер, то он автоматически создает определения JSON для сущностей фабрики данных (связанных служб, наборов данных и конвейера). При использовании инструментов и интерфейсов API (за исключением API .NET) вы самостоятельно определяете эти сущности фабрики данных в формате JSON.  Для примера с определениями JSON для объектов Data Factory, которые используются для копирования данных для индекса поиска, [см. пример JSON: Копирование данных с внутреннего сервера S'L В разделе индекса когнитивного поиска Azure](#json-example-copy-data-from-on-premises-sql-server-to-azure-cognitive-search-index) в этой статье.

В следующих разделах приведены сведения о свойствах JSON, которые используются для определения сущностей Data Factory, специфичных для индекса поиска:

## <a name="linked-service-properties"></a>Свойства связанной службы

В следующей таблице приведены описания элементов JSON, специфичными для службы связанного с Azure Cognitive Search.

| Свойство | Описание | Обязательно |
| -------- | ----------- | -------- |
| type | Свойство типа должно быть установлено в **AzureSearch**. | Да |
| url | URL для поисковой службы. | Да |
| ключ | Ключ от админ для поисковой службы. | Да |

## <a name="dataset-properties"></a>Свойства набора данных

Полный список разделов и свойств, используемых для определения наборов данных, см. в статье [Наборы данных в фабрике данных Azure](data-factory-create-datasets.md). Разделы структуры, доступности и политики JSON набора данных одинаковы для всех типов наборов данных. Раздел **typeProperties** отличается для каждого типа набора данных. Раздел typeProperties для набора данных типа **AzureSearchIndex** содержит следующие свойства.
...
| WriteBatchSize | Загружает данные в индекс поиска, когда размер буфера достигает writeBatchSize. Ознакомьтесь с разделом [Свойство WriteBatchSize](#writebatchsize-property). | 1–1000. Значение по умолчанию — 1000. | нет |
...
## <a name="json-example-copy-data-from-on-premises-sql-server-to-azure-cognitive-search-index"></a>Пример JSON: Копирование данных с внутреннего сервера S'L на индекс когнитивного поиска Azure
...
Образец копирует данные временных рядов из предварительной базы данных S'L Server для поиска индекса ежечасно. Используемые в этом примере свойства JSON описаны в разделах, следующих за примерами.
...
**Служба когнитивного поиска Azure:**
...
**Набор данных вывода когнитивного поиска Azure:**

Образец копирует данные в индекс когнитивного поиска Azure, названный **продуктами.** Фабрика данных не создает индекс. Чтобы проверить пример, создайте индекс с таким именем. Создайте индекс поиска с тем же количеством столбцов, что и в наборе данных ввода. Новые записи добавляются в индекс поиска каждый час.
...
**Копирование активности в конвейере с исходным источником S'L и поглотителем azure Cognitive Search Index:**
