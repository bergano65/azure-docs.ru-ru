---
title: Перемещение данных из источника HTTP - Azure
...
| password | Пароль пользователя (**username**). | Да |

**Пример: Использование базовой, дайджест, или Windows аутентификации**
...
| Свойство | Описание | Обязательно |
| --- | --- | --- |
| embeddedCertData | Содержимое двоичных данных PFX-файла с кодировкой Base64. | Укажите либо **встроенныеCertData,** либо **certThumbprint** |
| certThumbprint | Отпечаток сертификата, который был установлен в хранилище сертификатов на компьютере шлюза. Применимо, только когда копирование данных выполняется из локального источника HTTP. | Укажите либо **встроенныеCertData,** либо **certThumbprint** |
...
2. Расширить **сертификаты** > **Личные,** а затем выбрать **Сертификаты**.
3. Нажмите справа на сертификат из личного магазина, а затем выберите **все задачи** >**Управления частными ключами.**
...
Раздел **typeProperties** отличается для каждого типа набора данных. Раздел **typeProperties** содержит сведения о расположении данных в хранилище. Раздел **typeProperties** набора данных типа **Http** содержит перечисленные ниже свойства:
...
В следующих примерах приведены примеры определений JSON, которые можно использовать для создания конвейера с помощью [Visual Studio](data-factory-copy-activity-tutorial-using-visual-studio.md) или [Azure PowerShell.](data-factory-copy-activity-tutorial-using-powershell.md) Это пример процесса копирования данных из источника HTTP в хранилище BLOB-объектов Azure. Тем не менее данные можно копировать *непосредственно* из любых источников в любой [поддерживаемый](data-factory-data-movement-activities.md#supported-data-stores-and-formats) приемник используя действие копирования в фабрике данных Azure.
...
Данные записываются в новую каплю каждый час **(частота**: **час,** **интервал**: **1**).
