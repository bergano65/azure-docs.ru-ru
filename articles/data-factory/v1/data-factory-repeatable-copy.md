---
title: Повторяющаяся операция копирования в фабрике данных Azure
...
Теперь допустим, что в исходном файле вы нашли ошибку и изменили значение параметра Down Tube (изменив 2 на 4). Если вы повторно перезапустите срез данных для этого периода вручную, вы найдете две новые записи, добавленные в базу данных Azure SQL или SQL Server. В этом примере предполагается, что ни один из столбцов таблицы не содержит ограничений первичного ключа.
...
> [!IMPORTANT]
> В настоящее время sliceIdentifierColumnName не поддерживается для Azure синапсе Analytics. 
