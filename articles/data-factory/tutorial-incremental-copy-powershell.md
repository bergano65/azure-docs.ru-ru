...
1. Создайте JSON-файл с именем AzureStorageLinkedService.json в папке C:\ADF со следующим содержимым. (Создайте папку ADF, если она еще не существует.) Перед сохранением файла замените `<accountName>` и `<accountKey>` именем и ключом вашей учетной записи хранения.
...
1. Создайте в папке C:\ADF файл JSON с именем AzureSQLDatabaseLinkedService.json со следующим содержимым. (Создайте папку ADF, если она еще не существует.) Перед сохранением файла замените &lt;Server&gt;,&gt;базы данных &lt;, &lt;идентификатор пользователя&gt;и &lt;пароля&gt; именем сервера, базы данных, идентификатора пользователя и пароля.
