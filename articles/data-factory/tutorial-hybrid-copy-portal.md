...
1. На панели слева щелкните **Больше служб**. Отфильтруйте содержимое по ключевому слову **хранение**, а затем выберите **Учетные записи хранения**.
...
1. На вкладке **Общие** внизу окна **свойств** в поле **Имя** введите **SQLServerToBlobPipeline**.

   ![Имя конвейера](./media/tutorial-hybrid-copy-portal/pipeline-name.png)

1. На панели инструментов **Действия** разверните узел **Data Flow** (Поток данных). Перетащите действие **копирования** на рабочую область конструирования конвейера. Задайте этому действию имя **CopySqlServerToAzureBlobActivity**.
...
1. В окне **Новый набор данных** найдите **SQL Server**. Выберите **SQL Server**, а затем нажмите кнопку **Готово**. Появится новая вкладка с именем **SqlServerTable1**. Кроме того, набор данных **SqlServerTable1** появится слева в представлении в виде дерева. 

   ![Выбор SQL Server](./media/tutorial-hybrid-copy-portal/select-sql-server.png)

1. На вкладке **Общие** внизу окна **свойств** в поле **Имя** введите **SqlServerDataset**.

   ![Имя исходного набора данных](./media/tutorial-hybrid-copy-portal/source-dataset-name.png)

1. Перейдите на вкладку **Подключение** и выберите **+ Создать**. На этом этапе вы создадите подключение к исходному хранилищу данных (база данных SQL Server). 
...
1. В окне **Integration Runtime Setup** (Настройка среды выполнения интеграции) выберите **Частная сеть**, а затем нажмите кнопку **Далее**. 

   ![Выбор частной сети](./media/tutorial-hybrid-copy-portal/select-private-network.png)

1. Укажите имя среды выполнения интеграции и нажмите кнопку **Далее**.
...
1. В окне **New Linked Service** (Новая связанная служба) убедитесь, что в разделе **Connect via integration runtime** (Подключение через среду выполнения интеграции) выбрана созданная выше **среда выполнения интеграции**. 
...
1. В окне **New Linked Service** (Новая связанная служба) выполните следующие действия:
    ...
    b. Необходимо убедиться, что в области **Connect via integration runtime** (Подключиться при помощи среды выполнения интеграции) отображается созданная ранее локальная среда выполнения интеграции.
    ...
    Е. В полях **имени пользователя** и **пароля** введите имя пользователя и пароль. Если в имени учетной записи пользователя или имени сервера необходимо использовать символ обратной косой черты (\\), добавьте escape-символ (\\). Например, *mydomain\\\\myuser*.
    ...
    h. Чтобы сохранить связанную службу, щелкните **Готово**.
...
1. В окне **Новый набор данных** выберите **Хранилище BLOB-объектов Azure**. Затем нажмите кнопку **Готово**. Откроется новая вкладка для этого набора данных. Кроме того, набор данных появится в представлении в виде дерева. 

    ![Выбор хранилища BLOB-объектов](./media/tutorial-hybrid-copy-portal/select-azure-blob-storage.png)

1. В поле **Имя** введите **AzureBlobDataset**.

    ![Целевой набор данных — имя](./media/tutorial-hybrid-copy-portal/sink-dataset-name.png)

1. Перейдите на вкладку **Подключение** в нижней части окна **Свойства**. Рядом с полем **Связанная служба** выберите **+ Создать**. 
...
1. В окне **New Linked Service** (Новая связанная служба) выполните следующие действия:
    ...
    b. В поле **Имя учетной записи хранения** выберите используемую учетную запись хранения.
...
1. Дождитесь всплывающего окна с сообщением **Публикация выполнена**. Чтобы проверить состояние публикации, выберите ссылку **Показать уведомления** слева. Чтобы закрыть окно уведомлений, выберите **Закрыть**. 
...
![Проверка имени выходного файла](./media/tutorial-hybrid-copy-portal/sink-output.png)
