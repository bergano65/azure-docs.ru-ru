...
В этом руководстве используйте учетную запись хранения Azure общего назначения (хранилища BLOB-объектов) в качестве целевого (принимающего) хранилища данных. Если у вас нет учетной записи хранения Azure общего назначения, см. инструкции по [созданию учетной записи хранения](../storage/common/storage-account-create.md). Конвейер фабрики данных, который был создан в рамках этого руководства, копирует данные из локальной базы данных SQL Server (источника) в хранилище BLOB-объектов (приемник). 
...
1. В поле "Имя" введите **TutorialIntegrationRuntime**. Выберите **Далее**.
...
1. Убедитесь, что в диалоговом окне **New Linked Service** (Новая связанная служба) для поля **Connect via integration runtime** (Подключение через среду выполнения интеграции) выбран вариант **TutorialIntegrationRuntime**. Затем выполните следующие действия:
    ...
    б. В поле **Имя сервера**введите имя экземпляра SQL Server.

    в. В поле **Имя базы данных** введите имя базы данных, которая содержит таблицу **emp**.

    . В поле **Тип проверки подлинности**, выберите нужный тип аутентификации, который будет использоваться в фабрике данных для подключения к базе данных SQL Server.
...
1. Теперь снова отобразится окно с целевым набором данных. На вкладке **Подключение** сделайте следующее:
    ...
    б. В поле **Путь к файлу** введите значение **adftutorial/fromonprem** для частей **Container/ Directory** (Контейнер/Каталог). Если указанной папки выходных данных не существует в контейнере adftutorial, она будет автоматически создана в фабрике данных.

    в. Для части **Файл** выберите **Добавить динамическое содержимое**.
    ![динамическое выражение для разрешения имени файла](./media/tutorial-hybrid-copy-portal/file-name.png)

    . Добавьте `@CONCAT(pipeline().RunId, '.txt')` и нажмите кнопку **Готово**. Файл будет переименован в PipelineRunID.txt.
...
1. Перейдите на вкладку **мониторинг** . Вы увидите конвейер, который вы вручную активировали на предыдущем шаге.
