...
Выбирает значение столбца по его относительное положение (на основе 1) в потоке. Возвращается значение NULL, если позиция находится вне допустимых границ. Возвращаемое значение имеет тип конвертации, на один из функции преобразования типов (TO_DATE, TO_STRING...) Вычисляемый входных данных не поддерживаются, но вы можете использовать подстановки параметров
...
Преобразует отметка времени относительно времени UTC. При необходимости можно передать в форме «GMT», «PST», «UTC», «Америка/Каймановы» часовой пояс. По умолчанию для текущего часового пояса
...
Преобразует любое числовое или строковое значение в целое. Необязательный десятичный формат Java можно использовать для преобразования. Усекает любого типа long, float, double
* ``toInteger(123) -> 123``
* ``toInteger('123') -> 123``
* ``toInteger('$123', '$###') -> 123``
*********************************
...
Нормализовать строковое значение для разделения символов с диакритическими знаками Юникода
* ``normalize('boys') -> 'boys'``
*********************************
<code>not</code>
==============================
