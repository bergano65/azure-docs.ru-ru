...
Например, в `allowSchemaDrift: true,` преобразовании «источник» указывает службе включать все столбцы из исходного набора данных в потоке данных, даже если они не включены в проекцию схемы.
...
Это универсальный шаблон статистического выражения, демонстрирующий, как можно обеспечить возможность сохранения оставшихся столбцов в выходных метаданных при построении статистических выражений. В этом случае мы используем ```first()``` функцию, чтобы выбрать первое значение в каждом столбце, имя которого не является «Movie». Чтобы использовать эту функцию, создайте преобразование «Статистическая обработка» с именем Дистинктровс и вставьте его в скрипт поверх существующего статистического сценария Дистинктровс.
...
Используйте этот код в скрипте потока данных для создания нового производного столбца с ```DWhash``` именем, создающего ```sha1``` хэш трех столбцов.
