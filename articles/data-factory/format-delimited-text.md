...
| rowDelimiter     | Отдельный символ или "\r\n", используемый для разделения строк в файле.<br>Значение по умолчанию — любое из следующих значений **при чтении: ["\r\n", "\r", "\n"]** и **"\n" или "\r\n" при записи** с сопоставлением потока данных и действия копирования соответственно. <br>Если `rowDelimiter` имеет значение без разделителя (пустая строка), `columnDelimiter` должен быть задан как не разделитель (пустая строка), а это означает, что все содержимое будет рассматриваться как одно значение. | Нет       |
