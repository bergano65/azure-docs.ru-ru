...
Фабрика данных может иметь один или несколько конвейеров. Конвейеры — это логические группы действий, которые вместе отвечают за выполнение задачи. Например, конвейер может содержать набор действий, принимающих и очищающих данные журнала, а затем запускающий поток данных сопоставления для анализа данных журнала. Конвейер позволяет управлять действиями как набором, а не каждым из них по отдельности. Вы развертываете и планируете конвейер, а не действия отдельно.
...
[Filter](control-flow-filter-activity.md) | Применение критерия фильтра к входному массиву
...
Тег | ОПИСАНИЕ | введите | обязательные
--- | ----------- | ---- | --------
Имя | Имя конвейера. Укажите имя, представляющее действие, которое выполняет конвейер. <br/><ul><li>Максимальное количество знаков: 140</li><li>Должно начинаться с буквы, цифры или символа подчеркивания (\_).</li><li>Следующие знаки не допускаются: ".", "+", "?", "/", "<", ">", "*", "%", "&", ":", "\".</li></ul> | Строка, | Yes
...
concurrency | Максимальное количество параллельных запусков конвейера. По умолчанию максимальное значение отсутствует. Если достигнут предел параллелизма, дополнительные запуски конвейера будут поставлены в очередь до завершения предыдущих версий. | Число | Нет 
...
Тег | ОПИСАНИЕ | обязательные
--- | ----------- | ---------
Имя | Имя действия. Укажите имя, представляющее операцию, которую выполняет действие. <br/><ul><li>Максимальное количество знаков: 55</li><li>Должно начинаться с буквы, цифры или символа подчеркивания (\_).</li><li>Следующие знаки не допускаются: ".", "+", "?", "/", "<", ">", "*", "%", "&", ":", "\". | Yes</li></ul>
Описание | Текст, описывающий, для чего используется действие | Yes
type | Тип действия. Разные типы действий описаны в разделах, посвященных [действиям перемещения данных](#data-movement-activities), [действиям преобразования данных](#data-transformation-activities) и [действиям управления](#control-flow-activities). | Yes
linkedServiceName (имя связанной службы) | Имя связанной службы, используемой действием.<br/><br/>Для действия может потребоваться указать службу, связанную с обязательной вычислительной средой. | Да, для действия HDInsight, действия пакетной оценки показателей службы "Машинное обучение Azure" и действия хранимой процедуры. <br/><br/>Нет — для всех остальных
...
Имя JSON | ОПИСАНИЕ | Допустимые значения | обязательные
--------- | ----------- | -------------- | --------
...
secureOutput | Если задано значение true, выходные данные действия будут рассматриваться как безопасные. Они не будут регистрироваться для мониторинга. | Логическое значение. | Нет. Значение по умолчанию — false.
...
### <a name="activity-dependency"></a>зависимость действия;
...
Например, предположим, что у вас есть триггер планировщика, «активировать A», который я хочу запустить в моем конвейере «Микопипипелине». Определите триггер, как показано в следующем примере:
