...
--- | ----------- | ---- | --------
name | Имя конвейера. Укажите имя, представляющее действие, которое выполняет конвейер. <br/><ul><li>Максимальное количество знаков: 140</li><li>Должно начинаться с буквы, цифры или символа подчеркивания (\_).</li><li>Следующие знаки не допускаются: ".", "+", "?", "/", "<", ">", "*", "%", "&", ":", "\".</li></ul> | Строка | Yes
...
--- | ----------- | ---------
name | Имя действия. Укажите имя, представляющее операцию, которую выполняет действие. <br/><ul><li>Максимальное количество знаков: 55</li><li>Должно начинаться с буквы, цифры или символа подчеркивания (\_).</li><li>Следующие знаки не допускаются: ".", "+", "?", "/", "<", ">", "*", "%", "&", ":", "\". | Yes</li></ul>
...
--------- | ----------- | -------------- | --------
timeout | Указывает время ожидания для выполнения действия. | Timespan | № Время ожидания по умолчанию — 7 дней.
retry | Максимальное количество повторных попыток | Целое число  | № Значение по умолчанию — 0.
retryIntervalInSeconds | Задержка между повторными попытками (в секундах) | Целое число  | № По умолчанию это 20 секунд.
secureOutput | Если задано значение true, выходные данные действия будут рассматриваться как безопасные. Они не будут регистрироваться для мониторинга. | Логическое | № Значение по умолчанию — false.
