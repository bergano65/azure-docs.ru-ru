...
### <a name="error-messagessis-executor-exit-code--1073741819"></a>Сообщение об ошибке: "Исполнитель SSIS код выхода: -1073741819.»

* Возможной причиной & Рекомендуемое действие:
  * Эта ошибка может быть из-за ограничений для источником и назначением Excel, когда несколько Excel источников и назначений выполняются в параллельном режиме в многопоточную. Можно обойти, это ограничение, изменить компоненты Excel для последовательного выполнения, или разделить их на разные пакеты и триггера через «Execute Package Task» со свойством ExecuteOutOfProcess, имеющим значение true.
...
### <a name="error-message-failed-to-retrieve-resource-from-master-microsoftsqlserverintegrationservicesscalescaleoutcontractcommonmasterresponsefailedexception-code300004-descriptionload-file--failed"></a>Сообщение об ошибке: «Не удалось получить ресурс из главной. Microsoft.SqlServer.IntegrationServices.Scale.ScaleoutContract.Common.MasterResponseFailedException: Код: 300004. Описание: загрузить файл «***» не удалось.»

* Возможной причиной & Рекомендуемое действие:
  * Если действие SSIS выполняет пакет из файловой системы (файл пакета или файла проекта), эта ошибка возникает, если файл проекта, пакета или конфигурации недоступен с учетными данными доступа пакета введен в действие SSIS
    * Если вы используете файлов Azure:
      * Путь к файлу должен начинаться с \\ \\ \<имя учетной записи хранения\>. file.core.windows.net\\\<путь к общей папке\>
      * Имя домена должно быть «Azure»
      * Имя пользователя должно быть \<имя учетной записи хранения\>
      * Пароль должен быть \<ключ доступа к хранилищу\>
    * Если выполняется с помощью файла на месте, проверьте, если виртуальная сеть, учетные данные доступа пакета и разрешения настроены надлежащим образом среды выполнения интеграции Azure SSIS можно получить доступ к локальной общей папке

### <a name="error-message-the-file-name--specified-in-the-connection-was-not-valid"></a>Сообщение об ошибке: «Имя файла «...» указанный в подключение недопустимо»

* Возможной причиной & Рекомендуемое действие:
  * Указано недопустимое имя файла
  * Убедитесь, что вы используете полное доменное имя (полное доменное имя) вместо короткое время в диспетчер соединений
