...
- **Решение**. Добавьте преобразование SELECT после объединения и выберите "удалить дублирующиеся столбцы" как для входных, так и для выходных данных.

### <a name="error-message-possible-cartesian-product"></a>Сообщение об ошибке: возможная декартово Продукция

- **Симптомы**: преобразование "присоединение" или "Уточняющий запрос" обнаружило возможный декартый продукт при выполнении потока данных

- **Причина**. Если вы не настроили ADF явным образом для использования перекрестного объединения, поток данных может завершиться ошибкой

- **Решение**. измените преобразование «Уточняющий запрос» или «соединение» на соединение с помощью пользовательского перекрестного объединения и введите условие уточняющего запроса или объединения в редакторе выражений. Если вы хотите явно создать полное декартово произведение, используйте преобразование «Производный столбец» в каждом из двух независимых потоков перед соединением, чтобы создать искусственный ключ для сопоставления. Например, создайте новый столбец в производном столбце в каждом потоке с именем ```SyntheticKey``` и установите его равным ```1```. Затем используйте ```a.SyntheticKey == b.SyntheticKey``` в качестве пользовательского выражения JOIN.

> [!NOTE]
> Не забудьте включить по крайней мере один столбец с каждой стороны левой и правой связей в пользовательском перекрестном соединении. Выполнение перекрестных соединений со статическими значениями вместо столбцов с каждой стороны приведет к полному сканированию всего набора данных, что приведет к плохому выполнению потока данных.
