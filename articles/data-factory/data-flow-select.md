...
Если в имени столбца имеется несколько совпадений регулярного выражения, вы можете ссылаться на конкретные совпадения `$n` , используя Where "n", к которому относится совпадение. Например, "$2" ссылается на второе совпадение в имени столбца.
...
Имена столбцов можно параметризовать с помощью сопоставления на основе правил. Используйте ключевое ```name``` слово для сопоставления имен входящих столбцов с параметром. Например, если имеется параметр ```mycolumn```потока данных, можно создать правило, которое будет соответствовать любому имени столбца, равному. ```mycolumn``` Можно переименовать сопоставленный столбец в жестко заданную строку, такую как "бизнес-ключ", и ссылаться на нее явным образом. В этом примере условие сопоставления имеет ```name == $mycolumn``` значение, а условием имени является "бизнес-ключ". 
