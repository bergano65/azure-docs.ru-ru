...
> [!IMPORTANT]
> При копировании данных с помощью локальной среды выполнения интеграции Настройте корпоративный брандмауэр, разрешающий исходящий трафик к `<ADLS account name>.azuredatalakestore.net` порту 443 `login.microsoftonline.com/<tenant>/oauth2/token` и через него. Последняя – это служба токенов безопасности Azure, с которой должна взаимодействовать среда выполнения интеграции для получения маркера доступа.
...
Для Azure Data Lake Store Gen1 в `location` параметрах набора данных на основе формата поддерживаются следующие свойства:
...
Для Azure Data Lake Store Gen1 в `storeSettings` параметрах в источнике копирования на основе формата поддерживаются следующие свойства:
...
Для Azure Data Lake Store Gen1 в `storeSettings` параметрах в приемнике копирования на основе формата поддерживаются следующие свойства:
...
| Да |mergeFiles | Папка1<br/>&nbsp;&nbsp;&nbsp;&nbsp;Файл1<br/>&nbsp;&nbsp;&nbsp;&nbsp;Файл2<br/>&nbsp;&nbsp;&nbsp;&nbsp;Вложенная_папка1<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Файл3<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Файл4<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Файл5 | Целевая папка "Папка1" создается со следующей структурой: <br/><br/>Папка1<br/>&nbsp;&nbsp;&nbsp;&nbsp;Содержимое file1 + file2 + Файл3 + "Файл4" + "Файл5" объединяется в один файл с автоматически сформированным именем файла. |
...
| Свойство | Описание | Обязательный |
|:--- |:--- |:--- |
| type | `type` Свойство источника действия копирования должно иметь значение **AzureDataLakeStoreSource**. |Да |
...
| Свойство | Описание | Обязательный |
|:--- |:--- |:--- |
| type | `type` Свойство приемника действия копирования должно иметь значение **AzureDataLakeStoreSink**. |Да |
