...
| ---- | ----------- | -------- | -------------- | ---------------- |
| Магазин | Хранилище должно быть `hive` | yes |  `hive` | store | 
| Формат | Выполняется ли чтение из таблицы или запроса | yes | `table` или `query` | format |
...
| Промежуточное | Таблица Hive всегда будет промежуточной. | yes | `true` | промежуточное |
| Контейнер хранилища | Контейнер хранилища, используемый для размещения данных перед чтением из Hive или записи в Hive. Кластер Hive должен иметь доступ к этому контейнеру. | yes | Строка | storageContainer |
