...
Чтобы скопировать данные из облачного хранилища Google, необходимо предоставить соответствующие разрешения. Разрешения, определенные в учетной записи службы, могут содержать `storage.buckets.get` , `storage.buckets.list` или `storage.objects.get` для операций объекта.
