...
В этой статье описано, как копировать данные из Google Cloud Storage. Дополнительные сведения о Фабрике данных Azure см. во [вводной статье](introduction.md).

## <a name="supported-capabilities"></a>Поддерживаемые возможности

Этот соединитель облачного хранилища Google поддерживается для следующих действий:
...
В учетной записи хранения Google Cloud требуется следующая настройка:

1. Обеспечение взаимодействия для учетной записи хранения Google Cloud
2. Укажите проект по умолчанию, содержащий данные, которые необходимо скопировать.
3. Создайте ключ доступа.

![Получение ключа доступа для облачного хранилища Google](media/connector-google-cloud-storage/google-storage-cloud-settings.png)

## <a name="required-permissions"></a>Необходимые разрешения

Чтобы скопировать данные из Google Cloud Storage, убедитесь, что вы предоставили следующие разрешения:

- **Для выполнения действия копирования:**: `s3:GetObject` и `s3:GetObjectVersion` для операций с объектами.
- **Для создания графического пользовательского интерфейса фабрики данных**: `s3:ListAllMyBuckets` и для операций с `s3:ListBucket` / `s3:GetBucketLocation` контейнерами дополнительно требуются для таких операций, как проверка подключения и просмотр и Навигация по путям к файлам. Если вы не хотите предоставлять эти разрешения, пропустите проверку соединения на странице создания связанной службы и укажите путь непосредственно в параметрах набора данных.
...
| accessKeyId | Идентификатор секретного ключа доступа. Чтобы найти ключ доступа и секрет, см. раздел [Предварительные требования](#prerequisites). |Да |
...
| serviceUrl | Укажите пользовательскую конечную точку **`https://storage.googleapis.com`** S3 как. | Да |
...
Следующие свойства поддерживаются для облачного хранилища Google в `location` разделе Параметры в наборе данных на основе формата:
...
Следующие свойства поддерживаются для облачного хранилища Google в `storeSettings` разделе Параметры в источнике копирования на основе формата:
...
| prefix                   | Префикс для ключа объекта S3 в заданном контейнере, настроенном в наборе данных для фильтрации исходных объектов. Выбираются объекты, ключи которых начинаются с этого префикса. Применяется только в `wildcardFolderPath` том `wildcardFileName` случае, если свойства и не указаны. |                                                             |
