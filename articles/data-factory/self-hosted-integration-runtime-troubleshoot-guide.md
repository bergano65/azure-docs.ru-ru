...
### <a name="ssltls-certificate-issue"></a>Проблемы с сертификатом SSL/TLS

#### <a name="symptoms"></a>Симптомы

При попытке SSL включить сертификат SSL (/транспорт Layer Security, TLS) (дополнительно) путем выбора сертификата (после выбора локально **размещенного ИК-Configuration Manager**  >  **Удаленный доступ из интрасети**) появляется следующее сообщение об ошибке:

"Параметры удаленного доступа недопустимы. Проверка удостоверения для исходящего сообщения не пройдена. Ожидаемое удостоверение DNS удаленной конечной точки — "abc.microsoft.com", но удаленная конечная точка предоставила утверждение DNS "microsoft.com". Если это легальная удаленная конечная точка, можно устранить проблему, явно указав удостоверение DNS "microsoft.com" в качестве свойства Identity объекта EndpointAddress при создании прокси-сервера канала ".

В предыдущем примере к выбранному сертификату добавляется "microsoft.com".

#### <a name="cause"></a>Причина

Это известная проблема в Windows Communication Foundation (WCF). Проверка SSL/TLS в WCF проверяет только последний DNSName в поле " **альтернативное имя субъекта** " (SAN). 

#### <a name="resolution"></a>Решение

Сертификат с подстановочными знаками поддерживается в локальной среде фабрики данных Azure версии 2. Как правило, эта проблема возникает из-за неправильного сертификата SSL. Последний DNSName в сети хранения данных должен быть допустимым. 

Чтобы проверить и исправить DNSName, выполните следующие действия. 

1. Откройте консоль управления.
1. В разделе **сведения о сертификате** дважды проверьте значение в полях **субъект** и **альтернативное имя субъекта** . Например, "DNS-имя = microsoft.com.com" не является допустимым именем.
1. Чтобы удалить неверный DNSName, обратитесь к Организации, выдающей сертификат.
...
Для обновления до версии 5. x локальной среды выполнения интеграции фабрики данных Azure требуется **.NET Framework Runtime 4.7.2** или более поздней версии. На странице загрузки вы найдете ссылки для скачивания последней версии 4. x и последних двух версий 5. x. 

Для клиентов фабрики данных Azure версии 2:
- Если автоматическое обновление включено и вы уже обновили среду выполнения .NET Framework до 4.7.2 или более поздней версии, то локальная среда выполнения интеграции будет автоматически обновлена до последней версией 5. x.
- Если автоматическое обновление включено и вы еще не обновили среду выполнения .NET Framework до 4.7.2 или более поздней версии, то локальная среда выполнения интеграции не будет автоматически обновлена до последней версией 5. x. Локальная среда выполнения интеграции останется в текущей версии 4. x. Вы видите предупреждение об обновлении среды выполнения .NET Framework на портале и в клиенте локальной среды выполнения интеграции.
- Если автоматическое обновление отключено и вы уже обновили среду выполнения .NET Framework до 4.7.2 или более поздней версии, вы можете вручную скачать последнюю версию 5. x и установить ее на компьютере.
- Если автоматическое обновление отключено и вы не обновили среду выполнения .NET Framework до 4.7.2 или более поздней версии. При попытке вручную установить локальную среду выполнения интеграции 5. x и зарегистрировать ключ вам потребуется сначала обновить версию среды выполнения .NET Framework.
