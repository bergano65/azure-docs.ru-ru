...
Это уведомление предназначено для **исходящих подключений** из **Integration Runtime** , выполняемых **локально** или в **ВИРТУАЛЬНОЙ частной сети Azure** , в службу ADF. Например, если у вас есть локальная служба IR или Azure-SQL Server Integration Services (SSIS) в виртуальной сети Azure, которой требуется доступ к службе ADF, необходимо проверить, нужно ли добавить этот новый диапазон IP-адресов в правила **группы безопасности сети (NSG)** . Если исходящее правило NSG использует тег службы, это не повлияет на.

#### <a name="more-details"></a>Дополнительные сведения

Эти новые диапазоны IP-адресов **влияют только на правила исходящего трафика** из **локального брандмауэра** или **виртуальной частной сети Azure** в службу ADF (см. раздел Настройка [брандмауэра и разрешение на IP-адрес](data-movement-security-considerations.md#firewall-configurations-and-allow-list-setting-up-for-ip-address-of-gateway) для справки), если в локальной сети или в виртуальной сети Azure требуется взаимодействие с локальными сетями IR или SSIS, которые должны взаимодействовать со службой ADF.

Для существующих пользователей, использующих **VPN-подключение Azure**:

1. Проверьте все исходящие правила NSG в частной сети, где настроены службы SSIS или Azure SSIS. Если ограничения для исходящего трафика отсутствуют, они не влияют на них.
1. Если у вас есть ограничения на исходящие правила, проверьте, не используется ли тег службы. Если вы используете тег службы, не нужно изменять или добавлять ничего, так как новые диапазоны IP-адресов находятся под существующим тегом службы. 
  
    ![Проверка назначений](media/self-hosted-integration-runtime-troubleshoot-guide/destination-check.png)

1. Если вы используете IP-адреса непосредственно в параметре правила, проверьте, добавлены ли все диапазоны IP-адресов в [ссылках на загрузку диапазона IP-адресов для тегов служб](https://docs.microsoft.com/azure/virtual-network/service-tags-overview#discover-service-tags-by-using-downloadable-json-files). Мы уже поместили новые диапазоны IP-адресов в этот файл. Для новых пользователей. для настройки правил NSG нужно просто выполнить соответствующие настройки, связанные с локальными средами IR или SSIS в нашем документе.

Для существующих пользователей, имеющих IR или **локальные ИК-** соединения служб SSIS:

- Проверьте с помощью команды сетевой инфраструктуры и Узнайте, нужно ли включать новые IP-адреса диапазонов при обмене данными для исходящих правил.
- Для правил брандмауэра, основанных на именах FQDN, никакие обновления не требуются при использовании параметров, описанных в статье о [конфигурации брандмауэра и настройке списка разрешений для IP-адреса](data-movement-security-considerations.md#firewall-configurations-and-allow-list-setting-up-for-ip-address-of-gateway). 
- Некоторые локальные брандмауэры поддерживают теги службы, если вы используете обновленный файл конфигурации тегов служб Azure, другие изменения не требуются.
