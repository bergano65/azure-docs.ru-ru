...
| authenticationType | Допустимые значения — **Анонимная** или **Windows**. <br><br> Чтобы использовать **проверку подлинности Kerberos** для соединителя HDFS, обратитесь к [этому разделу](#use-kerberos-authentication-for-hdfs-connector) , чтобы настроить локальную среду соответствующим образом. |Да |
...
Следующие свойства поддерживаются для HDFS в разделе `location` параметры в наборе данных на основе формата:

| Свойство   | Описание                                                  | Обязательный |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | Свойство `location` Type в наборе данных должно иметь значение **хдфслокатион**. | Да      |
...
Следующие свойства поддерживаются для HDFS в разделе `storeSettings` параметры в источнике копирования на основе формата:
...
### <a name="option-1-join-self-hosted-integration-runtime-machine-in-kerberos-realm"></a><a name="kerberos-join-realm"></a>Вариант 1. присоединение самостоятельно размещенного Integration Runtime машины в области Kerberos
