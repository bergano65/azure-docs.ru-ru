---
title: Используйте пользовательские действия в конвейере
...
- [Деятельность по перемещению данных](copy-activity-overview.md) для перемещения данных между [поддерживаемыми источниками и хранилищами данных sink.](copy-activity-overview.md#supported-data-stores-and-formats)
- [Мероприятия по преобразованию данных](transform-data.md) для преобразования данных с помощью вычислительных служб, таких как Azure HDInsight, Azure Batch и Azure Machine Learning.
...
| удержаниеTimeInDays | Время хранения файлов, представленных для пользовательской деятельности. Значение по умолчанию составляет 30 дней. | нет |

&#42; Свойства `resourceLinkedService` и `folderPath` (оба) должны быть указаны либо пропущены.

> [!NOTE]
> Если вы передаете связанные службы в качестве справочныхобъектов в пользовательской деятельности, рекомендуется пройти службу с включенным подключенным ключом Azure Key Vault (поскольку она не содержит безопасных строк) и получить учетные данные с помощью секретного имени непосредственно из Key Убежище из кода. Здесь можно найти [пример,](https://github.com/nabhishek/customactivity_sample/tree/linkedservice) на примере которого ссылки на службу aKV включены связанные, извлекают учетные данные из Key Vault, а затем получают доступ к хранилищу в коде.
...
> [!IMPORTANT]
> - Свойства activity.json, linkedServices.json и datasets.json хранятся в папке среды выполнения пакетной задачи. В этом примере в `"https://adfv2storage.blob.core.windows.net/adfjobs/\<GUID>/runtime/"` пути хранятся activity.json, linkedServices.json и datasets.json. При необходимости их следует очищать отдельно.
...
В версии 1 Azure Data Factory вы реализуете (custom) деятельность DotNet, создавая `Execute` проект библиотеки класса .NET с классом, который реализует метод `IDotNetActivity` интерфейса. Связанные службы, наборы данных и расширенные свойства, входящие в полезные данные JSON для (настраиваемого) действия DotNet, передавались в метод выполнения в виде строго типизированных объектов. Дополнительные сведения о поведении версии 1 см. в разделе [Использование настраиваемых действий в конвейере фабрики данных Azure](v1/data-factory-use-custom-activities.md). Из-за этой реализации код активности DotNet версии 1 должен быть нацелен на .NET Framework 4.5.2. Действие DotNet версии 1 также должно выполняться на узлах пула пакетной службы Azure на основе Windows.

В пользовательской деятельности Фабрики данных Azure Data Factory V2 не требуется внедрять интерфейс .NET. Теперь все команды, сценарии и пользовательский код можно компилировать и выполнять в виде исполняемого файла. Чтобы настроить эту реализацию, необходимо указать свойство `Command` вместе со свойством `folderPath`. Настраиваемое действие передает исполняемый файл и его зависимости в каталог `folderpath` и выполняет указанную команду.
...
| ---- | ---- | ---- |
|Способ определения настраиваемой логики      |Путем предоставления исполняемого файла.      |Внедряя .NET DLL      |
|Среда выполнения настраиваемой логики      |Windows или Linux      |Windows (.NET Framework 4.5.2)      |
|Выполнение скриптов      |Поддерживает выполнение сценариев напрямую (например, "cmd /c echo hello world" на виртуальной машине Windows)      |Требуется реализация в .NET DLL      |
...
|Извлечение данных в настраиваемой логике      |Анализ файлов activity.json, linkedServices.json и datasets.json, которые хранятся в одной папке с исполняемым файлом.      |Через .NET SDK (.NET кадр 4.5.2)      |
|Ведение журнала      |Запись непосредственно в STDOUT      |Внедрение Logger в .NET DLL      |

Если у вас есть существующий код .NET, написанный для деятельности DotNet версии 1 (Custom), необходимо изменить код, чтобы он работал с текущей версией пользовательской деятельности. Для этого следует изменить этот код в соответствии со следующими общими рекомендациями.

  - Измените проект из библиотеки класса .NET на консольное приложение.
