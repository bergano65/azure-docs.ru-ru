...
> [!IMPORTANT]
> - Свойства activity.json, linkedServices.json и datasets.json хранятся в папке среды выполнения пакетной задачи. В этом примере путь к файлу Activity. JSON, linkedServices. JSON и DataSets. JSON хранится в `"https://adfv2storage.blob.core.windows.net/adfjobs/\<GUID>/runtime/"` пути. При необходимости их следует очищать отдельно.
