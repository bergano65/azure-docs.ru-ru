...
Чтобы переключиться на другое репо Git, щелкните **значок Git Repo Настройки** в правом верхнем углу страницы обзора Data Factory. Если значок не отображается, очистите кэш локального браузера. Чтобы разорвать связь с текущим репозиторием, выберите значок.
...
*   Только выбранный набор людей должен быть разрешен к публикации на заводе. Для этого они должны иметь роль **вкладчика Data Factory** на заводе. Для получения дополнительной информации [Roles and permissions for Azure Data Factory](concepts-roles-permissions.md)о разрешениях см.
