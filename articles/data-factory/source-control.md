...
Чтобы сделать процесс разработки удобнее, Фабрика данных Azure позволяет настроить репозиторий Git в Azure Repos или GitHub. Git — это система управления версиями, упрощающая отслеживание изменений и совместную работу. В этом учебнике описано, как настроить репозиторий Git и работать в нем, а также приведены рекомендации и способы устранения неполадок.
...
-   **Частичное сохранение.** При разработке в службе фабрики данных изменения нельзя сохранить в виде черновика, а все публикации должны пройти проверку фабрики данных. Если конвейеры не завершили операции или вы просто не хотите потерять изменения в случае сбоя компьютера, интеграция с Git позволяет выполнять добавочные изменения ресурсов фабрики данных независимо от состояния, в котором они находятся. Настройка репозитория Git позволяет сохранять изменения и опубликовать их только после тестирования.
...
> [!NOTE]
> Если в службе "Фабрика данных Azure" настраивается использование репозитория Git, непосредственная разработка в интерфейсе этой службы отключается. Изменения можно вносить непосредственно в службу с помощью PowerShell или пакета SDK.
...
#### <a name="configuration-method-1-azure-data-factory-home-page"></a>Метод настройки 1. Домашняя страница Фабрики данных Azure

На домашней странице Фабрики данных Azure выберите **Set up Code Repository** (Настроить репозиторий кода).

![Настройка репозитория кода Azure Repos](media/author-visually/configure-repo.png)

#### <a name="configuration-method-2-ux-authoring-canvas"></a>Метод настройки 2. Пользовательский интерфейс "Работа над холстом"
...
Оба метода открывают панель настройки параметров репозитория.
...
| **Ветвь совместной работы** | Ветвь вашей совместной работы в Azure Repos, которая используется для публикации. По умолчанию используется `master`. Измените этот параметр, если нужно опубликовать ресурсы из другой ветви. | `<your collaboration branch name>` |
...
Обычно не нужно, чтобы у каждого участника группы имелись разрешения на обновление фабрики. Рекомендуется использовать следующие параметры разрешений:

*   Всем участникам команды следует предоставить разрешения на чтение фабрики данных.
*   Публиковать в фабрике нужно разрешить только определенным пользователям. Для этого у них должна быть роль **Data Factory contributor** (Участник фабрики данных) в группе ресурсов, в которой находится фабрика. Дополнительные сведения о разрешениях см. в статье [Роли и разрешения для службы "Фабрика данных Azure"](concepts-roles-permissions.md).
...
- У пользователя несколько ветвей. В одной из ветвей функциональностей пользователь удалил связанную службу, которая не использует AKV (такие службы публикуются немедленно, независимо от того, находятся ли они в Git или нет), но не выполнил слияние ветви функциональности с ветвью совместной работы.
