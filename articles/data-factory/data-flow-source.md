...
**Корневой путь к разделу:** Если в источнике файлов имеются секционированные папки с форматом ```key=value``` (например, Year = 2019), то верхний уровень этого дерева папок секционирования можно назначить имени столбца в потоке потока данных.
...
**Входные данные:** Укажите, следует ли указывать источник в таблице (эквивалентно ```Select * from <table-name>```) или введите пользовательский запрос SQL.
