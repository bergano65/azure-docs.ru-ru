...
![Параметры секции источника](media/data-flow/sourcepart3.png "секционирование")
...
Выберите столбец секции на вашей исходной таблицы. Также можно задайте число секций.
...
Примеры подстановочный знак:

* ```*``` Представляет любой набор символов
* ```**``` Представляет вложение directory рекурсивные
* ```?``` Заменяет один символ
* ```[]``` Совпадает с одним из дополнительных символов в квадратных скобках

* ```/data/sales/**/*.csv``` Возвращает все файлы csv в /data/sales
* ```/data/sales/20??/**``` Возвращает все файлы в 20-м веке
* ```/data/sales/2004/*/12/[XY]1?.csv``` Получает все CSV-файлы в 2004 году в декабре, начиная с X или Y, предшествует из 2 цифр

Контейнер должно быть указано в наборе данных. Путем с подстановочными знаками, поэтому должен включать путь к папке, из корневой папки.
...
Чтобы переместить исходные файлы для другого расположения постобработки, сначала выберите «Переместить» для операции файл. Затем задайте в каталог «from». Если вы не используете никакие подстановочные знаки для пути, то параметр «from» будет той же папке вашей исходной папки.

Если у вас есть исходный путь с подстановочными знаками, например:

```/data/sales/20??/**/*.csv```

Можно указать «from» в качестве
...
В этом случае всех вложенных /data/sales, которые были им в свою очередь, перемещаются по отношению к /backup/priorSales.
...
* **Уровень изоляции**: Значение по умолчанию для источников SQL в ADF сопоставление потоки данных — Read Uncommitted. Уровень изоляции можно изменить на одно из следующих значений:
* Read Committed
* Незафиксированное чтение
* Повторяющаяся операция чтения
* Сериализуемый
* Нет (не учитывать уровень изоляции)
