...
| noTruncation | Указывает, следует ли усекать возвращенный результирующий набор. По умолчанию результат усекается после 500 000 записей или 64 МБ. Для надлежащего поведения действия настоятельно рекомендуется усечение. |Нет |
