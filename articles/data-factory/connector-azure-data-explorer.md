...
>[!NOTE]
>Копирование данных в или из Azure обозреватель данных из/в локальное хранилище данных с помощью автономной Integration Runtime поддерживается с версии 3,14.
...
>[!TIP]
>Пошаговое руководство по использованию соединителя Azure обозреватель данных см. в статье [копирование данных в azure обозреватель данных и обратно с помощью фабрики данных Azure](../data-explorer/data-factory-load-data.md).
...
2. Предоставьте субъекту-службе соответствующее разрешение в Azure обозреватель данных. Дополнительные сведения о ролях и разрешениях, а также об управлении разрешениями см. в разделе [Управление разрешениями базы данных Azure обозреватель данных](../data-explorer/manage-database-permissions.md) . В общем случае необходимо

    - В **качестве источника**предоставьте по крайней мере роль **средства просмотра базы** данных для базы данных.
    - В **качестве приемника**предоставьте по крайней мере роль **приема базы данных** для базы данных.

>[!NOTE]
>При использовании пользовательского интерфейса ADF учетная запись пользователя для входа используется для перечисления кластеров Azure обозреватель данных, баз данных и таблиц. Вручную введите имя, если у вас нет разрешения на такую операцию.
...
| tenant | Укажите сведения о клиенте (доменное имя или идентификатор клиента), в котором находится приложение. Это то, что вы обычно узнаете как "**идентификатор центра сертификации**" в [строке подключения Kusto](https://docs.microsoft.com/azure/kusto/api/connection-strings/kusto#application-authentication-properties). Получите его, наведя указатель мыши на правый верхний угол страницы портала Azure. | Да |
| servicePrincipalId | Укажите идентификатора клиента приложения. Это то, что вы обычно узнаете как "**идентификатор клиента приложения AAD**" в [строке подключения Kusto](https://docs.microsoft.com/azure/kusto/api/connection-strings/kusto#application-authentication-properties). | Да |
| servicePrincipalKey | Укажите ключ приложения. Это то, что вы обычно узнаете как "**ключ приложения AAD**" в [строке подключения Kusto](https://docs.microsoft.com/azure/kusto/api/connection-strings/kusto#application-authentication-properties). Пометьте это поле как **SecureString**, чтобы безопасно хранить его в фабрике данных, или [добавьте ссылку на секрет, хранящийся в Azure Key Vault](store-credentials-in-key-vault.md). | Да |
...
| запрос | Запрос только для чтения в [формате KQL](/azure/kusto/query/). Используйте пользовательский запрос KQL в качестве ссылки. | Да |
...
| Усечение | Указывает, следует ли усекать возвращенный результирующий набор. По умолчанию результат усекается после 500 000 записей или 64 МБ. Для надлежащего поведения действия настоятельно рекомендуется усечение. |Нет |

>[!NOTE]
>Для источника обозреватель данных Azure по умолчанию установлен предельный размер 500 000 записей или 64 МБ. Чтобы получить все записи без усечения, можно указать `set notruncation;` в начале запроса. Дополнительные сведения см. в разделе [ограничения запросов](https://docs.microsoft.com/azure/kusto/concepts/querylimits) .
...
| Свойство | Описание | Обязательно для заполнения |
|:--- |:--- |:--- |
| type | Свойству **type** приемника действия копирования необходимо присвоить значение **AzureDataExplorerSink** | Да |
| ingestionMappingName | Имя предварительно созданного **[сопоставления](/azure/kusto/management/mappings#csv-mapping)** в таблице Kusto. Чтобы сопоставить столбцы из источника с Azure обозреватель данных, которые применяются ко **[всем поддерживаемым исходным хранилищам/форматам](copy-activity-overview.md#supported-data-stores-and-formats)** , включая форматы CSV/JSON/Avro и т. д., можно использовать [сопоставление столбцов](copy-activity-schema-and-type-mapping.md) действия копирования (неявно по имени или явно настроенное) и сопоставления обозреватель данных Azure или VSPS. | Нет |
...
* Узнайте больше о [копировании данных из фабрики данных Azure в azure обозреватель данных](/azure/data-explorer/data-factory-load-data).