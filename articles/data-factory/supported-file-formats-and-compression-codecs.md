...
>[!NOTE]
>В фабрике данных появился новый текстовый формат с разделителями датсет. Дополнительные сведения см. в статье о [формате текста](format-delimited-text.md) с разделителями. Следующие конфигурации в наборе данных хранилища данных на основе файлов по-прежнему поддерживаются "как есть" для обратного компабитилити. Рекомендуется использовать новую модель.
...
>[!NOTE]
>В фабрике данных появился новый формат Parquet датсет, дополнительные сведения см. в статье о [формате Parquet](format-parquet.md) . Следующие конфигурации в наборе данных хранилища данных на основе файлов по-прежнему поддерживаются "как есть" для обратного компабитилити. Рекомендуется использовать новую модель.
...
| UInt64 | Binary или Int64 | UInt64 | Десятичное |
| Единое | Плавающая область | Н/Д | Н/Д |
| Double | Double | Н/Д | Н/Д |
| Десятичное | Бинарный | Десятичное | Десятичное |
