...
|:--- |:--- |:--- |
| Тип | Для свойства Type необходимо задать значение **азурефилестораже**. | ДА |
...
>[!TIP]
>При использовании пользовательского интерфейса ADF для разработки можно найти конкретную запись "хранилище файлов Azure" для создания связанной службы, которая в своюмся случае создает объект типа `FileServer`.
...
Для хранилища файлов Azure поддерживаются следующие свойства в разделе Параметры `location` в наборе данных на основе формата:

| Свойство   | Описание                                                  | Обязательно для заполнения |
| ---------- | ------------------------------------------------------------ | -------- |
| Тип       | Для свойства Type в параметре `location` в наборе данных необходимо задать значение **филесерверлокатион**. | ДА      |
...
Для хранилища файлов Azure поддерживаются следующие свойства в разделе `storeSettings` параметры в источнике копирования на основе формата:

| Свойство                 | Описание                                                  | Обязательно для заполнения                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| Тип                     | Для свойства Type в `storeSettings` должно быть задано значение **филесерверреадсеттинг**. | ДА                                           |
...
Для хранилища файлов Azure поддерживаются следующие свойства в параметрах `storeSettings` в приемнике копирования на основе формата:

| Свойство                 | Описание                                                  | Обязательно для заполнения |
| ------------------------ | ------------------------------------------------------------ | -------- |
| Тип                     | Для свойства Type в `storeSettings` должно быть задано значение **филесервервритесеттинг**. | ДА      |
...
| Свойство | Описание | Обязательно для заполнения |
|:--- |:--- |:--- |
| Тип | Свойство type приемника действия копирования должно иметь значение **FileSystemSink**. |ДА |
...
| true |mergeFiles | Папка1<br/>&nbsp;&nbsp;&nbsp;&nbsp;Файл1<br/>&nbsp;&nbsp;&nbsp;&nbsp;Файл2<br/>&nbsp;&nbsp;&nbsp;&nbsp;Вложенная_папка1<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Файл3<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Файл4<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Файл5 | Целевая папка "Папка1" создается со следующей структурой: <br/><br/>Папка1<br/>&nbsp;&nbsp;&nbsp;&nbsp;file1 + file2 + Файл3 + "Файл4" + File 5 содержимое объединяется в один файл с автоматически созданным именем файла |
