...
| дисаблеметриксколлектион | Фабрика данных собирает такие метрики, как Cosmos DB RUs для оптимизации производительности и рекомендаций по копированию. Если вы отвечаете за такое поведение, укажите `true` для его отключения. | Нет (значение по умолчанию — `false`) |
