...
- Копирование данных из службы Azure Cosmos DB и в нее с помощью [API SQL](../cosmos-db/introduction.md).
...
| детектдатетиме | Определяет, следует ли определять значение DateTime из строковых значений в документах. Допустимые значения: **true** (по умолчанию), **false**. | нет |
...
**Размер пакета**: целое число, представляющее количество объектов, записываемых в Cosmos DB коллекцию в каждом пакете. Обычно достаточно начать с размера пакета по умолчанию. Для дальнейшей настройки этого значения Обратите внимание на следующее.
...
**Бюджет пропускной способности записи:** Целое число, представляющее объект RUs, который требуется выделить для этой операции записи потока данных, из общей пропускной способности, выделенной для коллекции.
