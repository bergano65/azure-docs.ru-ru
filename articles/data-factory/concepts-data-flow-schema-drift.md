...
ms.locfileid: "68640229"
---
# <a name="mapping-data-flow-schema-drift"></a>Смещение схемы потока данных сопоставления
...
Вы можете увидеть эту функцию смещения схемы на работе с примером потока данных фабрики данных Azure "демонстрация такси". Включите сеанс отладки с помощью переключателя отладки в верхней части области конструктора Потока данных, чтобы можно было просматривать результаты в интерактивном режиме:
...
Преобразование SELECT и Sink поддерживает сопоставление шаблонов с помощью сопоставления на основе правил. Это позволит вам создавать правила, которые могут сопоставлять смещенные столбцы с псевдонимами столбцов и переносить эти столбцы в место назначения.
