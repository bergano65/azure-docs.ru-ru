...
В этой статье описывается основная статья по интеграции фабрики данных Azure — машинного обучения (классическая модель). [Создание прогнозирующих конвейеров с помощью машинное обучение Azure и фабрики данных Azure](transform-data-using-machine-learning.md). Перед прочтением этой статьи ознакомьтесь с основной статьей, если вы еще этого не сделали.

## <a name="overview"></a>Обзор
В рамках процесса работы с моделями эксплуатацию ML Studio (классические модели) модель будет обучена и сохранена. После этого она используется для создания прогнозной веб-службы. Созданная веб-служба может впоследствии использоваться на веб-сайтах, панелях мониторинга и в мобильных приложениях.
...
В следующем фрагменте кода JSON определяется действие выполнения пакета в студии машинного обучения (классический).
