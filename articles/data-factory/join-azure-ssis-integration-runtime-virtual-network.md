...
> [!NOTE]
> Используйте полное доменное имя (FQDN) для имени частного узла, например `<your_private_server>.contoso.com` `<your_private_server>`, используйте вместо, так как Azure-SSIS IR не будет автоматически добавлять собственный DNS-суффикс.
...
| Направление | Транспортный протокол | Источник | Диапазон портов источника | Назначение | Диапазон портов назначения | Комментарии |
|---|---|---|---|---|---|---|
...
| Направление | Транспортный протокол | Источник | Диапазон портов источника | Назначение | Диапазон портов назначения | Комментарии |
|---|---|---|---|---|---|---|
| Исходящие | TCP | Виртуальная сеть | * | AzureCloud; | 443 | Узлы Azure-SSIS IR в виртуальной сети используют этот порт для доступа к службам Azure, таким как служба хранилища Azure и концентраторы событий Azure. |
...
| Исходящие | TCP | Виртуальная сеть | * | Память | 445 | Используемых Это правило требуется только в том случае, если требуется выполнить пакет служб SSIS, хранящийся в службе файлов Azure. |
||||||||
...
Эти ресурсы будут созданы при запуске Azure-SSIS IR. Они будут удалены после прекращения Azure-SSIS IR. Если вы перенесете собственные статические общедоступные IP-адреса для Azure-SSIS IR, ваши собственные статические IP-адреса не будут удалены при остановке Azure-SSIS IR. Чтобы предотвратить блокировку Azure-SSIS IR, не используйте эти сетевые ресурсы в других ресурсах.
