...
Если вам нужно использовать собственный DNS-сервер в виртуальной сети, присоединенной Azure-SSIS IR для разрешения имени частного узла, убедитесь, что он также может разрешить глобальные имена узлов Azure (например, большой двоичный объект службы хранилища Azure с именем `<your storage account>.blob.core.windows.net`). 
...
> [!NOTE]
> Используйте полное доменное имя (FQDN) для имени частного узла, например, используйте `<your_private_server>.contoso.com` вместо `<your_private_server>`, так как Azure-SSIS IR не будет автоматически добавлять собственный DNS-суффикс.
...
| Вход. | TCP | корпнетсав | * | Виртуальная сеть | 3389 | Используемых Это правило требуется только тогда, когда программа Microsoft Support просит клиента открыть для расширенного устранения неполадок и может быть закрыта сразу после устранения неполадок. Тег службы **корпнетсав** позволяет использовать удаленный рабочий стол только рабочим станциям с защищенным доступом в корпоративной сети Майкрософт. Этот тег службы нельзя выбрать на портале. он доступен только с помощью Azure PowerShell или Azure CLI. <br/><br/> На уровне NIC NSG порт 3389 открыт по умолчанию, и мы позволяем управлять портом 3389 на уровне подсети NSG, в то время как Azure-SSIS IR по умолчанию отключил порт 3389 для правила брандмауэра Windows на каждом из IR-узлов для защиты. |
||||||||
...
| Исход. | TCP | Виртуальная сеть | * | Интернет | 80 | Используемых Узлы Azure-SSIS IR в виртуальной сети используют этот порт для скачивания списка отзыва сертификатов из Интернета. Если заблокировать этот трафик, вы можете столкнуться с понижением производительности при запуске функции "IR" и "потерять", чтобы проверить список отзыва сертификатов для использования сертификата. Если вы хотите дополнительно сузить место назначения до определенных полных доменных имен, ознакомьтесь с разделом **Использование Azure ExpressRoute или UDR** .|
| Исход. | TCP | Виртуальная сеть | * | SQL | 1433, 11000-11999 | Используемых Это правило требуется только в том случае, если узлы Azure-SSIS IR в виртуальной сети обращаются к SSISDB, размещенной на сервере базы данных SQL. Если для политики подключения сервера базы данных SQL задано значение **прокси** вместо **перенаправления**, то требуется только порт 1433. <br/><br/> Это правило безопасности для исходящего трафика не применимо к SSISDB, размещенной на управляемом экземпляре в виртуальной сети или на сервере базы данных Azure, настроенном с помощью частной конечной точки. |
| Исход. | TCP | Виртуальная сеть | * | Виртуальная сеть | 1433, 11000-11999 | Используемых Это правило требуется только в том случае, если узлы Azure-SSIS IR в виртуальной сети обращаются к SSISDB, размещенной на управляемом экземпляре в виртуальной сети или на сервере базы данных Azure, для которого настроена частная конечная точка. Если для политики подключения сервера базы данных SQL задано значение **прокси** вместо **перенаправления**, то требуется только порт 1433. |
| Исход. | TCP | Виртуальная сеть | * | Хранилище | 445 | Используемых Это правило требуется только в том случае, если требуется выполнить пакет служб SSIS, хранящийся в службе файлов Azure. |
||||||||
...
Например, если Azure-SSIS IR находится в `UK South` и вы хотите проверить исходящий трафик через брандмауэр Azure, сначала необходимо получить список диапазонов `BatchNodeManagement.UKSouth` тегов службы из [ссылки "Теги службы](https://www.microsoft.com/download/details.aspx?id=56519) " или через [API обнаружения тегов служб](https://aka.ms/discoveryapi). Затем примените следующие определяемые пользователем маршруты маршрутов диапазонов IP-адресов с типом следующего прыжка в **Интернете** вместе с маршрутом 0.0.0.0/0 и типом следующего прыжка в качестве **виртуального устройства**.
...
-   Порт 443 с целевым объектом в качестве облачных служб Azure.
    ...
    При использовании брандмауэра Azure можно указать правило сети с тегом службы AzureCloud. в противном случае вы можете разрешить назначение как все в устройстве брандмауэра.
...
-   Порт 1433, 11000-11999 с целевым объектом в качестве SQL Azure (требуется только в том случае, если узлы Azure-SSIS IR в виртуальной сети обращаются к SSISDB, размещенной на сервере базы данных SQL).
...
> [!NOTE]
> Для Azure SQL и хранилища при настройке конечных точек службы виртуальной сети в подсети трафик между Azure-SSIS IR и Azure SQL в том же регионе \ хранилище Azure в том же регионе или в парном регионе будет направляться непосредственно в Microsoft Azure магистральной сети. вместо устройства брандмауэра.
