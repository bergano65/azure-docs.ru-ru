...
## <a name="host-the-ssis-catalog-database-in-azure-sql-database-with-virtual-network-service-endpointsmanaged-instance"></a>Размещение базы данных каталога SSIS в базе данных SQL Azure с использованием конечных точек служб виртуальной сети или управляемого экземпляра
Если каталог SSIS размещен в базе данных SQL Azure с использованием конечных точек служб виртуальной сети или управляемого экземпляра, среду выполнения интеграции Azure-SSIS можно присоединить к: 

- той же виртуальной сети; 
- другой виртуальной сети, где есть подключение "сеть — сеть" к той сети, которая используется для базы данных SQL Azure с конечными точками служб виртуальной сети или управляемым экземпляром. 

Если среда выполнения интеграции Azure SSIS присоединяется к виртуальной сети управляемого экземпляра, используйте для них разные подсети. Чтобы присоединить среду выполнения интеграции Azure SSIS к другой виртуальной сети, отличной от сети управляемого экземпляра, используйте пиринг между виртуальными сетями (в том же регионе) или подключение типа "виртуальная сеть — виртуальная сеть". См. статью [Подключение приложения к управляемому экземпляру базы данных SQL](../sql-database/sql-database-managed-instance-connect-app.md).

Виртуальная сеть может быть развернута с использованием классической модели развертывания или модели развертывания с помощью Azure Resource Manager.
...
- Присоединить MSSQL Integration Services IR к виртуальной сети Azure (текущая версия) можно двумя способами.

  - Использовать встроенную роль *Участник сетей*. Для этой роли требуется разрешение *Microsoft.Network/\**, которое имеет намного больше области резервирования.

  - Создать настраиваемую роль, которая включает разрешение *Microsoft.Network/virtualNetworks/\*/join/action*. 

- При присоединении MSSQL Integration Services IR к классической виртуальной сети Azure, рекомендуется использовать встроенную роль *Участник классических виртуальных машин*. В противном случае необходимо определить пользовательскую роль, которая включает разрешение на присоединение к виртуальной сети.
...
1. На странице **Общие параметры** окна **Настройка среды выполнения интеграции** нажмите кнопку **Далее**. 
...
1. На странице **параметров SQL** введите пароль администратора и нажмите кнопку **Далее**. 
...
1. На странице **Дополнительные параметры** выполните следующие действия. 
   ...
   д. Нажмите кнопку **Проверить виртуальную сеть**. Если проверка пройдет успешно, щелкните **Обновить**. 
...
1. Теперь можно запустить среду выполнения интеграции, нажав кнопку **запуска** в столбце **Действия** для выбранной среды выполнения интеграции Azure SSIS. Запуск IR Azure SSIS занимает приблизительно от 20 до 30 минут. 
...
- [Создание среды выполнения интеграции Azure SSIS в фабрике данных Azure](create-azure-ssis-integration-runtime.md). Эта статья дополняет соответствующее руководство, а также предоставляет инструкции по использованию базы данных SQL Azure с конечной точкой службы виртуальной сети или управляемым экземпляром и присоединению среды выполнения интеграции к виртуальной сети. 
