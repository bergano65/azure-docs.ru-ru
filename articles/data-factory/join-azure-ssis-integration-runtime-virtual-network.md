...
- Необходимо подключаться к источникам данных или ресурсам, которые разрешают доступ только с конкретных статических общедоступных IP-адресов из пакетов служб SSIS, которые выполняются на Azure-SSIS IR.
...
Если вы хотите защитить источники данных или ресурсы, разрешая доступ только из конкретных статических общедоступных IP-адресов, вы можете использовать собственные [общедоступные IP-адреса](https://docs.microsoft.com/azure/virtual-network/virtual-network-public-ip-address) при присоединении Azure-SSIS IR к подсети виртуальной сети. В этом случае IP-адреса Azure-SSIS IR будут исправлены для указанных вами. Затем добавьте правило брандмауэра IP-адреса в источники данных или ресурсы, чтобы разрешить доступ с этих IP-адресов.
...
- Если вы присоединяете Azure-SSIS IR к виртуальной сети Azure Resource Manager, у вас есть два варианта:
  ...
  - Создать настраиваемую роль, которая включает только нужное разрешение _Microsoft.Network/virtualNetworks/\*/join/action_. Если вы хотите также перенести собственные общедоступные IP-адреса для среды выполнения интеграции SSIS, помимо того, чтобы присоединить его к Azure Resource Manager виртуальной сети, включите в роль разрешение _Microsoft. Network/publicIPAddresses/*/Жоин/Актион_ .
...
-   Предоставьте два неиспользуемых статических общедоступных IP-адреса, которые еще не связаны с другими ресурсами службы Azure. При обновлении Azure-SSIS IR будет использоваться дополнительный.

-   Общедоступные IP-адреса должны быть статическими и стандартными. Дополнительные сведения см. в разделе [номера SKU общедоступного IP-адреса](https://docs.microsoft.com/azure/virtual-network/virtual-network-ip-addresses-overview-arm#sku) .

-   Статические общедоступные IP-адреса должны иметь DNS-имена. Если вы не настроили DNS-имя при создании общедоступного IP-адреса, это можно также настроить в портал Azure.

![Azure SSIS IR](media/ssis-integration-runtime-management-troubleshoot/setup-publicipdns-name.png)

-   Статические общедоступные IP-адреса и виртуальная сеть должны находиться в одной и той же подписке и в одном регионе.
...
> [!NOTE]
> Теперь вы можете присвоить своим Azure-SSIS IR собственные статические общедоступные IP-адреса. В этом сценарии вы создадите только подсистему балансировки нагрузки Azure и группу безопасности сети. Кроме того, ресурсы будут создаваться в той же группе ресурсов, что и общедоступные IP-адреса, а не в виртуальной сети.

Эти ресурсы будут созданы при запуске IR. Они будут удалены при остановке IR. Обратите внимание, что если вы перенесете собственные общедоступные IP-адреса, общедоступные IP-адреса не будут удалены после остановки IR. Чтобы предотвратить блокировку ИК-остановки, не используйте эти сетевые ресурсы в других ресурсах. 

Убедитесь в отсутствии блокировки ресурсов для группы ресурсов или подписки, к которой принадлежит виртуальная сеть (или общедоступные IP-адреса при их наличии). Если вы настраиваете блокировку только для чтения или удаление, запуск и остановка IR могут завершиться сбоем, иначе IR может перестать отвечать. 

Убедитесь, что у вас нет политики Azure, которая не позволяет создавать следующие ресурсы в группе ресурсов или подписке, к которой принадлежит виртуальная сеть (или общедоступные IP-адреса при их наличии): 
...
- Можно ли добавить статический IP-адрес Azure-SSIS IR в список разрешений брандмауэра для источника данных?
    ...
    Теперь вы можете получить собственные статические общедоступные IP-адреса для Azure-SSIS IR. В этом случае можно добавить указанные IP-адреса в списки разрешений брандмауэра для источников данных. Вы также можете рассмотреть следующие варианты, чтобы разрешить Azure-SSIS IR доступ к источнику данных в зависимости от сценария.
...
- Зачем нужно предоставлять два статических общедоступных адреса, если мне нужно использовать собственные общедоступные IP-адреса для Azure-SSIS IR?

    Azure-SSIS IR автоматически обновляется на регулярной основе. Новые узлы IR создаются во время обновления, и старые узлы будут удалены. Однако во избежание простоя старые узлы не удаляются до тех пор, пока новые узлы не будут готовы. Поэтому ваш первый общедоступный IP-адрес, используемый старыми узлами, не может быть освобожден немедленно, и для создания новых IR-узлов требуется другой общедоступный адрес.
- Я принимаю статические общедоступные IP-адреса для Azure-SSIS IR, но IR по-прежнему не может получить доступ к источникам данных или ресурсам.

    - Убедитесь, что два статических общедоступных IP-адреса добавлены в список разрешений источников данных или ресурсов. После обновления Azure-SSIS IR общедоступный IP-адрес IR переключается на дополнительный общедоступный IP-адрес. Если добавить в список разрешений только один из них, доступ может быть нарушен после обновления.

    - Если источником данных является служба Azure, проверьте, настроена ли подсеть виртуальной сети с конечной точкой службы. Если заданы конечные точки службы, трафик службы переключается на использование частных адресов, управляемых службами Azure, в качестве исходных IP-адресов при доступе к службе Azure из виртуальной сети. В этом случае добавление собственных общедоступных IP-адресов в список разрешений не вступит в действие.
...
1. На странице **Дополнительные параметры** выполните следующие действия. 
   ...
   д) Если вы хотите использовать для Azure-SSIS IR собственный статический общедоступный IP-адрес, установите флажок сделать **статические общедоступные IP-адреса** . Затем укажите первый и второй статический общедоступный IP-адрес для Azure-SSIS IR. Можно также нажать кнопку **создать новую** кнопку, чтобы создать новый общедоступный IP-адрес. Дополнительные сведения см. в разделе [Выбор статических общедоступных IP-адресов](#publicIP) для требований общедоступных IP-адресов.

   е) Если вы также хотите настроить локальное IR в качестве прокси-сервера для Azure-SSIS IR, установите флажок **настроить автономное** размещение. Дополнительные сведения см. в статье [Настройка автономного IR в качестве прокси-сервера для Azure-SSIS IR](https://docs.microsoft.com/azure/data-factory/self-hosted-integration-runtime-proxy-ssis).

   ж. Нажмите кнопку **Проверка виртуальной сети** . Если проверка прошла успешно, нажмите кнопку **Далее** . 
