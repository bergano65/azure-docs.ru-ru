...
- Вы размещаете базу данных каталога SQL Server Integration Services (SSIS) в базе данных SQL Azure с использованием конечных точек служб для виртуальной сети или управляемого экземпляра. 
...
Если каталог SSIS размещен в базе данных SQL Azure с использованием конечных точек служб для виртуальной сети или управляемого экземпляра, среду выполнения интеграции Azure-SSIS можно присоединить к: 

- той же виртуальной сети; 
- другой виртуальной сети, имеющей подключение типа "сеть — сеть" с той сетью, в которой находится управляемый экземпляр. 

Если вы размещаете каталог SSIS в Базе данных SQL Azure с использованием конечных точек служб для виртуальной сети, обязательно присоедините Azure-SSIS IR к той же виртуальной сети и подсети.

Если Azure-SSIS IR присоединяется к виртуальной сети управляемого экземпляра, используйте для них разные подсети. Если Azure-SSIS IR присоединяется к другой виртуальной сети, отличной от сети управляемого экземпляра, используйте пиринг между виртуальными сетями (в том же регионе) или подключение типа "виртуальная сеть — виртуальная сеть". См. статью [Подключение приложения к управляемому экземпляру базы данных SQL](../sql-database/sql-database-managed-instance-connect-app.md).
...
-   Убедитесь в отсутствии блокировки ресурсов в группе ресурсов и в подписке, к которым принадлежит виртуальная сеть. Если вы настраиваете блокировки только для чтения или блокировки удаления, запуска и остановки среды выполнения Интеграции может произойти сбой или перестать отвечать на запросы. 
...
- [Создание среды выполнения интеграции Azure SSIS в фабрике данных Azure](create-azure-ssis-integration-runtime.md). Эта статья дополняет соответствующее руководство, а также предоставляет инструкции по использованию базы данных SQL Azure с конечной точкой службы для виртуальной сети или управляемым экземпляром и присоединению среды выполнения интеграции к виртуальной сети. 
