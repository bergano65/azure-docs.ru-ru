...
| `sncQop` | Применяемый уровень защиты SNC.<br/>Применяется, `sncMode` если включен. <br/>Допустимые значения `1` : (проверка подлинности) `2` , `3` (целостность) `8` , (конфиденциальность) `9` , (по умолчанию), (максимум). | Нет |
...
| `partitionLowerBound`              | Минимальное значение столбца, указанного в `partitionColumnName` , будет использоваться для продолжения секционирования. (Примечание. `partitionLowerBound` не может иметь значение "0", если `PartitionOnInt`параметр секционирования имеет значение) | Нет       |
...
<br/>
>`partitionOption` `partitionUpperBound` `partitionLowerBound``maxPartitionsNumber`В качестве примера количество строк в каждой секции вычисляется с помощью следующей формулы: (всего строк, попадающих в диапазон и)/. `partitionOnInt`<br/>
<br/>
