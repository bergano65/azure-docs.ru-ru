...
### <a name="example-1-connect-to-an-sap-application-server"></a>Пример 1. подключение к серверу приложений SAP
...
## <a name="join-sap-tables"></a>Присоединение к таблицам SAP

В настоящее время соединитель таблиц SAP поддерживает только одну одну таблицу с модулем функции по умолчанию. Чтобы получить Объединенные данные нескольких таблиц, можно использовать свойство [кустомрфкреадтаблефунктионмодуле](#copy-activity-properties) в соединителе таблицы SAP, как описано ниже.

- [Написание пользовательского модуля функции](#create-custom-function-module), который может принимать запрос в качестве параметров и применять собственную логику для получения данных.
- В поле "пользовательский модуль функции" введите имя модуля пользовательской функции.
- Для параметра "Параметры таблицы RFC" укажите инструкцию JOIN таблицы для передачи в модуль функции в качестве параметров, например " `<TABLE1>` внутреннее соединение `<TABLE2>` в COLUMN0".

Ниже приведен пример:

![Соединение таблиц SAP](./media/connector-sap-table/sap-table-join.png) 

>[!TIP]
>Можно также рассмотреть возможность объединения Объединенных данных в представлении, которое поддерживается соединителем таблиц SAP.
>Вы также можете попытаться извлечь связанные таблицы для подключения к Azure (например, службе хранилища Azure, базе данных SQL Azure), а затем использовать поток данных, чтобы продолжить присоединение или фильтрацию.

## <a name="create-custom-function-module"></a>Создание пользовательского модуля функции

В настоящее время для таблицы SAP мы поддерживаем свойство [кустомрфкреадтаблефунктионмодуле](#copy-activity-properties) в источнике копирования, которое позволяет использовать собственную логику и обрабатывать данные.

Ниже приведены некоторые требования для начала работы с "пользовательским модулем функции".

- Определение:

    ![Определение](./media/connector-sap-table/custom-function-module-definition.png) 

- Экспортируйте данные в одну из следующих таблиц:

    ![Экспорт таблицы 1](./media/connector-sap-table/export-table-1.png) 

    ![Экспорт таблицы 2](./media/connector-sap-table/export-table-2.png)
 
Ниже приведены иллюстрации работы соединителя таблиц SAP с настраиваемым модулем функции.

1. Создание подключения к серверу SAP через SAP NCO.

1. Вызов "модуль настраиваемой функции" с набором параметров, как показано ниже:

    - QUERY_TABLE: имя таблицы, заданное в наборе данных таблицы SAP в ADF; 
    - Разделитель: разделитель, заданный в источнике таблицы SAP ADF; 
    - ROWCOUNT/Option/Fields: параметры ROWCOUNT/Aggregateed/Fields, заданные в источнике таблицы ADF.

1. Получите результат и проанализируйте данные следующими способами:

    1. Проанализируйте значение в таблице Fields, чтобы получить схемы.

        ![Анализ значений в полях](./media/connector-sap-table/parse-values.png)

    1. Получите значения выходной таблицы, чтобы узнать, в какой таблице содержатся эти значения.

        ![Получение значений в выходной таблице](./media/connector-sap-table/get-values.png)

    1. Получение значений в OUT_TABLE, анализ данных и запись их в приемник.
