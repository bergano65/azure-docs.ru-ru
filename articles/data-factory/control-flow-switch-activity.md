---
title: Переключение активности на фабрике данных Azure
description: Активность коммутатора позволяет контролировать процессинговый поток на основе условия.
...
---
# <a name="switch-activity-in-azure-data-factory"></a>Переключение активности на фабрике данных Azure

[!INCLUDE[appliesto-adf-asa-md](includes/appliesto-adf-asa-md.md)]

Активность коммутатора обеспечивает ту же функциональность, что и заявление коммутатора на языках программирования. Он оценивает набор действий, соответствующих делу, который соответствует оценке состояния.
...
-------- | ----------- | -------------- | --------
name | Название действия коммутатора. | Строка | Да
type | Должна быть настроена на *коммутатор** | Строка | Да
expression | Выражение, которое необходимо оценить для значения строки | Выражение со строкой типа результата | Да
cases | Набор случаев, содержащих значение, и набор действий для выполнения, когда значение соответствует оценке выражения. Должен быть как минимум один случай. Максимальный лимит в 25 случаев. | Массив объектов кейса | Да
defaultActivities | Набор действий, выполняемых при невыполнении оценки выражения. | Массив действий | Да

## <a name="example"></a>Пример

В этом примере конвейер копирует данные из входной папки в выходную папку. Выходная папка определяется значением параметра конвейера routeSelection.
...
### <a name="pipeline-with-switch-activity-adfv2quickstartpipelinejson"></a>Трубопровод с коммутатором (Adfv2'uickStartPipeline.json)
