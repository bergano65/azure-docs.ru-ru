---
title: Копирование данных из источника REST с помощью фабрики данных Azure
...
- **РАЗъем REST** специально поддерживает копирование данных из RESTful AIS; 
...
- Извлечение данных с помощью одной из следующих аутентификаций: **Anonymous**, **Basic**, **AAD-сервис и** **управляемые идентификаторы для ресурсов Azure.**
...
|:--- |:--- |:--- |
| type | Свойство **типа** должно быть установлено на **RestService.** | Да |
| url | Базовый URL-адрес службы REST. | Да |
| enableServerCertificateValidation | Следует ли проверять сервер стороне SSL-сертификат при подключении к конечной точке. | нет<br /> (значение по умолчанию **true**) |
| authenticationType | Тип проверки подлинности, используемый для подключения к службе REST. Допустимые значения: **Anonymous**, **Basic**, **AadServicePrincipal** и **ManagedServiceIdentity**. Обратитесь к соответствующим разделам ниже, в которых описываются дополнительные свойства и примеры. | Да |
| connectVia | [Среда выполнения интеграции](concepts-integration-runtime.md), используемая для подключения к хранилищу данных. Узнайте больше из [раздела Prerequisites.](#prerequisites) Если не указано другое, это свойство по умолчанию использует интегрированную среду выполнения Azure. |нет |
...
| relativeUrl | Относительный URL-адрес ресурса, который содержит данные. Если свойство не задано, используется только URL-адрес, указанный в определении связанной службы. Разъем HTTP копирует данные `[URL specified in linked service]/[relative URL specified in dataset]`из объединенного URL:: . | нет |

Если вы `requestMethod`устанавливали, `additionalHeaders` `requestBody` и `paginationRules` в наборе данных, он по-прежнему поддерживается как есть, в то время как вам предлагается использовать новую модель в источнике активности в будущем.
...
>[!NOTE]
>РАЗъем REST игнорирует любой заголовок `additionalHeaders`"Accept", указанный в. Как разъем REST только поддержка ответа в JSON, `Accept: application/json`он будет автоматически генерировать заголовок .

**Пример 1: Использование метода Get с pagination**
...
Обычно REST API ограничивает размер полезной нагрузки ответа на один запрос разумным числом; при возвращении большого объема данных он разделяет результат на несколько страниц и требует, чтобы абоненты отправляли последовательные запросы, чтобы получить следующую страницу результата. Обычно запрос одной страницы является динамическим и состоит из сведений, возвращенных из ответа предыдущей страницы.

Этот универсальный соединитель REST поддерживает следующие шаблоны разбиения на страницы: 

* Абсолютный или относительный URL-адрес следующего запроса - значение свойства в текущем органе ответа
* Абсолютное или относительное значение URL-адреса следующего запроса в текущих заголовках ответов
...
**Правила pagination** определяются как словарь в наборе данных, который содержит одну или несколько чувствительных к случаям ключевых значений пар. Эта конфигурация будет использоваться для создания запроса, начиная со второй страницы. Разъем перестанет итерировать, когда он получает код статуса HTTP 204 (Без содержимого), или любое из выражений JSONPath в "paginationRules" возвращаетнул.

**Поддерживаемые ключи** в правилах разбиения на страницы

| Ключ | Описание |
|:--- |:--- |
| AbsoluteUrl | Указывает URL-адрес для выполнения следующего запроса. Это может быть **либо абсолютный URL, либо относительный URL.** |
...
Соответствующая конфигурация источника активности `paginationRules` копии REST, особенно следующее:
...
В этом разделе описывается, как использовать шаблон решения для копирования данных из разъема REST в хранилище озер Azure Data в формате JSON с помощью OAuth. 

### <a name="about-the-solution-template"></a>О шаблоне решения

Шаблон содержит два действия:
- **Веб-активность** получает маркер носителя, а затем переносит его в последующее действие Copy в качестве авторизации.
- **Копирование** действий копирует данные из REST в Хранилище озер данных Azure.

Шаблон определяет два параметра.
- **SinkContainer** — это путь корневой папки, на котором данные копируется в хранилище озер Лазурных данных. 
- **SinkDirectory** — это путь каталога под корнем, на который копируется данные в хранилище озер Лазурных данных. 

### <a name="how-to-use-this-solution-template"></a>Использование шаблона решения

1. Перейдите к копии из REST или HTTP с помощью шаблона **OAuth.** Создайте новое соединение для подключения к источнику. 
    ![Создание новых подключений](media/solution-template-copy-from-rest-or-http-using-oauth/source-connection.png)

    Ниже приведены основные шаги для новых связанных настроек службы (REST):
    
     1. В **базовом URL**укажите параметр URL для собственного источника службы REST. 
     2. Для **типа аутентификации**выберите *Anonymous*.
        ![Новое соединение REST](media/solution-template-copy-from-rest-or-http-using-oauth/new-rest-connection.png)

2. Создайте новое соединение для destination Connection.  
    ![Новое соединение Gen2](media/solution-template-copy-from-rest-or-http-using-oauth/destination-connection.png)
...
4. Вы увидите, что трубопровод создан в ![следующем примере: Pipeline](media/solution-template-copy-from-rest-or-http-using-oauth/pipeline.png)

5. Выберите **веб-активность.** В **настройках**укажите соответствующий **URL,** **Метод,** **Заголовки**и **Тело** для извлечения маркера oAuth bearer из API входа в службу, из которой требуется скопировать данные. Заполнитель шаблона демонстрирует образец Azure Active Directory (AAD) OAuth. Заметка AAD аутентификация ведется при поддержке разъема REST, вот только пример для потока OAuth. 

    | Свойство | Описание |
    |:--- |:--- |:--- |
    | URL-адрес |Укажите URL-адрес, чтобы получить токен oAuth bearer от. например, в образце здесь этоhttps://login.microsoftonline.com/microsoft.onmicrosoft.com/oauth2/token |. 
    | Метод | Метод HTTP. Разрешенные значения **почтамт** и **получают.** | 
    | Заголовки | Заголовок определяется пользователем, который ссылается на одно имя заголовка в запросе HTTP. | 
    | Текст | Текст HTTP-запроса. | 

    ![Pipeline](media/solution-template-copy-from-rest-or-http-using-oauth/web-settings.png)

6. В активности **данных Copy,** выберите вкладку *«Источник»,* можно увидеть, что маркер нососа (access_token), извлеченный из предыдущего шага, будет передан активности копирования данных в качестве **авторизации** под дополнительными заголовками. Затвердите настройки следующих свойств перед запуском конвейера.
   ...
   ![Копирование источника аутентификации](media/solution-template-copy-from-rest-or-http-using-oauth/copy-data-settings.png)

7. Выберите **отключать,** введите **параметры,** а затем выберите **Finish.**
   ![Выполнение конвейера](media/solution-template-copy-from-rest-or-http-using-oauth/pipeline-run.png) 

8. Когда запуск конвейера завершается успешно, можно увидеть результат, ![аналогичный следующему примеру: результат выполнения трубопровода](media/solution-template-copy-from-rest-or-http-using-oauth/run-result.png) 

9. Нажмите на значок "Выход" в столбце WebActivity в **действиях,** вы увидите access_token, возвращенную службой.

   ![Выход токенов](media/solution-template-copy-from-rest-or-http-using-oauth/token-output.png) 

10. Нажмите на значок "Ввод" в столбце CopyActivity in **Actions,** вы увидите access_token, извлеченные WebActivity, передается CopyActivity для проверки подлинности. 

    ![Ввод токенов](media/solution-template-copy-from-rest-or-http-using-oauth/token-input.png)
        
    >[!CAUTION] 
    >Чтобы избежать входа токена в простой текст, включите "Безопасный выход" в веб-активности и "Безопасный вход" в действие Copy.
