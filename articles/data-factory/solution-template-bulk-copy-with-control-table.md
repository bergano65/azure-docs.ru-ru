...
Последние три параметра, определяющие путь в целевом хранилище, отображаются только в том случае, если выбранное назначение является хранилищем на основе файлов. Если в качестве целевого хранилища вы выбрали "Azure Synapse Analytics (ранее — Хранилище данных SQL)", эти параметры не требуются. Но имена таблиц и схемы в Azure синапсе Analytics должны совпадать с именами в базе данных источника.
...
9. Используемых Если в качестве назначения данных выбрано "Azure синапсе Analytics (ранее SQL DW)", необходимо ввести подключение к хранилищу BLOB-объектов Azure для промежуточного хранения в соответствии с требованиями Azure синапсе Analytics polybase. Шаблон автоматически создаст путь к контейнеру для хранилища BLOB-объектов. Проверьте, создан ли контейнер, после выполнения конвейера.
