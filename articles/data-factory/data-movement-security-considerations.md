...
> [!NOTE]
> Во время передачи данных в базу данных и из нее все подключения к Базе данных SQL Azure и хранилищу данных SQL Azure должны быть зашифрованы (с помощью SSL или TLS). Разрабатывая конвейер с помощью JSON, добавьте свойство "encryption" и в строке подключения задайте для него значение **true**. Для хранения Azure можно использовать **HTTPS** в строке соединения.
...
Учетные данные могут храниться на фабрике данных или [ссылаться на фабрику данных](store-credentials-in-key-vault.md) во время выполнения из Azure Key Vault. При хранении учетных данных на фабрике данных они всегда хранятся в зашифрованном виде на автономном времени выполнения интеграции. 
...
- **Храните учетные данные локально, не пропуская учетные данные через бэкэнд Azure в автономное время выполнения интеграции.** Если вы хотите шифровать и хранить учетные данные локально на автономном времени выполнения интеграции без необходимости передавать учетные данные через бэкэнд фабрики данных, выполните последующие действия в [учетных данных Encrypt для локальных хранилищ данных в Azure Data Factory.](encrypt-credentials-self-hosted-integration-runtime.md) Этот вариант поддерживают все соединители. Локальная среда выполнения интеграции использует Windows [DPAPI](https://msdn.microsoft.com/library/ms995355.aspx) для шифрования конфиденциальных данных и учетных данных. 
   ...
   Используйте **New-AzDataFactoryV2LinkedServiceEncryptedCredentialcmcredential** cmdlet для шифрования связанных учетных данных службы и конфиденциальных данных в связанной службе. Затем можно использовать возвращенный JSON (с элементом **EncryptedCredential** в строке соединения) для создания подключенной службы с помощью cmdlet **Set-AzDataFactoryV2LinkedService.**  
...
По умолчанию PowerShell использует порт 8060 на машине с автономной интеграцией времени выполнения для безопасной связи. При необходимости этот порт можно изменить.  
...
| Источник      | Назначение                              | Конфигурация сети                    | Настройка среды выполнения интеграции                |
| ----------- | ---------------------------------------- | ---------------------------------------- | ---------------------------------------- |
| В локальной среде | Виртуальные машины и облачные службы, развернутые в виртуальных сетях | VPN-подключение IPSec (типа "точка — сеть" или "сеть — сеть") | Самохлаженное время выполнения интеграции должно быть установлено на виртуальной машине Azure в виртуальной сети.  |
| В локальной среде | Виртуальные машины и облачные службы, развернутые в виртуальных сетях | ExpressRoute (частный пиринг)           | Самохлаженное время выполнения интеграции должно быть установлено на виртуальной машине Azure в виртуальной сети.  |
| В локальной среде | Службы на базе Azure, у которых есть общедоступная конечная точка | ExpressRoute (Microsoft peering)            | Самохозняционное время выполнения интеграции может быть установлено на месте или на виртуальной машине Azure. |
...
### <a name="firewall-configurations-and-allow-list-setting-up-for-ip-addresses"></a><a name="firewall-configurations-and-allow-list-setting-up-for-ip-address-of-gateway"></a>Конфигурации брандмауэра и позволяют создавать список для IP-адресов
...
> [!NOTE] 
> Возможно, вам придется управлять портами или настроить список разрешений для доменов на корпоративном уровне брандмауэра, как того требуют соответствующие источники данных. В этой таблице в качестве примеров используются база данных SQL Azure, хранилище данных SQL Azure и Azure Data Lake Store.   
...
#### <a name="ip-configurations-and-allow-list-setting-up-in-data-stores"></a>Конфигурации IP и позволяет создавать список в хранилищах данных
Некоторые хранилища данных в облаке также требуют, чтобы вы позволили IP-адрес машины получить доступ к магазину. Убедитесь, что IP-адрес самохлаженной интеграционной интеграции работает машина допускается или настроен в брандмауэре надлежащим образом.

Следующие облачные хранилища данных требуют, чтобы вы разрешали IP-адрес самохаудалой интеграционной интеграционной машины. Некоторые из этих хранилики данных, по умолчанию, могут не требовать списка разрешений. 
...
Локальная среда выполнения интеграции устанавливает HTTP-подключения для доступа к Интернету. Для установки этих подключений для локальной среды выполнения интеграции должны быть открыты исходящие порты 443. Открыть входящий порт 8060 только на уровне машины (не корпоративного уровня брандмауэра) для применения менеджера учетных данных. Если база данных SQL Azure или хранилище данных SQL Azure используются в качестве исходного или целевого объектов, вам также необходимо открыть порт 1433. Для получения дополнительной информации ознакомьтесь [с конфигурациями Firewall и разрешите настройку списка для раздела IP-адресов.](#firewall-configurations-and-allow-list-setting-up-for-ip-address-of-gateway) 
