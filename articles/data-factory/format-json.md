...
| type<br/>(*в `compression` разделе *) | Кодек сжатия, используемый для чтения и записи JSON-файлов. <br>Допустимые значения: **bzip2**, **gzip**, **Deflate**, **ZipDeflate**, **таргзип**, **привязки**или **lz4**. Значение по умолчанию не сжато.<br>**Примечание** . в настоящее время действие копирования не поддерживает "привязку" & "lz4", а поток данных сопоставления не поддерживает "ZipDeflate".<br>**Обратите внимание** , что при использовании действия **ZipDeflate**копирования для распаковки / файлов**таргзип** ZipDeflate и записи в файловый хранилище данных файлы по умолчанию извлекаются в папку: `<path specified in dataset>/<folder named as source compressed file>/` , с помощью команды " `preserveZipFileNameAsFolder` / `preserveCompressionFileNameAsFolder` [Источник действия копирования](#json-as-source) " можно указать, следует ли сохранять имя ZIP-файла в виде структуры папок. | Нет.  |
