...
| name | Описание | Обязательно | Допустимые значения | Свойство сценария потока данных |
| ---- | ----------- | -------- | -------------- | ---------------- |
...
| Один документ | Сопоставление потоков данных считывает один документ JSON из каждого файла | Нет | `true` или `false` | сингледокумент |
| Имена столбцов без кавычек | Если выбраны **имена столбцов без кавычек** , потоки данных сопоставления СЧИТЫВАЮТ столбцы JSON, которые не заключены в кавычки. | Нет | `true` или `false` |  ункуотедколумннамес |
| Содержит комментарии | SELECT **содержит комментарии** , если данные JSON имеют комментарии в стиле C или C++ | Нет | `true` или `false` | аскомментс |
| Одинарные кавычки | Считывает столбцы JSON, которые не заключены в кавычки | Нет | `true` или `false` | синглекуотед |
| Обратная косая черта | Выберите **обратную косую черту** , если символы обратной косой черты используются для экранирования символов в данных JSON. | Нет | `true` или `false` | баккслашескапе |
