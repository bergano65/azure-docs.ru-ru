...
| type | Тип команды экспорта, установленный в значение _ * Сновфлакикспорткопикомманд * *. | Да |
...
- **Связанная служба приемника** — это [**хранилище BLOB-объектов Azure**](connector-azure-blob-storage.md) с проверкой подлинности **подписанного общего доступа** . Если вы хотите напрямую копировать данные в Azure Data Lake Storage 2-го поколения в следующем поддерживаемом формате, можно создать связанную службу BLOB-объектов Azure с проверкой подлинности SAS в соответствии с учетной записью ADLS 2-го поколения, чтобы не использовать [промежуточное копирование из снежинки](#staged-copy-from-snowflake).
...
| type | Тип команды импорта, установленный в значение _ * Сновфлакеимпорткопикомманд * *. | Да |
...
- **Исходная связанная служба** — это [**хранилище BLOB-объектов Azure**](connector-azure-blob-storage.md) с проверкой подлинности **подписанного общего доступа** . Если вы хотите напрямую копировать данные из Azure Data Lake Storage 2-го поколения в следующем поддерживаемом формате, можно создать связанную службу BLOB-объектов Azure с проверкой подлинности SAS в соответствии с учетной записью ADLS 2-го поколения, чтобы не использовать  [промежуточное копирование в снежинки](#staged-copy-to-snowflake).
...
| Название | Описание | Обязательный | Допустимые значения | Свойство сценария потока данных |
| ---- | ----------- | -------- | -------------- | ---------------- |
| Update - метод | Укажите, какие операции разрешены в назначении «снежинка».<br>Для обновления, Upsert или удаления строк необходимо [Преобразование «изменение строки](data-flow-alter-row.md) » для добавления тегов в строки для этих действий. | Да | `true` или `false` | удаляемые <br/>Insertable <br/>обновляемые <br/>упсертабле |
