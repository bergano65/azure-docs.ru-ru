...
-------- | ----------- | -------------- | --------
name | Имя веб-действия | Строковое | Да
type | Необходимо задать значение **WebActivity**. | Строковое | Да
...
body | Представляет полезные данные, отправляемые конечной точке.  | Строка (или выражение с типом результата "строка"). <br/><br/>Просмотрите схему полезных данных запроса в разделе [Схема полезных данных запроса](#request-payload-schema). | Требуется для методов POST или PUT.
