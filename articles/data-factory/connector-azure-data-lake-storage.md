...
>[!NOTE]
>- Если вы хотите использовать общедоступную среду выполнения интеграции Azure для подключения к Data Lake Storage 2-го поколения, используя параметр **разрешить доверенным службам Майкрософт доступ к этой учетной записи хранения** , включенной в брандмауэре службы хранилища Azure, необходимо использовать [управляемую идентификацию аутентификации](#managed-identity).
>- При использовании инструкций Polybase или COPY для загрузки данных в Azure синапсе Analytics, если в исходной или промежуточной Data Lake Storage 2-го поколения настроена конечная точка виртуальной сети Azure, необходимо использовать управляемую проверку подлинности идентификации, как это требуется для синапсе. Дополнительные предварительные требования к конфигурации см. в разделе [Проверка подлинности с использованием управляемого удостоверения](#managed-identity).
...
| сервицепринЦипалкредентиалтипе | Тип учетных данных, используемый для проверки подлинности субъекта-службы. Допустимые значения: **сервицепринЦипалкэй** и **сервицепринЦипалцерт**. | Да |
| сервицепринЦипалкредентиал | Учетные данные субъекта-службы. <br/> При использовании **сервицепринЦипалкэй** в качестве типа учетных данных укажите ключ приложения. Пометьте это поле как **SecureString** , чтобы безопасно хранить его в фабрике данных, или [сослаться на секрет, хранящийся в Azure Key Vault](store-credentials-in-key-vault.md). <br/> При использовании **сервицепринЦипалцерт** в качестве учетных данных сослаться на сертификат в Azure Key Vault. | Да |
| servicePrincipalKey | Укажите ключ приложения. Пометьте это поле как **SecureString** , чтобы безопасно хранить его в фабрике данных, или [сослаться на секрет, хранящийся в Azure Key Vault](store-credentials-in-key-vault.md). <br/> Это свойство по-прежнему поддерживается "как есть" для `servicePrincipalId`  +  `servicePrincipalKey` . Когда ADF добавляет новую проверку подлинности с помощью сертификата субъекта-службы, Новая модель для проверки подлинности на базе субъекта — `servicePrincipalId`  +  `servicePrincipalCredentialType`  +  `servicePrincipalCredential` | Нет |
...
Вы также можете сохранить ключ субъекта-службы в Azure Key Vault.
...
**Пример. Использование проверки подлинности сертификата субъекта-службы**
```json
{
    "name": "AzureDataLakeStorageGen2LinkedService",
    "properties": {
...
>[!IMPORTANT]
>Если для загрузки данных из Data Lake Storage 2-го поколения в Azure синапсе Analytics используется инструкция Polybase или COPY, то при использовании проверки подлинности управляемого удостоверения для Data Lake Storage 2-го поколения убедитесь, что в [этом руководстве](../azure-sql/database/vnet-service-endpoint-rule-overview.md#impact-of-using-virtual-network-service-endpoints-with-azure-storage)вы также выполните шаги 1 – 3. Эти действия позволят зарегистрировать сервер в Azure AD и назначить серверу роль участника данных BLOB-объекта хранилища. Фабрика данных обрабатывает остальные компоненты. Если вы настраиваете хранилище BLOB-объектов с конечной точкой виртуальной сети Azure, вам также нужно **разрешить доверенным службам Майкрософт доступ к этой учетной записи хранения** , включенной в меню " **брандмауэры** учетных записей хранения Azure" и "Параметры виртуальных сетей" в соответствии с требованиями синапсе.
...
| ВАРИАНТ 1. Статический путь<br> | Копирование из указанной файловой системы или пути к папке либо файлу, которые указаны в наборе данных. Если вы хотите скопировать все файлы из файловой системы или папки, дополнительно укажите для `wildcardFileName` значение `_`. |  |
