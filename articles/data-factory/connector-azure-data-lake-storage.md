...
- [Сохранять списки управления доступом](#preserve-acls) при копировании из Azure Data Lake Storage 1-го поколения или Gen2.
...
| servicePrincipalKey | Укажите ключ приложения. Пометьте это поле как `SecureString` , чтобы безопасно хранить его в фабрике данных. Или можно [сослаться на секрет, хранящийся в Azure Key Vault](store-credentials-in-key-vault.md). | Да |
...
Следующие свойства поддерживаются для Data Lake Storage 2-го поколения в разделе `location` "Параметры" в наборе данных на основе формата:

| Свойство   | Описание                                                  | Обязательный |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | Свойство `location` Type в наборе данных должно иметь значение **азуреблобфслокатион**. | Да      |
| Системой | Data Lake Storage 2-го поколения имя файловой системы.                              | Нет       |
...
Следующие свойства поддерживаются для Data Lake Storage 2-го поколения в разделе `storeSettings` параметры в источнике копирования на основе формата:
...
Следующие свойства поддерживаются для Data Lake Storage 2-го поколения в разделе `storeSettings` параметры в приемнике копирования на основе формата:
...
## <a name="preserve-acls-from-data-lake-storage-gen1gen2"></a><a name="preserve-acls"></a>Сохранение списков управления доступом из Data Lake Storage 1-го поколения или Gen2

При копировании файлов из Azure Data Lake Storage 1-го поколения или Gen2 в Gen2 можно сохранить список управления доступом (ACL) POSIX вместе с данными. Дополнительные сведения см. в статье [сохранение списков управления доступом из Data Lake Storage 1-го поколения или Gen2 в Gen2](copy-activity-preserve-metadata.md#preserve-acls).
