---
title: Копирование и преобразование данных в Azure Data Lake Storage 2-го поколения с помощью фабрики данных
...
>[!IMPORTANT]
>Если вы включили параметр **разрешить доверенным службам Майкрософт доступ к этой учетной записи хранения** в параметрах брандмауэра службы хранилища Azure и хотите использовать среду выполнения интеграции Azure для подключения к Data Lake Storage 2-го поколения, необходимо использовать [управляемое удостоверение. Проверка подлинности](#managed-identity) для ADLS 2-го поколения.
...
| accountKey | Ключ учетной записи для Data Lake Storage 2-го поколения. Пометьте это поле как SecureString, чтобы безопасно хранить его в фабрике данных, или [добавьте ссылку на секрет, хранящийся в Azure Key Vault](store-credentials-in-key-vault.md). |Yes |
...
>[!NOTE]
>Если вы используете пользовательский интерфейс фабрики данных для создания, а субъект-служба не настроен с ролью "читатель данных BLOB-объекта хранилища/участник" в IAM, при выполнении теста подключения или просмотре или переходе по папкам выберите "проверить подключение к пути к файлу" или "Обзор по указанному пути". Укажите файловую систему или путь с разрешением EXECUTE, чтобы продолжить.
...
| servicePrincipalKey | Укажите ключ приложения. Пометьте это поле как `SecureString` для безопасного хранения в фабрике данных. Или можно [сослаться на секрет, хранящийся в Azure Key Vault](store-credentials-in-key-vault.md). | Yes |
| клиенте | Укажите сведения о клиенте (доменное имя или идентификатор клиента), в котором находится приложение. Извлеките его, наведя указатель мыши на правый верхний угол портал Azure. | Yes |
...
>[!NOTE]
>Если вы используете пользовательский интерфейс фабрики данных для создания, а управляемое удостоверение не задано с помощью роли "читатель данных BLOB-объекта хранилища/участник" в IAM, при выполнении тестового подключения или просмотре папок (Просмотр по указанному пути) выберите "проверить подключение к пути к файлу" или "Обзор с указанной траекторией". Укажите файловую систему или путь с разрешением EXECUTE, чтобы продолжить.
...
| Свойство   | ОПИСАНИЕ                                                  | обязательные |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | Свойство Type в разделе `location` в наборе данных должно иметь значение **азуреблобфслокатион**. | Yes      |
...
| Свойство                 | ОПИСАНИЕ                                                  | обязательные                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| type                     | Свойство Type в разделе `storeSettings` должно иметь значение **азуреблобфсреадсеттинг**. | Yes                                           |
...
| Свойство                 | ОПИСАНИЕ                                                  | обязательные |
| ------------------------ | ------------------------------------------------------------ | -------- |
| type                     | Свойство Type в разделе `storeSettings` должно иметь значение **азуреблобфсвритесеттинг**. | Yes      |
...
При настройке исходного кода как Data Lake Storage 1-го поколения с параметром двоичного копирования или двоичного формата и приемника, как Data Lake Storage 2-го поколения с параметром двоичного копирования или двоичного формата, параметр **сохранить** можно найти на странице **копирование данных параметры средства** или на **действие копирования** > вкладка " **Параметры** " для создания действия.
