...
>[!IMPORTANT]
>При использовании PolyBase для загрузки данных из ADLS Gen2 в хранилище данных SQL, при использовании ADLS Gen2 управляемых удостоверений проверки подлинности, убедитесь, что вы также следовать инструкциям, #1 и 2 # в [в этом руководстве](../sql-database/sql-database-vnet-service-endpoint-rule-overview.md#impact-of-using-vnet-service-endpoints-with-azure-storage) для регистрации сервера базы данных SQL с Azure Active Directory (AAD) и назначение хранилища больших двоичных объектов данных участника роли RBAC к серверу базы данных SQL; остальные будут обрабатываться ADF. Если ваш Gen2 ADLS настроен с конечной точкой службы виртуальной сети, использовать PolyBase для загрузки данных из нее, необходимо использовать управляемое удостоверение проверки подлинности.
...
| Свойство   | ОПИСАНИЕ                                                  | Обязательно для заполнения |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | Свойство типа в списке `location` в наборе данных должно быть присвоено **AzureBlobFSLocation**. | Yes      |
| fileSystem | Имя 2-го поколения ADLS файловой системе.                              | Нет        |
...
>[!TIP]
>Для копирования данных из Gen1 хранилища Озера данных Azure в 2-го поколения в целом, см. в разделе [копирование данных из Gen1 хранилища Озера данных Azure 2-го поколения с фабрикой данных Azure](load-azure-data-lake-storage-gen2-from-gen1.md) Пошаговое руководство и рекомендации.

После копирования файлов из хранилища Озера данных Azure (ADLS) Gen1 Gen2, вы можете сохранить списки управления доступом (ACL) POSIX вместе с данными. Управление доступом в подробности, см. в разделе [управление доступом в Azure Data Lake хранилища Gen1](../data-lake-store/data-lake-store-access-control.md) и [управление доступом в Azure Data Lake хранилища Gen2](../storage/blobs/data-lake-storage-access-control.md).

Следующие списки управления доступом могут быть сохранены с помощью действия копирования фабрики данных Azure, можно выбрать один или несколько типов:

- **ACL**: Скопировать и сохранить **списки управления доступом POSIX** над файлами и каталогами. Он будет копировать полный существующими списками управления доступом из источника в приемник. 
- **Владелец**: Скопировать и сохранить **владелец** файлов и каталогов. Необходим доступ суперпользователя приемник ADLS Gen2.
- **Группа**: Скопировать и сохранить **группа владельцев** файлов и каталогов. Необходим доступ суперпользователя в приемник ADLS Gen2 или владельца (если владелец также является участником целевой группы).

Если указать для копирования из папки, фабрика данных реплицирует списки ACL для этого данной папки, а также файлы и каталоги в нем (если `recursive` задано значение true). Если указать для копирования из одного файла, списки управления доступом на этот файл копируется.

>[!IMPORTANT]
>Если вы решили сохранить списки управления доступом, убедитесь, что высокий, вы предоставили достаточно разрешений для ADF работать с вашей учетной записи ADLS Gen2 приемника. Например использование ключа проверки подлинности учетной записи, или назначьте роль владельца данных хранилища BLOB-объектов для участника или управляемые удостоверения службы.

При настройке источника как ADLS Gen1 с двоичное копирование параметр/двоичный формат и приемником как ADLS Gen2 с двоичное копирование параметр/двоичный формат, можно найти **Сохранить** в диалоговом окне **страницу параметров инструмента копирования данных** или в **действие копирования "->" Параметры** вкладку разработка действий.

![Поколение 1 ADLS на уровень Gen2 сохранить ACL](./media/connector-azure-data-lake-storage/adls-gen2-preserve-acl.png)
