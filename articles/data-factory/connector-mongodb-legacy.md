...
|:--- |:--- |:--- |
| type |Свойству type необходимо задать значение **MongoDB** |Да |
...
| authenticationType | Тип проверки подлинности, используемый для подключения к базе данных MongoDB.<br/>Допустимые значения: **Basic** (обычная) и **Anonymous** (анонимная). |Да |
...
| Свойство | ОПИСАНИЕ | Обязательно для заполнения |
|:--- |:--- |:--- |
| type | Для свойства type набора данных необходимо задать следующее значение: **MongoDbCollection** | Да |
...
| Свойство | ОПИСАНИЕ | Обязательно для заполнения |
|:--- |:--- |:--- |
| type | Свойству type источника действия копирования необходимо задать значение **MongoDbSource** | Да |
...
>
> В настоящее время не поддерживаются следующие типы данных MongoDB: DBPointer, JavaScript, макс./мин. ключ, регулярное выражение, символ, метка времени, не определено.
...
| --- | --- | --- | --- | --- |
| 1111 |ABC |[{invoice_id:"123", item:"toaster", price:"456", discount:"0.2"}, {invoice_id:"124", item:"oven", price: "1235", discount: "0.2"}] |Серебряный |[5,6] |
| 2222 |XYZ |[{invoice_id:"135", item:"fridge", price: "12543", discount: "0.0"}] |Золотой |[1,2] |
...
| _№ | ExampleTable_Invoices_dim1_idx | счет_№ | item | цена | Скидка |
| --- | --- | --- | --- | --- | --- |
