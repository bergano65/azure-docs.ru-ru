---
title: Действие "получить метаданные" в фабрике данных Azure | Документация Майкрософт
...
| [Файлы Azure](connector-azure-file-storage.md) | √/√ | √/√ | √ | √/√ | √/√ | √ | п | √ | √ | √/√ |
...
- Для хранилища BLOB-объектов `lastModified` Azure применяется к контейнеру и большому двоичному объекту, но не к виртуальной папке.
...
| itemType | Тип файла или папки. Возвращаемое значение `File` — `Folder`или. |
...
| exists (существует)| Существует ли файл, папка или таблица. Обратите внимание `exists` , что если в списке полей получить метаданные указано значение, действие не будет выполняться, даже если файл, папка или таблица не существуют. `exists: false` Вместо этого в выходных данных возвращается. |
