...
Чтобы использовать Азуредтексек, скачайте и установите последнюю версию SSMS, которая имеет версию 18,3 или более позднюю. Скачайте его с [этого веб-сайта](/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-2017).
...
Вызов Азуредтексек предлагает аналогичные параметры, такие как вызов dtexec. Дополнительные сведения см. в статье [dtexec Utility](/sql/integration-services/packages/dtexec-utility?view=sql-server-2017). В настоящее время поддерживаются следующие параметры:
