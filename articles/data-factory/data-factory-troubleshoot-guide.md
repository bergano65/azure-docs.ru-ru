...
- **Рекомендация**: укажите абсолютные пути для схем адресации рабочей области или `dbfs:/folder/subfolder/foo.py` для файлов, хранящихся в файловой системе "кирпичи".
...
- **Рекомендация**: обновление функции Azure для возврата допустимых полезных данных JSON, например, функция C# может возвращать (ActionResult) New окобжектресулт (\""\"{\"ID\": 123}");
...
- **Рекомендация**. Попробуйте предоставить субъект-службу с разрешениями на создание кластера HDInsight в указанной подписке и повторите попытку. Убедитесь, что [Управление удостоверениями настроено правильно](https://docs.microsoft.com/azure/hdinsight/hdinsight-managed-identities). Если это решение не является приемлемым, обратитесь в службу поддержки ADF для получения дополнительной помощи.
...
- **Рекомендация**: исправьте учетные данные и повторно разверните связанную службу. Сначала убедитесь, что учетные данные работают в HDInsight, открыв универсальный код ресурса (URI) кластера в любом браузере и пытаясь войти в систему. Если учетные данные не работают, их можно сбросить с портал Azure. Для кластера ESP можно [сбросить пароль с помощью самостоятельного сброса пароля](https://docs.microsoft.com/azure/active-directory/user-help/active-directory-passwords-update-your-own-password).
...
- **Рекомендация**: в случае ошибки 502 в большинстве случаев это связано с тем, что работа сервера Ambari завершена. Вы можете перезапустить службы Ambari, перезагрузив головной узел.  

    1. Подключитесь к одному из узлов в Hdinsight с помощью SSH.
    2. Найдите узел активного головного узла, выполнив команду ping хеаднодехост.
    3. Подключитесь к активному головному узлу, так как сервер Ambari находится на активном головном узле с помощью SSH.  
    4. Перезагрузите активный головной узел.

        Дополнительные сведения см. в документации по устранению неполадок Azure HDInsight, например: [AMBARI UI 502 Error](https://hdinsight.github.io/ambari/ambari-ui-502-error.html), [рпктимеаутексцептион for Apache Spark Thrift Server](https://docs.microsoft.com/azure/hdinsight/spark/apache-spark-troubleshoot-rpctimeoutexception), [Устранение ошибок шлюза в шлюзе приложений](https://docs.microsoft.com/azure/application-gateway/application-gateway-troubleshooting-502).
...
- **Рекомендация**. Попробуйте ограничить количество параллельных заданий, отправляемых в HDInsight. См. сведения о параллелизме действий фабрики данных, если задания отправляются одним действием. Измените триггеры, чтобы параллельные запуски конвейера были распределены по времени. Сведения об [HDInsight documentation](https://docs.microsoft.com/azure/hdinsight/hdinsight-hadoop-templeton-webhcat-debug-errors) ошибке см. `templeton.parallellism.job.submit` в документации по HDInsight.
...
- **Рекомендация**: сообщение об ошибке должно помочь определить причину проблемы. Исправьте конфигурацию JSON и повторите попытку. Дополнительные https://docs.microsoft.com/azure/data-factory/compute-linked-services#azure-hdinsight-on-demand-linked-service сведения см. в этой записи.
...
- **Рекомендация**. Убедитесь, что выбранная связанная служба имеет один из поддерживаемых типов для этого действия. Действия HDI поддерживают связанные службы AzureBlobStorage и Азуреблобфсстораже. Дополнительные сведения см. в статье [Сравнение вариантов хранения для использования с кластерами Azure HDInsight](https://docs.microsoft.com/azure/hdinsight/hdinsight-hadoop-compare-storage-options) .
...
1. Если приложение использует сертификаты TLS/SSL, добавьте сертификат Fiddler на устройство. Последовательно выберите **пункты Сервис** > **Fiddler параметры** > **HTTPS** > **действия** > **экспорт корневого сертификата на Рабочий стол**.
...
1. Выберите **файл** > **сохранить** > **все сеансы**.
