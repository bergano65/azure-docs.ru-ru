...
Дополнительные сведения см. в разделе [ Ветвления и создание цепочки действий в конвейере фабрики данных](tutorial-control-flow.md).
...
- **Диспетчеризация действий**. Для преобразования среда выполнения интеграции предоставляет возможность встроенного выполнения пакетов служб SSIS.
...
Связанные службы напоминают строки подключения, определяющие сведения о подключении, необходимые для подключения фабрики данных к внешним ресурсам. Представьте себе так: Связанная служба определяет соединение с источником данных, а DataSet представляет структуру данных. Например, связанная служба хранилища Azure определяет строку подключения для подключения к учетной записи хранения Azure. И набор данных BLOB-объектов Azure указывает контейнер больших двоичных объектов и папку, содержащую данные.
...
### <a name="which-data-factory-version-do-i-use-to-create-data-flows"></a>Какую версию фабрики данных использовать для создания потоков данных?
Используйте версию фабрики данных версии 2 для создания потоков данных.
  
### <a name="i-was-a-previous-private-preview-customer-who-used-data-flows-and-i-used-the-data-factory-v2-preview-version-for-data-flows"></a>Я был ранее закрытой предварительной версией клиента, который использовал потоки данных, и использовал предварительную версию фабрики данных версии 2 для потоков данных.
Теперь эта версия устарела. Использование фабрики данных версии 2 для потоков данных.
  
### <a name="what-has-changed-from-private-preview-to-limited-public-preview-in-regard-to-data-flows"></a>Что изменилось с закрытой предварительной версии до ограниченной общедоступной предварительной версии в отношении потоков данных?
Вам больше не придется приносить собственные кластеры Azure Databricks. Фабрика данных будет управлять созданием и остановкой кластеров. Наборы данных BLOB-объектов и Azure Data Lake Storage 2-го поколения наборы данных разделяются на текст с разделителями и наборы данных Apache Parquet. Вы по-прежнему можете использовать Data Lake Storage 2-го поколения и хранилище больших двоичных объектов для хранения этих файлов. Используйте соответствующую связанную службу для этих механизмов хранения.

### <a name="can-i-migrate-my-private-preview-factories-to-data-factory-v2"></a>Можно ли перенести собственные фабрики предварительной версии в фабрику данных версии 2?
...
Если корпорация Майкрософт предоставляет справку или устранение неполадок с потоками данных, укажите план кода DSL. Для этого выполните следующие действия.

1. В конструкторе потока данных выберите **код** в правом верхнем углу. Это приведет к отображению редактируемого кода JSON для потока данных.
2. В представлении кода выберите Plan ( **план** ) в правом верхнем углу. Этот переключатель переключится с JSON на отформатированный план скрипта DSL с поддержкой только для чтения.
3. Скопируйте и вставьте этот скрипт или сохраните его в текстовом файле.

### <a name="how-do-i-access-data-by-using-the-other-80-dataset-types-in-data-factory"></a>Разделы справки получить доступ к данным с помощью других типов наборов данных 80 в фабрике данных?
