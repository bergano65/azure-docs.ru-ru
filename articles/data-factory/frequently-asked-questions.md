...
### <a name="which-adf-version-do-i-use-to-create-data-flows"></a>Какая версия фабрики данных AZURE следует использовать для создания потоков данных
Создание потоков данных с помощью версии ADF V2
  
### <a name="i-was-a-previous-private-preview-customer-using-data-flows-and-i-used-the-adf-v2-wdata-flows-preview-version"></a>Я был предыдущих клиентам закрытой предварительной версии, с помощью обмена данными, и я использовал в предварительной версии ADF V2 w/данных потоков
Эта версия больше не используется. Используйте ADF V2 для потоков данных
  
### <a name="what-has-changed-from-private-preview-to-limited-public-preview-in-data-flows"></a>Что изменилось с закрытой предварительной версии на в потоках данных в ограниченной общедоступной предварительной версии?
Больше не придется вывести собственные кластеры Databricks. Создание кластера и демонтажа для управления ADF. Наборы данных больших двоичных объектов и наборов данных ADLS разделены разделителем и Parquet наборов данных. ADLS & Store BLOB-объектов по-прежнему можно использовать для хранения этих файлов. Используйте соответствующую связанную службу для этих подсистемы хранилища.

### <a name="can-i-migrate-my-private-preview-factories-to-adf-v2"></a>Можно ли перенести мой фабрик закрытой предварительной версии ADF V2

[Да, выполнить приведенные здесь инструкции](https://www.slideshare.net/kromerm/adf-mapping-data-flow-private-preview-migration)

### <a name="i-need-help-troubleshooting-my-data-flow-logic-what-do-you-need"></a>Мне нужна помощь, устранение неполадок моей логику потока данных, что необходимо?

Когда корпорация Майкрософт предоставляет помощь или Устранение неполадок с помощью потоки данных, укажите «DSL кода план». Для этого выполните следующие действия.

* В конструктор потоков данных щелкните «Код» в правом верхнем углу. При этом отобразится редактируемый код JSON для потока данных.
* В представлении кода нажмите кнопку «План» в правом верхнем углу. Переключатель план переключается из JSON только для чтения формате DSL сценарий плана.
* Скопируйте и вставьте этот сценарий или сохраните его в текстовый файл.

### <a name="how-do-i-access-data-using-the-other-80-dataset-types-in-adf"></a>Как получить доступ к данных с помощью 80 других типов наборов данных в ADF?

Поток данных в настоящее время позволяет базе данных SQL Azure, хранилище данных SQL Azure, текстовых файлов с разделителем из BLOB-объектов или ADLS и файлы Parquet из BLOB-объектов или ADLS в собственном коде источника и приемника. Использование действия копирования для промежуточного хранения данных из любого из других соединителей, а затем выполните операцию потока данных для преобразования данных, после его размещается. Например конвейер сначала скопирует в BLOB-объектов и затем действием потока данных будет использовать набор данных в источнике для преобразования этих данных.
