...
1. Найдите тег **System.NET** по умолчанию, как показано в следующем коде:
    ...
    Тег прокси позволяет дополнительным свойствам указать обязательные параметры, такие как `scriptLocation` . Синтаксис см. в разделе [ \<proxy\> элемент (сетевые параметры)](https://msdn.microsoft.com/library/sa91de1e.aspx) .
