...
|------|----|-----------|
|`-rn`,<br/>`-RegisterNewNode`|"`<AuthenticationKey>`" ["`<NodeName>`"]|Регистрация узла локальной среды выполнения интеграции с указанным ключом проверки подлинности и именем узла.|
...
|`-erac`,<br/>`-EnableRemoteAccessInContainer`|"`<port>`" ["`<thumbprint>`"]|Включите удаленный доступ к текущему узлу, когда узел выполняется в контейнере.|
|`-dra`,<br/>`-DisableRemoteAccess`||Отключить удаленный доступ к текущему узлу. Удаленный доступ необходим для многоузловой установки. Командлет PowerShell **New-AzDataFactoryV2LinkedServiceEncryptedCredential** работает, даже если удаленный доступ отключен. Это поведение имеет значение true, если командлет выполняется на том же компьютере, где размещена локальная IR-узел.|
|`-k`,<br/>`-Key`|"`<AuthenticationKey>`"|Перезапишите или обновите предыдущий ключ проверки подлинности. Будьте внимательны с этим действием. Предыдущий узел IR может переходить в режим «вне сети», если ключ является новой средой выполнения интеграции.|
|`-gbf`,<br/>`-GenerateBackupFile`|"`<filePath>`" "`<password>`"|Создать файл резервной копии для текущего узла. Файл резервной копии содержит ключ узла и учетные данные хранилища данных.|
|`-ibf`,<br/>`-ImportBackupFile`|"`<filePath>`" "`<password>`"|Восстановите узел из файла резервной копии.|
|`-r`,<br/>`-Restart`||Перезапустите службу узла локальной среды выполнения интеграции.|
|`-s`,<br/>`-Start`||Запустите службу узла локальной среды выполнения интеграции.|
|`-t`,<br/>`-Stop`||Завершите работу службы узла локальной среды выполнения интеграции.|
|`-sus`,<br/>`-StartUpgradeService`||Запустите службу обновления локальной среды выполнения интеграции.|
|`-tus`,<br/>`-StopUpgradeService`||Останавливает службу обновления локальной среды выполнения интеграции.|
|`-tonau`,<br/>`-TurnOnAutoUpdate`||Включите автоматическое обновление среды выполнения интеграции с локальным размещением.|
|`-toffau`,<br/>`-TurnOffAutoUpdate`||Отключите автоматическое обновление среды выполнения интеграции с локальным размещением.|
|`-ssa`,<br/>`-SwitchServiceAccount`|"`<domain\user>`" ["`<password>`"]|Настройте DIAHostService для запуска от имени новой учетной записи. Используйте пустой пароль "" для системных учетных записей и виртуальных учетных записей.|
