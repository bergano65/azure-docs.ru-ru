...
## <a name="setting-up-a-self-hosted-ir-on-an-azure-vm-by-using-an-azure-resource-manager-template"></a>Настройка локальной среды IR на виртуальной Машине Azure с помощью шаблона Azure Resource Manager 
...
- На компьютере может быть установлен только один экземпляр локальной среды выполнения интеграции. Если у вас есть две фабрики данных, которым требуется доступ к локальным источникам данных, либо использовать [локальной IR, функции совместного использования](#sharing-the-self-hosted-integration-runtime-with-multiple-data-factories) совместно использовать локальную среду выполнения интеграции, или установите среду выполнения интеграции на двух локальные компьютеры, для каждой фабрики данных.  
...
- Версии поддерживаемых операционных систем, Windows 7 с пакетом обновления 1, Windows 8.1, Windows 10, Windows Server 2008 R2 SP1, Windows Server 2012, Windows Server 2012 R2, Windows Server 2016 и Windows Server 2019. Установка локальной среды выполнения интеграции на контроллер домена не поддерживается.
...
- Для автоматизации локальная среда IR операции установки, см. в статье [под разделом](#automation-support-for-self-hosted-ir-function).  
...
## <a name="automation-support-for-self-hosted-ir-function"></a>Поддержка модели автоматизации локальная IR-функция


> [!NOTE]
> Если вы планируете настроить локальную среду выполнения Интеграции на виртуальной машине Azure и хотите автоматизировать установку с помощью шаблонов Azure Resource Manager, см. в статье [разделе](#setting-up-a-self-hosted-ir-on-an-azure-vm-by-using-an-azure-resource-manager-template).

Можно использовать командную строку для настройке и администрировании среды существующих локальную среду выполнения интеграции Это может использоваться специально для автоматизации установки, регистрации локальную среду IR узлов. 

**Dmgcmd.exe** входит в локальную среду установки, обычно находится: C:\Program Files\Microsoft Integration Runtime\3.0\Shared\ folder. Это поддерживает различные параметры и могут вызываться с помощью командной строки с помощью пакетных сценариев для автоматизации. 
...
 *Сведения о (параметры и свойства):* 

| Свойство                                                    | ОПИСАНИЕ                                                  | Обязательно для заполнения |
| ----------------------------------------------------------- | ------------------------------------------------------------ | -------- |
| RegisterNewNode "`<AuthenticationKey>`"                     | Регистрация Integration Runtime (self-Hosted) с указанным ключом проверки подлинности | Нет       |
| EnableRemoteAccess "`<port>`" ["`<thumbprint>`"]            | Включить удаленный доступ на текущем узле для Настройка высокого уровня доступности кластера и/или при включении параметра учетных данных непосредственно в локальную среду IR (минуя службы ADF) с помощью  **Новый AzDataFactoryV2LinkedServiceEncryptedCredential** командлет с удаленного компьютера в одной сети. | Нет       |
| EnableRemoteAccessInContainer "`<port>`" ["`<thumbprint>`"] | Включить удаленный доступ для текущего узла, когда на узле выполняется в контейнере | Нет       |
| DisableRemoteAccess                                         | Отключите удаленный доступ для текущего узла. Удаленный доступ необходим для установки с несколькими узлами. New -**AzDataFactoryV2LinkedServiceEncryptedCredential** командлет PowerShell, по-прежнему работает, даже если удаленный доступ отключен до тех пор, пока он выполняется на той же машине, что узел локальной среды выполнения Интеграции. | Нет       |
| Ключ "`<AuthenticationKey>`"                                 | Перезаписать / update предыдущий ключ проверки подлинности. Будьте внимательны в том случае, это может привести к вашей локальной среде предыдущий узел среды выполнения Интеграции, отключиться от сети, если ключ является новой среды выполнения интеграции. | Нет       |
| GenerateBackupFile "`<filePath>`" "`<password>`"            | Создать файл резервной копии для текущего узла, файл резервной копии содержит узел ключ и данные хранилища учетных данных | Нет       |
| ImportBackupFile "`<filePath>`" "`<password>`"              | Восстановите узел из файла резервной копии                          | Нет       |
| Перезагрузить                                                     | Перезапустите службу Integration Runtime (self-Hosted) узла   | Нет       |
| Начало                                                       | Запустите службу Integration Runtime (self-Hosted) узла     | Нет       |
| Остановить                                                        | Остановить службу Integration Runtime (self-Hosted) обновления        | Нет       |
| StartUpgradeService                                         | Запустить службу Integration Runtime (self-Hosted) обновления       | Нет       |
| StopUpgradeService                                          | Остановить службу Integration Runtime (self-Hosted) обновления        | Нет       |
| TurnOnAutoUpdate                                            | Включить Integration Runtime (self-Hosted) автоматическое обновление        | Нет       |
| TurnOffAutoUpdate                                           | Отключить Integration Runtime (self-Hosted) автоматическое обновление       | Нет       |
| SwitchServiceAccount «< domain\user >» [«password»]           | Задайте выполнение DIAHostService как новой учетной записи. Используйте пустой пароль ("») для учетной записи system или виртуальной учетной записи | Нет       |
| LogLevel `<logLevel>`                                       | Задать уровень ведения журнала трассировки событий Windows (Off, ошибка, подробный или все). Обычно используется службой поддержки Майкрософт во время отладки. | Нет       |
...
Можно связать несколько узлов, установив программное обеспечение локальной среды выполнения интеграции из [Центра загрузки](https://www.microsoft.com/download/details.aspx?id=39717). Затем, регистр, он с помощью любого из ключей проверки подлинности получен **New AzDataFactoryV2IntegrationRuntimeKey** командлет, как описано в разделе [руководстве](tutorial-hybrid-copy-powershell.md).
...
> [!NOTE]
> Этот сертификат используется для шифрования порты на узел локальной среды выполнения Интеграции, используемый для **обмен данными между узлами** (для синхронизации состояния, которая включает в себя связанные службы учетные данные для синхронизации между узлами), так и при **с помощью PowerShell командлет для связанной службы учетных данных параметр** из локальной сети. Мы рекомендуем использовать этот сертификат, если ваша частная сетевая среда небезопасная или если вы хотите защитить связь между узлами в своей частной сети. Перемещение данных при передаче из локальной среды IR в другие хранилища данных всегда выполняется по зашифрованному каналу, независимо от того, установлен этот сертификат или нет. 
...
Если настроена, среда выполнения интеграции использует прокси-сервер для подключения к облачной службе, источника / назначения (те, с помощью протокола HTTP / протокол HTTPS). Это Select **изменить ссылку** во время начальной настройки. Появится диалоговое окно параметров прокси-сервера.
...
* При попытке регистрации локальной среды выполнения интеграции появляется следующая ошибка: "Failed to register this Integration Runtime node! Confirm that the Authentication key is valid and the integration service Host Service is running on this machine" (Зарегистрировать этот узел среды выполнения интеграции не удалось. Убедитесь, что используется действительный ключ проверки подлинности и что на этом компьютере запущена служба узла среды выполнения).
...
Если вы используете PowerShell для шифрования учетных данных с другого компьютера (в сети), отличный от установленной средой выполнения интеграции, вы можете включить **удаленный доступ из интрасети** параметр. При запуске PowerShell для шифрования учетных данных на одном компьютере, в котором установлена локальная среда выполнения интеграции, не удается включить **удаленный доступ из интрасети**.
