...
## <a name="setting-up-a-self-hosted-ir-on-an-azure-vm-by-using-an-azure-resource-manager-template"></a>Настройка самостоятельно размещенного IR на виртуальной машине Azure с помощью шаблона Azure Resource Manager 
...
- На компьютере может быть установлен только один экземпляр локальной среды выполнения интеграции. Если у вас есть две фабрики данных, которым требуется доступ к локальным источникам данных, то либо используйте [функцию совместного доступа](#sharing-the-self-hosted-integration-runtime-with-multiple-data-factories) к локальной среде для совместного размещения, либо установите локальную среду выполнения интеграции на двух локальных компьютерах, один для Каждая фабрика данных.  
...
- Задачи могут завершаться ошибкой, если данные извлекаются в форматах Parquet, ORC или Avro. Создание файла выполняется на компьютере с локальной интеграцией и требует, чтобы следующие необходимые компоненты работали должным образом (см. раздел [Формат Parquet в фабрике данных Azure](https://docs.microsoft.com/azure/data-factory/format-parquet#using-self-hosted-integration-runtime)).
    ...
    - Среда выполнения Java (JRE) версии 8 от поставщика JRE, например, [внедрения OpenJDK](https://adoptopenjdk.net/), гарантирующая, что задана переменная среды `JAVA_HOME`.
...
- Сведения об автоматизации автономных операций настройки ИК-связи см. в [разделе ниже](#automation-support-for-self-hosted-ir-function).  
...
11. В диспетчере конфигураций Microsoft Integration Runtime откройте страницу **Регистрация Integration Runtime (Self-hosted)** и выполните следующие действия.
    ...
    В. Выберите **Зарегистрировать**.

## <a name="automation-support-for-self-hosted-ir-function"></a>Поддержка автоматизации для самостоятельно размещенной ИК-функции


> [!NOTE]
> Если вы планируете настроить локальное IR на виртуальной машине Azure и хотите автоматизировать установку с помощью шаблонов Azure Resource Manager, ознакомьтесь с [разделом](#setting-up-a-self-hosted-ir-on-an-azure-vm-by-using-an-azure-resource-manager-template).
...
**Дмгкмд. exe** входит в локальную установку, которая обычно находится в папке: Папка C:\Program Files\Microsoft Integration Runtime\3.0\Shared\. Это поддерживает различные параметры и может вызываться через командную строку с помощью сценариев пакетной службы для автоматизации. 
...
| Енаблеремотеакцессинконтаинер "`<port>`" ["`<thumbprint>`"] | Включить удаленный доступ к текущему узлу, когда узел работает в контейнере | Нет       |
...
| Импортбаккупфиле "`<filePath>`" "`<password>`"              | Восстановить узел из файла резервной копии                          | Нет       |
