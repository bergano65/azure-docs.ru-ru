...
1. Фабрика данных Azure взаимодействует с локальной средой выполнения интеграции для планирования заданий и управления ими. Обмен данными осуществляется через канал управления, использующий общее подключение [ретранслятора служебной шины Azure](../azure-relay/relay-what-is-it.md#wcf-relay) . Когда необходимо запустить задание действия, фабрика данных помещает запрос в очередь вместе с любыми учетными данными. Это делается в том случае, если учетные данные еще не хранятся в локальной среде выполнения интеграции. Локальная среда выполнения интеграции запускает задание после опроса очереди.
...
- Рекомендуемая минимальная конфигурация для компьютера с локальной средой выполнения интеграции — процессор с тактовой частотой 2 ГГц с 4 ядрами, 8 ГБ ОЗУ и 80 ГБ свободного пространства на жестком диске.
...
1. Запустите управляемый файл удостоверений напрямую или сохраните его на жесткий диск и запустите его.
...
1. На странице **все готово для установки** нажмите кнопку **установить**.
