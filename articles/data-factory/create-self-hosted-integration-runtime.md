...
1. Найдите тег **System.NET** по умолчанию, как показано в следующем коде:
    ...
    Тег прокси позволяет дополнительным свойствам указать обязательные параметры `scriptLocation`, такие как. Синтаксис см. в разделе [ \<элемент прокси\> (сетевые параметры)](https://msdn.microsoft.com/library/sa91de1e.aspx) .
