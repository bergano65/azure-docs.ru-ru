...
- **Для создания графического интерфейса фабрики данных**. `s3:ListAllMyBuckets` разрешения для операций с `s3:ListBucket` / `s3:GetBucketLocation` контейнерами Amazon S3 дополнительно необходимы для таких операций, как проверка подключения и просмотр и Навигация по путям к файлам. Если вы не хотите предоставлять эти разрешения, пропустите проверку соединения на странице создания связанной службы и укажите путь непосредственно в параметрах набора данных.
