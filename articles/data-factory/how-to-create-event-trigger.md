...
-   **Blob path begins with** (Путь большого двоичного объекта начинается с) ("/имя_контейнера/BLOB-объекты/имя_папки") — получает события любого BLOB-объекта в контейнере "имя_контейнера" и папке "имя_папки". Вы можете также указать вложенную папку. Например, "/имя_контейнера/большие_двоичные_объекты/имя_папки/имя_вложенной_папки/".
