...
В предыдущем примере триггер настроен на срабатывание при создании пути BLOB-объекта, завершающегося в файле. csv, в папке Sample-testing — Data. Свойства **FolderPath** и **filename** захватывают расположение нового большого двоичного объекта. Например, `@triggerBody().folderPath` при добавлении мовиесдб. csv в образец пути — проверка данных или событий имеет значение `sample-data/event-testing` и `@triggerBody().fileName` имеет значение. `moviesDB.csv` Эти `sourceFolder` значения сопоставлены в примере с параметрами конвейера и `sourceFile` могут использоваться в конвейере как `@pipeline().parameters.sourceFolder` и `@pipeline().parameters.sourceFile` соответственно.

## <a name="json-schema"></a>Схема JSON

В таблице ниже приводится обзор элементов схемы, относящихся к триггерам на основе событий:

| **Элемент JSON** | **Описание** | **Type** | **Допустимые значения** | **Обязательное** |
| ---------------- | --------------- | -------- | ------------------ | ------------ |
...
| **событиях** | Тип событий, вызывающих срабатывание триггера. | Массив    | Microsoft.Storage.BlobCreated, Microsoft.Storage.BlobDeleted | Да, любое сочетание этих значений. |
...
| **игноримптиблобс** | Указывает, будут ли большие двоичные объекты с нулевым байтами инициировать запуск конвейера. По умолчанию задано значение true. | Логическое значение | true или false | Нет |
