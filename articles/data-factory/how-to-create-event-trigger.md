...
5. Выберите свою учетную запись хранения в раскрывающемся списке подписки Azure или вручную с помощью идентификатора ресурса учетной записи хранения. Выберите контейнер, в котором должны выполняться события. Выбор контейнера является необязательным, но учитывать, что выбор всех контейнеров может привести к большому числу событий.
   ...
   > [!NOTE]
   > Сейчас триггер событий поддерживает только учетные записи хранения версии 2 (общее назначение).

   > [!NOTE]
   > В связи с ограничением сетки событий Azure фабрика данных Azure поддерживает не более 500 триггеров событий для каждой учетной записи хранения.
...
| ---------------- | --------------- | -------- | ------------------ | ------------ |
| **scope** | Идентификатор ресурса Azure Resource Manager учетной записи хранения. | Строка | Идентификатор Azure Resource Manager | Да |
