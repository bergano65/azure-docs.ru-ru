---
title: Копирование данных в SQL Server и из них с помощью фабрики данных Azure | Документация Майкрософт
...
**Пример 2. Использовать проверку подлинности SQL с паролем в Azure Key Vault**
...
| tableName |Это свойство является именем таблицы или представления в SQL Server экземпляре базы данных, на который ссылается связанная служба. | "Нет" для источника, "Да" для приемника |
...
| writeBatchTimeout |Это свойство определяет время ожидания при выполнении операции пакетной вставки, по истечении которого она считается не выполненной.<br/>Допустимые значения — для интервала времени. Например, "00:30:00" в течение 30 минут. |Нет |
...
6. **Проверьте подключение**. Чтобы подключиться к SQL Server с помощью полного имени, используйте SQL Server Management Studio с другого компьютера. Например, `"<machine>.<domain>.corp.<company>.com,1433"`.
