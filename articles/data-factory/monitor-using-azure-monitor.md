...
| --- | --- | --- |
| **storageAccountId** |Строковый тип | Идентификатор ресурса учетной записи хранения, в которую необходимо отправить журналы диагностики. |
| **serviceBusRuleId** |Строковый тип | Идентификатор правила служебной шины для пространства имен Service-Bus, в котором должны быть созданы концентраторы событий для потоковой передачи журналов диагностики. Идентификатор правила имеет формат `{service bus resource ID}/authorizationrules/{key name}` .|
...
| **category**| Строковый тип| Массив категорий журналов и их политик хранения. |
| **timegrain** | Строковый тип | Гранулярность метрик, которые записываются в формате длительности ISO 8601. Значение свойства должно быть равно `PT1M` , что указывает одну минуту. |
...
| --- | --- | --- | --- |
| **Уровень** |Строковый тип | Уровень журналов диагностики. Для журналов выполнения действий задайте для свойства значение 4. | `4` |
| **correlationId** |Строковый тип | Уникальный идентификатор для отслеживания конкретного запроса. | `319dc6b4-f348-405e-b8d7-aafc77b73e77` |
| **time** | Строковый тип | Время события в формате UTC TimeSpan `YYYY-MM-DDTHH:MM:SS.00000Z` . | `2017-06-28T21:00:27.3534352Z` |
|**activityRunId**| Строковый тип| Идентификатор выполнения действия. | `3a171e1f-b36e-4b80-8a54-5625394f4354` |
|**pipelineRunId**| Строковый тип| Идентификатор выполнения конвейера. | `9f6069d6-e522-4608-9f99-21807bfc3c70` |
|**resourceId**| Строковый тип | Идентификатор, связанный с ресурсом фабрики данных. | `/SUBSCRIPTIONS/<subID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
|**category**| Строковый тип | Категория журналов диагностики. В качестве значения свойства задайте `ActivityRuns`. | `ActivityRuns` |
|**level**| Строковый тип | Уровень журналов диагностики. В качестве значения свойства задайте `Informational`. | `Informational` |
|**operationName**| Строковый тип | Имя действия с его состоянием. Если действие является начальным пульсом, значение свойства равно `MyActivity -` . Если действие является конечным периодом пульса, свойство имеет значение `MyActivity - Succeeded` . | `MyActivity - Succeeded` |
|**pipelineName**| Строковый тип | Имя конвейера. | `MyPipeline` |
|**activityName**| Строковый тип | Имя действия. | `MyActivity` |
|**start**| Строковый тип | Время начала действия выполняется в формате времени в формате UTC. | `2017-06-26T20:55:29.5007959Z`|
|**end**| Строковый тип | Время окончания действия выполняется в формате времени в формате UTC. Если в журнале диагностики указано, что действие началось, но еще не завершено, свойство имеет значение `1601-01-01T00:00:00Z` . | `2017-06-26T20:55:29.5007959Z` |
...
| **correlationId** |Строковый тип | Уникальный идентификатор для отслеживания конкретного запроса. | `319dc6b4-f348-405e-b8d7-aafc77b73e77` |
| **time** | Строковый тип | Время события в формате UTC TimeSpan `YYYY-MM-DDTHH:MM:SS.00000Z` . | `2017-06-28T21:00:27.3534352Z` |
|**runId**| Строковый тип| Идентификатор выполнения конвейера. | `9f6069d6-e522-4608-9f99-21807bfc3c70` |
|**resourceId**| Строковый тип | Идентификатор, связанный с ресурсом фабрики данных. | `/SUBSCRIPTIONS/<subID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
|**category**| Строковый тип | Категория журналов диагностики. В качестве значения свойства задайте `PipelineRuns`. | `PipelineRuns` |
|**level**| Строковый тип | Уровень журналов диагностики. В качестве значения свойства задайте `Informational`. | `Informational` |
|**operationName**| Строковый тип | Имя конвейера и его состояние. По завершении выполнения конвейера свойство имеет значение `Pipeline - Succeeded` . | `MyPipeline - Succeeded`. |
|**pipelineName**| Строковый тип | Имя конвейера. | `MyPipeline` |
|**start**| Строковый тип | Время начала действия выполняется в формате времени в формате UTC. | `2017-06-26T20:55:29.5007959Z`. |
|**end**| Строковый тип | Время окончания действия выполняется в формате времени в формате UTC. Если в журнале диагностики указано, что действие началось, но еще не завершено, свойство имеет значение `1601-01-01T00:00:00Z` .  | `2017-06-26T20:55:29.5007959Z` |
|**status**| Строковый тип | Конечное состояние выполнения конвейера. Возможные значения свойств: `Succeeded` и `Failed` . | `Succeeded`|
...
| **correlationId** |Строковый тип | Уникальный идентификатор для отслеживания конкретного запроса. | `319dc6b4-f348-405e-b8d7-aafc77b73e77` |
| **time** | Строковый тип | Время события в формате UTC TimeSpan `YYYY-MM-DDTHH:MM:SS.00000Z` . | `2017-06-28T21:00:27.3534352Z` |
|**triggerId**| Строковый тип| Идентификатор запуска триггера. | `08587023010602533858661257311` |
|**resourceId**| Строковый тип | Идентификатор, связанный с ресурсом фабрики данных. | `/SUBSCRIPTIONS/<subID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
|**category**| Строковый тип | Категория журналов диагностики. В качестве значения свойства задайте `PipelineRuns`. | `PipelineRuns` |
|**level**| Строковый тип | Уровень журналов диагностики. В качестве значения свойства задайте `Informational`. | `Informational` |
|**operationName**| Строковый тип | Имя триггера с окончательным состоянием, которое указывает, успешно ли запущен триггер. Если пульс прошел успешно, значение свойства равно `MyTrigger - Succeeded` . | `MyTrigger - Succeeded` |
|**triggerName**| Строковый тип | Имя триггера. | `MyTrigger` |
|**triggerType может принимать**| Строковый тип | Тип триггера. Возможные значения свойств: `Manual Trigger` и `Schedule Trigger` . | `ScheduleTrigger` |
|**triggerEvent**| Строковый тип | Событие триггера. | `ScheduleTime - 2017-07-06T01:50:25Z` |
|**start**| Строковый тип | Время начала срабатывания триггера в формате TimeSpan UTC. | `2017-06-26T20:55:29.5007959Z`|
|**status**| Строковый тип | Конечное состояние, показывающее, успешно ли вызван триггер. Возможные значения свойств: `Succeeded` и `Failed` . | `Succeeded`|
...
| Свойство                   | Тип   | Описание                                                   | Пример                        |
| -------------------------- | ------ | ------------------------------------------------------------- | ------------------------------ |
| **time**                   | Строковый тип | Время события в формате UTC: `YYYY-MM-DDTHH:MM:SS.00000Z` | `2017-06-28T21:00:27.3534352Z` |
| **operationName**          | Строковый тип | Имя операции со IR для служб SSIS                            | `Start/Stop/Maintenance` |
| **category**               | Строковый тип | Категория журналов диагностики                               | `SSISIntegrationRuntimeLogs` |
| **correlationId**          | Строковый тип | Уникальный идентификатор для отслеживания определенной операции             | `f13b159b-515f-4885-9dfa-a664e949f785Deprovision0059035558` |
| **dataFactoryName**        | Строковый тип | Имя ADF                                          | `MyADFv2` |
| **интегратионрунтименаме** | Строковый тип | Имя службы SSIS IR                                      | `MySSISIR` |
| **level**                  | Строковый тип | Уровень журналов диагностики                                  | `Informational` |
| **resultType**             | Строковый тип | Результат работы службы SSIS IR                          | `Started/InProgress/Succeeded/Failed` |
| **message**                | Строковый тип | Выходное сообщение для операции IR службы SSIS                  | `The stopping of your SSIS integration runtime has succeeded.` |
| **resourceId**             | Строковый тип | Уникальный идентификатор ресурса ADF                            | `/SUBSCRIPTIONS/<subscriptionID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
...
| -------------------------- | ------ | -------------------------------------------------------------------- | ------------------------------ |
| **time**                   | Строковый тип | Время события в формате UTC: `YYYY-MM-DDTHH:MM:SS.00000Z`        | `2017-06-28T21:00:27.3534352Z` |
| **operationName**          | Строковый тип | Для этого параметра задается значение `YourSSISIRName-SSISPackageEventMessageContext`       | `mysqlmissisir-SSISPackageEventMessageContext` |
| **category**               | Строковый тип | Категория журналов диагностики                                      | `SSISPackageEventMessageContext` |
| **correlationId**          | Строковый тип | Уникальный идентификатор для отслеживания определенной операции                    | `e55700df-4caf-4e7c-bfb8-78ac7d2f28a0` |
| **dataFactoryName**        | Строковый тип | Имя ADF                                                 | `MyADFv2` |
| **интегратионрунтименаме** | Строковый тип | Имя службы SSIS IR                                             | `MySSISIR` |
| **level**                  | Строковый тип | Уровень журналов диагностики                                         | `Informational` |
| **Операции**            | Строковый тип | Уникальный идентификатор для отслеживания определенной операции в SSISDB          | `1` (1 означает операции, связанные с пакетами, которые **не** ХРАНЯТСЯ в SSISDB/Invoked через T-SQL) |
| **контекстдепс**           | Строковый тип | Глубина контекста сообщения о событии                              | `0` (0 означает контекст перед началом выполнения пакета, 1 означает контекст при возникновении ошибки и увеличивается по мере того, как контекст находится дальше от ошибки). |
| **packagePath**            | Строковый тип | Путь к объекту пакета в качестве источника контекста сообщения о событии      | `\Package` |
| **contextType**            | Строковый тип | Тип объекта пакета в качестве источника контекста сообщения о событии      | `60`(см. [Дополнительные типы контекста](https://docs.microsoft.com/sql/integration-services/system-views/catalog-event-message-context?view=sql-server-ver15#remarks)) |
| **контекстсаурценаме**      | Строковый тип | Имя объекта пакета в качестве источника контекста сообщения о событии      | `MyPackage` |
| **контекстсаурцеид**        | Строковый тип | Уникальный идентификатор объекта пакета в качестве источника контекста сообщения о событии | `{E2CF27FB-EA48-41E9-AF6F-3FE938B4ADE1}` |
| **propertyName**           | Строковый тип | Имя свойства пакета для источника контекста сообщения о событии   | `DelayValidation` |
| **propertyValue**          | Строковый тип | Значение свойства Package для источника контекста сообщения о событии  | `False` |
| **resourceId**             | Строковый тип | Уникальный идентификатор ресурса ADF                                   | `/SUBSCRIPTIONS/<subscriptionID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
...
| -------------------------- | ------ | ------------------------------------------------------------------ | ------------------------------ |
| **time**                   | Строковый тип | Время события в формате UTC: `YYYY-MM-DDTHH:MM:SS.00000Z`      | `2017-06-28T21:00:27.3534352Z` |
| **operationName**          | Строковый тип | Для этого параметра задается значение `YourSSISIRName-SSISPackageEventMessages`           | `mysqlmissisir-SSISPackageEventMessages` |
| **category**               | Строковый тип | Категория журналов диагностики                                    | `SSISPackageEventMessages` |
| **correlationId**          | Строковый тип | Уникальный идентификатор для отслеживания определенной операции                  | `e55700df-4caf-4e7c-bfb8-78ac7d2f28a0` |
| **dataFactoryName**        | Строковый тип | Имя ADF                                               | `MyADFv2` |
| **интегратионрунтименаме** | Строковый тип | Имя службы SSIS IR                                           | `MySSISIR` |
| **level**                  | Строковый тип | Уровень журналов диагностики                                       | `Informational` |
| **Операции**            | Строковый тип | Уникальный идентификатор для отслеживания определенной операции в SSISDB        | `1` (1 означает операции, связанные с пакетами, которые **не** ХРАНЯТСЯ в SSISDB/Invoked через T-SQL) |
| **мессажетиме**            | Строковый тип | Время создания сообщения о событии в формате UTC          | `2017-06-28T21:00:27.3534352Z` |
| **messageType**            | Строковый тип | Тип сообщения о событии                                     | `70`(см. [Дополнительные типы сообщений](https://docs.microsoft.com/sql/integration-services/system-views/catalog-operation-messages-ssisdb-database?view=sql-server-ver15#remarks)) |
| **messageSourceType**      | Строковый тип | Тип источника сообщения о событии                              | `20`(см. [Дополнительные типы источников сообщений](https://docs.microsoft.com/sql/integration-services/system-views/catalog-operation-messages-ssisdb-database?view=sql-server-ver15#remarks)) |
| **message**                | Строковый тип | Текст сообщения о событии                                     | `MyPackage:Validation has started.` |
| **packageName**            | Строковый тип | Имя выполненного файла пакета                             | `MyPackage.dtsx` |
| **eventName**              | Строковый тип | Имя связанного события времени выполнения                                 | `OnPreValidate` |
| **мессажесаурценаме**      | Строковый тип | Имя компонента пакета в качестве источника сообщения о событии         | `Data Flow Task` |
| **мессажесаурцеид**        | Строковый тип | Уникальный идентификатор компонента пакета в качестве источника сообщения о событии    | `{1a45a5a4-3df9-4f02-b818-ebf583829ad2}    ` |
| **субкомпонентнаме**       | Строковый тип | Имя компонента потока данных в качестве источника сообщения о событии       | `SSIS.Pipeline` |
| **packagePath**            | Строковый тип | Путь к объекту пакета в качестве источника сообщения о событии            | `\Package\Data Flow Task` |
| **ексекутионпас**          | Строковый тип | Полный путь от родительского пакета к выполненному компоненту            | `\Transformation\Data Flow Task` (Этот путь также захватывает итерации компонента) |
| **Tидентификатор**               | Строковый тип | Уникальный идентификатор потока, выполняемого при занесении в журнал сообщения о событии | `{1a45a5a4-3df9-4f02-b818-ebf583829ad2}    ` |
...
| -------------------------- | ------ | ---------------------------------------------------------------- | ------------------------------ |
| **time**                   | Строковый тип | Время события в формате UTC: `YYYY-MM-DDTHH:MM:SS.00000Z`    | `2017-06-28T21:00:27.3534352Z` |
| **operationName**          | Строковый тип | Для этого параметра задается значение `YourSSISIRName-SSISPackageExecutableStatistics`  | `mysqlmissisir-SSISPackageExecutableStatistics` |
| **category**               | Строковый тип | Категория журналов диагностики                                  | `SSISPackageExecutableStatistics` |
| **correlationId**          | Строковый тип | Уникальный идентификатор для отслеживания определенной операции                | `e55700df-4caf-4e7c-bfb8-78ac7d2f28a0` |
| **dataFactoryName**        | Строковый тип | Имя ADF                                             | `MyADFv2` |
| **интегратионрунтименаме** | Строковый тип | Имя службы SSIS IR                                         | `MySSISIR` |
| **level**                  | Строковый тип | Уровень журналов диагностики                                     | `Informational` |
| **executionId**            | Строковый тип | Уникальный идентификатор для отслеживания определенного выполнения в SSISDB      | `1` (1 обозначает выполнение, связанное с пакетами, которые **не** ХРАНЯТСЯ в SSISDB/Invoked через T-SQL) |
| **ексекутионпас**          | Строковый тип | Полный путь от родительского пакета к выполненному компоненту          | `\Transformation\Data Flow Task` (Этот путь также захватывает итерации компонента) |
| **startTime**              | Строковый тип | Время, когда исполняемый объект вводит этап перед выполнением в формате UTC  | `2017-06-28T21:00:27.3534352Z` |
| **Завершения**                | Строковый тип | Время, когда исполняемый файл переходит в фазу после выполнения в формате UTC | `2017-06-28T21:00:27.3534352Z` |
| **ексекутиондуратион**      | Строковый тип | Время выполнения исполняемого файла в миллисекундах                   | `1,125` |
| **executionResult**        | Строковый тип | Результат выполнения исполняемого файла                                 | `0` (0 означает успех, 1 означает сбой, 2 означает завершение, а 3 означает отмену) |
| **executionValue**         | Строковый тип | Определяемое пользователем значение, возвращаемое выполняемым исполняемым файлом            | `1` |
| **resourceId**             | Строковый тип | Уникальный идентификатор ресурса ADF                               | `/SUBSCRIPTIONS/<subscriptionID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
...
| -------------------------- | ------ | ------------------------------------------------------------------- | ------------------------------ |
| **time**                   | Строковый тип | Время события в формате UTC: `YYYY-MM-DDTHH:MM:SS.00000Z`       | `2017-06-28T21:00:27.3534352Z` |
| **operationName**          | Строковый тип | Для этого параметра задается значение `YourSSISIRName-SSISPackageExecutionComponentPhases` | `mysqlmissisir-SSISPackageExecutionComponentPhases` |
| **category**               | Строковый тип | Категория журналов диагностики                                     | `SSISPackageExecutionComponentPhases` |
| **correlationId**          | Строковый тип | Уникальный идентификатор для отслеживания определенной операции                   | `e55700df-4caf-4e7c-bfb8-78ac7d2f28a0` |
| **dataFactoryName**        | Строковый тип | Имя ADF                                                | `MyADFv2` |
| **интегратионрунтименаме** | Строковый тип | Имя службы SSIS IR                                            | `MySSISIR` |
| **level**                  | Строковый тип | Уровень журналов диагностики                                        | `Informational` |
| **executionId**            | Строковый тип | Уникальный идентификатор для отслеживания определенного выполнения в SSISDB         | `1` (1 обозначает выполнение, связанное с пакетами, которые **не** ХРАНЯТСЯ в SSISDB/Invoked через T-SQL) |
| **packageName**            | Строковый тип | Имя выполненного файла пакета                              | `MyPackage.dtsx` |
| **имя_задания**               | Строковый тип | Имя выполненной задачи потока данных                                 | `Data Flow Task` |
| **субкомпонентнаме**       | Строковый тип | Имя компонента потока данных                                     | `Derived Column` |
| **этапах**                  | Строковый тип | Имя фазы выполнения                                         | `AcquireConnections` |
| **startTime**              | Строковый тип | Время начала фазы выполнения в формате UTC                  | `2017-06-28T21:00:27.3534352Z` |
| **Завершения**                | Строковый тип | Время окончания фазы выполнения в формате UTC                    | `2017-06-28T21:00:27.3534352Z` |
| **ексекутионпас**          | Строковый тип | Путь выполнения задачи потока данных                            | `\Transformation\Data Flow Task` |
| **resourceId**             | Строковый тип | Уникальный идентификатор ресурса ADF                                  | `/SUBSCRIPTIONS/<subscriptionID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
...
| ---------------------------- | ------ | ------------------------------------------------------------------ | ------------------------------ |
| **time**                     | Строковый тип | Время события в формате UTC: `YYYY-MM-DDTHH:MM:SS.00000Z`      | `2017-06-28T21:00:27.3534352Z` |
| **operationName**            | Строковый тип | Для этого параметра задается значение `YourSSISIRName-SSISPackageExecutionDataStatistics` | `mysqlmissisir-SSISPackageExecutionDataStatistics` |
| **category**                 | Строковый тип | Категория журналов диагностики                                    | `SSISPackageExecutionDataStatistics` |
| **correlationId**            | Строковый тип | Уникальный идентификатор для отслеживания определенной операции                  | `e55700df-4caf-4e7c-bfb8-78ac7d2f28a0` |
| **dataFactoryName**          | Строковый тип | Имя ADF                                               | `MyADFv2` |
| **интегратионрунтименаме**   | Строковый тип | Имя службы SSIS IR                                           | `MySSISIR` |
| **level**                    | Строковый тип | Уровень журналов диагностики                                       | `Informational` |
...
| **packageName**              | Строковый тип | Имя выполненного файла пакета                             | `MyPackage.dtsx` |
| **имя_задания**                 | Строковый тип | Имя выполненной задачи потока данных                                | `Data Flow Task` |
| **датафловпасидстринг**     | Строковый тип | Уникальный идентификатор для отслеживания пути потока данных                          | `Paths[SQLDB Table3.ADO NET Source Output]` |
| **датафловпаснаме**         | Строковый тип | Имя пути потока данных                                         | `ADO NET Source Output` |
| **саурцекомпонентнаме**      | Строковый тип | Имя компонента потока данных, который отправляет данные                    | `SQLDB Table3` |
| **дестинатионкомпонентнаме** | Строковый тип | Имя компонента потока данных, который получает данные                 | `Derived Column` |
| **ровссент**                 | Строковый тип | Число строк, отправленных компонентом источника                        | `500` |
| **Время создания**              | Строковый тип | Время получения значений строк в формате UTC                | `2017-06-28T21:00:27.3534352Z` |
| **ексекутионпас**            | Строковый тип | Путь выполнения задачи потока данных                           | `\Transformation\Data Flow Task` |
| **resourceId**               | Строковый тип | Уникальный идентификатор ресурса ADF                                 | `/SUBSCRIPTIONS/<subscriptionID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
