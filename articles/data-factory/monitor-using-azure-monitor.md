...
Дополнительные сведения см. в разделе [Azure Monitor обзор](https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitoring-overview-azure-monitor).
...
* **Концентратор событий**: потоковая передача журналов в концентраторы событий Azure. Журналы становятся входными данными для партнерской службы или пользовательского решения аналитики, например Power BI.
...
1. Присвойте параметру имя, выберите **отправить log Analytics**, а затем выберите рабочую область из **log Analytics рабочей области**.
    ...
    * В режиме _конкретного ресурса_ журналы диагностики из фабрики данных Azure поступают в таблицы _адфактивитирун_, _адфпипелинерун_, _адфтригжеррун_, _адфссисинтегратионрунтимелогс_, _адфссиспаккажеевентмессажеконтекст_, _ADFSSISPackageEventMessages_, _ADFSSISPackageExecutableStatistics_, _ADFSSISPackageExecutionComponentPhases_и _ADFSSISPackageExecutionDataStatistics_ .

      Можно выбрать различные журналы, относящиеся к рабочим нагрузкам, для отправки в Log Analytics таблицы. Например, если вы не используете SQL Server Integration Services (SSIS) вообще, не нужно выбирать журналы служб SSIS. Если вы хотите вести журнал операций запуска, отмены или обслуживания служб SSIS Integration Runtime (IR), можно выбрать пункт журналы IR для служб SSIS. При вызове выполнения пакетов служб SSIS только с помощью T-SQL можно выбрать только журналы пакетов служб SSIS. При вызове выполнения пакетов служб SSIS с помощью действий выполнить пакет служб SSIS в конвейерах ADF можно выбрать все журналы.

    * Если выбрать _аллметрикс_, метрики для количества СУЩНОСТей ADF/размера, действия/конвейера/триггера выполняются, Integration Runtime (IR) загрузка ЦП/память/число узлов/очередь, а также для выполнения пакетов служб SSIS и операций запуска и завершения служб SSIS, которые будут доступны для мониторинга и создания оповещений.
...
При установке службы "аналитика фабрики данных Azure" в разделе "книги" выбранной Log Analytics рабочей области создается набор представлений по умолчанию. В результате будут включены следующие метрики:
...
|--------------------------------------|------------------------------------------|----------|----------------------|--------------------------------|
| активитиканцеледрунс                 | Метрики выполненных действий отменены           | Count    | Итог                | Общее число выполнений действий, которые были отменены в течение минуты. |
...
| пипелинеканцеледрунс                 | Отмененные метрики запуска конвейера           | Count    | Итог                | Общее число выполнений конвейера, которые были отменены в течение минуты. |
...
| тригжерканцеледрунс                  | Метрики выполненных триггеров отменены            | Count    | Итог                | Общее число запусков триггеров, которые были отменены в течение минуты. |
...
| ссисинтегратионрунтиместартканцелед  | Метрики запуска служб SSIS "отменено"           | Count    | Итог                | Общее число запусков служб SSIS, которые были отменены в течение минуты. |
...
| ссиспаккажеексекутионканцелед         | Метрики выполнения пакета служб SSIS отменены  | Count    | Итог                | Общее число выполнений пакетов служб SSIS, которые были отменены в течение минуты. |
...
> [!NOTE]
> Создаются только завершенные, активированные действия и запуски конвейера. Выполняющиеся и песочницы/Отладка **не** создаются. С другой стороны, выдаются все события из выполнения пакетов служб SSIS, включая завершенные, выполняемые и вызываемые с помощью T-SQL в SSMS/агент SQL Server/другие назначенные инструменты или как активированные, изолированные и отладочные запуски для выполнения действий пакета служб SSIS в конвейерах ADF.
...
Это атрибуты и свойства журнала служб SSIS Integration Runtime (IR), а также операции запуска, отмены и обслуживания.
...
| **интегратионрунтименаме** | Строка | Имя службы SSIS IR                                             | `MySSISIR` |
| **уровень**                  | Строка | Уровень журналов диагностики                                         | `Informational` |
| **operationId**            | Строка | Уникальный идентификатор для отслеживания определенной операции в SSISDB          | `1`(1 означает операции, связанные с пакетами, которые не хранятся в SSISDB) |
...
Это атрибуты или свойства журнала сообщений о событиях, создаваемых при выполнении пакетов служб SSIS в среде IR служб SSIS. Они представляют подобную информацию как [таблицу или представление сообщений о событиях SSISDB](https://docs.microsoft.com/sql/integration-services/system-views/catalog-event-messages?view=sql-server-ver15) , в котором показан подробный текст и метаданные сообщений о событиях. Они создаются на любом уровне ведения журнала, за исключением `None` .
...
Это атрибуты или свойства журнала исполняемой статистики, создаваемой пакетами служб SSIS для среды выполнения интеграции SSIS, где исполняемые объекты — это контейнеры или задачи в потоках управления пакетами. Они представляют подобную информацию в виде [таблицы или представления исполняемой статистики SSISDB](https://docs.microsoft.com/sql/integration-services/system-views/catalog-executable-statistics?view=sql-server-ver15) , которая показывает строку для каждого выполняемого исполняемого файла, включая его итерации. Они создаются на любом уровне ведения журнала `None` , кроме и полезны для выявления узких мест и сбоев на уровне задач.
...
| **интегратионрунтименаме** | Строка | Имя службы SSIS IR                                         | `MySSISIR` |
| **уровень**                  | Строка | Уровень журналов диагностики                                     | `Informational` |
| **executionId**            | Строка | Уникальный идентификатор для отслеживания определенного выполнения в SSISDB      | `1`(1 обозначает выполнение, связанное с пакетами, которые не хранятся в SSISDB). |
| **ексекутионпас**          | Строка | Полный путь от родительского пакета к выполненному компоненту          | `\Transformation\Data Flow Task`(Этот путь также захватывает итерации компонента) |
...
Это атрибуты или свойства журнала статистики времени выполнения для компонентов потока данных, созданных при выполнении пакетов служб SSIS в среде IR. Они представляют подобную информацию в виде [таблицы или представления в виде этапов выполнения компонента SSISDB](https://docs.microsoft.com/sql/integration-services/system-views/catalog-execution-component-phases?view=sql-server-ver15) , которая показывает время, затраченное компонентами потока данных на всех этапах выполнения. Они создаются при выборе `Performance/Verbose` уровня ведения журнала и удобно для сбора статистики выполнения потока данных.
...
Это атрибуты или свойства журнала перемещения данных с помощью каждого контейнера конвейеров потока данных, от вышестоящего до нижестоящих компонентов, созданных пакетами служб SSIS в среде выполнения интеграции служб SSIS. Они сообщают подобную информацию в виде [таблицы или представления статистики данных выполнения SSISDB](https://docs.microsoft.com/sql/integration-services/system-views/catalog-execution-data-statistics?view=sql-server-ver15) , показывающей количество строк данных, перемещаемых с помощью задач потока данных. Они создаются при выборе `Verbose` уровня ведения журнала и удобного для вычисления пропускной способности потока данных.
...
Чтобы претянуть & смены рабочих нагрузок SQL Server Integration Services (SSIS), можно выполнить [инициализацию служб SSIS Integration Runtime (IR) в фабрике данных Azure (ADF)](https://docs.microsoft.com/azure/data-factory/tutorial-deploy-ssis-packages-azure) , которая поддерживает:
...
После подготовки можно [проверить состояние работы служб SSIS IR с помощью Azure PowerShell или в центре **мониторинга** на портале ADF](https://docs.microsoft.com/azure/data-factory/monitor-integration-runtime#azure-ssis-integration-runtime). При использовании модели развертывания проекта журналы выполнения пакетов служб SSIS хранятся в внутренних таблицах и представлениях SSISDB, поэтому их можно запрашивать, анализировать и визуально представить с помощью определенных средств, таких как SQL Server Management Studio (SSMS). С помощью модели развертывания пакетов журналы выполнения пакетов служб SSIS могут храниться в файловой системе или в файлах Azure в виде CSV-файлов, которые все еще необходимо проанализировать и обработать с помощью других назначенных средств, прежде чем их можно будет запрашивать, анализировать и визуально представить.

Теперь с [Azure Monitorной](https://docs.microsoft.com/azure/azure-monitor/platform/data-platform) интеграцией все метрики и журналы, созданные с помощью IR-операций служб SSIS и выполнения пакетов служб SSIS, можно запрашивать, анализировать и отображать визуально в портал Azure, а также на них могут быть вызваны предупреждения.
...
Чтобы отправить все метрики и журналы, созданные с помощью IR-операций служб SSIS и выполнения пакетов служб SSIS, в Azure Monitor, выполните пошаговые инструкции по [настройке параметров диагностики и рабочей области для ADF](https://docs.microsoft.com/azure/data-factory/monitor-using-azure-monitor#configure-diagnostic-settings-and-workspace).

### <a name="ssis-operational-metrics"></a>Операционные метрики служб SSIS

Операционные [метрики](https://docs.microsoft.com/azure/azure-monitor/platform/data-platform-metrics) служб SSIS — это счетчики производительности и числовые значения, описывающие состояние операций запуска и окончания IR в службах SSIS и выполнения пакетов служб SSIS в определенный момент времени. Они входят в состав [метрик ADF в Azure Monitor, в](https://docs.microsoft.com/azure/data-factory/monitor-using-azure-monitor#data-factory-metrics)том числе для объектов ADF (количество и размер), действий, конвейеров и триггеров, а также загрузки ЦП, памяти, количества узлов и очереди.
...
Операционные [журналы](https://docs.microsoft.com/azure/azure-monitor/platform/data-platform-logs) служб SSIS — это события, создаваемые IR-ОПЕРАЦИЯМИ служб SSIS и выполнения пакетов служб SSIS, которые предоставляют достаточный контекст или информацию о выявленных проблемах и полезны для анализа основных причин. 

При настройке параметров диагностики и рабочей области для ADF на Azure Monitor можно выбрать соответствующие журналы операций служб SSIS и отправить их Log Analytics на основе обозреватель данных Azure, где они будут доступны для [анализа с помощью расширенного языка запросов](https://docs.microsoft.com/azure/azure-monitor/log-query/log-query-overview), [представления на панели мониторинга Azure](https://docs.microsoft.com/azure/azure-monitor/learn/tutorial-app-dashboards)и [оповещений практически в реальном времени](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-log).

![Назовите свои параметры и выберите рабочую область log-Analytics.](media/data-factory-monitor-oms/monitor-oms-image2.png)

Схемы и содержимое журналов выполнения пакетов служб SSIS в Azure Monitor и Log Analytics похожи на те, которые относятся к внутренним таблицам и представлениям SSISDB.
...
Выбранные журналы выполнения пакетов служб SSIS всегда отправляются в Log Analytics независимо от методов вызова, например в SQL Server Data Tools с поддержкой Azure (SSDT) с помощью T-SQL в SSMS/агент SQL Server/другие назначенные средства или в качестве активированных, изолированных и отладочных запусков для выполнения действий пакетов служб SSIS в конвейерах ADF.

При запросе журналов выполнения пакетов служб SSIS в журналах аналитики их можно объединить с помощью свойств ExecutionId/CorrelationId. Для операций и выполнений, относящихся к пакетам, **не** хранящимся в SSISDB, всегда устанавливается значение 1.

![Запрос журналов выполнения пакетов служб SSIS на Log Analytics](media/data-factory-monitor-oms/log-analytics-query.png)
