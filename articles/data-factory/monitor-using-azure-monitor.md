...
Чтобы получить доступ к метрикам, выполните инструкции в разделе [Azure Monitor Data Platform](../azure-monitor/platform/data-platform.md).
...
| **contextType**            | Строка | Тип объекта пакета в качестве источника контекста сообщения о событии      | `60`(см. [Дополнительные типы контекста](/sql/integration-services/system-views/catalog-event-message-context#remarks)) |
...
Ниже приведены атрибуты журнала сообщений о событиях, созданных при выполнении пакетов служб SSIS в среде IR служб SSIS. Они представляют аналогичные сведения в [таблице или представлении сообщений о событиях SSISDB](/sql/integration-services/system-views/catalog-event-messages) , в которых показан подробный текст и метаданные сообщений о событиях. Они создаются на любом уровне ведения журнала, за исключением `None` .
...
Теперь с [Azure Monitorной](../azure-monitor/platform/data-platform.md) интеграцией можно запрашивать, анализировать и визуально представлять все метрики и журналы, созданные с помощью IR-операций SSIS и выполнения пакетов служб ssis на портал Azure. Кроме того, на них можно создавать оповещения.
...
Операционные [журналы](../azure-monitor/platform/data-platform-logs.md) служб SSIS — это события, СОЗДАВАЕМые IR-ОПЕРАЦИЯми служб SSIS и выполнения пакетов служб SSIS, которые обеспечивают достаточный контекст при обнаруженных проблемах и полезны для анализа основных причин. 

При настройке параметров диагностики и рабочей области для ADF на Azure Monitor можно выбрать соответствующие рабочие журналы служб SSIS и отправить их Log Analytics на основе обозреватель данных Azure. В нем они будут доступны для [анализа с помощью расширенного языка запросов](../azure-monitor/log-query/log-query-overview.md), [представления на панели мониторинга Azure](../azure-monitor/learn/tutorial-app-dashboards.md)и [оповещений практически в реальном времени](../azure-monitor/platform/alerts-log.md).
