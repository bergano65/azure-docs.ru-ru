...
Azure Monitor предоставляет метрики и журналы инфраструктуры базового уровня для большинства служб Azure. Журналы диагностики Azure выдаются ресурсом. Они содержат подробные и своевременные данные о работе этого ресурса. И фабрика данных Azure записывает журналы диагностики в монитор.
...
| **timeGrain** | Строка | Гранулярность метрик, которые записываются в формате длительности ISO 8601. Значение свойства должно быть `PT1M`равно, что указывает одну минуту. |
...
| --- | --- | --- | --- |
| **Level** |Строка | Уровень журналов диагностики. Для журналов выполнения действий задайте для свойства значение 4. | `4` |
| **correlationId** |Строка | Уникальный идентификатор для отслеживания конкретного запроса. | `319dc6b4-f348-405e-b8d7-aafc77b73e77` |
| **time** | Строка | Время события в формате `YYYY-MM-DDTHH:MM:SS.00000Z`UTC TimeSpan. | `2017-06-28T21:00:27.3534352Z` |
...
|**уровню**| Строка | Уровень журналов диагностики. В качестве значения свойства задайте `Informational`. | `Informational` |
|**operationName**| Строка | Имя действия с его состоянием. Если действие является начальным пульсом, значение свойства равно `MyActivity -`. Если действие является конечным периодом пульса, свойство имеет `MyActivity - Succeeded`значение. | `MyActivity - Succeeded` |
...
|**end**| Строка | Время окончания действия выполняется в формате времени в формате UTC. Если в журнале диагностики указано, что действие началось, но еще не завершено, свойство имеет `1601-01-01T00:00:00Z`значение. | `2017-06-26T20:55:29.5007959Z` |
...
|**уровню**| Строка | Уровень журналов диагностики. В качестве значения свойства задайте `Informational`. | `Informational` |
|**operationName**| Строка | Имя конвейера и его состояние. По завершении выполнения конвейера свойство имеет `Pipeline - Succeeded`значение. | `MyPipeline - Succeeded`. |
|**pipelineName**| Строка | Имя конвейера. | `MyPipeline` |
|**start**| Строка | Время начала действия выполняется в формате времени в формате UTC. | `2017-06-26T20:55:29.5007959Z`. |
|**end**| Строка | Время окончания действия выполняется в формате времени в формате UTC. Если в журнале диагностики указано, что действие началось, но еще не завершено, свойство имеет `1601-01-01T00:00:00Z`значение.  | `2017-06-26T20:55:29.5007959Z` |
...
|----------------------|---------------------------------|----------|----------------------|-------------------------------------------------------|
| PipelineSucceededRuns | Метрики успешных выполнений конвейеров. | Счетчик    | Итог                | Общее количество выполнений конвейеров, выполненных в течение минуты. |
| PipelineFailedRuns   | Метрики неудачных выполнений конвейеров.    | Счетчик    | Итог                | Общее число запусков конвейера, завершившихся сбоем, в течение минуты.    |
| ActivitySucceededRuns | Метрики успешных выполнений действий. | Счетчик    | Итог                | Общее число выполнений действий, выполненных в течение минуты.  |
| ActivityFailedRuns   | Метрики неудачных выполнений действий.    | Счетчик    | Итог                | Общее число выполнений действий, завершившихся сбоем, в течение минуты.     |
| TriggerSucceededRuns | Метрики успешных запусков триггеров.  | Счетчик    | Итог                | Общее число запусков триггеров, выполненных в течение минуты.   |
| TriggerFailedRuns    | Метрики неудачных запусков триггеров.     | Счетчик    | Итог                | Общее число выполнений триггеров в течение минуты.      |
