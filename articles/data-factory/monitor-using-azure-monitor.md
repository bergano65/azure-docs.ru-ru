...
| --- | --- | --- |
| **storageAccountId** |Тип String | Идентификатор ресурса учетной записи хранения, в которую необходимо отправить журналы диагностики. |
| **serviceBusRuleId** |Тип String | Идентификатор правила служебной шины для пространства имен Service-Bus, в котором должны быть созданы концентраторы событий для потоковой передачи журналов диагностики. Идентификатор правила имеет формат `{service bus resource ID}/authorizationrules/{key name}` .|
...
| **category**| Тип String| Массив категорий журналов и их политик хранения. |
| **timegrain** | Тип String | Гранулярность метрик, которые записываются в формате длительности ISO 8601. Значение свойства должно быть равно `PT1M` , что указывает одну минуту. |
...
| --- | --- | --- | --- |
| **Level** |Тип String | Уровень журналов диагностики. Для журналов выполнения действий задайте для свойства значение 4. | `4` |
| **correlationId** |Тип String | Уникальный идентификатор для отслеживания конкретного запроса. | `319dc6b4-f348-405e-b8d7-aafc77b73e77` |
| **time** | Тип String | Время события в формате UTC TimeSpan `YYYY-MM-DDTHH:MM:SS.00000Z` . | `2017-06-28T21:00:27.3534352Z` |
|**activityRunId**| Тип String| Идентификатор выполнения действия. | `3a171e1f-b36e-4b80-8a54-5625394f4354` |
|**pipelineRunId**| Тип String| Идентификатор выполнения конвейера. | `9f6069d6-e522-4608-9f99-21807bfc3c70` |
|**resourceId**| Тип String | Идентификатор, связанный с ресурсом фабрики данных. | `/SUBSCRIPTIONS/<subID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
|**category**| Тип String | Категория журналов диагностики. В качестве значения свойства задайте `ActivityRuns`. | `ActivityRuns` |
|**уровню**| Тип String | Уровень журналов диагностики. В качестве значения свойства задайте `Informational`. | `Informational` |
|**operationName**| Тип String | Имя действия с его состоянием. Если действие является начальным пульсом, значение свойства равно `MyActivity -` . Если действие является конечным периодом пульса, свойство имеет значение `MyActivity - Succeeded` . | `MyActivity - Succeeded` |
|**pipelineName**| Тип String | Имя конвейера. | `MyPipeline` |
|**activityName**| Тип String | Имя действия. | `MyActivity` |
|**start**| Тип String | Время начала действия выполняется в формате времени в формате UTC. | `2017-06-26T20:55:29.5007959Z`|
|**конце**| Тип String | Время окончания действия выполняется в формате времени в формате UTC. Если в журнале диагностики указано, что действие началось, но еще не завершено, свойство имеет значение `1601-01-01T00:00:00Z` . | `2017-06-26T20:55:29.5007959Z` |
...
| **correlationId** |Тип String | Уникальный идентификатор для отслеживания конкретного запроса. | `319dc6b4-f348-405e-b8d7-aafc77b73e77` |
| **time** | Тип String | Время события в формате UTC TimeSpan `YYYY-MM-DDTHH:MM:SS.00000Z` . | `2017-06-28T21:00:27.3534352Z` |
|**runId**| Тип String| Идентификатор выполнения конвейера. | `9f6069d6-e522-4608-9f99-21807bfc3c70` |
|**resourceId**| Тип String | Идентификатор, связанный с ресурсом фабрики данных. | `/SUBSCRIPTIONS/<subID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
|**category**| Тип String | Категория журналов диагностики. В качестве значения свойства задайте `PipelineRuns`. | `PipelineRuns` |
|**уровню**| Тип String | Уровень журналов диагностики. В качестве значения свойства задайте `Informational`. | `Informational` |
|**operationName**| Тип String | Имя конвейера и его состояние. По завершении выполнения конвейера свойство имеет значение `Pipeline - Succeeded` . | `MyPipeline - Succeeded`. |
|**pipelineName**| Тип String | Имя конвейера. | `MyPipeline` |
|**start**| Тип String | Время начала действия выполняется в формате времени в формате UTC. | `2017-06-26T20:55:29.5007959Z`. |
|**конце**| Тип String | Время окончания действия выполняется в формате времени в формате UTC. Если в журнале диагностики указано, что действие началось, но еще не завершено, свойство имеет значение `1601-01-01T00:00:00Z` .  | `2017-06-26T20:55:29.5007959Z` |
|**status**| Тип String | Конечное состояние выполнения конвейера. Возможные значения свойств: `Succeeded` и `Failed` . | `Succeeded`|
...
| **correlationId** |Тип String | Уникальный идентификатор для отслеживания конкретного запроса. | `319dc6b4-f348-405e-b8d7-aafc77b73e77` |
| **time** | Тип String | Время события в формате UTC TimeSpan `YYYY-MM-DDTHH:MM:SS.00000Z` . | `2017-06-28T21:00:27.3534352Z` |
|**triggerId**| Тип String| Идентификатор запуска триггера. | `08587023010602533858661257311` |
|**resourceId**| Тип String | Идентификатор, связанный с ресурсом фабрики данных. | `/SUBSCRIPTIONS/<subID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
|**category**| Тип String | Категория журналов диагностики. В качестве значения свойства задайте `PipelineRuns`. | `PipelineRuns` |
|**уровню**| Тип String | Уровень журналов диагностики. В качестве значения свойства задайте `Informational`. | `Informational` |
|**operationName**| Тип String | Имя триггера с окончательным состоянием, которое указывает, успешно ли запущен триггер. Если пульс прошел успешно, значение свойства равно `MyTrigger - Succeeded` . | `MyTrigger - Succeeded` |
|**triggerName**| Тип String | Имя триггера. | `MyTrigger` |
|**triggerType может принимать**| Тип String | Тип триггера. Возможные значения свойств: `Manual Trigger` и `Schedule Trigger` . | `ScheduleTrigger` |
|**triggerEvent**| Тип String | Событие триггера. | `ScheduleTime - 2017-07-06T01:50:25Z` |
|**start**| Тип String | Время начала срабатывания триггера в формате TimeSpan UTC. | `2017-06-26T20:55:29.5007959Z`|
|**status**| Тип String | Конечное состояние, показывающее, успешно ли вызван триггер. Возможные значения свойств: `Succeeded` и `Failed` . | `Succeeded`|
...
| Свойство                   | Тип   | Описание                                                   | Пример                        |
| -------------------------- | ------ | ------------------------------------------------------------- | ------------------------------ |
| **time**                   | Тип String | Время события в формате UTC:`YYYY-MM-DDTHH:MM:SS.00000Z` | `2017-06-28T21:00:27.3534352Z` |
| **operationName**          | Тип String | Имя операции со IR для служб SSIS                            | `Start/Stop/Maintenance` |
| **category**               | Тип String | Категория журналов диагностики                               | `SSISIntegrationRuntimeLogs` |
| **correlationId**          | Тип String | Уникальный идентификатор для отслеживания определенной операции             | `f13b159b-515f-4885-9dfa-a664e949f785Deprovision0059035558` |
| **dataFactoryName**        | Тип String | Имя ADF                                          | `MyADFv2` |
| **интегратионрунтименаме** | Тип String | Имя службы SSIS IR                                      | `MySSISIR` |
| **уровню**                  | Тип String | Уровень журналов диагностики                                  | `Informational` |
| **resultType**             | Тип String | Результат работы службы SSIS IR                          | `Started/InProgress/Succeeded/Failed` |
| **message**                | Тип String | Выходное сообщение для операции IR службы SSIS                  | `The stopping of your SSIS integration runtime has succeeded.` |
| **resourceId**             | Тип String | Уникальный идентификатор ресурса ADF                            | `/SUBSCRIPTIONS/<subscriptionID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
...
| -------------------------- | ------ | -------------------------------------------------------------------- | ------------------------------ |
| **time**                   | Тип String | Время события в формате UTC:`YYYY-MM-DDTHH:MM:SS.00000Z`        | `2017-06-28T21:00:27.3534352Z` |
| **operationName**          | Тип String | Для этого параметра задается значение`YourSSISIRName-SSISPackageEventMessageContext`       | `mysqlmissisir-SSISPackageEventMessageContext` |
| **category**               | Тип String | Категория журналов диагностики                                      | `SSISPackageEventMessageContext` |
| **correlationId**          | Тип String | Уникальный идентификатор для отслеживания определенной операции                    | `e55700df-4caf-4e7c-bfb8-78ac7d2f28a0` |
| **dataFactoryName**        | Тип String | Имя ADF                                                 | `MyADFv2` |
| **интегратионрунтименаме** | Тип String | Имя службы SSIS IR                                             | `MySSISIR` |
| **уровню**                  | Тип String | Уровень журналов диагностики                                         | `Informational` |
| **operationId**            | Тип String | Уникальный идентификатор для отслеживания определенной операции в SSISDB          | `1`(1 означает операции, связанные с пакетами, которые **не** ХРАНЯТСЯ в SSISDB/Invoked через T-SQL) |
| **контекстдепс**           | Тип String | Глубина контекста сообщения о событии                              | `0`(0 означает контекст перед началом выполнения пакета, 1 означает контекст при возникновении ошибки и увеличивается по мере того, как контекст находится дальше от ошибки). |
| **packagePath**            | Тип String | Путь к объекту пакета в качестве источника контекста сообщения о событии      | `\Package` |
| **contextType**            | Тип String | Тип объекта пакета в качестве источника контекста сообщения о событии      | `60`(см. [Дополнительные типы контекста](https://docs.microsoft.com/sql/integration-services/system-views/catalog-event-message-context?view=sql-server-ver15#remarks)) |
| **контекстсаурценаме**      | Тип String | Имя объекта пакета в качестве источника контекста сообщения о событии      | `MyPackage` |
| **контекстсаурцеид**        | Тип String | Уникальный идентификатор объекта пакета в качестве источника контекста сообщения о событии | `{E2CF27FB-EA48-41E9-AF6F-3FE938B4ADE1}` |
| **propertyName**           | Тип String | Имя свойства пакета для источника контекста сообщения о событии   | `DelayValidation` |
| **propertyValue**          | Тип String | Значение свойства Package для источника контекста сообщения о событии  | `False` |
| **resourceId**             | Тип String | Уникальный идентификатор ресурса ADF                                   | `/SUBSCRIPTIONS/<subscriptionID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
...
| -------------------------- | ------ | ------------------------------------------------------------------ | ------------------------------ |
| **time**                   | Тип String | Время события в формате UTC:`YYYY-MM-DDTHH:MM:SS.00000Z`      | `2017-06-28T21:00:27.3534352Z` |
| **operationName**          | Тип String | Для этого параметра задается значение`YourSSISIRName-SSISPackageEventMessages`           | `mysqlmissisir-SSISPackageEventMessages` |
| **category**               | Тип String | Категория журналов диагностики                                    | `SSISPackageEventMessages` |
| **correlationId**          | Тип String | Уникальный идентификатор для отслеживания определенной операции                  | `e55700df-4caf-4e7c-bfb8-78ac7d2f28a0` |
| **dataFactoryName**        | Тип String | Имя ADF                                               | `MyADFv2` |
| **интегратионрунтименаме** | Тип String | Имя службы SSIS IR                                           | `MySSISIR` |
| **уровню**                  | Тип String | Уровень журналов диагностики                                       | `Informational` |
| **operationId**            | Тип String | Уникальный идентификатор для отслеживания определенной операции в SSISDB        | `1`(1 означает операции, связанные с пакетами, которые **не** ХРАНЯТСЯ в SSISDB/Invoked через T-SQL) |
| **мессажетиме**            | Тип String | Время создания сообщения о событии в формате UTC          | `2017-06-28T21:00:27.3534352Z` |
| **messageType**            | Тип String | Тип сообщения о событии                                     | `70`(см. [Дополнительные типы сообщений](https://docs.microsoft.com/sql/integration-services/system-views/catalog-operation-messages-ssisdb-database?view=sql-server-ver15#remarks)) |
| **messageSourceType**      | Тип String | Тип источника сообщения о событии                              | `20`(см. [Дополнительные типы источников сообщений](https://docs.microsoft.com/sql/integration-services/system-views/catalog-operation-messages-ssisdb-database?view=sql-server-ver15#remarks)) |
| **message**                | Тип String | Текст сообщения о событии                                     | `MyPackage:Validation has started.` |
| **packageName**            | Тип String | Имя выполненного файла пакета                             | `MyPackage.dtsx` |
| **eventName**              | Тип String | Имя связанного события времени выполнения                                 | `OnPreValidate` |
| **мессажесаурценаме**      | Тип String | Имя компонента пакета в качестве источника сообщения о событии         | `Data Flow Task` |
| **мессажесаурцеид**        | Тип String | Уникальный идентификатор компонента пакета в качестве источника сообщения о событии    | `{1a45a5a4-3df9-4f02-b818-ebf583829ad2}    ` |
| **субкомпонентнаме**       | Тип String | Имя компонента потока данных в качестве источника сообщения о событии       | `SSIS.Pipeline` |
| **packagePath**            | Тип String | Путь к объекту пакета в качестве источника сообщения о событии            | `\Package\Data Flow Task` |
| **ексекутионпас**          | Тип String | Полный путь от родительского пакета к выполненному компоненту            | `\Transformation\Data Flow Task`(Этот путь также захватывает итерации компонента) |
| **Tидентификатор**               | Тип String | Уникальный идентификатор потока, выполняемого при занесении в журнал сообщения о событии | `{1a45a5a4-3df9-4f02-b818-ebf583829ad2}    ` |
...
| -------------------------- | ------ | ---------------------------------------------------------------- | ------------------------------ |
| **time**                   | Тип String | Время события в формате UTC:`YYYY-MM-DDTHH:MM:SS.00000Z`    | `2017-06-28T21:00:27.3534352Z` |
| **operationName**          | Тип String | Для этого параметра задается значение`YourSSISIRName-SSISPackageExecutableStatistics`  | `mysqlmissisir-SSISPackageExecutableStatistics` |
| **category**               | Тип String | Категория журналов диагностики                                  | `SSISPackageExecutableStatistics` |
| **correlationId**          | Тип String | Уникальный идентификатор для отслеживания определенной операции                | `e55700df-4caf-4e7c-bfb8-78ac7d2f28a0` |
| **dataFactoryName**        | Тип String | Имя ADF                                             | `MyADFv2` |
| **интегратионрунтименаме** | Тип String | Имя службы SSIS IR                                         | `MySSISIR` |
| **уровню**                  | Тип String | Уровень журналов диагностики                                     | `Informational` |
| **executionId**            | Тип String | Уникальный идентификатор для отслеживания определенного выполнения в SSISDB      | `1`(1 обозначает выполнение, связанное с пакетами, которые **не** ХРАНЯТСЯ в SSISDB/Invoked через T-SQL) |
| **ексекутионпас**          | Тип String | Полный путь от родительского пакета к выполненному компоненту          | `\Transformation\Data Flow Task`(Этот путь также захватывает итерации компонента) |
| **startTime**              | Тип String | Время, когда исполняемый объект вводит этап перед выполнением в формате UTC  | `2017-06-28T21:00:27.3534352Z` |
| **Завершения**                | Тип String | Время, когда исполняемый файл переходит в фазу после выполнения в формате UTC | `2017-06-28T21:00:27.3534352Z` |
| **ексекутиондуратион**      | Тип String | Время выполнения исполняемого файла в миллисекундах                   | `1,125` |
| **executionResult**        | Тип String | Результат выполнения исполняемого файла                                 | `0`(0 означает успех, 1 означает сбой, 2 означает завершение, а 3 означает отмену) |
| **executionValue**         | Тип String | Определяемое пользователем значение, возвращаемое выполняемым исполняемым файлом            | `1` |
| **resourceId**             | Тип String | Уникальный идентификатор ресурса ADF                               | `/SUBSCRIPTIONS/<subscriptionID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
...
| -------------------------- | ------ | ------------------------------------------------------------------- | ------------------------------ |
| **time**                   | Тип String | Время события в формате UTC:`YYYY-MM-DDTHH:MM:SS.00000Z`       | `2017-06-28T21:00:27.3534352Z` |
| **operationName**          | Тип String | Для этого параметра задается значение`YourSSISIRName-SSISPackageExecutionComponentPhases` | `mysqlmissisir-SSISPackageExecutionComponentPhases` |
| **category**               | Тип String | Категория журналов диагностики                                     | `SSISPackageExecutionComponentPhases` |
| **correlationId**          | Тип String | Уникальный идентификатор для отслеживания определенной операции                   | `e55700df-4caf-4e7c-bfb8-78ac7d2f28a0` |
| **dataFactoryName**        | Тип String | Имя ADF                                                | `MyADFv2` |
| **интегратионрунтименаме** | Тип String | Имя службы SSIS IR                                            | `MySSISIR` |
| **уровню**                  | Тип String | Уровень журналов диагностики                                        | `Informational` |
| **executionId**            | Тип String | Уникальный идентификатор для отслеживания определенного выполнения в SSISDB         | `1`(1 обозначает выполнение, связанное с пакетами, которые **не** ХРАНЯТСЯ в SSISDB/Invoked через T-SQL) |
| **packageName**            | Тип String | Имя выполненного файла пакета                              | `MyPackage.dtsx` |
| **имя_задания**               | Тип String | Имя выполненной задачи потока данных                                 | `Data Flow Task` |
| **субкомпонентнаме**       | Тип String | Имя компонента потока данных                                     | `Derived Column` |
| **этапах**                  | Тип String | Имя фазы выполнения                                         | `AcquireConnections` |
| **startTime**              | Тип String | Время начала фазы выполнения в формате UTC                  | `2017-06-28T21:00:27.3534352Z` |
| **Завершения**                | Тип String | Время окончания фазы выполнения в формате UTC                    | `2017-06-28T21:00:27.3534352Z` |
| **ексекутионпас**          | Тип String | Путь выполнения задачи потока данных                            | `\Transformation\Data Flow Task` |
| **resourceId**             | Тип String | Уникальный идентификатор ресурса ADF                                  | `/SUBSCRIPTIONS/<subscriptionID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
...
| ---------------------------- | ------ | ------------------------------------------------------------------ | ------------------------------ |
| **time**                     | Тип String | Время события в формате UTC:`YYYY-MM-DDTHH:MM:SS.00000Z`      | `2017-06-28T21:00:27.3534352Z` |
| **operationName**            | Тип String | Для этого параметра задается значение`YourSSISIRName-SSISPackageExecutionDataStatistics` | `mysqlmissisir-SSISPackageExecutionDataStatistics` |
| **category**                 | Тип String | Категория журналов диагностики                                    | `SSISPackageExecutionDataStatistics` |
| **correlationId**            | Тип String | Уникальный идентификатор для отслеживания определенной операции                  | `e55700df-4caf-4e7c-bfb8-78ac7d2f28a0` |
| **dataFactoryName**          | Тип String | Имя ADF                                               | `MyADFv2` |
| **интегратионрунтименаме**   | Тип String | Имя службы SSIS IR                                           | `MySSISIR` |
| **уровню**                    | Тип String | Уровень журналов диагностики                                       | `Informational` |
...
| **packageName**              | Тип String | Имя выполненного файла пакета                             | `MyPackage.dtsx` |
| **имя_задания**                 | Тип String | Имя выполненной задачи потока данных                                | `Data Flow Task` |
| **датафловпасидстринг**     | Тип String | Уникальный идентификатор для отслеживания пути потока данных                          | `Paths[SQLDB Table3.ADO NET Source Output]` |
| **датафловпаснаме**         | Тип String | Имя пути потока данных                                         | `ADO NET Source Output` |
| **саурцекомпонентнаме**      | Тип String | Имя компонента потока данных, который отправляет данные                    | `SQLDB Table3` |
| **дестинатионкомпонентнаме** | Тип String | Имя компонента потока данных, который получает данные                 | `Derived Column` |
| **ровссент**                 | Тип String | Число строк, отправленных компонентом источника                        | `500` |
| **Время создания**              | Тип String | Время получения значений строк в формате UTC                | `2017-06-28T21:00:27.3534352Z` |
| **ексекутионпас**            | Тип String | Путь выполнения задачи потока данных                           | `\Transformation\Data Flow Task` |
| **resourceId**               | Тип String | Уникальный идентификатор ресурса ADF                                 | `/SUBSCRIPTIONS/<subscriptionID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
