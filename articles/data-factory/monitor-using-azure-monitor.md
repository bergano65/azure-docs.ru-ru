...
| --- | --- | --- |
| storageAccountId |Строковое | Идентификатор учетной записи хранения, в которую необходимо отправить журналы диагностики. |
...
| workspaceId | Комплексный тип | Массив интервалов времени метрики и политики их хранения. В настоящее время это свойство пусто. |
...
| logs| Комплексный тип| Имя категории журнала диагностики для типа ресурса. Чтобы получить список категорий журналов диагностики ресурса, сначала выполните операцию получения параметров диагностики. |
...
| enabled| логический | Указывает, включена ли коллекция этой метрики или категории журнала для выбранного ресурса.|
| retentionPolicy| Комплексный тип| Описывает политику хранения метрики или категории журналов. Используется только для хранения параметра учетной записи.|
| дн.| int| Число дней для хранения метрик или журналов. Нулевое значение означает, что журналы хранятся неограниченно долго. Используется только для хранения параметра учетной записи. |
...
| --- | --- | --- | --- |
| Уровень |Строковое | Уровень журналов диагностики. Уровень 4 всегда относится к журналам выполнения действий. | `4`  |
| correlationId |Строковое | Уникальный идентификатор для полного отслеживания конкретного запроса. | `319dc6b4-f348-405e-b8d7-aafc77b73e77` |
| time | Строковое | Время события в формате UTC`YYYY-MM-DDTHH:MM:SS.00000Z` | `2017-06-28T21:00:27.3534352Z` |
|activityRunId| Строковое| Идентификатор выполнения действия. | `3a171e1f-b36e-4b80-8a54-5625394f4354` |
|pipelineRunId| Строковое| Идентификатор выполнения конвейера. | `9f6069d6-e522-4608-9f99-21807bfc3c70` |
|resourceId| Строковое | Идентификатор связанного ресурса фабрики данных. | `/SUBSCRIPTIONS/<subID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
|category| Строковое | Категория журналов диагностики. Для этого свойства задайте значение ActivityRuns. | `ActivityRuns` |
|уровень| Строковое | Уровень журналов диагностики. Для этого свойства задайте значение Informational. | `Informational` |
|operationName| Строковое |Имя действия и состояние. Если состояние — начать пульс, это `MyActivity -`. Если состояние — окончить пульс, это `MyActivity - Succeeded` с конечным состоянием. | `MyActivity - Succeeded` |
|pipelineName| Строковое | Имя конвейера. | `MyPipeline` |
|activityName| Строковое | Имя действия. | `MyActivity` |
|start| Строковое | Время начала выполнения действия в формате UTC. | `2017-06-26T20:55:29.5007959Z`|
|end| Строковое | Время окончания выполнения действия в формате UTC. Если действие не завершено (журнал диагностики для действия выполняется), устанавливается значение по умолчанию `1601-01-01T00:00:00Z`.  | `2017-06-26T20:55:29.5007959Z` |
...
| Свойство | type | Описание | Пример |
| --- | --- | --- | --- |
| Уровень |Строковое | Уровень журналов диагностики. Уровень 4 относится к журналам выполнения действий. | `4`  |
| correlationId |Строковое | Уникальный идентификатор для полного отслеживания конкретного запроса. | `319dc6b4-f348-405e-b8d7-aafc77b73e77` |
| time | Строковое | Время события в формате UTC`YYYY-MM-DDTHH:MM:SS.00000Z` | `2017-06-28T21:00:27.3534352Z` |
|runId| Строковое| Идентификатор выполнения конвейера. | `9f6069d6-e522-4608-9f99-21807bfc3c70` |
|resourceId| Строковое | Идентификатор связанного ресурса фабрики данных. | `/SUBSCRIPTIONS/<subID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
|category| Строковое | Категория журналов диагностики. Для этого свойства задайте значение PipelineRuns. | `PipelineRuns` |
|уровень| Строковое | Уровень журналов диагностики. Для этого свойства задайте значение Informational. | `Informational` |
|operationName| Строковое |Имя конвейера и его состояние. Pipeline - Succeeded с конечным состоянием при завершении выполнения конвейера.| `MyPipeline - Succeeded` |
|pipelineName| Строковое | Имя конвейера. | `MyPipeline` |
|start| Строковое | Время начала выполнения действия в формате UTC. | `2017-06-26T20:55:29.5007959Z`|
|end| Строковое | Время окончания выполнения действия в формате UTC. Если действие не завершено (журнал диагностики для действия выполняется), устанавливается значение по умолчанию `1601-01-01T00:00:00Z`.  | `2017-06-26T20:55:29.5007959Z` |
|Состояние| Строковое | Конечное состояние выполнения конвейера (Succeeded или Failed). | `Succeeded`|
...
| Свойство | type | Описание | Пример |
| --- | --- | --- | --- |
| Уровень |Строковое | Уровень журналов диагностики. Задайте уровень 4 для журналов выполнения действий. | `4`  |
| correlationId |Строковое | Уникальный идентификатор для полного отслеживания конкретного запроса. | `319dc6b4-f348-405e-b8d7-aafc77b73e77` |
| time | Строковое | Время события в формате UTC`YYYY-MM-DDTHH:MM:SS.00000Z` | `2017-06-28T21:00:27.3534352Z` |
...
|category| Строковое | Категория журналов диагностики. Для этого свойства задайте значение PipelineRuns. | `PipelineRuns` |
|уровень| Строковое | Уровень журналов диагностики. Для этого свойства задайте значение Informational. | `Informational` |
|operationName| Строковое |Имя триггера с конечным состоянием в случае успешного запуска. MyTrigger - Succeeded при успешном запуске пульса.| `MyTrigger - Succeeded` |
|triggerName| Строковое | Имя триггера. | `MyTrigger` |
|triggerType| Строковое | Тип триггера (ручной триггер или запланированный триггер). | `ScheduleTrigger` |
|triggerEvent| Строковое | Событие триггера. | `ScheduleTime - 2017-07-06T01:50:25Z` |
|start| Строковое | Время запуска триггера в формате UTC. | `2017-06-26T20:55:29.5007959Z`|
|Состояние| Строковое | Конечное состояние запуска триггера (Succeeded или Failed). | `Succeeded`|
...
|----------------------|---------------------------------|----------|----------------------|-------------------------------------------------------|
| PipelineSucceededRuns | Метрики успешных выполнений конвейеров. | Count    | Всего                | Общее количество успешных выполнений конвейеров в минутном окне. |
...
| ActivitySucceededRuns | Метрики успешных выполнений действий. | Count    | Всего                | Общее количество успешных выполнений действий в минутном окне.  |
