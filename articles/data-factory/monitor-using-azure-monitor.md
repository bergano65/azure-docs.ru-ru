...
| --- | --- | --- |
| **сторажеаккаунтид** |Строка, | Идентификатор ресурса учетной записи хранения, в которую необходимо отправить журналы диагностики. |
| **Запросе** |Строка, | Идентификатор правила служебной шины для пространства имен Service-Bus, в котором должны быть созданы концентраторы событий для потоковой передачи журналов диагностики. Идентификатор правила имеет формат `{service bus resource ID}/authorizationrules/{key name}`.|
...
|**активитирунид**| Строка,| Идентификатор выполнения действия. | `3a171e1f-b36e-4b80-8a54-5625394f4354` |
|**пипелинерунид**| Строка,| Идентификатор выполнения конвейера. | `9f6069d6-e522-4608-9f99-21807bfc3c70` |
...
|**category**| Строка, | Категория журналов диагностики. Присвойте свойству значение `ActivityRuns`. | `ActivityRuns` |
|**level**| Строка, | Уровень журналов диагностики. Присвойте свойству значение `Informational`. | `Informational` |
...
|**конце**| Строка, | Время окончания действия выполняется в формате времени в формате UTC. Если в журнале диагностики указано, что действие началось, но еще не завершено, свойство имеет `1601-01-01T00:00:00Z`значение. | `2017-06-26T20:55:29.5007959Z` |
...
|**resourceId**| Строка, | Идентификатор, связанный с ресурсом фабрики данных. | `/SUBSCRIPTIONS/<subID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
|**category**| Строка, | Категория журналов диагностики. Присвойте свойству значение `PipelineRuns`. | `PipelineRuns` |
|**level**| Строка, | Уровень журналов диагностики. Присвойте свойству значение `Informational`. | `Informational` |
...
|**тригжеревент**| Строка, | Событие триггера. | `ScheduleTime - 2017-07-06T01:50:25Z` |
...
> [!NOTE]
> Аналитика фабрики данных Azure (Предварительная версия) отправляет журналы диагностики в целевые таблицы для _конкретных ресурсов_ . Запросы можно создавать в следующих таблицах: _Адфпипелинерун_, _адфтригжеррун_и _адфактивитирун_.
