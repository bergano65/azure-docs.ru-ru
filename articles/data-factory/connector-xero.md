...
- Таблицы со сложными элементами необходимо разделять на несколько. Например, банковские транзакции имеют сложную структуру данных «LineItems», поэтому данные банковской транзакции сопоставляются с таблицей `Bank_Transaction` и `Bank_Transaction_Line_Items` , `Bank_Transaction_ID` как внешний ключ для связывания их друг с другом.

- Данные Xero имеют две схемы: `Minimal` (по умолчанию) и `Complete`. Полная схема содержит предварительные таблицы вызовов, для которых требуются дополнительные данные (например, столбец ИДЕНТИФИКАТОРов) перед выполнением нужного запроса.
