...
| Элемент JSON | Описание | Тип | Допустимые значения | Обязательно для заполнения |
|:--- |:--- |:--- |:--- |:--- |
...
| **dependsOn: размер** | Размер окна "переворачивающегося" зависимостей. | Timespan<br/>(чч:мм:сс)  | Положительное значение TimeSpan, где по умолчанию — размер окна дочернего триггера  | Нет |
| **dependsOn: смещение** | Смещение триггера зависимостей. | Timespan<br/>(чч:мм:сс) |  Значение TimeSpan, которое должно быть отрицательным в самостоятельной зависимости. Если значение не указано, окно совпадает с самим триггером. | Самостоятельная зависимость: Да<br/>Другое: нет  |
