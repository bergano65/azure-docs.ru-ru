---
title: Создание акробатических триггеров окон на фабрике данных Azure
...
1. Чтобы создать акробатичный триггер окна в uI Data Factory, выберите вкладку **Триггеры,** а затем выберите **новый.** 
1. После того, как панель конфигурации триггера откроется, выберите **Tumbling Window,** а затем определите свойства триггера акробатического окна. 
...
| **зависит от: тип** | Тип TumblingWindowTriggerСправка. Требуется, если зависимость установлена. | Строка |  "TumblingWindowTriggerЗависимостьСправка", "SelfDependencyTumblingтолентие" | нет |
| **зависит от: размер** | Размер окна акробатика зависимости. | Timespan<br/>(чч:мм:сс)  | Положительное значение временной промежутки, где по умолчанию является размер окна триггера ребенка  | нет |
| **зависит от: смещение** | Смещение триггера зависимости. | Timespan<br/>(чч:мм:сс) |  Значение временной продолжительности, которое должно быть отрицательным в самостоятельной зависимости. Если значение не указано, окно такое же, как и сам триггер. | Самозависимость: Да<br/>Другое: Нет  |
...
### <a name="tumbling-window-trigger-dependency"></a>Зависимость от триггера окна

Если вы хотите убедиться, что акробатика триггер акробатика выполняется только после успешного выполнения другого акробатического триггера окна в фабрике данных, [создайте зависимость от триггера акробатического окна.](tumbling-window-trigger-dependency.md) 
...
2. Создайте триггер с помощью смдлета **Set-AzDataFactoryV2Trigger:**

    ```powershell
    Set-AzDataFactoryV2Trigger -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name "MyTrigger" -DefinitionFile "C:\ADFv2QuickStartPSH\MyTrigger.json"
    ```
    
3. Подтвердите, что состояние триггера **остановлено** с помощью **cmdlet Get-AzDataFactoryV2Trigger:**

    ```powershell
    Get-AzDataFactoryV2Trigger -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name "MyTrigger"
    ```

4. Запустите триггер с помощью cmdlet **Start-AzDataFactoryV2Trigger:**

    ```powershell
    Start-AzDataFactoryV2Trigger -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name "MyTrigger"
    ```

5. Подтвердите, что состояние триггера **запускается** с помощью **cmdlet Get-AzDataFactoryV2Trigger:**

    ```powershell
    Get-AzDataFactoryV2Trigger -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name "MyTrigger"
    ```

6. Запуск триггера в Azure PowerShell с помощью cmdlet **Get-AzDataFactoryV2TriggerRun.** Чтобы получить сведения о выполнениях триггера, периодически выполняйте следующую команду. Обновите значения **TriggerRunStartedAfter** и **TriggerRunStartedBefore** в соответствии со значениями в определении триггера:
