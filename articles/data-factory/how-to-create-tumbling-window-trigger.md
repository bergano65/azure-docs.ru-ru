...
| Элемент JSON | Description (Описание) | Тип | Допустимые значения | Обязательно |
|:--- |:--- |:--- |:--- |:--- |
| **type** | Тип триггера. Тип является фиксированным значением "Тумблингвиндовтригжер". | string | "TumblingWindowTrigger" | Да |
...
| **dependsOn: type** | Тип Тумблингвиндовтригжерреференце. Требуется, если задана зависимость. | string |  "Тумблингвиндовтригжердепенденциреференце", "Селфдепенденцитумблингвиндовтригжерреференце" | Нет |
| **dependsOn: size** | Размер окна "переворачивающегося" зависимостей. | Timespan<br/>(чч:мм:сс)  | Положительное значение TimeSpan, где по умолчанию — размер окна дочернего триггера  | Нет |
| **dependsOn: offset** | Смещение триггера зависимостей. | Timespan<br/>(чч:мм:сс) |  Значение TimeSpan, которое должно быть отрицательным в самостоятельной зависимости. Если значение не указано, окно совпадает с самим триггером. | Самостоятельная зависимость: Да<br/>Другое: нет  |
