...
| **dependsOn: offset** | Смещение триггера зависимостей. | Временной диапазон<br/>(чч:мм:сс) |  Значение TimeSpan, которое должно быть отрицательным в самостоятельной зависимости. Если значение не указано, окно совпадает с самим триггером. | Самозависимость: Да<br/>Другое: Нет  |
