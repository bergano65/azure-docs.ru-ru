---
title: Копирование данных из SAP Business Warehouse через Open Hub с помощью Фабрики данных Azure
...
ADF SAP BW открытый соединитель концентратора предлагает два дополнительных свойства: `excludeLastRequest` и `baseRequestId`, которые можно использовать для управления разностной нагрузкой из открытого центра. 
...
Если вы настраиваете `excludeLastRequest` и `baseRequestId` в наборе данных, они по-прежнему поддерживаются "как есть", а в дальнейшем рекомендуется использовать новую модель в источнике действий.
...
Чтобы ускорить загрузку данных, можно задать [`parallelCopies`](copy-activity-performance.md#parallel-copy) действия копирования, чтобы загружать данные из SAP BW открытых концентраторов параллельно. Например, если задать для `parallelCopies` значение 4, фабрика данных одновременно выполняет четыре вызова RFC, и каждый вызов RFC извлекает часть данных из SAP BW открытой таблицы центра, секционированную по ИДЕНТИФИКАТОРу запроса DTP и ИДЕНТИФИКАТОРу пакета. Это применимо, когда число уникальных ИДЕНТИФИКАТОРов запроса DTP и идентификатора пакета превышает значение `parallelCopies`. При копировании данных в хранилище данных на основе файлов оно также передается в папку в виде нескольких файлов (указывается только имя папки). в этом случае производительность лучше, чем при записи в один файл.
