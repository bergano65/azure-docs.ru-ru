...
batchCount | Число пакетов, которое должно использоваться для управления количеством параллельного выполнения (в случае, если isSequential имеет значение false). Это верхний лимит параллелизма, но для каждого действия не всегда выполняется на этом количестве | Целое число (максимум 50) | Нет. Значение по умолчанию — 20.
...
Для агрегирования результатов деятельности __foreach,__ пожалуйста, используйте _переменные_ и переменную активность _приложения._
