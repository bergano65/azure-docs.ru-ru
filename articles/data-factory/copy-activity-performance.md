...
| Копирование данных в базе данных SQL Azure или Azure Cosmos DB |От 4 до 16, в зависимости от уровня базы данных SQL Azure или Cosmos DB (число Dtu/RUs) в качестве приемника |
...
| Копирование данных из любого источника в хранилище таблиц Azure |4\. |
| Все остальные сценарии копирования |1 |

> [!TIP]
> При копировании данных между файловыми хранилищами, поведение по умолчанию обычно предоставляет оптимальную пропускную способность. Поведение по умолчанию определен автоматически на основе вашего шаблона файла источника.
...
* При копировании данных из хранилищ, которые не являются на основе файлов (за исключением базы данных Oracle как источник данных секционирование включено) для хранилищ, которые основаны на файлах, служба перемещения данных игнорирует **parallelCopies** свойство. В этом случае параллелизм не применяется, даже если задан соответствующий параметр.
...
В настоящее время нельзя копировать данные между хранилищами данных, которые подключены через различные IRs локальную среду, ни с, а также без промежуточного копирования. Для таких случаев можно настроить два явным образом связанных действие копирования из источника в промежуточную среду, а затем из промежуточной в приемник.
