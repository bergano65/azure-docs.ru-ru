...
* Действие скрипта сейчас поддерживается связанными службами Azure HDInsight, которые доступны по требованию.  
...
| clusterType                  | Тип создаваемого кластера HDInsight. Допустимые значения: Hadoop и Spark. Если не указано другое, по умолчанию используется значение Hadoop. Кластеры с корпоративными пакетами безопасности сейчас не поддерживаются | Нет        |
...
> 
> [!IMPORTANT]
> Сейчас службы, связанные с HDInsight, не поддерживают HBase, интерактивные запросы (Hive LLAP), Storm и кластеры с поддержкой корпоративной безопасности (присоединенные к домену). 
>
> 
