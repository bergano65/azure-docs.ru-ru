...
|:--- |:--- |:--- |
| type | Для свойства типа необходимо задать значение **Db2** | Yes |
| имя сервера |Имя сервера DB2. Вы можете указать номер порта следом за именем сервера, разделив их двоеточием, например `server:port`. |Yes |
...
| паккажеколлектион | Укажите, где при запросе к базе данных ADF создает необходимые пакеты в автоматическом режиме. | Нет |
...
| Свойство | ОПИСАНИЕ | обязательные |
|:--- |:--- |:--- |
| type | Свойство Type набора данных должно иметь значение **Db2Table** . | Yes |
...
| Свойство | ОПИСАНИЕ | обязательные |
|:--- |:--- |:--- |
| type | Свойство Type источника действия копирования должно иметь значение **Db2Source** . | Yes |
| запрос | Используйте пользовательский SQL-запрос для чтения данных. Например, `"query": "SELECT * FROM \"DB2ADMIN\".\"Customers\""`. | Нет (если для набора данных задано свойство tableName) |
