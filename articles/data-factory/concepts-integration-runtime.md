...
- Для действия копирования ADF-файл предоставляет лучшие усилия для автоматического обнаружения приемника и исходного хранилища данных, чтобы выбрать лучшее расположение, либо в том же регионе (если доступно), либо в ближайшем расположении в той же географической области или в случае отсутствия возможности обнаружить использование региона фабрики данных в качестве альтернативы.
...
- Расположение Azure-SSIS IR не обязательно должно совпадать с расположением фабрики данных, но оно должно совпадать с расположением собственной базы данных SQL Azure или сервера Управляемый экземпляр, где будет размещена служба SSISDB. Это позволит среде выполнения интеграции Azure SSIS легко обращаться к базе данных SSIS, не генерируя лишний трафик между расположениями.
- Если у вас нет базы данных SQL Azure или сервера Управляемый экземпляр для размещения SSISDB, но у вас есть локальные источники данных или назначения, следует создать новую базу данных SQL Azure или сервер Управляемый экземпляр в том же расположении, что и виртуальная сеть, подключенная к локальной сети.  Таким образом можно создать Azure-SSIS IR с помощью новой базы данных SQL Azure или сервера Управляемый экземпляр и присоединить эту виртуальную сеть в том же расположении, что позволит свести к минимуму перемещение данных в разных местах.
- Если расположение существующей базы данных SQL Azure или сервера Управляемый экземпляр, где размещается SSISDB, не совпадает с расположением виртуальной сети, подключенной к локальной сети, сначала создайте Azure-SSIS IR с помощью существующей базы данных SQL Azure или сервера Управляемый экземпляр и Подключите другую виртуальную сеть в том же расположении, а затем настройте виртуальную сеть для подключения к виртуальной сети между разными расположениями.
...
- [Создайте среду выполнения интеграции Azure SSIS](create-azure-ssis-integration-runtime.md). Эта статья дополняет соответствующее руководство, а также предоставляет инструкции по использованию Управляемого экземпляра Базы данных SQL Azure и присоединению среды выполнения интеграции к виртуальной сети. 
