...
Azure Integration Runtime поддерживает подключение к хранилищам данных и вычислению служб с общедоступными конечными точками. Для среды виртуальных сетей Azure следует использовать локально размещенную среду выполнения интеграции.
...
- Отправка следующих действий преобразования против вычислительных ресурсов в предварительном или Azure Виртуальная сеть: HDInsight Hive деятельности (BYOC-Принесите свой собственный кластер), HDInsight Свинья деятельности (BYOC), HDInsight MapReduce деятельности (BYOC), HDInsight Spark деятельность (BYOC), активность потоковой передачи HDInsight (BYOC), деятельность по выполнению пакетов машинного обучения, деятельность ресурса обновления машинного обучения, деятельность Сохраненные процедуры, активность Data Lake Analytics U-S'L, пользовательские действия (запуски на Azure Batch), Lookup деятельности и получения активности Метаданных.
...
Если вы решите использовать **иК-управлику Azure,** которая является по умолчанию, 
...
- Для выполнения деятельности Lookup/GetMetadata/Delete (также известного как действия трубопровода), диспетчеризации активности преобразования (также известной как Внешние действия) и авторизации операций (соединение тестов, список папок просмотра и список таблиц, предварительные данные) ADF будет использовать ИК в области фабрики данных.

- Для Data Flow ADF будет использовать ИК в регионе фабрики данных. 
...
- Если местоположение существующей базы данных Azure S'L или сервера управляемых экземпляров, на котором размещается SSISDB, не совпадает с местоположением виртуальной сети, подключенной к вашей сети, сначала создайте ИК Azure-SSIS с помощью существующей базы данных Azure S-L или Управляемый сервер Instance и присоединение к другой виртуальной сети в том же месте, а затем настроить виртуальную сеть на виртуальное сетевое соединение между различными местоположениями.
