...
- **Поток данных**: Выполнение [потока данных](concepts-data-flow-overview.md) в управляемой среде вычислений Azure.  
...
- **Диспетчеризация действий**.  Назначение и мониторинг действий по преобразованию с использованием разнообразных служб вычислений, например Azure Databricks, Azure HDInsight, машинное обучение Azure, базы данных SQL Azure, SQL Server и многое другое.
...
В фабрике данных действия определяют выполняемые операции. Связанная служба обозначает целевое хранилище данных или службу вычислений. Среда выполнения интеграции соединяет между собой действия и связанные службы.  Он ссылается на связанную службу или на мероприятие и предоставляет вычислительную среду, где действие выполнения или диспетчеризации. Такая схема позволяет выполнять действия в регионе, который максимально близко расположен к целевому хранилищу данных или службе вычислений, обеспечивает высокую производительность и соблюдение требований по безопасности и соответствию.
...
- Диспетчеризация нескольких действий преобразования в общедоступной сети: Databricks Notebook или JAR-файл / Python действие, действие HDInsight Hive, действие HDInsight Pig, действие HDInsight MapReduce, действие HDInsight Spark, действие потоковой передачи HDInsight, действие пакетного выполнения машинного обучения, Machine Learning обновить ресурс действия, действие хранимой процедуры, действие U-SQL аналитики Озера данных, настраиваемое действие .NET, веб-действие, действие поиска и действие получения метаданных.

### <a name="azure-ir-network-environment"></a>Сетевая среда Azure IR
Среда выполнения интеграции Azure поддерживает подключение к хранилищам данных и вычислительными службами с общедоступными конечными точками. Для среды виртуальных сетей Azure следует использовать локально размещенную среду выполнения интеграции.
...
> [!NOTE] 
> Среда выполнения интеграции Azure имеет свойства, связанные с выполнения потока данных, которая определяет базовой вычислительную инфраструктуру, которая будет использоваться для запуска на потоки данных. 
...
- Диспетчеризация нескольких действий преобразования для вычислительных ресурсов, размещенных локально или в виртуальной сети Azure: Действие HDInsight Hive (BYOC перевести в собственном кластере кластере), действие HDInsight Pig (BYOC), действие HDInsight MapReduce (BYOC), действие HDInsight Spark (BYOC), HDInsight действие потоковой передачи (BYOC), действие пакетного выполнения машинного обучения, машинного обучения Обновление действия ресурсов, действие хранимой процедуры, действие U-SQL аналитики Озера данных, настраиваемое действие (при запуске в пакетной службе Azure), действия поиска и действие получения метаданных.
...
Если вы решили использовать **автоматическое разрешение Azure IR** используется по умолчанию 
...
- Для выполнения действия Lookup, GetMetadata и удаление (также известный как действий в конвейере), действия преобразования диспетчеризации (также известный как внешние действия) и создания операций (проверить подключение, список папок для обзора и список таблиц, предварительный просмотр данных), ADF использует среду выполнения Интеграции в области фабрики данных.

- Для потока данных ADF будет использовать среду выполнения Интеграции в области фабрики данных. 

  > [!TIP] 
  > Убедитесь, что поток данных выполняется в том же регионе, в соответствующие хранилища данных (если это возможно) будет хорошей практикой. Вы можете добиться этого, автоматическое разрешение Azure IR (если расположение хранилища данных — то же, что расположение фабрики данных), или создав новый экземпляр среды выполнения Интеграции Azure в том же регионе, что хранилища данных и затем выполнять поток данных на нем. 
...
Действие потока данных выполняется в среде выполнения интеграции, связанного с ним. 
