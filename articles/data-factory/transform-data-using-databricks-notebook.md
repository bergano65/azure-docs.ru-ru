...
1.  На панели **Новая фабрика данных** введите **ADFTutorialDataFactory** в поле **Имя**.
    ...
    Имя фабрики данных Azure должно быть  *глобально уникальным*. Если появится следующая ошибка, измените имя фабрики данных. (Например, используйте  **\<yourname\>ADFTutorialDataFactory**). Правила именования для артефактов службы "Фабрика данных" см. в статье  [Фабрика данных Azure — правила именования](https://docs.microsoft.com/azure/data-factory/naming-rules) .
...
1.  Для поля  **Группа ресурсов** выполните одно из следующих действий:
    
    - Выберите  **Использовать существующий** и укажите существующую группу ресурсов в раскрывающемся списке.
    
    - Выберите  **Создать новый** и укажите имя группы ресурсов.

    Некоторые действия, описанные в этом руководстве, предполагают, что для группы ресурсов используется имя  **ADFTutorialResourceGroup** . Сведения о группах ресурсов см. в статье  [Общие сведения о диспетчере ресурсов Azure](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-overview).

1.  Для поля  **Версия** выберите  **V2**.

1.  Для поля  **Расположение** выберите расположение фабрики данных.
...
1.  Когда создание завершится, откроется страница  **Фабрика данных** . Выберите плитку  **Создание и мониторинг** , чтобы открыть на отдельной вкладке приложение пользовательского интерфейса службы "Фабрика данных".
...
1.  На странице  **Начало работы**  переключитесь на вкладку  **Правка**  на левой панели.
...
1.  В окне  **Новая связанная служба**  выберите  **Вычисление** \> **Azure Databricks** и щелкните  **Продолжить**.
...
1.  В окне  **Новая связанная служба**  сделайте следующее:
...
1.  Нажмите кнопку  **+** (плюс) и в меню выберите  **Конвейер** .
...
1.  На панели элементов  **Действия**  разверните  **Databricks**. Перетащите действие  **Записная книжка**  с панели элементов  **Действия**  в область конструктора конвейера.
...
1.  В свойствах для окна действия  **Databricks** **Notebook**  в нижней части страницы выполните следующие действия:
    ...
    a. Перейдите на вкладку **Azure Databricks** .
...
1.  Перейдите на вкладку  **Monitor** (Мониторинг). Убедитесь, что здесь отображается запуск конвейера. Для создания кластера заданий Databricks, где выполняется записная книжка, требуется приблизительно 5–8 минут.
