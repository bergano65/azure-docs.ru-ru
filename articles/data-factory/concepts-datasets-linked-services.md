---
title: Наборы данных в фабрике данных Azure | Документация Майкрософт
description: Дополнительные сведения о наборах данных в фабрике данных. Наборы данных представляют входные и выходные данные.
...
Перед созданием набора данных, необходимо создать [ **связанная служба** ](concepts-linked-services.md) чтобы связать хранилище данных с фабрикой данных. Связанные службы напоминают строки подключения, определяющие сведения о подключении, необходимые для подключения фабрики данных к внешним ресурсам. Таким образом, набор данных представляет структуру данных в связанных хранилищах данных, а связанная служба определяет подключение к источнику данных. Например, связанная служба хранилища Azure связывает учетную запись хранения с фабрикой данных. Набор данных больших двоичных объектов Azure представляет контейнер больших двоичных объектов и папку в учетной записи хранения Azure, содержащую входные большие двоичные объекты для обработки.
...
structure | Схема набора данных. Дополнительные сведения см. в разделе [схему набора данных](#dataset-structure-or-schema). | Нет |
...
### <a name="data-flow-compatible-dataset"></a>Совместимые dataset потока данных

[!INCLUDE [notes](../../includes/data-factory-data-flow-preview.md)]

См. в разделе [поддерживаемые типы набора данных](#dataset-type) список типов наборов данных, которые являются [потока данных](concepts-data-flow-overview.md) совместимы. Наборы данных, которые совместимы с потока данных требуют определения точного набора данных для преобразования. Таким образом определение JSON несколько отличается. Вместо _структуры_ имеют свойства, наборы данных, которые являются потока данных совместимы _схемы_ свойство.

В поток данных наборы данных используются в преобразованиях источника и приемника. Наборы данных определения схемы базовых данных. Если в данных без схемы, можно использовать смещение схемы для источника и приемника. Схемы в наборе данных представляет тип физических данных и форму.

Определив схему из набора данных, вы получите связанные типы данных, форматы данных, расположение файла и сведения о соединении из связанного связанную службу. Метаданные из набора данных отображается в вашей преобразования исходного как источник *проекции*. Проекция в преобразование «источник» представляет данные потока данных с определенными именами и типами.

При импорте схемы потока данных набора данных, выберите **Импорт схемы** кнопку и выберите Импорт из источника или из локального файла. В большинстве случаев вы импортируете схему непосредственно из источника. Но если уже имеется файл локальной схемы (файла Parquet или CSV с заголовками), можно направить фабрики данных на основе схемы для этого файла.
...
name | Имя набора данных. Дополнительные сведения см. в статье [Фабрика данных Azure — правила именования](naming-rules.md). |  Да |
type | Тип набора данных. Укажите один из типов, которые поддерживает Фабрика данных (например, AzureBlob или AzureSqlTable). <br/><br/>Дополнительные сведения см. в разделе [о типах наборов данных](#dataset-type). | Да |
schema | Схема набора данных. Дополнительные сведения см. в разделе [совместимые наборы данных в поток данных](#dataset-type). | Нет |
typeProperties | Свойства каждого типа отличаются (например, свойства большого двоичного объекта Azure и таблицы SQL Azure). Сведения о поддерживаемых типах и их свойствах см. в разделе [Тип набора данных](#dataset-type). | Да |
...
## <a name="dataset-structure-or-schema"></a>Структура набора данных или схемы
**Структуры** раздел или **схемы** (совместимый потока данных) наборов данных раздел является необязательным. Он определяет схему набора данных. В нем содержится коллекция имен и типов данных столбцов. Используйте этот раздел, чтобы указать сведения о типах, которые используются для преобразования типов и сопоставления столбцов из источника в приемник.
