---
title: Наборы данных
description: Узнайте о наборах данных на фабрике данных. Наборы данных представляют входные и выходные данные.
...
Прежде чем создать набор данных, необходимо создать [**связанную службу,**](concepts-linked-services.md) связывающую хранилище данных с фабрикой данных. Связанные службы напоминают строки подключения, определяющие сведения о подключении, необходимые для подключения фабрики данных к внешним ресурсам. Таким образом, набор данных представляет структуру данных в связанных хранилищах данных, а связанная служба определяет подключение к источнику данных. Например, связанная служба хранилища Azure связывает учетную запись хранения с фабрикой данных. Набор данных больших двоичных объектов Azure представляет контейнер больших двоичных объектов и папку в учетной записи хранения Azure, содержащую входные большие двоичные объекты для обработки.
...
type | Тип набора данных. Укажите один из типов, которые поддерживает фабрика данных (например: AzureBlob, AzureSqlTable). <br/><br/>Дополнительные сведения см. в разделе [о типах наборов данных](#dataset-type). | Да |
structure | Схема набора данных. Для получения подробной информации [см.](#dataset-structure-or-schema) | нет |
typeProperties | Свойства каждого типа отличаются (например, свойства большого двоичного объекта Azure и таблицы SQL Azure). Для получения подробной информации об [Dataset type](#dataset-type)поддерживаемых типах и их свойствах см. | Да |

### <a name="data-flow-compatible-dataset"></a>Набор данных, совместимый с потоком данных



Просматривайте [поддерживаемые типы наборов данных](#dataset-type) для списка типов наборов данных, совместимых с [потоком данных.](concepts-data-flow-overview.md) Наборы данных, совместимые для потока данных, требуют точнозерновых определений наборов данных для преобразований. Таким образом, определение JSON немного отличается. Вместо свойства _структуры_ наборы данных, совместимые с потоком данных, имеют _свойство схемы._

В потоке данных наборы данных используются в преобразованиях источников и топов. Наборы данных определяют основные схемы данных. Если у ваших данных нет схемы, можно использовать дрейф схемы для исхода и раковины. Схема в наборе данных представляет тип и форму физических данных.

Определяя схему из набора данных, вы получите соответствующие типы данных, форматы данных, местоположение файла и информацию о подключении от связанной службы Linked. Метаданные из наборов данных отображаются в преобразовании исходного кода в качестве *исходной проекции.* Проекция в исходной трансформации представляет данные потока данных данных с определенными именами и типами.

При импорте схемы набора данных Data Flow выберите кнопку **«Импортная схема»** и выберите импорт из источника или из локального файла. В большинстве случаев схема будет импортироваться непосредственно из источника. Но если у вас уже есть локальный файл схемы (файл паркета или CSV с заголовками), вы можете направить Data Factory, чтобы основать схему на этом файле.
...
name | Имя набора данных. Дополнительные сведения см. в статье [Фабрика данных Azure — правила именования](naming-rules.md). |  Да |
type | Тип набора данных. Укажите один из типов, которые поддерживает фабрика данных (например: AzureBlob, AzureSqlTable). <br/><br/>Дополнительные сведения см. в разделе [о типах наборов данных](#dataset-type). | Да |
схема | Схема набора данных. Для получения подробной информации смотрите [наборы данных, совместимых с Data Flow.](#dataset-type) | нет |
typeProperties | Свойства каждого типа отличаются (например, свойства большого двоичного объекта Azure и таблицы SQL Azure). Для получения подробной информации об [Dataset type](#dataset-type)поддерживаемых типах и их свойствах см. | Да |
...
Существует множество различных типов наборов данных в зависимости от используемого хранилища данных. Список хранимых данных, поддерживаемых Data Factory, можно найти из статьи [обзора Connector.](connector-overview.md) Щелкните хранилище данных, чтобы узнать, как для него создать связанную службу и набор данных.
...
## <a name="dataset-structure-or-schema"></a>Структура или схема набора данных
Раздел **структуры** или **схемы** (совместимый поток данных) наборы данных раздела неявляются. Он определяет схему набора данных. В нем содержится коллекция имен и типов данных столбцов. Используйте этот раздел, чтобы указать сведения о типах, которые используются для преобразования типов и сопоставления столбцов из источника в приемник.
...
-------- | ----------- | --------
name | Имя столбца. | Да
type | Тип данных столбца. Фабрика данных поддерживает следующие промежуточные типы данных в качестве допустимых значений: **Int16, Int32, Int64, Single, Double, Decimal, Byte[], Boolean, String, Guid, Datetime, Datetimeoffset и Timespan**. | нет
...
format | Строка формата будет использоваться, если указан тип .NET `Datetime` или `Datetimeoffset`. Сведения о формате даты и времени см. в статье [Строки настраиваемых форматов даты и времени](https://docs.microsoft.com/dotnet/standard/base-types/custom-date-and-time-format-strings). | нет
