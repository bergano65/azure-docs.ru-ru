...
Следующий пример принимает входящий поток `MoviesYear` и группирует строки по столбцу `year`. Преобразование создает статистический столбец `avgrating`, результатом которого является среднее значение столбца `Rating`. Это преобразование «Статистическая обработка» называется `AvgComedyRatingsByYear`. В интерфейсе фабрики данных это преобразование выглядит как на изображении ниже:
