...
**type** | Для этого свойства необходимо задать значение "веб-перехватчик". | String | Да |
...
**headers** | Заголовки, которые отправляются в запрос. Ниже приведен пример, который задает язык и тип запроса: `"headers" : { "Accept-Language": "en-us", "Content-Type": "application/json" }`. | Строка или выражение со значением **resultType** строки. | Да. Требуется заголовок `Content-Type`, такой как `"headers":{ "Content-Type":"application/json"}`. |
**body** | Представляет полезные данные, отправляемые конечной точке. | Допустимый формат JSON или выражение со значением **resultType** в формате JSON. Схему полезных данных запроса см. в статье [схема полезных данных запроса](https://docs.microsoft.com/azure/data-factory/control-flow-web-activity#request-payload-schema) . | Да |
**проверке подлинности** | Метод проверки подлинности, используемый для вызова конечной точки. Поддерживаются следующие типы: "базовый" и "ClientCertificate". Дополнительные сведения см. в разделе [Authenticate to the Speech API](https://docs.microsoft.com/azure/data-factory/control-flow-web-activity#authentication) (Аутентификация в API речи). Если проверка подлинности не требуется, исключите это свойство. | Строка или выражение со значением **resultType** строки. | нет |
**timeout** | Время ожидания действием обратного вызова, указанного параметром **каллбаккури** для вызова. Значение по умолчанию — 10 минут ("00:10:00"). Значения *имеют формат TimeSpan*. *чч*:*мм*:*СС*. | String | нет |
