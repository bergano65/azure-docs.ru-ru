...
**Верхний** | Заголовки, которые отправляются в запрос. Ниже приведен пример, который задает язык и тип запроса: `"headers" : { "Accept-Language": "en-us", "Content-Type": "application/json" }`. | Строка или выражение со значением **resultType** строки. | Да. Требуется `Content-Type` заголовок, `"headers":{ "Content-Type":"application/json"}` например. |
...
**Отчет о состоянии при обратном вызове** | Позволяет пользователю сообщать о неудачном завершении действия веб-перехватчика. | Логическое значение | Нет |
