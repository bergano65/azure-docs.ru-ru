...
> [!IMPORTANT]
> При копировании данных с помощью Integration Runtime фабрики данных Azure настройте [Брандмауэр сервера SQL Azure](https://msdn.microsoft.com/library/azure/ee621782.aspx#ConnectingFromAzure) таким образом, чтобы службы Azure могли получать доступ к серверу.
...
2. **[Предоставьте администратору Azure Active Directory](../sql-database/sql-database-aad-authentication-configure.md#provision-an-azure-active-directory-administrator-for-your-azure-sql-database-server)** для сервера Azure SQL server на портал Azure, если вы еще этого не сделали. Администратор Azure AD может быть пользователем Azure AD или группой Azure AD. Если вы предоставляете группе с управляемым удостоверением роль администратора, пропустите шаги 3 и 4. Администратор будет иметь полный доступ к базе данных.
...
1. **[Предоставьте администратору Azure Active Directory](../sql-database/sql-database-aad-authentication-configure.md#provision-an-azure-active-directory-administrator-for-your-azure-sql-database-server)** для сервера Azure SQL server на портал Azure, если вы еще этого не сделали. Администратор Azure AD может быть пользователем Azure AD или группой Azure AD. Если вы предоставляете группе с управляемым удостоверением роль администратора, пропустите шаги 3 и 4. Администратор будет иметь полный доступ к базе данных.
...
| Свойство  | Описание                                                  | Обязательный                    |
| :-------- | :----------------------------------------------------------- | :-------------------------- |
| type      | Свойство **Type** набора данных должно иметь значение **AzureSqlDWTable**. | Да                         |
...
Чтобы скопировать данные из Azure синапсе Analytics, установите свойство **Type** в источнике действия копирования в значение **SqlDWSource**. В разделе **источник** действия копирования поддерживаются следующие свойства.
...
### <a name="azure-synapse-analytics-as-sink"></a><a name="azure-sql-data-warehouse-as-sink"></a>Azure синапсе Analytics в качестве приемника
...
Чтобы скопировать данные в хранилище данных SQL Azure, задайте тип приемника **SqlDWSink** в действии копирования. В разделе **приемника** действия копирования поддерживаются следующие свойства:
...
| polyBaseSettings  | Группа свойств, которые могут быть заданы, если `allowPolybase` свойство имеет значение **true**. | Нет.<br/>Применяется при использовании Polybase. |
...
| копикоммандсеттингс | Группа свойств, которые могут быть заданы, `allowCopyCommand` если свойство имеет значение true. | Нет.<br/>Применяется при использовании копирования. |
...
В действии копирования поддерживаются `polyBaseSettings` следующие параметры polybase:

| Свойство          | Описание                                                  | Обязательный                                      |
| :---------------- | :----------------------------------------------------------- | :-------------------------------------------- |
| rejectValue       | Указывает количество или процент строк, которые могут быть отклонены, прежде чем запрос завершится с ошибкой.<br/><br/>Дополнительные сведения о параметрах отклонения в Polybase см. в разделе arguments [инструкции CREATE External Table (Transact-SQL)](https://msdn.microsoft.com/library/dn935021.aspx). <br/><br/>Допустимые значения: 0 (по умолчанию), 1, 2 и. т. д. | Нет                                            |
| rejectType        | Указывает, является ли параметр **rejectValue** литеральным или процентным.<br/><br/>Допустимые значения: **value** (по умолчанию) и **процент**. | Нет                                            |
...
1. **Связанная служба источника** имеет следующие типы и методы проверки подлинности:
    ...
    | :----------------------------------------------------------- | :---------------------------------------------------------- |
    | [Большой двоичный объект Azure](connector-azure-blob-storage.md)                | Проверка подлинности ключа учетной записи, управляемая идентификация |
...
2. **Исходный формат данных** — **Parquet**, **ORC**или **текст с разделителями**, со следующими конфигурациями:
   ...
   7. `compression`не может иметь **Сжатие**, **gzip**или **Deflate**.

3. Если источником является папка, `recursive` в действии копирования должно быть задано значение true.

4. `wildcardFolderPath``modifiedDateTimeStart`, `wildcardFilename`, `modifiedDateTimeEnd` и `additionalColumns` не указаны.
...
В следующих разделах приводятся рекомендации в дополнение к тем, которые были упомянуты в рекомендациях по работе с [Azure синапсе Analytics](../synapse-analytics/sql/best-practices-sql-pool.md).
...
Загрузки PolyBase ограничены записями размером менее 1 МБ. Его нельзя использовать для загрузки в ПЕРЕМЕННЫЕ VARCHR (MAX), NVARCHAR (MAX) или VARBINARY (MAX). Дополнительные сведения см. в статье [Ограничения емкости хранилища данных SQL](../synapse-analytics/sql-data-warehouse/sql-data-warehouse-service-capacity-limits.md#loads).
...
## <a name="use-copy-statement-to-load-data-into-azure-sql-data-warehouse-preview"></a><a name="use-copy-statement"></a>Использование инструкции COPY для загрузки данных в хранилище данных SQL Azure (Предварительная версия)
...
1. **Исходная связанная служба и формат** имеют следующие типы и методы проверки подлинности:
    ...
    | :----------------------------------------------------------- | -------------------------- | :----------------------------------------------------------- |
    | [Большой двоичный объект Azure](connector-azure-blob-storage.md)                | [Текст с разделителями](format-delimited-text.md)             | Аутентификация ключа учетной записи, проверка подлинности подписанного URL-идентификатора, проверка подлинности с помощью управляемого удостоверения |
...
2. Параметры формата имеют следующие значения:
   ...
   1. Для **Parquet**: `compression` не может иметь **Сжатие**, **привязку**или **gzip**.
   2. Для **ORC**: `compression` не может иметь **Сжатие**, **```zlib```** или **привязку**.
   3. Для **текста с разделителями**:
      ...
      3. `encodingName`по умолчанию используется значение **UTF-8 или UTF-16**.
      4. `escapeChar`должен быть таким же `quoteChar`, как и, и не является пустым.
      5. `skipLineCount`параметр принимает значение по умолчанию или значение 0.
...
В действии копирования поддерживаются `allowCopyCommand` следующие параметры инструкции Copy:
...
**Размер пакета**: введите размер пакета для фрагментов больших данных в операциях чтения. В потоках данных ADF будет использовать этот параметр для установки кэширования Spark по столбцам. Это поле параметра, в котором будут использоваться параметры Spark по умолчанию, если они не заданы.
...
При копировании данных из или в Azure синапсе Analytics в качестве промежуточных типов данных фабрики данных Azure используются следующие сопоставления типов данных Azure синапсе Analytics. Сведения о том, как действие копирования сопоставляет исходную схему и тип данных с приемником, см. в разделе [сопоставления схем и типов данных](copy-activity-schema-and-type-mapping.md) .

>[!TIP]
>Дополнительные сведения о поддерживаемых типах данных и обходных путях для неподдерживаемых типов см. в статье о [типах данных таблиц в Azure синапсе Analytics](../synapse-analytics/sql/develop-tables-data-types.md) .
...
Список хранилищ данных, поддерживаемых в качестве источников и приемников в рамках действия копирования в фабрике данных Azure, см. в разделе [Поддерживаемые хранилища данных и форматы](copy-activity-overview.md#supported-data-stores-and-formats).
