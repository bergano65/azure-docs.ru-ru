...
1. **Служба, связанная с источником,** связана со следующими типами и методами аутентификации:
    ...
    | :----------------------------------------------------------- | :---------------------------------------------------------- |
    | [Лазурный Blob](connector-azure-blob-storage.md)                | Проверка подлинности ключа учетной записи, управляемая аутентификация личности |
    | [Хранилище Azure Data Lake Storage 1-го поколения](connector-azure-data-lake-store.md) | Проверка подлинности субъекта-службы                            |
    | [Лазурное хранилище озер данных Gen2](connector-azure-data-lake-storage.md) | Проверка подлинности ключа учетной записи, управляемая аутентификация личности |
...
Следующие разделы предоставляют лучшие практики в дополнение к тем, которые упоминаются в [рекомендациях Azure Synapse Analytics.](../synapse-analytics/sql-data-warehouse/sql-data-warehouse-best-practices.md)
...
1. **Служба и формат, связанный с источником,** относятся к следующим типам и методам аутентификации:
    ...
    | :----------------------------------------------------------- | -------------------------- | :----------------------------------------------------------- |
    | [Лазурный Blob](connector-azure-blob-storage.md)                | [Делимитированый текст](format-delimited-text.md)             | Проверка подлинности ключа учетной записи, общая аутентификация подписи доступа, основная аутентификация службы, управляемая аутентификация идентификации |
    ...
    | [Лазурное хранилище озер данных Gen2](connector-azure-data-lake-storage.md) | [Делимитированый текст](format-delimited-text.md)<br/>[Паркетным](format-parquet.md)<br/>[Орк](format-orc.md) | Проверка подлинности ключа учетной записи, основная аутентификация службы, управляемая аутентификация личности |
...
2. Настройки формата приведены в следующем:
   ...
   2. Для **ORC**: `compression` не может быть **сжатия,** **zlib**, или **Snappy**.
...
>[!TIP]
>Ссылайтесь на [типы данных таблицы в статье Azure Synapse Analytics,](../synapse-analytics/sql-data-warehouse/sql-data-warehouse-tables-data-types.md) на которых поддерживаемые типы данных и обходные пути для неподдерживаемых.
