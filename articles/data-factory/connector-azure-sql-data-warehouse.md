...
| дисаблеметриксколлектион | Фабрика данных собирает такие метрики, как DWU хранилища данных SQL для оптимизации производительности копирования и рекомендации. Если вы отвечаете за такое поведение, укажите значение `true`, чтобы отключить его. | Нет (значение по умолчанию — `false`) |
...
2. **Исходный формат данных** — **Parquet**, **ORC**или **текст с разделителями**, со следующими конфигурациями:
   ...
   2. Имя файла пустое или указывает на один файл. Если указать имя файла с подстановочными знаками в действии копирования, оно может быть только `*` или `*.*`.
   3. `rowDelimiter` — **Default**, **\n**, **\r\n**или **\r**.
   4. `nullValue` принимает значение по умолчанию или задает **пустую строку** (""), а `treatEmptyAsNull` — по умолчанию или значение true.
   5. `encodingName` принимает значение по умолчанию или значение **UTF-8**.
...
3. Если источником является папка, для `recursive` в действии копирования должно быть задано значение true.
