...
Интеграция Runtime с большим количеством ядер увеличивает количество узлов в вычислительных средах Spark и обеспечивает большую вычислительную мощность для чтения, записи и преобразования данных. Потоки данных ADF использует Spark для вычислительного движка. Среда Spark очень хорошо работает на ресурсах, оптимизированных памятью.
...
### <a name="decrease-cluster-compute-start-up-time-with-ttl"></a>Уменьшение времени запуска кластерных вычислений с помощью TTL

В ИК Azure есть свойство под свойствами Data Flow, которое позволит вам выдержать пул кластерных вычислений для вашей фабрики. С помощью этого пула можно последовательно отправлять действия потока данных для выполнения. После создания пула каждое последующее задание займет 1-2 минуты для кластера Spark по требованию для выполнения вашей работы. Первоначальная настройка пула ресурсов займет около 6 минут. Укажите время, которое вы хотите сохранить в группе ресурсов в настройках time-to-live (TTL).

## <a name="optimizing-for-azure-sql-database-and-azure-sql-data-warehouse-synapse"></a>Оптимизация для базы данных Azure S'L и synapse хранилища данных Azure S'L
...
* Размер пакета поручает ADF хранить данные в наборах в памяти Spark вместо строки за строкой. Размер пакета является необязательным параметром, и у вас могут не быть ресурсов на вычислительных узлах, если они не рассчитаны должным образом. Не устанавливая это свойство будет использовать Spark кэширования пакет по умолчанию.
...
Чтобы избежать перевалки потоков данных, установите **размер пакета** во вкладке «Настройки» для поглотителей Azure S'L DB и Azure S'L DW. Если размер пакета установлен, база данных ADF обрабатывает данные, записывая пакетами в зависимости от предоставленного размера. Не устанавливая это свойство будет использовать Spark кэширования пакет по умолчанию.
