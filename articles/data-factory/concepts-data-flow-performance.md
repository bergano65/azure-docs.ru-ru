...
Integration Runtime с большим числом ядер увеличивает количество узлов в вычислительных средах Spark и обеспечивает дополнительную вычислительную мощность для чтения, записи и преобразования данных. Потоки данных ADF используют в ядре вычислений Spark. Среда Spark прекрасно работает с ресурсами, оптимизированными для памяти.
...
### <a name="decrease-cluster-compute-start-up-time-with-ttl"></a>Уменьшить время начала вычислений кластера с TTL

В Azure IR в разделе Свойства потока данных имеется свойство, которое позволит вам выделить пул ресурсов кластера для фабрики. С помощью этого пула можно последовательно отправлять действия потока данных для выполнения. После установки пула для выполнения задания в кластере Spark по запросу потребуется 1-2 минут. Первоначальная настройка пула ресурсов займет около 6 минут. Укажите период времени, в течение которого требуется хранить пул ресурсов в параметре срок жизни (TTL).

## <a name="optimizing-for-azure-sql-database-and-azure-sql-data-warehouse-synapse"></a>Оптимизация для базы данных SQL Azure и хранилища данных SQL Azure синапсе
...
* Размер пакета указывает ADF-файл хранить данные в наборах в памяти Spark, а не построчно. Размер пакета является необязательным параметром, и на этих узлах могут быть исчерпаны ресурсы, если они имеют неправильный размер. Если не задать это свойство, будут использоваться параметры пакета по умолчанию для кэширования Spark.
...
Чтобы избежать построчной обработки потоков данных, задайте **Размер пакета** на вкладке Параметры для базы данных SQL Azure и приемников хранилища Azure SQL. Если задан размер пакета, ADF обрабатывает запись в пакетах на основе указанного размера. Если не задать это свойство, будут использоваться параметры пакета по умолчанию для кэширования Spark.
...
Управление производительностью соединений в потоке данных является очень распространенной операцией, которая будет выполняться на протяжении всего жизненного цикла преобразований данных. В ADF потоки данных не нуждаются в сортировке данных перед соединением, так как эти операции выполняются в качестве хэш-соединений в Spark. Тем не менее, можно воспользоваться улучшенной производительностью с помощью оптимизации соединений "вещание", которое применяется к преобразованиям соединений, существующих и поисков.

Это позволит избежать перемещения в случайном порядке, отправляя содержимое любой стороны отношения объединения в узел Spark. Это хорошо подходит для небольших таблиц, используемых для поиска ссылок. Большие таблицы, которые могут не поместиться в память узла, не являются хорошими кандидатами для оптимизации вещания.

Рекомендуемая конфигурация для потоков данных с множеством операций объединения заключается в том, чтобы задать для оптимизации значение "Авто" для "вещания" и использовать оптимизированную для памяти Azure Integration Runtime конфигурацию. При возникновении ошибок нехватки памяти или истечения времени ожидания вещания во время выполнения потока данных можно отключить оптимизацию вещания. Однако это приведет к более медленному выполнению потоков данных. При необходимости можно указать потоку данных включение только левую или правую часть объединения или и то, и другое.

![Параметры вещания](media/data-flow/newbroad.png "Параметры вещания")
