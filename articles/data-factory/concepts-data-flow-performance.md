...
### <a name="set-isolation-level-on-source-transformation-settings-for-sql-datasets"></a>Установка уровня изоляции на параметры преобразования источника для наборов данных SQL

* Read uncommitted предоставит быстрее достигать результатов за запрос на преобразование источника
...
### <a name="looping-through-file-lists"></a>Циклическое списки файлов

В большинстве случаев данные передаются в ADF будет выполняться лучше из конвейера, который предоставляет возможность преобразования источника потока данных для выполнения итерации по несколько файлов. Другими словами лучше использовать подстановочные знаки или списки файлов внутри источника данных потока, для выполнения итерации по большой список файлов, использование оператора ForEach в конвейере, вызывая выполнение потока данных в каждой итерации. Процесс потока данных будет выполняться быстрее, позволяя циклов в потоке данных.

Например если у меня есть список файлов данных от июля 2019 г, который должен обрабатывать в папке в хранилище BLOB-объектов, было бы более высокую производительность для вызова действия выполнения потока данных один раз из конвейера и использовать подстановочные знаки в источнике следующим образом :

```DateFiles/*_201907*.txt```

Это работают лучше, чем простой поиск по Store BLOB-объектов в конвейере, который затем используется для итерации по всем соответствующим файлам, с помощью ForEach с действием выполнения потока данных внутри.
