...
![Data Flow Monitoring](media/data-flow/mon003.png "Монитор потока данных 3") (Мониторинг потоков данных)
 ...
 При выполнении отладки конвейера в расчет общей производительности следует включить примерно одну минуту на подготовку кластера. При инициализации среды Azure Integration Runtime по умолчанию время подготовки может составить около 5 минут.
...
* Если скорость обработки должна быть выше, чем скорость ввода, попробуйте использовать кластер, **оптимизированный для вычислений**.
* Если требуется кэшировать больше данных в памяти, попробуйте использовать кластер, **оптимизированный для памяти**. Оптимизированный для памяти кластер имеет более высокую цену за ядро, чем оптимизированный для вычислений, но, скорее всего, скорость преобразования будет выше. Если при выполнении потоков данных возникают ошибки нехватки памяти, перейдите на конфигурацию Azure IR, оптимизированную для памяти.
...
При каждом преобразовании можно задать схему секционирования, которую должна использовать фабрика данных, на вкладке "Оптимизация". Рекомендуется сначала протестировать приемники на основе файлов с секционированием и оптимизациями по умолчанию.
...
При последовательном использовании For Each с потоком данных рекомендуется использовать параметр срока жизни в Azure Integration Runtime. Это связано с тем, что каждый файл будет требовать 5 минут на запуск кластера внутри перечислителя.
...
## <a name="join-performance"></a>Производительность объединения
...
Для потоков данных со множеством операций объединения рекомендуется оставить для оптимизации "Рассылка" значение "Авто" и использовать оптимизированную для памяти конфигурацию Azure Integration Runtime. При возникновении ошибок нехватки памяти или истечения времени ожидания рассылки во время выполнения потока данных можно отключить оптимизацию рассылкой. Однако это приведет к более медленному выполнению потоков данных. При необходимости можно указать, что поток данных должен принудительно отправлять только левую часть объединения, только правую часть или обе части.
