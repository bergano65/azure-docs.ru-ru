...
* Во время отладки предварительной версии данных в конструкторе потока данных можно ограничить объем проверяемых данных для каждого источника, установив ограничение на строку из ссылки параметры отладки в пользовательском интерфейсе конструктора потока данных. Обратите внимание, что сначала необходимо включить режим отладки.

![Параметры отладки](media/data-flow/debug-settings.png "Параметры отладки")

* При тестировании потоков данных из выполнения отладки конвейера можно ограничить количество строк, используемых для тестирования, задав размер выборки в каждом из источников. Не забудьте отключить выборку при планировании конвейеров по регулярному рабочему расписанию.

![Выборка строк](media/data-flow/source1.png "Выборка строк")
...
* При отладке в предварительной версии данных и отладке конвейера Обратите внимание, что размер и выборка для файловых наборов данных на основе файлов применяются только к количеству возвращаемых строк, а не к числу считанных строк. Это важно отметить, потому что это может повлиять на производительность выполнения отладки и, возможно, приведет к сбою последовательности.
* Помните, что отладочные кластеры по умолчанию представляют собой небольшие кластеры с одним узлом, поэтому для отладки используйте временные небольшие файлы. Перейдите в раздел Параметры отладки и укажите небольшое подмножество данных с помощью временного файла.
...
По умолчанию при включении отладки будет использоваться среда выполнения интеграции Azure по умолчанию, которая создается автоматически для каждой фабрики данных. Этот Azure IR по умолчанию установлен для 8 ядер, 4 для узла драйвера и 4 для рабочего узла с использованием общих свойств вычислений. При тестировании с большим объемом данных можно увеличить размер кластера отладки, создав новый Azure IR с более крупными конфигурациями и выбрав этот новый Azure IR при переключении на отладку. Это позволит ADF использовать этот Azure IR для предварительной версии данных и отладки конвейера с потоками данных.
