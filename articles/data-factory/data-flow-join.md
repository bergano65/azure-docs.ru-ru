...
> [!NOTE]
> Подсистема Spark, используемая потоками данных, иногда может использовать декартово продукты в условиях соединения. В этом случае можно переключиться на пользовательское перекрестное соединение и ввести условие объединения вручную. Это может привести к снижению производительности потоков данных, так как подсистеме выполнения может потребоваться вычислить все строки по обеим сторонам связи, а затем отфильтровать строки.
...
Этот тип объединения можно использовать для неэквивалентных соединений и условий ```OR```.
...
Ниже приведен пример преобразования «соединение» с именем `JoiningColumns`, которое принимает левый поток `LeftStream` и правый поток `RightStream`. Это преобразование принимает два потока и объединяет все строки, где столбец `leftstreamcolumn` больше, чем `rightstreamcolumn`столбца. Параметр `joinType` имеет значение `cross`. Вещание не включено, `broadcast` имеет значение `'none'`.
