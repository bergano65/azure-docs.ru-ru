...
| ***Нахождение файлов для копирования** _ |  |  |
| ВАРИАНТ 1. Статический путь<br> | Копирование из пути к папке или файлу, указанному в наборе данных. Если вы хотите скопировать все большие файлы из папки, дополнительно укажите `wildcardFileName` как `_`. |  |
...
| recursive | Указывает, следует ли читать данные рекурсивно из вложенных папок или только из указанной папки. Если для свойства recursive задано значение true, а приемником является файловое хранилище, то пустые папки и вложенные папки в приемнике не создаются. <br>Допустимые значения: _true * (по умолчанию) и *false*.<br>Это свойство не применяется при настройке `fileListPath`. |Нет |
