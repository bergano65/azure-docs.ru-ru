...
| усетемпфилеренаме | Укажите, следует ли передать во временный файл, переименовать или непосредственно записать в целевую папку или расположение файла. По умолчанию ADF сначала выполняет запись во временные файлы, а затем переименует их после завершения передачи, в то же время с 1) избежать конфликта записи, что приведет к повреждению файла, если у вас есть другой процесс записи в тот же файл, и 2) убедиться, что исходная версия файла существует во время Перенос целиком. Если сервер SFTP не поддерживает операцию переименования, отключите этот параметр и убедитесь, что у вас нет параллельной записи в целевой файл. См. Совет по устранению неполадок ниже этой таблицы. | Нет. Значение по умолчанию — true. |
