...
description: Сведения о том, как копировать данные с SFTP-сервера и на него с использованием Фабрики данных Azure.
...
ms.locfileid: "83653741"
---
# <a name="copy-data-from-and-to-sftp-server-using-azure-data-factory"></a>Копирование данных с SFTP-сервера и на него с помощью Фабрики данных Azure
...
В этой статье рассказывается, как скопировать данные с SFTP-сервера и на него. Дополнительные сведения о Фабрике данных Azure см. во [вводной статье](introduction.md).
...
- Копирование файлов с SFTP-сервера и на него с использованием **базовой** аутентификации или аутентификации **SshPublicKey**.
...
| Свойство   | Описание                                                  | Обязательно |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | Для свойства типа в `location` для набора данных необходимо задать значение **SftpLocation**. | Да      |
...
| Свойство                 | Описание                                                  | Обязательно |
| ------------------------ | ------------------------------------------------------------ | -------- |
| type                     | Для свойства типа в `storeSettings` необходимо задать значение **SftpWriteSettings**. | Да      |
...
| useTempFileRename | Укажите, следует ли передать данные во временный файл и переименовать его или сразу вести запись в целевую папку и файл. По умолчанию ADF сначала выполняет запись во временные файлы, а после завершения отправки переименовывает эти файлы, чтобы, во-первых, избежать конфликтов записи с повреждением файла, если другой процесс будет записывать тот же файл, и, во-вторых, сохранить исходную версию целевого файла на весь период передачи данных. Если SFTP-сервер не поддерживает операцию переименования, отключите этот параметр и следите за тем, чтобы не возникало параллельных операций записи в один целевой файл. Изучите также совет по устранению неполадок, указанный под этой таблицей. | Нет. Значение по умолчанию — true. |
...
>[!TIP]
>Если при записи данных на SFTP-сервер возникают ошибки UserErrorSftpPathNotFound, UserErrorSftpPermissionDenied или SftpOperationFail, а у пользователя SFTP нет правильных разрешений, проверьте, поддерживает ли этот SFTP-сервер операцию переименования. Если нет, то отключите параметр отправки через временный файл (`useTempFileRename`) и повторите попытку. В таблице выше приведены дополнительные данные об этом свойстве. Если для копирования используется локальная среда выполнения интеграции, используйте версию не ниже 4.6.
