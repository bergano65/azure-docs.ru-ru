...
| authenticationType | Укажите тип аутентификации.<br/>Допустимые значения: **Базовый**, **SshPublicKey**. Дополнительные свойства и примеры JSON см. в разделах [использование обычной проверки подлинности](#using-basic-authentication) и [использование проверки подлинности с открытым ключом SSH](#using-ssh-public-key-authentication) . |Да |
...
| пароль | Пароль пользователя (userName). Пометьте это поле как SecureString, чтобы безопасно хранить его в фабрике данных, или [добавьте ссылку на секрет, хранящийся в Azure Key Vault](store-credentials-in-key-vault.md). | Да |
...
Следующие свойства поддерживаются для SFTP в `location` параметрах в наборе данных на основе формата:

| Свойство   | Описание                                                  | Обязательный |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | Свойство `location` Type в наборе данных должно иметь значение **сфтплокатион**. | Да      |
...
| усетемпфилеренаме | Укажите, следует ли передать во временный файл, переименовать или непосредственно записать в целевую папку или расположение файла. По умолчанию ADF сначала выполняет запись во временные файлы, а затем при завершении отправки переименование файлов, в порядке 1), чтобы избежать конфликта записи, что приведет к повреждению файла, если другой процесс выполняет запись в тот же файл, и 2) убедитесь, что исходная версия файла существует во время передачи во весь файл. Если сервер SFTP не поддерживает операцию переименования, отключите этот параметр и убедитесь, что у вас нет параллельной записи в целевой файл. См. Совет по устранению неполадок ниже этой таблицы. | Нет. Значение по умолчанию — true. |
...
>[!TIP]
>Если при записи данных в SFTP произошла ошибка "Усереррорсфтппаснотфаунд", "Усереррорсфтппермиссиондениед" или "Сфтпоператионфаил", а используемый пользователь SFTP имеет соответствующее разрешение, проверьте, поддерживает ли сервер SFTP операцию переименования файлов (если нет), отключите параметр "передать с временным файлом" (`useTempFileRename`) и повторите попытку. Дополнительные сведения об этом свойстве см. в таблице выше. Если для копирования используется локально размещенная Integration Runtime, убедитесь, что используется версия 4,6 или более поздняя.
