...
Этот разъем HTTP поддерживается для следующих действий:
...
**Пример 2: Использование встроенныхCertData**
...
Следующие свойства поддерживаются `location` для HTTP в настройках в наборе данных на основе формата:

| Свойство    | Описание                                                  | Обязательно |
| ----------- | ------------------------------------------------------------ | -------- |
| type        | Свойство типа, под `location` аимтом данных, должно быть установлено в **HttpServerLocation.** | Да      |
| relativeUrl | Относительный URL-адрес ресурса, который содержит данные. Разъем HTTP копирует данные `[URL specified in linked service][relative URL specified in dataset]`из объединенного URL:: .   | нет       |
...
Следующие свойства поддерживаются `storeSettings` для HTTP в настройках в формате на основе источника копий:

| Свойство                 | Описание                                                  | Обязательно |
| ------------------------ | ------------------------------------------------------------ | -------- |
| type                     | Свойство типа `storeSettings` под должны быть установлены на **HttpReadSettings.** | Да      |
...
| Свойство | Описание | Обязательно |
|:--- |:--- |:--- |
| type | Свойство **типа** набора данных должно быть установлено в **HttpFile.** | Да |
...
| compression | Укажите тип и уровень сжатия данных. Дополнительные сведения см. в разделе [Поддержка сжатия](supported-file-formats-and-compression-codecs-legacy.md#compression-support).<br/><br/>Поддерживаемые типы: **GZip**, **Deflate**, **BZip2** и **ZipDeflate**.<br/>Поддерживаемые уровни: **Оптимальные** и **быстрые**. |нет |
...
| Свойство | Описание | Обязательно |
|:--- |:--- |:--- |
| type | Свойство **типа** источника активности копирования должно быть установлено на **HttpSource.** | Да |
| httpRequestTimeout | Время ожидания (значение **Временной диапазон**) ответа для HTTP-запроса. Это значение является интервалом времени для получения ответа, а не считывания данных ответа. По умолчанию используется значение **00:01:40**.  | нет |
