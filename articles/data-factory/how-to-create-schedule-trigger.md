---
title: Создание триггеров расписания на фабрике данных Azure
...
> [!NOTE]
> Для полного пошагового решения создания конвейера и спускового крючка графика, который связывает триггер с конвейером, а также запускает и контролирует конвейер, [см.](quickstart-create-data-factory-portal.md)

1. Переключитесь на вкладку **Edit,** показанную с помощью символа карандаша. 

    ![Открытие вкладки "Правка"](./media/how-to-create-schedule-trigger/switch-edit-tab.png)

1. Выберите **Триггер** в меню, а затем выберите **New/Edit**. 

    ![Меню создания триггера](./media/how-to-create-schedule-trigger/new-trigger-menu.png)

1. На странице **Добавить триггеры,** выберите **триггер ...**, а затем выберите **новый**. 

    ![Добавление триггеров — новый триггер](./media/how-to-create-schedule-trigger/add-trigger-new-button.png)

1. На странице **New Trigger** сделайте следующие шаги: 
    ...
    1. В поле **Окончание** выберите **Нет конца**, если вы не хотите указывать дату и время окончания для триггера. Чтобы указать время даты окончания, выберите **на дату**и указать время окончания, а затем выберите **OK**. Каждое выполнение конвейера оплачивается. Если выполняется тестирование, настройте конвейер таким образом, чтобы он активировался всего несколько раз. Но при этом обязательно оставьте достаточно времени, чтобы конвейер успел запуститься между временем публикации и временем окончания. Триггер вступит в силу только после публикации решения в службе "Фабрика данных", а не сразу при сохранении триггера в пользовательском интерфейсе.

        ![Настройки триггера](./media/how-to-create-schedule-trigger/trigger-settings.png)

1. В новом окне **триггера** выберите **«Да»** в **опционе «Активированный»,** а затем выберите **OK**. С помощью этого флажка позже можно деактивировать этот триггер. 

    ![Настройки триггера — кнопка "Далее"](./media/how-to-create-schedule-trigger/trigger-settings-next.png)

1. В **окне нового триггера** просмотрите сообщение предупреждения, а затем выберите **OK**.

    ![Настройки триггера — кнопка "Готово"](./media/how-to-create-schedule-trigger/new-trigger-finish.png)

1. Выберите **Опубликовать все** для публикации изменений в Data Factory. До публикации изменений в Data Factory триггер не запускает запуск конвейера. 

    ![Кнопка "Опубликовать"](./media/how-to-create-schedule-trigger/publish-2.png)

1. Переключиться на **вкладку Pipeline** влево, а затем выберите **Обновление,** чтобы обновить список. Вы увидите, что запуск конвейера срабатывает запланированным триггером. Обратите внимание на значения в столбце **Активировано**. Если вы используете опцию **Trigger Now,** вы увидите ручной триггер в списке. 
...
1. Создайте триггер с помощью смдлета **Set-AzDataFactoryV2Trigger:**

    ```powershell
    Set-AzDataFactoryV2Trigger -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name "MyTrigger" -DefinitionFile "C:\ADFv2QuickStartPSH\MyTrigger.json"
    ```

1. Подтвердите, что состояние триггера **остановлено** с помощью **cmdlet Get-AzDataFactoryV2Trigger:**

    ```powershell
    Get-AzDataFactoryV2Trigger -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name "MyTrigger"
    ```

1. Запустите триггер с помощью cmdlet **Start-AzDataFactoryV2Trigger:**

    ```powershell
    Start-AzDataFactoryV2Trigger -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name "MyTrigger"
    ```

1. Подтвердите, что состояние триггера **запускается** с помощью **cmdlet Get-AzDataFactoryV2Trigger:**

    ```powershell
    Get-AzDataFactoryV2Trigger -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name "MyTrigger"
    ```

1.  Запуск триггера в Azure PowerShell с помощью cmdlet **Get-AzDataFactoryV2TriggerRun.** Для этого следующую команду нужно выполнять периодически. Обновите значения **TriggerRunStartedAfter** и **TriggerRunStartedBefore** в соответствии со значениями в определении триггера:
...
| **Интервал** | Положительное целое число, указывающее интервал для значения **frequency**, которое определяет, как часто выполняется триггер. Например, если **interval** имеет значение 3, а для элемента **frequency** выбран вариант week (неделя), триггер выполняется один раз каждые 3 недели. |
...
| **Интервал** | Число | нет | 1 | 1–1000 | `"interval":10` |
...
|:--- |:--- |:--- |
| **Минут** | Минуты часа, в которые будет выполняться триггер. | <ul><li>Целое число</li><li>массив целых чисел</li></ul>
| **Часов** | Часы дня, в которые будет выполняться триггер. | <ul><li>Целое число</li><li>массив целых чисел</li></ul> |
| **Будние дни** | Дни недели, по которым выполняется триггер. Значение можно указать только с еженедельной частотой. | <ul><li>Понедельник, вторник, среда, четверг, пятница, суббота, воскресенье</li><li>Массив значений дня (максимальный размер массива — 7)</li><li>Значения дня можно вводить в любом регистре</li></ul> |
