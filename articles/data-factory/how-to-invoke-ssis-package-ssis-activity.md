...
При выборе в качестве расположения пакета **внедренного пакета** выполните следующие действия.
   ...
   1. Перетащите пакет, чтобы запустить или **передать** его из папки с файлами в предоставленное поле. Пакет будет автоматически сжат и внедрен в полезные данные действия. После внедрения можно **загрузить** пакет позже для редактирования. Можно также **параметризовать** внедренный пакет, назначив его параметру конвейера, который может использоваться в нескольких действиях, таким образом, оптимизируя размер полезных данных конвейера. 
   
   1. Если встроенный пакет не зашифрован и в нем обнаружено использование задачи «Выполнение пакета», то флажок **выполнить пакетную задачу** будет автоматически установлен, и соответствующие дочерние пакеты со ссылками на файловую систему будут добавлены автоматически. 
   
      Если не удается обнаружить использование задачи «Выполнение пакета», необходимо вручную установить флажок « **выполнение задачи пакета** » и добавить соответствующие дочерние пакеты со ссылками файловой системы друг на друга, а также внедрить их. Если дочерние пакеты используют ссылки SQL Server, убедитесь, что SQL Server доступен Azure-SSIS IR.  Использование ссылок на проекты для дочерних пакетов в настоящее время не поддерживается.
