...
Используйте [Построитель выражений](concepts-data-flow-expression-builder.md) для ввода условия соответствия. Создайте логическое выражение, которое соответствует столбцам, основанным на `name` `type` столбцах,, `stream` и `position` . Шаблон будет влиять на любой столбец, смещенное или определенное, где условие возвращает значение true.
