...
1. На странице **Новая фабрика данных** в поле **Имя** введите **ADFTutorialDataFactory**.
   ...
   Имя фабрики данных должно быть *глобально уникальным*. Если появится сообщение об ошибке со значением Name, введите другое имя для фабрики данных (например, Йоурнамеадфтуториалдатафактори). Дополнительные сведения о правилах именования артефактов фабрики данных см. в статье [Фабрика данных Azure — правила именования](naming-rules.md).
...
1. По завершении создания вы увидите уведомление в центре уведомлений. Выберите **Переход к ресурсу** , чтобы открыть страницу **фабрики данных** .
...
1. В разделе **Конфигурация виртуальной сети (Предварительная версия)** выберите **включить**.
...
1. На странице **Начало работы** выберите **Create pipeline** (Создать конвейер).
   ...
   ![Снимок экрана, показывающий создание конвейера.](./media/doc-common-process/get-started-page.png)
...
1. Убедитесь, что вы включили **интерактивную разработку**. Включение может занять около минуты.

    ![Снимок экрана, показывающий интерактивную разработку.](./media/tutorial-data-flow-private/interactive-authoring.png)
...
1. Не закрывайте диалоговое окно, а затем перейдите в свою учетную запись хранения.

1. Следуйте инструкциям в [этом разделе](#approval-of-a-private-link-in-a-storage-account), чтобы утвердить частную ссылку.

1. Вернитесь к диалоговому окну. Щелкните **проверить подключение** еще раз и выберите **создать** , чтобы развернуть связанную службу.
...
Если вы не использовали гиперссылку при проверке предыдущего подключения, перейдите по пути. Теперь необходимо создать управляемую закрытую конечную точку, которая будет подключаться к созданной связанной службе.

1. Перейдите на вкладку **Управление** .

   > [!NOTE]
   > Вкладка **Управление** может быть недоступна для всех экземпляров фабрики данных. Если вы не видите его, вы можете получить доступ к частным конечным точкам, выбрав **создать**  >  **подключения**  >  **Частная конечная точка**.

1. Перейдите в раздел **управляемые частные конечные точки** .
1. В разделе **управляемые частные конечные точки**выберите **+ создать** .

    ![Снимок экрана, на котором показана кнопка создания управляемых закрытых конечных точек.](./media/tutorial-data-flow-private/new-managed-private-endpoint.png) 
...
1. Нажмите кнопку **Создать**.
1. Через несколько секунд вы увидите, что для созданной частной ссылки требуется утверждение.
1. Выберите созданную закрытую конечную точку. Вы видите гиперссылку, которая приведет к утверждению частной конечной точки на уровне учетной записи хранения.
...
1. В учетной записи хранения перейдите к **папке частные конечные точки подключения** в разделе **Параметры** .
...
1. Вернитесь в раздел **управляемые частные конечные точки** на вкладке **Управление** в фабрике данных.
