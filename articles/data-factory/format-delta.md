...
| Update - метод | Укажите, какие операции обновления разрешены в Дельта Lake. Для методов, которые не вставляются, для пометки строк требуется предшествующее преобразование ALTER Row. | да | `true` либо `false` | удаляемые <br> Insertable <br> обновляемые <br> merge |
