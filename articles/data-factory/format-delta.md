...
| Файловая система | Контейнер/файловая система разностной версии Lake | да | Строковый | fileSystem |
| Путь к папке | Непосредственное от Дельта Lake | да | Строковый | folderPath |
| Тип сжатия | Тип сжатия разностной таблицы | нет | `bzip2`<br>`gzip`<br>`deflate`<br>`ZipDeflate`<br>`snappy`<br>`lz4` | компрессионтипе |
| Уровень сжатия | Выберите, будет ли сжатие выполняться как можно быстрее или если полученный файл должен быть оптимально сжат. | обязательный `compressedType` , если указан. | compressionLevel |
