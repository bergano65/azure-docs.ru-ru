...
| Уровень сжатия | Выберите, будет ли сжатие выполняться как можно быстрее или если полученный файл должен быть оптимально сжат. | обязательный `compressedType` , если указан. | `Optimal` либо `Fastest` | compressionLevel |
...
| Update - метод | Укажите, какие операции обновления разрешены в Дельта Lake. Для методов, которые не вставляются, для пометки строк требуется предшествующее преобразование ALTER Row. | да | `true` либо `false` | удаляемые <br> Insertable <br> обновляемые <br> упсертабле |
