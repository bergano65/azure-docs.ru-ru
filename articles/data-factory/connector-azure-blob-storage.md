...
>[!NOTE]
>- Если вы хотите использовать общедоступную среду выполнения интеграции Azure для подключения к хранилищу BLOB-объектов, используя параметр **разрешить доверенным службам Майкрософт доступ к этой учетной записи хранения** , включенный в брандмауэре службы хранилища Azure, необходимо использовать [проверку подлинности управляемого удостоверения](#managed-identity).
>- При использовании инструкций Polybase или COPY для загрузки данных в Azure синапсе Analytics, если в исходном или промежуточном хранилище BLOB-объектов настроена конечная точка виртуальной сети Azure, необходимо использовать проверку подлинности управляемого удостоверения в соответствии с требованиями синапсе. Дополнительные требования к конфигурации см. в разделе [Проверка подлинности управляемого удостоверения](#managed-identity) .
...
Дополнительные сведения о подписанных URL-адресах см. в разделе [Использование подписанных URL-адресов (SAS): Что такое подписанный URL-адрес?](../storage/common/storage-sas-overview.md)
...
2. Предоставьте правильное разрешение субъекта-службы в Хранилище BLOB-объектов Azure. Дополнительные сведения о ролях см. в статье [использование портал Azure для назначения роли Azure для доступа к данным BLOB-объектов и очередей](../storage/common/storage-auth-aad-rbac-portal.md).
...
| аккаунткинд | Укажите тип учетной записи хранения. Допустимые значения: **Storage** (общего назначения v1), **StorageV2** (общее назначение версии 2), **блобстораже** или **блоккблобстораже**. <br/> При использовании связанной службы больших двоичных объектов Azure в потоке данных проверка подлинности управляемого удостоверения или субъекта-службы не поддерживается, если тип учетной записи является пустым или хранилищем. Укажите правильный тип учетной записи, выберите другую проверку подлинности или обновите учетную запись хранения до общего назначения версии 2. |Нет |
...
>[!NOTE]
>Если учетная запись BLOB-объекта поддерживает [обратимое удаление](../storage/blobs/soft-delete-blob-overview.md), проверка подлинности субъекта-службы не поддерживается в потоке данных.
...
2. Предоставьте разрешение на управляемое удостоверение в хранилище BLOB-объектов Azure. Дополнительные сведения о ролях см. в статье [использование портал Azure для назначения роли Azure для доступа к данным BLOB-объектов и очередей](../storage/common/storage-auth-aad-rbac-portal.md).

    - В **качестве источника** в **элементе управления доступом (IAM)** предоставьте по крайней мере роль **читателя данных BLOB-объекта хранилища** .
    - В **качестве приемника** в **элементе управления доступом (IAM)** предоставьте по крайней мере роль **участника данных BLOB-объекта хранилища** .

>[!IMPORTANT]
>Если вы используете инструкцию Polybase или COPY для загрузки данных из хранилища BLOB-объектов (в качестве исходного или промежуточного) в Azure синапсе Analytics, при использовании проверки подлинности управляемого удостоверения для хранилища BLOB-объектов убедитесь, что в [этом руководстве](../azure-sql/database/vnet-service-endpoint-rule-overview.md#impact-of-using-virtual-network-service-endpoints-with-azure-storage)вы также выполните шаги 1 – 3. Эти действия позволят зарегистрировать сервер в Azure AD и назначить серверу роль участника данных BLOB-объекта хранилища. Фабрика данных обрабатывает остальные компоненты. Если вы настраиваете хранилище BLOB-объектов с конечной точкой виртуальной сети Azure, вам также нужно **разрешить доверенным службам Майкрософт доступ к этой учетной записи хранения** , включенной в меню " **брандмауэры** учетных записей хранения Azure" и "Параметры виртуальных сетей" в соответствии с требованиями синапсе.
...
> [!NOTE]
> Если учетная запись BLOB-объекта поддерживает [обратимое удаление](../storage/blobs/soft-delete-blob-overview.md), то в потоке данных не поддерживается проверка подлинности управляемого удостоверения.
...
| ВАРИАНТ 1. Статический путь<br> | Копирование из указанного контейнера или папки/пути к файлу, которые указаны в наборе данных. Если вы хотите скопировать все большие двоичные объекты из контейнера или папки, дополнительно укажите `wildcardFileName` в качестве `_` . |  |
| ВАРИАНТ 2. Префикс BLOB-объекта<br>- prefix | Префикс для имени большого двоичного объекта в заданном контейнере, настроенном в наборе данных для фильтрации исходных больших двоичных объектов. Будут выбраны большие двоичные объекты, имена которых начинаются с `container_in_dataset/this_prefix` . Он использует фильтр на стороне службы для хранилища BLOB-объектов, что обеспечивает лучшую производительность по сравнению с фильтром с подстановочными знаками.<br><br>При использовании префикса и выборе копирования в файловый приемник с сохранением иерархии Обратите внимание на вложенный путь после последнего "/" в префиксе. Например, у вас есть исходный код  `container/folder/subfolder/file.txt` и настраивается префикс в виде `folder/sub` , а затем сохраненный путь к файлу — `subfolder/file.txt` . | нет                                                          |
...
* [Изменений](format-delta.md#mapping-data-flow-properties)
