...
тип | Необходимо задать **Until**. | String | Да
expression | Выражение, для которого возвращается значение true или false. | Выражение.  | Да
время ожидания | Цикл do-until выполняется, пока не истечет указанное здесь время ожидания. | Строка. `d.hh:mm:ss` (или) `hh:mm:ss`. Значение по умолчанию — 7 дней. Максимальное значение 90 дней. | Нет
Операции | Набор действий, которые выполняются до тех пор, пока выражение не получит значение `true`. | Массив действий. |  Да
