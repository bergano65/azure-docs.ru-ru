...
| type | Кодек сжатия, используемый для чтения и записи двоичных файлов. <br>Допустимые значения: **bzip2**, **gzip**, **Deflate**, **ZipDeflate**или **таргзип**. <br>**Обратите внимание** , что при использовании действия **ZipDeflate**копирования для распаковки / файлов**таргзип** ZipDeflate и записи в файловый приемник данных файлы по умолчанию извлекаются в папку:, с `<path specified in dataset>/<folder named as source compressed file>/` помощью параметра `preserveZipFileNameAsFolder` / `preserveCompressionFileNameAsFolder` [Источник действия копирования](#binary-as-source) можно указать, следует ли сохранять имена сжатых файлов в виде структуры папок.| нет       |
