---
title: Понимание цен на фабрику данных Azure на примерах
...
> [!NOTE]
> Цены, используемые в приведенных ниже примерах, являются гипотетическими и не предназначены для фактического ценообразования.
...
## <a name="using-mapping-data-flow-debug-for-a-normal-workday"></a>Использование отладки потока данных для обычного рабочего дня

Как инженер данных, вы отвечаете за проектирование, создание и тестирование картографических потоков данных каждый день. Вы входите в UI ADF утром и включаете режим Debug для потоков данных. По умолчанию TTL для сеансов Debug составляет 60 минут. Вы работаете в течение дня в течение 8 часов, так что ваш сеанс Debug никогда не истекает. Таким образом, ваш сбор за день будет:

**8 (часы) x 8 (оптимизированные ядра) x $0.193 и $12.35**

## <a name="transform-data-in-blob-store-with-mapping-data-flows"></a>Преобразование данных в хранилище каблов с помощью картографических потоков данных

В этом сценарии необходимо визуально преобразовать данные в Blob Store в потоки данных ADF в почасовом графике.

Для выполнения сценария необходимо создать конвейер со следующими элементами.

1. Активность потока данных с логикой преобразования.

2. Набор входных данных для данных в хранилище Azure.
...
| Получение конвейера | 1 Чтение и запись сущности |
| Выполнение конвейера | 2 Выполнение действий (1 для запуска триггера, 1 — для выполнения действий) |
| Предположения потока данных: время выполнения 10 мин | 10 \* 16 ядер General Compute с TTL из 10 |
| Допущение отслеживания конвейера: произошло только 1 выполнение | 2 Повторное отслеживание записей о выполнении (1 для выполнения конвейера, 1 для запуска действия) |

**Всего цена сценария: $1.4631**

- Операции Фабрики данных = **0,0001 долл. США**
  - Чтение и запись = 10\*00001 = 0,0001 долл. США [1 R/W = 0,50/50 000 долл. США = 0.00001]
  - Отслеживание  = 2\*000005 = 0,00001 долл. США [1 Отслеживание = 0,25 долл. США/50 000 = 0,000005]
- Выполнение трубопроводной оркестровки &amp; - **$1.463**
  - Выполнение действий = 001\*2 = 0,002 [1 выполнение = 1 долл. США/1000 = 0,001]
  - Действия потока данных - $1.461 произрастали в течение 20 минут (10 минут времени исполнения и 10 минут TTL). $0.274/час на Azure Интеграция Runtime с 16 ядрами общего вычисления

## <a name="next-steps"></a>Дальнейшие действия

Теперь, когда вы ознакомились с ценами на Фабрику данных Azure, можно начинать работу.

- [Создание фабрики данных с помощью uI-иного иного иного иного использования Azure Data Factory](quickstart-create-data-factory-portal.md)

- [Знакомство с Фабрикой данных Azure](introduction.md)

- [Визуальное авторство на фабрике данных Azure](author-visually.md)
