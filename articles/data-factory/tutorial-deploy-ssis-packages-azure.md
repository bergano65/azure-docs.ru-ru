...
Чтобы создать ваш ADF-файл с помощью портала Azure, выполните пошаговые инструкции в статье [Создание ADF-файла с помощью пользовательского интерфейса](https://docs.microsoft.com/azure/data-factory/quickstart-create-data-factory-portal#create-a-data-factory) и выберите **Закрепить на панели мониторинга**, чтобы разрешить быстрый доступ после его создания. 

После создания ADF-файла откройте страницу обзора на портале Azure и щелкните плитку**Author & Monitor** (Автор и Отслеживание), чтобы открыть страницу **Приступить к работе** на отдельной вкладке, где можно продолжить создание Azure-SSIS IR.   
...
1. На странице **Начало работы** щелкните плитку **Configure SSIS Integration Runtime** (Настройка среды выполнения интеграции SSIS). 
...
1. На странице **Дополнительные параметры** сделайте следующее: 
   ...
   c. После установки флажка в поле **Select a VNet...** (Выбрать виртуальную сеть) необходимо определиться, будет ли среда выполнения интеграции присоединена к виртуальной сети. Проверьте, используется ли сервер Базы данных SQL Azure с конечными точками служб для виртуальной сети или Управляемым экземпляром с частной конечной точки с целью размещения SSISDB или требования доступа к локальным данным без настройки локальных IR. Сведения об этом см. в статье [Создание Azure-SSIS IR в виртуальной сети](https://docs.microsoft.com/azure/data-factory/create-azure-ssis-integration-runtime). 

   d. На вкладке **Настройка Локально размещаемой...** Выберите, нужно ли настроить Локально размещаемую IR в качестве прокси-сервера для Azure-SSIS IR. см. раздел [Настройка Локально размещаемой IR в качестве прокси-сервера](https://docs.microsoft.com/azure/data-factory/self-hosted-integration-runtime-proxy-ssis). 

1. Щелкните **Далее**. 

1. На странице **Сводка** проверьте все параметры подготовки, закладку рекомендуемых ссылок на документацию и нажмите кнопку **Готово**, чтобы начать создание среды выполнения интеграции. 
