---
title: Начало работы с хранилищем Azure Blob с помощью Visual Studio (ASP.NET Core)
...
1. В диалоговом окне **Новый проект** выберите **Веб** > **Веб-приложение ASP.NET Core** > **AspNetCoreStorage**. Затем выберите **OK**.
...
1. В диалоговом окне **Создать веб-приложение ASP.NET Core** выберите **.NET Core** > **ASP.NET Core 2.0** > **Веб-приложение (модель-представление-контроллер)**. Затем выберите **OK**.
...
2. Из контекстного меню выберите **Добавленную** > **службу**Connected Service.
...
> [!TIP]
> Сведения о создании учетной записи хранения на [портале Azure](https://portal.azure.com) см. в разделе [Об учетных записях хранения Azure](../storage/common/storage-account-create.md).
>
...
2. Из контекстного меню выберите **Добавить** > **контроллер**.
...
2. Из контекстного меню выберите **Добавить** > **new Folder**. Назовите новую папку *Blobs*. 

1. В **обозревателе решений** разверните папку **Представления** и щелкните правой кнопкой мыши папку **Blobs**.

4. Из контекстного меню выберите **Add** > **View**.
...
1. Получив ссылку на большой двоичный объект, вы можете передать в него любой поток данных, вызвав метод `UploadFromStream` объекта ссылки на большой двоичный объект. Метод `UploadFromStream` создает большой двоичный объект, если он еще не существует, или заменяет его, если он существует. (Изменить * &lt;>файла к* полностью квалифицированной траектории загрузки файла.)
...
2. Из контекстного меню выберите **Add** > **View**.
...
1. Чтобы скачать большой двоичный объект, используйте метод `CloudBlockBlob.DownloadToStream`. Следующий код передает содержимое большого двоичного объекта в объект потока. Затем этот объект сохраняется в локальном файле. (Изменить * &lt;>локального файла* на полностью квалифицированное имя файла, представляющее, где должна быть загружена капля.) 
...
В этом руководстве вы узнали, как сохранять и извлекать большие двоичные объекты, а также выводить их список в службе хранилища Azure с помощью ASP.NET Core. Просмотрите дополнительные руководства, чтобы изучить дополнительные возможности хранения данных в Azure.
  ...
  * [Начало работы с службами хранения данных Azure Table и Visual Studio (ASP.NET)](vs-storage-aspnet-getting-started-tables.md)
  * [Начало работы с службами хранения azure Queue и визуальной студии (ASP.NET)](vs-storage-aspnet-getting-started-queues.md)
