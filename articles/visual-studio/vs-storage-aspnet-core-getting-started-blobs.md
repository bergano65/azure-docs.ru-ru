---
title: Приступая к работе с хранилищем BLOB-объектов Azure с помощью Visual Studio (ASP.NET Core)
...
2. В контекстном меню выберите команду **Добавить** > **подключенную службу**.
...
2. В контекстном меню выберите команду **Добавить** > **новую папку**. Назовите новую папку *Blobs*. 
...
1. Получив ссылку на большой двоичный объект, вы можете передать в него любой поток данных, вызвав метод `UploadFromStream` объекта ссылки на большой двоичный объект. Метод `UploadFromStream` создает большой двоичный объект, если он еще не существует, или заменяет его, если он существует. (Замените * &lt;File-to-upload>* на полный путь к файлу для отправки.)
...
1. Чтобы скачать большой двоичный объект, используйте метод `CloudBlockBlob.DownloadToStream`. Следующий код передает содержимое большого двоичного объекта в объект потока. Затем этот объект сохраняется в локальном файле. (Замените * &lt;Local-File-Name>* полным именем файла, представляющего место для скачивания большого двоичного объекта.) 
...
В этом руководстве вы узнали, как сохранять и извлекать большие двоичные объекты, а также выводить их список в службе хранилища Azure с помощью ASP.NET Core. Просмотрите дополнительные руководства, чтобы изучить дополнительные возможности хранения данных в Azure.
  ...
  * [Приступая к работе с хранилищем таблиц Azure и подключенными службами Visual Studio (ASP.NET)](vs-storage-aspnet-getting-started-tables.md)
  * [Начало работы с хранилищем очередей Azure и подключенными службами Visual Studio (ASP.NET)](vs-storage-aspnet-getting-started-queues.md)
