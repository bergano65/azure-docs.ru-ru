---
title: Начало работы с хранилищем Azure Blob с помощью Visual Studio (ASP.NET)
...
Следующие шаги создают метод для подключения к учетной записи хранилища с помощью строки подключения в **Web.config.** Шаги также создают ссылку на контейнер.  Параметр строки подключения в **Web.config** имеет имя в формате `<storageaccountname>_AzureStorageConnectionString`. 
...
1. Если папки **Blobs** нет, создайте ее. Из контекстного меню выберите **Добавить** > **new Folder**. Назовите новую папку *Blobs*. 
 
1. В **обозревателе решений** разверните папку **Представления** и щелкните правой кнопкой мыши папку **Blobs**.

1. Из контекстного меню выберите **Add** > **View**.
...
1. Чтобы скачать большой двоичный объект, используйте метод `CloudBlockBlob.DownloadToStream`. Следующий код передает содержимое большого двоичного объекта в объект потока. Затем этот объект сохраняется в локальном файле. (Изменить * &lt;>локального файла* на полностью квалифицированное имя файла, представляющее, где должна быть загружена капля.) 
    ...
    В следующем примере показан готовый метод `DownloadBlob` (с абсолютным путем к создаваемому локальному файлу).
