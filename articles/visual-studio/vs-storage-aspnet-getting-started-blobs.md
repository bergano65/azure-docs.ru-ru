---
title: Начало работы с хранилищем BLOB-объектов Azure с помощью Visual Studio (ASP.NET)
...
Ниже описано, как создать метод для подключения к учетной записи хранения с помощью строки подключения в **файле Web. config**. Эти шаги также создают ссылку на контейнер.  Параметр строки подключения в **Web.config** имеет имя в формате `<storageaccountname>_AzureStorageConnectionString`. 
...
1. Если папки **Blobs** нет, создайте ее. В контекстном меню выберите команду **Добавить** > **новую папку**. Назовите новую папку *Blobs*. 
