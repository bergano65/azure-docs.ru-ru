---
title: Начало работы с хранилищем таблиц Azure с помощью Visual Studio (ASP.NET)
...
1. В рамках метода **CreateTable** получите объект **CloudStorageAccount**, который представляет сведения об учетной записи хранения. Используйте следующий код, чтобы получить строку подключения к хранилища и информацию об учетной записи хранилища из конфигурации службы Azure: (Изменить * &lt;имя учетной записи хранилища>* на имя учетной записи хранения Azure, к доступу к вам).
...
1. Выполнить приложение и выбрать **таблицу «Создание»,** чтобы увидеть результаты, аналогичные следующему скриншоту:
...
1. Выполнить приложение и выбрать **сущность Добавить,** чтобы увидеть результаты, аналогичные следующему скриншоту:
...
1. Выполнить приложение и выбрать **объекты** добавления, чтобы увидеть результаты, аналогичные следующему скриншоту:
...
1. Выполнить приложение и выбрать **Get Single,** чтобы увидеть результаты, аналогичные следующему скриншоту:
...
1. Создайте экземпляр объекта **TableQuery**, поместив запрос в предложение **Where**. Используя класс **CustomerEntity** и данные, представленные в разделе [Добавить пакет сущностей к таблице](#add-a-batch-of-entities-to-a-table), следующий фрагмент кода запрашивает таблицу для всех сущностей, где **PartitionKey** (фамилия клиента) имеет значение "Смит":
...
1. Выполнить приложение и выбрать **раздел Get,** чтобы увидеть результаты, аналогичные следующему скриншоту:
...
1. Выполнить приложение и выбрать **объект Delete,** чтобы увидеть результаты, аналогичные следующему скриншоту:
...
Просмотрите дополнительные руководства, чтобы изучить дополнительные возможности хранения данных в Azure.
  ...
  * [Начатые с хранения azure капли и визуальных студийных подключенных услуг (ASP.NET)](../storage/vs-storage-aspnet-getting-started-blobs.md)
  * [Приступая к работе с хранилищем очередей Azure и подключенными службами Visual Studio (ASP.NET)](../storage/vs-storage-aspnet-getting-started-queues.md)
