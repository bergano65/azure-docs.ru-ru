...
Вы можете сохранить ключи SSH как Azure Key Vault секреты и использовать эти секреты для подключения к виртуальным машинам с помощью Azure бастиона. Вы можете управлять доступом пользователей к этим секретам, [назначив политики Key Vault доступа](../key-vault/general/assign-access-policy-portal.md) как для отдельных пользователей, так и для групп Azure AD. Чтобы использовать этот метод для подключения к виртуальной машине, пользователям потребуются следующие разрешения:
- **Получить** доступ к секретам, хранящимся в выбранном Azure Key Vault
- **Перечисление** доступа к секретам, хранящимся в выбранном Azure Key Vault
...
**Руководство**. Azure бастиона интегрируется с Azure Active Directory (Azure AD) для доступа к службе и управления ей. Настройте Azure Active Directory многофакторной идентификации для клиента Azure AD. Azure AD поддерживает строгое управление проверкой подлинности с помощью многофакторной проверки подлинности (MFA) и надежных методов безпарольного ввода.
