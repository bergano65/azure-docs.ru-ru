...
**Вопрос.** При попытке создать NSG в подсети Azure бастиона возникает следующая ошибка: *" <NSG name> группа безопасности сети не имеет необходимых правил для Azure бастиона Subnet азуребастионсубнет"*.
...
Ответ **.** Azure бастиона поддерживает вход в виртуальную машину, присоединенную к домену, для входа в домен на основе пароля пользователя. При указании учетных данных домена в портал Azure используйте формат имени участникаusername@domain-пользователя () вместо формата *домен \ имя_пользователя* для входа. Эта поддержка поддерживается для присоединения к домену или гибридных виртуальных машин (присоединенных к домену, а также к присоединенным к Azure AD). Он не поддерживается для виртуальных машин, присоединенных к Azure AD.
