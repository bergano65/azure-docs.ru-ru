---
title: Данные предварительной версии службы "Аналитика временных рядов Azure". Хранилище данных и входящий трафик в предварительной версии службы "Аналитика временных рядов Azure" | Документация Майкрософт
...
Аналитика временных рядов (предварительная версия) использует хранилище BLOB-объектов Azure с типом файла Parquet. Аналитика временных рядов управляет всеми операциями данных, включая создание больших двоичных объектов, индексирование и секционирование данных в учетной записи хранения Azure. С помощью учетной записи хранения Azure можно создать эти большие двоичные объекты.
...
Логическая секция – это секция в пределах физической, которая хранит все данные, связанные со значением ключа одной секции. В основе логической секции Аналитики временных рядов (предварительная версия) каждого большого двоичного объекта лежит два свойства.
...
Аналитика временных рядов (предварительная версия) предоставляет высокопроизводительные запросы, основанные на этих двух свойствах. Эти два свойства также предоставляют наиболее эффективный метод для быстрой доставки данных Аналитики временных рядов.
...
## <a name="your-azure-storage-account"></a>Ваша учетная запись хранения Azure
...
* из обозревателя Аналитики временных рядов (предварительная версия);
* из API-интерфейсов Аналитики временных рядов (предварительная версия);
* непосредственно из учетной записи хранения Azure.

#### <a name="from-the-time-series-insights-preview-explorer"></a>Из обозреватель Аналитики временных рядов (предварительная версия)

Данные можно экспортировать в CSV-файл из обозревателя Аналитики временных рядов (предварительная версия). Дополнительные сведения см. в статье [Визуализация данных в обозревателе (предварительная версия)](./time-series-insights-update-explorer.md).

#### <a name="from-the-time-series-insights-preview-apis"></a>Из API-интерфейсов Аналитики временных рядов (предварительная версия)

Конечную точку API можно охватить на `/getRecorded`. Дополнительные сведения об этом API см. в статье [Запрашивание данных](./time-series-insights-update-tsq.md).
...
Не удаляйте большие двоичные объекты, поскольку Аналитика временных рядов (предварительная версия) поддерживает метаданные о больших двоичных объектах.

## <a name="ingress"></a>Входящие

### <a name="time-series-insights-ingress-policies"></a>Политики входящего трафика Аналитики временных рядов

Аналитика временных рядов (предварительная версия) поддерживает тот же источник события и типы файлов, которые в настоящее время поддерживает Аналитика временных рядов.
...
Аналитика временных рядов (предварительная версия) индексирует данные с помощью стратегии оптимизации размера большого двоичного объекта. Данные становятся доступными для запроса после индексирования, которое зависит от того, какой объем данных поступает и на какой скорости.
