...
- [Концентратор Azure IoT](../iot-hub/about-iot-hub.md)
- [Концентраторы событий Azure](../event-hubs/event-hubs-about.md)
...
По умолчанию предварительный просмотр Time Series Insights может глотать поступающие данные со скоростью **до 1 мегабайт в секунду (MBps) в среде Time Series Insights.**
...
* **Пример 1.**
    ...
    Contoso Shipping имеет 100 000 устройств, которые излучают событие три раза в минуту. Размер события составляет 200 байтов. Они используют концентратор событий с четырьмя разделами в качестве источника событий Time Series Insights.
...
* **Пример 2:**
    ...
    Contoso Fleet Analytics имеет 60 000 устройств, которые излучают событие каждую секунду. Они используют количество разделов IoT Hub 24, 4, как источник событий Time Series Insights. Размер события составляет 200 байтов.

    * Скорость приема в окружающую среду будет: **20 000 устройств, 200 байтов/событие - 1 событие/сек - 4 Мбит/с.**
    * Коэффициент на перегородку составит 1 Мбит/с.
    * Contoso Fleet Analytics может отправить запрос на портал Time Series Insights через портал Azure для увеличения коэффициента приема в окружающую среду.
