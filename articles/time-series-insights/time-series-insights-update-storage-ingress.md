...
Azure Time Series Insights Preview поддерживает максимум два источника событий в экземпляре. При подключении источника событий среда TSI будет читать все события, которые в настоящее время хранятся в концентраторе Iot или Event, начиная с самого старого события. 
...
| **double** | Двухточная 64-разрядная плавающая точка [IEEE 754.](https://ieeexplore.ieee.org/document/8766229) |
