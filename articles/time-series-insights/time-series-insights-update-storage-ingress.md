...
В обоих случаях свойство time файла Parquet соответствует времени создания BLOB-объекта. Данные в папке `PT=Time` сохраняются без изменений с момента их записи в файл. С течением времени данные в папке `PT=TsId` будут оптимизированы для запросов и не являются статическими.
