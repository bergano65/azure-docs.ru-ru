...
Эти API-интерфейсы доступны в обоих хранилищах (горячий и холодного) в решении для многоуровневой системы хранения данных. 
...
### <a name="selecting-store-type"></a>Выбор типа магазина

Указанные выше API-интерфейсы могут выполняться только для одного из двух типов хранения (холодный или теплый) в одном вызове. Параметры URL-адреса запроса используются для указания [типа хранилища](/rest/api/time-series-insights/dataaccessgen2/query/execute#uri-parameters) , в котором должен выполняться запрос. 

Если параметр не указан, запрос будет выполняться в холодном хранилище по умолчанию. Если запрос охватывает диапазон времени, перекрывающийся как холодный, так и горячий, рекомендуется направить запрос в холодный магазин для обеспечения лучшей работы, так как горячий магазин будет содержать только частичные данные. 
