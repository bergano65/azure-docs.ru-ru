...
* Например, следующий вложенный код JSON:
   ...
   Преобразуется в: `data_flow` при плоском.

> [!IMPORTANT]
> * В предварительной версии службы "аналитика временных рядов Azure `_` " для девстраивания столбцов используются символы подчеркивания ().
> * Обратите внимание на разницу от общей доступности, в которой используются точки ( `.` ).
...
* В примере JSON имеется внешний массив, который использует данные [экземпляра временных рядов](./concepts-model-overview.md#time-series-model-instances) для повышения эффективности сообщения. Несмотря на то, что экземпляры временных рядов не изменяют метаданные устройства, они часто предоставляют полезные свойства для анализа данных.
...
Давайте подробнее рассмотрим, как использовать [экземпляр временных рядов](./concepts-model-overview.md#time-series-model-instances) для более оптимального формирования JSON. 
...
| deviceId  | Type | L1 | L2 | TIMESTAMP | Частота series_Flow ft3/с | series_Engineная давление psi |
| ---- | ---- | ---- | ---- | ---- | ---- | ---- |
...
В приведенном выше примере плоское свойство представит `data["flow"]` конфликт имен со `data_flow` свойством.
