...
ms.locfileid: "72989819"
---
# <a name="time-series-model"></a>Модель временных рядов

В этом документе описывается модель временных рядов, их возможности, а также приводятся инструкции по созданию и обновлению собственной среды в среде "аналитика временных рядов".
...
> * Ознакомьтесь с обзорным [обозревателем "аналитика временных рядов](time-series-insights-update-explorer.md) ", чтобы узнать, как перемещаться по ПОЛЬЗОВАТЕЛЬСКОМу интерфейсу модели временных рядов.

## <a name="summary"></a>Резюме

Обычно данные, собираемые с устройств IoT, не имеют контекстных сведений, что затрудняет поиск и анализ датчиков. Основной мотивацией модели временных рядов является упрощение поиска и анализа данных в IoT и временных рядах. Эта цель достигается за счет включения контроль, обслуживания и обогащения данных временных рядов, чтобы помочь подготовить готовые к отправке наборам данные для анализа.

## <a name="scenario-contosos-new-smart-oven"></a>Сценарий: Новая смарт-печь компании Contoso

**Рассмотрим вымышленный сценарий новой компании Contoso Smart печь.** В этом сценарии предположим, что каждая Интеллектуальная микросхема Contoso имеет пять датчиков температуры, по одному для каждого из четырех ведущих единиц записи и один для самой печь. До последнего времени каждый датчик температуры Contoso отправил, хранит и наглядно выдает данные по отдельности. Для наблюдения за устройством кухни компания Contoso основывалась на базовых диаграммах, по одной на каждый отдельный датчик.
...
* Клиенты хотят узнать, как интенсивно общая печь получится, когда большая часть основных устройств записи находилась, и компания Contoso имела больше сложностей по анализу и представлению единого ответа о условиях общей печь.
* Инженеры компании Contoso хотят убедиться, что самые популярные запущенные в данный момент запускаются одновременно, не нарушают работу Power Draw, и возникли трудности с перекрестным связыванием, какие датчики температуры и напряжения связаны друг с другом, и как их разместить в хранилище.
* Группа специалистов по контролю качества Contoso хотела проводить аудит и сравнение истории между двумя версиями датчиков. при этом было сложно определить, какие данные имеют версию датчика.
...
* Визуализация данных оказывается полезной, если вы можете связать и объединить данные в удобном для просмотра виде. Например, отображение датчиков напряжения и датчиков температуры.
...
**Модель временных рядов предоставляет удобное решение** для многих сценариев, встречающихся в вымышленном примере выше:
...
* Определите свойства, связанные с экземплярами, определенные как *поля экземпляра* , и используйте их для создания иерархий.

### <a name="components"></a>Компоненты

Модель временных рядов имеет три основных компонента:
...
В большинстве случаев экземпляры уникально идентифицируются **deviceId** или **assetId**, они сохраняются как идентификаторы временных рядов.

Экземпляры имеют описательные сведения, связанные с ними, называемые такими *свойствами экземпляра* , как идентификатор временных рядов, тип, имя, описание, иерархии и поля экземпляров. Свойства экземпляр содержат, как минимум, сведения об иерархии.

Поля экземпляра — это набор описательных сведений, которые могут включать значения для уровней иерархии, а также для производителей, операторов и т. д.

После настройки источника событий для среды "аналитика временных рядов" экземпляры автоматически обнаруживаются и создаются в модели временных рядов. Экземпляры можно создавать или обновлять с помощью обозревателя "аналитика временных рядов" с помощью моделей временных рядов — запросов.
...
| name | Свойство *Name* является необязательным и учитывает регистр. Если *имя* недоступно, по умолчанию будет использоваться *тимесериесид*. Если указано *имя* , *тимесериесид* будет по-прежнему [доступен.](time-series-insights-update-explorer.md#preview-well) |
| Description (Описание) | Текстовое описание экземпляра. |
| хиерарчидс | Определяет, к каким иерархиям принадлежит экземпляр. |
| инстанцефиелдс | *instanceFields* — это свойства экземпляра и любые статические данные, определяющие этот экземпляр. Они определяют значения свойств иерархии или неиерархические свойства и в то же время обеспечивают индексацию для выполнения операций поиска. |

> [!NOTE]
> Иерархии строятся с помощью **полей экземпляров**, а дополнительные **инстанцефиелдс** можно добавить для дальнейшего определения свойства экземпляра.
...
> [!TIP]
> API экземпляра службы "аналитика временных рядов" и поддержка CRUD см. в статье о [запросах данных](time-series-insights-update-tsq.md#time-series-model-query-tsm-q-apis) и [документации по API экземпляра](https://docs.microsoft.com/rest/api/time-series-insights/preview-model#instances-api).
...
| Свойство | Описание |
| ---| ---|
| id | Уникальный идентификатор, используемый иерархией, например при определении экземпляра. |
...
> [!TIP]
> API экземпляра службы "аналитика временных рядов" и поддержка CRUD см. в статье о [запросах данных](time-series-insights-update-tsq.md#time-series-model-query-tsm-q-apis) и в [документации по API-интерфейсу иерархии](https://docs.microsoft.com/rest/api/time-series-insights/preview-model#hierarchies-api).

### <a name="hierarchy-example"></a>Пример иерархии

Рассмотрим пример, в котором в иерархии **H1** есть *Сборка*, *этаж*и *комната* в рамках определения **инстанцефиелднамес** :
...
Учитывая **поля экземпляра** , используемые в приведенном выше определении, и несколько временных рядов, атрибуты и значения иерархии будут отображаться, как показано в следующей таблице.
...
Временные ряды **id1** и **ID4** будут отображаться как часть иерархии **H1** в обозревателе службы " [аналитика временных рядов Azure](time-series-insights-update-explorer.md) ", так как они полностью определены и правильно упорядочены в *зданиях*, *этажах*и *комнатах* . Вход.

Другие будут классифицироваться в *неродительских экземплярах* , так как они не соответствуют указанной иерархии данных.
...
> [!TIP]
> Для API экземпляра службы "аналитика временных рядов" и поддержки CRUD обратитесь к статье о [запросах данных](time-series-insights-update-tsq.md#time-series-model-query-tsm-q-apis) и [документации по API типа](https://docs.microsoft.com/rest/api/time-series-insights/preview-model#types-api).
...
| Description (Описание) | Строковое описание типа. |
| variables | Укажите переменные, связанные с типом. |

Типы будут соответствовать следующему примеру JSON:
...
В таблице ниже отображаются свойства, относящиеся к каждому виду переменной.
...
| Интерполяция переменных | Интерполяция указывает, как воссоздать сигнал с помощью существующих данных. Параметр *пошаговой* интерполяции доступен для переменных категории. |
| Категории переменных | Категории создают сопоставление между значениями, поступающими от устройства или датчиков, к метке. |
