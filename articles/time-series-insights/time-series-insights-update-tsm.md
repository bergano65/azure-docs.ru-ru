...
[Общие сведения о модели серии @no__t 1Time](media/v2-update-tsm/tsm.png)](media/v2-update-tsm/tsm.png#lightbox)
...
Модели временных рядов имеют три основных компонента:

* <a href="#time-series-model-types">Типы моделей временных рядов</a>
* <a href="#time-series-model-hierarchies">Иерархии моделей временных рядов</a>
* <a href="#time-series-model-instances">Экземпляры модели временных рядов</a>
...
[Таблица определений переменных ![Type](media/v2-update-tsm/table.png)](media/v2-update-tsm/table.png#lightbox)

| Определение | Описание |
| --- | ---|
| Тип переменной |  Поддерживаются *числовые* и *статистические* типы |
...
| Значение переменной | Значения переменных используются при вычислениях. Соответствующее поле для ссылки на интересующую вас точку данных. |
...
В предыдущем примере **id1** и **ID4** отображаются как часть иерархии H1 в обозревателе службы "аналитика временных рядов Azure", а остальные объекты классифицируются по *неродительским экземплярам* , так как они не соответствуют указанной иерархии данных.
...
Свойство *Name* является необязательным и учитывает регистр. Если *name* недоступно, по умолчанию будет использоваться ИД временного ряда. Если *name* предоставлено, ИД временного ряда будет доступен в резервуаре (сетка, которая находится под диаграммами в проводнике).
