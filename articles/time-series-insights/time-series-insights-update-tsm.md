...
* `Location`Определяет иерархию с родительским `states` и `cities`дочерним. У `location` каждого из них `states`может быть несколько, которые, в `cities`свою очередь, могут иметь несколько.
* `ManufactureDate`Определяет иерархию с родительским `year` и `month`дочерним. У `ManufactureDate` каждого из них `years`может быть несколько, которые, в `months`свою очередь, могут иметь несколько.
...
Рассмотрим пример, где в **H1** иерархии H1 `building`находится `floor`, и `room` как часть определения **инстанцефиелднамес** :
