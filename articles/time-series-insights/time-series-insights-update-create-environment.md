...
1. Укажите приведенные ниже параметры на странице **Create Device Simulation solution** (Создание решения для имитации устройств).
    ...
    | Параметр | Действие |
    | --- | --- |
    | **Имя решения** | Введите уникальное значение для новой группы ресурсов. В эту группу помещаются все выбранные ресурсы Azure после их создания. |
    | **Подписка** | Выберите подписку, которая использовалась для создания среды Аналитики временных рядов. |
    | **Регион** | Выберите регион, который использовался для создания среды Аналитики временных рядов. |
    | **Развертывание дополнительных ресурсов Azure** | Оставьте установленным флажок **Центр Интернета вещей**. Имитированные устройства используют Центр Интернета вещей для подключения или потоковой передачи данных. |
 
    Щелкните **Создать решение**. Подождите 10–15 минут, пока решение развертывается.
...
Чтобы начать создание среды Аналитики временных рядов Azure, вам понадобятся имена Центра Интернета вещей, подписки и группы ресурсов.

1. Перейдите к панели мониторинга акселератора решений. Выполните вход, используя ту же учетную запись в подписке Azure. Найдите имитацию устройства, созданную в предыдущем разделе.

1. Выберите симулятор устройства и щелкните **Запуск**. В области акселератора решений для имитации устройств справа выберите параметр **Портал управления Azure**.
...
1. Запишите имена центра Интернета вещей, подписки и группы ресурсов.
    ...
    [![Сведения на панели мониторинга симулятора устройств на портале Azure](media/v2-update-provision/device-eight-portal.png)](media/v2-update-provision/device-eight-portal.png#lightbox)
...
1. В области **Создание среды Аналитики временных рядов** на вкладке **Основы** задайте следующие параметры.
    ...
    | **Местоположение.** | Выберите регион центра обработки данных для среды службы "Аналитика временных рядов Azure" (предварительная версия). Чтобы избежать лишних затрат на передачу данных и ненужных сетевых задержек, лучше всего создать среду службы "Аналитика временных рядов Azure" (предварительная версия) в одном регионе с остальными ресурсами Интернета вещей. |
