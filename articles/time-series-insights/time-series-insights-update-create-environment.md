...
1. По завершении подготовки вы увидите два обновления, которые будут показывать, что состояние развертывания изменилось с **Подготовка** на **Готово**. 
   ...
   [![Подготовка решения для имитации устройств завершена.](media/v2-update-provision/iot-solution-accelerator-ready.png)](media/v2-update-provision/iot-solution-accelerator-ready.png#lightbox)
...
1. На вкладке **Источник события** задайте следующие параметры.
   ...
   | **Название** | Введите уникальное значение для имени источника события. |
...
1. Щелкните **+Новая имитация**. Когда загрузится страница **Simulation setup** (Настройка имитации), введите обязательные параметры.
    ...
    | Параметр | Действие |
    | --- | --- |
    | **Название** | Введите уникальное имя имитатора. |
    | **Описание** | Введите определение. |
    | **Длительность имитации** | Выберите **Бессрочный запуск**. |
    | **Модель устройства** | Щелкните **Добавить тип устройства**. <br />**Name**: Введите **Лифт**. <br />**Количество**. Введите значение **3**. <br /> Оставьте остальные значения по умолчанию |
...
1. Затем выберите вкладку **Переменные**. 
   ...
   Выберите **Добавить переменную** и заполните следующие значения для первой переменной типа лифта. В итоге вы создадите три переменные.
    ...
    | Параметр | Действие |
    | --- | --- |
    | **Название** | Введите **Средняя температура**. |
    | **Вид** | Выберите **Числовой формат** |
    | **Value** | Выбрать из предустановок: Выберите **temperature (Double)** (температура (двойная)). <br /> Примечание. Автоматическое заполнение поля **Значение** может начаться через несколько минут после того, как служба "Аналитика временных рядов Azure" (предварительная версия) начнет получать события.|
    ...
    | Параметр | Действие |
    | --- | --- |
    | **Название** | Введите **срзнач вибрации**. |
    | **Вид** | Выберите **Числовой формат** |
    | **Value** | Выбрать из предустановок: Выберите **вибрации (Double)** . <br /> Примечание. Автоматическое заполнение поля **Значение** может начаться через несколько минут после того, как служба "Аналитика временных рядов Azure" (предварительная версия) начнет получать события.|
    | **Операция агрегирования** | Разверните **Дополнительные параметры**. <br /> Выберите **AVG**. |
    ...
    | Параметр | Действие |
    | --- | --- |
    | **Название** | Введите **нижний предел** |
    | **Вид** | Выберите **Категориальный** |
    | **Value** | Выбрать из предустановок: Выберите **Уровень (Double)** . <br /> Примечание. Автоматическое заполнение поля **Значение** может начаться через несколько минут после того, как служба "Аналитика временных рядов Azure" (предварительная версия) начнет получать события.|
...
1. Щелкните **Сохранить**. Должны отобразиться три созданные переменные.

    [![После добавления типа его следует просмотреть в представлении модели.](media/v2-update-provision/tsi-add-type-and-view.png)](media/v2-update-provision/tsi-add-type-and-view.png#lightbox)
...
1. Выберите вкладку **Иерархии**. Затем щелкните **+ Добавить**.
   ...
   | Параметр | Действие |
   | --- | ---|
   | **Название** | Введите **Иерархия расположений**. |
