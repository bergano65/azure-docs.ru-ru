...
Этот учебник поможет вам создать среду Аналитики временных рядов, заполненной данными из имитированных устройств. Из этого руководства вы узнаете, как выполнять следующие задачи:
...
1. На странице **среды службы "Аналитика временных рядов"** введите значения для обязательных параметров.
   ...
   **Местоположение.** | Выберите регион для центра обработки данных для среды Аналитики временных рядов. Чтобы избежать лишних затрат на передачу данных и ненужных сетевых задержек, лучше всего хранить среду Аналитики временных рядов в одном регионе с остальными ресурсами Интернета вещей.
...
1. Откройте страницу **Обзор** для группы ресурсов, которую вы создали для среды Аналитики временных рядов, а затем выберите нужную среду Аналитики временных рядов:
...
1. На странице среды Аналитики временных рядов выберите **Источники событий** и щелкните **+Добавить**.
...
1. Введите необходимые параметры на странице **Новый источник событий**.
   ...
   **Имя свойства для метки времени** | Укажите `timestamp`.
   ...
   Завершив ввод данных, щелкните **Создать** для добавления источника событий. Вернувшись на страницу **Обзор** для группы ресурсов, вы увидите рядом с ресурсом среды Аналитики временных рядов новый ресурс "Источник событий Аналитики временных рядов".
...
1. Обозреватель Аналитики временных рядов выполнит начальную загрузку, а затем аутентификацию с использованием вашей учетной записи для портала Azure. На начальной странице есть область диаграмм, в которой вы увидите наличие имитированных данных телеметрии в среде Аналитики временных рядов. Чтобы отфильтровать данные по сокращенному периоду, выберите раскрывающийся список в левом верхнем углу. Затем укажите период, включающий время выполнения имитации устройств. Затем щелкните значок с изображением лупы.
...
В этом учебнике описано, как создать несколько действующих служб Azure для поддержки среды Аналитики временных рядов и решения для имитации устройств. Если вы решите прервать и (или) приостановить работу с этой серией руководств, не забудьте удалить все эти ресурсы, чтобы избежать лишних затрат.

В меню слева на портале Azure сделайте следующее:

1. Щелкните значок **Группы ресурсов** и выберите группу ресурсов, которую вы создали для среды Аналитики временных рядов. В верхней части страницы щелкните **Удалить группу ресурсов**, введите имя группы ресурсов и щелкните **Удалить**.
