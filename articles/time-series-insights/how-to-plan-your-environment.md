...
> [!NOTE]
> Дополнительные сведения о запросах к горячим данным см. в [справочнике по API](/rest/api/time-series-insights/dataaccessgen2/query/execute#uri-parameters).
...
Можно выбрать до трех ключей, чтобы уникальным образом отличать ресурсы. Дополнительные сведения см. в статье рекомендации [по выбору идентификатора временного ряда](./how-to-select-tsid.md) и [правил приема](concepts-json-flattening-escaping-rules.md).
...
Модель является динамичной, поэтому ее можно построить в любое время. Чтобы быстро приступить к работе, создайте и отправьте его перед отправкой данных в службу "аналитика временных рядов Azure". Для создания модели прочтите статью [Использование модели временных рядов](./concepts-model-overview.md).
