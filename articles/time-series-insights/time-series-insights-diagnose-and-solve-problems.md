---
title: Диагностика, устранение неполадок и устранение проблем с помощью службы "аналитика временных рядов Azure" | Документация Майкрософт
...
Среда службы "Аналитика временных рядов Azure" поддерживает данные только в формате JSON. Примеры JSON см. в статье [поддерживаемые фигуры JSON](./how-to-shape-query-json.md).
...
* Для Центра Интернета вещей в Центре Интернета вещей Azure необходимо указать ключ с разрешениями на **подключение службы**. Выберите политики **iothubowner** или **службы** , так как оба имеют разрешения на **Подключение к службе** .
...
Ограничение регулирования применяется на основе типа номера SKU и емкости среды. Все источники событий в среде совместно используют емкость. Если источник событий для центра Интернета вещей или концентратора событий отправляет данные сверх установленных ограничений, вы будете регулировать регулирование и задержку.
...
Чтобы получить общее представление о принципах работы логики обработки, прочитайте [поддерживаемые фигуры JSON](./how-to-shape-query-json.md).
