...
> * Узнайте, [как работать с моделью временных рядов](/azure/time-series-insights/how-to-edit-your-model) с помощью обозревателя TSI "аналитика временных рядов Azure".
...
Модель временных рядов можно создать и управлять с помощью [ОБОЗРЕВАТЕЛЯ TSI временных рядов Azure](/azure/time-series-insights/concepts-model-overview). Параметрами модели временных рядов можно управлять через [API параметров модели](https://docs.microsoft.com/rest/api/time-series-insights/reference-model-apis).
...
После настройки источника событий для среды Gen2 "аналитика временных рядов Azure" экземпляры автоматически обнаруживаются и создаются в модели временных рядов. Экземпляры можно создать или обновить с помощью обозревателя TSI временных рядов Azure, используя запросы модели временных рядов.
...
Временные ряды **id1** и **ID4** отображаются как часть ИЕРАРХИИ **H1** в [обозревателе TSI "аналитика временных рядов Azure](time-series-insights-update-explorer.md) ", так как они полностью определены и правильно упорядочивают параметры *сборки*, *этажа*и *комнаты* .
