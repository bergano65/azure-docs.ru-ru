...
Данные временных рядов — это сведения об изменении состояния ресурса или процесса с течением времени. Данные временных рядов индексируется по меткам времени, и время — это самая важная ось, вдоль которой упорядочены такие данные. Обычно данные временных рядов поступают в строгой очередности, поэтому обычно их добавление в базу данных рассматривается скорее как операция вставки, а не обновления.
