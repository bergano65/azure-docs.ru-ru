...
> [!TIP]
> Если источником событий является центр Интернета вещей, идентификатор временных рядов, скорее всего, будет иметь значение ***iothub-Connection-Device-ID** _.

Ниже приведены основные рекомендации.

_ Выберите ключ секции с множеством различных значений (например, сотни или тысячи). Во многих случаях это может быть идентификатор устройства, идентификатор датчика или идентификатор тега в JSON.
