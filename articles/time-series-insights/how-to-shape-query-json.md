...
Подумайте о том, как вы отправляете события в службу "аналитика временных рядов Azure". А именно, всегда:
...
1. Убедитесь, что вы не достигли максимального ограничения для свойств "аналитика временных рядов Azure":
...
1. Не применяйте глубокую вложенность массивов. Служба "аналитика временных рядов Azure" поддерживает до двух уровней вложенных массивов, содержащих объекты. Служба "аналитика временных рядов Azure" преобразует массивы в сообщениях в несколько событий с парами значений свойств.
...
Рассмотрим следующие полезные данные JSON, которые отправляются в среду с общедоступной версией службы "аналитика временных рядов Azure" с помощью [объекта сообщения устройства IOT](https://docs.microsoft.com/dotnet/api/microsoft.azure.devices.message?view=azure-dotnet) , который сериализуется в JSON при отправке в облако Azure:
...
> - Эталонные данные используются для сокращения количества байтов, передаваемых по сети. Два атрибута **MessageId** и **девицелокатион** соединяются с помощью свойства ключа **deviceId**. Эти данные соединяются с данными телеметрии во время приема, а затем сохраняются в службе "аналитика временных рядов Azure" для запросов.
> - Используется два уровня вложенности — максимальный объем вложенности, поддерживаемый службой "аналитика временных рядов Azure". Критически важно избегать глубоко вложенных массивов.
...
- Дополнительные сведения о синтаксисе запросов для доступа REST API к данным службы "аналитика временных рядов Azure" см. в статье [синтаксис запросов аналитики временных рядов Azure](https://docs.microsoft.com/rest/api/time-series-insights/ga-query-syntax) .
