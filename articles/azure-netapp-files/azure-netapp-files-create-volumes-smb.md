...
* Azure NetApp Filesая Делегированная подсеть должна иметь доступ ко всем контроллерам домена домен Active Directory служб (ADDS) в домене, включая все локальные и удаленные контроллеры домена. В противном случае может произойти прерывание работы службы.  
    ...
    При наличии контроллеров домена, недоступных через Azure NetApp Files делегированную подсеть, можно указать Active Directoryный сайт во время создания подключения Active Directory.  Azure NetApp Files должны обмениваться данными только с контроллерами домена на сайте, где находится Azure NetApp Files адресное пространство делегированной подсети.
