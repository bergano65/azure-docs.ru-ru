...
Эта ошибка отображается, если другой обработке ошибок не удалось обработать ошибку при выполнении действия с ресурсом.   Он содержит текст "ошибка". `{action}` Может быть любым из`getting`(, `creating`, `updating`или `deleting`).  `{resourceTypeName}` — Это `resourceTypeName` ( `netAppAccount` `capacityPool`например,,, и т. д.). `volume`
...
Это сообщение указывает на ошибку в полях `RuleIndex`для, `AllowedClients`, `UnixReadOnly`, `UnixReadWrite` `Nfsv3`, и. `Nfsv4`

* Причина.   
Запрос на проверку входных данных завершился неудачей по крайней мере `RuleIndex`для `AllowedClients`одного `UnixReadOnly`из `UnixReadWrite`следующих `Nfsv`полей:, `Nfsv4`,,, 3 и.
* Решение.   
Убедитесь, что в командной строке заданы все необходимые и неконфликтующие параметры. Например, нельзя одновременно задать оба параметра `UnixReadOnly` и. `UnixReadWrite`
* Обходное решение.   
См. решение выше.

***Диапазон {0} {1} IP-адресов для {2} виртуальной ЛС уже используется***
