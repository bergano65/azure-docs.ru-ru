...
Эта ошибка возникает при попытке обновить или исправить том, а свойство `BaremetalTenantId` имеет измененное значение.

* Причина.   
Вы пытаетесь обновить том, а свойство `BaremetalTenantId` имеет другое значение из значения, хранящегося в Azure.
* Решение.   
Не включайте `BaremetalTenantId` в запрос Patch и Update (размещение). Кроме того, убедитесь, что в запросе `BaremetalTenantId` совпадает.
...
Эта ошибка возникает при попытке обновить или исправить пул ресурсов с измененным свойством `PoolId`.

* Причина.   
Вы пытаетесь обновить `PoolId` свойство пула емкости. Свойство `PoolId` является свойством только для чтения и не может быть изменено.
* Решение.   
Не включайте `PoolId` в запрос Patch и Update (размещение).  Кроме того, убедитесь, что в запросе `PoolId` совпадает.
...
* Решение.    
Нет подходящих вариантов. Имя домена изменить нельзя.
* Обходное решение.   
...
Эта ошибка отображается, если другой обработке ошибок не удалось обработать ошибку при выполнении действия с ресурсом.   Он содержит текст "ошибка". @No__t_0 может быть любой из (`getting`, `creating`, `updating` или `deleting`).  @No__t_0 — это `resourceTypeName` (например, `netAppAccount`, `capacityPool`, `volume` и т. д.).
...
* Причина.   
Идентификатор файловой системы задается при создании тома. `FileSystemId` не может быть изменена в дальнейшем.
* Решение.   
Не включайте `FileSystemId` в запрос Patch и Update (размещение).  Кроме того, убедитесь, что в запросе `FileSystemId` совпадает.

***ActiveDirectory с идентификатором: "{String}" не существует.***

@No__t_0 часть — это значение, введенное в свойстве `ActiveDirectoryId` для соединения Active Directory.
...
***Для {1} получено недопустимое значение "{value}".***
...
* Решение.   
Убедитесь, что в командной строке заданы все необходимые и неконфликтующие параметры. Например, нельзя одновременно задать параметры `UnixReadOnly` и `UnixReadWrite`.
* Обходное решение.   
См. решение выше.

***Диапазон IP-адресов {0} {1} для VLAN {2} уже используется***
...
* Решение.   
Не включайте `MountTargets` в запрос Patch и Update (размещение).  Кроме того, убедитесь, что в запросе `MountTargets` совпадает.
...
* Обходное решение.   
Нет подходящих вариантов. Базовый API необходим для управления томами.
...
* Обходное решение.   
Нет подходящих вариантов. Допускается только один Active Directory.
...
* Причина.   
Свойство `OwnerId` задается при создании тома. Свойство не может быть изменено в дальнейшем.
* Решение.   
Не включайте `OwnerId` в запрос Patch и Update (размещение). Кроме того, убедитесь, что в запросе `OwnerId` совпадает.
...
* Решение.   
Нет подходящих вариантов. Эти ресурсы не имеют свойств, которые могут быть изменены.

***Размер пула слишком мал для общего размера тома.***

Эта ошибка возникает при обновлении размера пула ресурсов, размер которого меньше общего `usedBytes` значение всех томов в пуле емкости.  Эта ошибка также может возникать при создании нового тома или изменении размера существующего тома, а новый размер тома превышает свободное пространство в пуле емкости.
...
Эта ошибка возникает при создании ресурса и заполнении свойства Name другим значением, отличным от свойства Name `resourceId`.
...
* Причина.   
Свойство `ProtocolType` задается при создании тома.  Его невозможно обновить.
* Решение.   
Нет подходящих вариантов.
* Обходное решение.   
...
Эта ошибка возникает при попытке создать ресурс (`NetAppAccount`, `CapacityPool`, `Volume` или `Snapshot`), но квота достигла предела.
...
* Обходное решение.   
Нет подходящих вариантов.  См. решение выше. 

***Невозможно изменить SubnetId.***

Эта ошибка возникает при попытке изменить `subnetId` после создания тома.  `SubnetId` должны быть заданы при создании тома и не могут быть изменены позже.
...
Эта ошибка возникает при попытке создать новый том, но `subnetId` не является `resourceId` для подсети.

* Причина.   
Эта ошибка возникает при попытке создать новый том, но `subnetId` не является `resourceId` для подсети. 
* Решение.   
Проверьте значение `subnetId`, чтобы убедиться в том, что в нем содержится `resourceId` для используемой подсети.
* Обходное решение.   
Нет подходящих вариантов. См. решение выше. 
...
* Решение.   
Выберите другую подсеть, делегированную `Microsoft.NetApp/volumes`.
* Обходное решение.   
...
* Решение.   
Нет подходящих вариантов. Ресурс, который вы пытаетесь обновить, не поддерживает операцию обновления. Поэтому его нельзя изменить.
* Обходное решение.   
