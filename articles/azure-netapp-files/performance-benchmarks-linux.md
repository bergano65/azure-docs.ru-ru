...
Диаграммы сравнивают преимущества `nconnect` с подключенным томом, не подключенным к сети. На графах FIO создал рабочую нагрузку из одного экземпляра D32s_v3 в регионе Azure US-west2.

### <a name="linux-read-throughput"></a>Пропускная способность чтения Linux  

На следующих диаграммах показаны последовательные операции чтения ~ 3 500 MiB/s `nconnect`с, примерно 2.3 x не`nconnect`-.
...
На следующих диаграммах показаны последовательные операции записи. Они указывают, `nconnect` что не имеет заметного преимущества для последовательных операций записи. 1 500. MiB/s — это примерно верхний предел объема последовательного тома для записи и предельный объем исходящих экземпляров D32s_v3.
...
На следующих диаграммах показано случайное считывание ~ 200 000 операций `nconnect`ввода-вывода в секунду`nconnect`с помощью, примерно 3 раза, а не.
...
На следующих диаграммах показана случайная запись ~ 135 000 операций `nconnect`ввода-вывода в секунду`nconnect`с параметром, примерно 3 раза, а не.
