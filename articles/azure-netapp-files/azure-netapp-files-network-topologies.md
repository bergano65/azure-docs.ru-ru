...
* Определяемые пользователем маршруты (определяемые пользователем маршруты) с префиксом адреса в качестве подсети файлов NetApp для Azure
...
* Azure NetApp Files не поддерживается в виртуальной глобальной сети Azure

К Azure NetApp Files относятся следующие ограничения сети.

* Число IP-адресов, используемых в виртуальной сети с Azure NetApp Files (включая одноранговые виртуальных сетей), не может превышать 1000. Мы работаем над тем, чтобы увеличить это ограничение для удовлетворения требований масштабирования клиентов. 
...
|    Подключение к тому в одноранговой виртуальной сети (между регионами или глобальными пиринга)    |    Нет    |    Нет    |
...
Подсети разбивают виртуальную сеть на отдельные адресные пространства, используемые ресурсами Azure.  Azure NetApp Filesные тома содержатся в подсети специального назначения, называемой [делегированной подсетью](https://docs.microsoft.com/azure/virtual-network/virtual-network-manage-subnet). 
...
Рассмотрим виртуальную сеть 2 и виртуальную сеть 3 на приведенной выше схеме. Если VM 1 требуется подключиться к виртуальной машине 2 или 2, или если VM 2 требуется подключиться к виртуальной машине 1 или 1, необходимо включить пиринг виртуальных сетей между VNet 2 и VNet 3. 
...
* В описанной выше архитектуре, если шлюз в периферийной виртуальной сети также существует, подключение к тому использовании из локального подключения через шлюз в концентраторе будет потеряно. По своей структуре предпочтение будет предоставлено шлюзу в периферийной виртуальной сети, так что только компьютеры, подключающиеся по этому шлюзу, могут подключаться к тому использовании.
