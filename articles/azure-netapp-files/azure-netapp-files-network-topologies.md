...
* Файлы Azure NetApp не поддерживаются виртуальным WAN Azure

Следующие сетевые ограничения распространяются на файлы NetApp Azure:

* Количество ис-чатов, использоваемых в VNet с файлами NetApp Azure (включая заглянувvvvvvvские VNets), не может превышать 1000. Мы работаем над увеличением этого лимита для удовлетворения потребностей клиентов в масштабах. 
...
|    Связь с объемом в загляну vNet (Перекрестный регион или глобальное пиринг)    |    нет    |    Отсутствуют    |
...
* В вышеупомянутой архитектуре, если в спицком VNET также есть шлюз, подключение к объему ANF с момента подключения к шлюзу в концентраторе будет потеряно. По замыслу, предпочтение будет отдаваться шлюзу в спицком VNet, и поэтому только машины, соединяющие сяпок над этим шлюзом, могут подключиться к объему ANF.
