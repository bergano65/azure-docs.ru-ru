...
For example, contoso.com and www\.contoso.com can point to the same Traffic Manager profile. Это происходит до тех пор, пока профиль диспетчера трафика будет содержать только настроенные внешние конечные точки.
...
|Name  |Операционная система  |Location  |Уровень цен  |
|---------|---------|---------|---------|
...
3. Запишите адрес из раздела **URL-адрес**, а затем в разделе **Исходящие IP-адреса** просмотрите первый IP-адрес в списке. You'll use this information later when you configure your Traffic Manager end points.
...
4. Чтобы выполнить настройку конечной точки, воспользуйтесь сведениями из следующей таблицы:
   ...
   |Тип  |Name  |Выбор пути миграции  |Location  |Параметры пользовательского заголовка|
   |---------|---------|---------|---------|---------|
...
4. Under **Custom domain**, type your custom domain name. Например, contoso.com.
5. Выберите **Проверка**.
...
6. Чтобы просмотреть новые имена узла в разделе **Назначенные сайту имена узлов**, обновите страницу браузера. The refresh on the page doesn't always show changes right away.
