...
* `Microsoft.Network/privateDNSZones/read` предоставляет разрешение на чтение частных зон DNS, но не их изменение, позволяя увидеть зону, в которой создается запись CNAME.
