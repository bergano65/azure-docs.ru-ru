...
description: В этой статье узнайте, как использовать Azure DNS вместе, чтобы обеспечить DNS для ресурсов Azure.
services: dns
author: rohinkoul
ms.service: dns
ms.topic: article
...
Azure DNS предоставляет DNS для личного домена любому из ресурсов Azure, который поддерживает личные домены или имеет полное доменное имя (FQDN). Например, у вас есть веб-приложение Azure, и вы хотите,\.чтобы ваши пользователи получили к нему доступ, используя contoso.com, либо contoso.com в качестве F-DN. В этой статье описывается настройка службы Azure с Azure DNS для использования личных доменов.
...
Для настройки пользовательского домена для служб, которые используют общедоступный ресурс IP-адреса, такие как application Gateway, Load Balancer, Cloud Service, Ресурсменеджер VMs, и, Классический VMs, используется запись.

Перейдите на**IP-адрес** **Network** > Public, выберите общественный IP-ресурс и нажмите **«Конфигурация».** Запишите показанный IP-адрес.
...
|Тип     | Объект        | Используется запись А, так как ресурсом является IP-адрес.        |
|Срок жизни     | 1        | 1 используется для обозначения 1 часа.        |
|Единица срока жизни     | Часы        | В качестве единицы измерения времени используется час.         |
...
Перейдите к**учетным записям** **хранения,** > выберите учетную запись хранения и нажмите **пользовательский домен.** Запишите полное доменное имя из шага 2. Это значение используется для создания первой записи CNAME.
...
Перейдите назад к учетной записи хранилища, нажав**на учетные записи** **хранения,** > выберите учетную запись хранения и нажмите **пользовательский домен.** В текстовом поле введите созданный псевдоним без префикса asverify, установите флажок **Использовать непрямую проверку CNAME**, а затем нажмите кнопку **Сохранить**. После завершения этого шага вернитесь в зону DNS и создайте запись CNAME без префикса asverify.  После этого можно безопасно удалить запись CNAME с префиксом cdnverify.
...
Перейдите к **сети** > **CDN профилей,** выберите профиль CDN.
...
Перейдите назад к своей цели CDN, нажав **на профили Networking** > **CDN**и выберите профиль CDN. Щелкните **+ Личный домен**, введите свой псевдоним записи CNAME без префикса cdnverify и щелкните **Добавить**.
