...
## <a name="scenario-split-horizon-functionality"></a>Сценарий. Функциональные возможности горизонта
...
Теперь, когда интернет-клиент отправит запрос DNS для поиска VNETA-VM1.contoso.com, служба Azure вернет запись общедоступного IP-адреса из общедоступной зоны. Если один и тот же запрос DNS выдается из другой виртуальной Машины (например: VNETA-VM2) в той же виртуальной сети A, Azure вернет запись частного IP-адреса из частной зоны. 
...
Дополнительные сведения о зонах и записях DNS, посетив страницу: [DNS обзоре зон и записей](dns-zones-records.md).
