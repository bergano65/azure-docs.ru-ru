...
* Виртуальная сеть должна быть пустой (то есть записей виртуальной машины нет) при изначальной (то есть в первый раз) связи с закрытый зоной в качестве виртуальной сети для регистрации или разрешения. Однако виртуальная сеть не обязательно должна быть пустой при последующем связывании с другими частными зонами в качестве виртуальной сети регистрации или разрешения.
* Условная пересылка (например, для поддержки разрешения между Azure и локальными сетями) сейчас не поддерживается. Сведения о реализации этого сценария через другие механизмы см. в статье [Разрешение имен ресурсов в виртуальных сетях Azure](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md).
