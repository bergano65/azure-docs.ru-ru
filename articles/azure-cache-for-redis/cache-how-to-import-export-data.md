...
> [!NOTE]
> Перед началом операции импорта убедитесь, что RDB-файл или RDB-файлы базы данных Redis переданы в страничные или блочные BLOB-объекты в службе хранилища Azure, размещенные в том же регионе и подписке, что и ваш экземпляр кэша Azure для Redis. Дополнительные сведения см. в статье [Приступая к работе с хранилищем BLOB-объектов Azure с помощью .NET](../storage/blobs/storage-quickstart-blobs-dotnet.md). Если вы экспортировали RDB-файл с помощью функции [экспорта кэша Azure для Redis](#export), то этот файл уже находится в страничном BLOB-объекте и готов к импорту.
>
>
...
5. Щелкните **Импорт** для начала процесса импорта.
   ...
   >
    ...
    Ход выполнения операции импорта можно отслеживать, выбирая уведомления на портале Azure или просматривая события в [журнале аудита](../azure-resource-manager/management/view-activity-logs.md).
...
1. Чтобы экспортировать текущее содержимое кэша в хранилище, [перейдите к кэшу](cache-configure.md#configure-azure-cache-for-redis-settings) на портале Azure и нажмите кнопку **Экспорт данных** в **меню ресурсов**.
    ...
    ![В области навигации для contoso5premium параметр Экспорт данных в списке Администрирование выделяется.](./media/cache-how-to-import-export-data/cache-export-data-choose-storage-container.png)
2. Щелкните **Выберите контейнер хранилища** и выберите требуемую учетную запись хранения. Эта учетная запись хранения должна относиться к той же подписке и тому же региону, что ваш кэш.
   ...
   >
    ...
    ![Учетная запись хранения](./media/cache-how-to-import-export-data/cache-export-data-choose-account.png)
3. Выберите нужный контейнер BLOB-объектов и нажмите кнопку **Выбрать**. Чтобы использовать новый контейнер, щелкните **Добавить контейнер** и выберите его из списка.
    ...
    ![В контейнерах для contoso55 Параметр + Container выделяется. В списке есть один контейнер, качесавес, и он выделен и выделен. Параметр выбора выбран и выделен.](./media/cache-how-to-import-export-data/cache-export-data-container.png)
