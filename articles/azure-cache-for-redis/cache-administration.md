...
* **И Master, и реплика** . при перезагрузке обоих узлов кэша все данные теряются в кэше, и подключения к кэшу будут завершаться сбоем, пока основной узел не вернется в режим «в сети». Если вы настроили [сохраняемость данных](cache-how-to-premium-persistence.md), то последняя резервная копия восстанавливается, когда кэш снова становится доступным. Но все записи в кэше, выполненные после последней архивации, будут утеряны.
...
Чтобы протестировать приложение на устойчивость в случае сбоя основного узла кэша, перезагрузите **главный** узел. Чтобы проверить устойчивость приложения к сбою вторичного узла, перезагрузите узел **реплики** . Чтобы протестировать приложение на устойчивость в случае общего сбоя кэша, перезагрузите **оба** узла.
