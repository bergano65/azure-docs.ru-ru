...
* [Какое предложение и размер кэша Redis для Azure мне следует использовать?](#what-azure-cache-for-redis-offering-and-size-should-i-use)
...
### <a name="what-is-azure-cache-for-redis"></a>Обзор кэша Azure для Redis
...
| Уровень цен | Размер | Ядра ЦП | Доступная пропускная способность | Размер значения 1 КБ | Размер значения 1 КБ |
| --- | --- | --- | --- | --- | --- |
...
| C0 | 250 МБ | Общая | 100 / 12,5  |  15 000 |   7500 |
...
> [!NOTE]
> Программы командной строки Redis не работают с SSL-портом, но вы можете использовать служебную программу, например `stunnel`, для безопасного подключения средств к порту SSL, следуя указаниям в статье [Использование программы командной строки Redis с кэшем Azure для Redis](https://docs.microsoft.com/azure/azure-cache-for-redis/cache-how-to-redis-cli-tool) .
>
>
...
* Можно также указать параметр минимальной настройки потоков с помощью [параметра конфигурации *MinIoThreads* или *минворкерсреадс* ](https://msdn.microsoft.com/library/vstudio/7w2sway1(v=vs.100).aspx) в элементе конфигурации `<processModel>` в `Machine.config`, обычно расположенном по адресу `%SystemRoot%\Microsoft.NET\Framework\[versionNumber]\CONFIG\`. **Установка минимального числа потоков таким способом обычно не рекомендуется, так как это параметр на уровне системы.**

  > [!NOTE]
  > В этом элементе конфигурации задается параметр *per-core*. Например, если вы используете 4-ядерный компьютер и хотите, чтобы параметр *minIoThreads* был 200 во время выполнения, используйте `<processModel minIoThreads="50"/>`.
  >
...
### <a name="which-azure-cache-offering-is-right-for-me"></a>Какой кэш Azure подходит мне лучше всего?
> [!IMPORTANT]
...
### <a name="in-role-cache"></a>Кэш роли
