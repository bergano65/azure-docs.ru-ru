...
| C0 | 250 МБ | Общий доступ | 100 / 12,5  |  15 000 |   7500 |
...
* Можно также указать параметр минимальной `<processModel>` настройки потоков с помощью [параметра конфигурации *minIoThreads* или *минворкерсреадс* ](https://msdn.microsoft.com/library/vstudio/7w2sway1(v=vs.100).aspx) в элементе Configuration в `Machine.config`, обычно расположенном по адресу `%SystemRoot%\Microsoft.NET\Framework\[versionNumber]\CONFIG\` . . **Установка минимального числа потоков таким способом обычно не рекомендуется, так как это параметр на уровне системы.**
