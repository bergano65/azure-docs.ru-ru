...
description: Изучите ответы на общие вопросы, шаблоны и рекомендации для Azure Cache для Redis
author: yegu-ms
ms.author: yegu
ms.service: cache
ms.topic: conceptual
...
* [Что такое Azure Cache для Redis?](#what-is-azure-cache-for-redis)
...
Вопросы, обсуждаемые в этом разделе, посвящены общим проблемам мониторинга и устранения неполадок. Для получения дополнительной информации о мониторинге и устранении неполадок в экземплярах Azure для экземпляров Redis [см.](cache-how-to-monitor.md)
...
* **Память.** Уровни Базовый и Стандартный предлагают от 250 МБ до 53 ГБ. Уровень Premium предлагает до 1,2 ТБ (как кластер) или 120 ГБ (не кластеризированный). Для получения дополнительной [Azure Cache for Redis Pricing](https://azure.microsoft.com/pricing/details/cache/)информации см.
* **Производительность сети**: если у вас есть рабочая нагрузка, которая требует высокой пропускной способности, то уровень "Премиум" предоставит большую пропускную способность по сравнению с уровнями "Стандартный" или "Базовый". Также в рамках каждого уровня пропускная способность для кэшей большего размера выше из-за виртуальной машины, на которой размещен кэш. Для получения дополнительной информации смотрите [следующую таблицу](#cache-performance).
...
* **Выделенные ядра для Redis Server:** В уровне Premium все размеры кэша имеют выделенное ядро для Redis. На уровнях "Базовый" и "Стандартный" отдельное ядро выделяется для размера C1 и выше.
...
* Пропускная способность кэшей равного размера для уровня "Премиум" выше, чем для уровня "Стандартный". Например, с 6 ГБ кэша пропускная стоимость P1 составляет 180 000 запросов в секунду (RPS) по сравнению с 100 000 RPS для C3.
* С кластеризацией Redis пропускная способность линейно увеличивается по мере увеличения числа сегментов (узлов) в кластере. Например, если вы создаете кластер P4 из 10 осколков, то доступная пропускная информация составляет 400 000 и 10 и 4 миллиона RPS.
...
- [Облако Azure China 21Vianet - Azure Cache для Redis](https://www.azure.cn/home/features/redis-cache/)
- [Microsoft Azure для Германии](https://azure.microsoft.com/overview/clouds/germany/)

Подробнее об использовании Azure Cache для Redis с PowerShell в облаке правительства Azure, облаком Azure China 21Vianet и Microsoft Azure Germany можно [просмотреть, как подключиться к другим облакам - Azure Cache для Redis PowerShell.](cache-how-to-manage-redis-cache-powershell.md#how-to-connect-to-other-clouds)
...
Кэш Microsoft Azure для Redis основан на популярном продукте с открытым кодом — кэше Azure для Redis. К нему могут ознакомиться самые разнообразные [клиенты Redis](https://redis.io/clients) для многих языков программирования. Каждый клиент имеет собственный API, который вызывает экземпляр кэша Azure для Redis с помощью [команд Redis](https://redis.io/commands).
...
* Лучше всего Redis работает с меньшими значениями, поэтому рассмотрите возможность разделения больших данных на несколько ключей. В [этом обсуждении Redis,](https://groups.google.com/forum/#!searchin/redis-db/size/redis-db/n7aa2A4DZDs/3OeEPHSQBAAJ)100 kb учтено большим. Прочитайте [эту статью](https://gist.github.com/JonCole/db0e90bedeb3fc4823c2#large-requestresponse-size) , чтобы ознакомиться с примером проблемы, вызванной большими значениями.
* Настройте [параметры ThreadPool](#important-details-about-threadpool-growth) , чтобы избежать тайм-аутов.
* Используйте по крайней мере значение connectTimeout по умолчанию, равное 5 секундам. Этот интервал дает StackExchange.Redis достаточно времени, чтобы восстановить соединение в случае сетевого всплеска.
...
* Избегайте использования определенных команд Redis, которые занимают много времени, если вы не полностью понимаете влияние этих команд. Например, не запускайте команду [KEYS](https://redis.io/commands/keys) в производстве. В зависимости от количества ключей, это может занять много времени, чтобы вернуться. Redis является однопотоковым сервером и обрабатывает команды по одной. Если вы выдадите после KEYS другие команды, они не будут обрабатываться, пока Redis обрабатывает команду KEYS. [Сайт redis.io](https://redis.io/commands/) содержит сведения о временной сложности каждой поддерживаемой операции. Щелкните каждую команду, чтобы просмотреть сведения о сложности операции.
* Размеры ключей — следует использовать пары "ключ-значение" небольшого или большого размера? Это зависит от сценария развертывания. Если сценарий требует больших ключей, можно настроить значение ConnectionTimeout, затем повторить значение и настроить логику повтора. С точки зрения сервера Redis меньшие значения дают более высокую производительность.
...
* [Включите диагностику кэша](cache-how-to-monitor.md#enable-cache-diagnostics), чтобы можно было [наблюдать](cache-how-to-monitor.md) за работоспособностью кэша. Вы можете просматривать метрики на портале Azure, а также [загружать и просматривать](https://github.com/rustd/RedisSamples/tree/master/CustomMonitoring) их с помощью инструментов по вашему выбору.
...
CLR ThreadPool имеет два типа потоков - "Рабочий" и "I/O Completion Port" (IOCP) потоков.
...
* Потоки IOCP используются при асинхронном исмообовенном использовавании, например, при чтении из сети.
...
Когда количество существующих (занятых) потоков достигает "минимального" количества потоков, ThreadPool изменяет частоту внедрения новых потоков до величины в один поток за 500 миллисекунд. Как правило, если ваша система получает взрыв работы, нуждающейся в потоке IOCP, она будет обрабатывать эту работу быстро. Однако если величина нагрузки больше настроенного параметра "Минимум", при обработке некоторых операций возникнет некоторая задержка, так как ThreadPool будет ожидать возникновения одного из двух событий.

1. Освобождение существующего потока для выполнения работы.
2. Ни один существующий поток не становится бесплатным для 500 мс, поэтому создается новый поток.

В принципе, это означает, что, когда количество занятых потоков больше, чем потоки Min, вы, вероятно, заплатите 500-мс задержку, прежде чем сетевой трафик обрабатывается приложением. Кроме того, важно обратить внимание, что если существующий поток простаивает более 15 секунд (на основе моих наблюдений), он будет удален, и этот цикл наращивания и уменьшения количества потоков может повториться.
...
В предыдущем примере можно увидеть, что для потока IOCP имеется шесть занятых потоков, и система настроена таким образом, чтобы позволить четыре минимальных потока. В этом случае клиент, скорее всего, видел бы две 500-мм задержки, потому что 6 > 4.
...
С учетом этой информации мы настоятельно рекомендуем клиентам устанавливать значение параметра "Минимум" для рабочих потоков и потоков IOCP так, чтобы оно превышало значение по умолчанию. Мы не можем дать универсальное руководство о том, каким должно быть это значение, поскольку правильное значение для одного приложения, скорее всего, будет слишком высоким или низким для другого приложения. Этот параметр также может повлиять на производительность других компонентов сложных приложений, поэтому каждый клиент должен точно настроить этот параметр в соответствии со своими потребностями. Хорошей отправной точкой является 200 или 300, после чего нужно проверить и изменить этот параметр в соответствии с результатом.

Как настроить этот параметр:

* Мы рекомендуем изменить эту настройку программно, используя [метод ThreadPool.SetMinThreads (...)](/dotnet/api/system.threading.threadpool.setminthreads#System_Threading_ThreadPool_SetMinThreads_System_Int32_System_Int32_) в `global.asax.cs`. Пример:
...
```
  ...
  > [!NOTE]
  > Значение, указанное этим методом, является глобальной настройкой, затрагивающей весь AppDomain. Например, если у вас есть 4-ядерная машина и вы хотите установить *minWorkerThreads* и *minIoThreads* до 50 на процессор во время выполнения времени, вы будете использовать **ThreadPool.SetMinThreads (200, 200)**.

* Также можно указать минимальные настройки потоков, используя [ *настройки конфигурации minIoThreads* или *minWorkerThreads* ](https://msdn.microsoft.com/library/vstudio/7w2sway1(v=vs.100).aspx) под `<processModel>` элементом конфигурации в, `Machine.config`обычно расположенном по адресу `%SystemRoot%\Microsoft.NET\Framework\[versionNumber]\CONFIG\`. **Устанавливать количество минимальных потоков таким образом, как правило, не рекомендуется, поскольку это общесистемная настройка.**

  > [!NOTE]
  > В этом элементе конфигурации задается параметр *per-core*. Например, если у вас есть 4-ядерная машина и вы хотите, чтобы настройка *minIoThreads* была 200 во время выполнения, вы бы использовали. `<processModel minIoThreads="50"/>`
  >
...
Простои происходят на клиенте, который вы используете для обращения к Redis. Когда команда отправляется на сервер Redis, она помещается в очередь, и со временем сервер Redis выбирает команду и выполняет его. Однако во время этого процесса клиент может простаивать, и в этом случае на вызывающей стороне возникает исключение. Для получения дополнительной информации о проблемах тайм-аута устранения неполадок [см.](cache-troubleshoot-client.md) [StackExchange.Redis timeout exceptions](cache-troubleshoot-timeouts.md#stackexchangeredis-timeout-exceptions)
...
Azure Кэш для Redis, как правило, доступен в размерах до 120 ГБ и имеет доступность SLA 99,9%. Новый [премиум-уровень](cache-premium-tier-intro.md) предлагает размеры до 1,2 ТБ и поддержку кластеризации, VNET и настойчивости, с 99,9% SLA.
