...
* **Этап 1.** Мы настроим в качестве минимальной версии по умолчанию для протокола TLS значение 1,2 для вновь создаваемых экземпляров кэша. (Используется TLS 1,0.) На этом этапе существующие экземпляры кэша обновляться не будут. При необходимости вы можете [изменить минимальную версию TLS](cache-configure.md#access-ports) на 1,0 или 1,1 для обратной совместимости. Это изменение можно выполнить с помощью портал Azure или других API-интерфейсов управления.
...
| Cloud               | Начальная дата первого этапа | Дата начала фазы 2 |
|---------------------|--------------------|--------------------|
