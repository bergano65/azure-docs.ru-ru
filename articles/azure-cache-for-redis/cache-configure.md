...
Чтобы обновить кэш, щелкните **Обновление,** чтобы изменить уровень ценообразования и [масштабировать](#scale) кэш. Дополнительные сведения о выборе ценовой категории кэша см. в разделе [What Azure Cache for Redis offering and size should I use?](cache-faq.md#what-azure-cache-for-redis-offering-and-size-should-i-use) (Какое предложение и размер кэша Azure для Redis мне следует использовать?)
...
Нажмите **размер кластера,** чтобы изменить размер кластера для запущенного премиум-кэша с включенным кластеризмом.
...
Раздел **Блокировки** позволяет блокировать подписку, группу ресурсов или ресурс, чтобы предотвратить случайное исключение или изменение критически важных ресурсов другимпользователям в вашей организации. Для получения дополнительной информации смотрите [ресурсы блокировки с менеджером ресурсов Azure.](../azure-resource-manager/management/lock-resources.md)
...
>[!NOTE]
>В дополнение к архивации метрик кэша в хранилище можно также [передавать их в концентратор событий или отправлять журналы Azure Monitor.](../azure-monitor/platform/stream-monitoring-data-event-hubs.md)
>
>
...
Для получения дополнительной информации о [https://redis.io/commands](https://redis.io/commands)командах Redis см.
...
> - В кэше Azure для Redis поддерживаются не все команды Redis. Список команд Redis, отключенных в кэше Azure для Redis, см. в предыдущем разделе — [Команды Redis не поддерживаются в кэше Azure для Redis](#redis-commands-not-supported-in-azure-cache-for-redis). Для получения дополнительной информации о [https://redis.io/commands](https://redis.io/commands)командах Redis см.
>
>
