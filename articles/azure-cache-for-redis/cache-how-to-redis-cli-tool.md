...
При использовании кэша Azure для Redis по умолчанию включается только TLS-порт (6380). Программа `redis-cli.exe` командной строки не поддерживает TLS. У вас есть два варианта конфигурации для использования программы.

1. [Включение порта, не являющегося TLS (6379)](cache-configure.md#access-ports) - **Эта конфигурация не рекомендуется** , так как в этой конфигурации ключи доступа отправляются через TCP в виде открытого текста. Это изменение может нарушить доступ к кэшу. Единственным сценарием, где может потребоваться эта конфигурация, является просто доступ к кэшу теста.
