...
| Всего операций |Общее количество команд, обработанных сервером кэша за указанный интервал отчетности. Это значение соответствует значению `total_commands_processed` команды INFO Redis. Если кэш Azure для Redis используется исключительно для Pub/, то `Cache Hits`метрики для, `Cache Misses` `Gets`, или `Sets`не будут отображаться, но будут использоваться `Total Operations` метрики, отражающие использование кэша для операций Pub/reby. |
