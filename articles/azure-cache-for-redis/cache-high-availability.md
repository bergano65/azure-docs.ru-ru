...
| [Георепликация](#geo-replication) | Экземпляры связанного кэша в двух регионах с управляемой пользователем отработкой отказа | 99,9% (уровень Premium, один регион) |-|✔|-|
...
### <a name="enterprise-and-enterprise-flash-tiers"></a>Корпоративные и корпоративные Flash-уровни
...
[Георепликация](cache-how-to-geo-replication.md) — это механизм связывания двух кэшей Azure для экземпляров Redis, обычно охватывающих два региона Azure. В качестве основного связанного кэша выбирается один кэш, а другой — вторичный связанный кэш. Только основной связанный кэш принимает запросы на чтение и запись. Данные, записанные в основной кэш, реплицируются во вторичный связанный кэш. Вторичный связанный кэш может использоваться для обслуживания запросов на чтение. Обмен данными между основным и вторичным экземплярами кэша обеспечивается с помощью TLS.

Георепликация разработана главным образом для аварийного восстановления. Она дает возможность создавать резервные копии данных кэша в другом регионе. По умолчанию приложение записывает данные в основной регион и считывает их из него. При необходимости можно настроить чтение из дополнительного региона. Георепликация не обеспечивает автоматический переход на другой ресурс из-за проблем с добавлением задержки сети между регионами, если остальная часть приложения остается в основном регионе. Необходимо управлять и инициировать отработку отказа, отменяя связь с дополнительным кэшем. Это сделает его новым основным экземпляром.
