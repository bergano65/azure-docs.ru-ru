...
description: Узнайте, как устранять проблемы с потерей данных в кэше Azure для Redis, такие как частичная утрата ключей, истечение срока действия ключа или полная утрата ключей.
author: yegu-ms
ms.author: yegu
ms.service: cache
ms.topic: conceptual
...
| Причина: | Описание |
|---|---|
| [Окончание срока действия ключа](#key-expiration) | Ключи удалены, так как для них заданы истечение времени ожидания. |
...
Кэш Azure для Redis удаляет ключ автоматически, если этому ключу назначено время ожидания, и этот период прошел. Дополнительные сведения об истечении срока действия ключа Redis см. в документации по команде [Expires](https://redis.io/commands/expire) . Значения времени ожидания также можно задать с помощью команд [Set](https://redis.io/commands/set), [сетекс](https://redis.io/commands/setex), [жетсет](https://redis.io/commands/getset)и других ** \*хранилищ** .

Чтобы получить статистику о том, сколько ключей устарело, используйте команду [info](https://redis.io/commands/info) . В `Stats` разделе показано общее число ключей с истекшим сроком действия. В `Keyspace` разделе содержатся дополнительные сведения о числе ключей с истечением времени ожидания и средним значением времени ожидания.
...
Для хранения данных в кэше Azure для Redis требуется пространство памяти. Он очищает ключи для освобождения доступной памяти при необходимости. Если значения **used_memory** или **Used_memory_rss** в команде [info](https://redis.io/commands/info) имеют настроенный параметр **MaxMemory** , то в кэше Azure для Redis начинается удаление ключей из памяти на основе [политики кэша](https://redis.io/topics/lru-cache).
...
Клиенты Redis могут выдать команду [Del](https://redis.io/commands/del) или [хдел](https://redis.io/commands/hdel) , чтобы явно удалить ключи из кэша Azure для Redis. Количество операций удаления можно отвести с помощью команды [info](https://redis.io/commands/info) . Если были вызваны команды **Del** или **хдел** , они будут перечислены в `Commandstats` разделе.
...
Клиенты могут вызвать команду [флушдб](https://redis.io/commands/flushdb) , чтобы удалить все ключи из *одной* базы данных или [флушалл](https://redis.io/commands/flushall) , чтобы удалить все ключи из *всех* баз данных в кэше Redis. Чтобы узнать, были ли ключи очищены, используйте команду [info](https://redis.io/commands/info) . В `Commandstats` разделе показано, вызвана ли команда **flush** :
...
Рекомендуется использовать [сохраняемость данных Redis](https://redis.io/topics/persistence) и [георепликацию](https://docs.microsoft.com/azure/azure-cache-for-redis/cache-how-to-geo-replication) , чтобы улучшить защиту данных от этих сбоев инфраструктуры.
