...
На создание кэша требуется некоторое время. Вы можете отслеживать ход выполнения на странице **Обзор** службы "Кэш Azure для Redis". Когда параметр **Состояние** примет значение **Выполняется**, кэш будет готов к использованию. Создав кэш, конфигурацию виртуальной сети можно просмотреть, щелкнув **Виртуальная сеть** в **меню ресурсов**.
...
| Порты | Direction | Транспортный протокол | Назначение | Локальный IP-адрес | Удаленный IP-адрес |
| --- | --- | --- | --- | --- | --- |
...
| 53 |Исходящее |TCP/UDP |Зависимости Redis в DNS (Интернет/виртуальная сеть) | (Подсеть Redis) | 168.63.129.16 и 169.254.169.254 <sup>2</sup> , а также любой пользовательский DNS-сервер для подсети <sup>3</sup> |
