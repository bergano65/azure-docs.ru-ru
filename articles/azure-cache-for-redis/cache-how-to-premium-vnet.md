---
title: Настройка виртуальной сети - Premium Azure Кэш для Redis
...
Выберите нужную подсеть из списка выпадающих **subnet.**  При желании укажите **Static IP-адрес**. Поле **Static IP-адреса** является необязательным, и если ни один не указан, один выбирается из выбранной подсети.
...
Существует девять требований к исходящим портам. Исходящие запросы в этих диапазонах либо исходят в другие службы, необходимые для функционирования кэша, либо внутренни для подсети Redis для межузловой связи. Для георепликации существуют дополнительные исходящие требования для связи между подсетями первичного и вторичного кэша.
...
| 443 | Исходящие | TCP | Зависимость От Убежища ключей Azure | (Подсеть Redis) | AzureKeyVault <sup>1</sup> |
| 53 |Исходящие |TCP/UDP |Зависимости Redis в DNS (Интернет/виртуальная сеть) | (Подсеть Redis) | 168.63.129.16 и 169.254.169.254 <sup>2</sup> и любой пользовательский DNS-сервер для подсети <sup>3</sup> |
...
| 15000-15999 |Исходящие |TCP |Внутренние коммуникации для Redis и Geo-Replication | (Подсеть Redis) |(Подсеть Redis) (Гео-реплика одноранговой подсети) |
| 6379-6380 |Исходящие |TCP |Внутренний обмен данными для Redis | (Подсеть Redis) |(Подсеть Redis) |

<sup>1</sup> Вы можете использовать тег службы 'AzureKeyVault' с группами сетевой безопасности менеджера ресурсов.

<sup>2</sup> Эти IP-адреса, принадлежащие корпорации Майкрософт, используются для адресов Host VM, который обслуживает Azure DNS.

<sup>3</sup> Не требуется для подсетей без пользовательского DNS-сервера или новых кэшов Redis, которые игнорируют пользовательские DNS.

#### <a name="geo-replication-peer-port-requirements"></a>Требования к порту сверстников георепликации

Если вы используете георепликацию между кэшами в виртуальных сетях Azure, обратите внимание, что рекомендуемая конфигурация заключается в разблокировании портов 15000-15999 для всей подсети как входящих, так и в исходящих направлениях к обоим кэшам, так что все компоненты реплики в подсети могут напрямую общаться друг с другом даже в случае будущего геоотказа.
...
| Порты | Направление | Транспортный протокол | Назначение | Локальный IP-адрес | Удаленный IP-адрес |
| --- | --- | --- | --- | --- | --- |
| 6379, 6380 |Входящий трафик |TCP |Обмен данными между клиентом и Redis, балансировка нагрузки Azure | (Подсеть Redis) | (Подсеть Redis), Виртуальная сеть, балансер загрузки Azure <sup>1</sup> |
...
| 15000-15999 |Входящий трафик |TCP |Сообщение клиента с кластерами Redis, балансировкой нагрузки Azure и георепликацией | (Подсеть Redis) |Виртуальная сеть, балансер загрузки Azure, (подсеть георепликации) |
...
<sup>1</sup> Для авторизации правил NSG можно использовать тег службы «AzureLoadBalancer» (менеджер ресурсов) (или «AZURE_LOADBALANCER» для классического.
...
* Исходящие сетевые подключения к *ocsp.msocsp.com*, *mscrl.microsoft.com* и *crl.microsoft.com*. Это подключение необходимо для поддержки функциональности TLS/SSL.
