 ...
 * **Разрабатывайте систему так, чтобы она могла работать с нестабильной работе подключений** . [из-за исправлений и](https://gist.github.com/JonCole/317fe03805d5802e31cfa37e646e419d#file-azureredis-patchingexplained-md)отработки отказа.

 * **Настройте [параметр MaxMemory-reserved](cache-configure.md#maxmemory-policy-and-maxmemory-reserved) , чтобы повысить скорость реагирования системы** при условиях нехватки памяти.  Этот параметр особенно важен для рабочих нагрузок с интенсивной записью или при хранении больших значений (100 КБ или более) в Redis.  Я бы рекомендовал начать с 10% от размера кэша, а затем увеличивать при наличии загрузок с интенсивной записью. При выборе значения ознакомьтесь с [некоторыми соображениями](cache-how-to-troubleshoot.md#considerations-for-memory-reservations) .

 * **Redis работает наилучшим образом с меньшими значениями**, поэтому рассмотрите возможность разбить данные большего размера на несколько ключей.  В [этом Redisм обсуждении](https://stackoverflow.com/questions/55517224/what-is-the-ideal-value-size-range-for-redis-is-100kb-too-large/)приводятся некоторые соображения, которые следует тщательно рассмотреть.  Прочитайте [эту статью](cache-how-to-troubleshoot.md#large-requestresponse-size) , чтобы ознакомиться с примером проблемы, вызванной большими значениями.
 ...
 * **Избегайте дорогостоящих команд** . Некоторые Redis операции, такие как [команды Keys](https://redis.io/commands/keys), являются *очень* ресурсоемкими, и их следует избегать.  Дополнительные сведения см. в [некоторых вопросах, связанных](cache-how-to-troubleshoot.md#expensive-commands) с дорогостоящими командами.
 ...
 * **Начните с использования `redis-benchmark.exe`**  , чтобы получить возможную пропускную способность и задержку перед написанием собственных тестов производительности.  Redis. документацию по производительности можно [найти здесь](https://redis.io/topics/benchmarks).  Обратите внимание, что Redis-testing не поддерживает SSL, поэтому перед выполнением теста необходимо [включить порт без SSL](cache-configure.md#access-ports) на портале.  [Совместимую с Windows версию Redis-Benchmark. exe можно найти здесь.](https://github.com/MSOpenTech/redis/releases)
...
**Чтобы проверить задержку,** выполните следующие действия. Это позволит проверить запросы GET с помощью полезных данных 1000.
