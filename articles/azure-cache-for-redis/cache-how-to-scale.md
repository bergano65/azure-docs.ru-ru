...
description: Узнайте, как масштабировать кэш Azure для экземпляров Redis с помощью портала Azure и таких инструментов, как Azure PowerShell и Azure CLI.
author: yegu-ms
ms.author: yegu
ms.service: cache
ms.topic: conceptual
...
Кэш Azure можно масштабировать для экземпляров Redis с помощью cmdlet [Set-AzRedisCache](https://docs.microsoft.com/powershell/module/az.rediscache/set-azrediscache) при `Size` `Sku`изменении свойств или `ShardCount` свойств. В следующем примере демонстрируется масштабирование кэша с именем `myCache` в кэш размером 2,5 ГБ. 
...
* Когда **стандартный** кэш масштабируется до другого размера или до кэша **Premium,** одна из реплик закрывается и перезачисляется на новый размер и передаваемые данные, а другая реплика выполняет сбой перед его переобеспечением, подобно процессу, который происходит во время сбоя одного из узлов кэша.
...
Время масштабирования зависит от количества данных в кэше, при этом большие объемы данных занимают больше времени. Масштабирование занимает около 20 минут. Для кластерных кэшов масштабирование занимает около 20 минут на осколок.
