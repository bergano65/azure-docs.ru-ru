...
ms.locfileid: "83196369"
---
# <a name="azure-cache-for-redis"></a>Кэш Redis для Azure

Кэш Azure для Redis обеспечивает хранение данных в памяти с помощью программного обеспечения с открытым исходным кодом [Redis](https://redis.io/). При использовании в качестве кэша Redis повышает производительность и масштабируемость систем, которые в значительной степени зависят от внутренних хранилищ данных. Для повышения производительности часто запрашиваемые данные копируются в быстрое хранилище данных, расположенное ближе к приложению. Благодаря Кэшу Azure для Redis это быстрое хранилище размещается в памяти, а не загружается с диска с помощью базы данных.

Кэш Azure для Redis может использоваться как распределенный кэш данных, хранилище сеансов и брокер сообщений. Производительность приложений повышается за счет использования низкой задержки и высокой пропускной способности модуля Redis.

Кэш Azure для Redis предоставляет доступ к защищенному выделенному кэшу Redis. Он управляется корпорацией Майкрософт и предоставляется для любого приложения в среде Azure и за ее пределами.
