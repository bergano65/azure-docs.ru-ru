---
title: Настройка сохранения данных - Premium Azure Кэш для Redis
...
Persistence записывает данные Redis в учетную запись хранения Azure, которая вам принадлежит и управляет ею. Можно настроить из **нового кэша Azure для** лезвия Redis во время создания кэша и в **меню ресурсов** для существующих кэшей премиум-класса.

> [!NOTE]
> 
> Azure Storage автоматически шифрует данные при их сохранении. Вы можете использовать свои собственные ключи для шифрования. Для получения дополнительной [Customer-managed keys with Azure Key Vault](/azure/storage/common/storage-service-encryption)информации см.
> 
> 
...
Настойчивость Redis включена на лезвии **сохранения данных,** выбирая либо **rdB,** либо **AOF.** Для новых кэшей доступ к этой колонке осуществляется при создании кэша, как описано в предыдущем разделе. Для существующих кэшов доступ к лезвию **сохранения данных** из **меню ресурсов** для кэша.
...
Да, можно изменить частоту резервного копирования для сохранения RDB на лезвии **сохранения данных.** Инструкции см. в разделе "Настройка сохраняемости Redis".
