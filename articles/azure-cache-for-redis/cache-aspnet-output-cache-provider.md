...
| *Порт* | положительное целое число | 6379 (не-TLS/SSL)<br/>6380 (TLS/SSL) | Порт сервера Redis |
...
| *Ssl* | Логическое | **false** | Подключение к серверу Redis через TLS. Это значение **является ложным** по умолчанию, поскольку Redis не поддерживает TLS из коробки. **Если вы используете Кэш Azure Redis, который поддерживает SSL из коробки, не забудьте установить это верно, чтобы улучшить безопасность.**<br/><br/>Порт, не отключаемый TLS, по умолчанию отключен для новых кэшов. Укажите **истинное** для этого параметра для использования порта TLS. Для получения дополнительной информации о включении порта, не являваемого TLS, см. раздел [«Порты доступа»](cache-configure.md#access-ports) в разделе [«Настройка темы кэша».](cache-configure.md) |
