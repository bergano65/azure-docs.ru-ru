...
Azure Dev Spaces также лучше работает при отладке одного экземпляра или Pod. Файл содержит параметр replicaCount, который указывает количество модулей Pod, которое Kubernetes выполняется для вашей службы. `azds.yaml` Если вы измените *replicaCount* , чтобы настроить приложение для запуска нескольких модулей Pod для определенной службы, отладчик подключится к первому модулю, если в списке указано в алфавитном порядке. Отладчик присоединяется к другому модулю pod при перезапуске оригинального модуля pod, что может привести к непредвиденному поведению.
...
При запуске `azds up`может появиться эта ошибка. И, `azds prep` и должны запускаться из корневого каталога проекта, который требуется запустить в пространстве разработки. `azds up`
...
### <a name="error-release-azds-identifier-spacename-servicename-failed-services-servicename-already-exists-or-pull-access-denied-for-servicename-repository-does-not-exist-or-may-require-docker-login"></a>Ошибка "Release аздс-\<идентификатор\>-\<спаценамеServiceName—сбой\>: службы"\<ServiceName-\<\> \>"уже существует" или "опрашивающий доступ запрещен для \<ServiceName\>, репозиторий не существует или может требовать" DOCKER Login "
...
Чтобы устранить эту проблему, обновите `launch.json` файл `.vscode` в подкаталоге папки проекта. Измените каталог `configurations->cwd`, чтобы он указывал на тот же каталог, что и `WORKDIR`, определенный в `Dockerfile` проекта. Возможно, вам также потребуется обновить директиву `configurations->program`.
...
Чтобы устранить эту проблему, используйте учетную запись с правами *владельца* или *участника* в подписке Azure, `Microsoft.DevSpaces` вручную зарегистрируйте пространство имен:
...
1. `azds.yaml` Измените файл, чтобы задать для контекста сборки уровень решения.
2. Измените файлы `Dockerfile.develop` `.csproj`и, чтобы они ссылались на файлы проекта, например, правильно относительные по отношению к новому контексту сборки. `Dockerfile`
3. Добавьте в тот же каталог, где находится `.sln` файл. `.dockerignore`
4. При необходимости обновите с помощью дополнительных записей. `.dockerignore`
