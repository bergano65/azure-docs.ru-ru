...
В настоящее время Azure Dev Spaces лучше всего работает при отладке одного экземпляра (pod). Файл azds.yaml содержит параметр replicaCount, указывающий количество модулей pod, которые будут выполняться для вашей службы. Если вы измените replicaCount, чтобы настроить приложение для запуска нескольких модулей pod для данной службы, отладчик присоединиться к первому модулю pod (при перечислении в алфавитном порядке). Если этот модуль pod по какой-либо причине перезапускается, отладчик присоединиться к другому модулю pod, что может привести к непредвиденному поведению.
...
### <a name="re-running-a-service-after-controller-re-creation"></a>Повторный запуск службы после повторного создания контроллера
Эта ошибка может появиться при попытке перезапустить службу после удаления и повторного создания контроллера Azure Dev Spaces, связанного с этим кластером. Подробные выходные данные будут выглядеть следующим образом:
...
Это связано с тем, что удаление контроллера Dev Spaces не приводит к удалению служб, ранее установленных этим контроллером. Воссоздание контроллера и последующая попытка запустить службы с использованием нового контроллера завершаются неудачно, поскольку старые службы все еще находятся на месте.
...
### <a name="reason"></a>Причина
На компьютере, на котором ведется разработка, не установлено расширение VS Code для C#. Расширение C# включает поддержку отладки для .Net Core (CoreCLR).
...
К сожалению, он может конфликтовать с определенными службами, работающих в этих модулях. В частности, он конфликтует с модулями, выполняющими кэш Redis для Azure, вызывая ошибки подключения и сбои при обмене данными по модели "ведущий/ведомый".
