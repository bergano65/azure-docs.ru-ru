...
При запуске службы в пространстве разработки этот модуль служб внедряется в [дополнительные контейнеры для инструментирования](how-dev-spaces-works.md#prepare-your-aks-cluster). В этих контейнерах не определены запросы ресурсов или ограничения, в результате чего горизонтальный Автомасштабирование Pod будет отключен для Pod.

### <a name="try"></a>Попытка
Запустите горизонтальный Автомасштабирование Pod в пространстве имен, для которого не включены модули разработки.
