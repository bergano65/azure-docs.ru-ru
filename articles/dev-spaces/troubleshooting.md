...
В CLI вы можете выводить больше сведений во время выполнения команды, используя параметр `--verbose`. Вы также можете просмотреть более подробные журналы в `%TEMP%\Azure Dev Spaces`. На компьютере Mac каталог *TEMP* можно найти, запустив `echo $TMPDIR` в окне терминала. На компьютере Linux обычно `/tmp`используется каталог *TEMP* . Кроме того, убедитесь, что в [файле конфигурации Azure CLI](/cli/azure/azure-cli-configuration?view=azure-cli-latest#cli-configuration-values-and-environment-variables)включено ведение журнала.

Azure Dev Spaces также лучше работает при отладке одного экземпляра или Pod. `azds.yaml` Файл содержит параметр *replicaCount*, который указывает количество модулей Pod, которое Kubernetes выполняется для вашей службы. Если вы измените *replicaCount* , чтобы настроить приложение для запуска нескольких модулей Pod для определенной службы, отладчик подключится к первому модулю, если в списке указано в алфавитном порядке. Отладчик присоединяется к другому модулю pod при перезапуске оригинального модуля pod, что может привести к непредвиденному поведению.
...
### <a name="error-azure-dev-spaces-cli-not-installed-properly-when-running-az-aks-use-dev-spaces"></a>Ошибка "Azure Dev Spaces неправильной установки CLI" при выполнении команды AZ AKS use-dev-Spaces
...
Вы по-прежнему можете использовать Azure Dev Spaces с кодом, написанным на других языках, но необходимо вручную создать Dockerfile перед `azds up` первым запуском.
...
После получения соответствующего Dockerfile запустите `azds up` приложение, чтобы запустить его в Azure dev Spaces.
...
При запуске `azds up`может появиться эта ошибка. `azds up` И, `azds prep` и должны запускаться из корневого каталога проекта, который требуется запустить в пространстве разработки.

Чтобы устранить эту проблему:
1. Измените текущий каталог на корневую папку, содержащую код службы. 
1. Если у вас нет файла _аздс. YAML_ в папке Code, выполните команду `azds prep` , чтобы создать docker, Kubernetes и Azure dev Spaces ресурсов.
...
### <a name="error-release-azds-identifier-spacename-servicename-failed-services-servicename-already-exists-or-pull-access-denied-for-servicename-repository-does-not-exist-or-may-require-docker-login"></a>Ошибка "Release аздс-\<идентификатор\>-\<спаценаме\>-\<ServiceName\> \<: службы"\>ServiceName "уже существуют" или "отказано в доступе \<для\>ServiceName, репозиторий не существует или может требовать" DOCKER Login "
...
Azure Dev Spaces можно настроить так, чтобы они указывали на определенные _Dockerfile_ в проекте. Если Azure Dev Spaces не использует файл _Dockerfile_, который вы хотели использовать для сборки своих контейнеров, то можно явно указать Azure Dev Spaces, какой файл Dockerfile нужно использовать. 
...
В командной строке используйте, `--verbose` чтобы включить более подробное ведение журнала. Формат выходных данных также можно указать с помощью `--output`команды. Пример:
...
При использовании [Azure dev Spaces для подключения кластера AKS к компьютеру разработки](how-to/connect.md)может возникнуть ошибка, из-за которой сетевой трафик не пересылается между компьютером разработки и кластером AKS.

При подключении компьютера разработки к кластеру AKS Azure Dev Spaces перенаправляет сетевой трафик между кластером AKS и компьютером разработки, изменяя `hosts` файл компьютера разработки. Azure Dev Spaces создает запись в `hosts` с адресом службы Kubernetes, которую вы заменяете именем узла. Эта запись используется с перенаправлением портов для направления сетевого трафика между компьютером разработки и кластером AKS. Если служба на компьютере разработки конфликтует с портом заменяемой службы Kubernetes, Azure Dev Spaces не может перенаправить сетевой трафик для службы Kubernetes. Например, служба *Windows BranchCache* обычно привязана к *0.0.0.0:80*, что приведет к конфликту порта 80 на всех локальных IP-адресах.

Чтобы устранить эту проблему, необходимо отключить все службы или процессы, конфликтующие с портом службы Kubernetes, которую вы пытаетесь заменить. Для проверки того, какие службы или процессы на компьютере разработки конфликтуют, можно использовать средства, такие как *netstat*.
...
### <a name="error-no-azureassignedidentity-found-for-podazdsazds-webhook-deployment-id-in-assigned-state"></a>Ошибка "не найдено Азуреассигнедидентити для Pod: аздс/аздс-веб-перехватчика —\<Deployment\> -ID в назначенном состоянии"

При запуске службы с Azure Dev Spaces в кластере AKS с установленным [управляемым удостоверением](../aks/use-managed-identity.md) и [управляемыми удостоверениями Pod](../aks/developer-best-practices-pod-security.md#use-pod-managed-identities) процесс может зависнуть после шага *установки диаграммы* . Если вы просматриваете *аздс-инжектор-* The в пространстве имен *аздс* , может появиться эта ошибка.

Azure Dev Spaces служб, выполняемых в кластере, используют управляемое удостоверение кластера для взаимодействия с Azure Dev Spaces серверными службами за пределами кластера. Если установлено управляемое удостоверение Pod, сетевые правила настраиваются на узлах кластера для перенаправления всех вызовов управляемого удостоверения учетных данных в [установленный в кластере демон управляемого удостоверения узла (NMI)](https://github.com/Azure/aad-pod-identity#node-managed-identity). Этот демон NMI определяет вызывающий модуль и проверяет, что для доступа к запрошенному управляемому удостоверению модуль Pod помечен соответствующим образом. Azure Dev Spaces не может определить, установлен ли в кластере управляемое удостоверение Pod, и не сможет выполнить необходимую настройку, чтобы разрешить службам Azure Dev Spaces доступ к управляемому удостоверению кластера. Так как службы Azure Dev Spaces не настроены для доступа к управляемому удостоверению кластера, Демон NMI не позволит им получить маркер AAD для управляемого удостоверения и не сможет взаимодействовать с Azure Dev Spaces серверными службами.

Чтобы устранить эту проблему, примените [азуреподидентитексцептион](https://github.com/Azure/aad-pod-identity/blob/master/docs/readmes/README.app-exception.md) для *аздс-инжектор-перехватчика* и Update, инструментированные с помощью Azure dev Spaces, чтобы получить доступ к управляемому удостоверению.

Создайте файл с именем *вебхукексцептион. YAML* и скопируйте следующее определение YAML:
...
Приведенный выше файл создает объект *азуреподидентитексцептион* для *аздс-инжектор-перехватчика*. Чтобы развернуть этот объект, используйте `kubectl`следующую команду:

```cmd
kubectl apply -f webhookException.yaml
```

Чтобы обновить модули Pod, инструментированные с помощью Azure Dev Spaces для доступа к управляемому удостоверению, обновите *пространство имен* в `kubectl` приведенном ниже определении YAML и примените его для каждого пространства разработки.
...
Кроме того, можно создать объекты *азуреидентити* и *азуреидентитибиндинг* и обновить метки Pod для рабочих нагрузок, выполняемых в пространствах, инструментированных с помощью Azure dev Spaces для доступа к управляемому удостоверению, созданному кластером AKS.
...
Приведенная выше команда выводит *ClientID* и *resourceId* для управляемого удостоверения. Пример:
...
Чтобы создать объект *азуреидентити* , создайте файл с именем *параметра clusteridentity. YAML* и используйте следующее определение YAML с подробными сведениями об управляемом удостоверении из предыдущей команды:
...
Чтобы создать объект *азуреидентитибиндинг* , создайте файл с именем *клустеридентитибиндинг. YAML* и используйте следующее определение YAML:
...
Чтобы развернуть объекты *азуреидентити* и *азуреидентитибиндинг* , используйте `kubectl`:
...
После развертывания объектов *азуреидентити* и *азуреидентитибиндинг* любая рабочая нагрузка с меткой *аадподидбиндинг: My-метка-value* может получить доступ к управляемому удостоверению кластера. Добавьте эту метку и повторно разверните все рабочие нагрузки, выполняющиеся в любом пространстве разработки. Пример:
...
Эта ошибка может появиться при запуске отладчика Visual Studio Code. Возможно, у вас нет расширения VS Code для C#, установленного на компьютере разработки. Расширение C# включает поддержку отладки для .NET Core (CoreCLR).
...
Чтобы устранить эту проблему, обновите `launch.json` файл в `.vscode` подкаталоге папки проекта. Измените каталог `configurations->cwd`, чтобы он указывал на тот же каталог, что и `WORKDIR`, определенный в `Dockerfile` проекта. Возможно, вам также потребуется обновить директиву `configurations->program`.
...
Эта ошибка может произойти, `azds.exe` если не установлена или неправильно настроена.
...
Чтобы устранить эту проблему, используйте учетную запись с правами *владельца* или *участника* в подписке Azure, вручную Зарегистрируйте `Microsoft.DevSpaces` пространство имен:
...
* Конфигурация порта не согласована в _Dockerfile_, Helm диаграмме и любом коде сервера, который открывает порт.
...
1. Проверьте конфигурацию порта в следующих ресурсах:
    * **[Helm диаграмма](https://docs.helm.sh):** задается `service.port` значениями `deployment.containerPort` и в поле Values. YAML, `azds prep` сформированными командой.
...
1. Измените `azds.yaml` файл, чтобы задать для контекста сборки уровень решения.
...
3. Добавьте `.dockerignore` в тот же каталог, где находится `.sln` файл.
4. При необходимости `.dockerignore` обновите с помощью дополнительных записей.
...
При запуске службы в пространстве разработки этот модуль служб [внедряется в дополнительные контейнеры для инструментирования](how-dev-spaces-works-cluster-setup.md#prepare-your-aks-cluster) , а все контейнеры в Pod должны иметь ограничения ресурсов и запросы, настроенные для горизонтального автомасштабирования Pod.

Чтобы устранить эту проблему, примените запрос ресурса и ограничьте его до контейнеров с внедренными пространствами разработки. Запросы и ограничения ресурсов могут быть применены к внедренному контейнеру (девспацес-proxy) путем добавления `azds.io/proxy-resources` заметки в спецификацию Pod. Значение должно быть задано для объекта JSON, представляющего раздел Resources в спецификации контейнера для учетной записи-посредника.
...
### <a name="enable-azure-dev-spaces-on-aks-cluster-with-restricted-egress-traffic-for-cluster-nodes"></a>Включение Azure Dev Spaces в кластере AKS с ограниченным трафиком исходящего трафика для узлов кластера
...
* Используйте `az aks use-dev-spaces -g <resource group name> -n <cluster name>` для обновления текущего контекста. Эта команда также включает Azure Dev Spaces в кластере AKS, если еще не включен. Кроме того, можно использовать `kubectl config use-context <cluster name>` для обновления текущего контекста.
* Используйте `az account show` для отображения текущей целевой подписки Azure и убедитесь, что она правильная. Вы можете изменить целевую подписку с помощью `az account set`.

### <a name="error-using-dev-spaces-after-rotating-aks-certificates"></a>Ошибка при использовании пространств разработки после поворота сертификатов AKS
...
Чтобы устранить эту проблему, убедитесь, что в *kubeconfig* есть обновленные `az aks get-credentials` сертификаты, используя `azds controller refresh-credentials` , а затем выполните команду. Пример:
