...
Создайте новый кластер AKS в регионе с этой функцией предварительной версии. Приведенные ниже команды создают группу ресурсов с именем *MyResourceGroup* и новый кластер AKS с именем *мякс*.
...
Используйте команду *use-dev-Spaces* , чтобы включить пространства разработки в кластере AKS и следовать инструкциям. Следующая команда включает Dev Spaces в кластере *MyAKS* в группе *MyResourceGroup* и создает пространство разработки по умолчанию.
...
Подтвердите, что *развертывание* не выполняется в пространстве имен аздс. Пример:
