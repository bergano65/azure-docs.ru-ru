...
Azure Dev Spaces предоставляет два рекомендуемых, упрощенных варианта хранения секретов в диаграммах Helm, созданных с помощью средств клиента Azure Dev Spaces: в файле `values.dev.yaml` и встроенного непосредственно в `azds.yaml`. Не рекомендуется хранить секреты в `values.yaml`. За пределами двух подходов к Helm диаграммам, созданным средствами клиента, определенными в этой статье, при создании собственной диаграммы Helm вы можете использовать диаграмму Helm непосредственно для управления и хранения секретов.
...
## <a name="method-2-azdsyaml"></a>Метод 2. аздс. YAML
...
## <a name="passing-secrets-as-build-arguments"></a>Передача секретов в качестве аргументов сборки

В предыдущих разделах было показано, как передавать секреты для использования во время выполнения контейнера. Вы также можете передать секрет во время сборки контейнера, например пароль для частного NuGet, с помощью `azds.yaml`.

В `azds.yaml`задайте секреты времени сборки в *конфигурациях. Разработка. сборка. args* с помощью синтаксиса `<variable name>: ${secret.<secret name>.<secret key>}`. Пример.
...
>[!NOTE]
> Имена секретов и ключи могут содержать символ `.`. Используйте `\` для экранирования `.` при передаче секретов в качестве аргументов сборки. Например, для передачи секрета с именем *foo. Bar* с ключом *Token*: `MYTOKEN: ${secret.foo\.bar.token}`. Кроме того, секреты можно оценивать с помощью префиксного и постфиксного текста. Пример: `MYURL: eus-${secret.foo\.bar.token}-version1`. Кроме того, секреты, доступные в родительских и бабушкх пространствах, могут передаваться как аргументы сборки.
