...
Azure Dev Spaces предоставляет два рекомендуемых, упрощенных варианта хранения секретов в диаграммах Helm, созданных с помощью средств клиента Azure Dev Spaces: в файле `values.dev.yaml` и встроенного непосредственно в `azds.yaml`. Не рекомендуется хранить секреты в `values.yaml`.
...
В проекте, который уже подготовлен с Azure Dev Spaces, создайте файл `values.dev.yaml` в той же папке, что и `azds.yaml`, чтобы определить секретные ключи и значения. Пример.
...
Убедитесь, что `azds.yaml` файл ссылается на `values.dev.yaml` как необязательный с помощью `?`. Пример.
...
Чтобы убедиться, что секреты созданы, используйте `kubectl`.
...
Создайте файл `.env` в той же папке, что и `azds.yaml`, чтобы определить значения *$PLACEHOLDER* . Пример.
