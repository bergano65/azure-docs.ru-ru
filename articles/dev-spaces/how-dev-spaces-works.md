...
* Использует [прямого порта](https://kubernetes.io/docs/tasks/access-application-cluster/port-forward-access-application-cluster/) для разрешения веб-доступа для приложения с помощью http://locahost.
...
! [Azure пробелы разработки подготовить кластер]] (media/how-dev-spaces-works/prepare-cluster.svg)
...
Объект инициализатора Kubernetes используется для добавления модулей POD с тремя контейнерами во время развертывания для инструментирования: контейнер devspaces прокси, devspaces прокси init контейнер и контейнер devspaces сборки. **Все три эти контейнеры выполняются корневой доступ в кластере AKS.**
