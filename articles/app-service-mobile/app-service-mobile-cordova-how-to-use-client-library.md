---
title: Использование подключаемого модуля Apache Cordova
...
7. Найдите элемент "allowedExternalRedirectUrls".  Ему может быть присвоено значение NULL или массив значений.  Измените его значение на следующее:
    ...
    Замените URL-адреса на URL-адреса службы.  Примеры включают `http://localhost:3000` (для примера службы Node. js) или `http://localhost:4400` (для службы Ripple).  Это только примеры URL-адресов. Ваша ситуация, включая упомянутые в приведенных примерах службы, может быть совершенно другой.
