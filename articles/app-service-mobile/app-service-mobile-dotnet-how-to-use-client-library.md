---
title: Работа с управляемой клиентской библиотекой
description: Узнайте, как использовать клиентскую библиотеку .NET для мобильных приложений службы приложений Azure с приложениями Windows и Xamarin.
...
Предполагается, что вы уже создали и опубликовали проект внутреннего сервера мобильных приложений, который содержит по меньшей мере одну таблицу.  В коде, который используется в этом разделе, применяется таблица `TodoItem`, которая содержит следующие столбцы: `Id`, `Text` и `Complete`. Эта таблица представляет собой ту же таблицу, созданную при завершении [запуска мобильных приложений Azure.][1]
...
* **Студия Ксамарин** Нажмите правой кнопкой мыши вашего проекта, `Microsoft.Azure.Mobile.Client` нажмите **Добавить** > **NuGet пакеты,** поиск пакета, а затем нажмите **Добавить пакет**.
...
## <a name="create-the-mobile-apps-client"></a><a name="create-client"></a>Создание клиента мобильных приложений
...
Ниже подробно описано, как выполнить поиск, извлечение записей и изменение данных в таблице.  Рассмотрены следующие вопросы:

* [Создание справочника таблицы](#instantiating)
* [Запрос данных](#querying)
* [Фильтр вернул данные](#filtering)
* [Сортировка возвращаемых данных](#sorting)
* [Возврат данных на страницах](#paging)
* [Выберите определенные столбцы](#selecting)
* [Поиск записи по идентификатору](#lookingup)
* [Работа с нетипизированными запросами](#untypedqueries)
* [Добавление данных](#inserting)
* Обновление данных
...
### <a name="how-to-create-a-table-reference"></a><a name="instantiating"></a>Практическое руководство. Создание ссылки на таблицу
...
Возвращаемый объект использует типизированную модель сериализации. Также поддерживается нетипизированная модель сериализации. Следующий пример [создает ссылку на нетипивную таблицу:]
...
### <a name="how-to-query-data-from-your-mobile-app"></a><a name="querying"></a>Практическое руководство. Запрос данных из мобильного приложения
...
### <a name="how-to-filter-returned-data"></a><a name="filtering"></a>Как: Фильтр вернулся данных
Следующий код иллюстрирует способ фильтрации данных путем включения предложения `Where` в запрос. Он возвращает все элементы таблицы `todoTable`, свойство `Complete` которых равно `false`. Функция [Where] применяет сярвидную фильтрацию предиката к запросу против таблицы.
...
### <a name="how-to-sort-returned-data"></a><a name="sorting"></a>Практическое руководство. Сортировка возвращаемых данных
...
### <a name="how-to-return-data-in-pages"></a><a name="paging"></a>Практическое руководство. Возврат данных на страницах
...
### <a name="how-to-select-specific-columns"></a><a name="selecting"></a>Практическое руководство. Выбор определенных столбцов
...
### <a name="how-to-look-up-data-by-id"></a><a name="lookingup"></a>Практическое руководство. Поиск данных по идентификатору
...
### <a name="how-to-execute-untyped-queries"></a><a name="untypedqueries"></a>Практическое руководство. Выполнение нетипизированных запросов
...
### <a name="how-to-insert-data-into-a-mobile-app-backend"></a><a name="inserting"></a>Практическое руководство. Вставка данных в серверную часть мобильных приложений
Все типы клиентов должны содержать член с именем **Id**(идентификатор), который по умолчанию является строкой. Этот **идентификатор** необходим для выполнения операций CRUD и для синхронизации в автономном режиме. Следующий код иллюстрирует, как использовать метод [InsertAsync] для вставки новых строк в таблицу. Параметр содержит данные, которые вставляются в качестве объекта .NET.
...
### <a name="how-to-modify-data-in-a-mobile-app-backend"></a><a name="modifying"></a>Практическое руководство. Изменение данных в серверной части мобильных приложений
...
### <a name="how-to-delete-data-in-a-mobile-app-backend"></a><a name="deleting"></a>Практическое руководство. Удаление данных в серверной части мобильных приложений
...
### <a name="how-to-use-optimistic-concurrency-for-conflict-resolution"></a><a name="optimisticconcurrency"></a>Практическое руководство. Использование оптимистичного параллелизма для устранения конфликтов
...
Чтобы включить оптимистичный параллелизм, определите столбец в классе таблицы для системного свойства `version` . Пример:
...
### <a name="how-to-bind-mobile-apps-data-to-a-windows-user-interface"></a><a name="binding"></a>Практическое руководство. Привязка данных мобильных приложений к пользовательскому интерфейсу Windows
...
### <a name="change-the-page-size"></a><a name="pagesize"></a>Изменение размера страницы
...
## <a name="work-with-offline-tables"></a><a name="#offlinesync"></a>Работа с автономными таблицами
...
3. (необязательно). Для устройств Windows нажмите **Ссылки** > **Добавить справку ...**, расширить папку **Windows** > **расширения,** а затем включить соответствующий **S'Lite для Windows** SDK вместе с **визуальным C ' 2013 Runtime для Windows** SDK.
...
### <a name="syncing-an-offline-table"></a><a name="syncoffline"></a>Синхронизация автономной таблицы
...
## <a name="work-with-a-custom-api"></a><a name="#customapi"></a>Работа с настраиваемым API
...
## <a name="authenticate-users"></a><a name="authentication"></a>Аутентификация пользователей
...
### <a name="client-managed-authentication"></a><a name="clientflow"></a>Управляемая клиентом проверка подлинности.
...
#### <a name="authenticate-users-with-the-active-directory-authentication-library"></a><a name="adal"></a>Аутентификация пользователей с помощью Библиотеки проверки подлинности Active Directory
...
#### <a name="single-sign-on-using-a-token-from-facebook-or-google"></a><a name="client-facebook"></a>Единый вход с помощью маркера Google или Facebook
...
### <a name="server-managed-authentication"></a><a name="serverflow"></a>Управляемая сервером проверка подлинности.
...
### <a name="caching-the-authentication-token"></a><a name="caching"></a>Кэширование маркера аутентификации
В некоторых случаях после первой успешной аутентификации можно не выполнять вызов метода входа в систему. Для этого следует сохранить маркер аутентификации.  Приложения Microsoft Store и UWP могут использовать [PasswordVault] для сохранения текущего маркера аутентификации после успешного входа в систему.
...
Приложения Xamarin используют AAP [Xamarin.Auth] для надежного хранения учетных данных в объекте **учетной записи.** Пример использования этих API см. в файле кода [AuthStore.cs], который включен в [пример совместного использования фотографий ContosoMoments](https://github.com/azure-appservice-samples/ContosoMoments).
...
## <a name="push-notifications"></a><a name="pushnotifications"></a>Нажмите Уведомления
Push-уведомления рассматриваются в следующих разделах.

* [Регистрация для получения push-уведомлений](#register-for-push)
* [Получение ИД безопасности пакета Microsoft Store](#package-sid)
* [Регистрация с помощью межплатформенных шаблонов](#register-xplat)

### <a name="how-to-register-for-push-notifications"></a><a name="register-for-push"></a>Как: Регистрация на Push-уведомления
...
### <a name="how-to-obtain-a-microsoft-store-package-sid"></a><a name="package-sid"></a>Получение SID пакета Microsoft Store
...
1. В Visual Studio Solution Explorer, правой кнопкой мыши проекта приложения Microsoft Store, нажмите **Store** > **Associate App с магазином ...**.
...
5. Нажмите **приложение управления** > **приложением идентификатор,** а затем прокрутите **вниз,** чтобы найти свой пакет SID .
...
### <a name="how-to-register-push-templates-to-send-cross-platform-notifications"></a><a name="register-xplat"></a>Использование шаблонов для отправки кроссплатформенных push- уведомлений
...
## <a name="miscellaneous-topics"></a><a name="misc"></a>Разное
### <a name="how-to-handle-errors"></a><a name="errors"></a>Практическое руководство. Обработка ошибок
...
### <a name="how-to-customize-request-headers"></a><a name="headers"></a>Практическое руководство. Настройка заголовков запроса
...
[ВключитьТотальтолик]: https://msdn.microsoft.com/library/azure/dn250560(v=azure.10).aspx
...
[Принять]: https://msdn.microsoft.com/library/azure/dn250574(v=azure.10).aspx
[Выбрать]: https://msdn.microsoft.com/library/azure/dn250569(v=azure.10).aspx
[Пропустить]: https://msdn.microsoft.com/library/azure/dn250573(v=azure.10).aspx
...
[Где]: https://msdn.microsoft.com/library/azure/dn250579(v=azure.10).aspx
