...
В этом руководстве показано, как реализовать типичные сценарии с использованием управляемой клиентской библиотеки для мобильных приложений службы приложений Azure в приложениях Windows и Xamarin. Если вы не знакомы с мобильными приложениями, сначала ознакомьтесь с руководством по началу работы с [мобильными приложениями Azure][1] . В данном руководстве мы сосредоточимся на управляемом пакете SDK клиентской части. Дополнительные сведения о серверных пакетах SDK для мобильных приложений см. в документации по [серверу SDK для .NET][2] или к [пакету SDK для сервера Node. js][3].

## <a name="reference-documentation"></a>Справочная документация
Справочная документация по пакету SDK клиента находится здесь: [Azure Mobile Apps .NET client reference][4](Клиент .NET мобильных служб Azure).
Несколько примеров клиентов доступно в [репозитории GitHub Azure Samples][5](Примеры для Azure).
...
[Атрибут jsonpropertyattribute][6] используется для определения сопоставления *PropertyName* между полем клиента и полем таблицы.

Сведения о создании таблиц в серверной части мобильных приложений см. в [разделе пакет SDK для сервера .NET][7] или в [разделе пакет SDK для сервера Node. js][8]. При создании серверной части мобильного приложения на портале Azure с помощью краткого руководства можно использовать параметр **Простые таблицы** на [Портал Azure].
...
* **Xamarin Studio** Щелкните проект правой кнопкой мыши, выберите **добавить** > **Добавить пакеты NuGet**, найдите пакет `Microsoft.Azure.Mobile.Client` и нажмите кнопку **Добавить пакет**.
...
В приведенном выше коде замените `MOBILE_APP_URL` URL-адресом серверной части мобильного приложения, который можно найти в колонке серверной части мобильного приложения на [Портал Azure]. Объект MobileServiceClient должен быть одноэлементным.
...
Все типы клиентов должны содержать член с именем **Id**(идентификатор), который по умолчанию является строкой. Этот **идентификатор** необходим для выполнения операций CRUD и для автономной синхронизации. В следующем коде показано, как использовать метод [InsertAsync] для вставки новых строк в таблицу. Параметр содержит данные, которые вставляются в качестве объекта .NET.
...
При выполнении обновления необходимо указать поле `id` . Серверная часть использует поле `id` , чтобы определить, какую из строк нужно обновить. Поле `id` можно получить из результатов вызова метода `InsertAsync`. При попытке обновить элемент без указания значения `ArgumentException` порождается исключение `id`.
...
При создании запроса на удаление необходимо указать идентификатор. Другие свойства не передаются в службу или игнорируются ею. В результате вызова функции `DeleteAsync` обычно возвращается значение `null`. Идентификатор для передачи можно получить в результате вызова метода `InsertAsync` . При попытке удалить элемент без указания поля `MobileServiceInvalidOperationException` порождается исключение `id`.
...
По умолчанию мобильные приложения Azure выдают не больше 50 элементов на запрос.  Можно изменить параметры разбиения по страницам, увеличив максимальный размер страницы для клиента и сервера.  Чтобы увеличить размер запрошенной страницы, при использовании `PullOptions` укажите `PullAsync()`.
...
2. (Необязательный шаг.) Чтобы обеспечить поддержку устройств Windows, установите один из следующих пакетов среды выполнения SQLite:
   ...
   * **Универсальная платформа Windows** Установите [SQLite для универсальных окон][5].
...
1. Настройте серверную часть мобильного приложения для входа с помощью AAD, следуя указаниям в учебнике [Как настроить службу приложений для входа с помощью Active Directory]. Обязательно выполните дополнительный этап регистрации собственного клиентского приложения.
...
* [Получение SID пакета Microsoft Store](#package-sid)
...
1. В обозревателе решений Visual Studio щелкните правой кнопкой мыши проект приложения Microsoft Store и щелкните **Магазин** > **Связать приложение с Магазином…**
