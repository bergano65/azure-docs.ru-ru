---
title: Работа с Задней серверной SDK Node.js
...
### <a name="create-a-basic-nodejs-back-end-by-using-the-command-line"></a><a name="howto-cmdline-basicapp"></a>Создание базовой серверной части на основе Node.js с помощью командной строки
...
* [Быстрый запуск клиента в Магазине Windows]
* [Быстрый запуск клиента Xamarin.iOS]
* [Xamarin.Android клиент quickstart]
* [Xamarin.Forms клиент quickstart]

Код этого базового приложения можно найти в [примере basicapp на сайте GitHub].

### <a name="create-a-nodejs-back-end-by-using-visual-studio-2015"></a><a name="howto-vs2015-basicapp"></a>Создание серверной части на основе Node.js с помощью Visual Studio 2015
...
1. Расширьте **шаблоны** > **JavaScript** > **Node.js**.
...
1. Либо запустите приложение локально (API `http://localhost:3000`обслуживается в ней), либо опубликуйте в Azure.

### <a name="create-a-nodejs-back-end-by-using-the-azure-portal"></a><a name="create-node-backend-portal"></a>Создание серверной части Node.js с помощью портала Azure
...
### <a name="download-the-nodejs-back-end-quickstart-code-project-by-using-git"></a><a name="download-quickstart"></a>Загрузка серверной части на основе Node.js в виде готового кода для быстрого запуска с помощью Git
...
### <a name="publish-your-nodejs-back-end-to-azure"></a><a name="howto-publish-to-azure"></a>Публикация серверной части на основе Node.js в Azure
...
### <a name="enable-a-home-page-for-your-application"></a><a name="howto-enable-homepage"></a>Включение домашней страницы для приложения
...
## <a name="table-operations"></a><a name="TableOperations"></a>Операции с таблицами
...
### <a name="define-tables-by-using-a-dynamic-schema"></a><a name="howto-dynamicschema"></a>Определение таблиц с помощью динамической схемы
...
### <a name="define-tables-by-using-a-static-schema"></a><a name="howto-staticschema"></a>Определение таблиц с помощью статической схемы
...
### <a name="use-sql-server-express-as-a-development-data-store-on-your-local-machine"></a><a name="howto-sqlexpress-setup"></a>Использование SQL Server Express в качестве хранилища данных при разработке на локальном компьютере
...
1. Запустите диспетчер конфигурации SQL Server 2014.
   ...
   c. Нажмите справа на **TCP/IP** и выберите **Включить**. Нажмите кнопку **ОК** во всплывающем диалоговом окне.
   ...
   i. Правой кнопкой **мыши сервера S'L (S'LEXPRESS)** и выберите **перезагрузку.**
...
1. Запустите SQL Server 2014 Management Studio и подключитесь к локальному экземпляру SQL Server Express.
   ...
   1. Расширьте**логины** **безопасности** > в Object Explorer.
   1. Правый клик **Логины** и выберите **Новый Войти**.
   1. Введите имя входа. Выберите параметр **Проверка подлинности SQL Server**. Введите пароль, а затем в поле **Confirm password** (Подтверждение пароля) введите его еще раз. Пароль должен соответствовать требованиям к сложности паролей, установленным в Windows.
   1. Нажмите кнопку **ОК**.
...
### <a name="configure-your-project-for-local-development"></a><a name="howto-config-localdev"></a>Настройка проекта для локальной разработки
...
Этот пример **файла azureMobile.js** реализует предыдущие настройки базы данных:
...
### <a name="configure-app-settings-for-your-mobile-app"></a><a name="howto-appsettings"></a>Настройка параметров для мобильного приложения

Для большинства параметров в файле azureMobile.js имеется эквивалентный параметр приложения на [портале Azure]. Используйте следующий список для настройки приложения в **настройках приложения:**
...
1. Прокрутите в разделе **настроек приложения.**
...
### <a name="use-sql-database-as-your-production-data-store"></a><a name="howto-use-sqlazure"></a>Использование базы данных SQL для хранения данных в рабочей среде
...
1. Будет выбран план службы приложений по умолчанию. Если вы хотите изменить план службы приложений, сделайте следующее:
   ...
   а. Выберите **план** > обслуживания приложений **«Создайте новый**.
...
1. В новом задней части мобильных приложений выберите **Настройки** > **Данные мобильных** > **приложений** > **.Добавлено**.
...
### <a name="use-authentication-claims-with-your-tables"></a><a name="howto-tables-getidentity"></a>Использование утверждений проверки подлинности для таблиц
...
### <a name="disable-access-to-specific-table-operations"></a><a name="howto-tables-disabled"></a>Запрет доступа к определенным операциям с таблицей
...
### <a name="adjust-the-query-that-is-used-with-table-operations"></a><a name="howto-tables-query"></a>Изменение запроса, используемого в операциях с таблицей
...
### <a name="configure-a-soft-delete-on-a-table"></a><a name="howto-tables-softdelete"></a>Настройка обратимого удаления для таблицы
...
### <a name="seed-your-database-with-data"></a><a name="howto-tables-seeding"></a>Заполнение базы данных данными
...
### <a name="enable-swagger-support"></a><a name="Swagger"></a>Включение поддержки Swagger
...
## <a name="push-notifications"></a><a name="push"/>Push-уведомления
...
### <a name="send-push-notifications"></a><a name="send-push"></a>Отправка push-уведомлений
...
### <a name="send-push-notifications-to-an-authenticated-user-by-using-tags"></a><a name="push-user"></a>Отправка push-уведомлений прошедшему проверку подлинности пользователю с помощью тегов
...
## <a name="custom-apis"></a><a name="CustomAPI"></a>Пользовательские AIS

### <a name="define-a-custom-api"></a><a name="howto-customapi-basic"></a>Определение настраиваемого интерфейса API
...
### <a name="require-authentication-for-access-to-a-custom-api"></a><a name="howto-customapi-auth"></a>Обязательная проверка подлинности для доступа к настраиваемому API
...
### <a name="handle-large-file-uploads"></a><a name="howto-customapi-auth"></a>Обработка передачи больших файлов
...
### <a name="execute-custom-sql-statements"></a><a name="howto-customapi-sql"></a>Выполнение пользовательских инструкций SQL
...
### <a name="debug-diagnose-and-troubleshoot-mobile-apps"></a><a name="howto-diagnostic-logs"></a>Отладка, диагностика и устранение неполадок функции "Мобильные приложения"
...
[Xamarin.iOS Клиент быстрый запуск]: app-service-mobile-xamarin-ios-get-started.md
...
[Настройка аутентификации Azure Active Directory]: ../app-service/configure-authentication-provider-aad.md
