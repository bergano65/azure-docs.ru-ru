---
title: Включить синхронизацию в автономном режиме (iOS)
...
## <a name="review-the-client-sync-code"></a><a name="review-sync"></a>Просмотр кода синхронизации клиента
...
* **Объектив-C**. В методе **QSTodoService.init**:

   ```objc
   MSCoreDataStore *store = [[MSCoreDataStore alloc] initWithManagedObjectContext:context];
   self.client.syncContext = [[MSSyncContext alloc] initWithDelegate:nil dataSource:store callback:nil];
   ```    
* **Свифт**. В методе **ToDoTableViewController.viewDidLoad**:
...
* **Объектив-C**. Метод `syncData` сначала передает новые изменения, а затем вызывает **pullData** для получения данных из удаленной серверной части. В свою очередь, метод **pullData** получает новые данные, удовлетворяющие запросу:
...
## <a name="review-the-core-data-model"></a><a name="review-core-data"></a>Обзор модели Core Data
...
> [!NOTE]
> Мобильные приложения SDK резервы названия**``** столбцов, которые начинаются с ". Не используйте этот префикс где-либо, кроме системных столбцов. В противном случае при использовании удаленной серверной части имена таких столбцов будут изменены.
>
>
...
| version | Строка | (необязательно) Используется для обнаружения конфликтов, сопоставляется со свойством version |

## <a name="change-the-sync-behavior-of-the-app"></a><a name="setup-sync"></a>Изменение режима синхронизации приложения
...
## <a name="test-the-app"></a><a name="test-app"></a>Тестирование приложения
...
1. Измените URL-адрес мобильного приложения в файле **QSTodoService.m** на недействительный и снова запустите приложение:
   ...
   **Объектив-C**. В файле QSTodoService.m:
   ```objc
   self.client = [MSClient clientWithApplicationURLString:@"https://sitename.azurewebsites.net.fail"];
   ```
   **Свифт**. В файле ToDoTableViewController.swift:
   ```swift
...
3. Просмотр содержимого удаленной таблицы **TodoItem:**
...
[Создание приложения для iOS]: app-service-mobile-ios-get-started.md
