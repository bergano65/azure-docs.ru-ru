...
* @no__t 0Android: Включить автономную синхронизацию @ no__t-0
* @no__t 0Apache Cordova: Включить автономную синхронизацию @ no__t-0
* @no__t 0iOS: Включить автономную синхронизацию @ no__t-0
* @no__t 0Xamarin iOS: Включить автономную синхронизацию @ no__t-0
* @no__t 0Xamarin Android: Включить автономную синхронизацию @ no__t-0
* @no__t 0Xamarin. Forms: Включить автономную синхронизацию @ no__t-0
* Платформа Windows [Universal: Включить автономную синхронизацию @ no__t-0
...
Чтобы использовать реализацию на основе SQLite для Windows Phone или Microsoft Store, необходимо установить расширение SQLite. Дополнительные сведения см. в разделе Платформа Windows [Universal: Включите автономную синхронизацию @ no__t-0. Android и iOS поставляются с версией SQLite в операционной системе устройства, поэтому нет необходимости ссылаться на собственную версию SQLite.
...
* **Отправка**: Push — это операция в контексте синхронизации, которая отправляет все изменения CUD с момента последней отправки. Обратите внимание, что нельзя передать только изменения в отдельной таблице, так как в противном случае операции могут быть отправлены не по порядку. Операция отправки выполняет серию вызовов REST к серверной части мобильного приложения Azure, которая, в свою очередь, изменяет сервер базы данных.
* **Извлечение**: Извлечение выполняется отдельно для каждой таблицы и может быть настроено с помощью запроса для получения только подмножества данных сервера. Клиентские пакеты SDK для мобильных приложений Azure вставляют результирующие данные в локальное хранилище.
* **Неявные push-уведомления**: Если операция Pull выполняется для таблицы, которая имеет ожидающие локальные обновления, то сначала выполняется `push()` в контексте синхронизации. Это позволяет свести к минимуму конфликты между изменениями, которые уже поставлены в очередь, и новыми данными с сервера.
...
* **Очистка**: Можно очистить содержимое локального хранилища, используя `IMobileServiceSyncTable.PurgeAsync`.
