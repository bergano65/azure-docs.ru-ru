---
title: Синхронизация данных в автономном режиме
...
Чтобы использовать реализацию на основе SQLite для Windows Phone или Microsoft Store, необходимо установить расширение SQLite. Для получения дополнительной информации [см.] Android и iOS отправляются с версией S'Lite в самой операционной системе устройства, поэтому нет необходимости ссылаться на собственную версию S'Lite.
...
Локальный магазин связан с контекстом синхронизации с `IMobileServicesSyncContext.InitializeAsync(localstore)` помощью метода инициализации, например, в [SDK клиента .NET.]

## <a name="how-offline-synchronization-works"></a><a name="how-sync-works"></a>Как работает автономная синхронизация
...
* **Добавочная синхронизация**: первый параметр операции извлечения — *имя запроса* , которое используется только на стороне клиента. Если вы используете имя не-нулевого запроса, Azure Mobile SDK выполняет *поэтапную синхронизацию.* Каждый раз, когда операция pull возвращает `updatedAt` набор результатов, последняя метка времени из этого набора результатов хранится в таблицах локальной системы SDK. Последующие операции извлечения будут получать только записи после этой метки времени.
