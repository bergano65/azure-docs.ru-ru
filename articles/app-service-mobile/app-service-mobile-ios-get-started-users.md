---
title: Добавление проверки подлинности в iOS
description: Узнайте, как использовать мобильные приложения Azure для проверки подлинности пользователей приложения iOS с помощью поставщиков удостоверений, таких как AAD, Google, Facebook, Twitter и Майкрософт.
...
1. На компьютере Mac откройте файл *QSTodoListViewController.m* в Xcode и добавьте следующий метод.
    ...
    Измените *google* на *microsoftaccount*, *twitter*, *facebook* или *windowsazureactivedirectory*, если Google не используется как поставщик удостоверений. При использовании Facebook необходимо [список разрешений домены Facebook][1] в приложении.
...
4. Откройте файл `QSAppDelegate.m` и добавьте в него следующий код.
   ...
   Добавьте этот код непосредственно перед строкой `#pragma mark - Core Data stack`.  Замените _AppName_ значением urlScheme, которое использовалось на шаге 1.
...
3. Откройте файл `AppDelegate.swift` и добавьте следующую строку в класс `AppDelegate`.
    ...
    Замените _AppName_ значением urlScheme, которое использовалось на шаге 1.
...
Для аутентификации службы приложений используется технология взаимодействия приложений Apple.  Дополнительные сведения об этой теме см. в [документации Apple][2] .
<!-- URLs. -->
