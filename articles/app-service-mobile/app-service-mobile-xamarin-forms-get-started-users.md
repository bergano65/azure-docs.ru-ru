---
title: Начало работы с проверкой подлинности в приложении Xamarin Forms
description: Узнайте, как использовать мобильные приложения для проверки подлинности пользователей приложения Xamarin Forms с помощью поставщиков удостоверений, таких как AAD, Google, Facebook, Twitter и Майкрософт.
...
Для лучшего результата работы с этим руководством рекомендуется сначала выполнить инструкции из учебника [Создание приложения Xamarin Forms][1] . Завершив работу, вы получите проект Xamarin Forms — кроссплатформенное приложение TodoList.

Если вы не используете скачанный проект быстрого запуска сервера, в проект необходимо добавить пакет расширений для аутентификации. в статье [Работа с пакетом SDK для внутреннего сервера .NET для мобильных приложений Azure][2].
...
Мобильные приложения используют метод расширения [LoginAsync][3] на [MobileServiceClient][4] для входа пользователя с проверкой подлинности службы приложений. В этом примере используется управляемый сервером поток проверки подлинности, который отображает интерфейс входа поставщика в приложение. Дополнительные сведения см. в разделе [управляемая сервером проверка подлинности][5]. Чтобы обеспечить лучшее взаимодействие с пользователем в рабочем приложении, вместо этого следует использовать [управляемую клиентом проверку подлинности][6].
...
5. Обновите класс **MainActivity**, добавив поле **MobileServiceUser** и метод **Authenticate**, который необходим для интерфейса **IAuthenticate**, как показано ниже.
    ...
    Если используется поставщик удостоверений, отличный от Facebook, выберите другое значение [MobileServiceAuthenticationProvider][7].
...
1. Добавьте следующую строку кода в конструктор для класса **MainPage** перед вызовом метода`LoadApplication()` :
