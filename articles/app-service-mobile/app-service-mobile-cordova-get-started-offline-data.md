...
2. Затем замените код
    ...
    Предыдущие дополнения кода позволяют выполнить инициализацию локального хранилища и определить локальную таблицу, соответствующую значениям столбцов, используемых в серверной части Azure. (В этом коде не нужно включать все значения столбцов.)  Поле `version` поддерживается мобильным внутренним сервером и используется для разрешения конфликтов.
