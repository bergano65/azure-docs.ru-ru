---
title: Включить синхронизацию в автономном режиме (Cordova)
description: Узнайте, как использовать мобильное приложение App Service для кэша и синхронизации автономных данных в приложении Cordova.
...
Дополнительные сведения о функции автономной синхронизации см. в статье [Синхронизация автономных данных в мобильных приложениях Azure]. Сведения об использовании API см. в [документации по API](https://azure.github.io/azure-mobile-apps-js-client).
...
2. Затем замените код
    ...
    Предыдущие дополнения кода позволяют выполнить инициализацию локального хранилища и определить локальную таблицу, соответствующую значениям столбцов, используемых в серверной части Azure. (Не нужно включать все значения столбца в этот код.)  Поле `version` поддерживается мобильным бэкэндом и используется для разрешения конфликтов.
...
5. (Необязательно.) С помощью Visual Studio просмотрите таблицу базы данных SQL Azure, чтобы увидеть, что данные в серверной базе данных не изменились.
    ...
    В Visual Studio в откройте **обозреватель сервера**. Перейдите к базе данных в **базах**->данных Azure**S'L.** Щелкните правой кнопкой мыши базу данных и выберите пункт **Открыть в обозревателе объектов SQL Server**. Теперь можно перейти к таблице базы данных SQL и ее содержимому.
...
* [Инструменты визуальной студии для Apache Cordova]
...
[здесь]: https://github.com/Azure-Samples/app-service-mobile-cordova-client-conflict-handling
