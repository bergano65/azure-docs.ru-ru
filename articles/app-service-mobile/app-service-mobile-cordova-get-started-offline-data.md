---
title: Включить автономную синхронизацию (Cordova)
description: Узнайте, как использовать мобильное приложение службы приложений для кэширования и синхронизации автономных данных в приложении Cordova.
...
2. Затем замените код
    ...
    Предыдущие дополнения кода позволяют выполнить инициализацию локального хранилища и определить локальную таблицу, соответствующую значениям столбцов, используемых в серверной части Azure. (В этом коде не нужно включать все значения столбцов.)  Поле `version` обслуживается мобильным внутренним сервером и используется для разрешения конфликтов.
