...
В этом руководстве описано использование клиентского пакета Android SDK.  Дополнительные сведения о серверных пакетах SDK для мобильных приложений, см. в разделе [работа с серверной частью .NET SDK][10] or [How to use the Node.js backend SDK][11].

## <a name="reference-documentation"></a>Справочная документация

Можно найти [Справочник javadocs по API][12] для клиентской библиотеки Android на сайте GitHub.
...
* [Создание серверной части мобильного приложения][13] для использования с приложением Android.
...
1. Добавьте следующий код в *проекта* уровень **build.gradle** файла:
...
2. Добавьте следующий код в файл *build.gradle* уровня **Module app** внутри тега *dependencies*.
    ...
    Сейчас последней версией является 3.4.0. Поддерживаемые версии перечислены [на bintray][14].
...
Сведения о создании дополнительных таблиц в серверной части мобильных приложений см. в разделе [Практическое руководство. Определение контроллера таблиц][15] (.NET backend) or [Define Tables using a Dynamic Schema][16] (Серверная часть Node.js).
...
* `String id`: глобальный уникальный идентификатор для записи.  Рекомендуется, сделать этот идентификатор представлением строки [UUID][17] объекта.
...
Поле `id` является обязательным.  Поля `updatedAt` и `version` используются при синхронизации в автономном режиме (для добавочной синхронизации и устранения конфликтов соответственно).  `createdAt` — это эталонное поле. Оно не используется клиентом.  Имена свойств можно передавать по сети. Они не изменяются.  Тем не менее, можно создать сопоставление между своим объектом и имен «на лету», с помощью [gson][3] библиотеки.  Пример:
...
Чтобы получить доступ к таблице, сначала создайте [MobileServiceTable][8] путем вызова метода **getTable** метод [MobileServiceClient][9].  У этого метода две перегрузки.
...
Вы найдете полный пример в [Android проект быстрого запуска][21].
...
В следующем коде показано, как вставить данные. Первым шагом является создание [JsonObject][1] , which is part of the [gson][3] библиотеки.
...
Можно указать стратегию преобразования, которая применяется к каждому столбцу, с помощью [gson][3] API. Клиентская библиотека Android использует [gson][3] незаметно для сериализации объектов Java в данные JSON перед отправкой данных в службе приложений Azure.  В следующем коде используется метод **setFieldNamingStrategy()** для задания стратегии. Этот пример удалит начальный знак ("m"), а затем преобразует следующий знак в нижний регистр для каждого имени поля. Например, он преобразует "mId" в "id".  Реализуйте стратегию преобразования, чтобы снизить потребность в аннотациях `SerializedName()` в большинстве полей.
