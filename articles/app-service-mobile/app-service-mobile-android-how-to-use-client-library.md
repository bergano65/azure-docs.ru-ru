...
>
> Если вы хотите интегрировать облачные службы в мобильное приложение, зарегистрируйтесь в [центре приложений](https://appcenter.ms/signup?utm_source=zumo&utm_medium=Azure&utm_campaign=zumo%20doc) уже сегодня.
...
В этом руководстве описано использование клиентского пакета Android SDK.  Дополнительные сведения о клиентских пакетах SDK для мобильных приложений см. в разделе [Работа с серверной частью пакета SDK для .NET][10] или использование СЕРВЕРНОГО [пакета SDK для Node. js][11].

## <a name="reference-documentation"></a>Справочная документация

[Справочник по API Справочник Javadocs по][12] для клиентской библиотеки Android можно найти на сайте GitHub.
...
1. Добавьте следующий код в файл **Build. gradle** уровня *проекта* :
...
2. Добавьте следующий код в файл *build.gradle* уровня **Module app** внутри тега *dependencies*.
    ...
    Сейчас последней версией является 3.4.0. Поддерживаемые версии перечислены [в бинтрай][14].
...
Сведения о создании дополнительных таблиц в серверной части мобильных приложений см. в разделе [как определить контроллер таблиц][15] (серверная часть .NET) или [определить таблицы с помощью динамической схемы][16] (серверная часть Node. js).
...
* `String id` — глобальный уникальный идентификатор записи.  Рекомендуется сделать идентификатор строковым представлением объекта [UUID][17] .
...
Чтобы получить доступ к таблице, сначала создайте объект [MobileServiceTable][8] , вызвав метод GetObject в **объекте** [MobileServiceClient][9].  У этого метода две перегрузки.
...
Приведенный выше пример возвращает все результаты (вплоть до максимального размера страницы, заданного на сервере).  Метод `.execute()` выполняет запрос в серверной части.  Запрос преобразуется в запрос [OData v3][19] перед передачей в серверную часть мобильных приложений.  После получения этот запрос преобразовывается в серверной части в инструкцию SQL перед выполнением его на экземпляре SQL Azure.  Так как активность сети занимает некоторое время, метод `.execute()` возвращает [`ListenableFuture<E>`][18].
...
Полный пример можно найти в проекте быстрого запуска [Android][21].
...
В следующем коде показано, как вставить данные. Первым шагом является создание [JsonObject][1], который является частью библиотеки [gson][3] .
...
Можно задать разрешения таблиц, чтобы предоставить доступ к определенным операциям только пользователям, прошедшим проверку подлинности. Также можно использовать идентификатор безопасности пользователя, прошедшего проверку, чтобы изменять запросы.  Дополнительную информацию см. в разделе [Приступая к работе с аутентификацией в мобильных службах] и справочной документации по пакету SDK для сервера.
...
Получите идентификатор вошедшего в систему пользователя из **MobileServiceUser** с помощью метода **getUserId**. Пример использования Futures для вызова API асинхронного входа см. в статье [Приступая к работе с аутентификацией в мобильных службах].
...
Можно указать стратегию преобразования, которая применяется к каждому столбцу с помощью API [gson][3] . Клиентская библиотека Android использует [gson][3] в фоновом режиме для сериализации объектов Java в данные JSON перед отправкой данных в службу приложений Azure.  В следующем коде используется метод **setFieldNamingStrategy()** для задания стратегии. Этот пример удалит начальный знак ("m"), а затем преобразует следующий знак в нижний регистр для каждого имени поля. Например, он преобразует "mId" в "id".  Реализуйте стратегию преобразования, чтобы снизить потребность в аннотациях `SerializedName()` в большинстве полей.
...
[Приступая к работе с аутентификацией в мобильных службах]: app-service-mobile-android-get-started-users.md
