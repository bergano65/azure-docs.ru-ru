...
> [!NOTE]
> Центр приложений Visual Studio вкладывает средства в новые и интегрированной службы, необходимые для разработки мобильных приложений. Разработчики могут использовать **построения**, **теста** и **распределить** служб для настройки конвейера непрерывной интеграции и доставки. После развертывания приложения, разработчики могут отслеживать состояние и использования их приложений с помощью **Analytics** и **диагностики** служб и общайтесь с пользователями, с помощью **Push** Служба. Разработчики также могут использовать **Auth** подлинность пользователей и **данных** службы для сохранения и синхронизировать данные приложения в облаке. Ознакомьтесь с [App Center](https://appcenter.ms/?utm_source=zumo&utm_campaign=app-service-mobile-cordova-get-started-push) уже сегодня.
>

## <a name="overview"></a>Обзор

В этом руководстве вы Добавление Push-уведомлений для [краткое руководство по Apache Cordova][5] проекта, чтобы на устройство отправляется Push-уведомление каждый раз при вставке записи.

Если вы не используете скачанный ознакомительный проект сервера, необходимо добавить пакет расширений для push-уведомлений. Дополнительные сведения см. в разделе [работа с внутреннего сервера .NET SDK для мобильных приложений][1].
...
* [Инструменты Visual Studio для Apache Cordova][4]
* [Активная учетная запись Azure][3]
* Завершенный [краткое руководство по Apache Cordova][5] проекта
* (Android) Объект [учетная запись Google][6] с проверенным электронным адресом
* (iOS) [Участие в программе разработки Apple][7] и устройство iOS (iOS, симулятор не поддерживает Push-уведомления)
* (Windows) Объект [учетную запись разработчика Microsoft Store][8] и устройство Windows 10

## <a name="configure-hub"></a>Настройка концентратора уведомлений

[!INCLUDE [app-service-mobile-configure-notification-hub](../../includes/app-service-mobile-configure-notification-hub.md)]

[Просмотрите видео, демонстрирующее шаги в этом разделе][9].
...
Приложения Apache Cordova не обрабатывают возможностей устройства или сети изначально.  Эти возможности обеспечиваются подключаемыми модулями, которые публикуются в [npm][10] или на сайте GitHub. Подключаемый модуль `phonegap-plugin-push` обрабатывает push-уведомления сети.
...
2. Добавьте новый метод **registerForPushNotifications** , как показано ниже.
    ...
    ```
3. (Android) В приведенном выше коде замените `Your_Project_ID` числовым Идентификатором проекта приложения из [Google Developer Console][18].
...
В приложении Cordova откройте файл **config.xml**. Затем замените `Your_Project_ID` числовым Идентификатором проекта приложения из [Google Developer Console][18].
...
* *На физическом устройстве*. Подключите устройство Android к компьютеру разработчика с помощью кабеля USB.  Вместо **Эмулятор Google Android** выберите **Устройство**. Visual Studio развернет приложение на устройстве и запустит его. После этого вы сможете работать с приложением на устройстве.

  Общий экран приложения, такие как [Mobizen][20] может помочь в разработке приложений Android. Приложение Mobizen транслирует экран Android в веб-браузер на компьютере.

* *В эмуляторе Android*. Для использования эмулятора необходимо выполнить дополнительную настройку.
    ...
    Если вы хотите использовать быстрее x86 эмулятор, [установите драйвер HAXM][11], а затем настройте его использование в эмуляторе.
...
Прежде, чем можно запускать приложение Cordova в iOS с помощью Visual Studio, выполните действия, описанные в [руководства по установке iOS][12] установить и запустить агент удаленной сборки.

Убедитесь, что вы можете создать приложение для iOS. Приведенные в этом руководстве шаги по установке необходимы для сборки приложения для iOS в Visual Studio. Если у вас нет компьютера Mac, можно использовать агент удаленной сборки в такой службе, как MacInCloud. Дополнительные сведения см. в разделе [запуска приложения iOS в облаке][21].
...
[Видео, демонстрирующее аналогичные действия][13]
...
* Если вы еще не сделано, продолжения работы с учебником по [Добавление проверки подлинности][14] в приложение Apache Cordova.

Сведения об использовании следующих пакетов SDK:

* [Пакет SDK для Apache Cordova][15]
* [Серверный пакет SDK для ASP.NET][1]
* [Серверный пакет SDK для Node.js][16]
