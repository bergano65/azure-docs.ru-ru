...
Если вы не используете скачанный ознакомительный проект сервера, необходимо добавить пакет расширений для push-уведомлений. Дополнительные сведения см. [в разделе Работа с пакетом SDK серверной части .NET для мобильных приложений][1].
...
* Готовый [Apache Cordova][5] проект быстрого запуска
* Android [Учетная запись Google][6] с проверенным адресом электронной почты
* iOS [Членство в программе для разработчиков Apple][7] и устройство iOS (имитатор iOS не поддерживает Push-уведомления)
* Windows [Учетная запись разработчика Microsoft Store][8] и устройство Windows 10

## <a name="configure-hub"></a>Настройка концентратора уведомлений

[!INCLUDE [app-service-mobile-configure-notification-hub](../../includes/app-service-mobile-configure-notification-hub.md)]

[Просмотрите видео, показывающие действия, описанные в этом разделе][9].
...
2. Добавьте новый метод **registerForPushNotifications** , как показано ниже.
    ...
    ```
3. Android В приведенном выше коде замените `Your_Project_ID` на числовой идентификатор проекта для приложения из [консоли Google Developer][18].
...
В приложении Cordova откройте файл **config.xml**. Затем замените `Your_Project_ID` числовым ИДЕНТИФИКАТОРом проекта приложения из [консоли Google Developer][18].
...
* *На физическом устройстве*. Подключите устройство Android к компьютеру разработчика с помощью кабеля USB.  Вместо **Эмулятор Google Android** выберите **Устройство**. Visual Studio развернет приложение на устройстве и запустит его. После этого вы сможете работать с приложением на устройстве.
  ...
  Такие приложения, как [Mobizen][20] , могут помочь в разработке приложений Android. Приложение Mobizen транслирует экран Android в веб-браузер на компьютере.
...
* *В эмуляторе Android*. Для использования эмулятора необходимо выполнить дополнительную настройку.
    ...
    Если вы хотите использовать более быстрый эмулятор x86, [установите драйвер HAXM][11], а затем настройте эмулятор для его использования.
...
Прежде чем запускать приложение Cordova в iOS с помощью Visual Studio, выполните действия, описанные в [руководстве по установке iOS][12] , чтобы установить и запустить агент удаленной сборки.

Убедитесь, что вы можете создать приложение для iOS. Приведенные в этом руководстве шаги по установке необходимы для сборки приложения для iOS в Visual Studio. Если у вас нет компьютера Mac, можно использовать агент удаленной сборки в такой службе, как MacInCloud. Дополнительные сведения см. [в статье запуск приложения iOS в облаке][21].
