---
title: Добавление push-уведомлений в приложение Apache Cordova
...
* [Инструменты визуальной студии для Apache Cordova][4]
...
[Смотреть видео, показывающее шаги в этом разделе][9].

## <a name="update-the-server-project"></a>Обновление серверного проекта

[!INCLUDE [app-service-mobile-update-server-project-for-push-template](../../includes/app-service-mobile-update-server-project-for-push-template.md)]

## <a name="modify-your-cordova-app"></a><a name="add-push-to-app"></a>Изменение приложения Cordova
...
1. В обозревателе решений откройте файл `config.xml`. Далее выберите **Plugins** > **Custom**. Используйте **Git** в качестве источника установки.
...
Выполните те же действия, что и для установки подключаемого модуля push-уведомлений. Добавьте подключаемый модуль устройства из списка основных подключаемых модулей. (Чтобы найти его, выберите **Плагины** > **Core**.) Вам нужен этот плагин, чтобы получить название платформы.
...
2. Добавьте новый метод **registerForPushNotifications** , как показано ниже.
    ...
    ```
3. (Android) В предыдущем коде `Your_Project_ID` замените идентификатор численного проекта для вашего приложения с [консоли Google Developer.][18]
...
#### <a name="enable-firebase-cloud-messaging"></a><a name="enable-gcm"></a>Включение Firebase Cloud Messaging
...
#### <a name="configure-the-mobile-app-back-end-to-send-push-requests-using-fcm"></a><a name="configure-backend"></a>Настройка серверной части мобильного приложения для отправки push-запросов с использованием FCM
...
#### <a name="configure-your-android-device-for-usb-debugging"></a><a name="configure-device"></a>Настройка устройства Android для отладочного режима USB
...
1. Перейти к **настройкам** > **о телефоне**. Коснитесь пункта **Build number** (Номер сборки) и дождитесь включения режима разработчика (примерно семь секунд).
2. Вернуться в **параметры настройки** > **разработчика,** включить **USB отладки**. Затем подключите телефон Android к компьютеру для разработки с помощью кабеля USB.
...
1. В Visual Studio выберите **Инструменты** > **Android** > **Android SDK Manager.** Разверните папку **Extras** (Дополнительно). Установите флажки напротив следующих пакетов SDK.
...
* *На эмуляторе Android.* Для использования эмулятора необходимо выполнить дополнительную настройку.
    ...
    Добавьте учетную запись Google на устройство Android, выбрав**учетную запись** **Apps** > **Настройки Добавление.** >  Затем следуйте инструкциям на экране.
...
Прежде чем вы сможете запустить приложение Cordova на iOS с помощью Visual Studio, пройдите через шаги в руководстве по [настройке iOS][12] для установки и запуска агента удаленной сборки.
...
* Если вы еще не сделали этого, продолжайте учебник, [добавляя аутентификацию][14] в приложение Apache Cordova.
