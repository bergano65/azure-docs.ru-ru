...
1. Перейдите к [портал Azure](https://portal.azure.com/) и перейдите к созданному вами мобильному приложению. В колонке `Overview` найдите URL-адрес, который является общедоступной конечной точкой для мобильного приложения. Пример. имя сайта для имени приложения "test123" будет https://test123.azurewebsites.net.

2. Откройте файл `Constants.cs` в этой папке — Xamarin. Forms/ЗУМОАППНАМЕ. Имя приложения — `ZUMOAPPNAME`.

3. В `Constants.cs` классе Замените переменную `ZUMOAPPURL` на общую конечную точку выше.
