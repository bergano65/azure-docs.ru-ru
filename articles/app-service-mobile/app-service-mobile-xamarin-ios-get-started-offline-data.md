---
title: Включить синхронизацию в автономном режиме (Xamarin iOS)
description: Узнайте, как использовать мобильное приложение App Service для кэша и синхронизации автономных данных в приложении Xamarin iOS.
...
3. Повторно выполните сборку и запустите клиентское приложение. Приложение работает так же, как и до включения синхронизации в автономном режиме. Однако локальная база данных теперь заполнена данными, которые могут быть использованы в автономном сценарии.
...
5. (Необязательно.) Если на компьютере установлена среда Visual Studio, откройте **обозреватель серверов**. Перейдите к базе данных в **базах**-> данных Azure**S'L.** Щелкните правой кнопкой мыши базу данных и выберите пункт **Открыть в обозревателе объектов SQL Server**. Теперь можно перейти к таблице базы данных SQL и ее содержимому. Убедитесь, что данные в серверной базе данных не изменились.
