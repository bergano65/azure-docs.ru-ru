---
title: Работа с пакетом SDK для внутреннего сервера .NET
...
Справочная документация по пакету SDK для сервера находится здесь: [Справочник по .NET для мобильных приложений Azure][1].
...
4. Введите имя проекта. Нажмите кнопку **ОК**.
5. В разделе *Шаблоны ASP.NET 4.5.2*выберите **Мобильное приложение Azure**. Установите флажок **Разместить в облаке** , чтобы создать мобильный внутренний сервер в облаке, в котором можно опубликовать этот проект.
6. Последовательно выберите **ОК**.

## <a name="install-sdk"></a>Практическое руководство. Скачивание и инициализация пакета SDK
Пакет SDK доступен в [NuGet.org]. Этот пакет включает базовые функции, необходимые для начала работы с пакетом SDK. Для инициализации пакета SDK необходимо выполнить действия с объектом **HttpConfiguration** .
...
Используйте ключ подписывания для параметра *параметра authsigningkey нужно* в локальной конфигурации приложения.  Теперь ваш мобильный сервер поддерживает проверку маркеров при локальном запуске, который клиент получает маркер из конечной точки на основе облака.
