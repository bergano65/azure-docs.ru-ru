...
> [!TIP]
> [Пакет SDK для сервера .NET для мобильных приложений Azure][2] — это открытый исходный код на сайте GitHub. Репозиторий содержит весь исходный код, а также полный набор модульных тестов пакета SDK для сервера и некоторые примеры проектов.
>
>
...
Чтобы создать мобильную серверную часть службы приложений, выполните инструкции из [краткого руководства][3] или выполните следующие действия.
...
4. Введите имя проекта. Затем нажмите кнопку **ОК**.
...
Установите [пакет Azure SDK для .NET][4] (версия 2.9.0 или более поздней), чтобы создать проект мобильных приложений Azure в Visual Studio. После установки пакета SDK создайте приложение ASP.NET, сделав следующее:
...
## <a name="install-sdk"></a>Руководство. Скачивание и инициализация пакета SDK
...
* [Microsoft.Azure.Mobile.Server.Quickstart] поддерживает базовую настройку мобильных приложений. Чтобы добавить его в конфигурацию, во время инициализации необходимо вызвать метод расширения **UseDefaultConfiguration**. Это расширение включает в себя следующие модули: пакеты Notifications, Authentication, Entity, Tables, Cross-domain и Home. Этот пакет используется в кратком руководстве по созданию мобильных приложений, доступном на портале Azure.
...
## <a name="define-table-controller"></a>Руководство. Определение контроллера таблиц
...
### <a name="adjust-pagesize"></a>Руководство. Изменение размера постраничного представления таблиц
...
## <a name="how-to-work-with-authentication"></a>Практическое руководство. Использование аутентификации
...
* [Руководство. Добавление аутентификации в серверный проект](#add-auth)
* [Руководство. Использование пользовательской аутентификации для приложения](#custom-auth)
* [Руководство. Получение сведений о пользователе, прошедшем аутентификацию](#user-info)
* [Руководство. Ограничение доступа к данным для полномочных пользователей](#authorize)

### <a name="add-auth"></a>Руководство. Добавление аутентификации в серверный проект
...
### <a name="custom-auth"></a>Руководство. Использование пользовательской аутентификации для приложения
> [!IMPORTANT]
...
### <a name="user-info"></a>Руководство. Получение сведений о пользователе, прошедшем аутентификацию
...
### <a name="authorize"></a>Руководство. Ограничение доступа к данным для полномочных пользователей
...
## <a name="tags"></a>Руководство. Включение принудительной отправки push-уведомлений с использованием тегов
...
Пример см. в разделе примеры [добавленных клиентом тегов push-уведомлений][5] .

## <a name="push-user"></a>Практическое руководство. Отправка push-уведомлений пользователям, прошедшим аутентификацию
...
При регистрации для работы с push-уведомлениями на клиенте, прошедшем проверку подлинности, убедитесь, что проверка подлинности завершена, и только после этого начинайте регистрацию. Дополнительные сведения см. в разделе [Push to Users][6] статьи краткое руководство по выполнению мобильных приложений службы приложений для серверной части .NET.

## <a name="how-to-debug-and-troubleshoot-the-net-server-sdk"></a>Практическое руководство. Отладка и устранение неполадок с пакетом SDK для сервера .NET
...
1. Выполните действия, описанные в разделе [Включение ведения журнала приложений (Windows)](../app-service/troubleshoot-diagnostic-logs.md#enable-application-logging-windows).
...
5. Скачайте и оцените журналы, как описано в разделе [доступ к файлам журнала](../app-service/troubleshoot-diagnostic-logs.md#access-log-files).
