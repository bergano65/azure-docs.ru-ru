---
title: Работа с сервером резервного копирования .NET SDK
...
## <a name="how-to-create-a-net-mobile-app-backend"></a><a name="create-app"></a>Практическое руководство. Создание серверной части мобильного приложения .NET
...
4. Введите имя проекта. После этого щелкните **OK**.
5. Выберите **Мобильное приложение Azure** из списка шаблонов.
6. Нажмите **OK,** чтобы создать решение.
...
1. Откройте диалоговое окно **Новый проект** (последовательно выберите **Файл** > **Создать** > **Проект**).
2. Расширьте **шаблоны** > **Visual C и**выберите **Веб**.
...
## <a name="how-to-download-and-initialize-the-sdk"></a><a name="install-sdk"></a>Практическое руководство. Скачивание и инициализация пакета SDK
SDK доступен на [NuGet.org]. Этот пакет включает в себя базовую функциональность, необходимую для начала работы с помощью SDK. Для инициализации пакета SDK необходимо выполнить действия с объектом **HttpConfiguration** .
...
### <a name="initialize-the-server-project"></a><a name="server-project-setup"></a> Инициализация серверного проекта
Инициализацию проекта внутреннего сервера .NET можно выполнить аналогично инициализации проектов ASP.NET — включив класс запуска OWIN. Убедитесь, что указана ссылка на пакет NuGet `Microsoft.Owin.Host.SystemWeb`. Чтобы добавить этот класс в Visual Studio, нажмите правой кнопкой мыши на вашем сервере проекта и выберите **Добавить** >
**новый пункт**, то **Web** > **General** > **OWIN Запуск класса.**  Будет создан класс с указанным ниже атрибутом.
...
* [Microsoft.Azure.Mobile.Server.Home](https://www.nuget.org/packages/Microsoft.Azure.Mobile.Server.Home/) реализует стандартную *страницу «Это мобильное приложение запущено и работает»* для корневого каталога веб-сайта. Чтобы добавить его в конфигурацию, вызовите метод расширения **AddMobileAppHomeController** .
* [Microsoft.Azure.Mobile.Server.Tables](https://www.nuget.org/packages/Microsoft.Azure.Mobile.Server.Tables/) содержит классы для работы с данными и настраивает конвейер данных. Чтобы добавить его в конфигурацию, вызовите метод расширения **AddTables** .
* [Microsoft.Azure.Mobile.Server.Entity](https://www.nuget.org/packages/Microsoft.Azure.Mobile.Server.Entity/) позволяет платформе Entity Framework получать доступ к данным в базе данных SQL. Чтобы добавить его в конфигурацию, вызовите метод расширения **AddTablesWithEntityFramework** .
...
* [Microsoft.Azure.Mobile.Server.CrossDomain](https://www.nuget.org/packages/Microsoft.Azure.Mobile.Server.CrossDomain/) создает контроллер, который передает устаревшим веб-браузерам данные из вашего мобильного приложения. Чтобы добавить его в конфигурацию, вызовите метод расширения **MapLegacyCrossDomainController** .
...
## <a name="how-to-publish-the-server-project"></a><a name="publish-server-project"></a>Практическое руководство. Публикация серверного проекта
...
3. Если попросят выбрать цель публикации, нажмите **Microsoft Azure App Service** > **Next,** затем (при необходимости) вопийте с учетными данными Azure.
...
## <a name="how-to-define-a-table-controller"></a><a name="define-table-controller"></a> Практическое руководство. Определение контроллера таблиц
...
Объект передачи данных — это обычный объект C#, наследуемый от `EntityData`.  Пример:
...
1. Нажмите правой кнопкой мыши на папку контроллеров и выберите **Добавить** > **контроллер ...**.
...
### <a name="how-to-adjust-the-table-paging-size"></a><a name="adjust-pagesize"></a>Практическое руководство: изменение размера постраничного представления таблиц
...
1. В Visual Studio, правой кнопкой кнопку контроллеров папку, а затем нажмите **Добавить** > **контроллер**, выберите **Web API 2&mdash;Контроллер Пустой** и нажмите **Добавить**.
...
* [Как получить: Получить информацию о достоверной проверке пользователя](#user-info)
* [Как: Ограничить доступ к данным для авторизованных пользователей](#authorize)

### <a name="how-to-add-authentication-to-a-server-project"></a><a name="add-auth"></a>Практическое руководство. Добавление аутентификации в серверный проект
...
### <a name="how-to-use-custom-authentication-for-your-application"></a><a name="custom-auth"></a>Практическое руководство. Использование пользовательской проверки подлинности для приложения
> [!IMPORTANT]
...
### <a name="how-to-retrieve-authenticated-user-information"></a><a name="user-info"></a>Практическое руководство. Получение сведений о пользователе, прошедшем проверку подлинности
...
### <a name="how-to-restrict-data-access-for-authorized-users"></a><a name="authorize"></a>Практическое руководство. Ограничение доступа к данным для авторизованных пользователей
...
## <a name="how-to-enable-targeted-push-using-tags"></a><a name="tags"></a>Практическое руководство. Включение принудительной отправки push-уведомлений с использованием тегов
...
## <a name="how-to-send-push-notifications-to-an-authenticated-user"></a><a name="push-user"></a>Практическое руководство. Отправка push-уведомлений аутентифицированному пользователю
...
* [Включить диагностическую регистрацию в службе приложений Azure](../app-service/troubleshoot-diagnostic-logs.md)
...
1. Выполните последующие действия в [журнале включить приложение (Windows)](../app-service/troubleshoot-diagnostic-logs.md#enable-application-logging-windows).
...
5. Скачать и оценить журналы, как описано в [файлах журналов access.](../app-service/troubleshoot-diagnostic-logs.md#access-log-files)

### <a name="local-debugging-with-authentication"></a><a name="local-debug"></a>Локальная отладка с проверкой подлинности
...
Используйте ключ подписи для параметра *authSigningKey* в локальной конфигурации приложения.  Ваш мобильный бэкэнд теперь оборудован для проверки токенов при локальной работе, который клиент получает токен из облачной точки.
