...
1. В окне **Отправка приглашения по электронной почте** введите необязательное сообщение и нажмите кнопку **Отправить**. 
    ...
    В столбце **приглашения** списка **пользователей** отображается состояние приглашения для каждого добавленного пользователя. Состояние должно измениться на " **Отправка** ", а затем на " ** \<Дата отправки">**. 
...
1. В области **Пользователи** выберите **Квота на пользователя: \<число> часов** на панели инструментов. 
1. В окне **Квота на пользователя** укажите количество часов, которое должно быть присвоено каждому учащему за пределами запланированного времени класса, а затем нажмите кнопку **сохранить**.
    ...
    Измененные значения теперь отображаются в поле **Квота на пользователя: \<количество часов>** на панели инструментов и в списке Пользователи, как показано ниже:
...
1. В поле **корректировка квоты \<для выбранного пользователя или адреса электронной почты пользователей>** введите число дополнительных часов лаборатории, которые нужно предоставить выбранному учащимся или учащимся, а затем нажмите кнопку **Применить**. 
...
- Учетная запись учащегося, предоставляемая Azure Active Directoryным экземпляром университета для Office 365. 
...
> [!IMPORTANT]
> Когда студенты входят в лабораторную среду, им не предоставляется возможность создания учетная запись Майкрософт. По этой причине мы рекомендуем включить эту ссылку для регистрации, http://signup.live.comв сообщении электронной почты с регистрацией лаборатории, которое вы отправляете учащимся, использующим учетные записи сторонних производителей.
