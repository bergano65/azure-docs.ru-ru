...
В классе этичного взлома учащиеся могут изучать современные методы защиты от уязвимостей. Каждый учащийся получает виртуальную машину Windows Server с двумя вложенными виртуальными машинами: одну виртуальную машину с образом [Metasploitable3](https://github.com/rapid7/metasploitable3) и другой компьютер с образом [Kali Linux](https://www.kali.org/) . Виртуальная машина Metasploitable используется для изучения проникновений, а виртуальная машина Kali предоставляет доступ к средствам, необходимым для выполнения задач анализа.
...
Сценарий, который автоматизирует задачи, описанные выше, доступен в [сценариях служб лаборатории этического взлома](https://github.com/Azure/azure-devtestlab/tree/master/samples/ClassroomLabs/Scripts/EthicalHacking).
...
1. Загрузка образа из [https://www.offensive-security.com/kali-linux-vm-vmware-virtualbox-image-download/](https://www.offensive-security.com/kali-linux-vm-vmware-virtualbox-image-download/).  
...
1. В **диспетчере Hyper-V**выберите **действие** -> **начать**, а затем выберите **действие** -> **Подключиться** для подключения к виртуальной машине.  
...
1. Создайте новую виртуальную машину Hyper-V.
    ...
    1. Выберите **действие** -> **новую** **виртуальную машину** -> .
    ...
    1. Теперь образ Метасплоитабле готов к использованию. В **диспетчере Hyper-V**выберите **действие** -> **начать**, а затем выберите **действие** -> **Подключиться** для подключения к виртуальной машине.  Имя пользователя по умолчанию — **мсфадмин** , а пароль — **мсфадмин**. 
