...
4. На странице **выбора характеристик виртуальных машин** выполните следующие действия:
    1. Выберите **размер** для виртуальных машин, созданных в лаборатории. В настоящее время **небольшой**, **Средний**, **Средний (виртуализации)** , **больших**, и **GPU** размеры: разрешено. Дополнительные сведения см. в разделе [размеры виртуальных Машин](#vm-sizes) раздел.
...
5. На странице **Задание учетных данных** укажите учетные данные по умолчанию для всех виртуальных машин в лаборатории. 
    ...
    3. Отключить **использовать тот же пароль для всех виртуальных машин** параметр, если вы хотите учащихся, чтобы задать свои пароли. Этот шаг **не является обязательным**. 

        Преподаватель можно использовать тот же пароль для всех виртуальных машин в лаборатории или позволить учащимся возможность задавать пароли для своих виртуальных машин. По умолчанию этот параметр включен для всех Windows и Linux образов, за исключением Ubuntu. При выборе **Ubuntu** виртуальной Машины, этот параметр отключен, учащиеся будет предложено установить пароль при входе в первый раз.
...
8. В рамках этого руководства можно выполнить следующие необязательные действия: 
    ...
    1. Выберите **сброс пароля** для сброса пароля для виртуальной Машины. 
...
| ---- | ----- | --- | ----------- | 
| Небольшой | 2 | 3,5 ГБ | Этот размер подходит для командной строки, открыв веб-браузер, низким уровнем трафика веб-серверами, малых и средних баз данных. |
| Средние | 4\. | 7 ГБ | Этот размер подходит для реляционных баз данных, кэширование в памяти и аналитика | 
| Средний (Nested virtualization) | 4\. | 16 ГБ | Этот размер подходит для реляционных баз данных, кэширование в памяти и аналитики. Этот размер также поддерживает вложенную виртуализацию. <p>Этот размер может использоваться в сценариях, где каждый учащийся требуется несколько виртуальных машин. Преподаватели можно использовать вложенную виртуализацию для настройки нескольких виртуальных машин небольшого размера вложенные внутри виртуальной машины. </p> |
| Большой | 8 | 32 ГБ | Этот размер подходит для приложений, которым требуются более быстрые процессоры, более производительные локальные диски, большие базы данных, кэшей большого объема памяти. Этот размер также поддерживает вложенной виртуализации |  
| Графический процессор | 12 | 112 ГБ | Этот размер подходит для рабочих нагрузок вычислений, графической обработки и визуализаций | 
