...
- **Управляемость** вс-президентов, артефактов, формул, конфигурации сети, разрешений, политик и т.д. проще, так как все обновления требуются только в одной подписке, а не для обновления во многих подписках.
...
- **Владелец подписки** - Владелец подписки имеет права на администрирование подписки Azure, включая назначение пользователей, управление политиками, создание & управление топологией сетей, запрос увеличения квот и т.д. Для получения дополнительной информации смотрите [эту статью](../role-based-access-control/rbac-and-directory-admin-roles.md).
