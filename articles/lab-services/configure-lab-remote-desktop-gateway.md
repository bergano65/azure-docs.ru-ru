...
- Сертификат TLS/SSL должен быть установлен на шлюзе для обработки трафика HTTPS. Сертификат должен соответствовать полностью квалифицированного доменного имени (ФЗДН) балансопарка нагрузки для фермы шлюзов или ФЗН самой машины, если есть только одна машина. Сертификаты Wild-card TLS/SSL не работают.  
...
- DNS для F-DN, связанный с сертификатом TLS/SSL, установленным на шлюзовных машинах, должен направлять трафик на шлюзную машину или балансоровую нагрузку на ферму шлюзов.
...
1. В разделе **Удаленного рабочего стола** введите полностью квалифицированное доменное имя (ФЗДН) или IP-адрес шлюза удаленного рабочего стола или фермы для поля **хоста Gateway.** Это значение должно соответствовать F-DN сертификата TLS/SSL, используемого на шлюзовых машинах.
...
2. Получите сертификат TLS/SSL. F-DN, связанный с сертификатом TLS/SSL, должен быть для домена, который вы контролируете. Сохраните отпечаток пальца, пароль и кодирование Base64 для этого сертификата. Чтобы получить отпечаток пальца с помощью PowerShell, используйте следующие команды.
...
4. Развертывание **azuredeploy.json** от [https://github.com/Azure/azure-devtestlab/tree/master/samples/DevTestLabs/GatewaySample/arm/gateway](https://github.com/Azure/azure-devtestlab/tree/master/samples/DevTestLabs/GatewaySample/arm/gateway). Шаблон принимает следующие параметры:
    ...
    - sslCertificate - Кодирование сертификата TLS/SSL для шлюза Base64.
    - sslCertificatePassword - Пароль сертификата TLS/SSL для шлюза.
    - sslCertificateThumbprint - Отпечаток пальца сертификата для идентификации в местном магазине сертификатов TLS/SSL.
...
5. Назначьте DNS таким образом, чтобы F-DN сертификата TLS/SSL направлялся на IP-адрес gatewayIP с предыдущего шага.
