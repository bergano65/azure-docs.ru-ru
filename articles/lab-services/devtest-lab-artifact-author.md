...
## <a name="secrets-as-secure-strings"></a>Секреты как безопасные строки
Объявить секреты как безопасные строки. Вот синтаксис для объявления безопасного `parameters` параметра строки в разделе файла **artifactfile.json:**
...
Для команды установки артефакта запустите скрипт PowerShell, который использует безопасную строку, созданную с помощью команды ConvertTo-SecureString. 
...
Для полного примера artifactfile.json и artifact.ps1 (скрипт PowerShell) [см. этот пример на GitHub.](https://github.com/Azure/azure-devtestlab/tree/master/Artifacts/windows-test-paramtypes)

Еще один важный момент, чтобы отметить, не регистрировать секреты консоли, как выход захвачен для отладки пользователя. 
...
1. Установите редактор JSON — он требуется для работы с файлами определения артефактов. Мы рекомендуем использовать [Visual Studio Code,](https://code.visualstudio.com/)который доступен для Windows, Linux и OS X.
