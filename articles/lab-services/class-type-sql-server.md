...
В этой статье мы покажем, как настроить шаблон виртуальной машины в лаборатории с помощью [Visual Studio 2019](https://visualstudio.microsoft.com/vs/), [SQL Server Management Studio](/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-ver15)и [Azure Data Studio](https://github.com/microsoft/azuredatastudio).  Для этого лабораторного занятия мы будем использовать одну общую [базу данных SQL Server](../azure-sql/database/sql-database-paas-overview.md) для всей лаборатории. [База данных SQL Azure](../azure-sql/database/sql-database-paas-overview.md) — это платформа как услуга (PaaS), ядро СУБД предложение из Azure.
...
12. При необходимости измените параметры базы данных для класса.  Можно выбрать один из вариантов: подготовленный и бессерверный.  В этом примере мы будем использовать автомасштабируемый режим без сервера с параметром max виртуальных ядер из 4, min виртуальных ядер, равного 1. Параметр приостановки будет включен в минимальное значение, равное 1 часу. Щелкните **Применить**.
...
[SQL Server Management Studio (SSMS)](/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-ver15) — это интегрированная среда для управления любой инфраструктурой SQL.  SSMS — это средство, используемое администраторами баз данных для развертывания, мониторинга и обновления инфраструктуры данных.
