...
description: В этой статье содержатся ответы на некоторые часто задаваемые вопросы (часто задаваемые вопросы) о Лабораториях Azure DevTest Labs.
...
Наш блог DevTest Labs Team был удален с 20 марта 2019 года. 

### <a name="where-can-i-track-feature-updates-from-now-on"></a>Где я могу отслеживать обновления функций отныне?
Отныне мы будем публиковать обновления функций и информационные сообщения в блогах ВАЗа и обновления Azure. Эти сообщения в блоге также ссылку на нашу документацию, где это необходимо.

Подпишитесь на [блог DevTest Labs Azure](https://azure.microsoft.com/blog/tag/azure-devtest-labs/) и [обновления DevTest Labs Azure,](https://azure.microsoft.com/updates/?product=devtest-lab) чтобы быть в курсе новых функций в DevTest Labs.

### <a name="what-happens-to-the-existing-blog-posts"></a>Что происходит с существующими блога?
В настоящее время мы работаем над миграцией существующих сообщений в блоге (за исключением обновлений сего) к нашей [документации DevTest Labs.](devtest-lab-overview.md) Когда блог MSDN будет универслен, он будет перенаправлен на обзор документации для DevTest Labs. После перенаправки можно найти статью, которую вы ищете в заголовке «Фильтр». Мы не мигрировали все должности еще, но должно быть сделано к концу этого месяца. 


### <a name="where-do-i-see-outage-updates"></a>Где можно увидеть обновления сотов?
Мы будем размещать обновления сбоя с помощью нашей ручки Twitter с этого момента. Следуйте за нами на Twitter, чтобы получить последние обновления о перебоях и известных ошибок.

### <a name="twitter"></a>Twitter
Наша ручка Twitter:[@azlabservices](https://twitter.com/azlabservices)
...
Учетная запись Майкрософт используется практически для всех действий с устройствами и службами Майкрософт. Это адрес электронной почты и пароль, которые вы используете для вхождения в Skype, Outlook.com, OneDrive, Windows phone, Azure и Xbox Live. что означает, что ваши файлы, фотографии, контакты и параметры доступны на любом вашем устройстве.
...
Доступ к DevTest Labs определяется службой управления доступом на основе ролей (RBAC). Чтобы понять особенности доступа, нужно узнать разницу между разрешением, ролью и областью в соответствии с RBAC.
...
- **Пользователь лаборатории.** Эта роль позволяет просматривать все ресурсы лаборатории, такие как виртуальные машины, политики и виртуальные сети. Однако лабораторный пользователь не может изменять политики или любые ввосажие-
...
Центральные ИТ должны владеть только тем, что необходимо, и позволять группам проектов и приложений иметь необходимый уровень контроля. Как правило, это означает, что центральный ИТ-отдел владеет подпиской и обрабатывает основные ИТ-функции, такие как сетевые конфигурации. Набор **владельцев** подписки должен быть небольшим. Эти владельцы могут назначить дополнительных владельцев, когда есть необходимость, или применить политики уровня подписки, например, "Нет общественной ИС".
...
Ресурс DevTest Labs должен принадлежать владельцам, которые связаны с командой проекта или приложения, Это потому, что они понимают свои требования к машинам, и требуется программное обеспечение. В большинстве организаций владельцем этого ресурса DevTest Labs часто является ведущий руководитель разработки или руководитель проекта. Этот владелец может управлять пользователями и политиками в рамках лабораторной среды, а также контролировать все виртуальные машины в среде DevTest Labs.

Члены команды проекта/приложения должны быть добавлены к роли **пользователей DevTest Labs.** Эти пользователи могут создавать виртуальные машины в соответствии с политиками на уровне лаборатории и подписки. Кроме того, они могут управлять собственными виртуальными машинами. Они не могут управлять виртуальными машинами, которые принадлежат другим пользователям.

Для получения дополнительной [Azure enterprise scaffold – prescriptive subscription governance documentation](/azure/architecture/cloud-adoption/appendix/azure-scaffold)информации см.
...
Первый уровень элементов управления для удаленного доступа заключается в применении политики удаленного доступа через VPN-соединение, которое не подключено непосредственно к лаборатории.

Второй уровень элементов управления — это применение набора объектов групповой политики, которые предотвращают копирование и вставку при использовании удаленного рабочего стола. Можно внедрить политику сети, которая запрещает использование служб, например FTP или RDP, для подключения из-за пределов среды. Пользователь-определенный трафик может привести к возвращению всего сетевого трафика Azure к месту, но в сети не может быть учтены все URL-адреса, которые могут позволить загружать данные, если они не будут контролироваться через прокси для сканирования содержимого и сеансов. В виртуальной сети, используемой в DevTest Labs, необходимо запретить общедоступные IP-адреса, чтобы предотвратить подключения типа "мост" к внешним сетевым ресурсам.

В конечном счете, один и тот же тип ограничений должен применяться в организации, что будет учитывать все возможные методы съемных носителей или внешних URL-адресов, которые могли бы принимать публикацию контента. Для проверки и внедрения политики безопасности необходимо привлечь соответствующего специалиста. Дополнительные рекомендации см. на веб-сайте [Microsoft Cyber Security](https://www.microsoft.com/security/default.aspx?&WT.srch=1&wt.mc_id=AID623240_SEM_sNYnsZDs) (Кибер-безопасность Майкрософт).
...
[В GitHub доступен репозиторий шаблонов Azure Resource Manager Azure для лабораторий](https://azure.microsoft.com/resources/templates/101-dtl-create-lab), которые можно развернуть "как есть" или изменить, чтобы создать пользовательские шаблоны для своих лабораторий. Каждый шаблон имеет ссылку для развертывания лаборатории, как это в вашей собственной подписке Azure. Вы также можете настроить шаблон и [развернуть его с помощью PowerShell или Azure CLI](../azure-resource-manager/templates/deploy-powershell.md).


### <a name="can-i-have-all-virtual-machines-to-be-created-in-a-common-resource-group-instead-having-each-machine-in-its-own-resource-group"></a>Могу ли я иметь все виртуальные машины, которые будут созданы в общей группе ресурсов, а не иметь каждую машину в своей собственной группе ресурсов?
Да, как владелец лаборатории, вы можете либо позволить лаборатории обрабатывать распределение группы ресурсов для вас, либо все виртуальные машины созданы в общей группе ресурсов, которые вы указываете.

Сценарий отдельной группы ресурсов:
-   DevTest Labs создает новую группу ресурсов для каждой публичной/ частной виртуальной машины IP, которую вы вращаете вверх
-   DevTest Labs создает группу ресурсов для общих IP-машин, которые принадлежат к одному размеру.

Общий сценарий группы ресурсов:
-   Все виртуальные машины закручены в общей группе ресурсов, указанной вами. Узнайте больше [о распределении групп ресурсов для лаборатории.](https://aka.ms/RGControl)
...
### <a name="how-do-i-determine-the-ratio-of-users-per-lab-and-the-overall-number-of-labs-that-are-needed-across-an-organization"></a>Как определить соотношение пользователей в лаборатории и общее количество лабораторий, необходимых в организации?
...
Мы рекомендуем правильно предоставить разрешения на уровне лаборатории, чтобы только авторизованные пользователи могли удалять ресурсы или изменять политики лаборатории. Разработчиков следует назначать в группу **Пользователи DevTest Labs**. Ведущий разработчик или ответственный за инфраструктуру получает статус **владельца DevTest Labs**. Мы рекомендуем вам иметь только двух владельцев лабораторий. Эту же политику следует применять и к репозиториям кода, чтобы избежать его повреждения. Пользователи лабораторий имеют право на использование ресурсов, но не могут обновлять лабораторные политики. Сведения о наборе ролей и прав в лаборатории для каждой встроенной группы см. в статье [Добавление владельцев и пользователей в Azure DevTest Labs](devtest-lab-add-devtest-user.md).

### <a name="how-do-i-share-a-direct-link-to-my-lab"></a>Как предоставить прямую ссылку на мою лабораторию?

1. На [портале Azure](https://portal.azure.com)перейдите в лабораторию.
2. Копируйте **URL-адрес лаборатории** из браузера, а затем делитесь им с пользователями лаборатории.

> [!NOTE]
> Если лабораторию используют внешние пользователи с учетной записью Майкрософт, не являющиеся участниками экземпляра Active Directory вашей организации, они могут получить сообщение об ошибке при переходе по предоставленной ссылке. В этом случае предложите им выбрать свое имя в правом верхнем углу на портале Azure. А затем из меню Каталог выбрать каталог, в котором расположена лаборатория.

## <a name="virtual-machines"></a>Виртуальные машины

### <a name="why-cant-i-see-vms-on-the-virtual-machines-page-that-i-see-in-devtest-labs"></a>Почему я не могу видеть виртуальные компьютеры на странице Виртуальные машины, которые я вижу в DevTest Labs?
При создании VM в DevTest Labs вам дается разрешение на доступ к VM. Вы можете просматривать VM как на странице лабораторий, так и на странице **Виртуальных Машин.** Пользователи, назначенные на роль **владельца DevTest Labs,** могут видеть все виртуальные компьютеры, созданные в лаборатории, на странице **«Все виртуальные машины»** лаборатории. Тем не менее, пользователям, которые имеют роль **пользователя DevTest Labs,** автоматически не предоставляется доступ к ресурсам VM, созданным другими пользователями. Таким образом, эти виртуальные технологии не отображаются на странице **Виртуальные машины.**
...
- При создании VM можно [создать шаблон управления ресурсами](devtest-lab-add-vm.md#save-azure-resource-manager-template) и [развернуть шаблон управления ресурсами из Windows PowerShell.](../azure-resource-manager/templates/deploy-powershell.md)
- Вы также можете указать несколько экземпляров машины, которые будут созданы во время создания виртуальной машины. Чтобы узнать больше о создании нескольких экземпляров [creating a lab virtual machine](devtest-lab-add-vm.md)виртуальных машин, см.
...
2.  Создайте [пользовательское изображение](devtest-lab-create-template.md) в лаборатории DevTest Labs.
...
Чтобы использовать изображения клиентской ОС Windows (Windows 7 или более поздняя версия) для разработки или тестирования в Azure, выполнить один из следующих шагов:
...
Как владелец лаборатории, вы можете удалить ввожжа из лаборатории на портале Azure. или с помощью скрипта PowerShell. В следующем примере, под **значениями для изменения** комментариев, измените значения параметра. Вы можете получить значения subscriptionId, labResourceGroup и labName из лабораторного панели на портале Azure.
...
Шаблоны менеджера ресурсов развертываются в среде DevTest Labs, используя шаги, упомянутые в [функции «Среда» в статье DevTest Labs.](devtest-lab-test-env.md) Вкратце, вам нужно разместить нужные шаблоны Resource Manager в репозиторий Git (Azure Repos или GitHub) и добавить в лабораторию [частный репозиторий для шаблонов](devtest-lab-test-env.md). Этот сценарий может оказаться нецелесообразным, если вы используете DevTest Labs для размещения машин разработки, но может быть полезен, если вы строите промежуточную среду, которая является репрезентативной для производства.

Также стоит отметить, что количество виртуальных машин в лаборатории или на одного пользователя ограничивает только количество машин, созданных в самой лаборатории, а не каких-либо сред (шаблоны Resource Manager).
...
Смотрите это [видео на изображение завод шаблон](https://sec.ch9.ms/ch9/8e8a/9ea0b8d4-b803-4f23-bca4-4808d9368e8a/dtlimagefactory_mid.mp4). Это достаточно сложный сценарием, и предлагаемые для него скрипты служат только примерами для изучения. Если потребуется внести в них изменения, поддерживайте и обслуживайте специально созданные для вашей среды скрипты.

Подробную информацию о создании фабрики изображений можно узнать [в Azure DevTest Labs.](image-factory-create.md)
...
Для более подробного объяснения [см.](devtest-lab-comparing-vm-base-image-types.md)
...
- Используйте [Azure Storage Explorer](../vs-azure-tools-storage-manage-with-storage-explorer.md). Обозреватель хранилищ — это автономное приложение, работающее на платформе Windows, OSX и Linux.
...
- Установка и настройка приложения может занять много часов, что не является эффективным использованием вычислительного времени для добавления на изображение Azure Marketplace.
...
- Использование пользовательских изображений не следует рассматривать легкомысленно. Они вводят дополнительную сложность, так как теперь вы должны управлять VHD-файлами для базовых изображений. Также вам придется регулярно устанавливать в базовые образы исправления и обновления программного обеспечения. Это могут быть обновления операционной системы и (или) любые обновления конфигурации для основного пакета программного обеспечения.

## <a name="artifacts"></a>Артефакты

### <a name="what-are-artifacts"></a>Что такое артефакты?
Артефакты — это настраиваемые элементы, используемые для развертывания последних обновлений или средств разработки на виртуальной машине. Прикрепите артефакты к виртуальной машине при ее создании. После подготовки виртуальной машины артефакты используются для ее развертывания и настройки. Различные уже существующие артефакты доступны в нашем [общедоступном репозитории GitHub.](https://github.com/Azure/azure-devtestlab/tree/master/Artifacts) Однако вы также можете [создать собственные](devtest-lab-artifact-author.md).
...
- Создайте группу, названную `All DevTest Labs Developers` в Active Directory Azure, которая управляется централизованно. Любой разработчик, который участвует в разработке артефактов, должен входить в эту группу.
...
Если вы используете Azure DevOps, вы можете использовать [расширение DevTest Labs Tasks](https://marketplace.visualstudio.com/items?itemName=ms-azuredevtestlabs.tasks) для автоматизации конвейера выпуска в DevTest Labs. Это расширение можно использовать для выполнения следующих задач:
...
- Удалите VM в конце конвейера выпуска, когда он больше не нужен.
...
Если вам вычат виртуальные компании с существующей инфраструктурой, подумайте об использовании существующей виртуальной сети в среде DevTest Labs. Если вы используете ExpressRoute, вы можете свести к минимуму количество VNets / Subnets, так что вы не фрагментировать пространство IP-адреса, которое получает назначен для использования в подписках.

Рассмотрите возможность использования шаблона вглядывания VNet здесь[(модель Hub-Spoke)](/azure/architecture/reference-architectures/hybrid-networking/hub-spoke)тоже. Такой подход обеспечивает связь между подписками vnet/subnet. В противном случае каждой среде DevTest Labs потребуется собственная виртуальная сеть.
...
- **Входящий трафик** - Если виртуальная машина не имеет общедоступный IP-адрес, то она не может быть достигнута в Интернете. Общий подход заключается в обеспечении того, чтобы политика уровня подписки была установлена, таким образом, чтобы ни один пользователь не мог создать общедоступный IP-адрес.
- **Исходящий трафик** - Если вы хотите предотвратить виртуальные машины доступа к общественному интернету напрямую и заставить трафик через корпоративный брандмауэр, то вы можете маршрут трафика на месте через экспресс-маршрут или VPN, с помощью принудительного маршрута.

> [!NOTE]
> Если у вас есть прокси-сервер, который блокирует трафик без настроек прокси, не забудьте добавить исключения в учетную запись хранения артефактов лаборатории.
...
Если один ресурс является родительским для другого ресурса, то перед созданием этого дочернего ресурса должен существовать его родительский ресурс. Если родительский ресурс не существует, вы видите сообщение **ParentResourceNotFound.** Если не задать зависимость от родительского ресурса, то дочерний ресурс может быть развернут раньше родительского.
...
Ошибки при развертывании виртуальной машины записываются в журналы действий. Вы можете найти журналы активности лаборатории VM в **журналах аудита** или **виртуальной машинной диагностики** в меню ресурсов на странице VM лаборатории (страница появляется после выбора VM из списка Моих виртуальных машин).
...
### <a name="why-do-i-get-location-is-not-available-for-resource-type-error-when-trying-to-create-a-lab"></a>Почему при попытке создать лабораторию происходит ошибка типа ресурса недоступна?
При попытке создания лаборатории может быть получено сообщение об ошибке, аналогичное следующему:

```
The provided location 'australiacentral' is not available for resource type 'Microsoft.KeyVault/vaults'. List of available regions for the resource type is 'northcentralus,eastus,northeurope,westeurope,eastasia,southeastasia,eastus2,centralus,southcentralus,westus,japaneast,japanwest,australiaeast,australiasoutheast,brazilsouth,centralindia,southindia,westindia,canadacentral,canadaeast,uksouth,ukwest,westcentralus,westus2,koreacentral,koreasouth,francecentral,southafricanorth
```

Вы можете устранить эту ошибку, приняв один из следующих шагов:

#### <a name="option-1"></a>Вариант 1
Проверьте наличие типа ресурсов в регионах Azure на странице [Продуктов, доступной по](https://azure.microsoft.com/global-infrastructure/services/) странице региона. Если тип ресурса недоступен в определенном регионе, DevTest Labs не поддерживает создание лаборатории в этом регионе. Выберите другой регион при создании лаборатории.

#### <a name="option-2"></a>Вариант 2
Если тип ресурса доступен в вашем регионе, проверьте, зарегистрирован ли он в подписке. Это может быть сделано на уровне владельца подписки, как показано в [этой статье](../azure-resource-manager/management/resource-providers-and-types.md).
