...
- Размер виртуальной машины определяет число дисков данных, которые можно к ней подключить. Для получения [подробной](https://docs.microsoft.com/azure/virtual-machines/windows/sizes)информации см.
...
Через несколько мгновений новый диск данных создается и прикрепляется к VM и появляется в списке **дисков data** для этого VM.
...
Через несколько мгновений диск данных прикрепляется к VM и появляется в списке **дисков data** для этого VM.
...
1. В меню слева выберите **Диски**.
1. Из списка **дисков данных**выберите диск данных, который вы хотите отсоединить.
...
1. Право нажмите на диск данных вы хотите отсоединить - или выбрать его эллипсис **(...**) - и выбрать **Detach**.
