---
title: Подключение среды к виртуальной сети лаборатории в Azure DevTest Labs | Документация Майкрософт
description: Узнайте, как для подключения среды (например, кластер Service Fabric) в виртуальной сети в лаборатории в Azure DevTest Labs
...
## <a name="sample-environment-that-uses-lab-vnet"></a>Пример среды, использует лабораторию виртуальной сети
Вот простая среда шаблон, который подключается подсети лаборатории. В этом образце `DTLSubnetId` параметр представляет идентификатор подсети, в которой существует лаборатории. Она назначается: `$(LabSubnetId)`, который автоматически разрешается с DevTest Labs на идентификатор подсети лаборатории. **Подсети** свойство **сетевой интерфейс** виртуальной машины в этом определении присваивается `DTLSubnetId` таким образом, чтобы он будет присоединен той же подсети. 
