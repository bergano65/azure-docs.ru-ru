...
Ниже приведен простой шаблон среды, который подключает подсеть лаборатории. В этом примере `DTLSubnetId` параметр представляет идентификатор подсети, в которой существует Лаборатория. Он назначается: `$(LabSubnetId)`, который автоматически разрешается DevTest Labs по идентификатору подсети лаборатории. Свойство **Subnet** **сетевого интерфейса** виртуальной машины в этом определении имеет значение `DTLSubnetId` , чтобы оно присоединяется к той же подсети. 
