---
title: Подключение сред к внсети лаборатории в Azure DevTest Labs (ru) Документы Майкрософт
description: Узнайте, как подключить среду (например, кластер Service Fabric) к виртуальной сети лаборатории в Лабораториях Azure DevTest Labs
...
---
# <a name="connect-an-environment-to-your-labs-virtual-network-in-azure-devtest-labs"></a>Подключение среды к виртуальной сети вашей лаборатории в Лабораториях Azure DevTest Labs
Azure DevTest Labs упрощает создание вневых властных точек в лаборатории со [встроенными сетями.](devtest-lab-configure-vnet.md) Он обладает большой гибкостью с возможностью [создания мульти-VM-сред.](devtest-lab-test-env.md) В этой статье показано, как подключить виртуальные виртуальные технологии в среде к виртуальной сети лаборатории. Одним из сценариев, при котором используется эта функция, является настройка приложения N-уровня с уровнем данных S'L Server, подключенного к лаборатории VNet, позволяющей тестировать VMs в лаборатории для доступа к нему.  

## <a name="sample-environment-that-uses-lab-vnet"></a>Образец среды, используюейкоторой лабораторию VNet
Вот простой шаблон среды, который соединяет подсеть лаборатории. В этом примере `DTLSubnetId` параметр представляет идентификатор подсети, в которой существует лаборатория. Он назначен: `$(LabSubnetId)`, который автоматически решается DevTest Labs в ID подсети лаборатории. Свойство **подсети** **сетевого интерфейса** VM в этом `DTLSubnetId` определении настроено на то, чтобы он соединял ту же подсеть. 
