...
- **Проверьте учетную запись хранения лаборатории по умолчанию** (то есть первую учетную запись хранения, созданную при создании лаборатории, имя которой обычно начинается с буквы "a" и заканчивается с помощью многозначного числа, равного\<лабнаме\>#).
...
Если ни одна из этих ошибок не существует и вы по-прежнему не можете применить артефакты, вы можете подать заявку на службу поддержки Azure. Перейдите на [сайт поддержки Azure](https://azure.microsoft.com/support/options/) и выберите **получить поддержку**.
