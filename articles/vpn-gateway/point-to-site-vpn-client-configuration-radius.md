---
title: 'Azure VPN Gateway: Создайте & установить файлы vpn клиента - P2S RADIUS'
...
1. [Настройка VPN шлюза Azure для подключения P2S.](point-to-site-how-to-radius-ps.md)
2. [Навладив сервер RADIUS для проверки подлинности.](point-to-site-how-to-radius-ps.md#radius) 
...
### <a name="1-generate-vpn-client-configuration-files"></a><a name="usernamefiles"></a>1. Создание файлов конфигурации VPN

Вы можете создавать файлы конфигурации VPN клиента с помощью портала Azure или с помощью Azure PowerShell.

#### <a name="azure-portal"></a>Портал Azure

1. Перейдите к виртуальному сетевому шлюзу.
2. Нажмите кнопку «Точка **к месту» конфигурации**.
3. Нажмите **Скачать VPN клиента**.
4. Выберите клиента и заполните любую запрашиваемую информацию.
5. Нажмите **Скачать** для создания файла .zip.
6. Файл .zip будет загружаться, как правило, в папку загрузки.
...
Выполненная команда возвращает ссылку. Копировать и вставить ссылку на веб-браузер, чтобы скачать **VpnClientConfiguration.zip**. Распакуйте файл. Отобразятся следующие папки: 
...
1. Выберите файлы конфигурации VPN-клиента, которые соответствуют архитектуре компьютера Windows. Для 64-разрядной архитектуры процессора выберите пакет установок **установки VpnClientSetupAmd64.** Для 32-битной архитектуры процессора выберите пакет установок **установки VpnClientSetupX86.** 
2. Дважды щелкните пакет, чтобы установить его. Если вы видите всплывающее окно SmartScreen, выберите **Больше информации** > **Выполнить в любом случае**.
...
11. Выберите **настройки аутентификации**. В списке выберите **Username** (Имя пользователя) и введите свои учетные данные. Если учетные данные введены ранее, **имя пользователя** будет выбрано автоматически. Имя пользователя и пароль подставляются предварительно. Нажмите кнопку **ОК**, чтобы сохранить настройки.
...
7. В разделе **Client** (Клиент) подключения выберите значение **EAP** для параметра **Authentication** (Проверка подлинности) и введите имя пользователя и пароль. Необходимо выбрать значок замка справа, чтобы сохранить эти сведения. Затем выберите **Сохранить**.
...
### <a name="1-generate-vpn-client-configuration-files"></a><a name="certfiles"></a>1. Создание файлов конфигурации VPN
...
1. Выберите пакет конфигурации и установите его на клиентском устройстве. Для 64-разрядной архитектуры процессора выберите пакет установок **установки VpnClientSetupAmd64.** Для 32-битной архитектуры процессора выберите пакет установок **установки VpnClientSetupX86.** Если вы видите всплывающее окно SmartScreen, выберите **Больше информации** > **Выполнить в любом случае**. Вы также можете сохранить пакет для установки на других клиентских компьютерах.
2. Для выполнения проверки подлинности каждому клиенту требуется сертификат клиента. Установите сертификат клиента. Для получения информации о [Client certificates for point-to-site](vpn-gateway-certificates-point-to-site.md)сертификатах клиента см. Установка созданного сертификата описывается в разделе [Установка сертификата клиента для аутентификации Azure на основе сертификата при подключениях типа "точка — сеть"](point-to-site-how-to-vpn-client-install-azure-cert.md).
3. На клиентском компьютере перейдите в раздел **Параметры сети** и выберите **VPN**. Для VPN-подключения отображается имя виртуальной сети, к которой оно устанавливается.
...
3. Откройте диалоговое окно **Network** (Сеть) в разделе **Network Preferences** (Параметры сети). Выберите **+** для создания нового профиля подключения VPN для подключения P2S к виртуальной сети Azure.
