---
title: 'Налаживать соединение VPN Gateway От VNet: портал Azure'
...
- **Параметры виртуальной сети**
    ...
    - **Подписка**: Выберите подписку, которая вы хотите использовать.
    ...
    - **Шлюз подсеть**:
        - **Имя**: *GatewaySubnet* является автозаполненным
...
- **Параметры шлюза виртуальной сети**
    ...
    - **SKU**: Выберите шлюз SKU вы хотите использовать.
    - **Имя публичного IP-адреса**: VNet1GWpip
    - **Подключения**
       - **Имя**: VNet1toVNet4
       - **Общий ключ**: Вы можете создать общий ключ самостоятельно. При создании подключения между виртуальными сетями значения должны совпадать. Для этого упражнения используйте abc123.
...
- **Параметры шлюза виртуальной сети** 
    ...
    - **Тип VPN**. Выберите **На основе маршрута**.
    - **SKU**: Выберите шлюз SKU вы хотите использовать.
    - **Имя публичного IP-адреса**: VNet4GWpip
    - **Подключения** 
       - **Имя**: VNet4toVNet1
       - **Общий ключ**: Вы можете создать общий ключ самостоятельно. При создании подключения между виртуальными сетями значения должны совпадать. Для этого упражнения используйте abc123.
...
После настройки VNet1 создайте VNet4 и шлюз VNet4, повторив предыдущие шаги и заменив значения значениями VNet4. Вам не нужно ждать, пока виртуальный сетевой шлюз для VNet1 не закончит создание, прежде чем настроить VNet4. При использовании собственных значений убедитесь, что адресные пространства не перекрываются с другими виртуальными сетями, к которым нужно подключиться.

## <a name="configure-the-vnet1-gateway-connection"></a>Настройка подключения шлюза VNet1
Когда виртуальные сетевые шлюзы для VNet1 и VNet4 будут завершены, вы можете создать свои виртуальные сетевые шлюзы. В этом разделе мы создадим подключение между VNet1 и VNet4. Эти действия выполняются только для виртуальных сетей в одной подписке. Если ваши VNets находятся в разных подписках, вы должны использовать [PowerShell,](vpn-gateway-vnet-vnet-rm-ps.md) чтобы сделать соединение. Но если виртуальные сети находятся в разных группах ресурсов в одной подписке, их можно подключить с помощью портала.

1. На портале Azure выберите **Все ресурсы**, введите *шлюз виртуальной сети* в поле поиска, а затем перейдите к шлюзу виртуальной сети для виртуальной сети. Например, **VNet1GW**. Выберите шлюз, чтобы открыть страницу **виртуального сетевого шлюза.** В разделе **Параметры** выберите **Подключения**.

   ![Страница подключения](./media/vpn-gateway-howto-vnet-vnet-resource-manager-portal/connections.png "Страница подключения")
2. Выберите **«Добавить,** чтобы открыть страницу **подключения добавления.**
...
3. На странице **Добавление подключения** укажите значения для подключения:
   ...
   - **Тип подключения**: Выберите **VNet-to-VNet** из выпадения вниз.

   - **Первый виртуальный сетевой шлюз:** Это значение поля автоматически заполняется, потому что вы создаете это соединение из указанного виртуального сетевого шлюза.

   - **Второй виртуальный сетевой шлюз**: Это поле является виртуальным сетевым шлюзом VNet, к которым вы хотите создать соединение. Щелкните **Выберите другой шлюз виртуальной сети**, чтобы открыть страницу **Выбор шлюза виртуальной сети**.
     ...
     - **Общий ключ (PSK):** В этой области введите общий ключ для подключения. Этот ключ можно сгенерировать или создать самостоятельно. В подключении типа "сеть — сеть" один и тот же ключ используется и для локального устройства, и для подключения шлюза виртуальной сети. То же самое и здесь, но вместо подключения к VPN-устройству выполняется подключение к другому шлюзу виртуальной сети.
    
4. Нажмите кнопку **ОК** , чтобы сохранить изменения.

## <a name="configure-the-vnet4-gateway-connection"></a>Настройка подключения шлюза VNet4
Затем создайте соединение с VNet4 на VNet1. На портале найдите виртуальный сетевой шлюз, связанный с VNet4. Выполните шаги из предыдущего раздела, заменив значения для создания соединения с VNet4 на VNet1. Используйте тот же общий ключ.

## <a name="verify-your-connections"></a>Проверка подключений

1. Найдите шлюз виртуальной сети на портале Azure. 
2. На странице **шлюза виртуальной сети** выберите **Подключения**, чтобы просмотреть страницу **Подключения** для шлюза виртуальной сети. После того, как соединение установлено, вы увидите изменение значений **статуса** в **Connected.**

   ![Проверка соединений](./media/vpn-gateway-howto-vnet-vnet-resource-manager-portal/view-connections.png "Проверка соединений")
3. Под столбтем **«Имя»** выберите одно из подключений для просмотра дополнительной информации. Когда начнется передача данных, появятся значения для параметров **Входящие данные** и **Исходящие данные**.
