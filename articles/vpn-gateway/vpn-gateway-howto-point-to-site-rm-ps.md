---
title: 'Подключение к VNet с компьютера - P2S VPN и аутентификация сертификата Azure: PowerShell'
...
ms.locfileid: "76045084"
---
# <a name="configure-a-point-to-site-vpn-connection-to-a-vnet-using-native-azure-certificate-authentication-powershell"></a>Настройте VPN-соединение от точки к сайту к VNet с помощью аутентификации сертификата Azure: PowerShell
...
>[!NOTE]
> Большинство шагов в этой статье могут использовать Azure Cloud Shell. Тем не менее, чтобы отправить открытый ключ корневого сертификата необходимо использовать PowerShell или портал Azure локально.
>
...
## <a name="1-sign-in-and-set-variables"></a><a name="declare"></a>1. Войти и установить переменные
...
## <a name="2-configure-a-vnet"></a><a name="ConfigureVNet"></a>2. Настройка VNet
...
* Параметр -VpnClientProtocol используется, чтобы указать типы туннелей, которые необходимо включить. Параметры туннеля **OpenVPN, SSTP** и **IKEv2**. Вы можете включить одну из них или любую поддерживаемую комбинацию. Если требуется включить несколько типов, укажите имена, разделенные запятой. OpenVPN и SSTP не могут быть включены вместе. Для подключения клиент strongSwan в Android и Linux и собственный VPN-клиент IKEv2 в iOS и OSX используют только туннель IKEv2. Клиенты Windows сначала пытаются подключиться через IKEv2, и если им это не удается, возвращаются к SSTP. Вы можете использовать клиент OpenVPN для подключения к типу туннеля OpenVPN.
* Виртуальный сетевой шлюз SKU «Основной» не поддерживает аутентификацию IKEv2, OpenVPN или RADIUS. Если вы планируете подключение к своей виртуальной сети клиентов Mac, не используйте SKU "Базовый".
...
## <a name="4-add-the-vpn-client-address-pool"></a><a name="addresspool"></a>4. Добавьте пул адресов VPN
...
### <a name="1-obtain-the-cer-file-for-the-root-certificate"></a><a name="cer"></a>1. Получить файл .cer для корневого сертификата
...
## <a name="6-upload-the-root-certificate-public-key-information"></a><a name="upload"></a>6. Загрузить информацию об общедоступных ключах корневого сертификата
...
>[!NOTE]
> Вы не можете загрузить файл .cer с помощью Azure Cloud Shell. Вы можете использовать PowerShell локально на компьютере, либо использовать [шаги портала Azure.](vpn-gateway-howto-point-to-site-resource-manager-portal.md#uploadfile)
>
...
2. Добавьте собственный путь к файлу, а затем выполните командлеты.
   ...
   ```
3. Отправьте сведения об открытом ключе в Azure. После отправки сведений о сертификате Azure рассматривает его как доверенный корневой сертификат. При загрузке убедитесь, что вы работаете PowerShell локально на вашем компьютере, или вместо этого, вы можете использовать [шаги портала Azure.](vpn-gateway-howto-point-to-site-resource-manager-portal.md#uploadfile) Вы не можете загрузить с помощью Azure Cloud Shell.
...
## <a name="7-install-an-exported-client-certificate"></a><a name="clientcertificate"></a>7. Установка экспортируемого сертификата клиента
...
## <a name="8-configure-the-native-vpn-client"></a><a name="clientconfig"></a>8. Настройка родного VPN-клиента
