---
title: Установка подключения "точка — сеть" между компьютером и виртуальной сетью Azure с использованием собственной аутентификации Azure на основе сертификата и PowerShell | Документация Майкрософт
...
ms.locfileid: "65989443"
---
# <a name="configure-a-point-to-site-connection-to-a-vnet-using-native-azure-certificate-authentication-powershell"></a>Настройка подключения "точка — сеть" к виртуальной сети с использованием собственной аутентификации Azure на основе сертификата и PowerShell
...
* Параметр -VpnClientProtocol используется, чтобы указать типы туннелей, которые необходимо включить. Параметры туннеля **OpenVPN, SSTP** и **IKEv2**. Вы можете включить один из них или все поддерживаемые сочетания. Если вы хотите включить несколько типов, укажите имена, разделенные запятой. OpenVPN и SSTP не может быть одновременно разрешены. Для подключения клиент strongSwan в Android и Linux и собственный VPN-клиент IKEv2 в iOS и OSX используют только туннель IKEv2. Клиенты Windows сначала пытаются подключиться через IKEv2, и если им это не удается, возвращаются к SSTP. Клиент OpenVPN можно использовать для подключения к OpenVPN Тип туннеля.
* SKU «Basic» шлюз виртуальной сети не поддерживает проверку подлинности IKEv2, OpenVPN или RADIUS. Если вы планируете подключение к своей виртуальной сети клиентов Mac, не используйте SKU "Базовый".
