...
ExpressRoute позволяет переносить локальные сети в Microsoft Cloud по частному подключению, которое обеспечивается поставщиком услуг подключения. ExpressRoute позволяет устанавливать подключения к облачным службам Майкрософт, таким как Microsoft Azure, Microsoft 365 и CRM Online. Подключение может осуществляться из любой сети (IP VPN), сети Ethernet типа "точка-точка" или виртуального кросс-подключения через поставщика услуг подключения при совместном размещении.
