---
title: 'Создание соединения между VNets: классический: портал Azure'
...
## <a name="step-1---plan-your-ip-address-ranges"></a><a name="plan"></a>Шаг 1. Планирование диапазонов IP-адресов
...
## <a name="step-2---create-the-virtual-networks"></a><a name="vnetvalues"></a>Шаг 2. Создание виртуальных сетей
...
## <a name="step-3---configure-the-local-site"></a><a name="localsite"></a>Шаг 3. Настройка локального сайта
...
2. На **новой** странице VPN Connection выберите **сайт на сайт**.
...
## <a name="step-4---create-the-virtual-network-gateway"></a><a name="gw"></a>Шаг 4. Создание шлюза виртуальной сети
...
## <a name="step-5---configure-testvnet4-settings"></a><a name="vnet4settings"></a>Шаг 5. Настройка параметров TestVNet4
...
## <a name="step-6---update-the-local-sites"></a><a name="updatelocal"></a>Шаг 6. Обновление локальных сайтов
...
## <a name="step-7---retrieve-values-from-the-network-configuration-file"></a><a name="getvalues"></a>Шаг 7. Получение значений из файла конфигурации сети
...
1. Скачайте и установите последнюю версию командлетов PowerShell для управления службами Azure. Для получения дополнительной информации [см.](#powershell)

2. Откройте консоль PowerShell с повышенными правами. Используйте следующие примеры, чтобы помочь вам подключиться. Вы должны выполнить эти команды локально с помощью модуля управления службой PowerShell. Чтобы перейти к управлению службами, используйте эту команду:

   ```powershell
   azure config mode asm
   ```
3. Подключитесь к учетной записи. Для подключения используйте следующий пример кода:

   ```powershell
   Add-AzureAccount
   ```
4. Просмотрите подписки учетной записи.

   ```powershell
   Get-AzureSubscription
   ```
5. При наличии нескольких подписок выберите подписку, которую вы хотите использовать.

   ```powershell
   Select-AzureSubscription -SubscriptionId "Replace_with_your_subscription_ID"
   ```
6. Экспортируйте и просмотрите файл конфигурации сети. Создайте каталог на компьютере, а затем экспортируйте в него файл конфигурации сети. В этом примере файл конфигурации сети экспортируется в каталог **C:\AzureNet**.

   ```powershell
   Get-AzureVNetConfig -ExportToFile C:\AzureNet\NetworkConfig.xml
   ```
7. Откройте файл в текстовом редакторе и просмотрите имена для виртуальных сетей и сайтов. Эти имена будут имена, которые вы используете при создании ваших соединений.<br>Имена виртуальных сетей перечислены как **VirtualNetworkSite name =**.<br>Имена сайтов перечислены как **LocalNetworkSiteRef name =**.

## <a name="step-8---create-the-vpn-gateway-connections"></a><a name="createconnections"></a>Шаг 8. Создание подключений VPN-шлюза
...
## <a name="vnet-to-vnet-considerations-for-classic-vnets"></a><a name="faq"></a>Рекомендации по работе с подключением типа "виртуальная сеть — виртуальная сеть" для классических виртуальных сетей
...
* Облачный сервис или конечная точка балансировки нагрузки не могут охватывать виртуальные сети, даже если они соединены друг с другом.
