...
**Не удалось найти сертификат, который может использоваться с протоколом расширенной проверки подлинности. (Ошибка 798)**
...
1. Откройте диспетчер сертификатов. Щелкните **Пуск**, введите **manage computer certificates** (управление сертификатами компьютеров) и щелкните **manage computer certificates** (Управление сертификатами компьютеров) в результатах поиска.
...
## <a name="the-network-connection-between-your-computer-and-the-vpn-server-could-not-be-established-because-the-remote-server-is-not-responding"></a>Не удалось установить сетевое подключение между вашим компьютером и VPN-сервера, так как удаленный сервер не отвечает

### <a name="symptom"></a>Симптом

При попытке подключиться к виртуальной сети Azure gteway, с помощью IKEv2 в Windows, вы получаете следующее сообщение об ошибке:

**Не удалось установить сетевое подключение между вашим компьютером и VPN-сервера, так как удаленный сервер не отвечает**

### <a name="cause"></a>Причина:
 
 Проблема возникает, если версия Windows не поддерживает IKE фрагментации
...
**Полученное сообщение было непредвиденное или оно имеет неправильный формат. (Ошибка 0x80090326)**
...
**Цепочка сертификатов обработана, но обработка прервана на корневом сертификате, который не является доверенным поставщиком доверия.**
...
## <a name="file-download-error-target-uri-is-not-specified"></a>Ошибка скачивания файла: не указано URI назначения

### <a name="symptom"></a>Симптом

Отображается следующее сообщение об ошибке:

**Произошла ошибка скачивания файла. Не указан целевой URI.**
...
## <a name="vpn-client-error-azure-vpn-custom-script-failed"></a>Ошибка VPN-клиента: сбой настраиваемого скрипта VPN Azure 

### <a name="symptom"></a>Симптом

При попытке подключения к виртуальной сети Azure с помощью VPN-клиента появляется следующее сообщение об ошибке:

**Сбой настраиваемого сценария (для обновления таблицы маршрутизации). (Ошибка 8007026f)**
...
**Не удалось сохранить шлюз виртуальной сети &lt; *имя шлюза*&gt;. Данные для сертификата &lt; *идентификатор сертификата* &gt; является недопустимым.**
...
Чтобы устранить эту проблему, загрузите и повторно разверните пункты сайта пакета на всех клиентах.
...
## <a name="vpn-client-error-the-connection-was-prevented-because-of-a-policy-configured-on-your-rasvpn-server-error-812"></a>Ошибка VPN-клиента: Подключение не выполнено из-за политики, заданной на вашем RAS/VPN сервере. (Ошибка 812.)"
...
## <a name="error-file-download-error-target-uri-is-not-specified"></a>Ошибка: "Произошла ошибка скачивания файла. URI назначения не указан"
