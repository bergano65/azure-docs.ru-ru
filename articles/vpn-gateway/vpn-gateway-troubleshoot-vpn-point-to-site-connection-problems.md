---
title: Проблемы с устранением проблем между Azure от точки к сайту
...
**Сертификат не может быть найден, который может быть использован с этим расширяемым протоколом аутентификации. (Ошибка 798)**
...
## <a name="the-network-connection-between-your-computer-and-the-vpn-server-could-not-be-established-because-the-remote-server-is-not-responding"></a>Сетевое соединение между компьютером и VPN-сервером не может быть установлено, поскольку удаленный сервер не отвечает

### <a name="symptom"></a>Симптом

При попытке подключения к виртуальному сетевому шлюзу Azure с помощью IKEv2 в Windows вы получаете следующее сообщение об ошибке:

**Сетевое соединение между компьютером и VPN-сервером не может быть установлено, поскольку удаленный сервер не отвечает**

### <a name="cause"></a>Причина
 
 Проблема возникает, если версия Windows не имеет поддержки для фрагментации IKE
...
2. Установите значение раздела реестра. Создайте `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\RasMan\ IKEv2\DisableCertReqPayload` или установите ключ REG_DWORD в реестре до 1.
...
**Полученное сообщение было неожиданным или плохо отформатированным. (Ошибка 0x80090326)**
...
**Ошибка загрузки файлов. Целевой uri не указан.**
...
**Пользовательский скрипт (для обновления таблицы реаутинга) вышел из строя. (Ошибка 8007026f)**
...
4. Щелкните правой кнопкой мыши узел **Доверенные корневые центры сертификации**. Нажмите **All-Task** > **Import**и просмотрите файл .cer, извлеченный из пакета конфигурации VPN.
...
**Не удалось сохранить &lt;виртуальной сети *шлюз шлюз имя*&gt;. Данные &lt; *идентификатора* &gt; сертификата недействительны.**
...
**Не удалось сохранить &lt;виртуальной сети *шлюз шлюз имя*&gt;. Имя &lt; *сертификата ресурса, которое вы пытаетесь загрузить,* &gt; является недействительным.**
...
**Не удалось загрузить файл. Детали ошибки: ошибка 503. Сервер занят.**
...
Чтобы решить эту проблему, перезагрузите и передислоцируете пакет Point в пакет Site для всех клиентов.
...
Чтобы устранить эту проблему, [сбросьте VPN-шлюз Azure](vpn-gateway-resetgw-classic.md). Чтобы убедиться, что используются новые маршруты, клиенты VPN "точка-сеть" необходимо загрузить заново после успешной настройки пиринга виртуальной сети.
...
## <a name="vpn-client-error-dialing-vpn-connection-vpn-connection-name-status--vpn-platform-did-not-trigger-connection"></a>Ошибка VPN Клиента: Набор <VPN Connection Name>VPN-соединения, Статус и VPN Платформа не вызвали подключение

Вы также можете увидеть следующую ошибку в Event <User> Viewer от <VPN Connection Name> RasClient: "Пользователь набрал соединение с именем, которое не удалось. Код ошибки, возвращенный при сбое, составляет 1460."

### <a name="cause"></a>Причина

Клиент VPN Azure не имеет разрешения на использование приложений "Background apps" в настройках приложений для Windows.

### <a name="solution"></a>Решение

1. В Windows перейдите к настройкам -> Конфиденциальность -> Фоновые приложения
2. Переключить "Пусть приложения работают в фоновом режиме" на On
...
## <a name="vpn-package-installer-doesnt-complete"></a>Установка VPN-пакета не завершена
