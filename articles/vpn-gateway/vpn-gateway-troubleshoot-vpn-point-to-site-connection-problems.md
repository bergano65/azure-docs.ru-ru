...
Чтобы устранить эту проблему, [сбросьте VPN-шлюз Azure](./reset-gateway.md). Чтобы убедиться, что используются новые маршруты, клиенты VPN "точка-сеть" необходимо загрузить заново после успешной настройки пиринга виртуальной сети.
...
Убедитесь, что сервер RADIUS настроен правильно. Дополнительные сведения см. в статье [Интеграция аутентификации RADIUS с Azure AD сервер многофакторной идентификации](../active-directory/authentication/howto-mfaserver-dir-radius.md).
