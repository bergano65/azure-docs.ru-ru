---
title: Подключение локальной сети к виртуальной сети Azure. Создание VPN-подключения "сеть — сеть" с помощью классического портала | Документация Майкрософт
...
* Сейчас, чтобы указать общий ключ и создать подключение VPN-шлюза, требуется PowerShell. Установите последнюю версию командлетов PowerShell для управления службами Azure. Сведения об установке командлетов см. в разделе [Управление службами](/powershell/azure/servicemanagement/install-azure-ps). Дополнительные сведения об установке PowerShell в целом см. в [статье Установка и настройка Azure PowerShell](/powershell/azure/overview). Если вы работаете с этой конфигурацией в PowerShell, убедитесь, что вы запустили ее от имени администратора.
...
* **Подсети:**
  ...
  * Серверная часть: 10.12.0.0/24 (необязательно для этой задачи)
* **Подсеть шлюза:** 10.11.255.0/27
* **Группа ресурсов**. TestRG1
* **Расположение:** East US
* **DNS-сервер:** 10.11.0.3 (необязательно для этой задачи)
...
Эти команды необходимо выполнять локально с помощью модуля управления службами PowerShell. Чтобы переключиться на управление службами, используйте следующую команду:
