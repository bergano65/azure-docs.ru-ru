...
В этой статье показано, как с помощью портала Azure создать подключение типа "сеть — сеть" с использованием VPN-шлюза между вашей локальной сетью к виртуальной. Шаги в этой статье применяются к классической модели развертывания и не применяются к текущей модели развертывания, менеджер ресурсов. Эту конфигурацию также можно создать с помощью разных средств или моделей развертывания, выбрав вариант из следующего списка:
...
* PowerShell требуется для того, чтобы указать общий ключ и создать соединение VPN шлюза. [!INCLUDE [vpn-gateway-classic-powershell](../../includes/vpn-gateway-powershell-classic-locally.md)]
...
2. Нажмите*кнопку «Создать ресурс».* В поле **Поиск по Marketplace** введите "Виртуальная сеть". Найдите **Виртуальную сеть** из возвращенного списка и нажмите, чтобы открыть страницу **Виртуальной сети.**
3. щелкните **(изменить на классический),** а затем нажмите **Создать**.
4. В колонке **Create virtual network (classic)** (Создание виртуальной сети (классической)) настройте параметры виртуальной сети. На этой странице добавьте первое адресное пространство и единый диапазон адресов подсети. После создания VNet можно вернуться назад и добавить дополнительные подсети и адресные пространства.

   ![Страница создания виртуальной сети](./media/vpn-gateway-howto-site-to-site-classic-portal/createvnet.png "Страница создания виртуальной сети")
5. Убедитесь, что вы используете правильную **подписку** . Подписки можно менять с помощью раскрывающегося списка.
6. Щелкните **Группа ресурсов** и либо выберите имеющуюся, либо создайте группу ресурсов. Чтобы создать группу, введите ее имя. Для получения дополнительной информации о группах ресурсов посетите [обзор менеджера ресурсов Azure](../azure-resource-manager/management/overview.md#resource-groups).
...
## <a name="2-add-additional-address-space"></a><a name="additionaladdress"></a>2. Добавить дополнительное адресное пространство
...
## <a name="3-specify-a-dns-server"></a><a name="dns"></a>3. Указать DNS-сервер
...
## <a name="4-configure-the-local-site"></a><a name="localsite"></a>4. Настройка местного сайта
...
1. На странице для вашего VNet, под **настройками,** нажмите **Диаграмма**.
1. На странице **VPN-соединений** нажмите **Вы не имеете существующих VPN-соединений. Нажмите здесь, чтобы начать работу**.
1. Для **типа подключения**оставьте **выбранный сайт для сайта.**
4. Щелкните **Локальный сайт > Настройка обязательных параметров**, чтобы открыть колонку **Локальный сайт**. Настройте параметры и нажмите кнопку **ОК**, чтобы сохранить их.
   ...
   ![Местный сайт](./media/vpn-gateway-howto-site-to-site-classic-portal/localnetworksite.png "Настройка локального сайта")

Нажмите **OK,** чтобы закрыть страницу локального сайта. **Не нажимайте OK, чтобы закрыть новую страницу VPN Connection.**

## <a name="5-configure-the-gateway-subnet"></a><a name="gatewaysubnet"></a>5. Настройка подсети шлюза
...
1. На странице **Новое VPN-подключение** установите флажок **Немедленно создать шлюз**. Появится страница "Дополнительная настройка шлюза". Если не установить флажок, колонка настройки подсети шлюза не появится.
   ...
   ![Конфигурация шлюза - Subnet, размер, тип разгрома](./media/vpn-gateway-howto-site-to-site-classic-portal/optional.png "Конфигурация шлюза - Subnet, размер, тип разгрома")
...
3. На странице **Конфигурация шлюза** щелкните **Подсеть — Настройка обязательных параметров**, чтобы открыть страницу **Добавить подсеть**. Когда вы закончите настройку этих настроек, нажмите **OK.**

   ![Конфигурация шлюза - подсеть шлюза](./media/vpn-gateway-howto-site-to-site-classic-portal/subnetrequired.png "Конфигурация шлюза - подсеть шлюза")
...
## <a name="6-specify-the-sku-and-vpn-type"></a><a name="sku"></a>6. Укажите тип SKU и VPN
...
1. Выберите **размер** шлюза. Это SKU шлюза, который будет использоваться для создания шлюза виртуальной сети. Классические VPN-шлюзы используют старые (устаревшие версии) SKU. Дополнительные сведения об устаревших версиях SKU шлюза см. в статье [Работа со SKU шлюза виртуальной сети (старые версии SKU)](vpn-gateway-about-skus-legacy.md).
   ...
   ![Выберите тип SKUL и VPN](./media/vpn-gateway-howto-site-to-site-classic-portal/sku.png "Выберите тип SKU и VPN")
2. Выберите **тип маршрутизации** для шлюза. Его также называют типом VPN. Важно выбрать правильный тип, поскольку вы не можете преобразовать шлюз из одного типа в другой. VPN-устройство должно быть совместимым с выбранным типом маршрутизации. Для получения дополнительной информации о типе routing, [см.](vpn-gateway-about-vpn-gateway-settings.md#vpntype) В статье могут упоминаться такие типы VPN, как RouteBased и PolicyBased. RouteBased соответствует динамическому VPN-подключению, а PolicyBased — статическому.
3. Нажмите кнопку **ОК**, чтобы сохранить настройки.
4. На **новой** странице VPN Connection нажмите **OK** в нижней части страницы, чтобы начать развертывание виртуального сетевого шлюза. В зависимости от выбранного номера SKU создание шлюза виртуальной сети может занять до 45 минут.

## <a name="7-configure-your-vpn-device"></a><a name="vpndevice"></a>7. Наверскакните свое VPN-устройство
...
## <a name="8-create-the-connection"></a><a name="CreateConnection"></a>8. Создание соединения
...
> [!NOTE]
> В настоящее время этот этап недоступен на портале Azure. Необходимо использовать версию командлетов Azure PowerShell для управления службами. Смотрите [Прежде чем начать](#before) для получения информации об установке этих cmdlets.
>

### <a name="step-1-connect-to-your-azure-account"></a>Шаг 1. Подключение к учетной записи Azure

Вы должны выполнить эти команды локально с помощью модуля управления службой PowerShell. 
...
При создании классического VNet на портале (не использующем PowerShell) Azure добавляет имя группы ресурсов в короткое имя. Например, согласно Azure, имя VNet, созданное для этого упражнения, — «Группа TestRG1 TestVNet1», а не «TestVNet1». PowerShell требует полное название виртуальной сети, а не короткое имя, которое появляется на портале. Длинное имя не видно на портале. Следующие шаги помогут экспортировать файл конфигурации сети для получения точных значений для виртуального имени сети. 
...
## <a name="9-verify-your-connection"></a><a name="verify"></a>9. Проверить подключение
...
* Для получения информации о принудительном туннеле, [см.](vpn-gateway-about-forced-tunneling.md)
