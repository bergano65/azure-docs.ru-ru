...
## <a name="generatecert"></a>3. Создание сертификатов
...
### <a name="getcer"></a>1. получите CER файл для корневого сертификата.
...
## <a name="addresspool"></a>4. Добавление пула адресов клиента
...
1. После создания шлюза виртуальной сети перейдите к разделу **Параметры** на странице шлюза виртуальной сети. В разделе **Параметры** выберите Конфигурация " **точка — сеть**". Щелкните **настроить сейчас** , чтобы открыть страницу Конфигурация.
   ...
   ![Страница "точка — сеть"](./media/vpn-gateway-howto-point-to-site-resource-manager-portal/point-to-site-configure.png "Настройка "точка — сеть" сейчас")
...
## <a name="tunneltype"></a>5. Настройка типа туннеля
...
## <a name="authenticationtype"></a>6. Настройка типа проверки подлинности
...
## <a name="uploadfile"></a>7. Отправка данных открытого сертификата корневого сертификата
...
5. Выберите **сохранить** в верхней части страницы, чтобы сохранить все параметры конфигурации.
   ...
   ![Сохранить конфигурацию](./media/vpn-gateway-howto-point-to-site-resource-manager-portal/save.png "сохранить конфигурацию")
...
2. На странице состояния **подключения** щелкните **Подключить**. Если появится окно **Выбор сертификата**, убедитесь в том, что отображается сертификат клиента, с помощью которого вы хотите подключиться к сети. Если это не так, используйте стрелку раскрывающегося списка, чтобы выбрать правильный сертификат, а затем нажмите кнопку **ОК**.
   ...
   ![VPN-клиент подключается к Azure](./media/vpn-gateway-howto-point-to-site-resource-manager-portal/clientconnect.png "подключение")
...
Ознакомьтесь с подробными инструкциями в разделе [Установка Mac (OS X)](https://docs.microsoft.com/azure/vpn-gateway/point-to-site-vpn-client-configuration-azure-cert#installmac). Если у вас возникают неполадки подключения, убедитесь, что шлюз виртуальной сети не использует SKU "Базовый". SKU "Базовый" не поддерживается для клиентов Mac.
  ...
  ![Подключение к Mac](./media/vpn-gateway-howto-point-to-site-rm-ps/applyconnect.png "Подключение")
