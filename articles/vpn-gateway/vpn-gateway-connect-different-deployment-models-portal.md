...
### <a name="example-settings"></a><a name="values"></a>Примеры настроек
...
### <a name="1-create-a-classic-vnet"></a>1. <a name="classicvnet"> </a>Создать классический VNet
...
### <a name="2-configure-the-local-site"></a>2. <a name="local"> </a>Настройка местного сайта

1. Перейдите к колонке **Все ресурсы** и найдите **ClassicVNet** в списке.
2. Нажмите **шлюз** в разделе **Настройки** меню, а затем нажмите на баннер, чтобы создать шлюз.
...
### <a name="3-create-the-virtual-network-gateway"></a><a name="classicgw"></a>3. Создание виртуального сетевого шлюза
...
2. Щелкните **Дополнительная настройка шлюза**, чтобы открыть страницу **Настройка шлюза**.
   ...
   ![Открытая страница конфигурации шлюза](./media/vpn-gateway-connect-different-deployment-models-portal/optionalgatewayconfiguration.png "Открытая страница конфигурации шлюза")
...
### <a name="4-copy-the-virtual-network-gateway-public-ip-address"></a><a name="ip"></a>4. Копировать виртуальный сетевой шлюз Общественный IP-адрес
...
### <a name="3-create-a-local-network-gateway"></a><a name="createlng"></a>3. Создание локального сетевого шлюза
...
3. В разделе **VPN-подключения** щелкните имя своего локального сайта на схеме.
   ...
   ![VPN-соединения](./media/vpn-gateway-connect-different-deployment-models-portal/vpnconnections.png "VPN-подключения")
4. На странице **VPN-подключения типа «сеть-сеть»** щелкните имя сайта.

   ![Имя сайта](./media/vpn-gateway-connect-different-deployment-models-portal/sitetosite3.png "Местное название сайта")
5. На странице подключения для локального сайта щелкните его имя, чтобы открыть страницу **Локальный сайт**.

   ![Открытый локальный сайт](./media/vpn-gateway-connect-different-deployment-models-portal/openlocal.png "Открытие локального сайта")
6. На странице **Локальный сайт** замените **IP-адрес VPN-шлюза** IP-адресом шлюза Resource Manager.

   ![Шлюз-ip-адрес](./media/vpn-gateway-connect-different-deployment-models-portal/gwipaddress.png "IP-адрес шлюза")
...
### <a name="1-connect-to-your-azure-account"></a>1. Подключите к учетной записи Azure
...
### <a name="3-create-the-connection"></a>3. Создание соединения
...
- **-SharedKey** — это значение, которое вы создаете и укажете. В этом примере мы использовали *abc123*, но можно создать что-нибудь посложнее. Важно, чтобы заданное здесь значение совпадало со значением, которое вы указали при создании подключения от виртуальной сети Resource Manager к классической виртуальной сети.
