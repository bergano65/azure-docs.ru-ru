...
2. Дважды щелкните пакет, чтобы установить его. Если отображается всплывающее окно SmartScreen, щелкните **Дополнительные сведения**, а затем **выполните все равно**.
3. На клиентском компьютере перейдите к **параметрам сети** и щелкните **VPN**. Для VPN-подключения отображается имя виртуальной сети, к которой оно устанавливается. 
...
* **Файл vpnsettings. XML**, который содержит важные параметры, такие как адрес сервера и тип туннеля. 
