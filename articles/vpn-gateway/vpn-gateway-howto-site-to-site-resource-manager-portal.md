---
title: 'Подключение к виртуальной сети Azure: Сайт-к-сайт VPN: Портал'
...
### <a name="example-values"></a><a name="values"></a>Значения примера

В примерах этой статьи мы используем следующие значения. Эти значения можно использовать для создания тестовой среды или для лучшего понимания примеров в этой статье. Для получения дополнительной информации о настройках VPN Gateway в целом, [см.](vpn-gateway-about-vpn-gateway-settings.md)

* **Название виртуальной сети:** VNet1
...
* **Тип VPN:** Маршрутна
* **Тип соединения:** От сайта до сайта (IPsec)
* **Тип шлюза:** Vpn
* **Название шлюза локальной сети:** Сайт1
...
* **Подробности > регионе:** Восток США
* **Виртуальная сеть > виртуальная сеть:** VNet1
* **Подробности > название:** VNet1GW
* **Детали экземпляра > типу шлюза:** Vpn
* **Подробности > типVPN:** Маршрутна
* **Виртуальная сеть > gateway диапазон адрес:** 10.1.255.0/27
* **Общественный IP-адрес > имя публичного IP-адреса:** VNet1GWpip
...
## <a name="3-create-the-local-network-gateway"></a><a name="LocalNetworkGateway"></a>3. Создание локального сетевого шлюза
...
- Общедоступный IP-адрес шлюза виртуальной сети. Общедоступный IP-адрес можно просмотреть с помощью портала Azure, PowerShell или CLI. Чтобы найти общедоступный IP-адрес вашего VPN шлюза с помощью портала Azure, перейдите на **виртуальные сетевые шлюзы,** а затем нажмите на имя вашего шлюза.

[!INCLUDE [Configure a VPN device](../../includes/vpn-gateway-configure-vpn-device-include.md)]

## <a name="5-create-the-vpn-connection"></a><a name="CreateConnection"></a>5. Создание VPN-соединения

Создайте VPN-подключение типа "сеть — сеть" между шлюзом виртуальной сети и локальным VPN-устройством.

[!INCLUDE [Add a site-to-site connection](../../includes/vpn-gateway-add-site-to-site-connection-portal-include.md)]

## <a name="6-verify-the-vpn-connection"></a><a name="VerifyConnection"></a>6. Проверить подключение VPN
