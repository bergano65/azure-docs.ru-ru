...
В этой статье описаны варианты, доступные организациям для настройки удаленного доступа для своих пользователей или для дополнения существующих решений дополнительными возможностями во время эпидемии COVID-19.

Решение Azure point-to-site основано на облаке и может быть быстро подготовлено для удовлетворения возросшего спроса пользователей на работу из дома. Он может легко масштабироваться и выключаться так же легко и быстро, когда увеличенная емкость больше не нужна.
...
![от точки к месту](./media/working-remotely-support/ostable.png "OS")

## <a name="scenario-1---users-need-access-to-resources-in-azure-only"></a>Сценарий 1 - Пользователям нужен доступ только к ресурсам в Azure

В этом случае удаленным пользователям необходим доступ только к ресурсам, наиболее доступным.

![от точки к месту](./media/working-remotely-support/scenario1.png "Сканарио 1")

На высоком уровне необходимы следующие шаги, чтобы пользователи могли безопасно подключаться к ресурсам Azure:

1. Создание виртуального сетевого шлюза (если его не существует)
2. Настройка ОТ точки к сайту VPN на шлюзе
    3. [Для проверки подлинности сертификата, перейдите по этой ссылке](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-howto-point-to-site-resource-manager-portal#creategw).
    2.  [Для OpenVPN, перейдите по этой ссылке](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-howto-openvpn).
    3.  [Для проверки подлинности Azure AD перейдите по этой ссылке.](https://docs.microsoft.com/azure/vpn-gateway/openvpn-azure-ad-tenant)
    4.  [Для устранения неполадок от точки к сайту соединения, перейдите по этой ссылке](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-troubleshoot-vpn-point-to-site-connection-problems).
3. Скачать и распространить конфигурацию VPN клиента
4. Распространение сертификатов (при выборе проверки подлинности сертификата) среди клиентов
5. Подключение к VPN Azure

## <a name="scenario-2---users-need-access-to-resources-in-azure-andor-on-prem-resources"></a>Сценарий 2 - Пользователям необходим доступ к ресурсам в Ресурсах Azure и/или на prem
...
1. Создание виртуального сетевого шлюза (если его не существует)
2. Настройка от точки к сайту VPN на шлюзе (см. Сценарий 1 выше)
3. Настройка туннеля от сайта к месту на azure Virtual Network Gateway с включенным BGP
4. Настройка на устройстве помещения для подключения к виртуальному сетевому шлюзу Azure
5. Скачать профиль точки к сайту с портала Azure и распространить среди клиентов

[Перейдите по этой ссылке, чтобы узнать, как настроить VPN-туннель от сайта к сайту](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-howto-site-to-site-resource-manager-portal)
