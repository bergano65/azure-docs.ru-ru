---
title: 'Azure VPN Gateway: настройки конфигурации'
...
Если у вас есть VPN-шлюз и вы хотите использовать другой номер SKU шлюза, можно либо изменить размер SKU шлюза, или перейти на другой номер SKU. При переходе на другой номер SKU шлюза существующий шлюз полностью удаляется и создается новый. Сборка шлюза может занять до 45 минут. Для сравнения, при восстановлении размера шлюза SKU, не так много простоев, потому что вам не нужно удалять и перестраивать шлюз. Если у вас есть возможность изменить размер SKU шлюза, а не перейти на другой номер SKU, выберите именно этот вариант. Но для изменения размера есть определенные правила:

1. За исключением Basic SKU, вы можете изменить размер VPN шлюза SKU на другой VPN шлюз SKU в том же поколении (Generation1 или Generation2). Например, VpnGw1 поколения1 можно переоставить на VpnGw2 поколения1, но не на VpnGw2 поколения2.
...
3. Вы **не можете** пересмотреть размер от Основных /Стандартных/Высокопроизводительных Скидо до VPNGw SKUs. Вместо этого нужно [изменить](#change) номера SKU на новые.
...
| Не поддерживается | [Лазурный CLI](/cli/azure/network/vnet-gateway)|
