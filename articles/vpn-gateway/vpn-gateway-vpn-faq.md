---
title: Часто задаваемые вопросы VPN Azure VPN
...
### <a name="is-there-an-additional-cost-for-setting-up-a-vpn-gateway-as-active-active"></a>Взимается ли дополнительная плата за настройку VPN-шлюза в режиме "активный — активный"?
...
### <a name="can-i-update-my-policy-based-vpn-gateway-to-route-based"></a>Могу ли я обновить VPN-шлюз на основе политики до маршрутного?
...
1. Удалите шлюз:
   ...
   - [Лазурная силаШелл](vpn-gateway-delete-vnet-gateway-powershell.md)
   - [Azure PowerShell - классический](vpn-gateway-delete-vnet-gateway-classic-powershell.md)
1. [Создайте новый шлюз типа, который вы хотите, и завершите установку VPN.](vpn-gateway-howto-site-to-site-resource-manager-portal.md#VNetGateway)
...
Зональные резервные и зональные шлюзы (шлюзы SKUs, в названии которых есть _АЗ)_ полагаются на общедоступный IP-ресурс _Standard SKU_ Azure. Общедоступные иС-ресурсы Azure Standard SKU должны использовать метод статического распределения. Таким образом, вы будете иметь общедоступный IP-адрес для вашего VPN шлюза, как только вы создаете стандартный SKU общедоступный IP ресурс, который вы собираетесь использовать для него.

Для незональных избыточных и незональных шлюзов (шлюзов SKUs, у которых _нет_ _АК_ в имени), вы не можете получить IP-адрес ШЛюза VPN до его создания. IP-адрес изменяется только в том случае, если вы удалите и воссоздаете свой VPN шлюз.
...
Как указывалось выше, зональные резервные и зональные шлюзы (шлюзы SKUs, в названии которых есть _АЗ),_ полагаются на общедоступный IP-ресурс _Standard SKU_ Azure. Общедоступные иС-ресурсы Azure Standard SKU должны использовать метод статического распределения.

Для незональных избыточных и незональных шлюзов (шлюзы SKUs, не _имеюющие_ ак-игоны, поддерживается только динамическое назначение IP-адреса. _AZ_ Однако это не означает, что IP-адрес меняется после того, как он был назначен на ваш VPN шлюз. Единственный раз, когда VPN шлюз IP-адрес изменения, когда шлюз удаляется, а затем воссозданы. Публичный IP-адрес VPN шлюза не изменяется при изменении размера, сработки или завершения других внутренних работ по техническому обслуживанию и обновлению вашего VPN шлюза.
...
## <a name="how-do-i-change-the-authentication-type-for-my-point-to-site-connections"></a>Как изменить тип проверки подлинности для подключения к сайту?

Вы можете изменить метод проверки подлинности для подключения к сайту, перейдя в раздел **конфигурации point-to-site** под VPN Gateway и проверив нужную кнопку радио. Актуальные параметры: **сертификат Azure, проверка подлинности RADIUS и активный каталог Azure.** Обратите внимание, что текущие клиенты **могут не иметь возможности подключиться** после изменения до тех пор, пока новый профиль не будет загружен и настроен на клиента.
...
Нет. По умолчанию Azure создает разные общие ключи для разных VPN-подключений. Однако можно использовать командлет API REST или PowerShell для задания ключа VPN-шлюза, чтобы установить собственное значение ключа. Ключ должен быть распечатать ASCII символов.
...
## <a name="virtual-network-faq"></a><a name="faq"></a>Виртуальные задаваемые вопросы о сети
