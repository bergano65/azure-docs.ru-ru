---
title: 'VPN-шлюз Azure: вопросы и ответы'
...
Избыточные зоны и шлюзы зональные (номера SKU шлюзов с _AZ_ в имени) используют ресурс общедоступного IP-адреса Azure уровня " _стандартный_ ". Ресурсы общедоступного IP-адреса для SKU уровня "Стандартный" должны использовать статический метод выделения. Таким образом, вы получите общедоступный IP-адрес для VPN-шлюза, как только вы создадите ресурс общедоступного IP-адреса стандартного SKU, который планируется использовать для него.

Для неизбыточных и незональныеных шлюзов (SKU шлюзов, в которых _отсутствует_ _AZ_ в имени) невозможно получить IP-адрес шлюза VPN до его создания. IP-адрес изменяется только при удалении и повторном создании VPN-шлюза.
...
Как указано выше, избыточные в рамках зоны и зональные шлюзы (номера SKU шлюзов с _AZ_ в имени) используют ресурс общедоступного IP-адреса Azure уровня " _стандартный_ ". Ресурсы общедоступного IP-адреса для SKU уровня "Стандартный" должны использовать статический метод выделения.

Для шлюзов, не являющихся избыточными и не являющихся зональные (номера SKU шлюзов, у которых _нет_ _AZ_ в имени), поддерживается только назначение динамических IP-адресов. Однако это не означает, что IP-адрес изменится после назначения VPN-шлюзу. IP-адрес шлюза VPN изменяется только при удалении и повторном создании шлюза. Общедоступный IP-адрес VPN-шлюза не меняется при изменении размера, сбросе или завершении другого внутреннего обслуживания и обновления VPN-шлюза.
...
Нет. По умолчанию Azure создает разные общие ключи для разных VPN-подключений. Однако можно использовать командлет API REST или PowerShell для задания ключа VPN-шлюза, чтобы установить собственное значение ключа. Ключ должен быть печатаемыми символами ASCII.
