...
1. Определите [рекомендованную пропускную способность](../virtual-machines/sizes.md) для используемого размера виртуальной машины Azure.
...
Если вычисленная пропускная способность не соответствует требованиям базовой пропускной способности приложения, необходимо увеличить пропускную способность ресурса, который вы определили как узкое место. Чтобы изменить размер VPN-шлюза Azure, см. статью [Изменение SKU шлюза](vpn-gateway-about-vpn-gateway-settings.md#gwsku). Чтобы изменить размер виртуальной машины, см. статью [Изменение размера виртуальной машины](../virtual-machines/windows/resize-vm.md). Если вы не столкнулись с ожидаемой пропускной способностью Интернета, вы также можете обратиться к поставщику услуг Интернета.
...
> [!Note]
> Iperf — не единственный инструмент. [NTTTCP — это альтернативное решение для тестирования](../virtual-network/virtual-network-bandwidth-testing.md).
...
* Приложения для копирования файлов, такие как проводник и RDP, не используют несколько потоков при копировании. Для повышения производительности используйте многопоточное приложение, например [Richcopy](/previous-versions/technet-magazine/dd547088(v=msdn.10)), которое копирует файлы с помощью 16 или 32 потоков. Чтобы изменить номер потока для копирования файлов в ричкопи, щелкните **действие**  >  **Копировать параметры** копирования  >  **файл копировать**.
...
* Недостаточная скорость чтения и записи виртуальной машины. Дополнительные сведения см. в статье [Устранение неполадок службы хранилища Azure](/previous-versions/azure/storage/common/storage-e2e-troubleshooting).
