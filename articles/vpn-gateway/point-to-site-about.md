---
title: О VPN-соединениях Azure Point-to-Site VPN шлюз
...
* **OpenVPN® протокол**, протокол VPN на основе SSL/TLS. Решение SSL VPN может проникать в брандмауэры, так как большинство брандмауэров открывают порт TCP 443, который использует SSL. OpenVPN можно использовать для подключения с Android, iOS (версии 11.0 и выше), Windows, Linux и Mac (версии OSX 10.13 и выше).

* SSTP (Secure Socket Tunneling Protocol) — разработанный корпорацией Майкрософт VPN-протокол на основе SSL. Решение SSL VPN может проникать в брандмауэры, так как большинство брандмауэров открывают порт TCP 443, который использует SSL. Протокол SSTP поддерживается только на устройствах Windows. Azure поддерживает все версии Windows с протоколом SSTP (Windows 7 и более поздние версии).
...
### <a name="authenticate-using-native-azure-active-directory-authentication"></a>Authenticate с помощью родной azure Active Directory аутентификации

Аутентификация Azure AD позволяет пользователям подключаться к Azure, используя свои учетные данные Active Directory Azure. Проверка подлинности Native Azure AD поддерживается только для протокола OpenVPN и Windows 10 и требует использования [VPN Клиента Azure.](https://go.microsoft.com/fwlink/?linkid=2117554)

С помощью нативной аутентификации Azure AD можно использовать условный доступ Azure AD, а также функции Multi-Factor Authentication (MFA) для VPN.

На высоком уровне необходимо выполнить следующие действия для настройки аутентификации Azure AD:

1. [Настройка клиента AAD](openvpn-azure-ad-tenant.md)

2. [Включить аутентификацию Azure AD на шлюзе](openvpn-azure-ad-tenant.md#enable-authentication)

3. [Скачать и настроить клиент Vpn Azure](https://go.microsoft.com/fwlink/?linkid=2117554)
...
  
Сервер RADIUS может быть развернут на месте или в вашей Azure VNet. Во время аутентификации VPN-шлюз Azure выполняет роль транзитного шлюза и переадресовывает сообщения аутентификации между сервером RADIUS и подключаемым устройством. Поэтому необходимо обеспечить доступность сервера RADIUS для шлюза. Если сервер RADIUS развернут локально, для обеспечения доступности требуется VPN-подключение "сеть — сеть" из Azure к локальному сайту.  
  
...
![от точки к месту](./media/point-to-site-about/p2s.png "Точка-сеть")
...
|**Шифр** | **Целостности** | **Prf** | **Группа DH** |
|---        | ---           | ---       | ---   |
...
### <a name="to-remove-the-configuration-of-a-p2s-connection"></a>Удаление конфигурации соединения P2S

Для шагов, [см. часто задаваемые вопросы](#removeconfig), ниже.
