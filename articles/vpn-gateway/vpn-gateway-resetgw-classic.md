---
title: Перезачет vpn шлюз Azure для восстановления туннеля IPsec
...
## <a name="azure-portal"></a><a name="portal"></a>Портал Azure
...
Командлет сброса шлюза — **Reset-AzVirtualNetworkGateway**. Перед сбросить, убедитесь, что у вас есть последняя версия [PowerShell Az cmdlets.](https://docs.microsoft.com/powershell/module/az.network) В следующем примере выполняется сброс шлюза виртуальной сети с именем VNet1GW в группе ресурсов TestRG1:
...
### <a name="classic-deployment-model"></a><a name="resetclassic"></a>Классическая модель развертывания

Командлет сброса шлюза — **Reset-AzureVNetGateway**. Cmdlets Azure PowerShell для управления обслуживанием должны быть установлены локально на рабочем столе. Нельзя использовать облачную оболочку Azure. Перед выполнением сброса убедитесь, что у вас установлена последняя версия командлетов [PowerShell для управления службами](https://docs.microsoft.com/powershell/azure/servicemanagement/install-azure-ps?view=azuresmps-4.0.0#azure-service-management-cmdlets). При использовании этой команды убедитесь, что вы используете полное имя виртуальной сети. Классические VNets, созданные с помощью портала, имеют длинное название, которое требуется для PowerShell. Вы можете просмотреть длинное имя с помощью 'Get-AzureVNetConfig-ExportToFile C:'Myfoldername-NetworkConfig.xml'.

Следующий пример сбрасывает шлюз для виртуальной сети под названием "Группа TestRG1 TestVNet1" (которая показывает, как просто "TestVNet1" на портале):
...
## <a name="azure-cli"></a><a name="cli"></a>Лазурный CLI
