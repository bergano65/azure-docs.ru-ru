...
- **Создание параметров виртуальной сети (классической)**
   - **Name**: Enter *VNet1*.
   - **Address space**: Enter *192.168.0.0/16*. В этом примере мы используем только одно адресное пространство, но для виртуальной сети можно настроить несколько, как показано на схеме.
   ...
   - **Resource group**: Enter *TestRG*. Выберите **Создать новый**, если группа ресурсов не существует.
   - **Location**: Select **East US** from the list.

  - **Параметры VPN-подключения**
    - **Connection type**: Select **Point-to-site**.
    - **Client Address Space**: Enter *172.16.201.0/24*. VPN-клиенты, подключающиеся к виртуальной сети с помощью этого подключения "точка — сеть", получают IP-адреса из указанного пула.
...
1. On the [Azure portal](https://portal.azure.com) menu or from the **Home** page, select **Create a resource**. Откроется страница **создания**.
...
1. To connect to your VNet, on the client computer, go to **VPN connections** in the Azure portal and locate the VPN connection that you created. Название VPN-подключения совпадает с названием вашей виртуальной сети. Нажмите кнопку **Подключиться**. Если появится всплывающее сообщение о сертификате, выберите **Продолжить**, чтобы использовать более высокий уровень привилегий.
...
3. Go to the classic virtual network. Выберите **VPN-подключение типа "точка — сеть"** , а затем щелкните **Управление сертификатами**, чтобы открыть страницу **Сертификаты**.
