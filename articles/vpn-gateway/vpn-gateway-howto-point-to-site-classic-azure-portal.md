...
- **Создание параметров виртуальной сети (классической)**
   ...
   - **Название подсети**: Введите *FrontEnd*.
   - **Диапазон адресов subnet**: Введите *192.168.1.0/24*.
   ...
   - **Расположение**: Выберите **Восток США** из списка.

  - **Параметры VPN-подключения**
    - **Тип подключения**: Выберите **пункт к месту**.
    - **Пространство адреса клиента**: Введите *172.16.201.0/24*. VPN-клиенты, подключающиеся к виртуальной сети с помощью этого подключения "точка — сеть", получают IP-адреса из указанного пула.

- **Параметры подсети конфигурации шлюза**
   - **Имя**: Автозаполненный с *GatewaySubnet*.
   - **Диапазон адресов**: Введите *192.168.200.0/24*. 

- **Настройки конфигурации шлюза:**
   - **Размер**: Выберите шлюз SKU, который вы хотите использовать.
   - **Тип реуктора**: Выберите **динамический**.
...
1. В меню [портала Azure](https://portal.azure.com) или на **главной** странице выберите **«Создайте ресурс».** Откроется страница **создания**.
...
### <a name="part-2-create-a-gateway-subnet-and-a-dynamic-routing-gateway"></a>Часть 2: Создание подсети шлюза и динамического шлюза для разгрома
...
1. На портале Azure перейдите в виртуальную сеть, для которой требуется создать шлюз.
...
7. Выберите **размер** шлюза. Размер — это SKU шлюза для шлюза виртуальной сети. На портале Azure SKU по умолчанию — **По умолчанию**. Для получения дополнительной информации о шлюзах SKUs, [см.](vpn-gateway-about-vpn-gateway-settings.md#gwsku)
...
Чтобы создать подключение типа "точка — сеть" на другом клиентском компьютере, отличном от того, который использовался для создания сертификатов клиентов, установите сертификат клиента. При установке сертификата клиента потребуется пароль, созданный при экспорте сертификата клиента. Как правило, можно установить сертификат, просто дважды щелкнув его. Для получения дополнительной информации [см.](vpn-gateway-certificates-point-to-site.md#install)
...
1. Чтобы подключиться к Вашему VNet, на клиентском компьютере, перейдите на **VPN-соединения** на портале Azure и найдите созданное соединение VPN. Название VPN-подключения совпадает с названием вашей виртуальной сети. Выберите **Подключиться**. Если появится всплывающее сообщение о сертификате, выберите **Продолжить**, чтобы использовать более высокий уровень привилегий.
...
3. Перейти к классической виртуальной сети. Выберите **VPN-подключение типа "точка — сеть"**, а затем щелкните **Управление сертификатами**, чтобы открыть страницу **Сертификаты**.
...
6. На странице **Отпечаток** вставьте отпечаток сертификата в виде одной непрерывной текстовой строки без пробелов. Выберите **OK** для завершения.
