...
1. Обновите **девсеттингс. plist** , чтобы включить следующие записи конфигурации, используя собственные значения из подготовленного центра уведомлений:
   ...
   | Ключ                            | Type                     | Значение                     |
   |--------------------------------| -------------------------| --------------------------|
   | нотификатионхубкэй             | Строка,                   | @no__t 0hubKey >                  |
   | нотификатионхубкэйнаме         | Строка,                   | @no__t 0hubKeyName >              |
   | нотификатионхубнаме            | Строка,                   | @no__t 0hubName >                 |
   | нотификатионхубнамеспаце       | Строка,                   | @no__t 0hubNamespace >            |
   ...
   Чтобы не усложнять его, укажите `DefaultFullSharedAccessSignature`, чтобы можно было использовать маркер для отправки уведомлений. На практике `DefaultListenSharedAccessSignature` будет лучшим выбором для ситуаций, когда вы хотите получать только уведомления.

1. В **навигаторе по проекту**выберите **имя проекта** и перейдите на вкладку **Общие** .

1. Найдите **удостоверение** и задайте значение **идентификатора пакета** таким образом, чтобы оно совпадало с `com.<organization>.PushDemo`, которое является значением, используемым для **идентификатора приложения** из предыдущего шага.
...
1. Xcode должен автоматически извлечь соответствующее значение **профиля подготовки** на основе **идентификатора пакета**. Если значение нового **профиля подготовки** не отображается, попробуйте обновить профили для **удостоверения подписывания** , выбрав **Xcode** > **предпочтения** > **Account** > **Просмотреть сведения**. Выберите **удостоверение подписывания**, а затем нажмите кнопку **Обновить** в правом нижнем углу, чтобы скачать профили.
...
1. В Xcode выберите **файл** > **Новый**файл  > **файл** **заголовка** > . Назовите файл заголовка **бридгингхеадер. h**.
...
1. Обновите **параметры сборки** целевого объекта, чтобы они ссылались на заголовок моста:
   ...
   1. Введите путь к файлу `'<ProjectName>/BridgingHeader.h'` в параметре **заголовка "цель-C** ". Это путь к файлу для заголовка моста.
