...
3. Введите соответствующее **имя проекта** и нажмите кнопку **Create** (Создать).
4. Щелкните значок уведомления (колокольчик) на панели инструментов и выберите сообщение **Создание проекта**.

    ![Уведомление о создании проекта](media/notification-hubs-chrome-get-started/project-creation-notification.png)
5. Запишите номер созданного проекта из поля **Project Number** (Номер проекта) на странице **Projects** (Проекты). Используйте этот номер в качестве **идентификатора отправителя GCM** в приложении Chrome для регистрации в GCM.

    ![Консоль Google Cloud: поле Project Number (Номер проекта)](media/notification-hubs-chrome-get-started/gcm-project-number.png)
6. На панели мониторинга выберите **Перейти к обзору API**.

    ![Кнопка "Перейти к обзору API"](media/notification-hubs-chrome-get-started/go-to-apis-overview-button.png)
7. На странице "API и сервисы" выберите **Включить API и сервисы**.

    ![Включение API и сервисов](media/notification-hubs-chrome-get-started/enable-apis-and-services.png)
8. Выполните поиск по списку, введя ключевое слово **cloud messaging**. Выберите **Google Cloud Messaging** в отфильтрованном списке.

    ![API Google Cloud Messaging](media/notification-hubs-chrome-get-started/google-cloud-messaging-api.png)
9. На странице **Google Cloud Messaging** нажмите кнопку **Включить**.

    ![Включение GCM](media/notification-hubs-chrome-get-started/enable-gcm.png)
10. На странице **API и сервисы** переключитесь на вкладку **Учетные данные**. Нажмите кнопку **Создать учетные данные** и выберите **Ключ API**.

    ![Кнопка создания ключа API](media/notification-hubs-chrome-get-started/create-api-key-button.png)
11. В диалоговом окне **Ключ API создан** нажмите кнопку копирования, чтобы скопировать ключ в буфер обмена. Сохраните его где-либо. Используйте это значение для настройки центра уведомлений в следующем разделе, чтобы он мог отправлять push-уведомления в GCM.
...
&emsp;&emsp;6. Выберите **Google (GCM)** в категории **Параметры уведомления**, введите **ключ API** для проекта GCM и щелкните **Сохранить**.

&emsp;&emsp;&emsp;&emsp;![Центры уведомлений Azure — Google (GCM)](media/notification-hubs-chrome-get-started/configure-gcm-api-key.png)
...
> [!NOTE]
> Push-уведомления можно отправлять с помощью Центров уведомлений с любого сервера через общедоступный [интерфейс REST](https://msdn.microsoft.com/library/windowsazure/dn223264.aspx). Больше примеров, включающих использование разных платформ, можно найти на [портале документации](https://azure.microsoft.com/documentation/services/notification-hubs/).
