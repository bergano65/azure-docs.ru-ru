...
10. В файле проекта **AppDelegate.m** добавьте следующую инструкцию `import`:

    ```objc
    #import "Constants.h"
    ```

11. В том же файле **AppDelegate.m** замените весь код после `didFinishLaunchingWithOptions` следующим:
...
2. Кроме того, в **ViewController.h** добавьте следующее объявление свойства после объявления `@interface`:
