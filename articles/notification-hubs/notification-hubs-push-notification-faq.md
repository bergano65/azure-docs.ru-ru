---
title: Центры уведомлений Azure часто задаваемые вопросы (часто задаваемые вопросы) Документы Майкрософт
description: Часто задаваемые вопросы о разработке и внедрении решений в центрах уведомлений Azure.
...
* **Multi-tenancy**: Вы можете работать с учетными данными системы уведомлений Platform на уровне пространства имен. Эта возможность позволяет легко распределять между клиентами узлы в одном пространстве имен.
* **Запланированная отправка**. Вы можете запланировать отправку уведомлений в любое время.
* **Массовые операции**: Включает в себя регистрации Экспорт/ Импорт функциональность, описанная в [регистрации Экспорт / Импорт] документа.
...
> [!NOTE]
> Поскольку push-уведомления зависят от сторонних систем уведомления платформы, таких как служба push Notification Service от Apple (APNs) и Cloud Messaging от Google Firebase (FCM), нет гарантии SLA для доставки этих сообщений. После того как Центр уведомлений передаст пакеты системам отправки уведомлений платформы (с поддержкой соглашений об уровне обслуживания), непосредственная доставка push-уведомлений возлагается на эти системы (без поддержки соглашений об уровне обслуживания).
...
Перейдите на **[портал]** > Azure Уведомления**концентраторов имен** или **концентратов уведомлений.** Выберите ресурс, который требуется обновить, и перейдите на **страницу цен**. Обратите внимание на следующие требования:
...
Если вам нужна поддержка более чем 10 миллионов зарегистрированных устройств, необходимо переехать устройства на несколько концентраторов.
...
> [!NOTE]
> Общая стоимость использования может увеличиваться в зависимости от количества отправленных push-уведомлений. Ознакомьтесь с ограничениями для категорий на странице [с ценами на Центры уведомлений].
...
Географическое распределение не всегда является критически важным для push-уведомлений. Различные PNSes (например, APN или FCM), которые доставляют push-уведомления на устройства, распределяются неравномерно.
...
> [!NOTE]
> Концентраторы уведомлений Azure не регистрирует полезную нагрузку сообщений.
...
Мы предоставляем аварийное восстановление метаданных со своей стороны (имя Центра уведомлений, строка подключения и другие важные данные). В случае аварийного восстановления данные регистрации являются *единственным утерянным сегментом* инфраструктуры Центров уведомлений. Необходимо реализовать решение для заселения этих данных в новый концентратор после восстановления:
...
### <a name="is-all-of-my-data-stored-in-encrypted-form"></a>Все ли мои данные хранятся в зашифрованном виде?

Концентраторы уведомлений Azure шифруют все данные клиентов в состоянии покоя, за исключением регистрационных тегов. По этой причине не следует хранить личные или конфиденциальные данные с помощью тегов.
...
Вы также можете программно получить доступ к метрикам. Дополнительные сведения см. в следующих статьях:

- [Извлекайте метрики Azure Monitor с помощью .NET](https://azure.microsoft.com/resources/samples/monitor-dotnet-metrics-api/). В этом примере используется имя пользователя и пароль. Чтобы использовать сертификат, перегрузите метод FromServicePrincipal, чтобы предоставить сертификат, как показано в [этом примере.](https://github.com/Azure/azure-libraries-for-net/blob/master/src/ResourceManagement/ResourceManager/Authentication/AzureCredentialsFactory.cs) 
- [Получение метрик и журналов активности для ресурса](https://azure.microsoft.com/resources/samples/monitor-dotnet-query-metrics-activitylogs/)
- [Пошаговое руководство по REST API Azure Monitor](../azure-monitor/platform/rest-api-walkthrough.md)

> [!NOTE]
> Успешные уведомления означают просто, что push-уведомления были доставлены во внешние PNS (например, APNs для iOS и macOS или FCM для устройств Android). Службы PNS отвечают за доставку уведомлений на целевые устройства. Как правило, служба PNS не предоставляет метрики доставки третьим лицам.  
