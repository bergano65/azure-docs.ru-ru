...
* **Конечная точка**: это переключатель в колонке портала концентраторов уведомлений и строковое поле в API. Допустимые значения: `https://api.push.apple.com` или `https://api.sandbox.push.apple.com`. Центры уведомлений используют это значение для среды рабочего или песочницы для отправки уведомлений. Это должно совпадать с `aps-environment` назначением в приложении, в противном случае созданные маркеры устройств APNs не соответствуют среде, и уведомления не будут отправлены.
