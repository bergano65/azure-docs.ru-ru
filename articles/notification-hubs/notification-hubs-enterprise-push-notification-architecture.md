---
title: Архитектура энтерийных центров уведомлений нажмите
description: Узнайте об использовании концентраторов уведомлений Azure в корпоративной среде
...
Одно из наиболее распространенных требований — возможность отправки push-уведомлений пользователям с помощью мобильных приложений при определенных событиях в серверной системе. Например, клиент банка, у которого есть банковское приложение банка на iPhone, хочет получить уведомление, когда дебет производится выше определенной суммы со счета или интрасети, где сотрудник финансового отдела, у которого есть приложение для утверждения бюджета на Windows Phone, хочет быть уведомлены, когда запрос на утверждение получен.
...
2. **ReceiveAndSendNotification**
    ...
    а. Этот проект использует *пакеты WindowsAzure.ServiceBus* и **Microsoft.Web.WebJobs.Publish** NuGet и основан на [программировании Service Bus Pub/Sub.]
...
2. Выполнить **EnterprisePushMobileApp**, который запускает приложение Магазина Windows.
...
[Сервисный автобус Azure]: https://azure.microsoft.com/documentation/articles/fundamentals-service-bus-hybrid-solutions/
