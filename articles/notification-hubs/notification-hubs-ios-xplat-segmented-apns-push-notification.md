...
Эта статья основана на приложении, созданном в [учебнике: Push-уведомления для приложений iOS с помощью центров уведомлений Azure][get-started]. Прежде чем приступить к работе с этим руководством, необходимо уже пройти [Учебник: Push-уведомления в приложения iOS с помощью центров уведомлений Azure][get-started].
...
10. Следующие методы уже должны присутствовать в `AppDelegate.m` в учебнике начало [работы с центрами уведомлений][get-started] . В противном случае добавьте их.
...
Как правило, уведомления отправляются серверной службой, но вы можете отправлять уведомления об экстренных новостях непосредственно из приложения. Для этого необходимо обновить метод `SendNotificationRESTAPI`, определенный в учебнике [Приступая к работе с центрами уведомлений][get-started] .
