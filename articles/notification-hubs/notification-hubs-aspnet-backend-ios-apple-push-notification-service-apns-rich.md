...
2. Щелкните **Возможности**, включите **Background Modes** (Фоновые режимы) и установите флажок **Remote Notifications** (Удаленные уведомления).
    ...
    ![][IOS3]
3. Перейдите к файлу **Main.storyboard**и убедитесь, что у вас есть контроллер представления (который в этом руководстве называется контроллером домашнего представления), созданный при работе с руководством [Уведомление пользователя](notification-hubs-aspnet-backend-ios-apple-apns-notification.md).
