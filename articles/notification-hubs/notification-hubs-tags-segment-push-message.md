...
На рисунке сообщение с меткой `user_Alice` приступает ко всем устройствам, помеченным `user_Alice`.
...
Выражения тегов поддерживают общие логические операторы, такие как `AND` (`&&`), `OR` (`||`) и `NOT` (`!`). Они также могут содержать круглые скобки. Выражения тегов, использующие только операторы `OR`, могут ссылаться на 20 тегов. в противном случае выражения тегов ограничены 6 тегами.
