...
Установка этого значения равным 10 больше не разрешена для фоновых уведомлений, и необходимо задать значение для каждого запроса. Apple не будет предоставлять фоновые уведомления, если это значение отсутствует. Вот несколько примеров:
