...
Для серверов, поддерживающих максимальный объем хранилища до 4 ТБ, полные резервные копии выполняются каждую неделю. Разностное резервное копирование выполняется дважды в день. Резервные копии журналов транзакций происходят каждые пять минут.


#### <a name="servers-with-up-to-16-tb-storage"></a>Серверы с хранилищем объемом до 16 ТБ

В подмножестве [регионов Azure](https://docs.microsoft.com/azure/postgresql/concepts-pricing-tiers#storage)все новые подготовленные серверы могут поддерживать до 16 ТБ хранилища. Резервное копирование на этих больших серверах хранилища основано на моментальных снимках. Первая полная резервная копия моментального снимка запланирована сразу же после создания сервера. Первая полная резервная копия моментальных снимков сохраняется как базовая резервная копия сервера. Последующие резервные копии моментальных снимков — это только разностные резервные копии. 

Разностные резервные копии моментальных снимков выполняются по крайней мере один раз в день. Разностные резервные копии моментальных снимков не выполняются по фиксированному расписанию. Разностные резервные копии моментальных снимков выполняются каждые 24 часа, если журнал транзакций (бинлог в MySQL) превышает 50 ГБ с момента создания последней разностной резервной копии. В день разрешено не более шести разностных моментальных снимков. 

Резервные копии журналов транзакций происходят каждые пять минут. 
...
- Устаревшие серверы с хранилищем объемом 4 ТБ будут хранить до 2 полных резервных копий базы данных, все разностные резервные копии и резервные копии журналов транзакций, выполненные с момента самой ранней полной резервной копии базы данных.
-   Серверы с большим объемом хранилища (16 ТБ) будут хранить полный моментальный снимок базы данных, все разностные моментальные снимки и резервные копии журналов транзакций за последние 8 дней.
