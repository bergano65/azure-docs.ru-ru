...
> [!NOTE]
>
> Группы серверов масштаба, работающие в более старых версиях Цитус Engine, могут не предлагать все перечисленные ниже таблицы.
...
| хасметадата      | boolean | Зарезервировано для внутреннего использования.                                                                                                                                                                 |
| IsActive         | boolean | Является ли узел активным принятием размещений сегментов.                                                                                                                                     |
| нодероле         | текст    | Является ли узел первичным или вторичным                                                                                                                                                 |
| нодеклустер      | текст    | Имя кластера, содержащего этот узел                                                                                                                                               |
| шаулдхавешардс | boolean | Если значение равно false, сегменты будут перемещены из узла (в стоке) при повторном распределении, а сегменты из новых распределенных таблиц не будут помещены на узел, если они не расположены вместе с сегментами. |
...
| type                        | текст    | Часть стабильного адреса, используемого во время обновления PG   |
...
| Имя                           | Тип    | Описание                                                                                                                                       |
|--------------------------------|---------|---------------------------------------------------------------------------------------------------------------------------------------------------|
| default_strategy               | boolean | Следует ли rebalance_table_shards по умолчанию выбирать эту стратегию. Использовать citus_set_default_rebalance_strategy для обновления этого столбца             |
...
* Узнайте, как некоторые [функции масштабирования](reference-hyperscale-functions.md) меняют системные таблицы
