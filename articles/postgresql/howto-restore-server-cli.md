---
title: Резервное копирование и восстановление - Azure CLI - База данных Azure для PostgreS'L - Единый сервер
description: Узнайте, как настроить конфигурации резервного копирования и восстановить сервер в базе данных Azure для PostgreS-L - Single Server с помощью Azure CLI.
author: rachel-msft
ms.author: raagyema
...
---
# <a name="how-to-back-up-and-restore-a-server-in-azure-database-for-postgresql---single-server-using-the-azure-cli"></a>Как создать резервную связь и восстановить сервер в базе данных Azure для PostgreS'L - Единый сервер с помощью Azure CLI
...
- [База данных Azure для сервера и базы данных PostgreS'L](quickstart-create-server-database-azure-cli.md)
...
После завершения восстановления найдите новый сервер, чтобы убедиться, что данные были восстановлены, как и ожидалось. Новый сервер имеет то же имя входа администратора сервера и пароль, который был действителен для существующего сервера на момент начала восстановления. Пароль можно изменить на странице **Обзор** для нового сервера.

Новый сервер, созданный во время восстановления, не имеет правил брандмауэра или конечных точек службы VNet, существовавших на исходном сервере. Эти правила должны быть настроены отдельно для этого нового сервера.
...
- Узнайте больше о [резервных предложениях службы](concepts-backup.md)
- Узнать о [репликах](concepts-read-replicas.md)
- Узнайте больше о вариантах [непрерывности бизнеса](concepts-business-continuity.md)
