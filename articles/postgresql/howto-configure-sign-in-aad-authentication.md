...
Только пользователи с правами администратора Azure AD могут создавать и включать пользователей для проверки подлинности на основе Azure AD. Не рекомендуется использовать администратора Azure AD для обычных операций с базами данных, так как он обладает повышенными разрешениями пользователя (например, CREATEDB).

Чтобы задать администратора Azure AD (вы можете использовать пользователя или группу), выполните следующие действия.
...
> [!IMPORTANT]
> При настройке администратора новый пользователь добавляется на сервер базы данных Azure для PostgreSQL с полными разрешениями администратора. Пользователь с правами администратора Azure AD в базе данных Azure для PostgreSQL будет иметь `azure_ad_admin`роль.
...
При использовании клиента `psql` командной строки маркер доступа должен передаваться через переменную `PGPASSWORD` среды, так как маркер доступа превышает длину пароля, которую `psql` можно принимать напрямую:
...
3. Создание роли `<user>@yourtenant.onmicrosoft.com` в базе данных Azure для PostgreSQL.
4. Сделайте `<user>@yourtenant.onmicrosoft.com` член роли azure_ad_user. Эта возможность должна быть предоставлена только пользователям Azure AD.
...
В маловероятном случае, если существующие пользователи уже соответствуют именам пользователей Azure AD, вы можете предоставить им `azure_ad_user` роль, чтобы включить их для проверки подлинности Azure AD:
