...
Чтобы разрешить доступ к единственному IP-адресу, укажите один и тот же адрес в `--start-ip-address` и `--end-ip-address` , как в этом примере, заменив IP, показанный здесь, на свой IP.
```azurecli-interactive
