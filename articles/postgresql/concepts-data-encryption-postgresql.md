...
> [!NOTE]
> В настоящее время для использования этой возможности требуется запросить соответствующие права доступа. Для этого обратитесь по адресу AskAzureDBforPostgreSQL@service.microsoft.com.
...
Шифрование данных для отдельного сервера базы данных Azure для PostgreSQL предоставляет следующие преимущества.
...
* Key Vault и отдельный сервер базы данных Azure для PostgreSQL должны принадлежать одному клиенту Azure Active Directory (Azure AD). Взаимодействие между хранилищем ключей и сервером, размещенными в разных клиентах, не поддерживается. При последующем перемещении ресурсов требуется перенастроить шифрование данных.
