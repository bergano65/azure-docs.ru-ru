...
description: Шифрование данных одного сервера в базе данных Azure для PostgreSQL с помощью ключа, управляемого клиентом
author: kummanish
ms.author: manishku
...
Key Vault администратор может также [включить ведение журнала событий Key Vault аудита](https://docs.microsoft.com/azure/azure-monitor/insights/azure-key-Vault), чтобы их можно было проследить позднее.
...
После того как база данных Azure для PostgreSQL с одним сервером шифруется с помощью управляемого ключа клиента, хранящегося в Key Vault, любая созданная копия сервера, как и локальное, либо геовосстановление, или реплики чтения, также шифруется с помощью того же клиента. управляемый ключ. Однако их можно изменить в соответствии с управляемым ключом нового клиента для шифрования. При изменении ключа, управляемого клиентом, старые резервные копии сервера начнут использовать последний ключ.
...
Узнайте, как настроить шифрование данных с помощью управляемого клиентом ключа для базы данных Azure для PostgreSQL на одном сервере с использованием [портал Azure](howto-data-encryption-portal.md).
