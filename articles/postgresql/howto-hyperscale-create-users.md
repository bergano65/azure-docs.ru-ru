...
Поскольку масштабирование является управляемой службой PaaS, только корпорация Майкрософт может войти с помощью роли суперпользователя `postgres`. Для ограниченного административного доступа масштабирование предоставляет роль `citus`.

Разрешения для роли `citus`:

* Чтение всех переменных конфигурации, даже переменных, обычно видимых только для пользователей.
* Чтение всех представлений PG\_stat\_\* и использование различных расширений, связанных с статистикой — даже представлений или расширений, обычно видимых только для пользователей.
...
Особенно, роль `citus` имеет некоторые ограничения.
...
Как уже упоминалось, учетная запись администратора `citus` не имеет разрешения на создание дополнительных пользователей. Чтобы добавить пользователя, используйте интерфейс портал Azure.
...
Пользователь будет создан на узле координатора группы серверов и распространен на все рабочие узлы. Роли, созданные с помощью портал Azure, имеют атрибут `LOGIN`, что означает, что они являются настоящими пользователями, которые могут входить в базу данных.
