...
Вы можете легко масштабировать свой уровень вычислений, виртуальных ядер и хранилище с помощью следующей команды. Вы можете просмотреть все операции сервера, которые можно выполнить с помощью команды [AZ postgres гибкого сервера Server Overview](/cli/azure/PostgreSQL/server) .
