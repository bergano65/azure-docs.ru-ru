...
Создайте гибкий сервер с помощью команды [az postgreSQL flexible-server create](/cli/azure/postgresql/flexible-server?view=azure-cli-latest&preserve-view=true). Следующая команда создает сервер, используя параметры и значения по умолчанию для локального контекста Azure CLI:
