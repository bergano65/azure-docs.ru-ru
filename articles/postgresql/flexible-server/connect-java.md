---
title: Краткое руководство. Использование Java и JDBC с Гибким сервером Базы данных Azure для PostgreSQL
description: Из этого краткого руководства вы узнаете, как использовать Java и JDBC с Гибким сервером Базы данных Azure для PostgreSQL.
author: mksuni
ms.author: sumuth
...
ms.locfileid: "98606013"
---
# <a name="quickstart-use-java-and-jdbc-with-azure-database-for-postgresql-flexible-server"></a>Краткое руководство. Использование Java и JDBC с Гибким сервером Базы данных Azure для PostgreSQL

В этой статье показано, как создать пример приложения, которое использует Java и [JDBC](https://en.wikipedia.org/wiki/Java_Database_Connectivity) для сохранения данных на [Гибком сервере Базы данных Azure для PostgreSQL](./index.yml) и их извлечения из нее.
...
> [!NOTE]
> См. сведения о [создании сервера Базы данных Azure для PostgreSQL с помощью портала Azure](./quickstart-create-server-portal.md).
...
База данных **postgres** на созданном вами ранее сервере PostgreSQL пуста. Для работы с этим кратким руководством мы создадим базу данных с именем `demo`, выполнив следующие действия:

1. Установите локальный экземпляр [psql](https://www.postgresql.org/docs/current/static/app-psql.html), чтобы подключиться к серверу PostgreSQL Azure. 
2. Чтобы подключиться к базе данных **postgres** на своем сервере PostgreSQL, выполните следующую команду psql:
...
3. Создайте новую базу данных с именем **demo**. Для этого в командной строке введите следующую команду:
