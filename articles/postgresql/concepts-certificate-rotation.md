...
Новый сертификат будет использоваться начиная с 26 октября 2020 г. (10/26/2020). При использовании проверки ЦС или полной проверки сертификата сервера при подключении из клиента PostgreSQL (sslmode = Verify-CA или sslmode = Verify-Full) необходимо обновить конфигурацию приложения до 26 октября 2020 г. (10/26/2020).

## <a name="how-do-i-know-if-my-database-is-going-to-be-affected"></a>Разделы справки понять, будет ли затронута моя база данных?

Все приложения, использующие SSL/TLS и проверяющие корневой сертификат, должны обновить корневой сертификат для подключения к базе данных Azure для PostgreSQL. Если вы не используете SSL/TLS сейчас (sslmode = Disable) или не проверяете сертификат сервера (sslmode = Allow, sslmode = предпочитать или sslmode = обязательно), это не повлияет на доступность приложения. Вы можете проверить, пытается ли клиентское приложение использовать режим SSL с предопределенным доверенным центром сертификации (CA) [здесь](concepts-ssl-connection-security.md#enforcing-tls-connections).
...
Если вы не обновите корневой сертификат до 26 октября 2020, то приложения, подключающиеся по протоколу SSL/TLS и выполняющие проверку корневого сертификата, не смогут взаимодействовать с сервером базы данных PostgreSQL, и приложение будет испытывать проблемы с подключением к серверу базы данных PostgreSQL.
