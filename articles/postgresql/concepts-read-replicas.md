...
В Azure Monitor доступна метрика **Max Lag across Replicas** (Максимальная задержка между репликами). Эта метрика доступна только на главном сервере. Она показывает задержку между главной репликой и репликой с наибольшей задержкой в байтах. 
...
Если вы решили остановить репликацию между основным сервером и репликой, для применения таких изменений реплика будет перезапущена. Затем реплика станет сервером для чтения и записи. Вы не сможете снова преобразовать этот сервер в реплику.
