...
* Доступность по регионам. база данных Azure для PostgreSQL доступна в западной части США 2, центральном Франции, Северной и Германии. Однако их парные регионы недоступны.
...
Если вы попытаетесь обновить значения сервера, описанные выше, но не соблюдаете ограничения, появится сообщение об ошибке.
...
[PostgreSQL требует](https://www.postgresql.org/docs/current/runtime-config-resource.html#GUC-MAX-PREPARED-TRANSACTIONS) , чтобы значение параметра `max_prepared_transactions` в реплике чтения было больше или равно главному значению. в противном случае реплика не будет запущена. Если вы хотите изменить `max_prepared_transactions` в главной базе, сначала измените его на репликах.
