...
При отмене репликации реплика теряет все ссылки на предыдущие главные и другие реплики. Автоматическая отработка отказа между главной и репликой отсутствует. 
...
[PostgreSQL требует](https://www.postgresql.org/docs/10/runtime-config-resource.html#GUC-MAX-PREPARED-TRANSACTIONS) , чтобы значение `max_prepared_transactions` параметра в реплике чтения было больше или равно главному значению; в противном случае реплика не будет запущена. Если вы хотите изменить `max_prepared_transactions` на главном мастере, сначала измените его на репликах.
