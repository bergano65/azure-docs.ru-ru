...
Виртуальных ядер и ценовую категорию также можно изменить в реплике в следующих случаях.
* Postgres требует, чтобы параметр `max_connections` на реплике имел значение не ниже, чем на главном сервере. В противном случае реплика не запустится. В базе данных Azure для PostgreSQL `max_connections` значение параметра основано на номере SKU (виртуальных ядер и ценовая категория). Дополнительные сведения см. в статье [Ограничения в базе данных Azure для PostgreSQL](concepts-limits.md). 
* Масштабирование до или из ценовой категории "базовый" не поддерживается
