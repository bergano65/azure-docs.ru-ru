---
title: Частная ссылка - Портал Azure - База данных Azure для PostgreS'L - Единый сервер
description: Узнайте, как настроить частную ссылку для базы данных Azure для почтового сервера PostgreS-L с портала Azure
author: kummanish
ms.author: manishku
...
---
# <a name="create-and-manage-private-link-for-azure-database-for-postgresql---single-server-using-portal"></a>Создание и управление частной ссылкой для базы данных Azure для PostgreS-L - Единый сервер с помощью portal

Частная конечная точка — ключевой компонент для построения частной ссылки в Azure. Это позволяет ресурсам Azure, таким как виртуальные машины (VM), обмениваться данными в частном порядке с ресурсами частной ссылки.  В этой статье вы узнаете, как использовать портал Azure для создания VM в виртуальной сети Azure и базу данных Azure для одного сервера PostgreS'L с частной конечной точкой Azure.
...
В этом разделе вы создадите виртуальную сеть и подсеть для размещения VM, который используется для доступа к ресурсу Private Link (сервер PostgreS'L в Azure).
...
## <a name="create-an-azure-database-for-postgresql-single-server"></a>Создание базы данных Azure для единого сервера PostgreS'L

В этом разделе будет создана база данных Azure для сервера PostgreS'L в Azure. 

1. На верхней левой стороне экрана на портале Azure выберите **Создать базу** > **данных ресурсов** > **Azure Database для PostgreS'L.**

1. В **azure Database для опции развертывания PostgreS'L**выберите **единый сервер** и предоставьте эту информацию:
    ...
    | Расположение | Выберите область Azure, где вы хотите, чтобы ваш сервер PostgreS'L проживал. |
    |Версия  | Выберите требуемую версию базы данных сервера PostgreS'L.|
...
В этом разделе вы создадите сервер PostgreS'L и добавите к нему частную конечную точку. 
...
6. В окне **Создание частной конечной точки — Ресурс** введите или выберите следующую информацию:
    ...
    | Подписка| Выберите свою подписку. |
    | Тип ресурса | Выберите **серверы Microsoft.DBforPostgreS'L/** |
...
8. При **создании частной конечной точки - Конфигурация**введите или выберите эту информацию:
    ...
    |Частная зона DNS |Выберите *(Новый)privatelink.postgres.database.azure.com* |
    |||

1. Выберите **Review + create** (Просмотреть и создать). Вы будете перенаправлены на страницу **Просмотр и создание**, где Azure проверит вашу конфигурацию. 
2. При походе пройденое сообщение **«Проверка»** выберите **«Создать».** 

    ![Создана частная ссылка](media/concepts-data-access-and-security-private-link/show-postgres-private-link.png)
...
Когда вы закончите использовать частную конечную точку, сервер PostgreS'L и VM, удалите группу ресурсов и все ресурсы, которые она содержит:
...
В этом порядке вы создали VM в виртуальной сети, базу данных Azure для PostgreS-L - Единый сервер и частную конечную точку для частного доступа. Вы подключились к одному VM из Интернета и надежно передавались на сервер PostgreS'L с помощью Private Link. Чтобы узнать больше о частных конечных точках, смотрите [Что такое частная конечная точка Azure.](https://docs.microsoft.com/azure/private-link/private-endpoint-overview)
