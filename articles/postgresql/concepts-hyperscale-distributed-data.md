...
В этой статье описываются три табличных типа в базе данных Azure для PostgreSQL – Цитус (Предварительная версия).
...
Если узел координатора хочет определить, какой сегмент содержит строку `github_events`, он хэширует значение столбца распределения в строке. Затем узел проверяет, какой диапазон сегментов @ no__t-нулей содержит хэшированное значение. Диапазоны определяются таким образом, чтобы изображение хэш-функции было несвязанным объединением.
