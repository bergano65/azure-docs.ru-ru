---
title: Магазин запросов - База данных Azure для PostgreS'L - Единый сервер
description: В этой статье описывается функция магазина запросов в базе данных Azure для PostgreS-L - Single Server.
author: rachel-msft
ms.author: raagyema
...
4. Установите значение `TOP` и **сохранить**.

Для включения статистики ожидания в магазин запросов: 
1. Найдите параметр `pgms_wait_sampling.query_capture_mode`.
1. Установите значение `ALL` и **сохранить**.


Кроме того, эти параметры можно установить с помощью Azure CLI.
...
## <a name="access-query-store-information"></a>Доступ к информации о магазине запросов

Данные магазина запросов хранятся в базе данных azure_sys на сервере Postgres. 
...
Вы также можете излучать данные хранилища запросов в [журналы Azure Monitor](../azure-monitor/log-query/log-query-overview.md) для аналитики и оповещения, концентраторы событий для потоковой передачи и хранилище Azure для архивирования. Категории журнала для **настройки являются queryStoreRuntimeStatistics** и **queryStoreWaitStatistics**. Чтобы узнать о настройках, посетите статью [диагностических настроек Azure Monitor.](../azure-monitor/platform/diagnostic-settings.md)
...
- [Прочитайте реплики,](concepts-read-replicas.md) реплицируют данные магазина запросов с основного сервера. Это означает, что магазин запросов с читлями не предоставляет статистических данных о запросах, запускаемых на реплике чтения.
