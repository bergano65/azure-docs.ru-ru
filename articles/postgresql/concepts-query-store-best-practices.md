---
title: Рекомендации по хранению запросов в базе данных Azure для PostgreS'L - Единый сервер
description: В этой статье описаны рекомендации для магазина запросов в базе данных Azure для PostgreS-L - Single Server.
author: rachel-msft
ms.author: raagyema
...
|---|---|
|_Все_  |Тщательный анализ рабочей нагрузки с учетом всех запросов, периодичности их выполнения и других статистических данных. Выявление новых запросов в рабочей нагрузке. Узнать, используются ли специальные запросы для определения возможностей для пользовательской или автоматической параметризации. Режим _All_ предусматривает повышенные затраты на использование ресурсов. |
...
> [!NOTE] 
> **pg_qs.query_capture_mode** заменяет **pgms_wait_sampling.query_capture_mode**. Если pg_qs,query_capture_mode _нет,_ настройка pgms_wait_sampling.query_capture_mode не имеет никакого эффекта. 
