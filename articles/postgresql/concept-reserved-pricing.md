...
Например, предположим, что вы используете одну общую цель го поколения – 32 Виртуальное ядро PostgreSQL Database и два оптимизированных для памяти го поколения — 16 Виртуальное ядро PostgreSQL баз данных. Кроме того, предположим, что вы планируете развертывание в течение следующего месяца, а также еще одно общее назначение го поколения – 8 Виртуальное ядро Database Server и один сервер базы данных, оптимизированный для памяти го поколения – 32 Виртуальное ядро. Предположим, вам известно, что эти ресурсы понадобятся по крайней мере в течение одного года. В этом случае следует приобрести 40 (32 + 8) виртуальных ядер, одноразовое резервирование для отдельной базы данных общего назначения — го поколения и 64 (2x16 + 32) Виртуальное ядро один год для базы данных, оптимизированной для памяти — го поколения
...
Гибкость размеров виртуального ядра позволяет увеличивать или уменьшать масштаб в пределах уровня производительности и региона, не теряя преимуществ приобретенной резервной мощности. При масштабировании до более высокого виртуальных ядер, чем ваша зарезервированная емкость, вам будет выставлен счет за избыточную виртуальных ядер с использованием цен с оплатой по мере использования.
