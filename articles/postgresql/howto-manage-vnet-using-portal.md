---
title: Использование виртуальных сетевых правил - Портал Azure - База данных Azure для PostgreS'L - Единый сервер
description: Создавайте и управляйте конечными точками службы VNet и правилами базы данных Azure для PostgreS-L - Единый сервер с помощью портала Azure
author: bolzmj
ms.author: mbolz
...
---
# <a name="create-and-manage-vnet-service-endpoints-and-vnet-rules-in-azure-database-for-postgresql---single-server-by-using-the-azure-portal"></a>Создавайте и управляйте конечными точками службы VNet и правилами VNet в базе данных Azure для PostgreS-L - Единый сервер с помощью портала Azure
...
2. Убедитесь, что доступ к службам Azure настроен на **OFF.**

> [!Important]
> Если вы оставите набор управления В ON, ваш сервер базы данных Azure PostgreS'L принимает сообщение из любой подсети. что сделает доступ слишком незащищенным. Функция конечной точки службы Microsoft Azure Virtual Network в координации с функцией виртуального сетевого правила базы данных Azure Database для PostgreS'L может вместе уменьшить область вашей поверхности безопасности.
...
- Для получения помощи при подключении к базе данных Azure для сервера PostgreS'L см. [Библиотеки подключения для базы данных Azure для PostgreS'L](./concepts-connection-libraries.md)
