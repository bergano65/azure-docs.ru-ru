---
title: Руководство. Маршрутизация электрических транспортных средств с помощью Записных книжек Azure (Python) | Microsoft Azure Maps
...
ms.locfileid: "76844757"
---
# <a name="tutorial-route-electric-vehicles-by-using-azure-notebooks-python"></a>Руководство. Маршрутизация электрических транспортных средств с помощью Записных книжек Azure (Python)
...
Создайте подписку для учетной записи Azure Maps согласно [этим инструкциям](quick-demo-map-app.md#create-an-account-with-azure-maps). Требуется подписка на учетную запись Azure Maps с ценовой категорией S1. 
...
1. После успешной отправки на странице проекта появится ваш файл. Дважды щелкните файл, чтобы открыть его в виде записной книжки Jupyter Notebook.

Чтобы лучше понять функциональность файла записной книжки, выполняйте код в одной ячейке за раз. Вы можете выполнять код в каждой ячейке, нажимая кнопку **Запустить** в верхней части приложения записной книжки.
...
Чтобы выполнить код в записной книжке, необходимо установить пакеты на уровне проекта, выполнив шаги ниже:
...
После отправки данных в службу вызовите [службу получения изображения карты](https://docs.microsoft.com/rest/api/maps/render/getmapimage) в Azure Maps. Эта служба используется для отображения зарядных точек и максимальной доступной границы на статическом изображении карты. Для этого выполняется следующий скрипт:
