...
Служба преобразования [Azure Maps](https://docs.microsoft.com/rest/api/maps/data/conversion) может извлекать следующие классы функций из файла DWG:
...
* Самопересекающиеся многоугольники будут автоматически исправлены, а [служба преобразования Azure Maps](https://docs.microsoft.com/rest/api/maps/data/conversion) выдаст предупреждение. Рекомендуется вручную проверить исправления, так как они могут не соответствовать ожидаемым результатам.
...
В таблице ниже приведены поддерживаемые типы сущностей и поддерживаемые функции для каждого слоя. Если слой содержит неподдерживаемые типы сущностей, то [служба преобразования Azure Maps](https://docs.microsoft.com/rest/api/maps/data/conversion) пропустит эти сущности.  
...
Папка в ZIP-архиве должна содержать файл манифеста в корневом каталоге, а файл должен называться **manifest.json**. В нем описываются файлы DWG, что позволяет службе преобразования [Azure Maps](https://docs.microsoft.com/rest/api/maps/data/conversion) анализировать их содержимое. Будут приняты только файлы, указанные в манифесте. Файлы, находящиеся в папке в ZIP-архиве, но неправильно указанные в манифесте, будут игнорироваться.
...
Хотя при использовании объектов манифеста существуют требования, не все объекты являются обязательными. В таблице ниже показаны обязательные и необязательные объекты для версии 1.1 [службы преобразования Azure Maps](https://docs.microsoft.com/rest/api/maps/data/conversion).
...
|-----------|------|----------|-------------|
|levelName    |string/int    |Да |    Описательное имя уровня. Пример: Этаж 1, зал ожидания, синяя стоянка, цоколь и т. д.|
...
|heightAboveFacilityAnchor | NUMERIC |    false |    Высота уровня над первым этажом в метрах. |
| verticalExtent | NUMERIC | false | Высота от пола до потолка (толщина) уровня в метрах. |
|filename |    string/int |    Да |    Путь файловой системы к рисунку САПР для уровня здания. Он должен начинаться с коревого каталога ZIP-архива здания. |
...
|angle|    NUMERIC|    Да|   Угол по часовой стрелке (в градусах) между истинным Севером и вертикальной осью (Y) рисунка.   |

### <a name="dwglayers"></a>dwgLayers

| Свойство  | Тип | Обязательно | Описание |
|-----------|------|----------|-------------|
|exterior    |Массив значений типа string/int|    Да|    Имена слоев, определяющих экстерьер здания.|
|unit|    Массив значений типа string/int|    Да|    Имена слоев, определяющих секции.|
|wall|    Массив значений типа string/int    |false|    Имена слоев, определяющих стены.|
|door    |Массив значений типа string/int|    false   | Имена слоев, определяющих двери.|
|unitLabel    |Массив значений типа string/int|    false    |Имена слоев, определяющих имена секций.|
|зона | Массив значений типа string/int    | false    | Имена слоев, определяющих зоны.|
|zoneLabel | Массив значений типа string/int |     false |    Имена слоев, определяющих имена зон.|
...
| Свойство  | Тип | Обязательно | Описание |
|-----------|------|----------|-------------|
|unitName    |string/int    |Да    |Имя секции, связанное с этой записью `unitProperty`. Эта запись допустима, только если на слоях `unitLabel` обнаружена метка, соответствующая `unitName`. |
...
|nameAlt|    string/int|    false|    Альтернативное имя для секции. |
|nameSubtitle|    string/int    |false|    Подзаголовок для секции. |
|addressRoomNumber|    string/int|    false|    Номер комнаты, секции, квартиры или номера в секции.|
|verticalPenetrationCategory|    string/int|    false| Если это свойство определено, то итоговая функция будет "Вертикальный ввод", а не "Секция". Вертикальные проемы можно использовать для перехода к другим функциям вертикальных проемов на уровнях выше или ниже текущего. "Вертикальный ввод" — это имя [Категории](https://aka.ms/pa-indoor-spacecategories). Если это свойство определено, свойство categoryName переопределяется с помощью verticalPenetrationCategory. |
