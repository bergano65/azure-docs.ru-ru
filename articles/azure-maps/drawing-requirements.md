...
|-----------|------|----------|-------------|
|levelName    |string    |true |    Описательное имя уровня. Пример: Этаж 1, зал ожидания, синяя стоянка, цоколь и т. д.|
...
|filename |    string |    true |    Путь файловой системы к рисунку САПР для уровня здания. Он должен начинаться с коревого каталога ZIP-архива здания. |
...
|unit|    Массив строк|    true|    Имена слоев, определяющих секции.|
...
| Свойство  | Type | Обязательно | Описание |
|-----------|------|----------|-------------|
|unitName    |string    |Да    |Имя секции, связанное с этой записью `unitProperty`. Эта запись допустима, только если на слоях `unitLabel` обнаружена метка, соответствующая `unitName`. |
...
| Свойство  | Type | Обязательно | Описание |
|-----------|------|----------|-------------|
|zoneName        |string    |Да    |Имя зоны, связываемой с записью `zoneProperty`. Эта запись допустима, только если на слое `zoneLabel` зоны обнаружена метка, соответствующая `zoneName`.  |
|categoryName|    строка|    false    |Имя категории. Полный список категорий см. [здесь](https://aka.ms/pa-indoor-spacecategories). |
