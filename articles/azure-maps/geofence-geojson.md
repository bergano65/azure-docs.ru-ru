...
| recurrenceType | string | false |   Тип повторений для периода. Возможны следующие значения: `Daily`, `Weekly`, `Monthly` или `Yearly`. Значение по умолчанию: `Daily`.|
...
* Для каждого компонента, который содержит `MultiPoint`, `MultiLineString`, `MultiPolygon` или `GeometryCollection`, свойства применяются ко всем элементам. Например, все точки в `MultiPoint` будут использовать один и тот же радиус для формирования нескольких круговых геозоныов.
