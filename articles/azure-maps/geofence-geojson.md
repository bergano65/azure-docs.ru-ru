...
| recurrenceType | string | false |   Тип повторений для периода. Возможны следующие значения: `Daily`, `Weekly`, `Monthly` или `Yearly`. По умолчанию имеет значение `Daily`.|
| businessDayOnly | Boolean | false |  Указывает, что данные считаются допустимыми только в рабочие дни. По умолчанию имеет значение `false`.|
