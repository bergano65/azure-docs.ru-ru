...
description: Узнайте, как применять рекомендации для Служба поиска с помощью карт Microsoft Azure.
...
* Чтобы зависеть от области релевантности результатов, определите `lat` и `lon` параметры координат. Используйте параметр `radius`, чтобы задать радиус области поиска.
...
* Параметры `minFuzzyLevel` и `maxFuzzyLevel` помогают возвращать соответствующие соответствия даже в том случае, если параметры запроса не точно соответствуют необходимой пользователю информации. Чтобы повысить производительность и сократить необычные результаты, задайте для поисковых запросов значения по умолчанию `minFuzzyLevel=1` и `maxFuzzyLevel=2`. 
    ...
    Например, если для параметра `maxFuzzyLevel` задано значение 2, условие поиска *рестрантся* с *рестораном*. При необходимости можно переопределить нечеткие уровни по умолчанию. 

* Используйте параметр `idxSet`, чтобы задать приоритеты для точного набора типов результатов. Чтобы задать приоритеты для точного набора результатов, можно отправить разделенный запятыми список индексов. В списке порядок элементов не имеет значения. Azure Maps поддерживает следующие индексы:
    ...
    * `Geo` - **географических регионах**: административные подразделения земли. Географией может быть страна, штат или город, например.
    ...
    * `POI` - **интересующие вас моменты**: точки на карте, которые считаются важными для внимания или которые могут представлять интерес. [API адреса поиска](https://docs.microsoft.com/rest/api/maps/search/getsearchaddress) не возвращает интереса.  
    * `Str` - **улиц**: улиц на карте.
    * `XStr` - **кросс или**пересечением: точки соединения или места, где два пересечений улиц.


#### <a name="usage-examples"></a>Примеры использования

* `idxSet=POI` интереса только поиск. 

* `idxSet=PAD,Addr` — Поиск только адресов. `PAD` указывает адрес точки, а `Addr` указывает диапазон адресов.
...
Используйте параметр `language`, чтобы задать язык для возвращаемых результатов поиска. Если в запросе не задан язык, по умолчанию Служба поиска использует наиболее распространенный язык в стране или регионе. Если на указанном языке нет доступных данных, используется язык по умолчанию. 
...
Чтобы найти другие совпадения для частичных запросов, задайте для параметра `typeahead` значение `true`. Этот запрос интерпретируется как часть входных данных, а поиск переходит в прогнозирующий режим. Если не задать для параметра `typeahead` значение `true`, то служба предполагает, что все соответствующие сведения переданы.

В следующем примере запроса служба поиска адреса запрашивается для *Майкрософт*. Здесь для параметра `typeahead` задано значение `true`. Ответ показывает, что служба поиска интерпретирует запрос как частичный запрос. Ответ содержит результаты для автоматически предлагаемого запроса.
...
Чтобы найти перекрестные адреса, необходимо закодировать универсальный код ресурса (URI) для работы с специальными символами в адресе. Рассмотрим этот пример адреса: *1-ый дом & Union, Сиэтл*. В этом случае перед отправкой запроса необходимо закодировать символ амперсанда (`&`). 
...
Настоятельно рекомендуется использовать параметр `countrySet`, чтобы указать страны, в которых приложение нуждается в покрытии. Поведение по умолчанию — поиск по всему миру. Этот широкий поиск может вернуть ненужные результаты, и поиск может занять много времени.
...
Давайте найдем адрес в Сиэтле, выполнив запрос на поиск адреса Azure Maps Служба поиска. В следующем URL-адресе запроса мы задали для параметра `countrySet` значение `US` для поиска адреса в США.
...
Параметр `Score` для каждого объекта ответа показывает, как показатель сопоставления относится к оценкам других объектов в том же ответе. Дополнительные сведения о параметрах объекта ответа см. в разделе [Получение адреса поиска](https://docs.microsoft.com/rest/api/maps/search/getsearchaddress).
...
Тип отклика *Geometry* может включать идентификатор геометрии, возвращаемый в объекте `dataSources` в разделе `geometry` и `id`. Например, [службу поиска многоугольников](https://docs.microsoft.com/rest/api/maps/search/getsearchpolygon) можно использовать для запроса геометрических данных в формате геоjson. Используя этот формат, можно получить структуру города или аэропорта для набора сущностей. Затем эти данные границ можно использовать для [настройки геозоны](https://docs.microsoft.com/azure/azure-maps/tutorial-geofence) или [поиска интереса внутри геометрии](https://docs.microsoft.com/rest/api/maps/search/postsearchinsidegeometry).


Ответы для API [адреса поиска](https://docs.microsoft.com/rest/api/maps/search/getsearchaddress) или API [нечеткого поиска](https://docs.microsoft.com/rest/api/maps/search/getsearchfuzzy) могут включать идентификатор геометрии, возвращаемый в объекте `dataSources` в разделе `geometry` и `id`:
...
* Узнайте [, как создавать запросы Azure Maps служба поиска](https://docs.microsoft.com/azure/azure-maps/how-to-search-for-address).
* Изучите [документацию по API Azure Maps служба поиска](https://docs.microsoft.com/rest/api/maps/search). 
