---
title: Просмотр метрик использования API API Azure Maps (ru) Карты Microsoft Azure
description: В этой статье вы узнаете, как просматривать метрики для aPI-извне Microsoft Azure Maps на портале Azure.
author: philmea
ms.author: philmea
ms.date: 08/06/2018
ms.topic: conceptual
...
В этой статье показано, как просматривать метрики использования API для учетной записи Azure Maps на [портале Azure.](https://portal.azure.com) Метрики отображаются в удобном графическом формате вдоль шкалы настраиваемой продолжительности времени.
...
![Обзор метрик использования Azure Maps](media/how-to-view-api-usage/portal-overview.png)
...
4. На панели **метрик** выберите один из следующих вариантов:
...
5. Затем можно выбрать *диапазон времени*, щелкнув **За последние 24 часа (Автоматически)**. По умолчанию диапазон времени установлен в 24 часа. После нажатия вы увидите все выбранные временные диапазоны. Вы можете выбрать *степень детализации времени*, а также отображение времени в *местном часовом поясе* или в формате *GMT* в том же раскрывающемся списке. Щелкните **Применить**.

    ![Диапазон времени метрик Azure Maps](media/how-to-view-api-usage/time-range.png)

6. Как только вы добавите метрику, вы можете **добавить фильтр** из свойств, относящихся к этой метрике. Затем выберите значение свойства, которое вы хотите увидеть, отраженное на графике.

    ![Фильтр метрик использования Azure Maps](media/how-to-view-api-usage/filter.png)

7. Вы также можете **применить разделение** для своей метрики на основе выбранного свойства метрики. Это позволяет разделить график на несколько графиков для каждого значения этого свойства. На следующем рисунке цвет каждого графика соответствует значению свойства, показанному в нижней части графика.

    ![Расщепление метрик использования Azure Maps](media/how-to-view-api-usage/splitting.png)
...
> [Лазурные карты веб SDK Как-к](how-to-use-map-control.md)

> [!div class="nextstepaction"] 
> [Лазурный Карты Android SDK Как-к](how-to-use-android-map-control-library.md)
