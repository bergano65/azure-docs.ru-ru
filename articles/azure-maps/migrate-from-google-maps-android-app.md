...
ms.locfileid: "75909189"
---
# <a name="migrate-an-android-app-from-google-maps"></a>Перенос приложения Android из Google Карт

Пакет SDK для Android в Azure Maps имеет интерфейс API, который очень похож на веб-пакет SDK. При разработке с одним из этих пакетов SDK применяются многие те же самые основные понятия, рекомендации и архитектуры, так что вы сможете с легкостью перенести навыки работы с одного на другой.

Пакет SDK для Android в Azure Maps поддерживает минимальную версию API 21 для Android: Android 5.0.0 (Lollipop).

Все примеры предоставлены на языке Java, но в пакете SDK для Android в Azure Maps можно также использовать Kotlin.

Дополнительные сведения о разработке с помощью этого пакета SDK см. в [руководствах по пакетам SDK для Android в Azure Maps](how-to-use-android-map-control-library.md).

## <a name="load-a-map"></a>Загрузка карты

Загрузка карты в приложении Android с помощью Google Карт или Azure Maps состоит из многих одних и тех же этапов. При использовании любого из пакетов SDK необходимо:
...
- Переслать все методы жизненного цикла из действия, содержащего представление карты, в соответствующие объекты в классе map. В частности, необходимо переслать следующие методы:
    - `onCreate(Bundle)`
    - `onStart()`
    - `onResume()`
    - `onPause()`
...
1.  В файле **MainActivity.java** необходимо добавить операции импорта для пакета SDK для Google Карт. Перешлите все методы жизненного цикла из действия, содержащего представление карты, в соответствующие объекты в классе map. Экземпляр `MapView` можно извлечь из фрагмента карты с помощью метода `getMapAsync(OnMapReadyCallback)`. `MapView` автоматически инициализирует систему карт и представление. Измените файл **MainActivity.java** следующим образом:
...
2. Обновите файл **app/build.gradle** и добавьте в него следующий код.
    ...
    3. Обновите блок зависимостей и добавьте новую строку зависимости реализации для последнего пакета SDK для Android в Azure Maps:
        ...
        > [!Note]
        > Пакет SDK для Android в Azure Maps регулярно обновляется и улучшается. Последний номер версии Azure Maps можно просмотреть в документации по [началу работы с элементом управления картой в Android](how-to-use-android-map-control-library.md). Кроме того, можно задать для номера версии значение с "0.2" до "0+", чтобы он всегда указывал на последнюю версию.
...
4. В файле **MainActivity.java** потребуется:
    ...
    * получить экземпляр элемента управления картой в методе **onCreate**.
    ...
    Элемент управления картой содержит собственные методы жизненного цикла для управления жизненным циклом OpenGL Android, которые должны вызываться непосредственно из содержащегося действия. Чтобы приложение правильно вызывало методы жизненного цикла элемента управления картой, переопределите следующие методы жизненного цикла в действии, содержащем элемент управления картой, и вызовите соответствующий метод управления картой. 
...
Язык карты можно задать в методе `onCreate` основного действия, добавив следующий код перед установкой представления контекста карты. Ниже показано, как ограничить язык до французского с использованием кода fr.
...
Камеру элемента управления картой в Google Картах можно переместить программным способом с помощью метода `moveCamera`, который позволяет указать центр карты и уровень масштабирования. Для изменения типа отображаемой карты можно использовать метод `setMapType`.
...
В обоих примерах приведенное выше изображение добавляется в папку с рисунками ресурсов приложений.

**До: Google Карты**

В Google Картах пользовательские изображения можно использовать для меток, загружая их с помощью параметра `icon` метки. Сопоставить точку изображения с координатами можно с помощью параметра `anchor`. Привязка задается относительно измерений изображения. В данном случае 0,2 единицы в ширину и 1 единица в высоту.
...
В Azure Maps ломаные линии называются объектами LineString или MultiLineString. Эти объекты можно добавлять в источник данных и отображать с помощью слоя линий. Обратите внимание, что единицы "пикселей" толщины и массива штриха приводятся в соответствие с веб-пакетом SDK для Azure Maps с использованием одних и тех же значений в обоих пакетах SDK, что дает одинаковые результаты.
...
В Azure Maps объекты Polygon и MultiPolygon можно добавить в источник данных и отобразить на карте с помощью слоев. Площадь многоугольника можно отобразить на слое многоугольников. Контур многоугольника можно визуализировать с помощью слоя линий. Обратите внимание, что единицы "пикселей" толщины и массива штриха приводятся в соответствие с веб-пакетом SDK для Azure Maps с использованием одних и тех же значений в обоих пакетах SDK, что дает одинаковые результаты.
...
Слои фрагментов, также известные как наложение изображений в Google Картах, позволяют накладывать изображения слоев, разбитые на небольшие фрагменты, которые соответствуют системе разбиения карты. Это распространенный способ наложения изображений слоев или очень больших наборов данных.
...
В Azure Maps слой фрагментов можно добавить на карту во многом так же, как и любой другой слой. Форматированный URL-адрес с заполнителями значений x, y и масштаба (`{x}`, `{y}`, `{z}` соответственно) используется для указания уровню места доступа к фрагментам. Слои фрагментов в Azure Maps также поддерживают заполнители `{quadkey}`, `{bbox-epsg-3857}` и `{subdomain}`. Чтобы слой фрагментов был частично прозрачным, используется значение непрозрачности 0,8. Обратите внимание, что хоть непрозрачность и прозрачность аналогичны, для них используются инвертированные значения. Чтобы преобразовать их одно в другое, просто вычтите их значение из единицы.
