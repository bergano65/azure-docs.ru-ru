...
Если вы внимательно следите за структурой ответа, вы увидите, что она содержит параметры для каждого транзитного объекта. Каждый транзитный объект имеет такие параметры `id`, `type`как `stopName`, `mainTransitType`, `mainAgencyName`,, и положения объекта в координатах.

В целях обучения мы будем использовать `id` точку останова в качестве источника для нашего маршрута в следующем разделе.  
...
2. На вкладке Построитель выберите метод **Get** HTTP, введите следующий URL-адрес запроса для КОНЕЧНОЙ точки API и нажмите кнопку **Отправить**.
    ...
    Мы зазапрашиваем открытые транзитные маршруты для шины, указав параметры `modeType` и `transitType` . URL-адрес запроса содержит расположения, полученные в предыдущих разделах. `originType`Для у нас теперь есть **стопид**. И для `destionationType`у нас есть **Расположение**.
