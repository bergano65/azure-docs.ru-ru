...
Чтобы запросить транзитную информацию для определенной области города, вам потребуется `metroId` этой области. [Интерфейс API "получить область метро](https://aka.ms/AzureMapsMobilityMetro) " позволяет запрашивать области метро, в которых доступна Azure Mapsная служба Mobility Service. Ответ содержит такие сведения, как `metroId`, `metroName`и представление геометрии области Metro в формате геоjson.
...
Если вы внимательно следите за структурой ответа, вы увидите, что она содержит параметры для каждого транзитного объекта. Каждый транзитный объект имеет такие параметры, как `id`, `type`, `stopName`, `mainTransitType`, `mainAgencyName`и положения в координатах объекта.

В целях обучения мы будем использовать `id` шины в качестве источника, для нашего маршрута в следующем разделе.  
...
3. Если взглянуть на ответ внимательно, он содержит несколько расположений в результатах поиска в виде стрелки. Каждый результат содержит координаты расположения под **позицией**. Скопируйте `lat` и `lon` под **положением** первого результата.
...
2. На вкладке Построитель выберите метод **Get** HTTP, введите следующий URL-адрес запроса для КОНЕЧНОЙ точки API и нажмите кнопку **Отправить**.
    ...
    Мы зазапрашиваем открытые транзитные маршруты для шины, указав параметры `modeType` и `transitType`. URL-адрес запроса содержит расположения, полученные в предыдущих разделах. Для `originType`теперь у нас есть **стопид**. А для `destionationType`у нас есть **Расположение**.
...
2. На вкладке Построитель выберите метод **Get** HTTP. Введите следующий URL-адрес запроса для конечной точки API и нажмите кнопку **Отправить**.
    ...
    Мы установим для параметра `detailType` значение **Geometry** , чтобы ответ содержал сведения об ошибке для общедоступного транзита и последующего включения навигации для прохода и нахождения велосипеда маршрута.
