...
Далее будет показано, как создать две конечные точки [приложения логики](../event-grid/handler-webhooks.md#logic-apps), которые будут отправлять уведомления по электронной почте. Вот как создать первую из них:
