...
Для выполнения процедур, описанных в этой статье, необходимо сначала [создать учетную запись Azure Maps](https://docs.microsoft.com/azure/azure-maps/how-to-manage-account-keys#create-a-new-account) в ценовой категории S1.
...
4. В файле **MainActivity. Java** потребуется следующее:
    ...
    * получение экземпляра элемента управления Map в методе **OnCreate**
    ...
    Установка сведений о проверке подлинности в `AzureMaps` классе глобально `setAadProperties` `setSubscriptionKey` с помощью методов или делает это, поэтому вам не придется добавлять данные проверки подлинности в каждое представление. 
