...
После создания учетной записи Azure Maps создаются первичный и вторичный ключи. Используйте первичный ключ в качестве ключа подписки, иногда эти имена взаимозаменяемы. Вторичный ключ можно использовать в таких сценариях, как изменение ключевых изменений. В любом случае для вызова Azure Maps требуется ключ. Этот процесс называется [проверкой подлинности с помощью общего ключа](https://docs.microsoft.com/azure/azure-maps/azure-maps-authentication#shared-key-authentication). Дополнительные сведения об общем ключе и аутентификации Azure AD см. в статье [Проверка подлинности с помощью Azure Maps](https://aka.ms/amauth) .
...
1. Выберите **Azure Active Directory** в меню портала. Введите имя для регистрации. Щелкните **Регистрация приложений** нажмите кнопку **создать регистрацию**. В поле **URI перенаправления** укажите домашнюю страницу веб-приложения. Например, https://localhost/. Если у вас уже есть зарегистрированное приложение, перейдите к шагу 2.
...
2. Чтобы назначить разрешения делегированного API для Azure Maps, перейдите в приложение в разделе **Регистрация приложений**, а затем выберите **разрешения API**. Выберите **Добавить разрешение**. Найдите и выберите **Azure Maps** в разделе **Выбор API**.

    ![Разрешения API приложений](./media/how-to-manage-authentication/app-permissions.png)

3. В разделе **Выбор разрешений**установите флажок для **олицетворения пользователя**, а затем нажмите кнопку **выбрать** внизу.
...
4. Выполните шаг a или b в зависимости от метода проверки подлинности.
    ...
    1. Если приложение использует проверку подлинности на основе маркеров пользователя с помощью веб-пакета SDK Azure Maps, включите `oauth2AllowImplicitFlow`, задав для него значение true в разделе Манифест регистрации приложения.
    
       ![Манифест приложения](./media/how-to-manage-authentication/app-manifest.png)

    2. Если приложение использует проверку подлинности сервера или приложения, перейдите в колонку **сертификаты & секреты** в разделе Регистрация приложения и либо создайте пароль, либо отправьте сертификат открытого ключа в регистрацию приложения. При создании пароля его можно безопасно сохранить для последующего использования. Этот пароль будет использоваться для получения маркеров из Azure AD.
