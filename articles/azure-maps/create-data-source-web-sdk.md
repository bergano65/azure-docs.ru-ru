...
Источник данных на основе геообъектного формата JSON может загружать и хранить данные локально с помощью класса `DataSource`. Данные геоjson можно создавать или создавать вручную с помощью вспомогательных классов в пространстве имен [Atlas. Data](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.data) . Класс `DataSource` предоставляет функции для импорта локальных и удаленных геоjson файлов. Удаленные геоjson-файлы должны размещаться на конечной точке с поддержкой CORs. Класс `DataSource` предоставляет функциональные возможности для данных точки кластеризации. Данные можно легко добавлять, удалять и обновлять с помощью класса `DataSource`.


> [!TIP]
> Если вы хотите перезаписать все данные в `DataSource`, то при вызове `clear` затем `add` функции, на карте будет предпринята попытка повторного отображения в два раза, что может привести к некоторой задержке. Вместо этого используйте функцию `setShapes`, которая будет удалять и заменять все данные в источнике данных и запускать только одну повторную визуализацию схемы.
...
Все слои, использующие Векторный источник, должны указывать значение `sourceLayer`. 

После создания источники данных можно добавить на карту с помощью свойства `map.sources`, которое является [саурцеманажер](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.sourcemanager). В следующем коде показано, как создать `DataSource` и добавить его в карту.
