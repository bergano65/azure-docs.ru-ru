...
Источник данных на основе геообъектного формата JSON загружает и сохраняет данные локально с помощью класса `DataSource`. Данные геоjson можно создавать или создавать вручную с помощью вспомогательных классов в пространстве имен [Atlas. Data](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.data) . Класс `DataSource` предоставляет функции для импорта локальных и удаленных геоjson файлов. Удаленные геоjson-файлы должны размещаться на конечной точке с поддержкой CORs. Класс `DataSource` предоставляет функциональные возможности для данных точки кластеризации. И можно легко добавлять, удалять и обновлять данные с помощью класса `DataSource`.


> [!TIP]
> Допустим, вы хотите перезаписать все данные в `DataSource`. Если вы вызываете `clear` а затем выполняете функции `add`, Map может повторно отрисовываться дважды, что может привести к некоторой задержке. Вместо этого используйте функцию `setShapes`, которая удалит и заменит все данные в источнике данных и запустит только одну повторную визуализацию схемы.
