...
|---------------------------------------|-----------------------------|
| Автозаполнение                           | [Поиск](https://docs.microsoft.com/rest/api/maps/search)     |
| Маршруты (в том числе для грузовика)          | [Направления маршрутов](https://docs.microsoft.com/rest/api/maps/route/getroutedirections)                          |
| Матрица расстояний                       | [Матрица маршрута](https://docs.microsoft.com/rest/api/maps/route/postroutematrixpreview)                          |
| Снимки — статическая карта                  | [Прорисовка](https://docs.microsoft.com/rest/api/maps/render/getmapimage)                                   |
| Вычисление изохрон                            | [Диапазон маршрута](https://docs.microsoft.com/rest/api/maps/route/getrouterange)                                    |
| Локальная аналитика                        | [Поиск](https://docs.microsoft.com/rest/api/maps/search) + [диапазон маршрута](https://docs.microsoft.com/rest/api/maps/route/getrouterange)    |
| Локальный поиск                          | [Поиск](https://docs.microsoft.com/rest/api/maps/search)     |
| Распознавание расположения (точки интереса)           | [Поиск](https://docs.microsoft.com/rest/api/maps/search)     |
| Расположения (геокодирование и обратное геокодирование) | [Поиск](https://docs.microsoft.com/rest/api/maps/search)                                               |
| Привязка к дороге                          | [Запрос направлений маршрута](https://docs.microsoft.com/rest/api/maps/route/postroutedirections)                         |
| Службы пространственных данных (SDS)           | [Поиск](https://docs.microsoft.com/rest/api/maps/search) + [маршрута](https://docs.microsoft.com/rest/api/maps/route) и другие службы Azure |
| Часовой пояс                             | [Часовой пояс](https://docs.microsoft.com/rest/api/maps/timezone)  |
| Дорожные происшествия                     | [Сведения о дорожных происшествиях](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficincidentdetail)                     |
...
-   [Azure Maps Creator](https://docs.microsoft.com/azure/azure-maps/creator-indoor-maps) — создание настраиваемого частного цифрового двойника зданий и пространств.
-   [Пространственные операции](https://docs.microsoft.com/rest/api/maps/spatial) — разгрузка сложных пространственных вычислений и операций, таких как геозонирование, в службу.
-   [Фрагменты карт](https://docs.microsoft.com/rest/api/maps/render/getmaptile) — доступ к фрагментам дорог и снимков из Azure Maps в виде растровых и векторных фрагментов.
-   [Пакетная маршрутизация](https://docs.microsoft.com/rest/api/maps/route/postroutedirectionsbatchpreview) — позволяет выполнять до 1000 запросов маршрута в одном пакете за период времени. Маршруты вычисляются параллельно на сервере для ускорения обработки.
-   [Дорожное](https://docs.microsoft.com/rest/api/maps/traffic) движение — доступ к данным дорожного движения в реальном времени в виде растровых и векторных фрагментов.
-   [API геолокации](https://docs.microsoft.com/rest/api/maps/geolocation/getiptolocationpreview) — получение расположения IP-адреса.
-   [Службы погоды](https://docs.microsoft.com/rest/api/maps/weather) — доступ к данным о текущей и прогнозируемой погоде в реальном времени.

Обязательно ознакомьтесь со следующими рекомендациями:

-   [Рекомендации по использованию службы поиска Azure Maps](https://docs.microsoft.com/azure/azure-maps/how-to-use-best-practices-for-search)
-   [Рекомендации по построению маршрутов](https://docs.microsoft.com/azure/azure-maps/how-to-use-best-practices-for-routing)
...
-   [Геокодирование адресов в свободной форме.](https://docs.microsoft.com/rest/api/maps/search/getsearchaddress) Позволяет указать отдельную строку адреса (например, `"1 Microsoft way, Redmond, WA"`) и немедленно обработать запрос. Мы рекомендуем использовать эту службу, если нужно быстро выполнить геокодирование отдельных адресов.
...
| `key`                              | `subscription-key` — ознакомьтесь также с документацией по [аутентификации в Azure Maps](https://docs.microsoft.com/azure/azure-maps/azure-maps-authentication). |
| `culture` (`c`)                  | `language` — ознакомьтесь с документацией о [поддерживаемых языках](https://docs.microsoft.com/azure/azure-maps/supported-languages). |
| `userRegion` (`ur`)              | `view` — ознакомьтесь с документацией о [поддерживаемых представлениях](https://aka.ms/AzureMapsLocalizationViews). |
...
Пример использования службы поиска описан [здесь](https://docs.microsoft.com/azure/azure-maps/how-to-search-for-address). Обязательно просмотрите [рекомендации по поиску](https://docs.microsoft.com/azure/azure-maps/how-to-use-best-practices-for-search).
...
Обязательно просмотрите [рекомендации по поиску](https://docs.microsoft.com/azure/azure-maps/how-to-use-best-practices-for-search).
...
Кроме того, обязательно ознакомьтесь с документацией по [лучшим методикам построения маршрутов](https://docs.microsoft.com/azure/azure-maps/how-to-use-best-practices-for-routing).
...
Azure Maps позволяет привязать координаты к дорогам с помощью API [направлений маршрута](https://docs.microsoft.com/rest/api/maps/route/postroutedirections). Эту службу можно использовать для воссоздания логического маршрута по набору координат. Она является аналогом API привязки к дороге в Картах Bing.
...
| `imagerySet`             | `layer` и `style`. Ознакомьтесь с документацией о [поддерживаемых стилях карт](https://docs.microsoft.com/azure/azure-maps/supported-map-styles).   |
| `mapArea` (`ma`)         | `bbox`                                         |
...
Дополнительные сведения см. в статье [Отображение пользовательских данных на растровой карте](https://docs.microsoft.com/azure/azure-maps/how-to-render-custom-data).
...
Однако для расположений путей в Azure Maps необходимо указывать координаты в формате `longitude latitude`, а в Картах Bing — в формате `latitude,longitude`. Обратите также внимание на то, что в Azure Maps значения долготы и широты **разделены запятыми, а не пробелами**. В настоящее время служба Azure Maps не поддерживает закодированные пути. Большие наборы данных можно загрузить в качестве заливки GeoJSON в API хранилища данных Azure Maps, как описано [здесь](https://docs.microsoft.com/azure/azure-maps/how-to-render-custom-data#get-data-from-azure-maps-data-storage).
...
-   Диапазон [маршрута](https://docs.microsoft.com/rest/api/maps/route/getrouterange)**: можно вычислить многоугольник, охватывающий область, которую можно проехать (пройти) в любом направлении от точки отсчета с учетом заданного времени, расстояния, имеющегося топлива или допустимых транспортных расходов.
...
-   **Местный поиск.** Поиск ближайших точек интереса (радиальный поиск) по имени или типу сущности (категории). Интерфейсы API [поиска точек интереса](https://docs.microsoft.com/rest/api/maps/search/getsearchpoi) и [поиска точек интереса по категориям](https://docs.microsoft.com/rest/api/maps/search/getsearchpoicategory) в Azure Maps очень похожи на этот API.
-   **Распознавание расположения.** Позволяет выполнить поиск точек интереса, которые находятся на определенном расстоянии от расположения. Больше всего на этот API похож API [поиска поблизости](https://docs.microsoft.com/rest/api/maps/search/getsearchnearby) в Azure Maps.
-   **Местная аналитика.** Поиск точек интереса, которые находятся в пределах заданного максимального времени поездки или расстояния от определенных координат. Эту функцию можно реализовать в Azure Maps, сначала выполнив вычисление изохроны, а затем передав ее в API [поиска по геометрическим данным](https://docs.microsoft.com/rest/api/maps/search/postsearchinsidegeometry).
...
-   [Сегменты дорожного движения.](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficflowsegment) Предоставляет сведения о скорости и времени поездки для фрагмента дороги, ближайшего к заданным координатам.
-   [Фрагменты дорожного движения.](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficflowtile) Предоставляет растровые и векторные фрагменты, содержащие данные о дорожном движении. Их можно использовать с элементами управления Azure Maps или сторонними элементами управления картой, такими как Leaflet. Векторные фрагменты можно также использовать для расширенного анализа данных.
-   [Сведения о дорожных происшествиях.](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficincidentdetail) Предоставляет сведения о дорожных происшествиях в пределах ограничивающего прямоугольника с учетом масштаба и модели дорожного движения.
-   [Фрагменты с дорожными происшествиями.](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficincidenttile) Предоставляет растровые и векторные фрагменты, содержащие данные о дорожных происшествиях.
-   [Окно просмотра дорожных происшествий.](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficincidentviewport) Извлекает в окно просмотра юридические и технические сведения, описанные в запросе, например идентификатор модели дорожного движения.
...
1.  Передайте запрос для границы, которую требуется получить, в один из следующих API поиска:
    -   [Геокодирование адресов в свободной форме.](https://docs.microsoft.com/rest/api/maps/search/getsearchaddress)
    -   [Геокодирование структурированных адресов.](https://docs.microsoft.com/rest/api/maps/search/getsearchaddressstructured)
    -   [Пакетное геокодирование адресов.](https://docs.microsoft.com/rest/api/maps/search/postsearchaddressbatchpreview)
    -   [Поиск нечетких соответствий](https://docs.microsoft.com/rest/api/maps/search/getsearchfuzzy)
    -   [Пакетный поиск нечетких соответствий.](https://docs.microsoft.com/rest/api/maps/search/postsearchfuzzybatchpreview)
2.  Если у требуемых результатов есть идентификаторы геометрических объектов, передайте их в [API поиска многоугольников](https://docs.microsoft.com/rest/api/maps/search/getsearchpolygon).
...
-   [Общие сведения о типах пространственных данных SQL Azure](https://docs.microsoft.com/sql/relational-databases/spatial/spatial-data-types-overview)
-   [Пространственные SQL Azure — запрос ближайшего соседа](https://docs.microsoft.com/sql/relational-databases/spatial/query-spatial-data-for-nearest-neighbor)
-   [Общие сведения о геопространственных возможностях Azure Cosmos DB](https://docs.microsoft.com/azure/cosmos-db/geospatial)

## <a name="client-libraries"></a>Клиентские библиотеки

Azure Maps предоставляет клиентские библиотеки для следующих языков:

-   JavaScript, TypeScript, Node.js — [документация](https://docs.microsoft.com/azure/azure-maps/how-to-use-services-module) \| [пакет NPM](https://www.npmjs.com/package/azure-maps-rest).
