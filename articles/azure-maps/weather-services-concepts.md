---
title: Основные понятия служб погоды (Предварительная версия) в Microsoft Azure Maps
description: Сведения о концепциях, которые относятся к Microsoft Azure Maps (Предварительная версия).
author: anastasia-ms
ms.author: v-stharr
ms.date: 09/10/2020
ms.topic: conceptual
...
ms.locfileid: "96904743"
---
# <a name="weather-services-preview-in-azure-maps"></a>Службы погоды (Предварительная версия) в Azure Maps
...
В этой статье рассматриваются основные понятия, относящиеся к Azure Maps [служб погоды](/rest/api/maps/weather). Мы рекомендуем ознакомиться с этой статьей перед началом работы с API-интерфейсами погоды.
...
Некоторые API-интерфейсы службы погоды (Предварительная версия) возвращают `iconCode` в ответ. `iconCode`— Числовое значение, используемое для определения значка. Не следует напрямую связываться с этими изображениями из приложений, URL-адреса могут и изменяться.
...
| 33           | :::image type="icon" source="./media/weather-services-concepts/clear-night.png"::: | нет  |  Да   | Clear|
...
С помощью [Get-API плитки карты v2](/rest/api/maps/renderv2/getmaptilepreview) пользователи могут запрашивать последние лепестковые и инфракрасные спутниковые изображения. Ниже приведены сведения о том, как интерпретировать цвета, используемые для лепестковых и вспомогательных плиток.
...
[API получения ежедневных индексов](/rest/api/maps/weather) позволяет пользователям ограничить возвращаемые результаты определенными типами индекса или группами индексов.
