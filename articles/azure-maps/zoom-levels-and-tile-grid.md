...
В службе "Карты Azure" используется сферическая система координат проекции Меркатора (EPSG: 3857). Проекция — это математическая модель, используемая для преобразования сферического земного шара в плоскую карту. Сферический проекция Меркатора растягивает карту на полюсам, чтобы создать квадратную карту. Это значительно искажает масштаб и область схемы, но имеет два важных свойства, которые перевешивают это искажение:

- Это согласованная проекция, которая означает, что она сохраняет форму относительно мелких объектов. Это особенно важно при отображении аэрофотосъемки изображений, так как мы хотим избежать искажения формы зданий. Квадратные здания должны выглядеть как квадратные, а не прямоугольные.
- Это цилиндрическая проекция, которая означает, что Север и Юг всегда расположены вертикально, а Запад и Восток всегда находятся слева и справа. 

Чтобы оптимизировать производительность извлечения и отображения карт, эта схема делится на квадратные плитки. В Azure Maps SDK используются плитки, размер которых составляет 512 x 512 пикселей для карт дороги и меньше 256 x 256 пикселей для вспомогательных изображений. Карты Azure предоставляют растровые и векторные плитки для 23 уровней масштабирования с номерами от 0 до 22. На нулевом уровне масштаба вся карта мира помещается на одной плитке:
...
![макет плитки карты 2x2](media/zoom-levels-and-tile-grid/map-2x2-tile-layout.png)</center>
...
Azure Maps Интерактивные элементы управления картой для веб-и Android поддерживают уровни масштабирования 25, с номерами от 0 до 24. Хотя дорожные данные будут доступны только на уровнях масштабирования в, если плитки доступны.

В следующей таблице приведен полный список значений для уровней масштаба, где размер плитки составляет 512 пикселей в квадрате:
...
После выбора проекции и масштабирования для каждого уровня масштаба можно преобразовать географические координаты в координаты пикселей. Полная ширина и высота изображения схемы мира для определенного уровня масштаба можно вычислить следующим образом:
...
![карту, отображающую размеры в пикселах](media/zoom-levels-and-tile-grid/map-width-height.png)</center>

Учитывая широту и долготу в градусах, а также уровень детализации, координаты пиксельной оси можно вычислить следующим образом:
...
Значения широты и долготы считаются на WGS 84. Несмотря на то, что Azure Maps использует сферическую проекцию, очень важно преобразовать все географические координаты в общую, а WGS 84 был выбран в качестве этой. Значение долготы принимается в диапазоне от-180 до + 180 градусов, а значение широты должно быть обрезано до диапазона от-85,05112878 до 85,05112878. Это позволяет избежать несовпадения в полюсам и приводит к тому, что проецируемая проекция будет квадратной.

## <a name="tile-coordinates"></a>Координаты плитки

Чтобы оптимизировать производительность при извлечении и отображении карт, визуализированная схема обрезана до плиток. По мере того, как количество пикселов отличается на каждом уровне масштаба, это число плиток:
...
Каждой плитке присваиваются координаты координат от (0,0) в верхнем левом углу до *(2<sup>масштаба</sup>– 1, 2<sup>масштаба</sup>– 1)* в правом нижнем углу. Например, на уровне масштабирования 2 координаты плитки находятся в диапазоне от (0, 0) до (7, 7) следующим образом:

<center>

![ная схема координат плитки](media/zoom-levels-and-tile-grid/map-tiles-x-y-coordinates-7x7.png)</center>
...
![сетку масштабирования для](media/zoom-levels-and-tile-grid/api_x_y.png)</center> уровня масштаба 1

## <a name="quadkey-indices"></a>Индексы куадкэй

Некоторые платформы сопоставления используют `quadkey`ное соглашение об именовании индексирования, объединяющее координаты зи в строку одного измерения, именуемую `quadtree` ключами или `quadkeys` короткими. Каждый `quadkey` однозначно определяет одну плитку на определенном уровне детализации, и ее можно использовать в качестве ключа в общих индексах сбалансированного дерева базы данных. Azure Maps пакеты SDK поддерживают наложение уровней мозаики, которые используют `quadkey` соглашения об именовании в дополнение к соглашениям об именовании, как описано в документе [Добавление мозаичного слоя](map-add-tile-layer.md) .

> [!NOTE]
> Соглашение об именовании `quadkeys` работает только для одного или более уровней масштаба. Уровень масштабирования для поддержки Azure Maps SDK 0, который является одной плиткой карты для всего мира. 

Чтобы преобразовать координаты плитки в `quadkey`, биты координат Y и X чередуются, а результат интерпретируется как число с основанием 4 (с ведущими нулями) и преобразуется в строку. Например, заданные координаты координат мозаики (3, 5) на уровне 3, `quadkey` определяется следующим образом:
...
`Qquadkeys` иметь несколько интересных свойств. Во-первых, длина `quadkey` (количество цифр) равна уровню масштаба соответствующей плитки. Во-вторых, `quadkey` любой плитки начинается с `quadkey` его родительской плитки (содержащей плитку на предыдущем уровне). Как показано в примере ниже, плитка 2 является родителем плиток от 20 до 23:

<center>

![ная пирамидальная](media/zoom-levels-and-tile-grid/quadkey-tile-pyramid.png)</center> плитки Куадкэй

Наконец, `quadkeys` предоставить ключ одномерного индекса, который обычно сохраняет расстояние между плитками в пространстве координат. Другими словами, две плитки с координатами, расположенными поблизости, обычно имеют `quadkeys`, которые относительно близко близки друг к другу. Это важно для оптимизации производительности базы данных, поскольку соседние плитки часто запрашиваются в группах, и желательно удерживать эти плитки на одних и тех же дисковых блоках, чтобы свести к минимальному числу операций чтения с диска.
...
> [Получение плиток карт](https://docs.microsoft.com/rest/api/maps/render/getmaptile)

> [!div class="nextstepaction"]
> [Получение плиток потока трафика](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficflowtile)

> [!div class="nextstepaction"]
> [Получение плиток "инцидент трафика"](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficincidenttile)
