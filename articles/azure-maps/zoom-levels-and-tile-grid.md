...
ms.locfileid: "68976061"
---
# <a name="zoom-levels-and-tile-grid"></a>Уровни увеличения и сетка фрагментов

Azure Maps использовать систему координат "сферическая Меркатора проекция" (EPSG: 3857). Проекция — это математическая модель, используемая для преобразования сферического земного шара в плоскую карту. Сферический проекция Меркатора растягивает карту на полюсам, чтобы создать квадратную карту. Это значительно искажает масштаб и область схемы, но имеет два важных свойства, которые перевешивают это искажение:
...
Каждый дополнительный уровень масштаба четыре делит плитки предыдущего, создавая сетку с<sup>масштабом</sup>2 x 2.<sup></sup> 22-й уровень масштаба — это сетка 2<sup>22</sup> x 2<sup>22</sup>, или 4 194 304 x 4 194 304 фрагмента (всего 17 592 186 044 416 фрагментов).
...
![Схема, показывающая размеры пикселей](media/zoom-levels-and-tile-grid/map-width-height.png)</center>
...
![Сетка "Масштаб" для уровня масштаба 1](media/zoom-levels-and-tile-grid/api_x_y.png)</center>

## <a name="quadkey-indices"></a>Индексы куадкэй

Некоторые платформы сопоставления используют соглашение об именовании куадкэй индексирования, объединяющее координаты Зи мозаики в строку одного измерения с именем QuadTree Keys или «куадкэйс». Каждый куадкэй однозначно определяет одну плитку на определенном уровне детализации, и ее можно использовать в качестве ключа в общих индексах сбалансированного дерева базы данных. Azure Maps пакеты SDK поддерживают наложение уровней мозаики, которые используют соглашение об именовании куадкэй в дополнение к другим соглашениям об именовании, как описано в документе [Добавление мозаичного слоя](map-add-tile-layer.md) .

> [!NOTE]
> Соглашение об именовании куадкэйс работает только для одного или более уровней масштаба. Уровень масштабирования для поддержки Azure Maps SDK 0, который является одной плиткой карты для всего мира. 

Чтобы преобразовать координаты плитки в куадкэй, биты координат Y и X чередуются, а результат интерпретируется как число с основанием 4 (с ведущими нулями) и преобразуется в строку. Например, заданные координаты координат мозаики (3, 5) на уровне 3, куадкэй определяется следующим образом:
...
Куадкэйс имеет несколько интересных свойств. Во-первых, длина куадкэй (количество цифр) равна уровню масштаба соответствующей плитки. Во-вторых, куадкэй любой плитки начинается с куадкэй его родительской плитки (содержащей плитку на предыдущем уровне). Как показано в примере ниже, плитка 2 является родителем плиток от 20 до 23:

<center>

![Пирамидальная с плиткой куадкэй](media/zoom-levels-and-tile-grid/quadkey-tile-pyramid.png)</center>

Наконец, куадкэйс предоставляет ключ одномерного индекса, который обычно сохраняет сходство плиток в пространстве координат. Другими словами, две плитки с координатами, расположенными поблизости, обычно имеют куадкэйс, которые относительно близко друг к другу. Это важно для оптимизации производительности базы данных, поскольку соседние плитки часто запрашиваются в группах, и желательно удерживать эти плитки на одних и тех же дисковых блоках, чтобы свести к минимальному числу операций чтения с диска.
...
> - [Веб-пакет SDK: Отображение вычислений пикселей и позиций](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map#pixelstopositions-pixel---)
