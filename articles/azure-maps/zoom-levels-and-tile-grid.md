---
title: Увеличить уровни и плитки сетки Карты Microsoft Azure
description: В этой статье вы узнаете о уровнях масштабирования и сетке плитки в Microsoft Azure Maps.
...
ms.locfileid: "79530297"
---
# <a name="zoom-levels-and-tile-grid"></a>Уровни увеличения и сетка фрагментов

В службе "Карты Azure" используется сферическая система координат проекции Меркатора (EPSG: 3857). Проекция — это математическая модель, используемая для преобразования сферического глобуса в плоскую карту. Проекция Сферического Меркатора растягивает карту на полюсах, чтобы создать квадратную карту. Эта проекция значительно искажает масштаб и область карты, но имеет два важных свойства, которые перевешивают это искажение:

- Это конформная проекция, которая означает, что она сохраняет форму относительно небольших объектов. Сохранение формы мелких объектов особенно важно при показе аэрофотоснимков. Например, мы хотим избежать искажения формы зданий. Квадратные здания должны казаться квадратными, а не прямоугольными.
- Это цилиндрическая проекция. Север и юг всегда вверх и вниз, а запад и восток всегда влево и вправо. 

Чтобы оптимизировать производительность поиска и отображения карты, карта разделена на квадратные плитки. Плитки использования Azure Maps SDK, которые имеют размер 512 x 512 пикселей для дорожных карт и меньше 256 x 256 пикселей для спутниковых снимков. Azure Maps предоставляет raster и векторные плитки для 23 уровней зума, пронумерованных от 0 до 22. На нулевом уровне масштаба вся карта мира помещается на одной плитке:
...
![2x2 макет плитки карты](media/zoom-levels-and-tile-grid/map-2x2-tile-layout.png)</center>

Каждый дополнительный уровень зума четырехстороннего делит плитки предыдущего, создавая сетку 2<sup>зум</sup> х 2<sup>зум.</sup> 22-й уровень масштаба — это сетка 2<sup>22</sup> x 2<sup>22</sup>, или 4 194 304 x 4 194 304 фрагмента (всего 17 592 186 044 416 фрагментов).

Интерактивные карты Azure Maps для веб-систем и Android поддерживают 25 уровней зума с числом от 0 до 24. Хотя дорожные данные будут доступны только на уровне масштабирования, когда плитки доступны.

В следующей таблице приведен полный список значений для уровней масштабирования, где размер плитки составляет 512 пикселей:
...
## <a name="pixel-coordinates"></a>Координаты пикселей

Выбрав проекцию и масштаб для использования на каждом уровне масштабирования, мы можем преобразовать географические координаты в пиксельные координаты. Полная ширина пикселя и высота картографического изображения мира для определенного уровня зума рассчитывается как:
...
Поскольку ширина и высота карты различны на каждом уровне масштабирования, то и координаты пикселей. Пиксель в верхнем левом углу карты всегда имеет пиксельные координаты (0, 0). Пиксель в правом нижнем углу карты имеет пиксельные координаты *(ширина-1, высота-1)* или ссылаясь на уравнения в предыдущем разделе, *(tileSize \* 2<sup>зум</sup>-1, tileSize \* 2<sup>зум</sup>-1)*. Например, при использовании 512 квадратных плиток на уровне 2 координаты пикселей варьируются от (0, 0) до (2047, 2047), как это:

<center>

![Карта, показывающая размеры пикселей](media/zoom-levels-and-tile-grid/map-width-height.png)

</center>

Учитывая широту и долготу в градусах, а также уровень детализации, координаты пикселя XY рассчитываются следующим образом:
...
Значения широты и долготы, как предполагается, будут находиться на WGS 84 datum. Несмотря на то, что Azure Maps использует сферическую проекцию, важно преобразовать все географические координаты в общий datum. WGS 84 является выбранным datum. Предполагается, что значение долготы будет варьироваться от -180 градусов до 180 градусов, а значение широты должно быть обрезано в диапазоне от -85.05112878 до 85.05112878. Придерживаясь этих значений, это позволяет избежать сингулярности на полюсах, и гарантирует, что проецируемая карта представляет собой квадратную форму.

## <a name="tile-coordinates"></a>Координаты плитки

Для оптимизации производительности поиска и отображения карты отображается на плитки. Количество пикселей и количество плиток различаются на каждом уровне зума:
...
Каждой плитке даются координаты XY, начиная от (0, 0) в левом верхнем углу до *(2<sup>зума</sup>-1, 2<sup>зума</sup>-1)* в правом нижнем углу. Например, на уровне увеличения 2 координаты плитки варьируются от (0, 0) до (7, 7) следующим образом:

<center>

![Карта координат плитки](media/zoom-levels-and-tile-grid/map-tiles-x-y-coordinates-7x7.png)</center>

Учитывая пару пиксельных координат XY, вы можете легко определить координаты плитки XY плитки, содержащей этот пиксель:
...
Плитки называются уровнем зума. Координаты x и y соответствуют положению плитки на сетке для этого уровня масштабирования.

При определении уровня масштабирования помните, что каждое местоположение находится в фиксированном положении на плитке. В результате количество плиток, необходимых для отображения данного простора территории, зависит от конкретного размещения зум-сетки на карте мира. Например, есть две точки, расстояние между которыми составляет 900 метров. Для отображения этого маршрута *может* понадобиться только три плитки на 17-ом уровне масштабирования. Но если западная точка находится а правой части своего фрагмента, а восточная — в левой части своего, может понадобиться четыре фрагмента:

<center>

![Демонстрация увеличения масштаба](media/zoom-levels-and-tile-grid/zoomdemo_scaled.png)</center>

После выбора уровня масштаба вычисляются значения x и y. Верхняя левая плитка в каждой сетке зума составляет х 0, y'0; в правом нижнем углу плитка находится на х 2<sup>зум-1</sup>, y'2<sup>зум-1</sup>.

Вот сетка масштабирования для первого уровня масштаба:

<center>

![Сетка масштаба для первого уровня масштаба](media/zoom-levels-and-tile-grid/api_x_y.png)</center>

## <a name="quadkey-indices"></a>Индексы квадки

Некоторые платформы отображения используют конвенцию `quadkey` именования индексирования, которая `quadtree` объединяет `quadkeys` координаты плитки y в строку с одним измерением, называемую ключами или для краткости. Каждый `quadkey` однозначно идентифицирует одну плитку на определенном уровне детализации, и ее можно использовать в качестве ключа в общих индексах B-дерева базы данных. SDK Azure Maps поддерживают наложение слоев `quadkey` плитки, которые используют конвенцию именования в дополнение к другим соглашениям именования, как это описано в документе [слоя плитки.](map-add-tile-layer.md)

> [!NOTE]
> Конвенция `quadkeys` именования работает только для увеличения уровней одного или более. Уровень увеличения поддержки Azure Maps SDK 0, который представляет собой единую карту плитки для всего мира. 

Для преобразования координат плитки `quadkey`в, биты координат Y и X переплетаются, и результат интерпретируется как номер базы-4 (с ведущими нулями поддерживается) и преобразуется в строку. Например, данная плитка XY координаты (3, 5) на уровне 3, `quadkey` определяется следующим образом:
...
`Qquadkeys`есть несколько интересных свойств. Во-первых, длина `quadkey` (число цифр) равна уровню масштабирования соответствующей плитки. Во-вторых, `quadkey` любая `quadkey` плитка начинается с родительской плитки (содержащей плитку на предыдущем уровне). Как показано в приведенном ниже примере, плитка 2 является родителем плиток от 20 до 23:

<center>

![Пирамида квадратной плитки](media/zoom-levels-and-tile-grid/quadkey-tile-pyramid.png)</center>

Наконец, `quadkeys` обеспечить одномерный ключ индекса, который обычно сохраняет близость плитки в пространстве XY. Другими словами, две плитки, которые имеют `quadkeys` близлежащие координаты XY, как правило, имеют относительно близко друг к другу. Это важно для оптимизации производительности базы данных, так как соседние плитки часто запрашиваются в группах, и желательно держать эти плитки на тех же дисковых блоках, чтобы свести к минимуму количество считываемых дисков.

## <a name="tile-math-source-code"></a>Источник плитки математики

Следующий пример кода иллюстрирует, как реализовать функции, описанные в этом документе. Эти функции могут быть легко переведены на другие языки программирования по мере необходимости.
...
> [!NOTE]
> Интерактивные элементы управления картами в SDK Azure Maps имеют функции помощника для преобразования между геопространственными позициями и пикселями viewport. 
> - [Веб-SDK: Расчеты пикселей и положения карты](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map#pixelstopositions-pixel---)

## <a name="next-steps"></a>Дальнейшие действия

Непосредственной доступ к картографическим плитам из служб Azure Maps REST:

> [!div class="nextstepaction"]
> [Получить плитки карты](https://docs.microsoft.com/rest/api/maps/render/getmaptile)

> [!div class="nextstepaction"]
> [Получить плитки потока трафика](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficflowtile)

> [!div class="nextstepaction"]
> [Получить плитки дорожно-транспортных происшествий](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficincidenttile)

Узнайте больше о геопространственных концепциях:
