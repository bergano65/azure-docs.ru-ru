...
Кроме заполнения многоугольника цветом, можно использовать узор изображения для заполнения многоугольника. Загрузите шаблон изображения в ресурсы с изображением Sprite, а затем сослаться на этот образ со `fillPattern` свойством слоя многоугольников.
...
Azure Maps веб-пакет SDK преобразует эти `Point` функции в `Polygon` компоненты. Затем эти функции отображаются на карте с использованием многоугольников и слоев линий, как показано в следующем примере кода.
