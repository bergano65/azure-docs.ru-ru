...
> [!Tip]
> Корпорация Майкрософт рекомендует реализовать Azure Active Directory (Azure AD) и управление доступом на основе ролей Azure (Azure RBAC) для рабочих приложений. Общие сведения об основных понятиях см. в разделе [Azure Mapsная проверка подлинности](./azure-maps-authentication.md).
...
> [!Tip]
> Если приложение размещено в среде Azure, следует реализовать управляемое удостоверение, чтобы снизить затраты и сложность управления секретом для проверки подлинности в Azure Key Vault. [Чтобы подключиться через управляемое удостоверение](../key-vault/general/tutorial-net-create-vault-azure-web-app.md), ознакомьтесь со следующим руководством Azure Key Vault.

Управляющее приложение отвечает за получение общего ключа из защищенного хранилища. Для реализации с Azure Key Vault требуется проверка подлинности в Azure AD для доступа к секрету. Вместо этого мы рекомендуем обеспечить прямую проверку подлинности Azure AD для Azure Maps в результате дополнительных требований к сложности и эксплуатации при использовании проверки подлинности с использованием общего ключа.
...
См. раздел [Обзор управляемых удостоверений](../active-directory/managed-identities-azure-resources/overview.md) , чтобы предоставить приложению доступ к управляемому удостоверению.
...
Вы предоставляете *Управление доступом на основе ролей Azure (Azure RBAC)* , назначив созданное управляемое удостоверение или субъект-службу одному или нескольким определениям ролей Azure Maps. Чтобы просмотреть определения ролей Azure, доступные для Azure Maps, перейдите в раздел **Управление доступом (IAM)** . Выберите **роли** , а затем найдите роли, которые начинаются с *Azure Maps* . Эти Azure Maps роли являются ролями, к которым можно предоставить доступ.

> [!div class="mx-imgBorder"]
> ![Просмотр доступных ролей](./media/how-to-manage-authentication/how-to-view-avail-roles.png)

1. Перейдите к **учетной записи Azure Maps** . Выберите **Управление доступом (IAM)** > **Назначения ролей** .
...
После настройки управляемого удостоверения для ресурса размещения используйте пакет Azure SDK или REST API, чтобы получить маркер для Azure Maps, см. сведения о [получении маркера доступа](../active-directory/managed-identities-azure-resources/how-to-use-vm-token.md). В соответствии с руководством, ожидание заключается в том, что будет возвращен маркер доступа, который может использоваться в запросах REST API.
...
Более подробные примеры см. в статье [сценарии проверки подлинности в Azure AD](../active-directory/develop/authentication-vs-authorization.md).
