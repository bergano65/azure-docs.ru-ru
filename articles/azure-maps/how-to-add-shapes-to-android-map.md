---
title: Добавление фигур к картам Android Карты Microsoft Azure
description: В этой статье вы узнаете, как визуализировать различные фигуры на карте с помощью Microsoft Azure Maps Android SDK.
author: philmea
ms.author: philmea
ms.date: 04/26/2019
ms.topic: conceptual
...
ms.locfileid: "80335593"
---
# <a name="add-a-shape-to-a-map-using-azure-maps-android-sdk"></a>Добавьте форму на карту с помощью Azure Maps Android SDK

В этой статье показано, как визуализировать фигуры на карте с помощью Azure Maps Android SDK.
...
## <a name="add-a-line-to-the-map"></a>Добавить строку на карту

Вы можете добавить строку на карту с помощью **слоя line,** выполните ниже шаги, чтобы добавить строку на карте.
...
2. Скопируйте следующий фрагмент кода ниже в метод `MainActivity.java` **onCreate()** вашего класса.
    ...
    Фрагмент кода выше сначала получает экземпляр управления картой Azure Maps с помощью метода обратного вызова **onReady()** обратный вызов. Затем он создает объект источника данных с помощью класса **DataSource** и добавляет его на карту. Затем он создает список объектов **точки.** **LineString** создается из списка точек и добавляется к источнику данных. **Линейный слой** отображает линейные объекты, завернутые в источник данных на карте. Затем создается линейный слой и добавляется к нему источник данных.
...
![Линия, отрисована на карте Android](./media/how-to-add-shapes-to-android-map/android-map-line.png)</center>


## <a name="add-a-polygon-to-the-map"></a>Добавление многоугольника на карту

**Слой полигона** позволяет отобразить область полигона на карту. Следуйте ниже, чтобы добавить полигон на карте.
...
2. Скопируйте следующий фрагмент кода в метод **onCreate()** вашего `MainActivity.java` класса.
    ...
    Фрагмент кода выше сначала получает экземпляр управления картой Azure Maps с помощью метода обратного вызова **onReady()** обратный вызов. Затем он создает объект источника данных с помощью класса **DataSource** и добавляет его на карту. Затем объект **Polygon** создается из списка объектов **Точек** и добавляется в источник данных. **Слой полигона** отображает данные, завернутые в источник данных на карте. Затем он создает многоугольный слой для визуализации области полигона и добавляет к нему источник данных. Линейный **слой** отображает объекты строки, завернутые в источник данных. Последняя часть фрагмента кода создает линейный слой для визуализации контура полигона и добавляет к нему источник данных.
...
Если вы запустите приложение сейчас, вы должны увидеть полигон на карте, как показано ниже:

<center>

![Полигон, отрисованный на карте Android](./media/how-to-add-shapes-to-android-map/android-map-polygon.png)</center>
