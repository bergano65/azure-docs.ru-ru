...
Мы поддерживаем сортировку по дате последнего обновления: _sort = _lastUpdated. Дополнительные сведения о других поддерживаемых параметрах поиска см. на [странице поддерживаемых возможностей](https://docs.microsoft.com/azure/healthcare-apis/fhir-features-supported#search).

### <a name="how-does-export-work"></a>Как работает $export?

$export является частью спецификации FHIR: https://hl7.org/fhir/uv/bulkdata/export/index.html . Если служба FHIR настроена с управляемым удостоверением и учетной записью хранения, и если управляемое удостоверение имеет доступ к этой учетной записи хранения, можно просто вызвать $export в API FHIR, и все ресурсы FHIR будут экспортированы в учетную запись хранения. Дополнительные сведения см. [в статье $Export](https://docs.microsoft.com/azure/healthcare-apis/export-data).
...
Мы включили ведение журнала диагностики и разрешаете Просмотр образцов запросов для этих журналов. Дополнительные сведения о включении журналов аудита и образцов запросов см. в [этом разделе](https://docs.microsoft.com/azure/healthcare-apis/enable-diagnostic-logging). Если вы хотите включить дополнительные сведения в журналы, извлеките их [с помощью настраиваемых заголовков HTTP](https://docs.microsoft.com/azure/healthcare-apis/use-custom-headers).
...
Да, API Azure для FHIR позволяет настраивать управляемые клиентом ключи, используя поддержку из Cosmos DB. Дополнительные сведения о шифровании данных с помощью личного ключа см. в [этом разделе](https://docs.microsoft.com/azure/healthcare-apis/customer-managed-key).
