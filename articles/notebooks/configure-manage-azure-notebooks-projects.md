---
title: Настройка и управление анонсом ноутбуков Azure
description: Узнайте, как управлять метаданными проекта, файлами проектов, средой проекта и этапами настройки через ui и прямой доступ к терминалам Azure Notebooks.
ms.topic: how-to
ms.date: 02/28/2020
...
ms.locfileid: "79280603"
---
# <a name="manage-and-configure-projects-in-azure-notebooks-preview"></a><a id="manage-and-configure-projects" />Управление и настройка проектов в Azure Ноутбуки Предварительный

Проект в Azure Notebooks Preview по существу представляет собой конфигурацию базовой виртуальной машины Linux, в которой работают ноутбуки Jupyter, а также папку с файлами и описательные метаданные. 
...
По умолчанию проекты работают на уровне **Free Compute,** который ограничен 4 ГБ памяти и 1 ГБ данных для предотвращения злоупотреблений. Вы можете обойти эти ограничения и увеличить вычислительную мощность с помощью другой виртуальной машины, которую вы зарезервировали в подписке Azure. Для получения дополнительной информации, [см.](use-data-science-virtual-machine.md)
...
| **Папку** | Вложенная папка | Создает редактируемое поле в списке файлов проекта, в которое следует ввести имя папки. |
...
Команда **Upload** предоставляет два варианта импорта данных из других мест: **от URL** и с **компьютера.** Дополнительные сведения см. в статье [Work with data files in Azure Notebook projects](work-with-project-data-files.md) (Работа с файлами данных в проектах службы "Записные книжки Azure").
...
Впервые Блокноты Azure создают сервер для проекта, он ищет файл в проекте под названием *aznbsetup.sh*. Если этот файл присутствует, блокноты Azure запускает его. Выходные данные скрипта сохраняются в папке проекта в виде файла *.aznbsetup.log*.
...
- **Requirements.txt**: Во втором списке выпадающих ниже, выберите файл *requirements.txt,* который уже в проекте. Затем выберите версию Python в третьем раскрывающемся списке, который появится. С помощью файла *requirements.txt* служба "Записные книжки Azure" выполняет команду `pip install -r` с файлом *requirements.txt* при запуске сервера записной книжки. Пакеты из самой записной книжки явно устанавливать не нужно.

- **Сценарий оболочки:** Во втором списке выпадающих вниз, выберите скрипт оболочки Bash в проекте (обычно файл с расширением *.sh),* который содержит любые команды, которые вы хотите запустить для инициализации среды.

- **Environment.yml**: Во втором списке отсева выберите файл *environments.yml* для проектов Python с использованием среды conda.

   > [!WARNING]
   > Поскольку это служба предварительного просмотра, в настоящее `Environment.yml` время существует известная проблема, при которой параметр не применяется к вашему проекту, как ожидалось. Проект и ноутбуки Jupyter внутри не загружают указанный файл среды в настоящее время.
