...
| Копировать ссылку | y | Копирует ссылку на файл в буфер обмена. |
...
| Изменение файла | i | Открывает файл для редактирования. |
...
| Удаление | x | Запрашивает подтверждение, а затем окончательно удаляет файл из проекта. Такое удаление невозможно отменить. |
...
- **Environment. yml**. во втором раскрывающемся списке выберите файл *среды. yml* для проектов Python с помощью среды conda.
   ...
   > [!WARNING]
   > Так как это предварительная версия службы в разработке, в настоящее время существует известная проблема, при которой параметр `Environment.yml` не применяется к проекту должным образом. Проект и записные книжки Jupyter в настоящее время не загружают указанный файл среды.
