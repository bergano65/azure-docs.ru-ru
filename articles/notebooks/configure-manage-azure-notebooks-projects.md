...
ms.locfileid: "75646268"
---
# <a name="a-idmanage-and-configure-projects--manage-and-configure-projects-in-azure-notebooks-preview"></a><a id="manage-and-configure-projects" /> управления и настройки проектов в предварительной версии Azure Notebooks
...
| Тип элемента | Description | Поведение команды |
| --- | --- | --- |
...
| Get-Help | Сочетания клавиш | Действия |
| --- | --- | --- |
| Запуск | r (или щелчок) | Запускает файл записной книжки. Другие типы файлов открываются для просмотра.  |
...
| Предварительная версия | p | Открывает предварительную HTML-версию файла. Для записных книжек предварительная версия — это отображение записной книжки только для чтения. Дополнительные сведения см. в разделе [Предварительный просмотр](#preview). |
| Edit file (Изменить файл) | i | Открывает файл для редактирования. |
| Загрузить | d | Загружает ZIP-файл, содержащий файл или содержимое папки. |
| Переименовать | a | Запрашивает новое имя для файла или папки. |
| Удалить | x | Запрашивает подтверждение, а затем окончательно удаляет файл из проекта. Такое удаление невозможно отменить. |
| Переместить | мин | Перемещает файл в другую папку в том же проекте. |
...
| Get-Help | Сочетания клавиш | Действия |
| --- | --- | --- |
| Распространение | с | Отображает всплывающее окно общего доступа, в котором можно получить ссылку, предоставить общий досуп в социальных сетях, получить HTML-код для встраивания и отправить сообщение электронной почты. |
| Клонировать | с  | Клонирует записную книжку в вашу учетную запись. |
| Запуск | r | Запускает записную книжку, если вам это разрешено. |
| Загрузить | d | Скачивает копию записной книжки. |
...
- **Требования. txt**. во втором раскрывающемся списке выберите файл *требований. txt* , который уже находится в проекте. Затем выберите версию Python в третьем раскрывающемся списке, который появится. С помощью файла *requirements.txt* служба "Записные книжки Azure" выполняет команду `pip install -r` с файлом *requirements.txt* при запуске сервера записной книжки. Пакеты из самой записной книжки явно устанавливать не нужно.
