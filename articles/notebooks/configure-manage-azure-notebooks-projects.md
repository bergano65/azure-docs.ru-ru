...
| Предварительный просмотр | p | Открывает предварительную HTML-версию файла. Для записных книжек предварительная версия — это отображение записной книжки только для чтения. Дополнительные сведения см. в разделе [Предварительный просмотр](#preview). |
...
- **Environment. yml**. во втором раскрывающемся списке выберите файл *среды. yml* для проектов Python с помощью среды conda.
   ...
   > [!WARNING]
   > Так как это предварительная версия службы в разработке, в настоящее время существует известная проблема, при которой `Environment.yml` параметр не применяется к проекту должным образом. Проект и записные книжки Jupyter в настоящее время не загружают указанный файл среды.
