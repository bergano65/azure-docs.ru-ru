...
[![An странице профиля записных книжек Azure](media/accounts/profile-page.png)](media/accounts/profile-page.png#lightbox)
...
Идентификатор пользователя должен состоять из от четырех до шестнадцати буквы, цифры и дефисы. Другие знаки не допускаются, и идентификатор пользователя не может начинаться или заканчиваться дефисом, а также содержать несколько дефисов подряд. Поскольку идентификаторы пользователей являются уникальными для всех учетных записей записных книжек Azure, может появиться сообщение, «Идентификатор пользователя уже используется.» (Сообщение также появляется при попытке использовать товарным знаком корпорации Майкрософт в качестве идентификатора пользователя) В таком случае выберите другой код.
