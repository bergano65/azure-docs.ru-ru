...
**Уровень "Стандартный"** включает такие расширенные средства обнаружения угроз, как аналитика угроз, анализ поведения, обнаружение аномалий и инцидентов безопасности, а также возможность создавать отчеты об определении угроз. Можно запустить бесплатную пробную версию уровня "стандартный". Чтобы повысить уровень, выберите [ценовую категорию](https://docs.microsoft.com/azure/security-center/security-center-pricing) в политике безопасности. Дополнительные сведения см. на [странице с ценами](https://azure.microsoft.com/pricing/details/security-center/).

### <a name="how-can-i-track-who-in-my-organization-performed-pricing-tier-changes-in-azure-security-center"></a>Как отследить выполнившего в моей организации ценовую в центре безопасности Azure
Как подписка Azure может иметь несколько администраторов с разрешениями могут менять ценовую категорию, пользователю может потребоваться узнать, кто выполнял изменение ценовой. Чтобы использовать его, можно использовать журнал действий Azure. См. Дополнительные инструкции [здесь](https://techcommunity.microsoft.com/t5/Security-Identity/Tracking-Changes-in-the-Pricing-Tier-for-Azure-Security-Center/td-p/390832)
...
Центр безопасности собирает данные из виртуальных машин (ВМ) Azure, масштабируемые наборы виртуальных машин (VMSS), контейнеры IaaS и компьютеров, отличных от Azure (включая локальную) для отслеживания угроз и уязвимостей системы безопасности. Для сбора данных используется агент Microsoft Monitoring Agent, который считывает различные конфигурации, связанные с безопасностью, и журналы событий с компьютера, а также копирует данные в рабочую область для анализа.

### <a name="am-i-billed-for-azure-monitor-logs-on-the-workspaces-created-by-security-center"></a>Оплата за журналы Azure Monitor в рабочих областях, созданных центром безопасности?
№ Рабочие области, созданные центром безопасности, настроены для журналов Azure Monitor для каждого узла выставления счетов, не взимается Azure Monitor журналы. Счета за использование центра безопасности всегда выставляются в зависимости от его политики безопасности и решений, установленных в рабочей области:
...
> [!NOTE]
> Ценовая категория рабочих областей, созданных с помощью центра безопасности log analytics не влияет на выставление счетов центра безопасности.
>
>
...
- Windows или Linux [агента виртуальной машины Azure](https://docs.microsoft.com/azure/virtual-machines/extensions/agent-windows) установлен.
...
### Что делать, если Microsoft Monitoring Agent уже установлен в качестве расширения на виртуальной Машине?<a name="mmaextensioninstalled"></a>
При установке агента мониторинга в качестве расширения конфигурации расширения позволяет создавать отчеты только к одной рабочей области. Центр безопасности не переопределяет установленные подключения к рабочим областям пользователя. Центр безопасности сохраняет данные безопасности из виртуальной Машины в рабочей области, которая уже подключен, при условии, что решение «Безопасность» или «SecurityCenterFree» был установлен в его. Центр безопасности может обновить версию расширения до последней версии в этом процессе.

Дополнительные сведения см. в разделе [автоматической подготовки в случаях установки агента в созданной ранее](security-center-enable-data-collection.md#preexisting).


### Что делать, если у меня есть Microsoft Monitoring Agent установлен непосредственно на компьютере, но не как расширение (Direct Agent)?<a name="directagentinstalled"></a>
...
Агент, установленный продолжит занесения в его уже настроенных рабочих областях, а также будут отправлять отчеты в рабочей области, настроенной в центре безопасности (Множественная адресация поддерживается на компьютерах Windows).
Если настроенной рабочей области — это рабочее пространство пользователя (не рабочую центра безопасности по умолчанию), необходимо установить «безопасности / отчетности, решение «SecurityCenterFree» на его для центра безопасности для запуска обработки событий из виртуальных машин и компьютеров Рабочая область.
...
Для существующих машин на подписки подключено к центру безопасности перед 2019-03-17, если будут обнаружены существующего агента, расширение Microsoft Monitoring Agent не будет установлен и компьютер, не будут затронуты. Для этих компьютеров см. в разделе «Разрешить проблемы мониторинга работоспособности агентов на компьютерах» рекомендации по решению проблемы установки агента на этих машинах
...
### Что произойдет, если агент System Center Operations Manager (SCOM) уже установлен на моей виртуальной Машине?<a name="scomomsinstalled"></a>
Центр безопасности установит Microsoft Monitoring Agent расширение side-by-side существующий агент System Center Operations Manager. Существующий агент SCOM продолжит обычно отчетов на сервере System Center Operations Manager. Обратите внимание на то, что агент System Center Operations Manager и Microsoft Monitoring Agent совместно использовать общие библиотеки времени выполнения, которые будут обновлены до последней версии во время этого процесса. Обратите внимание: Если отключить автоматическую подготовку на агента версии 2012, System Center Operations Manager (возможности управления могут быть потеряны, если сервер System Center Operations Manager также не версии 2012).
...
- Установка расширения Microsoft Monitoring Agent (MMA) обновляет версию агента. Это относится к прямому агенту и агенту SCOM (в последнем случае SCOM и MMA совместно используют общие библиотеки среды выполнения - которых будет обновляться в процессе). Если установлен агент SCOM версии 2012, который был обновлен, возможности управления могут быть потеряны, если сервер SCOM также версии 2012. Можно отказаться от автоматической подготовки, если установлен агент SCOM версии 2012.
...
## Существующие монитор Azure журналы клиентов<a name="existingloganalyticscust"></a>
...
Если на виртуальной машине уже установлен агент Microsoft Monitoring Agent в качестве расширения Azure, центр безопасности не переопределяет установленное подключение рабочей области. Он использует существующую рабочую область. При условии, что решение «Безопасность» или «SecurityCenterFree» был установлен в рабочую область, которую он отправляет отчеты для виртуальной Машины будут защищены. 

Центр безопасности устанавливается на рабочей области, выбранного в окне сбора данных Если еще не установлен, и решение применяется только к релевантным виртуальным машинам. Когда вы добавляете решение, оно по умолчанию автоматически развертывается на всех агентах Windows и Linux, подключенных к рабочей области Log Analytics. Функция [нацеливания решений](../operations-management-suite/operations-management-suite-solution-targeting.md) позволяет применять к решениям область действия.
...
### Почему защитить изменение значений оценки? <a name="secure-score-faq"></a>
Начиная с февраля 2019 г. центр безопасности скорректировать оценку несколько рекомендаций, чтобы лучше приспособить их серьезности. В результате Эта корректировка возможны изменения в целом Защита значения оценки.  Дополнительные сведения о Оценка безопасности, см. в разделе [Secure расчета показателей](security-center-secure-score.md).
