...
---
# <a name="protect-your-network-resources"></a>Защитите свои сетевые ресурсы
...
В этой статье объясняется страница **Сети** в разделе безопасности ресурсов Центра безопасности.

Полный список рекомендаций для сетевых сетей можно узнать в [рекомендациях Networking.](recommendations-reference.md#recs-network)

В этой статье рассматриваются рекомендации по безопасности сети для ресурсов Azure. Рекомендации по сети сосредоточены вокруг брандмауэров следующего поколения, групп сетевой безопасности, доступа JIT VM, чрезмерно разрешительных входящих правил дорожного движения и многое другое. Список рекомендаций для сети и действия по исправлению проблем см. в статье [Управление рекомендациями по безопасности в центре безопасности Azure](security-center-recommendations.md).
...
- Им, подсетями и VNets типа ресурсов resource Manager (ресурсы Classic Azure не поддерживаются)
- Peered VNets
...
1. При выборе определенного ресурса на карте справа открывается панель с общими сведениями о ресурсе, подключенными решениями по безопасности (при наличии) и рекомендациями для ресурса. Так вы можете просмотреть ресурс любого типа. 
2. Нажмите **Трафик**, чтобы увидеть список возможного входящего и исходящего трафика для ресурса — это полный список всех объектов, которые могут взаимодействовать с ресурсом и с которыми он может взаимодействовать, а также всех протоколов и портов. Например, при выборе VM отображаются все вм: ВМ, с которыми он может общаться, и при выборе подсети отображаются все подсети, с которыми он может общаться.
...
[![Наследие Сети](./media/security-center-network-recommendations/legacy-networking.png)](./media/security-center-network-recommendations/legacy-networking.png#lightbox)
...
В представлении топологии на первом уровне находятся виртуальные сети. Второй отображает подсети, а третий уровень отображает виртуальные машины, которые принадлежат к этим подсетям. В правом столбце отображается текущее состояние рекомендаций по группам безопасности сети для этих ресурсов.

На третьем уровне находятся виртуальные машины, как описано ранее. Вы можете нажать на любой ресурс, чтобы узнать больше или применить необходимый контроль безопасности или конфигурацию.
