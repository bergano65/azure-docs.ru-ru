...
### <a name="cloud-services"></a>Облачные сервисы
...
2. Поиск политики **агент развертывания Log Analytics для масштабируемых наборов виртуальных машин Windows** и щелкните его.
...
|Служба приложений|10|Удаленная отладка должна быть отключена для веб-приложений.|Отключите режим отладки для веб-приложений, если использовать его больше не нужно. Для удаленной отладки нужно, чтобы в приложении-функции были открыты входящие порты.|
...
|Служба приложений|10|Настройте ограничение IP-адресов для веб-приложений|Определите список IP-адресов, с которых разрешен доступ к вашему приложению. Ограничение IP-адресов обеспечивает защиту веб-приложения от обычных атак.|
...
|Служба приложений|20|Средства CORS не должны разрешать всем ресурсам доступ к вашему приложению-функции| Разрешите взаимодействие с приложением-функцией только необходимым доменам. Общий доступ к ресурсам независимо от источника (CORS) не должен разрешать доступ к вашему приложению-функции всем доменам.|
|Вычислительные ресурсы (пакетная служба)|1|Настройка правил оповещений по метрикам в учетной записи пакетной службы|Настройте правила оповещений по метрикам в учетной записи пакетной службы и включите метрики Pool Delete Complete Events (События завершения удаления пула) и Pool Delete Start Events (События начала удаления пула).|
...
|Вычислительные ресурсы (поиск)|5|Включение журналов диагностики в службе поиска|Включите журналы и сохраняйте их в течение года. Это позволит воссоздать следы действий для анализа инцидентов безопасности или при компрометации сети. |
|Вычислительные ресурсы (служебная шина)|5|Включение журналов диагностики в служебной шине|Включите журналы и сохраняйте их в течение года. Это позволит воссоздать следы действий для анализа инцидентов безопасности или при компрометации сети. |
|Вычислительные ресурсы (Stream Analytics)|5|Включение журналов диагностики в Azure Stream Analytics|Включите журналы и сохраняйте их в течение года. Это позволит воссоздать следы действий для анализа инцидентов безопасности или при компрометации сети. |
...
|Вычислительные ресурсы (пакетная служба)|5|Включение журналов диагностики в учетных записях пакетной службы|Включите журналы и сохраняйте их в течение года. Это позволит воссоздать следы действий для анализа инцидентов безопасности или при компрометации сети. |
|Вычислительные ресурсы (концентратор событий)|5|Включение журналов диагностики в концентраторе событий|Включите журналы и сохраняйте их в течение года. Это позволит воссоздать следы действий для анализа инцидентов безопасности или при компрометации сети. |
|Вычислительные ресурсы (Logic Apps)|5|Включение журналов диагностики в Logic Apps|Включите журналы и сохраняйте их в течение года. Это позволит воссоздать следы действий для анализа инцидентов безопасности или при компрометации сети. |
|Вычислительные ресурсы (Service Fabric)|15|Задание свойству ClusterProtectionLevel значения EncryptAndSign в Service Fabric|Service Fabric поддерживает три уровня защиты (None (Нет), Sign (Подписывание) и EncryptAndSign (Шифрование и подписывание)) для взаимодействия между узлами с использованием основного сертификата кластера.  Задайте уровень защиты, чтобы обеспечить шифрование и цифровое подписывание всех сообщений, передаваемых между узлами. |
...
|Машина|50|Установите агент наблюдения на ваших компьютерах|Установите Monitoring Agent для включения сбора данных, сканирования обновлений, сканирования базовых показателей и защиты конечных точек на каждом компьютере.|
...
|Машина|40|Устраните проблемы работоспособности агента наблюдения на ваших компьютерах|Для полной защиты в центре безопасности решите проблемы Monitoring Agent на своих компьютерах, следуя инструкциям по устранению неполадок| 
|Машина|40|Устраните проблемы работоспособности защиты конечных точек на ваших компьютерах|Для полной защиты в центре безопасности решите проблемы Monitoring Agent на своих компьютерах, следуя инструкциям по устранению неполадок.|
|Машина|40|Устраните проблемы с отсутствующими данными сканирования на ваших компьютерах|Устраните проблемы с отсутствующими данными сканирования на виртуальных машинах и компьютерах. Отсутствующие данные сканирования на виртуальных машинах приводят к отсутствию оценок безопасности, таких как сканирование обновлений, сканирование базовых показателей, а также к отсутствию сканирования решения для защиты конечных точек.|
|Машина|40|Установите обновления системы на ваших компьютерах|Установите недостающие системные обновления безопасности и критические обновления для защиты виртуальных машин и физических компьютеров с ОС Windows и Linux.
|Машина|15|Добавить брандмауэр веб-приложения| Разверните решение брандмауэра веб-приложения (WAF) для обеспечения безопасности веб-приложений. |
...
|Машина|35|Устраните уязвимости в конфигурации безопасности на ваших компьютерах|Устраните уязвимости в конфигурации безопасности на компьютерах для их защиты от атак. |
|Машина|35|Устраните уязвимости в конфигурации безопасности контейнеров|Устраните уязвимости в конфигурации безопасности на компьютерах с Docker для защиты этих компьютеров от атак.|
|Машина|25|Включить адаптивное управление приложениями|Включите управление приложениями для определения приложений, которые могут запускаться на виртуальных машинах в среде Azure. Это поможет усилить защиту виртуальных машин от вредоносных программ. Центр безопасности анализирует приложения, выполняющиеся на каждой виртуальной машине, с помощью машинного обучения и на основе этих аналитических данных помогает применять правила разрешений. Эта возможность упрощает настройку и поддержку правил разрешения приложений.|
|Машина|20|Установите решение защиты конечных точек на ваших компьютерах|Установите решение для защиты конечных точек на виртуальных машинах для их защиты от угроз и уязвимостей.|
...
|Машина|30|Установка решения для оценки уязвимости на виртуальных машинах|Установка решения для оценки уязвимости на виртуальных машинах|
|Машина|15|Добавить брандмауэр веб-приложения| Разверните решение брандмауэра веб-приложения (WAF) для обеспечения безопасности веб-приложений. |
...
|Машина|30|Устраните уязвимости с помощью решения для оценки уязвимостей|Виртуальные машины, для которых развертывается решение по оценке уязвимостей от стороннего производителя, постоянно оцениваются на предмет уязвимостей приложений и ОС. На случай обнаружения таких уязвимостей в рекомендациях есть дополнительные сведения.|
|Набор масштабирования виртуальной машины |4.|Включите журналы диагностики в масштабируемых наборах виртуальных машин|Включите журналы и сохраняйте до года. Это позволяет воссоздать действия в целях контроля. Это полезно в том случае, если произойдет инцидент с безопасностью, или в случае несанкционированного доступа к сети.|
|Набор масштабирования виртуальной машины|35|Устранение уязвимостей в конфигурации безопасности в масштабируемом наборе виртуальных машин|Устраните уязвимости в конфигурации безопасности в масштабируемых наборах виртуальных машин для их защиты от атак. |
|Набор масштабирования виртуальной машины|5|Устранение проблем с работоспособностью защиты конечных точек на масштабируемом наборе виртуальных машин|Устраните сбои работоспособности конечных точек в масштабируемых наборах виртуальных машин для их защиты от угроз и уязвимостей. |
|Набор масштабирования виртуальной машины|10|Установка решения защиты конечных точек на масштабируемом наборе виртуальных машин|Установите решение для защиты конечных точек в масштабируемых наборах виртуальных машин для их защиты от угроз и уязвимостей. |
|Набор масштабирования виртуальной машины|40|Установка обновлений системы на масштабируемые наборы виртуальных машин|Установите недостающие обновления безопасности и критические обновления для защиты масштабируемых наборов виртуальных машин Windows и Linux. |
