...
3. Джефф должен решить, какие рекомендации нужно применить в первую очередь. Поэтому Джефф щелкает "безопасный показатель" и начинает обработку рекомендаций в зависимости от того, насколько она повышает его [влияние на безопасность](security-center-secure-score.md).
