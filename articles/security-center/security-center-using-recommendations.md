...
6. Джефф имеет множество виртуальных машин, подключенных к Интернету, и, поскольку их порты предоставляются, они беспокоит, что злоумышленник может получить контроль над серверами. Поэтому Джефф решает использовать **JIT-доступ к виртуальной машине** [security-center просто в time.md].
