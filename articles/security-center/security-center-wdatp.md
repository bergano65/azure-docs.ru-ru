...
- **Автоматическая адаптация**. Центр безопасности автоматически включает защитник Майкрософт для датчика конечных точек для всех серверов Windows, отслеживаемых центром безопасности. За исключением тех, которые работают под управлением Windows Server 2019, которые должны быть подключены с помощью локального скрипта, групповая политика объекта (GPO) или [Microsoft Endpoint Configuration Manager](/mem/configmgr/) (прежнее название — SCCM).
