...
> [!NOTE]
> Информационная панель событий безопасности будет прекращена 31 июля 2019 г. Дополнительные сведения и альтернативные службы, см. в разделе [вывода из эксплуатации центра безопасности функций (июль 2019)](security-center-features-retirement-july2019.md#menu_events).
...
> [!NOTE]
> В области **Информационная панель событий** в столбце **События** показано количество событий в каждой рабочей области.  Этот столбец пуст для некоторых рабочих областей, так как к ним применяется центр безопасности уровня "Бесплатный". Уровня "бесплатный" центра безопасности будет собирать события, но события не сохраняются в журналах Azure Monitor и не доступны на панели мониторинга.
>
>
...
- [Основные сведения о по журналам в Kusto](../log-analytics/log-analytics-log-search-new.md) — Описание использования поисков по журналам в журналах Azure Monitor и основные понятия, которые следует рассмотреть перед созданием поиск по журналам
- [Справочник по поиску Kusto](../log-analytics/log-analytics-search-reference.md) — Узнайте, как создавать собственные запросы событий на языке запросов в журнале
