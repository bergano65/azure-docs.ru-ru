...
> [!NOTE]
> Панель мониторинга событий безопасности была прекращена 31 июля 2019. Дополнительные сведения и альтернативные службы см. в разделе [выбытие средств центра безопасности (июль 2019)](security-center-features-retirement-july2019.md#menu_events).
...
Центр безопасности предоставляет готовые важные события. Вы можете добавлять важные события на основе собственного запроса, используя [язык запросов Kusto](../log-analytics/log-analytics-search-reference.md). Вернемся к области **Информационная панель событий**, чтобы добавить важное событие.
...
> [!NOTE]
> В области **Информационная панель событий** в столбце **События** показано количество событий в каждой рабочей области.  Этот столбец пуст для некоторых рабочих областей, так как к ним применяется центр безопасности уровня "Бесплатный". На уровне Free центр безопасности соберет события, но события не сохраняются в журналах Azure Monitor и недоступны на панели мониторинга.
>
>
...
- [Основные сведения о поиске по журналам в Kusto](../log-analytics/log-analytics-log-search-new.md) — описывает, как поиск по журналам используется в журналах Azure Monitor и содержит основные понятия, которые следует знать перед созданием поиска по журналам.
- [Справочник по поиску Kusto](../log-analytics/log-analytics-search-reference.md) . Узнайте, как создавать собственные запросы событий с помощью языка запросов в журнале.
