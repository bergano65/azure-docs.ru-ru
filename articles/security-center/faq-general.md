...
Кроме того, центр безопасности Azure также может отслеживать не относящиеся к Azure (включая локальные) компьютеры. Поддерживаются как [Компьютеры Windows](./quick-onboard-windows-computer.md) , так и [компьютеры Linux](./quick-onboard-linux-computer.md) .
