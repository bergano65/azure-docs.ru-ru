...
- Какие из моих подписок уровня "Стандартный" имеют непревзойденные рекомендации?
- На каких компьютерах с тегом "Production" отсутствует Log Analytics агент?
- На скольких компьютерах, помеченных с помощью определенного тега, имеются необработанные рекомендации?
...
- **Фильтры** . несколько фильтров в верхней части страницы позволяют быстро уточнить список ресурсов в соответствии с вопросом, на который вы пытаетесь ответить. Например, если вы хотите ответить на вопрос о том, *что у моих компьютеров с тегом "Production" отсутствует агент log Analytics?* фильтр **мониторинга агента** можно объединить с фильтром **тегов** , как показано на следующем рисунке:
    ...
    ![Фильтрация рабочих ресурсов, которые не отслеживаются](./media/asset-inventory/filtering-to-prod-unmonitored.gif)
...
- **Параметры управления активами** — Инвентаризация позволяет выполнять сложные запросы на обнаружение. Когда вы нашли ресурсы, которые соответствуют запросам, Инвентаризация предоставляет следующие сочетания клавиш для таких операций, как:
    ...
    - Подключение новых серверов к центру безопасности — использование кнопки " **Добавить серверы, не относящиеся к серверам Azure** "
...
1. При необходимости для вывода определенного ресурса введите имя в поле **Фильтр по имени** .

1. Выберите соответствующие параметры в фильтрах, чтобы создать конкретный запрос, который необходимо выполнить.

    ![Фильтры запасов](./media/asset-inventory/inventory-filters.png)
...
1. Чтобы использовать фильтр **ценовой категории** , выберите один или несколько параметров ("бесплатный", "частичный" или "Стандартный"):
    ...
    - **Стандартные** — ресурсы, которые находятся в ценовой категории "Стандартный"
    - **Частичное** — это относится к подпискам, которые находятся в ценовой категории "Стандартный", но с отключенными дополнительными планами безопасности. Например, следующая подписка находится на уровне Standard, но в ней отключено пять элементов уровня "Стандартный". 

        ![Подписка в ценовой категории Standard (частичная)](./media/asset-inventory/pricing-tier-partial.png)

1. Чтобы проанализировать результаты запроса, выберите интересующие вас ресурсы.

1. При необходимости выберите **вид в обозревателе ресурсов обозреватель** , чтобы открыть запрос в обозревателе графа ресурсов.
...
В представлении "Инвентаризация" перечисляются ресурсы с точки зрения Cloud Security Management (КСПМ). Фильтры не возвращают каждый ресурс в вашей среде; только те, у которых есть необработанные (или "активные") рекомендации. 

Например, если у вас есть девять подписок, но в настоящее время у вас есть рекомендации, то при фильтрации по **типу ресурса = подписки** вы увидите только восемь подписок с активными рекомендациями:

![Не все возвращены, если нет активных рекомендаций](./media/asset-inventory/filtered-subscriptions-some.png)


### <a name="why-do-some-of-my-resources-show-blank-values-in-the-pricing-or-agent-monitoring-columns"></a>Почему некоторые ресурсы показывают пустые значения в столбцах "цены" или "Мониторинг агента"?
...
![Некоторые ресурсы показывают пустые сведения в столбцах "Мониторинг агента" или "цены"](./media/asset-inventory/agent-pricing-blanks.png)
