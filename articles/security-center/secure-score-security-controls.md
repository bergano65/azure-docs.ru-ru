...
Центр безопасности постоянно оценивает ресурсы, подписки и организацию на предмет проблем безопасности. Затем он объединяет все результаты в одну оценку, чтобы вы могли понять текущее состояние системы безопасности: чем выше оценка, тем ниже выявленный уровень риска. Используйте эту оценку, чтобы следить за инициативами и проектами в своей организации. 
...
- **Элементы управления безопасностью**. Каждый элемент управления является логической группой связанных рекомендаций по безопасности и отражает уязвимые области атак. Элемент управления — это набор рекомендаций по безопасности с инструкциями по их внедрению. Оценка повышается только при выполнении *всех* рекомендаций для отдельного ресурса в элементе управления.
    ...
    Дополнительные сведения см. в разделе [Как вычисляется оценка безопасности](secure-score-security-controls.md#how-the-secure-score-is-calculated) ниже. 
...
> Так как данная улучшенная версия все еще находится на этапе предварительной версии, на портале Azure доступна возможность использовать предыдущий способ вычисления оценки безопасности. 


## <a name="locating-your-secure-score"></a>Поиск оценки безопасности

Центр безопасности наглядно отображает вашу оценку: это первое, что показано на странице обзора. Если перейти на специальную страницу оценки безопасности, вы увидите, что оценка разбивается по подпискам. Щелкните одну подписку, чтобы просмотреть подробный список приоритетных рекомендаций и их потенциальное влияние на оценку подписки.

## <a name="how-the-secure-score-is-calculated"></a>Как вычисляется оценка безопасности 
...
## <a name="improving-your-secure-score"></a>Повышение оценки безопасности
...
<tbody>
  ...
  <tr>
    ...
    <td class="tg-lboi"; width=55%>- На виртуальных машинах должен применяться JIT-доступ к сети.<br>- Виртуальные машины должны быть связаны с группой безопасности сети.<br>- Порты управления на виртуальных машинах должны быть закрыты.</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-lboi"; width=55%>- На серверах SQL Server должна быть включена Расширенная защита данных.<br>- Должны быть устранены уязвимости в образах Реестра контейнеров Azure.<br>- Уязвимости в базах данных SQL должны быть устранены.<br>- Уязвимости должны быть устранены с помощью решения для оценки уязвимостей.<br>- В управляемых экземплярах SQL должна быть включена оценка уязвимостей.<br>- На серверах SQL Server должна быть включена оценка уязвимостей.<br>- Необходимо установить решение для оценки уязвимостей на виртуальных машинах.</td>
  </tr>
  <tr>
    ...
    <td class="tg-lboi"; width=55%>- На виртуальных машинах должно применяться шифрование дисков.<br>- В базах данных SQL должно применяться прозрачное шифрование данных.<br>- Необходимо включить шифрование для переменных учетной записи службы автоматизации.<br>- Свойство ClusterProtectionLevel в кластерах Service Fabric должно иметь значение EncryptAndSign.<br>- Предохранитель TDE сервера SQL должен быть зашифрован вашим собственным ключом.</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-lboi"; width=55%>- Необходимо удалить устаревшие учетные записи из вашей подписки (предварительная версия).<br>- Необходимо удалить устаревшие учетные записи с разрешениями владельца из вашей подписки (предварительная версия).<br>- Необходимо удалить внешние учетные записи с разрешениями владельца из вашей подписки (предварительная версия).<br>- Необходимо удалить внешние учетные записи с разрешениями на запись из вашей подписки (предварительная версия).<br>- Подписке должно быть назначено более одного владельца.<br>- В службах Kubernetes следует использовать управление доступом на основе ролей (RBAC) (предварительная версия).<br>- Кластеры Service Fabric должны использовать только Azure Active Directory для аутентификации клиентов.</td>
  </tr>
  <tr>
    ...
    <td class="tg-lboi"; width=55%>- В службах Kubernetes должны быть определены политики безопасности контейнеров pod.<br>- Необходимо устранить уязвимости в конфигурациях безопасности контейнера.<br>- Уязвимости конфигурации безопасности на ваших компьютерах должны быть устранены.<br>- Уязвимости в конфигурации безопасности в масштабируемом наборе виртуальных машин должны быть устранены.<br>- Агент наблюдения должен быть установлен на виртуальных машинах.<br>- Необходимо установить агент наблюдения на ваших компьютерах.<br>- Необходимо установить агент наблюдения в масштабируемых наборах виртуальных машин.<br>- Необходимо устранить проблемы работоспособности агента наблюдения на ваших компьютерах.</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-lboi"; width=55%>- Должна быть включена Защита от атак DDoS (цен. категория "Стандартный").</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-lboi"; width=55%>- Подписке должно быть назначено не более 3 владельцев.<br>- Внешние учетные записи с разрешениями на чтение должны быть удалены из подписки.<br>- Для учетных записей с разрешениями на чтение в вашей подписке должна быть включена Многофакторная идентификация.<br>- Необходимо ограничить доступ к учетным записям хранения в конфигурациях брандмауэра и виртуальной сети.<br>- Из пространства имен концентратора событий должны быть удалены все правила авторизации, за исключением RootManageSharedAccessKey.<br>- Для серверов SQL Server должен быть подготовлен администратор Azure Active Directory.<br>- Необходимо определить правила авторизации в экземпляре концентратора событий.<br>- Необходимо перенести учетные записи хранения на новые ресурсы Azure Resource Manager.<br>- Виртуальные машины должны быть перенесены на новые ресурсы Azure Resource Manager.<br>- Параметры Расширенной защиты данных для сервера SQL Server должны содержать адрес электронной почты для получения оповещений системы безопасности.<br>- Расширенная защита данных должна быть включена в управляемых экземплярах.<br>- В параметрах Расширенной защиты данных управляемого экземпляра SQL должны быть включены все типы расширенной защиты от угроз.<br>- В параметрах Расширенной защиты данных для сервера SQL Server следует включить уведомления по электронной почте для администраторов и владельцев подписок.<br>- Для параметра "Типы Расширенной защиты от угроз" в параметрах Расширенной защиты данных сервера SQL Server необходимо установить значение "Все".<br>- Подсети должны быть связаны с группой безопасности сети.<br>- В параметрах Расширенной защиты данных сервера SQL Server должны быть включены все типы расширенной защиты от угроз.<br>- [Предварительная версия.] Должен быть включен Windows Exploit Guard. <br>- [Предварительная версия.] Должен быть установлен агент гостевой конфигурации.</td>
  </tr>
</tbody>
</table>
...
### <a name="why-has-my-secure-score-gone-down"></a>Почему моя оценка безопасности снизилась?
Центр безопасности перешел на использование расширенной оценки безопасности (в настоящее время находится на этапе предварительной версии), что включает в себя изменения в способе вычисления оценки. Теперь необходимо выполнить все рекомендации для ресурса, чтобы получить баллы. Кроме того, теперь используется шкала оценок от 0 до 10.
...
### <a name="is-the-previous-experience-of-the-secure-score-still-available"></a>Доступна ли предыдущая модель вычисления оценки безопасности? 
Да. Пока что будут доступны обе модели, чтобы упростить переход. Однако предыдущая модель со временем перестанет использоваться. 
