...
## <a name="run-time-protection---real-time-threat-protection"></a>Защита во время выполнения — защита от угроз в режиме реального времени
...
Мы обнаруживаем угрозы на уровне узла и кластера AKS. Полные сведения см. в статье [Защита от угроз для контейнеров Azure](threat-protection.md#azure-containers).
...
Сканер Qualys не поддерживает такие образы, как "поверхностные образы [DOCKER](https://hub.docker.com/_/scratch/) ", или образы "дистролесс", которые содержат только приложение и его зависимости времени выполнения (без диспетчера пакетов, оболочки или ОС).

### <a name="how-does-we-scan-azure-security-center-scan-an-image"></a>Как сканируется образ в центре безопасности Azure?
Образ извлекается из реестра. Затем он выполняется в изолированной песочнице с сканером Qualys, который извлекает список известных уязвимостей.
