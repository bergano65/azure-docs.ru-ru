...
Марта — специалист по операциям безопасности. К их обязанностям относятся:
...
Сэм — это аналитик безопасности, а также следующие обязанности:
...
Так как Марта и Григорий выполняют разные роли, они будут использовать разные компоненты центра безопасности для получения сведений, необходимых для работы. Роман будет использовать **оповещения системы безопасности** в рамках ежедневного мониторинга.

![Оповещения безопасности](./media/security-center-incident-response/security-center-incident-response-fig3.png)

Она будет использовать их на этапах обнаружения и оценки. После того как Роман завершит начальную оценку, он может сообщить о ней, если требуется дополнительное исследование. На этом этапе Григорий воспользуется сведениями, полученными из центра безопасности и других источников, чтобы перейти к этапу диагностики.
...
Марта вошла на портал Azure и открыла консоль центра безопасности. В рамках ежедневных действий по мониторингу они начали просматривать предупреждения безопасности с высоким приоритетом, выполнив следующие действия.
...
После прочтения описания Роман убедился в том, что это не ложный положительный результат, и что он должен эскалировать этот случай с SAM.
