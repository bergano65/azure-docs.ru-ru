...
Мы рекомендуем использовать встроенную в центр безопасности функцию карт оповещений. **Схема оповещений системы безопасности (Предварительная версия)** . Эта функция обеспечивает оптимизированный интерфейс и работает во всех подписках и связанных рабочих областях. Она обеспечивает высокоуровневое представление в среде и не ориентировано на отдельную рабочую область.
...
Мы вышли из этой предварительной версии функции. Чтобы восстановить параметры безопасности по умолчанию после даты выбытия, сделайте это через API или PowerShell, выполнив [следующие инструкции](https://aka.ms/ascresetsecurityconfigurations) .
