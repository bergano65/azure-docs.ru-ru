...
description: В этой статье описываются функции центра безопасности, которые будут прекращены 31 июля 2019 г.
...
Мы внесли несколько [улучшений](https://azure.microsoft.com/updates/?product=security-center) в центр безопасности Azure за последние шесть месяцев.
Благодаря этим усовершенствованным возможностям мы удаляем некоторые избыточные функции и связанные API из центра безопасности 31 июля 2019.  

Большинство этих функций снятия с учета можно заменить новыми функциями в центре безопасности Azure или Azure Log Analytics. Другие функции можно реализовать с помощью [Azure Sentinel (Предварительная версия)](https://azure.microsoft.com/services/azure-sentinel/).

Функции центра безопасности для снятия с учета:
...
Мы отменяем панель мониторинга событий, которая отображается при выборе рабочей области:
...
Мы провыкли все три экрана, показанные на предыдущих шагах. Данные останутся доступными в решении Log Analytics безопасности и не будут изменены или удалены.
...
Мы удалим кнопку **Переход к сопоставлению событий безопасности** и панель мониторинга для каждой рабочей области.

![Кнопка "схема оповещений системы безопасности"][10]

При нажатии кнопки **Перейти к карте событий безопасности** открывается панель мониторинга "аналитика угроз". Мы отменяем панель мониторинга аналитики угроз.  

![Панель мониторинга для анализа угроз][11]

При выборе рабочей области для просмотра ее панели мониторинга для анализа угроз необходимо открыть экран "схема оповещений системы безопасности" (Предварительная версия) в Log Analytics. Этот экран будет снят с учета.

![Оповещения системы безопасности сопоставлены в Log Analytics][12]

Существующие данные останутся доступными в решении Log Analytics безопасности и не будут изменены и удалены.
...
Мы пропустим [Пользовательский интерфейс оповещений](https://docs.microsoft.com/azure/security-center/security-center-custom-alert) 30 июня 2019, так как его базовая инфраструктура снимается с учета. До этого момента можно изменить существующие пользовательские правила генерации оповещений, но вы не сможете добавлять новые. После даты выбытия все определенные пользовательские предупреждения не вступят в силу, а оповещения системы безопасности, основанные на этих правилах, не будут созданы.
...
Для сохранения существующих оповещений и их создания с помощью Azure Sentinel:
...
[Функция расследования](https://docs.microsoft.com/azure/security-center/security-center-investigation) в центре безопасности помогает рассмотреть потенциальный инцидент безопасности. Эта функция позволяет понять область инцидента и оценить ее основную причину. Эта функция удаляется из центра безопасности, так как она была заменена с помощью [Azure Sentinel](https://azure.microsoft.com/services/azure-sentinel/).

![Инцидент][14]

Когда вы наберете кнопку **исследовать** на экране **инцидента безопасности** , вы открываете панель мониторинга для расследования (Предварительная версия) в log Analytics. Мы провыкли панель мониторинга расследования.  
...
Центр безопасности может включать [интегрированные решения безопасности в Azure](https://docs.microsoft.com/azure/security-center/security-center-partner-integration). Мы вывыкли следующие решения партнеров из центра безопасности. Эти решения включены в [Azure Sentinel](https://azure.microsoft.com/services/azure-sentinel/) вместе с несколькими дополнительными источниками данных.
...
Центр безопасности Azure отслеживает конфигурации безопасности, применяя набор из [более чем 150 рекомендуемых правил](https://gallery.technet.microsoft.com/Azure-Security-Center-a789e335). для усиления защиты операционной системы. Эти правила относятся к брандмауэрам, аудиту, политикам паролей и т. д. Если конфигурация компьютера уязвима к атакам, центр безопасности формирует рекомендации по безопасности. [Экран изменение конфигурации безопасности](https://docs.microsoft.com/azure/security-center/security-center-customize-os-security-config) позволяет пользователям настраивать конфигурацию безопасности ОС по умолчанию в центре безопасности.

Мы отменяем эту функцию предварительной версии. Если после даты выбытия вы хотите сбросить настройки безопасности к значениям по умолчанию, это можно сделать с помощью API или PowerShell, выполнив [приведенные ниже инструкции](https://aka.ms/ascresetsecurityconfigurations) .
