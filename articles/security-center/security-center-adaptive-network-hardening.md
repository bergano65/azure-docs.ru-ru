...
1. В центре безопасности, выберите **сети** -> **адаптивной Усиление защиты сети**. Виртуальные машины сетевого отображаются в категории три отдельные вкладки:
   ...
   * **Непроверенных ресурсы**: Виртуальные машины, которые алгоритм адаптивной Усиление защиты сети не может выполняться на одной из следующих причин:
...
3. Выберите правила, которые вы хотите применить группу безопасности сети и нажмите кнопку **принудительное применение**.
      ...
      > [!NOTE]
      > Применение правила добавляются к NSG(s), защищающую эту виртуальную Машину. (Виртуальная машина позволяет защитить их, связанного с ее сетевого Адаптера, группы безопасности сети или подсети, в которой размещена виртуальная машина)
...
* Объект **запретить весь трафик** правило является единственным типом правила «запретить», здесь будут перечислены, и не может быть изменено. Тем не менее, можно удалить его (см. в разделе [удаление правила](#delete-rule)).
  > [!NOTE]
  > Объект **запретить весь трафик** правило рекомендуется, в результате выполнения алгоритма, Центр безопасности не идентификации трафика, который должно быть разрешено, на основе существующей конфигурации группы безопасности сети. Таким образом рекомендуется правило находится в том, чтобы запретить весь трафик к указанному порту. Имя этого типа правила отображается как "*созданные системой*«. После применения этого правила, его фактическое имя в группу безопасности сети будет строка, состоящая из протокол, направление трафика, «DENY» и случайное число.

*Чтобы изменить правило адаптивной Усиление защиты сети:*

1. Чтобы изменить некоторые параметры правила, в **правила** вкладке, щелкните многоточие (...) в конце строки правила и нажмите кнопку **изменить**.
...
1. В **изменить правило** окно, обновление сведений, которые вы хотите изменить и нажмите кнопку **Сохранить**.
   ...
   ![Изменение правила](./media/security-center-adaptive-network-hardening/edit-hard-rule3.png)
...
1. В **новое правило** окно, введите сведения и нажмите кнопку **добавить**.

   > [!NOTE]
   > После нажатия кнопки **добавить**успешно добавлены правила и отобразится рекомендуемых правил. Тем не менее еще не применены ее в группу безопасности сети. Чтобы активировать его, необходимо выбрать правило в списке и нажмите кнопку **принудительное применение** (как описано в следующем шаге).
...
1. В **правила** вкладке, щелкните многоточие (...) в конце строки правила и нажмите кнопку **удалить**.  
