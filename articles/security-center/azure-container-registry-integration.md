...
Для более глубокого ознакомления с уязвимостями реестра и образов пользователи уровня "Стандартный" центра безопасности Azure могут включить дополнительный пакет реестров контейнеров. Плата за использование этой функции насчитывается отдельно для каждого образа, а не для проверки. Дополнительные сведения см. на странице [цен](security-center-pricing.md). При включенном наборе центр безопасности автоматически сканирует изображения в реестре каждый раз, когда в реестр помещается образ.
