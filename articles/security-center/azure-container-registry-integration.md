...
Если вы используете уровень "Стандартный" центра безопасности Azure, можно добавить пакет реестров контейнеров. Эта дополнительная функция позволяет более подробно ознакомиться с уязвимостями образов в реестров на основе ARM. Включите или отключите набор на уровне подписки, чтобы охватить все реестры в подписке. Плата за этот компонент насчитывается отдельно для каждого образа, а не для проверки, как показано на [странице с ценами](security-center-pricing.md). 

При включении пакета реестров контейнеров обеспечивается готовность центра безопасности к сканированию образов, помещаемых в реестр. Сканирование выполняется на уровне образа. Центр безопасности не проверяет реестр, он проверяет образы, хранящиеся в реестре. 
...
По завершении проверки (обычно через 10 минут) результаты доступны в следующих рекомендациях центра безопасности:
