...
> [!NOTE]
> Агент может собирать журналы из нескольких источников, но должны быть установлены на компьютере выделенных прокси-сервера.
...
## <a name="step-2-forward-common-event-format-cef-logs-to-syslog-agent"></a>Шаг 2. Накат журналов общий формат событий (CEF) для системного журнала агента

Установите решение безопасности для отправки сообщения системного журнала в формате CEF к своему агенту по системного журнала. Убедитесь, что использовать те же параметры, которые отображаются в конфигурации агента. Как правило, они:
