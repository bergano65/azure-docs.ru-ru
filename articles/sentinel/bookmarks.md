...
3. Выберите **Просмотреть результаты запроса**. Например:
...
7. В разделе **сведения о запросе** используйте раскрывающиеся списки для извлечения сведений из результатов запроса для типов сущностей **учетная запись**, **узел**и **IP-адрес** . Это действие сопоставляет выбранный тип сущности с конкретным столбцом из результата запроса. Например:
