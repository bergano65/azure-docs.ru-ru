...
5. В верхней части окна развернуто в Microsoft Azure Sentinel (Log Analytics) выполните следующие действия.
    -   **Имя**: введите имя сервера пересылки, длина которого не превышает 30 символов. Выберите уникальное, понятное имя. Это имя отображается в списке серверов пересылки на экране **конфигурации** и на панелях мониторинга на экране **панели мониторинга** . Например: Microsoft Azure Log Analytics Восток. Это поле обязательно к заполнению.
...
7. В разделе **назначение Azure**выполните следующие действия. 
    ...
    - **Первичный ключ**: вставьте ниже первичный ключ. Это поле обязательно к заполнению.
    - **Имя настраиваемого журнала**: введите имя пользовательского журнала в портал Microsoft Azure log Analytics рабочей области, в которую будут пересылаться события. Значение по умолчанию — СимантеЦикдкс. Это поле обязательно к заполнению.
