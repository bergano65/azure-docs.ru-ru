...
description: Подключите любое локальное устройство, поддерживающее syslog, в метку Azure, используя агент на компьютере Linux между устройством и Sentinel. 
services: sentinel
...
>    2. Чтобы подключить данные системного журнала, последовательно выберите **Параметры**  >  **Рабочая область Параметры**  >  **Дополнительные параметры**  >  **Data**  >  **Журнал syslog** и настройте средства и их приоритеты, чтобы они не совпадали с теми же возможностями и свойствами, которые использовались в конфигурации CEF. <br></br>Если выбран параметр **Применить ниже конфигурацию к моим компьютерам**, он применяет эти параметры числа ко всем виртуальным машинам, подключенным к этой рабочей области.
