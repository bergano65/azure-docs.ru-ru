...
| `plugin_flush_interval` | number | Дополнительное поле. Задайте значение, чтобы определить максимальный интервал (в секундах) между передачами сообщений в Log Analytics. Значение по умолчанию — 5. |
    | `amount_resizing` | Логическое | Значение true или false. Включите или отключите механизм автоматического масштабирования, который корректирует размер буфера сообщений в соответствии с объемом полученных данных журнала. |
