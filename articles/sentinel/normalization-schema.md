...
| Записей | Единица данных, отправляемых с устройства отчетов. Часто это называется «log», «Event» или «Alert», но не обязательно должен быть одним из них. |
|
...
| **MAC-адрес** | Тип String | Двоеточие — шестнадцатеричная нотация |
...
| **Пользователь** | Тип String | Доступны следующие три поля пользователя:<ul><li>Имя пользователя</li><li>Имя участника-пользователя</li><li>Домен пользователя</li></ul> |
| **Идентификатор пользователя** | Тип String | В настоящее время поддерживаются следующие 2 идентификатора пользователей:<ul><li>Идентификатор безопасности пользователя</li><li>Идентификатор Azure Active Directory</li></ul> |
| **Устройство** | Тип String | Поддерживаются следующие 3 столбца устройства/узла:<ul><li>ID</li><li>Имя</li><li>Полное доменное имя (FQDN)</li></ul> |
| **Страна** | Тип String | Строка, использующая ISO 3166-1 в соответствии с этим приоритетом:<ul><li>Коды Alpha-2 (т. е. US для США)</li><li>Коды Alpha-3 (т. е. USA для США)</li><li>короткое имя;</li></ul> |
| **Регион** | Тип String | Имя подразделения страны по ISO 3166-2 |
| **Город** | Тип String | |
| **Долгот** | Тип Double | Представление координат ISO 6709 (десятичное число со знаком) |
| **Широта** | Тип Double | Представление координат ISO 6709 (десятичное число со знаком) |
| **Хэш-алгоритм** | Тип String | Поддерживаются следующие 4 хэш-столбца:<ul><li>MD5</li><li>SHA1</li><li>SHA256</li><li>SHA512</li></ul> |
| **Тип файла** | Тип String | Тип файла:<ul><li>Расширение</li><li>Класс</li><li>намедтипе</li></ul> |
| 
...
|-|-|-|-|-|
| EventType | Тип String | Трафик | Тип собираемого события | Событие |
| евентсубтипе | Тип String | Аутентификация | Дополнительное описание типа, если применимо | Событие |
...
| двкмакаддр | Тип String | 06:10:9f: EB: 8F: 14 | MAC-адрес сетевого интерфейса устройства отчетов, с которого было отправлено событие. | устройство;<br>Mac |
| двчостнаме | Имя устройства (строка) | syslogserver1.contoso.com | Имя устройства, создающего сообщение. | Устройство |
| евентпродукт | Тип String | оффицешарепоинт | Продукт, создающий событие. | Событие |
...
| евентрепортурл | Тип String | https://192.168.1.1/repoerts/ae3-56.htm | Ссылка на полный отчет, созданный устройством создания отчетов | Событие |
| евентвендор | Тип String | Microsoft | Поставщик продукта, создающего событие. | Событие |
...
| евентресултдетаилс | Тип String | Неправильный пароль | Причина или подробные сведения для результата, полученного в Евентресулт | Событие |
| евентсчемаверсион | Real | 0,1 | Версия схемы Sentinel Azure. Сейчас 0,1. | Событие |
| евентсеверити | Тип String | Низкий | Если полученное действие имеет влияние на безопасность, обозначает серьезность воздействия. | Событие |
| евенторигиналуид | Тип String | af6ae8fe-ff43-4a4c-b537-8635976a2b51 | Идентификатор записи с устройства отчетов. | Событие |
...
| нетворкаппликатионпротокол | Тип String | HTTPS | Протокол уровня приложения, используемый соединением или сеансом. | Сеть |
...
| нетворкдиректион | Множественное значение: Inbound, Outbound (строка) | Входящие | Направление соединения или сеанса в организацию или из нее. | Сеть |
| дстжеоЦити | Тип String | Берлингтон | Город, связанный с IP-адресом назначения | Местоназначение<br>Географические |
...
| дстдвкфкдн | Тип String | victim_pc. contoso. local | Полное доменное имя узла, на котором был создан журнал | Местоназначение<br>Устройство |
...
| дстмакаддр | Тип String | 06:10:9f: EB: 8F: 14 | MAC-адрес сетевого интерфейса, в котором подключение или сеанс прерваны, чаще всего называемые конечным компьютером MAC в сетевом пакете. | Местоназначение<br>АДРЕС |
| дстдвкмакаддр | Тип String | 06:10:9f: EB: 8F: 14 | Конечный MAC-адрес устройства, которое не связано непосредственно с сетевым пакетом. | Местоназначение<br>устройство;<br>АДРЕС |
| дстдвкдомаин | Тип String | CONTOSO | Домен целевого устройства. | Местоназначение<br>Устройство |
| дстпортнумбер | Тип Integer | 443 | Конечный IP-порт. | Местоназначение<br>Порт |
...
| дстзоне | Тип String | DMZ | Зона сети назначения, определенная на устройстве отчетов. | Назначение |
...
| двЦинбаундинтерфаце | Тип String | eth0 | Если сообщение передается промежуточным устройством, например брандмауэром, то сетевой интерфейс, используемый им для подключения к исходному устройству. | Устройство |
| двкаутбаундинтерфаце | Тип String  | Ethernet адаптер Ethernet 4 | Если сообщение передается промежуточным устройством, например брандмауэром, то сетевой интерфейс, используемый им для подключения к целевому устройству. | Устройство |
| нетворкдуратион | Тип Integer | 1500 | Время в миллисекундах, необходимое для завершения сетевого сеанса или подключения | Сеть |
| нетворкикмпкоде | Тип Integer | 34 | Для ICMP-сообщения введите числовое значение типа ICMP (RFC 2780 или RFC 4443). | Сеть |
| нетворкикмптипе | Тип String | Объект назначения недоступен | Для сообщения ICMP введите текст сообщения типа ICMP (RFC 2780 или RFC 4443). | Сеть |
...
| хттпрекуесттиме | Тип Integer | 700 | Время, затраченное на отправку запроса на сервер, если применимо. | Http |
| HttpResponseTime | Тип Integer | 800 | Время, затраченное на получение ответа на сервере (если применимо). | Http |
| нетворкруленаме | Тип String | анянидроп | Имя или идентификатор правила, по которому было принято решение Девицеактион | Сеть |
...
| сркжеоЦити | Тип String | Берлингтон | Город, связанный с исходным IP-адресом | Источника<br>Географические |
...
| сркдвкос | Тип String | iOS | ОС исходного устройства | Источника<br>Устройство |
| сркдвкмоделнаме | Тип String | Samsung Galaxy, Примечание | Имя модели исходного устройства | Источника<br>Устройство |
| сркдвкмоделнумбер | Тип String | 10.0 | Номер модели исходного устройства | Источника<br>Устройство |
| сркдвктипе | Тип String | Мобильные приложения | Тип исходного устройства | Источника<br> Устройство |
| срЦинтефаценаме | Тип String | eth01 | Сетевой интерфейс, используемый исходным устройством для подключения или сеанса. | Источник |
| срЦинтерфацегуид | Тип String | 46ad544b-eaf0-47ef-827c-266030f545a6 | Идентификатор GUID используемого сетевого интерфейса | Источник |
...
| сркмакаддр | Тип String | 06:10:9f: EB: 8F: 14 | MAC-адрес сетевого интерфейса, из которого создан сеанс OD. | Источника<br>Mac |
| сркдвкмакаддр | Тип String | 06:10:9f: EB: 8F: 14 | Исходный MAC-адрес устройства, которое не связано непосредственно с сетевым пакетом. | Источника<br>устройство;<br>Mac |
| сркпортнумбер | Тип Integer | 2335 | IP-порт, с которого было создано подключение. Не может быть релевантным для сеанса, включающего несколько соединений. | Источника<br>Порт |
| сркжеорегион | Region (строка) | Vermont | Регион в стране, связанный с исходным IP-адресом | Источника<br>Географические |
| сркресаурцеид | Тип String | /Subscriptions/3c1bb38c-82e3-4f8d-A115-a7110ba70d05/resourcegroups/contoso77/providers/Микрософт.компуте/виртуалмачинес/syslogserver1 | Идентификатор ресурса устройства, создающего сообщение. | Источник |
...
| сркнатпортнумбер | Тип Integer | 345 | Если сообщается промежуточным устройством NAT, например брандмауэром, то порт, используемый устройством NAT для связи с назначением. | Источник NAT,<br>Порт |
...
| сркзоне | Тип String | Касание | Зона сети источника, определенная на устройстве отчетов. | Источник |
| NetworkProtocol | Тип String | TCP | IP-протокол, используемый соединением или сеансом. Обычно TCP, UDP или ICMP | Сеть |
| клаудаппнаме | Тип String | Facebook | Имя конечного приложения для HTTP-приложения, определяемое прокси-сервером. | Cloud |
| клаудаппид | Тип String | 124 | Идентификатор целевого приложения для HTTP-приложения, идентифицируемого прокси-сервером. Это значение обычно относится к используемому прокси-серверу. | Cloud |
| клаудаппоператион | Тип String | DeleteFile | Операция, выполненная пользователем в контексте приложения назначения для HTTP-приложения, определяемое прокси-сервером. Это значение обычно относится к используемому прокси-серверу. | Cloud |
| клаудапприсклевел | Тип String | 3 | Уровень риска, связанный с HTTP-приложением, определяемым прокси-сервером. Это значение обычно относится к используемому прокси-серверу. | Cloud |
...
| FilePath | Тип String | C:\Malicious\ImNotMalicious.exe | Полный путь к файлу, включая имя файла | Файл |
| FileHashMd5 | Тип String | 51BC68715FC7C109DCEA406B42D9D78F | Значение хэша MD5 для файла, переданного по сетевым подключениям для протоколов. | Файл |
| FileHashSha1 | Тип String | 491AE3... C299821476F4 | Хэш-значение SHA1 файла, переданного по сетевым подключениям для протоколов. | Файл |
| FileHashSha256 | Тип String | 9B8F8EDB... C129976F03 | Значение хэша SHA256 файла, переданного по сетевым подключениям для протоколов. | Файл |
| FileHashSha512 | Тип String | 5E127D... F69F73F01F361 | Значение хэша SHA512 файла, переданного по сетевым подключениям для протоколов. | Файл |
| FileExtension |  Тип String | exe | Тип файла, передаваемого по сетевым подключениям для таких протоколов, как FTP и HTTP. | Файл
| филемиметипе | Тип String | приложение или мсворд | Тип MIME файла, передаваемого по сетевым подключениям для таких протоколов, как FTP и HTTP. | Файл |
| FileSize | Тип Integer | 23500 | Размер файла в байтах, переданного по сетевым подключениям для протоколов. | Файл |
| HttpVersion | Тип String | 2.0 | Версия HTTP-запроса для сетевых подключений HTTP/HTTPS. | Http |
| хттпрекуестмесод | Тип String | GET | Метод HTTP для сетевых сеансов HTTP/HTTPS. | Http |
| HttpStatusCode | Тип String | 404 | Код состояния HTTP для сетевых сеансов HTTP/HTTPS. | Http |
| хттпконтенттипе | Тип String | multipart/form-data; граница = что-то | Заголовок типа содержимого ответа HTTP для сетевых сеансов HTTP/HTTPS. | Http |
| хттпреферрероригинал | Тип String | https://developer.mozilla.org/en-US/docs/Web/JavaScript | Заголовок HTTP-источника ссылки для сетевых сеансов HTTP/HTTPS. | Http |
| хттпусераженторигинал | Тип String | Mozilla/5.0 (Windows NT 10,0; WOW64) Апплевебкит/537.36 (КХТМЛ, например Gecko) Chrome/83.0.4103.97 Safari/537.36 | Заголовок HTTP-агента пользователя для сетевых сеансов HTTP/HTTPS. | Http |
| хттпрекуестксфф | Тип String | 120.12.41.1 | Заголовок HTTP X-Forwardd-for для сетевых сеансов HTTP/HTTPS. | Http |
| урлкатегори | Тип String | Поисковые системы | Определенная группировка URL-адреса (или, возможно, на основе домена в URL-адресе) связана с тем, что он представляет (т. е. взрослые, Новости, реклама, приостановленные домены и т. д.). | url |
| урлоригинал | Тип String | https://contoso.com/fo/?k=v&q = u # f | URL-адрес запроса HTTP для сетевых сеансов HTTP/HTTPS. | Url |
| урлхостнаме | Тип String | contoso.com | Доменная часть URL-адреса запроса HTTP для сетевых сеансов HTTP/HTTPS. | Url |
| ThreatCategory | Тип String | Типа | Категория угрозы, определяемая системой безопасности, например шлюзом безопасности IP-адресов и связанная с этим сетевым сеансом. | Угроза |
| среатид | Тип String | TR. 124 | Идентификатор угрозы, определяемый системой безопасности, например шлюзом безопасности IP-адресов и связанный с этим сетевым сеансом. | Угроза |
| ThreatName | Тип String | Файл теста EICAR | Имя обнаруженной угрозы или вредоносной программы | Угроза |
