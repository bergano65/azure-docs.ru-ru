...
3. Определение потери данных и восстановление из резервных копий.
   ...
   Когда Service Fabric вызывает `OnDataLossAsync` метод, это всегда обусловлено возможной _потерей данных._ Service Fabric всегда направляет этот вызов к _лучшей_ оставшейся реплике. Это любая реплика с лучшим показателем состояния. 
   ...
   1. Запущенные журналы `OnDataLossAsync` реализации и запускают все необходимые административные оповещения.
