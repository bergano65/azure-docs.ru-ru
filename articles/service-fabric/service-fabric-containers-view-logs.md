...
Доступ к журналам контейнеров можно получить с помощью [Service Fabric Explorer](service-fabric-visualizing-your-cluster.md).  В веб-браузере откройте Service Fabric Explorer из конечной точки управления кластера, перейдя к `http://mycluster.region.cloudapp.azure.com:19080/Explorer`.  
...
Параметр **RunInteractive** соответствует `--interactive`флагам`tty` [ и ](https://docs.docker.com/engine/reference/commandline/run/#options) в Docker. Если в файле манифеста этому параметра присвоено значение true, эти флаги используются для запуска контейнера.  
