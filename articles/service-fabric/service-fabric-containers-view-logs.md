---
title: Просмотр журналов контейнеров в структуре службы Azure
...
Журналы контейнеров расположены на узле кластера, на котором выполняется экземпляр службы контейнеров. Например, чтобы просмотреть журналы контейнера интерфейсной части [веб-приложения для голосования в Linux](service-fabric-quickstart-containers-linux.md). В представлении дерева расширьте ткань **Кластерных**>**приложений**>**VotingType:/Голосование/azurevotefront**>**fabric:/Voting/azurevotefront**.  Затем разверните раздел (здесь это d1aa737e-f22a-e347-be16-eec90be24bc1) и убедитесь, что контейнер запущен на узле кластера *_lnxvm_0*.
