...
Параметр **рунинтерактиве** соответствует [флагам](https://docs.docker.com/engine/reference/commandline/run/#options)`--interactive` и `tty` DOCKER. Если в файле манифеста этому параметра присвоено значение true, эти флаги используются для запуска контейнера.  
