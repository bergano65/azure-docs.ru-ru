---
title: Azure Service Fabric CLI — приложение sfctl
description: Сведения о sfctl, интерфейсе командной строки Azure Service Fabric. Содержит список команд для управления приложениями.
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
Подготавливает или регистрирует Service Fabric тип приложения в кластере с помощью пакета ". sfpkg" во внешнем хранилище или с помощью пакета приложения в хранилище образов.

Подготавливает тип приложения Service Fabric в кластере. Подготовка требуется перед созданием новых приложений. Операцию инициализации можно выполнить либо в пакете приложения, заданном параметром Релативепасинимажесторе, либо с помощью URI внешней ". sfpkg". Если значение --external-provision не задано, то эта команда ожидает подготовки хранилища образов.
...
| --health-check-retry-timeout | Продолжительность времени между попытками выполнить проверку работоспособности, если приложение или кластер неработоспособны.  Значение по умолчанию\: PT0H10M0S. |
| --health-check-stable-duration | Период времени, в течение которого приложение или кластер должен оставаться работоспособным, прежде чем процесс обновления перейдет к следующему домену обновления.  Значение по умолчанию\: PT0H2M0S. <br><br> Сначала он интерпретируется как строка, представляющая длительность ISO 8601. В случае неудачи он интерпретируется как число, представляющее общее количество миллисекунд. |
| --health-check-wait-duration | Продолжительность ожидания после завершения обновления домена до начала процесса проверки работоспособности.  Значение по умолчанию\: 0. |
...
| --timeout -t | Значение по умолчанию\: 60. |
| --upgrade-domain-timeout | Время выполнения каждого домена обновления, по истечении которого выполняется действие FailureAction.  Значение по умолчанию\: P10675199DT02H48M05.4775807S. <br><br> Сначала он интерпретируется как строка, представляющая длительность ISO 8601. В случае неудачи он интерпретируется как число, представляющее общее количество миллисекунд. |
...
|Аргумент|Описание|
| --- | --- |
| --path [обязательный] | Путь к локальному пакету приложения. |
| --сжатие | Применимо только к Service Fabricным пакетам приложений. Создайте новую папку, содержащую сжатый пакет приложения, в расположение по умолчанию или в расположение, указанное параметром сжатый путь, а затем отправьте созданную папку. <br><br> Если в sfctl уже создан сжатый файл, он будет перезаписан, если установлен этот флаг. Если каталог не является пакетом приложения, возвращается ошибка. Если он уже является сжатым пакетом приложения, папка будет скопирована как есть. По умолчанию вновь созданный сжатый пакет приложения будет удален после успешной отправки. Если отправка не прошла успешно, необходимо вручную очистить сжатый пакет по мере необходимости. Удаление не приводит к удалению пустых dirs, которые могли быть созданы, если параметр сжатого расположения ссылается на несуществующие каталоги. |
| --сжатое расположение | Расположение для размещения сжатого пакета приложения. <br><br> Если расположение не указано, сжатый пакет будет помещен в созданную папку с именем sfctl_compressed_temp в родительском каталоге, указанном в аргументе Path. Например, если аргумент path имеет значение C \: /Фолдера/апппкг, сжатый пакет будет добавлен в C \: /Фолдера/sfctl_compressed_temp/апппкг. |
| --imagestore-string | Целевое хранилище образов для передачи пакета приложения.  Значение по умолчанию\: fabric\:ImageStore. <br><br> Чтобы отправить файл в расположение, запустите этот параметр с файлом \: . В противном случае значение должно быть строкой подключения к хранилищу образов, например значением по умолчанию. |
| --Держитесь-сжатый | Необходимость сохранения созданного сжатого пакета при успешном завершении отправки. <br><br> Если не задано, то при успешном завершении сжатые пакеты приложений будут удалены. Если передача не была выполнена успешно, то пакет приложения всегда будет храниться в выходном каталоге для повторной отправки. |
| --show-progress | Отображение хода передачи файлов для больших пакетов. |
| --timeout -t | Общее время ожидания в секундах. Отправка завершается ошибкой и возвращает ошибку после истечения времени ожидания передачи. Это время ожидания применяется ко всему пакету приложения, а время ожидания отдельных файлов равно значению оставшегося времени ожидания. Параметр timeout не включает время, необходимое для сжатия пакета приложения.  Значение по умолчанию \: 300. |
