---
title: Приложение Azure Service Fabric CLI- sfctl
description: Узнайте о sfctl, интерфейсе командной строки Azure Service Fabric. Включает список команд для управления приложениями.
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
| provision | Предусматривает или регистрирует тип приложения Service Fabric с кластером, используя пакет '.sfpkg' во внешнем магазине или используя пакет приложений в хранилище изображений. |
...
Предусматривает или регистрирует тип приложения Service Fabric с кластером, используя пакет '.sfpkg' во внешнем магазине или используя пакет приложений в хранилище изображений.

Подготавливает тип приложения Service Fabric в кластере. Это положение необходимо, прежде чем любые новые приложения могут быть мгновенно. Операция обеспечения может быть выполнена либо на пакете приложений, указанном relativePathInImageStore, либо с помощью URI внешнего '.sfpkg'. Если значение --external-provision не задано, то эта команда ожидает подготовки хранилища образов.
...
|Аргумент|Описание|
| --- | --- |
| --путь (Обязательно) | Путь к локальному пакету приложения. |
| --компресс | Применяется только к пакетам приложений Service Fabric. Создайте новую папку, содержащую пакет сжатого приложения, либо к местоположению по умолчанию, либо к местоположению, указанному параметром сжатого местоположения, а затем загрузите вновь созданную папку. <br><br> Если уже есть сжатый файл, генерируемый sfctl, он будет перезаписан, если этот флаг установлен. Ошибка будет возвращена, если каталог не является пакетом приложений. Если это уже сжатый пакет приложений, папка будет скопирована как есть. По умолчанию недавно созданный пакет сжатого приложения будет удален после успешной загрузки. Если загрузка не удалась, пожалуйста, вручную очистите сжатый пакет по мере необходимости. Удаление не удаляет пустые реж. |
| --сжатое местоположение | Место для размещения пакета сжатого приложения. <br><br> Если место не предусмотрено, сжатый пакет будет помещен под недавно созданную папку под названием sfctl_compressed_temp под родительским каталогом, указанным в аргументе пути. Например, если аргумент пути\:имеет значение C /FolderA/AppPkg, то сжатый пакет будет добавлен в C\:/FolderA/sfctl_compressed_temp/AppPkg. |
| --imagestore-string | Целевое хранилище образов для передачи пакета приложения.  Значение по умолчанию\: fabric\:ImageStore. <br><br> Чтобы загрузить в местоположение файла, запустите этот параметр с 'файл'.\: В противном случае значение должно быть строкой соединения хранилища изображений, например значением по умолчанию. |
| --держать-сжатый | Сохранить или не сохранить сгенерированный сжатый пакет при успешном завершении загрузки. <br><br> Если не установить, то после успешного завершения, сжатые пакеты приложений будут удалены. Если загрузка не была успешной, то пакет приложений всегда будет храниться в каталоге вывода для повторной загрузки. |
| --show-progress | Отображение хода передачи файлов для больших пакетов. |
| --timeout -t | Общий тайм-аут в секундах. Загрузка не удастся и вернуть ошибку после того, как продолжительность тайм-аута загрузки прошла. Этот тайм-аут распространяется на весь пакет приложений, и отдельные тайм-ауты будут равен оставшейся продолжительности тайм-аута. Тайм-аут не включает время, необходимое для сжатия пакета приложения.  По\: умолчанию 300. |
