---
title: Azure Сервис Ткань CLI- sfctl раздел
description: Узнайте о sfctl, интерфейсе командной строки Azure Service Fabric. Включает список команд для управления разделами для службы.
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
| сведения | Возвращает сведения о секции Service Fabric. |
...
- PartialDataLoss: Удаляется только кворум реплик, и OnDataLoss срабатывает для раздела, но фактическая потеря данных зависит от наличия репликации в полете.  
- FullDataLoss: Все реплики удаляются, следовательно, все данные потеряны и OnDataLoss срабатывает. Этот API следует вызывать только со службой с отслеживанием состояния в качестве целевого объекта. Вызов этого API с системной службой в качестве целевого объекта не рекомендуется.
