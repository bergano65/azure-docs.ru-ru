...
- Центральное хранилище секретов (CSS).
    ...
    Добавьте приведенный ниже элемент в конфигурацию кластера `fabricSettings` в разделе, чтобы включить все необходимые функции для поддержки кэйваултреференце.
...
- Использование Кэйваултреференце в приложении
    ...
    Service Fabric при создании экземпляра службы будет разрешать параметр Кэйваултреференце с помощью управляемого удостоверения приложения. Каждый параметр, указанный `<Section  Name=dbsecrets>` в разделе, будет представлять собой файл в папке, на которую указывает EnvironmentVariable секретпас. Ниже приведен фрагмент кода C#, демонстрирующий чтение Дбпассворд в приложении.
    ...
    ```
    > [!NOTE] 
    > Для сценария контейнера точку подключения можно использовать для управления тем, `secrets` где будет подключено.
