...
- Центральное хранилище секретов (CSS).
    ...
    Добавьте приведенный ниже элемент в конфигурацию кластера в разделе `fabricSettings` , чтобы включить все необходимые функции для поддержки кэйваултреференце.
...
Предположим, что приложению необходимо считать пароль к серверной базе данных, хранящийся в keyvault, Service Fabric поддержка Кэйваултреференце упрощает эту задачу. Ниже приведен пример `DBPassword` , считывающий секрет из keyvault с помощью поддержки Service Fabric кэйваултреференце.

- Добавление раздела в settings.xml

    Определение `DBPassword` параметра с типом `KeyVaultReference` и значением`<KeyVaultURL>`
...
- Использование Кэйваултреференце в приложении
    ...
    Service Fabric при создании экземпляра службы будет разрешать параметр Кэйваултреференце с помощью управляемого удостоверения приложения. Каждый параметр, указанный в разделе, `<Section  Name=dbsecrets>` будет представлять собой файл в папке, на которую указывает EnvironmentVariable секретпас. Ниже приведен фрагмент кода C#, демонстрирующий чтение Дбпассворд в приложении.
    ...
    ```
    > [!NOTE] 
    > Для сценария контейнера точку подключения можно использовать для управления тем, где `secrets` будет подключено.
