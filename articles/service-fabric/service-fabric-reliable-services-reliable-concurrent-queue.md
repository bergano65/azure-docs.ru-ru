...
description: ReliableConcurrentQueue представляет собой очередь высокой пропускной стол, которая позволяет параллельные enqueues и dequeues.
ms.topic: conceptual
ms.date: 5/1/2017
...
* Дорогая обработка на разбивных элементах не должна выполняться во время действия транзакции, чтобы избежать длительных транзакций, которые могут оказать влияние на производительность системы.
...
Этот шаблон программирования подразумевает, что задача получателя выполняет вывод из очереди в пакетном режиме, вместо того чтобы выполнять один вывод за раз. Пользователь может регулировать задержки между пакетами и размеры пакетов. В следующем фрагменте кода показана эта модель программирования. Имейте в виду, что в этом примере обработка выполняется после транзакции, поэтому, если во время обработки произошла неисправность, необработанные элементы будут потеряны без обработки.  Кроме того, обработка может быть выполнена в рамках транзакции, однако она может негативно сказаться на производительности и требует обработки уже обработанных элементов.
...
* [Надежное резервное копирование и восстановление служб (восстановление в случае стихийных бедствий)](service-fabric-reliable-services-backup-restore.md)
* [Надежная конфигурация государственного менеджера](service-fabric-reliable-services-configuration.md)
* [Начало работы с сервисными службами Web API](service-fabric-reliable-services-communication-webapi.md)
...
* [Ссылка на разработчика для надежных коллекций](https://msdn.microsoft.com/library/azure/microsoft.servicefabric.data.collections.aspx)
