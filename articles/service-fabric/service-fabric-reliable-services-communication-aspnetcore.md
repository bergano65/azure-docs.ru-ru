...
`ICommunicationListener` Реализации KESTREL и HTTP. sys в пакетах `Microsoft.ServiceFabric.AspNetCore.*` NuGet имеют похожие шаблоны использования. Но они выполняют несколько различных действий, характерных для каждого веб-сервера. 
...
Оба прослушивателя связи предоставляют конструктор, который принимает следующие аргументы:
 ...
 - **`string endpointName`**: Это имя `Endpoint` конфигурации в ServiceManifest. XML. В основном это место, где два прослушивателя связи различаются. Для HTTP. *requires* sys требуется `Endpoint` конфигурация, а Kestrel — нет.
 - **`Func<string, AspNetCoreCommunicationListener, IWebHost> build`**: Это лямбда-выражение, которое реализуется, в котором вы создаете и `IWebHost`возвращаете. Он позволяет настроить `IWebHost` способ, который обычно будет в ASP.NET Core приложении. Лямбда-выражение предоставляет URL-адрес, который создается автоматически в зависимости от используемых параметров интеграции Service Fabric и предоставленной `Endpoint` конфигурации. Затем этот URL-адрес можно изменить или использовать для запуска веб-сервера.

## <a name="service-fabric-integration-middleware"></a>ПО промежуточного уровня для интеграции Service Fabric
Пакет `Microsoft.ServiceFabric.AspNetCore` NuGet включает метод `UseServiceFabricIntegration` расширения для, который `IWebHostBuilder` добавляет по промежуточного слоя с учетом Service Fabric. Это по промежуточного слоя настраивает Kestrel или HTTP `ICommunicationListener` . sys для регистрации уникального URL-адреса службы с Service Fabric служба именования. Затем он проверяет клиентские запросы, чтобы обеспечить подключение клиентов к правильной службе. 
...
Вы можете использовать HTTP. sys в Reliable Services, импортировав пакет NuGet **Microsoft. ServiceFabric. AspNetCore. HttpSys** . Этот пакет содержит `HttpSysCommunicationListener`реализацию `ICommunicationListener`. `HttpSysCommunicationListener`позволяет создать ASP.NET Core веб-узел в надежной службе, используя HTTP. sys в качестве сервера.
...
### <a name="endpoint-configuration"></a>Конфигурация конечной точки
Для `Endpoint` использования Kestrel не требуется конфигурация. 
...
После импорта пакета `Microsoft.ServiceFabric.AspNetCore.Configuration` NuGet необходимо зарегистрировать источник конфигурации Service Fabric с помощью API конфигурации ASP.NET Core. Для этого нужно проверить расширения **аддсервицефабрикконфигуратион** в `Microsoft.ServiceFabric.AspNetCore.Configuration` пространстве имен `IConfigurationBuilder`.
...
Service Fabric поддерживает несколько пакетов конфигурации. По умолчанию имя пакета включается в ключ конфигурации. Но можно установить для `IncludePackageName` флага значение false, как показано ниже.
```csharp
public Startup()
{
    ICodePackageActivationContext activationContext = FabricRuntime.GetActivationContext();
