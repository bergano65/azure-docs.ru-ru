...
Оба прослушивателя связи предоставляют конструктор, который принимает следующие аргументы:
 ...
 - **`string endpointName`**: Это имя `Endpoint` конфигурации в ServiceManifest.xml. В основном это место, где два прослушивателя связи различаются. Для *requires* HTTP.sys требуется `Endpoint` Конфигурация, а Kestrel — нет.
...
`Endpoint`Для веб-серверов, использующих API HTTP-сервера Windows, требуется настройка, в том числе HTTP.sys. Веб-серверы, использующие API сервера HTTP Windows, должны сначала зарезервировать свой URL-адрес с помощью HTTP.sys (обычно это выполняется с помощью средства [netsh](https://msdn.microsoft.com/library/windows/desktop/cc307236(v=vs.85).aspx) ). 

Для этого действия требуются повышенные привилегии, которые у служб нет по умолчанию. Параметры "http" или "HTTPS" для `Protocol` свойства `Endpoint` конфигурации в ServiceManifest.xml используются специально, чтобы указать Service Fabric времени выполнения зарегистрировать URL-адрес с HTTP.sys от вашего имени. Для этого используется строгий префикс URL-адреса с [*подстановочными знаками*](https://msdn.microsoft.com/library/windows/desktop/aa364698(v=vs.85).aspx) .
...
| Конфигурация порта | статический | Хорошо известный статический порт необходимо настроить в конфигурации `Endpoints` файла ServiceManifest.xml, например 80 для HTTP и 443 для HTTPS. |
| ServiceFabricIntegrationOptions | Отсутствуют | Используйте `ServiceFabricIntegrationOptions.None` параметр при настройке по промежуточного слоя интеграции Service Fabric, чтобы служба не пыталась проверить входящие запросы на уникальный идентификатор. Внешние пользователи приложения не узнают уникальную идентификационную информацию, которую использует по промежуточного слоя. |
