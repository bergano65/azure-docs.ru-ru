...
description: Использование управляемых удостоверений в коде приложения Azure Service Fabric для доступа к службам Azure. Эта функция предоставляется в общедоступной предварительной версии.
ms.topic: article
ms.date: 10/09/2019
...
- формирует универсальный код ресурса (URI) путем сцепления управляемой конечной точки удостоверения (MSI_ENDPOINT Value) с версией API и ресурсом (аудиторией), требуемым для токена.
...
| `api-version` | Параметр строки запроса, указывающий версию API службы токенов управляемого удостоверения; в настоящее время единственным допустимым значением является `2019-07-01-preview`и может изменяться. |
| `resource` | Параметр строки запроса, указывающий URI идентификатора приложения целевого ресурса. Это будет отражено в утверждении `aud` (аудитории) выданного маркера. В этом примере запрашивается маркер для доступа к Azure Key Vault, для которого URI идентификатора приложения имеет значение https:\//keyvault.azure.com/. |
...
| `expires_on` | Метка времени истечения срока действия маркера доступа; представляется в виде количества секунд с "1970-01-01T0:0: 0Z UTC" и соответствует утверждению `exp` маркера. В этом случае срок действия маркера истекает в 2019-08-08T06:10:11 + 00:00 (в RFC 3339).|
| `resource` | Ресурс, для которого был выдан маркер доступа, заданный с помощью параметра строки запроса `resource` запроса. соответствует утверждению "AUD" токена. |
