...
description: Узнайте, как отладить контейнеры Windows в Azure Service Fabric с помощью Visual Studio 2019.
ms.topic: article
ms.date: 02/14/2019
ms.author: mikhegn
...
---
# <a name="how-to-debug-windows-containers-in-azure-service-fabric-using-visual-studio-2019"></a>Как: Отошибок windows контейнеров в Azure Service Fabric с помощью Visual Studio 2019

С Visual Studio 2019 вы можете отладить приложения .NET в контейнерах в качестве сервисов Service Fabric. В этой статье показано, как настроить среду, а затем выполнить отладку приложения .NET в контейнере, выполняемом в локальном кластере Service Fabric.
...
1. Для поддержки разрешения DNS между контейнерами необходимо настроить локальный кластер разработки с использованием имени машины. Также эти действия необходимо выполнить, если нужно обратиться к службам через обратный прокси-сервер.
...
* Использование локального хоста для ClusterF-DNorIP не будет поддерживать разрешение DNS в контейнерах.
    * Решение. Настройка локального кластера с помощью имени компьютера (см. выше)
* Запуск Windows10 в виртуальной машине не получит ответа DNS обратно в контейнер.
    * Решение. Отключить разгрузку контрольной суммы UDP для протокола IPv4 на сетевом адаптере виртуальной машины.
    * Запуск Windows10 ухудшит производительность сетей на машине.
    * https://github.com/Azure/service-fabric-issues/issues/1061
* Разрешивание служб в том же приложении с использованием имени службы DNS не работает на Windows10, если приложение было развернуто с помощью Docker Compose
...
* При использовании IP-адреса для ClusterFQDNorIP изменение основного IP-адреса на узле нарушит функциональность DNS.
    * Решение. Повторно создайте кластер, использующий новый основной IP-адрес узла, или используйте имя компьютера. Эта поломка по замыслу.
* Если кластер, с которого был создан, не разрешим в сети, DNS выйдет из строя.
    * Решение. Повторно создайте локальный кластер с помощью основного IP-адреса узла. Эта неудача по замыслу.
...
1. Нажмите **F5,** чтобы начать отладку приложения.

    Visual Studio поддерживает типы консоли и проектов ASP.NET для .NET и .NET Core.

## <a name="next-steps"></a>Дальнейшие действия
Чтобы узнать больше о возможностях Service Fabric и контейнеров, смотрите [обзор контейнеров Service Fabric.](service-fabric-containers-overview.md)
