...
Основные правила являются минимальными для безопасности управляемого кластера Azure Service Fabric. Не удалось открыть следующие порты или список разрешений. IP/URL не позволит правильно работать с кластером и может не поддерживаться. Если это правило задано, необходимо использовать [Автоматическое обновление образа ОС](../virtual-machine-scale-sets/virtual-machine-scale-sets-automatic-upgrade.md), иначе потребуется открыть дополнительные порты.

### <a name="inbound"></a>Входящие 
|Priority   |Имя               |Порт        |Протокол  |Источник             |Назначение       |Действие   
|---        |---                |---         |---       |---                |---               |---
