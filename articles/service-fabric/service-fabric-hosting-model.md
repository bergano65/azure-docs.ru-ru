...
- Реплики из разных *ServiceTypes* должны обмениваться некоторыми общими данными с высокой инициализацией или стоимостью памяти.
...
Модель размещения с монопольным процессом не согласуется с моделью приложения, которая имеет несколько типов *ServiceType* для каждого *ServicePackage*. Это связано с тем, что несколько *ServiceTypes* в *сервисных пакетах* предназначены для достижения более высокого совместного использования ресурсов между репликами и позволяют повысить плотность реплик за каждый процесс. Модель с монопольным процессом предназначена для достижения других целей.
