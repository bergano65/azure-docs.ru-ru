...
Прежде чем продолжить, убедитесь, что вы понимаете различные понятия и связи, описанные в разделе [моделирование приложения в Service Fabric][a1]. 
...
Предположим, что у нас есть кластер с тремя узлами, и мы создаем *приложение Application* **Fabric:/APP1** типа ' MyAppType '. Внутри этого приложения **fabric:/App1** мы создаем службу **fabric:/App1/ServiceA** типа MyServiceType. Служба имеет 2 раздела (например, **P1** и **P2**) с 3 репликами на каждый. На следующей схеме показано представление этого приложения, развернутого на узле.
...
Если вы используете Service Fabric версии 5,6 или более поздней, можно выбрать модель с монопольным процессом во время создания службы (с помощью [PowerShell][p1], [RESTful][r1]или [FabricClient][c1]). Укажите для параметра **ServicePackageActivationMode** значение ExclusiveProcess.
...
>
>- Чтобы просмотреть модель размещения службы, запросите [Описание службы][p2]и посмотрите на значение **ServicePackageActivationMode**.
>
>

## <a name="work-with-a-deployed-service-package"></a>Работа с развернутым пакетом службы
Активная копия *ServicePackage* на узле называется [развернутым пакетом службы][p3]. Если при создании служб используется модель с монопольным процессом, для заданного приложения можно развернуть несколько пакетов служб одного пакета *ServicePackage*. Во время выполнения операций, относящихся к развернутому пакету службы, необходимо предоставить значение **ServicePackageActivationId**, позволяющее идентифицировать конкретный развернутый пакет службы. Например, укажите идентификатор, если вы [сообщаете о работоспособности развернутого пакета службы][p4] или [перезапустите пакет кода развернутого пакета службы][p5].

**Значение servicepackageactivationid** развернутого пакета службы можно узнать, выполнив запрос к списку [развернутых пакетов служб][p3] на узле. При запросе к [развернутым типам служб][p6], [развернутым репликам][p7]и [развернутым пакетам кода][p8] на узле результат запроса также содержит **значение servicepackageactivationid** родительского развернутого пакета службы.
...
>
> - В модели размещения с монопольным процессом на заданном узле заданного приложения активируется одна или несколько копий *ServicePackage*. Каждая активация имеет *непустой* **значение servicepackageactivationid**, заданную при выполнении операций, относящихся к развернутому пакету службы. 
>
...
>
> - Не выполняйте один запрос и не кэшируйте значение **ServicePackageActivationId**. Оно создается динамически и может изменяться по различным причинам. Перед выполнением операции, требующей **значение servicepackageactivationid**, сначала необходимо запросить список [развернутых пакетов служб][p3] на узле. Затем используйте значение **ServicePackageActivationId** из результата запроса для выполнения исходной операции.
>
>

## <a name="guest-executable-and-container-applications"></a>Гостевые исполняемые приложения и приложения-контейнеры
Service Fabric обрабатывает [Гостевые исполняемые][a2] и [контейнерные][a3] приложения как службы без отслеживания состояния, которые являются автономными. Для них в процессе *ServiceHost* (в процессе или контейнере) нет среды выполнения Service Fabric. Так как эти службы являются самодостаточными, для этих служб число реплик на *ServiceHost* неприменимо. Наиболее распространенной конфигурацией, используемой для этих служб, является один раздел с [InstanceCount][c2] , равным – 1 (одна копия кода службы, выполняющаяся на каждом узле кластера). 
