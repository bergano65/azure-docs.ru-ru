---
title: Развертывание и обновление с помощью менеджера ресурсов Azure
...
4. В шаблоне Resource Manager под объявлением кластера опишите свойства каждого приложения. Эти свойства включают в себя количество реплик или экземпляров, а также все цепочки зависимостей между ресурсами (другие приложения или службы). Список комплексных свойств можно узнать на листе [REST API Swagger Spec.](https://aka.ms/sfrpswaggerspec) Обратите внимание, что это не заменяет манифесты приложения или службы, а описывает некоторые из того, что содержится в них как часть шаблона менеджера ресурсов кластера. Ниже приведен пример шаблона, который включает в себя развертывание службы без отслеживания состояния *Service1* и службы с отслеживанием состояния *Service2* в качестве части приложения *Application1*.
...
## <a name="remove-service-fabric-resource-provider-application-resource"></a>Удалить ресурс поставщика ресурсов service Fabric
Следующее спровоцирует пакет приложения, который будет неподготовлен из кластера, и это позволит очистить используемое дисковое пространство:
```powershell
...
```
Простое удаление Microsoft.ServiceFabric/clusters/application из шаблона ARM не приведет к непредоставлению приложения

>[!NOTE]
> После завершения удаления вы больше не должны видеть версию пакета в SFX или ARM. Нельзя удалять ресурс версии типа приложения, с которым работает приложение; ARM/SFRP предотвратит это. Если вы попытаетесь отменить запуск пакета, время выполнения SF предотвратит это.
