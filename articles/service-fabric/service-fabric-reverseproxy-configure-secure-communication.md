---
title: Azure Service Fabric обратная прокси-безопасность связи
description: Настройте обратный прокси для обеспечения безопасной сквозной связи в приложении Azure Service Fabric.
author: kavyako
ms.topic: conceptual
ms.date: 08/10/2017
ms.author: kavyako
...
- **ServiceCommonNameAndIssuer**: обратный прокси-сервер проверяет сертификат, представленный службой, на основании общего имени сертификата и отпечатка непосредственного издателя. Задайте для свойства **ApplicationCertificateValidationPolicy** значение **ServiceCommonNameAndIssuer** в разделе [**ApplicationGateway/Http**](./service-fabric-cluster-fabric-settings.md#applicationgatewayhttp).
   ...
   Если обратный прокси-сервер endpoint подключается к представлению сертификата, общее имя и отпечаток пальца эмитента совпадает с любыми значениями, указанными здесь, устанавливается канал TLS.
...
- **ServiceCertificateThumbprints**: обратный прокси-сервер будет проверять сертификат службы на основе отпечатка. Вы можете выбрать этот вариант, если для служб настроены самозаверяющие сертификаты: задайте для свойства **ApplicationCertificateValidationPolicy** значение **ServiceCertificateThumbprints** в разделе [**ApplicationGateway/Http**](./service-fabric-cluster-fabric-settings.md#applicationgatewayhttp).
   ...
   Если отпечаток пальца сертификата сервера указан в этой конфигурации, обратный прокси сменяет сяртрию соединения TLS. В противном случае — он завершает подключение и не выполняет запроса клиента с ошибкой 502 (неверный шлюз). В строке состояния HTTP также будет содержаться фраза "Недопустимый сертификат SSL".
...
Прекращение TLS происходит на обратном прокси, и все данные сертификата клиента потеряны. Чтобы службы могли выполнять аутентификацию на основе сертификата клиента, задайте параметр **ForwardClientCertificate** в разделе [**ApplicationGateway/Http**](./service-fabric-cluster-fabric-settings.md#applicationgatewayhttp).

1. Когда **ForwardClientCertificate** установлен **на ложное,** обратный прокси не будет запрашивать сертификат клиента во время рукопожатия TLS с клиентом.
Это поведение установлено по умолчанию.

2. Когда **ForwardClientCertificate** установлен на **истину,** обратный прокси запрашивает сертификат клиента во время его рукопожатия TLS с клиентом.
...
* Ссылка на [настройку обратного прокси для подключения к защищенным службам](https://github.com/Azure-Samples/service-fabric-cluster-templates/tree/master/Reverse-Proxy-Sample#configure-reverse-proxy-to-connect-to-secure-services)
