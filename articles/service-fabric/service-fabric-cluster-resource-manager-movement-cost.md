---
title: 'Менеджер кластерных ресурсов service Fabric: Стоимость движения'
description: Узнайте о стоимости перемещения услуг Service Fabric и о том, как она может быть указана в соответствии с любыми архитектурными потребностями, включая динамическую конфигурацию.
author: masnider
ms.topic: conceptual
ms.date: 08/18/2017
ms.author: masnider
...
MoveCost имеет пять уровней: нулевой, низкий, средний, высокий и VeryHigh. Применяются следующие правила.

* MoveCosts относительно друг друга, за исключением нулевой и VeryHigh. 
* Стоимость перемещения Zero означает, что перемещение является бесплатным и не должно учитываться при оценке решения.
* Установка стоимости перемещения в High или VeryHigh *не* гарантирует, что реплика *никогда не* будет перемещена.
* Реплики с veryHigh стоимость перемещения будут перемещены только в том случае, если есть нарушение ограничения в кластере, которые не могут быть исправлены каким-либо другим способом (даже если это требует перемещения многих других реплик, чтобы исправить нарушение)
...
> [!IMPORTANT]
> Использование стоимости перемещения VeryHigh следует тщательно рассмотреть, поскольку это существенно ограничивает способность менеджера кластерных ресурсов находить глобально оптимальное решение для размещения в кластере. Реплики с veryHigh стоимость перемещения будут перемещены только в том случае, если есть нарушение ограничения в кластере, которые не могут быть исправлены каким-либо другим способом (даже если это требует перемещения многих других реплик, чтобы исправить нарушение)
