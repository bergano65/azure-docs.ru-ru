...
Service Fabric Explorer также размещается в конечной точке управления HTTP кластера Service Fabric. Чтобы запустить SFX в веб-браузере, просмотрите конечную точку управления http\/кластера с любого браузера - например, https: /clusterF-DN:19080.
...
Если вы попытаетесь подключиться к безопасному кластеру, то в зависимости от конфигурации кластера вам потребуется предъявить сертификат клиента или войти в систему использования AAD.
...
Например, чтобы удалить экземпляр приложения, выберите приложение из дерева слева, а затем выберите**Приложение «Удалить** **действия».** > 
...
## <a name="image-store-viewer"></a>Зритель магазина изображений
Зритель изображения — это функция, предлагаемая при использовании Native Image Store, которая позволяет просматривать текущее содержимое хранилища изображений и получать информацию о файлах и папках, а также удалять файлы/папки.

![Схема кластера в обозревателе Service Fabric][sfx-imagestore]

## <a name="backup-and-restore"></a>Резервное копирование и восстановление
Сервис Fabric Explorer предлагает возможность взаимодействия с [резервного копирования и восстановления](./service-fabric-reliable-services-backup-restore.md). Для просмотра функций резервного копирования и восстановления в SFX необходимо ввести расширенный режим.

![Включить расширенный режим][0]
 
Возможны следующие операции:

* Создайте, отодвило и удалите политику резервного копирования.
* Включить и отключить резервное копирование для приложения, службы или раздела.
* Приостановить и возобновить резервное копирование для приложения, службы или раздела.
* Триггер и отслеживание резервного копирования раздела.
* Триггер и трек Восстановление для раздела.

Для получения дополнительной информации об услуге резервного копирования и восстановления [см.](https://docs.microsoft.com/rest/api/servicefabric/sfclient-index-backuprestore)
