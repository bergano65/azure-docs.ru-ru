...
## <a name="remove-a-non-primary-node-type"></a>Удаление типа узла, не являющегося первичным
...
4. Отключите данные для типа узла.
...
6. Удаление данных для типа узла.
...
7. Удалите тип узла из раздела Service Fabric.
    ...
    - Для кластеров с высоким уровнем устойчивости обновите ресурс кластера в шаблоне и настройте политики исправности, чтобы игнорировать структуру:/работоспособность системного приложения, добавив `applicationDeltaHealthPolicies`, как указано ниже. Приведенная ниже политика должна пропускать существующие ошибки, но не разрешать новые ошибки работоспособности. 
    ...
    Параметр `upgradeReplicaSetCheckTimeout` указывает максимальное время, в течение которого Service Fabric ожидает, пока Секция находится в надежном состоянии, если оно еще не находится в надежном состоянии. После проверки безопасности для всех секций на узле Service Fabric переходит к обновлению на этом узле.
