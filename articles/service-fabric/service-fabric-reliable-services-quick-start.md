---
title: 'Создайте свое первое приложение Service Fabric в C #'
...
Запуск Visual Studio 2017 или Visual Studio 2019 в качестве администратора, а также создать новый проект приложения Service Fabric под названием *HelloWorld*:
...
Затем создайте проект обслуживания без состояния с использованием **.NET Core 2.0** под названием *HelloWorldStateless:*
...
> [!NOTE]
> Подробную информацию о том, как работать со стеком связи, можно узнать в [службе связи с ASP.NET Core](service-fabric-reliable-services-communication-aspnetcore.md)
...
Выберите **.NET Core 2.0 -> Государственная служба** и назовите его *HelloWorldStateful*. Нажмите кнопку **ОК**.
...
Служба с отслеживанием состояния имеет такие же точки входа, как и служба без отслеживания состояния. Основным отличием является наличие *государственного поставщика,* который может надежно хранить состояние. Service Fabric поставляется с реализацией государственного поставщика под названием [Reliable Collections,](service-fabric-reliable-services-reliable-collections.md)которая позволяет создавать реплицированные структуры данных через Reliable State Manager. Служба Reliable Service с отслеживанием состояния использует этот поставщик состояний по умолчанию.
...
* Когда вы зафиксируете транзакции в Reliable Collections, объекты реплицируются и становятся высокодоступными. Объекты, сохраненные в Reliable Collections, хранятся в локальной памяти вашей службы. Это означает, что у вас есть локальная ссылка на объект.
   ...
   Очень важно не изменять локальные экземпляры этих объектов без обновления коллекции в транзакции, так как эти изменения не реплицируются автоматически. Необходимо повторно вставить объект обратно в словарь или использовать один из методов *обновления* в словаре.
...
Надежные коллекции имеют многие `System.Collections.Generic` из `System.Collections.Concurrent` тех же операций, что и их и аналоги, за исключением языкового интегрированного запроса (LINЗ). Операции в надежных коллекциях являются асинхронными. Это вызвано тем, что операции записи с Reliable Collections выполняют операции ввода-вывода для репликации и сохранения данных на диске.
