---
title: Масштабирование кластеров Azure Service Fabric
description: Сведения о масштабировании кластеров Azure Service Fabric. По мере изменения требований приложения могут меняться и кластеры Service Fabric.
ms.topic: conceptual
ms.date: 11/13/2018
ms.author: atsenthi
...
- Ручное масштабирование требует вху в систему и явно госпросил операции масштабирования. Если операции масштабирования нужно выполнять часто или в непредвиденные моменты, этот подход может оказаться не лучшим решением.
...
Один из способов реализации этой "самодельной" функции автомасштабирования заключается в добавлении новой службы без отслеживания состояния в приложение Service Fabric для управления масштабированием. Создание собственной службы масштабирования обеспечивает максимальный уровень контроля и настройки через поведение масштабирования приложения. Это может быть полезно для сценариев, требующих точного контроля над тем, когда и как приложение масштабируется или выходит. Тем не менее, этот элемент управления сопровождается компромиссом сложности кода. Для использования такого подхода нужен собственный нестандартный код масштабирования. В рамках метода `RunAsync` службы набор триггеров может определить, требуется ли масштабирование (включая проверку таких параметров, как максимальный размер кластера и время ожидания следующей операции масштабирования).   

API, используемый для взаимодействия наборов виртуальной шкалы машин (как для проверки текущего числа экземпляров виртуальной машины, так и для его изменения) — это [свободно используемая библиотека Azure Management Compute.](https://www.nuget.org/packages/Microsoft.Azure.Management.Compute.Fluent/) Свободная библиотека вычислений предоставляет простой в использовании API-интерфейс для взаимодействия с масштабируемыми наборами виртуальных машин.  Для взаимодействия с кластером Service Fabric используется класс [System.Fabric.FabricClient](/dotnet/api/system.fabric.fabricclient).
