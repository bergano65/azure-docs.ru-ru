...
Отчеты о работоспособности отправляются в диспетчер работоспособности через клиент работоспособности, которая находится в клиенте fabric. Диспетчер работоспособности сохраняет отчеты в хранилище данных о работоспособности. Клиент работоспособности можно настроить с помощью приведенных ниже параметров.

* **HealthReportSendInterval**. Задержка между временем будет добавлен отчет для клиента и время он отправляется диспетчеру работоспособности. Она используется для отправки пакетных отчетов одним сообщением вместо отдельных сообщений для каждого отчета. Пакетная обработка повышает производительность. Значение по умолчанию: 30 секунд.
* **HealthReportRetrySendInterval**. Интервал, по которому клиент работоспособности повторно отправляет накопленные работоспособности сообщает диспетчеру работоспособности. Значение по умолчанию: 30 секунд, минимальное: 1 секунда.
* **HealthOperationTimeout**. Время ожидания отчета сообщения, отправленного диспетчеру работоспособности. После истечения времени, клиент работоспособности повторять попытки, пока диспетчер работоспособности подтверждает, что отчет был обработан. Значение по умолчанию — две минуты.

> [!NOTE]
> Чтобы отправить отчеты, объединенные в пакеты, клиент Fabric должен работать как минимум на протяжении времени, указанного в HealthReportSendInterval. Если сообщение теряется или диспетчер работоспособности не удалось применить его из-за временных ошибок, клиент fabric должен поддерживаться больше ему, повторная попытка.
> 
> 

Буферизация на клиенте выполняется с учетом уникальности отчетов. Например, если какой-либо неисправный информатор отправляет 100 отчетов в секунду для одного свойства одной сущности, будет использоваться только последняя версия отчета. В очереди клиента существует только один такой отчет. Если настроена пакетная обработка, число отчетов, отправляемых в диспетчер работоспособности — только по одному на интервал отправки. Используется последний добавленный отчет, который отображает актуальное состояние сущности.
...
[Общие сведения о работоспособности Service Fabric мониторинг](service-fabric-health-introduction.md)

[Просмотр отчетов о работоспособности Service Fabric](service-fabric-view-entities-aggregated-health.md)

[Как отчеты и проверка работоспособности службы](service-fabric-diagnostics-how-to-report-and-check-service-health.md)

[Используйте отчеты о работоспособности системы для устранения неполадок](service-fabric-understand-and-troubleshoot-with-system-health-reports.md)
