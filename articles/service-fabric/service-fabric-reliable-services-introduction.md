---
title: Обзор модели программирования Надежной службы
...
Azure Service Fabric упрощает написание и управление службами без гражданства и состояния. В этой статье рассматриваются следующие вопросы.
...
*Надежные услуги* — одна из моделей программирования, доступных на Service Fabric. Другой является модель программирования *надежного актера,* которая обеспечивает виртуальную платформу приложения [актера](https://research.microsoft.com/en-us/projects/orleans/) поверх модели Надежных Услуг. Дополнительные сведения о Reliable Actors см. в статье [Общие сведения о надежных субъектах Service Fabric](service-fabric-reliable-actors-introduction.md).
...
## <a name="what-are-reliable-services"></a>Что такое надежные услуги
...
* Доступ к AIS Service Fabric. В отличие от сервисных сервисов, смоделированных как [Гостевые Исполнители,](service-fabric-guest-executables-introduction.md)надежные службы могут использовать AA-аДИ Service Fabric напрямую. Это дает службам такие возможности:
  * Запрос к системе
  * Сообщить о состоянии здоровья в кластере
  * Получение уведомлений об изменениях конфигурации и кода
  * Найти и общаться с другими службами,
  * Использование [надежных коллекций](service-fabric-reliable-services-reliable-collections.md)
  * Получите доступ ко многим другим возможностям, все из первоклассной модели программирования на нескольких языках программирования.
* Простая модель для запуска собственного кода, который похож на другие знакомые модели программирования. у создаваемого кода четко определенная точка входа и простой в управлении жизненный цикл.
...
## <a name="what-makes-reliable-services-different"></a>Чем отличаются надежные услуги

Надежные услуги отличаются от услуг, которые вы, возможно, писали ранее, потому что Service Fabric предоставляет:
...
* **Масштабируемость** — службы отвязываются от конкретного оборудования и могут увеличиваться или уменьшаться в масштабе (насколько нужно) путем добавления или удаления оборудования или других ресурсов. Службы легко секционируются (особенно в случае с отслеживанием состояния). Это дает возможность масштабировать службу и обрабатывать частичные сбои. Службы могут создаваться и удаляться динамически с помощью кода, что позволяет создавать больше экземпляров по мере необходимости, например, в ответ на запросы клиентов. И наконец, платформа Service Fabric способствует тому, что службы имеют небольшой размер. Service Fabric позволяет подготавливать тысячи служб в рамках одного процесса. То есть нет необходимости выделять целые экземпляры ОС или процессы для одного экземпляра службы.
* **Последовательность** - Любая информация, хранящаяся в надежной службе, может быть гарантирована согласованной. Это верно даже в нескольких надежных коллекциях в службе. Изменения в нескольких коллекциях в пределах службы можно выполнять путем атомарных транзакций.

## <a name="service-lifecycle"></a>Жизненный цикл службы

Независимо от того, какую службу вы используете (с отслеживанием или без отслеживания состояния), надежные службы обеспечивают простой жизненный цикл, позволяющий быстро подключить код и приступить к работе.  Запуск новой службы требует реализации двух методов:
...
Если вы впервые узнали о Reliable Services, читайте статью дальше. Если вы ищете подробный пошаговой части жизненного цикла надежных услуг, ознакомьтесь [с обзором жизненного цикла Надежных Служб.](service-fabric-reliable-services-lifecycle.md)

## <a name="example-services"></a>Примеры служб

Давайте подробнее рассмотрим, как работает модель Надежных Служб как с службами без состояния, так и с службами состояния.

### <a name="stateless-reliable-services"></a>Надежные службы без отслеживания состояния

*Служба без гражданства* — это служба без гражданства, в которой не поддерживается состояние в службе между вызовами. Любое имеющееся состояние является одноразовым и не требует синхронизации, репликации, постоянного сохранения или высокой доступности.
...
Допустим, нам нужно создать службу, которая обрабатывает изображения. Для этого служба принимает изображение и ряд преобразований, которые необходимо с ним выполнить. Эта служба возвращает прослушиватель связи (допустим, веб-API), который предоставляет такой API, как `ConvertImage(Image i, IList<Conversion> conversions)`. Когда он получает запрос, служба хранит `IReliableQueue`его в, и возвращает некоторые идентификатора клиенту, чтобы он мог отслеживать запрос.
...
Хотя эта услуга звучит как типичная услуга .NET, разница`IReliableQueue` `IReliableDictionary`в том, что используемые структуры данных (и) предоставляются Service Fabric и являются очень надежными, доступными и последовательными.

## <a name="when-to-use-reliable-services-apis"></a>Когда следует использовать API надежных служб

Рассмотрим ApIs надежных служб, если:

* Вы хотите, чтобы код вашей службы (и, по желанию, состояние) был очень доступным и надежным.
