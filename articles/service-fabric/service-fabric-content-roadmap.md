...
Тип приложения — имя и версия, назначенные коллекции типов служб. Это определяется в файле ApplicationManifest.xml.

![Типы служб и типы приложений Service Fabric][cluster-imagestore-apptypes]

Пакет приложения — каталог на диске, содержащий файл ApplicationManifest.xml типа приложения, который ссылается на пакеты службы для каждого типа службы, которые составляют тип приложения. Например, пакет почтового приложения может содержать ссылки на пакет службы очередей, пакет службы внешнего интерфейса и пакет службы базы данных.  

Файлы в каталоге пакета приложения копируются в хранилище образов кластера Service Fabric. Вы сможете создать именованное приложение этого типа для запуска в кластере. Создав именованное приложение, можно создать именованной службы из одного из типов служб соответствующего типа приложения. 
...
Ключевое отличие с Service Fabric — активная ориентация на создание служб с отслеживанием состояния, с помощью [встроенных моделей программирования](service-fabric-choose-framework.md) или контейнерных служб с отслеживанием состояния. В [сценариях приложений](service-fabric-application-scenarios.md) описаны ситуации, в которых используются службы с отслеживанием состояния.
