...
Пакет приложения — это каталог на диске, содержащий файл ApplicationManifest. XML типа приложения, который ссылается на пакеты служб для каждого типа службы, составляющего тип приложения. Например, пакет почтового приложения может содержать ссылки на пакет службы очередей, пакет службы внешнего интерфейса и пакет службы базы данных.  
