---
title: Дополнительные сведения об Azure Service Fabric
...
### <a name="design-time-service-type-service-package-and-manifest-application-type-application-package-and-manifest"></a>Время разработки: тип службы, пакет службы и манифест, тип приложения, пакет приложения и манифест
Тип службы — имя и версия, назначенные пакетам кода, пакетам данных и пакетам конфигурации службы. Он определен в файле ServiceManifest.xml. Тип службы состоит из исполняемого кода и параметров конфигурации службы, которые загружаются во время выполнения, и статических данных, используемых службой.
...
Тип приложения — имя и версия, назначенные коллекции типов служб. Он определен в файле ApplicationManifest.xml.

![Типы служб и типы приложений Service Fabric][cluster-imagestore-apptypes]

Пакет приложения — это каталог на диске, который содержит файл ApplicationManifest.xml типа приложения, который ссылается на пакеты служб для каждого типа службы, составляющего тип приложения. Например, пакет почтового приложения может содержать ссылки на пакет службы очередей, пакет службы внешнего интерфейса и пакет службы базы данных.  

Файлы в каталоге пакета приложения копируются в хранилище образов кластера Service Fabric. Вы сможете создать именованное приложение этого типа для запуска в кластере. После создания именованного приложения можно создать именованную службу из одного из типов служб типа приложения. 
...
Ключевое отличие Service Fabric — это надежный фокус на создании служб с отслеживанием состояния с помощью [встроенных моделей программирования](service-fabric-choose-framework.md) или контейнерных служб с отслеживанием состояния. В [сценариях приложений](service-fabric-application-scenarios.md) описаны ситуации, в которых используются службы с отслеживанием состояния.
...
Если вы добавите новые узлы в кластер, реплики секции подвергнутся повторной балансировке Service Fabric с учетом этих узлов. Общая производительность приложения улучшится, а конфликт доступа к памяти уменьшится. При неэффективном использовании узлов в кластере вы можете уменьшить их количество. Service Fabric снова перераспределит реплики и экземпляры секции по меньшему количеству узлов, чтобы эффективно использовать оборудование на каждом узле. Кластеры в Azure можно масштабировать [вручную](service-fabric-cluster-scale-in-out.md) или [программным способом](service-fabric-cluster-programmatic-scaling.md). Автономные кластеры можно масштабировать [вручную](service-fabric-cluster-windows-server-add-remove-nodes.md).
...
Автономный кластер является ресурсом, который полностью принадлежит вам. Вы отвечаете за исправления базовой операционной системы и запуск обновлений Service Fabric. Если кластер может подключиться к [https://www.microsoft.com/download](https://www.microsoft.com/download) , можно настроить кластер для автоматической загрузки и подготовки нового пакета среды выполнения Service Fabric. Затем можно инициировать само обновление. Если кластер не может получить доступ [https://www.microsoft.com/download](https://www.microsoft.com/download) , можно вручную скачать новый пакет среды выполнения с подключенного к Интернету компьютера, а затем запустить обновление. Дополнительные сведения см. в статье [Обновление автономного кластера Azure Service Fabric в Windows Server](service-fabric-cluster-upgrade-windows-server.md).
...
* запросы о работоспособности (с помощью [PowerShell](/powershell/module/servicefabric/?view=azureservicefabricps), [CLI](service-fabric-sfctl.md), [API C# FabricClient](/dotnet/api/system.fabric.fabricclient.healthclient) и [API Java FabricClient](/java/api/system.fabric) или [REST APIs](/rest/api/servicefabric));
...
* Просмотрите [примеры кода для Service Fabric](/samples/browse/?products=azure).
* Узнайте о [вариантах поддержки Service Fabric](service-fabric-support.md).
* Просматривайте статьи и обновления в [блоге группы разработчиков](https://techcommunity.microsoft.com/t5/azure-service-fabric/bg-p/Service-Fabric).
