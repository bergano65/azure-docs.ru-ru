---
title: Узнайте больше о структуре службы Azure
...
### <a name="design-time-service-type-service-package-and-manifest-application-type-application-package-and-manifest"></a>Время проектирования: тип обслуживания, пакет услуг и манифест, тип приложения, пакет приложений и манифест
Тип службы — имя и версия, назначенные пакетам кода, пакетам данных и пакетам конфигурации службы. Это определено в файле ServiceManifest.xml. Тип службы состоит из исполняемых параметров конфигурации кода и службы, которые загружаются во время выполнения, и статических данных, потребляемых службой.
...
Тип приложения — имя и версия, назначенные коллекции типов служб. Это определено в файле ApplicationManifest.xml.

![Типы служб и типы приложений Service Fabric][cluster-imagestore-apptypes]

Пакет приложений представляет собой каталог дисков, содержащий файл ApplicationManifest.xml типа приложения, который ссылается на пакеты услуг для каждого типа службы, который составляет тип приложения. Например, пакет почтового приложения может содержать ссылки на пакет службы очередей, пакет службы внешнего интерфейса и пакет службы базы данных.  

Файлы в каталоге пакета приложения копируются в хранилище образов кластера Service Fabric. Вы сможете создать именованное приложение этого типа для запуска в кластере. После создания названного приложения можно создать именованный сервис из одного из типов службы приложения. 
...
[Кластер Service Fabric](service-fabric-deploy-anywhere.md) — это набор виртуальных или физических машин, в которые развертываются и управляются ваши микрослужбы. Кластеры можно масштабировать до нескольких тысяч машин.
...
[Раздел](service-fabric-concepts-partitioning.md) не является уникальным для Service Fabric. Распространенный пример секционирования — это секционирование данных или сегментирование. Службы с отслеживанием состояния с большим количеством состояний разделяют данные по секциям. Каждая секция отвечает за часть полного состояния службы. 
...
Ключевой дифференциацией с Service Fabric является ее сильное внимание на создании государственных услуг, либо с [встроенными в модели программирования](service-fabric-choose-framework.md) или с контейнерными государственными службами. В [сценариях приложений](service-fabric-application-scenarios.md) описаны ситуации, в которых используются службы с отслеживанием состояния.
...
[Надежные услуги](service-fabric-reliable-services-introduction.md) — это легкая платформа для написания сервисов, которые интегрируются с платформой Service Fabric и извлекают выгоду из полного набора функций платформы. Службы Reliable Services могут быть без отслеживания состояния, как и большинство платформ служб, таких как веб-серверы или рабочие роли в облачных службах Azure. При этом состояние сохраняется во внешнем решении, таком как база данных Azure или хранилище таблиц Azure. Службы Reliable Services могут также быть с отслеживанием состояния. При этом состояние сохраняется прямо в службу с использованием Reliable Collections. Состояние становится [высокодоступным](service-fabric-availability-services.md) за счет репликации и распределения путем [секционирования](service-fabric-concepts-partitioning.md), которыми автоматически управляет Service Fabric.

### <a name="reliable-actors"></a>Надежные субъекты
Созданная поверх надежных служб, платформа [Reliable Actor](service-fabric-reliable-actors-introduction.md) представляет собой платформу приложений, которая реализует шаблон Virtual Actor, основанный на шаблоне проектирования актера. Платформа надежных субъектов использует независимые единицы вычислений и состояний с однопоточным выполнением, которые называются субъектами. Платформа надежных субъектов обеспечивает встроенное взаимодействие для субъектов, а также предустановленное сохранение состояния и масштабируемые конфигурации.
...
Чтобы создать службы именно в масштабах облака, очень важно, чтобы приложения и службы были устойчивы к реальным сбоям. Служба анализа сбоев предназначена для проверки служб, созданных на платформе Service Fabric. С [помощью службы анализа неисправностей](service-fabric-testability-overview.md)вы можете вызвать значимые ошибки и запустить полные сценарии тестирования в отношении приложений. Вызываемые ошибки и сценарии позволяют воспроизвести и проверить в контролируемых, безопасных и согласованных условиях разные состояния и переходы, происходящие со службой в течение ее жизненного цикла.
...
[Сценарии](service-fabric-testability-scenarios.md) представляют сложные операции, состоящие из одного или нескольких действий. Служба анализа сбоев включает два полных встроенных сценария:
...
Автономный кластер является ресурсом, который полностью принадлежит вам. Вы отвечаете за исправления базовой операционной системы и запуск обновлений Service Fabric. Если кластер может [https://www.microsoft.com/download](https://www.microsoft.com/download)подключиться к вам, вы можете настроить кластер для автоматической загрузки и предоставления нового пакета времени выполнения Service Fabric. Затем можно инициировать само обновление. Если кластер не может [https://www.microsoft.com/download](https://www.microsoft.com/download)получить доступ, вы можете вручную загрузить новый пакет времени выполнения из подключенного к Интернету машины, а затем инициировать обновление. Дополнительные сведения см. в статье [Обновление автономного кластера Azure Service Fabric в Windows Server](service-fabric-cluster-upgrade-windows-server.md).
...
Готовые к использованию компоненты Service Fabric поддерживают создание отчетов о работоспособности по всем сущностям в кластере. [Отчеты о работоспособности системы](service-fabric-understand-and-troubleshoot-with-system-health-reports.md) обеспечивают видимость функциональности кластеров и приложений и помечать проблемы в области работы. Что касается приложений и служб, отчеты о работоспособности системы проверяют правильность реализации и поведения сущностей на уровне среды выполнения Service Fabric. Отчеты не обеспечивают никакого мониторинга работоспособности бизнес-логики службы или обнаружения процессов, которые перестали отвечать. Чтобы добавить сведения о работоспособности конкретно для логики вашей службы, ознакомьтесь со статьей о [добавлении настраиваемых отчетов о работоспособности Service Fabric](service-fabric-report-health.md).
...
* Просмотрите [примеры кода для Service Fabric](https://aka.ms/servicefabricsamples).
