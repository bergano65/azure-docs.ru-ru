---
title: Различия между облачными службами и тканью обслуживания
...
[Облачные сервисы аналогичны Сервисной ткани по степени контроля и простоте использования, но теперь это устаревший сервис, и сервисная ткань рекомендуется для новой разработки;](https://docs.microsoft.com/azure/app-service/overview-compare) ниже приводится сравнение API:


| **API облачного сервиса** | **Сервисная ткань API** | **Примечания** |
| --- | --- | --- |
| RoleInstance.GetID | FabricRuntime.GetNodeContext.NodeId или . NodeName | ID является свойством NodeName |
| RoleInstance.GetFaultDomain | FabricClient.queryManager.GetNodeList | Фильтр на NodeName и использовать FD свойство |
| RoleInstance.GetUpgradeDomain | FabricClient.queryManager.GetNodeList | Фильтр на NodeName и использование свойства обновления |
| RoleInstance.GetInstanceEndpoints | FabricRuntime.GetActivationКонтекст или именования (ResolveService) | CodePackageActivationContext, который предоставляется как FabricRuntime.GetActivationContext и в рамках реплик через ServiceInitializationParameters.CodePackageActivationContext, предоставленных во время . Инициализации |
| RoleEnvironment.GetRoles | FabricClient.queryManager.GetNodeList | Если вы хотите сделать тот же вид фильтрации по типу вы можете получить список типов узлов из кластера манифест через FabricClient.ClusterManager.GetClusterManifest и захватить роль / узел типов оттуда. |
| RoleEnvironment.GetisAvailable | Подключите-WindowsFabricCluster или создать FabricRuntime указал на определенный узла | * |
| RoleEnvironment.GetLocalResource | CodePackageActivationКонтекст.Log/Temp/Work | * |
| RoleEnvironment.GetCurrentRoleInstance | CodePackageActivationКонтекст.Log/Temp/Work | * |
| МестныйРесурс.GetRootPath | CodePackageActivationКонтекст.Log/Temp/Work | * |
| Role.GetInstances | FabricClient.queryManager.GetNodeList или ResolveService | * |
| RoleInstanceEndpoint.GetIPEndpoint | FabricRuntime.GetActivationКонтекст или именования (ResolveService) | * |
