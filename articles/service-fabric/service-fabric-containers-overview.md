---
title: Общие сведения о Service Fabric и контейнерах
...
[Контейнеризация существующего приложения .NET](service-fabric-host-app-in-a-container.md)  
...
В Windows Server 2016 и более поздних версиях предусмотрены два разных типа контейнеров, которые отличаются уровнем изоляции. Как и в контейнерах Docker, в контейнерах Windows Server предусмотрена изоляция пространства имен и файловой системы. Эти контейнеры совместно используют ядро и узел, на котором выполняются. В Linux эта изоляция обычно обеспечивается с помощью cgroups и пространств имен. У контейнеров Windows Server все аналогично.
...
Service Fabric поддерживает развертывание контейнеров DOCKER в Linux, а также контейнеры Windows Server в Windows Server 2016 и более поздней версии вместе с поддержкой режима изоляции Hyper-V.

> [!NOTE]
> Контейнеры не поддерживаются в локальных Service Fabric кластерах (без кластеров Linux в OneBox или кластерах Windows при локальных Service Fabricных установках).
...
Подробный обзор поддержки контейнера в Azure, например, сведения о создании кластера Kubernetes со Службой Azure Kubernetes, создании частного реестра Docker в службе "Реестр контейнеров Azure" и многом другом, см. в статье [Azure для контейнеров](../containers/index.yml).
...
[Дополнительные сведения о контейнерах Windows](/virtualization/windowscontainers/about/)
