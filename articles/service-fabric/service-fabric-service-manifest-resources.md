---
title: Уточнение конечных точек службы Service Fabric
...
>
> С выпуском 7.0 мы выпустим **Предупреждение о здоровье,** когда обнаружим использование диапазона портов приложений выходит за рамки HostingConfig::ApplicationPortExhaustThresholdPercentage (по умолчанию 80%).
>
...
Предположим, что вы задали неверное значение. Как и для Порта вы указали строку значение "Foo" вместо Int.  Команда New-ServiceFabricApplication выйдет из строя с ошибкой: параметр переопределения с именем 'ServiceEndpoint1' атрибут 'Port1' в разделе 'ResourceOverrides' недействителен. The value specified is 'Foo' and required is 'int' (Недопустимый параметр переопределения ServiceEndpoint1 атрибута Port1 в разделе ResourceOverrides. Указано значение Foo, а требуется целое число).
