...
Azure Service Fabric предоставляет функции для [периодического резервного копирования данных](service-fabric-backuprestoreservice-quickstart-azurecluster.md) и резервного копирования данных по мере необходимости. Резервное копирование по требованию полезно, поскольку оно _защищает_/от_повреждения данных_ из-за запланированных изменений в базовой службе или ее среде.
...
Убедитесь, что Расширенный режим включен в настройках Service Fabric Explorer.
1. Выберите нужные разделы и нажмите на Действия. 
2. Выберите резервное копирование раздела триггера и заполните информацию для Azure:

    ![Резервное копирование раздела триггера][0]

    или FileShare:

    ![Триггер раздела Резервного копирования FileShare][1]
...
- **Принято:** Резервное копирование началось на разделе и находится в процессе.
  ```
  ...
  FailureError            :
  ```
- **Успех,** **Неудача**, или **Тайм-аут**: Запрошенное резервное копирование по требованию может быть завершено в любом из следующих состояний:
  - **Состояние**резервного копирования успеха : _Показывает,_ что состояние раздела успешно резервное копирование. В ответе в этом случае возвращаются параметры _BackupEpoch_ и _BackupLSN_ для секции, а также время в формате UTC.
    ```
    BackupState             : Success
    ...
    FailureError            :
    ```
  - **Сбой**: Состояние резервного копирования _показывает,_ что сбой произошел во время резервного копирования состояния раздела. Причины сбоя указаны в ответе на запрос.
    ```
    BackupState             : Failure
    TimeStampUtc            : 0001-01-01T00:00:00Z
    BackupId                : 00000000-0000-0000-0000-000000000000
    ...
    ```
  - **Состояние**резервного копирования _Тайм-аута_ указывает на то, что резервное копирование состояния раздела не может быть создано за определенное время. Время ожидания по умолчанию составляет 10 минут. В этом случае инициируйте новый запрос на резервное копирование с увеличенным значением [BackupTimeout](https://docs.microsoft.com/rest/api/servicefabric/sfclient-api-backuppartition#backuptimeout).
    ```
    BackupState             : Timeout
    TimeStampUtc            : 0001-01-01T00:00:00Z
    BackupId                : 00000000-0000-0000-0000-000000000000
