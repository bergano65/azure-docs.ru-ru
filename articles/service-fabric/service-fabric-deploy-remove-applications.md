---
title: Развертывание Azure Service Fabric с PowerShell
description: Узнайте об удалении и развертывании приложений в функции службы Azure Fabric и о том, как выполнять эти действия в Powershell.
ms.topic: conceptual
ms.date: 01/19/2018
...
Если вы используете Visual Studio для развертывания и отладки приложений в локальном кластере разработки, все описанные выше действия выполняются автоматически с помощью сценария PowerShell.  Этот скрипт содержится в папке *Скриптов* проекта приложения. Эта статья содержит основные сведения о действиях, выполняемых этим сценарием. Они помогут вам выполнять те же операции вне Visual Studio. 
...
Выпуск: Загрузка пакет успешно завершена, но [регистрация-СервисFabricApplicationType](/powershell/module/servicefabric/register-servicefabricapplicationtype?view=azureservicefabricps) раз. Попробовать:
...
[Пакет приложения](service-fabric-package-apps.md)
