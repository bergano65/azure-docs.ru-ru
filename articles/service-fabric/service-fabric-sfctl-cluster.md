...
| выбрать | Выполняет подключение к конечной точке кластера Service Fabric. |
...
Следующие API-интерфейсы запуска ошибок операций, которые могут быть отменены с помощью CancelOperation\: StartDataLoss, StartQuorumLoss, StartPartitionRestart, использованием StartNodeTransition. Если для force задано значение false, то указанная операция, вызываемая пользователем, будет корректно остановлена и очищена.  Если для force задано значение true, команда будет прервана и внутреннее состояние может проигнорироваться.  Следует с осторожностью использовать true в качестве значения force. Если для force задано значение true, вызов этого API будет запрещен, пока этот API не будет вызван в той же тестовой команде, в которой для force задано значение false, или же если в команде параметр OperationState уже имеет значение OperationState.RollingBack. 
...
При подключении к защищенному кластеру укажите сертификат (CRT-файл) и файл ключа (KEY-файл) или отдельный файл, содержащий сертификат и ключ (PEM-файл). Не указывайте и то, и другое. При подключении к защищенному кластеру укажите также абсолютный путь к файлу пакета ЦС или каталогу сертификатов доверенного ЦС (при необходимости). При использовании каталога сертификатов ЦС значение `c_rehash <directory>`, предоставленное OpenSSL, следует выполнить первым, чтобы вычислить хэши сертификатов и создать соответствующие символьные ссылки.
