---
title: Azure Сервис Ткань CLI- кластер sfctl
description: Узнайте о sfctl, интерфейсе командной строки Azure Service Fabric. Включает список команд для управления кластерами.
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
|Аргумент|Описание|
| --- | --- |
| --state-filter | Используется для фильтрации по OperationState для операций, вызываемых пользователем. - 65535 - выберите Все - 1 - выберите Запуск - 2 - выберите RollingBack - 8 - выберите Завершено - 16 - выберите Неисправный - 32 - выберите Отменено - 64 - выберите ForceCancelled.  Значение по умолчанию\: 65535. |
| --timeout -t | Тайм-аут сервера для выполнения операции в считанные секунды. Этот тайм-аут определяет продолжительность времени, которую клиент готов ждать, пока запрошенная операция завершится. Значение по умолчанию для этого параметра составляет 60 секунд.  Значение по умолчанию\: 60. |
| --type-filter | Используется для фильтрации по OperationType для операций, вызываемых пользователем. - 65535 - выберите все - 1 - выберите РазделDataLoss. - 2 - выберите разделКурумЛоссс. - 4 - выберите РазделRestart. - 8 - выберите NodeTransition.  Значение по умолчанию\: 65535. |
...
При подключении к защищенному кластеру укажите сертификат (CRT-файл) и файл ключа (KEY-файл) или отдельный файл, содержащий сертификат и ключ (PEM-файл). Не указывайте и то, и другое. При подключении к защищенному кластеру укажите также абсолютный путь к файлу пакета ЦС или каталогу сертификатов доверенного ЦС (при необходимости).  Нет подключения к кластеру без запуска этой команды, включая подключение к локальному хосте. Однако для подключения к локальному кластеру не требуется явной точки.  При использовании самоподписанного сертификата или другого сертификата, не подписанного хорошо известным ЦС, пройдите в параметре --ca, чтобы гарантировать, что проверка проходит. Если не на производственном кластере, чтобы обойти проверку стороны клиента (полезно для самостоятельного подписанного или не очень известного CA подписанного), используйте опцию --no-verify. Хотя это возможно, это не рекомендуется для производственных кластеров. Ошибка проверки сертификата может привести к иным.
...
| --ca | Абсолютный путь к каталогу сертификатов ЦС, которые следует считать действительными, или путь к файлу пакета ЦС. При использовании каталога сертификатов ЦС значение `c_rehash <directory>`, предоставленное OpenSSL, следует выполнить первым, чтобы вычислить хэши сертификатов и создать соответствующие символьные ссылки. Это используется для проверки того, что сертификат, возвращенный кластером, является действительным. |
| --cert | Абсолютный путь к файлу сертификата клиента. |
| --конечная точка | URL-адрес конечной точки кластера, включая порт и префикс HTTP или HTTPS. Как правило, конечная точка будет\:выглядеть что-то \:вроде https //<ваш URL>19080 году. Если конечная точка не дается,\:она будет\:по умолчанию http //localhost 19080.  По\: \:умолчанию http\://localhost 19080. |
