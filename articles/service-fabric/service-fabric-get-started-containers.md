---
title: Создание контейнерного приложения Azure Service Fabric
description: Создание первого приложения-контейнера Windows в Azure Service Fabric. Создайте изображение Docker с помощью приложения Python, переместите изображение в реестр контейнеров, а затем создайте и разместите контейнер в Azure Service Fabric.
ms.topic: conceptual
ms.date: 01/25/2019
...
Чтобы запустить существующее приложение в контейнере Windows кластера Service Fabric, не требуется вносить изменения в приложение. В этой статье вы можете создать изображение Docker, содержащее веб-приложение Python [Flask,](http://flask.pocoo.org/) и развернуть его в кластер Azure Service Fabric. Кроме того, вы предоставите общий доступ к контейнерному приложению через [реестр контейнеров Azure](/azure/container-registry/). Для работы с этой статьей необходимо знание основных понятий Docker. Чтобы ознакомиться с Docker, см. этот [обзор Docker](https://docs.docker.com/engine/understanding-docker/).
...
* Компьютер для разработки, на котором установлено ПО, перечисленное ниже.
  ...
  * [Сервис Ткань SDK и инструменты](service-fabric-get-started.md).
  *  Docker для Windows. [Получить Докер CE для Windows (стабильный)](https://store.docker.com/editions/community/docker-ce-desktop-windows?tab=description). После установки и запуска Docker щелкните правой кнопкой мыши значок в области уведомлений и выберите **Switch to Windows containers** (Переключиться на контейнеры Windows). Это необходимо для запуска образов Docker на базе Windows.
...
* Если версия содержит *x.x.14323.x*, выберите для операционной системы значение *WindowsServer 2016-Datacenter-with-Containers* при [создании кластера](service-fabric-cluster-creation-via-portal.md).
  * Если версия содержит *x.x.16299.x*, выберите для операционной системы значение *WindowsServerSemiAnnual Datacenter-Core-1709-with-Containers* при [создании кластера](service-fabric-cluster-creation-via-portal.md).
...
Если эта команда ничего не возвращает, запустите следующую команду и проинспектируйте элемент **NetworkSettings**->**Networks** для IP-адреса:
```
docker inspect my-web-site
```

Подключитесь к запущенному контейнеру. Открыть веб-браузер, указывающий на ВОЗВРАЩЕНный IP-адрес, например "http:\//172.31.194.61". Вы должны увидеть заголовок Hello World! в браузере.
...
1. Запустите среду Visual Studio. Выберите **файл** > **Новый** > **проект**.
...
- Контейнеры Windows Server, построенные с помощью Windows Server 2016, работают в режиме изоляции Hyper-V только на хостах под управлением версии Windows Server 1709. 
