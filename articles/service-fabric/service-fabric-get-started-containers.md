...
Начиная с версии 6.1 Service Fabric автоматически интегрирует события [Docker HEALTHCHECK](https://docs.docker.com/engine/reference/builder/#healthcheck) в отчеты о работоспособности системы. Это означает, что если в вашем контейнере включена инструкция **HEALTHCHECK**, то Service Fabric будет отправлять отчет о работоспособности при каждом изменении состояния контейнера согласно сведениям Docker. В **Service Fabric Explorer** будет отображаться состояние работоспособности [ОК](service-fabric-visualizing-your-cluster.md), если значение *health_status* равно *healthy*. Если же значение **health_status** равно *unhealthy*, отобразится *предупреждение*. 
...
В поле **Конечная точка подключения** введите конечную точку управления для кластера. Пример: `containercluster.westus2.cloudapp.azure.com:19000`. Конечную точку подключения к клиенту можно найти на вкладке "Обзор" кластера на [портале Azure](https://portal.azure.com).
