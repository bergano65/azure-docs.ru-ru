...
* **Сохраняемое состояние**: Состояние сохраняется на диске и реплицируется на трех или более реплик. Это самый устойчивый вариант хранения состояния, при котором оно может сохраниться даже после полного сбоя кластера.
* **Непостоянное состояние**: Состояние реплицируется на трех или более реплик и хранится только в памяти. Этот вариант обеспечивает устойчивость в случае сбоя узла, сбоя субъекта и во время обновления и балансировки ресурсов. Однако состояние не сохраняется на диск, поэтому если все реплики будут потеряны одновременно, состояние также теряется.
* **Несохраняемое состояние**: Состояние не реплицируется или не записывается на диск, следует использовать только для субъектов, которым не требуется надежно хранить состояние.
