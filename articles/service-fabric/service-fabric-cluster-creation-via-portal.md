---
title: Создание кластера Service Fabric в Azure с помощью портала Azure
...
Если вы впервые создаете кластер Service Fabric или развертываете кластер для тестовых рабочих нагрузок, можно перейти к следующему разделу (**Создание кластера в портал Azure**) и создать в системе сертификаты, необходимые для кластеров, выполняющих тестовые рабочие нагрузки. Если вы настраиваете кластер для рабочих нагрузок, тогда продолжайте чтение.
...
* **Проверка подлинности кластера:** Проверяет связь между узлами для Федерации кластеров. Только узлы, которые могут подтвердить свою подлинность с помощью этого сертификата, могут присоединиться к кластеру.
* **Проверка подлинности сервера:** Выполняет проверку подлинности конечных точек управления кластера в клиенте управления, чтобы клиент управления знал о том, что он говорит о реальном кластере. Этот сертификат также предоставляет TLS для API управления HTTPS и для Service Fabric Explorer по протоколу HTTPS.
...
Сертификаты приложений не могут быть настроены при [создании кластера с помощью портал Azure](https://github.com/MicrosoftDocs/azure-docs/blob/master/articles/service-fabric/service-fabric-cluster-creation-via-portal.md). Чтобы настроить сертификаты приложения в процессе настройки кластера, необходимо [создать кластер с помощью Azure Resource Manager][create-cluster-arm]. Также можно добавить сертификаты приложения в кластер после его создания.

## <a name="create-cluster-in-the-azure-portal"></a>Создание кластера на портале Azure

Создание производственного кластера в соответствии с потребностями приложения необходимо планировать. Чтобы упростить этот процесс, настоятельно рекомендуем ознакомиться с [рекомендациями по планированию загрузки кластера Service Fabric][service-fabric-cluster-capacity]. 
...
Колонка **Создание кластера Service Fabric** содержит следующие четыре шага.
...
5. Выберите **начальную емкость масштабируемого набора виртуальных машин** для типа узла. Можно увеличить или уменьшить количество виртуальных машин в типе узла в дальнейшем, но на основном типе узла минимальное значение равно пяти для рабочих нагрузок. Для других типов узлов можно задать минимум одну виртуальную машину. Минимальное **количество** виртуальных машин для первичного узла зависит от выбранного для кластера уровня **надежности**.  
...
![На снимке экрана показана страница Безопасность с выделенным базовым ключом с областью хранилища ключей и областью создание хранилища ключей.][CreateKeyVault]

После создания хранилища ключей измените политики доступа для хранилища ключей. 

![На снимке экрана показана панель Создание кластера Service Fabric с выбранным параметром безопасность 3 и объясняется, что хранилище ключей не включено.][CreateKeyVault2]
...
![На снимке экрана показана панель "Создание кластера Service Fabric" с открытой областью безопасности и панелью политики доступа.][CreateKeyVault3]

Укажите имя сертификата и нажмите кнопку **ОК**.

![На снимке экрана показана панель "Создание кластера Service Fabric" с выбранным параметром "безопасность", но без объяснения того, что хранилище ключей не включено.][CreateKeyVault4]
...
![На снимке экрана показано диалоговое окно Security configure Security Settings (параметры безопасности кластера).][SecurityCustomOption]
...
2. Выберите вкладку "Свойства", скопируйте значение идентификатора ресурса и вставьте его в поле исходного хранилища ключей в другом окне браузера. 
    ...
    ![На снимке экрана показан окно свойств для хранилища ключей.][CertInfo0]
...
5. Щелкните идентификаторы GUID, которые показаны под текущей версией.
    ...
    ![Снимок экрана: окно сертификата для хранилища ключей][CertInfo1]
...
7. Скопируйте идентификатор секрета и вставьте его в поле URL-адреса сертификата в другом окне браузера.
    ...
    ![Снимок экрана показывает диалоговое окно версия сертификата с возможностью копирования идентификатора сертификата.][CertInfo2]
...
![На снимке экрана показана страница сводки "Создание кластера Service Fabric" со ссылкой для просмотра и скачивания сертификата.][Summary]
...
Чтобы выполнять операции управления в кластере с помощью PowerShell или CLI, необходимо подключиться к кластеру. Дополнительные сведения см. в статье [Подключение к кластеру](service-fabric-connect-to-secure-cluster.md).
