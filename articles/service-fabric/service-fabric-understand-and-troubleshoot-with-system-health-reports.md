...
В системных отчетах о работоспособности отражаются показатели функциональности кластеров и приложений, а также отмечаются проблемы. Что касается приложений и служб, отчеты о работоспособности системы проверяют правильность реализации и поведения сущностей на уровне платформы Service Fabric. Отчеты не обеспечивают Отслеживание работоспособности бизнес-логики службы или обнаружение процессов, которые не отвечают. Пользовательские службы могут расширить данные о работоспособности сведениями, характерными для их логики.
...
* **Свойство**: Начинается с **окружение** и включает сведения об узле.
* **Дальнейшие действия**: Проверьте, почему теряется. (например, проверьте связь между узлами кластера).
...
* **Ожидание трансляции**: FM и FMM ожидает ответа широковещательные сообщения от других узлов.

  * **Дальнейшие действия**: Проверьте, ли проблем с сетевым подключением между узлами.
* **Ожидание узлов**: Диспетчер отработки Отказов или мастер диспетчера отработки Отказов уже получили ответ трансляции от других узлов и ожидают ответа от указанных узлов. В отчете о работоспособности перечислены узлы, для которых диспетчер отработки отказов или мастер диспетчера отработки отказов ожидают ответ.
   * **Дальнейшие действия**: Проверьте сетевое подключение между диспетчером отработки Отказов или мастер диспетчера отработки Отказов и указанными узлами. Исследуйте каждый узел, указанный для других возможных проблем.

* **SourceID**: System.FM или System.FMM
* **Свойство**: Перестроение.
* **Дальнейшие действия**: Проверьте сетевое подключение между узлами, а также состояние какого-либо определенного узла, перечисленных в описании отчета о работоспособности.

### <a name="seed-node-status"></a>Начальное состояние узла
**System.FM** выдает предупреждение уровня кластера, если некоторые начальные узлы находятся в неработоспособном состоянии. Начальные узлы являются узлы, которые поддерживать доступность базового кластера. Эти узлы гарантируют работу кластера, арендуя другие узлы и выступая в роли прерывателя связи во время определенных сбоев сети. Если большинство начальных узлов не работают в кластере и они не возвращаются, кластер автоматически завершает работу. 

Начальный узел находится в неработоспособном состоянии, если его состояние узла не работает "," удалено или Unknown.
Отчет-предупреждение для начальное состояние узла будут перечислены все неработоспособные начальных узлов с подробными сведениями.

* **SourceID**: System.FM
* **Свойство**: SeedNodeStatus
* **Дальнейшие действия**: Если это предупреждение появляется в кластере, выполните приведенные ниже инструкции, чтобы исправить его: Для кластера под управлением Service Fabric версии 6.5 или более поздней версии: Для кластера Service Fabric в Azure после начальный узел выйдет из строя, Service Fabric будет пытаться автоматически изменить его на узел без начального значения. Чтобы это произошло, убедитесь, что количество узлов не начальное значение в тип первичного узла является больше или равно числу вниз начальных узлов. При необходимости добавьте дополнительные узлы для типа первичного узла, чтобы добиться этого.
В зависимости от состояния кластера может занять некоторое время, чтобы устранить эту проблему. После этого отчет-предупреждение автоматически очищается.

Для изолированного кластера Service Fabric снимите отчет-предупреждение всех начальных узлов нужно работоспособное состояние. В зависимости от того, почему начальные узлы находятся в неработоспособном состоянии, необходимо выполнить разные действия: если начальный узел — устройство, пользователи должны вызвать начальное значение узла; Если начальный узел удаленные или неизвестно, это начальный узел [должен быть удален из кластера](https://docs.microsoft.com/azure/service-fabric/service-fabric-cluster-windows-server-add-remove-nodes).
Отчет-предупреждение автоматически очищается при всех начальных узлов работоспособное состояние.

Для кластера Service Fabric версии старше 6.5: В этом случае отчет-предупреждение необходимо вручную очистить. **Пользователям следует убедиться, все начальные узлы работоспособное состояние до очистке отчета**: если начальный узел не работает, необходимо открыть этот узел начальное значение; если начальный узел удаленные или неизвестно, что начальный узел должен быть удален из кластера.
После всех начальных узлов работоспособное состояние, используйте следующую команду из Powershell для [снимите отчет-предупреждение](https://docs.microsoft.com/powershell/module/servicefabric/send-servicefabricclusterhealthreport):
...
* **Дальнейшие действия**: Эта проблема может стать проблемой, так как регулирующие пакеты службы не задается надлежащим образом и [ресурсами](service-fabric-resource-governance.md) не работает должным образом. Обновите манифест кластера, используя правильные значения емкости узлов для этих метрик, или не указывайте их, тогда Service Fabric автоматически обнаружит доступные ресурсы.
...
* **SourceId**: System.CM
* **Свойство**: Состояние.
* **Дальнейшие действия**: Если приложение было создано или обновлено, оно должно включать отчет о работоспособности диспетчера кластера. Или проверьте состояние приложения, выполнив запрос (например, командлет PowerShell **Get-ServiceFabricApplication -ApplicationName** *имя_приложения*).
...
* **SourceId**: System.FM
* **Свойство**: Состояние.
* **Дальнейшие действия**: Если состояние работоспособности не ОК, это возможно, что некоторые реплики не созданные, открыт или неправильно повышен до первичной или вторичной. 
...
* **Дальнейшие действия**: Изучение кода службы или аварийных дампов, чтобы определить, почему происходит сбой операции.
...
* **SourceId**: System.RA
* **Свойство**: Перенастройка.
* **Дальнейшие действия**: Изучите локальные или удаленные реплики в зависимости от описания отчета о работоспособности.
...
* **Дальнейшие действия**: Проверьте, почему вызов выполняется дольше, чем ожидалось.
...
- **IStatefulServiceReplica.Open**: Это предупреждение указывает, что вызов `CreateServiceInstanceListeners`, `ICommunicationListener.OpenAsync`, или если переопределен, `OnOpenAsync` зависла.
...
- **IStatefulServiceReplica.ChangeRole (S)** и **IStatefulServiceReplica.ChangeRole(N)** : Наиболее распространенный случай — когда служба не учитывает маркер отмены, передаваемый в `RunAsync`. В этом случае лучше всего перезапуск реплики.

- **IStatefulServiceReplica.ChangeRole(P)** : Наиболее распространенный случай — служба не вернула задачу из `RunAsync`.

Остальные вызовы API, которые могут быть заблокированы, относятся к интерфейсу **IReplicator**. Пример:
...
* **Дальнейшие действия**: Если приложение не работает, выясните причину ошибки при активации.
...
* **SourceId**: System.Hosting
* **Свойство**: **Скачайте**, включая версию выпуска.
* **Дальнейшие действия**: Исследуйте причину ошибки при скачивании на узле.
...
* **SourceId**: System.Hosting
* **Свойство**: Использует префикс **CodePackageActivation** и содержит имя пакета кода и точку входа вида *CodePackageActivation:CodePackageName:SetupEntryPoint / EntryPoint*. Например, **CodePackageActivation:Code:SetupEntryPoint**.
...
* **Описание** Указывает обнаружена ошибка.
...
* **SourceId**: System.Hosting
* **Свойство**: **ResourceGovernance**.
* **Дальнейшие действия**: Предпочтительный способ решения этой проблемы является изменение манифеста кластера, чтобы включить автоматическое обнаружение доступных ресурсов. Другой способ — обновление манифеста кластера, путем указания правильных значений емкости узла для этих метрик.
