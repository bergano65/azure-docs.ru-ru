...
* **Свойство**: Начинается с **соседства** и включает информацию о узлах.
...
### <a name="seed-node-status"></a>Статус семенного узла
**System.FM** сообщает о предупреждении уровня кластера, если некоторые узлы семян неработоспособны. Узлы семян являются узлами, которые поддерживают доступность базового кластера. Эти узлы гарантируют работу кластера, арендуя другие узлы и выступая в роли прерывателя связи во время определенных сбоев сети. Если большинство узлов семян в кластере не свернуты и они не возвращаются обратно, кластер автоматически выключается. 

Узлы семян нездоровы, если его статус узла вниз, удален или неизвестен.
Отчет о предупреждении о состоянии семенного узла будет перечислять все нездоровые узлы семян с подробной информацией.

* **Источник**: System.FM
* **Недвижимость**: SeedNodeStatus
* **Следующие шаги**: Если это предупреждение отображается в кластере, следуйте ниже инструкции, чтобы исправить его: Для кластера работает версия Service Fabric 6.5 или выше: Для кластера Service Fabric на Azure, после того, как узел семян выходит из строя, Service Fabric будет пытаться автоматически изменить его на узел без семян. Чтобы это произошло, убедитесь, что количество узлов, не связанных с семенами, в типе первичного узла больше или равно числу узлов семян Down. При необходимости добавьте дополнительные узлы к типу первичного узла для достижения этой цели.
В зависимости от статуса кластера может потребоваться некоторое время, чтобы устранить проблему. Как только это сделано, отчет о предупреждении автоматически очищается.

Для автономного кластера Service Fabric, чтобы очистить отчет о предупреждении, все узлы семян должны стать здоровыми. В зависимости от того, почему узлы семян нездоровы, необходимо предпринять различные действия: если узла семян нет, пользователям необходимо довести этот узлы семян; если узла семян удалена или неизвестна, этот узлы [семян необходимо удалить из кластера.](https://docs.microsoft.com/azure/service-fabric/service-fabric-cluster-windows-server-add-remove-nodes)
Отчет о предупреждении автоматически очищается, когда все узлы семян становятся здоровыми.

Для кластерной версии Service Fabric старше 6.5: В этом случае отчет о предупреждении должен быть удален вручную. **Пользователи должны убедиться, что все узлы семян становятся здоровыми перед очисткой отчета:** если узла семян вниз, пользователи должны принести, что узлы семян вверх; если узла семян удаленили или неизвестны, что узлы семян должны быть удалены из кластера.
После того, как все узлы семян становятся здоровыми, используйте следующие команды от Powershell, чтобы [очистить отчет о предупреждении:](https://docs.microsoft.com/powershell/module/servicefabric/send-servicefabricclusterhealthreport)
...
* **Свойство**: Начинается с **сертификата** и содержит более подробную информацию о типе сертификата.
...
* **SourceId**: System.PLB.
* **Свойство**: Начинается с **репликиОграничение Нарушение**.
...
Свойство и текст указывают, какие API привели к блокировке. Дальнейшие действия отличаются и зависят от программных интерфейсов, вызвавших блокировку. Любой API на *IStatefulServiceReplica* или *IStatelessServiceInstance* обычно является ошибкой в сервисном коде. В следующем разделе описывается, как они переводятся в [модель Надежных Услуг:](service-fabric-reliable-services-lifecycle.md)

- **IStatefulServiceReplica.Open**: Это предупреждение указывает `CreateServiceInstanceListeners` `ICommunicationListener.OpenAsync`на то, что `OnOpenAsync` вызов, или, если переопределен, застрял.
...
- **IStatefulServiceReplica.ChangeRole(S)** и **IStatefulServiceReplica.ChangeRole(N)**. Наиболее распространенный случай — когда служба не учитывает маркер отмены, передаваемый в `RunAsync`. В этом случае лучшим решением является перезагрузка реплики.

- **IStatefulServiceReplica.ChangeRole(P)**: Наиболее распространенным случаем является то, что служба не вернула задачу от `RunAsync`.

Другие вызовы API, которые могут застрять, находятся на интерфейсе **IReplicator.** Пример:
...
**System.Replicator** на первичной реплике сообщает предупреждение, когда подключение к вторичному (удаленному) репликатору не является здоровым. Адрес удаленного репликатора можно посмотреть в сообщении отчетов. Это упрощает определение ложных конфигураций и сетевых проблем между репликаторами.
...
* **Свойство:** начинается с префикса **Duration_** и определяет медленную операцию и имя экземпляра Service Fabric, к которому применяется операция. Например, если создание сервиса по **названию ткани: /MyApp/MyService** занимает слишком много времени, свойство **Duration_AOCreateService.fabric:/MyApp/MyService**. Authority Owner указывает на роль секции именования для этого имени и операции.
...
System.Hosting сообщает об отсутствии проблем, если тип службы успешно зарегистрирован. Он сообщает об ошибке, если регистрация не была выполнена вовремя, как настроено с помощью **ServiceTypeRegistrationTimeout**. При закрытии среды выполнения регистрация типа службы на узле отменяется и служба Hosting отображает предупреждение.
...
* **Описание**: Очки к обнаруженной ошибке.
