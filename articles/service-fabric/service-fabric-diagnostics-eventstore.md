---
title: Магазин событий Azure Service Fabric
description: Узнайте о EventStore службы Службы Azure Fabric, способе понять и контролировать состояние кластера или рабочих нагрузок в любое время.
author: srrengar
ms.topic: conceptual
ms.date: 6/6/2019
ms.author: srrengar
...
---
# <a name="eventstore-overview"></a>Обзор EventStore
...
>[!NOTE]
>Начиная с версии Service Fabric 6.4 AБИИ eventStore и UX, как правило, доступны для кластеров Windows Azure. Мы работаем над переносом этих функциональных возможностей в Linux, а также в изолированные кластеры.
...
Чтобы узнать больше о API, ознакомьтесь со [ссылкой на EventStore API.](https://docs.microsoft.com/rest/api/servicefabric/sfclient-index-eventsstore)
...
### <a name="azure-cluster-version-65"></a>Кластерная версия Azure 6.5
Если кластер Azure будет обновлен до версии 6.5 или выше, EventStore будет автоматически включен в кластер. Чтобы отказаться от этого, необходимо обновить шаблон кластера следующим образом:

* Используйте aPI-версию или более новую `2019-03-01` 
...
### <a name="azure-cluster-version-64"></a>Кластерная версия Azure 6.4

Если вы используете версию 6.4, вы можете отсеивать шаблон менеджера ресурсов Azure, чтобы включить службу EventStore. Это делается путем выполнения [обновления кластерной конфигурации](service-fabric-cluster-config-upgrade-azure.md) и добавления следующего кода, вы можете использовать PlacementServices для размещения реплик службы EventStore на определенном NodeType, например, NodeType, посвященный системным службам. Раздел `upgradeDescription` настраивает обновление конфигурации, чтобы активировать перезапуск узлов. Раздел можно удалить в другом обновлении.
