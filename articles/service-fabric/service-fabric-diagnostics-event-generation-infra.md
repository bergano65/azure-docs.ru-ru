---
title: Мониторинг уровня платформы Лазурного сервиса
...
* **Оперативная работа** Операции высокого уровня, выполняемые Service Fabric и кластером, включая события для приближающихся узлов, развертывание нового приложения или откат обновления и т.д. Полный список мероприятий [смотрите здесь.](service-fabric-diagnostics-event-generation-operational.md)  
...
Это предпочтительный способ сбора данных производительности на компьютере. У агентов обычно есть список возможных метрик производительности, которые можно собрать. Процесс их выбора или изменения достаточно прост. Читать о Azure Monitor, предлагая журналы Azure Monitor в [интеграции журналов Azure Monitor](service-fabric-diagnostics-event-analysis-oms.md) service Fabric и [настройке агента Log Analytics,](../log-analytics/log-analytics-windows-agent.md) чтобы узнать больше об агенте Log Analytics, который является одним из таких агентов мониторинга, который способен собирать данные о производительности для кластерных ВМ и развернутых контейнеров.
...
* Прочитайте об [интеграции журналов Azure Monitor службы](service-fabric-diagnostics-event-analysis-oms.md) для сбора кластерной диагностики и создания пользовательских запросов и оповещений
