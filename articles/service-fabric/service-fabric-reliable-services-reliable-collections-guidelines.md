...
* Хранимые службы нельзя сделать временными. Изменение ```HasPersistedState``` флага для ```false``` необходимости повторного создания всей службы с нуля
* НЕВОЗМОЖНО сохранить временные службы. Изменение ```HasPersistedState``` флага для ```true``` необходимости повторного создания всей службы с нуля
