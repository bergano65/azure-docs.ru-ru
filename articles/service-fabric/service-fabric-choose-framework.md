...
Эксклюзивные для сервисной ткани, Надежные услуги также могут быть состояния, где состояние сохраняется непосредственно в самой службе с помощью надежных коллекций. Состояние становится высокодоступным за счет репликации и распределения путем секционирования, которыми автоматически управляет Service Fabric.
...
Созданная поверх надежных служб, платформа Reliable Actor представляет собой платформу приложений, которая реализует шаблон [Virtual Actor,](https://research.microsoft.com/en-us/projects/orleans/) основанный на [вычислительной модели актера.](https://en.wikipedia.org/wiki/Actor_model) Платформа Reliable Actor использует независимые блоки вычислений и состояния с однопотоковым исполнением, называемым *актерами.* Платформа надежных субъектов обеспечивает встроенное взаимодействие для субъектов, а также предустановленное сохранение состояния и масштабируемые конфигурации.
...
[Сервисная ткань и ASP.NET ядро](service-fabric-reliable-services-communication-aspnetcore.md)
