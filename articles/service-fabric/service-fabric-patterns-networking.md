---
title: Шаблоны сетей для службы Azure Fabric
...
- [Статический публичный IP-адрес](#staticpublicip)
...
### <a name="allowing-the-service-fabric-resource-provider-to-query-your-cluster"></a>Разрешение поставщику ресурсов Service Fabric заставить вас задать запрос кластера
...
После развертывания вы можете увидеть, что подсистема балансировки нагрузки привязана к общедоступному статическому IP-адресу из другой группы ресурсов. Конечная точка подключения клиента Service Fabric и конечная точка [Service Fabric Explorer](service-fabric-visualizing-your-cluster.md) указывают на DNS F-DN статического IP-адреса.

<a id="internallb"></a>
## <a name="internal-only-load-balancer"></a>Подсистема балансировки нагрузки только для внутреннего использования

В этом сценарии внешняя подсистема балансировки нагрузки в шаблоне по умолчанию Service Fabric заменяется подсистемой балансировки нагрузки только для внутреннего использования. Смотрите [более раннюю статью](#allowing-the-service-fabric-resource-provider-to-query-your-cluster) о последствиях для портала Azure и для поставщика ресурсов Service Fabric.
...
В этом сценарии к существующей внешней подсистеме балансировки нагрузки для одного типа узла добавляется внутренняя подсистема балансировки нагрузки для того же типа узла. Внутренний порт, подключенный к внутреннему пулу адресов, может быть назначен только одной подсистеме балансировки нагрузки. Выберите, какая подсистема балансировки нагрузки будет управлять портами вашего приложения, а какая — конечными точками управления (порты 19000 и 19080). Если вы поместите конечные точки управления на внутренний балансера нагрузки, имейте в виду ограничения поставщика ресурсов Service Fabric, [обсуждаемые ранее в статье.](#allowing-the-service-fabric-resource-provider-to-query-your-cluster) В используемом примере конечные точки управления, остаются во внешней подсистеме балансировки нагрузки. Также мы добавляем порт приложения 80 и размещаем его во внутренней подсистеме балансировки нагрузки.
...
## <a name="notes-for-production-workloads"></a>Заметки для производственных нагрузок

Вышеприведенные шаблоны GitHub предназначены для работы с SKU по умолчанию для Azure Standard Load Balancer (SLB), Basic SKU. Этот SLB не имеет SLA, поэтому для производственных нагрузок стандартный SKU должен быть использован. Подробнее об этом читайте [в обзоре баланса стандартных перегрузок Azure.](/azure/load-balancer/load-balancer-standard-overview) Любой кластер Service Fabric, использующий Стандартный SKU для SLB, должен гарантировать, что каждый тип узла имеет правило, позволяющее исходящих трафика на порт 443. Это необходимо для завершения настройки кластера, и любое развертывание без такого правила завершится неудачей. В приведенном выше примере балансиста нагрузки «только внутренний» необходимо добавить дополнительный сбалансеру внешней нагрузки в шаблон с правилом, позволяющим исходящий трафик для порта 443.
