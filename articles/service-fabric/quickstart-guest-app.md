---
title: Быстро развернуть существующее приложение в кластер
...
Перед началом работы [настройте среду разработки](service-fabric-get-started.md). Которая включает в себя установку сервиса Fabric SDK и Visual Studio 2019 или 2015.
...
Если у вас нет подписки На Azure, создайте [бесплатную учетную запись][create-account].
...
>[!IMPORTANT]
>Node.js может легко преодолеть лимит в 260 знаков для путей Windows. Используйте короткий путь для самого проекта, например **c:\code\svc1**. Дополнительно вы можете следовать **[этим инструкциям,](https://stackoverflow.com/a/41687101/1664231)** чтобы включить длинные пути файла в Windows 10.
...
Проект приложения **(MyGuestApp**) не содержит кода напрямую. Проект ссылается на набор сервисных проектов. Кроме того, он содержит три других типа контента:
...
Откройте файл **ServiceManifest.xml** в проекте. В нижней части манифеста, `<Resources> \ <Endpoints>` есть с записью уже определены. Измените эту запись, чтобы добавить `Port`, `Protocol` и `Type`. 
...
Если вы нажмете **F5** и запустите проект, он будет развернут в локальном кластере. Тем не менее давайте лучше развернем его в Azure.
...
В разделе **Конечная точка подключения:** выберите кластер Service Fabric для развертывания. Если у вас его нет, выберите ** &lt;Создать новый кластер... &gt; ** который открывает окно веб-браузера для портала Azure. Дополнительные сведения см. в разделе [Создание кластера на портале Azure](service-fabric-cluster-creation-via-portal.md#create-cluster-in-the-azure-portal). 
...
Создание нового кластера Service Fabric займет некоторое время. Как только он был создан, вернитесь к ** &lt;&gt;** диалогу публикации и выберите Refresh . Новый кластер появится в окне раскрывающегося списка. Выберите его.
...
Перейдите по этому адресу, `HELLO WORLD` где вы увидите ответ.

## <a name="delete-the-cluster"></a>Удаление кластера

Не забудьте удалить все ресурсы, которые вы создали для этого quickstart, как вы взимается плата за эти ресурсы.
