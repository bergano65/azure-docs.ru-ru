...
Откройте файл **ServiceManifest.xml** в проекте. В нижней части манифеста есть запись `<Resources> \ <Endpoints>` с уже определенным элементом. Измените эту запись, чтобы добавить `Port`, `Protocol` и `Type`. 
