...
Откройте файл **ServiceManifest.xml** в проекте. В нижней части манифеста есть `<Resources> \ <Endpoints>` с уже определенным элементом. Измените эту запись, чтобы добавить `Port`, `Protocol` и `Type`. 
...
Перейдите к этому адресу, где вы увидите `HELLO WORLD` ответ.
