---
title: Службы автоматического масштабирования и контейнеров Azure Service Fabric
...
* _Нижний порог нагрузки_ — это значение, определяющее, когда служба будет **масштабирована.** Если среднее время загрузки всех экземпляров разделов ниже этого значения, служба свернется.
* _Верхний порог нагрузки_ — это значение, определяющее, когда служба будет **масштабирована.** Если средняя загрузка всех экземпляров раздела выше этого значения, то служба будет уменьшена.
...
* _Минимальный отсчет экземпляров_ определяет нижний предел для масштабирования. Если число экземпляров раздела достигает этого предела, служба не будет свертываться, независимо от загрузки.
...
* _Нижний порог нагрузки_ — это значение, определяющее, когда служба будет **масштабирована.** Если среднее время загрузки всех разделов службы ниже этого значения, служба свернется.
* _Верхний порог нагрузки_ — это значение, определяющее, когда служба будет **масштабирована.** Если средняя загрузка всех разделов службы выше этого значения, то услуга будет масштабирована.
* _Интервал масштабирования_ определяет, как часто будет проверяться триггер. После проверки триггера, если необходимо масштабирование, будет применен соответствующий механизм. Если масштабирование не требуется, никакие действия выполняться не будут. В обоих случаях до истечения интервала масштабирования триггер снова проверяться не будет.
...
* _Минимальный отсчет экземпляров_ определяет нижний предел для масштабирования. Если число разделов службы достигает этого предела, служба не будет свертываться, независимо от нагрузки.
