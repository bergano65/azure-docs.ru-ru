---
title: Защищайте кластер, работающий на Windows, используя безопасность Windows
...
Образец файла конфигурации *ClusterConfig.gMSA.Windows.MultiMachine.JSON* загружен с помощью [Microsoft.Azure.ServiceFabric.WindowsServer.\< Версия>.zip](https://go.microsoft.com/fwlink/?LinkId=730690) автономный кластерный пакет содержит шаблон для настройки безопасности Windows с помощью [группы управляемых сервисных счетов (gMSA)](https://technet.microsoft.com/library/hh831782.aspx):  
...
| --- | --- |
| ClusterCredentialType |Набор ими *для Windows* для обеспечения безопасности узлов.  | 
...
| идентификации |Добавьте пользователя домена "домен\имя_пользователя" для удостоверения клиента. |
...
Эту модель не рекомендуется использовать. Рекомендуется использовать gMSA, как описано выше. Образец *конфигурации ClusterConfig.Windows.MultiMachine.JSON* файл конфигурации загружен с [Microsoft.Azure.ServiceFabric.WindowsServer.\< версия>.zip](https://go.microsoft.com/fwlink/?LinkId=730690) автономный кластерный пакет содержит шаблон для настройки безопасности Windows.  Безопасность Windows настраивается в разделе **Properties** . 
...
[Безопасность узлов](service-fabric-cluster-security.md#node-to-node-security) настраивается с помощью **ClusterIdentity,** если вы хотите использовать группу машин в домене Active Directory. Дополнительные сведения см. в статье [How to Create a Group in Active Directory](https://msdn.microsoft.com/library/aa545347(v=cs.70).aspx) (Как создать группу компьютеров в Active Directory).

[Безопасность от клиента к узлам](service-fabric-cluster-security.md#client-to-node-security) настраивается с помощью **ClientIdentities.** Чтобы установить отношения доверия между клиентом и кластером, необходимо настроить кластер таким образом, чтобы ему были известны удостоверения клиентов, которым можно доверять. Установить отношения доверия можно двумя разными способами:
