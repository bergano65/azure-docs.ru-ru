...
Поведение загрузки сертификата сертификата по умолчанию Azure Service Fabrics SDK заключается в развертывании и использовании определенного сертификата с датой истечения срока действия в будущем; независимо от их первичного или вторичного определения конфигурации. Возвращение к классическому поведению не рекомендуется продвинутым действием и требует установки параметра "UseSecondaryIfNewer" на ложное в конфигурации. `Fabric.Code`
...
Для защиты кластера всегда требуется хотя бы один действительный (не отозванный и не просроченный) сертификат. Сертификат, развернутый с самым дальним в будущем сроком годности, будет использоваться, и его удаление сделает ваш кластер прекратить работу; обеспечить удаление сертификата, срок действия которого истек, или неиспользованного сертификата, срок действия которого истекает в ближайшее время.
...
> [!TIP]
> Теперь есть лучший и простой способ добавления вторичного сертификата с помощью cmdlet [Add-AzServiceFabricClusterCertificate.](/powershell/module/az.servicefabric/add-azservicefabricclustercertificate) Остальные инструкции в этом разделе выполнять не обязательно.  Кроме того, вам не нужен шаблон, первоначально используемый для создания и развертывания кластера при использовании [cmdlet Add-AzServiceFabricClusterCertificate.](/powershell/module/az.servicefabric/add-azservicefabricclustercertificate)
...
> [!NOTE]
> Если вы ищете шаблон образца и параметры, которые можно использовать для подражания или в качестве отправной точки, то загрузите его из этого [git-репо.](https://github.com/Azure-Samples/service-fabric-cluster-templates/tree/master/Cert-Rollover-Sample) 
> 
> 

### <a name="edit-your-resource-manager-template"></a>Изменение шаблона Resource Manager

Для удобства выполнения инструкций пример 5-VM-1-NodeTypes-Secure_Step2.JSON содержит все изменения, которые мы будем вносить. образец доступен на [git-репо](https://github.com/Azure-Samples/service-fabric-cluster-templates/tree/master/Cert-Rollover-Sample).
...
4. Внесите изменения **во все** определения ресурсов **Microsoft.Compute/virtualMachineScaleSets** - Найдите определение ресурсов Microsoft.Compute/virtualMachineScaleSets. Перейдите к подразделу "publisher": "Microsoft.Azure.ServiceFabric" в разделе "virtualMachineProfile".
...
- Вопийте в учетной записи Azure и выберите конкретную подписку azure. Это важный шаг для пользователей, имеющих доступ к нескольким подпискам Azure.
...
Разверните шаблон в группу ресурсов. Используйте группу ресурсов, в которой развернут кластер. Выполнить команду New-AzResourceGroupDeployment. Не нужно указывать режим, так как по умолчанию используется **добавочный**режим.
...
## <a name="deploying-client-certificates-to-the-cluster"></a>Развертывание сертификатов клиента в кластер.
...
## <a name="adding-application-certificates-to-a-virtual-machine-scale-set"></a>Добавление сертификатов приложений в набор виртуальной шкалы машин

Для развертывания сертификата, который используется для приложений в кластере, смотрите [этот пример сценария Powershell.](scripts/service-fabric-powershell-add-application-certificate.md)
