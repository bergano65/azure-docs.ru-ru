...
Для защиты кластера всегда требуется хотя бы один действительный (не отозванный и не просроченный) сертификат. Сертификат, развернутый с самой поздней датой окончания срока действия, будет использован, и его удаление сделает кластер перестанет работать. обязательно удалите только просроченный сертификат или неиспользованный сертификат, срок действия которого истекает в ближайшее время.
