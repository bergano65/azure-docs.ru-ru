---
title: Диагностика stateful надежных служб Службы службы Azure
...
*ServiceFabricPartitionID* — это строка идентификатора секции Service Fabric, c которой связан экземпляр счетчика производительности. Идентификатор раздела — это GUID, [`Guid.ToString`](https://msdn.microsoft.com/library/97af8hh4.aspx) а его представление строкгенерируется через с помощью спецификации формата «D».

*ServiceFabricReplicaId* — идентификатор, связанный с конкретной репликой надежной службы. Идентификатор реплики включен в имя счетчика производительности, чтобы обеспечить его уникальность и избежать конфликта с другими контраприточными экземплярами производительности, генерируемыми тем же разделом. Дополнительные сведения о репликах и их роли в надежных службах см. в [этой статье](service-fabric-concepts-replica-lifecycle.md).
...
*StateProviderId* — это идентификатор, связанный с государственным поставщиком в рамках надежной службы. Идентификатор поставщика состояния включен в имя счетчика производительности, чтобы отличать TStore от другого.
...
*StateProviderName* — это имя, связанное с государственным поставщиком в рамках надежной службы. Имя поставщика состояния включено в имя экземпляра счетчика производительности для пользователей, чтобы легко определить, какое состояние оно предоставляет.
...
| Вход флеш Байтс / сек | Число байтов, записываемых на диск репликатором транзакций в секунду. |
| Регулируемых операций/с | Число операций, отклоняемых репликатором транзакций каждую секунду из-за регулирования. |
| Avg. Транзакция ms/Commit | Среднее время задержки фиксации на транзакцию (в мс). |
| Avg. Флеш Задержка (ms) | Средняя продолжительность операции окончательной записи данных на диск, инициированной репликатором транзакций (в мс). |
