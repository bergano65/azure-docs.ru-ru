...
description: Узнайте о масштабировании в azure Service Fabric и различных методах, используемых для масштабирования приложений.
author: masnider
ms.topic: conceptual
ms.date: 08/26/2019
ms.author: masnider
...
![Расположение раздела с тремя узлами](./media/service-fabric-concepts-scalability/layout-three-nodes.png)
</center>
...
![Расположение раздела с четырьмя узлами](./media/service-fabric-concepts-scalability/layout-four-nodes.png)
</center>
...
Из-за различий в реализации между операционными системами выбор использования Service Fabric с Windows или Linux может быть жизненно важной частью масштабирования приложения. Одним из потенциальных препятствий является то, как осуществляется поэтапная вырубка. Service Fabric на Windows использует драйвер ядра для журнала one-per-machine, совместно гостевого реплики службы. Этот журнал весит около 8 ГБ. Linux, с другой стороны, использует 256 МБ постановки журнала для каждой реплики, что делает его менее идеальным для приложений, которые хотят максимизировать количество легких реплик службы работает на данном узла. Эти различия во временных требованиях к хранению потенциально могут служить основой для желаемой платформы для развертывания кластера Service Fabric.
