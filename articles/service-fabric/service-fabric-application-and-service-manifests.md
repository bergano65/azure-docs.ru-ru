---
title: Описание приложений и услуг Службы обслуживания Azure
...
**ConfigPackage** объявляет папку с именем, указанным в атрибуте **Name**, которая содержит файл *Settings.xml*. Файл параметров содержит разделы заданных пользователем параметров пар "ключ — значение", которые считываются процессом во время выполнения. Во время обновления, если изменилась только **версия** **ConfigPackage,** то процесс выполнения не перезапускается. Вместо этого при помощи обратного вызова в процесс передается уведомление о том, что параметры конфигурации изменились, поэтому они были перезагружены в динамическом режиме. Вот пример *файла Settings.xml:*
...
**Ограничения размещения** – это операторы, определяющие, где должны работать службы. Эти операторы прикрепляются к отдельным службам, которые вы выбираете для одного или нескольких свойств узлов. Для получения дополнительной информации [см.](https://docs.microsoft.com/azure/service-fabric/service-fabric-cluster-resource-manager-cluster-description#placement-constraints-and-node-property-syntax)

**Политики** (не установленные в предыдущем примере) описывают сбор журналов, [выполнение по умолчанию,](service-fabric-application-runas-security.md) [работу](service-fabric-health-introduction.md#health-policies)и политики доступа [к безопасности,](service-fabric-application-runas-security.md) установленные на уровне приложения, включая вопрос о том, имеет ли служба (ы) доступ к времени выполнения Service Fabric.

> [!NOTE] 
> По умолчанию приложения Service Fabric имеют доступ к времени выполнения Service Fabric в виде конечной точки, принимающей запросы, зависящие от приложений, и переменные среды, указывающие на пути файлов на хосте, содержащем Fabric, и файлы, связанные с приложениями . Рассмотрите возможность отключения этого доступа, когда приложение хостов ненадежный код (т.е. код, происхождение которого неизвестно, или который владелец приложения знает, что не безопасно выполнить). Для получения дополнительной информации, пожалуйста, ознакомьтесь с [рекомендациями по безопасности в Service Fabric](service-fabric-best-practices-security.md#platform-isolation). 
>
