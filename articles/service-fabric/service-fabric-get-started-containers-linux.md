...
Начиная с последнего обновления версии 6.4, вы можете указать, что оценки DOCKER HEALTHCHECK должны выводиться как ошибки. Если этот параметр включен, появится отчет о работоспособности **ОК** , если *health_status* является *работоспособным* и при неработоспособности *health_status* появится **сообщение об ошибке** .
...
Если для *треатконтаинерунхеалсистатусасеррор* задано **значение true**, отчеты о работоспособности **ошибок** будут отображаться, если *health_status* контейнера находится в неработоспособном *состоянии*.
