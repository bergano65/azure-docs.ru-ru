---
title: Настройка Reliable Services Service Fabric Azure
description: Узнайте, как настроить Reliable Services с отслеживанием состояния в глобальном приложении Service Fabric Azure и для одной службы.
author: sumukhs
ms.topic: conceptual
ms.date: 10/02/2017
ms.author: sumukhs
...
| MaxSecondaryReplicationQueueSize |Число операций |16384 |Максимальное количество операций в дополнительной очереди. Операция освобождается после того, как для ее состояния будет обеспечена высокая доступность посредством сохранения. Это значение должно быть больше 64 и представлять собой степень числа 2. |
...
| енаблестаблереадс |Логическое |Нет |Включение стабильных операций чтения позволяет вторичным репликам возвращать значения, которые были подтверждено) кворума. |
