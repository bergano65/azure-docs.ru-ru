...
Надежная очередь использует блокировки на уровне операций, что позволяет одной транзакции с `TryPeekAsync` и (или) `TryDequeueAsync` и по одной транзакции с `EnqueueAsync` за раз.
...
| Запрос \ Предоставлено | Нет | Общая | Обновить | Монопольная блокировка |
| --- |:--- |:--- |:--- |:--- |
