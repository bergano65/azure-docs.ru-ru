---
title: Масштабирование кластера Service Fabric в или из
description: Масштабируйте кластер Service Fabric в соответствии со спросом, устанавливая правила автоматической шкалы для каждого набора типов узлов/виртуальной машины. Добавление узлов в кластер Service Fabric
ms.topic: conceptual
ms.date: 03/12/2019
...
> [!WARNING]
> Прочитайте этот раздел перед масштабированием
...
## <a name="set-auto-scale-rules-for-the-node-typevirtual-machine-scale-set"></a>Установите правила автоматической шкалы для набора типа узла/виртуального набора машин
Если в кластере есть несколько типов узлов, повторите это для каждого типа узлов/виртуальных наборов масштабов машины, которые вы хотите масштабировать (в или из). Перед настройкой автомасштабирования обратите внимание на необходимое количество узлов. Минимальное количество узлов, необходимое для основного типа узлов, зависит от выбранного уровня надежности. См. дополнительные сведения об [уровнях надежности](service-fabric-cluster-capacity.md).
...
Следуйте этим [инструкциям, чтобы настроить авто-масштаб для каждого виртуального набора масштаба машины.](../virtual-machine-scale-sets/virtual-machine-scale-sets-autoscale-overview.md)

> [!NOTE]
> В сценарии уменьшения масштаба, если ваш тип узла не имеет [уровня прочности][durability] золота или серебра, вам необходимо позвонить в [cmdlet Remove-ServiceFabricState](https://docs.microsoft.com/powershell/module/servicefabric/remove-servicefabricnodestate) с соответствующим именем узла. Для бронзовой долговечности, не рекомендуется масштабировать более одного узла за один раз.
> 
> 

## <a name="manually-add-vms-to-a-node-typevirtual-machine-scale-set"></a>Вручную добавляйте виртуальные вывЕСы в набор масштабов типа узла/виртуального станка
...
> [!NOTE]
> Добавление ВМ требует времени, поэтому не ожидайте, что дополнения будут мгновенными. Так что планируйте добавить емкость заблаговременно, чтобы обеспечить более 10 минут, прежде чем мощность VM доступна для реплик / экземпляров службы, чтобы получить помещены.
> 

### <a name="add-vms-using-a-template"></a>Добавление ВМ с помощью шаблона
Следуйте примеру/инструкциям в [галерее шаблонов quickstart,](https://github.com/Azure/azure-quickstart-templates/tree/master/201-vmss-scale-existing) чтобы изменить количество vMs в каждом типе узла. 

### <a name="add-vms-using-powershell-or-cli-commands"></a>Добавление VM с помощью команд PowerShell или CLI
...
## <a name="manually-remove-vms-from-a-node-typevirtual-machine-scale-set"></a>Вручную удалить Виртуальные компьютеры из набора масштабов узлов/виртуальной машины
При масштабе в типе узла вы удаляете экземпляры VM из набора масштабов. Если тип узла является бронзовым уровнем долговечности, Service Fabric не знает, что произошло, и сообщает, что узла не хватает. Затем Service Fabric сообщает о неработоспособном состоянии кластера. Чтобы предотвратить плохое состояние, необходимо убрать узла из кластера и удалить состояние узла.

Службы системы обслуживания работают в основном типе узла в кластере. При уменьшении типа первичного узла никогда не снижайте количество экземпляров до меньше, чем гарантирует [уровень надежности.](service-fabric-cluster-capacity.md) 
...
Шаги для ручного удаления состояния узлов применяются только к типам узлов с *бронзовым* уровнем прочности.  Для уровня прочности *Silver* и *Gold* эти шаги выполняются автоматически платформой. Дополнительные сведения об устойчивости см. в разделе [Характеристики устойчивости кластера][durability].
...
* [Обновления кластеров](service-fabric-cluster-upgrade.md)
