...
Добавьте параметр для имени экземпляра нового масштабируемого набора. Обратите внимание, что `vmNodeType1Name` уникален для нового масштабируемого набора, а значения Count и size идентичны исходному масштабируемому набору.
...
* Новый масштабируемый набор ссылается на новый тип узла:

    ```json
    "nodeTypeRef": "[parameters('vmNodeType1Name')]",
    ```

* Новый масштабируемый набор ссылается на тот же внутренний адрес подсистемы балансировки нагрузки и подсеть, что и исходный, но использует другой пул NAT для входящего трафика подсистемы балансировки нагрузки:
...
Затем добавьте запись в `nodeTypes` список ресурса *Microsoft. ServiceFabric/Clusters* . Используйте те же значения, что и для исходной записи типа узла, за исключением `name` , который должен ссылаться на новый тип узла (*vmNodeType1Name*).
