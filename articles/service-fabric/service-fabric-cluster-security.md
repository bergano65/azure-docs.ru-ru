---
title: Безопасность кластера услуг Службы Azure Fabric
...
Кластер Azure Service Fabric — это ресурс, владельцем которого вы являетесь. Вам следует защитить кластеры, чтобы к ним не смогли подключиться неавторизованные пользователи. Защита кластера особенно важна при выполнении на нем производственных рабочих нагрузок. Возможно создать незащищенный кластер, однако, если кластер предоставляет конечные точки управления общедоступному интернету, анонимные пользователи могут подключиться к нему. Незащищенные кластеры нельзя использовать для выполнения производственных задач. 
...
* Сертификат идентификации сервера и tLS-шифрования http-сообщения
...
* **Имя субъекта сертификата должно совпадать с доменным именем, которое используется для получения доступа к кластеру Service Fabric**. Это соответствие необходимо для обеспечения TLS для конечных точек управления HTTPS кластера и Service Fabric Explorer. Вы не можете получить сертификат TLS/SSL в органе по сертификации (CA) для домена q.cloudapp.azure.com. Необходимо получить имя личного домена для кластера. При запросе сертификата из ЦС имя субъекта сертификата должно совпадать с именем личного домена, используемого для кластера.

Необходимо учитывать также следующие факторы.

* Поле **Субъект** может содержать несколько значений, каждое из которых содержит сокращение в виде префикса, определяющее тип значения. Как правило, инициализация **CN** (для *общего имени);* например, **CN\.и www contoso.com**.
* Поле **Субъект** может быть пустым.
* Если необязательное поле **Альтернативное имя субъекта** заполнено, его значение должно содержать общее имя сертификата и альтернативное имя субъекта. Они вводятся как значения **DNS Name.** Чтобы узнать о создании сертификатов с альтернативными именами субъектов, ознакомьтесь со статьей [Как добавить дополнительное имя субъекта сертификата безопасного LDAP](https://support.microsoft.com/kb/931351).
* Значение поля **«Предназначение целей»** сертификата должно включать соответствующее значение, такое как **проверка подлинности серверов** или **клиентская аутентификация.**
