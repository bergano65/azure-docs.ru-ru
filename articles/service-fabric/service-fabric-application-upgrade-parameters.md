...
- Visual Studio
- SFCTL
- [Остальные](https://docs.microsoft.com/rest/api/servicefabric/sfclient-api-startapplicationupgrade)
...
> | WhatIf | PS | Разрешенные значения **верны** и **ложны.** Показывает, что произойдет при запуске командлета. Командлет не выполняется. |
...
| Параметр | Описание |
| --- | --- |
default-service-health-policy | [JSON](https://docs.microsoft.com/rest/api/servicefabric/sfclient-model-servicetypehealthpolicy) закодировал спецификацию политики работоспособности, используемой по умолчанию для оценки работоспособности типа службы. Сопоставление является пустым по умолчанию. |
...
upgrade-domain-timeout | Время, которое должен завершить каждый домен обновления, прежде чем *будет выполнено действие FailureAction.* Сначала он интерпретируется как строка, представляющая длительность ISO 8601. В случае неудачи он интерпретируется как число, представляющее общее количество миллисекунд. Значение по умолчанию — "никогда" (бесконечно). Его рекомендуется изменить в соответствии с вашим приложением. Значение по умолчанию: P10675199DT02H48M05.4775807S. |
upgrade-timeout | Время, которое должен завершить каждый домен обновления, прежде чем *будет выполнено действие FailureAction.* Сначала он интерпретируется как строка, представляющая длительность ISO 8601. В случае неудачи он интерпретируется как число, представляющее общее количество миллисекунд. Значение по умолчанию — "никогда" (бесконечно). Его рекомендуется изменить в соответствии с вашим приложением. Значение по умолчанию: P10675199DT02H48M05.4775807S.|
warning-as-error | Разрешенные значения **верны** и **ложны.** Значение по умолчанию является **ложным.** Можно передать в виде флага. Считать предупреждения работоспособности приложения ошибками при оценке работоспособности приложения во время обновления. По умолчанию Service Fabric не считает предупреждения работоспособности ошибками (сбоями), чтобы обновление могло продолжиться даже при появлении предупреждений. |
