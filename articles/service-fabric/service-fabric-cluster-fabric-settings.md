...
|ИспользованиеОтдельныйвторичныйMoveCost|Логическое значение, по умолчанию — false | Динамический|Установка, которая определяет, если PLB следует использовать различные затраты перемещения для вторичного на каждом узлах Если UseSeparateSecondaryMoveCost выключен: - Отчетная стоимость перемещения для вторичного на одном узде приведет к overwritting move cost для каждого вторичного (на всех других узлах) Если UseSeparateSecondaryMoveCost включен: - Отчетная стоимость перемещения для вторичного на одном узлах вступит в силу только на том, что вторичные (не влияет на второстепенные на других узлах) - Если реплика аварии происходит - новая реплика создается с затратой на движение по умолчанию, указанной на службе уровень - Если PLB перемещает существующую реплику - стоимость перемещения идет с ним |
...
|TLS1_2_CipherList| строка| Статические|При установке на непустую строку; переопределяет поддерживаемый список шифров для TLS1.2 и ниже. Смотрите документацию "openssl-ciphers" для извлечения поддерживаемого списка шифров и список формата Пример сильного шифровального списка для TLS1.2: "ECDHE-ECDSA-AES256-GCM-SHA384:ECDSA-AES128-GCM-SHA ECDHE-RSA-AES-128-GCM-SHA256:ECDHE-ECDSA-AES256-CBC-SHA384:ECDHE-ECDSA-AES128 -CBC-SHA256:ECDHE-RSA-AES256-CBC-SHA384:ECDHE-RSA-AES128-CBC-SHA256" Применяется только к Linux. |
