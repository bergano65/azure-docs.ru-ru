...
|GatewayAuthCredentialType |Строка, значение по умолчанию — "None" |Статическое| Указывает тип учетных данных безопасности для использования в конечной точке шлюза приложений HTTP допустимые значения: None/X509. |
...
|MaxTimeoutRetryBuffer | Время в секундах, по умолчанию — 600 |Динамический|Временной диапазон в секундах. Максимальное время ожидания операции, при которой выполняется внутренняя повторная попытка из-за истечения времени ожидания, `<Original Time out> + <MaxTimeoutRetryBuffer>`. Дополнительное время ожидания добавляется с шагом в значение параметра MinOperationTimeout. |
...
|енаблеплатформевентсфилесинк |Логическое значение, по умолчанию — false | Статическое |Включение и отключение событий платформы, записываемых на диск |
...
|фаилуререпортинжекспедитедрепортингинтерваленаблед | Логическое значение, по умолчанию — true | Статическое | Обеспечивает более быструю скорость передачи в DCA, когда FabricHost находится в режиме отчетов о сбоях. |
|фаилуререпортингтимеаут | Интервал времени, по умолчанию — Common::TimeSpan::FromSeconds(60) | Статическое |Временной диапазон в секундах. Время ожидания для отчетов об ошибках DCA в случае, когда FabricHost обнаруживает сбой запуска на раннем этапе. | 
|рундкаонстартупфаилуре | Логическое значение, по умолчанию — true | Статическое |Определяет, следует ли запускать DCA для отправки журналов при возникновении проблем с запуском в FabricHost. | 
...
|енаблеимажесторехеалсрепортинг |Логическое значение, по умолчанию — true |Статическое|Config, чтобы определить, должна ли служба хранилища файлов сообщать о своей работоспособности. |
...
| кнснетворкплугинкнмурлпорт | wstring, по умолчанию — L "48080" | Статическое | Порт URL-адреса API Azure кНм |
| кнснетворкплугинкнсурлпорт | wstring, по умолчанию — L "10090" | Статическое | Порт URL-адреса Azure CNS |
...
|дефаултднссеарчсуффиксемпти|Логическое значение, по умолчанию — false|Статическое|По умолчанию имя службы добавляется к имени DNS SF для служб контейнеров. Эта функция останавливает это поведение, чтобы по умолчанию в пути разрешения ничего не добавлялось к DNS-имени SF.|
...
|днссерверлисттвоипс | Логическое значение, по умолчанию — false | Статическое | Эти флаги дважды добавляют локальный DNS-сервер, чтобы помочь устранить временные неполадки. |
| донотинжектлокалднссервер | Логическое значение, по умолчанию — false | Статическое | Предотвращает внедрение средой выполнения локального IP-адреса в качестве DNS-сервера для контейнеров. |
...
|SharedLogThrottleLimitInPercentUsed|Целое число, по умолчанию — 0 | Статическое | Процент использования общего журнала, который будет вызывать регулирование. Значение должно быть в диапазоне от 0 до 100. Значение 0 подразумевает использование процентного значения по умолчанию. Значение 100 не подразумевает никакого регулирования вообще. Значение от 1 до 99 указывает процент использования журнала, при превышении которого будет выполняться регулирование. Например, если общий журнал равен 10 ГБ, а значение — 90, регулирование будет происходить после того, как 9 ГБ будет использоваться. Рекомендуется использовать значение по умолчанию.|
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|IsEnabled|Логическое значение, по умолчанию — false|Статическое|Пометьте, как контролировать присутствие и состояние службы токенов управляемого удостоверения в кластере. это обязательное условие для использования функций управляемого удостоверения Service Fabric приложений.|
...
|PreferUpgradedUDs|Логическое значение, по умолчанию — true|Динамический|Включает и отключает логику, которая является предпочтительной для перемещения в уже измененные домены обновления.|
...
| **Параметр** | **Допустимые значения** |**Политика обновления**| **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|AADCertEndpointFormat|Строка, значение по умолчанию — ""|Статическое|Формат конечной точки сертификата AAD, коммерческая служба Azure по умолчанию, указана для среды не по умолчанию, такой как Azure для государственных организаций "https:\//login.microsoftonline.us/{0}/FederationMetadata/2007-06/FederationMetadata.XML" |
...
|AADLoginEndpoint|Строка, значение по умолчанию — ""|Статическое|Конечная точка входа AAD, коммерческая служба Azure по умолчанию, указана для среды не по умолчанию, такой как Azure для государственных организаций "https:\//login.microsoftonline.us" |
...
|AADTokenEndpointFormat|Строка, значение по умолчанию — ""|Статическое|Конечная точка токена AAD, коммерческая служба Azure по умолчанию, указана для среды не по умолчанию, такой как Azure для государственных организаций "https:\//login.microsoftonline.us/{0}" |
...
| енфорцелинуксминтлсверсион | Логическое значение, по умолчанию — false | Динамический | Если задано значение true; поддерживается только TLS версии 1.2 +.  Значение false; Поддержка более ранних версий TLS. Применимо только к Linux |
...
|Удалить |Строка, значение по умолчанию — "Admin" |Динамический| Настройки безопасности для операции удаления клиента хранилища образов. |
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|PropertyGroup| УсерсервицеметриккапаЦитиесмап, значение по умолчанию — None | Статическое | Набор ограничений по управлению ресурсами пользовательских служб должен быть статическим, так как он влияет на логику автоопределения |
