...
|ForwardClientCertificate|Логическое значение, по умолчанию — false|Динамический|Если задано значение false, обратный прокси-сервер не будет запрашивать сертификат клиента. Если задано значение true, обратный прокси-сервер запрашивает сертификат клиента во время подтверждения TLS и перенаправляет строку формата PEM в кодировке Base64 в заголовок службы в заголовке с именем X-Client-Certificate. служба может завершить запрос с соответствующим кодом состояния после проверки данных сертификата. Если клиент при значении true не предоставляет сертификат, обратный прокси-сервер пересылает пустой заголовок и передает принятие решения службе. Обратный прокси-сервер будет выступать в качестве прозрачного уровня. Дополнительные сведения см. в разделе [Настройка проверки подлинности сертификата клиента через обратный прокси-сервер](service-fabric-reverseproxy-configure-secure-communication.md#setting-up-client-certificate-authentication-through-the-reverse-proxy). |
...
|DefaultContainerRepositoryPasswordType|Строка, значение по умолчанию — ""|Статические|Если строка не пустая, значение может быть "encrypted" или "Секретссторереф".|
...
|преферредпримаридомаинсконстраинтприорити| Целое число, по умолчанию — 1 | Динамический| Определяет приоритет ограничения предпочтительного основного домена: 0 — жесткое; 1: Soft; отрицательный: пропуск |
...
|субклустеринженаблед|Логическое значение, по умолчанию — false | Динамический |Подтверждать подкластеризацию при вычислении стандартного отклонения для балансировки |
|субклустерингрепортингполици| Целое число, по умолчанию — 1 |Динамический|Определяет, как и при отправке отчетов о работоспособности подкластеров: 0: не сообщать; 1: предупреждение; 2: ОК |
...
|усесепаратесекондаримовекост | Логическое значение, по умолчанию — false | Динамический|Параметр, определяющий, следует ли использовать отдельные затраты на перемещение для вторичных реплик. |
...
| **Параметр** | **Допустимые значения** |**Политика обновления**| **Руководство или краткое описание** |
| --- | --- | --- | --- |
|AADCertEndpointFormat|Строка, значение по умолчанию — ""|Статические|Формат конечной точки сертификата AAD, коммерческая служба Azure по умолчанию, указана для среды не по умолчанию,\/такой как{0}Azure для государственных организаций "https:/login.microsoftonline.US//FederationMetadata/2007-06/FederationMetadata.XML". |
...
| енфорцелинуксминтлсверсион | Логическое значение, по умолчанию — false | Статические | Если задано значение true; поддерживается только TLS версии 1.2 +.  Значение false; Поддержка более ранних версий TLS. Применимо только к Linux |
| енфорцепревалидатиононсекуритичанжес | Логическое значение, по умолчанию — false| Динамический | Установите флаг управления поведением обновления кластера при обнаружении изменений параметров безопасности. Если задано значение "true", то при обновлении кластера будет предпринята попытка убедиться, что по крайней мере один из сертификатов, соответствующих любому из правил представления, может передать соответствующее правило проверки. Предварительная проверка выполняется до применения новых параметров к любому узлу, но выполняется только на узле, где размещается первичная реплика службы диспетчера кластеров на момент инициации обновления. Значение по умолчанию в настоящий момент равно false; начиная с версии 7,1 для новых кластеров Azure Service Fabric параметру будет присвоено значение true.|
...
|TLS1_2_CipherList| строка| Статические|Если задана непустая строка; переопределяет поддерживаемый список шифров для TLS 1.2 и ниже. См. документацию "OpenSSL-ciphers" для получения списка поддерживаемых шифров и пример формата списка строгого списка шифров для TLS 1.2: "ECDHE-ECDSA-AES256-GCM-SHA384: ECDHE-ECDSA-AES128-GCM-SHA256: ECDHE-RSA-AES256-GCM-SHA384: ECDHE-RSA-AES-128-GCM-SHA256: ECDHE-ECDSA-AES256-CBC-: SHA384-ECDSA-ECDHE-CBC-SHA256: AES128-RSA-ECDHE-CBC-SHA256 — применяется только к Linux. |

## <a name="securityadminclientx509names"></a>Security/AdminClientX509Names

| **Параметр** | **Допустимые значения** | **Политика обновления** | **Руководство или краткое описание** |
| --- | --- | --- | --- |
|PropertyGroup|X509NameMap, по умолчанию — None|Динамический|Это список пар "имя" и "значение". Каждое имя относится к общему имени или имени DnsName сертификатов X509, разрешенных для операций клиента администратора. Для данного имени "значение" представляет собой разделенный запятыми список отпечатков сертификатов для закрепления издателя, если он не пуст, прямой издатель сертификатов клиента администратора должен находиться в списке. |
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Руководство или краткое описание** |
| --- | --- | --- | --- |
|PropertyGroup|X509NameMap, по умолчанию — None|Динамический|Это список пар "имя" и "значение". Каждое имя относится к общему имени или имени DnsName сертификатов X509, разрешенных для операций клиента. Для заданного "имени" значение представляет собой разделенный запятыми список отпечатков сертификатов для закрепления издателя, если он не пуст, прямой издатель сертификатов клиента должен находиться в списке.|
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Руководство или краткое описание** |
| --- | --- | --- | --- |
|PropertyGroup|X509NameMap, по умолчанию — None|Динамический|Это список пар "имя" и "значение". Каждое имя относится к общему имени или имени DnsName сертификатов X509, разрешенных для операций кластера. Для заданного "имени" значение является разделенным запятыми списком отпечатков сертификатов для закрепления издателя, если он не пуст, прямой издатель сертификатов кластера должен находиться в списке.|
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Руководство или краткое описание** |
| --- | --- | --- | --- |
|PropertyGroup|X509NameMap, по умолчанию — None|Динамический|Это список пар "имя" и "значение". Каждое имя относится к общему имени или имени DnsName сертификатов X509, разрешенных для операций сервера. Для заданного "имени" значение представляет собой разделенный запятыми список отпечатков сертификатов для закрепления издателя, если он не пуст, прямой издатель сертификатов сервера должен находиться в списке.|
