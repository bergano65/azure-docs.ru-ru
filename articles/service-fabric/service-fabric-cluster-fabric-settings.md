...
|SecretEncryptionCertX509StoreName|Строка, значение по умолчанию — "My"|   Динамический|    Это означает, что сертификат используется для шифрования и дешифрования учетных записей. Название хранилища сертификатов X.509, которое используется для шифрования и расшифровки учетных данных хранилища, используемых службой резервного восстановления |
...
|MaxTimeoutRetryBuffer | Время в секундах, по умолчанию — 600 |Динамический|Временной диапазон в секундах. Максимальное время ожидания операции при внутренней повторной попытке из-за истечения времени ожидания `<Original Time out> + <MaxTimeoutRetryBuffer>`. Дополнительное время ожидания добавляется с шагом в значение параметра MinOperationTimeout. |
...
|комплетеклиентрекуест | Логическое значение, по умолчанию — false |Динамический| Завершите запрос клиента, если он принят CM. |
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|админонлихттпаудит |Логическое значение, по умолчанию — true | Динамический | Исключите HTTP-запросы, которые не влияют на состояние кластера от аудита. Находится исключаются только запросы типа "GET"; но это может быть изменено. |
...
|аудисттпрекуестс |Логическое значение, по умолчанию — false | Динамический | Включите или отключите аудит HTTP. Аудит предназначен для просмотра действий, выполненных в кластере. включая тех, кто инициировал запрос. Обратите внимание, что это лучшее ведение журнала. и могут произойти потери данных. HTTP-запросы с проверкой подлинности "User" не записываются. |
|каптурехттптелеметри|Логическое значение, по умолчанию — true | Динамический | Включить или отключить телеметрию HTTP. Целью телеметрии является Service Fabric возможность захвата данных телеметрии для планирования будущей работы и выявления проблемных областей. Данные телеметрии не записывают никаких персональных данных или текста запроса. Данные телеметрии фиксируют все HTTP-запросы, если иное не настроено. |
|ClusterId |String | Динамический |Уникальный идентификатор кластера. Он создается при создании кластера. |
|ConsumerInstances |String | Динамический |Список экземпляров потребителя приложения для устройства. |
...
|фаилуресонлихттптелеметри | Логическое значение, по умолчанию — false | Динамический | Значение, если включено отслеживание данных телеметрии HTTP. захватить только неудачные запросы. Это позволит сократить количество событий, создаваемых для телеметрии. |
|хттптелеметрикаптуреперцентаже | целое число, по умолчанию — 50 | Динамический | Значение, если включено отслеживание данных телеметрии HTTP. захватить только случайный процент запросов. Это позволит сократить количество событий, создаваемых для телеметрии. |
|MaxDiskQuotaInMB |Целое число, по умолчанию — 65536 | Динамический |Квота диска (в МБ) для файлов журнала Windows Fabric. |
|ProducerInstances |String | Динамический |Список экземпляров производителя приложения для устройства. |
...
|HealthReportRetrySendInterval |Время в секундах, по умолчанию — 30, минимальное — 1 |Динамический|Временной диапазон в секундах. Интервал времени, в течение которого компонент отчетов повторно отправляет накопленные отчеты о работоспособности в Диспетчер работоспособности. |
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|алловнодестатеремоведфорсидноде|Логическое значение, по умолчанию — false |Динамический|Флаг, указывающий, разрешено ли удаление состояния узла для начального узла |
...
| сиднодекуорумаддитионалбуффернодес | Целое число, по умолчанию — 0 | Динамический | Буфер узлов начальных значений, которые должны быть готовы (вместе с кворумом узлов начального уровня) FM, должен допускать отработку узлов с максимальным (Тоталнумсиднодес-(Сиднодекуорум + Сиднодекуорумаддитионалбуффернодес)) начальным значением. |
...
|дискспацехеалсрепортингинтервалвхененаугхдискспаце |Интервал времени, по умолчанию — Common::TimeSpan::FromMinutes(15)|Динамический|Временной диапазон в секундах. Интервал времени между проверками места на диске для события работоспособности при наличии достаточного места на диске. |
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|аутоматикунпровисионинтервал|Интервал времени, по умолчанию — Common:: TimeSpan:: FromMinutes (5)|Динамический|Временной диапазон в секундах. Интервал очистки для отмены регистрации типа приложения во время очистки автоматического типа приложения.|
...
|максунуседапптипеверсионстокип | Целое число, по умолчанию — 3 |Динамический|Эта конфигурация определяет количество неиспользуемых версий типов приложений, которые должны быть пропущены для очистки. Этот параметр применим только в том случае, если параметр Клеанупунуседаппликатионтипес включен. |
...
| **Параметр** | **Допустимые значения** |**Политика обновления**| **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|PropertyGroup|KeyDoubleValueMap, по умолчанию — None|Динамический|Определяет часть нагрузки, которая фиксируется с репликой при переключении, принимает значение от 0 (загрузка не зависит от реплики) и 1 (Загрузка загрузок с репликой по умолчанию) |
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|Counters |String | Динамический |Разделенный запятыми список счетчиков производительности, данные которых необходимо собирать. |
...
|валидатепримариплацементконстраинтонпромоте| Логическое значение, по умолчанию — true |Динамический|Указывает, оценивается ли выражение PlacementConstraint для службы для первичной настройки при отработке отказа. |
...
|ноделоадсоператионалтраЦинженаблед | Логическое значение, по умолчанию — true |Динамический|Файл конфигурации, позволяющий загрузить в хранилище событий операционную трассировку с помощью узла. |
|ноделоадсоператионалтраЦингинтервал | Интервал времени, по умолчанию — Common:: TimeSpan:: FromSeconds (20) | Динамический|Временной диапазон в секундах. Интервал, с которым производится трассировка загрузки узла в хранилище событий для каждого домена службы. |
...
|акцептекспиредпиннедклустерцертификате|Логическое значение, по умолчанию — false|Динамический|Флаг, указывающий, следует ли принимать просроченные сертификаты кластера, объявленные с помощью отпечатка, применяются только к сертификатам кластера. чтобы поддерживать работу кластера в активном состоянии. |
...
|жетфолдерсизе |Строка, значение по умолчанию — "Admin" |Динамический|Настройка безопасности для получения размера папки внесения |
...
|MoveNextFabricUpgradeDomain |Строка, значение по умолчанию — "Admin" |Динамический| Настройка безопасности для возобновления обновления кластера с помощью явного домена обновления. |
