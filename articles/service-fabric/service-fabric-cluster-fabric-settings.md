...
|ExpectedNodeDeactivationDuration|Интервал времени, значение по умолчанию — Common::TimeSpan::FromSeconds(60.0 \* 30)|Динамический|Временной диапазон в секундах. Ожидаемая длительность завершения деактивации на узле. |
|ExpectedNodeFabricUpgradeDuration|Интервал времени, значение по умолчанию — Common::TimeSpan::FromSeconds(60.0 \* 30)|Динамический|Временной диапазон в секундах. Ожидаемая длительность обновления узла во время обновления Windows Fabric. |
|ExpectedReplicaUpgradeDuration|Интервал времени, значение по умолчанию — Common::TimeSpan::FromSeconds(60.0 \* 30)|Динамический|Временной диапазон в секундах. Ожидаемая длительность обновления всех реплик на узле во время обновления приложения. |
...
|ReplicaRestartWaitDuration|Интервал времени, значение по умолчанию — Common::TimeSpan::FromSeconds(60.0 \* 30)|Не разрешено|Временной диапазон в секундах. Время ожидания перед повторным запуском реплики для службы диспетчера отработки отказов. |
...
|StandByReplicaKeepDuration|Интервал времени, значение по умолчанию — Common::TimeSpan::FromSeconds(3600.0 \* 24 \* 7)|Не разрешено|Временной диапазон в секундах. Время хранения реплики, находящейся в режиме ожидания, для службы диспетчера отработки отказов. |
...
|CleanupApplicationPackageOnProvisionSuccess|Логическое значение, по умолчанию — false |Динамический|Включает или отключает автоматическую очистку пакета приложения при успешном выполнении.

*Рекомендуется использовать `true` .* | | Клеанупунуседаппликатионтипес | Bool, по умолчанию — FALSE | Динамический | Эта конфигурация, если она включена, позволяет автоматически отменять регистрацию неиспользуемых версий типов приложений, пропуская последние три неиспользованные версии, таким образом обрезая дисковое пространство, занимаемое хранилищем образов. Автоматическая очистка запускается в конце успешной подготовки для конкретного типа приложения, а также периодически выполняется один раз в день для всех типов приложений. Количество неиспользуемых версий для пропуска можно настроить с помощью параметра MaxUnusedAppTypeVersionsToKeep. 

*Рекомендуется использовать `true` .*
| | Дисаблечекксумвалидатион | Bool, по умолчанию — false | Статический | Эта конфигурация позволяет включить или отключить проверку контрольной суммы во время подготовки приложения. | | Дисаблесерверсидекопи | Bool, по умолчанию — false | Статический | Эта конфигурация включает или отключает копию пакета приложения на стороне сервера в ImageStore во время подготовки приложения. | | Имажекачинженаблед | Bool, значение по умолчанию — true | Статический | Эта конфигурация позволяет включить или отключить кэширование. | | ImageStoreConnectionString | SecureString | Статический | Строка подключения к корню для ImageStore. | | Имажестореминимумтрансфербпс | Целое число, по умолчанию — 1024 | Динамический | Минимальная скорость передачи между кластером и ImageStore. Это значение не определяет время ожидания при доступе к внешнему хранилищу образов. Измените его только при высокой задержке между кластером и хранилищем образов. Это позволит выделить кластеру больше времени на скачивание из внешнего хранилища образов. | | Максунуседапптипеверсионстокип | Целое число, по умолчанию — 3 | Динамический | Эта конфигурация определяет количество неиспользуемых версий типов приложений, которые должны быть пропущены для очистки. Этот параметр применим только в том случае, если параметр CleanupUnusedApplicationTypes включен.

*Рекомендуется использовать значение по умолчанию ( `3` ).*|
...
|UseSeparateSecondaryMoveCost | Логическое значение, по умолчанию — false | Динамический|Параметр, определяющий необходимость использования отдельных затрат на перемещение для вторичных реплик. |
