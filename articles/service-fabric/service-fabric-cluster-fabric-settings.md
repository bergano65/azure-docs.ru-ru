...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|AdminOnlyHttpAudit |Логическое значение, по умолчанию — true | Динамический | Исключите HTTP-запросы, которые не влияют на состояние кластера из аудита. В настоящее время; исключаются только запросы типа «Получить»; но это может быть изменена. |
...
|AuditHttpRequests |Логическое значение, по умолчанию — false | Динамический | Включите или выключить аудит HTTP. Аудит предназначена для просмотра действий, выполненных для кластера. в том числе кто инициировал запрос. Обратите внимание, что это лучший ведения журнала попытки; и может привести к потере трассировки. HTTP-запросов с проверкой подлинности «User», отсутствуют. |
|CaptureHttpTelemetry|Логическое значение, по умолчанию — false | Динамический | Отключение телеметрии HTTP. Данные телеметрии предназначена для Service Fabric иметь возможность собирать данные телеметрии по планированию будущей работе и определение областей проблем. Данные телеметрии не записывает любые персональные данные и тексте запроса. Данные телеметрии захватывает все HTTP-запросы, если иное не настроено в противном случае. |
...
|FailuresOnlyHttpTelemetry | Логическое значение, по умолчанию — true | Динамический | Если включен сбор данных телеметрии HTTP; Захват только неудачных запросов. Это помогает сократить число событий, создаваемых для телеметрии. |
|HttpTelemetryCapturePercentage | Целое число, по умолчанию — 50 | Динамический | Если включен сбор данных телеметрии HTTP; Запишите случайный процент запросов. Это помогает сократить число событий, создаваемых для телеметрии. |
...
|HealthReportRetrySendInterval |Время в секундах, по умолчанию — 30, минимальное значение — 1 |Динамический|Временной диапазон в секундах. Интервал, по которому компонент создания отчетов повторно отправляет накопленные работоспособности сообщает диспетчеру работоспособности. |
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|AutomaticUnprovisionInterval|Интервал времени, по умолчанию — Common::TimeSpan::FromMinutes(5)|Динамический|Временной диапазон в секундах. Интервал очистки для разрешенных для отмены регистрации типа приложения во время очистки автоматическое применение типа.|
...
|CleanupApplicationPackageOnProvisionSuccess|Логическое значение, по умолчанию — false |Динамический|Включает или отключает автоматическую очистку пакета приложения в успешной подготовки. |
|CleanupUnusedApplicationTypes|Логическое значение, по умолчанию — false |Динамический|Эта конфигурация Если параметр включен, позволяет автоматически отменить регистрацию версии типа неиспользуемые приложения, пропуск последние три версии неиспользуемые, тем самым сокращение места на диске, занимаемое в хранилище образов. Автоматическая очистка будет запускаться в конце успешной подготовки для этого типа для конкретного приложения и также выполняется периодически один раз в день для всех типов приложений. Число неиспользуемых версий для пропуска настраивается с помощью параметра «MaxUnusedAppTypeVersionsToKeep». |
...
|MaxUnusedAppTypeVersionsToKeep | Целое число, по умолчанию — 3 |Динамический|Эта конфигурация определяет число версий типов неиспользуемые приложения пропуск для очистки. Этот параметр применим только в том случае, если включен параметр CleanupUnusedApplicationTypes. |
...
|Delete (Удалить) |Строка, значение по умолчанию — "Admin" |Динамический| Настройки безопасности для операции удаления клиента хранилища образов. |
