...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|GlobalTicketLeaseDuration|Интервал времени, по умолчанию — Common::TimeSpan::FromSeconds(300)|Статическое|Временной диапазон в секундах. Узлы в кластере должны использовать глобальную аренду с участниками кворума. Участники кворума отправляют свои глобальные аренды для распространения во всем кластере на этот период. Если это время истекает, аренда теряется. Потеря кворума аренды приводит к тому, что узел сбрасывает кластер. При этом прекращается обмен данными с кворумом узлов на этот период времени.  Это значение необходимо изменить в зависимости от размера кластера. |
