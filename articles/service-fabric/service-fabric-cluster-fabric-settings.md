...
|SecureOnlyMode|Логическое значение, по умолчанию — false|Динамический| SecureOnlyMode. true: обратный прокси-сервер выполняет переадресацию только в те службы, которые публикуют безопасные конечные точки. false: обратный прокси-сервер может переадресовывать запросы к защищенным и незащищенным конечным точкам. Дополнительные сведения см. в разделе о [логике выбора конечной точки обратного прокси-сервера](service-fabric-reverseproxy-configure-secure-communication.md#endpoint-selection-logic-when-services-expose-secure-as-well-as-unsecured-endpoints).  |
...
|каптурехттптелеметри|Логическое значение, по умолчанию — false | Динамический | Включить или отключить телеметрию HTTP. Целью телеметрии является Service Fabric возможность захвата данных телеметрии для планирования будущей работы и выявления проблемных областей. Данные телеметрии не записывают никаких персональных данных или текста запроса. Данные телеметрии фиксируют все HTTP-запросы, если иное не настроено. |
|ClusterId |Строковое | Динамический |Уникальный идентификатор кластера. Он создается при создании кластера. |
|ConsumerInstances |Строковое | Динамический |Список экземпляров потребителя приложения для устройства. |
...
|фаилуресонлихттптелеметри | Логическое значение, по умолчанию — true | Динамический | Значение, если включено отслеживание данных телеметрии HTTP. захватить только неудачные запросы. Это позволит сократить количество событий, создаваемых для телеметрии. |
...
|ProducerInstances |Строковое | Динамический |Список экземпляров производителя приложения для устройства. |
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|Enabled |Логическое значение, по умолчанию — false |Статические|Включенный флаг для службы хранилища образов. Значение по умолчанию — false |
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
|Counters |Строковое | Динамический |Разделенный запятыми список счетчиков производительности, данные которых необходимо собирать. |
...
|енфорцеусерсервицеметриккапаЦитиес|Логическое значение, по умолчанию — false | Статические |Включает защиту служб структуры. все пользовательские службы находятся в одном объекте задания/кграуп и ограничены указанным объемом ресурсов. это необходимо сделать статическим (требуется перезапуск FabricHost) в качестве создания или удаления объекта задания пользователя и установки ограничений на работу во время открытия Узла структуры |
...
|PreferredLocationConstraintPriority | Целое число, по умолчанию — 2| Динамический|Определяет приоритет ограничения предпочтительного расположения. 0 — строгое ограничение; 1 — нестрогое ограничение; 2 — оптимизация; отрицательное значение — Пропустить |
...
|AADTokenEndpointFormat|Строка, значение по умолчанию — ""|Статические|Конечная точка токена AAD, коммерческая служба Azure по умолчанию, указана для среды не по умолчанию,{0}такой как Azure для государственных организаций "https:\//Login.microsoftonline.US/" |
...
|ClientClaims|Строка, значение по умолчанию — ""|Динамический|Все возможные утверждения, ожидаемые от клиентов для подключения к шлюзу. В этом списке используется оператор ИЛИ: ClaimsEntry \|\| ClaimsEntry \|\| ClaimsEntry... Каждый элемент ClaimsEntry представляет собой список с операторами И: ClaimType=ClaimValue && ClaimType=ClaimValue && ClaimType=ClaimValue... |
...
|Оператор delete |Строка, значение по умолчанию — "Admin" |Динамический| Настройки безопасности для операции удаления клиента хранилища образов. |
...
|Отправьте |Строка, значение по умолчанию — "Admin" | Динамический|Настройка безопасности для операции передачи клиента хранилища образов. |
...
|FabricDataRoot |Строковое | Не разрешено |Корневой каталог данных Service Fabric. По умолчанию для Azure это d:\svcfab. |
|FabricLogRoot |Строковое | Не разрешено |Корневой каталог журналов Service Fabric. В нем хранятся журналы и трассировки Service Fabric. |
...
|ServiceRunAsAccountName |Строковое | Не разрешено |Имя учетной записи, используемой для запуска службы узла Fabric. |
