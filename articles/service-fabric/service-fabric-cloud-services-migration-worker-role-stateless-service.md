...
По существу рабочая роль представляет собой рабочую нагрузку без отслеживания состояния (все экземпляры такой рабочей нагрузки идентичны, а запросы могут передаваться на любой экземпляр в любое время). Ни один из экземпляров обычно не помнит предыдущие запросы. Состояние, в котором функционирует рабочая нагрузка на управляет внешнее хранилище состояний, например хранилище таблиц Azure или Azure Cosmos DB. В платформе Service Fabric рабочей нагрузке этого типа соответствует служба без отслеживания состояния. Перенести рабочую роль на платформу Service Fabric проще всего так: следует преобразовать код рабочей роли в код службы без отслеживания состояния.
...
* **Жизненный цикл:** Главное отличие — что рабочая роль — это виртуальная машина, и поэтому ее жизненный цикл привязан к виртуальной Машине, которая включает в себя события запуска и останавливает виртуальную Машину. Жизненный цикл службы Service Fabric отделен от жизненного цикла виртуальной машины и поэтому не включает в себя события запуска и остановки (основной) виртуальной машины.
* **Время существования:** Осуществляет очистку экземпляра рабочей роли, если `Run` метод завершает работу. Тем не менее метод `RunAsync` в службе Service Fabric может выполняться столько, сколько нужно, и экземпляр службы выключаться не будет. 
