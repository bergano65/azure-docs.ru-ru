...
![два разных макета кластера][Image2]
</center>
...
### <a name="default-approach"></a>Подход по умолчанию
По умолчанию кластер диспетчер ресурсов поддерживает распределение служб между доменами сбоя и обновления. Это моделируется как [ограничение](service-fabric-cluster-resource-manager-management-integration.md). Ограничение для доменов сбоя и обновления: "для данной секции службы не должно быть разницы больше единицы в количестве объектов службы (экземпляры службы без отслеживания состояния или реплики службы с отслеживанием состояния) между любыми двумя доменами в одном уровень иерархии ".
...
Service Fabric ожидать, что в некоторых случаях может потребоваться выполнение определенных рабочих нагрузок в определенных конфигурациях оборудования. Например:
...
![различные рабочие нагрузки для][Image5]
макета кластера </center>
...
Например, можно написать ограничение на размещение `"(NodeType == NodeType03)"`. **NodeType** — это часто используемое свойство. Это полезно, так как оно соответствует 1:1 с типом компьютера. Каждый тип компьютера соответствует типу рабочей нагрузки в традиционном n-уровневом приложении.
...
Предположим, что для типа узла в ClusterManifest. XML определены следующие свойства узла:
...
В следующем примере показаны свойства узла, определенные с помощью ClusterConfig. JSON для автономных развертываний или Template. JSON для кластеров, размещенных в Azure. 

> [!NOTE]
> В шаблоне Azure Resource Manager тип узла обычно является параметризованным. Он будет выглядеть так `"[parameters('vmNodeType1Name')]"`, а не NodeType01.
>
...
Если все узлы NodeType01 являются допустимыми, можно также выбрать тип узла с ограничением `"(NodeType == NodeType01)"`.
...
Ниже приведен пример емкости, определяемой с помощью ClusterConfig. JSON для автономных развертываний или файла Template. JSON для кластеров, размещенных в Azure: 
...
Ниже приведен пример того, как можно указать буферизованную емкость через ClusterConfig. JSON для автономных развертываний или Template. JSON для кластеров, размещенных в Azure:
