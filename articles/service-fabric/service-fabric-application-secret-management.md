---
title: Управление секретами приложений службы Azure Fabric
...
* [Настройка сертификата шифрования и шифрование секретов в кластерах Linux.][secret-management-linux-specific-link]
...
Секреты также должны быть включены в приложение Service Fabric, указав сертификат в манифесте приложения. Добавьте элемент **SecretsCertificate** в **ApplicationManifest.xml** и включите отпечаток пальца желаемого сертификата.
...
```
> [!NOTE]
> После активации приложения, которое указывает SecretsCertificate, Service Fabric найдет соответствующий сертификат и предоставит удостоверение, которое приложение работает под полным разрешением на личный ключ сертификата. Service Fabric также будет следить за сертификатом за изменениями и соответствующим образом повторно применять разрешения. Чтобы обнаружить изменения для сертификатов, заявленных общим именем, Service Fabric выполняет периодическую задачу, которая находит все соответствующие сертификаты и сравнивает ее с кэшированным списком отпечатков пальцев. При обнаружении нового отпечатка пальца это означает, что сертификат по этому объекту был обновлен. Задача выполняется один раз в минуту на каждом узлах кластера.
>
> В то время как SecretsCertificate позволяет делать заявления на основе объектов, обратите внимание, что зашифрованные настройки привязаны к ключевой паре, которая была использована для шифрования настройки на клиенте. Необходимо убедиться, что исходный сертификат шифрования (или эквивалент) соответствует заявлению на основе предмета и установлен, включая соответствующий закрытый ключ, на каждый узла кластера, который может разместить приложение. Все сертификаты, соответствующие действительности, соответствующие действительности декларации и построенные из той же пары ключей, что и исходный сертификат шифрования, считаются эквивалентами.
>
...
* Сервис Ткань [Секреты Store](service-fabric-application-secret-store.md) 
