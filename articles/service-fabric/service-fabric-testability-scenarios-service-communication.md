---
title: 'Тестируемость: Сервисная связь'
...
* Каждая служба, к которой можно подключиться, имеет *адрес* для прослушивания (например, HTTP или WebSockets). При перемещении экземпляра или раздела службы меняется адрес соответствующей конечной точки (Он перемещается в другой узла с другим IP-адресом.) Если вы используете встроенные компоненты связи, они будут обрабатывать повторно разрешающие адреса службы для вас.
