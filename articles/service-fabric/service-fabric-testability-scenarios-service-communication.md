...
* Каждая служба, к которой можно подключиться, имеет *адрес* для прослушивания (например, HTTP или WebSockets). При перемещении экземпляра или раздела службы меняется адрес соответствующей конечной точки (Он перемещается на другой узел с другим IP-адресом.) Если вы используете встроенные компоненты связи, они будут поддерживать повторное разрешение адресов служб.
