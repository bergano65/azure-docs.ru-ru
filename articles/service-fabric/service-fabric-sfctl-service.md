---
title: Служба обслуживания Azure Fabric CLI- sfctl
description: Узнайте о sfctl, интерфейсе командной строки Azure Service Fabric. Включает список команд для управления службами, типами служб и пакетами услуг.
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
| --move-cost | Указывает стоимость перемещения для службы. Возможные значения:\: "Нулевой", "Низкий", "Средний", "Высокий", "Очень высокий". |
...
| --placement-policy-list | Список политик размещения для службы в формате JSON, а также связанные доменные имена. Политики могут быть одним\: `NonPartiallyPlaceService` `PreferPrimaryDomain`или `RequireDomain` `RequireDomainDistribution`более , , . |
...
| --сервис-место размещения | Продолжительность, в течение которой реплики могут оставаться InBuild до сообщения о том, что сборка застряла. Это значение относится только к службам без отслеживания состояния. |
