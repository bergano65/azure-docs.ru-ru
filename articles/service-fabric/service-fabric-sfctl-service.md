---
title: Azure Service Fabric CLI — служба sfctl
description: Сведения о sfctl, интерфейсе командной строки Azure Service Fabric. Содержит список команд для управления службами, типами служб и пакетами служб.
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
| --move-cost | Указывает стоимость перемещения для службы. Возможные значения:\: "Zero", "Low", "Medium", "High", "Верихигх". |
...
| --placement-policy-list | Список политик размещения для службы в формате JSON, а также связанные доменные имена. Политики могут быть одной или несколькими\: `NonPartiallyPlaceService`, `PreferPrimaryDomain`, `RequireDomain`, `RequireDomainDistribution`. |
...
| --Service-время размещения | Время, в течение которого реплики могут оставаться в процессе сборки, прежде чем сообщает о зависании сборки. Это значение относится только к службам без отслеживания состояния. |
