...
| --move-cost | Указывает стоимость перемещения для службы. Возможные значения: \: "0", "Low", "Medium", "High", "верихигх". |
...
| --placement-policy-list | Список политик размещения для службы в формате JSON, а также связанные доменные имена. Политика может иметь одно или несколько из \: `NonPartiallyPlaceService` , `PreferPrimaryDomain` , `RequireDomain` , `RequireDomainDistribution` . |
