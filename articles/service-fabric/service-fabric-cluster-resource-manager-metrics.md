...
| --- | --- | --- | --- | --- |
| PrimaryCount |0 |0 |1 |Высокая |
| ReplicaCount |0 |1 |1 |Средние |
| Количество |1 |1 |1 |Низкая |
...
Метрики по умолчанию прекрасно подходят для начала работы. Однако на этом их возможности исчерпываются. Пример: Какова вероятность того, что выбранная схема секционирования приведет равномерному идеально все секции? Какова вероятность того, что заданная служба испытывает одинаковую нагрузку все время или даже во всех секциях прямо сейчас?
...
* Вес: Вес метрики определяет, насколько важным этой метрики по сравнению с другими метриками для этой службы.
* Нагрузка по умолчанию: Нагрузка по умолчанию представляется по-разному в зависимости от того, является ли служба без отслеживания состояния или с отслеживанием состояния.
  ...
  * Для служб с отслеживанием состояния можно определить следующие свойства:
    * PrimaryDefaultLoad: Величина этой метрики по умолчанию при первичной потребляет этой службы
    * SecondaryDefaultLoad — Величина этой метрики по умолчанию при базу данных-получатель потребляет этой службы
...
![Пример веса метрики и его влияние на решения балансировки нагрузки][Image3]
</center>
...
![Влияние решения только глобальной балансировки][Image4]
</center>
