---
title: Управление загрузкой приложений Service Fabric Azure с помощью метрик
...
Весь смысл определения метрики заключается в том, чтобы представить определенную нагрузку. *Нагрузка* — это количество единиц определенной метрики, используемое экземпляром или репликой службы на заданном узле. Нагрузку можно настроить практически в любой момент. Пример:
  ...
  - Можно определить нагрузку при создании службы. Этот тип конфигурации нагрузки называется _загрузкой по умолчанию_.
  - Сведения о метриках, включая загрузки по умолчанию, для службы можно обновить после создания службы. Это обновление метрики выполняется путем _обновления службы_.
  - Нагрузки для заданной секции можно сбросить, восстановив значения по умолчанию для этой службы. Это обновление метрики называется _сбросом загрузки секции_.
  - Можно получать отчеты о нагрузке для каждого объекта службы, динамически во время выполнения. Это обновление метрики называется _выгрузкой отчетов_.
  - Загрузить реплики или экземпляры секции можно также с помощью вызова API структуры для создания отчетов по значениям нагрузки. Это обновление метрики называется _отчетной нагрузкой для секции_.
...
## <a name="reporting-load-for-a-partition"></a>Создание отчетов о нагрузке для секции
В предыдущем разделе описано, как реплики служб или экземпляры сообщают о своей нагрузке. Существует дополнительный параметр для динамического отчета о нагрузке с помощью FabricClient. При отправке отчета о загрузке для секции можно создать отчет о нескольких секциях одновременно.

Эти отчеты будут использоваться точно так же, как и при загрузке отчетов, поступающих из реплик или самих экземпляров. Сообщаемые значения будут действительны до тех пор, пока не будут переданы новые значения загрузки репликой или экземпляром или сообщением о новом значении нагрузки для секции.

С помощью этого API существует несколько способов обновления нагрузки в кластере.

  - Раздел службы с отслеживанием состояния может обновить свою основную реплику нагрузки.
  - Службы с отслеживанием состояния и без отслеживания состояния могут обновлять нагрузку всех вторичных реплик или экземпляров.
  - Службы с отслеживанием состояния и без отслеживания состояния могут обновлять загрузку определенной реплики или экземпляра на узле.

Можно также одновременно объединить любое из этих обновлений на секцию.

Обновление нагрузок для нескольких секций возможно с помощью одного вызова API. в этом случае выходные данные будут содержать ответ на секцию. Если обновление секции не было успешно применено по какой-либо причине, обновления для этой секции будут пропущены, и будет предоставлен соответствующий код ошибки для целевой секции:

  - Партитионнотфаунд — указанный идентификатор секции не существует.
  - Реконфигуратионпендинг-Partition в данный момент перестраивается.
  - Инвалидфорстателесссервицес — предпринята попытка изменить загрузку первичной реплики для секции, принадлежащей службе без отслеживания состояния.
  - Репликадоеснотексист — вторичная реплика или экземпляр не существует на указанном узле.
  - InvalidOperation — может произойти в двух случаях: обновление нагрузки для секции, принадлежащей системному приложению, или обновление прогнозируемой нагрузки не включено.

Если возвращаются некоторые из этих ошибок, можно обновить входные данные для определенной секции и повторить обновление для определенной секции.
...
В этом примере будет выполнено обновление последней загруженной нагрузки для секции _53df3d7f-5471-403b-b736-bde6ad584f42_. Загрузка первичной реплики для метрики _CustomMetricName0_ будет обновлена значением 100. В то же время загрузка одной и той же метрики для определенной вторичной реплики, расположенной на узле _NodeName0_, будет обновлена со значением 200.
...
![Пример веса метрики и его влияние на решения балансировки][Image3]
</center>
...
- Определение метрик дефрагментации — это один из способов консолидации нагрузки на узлах, а не их распространения. Дополнительные сведения о настройке дефрагментации см. в [этой статье](service-fabric-cluster-resource-manager-defragmentation-metrics.md) .
