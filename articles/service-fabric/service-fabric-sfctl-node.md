...
Начиная с Service Fabric 6.5, чтобы использовать этот командлет для начальных узлов, измените параметр начальных узлов регулярных узлы (не начальное значение), а затем вызвать этот командлет для удаления состояния узла. Если кластер работает в Azure, после начальный узел выйдет из строя, Service Fabric будет пытаться автоматически изменить его на узел без начального значения. Чтобы это произошло, убедитесь, что количество узлов не начальное значение в тип первичного узла является не меньше числа вниз начальных узлов. При необходимости добавьте дополнительные узлы для типа первичного узла, чтобы добиться этого. Для автономного кластера, если начальный узел вниз не должен возвращаться к работе с ее состоянием без изменений, выполните удалить узел из кластера, см. в разделе [удалить узлы из изолированного кластера Service Fabric](/azure/service-fabric/service-fabric-cluster-windows-server-add-remove-nodes) 
