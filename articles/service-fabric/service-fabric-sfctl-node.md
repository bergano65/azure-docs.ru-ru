...
|Command|Описание|
| --- | --- |
| отключить | Деактивирует узел кластера Service Fabric с указанной целью деактивации. |
...
Начиная с Service Fabric 6,5, чтобы использовать этот командлет для начальных узлов, измените начальные узлы на обычные (не начальные) узлы, а затем вызовите этот командлет, чтобы удалить состояние узла. Если кластер работает в Azure, после отключения начального узла Service Fabric попытается автоматически изменить его на узел, не являющийся начальным. Чтобы это произошло, убедитесь, что количество неначальных узлов в типе первичного узла не меньше количества начальных узлов. При необходимости добавьте дополнительные узлы в тип первичного узла, чтобы добиться этого. Если для автономного кластера не ожидается, что узел начального значения не будет восстановлен с состоянием без изменений, удалите узел из кластера, раздел [Удаление узлов из Service Fabric изолированного кластера](/azure/service-fabric/service-fabric-cluster-windows-server-add-remove-nodes) . 
