...
> [!NOTE]
> — Поддерживаемый сценарий по вертикали операции масштабирования: Можно перенести Мои кластера Service Fabric и приложения из неуправляемого диска на управляемые диски без простоя приложения. Подготовив виртуальную машину масштабируемых наборов с управляемыми дисками и выполняет обновление приложения с ограничениями на размещение, предназначенных подготовленная емкость; кластер Service Fabric можно запланировать рабочей нагрузки подготовленного кластера емкости узла, которая разворачивается по доменам обновления без простоя приложения. [Azure SKU Basic подсистемы балансировки нагрузки](https://docs.microsoft.com/azure/load-balancer/load-balancer-overview#skus) конечные точки серверного пула может быть виртуальные машины в одну группу доступности или масштабируемый набор виртуальных машин. Это означает, что при перемещении приложения Service Fabric систем между масштабируемых наборов нельзя использовать подсистему балансировки нагрузки SKU "базовый", не вызывая временной недоступности Service Fabric кластера конечная точка управления, даже если кластер и его применение по-прежнему выполняются; Обычно пользователь подсистему балансировки нагрузки уровня "стандартный", при выполнении переключения виртуального IP-адрес (VIP) между ресурсами, базовый SKU балансировки Нагрузки и использует стандартную балансировку Нагрузки SKU, чтобы устранить все будущие приблизительно 30 секунд, из отсутствия доступа, необходимые для переключения виртуального IP-адреса.
