...
* *Память* (имя `servicefabric:/_MemoryInMB`метрики): память выражается в мегабайтах и сопоставляется с физической памятью, доступной на компьютере.
...
* Ограничение использования ресурсов применяется только к `servicefabric:/_CpuCores` метрикам `servicefabric:/_MemoryInMB` ресурсов и
