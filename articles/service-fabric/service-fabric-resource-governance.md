...
* *Память* (метрическое имя `servicefabric:/_MemoryInMB`): Память выражается в мегабайтах, и она отображает физическую память, которая доступна на машине.
...
Для большинства клиентов и сценариев автоматическое обнаружение емкостей узлов для процессора и памяти является рекомендуемой конфигурацией (автоматическое обнаружение включено по умолчанию). Однако, если требуется полная ручная настройка емкостей узлов, можно настроить их на тип узла, используя механизм для описания узлов в кластере. Вот пример того, как настроить тип узла с четырьмя ядрами и 2 ГБ памяти:
...
> [!IMPORTANT]
> Начиная с версии Service Fabric 7.0, мы обновили правило расчета емкости ресурсов узлов в тех случаях, когда пользователь вручную предоставляет значения для емкости ресурсов узлов. Рассмотрим следующий сценарий:
>
> * Всего на узлах 10 ядер cpu
> * SF настроен на использование 80% от общего объема ресурсов для пользовательских служб (настройка по умолчанию), что оставляет буфер 20% для других служб, работающих на узлах (в том же случае службы системы service Fabric)
> * Пользователь решает вручную переопределить емкость ресурса узла для метрики ядер cpu и устанавливает его на 5 ядер
>
> Мы изменили правило о том, как доступная емкость для услуг пользователей Service Fabric рассчитывается следующим образом:
>
> * До того, как Service Fabric 7.0 доступная емкость для пользовательских услуг будет рассчитана до **5 ядер** (буфер емкости 20% игнорируется)
> * Начиная с Service Fabric 7.0, доступная емкость для пользовательских услуг будет рассчитана до **4 ядер** (буфер емкости 20% не игнорируется)
...
В этом примере пакет службы **ServicePackageA** получает одно ядро на узлах, где он размещается. Этот пакет услуг содержит два пакета кода **(CodeA1** и `CpuShares` **CodeA2),** и оба указывают параметр. Пропорция CpuShares 512:256 разделяет ресурсы ядра между двумя пакетами кода.
...
При определении параметров управления ресурсами можно использовать [параметры приложения](service-fabric-manage-multiple-environment-app-configuration.md) для управления несколькими конфигурациями приложений. Ниже приведен пример с использованием параметров приложения.
...
## <a name="enforcing-the-resource-limits-for-user-services"></a>Обеспечение предельных затрат ресурсов для служб пользователей

При применении управления ресурсами к службам Service Fabric гарантирует, что эти службы, управляемые ресурсами, не могут превышать свои ресурсы, многим пользователям по-прежнему необходимо запускать некоторые из своих служб Service Fabric в неуправляемом режиме. При использовании неуправляемых сервисов Service Fabric можно столкнуться с ситуациями, когда «беглые» неуправляемые службы потребляют все доступные ресурсы в узлах Service Fabric, что может привести к серьезным проблемам, таким как:

* Ресурсное голодание других служб, работающих на узлах (включая услуги системы Service Fabric)
* Узлы, окажемые в нездоровом состоянии
* Безответственные AIS управления кластером Service Fabric

Чтобы предотвратить возникновение таких ситуаций, Service Fabric позволяет *обеспечить соблюдение лимитов ресурсов для всех служб пользователей Service Fabric, работающих на узлах* (как регулируемых, так и неуправляемых), чтобы гарантировать, что службы пользователей никогда не будут использовать больше, чем указанное количество ресурсов. Это достигается путем установления значения для enforceUserServiceMetricCapacity конфигурации в разделе PlacementAndLoadBalancing ClusterManifest к истине. Эта настройка отключена по умолчанию.
...
* Соблюдение лимита ресурсов применяется только к метрикам `servicefabric:/_CpuCores` и `servicefabric:/_MemoryInMB` ресурсам
* Обеспечение предельных ресурсов работает только в том случае, если возможности узлов для метрик ресурсов доступны для Service Fabric либо через механизм автоматического обнаружения, либо через пользователей, вручную указывающих возможности узлов (как поясняется в [настройке кластера для включения](service-fabric-resource-governance.md#cluster-setup-for-enabling-resource-governance) раздела управления ресурсами).Если возможности узлов не настроены, возможности обеспечения соблюдения ограниченности ресурсов не могут быть использованы, так как Service Fabric не может знать, сколько ресурсов можно резервировать для пользовательских служб.Service Fabric выдаст предупреждение о вреде для здоровья, если "EnforceUserServiceMetricCapacity" соответствует действительности, но возможности узлов не настроены.
