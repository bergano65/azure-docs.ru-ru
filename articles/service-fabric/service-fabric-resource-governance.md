...
Чтобы предотвратить возникновение таких ситуаций, Service Fabric позволяет *применять ограничения ресурсов для всех Service Fabric пользовательских служб, выполняемых на узле* (как управляемые, так и не управляемые), чтобы гарантировать, что пользовательские службы никогда не будут использовать больше указанный объем ресурсов. Это достигается путем установки значения ЕнфорцеусерсервицеметриккапаЦитиес config в разделе ПлацементандлоадбаланЦинг параметра ClusterManifest в значение true. По умолчанию этот параметр отключен.
