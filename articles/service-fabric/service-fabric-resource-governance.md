...
* *Память* (имя метрики — `servicefabric:/_MemoryInMB`): память измеряется в мегабайтах и сопоставляется с физической памятью, доступной на компьютере.
...
* *MemoryReservationInMB* — мягкое ограничение системы управления памятью, которое применяется, только когда на узле обнаруживается состязание за память.
...
* *MaximumIOps* — максимальное количество операций ввода-вывода в секунду, которое может использовать контейнер (для чтения и записи).
* *MaximumIOBytesps* — максимальное количество вводов-выводов (байт в секунду), которое может использовать контейнер (для чтения и записи).
* *BlockIOWeight* — количество вводов-выводов в блоке по отношению к другим контейнерам.
