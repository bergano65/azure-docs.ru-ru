...
* **MaxClusterStabilizationTimeout**. Максимальное время ожидания для кластера в работоспособное состояние до невыполнения теста. Проверяется, работоспособны ли кластер и службы, соответствует ли размер набора целевых реплик разделу службы, а также отсутствуют ли реплики InBuild.
...
* **EnableMoveReplicaFaults**. Включает или отключает сообщения об ошибках, которые приводит к перемещению первичных или вторичных реплик. Эти ошибки отключены по умолчанию.
* **WaitTimeBetweenIterations**. Количество времени ожидания между итерациями, то есть после цикла сбоев и соответствующей проверки.
...
### <a name="important-configuration-options"></a>Важные параметры конфигурации
* **PartitionSelector**: Объект выбора, указывающий раздел, который требуется для которых будет включен.
* **TimeToRun**. Общее время выполнения теста до завершения.
* **MaxServiceStabilizationTimeout**: Максимальное время ожидания для кластера в работоспособное состояние до невыполнения теста. Проверяется, работоспособна ли служба, соответствует ли размер набора целевых реплик всем разделам, а также отсутствуют ли реплики InBuild.
* **WaitTimeBetweenFaults**. Количество времени ожидания между каждой циклом сбоя и проверки.
