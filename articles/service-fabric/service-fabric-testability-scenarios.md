---
title: Создание тестов хаотичных сбоев и отработки отказа для Azure Service Fabric | Документация Майкрософт
...
* **TimeToRun**. Общее время, в течение которого тест будет выполнен до успешного завершения. Тест может завершиться ранее вместо ошибки проверки.
* **MaxClusterStabilizationTimeout**. Максимальное количество времени, в течение которого кластер будет ожидать работоспособности, прежде чем будет выполнен сбой теста. Проверяется, работоспособны ли кластер и службы, соответствует ли размер набора целевых реплик разделу службы, а также отсутствуют ли реплики InBuild.
* **MaxConcurrentFaults**. Максимальное число одновременных ошибок, вызванных каждой итерацией. Чем больше это число, тем более интенсивно проводится тест, что приведет к более сложным отработкам отказа и переходам. Во время этого теста при отсутствии внешних ошибок исключена вероятность потери кворума или данных, независимо от значения этого параметра.
* **EnableMoveReplicaFaults**. Включает или отключает ошибки, вызывающие перемещение первичных или вторичных реплик. Эти ошибки отключены по умолчанию.
* **WaitTimeBetweenIterations**. Время ожидания между итерациями, т. е. после циклов ошибок и соответствующей проверки.
...
Модуль Service Fabric PowerShell содержит два способа запуска сценария Chaos. `Invoke-ServiceFabricChaosTestScenario` является клиентским, и если клиентский компьютер завершает работу в ходе теста, дальнейшие ошибки не будут введены. Кроме того, существует набор команд, предназначенных для сохранения выполнения теста в случае завершения работы компьютера. `Start-ServiceFabricChaos` использует системную службу с отслеживанием состояния и надежной системной службой под названием службы анализа сбоев, гарантируя, что ошибки будут отображаться, пока TimeToRun не станет. `Stop-ServiceFabricChaos` можно использовать для ручной отмены сценария, а `Get-ServiceFabricChaosReport` — для получения отчета. Дополнительные сведения см. в [справочнике по Azure Service Fabric PowerShell](https://docs.microsoft.com/powershell/module/servicefabric/?view=azureservicefabricps) и [принудительном управлении Chaos в кластерах Service Fabric](service-fabric-controlled-chaos.md).
...
### <a name="important-configuration-options"></a>Важные параметры конфигурации
* **PartitionSelector**: Объект Selector, указывающий секцию, на которую необходимо ориентироваться.
* **TimeToRun**. Общее время, в течение которого тест будет выполняться перед завершением.
* **Макссервицестабилизатионтимеаут**: Максимальное количество времени, в течение которого кластер будет ожидать работоспособности, прежде чем будет выполнен сбой теста. Проверяется, работоспособна ли служба, соответствует ли размер набора целевых реплик всем разделам, а также отсутствуют ли реплики InBuild.
* **WaitTimeBetweenFaults**. Время ожидания между всеми циклами сбоя и проверки.
