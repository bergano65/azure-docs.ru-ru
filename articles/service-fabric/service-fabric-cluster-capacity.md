---
title: Планирование кластерной емкости Service Fabric
...
* Свойства каждого типа узла (размер, первичный, интернет облицовка, количество вс-миноносов и т.д.)
...
| Серебряная           | 5                              | VMs одного ядра или выше с по крайней мере 50 ГБ локального SSD                      | Можно отложить до утверждения кластером Service Fabric | Задержка на любой значительный период времени невозможна                                                    |
| Бронзовая           | 1                              | VMs с не менее 50 ГБ местного SSD                                              | Не будет задержано кластером Service Fabric           | Задержка на любой значительный период времени невозможна                                                    |

> [!WARNING]
> Типы узлов, выполняющиеся с устойчивостью Bronze, _не получают привилегий_. Это означает, что задания инфраструктуры, влияющие на рабочие нагрузки с состоянием, не будут остановлены или отложены, что может повлиять на рабочие нагрузки. Используйте уровень устойчивости Bronze для типов узлов, на которых выполняются только рабочие нагрузки без учета состояния. Для рабочих нагрузок рекомендуется использовать Silver или более высокий уровень. 
> 
...
- Принять более безопасные способы сделать VM SKU изменения (Scale вверх /вниз): Изменение VM SKU виртуального набора масштаба машины требует ряда шагов и соображений. Ниже описывается, как избежать распространенных проблем.
    ...
    - **Для типа первичного узла:** Если выбранный вами VM SKU находится на емкости и вы хотели бы перейти на более крупный VM SKU, следуйте нашим рекомендациям по [вертикальному масштабированию для основного типа узлов.](https://docs.microsoft.com/azure/service-fabric/service-fabric-scale-up-node-type) 
...
- Рекомендуемый VM SKU является Стандартным D2_V2 или эквивалентом с минимум 50 ГБ локального SSD.
- Минимальное поддерживаемое использование VM SKU является Standard_D2_V3 или Стандартным D1_V2 или эквивалентом с минимум 50 ГБ локального SSD. 
...
- Серия VM SKUs не поддерживается для производственных нагрузок по причинам производительности.
...
- Рекомендуемый VM SKU является Стандартным D2_V2 или эквивалентом. 
