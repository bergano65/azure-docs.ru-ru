...
| Silver           | 5                              | Виртуальные машины из одного или более поздней версии с по крайней мере 50 ГБ локальный SSD                      | Можно отложить до утверждения кластером Service Fabric | Задержка на любой значительный период времени невозможна                                                    |
| Bronze           | 1                              | Виртуальные машины с по крайней мере 50 ГБ локальный SSD                                              | Не будет задержано кластером Service Fabric           | Задержка на любой значительный период времени невозможна                                                    |
...
- Реализуйте более безопасные способы изменения номера SKU виртуальной машины (масштабирования). Изменение номера SKU масштабируемый набор виртуальных машин требуется ряд инструкции и рекомендации. Ниже описывается, как избежать распространенных проблем.
    ...
    - **Для типа первичного узла:** Если номер SKU виртуальной Машины, вы выбрали — достигнуто максимальное число, и вы хотите изменить до большего размера SKU виртуальной Машины, следуйте указаниям по на [вертикальное масштабирование для типа первичного узла](https://docs.microsoft.com/azure/service-fabric/service-fabric-scale-up-node-type). 
...
- Рекомендуемый номер SKU виртуальной Машины — Standard D2_V2 или аналогичной с минимальным объемом 50 ГБ на локальном SSD.
- Минимальный поддерживаемый номер SKU виртуальной Машины — виртуальная машина Standard_D2_V3 Standard D1_V2 или их эквивалент с минимальным объемом 50 ГБ на локальном SSD. 
...
- Серия SKU виртуальных Машин не поддерживаются для производственных рабочих нагрузок по соображениям производительности.
...
- Рекомендуемый номер SKU виртуальной Машины — Standard D2_V2 или наличие эквивалентных прав. 
