...
| Серебряный           | 5                              | Виртуальные машины с одним или несколькими ядрами                                                        | Можно отложить до утверждения кластером Service Fabric | Задержка на любой значительный период времени невозможна                                                    |
...
- Реализуйте более безопасные способы изменения номера SKU виртуальной машины (масштабирования). Изменение номер SKU виртуальной Машины масштабируемого набора виртуальных машин по своей сути является небезопасной операцией и поэтому следует избегать, если это возможно. Ниже описывается, как избежать распространенных проблем.
    - **Для типов вторичных узлов:** Рекомендуется создать новый масштабируемый набор виртуальных машин, изменить ограничение размещения служб, чтобы добавить новый тип узла для этого набора масштабирования виртуальных машин, а затем сократить число экземпляров набора масштабирования старого виртуальной машины на ноль, один узел за раз (это сделано для того том, что удаление узлов не влияют на надежность кластера).
    - **Для типа первичного узла:** Мы рекомендуем не изменять номер SKU виртуальной Машины типа первичного узла. Изменение номера SKU для типа первичного узла не поддерживается. Если причиной создания номера SKU является увеличение емкости, рекомендуем добавить экземпляры. Если это невозможно, создайте кластер и [восстановите состояние приложения](service-fabric-reliable-services-backup-restore.md) (если это применимо) из старого кластера. Не нужно восстанавливать состояния службы системы, так как при развертывании приложений в новом кластере они будут созданы заново. Если приложения в кластере выполняются без отслеживания состояния, разверните их в новом кластере.  Восстановление выполнять не нужно. Если вы используете неподдерживаемый маршрут и хотите изменить номер SKU виртуальной машины, внесите изменения в определение модели масштабируемого набора виртуальных машин, указав новый номер SKU. Если в кластере используется только один тип узла, то убедитесь, что все приложения с отслеживанием состояния своевременно реагируют на все [события жизненного цикла реплики службы](service-fabric-reliable-services-lifecycle.md) (например, задержку сборки реплики) и длительность восстановления реплики службы не превышает пяти минут (для уровня устойчивости Silver). 
...
* Platinum: запуск системных служб с целевым набором из семи реплик.
...
- **Число экземпляров виртуальных Машин для выполнения любой рабочей нагрузки в Azure:** Необходимо указать минимальный размер для типа первичного узла 5, а также уровень надежности Silver.  
...
- **НОМЕР SKU ВИРТУАЛЬНОЙ МАШИНЫ:** Тип первичного узла используется, где для запуска системных служб, поэтому номер SKU виртуальной Машины, выберите для него, необходимо взять в общей пиковой нагрузки вы планируете разместить в кластере. Чтобы пояснить, что имеется ввиду, приведу аналогию. Представьте, что тип первичного узла — ваши легкие, которые снабжают кислородом мозг. Поэтому, если мозг не получает достаточно кислорода, то тело страдает. 
...
- Минимальным требованиям соответствует локальный диск SSD емкостью в 14 ГБ. Мы рекомендуем использовать не менее 50 ГБ. Для рабочих нагрузок, особенно при использовании контейнеров Windows, требуются диски большего размера. 
...
**НОМЕР SKU ВИРТУАЛЬНОЙ МАШИНЫ:** Это тип узла запущенным служб приложения, поэтому номер SKU виртуальной Машины выберите, следует принять во внимание пиковую нагрузку, которую планируется разместить в каждом узле. Требуемая емкость для типа узла определяется в соответствии с рабочей нагрузкой, которую вы планируете выполнять в кластере. Поэтому мы не можем предоставить рекомендации для конкретной нагрузки и приводим здесь только общие указания, которые помогут вам начать работу.
...
**Количество экземпляров виртуальных Машин:** Для рабочих нагрузок без отслеживания состояния минимальный поддерживаемый первичного узла, не являющегося типа равно 2. Это дает возможность запустить два экземпляра приложения без отслеживания состояния и позволяет службе перенести потерю экземпляра виртуальной машины. 

**НОМЕР SKU ВИРТУАЛЬНОЙ МАШИНЫ:** Это тип узла запущенным служб приложения, поэтому номер SKU виртуальной Машины выберите, следует принять во внимание пиковую нагрузку, которую планируется разместить в каждом узле. Потребности в емкости для типа узла определяются рабочей нагрузкой, которую вы планируете запускать в кластере. Мы не можем дать вам качественные рекомендации конкретно по вашей рабочей нагрузке.  Однако доступна общая рекомендация, которая поможет вам приступить к работе.
