---
title: Сложные типы XML-схемы модели службы Service Fabric Azure
...
|Атрибут|Значение|
|---|---|
|content|9 элементов, 0 атрибутов|
|определение|globally|
...
Имя раздела в файле Settings.xml для ConfigPackage.
...
|---|---|
|name|ConfigPackage|
|тип|[ConfigPackageDescriptionType](#configpackagedescriptiontype-complextype)|
|minOccurs|0|
|maxOccurs|unbounded|
...
|Атрибут|Значение|
|---|---|
|content|0 элементов, 8 атрибутов|
|определение|globally|
...
|Атрибут|Значение|
|---|---|
|content|10 элементов, 8 атрибутов|
|определение|globally|
...
Используйте учетные данные репозитория по умолчанию, указанные в файле clusterManifest.xml, вместо учетных данных, указанных в теге RepositoryCredentials в файле ApplicationManifest.xml.
...
Используйте аутентификацию на основе токена MSI (или указанную пользователем конечную точку) для получения токена, необходимого для выполнения аутентификации.
...
Режим изоляции контейнера. Допустимые значения — default, process или hyperv (поддерживается только для контейнеров Windows).
...
Флажок, указывающий, как будет выполняться удаление контейнеров после завершения их работы: автоматически (true) или нет (false).  Если выбрано значение false, удалите контейнеры вручную.
...
|---|---|
|name|Метка|
|тип|[ContainerLabelType](#containerlabeltype-complextype)|
|minOccurs|0|
|maxOccurs|unbounded|
...
NetworkType. В настоящее время поддерживаются типы: Open (Открытая) и Isolated (Изолированная).
...
Имя конечной точки, которое должно быть объявлено в разделе ресурсов манифеста службы.
...
Описывает связь между пакетом службы и сетью контейнера, а также при необходимости связь между конечными точками пакета службы и сети контейнера.
...
Имя сети контейнера (без учета регистра). Это должно быть имя уже созданной или зарезервированной сети контейнера, включающее Open или NAT.
...
|тип|[ContainerNetworkPolicyEndpointBindingType](#containernetworkpolicyendpointbindingtype-complextype)|
|minOccurs|0|
|maxOccurs|unbounded|
...
|Атрибут|Значение|
|---|---|
|content|4 элемента, 10 атрибутов|
|определение|globally|
...
#### <a name="port"></a>Порт
Номер порта для конечной точки. Переопределяет значение в манифесте служб.
...
Протокол конечной точки. Переопределяет значение в манифесте служб. Конечные точки HTTPS также должны иметь EndpointCertificate и EndpointBindingPolicy, объявленные в манифесте приложения. Позже протокол невозможно будет изменить при обновлении приложения. 
...
#### <a name="type"></a>Тип
Тип конечной точки. Переопределяет значение в манифесте служб. Входная конечная точка используется, когда необходимо предоставить порт вовне. Внутренние конечные точки используются для передачи данных внутри приложения.
...
Схема URI. Переопределяет значение в манифесте служб. Например, HTTP, HTTPS или FTP.
...
Суффикс пути. Например, /myapp1. Переопределяет значение в манифесте служб.
...
#### <a name="protocol"></a>Протокол
Протокол для конечной точки: http, https, tcp или udp. Конечные точки HTTPS также должны иметь EndpointCertificate и EndpointBindingPolicy, объявленные в манифесте приложения. Позже протокол невозможно будет изменить при обновлении приложения. 
...
|тип|[EnvironmentVariableOverrideType](#environmentvariableoverridetype-complextype)|
|minOccurs|0|
|maxOccurs|unbounded|
...
|Атрибут|Значение|
|---|---|
|content|21 элемент, 0 атрибутов|
|определение|globally|
...
|Атрибут|Значение|
|---|---|
|content|2 элемента, 7 атрибутов|
|определение|globally|
...
Описывает политики сети, включая политики сети контейнера для пакета службы.
...
|тип|[ContainerNetworkPolicyType](#containernetworkpolicytype-complextype)|
|minOccurs|0|
|maxOccurs|unbounded|
...
#### <a name="passwordencrypted"></a>PasswordEncrypted
Указывает, является ли пароль зашифрованным или обычным текстом. Этот атрибут нерекомендуемый. Чтобы указать зашифрованный пароль, используйте атрибут Type.
...
|Атрибут|Значение|
|---|---|
|content|0 элементов, 12 атрибутов|
|определение|globally|
...
Ограничения памяти (в МБ). Это значение должно быть положительным целым числом. Пакеты кода (контейнеры или процессы) не могут выделить больше памяти, чем задано ограничением, а при попытке сделать это возникнет проблема нехватки памяти.
...
Общий объем используемой памяти подкачки, в МБ. Это значение должно быть положительным целым числом.
...
Мягкое ограничение (в МБ) для управления памятью, которое применяется, только когда на узле обнаруживается состязание за память. Это значение должно быть положительным целым числом.
...
Относительный вес ЦП. Это значение должно быть положительным целым числом.
...
Процент доступных используемых ЦП (только для Windows). Это значение должно быть положительным целым числом. Если ограничения ресурсов ЦП указаны для пакета службы, этот параметр игнорируется.
...
Максимальная скорость ввода-вывода (чтение и запись) с точки зрения операций ввода-вывода, которые могут использоваться. Это значение должно быть положительным целым числом.
...
Максимальная скорость ввода-вывода (в байтах в секунду), которую можно использовать (чтение и запись). Это значение должно быть положительным целым числом.
...
Дисковая квота для контейнеров.  Это значение должно быть положительным целым числом.
...
Ограничения памяти ядра в байтах.  Это значение должно быть положительным целым числом.  Обратите внимание, что это применимо только для Linux. При использовании данного атрибута в Docker на платформе Windows возникнет ошибка.
...
Размер /dev/shm в байтах. Если размер не указан, система использует 64 МБ.  Это значение должно быть положительным целым числом.  Обратите внимание, что это применимо только для Linux. Однако при использовании данного атрибута в Docker ошибка не возникнет.  Атрибут просто будет проигнорирован.
...
Права на предоставление: чтение, изменение или полная. Значение по умолчанию — на чтение.
...
Тип ресурса, определенный в разделе ресурсов в манифесте служб. Конечная точка или сертификат.
...
Указывает, должны ли сведения, связанные со средой выполнения Service Fabric, предоставляться пакетам кода в пакете службы.
...
Указывает, следует ли использовать реплицируемое хранилище блоков Service Fabric для предоставления тома для контейнеров в пакете службы.
...
|Атрибут|Значение|
|---|---|
|content|11 элементов, 0 атрибутов|
|определение|globally|
...
|тип|[ServiceFabricRuntimeAccessPolicyType](#servicefabricruntimeaccesspolicytype-complextype)|
|minOccurs|0|
|maxOccurs|1|
...
|тип|[ServicePackageContainerPolicyType](#servicepackagecontainerpolicytype-complextype)|
|minOccurs|0|
|maxOccurs|1|
...
|тип|[ConfigPackagePoliciesType](#configpackagepoliciestype-complextype)|
|minOccurs|0|
...
|тип|[NetworkPoliciesType](#networkpoliciestype-complextype)|
|minOccurs|0|
|maxOccurs|1|
...
#### <a name="isolation"></a>Изоляция
Режим изоляции контейнера. Допустимые значения: default, process или hyperv.
...
Предел ЦП в количестве логических ядер. Это значение должно быть положительным целым числом.
...
#### <a name="memoryinmb"></a>MemoryInMB
Ограничения памяти (в МБ). Это значение должно быть положительным целым числом.
...
|Атрибут|Значение|
|---|---|
|content|11 элементов, 4 атрибута|
|определение|globally|
...
|Атрибут|Значение|
|---|---|
|content|5 элементов, 2 атрибута|
|определение|globally|
|name|ServiceType|
...
|Атрибут|Значение|
|---|---|
|content|0 элементов, 11 атрибутов|
|определение|globally|
