...
Диспетчер кластерных ресурсов выполняет три разные категории работы. Они приведены ниже.
...
![Пример действий балансировки порога][Image2]
</center>
...
![Схема, показывающая, как сбалансировать службы вместе.][Image4]
</center>
...
![Схема, показывающая, что диспетчер ресурсов кластера определяет, какие службы связаны.][Image5]
</center>
...
* Кластерный диспетчер ресурсов может распознать и управлять подкластером (ситуация, которая иногда возникает при использовании ограничений на размещение и балансировки). Сведения о том, как подкластеризация может повлиять на балансировку и способ ее решения, см. [здесь](cluster-resource-manager-subclustering.md) .
