...
1. Обновите конфигурацию управляющей программы Docker в узле с помощью следующих параметров и перезапустите управляющую программу Docker: 
    ...
    >[!NOTE]
    >
    >Рекомендуется изменить управляющую программу непосредственно в DOCKER, так как расположение файла Daemon. JSON может отличаться от компьютера к компьютеру. Например, ~/Library/Containers/com.docker.docker/Data/database/com.docker.driver.amd64-linux/etc/docker/daemon.json.
    >
...
5. Для запуска кластера потребуется некоторое время. Во время запуска вы можете просмотреть состояние работоспособности кластеров, можете перейти к панели мониторинга кластера [http://localhost:19080](http://localhost:19080) или просмотреть журналы с помощью следующей команды:
...
6. Чтобы остановить и очистить контейнер, используйте следующую команду: Тем не менее мы будем использовать этот контейнер на следующем шаге.
...
Чтобы установить интерфейс командной строки Service Fabric ([) на компьютере Mac, см. инструкции в ](service-fabric-cli.md#cli-mac)этом разделе`sfctl`.
...
5. Чтобы создать приложение Service Fabric Java на компьютере Mac, установите JDK 1.8 и Gradle на хост-компьютере. Это можно сделать с помощью [HomeBrew](https://brew.sh/), как показано ниже: 
    ...
    > [!IMPORTANT]
    > В текущих версиях `brew cask install java` может устанавливаться более поздняя версия JDK.
