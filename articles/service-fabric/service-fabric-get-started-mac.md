...
5. Для запуска кластера потребуется некоторое время. Во время запуска вы можете просмотреть состояние работоспособности кластеров, можете перейти к панели мониторинга кластера `http://localhost:19080` или просмотреть журналы с помощью следующей команды:
