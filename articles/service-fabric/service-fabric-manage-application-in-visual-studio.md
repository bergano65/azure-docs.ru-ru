...
1. **Обновить приложение.** В этом режиме можно быстро изменять и отлаживать код, а также изменять статические веб-файлы во время отладки. Этот режим работает, только если локальный кластер разработки находится в [режиме с 1 узлом](./service-fabric-get-started-with-a-local-cluster.md#one-node-and-five-node-cluster-mode). Это режим отладки приложения по умолчанию.
