...
* Эта операция приведет к тому, что в кластере будут находиться только {0} потенциальные узлы начальных значений, а {1} требуется как минимум.
* Удаление {0} начальных узлов из {1} приведет к тому, что кластер будет отключен из-за потери кворума начального узла. Максимальное число узлов начального значения, которое можно удалить за раз, — {2}.
