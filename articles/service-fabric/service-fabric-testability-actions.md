...
* Ненормальные ошибки. Эти сбои имитируют ошибки, например перезагрузки компьютеров и аварийное завершение процессов. В случае таких ошибок контекст выполнения процесса немедленно прерывается. Это означает, что очистка состояния возможна только после повторного запуска приложения.
* Нормальные ошибки. Эти сбои имитируют нормальные действия, например перемещение реплик и операции удаления, инициированные балансировкой нагрузки. В таких случаях служба получает уведомление о закрытии и может выполнить очистку состояния перед завершением работы.
...
Выходные данные первой команды **Get-ServiceFabricNode** (командлет из модуля PowerShell Service Fabric) показывают, что в локальном кластере существует пять узлов: от Node.1 до Node.5. После выполнения командлета **Restart-ServiceFabricNode** (действия, доступного благодаря Testability) на узле с именем Node.4 мы увидим, что время работы узла сброшено.
...
Дополнительные сведения см. в PartitionSelector и ReplicaSelector.
