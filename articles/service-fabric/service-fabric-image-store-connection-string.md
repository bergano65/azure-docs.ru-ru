...
В некоторой документации мы упоминаем о существовании параметра ImageStoreConnectionString, не описывая его значение. После того как вы просматриваете статью, например [развертывание и удаление приложений с помощью PowerShell][10], все, что вам нужно сделать, — это скопировать или вставить значение, как показано в манифесте кластера целевого кластера. Поэтому параметр должен быть настроен для каждого кластера, но при создании кластера с помощью [портал Azure][11]нет возможности настраивать этот параметр и всегда иметь значение "Fabric: ImageStore". Для чего тогда нужен этот параметр?
