...
1. Выполните `docker login` команду, чтобы войти в реестр контейнеров с [учетными данными реестра](../container-registry/container-registry-authentication.md).
...
1. Создайте новый каталог за пределами локальной копии (то есть не в каталоге *service-fabric-java-getting-started* и не в его дочерних каталогах). Перейдите в этот каталог и создайте шаблон для приложения контейнера с помощью Yeoman: 
   ...
   * Назовите приложение: сервицефабриктомкат
   * Имя службы приложения: томкатсервице
   * Введите имя образа: Укажите URL-адрес образа контейнера в реестре контейнеров. Например, myregistry.azurecr.io/samples/tomcattest.
   ...
   * Число экземпляров гостевого приложения контейнера: 1
