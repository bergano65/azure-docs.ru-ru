---
title: Используйте захват пакетов для упреждающего сетевого мониторинга с помощью предупреждений - Azure Functions
...
2. В колонке **Приложение-функция** введите следующие значения и нажмите кнопку **ОК** для создания приложения:
    ...
    |**Ресурсная группа**|PacketCaptureRG|Группа ресурсов, куда будет помещено приложение-функция.|
    |**План хостинга**|План потребления| Тип плана, который использует приложение-функция. Можно выбрать план потребления или план службы приложений Azure. |
    |**Расположение**|Центральная часть США| Регион для создания приложения-функции.|
    |**Учетная запись хранилища**|{autogenerated}| Это учетная запись хранения, необходимая функциям Azure для хранилища общего назначения.|
...
4. Выберите **HttpTrigger-Powershell** и введите остальные сведения. Наконец, чтобы создать функцию, выберите **Создать**.
    ...
    |**Параметр** | **Значение** | **Подробно** |
    |---|---|---|
    |**Сценарий**|В экспериментальном режиме|Тип сценария|
    |**Имя функции**|AlertPacketCapturePowerShell|Имя функции|
    |**Уровень авторизации**|Компонент|Уровень авторизации для функции|
...
1. Выберите **настройки** > приложения функции**Перейдите к редактору службы приложений.**
...
1. Нажмите справа на subfolder **Az.Network,** а затем выберите **загрузить файлы.** 

6. Перейдите к модулям Azure. В локальной папке **Az.Network** выберите все файлы в папке. Затем выберите **OK**. 

7. Повторите эти действия для **Az.Accounts** и **Az.Resources**.
...
1. Перейдите к приложению-функции. Затем выберите **настройки** > приложения Функция**Настройка настройки приложения.**
...
Перейдите к существующей виртуальной машине и добавьте правило генерации оповещений. Дополнительные сведения о настройке оповещений см. в статье [Создание оповещений в Azure Monitor для служб Azure с помощью портала Azure](../monitoring-and-diagnostics/insights-alerts-portal.md). Введите указанные ниже значения в колонке **Правило генерации оповещений** и нажмите кнопку **ОК**.
  ...
  |**Параметр** | **Значение** | **Подробно** |
  |---|---|---|
  |**Название**|TCP_Segments_Sent_Exceeded|Имя правила генерации оповещений.|
  |**Описание**|Число отправленных TCP-сегментов превысило пороговое значение|Описание для правила генерации оповещений.|
  |**Метрические**|Отправлено TCP-сегментов| Метрика, используемая для активации оповещения. |
  |**Состояние**|Больше| Условие, используемое при вычислении метрики.|
  |**Порог**|100| Это значение метрики, при достижении которого запускается оповещение. Оно должно быть допустимым для вашей среды.|
  |**Период**|За последние пять минут| Определяет период, за который проверяется пороговое значение для метрики.|
  |**Вебхук**|[URL-адрес веб-перехватчика из приложения-функции]| Это URL-адрес веб-перехватчика из приложения-функции, созданного на предыдущих шагах.|
