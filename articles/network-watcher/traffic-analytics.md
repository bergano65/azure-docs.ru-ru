...
> [!NOTE]
> Трафик Аналитика теперь поддерживает сбор NSG Flow журналы данных на более высокой частоте 10 минут
...
- **Группа сетевой безопасности (NSG) журналы потоков**: Разрешить вам просматривать информацию о проникновении и вывести IP-трафика через группу сетевой безопасности. Эти журналы потоков записываются в формате JSON. В них отображаются входящие и исходящие потоки по каждому правилу, сетевая карта, с которой связан поток, сведения о пяти кортежах потока (IP-адрес источника и места назначения, порт источника и места назначения, протокол), а также сведения о состоянии трафика (разрешен или запрещен). Дополнительные сведения о журналах потоков NSG см. в статье [Общие сведения о ведении журнала потоков для групп безопасности сети](network-watcher-nsg-flow-logging-overview.md).
- **Log Analytics**: служба Azure, которая собирает данные мониторинга и сохраняет их в центральный репозиторий. Эти данные могут содержать события, данные о производительности или пользовательские данные, полученные с помощью API Azure. Собранные данные доступны для оповещения, анализа и экспорта. Мониторинг приложений, таких как монитор производительности сети и аналитика трафика, строится с использованием журналов Azure Monitor в качестве основы. Для получения дополнительной информации смотрите [журналы Azure Monitor](../log-analytics/log-analytics-overview.md?toc=%2fazure%2fnetwork-watcher%2ftoc.json).
- **Рабочее пространство для журналов журнала «Аналитика»:** экземпляр журналов Azure Monitor, где хранятся данные, относящиеся к учетной записи Azure. Для получения дополнительной информации о рабочих областях Log Analytics [см.](../azure-monitor/learn/quick-create-workspace.md?toc=%2fazure%2fnetwork-watcher%2ftoc.json)
...
## <a name="supported-regions-nsg"></a>Поддерживаемые регионы: NSG 
...
## <a name="supported-regions-log-analytics-workspaces"></a>Поддерживаемые регионы: Рабочие области аналитики журналов
...
На портале Azure перейдите на **систему просмотра сети,** а затем выберите **журналы потока NSG.** Выберите группу безопасности сети, для которой нужно включить журнал потоков NSG, как показано на следующем рисунке:
...
Если вы попытаетесь включить аналитику трафика для NSG, которая размещается в любом регионе, кроме [поддерживаемых регионов,](#supported-regions-nsg)вы получите ошибку "Не найдено".
...
2. Выберите *версию 2* для **версии Flow Logs.** В версии 2 доступна статистика сеансов потоков (количество байт и пакетов).
3. Выберите учетную запись хранения из имеющихся, в которой будут храниться журналы потоков. Если вы хотите обеспечить постоянное хранение данных, установите значение *0*. За использование учетной записи хранения в службе хранилища Azure будет взиматься плата. Убедитесь, что в вашем хранилище не имеется "Хранилища данных озера Gen2 Иерархическое Namespace Enabled" установлен на истину.
...
6. Выберите интервал обработки. На основе вашего выбора журналы потоков будут собираться со счета хранения и обрабатываться Traffic Analytics. Вы можете выбрать интервал обработки каждые 1 час или каждые 10 минут. 
...
Повторите шаги выше для любых других групп NSG, для которых нужно включить решение "Аналитика трафика". Данные из журналов потоков отправляются в рабочую область, поэтому проверьте, разрешено ли в соответствии с законом и правилами в вашей стране размещать хранилище данных в регионе, где есть рабочая область. Если вы установили различные интервалы обработки для различных НГУ, данные будут собираться через разные промежутки времени. Например: Вы можете включить интервал обработки 10 минут для критических VNETs и 1 час для некритических VNETs.

Вы также можете настроить аналитику трафика с помощью смдлета [Set-AzNetworkWatcherFlowigFlowLog](/powershell/module/az.network/set-aznetworkwatcherconfigflowlog) PowerShell в Azure PowerShell. Выполните `Get-Module -ListAvailable Az`, чтобы узнать установленную версию. Если вам необходимо выполнить обновление, ознакомьтесь со статьей, посвященной [установке модуля Azure PowerShell](/powershell/azure/install-Az-ps).

## <a name="view-traffic-analytics"></a>Просмотр решения "Аналитика трафика"

Чтобы просмотреть трафик Analytics, поиск **для просмотра сети** в панели поиска портала. Оказавшись внутри Network Watcher, чтобы изучить аналитику трафика и его возможности, выберите **трафик Analytics** из левого меню. 
...
- На геокарте показана верхняя лента для выбора таких параметров, как центры обработки данных (Развертывание/Нет-развертывание/Актив/Неактивный/Неактивный/Трафик Аналитика включена/аналитика трафика не включена) и страны/регионы, вносящие доброкачественный/вредоносный трафик в активное развертывание:
...
- Геокарта показывает распределение трафика в центр обработки данных из стран/регионов и континентов, связывающих его синим (доброкачественный трафик) и красным (вредоносный трафик) цветными линиями:

    ![Представление геокарты, демонстрирующее распределение трафика в страны/регионы и континенты](./media/traffic-analytics/geo-map-view-showcasing-traffic-distribution-to-countries-and-continents.png)
...
- Вы можете отфильтровать топологию виртуальной сети на основе подписок, рабочих областей, групп ресурсов и интервала времени. Дополнительные фильтры, которые помогут вам понять поток: Тип потока (InterVNet, IntraVNET и так далее), Направление потока (Входящий, Исходящий), Статус потока (Разрешенный, заблокированный), VNETs (целевые и подключенные), Тип соединения (Peering или Шлюз - P2S и S2S) и NSG. Используйте эти фильтры, чтобы сосредоточиться на виртуальных сетях, которые вы хотите подробно рассмотреть.
...
Чтобы получить ответы на часто задаваемые вопросы, [см.](traffic-analytics-faq.md)
...
- Чтобы понять схему и детали обработки трафика Analytics, [см.](traffic-analytics-schema.md)
