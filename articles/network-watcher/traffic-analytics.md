...
- **Log Analytics**: служба Azure, которая собирает данные мониторинга и сохраняет их в центральный репозиторий. Эти данные могут содержать события, данные о производительности или пользовательские данные, полученные с помощью API Azure. Собранные данные доступны для оповещения, анализа и экспорта. Мониторинг приложений, например аналитика трафика и монитор производительности сети, созданы с помощью Azure Monitor журналы как основу. Дополнительные сведения см. в разделе [журналы Azure Monitor](../log-analytics/log-analytics-overview.md?toc=%2fazure%2fnetwork-watcher%2ftoc.json).
- **Рабочая область Log Analytics**. Экземпляр журналы Azure Monitor, где хранятся данные, относящиеся к учетной записи Azure. Дополнительные сведения о рабочих областях Log Analytics см. в разделе [создать рабочую область Log Analytics](../azure-monitor/learn/quick-create-workspace.md?toc=%2fazure%2fnetwork-watcher%2ftoc.json).
...
2. Выберите *версии 2* для **журналами потоков для версии**. В версии 2 доступна статистика сеансов последовательностей (количество байт и пакетов).
...
- Вы можете отфильтровать топологию виртуальной сети на основе подписок, рабочих областей, групп ресурсов и интервала времени. Дополнительные фильтры, которые помогут вам определить поток: Поток типа (между виртуальными сетями, IntraVNET и т. д.), направление потока (входящий, исходящий), состояние потока (разрешенных, заблокированных), виртуальных сетей (целевые и подключенные), тип соединения (Пиринг или шлюз: P2S и S2S) и группы безопасности сети. Используйте эти фильтры, чтобы сосредоточиться на виртуальных сетях, которые вы хотите подробно рассмотреть.
...
- Чтобы понять схему и обработки сведения об аналитике трафика Azure, см. в разделе [схема аналитики трафика](traffic-analytics-schema.md).
