...
- "Microsoft. Network/ЕкспрессраутеЦиркуитс/Read"
...
В портал Azure перейдите к **наблюдателю за сетями**, а затем выберите **журналы потоков NSG**. Выберите группу безопасности сети, для которой нужно включить журнал потоков NSG, как показано на следующем рисунке:
...
Если вы попытаетесь включить аналитику трафика для NSG, размещенной в любом регионе, отличном от [поддерживаемых регионов](#supported-regions-nsg), появится сообщение об ошибке "не найдено".
...
3. Выберите учетную запись хранения из имеющихся, в которой будут храниться журналы потоков. Убедитесь, что в хранилище не включено значение "Data Lake Storage 2-го поколения иерархическое пространство имен".
...
- На географической карте показана верхняя лента для выбора параметров, таких как центры обработки данных (развернутые/незавершенные/активные/неактивные/Аналитика трафика включенные Аналитика трафика или отключенные), а также страны и регионы, которые участвуют в активном развертывании по небезопасному или вредоносному трафику:
...
- Вы можете отфильтровать топологию виртуальной сети на основе подписок, рабочих областей, групп ресурсов и интервала времени. Дополнительные фильтры, помогающие понять последовательность: тип потока (между виртуальными сетями, Интравнет и т. д.), направление потока (входящий, исходящий трафик), состояние потока (разрешено, заблокировано), виртуальных сетей (назначенный и подключенный), тип подключения (пиринг или шлюз-P2S и S2S) и NSG. Используйте эти фильтры, чтобы сосредоточиться на виртуальных сетях, которые вы хотите подробно рассмотреть.
...
Чтобы получить ответы на часто задаваемые вопросы, см. [вопросы и ответы по аналитике трафика](traffic-analytics-faq.md).
