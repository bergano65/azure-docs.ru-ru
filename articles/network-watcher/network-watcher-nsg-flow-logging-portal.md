...
| C            | Состояние потока (**только в версии 2**) | Фиксируется состояние потоков. Возможные состояния. **B**: начать после создания потока. Статистика не предоставляется. **C**: продолжить для текущего потока. Статистика предоставляется с интервалом в 5 минут. **Е**: завершить после окончания потока. Статистика предоставляется. |
