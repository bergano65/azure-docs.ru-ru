...
3. Введите или выберите приведенные ниже сведения, примите значения по умолчанию для остальных параметров и щелкните **Создать**.
    ...
    > [!NOTE]
    > Хотя поставщики Microsoft.Insight и Microsoft.Network в настоящее время поддерживаются Службой хранилища Azure, журналы потоков NSG еще подключены не полностью Чтобы включить ведение журнала потоков NSG, по-прежнему нужно выбрать **Все сети**, пока эта функция не будет подключена полностью. 
