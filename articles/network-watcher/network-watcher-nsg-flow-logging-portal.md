...
8. Выберите версию журналов последовательностей. В версии 2 доступна статистика сеансов последовательностей (количество байт и пакетов). ![Выбор версии журналов последовательностей](./media/network-watcher-nsg-flow-logging-portal/select-flow-log-version.png)
...
### <a name="version-1-flow-log-event"></a>Событие журнала последовательностей версии 1
```json
{
...
```
### <a name="version-2-flow-log-event"></a>Событие журнала последовательностей версии 2
```json
{
...
| 443         | Конечный порт       | Порт назначения, на который был отправлен поток. Так как трафик отправлялся на порт 443, файл был обработан с помощью правила с именем **UserRule_default-allow-rdp** в файле журнала этой последовательности.                                                |
...
| C            | Состояние последовательности (**только в версии 2**) | Фиксируется состояние последовательности. Возможны следующие состояния. **B**. Начать при создании потока. Статистика не предоставляется. **C**. Продолжить для текущего потока. Статистика предоставляется с интервалом в 5 минут. **E**. Завершить после окончания потока. Статистика предоставляется. |
