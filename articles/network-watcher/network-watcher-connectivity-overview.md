...
|Hops[].Issues[].Origin | Текущий прыжок, где возникла проблем. Доступны следующие значения:<br/> **Inbound**. Проблема обнаружена в пути от предыдущего прыжка к текущему.<br/>**Outbound**. Проблема обнаружена в пути от текущего прыжка к следующему.<br/>**Local**. Проблема возникла на текущем прыжке.|
|Hops[].Issues[].Severity | Степень серьезности обнаруженной проблемы. Возможные значения: **Error** и **Warning**. |
|Hops[].Issues[].Type |Тип обнаруженной проблемы. Доступны следующие значения: <br/>**ЦП**<br/>**Память**<br/>**GuestFirewall**<br/>**днсресолутион**<br/>**NetworkSecurityRule**<br/>**UserDefinedRoute** |
