...
> [!NOTE]
> Хранение доступно только при использовании [учетных записей хранения общего назначения версии 2 (GPv2)](https://docs.microsoft.com/azure/storage/common/storage-account-overview#types-of-storage-accounts). 
...
**Неверный байт и количество пакетов для входящих потоков**: [группы безопасности сети (группы безопасности сети)](https://docs.microsoft.com/azure/virtual-network/security-overview) реализуются как [брандмауэр с отслеживанием состояния](https://en.wikipedia.org/wiki/Stateful_firewall?oldformat=true). Однако из-за ограничений платформы правила, управляющие входящими потоками, реализуются без отслеживания состояния. Из-за этого числа байт и пакетов не записываются для этих потоков. Следовательно, число байтов и пакетов, сообщаемых в журналах потоков NSG (и Аналитика трафика), может отличаться от реальных чисел. Кроме того, входящие потоки теперь не завершаются. Это ограничение планируется исправить до 2020 декабря. 
