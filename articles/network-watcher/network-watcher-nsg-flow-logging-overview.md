...
Журналы потока групп безопасности сети (NSG) являются функцией Наблюдателя за сетями, которая позволяет просматривать сведения о входящем и исходящем IP-трафике через NSG. Журналы последовательностей записываются в формате JSON и содержат исходящие и входящие последовательности на основе каждого правила, сетевой интерфейс (NIC), к которому применяется последовательность, 5-кортежную информацию о последовательности (IP-адреса источника и получателя, порт источника, назначения и протокол), а также сведения о состоянии трафика (разрешен или запрещен) и пропускной способности (количество байт и пакетов, только в версии 2).
...
Также можно анализировать журналы последовательностей с помощью решения [Аналитика трафика](traffic-analytics.md).
        ...
        * **rule** — правило, для которого отображены потоки.
            * **flows** — набор потоков.
                ...
                * **flowTuples** — строка с разделителями-запятыми, содержащая несколько свойств кортежа потока.
                    ...
                    * **Flow State - Version 2 Only** — фиксирует состояние последовательности. Возможные состояния. **B**: начать после создания последовательности. Статистика не предоставляется. **C**: продолжить для текущего потока. Статистика предоставляется с интервалом в 5 минут. **Е**: завершить после окончания потока. Статистика предоставляется.
...
## <a name="nsg-flow-logs-version-2"></a>Журналы последовательностей версии 2
> [!NOTE] 
...
В версии 2 журналов введено состояние потока. Вы можете указать, какая версия журналов последовательностей будет сохраняться. Чтобы узнать, как включить журналы потока, см. статью [Руководство по регистрации потока сетевого трафика на виртуальную машину и с нее с помощью портала Azure](network-watcher-nsg-flow-logging-portal.md).
...
### <a name="version-1-nsg-flow-log-format-sample"></a>Пример формата журнала последовательности версии 1
```json
{
    "records": [
        {
    ...
    [
        ...
        ...
```
### <a name="version-2-nsg-flow-log-format-sample"></a>Пример формата журнала последовательности версии 2
```json
 {
    "records": [
        {
