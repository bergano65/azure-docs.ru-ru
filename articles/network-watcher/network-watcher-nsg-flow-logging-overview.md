...
**Проблемы с определяемыми пользователем правилами входящего трафика TCP**. [группы безопасности сети (группы безопасности сети)](https://docs.microsoft.com/azure/virtual-network/security-overview) реализованы в виде [брандмауэра с отслеживанием состояния](https://en.wikipedia.org/wiki/Stateful_firewall?oldformat=true). Однако из-за текущих ограничений платформы определяемые пользователем правила, влияющие на входящие TCP-потоки, реализуются без отслеживания состояния. Из-за этого потоки, затрагиваемые определяемыми пользователем правилами для входящих подключений, не будут завершаться. Кроме того, для этих потоков не записываются байты и количество пакетов. Следовательно, число байтов и пакетов, сообщаемых в журналах потоков NSG (и Аналитика трафика), может отличаться от реальных чисел. Флаг согласия, использующий эти проблемы, планируется сделать доступным до последней версии 2020 декабря. В предварительной ситуации клиенты, которые сталкиваются с серьезными проблемами из-за такого поведения, могут запрашивать поддержку в рамках поддержки, попросите получить поддержку в разделе "наблюдатель за сетями" > NSG журналы потоков.  
