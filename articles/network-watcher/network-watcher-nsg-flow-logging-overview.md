...
**Входящие потоки, зарегистрированные с IP-адресов на виртуальных машинах без общедоступных IP-адресов**: Виртуальные машины, у которых нет общедоступного IP-адреса, назначенного через общедоступный IP-адрес, связанный с сетевой картой в качестве общедоступного IP-адреса уровня экземпляра или входящего в пул серверной части базового балансировщика нагрузки, используют [SNAT по умолчанию](../load-balancer/load-balancer-outbound-connections.md#defaultsnat) и получают IP-адрес, назначенный Azure для упрощения исходящее подключение. В результате вы можете увидеть записи журнала потоков для последовательностей из IP-адресов Интернета, если последовательность направлена на порт в диапазоне портов, назначенных для SNAT. В то время как Azure не разрешает передачу этих потоков на виртуальную машину, попытка записи в журнал и появляется в журнале потоков NSG для наблюдателя за сетями. Рекомендуется явно заблокировать нежелательный входящий Интернет для NSG.
