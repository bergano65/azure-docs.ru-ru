...
3. Введите или выберите следующие значения, примите значения по умолчанию для остальных параметров и нажмите кнопку **ОК**:
    ...
    |Пароль| Введите выбранный пароль. Пароль должен содержать минимум 12 символов и соответствовать определенным требованиям к сложности.|
...
Если в хотя бы одном регионе уже включена служба "Наблюдатель за сетями", перейдите к разделу [Применение проверки IP-потока](#use-ip-flow-verify).
...
При создании виртуальной машины Azure применяет к ней стандартные правила разрешений и запретов трафика. Позднее вы можете переопределить эти значения по умолчанию, чтобы разрешить или запретить дополнительные типы трафика.

1. На портале выберите **Все службы**. В поле *Фильтр* в разделе **Все службы** введите *Наблюдатель за сетями*. Когда в результатах появится **Наблюдатель за сетями**, выберите его.
...
5. Выполните шаг 3 еще раз, но укажите для **направления** значение **Входящие**, **локальный порт** **80** и **удаленный порт** **60000**. Вернется результат с информацией о том, что такой доступ запрещается правилом безопасности с именем **DefaultInboundDenyAll**.
...
2. В разделе **Поддержка и устранение неполадок** выберите **Действующие правила безопасности**, как показано на следующем изображении:
    ...
    На шаге 3 в разделе **применения проверки IP-потока** вы узнали, что причиной разрешения обмена данными является правило **AllowInternetOutbound**. На предыдущем изображении можно увидеть, что **назначение** для правила — **Интернет**. Не совсем ясно, почему примененный на шаге 3 в разделе **применения проверки IP-потока** адрес 13.107.21.200 относится к категории **Интернет**.
