...
### <a name="are-there-caveats-for-using-nsg-flow-logs"></a>Существуют ли предостережения при использовании журналов потоков NSG?
...
- **Учетные записи хранения не должны быть брандмауэра**. из-за внутренних ограничений учетные записи хранения должны быть доступны через общедоступный Интернет для работы с ними журналов потоков NSG. Трафик будет по-прежнему направляться через Azure внутренним образом, и плата за них не взимается.
...
Оба этих ограничения должны быть решены по Янв 2020.
