...
- **Баннер.** Баннер в верхней части географической схемы предоставляет кнопки для выбора фильтров распределения трафика (например, развертывания, трафика из стран и регионов, а также вредоносных программ). При нажатии кнопки соответствующий фильтр применяется на карте. Например, если нажать кнопку "Активный", карта выделит активные центры обработки данных в развертывании.
- **Карта.** Под баннером в разделе "схема" показано распределение трафика между центрами обработки данных Azure и странами/регионами.
