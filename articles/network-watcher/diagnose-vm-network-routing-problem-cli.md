---
title: Диагностика проблемы с вымыванием сети VM - Azure CLI
...
Если вы решили установить и использовать Azure CLI локально, эта статья требует, чтобы вы запускали версию Azure CLI 2.0.28 или позже. Выполните командлет `az --version`, чтобы узнать установленную версию. Если вам необходимо выполнить установку или обновление, см. статью [Установка Azure CLI](/cli/azure/install-azure-cli). После проверки версии Azure `az login` CLI запустите соединение с Azure. Команды Azure CLI в этой статье отформатированы для запуска в оболочке Bash.

## <a name="create-a-vm"></a>Создание виртуальной машины

Прежде чем создать виртуальную машину, создайте группу ресурсов, которая будет содержать эту виртуальную машину. Создайте группу ресурсов с помощью команды [az group create](/cli/azure/group#az-group-create). Следующий пример создает группу ресурсов под названием *myResourceGroup* в *восточном* месте:

```azurecli-interactive
az group create --name myResourceGroup --location eastus
```

Создайте VM с [az vm создать.](/cli/azure/vm#az-vm-create) Также команда создает ключи SSH, если они не существуют в расположении ключей по умолчанию. Чтобы использовать определенный набор ключей, используйте параметр `--ssh-key-value`. Следующий пример создает VM под названием *myVm:*
...
Создание виртуальной машины занимает несколько минут. Не продолжайте оставшиеся действия до тех пор, пока VM не будет создан и не будет восстановлен выход Azure CLI.
...
Через несколько секунд, выход сообщает вам, что **nextHopType** является **Интернет**, и что **routeTableId** является **системный маршрут**. Из этого результата вы узнаете, что существует допустимый системный маршрут к назначению.
...
Выход возвращается сообщает вам, что **ни один из** них является **nextHopType**, и что **routeTableId** также **системный маршрут.** Так мы узнаем, что, хотя есть допустимый системный маршрут к месту назначения, следующий прыжок для маршрутизации трафика в пункт назначения отсутствует.
