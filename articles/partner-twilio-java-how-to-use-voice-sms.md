...
Сведения о ценах на Twilio доступны на веб-сайте [цен на Twilio][twilio_pricing]. Клиентам Azure доступно [специальное предложение][special_offer]: бесплатный кредит на 1000 текстовых сообщений или 1000 минут входящих вызовов. Чтобы подписаться на это предложение или [https://ahoy.twilio.com/azure][special_offer]получить дополнительную информацию, пожалуйста, посетите .
...
### <a name="twilio-verbs"></a><a id="Verbs"></a>Твилио Вербальс
API использует глаголы Twilio; например, глагол ** &lt;Say&gt; ** поручает Twilio вывешить сообщение на вызове.

Ниже приведен список команд Twilio.

* **Наберите:&gt;Подключает абонента к другому телефону. &lt;**
* **Соберите&gt;: Собирает числовые цифры, введенные на клавиатуре &lt;** телефона.
* **Hangup&gt;: Окончание &lt;** вызова.
* **Воспроизведение&gt;: Воспроизведение аудиофайла. &lt;**
* Очередь : Добавить в очередь абонентов. ** &lt;&gt;**
* **Пауза&gt;: Ожидание молча в течение определенного количества &lt;** секунд.
* Запись : Записывает голос вызываемого и возвращает URL файла, содержащего запись. ** &lt;&gt;**
* **Перенаправить:&gt;Передача управления вызовом или SMS в TwiML по другому URL. &lt;**
* Отклонить : Отклоняет входящий звонок на ваш номер Twilio без выставления счетов. ** &lt;&gt;**
* **Скажем:&gt;Преобразует текст в речь, которая сделана на вызове. &lt;**
* **Sms&gt;: Отправляет SMS-сообщение. &lt;**
...
1. Получите JAR-файл Twilio и добавьте его к пути построения Java и к WAR-сборке. В [https://github.com/twilio/twilio-java][twilio_java], вы можете скачать источники GitHub и создать свой собственный JAR, или скачать предварительно построенный JAR (с или без зависимостей).
2. Убедитесь, что в хранилище ключей **cacerts** JDK содержится сертификат Equifax Secure Certificate Authority с отпечатком MD5 67:CB:9D:C0:13:24:8A:82:9B:B2:17:1E:D1:1B:EC:D4 (серийный номер 35:DE:F4:CF, отпечаток SHA1 D2:32:09:AD:23:D3:14:23:21:74:E4:0D:7F:9D:62:13:97:86:63:3A). Это сертификат органа (CA) сертификат [https://api.twilio.com][twilio_api_service] для службы, который называется при использовании AIS Twilio. Сведения о том, как убедиться, что хранилище ключей **cacerts** JDK содержит правильный сертификат ЦС, см. в разделе [Добавление сертификата в хранилище сертификатов ЦС Java][add_ca_cert].
...
Далее показано, как осуществлять исходящий вызов с использованием класса **Call**. Этот код также использует сайт из Twilio для выдачи ответа на языке разметки Twilio (TwiML). Замените значения на номера **из** **и** по телефону и убедитесь, что вы проверите **номер** телефона для вашей учетной записи Twilio до запуска кода.
...
Ниже показано, как отправить SMS-сообщение с использованием класса **Message**. Номер **from** **4155992671**, предоставляется Twilio для пробных учетных записей для отправки SMS-сообщений. Номер **to** должен быть проверен для вашей учетной записи Twilio до запуска кода.
...
Когда приложение инициирует вызов API Twilio — например, с использованием метода **CallCreator.create** — Twilio отправляет ваш запрос на URL-адрес, который должен возвратить ответ TwiML. В приведенном выше примере используется [https://twimlets.com/message][twimlet_message_url]URL-адрес, предоставленный Twilio. (Хотя TwiML предназначается для использования веб-службами, TwiML можно также просмотреть в браузере. Например, [https://twimlets.com/message][twimlet_message_url] нажмите кнопку, чтобы увидеть пустой ** &lt;&gt; ** элемент ответа; в качестве еще [https://twimlets.com/message?Message%5B0%5D=Hello%20World%21][twimlet_message_url_hello_world] одного примера нажмите кнопку ** &lt;&gt; ** ,to see элемент ответа, содержащий элемент ** &lt;Say.)&gt; **
