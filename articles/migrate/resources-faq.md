...
Да, служба "Миграция Azure" пока поддерживает оценку только для рабочих нагрузок VMware. Поддержка для Hyper-V находятся в предварительной версии, войдите в систему [здесь](https://aka.ms/migratefuture) для получения доступа к предварительной версии. Поддержка для физических серверов будет добавлена в будущем.
...
**Миграция из Hyper-V в Azure**: Общедоступная версия "Миграция Azure" в настоящее время поддерживает оценки виртуальных машин VMware для миграции в Azure. Поддержка для Hyper-V доступна в предварительной версии с поддержкой рабочей среде. Если вы заинтересованы в ознакомлении с предварительной версии, войдите в систему [здесь](https://aka.ms/migratefuture).
...
Обнаружение на основе агента — параметр, доступный на основе обнаружения на основе устройства и помогает клиентам [визуализация зависимостей](how-to-create-group-machine-dependencies.md) из местных виртуальных машин. Агенты зависимостей собирают такие данные, как полное доменное имя, сведения об ОС, IP-адрес, MAC-адрес, сведения о процессах, выполняющихся внутри виртуальной машины, а также о входящих и исходящих TCP-подключениях этой машины. Обнаружение на основе агентов является необязательным. Вы можете не устанавливать агенты, если не требуется визуализировать зависимости виртуальных машин.
...
С помощью непрерывной профилирования данных о производительности, нет необходимости изменение vCenter Server уровень статистики, чтобы провести оценку на основе производительности. Модуль сборщика будет профилировать локальных компьютеров для оценки данных производительности виртуальных машин. Это практически не повлияет на производительность узлов ESXi, а также vCenter Server.
...
Визуализация зависимостей позволяет оценивать группы виртуальных машин для миграции с большей уверенностью, перекрестно проверяя зависимости машин перед тем, как выполнить оценку. Визуализация зависимостей поможет избежать непредвиденных сбоев при миграции в Azure. Служба "Миграция Azure" использует решение схемы услуги в журналах Azure Monitor, чтобы включить визуализацию зависимостей.
