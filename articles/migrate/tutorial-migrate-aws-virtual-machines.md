---
title: Обнаружение, оценка и перенос виртуальных машин Amazon Web Services (AWS) в Azure
...
Из этого учебника вы узнаете, как обнаруживать, оценивать и переносить виртуальные машины Amazon Web Services (AWS) в виртуальные машины Azure, используя средства оценки и миграции сервера службы "Миграция Azure".

> [!NOTE]
> При миграции виртуальных машин AWS в Azure виртуальные машины обрабатываются так, как если бы они были физическими серверами. Чтобы перенести виртуальные машины AWS в Azure, вы будете использовать поток переноса сервера для миграции физических компьютеров.
...
## <a name="discover-and-assess-aws-vms"></a>Обнаружение и оценка виртуальных машин AWS  
...
1. Оценку можно выполнить, обрабатывая виртуальные машины AWS как физические компьютеры с целью проведения оценки с помощью средства оценки сервера службы "Миграция Azure". Выполните инструкции из этого [учебника](https://docs.microsoft.com/azure/migrate/tutorial-prepare-physical), чтобы настроить Azure и подготовить виртуальные машины AWS к оценке.
2. Затем следуйте инструкциям в этом [учебнике](https://docs.microsoft.com/azure/migrate/tutorial-assess-physical), чтобы настроить проект и устройство Миграции Azure для обнаружения и оценки виртуальных машин AWS.
...
## <a name="1-prerequisites-for-migration"></a>1. Предварительные требования для миграции

- Убедитесь, что виртуальные машины AWS, которые нужно перенести, работают под управлением поддерживаемой версии ОС. Для выполнения миграции виртуальные машины AWS обрабатываются как физические компьютеры. Просмотрите [поддерживаемые операционные системы](https://docs.microsoft.com/azure/site-recovery/vmware-physical-azure-support-matrix#replicated-machines) для рабочего процесса миграции физического сервера. Прежде чем приступить к реальной миграции, мы рекомендуем выполнить тестовую миграцию (тестовую отработку отказа), чтобы проверить, работает ли виртуальная машина должным образом.
- Убедитесь, что виртуальные машины AWS соответствуют [поддерживаемым конфигурациям](https://docs.microsoft.com/azure/migrate/migrate-support-matrix-physical-migration#physical-server-requirements) для переноса в Azure.
- Убедитесь, что виртуальные машины AWS, которые реплицируются в Azure, соответствуют [требованиям к виртуальным машинам Azure](https://docs.microsoft.com/azure/migrate/migrate-support-matrix-physical-migration#azure-vm-requirements).
...
## <a name="2-prepare-azure-resources-for-migration"></a>2. Подготовка ресурсов Azure для миграции
...
## <a name="3-prepare-aws-instances-for-migration"></a>3. Подготовка экземпляров AWS к переносу
...
- Настройте отдельную виртуальную машину EC2 для размещения устройства репликации. Этот экземпляр должен работать в Windows Server 2012 R2 или Windows Server 2016. [Ознакомьтесь](https://docs.microsoft.com/azure/migrate/migrate-replication-appliance#appliance-requirements) с требованиями к оборудованию, программному обеспечению и сети для устройства.
- Устройство не следует устанавливать на исходной виртуальной машине, которую вы намерены реплицировать. Оно должно быть развернуто на другой виртуальной машине.
...
## <a name="4-add-the-server-migration-tool"></a>4. Добавление средства "Миграция сервера"
...
## <a name="5-set-up-the-replication-appliance"></a>5. Настройка устройства репликации
...
## <a name="6-install-the-mobility-service"></a>6. Установка службы Mobility
...
3. Найдите версию установщика, которая соответствует ОС исходной виртуальной машины AWS. Проверьте [поддерживаемые операционные системы](https://docs.microsoft.com/azure/site-recovery/vmware-physical-azure-support-matrix#replicated-machines).
...
## <a name="7-enable-replication-for-aws-vms"></a>7. Включение репликации для виртуальных машин AWS
...
6. В поле **Guest credentials** (Учетные данные гостя) укажите фиктивную учетную запись, которая будет использоваться для установки службы "Мобильность" вручную (принудительная установка не поддерживается). Выберите **Next: Виртуальные машины**.
...
12. В разделе **Вычисления** просмотрите имя виртуальной машины, размер, тип диска ОС и группу доступности. Виртуальные машины должны соответствовать [требованиям Azure](migrate-support-matrix-physical-migration.md#azure-vm-requirements).
...
## <a name="8-track-and-monitor-replication-status"></a>8. Отслеживание и мониторинг состояния репликации
...
## <a name="9-run-a-test-migration"></a>9. Выполнение тестовой миграции
...
## <a name="10-migrate-aws-vms"></a>10. Миграция виртуальных машин AWS
...
2. Установите агент [Linux](https://docs.microsoft.com/azure/virtual-machines/extensions/agent-linux) на перенесенных компьютерах. Агент Windows виртуальной машины Azure предварительно устанавливается в процессе миграции.
...
**Ответ.** Проверьте поддерживаемые операционные системы для виртуальных машин AWS.  
