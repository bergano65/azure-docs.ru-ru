...
Этот разрыв можно устранить, включив [Обнаружение приложений](./how-to-discover-applications.md) на виртуальных машинах VMware. При оценке сервера используется операционная система, обнаруженная на виртуальной машине, с использованием предоставленных учетных данных гостя. Эти данные операционной системы определяют правильные сведения о ОС в случае виртуальных машин Windows и Linux.
...
## <a name="azure-disk-skus-bigger-than-on-premises-in-an-azure-vm-assessment"></a>Номера SKU дисков Azure больше, чем в локальной среде, в оценке виртуальных машин Azure.
...
## <a name="low-confidence-rating-on-physical-server-assessments"></a>Низкая степень достоверности при оценке физических серверов
Оценка назначается на основе доступности точек данных, необходимых для расчета оценки. В случае с физическими серверами, на которых включена виртуализация Hyper-V, существует известный недостаток продукта из-за того, что оценка низкой достоверности может быть неправильно назначена оценкам физического сервера. На этих серверах сейчас служба "миграция Azure" обнаруживает как физические, так и виртуальные адаптеры. Пропускная способность сети захватывается на обнаруженных виртуальных сетевых адаптерах, но не на физических сетевых адаптерах. Из-за отсутствия точек данных на физических сетевых адаптерах может снизиться Оценка достоверности, что приводит к низкой оценке. Это разрыв продукта, который будет рассмотрен в дальнейшем.
