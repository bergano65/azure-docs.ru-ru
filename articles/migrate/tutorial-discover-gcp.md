...
**Требование** | **Сведения**
--- | ---
**Устройство** | Вам потребуется экземпляр виртуальной машины GCP, на котором будет запущено устройство службы "Миграция Azure". На компьютере должны быть следующие компоненты:<br/><br/> — Установлен Windows Server 2016.<br/> _Запуск модуля на компьютере с Windows Server 2019 не поддерживается._<br/><br/> — 16 ГБ ОЗУ, 8 виртуальных ЦП, около 80 ГБ дискового пространства и внешний виртуальный коммутатор.<br/><br/> — Статический или динамический IP-адрес, а также доступ к Интернету напрямую или через прокси-сервер.
