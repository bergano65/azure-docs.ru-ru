...
**Создание приложений Azure AD** | При регистрации виртуального модуля служба "Миграция Azure" создает два приложения Azure Active Directory (Azure AD). <br/><br/> — Первое приложение используется для обмена данными между агентами, выполняющимися на устройстве и в службе "Миграция Azure". <br/><br/> — Второе приложение используется исключительно для доступа к хранилищу Key Vault, созданному в подписке пользователя для миграции виртуальных машин VMware без агента.   | Учетная запись Azure должна иметь эти [разрешения](https://docs.microsoft.com/azure/migrate/tutorial-prepare-vmware#assign-permissions-to-create-azure-ad-apps) на создание приложений Azure AD.
**Создание Key Vault** | — Первое хранилище Key Vault создается при регистрации модуля и используется для управления сертификатом, скачанным в модуль во время его настройки. <br/><br/> – Чтобы выполнить миграцию виртуальных машин VMware без агента, служба "Миграция Azure" создает другое хранилище Key Vault, которое используется для управления ключами доступа к учетной записи репликации в подписке.| Чтобы разрешить службе "Миграция Azure" создать Key Vault, установите разрешения (владельца или участника и администратора доступа пользователей) в группе ресурсов, в которой находится проект службы "Миграция Azure".
