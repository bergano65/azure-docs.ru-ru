...
- Перенесите виртуальные машины Hyper-V или VMware, если по какой-либо причине вы не можете использовать стандартный процесс миграции [Hyper-V](tutorial-migrate-hyper-v.md)или [VMware](server-migrate-overview.md).
...
> [!NOTE]
> Миграция на основе агента с помощью средства миграции сервера службы "Миграция Azure" имеет ту же архитектуру репликации, что и функция аварийного восстановления на основе агента службы Azure Site Recovery, и некоторые из применяемых компонентов используют одну и ту же базу кода. Некоторые требования могут ссылаться на документацию Site Recovery.
...
2. Проверьте настройки виртуальной машины. Локальные виртуальные машины, которые реплицируются в Azure, должны соответствовать [требованиям к виртуальным машинам Azure](migrate-support-matrix-physical-migration.md#azure-vm-requirements).
...
Перед началом необходимо подготовить компьютер под управлением Windows Server 2016 для размещения устройства репликации. Компьютер должен соответствовать [таким требованиям](migrate-replication-appliance.md). Устройство не должно быть установлено на исходном компьютере, который вы намерены защитить.
...
11. Запустите файл установки устройства репликации, как описано в следующей процедуре. После установки будет автоматически запущен мастер настройки модуля (мастер можно также запустить вручную с помощью ярлыка cspsconfigtool, который создается на рабочем столе модуля). Используйте вкладку "Управление учетными записями" мастера, чтобы добавить сведения об учетной записи для принудительной установки службы "Мобильность". В рамках этого руководства мы будем устанавливать службу "Мобильность" вручную на компьютерах, которые нужно реплицировать. Поэтому создайте на этом этапе фиктивную учетную запись и продолжайте работу.
...
6. В поле **Учетные данные гостя** укажите фиктивную учетную запись, которая будет использоваться для установки службы "Мобильность" вручную (физические серверы не поддерживают принудительную установку). Выберите **Next: Виртуальные машины**.
