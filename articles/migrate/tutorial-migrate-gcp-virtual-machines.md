---
title: Обнаружение, оценка готовности и миграция экземпляров виртуальных машин Google Cloud Platform (GCP) в Azure
description: В этой статье описывается, как перенести виртуальные машины GCP в Azure с помощью службы "Миграция Azure".
ms.topic: tutorial
ms.date: 08/19/2020
ms.custom: MVC
...
В этом руководстве вы узнаете, как обнаруживать, оценивать готовность и переносить виртуальные машины Google Cloud Platform (GCP) в виртуальные машины Azure, используя средство оценки серверов оценка сервера" и "Миграция Azure: миграция сервера".
...
> * Подготовка экземпляров виртуальных машин GCP к миграции.
...
> * Установка службы "Мобильность" на виртуальных машинах GCP, которые нужно перенести.
...
Перед миграцией в Azure рекомендуется выполнить обнаружение виртуальных машин и оценку миграции. Эта оценка помогает выбрать соответствующий размер виртуальных машин GCP для миграции в Azure и рассчитать потенциальные расходы на выполнение в Azure.

Настройте оценку следующим образом:

1. Выполните инструкции из этого [учебника](./tutorial-discover-gcp.md), чтобы настроить Azure и подготовить виртуальные машины GCP к оценке. Обратите внимание на следующее.

    - Миграция Azure использует аутентификацию с паролем при обнаружении экземпляров виртуальных машин GCP. По умолчанию экземпляры GCP не поддерживают проверку подлинности с использованием пароля. Прежде чем выполнять обнаружение, включите проверку подлинности с использованием пароля.
...
2. Затем следуйте инструкциям в этом [учебнике](./tutorial-assess-gcp.md), чтобы настроить проект и устройство Миграции Azure для обнаружения и оценки виртуальных машин GCP.
...
- Убедитесь, что виртуальные машины GCP, которые нужно перенести, работают под управлением поддерживаемой версии ОС. Для выполнения миграции виртуальные машины GCP обрабатываются как физические компьютеры. Просмотрите [поддерживаемые операционные системы и версии ядра](../site-recovery/vmware-physical-azure-support-matrix.md#replicated-machines) для рабочего процесса миграции физического сервера. Для проверки версий ОС и ядра для виртуальных машин Linux можно использовать стандартные команды, такие как *hostnamectl* или *uname -a* .  Прежде чем приступить к реальной миграции, мы рекомендуем выполнить тестовую миграцию, чтобы проверить, работает ли виртуальная машина должным образом.
- Убедитесь, что виртуальные машины GCP соответствуют [поддерживаемым конфигурациям](./migrate-support-matrix-physical-migration.md#physical-server-requirements) для переноса в Azure.
- Убедитесь, что виртуальные машины GCP, которые реплицируются в Azure, соответствуют [требованиям к виртуальным машинам Azure](./migrate-support-matrix-physical-migration.md#azure-vm-requirements).
...
## <a name="prepare-gcp-instances-for-migration"></a>Подготовка экземпляров GCP к миграции

Чтобы подготовиться к переносу GCP в Azure, необходимо подготовить и развернуть устройство репликации для миграции.
...
- **Сервер конфигурации** : Сервер конфигурации используется для управления обменом данными между виртуальными машинами GCP и Azure, а также репликацией данных.
...
- Настройте отдельную виртуальную машину GCP для размещения устройства репликации. Этот экземпляр должен работать в Windows Server 2012 R2 или Windows Server 2016. [Ознакомьтесь](./migrate-replication-appliance.md#appliance-requirements) с требованиями к оборудованию, программному обеспечению и сети для устройства.
...
7. В разделе **Сведения о проекте** укажите имя проекта и регион для создания проекта, а затем щелкните **Далее** . Просмотрите список поддерживаемых регионов для [общедоступного](migrate-support-matrix.md#supported-geographies-public-cloud) облака и облака для [государственных организаций](migrate-support-matrix.md#supported-geographies-azure-government).
    - Географический регион проекта используется только для хранения обнаруженных метаданных машин GCP.
...
Первым шагом миграции является настройка устройства репликации. Чтобы настроить устройство для миграции виртуальных машин GCP, необходимо скачать файл установщика для устройства, а затем запустить его на [подготовленной вами виртуальной машине](#prepare-a-machine-for-the-replication-appliance).
...
8. Скопируйте файл установки устройства и файл ключа на виртуальную машину GCP под управлением Windows Server 2016 или Windows Server 2012, которую вы создали для устройства репликации.
...
Агент службы "Мобильность" должен быть установлен на исходных виртуальных машинах GCP, подлежащих миграции. Установщики агента доступны на устройстве репликации. Найдите правильный установщик и установите агент на каждой машине, которую хотите перенести. Сделайте следующее:

1. Войдите на устройство репликации.
2. Перейдите по ссылке **%ProgramData%\ASR\home\svsystems\pushinstallsvc\repository** .
3. Найдите версию установщика, которая соответствует ОС исходной виртуальной машины GCP. Проверьте [поддерживаемые операционные системы](../site-recovery/vmware-physical-azure-support-matrix.md#replicated-machines).
4. Скопируйте файл установщика на исходную виртуальную машину GCP, которую вы хотите перенести.
...
### <a name="installation-guide-for-windows-gcp-vms"></a>Руководство по установке виртуальных машин Windows GCP

1. Извлеките содержимое файла установщика в локальную папку (например, C:\Temp) на виртуальной машине GCP следующим образом:
...
### <a name="installation-guide-for-linux-gcp-vms"></a>Руководство по установке виртуальных машин Linux GCP

1. Извлеките содержимое файла tarball установщика в локальную папку (например, /tmp/MobSvcInstaller) на виртуальной машине GCP следующим образом:
    ```
...
## <a name="enable-replication-for-gcp-vms"></a>Включение репликации для виртуальных машин GCP
...
- После завершения начальной репликации начинается разностная репликация. Добавочные изменения дисков виртуальной машины GCP периодически реплицируются на диски реплики в Azure.
...
- Запуск тестовой миграции позволяет проверить, что миграция будет работать должным образом, не затрагивая виртуальные машины GCP, которые остаются работоспособными и продолжают репликацию.
...
## <a name="migrate-gcp-vms"></a>Миграция виртуальных машин GCP

Убедившись, что тестовая миграция работает должным образом, вы можете выполнить миграцию виртуальных машин GCP.
...
1. После завершения миграции щелкните правой кнопкой мыши виртуальную машину и выберите **Stop migration** (Остановка миграции). Таким образом вы сделаете следующее:
    - Прекратите выполнение репликации для виртуальной машины GCP.
    - Удалите виртуальную машину GCP из числа **реплицируемых серверов** в средстве для миграции серверов: Миграции Azure.
...
**Вопрос** . В списке обнаруженных серверов для миграции не отображается виртуальная машина GCP.   
**Ответ.** Проверьте, соответствует ли ваше устройство репликации требованиям. Убедитесь, что агент мобильности установлен на исходной виртуальной машине, предназначенной для миграции, и зарегистрирован на сервере конфигурации. Проверьте правила брандмауэра, чтобы включить сетевой путь между устройством репликации и исходными виртуальными машинами GCP.  
...
**Вопрос** . При попытке обнаружить мои виртуальные машины GCP я получаю ошибку "Failed to fetch BIOS GUID" (Не удалось получить глобальный уникальный идентификатор BIOS).   
**Ответ.** Используйте для проверки подлинности имя входа root и не используйте псевдоимя пользователя. Если вы не можете использовать привилегированного пользователя, убедитесь, что для пользователя установлены необходимые возможности в соответствии с инструкциями, приведенными в [таблице поддержки](migrate-support-matrix-physical.md#physical-server-requirements). Также проверьте поддерживаемые операционные системы для виртуальных машин GCP.  
...
**Вопрос** . Мне не удается обнаружить экземпляры GCP с помощью Миграции Azure из-за кода состояния HTTP 504 из удаленной службы управления Windows.    
...
**Вопрос** . Нужно ли вносить изменения перед переносом моих виртуальных машин GCP в Azure?   
**Ответ.** Возможно, перед переносом виртуальных машин EC2 в Azure вам потребуется внести приведенные ниже изменения.

- Если вы используете cloud-init для подготовки виртуальной машины, вам может потребоваться отключить cloud-init на виртуальной машине перед его репликацией в Azure. Действия по подготовке, выполняемые cloud-init на виртуальной машине, могут быть доступны только для GCP и станут недействительными после миграции в Azure.  
- Ознакомьтесь с разделом [предварительных требований](#prerequisites), чтобы определить, нужно ли внести ли какие-либо изменения для операционной системы, прежде чем перенести виртуальные машины в Azure.
