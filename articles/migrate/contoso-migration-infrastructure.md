...
--- | --- | ---
[Статья 1. Миграция Contoso: общие сведения](contoso-migration-overview.md) | Обзор серии статей, стратегии миграции компании Contoso и используемых в этой серии примеров приложений. | Доступна
Статья 2. Развертывание инфраструктуры Azure | Компания Contoso готовит локальную инфраструктуру и инфраструктуру Azure для миграции. Для всех статей миграции в серии используется одна и та же инфраструктура. | Эта статья
[Статья 3. Миграция компании Contoso. Оценка локальных рабочих нагрузок для миграции в Azure](contoso-migration-assessment.md)  | Компания Contoso выполняет оценку локального приложения SmartHotel360 в VMware. Для оценки виртуальных машин приложения Contoso использует службу "Миграция Azure". Для оценки базы данных SQL Server приложения используется Помощник по миграции данных. | Доступна
...
[Статья 5. Миграция приложения компании Contoso: повторное размещение локального приложения на виртуальных машинах Azure](contoso-migration-rehost-vm.md) | Специалисты компании Contoso переносят виртуальные машины приложения SmartHotel360 на виртуальные машины Azure с помощью службы Site Recovery. | Доступна
...
[Статья 13. Миграция в компании Contoso: повторное создание локального приложения для использования в Azure](contoso-migration-rebuild.md) | Специалисты компании Contoso выполняют повторную сборку приложения SmartHotel, используя ряд возможностей и служб Azure, включая Службу приложений Azure, Службу Azure Kubernetes (AKS), Функции Azure, Cognitive Services и Azure Cosmos DB. | Доступна
[Статья 14. Contoso: масштабирование миграции в Azure](contoso-migration-scale.md) | Ознакомившись с вариантами сочетаний миграции, компания Contoso готовится к полномасштабной миграции в Azure. | Доступна 
...
**Шаг 2. Гибридное удостоверение**. Как будет осуществляться управление доступом к ресурсам Azure и локальным ресурсам после миграции? Как обеспечить поддержку управления идентификацией в облаке или полностью перейти на такое управление?  
...
- Office 365 использует Azure AD для управления пользователями. Если бы компания Contoso использовала Office 365, у нее уже был бы клиент Azure AD и она могла бы использовать его в качестве основного экземпляра службы AD.
...
1. После распространения записей DNS в окне сведений об имени была нажата кнопка **Проверить** для проверки имени личного домена.
...
1. В группе ресурсов **ControlCobRG** специалисты компании щелкают **Управление доступом (IAM)** > **Добавить назначение ролей**.
2. Они щелкают **Добавить назначение ролей** > **Роль**, > **Участник**, а затем выбирают из списка группу AD под названием **ContosoCobRG**. После этого группа появляется в списке **Выбранные элементы**. 
...
Очень важно, чтобы спроектированные компанией Contoso сети обеспечивали безопасность и масштабируемость гибридного развертывания. Для этого Contoso реализует долгосрочный подход, гарантирующий устойчивость проектируемых виртуальных сетей, и их готовность к внедрению на предприятии. Подробные сведения о планировании виртуальных сетей см. в этой [статье](https://docs.microsoft.com/azure/virtual-network/virtual-network-vnet-plan-design-arm).
...
- **Концентратор**. Виртуальная сеть концентратора в восточной части США 2 является центральной точкой основного подключения к локальному центру обработки данных.
...
- **Концентратор**. Виртуальная сеть концентратора в восточной части США 2 является центральной точкой основного подключения к локальному центру обработки данных, а периферийные виртуальные сети в том же регионе можно при необходимости использовать для изоляции рабочих нагрузок, если ими нужно управлять отдельно от других периферийных виртуальных сетей.
...
- **Подсети**. Эти подсети будут спроектированы подобно подсетям в восточной части США 2. за тем исключением, что здесь компании Contoso не требуется подсеть для контроллеров домена.
...
- **Блокировки**. Azure позволяет блокировать подписки, группы ресурсов и другие ресурсы, чтобы их могли изменять только пользователи с необходимыми полномочиями.
...
- **Центр безопасности Azure**. Центр безопасности Azure обеспечивает унифицированное управление безопасностью и расширенную защиту от угроз для гибридных облачных рабочих нагрузок. С помощью центра безопасности можно применять политики безопасности для рабочих нагрузок, ограничить уязвимость перед угрозами, а также выявлять атаки и реагировать на них.  [Узнайте больше](https://docs.microsoft.com/azure/security-center/security-center-intro).
- **Группы безопасности сети (NSG)**. Представляют собой фильтр (брандмауэр), содержащий перечень правил безопасности, которые (в случае их применения) разрешают или запрещают передачу сетевого трафика ресурсам, подключенным к виртуальным сетям Azure. [Узнайте больше](https://docs.microsoft.com/azure/virtual-network/security-overview).
- **Шифрование данных**. Шифрование дисков Azure — это возможность, которая помогает зашифровать диски виртуальных машин IaaS под управлением Windows и Linux. [Узнайте больше](https://docs.microsoft.com/azure/security/azure-security-encryption-atrest).
