...
3. В **разделе 1. Создание ключа проекта для службы "миграция Azure**" укажите имя для устройства "миграция Azure", которое будет настроено для обнаружения виртуальных машин Hyper-V. имя должно состоять из букв и цифр и не больше 14 символов.
...
В **2. Скачайте устройство для переноса Azure**, выберите. VHD-файл и щелкните **скачать**. 

   ![Параметры для обнаружения компьютеров](./media/tutorial-assess-hyper-v/servers-discover.png)


   ![Параметры для создания ключа](./media/tutorial-assess-hyper-v/generate-key-hyperv.png)
...
4. Если учетная запись пользователя Azure, используемая для ведения журнала, имеет правильные [разрешения](tutorial-prepare-hyper-v.md#prepare-azure) на ресурсы Azure, созданные во время создания ключа, будет инициирована регистрация устройства.
...
1. На **шаге 1. Укажите учетные данные узла Hyper-V**, щелкните **Добавить учетные данные** , чтобы указать понятное имя для учетных данных, добавить **имя пользователя** и **пароль** для узла или кластера Hyper-V, которые устройство будет использовать для обнаружения виртуальных машин. Щелкните **Save**(Сохранить).
1. Если вы хотите добавить несколько учетных данных одновременно, щелкните **Добавить еще** , чтобы сохранить и добавить дополнительные учетные данные. Для обнаружения виртуальных машин Hyper-V поддерживается несколько учетных данных.
1. На **шаге 2. Укажите сведения об узле или кластере Hyper-v**, щелкните **Добавить источник обнаружения** , чтобы указать IP-адрес узла или кластера, а также **полное доменное имя (FQDN)** или псевдоним для учетных данных для подключения к узлу или кластеру.
1. Можно **добавить один элемент** за раз или **Добавить несколько элементов** в одном пути. Существует также параметр для предоставления сведений об узле или кластере Hyper-V через **Импорт CSV**.

    ![Параметры для добавления источника обнаружения](./media/tutorial-assess-hyper-v/add-discovery-source-hyperv.png)

    - При выборе **пункта добавить один элемент**необходимо указать понятное имя для учетных данных, а также **IP-адрес** узла или кластера Hyper-V, а затем нажать кнопку **сохранить**.
    ...
    - При выборе **Импорт CSV**-файла можно скачать файл шаблона CSV, заполняя его на IP-адрес узла или кластера, а также его **полное доменное** имя и понятное название для учетных данных. Затем файл импортируется в устройство, **Проверьте** записи в файле и нажмите кнопку **сохранить**.

1. При нажатии кнопки сохранить устройство попытается проверить подключение к добавленным узлам или кластерам Hyper-V и отобразит **состояние проверки** в таблице для каждого узла или кластера.
    - Для успешной проверки узлов и кластеров можно просмотреть дополнительные сведения, щелкнув их IP-адрес или полное доменное имя.
    - Если проверка для узла завершается неудачно, проверьте ошибку, щелкнув ошибка **проверки** в столбце Состояние таблицы. Устраните проблему и повторите проверку.
    - Чтобы удалить узлы или кластеры, нажмите кнопку **Удалить**.
...
1. Нажмите кнопку **запустить обнаружение**, чтобы начать обнаружение виртуальной машины с успешно проверенных узлов и кластеров. После успешной инициализации обнаружения можно проверить состояние обнаружения для каждого узла или кластера в таблице.

Запустится обнаружение. Метаданные обнаруженных серверов появляются в портал Azure примерно 2 минуты на узел.
