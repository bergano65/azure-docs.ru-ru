...
4. Теперь они добавляют сертификат клиента, который будет использовать администратор на кластере. Это позволяет Azure DevOps Services подключаться к кластеру для развертывания приложений в контейнере выпуска. Для этого они открывают хранилище ключей на портале и выбирают пункты **Сертификаты** > **Generate/Import** (Создать или импортировать).
...
1.   В учетной записи Azure DevOps Contoso они создают проект (**ContosoSmartHotelRearchitect**) и выбирают **Git** для управления версиями.
...
10. Они фиксируют обновленный код и отправляют его в Azure DevOps Services.
...
1. В Azure DevOps Services они выбирают пункты **Сборка и выпуск** > **Новый конвейер**.

    ![Новый конвейер](./media/contoso-migration-rearchitect-container-sql/pipeline1.png)

2. Они выбирают **Azure DevOps Services Git** и соответствующий репозиторий.
...
4. Специалисты компании меняют образы тегов действия для **создания образа** и настраивают задачу на использование подготовленного ACR.

     ![Реестр](./media/contoso-migration-rearchitect-container-sql/pipeline4.png)

5. В задаче **Push images** (Отправка образов) они настраивают отправку образа в ACR и включают последний тег.
...
8. После успешной сборки они переходят к конвейеру выпуска. В Azure DevOps Services они выбирают пункты **Выпуски** > **Новый конвейер**.

    ![Конвейер выпуска](./media/contoso-migration-rearchitect-container-sql/pipeline6.png)    

9. Они выбирают шаблон **Развертывание Azure Service Fabric** и указывают имя стадии (**SmartHotelSF**).

    ![Среда](./media/contoso-migration-rearchitect-container-sql/pipeline7.png)

10. Они указывают имя конвейера (**ContosoSmartHotel360Rearchitect**). Для стадии они выбирают вариант **1 задание, 1 задача**, чтобы настроить развертывание Service Fabric.
...
19. Чтобы подключиться к приложению, они направляют трафик по общедоступному IP-адресу подсистемы балансировки нагрузки Azure на узлах Service Fabric.
