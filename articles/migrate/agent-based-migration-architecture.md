---
title: Архитектура миграции на основе агентов в Azure миграция миграции сервера
...
**CPU** | **Память** | **Свободное место для кэширования данных** | **Частота обновлений** | **Ограничения репликации**
--- | --- | --- | --- | ---
...
12 виртуальных ЦП (2 сокета * 6 ядер \@ с частотой 2,5 ГГц), память объемом 24 ГБ | 1 TБ | 1-2 TБ | 151-225 компьютеров.
...
- Параметр реестра HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows Azure Баккуп\репликатион\уплоадсреадспервм указывает количество потоков, используемых для обмена данными (исходная или разностная репликация) на диске. Если задать высокое значение, пропускная способность сети, используемая для репликации, увеличивается. Значение по умолчанию — четыре. Максимальное значение — 32. Следите за трафиком для оптимизации значения.
