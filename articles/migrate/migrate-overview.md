...
-  Проект службы "Миграция Azure" можно создать только в регионе США. Но миграцию можно запланировать в любое целевое расположение Azure.
    - В регионе, где создается проект миграции, сохраняются только метаданные, собранные в локальной среде.
    - Метаданные сохраняются только в одном из следующих регионов в выбранном географическом регионе: центрально-западная часть США или восточная часть США.
    - Если визуализация зависимостей используется путем создания рабочей области Log Analytics, такая рабочая область создается в одном регионе с проектом.
