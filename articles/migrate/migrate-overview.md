...
- Поддержка Hyper-V доступна в предварительной версии с возможностью запуска в рабочей среде. Если вы хотите опробовать эту возможность, выполните вход по [этой ссылке](https://aka.ms/migratefuture).
- Для оценки физических серверов вы можете воспользоваться [средствами наших партнеров](https://azure.microsoft.com/migration/partners/).
- Вы можете обработать до 1500 виртуальных машин при помощи одной операции обнаружения в одном проекте. Доступна предварительная версия, которая позволяет обнаруживать до 10 000 виртуальных машин VMware в одном проекте с помощью одного устройства. Если вы хотите опробовать ее, заполните [эту форму](https://aka.ms/migratefuture).
...
**Свойство** | **Сведения**
--- | ---
**Целевое расположение** | Расположение Azure для миграции.<br/><br/>Сейчас в службе "Миграция Azure" поддерживается 33 региона в качестве целевых расположений для миграции. ([проверьте доступность по регионам](https://azure.microsoft.com/global-infrastructure/services/)). По умолчанию в качестве целевого региона указана восточная часть США.
...
**Серия виртуальной машины** | Серия виртуальной машины VM, используемая для оценки размера. Например, если у вас есть рабочая среда, которую вы не планируете переносить на виртуальные машины Azure серии А, эту серию можно исключить из списка серий. Изменение размера выполняется только для выбранных серий.   
...
|Локальная виртуальная машина | Рабочая область Log Analytics | [Microsoft Monitoring Agent (MMA)](../log-analytics/log-analytics-windows-agent.md) использует TCP-порт 443 для подключения к журналам Azure Monitor. Этот порт нужен, только если используется визуализация зависимостей, для которой требуется агент MMA.|
...
## <a name="want-to-learn-more-from-community-experts"></a>Хотите получить дополнительную информацию от экспертов сообщества?
Посетите форум MSDN [Azure Migrate](https://social.msdn.microsoft.com/Forums/home?forum=AzureMigrate&filter=alltypes&sort=lastpostdesc) или [Stack Overflow](https://stackoverflow.com/search?q=azure+migrate).

## <a name="need-help-contact-us"></a>Требуется помощь? Свяжитесь с нами.  
Если у вас есть вопросы или вам нужна помощь, [создайте запрос в службу поддержки](https://ms.portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest). Если ваш запрос в службу поддержки предполагает развернутые технические рекомендации, сверьтесь со страницей [Сравнение планов поддержки](https://azure.microsoft.com/support/plans/).     
