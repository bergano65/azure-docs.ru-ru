...
Существует два метода, которые модуль сборщика может использовать для обнаружения: однократное или непрерывное обнаружение.


### <a name="one-time-discovery"></a>Однократное обнаружение

Сборщик данных единовременно взаимодействует с vCenter Server для сбора метаданных о виртуальных машинах. При использовании этого метода:
...
- Для этого метода обнаружения необходимо установить для параметров статистики в vCenter Server уровень 3.
- После установки уровня 3 для создания данных счетчиков производительности понадобится один день. Поэтому рекомендуется выполнить обнаружение через один день.
...
- Служба "Миграция Azure" собирает для каждой метрики средние счетчики (не пиковые счетчики), что может привести к уменьшению размера.
