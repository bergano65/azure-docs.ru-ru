...
- Служба "Миграция Azure" собирает счетчик средних значений (а не счетчик пиковых нагрузок) для каждой метрики.

### <a name="continuous-discovery"></a>Непрерывное обнаружение

Модуль сборщика непрерывно подключен к проекту "Миграция Azure".
...
> [!NOTE]
> Функция непрерывного обнаружения доступна в предварительной версии. Если для параметров статистики vCenter Server настроен не 3 уровень, рекомендуется использовать этот метод.
...
net.received.average | 2 | 3 |  Вычисляет размер виртуальной машины и затраты на сеть                        |
net.transmitted.average | 2 | 3 | Вычисляет размер виртуальной машины и затраты на сеть    
