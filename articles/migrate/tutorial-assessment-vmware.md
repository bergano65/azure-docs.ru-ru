...
- **VMware**. Управление виртуальными машинами, которые планируется перенести, должно осуществляться с помощью vCenter Server версии 5.5, 6.0, 6.5 или 6.7. Кроме того, необходим один узел ESXi версии 5.5 или более поздней для развертывания виртуальной машины сборщика.
...
**"География"** | **Расположение хранения**
--- | ---
...
#### <a name="continuous-discovery"></a>Непрерывное обнаружение
  ...
  Для OVA-файла версии 1.0.10.11
...
6. В разделе **Specify vCenter Server details** (Укажите сведения о vCenter Server) выполните следующие действия:
    ...
    - В поле **Область сбора данных** выберите область обнаружения виртуальных машин. Сборщик может обнаруживать только виртуальные машины в указанной области. Областью может быть определенная папка, центр обработки данных или кластер. Область не должна содержать более 1500 виртуальных машин. [Подробнее](how-to-scale-assessment.md) об обнаружении более крупной среды.
       ...
       > [!NOTE]
       > В **области сбора** указаны только папки узлов и кластеров. Папки виртуальных машин нельзя выбрать в качестве области сбора напрямую. Но вы можете выполнить обнаружение, воспользовавшись учетной записью vCenter, которая имеет доступ к отдельным виртуальным машинам. [Узнайте больше](https://docs.microsoft.com/azure/migrate/how-to-scale-assessment#set-up-permissions) о том, как выбрать папку виртуальных машин в качестве области сбора.
...
8. Проверьте статус обнаружения в разделе **Просмотр хода выполнения сбора**. [Дополнительные сведения](https://docs.microsoft.com/azure/migrate/concepts-collector) о том, какие данные собирает сборщик службы "Миграция Azure".
