...
- Если нужно ограничить доступ, назначьте дочерним объектам (узлы vSphere, хранилища данных, виртуальные машины и сети) роль без доступа с параметром распространения на дочерний объект.
...
2. На **компьютерах Discover** доступны два варианта устройств. Чтобы загрузить устройство, соответствующее вашим предпочтениям, нажмите **Загрузить**.

    a. **Однократное обнаружение**. Устройство этой модели связывается с сервером vCenter для сбора метаданных о виртуальных машинах. Для сбора данных о производительности виртуальных машин оно использует исторические данные о производительности, хранящиеся в сервере vCenter, и собирает историю производительности за последний месяц. В этой модели служба "Миграция Azure" собирает счетчик средних значений (по сравнению со счетчиком пиковых нагрузок) для каждой метрики, [Дополнительные сведения] (https://docs.microsoft.com/azure/migrate/concepts-collector#what-data-is-collected). Поскольку это однократное обнаружение, изменения в локальной среде не отражаются после завершения обнаружения. Если необходимо, чтобы изменения отражались, вы должны сделать повторное обнаружение той же среды для того же проекта.

    b. **Непрерывное обнаружение**. Устройство этой модели постоянно профилирует локальную среду, чтобы собрать данные об использовании в реальном времени для каждой виртуальной машины. В этой модели пиковые счетчики собираются для каждой метрики (использование ЦП, памяти и т. д.). Эта модель зависит от параметров статистики сервера vCenter Server для сбора данных о производительности. Вы можете в любое время остановить непрерывное профилирование в устройстве.

    > [!NOTE]
    > Функция непрерывного обнаружения доступна в предварительной версии.
...
Длительность однократного обнаружения зависит от количества виртуальных машин, обнаружение которых выполняется. Как правило, после завершения работы сборщика требуется около часа для сбора данных настройки и производительности на 100 виртуальных машинах. Вы можете создавать оценки (как на основе результатов, так и на основе оценки помещений) сразу после обнаружения.

При непрерывном обнаружении (в предварительной версии) сборщик будет постоянно профилировать локальную среду и отправлять данные о производительности с интервалом в час. Можно просмотреть компьютеры на портале через час после открытия. Настоятельно рекомендуется подождать по крайней мере день до создания оценки производительности для виртуальных машин.
...
- Вы не профилировали свою среду на время создания оценки. Например, если создавать оценки с продолжительностью производительности набора в 1 день, нужно подождать как минимум день после запуска обнаружения сбора всех точек данных.

**Распространенные причины**  

- В течение периода, для которого рассчитывается оценка, несколько виртуальных машин были отключены. Если отключить какие-либо виртуальные машины на некоторое время, мы не сможем собирать данные о производительности за этот период.
