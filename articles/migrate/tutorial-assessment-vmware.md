...
- **Настройки статистики**. Это предварительное условие применимо только к однократному обнаружению модели. Для работы однократного обнаружения перед началом развертывания серверу vCenter Server нужно задать параметры статистики уровня 3. Если указать уровень ниже 3, оценка будет работать, но данные о производительности хранилища и сети собираться не будут. Рекомендации по размеру в этом случае будут основаны на данных производительности ЦП и памяти, а также на данных конфигурации диска и сетевых адаптеров.
...
5. Укажите регион для создания проекта и нажмите кнопку **Создать**. Проект службы "Миграция Azure" можно создать только в регионе США. При этом по-прежнему можно планировать перенос в любое целевое расположение Azure. Метаданные, собранные с локальных виртуальных машин, хранятся только в регионе, который задан для проекта.
...
2. В области **Обнаружение компьютеров** доступно два варианта сборщика. Выберите **Загрузить**, чтобы загрузить требуемый модуль.
    ...
    a. **Однократное обнаружение.** Сборщик для этой модели обнаружения связывается с vCenter Server для сбора метаданных о виртуальных машинах. Данные о производительности виртуальных машин за последний месяц собираются из журнала производительности, хранящегося в vCenter Server. При использовании этой модели служба "Миграция Azure" собирает данные счетчиков средних значений (в отличие от счетчиков пиковых значений) для каждой метрики. [Подробнее](https://docs.microsoft.com/azure/migrate/concepts-collector#what-data-is-collected). Поскольку это однократное обнаружение, изменения в локальной среде не отражаются после завершения обнаружения. Чтобы такие изменения учитывались, необходимо выполнить повторное обнаружение той же среды для того же проекта.
    ...
    Обратите внимание, что устройство только непрерывно собирает данные о производительности, оно не обнаруживает изменения конфигурации в локальной среде (например, добавление, удаление виртуальной машины, добавление диска и т. д.). Чтобы отразить изменения на портале, вы можете сделать следующее:

    1. Добавление элементов (виртуальных машин, дисков, ядер и т. д.): чтобы отразить эти изменения на портале Azure, вы можете остановить обнаружение с устройства, а затем запустить его снова. Это позволит обновить изменения в проекте "Миграция Azure".

    2. Удаление виртуальных машин: из-за особенностей конструкции устройства удаление виртуальных машин не отражается даже при остановке и повторном запуске обнаружение. Это связано с тем, что данные из последующих обнаружений добавляются к более старым обнаружениям, а не переопределяются. В этом случае вы можете просто игнорировать виртуальную машину на портале, удалив ее из своей группы и пересчитав оценку.

    > [!NOTE]
    > Предоставляется предварительная версия функции непрерывного обнаружения. Рекомендуем использовать этот метод, так как он собирает детализированные данные о производительности и приводит к выбору оптимальной конфигурации.
...
При однократном обнаружении длительность процедуры зависит от количества виртуальных машин, обнаружение которых выполняется. Как правило, после завершения работы сборщика требуется около часа для сбора данных настройки и производительности на 100 виртуальных машинах. Вы можете создавать оценки (как на основе результатов, так и на основе оценки помещений) сразу после обнаружения.
...
> [!NOTE]
> При непрерывном обнаружении перед созданием оценки настоятельно рекомендуется подождать по крайней мере один день после начала обнаружения. Если необходимо обновить существующую оценку с помощью последних данных о производительности, можно использовать команду для оценки **Пересчитать**, чтобы ее обновить.
...
> [!NOTE]
> Если оценка достоверности ниже 4 звезд, для модели однократного обнаружения рекомендуем задать для статистики vCenter Server уровень 3, подождать в течение периода, для которого нужно сформировать оценку (1 день, 1 неделя или 1 месяц), а затем выполнить обнаружение и формирование оценки. Для выполнения непрерывного обнаружения модели подождите по крайней мере один день, чтобы устройство могло профилировать среду, а затем *пересчитайте* оценку заново. Если не сделать этого, определение размера на основе производительности может оказаться ненадежным. Рекомендуется переключиться на режим *определения размера как для локальной ВМ*, изменив свойства оценки.
