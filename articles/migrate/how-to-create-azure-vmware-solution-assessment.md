...
3. В поле **Оценка серверов** выберите тип оценки "решение Azure VMware (AVS)" и выберите источник обнаружения.

    :::image type="content" source="./media/how-to-create-avs-assessment/assess-servers-avs.png" alt-text="Основные сведения о добавлении оценки":::

4. Нажмите кнопку **изменить** , чтобы проверить свойства оценки.

    :::image type="content" source="./media/tutorial-assess-vmware-azure-vmware-solution/assess-servers.png" alt-text="Расположение кнопки &quot;Изменить&quot; для просмотра свойств оценки":::

1. В поле **выберите компьютеры для** оценки  >  **имени оценки** > укажите имя оценки. 
 
1. В окне **Выбор или создайте группу** > выберите **создать** и укажите имя группы. Группа собирает одну или несколько виртуальных машин для оценки.
...
1. Щелкните **Далее**, чтобы перейти к этапу **Просмотр и создание оценки** и проверить сведения об оценке.
...
- **Использование на УЗЛАХ AVS**: планируемое использование ЦП, памяти и хранилища на всех узлах.
    ...
    - В настоящее время для памяти, дедупликации и сжатия задано значение 100% использования памяти и 1,5, а также сжатие, которое будет определяться пользователем в последующих выпусках, что позволит пользователю настроить требуемое изменение размера.
...
2. В разделе **Оценки** щелкните оценку, чтобы открыть сведения о ней.
    ...
    :::image type="content" source="./media/how-to-create-avs-assessment/avs-assessment-summary.png" alt-text="Сводка по оценке AVS":::
