...
**Требование** | **Сведения**
--- | ---
**vCenter Server или узел ESXi** | Вам потребуется сервер vCenter Server версии 5.5, 6.0, 6.5 или 6.7.<br/><br/> Виртуальные машины следует разместить на узле ESXi версии 5.5 или более поздней.<br/><br/> Разрешите входящие подключения через TCP-порт 443 в vCenter Server, чтобы с помощью модуля собирать данные о конфигурации и производительности.<br/><br/> По умолчанию устройство подключается к vCenter через порт 443. Если vCenter Server ожидает передачи данных через другой порт, вы сможете изменить порт при указании сведений о vCenter Server в диспетчере конфигурации устройства.<br/><br/> На сервере ESXi, на котором размещены виртуальные машины, обязательно разрешите входящий доступ через TCP-порт 443 для обнаружения приложений, установленных на виртуальных машинах, и зависимостей виртуальных машин.
**Устройство** | Чтобы выделить ВМ для устройства службы "Миграция Azure", серверу vCenter Server понадобятся такие ресурсы:<br/><br/> — 32 ГБ ОЗУ, 8 виртуальных ЦП, 80 ГБ дискового пространства.<br/><br/> — Внешний виртуальный коммутатор и доступ к Интернету для виртуальной машины устройства (напрямую или через прокси-сервер).
**Виртуальные машины** | Поддерживаются все версии ОС Windows и Linux для обнаружения метаданных конфигурации и производительности, а также для обнаружения приложений, установленных на виртуальных машинах. <br/><br/> [Здесь](migrate-support-matrix-vmware.md#dependency-analysis-requirements-agentless) можно узнать о версиях ОС, поддерживаемых для анализа зависимостей без агента.<br/><br/> Чтобы вы могли обнаруживать установленные приложения и зависимости виртуальных машин, на виртуальных машинах должны быть установлены средства VMware Tools (версии старше 10.2.0), а на виртуальных машинах Windows — PowerShell версии 2.0 или более поздней.
...
- разрешениями владельца или участника, а также разрешениями администратора доступа пользователей в подписке Azure для создания Key Vault, используемого во время миграции VMware без агента.
...
Для обнаружения и оценки виртуальных машин VMware средству "Оценка сервера" требуется учетная запись vCenter Server только для чтения. Если вы также хотите обнаруживать установленные приложения и зависимости виртуальных машин, для учетной записи необходимо включить права в разделе **Virtual Machines > Guest Operations** (Виртуальные машины > Гостевые операции).
...
6. Если вы также хотите обнаруживать установленные приложения и зависимости виртуальных машин, перейдите к пункту **Roles** (Роли), выберите **Read-only** (Только для чтения) и в разделе **Privileges** (Привилегии) выберите **Guest Operations** (Гостевые операции). Вы можете распространить разрешения на все объекты в vCenter Server, установив флажок Propagate to children (Распространить на дочерние объекты).
...
Для обнаружения установленных приложений и зависимостей виртуальных машин требуется учетная запись пользователя с необходимыми правами на виртуальных машинах. Учетную запись пользователя можно указать в диспетчере конфигурации устройства. Устройство не устанавливает какие-либо агенты на виртуальных машинах.

1. Для виртуальных машин Windows создайте учетную запись (локальную или доменную) с разрешениями администратора на виртуальных машинах.
2. Для виртуальных машин Linux создайте учетную запись с разрешениями уровня привилегированного пользователя. Или же вы можете создать учетную запись с такими разрешениями для файлов /bin/netstat и /bin/ls: CAP_DAC_READ_SEARCH и CAP_SYS_PTRACE.

> [!NOTE]
> В настоящее время служба "Миграция Azure" поддерживает одну учетную запись пользователя для виртуальных машин Windows и одну учетную запись пользователя для виртуальных машин Linux. Их можно использовать на устройстве для обнаружения установленных приложений и зависимостей виртуальных машин.
...
Миграция Azure Средство "Оценка сервера" использует упрощенное устройство Миграции Azure. Устройство выполняет обнаружение виртуальных машин и отправляет метаданные об их конфигурации и производительности в службу "Миграция Azure". Устройство можно настроить путем развертывания шаблона OVA, который можно скачать из проекта Миграции Azure.

> [!NOTE]
> Если по какой-либо причине вы не можете настроить устройство с помощью шаблона, настройте его с помощью скрипта PowerShell на существующем сервере Windows Server 2016. [Подробнее](deploy-appliance-script.md#set-up-the-appliance-for-vmware).
...
1. В разделе **Шаг 1. Provide vCenter Server credentials** (Предоставление учетных данных vCenter Server) щелкните **Добавить учетные данные**, чтобы указать понятное имя для учетных данных, добавьте значения для пунктов **Имя пользователя** и **Пароль** для учетной записи vCenter Server, которую виртуальный модуль будет использовать для обнаружения виртуальных машин в экземпляре vCenter Server.
    - У вас должна быть учетная запись, настроенная с необходимыми разрешениями согласно инструкциям в предыдущем руководстве.
...
1. На **шаге 3 Provide VM credentials to discover installed applications and to perform agentless dependency mapping** (Предоставление учетных данных виртуальной машины для обнаружения установленных приложений и выполнения сопоставления зависимостей без агента) щелкните **Добавить учетные данные** и укажите операционную систему, для которой предоставляются учетные данные, понятное имя учетных данных и значения для пунктов **Имя пользователя** и **Пароль**. Затем нажмите кнопку **Сохранить**.
    ...
    - При необходимости можете добавить учетные данные, если вы создали учетную запись, которая будет использоваться для [обнаружения приложений](how-to-discover-applications.md) или [анализа зависимостей без агента](how-to-create-group-machine-dependencies-agentless.md).
    ...
    - Проверьте разрешения, необходимые учетной записи для [обнаружения приложений](migrate-support-matrix-vmware.md#application-discovery-requirements) или для [анализа зависимостей без агента](migrate-support-matrix-vmware.md#dependency-analysis-requirements-agentless).
...
- После завершения обнаружения виртуальных машин вы можете включить на портале анализ зависимостей без агента на нужных виртуальных машинах.


## <a name="next-steps"></a>Дальнейшие действия

- [Оцените готовность виртуальных машин VMware](./tutorial-assess-vmware-azure-vm.md) к переносу на виртуальные машины Azure.
