---
title: Оценка готовности экземпляров виртуальных машин GCP к миграции в Azure с помощью средства оценки серверов службы "Миграция Azure"
description: В этой статье описывается, как оценивать готовность экземпляров виртуальных машин GCP к миграции в Azure с помощью средства оценки серверов службы "Миграция Azure".
author: rashi-ms
ms.author: rajosh
ms.manager: abhemraj
ms.topic: tutorial
...
ms.locfileid: "96753320"
---
# <a name="tutorial-assess-google-cloud-platform-gcp-vm-instances-for-migration-to-azure"></a>Руководство по Оценка готовности экземпляров виртуальных машин Google Cloud Platform (GCP) к миграции в Azure

В рамках перехода на Azure вам нужно оценить локальные рабочие нагрузки, чтобы определить готовность облака, выявить риски и оценить затраты и сложность.

В этой статье описывается, как оценивать готовность экземпляров виртуальных машин Google Cloud Platform (GCP) к миграции в Azure с помощью средства оценки серверов службы "Миграция Azure".
...
- Перед выполнением действий, описанных в этом учебнике, выполните инструкции из первого учебника этой серии, чтобы [обнаружить локальные данные инвентаризации](tutorial-discover-gcp.md). 


## <a name="decide-which-assessment-to-run"></a>Выбор оценки для выполнения

Решите, следует ли выполнять оценку с помощью критериев определения размера, основанных на данных производительности или данных/метаданных конфигурации компьютера, которые были собраны локально в исходном виде.

**Оценка** | **Сведения** | **Рекомендация**
--- | --- | ---
**Как в локальной среде** | Оценка на основе данных или метаданных конфигурации компьютера.  | Рекомендуемый размер виртуальной машины Azure подбирается на основе размера локальной виртуальной машины.<br/><br> Рекомендуемый тип диска Azure зависит от того, какое значение вы выбрали для параметра типа хранилища при выполнении оценки.
**На основе производительности** | Оценки на основе собранных данных производительности. | Рекомендуемый размер виртуальной машины Azure подбирается на основе данных об использовании ЦП и памяти.<br/><br/> Рекомендуемый тип диска зависит от операций ввода-вывода в секунду и пропускной способности локальных дисков.
...
1. На странице **Серверы** > **Серверы Windows и Linux** щелкните **Оценка и миграция серверов**.
   ...
   ![Расположение кнопки "Оценка и миграция серверов"](./media/tutorial-assess-gcp/assess.png)

2. В **средстве оценки серверов службы "Миграция Azure" щелкните **Оценка**.

    ![Расположение кнопки оценки](./media/tutorial-assess-gcp/assess-servers.png)
...
6. Щелкните **Просмотреть все**, чтобы просмотреть свойства оценки.
    ...
    ![Расположение кнопки "Просмотреть все" для просмотра свойств оценки](./media/tutorial-assess-gcp/assessment-name.png)
...
8. В поле **Размер виртуальной машины**:
    ...
    - В поле **Фактор комфорта** укажите буфер, который вы хотите использовать во время оценки. Он учитывается, например, для сезонного использования и малого количества записей в журнале с потенциальным повышением в будущем. Например, если вы используете фактор комфорта "2": **Сведения** | **Использование** | **Добавление фактора комфорта (2.0)** Количество операций вода-вывода в секунду при чтении | 100 | 200 операций вода-вывода в секунду при записи | 100 | Пропускная способность при чтении: 200 | 100 Мбит/с | Пропускная способность операций записи: 200 Мбит/с | 100 Мбит/с | 200 Мбит/с
...
10. При внесении изменений нажмите кнопку **Сохранить**.

    ![Свойства оценки](./media/tutorial-assess-gcp/assessment-properties.png)

11. В поле **Оценка серверов** щелкните **Далее**.
12. В разделе **Выбор компьютеров для оценки** выберите **Создать** и укажите имя группы. 
13. Выберите устройство, а также виртуальные машины, которые необходимо добавить в группу. Затем нажмите кнопку **Далее**.
14. На странице **Review + create assessment (Проверка и создание оценки) проверьте сведения об оценке и щелкните **Создать оценку**, чтобы создать группу и запустить оценку.
...
2. В разделе **Оценки** щелкните оценку, чтобы открыть сведения о ней. В качестве примера (оценки и затраты только для примера): 
    ...
    ![Сводка оценки](./media/tutorial-assess-gcp/assessment-summary.png)
...
![Оценка достоверности](./media/tutorial-assess-gcp/confidence-rating.png)
