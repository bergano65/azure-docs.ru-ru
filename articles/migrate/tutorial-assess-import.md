...
**Total disks read ops** (Всего операций чтения с дисков) | Нет | Количество операций чтения в секунду для всех дисков (объединено). <br/> Используйте это поле, если не удается предоставить данные на уровне диска. 
**Total disks write ops** (Всего операций записи на диски) | нет | Количество операций записи в секунду для всех дисков (объединено). <br/> Используйте это поле, если не удается предоставить данные на уровне диска.
**Total disks read throughput** (Общая пропускная способность чтения с дисков) | нет | Скорость чтения данных со всех дисков в МБ/с. <br/> Используйте это поле, если не удается предоставить данные на уровне диска. 
**Total disks write throughput** (Общая пропускная способность записи на диски) | Нет | Скорость записи данных на все диски в МБ/с. <br/> Используйте это поле, если не удается предоставить данные на уровне диска.
...
Если не удается предоставить данные на уровне диска, вы можете предоставить данные о производительности диска для каждого сервера, используя следующие поля. Дополнительные сведения о каждом поле см. в [этом разделе](#add-server-information).
- Total disks read ops (Всего операций чтения с дисков)
- Total disks write ops (Всего операций записи на диски)
- Total disks read throughput (Общая пропускная способность чтения с дисков)
- Total disks write throughput (Общая пропускная способность записи на диски)
