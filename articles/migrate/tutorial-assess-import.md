...
**OS name** (Имя ОС) | Да | Операционная система сервера.
...
**Custom server ID** (Пользовательский идентификатор сервера) | нет | Уникальный идентификатор сервера в локальной среде. <br/> Полезно для отслеживания импортированного сервера по локальному идентификатору.
**Application 1 name** (Имя приложения 1) | нет | Имя рабочей нагрузки, которая выполняется на сервере.<br/>Вы можете добавить сведения о дополнительных приложениях, [добавив столбцы](#add-multiple-applications) в этот шаблон. В общей сложности можно добавить до пяти приложений.
...
**Application 1 license expiry** (Срок действия лицензии приложения 1) | нет | Срок действия лицензии для рабочей нагрузки (если это применимо).
**Business unit** (Бизнес-подразделение) | нет | Бизнес-подразделение, к которому относится этот сервер.
...
- Application 2 name (Имя приложения 2)
- Application 2 type (Тип приложения 2)
- Application 2 version (Версия приложения 2)
- Application 2 license expiry (Срок действия лицензии приложения 2)

> [!NOTE]
> Сведения о приложениях полезны при оценке готовности локальной среды к миграции. Но средство оценки серверов в службе "Миграция Azure" сейчас не выполняет обработку на уровне приложений и не учитывает приложения при создании оценки.
...
6. В разделе **Создание или выбор группы** выберите **Создать** и укажите имя группы. Группа собирает одну или несколько виртуальных машин для оценки.
