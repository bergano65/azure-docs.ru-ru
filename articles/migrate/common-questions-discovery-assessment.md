...
## <a name="how-do-i-choose-the-assessment-type"></a>Разделы справки выбрать тип оценки?
...
- Используйте оценки **решения Azure VMware (AVS)** , если хотите оценить локальные [виртуальные машины VMware](how-to-set-up-appliance-vmware.md) для миграции в [решение Azure VMware (AVS)](https://docs.microsoft.com/azure/azure-vmware/introduction) с помощью этого типа оценки. [Подробнее](concepts-azure-vmware-solution-assessment-calculation.md)

- Вы можете использовать общую группу с компьютерами VMware только для выполнения обоих типов оценки. Обратите внимание, что если вы используете оценку AVS в Azure для миграции в первый раз, рекомендуется создать новую группу компьютеров VMware.
...
- Оценка AVS может выполняться для групп, имеющих только компьютеры VMware. Удалите все компьютеры, не относящиеся к VMware, из группы, если планируется выполнять оценку AVS.
- Если вы используете оценку AVS в Azure для миграции в первый раз, рекомендуется создать новую группу компьютеров VMware.

## <a name="how-do-i-select-ftt-raid-level-in-avs-assessment"></a>Разделы справки выбрать уровень ФТТ-RAID в оценке AVS?

Подсистема хранилища, используемая в AVS, — это vSAN. политики хранилища vSAN определяют требования к хранилищу для виртуальных машин. Эти политики гарантируют требуемый уровень обслуживания для виртуальных машин, так как они определяют способ выделения хранилища для виртуальной машины. Это доступные комбинации ФТТ-RAID: 
...
Для компьютеров, импортированных с помощью CSV-файла, средство миграции по умолчанию в оценке AVS неизвестно. Однако для компьютеров VMware рекомендуется использовать решение гибридного облака VMWare (ХККС). [Подробнее](https://docs.microsoft.com/azure/azure-vmware/hybrid-cloud-extension-installation)
...
Предварительные условия | [Ознакомьтесь](concepts-dependency-visualization.md#agentless-analysis) с требованиями к развертыванию и предварительным требованиям. | [Ознакомьтесь](concepts-dependency-visualization.md#agent-based-analysis) с требованиями к развертыванию и предварительным требованиям.
