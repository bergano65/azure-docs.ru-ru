...
6. В разделе **Сведения о проекте** укажите имя проекта и регион для создания проекта. [Просмотреть регионы](migrate-support-matrix.md#supported-geographies), в которых можно создать проект Миграции Azure.
...
- Устройство можно настроить на виртуальной машине Hyper-V с помощью скачанного виртуального жесткого диска Hyper-V. Кроме того, вы можете настроить модуль на виртуальной машине или физическом компьютере, используя сценарий установщика PowerShell.
- В этом руководстве используется виртуальный жесткий диск. Если вы хотите настроить модуль с помощью сценария, ознакомьтесь с [этой статьей](deploy-appliance-script.md).
