...
2. [Просмотрите требования](migrate-support-matrix-hyper-v-migration.md#hyper-v-hosts) к узлу Hyper-V и URL-адресам Azure, к которым узлам Hyper-V необходим доступ.
3. [Просмотрите требования](migrate-support-matrix-hyper-v-migration.md#hyper-v-vms) к виртуальным машинам Hyper-V, которые вы хотите перенести. Виртуальные машины Hyper-V должны соответствовать [требованиям Azure](migrate-support-matrix-hyper-v-migration.md#azure-vm-requirements).
2. Советуем выполнить инструкции из предыдущего руководства в этой серии. В [первом руководстве](tutorial-prepare-hyper-v.md) показано, как настроить Azure и Hyper-V для миграции. Во втором руководстве показано, как [оценить виртуальные машины Hyper-V](tutorial-assess-hyper-v.md) перед миграцией, используя средство "Оценка сервера" службы "Миграция Azure". 
    > [!NOTE]
    > Хотя мы рекомендуем опробовать оценку, не нужно выполнять оценку перед миграцией виртуальных машин.
    > Для миграции виртуальных машин Hyper-V средство "Миграция сервера" службы "Миграция Azure" запускает агенты программного обеспечения (поставщик Microsoft Azure Site Recovery и агент служб восстановления Microsoft Azure) на узлах Hyper-V или узлах кластера для оркестрации и репликации данных в службу "Миграция Azure". [Модуль службы "Миграция Azure"](migrate-appliance.md) не используется для миграции Hyper-V.

3. Убедитесь, что учетной записи Azure назначена роль участника виртуальной машины, что позволит получить:
...
4. [Настройте сеть Azure](../virtual-network/manage-virtual-network.md#create-a-virtual-network). В процессе миграции в Azure созданные виртуальные машины Azure присоединяются к сети Azure, указанной при настройке миграции.
