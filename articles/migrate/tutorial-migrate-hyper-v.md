...
Если вы не изучили второе руководство для оценки виртуальных машин Hyper-V, [следуйте этим инструкциям](how-to-add-tool-first-time.md), чтобы настроить проект службы "Миграция Azure" и добавить средство оценки серверов в проект.
...
Средство "Миграция сервера" службы "Миграция Azure" запускает агент программного обеспечения на узлах Hyper-V или узлах кластера, чтобы управлять процессами и реплицировать данные в службу "Миграция Azure". Это средство не требует выделенного модуля для миграции.

- Модуль для оценки сервера в службе "Миграция Azure" выполняет обнаружение виртуальных машин и инициирует отправку метаданных и данных производительности в средство "Оценка серверов" службы "Миграция Azure".
- Управление миграцией и репликация данных выполняются поставщиком Microsoft Azure Site Recovery и агентом службы Microsoft Azure Site Recovery.
