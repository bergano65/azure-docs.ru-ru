...
**Собранные данные** |  Визуализация зависимостей без агента работает путем записи данных о подключении TCP с компьютеров, для которых он включен. После запуска обнаружения зависимостей устройство собирает эти данные с компьютеров путем опроса каждые пять минут:<br/> — TCP-подключения.<br/> — Имена процессов, имеющих активные соединения.<br/> — Имена установленных приложений, которые запускают процесс с активными подключениями.<br/> — Число подключений, обнаруженных при каждом интервале опроса.
