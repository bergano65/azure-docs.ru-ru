...
**Создание Key Vault** | Key Vault создается при регистрации виртуального модуля и используется для управления сертификатом, скачанным в модуль во время его настройки.| Чтобы разрешить службе "Миграция Azure" создать Key Vault, у вашей четной записи должны быть разрешения участника для группы ресурсов, в которой находится проект службы "Миграция Azure".
