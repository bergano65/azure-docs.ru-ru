...
1. На вкладке **Оценка серверов** > **Тип оценки** выберите **Azure VMware Solution (AVS) (Preview)** (Решение Azure VMware (AVS) (предварительная версия)).
...
1. В разделе **Свойства оценки** > **Свойства целевого объекта**:
    ...
    - В поле **Целевое расположение** выберите регион Azure, в который необходимо выполнить миграцию.
       ...
       - Сейчас вы можете выполнять оценку в четырех регионах ("Восточная Австралия", "Восточная часть США", "Западная Европа", "Западная часть США").
...
- Использование ресурсов на узлах AVS: планируемое использование ЦП, памяти и хранилища на всех узлах.
    ...
    - Для памяти сейчас установлен уровень использования 100 %, для дедупликации и сжатия — 1,5. В последующих выпусках эти входные данные будут определяться пользователем, что позволит более точно настроить требуемый размер.
