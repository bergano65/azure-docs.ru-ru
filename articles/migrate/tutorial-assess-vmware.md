---
title: Оценка готовности виртуальных машин VMware к миграции в Azure
...
Средство оценки серверов службы "Миграция Azure" использует упрощенный модуль Миграции Azure. Этот модуль выполняет обнаружение виртуальных машин и отправляет метаданные, а также данные производительности в службу "Миграция Azure".
- Модуль можно настроить на виртуальной машине VMware с помощью загруженного шаблона OVA. Кроме того, вы можете настроить модуль на виртуальной машине или физическом компьютере, используя сценарий установщика PowerShell.
- В рамках этого руководства используется шаблон OVA. Если вы хотите настроить модуль с помощью сценария, ознакомьтесь с [этой статьей](deploy-appliance-script.md).
...
4. В разделе **Обнаружение приложений и зависимостей на виртуальных машинах** при необходимости щелкните элемент **Добавить учетные данные** и укажите операционную систему, к которой относятся учетные данные, а также имя пользователя и пароль для этих учетных данных. Затем нажмите кнопку **Добавить**.
