...
Чтобы задать область, необходимо выполнить указанные ниже действия.
1.  Создать учетную запись vCenter.
2.  Определить новую роль с необходимыми разрешениями. (<em>требуется для переноса сервера без агента</em>)
3.  Назначить разрешения учетной записи пользователя для объектов vCenter.

**Создание учетной записи vCenter**
1.  Войдите в веб-клиент vSphere с правами администратора vCenter Server.
2.  Щелкните вкладку **Администрирование** > **пользователей единого входа и групп**  > **пользователей**.
3.  Щелкните значок **Новый пользователь**.
4.  Введите необходимые данные для создания нового пользователя и нажмите кнопку **ОК**.

**Определение новой роли с необходимыми разрешениями** (<em>требуется для переноса сервера без агента</em>)
1.  Войдите в веб-клиент vSphere с правами администратора vCenter Server.
2.  Выберите к **Администрирование** > **Диспетчер ролей**.
3.  Выберите vCenter Server из раскрывающегося списка.
4.  Щелкните действие **Создать роль**.
5.  Введите имя новой роли. (например, <em>Azure_Migrate</em>).
6.  Назначьте эти [разрешения](https://docs.microsoft.com/azure/migrate/migrate-support-matrix-vmware#agentless-migration-vcenter-server-permissions) для новой определенной роли.
7.  Последовательно выберите **ОК**.

**Назначение разрешений для объектов vCenter**

Существует 2 подхода к назначению разрешений для объектов инвентаризации в vCenter для учетной записи пользователя vCenter с назначенной ей ролью.
- Для оценки сервера роль **только чтение** необходимо применять к учетной записи пользователя vCenter для всех родительских объектов, в которых размещаются виртуальные машины, которое нужно обнаружить. Все родительские объекты (узел, папка узлов, кластер, папка кластеров) в иерархии до центра обработки данных должны быть включены. Эти разрешения должны распространяться на дочерние объекты в иерархии.
