---
title: Поддержка VMware в службе "миграция Azure"
description: Узнайте о поддержке технологии оценки и миграции VMware в службе "миграция Azure".
ms.topic: conceptual
ms.date: 01/02/2020
...
**Migrate VMware VMs to Azure (agentless)** (Перенос виртуальных машин VMware (без агента)) | Миграцию можно выполнить с помощью миграции без агента или с помощью миграции на основе агентов. [Дополнительные сведения](server-migrate-overview.md)
...
--- | ---
Только Оценка | vCenter Server учетную запись только для чтения.
Оценка с помощью [визуализации зависимостей](how-to-create-group-machine-dependencies-agentless.md) для [обнаружения приложений или без](how-to-discover-applications.md) агента | Учетная запись vCenter Server с доступом только для чтения, а привилегии для **виртуальных машин** > **гостевых операций**.
...
**Обнаружение** | Устройство может обнаруживать до 10 000 виртуальных машин VMware на vCenter Server.<br/> Устройство может подключаться к одной vCenter Server.
...
**Размер диска** | диск ОС объемом 2 ТБ; 4 ТБ для дисков данных.
...
**Целевой диск** | Виртуальные машины можно перенести только на управляемые диски (стандартный HDD, SSD уровня "Премиум") в Azure.
**Размер диска** | диск ОС объемом 2 ТБ; 8 ТБ для дисков данных.
