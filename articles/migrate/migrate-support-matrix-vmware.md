---
title: Поддержка оценки и миграции VMware в службе "миграция Azure"
description: Сведения о поддержке оценки и миграции виртуальных машин VMware в службе "миграция Azure".
...
**Ограничения VMware**  | Оцените до 35 000 виртуальных машин VMware в одном проекте. В подписке Azure можно создать несколько проектов. Проект может содержать как виртуальные машины VMware, так и виртуальные машины Hyper-V, вплоть до ограничений оценки.
**География** | [Просмотрите](migrate-support-matrix.md#supported-geographies) список поддерживаемых регионов.
...
Служба "миграция Azure". Оценка серверов позволяет обнаруживать приложения, роли и компоненты. Обнаружение инвентаризации приложений позволяет выявление и планирование пути миграции, предназначенного для локальных рабочих нагрузок. Служба "миграция Azure". Оценка серверов обеспечивает безагентное обнаружение с использованием гостевых учетных данных компьютера, удаленный доступ к компьютерам с помощью WMI и вызовов SSH.

**Поддержка** | **Дополнительные сведения**
--- | ---
Поддерживаемые компьютеры | Локальные виртуальные машины VMware
Операционная система компьютера | Все версии Windows и Linux
Учетные данные | В настоящее время поддерживает использование одного учетного данных для всех серверов Windows, а также одну учетную запись для всех серверов Linux. Создайте учетную запись гостевого пользователя для виртуальных машин Windows, а также обычную или нормальную учетную запись пользователя (без доступа sudo) для всех виртуальных машин Linux.
Ограничения на компьютеры для обнаружения приложений | 10000 на устройство. 35000 на проект
...
- Если вы планируете обнаруживать приложения или визуализировать зависимость в безагентном режиме, создайте учетную запись vCenter Server с доступом только для чтения вместе с правами, включенными для **виртуальных машин** > **гостевых операций**.

  ![vCenter Server привилегий учетной записи](./media/tutorial-prepare-vmware/vcenter-server-permissions.png)

- Если вы не планируете выполнять обнаружение приложений и визуализацию зависимостей без агента, настройте учетную запись только для чтения для vCenter Server.

## <a name="assessment-appliance-requirements"></a>Оценка — требования к устройству

Служба "миграция Azure" запускает упрощенное устройство для обнаружения виртуальных машин VMware и отправки метаданных и данных производительности виртуальной машины в службу "миграция Azure". Устройство для VMware развертывается с помощью шаблона OVA, импортированного в vCenter Server. В следующей таблице перечислены требования к устройству.

**Поддержка** | **Дополнительные сведения**
--- | ---
**Развертывание устройства** | Вы развертываете устройство как виртуальную машину VMware. Вам потребуется достаточно ресурсов на vCenter Server, чтобы выделить виртуальную машину с 32 ГБ ОЗУ, 8 виртуальных ЦП, около 80 ГБ дискового накопителя и внешним виртуальным коммутатором.<br/><br/> Устройству требуется доступ к Интернету либо напрямую, либо через прокси-сервер.<br/> Виртуальная машина устройства должна быть развернута на узле ESXi, работающем под управлением версии 5,5 или более поздней.
...
*.windows.net <br/> *.msftauth.net <br/> *.msauth.net <br/> *. microsoft.com <br/> *. live.com | Войдите в подписку Azure.
*.microsoftonline.com <br/> *.microsoftonline-p.com | Создание Active Directory приложений для взаимодействия устройства с службой "миграция Azure".
...
https://aka.ms/latestapplianceservices<br/><br/> https://download.microsoft.com/download | Используется для переноса обновлений устройства в Azure.
...
## <a name="assessment-dependency-visualization"></a>Оценка — Визуализация зависимостей
...
- **Визуализация зависимостей на основе агентов**. чтобы использовать визуализацию зависимостей на основе агента, необходимо скачать и установить следующие агенты на каждом локальном компьютере, который необходимо проанализировать.
    - На каждом компьютере должен быть установлен Microsoft Monitoring Agent (MMA). Дополнительные [сведения](how-to-create-group-machine-dependencies.md#install-the-mma) об установке агента MMA.
    - Агент зависимостей должен быть установлен на каждом компьютере. Дополнительные [сведения](how-to-create-group-machine-dependencies.md#install-the-dependency-agent) об установке агента зависимостей.
...
Для репликации можно выбрать до 10 виртуальных машин. Если вы хотите перенести другие компьютеры, выполните репликацию в группы из 10. При миграции VMware без агента вы можете запустить до 100 репликаций одновременно.
...
**Поддержка** | **Дополнительные сведения**
--- | ---
с сервером vCenter | Версии 5,5, 6,0, 6,5 или 6,7.
...
**Разрешения** | **Дополнительные сведения**
--- | ---
...
**Устройство** | **Connection**
--- | ---
Appliance | Исходящие подключения через порт 443 для передачи реплицируемых данных в Azure, а также для взаимодействия со службами Службы "миграция Azure", координирующими репликацию и миграцию.
...
**цели iSCSI** | Виртуальные машины с целями iSCSI не поддерживаются для миграции без агента.
**Многопутевой ввод-вывод** | Не поддерживается.
**Хранилище vMotion** | Поддерживаются
**Объединенные сетевые карты** | Не поддерживается.
**Протокол** | Не поддерживается.
