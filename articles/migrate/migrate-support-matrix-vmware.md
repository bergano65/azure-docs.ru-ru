...
**Поддержка** | **Сведения**
--- | ---
**Поддерживаемые компьютеры** | В настоящее время поддерживается только для виртуальных машин VMware. Приложения, установленные на виртуальных машинах VMware до 10000, можно найти на каждом устройстве службы "миграция Azure".
**Операционные системы** | Поддержка виртуальных машин под управлением всех версий Windows и Linux.
**Требования к виртуальной машине** | Средства VMware должны быть установлены и запускаться на виртуальных машинах, на которых требуется обнаружить приложения. <br/><br/> Потребуются средства VMware версии после 10.2.0.<br/><br/> Для виртуальных машин требуется PowerShell версии 2.0 или более поздней.
**Discovery** | Сведения о приложениях, установленных на виртуальной машине, собираются из vCenter Server с помощью средств VMware, установленных на виртуальной машине. Устройство собирает сведения о приложении из vCenter Server с помощью API-интерфейсов vSphere. Обнаружение приложений выполняется без агента. На виртуальных машинах ничего не устанавливается, и устройство не подключается напрямую к виртуальным машинам. WMI/SSH должны быть включены и доступны на виртуальных машинах.
**vCenter** | VCenter Server учетной записи только для чтения, используемой для оценки, требуются права для  >  **гостевых операций** виртуальных машин, чтобы взаимодействовать с виртуальной машиной для обнаружения приложений.
...
**Виртуальные машины Windows** | Windows Server 2016<br/> Windows Server 2012 R2<br/> Windows Server 2012<br/> Windows Server 2008 R2 (64-разрядная версия).<br/>Microsoft Windows Server 2008 (32-bit). 
**Виртуальные машины Linux** | Red Hat Enterprise Linux 7, 6, 5<br/> Ubuntu Linux 14.04, 16.04<br/> Debian 7, 8.<br/> Oracle Linux 6, 7<br/> CentOS 5, 6, 7.<br/> SUSE Linux Enterprise Server 11 и более поздних версий.
**Требования к виртуальной машине** | Средства VMware (более поздние, чем 10.2.0) должны быть установлены и запускаться на виртуальных машинах, которые вы хотите проанализировать.<br/><br/> Для виртуальных машин требуется PowerShell версии 2.0 или более поздней.
**Метод обнаружения** |  Сведения о зависимостях между виртуальными машинами собираются из vCenter Server с помощью средств VMware, установленных на виртуальной машине. Устройство собирает сведения из vCenter Server с помощью API-интерфейсов vSphere. Обнаружение без агента. На виртуальной машине ничего не установлено, и устройство не подключается напрямую к виртуальным машинам. WMI/SSH должны быть включены и доступны на виртуальных машинах.
...
**Разрешения виртуальной машины Windows** |  Учетная запись (локальный администратор или домен) с разрешениями локального администратора на виртуальных машинах.
...
**Доступ к портам** | Устройство миграции Azure должно иметь возможность подключения к TCP-порту 443 на узлах ESXI с виртуальными машинами, зависимости которых необходимо обнаружить. VCenter Server возвращает подключение к узлу ESXI для загрузки файла, содержащего сведения о зависимостях.
...
**Служба Log Analytics** | Служба "Миграция Azure" использует решение [Сопоставление служб](../azure-monitor/vm/service-map.md) в [журналах Azure Monitor](../azure-monitor/logs/log-query-overview.md) для визуализации зависимостей.<br/><br/> Привяжите новую или существующую рабочую область Log Analytics к проекту Миграции Azure. Рабочую область для проекта Миграции Azure нельзя изменить после добавления. <br/><br/> Рабочая область должна находиться в той же подписке, что и проект Миграции Azure.<br/><br/> Рабочая область должна находиться в следующих регионах: Восточная часть США, Юго-Восточная Азия или Западная Европа. Рабочие области в других регионах не могут быть связаны с проектом.<br/><br/> Рабочая область должна находиться в регионе, в котором [поддерживается Сопоставление служб](../azure-monitor/vm/vminsights-configure-workspace.md#supported-regions).<br/><br/> В Log Analytics рабочая область, связанная с Миграцией Azure, помечается ключом проекта миграции и именем проекта.
**Необходимые агенты** | На каждом компьютере, который необходимо проанализировать, установите следующие агенты.<br/><br/> [Microsoft Monitoring Agent (MMA)](../azure-monitor/agents/agent-windows.md).<br/> [Агент зависимостей](../azure-monitor/agents/agents-overview.md#dependency-agent).<br/><br/> Если локальные компьютеры не подключены к Интернету, необходимо скачать и установить на них шлюз Log Analytics.<br/><br/> Узнайте больше об установке [агента зависимостей](how-to-create-group-machine-dependencies.md#install-the-dependency-agent) и [MMA](how-to-create-group-machine-dependencies.md#install-the-mma).
**Рабочая область Log Analytics** | Рабочая область должна находиться в той же подписке, что и проект Миграции Azure.<br/><br/> Миграция Azure поддерживает рабочие области в следующих регионах: Восточная часть США, Юго-Восточная Азия и Западная Европа.<br/><br/>  Рабочая область должна находиться в регионе, в котором [поддерживается Сопоставление служб](../azure-monitor/vm/vminsights-configure-workspace.md#supported-regions).<br/><br/> Рабочую область для проекта Миграции Azure нельзя изменить после добавления.
