...
В этой статье перечислены требования к предварительным требованиям и поддержке при оценке виртуальных машин VMware для миграции в Azure с помощью средства миграции Azure: Оценка сервера] (миграция служб — обзор. md # Azure-Migration-Server-оценке-Tool). Если вы хотите перенести виртуальные машины VMware в Azure, изучите [таблицу поддержки миграции](migrate-support-matrix-vmware-migration.md).
...
**Маршрутизатор** | Обнаружение приложений — без агента. Он использует гостевые учетные данные компьютера и удаленный доступ к компьютерам с помощью WMI и вызовов SSH.
...
**Разрешения (Визуализация зависимостей)** | Учетная запись Center Server с доступом только для чтения и привилегии, включенные для**гостевых операций** **виртуальных машин** > .
...
**Учетная запись Windows** |  Для анализа зависимостей устройству миграции Azure требуется учетная запись локального пользователя или администратора домена для доступа к виртуальным машинам Windows.
