...
3. Для последней версии устройства созданный хэш должен соответствовать этим [параметрам](./tutorial-discover-vmware.md#verify-security).
...
4. Если учетная запись пользователя Azure, используемая для ведения журнала, имеет необходимые [разрешения](./tutorial-discover-vmware.md#prepare-an-azure-user-account) на ресурсы Azure, созданные во время создания ключа, будет инициирована регистрация модуля.
...
1. В разделе **Шаг 1. Provide vCenter Server credentials** (Предоставление учетных данных vCenter Server) щелкните **Добавить учетные данные**, чтобы указать понятное имя для учетных данных, добавьте значения для пунктов **Имя пользователя** и **Пароль** для учетной записи vCenter Server, которую виртуальный модуль будет использовать для обнаружения виртуальных машин в экземпляре vCenter Server.
    - У вас должна быть учетная запись, настроенная с необходимыми разрешениями согласно инструкциям в [предыдущем руководстве](./tutorial-discover-vmware.md#create-an-account-to-access-vcenter).
