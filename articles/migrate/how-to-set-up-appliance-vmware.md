---
title: Настройка прибора Azure Migrate для VMware
description: Узнайте, как настроить прибор Azure Migrate для оценки и миграции VMw.
ms.topic: article
ms.date: 04/16/2020
...
ms.locfileid: "81538262"
---
# <a name="set-up-an-appliance-for-vmware-vms"></a>Настройка прибора для VMware VMs

Следуйте этой статье, чтобы настроить прибор Azure Migrate для оценки с помощью инструмента [оценки Azure Migrate:Server Assessment,](migrate-services-overview.md#azure-migrate-server-assessment-tool) а также для бесполой миграции с помощью инструмента [Azure Migrate:Server Migration.](migrate-services-overview.md#azure-migrate-server-migration-tool)

[Прибор Azure Migrate](migrate-appliance.md) — это легкий прибор, используемый Azure Migrate: Оценка серверов и миграция серверов для обнаружения находящихся на месте VMware VM, отправки метаданных VM/данных о производительности в Azure и для репликации VMware VMs во время бесагентной миграции.

Развертывание прибора можно несколькими методами:

- Настройка на VMware VM с помощью загруженного шаблона OVA. Это метод, описанный в этой статье.
- Настройка на VMware VM или физической машине со скриптом установки PowerShell. [Этот метод](deploy-appliance-script.md) следует использовать, если вы не можете настроить VM с помощью шаблона OVA, или если вы находитесь в правительстве Azure.
...
## <a name="appliance-deployment-ova"></a>Развертывание приборов (OVA)

Чтобы настроить прибор с помощью шаблона OVA:
...
2. В **Discover машины** > **Ваши машины виртуализированы?**, нажмите **Да, с гипервизором VMWare vSphere.**
...
2. Выполнить следующую команду, чтобы создать хэш для OVA:
    - ```C:\>CertUtil -HashFile <file_location> [Hashing Algorithm]```
    - Пример использования: ```C:\>CertUtil -HashFile C:\AzureMigrate\AzureMigrate.ova SHA256```
3. Для последней версии прибора генерируемый хэш должен соответствовать этим [настройкам.](https://docs.microsoft.com/azure/migrate/tutorial-assess-vmware#verify-security)
...
1. В консоли клиента vSphere щелкните шаблон **развертывания файлов** > **OVF.**
...
8. В **сетевом картировании**укажите сеть, к которой подключится VM. Этой сети требуется подключение к Интернету для отправки метаданных в средство "Оценка серверов" службы "Миграция Azure".
...
Настройте устройство в первый раз. Если развернуть прибор с помощью скрипта вместо шаблона OVA, первые два шага в процедуре не применимы.
...
4. В веб-приложении выберите **Настройка необходимых компонентов** и выполните приведенные ниже действия.
    ...
    - **Синхронизация времени**: время проверяется. Время на устройстве должно быть синхронизировано со временем в Интернете, чтобы обнаружение виртуальных машин работало правильно.
    - **Установите обновления**: Azure Migrate проверяет, что последние обновления прибора установлены.
    - **Установка VDDK**: Azure Migrate проверяет установку виртуального набора для разработки дисков VMWare vSphere (VDDK).
        - Azure Migrates использует VDDK для репликации машин во время миграции в Azure.
...
## <a name="start-continuous-discovery-by-providing-vcenter-server-and-vm-credential"></a>Начните непрерывное открытие, предоставляя vCenter Server и учетные данные VM
...
2. В полях **Имя пользователя** и **Пароль** укажите данные учетной записи только для чтения, которые устройство будет использовать для обнаружения виртуальных машин на сервере vCenter Server. Вы можете обозначить область обнаружения, ограничив доступ к учетной записи vCenter. [Подробнее](set-discovery-scope.md).
...
Просмотрите учебники для [оценки VMware](tutorial-assess-vmware.md) и [безагентской миграции.](tutorial-migrate-vmware.md)
