...
### <a name="i-started-discovery-but-i-dont-see-the-discovered-vms-on-azure-portal-server-assessment-and-server-migrate-tiles-show-a-status-of-discovery-in-progress"></a>Я начал обнаружение, но не вижу обнаруженные виртуальные машины на портал Azure. Плитки "Оценка сервера" и "миграция сервера" показывают состояние "обнаружение выполняется".
...
## <a name="assessment-issues"></a>Проблемы с оценкой
...
Миграция Azure: Оценка сервера сейчас не поддерживает расценки на на основе Соглашение Enterprise (EA). Чтобы решить эту проблему, используйте предложение Azure с оплатой по мере использования и используйте свойство "скидка", чтобы указать пользовательскую скидку, которую вы получаете. [Узнайте больше о том, как можно настроить оценку](https://aka.ms/migrate/selfhelp/eapricing).
...
В оценке сервера есть известный разрыв, не позволяющий обнаружить дополнительную версию ОС Linux, установленную на локальных виртуальных машинах (например, для RHEL 6,10, в настоящее время Оценка сервера обнаруживает только RHEL 6 в качестве версии ОС). Поскольку Azure поддерживает только определенные версии Linux, виртуальные машины Linux в настоящее время отмечены как условно готовые к оценке серверов. Вы можете вручную убедиться, что операционная система Linux, работающая на локальной виртуальной машине, одобрена в Azure, просмотрев документацию по [поддержке Linux в Azure.](https://aka.ms/migrate/selfhost/azureendorseddistros) После проверки подтвержденного дистрибутив можно проигнорировать это предупреждение.
...
Рекомендации по SKU виртуальной машины в оценке сервера зависят от свойств оценки. В оценке сервера можно создавать два типа оценки: "с учетом производительности" и "как локальные". Для оценки на основе производительности в ходе оценки серверов учитываются данные об использовании локальных виртуальных машин (ЦП, памяти, диска и использования сети), чтобы определить правильный номер SKU целевой виртуальной машины для локальных виртуальных машин. Кроме того, при изменении размера на основе производительности для определения эффективного использования учитывается эффективный фактор. При изменении размера в локальной среде данные о производительности не учитываются, и рекомендуется использовать целевой номер SKU в зависимости от того, что размещается в локальной среде.

Например, предположим, что существует локальная виртуальная машина с 4 ядрами и 8 ГБ памяти с 50% использования ЦП и 50% использования памяти, а в ходе оценки указывается отличный фактор 1,3. Если критерий определения размера для оценки — "как локальная", используется номер SKU виртуальной машины Azure с 4 ядрами и 8 ГБ памяти, однако, если критерий изменения размера зависит от эффективности использования ЦП и памяти (50% от 4 ядер * 1,3 = 2,6 ядер и 50% от 8 ГБ) память * 1,3 = 5,3-ГБ памяти), рекомендуется использовать самый дешевый номер SKU виртуальных машин из четырех ядер (ближайшее поддерживаемое число ядер) и 8 ГБ памяти (ближайший поддерживаемый объем памяти). [Узнайте больше о том, как выполняется оценка серверов.](https://docs.microsoft.com/azure/migrate/concepts-assessment-calculation#sizing)
...
Размер диска в оценке сервера зависит от двух параметров оценки — критерия изменения размера и типа хранилища. Если для критерия изменения размера задано значение "Автоматический", то значения операций ввода-вывода и пропускной способности диска будут считаться типом целевого диска (HDD (цен. категория "Стандартный"), SSD (цен. категория "Стандартный") или дисков уровня "Премиум"). При этом рекомендуется использовать номер SKU диска в типе диска, учитывая требования к размеру локального диска. Если в качестве критерия изменения размера выбран тип "Премиум", то рекомендуется использовать номер SKU диска уровня "Премиум" в Azure на основе требований к операциям ввода-вывода, пропускной способности и размеру локального диска. Та же логика используется для изменения размера диска, если для критерия изменения размера задано значение "HDD (цен. категория"Стандартный")", "SSD (цен. категория"Стандартный")" или "Премиум".

Например, если имеется локальный диск с объемом памяти 32 ГБ, а суммарные операции чтения и записи для диска составляет 800 операций ввода-вывода, то при оценке сервера будет рекомендован тип диска уровня "Премиум" (из-за более высоких требований к операциям ввода/вывода), а затем рекомендуется использовать номер SKU диска, поддерживающий требуемые операции ввода-вывода и размер. Ближайшее совпадение в этом примере — p15 (256 ГБ, 1100 операций ввода-вывода). Таким образом, несмотря на то, что размер, требуемый для локального диска, был 32 ГБ, для оценки сервера рекомендуется диск с большим размером из-за высокого требования к серверу операций ввода-вывода в секунду для локального диска.
...
Оценка серверов в настоящее время учитывает только затраты на лицензии ОС Windows. стоимость лицензии ОС для компьютеров Linux в настоящее время не рассматривается. 
...
Эти свойства применимы только для изменения размера с учетом производительности. Оценка сервера постоянно собирает данные о производительности локальных компьютеров и использует их для рекомендуемого номера SKU виртуальной машины и номера SKU диска в Azure. Ниже показано, как данные производительности собираются с помощью оценки сервера.
...
- Устройство выполняет сведение с 20/30-секундных выборок, чтобы создать одну точку данных каждые 10 минут. Чтобы создать одну точку данных, устройство выбирает пиковое значение из всех 20/30-секундных выборок и отправляет их в Azure.
- При создании оценки в оценке сервера на основе значения «длительность производительности» и «журнал производительности» определяется значение репрезентативного использования. Например, если журнал производительности составляет одну неделю, а использование процентиля — 95 процентиль, то служба "миграция Azure" будет сортировать все 10-минутные точки выборки за последнюю неделю в возрастающем порядке, а затем выбрать 95 процентиль процентиль в качестве репрезентативного значения.
Значение 95 процентиль процентиль позволяет пропускать выбросы, которые могут быть включены при выборе 99-м процентиль. Если вы хотите выбрать пиковое использование для периода и не хотите пропускать выбросы, следует выбрать 99-м процентиль в качестве использования процентилей.
