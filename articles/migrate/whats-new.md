...
-  Теперь на портале Azure можно перенести виртуальные машины VMware на виртуальные машины Azure с дисками, защищенных путем двойного шифрования с помощью ключей, управляемых платформой, и ключей, управляемых клиентом (CMK). Для такой миграции используется средство "Миграция сервера" службы "Миграция Azure" (репликация без агента).
- Теперь на портале Azure можно перенести виртуальные машины VMware на виртуальные машины Azure с дисками, защищенных путем шифрования на стороне сервера (SSE) с помощью ключей, управляемых клиентом (CMK), и путем двойного шифрования с помощью ключей, управляемых платформой и клиентом. Для такой миграции используется средство "Миграция сервера" службы "Миграция Azure" (репликация с агентом).
- Теперь на портале Azure можно перенести физические серверы и виртуальные машины из других облаков, таких как AWS и GCP, на виртуальные машины Azure с дисками, защищенных путем шифрования на стороне сервера (SSE) с помощью ключей, управляемых клиентом (CMK), и путем двойного шифрования с помощью ключей, управляемых платформой и клиентом. Для такой миграции используется средство "Миграция сервера" службы "Миграция Azure" (репликация с агентом).
