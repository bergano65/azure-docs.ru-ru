...
**Требование** | **VMware** 
--- | ---
**Разрешения** | Для локального или удаленного доступа к веб-приложению устройства необходимо быть администратором домена или локальным администратором на компьютере устройства.
...
**Сценарий PowerShell** | Скачайте сценарий (Азуремигратеинсталлер. ps1) в ZIP-папку на портале. [Дополнительные сведения](tutorial-assess-physical.md#set-up-the-appliance). Кроме того, можно [загрузить напрямую](https://go.microsoft.com/fwlink/?linkid=2105112).<br/><br/> Размер загрузки составляет 59,7 МБ.
...
- Необходимо разрешить доступ ко всем URL-адресам в списке. Если вы выполняете только оценку, вы можете пропустить URL-адреса, помеченные как обязательные только для миграции без агента VMware.
-  Если вы используете прокси-сервер на основе URL-адресов для подключения к Интернету, убедитесь, что прокси-сервер разрешает все записи CNAME, полученные при поиске URL-адресов.
...
*.services.visualstudio.com | Отправка журналов приложений, используемых для внутреннего мониторинга.
...
Название | container.GetType().Name
...
## <a name="collected-data---physical"></a>Собранные данные — физические
...
**DATA** | **КЛАСС WMI** | **СВОЙСТВО КЛАССА WMI**
--- | --- | ---
Полное доменное имя. | Win32_ComputerSystem | Домен, имя, Партофдомаин
Число ядер процессора | Win32_PRocessor | нумберофкорес
Выделенная память | Win32_ComputerSystem | тоталфисикалмемори
Серийный номер BIOS | Win32_ComputerSystemProduct | IdentifyingNumber
GUID BIOS | Win32_ComputerSystemProduct | UUID
Тип загрузки | Win32_DiskPartition | Проверьте наличие секции с типом = **GPT: System** для EFI/BIOS
OS name (Имя ОС) | Win32_OperatingSystem | Caption
Версия ОС |Win32_OperatingSystem | Версия
Архитектура ОС | Win32_OperatingSystem | осарчитектуре
Число дисков | Win32_DiskDrive | Модель, размер, DeviceID, MediaType, имя
Размер диска | Win32_DiskDrive | Размер
Список сетевых карт | Win32_NetworkAdapterConfiguration | Описание, индекс
IP-адрес сетевого адаптера | Win32_NetworkAdapterConfiguration | IPAddress
MAC-адрес сетевого адаптера | Win32_NetworkAdapterConfiguration | MACAddress
...
**DATA** | **LINUX** 
--- | --- 
Полное доменное имя. | Cat/прок/СИС/Кернел/хостнаме, hostname-f
Число ядер процессора |  /прок/кпуинфо \| awk '/^ процессор/{Print $3} ' \| WC-l
Выделенная память | Cat/прок/меминфо \| grep мемтотал \| awk "{printf"%. 0f ", $2/1024}"
Серийный номер BIOS | ЛШВ \| grep "серийный:" \| Head — N1 \| awk "{Print $2}" <br/> /УСР/сбин/дмидекоде-t 1 \| grep "последовательный \| " AWK "{$1 =" "; $2 =" "; Print}"
GUID BIOS | product_uuid Cat/СИС/класс/ДМИ/ИД/
Тип загрузки | [-d/СИС/фирмваре/Ефи]  && эхо EFI \| \| Echo BIOS
Имя или версия ОС | Мы получаем доступ к этим файлам для версии ОС и имени:<br/><br/> /етк/ос-релеасе<br/> /уср/либ/ос-релеасе <br/> /етк/ентерприсе-релеасе <br/> /етк/редхат-релеасе<br/> /етк/оракле-релеасе<br/>  /етк/сусе-релеасе<br/>  /етк/лсб-релеасе  <br/> /etc/debian_version
Архитектура ОС | Uname-m
Число дисков | fdisk-l \| egrep ' Disk. * bytes \| ' awk ' {Printing $2 \| } ' Cut-F1-d ': '
Загрузочный диск | DF/Boot \| SED-n 2P \| awk ' {Print $1} '
Размер диска | fdisk-l \| egrep ' Disk. * bytes \| ' egrep $Disk \| : awk ' {Print $5} '
Список сетевых карт | IP-o-4 адрес показывать \| awk ' {Print $2} '
IP-адрес сетевого адаптера | IP-адрес показывать \| $NIC grep \| inet awk ' {Print $2} \| ' вырезать-F1-d "/" 
MAC-адрес сетевого адаптера | IP-адрес показывать \| $NIC grep \| Ether awk ' {Print $2} '
...
**Data** | **класс WMI** | **Свойство класса WMI**
--- | --- | ---
Использование ЦП | Win32_PerfFormattedData_PerfOS_Processor | PercentIdleTime
Использование памяти | Win32_PerfFormattedData_PerfOS_Memory | аваилаблембитес
Число сетевых карт | Win32_PerfFormattedData_Tcpip_NetworkInterface | Получение числа сетевых устройств.
Получено данных на сетевой адаптер | Win32_PerfFormattedData_Tcpip_NetworkInterface  | битесрецеиведперсек
Передача данных на сетевой адаптер | BWin32_PerfFormattedData_Tcpip_NetworkInterface | битессентперсек
Число дисков | BWin32_PerfFormattedData_PerfDisk_PhysicalDisk | Число дисков
Сведения о диске | Win32_PerfFormattedData_PerfDisk_PhysicalDisk | Дисквритесперсек, Дисквритебитесперсек, Дискреадсперсек, Дискреадбитесперсек.
...
**Data** | **Linux** 
--- | --- 
Использование ЦП | Кот/прок/стат/| /прок/стат "ЦП" grep
Использование памяти | Бесплатная \| MEM \| grep "{Printing $3/$ 2 * 100,0}"
Число сетевых карт | ЛШВ-класс сетевого \| grep ETH [0-60] \| WC-l
Получено данных на сетевой адаптер | Cat/СИС/класс/нет/ЕС $ NIC/статистика/rx_bytes
Передача данных на сетевой адаптер | Cat/СИС/класс/нет/ЕС $ NIC/статистика/tx_bytes
Число дисков | fdisk-l \| egrep ' Disk. * bytes \| ' awk ' {Printing $2 \| } ' Cut-F1-d ': '
Сведения о диске | Кот/прок/дискстатс
