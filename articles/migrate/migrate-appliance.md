---
title: Архитектура устройства "миграция Azure" | Документация Майкрософт
description: Содержит общие сведения об устройстве "миграция Azure"
...
После настройки устройства можно получить удаленный доступ к виртуальной машине устройства через TCP-порт 3389. Вы также можете удаленно получить доступ к приложению веб-управления для устройства через порт 44368 с URL `https://<appliance-ip-or-name>:44368`-адресом:.
...
3. **Отправить данные**: Устройство отправляет собранные данные в службу "Миграция серверов Azure" и выполняет миграцию сервера Azure Migration через порт SSL 443.
...
4. **Оценка и миграция**: Теперь вы можете создавать оценки на основе метаданных, собранных устройством, с помощью функции "миграция сервера Azure". Кроме того, можно также приступить к переносу виртуальных машин VMware с помощью миграции сервера миграция Azure для координации репликации виртуальных машин без агента.
...
- Чтобы отключить автоматическое обновление, перейдите в редактор реестра > HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\AzureAppliance и установите раздел реестра — "автоматическое обновление" в значение 0 (DWORD).
