...
## <a name="maintain-copies-of-base-images"></a>Поддержка копий базовых образов

Для любого содержимого в реестре, которое зависит от базового содержимого, поддерживаемого в общедоступном реестре, например DOCKER Hub, рекомендуется скопировать содержимое в реестр контейнеров Azure или в другой частный реестр. Затем убедитесь, что вы создаете образы приложения, обратившись к частным базовым образам. Реестр контейнеров Azure предоставляет возможность [импорта изображений](container-registry-import-images.md) для простого копирования содержимого из общедоступных реестров или других реестров контейнеров Azure. В следующем разделе описывается использование задач записи контроля доступа для мониторинга обновлений базовых образов при создании обновлений приложения. Вы можете отслеживание обновлений базовых образов в собственных реестрах контейнеров Azure, а также при необходимости в вышестоящем общедоступном реестре.

## <a name="track-base-image-updates"></a>Мониторинг обновлений базовых образов

Решение "Задачи ACR" включают в себя возможность автоматически создавать образы при обновлении базового образа контейнера. Эту возможность можно использовать для обслуживания и обновления копий общедоступных базовых образов в реестрах контейнеров Azure, а затем для перестроения образов приложений, зависящих от базовых образов.
