...
При создании задачи записи контроля доступа при необходимости можно включить пользовательское удостоверение, назначенное системой удостоверение или и то, и другое. Например, передайте параметр `--assign-identity` при выполнении команды AZ запись [контроля][az-acr-task-create] доступа в Azure CLI.

Чтобы включить назначенное системой удостоверение, передайте `--assign-identity` без значения или `assign-identity [system]`. В следующем примере команда создает задачу Linux из общедоступного репозитория GitHub, который создает `hello-world` образ и включает управляемое системой удостоверение.
...
Чтобы включить назначенное пользователем удостоверение, передайте `--assign-identity` со значением *идентификатора ресурса* удостоверения. В следующем примере команда создает задачу Linux из общедоступного репозитория GitHub, который создает образ `hello-world` и включает управляемое пользователем удостоверение:
...
В зависимости от требований задачи предоставьте удостоверениям разрешения на доступ к другим ресурсам Azure. Примеры.
...
Чтобы добавить учетные данные для назначенного пользователем удостоверения для проверки подлинности в реестре *таржетрегистри*, передайте `use-identity` со значением *идентификатора клиента* удостоверения. Пример.
