---
title: Многоступенчатая задача для построения, тестирования & изображения патча
description: Введение в многоступенчатые задачи, функция ACR Tasks в реестре контейнеров Azure, которая обеспечивает рабочие процессы на основе задач для создания, тестирования и исправления изображений контейнеров в облаке.
ms.topic: article
ms.date: 03/28/2019
...
* [`build`](container-registry-tasks-reference-yaml.md#build): Создавайте одно или `docker build` несколько изображений контейнеров с помощью знакомого синтаксиса, в серии или параллельно.
* [`push`](container-registry-tasks-reference-yaml.md#push): Нажмите встроенные изображения в реестр контейнеров. Поддерживаются такие закрытые реестры, как Реестр контейнеров Azure, а также общедоступный Центр Docker.
* [`cmd`](container-registry-tasks-reference-yaml.md#cmd): Запустите контейнер, таким образом, чтобы он мог работать как функция в контексте выполнения задачи. Вы можете передавать параметры в `[ENTRYPOINT]` контейнера и указать такие свойства, как env, detach, и другие знакомые параметры `docker run`. Тип шага `cmd` включает модульное и функциональное тестирование, а также параллельное выполнение контейнеров.
...
Для нескольких сценариев [приведены примеры задач](container-registry-tasks-samples.md) для многоступенчатых файлов YAML и Dockerfiles.
...
* [Примеры](container-registry-tasks-samples.md) задач `task.yaml` - Примеры и файлы Docker для нескольких сценариев, простых для сложных.
