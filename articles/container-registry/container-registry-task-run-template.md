...
|sourceLocation     |Удаленный контекст для задачи сборки, например https://github.com/Azure-Samples/acr-build-helloworld-node. Dockerfile в корне репозитория создает образ контейнера для небольшого веб-приложения Node. js. При необходимости используйте вилку репозитория в качестве контекста сборки.         |
...
В демонстрационных целях создайте отдельный реестр контейнеров в качестве базового реестра и отправьте базовый образ Node. js, извлеченный из DOCKER Hub.
...
Чтобы назначить удостоверение роли Акрпулл в основном реестре, используйте команду [AZ Role назначение Create][az-role-assignment-create] . Эта роль имеет разрешения только на извлечение изображений из реестра.
...
|sourceLocation     |Удаленный контекст для задачи сборки, например * https://github.com/\<your-GitHub-ID\>/acr-build-helloworld-node.* |
