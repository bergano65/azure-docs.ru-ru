---
title: Ограничение доступа к реестру контейнеров Azure из виртуальной сети
...
В этой статье описаны два сценария создания правил доступа к сети для ограничения доступа к реестру контейнеров Azure: из виртуальной машины, развернутой в виртуальной сети, или из общедоступного IP-адреса виртуальной машины.
...
* Если у вас еще нет реестра контейнеров, создайте его (требуется номер SKU уровня "Премиум") и отправьте пример образа `hello-world` , например из DOCKER Hub. Например, используйте [портал Azure][quickstart-portal] or the [Azure CLI][quickstart-cli] для создания реестра. 

* Если вы хотите ограничить доступ к реестру с помощью виртуальной сети в другой подписке Azure, необходимо зарегистрировать поставщик ресурсов для реестра контейнеров Azure в этой подписке. Пример:
