...
[Виртуальная сеть Azure](../virtual-network/virtual-networks-overview.md) обеспечивает безопасное и частных сетей для Azure и локальным ресурсам. Ограничивая доступ к вашей закрытого реестра контейнеров Azure из виртуальной сети Azure, убедитесь, что только ресурсы в виртуальной сети, доступ к реестру. Для сценариев распределенного развертывания можно также настроить правила брандмауэра, чтобы разрешить доступ к реестру, только с определенных IP-адресов.

В этой статье показано два сценария, чтобы создать правила доступа к сети для ограничения доступа к реестру контейнеров Azure: из виртуальной машине, развернутой в виртуальной сети или из виртуальной Машины общедоступный IP-адрес.
...
* [Задачи ACR](container-registry-tasks-overview.md) операции не поддерживаются в реестр контейнеров, в виртуальной сети.
