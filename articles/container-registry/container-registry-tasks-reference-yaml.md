...
* `when: ["-"]` — Указывает, не зависит от других действий. Шаг с указанием `when: ["-"]` начнет выполняться немедленно и обеспечивает параллельное выполнение шагов.
* `when: ["id1", "id2"]` — Указывает, этот шаг зависит от действия с `id` «id1» и `id` «id2». Этот шаг не будет выполнен до тех пор, пока не будут завершены оба шага id1 и id2.
