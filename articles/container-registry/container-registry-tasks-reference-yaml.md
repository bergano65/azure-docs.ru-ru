...
| `workingDirectory` | строка | Да | Рабочий каталог контейнера во время выполнения. Если свойство задано в задаче, оно задает свойство `workingDirectory` по умолчанию для всех шагов. Если этот параметр указан на шаге, он переопределяет свойство, предоставленное задачей. | Да | `/workspace` |
| `env` | [строка, строка, ...] | Да |  Массив строк в формате `key=value`, который определяет переменные среды для задачи. Если свойство задано в задаче, оно задает свойство `env` по умолчанию для всех шагов. Если этот параметр указан на шаге, он переопределяет все переменные среды, унаследованные от задачи. | None |
| `secrets` | [секретный, секретный,...] | Да | Массив [секретных](#secret) объектов. | None |
...
| `keyvault` | строка | Да | URL-адрес Azure Key Vault секрета. | None |
| `clientID` | строка | Да | Идентификатор клиента [управляемого удостоверения, назначаемого пользователем](container-registry-tasks-authentication-managed-identity.md) для ресурсов Azure. | None |
...
| `driver` | строка | Да | Драйвер для управления сетью. | None |
...
| `network` | объект | Да | Определяет сеть, в которой выполняется контейнер. | None |
...
| `secret` | объект | Да | Определяет Azure Key Vault секретного или [управляемого удостоверения для ресурсов Azure](container-registry-tasks-authentication-managed-identity.md). | None |
