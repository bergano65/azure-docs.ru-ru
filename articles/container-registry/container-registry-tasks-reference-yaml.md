...
| `env` | [строка, строка, ...] | Да |  Массив строк в `key=value` формате, который определяет переменные среды для задачи. Если свойство задано в задаче, оно устанавливает свойство по умолчанию `env` для всех шагов. Если этот параметр указан на шаге, он переопределяет все переменные среды, унаследованные от задачи. | Нет |
...
| [`cmd`](#cmd) | Запускает контейнер в качестве команды с параметрами, передаваемыми в `[ENTRYPOINT]` контейнера. Тип `cmd` шага поддерживает такие параметры, `env`как `detach`, и другие привычные `docker run` параметры команды, позволяющие выполнять модульное и функциональное тестирование с параллельным выполнением контейнера. |
...
* Запустите образ из реестра контейнеров Azure. В следующем примере предполагается, что у вас `myregistry`есть реестр с именем и `myimage:mytag`пользовательский образ.
...
Имена переменных обычно говорят сами за себя. Ниже приведены сведения о часто используемых переменных. Начиная с версии `v1.1.0`YAML можно использовать сокращенный, предопределенный [псевдоним задачи](#aliases) вместо большинства переменных запуска. Например, вместо `{{.Run.Registry}}`используйте `$Registry` псевдоним.

### <a name="runid"></a>Run.ID

Каждое выполнение задач, `az acr run`выполняемых с помощью, или триггеров, `az acr task create`созданных с помощью, имеет уникальный идентификатор. Идентификатор представляет выполняющуюся в текущий момент команду Run.
