...
| `workingDirectory` | string | да | Рабочий каталог контейнера во время выполнения. Если свойство задано в задаче, оно устанавливает свойство по умолчанию `workingDirectory` для всех шагов. Если этот параметр указан на шаге, он переопределяет свойство, предоставленное задачей. | да | `/workspace` |
| `env` | [строка, строка, ...] | да |  Массив строк в `key=value` формате, который определяет переменные среды для задачи. Если свойство задано в задаче, оно устанавливает свойство по умолчанию `env` для всех шагов. Если этот параметр указан на шаге, он переопределяет все переменные среды, унаследованные от задачи. | да | None |
...
| `keyvault` | string | да | URL-адрес Azure Key Vault секрета. | None |
| `clientID` | string | да | Идентификатор клиента [управляемого удостоверения, назначаемого пользователем](container-registry-tasks-authentication-managed-identity.md) для ресурсов Azure. | None |
...
| `driver` | string | да | Драйвер для управления сетью. | None |
...
| Свойство | Тип | Необязательно | Описание | Значение по умолчанию |
| -------- | ---- | -------- | ----------- | ------- | 
| `name` | Строка | Нет | Имя тома для подключения. Может содержать только буквы, цифры, символы "-" и "_". | None |
| `secret` | Map [строка] строка | Нет | Каждый ключ на карте — это имя файла, созданного и заполняемого в томе. Каждое значение является строковой версией секрета. Значения секрета должны быть в кодировке Base64. | None |
...
| `entryPoint` | string | да | Переопределяет `[ENTRYPOINT]` контейнера шага. | None |
| `env` | [строка, строка, ...] | да | Массив строк в формате `key=value`, определяющий переменные среды для шага. | None |
| `expose` | [строка, строка, ...] | да | Массив портов, доступных из контейнера. |  None |
...
| `network` | object | да | Определяет сеть, в которой выполняется контейнер. | None |
| `ports` | [строка, строка, ...] | да | Массив портов, опубликованных из контейнера на узле. |  None |
...
| `secret` | object | да | Определяет Azure Key Vault секретного или [управляемого удостоверения для ресурсов Azure](container-registry-tasks-authentication-managed-identity.md). | None |
...
| `user` | string | да | Имя пользователя или UID контейнера | None |
| `volumeMounts` | object | Нет | Массив объектов [волумемаунт](#volumemount) . | None |
| `workingDirectory` | string | да | Задает рабочий каталог для шага. По умолчанию служба "Задачи ACR" создает корневой каталог в качестве рабочего каталога. Но если сборка включает несколько шагов, для предыдущих и последующих шагов можно использовать общие артефакты, указав один и тот же рабочий каталог. | `/workspace` |
...
| Свойство | Тип | Необязательно | Описание | Значение по умолчанию |
| -------- | ---- | -------- | ----------- | ------- | 
| `name` | Строка | Нет | Имя тома для подключения. Должно точно соответствовать имени из `volumes` Свойства. | None |
| `mountPath`   | string | нет | Абсолютный путь для подключения файлов в контейнере.  | None |
