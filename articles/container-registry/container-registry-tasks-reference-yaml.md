...
| Свойство | Тип | Необязательно | Description | Поддерживается ли переопределение | Значение по умолчанию |
| -------- | ---- | -------- | ----------- | ------------------ | ------------- |
...
| `stepTimeout` | int (секунды) | Да | Максимальное число секунд выполнения шага. Если свойство задано в задаче, оно задает свойство `timeout` по умолчанию для всех шагов. Если свойство `timeout` задано на шаге, оно переопределяет свойство, предоставленное задачей. | Да | 600 (10 минут) |
| `workingDirectory` | string | Да | Рабочий каталог контейнера во время выполнения. Если свойство задано в задаче, оно задает свойство `workingDirectory` по умолчанию для всех шагов. Если этот параметр указан на шаге, он переопределяет свойство, предоставленное задачей. | Да | `/workspace` |
| `env` | [строка, строка, ...] | Да |  Массив строк в формате `key=value`, который определяет переменные среды для задачи. Если свойство задано в задаче, оно задает свойство `env` по умолчанию для всех шагов. Если этот параметр указан на шаге, он переопределяет все переменные среды, унаследованные от задачи. | Нет |
| `secrets` | [секретный, секретный,...] | Да | Массив [секретных](#secret) объектов. | Нет |
| `networks` | [сеть, сеть,...] | Да | Массив [сетевых](#network) объектов. | Нет |

### <a name="secret"></a>secret

Секретный объект имеет следующие свойства.

| Свойство | Тип | Необязательно | Description | Значение по умолчанию |
| -------- | ---- | -------- | ----------- | ------- |
| `id` | string | Нет | Идентификатор секрета. | Нет |
| `keyvault` | string | Да | URL-адрес Azure Key Vault секрета. | Нет |
| `clientID` | string | Да | Идентификатор клиента [управляемого удостоверения, назначаемого пользователем](container-registry-tasks-authentication-managed-identity.md) для ресурсов Azure. | Нет |
...
| `disableWorkingDirectoryOverride` | bool | Да | Следует ли отключать функции переопределения `workingDirectory`. Используйте его в сочетании с `workingDirectory`, чтобы получить полный контроль над рабочим каталогом контейнера. | `false` |
...
| `user` | string | Да | Имя пользователя или UID контейнера | Нет |
