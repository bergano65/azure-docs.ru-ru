...
| `keyvault` | string | Да | URL-адрес Azure Key Vault секрета. | Отсутствуют |
| `clientID` | string | Да | Идентификатор клиента [управляемого удостоверения, назначаемого пользователем](container-registry-tasks-authentication-managed-identity.md) для ресурсов Azure. | Отсутствуют |
...
| `driver` | string | Да | Драйвер для управления сетью. | Отсутствуют |
...
| `entryPoint` | string | Да | Переопределяет `[ENTRYPOINT]` контейнера шага. | Отсутствуют |
...
| `user` | string | Да | Имя пользователя или UID контейнера | Отсутствуют |
...
Имена переменных обычно говорят сами за себя. Ниже приведены сведения о часто используемых переменных. Начиная с версии YAML `v1.1.0` можно использовать сокращенный, предопределенный [псевдоним задачи](#aliases) вместо большинства переменных запуска. Например, вместо `{{.Run.Registry}}` Используйте `$Registry` псевдоним.
