...
| -------- | ---- | -------- | ----------- | ------------------ | ------------- |
| `version` | string | Да | Версия файла `acr-task.yaml`, проанализированного службой "Задачи ACR". В то время как служба "Задачи ACR" стремится поддерживать обратную совместимость, это значение позволяет службе поддерживать совместимость в пределах заданной версии. Если этот атрибут не задан, по умолчанию до последней версии. | Нет | Нет |
| `stepTimeout` | int (секунды) | Да | Максимальное число секунд выполнения шага. Если свойство задается в задаче, он задает значение по умолчанию `timeout` свойства всех шагов. Если `timeout` свойство указано на шаге, оно переопределяет свойство, предоставленные задачей. | Да | 600 (10 минут) |
| `workingDirectory` | string | Да | Рабочий каталог контейнера во время выполнения. Если свойство задается в задаче, он задает значение по умолчанию `workingDirectory` свойства всех шагов. Если указано в шаге, оно переопределяет свойство, предоставленные задачей. | Да | `$HOME` |
| `env` | [строка, строка, ...] | Да |  Массив строк в `key=value` формат, определение переменных среды для задачи. Если свойство задается в задаче, он задает значение по умолчанию `env` свойства всех шагов. Если указано в шаге, он переопределяет все переменные среды, унаследованные от задачи. | Нет |
| `secrets` | [секрета, секрет,...] | Да | Массив [секрет](#secret) объектов. | Нет |
| `networks` | [, сети,...] | Да | Массив [сети](#network) объектов. | Нет |

### <a name="secret"></a>секрет

Объект секрета имеет следующие свойства.

| Свойство | type | Необязательно | Описание | Значение по умолчанию |
| -------- | ---- | -------- | ----------- | ------- |
| `id` | string | Нет | Идентификатор секрета. | Нет |
| `keyvault` | string | Да | Azure Key Vault URL-адрес секрета. | Нет |
| `clientID` | string | Да | Идентификатор клиента для назначаемого пользователем управляемого удостоверения для ресурсов Azure. | Нет |
...
| `driver` | string | Да | Драйвер для управления сетью. | Нет |
| `ipv6` | bool | Да | Указывает, включена ли сетей IPv6. | `false` |
| `skipCreation` | bool | Да | Нужно ли пропустить создание сети. | `false` |
| `isDefault` | bool | Да | Является ли сеть в сеть по умолчанию, предоставляемые с помощью реестра контейнеров Azure | `false` |
...
| `disableWorkingDirectoryOverride` | bool | Да | Следует ли отключить `workingDirectory` переопределение функциональности. Использовать в сочетании с `workingDirectory` иметь полный контроль над контейнера рабочий каталог. | `false` |
...
| `expose` | [строка, строка, ...] | Да | Массив порты, которые доступны из контейнера. |  Нет |
...
| `ignoreErrors` | bool | Да | Нужно ли пометьте этап как успешной независимо от того, произошла ли ошибка во время выполнения контейнера. | `false` |
| `isolation` | string | Да | Уровень изоляции контейнера. | `default` |
| `keep` | bool | Да | Следует ли сохранить контейнер этого шага после выполнения. | `false` |
| `network` | object | Да | Идентифицирует сети, в котором выполняется контейнер. | Нет |
| `ports` | [строка, строка, ...] | Да | Массив порты, которые публикуются из контейнера на узле. |  Нет |
| `pull` | bool | Да | Необходимость принудительного извлечения контейнера перед выполнением его, чтобы предотвратить любое поведение кэширования. | `false` |
| `privileged` | bool | Да | Необходимость запуска контейнера в привилегированном режиме. | `false` |
| `repeat` | int | Да | Число повторных попыток для повторения выполнения контейнера. | 0 |
| `retries` | int | Да | Число повторных попыток при сбое выполнения контейнера. Повторная попытка предпринимается только в том случае, если код выхода контейнера имеет ненулевое значение. | 0 |
| `retryDelay` | int (секунды) | Да | Задержка в секундах между повторными попытками выполнения контейнера. | 0 |
| `secret` | object | Да | Определяет, секрет хранилища ключей Azure или управляемого удостоверения для ресурсов Azure. | Нет |
| `startDelay` | int (секунды) | Да | Число секунд задержки выполнения контейнера. | 0 |
...
| `user` | string | Да | Имя пользователя или идентификатор UID данного контейнера | Нет |
...
Для задачи, запускаемой фиксации в репозиторий GitHub, а идентификатор фиксации.

### <a name="runbranch"></a>Run.Branch

Для задачи, запускаемой фиксации в репозиторий GitHub, укажите имя ветви.
