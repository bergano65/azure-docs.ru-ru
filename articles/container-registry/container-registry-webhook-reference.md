---
title: Ссылка на схему веб-крючка реестра
description: Ссылка на полезную нагрузку JSON для запросов webhook в реестре контейнеров Azure, которые генерируются при включении webhooks для нажатия или удаления событий артефакта
ms.topic: article
ms.date: 03/05/2019
...
Вы можете [настроить веб-перехватчики](container-registry-webhook.md) для реестра контейнеров, которые создают события при выполнении с ними определенных действий. Например, включите веб-крючки, которые срабатывают при перемещении изображения контейнера или диаграммы руля в реестр или удалены. Когда срабатывает веб-перехватчик, реестр контейнеров Azure направляет на указанную вами конечную точку запрос HTTP или HTTPS с информацией об этом событии. Конечная точка может обработать данные от веб-перехватчика и выполнить необходимые действия.
...
|[цель](#target)|Сложный тип|Целевой объект для действия, которое привело к созданию события веб-перехватчика.|
|[Запрос](#request)|Сложный тип|Запрос, который создал событие веб-перехватчика.|

### <a name="target"></a><a name="target"></a>Целевой
...
### <a name="payload-example-image-push-event"></a>Пример полезной нагрузки: событие нажатия изображения
...
Пример команды [Docker CLI,](https://docs.docker.com/engine/reference/commandline/cli/) которая запускает веб-крюк события **нажатия изображения:**

```bash
docker push myregistry.azurecr.io/hello-world:v1
```

## <a name="chart-push-event"></a>Событие нажатия диаграммы

Веб-крюк срабатывает при перемещении диаграммы Helm в репозиторий.

### <a name="chart-push-event-payload"></a>Полезная нагрузка события push-нажиме диаграммы
...
|`timestamp`|Дата и время|Время создания события веб-перехватчика.|
|`action`|Строка|Действие, которое привело к созданию события веб-перехватчика.|
|[цель](#helm_target)|Сложный тип|Целевой объект для действия, которое привело к созданию события веб-перехватчика.|

### <a name="target"></a><a name="helm_target"></a>Целевой
...
### <a name="payload-example-chart-push-event"></a>Пример полезной нагрузки: событие нажатия диаграммы
...
Пример команды [Azure CLI,](/cli/azure/acr) которая запускает веб-крюк **chart_push** событий:
...
Webhook срабатывает при удалении репозитория изображения или манифеста. Событие не создается при удалении тега.
...
|`timestamp`|Дата и время|Время создания события веб-перехватчика.|
|`action`|Строка|Действие, которое привело к созданию события веб-перехватчика.|
|[цель](#delete_target)|Сложный тип|Целевой объект для действия, которое привело к созданию события веб-перехватчика.|
|[Запрос](#delete_request)|Сложный тип|Запрос, который создал событие веб-перехватчика.|

### <a name="target"></a><a name="delete_target"></a>Целевой
...
### <a name="request"></a><a name="delete_request"></a>Запрос
...
Веб-хук срабатывает при удалении диаграммы или репозитория Helm. 

### <a name="chart-delete-event-payload"></a>Диаграмма удалить полезную нагрузку события
...
|`timestamp`|Дата и время|Время создания события веб-перехватчика.|
|`action`|Строка|Действие, которое привело к созданию события веб-перехватчика.|
|[цель](#chart_delete_target)|Сложный тип|Целевой объект для действия, которое привело к созданию события веб-перехватчика.|

### <a name="target"></a><a name="chart_delete_target"></a>Целевой
...
### <a name="payload-example-chart-delete-event"></a>Пример полезной нагрузки: событие удаления диаграммы
...
Пример команды [Azure CLI,](/cli/azure/acr) которая запускает веб-крюк **chart_delete** событий:
