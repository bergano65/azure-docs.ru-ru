...
Используя знакомый формат `docker build`, команда [AZ запись контроля][az-acr-build] доступа в Azure CLI принимает [контекст](#context-locations) (набор файлов для сборки), отправляет задачи записи контроля доступа и по умолчанию передает созданный образ в реестр после завершения.
...
По умолчанию задачи записи контроля доступа строят образы для ОС Linux и архитектуры AMD64. Укажите тег `--platform`, чтобы создать образы Windows или образы Linux для других архитектур. Укажите операционную систему и, возможно, поддерживаемую архитектуру в формате ОС/архитектуры (например, `--platform Linux/arm`). Для архитектур ARM можно дополнительно указать вариант в формате OS/Architecture/Variant (например, `--platform Linux/arm64/v8`):
