...
description: Создание токена с разрешениями, ограниченными конкретными репозиториями в реестре для извлечения или отправки образов или выполнения других действий
ms.topic: article
ms.date: 02/13/2020
...
* Эта функция доступна в реестрах контейнеров уровня **Премиум**. Сведения об уровнях служб реестра и ограничениях см. в статье [Уровни служб Реестра контейнеров Azure](container-registry-skus.md).
...
* Разрешения для репозитория групп **карт области**, которые применяются к токену или могут повторно применяться к другим токенам. Каждый токен связан с одной картой области. 
  ...
  Реестр контейнеров Azure также предоставляет несколько определяемых системой карт области, которые вы можете применить, с фиксированными разрешениями во всех репозиториях.
...
В выходных данных отображаются сведения о токене, включая два созданных пароля. Рекомендуется сохранить эти пароли в надежном месте для последующего использования при проверке подлинности. Повторно получить эти пароли невозможно, но можно создать новые.
...
1. В разделе **Службы** выберите **Токены (предварительная версия) > + Добавить**.
...
После создания токена создайте пароль. Для проверки подлинности в реестре токен должен быть включен и иметь действительный пароль.

Можно создать один или два пароля и задать дату окончания срока действия для каждого из них. 

1. На портале перейдите к нужному реестру контейнеров.
1. В разделе **Службы** выберите **Токены (предварительная версия)** и нужный токен.
...
1. После создания пароля скопируйте и сохраните его в надежном месте. После закрытия экрана вы не сможете повторно получить созданный пароль, но сможете создать новый.
    ...
    ![Создание пароля токена на портале](media/container-registry-repository-scoped-permissions/portal-token-password.png)

## <a name="authenticate-with-token"></a>Проверка подлинности с помощью токена

Когда пользователь или служба используют токен для проверки подлинности в целевом реестре, они предоставляют имя пользователя и один из созданных паролей. Метод проверки подлинности зависит от настроенных действий, связанных с токеном.
...
Выполните команду `docker login` для проверки подлинности в реестре, укажите имя токена в качестве имени пользователя и укажите один из его паролей. Токен должен иметь состояние `Enabled`.
...
### <a name="change-pushpull-permissions"></a>Изменение разрешений отправки/извлечения
...
Чтобы удалить репозиторий `samples/alpine`, используйте следующую команду [az acr repository delete][az-acr-repository-delete]. Для удаления образов или репозиториев токен не проходит проверку подлинности с помощью `docker login`. Вместо этого передайте имя и пароль токена в команду. В следующем примере используются переменные среды, созданные ранее в этой статье.
...
Для чтения метаданных токен не проходит проверку подлинности с помощью `docker login`. Вместо этого передайте имя и пароль токена в любую из команд. В следующем примере используются переменные среды, созданные ранее в этой статье.
...
В выходных данных отображаются определенные вами карты области, а также несколько определенных системой карт области, которые можно использовать для настройки токенов:
...
Если у вас нет пароля для токена или вы хотите создать новые пароли, выполните команду [az acr token credential generate][az-acr-token-credential-generate]. 
