...
В этой статье предполагается, что вы уже создали кластер AKS и у вас есть доступ к кластеру через клиент командной строки `kubectl`. Если вместо этого вы хотите создать кластер и настроить доступ к реестру контейнеров во время создания кластера, см. раздел [Tutorial: Разверните кластер AKS @ no__t-0 или выполните [аутентификацию с помощью реестра контейнеров Azure из службы Azure Kubernetes](../aks/cluster-container-registry-integration.md).
...
Теперь учетные данные субъекта-службы можно хранить в Kubernetesном секрете на [Извлечение образа][image-pull-secret], который будет ссылаться на кластер AKS при работе с контейнерами.
