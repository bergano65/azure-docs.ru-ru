...
Общие сведения об этих концепциях см. в статье [о реестрах, репозиториях и](container-registry-concepts.md)образах.
...
Чтобы удалить по тегу, используйте команду [AZ запись контроля доступа Delete][az-acr-repository-delete] и укажите имя образа в `--image` параметре. Удаляются все уникальные слои образа и все другие теги, связанные с удаляемым образом.
...
> [!TIP]
> Удаление *по тегу* не следует путать с удалением тега (обестегивание). Вы можете удалить тег с помощью команды Azure CLI [AZ удалить тег Repository репозитория][az-acr-repository-untag]. При удалить теги и изображения пробелы не освобождаются, поскольку данные [манифеста](container-registry-concepts.md#manifest) и слоя остаются в реестре. Удаляется только ссылка на тег.
...
Затем укажите дайджест, который вы хотите удалить, в команде [AZ][az-acr-repository-delete] запись в репозитории удаления. Команда имеет следующий формат.
...
> [!WARNING]
> Используйте следующий пример сценария с осторожностью: удаленные данные изображения являются невосстанавливаемыми. При наличии систем, которые извлекают образы по дайджесту манифеста (в отличие от имени образа), не следует запускать эти сценарии. Удаление дайджестов манифеста не позволит этим системам извлечь образы из реестра. Вместо того чтобы извлекать манифест, рекомендуется использовать уникальную схему *тегов* , [рекомендуемую рекомендацию][tagging-best-practices]. 
...
> [!WARNING]
> С осторожностью используйте следующие примеры сценариев — данные удаленных изображений восстановлению не подлежат. При наличии систем, которые извлекают образы по дайджесту манифеста (в отличие от имени образа), не следует запускать эти сценарии. Удаление образов без тегов не позволит этим системам получать образы из реестра. Вместо того чтобы извлекать манифест, рекомендуется использовать уникальную схему *тегов* , [рекомендуемую рекомендацию][tagging-best-practices].
