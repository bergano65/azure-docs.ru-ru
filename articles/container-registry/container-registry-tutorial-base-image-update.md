...
* В настоящее время для сборок изображений из Dockerfile задача ACR обнаруживает зависимости от базовых образов в том же реестре контейнера Azure, общедоступном репозитории Docker Hub или общедоступном репозитории в реестре контейнеров Майкрософт. Если базовый образ, указанный в инструкции `FROM`, находится в одном из этих мест, задача ACR добавляет веб-перехватчик, чтобы гарантировать, что он будет перестроен в любое время при обновлении его базы.
