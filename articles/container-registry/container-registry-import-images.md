...
* Поскольку клиентская среда не нуждается в локальной установке Docker, импортируйте изображение контейнера, независимо от поддерживаемого типа ОС.
...
Чтобы импортировать образы контейнеров при работе с этой статьей, требуется запустить Azure CLI в Azure Cloud Shell или локально (рекомендуется версия 2.0.55 или более поздняя). Чтобы узнать версию, выполните команду `az --version`. Если вам нужно установить или обновить, [см.][azure-cli]

> [!NOTE]
> Если нужно распространить идентичные образы контейнеров в нескольких регионах Azure, Реестр контейнеров Azure также поддерживает [георепликацию](container-registry-geo-replication.md). Гео-реплицируя реестр (требуется уровень обслуживания Премиум), вы можете обслуживать несколько регионов с одинаковым изображением и именами тегов из единого реестра.
>

## <a name="prerequisites"></a>Предварительные требования

Если у вас еще нет Реестра контейнеров Azure, создайте его. Для этапов [см. квикстарт: Создайте частный реестр контейнеров с помощью Azure CLI.](container-registry-get-started-azure-cli.md)
...
В следующем примере *mysourceregistry* находится в другой подписке, нежели *myregistry*, и в одном и том же клиенте Active Directory. Укажите идентификатор ресурса исходного реестра с параметром `--registry`. Обратите внимание, что `--source` параметр определяет только репозиторий и тег и тег, а не имя сервера входа в реестр.
