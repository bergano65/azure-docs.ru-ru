...
* `--concurrency` — Указывает, что задачи очистки x обрабатываются параллельно. Если этот параметр не указан, будет использоваться значение по умолчанию.
