---
title: Управление подписанными изображениями
description: Узнайте, как обеспечить доверие к содержимому для реестра контейнеров Azure, а также нажать и вытащить подписанные изображения.
ms.topic: article
ms.date: 09/06/2019
...
> [!NOTE]
> Доверие к содержимому — это функция [SKU ценовой категории "Премиум"](container-registry-skus.md) Реестра контейнеров Azure.
...
Сначала перейдите в реестр на портале Azure. В разделе **Политики** последовательно выберите **Доверие к содержимому** > **Включено** > **Сохранить**. В Azure CLI также можно использовать команду [обновления содержимого-доверия az acr.][az-acr-config-content-trust-update]
...
> [!NOTE]
> Вы не можете предоставить доверенное разрешение на нажатие изображения на [учетную запись admin](container-registry-authentication.md#admin-account) контейнерного реестра Azure.

Далее описано, как назначить роль `AcrImageSigner` на портале Azure и в инфраструктуре Azure CLI.

### <a name="azure-portal"></a>Портал Azure

Перейдите в свой реестр на портале Azure, а затем выберите **назначение ролей control (IAM)** > **Add role assignment**. В разделе **Добавление назначения ролей** выберите `AcrImageSigner` в поле **Роль**, затем одного или нескольких пользователей либо один или несколько субъектов-служб в поле **Выбрать** и выберите **Сохранить**.
...
> [!IMPORTANT]
> После изменения роли выполните `az acr login` для обновления локального маркера идентификации для Azure CLI, чтобы новые роли вступили в действие. Для получения информации о проверке ролей для идентификации см. [Управление доступом к ресурсам Azure с помощью RBAC и Azure CLI](../role-based-access-control/role-assignments-cli.md) и [Troubleshoot RBAC для ресурсов Azure.](../role-based-access-control/troubleshooting.md)
...
* Дополнительные сведения о доверии к содержимому см. в разделе [Content trust in Docker][docker-content-trust] (Доверие к содержимому в Docker). В этой статье мы затронули лишь ключевые моменты обширной темы доверия к содержимому, которая более подробно рассматривается в документации Docker.
