...
* Чтобы обслуживать большие двоичные объекты, представляющие уровни содержимого, служба "Реестр контейнеров Azure" использует конечные точки данных. Вы можете включить [выделенные конечные точки данных](container-registry-firewall-access-rules.md#enable-dedicated-data-endpoints) для реестра в каждом регионе, в который геореплицируется этот реестр. Эти конечные точки позволяют настроить правила доступа брандмауэра со строго ограниченными областями.
