...
description: Приступить к созданию и управлению геореплицированным реестром контейнеров Azure, который позволяет реестру обслуживать несколько регионов с многомастерными региональными репликами.
author: stevelas
ms.topic: article
ms.date: 08/16/2019
ms.author: stevelas
...
Настроить георепликацию так же просто, как и выбрать регионы на карте. Можно также управлять георепликацией с помощью инструментов, включая команды [репликации az acr](/cli/azure/acr/replication) в Azure CLI, или развертывать реестр, включенный для георепликации с [шаблоном Azure Resource Manager.](https://github.com/Azure/azure-quickstart-templates/tree/master/101-container-registry-geo-replication)
...
* Для управления рабочими процессами, зависят от нажатия обновлений на геореплицикатные данные, мы рекомендуем настроить [веб-крючки](container-registry-webhook.md) для реагирования на события push. Можно настроить региональные веб-перехватчики в геореплицированном реестре, чтобы отслеживать выполнение событий отправки во всех геореплицированных регионах.

## <a name="delete-a-replica"></a>Удаление реплики

После настройки реплики для реестра вы можете удалить ее в любое время, если она больше не нужна. Удалите реплику с помощью портала Azure или других инструментов, таких как команда [удаления репликации az acr](/cli/azure/acr/replication#az-acr-replication-delete) в Azure CLI.

Чтобы удалить реплику на портале Azure:

1. Перейдите в реестр контейнеров Azure и выберите **репликации.**
1. Выберите имя реплики и **удалите .** Подтвердите, что вы хотите удалить реплику.

> [!NOTE]
> Вы не можете удалить копию реестра в *домашней области* реестра, то есть место, где вы создали реестр. Удалить домашнюю реплику можно только удалив сам реестр.
