...
1. Отправьте тестовое изображение `hello-world:latest` в реестр или замените другое тестовое изображение по своему усмотрению.
1. Удалить тег образ `hello-world:latest`, например, с помощью команды [AZ Repository удалить тег][az-acr-repository-untag] . Манифест без тегов остается в реестре.
    ```azurecli
