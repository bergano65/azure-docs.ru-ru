...
Смотрите также [Рекомендации для пометки и версии изображений контейнеров](container-registry-image-tag-version.md) для стратегий пометить и версии изображений в реестре. 
...
Например, рассмотрим следующие теги образа контейнера. Изображения, которые используются по `aspnetcore`всей корпоративной организации, например, размещаются в корневом пространстве имен, в то время как изображения контейнеров, принадлежащих группам продуктов и маркетингу, используют свои собственные пространства имен.
...
|---|---|---|
| Отдельное удостоверение | Разработчик извлекает образы на свой компьютер для разработки или отправляет образы с него. | [az acr логин](/cli/azure/acr?view=azure-cli-latest#az-acr-login) |
| Удостоверение службы или автономное удостоверение | Конвейеры сборки и развертывания, в которых пользователь не участвует напрямую. | [Директор службы](container-registry-authentication.md#service-principal) |
