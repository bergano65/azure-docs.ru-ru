...
![Топология](media/signalr-disaster-recovery/topology.png)
...
Рис. 1. Перед отработкой отказа ![Before Failover](media/signalr-disaster-recovery/before-failover.png)

Рис. 2. После отработки отказа ![After Failover](media/signalr-disaster-recovery/after-failover.png)

Рис. 3. Спустя некоторое время после восстановления первичной службы ![Short time after primary recovers](media/signalr-disaster-recovery/after-recover.png)
...
Из этой статьи вы узнали, как настроить приложение и обеспечить устойчивость Службы Azure SignalR. Дополнительные сведения о подключении "сервер — клиент" и маршрутизации подключений Службы Azure SignalR см. в [этой статье](signalr-internals.md) с описанием внутренних компонентов Службы Azure SignalR.