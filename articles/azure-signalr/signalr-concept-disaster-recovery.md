...
Ваш экземпляр службы всегда работает в одном регионе и не выйдет из строя в другой регион, когда происходит сбой в общерегионе.
...
С помощью этой функции вы сможете восстановиться при катастрофе, но вам нужно настроить правильную топологию системы самостоятельно. Из этого документа вы узнаете, как выполнить настройку.
...
Первичный экземпляр и сервер приложения подключаются через обычные соединения сервера, но вторичный экземпляр и сервер приложения подключаются через специальный тип соединения, называемый слабым соединением.
Основное отличие слабого соединения заключается в том, что оно не принимает направление клиентского соединения, так как вторичный экземпляр находится в другом регионе. Маршрутизирование клиента в другой регион не является оптимальным выбором (увеличивает задержку).
...
Вы уже должны знать, как установить строку подключения службы SignalR через переменные среды `Azure:SignalR:ConnectionString`/ настройки приложения/web.cofig, в конфигурации вступления с именем .
...
Если вы предпочитаете хранить строки соединения в другом месте, вы также можете прочитать их в коде и использовать их в качестве параметров при вызове `AddAzureSignalR()` (в ASP.NET Core) или `MapAzureSignalR()` (в ASP.NET).
...
Можно настроить несколько первичных или вторичных экземпляров. Если существует несколько первичных и/или вторичных экземпляров, переговоры вернут конечную точку в следующем порядке:

1. Если в Сети есть хотя бы один основной экземпляр, верните случайный первичный экземпляр в Интернете.
2. Если все основные экземпляры не доходят, верните случайный вторичный экземпляр онлайн.
...
Следует отметить, какие шаблоны вы не хотите использовать, вам нужно подключить каждый экземпляр службы SignalR к серверу приложения в качестве основного.
...
Для сценариев масштабирования, таких как sharding, которые используют несколько экземпляров вместе для обработки большого количества соединений, прочитайте, [как масштабировать несколько экземпляров.](signalr-howto-scale-multi-instances.md)
