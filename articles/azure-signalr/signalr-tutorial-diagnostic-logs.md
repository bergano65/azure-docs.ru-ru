...
Имя | Описание
------- | -------
type | Тип события журнала. В настоящее время мы предоставляем сведения о подключении к службе Azure SignalR. Доступен `ConnectivityLogs` только тип
...
Collection | Коллекция событий журнала. Допустимые значения: `Connection` `Authorization` и`Throttling`
OperationName | Имя операции для события
...
Разница между `ConnectionAborted` и `ConnectionEnded` заключается в том `ConnectionEnded` , что ожидается отключение, которое активируется на стороне клиента или сервера. Несмотря на `ConnectionAborted` то, что обычно является непредвиденным событием удаления соединения, а причина прерывания будет `message`предоставлена в.
