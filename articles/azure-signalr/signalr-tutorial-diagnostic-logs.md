---
title: Журналы диагностики для службы Azure SignalR
description: Узнайте, как настроить журналы диагностики для службы Azure SignalR и как использовать ее для самостоятельного устранения неполадок.
author: wanl
ms.service: signalr
ms.topic: conceptual
ms.date: 12/17/2019
...
ms.locfileid: "75750260"
---
# <a name="diagnostic-logs-for-azure-signalr-service"></a>Журналы диагностики для службы Azure SignalR

В этом руководстве содержатся сведения о журналах диагностики для службы Azure SignalR и настройке журналов диагностики, а также об устранении неполадок с журналами диагностики.

## <a name="prerequisites"></a>Технические условия
Чтобы включить журналы диагностики, вам понадобится место для хранения данных журнала. В этом руководстве используется служба хранилища Azure и Log Analytics.

* Служба [хранилища Azure](../azure-monitor/platform/resource-logs-collect-storage.md) — хранение журналов диагностики для аудита политики, статического анализа или резервного копирования.
...
## <a name="set-up-diagnostic-logs-for-an-azure-signalr-service"></a>Настройка журналов диагностики для службы Azure SignalR

Вы можете просмотреть журналы диагностики для службы Azure SignalR. Эти журналы предоставляют более подробные сведения о подключении к экземпляру службы Azure SignalR. В журналах диагностики содержатся подробные сведения о каждом подключении. Например, основные сведения (идентификатор пользователя, идентификатор соединения и тип транспорта и т. д.) и сведения о событии (подключение, отключение и прерывание события и т. д.) соединения. Журналы диагностики можно использовать для идентификации проблем, отслеживания и анализа подключений.
...
1.  Затем щелкните **Добавить параметр диагностики**.
    ...
    ![Добавление журналов диагностики](./media/signalr-tutorial-diagnostic-logs/add-diagnostic-setting.png)
...
Служба Azure SignalR захватывает журналы диагностики в одной категории:
...
Журналы хранятся в учетной записи хранения, настроенной на панели **журналы диагностики** . Контейнер с именем `insights-logs-alllogs` создается автоматически для хранения журналов диагностики. Внутри контейнера журналы хранятся в файле `resourceId=/SUBSCRIPTIONS/XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX/RESOURCEGROUPS/XXXX/PROVIDERS/MICROSOFT.SIGNALRSERVICE/SIGNALR/XXX/y=YYYY/m=MM/d=DD/h=HH/m=00/PT1H.json`. По сути, путь объединяется `resource ID` и `Date Time`. Файлы журнала разбиваются по `hour`. Таким образом, минуты всегда `m=00`.
...
Имя | Description
------- | -------
type | Тип события журнала. В настоящее время мы предоставляем сведения о подключении к службе Azure SignalR. Доступен только тип `ConnectivityLogs`
...
Чтобы просмотреть журналы диагностики, выполните следующие действия.

1. Щелкните `Logs` в целевом Log Analytics.

    ![Пункт меню Log Analytics](./media/signalr-tutorial-diagnostic-logs/log-analytics-menu-item.png)

2. Введите `SignalRServiceDiagnosticLogs` и выберите диапазон времени, чтобы запросить журналы диагностики. Дополнительные сведения см. в статье Начало [работы с log Analytics в Azure Monitor](../azure-monitor/log-query/get-started-portal.md)
...
### <a name="troubleshooting-with-diagnostic-logs"></a>Устранение неполадок с помощью журналов диагностики

Чтобы устранить неполадки в службе Azure SignalR, можно включить журналы на стороне сервера или клиента для записи ошибок. В настоящее время служба Azure SignalR предоставляет журналы диагностики, также можно включить журналы для стороны службы.

При возникновении непредвиденного увеличения или удаления подключения можно воспользоваться журналами диагностики для устранения неполадок.
...
Если подключение будет отключено, журналы диагностики будут регистрировать это событие, которое будет отключаться, `ConnectionAborted` или `ConnectionEnded` в `operationName`.

Различие между `ConnectionAborted` и `ConnectionEnded` заключается в том, что `ConnectionEnded` является ожидаемым отключением, которое активируется на стороне клиента или сервера. Хотя `ConnectionAborted` обычно является непредвиденным событием разрыва соединения, а причина прерывания будет предоставлена в `message`.
...
Чтобы устранить неполадки, связанные с непредвиденным подключением, сначала нужно отфильтровать дополнительные подключения. В тестовое подключение клиента можно добавить уникальный идентификатор тестового пользователя. Если вы видите несколько клиентских подключений с одинаковым ИДЕНТИФИКАТОРом или IP-адресом тестового пользователя, убедитесь, что на стороне клиента создается и устанавливается больше подключений, чем предполагается. Проверьте клиентскую часть.

#### <a name="authorization-failure"></a>Сбой авторизации

Если для клиентских запросов получено 401 несанкционированный доступ, проверьте журналы диагностики. Если вы сталкиваетесь с `Failed to validate audience. Expected Audiences: <valid audience>. Actual Audiences: <actual audience>`, это означает, что все аудитории в маркере доступа являются недопустимыми. Попробуйте использовать допустимые аудитории, предложенные в журнале.


#### <a name="throttling"></a>Регулирование

Если вы обнаружите, что вы не можете установить клиентские подключения SignalR в службу Azure SignalR, проверьте журналы диагностики. При возникновении `Connection count reaches limit` в журнале диагностики устанавливается слишком много подключений к службе SignalR, что достигает предельного числа подключений. Рассмотрите возможность масштабирования службы SignalR. При возникновении `Message count reaches limit` в журнале диагностики это означает, что вы используете бесплатный уровень и используете квоту сообщений. Если вы хотите отправить больше сообщений, рассмотрите возможность изменения службы SignalR на уровень Standard для отправки дополнительных сообщений. Дополнительные сведения см. на странице [цен на службу Azure SignalR](https://azure.microsoft.com/pricing/details/signalr-service/).

### <a name="get-help"></a>Справка

Мы рекомендуем сначала устранить неполадки. Большинство проблем вызваны неполадками сервера приложений или сети. Выполните [инструкции по устранению неполадок с журналом диагностики](#troubleshooting-with-diagnostic-logs) и [базовым руководством по решению проблем](https://github.com/Azure/azure-signalr/blob/dev/docs/tsg.md) , чтобы найти основную причину.
...
> Примечание. Если вы открыли вопрос в GitHub, сохраните конфиденциальную информацию (например, "идентификатор ресурса", "журналы сервера/клиента") в частном порядке, отправку только членам в Организации корпорации Майкрософт.  
