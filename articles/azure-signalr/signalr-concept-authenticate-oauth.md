...
description: Узнайте, как реализовать собственную аутентификацию и интегрировать ее в службу Azure SignalR, следуя примеру e2e.
author: sffamily
ms.service: signalr
ms.topic: conceptual
ms.date: 11/13/2019
...
* [Настройка облачной оболочки Azure](https://docs.microsoft.com/azure/cloud-shell/quickstart)
...
2. Для вашей учетной записи перейдите к**настройкам разработчика** **настроек** > и нажмите **Зарегистрируйтесь в новом приложении**или в **приложении New OAuth** под *приложениями OAuth.*
...
3. Для нового приложения OAuth используйте следующие параметры, а затем щелкните **Регистрация приложения**.
    ...
    | Описание приложения | *Образец чата с помощью службы Azure SignalR с аутентификацией GitHub* | Полезное описание приложения, которое поможет пользователям приложения распознать содержимое используемой аутентификации. |
...
4. Запустите окно браузера и перейдите к `http://localhost:5000`. Чтобы войти с помощью GitHub, щелкните **эту** ссылку вверху.
    ...
    ![Авторизация приложения OAuth](media/signalr-concept-authenticate-oauth/signalr-authorize-oauth-app.png)
    ...
    ![Обнаружение учетной записи](media/signalr-concept-authenticate-oauth/signalr-oauth-account-identified.png)
...
1. Откройте [https://github.com](https://github.com) в браузере и перейдите к**настройкам** >  **настройки разработчика вашей** > учетной записи**Oauth Apps.**
...
В **фильтре по имени...** textbox введите имя группы ресурсов. В инструкциях в этой статье использовалась группа ресурсов с именем *SignalRTestResources*. В своей группе ресурсов в списке результатов щелкните **...**, а затем **Удалить группу ресурсов**.
...
> [Примеры CLI для Azure SignalR](./signalr-reference-cli.md)