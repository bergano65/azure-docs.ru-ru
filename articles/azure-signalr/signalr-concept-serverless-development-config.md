...
Пакеты SDK клиента SignalR уже содержат логику, необходимую для выполнения подтверждения согласования. Передайте в `HubConnectionBuilder`пакета SDK URL-адрес конечной точки для согласования, за исключением сегмента `negotiate`. Ниже приведен пример в JavaScript:
...
При запуске приложения-функции на локальном компьютере можно добавить `Host` раздел в *Local. Settings. JSON* , чтобы включить CORS. В разделе `Host` добавьте два свойства:

* `CORS` — введите базовый URL-адрес, который является источником клиентского приложения.
* `CORSCredentials` — задайте для него значение `true`, чтобы разрешить запросы "Вискредентиалс".
...
После настройки прошедшие проверку подлинности HTTP-запросы будут содержать заголовки `x-ms-client-principal-name` и `x-ms-client-principal-id`, содержащие имя пользователя и идентификатор пользователя, прошедшего проверку подлинности, соответственно.

Эти заголовки можно использовать в конфигурации привязки *сигналрконнектионинфо* для создания подключений, прошедших проверку подлинности. Ниже приведен пример C# функции Negotiate, которая использует заголовок `x-ms-client-principal-id`.
...
Затем можно отправить сообщения этому пользователю, задав свойство `UserId` сообщения SignalR.
