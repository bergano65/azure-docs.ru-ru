...
## <a name="why-do-i-get-an-error-catalog-already-exists-when-i-try-to-create-a-new-catalog"></a>Почему я получаю каталог ошибок *уже существует,* когда я пытаюсь создать новый каталог?

При покупке Office 365 E5 с лицензией Power BI Pro корпорация Майкрософт автоматически создает каталог по умолчанию в регионе подписки. Этот каталог использует бесплатный SKU. Лицензия пользователя Office 365 / Power BI управляется на странице администрации Office 365. 

Однако этот тип каталога данных не имеет **опции администратора** и не отображается на **портале Azure.** Вы не можете удалить этот тип каталога данных. Аналогичным образом, не разрешается переименовывать каталог данных, и вы не можете переместить его в другой регион. 

Учетные записи пользователей, которым присваивается лицензия Power BI Pro, имеют доступ к каталогу данных в связи с лицензионным соглашением, когда они подписались на Office 365 E5 с лицензией Power BI Pro. Этот тип пользователя имеет полный доступ к ресурсам каталога данных без административных привилегий. Такой пользователь *не* является частью роли **пользователя каталога** в каталоге данных Azure.
...
> [!NOTE]
> Средство регистрации источника данных в каталоге данных извлекает значение свойства первого класса **Description**, которое есть у таких источников данных, как службы SQL Server Analysis Services. Для реляционных баз данных *s-L* Server, в ней нет первоклассного свойства **Описания,** инструмент регистрации источника данных Data Catalog извлекает значение из **ms_description** расширенного свойства для объектов и столбцов. Это свойство не поддерживается для S'L Azure. Дополнительные сведения см. в статье [Использование расширенных свойств объектов базы данных](https://technet.microsoft.com/library/ms190243%28v=sql.105%29.aspx).
>
>
