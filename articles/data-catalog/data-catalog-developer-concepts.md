...
**Каталога данных Azure** концептуальная модель основана на четыре основных понятия: **Каталога**, **пользователей**, **активы**, и **заметки**.
...
<table><tr><td><b>Имя свойства</b></td><td><b>Тип данных</b></td><td><b>Комментарии</b></td></tr><tr><td> timestamp</td><td>DateTime</td><td>Время последнего изменения элемента. Это поле создается сервером при вставке и каждом изменении элемента. Значение этого свойства игнорируется во входных данных операции публикации.</td></tr><tr><td>идентификатор</td><td>URI</td><td>Абсолютный URL-адрес элемента (только для чтения). Это уникальный адресуемый URI для элемента.  Значение этого свойства игнорируется во входных данных операции публикации.</td></tr><tr><td>тип</td><td>String</td><td>Тип ресурса (только для чтения).</td></tr><tr><td>ETag</td><td>String</td><td>Строка, соответствующая версии элемента, которую можно использовать для оптимистического управления параллелизмом при выполнении операций, изменяющих элементы в каталоге. Можно указать "*" для соответствия любому значению.</td></tr></table>
...
<tr><td>fromSourceSystem</td><td>Boolean</td><td>Указывает, получены ли данные из исходной системы (например базы данных SQL Server или Oracle) или созданы пользователем.</td></tr>
</table>
...
<table>
<tr><td><b>Имя свойства</b></td><td><b>Тип данных</b></td><td><b>Комментарии</b></td></tr>
<tr><td>key</td><td>String</td><td>Указанный пользователем ключ, однозначно определяющий аннотацию в текущей коллекции. Длина ключа не может превышать 256 символов.</td></tr>
</table>
...
<table><tr><td><b>Тип ресурса (имя представления)</b></td><td><b>Дополнительные свойства</b></td><td><b>Тип данных</b></td><td><b>Разрешенные заметки</b></td><td><b>Комментарии</b></td></tr><tr><td>Таблица ("tables")</td><td></td><td></td><td>Описание<p>FriendlyName<p>Тег<p>Схема<p>ColumnDescription<p>ColumnTag<p> Эксперт<p>Предварительная версия<p>AccessInstruction<p>TableDataProfile<p>ColumnDataProfile<p>ColumnDataClassification<p>Документация<p></td><td>Таблица представляет любые табличные данные.  Пример. Таблица SQL, представление SQL, табличных таблица служб Analysis Services, Analysis Services — многомерные измерения, таблица Oracle и т. д.   </td></tr><tr><td>Мера ("measures")</td><td></td><td></td><td>Описание<p>FriendlyName<p>Тег<p>Эксперт<p>AccessInstruction<p>Документация<p></td><td>Этот тип представляет измерение Analysis Services.</td></tr><tr><td></td><td>measure</td><td>столбец</td><td></td><td>Метаданные, описывающие измерение</td></tr><tr><td></td><td>isCalculated </td><td>Boolean</td><td></td><td>Указывает, вычисляется измерение или нет.</td></tr><tr><td></td><td>measureGroup</td><td>String</td><td></td><td>Физический контейнер для измерения</td></tr><td>Ключевой показатель эффективности ("kpis")</td><td></td><td></td><td>Описание<p>FriendlyName<p>Тег<p>Эксперт<p>AccessInstruction<p>Документация</td><td></td></tr><tr><td></td><td>measureGroup</td><td>String</td><td></td><td>Физический контейнер для измерения</td></tr><tr><td></td><td>goalExpression</td><td>String</td><td></td><td>Числовое выражение MDX или вычисление, которое возвращает целевое значение ключевого показателя эффективности.</td></tr><tr><td></td><td>valueExpression</td><td>String</td><td></td><td>Числовое выражение MDX, которое возвращает фактическое значение ключевого показателя эффективности.</td></tr><tr><td></td><td>statusExpression</td><td>String</td><td></td><td>Выражение MDX, которое отражает состояние ключевого показателя эффективности в заданный момент времени.</td></tr><tr><td></td><td>trendExpression</td><td>String</td><td></td><td>Выражение MDX, которое оценивает значение ключевого индикатора производительности во времени. В качестве тренда может использоваться любой временной критерий, который удобно использовать в заданном бизнес-контексте.</td>
<tr><td>Отчет ("reports")</td><td></td><td></td><td>Описание<p>FriendlyName<p>Тег<p>Эксперт<p>AccessInstruction<p>Документация<p></td><td>Этот тип представляет отчет для служб отчетов SQL Server </td></tr><tr><td></td><td>assetCreatedDate</td><td>String</td><td></td><td></td></tr><tr><td></td><td>assetCreatedBy</td><td>String</td><td></td><td></td></tr><tr><td></td><td>assetModifiedDate</td><td>String</td><td></td><td></td></tr><tr><td></td><td>assetModifiedBy</td><td>String</td><td></td><td></td></tr><tr><td>Контейнер ("containers")</td><td></td><td></td><td>Описание<p>FriendlyName<p>Тег<p>Эксперт<p>AccessInstruction<p>Документация<p></td><td>Этот тип представляет контейнер других ресурсов, таких как база данных SQL, контейнер больших двоичных объектов Azure или модель служб Analysis Services.</td></tr></table>
...
<tr><td></td><td>тег</td><td>string</td><td>Тег, описывающий ресурс.</td></tr>
...
<tr><td></td><td>columnName</td><td>String</td><td>Имя столбца, на который ссылается это описание.</td></tr>
<tr><td></td><td>description</td><td>String</td><td>Краткое описание столбца (2-3 строки).</td></tr>
...
<tr><td></td><td>columnName</td><td>String</td><td>Имя столбца, на который ссылается этот тег.</td></tr>
<tr><td></td><td>тег</td><td>String</td><td>Тег, описывающий столбец.</td></tr>
...
<tr><td></td><td>размер</td><td>длинное целое число</td><td>Размер набора данных в байтах.  </td></tr>
...
<tr><td></td><td>columnName</td><td>String</td><td>Имя столбца, на который ссылается эта классификация.</td></tr>
<tr><td></td><td>классификация;</td><td>String</td><td>Классификация данных в этом столбце.</td></tr>
...
<tr><td></td><td>sourceType</td><td>string</td><td>Описывает тип источника данных.  Пример. SQL Server, базы данных Oracle, и т.д.  </td></tr>
<tr><td></td><td>objectType</td><td>string</td><td>Описывает тип объекта в источнике данных. Пример. Таблица, представление для SQL Server.</td></tr>

<tr><td>DataSourceLocation</td><td></td><td></td><td></td></tr>
<tr><td></td><td>протокол</td><td>string</td><td>Обязательный. Описывает протокол, используемый для связи с источником данных. Например, tds для SQl Server, oracle для Oracle и т. д. Список поддерживаемых протоколов см. в <a href="https://docs.microsoft.com/azure/data-catalog/data-catalog-dsr">спецификации ссылки на источник данных в разделе структуры DSL</a>.</td></tr>
<tr><td></td><td>адрес</td><td>Dictionary<string, object></td><td>Обязательный. Адрес — это набор данных, относящийся к протоколу. Он используется для определения источника данных, указанного в ссылке. Так как данные адреса относятся к определенному протоколу, это значит, что знать их нет смысла, не зная протокол.</td></tr>
<tr><td></td><td>проверка подлинности</td><td>string</td><td>Необязательный элемент. Схема проверки подлинности, используемая для связи с источником данных. Например, Windows, OAuth и т. д.</td></tr>
<tr><td></td><td>connectionProperties</td><td>Dictionary<string, object></td><td>Необязательный элемент. Дополнительные сведения о том, как подключиться к источнику данных.</td></tr>
...
<tr><td></td><td>firstName</td><td>string</td><td>Имя пользователя (для отображения). Необязательный элемент. Допустим только в контексте свойства "lastRegisteredBy". Не может быть указан при предоставлении субъекта безопасности для "roles", "permissions" и "experts".</td></tr>
<tr><td></td><td>lastName</td><td>string</td><td>Фамилия пользователя (для отображения). Необязательный элемент. Допустим только в контексте свойства "lastRegisteredBy". Не может быть указан при предоставлении субъекта безопасности для "roles", "permissions" и "experts".</td></tr>

<tr><td>столбец</td><td></td><td></td><td></td></tr>
<tr><td></td><td>name</td><td>string</td><td>Имя столбца или атрибута.</td></tr>
<tr><td></td><td>тип</td><td>string</td><td>тип данных столбца или атрибута. Допустимые для использования типы зависят от типа источника данных ресурса.  Поддерживается только подмножество типов.</td></tr>
...
<tr><td></td><td>precision</td><td>байт</td><td>Точность для столбца или атрибута. Определяется по информации из источника данных. Применимо только к некоторым типам источников данных.</td></tr>
<tr><td></td><td>isNullable</td><td>Boolean</td><td>Указывает, разрешены ли значения null для этого столбца. Определяется по информации из источника данных. Применимо только к некоторым типам источников данных.</td></tr>
...
<tr><td></td><td>тип </td><td>string</td><td>Тип столбца</td></tr>
<tr><td></td><td>min </td><td>string</td><td>Минимальное значение в наборе данных</td></tr>
<tr><td></td><td>максимум </td><td>string</td><td>Максимальное значение в наборе данных</td></tr>
...
<tr><td></td><td>тип</td><td>string</td><td>Тип свойства. Поддерживаемые значения: "bool", boolean", "byte", "guid", "int", "integer", "long", "string", "url".</td></tr>
...
### <a name="roles"></a>Роли
Существуют три роли: **Администратор**, **владельца**, и **участник**.  Каждая роль имеет собственную область действия и права, указанные в следующей таблице.
...
**Текст**
  ...
  **Назначение владельцев и ограничение их видимости для элемента существующей корневым**: **PUT** https:\//api.azuredatacatalog.com/catalogs/default/views/tables/042297b0...1be45ecd462a?api-version=2016-03-30
...
> [!NOTE]
> В PUT он не должен указывать полезные в тексте: PUT можно использовать для обновления только роли и разрешения.
> 
> 
