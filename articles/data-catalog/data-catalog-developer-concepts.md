...
![Концептуальная иллюстрация каталога данных Azure](./media/data-catalog-developer-concepts/concept2.png)
...
Примерами аннотаций являются описание, теги, схема, документация и т.д. Полный список типов активов и типов аннотаций в разделе модели объектов активов.
...
<table><tr><td><b>Имя свойства</b></td><td><b>Тип данных</b></td><td><b>Комментарии</b></td></tr><tr><td>TIMESTAMP</td><td>Дата и время</td><td>Время последнего изменения элемента. Это поле создается сервером при вставке и каждом изменении элемента. Значение этого свойства игнорируется во входных данных операции публикации.</td></tr><tr><td>идентификатор</td><td>URI</td><td>Абсолютный URL-адрес элемента (только для чтения). Это уникальный адресуемый URI для элемента.  Значение этого свойства игнорируется во входных данных операции публикации.</td></tr><tr><td>type</td><td>Строка</td><td>Тип ресурса (только для чтения).</td></tr><tr><td>etag</td><td>Строка</td><td>Строка, соответствующая версии элемента, которую можно использовать для оптимистического управления параллелизмом при выполнении операций, изменяющих элементы в каталоге. Можно указать "*" для соответствия любому значению.</td></tr></table>
...
<tr><td></td><td>tag</td><td>строка</td><td>Тег, описывающий ресурс.</td></tr>
...
<tr><td>ColumnTag ("columnTags")</td><td></td><td></td><td>Это свойство содержит тег для столбца. Каждый пользователь системы может добавить несколько тегов для заданного столбца, а также теги для нескольких столбцов. Изменить объекты ColumnTag может только создавший их пользователь. (Администраторы и владельцы ресурса могут удалить объект ColumnTag, но не изменить его.) Система хранит эти теги столбцов пользователей отдельно.  Таким образом, для каждого ресурса существует массив объектов ColumnTag.  ColumnTag слабо привязан к схеме, чтобы он мог выйти из синхронизации. ColumnTag может описать столбец, который больше не существует в схеме.  За синхронизацию тегов столбцов отвечает тот, кто производит запись.</td></tr>
...
<tr><td></td><td>content</td><td>строка</td><td>Инструкции для получения доступа к этому ресурсу-контейнеру данных. Это может быть URL-адрес, адрес электронной почты или набор инструкций.</td></tr>
...
<tr><td></td><td>mimeType</td><td>строка</td><td>Тип mime содержимого.</td></tr>
<tr><td></td><td>content</td><td>строка</td><td>Содержимое документации</td></tr>
...
<tr><td></td><td>protocol</td><td>строка</td><td>Обязательный элемент. Описывает протокол, используемый для связи с источником данных. Например: "tds" для сервера S'l, "оракул" для Oracle и т.д. Обратитесь к <a href="https://docs.microsoft.com/azure/data-catalog/data-catalog-dsr">спецификации ссылки на источник данных - DSL Структура</a> для списка поддерживаемых в настоящее время протоколов.</td></tr>
...
<table><tr><td><b>Role</b></td><td><b>Область</b></td><td><b>Права</b></td></tr><tr><td>Администратор</td><td>Каталог (все ресурсы и аннотации в составе каталога)</td><td>Read, Delete, ViewRoles.
...
```
  ...
  **Назначить владельцев и ограничить видимость для существующего элемента корня**: **PUT** https:\//api.azuredatacatalog.com/catalogs/default/views/tables/042297b0...1be45ecd462a?api-version=2016-03-30
