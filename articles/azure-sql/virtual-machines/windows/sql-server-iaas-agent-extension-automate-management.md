...
Расширение агента SQL Server IaaS работает с именованным экземпляром SQL Server если на виртуальной машине доступен единственный экземпляр SQL Server. Расширение не удается установить на виртуальных машинах с несколькими экземплярами SQL Server. 
...
Да. Расширение агента IaaS SQL будет регистрировать только один экземпляр SQL Server (ядро СУБД). Расширение агента IaaS SQL будет регистрировать экземпляр SQL Server по умолчанию в случае нескольких экземпляров. Если экземпляр по умолчанию отсутствует, то поддерживается только регистрация в упрощенном режиме. Чтобы выполнить обновление с упрощенного на полный режим управления, должен существовать либо экземпляр SQL Server по умолчанию, либо виртуальная машина должна иметь только один именованный экземпляр SQL Server.
