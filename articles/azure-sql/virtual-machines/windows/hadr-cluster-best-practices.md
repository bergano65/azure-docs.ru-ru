...
При запуске узлов отказоустойчивого кластера Windows на виртуальных машинах Azure с SQL Server AlwaysOn рекомендуется изменить параметр кластера на более неявное состояние мониторинга.  Это сделает кластер более стабильным и надежным.  Дополнительные сведения см. в разделе [IaaS with SQL AlwaysOn. пороговые значения сети отказоустойчивого кластера](/windows-server/troubleshoot/iaas-sql-failover-cluster).
