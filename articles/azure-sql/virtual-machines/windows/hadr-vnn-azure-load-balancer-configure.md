---
title: Настройка VNN с помощью Azure Load Balancer для HADR
description: Узнайте, как настроить балансировщик нагрузки Azure для маршрутизации трафика к имени виртуальной сети (VNN) для группы доступности или экземпляра отказоустойчивого кластера (FCI) с SQL Server на виртуальных машинах Azure для обеспечения высокой доступности и аварийного восстановления (HADR).
...
В этой статье описывается, как настроить подсистему балансировки нагрузки с помощью службы Azure Load Balancer. Балансировщик нагрузки будет маршрутизировать трафик в [прослушиватель группы доступности](availability-group-overview.md) или [экземпляры отказоустойчивого кластера (fci)](failover-cluster-instance-overview.md) с SQL Server на виртуальных машинах Azure для обеспечения высокой доступности и аварийного восстановления (HADR). 
...
- Установлена последняя версия [PowerShell](/powershell/azure/install-az-ps?view=azps-4.2.0). 
...
Чтобы проверить подключение, войдите на другую виртуальную машину в той же виртуальной сети. Откройте **SQL Server Management Studio** и подключитесь к имени SQL Server FCI или к прослушивателю группы доступности.
