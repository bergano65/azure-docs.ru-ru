...
5. В диалоговом окне **Создание подсистемы балансировки нагрузки** настройте подсистему балансировки нагрузки, задав следующие параметры:
   ...
   | **Частный IP-адрес** |Укажите свободный IP-адрес из нужной подсети. Используйте этот IP-адрес при создании прослушивателя в кластере. Далее вам понадобится использовать этот адрес для переменной `$ListenerILBIP` в скрипте PowerShell. |
...
> [!NOTE]
> Указанный порт должен быть открыт в брандмауэре обоих экземпляров SQL Server. Для обоих экземпляров требуется правило для входящего трафика используемого TCP-порта. Дополнительные сведения см. в статье [Добавление и изменение правила брандмауэра](/previous-versions/orphan-topics/ws.11/cc753558(v=ws.11)). 
> 
...
>[!IMPORTANT]
>Этот шаг применяется, только если группа доступности включена в [распределенную группу доступности](/sql/database-engine/availability-groups/windows/configure-distributed-availability-groups). 
...
1. Создайте правило балансировки нагрузки со следующими параметрами:
   ...
   |**протокол**; |TCP
   |**порт**. |5022 — порт для [прослушивателя конечной точки распределенной группы доступности](/sql/database-engine/availability-groups/windows/configure-distributed-availability-groups).</br> Это может быть любой доступный порт.  
