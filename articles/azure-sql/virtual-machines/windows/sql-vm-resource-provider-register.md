...
description: Регистрация виртуальной машины Azure SQL Server с помощью поставщика ресурсов виртуальной машины SQL для включения функций виртуальных машин SQL Server, развернутых за пределами Azure Marketplace, а также для обеспечения соответствия и улучшения управляемости.
...
ms.locfileid: "84039835"
---
# <a name="register-a-sql-server-virtual-machine-in-azure-with-the-sql-vm-resource-provider"></a>Регистрация виртуальной машины SQL Server в Azure с помощью поставщика ресурсов виртуальных машин SQL
[!INCLUDE[appliesto-sqlvm](../../includes/appliesto-sqlvm.md)]

Эта статья содержит сведения о регистрации виртуальной машины SQL Server в Azure с помощью поставщика ресурсов виртуальных машин SQL. При регистрации с помощью поставщика ресурсов в рамках подписки создается отдельный _ресурс_ **виртуальной машины SQL**. При отмене регистрации виртуальной машины SQL Server в поставщике ресурсов будет удален _ресурс_ **виртуальной машины SQL**, но не будет удалена фактическая виртуальная машина. 
...
- **Бесплатное управление.**  Регистрация с помощью поставщика ресурсов виртуальной машины SQL во всех трех режимах управляемости полностью бесплатная. За использование поставщика ресурсов или за изменение режимов управления дополнительная плата не взимается. 
...
Чтобы использовать поставщик ресурсов виртуальной машины SQL, необходимо сначала [зарегистрировать подписку с помощью поставщика ресурсов](#register-subscription-with-rp). После этого поставщик ресурсов будет иметь возможность создавать ресурсы в пределах этой конкретной подписки.
...
Экземпляры отказоустойчивого кластера и развертывания с несколькими экземплярами можно зарегистрировать только с помощью поставщика ресурсов виртуальной машины SQL в упрощенном режиме. 

# <a name="az-cli"></a>[Azure CLI](#tab/bash)

Зарегистрируйте виртуальную машину SQL Server в упрощенном режиме с помощью Azure CLI: 
...
Зарегистрируйте виртуальную машину SQL Server в упрощенном режиме с помощью PowerShell:  
...
Если расширение IaaS для SQL уже установлено на виртуальной машине вручную, можно зарегистрировать виртуальную машину SQL Server в полном режиме без перезагрузки службы SQL Server. **Однако если расширение IaaS для SQL не установлено, при регистрации в полном режиме будет установлено расширение IaaS для SQL в полном режиме и перезагружена служба SQL Server. Выполняйте дальнейшие действия с осторожностью.**
...
Зарегистрируйте виртуальную машину SQL Server 2008 в режиме NoAgent с помощью Azure CLI: 
...
Зарегистрируйте виртуальную машину SQL Server 2008 R2 в режиме NoAgent с помощью Azure CLI: 
...
Зарегистрируйте виртуальную машину SQL Server 2008 в режиме NoAgent с помощью PowerShell: 
  ...
  Зарегистрируйте виртуальную машину SQL Server 2008 R2 в режиме NoAgent с помощью PowerShell: 
...
## <a name="upgrade-to-full-management-mode"></a>Обновление до режима полного управления 

Виртуальные машины SQL Server с установленным расширением IaaS в *упрощенном* режиме могут обновить режим до _полного_ с помощью портала Azure, Azure CLI или PowerShell. Виртуальные машины SQL Server в режиме _NoAgent_ можно обновить до _полного_ режима после обновления операционной системы до Windows 2008 R2 или выше. Переход на использование более ранней версии невозможен. Чтобы сделать это, необходимо [отменить регистрацию](#unregister-vm-from-rp) виртуальной машины SQL Server в поставщике ресурсов виртуальной машины SQL. При этом будет удален _ресурс_ **виртуальной машины SQL**, но не фактическая виртуальная машина. 
...
1. Выберите виртуальную машину SQL Server и щелкните **Обзор**. 
...
1. Войдите на [портал Azure](https://portal.azure.com). 
1. Перейдите к [виртуальным машинам SQL Server](manage-sql-vm-portal.md).
...
## <a name="unregister-vm-from-rp"></a>Отмена регистрации виртуальной машины в поставщике ресурсов

Чтобы отменить регистрацию виртуальной машины SQL Server с помощью поставщика ресурсов виртуальной машины SQL, удалите *ресурс* виртуальной машины SQL, используя портал Azure или Azure CLI. При удалении *ресурса* виртуальной машины SQL виртуальная машина SQL Server не удаляется. Тем не менее будьте внимательны и осторожны при выполнении этих действий, так как при попытке удалить *ресурс* можно случайно удалить виртуальную машину. 

Отмена регистрации виртуальной машины SQL с помощью поставщика ресурсов виртуальной машины SQL необходима, чтобы перейти с полного режима управления на другой. 
...
1. Введите имя виртуальной машины SQL и **снимите флажок рядом с виртуальной машиной**.
...
# <a name="azure-cli"></a>[Azure CLI](#tab/azure-cli)
Чтобы отменить регистрацию виртуальной машины SQL Server в поставщике ресурсов с помощью Azure CLI, выполните команду [az sql vm delete](/cli/azure/sql/vm?view=azure-cli-latest#az-sql-vm-delete). Это удалит *ресурс* виртуальной машины SQL Server, но не саму виртуальную машину. 
...
# <a name="powershell"></a>[PowerShell](#tab/azure-powershell)
Чтобы отменить регистрацию виртуальной машины SQL Server в поставщике ресурсов с помощью Azure CLI, выполните команду [New-AzSqlVM](/powershell/module/az.sqlvirtualmachine/new-azsqlvm). Это удалит *ресурс* виртуальной машины SQL Server, но не саму виртуальную машину. 
...
Регистрация с помощью поставщика ресурсов виртуальной машины SQL требуется, если виртуальная машина SQL Server не была подготовлена из образов SQL Server в Azure Marketplace, а установлена самостоятельно, даже если установлено расширение IaaS для SQL Server. При регистрации с помощью поставщика ресурсов виртуальной машины SQL создается ресурс типа Microsoft.SqlVirtualMachines. При установке расширения IaaS для SQL Server этот ресурс не создается.
...
**Будет ли перезапущен SQL Server на моей виртуальной машине при регистрации с помощью поставщика ресурсов виртуальной машины SQL Server?**
...
Чтобы изменить режим полного управления, [отмените регистрацию](#unregister-vm-from-rp) виртуальной машины SQL Server с помощью поставщика ресурсов SQL Server, удалив *ресурс* SQL Server, и повторно зарегистрируйте виртуальную машину SQL Server с помощью поставщика ресурсов виртуальной машины SQL в другом режиме управления.
