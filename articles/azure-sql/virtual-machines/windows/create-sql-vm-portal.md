---
title: Подготовка виртуальной машины на портале Azure
...
В этом руководстве рассматриваются различные варианты, доступные при создании виртуальной машины SQL Server на платформе Windows на портале Azure. В этой статье рассматривается больше параметров конфигурации, чем в [кратком руководстве по виртуальным машинам SQL Server](sql-vm-create-portal-quickstart.md), и описывается одна возможная задача подготовки. 
...
1. На портале Azure в меню слева выберите **Azure SQL**. Если **Azure SQL** отсутствует в списке, выберите **Все службы** и в поле поиска введите Azure SQL. (Необязательно) Щелкните звезду рядом с **Azure SQL**, чтобы добавить этот элемент в избранное и область навигации слева. 
...
1. Введите `2017` в поле поиска образа SQL Server на плитке **Виртуальные машины SQL**, а затем в раскрывающемся списке выберите **Free SQL Server License: SQL Server 2017 Developer on Windows Server 2016** (Бесплатная лицензия на SQL Server: SQL Server 2017 Developer на базе Windows Server 2016). 


   ![Выбор образа виртуальной машины SQL](./media/create-sql-vm-portal/select-sql-vm-image-portal.png)

   > [!TIP]
   > В этом пошаговом руководстве используется выпуск Developer, так как это полнофункциональный выпуск SQL Server, предоставляемый бесплатно для тестирования в процессе разработки. Вы оплачиваете только стоимость выполнения виртуальной машины. Но вы можете выбрать для использования в этом пошаговом руководстве любой образ. Описание доступных образов см. в разделе [Начало работы с виртуальными машинами SQL](sql-server-on-azure-vm-iaas-what-is-overview.md#payasyougo).
...
* Создайте **виртуальную сеть** или используйте существующую для виртуальной машины SQL Server. Также выберите значение для параметра **Подсеть**. 

* В качестве значения параметра **Сетевая группа безопасности** выберите основную группу безопасности или расширенную группу безопасности. Если выбрана базовая группа, можно выбрать входящие порты для виртуальной машины SQL Server (те же значения, которые были настроены на вкладке **Основы**). Если выбрана расширенная группа, можно выбрать существующую группу безопасности сети или создать новую. 
...
На вкладке **Мониторинг** настройте параметры мониторинга и автоматического завершения работы. 

* В Azure по умолчанию включен параметр **Диагностика загрузки** в учетной записи хранения, которая будет использоваться для виртуальной машины. Эти параметры можно изменить, а также включить параметр **Диагностика гостевой ОС**. 
* На этой вкладке можно также включить **управляемое удостоверение, назначенное системой** и **автоматическое завершение работы**. 
...
Если требуется проверка подлинности SQL Server, на вкладке **Параметры SQL Server** выберите для параметра **Проверка подлинности SQL** значение **Включить**.
...
Если вы являетесь клиентом, участвующим в программе Software Assurance, вы можете воспользоваться [Преимуществом гибридного использования Azure](https://azure.microsoft.com/pricing/hybrid-benefit/), чтобы применить собственную лицензию SQL Server и сэкономить на ресурсах. 
...
Для шифрования резервной копии щелкните **Включить**. Затем введите **пароль**. Azure создает сертификат для шифрования резервных копий и использует указанный пароль для защиты этого сертификата. По умолчанию расписание задается автоматически, но можно создать график резервного копирования вручную, выбрав **Вручную**. 
