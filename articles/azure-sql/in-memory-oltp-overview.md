...
В этой статье описываются аспекты выполняющейся в памяти OLTP и индексов columnstore, которые относятся к базе данных SQL Azure и Управляемый экземпляр Azure SQL, а также включает примеры:
...
- Вы увидите два примера, иллюстрирующих использование выполняющейся в памяти OLTP, а также индексы columnstore.
...
- [Инструкции по индексам columnstore](/sql/relational-databases/indexes/columnstore-indexes-overview)
...
Технология OLTP в памяти предоставляет чрезвычайно быстрые операции доступа к данным, сохраняя все данные в памяти. Она также использует специализированные индексы, собственную компиляцию запросов и доступ к данным без защелок доступа для повышения производительности рабочей нагрузки OLTP. Существует два способа организации данных OLTP в памяти:
...
Краткое руководство по выполняющейся в памяти OLTP: быстрый запуск [1. технологии OLTP в памяти для повышения производительности T-SQL](/sql/relational-databases/in-memory-oltp/survey-of-initial-areas-in-in-memory-oltp) (другая статья поможет вам приступить к работе)
...
Дополнительные сведения о мониторинге использования хранилища OLTP в памяти и настройке оповещений при почти достижении ограничения см. [в разделе мониторинг хранилища в памяти](in-memory-oltp-monitor-space.md).
...
> [!Important]
> OLTP в памяти не поддерживается базами данных класса "Стандартный" или "Базовый". Таким образом, невозможно переместить базу данных, которая содержит объекты OLTP в памяти, на один из этих уровней.
...
*Масштабирование ресурсов на критически важный для бизнеса уровне*: данные в таблицах, оптимизированных для памяти, должны помещаться в хранилище выполняющейся в памяти OLTP, связанное с уровнем базы данных или управляемого экземпляра, или же он доступен в пуле эластичных БД. Если вы попытаетесь понизить уровень базы данных или переместить базу данных в пул, в котором нет достаточного объема хранилища выполняющейся в памяти OLTP, операция завершится ошибкой.
...
- [Использование выполняющейся в памяти OLTP в существующем приложении SQL Azure](in-memory-oltp-configure.md)
