...
В этом руководстве описан полный сценарий аварийного восстановления для мультитенантных приложений SaaS, использующих модель однотенантных баз данных. С помощью [геовосстановления](recovery-using-backups.md) вы восстановите базы данных каталогов и клиентов из автоматически сохраняемых геоизбыточных резервных копий в альтернативный регион восстановления. После устранения сбоя вы вернете измененные базы данных в исходный регион с помощью [георепликации](active-geo-replication-overview.md).

![На схеме показаны исходный и резервный регионы, в которых есть приложения, каталоги, исходные или зеркальные образы серверов и пулов, автоматическое резервное копирование в хранилище, а также регион восстановления, принимающий георепликацию резервной копии и имеющего сервер и пул для новых клиентов.](./media/saas-dbpertenant-dr-geo-restore/geo-restore-architecture.png)
...
* [Клиентская библиотека эластичной базы данных](elastic-database-client-library.md) (EDCL) — создание и обслуживание каталога клиентской базы данных. Расширенный каталог содержит периодически обновляемые сведения о конфигурации пула и базы данных.
* [Функции восстановления управления сегментами](elastic-database-recovery-manager.md) EDCL — для сохранения записей расположения базы данных в каталоге во время восстановления и возвращения в исходное расположение.  
* [Геовосстановление](../../key-vault/general/disaster-recovery-guidance.md) — восстановление баз данных каталогов и клиентов из автоматически сохраняемых геоизбыточных резервных копий. 
...
* [Георепликация](active-geo-replication-overview.md) — репатриация баз данных в исходную область после сбоя. Георепликация гарантирует отсутствие потерь данных и минимальное влияние на клиента.
* [DNS-псевдонимы сервера SQL](../../sql-database/dns-alias-overview.md) позволяют процессу синхронизации каталога подключаться к активному каталогу независимо от его расположения.  
...
3. На [портале Azure](https://portal.azure.com) найдите и откройте группу ресурсов, в которой было развернуто приложение.
   ...
   Обратите внимание на ресурсы и регион, в котором развертываются компоненты службы приложений и база данных SQL.
...
10. Отслеживает службу, чтобы определить, когда восстанавливаются базы данных. После восстановления базы данных клиента она помечается в каталоге как подключенная и регистрируется сумма rowversion для этой базы данных клиента. 
...
> * Восстановление баз данных в регион восстановления с помощью геовосстановления.
...
Чтобы узнать, как георепликация позволяет значительно сократить время восстановления для крупномасштабных многопользовательских приложений, изучите руководство [Аварийное восстановление мультитенантных приложений SaaS с использованием георепликации базы данных](../../sql-database/saas-dbpertenant-dr-geo-replication.md).
