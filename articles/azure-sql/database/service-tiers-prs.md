---
title: Премиум RS выбытие уровня службы
description: Премиум RS уровень служб для базы данных SQL Azure выводится из эксплуатации, а поддержка для нее завершается. см. раздел варианты миграции.
...
- Уровни служб виртуальных ядер
  ...
  Уровни служб **Общее назначение** и **Критически важный для бизнеса** в [модели приобретения на основе виртуальных ядер](service-tiers-vcore.md). Эти два уровня служб являются общедоступными. Модель приобретения на основе виртуальное ядро также предлагает уровень службы " **горизонтальное масштабирование** ", который адаптируется по запросу к потребностям вашей рабочей нагрузки с автоматическим масштабированием до 100 ТБ на базу данных. Уровень служб "Гипермасштабирование" обеспечивает производительность ввода-вывода, сопоставимую с уровнем обслуживания "Премиум" в [модели приобретения на основе единиц DTU](service-tiers-dtu.md), по цене, более близкой к уровню обслуживания уровня "Премиум RS".
...
- Преимущество гибридного использования Azure и цены на резервную мощность
  ...
  [Преимущество гибридного использования Azure и цены на резервную мощность](https://azure.microsoft.com/pricing/details/sql-database/) обеспечивает экономию до 80 % в отличие от тарифов в лицензии. Дополнительные сведения об этих параметрах см. в разделе [Преимущество гибридного использования Azure для SQL Server](https://azure.microsoft.com/pricing/hybrid-benefit/) и [Резервная мощность Базы данных SQL Azure](reserved-capacity-overview.md).
...
|**Если вы в данный момент...**|**Перенос на основе виртуальных ядер для сравнимых...**|**Перенос на основе единиц передачи данных для сравнимых...**|
|---|---|---|
|Уровень "Премиум RS 1"| Виртуальное ядро (го поколения) или критически важный для бизнеса 1 Виртуальное ядро (го поколения)|Уровень "Премиум 1"|
|Уровень "Премиум RS 2"| Виртуальных ядер (го поколения) и критически важный для бизнеса 2 виртуальных ядер (го поколения|Уровень "Премиум 2"|
|Уровень "Премиум RS 4"| Масштабирование 4 виртуальных ядер (го поколения) или критически важный для бизнеса 4 виртуальных ядер (го поколения)|Уровень "Премиум 4"
|Уровень "Премиум RS 6"| Виртуальных ядер (го поколения) и критически важный для бизнеса 6 виртуальных ядер (го поколения).|Уровень "Премиум 6"|
...
Наши уровни служб в рамках модели приобретения на основе виртуальных ядер, предусматривают специальные предложения, которые помогут вам сэкономить до 80 % от цены, включенной в лицензию. Используйте лицензии SQL Server Standard или Enterprise Edition с активной программой Software Assurance, чтобы сэкономить до 55% по сравнению с лицензиями, поставляемыми с [Преимущество гибридного использования Azure](https://azure.microsoft.com/pricing/hybrid-benefit/) SQL Server. Вы можете объединить Преимущество гибридного использования с ценой на [резервную мощность Базы данных SQL Azure](reserved-capacity-overview.md) и сэкономить до 80 % при внесении предоплаты на один или три года.  Активируйте преимущества для этих вариантов на портале Azure уже сегодня.
...
- Дополнительные сведения о конкретных объемах вычислительных ресурсов и доступных размерах хранилища для отдельной базы данных см. в статье [Ограничения ресурсов для отдельной базы данных в Базе данных SQL Azure при использовании модели приобретения на основе виртуальных ядер](resource-limits-vcore-single-databases.md)
- Дополнительные сведения о конкретных объемах вычислительных ресурсов и доступных размерах хранилища для эластичных пулов см. в статье [Ограничения для эластичных пулов в службе "База данных SQL Azure" в модели приобретения на основе виртуальных ядер](resource-limits-vcore-elastic-pools.md).
