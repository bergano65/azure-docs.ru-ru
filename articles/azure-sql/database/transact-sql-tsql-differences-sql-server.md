...
- Связанные с подключением: инструкции ENDPOINT. База данных SQL не поддерживает проверку подлинности Windows, но поддерживает такую же проверку подлинности Azure Active Directory. Для некоторых типов аутентификации требуется последняя версия SSMS. Дополнительные сведения см. в статье [Подключение к базе данных SQL или хранилищу данных SQL Azure с помощью Azure Active Directory проверки подлинности](authentication-aad-overview.md).
