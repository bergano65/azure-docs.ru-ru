...
Управляемый экземпляр SQL предоставляет возможность изменить избыточность хранилища на локально избыточный (LRS) или избыточный в пределах зоны (ZRS) BLOB-объекты хранилища, чтобы гарантировать, что данные остаются в том же регионе, где развернут управляемый экземпляр. Механизмы избыточности хранилища хранят несколько копий данных, чтобы защитить их от запланированных и незапланированных событий, включая временные сбои оборудования, сети или перебои электроэнергии, а также крупномасштабные стихийные бедствия. 

Возможность настройки избыточности хранилища резервных копий обеспечивает гибкость выбора между LRS, ZRS или RA-GRS Storage blobs для Управляемый экземпляр SQL. Настройка избыточности хранилища резервных копий во время процесса создания управляемого экземпляра после подготовки ресурса невозможно изменить избыточность хранилища. (Хранилище, избыточное в виде зоны (ZRS), в настоящее время доступно только в [определенных регионах](../../storage/common/storage-redundancy.md#zone-redundant-storage)).


> [!IMPORTANT]
> В SQL Управляемый экземпляр настроенная избыточность резервных копий применяется к параметрам краткосрочного хранения резервных копий, которые используются для восстановления на момент времени (PITR) и долгосрочного резервного копирования, используемого для долгосрочного резервного копирования (LTR).


> [!NOTE]
> Настраиваемая избыточность хранилища резервных копий базы данных SQL Azure в настоящее время доступна в виде ограниченного закрытого предварительного просмотра для некоторых клиентов в регионе Azure Юго-Восточной Азии. Если вы хотите учитывать регистрацию в этой закрытой предварительной версии, обратитесь по адресу [sqlbackuppreview@microsoft.com](mailto:sqlbackuppreview@microsoft.com) . 
...
- **Геовосстановление**  -  [Восстановление базы данных в другой географический регион](recovery-using-backups.md#geo-restore). Геовосстановление позволяет восстанавливаться после географической аварии, когда доступ к базе данных или резервным копиям в основном регионе невозможен. Она создает новую базу данных на любом существующем сервере или управляемом экземпляре в любом регионе Azure.
   > [!IMPORTANT]
   > Геовосстановление доступно только для управляемых экземпляров с настроенным геоизбыточным хранилищем резервных копий (RA-GRS).
...
> [!IMPORTANT]
> Настраиваемая избыточность хранилища для резервных копий в настоящее время доступна только для SQL Управляемый экземпляр и может быть указана только в процессе создания управляемого экземпляра. После подготовки ресурса нельзя изменить параметр избыточность хранилища резервных копий.
...
![Откройте окно "расчеты и Настройка хранилища" — колонка](./media/automated-backups-overview/open-configuration-blade-mi.png)

В колонке " **вычислить и хранилище** " найдите параметр избыточность хранилища резервных копий.
![Настройка избыточности хранилища резервных копий](./media/automated-backups-overview/select-backup-storage-redundancy-mi.png)
