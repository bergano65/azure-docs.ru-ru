...
- Параметры для настройки базы данных SQL Azure, Azure SQL Управляемый экземпляр и Azure синапсе Analytics (ранее — хранилище данных SQL Azure) позволяют пользователям выполнять административные задачи и получать доступ к данным, хранящимся в этих базах данных.
...
- **Учетная запись пользователя** — это отдельная учетная запись в любой базе данных, которая может быть, но не обязательно связана с именем входа. Если учетная запись пользователя не связана с именем входа, то учетные данные сохраняются с учетной записью пользователя.
