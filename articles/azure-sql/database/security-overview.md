...
База данных SQL, SQL Управляемый экземпляр и Azure синапсе принудительно применяют шифрование (SSL/TLS) для всех подключений. Это гарантирует, что все данные шифруются "при передаче" между клиентом и сервером независимо от значения параметра **Encrypt** или **TrustServerCertificate** в строке подключения.
