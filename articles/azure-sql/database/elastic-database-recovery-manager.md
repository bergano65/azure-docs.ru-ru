...
[Метод DetachShard](https://docs.microsoft.com/previous-versions/azure/dn842083(v=azure.100)) отсоединяет данный сегмент от сопоставления сегментов и удаляет сопоставления, связанные с ним.  
...
* Параметр *параметре shardmapname указано* — это имя сопоставления сегментов. Он необходим, только если один и тот же диспетчер сопоставления сегментов управляет несколькими сопоставлениями сегментов. Необязательный элемент.
...
* [Перечисление MappingDifferenceResolution](https://docs.microsoft.com/dotnet/api/microsoft.azure.sqldatabase.elasticscale.shardmanagement.recovery.mappingdifferenceresolution) используется для указания метода для устранения разницы между сопоставлениями сегментов.
