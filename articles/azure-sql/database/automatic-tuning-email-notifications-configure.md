...
Рекомендации по автоматической настройке базы данных SQL Azure можно просмотреть в [портал Azure](database-advisor-find-recommendations-portal.md), полученных с помощью [REST APIных](/rest/api/sql/databaserecommendedactions/listbydatabaseadvisor) вызовов или с помощью команд [T-SQL](https://azure.microsoft.com/blog/automatic-tuning-introduces-automatic-plan-correction-and-t-sql-management/) и [PowerShell](/powershell/module/az.sql/get-azsqldatabaserecommendedaction) . Эта статья основана на использовании сценария PowerShell для получения рекомендаций по автоматической настройке.
...
Сценарий PowerShell для получения рекомендаций по автоматической настройке использует команды [Get-азресаурце](/powershell/module/az.Resources/Get-azResource) и [Get-азсклдатабасерекоммендедактион](/powershell/module/az.Sql/Get-azSqlDatabaseRecommendedAction) , для которых требуется модуль Azure версии 4 и выше.
...
Чтобы узнать больше о возможностях Microsoft Flow, прочитайте статью [Начало работы с Microsoft Flow](/flow/getting-started).
...
2. Создание действия для получения выходных данных из выполненного скрипта PowerShell
   ...
   - Выберите "**+ новый шаг**", а затем "**Добавить действие**" в области "последовательность повторений".
   - В поле поиска введите "**Automation**" и выберите "Служба **автоматизации Azure — получить выходные данные задания**" в результатах поиска. Чтобы узнать больше о параметрах, доступных в этом разделе, изучите раздел [Служба автоматизации Azure](/connectors/azureautomation/#get-job-output).
