...
- Имена входа направляются шлюзом в конкретный экземпляр, где находится база данных.  В случае имен входа AAD учетные данные проверяются, прежде чем пытаться использовать этого пользователя для входа в запрошенную базу данных.  В случае сбоя запрашиваемая база данных никогда не получает доступа, поэтому аудит не выполняется.  В случае имен входа SQL эти учетные данные проверяются на запрошенных данных, поэтому в этом случае их можно проследить.  Успешные попытки входа, которые очевидно достигли базы данных, подлежат аудиту в обоих случаях.
