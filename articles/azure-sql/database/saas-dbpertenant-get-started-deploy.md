---
title: Руководство по SaaS для базы данных на клиента
...
ms.locfileid: "91356831"
---
# <a name="deploy-and-explore-a-multitenant-saas-app-that-uses-the-database-per-tenant-pattern-with-azure-sql-database"></a>Развертывание и изучение многоклиентского приложения SaaS, использующего шаблон базы данных на клиента с базой данных SQL Azure
...
1. Разверните приложения.
    ...
    а. Установите флажок, чтобы принять условия.
...
1. В интегрированной среде сценариев PowerShell откройте... \\ Обучающие модули \\ **UserConfig. PSM1**
...
Внутри приложения каждый клиент получает базу данных, развернутую в эластичном пуле.
...
В рабочей среде обычно создается запись CNAME DNS, которая [*указывает Интернет-домен компании*](../../traffic-manager/traffic-manager-point-internet-domain.md)   на DNS-имя диспетчера трафика.
...
1. В интегрированной среде сценариев PowerShell откройте... \\ Программы Learning modules \\ \\ *Demo-LoadGenerator.ps1* сценарии.
...
После успешной подготовки в браузере появится сайт *событий* нового клиента.
...
1. На [портале Azure](https://portal.azure.com) перейдите к списку серверов SQL. Откройте сервер **catalog-dpt-&lt;Пользователь&gt;**.
   ...
   ![На снимке экрана показана страница обзора сервера каталога с двумя базами данных.](./media/saas-dbpertenant-get-started-deploy/databases.png)
...
Затем воспользуйтесь [учебником по подготовке и каталогизации](saas-dbpertenant-provision-and-catalog.md).
