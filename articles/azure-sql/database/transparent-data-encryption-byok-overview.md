...
Используйте командлет Backup-AzKeyVaultKey, чтобы получить ключ в зашифрованном формате из хранилища первичного ключа, а затем используйте командлет Restore-AzKeyVaultKey и укажите хранилище ключей во втором регионе, чтобы клонировать ключ. Кроме того, можно использовать портал Azure для резервного копирования и восстановления ключа. Ключ в дополнительном хранилище ключей в другом регионе не должен быть помечен как предохранитель TDE, и он даже не разрешен.
