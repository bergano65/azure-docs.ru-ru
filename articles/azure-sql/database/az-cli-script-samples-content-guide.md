...
| [Создание эластичных пулов и перемещение баз данных в пуле](scripts/move-database-between-elastic-pools-cli.md) | Создает эластичные пулы и перемещает базы данных SQL в составе пулов, а также изменяет объемы вычислительных ресурсов. |
...
| [Настройка и выполнение отработки отказа для отдельной базы данных с помощью активной георепликации](../../sql-database/scripts/sql-database-setup-geodr-and-failover-database-cli.md)| Настраивает активную георепликацию для базы данных SQL Azure и выполняет для нее отработку отказа на вторичную реплику. |
| [Настройка и выполнение отработки отказа для базы данных в пуле с помощью активной георепликации](../../sql-database/scripts/sql-database-setup-geodr-and-failover-pool-cli.md)| Настраивает активную георепликацию для базы данных SQL Azure в составе эластичного пула SQ, а потом выполняет для нее отработку отказа на вторичную реплику. |
...
См. дополнительные сведения об [API управления отдельной базой данных с помощью Azure CLI](single-database-manage.md#azure-cli).
