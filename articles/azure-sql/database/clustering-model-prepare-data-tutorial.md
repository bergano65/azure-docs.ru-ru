...
description: В первой части этого руководства, состоящего из трех частей, показано, как подготовить данные из базы данных в Базе данных SQL Azure для выполнения кластеризации на языке R с помощью Служб машинного обучения в Базе данных SQL Azure (предварительная версия).
...
В первой части этого руководства, состоящего из трех частей, показано, как импортировать и подготовить данные из базы данных в Базе данных SQL Azure на языке R. Затем эти данные будут использоваться для обучения и развертывания модели кластеризации на языке R с помощью Служб машинного обучения в Базе данных SQL Azure (предварительная версия).
...
В первой и второй части из этой серии описывается, как создать некоторые скрипты R в RStudio для подготовки данных и обучения модели машинного обучения. В третьей части показано, как выполнить эти скрипты R в базе данных с помощью хранимых процедур.
...
В [третьей части](clustering-model-deploy-tutorial.md) показано, как создать хранимую процедуру, с помощью которой можно выполнять кластеризацию на языке R на основе новых данных.
...
* Импорт примера базы данных в базу данных в Базу данных SQL Azure.
* Разделение клиентов по разным измерениям с использованием языка R.
* Загрузка данных из базы данных в кадр данных R.
...
> [Руководство. Создание прогнозной модели на языке R с помощью Служб машинного обучения в Базе данных SQL Azure (предварительная версия)](clustering-model-build-tutorial.md)
