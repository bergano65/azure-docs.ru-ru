---
title: Добавление пакета R в Службу машинного обучения (предварительная версия)
...
- Установите [R](https://www.r-project.org) и [RStudio Desktop](https://www.rstudio.com/products/rstudio/download/) на локальном компьютере. Пакет R доступен для операционных систем Windows, macOS и Linux. В этом документе предполагается, что вы используете Windows.
...
![Установленные пакеты в R](./media/machine-learning-services-add-r-packages/r-installed-packages.png)

## <a name="add-a-package-with-sqlmlutils"></a>Добавление пакета с помощью sqlmlutils

Если необходимо использовать пакет, который еще не установлен в Базе данных SQL Azure, его можно установить с помощью [sqlmlutils](https://github.com/Microsoft/sqlmlutils). **sqlmlutils** — это пакет, с помощью которого пользователи могут взаимодействовать с базами данных SQL Microsoft Azure и SQL Server и выполнять код R или Python в SQL из клиента R или Python. Сейчас в базе данных SQL Azure поддерживается только версия R **sqlmlutils**.
...
![Содержимое таблицы RTestData](./media/machine-learning-services-add-r-packages/r-verify-package-install.png)
