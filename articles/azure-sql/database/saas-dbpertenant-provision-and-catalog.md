...
Помимо функций для приложения SaaS, каталог позволяет включить средства для баз данных. В примере с базой данных SaaS Wingtip Tickets для каждого клиента каталог используется для включения запросов между клиентами, которые рассматриваются в [руководстве по автоматизированной обработке отчетов](saas-tenancy-cross-tenant-reporting.md). Управление заданиями между базами данных рассматривается в руководствах по [управлению схемами](saas-tenancy-schema-management.md) и [анализу клиентов](saas-tenancy-tenant-analytics.md) .
...
> [!IMPORTANT]
> Данные сопоставления доступны в базе данных каталога, но *не редактируют их*. Изменяйте данные сопоставления только с помощью интерфейсов API EDCL. Прямые манипуляции с данными сопоставления могут привести к повреждению каталога. Такие операции не поддерживаются.
...
1. В интегрированной среде сценариев PowerShell откройте... \\ Обучающие модули \\ ProvisionAndCatalog \\ _Demo-ProvisionAndCatalog.ps1_ и устанавливают следующие параметры:
   ...
   * **$DemoScenario**  =  **1**, *подготавливает один клиент*.
...
* **Получите объект каталога.** Перейдите к функции Get-Catalog, которая составляет и возвращает объект каталога, используемый в сценарии более высокого уровня. При этом используются функции управления сегментами, импортируемые из **AzureShardManagement.psm1**. Объект каталога состоит из следующих элементов.
   ...
   * $catalogServerFullyQualifiedName создается с использованием стандартного, а также имени пользователя: _Catalog- \<user\> . Database. Windows .NET_.
...
* **Убедитесь, что ключ клиента уже существует.** Каталог проверяется, чтобы убедиться, что ключ доступен.
* **Подготовка базы данных клиента с помощью New-TenantDatabase.** Используйте клавишу F11, чтобы перейти к функции и узнать, как база данных подготавливается с помощью [шаблона Azure Resource Manager](../../azure-resource-manager/templates/quickstart-create-templates-use-the-portal.md).
    ...
    Шаблон диспетчер ресурсов находится в папке. ..\Learning Модулес\коммон\: *tenantdatabasecopytemplate.js*
...
1. В интегрированной среде сценариев PowerShell откройте файл ...\\Learning Modules\\ProvisionAndCatalog\\*Demo-ProvisionAndCatalog.ps1*. Измените значение параметра *$DemoScenario* на 3.
   ...
   * **$DemoScenario**  =  **3**. *подготавливает пакет клиентов*.
