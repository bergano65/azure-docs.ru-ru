...
| ---:| ---:|:--- |
| 4060 |16 |Невозможно открыть базу данных %.&#x2a;ls, запрашиваемую именем входа. Сбой при входе. Дополнительные сведения см. [в разделе ошибки с 4000 по 4999](https://docs.microsoft.com/sql/relational-databases/errors-events/database-engine-events-and-errors#errors-4000-to-4999) .|
...
- [Выполнение устойчивого подключения к SQL с помощью ADO.NET](https://docs.microsoft.com/sql/connect/ado-net/step-4-connect-resiliently-sql-ado-net)
...
Обсуждение *периода блокировки* для клиентов, использующих ADO.NET, доступно в разделе [Организация пулов соединений (ADO.NET)](https://msdn.microsoft.com/library/8xx3tyca.aspx).
...
1. Выполните следующий запрос SQL, чтобы проверить [sys.dm_exec_requests](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-requests-transact-sql) представление, чтобы увидеть все блокирующие запросы:
...
3. Настройка запроса к головному блоку.
   ...
   Подробное описание процедуры устранения неполадок см. в статье как [работает мой запрос в облаке?](https://docs.microsoft.com/archive/blogs/sqlblog/is-my-query-running-fine-in-the-cloud).
...
Дополнительные сведения об ограничениях базы данных см. в разделе  [ограничения ресурсов базы данных SQL для серверов](https://docs.microsoft.com/azure/sql-database/sql-database-resource-limits-database-server).
...
Дополнительные сведения об ограничениях ресурсов см. в разделе [ограничения ресурсов логических серверов SQL Server](https://docs.microsoft.com/azure/sql-database/sql-database-resource-limits-database-server).
...
Если исключение вызывается проблемами запросов, вы увидите стек вызовов, аналогичный приведенному ниже (Обратите внимание на ссылку на класс **SqlCommand** ). В этом случае [Настройте запросы](https://docs.microsoft.com/archive/blogs/sqlblog/is-my-query-running-fine-in-the-cloud).
...
1. Убедитесь, что протокол TCP/IP включен в качестве клиентского протокола на сервере приложений. Дополнительные сведения см. в разделе [Настройка клиентских протоколов](https://docs.microsoft.com/sql/database-engine/configure-windows/configure-client-protocols). На серверах приложений, на которых не установлены средства SQL, убедитесь, что протокол TCP/IP включен, запустив **cliconfg.exe** (служебная программа SQL Server Client Network).
...
См. раздел [Получение сведений о подключении](https://docs.microsoft.com/azure/sql-database/sql-database-connect-query-ssms#get-sql-server-connection-information).
...
- [Архитектура подключения к базе данных SQL Azure](https://docs.microsoft.com/azure/sql-database/sql-database-connectivity-architecture)
- [Управление доступом к сети для базы данных SQL Azure и Azure синапсе Analytics](https://docs.microsoft.com/azure/sql-database/sql-database-networkaccess-overview)
