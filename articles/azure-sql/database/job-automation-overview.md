...
Вы можете создавать и добавлять в расписание задачи, которые могут периодически выполняться по отношению к одной или нескольким базам данных для выполнения запросов Transact-SQL (T-SQL) и задач по обслуживанию.
...
Доступны следующие технологии планирования заданий.

- **Задания агента SQL**  — классический и проверенный компонент планирования заданий SQL Server, доступный в управляемом экземпляре Azure SQL. Задания агента SQL недоступны в Базе данных SQL Azure.
- **Задания эластичной базы данных (предварительная версия)**  — службы планирования заданий, выполняющие пользовательские задания для одной или нескольких баз данных в Базе данных SQL Azure.

Стоит отметить пару различий между агентом SQL (доступным как локально, так и как часть Управляемого экземпляра SQL) и агентом заданий обработки эластичных баз данных (доступным для отдельных баз данных в Базе данных SQL Azure и для баз данных в Azure Synapse Analytics).
...
Задание может выполняться на одном локальном или на нескольких удаленных серверах. Задания агента SQL являются внутренним компонентом ядра СУБД, выполняемым в службе Управляемого экземпляра.
...
Агент SQL позволяет создавать различные типы шагов задания, например, шаги задания Transact-SQL, выполняющие один пакет Transact-SQL для базы данных, или шаги команды ОС или PowerShell, которые могут выполнять пользовательский сценарий ОС, шаги задания SSIS, позволяющие загружать данные с помощью среды выполнения SSIS, или шаги [репликации](../managed-instance/replication-transactional-overview.md), которые могут публиковать изменения вашей базы данных в других базах данных.

[Репликация транзакций](../managed-instance/replication-transactional-overview.md) является компонентом ядра СУБД, дающим возможность публиковать изменения, внесенные в одной или нескольких таблицах, в одной базе данных, и публиковать или передать их в набор баз данных подписчиков. Публикация изменений реализуется с помощью следующих типов шагов задания агента SQL:
...
> [!Note]
> Сейчас Управляемый экземпляр SQL не позволяет запустить задание, если экземпляр находится в режиме ожидания.
...
В системах с несколькими экземплярами в Управляемом экземпляре SQL или сервере SQL Server обязанности оператора могут разделяться между несколькими лицами. Оператор не содержит сведений о безопасности и не определяет субъект безопасности.

Создать операторы можно с помощью SSMS или сценария Transact-SQL, показанного в следующем примере:
...
Вы можете изменить любые задания и назначить оператор, который будет получать сообщения по электронной почте после завершения, сбоя или успешного завершения задания, с помощью SSMS или следующего сценария Transact-SQL:
...
- Сейчас в управляемом экземпляре не поддерживается включение или отключение агента SQL. Агент SQL работает всегда.
...
## <a name="elastic-database-jobs-preview"></a>Задания эластичной базы данных (предварительная версия)
...
|[**База данных заданий**](#job-database) | База данных в Базе данных SQL Azure, которую агент заданий использует для хранения данных, связанных с заданиями, определений заданий и т. д. |
...
Для создания **агента заданий обработки эластичных баз данных** требуется наличие базы данных в Базе данных SQL Azure. Агент настраивает эту существующую базу данных в качестве [*базы данных заданий*](#job-database).
...
*База данных заданий* не обязательно должна быть новой, но должна быть чистой, пустой и иметь цель служб S0 или выше. Рекомендуемая цель служб для *базы данных заданий*  — S1 или выше, однако оптимальный выбор зависит от уровня производительности, требуемого для заданий: количества шагов в задании, число целевых объектов и частоты выполнения заданий. Например, база данных S0 может быть достаточной для того, чтобы агент заданий выполнял несколько заданий в час, работая с меньше, чем десятью базами данных, однако при использовании базы данных S0 запуск заданий каждую минуту может быть недостаточно эффективным, и лучше использовать более высокий уровень служб.
...
- **Логический сервер SQL Server**  — если указан сервер, все базы данных, существующие на сервере во время выполнения задания, являются частью группы. Учетные данные главной базы данных должны быть предоставлены таким образом, чтобы группа могла быть перечислена и обновлена ​​до выполнения задания.
