...
description: Запросы аналитики между клиентами с использованием данных, извлеченных из базы данных SQL Azure, Azure синапсе Analytics, фабрики данных Azure или Power BI.
...
ms.locfileid: "91361115"
---
# <a name="explore-saas-analytics-with-azure-sql-database-azure-synapse-analytics-data-factory-and-power-bi"></a>Изучите аналитику SaaS с помощью базы данных SQL Azure, Azure синапсе Analytics, фабрики данных и Power BI
...
1. **Извлеките данные** из каждой базы данных клиента в хранилище аналитики, в данном случае — в пул SQL.
...
![Обзор архитектуры](./media/saas-tenancy-tenant-analytics-adf/adf_overview.png)
...
![Обзор архитектуры](./media/saas-tenancy-tenant-analytics-adf/starschematables.JPG)
...
- Разверните SaaS-приложение Wingtip Tickets c однотенантной БД. Сведения о развертывании менее чем за пять минут см. в статье [развертывание и изучение приложения SaaS Wingtip](../../sql-database/saas-dbpertenant-get-started-deploy.md).
...
- Установите Power BI Desktop. [Скачайте Power BI Desktop](https://powerbi.microsoft.com/downloads/).
...
1. В интегрированной среде сценариев PowerShell откройте файл *…\Learning Modules\Operational Analytics\Tenant Analytics DW\Demo-TenantAnalyticsDW.ps1* и задайте значение, приведенное ниже.
    - **$DemoScenario**  =  **1** . приобретение билетов на мероприятия для всех мест проведения
...
### <a name="deploy-azure-synapse-analytics-data-factory-and-blob-storage"></a>Развертывание Azure синапсе Analytics, фабрики данных и хранилища BLOB-объектов

В приложении Wingtip Tickets данные о транзакциях клиентов распределяются по нескольким базам данных. Фабрика данных Azure (ADF) используется для оркестрации извлечения, загрузки и преобразования (ELT) этих данных в хранилище данных. Для наиболее эффективного загрузки данных в Azure синапсе Analytics (ранее — хранилище данных SQL) ADF извлекает данные в промежуточные файлы больших двоичных объектов, а затем использует [polybase](https://docs.microsoft.com/azure/sql-data-warehouse/design-elt-data-loading) для загрузки данных в хранилище данных.

На этом шаге вы развернете дополнительные ресурсы, используемые в учебнике: пул SQL с именем _tenantanalytics_, фабрика данных Azure с именем _дбтодвлоад- \<user\> _и учетная запись хранения Azure с именем _вингтипстагинг \<user\> _. Учетная запись хранения используется для временного хранения файлов извлеченных данных в виде больших двоичных объектов перед их загрузкой в хранилище данных. На этом шаге также развертывается схема хранилища данных и определяются конвейеры ADF, которые выполняют оркестрацию процесса ELT.
...
1. В интегрированной среде сценариев PowerShell откройте файл *…\Learning Modules\Operational Analytics\Tenant Analytics DW\Demo-TenantAnalyticsDW.ps1* и задайте:
    - **$DemoScenario**  =  **2** . Развертывание хранилища данных аналитики клиента, хранилища BLOB-объектов и фабрики данных
...
![На снимке экрана показан обозреватель объектов с таблицами, развернутыми для отображения различных объектов базы данных.](./media/saas-tenancy-tenant-analytics-adf/DWtables.JPG)

#### <a name="blob-storage"></a>Хранилище BLOB-объектов

1. В [портал Azure](https://ms.portal.azure.com)перейдите к группе ресурсов, которая использовалась для развертывания приложения. Убедитесь, что была добавлена учетная запись хранения с именем **вингтипстагинг \<user\> ** .

   ![DWtables](./media/saas-tenancy-tenant-analytics-adf/adf-staging-storage.PNG)

1. Чтобы просмотреть имеющиеся объекты, щелкните Учетная запись хранения **вингтипстагинг \<user\> ** .
...
В [портал Azure](https://ms.portal.azure.com) в группе ресурсов убедитесь, что добавлена фабрика данных Azure с именем _ \<user\> дбтодвлоад_ .

 ![adf_portal](./media/saas-tenancy-tenant-analytics-adf/adf-data-factory-portal.png)

В этом разделе рассматривается созданная фабрика данных.
Чтобы запустить фабрику данных, сделайте следующее:

1. На портале щелкните фабрику данных с именем **дбтодвлоад- \<user\> **.
...
Существует также три параметризованных связанных служб, связывающих фабрику данных с исходными базами данных SQL, целевым пулом SQL и промежуточным хранилищем BLOB-объектов. На вкладке **Author** (Создание) щелкните **Подключения**, чтобы изучить связанные службы, как показано на следующем рисунке:
...
Azure синапсе (ранее — хранилище данных SQL) используется в качестве хранилища аналитики для выполнения статистической обработки данных клиента. В этом примере Polybase используется для загрузки данных в хранилище данных. Необработанные данные загружаются в промежуточные таблицы, в которых есть столбец идентификаторов, используемый для отслеживания строк, преобразованных в таблицы схемы типа "звезда". На следующем изображении показан шаблон загрузки: ![ Диаграмма показывает шаблон загрузки таблиц базы данных.](./media/saas-tenancy-tenant-analytics-adf/loadingpattern.JPG)

В этом примере используются таблицы измерения SCD (медленно изменяющиеся измерения) типа 1. В каждом измерении есть суррогатный ключ, определенный с помощью столбца идентификаторов. Мы рекомендуем заранее заполнять таблицу измерения даты, чтобы сэкономить время. Для других таблиц измерений CREATE TABLE как SELECT... Инструкция (CTAS) используется для создания временной таблицы, содержащей существующие измененные и неизмененные строки вместе с суррогатными ключами. Это делается с помощью оператора IDENTITY_INSERT=ON. Затем новые строки вставляются в таблицу с помощью оператора IDENTITY_INSERT=OFF. Чтобы выполнить откат без проблем, имеющейся таблице измерения и временной таблице присваивается имя новой таблицы измерения. Перед каждым запуском старая таблица измерения удаляется.
...
1. Щелкните **Активировать** и в извлеченном меню выберите **Trigger Now** (Активировать сейчас). Это действие запускается в конвейере немедленно. В сценарии для рабочей среды можно задать расписание для запуска конвейера, чтобы обновлять данные по расписанию.
  ![На снимке экрана показаны ресурсы фабрики для конвейера с именем S Q L D B до D W с выбранным параметром триггера развернутый и активированный.](./media/saas-tenancy-tenant-analytics-adf/adf_trigger.JPG)
...
1. Нажимайте кнопку **Обновить**, пока состояние конвейера SQLDBToDW не изменится на **Успешно**.
  ![На снимке экрана показан конвейер с ч L D B до D W с состоянием "успех".](./media/saas-tenancy-tenant-analytics-adf/adf_monitoring.JPG)
...
3. В окне **Получение данных** выберите **база данных SQL Azure**.
...
- Дополнительные [руководства, которые создаются на основе приложения SaaS Wingtip](../../sql-database/saas-dbpertenant-wingtip-app-overview.md#sql-database-wingtip-saas-tutorials).
