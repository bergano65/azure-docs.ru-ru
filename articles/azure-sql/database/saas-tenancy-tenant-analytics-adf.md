---
title: Выполнение аналитических запросов к клиентским базам данных
description: Сведения об аналитических запросах по нескольким клиентам с использованием данных, извлеченных из служб "База данных SQL Azure", Azure Synapse Analytics, "Фабрика данных Azure" или Power BI.
...
1. **Извлечение данных** из каждой базы данных клиентов в хранилище аналитики, например выделенный пул SQL.
...
![Иллюстрация схемы типа "звезда", используемой в этом руководстве.](./media/saas-tenancy-tenant-analytics-adf/starschematables.JPG)
...
На этом шаге развертываются дополнительные ресурсы, используемые в этом руководстве: выделенный пул SQL с именем _tenantanalytics_ , Фабрика данных Azure с именем _dbtodwload-\<user\>_ и учетная запись хранения Azure с именем _wingtipstaging\<user\>_ . Учетная запись хранения используется для временного хранения файлов извлеченных данных в виде больших двоичных объектов перед их загрузкой в хранилище данных. На этом шаге также развертывается схема хранилища данных и определяются конвейеры ADF, которые выполняют оркестрацию процесса ELT.
...
![Снимок экрана обозревателя объектов с развернутыми таблицами для отображения разных объектов базы данных.](./media/saas-tenancy-tenant-analytics-adf/DWtables.JPG)

#### <a name="blob-storage"></a>Хранилище BLOB-объектов

1. На [портале Azure](https://ms.portal.azure.com) перейдите к группе ресурсов, которую использовали для развертывания приложения. Проверьте, добавлена ли учетная запись хранения с именем **wingtipstaging\<user\>** .

   ![DWtables](./media/saas-tenancy-tenant-analytics-adf/adf-staging-storage.PNG)

1. Щелкните учетную запись хранения **wingtipstaging\<user\>** , чтобы изучить присутствующие в ней объекты.
...
Убедитесь, что на [портале Azure](https://ms.portal.azure.com) в группе ресурсов добавлена фабрика данных Azure с именем _dbtodwload-\<user\>_ .
...
1. На портале выберите фабрику данных с именем **dbtodwload-\<user\>** .
...
Есть также три параметризованные связанные службы, которые связывают фабрику данных с исходными базами данных SQL, целевым выделенным пулом SQL и промежуточным хранилищем BLOB-объектов. На вкладке **Author** (Создание) щелкните **Подключения** , чтобы изучить связанные службы, как показано на следующем рисунке:
...
Azure Synapse (ранее — Хранилище данных SQL) используется в качестве хранилища аналитики при выполнении статистических операций с данными клиентов. В этом примере PolyBase используется для загрузки данных в хранилище данных. Необработанные данные загружаются в промежуточные таблицы, в которых есть столбец идентификаторов, используемый для отслеживания строк, преобразованных в таблицы схемы типа "звезда". На следующем рисунке показан шаблон загрузки: ![Схема с шаблоном загрузки для таблиц базы данных.](./media/saas-tenancy-tenant-analytics-adf/loadingpattern.JPG)
...
1. Щелкните **Активировать** и в извлеченном меню выберите **Trigger Now** (Активировать сейчас). Это действие запускается в конвейере немедленно. В сценарии для рабочей среды можно задать расписание для запуска конвейера, чтобы обновлять данные по расписанию.
  ![Снимок экрана с ресурсами фабрики для конвейера с именем SQLDBToDW, где развернут вариант "Триггер" и выбрано действие "Активировать сейчас".](./media/saas-tenancy-tenant-analytics-adf/adf_trigger.JPG)
...
1. Нажимайте кнопку **Обновить** , пока состояние конвейера SQLDBToDW не изменится на **Успешно**.
  ![Снимок экрана с конвейером SQLDBToDW, для которого указано состояние "Успешно".](./media/saas-tenancy-tenant-analytics-adf/adf_monitoring.JPG)
