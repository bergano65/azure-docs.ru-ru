...
:::image type="content" source="media/azure-defender-for-sql/enable-azure-defender.png" alt-text="Включение защитника Azure":::
...
Чтобы переопределить параметры защитника Azure для определенной базы данных, установите флажок **включить защитник Azure для SQL на уровне базы данных** в параметрах **центра безопасности** базы данных. Используйте этот параметр только в том случае, если у вас есть определенное требование для получения отдельных оповещений о повышенной защите от угроз или результатов оценки уязвимостей для отдельной базы данных вместо предупреждений и результатов, полученных для всех баз данных на сервере или управляемом экземпляре.

После выбора флажка можно настроить соответствующие параметры для этой базы данных.

:::image type="content" source="media/azure-defender-for-sql/enable-for-database-level.png" alt-text="Включение защитника Azure на уровне базы данных":::

Параметры защитника Azure для сервера или управляемого экземпляра можно также получить в области базы данных защитника Azure. Щелкните **Параметры** в главной области центра безопасности, а затем щелкните **Просмотреть параметры защитника Azure для SQL Server**.
