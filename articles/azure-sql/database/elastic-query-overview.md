...
Эластичный запрос можно использовать, чтобы сделать данные, расположенные в базе данных SQL, доступными для других баз данных SQL. Это позволяет запросам из одной базы данных ссылаться на таблицы в любой другой удаленной базе данных в базе данных SQL. Первым этапом является определение внешнего источника данных для каждой удаленной базы данных. Внешний источник данных определяется в локальной базе данных, из которой необходимо получить доступ к таблицам, расположенным в удаленной базе данных. Вносить изменения в удаленную базу данных не нужно. В стандартных сценариях вертикального секционирования, когда с разными базами данных используются разные схемы, эластичные запросы позволяют решать такие стандартные задачи, как доступ к ссылочным данным и создание межбазовых запросов.
...
Следующие шаги позволяют настроить запросы к эластичной базе данных для сценариев вертикального секционирования, которым требуется доступ к таблице, расположенной в удаленных базах данных SQL, с той же схемой:
...
Следующие шаги позволяют настроить запросы к эластичной базе данных для сценариев горизонтального секционирования, которым требуется доступ к набору таблиц, размещенных в (обычно) в базе данных SQL.
...
* Импорт и экспорт базы данных SQL пока не поддерживает внешние источники данных и внешние таблицы. Если вам нужно использовать функции импорта и экспорта, удалите эти объекты перед экспортом и создайте их заново после импорта.
...
* Эластичный запрос работает только с базой данных SQL Azure. Его нельзя использовать для запроса экземпляра SQL Server.

## <a name="share-your-feedback"></a>Поделитесь своими отзывами
