...
![автоматическая отработка отказа](./media/auto-failover-group-overview/auto-failover-group-mi.png)
...
Дополнительные сведения о создании вторичного Управляемый экземпляр SQL в той же зоне DNS, что и у основного экземпляра, см. [в разделе Создание вторичного управляемого экземпляра](../managed-instance/failover-group-add-instance-tutorial.md#3---create-a-secondary-managed-instance).
...
Группа отработки отказа будет управлять отработкой отказа всех баз данных в Управляемый экземпляр SQL. При создании группы каждая база данных в экземпляре будет автоматически реплицироваться на дополнительный SQL-Управляемый экземпляр. Вы не можете использовать группы отработки отказа для запуска частичной отработки отказа подмножества баз данных.
...
- Необходимо настроить группы безопасности сети (NSG), чтобы порты 5022 и диапазон 11000 ~ 12000 были открытыми входящими и исходящими для подключений из подсети другого управляемого экземпляра. Это позволяет обеспечить репликацию трафика между экземплярами.
