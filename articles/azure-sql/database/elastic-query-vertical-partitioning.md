...
> [!NOTE]
> Убедитесь, что в не указан `<username>` суффикс **" \@ ServerName"** .

## <a name="create-external-data-sources"></a>Создание внешних источников данных

Синтаксис:

<External_Data_Source>:: = создать внешний источник данных <data_source_name> с (TYPE = RDBMS, LOCATION = ' <fully_qualified_server_name> ', DATABASE_NAME = ' <remote_database_name> ',  
...
Создание внешней таблицы [database_name. [ schema_name ] . | schema_name. ] table_name  
...
Вы можете использовать обычные SQL Server строки подключения, чтобы подключить свои средства бизнес-аналитики и интеграции данных к базам данных на сервере с включенными эластичными запросами и определенными внешними таблицами. Убедитесь, что в качестве источника данных для вашего инструмента поддерживается SQL Server. Затем добавьте ссылку на эластичную базу данных запросов и ее внешние таблицы так же, как и ссылку на любую другую базу данных SQL Server, к которой вы подключались бы с помощью своего средства.
