...
Многие приложения имеют нормативные требования, соответствие требованиям или другие бизнес-цели, для которых требуется хранить резервные копии баз данных за пределами 7-35 дней, предоставляемых базой данных SQL Azure и Azure SQL Управляемый экземпляр [автоматических резервных копий](automated-backups-overview.md). Используя функцию долгосрочного хранения (LTR), можно хранить указанную базу данных SQL и полную резервную копию SQL Управляемый экземпляр в хранилище BLOB-объектов Azure с геоизбыточным хранилищем с доступом на чтение до 10 лет. Впоследствии любую резервную копию можно восстановить как новую базу данных.

Кроме того, SQL Управляемый экземпляр предоставляет [настраиваемую избыточность хранилища резервных копий](automated-backups-overview.md#backup-storage-redundancy) , которая обеспечивает гибкость при выборе между локальным избыточным (LRS), избыточным в пределах зоны (ZRS) или геоизбыточным (RA-GRS) [BLOB-объектами хранилища](../../storage/common/storage-redundancy.md). Этот параметр в настоящее время доступен только в процессе создания управляемого экземпляра и не может быть изменен после подготовки ресурса.
...
## <a name="sql-managed-instance-support"></a>Поддержка Управляемый экземпляр SQL
