...
Можно создавать отчеты из нескольких баз данных из одной точки соединения с помощью [эластичного запроса](elastic-query-overview.md). Базы данных должны быть горизонтально секционированными (сегментированными).

Если база данных уже существует, см. статью [Перенос существующих баз данных для масштабирования](elastic-convert-to-use-elastic-tools.md).

Чтобы узнать, какие объекты SQL требуются для выполнения запросов, см. статью [Отчеты по масштабируемым облачным базам данных (предварительная версия)](elastic-query-horizontal-partitioning.md).

## <a name="prerequisites"></a>Предварительные условия

Загрузите и запустите пример [Приступая к работе с инструментами эластичной базы данных](elastic-scale-get-started.md).
...
1. Выполните сборку и запустите пример приложения " **Приступая к работе со средствами эластичной базы данных** ", выполнив действия, описанные в разделе [Загрузка и запуск примера приложения](elastic-scale-get-started.md#download-and-run-the-sample-app-1). После завершения всех шагов вы увидите следующую командную строку:
...
2. Создайте новую базу данных в базе данных SQL Azure на том же сервере, что и настройка сегмента. Назначьте базе данных имя ElasticDBQuery.
...
2. Подключитесь к базе данных ElasticDBQuery и выполните следующие команды T-SQL:
    ...
    "Username" и "Password" должны быть такими же, как и сведения об имени входа, используемые в шаге 3 раздела [скачать и запустить пример приложения](elastic-scale-get-started.md#download-and-run-the-sample-app) в статье **Приступая к работе со средствами эластичной базы данных** .
