...
> [!NOTE]
> SKU шлюза влияет на производительность пропускной способности. В этой статье описывается развертывание шлюза с самым базовым номером SKU ( `HwGw1` ). Разверните более высокий номер SKU (например `VpnGw3` ,), чтобы повысить пропускную способность. Все доступные параметры см. в разделе [SKU шлюзов](../../vpn-gateway/vpn-gateway-about-vpngateways.md#benchmark) . 
