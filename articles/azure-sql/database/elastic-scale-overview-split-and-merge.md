...
1. [Руководство по разбиению и слиянию эластичных баз данных](elastic-scale-configure-deploy-split-and-merge.md)
2. [Настройка параметров безопасности для службы разделения и объединения](elastic-scale-split-merge-security-configuration.md)
3. [Вопросы безопасности служб разделения и объединения](elastic-scale-split-merge-security-configuration.md)
4. [Управление размещением сегментов](elastic-scale-shard-map-management.md)
5. [Перенос существующих баз данных для масштабирования](elastic-convert-to-use-elastic-tools.md)
6. [Инструменты эластичных баз данных](elastic-scale-introduction.md)
7. [Глоссарий по средствам работы с эластичными базами данных](elastic-scale-glossary.md)
...
- **Сжатием путем слияния**
  ...
  Из-за сезонных изменений в характере бизнеса потребности в емкости могут меняться. Это средство позволяет сократить число единиц масштабирования, когда бизнес временно замедляется. В этом случае функция "Merge" в службе разбиения и объединения эластичного масштабирования охватывает это требование.
...
- **Службы, размещаемые на стороне клиента**
  ...
  Служба разбиения/объединения реализуется в виде службы, размещаемой на стороне клиента. Необходимо развернуть и разместить службу в вашей подписке Microsoft Azure. Пакет, загружаемый через NuGet, содержит шаблон конфигурации, в который нужно внести информацию о вашем развертывании. Подробнее см. в [руководстве по службе разбиения и слияния](elastic-scale-configure-deploy-split-and-merge.md). Так как служба выполняется в подписке Azure, вы можете настроить большинство аспектов безопасности службы и управлять ими. Шаблон по умолчанию включает в себя параметры для настройки TLS, проверки подлинности клиента на основе сертификатов, шифрования для сохраненных учетных данных, защиты DoS и ограничений IP-адресов. Дополнительные сведения об аспектах безопасности можно получить в документе [Настройка параметров безопасности для службы разбиения и объединения](elastic-scale-split-merge-security-configuration.md).
...
- **Сведения о сегментировании**
  ...
  - **Другие таблицы**
    ...
    Информация о ссылочных и сегментированных таблицах предоставляется интерфейсами API `SchemaInfo` в сопоставлении сегментов. Следующий пример демонстрирует использование этих API для объекта диспетчера сопоставления сегментов.
    ...
    Таблицы "регион" и "страна" определяются как ссылочные таблицы и будут скопированы с помощью операций разбиения и объединения. "клиент" и "заказы", в свою очередь, определяются как сегментированные таблицы. `C_CUSTKEY` и `O_CUSTKEY` используются в качестве ключа сегментирования.
...
- **Согласованность сопоставления сегментов и окончательное завершение**
...
- **Схема сегментов**
  ...
  Следующий раздел параметров запроса описывает сопоставление сегментов и базу данных, в которой размещается сопоставление сегментов. В частности, необходимо указать имя сервера и базы данных, где размещается shardmap, учетные данные для подключения к базе данных сопоставления сегментов и, наконец, имя сопоставления сегментов. В настоящее время операция принимает только один набор учетных данных. Эти учетные данные должны иметь достаточные разрешения для изменения сопоставления сегментов, а также пользовательских данных в этих сегментах.

- **Исходный диапазон (разбиение и слияние)**
...
- **Поведение раздельного источника (разделение)**
...
- **Целевой сегмент (с разделением)**

  После предоставления сведений об источнике операции разбиения необходимо определить, куда следует копировать данные, указав имя сервера и базы данных для целевого объекта.

- **Целевой диапазон (слияние)**
...
1. Следуйте инструкциям в разделе [Развертывание службы разбиения и объединения](elastic-scale-configure-deploy-split-and-merge.md).
