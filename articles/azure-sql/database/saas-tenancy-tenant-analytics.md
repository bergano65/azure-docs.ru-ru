---
title: Межклиентская аналитика с помощью извлеченных данных
description: Запросы аналитики между клиентами, использующие данные, извлеченные из нескольких баз данных SQL Azure в одном приложении клиента.
...
3.  Используйте средства **бизнес-аналитики** для отображения полезных ценных сведений, которые могут помочь в принятии решений. 
...
![На схеме показан обзор архитектуры, используемой в этой статье.](./media/saas-tenancy-tenant-analytics/architectureOverview.png)
...
Часто используется большое число транзакционных баз данных, которые совместно содержат данные всех клиентов. Вам необходимо объединить данные клиента из большого числа транзакционных баз данных в одно хранилище аналитики. Статистическая обработка позволяет выполнять эффективный запрос данных. В этом руководстве для хранения статистических данных используется база данных SQL Azure.
...
2. Задайте в сценарии для переменной $DemoScenario значение в зависимости от хранилища аналитики:
    - Чтобы использовать базу данных SQL без хранилища столбцов, установите **$DemoScenario**  =  **2** .
    - Чтобы использовать базу данных SQL с хранилищем столбцов, установите **$DemoScenario**  =  **3** .  
...
Теперь, когда вы развернули приложение и заполнили его интересными данными клиента, используйте [SQL Server Management Studio (SSMS)](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms) для подключения серверов **tenants1-DPT- &lt; user &gt; ** и **Catalog-DPT- &lt; пользователь &gt; ** с помощью имени входа — *разработчика*, Password = *P \@ ssword1*. Дополнительные рекомендации см. в этом [ознакомительном руководстве](../../sql-database/saas-dbpertenant-wingtip-app-overview.md).

![Обзор архитектуры](./media/saas-tenancy-tenant-analytics/ssmsSignIn.png)

В обозревателе объектов сделайте следующее:

1. Разверните сервер *tenants1-DPT- &lt; user &gt; * .
...
![На снимке экрана показана база данных Екстракттиккетс с TicketsRawData d b o, выбранной в обозревателе объектов.](./media/saas-tenancy-tenant-analytics/ticketExtracts.png)
...
5. Выберите **база данных** на левой панели, введите имя пользователя = *Developer*и введите password = *P \@ ssword1*. Нажмите кнопку **Соединить**.  
    ...
    ![На снимке экрана показано диалоговое окно SQL Server базы данных, в котором можно ввести имя пользователя и пароль.](./media/saas-tenancy-tenant-analytics/databaseSignIn.PNG)
...
![На снимке экрана показана Power B визуализация и элементы управления для визуализации данных в правой части.](./media/saas-tenancy-tenant-analytics/TotalTicketsByVenues.PNG)
...
![На снимке экрана показано Power B Визуализация с названием "распределение продаж билетов" и "день продажи".](./media/saas-tenancy-tenant-analytics/SaleVersusDate.PNG)
...
![На снимке экрана отображается Power B с названием среднее число билетов, проданных каждым из мест проведения.](./media/saas-tenancy-tenant-analytics/AvgTicketsByVenues.PNG)
