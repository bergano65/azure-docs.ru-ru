...
* Разверните приложение SaaS Wingtip. Это можно сделать менее чем за пять минут, следуя инструкциям из статьи [Развертывание и изучение мультитенантного приложения SaaS на основе базы данных SQL Azure, в котором используется отдельная база данных для каждого клиента](../../sql-database/saas-dbpertenant-get-started-deploy.md).
