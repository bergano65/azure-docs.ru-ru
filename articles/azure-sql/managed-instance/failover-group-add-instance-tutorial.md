...
> - Тестовая отработка отказа.
  ...
  > - Для управляемых экземпляров, участвующих в группе отработки отказа, требуется [Azure ExpressRoute](../../expressroute/expressroute-howto-circuit-portal-resource-manager.md) или два ПОДКЛЮЧЕННЫХ VPN-шлюза. Пиринг глобальной виртуальной сети не поддерживается. В этом руководстве содержатся инструкции по созданию и подключению VPN-шлюзов. Если вы уже настроили ExpressRoute, пропустите эти шаги. 
...
1. Заполните обязательные поля, чтобы настроить виртуальную сеть для вторичного управляемого экземпляра, а затем нажмите кнопку **создать**. 
   ...
   В следующей таблице приведены значения, необходимые для вторичной виртуальной сети.
    ...
    | **Адресное пространство** | Адресное пространство виртуальной сети, например `10.128.0.0/16` . | 
...
## <a name="create-a-primary-gateway"></a>Создание основного шлюза 

Чтобы два управляемых экземпляра участвовали в группе отработки отказа, необходимо настроить ExpressRoute или шлюз, настроенный между виртуальными сетями двух управляемых экземпляров, чтобы разрешить сетевое взаимодействие. Если вы решили настроить [ExpressRoute](../../expressroute/expressroute-howto-circuit-portal-resource-manager.md) вместо подключения двух VPN-шлюзов, перейдите к [шагу 7](#create-a-failover-group).  
...
> [!NOTE]
> SKU шлюза влияет на производительность пропускной способности. В этом руководстве выполняется развертывание шлюза с самым базовым номером SKU ( `HwGw1` ). Разверните более высокий номер SKU (например `VpnGw3` ,), чтобы повысить пропускную способность. Все доступные параметры см. в разделе [SKU шлюзов](../../vpn-gateway/vpn-gateway-about-vpngateways.md#benchmark) .
...
1. На странице **Основные сведения** выберите следующие значения и нажмите кнопку **ОК**. 
...
1. На странице **Параметры** выберите или введите следующие значения, а затем нажмите кнопку **ОК**.
    1. Выберите основной сетевой шлюз для **первого шлюза виртуальной сети**, например `primaryGateway` .  
    1. Выберите дополнительный сетевой шлюз для **второго шлюза виртуальной сети**, например `secondaryGateway` . 
...
1. На странице " **Проверка и создание** " Проверьте параметры двунаправленного подключения, а затем нажмите кнопку **ОК** , чтобы создать подключение. 
