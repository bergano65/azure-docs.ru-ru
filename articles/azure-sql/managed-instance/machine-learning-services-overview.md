...
Если вы хотите присоединиться к программе предварительной версии и принять эти условия, вы можете запросить регистрацию, создав запрос в службу поддержки Azure по адресу [https://azure.microsoft.com/support/create-ticket/](https://azure.microsoft.com/support/create-ticket/) . Для "тип проблемы" выберите "техническая", для "служба" выберите "Управляемый экземпляр Базы данных SQL", а для "тип проблемы" выберите "другое". В тексте запроса укажите, что вы хотите использовать ограниченную общедоступную предварительную версию Машинного обучения для Управляемого экземпляра SQL. Кроме того, укажите имя логического сервера, регион и идентификатор подписки.
