...
ms.locfileid: "79215462"
---
# <a name="automate-management-tasks-using-database-jobs"></a>Автоматизация задач управления с помощью заданий базы данных

База данных SQL Azure позволяет создавать и добавлять в расписание задачи, которые могут периодически выполняться по отношению к одной или нескольким базам данных для выполнения запросов T-SQL и задач по обслуживанию.
...
- Сбор данных для отчетов
  - Сбор данных из коллекции баз данных SQL Azure в одну целевую таблицу.
...
- **Задания агента SQL** — классический и проверенный компонент планирования заданий SQL Server, доступный в Управляемом экземпляре. Задания агента SQL недоступны в отдельных базах данных SQL Azure.
- **Задания эластичной базы данных (предварительная версия)**  — службы планирования заданий, выполняющие пользовательские задания для одной или нескольких баз данных SQL Azure.
...
> [!Note]
> Сейчас Управляемый экземпляр не позволяет запустить задание, если экземпляр находится в режиме ожидания.
...
Вы можете уведомить оператора, что с вашими заданиями агента SQL что-то произошло. Оператор определяет контактные сведения о лице, ответственном за обслуживание одного или нескольких Управляемых экземпляров. Иногда обязанности оператора возлагаются на одно лицо.
...
Агент заданий обработки эластичных баз данных является бесплатным. Счета за использование базы данных заданий выставляются в тот же способ, что и за использование любой базы данных SQL.
...
Если операции с базой данных заданий выполняются медленнее, чем ожидалось, то производительность базы данных и использование ресурсов в базе данных заданий в периоды медленной работы нужно [отслеживать](sql-database-monitor-tune-overview.md#sql-database-resource-monitoring) с помощью портала Azure или динамического административного представления [sys.dm_db_resource_stats](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database). Если использование ресурса, например ЦП, операций ввода-вывода данных или записи журнала, приближается к 100 % и сопоставляется с периодами медленной работы, рассмотрите возможность постепенного масштабирования базы данных до более высоких целей обслуживания (в [модели DTU](sql-database-service-tiers-dtu.md) или в [модели vCore](sql-database-service-tiers-vcore.md)), пока производительность базы данных заданий не увеличится.
