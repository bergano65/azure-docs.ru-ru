...
- Сейчас в управляемом экземпляре не поддерживается включение или отключение агента SQL. Агент SQL работает постоянно.
