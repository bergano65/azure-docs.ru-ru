---
title: Управляемый экземпляр определяет размер VNet/subnet
...
Для управляемого экземпляра необходимо не менее 16 и не более 256 IP-адресов в подсети. В результате при определении диапазонов IP-адресов подсети можно использовать маски подсети между /28 и /24. Сеть маска бит /28 (14 хостов в сети) является хорошим размером для одной общей цели или бизнес-критического развертывания. Маска бит /27 (30 хостов в сети) идеально подходит для нескольких развертываний управляемых экземпляров в рамках одного и того же VNet. Параметры бита маски /26 (62 хоста) и /24 (254 хоста) позволяют дальнейшее масштабирование из VNet для поддержки дополнительных управляемых инстанций.

> [!IMPORTANT]
> Размер подсети с 16 IP-адресами является минимальным с ограниченным потенциалом, когда операция масштабирования, как изменение размера vCore не поддерживается. Рекомендуется выбрать поднет с приставкой /27 или самой длинной префиксом.
