...
В рабочей среде обычно создается запись CNAME DNS, которая [*указывает Интернет-домен*](../traffic-manager/traffic-manager-point-internet-domain.md) компании на DNS-имя диспетчера трафика.
...
1. В интегрированной среде сценариев PowerShell откройте... \\Программы\\Learning\\modules*Demo-LoadGenerator. ps1* script.
