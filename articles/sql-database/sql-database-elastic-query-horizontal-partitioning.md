...
> [!NOTE]
> Убедитесь, что *\<"имя\>пользователя"* не содержит суффикса *"имя\@сервера".*
...
1. **SHARDED** означает, что данные горизонтально разделены по базам данных. Ключ секционирования для распределения данных указывается в параметре **<sharding_column_name>**.
...
3. **ROUND\_ROBIN** означает, что таблица горизонтально разделена с помощью зависимого от приложения метода распределения.
