...
> [!NOTE]
> Убедитесь, что *"\<username\>"* не содержит суффикс *"\@ServerName* ".
...
3. **Циклический\_перебор** означает, что таблица горизонтально секционирована с помощью метода распределения, зависящего от приложения.
