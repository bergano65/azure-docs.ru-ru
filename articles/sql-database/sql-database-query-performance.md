...
description: Мониторинг производительности запросов определяет наиболее многопотребляющие и длительные запросы для отдельных и объединенных баз данных в базе данных Azure S'L.
...
Query Performance Insight обеспечивает интеллектуальный анализ запросов для отдельных и объединенных баз данных. Это помогает определить основные ресурсоемкие и длительные запросы в рабочей нагрузке. Это поможет вам найти запросы для оптимизации для повышения общей производительности рабочей нагрузки и эффективного использования ресурса, за который вы платите. Запрос Performance Insight поможет вам тратить меньше времени на устранение производительности базы данных, предоставляя:

* Более глубокое понимание потребления ресурсов ресурсов баз данных (DTU)
* Подробная информация о топовых запросах базы данных по цП, продолжительности и количеству выполнения (потенциальные кандидаты на настройку для повышения производительности)
* Возможность просверлить детали запроса, просмотреть текст запроса и историю использования ресурсов
* Аннотации, показывающие рекомендации по производительности от [советников по базам данных](sql-database-advisor.md)
...
2. Из меню слева откройте **Интеллектуальную производительность** > **запроса Insight**.
...
5. Открыть **рекомендации по производительности интеллектуальной** > **производительности** и проверить, есть ли какие-либо рекомендации по производительности. Дополнительные сведения о встроенных рекомендациях по производительности см. в статье [Помощник по базам данных SQL Azure](sql-database-advisor.md).
...
1. С помощью флажков выделите отдельные запросы или отмените их выделение, чтобы включить их в диаграмму или исключить из нее.
   ...
   >
   > 1. На портале Azure выберите > **мониторинг**базы **данных Azure S'L.**
   > 2. Выберите **метрики**.
...
5. Чтобы просмотреть настраиваемое представление, нажмите кнопку **Перейти >**.
   ...
   >
   > 1. Выберите > **мониторинг**базы **данных Azure S'L.**
   > 2. Выберите **метрики**.
   > 3. Выберите **+ Добавить диаграмму**.
   > 4. Выберите процент DTU на диаграмме.
   > 5. Дополнительно выберите **За последние 24 часа** в меню в верхнем левом углу и укажите интервал, равный одной минуте.
...
Для выявления часто выполняемых ("болтливых") запросов:
...
## <a name="optimize-the-query-store-configuration"></a>Оптимизация конфигурации магазина запросов
...
* **Размер на основе**: Если эта политика установлена **на AUTO,** он будет очищать данные автоматически, когда почти максимальный размер достигается.
* **Время основано**: По умолчанию, эта политика установлена на 30 дней. Если в хранилище запросов не хватает места, будут удалены данные запросов старше 30 дней.

Для политики сбора можно задать следующие значения.

* **Все:** Магазин запросов фиксирует все запросы.
* **Auto**: Магазин запросов игнорирует нечастые запросы и запросы с незначительной компиляцией и длительностью выполнения. Пороговые значения числа выполнений, а также времени компиляции и выполнения определяются внутренним образом. Это параметр по умолчанию.
* **Нет:** Магазин запросов не прекращает захват новых запросов, но статистика времени выполнения уже захваченных запросов по-прежнему собирается.
...
Рассмотрите возможность использования [Azure S'L Analytics](../azure-monitor/insights/azure-sql.md) для расширенного мониторинга производительности большого парка единых и объединенных баз данных, эластичных пулов, управляемых экземпляров и баз данных экземпляров.
