...
1. В меню портал Azure выберите пункт **Справка и поддержка**, а затем выполните поиск и выберите **Справка и поддержка** на любой странице.

2. В [справке и поддержке Azure](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/overview)выберите [**новый запрос в службу поддержки**](https://ms.portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest).

3. В качестве **типа проблемы**выберите **пределы службы и подписки (квоты)** .

4. Выберите подписку, которая будет использоваться для создания баз данных.

5. В качестве **типа квоты**выберите **база данных SQL**.

    ![Колонка "Справка и поддержка Azure"](media/sql-database-service-tier-hyperscale/new-support-request-screen.png)

6. Нажмите кнопку **Далее: решения**.

7. Щелкните **указать подробности**.
...
| Восстановление базы данных, отличной от «масштабирование», в масштабах и наоборот | Невозможно восстановить базу данных с масштабированием в базу данных, не отменяя возможности масштабирования, а также восстановить базу данных без возможности масштабирования в базу данных для масштабирования.|
...
| Миграция баз данных с помощью объектов OLTP в памяти | Масштабирование поддерживает только подмножество типов объектов OLTP в памяти, включая оптимизированные для памяти табличные типы, скомпилированные в собственном код хранимые процедуры и функции. Однако если в базе данных имеются любые объекты OLTP в памяти, прямая миграция с уровней "Премиум" и "критически важный для бизнеса" на "горизонтальное" не поддерживается. Миграция такой базы данных на подмасштабирование требует выполнения трех шагов: (1) удаление всех объектов OLTP в памяти и их зависимостей. Чтобы сохранить данные в устойчивых таблицах, оптимизированных для памяти, преобразуйте их в таблицы диска. (2) измените уровень обслуживания базы данных на "горизонтальное масштабирование". (3) повторное создание ранее удаленных объектов. Устойчивые и неустойчивые таблицы, оптимизированные для памяти, в настоящее время не поддерживаются в масштабах и должны оставаться дисковой таблицей. Поддерживаются оптимизированные для памяти табличные переменные. |
| Отслеживание изменений | Отслеживание изменений в настоящее время находится в общедоступной предварительной версии и может быть включена в новых или существующих базах данных с масштабированием. |
