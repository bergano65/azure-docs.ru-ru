...
- **Служба хранилища**:
  ...
  Указывать максимальный объем данных при настройке базы данных ценовой категории "Гипермасштабирование" не нужно. На уровне "горизонтальный" вы платите за хранилище для базы данных на основе фактического распределения. Хранилище автоматически выделяется между 40 ГБ и 100 ТБ в 10 ГБ с шагом в 10 ГБ. При необходимости несколько файлов данных могут увеличиваться одновременно. База данных с масштабированием создается с начальным размером 10 ГБ и начинает расти на 10 ГБ каждые 10 минут, пока не достигнет размера 40 ГБ.
...
В базах данных масштаба `ApplicationIntent` аргумент в строке подключения, предоставленной клиентом, определяет, направляется ли соединение к реплике записи или к вторичной реплике только для чтения. Если параметру `ApplicationIntent` присвоено значение `READONLY` и у базы данных нет вторичной реплике, подключение будет направляться к первичной реплике в режиме по умолчанию `ReadWrite`.
...
| Восстановление на момент времени | После переноса базы данных на уровень службы "масштабирование" восстановление до точки во времени до миграции не поддерживается.|
