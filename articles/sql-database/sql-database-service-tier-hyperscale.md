...
- Практически мгновенного создания резервных копий, (на основе файлов моментальных снимков хранятся в хранилище BLOB-объектов Azure) базы данных независимо от размера, не влияя на операции ввода-ВЫВОДА на вычислительные ресурсы  
...
Следующая команда T-SQL создает базу данных ценовой категории "Гипермасштабирование". Необходимо указать целевые выпуск и службу в инструкции `CREATE DATABASE`. Ссылаться на [ограничения ресурсов](https://docs.microsoft.com/azure/sql-database/sql-database-vcore-resource-limits-single-databases#hyperscale-service-tier) список целей допустимым обслуживания.
...
| Проблема | Описание |
| :---- | :--------- |
| На панели по управлению резервными копиями для логического сервера не показываются Гипермасштабируемого баз данных будут отфильтрованы из SQL server  | Гипермасштабируемого имеет отдельный метод для управления резервными копиями, и таким образом долгосрочного хранения и точки в резервной копии параметры хранения времени не применяются / становятся недействительными. Следовательно, базы данных ценовой категории "Гипермасштабирование" не отображаются в области "Управление резервными копиями". |
| Восстановление до точки во времени | После миграции базы данных на уровень Гипермасштабируемые службы не поддерживается восстановление на определенный момент времени перед миграцией.|
...
| Эластичные пулы |  Пулы эластичных баз данных с крупномасштабным базы данных SQL в настоящее время не поддерживаются.|
