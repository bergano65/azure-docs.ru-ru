---
title: Общие сведения об обеспечении безопасности в Базе данных SQL Azure | Документация Майкрософт
...
Авторизация — это по сути набор разрешений, которые назначены пользователю в базе данных SQL Azure и которые определяют, какие действия доступны этому пользователю. Разрешениями осуществляется путем добавления учетных записей пользователей в [роли базы данных](/sql/relational-databases/security/authentication-access/database-level-roles) и назначение разрешений уровня базы данных для этих ролей, либо путем предоставления определенных пользователем [разрешениями на уровне объекта](/sql/relational-databases/security/permissions-database-engine). См. дополнительные сведения об [операциях входа и пользователях](sql-database-manage-logins.md).

Рекомендуется создайте пользовательские роли, при необходимости. Добавление пользователей в роль с минимальным числом привилегий, необходимых для выполнения должностных функций. Не назначайте разрешения непосредственно для пользователей. Учетная запись администратора сервера является членом роли db_owner встроенные, который должен предоставляться только несколько пользователей с помощью административных обязанностей и широких прав доступа. Для приложений баз данных SQL Azure, используйте [EXECUTE AS](/sql/t-sql/statements/execute-as-clause-transact-sql) для указания контекста выполнения вызванный модуль или использовать [ролей приложения](/sql/relational-databases/security/authentication-access/application-roles) с ограниченными разрешениями. Это гарантирует, что приложение, которое подключается к базе данных имеет наименьшие права доступа, необходимые для приложения. Следуя приведенным ниже рекомендациям также способствует разделению обязанностей.

### <a name="row-level-security"></a>Безопасность на уровне строк

Безопасность на уровне строк позволяет пользователям управлять доступом к строкам в таблице базы данных в зависимости от характеристик пользователя, выполняющего запрос (например, членство в группе или контекст выполнения). Безопасность на уровне строк может также использоваться для реализации концепции безопасности метками. Дополнительные сведения см. в статье [Безопасность на уровне строк](/sql/relational-databases/security/row-level-security).
...
Рекомендуется, рекомендуется в подключение вашего приложения строка вы укажите зашифрованное соединение и _**не**_ доверять сертификату сервера. Это заставляет приложение для проверки сертификата сервера и таким образом предотвращает приложения не подвержена man в среднем типа атак.

Например, при использовании драйвера ADO.NET это достигается с помощью **Encrypt = True** и **TrustServerCertificate = False**. Строка подключения, полученная с портала Azure, будет содержать правильные параметры.
...
### <a name="feature-restrictions"></a>Ограничения компонентов

Функция ограничения не позволяют некоторые виды путем внедрения кода SQL, от утечки сведений о базе данных, даже в том случае, если путем внедрения кода SQL выполнена успешно. Дополнительные сведения см. в разделе [ограничения компонентов для базы данных SQL Azure](sql-database-feature-restrictions.md).
