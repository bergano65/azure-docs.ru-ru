...
- Порт 1433 (исходящий TCP) должен быть открыт, если Издатель/Дистрибьютор находятся на управляемом экземпляре, а абонент - нет. Возможно, также потребуется изменить управляемое правило безопасности NSG `allow_linkedserver_outbound` для **тега службы назначения** port 1433. `virtualnetwork` `internet` 
...
В этой конфигурации один управляемый экземпляр публикует изменения в дистрибуции, размещенные на другом управляемом экземпляре, который может обслуживать многие экземпляры, управляемые исходными данными, и распределять изменения по одной или многим целям в управляемом экземпляре, единой базе данных, объединенной базе данных или сервере S'L.
