...
## <a name="when-to-use-multi-model-capabilities"></a>Когда использовать возможности мультимодели
...
В следующих случаях следует использовать мультимодельные возможности базы данных Azure S'L:
- У вас есть некоторая информация или структуры, которые лучше подходят для моделей НоСЗЛ, и вы не хотите использовать отдельную базу данных НоСЗЛ.
- Большинство ваших данных подходит для реляционной модели, и вам необходимо смоделировать некоторые части данных в стиле НоСЗЛ.
- Для запроса и анализа реляционных данных и данных NoS'L необходимо использовать богатый язык Transact-S'L, а также интегрировать их с различными инструментами и приложениями, использующими язык S'L.
- Необходимо применять такие функции базы данных, как [технологии памяти,](sql-database-in-memory.md) чтобы повысить производительность аналитических или обработки ваших структур данных НоСЗЛ, использовать [транзакционную репликацию](sql-database-managed-instance-transactional-replication.md) или [читаемые реплики](sql-database-read-scale-out.md) для создания копии данных в другом месте и разгрузить некоторые аналитические рабочие нагрузки из основной базы данных.
...
- [Пары ключей](#key-value-pairs) не поддерживаются в качестве специальных функций, так как пары с ключевым значением могут быть по-своей моделируемы в виде таблиц с двумя столбцами.
...
Пространственные данные представляют сведения о физическом расположении и форме геометрических объектов. Эти объекты могут быть точечными или более сложными объектами, такими как страны/регионы, дороги или озера.
...
* [Данные XML](https://docs.microsoft.com/sql/relational-databases/xml/xml-data-sql-server)
