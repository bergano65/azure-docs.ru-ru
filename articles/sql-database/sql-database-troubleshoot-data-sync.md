...
В этой статье описывается, как устранить известные проблемы с azure S'L Data Sync. Если есть решение по проблеме, она предоставляется здесь.
...
- [Сбой синхронизации в uI портала для предварительных баз данных, связанных с агентом клиента](#sync-fails)

- [Моя группа синхронизации застряла в состоянии обработки](#sync-stuck)

- [Я вижу ошибочные данные в таблицах](#sync-baddata)

- [Несогласованные данные первичного ключа после успешной синхронизации](#sync-pkdata)

- [Я вижу значительное ухудшение производительности](#sync-perf)

- [Я вижу это сообщение: "Не может \<вставить значение NULL в столбец>. Колонка не допускает аннулирования". Что это означает, и как я могу это исправить?](#sync-nulls)

- [Как синхронизация данных обрабатывает круговые ссылки? То есть, когда одни и те же данные синхронизируются в нескольких группах синхронизации и постоянно меняются в результате?](#sync-circ)
...
- **Разрешение**. Освободите дополнительное место на диске, на котором находится каталог %TEMP%.
...
- **Разрешение**. Убедитесь, что агент клиента в сети, а затем повторите попытку.

- **Причина**. Агент клиента удален или отсутствует.

- **Разрешение**. Агент клиента удален или отсутствует:
...
- **Разрешение**. Перезапустите службу синхронизации данных SQL.

    1. В меню **«Пуск»** ищите **услуги**.
    1. В результатах поиска выберите **Службы.**
    1. Найдите службу **синхронизации данных s'L.**
    1. Если статус службы **остановлен,** нажмите правой кнопкой мыши имя службы, а затем выберите **Начало.**
...
- **Разрешение**. Убедитесь, что имена таблиц, участвующих в синхронизации, отличаются, даже если они относятся к разным схемам в базе данных.

### <a name="i-see-inconsistent-primary-key-data-after-a-successful-sync"></a><a name="sync-pkdata"></a>Я вижу несовместимые первичные ключевые данные после успешной синхронизации
...
- **Разрешение**. Чтобы предотвратить эту проблему, убедитесь, что данные в столбце первичного ключа не изменены. Чтобы устранить эту проблему после ее возникновения, удалите строку, которая содержит несогласованные данные, из всех конечных точек в группе синхронизации. Затем снова вставьте эту строку.
...
- **Разрешение**. Наилучшим решением является предотвращение. Убедитесь, что в группах синхронизации нет циклических ссылок. Любая строка, синхронизируемая с одной группой синхронизации, не может синхронизироваться c другой группой синхронизации.

### <a name="i-see-this-message-cannot-insert-the-value-null-into-the-column-column-column-does-not-allow-nulls-what-does-this-mean-and-how-can-i-fix-it"></a><a name="sync-nulls"></a>Я вижу это сообщение: "Не может \<вставить значение NULL в столбец>. В столбце запрещены значения NULL". Что это означает, и как это можно исправить? 
...
- [Я не могу удалить группу синхронизации](#setup-delete)

- [Я не могу отменить регистрацию предварительной базы данных S'L Server](#setup-unreg)
...
- [Группа синхронизации имеет статус "вне даты"](#setup-date2)

- [Группа синхронизации не может быть удалена в течение трех минут после установки или остановки агента](#setup-delete2)

- [Что происходит, когда я восстанавливаю потерянную или поврежденную базу данных?](#setup-restore)

### <a name="i-get-a-disk-out-of-space-message"></a><a name="setup-space"></a>Я получаю сообщение "диск из космоса"
...
- **Разрешение**. Вручную удалите файлы синхронизации, которые находятся в папке %temp% (`del \*sync\* /s`). Затем удалите подкаталоги в папке %temp%.
...
- **Разрешение**. Убедитесь, что базы данных SQL и базы данных SQL Server подключены к сети.

- **Причина**. Группа синхронизации подготавливается или синхронизируется.

- **Разрешение**. Дождитесь завершения процесса подготовки или синхронизации и повторите попытку удалить группу синхронизации.

### <a name="i-cant-unregister-an-on-premises-sql-server-database"></a><a name="setup-unreg"></a>Невозможно отменить регистрацию локальной базы данных SQL Server

- **Причина**. Скорее всего, вы пытаетесь отменить регистрацию базы данных, которая уже удалена.

- **Разрешение**. Чтобы отменить регистрацию локальной базы данных SQL Server, выберите эту базу данных и щелкните **Принудительно удалить**.
...
### <a name="i-dont-have-sufficient-privileges-to-start-system-services"></a><a name="setup-perms"></a>У меня нет достаточных привилегий для запуска системных служб
...
- **Разрешение**. Предоставьте учетной записи пользователя учетные данные для входа в качестве службы.

  1. Перейти к **запуску** > **панели** > управления**административных инструментов** > **местной политики безопасности** > **местной политики** > **управления правами пользователей**политики .
...
### <a name="a-database-has-an-out-of-date-status"></a><a name="setup-date"></a>База данных имеет статус "вне даты"
...
- **Разрешение**. Чтобы избежать состояния **Out-of-Date** (Устарела), нужно сделать так, чтобы ни одна из ваших баз данных не находилась вне сети более 45 дней.
...
- **Разрешение**. Чтобы избежать состояния **Out-of-Date** (Устарела) группы синхронизации, регулярно проверяйте результаты заданий синхронизации в средстве просмотра журнала. Изучайте и устраняйте проблемы изменений, которые не применяются.
