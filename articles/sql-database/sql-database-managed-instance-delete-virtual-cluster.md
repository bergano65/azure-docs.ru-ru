...
> [!TIP]
> Если нет управляемых экземпляров, в виртуальный кластер, и вы не сможете удалить виртуальный кластер, убедитесь, что у вас нет при развертывании текущего экземпляра в процессе выполнения. Сюда входят запущен и отмены развертывания, все еще выполняется. Просмотр развертываний группы ресурсов экземпляра была развернута для сообщит любые развертывания в процессе выполнения. В этом случае ожидать для завершения развертывания, удаления управляемого экземпляра, а затем виртуальный кластер.
