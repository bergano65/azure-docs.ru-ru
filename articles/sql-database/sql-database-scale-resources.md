...
База данных SQL Azure позволяет динамически добавлять дополнительные ресурсы для базы данных с минимальными [простоя](https://azure.microsoft.com/support/legal/sla/sql-database/v1_2/); тем не менее, есть параметр за период где потеряно подключение к базе данных на короткий промежуток времени, который может быть устранить, используя логику повторных попыток.
...
- В [модели приобретения на основе DTU](sql-database-service-tiers-dtu.md) набор вычислительных операций, памяти и ресурсов ввода-вывода предоставляется на трех уровнях служб (каждый из которых предусматривает поддержку различных рабочих нагрузок баз данных): "Базовый", "Стандартный" и "Премиум". Для каждого уровня производительности на уровнях обслуживания предусмотрено отдельное сочетание этих ресурсов, к которым можно добавить ресурсы хранилища.
- [Модель приобретения на основе виртуальных ядер](sql-database-service-tiers-vcore.md) позволяет выбрать количество виртуальных ядер, объем памяти и размер и скорость хранилища. Эта модель приобретения предлагает три уровня служб: Общего назначения, критически важный для бизнеса и ИТ.
...
> [!NOTE]
> Динамическое масштабирование отличается от автомасштабирования. Автомасштабирование — когда служба масштабируется автоматически в зависимости от критериев, в то время как динамическая масштабируемость позволяет вручную масштабировать с минимальным временем простоя.
