---
title: 'Учебник: Гео-репликация & неудачу на портале'
description: Нанастройка георепликации для единой или объединенной базы данных в базе данных Azure S'L с помощью портала Azure и инициировать сбой.
...
Сведения о группах автоматической отработки отказа с отдельными базами данных и базами данных в пуле см. в [этом разделе](sql-database-auto-failover-group.md#best-practices-of-using-failover-groups-with-single-databases-and-elastic-pools). Для получения информации об группах автоматического [Best practices of using failover groups with managed-instances](sql-database-auto-failover-group.md#best-practices-of-using-failover-groups-with-managed-instances)отказа с управляемыми инстанциями см.
...
1. На [портале Azure](https://portal.azure.com)просмотрите базу данных, которую необходимо настроить для георепликации.
...
1. На [портале Azure](https://portal.azure.com)просмотрите основную базу данных в партнерстве георепликации.
2. На лезвии базы данных S'L выберите**георепликацию** **всех настроек.** > 
3. В списке **SECONDARIES** выберите базу данных, которая вы хотите стать новой основной, и нажмите **Forced Failover.**
...
Команда немедленно переключит базу данных-получатель на роль базы данных-источника. Этот процесс обычно должен завершиться в течение 30 сек или меньше.
...
1. На [портале Azure](https://portal.azure.com)просмотрите основную базу данных в партнерстве георепликации.
2. На странице базы данных S'L выберите **георепликацию.**
3. В списке **SECONDARIES** выберите базу данных, которая требуется удалить из партнерства георепликации.
...
* Чтобы узнать больше об активной георепликации, [см.](sql-database-active-geo-replication.md)
