...
1. В разделе **Сведения о базе данных** введите или выберите следующие значения: 
   ...
   - **Вычисления и хранилище**. Выберите **Настройка базы данных** и в целях этого краткого руководства укажите уровень служб **Стандартный**, а затем с помощью ползунка выберите **10 DTU (S0)** и **1** ГБ хранилища. Нажмите кнопку **Применить**. 
    ...
    ![Настройка уровня служб](media/sql-database-get-started-portal/create-database-s1.png) 
      ...
      > [!NOTE]
      > В этом кратком руководстве используется [модель приобретения на основе единиц DTU](sql-database-service-tiers-dtu.md). Также доступна [модель приобретения на основе числа виртуальных ядер](sql-database-service-tiers-vcore.md).
      > [!IMPORTANT]
