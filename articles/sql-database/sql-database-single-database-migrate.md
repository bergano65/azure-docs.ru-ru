...
Ниже представлен общий рабочий процесс переноса базы данных SQL Server в отдельную базу данных или базу данных в пуле с помощью этого метода. Перенос в управляемый экземпляр описывается в разделе [Перенос экземпляра SQL Server в Управляемый экземпляр Базы данных SQL Azure](sql-database-managed-instance-migrate.md).
