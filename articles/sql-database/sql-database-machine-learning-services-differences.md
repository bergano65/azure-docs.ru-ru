...
Если вы используете RODBC в базе данных SQL Azure, то нельзя записывать во временную таблицу, является ли она создается в пределах или вне пределов `sp_execute_external_script` сеанса. Решение — использовать [RxOdbcData](https://docs.microsoft.com/machine-learning-server/r-reference/revoscaler/rxodbcdata) и [rxDataStep](https://docs.microsoft.com/machine-learning-server/r-reference/revoscaler/rxdatastep) (с перезаписать = FALSE и добавьте = «строки») для записи в глобальной временной таблицы, созданные до `sp_execute_external_script` запроса.
