...
ms.locfileid: "67839851"
---
# <a name="accelerated-database-recovery"></a>Восстановление ускоренной базы данных

**Ускоренное восстановление базы данных (ADR)** — это новая функция ядра СУБД SQL, которая значительно повышает доступность базы данных, особенно при наличии длительных транзакций, путем изменения процесса восстановления ядра СУБД SQL. Правила автоматического Развертывания в настоящее время доступна для отдельных баз данных и базы данных в составе в базе данных SQL Azure и баз данных в хранилище данных SQL Azure (в настоящее время в общедоступной предварительной версии). Ниже приведены основные преимущества ADR:
...
На высоком уровне правила автоматического Развертывания обеспечивает быстрый базы данных восстановления, управления версиями все изменения физической базы данных и только Отмена логические операции, которые ограничены, могут быть отменены практически мгновенно. Любая транзакция, которая была активна на момент сбоя, помечается как прерванная, и поэтому любые версии, созданные такими транзакциями, могут быть проигнорированы в параллельных запросах пользователей.
