---
title: Ускоренное восстановление базы данных в базе данных SQL Azure
...
ms.locfileid: "73691355"
---
# <a name="accelerated-database-recovery"></a>Ускоренное восстановление базы данных

**Ускоренное восстановление базы данных (ADR)** — это новая функция ядра СУБД SQL, которая значительно повышает доступность базы данных, особенно при наличии длительных транзакций, путем изменения процесса восстановления ядра СУБД SQL. В настоящее время ADR доступен для отдельных баз данных и баз данных в составе пула в базе данных SQL Azure и баз данных в хранилище данных SQL Azure (в настоящее время это общедоступная Предварительная версия). Ниже приведены основные преимущества ADR:
...
Кроме того, ядро СУБД SQL не может усечь журнал транзакций при длительных транзакциях, потому что соответствующие записи журнала необходимы для процессов восстановления и отката. В результате такого конструктивного механизма СУБД SQL некоторые клиенты сталкиваются с проблемой того, что размер журнала транзакций растет очень много и потребляет огромные объемы дискового пространства.
...
- Сделайте восстановление непрерывным или мгновенным, устранив необходимость сканировать журнал с или до начала самой старой активной транзакции. В случае с ADR журнал транзакций обрабатывается только из последней успешной контрольной точки (или самого старого номера LSN, который был изменен в журнале страниц). В результате на время восстановления не влияют длительные транзакции.
...
- **Этап анализа**
  ...
  Процесс остается таким же, как и на сегодняшний день, с добавлением реконструирования sLog и копированием записей журнала для операций без управления версиями.
...
- **Логическая отмена**
  ...
  Логический откат — это асинхронный процесс, ответственный за отмену изменений на уровне строк, предоставляя мгновенный откат и отмену операций с управлением версиями.
