...
description: Сведения о том, как на портале Azure создать сервер Базы данных SQL и базу данных, а затем выполнять запросы к ней.
...
База данных Azure SQL обладает определенным набором [вычислительных ресурсов и ресурсов хранения](sql-database-service-tiers-dtu.md). База данных создается на [сервере Базы данных SQL](sql-database-features.md), который размещается в [группе ресурсов Azure](../azure-resource-manager/resource-group-overview.md).
...
Сохраните эту группу ресурсов, экземпляр SQL Server и Базу данных SQL, чтобы перейти к [дальнейшим действиям](#next-steps). Далее показано, как установить подключение и выполнить запрос к базе данных с помощью разных методов. 
...
- Создайте правило брандмауэра на уровне сервера, чтобы подключаться к Базе данных Azure SQL с помощью локальных или удаленных средств. Подробные сведения см. в разделе [Создание правила брандмауэра на уровне сервера](sql-database-get-started-portal-firewall.md).
