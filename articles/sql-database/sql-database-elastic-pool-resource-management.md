...
> [!IMPORTANT]
> В сжатых пулах с множеством активных баз данных может оказаться невозможным увеличить число баз данных в пуле вплоть до максимума, задокументированного для эластичных пулов [DTU](sql-database-dtu-resource-limits-elastic-pools.md) и [Виртуальное ядро](sql-database-vcore-resource-limits-elastic-pools.md) .
> 
...
Эти представления можно использовать для отслеживания использования ресурсов и устранения конфликтов ресурсов практически в реальном времени. Рабочая нагрузка пользователя на первичную и доступную для чтения вторичные реплики, включая геореплики, `SloSharedPool1` классифицируется в `UserPrimaryGroup.DBId[N]` пул ресурсов и группу `N` рабочей нагрузки, где означает значение идентификатора базы данных.
...
Этот запрос возвращает текущее значение `tempdb_log_used_percent` метрики. Этот запрос может быть выполнен в любой базе данных в эластичном пуле.
