...
События аудита записываются в Log Analytics рабочую область, определенную во время `AzureDiagnostics` настройки аудита, в `SQLSecurityAuditEvents`таблицу с категорией. Дополнительную полезную информацию о языке поиска и командах Log Analytics см. в [документации по поиску Log Analytics](../log-analytics/log-analytics-log-search.md).
...
| connection_id | Недоступно | Идентификатор соединения на сервере | Идентификатор GUID | Недоступно |
...
| succeeded | succeeded_s | Показывает, было ли успешным действие, запустившее событие. Для событий, отличных от имени для входа и пакетной службы, он сообщает только о том, успешно ли выполнена проверка разрешений, а не на выполнение операции. 1 = успешное завершение, 0 = сбой | bit | строка |
