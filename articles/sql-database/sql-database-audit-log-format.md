...
| Имя (BLOB-объект) | Имя (концентраторы событий/Log Analytics) | Description (Описание) | Тип большого двоичного объекта | Тип концентраторов событий и Log Analytics |
|-------------|---------------------------------|-------------|-----------|-------------------------------|
...
| Недоступно | securable_class_type_s | Защищаемый объект, сопоставляемый с class_type, подлежит аудиту | Недоступно | строка |
...
| проблем | session_id_d | Идентификатор сеанса, в котором произошло событие | smallint | int |
| session_server_principal_name | session_server_principal_name_s | Участник сервера для сеанса | имеет sysname | строка |
| баланс | statement_s | Выполненная инструкция T-SQL (при наличии) | nvarchar (4000) | строка |
