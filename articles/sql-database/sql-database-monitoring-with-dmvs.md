---
title: Мониторинг производительности с помощью DMV
...
Эта статья содержит набор запросов DMV, которые можно выполнить с помощью студии управления серверами S'L или Azure Data Studio для обнаружения следующих типов проблем с производительностью запроса:

- [Выявление запросов, связанных с чрезмерным потреблением процессора](#identify-cpu-performance-issues)
- [PAGELATCH_ и WRITE_LOG ожидания, связанные с узкими местами использовались](#identify-io-performance-issues)
- [PAGELATCH_ ожидания, вызванные раздором TempDB](#identify-tempdb-performance-issues)
- [RESOURCE_SEMAHPORE ожидания, вызванные проблемами с ожиданием предоставления памяти](#identify-memory-grant-wait-performance-issues)
- [Определение баз данных и размеров объектов](#calculating-database-and-objects-sizes)
- [Получение информации об активных сеансах](#monitoring-connections)
- [Получение общесистемной информации об использовании ресурсов баз данных и баз данных](#monitor-resource-use)
- [Получение информации о производительности запроса](#monitoring-query-performance)
...
Используйте следующий запрос для идентификации использования ввода-вывода для данных и журналов. Если данные или журнал используют ввод-вывод более чем на 80 %, это означает, что пользователи использовали доступный ввод-вывод для уровня служб базы данных SQL.
...
При определении проблем с производительностью операций ввода-вывода главные типы ожидания, связанные с проблемами `tempdb`, — `PAGELATCH_*` (не `PAGEIOLATCH_*`). Однако ожидания `PAGELATCH_*` не всегда означают состязание `tempdb`.  Этот тип ожидания может также указывать на состязание за страницы данных объекта пользователя из-за одновременных запросов к одной странице данных. Для дальнейшего `tempdb` подтверждения утверждения используйте [sys.dm_exec_requests](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-requests-transact-sql) чтобы `2:x:y` подтвердить, `tempdb` что wait_resource значение `x` начинается с `y` того, где 2 является идентификатором базы данных, является идентификатором файла и является идентификатором страницы.  
...
Судя по этим данным, пиковая нагрузка на процессор составляет чуть более 50 % от максимальной нагрузки для объема вычислительных ресурсов P2 (полдень вторника). Если процессор является доминирующим фактором в профиле ресурсов приложения, то вы можете решить, что P2 является правильным размером вычислений, чтобы гарантировать, что рабочая нагрузка всегда подходит. Если ожидается рост нагрузки на приложение с течением времени, рекомендуется увеличить запас ресурсов, чтобы приложения не достигло предела производительности. Увеличив объем вычислительных ресурсов, можно избежать заметных пользователю ошибок, которые могут возникнуть из-за нехватки в базе данных мощности для эффективной обработки запросов, особенно в средах, чувствительных к задержкам. Это может быть база данных, поддерживающая приложение, создающее веб-страницы на основе запросов к базе данных.
...
1. Чтобы посмотреть на использование ресурсов за прошедшую неделю для базы данных userdb1, можно запустить этот запрос:
...
Чтобы получить представление о частоте входа, можно проанализировать шаблоны работы пользователей и приложений. Кроме того, можно запустить реальные нагрузки в тестовой среде, чтобы убедиться в том, что вы не приближаетесь к этим или другим ограничениям, описанным в этой статье. Не существует ни одного представления запроса или динамического управления (DMV), которое может отображать одновременное количество входа или историю.
...
Опять же, эти запросы возвращают значение счетчика на определенный момент времени. Если со временем вы соберете несколько образцов, вы будете иметь лучшее понимание использования сеанса.
