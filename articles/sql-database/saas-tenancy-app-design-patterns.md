...
- *Мультитенантность.* &nbsp; В каждой базе данных хранятся данные нескольких отдельных клиентов (с механизмами защиты конфиденциальности данных).
...
- **Изоляция клиента.** &nbsp; Изоляция и производительность данных (влияет ли рабочая нагрузка одного клиента на другие).

- **Затраты для каждого клиента.** &nbsp; Затраты на базы данных.
...
Операции управления можно создавать в скриптах и предлагать через модель [devops][http-visual-studio-devops-485m] .  Их даже можно автоматизировать и реализовывать в приложении.
...
| Измерение | Изолированное приложение | Однотенантная база данных | Сегментированная мультитенантная база данных |
| :---------- | :------------- | :------------------ | :------------------- |
| Масштаб | Среднее<br />1–100 | Очень высокая<br />1–100 000 | Без ограничений<br />1–1 000 000 |
...
- [Развертывание и изучение многопользовательского приложения Wingtip, использующего модель SaaS "база данных — клиент" в базе данных SQL Azure][docu-sql-db-saas-tutorial-deploy-wingtip-db-per-tenant-496y]

- [Добро пожаловать в пример приложения SaaS Azure для базы данных SQL в Wingtip Tickets][docu-saas-tenancy-welcome-wingtip-tickets-app-384w]
