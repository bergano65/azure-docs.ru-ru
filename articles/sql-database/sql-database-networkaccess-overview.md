...
Служба экспорта импорта не **работает, позволяя службам Azure получить доступ к серверу,** установленному для OFF. Однако вы можете обойти проблему, [вручную запустив sqlpackage.exe из Azure VM или выполняя экспорт](https://docs.microsoft.com/azure/sql-database/import-export-from-vm) непосредственно в коде, используя API DACFx.

### <a name="data-sync"></a>Синхронизация данных
Чтобы использовать функцию синхронизации данных с **помощью служб Allow Azure для доступа к серверу,** установленного для OFF, необходимо создать отдельные записи правил брандмауэра, чтобы [добавить IP-адреса](sql-database-server-level-firewall-rule.md) из **тега службы Sql** для региона, вмещающего базу данных **концентратора.**
