...
Чтобы использовать функцию синхронизации данных с параметром **Разрешить доступ к службам Azure** со значением **Off**, необходимо создать отдельные записи правил брандмауэра, чтобы [Добавить IP-адреса](sql-database-server-level-firewall-rule.md) из **тега службы SQL** для региона, в котором размещена база данных- **концентратор** .
