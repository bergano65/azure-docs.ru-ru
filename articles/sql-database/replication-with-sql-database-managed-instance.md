...
> [!NOTE]
> В этой статье описывается использование [репликации транзакций](https://docs.microsoft.com/sql/relational-databases/replication/transactional/transactional-replication) в Azure SQL управляемый экземпляр. Он не связан с активной георепликацией или [группами отработки отказа](https://docs.microsoft.com/azure/sql-database/sql-database-auto-failover-group). это функция управляемый экземпляр Azure SQL, которая позволяет создавать полные доступные для чтения реплики отдельных экземпляров.
...
Репликация транзакций доступна в общедоступной предварительной версии [управляемого экземпляра базы данных SQL Azure](sql-database-managed-instance.md). В управляемом экземпляре может размещаться база данных издателя, распространителя и подписчика. Доступные конфигурации см. в разделе [конфигурации репликации транзакций](sql-database-managed-instance-transactional-replication.md#common-configurations) .
...
- Порт 445 (исходящий трафик TCP) открыт в правилах безопасности NSG для доступа управляемых экземпляров к файловому ресурсу Azure.  Если возникла ошибка "не удалось подключиться к \<хранилищу учетных записей хранения Azure> с ошибкой ОС 53", необходимо добавить правило исходящего трафика в NSG соответствующей подсети управляемый экземпляр SQL.
...
Вы можете очистить ресурсы Azure, [удалив ресурсы управляемого экземпляра из группы ресурсов](../azure-resource-manager/management/manage-resources-portal.md#delete-resources) , а затем удалив группу `SQLMI-Repl`ресурсов. 
