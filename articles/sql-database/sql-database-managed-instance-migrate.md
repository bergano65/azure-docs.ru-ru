...
Если вам нужно сравнить эффективность выполнения рабочей нагрузки в управляемом экземпляре с исходной рабочей нагрузки на сервере SQL Server, вам потребовалось бы создать базовых показателей производительности, который будет использоваться для сравнения. Ниже приведены некоторые из параметров, которые необходимы для измерения в экземпляре SQL Server. 
...
- [Мониторинг использования ЦП в экземпляре SQL Server](https://techcommunity.microsoft.com/t5/Azure-SQL-Database/Monitor-CPU-usage-on-SQL-Server/ba-p/680777#M131) и проверьте объем вычислительной мощности, используемой в настоящее время (с помощью динамических административных представлений, SQL Server Management Studio или другие средства мониторинга). Вы можете подготовить управляемый экземпляр, который совпадает с количеством ядер, которые используются на сервере SQL Server, иметь в виду, что может потребоваться масштабироваться в соответствии с характеристиками ЦП [характеристик виртуальной Машины, где установлен экземпляр управляемого](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-resource-limits#hardware-generation-characteristics).
- Проверьте объем доступной памяти на экземпляр SQL Server и выберите [уровень служб, который имеет соответствующий памяти](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-resource-limits#hardware-generation-characteristics). Оно полезно для измерения ожидаемое время существования страницы в экземпляре SQL Server для определения [нужна дополнительная память](https://techcommunity.microsoft.com/t5/Azure-SQL-Database/Do-you-need-more-memory-on-Azure-SQL-Managed-Instance/ba-p/563444).
- Измерение задержки операций ввода-ВЫВОДА файла подсистемы выбирать между уровнями служб общего назначения "и" критически важный для бизнеса.
