...
В этой статье приводятся ответы на часто задаваемые вопросы о клиентах, которые рассматривали базу данных в масштабе служб базы данных SQL Azure, которая обычно называется базой данных для масштабирования. В этой статье описываются сценарии, которые поддерживает уровень службы "Гипермасштабирование", и службы с разными компонентами, совместимые с уровнем "Гипермасштабирование" Базы данных SQL в целом.
...
Уровни служб на основе виртуальное ядро в основном отличаются в зависимости от доступности, типа хранилища и операций ввода-вывода.
...
|**Доступность**|Все|1 реплика, отсутствие возможности масштабирования для чтения, отсутствие локального кэша | Несколько реплик, до 4 чтения, с частичным масштабированием, часть локального кэша | 3 реплики, 1 операция масштабирования для чтения, высокая доступность, избыточная между зонами, полный локальный кэш |
...
Уровень масштабирования базы данных SQL Azure в настоящее время доступен в регионах, перечисленных в разделе [Общие сведения о масштабировании базы данных SQL Azure](sql-database-service-tier-hyperscale.md#regions).
...
- **Свертывание и развертывание**
  ...
  На уровне "Гипермасштабирование" вы также можете подготовить к работе один или несколько дополнительных вычислительных узлов, которые можно использовать для обслуживания запросов на чтение. Это означает, что вы можете использовать эти дополнительные вычислительные узлы как узлы только для чтения, чтобы выгрузить рабочую нагрузку чтения из первичного вычислительного узла. В дополнение к доступу только для чтения, эти узлы также служат в качестве режима горячей замены в случае отработки отказа с первичной реплики.
...
Уровень "Гипермасштабирование" Базы данных SQL поддерживает все рабочие нагрузки SQL Server, но в первую очередь он оптимизирован для OLTP. Вы также можете перенести гибридные рабочие нагрузки (HTAP) и аналитические (киоски данных).
...
Если в настоящее время выполняются интерактивные аналитические запросы с помощью SQL Server в качестве хранилища данных, то масштабирование базы данных SQL является отличным вариантом, так как вы можете размещать относительно небольшие хранилища данных (например, за несколько ТБ до 10 ТБ) по меньшей цене, и вы можете перенести данные арехаусе рабочую нагрузку на масштабирование базы данных SQL без изменения кода T-SQL.
...
Базы данных с масштабированием создаются с одной репликой чтения (итог по двум репликам) по умолчанию. Количество реплик только для чтения можно масштабировать между 0 и 4 с помощью [портал Azure](https://portal.azure.com), [T-SQL](https://docs.microsoft.com/sql/t-sql/statements/alter-database-transact-sql?view=azuresqldb-current), [PowerShell](https://docs.microsoft.com/powershell/module/azurerm.sql/set-azurermsqldatabase) или [CLI](https://docs.microsoft.com/cli/azure/sql/db#az-sql-db-update).
...
В масштабируемых базах данных устойчивость предоставляется на уровне хранилища. Для обеспечения устойчивости требуется только одна реплика. Когда реплика вычисления выходит из строя, новая реплика создается автоматически без потери данных.
...
Да. Имеющиеся базы данных Azure SQL можно переместить на уровень "Гипермасштабирование". Это односторонняя миграция. Невозможно переместить базы данных с уровня служб "Гипермасштабирование" на другой уровень служб. Мы рекомендуем сделать копию ваших рабочих баз данных и выполнить перенос на уровень "Гипермасштабирование" для подтверждения концепции.
...
Да. Некоторые функции базы данных SQL Azure пока не поддерживаются в масштабировании, включая, помимо прочего, резервное копирование долгосрочного хранения. После переноса баз данных на уровень "Гипермасштабирование" эти функции перестают работать.  Мы планируем, чтобы эти ограничения были временными.

### <a name="can-i-move-my-on-premises-sql-server-database-or-my-sql-server-virtual-machine-database-to-hyperscale"></a>Можно ли переместить локальную базу данных SQL Server или мою SQL Server базу данных виртуальной машины в горизонтальную шкалу

Да. Можно использовать все существующие технологии миграции для перехода на масштабирование, включая репликацию транзакций, а также любые другие технологии перемещения данных (массового копирования, фабрики данных Azure, Azure Databricks, SSIS). Дополнительные сведения см. в статье [Что такое Azure Database Migration Service?](../dms/dms-overview.md).
...
Изменение масштаба может занять 100 МБ/с для новых или измененных данных, но время, необходимое для перемещения данных в базы данных SQL Azure, также влияет на доступную пропускную способность сети, скорость чтения исходного кода и целевую цель уровня обслуживания для базы данных масштаба.
...
При использовании базы данных-получателя по умолчанию с первичным и 1 доступным для чтения, соглашение об уровне обслуживания составляет 99,95%.  При наличии большего количества реплик соглашение об уровне обслуживания переходит на 99,99%.  
...
Да. Некоторые функции базы данных SQL Azure не поддерживаются в масштабах, включая, но не ограниченные долгосрочные резервные копии хранения. После переноса баз данных на уровень "Гипермасштабирование" эти функции перестают работать.
...
Мы видели целостность данных об изменениях в 100 МБ/с (создание данных журнала транзакций).
...
Время ожидания операций ввода-вывода в секунду и операций с вводом-выводом зависит от шаблонов рабочей нагрузки.  Если доступ к данным, к которым требуется обращаться, является локальным для кэша вычислений, он будет иметь те же шаблоны ввода-вывода, что и локальный SSD.   
...
### <a name="can-i-provision-multiple-primary-compute-nodes-such-as-a-multi-master-system-where-multiple-primary-compute-heads-can-drive-a-higher-level-of-concurrency"></a>Можно ли подготавливать несколько основных кластерных узлов, таких как многокомпонентная система, где несколько основных процессорных головок могут обеспечить более высокий уровень параллелизма.
...
По умолчанию создаются 2 реплики для баз данных с масштабированием. Если вы хотите изменить количество реплик, это можно сделать с помощью [портал Azure](https://portal.azure.com).
...
### <a name="how-do-i-validate-if-i-have-successfully-connected-to-secondary-compute-node-using-ssms--other-client-tools"></a>Разделы справки проверить, успешно ли выполнено подключение к дополнительному кластерному узлу с помощью SSMS или других клиентских средств?

Вы можете выполнить следующий запрос T-SQL с помощью SSMS или других клиентских средств: `SELECT DATABASEPROPERTYEX ( '<database_name>' , 'updateability' )`.
Результат — `READ_ONLY`, если подключение указывает на вторичный узел, предназначенный только для чтения, или `READ_WRITE`, если подключение указывает на основной узел.
...
Нет. Подключиться к реплике чтения и масштабирования можно только с помощью параметра `ApplicationIntent=ReadOnly`.
...
Нет. Рабочая нагрузка только для чтения перенаправляется на реплику произвольного масштаба чтения.
...
Нет. Вторичные кластерные узлы также используются для высокой доступности, поэтому они должны быть той же конфигурацией, что и база данных-источник, в случае отработки отказа.
...
Нет. @No__t-0 настраивается в зависимости от подготовки размера вычислений, вторичные кластерные узлы имеют тот же размер, что и основное вычисление.
