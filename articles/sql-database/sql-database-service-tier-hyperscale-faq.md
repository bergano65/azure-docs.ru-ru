...
Изменение масштаба может занять 100 МБ/с для новых или измененных данных.
...
Результат заключается `READ_ONLY` в том, что подключение указывает на вторичный узел, предназначенный только для `READ_WRITE` чтения, или если ваше подключение указывает на основной узел.
...
Нет. Подключиться к реплике чтения и масштабирования можно только с помощью `ApplicationIntent=ReadOnly`указания.
...
Нет. `tempdb` Настраивается на основе подготовки размера вычислений, вторичные кластерные узлы имеют тот же размер, что и основное вычисление.
