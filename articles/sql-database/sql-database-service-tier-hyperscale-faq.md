...
- **Свертывание и развертывание**
  ...
  Подготовка каждой из этих дополнительных реплик вычислений может выполняться за постоянное время и является оперативной операцией. Можно подключиться к этим дополнительным репликам вычислений только для чтения, задав для `ApplicationIntent` аргумента в строке подключения значение `ReadOnly`. Все соединения с намерением `ReadOnly` приложения автоматически направляются в одну из дополнительных доступных для чтения реплик.
