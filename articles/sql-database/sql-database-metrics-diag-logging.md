...
Включите ведение журналов метрик и диагностики в Базе данных SQL и в базах данных Управляемого экземпляра; они не включены по умолчанию.

Можно настроить Базы данных SQL Azure и базы данных в Управляемом экземпляре для сбора следующих диагностических данных телеметрии:
...
Меню **Параметры диагностики** для каждой из баз данных на портале Azure используется для настройки потоковой передачи диагностических данных телеметрии для Баз данных SQL Azure и баз данных в Управляемом экземпляре. Можно задать следующие назначения: Служба хранилища Azure, Центры событий Azure и Azure Log Analytics.
...
> [!NOTE]
> Журналы аудита безопасности нельзя включить в настройках диагностики базы данных. Сведения о том, как включить потоковую передачу данных журнала аудита, см. в руководстве по [настройке аудита базы данных](sql-database-auditing.md#subheading-2), а также в статье о [журналах аудита SQL в Azure Log Analytics и Центрах событий Azure](https://blogs.msdn.microsoft.com/sqlsecurity/2018/09/13/sql-audit-logs-in-azure-log-analytics-and-azure-event-hubs/).
> [!TIP]
...
> [!TIP]
> Повторите эти шаги для каждой базы данных в Управляемом экземпляре, который хотите отслеживать.
...
> [!TIP]
> Повторите описанные выше действия для каждого Управляемого экземпляра, который хотите отслеживать.
...
Аналитика SQL Azure — это облачное решение, которое отслеживает производительность баз данных Azure SQL, эластичных пулов и Управляемого экземпляра в нужном масштабе и в нескольких подписках. Решение поможет собирать и визуализировать показатели производительности Базы данных SQL Azure, а также обладает встроенными интеллектуальными возможностями для устранения неполадок производительности.
...
Если вы используете эластичные пулы или Управляемый экземпляр, вам также необходимо настроить потоковую передачу диагностических данных телеметрии из этих ресурсов.
...
Сведения о журналах для Управляемого экземпляра см. в приведенной ниже таблице.
...
## <a name="logs-for-azure-sql-databases-and-managed-instance-databases"></a>Журналы для Баз данных SQL Azure и баз данных Управляемого экземпляра

Сведения о журналах для баз данных SQL Azure и Управляемого экземпляра см. в приведенной ниже таблице.
