...
ms.locfileid: "77544306"
---
# <a name="azure-sql-database-metrics-and-diagnostics-logging"></a>Метрики и журналы диагностики Базы данных SQL Azure

Из этой статьи вы узнаете, как настроить ведение журнала телеметрии диагностики для баз данных SQL Azure с помощью шаблона портал Azure, PowerShell, Azure CLI, Azure Monitor REST API и Azure Resource Manager. Эти диагностические сведения можно использовать для оценки статистики использования ресурсов и выполнения запросов.

Отдельные базы данных, базы данных в составе пула и базы данных управляемых экземпляров могут передавать метрики и журналы диагностики для упрощения мониторинга производительности. Вы можете настроить базу данных SQL Azure для передачи сведений об использовании ресурсов, рабочих ролях и сеансах, а также о подключении к одному из следующих ресурсов Azure:
...
В этой статье содержатся рекомендации по включению телеметрии диагностики для всех баз данных SQL Azure.
...
Включите метрики и ведение журнала диагностики для баз данных SQL. Ведение журнала диагностики не включено по умолчанию. Вы можете настроить базы данных SQL Azure для сбора следующих диагностических данных телеметрии:
...
## <a name="configure-diagnostic-logging-by-using-the-azure-portal"></a>Настройка ведения журнала диагностики с помощью портал Azure

Для настройки потоковой передачи диагностических данных диагностики можно использовать меню " **параметры диагностики** " для каждой отдельной базы или управляемого экземпляра, портал Azure в составе пула. Кроме того, диагностические данные диагностики можно также настроить отдельно для контейнеров базы данных: эластичные пулы и управляемые экземпляры. 
...
Контейнер эластичного пула имеет собственные данные телеметрии отдельно от телеметрии каждой отдельной базы данных.
...
### <a name="configure-streaming-of-diagnostics-telemetry-for-single-databases-and-pooled-database"></a>Настройка потоковой передачи телеметрии диагностики для отдельных баз данных и базы данных в составе пула
...
### <a name="configure-diagnostic-logging-by-using-powershell"></a>Настройка ведения журнала диагностики с помощью PowerShell
...
### <a name="configure-diagnostic-logging-by-using-the-azure-cli"></a>Настройка ведения журнала диагностики с помощью Azure CLI
...
### <a name="configure-diagnostic-logging-by-using-the-rest-api"></a>Настройка ведения журнала диагностики с помощью REST API
