...
Это обусловлено управляемый экземпляр — это контейнер базы данных с помощью свои собственные данные телеметрии, отдельно от телеметрии отдельный экземпляр базы данных.
...
1. Щелкните **Сохранить**.
1. Кроме того настройте потоковую передачу данных телеметрии системы диагностики для каждой базы данных экземпляра в управляемый экземпляр, который вы хотите отслеживать, выполнив шаги, описанные в следующем разделе.
