...
При включении ведения журналов метрик и диагностики нужно указать ресурс Azure для сбора диагностических данных телеметрии. Доступны следующие варианты:
...
> [!NOTE]
> Если вы также используете эластичные пулы или управляемый экземпляр, рекомендуем включить диагностическую телеметрию и для этих ресурсов. Контейнеры баз данных в эластичных пулах и управляемый экземпляр создают собственные диагностические данные телеметрии.

## <a name="enable-logging-for-azure-sql-databases"></a>Включение ведения журнала для баз данных SQL Azure
...
Меню **Параметры диагностики** можно использовать для каждой из отдельных баз данных, баз данных в пуле или баз данных экземпляра на портале Azure, чтобы настроить потоковую передачу диагностических данных телеметрии для баз данных SQL Azure. Можно задать следующие назначения: Служба хранилища Azure, Центры событий Azure и Azure Log Analytics.

### <a name="configure-streaming-of-diagnostics-telemetry-for-single-pooled-or-instance-databases"></a>Настройка потоковой передачи диагностических данных телеметрии для отдельных баз данных, баз данных в пуле или баз данных экземпляра
...
> [!NOTE]
> Журналы аудита безопасности нельзя включить в настройках диагностики базы данных. Сведения о том, как включить потоковую передачу данных журнала аудита, см. в разделе о [настройке аудита базы данных](sql-database-auditing.md#subheading-2), а также в статье о [журналах аудита в Azure Log Analytics и Центрах событий Azure](https://blogs.msdn.microsoft.com/sqlsecurity/2018/09/13/sql-audit-logs-in-azure-log-analytics-and-azure-event-hubs/).
> [!TIP]
> Повторите эти шаги для каждой Базы данных SQL Azure, которую вы хотите отслеживать.

### <a name="configure-streaming-of-diagnostics-telemetry-for-instance-databases-in-managed-instance"></a>Настройка потоковой передачи диагностических данных телеметрии для баз данных экземпляра в управляемом экземпляре

   ![Значок базы данных экземпляра в управляемом экземпляре](./media/sql-database-metrics-diag-logging/icon-mi-database-text.png)

Чтобы включить потоковую передачу диагностических данных телеметрии для баз данных экземпляра в управляемом экземпляре, сделайте следующее:
...
> [!TIP]
> Повторите эти шаги для каждого экземпляра, который нужно отслеживать.

## <a name="enable-logging-for-elastic-pools-or-managed-instances"></a>Включение ведения журналов для эластичных пулов или управляемых экземпляров

Включите диагностическую телеметрию для эластичных пулов и управляемого экземпляра в качестве контейнеров базы данных. У них есть собственная диагностическая телеметрия, которая по умолчанию не включена.
...
1. Введите имя настройки для вашей собственной ссылки.
1. Выберите целевой ресурс для потоковой передачи диагностических данных: **Архивировать в учетной записи хранения**, **Передать в концентратор событий** или **Отправить в Log Analytics**.
1. Для Log Analytics выберите **Настройка** и создайте рабочую область, выбрав **+ Создать рабочую область**, либо выберите существующую рабочую область.
...
> [!TIP]
> Повторите эти шаги для каждого эластичного пула, который вы хотите отслеживать.
...
1. Введите имя настройки для вашей собственной ссылки.
1. Выберите целевой ресурс для потоковой передачи диагностических данных: **Архивировать в учетной записи хранения**, **Передать в концентратор событий** или **Отправить в Log Analytics**.
1. Для Log Analytics выберите **Настройка** и создайте рабочую область, выбрав **+ Создать рабочую область**, либо используйте существующую рабочую область.
...
> [!TIP]
> Повторите описанные выше действия для каждого управляемого экземпляра, который нужно отслеживать.
...
Журналы метрик и диагностики Базы данных SQL можно передать в службу "Аналитика SQL Azure" с помощью встроенной функции **Отправить в Log Analytics** на вкладке настроек диагностики на портале. Log Analytics также можно включить, воспользовавшись параметром диагностики с помощью командлетов PowerShell, Azure CLI или REST API Azure Monitor.
...
## <a name="logs-for-managed-instances"></a>Журналы для управляемых экземпляров

В следующей таблице приведены сведения о журналах для управляемых экземпляров.

### <a name="resource-usage-statistics"></a>Статистика использования ресурсов
...
## <a name="logs-for-single-pooled-and-instance-databases"></a>Журналы для отдельных баз данных, баз данных в пуле или баз данных экземпляра

В следующих таблицах приведены сведения о журналах для отдельных баз данных SQL Azure, баз данных в пуле или баз данных экземпляра.
