---
title: Автоматизация репликации изменений схемы в синхронизации данных S'L
...
> [!IMPORTANT]
> Рекомендуем внимательно прочесть эту статью, особенно разделы о [разрешении неполадок](#troubleshoot) и [дополнительных рекомендациях](#other), прежде чем начать реализовывать репликацию изменений схемы в среде синхронизации. Мы также рекомендуем читать [данные Sync через несколько облачных и предварительных баз данных с помощью синхронизации данных S'L.](sql-database-sync-data.md) Некоторые операции базы данных могут нарушить решение, описанное в этой статье. Для устранения неполадок Transact-SQL и SQL Server могут потребоваться дополнительные знания о домене.
...
## <a name="other-considerations"></a><a name="other"></a> Дополнительные рекомендации
