...
1.  Запросите таблицу отслеживания изменений схемы, чтобы отобразить все команды с идентификатором, значение которого больше значения идентификатора, полученное на предыдущем шаге.
    ...
    1\.  Игнорируйте команды, которые невозможно выполнить в базе данных конечной точки. Необходимо устранить несовместимость схем. Отмените исходные изменения схемы, если несовместимость влияет на приложение.

    2\.  Примените вручную те команды, которые должны быть применены.
