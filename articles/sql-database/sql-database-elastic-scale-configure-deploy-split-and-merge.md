...
2. Создайте сервер Базы данных SQL Azure (или выберите существующий сервер), где будут созданы диспетчер сопоставлений сегментов и сегменты.
   ...
   > [!NOTE]
   > Сценарий *SetupSampleSplitMergeEnvironment. ps1* создает все эти базы данных на том же сервере по умолчанию для упрощения сценария. Это не является ограничением службы разбиения и объединения.
...
3. Выполните сценарий *SetupSampleSplitMergeEnvironment. ps1* , чтобы создать образец среды.
...
5. Выполните сценарий *ексекутесамплесплитмерже. ps1* , чтобы выполнить операцию разбиения (перемещение половины данных по первому сегменту во второй сегмент), а затем операцию Merge (перемещение данных обратно в первый сегмент). Если настроить SSL и оставить конечную точку http отключенной, убедитесь, что можно использовать вместо этого конечную точку https://.
