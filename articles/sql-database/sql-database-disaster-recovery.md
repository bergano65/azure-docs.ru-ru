...
- [Группы автоматического отказа](sql-database-auto-failover-group.md)
...
> [!NOTE]
> База данных-источник и база данных-получатель должны иметь один и тот же уровень служб. Также настоятельно рекомендуется создать вторичную базу данных с тем же размером вычислений (DTUs или vCores) в качестве основной. Для получения дополнительной информации смотрите [Обновление или понижение в качестве основной базы данных.](sql-database-active-geo-replication.md#upgrading-or-downgrading-primary-database)

> [!NOTE]
> Используйте одну или несколько групп failover для управления сбой нескольких баз данных.
> При добавлении существующей связи георепликации к группе отработки отказа убедитесь, что база данных-получатель геореплицируемых данных настроена с тем же уровнем служб и объемом вычислительных ресурсов, что и база данных-источник. Для получения дополнительной информации [см.](sql-database-auto-failover-group.md)
...
- Выполните [аварийное восстановление упражнения](sql-database-disaster-recovery-drills.md). Чтобы сымитировать сбой для географического восстановления, можно удалить или переименовать исходную базу данных, что приведет к сбою подключения приложения. Чтобы сымитировать сбой с помощью групп отработки отказа, отключите веб-приложение или виртуальную машину, подключенную к базе данных, или отработку отказа базы данных, чтобы вызвать сбой подключения приложения.
...
- [Не сбой на вторичном сервере с помощью Transact-S'L (T-S'L)](/sql/t-sql/statements/alter-database-transact-sql?view=azuresqldb-current#e-failover-to-a-geo-replication-secondary)
...
Убедитесь, что все учетные данные, используемые приложением, существуют на сервере с восстановленной базой данных. Для получения дополнительной информации [см. Конфигурацию безопасности для георепликации.](sql-database-geo-replication-security-config.md)
