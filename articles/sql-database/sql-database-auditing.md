...
ms.locfileid: "78184763"
---
# <a name="get-started-with-sql-database-auditing"></a>Приступая к работе с аудитом базы данных SQL

Аудит для [базы данных SQL](sql-database-technical-overview.md) Azure и [Azure синапсе Analytics](../sql-data-warehouse/sql-data-warehouse-overview-what-is.md) отслеживает события базы данных и записывает их в журнал аудита в учетной записи хранения Azure, log Analytics рабочей области или концентраторах событий. Аудит также дает следующие возможности:
...
> [!NOTE] 
> Эта статья относится к Azure SQL Server и к базам данных SQL и базе данных Azure синапсе Analytics, созданным на сервере Azure SQL. Для простоты база данных SQL используется при обращении к базе данных SQL и Azure синапсе.
...
> - Поддерживается хранение данных за пределами виртуальной сети и брандмауэра.
...
## <a id="subheading-2"></a>Настройка аудита для сервера

В следующем разделе описывается настройка аудита на портале Azure.

  > [!NOTE]
   >Теперь у вас есть несколько вариантов настройки места записи журналов аудита. Вы можете записывать журналы в учетную запись хранения Azure, в Log Analytics рабочую область для использования с помощью Azure Monitor журналов или в концентратор событий для использования концентратора событий. Вы также можете настроить любое сочетание этих вариантов, и журналы аудита будут записываться в каждый из них.
...
### <a id="audit-storage-destination">Аудит назначения хранилища</a>

Чтобы настроить запись журналов аудита в учетную запись хранения, выберите **Хранилище** и щелкните **Сведения о хранилище**. Выберите учетную запись хранения Azure, в которой будут храниться журналы, и укажите срок хранения. Нажмите кнопку **ОК**. Журналы старше, чем срок хранения, удаляются.
...
Чтобы настроить учетную запись хранения в виртуальной сети или брандмауэре, вам потребуется [Active Directory администратор](https://docs.microsoft.com/azure/sql-database/sql-database-aad-authentication-configure?tabs=azure-powershell#provision-an-azure-active-directory-administrator-for-your-managed-instance) на сервере. Включите параметр **разрешить доверенным службам Майкрософт доступ к этой учетной записи** хранения в учетной записи хранения. Кроме того, необходимо иметь разрешение Microsoft. Authorization/roleAssignments/Write для выбранной учетной записи хранения.

Рекомендуется быть [администратором доступа пользователей](../role-based-access-control/built-in-roles.md#user-access-administrator) , чтобы предоставить управляемому удостоверению роль "участник данных BLOB-объекта хранилища". Дополнительные сведения о разрешениях и управлении доступом на основе ролей см [. в статье что такое управление доступом на основе ролей (RBAC) для ресурсов Azure?](../role-based-access-control/overview.md) и [Добавление или удаление назначений ролей с помощью Azure RBAC и портал Azure](../role-based-access-control/role-assignments-portal.md)

### <a id="audit-log-analytics-destination">Аудит для Log Analytics назначения</a>
...
### <a id="audit-event-hub-destination">Аудит назначения концентратора событий</a>
...
> [!WARNING]
> Включение аудита на сервере, на котором имеется пул SQL, **приведет к возобновлению работы пула SQL и повторной приостановке** , что может повлечь за собой оплату счетов.
< < < < < < < HEAD = = = = = = = чтобы настроить запись журналов аудита в концентратор событий, выберите **концентратор событий (Предварительная версия)** и откройте **сведения о концентраторе событий**. Выберите концентратор событий, в который необходимо записывать журналы, а затем нажмите кнопку **ОК**. Убедитесь, что концентратор событий находится в том же регионе, что база данных и сервер.
...
- Настроить события аудита можно с помощью [командлетов PowerShell](#subheading-7) или [REST API](#subheading-9).
...
- Сведения о настройке неизменяемого аудита учетной записи хранения см. в разделе [разрешение записи BLOB-объектов с защищенным добавлением](../storage/blobs/storage-blob-immutable-storage.md#allow-protected-append-blobs-writes). Обратите внимание, что имя контейнера для аудита — **sqldbauditlogs**.
