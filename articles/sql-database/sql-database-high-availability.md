...
- Уровень вычислений без отслеживания состояния, который запускает процесс `sqlserver.exe` и содержит только временные и кэшированные данные на подключенном SSD, такие как TempDB, база данных model, кэш планов, буферный пул и пул хранилища столбцов. Этот узел работает в Azure Service Fabric, который инициализирует `sqlserver.exe`, управляет работоспособностью узла и при необходимости выполняет отработку отказа на другой узел.
