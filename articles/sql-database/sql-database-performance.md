...
| :----------------------------- | ----- | ----- |
| **Обзор производительности** — мониторинг всех действий производительности для базы данных. | Yes | Нет | 
| **Рекомендации по производительности** . здесь показаны рекомендации по производительности, которые могут повысить производительность рабочей нагрузки. | Yes | Нет | 
| **Анализ производительности запросов** — показывает производительность наиболее ресурсоемких запросов к базе данных. | Yes | Нет | 
| **Автоматическая настройка** . Используйте базу данных SQL Azure для автоматической оптимизации производительности базы данных. | Yes | Нет | 
...
* Возможность получить подробные сведения о запросе. 
  ...
  ![Анализ производительности запросов для базы данных SQL Azure](./media/sql-database-performance/query-performance-insights-annotated.png)
...
Базы данных SQL Azure могут автоматически настраивать производительность, применяя [рекомендации по производительности](sql-database-advisor.md). Кроме того, вы можете узнать, [как включить автоматическую настройку](sql-database-automatic-tuning-enable.md).

  ![Автоматическая настройка базы данных SQL Azure](./media/sql-database-performance/automatic-tuning-annotated.png)

Чтобы узнать больше, прочитайте статью [Автоматическая настройка](sql-database-automatic-tuning.md).
