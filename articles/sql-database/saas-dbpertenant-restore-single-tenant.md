---
title: Восстановление базы данных в многоклиентское приложение SaaS
...
* Разверните приложение SaaS Wingtip. Сведения о развертывании менее чем за пять минут см. в статье [развертывание и изучение приложения SaaS Wingtip](saas-dbpertenant-get-started-deploy.md).
...
Если вы используете базу данных с [активной георепликацией](sql-database-active-geo-replication.md), мы рекомендуем выполнить параллельное восстановление и скопировать необходимые данные из восстановленной копии в исходную базу данных. После замены исходной базы данных восстановленной необходимо перенастроить и повторно синхронизировать георепликацию.
...
1. Откройте концентратор событий (http://events.wtp.&ltuser&gt;. Trafficmanager.NET) и выберите **contoso концерт зал**.
...
1. В интегрированной среде сценариев PowerShell последовательно откройте …\\Learning Modules\\Business Continuity and Disaster Recovery\\RestoreTenant\\*Demo-RestoreTenant.ps1* и задайте следующее значение:
   ...
   * **$DemoScenario** = **1**, *Удалить Последнее событие (без продаж билетов)*.
...
3. Задайте **$DemoScenario** = **2**, *одновременно восстановите клиент*.
...
1. Установите **$DemoScenario** = **4**, *удалите восстановленный клиент*.
...
2. Установите **$DemoScenario** = **5**, *восстановите клиент на месте*.
