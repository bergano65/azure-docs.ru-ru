...
### <a name="database-mail-db_mail"></a>Database Mail (db_mail)
 - `sp_send_dbmail`не удается отправить вложения @file_attachments с помощью параметра. Локальная файловая система и общие папки екстертал или хранилище BLOB-объектов Azure недоступны для этой процедуры.
...
- Резервные копии, содержащие базы данных размером больше 8 ТБ, активные объекты OLTP в памяти, или количество файлов, размер которых превышает 280 файлов на экземпляр, не могут быть восстановлены на экземпляре общего назначения. 
