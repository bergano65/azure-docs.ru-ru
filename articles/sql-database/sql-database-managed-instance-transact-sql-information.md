---
title: Различия T-SQL для управляемого экземпляра базы данных SQL Azure
...
- [MEMORY_OPTIMIZED](/sql/relational-databases/in-memory-oltp/introduction-to-memory-optimized-tables) (не поддерживается на уровне общего назначения)
...
- `DATASOURCE` требуется в команде `BULK INSERT` при импорте файлов из хранилища больших двоичных объектов Azure. См. статью [BULK INSERT (Transact-SQL)](/sql/t-sql/statements/bulk-insert-transact-sql).
...
- `ASYNC RESTORE`: восстановление продолжится, даже если соединение с клиентом будет прервано. Если подключение разорвано, можно проверить состояние операции восстановления в представлении `sys.dm_operation_status`, а также для создания и удаления базы данных. См. статью [sys.dm_operation_status (база данных SQL Azure)](/sql/relational-databases/system-dynamic-management-views/sys-dm-operation-status-azure-sql-database). 
