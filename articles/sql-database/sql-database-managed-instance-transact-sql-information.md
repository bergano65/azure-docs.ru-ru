...
### <a name="database-mail-db_mail"></a>Database Mail (db_mail)
 - `sp_send_dbmail`не удается отправить вложения @file_attachments с помощью параметра. Локальная файловая система, а также экстенты или хранилище BLOB-объектов Azure недоступны для этой процедуры.
