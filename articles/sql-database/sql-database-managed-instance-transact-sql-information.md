...
- Большинство параметров общего `WITH` поддерживаются.
...
- С помощью параметра развертывания управляемого экземпляра в базе данных SQL Azure аудит работает на уровне сервера. Файлы журнала `.xel` хранятся в хранилище больших двоичных объектов Azure.
...
- Файл `CREATE FROM` @ no__t-1 @ no__t-2 не поддерживается для сертификатов.
- Сертификат `CREATE` @ no__t-1 @ no__t-2 из `FILE` @ no__t-4 @ no__t-5 не поддерживается. Невозможно использовать файлы закрытых ключей. 
...
- Ограничения общедоступной предварительной версии для участников сервера Azure AD (имена входа):
  ...
  - Ограничения администратора Active Directory для управляемого экземпляра:
    ...
    - Администратор Azure AD, используемый для настройки управляемого экземпляра, не может быть использован для создания участника-службы (имени входа) Azure AD в пределах управляемого экземпляра. Необходимо создать первый сервер-участник Azure AD (имя входа) с помощью учетной записи SQL Server, которая является ролью `sysadmin`. Это временное ограничение будет удалено после того, как участники сервера Azure AD (имена входа) станут общедоступными. При попытке использовать учетную запись администратора Azure AD для создания имени входа отображается следующая ошибка: `Msg 15247, Level 16, State 1, Line 1 User does not have permission to perform this action.`.
      - В настоящее время первое имя входа Azure AD, созданное в базе данных master, должно быть создано стандартной учетной записью SQL Server (отличной от Azure AD), которая является ролью `sysadmin` с помощью инструкции [CREATE LOGIN](/sql/t-sql/statements/create-login-transact-sql?view=azuresqldb-mi-current) от внешнего поставщика. После выхода общедоступной группы это ограничение будет удалено. Затем можно создать начальное имя входа Azure AD с помощью Active Directory администратора для управляемого экземпляра.
...
- База данных не может содержать файловые группы, содержащие данные FILESTREAM. Восстановление завершается сбоем, если bak содержит данные `FILESTREAM`. 
...
- Невозможно указать путь к файлу в инструкции `ALTER DATABASE ADD FILE (FILENAME='path')` T-SQL. Удалите `FILENAME` из сценария, так как управляемый экземпляр автоматически помещает файлы. 
...
- `DATASOURCE` требуется в команде `BULK INSERT` при импорте файлов из хранилища больших двоичных объектов Azure. См. статью [BULK INSERT (Transact-SQL)](https://docs.microsoft.com/sql/t-sql/statements/bulk-insert-transact-sql).
- `DATASOURCE` требуется в функции `OPENROWSET` при чтении содержимого файла из хранилища BLOB-объектов Azure. См. статью [OPENROWSET (Transact-SQL)](https://docs.microsoft.com/sql/t-sql/functions/openrowset-transact-sql).
...
### <a name="database-mail-db_mail"></a>Database Mail (db_mail)
 ...
 - См. известные проблемы, связанные с параметром `@query` и проверкой подлинности.
 
### <a name="dbcc"></a>DBCC

Недокументированные инструкции DBCC, включенные в SQL Server, не поддерживаются в управляемых экземплярах.

- Поддерживается только ограниченное число глобальных флагов трассировки. Уровень сеанса `Trace flags` не поддерживается. См. раздел [Флаги трассировки](https://docs.microsoft.com/sql/t-sql/database-console-commands/dbcc-traceon-trace-flags-transact-sql).
...
- Целевой объект `etw_classic_sync` не поддерживается. Храните в хранилище BLOB-объектов Azure файлы @no__t – 0. См. раздел [Целевой объект etw_classic_sync_target](https://docs.microsoft.com/sql/relational-databases/extended-events/targets-for-extended-events-in-sql-server#etw_classic_sync_target-target).
- Целевой объект `event_file` не поддерживается. Храните в хранилище BLOB-объектов Azure файлы @no__t – 0. См. раздел [Целевой объект event_file](https://docs.microsoft.com/sql/relational-databases/extended-events/targets-for-extended-events-in-sql-server#event_file-target).
...
- Функцию `OPENROWSET` можно использовать для выполнения запросов только в экземплярах SQL Server. Они могут быть как управляемыми, так и локальными, так и виртуальными машинами. См. статью [OPENROWSET (Transact-SQL)](https://docs.microsoft.com/sql/t-sql/functions/openrowset-transact-sql).
...
- Источник: 
  - единственным поддерживаемым вариантом является `FROM URL` (хранилище больших двоичных объектов Azure).
...
- @no__t — 0, если брокер не включен в BAK-файле. 
- @no__t — 0, если брокер не включен в BAK-файле. 
- `AUTO_CLOSE=OFF`, если база данных в BAK-файле имеет @no__t – 1. 
- `RECOVERY FULL`, если база данных в BAK-файле имеет режим восстановления @no__t 1 или `BULK_LOGGED`.
...
- Резервные копии поврежденных баз данных могут быть восстановлены в зависимости от типа повреждения, но автоматические резервные копии не будут выполняться до устранения повреждений. Убедитесь, что на исходном экземпляре выполняется `DBCC CHECKDB`, и для предотвращения этой проблемы используйте резервное копирование `WITH CHECKSUM`.
- Восстановление файла `.BAK` базы данных, содержащего все ограничения, описанные в этом документе (например, `FILESTREAM` или `FILETABLE`), невозможно восстановить на Управляемый экземпляр.
- не удается восстановить файлы `.BAK`, содержащие несколько резервных наборов данных. 
- не удается восстановить файлы `.BAK`, содержащие несколько файлов журнала.
...
- `Extended stored procedures` не поддерживается, что включает `sp_addextendedproc` @ no__t-2and `sp_dropextendedproc`. См. раздел [Расширенные хранимые процедуры](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/general-extended-stored-procedures-transact-sql).
...
- `SERVERPROPERTY('InstanceName')` возвращает значение NULL, так как понятие экземпляра в том виде, в котором оно существует для SQL Server, не применяется к управляемому экземпляру. См. статью [SERVERPROPERTY (Transact-SQL)](https://docs.microsoft.com/sql/t-sql/functions/serverproperty-transact-sql).
- `@@SERVERNAME` возвращает полное имя "Connected" DNS, например my-managed-instance.wcus17662feb9ce98.database.windows.net. См. статью [@SERVERNAME](https://docs.microsoft.com/sql/t-sql/functions/servername-transact-sql). 
...
- `@@SERVICENAME` возвращает значение NULL, так как концепция службы в том виде, в котором она существует для SQL Server, не применяется к управляемому экземпляру. См. статью [@SERVICENAME](https://docs.microsoft.com/sql/t-sql/functions/servicename-transact-sql).
