...
- Резервные копии, содержащие базы данных размером больше 8 ТБ, активные объекты OLTP в памяти или более 280 файлов, нельзя восстановить в экземпляре общего назначения. 
- Резервные копии, содержащие базы данных большего размера, чем 4 ТБ или объекты OLTP в памяти с общим размером, превышающим размер, описанный в разделе [ограничения ресурсов](sql-database-managed-instance-resource-limits.md) , не могут быть восстановлены в экземпляре критически важный для бизнеса.
...
Максимальный размер `tempdb` файла не может быть больше 24 ГБ на ядро на уровне общего назначения. Максимальный `tempdb` размер критически важный для бизнеса уровня ограничен размером хранилища экземпляра. `Tempdb`Размер файла журнала ограничен 120 ГБ на уровнях общего назначения и критически важный для бизнеса. `tempdb` База данных всегда разделяется на 12 файлов данных. Этот максимальный размер для каждого файла нельзя изменить, и новые файлы нельзя добавить в `tempdb`. Некоторые запросы могут возвращать ошибку, если им требуется более 24 ГБ на ядро в `tempdb` или если они создают более 120 ГБ данных журнала. `Tempdb`всегда создается повторно как пустая база данных при запуске или отработки отказа экземпляра, а любые изменения, `tempdb` внесенные в, не сохраняются. 

### <a name="cant-restore-contained-database"></a>Не удается восстановить автономную базу данных

Управляемый экземпляр не может восстановить [автономные базы данных](https://docs.microsoft.com/sql/relational-databases/databases/contained-databases). Восстановление на момент времени существующих автономных баз данных не работает на управляемом экземпляре. Тем временем мы рекомендуем удалить параметр включения из баз данных, размещенных на управляемом экземпляре. Не используйте параметр включения для рабочих баз данных. 
...
Например, у управляемого экземпляра общего назначения может быть один большой файл размером 1,2 ТБ, размещенный на диске объемом 4 ТБ. У него также может быть 248 файлов размером 1 ГБ, каждый из которых размещается на отдельных дисках размером 128 ГБ. В данном примере:
...
SQL Server Management Studio и SQL Server Data Tools могут столкнуться с некоторыми проблемами при доступе к управляемому экземпляру.
...
### <a name="database-mail-profile"></a>Профиль Database Mail

Необходимо вызвать `AzureManagedInstance_dbmail_profile`профиль Database Mail, используемый агент SQL Server. Для других имен профилей Database Mail ограничений не существует.
...
### <a name="tde-encrypted-databases-with-a-service-managed-key-dont-support-user-initiated-backups"></a>Зашифрованные базы данных TDE с ключом, управляемым службой, не поддерживают резервное копирование, инициированное пользователем.

Невозможно выполнить `BACKUP DATABASE ... WITH COPY_ONLY` в базе данных, которая зашифрована с помощью прозрачное шифрование данных, управляемого службой (TDE). Управляемые службой TDE принудительное шифрование резервных копий с помощью внутреннего ключа TDE. Ключ не может быть экспортирован, поэтому восстановить резервную копию невозможно.

**Решение:** Используйте автоматическое резервное копирование и восстановление на момент времени или используйте [управляемую клиентом (BYOK) TDE](https://docs.microsoft.com/azure/sql-database/transparent-data-encryption-azure-sql#customer-managed-transparent-data-encryption---bring-your-own-key) . Также можно отключить шифрование базы данных.
