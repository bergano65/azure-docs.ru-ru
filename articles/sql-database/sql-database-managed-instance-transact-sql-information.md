...
- Поддерживается уведомление по электронной почте. Необходимо настроить профиль компонента Database Mail. Агент SQL можно использовать профиль электронной почты только одну базу данных, и ему должно быть присвоено `AzureManagedInstance_dbmail_profile`.  
...
- Нельзя восстановить резервные копии, содержащий базы данных с активной в памяти объекты экземпляра общего назначения.  
...
Каждый управляемый экземпляр общие цели работало до 35 ТБ хранилища, зарезервированный для Azure уровня "премиум" на диске, а каждый файл базы данных размещается на отдельном физическом диске. Поддерживаются диски размером 128 ГБ, 256 ГБ, 512 ГБ, 1 ТБ или 4 ТБ. Неиспользуемое пространство на диске не оплачивается, но суммарный размер дисков Azure уровня "Премиум" не может превышать 35 ТБ. В некоторых случаях из-за внутренней фрагментации размер Управляемого экземпляра, которому требуется не более 8 ТБ дискового пространства, может превысить ограничение в 35 ТБ.

Например, общего назначения управляемого экземпляра может иметь одну файле 1,2 ТБ в размер, который помещается в диск емкостью 4 ТБ и 248 файлов (каждого 1 ГБ), которые размещаются на разных дисках 128 ГБ. В данном примере:
...
`@query` параметр в [sp_send_db_mail](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-send-dbmail-transact-sql) процедуры не работают.

### <a name="database-mail-profile"></a>Профиль компонента Database Mail

Профиль компонента database mail агент SQL должен вызываться `AzureManagedInstance_dbmail_profile`. Существует никаких ограничений относительно других имен профиль электронной почты базы данных.
