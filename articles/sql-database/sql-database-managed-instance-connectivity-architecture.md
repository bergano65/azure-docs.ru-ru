...
> [!NOTE]
> Трафик, идущий к службам Azure, которые находятся внутри области управляемый экземпляр оптимизирована, и по этой причине не преобразованный через NAT для управляемого экземпляра конечной точки общедоступного IP-адрес управления. По этой причине, если необходимо использовать правила брандмауэра на основе IP, чаще всего для хранилища, служба должна находиться в разных регионах с управляемым экземпляром.
...
- **Выделенная подсеть:** Подсети управляемого экземпляра не может содержать другие облачные службы, связанной с ним, и не может быть подсеть шлюза. Подсети не может содержать любой ресурс, но управляемый экземпляр и более поздней версии, невозможно добавить другие типы ресурсов в подсети.
...
| ИМЯ       |Port                        |Протокол|`Source`           |Место назначения|Действие|
|------------|----------------------------|--------|-----------------|-----------|------|
|управление  |9000, 9003, 1438, 1440, 1452|TCP     |Все              |MI SUBNET  |Allow |
|mi_subnet   |Все                         |Все     |MI SUBNET        |MI SUBNET  |Allow |
|health_probe|Все                         |Все     |AzureLoadBalancer|MI SUBNET  |Allow |

### <a name="mandatory-outbound-security-rules"></a>Обязательные правила безопасности для исходящего трафика

| ИМЯ       |Port          |Протокол|`Source`           |Место назначения|Действие|
|------------|--------------|--------|-----------------|-----------|------|
|управление  |80, 443, 12000|TCP     |MI SUBNET        |AzureCloud; |Allow |
|mi_subnet   |Все           |Все     |MI SUBNET        |MI SUBNET  |Allow |
