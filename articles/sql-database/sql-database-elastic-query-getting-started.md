---
title: Отчет по масштабируемым облачным базам данных
...
1. Создайте и запустите Запуск с помощью примера **приложения инструментов Elastic Database,** выстраиваясь в разделе статьи [Download and run the sample app.](sql-database-elastic-scale-get-started.md#download-and-run-the-sample-app-1) Как только вы закончите все шаги, вы увидите следующий запрос команды:
...
2. Подключитесь к базе данных ElasticDBQuery и выполните следующие команды T-SQL:
    ...
    "Имя пользователя" и "пароль" должны быть такими же, как логин информация, используемая в шаге 3 раздела [Скачать и запустить образец приложения](sql-database-elastic-scale-get-started.md#download-and-run-the-sample-app) в **Начало начал с эластичной базы данных инструменты** статьи.
...
4. В **мастере подключения к данным** введите имя сервера и учетные данные для входа. Нажмите **Далее**.
5. В диалоговом окне **Выберите базу данных, содержащую нужные сведения** выберите базу данных **ElasticDBQuery**.
6. Выберите таблицу **Клиенты** в представлении списка и нажмите кнопку **Далее**. Затем нажмите кнопку **Готово**.
