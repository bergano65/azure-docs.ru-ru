---
title: Экспорт одной или объединенной базы данных в файл BACPAC
...
- Для того чтобы экспорт был транзакционным, необходимо либо убедиться, что во время экспорта не происходит активности записи, либо экспортируется из [согласованной транзакционно копии](sql-database-copy.md) базы данных Azure S'L.
...
- Хранение за брандмауэром в настоящее время не поддерживается.
...
2. Введите имя BACPAC-файла, выберите учетную запись хранения Azure и контейнер для экспорта, а также соответствующие учетные данные для доступа к базе данных-источнику. Для входа администратора **администратора** сервера S'L здесь требуется даже если вы являетесь администратором Azure, так как наличие администратора Azure не приравнивается к разрешению администратора администратора сервера S'L Server.
...
Для отправки запроса на экспортную базу данных в службу базы данных Azure S'L воспользуйтесь смдlet [New-AzSqlDatabaseExport.](/powershell/module/az.sql/new-azsqldatabaseexport) Операция экспорта может занять некоторое время в зависимости от размера базы данных.
...
Чтобы проверить состояние экспортного запроса, используйте [Get-AzSqlDatabaseImportExportStatus](/powershell/module/az.sql/get-azsqldatabaseimportexportstatus) cmdlet. Если выполнить его немедленно после запроса, то обычно возвращается сообщение **Состояние: выполняется**. Отображение сообщения **Состояние: выполнен** означает, что экспорт завершен.
