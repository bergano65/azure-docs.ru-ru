---
title: Преемственность облачного бизнеса - восстановление базы данных
...
- Повреждение или удаление данных, которое, как правило, возникает из-за ошибки приложения или пользователя. Такие сбои специфичны для приложений и, как правило, не могут быть обнаружены службой базы данных.
...
- Ошибки обновления или обслуживания, непредвиденные проблемы, возникающие при плановом обслуживании или обновлении инфраструктуры, могут потребовать быстрого отката до состояния предварительной базы данных.

Для смягчения локальных сбоев в аппаратном и программном обеспечении база данных s'L содержит [архитектуру высокой доступности,](sql-database-high-availability.md)которая гарантирует автоматическое восстановление после этих сбоев с доступностью до 99,995% SLA.  

Для защиты вашего бизнеса от потери данных база данных автоматически создает полные резервные копии базы данных еженедельно, резервные копии дифференциальных баз данных каждые 12 часов, а резервные копии журналов транзакций — каждые 5 - 10 минут. Резервные копии хранятся в хранилище RA-GRS не менее 7 дней для всех уровней обслуживания. Все уровни обслуживания, за исключением базовой поддержки настраиваемый период хранения резервного копирования для точки в срок восстановления, до 35 дней. 

База данных S'L также предоставляет несколько функций непрерывности бизнеса, которые можно использовать для смягчения различных незапланированных сценариев. 

- [Временные таблицы](sql-database-temporal-tables.md) позволяют восстанавливать версии строк из любой точки во времени.
- [Встроенные автоматизированные резервные копии](sql-database-automated-backups.md) и [восстановление Point in Time](sql-database-recovery-using-backups.md#point-in-time-restore) позволяют восстановить полную базу данных в определенный момент времени в течение настроенного периода хранения до 35 дней.
...
- [Активная георепликация](sql-database-active-geo-replication.md) позволяет создавать читаемые реплики и вручную переходить к любым репликам в случае сбоя центра обработки данных или обновления приложения.
...
Автоматическое резервное копирование базы данных можно использовать для восстановления базы данных в уотную точку в прошлом. Таким образом, вы можете восстановить сядеев из-за повреждения данных, вызванного человеческими ошибками. Восстановление point-in-time позволяет создать новую базу данных на том же сервере, который представляет состояние данных до повреждения события. Для большинства баз данных операции восстановления занимают менее 12 часов. Восстановление очень большой или очень активной базы данных может занять больше времени. Дополнительные сведения см. в разделе [Время восстановления](sql-database-recovery-using-backups.md#recovery-time). 

Если максимальный период удержания резервного копирования для восстановления момента в времени (PITR) недостаточен для вашего приложения, его можно продлить, настроив политику долгосрочного удержания (LTR) для базы данных(ы). Дополнительные сведения см. в разделе [Долгосрочное хранение резервных копий](sql-database-long-term-retention.md).

## <a name="compare-geo-replication-with-failover-groups"></a>Сравните георепликацию с группами неудач

[Группы автоматического отказа](sql-database-auto-failover-group.md#auto-failover-group-terminology-and-capabilities) упрощают развертывание и использование [георепликации](sql-database-active-geo-replication.md) и добавляют дополнительные возможности, описанные в следующей таблице:

|                                              | Георепликация | Группы отработки отказа  |
|:---------------------------------------------| :-------------- | :----------------|
| автоматический переход на другой ресурс                           |     нет          |      Да         |
| Сбой в нескольких базах данных одновременно  |     нет          |      Да         |
| Пользователь должен обновить строку соединения после сбоя      |     Да         |      нет          |
| Управляемый экземпляр поддерживается                   |     нет          |      Да         |
| Может быть в том же регионе, что и первичный             |     Да         |      нет          |
| Несколько реплик                            |     Да         |      нет          |
| Поддержка считываемого масштаба                          |     Да         |      Да         |
| &nbsp; | &nbsp; | &nbsp; |
...
- Наконец, вы можете быстро восстановиться после сбоя, если вы настроили либо геовторичные с помощью [активной георепликации,](sql-database-active-geo-replication.md) либо [группу автоматического отказа](sql-database-auto-failover-group.md) для базы данных или баз данных. В зависимости от выбора технологии можно использовать переход на другой ресурс автоматически или вручную. Хотя отработка отказа занимает всего несколько секунд, службе понадобится как минимум 1 час, чтобы активировать ее. Нужно удостовериться, что отработка отказа соответствует масштабу сбоя. Кроме того, при отработке отказа возможна небольшая потеря данных из-за характера асинхронной репликации. 

При разработке плана обеспечения непрерывности бизнес-процессов нужно понимать, какое максимальное время до полного восстановления приложения после аварийного события допустимо. Время, необходимое для полного восстановления приложения, известно как цель времени восстановления (RTO). Вы также должны понимать максимальный период последних обновлений данных (временной интервал) приложение может терпеть потери при восстановлении от незапланированного события. Потенциальная потеря данных известна как цель точки восстановления (RPO).

Различные методы восстановления предлагают различные уровни RPO и RTO. Вы можете выбрать конкретный метод восстановления или использовать комбинацию методов для достижения полного восстановления приложения. В следующей таблице сравниваются RPO и RTO каждого варианта восстановления. Группы автоматического отказа упрощают развертывание и использование георепликации и добавляют дополнительные возможности, описанные в следующей таблице.
...
| Ручная база данных сбой | 30 с | 5 с |

> [!NOTE]
> *Ручная база данных failover* относится к сбою одной базы данных к ее гео-реплицированных вторичных с помощью [незапланированного режима.](sql-database-active-geo-replication.md#active-geo-replication-terminology-and-capabilities)
...
Вы можете использовать комбинацию резервных копий базы данных и активной георепликации в зависимости от требований приложения. Для обсуждения соображений проектирования автономных баз данных и эластичных пулов, использующих эти [Elastic pool disaster recovery strategies](sql-database-disaster-recovery-strategies-for-applications-with-elastic-pool.md)функции непрерывности бизнеса, [см.](sql-database-designing-cloud-solutions-for-disaster-recovery.md)

В следующих разделах описываются инструкции по восстановлению с использованием резервных копий базы данных или активной георепликации. Для получения подробных шагов, включая требования к планированию, шаги восстановления после публикации и информацию о том, как смоделировать сбой для выполнения аварийного сверла, [см.](sql-database-disaster-recovery.md)
...
Для обсуждения соображений проектирования приложений для автономных баз данных и эластичных [Elastic pool disaster recovery strategies](sql-database-disaster-recovery-strategies-for-applications-with-elastic-pool.md)пулов [см.](sql-database-designing-cloud-solutions-for-disaster-recovery.md)
