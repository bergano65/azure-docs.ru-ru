...
- Повреждение или удаление данных, которое, как правило, возникает из-за ошибки приложения или пользователя. Такие сбои зависят от приложения и обычно не определяются службой базы данных.
...
- Обновление или обслуживание ошибок, непредвиденных проблем, возникающих во время планового обслуживания инфраструктуры или обновления может потребоваться Быстрая отката в состояние предыдущей базы данных.

Во избежание локальное оборудование или программным обеспечением, база данных SQL включает [архитектура высокого уровня доступности](sql-database-high-availability.md), что гарантирует автоматическое восстановление после этих сбоев с до % 99.995 соглашением об уровне обслуживания.  

Для защиты вашей компании от потери данных, база данных SQL автоматически создает полные резервные копии базы данных раз в неделю, разностные резервные копии каждые 12 часов и резервную копию журнала транзакций каждые 5 – 10 минут. Резервные копии хранятся в хранилище RA-GRS для менее 7 дней для всех уровней обслуживания. Все уровни служб, за исключением Basic поддерживает заданного срока хранения резервных копий для восстановления на момент времени, до 35 дней. 

База данных SQL также предоставляет некоторые возможности обеспечения непрерывности бизнеса, которые можно использовать для успешного выполнения различных сценариев незапланированной. 

- [Временные таблицы](sql-database-temporal-tables.md) позволяют восстанавливать версии строк из любой точки во времени.
- [Встроенные автоматически создаваемых резервных копий](sql-database-automated-backups.md) и [восстановление на момент времени](sql-database-recovery-using-backups.md#point-in-time-restore) дает возможность восстановления всей базы данных на определенный момент времени в течение заданного периода хранения до 35 дней.
...
## <a name="recover-a-database-within-the-same-azure-region"></a>Восстановить базу данных в одном регионе Azure

Можно использовать автоматическое резервное копирование, восстановление базы данных до точки во времени в прошлом. Таким образом, вы можете восстановить из повреждения данных, из-за ошибок оператора. Восстановление обеспечении времени позволяет создать новую базу данных на том же сервере, представляющее состояние данных до поврежденного события. Для большинства баз данных операций восстановления занимает менее 12 часов. Может занять больше времени на восстановление базы данных очень большие или очень активным. Дополнительные сведения см. в разделе [Время восстановления](sql-database-recovery-using-backups.md#recovery-time). 

Если максимальный период поддерживаемый хранения резервных копий для восстановления на момент времени (PITR) недостаточно для вашего приложения, его можно расширить, настроив долгосрочную политику хранения (LTR) для баз данных. Дополнительные сведения см. в разделе [Долгосрочное хранение резервных копий](sql-database-long-term-retention.md).

## <a name="recover-a-database-to-another-azure-region"></a>Восстановить базу данных в другой регион Azure
...
При разработке плана обеспечения непрерывности бизнес-процессов нужно понимать, какое максимальное время до полного восстановления приложения после аварийного события допустимо. Время, необходимое для приложения, чтобы полностью восстановить известна как цель времени восстановления (RTO). Необходимо также понимать максимальный период последних обновлений данных (интервал времени), приложение может выдержать потери при восстановлении после незапланированного аварийного события. Возможная потеря данных известен как целевую точку восстановления (RPO).

Альтернативные методы восстановления предлагаются различные уровни RPO и RTO. Можно выбрать определенный метод, или использовать комбинацию методов в achicethe приложения полного восстановления. В следующей таблице сравниваются RPO и RTO каждого варианта восстановления.

| Метод восстановления | RTO | RPO |
| --- | --- | --- | 
| Геовосстановление из геореплицированных резервных копий | 12 ч | 1 ч |
| Группы автоматической отработки отказа | 1 ч | 5 s |
| Отработка отказа базы данных вручную | 30 s | 5 s |
...
Вы можете использовать сочетание резервных копий базы данных и активной георепликации в зависимости от требований приложения. Описание рекомендации по проектированию для автономных баз данных и эластичных пулов, используя эти возможности обеспечения непрерывности бизнеса, см. в разделе [проектирование приложения для аварийного восстановления облака](sql-database-designing-cloud-solutions-for-disaster-recovery.md) и [аварийного эластичного пула стратегии восстановления](sql-database-disaster-recovery-strategies-for-applications-with-elastic-pool.md).
...
- Убедитесь, что заданы соответствующие правила брандмауэра для IP-адресов уровня сервера, позволяющие пользователям подключаться к [брандмауэрам уровня базы данных](sql-database-firewall-configure.md#manage-server-level-ip-firewall-rules-using-the-azure-portal) или использовать их, чтобы включить соответствующие правила.
