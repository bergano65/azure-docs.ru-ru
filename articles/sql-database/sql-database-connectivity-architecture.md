...
- **Перенаправление (рекомендуется):** Клиенты устанавливают подключения непосредственно к узлу, на котором размещена база данных, что приводит к снижению задержки и повышению пропускной способности. Для подключений, использующих этот режим, клиенты должны:
   - Разрешите исходящие подключения от клиента ко всем IP-адресам Azure в регионе на портах в диапазоне 11000 11999. Используйте теги службы для SQL, чтобы упростить управление.  
...
- **По умолчанию:** Это политика подключения, которая действует на всех серверах после создания, если политика подключения не была явно изменена на `Proxy` или `Redirect`. Политика по умолчанию`Redirect` для всех подключений клиентов, исходящих в Azure (например, с виртуальной машины Azure), и `Proxy`для всех подключений клиентов, исходящих извне (например, подключения с локальной рабочей станции).
