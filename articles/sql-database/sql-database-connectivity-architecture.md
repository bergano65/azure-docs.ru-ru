...
- **По умолчанию:** Это политика подключения, которая действует на всех серверах после создания, если политика подключения не была явно изменена на `Proxy` или `Redirect`. Действующая политика зависит от того, создается подключение из Azure (`Redirect`) или за его пределами (`Proxy`).
...
> [!IMPORTANT]
> Дополнительно откройте порты 14000-14999, чтобы включить [подключение с помощью DAC](https://docs.microsoft.com/en-us/sql/database-engine/configure-windows/diagnostic-connection-for-database-administrators?view=sql-server-2017#connecting-with-dac) .
