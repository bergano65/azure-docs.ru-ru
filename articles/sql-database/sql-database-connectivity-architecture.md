...
> [!IMPORTANT]
> **[Планируемые изменения] Для конечной точки подключения службы на серверах Azure SQL `Default` изменения в работе подключения `Redirect`.**
...
> - Логика автоматического развертывания создает базу данных SQL server, при условии, что поведение по умолчанию для конечной точки подключения службы `Proxy`
>
> Если подключения конечной точки службы не удается установить к серверу Azure SQL Server и вы подозреваете, что это изменение вас затронуло, убедитесь, что для типа подключения явным образом присвоено `Redirect`. Если это так, необходимо открыть правила брандмауэра виртуальной Машины и группы безопасности сети (NSG) для всех IP-адресов Azure в регионе, принадлежащие Sql [тег службы](../virtual-network/security-overview.md#service-tags) для портов 11000 – 11999. Если этот вариант вам не подходит, явно переключите сервер на `Proxy`.
> [!NOTE]
