---
title: Настройка управляемого экземпляра
...
- [Настройка настраиваемого DNS](sql-database-managed-instance-custom-dns.md) — необходимо настроить НАСТРАИВАЕМый DNS, если требуется получить доступ к внешним ресурсам в пользовательских доменах из управляемого экземпляра через связанный сервер профилей почты.
...
- [Поиск IP-адреса конечной точки управления](sql-database-managed-instance-find-management-endpoint-ip-address.md) — управляемый экземпляр использует общедоступную конечную точку для управления. IP-адрес конечной точки управления можно определить, используя сценарий, описанный здесь.
- [Убедитесь, что встроенная защита брандмауэра](sql-database-managed-instance-management-endpoint-verify-built-in-firewall.md) — управляемый экземпляр защищен с помощью встроенного брандмауэра, который разрешает трафик только на необходимых портах. Встроенные правила брандмауэра можно проверить, используя сценарий, описанный в этом руководстве.
- [Connect Applications](sql-database-managed-instance-connect-app.md) — управляемый экземпляр размещается в собственной частной виртуальной сети Azure с частным IP-адресом. Узнайте о различных шаблонах подключения приложений к управляемому экземпляру.
...
- [Создание оповещений](sql-database-managed-instance-alerts.md) позволяет настроить оповещения для наблюдаемых метрик, таких как загрузка ЦП, потребление дискового пространства, операции ввода-вывода в секунду и другие для управляемого экземпляра. В этом руководство вы узнаете, как включить и настроить оповещения для управляемого экземпляра.

## <a name="next-steps"></a>Дальнейшие шаги

- Дополнительные сведения о [руководствах по отдельным базам данных](sql-database-howto-single-database.md)
