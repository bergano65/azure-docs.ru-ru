...
Синхронизация данных полезна в тех случаях, когда данные должны обновляться в нескольких базах данных Azure S'L или базах данных сервера S'L. Ниже приведены основных варианты использования синхронизации данных.

- **Гибридная синхронизация данных:** С помощью Data Sync можно синхронизировать данные между базами данных и базами данных Azure S'L для включения гибридных приложений. Эта возможность может привлечь клиентов, которые рассматривают переход в облако и собираются поместить некоторые из своих приложений в Azure.
...
- **Глобально распределенные приложения:** Многие предприятия охватывают несколько регионов и даже несколько стран / регионов. Чтобы свести к минимуму задержки в сети, рекомендуется хранить свои данные в регионе, ближайшем к вам. Используя синхронизацию данных, можно легко обеспечить синхронизацию баз данных в регионах по всему миру.
...
| масштаб чтения; | [Использование реплик только для чтения для распределения рабочих нагрузок запросов только для чтения (предварительная версия)](sql-database-read-scale-out.md) |
...
- **База данных синхронизации** содержит метаданные и журнал для синхронизации данных. База данных синхронизации должна быть базой данных Azure S'L, расположенной в том же регионе, что и база данных концентраторов. База данных синхронизации создается клиентом и принадлежит ему.
...
- **Направление синхронизации**: синхронизация может быть двунаправленной или выполняться в одном направлении. То есть, Направление синхронизации может быть *концентратором для участника*или *членом концентратора*или и то, и другое.
...
- **Отслеживание изменения данных**. Служба синхронизации данных отслеживают изменения при помощи триггеров вставки, обновления и удаления. Изменения записываются во вспомогательную таблицу в пользовательской базе данных. Обратите внимание, что BULK INSERT не запускает триггеры по умолчанию. Если FIRE_TRIGGERS не указана, триггеры вставки не выполняются. Добавьте параметр FIRE_TRIGGERS, чтобы служба синхронизации данных могла отслеживать эти вставки. 
...
| Недостатки | — Задержка 5 мин или дольше<br/>— Отсутствует согласованность транзакций<br/>— Большее влияние на производительность | - Не уможет опубликовать из единой базы данных базы данных Azure S-L или объединенную базу данных<br/>— Дорогое обслуживание |
...
Поскольку синхронизация данных основана на триггере, согласованность транзакций не гарантируется. Корпорация Майкрософт гарантирует, что все изменения будут внесены в конечном итоге и что синхронизация данных не приведет к потере данных.
...
> [!IMPORTANT]
> Изменение значения существующего основного ключа приведет к следующему неисправного поведения:
> - Данные между концентратором и членом могут быть потеряны, даже если синхронизация не сообщает о какой-либо проблеме.
> - Синхронизация может выйти из строя, поскольку таблица отслеживания имеет несуществующую строку из источника из-за основного изменения ключа.
...
- Таблица не может иметь столбец идентификации, который не является основным ключом.
- Основной ключ не может иметь следующие типы данных: sql_variant, двоичный, варбинарный, изображение, xml.
...
- Имена объектов (базы данных, таблицы и столбцы) не могут содержать период печати символов (.), левый квадратный кронштейн (яп. ) или правый квадратный кронштейн.
...
За саму службу синхронизации данных S'L не взимается плата. Тем не менее, вы по-прежнему собираете плату за передачу данных для перемещения данных в экземпляре базы данных и из нее. Дополнительные сведения см. на странице [цен на базу данных SQL](https://azure.microsoft.com/pricing/details/sql-database/).
...
### <a name="can-i-use-data-sync-to-sync-between-sql-databases-that-belong-to-different-clouds-like-azure-public-cloud-and-azure-china-21vianet"></a>Могу ли я использовать синхронизацию данных для синхронизации между базами данных S'L, которые принадлежат к различным облакам (например, Azure Public Cloud и Azure China 21Vianet)
...
Не рекомендуется использовать синхронизацию данных для создания резервного копирования данных. Вы не можете создать резервную систему и восстановить его в определенный момент времени, так как синхронизация синхронизации данных не будет версиями. Кроме того, синхронизация данных s'L не резервирует другие объекты S'L, такие как сохраненные процедуры, и не выполняет эквивалент операции восстановления быстро.
...
- Если выбранные таблицы схемы синхронизации еще не вданных баз данных концентратора или членов, то при развертывании группы синхронизации служба автоматически создает соответствующие таблицы и столбцы с настройками коллажа, выбранными в базах данных пустых пунктов назначения.
...
Корневая база данных федерации может использоваться в службе синхронизации данных SQL без каких-либо ограничений. Вы не можете добавить конечную точку Federated Database в текущую версию синхронизации данных S'L.
...
Является ли синхронизация данных в Синхронизации данных, как ожидалось? Сведения об отслеживании действий и устранении неполадок см. в следующих статьях:

- [Мониторинг синхронизации данных Azure с журналами Azure Monitor с помощью журналов Azure Monitor](sql-database-sync-monitor-oms.md)
- [Устранение неполадок с синхронизацией данных SQL Azure (предварительная версия)](sql-database-troubleshoot-data-sync.md)
...
- [Обзор базы данных S'L](sql-database-technical-overview.md)
