...
> [!TIP]
> Краткие руководства по настройке и подключению к Управляемый экземпляр с помощью SQL Server Management Studio в Microsoft Windows см. в разделе [Краткое руководство. Настройка виртуальной машины Azure для подключения к управляемый экземпляр базы данных SQL Azure](sql-database-managed-instance-configure-vm.md) и [Краткое руководство. Настройка подключения типа "точка — сеть" к управляемый экземпляр базы данных SQL Azure из локальной](sql-database-managed-instance-configure-p2s.md)среды.
> [!IMPORTANT]
