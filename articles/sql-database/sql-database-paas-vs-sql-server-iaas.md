...
- [База данных SQL Azure](https://azure.microsoft.com/services/sql-database/). Полностью управляемое ядро СУБД SQL в зависимости от последнего стабильного корпоративного выпуска SQL Server. Это реляционная база данных, которая предоставляется как услуга (DBaaS). Она размещается в облаке Azure и относится к отраслевым категориям *Платформа как услуга (PaaS)*. [База данных SQL](sql-database-technical-overview.md) создана на основе стандартного аппаратного и программного обеспечения, которое принадлежит корпорации Майкрософт, а также размещается и обслуживается ею. С базой данных SQL можно использовать встроенные функции и возможности, которые требуют сложной настройки в SQL Server. При использовании базы данных SQL вы платите по мере использования и имеете возможность увеличивать масштаб базы данных или развертывать ее для повышения производительности без прерывания работы. База данных SQL предоставляет дополнительные функции, недоступные в SQL Server, например встроенную аналитику и управление. База данных SQL Azure предлагает несколько вариантов развертывания:
  - Можно развернуть одну базу данных на [логическом сервере](sql-database-logical-servers.md). Логический сервер с отдельными базами данных в пуле обеспечивает большую часть возможностей уровня базы данных в SQL Server. Этот вариант оптимизирован для современной разработки новых облачных приложений.
...
| SQL Server на виртуальной машине | База данных SQL (управляемый экземпляр) | База данных SQL (логический сервер) |
| --- | --- | --- |
...
|  | Разработчики баз данных, для которых важны высокая доступность, аварийное восстановление и обновление. | Аналогично базам данных на логическом сервере Базы данных SQL Azure. | Команды, способные настроить и поддерживать высокую доступность, аварийное восстановление и установку исправлений для SQL Server. Предлагаемые функции автоматизации позволяют значительно упростить эти процессы. | |
|  | Разработчики, которые не хотят тратить силы на настройку конфигурации и базовой операционной системы. | Аналогично базам данных на логическом сервере Базы данных SQL Azure. | Вам потребуется настраиваемая среда со всеми правами администратора. | |
|  | Базы данных размером до 100 ТБ. | Аналогично базам данных на логическом сервере Базы данных SQL Azure. | Экземпляры SQL Server с хранилищем объемом до 64 ТБ. Экземпляр может поддерживать любое необходимое количество баз данных. |
...
С отдельной базой данных SQL можно выбрать уровень служб, который соответствует вашим потребностям, из широкого диапазона цен, начиная с 5 долл. США в месяц на уровне "Базовый".
Благодаря управляемым экземплярам базы данных SLQ Azure вы можете использовать собственную лицензию. Дополнительные сведения об использовании собственной лицензии см. в статье [Перемещение лицензий в рамках программы Software Assurance в Azure](https://azure.microsoft.com/pricing/license-mobility/) или используйте [калькулятор преимущества гибридного использования Azure](https://azure.microsoft.com/en-us/pricing/hybrid-benefit/#sql-database), чтобы узнать, как **сэкономить до 40 %**.
