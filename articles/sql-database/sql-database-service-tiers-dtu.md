---
title: Уровни обслуживания - модель покупки на основе DTU
description: Дополнительные сведения об уровнях служб в модели приобретения на основе единиц DTU для отдельной базы данных или базы данных в пуле, чтобы обеспечить размеры вычислений и хранилища.
...
ms.locfileid: "75562125"
---
# <a name="service-tiers-in-the-dtu-based-purchase-model"></a>Уровни службы в модели приобретения на основе единицы DTU

Уровни службы в модели приобретения на основе единицы DTU отличаются диапазоном объемов вычислительных ресурсов с фиксированным включенным объемом хранилища, фиксированным сроком хранения резервных копий и фиксированной ценой. Все уровни обслуживания в модели покупки на основе DTU обеспечивают гибкость изменения размеров вычислений с минимальным [временем простоя;](https://azure.microsoft.com/support/legal/sla/sql-database/v1_2/) однако существует переход ный период, при котором подключение теряется в базе данных в течение короткого периода времени, которое может быть смягчено с помощью логики повторной попытки. Плата за отдельные базы данных и эластичные пулы начисляется за каждый час использования в соответствии с уровнем служб и объемом вычислительных ресурсов.
...
|Максимальное удержание резервного копирования|7 дней|35 дней|35 дней|
|ЦП|Низкий|Низкий, средний, высокий|Средний, высокий|
|Пропускная способность ввода-вывода (приблизительно) |1-5 IOPS на ДТУ| 1-5 IOPS на ДТУ | 25 IOPS на ДТУ|
...
> [!IMPORTANT]
> Базовые, стандартные уровни обслуживания S0, S1 и S2 обеспечивают менее одного vCore (CPU).  Для рабочих нагрузок с интенсивным процессором рекомендуется уровень обслуживания S3 или более. 
>
>Что касается хранения данных, то базовые уровни службы Standard S0 и S1 размещаются на стандартных страницах Blobs. Standard Page Blobs используют средства хранения на основе жесткого диска (HDD) и лучше всего подходят для разработки, тестирования и других нечасто доступных рабочих нагрузок, которые менее чувствительны к изменчивости производительности.
>

> [!NOTE]
> Вы можете получить бесплатную базу данных Azure S'L на уровне основных служб в сочетании с бесплатной учетной записью Azure для изучения Azure. Сведения см. в разделе [Создание управляемой облачной базы данных с помощью бесплатной учетной записи Azure](https://azure.microsoft.com/free/services/sql-database/).

## <a name="single-database-dtu-and-storage-limits"></a>DTU отдельной базы данных и ограничения хранилища

Объем вычислительных ресурсов выражается в единицах транзакций базы данных (DTU) для отдельных баз данных, а для эластичных пулов — в единицах транзакций эластичной базы данных (eDTU). Для получения дополнительной информации о DTU и eDTUs, [см.](sql-database-purchase-models.md#dtu-based-purchasing-model)
...
> [!IMPORTANT]
> Иногда требуется сжать базу данных, чтобы освободить неиспользуемое пространство. Для получения дополнительной информации посетите [управление пространством файлов в базе данных Azure S'L.](sql-database-file-space-management.md)
