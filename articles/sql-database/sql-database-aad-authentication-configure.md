...
- Если учетная запись Azure AD существует в базе данных master для MI, созданной с помощью команды `CREATE LOGIN [myaadaccount] FROM EXTERNAL PROVIDER`T-SQL, ее нельзя настроить в качестве администратора Azure AD для MI. При установке имени входа в качестве администратора Azure AD можно использовать команды портал Azure, PowerShell или интерфейса командной строки для создания имени входа Azure AD.
  - Имя входа должно быть удалено в базе данных master с помощью `DROP LOGIN [myaadaccount]`команды, прежде чем учетная запись может быть создана как администратор Azure AD.
  - Настройте учетную запись администратора Azure AD в портал Azure после `DROP LOGIN` завершения. 
...
Следующий сценарий подготавливает группу администраторов Azure AD с именем **DBA_Group** (идентификатор `40b79501-b343-44ed-9ce7-da4c8cc7353f`объекта) для сервера **demo_server** в группе ресурсов с именем **Group-23**:
...
Для подключения к базе данных с помощью встроенной проверки подлинности и удостоверения Azure AD ключевое слово Authentication в строке подключения к базе `Active Directory Integrated`данных должно иметь значение. В следующем примере кода C# используется ADO .NET.
...
Для подключения к базе данных с использованием только облачных учетных записей удостоверений Azure AD или пользователей, использующих гибридные удостоверения Azure AD, ключевому `Active Directory Password`слову Authentication должно быть присвоено значение. Строка подключения должна содержать ключевые слова для идентификатора пользователя (UID) и пароля (PWD), а также их значения. В следующем примере кода C# используется ADO .NET.
...
> [!NOTE]
> `sqlcmd``-G` команда не работает с системными удостоверениями и требует входа субъекта-пользователя.
