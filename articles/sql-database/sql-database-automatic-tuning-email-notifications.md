...
- Нажмите на**кнопку "Создать ресурс"** в левом верхнем углу
- Поиск "**Автоматизация**" (нажмите введите)
...
- Оказавшись внутри панели "Создать учетную запись автоматизации", нажмите на "**Создать**"
...
- После того, как в панели учетной записи автоматизации, нажмите на пункт меню **"Runbooks"** на левой стороне, чтобы создать новую книгу автоматизации Azure с помощью сценария PowerShell. Чтобы узнать больше о создании модулей runbook службы автоматизации, ознакомьтесь с разделом [Создание или импорт модуля Runbook в службе автоматизации Azure](../automation/manage-runbooks.md#creating-a-runbook).
- Чтобы добавить новый runbook, нажмите на опцию меню **«Добавить runbook»,** а затем нажмите на кнопку **«Быстрое создание – Создайте новый runbook».**
- В панели Runbook введите имя вашего runbook (для целей этого примера, "**AutomaticTuningEmailAutomation**" используется), выберите тип runbook как **PowerShell** и напишите описание этого runbook, чтобы описать его назначение.
...
- Внутри панели **"Edit PowerShell Runbook",** выберите "**RUNBOOKS**" в дереве меню и расширьте представление, пока не увидите название своей книги (в этом примере "**AutomaticTuningEmailAutomation**"). Выберите этот runbook.
- На первой строке "Edit PowerShell Runbook" (начиная с числа 1), копировать-вставить следующий код сценария PowerShell. Этот сценарий PowerShell предоставляется "как есть" и дает возможность приступить к работе. Измените его в соответствии со своими потребностями.
...
1. "**Azure Automation - Создание задания**" - используется для выполнения сценария PowerShell для получения рекомендаций автоматической настройки в беговой книге Azure Automation
2. "**Azure Automation - Получить выход за дания**" - используется для получения вывода из выполненного сценария PowerShell
3. "**Office 365 Outlook - Отправить по электронной почте**" - используется для отправки электронной почты. Электронные сообщения отправляются с помощью учетной записи Office 365 пользователя, создавшего поток.
...
- Элемент меню **"Мои потоки"**
- Внутри Моих потоков выберите ссылку **«Создание из пустой»** в верхней части страницы
- Нажмите на ссылку "**Поиск сотен разъемов и триггеров**" в нижней части страницы
- В поисковом поле типа **"повторение",** и выберите "**Расписание - Повторение**" из результатов поиска, чтобы запланировать работу доставки электронной почты для запуска.
...
3. Создание действия для отправки электронных сообщений с помощью интеграции Office 365
   ...
   - В поле типа **«To»** введите адрес электронной почты, на который необходимо отправить письмо с уведомлением
   ...
   - Нажмите внутри поля "**Body**" для меню **"Динамического содержимого",** чтобы показать. Из этого меню, под "**Получить выход задания**", выберите "**Содержание**"
   - Полное создание этого действия, нажав на "**Сохранить поток**"
