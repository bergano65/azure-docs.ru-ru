...
- [& классификация обнаружения данных](sql-database-data-discovery-and-classification.md) предоставляет возможности, встроенные в базу данных SQL Azure для обнаружения, классификации и маркировки & создания отчетов о конфиденциальных данных в базах данных. Она может использоваться для просмотра состояния классификации базы данных, а также отслеживания доступа к конфиденциальным данным в базе данных и за ее пределами.
...
Включите службу ADS SQL, чтобы активировать все ее функции. Вы можете включить ADS для всех баз данных SQL на вашем сервере или управляемом экземпляре Базы данных SQL. Для включения или управления параметрами БАННЕРа необходимо принадлежать к роли [диспетчера безопасности SQL](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#sql-security-manager) , роли администратора базы данных SQL или роли администратора SQL Server. 
...
Включите рекламу, перейдя к **расширенной защите данных** под заголовком **Безопасность** сервера базы данных SQL или управляемого экземпляра. Чтобы включить ADS для всех баз данных на сервере или управляемом экземпляре базы данных, щелкните **Включить Расширенную защиту данных на сервере**.
...
## <a name="3-manage-ads-settings-on-your-sql-database-server-or-managed-instance"></a>3. Управление параметрами БАННЕРов на сервере базы данных SQL или управляемом экземпляре
...
## <a name="4-manage-ads-settings-for-a-sql-database"></a>4. Управление параметрами БАННЕРов для базы данных SQL

Чтобы переопределить параметры ADS для конкретной базы данных, установите флажок, позволяющий **включить расширенную защиту данных на уровне базы данных**. Используйте этот параметр только в том случае, если у вас есть определенное требование для получения отдельных дополнительных оповещений или результатов оценки уязвимости для отдельной базы данных вместо предупреждений и результатов, полученных для всех баз данных на сервере базы данных или управляемом экземпляре.
...
- Дополнительные сведения о [классификации & обнаружения данных](sql-database-data-discovery-and-classification.md) 
