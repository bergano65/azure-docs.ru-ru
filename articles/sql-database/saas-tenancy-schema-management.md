...
В приложении Wingtip Tickets каждая клиентская база данных включает набор поддерживаемых типов мест проведения. Каждое место проведения относится к определенному типу, который определяет тип потенциальных мероприятий и фоновое изображение, используемое в приложении. Чтобы приложение поддерживало новые типы мероприятий, необходимо обновить эталонные данные и добавить новые типы мест проведения.  В этом упражнении вы развернете обновление ко всем базам данных клиента добавить два дополнительных типа объектов: *Motorcycle Racing* (Мотоциклетные гонки) и *Swimming Club* (Плавательный клуб).
...
1. Измените инструкцию: ЗАДАЙТЕ @wtpUser = &lt;пользователя&gt; и замените значение пользователя, используемое при развертывании приложения Wingtip Tickets SaaS базы данных для каждого клиента
1. Убедитесь, что вы подключены к _jobagent_ базы данных и нажмите клавишу **F5** для запуска сценария
...
Далее ознакомьтесь [автоматизированную систему отчетности руководстве](saas-tenancy-cross-tenant-reporting.md) чтобы узнать о выполнении распределенных запросов в клиентских базах данных.
