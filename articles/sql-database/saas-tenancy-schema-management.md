...
В приложении Wingtip Tickets каждая клиентская база данных включает набор поддерживаемых типов мест проведения. Каждое место проведения относится к определенному типу, который определяет тип потенциальных мероприятий и фоновое изображение, используемое в приложении. Чтобы приложение поддерживало новые типы мероприятий, необходимо обновить эталонные данные и добавить новые типы мест проведения.  В этом упражнении вы развернете обновление для всех баз данных клиента, чтобы добавить два дополнительных типа мест: *Motorcycle Racing* (Мотоциклетные гонки) и *Swimming Club* (Плавательный клуб).
...
1. Измените инструкцию: Set @wtpUser = &lt;user&gt; и замена значения User, используемого при развертывании приложения SaaS Wingtip Tickets для каждого клиента
1. Убедитесь, что вы подключены к базе данных _jobagent_ , и нажмите клавишу **F5** , чтобы запустить сценарий.
