...
1. Чтобы убедиться, что в настоящее время **не** включены *Motorcycle* и *Swimming клуб* , перейдите к базе данных _contosoconcerthall_ на сервере *tenants1-DPT-&lt;пользователя&gt;* и запросите таблицу *VenueTypes* .
...
После выполнения скрипта вы можете проверить, были ли эталонные данные обновлены.  В SSMS перейдите к базе данных *contosoconcerthall* на сервере *tenants1-dpt -&lt;пользователь&gt;* и запросите таблицу *VenueTypes*.  Убедитесь **, что в настоящее время имеются** *Motorcycleные скачки* и *Swimming клуб* .
