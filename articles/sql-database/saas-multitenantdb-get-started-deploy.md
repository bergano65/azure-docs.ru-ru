---
title: Развертывание осколки мультитенантной базы данных SaaS приложение
...
> - Как арендаторы отображаются к их данным с *каталогом.*
...
- Установите последнюю версию Azure PowerShell. Для получения подробной информации [см.][link-azure-get-started-powershell-41q]
...
- *Пользователь:* **af1**  *(Их инициалы, плюс цифра. Используйте другое значение (например, af2), если вы развернете приложение во второй раз).*
- *Группа ресурсов:* **wingtip-mt-af1** *(wingtip-mt указывает, что это осколочное мультитенантное приложение.*
...
1. Введите необходимые значения параметров для развертывания.
    ...
    - Для **группы ресурсов** - Выберите **Создать новый,** а затем укажите **имя** для группы ресурсов (дело чувствительное).
...
3. Нажмите **Скачать ЗИП** и сохранить файл.
...
Скрипты расположены в *.. WingtipTicketsSaAS-MultiTenantDb-мастер\\Учебные\\ модули папку. \\*
...
1. Открыть... \\Учебные\\модули*UserConfig.psm1* в *PowerShell ISE*.
...
1. Откройте **концентратор событий** в браузере:
   - http://events.wingtip-mt.&lt;пользователь&gt;.trafficmanager.net &nbsp; *(Заменить &lt;пользователя&gt; значением пользователя вашего развертывания.)*
...
1. Открыть... \\Учебные\\модули\\Обеспечение и Каталог*Демо-ПоложенияАрендаторы.ps1* в *PowerShell ISE*.
...
Освежите **концентратор событий,** и новый арендатор теперь отображается в списке.
...
1. В... \\Учебные\\модули\\Обеспечение и Каталог*Демо-ПоложенияTenants.ps1*, изменить *$TenantName* **Salix Salsa**, *$VenueType* **танцевать** и *$Scenario* до **2**.

2. Нажмите клавишу **F5** для повторного запуска скрипта.
    - Это **F5** пресс положения нового арендатора в отдельной базе данных. Базы данных и клиент регистрируются в каталоге. Затем браузер открывает страницу мероприятий клиента.
...
3. Обновление **концентратора событий** и двух новых арендаторов теперь отосвазано в списке.
...
2. Нажмите **на&lt;&gt; пользовательский сервер каталога mt.** На сервере каталога содержится две базы данных с именем *tenantcatalog* и *basetenantdb*. База данных *basetenantdb* — это пустой шаблон базы данных. Он копируется для создания клиентской базы данных, используемой для нескольких клиентов или только для одного.
...
1. Просмотрите на **сервере&lt;пользователя&gt; tenants1 mt** и нажмите **на tenants1** для просмотра использования ресурсов для базы данных, в базе данных которого четыре клиента. Каждый арендатор подвергается случайной высокой нагрузке от генератора нагрузки:
...
2. Просмотрите на **сервере&lt;пользователя&gt; tenants1 mt** и нажмите на базу данных **salixsalsa.** Вы можете увидеть использование ресурсов в этой базе данных, содержащей только один клиент.
...
> - сведения о серверах и базах данных, формирующих приложение;
> - Арендаторы отображаются на их данных с *каталогом*.
...
Теперь попробуйте [Положение и каталог учебник](sql-database-saas-tutorial-provision-and-catalog.md).
