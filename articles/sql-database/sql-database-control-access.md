...
description: Узнайте, как предоставить доступ к База данных SQL Microsoft Azure и Azure синапсе.
...
ms.locfileid: "78197850"
---
# <a name="azure-sql-database-and-azure-synapse-access-control"></a>База данных SQL Azure и контроль доступа Azure синапсе

Чтобы обеспечить безопасность, [базу данных SQL](sql-database-technical-overview.md) Azure и [синапсе управления Azure](../sql-data-warehouse/sql-data-warehouse-overview-what-is.md) с помощью правил брандмауэра, ОГРАНИЧИВАЮЩИХ подключение по IP-адресу, механизмы проверки подлинности, запрашивающие пользователей о том, что пользователи должны подтвердить свою личность, и механизмы авторизации, ограничивающие пользователей конкретными действиями 

> [!IMPORTANT]
> Обзор функций защиты в базе данных SQL см. в [этой статье](sql-database-security-overview.md). Ознакомьтесь с руководством [Защита базы данных SQL Azure](sql-database-security-tutorial.md). Общие сведения о функциях безопасности SQL Analytics в Azure синапсе см. в статье [Обзор безопасности Azure синапсе](../sql-data-warehouse/sql-data-warehouse-overview-manage-security.md) .
