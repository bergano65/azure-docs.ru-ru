...
Разработчики SaaS создают приложения на основе уровней данных большого размера, состоящих из нескольких баз данных. Обычно для каждого клиента подготавливается одна база данных. Но разные клиенты часто имеют различные и непредсказуемые модели использования, и трудно предсказать потребности в ресурсах каждого отдельного пользователя базы данных. Обычно имеется два варианта:
...
Чем больше баз данных можно добавить в пул, тем больше экономия. В зависимости от шаблона использования приложений можно увидеть экономию с помощью двух баз данных S3.
...
Примечание этот пример не учитывает использование других баз данных в пуле. Если все базы данных используются в любой заданной временной точке, тогда одновременно испытывать пиковую нагрузку могут менее 2/3 (или 67 %) от числа баз данных.
...
**Пример модели закупок на основе DTU:** база данных S3, которая достигает 100 DTU и в среднем использует 67 DTU или менее, является хорошим кандидатом для обмена eDTU в пуле. Кроме того, база данных уровня S1 с пиковым использованием 20 единиц DTU и средним использованием 13 единиц DTU или менее хорошо подходит для пула.
...
1. Перейдите на [портал Azure,](https://portal.azure.com) чтобы создать эластичный пул. Поиск и выберите **Azure S'L.**
2. Выберите **Добавить**, чтобы открыть страницу **выбора варианта развертывания SQL**. Дополнительную информацию об эластичных пулах можно просмотреть, выбрав **сведения о показе** на плитке **базы данных.**
3. На **плитке Базы данных** выберите **упругий пул** в выпадении **типа ресурса,** а затем выберите **Создать:**

   ![Создание эластичного пула](./media/sql-database-elastic-pool/create-elastic-pool.png)


1. Или вы можете создать эластичный пул, перенаправившись на существующий сервер Azure S'L и нажав **на новый пул,** чтобы создать пул непосредственно в этот сервер.
...
Чтобы начать отслеживать эластичный пул, найдите и откройте его на портале. Сначала вы увидите экран, который дает вам обзор состояния вашего эластичного пула. В том числе:
...
- [SnelStart](https://azure.microsoft.com/resources/videos/azure-sql-database-case-study-snelstart/)
  ...
  SnelStart использовала эластичные пулы с базой данных Azure S'L для быстрого расширения своих бизнес-услуг в размере 1000 новых баз данных Azure S'L в месяц.

- [Umbraco](https://azure.microsoft.com/resources/videos/azure-sql-database-case-study-umbraco/)

  Umbraco использует эластичные пулы с базой данных Azure S'L для быстрого предоставления и масштабирования услуг для тысяч арендаторов в облаке.
