...
- Создайте отдельную группу для администраторов Azure AD для каждого сервера базы данных SQL.
...
Управление доступом (также называемое авторизацией) — это процесс управления доступом и привилегиями авторизованных пользователей к базе данных SQL Azure и управления ими.
...
- Используйте аудит SQL и классификацию данных в сочетании. 
  - В журнале [аудита базы данных SQL](sql-database-auditing.md) можно отслеживать доступ специально для конфиденциальных данных. Можно также просмотреть такие сведения, как данные, к которым осуществлялся доступ, а также их метку чувствительности. Дополнительные сведения см. в разделе [Обнаружение данных & классификация](sql-database-data-discovery-and-classification.md) и [Аудит доступа к конфиденциальным данным](sql-database-data-discovery-and-classification.md#audit-sensitive-data). 
