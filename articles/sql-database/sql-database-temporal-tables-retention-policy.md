---
title: Управление историческими данными в темпоральных таблицах с политикой хранения
...
Флаг базы данных **is_temporal_history_retention_enabled** по умолчанию имеет значение ON (Вкл.), но пользователи могут изменить его с помощью инструкции ALTER DATABASE. Он также автоматически получает значение OFF после операции [восстановления до точки во времени](sql-database-recovery-using-backups.md). Чтобы включить темпоральную очистку исторических данных для базы данных, выполните следующую инструкцию:
...
Процесс очистки зависит от структуры индексов таблицы исторических данных. Обратите внимание, что *политику хранения с конечным сроком можно настроить только для таблиц журнала с кластеризованным индексом (сбалансированное дерево или columnstore)* . Для очистки устаревших данных из всех темпоральных таблиц с ограниченным сроком хранения создается фоновая задача.
Логика очистки для кластеризованного индекса типа rowstore (сбалансированное дерево) удаляет устаревшие строки мелкими фрагментами (не более 10 000), чтобы свести к минимуму нагрузку на журнал базы данных и подсистему ввода-вывода. Несмотря на то что логика очистки использует индекс сбалансированного дерева, порядок удаления строк, возраст которых превышает срок хранения, не может быть гарантирован. Поэтому *не используйте в приложениях никакие допущения в отношении порядка очистки*.

Задача очистки для кластеризованного индекса типа columnstore удаляет полные [группы строк](https://msdn.microsoft.com/library/gg492088.aspx) за один раз (обычно они содержит по 1 миллиону строк). Это очень эффективный механизм, особенно если данные журнала формируются с высокой скоростью.
...
*Сообщение 13765, уровень 16, состояние 1 <br></br> установка конечного срока хранения в темпоральной таблице с версией системы "temporalstagetestdb. dbo. WebsiteUserInfo" завершилась сбоем, так как таблица журнала "temporalstagetestdb. dbo. WebsiteUserInfoHistory" не содержит необходимый кластеризованный индекс. Рассмотрите возможность создания кластеризованного индекса columnstore или сбалансированного дерева, начиная со столбца, который соответствует концу периода SYSTEM_TIME, в таблице журнала.*
...
*Сообщение 13766, уровень 16, состояние 1 <br></br> не может удалить кластеризованный индекс "WebsiteUserInfoHistory. IX_WebsiteUserInfoHistory", так как он используется для автоматической очистки устаревших данных. Если необходимо удалить этот индекс, рассмотрите возможность установки значения INFINITE для HISTORY_RETENTION_PERIOD в соответствующей темпоральной таблице с системным управлением версиями.*
