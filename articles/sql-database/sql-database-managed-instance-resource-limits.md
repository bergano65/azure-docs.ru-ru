...
| Среда выполнения приложений | 8, 16, 24 виртуальных ядер | 8, 16, 24, 32, 40, 64, 80 виртуальных ядер |
...
| Максимальный размер хранилища ("Критически важный для бизнеса") | 1 ТБ | 1 ТБ, 2 ТБ или 4 ТБ в зависимости от количества ядер |
...
| Память | 4-е поколение: 56–168 ГБ<br/>5-е поколение: 40,8–408 ГБ<br/>\*Пропорционально количеству виртуальных ядер | 4-е поколение: 56–168 ГБ <br/> 5-е поколение: 40,8–408 ГБ<br/>\*Пропорционально количеству виртуальных ядер |
| Максимальный размер хранилища | 8 ТБ | 4-е поколение: 1 ТБ <br/> 5-е поколение: <br/>— 1 ТБ для 8, 16 виртуальных ядер<br/>— 2 ТБ для 24 виртуальных ядер<br/>— 4 ТБ для 32, 40, 64, 80 виртуальных ядер |
...
| Операции ввода-вывода в секунду для данных или журнала (приблизительно) | 500–7,500 МБ/с на каждый файл<br/>\*[Зависит от размера файла](https://docs.microsoft.com/azure/virtual-machines/windows/premium-storage-performance#premium-storage-disk-sizes)| 11–110 тис. (1375 на виртуальное ядро) |
...
- Пропускная способность и операции ввода-вывода в секунду также зависят от размера страницы, который явно не ограничивается Управляемым экземпляром.
...
> [!Note]
> Эти ограничения указаны параметры по умолчанию и не технических ограничений. Ограничения могут быть повышения по требованию, создав специальные [запрос в службу поддержки на портале Azure](#obtaining-a-larger-quota-for-sql-managed-instance) при необходимости дополнительные управляемые экземпляры в текущей области. В качестве альтернативы можно создать Управляемые экземпляры в другом регионе Azure. В этом случае отправлять запрос на поддержку не нужно.
...
** Максимальное число экземпляров на одном уровне служб применяется в случае, если экземпляры отсутствуют на другом уровне служб. Если вы планируете смешивать экземпляры уровня "Критически важный для бизнеса" и "Общее назначение" в одной подсети, используйте следующий раздел в качестве ориентира для допустимых сочетаний. Запомните простое правило: общее число подсетей не может превышать 3, а общее число единиц экземпляров — 12.
...
Если вам нужны дополнительные Управляемые экземпляры в текущих регионах, вы можете отправить запрос на поддержку, чтобы расширить квоту, с помощью портала Azure.
