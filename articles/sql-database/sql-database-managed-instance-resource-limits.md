...
| Максимальный размер tempdb | Ограничено до 24 ГБ/Виртуальное ядро (96-1 920 ГБ) и размера экземпляра, доступного в данный момент.<br/>Добавьте дополнительные виртуальных ядер, чтобы получить больше пространства TempDB. | До текущего размера экземпляра. Размер файла журнала TempDB в настоящее время ограничен 24 ГБ/Виртуальное ядро. |
...
| Максимальный размер файла | Ограничение в 8 ТБ, доступный размер экземпляра (не более 2 ТБ-8 ТБ) и [место выделения памяти на диске Azure класса Premium](sql-database-managed-instance-transact-sql-information.md#exceeding-storage-space-with-small-database-files). | Ограничение в 4 ТБ и текущий доступный размер экземпляра (до 1 ТБ – 4 ТБ). |
| Максимальный размер файла журнала | Ограничение составляет 2 ТБ, доступный размер экземпляра и место для [размещения хранилища на диске Azure класса Premium](sql-database-managed-instance-transact-sql-information.md#exceeding-storage-space-with-small-database-files). | Ограничено до 2 ТБ и размера экземпляра, доступного в данный момент. |
