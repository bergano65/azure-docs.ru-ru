...
| Максимальное зарезервированное хранилище экземпляра |  Общего назначения: 8 ТБ<br/>Критически важный для бизнеса: 1 TБ | Общего назначения: 8 ТБ<br/> Критически важный для бизнеса 1 ТБ, 2 ТБ или 4 ТБ в зависимости от числа ядер |
...
| 16 виртуальных ядер | 15,77 ГБ | 20 GB |
...
| --- | --- | --- |
| Количество виртуальных ядер\* | 4-е поколение: 8, 16, 24<br/>5-е поколение: 4, 8, 16, 24, 32, 40, 64, 80 | 4-е поколение: 8, 16, 24 <br/> 5-е поколение: 4, 8, 16, 24, 32, 40, 64, 80 <br/>@no__t — 0Same число виртуальных ядер, выделенное для запросов только для чтения. |
| Максимальный объем памяти | 4-е поколение: 56 ГБ — 168 ГБ (7 ГБ/Виртуальное ядро)<br/>5-е поколение: 20,4 ГБ — 408 ГБ (5.1 ГБ/Виртуальное ядро)<br/>Добавьте дополнительные виртуальных ядер, чтобы получить больше памяти. | 4-е поколение: 56 ГБ — 168 ГБ (7 ГБ/Виртуальное ядро)<br/>5-е поколение: 20,4 ГБ — 408 ГБ (5.1 ГБ/Виртуальное ядро) для запросов на чтение и запись<br/>+ дополнительно 20,4 ГБ — 408 ГБ (5.1 ГБ/Виртуальное ядро) для запросов только для чтения.<br/>Добавьте дополнительные виртуальных ядер, чтобы получить больше памяти. |
...
\* в разделе Планирование развертываний, учтите, что для уровня служб критически важный для бизнеса (BC) требуется четыре (4) раза больше Виртуальное ядро, чем уровень служб общего назначения (GP). Пример: 1 GP Виртуальное ядро = 1 Виртуальное ядро Unit и 1 BC Виртуальное ядро = 4 Виртуальное ядро единиц. Чтобы упростить анализ потребления по умолчанию, суммируйте единицы Виртуальное ядро во всех подсетях в регионе, где развернуты управляемые экземпляры, и сравните результаты с ограничениями единиц экземпляра для вашего типа подписки. Ограничение **максимального числа единиц Виртуальное ядро** применяется к каждой подписке в регионе. Ограничения на отдельные подсети отсутствуют, за исключением того, что сумма всех виртуальных ядер, развернутых в нескольких подсетях, должна быть меньше или равна **максимальному числу единиц Виртуальное ядро**.

\* @ no__t-1 более крупные ограничения подсети и виртуальное ядро доступны в следующих регионах: Восточная Австралия, Восточная часть США, Восточная часть США 2, Северная Европа, Юго-Центральная часть США, Юго-Восточная Азия, южная часть Соединенного Королевства, Западная Европа, Западная часть США 2.
