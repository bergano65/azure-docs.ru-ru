...
| Максимальный объем памяти (соотношение ресурсов памяти и ядра) | 7 ГБ на виртуальное ядро<br/>Добавление дополнительных виртуальных ядер, чтобы обеспечить больший объем памяти. | 5,1 ГБ на виртуальное ядро<br/>Добавление дополнительных виртуальных ядер, чтобы обеспечить больший объем памяти. |
...
| Экземпляр max зарезервировано хранилище (общего назначения) |  8 ТБ | 8 ТБ |
| Экземпляр max зарезервировано хранилище (критически важный для бизнеса) | 1 TБ | 1 ТБ, 2 ТБ или 4 ТБ в зависимости от количества ядер |
...
| Максимальный объем памяти | 4-е поколение: 56 – 168 ГБ (7 ГБ/виртуальное)<br/>5-е поколение: 40.8 – 408 ГБ (5.1 ГБ/виртуальное)<br/>Добавление дополнительных виртуальных ядер, чтобы обеспечить больший объем памяти. | 4-е поколение: 56 – 168 ГБ (7 ГБ/виртуальное)<br/>5-е поколение: 40.8 – 408 ГБ (5.1 ГБ/виртуальное)<br/>Добавление дополнительных виртуальных ядер, чтобы обеспечить больший объем памяти. |
| Экземпляр max зарезервировано размер хранилища | -2 ТБ для 4 виртуальных ядра (5-го поколения только)<br/>— 8 ТБ для других размеров | 4-е поколение: 1 TБ <br/> 5-е поколение: <br/>-1 ТБ для 4, 8, 16 виртуальных ядер<br/>— 2 ТБ для 24 виртуальных ядер<br/>— 4 ТБ для 32, 40, 64, 80 виртуальных ядер |
...
| Максимальное количество файлов базы данных на один экземпляр | До 280 | 32 767 файлов для одной базы данных |
| Операции ввода-вывода в секунду для данных или журнала (приблизительно) | 500–7,500 МБ/с на каждый файл<br/>\*[Увеличение размера файла, чтобы получить дополнительные операции ввода-ВЫВОДА](https://docs.microsoft.com/azure/virtual-machines/windows/premium-storage-performance#premium-storage-disk-sizes)| 11-110 тысяч (1375/виртуальное)<br/>Добавление дополнительных виртуальных ядер, чтобы повысить производительность операций ввода-ВЫВОДА. |
| Предельная пропускная способность записи журнала | 3 МБ/с на виртуальное ядро<br/>Max 22 МБ в секунду на один экземпляр | 4 МБ в секунду на виртуальное ядро<br/>Max 48 МБ в секунду на один экземпляр|
| Пропускная способность данных (приблизительно) | 100–250 МБ/с на каждый файл<br/>\*[Увеличение размера файла, чтобы повысить производительность операций ввода-ВЫВОДА](https://docs.microsoft.com/azure/virtual-machines/windows/premium-storage-performance#premium-storage-disk-sizes) | Н/Д |
| Задержка ввода-ВЫВОДА хранилища (приблизительно) | 5–10 мс | 1–2 мс |
| Максимальный размер tempdb | 192–1,920 ГБ (24 ГБ на виртуальное ядро)<br/>Добавление дополнительных виртуальных ядер, чтобы получить больше места в базе данных TempDB. | Ограничивается размер при хранении max экземпляра. Размер файла журнала TempDB в настоящее время ограничен объемом 24 ГБ/виртуальное. |
