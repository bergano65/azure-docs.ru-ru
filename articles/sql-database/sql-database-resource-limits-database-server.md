...
Для баз данных уровня "базовый", "Стандартный" и "общего назначения" `primary_group_max_io` , которые используют файлы данных в службе хранилища Azure, значение может быть недостижимо, если в базе данных недостаточно файлов данных для кумулятивного предоставления этого числа операций ввода-вывода или если данные не распределяются равномерно по файлам, или если уровень производительности базовых больших двоичных объектов ограничивает количество операций ввода/вывода, заданное в параметрах Аналогичным образом, при небольшом объеме операций ввода-вывода `primary_max_log_rate` журнала, создаваемых частыми фиксациями транзакций, это значение может быть недостижимо рабочей нагрузкой в связи с ограничением в секунду для хранилища больших двоичных объектов Azure.
...
Чтобы просмотреть и записать операции ввода-вывода, пропускную способность и задержку для каждого файла базы данных, используйте функцию [sys. dm_io_virtual_file_stats ()](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-io-virtual-file-stats-transact-sql) . Эта функция выдает все операции ввода-вывода для базы данных, включая фоновые операции ввода `avg_data_io_percent`-вывода, которые не учитывают базу, но используют операций ввода-вывода и пропускную способность базового хранилища, что может повлиять на наблюдаемую задержку Функция также предоставляет дополнительную задержку, которая может быть представлена в управлении ресурсами ввода-вывода для операций чтения и `io_stall_queued_read_ms` записи `io_stall_queued_write_ms` в столбцах и соответственно.
