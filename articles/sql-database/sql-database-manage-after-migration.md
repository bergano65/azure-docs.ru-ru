---
title: Управление едиными и объединенными базами данных после миграции
...
На [портале Azure](https://portal.azure.com/)можно контролировать использование отдельных баз данных, выбрав базу данных и нажав на диаграмму **мониторинга.** Появится окно **Метрика**, которое можно изменить, нажав кнопку **Изменить диаграмму**. Добавьте следующие метрики:
...
После добавления этих метрик можно продолжить их просмотр в таблице **мониторинга** с дополнительной информацией о окне **метрики.** Все четыре метрики показывают средний процент использования в соответствии с количеством единиц **DTU** для вашей базы данных. Узнайте больше об уровнях служб, ознакомившись с [моделью приобретения на основе единиц DTU](sql-database-service-tiers-dtu.md) и [моделью приобретения на основе виртуальных ядер](sql-database-service-tiers-vcore.md).  
...
- Идентификационная & аутентификация ([проверка подлинности СЗЛ и активная каталог Azure (AAD) аутентификация).](sql-database-manage-logins.md)
...
- [Проверка подлинности СЗЛ](https://docs.microsoft.com/sql/relational-databases/security/choose-an-authentication-mode#connecting-through-sql-server-authentication)
...
Правила брандмауэра можно создавать на уровне сервера и базы данных. Правила брандмауэра для IP-адресов на уровне сервера можно создавать с помощью портала Azure или с помощью SSMS. Для получения дополнительной информации о том, как установить правило брандмауэра уровня сервера и базы данных, см.: [Создайте правила брандмауэра IP в базе данных S'L.](sql-database-security-tutorial.md#create-firewall-rules)

#### <a name="service-endpoints"></a>Конечные точки служб

По умолчанию в базе данных SQL настроен параметр "Разрешить службам Azure доступ к серверу". Это означает, что все виртуальные машины в Azure могут подключаться к базе данных. Но эти службы все равно должны пройти проверку подлинности. Чтобы настроить доступ к базе данных через все IP-адреса Azure, параметр "Разрешить службам Azure доступ к серверу" нужно отключить. Кроме того, можно настроить [конечные точки службы VNet.](sql-database-vnet-service-endpoint-rule-overview.md)
...
Другим вариантом является предоставление [зарезервированных IP-адресов](../virtual-network/virtual-networks-reserved-public-ip.md) для ви-х и добавление этих конкретных IP-адресов VM в настройках брандмауэра сервера. Назначая зарезервированные IP-адреса, вы избавляетесь от необходимости обновления правил брандмауэра при изменении IP-адресов.
...
База данных S'L соответствует целому ряду нормативных требований. Чтобы просмотреть последний набор совместимия данных, которые были выполнены в базе данных S'L, посетите [Целевой центр Майкрософт](https://gallery.technet.microsoft.com/Overview-of-Azure-c1be3942) и проверите сложности, которые важны для вашей организации, чтобы узнать, включена ли база данных S'L в соответствии с совместимыми службами Azure. Важно отметить, что хотя служба "База данных SQL" может быть сертифицирована как совместимая служба, это не гарантирует автоматическое соответствие нормативным требованиям организации.
...
#### <a name="azure-sql-analytics-preview-in-azure-monitor-logs"></a>Аналитика Azure S'L (Предварительный просмотр) в журналах Azure Monitor

[Журналы Azure Monitor](../azure-monitor/insights/azure-sql.md) позволяют собирать и визуализировать ключевые показатели производительности базы данных Azure S'L, поддерживая до 150 000 баз данных S'L и 5000 упругих пулов S'L на рабочее пространство. Ее можно использовать для отслеживания и получения уведомлений. Вы можете отслеживать метрики базы данных SQL и эластичных пулов в нескольких подписках Azure и эластичных пулах и использовать их при выявлении проблем на каждом уровне стека приложений.

### <a name="i-am-noticing-performance-issues-how-does-my-sql-database-troubleshooting-methodology-differ-from-sql-server"></a>Я замечаю проблемы с производительностью: Чем моя методология устранения неполадок в базе данных s'L отличается от методологии устранения неполадок в S'L Server
...
Кроме того, если вы заметили ухудшение общей производительности базы данных, вы можете отслеживать [sys.dm_db_resource_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database) и [sys.resource_stats](/sql/relational-databases/system-catalog-views/sys-resource-stats-azure-sql-database) динамические представления управления, чтобы понять процессор, io-ум и потребление памяти. На снижение производительности может влиять нехватка ресурсов в базе данных. Возможно, потребуется изменить объем вычислительных ресурсов и (или) уровень служб в соответствии с различными требованиями к расширению и сжатию рабочих нагрузок.
...
|---|---|
|**Basic**|Приложения с несколькими пользователями и база данных с низкими требованиями к параллелизму, масштабированию и производительности. |
...
При использовании шаблона приложения SaaS или в сценарии консолидации базы данных советуем использовать пул эластичных баз данных. Это позволит сократить расходы. Эластичный пул — это отличный способ достижения консолидации базы данных и оптимизации затрат. Чтобы узнать больше об управлении несколькими базами данных с помощью эластичного пула, см.: [Управление пулами и базами данных.](sql-database-elastic-pool-manage.md#azure-portal-manage-elastic-pools-and-pooled-databases)
