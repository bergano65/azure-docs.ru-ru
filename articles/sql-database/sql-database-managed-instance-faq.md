...
**Где можно найти список возможностей, поддерживаемых управляемым экземпляром?**
...
**Где можно найти технические характеристики и ограничения ресурсов для управляемого экземпляра?**
...
Об ошибках и известных проблемах см. в статье [Известные проблемы](sql-database-release-notes.md#known-issues).

## <a name="new-features"></a>Новые функции

**Где можно найти новейшие функции и функции в общедоступной предварительной версии?**

Для новых и предварительных версий функций см. [заметки о выпуске](sql-database-release-notes.md?tabs=managed-instance).
...
**Может ли управляемый экземпляр иметь то же имя, что и локальная SQL Server?**

Изменение имени управляемого экземпляра не поддерживается.

Невозможно изменить зону DNS по умолчанию для управляемого экземпляра *. Database.Windows.NET* . 
...
## <a name="move-db-from-mi"></a>Перемещение базы данных из MI 

**Как переместить базу данных из управляемого экземпляра обратно в SQL Server или базу данных SQL Azure?**
...
## <a name="migrate-instance-db"></a>Перенести базу данных экземпляра
...
Если в подсети, в которой подготовлен управляемый экземпляр, нет достаточного IP-адреса, необходимо создать в нем новую подсеть и новый управляемый экземпляр. Мы также рекомендуем создать подсеть с дополнительными IP-адресами, алокатед так, что будущие операции обновления будут избегать подобной ситуации (для определения размера подсети, см. статью [как определить размер](sql-database-managed-instance-determine-size-vnet-subnet.md)подсети vnet). После подготовки нового экземпляра можно вручную создать резервную копию и восстановить данные между старым и новым экземплярами или выполнить [восстановление на момент времени](sql-database-managed-instance-point-in-time-restore.md?tabs=azure-powershell)с перекрестным экземпляром. 
...
## <a name="back-up-storage-cost"></a>Стоимость резервного копирования хранилища 
...
**Существует ли способ отслеживанию затрат на выставление счетов за управляемый экземпляр?**
...
Плоскость управления управляемым экземпляром поддерживает правила NSG, защищающие порты управления.
...
В дополнение к правилам NSG встроенный брандмауэр защищает экземпляр на сетевом уровне. В связи с прикладным уровнем защищенные сертификаты защищаются.
  
...
## <a name="cost-saving-use-cases"></a>Затраты на сохранение вариантов использования

**Где можно найти варианты использования и сократить расходы с помощью управляемого экземпляра?**
...
Подключение к управляемому экземпляру по IP-адресу не поддерживается. Имя узла управляемого экземпляра сопоставляется с подсистемой балансировки нагрузки перед виртуальным кластером управляемых экземпляров. Поскольку один виртуальный кластер может размещать несколько управляемых экземпляров, соединение с ним не может быть направлено на соответствующий управляемый экземпляр без указания его имени.

Дополнительные сведения об архитектуре виртуальных кластеров управляемых экземпляров см. в статье [Архитектура подключения виртуального кластера](sql-database-managed-instance-connectivity-architecture.md#virtual-cluster-connectivity-architecture).
...
**Разделы справки устранить проблемы с производительностью управляемого экземпляра?**
...
**Можно ли восстановить зашифрованную базу данных на управляемый экземпляр?**
...
## <a name="migrate-from-single-db"></a>Миграция из одной базы данных 

**Как выполнить миграцию из одного или эластичного пула базы данных SQL Azure в управляемый экземпляр?**
