...
> [!IMPORTANT]
> Успешное выполнение эластичного запроса по большому набору баз данных в значительной степени зависит от наличия каждой из баз данных во время выполнения запроса. Если одна из баз данных недоступна, весь запрос выйдет из строя. Если вы планируете заставить зазапрос и сотни или тысячи баз данных одновременно, убедитесь, что в вашем клиентском приложении встроена логика повторной работы, или рассмотрите возможность использования [рабочих мест в базе данных Elastic Database](https://docs.microsoft.com/azure/sql-database/sql-database-job-automation-overview#elastic-database-jobs-preview) (предварительный просмотр) и запроса небольших подмножествах баз данных, консолидации результатов каждого запроса в одном пункте назначения.
...
* Упругий запрос работает только с базой данных Azure S'L. Вы не можете использовать его для запроса на внутренний сервер S'L, или сервера S'L в VM.
