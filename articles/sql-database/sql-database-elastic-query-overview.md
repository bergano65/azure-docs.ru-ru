...
> [!IMPORTANT]
> Успешное выполнение эластичного запроса по большому набору баз данных полагается на доступность каждой базы данных во время выполнения запроса. Если одна из баз данных недоступна, весь запрос завершится ошибкой. Если вы планируете запрашивать сотни или тысячи баз данных одновременно, убедитесь, что клиентское приложение имеет встроенную логику повторных попыток, или рассмотрите возможность использования [заданий эластичной базы данных](https://docs.microsoft.com/azure/sql-database/sql-database-job-automation-overview#elastic-database-jobs-preview) (Предварительная версия) и запросов к меньшим подмножествам баз данных, объединяя результаты каждого запроса в единое место назначения.
...
* Эластичный запрос работает только с базой данных SQL Azure. Его нельзя использовать для выполнения запросов к локальным SQL Server или SQL Server на виртуальной машине.
