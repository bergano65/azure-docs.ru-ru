...
- Включение конечных точек служб для виртуальной сети для базы данных SQL Azure также включает конечные точки для служб MySQL и PostgreSQL Azure. Тем не менее, если конечные точки включены в базе данных, подключение через них к экземплярам MySQL или PostgreSQL может завершиться ошибкой.
