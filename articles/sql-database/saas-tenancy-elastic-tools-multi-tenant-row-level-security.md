...
[Средства эластичных баз данных](sql-database-elastic-scale-get-started.md) и [безопасность на уровне строк (RLS)][rls] взаимодействуют, чтобы обеспечить масштабирование уровня данных приложения с несколькими клиентами с помощью базы данных SQL Azure. Вместе эти технологии позволяют создавать приложения с высокой степенью масштабирования уровня данных. Уровень данных поддерживает мультитенантные сегменты и использует **ADO.NET SqlClient** или **Entity Framework**. Дополнительные сведения см. в статье [Мультитенантные шаблоны клиента в Базе данных SaaS](saas-tenancy-app-design-patterns.md).

- **Средства эластичных баз данных** позволяют разработчикам развернуть уровень данных, используя стандартные методы сегментирования, набор библиотек .NET и шаблоны служб Azure. Управление сегментами с помощью [клиентской библиотеки эластичной базы данных][s-d-elastic-database-client-library] помогает автоматизировать и оптимизировать многие задачи инфраструктуры, обычно связанные с сегментированием.
...
- [Приложение Tailspin Surveys](../guidance/guidance-multitenant-identity-tailspin.md)
