...
ms.locfileid: "72893535"
---
# <a name="azure-sql-transparent-data-encryption-with-customer-managed-keys-in-azure-key-vault-bring-your-own-key-support"></a>Прозрачное шифрование данных Azure SQL с управляемыми клиентом ключами в Azure Key Vault: создание собственных ключей поддержки

[Прозрачное шифрование данных (TDE)](https://docs.microsoft.com/sql/relational-databases/security/encryption/transparent-data-encryption) с помощью Azure Key Vault позволяет зашифровать ключ шифрования базы данных (DEK), используя управляемый пользователем асимметричный ключ, который называется предохранителем TDE. Обычно это также называется поддержкой создания собственных ключей (BYOK) для прозрачного шифрования данных.  В сценарии создания собственных ключей предохранитель TDE хранится в управляемом клиентом экземпляре [Azure Key Vault](https://docs.microsoft.com/azure/key-vault/key-vault-secure-your-key-vault) — облачной системы Azure для управления внешними ключами. Предохранитель TDE может быть [создан](https://docs.microsoft.com/azure/key-vault/about-keys-secrets-and-certificates) хранилищем ключей или [передан](https://docs.microsoft.com/azure/key-vault/key-vault-hsm-protected-keys) в хранилище ключей с предварительно установленного устройства HSM. Ключ шифрования TDE, который хранится на загрузочной странице базы данных, шифруется и расшифровывается с помощью предохранителя TDE. Он хранится в Azure Key Vault и никогда не покидает это хранилище ключей.  База данных SQL должна иметь разрешения для хранилища ключей, принадлежащего клиенту, для шифрования и расшифрования DEK. Если разрешения логического сервера SQL Server для хранилища ключей отозваны, база данных будет недоступна, подключения будут отвергнуты и все данные шифруются. Для Базы данных SQL Azure предохранитель TDE настраивается на уровне логического сервера SQL и наследуется всеми базами данных, связанными с этим сервером. Для [Управляемого экземпляра SQL Azure](https://docs.microsoft.com/azure/sql-database/sql-database-howto-managed-instance) предохранитель TDE настраивается на уровне экземпляра и наследуется всеми *зашифрованными* базами данных этого экземпляра. В этом документе термин *сервер* обозначает как сервер, так и экземпляр, если не указано иное.
...
- Если вы планируете перемещение клиента, TDE с AKV необходимо будет перенастроить, чтобы узнать больше о [перемещении ресурсов](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-move-resources).
...
- Создайте хранилище ключей с включенной [обратимым удалением](https://docs.microsoft.com/azure/key-vault/key-vault-ovw-soft-delete) и защитой от потери данных в случае случайного удаления ключа — или хранилища ключей. Необходимо включить свойство "обратимое удаление" в хранилище ключей с помощью [интерфейса командной строки](https://docs.microsoft.com/en-us/azure/key-vault/key-vault-soft-delete-cli#enabling-soft-delete) или [PowerShell](https://docs.microsoft.com/en-us/azure/key-vault/key-vault-soft-delete-powershell#enabling-soft-delete) (этот параметр еще не доступен на портале AKV, но необходим для Azure SQL):  
...
- Предоставьте серверу Базы данных SQL доступ к хранилищу ключей с помощью удостоверения Azure Active Directory (Azure AD).  При использовании пользовательского интерфейса портала удостоверение Azure AD, которое предоставляет серверу права доступа к хранилищу ключей, создается автоматически.  Если вы настраиваете TDE с поддержкой BYOK через PowerShell, удостоверение Azure AD нужно создать отдельно и проверить успешность его создания. Подробные пошаговые инструкции по настройке TDE с поддержкой BYOK с помощью PowerShell см. в [этой статье](transparent-data-encryption-byok-azure-sql-configure.md), а по настройке TDE с поддержкой BYOK для управляемого экземпляра – [в этой](https://aka.ms/sqlmibyoktdepowershell).
   ...
   > [!NOTE]
   > Если удостоверение Azure AD **случайно удалено или разрешения сервера отзываются** с помощью политики доступа хранилища ключей или случайно перемещаются в другой клиент, сервер теряет доступ к хранилищу ключей и TDE зашифрованные базы данных. будет недоступно, и входы будут запрещены до тех пор, пока не восстановлены удостоверение и разрешения Azure AD логического сервера.  

- При использовании брандмауэров и виртуальных сетей с Azure Key Vault необходимо разрешить доверенным службам Майкрософт обходить этот брандмауэр. Выберите Да.

   > [!NOTE]
   > Если TDE зашифрованные базы данных SQL потеряют доступ к хранилищу ключей, так как они не могут обходить брандмауэр, базы данных будут недоступны, а входы отклоняются до тех пор, пока не будут восстановлены разрешения на обход брандмауэра.
...
- Чтобы обеспечить высокий уровень доступности зашифрованных баз данных, настройте каждый сервер базы данных SQL с двумя хранилищами ключей Azure, которые находятся в разных регионах.
...
- Создайте ключ шифрования на локальном устройстве HSM. Убедитесь, что это асимметричный ключ RSA 2048 или RSA HSM 2048, который можно поместить в Azure Key Vault.
...
- Если вы используете ключ с датой истечения срока действия — реализуйте систему предупреждений об истечении срока действия, чтобы повернуть ключ до истечения срока его действия: **после истечения срока действия ключа зашифрованные базы данных теряют доступ к их предохранителю TDE и будут недоступны** , и все входы в систему будут запрещены. ключ был повернут в новый ключ и выбран в качестве нового ключа и предохранителя TDE по умолчанию для логического сервера SQL Server.
...
### <a name="guidelines-for-monitoring-the-tde-with-azure-key-vault-configuration"></a>Рекомендации по мониторингу TDE с помощью конфигурации Azure Key Vault

Если логический сервер SQL Server теряет доступ к TDE предохранителю, управляемому клиентом, в Azure Key Vault, база данных блокирует все подключения и будет недоступна в портал Azure.  Наиболее распространенные причины этого:
- Хранилище ключей случайно удалено или защищено брандмауэром
- Ключ хранилища ключей случайно удален, отключен или просрочен
- Случайное удаление экземпляра логического SQL Server
- Основные разрешения, связанные с ключами для экземпляра логического SQL Server отзыва

 > [!NOTE]
 > База данных будет автоматически восстановлена и переведена в оперативный режим, если доступ к управляемому клиентом TDE предохранителю восстанавливается в течение 48 часов.  Если база данных недоступна из-за периодического сбоя сети, никаких действий не требуется, и базы данных будут автоматически возвращаться в оперативный режим.
  
- Дополнительные сведения об устранении неполадок в существующих конфигурациях см. в разделе [Устранение неполадок TDE](https://docs.microsoft.com/sql/relational-databases/security/encryption/troubleshoot-tde)
...
Если ключ, который может потребоваться для восстановления резервной копии, больше не находится в исходном хранилище ключей, возвращается следующее сообщение об ошибке: "целевой сервер `<Servername>` не имеет доступа ко всем URI AKV, созданным между \<меток времени #1 > и \<меток времени #2 > . Please retry operation after restoring all AKV Uris." (Целевой сервер не имеет доступа ко всем URI AKV, созданным с <метка времени №1> по <метка времени №2>. Повторите операцию после восстановления всех URI AKV).
