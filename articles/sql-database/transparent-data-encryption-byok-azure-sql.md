...
В этом сценарии ключ, используемый для шифрования ключа шифрования базы данных (DEK), который называется предохранителем TDE, представляет собой управляемый клиентом асимметричный ключ, хранящийся в принадлежащем заказчике и управляемом клиентом [Azure Key Vaultе (AKV)](https://docs.microsoft.com/azure/key-vault/key-vault-secure-your-key-vault), облачном управлении внешними ключами. системой. Key Vault — это высокодоступное и масштабируемое безопасное хранилище для криптографических ключей RSA, которое поддерживается FIPS 140-2 уровня 2 проверенных аппаратных модулей безопасности (HSM). Он не разрешает прямой доступ к сохраненному ключу, но предоставляет службы шифрования и расшифровки с помощью ключа для полномочных сущностей. Ключ может создаваться хранилищем ключей, импортировано или [перенесено в хранилище ключей из локального устройства HSM](https://docs.microsoft.com/azure/key-vault/key-vault-hsm-protected-keys).
...
- Для ключа не может быть задана дата активации или окончания срока действия.

- Ключ должен находиться в состоянии Enabled в хранилище ключей.
...
Если ключ, необходимый для восстановления резервной копии, больше не доступен для целевого сервера, то при восстановлении будет возвращено следующее сообщение об ошибке: "целевой сервер `<Servername>` не имеет доступа ко всем URI AKV, созданным между \<меткой времени #1 > и @no__t_ > #2 меток времени 2_.\< Повторите операцию после восстановления всех URI AKV. "
