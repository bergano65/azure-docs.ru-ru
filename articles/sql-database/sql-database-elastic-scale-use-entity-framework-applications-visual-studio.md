---
title: Использование эластичной клиентской библиотеки баз данных с рамочной системой entity
...
В этом документе показаны изменения, которые следует внести в приложение Entity Framework для интеграции со [средствами эластичных баз данных](sql-database-elastic-scale-introduction.md). Основное внимание уделяется составлению [управления осколочно-осколочно-разбойными](sql-database-elastic-scale-shard-map-management.md) картами и [зависящим от данных направлению](sql-database-elastic-scale-data-dependent-routing.md) с помощью подхода, ориентированного на рамочный **код** сущности. Руководство [Code First — создание базы данных](https://msdn.microsoft.com/data/jj193542.aspx) для Entity Framework используется в этом документе как пример. Примером кода для этого документа является часть набора примеров для эластичной базы данных в Visual Studio.
...
* Загрузите [эластичные dB Tools для образца интеграции системы Azure S- Entity - Entity Framework Integration.](https://github.com/Azure/elastic-db-tools/) Распакуйте образец в выбранную папку.
