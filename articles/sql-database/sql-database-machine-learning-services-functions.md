...
- Чтобы запустить пример кода в этих упражнениях, необходимо сначала включить [базу данных SQL Azure с службы машинного обучения (с R)](sql-database-machine-learning-services-overview.md) .
...
Для простоты мы используем пакет R `stats` , установленный и загруженный по умолчанию в базе данных SQL Azure с помощью службы машинного обучения (Предварительная версия). Пакет содержит сотни функций для общих статистических задач, между которыми они `rnorm` работают. Эта функция создает указанное число случайных чисел с использованием нормального распределения, учитывая стандартное отклонение и средства.
