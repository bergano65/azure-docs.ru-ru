...
| :----------------------------- | ----- | ----- |
| **CREATE INDEX** -определяет индексы, которые может повысить производительность рабочей нагрузки, создает индексы и автоматически проверяет, что улучшена производительность запросов. | Да | Нет | 
| **DROP INDEX** -идентифицирует избыточных и повторяющихся индексов ежедневно, за исключением уникальные индексы и индексы, которые не использовались в течение длительного времени (> 90 дней). Обратите внимание, что сейчас эта функция несовместима с приложениями, которые используют переключения секций и подсказки индекса. | Да | Нет |
| **ПРИМЕНИТЬ последний ОПТИМАЛЬНЫЙ ПЛАН** (автоматическое исправление плана) — определяет SQL-запросы, используя план выполнения, который выполняется медленнее, чем предыдущие хорошего плана и запросы с помощью последний известный удачный план плана вместо регрессионного. | Да | Да |
