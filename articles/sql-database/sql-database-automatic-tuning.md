...
| **ПРИМЕНИТЬ последний ОПТИМАЛЬНЫЙ ПЛАН** - идентифицирует SQL-запросы, используя план выполнения, который выполняется медленнее, чем предыдущие хорошего плана и запросы с помощью последний известный удачный план плана вместо регрессионного. | Yes | Yes |
