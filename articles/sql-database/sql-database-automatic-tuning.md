...
Автоматическая настройка должна быть включена в вашей подписке. Чтобы узнать, как включить автоматическую настройку с помощью портала Azure, см. статью [Включение автоматической настройки](sql-database-automatic-tuning-enable.md).
...
| Параметр автоматической настройки | Поддержка отдельной базы данных и базы данных в составе пула | Поддержка баз данных экземпляра |
| :----------------------------- | ----- | ----- |
| **CREATE INDEX** -определяет индексы, которые может повысить производительность рабочей нагрузки, создает индексы и автоматически проверяет, что улучшена производительность запросов. | Yes | Нет  | 
| **DROP INDEX** -идентифицирует избыточных и повторяющихся индексов ежедневно, за исключением уникальные индексы и индексы, которые не использовались в течение длительного времени (> 90 дней). Обратите внимание, что сейчас эта функция несовместима с приложениями, которые используют переключения секций и подсказки индекса. | Yes | Нет  |
| **ПРИМЕНИТЬ последний ОПТИМАЛЬНЫЙ ПЛАН** (автоматическое исправление плана) — определяет SQL-запросы, используя план выполнения, который выполняется медленнее, чем предыдущие хорошего плана и запросы с помощью последний известный удачный план плана вместо регрессионного. | Yes | Yes |

Автоматическая настройка идентифицирует рекомендации для использования параметров **Создать индекс**, **Удалить индекс** и **Force last good plan** (Принудительное выполнение последнего хорошего плана), с помощью которых можно оптимизировать производительность базы данных, и отображает их на [портале Azure](sql-database-advisor-portal.md). Получить эти параметры можно с помощью [T-SQL](https://docs.microsoft.com/sql/t-sql/statements/alter-database-transact-sql-set-options?view=azuresqldb-current) и [REST API](https://docs.microsoft.com/rest/api/sql/serverautomatictuning). Дополнительные сведения о применить последний ОПТИМАЛЬНЫЙ ПЛАН и настроить параметры автоматической настройки через T-SQL, см. в разделе [Автоматическая настройка вводит автоматическое исправление плана](https://azure.microsoft.com/blog/automatic-tuning-introduces-automatic-plan-correction-and-t-sql-management/).
...
Если вы вручную применяете рекомендации по настройке, автоматическая проверка производительности и механизмы возврата недоступны. Кроме того вручную примененных рекомендаций останется активной и отображаемых в список рекомендаций для 24 – 48 часов. Прежде чем система автоматически снимает их. Если вы хотите удалить рекомендацию раньше, можно отменить их вручную.

Параметры автоматической настройки можно включать и отключать отдельно для каждой базы данных, а также их можно настроить на серверах Баз данных SQL и применять для каждой базы данных, которая наследует параметры сервера. Серверы Баз данных SQL могут наследовать значения Azure по умолчанию для параметров автоматической настройки. Поддерживаемые сейчас значения Azure по умолчанию: FORCE_LAST_GOOD_PLAN включено, CREATE_INDEX включено, а DROP_INDEX отключено.
