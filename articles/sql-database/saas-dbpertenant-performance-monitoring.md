---
title: 'Приложение Saas: Мониторинг производительности многих баз данных'
...
* Чтобы управлять временным возрастанием нагрузки *отдельной* базы данных, **ее можно изъять из пула и назначить ей индивидуальный объем вычислительных ресурсов**. После снижения нагрузки эту базу данных можно вернуть в пул. Когда об этом известно заранее, базы данных могут быть перемещены упреждаюстраженно, чтобы гарантировать, что база данных всегда имеет ресурсы, в которые она нуждается, и чтобы избежать влияния на другие базы данных в пуле. Если эта необходимость предсказуемая, например из-за увеличения уровня продаж билетов на популярное мероприятие, это поведение управления можно интегрировать в приложение.
...
Для сценариев большого объема, где вы работаете со многими ресурсами, можно использовать [журналы Azure Monitor.](saas-dbpertenant-log-analytics.md) Это отдельная служба Azure, предоставляющая аналитику по испускаемым журналам, собранным в рабочей области analytics log. Журналы Azure Monitor могут собирать телеметрию из многих служб и использоваться для запроса и набора оповещений.
...
|:--|:--|
| 2 | Создание нормальной интенсивности нагрузки (приблизительно 40 DTU) |
| 3 | Создание нагрузки с более длительными и частыми пиками на базу данных.|
| 4 | Генерация нагрузки с более высокими очередями DTU в базе данных (приблизительно 80 DTU)|
| 5 | Создание нормальной нагрузки плюс высокая нагрузка на одного арендатора (приблизительно 95 DTU)|
...
1. В **интегрированной среде сценариев PowerShell** откройте скрипт …\\Learning Modules (Модули для обучения)\\Performance Monitoring and Management (Мониторинг производительности и управление ею)\\*Demo-PerformanceMonitoringAndManagement.ps1*. Не закрывайте этот скрипт, так как он еще понадобится в рамках этого руководства.
1. Установить **$DemoScenario** = **2**, *Генерировать нормальную нагрузку интенсивности.*
...
1. Установите *$DemoScenario* = **3**, _Создайте нагрузку с более длинными и более частыми всплесками в базе данных,_ чтобы увеличить интенсивность совокупной нагрузки на пул без изменения пиковой нагрузки, требуемой каждой базой данных.
1. Нажмите клавишу **F5**, чтобы применить эту нагрузку ко всем базам данных клиента.
...
1. В шаблоне **Эластичный пул** сделайте следующее.
...
1. Установите **$DemoScenario No 5, Создайте нормальную нагрузку плюс высокую нагрузку на одного арендатора (примерно 95 DTU).**
...
1. Щелкните пул, в котором содержится **contosoconcerthall**. Пул можно найти в разделе **Эластичный пул**.
...
3. **Нажмите на диаграмму мониторинга базы данных Elastic** **chart** и он открывает страницу **использования ресурсов базы данных,** где вы можете контролировать любую из баз данных. Так вы можете просмотреть сведения для базы данных *contosoconcerthall*.
4. В списке баз данных щелкните **contosoconcerthall**.
5. Выберите **Ценовая категория (единицы DTU масштабирования)**, чтобы открыть страницу **Настройка производительности**, на которой можно настроить индивидуальный объем вычислительных ресурсов для базы данных.
...
**Упреждающее масштабирование.** В упражнении выше показано, как масштабировать изолированную базу данных. Если бы руководство концертного зала Contoso проинформировало Wingtips о предстоящей продаже билетов, база данных могла бы быть перемещена из пула упреждающе. В противном случае можно реализовать отправку оповещений, чтобы определять активность в пуле или базе данных. Вряд ли вы хотите, чтобы рост нагрузки влиял на производительность других клиентов в пуле. Если клиент может предсказать, как долго ему потребуются дополнительные ресурсы, вы можете настроить модуль runbook службы автоматизации Azure, чтобы изъять базу данных из пула, а затем вернуть ее по заданному расписанию.
...
* [Автоматизация Azure](../automation/automation-intro.md)
* [Журналы Azure Monitor](saas-dbpertenant-log-analytics.md) - Настройка и использование учебника по журналам Azure Monitor
