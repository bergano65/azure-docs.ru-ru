...
|:--|:--|
| 2 | Создать нормальную нагрузку с интенсивностью (приблизительно 40 DTU) |
| 3 | Создание нагрузки с более длительными и частыми пиками на базу данных.|
| 4 | Создание нагрузки с большими нагрузками DTU на базу данных (приблизительно 80 DTU)|
| 5 | Создание обычной нагрузки и высокая нагрузка на один клиент (приблизительно 95 DTU)|
...
1. На *портале Azure* щелкните сервер *tenants1-dpt-\<пользователь\>* и выберите [Pool1](https://portal.azure.com).
...
1. Чтобы выполнить скрипт *создания нагрузки с более длительными и частыми пиками на базу данных*, задайте параметр  = $DemoScenario_3_. Это позволит повысить интенсивность общей нагрузки пула, не изменяя необходимую пиковую нагрузку каждой базы данных.
1. Нажмите клавишу **F5**, чтобы применить эту нагрузку ко всем базам данных клиента.
...
1. Установите **$DemoScenario = 5, создайте нормальную нагрузку, а также высокую нагрузку на один клиент (приблизительно 95 DTU).**
...
3. Щелкните **диаграмму** **мониторинг эластичной базы данных** и откроется страница **использование ресурсов базы данных** , где можно отслеживать любые базы данных. Так вы можете просмотреть сведения для базы данных *contosoconcerthall*.
...
**Упреждающее масштабирование.** В упражнении выше показано, как масштабировать изолированную базу данных. Если управление зал Contoso концерта было уведомлено wingtips от незавершенной продажи билета, база данных могла быть перемещена из пула в вытесненном состоянии. В противном случае можно реализовать отправку оповещений, чтобы определять активность в пуле или базе данных. Вряд ли вы хотите, чтобы рост нагрузки влиял на производительность других клиентов в пуле. Если клиент может предсказать, как долго ему потребуются дополнительные ресурсы, вы можете настроить модуль runbook службы автоматизации Azure, чтобы изъять базу данных из пула, а затем вернуть ее по заданному расписанию.
...
* [Azure Monitor журналов](saas-dbpertenant-log-analytics.md) . Руководство по настройке и использованию журналов Azure Monitor
