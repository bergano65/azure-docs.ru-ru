...
- Создание новой базы данных на том же логическом сервере, восстановленной до указанной точки во времени в пределах срока хранения.
- Создание базы данных на том же логическом сервере, восстановленной до момента удаления, если исходная база данных была удалена.
- Создание базы данных на любом логическом сервере в том же регионе, восстановленной до точки во времени последней резервной копии.
- Создание базы данных на любом логическом сервере в любом другом регионе, восстановленной до точки во времени последней реплицированной резервной копии.

После настройки [резервного копирования для длительного хранение](sql-database-long-term-retention.md) также можно использовать любую резервную копия LTR и любой логический сервер для создания новой базы данных в любом регионе.

> [!IMPORTANT]
> При восстановлении существующую базу данных невозможно перезаписать.

При использовании уровня службы Standard или Premium для восстановленной базы данных, взимается дополнительная оплата за хранилище при следующих условиях.
...
Чтобы восстановить отдельную базу данных, базу данных в составе пула или базу данных Управляемого экземпляра до точки во времени с помощью портала Azure, откройте страницу для базы данных и щелкните **Восстановление** на панели инструментов.
...
> [!Note]
> В Управляемом экземпляре геовосстановление недоступно.
...
- Для восстановления отдельной базы данных или базы данных в составе пула см. описание командлета [Restore-AzureRmSqlDatabase](https://docs.microsoft.com/powershell/module/azurerm.sql/restore-azurermsqldatabase).
...
- Чтобы восстановить базу данных Управляемого экземпляра, см. раздел [Восстановление базы данных в Управляемом экземпляре SQL Azure до точки во времени с помощью библиотеки AzureRm.Sql PowerShell](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2018/06/28/point-in-time-restore-of-a-database-on-azure-sql-managed-instance-using-azurerm-sql-powershell-library/).
...
Для восстановления отдельной базы данных или базы данных в составе пула с помощью Azure CLI см. описание команды [az sql db restore](https://docs.microsoft.com/cli/azure/sql/db#az-sql-db-restore).
