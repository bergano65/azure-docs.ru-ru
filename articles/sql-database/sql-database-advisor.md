...
После применения рекомендации по созданию индекса база данных SQL Azure сравнивает производительность запросов с базовой производительностью. Если новый индекс улучшит производительность, рекомендация будет отмечена как успешная и будет создан отчет о результатах. Если индекс не улучшил производительность, он будет автоматически отменен. База данных SQL использует этот процесс, чтобы гарантировать, что рекомендации улучшают производительность базы данных.
