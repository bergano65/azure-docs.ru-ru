---
title: Рекомендации по работе советников по базам данных для отдельных и объединенных баз данных
description: База данных Azure S'L предоставляет рекомендации для отдельных и объединенных баз данных, которые могут повысить производительность запроса в базе данных Azure S'L.
...
ms.locfileid: "79214137"
---
# <a name="database-advisor-performance-recommendations-for-single-and-pooled-databases"></a>Рекомендации по работе советника по базе данных для отдельных и объединенных баз данных

База данных SQL Azure развивается и адаптируется вместе с вашим приложением. Для отдельных и объединенных баз данных база данных s'L Database имеет ряд рекомендаций по базам данных, которые предоставляют индивидуальные рекомендации, которые позволяют максимизировать производительность. Эти консультанты по базам данных постоянно оценивают и анализируют историю использования и предоставляют рекомендации, основанные на шаблонах рабочей нагрузки, которые помогают повысить производительность.

## <a name="performance-overview"></a>Обзор производительности

Обзор производительности предоставляет сводку производительности базы данных и помогает вам с настройкой производительности и устранением неполадок.

![Обзор производительности для базы данных Azure S'L](./media/sql-database-performance/performance-overview-annotated.png)

- Элемент **Рекомендации** содержит разбор рекомендаций по настройке базы данных (если рекомендаций много, отображаются только первые три). Нажав на эту плитку, вы перенесете **[параметры рекомендации производительности.](sql-database-advisor-portal.md#viewing-recommendations)**
...
- На **плитке автоматической настройки** показана **[конфигурация автоматической настройки](sql-database-automatic-tuning-enable.md)** для базы данных (параметры настройки, которые автоматически применяются к базе данных). Щелкнув по этой плитке, вы откроете диалоговое окно настройки автоматизации.
...
## <a name="performance-recommendation-options"></a>Варианты рекомендаций по производительности

Варианты рекомендаций производительности, доступные для отдельных и объединенных баз данных в базе данных Azure S'L:

| Рекомендация по производительности | Поддержка отдельной базы данных и базы данных в пуле | Поддержка базы данных инстанций |
| :----------------------------- | ----- | ----- |
| **Создание рекомендаций по индексу** - рекомендует создание индексов, которые могут повысить производительность рабочей нагрузки. | Да | нет |
| **Рекомендации по индексу падения** - Рекомендует ежедневно еснять избыточные и дублирующие индексы, за исключением уникальных индексов и индексов, которые не использовались в течение длительного времени (>90 дней). Обратите внимание, что эта функция несовместима с приложениями, которые используют переключения секций и подсказки индекса. Удаление неиспользованных индексов не поддерживается для уровней premium и Business Critical. | Да | нет |
| **Parameterize рекомендации запросов (предварительный просмотр)** - Рекомендует принудительной параметризации в тех случаях, когда у вас есть один или несколько запросов, которые постоянно перекомпилируются, но в конечном итоге с тем же планом выполнения запроса. | Да | нет |
| **Рекомендации по исправлению схемных проблем (предварительный просмотр)** - Рекомендации по коррекции схемы появляются, когда служба базы данных S'L замечает аномалию в количестве ошибок, связанных с схемой, связанных с S'L, которые происходят в вашей базе данных S'L. Корпорация Майкрософт в данный момент не поддерживает рекомендации по устранению ошибок схемы. | Да | нет |

![Рекомендации по производительности для базы данных Azure S'L](./media/sql-database-performance/performance-recommendations-annotated.png)

Чтобы применить [рекомендации](sql-database-advisor-portal.md#applying-recommendations)по производительности, см. Для просмотра состояния рекомендаций [см.](sql-database-advisor-portal.md#monitoring-operations)
...
Индексы, созданные с помощью рекомендаций, всегда помечаются как созданные автоматически. Вы можете увидеть, какие индексы автоматически создаются, посмотрев [на представление sys.indexes.](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-indexes-transact-sql) Автоматически созданные индексы не блокируют команды ALTER/RENAME.
...
После применения рекомендации по созданию индекса база данных SQL Azure сравнивает производительность запросов с базовой производительностью. Если новый индекс улучшит производительность, рекомендация будет отмечена как успешная и будет создан отчет о результатах. Если индекс не улучшил производительность, он автоматически возвращается. База данных SQL использует этот процесс, чтобы гарантировать, что рекомендации улучшают производительность базы данных.
...
Если cPU, io-й убор данных или io-ввоза журнала превышают 80% в предыдущие 30 минут, рекомендация по индексу создания откладывается. Если после создания индекса доступный объем хранилища будет ниже 10 %, рекомендация перейдет в состояние ошибки. Если по прошествии нескольких дней служба автонастройки снова определит, что создание индекса полезно, процесс запустится повторно.
...
## <a name="parameterize-queries-recommendations-preview"></a>Параметризировать рекомендации запросов (предварительный просмотр)
...
"Схема проблемы" представляют собой класс ошибок синтаксиса в сервере S'L. Они происходят, когда определение запроса SQL и определение схемы базы данных не согласованы. Например, один из столбцов, ожидаемых запросом, может отсутствовать в целевой таблице или наоборот.

Рекомендация «Проблема схемы исправления» появляется, когда служба базы данных Лазурного Газа данных S'L замечает аномалию в количестве ошибок, связанных с схемой, связанных с S'L, которые происходят в вашей базе данных S'L. В следующей таблице показаны ошибки, которые относятся к схеме.
...
Разработчики могут рассмотреть возможность разработки пользовательских приложений с использованием рекомендаций по производительности для базы данных Azure S'L. Все рекомендации, перечисленные на портале базы данных, можно получить через [Get-AzSqlDatabaseRecommendedAction](https://docs.microsoft.com/powershell/module/az.sql/get-azsqldatabaserecommendedaction) API.
