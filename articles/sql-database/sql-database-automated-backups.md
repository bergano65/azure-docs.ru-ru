...
База данных SQL автоматически создает резервные копии баз данных, которые хранятся в течение периода от 7 до 35 дней, и использует геоизбыточное хранилище Azure с доступом на чтение (RA-GRS), чтобы обеспечить их сохранение, даже если центр обработки данных недоступен. Эти резервные копии создаются автоматически и без дополнительной платы. Для создания резервных копий не нужно ничего делать, и можно [изменить их срок хранения](#how-to-change-the-pitr-backup-retention-period). Резервные копии базы данных являются важной частью любой стратегии непрерывности бизнес-процессов и аварийного восстановления, так как они защищают данные от случайного повреждения или удаления. Если правила безопасности требуют, чтобы резервные копии были доступны в течение продолжительного времени (до 10 лет), можно настроить [долгосрочное хранение](sql-database-long-term-retention.md).
...
В каждой Базе данных SQL установлен период хранения резервных копий по умолчанию (от 7 до 35 дней). Его длительность зависит от [модели приобретения и уровня служб](#pitr-retention-period). Вы можете изменить период хранения резервных копий для базы данных на логическом сервере Azure. Дополнительные сведения см. в разделе [Как изменить период хранения резервной копии](#how-to-change-the-pitr-backup-retention-period).
...
Если вы используете [модель приобретения на основе виртуальных ядер](sql-database-service-tiers-vcore.md), срок хранения резервных копий по умолчанию составляет 7 дней (для отдельных баз данных, баз данных в составе пула и баз данных Управляемых экземпляров). Для всех баз данных SQL Azure (отдельных баз данных, баз данных в составе пула и баз данных Управляемых экземпляров) можно [изменить период хранения резервных копий, установив срок до 35 дней](#how-to-change-the-pitr-backup-retention-period).
...
База данных SQL поддерживает самообслуживание для восстановления на момент времени (PITR) путем автоматического создания полной резервной копии, разностных резервных копий и резервных копий журналов транзакций. Полные резервные копии базы данных создаются каждую неделю, разностные резервные копии — как правило, каждые 12 часов, а резервные копии журналов транзакций создаются каждые 5 – 10 минут. Периодичность архивации зависит от объема вычислительных ресурсов и числа действий, производимых с базой данных. Первое полное резервное копирование планируется сразу после создания базы данных. Обычно оно занимает не более 30 минут, но для базы данных большого размера может потребоваться больше времени. Например, начальное резервное копирование будет выполняться дольше для восстановленной базы данных или копии базы данных. После первого полного резервного копирования все последующие операции резервного копирования планируются автоматически и управляются без участия пользователя в фоновом режиме. Точное время создания всех копий базы данных определяет служба Базы данных SQL с учетом распределения общей рабочей нагрузки в системе.
...
Изменить период хранения резервных копий PITR по умолчанию можно с помощью портала Azure, PowerShell или REST API. Поддерживаются такие значения: 7, 14, 21, 28 или 35 дней. В следующих примерах показано, как установить период хранения PITR на 28 дней.
