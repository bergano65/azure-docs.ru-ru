...
ms.locfileid: "67813050"
---
# <a name="automated-backups"></a>Автоматическое резервное копирование

База данных SQL автоматически создает резервные копии баз данных, которые хранятся в диапазоне от 7 до 35 дней и использует Azure [геоизбыточное хранилище с доступом для чтения (RA-GRS)](../storage/common/storage-redundancy-grs.md#read-access-geo-redundant-storage) чтобы убедиться, что они сохраняются даже если центр обработки данных недоступен. Эти резервные копии создаются автоматически. Резервные копии базы данных являются важной частью любой стратегии непрерывности бизнес-процессов и аварийного восстановления, так как они защищают данные от случайного повреждения или удаления. Если правила безопасности требует, что резервные копии доступны в течение продолжительного периода времени (до 10 лет), вы можете настроить [Долгосрочное хранение](sql-database-long-term-retention.md) в отдельных базах данных и эластичных пулов.

[!INCLUDE [GDPR-related guidance](../../includes/gdpr-intro-sentence.md)]

## <a name="what-is-a-sql-database-backup"></a>Резервная копия базы данных SQL

База данных SQL использует технологию SQL Server для создания [полного резервного копирования](https://docs.microsoft.com/sql/relational-databases/backup-restore/full-database-backups-sql-server) каждую неделю [разностные резервные копии](https://docs.microsoft.com/sql/relational-databases/backup-restore/differential-backups-sql-server) каждые 12 часов и [резервные копии журналов транзакций](https://docs.microsoft.com/sql/relational-databases/backup-restore/transaction-log-backups-sql-server) каждые 5 – 10 минут. Резервные копии хранятся в [большие двоичные объекты хранилища RA-GRS](../storage/common/storage-redundancy-grs.md#read-access-geo-redundant-storage) , реплицируются на [сопряженный центр обработки данных](../best-practices-availability-paired-regions.md) для защиты от сбоя центра обработки данных. При восстановлении базы данных служба сама определяет, какие резервные копии (полные, разностные или резервные копии журналов транзакций) следует восстановить.

Эти резервные копии позволяют выполнить следующие операции:

- **Восстановить существующую базу данных в определенный момент времени в прошлом** в течение указанного периода, с помощью портала Azure, Azure PowerShell, Azure CLI или REST API. В одной базе данных и пулов эластичных баз данных эта операция создаст новую базу данных на том же сервере, где находится исходная база. В управляемом экземпляре этой операции можно создать копию базы данных или том же или другом управляемом экземпляре в той же подписке.
  - **[Изменить срок хранения резервных копий](#how-to-change-the-pitr-backup-retention-period)**  от 7 до 35 дней для настройки политики резервного копирования.
  - **Изменить политику долгосрочного хранения 10 лет** на одну базу данных и пулов эластичных баз данных с помощью [портала Azure](sql-database-long-term-backup-retention-configure.md#configure-long-term-retention-policies) или [Azure PowerShell](sql-database-long-term-backup-retention-configure.md#use-powershell-to-configure-long-term-retention-policies-and-restore-backups).
- **Восстановить удаленную базу данных до момента ее удаления** или в любое время в течение указанного периода. Удаленную базу данных можно восстановить только в том же логическом сервере или управляемого экземпляра, где был создан исходной базы данных.
...
- **Восстановление базы данных из определенной резервной копии долгосрочного** на одну базу данных или эластичного пула, если база данных настроена с долгосрочную политику хранения (LTR). LTR позволяет восстановить старую версию базы данных с помощью [портала Azure](sql-database-long-term-backup-retention-configure.md#view-backups-and-restore-from-a-backup-using-azure-portal) или [Azure PowerShell](sql-database-long-term-backup-retention-configure.md#use-powershell-to-configure-long-term-retention-policies-and-restore-backups) удовлетворить запрос на соответствие или запустить старую версию приложения. Дополнительные сведения см. в статье [Storing Azure SQL Database Backups for up to 10 years](sql-database-long-term-retention.md) (Хранение резервных копий баз данных SQL Azure до 10 лет).
...
Ознакомьтесь с некоторыми из этих операций, используя следующие примеры:

| | Портал Azure | Azure PowerShell |
|---|---|---|
| Изменение периода удержания резервных копий | [Отдельная база данных](sql-database-automated-backups.md#change-pitr-backup-retention-period-using-the-azure-portal) <br/> [Управляемый экземпляр](sql-database-automated-backups.md#change-pitr-for-a-managed-instance) | [Отдельная база данных](sql-database-automated-backups.md#change-pitr-backup-retention-period-using-powershell) <br/>[Управляемый экземпляр](https://docs.microsoft.com/powershell/module/az.sql/set-azsqlinstancedatabasebackupshorttermretentionpolicy) |
...
| Восстановление базы данных из в определенный момент времени | [Отдельная база данных](sql-database-recovery-using-backups.md#point-in-time-restore) | [Отдельная база данных](https://docs.microsoft.com/powershell/module/az.sql/restore-azsqldatabase) <br/> [Управляемый экземпляр](https://docs.microsoft.com/powershell/module/az.sql/restore-azsqlinstancedatabase) |
...
| Восстановление базы данных из хранилища BLOB-объектов Azure | Отдельной базы данных - н/д <br/>Управляемый экземпляр — н/д  | Отдельной базы данных - н/д <br/>[Управляемый экземпляр](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-get-started-restore) |
...
- — Уровень обслуживания Basic **один** недели.
- Категории "стандартный" — **пять** недель.
- Уровень служб "премиум" — **пять** недель.

#### <a name="vcore-based-purchasing-model"></a>Модель приобретения на основе виртуальных ядер

Если вы используете [модель приобретения на основе виртуальных ядер](sql-database-service-tiers-vcore.md), срок хранения резервных копий по умолчанию является **семь** дней (в один пул и экземпляр базы данных). Для всех баз данных SQL Azure (отдельных баз данных, баз данных в составе пула и баз данных экземпляров) вы можете [изменить период хранения резервных копий, установив срок до 35 дней](#how-to-change-the-pitr-backup-retention-period).
...
На постоянной основе база данных Azure SQL, команда разработчиков автоматически тесты восстановления из создаваемых автоматически резервных копий из баз данных помещается в логических серверов и пулов эластичных баз данных (это не доступны в управляемый экземпляр). После восстановления на момент времени баз данных также получают проверок целостности, с помощью инструкции DBCC CHECKDB.

Управляемый экземпляр принимает автоматического начальное резервное копирование с `CHECKSUM` баз данных, восстановленные с помощью собственного `RESTORE` команды или Data Migration Service после завершения миграции.
