...
В каждой Базе данных SQL установлен период хранения резервных копий по умолчанию (от 7 до 35 дней). Его длительность зависит от [модели приобретения и уровня служб](#pitr-retention-period). Вы можете изменить период хранения резервных копий для базы данных на логическом сервере Azure (эта функция скоро будет включена в Управляемом экземпляре). Дополнительные сведения см. в разделе [Изменение периода хранения резервных копий](#how-to-change-backup-retention-period).
...
Если вы используете [модель приобретения на основе виртуальных ядер](sql-database-service-tiers-vcore.md), срок хранения резервных копий по умолчанию составляет 7 дней (как на логических серверах, так и на управляемых экземплярах).

- Для отдельных баз данных и баз данных в составе пула вы можете [изменить срок хранения резервных копий до 35 дней](#how-to-change-backup-retention-period).
- Изменение срока хранения резервных копий недоступно в Управляемом экземпляре.
...
## <a name="how-do-automated-backups-impact-my-compliance"></a>Влияние автоматически создаваемых резервных копий на соответствие
...
> [!Note]
> Период хранения резервных копий по умолчанию (7 дней) не может быть изменен в Управляемом экземпляре.
