...
Теперь, когда вы развернули приложение и заполнили его с помощью интересных данных клиента, используйте [SQL Server Management Studio (SSMS)](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms) для **\<подключения пользователей\>** , **\>Catalog-MT-\<** , с помощью Имя входа = *разработчик*, пароль = *P\@ssword1*.
