---
title: Запуск аналитических запросов
...
1. В **PowerShell ISE**, открыть *...*
    - **$DemoScenario** = **1** Покупка билетов на мероприятия на всех площадках
...
2. Задайте в сценарии для переменной $DemoScenario значение в зависимости от выбранного хранилища аналитики. В целях обучения рекомендуется использовать базу данных SQL без columnstore.
    - Чтобы использовать базу данных S'L без columnstore, установите **$DemoScenario** = **2**
    - Для использования базы данных S'L с магазином столбцов установите **$DemoScenario** = **3**  
3. Нажмите **F5** для запуска демо-скрипта (который *называется\<Deploy-TenantAnalytics XX>.ps1),* который создает магазин аналитики арендаторов. 

Теперь, когда вы развернули приложение и заполнили его интересными данными о арендаторах, используйте [студию управления серверами (SSMS) для](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms) подключения **\<арендаторов1-mt- Пользователя\> ** и **каталога-mt-Пользовательские\<\> ** серверы с помощью Login - *разработчик*, Пароль и *P\@sssword1*.
...
4. Нажмите **F5,** чтобы запустить скрипт, который создает и запускает задание, которое извлекает данные о билетах и клиентах из каждой базы данных арендаторов. Это задание сохраняет данные в хранилище аналитики.
...
5. Выберите **базу данных** в левой панели, затем введите имя пользователя и *разработчика*и введите пароль P *\@sssword1*. Нажмите кнопку **Подключить**.  

    ![DatabaseSignIn](media/saas-multitenantdb-tenant-analytics/databaseSignIn.PNG)

6. В панели **Navigator,** под базой данных аналитики, выберите таблицы звездно-схемы: fact_Tickets, dim_Events, dim_Venues, dim_Customers и dim_Dates. Затем выберите **Загрузка**. 
...
Дополнительные [учебники, которые основывается на приложении Wingtip SaaS.](saas-dbpertenant-wingtip-app-overview.md#sql-database-wingtip-saas-tutorials) 
- [Упругие рабочие места](elastic-jobs-overview.md).
