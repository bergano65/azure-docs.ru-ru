...
1. В **интегрированной среде сценариев PowerShell**откройте *. ..\Learning Modules\Operational Analytics\Tenant Analytics\Demo-TenantAnalytics.ps1*и задайте следующее значение:
    - **$DemoScenario** = **1** покупки билетов на мероприятия для всех мест проведения
...
2. Задайте в сценарии для переменной $DemoScenario значение в зависимости от выбранного хранилища аналитики. В целях обучения рекомендуется использовать базу данных SQL без columnstore.
    - Чтобы использовать базу данных SQL без columnstore, установите **$DemoScenario** = **2** .
    - Чтобы использовать базу данных SQL с индексом columnstore, установите **$DemoScenario** = **3** .  
...
Теперь, когда вы развернули приложение и заполнили его интересными данными клиента, используйте [SQL Server Management Studio (SSMS)](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms) для подключения серверов **tenants1\<-\> MT-User** и **Catalog\<-\> MT-** a с помощью имени входа — *разработчика*, Password = *P\@ssword1*.
...
Дополнительные [руководства, которые создаются на основе приложения SaaS Wingtip](saas-dbpertenant-wingtip-app-overview.md#sql-database-wingtip-saas-tutorials). 
- [Задания обработки эластичных БД](elastic-jobs-overview.md).
