...
Эта страница содержит сведения о базе данных SQL Azure и Управляемый экземпляр проблемы производительности, обнаруженные в журнале ресурсов [Intelligent Insights](sql-database-intelligent-insights.md) . Метрики и журналы ресурсов можно передавать в [Azure Monitor журналы](../azure-monitor/insights/azure-sql.md), [концентраторы событий Azure](../azure-monitor/platform/resource-logs-stream-event-hubs.md), службу [хранилища Azure](sql-database-metrics-diag-logging.md#stream-into-azure-storage)или стороннее решение для настраиваемых оповещений и отчетов DevOps.
...
Intelligent Insights автоматически обнаруживает проблемы с производительностью баз данных в базе данных SQL Azure на основе времени ожидания выполнения запроса, ошибок или истечения времени ожидания. Intelligent Insights выводит шаблоны производительности в журнал ресурсов базы данных SQL. Ниже в таблице кратко описаны выявляемые шаблоны снижения производительности.
...
| [Создать запрос](sql-database-intelligent-insights-troubleshoot-performance.md#new-query) | Обнаружен новый запрос, который негативно влияет на общую производительность Базы данных SQL. | Обнаружен новый запрос, который негативно влияет на общую производительность базы данных. |
...
| [Состязание за TempDB](sql-database-intelligent-insights-troubleshoot-performance.md#tempdb-contention) | Несколько потоков пытается получить доступ к одному и тому же ресурсу TempDB, создавая узкое место. Это негативно влияет на производительность Базы данных SQL. | Несколько потоков пытается получить доступ к одному и тому же ресурсу TempDB, создавая узкое место. Это негативно влияет на производительность базы данных. |
| [Нехватка DTU эластичного пула](sql-database-intelligent-insights-troubleshoot-performance.md#elastic-pool-dtu-shortage) | Нехватка доступных eDTU в эластичном пуле негативно влияет на производительность Базы данных SQL. | Недоступно для Управляемого экземпляра, так как он использует модель с виртуальными ядрами. |
| [Регрессия плана](sql-database-intelligent-insights-troubleshoot-performance.md#plan-regression) | Обнаружен новый план или изменение рабочей нагрузки существующего плана. Это негативно влияет на производительность Базы данных SQL. | Обнаружен новый план или изменение рабочей нагрузки существующего плана. Это негативно влияет на производительность базы данных. |
...
Дополнительные сведения об оптимизации производительности запросов см. в разделе [Настройка запросов](https://msdn.microsoft.com/library/ms176005.aspx).
...
Ресурсы в базе данных SQL обычно называются [ресурсами DTU](sql-database-purchase-models.md#dtu-based-purchasing-model), которые состоят из смешанной меры ресурсов ЦП и операций ввода-вывода (операций ввода-вывода данных и журнала транзакций). [Ресурсы эластичного пула Azure](sql-database-elastic-pool.md) используются в качестве пула доступных ресурсов eDTU, совместно используемого несколькими базами данных в целях масштабирования. Когда ресурсов eDTU, доступных в эластичном пуле, недостаточно, чтобы обеспечить все базы данных в пуле, система выявляет проблему с производительностью из-за нехватки DTU эластичного пула.
...
Обычно Intelligent Insights требуется один час для выполнения анализа первопричин проблемы с производительностью. Если проблему не удается найти в Intelligent Insights, но это крайне важно для вас, используйте хранилище запросов, чтобы вручную определить первопричину проблемы производительности. (Как правило, эти проблемы менее одного часа назад.) Дополнительные сведения см. [в статье мониторинг производительности с помощью хранилища запросов](https://docs.microsoft.com/sql/relational-databases/performance/monitoring-performance-by-using-the-query-store).

## <a name="next-steps"></a>Дальнейшие шаги

- Изучение [Intelligent Insights](sql-database-intelligent-insights.md) концепций.
