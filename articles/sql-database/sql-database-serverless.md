...
| **Затраты на управление производительностью** |Lower|Высокие|
...
Если используется [управляемое клиентом прозрачное шифрование данных](transparent-data-encryption-byok-azure-sql.md) (BYOK) и Автоматическая приостановка работы базы данных без сервера происходит при удалении или отзыве ключа, то база данных остается в состоянии автоматического приостановки.  В этом случае при следующей попытке возобновления база данных остается приостановленной, пока ее состояние не станет недоступным по истечении приблизительно 10 минут или меньше.  После того как база данных станет недоступной, процесс восстановления будет таким же, как и для подготовленных вычислений баз данных.  Если база данных без сервера находится в режиме «в сети» при удалении или отзыве ключа, то база данных также становится недоступной через 10 минут или меньше, так же, как и подготовленные базы данных вычислений.
...
|Сущность|Метрика|Description|Units|
|---|---|---|---|
