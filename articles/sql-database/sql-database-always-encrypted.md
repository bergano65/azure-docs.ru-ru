...
В этой статье показано, как защитить конфиденциальные данные в базе данных SQL с помощью шифрования базы данных при помощи [мастера Always encrypted](https://msdn.microsoft.com/library/mt459280.aspx) в [SQL Server Management Studio (SSMS)](https://msdn.microsoft.com/library/hh213248.aspx). В ней также показано, как сохранить ключи в хранилище сертификатов Windows.
...
2. Щелкните **создать ресурс** > **данные + хранилище** > **база данных SQL**.
...
1. Щелкните **базы данных** > **SQL курс** > **Показывать строки подключения к базам данных**.
...
1. Откройте SSMS. (Щелкните **подключить** > **ядро СУБД** , чтобы открыть окно **Подключение к серверу** , если оно не открыто).
...
Вы можете проверить создание ключей в SSMS, перейдя к **Clinic** > **Security** > **разделу Безопасность Always encrypted ключи**. Таким образом можно увидеть новые ключи, созданные с помощью мастера.
...
* [Блог Always Encrypted](https://blogs.msdn.com/b/sqlsecurity/archive/tags/always-encrypted/)