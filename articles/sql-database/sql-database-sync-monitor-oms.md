...
description: Узнайте, как контролировать синхронизацию данных Azure S'L с помощью журналов Azure Monitor
...
Вам больше не требуется проверять журнал вручную на портале Azure, через PowerShell или REST API. С [журналами Azure Monitor](https://docs.microsoft.com/azure/log-analytics/log-analytics-overview)можно создавать оповещения, которые идут непосредственно на адреса электронной почты людей, которым необходимо видеть их при возникновении ошибки.
...
Внедрить пользовательское решение мониторинга журналов Azure Monitor для синхронизации данных с данными менее чем за час, выполняя следующие вещи:

Необходимо настроить три компонента:

-   Запуск PowerShell для передачи данных журналов синхронизации данных данных данных СЗЛ в журналы Azure Monitor.

-   Предупреждение Azure Monitor для уведомлений по электронной почте.

-   Представление монитора Azure для мониторинга.
...
10. Внесите необходимые изменения в учетной записи и конфигурации синхронизации данных SQL. (Дополнительные сведения см. в примере сценария.)
    ...
    3.  Информация о журналах Azure Monitor. Эти сведения можно найти на портале Azure, выбрав "Параметры | Подключенные источники". Для получения дополнительной информации об отправке данных в журналы Azure Monitor [см.](../azure-monitor/platform/data-collector-api.md)
...
3.  Выберите **ссылку Расписание для вашего runbook**.
...
## <a name="create-an-azure-monitor-reader-alert-for-email-notifications"></a>Создание оповещения о reader Azure Monitor для уведомлений по электронной почте

Чтобы создать оповещение, использующее журналы Azure Monitor, сделайте следующие вещи. В качестве предварительного условия необходимо, чтобы журналы Azure Monitor были связаны с рабочим пространством аналитики журналов.
...
4.  В разделе **Generate alert based on** (Создать оповещение на основе) выберите **Metric Measurement** (Измерение метрик).
    ...
    2.  После задания **Больше чем** введите пороговое значение, по достижении которого начнут поступать уведомления. В синхронизации данных ожидаются временные ошибки. Чтобы уменьшить шум, установите порог до 5.
...
Этот шаг создает представление Azure Monitor для визуального мониторинга всех указанных групп синхронизации. Представление содержит несколько компонентов:
...
Чтобы настроить представление Azure Monitor, сделайте следующие вещи:

1.  На домашней странице рабочего пространства Log Analytics выберите плюс слева, чтобы открыть **дизайнер ампло.**
...
**Журналы Azure Monitor:** В зависимости от использования может возникнуть стоимость, связанная с журналами Azure Monitor. На бесплатном уровне доступно 500 МБ получаемых данных в день. Ожидается, что в большинстве случаев это решение будет принимать менее чем 500 МБ в день. Чтобы уменьшить потребление, используйте фильтрацию по сбоям, включенную в runbook. При использовании более 500 МБ в день необходимо перейти на платный уровень во избежание риска возможной остановки аналитики по достижении ограничений. Для получения дополнительной [информации](https://azure.microsoft.com/pricing/details/log-analytics/)см.
