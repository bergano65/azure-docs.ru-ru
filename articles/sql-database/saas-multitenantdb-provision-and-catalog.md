---
title: Положение в SaaS мультитенантных
...
В Wingtip каталог реализуется в базе данных *tenantcatalog*. *tenantcatalog* создан с помощью функций управления сегментами в [клиентской библиотеке эластичной базы данных (EDCL)](sql-database-elastic-database-client-library.md). Библиотека позволяет приложению создавать, управлять и использовать *карту осколков,* которая хранится в базе данных. Карта сегментов содержит перекрестную ссылку на ключ клиента с его сегментами, то есть его сегментированную базу данных.
...
- Получите скрипты Wingtip и исходный код:
    - Скрипты Ибн-антеролетов И исходный код базы данных Wingtip Tickets SaaS и исходный код приложения доступны в репо [WingtipTicketsSaaS-MultitenantDB](https://github.com/microsoft/WingtipTicketsSaaS-MultiTenantDB) GitHub.
...
- **Регистрация арендатора в каталоге**: В каталог добавляется отображение между новым ключом клиента и существующей базой данных tenants1.
...
1. В *интегрированной среде сценариев PowerShell* откройте файл …\\Learning Modules\\ProvisionTenants\\*Demo-ProvisionTenants.ps1* и задайте следующие параметры.
   - **$TenantName** = **Bushwillow Blues**, название нового места.
   - **$VenueType** = **блюз**, один из заранее определенных типов место: блюз, классическая музыка, танец, джаз, дзюдо, автоспорт, многоцелевой, опера, рок-музыка, футбол (нижний корпус, без пробелов).
   - **$DemoScenario** = **1**, предоставить арендатора в общей базе данных с другими арендаторами.
...
## <a name="provision-a-tenant-in-its-own-database"></a>Предоставление арендатора в *собственной* базе данных
...
- **Подготовка клиента в базе данных клиента по умолчанию.** База данных клиента обновляется путем добавления новых данных клиента.
- **Регистрация арендатора в каталоге**: Отображение между новым ключом арендатора и базой данных *sequoiasoccer* добавляется в каталог.
...
1. В файле …\\Learning Modules\\ProvisionTenants\\*Demo-ProvisionTenants.ps1* задайте следующие параметры.
   - **$TenantName** = **Секвойя Футбол**, название нового места.
   - **$VenueType** = **футбол**, один из заранее определенных типов место проведения: блюз, классическая музыка, танец, джаз, дзюдо, автоспорт, многоцелевой, опера, рок-музыка, футбол (нижний корпус, без пробелов).
   - **$DemoScenario** = **2**, предоставить арендатора в свою собственную базу данных.

2. Добавить новую точку разрыва, поставив курсор в любом месте на линии 57, линия, которая говорит: * & &nbsp;$PSScriptRoot »New-TenantAndDatabase '*, и нажмите **F9**.
...
1. В *интегрированной среде сценариев PowerShell* откройте файл …\\Learning Modules\\ProvisionTenants\\*Demo-ProvisionTenants.ps1* и измените значение параметра *$DemoScenario* на 4.
   - **$DemoScenario** = **4**, чтобы предоставить партию арендаторов в общую базу данных.
...
1. В студии управления серверами S'L (SSMS) подключитесь к серверу арендаторов в **каталоге-MT.\<USER\>.database.windows.net**, с **разработчиком**Login и Password **sssword1\@**
...
- [Упругая библиотека клиентов базы данных](sql-database-elastic-database-client-library.md)
