---
title: Обеспечение безопасности Управляемого экземпляра Базы данных SQL Azure с помощью имен для входа Azure AD | Документация Майкрософт
description: Узнайте о методах и функциях, применяемых для защиты Управляемого экземпляра в Базе данных SQL Azure, и использовании имен для входа Azure AD
...
ms.locfileid: "55229139"
---
# <a name="tutorial-managed-instance-security-in-azure-sql-database-using-azure-ad-logins"></a>Руководство. Обеспечение безопасности Управляемого экземпляра в Базе данных SQL Azure с помощью имен для входа Azure AD

Управляемый экземпляр Базы данных SQL Azure обеспечивает практически все функции безопасности, имеющиеся в последней версии локального ядра СУБД SQL Server (Enterprise Edition):
...
> [!div class="checklist"]
> - создание имени для входа Azure Active Directory (AD) для Управляемого экземпляра;
> - предоставление разрешений именам для входа Azure AD в Управляемых экземплярах;
> - создание пользователей Azure AD на основе имен для входа Azure AD;
> - назначение разрешений пользователям Azure AD и обеспечение безопасности управляемой базы данных;
...
> [!NOTE]
> Имена для входа Azure AD для Управляемого экземпляра Базы данных SQL находятся на этапе **общедоступной предварительной версии**.
...
- Возможность доступа к Управляемому экземпляру Базы данных SQL Azure и [подготовленный пользователь с правами администратора Azure AD для Управляемого экземпляра](sql-database-aad-authentication-configure.md#provision-an-azure-active-directory-administrator-for-your-managed-instance). Дополнительные сведения см. на следующих ресурсах:
...
Доступ к Управляемому экземпляру возможен только по частному IP-адресу. Нет доступных конечных точек службы для подключения к Управляемому экземпляру из-за пределов его сети. Так же как и в изолированной локальной среде SQL Server, приложениям или пользователям требуется доступ к сети Управляемого экземпляра, прежде чем можно будет установить подключение. Дополнительные сведения см. в статье [Подключение приложения к Управляемому экземпляру Базы данных SQL](sql-database-managed-instance-connect-app.md).
...
## <a name="create-an-azure-ad-login-for-a-managed-instance-using-ssms"></a>Создание имени для входа Azure AD для Управляемого экземпляра с помощью SSMS

Первое имя для входа Azure AD должно быть создано с помощью стандартной учетной записи SQL Server (не Azure AD), то есть `sysadmin`. Примеры подключения к Управляемому экземпляру см. в следующих статьях:
...
> [!IMPORTANT]
> Учетную запись администратора Azure AD, используемую для настройки Управляемого экземпляра, нельзя применять для создания имени для входа Azure AD в Управляемом экземпляре. Необходимо создать первое имя для входа Azure AD с помощью учетной записи SQL Server — `sysadmin`. Это временное ограничение, которое будет устранено, как только имена для входа Azure AD станут общедоступными. При попытке использования учетной записи администратора Azure AD для создания имени для входа отобразится следующая ошибка: `Msg 15247, Level 16, State 1, Line 1 User does not have permission to perform this action.`
...
Чтобы создать другие имена для входа Azure AD, субъекту (SQL или Azure AD) необходимо предоставить роли или разрешения SQL Server.
...
- Чтобы предоставить только что созданному имени для входа Azure AD возможность создавать другие имена входа для других пользователей, групп или приложений Azure AD, предоставьте имени для входа роль сервера `sysadmin` или `securityadmin`. 
- Чтобы создавать другие имена для входа Azure AD, применяемому для этого имени для входа Azure AD необходимо предоставить как минимум разрешение **ALTER ANY LOGIN**. 
- По умолчанию только что созданным именам для входа Azure AD в базе данных master предоставляются следующие стандартные разрешения: **CONNECT SQL** и **VIEW ANY DATABASE**.
- В Управляемом экземпляре роль сервера `sysadmin` может предоставляться многим учетным именам для входа Azure AD.
...
1. Предоставьте имени для входа Azure AD роль сервера `sysadmin`, используя следующий синтаксис T-SQL:
...
## <a name="create-additional-azure-ad-logins-using-ssms"></a>Создание дополнительных имен для входа Azure AD в SSMS

После создания имени для входа Azure AD и предоставления ему привилегий `sysadmin` с его помощью можно создавать дополнительные имена для входа, используя предложение **FROM EXTERNAL PROVIDER** с **CREATE LOGIN**.

1. Подключитесь к серверу Управляемого экземпляра с помощью имени для входа Azure AD в SQL Server Management Studio. Введите имя сервера Управляемого экземпляра. При входе в систему с учетной записью Azure AD есть три варианта проверки подлинности в SSMS:
...
1. Создайте имя входа Управляемого экземпляра для группы в Azure AD. Чтобы вы смогли добавить имя для входа в Управляемый экземпляр, группа должна существовать в Azure AD. Ознакомьтесь со статьей [Создание простой группы и добавление в нее участников с помощью Azure Active Directory](../active-directory/fundamentals/active-directory-groups-create-azure-portal.md). Создайте группу _mygroup_ и добавьте в нее участников.
...
1. Проверьте разрешения сервера для только что созданного имени для входа Azure AD, выполнив следующую команду:
...
> [!NOTE]
> Гостевые пользователи Azure AD поддерживаются для имен для входа Управляемых экземпляров только в том случае, если добавляются в качестве группы Azure AD. Гостевой пользователь Azure AD — это учетная запись, приглашенная в Azure AD, к которой принадлежит Управляемый экземпляр, из другого экземпляра Azure AD. Например, joe@contoso.com (учетная запись Azure AD) или steve@outlook.com (учетная запись MSA) можно добавить в группу в aadsqlmi Azure AD. После того как пользователи добавлены в группу, в базе данных **master** Управляемого экземпляра для группы можно создать имя для входа с использованием синтаксиса **CREATE LOGIN**. Гостевые пользователи, которые являются участниками этой группы, могут подключиться к Управляемому экземпляру с помощью своих текущих имен для входа (например, joe@contoso.com или steve@outlook.com).

## <a name="create-an-azure-ad-user-from-the-azure-ad-login-and-give-permissions"></a>Создание пользователя Azure AD на основе имени для входа Azure AD и предоставление разрешений
...
1. В **обозревателе объектов** щелкните сервер правой кнопкой мыши и выберите **Создать запрос**.
1. В окне запроса создайте пользователя Azure AD на основе имени для входа Azure AD, используя следующий синтаксис:
...
1. Кроме того, поддерживается создание пользователя Azure AD на основе имени для входа Azure AD, которое является группой.
    ...
    > [!IMPORTANT]
    > При создании **пользователя** на основе имени для входа Azure AD необходимо указать user_name, совпадающее с login_name **имени для входа**.
...
Межбазовые запросы поддерживаются для учетных записей Azure AD с именами для входа Azure AD. Чтобы протестировать межбазовый запрос с помощью группы Azure AD, необходимо создать еще одну базу данных и таблицу. Если они уже существуют, их создание можно пропустить.
...
## <a name="additional-scenarios-supported-for-azure-ad-logins-public-preview"></a>Дополнительные сценарии, поддерживаемые для имен для входа Azure AD (общедоступная предварительная версия) 

- Имена для входа Azure AD поддерживают выполнение заданий и управление агентом SQL Server.
- Операции резервного копирования и восстановления базы данных могут быть выполнены с помощью имен для входа Azure AD.
- [Аудит](sql-database-managed-instance-auditing.md) всех инструкций, связанных с именами для входа Azure AD и событиями проверки подлинности.
- Выделенное административное соединение для имен для входа Azure AD, которые являются участниками роли сервера `sysadmin`.
- Имена для входа Azure AD поддерживают использование [служебной программы sqlcmd](/sql/tools/sqlcmd-utility) и средства [SQL Server Management Studio](/sql/ssms/download-sql-server-management-studio-ssms).
- Триггеры входа поддерживаются для событий входа, поступающих от имен для входа Azure AD.
- С использованием имен для входа Azure AD можно настроить компоненты Service Broker и Database Mail.
...
Полный обзор возможностей Управляемого экземпляра Базы данных SQL Azure см. по следующей ссылке:
