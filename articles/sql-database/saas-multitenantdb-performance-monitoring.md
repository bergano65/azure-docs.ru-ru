---
title: Мониторинг производительности осколки мультитенантной базы данных
...
* Чтобы управлять временным возрастанием нагрузки *отдельного* клиента, **его можно изъять из базы данных и назначить ему индивидуальный объем вычислительных ресурсов**. После снижения нагрузки этот клиент можно вернуть в мультитенантную базу данных. Когда об этом известно заранее, арендаторы могут быть перемещены упреждаюстраженно, чтобы гарантировать, что база данных всегда имеет ресурсы, в которые она нуждается, и чтобы избежать влияния на других арендаторов в базе данных мультиарендаторов. Если эта необходимость предсказуемая, например из-за увеличения уровня продаж билетов на популярное мероприятие, это поведение управления можно интегрировать в приложение.
...
1. Установить **$DemoScenario** = **1**, Предоставление _партии арендаторов_
...
|:--|:--|
| 2 | Создание нормальной нагрузки интенсивности (приблизительно 30 DTU) |
| 3 | Создание нагрузки с более длительными пиками на клиента|
| 4 | Генерация нагрузки с более высокими очередями DTU на одного арендатора (приблизительно 70 DTU)|
| 5 | Создание высокой интенсивности (приблизительно 90 DTU) на одного арендатора плюс нормальная нагрузка интенсивности на всех других арендаторов |
...
1. В **интегрированной среде сценариев PowerShell** откройте скрипт …\\Learning Modules (Модули для обучения)\\Performance Monitoring and Management (Мониторинг производительности и управление ею)\\*Demo-PerformanceMonitoringAndManagement.ps1*. Не закрывайте этот скрипт, так как он еще понадобится в рамках этого руководства.
1. Установить **$DemoScenario** = **2**, _Генерировать нормальную нагрузку интенсивности_
...
1. Установите *$DemoScenario* = **3**, _Создайте нагрузку с более длинными и более частыми всплесками в базе данных,_ чтобы увеличить интенсивность совокупной нагрузки на базу данных без изменения пиковой нагрузки, требуемой каждым арендатором.
...
Сегментированная мультитенантная модель позволяет выбрать, подготавливать ли новый клиент в мультитенантной базе данных вместе с другими клиентами или подготавливать его в собственной базе данных. Приготовив арендатора в собственной базе данных, он выигрывает от изоляции, присущей отдельной базе данных, что позволяет управлять производительностью этого арендатора независимо от других, восстанавливать этого арендатора независимо от других и т.д. Например, можно поместить бесплатную пробную или постоянных клиентов в базу данных с несколькими арендаторами и премиум-клиентов в отдельных базах данных.  Изолированными базами данных с одним клиентом также можно управлять коллективно в эластичном пуле для оптимизации затрат на ресурсы.
...
1. Установить **$Scenario** = **2**, _Предоставление арендатора в новой базе данных одного арендатора_
1. Нажмите клавишу **F5** для запуска скрипта.
...
1. Установить **$DemoScenario No 5**, Создать _нормальную нагрузку плюс высокая нагрузка на одного арендатора (примерно 90 DTU)._
1. Задайте значение **$SingleTenantName = Salix Salsa**
1. Чтобы выполнить скрипт, нажмите клавишу **F5**.

Перейдите на портал и перейдите к **обзору salixsalsa,** > **Overview** чтобы просмотреть диаграммы мониторинга. 
