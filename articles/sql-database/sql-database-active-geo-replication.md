...
- **Плановая отработка отказа**
  ...
  Планируется коммутаторы отработки отказа роли первичной и вторичной базы данных после завершения полной синхронизации. Это операцию в оперативном режиме, приводит к потере данных. Время выполнения операции зависит от размера журнала транзакций на сервере-источнике, должна быть синхронизирована. Плановая отработка отказа предназначен для следующих сценариев: (a) для того, чтобы выполнить аварийное Восстановление выполняется в рабочей среде после потери данных не допускается; (b), чтобы переместить базу данных в другой регион; и (c) для возврата базы данных в основной регион после устранения сбоя была устранена (Восстановление размещения).

- **Незапланированная отработка отказа**

  При незапланированной или принудительной отработке отказа база данных-получатель немедленно переключается на основную роль без какой-либо синхронизации с базой данных-источником. Все транзакции зафиксированы к первичной реплике, но не реплицируются на сервер-получатель, будут потеряны. Эта операция предназначена как метод восстановления во время сбоев, источник недоступен, когда необходимо быстро восстановить доступность базы данных. При исходной первичной реплике будет восстановлена он будет автоматически повторно подключиться и становится новым получателем. Все несинхронизированные транзакции до отработки отказа, будут сохранены в файл резервной копии, но не будут синхронизированы с новой первичной реплики во избежание конфликтов. Эти транзакции необходимо вручную осуществить слияние с самой последней версии базы данных-источника.
...
## <a name="preparing-secondary-database-for-failover"></a>Подготовка базы данных-получателя для отработки отказа

Приложения могут немедленно получить доступ к новой первичной реплики после отработки отказа, убедитесь, что требования к проверке подлинности для сервера-получателя и базы данных настроены правильно. Дополнительные сведения см. в разделе [Безопасность базы данных SQL после аварийного восстановления](sql-database-geo-replication-security-config.md). Чтобы гарантировать соответствие после отработки отказа, убедитесь, что политики хранения резервных копий базы данных-получателя совпадает с базы данных-получателя. Эти параметры не являются частью базы данных и не реплицируются. По умолчанию сервер-получатель будет настроен на период хранения PITR по умолчанию 7 дней. Дополнительные сведения см. в статье [Подробнее о резервном копировании базы данных SQL](sql-database-automated-backups.md).

## <a name="configuring-secondary-database"></a>Настройка базы данных-получателя

База данных-источник и база данных-получатель должны иметь один и тот же уровень служб. Кроме того, настоятельно рекомендуем создавать базу данных-источник и базу данных-получатель с одинаковым объемом вычислительных ресурсов (DTU или виртуальных ядер). Если возникают проблемы записи большого объема рабочей нагрузки базы данных-источника, получателя с нижней объем вычислений может оказаться справиться с ним. Это вызовет задержка повтора на вторичной реплике и потенциальных недоступности. Такое несоответствие может привести к потере большого объема данных, если потребуется применение принудительной отработки отказа. Для снижения этих рисков, действующие активной георепликации будет регулировать скорость первичной журнала, чтобы разрешить его базы данных-получатели отставать. Другим следствием несбалансированных дополнительной конфигурации — что после отработки отказа приложения будут наблюдаться снижение производительности из-за недостатка вычислительной мощности новой первичной реплике. Он потребуется выполнить обновление до большей вычислительной необходимые уровню, который будет невозможно до устранения сбоя. 


> [!IMPORTANT]
> Опубликованные RPO = 5 секунд, не может быть гарантирована, если база данных-получатель не настроен того же размера вычислений как основной. 


Если вы решите создать базу данных-получатель с более низким объемом вычислительных ресурсов, воспользуйтесь диаграммой журнала операций ввода-вывода в процентах на портале Azure. Это удобный способ рассчитать минимальный объем вычислительных ресурсов базы данных-получателя, необходимый, чтобы выдержать нагрузку репликации. Например, если уровень базы данных-источника — P6 (1000 DTU) и процент операций ввода-вывода для нее равен 50 %, то уровень базы-данных получателя должен быть не менее P4 (500 DTU). Данные журнала операций ввода-вывода также можно получить с помощью представления базы данных [sys.resource_stats](/sql/relational-databases/system-catalog-views/sys-resource-stats-azure-sql-database) или [sys.dm_db_resource_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database).  Регулирование сообщается как в состоянии ожидания HADR_THROTTLE_LOG_RATE_MISMATCHED_SLO [sys.dm_exec_requests](/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-requests-transact-sql) и [sys.dm_os_wait_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-os-wait-stats-transact-sql) представления базы данных. 
...
## <a name="keeping-credentials-and-firewall-rules-in-sync"></a>Синхронизация учетных данных и правила брандмауэра
...
## <a name="upgrading-or-downgrading-primary-database"></a>Повышения или понижения базы данных-источника
