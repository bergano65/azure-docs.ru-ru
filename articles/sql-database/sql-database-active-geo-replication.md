...
ms.locfileid: "82232008"
---
# <a name="creating-and-using-active-geo-replication"></a>Создание и использование активной георепликации

Активная Георепликация — это функция базы данных SQL Azure, которая позволяет создавать доступную для чтения базу данных-получатель отдельных баз данных на сервере базы данных SQL в том же или другом центре обработки данных (регионе).
...
1. Добавьте IP-адрес в список разрешений клиента, выполняющего изменения (Дополнительные сведения см. в разделе [Настройка брандмауэра](sql-database-firewall-configure.md)). 
...
1. Добавьте IP-адрес в список разрешений клиента, выполняющего изменения. Он должен иметь тот же точный IP-адрес основного сервера. 
...
- Ознакомьтесь с примерами скриптов в следующих статьях:
  - [Настройка и отработка отказа отдельной базы данных с помощью активной георепликации](scripts/sql-database-setup-geodr-and-failover-database-powershell.md)
  - [Настройка и отработка отказа базы данных в составе пула с помощью активной георепликации](scripts/sql-database-setup-geodr-and-failover-pool-powershell.md)
