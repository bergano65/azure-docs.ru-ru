---
title: Предоставление новых арендаторов в мультитенантном приложении
...
Помимо функций для приложения SaaS, каталог позволяет включить средства для баз данных. В выборке базы данных Wingtip Tickets SaaS-per-tenant каталог используется для включения запроса кросс-арендаторов, который исследуется в [специальном учебнике по отчетности.](saas-tenancy-cross-tenant-reporting.md) Управление работой по перекрестным базам данных изучается в учебниках по [управлению схемой](saas-tenancy-schema-management.md) и [аналитике арендаторов.](saas-tenancy-tenant-analytics.md)
...
> [!IMPORTANT]
> Данные отображения доступны в базе данных каталога, но *не отсваивайте их.* Изменяйте данные сопоставления только с помощью интерфейсов API EDCL. Прямые манипуляции с данными сопоставления могут привести к повреждению каталога. Такие операции не поддерживаются.
...
1. В PowerShell ISE, открыть ... \\Учебные\\модули\\ProvisionAndCatalog_Demo-ProvisionAndCatalog.ps1_ и устанавливают следующие параметры:
   ...
   * **$DemoScenario** = **1**, Положение *одного арендатора*.
...
* **Рассчитайте новый ключ арендатора.** Для создания ключа клиента из имени клиента используется хэш-функция.
* **Проверьте, существует ли ключ клиента.** Каталог проверяется, чтобы убедиться, что ключ доступен.
* **Подготовка базы данных клиента с помощью New-TenantDatabase.** Используйте клавишу F11, чтобы перейти к функции и узнать, как база данных подготавливается с помощью [шаблона Azure Resource Manager](../azure-resource-manager/resource-manager-template-walkthrough.md).
    ...
    Шаблон «Менеджер ресурсов» находится в папке «Обучение модулям»: *tenantdatabasecopytemplate.json*
...
1. В интегрированной среде сценариев PowerShell откройте файл ...\\Learning Modules\\ProvisionAndCatalog\\*Demo-ProvisionAndCatalog.ps1*. Измените значение параметра *$DemoScenario* на 3.
   ...
   * **$DemoScenario** = **3**, Предоставление *партии арендаторов*.
