...
3.  Используйте инструменты **бизнес-аналитики,** чтобы извлечь полезную информацию, которая может направлять принятие решений. 
...
2. Задайте в сценарии для переменной $DemoScenario значение в зависимости от хранилища аналитики:
    - Чтобы использовать базу данных S'L без хранилища столбцов, установите **$DemoScenario** = **2**
...
Теперь, когда вы развернули приложение и заполнили его интересными данными о клиентах, используйте [студию управления серверами (SSMS) для](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms) подключения **&lt;арендаторов1-dpt- Пользователь&gt; ** и **каталог-dpt-Пользовательские&lt;&gt; ** серверы с помощью Login - *разработчик*, Пароль и *P\@sssword1*. Дополнительные рекомендации см. в этом [ознакомительном руководстве](saas-dbpertenant-wingtip-app-overview.md).

![Обзор архитектуры](media/saas-tenancy-tenant-analytics/ssmsSignIn.png)

В обозревателе объектов сделайте следующее:

1. Расширьте *сервер пользователей&lt;tenants1-dpt.&gt; *
2. Разверните узел баз данных и просмотрите список клиентских баз данных.
3. Расширьте *сервер&lt;пользователя&gt; каталога-*
