...
3. Основной и дополнительный управляемые экземпляры должны находиться в одной группе ресурсов.
4. Виртуальных сетей, частью которого являются управляемые экземпляры, необходимо подключить через [VPN-шлюз](../vpn-gateway/vpn-gateway-about-vpngateways.md). Пиринг глобальной виртуальной сети не поддерживается.
5. У двух управляемых экземпляров виртуальных сетей не может быть перекрывающихся IP-адресов.
6. Необходимо настроить группы безопасности сети (NSG), чтобы порты 5022 и диапазон 11000 ~ 12000 были открытыми для входящих и исходящих подключений из другой управляемой подсети с экземпляром. Это позволяет обеспечить трафик репликации между экземплярами.
...
| [Add-Азсклдатабасетофаиловерграуп](https://docs.microsoft.com/powershell/module/az.sql/add-azsqldatabasetofailovergroup)|Добавляет одну или несколько баз данных в группу отработки отказа Базы данных SQL Azure.|
|  | |
