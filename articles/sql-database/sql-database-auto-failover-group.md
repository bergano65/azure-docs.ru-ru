...
- **Группа отработки отказа (ТУМАН)**
  ...
  Группу отработки отказа — это именованный набор баз данных, управляемых одним сервером базы данных SQL или в одном управляемом экземпляре, при сбое как единое целое в другой регион в случае, если все или часть баз данных-источников становятся недоступными из-за сбоя в основном регионе. При создании для управляемых экземпляров, группу отработки отказа содержит все пользовательские базы данных в экземпляре, и таким образом можно настроить только одну группу отработки отказа на экземпляре.
...
- **Источник**
  ...
  Сервер базы данных SQL или управляемый экземпляр, на котором размещена в группе отработки отказа баз данных-источников.
...
- **Зона DNS**
  ...
  Уникальный идентификатор, который создается автоматически при создании нового экземпляра. Для проверки подлинности подключений клиентов на любом экземпляре в той же зоне DNS подготавливается сертификат несколькими доменами (данных SAN) для данного экземпляра. Два управляемых экземпляров в одной и той же группе отработки отказа должны совместно использовать зоны DNS. 
  
  > [!NOTE]
  > Идентификатор зоны DNS не является обязательным для отработки отказа групп, созданных для серверов баз данных SQL.

- **Прослушиватель чтения и записи для группы отработки отказа**

  Формирование записи DNS CNAME, которая указывает на URL-адрес текущего источника. Она позволяет приложениям SQL для чтения и записи прозрачно повторно подключиться к базе данных-источнику, когда эта база данных изменяется после отработки отказа. Когда группа отработки отказа создается на сервере базы данных SQL, запись CNAME в DNS для URL-адрес прослушивателя формируется как `<fog-name>.database.windows.net`. При создании группы отработки отказа на управляемом экземпляре запись CNAME в DNS для URL-адрес прослушивателя формируется как `<fog-name>.zone_id.database.windows.net`.

- **Прослушиватель только для чтения для группы отработки отказа**

  Формируется запись DNS CNAME, которая указывает на прослушиватель только для чтения, указывающий на URL-адрес получателя. Она позволяет приложениям SQL для чтения прозрачно повторно подключиться к базе данных-получателю, используя специальные правила балансировки нагрузки. Когда группа отработки отказа создается на сервере базы данных SQL, запись CNAME в DNS для URL-адрес прослушивателя формируется как `<fog-name>.secondary.database.windows.net`. При создании группы отработки отказа на управляемом экземпляре запись CNAME в DNS для URL-адрес прослушивателя формируется как `<fog-name>.zone_id.secondary.database.windows.net`.
...
## <a name="best-practices-of-using-failover-groups-with-managed-instances"></a>Рекомендации по использованию групп отработки отказа с помощью управляемых экземпляров
...
Если приложение использует управляемый экземпляр в качестве уровня данных, следуйте следующим рекомендациям при проектировании непрерывности бизнес-процессов:

- **Создание экземпляра получателя в той же зоне DNS, что и для экземпляра источника**

  Чтобы обеспечить беспрерывное подключение к экземпляру источника после отработки отказа, экземпляры и источника, и получателя должны находиться в одной зоне DNS. Это гарантирует, что один и тот же сертификат несколькими доменами (данных SAN) может использоваться для проверки подлинности подключений клиентов на любой из двух экземпляров в группе отработки отказа. После подготовки вашего приложения к рабочему развертыванию создайте экземпляр получателя в другом регионе и убедитесь, что он использует ту же зону DNS, что и экземпляр источника. Это можно сделать, указав `DNS Zone Partner` необязательный параметр, с помощью портала Azure, PowerShell или REST API. 

  Дополнительные сведения о создании экземпляра-получателя в той же зоне DNS, что основной экземпляр см. в разделе [Управление группами отработки отказа с управляемые экземпляры (Предварительная версия)](#powershell-managing-failover-groups-with-managed-instances-preview).
...
- **Использование прослушивателя чтения и записи для рабочей нагрузки OLTP**
  ...
  При выполнении операций OLTP используйте `<fog-name>.zone_id.database.windows.net` в качестве URL-адреса сервера, чтобы все подключения автоматически перенаправлялись на сервер-источник. Этот URL-адрес не будет изменяться после отработки отказа. Отработка отказа включает в себя обновление DNS-записи, поэтому клиентские подключения будут перенаправляться на новый сервер-источник только после обновления кэша DNS на стороне клиента. Так как второй экземпляр совместно использует зону DNS с первичной, клиентское приложение будет сможет повторно подключиться к ней, используя один и тот же сертификат SAN.
...
- **Будьте готовы к потере данных**
  ...
  После запуска отработки отказа DNS обновления прослушивателя чтения и записи произойдут автоматически. Эта операция не приведет к потере данных. Однако процесс смены ролей баз данных в обычных условиях может занять до 5 минут. До окончания этого процесса некоторые базы данных в новом экземпляре источника будут оставаться в режиме только для чтения. Если инициируется отработка отказа с помощью PowerShell, вся операция является синхронной. Если он инициируется с помощью портала Azure, пользовательский Интерфейс будет показывать состояние завершения. Если она инициируется с помощью REST API, то используйте стандартный механизм опроса Azure Resource Manager для мониторинга выполнения.
...
## <a name="enabling-geo-replication-between-managed-instances-and-their-vnets"></a>Включить георепликацию между управляемых экземпляров и их виртуальными сетями

Если вы настроили группу отработки отказа между основным и дополнительным экземплярами управляемых в двух разных регионах, каждый экземпляр изолирован с помощью независимых виртуальной сети. Чтобы разрешить трафик репликации между этими виртуальными сетями убедитесь, что эти условия выполнены.

1. Два управляемых экземпляров должны находиться в разных регионах Azure.
2. Ваш получатель должен быть пустым (без пользовательских баз данных).
3. Управляемые экземпляры основного и дополнительного должны находиться в той же группе ресурсов.
4. Виртуальные сети, управляемых экземпляров, которые являются частью необходимостью подключения через [VPN-шлюз](../vpn-gateway/vpn-gateway-about-vpngateways.md). Пиринг глобальной виртуальной сети не поддерживается.
5. Двух виртуальных сетей управляемый экземпляр не может иметь перекрывающиеся IP-адресов.
6. Вам нужно настроить вашей группы безопасности сети (NSG) такой, порта 5022 и диапазон 11000 ~ 12000 откройте входящего и исходящего трафика для подключений от других управляемых внутренне подсети. Это позволяет обеспечить трафик репликации между экземплярами.

   > [!IMPORTANT]
   > Неправильная настройка правил безопасности NSG приводит к задержке операций копирования баз данных.

7. Дополнительный экземпляр настроен с правильным идентификатором зоны DNS. Зона DNS — это свойство является управляемым экземпляром, и его идентификатор включается в адрес имени узла. При создании первым управляемым экземпляром в каждой виртуальной сети и к другим экземплярам в той же подсети назначается тот же идентификатор, идентификатор зоны создается как случайная строка. После назначения, невозможно изменить зоны DNS. Управляемые экземпляры, включенных в ту же группу отработки отказа должны совместно использовать зоны DNS. Для этого, передавая идентификатор зоны первичного экземпляра как значение параметра DnsZonePartner при создании экземпляра-получателя. 
