...
[Группа отработки отказа](sql-database-auto-failover-group.md) — это декларативный слой абстракции, который позволяет сгруппировать несколько геореплицированных баз данных. Узнайте, как настроить группу отработки отказа для отдельной базы данных SQL Azure и тестовую отработку отказа с помощью портал Azure, PowerShell или Azure CLI.  В этом учебнике рассматривается следующее.

> [!div class="checklist"]
> - Создайте отдельную базу данных SQL Azure.
> - Создайте группу отработки отказа для отдельной базы данных между двумя логическими серверами SQL.
...
1. Выберите одну базу данных, созданную в разделе 1, `mySampleDatabase`например. 
1. Группы отработки отказа можно настраивать на уровне сервера. Выберите имя сервера в поле **имя сервера** , чтобы открыть параметры для сервера.
