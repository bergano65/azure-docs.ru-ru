...
- **Временная ошибка возникает во время команды SQL-запроса**.
...
- Не повторяйте сразу инструкцию SQL`SELECT`, которая закончилась временным сбоем. Установите новое подключение и повторите `SELECT`.
- Когда выполнение инструкции SQL `UPDATE` завершается временным сбоем, прежде чем выполнять ее еще раз, установите новое подключение. Логика повторных попыток обеспечивает полноценное выполнение транзакции базы данных или откат всей транзакции.
