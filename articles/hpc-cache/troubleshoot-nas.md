...
> [!NOTE]
> По умолчанию доступ Кэша HPC Azure squashe сжимает root.. Прочитайте [настройку дополнительных настроек кэша](configuration.md#configure-root-squash) для получения подробной информации.
...
* Если ваш VPN позволяет пинг-команды, вы можете проверить отправку полноразмерного пакета.
  ...
  * ``-s 1472``- Установите размер полезной нагрузки до 1472 байтов. Это максимальная полезная нагрузка размера для пакета объемом 1500 байт после учета накладных расходов Ethernet.
  ...
  Если пинг не справляется с 1472 байтами, вероятно, существует проблема размера пакета.

Чтобы решить проблему, возможно, потребуется настроить зажим MSS на VPN, чтобы сделать удаленную систему правильно обнаружить максимальный размер кадра. Чтобы узнать больше, читайте [документацию по параметрам VPN Gateway IPsec/IKE.](../vpn-gateway/vpn-gateway-about-vpn-devices.md#ipsec)

В некоторых случаях может помочь изменение настройки MTU для кэша HPC Azure на 1400. Однако, если вы ограничиваете MTU в кэше, необходимо также ограничить настройки MTU для клиентов и систем хранения бэк-энда, которые взаимодействуют с кэшем. [Прочтите дополнительные настройки Azure HPC Cache](configuration.md#adjust-mtu-value) для получения подробной информации.
