...
Чтобы узнать, какие порты необходимы для вашей системы, используйте следующую команду ``rpcinfo``. Эта команда перечисляет порты и форматирует соответствующие результаты в таблице. (Используйте IP-адрес системы вместо *< storage_IP >* .)
...
Убедитесь, что все порты, возвращенные запросом ``rpcinfo``, разрешают неограниченный трафик из подсети кэша HPC Azure.
...
* Серверы Linux обычно добавляют ``no_root_squash`` в экспортируемый путь в ``/etc/exports``.
...
``/ifs/accounting/payroll`` экспорта является дочерним элементом ``/ifs/accounting``, а ``/ifs/accounting`` сам является дочерним элементом ``/ifs``.

Если добавить ``payroll`` экспорта в качестве целевого объекта хранилища кэша HPC, то кэш будет подключен ``/ifs/`` и будет обращаться к каталогу зарплаты из него. Поэтому для доступа к ``/ifs/accounting/payroll`` экспорту кэш Azure HPC должен иметь корневой доступ к ``/ifs``.
...
Система NAS с иерархическими экспортами может предоставить разные дескрипторы файлов для одного и того же файла, если файл извлекается из разных экспортов. Например, клиент может подключить ``/ifs/accounting`` и получить доступ к ``payroll/2011.txt``у файла. Другой клиент подключает ``/ifs/accounting/payroll`` и обращается к ``2011.txt``у файла. В зависимости от того, как система хранения назначает дескрипторы файлов, эти два клиента могут получить один и тот же файл с разными дескрипторами файлов (один для ``<mount2>/payroll/2011.txt`` и один для ``<mount3>/2011.txt``).
...
Чтобы избежать такого возможного конфликта файлов в нескольких экспортах, кэш Azure HPC автоматически подключает неглубокий доступный экспорт в пути (``/ifs`` в примере) и использует файловый образ, указанный в этом экспорте. Если несколько экспортов используют один и тот же базовый путь, то кэш Azure HPC должен иметь корневой доступ к этому пути.
...
* Если VPN допускает команды ping, можно протестировать отправку пакета с полным размером.
  ...
  * ``-s 1472`` — задайте размер полезных данных 1472 байт. Это максимальный размер полезных данных для 1500-байтового пакета после учета накладных расходов Ethernet.
  ...
  Если проверка связи завершается с 1472 байт, может потребоваться настройка фиксации MSS в VPN, чтобы удаленная система правильно определяла максимальный размер кадра. Дополнительные сведения см. в [документации по параметрам IPsec и IKE VPN-шлюза](../vpn-gateway/vpn-gateway-about-vpn-devices.md#ipsec) .
