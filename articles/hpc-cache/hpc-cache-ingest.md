...
* Частичное автоматическое копирование ``msrsync``  -  ``msrsync`` с помощью — это служебная программа- ``rsync`` оболочка, которая запускает несколько параллельных процессов. Дополнительные сведения см. в статье о [методе приема данных кэша HPC в Azure](hpc-cache-ingest-msrsync.md).
