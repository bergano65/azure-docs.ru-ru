...
Вы можете использовать служебную программу [Авере клфслоад](https://aka.ms/avere-clfsload) для копирования данных в новый контейнер хранилища BLOB-объектов, прежде чем добавлять их в качестве целевого объекта хранилища. Эта служебная программа работает на виртуальной машине Linux и записывает данные в собственном формате, требуемом для кэша HPC для Azure. Это наиболее эффективный способ заполнения контейнера хранилища BLOB-объектов для использования с кэшем.

Этот параметр работает только с новыми пустыми контейнерами. Создайте контейнер перед использованием Авере Клфслоад.

Подробные сведения содержатся в [файле сведений Авере клфслоад](https://github.com/microsoft/Avere-CLFSLoad/blob/master/README.md). <!-- caution literal link -->

Общий обзор процесса:

1. Подготовьте систему Linux (физическую или виртуальную машину) с Python версии 3,6 или более поздней. (Для повышения производительности рекомендуется использовать Python 3,7.)
...
Эти требования подробно описаны в [файле readme Авере клфслоад](https://aka.ms/avere-clfsload).
