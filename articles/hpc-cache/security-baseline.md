...
Azure Active Directory предоставляет управление удостоверениями и доступом к ресурсам Azure, облачным приложениям и локальным приложениям. Сюда входят корпоративные удостоверения, такие как сотрудники, а также внешние удостоверения, такие как партнеры, поставщики и поставщики. Это обеспечивает единый вход (SSO) для управления доступом к данным и ресурсам Организации в локальной среде и в облаке, а также для обеспечения безопасного доступа к ним. Подключайте пользователей, приложения и устройства к Azure AD, чтобы обеспечить простой, безопасный доступ и более широкие возможности управления.
...
- [Как использовать управляемые клиентом ключи шифрования с помощью кэша HPC Azure](https://docs.microsoft.com/azure/hpc-cache/hpc-cache-create?tabs=azure-portal#enable-azure-key-vault-encryption-optional)
...
**Руководство**. кэш HPC поддерживает Azure Resource Manager развертываний. Используйте политику Azure для аудита и ограничения служб, которые пользователи могут подготавливать в вашей среде. Используйте граф ресурсов Azure для запроса и обнаружения ресурсов в своих подписках. Можно также использовать Azure Monitor, чтобы создать правила для активации оповещений при обнаружении неутвержденной службы.
...
- [Как создавать резервные копии ключей Key Vault в Azure](https://docs.microsoft.com/powershell/module/az.keyvault/backup-azkeyvaultkey?view=azps-5.1.0&amp;preserve-view=true)
