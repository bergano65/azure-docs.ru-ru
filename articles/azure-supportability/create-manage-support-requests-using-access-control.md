---
title: Контроль доступа для создания запросов в службу поддержки в Azure
...
> [!IMPORTANT]
> Этот ручной подход к управлению правами доступа к запросу на поддержку в Azure был заменен встроенной ролью. Дополнительные сведения см. в разделе [участник запроса на поддержку](../role-based-access-control/built-in-roles.md#support-request-contributor). 
...
Возьмем пример: Как владелец группы ресурсов с разрешениями на чтение в области подписки можно управлять всеми ресурсами в группе ресурсов, например веб-сайтами, виртуальными машинами и подсетями.
...
5. Вы увидите пользователя с добавленной для него новой пользовательской ролью "Участник с правом создавать запросы на поддержку" в рамках подписки, владельцем которой вы являетесь.
    ...
    Когда Джо выполняет вход на портал, Сергей видит подписку, к которой был добавлен Джо.
...
Чтобы запретить создание запросов на поддержку и управление ими, перейдите к подписке, щелкните "Параметры" и выберите пользователя (в данном случае это Дмитрий). Щелкните правой кнопкой мыши имя роли "Участник с правом создавать запросы на поддержку" и выберите "Удалить".
...
Когда Сергей входит на портал и пытается создать запрос в службу поддержки, Сергей сталкивается со следующей ошибкой:

![Ошибка подписки 2](./media/create-manage-support-requests-using-access-control/subscription-error-2.png)

Джо не может видеть запросы на поддержку, когда Джо выбирает "все запросы на поддержку"
