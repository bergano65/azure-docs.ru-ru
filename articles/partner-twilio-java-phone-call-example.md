...
В следующем примере показано, как выполнить звонок с веб-страницы, размещенной в Azure, с помощью службы Twilio. Полученное приложение подскажет пользователю значения телефонных звонков, как показано на следующем скриншоте.
...
1. Получение учетной записи Twilio и токена подтверждения подлинности. Чтобы начать работу с Twilio, оценить цены на [https://www.twilio.com/pricing][twilio_pricing]. Вы можете зарегистрироваться на [https://www.twilio.com/try-twilio][try_twilio]. Для получения информации о API, [https://www.twilio.com/api][twilio_api]предоставленном Twilio, см.
2. Получите JAR-файл Twilio. В [https://github.com/twilio/twilio-java][twilio_java_github], вы можете скачать источники GitHub и создать свой собственный JAR, или скачать предварительно построенный JAR (с или без зависимостей).
...
5. Убедитесь, что в хранилище ключей cacerts содержится сертификат Equifax Secure Certificate Authority с отпечатком MD5 67:CB:9D:C0:13:24:8A:82:9B:B2:17:1E:D1:1B:EC:D4 (серийный номер 35:DE:F4:CF, отпечаток SHA1 D2:32:09:AD:23:D3:14:23:21:74:E4:0D:7F:9D:62:13:97:86:63:3A). Это сертификат органа (CA) сертификат [https://api.twilio.com][twilio_api_service] для службы, который называется при использовании AIS Twilio. Дополнительные сведения о том, как добавить этот сертификат ЦС в хранилище cacert JDK, см. в статье [Добавление сертификата в хранилище сертификатов ЦС Java][add_ca_cert].
...
Помимо звонка makecall.jsp отображает конечную точку Twilio, версию API-интерфейса и сведения о состоянии звонка. Примером может быть следующий скриншот:
