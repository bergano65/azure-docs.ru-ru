...
Когда служба принимает веб-сокет, регистрация завершается, а установленный веб-сокет поддерживается в рабочем состоянии как "канал управления" для всех последующих взаимодействий. Услуга позволяет до 25 одновременных слушателей для одного гибридного соединения. Квота для AppHooks еще не определена.
...
Прослушиватели, присоединенные к гибридным подключениям с поддержкой HTTP, ДОЛЖНЫ обрабатывать жест `request`. Прослушиватель, который не обрабатывает `request` и, следовательно, вызывает повторяющиеся ошибки времени ожидания при установленном подключении, МОЖЕТ быть внесен в список блокировок службой в будущем.
...
| ---------------- | -------- | -------------------------------------------
| `sb-hc-action`   | Да      | Для роли слушателя параметр должен быть **sb-hc-action**
...
Получатель ДОЛЖЕН отвечать. Повторный отказ ответа на запросы при сохранении подключения может привести к тому, что прослушиватель попадет в список блокировок.
...
* **токен** - действительный, закодированный URL-кодом токен общего доступа Service Bus для пространства имен или гибридного соединения, который предоставляет право **прослушивания.**
