---
title: Метрики реле Azure в Azure Monitor Документы Майкрософт
description: В этой статье приводится информация о том, как можно использовать Azure Monitor для мониторинга состояния реле Azure.
...
---
# <a name="azure-relay-metrics-in-azure-monitor"></a>Метрики реле Azure в Azure Monitor 
...
Использование метрик в Azure Monitor пока что осуществляется бесплатно и находится на этапе предварительной версии. Тем не менее, если вы используете дополнительные решения для приема данных метрик, за использование этих решений может взиматься плата. Например, при архивации данных метрик в учетную запись хранения Azure плата будет взиматься службой хранилища Azure, Кроме того, журналы Azure Monitor выставляют счет, если вы транслируете данные метрик в журналы Azure Monitor для расширенного анализа.
...
|ActiveConnections |Количество активных подключений. Это значение является значением времени.<br/><br/> Единица измерения: число <br/> Тип агрегирования: итог <br/> Измерение: EntityName.|
|ActiveListeners |Количество активных слушателей. Это значение является значением времени.<br/><br/> Единица измерения: число <br/> Тип агрегирования: итог <br/> Измерение: EntityName.|
