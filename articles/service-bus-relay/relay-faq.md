...
description: В этой статье содержатся ответы на некоторые часто задаваемые вопросы о службе эстафеты Azure.
...
(**Это относится только к ретранслятотам WCF. Сообщения не являются затратами для гибридных подключений.**)
...
Ретрансляторы, открытые с помощью привязки WCF **netTCPRelay**, обрабатывают сообщения не по отдельности, а как поток данных, передаваемых через систему. Сведения о формировании отдельных сообщений, отправленных и полученных с помощью этой привязки, доступны только отправителю и прослушивателю. Для ретрансляторов, которые используют привязку **netTCPRelay,** все данные рассматриваются как поток для расчета платежных сообщений. В этом случае служебная шина вычисляет общий объем данных, отправленных или полученных в ходе отдельной ретрансляции с 5-минутными интервалами. Затем общий объем данных будет поделен на блоки по 64 КБ, чтобы определить количество оплачиваемых сообщений ретранслятору, обработанных за заданный период времени.
...
Подписанные URL-адреса (SAS) представляют собой механизм аутентификации на базе алгоритма безопасного хэширования SHA-256 или URI. Для получения информации о том, как создать собственные подписи в Node.js, PHP, Python, Java, C и C, [см.][Shared Access Signatures]
