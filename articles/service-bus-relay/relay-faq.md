...
Полные сведения о ценах на ретранслятор см. в таблице [гибридные подключения и ретрансляторов WCF][Pricing overview] на странице сведений о ценах на служебную шину. Помимо платы, указанной на этой странице, также взимается плата за связанные операции передачи данных, исходящих из центра данных, в котором подготавливается ваше приложение.
...
| --- | --- | --- | --- |
| Параллельные прослушиватели ретранслятора |Учреждение |Запросы на дополнительные соединения отклоняются, а вызывающий код получает исключение. |25 |
...
| Размер сообщения для ретрансляций [HttpRelayTransportBindingElement](/dotnet/api/microsoft.servicebus.httprelaytransportbindingelement) и [NetTcpRelayBinding](/dotnet/api/microsoft.servicebus.nettcprelaybinding) |Пространство имен |Нет ограничений на размер сообщения. |Неограниченная |
...
Описание распространенных исключений и предлагаемых действий см. в разделе [исключения ретранслятора][Relay exceptions].
...
Подписанные URL-адреса (SAS) представляют собой механизм аутентификации на базе алгоритма безопасного хэширования SHA-256 или URI. Сведения о том, как создавать собственные подписи в Node. js, PHP, Python, Java, C и C#, см. в статье [Проверка подлинности служебной шины с помощью подписей общего доступа][Shared Access Signatures].
