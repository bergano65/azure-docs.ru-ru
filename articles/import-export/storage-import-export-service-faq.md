...
### <a name="can-i-use-the-azure-importexport-service-to-copy-pst-mailboxes-and-sharepoint-data-to-microsoft-365"></a>Можно ли использовать службу импорта и экспорта Azure для копирования почтовых ящиков PST и данных SharePoint в Microsoft 365?

Да. Дополнительные сведения см. в статье [об импорте PST-файлов вашей организации](/microsoft-365/compliance/importing-pst-files-to-office-365).
...
В рамках одного задания импорта или экспорта может обрабатываться только 10 дисков. Если необходимо поставить более 10 дисков, нужно создать несколько заданий. Диски, связанные с одним и тем же заданием, необходимо отправлять одним пакетом.
Дополнительные сведения и рекомендации о том, когда емкость данных охватывает несколько заданий импорта диска, обратитесь в служба поддержки Майкрософт.

### <a name="the-uploaded-blob-shows-status-as-lease-expired-what-should-i-do"></a>В отправленном большом двоичном объекте отображается состояние "Lease expired" (Срок аренды истек). Что следует делать?

Можно пропустить поле "срок аренды истек". Служба импорта и экспорта арендует большой двоичный объект во время отправки, чтобы большой двоичный объект не обновлялся во время других процессов в параллельном режиме. Состояние "Lease expired" (Срок аренды истек) подразумевает, что служба импорта и экспорта больше не выполняет отправку и большой двоичный объект доступен для использования.
...
> [!NOTE]
> Центрам обработки данных в Индии требуется письмо с объявлением на бланке (чаллан доставки) для возврата дисков. Чтобы организовать требуемый этап входа, необходимо также зарегистрировать выбранный перевозчик и поделиться сведениями с центром обработки данных.

### <a name="are-there-any-restrictions-with-shipping-and-returning-my-drive-internationally"></a>Существуют ли какие либо ограничения при отправке и возврате диска на международном уровне?
...
После завершения передачи процесс возврата дисков в Международный адрес может занять больше времени, чем обычно 2-3 дней, необходимых для локальной доставки. На этапе, перечисленном в портал Azure в виде упаковки, группа Data Box гарантирует, что будет предоставлена правильная документация, чтобы обеспечить соответствие требованиям различных международных требований импорта и экспорта.

### <a name="are-there-any-special-requirements-for-delivering-my-disks-to-a-datacenter"></a>Существуют ли специальные требования для доставки дисков в центр обработки данных?

Требования зависят от конкретных ограничений центра обработки данных Azure.

- Существует несколько веб-сайтов, например Австралии, Германии и южная часть Соединенного Королевства, для которых требуется запись входящего идентификатора центра обработки данных Майкрософт в поправку по соображениям безопасности. Перед отправкой дисков или дисков в центр обработки данных обратитесь к Azure Датабокс Operations ( adbops@microsoft.com ), чтобы получить это число. Без этого номера пакет будет отклонен.
- Для центров обработки данных в Индии требуются персональные сведения о драйвере, например ИДЕНТИФИКАЦИОНная карта государственных организаций или номер подтверждения. (например, PAN, ААДХАР, DL), Name, Contact и номер формы автомобиля, чтобы получить проход записи шлюза. Чтобы избежать задержки доставки, проинформируйте оператора об этих требованиях.

### <a name="when-creating-a-job-the-shipping-address-is-a-location-that-is-different-from-my-storage-account-location-what-should-i-do"></a>При создании задания был указан адрес доставки, который отличается от расположения учетной записи хранения. Что следует делать?
