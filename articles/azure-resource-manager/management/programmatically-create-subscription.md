...
| `skuId` | Да      | String | Идентификатор SKU, определяющий тип плана Azure. |
| `owners`      | Нет       | String | Идентификатор объекта любого пользователя или субъекта-службы, который вы хотите добавить в качестве владельца RBAC в подписке при ее создании.  |
| `costCenter` | Нет      | String | Центр затрат, связанный с подпиской. Он отображается в CSV-файле использования. |
| `managementGroupId` | Нет      | String | Идентификатор группы управления, в которую будет добавлена подписка. Чтобы получить список групп управления, см. раздел [API группы управления-List](/rest/api/resources/managementgroups/list). Используйте идентификатор группы управления из API. |
...
| `resellerId`      | Нет       | String | Идентификатор MPN торгового посредника, который будет связан с подпиской.  |
