...
Используйте сведения в следующих разделах для создания подписок EA.
...
| `OfferType`   | Да      | Строковый | Предложение подписки. Доступны два варианта предложения EA: [MS-AZR-0017P](https://azure.microsoft.com/pricing/enterprise-agreement/) (для использования в рабочей среде) и [MS-AZR-0148P](https://azure.microsoft.com/offers/ms-azr-0148p/) (для разработки и тестирования, должен быть [включен с помощью портала EA](https://ea.azure.com/helpdocs/DevOrTestOffer)).                |
| `EnrollmentAccountObjectId`      | Да       | Строковый | Идентификатор объекта учетной записи для регистрации, в которой создается подписка и для которой выставляются счета. Это значение GUID, которое вы получили с помощью `Get-AzEnrollmentAccount`. |
...
Сначала установите это расширение для предварительного просмотра, выполнив `az extension add --name subscription` .

Выполните следующую команду [AZ Account Create](/cli/azure/ext/subscription/account?view=azure-cli-latest#-ext-subscription-az-account-create) , заменив на имя, `<enrollmentAccountObjectId>` `name` скопированное на первом шаге ( ```747ddfe5-xxxx-xxxx-xxxx-xxxxxxxxxxxx``` ). Если вы хотите указать владельцев, Узнайте, [как получить идентификаторы объектов пользователя](grant-access-to-create-subscription.md#userObjectId).
...
| `offer-type`   | Да      | Строковый | Предложение подписки. Доступны два варианта предложения EA: [MS-AZR-0017P](https://azure.microsoft.com/pricing/enterprise-agreement/) (для использования в рабочей среде) и [MS-AZR-0148P](https://azure.microsoft.com/offers/ms-azr-0148p/) (для разработки и тестирования, должен быть [включен с помощью портала EA](https://ea.azure.com/helpdocs/DevOrTestOffer)).                |
| `enrollment-account-object-id`      | Да       | Строковый | Идентификатор объекта учетной записи для регистрации, в которой создается подписка и для которой выставляются счета. Это значение GUID, которое вы получили с помощью `az billing enrollment-account list`. |
...
- Существует ограничение в 2000 подписок на учетную запись регистрации. После этого дополнительные подписки для учетной записи можно создать только в портал Azure. Если вы хотите создать больше подписок через API, создайте другую учетную запись регистрации.
...
```
Используйте `displayName` свойство для указания счета выставления счетов, для которого требуется создать подписки. Убедитесь, что Агриементтипе учетной записи — *микрософткустомерагримент*. Скопируйте `name` учетную запись.  Например, если вы хотите создать подписку для `Contoso` счета выставления счетов, скопируйте `5e98e158-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx` . Вставьте это значение в любое место, чтобы его можно было использовать на следующем шаге.
...
Используйте `invoiceSectionDisplayName` свойство для указания раздела счета, для которого необходимо создать подписки. Скопируйте `invoiceSectionId` `billingProfileId` и один из `skuId` разделов для счета. Например, если вы хотите создать подписку типа `Microsoft Azure plan` для `Development` раздела счета, скопируйте `/providers/Microsoft.Billing/billingAccounts/5e98e158-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_2019-05-31/billingProfiles/PBFV-XXXX-XXX-XXX/invoiceSections/GJGR-XXXX-XXX-XXX` , `/providers/Microsoft.Billing/billingAccounts/5e98e158-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_2019-05-31/billingProfiles/PBFV-xxxx-xxx-xxx` и `0001` . Вставьте эти значения в любое место, чтобы их можно было использовать на следующем шаге.
...
Выполните следующий запрос, заменив `<invoiceSectionId>` на `invoiceSectionId` копию, скопированную на втором шаге ( ```/providers/Microsoft.Billing/billingAccounts/5e98e158-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_2019-05-31/billingProfiles/PBFV-XXXX-XXX-XXX/invoiceSections/GJGR-XXXX-XXX-XXX``` ). Необходимо передать объект `billingProfileId` и, `skuId` скопированный на втором шаге, в параметрах запроса API. Если вы хотите указать владельцев, Узнайте, [как получить идентификаторы объектов пользователя](grant-access-to-create-subscription.md#userObjectId).
...
| Имя элемента  | Обязательно | Тип   | Описание                                                                                               |
|---------------|----------|--------|-----------------------------------------------------------------------------------------------------------|
| `displayName` | Да      | Строковый | Отображаемое имя подписки.|
| `billingProfileId`   | Да      | Строковый | Идентификатор профиля выставления счетов, для которого будет выставлен счет за оплату за подписку.  |
| `skuId` | Да      | Строковый | Идентификатор SKU, определяющий тип плана Azure. |
...
```
Используйте `displayName` свойство для указания счета выставления счетов, для которого требуется создать подписки. Убедитесь, что Агриементтипе учетной записи — *микрософтпартнерагримент*. Скопируйте `name` для учетной записи. Например, если вы хотите создать подписку для `Contoso` счета выставления счетов, скопируйте `99a13315-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx` . Вставьте это значение в любое место, чтобы его можно было использовать на следующем шаге.

### <a name="find-customers-that-have-azure-plans"></a>Поиск клиентов, имеющих планы Azure

Выполните следующий запрос, заменив `<billingAccountName>` на `name` копию, скопированную из первого шага ( ```5e98e158-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx``` ), чтобы вывести список всех клиентов в счете, для которого вы можете создавать подписки Azure.
...
Выполните следующий запрос, заменив `<customerId>` на `id` копию, скопированную на втором шаге ( ```/providers/Microsoft.Billing/billingAccounts/99a13315-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx/customers/2281f543-xxxx-xxxx-xxxx-xxxxxxxxxxxx``` ), чтобы получить список всех торговых посредников, доступных для клиента.
...
```
Используйте `description` свойство, чтобы указать торгового посредника, который будет связан с подпиской. Скопируйте `resellerId` для торгового посредника. Например, если необходимо связать `Wingtip` , скопируйте `3xxxxx` . Вставьте это значение в любое место, чтобы его можно было использовать на следующем шаге.
...
Выполните следующий запрос, заменив `<customerId>` на `id` копию, скопированную на втором шаге ( ```/providers/Microsoft.Billing/billingAccounts/99a13315-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx/customers/2281f543-xxxx-xxxx-xxxx-xxxxxxxxxxxx``` ). Передайте необязательный *реселлерид* , скопированный на втором шаге, в параметрах запроса API.
...
|---------------|----------|--------|-----------------------------------------------------------------------------------------------------------|
| `displayName` | Да      | Строковый | Отображаемое имя подписки.|
| `skuId` | Да      | Строковый | Идентификатор SKU плана Azure. Используйте *0001* для подписок типа "план Microsoft Azure" |
