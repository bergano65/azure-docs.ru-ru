...
Если вы разработчик программного обеспечения, которым требуется создавать приложение для управления ресурсами Azure клиента, в этой статье показано, как проверка подлинности с помощью API-интерфейсов Azure Resource Manager и получить доступ к ресурсам в других подписках.

Приложение может получить доступ к интерфейсам API Azure Resource Manager двумя способами:

1. **Доступ к приложению от имени пользователя**: для приложений, доступ к ресурсам для пользователя, выполнившего вход. Этот метод подходит для приложений, например веб-приложений и программ командной строки, которые осуществляют только интерактивное управление ресурсами Azure.
...
В этой статье содержатся пошаговые инструкции по созданию приложения, поддерживающего оба эти метода авторизации. Показано, как каждый шаг, с помощью REST API или C#. Полное приложение ASP.NET MVC можно получить на странице [https://github.com/dushyantgill/VipSwapper/tree/master/CloudSense](https://github.com/dushyantgill/VipSwapper/tree/master/CloudSense).
...
## <a name="register-application"></a>Зарегистрировать приложение
Прежде чем приступить к программированию, сначала нужно зарегистрировать веб-приложение в Azure Active Directory (AD). При регистрации приложения для него создается центральное удостоверение в Azure AD. Оно содержит основные сведения о приложении, например идентификатор клиента OAuth, URL-адреса ответа и учетные данные, используемые приложением для проверки подлинности и доступа к интерфейсам API Azure Resource Manager. Кроме того, регистрации приложения записываются различные делегированные разрешения, которые нужны приложению, при доступе к API-интерфейсов Microsoft для пользователя.

Чтобы зарегистрировать приложение, см. в разделе [краткое руководство: Регистрация приложения в платформе Microsoft identity](../active-directory/develop/quickstart-register-app.md). Присвойте приложению имя и выберите **учетных записей в любой организации directory** для типов поддерживаемых учетных записей. Для URL-адрес перенаправления Укажите домен, связанный с Azure Active Directory.

Чтобы войти в качестве приложения AD, требуется идентификатор приложения и секретный ключ. Идентификатор приложения отображается в разделе обзора приложения. Чтобы создать секрет и запросить разрешения API, см. в разделе [краткое руководство: Настройка клиентского приложения для доступа к веб-API](../active-directory/develop/quickstart-configure-app-access-web-apis.md). Укажите новый секрет клиента. Для API разрешений выберите **управления службами Azure**. Выберите **делегированные разрешения** и **user_impersonation**.
...
В этой статье приводятся примеры запросов REST API, используемых для проверки подлинности пользователя. Можно также использовать вспомогательных библиотек для проверки подлинности в коде. Дополнительные сведения об этих библиотеках см. в статье [Библиотеки проверки подлинности Azure Active Directory](../active-directory/active-directory-authentication-libraries.md). Рекомендации по интеграции функции управления удостоверениями в приложение см. в статье [Руководство разработчика по Azure Active Directory](../active-directory/develop/v1-overview.md).
...
Если вы не только для доступа к Azure Resource Manager для пользователя, но также позволяет пользователям выполнить вход в приложение с помощью своей учетной записью Azure AD, выдавать Open ID подключения авторизовать запрос. С помощью Open ID Connect приложение также получит маркер id_token из Azure AD, используемый приложением для выполнения входа пользователя.
...
Успешный ответ будет содержать маркер доступа (при доступе от имени пользователя) для Azure Resource Manager. Приложение использует этот маркер доступа для доступа к Resource Manager для пользователя. Срок действия маркеров доступа, выданных Azure AD, составляет один час. Маловероятно, что веб-приложения необходимо обновить (пользователь + приложение) маркер доступа. Однако если обновление потребуется, можно воспользоваться маркером обновления, который приложение получает в ответе маркера. Выполните запрос токена OAuth 2.0 к конечной точке токена Azure AD с помощью метода POST:
...
Несмотря на то, что маркеры обновления можно использовать для получения новых маркеров доступа для Azure Resource Manager, они не подходят для автономного доступа в приложении. Срок действия токенов обновления ограничен, и они привязаны к пользователю. Если пользователь покидает организацию, приложение, использующее маркер обновления, теряет доступ. Этот способ не подходит для приложений, используемых группами для управления ресурсами Azure.

## <a name="check-if-user-can-assign-access-to-subscription"></a>Проверка назначения пользователем доступа к подписке
Ваше приложение теперь имеет токен для доступа к Azure Resource Manager для пользователя. Далее нужно подключить приложение к подписке, чтобы оно могло управлять подписками, даже если пользователь отсутствует (долгосрочный автономный доступ).
...
Вызовите [API определения ролей Resource Manager](https://docs.microsoft.com/rest/api/authorization/roledefinitions) на вывод списка всех ролей Azure RBAC, а затем выполните итерацию на результат, чтобы найти определение роли по имени.
...
Не нужно вызывать этот API на постоянной основе. Узнав известный GUID определения роли, можно создать идентификатор определения роли следующего вида:
...
Для проверки этого приложения можно получить доступ к подписке, необходимо выполнить тест задач для подписки, используя маркер только для приложений.
...
Чтобы проверить, что приложение может обращаться к подписке Azure, вы также можете вызвать [разрешений Resource Manager](https://docs.microsoft.com/rest/api/authorization/permissions) API. Этот метод аналогичен методу определения наличия у пользователя права на управление доступом для подписки. Однако на этот раз вызовите API разрешений с помощью маркера доступа только для приложений, полученного на предыдущем шаге.
...
Только пользователи с разрешением на подписки управление доступом можно отключить подписку.
