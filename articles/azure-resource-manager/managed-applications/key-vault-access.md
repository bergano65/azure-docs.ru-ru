...
Чтобы передать секрет из Key Vault в шаблон в управляемом приложении, нужно использовать [связанный шаблон](../templates/linked-templates.md) и ссылку на Key Vault в параметрах для этого связанного шаблона. Укажите идентификатор ресурса Key Vault и имя секрета.
