...
Для развертывания решения можно использовать отдельный шаблон или основной шаблон с множеством связанных шаблонов. Связанные шаблоны могут быть отдельными файлами, связанными с основным шаблоном, или шаблонами, вложенными в основной шаблон.
...
## <a name="deployments-resource"></a>Ресурс развертываний
...
## <a name="external-template"></a>Внешний шаблон

Чтобы создать ссылку на внешний шаблон, используйте свойство **templateLink** . Вы не можете указать локальный файл или файл, доступный только в локальной сети. Можно предоставить только универсальный код ресурса (URI), который включает в себя **http** или **https**. Диспетчер ресурсов должен иметь доступ к шаблону.

Один из вариантов — разместить связанный шаблон в учетной записи хранения и использовать универсальный код ресурса (URI) этого элемента.

Параметры для внешнего шаблона можно указать во внешнем файле или в встроенном виде.

### <a name="external-parameters"></a>Внешние параметры

При предоставлении файла внешнего параметра используйте свойство **parametersLink** :
...
Чтобы передать значение из основного шаблона в связанный шаблон, используйте свойство **Parameters** .
