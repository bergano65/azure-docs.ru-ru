...
Диспетчер развертывания Azure предоставляется в закрытой предварительной версии. Чтобы использовать диспетчер развертывания Azure, заполните [форму регистрации](https://aka.ms/admsignup). Помогите нам улучшить этот компонент, отправляя [отзывы](https://aka.ms/admfeedback).
...
## <a name="supported-locations"></a>Поддерживаемые расположения

Для предварительной версии ресурсы диспетчера развертывания поддерживаются в центральной части США и восточной части США 2. При определении ресурсов в шаблонах топологии и развертывания, например модулей службы, источников артефактов и развертываний, описанных в этой статье, необходимо указать один из этих регионов в качестве расположения. Ресурсы, развертываемые для создания вашей службы, такие как виртуальные машины, учетные записи хранения и веб-приложения, поддерживаются во всех [стандартных расположениях](https://azure.microsoft.com/global-infrastructure/services/?products=all).  
...
Удостоверение должно находиться в одном из поддерживаемых диспетчером развертывания расположений, и это должно быть расположение, в которое выполняется развертывание.
...
Вы можете определить шаг, выполняемый до или после операции развертывания. Сейчас доступен только шаг `wait`. Шаге wait приостанавливает развертывания, после чего его можно продолжить. Это позволяет проверить, что правильно ли работает служба, прежде чем развертывать следующий модуль службы. В следующем примере показан общий формат шага wait.
