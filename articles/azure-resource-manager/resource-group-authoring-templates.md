...
| Имя элемента | обязательные | ОПИСАНИЕ |
|:--- |:--- |:--- |
| имя параметра |Yes |Имя параметра. Должно быть допустимым идентификатором JavaScript. |
| type |Yes |Тип значения параметра. Допустимые типы и значения: **string**, **securestring**, **int**, **bool**, **object**, **secureObject** и **array**. См. раздел [типы данных](#data-types). |
...
| Описание |Нет |Описание параметра, отображаемого для пользователей на портале. Дополнительные сведения см. в разделе [комментариев в шаблонах](#comments). |
...
| Имя элемента | обязательные | ОПИСАНИЕ |
|:--- |:--- |:--- |
| пространство_имен |Yes |Пространство имен для пользовательских функций. Используйте, чтобы избежать конфликтов имен с функциями шаблонов. |
| имя функции |Yes |Имя пользовательской функции. При вызове функции объедините имя функции с пространством имен. Например, чтобы вызвать функцию с именем uniqueName в пространстве имен contoso, используйте `"[contoso.uniqueName()]"`. |
...
| Тип выходных данных |Yes |Тип выходного значения. Выходные значения поддерживают те же типы, что и входные параметры функции. |
| Выходное значение |Yes |Выражение языка шаблона, которое вычисляется и возвращается из функции. |
...
| версия_API |Yes |Версия REST API, которая будет использована для создания ресурса. Чтобы определить доступные значения, см. раздел [Справочник по шаблону](/azure/templates/). |
| type |Yes |Тип ресурса. Это значение представляет собой сочетание пространства имен поставщика ресурсов и типа ресурса (например, **Microsoft.Storage/storageAccounts**). Чтобы определить доступные значения, см. раздел [Справочник по шаблону](/azure/templates/). Для дочернего ресурса формат типа зависит от того, является ли он вложенным в родительский ресурс или определен за пределами родительского ресурса. См. о [настройке имени и типа дочернего ресурса](child-resource-name-type.md). |
...
| Имя элемента | обязательные | ОПИСАНИЕ |
|:--- |:--- |:--- |
| Имя выходных данных |Yes |Имя выходного значения. Должно быть допустимым идентификатором JavaScript. |
...
В Visual Studio Code [расширение средств Azure Resource Manager](./resource-manager-tools-vs-code.md#install-resource-manager-tools-extension) может автоматически обнаружить шаблон диспетчер ресурсов и соответствующим образом изменить языковой режим. Если вы видите **Azure Resource Manager шаблон** в правом нижнем углу VS Code, можно использовать встроенные комментарии. В таком случае встроенные комментарии больше не будут помечены как недопустимые.

![Visual Studio Code режим шаблона Azure Resource Manager](./media/resource-group-authoring-templates/resource-manager-template-editor-mode.png)
