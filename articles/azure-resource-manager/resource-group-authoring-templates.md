...
Чтобы строковый литерал начинаются с левой скобкой `[` и заканчиваются закрывающая квадратная скобка `]`, не интерпретировался как выражение, добавьте дополнительную скобку, чтобы строка начиналась со `[[`. Например, переменная:

```json
"demoVar1": "[[test value]"
```

Разрешается `[test value]`.

Тем не менее если строковый литерал не заканчивается квадратной скобки, не экранировать первой квадратной скобки. Например, переменная:
...
| Имя элемента | Обязательно для заполнения | Описание |
|:--- |:--- |:--- |
| condition | Нет | Логическое значение, указывающее, будет ли подготавливаться ресурс во время этого развертывания. Если установлено значение `true`, при развертывании создается ресурс. Если установлено значение `false`, при развертывании ресурс не создается. См. в разделе [условие](#condition). |
| apiVersion |Да |Версия REST API, которая будет использована для создания ресурса. Чтобы определить доступные значения, см. в разделе [Справочник по шаблонам](/azure/templates/). |
| type |Да |Тип ресурса. Это значение представляет собой сочетание пространства имен поставщика ресурсов и типа ресурса (например, **Microsoft.Storage/storageAccounts**). Чтобы определить доступные значения, см. в разделе [Справочник по шаблонам](/azure/templates/). Дочерний ресурс формат типа зависит от независимо от того, ли она вложенным в родительский ресурс или определен за пределами родительского ресурса. См. в разделе [дочерние ресурсы](#child-resources). |
| name |Да |Имя ресурса. Имя должно соответствовать ограничениям компонентов URI, определенным в RFC3986. Кроме того, службы Azure, которые предоставляют имя ресурса внешним пользователям, проверяют это имя, чтобы убедиться, что это не попытка подделки другого удостоверения. Дочерний ресурс формат имени зависит от независимо от того, ли она вложенным в родительский ресурс или определен за пределами родительского ресурса. См. в разделе [дочерние ресурсы](#child-resources). |
...
| ресурсов |Нет |Дочерние ресурсы, которые зависят от определяемого ресурса. Следует указать только те типы ресурсов, которые разрешены в схеме родительского ресурса. Зависимость от родительского ресурса не подразумевается. Ее необходимо определить явным образом. См. в разделе [дочерние ресурсы](#child-resources). |

### <a name="condition"></a>Условие

Если нужно определить, во время развертывания для создания ресурса, используйте `condition` элемент. Этот элемент возвращает значение True или False. Если значение true, ресурс создан. Если значение false, ресурс не создан. Значение может применяться только ко всему ресурсу.
...
Если вы используете [ссылку](resource-group-template-functions-resource.md#reference) или [списка](resource-group-template-functions-resource.md#list) вычисляется функция с ресурсом, который развертывается по условию, функции, даже если ресурс не был развернут. Отобразится сообщение об ошибке, если функция ссылается на ресурс, который не существует. Используйте [Если](resource-group-template-functions-logical.md#if) функции, чтобы убедиться в том, функция вычисляется только для условий при развертывании ресурса. См. в разделе [Если функция](resource-group-template-functions-logical.md#if) пример шаблона, который используется, если и ссылка с условно развернутого ресурса.
...
Значения, которые можно указать для типа и имя различаются в зависимости от ли дочерний ресурс определен в родительский ресурс или за ее пределами родительского ресурса.

Будучи вложенными в родительский ресурс, используйте:
...
Если определен за пределами родительского ресурса, используйте:
...
Для вложенного типа имеет значение `databases` , но его полным типом ресурса является по-прежнему `Microsoft.Sql/servers/databases`. `Microsoft.Sql/servers/` не указывается, так как предполагается из типа родительского ресурса. Дочернему ресурсу присваивается имя `exampledatabase`, но его полное имя включает в себя имя родительского ресурса. `exampleserver` не указывается, так как предполагается из родительского ресурса.
...
| type |Да |Тип выходного значения. Выходные значения поддерживает те же типы, что и входные параметры шаблона. Если указать **securestring** для вывода типа, значение не будет отображаться в журнале развертывания и не удается получить из другого шаблона. Чтобы использовать значение секрета в более чем один шаблон, сохраните секрет в Key Vault и ссылка на секрет добавляется в файле параметров. Дополнительные сведения см. в разделе [использование Azure Key Vault для передачи защищенного значения параметра во время развертывания](resource-manager-keyvault-parameter.md). |
