...
---
# <a name="string-functions-for-arm-templates"></a>Функции строки для шаблонов ARM
...
| Параметр | Обязательно | Тип | Описание |
|:--- |:--- |:--- |:--- |
| arg1 |Да |строка или массив |Первая строка или массив для конкатации. |
| дополнительные аргументы |нет |строка или массив |Дополнительные строки или массивы в последовательном порядке для конкатации. |

Эта функция может принимать любое количество аргументов, а также строки или массивы параметров. Однако для параметров не удаешься как массивы, так и строки. Строки сопутствуют только другим струнам.
...
| Параметр | Обязательно | Тип | Описание |
|:--- |:--- |:--- |:--- |
| itemToTest |Да |массив, объект или строка |Значение для проверки, пусто ли оно. |
...
Создает отформатированную строку из вхотоза.
...
| arg1 | Да | строка, ряд, или булеан | Значение для включения в отформатированную строку. |
| дополнительные аргументы | нет | строка, ряд, или булеан | Дополнительные значения для включения в отформатированную строку. |

### <a name="remarks"></a>Примечания

Используйте эту функцию для формата строки в шаблоне. Он использует те же параметры форматирования, что и метод [System.String.Format](/dotnet/api/system.string.format) в .NET.

### <a name="examples"></a>Примеры

В следующем примере шаблона показано, как использовать функцию формата.
...
| Имя | Тип | Значение |
| ---- | ---- | ----- |
| formatTest | Строка | Здравствуйте, пользователь. Форматный номер: 8 175 133 |
...
Возвратное значение не является случайной строкой, а скорее результатом функции хэша по параметрам. Возвращаемое значение содержит 36 символов. Это не глобально уникальный. Чтобы создать новый GUID, который не основан на хэш-значении параметров, используйте функцию [newGuid.](#newguid)
...
Целое число, представляющее позицию искомого элемента. Значение отсчитывается, начиная с нуля. Если элемент не найден, -1 возвращается.
...
Целое число, представляющее последнюю позицию искомого элемента. Значение отсчитывается, начиная с нуля. Если элемент не найден, -1 возвращается.
...
Возвращает количество символов в строке, элементы в массиве или свойства уровня корней в объекте.
...
Возвращает значение в формате глобально уникального идентификатора. **Эта функция может быть использована только в значении по умолчанию для параметра.**
...
Функция newGuid отличается от функции [guid](#guid) тем, что она не принимает никаких параметров. Когда вы называете guid с тем же параметром, он возвращает один и тот же идентификатор каждый раз. Используйте guid, когда вам нужно надежно генерировать тот же GUID для конкретной среды. Используйте newGuid, когда вам каждый раз требуется другой идентификатор, например развертывание ресурсов в тестовой среде.

Функция newGuid использует [структуру Guid](/dotnet/api/system.guid) в рамочном интерфейсе .NET для создания глобально уникального идентификатора.

Если вы используете [опцию для передислокации более раннего успешного развертывания,](rollback-on-error.md)а более раннее развертывание включает параметр, в ключающий newGuid, параметр не переоценивается. Вместо этого значение параметра от предыдущего развертывания автоматически используется в развертывании отката.

В тестовой среде может потребоваться повторное развертывание ресурсов, которые живут только в течение короткого времени. Вместо того, чтобы создавать уникальные имена, вы можете использовать newGuid с [uniqueString](#uniquestring) для создания уникальных имен.

Будьте осторожны передислокации шаблона, который опирается на функцию newGuid для значения по умолчанию. При передислокации и не предоставлении значения для параметра функция переоценивается. Если требуется обновить существующий ресурс, а не создать новый, перейдите в значение параметра от предыдущего развертывания.
...
В следующем примере шаблон показывает параметр с новым идентификатором.
...
| Имя | Тип | Значение |
| ---- | ---- | ----- |
| guidOutput | строка | b76a51fc-bd72-4a77-b9a2-3c29e7d2e5551 |

В следующем примере используется функция newGuid для создания уникального имени для учетной записи хранилища. Этот шаблон может работать для тестовой среды, где учетная запись хранилища существует в течение короткого времени и не передислоцируется.
...
| Имя | Тип | Значение |
| ---- | ---- | ----- |
| имяOutput | строка | storagenziwvyru7uxie |
...
| numberToSkip |Да |INT |Число элементов или знаков, которые необходимо пропустить. Если это значение меньше или равно 0, то возвращаются все элементы или знаки в значении. Если он больше длины массива или строки, возвращается пустой массив или строка. |
...
| numberToTake |Да |INT |Число элементов или знаков, которые необходимо взять. Если это значение меньше или равно 0, то возвращается пустой массив или строка. Если он больше длины данного массива или строки, все элементы массива или строки возвращаются. |
...
Возвратное значение не является случайной строкой, а скорее результатом функции хэша. Возвращаемое значение содержит 13 знаков. Это не глобально уникальный. Вы можете добавить к значению префикс из своего соглашения об именовании, чтобы создать значимое имя. В следующем примере показан формат возвращаемого значения. Фактическое значение зависит от указанных параметров.
...
В следующем примере показано, как создать уникальное имя учетной записи хранения на основе вашей группы ресурсов. Внутри группы ресурсов имя не является уникальным, если построено таким же образом.
...
Если вам нужно создавать новое уникальное имя каждый раз, когда вы развертываете шаблон, и не намерены обновлять ресурс, вы можете использовать функцию [utcNow](template-functions-date.md#utcnow) с uniqueString. Этот подход можно использовать в тестовой среде. Например, [см.](template-functions-date.md#utcnow)
...
| Параметр | Обязательно | Тип | Описание |
|:--- |:--- |:--- |:--- |
| baseUri |Да |строка |Строка базового универсального кода ресурса (URI). Позаботьтесь о том, чтобы наблюдать за поведением в отношении обработки задней черты ('/'), как описано после этой таблицы.  |
...
* Если **baseUri** заканчивается в задней черту, результат просто **baseUri** следуют **relativeUri**.

* Если **baseUri** не заканчивается в задней черте одна из двух вещей происходит.  

   * Если **у baseUri** нет слэши вообще (кроме "//" вблизи фронта), то результат просто **baseUri** следует **за relativeUri**.

   * Если **baseUri** имеет некоторые слэши, но не заканчивается с чертой, все из последнего слэша вперед удаляется из **baseUri** и результат **baseUri** следуют **relativeUri**.
...
```
Для получения полной информации, **базовые** и **relativeUri** параметры решаются, как указано в [RFC 3986, раздел 5](https://tools.ietf.org/html/rfc3986#section-5).
