...
ms.locfileid: "80156112"
---
# <a name="develop-arm-templates-for-cloud-consistency"></a>Разработка шаблонов ARM для согласованности облачных технологий

[!INCLUDE [requires-azurerm](../../../includes/requires-azurerm.md)]

Основным преимуществом Azure является согласованность. Вложения в разработку для одного расположения можно повторно использовать для другого. Шаблон Azure Resource Manager (ARM) делает развертывание последовательным и повторяемым в средах, включая глобальные суверенные облака Azure, Azure и Azure Stack. Для повторного использования шаблонов в разных облачных средах, тем не менее, необходимо учитывать зависимости, присущие конкретным средам, о которых рассказывается в этом руководстве.
...
* Изолированные суверенные облака, такие как Azure Germany, Azure Government и Azure China 21Vianet. Национальные облака обеспечивают согласованную платформу с большинством замечательных возможностей, к которым имеют доступ клиенты глобальной платформы Azure по всему миру.
...
Остальная часть этого руководства описывает области, которые следует учитывать при планировании разработки новых или обновления существующих шаблонов ARM для Azure Stack. Как правило, такой контрольный список должен включать следующие элементы:
...
Для введения в шаблоны [Template deployment](overview.md)ARM см.

## <a name="ensure-template-functions-work"></a>Проверка работы функций шаблонов

Основным синтаксисом шаблона ARM является JSON. Шаблоны используют подмножество нотации JSON, расширяя синтаксис с помощью выражений и функций. Модуль обработки языка шаблонов часто обновляется для поддержки дополнительных функций шаблонов. Подробное объяснение доступных функций [ARM template functions](template-functions.md)шаблона см.
...
1. Сделайте клон репозитория GitHub: [https://github.com/marcvaneijk/arm-template-functions](https://github.com/marcvaneijk/arm-template-functions).
...
* **mystorageaccount1.blob.core.chinacloudapi.cn** при создании в облаке Azure China 21Vianet.
...
Следующий пример кода показывает последовательный подход к ссылке на параметры издателя, предложения и SKU в шаблонах ARM:
...
* [Рекомендации для шаблонов ARM](template-syntax.md)
