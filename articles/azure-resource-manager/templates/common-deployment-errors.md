...
| субскриптионнотрегистеред | При развертывании сетевых ресурсов поставщик ресурсов Microsoft. Network автоматически регистрируется в подписке. В некоторых случаях автоматическая регистрация не завершается вовремя. Чтобы избежать этой временной ошибки, перед развертыванием зарегистрируйте поставщик ресурсов Microsoft. Network. | [Устранение ошибок регистрации](error-register-resource-provider.md) |
...
Ошибки обоих типов возвращают код ошибки, с помощью которого можно устранить неполадки развертывания. Оба типа ошибок отображаются в [журнале действий](../management/view-activity-logs.md). Однако ошибки проверки не отображаются в журнале развертывания, так как при их наличии развертывание не запускается.
