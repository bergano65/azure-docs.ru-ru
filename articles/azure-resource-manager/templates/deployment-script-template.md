...
Узнайте, как использовать сценарии развертывания в шаблонах ресурсов Azure. С помощью нового типа ресурсов `Microsoft.Resources/deploymentScripts` пользователи могут выполнять сценарии в развертываниях шаблонов и просматривать результаты выполнения. Эти сценарии можно использовать для выполнения настраиваемых действий, в том числе с их помощью можно:
...
- **Identity**: для API скрипта развертывания версии 2020-10-01 или более поздней назначенное пользователем управляемое удостоверение является необязательным, если в скрипт не нужно выполнять какие-либо действия Azure.  Для API версии 2019-10-01-Preview требуется управляемое удостоверение, так как служба скрипта развертывания использует ее для выполнения скриптов. Сейчас поддерживаются только управляемые удостоверения, назначаемые пользователем.
...
Чтобы хранить экземпляр контейнера и учетную запись хранения для устранения неполадок, можно добавить в скрипт команду Sleep.  Например, [Start-Sleep](https://docs.microsoft.com/powershell/module/microsoft.powershell.utility/start-sleep).
