...
- **(Необязательно.) назначенное пользователем управляемое удостоверение с необходимыми разрешениями для выполнения операций в скрипте**. Для API скрипта развертывания версии 2020-10-01 или более поздней используется субъект развертывания для создания базовых ресурсов. Если скрипту требуется пройти проверку подлинности в Azure и выполнить действия, связанные с Azure, рекомендуется предоставить скрипту управляемое удостоверение, назначаемое пользователем. Управляемое удостоверение должно иметь необходимый доступ в Целевой группе ресурсов для завершения операции в скрипте. Вы также можете войти в Azure в скрипте развертывания. Для выполнения операций вне группы ресурсов необходимо предоставить дополнительные разрешения. Например, назначьте удостоверение для уровня подписки, если хотите создать новую группу ресурсов. 
...
- **Удостоверение**. для API скрипта развертывания версии 2020-10-01 или более поздней назначенное пользователем управляемое удостоверение является необязательным, если в скрипт не требуется выполнять какие-либо действия, связанные с Azure.  Для API версии 2019-10-01-Preview требуется управляемое удостоверение, так как служба скрипта развертывания использует ее для выполнения скриптов. Сейчас поддерживаются только управляемые удостоверения, назначаемые пользователем.
...
Вы несете ответственность за обеспечение целостности скриптов, на которые ссылается скрипт развертывания, **примарискриптури** или **суппортингскриптурис**.  Указывайте только те надежные сценарии.
...
Учетная запись хранения и экземпляр контейнера необходимы для выполнения сценария и устранения неполадок. Вы можете указать существующую учетную запись хранения. В противном случае учетная запись хранения вместе с экземпляром контейнера автоматически создается службой сценариев. Эти два создаваемых автоматически ресурса удаляются службой сценариев, когда выполнение сценария развертывания достигает состояния завершения. Плата взимается за ресурсы, пока ресурсы не будут удалены. Сведения о ценах приведены на страницах цен на [экземпляры контейнеров](https://azure.microsoft.com/pricing/details/container-instances/) и [службу хранилища Azure](https://azure.microsoft.com/pricing/details/storage/).
...
- **cleanupPreference**. Очистка параметров при завершении выполнения сценария. Поддерживаются такие значения:
  ...
  - **Onexpir**: удаляет автоматически созданные ресурсы только после истечения срока действия параметра **ретентионинтервал** . Если используется существующая учетная запись хранения, служба скриптов удаляет файловый ресурс, но оставляет учетную запись хранения.
...
Экземпляр контейнера и учетная запись хранения удаляются в соответствии с **клеануппреференце**. Однако если сценарий завершается ошибкой и **клеануппреференце** не имеет значение **Always**, то процесс развертывания автоматически удерживает контейнер в течение одного часа. Этот час можно использовать для устранения неполадок сценария. Если вы хотите, чтобы контейнер выполнялся после успешного развертывания, добавьте в сценарий шаг спящего режима. Например, добавьте [Start-Sleep](https://docs.microsoft.com/powershell/module/microsoft.powershell.utility/start-sleep) в конец скрипта. Если вы не добавите шаг спящего режима, контейнер будет установлен в состояние терминала и к нему нельзя получить доступ, даже если он еще не был удален.

## <a name="run-script-more-than-once"></a>Многократное выполнение сценария

Выполнение сценария развертывания является идемпотентной операцией. Если ни одно из свойств ресурса deploymentScripts (включая встроенный скрипт) не изменилось, сценарий не выполняется при повторном развертывании шаблона. Служба сценариев развертывания сравнивает имена ресурсов в шаблоне с существующими ресурсами в той же группе ресурсов. Существуют два варианта многократного выполнения одного сценария.

- Измените имя ресурса deploymentScripts. Например, используйте функцию шаблона [utcNow](./template-functions-date.md#utcnow) в качестве имени ресурса или части его имени. При изменении имени ресурса создается новый ресурс deploymentScripts. Рекомендуется хранить историю выполнения скрипта.
