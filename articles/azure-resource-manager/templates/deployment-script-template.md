...
- **Azure PowerShell версии 2.7.0, 2.8.0 или 3.0.0**. Эти версии не требуются для развертывания шаблонов. Но эти версии необходимы для локального тестирования сценариев развертывания. См. статью [Установка модуля Azure PowerShell](/powershell/azure/install-az-ps.md). Вы можете использовать предварительно настроенный образ DOCKER.  См. раздел [Настройка среды разработки](#configure-development-environment).
