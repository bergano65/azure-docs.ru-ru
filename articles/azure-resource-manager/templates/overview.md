...
* **Предварительный просмотр изменений**. перед развертыванием шаблона можно использовать [операцию "что если](template-deploy-what-if.md) ", чтобы получить предварительную версию изменений. В разделе что если вы увидите, какие ресурсы будут созданы, обновлены или удалены, а также какие свойства ресурсов будут изменены. Операция "что если" проверяет текущее состояние среды и устраняет необходимость в управлении состоянием.
