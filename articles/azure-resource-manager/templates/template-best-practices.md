...
ms.locfileid: "80156418"
---
# <a name="arm-template-best-practices"></a>Лучшие практики шаблона ARM

В этой статье содержатся рекомендации о том, как создать шаблон Azure Resource Manager (ARM). Эти рекомендации помогут вам избежать распространенных проблем при использовании шаблона ARM для развертывания решения.

Рекомендации о том, как управлять подписками Azure, можно найти в [корпоративном управлении Azure: Предписывающее управление подпиской.](/azure/architecture/cloud-adoption/appendix/azure-scaffold?toc=%2Fen-us%2Fazure%2Fazure-resource-manager%2Ftoc.json&bc=%2Fen-us%2Fazure%2Fbread%2Ftoc.json)
...
Ограничьте размер шаблона 4 МБ, а каждый параметр файла до 64 КБ. Ограничение 4 МБ применяется к конечному состоянию шаблона после того, как оно было расширено с помощью итеративных определений ресурсов и значений для переменных и параметров. 
...
* Используйте `allowedValues` только в крайнем случае. Используйте ее только в том случае, когда необходимо убедиться в том, что некоторые значения не включены в разрешенные параметры. Если вы `allowedValues` используете слишком широко, вы можете заблокировать допустимые развертывания, не обновляя свой список.
...
* Используйте верблюжьи чехла для переменных имен.
...
При принятии решения о том, какие [зависимости](define-resource-dependency.md) следует установить, используйте следующие рекомендации:
...
* Если вы используете в шаблоне *общедоступную конечную точку* (например, общедоступную конечную точку хранилища BLOB-объектов Azure), то *не следует жестко кодировать* пространство имен. Используйте функцию **reference** для динамического извлечения пространства имен. Вы можете использовать этот подход, чтобы развернуть шаблон в другом общедоступном пространстве имен, не изменяя конечную точку в шаблоне вручную. Задайте для версии API ту же версию, которая указана в учетной записи хранения в вашем шаблоне.
   ...
   Если учетная запись хранилища развернута в том же шаблоне, который вы создаете, и имя учетной записи хранилища не передается другому ресурсу в шаблоне, вам не нужно указывать пространство имени поставщика или apiVersion при ссылке на ресурс. В следующем примере показан упрощенный синтаксис.
...
* Для получения информации о структуре файла шаблонов [см.](template-syntax.md)
* Для рекомендаций о том, как создавать шаблоны, которые работают во всех облачных средах Azure, [см.](templates-cloud-consistency.md)
