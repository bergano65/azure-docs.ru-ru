...
**Автоматические удаления поддерживаются для развертываний групп ресурсов. В настоящее время развертывания в журнале развертывания для [подписки](deploy-to-subscription.md), [группы управления](deploy-to-management-group.md)и развертывания [клиентов](deploy-to-tenant.md) не удаляются.**
...
Развертывания удаляются из журнала при достижении 775 или более развертываний. Azure Resource Manager удаляет развертывания до тех пор, пока журнал не будет отключен до 750. Самые старые развертывания всегда удаляются первыми.
