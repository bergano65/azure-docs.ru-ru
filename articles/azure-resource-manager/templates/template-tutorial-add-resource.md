...
description: Здесь описаны действия по созданию первого шаблона Azure Resource Manager (шаблона ARM). Вы узнаете о синтаксисе файла шаблона и о том, как развернуть учетную запись хранения.
author: mumian
ms.date: 03/27/2020
ms.topic: tutorial
ms.author: jgao
...
ms.locfileid: "97106977"
---
# <a name="tutorial-add-a-resource-to-your-arm-template"></a>Руководство по добавлению ресурса в шаблон ARM

Из [предыдущего учебника](template-tutorial-create-first-template.md) вы узнали, как создать пустой шаблон Azure Resource Manager (ARM) и развернуть его. Теперь все готово для развертывания ресурса. В этом учебнике вы узнаете, как добавить учетную запись хранения. Для работы с этим учебником потребуется около **9 минут**.
...
Большинство ресурсов также имеют свойство `location`, которое задает регион, где развернут ресурс.
...
- `Error: Code=AccountNameInvalid; Message={provide-unique-name}` не является допустимым именем учетной записи хранения. Имя учетной записи хранения должно содержать от 3 до 24 знаков и состоять только из цифр и букв нижнего регистра.
...
- Имя "`Error: Code=StorageAccountAlreadyTaken; Message=The storage account named store1abc09092019`" уже занято.
