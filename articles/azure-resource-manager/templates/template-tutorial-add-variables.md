...
Вы уже знакомы с функцией [parameters](template-functions-deployment.md#parameters), поэтому мы не будем ее изучать.

Вы также знакомы с функцией [resourceGroup](template-functions-resource.md#resourcegroup). В этом случае вы получаете свойство `id` вместо свойства `location`, как показано в предыдущем учебнике. Свойство `id` возвращает полный идентификатор группы ресурсов, включая идентификатор подписки и имя группы ресурсов.
...
Функция [concat](template-functions-string.md#concat) принимает значения и объединяет их. Для этой переменной она принимает строку из параметра и из функции `uniqueString` и объединяет эти строки в одну.
