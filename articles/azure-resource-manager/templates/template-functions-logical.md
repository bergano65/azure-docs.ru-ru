---
title: Функции шаблона - логические
...
| Параметр | Обязательно | Тип | Описание |
|:--- |:--- |:--- |:--- |
| condition |Да |Логическое |Значение, чтобы проверить, является ли это правдой или ложью. |
...
Когда условие **верно,** оценивается только истинное значение. Когда условие **ложное,** оценивается только ложное значение. С функцией **if** можно включить выражения, которые являются только условно действительными. Например, можно ссылаться на ресурс, существующий под одним условием, но не под другим условием. Пример условной оценки выражений приведен в следующем разделе.
...
В следующем [примере шаблон](https://github.com/krnese/AzureDeploy/blob/master/ARM/deployments/conditionWithReference.json) показывает, как использовать эту функцию с выражениями, которые являются только условно действительными.
