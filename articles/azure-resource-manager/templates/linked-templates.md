...
Область задается через `expressionEvaluationOptions` свойство. По умолчанию `expressionEvaluationOptions` свойство имеет значение `outer`, то есть использует область родительского шаблона. Установите значение `inner` , чтобы выражения выводились в области вложенного шаблона.
...
В следующем шаблоне показано, как выражения шаблона разрешаются в соответствии с областью. Он содержит переменную с `exampleVar` именем, которая определена как в родительском, так и во вложенном шаблоне. Он возвращает значение переменной.
...
Значение `exampleVar` изменяется в зависимости от значения `scope` свойства в. `expressionEvaluationOptions` В следующей таблице показаны результаты для обеих областей.
...
При ссылке на связанный шаблон значение не `uri` должно быть локальным файлом или файлом, доступным только в локальной сети. Необходимо указать значение URI, которое можно скачать как **http** или **HTTPS**. 

> [!NOTE]
>
> Вы можете ссылаться на шаблоны с помощью параметров, которые в конечном итоге разрешаются **https**для чего-либо, использующего `_artifactsLocation` **http** или HTTPS, например с помощью параметра следующим образом:`"uri": "[concat(parameters('_artifactsLocation'), '/shared/os-disk-parts-md.json', parameters('_artifactsLocationSasToken'))]",`
...
Как и в случае с другими типами ресурсов, можно задать зависимости между связанным шаблоном и другими ресурсами. Если другим ресурсам требуется выходное значение из связанного шаблона, убедитесь, что связанный шаблон развернут перед ними. Если же связанный шаблон зависит от других ресурсов, обязательно разверните их перед развертыванием связанного шаблона.
