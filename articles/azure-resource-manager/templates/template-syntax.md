...
При указании в шаблоне значений типа Boolean и целых чисел не следует заключать значение в кавычки. Начальные и конечные строковые значения заключаются в двойные кавычки ( `"string value"` ).

Объекты начинаются с левой фигурной скобки ( `{` ) и заканчиваются закрывающей фигурной скобкой ( `}` ). Массивы начинаются с левой квадратной скобки ( `[` ) и заканчиваются правой квадратной скобкой ( `]` ).
...
В `variables` разделе вы можете создавать значения, которые можно использовать в шаблоне. Переменные определять не обязательно, однако они часто упрощают шаблон, снижая число сложных выражений. Формат каждой переменной соответствует одному из [типов данных](#data-types).
