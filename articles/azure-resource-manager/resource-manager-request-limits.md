...
ms.locfileid: "67705120"
---
# <a name="throttling-resource-manager-requests"></a>Регулирование запросов Resource Manager

Для каждой подписки и клиента Azure Resource Manager ограничивает число запросов на чтение до 12 000 и запросов на запись — до 1200 в час. Эти ограничения относятся к субъекту безопасности (пользователя или приложения) запросы и идентификатор подписки или идентификатор клиента Если запросы поступают из более чем субъект безопасности, лимит по подписке или клиента больше, чем 12 000 и 1200 в час.

Запросы применяются к вашей подписке или вашему клиенту. Запросы подписок, которые включают в себя передачу идентификатора подписки, такие как извлечение ресурсов в вашей подписке. Запросы клиента не включают идентификатор подписки (например, извлечение допустимых расположений Azure).
