---
title: Шаблон для создания предупреждений о здоровье ресурсов
...
> [!NOTE]
> Оповещения о здоровье ресурсов в настоящее время находятся в предварительном просмотре.
...
## <a name="resource-manager-template-options-for-resource-health-alerts"></a>Параметры шаблона управления ресурсами для предупреждений о здоровье ресурсов
...
Обратите внимание, что в некоторых событиях свойства currentHealthStatus и previousHealthStatus могут быть нулевыми. Например, при возникновении события обновленного может быть, что состояние работоспособности ресурса не изменилось со времени последнего отчета, доступна только дополнительная информация о событии (например, причина). Таким образом, использование вышеуказанного положения может привести к тому, что некоторые оповещения не будут срабатываны, так как значения properties.currentHealthStatus и properties.previousHealthStatus будут сведены к нулю.
...
События ресурсного здоровья могут быть инициированы платформой и событиями, инициированными пользователем. Оповещение имеет смысл отправлять только тогда, когда событие работоспособности вызвано платформой Azure.
...
```
Обратите внимание, что в некоторых случаях поле причины может быть сведено к нулю. То есть происходит переход работоспособности (например, доступен для недоступного) и событие регистрируется немедленно, чтобы предотвратить задержки уведомлений. Таким образом, использование вышеуказанного положения может привести к тому, что оповещение не срабатывает, так как значение свойства.clause будет сведено на нет.

## <a name="complete-resource-health-alert-template"></a>Полный шаблон оповещения о здоровье ресурсов

Используя различные корректировки, описанные в предыдущем разделе, вот шаблон образца, который настроен, чтобы максимизировать соотношение сигнала к шуму. Имейте в виду предостережения, отмеченные выше, где currentHealthStatus, previousHealthStatus, и вызвать значения свойства могут быть нулевыми в некоторых событиях.
...
-  [Обзор работоспособности ресурсов Azure](Resource-health-overview.md)
-  [Типы ресурсов и проверки работоспособности, доступные через Azure Resource Health](resource-health-checks-resource-types.md)
