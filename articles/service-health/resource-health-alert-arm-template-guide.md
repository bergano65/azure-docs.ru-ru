...
## <a name="resource-manager-template-options-for-resource-health-alerts"></a>Параметры шаблона Resource Manager для оповещения о работоспособности ресурсов
...
Обратите внимание на то, что это возможно для свойств currentHealthStatus и previousHealthStatus должен иметь значение null, в некоторые события. К примеру, когда событие Updated происходит вполне вероятно, что состояние работоспособности ресурса не изменился с момента последнего отчета, Дополнительные сведения о событий доступен только (например вызвать). Таким образом, использование в предложении выше может привести некоторые оповещения не запустилась, поскольку значения properties.currentHealthStatus и properties.previousHealthStatus устанавливается в значение null.
...
```
Обратите внимание на то, что это возможно для поля "причина" должен иметь значение null, в некоторые события. То есть работоспособности переход выполняется (например доступно на недоступный) и данное событие регистрируется, сразу же для предотвращения уведомлений задерживает. Таким образом, с помощью предложения выше может привести оповещение не запустилась, поскольку properties.clause значение свойства устанавливается значение null.

## <a name="complete-resource-health-alert-template"></a>Полный шаблон оповещения о работоспособности ресурсов

С помощью различных коррекции, описанным в предыдущем разделе, ниже приведен пример шаблона, который настроен для обеспечения максимальной сигнала помех. Имейте в виду предупреждения, указанным выше где currentHealthStatus, previousHealthStatus и значения свойств причина может иметь значение null, в некоторые события.
