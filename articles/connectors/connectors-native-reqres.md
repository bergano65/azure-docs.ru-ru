...
Триггер — это событие, которое можно использовать для запуска рабочего процесса, определенного в приложении логики. 
[Дополнительные сведения о триггерах](../connectors/apis-list.md).

Ниже приведен пример последовательности для настройки HTTP-запроса в конструкторе приложений логики.

1. Добавьте триггер **Request - When an HTTP request is received** (Запрос: при получении HTTP-запроса) в свое приложение логики. При необходимости можно предоставить схему JSON (с помощью такого средства, как [JSONSchema.net](https://jsonschema.net)) для текста запроса. Это позволит конструктору создать токены свойств в HTTP-запросе.
...
| Действие | ОПИСАНИЕ |
| --- | --- |
| Отклик |Возвращает ответ на связанный HTTP запрос |
...
| Заголовки |Заголовки |Объект JSON заголовков ответов, которые нужно включить |
