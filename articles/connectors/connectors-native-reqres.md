---
title: Реагирование на HTTP-запросы — Azure Logic Apps
description: Реагирование на события в режиме реального времени по протоколу HTTP с помощью Azure Logic Apps
services: logic-apps
...
ms.locfileid: "70914335"
---
# <a name="respond-to-http-requests-by-using-azure-logic-apps"></a>Реагирование на HTTP-запросы с помощью Azure Logic Apps

С помощью [Azure Logic Apps](../logic-apps/logic-apps-overview.md) и встроенного триггера запроса или действия ответа можно создавать автоматизированные задачи и рабочие процессы, которые получают и отвечают в реальном времени на HTTP-запросы. Например, вы можете использовать приложение логики:

* Ответ на HTTP-запрос данных в локальной базе данных.
* Запускает рабочий процесс, когда происходит событие внешнего веб-перехватчика.
* Вызов приложения логики из другого приложения логики.
...
1. После открытия конструктора приложений логики в поле поиска введите "HTTP-запрос" в качестве фильтра. В списке триггеров выберите **время получения HTTP-запроса** триггера, который является первым шагом в рабочем процессе приложения логики.
   ...
   ![Выбор триггера запроса HTTP](./media/connectors-native-reqres/select-request-trigger.png)
   ...
   | **Схема JSON тела запроса** | `schema` | Нет | Схема JSON, описывающая свойства и значения в теле входящего HTTP-запроса |
   |||||

1. В поле **схема JSON тела запроса** при необходимости введите схему JSON, описывающую текст HTTP-запроса во входящем запросе, например:
...
Вы можете использовать ответное действие, чтобы ответить на полезную нагрузку (данные) на входящий HTTP-запрос, но только в приложении логики, которое активируется HTTP-запросом. Действие ответа можно добавить в любой момент в рабочем процессе. Дополнительные сведения о базовом определении JSON для этого триггера см. в разделе [тип действия ответа](../logic-apps/logic-apps-workflow-actions-triggers.md#response-action).
...
1. Добавьте любые значения, необходимые для ответного сообщения. 
   ...
   | Имя свойства | Имя свойства JSON | Обязательное значение | Описание |
   |---------------|--------------------|----------|-------------|
   | **Код состояния** | `statusCode` | Да | Код состояния HTTP, возвращаемый в ответе |
