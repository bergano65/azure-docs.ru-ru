...
> [!NOTE]
> Триггер запроса поддерживает *только* протокол TLS 1.2 для входящих вызовов. Исходящие вызовы поддерживают протокол TLS версий 1.0, 1.1 и 1.2. Дополнительные сведения см. в статье [Решение проблемы с TLS 1.0](https://docs.microsoft.com/security/solving-tls1-problem).
>
> Если вы получаете ошибки подтверждения TLS, убедитесь в том, что используется протокол TLS 1.2. 
> Для входящих вызовов поддерживаются следующие наборы шифров:
>
