---
title: Получение и ответ на вызовы HTTPS
...
> [!NOTE]
> Триггер запроса поддерживает *только* протокол TLS 1,2 для входящих вызовов. Исходящие вызовы продолжают поддерживать TLS 1,0, 1,1 и 1,2. Дополнительные сведения см. [в разделе решение проблемы TLS 1,0](https://docs.microsoft.com/security/solving-tls1-problem).
>
> Если вы видите ошибки подтверждения SSL, убедитесь, что вы используете TLS 1,2. Для входящих вызовов ниже приведены поддерживаемые комплекты шифров:
>
...
1. После открытия конструктора приложений логики в поле поиска введите "HTTP-запрос" в качестве фильтра. В списке триггеров выберите **время получения HTTP-запроса** триггера, который является первым шагом в рабочем процессе приложения логики.
   ...
   | Имя свойства | Имя свойства JSON | Обязательно для заполнения | Description |
   |---------------|--------------------|----------|-------------|
...
1. В поле **схема JSON тела запроса** при необходимости введите схему JSON, описывающую текст входящего запроса, например:
   ...
   При вводе схемы JSON конструктор отображает напоминание о том, что в запрос включается заголовок `Content-Type`, и для этого заголовка задается значение `application/json`. Дополнительные сведения см. в разделе [Handle Content Types](../logic-apps/logic-apps-content-type.md).
...
|--------------------|-----------|-------------|
| `headers` | Объекты | Объект JSON, описывающий заголовки из запроса |
| `body` | Объекты | Объект JSON, описывающий содержимое текста из запроса. |
||||
...
1. Добавьте любые значения, необходимые для ответного сообщения. 
   ...
   Например, для поля **заголовки** включите `Content-Type` в качестве имени ключа и задайте для значения ключа значение `application/json`, как упоминалось ранее в этом разделе. В поле **основной текст** можно выбрать выходные данные основного текста триггера из списка динамического содержимого.
