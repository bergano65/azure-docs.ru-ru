---
title: Подключение к Центрам событий Azure с помощью Azure Logic Apps | Документация Майкрософт
...
См. технические сведения о <a href="https://docs.microsoft.com/connectors/eventhubs/" target="blank">соединителях Центров событий Azure</a>.
...
2. Перейдите к *пространству имен* Центров событий (но не к конкретному концентратору событий). На странице пространства имен в разделе **Параметры** выберите **Политики общего доступа**. В разделе **Утверждения** убедитесь, что у вас есть разрешения на **управление** для этого пространства имен.
...
3. Если вы хотите в дальнейшем вручную ввести сведения о подключении, получите строку подключения для пространства имен Центров событий. 
...
4. Теперь продолжайте дальше, используя варианты [Add an Event Hubs trigger](#add-trigger) (Добавить триггер Центров событий) или [Add an Event Hubs action](#add-action) (Добавить действие Центров событий).

<a name="add-trigger"></a>

## <a name="add-an-event-hubs-trigger"></a>Добавка триггера Центров событий
...
2. В поле поиска в качестве фильтра введите "Центры событий". В списке триггеров выберите нужный триггер. 

   В этом примере используется триггер  
   **Event Hubs — When events are available in Event Hub** (Центры событий — при наличии событий в концентраторе событий).

   ![Выбор триггера](./media/connectors-create-api-azure-event-hubs/find-event-hubs-trigger.png)

3. Если вам будет предложено ввести сведения о подключении, [создайте подключение к Центрам событий](#create-connection). Или, если подключение уже существует, введите необходимые сведения для триггера.
...
## <a name="add-an-event-hubs-action"></a>Добавление действия Центров событий
...
3. В поле поиска в качестве фильтра введите "Центры событий".
В списке действий выберите необходимое действие. 

   Для этого примера выберите это действие: **Центры событий – отправка события**

   ![Выбор Event Hubs — Send event (Центры событий — отправка события)](./media/connectors-create-api-azure-event-hubs/find-event-hubs-action.png)

4. Если вам будет предложено ввести сведения о подключении, [создайте подключение к Центрам событий](#create-connection). Или, если подключение уже существует, введите необходимые сведения для действия.
...
1. Когда вам будет предложено ввести информацию о соединении, укажите следующие сведения.
   ...
   | Пространство имен Центров событий | Yes | <*пространство_имен_концентраторов_событий*> | Выберите пространство имен Центров событий, которые необходимо использовать. | 
   |||||  
