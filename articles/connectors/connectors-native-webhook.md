...
1. Чтобы добавить другие доступные параметры, откройте список **Добавить новый параметр** и выберите нужные параметры.
   ...
   Дополнительные сведения о типах проверки подлинности, доступных для веб-перехватчика HTTP, см. [в разделе Добавление проверки подлинности исходящих вызовов](../logic-apps/logic-apps-securing-a-logic-app.md#add-authentication-outbound).
...
1. Теперь, когда целевая служба отправляет запрос `HTTP POST` URL-адресу обратного вызова, приложение логики срабатывает и включает все данные, передаваемые по запросу.
