...
> [!NOTE]
> Logic Apps обеспечивает безопасность транспортного уровня (TLS) 1,2 при получении вызова к триггеру или действию веб-перехватчика HTTP. Если вы видите ошибки подтверждения TLS, убедитесь, что используется TLS 1,2. Для входящих вызовов ниже приведены поддерживаемые комплекты шифров:
>
...
1. Укажите значения [параметров триггера веб-перехватчика HTTP](../logic-apps/logic-apps-workflow-actions-triggers.md#http-webhook-trigger) , которые необходимо использовать для вызовов подписки и отмены подписки.
   ...
   | **Подписка — текст** | Нет | Любой текст сообщения, включаемый в запрос подписки. Этот пример включает URL-адрес обратного вызова, однозначно определяющий подписчика, который является приложением логики `@listCallbackUrl()` , с помощью выражения для получения URL-адреса обратного вызова приложения логики. |
