...
* По умолчанию действия хранилища BLOB-объектов можно считывать или записывать файлы, которые являются *50 МБ или меньше*. Для обработки файлов, размер которых превышает 50 МБ до 1024 МБ, хранилище BLOB-объектов действия поддерживают [фрагментацию сообщения](../logic-apps/logic-apps-handle-large-messages.md). **Содержимое большого двоичного объекта Get** действие неявно использует фрагментации.

* Триггеры хранилище BLOB-объектов Azure не поддерживает фрагментацию. При запросе содержимого файла, триггеры выберите только те файлы, которые являются 50 МБ или меньше. Чтобы получить файлы размером более 50 МБ, следуйте приведенному ниже шаблону:

  * Используйте триггер хранилища BLOB-объектов Azure, который возвращает свойства файла, такие как **при добавлении или изменении (только свойства) в большой двоичный объект**.

  * Триггер хранилища BLOB-объектов Azure, выполните **содержимое большого двоичного объекта Get** действие, которое считывает полный файл и неявно использует фрагментации.
...
В этом примере показано, как можно запустить рабочий процесс приложения логики с **при добавлении или изменении (только свойства) в большой двоичный объект** срабатывает, когда свойства большого двоичного объекта получает добавлены или обновлены в контейнере хранилища.

1. В [портала Azure](https://portal.azure.com) или Visual Studio, создайте пустое приложение логики, которая открывает конструктор приложений логики. В этом примере используется портал Azure.
...
2. В поле поиска введите "большой двоичный объект azure" в качестве условия фильтра. В списке триггеров выберите нужный триггер.
   ...
   В этом примере использует этот триггер: **При добавлении или изменении (только свойства) в большой двоичный объект**
...
2. В конструкторе приложений логики, см. в разделе триггера или действия, выберите **новый шаг**.

   ![Добавление действия](./media/connectors-create-api-azureblobstorage/add-action.png) 

   Чтобы добавить действие между существующими шагами, наведите указатель мыши на соединяющую стрелку. Нажмите знак плюса ( **+** ), находящуюся и выберите **добавить действие**.
...
3. В поле поиска введите "большой двоичный объект azure" в качестве условия фильтра. В списке действий выберите любое необходимое действие.
   ...
   В этом примере это действие: **Получить содержимое BLOB-объектов**
