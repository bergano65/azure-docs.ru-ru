...
* Ваши адреса сервера SFTP и учетные данные учетной записи, которые позволяют приложению логики получать доступ к учетной записи SFTP. Чтобы использовать протокол [Secure Shell (SSH)](https://www.ssh.com/ssh/protocol/), вам также необходим доступ к закрытому ключу SSH и его пароль.
  ...
  > Если вы используете закрытый ключ SSH, убедитесь _*_, что вы_*_ * **скопировали** ключ _ ключа из файла закрытого ключа SSH и вставили этот ключ в сведения о подключении, _*_не вводите и не изменяйте ключ вручную_*_ , что может привести к сбою подключения. 
...
1. Укажите необходимые сведения для подключения.
   ...
   1. В триггере или действии SFTP, которое вы добавили, вставьте *полный* ключ, скопированный в свойство **закрытого ключа SSH** , которое поддерживает несколько строк. **_Обязательно вставьте_* _ ключ. _*_Не вводите или не изменяйте ключ вручную_*_ .
...
Дополнительные технические сведения о триггерах, действиях и ограничениях, которые приведены в описании OpenAPI соединителя (прежнее название — Swagger), можно найти на [странице справки](../data-factory/connector-sftp.md) соединителя.
