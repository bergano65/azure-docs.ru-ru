...
* По умолчанию действия SFTP-SSH могут считывать или записывать файлы, которые имеют *Размер 1 ГБ или меньше* , но только за один раз блоки размером *15 МБ* .

  Для работы с файлами, превышающими 15 МБ, действия SFTP-SSH могут использовать [фрагменты сообщений](../logic-apps/logic-apps-handle-large-messages.md). Однако действие копирование файла поддерживает только 15 МБ файлов, так как это действие не поддерживает фрагментацию сообщений. SFTP-триггеры SSH не поддерживают фрагментацию. Для отправки больших файлов необходимы разрешения на чтение и запись для корневой папки на сервере SFTP.
