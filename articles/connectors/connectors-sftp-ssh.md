...
* Действия SFTP-SSH можно считывать или записывать файлы, которые являются *1 ГБ или меньше* путем управления данными как *15 МБ частей*, не 1 ГБ единиц.
...
* Действия можно считывать или записывать файлы *до 1 ГБ* по сравнению с соединителя SFTP, но обрабатывает данные по частям 15 МБ, не 1 ГБ частей. Для файлов размером более 1 ГБ в действиях может использоваться [фрагментация сообщений](../logic-apps/logic-apps-handle-large-messages.md). Триггеры SFTP-SSH в настоящее время не поддерживают фрагментации.
...
При запросе содержимого файла, триггеры не получить файлы размером более 15 МБ. Чтобы извлечь файлы, размер которых превышает 15 МБ, выполните этот шаблон. 
