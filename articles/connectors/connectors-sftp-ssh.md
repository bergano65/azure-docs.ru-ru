...
По сравнению с [соединителем SFTP](../connectors/connectors-create-api-sftp.md), соединитель SFTP-SSH может считывать или записывать файлы размером до *1 ГБ*. С более существенными различиями можно ознакомиться в разделе [сравнения SFTP-SSH и SFTP](#comparison) далее в этой статье.
...
* Считывает или записывает файлы размером до *1 ГБ* по сравнению с соединителем SFTP, но обрабатывает данные в виде фрагментов по 50 МБ, а не по 1 ГБ.
