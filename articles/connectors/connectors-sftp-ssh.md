...
* По умолчанию действия SFTP-SSH могут считывать или записывать файлы, которые имеют *Размер 1 ГБ или меньше* , но только за один раз блоки размером *15 МБ* . Для работы с файлами, превышающими 15 МБ, действия SFTP-SSH могут использовать [фрагменты сообщений](../logic-apps/logic-apps-handle-large-messages.md). Однако действие копирование файла поддерживает только 15 МБ файлов, так как это действие не поддерживает фрагментацию сообщений. SFTP-триггеры SSH не поддерживают фрагментацию.
...
* Ваши адреса сервера SFTP и учетные данные учетной записи, которые позволяют приложению логики получать доступ к учетной записи SFTP. Вам также необходим доступ к закрытому ключу SSH и паролю закрытого ключа SSH.
  ...
  >
  > * **Закрытые форматы ключей**. Ключи RSA (Ривест Шамир Адельман) и DSA (алгоритм цифровой подписи) в форматах OpenSSH и ssh.com. Если закрытый ключ находится в формате файла выводимого (. PPK), сначала [преобразуйте ключ в формат файла OpenSSH (PEM)](#convert-to-openssh).
  >
