...
В этом разделе описаны соображения для рассмотрения триггеров и действий этого разъема.

<a name="create-file"></a>

### <a name="create-file"></a>Создать файл

Для создания файла на сервере SFTP можно использовать **действие файла** SFTP-SSH Create. Когда это действие создает файл, служба Logic Apps также автоматически вызывает ваш сервер SFTP, чтобы получить метаданные файла. Однако, если вы переместите вновь созданный файл до того, `404` `'A reference was made to a file or folder which does not exist'`как служба Logic Apps сможет позвонить, чтобы получить метаданные, вы получите сообщение об ошибке. Чтобы пропустить чтение метаданных файла после создания файла, выполните последующие действия, чтобы [добавить и установить **свойство метаданных файла** в **No**](#file-does-not-exist).
...
### <a name="sftp---ssh-action-get-file-content-using-path"></a>SFTP - SSH действие: Получить содержимое файла с помощью пути

Это действие получает содержимое из файла на сервере SFTP, указав путь файла. Например, вы можете добавить триггер из предыдущего примера и условие, которому должно соответствовать содержимое файла. Если оно соответствует условию, действие, которое получает содержимое, может выполняться.

<a name="troubleshooting-errors"></a>

## <a name="troubleshoot-errors"></a>Устранение неполадок

В этом разделе описаны возможные решения распространенных ошибок или проблем.

<a name="file-does-not-exist"></a>

### <a name="404-error-a-reference-was-made-to-a-file-or-folder-which-does-not-exist"></a>Ошибка 404: "Была сделана ссылка на файл или папку, которая не существует"

Эта ошибка может произойти, когда приложение логики создает новый файл на вашем сервере SFTP через действие **файла** SFTP-SSH, но вновь созданный файл немедленно перемещается до того, как служба Логических Приложений может получить метаданные файла. Когда приложение логики запускает действие **файла Create,** служба Logic Apps также автоматически вызывает ваш сервер SFTP, чтобы получить метаданные файла. Однако, если файл перемещен, служба Logic Apps больше не может `404` найти файл, так что вы получите сообщение об ошибке.

Если вы не можете избежать или отсрочить перемещение файла, вы можете пропустить чтение метаданных файла после создания файла, а не следующие следующие шаги:

1. В действии **файла Create** откройте новый список **параметров,** выберите **свойство метаданных файлов** Иполучить все файлы и установите значение **для No.**

1. Если вам понадобятся метаданные этого файла позже, вы можете использовать действие **метаданных файлов Get.**
