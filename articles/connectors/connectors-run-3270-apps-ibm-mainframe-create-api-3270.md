...
* **Методы**: в этом режиме определяется метод, `GetBalance`например, который описывает путь навигации по экрану. Вы также выбираете поля на каждом экране, которые становятся входными и выходными параметрами метода.
