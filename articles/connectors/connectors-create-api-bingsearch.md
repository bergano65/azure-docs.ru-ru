---
title: Подключения к поиску Bing
description: Автоматизировать задачи и рабочие процессы, которые находят результаты в поиске Bing с помощью приложений логики Azure
services: logic-apps
...
---
# <a name="find-results-in-bing-search-by-using-azure-logic-apps"></a>Поиск результатов в поиске Bing с помощью приложений логики Azure
...
Если у вас еще нет подписки Azure, [зарегистрируйтесь для получения бесплатной учетной записи Azure](https://azure.microsoft.com/free/). Если вы не работали с приложениями логики, см. руководства по [Azure Logic Apps](../logic-apps/logic-apps-overview.md) и [созданию первого приложения логики](../logic-apps/quickstart-create-first-logic-app-workflow.md).
...
Или, если подключение уже существует, введите необходимые сведения для триггера.
   ...
   |----------|----------|-------|-------------|
   | Поисковый запрос | Да | <*поисковые слова*> | Введите ключевые слова для поиска, которые необходимо использовать. |
   | по рынку; | Да | <*Языкового стандарта*> | Языковой стандарт для поиска. Значение по умолчанию — "en-US", но можно выбрать другое значение. |
   | Безопасный поиск | Да | <*уровень поиска*> | Уровень фильтра для исключения материалов для взрослых. Значение по умолчанию — "Средний", но можно выбрать другой уровень. |
   | Count | нет | <*подсчет результатов*> | Возвращает указанное число результатов. Значение по умолчанию — 20, но можно указать другое значение. Фактическое число возвращаемых результатов может быть меньше указанного числа. |
   | Offset | нет | <*пропустить-значение*> | Количество пропускаемых результатов перед возвращением результатов |
   |||||
...
5. Когда вы закончите, на панели инструментов дизайнера, выберите **Сохранить**.
...
2. Под триггером или действием выберите **Новый шаг** > **Добавить действие.**

   Этот пример использует этот триггер:

   **Bing Поиск - О новых новостях статьи**
...
В списке действий выберите необходимое действие.
   ...
   В этом примере используется это действие:

   **Bing Поиск - Список новостей по запросу**
...
4. Если вам будет предложено ввести сведения о подключении, [создайте свое подключение к поиску Bing](#create-connection). Или, если подключение уже существует, введите необходимые сведения для действия.
   ...
   | Свойство | Обязательно | Значение | Описание |
   |----------|----------|-------|-------------|
   | Поисковый запрос | Да | <*поиск-выражение*> | Введите выражение для запроса результатов триггера. Его можно выбрать из полей в списке динамического содержимого или создать выражение с помощью построителя выражений. |
   | по рынку; | Да | <*Языкового стандарта*> | Языковой стандарт для поиска. Значение по умолчанию — "en-US", но можно выбрать другое значение. |
   | Безопасный поиск | Да | <*уровень поиска*> | Уровень фильтра для исключения материалов для взрослых. Значение по умолчанию — "Средний", но можно выбрать другой уровень. |
   | Count | нет | <*подсчет результатов*> | Возвращает указанное число результатов. Значение по умолчанию — 20, но можно указать другое значение. Фактическое число возвращаемых результатов может быть меньше указанного числа. |
   | Offset | нет | <*пропустить-значение*> | Количество пропускаемых результатов перед возвращением результатов |
   ...
   Из этого списка выберите **Выражение** таким образом, чтобы отображался строитель выражения. 
...
1. Когда вам будет предложено ввести информацию о соединении, укажите следующие сведения.
   ...
   | Версия API | Да | <*API-версия*> | По умолчанию устанавливается текущая версия API поиска Bing. При необходимости можно выбрать более раннюю версию. |
   | Ключ API | Да | <*API-ключ*> | Ключ API поиска Bing, полученный ранее. Если у вас нет ключа, получите [ключ API теперь](https://azure.microsoft.com/try/cognitive-services/?api=bing-news-search-api). |  
   |||||  
