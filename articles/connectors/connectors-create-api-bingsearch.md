...
Или, если подключение уже существует, введите необходимые сведения для триггера.
   ...
   |----------|----------|-------|-------------|
   | Search Query | Yes | <*search-words*> | Введите ключевые слова для поиска, которые необходимо использовать. |
   | по рынку; | Yes | <*locale*> | Языковой стандарт для поиска. Значение по умолчанию — "en-US", но можно выбрать другое значение. |
   | Safe Search | Yes | <*search-level*> | Уровень фильтра для исключения материалов для взрослых. Значение по умолчанию — "Средний", но можно выбрать другой уровень. |
   | Количество | Нет | <*results-count*> | Возвращает указанное число результатов. Значение по умолчанию — 20, но можно указать другое значение. Фактическое число возвращаемых результатов может быть меньше указанного числа. |
