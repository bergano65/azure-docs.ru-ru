...
1. В поле поиска введите фильтр "Dynamics 365". Например, выберите в списке триггеров выберите триггер: **When a record is created**
...
|------------|--------------------|-------------|
| Текстовые поля | Одна строка текста | В эти поля необходимо вводить одну строку текста или динамическое содержимое текстового типа. <p><p>*Пример поля*: **Описание** и **категории** | 
| Целочисленные поля | Целое число | В некоторые поля необходимо ввести целое число или добавить динамическое содержимое целочисленного типа. <p><p>*Пример поля*: **Процент завершения** и **длительность** | 
| Поля даты | Дата и время | В некоторые поля необходимо ввести дату в формате "мм/дд/гггг" или добавить динамическое содержимое типа "дата". <p><p>*Пример поля*: **Создан на**, **Дата начала**, **Фактическое начало**, **Фактическое окончание**, и **срок** | 
...
Технические сведения, такие как триггеры, действия и ограничения, как определено соединителя OpenAPI (прежнее название — Swagger) файл, см. в разделе [соединителя справочной странице](/connectors/dynamicscrmonline/).
