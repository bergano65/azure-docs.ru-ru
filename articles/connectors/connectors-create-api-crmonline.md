---
title: Подключение к динамике 365
...
Если вы новичок в логических приложениях, просмотрите [Что такое Azure Logic Apps?](../logic-apps/logic-apps-overview.md).
...
| **Тип владельца** | Должно быть `systemusers` `teams`либо или . |
| **В отношении** | Требуется допустимый идентификатор записи, например идентификатор учетной записи или записи контакта. |
| **Тип "в отношении"** | Должен быть тип поиска, `accounts` например, или `contacts`. |
| **Клиентов** | Требуется допустимый идентификатор записи, например идентификатор учетной записи или записи контакта. |
| **Тип клиента** | Должен быть тип поиска, `accounts` например, или `contacts`. |
|||
...
2. На панели инструментов выберите одно из следующих действий.
   ...
   * Выберите **Pop Out**. ![всплывающие записи](./media/connectors-create-api-crmonline/popout-record.png) 
...
Для технических деталей, таких как триггеры, действия и ограничения, описанные в файле OpenAPI разъема (ранее Swagger), см. [connector's reference page](/connectors/dynamicscrmonline/)
