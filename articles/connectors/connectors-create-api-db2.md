...
1. В поле поиска введите слово db2 в качестве фильтра. Например, выберите в списке действий выберите действие: **Получение таблиц (Предварительная версия)**
...
| **сервер;** | Yes | Адрес или псевдоним, двоеточие и номер порта для сервера DB2, например myDB2server.cloudapp.net:50000 <p><p>**Примечание**. Это значение является строкой, которая представляет адрес TCP/IP или псевдоним, либо в формате IPv4 или IPv6, а затем двоеточие и номер порта TCP/IP. |
| **База данных** | Yes | Имя базы данных <p><p>**Примечание**. Это значение является строка, представляющая реляционной базы данных имя DRDA (RDBNAM): <p>– DB2 для z/OS принимает 16-байтовую строку, если база данных известна как расположение "IBM DB2 для z/OS". <br>– DB2 для i принимает 18-байтовую строку, если база данных известна как реляционная база данных "IBM DB2 для i". <br>– DB2 для LUW принимает в качестве значения 8-байтовую строку. |
| **Имя пользователя** | Yes | Имя пользователя для базы данных <p><p>**Примечание**. Это значение является строкой, длина которого зависит от конкретной базы данных: <p><p>– DB2 для z/OS принимает 8-байтовую строку. <br>– DB2 для i принимает 10-байтовую строку. <br>– DB2 для Linux или UNIX принимает 8-байтовую строку. <br>– DB2 для Windows принимает 30-байтовую строку. |
...
| Свойство | Обязательно для заполнения | ОПИСАНИЕ |
|----------|----------|-------------|
| **Подключаться с помощью локального шлюза данных** | Yes | Применяется, если необходимо локальное подключение. Показывает свойства локального подключения. |
| **Имя подключения** | Yes | Имя подключения, например MyLogicApp-DB2-connection | 
| **сервер;** | Yes | Адрес или псевдоним, двоеточие и номер порта для сервера DB2, например myDB2server:50000 <p><p>**Примечание**. Это значение является строкой, которая представляет адрес TCP/IP или псевдоним, либо в формате IPv4 или IPv6, а затем двоеточие и номер порта TCP/IP. |
| **База данных** | Yes | Имя базы данных <p><p>**Примечание**. Это значение является строка, представляющая реляционной базы данных имя DRDA (RDBNAM): <p>– DB2 для z/OS принимает 16-байтовую строку, если база данных известна как расположение "IBM DB2 для z/OS". <br>– DB2 для i принимает 18-байтовую строку, если база данных известна как реляционная база данных "IBM DB2 для i". <br>– DB2 для LUW принимает в качестве значения 8-байтовую строку. |
| **Authentication** | Yes | Тип проверки подлинности для подключения, например "Обычная" <p><p>**Примечание**. Выберите это значение из списка, которая включает в себя Basic или Windows (Kerberos). |
| **Имя пользователя** | Yes | Имя пользователя для базы данных <p><p>**Примечание**. Это значение является строкой, длина которого зависит от конкретной базы данных: <p><p>– DB2 для z/OS принимает 8-байтовую строку. <br>– DB2 для i принимает 10-байтовую строку. <br>– DB2 для Linux или UNIX принимает 8-байтовую строку. <br>– DB2 для Windows принимает 30-байтовую строку. |
| **Пароль** | Yes | Пароль для базы данных |
| **Шлюз** | Yes | Имя установленного локального шлюза данных <p><p>**Примечание**. Выберите это значение из списка, который включает все шлюзы данных, установленных в подписку Azure и группы ресурсов. |
||||
...
1. Укажите значения для всех обязательных свойств (*). После выбора таблицы это действие показывает соответствующие свойства записей из этой таблицы.
   ...
   | **КОД региона** | Yes | Идентификатор добавляемого региона, например 102 |
   |||| 
...
1. Укажите значения для всех обязательных свойств (*). После выбора таблицы это действие показывает соответствующие свойства записей из этой таблицы.
   ...
   | **КОД региона** | Yes | Новый идентификатор региона, например 102 |
   ||||
...
Технические сведения, такие как триггеры, действия и ограничения, как определено соединителя OpenAPI (прежнее название — Swagger) файл, см. в разделе [соединителя справочной странице](/connectors/db2/).
