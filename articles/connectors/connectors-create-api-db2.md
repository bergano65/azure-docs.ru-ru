---
title: Доступ и управление ресурсами IBM DB2
description: Читать, редотворно, обновлять и управлять ресурсами IBM DB2 путем создания автоматизированных рабочих процессов с помощью приложений Azure Logic Apps
services: logic-apps
...
---
# <a name="access-and-manage-ibm-db2-resources-by-using-azure-logic-apps"></a>Доступ и управление ресурсами IBM DB2 с помощью приложений логики Azure

С [помощью приложений Azure Logic Apps](../logic-apps/logic-apps-overview.md) и [разъема IBM DB2](/connectors/db2/)можно создавать автоматизированные задачи и рабочие процессы на основе ресурсов, хранящихся в базе данных DB2. Рабочие процессы могут подключаться к ресурсам в базе данных, считывать и выводить таблицы баз данных, добавлять, менять и удалять строки и т. д. В приложения логики можно включать действия, которые получают ответы от базы данных и делают выходные данные доступными для других действий.

В этой статье показано, как создать приложение логики, выполняющее различные операции с базами данных. Если вы новичок в логических приложениях, просмотрите [Что такое приложения логики Azure?](../logic-apps/logic-apps-overview.md)

## <a name="supported-platforms-and-versions"></a>Поддерживаемые платформы и версии

Соединитель DB2 содержит клиент Майкрософт, который взаимодействует с удаленными серверами DB2 по сети TCP/IP. Этот разъем можно использовать для доступа к облачным базам данных, таким как IBM DB2 для Windows, работающих в виртуализации Azure. Вы также можете получать доступ к локальным базам данных DB2 после [установки и настройки локального шлюза данных](../logic-apps/logic-apps-gateway-connection.md).

Разъем IBM DB2 поддерживает эти платформы и версии IBM DB2 наряду с совместимыми продуктами IBM DB2, которые поддерживают распределенную архитектуру реляционной базы данных (DRDA) S'L Access Manager (S'LAM) версии 10 и 11:
...
| **Базы данных** | Да | Имя базы данных <p><p>**Примечание**. Это строка, представляющая имя реляционной базы данных DRDA (RDBNAM): <p>– DB2 для z/OS принимает 16-байтовую строку, если база данных известна как расположение "IBM DB2 для z/OS". <br>– DB2 для i принимает 18-байтовую строку, если база данных известна как реляционная база данных "IBM DB2 для i". <br>– DB2 для LUW принимает в качестве значения 8-байтовую строку. |
| **Проверка подлинности** | Да | Тип проверки подлинности для подключения, например "Обычная" <p><p>**Примечание**. Выберите это значение из списка, включающего варианты "Обычная" и "Windows (Kerberos)". |
| **Пользователя** | Да | Имя пользователя для базы данных <p><p>**Примечание**. Это строка, длина которой зависит от конкретной базы данных: <p><p>– DB2 для z/OS принимает 8-байтовую строку. <br>– DB2 для i принимает 10-байтовую строку. <br>– DB2 для Linux или UNIX принимает 8-байтовую строку. <br>– DB2 для Windows принимает 30-байтовую строку. |
| **Пароль** | Да | Пароль для базы данных |
| **Шлюза** | Да | Имя установленного локального шлюза данных <p><p>**Примечание**. Выберите это значение из списка, включающего все установленные шлюзы данных в вашей подписке Azure и группе ресурсов. |
||||
...
Для получения более подробной информации об этом разъеме, таких как триггеры, действия и ограничения, описанные в файле Swagger разъема, смотрите [страницу ссылки разъема.](https://docs.microsoft.com/connectors/db2/)
