...
| Имя | Описание |
|------|-------------|
| [![][schedule-icon]<br>**Schedule** Расписание встроенного соединителя][schedule-doc] | — Запуск приложения логики в указанное повторение, начиная от базового до расширенных расписаний с [триггером **повторения**][schedule-recurrence-doc]. <br>— Запустите приложение логики, которое должно выполнять обработку данных в непрерывных блоках с помощью [триггера **скользящего окна**][schedule-sliding-window-doc]. <br>— Приостановка приложения логики в течение указанного времени с помощью [действия **delay**][schedule-delay-doc]. <br>— Приостановка приложения логики до указанной даты и времени с [ **задержкой до** действия][schedule-delay-until-doc]. |
| [![Пакетная встроенная соединительная линия ][batch-icon]<br> **Batch**][batch-doc] | — Обрабатывайте сообщения в пакетах с помощью триггера **Пакетные сообщения**. <br>— Вызывайте приложения логики, имеющие пакеты триггеров, с помощью действия **Send messages to batch** (Отправка сообщений в пакет). |
| [![HTTP-компонент встроенного соединителя HTTP ][http-icon]<br> **HTTP**][http-doc] | Вызов конечных точек HTTP или HTTPS с триггерами и действиями для HTTP. Другие встроенные триггеры HTTP и действия включают [Встроенный соединитель HTTP + Swagger][http-swagger-doc] и [HTTP + веб-перехватчик][http-webhook-doc]. |
| [![][http-request-icon]<br>**Request** Запрос встроенного соединителя запроса][http-request-doc] | — Сделайте свое приложение логики доступным для вызовов от других приложений или служб, активируйте события ресурса Event Grid или ответ на предупреждения Azure Security Center с помощью триггера **Запрос**. <br>— Отправляйте ответы в приложение или службу с помощью действия **Ответ**. |
...
| [![Встроенный ][inline-code-icon]<br> **Inline code** код встроенного соединителя встраиваемого кода][inline-code-doc] | Добавление и запуск фрагментов кода JavaScript из приложений логики. |
|||
...
| Имя | Описание |
|------|-------------|
| [![][condition-icon]<br>**Condition** Условие встроенного действия][condition-doc] | Оценка условия и выполнение различных действий на основе значения условия: True или False. |
...
| [![][scope-icon]<br>**Scope** Область встроенных действий][scope-doc] | Группировка действий в *области* , которые получают свое состояние после завершения действий в области. |
...
| [![Сетка событий ][azure-event-grid-icon]<br> **Azure** <br> **Grid** для управляемого соединителя службы "Сетка событий Azure"][azure-event-grid-doc] | Мониторинг событий, публикуемых службой "Сетка событий Azure", например при изменении ресурсов Azure или сторонних ресурсов. |
| [![SalesForce управляемого соединителя Salesforce ][salesforce-icon]<br> **Salesforce**][salesforce-doc] | Подключитесь к учетной записи Salesforce, чтобы вы могли создавать такие элементы, как записи, задания, объекты и многое другое, и управлять ими. |
|||
...
:::row:::
    ...
    :::column-end:::
    :::column:::
        [![Преобразования жидкостей ][liquid-icon]<br> **Liquid** преобразование ликвидности действий <br> **transforms**][json-liquid-transform-doc]
    :::column-end:::
:::row-end:::
:::row:::
    :::column:::
...
:::row:::
    ...
    :::column-end:::
    :::column:::
        [![FTP-соединитель FTP Connector ][ftp-icon]<br> **FTP**][ftp-doc]
    :::column-end:::
:::row-end:::
:::row:::
    :::column:::
