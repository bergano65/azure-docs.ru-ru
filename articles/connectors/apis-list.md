...
* [Модель ценообразования Logic Apps](../logic-apps/logic-apps-pricing.md)
...
|   |   |   |   |
|---|---|---|---|
| [значок API ![][schedule-icon]<br>**Расписание**][schedule-doc] | — Запуск приложения логики в указанное повторение, начиная от базового до расширенных расписаний с [триггером **повторения** ][schedule-recurrence-doc]. <p>— Запустите приложение логики, которое должно выполнять обработку данных в непрерывных блоках с помощью [триггера **скользящего окна** ][schedule-sliding-window-doc]. <p>— Приостановка приложения логики в течение указанного времени с помощью [действия **delay** ][schedule-delay-doc]. <p>— Приостановка приложения логики до указанной даты и времени с [ **задержкой до** действия][schedule-delay-until-doc]. | [значок API ![][batch-icon]<br>**пакетной** службы][batch-doc] | — Обрабатывайте сообщения в пакетах с помощью триггера **Пакетные сообщения**. <p>— Вызывайте приложения логики, имеющие пакеты триггеров, с помощью действия **Send messages to batch** (Отправка сообщений в пакет). |
| [значок API ![][http-icon]<br>**http**][http-doc] | Вызывайте конечные точки HTTP или HTTPS с триггерами и действиями для HTTP. Другие встроенные триггеры и действия HTTP включают [http + Swagger][http-swagger-doc] и [HTTP + веб-перехватчик][http-webhook-doc]. | [значок API ![][http-request-icon]<br>**запроса**][http-request-doc] | — Сделайте свое приложение логики доступным для вызовов от других приложений или служб, активируйте события ресурса Event Grid или ответ на предупреждения Azure Security Center с помощью триггера **Запрос**. <p>— Отправляйте ответы в приложение или службу с помощью действия **Ответ**. |
| [значок API ![][azure-api-management-icon]<br>**управления <br>API Azure**][azure-api-management-doc] | Вызывайте триггеры и действия, определенные API-интерфейсами, которые вы публикуете и контролируете с помощью управления API Azure. | [значок API ![][azure-app-services-icon]<br>**службы <br>приложений Azure**][azure-app-services-doc] | Вызывайте приложения API Azure или веб-приложения, размещенные в Службе приложений Azure. Если включен Swagger, триггеры и действия, определенные этими приложениями, отображаются как любые другие триггеры и действия первого класса.|
| [значок API ![][azure-logic-apps-icon]<br>**приложения логики <br>Azure**][nested-logic-app-doc] | Вызов других приложений логики, начинающихся с триггера **запроса** . |
|||||
...
|   |   |   |   |
|---|---|---|---|
| [значок API ![][azure-functions-icon]<br>**функции Azure**][azure-functions-doc] | Вызывайте функции Azure, которые запускают настраиваемые фрагменты кода (C# или Node.js) из ваших приложений логики. | [значок API ![][inline-code-icon]<br>**встроенный код**][azure-functions-doc] | Добавление и запуск фрагментов кода JavaScript из приложений логики. |
|||||
...
| [значок API ![][azure-blob-storage-icon]<br>**хранилище<br>больших двоичных объектов Azure**][azure-blob-storage-doc] | Подключитесь к учетной записи хранения, чтобы можно было создавать и управлять содержимым больших двоичных объектов. | [значок API ![][office-365-outlook-icon]<br>**Office 365<br>Outlook**][office-365-outlook-doc] | Подключитесь к учетной записи электронной почты Office 365, чтобы вы могли создавать и администрировать сообщения электронной почты, задачи, события календаря и собрания, контакты, запросы и многое другое. |
...
| [значок API ![][dynamics-365-icon]<br>**Dynamics 365<br>** ][dynamics-365-doc] | Подключитесь к учетной записи Dynamics 365, чтобы можно было создавать и администрировать записи, элементы и многое другое. | [значок API ![][azure-queues-icon]<br> **<br>очередей Azure**][azure-queues-doc] | Подключитесь к учетной записи хранения Azure, чтобы можно было создавать очереди и сообщения и управлять ими. |
| [значок API ![][ftp-icon]<br>**FTP**][ftp-doc] | Подключитесь к FTP-серверам, к которым можно получить доступ из Интернета, чтобы можно было работать с файлами и папками. | [значок API ![][file-system-icon]<br>**файл <br>система**][file-system-doc] | Подключитесь к локальной общей папке, чтобы можно было создавать файлы и управлять ими. |
...
|   |   |   |   |   |
|---|---|---|---|---|
| [значок API ![][biztalk-server-icon]<br>**BizTalk** <br>**Server**][biztalk-server-doc] | [значок API ![][file-system-icon]<br>**файл <br>система**][file-system-doc] | [значок API ![][ibm-db2-icon]<br>**IBM DB2**][ibm-db2-doc] | [значок API ![][ibm-informix-icon]<br>**IBM** <br>**Informix**][ibm-informix-doc] | [значок API ![][mysql-icon]<br>**MySQL**][mysql-doc] |
...
|   |   |   |   |   |
|---|---|---|---|---|
[значок API ![][as2-icon]<br>**AS2**][as2-doc] | [значок API ![][azure-blob-storage-icon]<br>**хранилище<br>больших двоичных объектов Azure**][azure-blob-storage-doc] | [значок API ![][azure-cosmos-db-icon]<br>**Azure Cosmos <br> DB**][azure-cosmos-db-doc] | [значок API ![][azure-event-hubs-icon]<br>**концентраторов <br>событий Azure**][azure-event-hubs-doc] | [значок API ![][azure-file-storage-icon]<br>**хранилище<br>файлов Azure**][azure-file-storage-doc] |
| [значок API ![][azure-service-bus-icon]<br>**службы Azure <br>ная шина**][azure-service-bus-doc] | [значок API ![][azure-sql-data-warehouse-icon]<br>**хранилище <br>данных SQL Azure**][azure-sql-data-warehouse-doc] | [значок API ![][azure-table-storage-icon]<br>**таблицы Azure <br>хранилище**][azure-table-storage-doc] | [значок API ![][azure-queues-icon]<br> **<br>очередей Azure**][azure-queues-doc] | [значок API ![][edifact-icon]<br>**EDIFACT**][edifact-doc] |
| [значок API ![][file-system-icon]<br>**файл <br>система**][file-system-doc] | [значок API ![][ftp-icon]<br>**FTP**][ftp-doc] | [значок API ![][ibm-3270-icon]<br>**IBM 3270**][ibm-3270-doc] | [значок API ![][ibm-db2-icon]<br>**IBM DB2**][ibm-db2-doc] | [значок API ![][ibm-mq-icon]<br>**IBM MQ**][ibm-mq-doc] |
| [значок API ![][sap-icon]<br>**SAP**][sap-connector-doc] | [значок API ![][sftp-ssh-icon]<br>**SFTP-SSH**][sftp-ssh-doc] | [значок API ![][smtp-icon]<br>**SMTP**][smtp-doc] | [значок API ![][sql-server-icon]<br>**SQL <br>Server**][sql-server-doc] | [значок API ![][x12-icon]<br>**X12**][x12-doc] |
||||||
