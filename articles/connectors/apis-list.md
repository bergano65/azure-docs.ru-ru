...
|-------|---------|-------------|
| **Центральный** | ![Пример соединителя ISE](./media/apis-list/example-core-connector.png) | Встроенные триггеры и действия с этой меткой запускаются в той же интегрированной среде сценариев, что и приложения логики. |
...
| Имя | Описание: |
|------|-------------|
| [![Расписание значков API ][schedule-icon]<br> **Schedule**][schedule-doc] | — Запуск приложения логики в указанное повторение, начиная от базового до расширенных расписаний с [триггером **повторения** ][schedule-recurrence-doc]. <br>— Запустите приложение логики, которое должно выполнять обработку данных в непрерывных блоках с помощью [триггера **скользящего окна** ][schedule-sliding-window-doc]. <br>— Приостановка приложения логики в течение указанного времени с помощью [действия **delay** ][schedule-delay-doc]. <br>— Приостановка приложения логики до указанной даты и времени с [ **задержкой до** действия][schedule-delay-until-doc]. |
| [![Пакет значков API ][batch-icon]<br> **Batch**][batch-doc] | — Обрабатывайте сообщения в пакетах с помощью триггера **Пакетные сообщения**. <br>— Вызывайте приложения логики, имеющие пакеты триггеров, с помощью действия **Send messages to batch** (Отправка сообщений в пакет). |
| [![Значок API ][http-icon]<br> **http**][http-doc] | Вызов конечных точек HTTP или HTTPS с триггерами и действиями для HTTP. Другие встроенные триггеры и действия HTTP включают [http + Swagger][http-swagger-doc] и [HTTP + веб-перехватчик][http-webhook-doc]. |
| [![Запрос значка API ][http-request-icon]<br> **Request**][http-request-doc] | — Сделайте свое приложение логики доступным для вызовов от других приложений или служб, активируйте события ресурса Event Grid или ответ на предупреждения Azure Security Center с помощью триггера **Запрос**. <br>— Отправляйте ответы в приложение или службу с помощью действия **Ответ**. |
| [![Значок API ][azure-api-management-icon]<br> ** <br> Управление API Azure**][azure-api-management-doc] | Вызывайте триггеры и действия, определенные API-интерфейсами, которые вы публикуете и контролируете с помощью управления API Azure. |
| [![Значок API ][azure-app-services-icon]<br> ** <br> службы приложений Azure**][azure-app-services-doc] | Вызывайте приложения API Azure или веб-приложения, размещенные в Службе приложений Azure. Если включен Swagger, триггеры и действия, определенные этими приложениями, отображаются как любые другие триггеры и действия первого класса. |
| [![Значок API ][azure-logic-apps-icon]<br> **Azure Logic <br> Apps**][nested-logic-app-doc] | Вызов других приложений логики, начинающихся с триггера **запроса** . |
|||
...
| Имя | Описание: |
|------|-------------|
| [![Значок API ][azure-functions-icon]<br> **функции Azure**][azure-functions-doc] | Вызывайте функции Azure, которые запускают настраиваемые фрагменты кода (C# или Node.js) из ваших приложений логики. |
| [![][inline-code-icon]<br>**Встроенный код** значка API][inline-code-doc] | Добавление и запуск фрагментов кода JavaScript из приложений логики. |
|||
...
| Имя | Описание: |
|------|-------------|
| [![Условие встроенного значка ][condition-icon]<br> **Condition**][condition-doc] | Оценка условия и выполнение различных действий на основе значения условия: True или False. |
| [![Встроенный значок ][for-each-icon]<br> **для каждого**][for-each-doc] | Выполнение одинаковых действий для каждого элемента в массиве. |
| [![Область встроенных значков ][scope-icon]<br> **Scope**][scope-doc] | Группировка действий в *области*, которые получают свое состояние после завершения действий в области. |
| [![Переключатель встроенного значка ][switch-icon]<br> **Switch**][switch-doc] | Группируйте действия в *варианты*, которым присваиваются уникальные значения, за исключением варианта по умолчанию. Запускайте только тот вариант, присвоенное значение которого соответствует результату выражения, объекта или токена. Если совпадений не существует, запустите вариант по умолчанию. |
| [![Завершение встроенного значка ][terminate-icon]<br> **Terminate**][terminate-doc] | Остановка выполнения активного рабочего процесса приложения логики. |
| [![Встроенный значок ][until-icon]<br> **до**][until-doc] | Повторяйте действия до тех пор, пока значение указанного условия не будет равно True или какое-нибудь из состояний не изменится. |
|||
...
| Имя | Описание: |
|------|-------------|
| [![Значок API ][azure-service-bus-icon]<br> **служебная шина Azure**][azure-service-bus-doc] | Управление асинхронными сообщениями, сеансами и подписками на темы с помощью наиболее часто используемого соединителя в системе Logic Apps. |
| [![Значок API ][sql-server-icon]<br> **SQL Server**][sql-server-doc] | Подключитесь SQL Server к локальной службе или базе данных SQL Azure в облаке, чтобы можно было управлять записями, выполнять хранимые процедуры или выполнять запросы. |
| [![Значок API ][azure-blob-storage-icon]<br> ** <br> хранилище BLOB-объектов Azure**][azure-blob-storage-doc] | Подключитесь к учетной записи хранения, чтобы можно было создавать и управлять содержимым больших двоичных объектов. |
| [![Значок API ][office-365-outlook-icon]<br> **Office 365 <br> Outlook**][office-365-outlook-doc] | Подключитесь к учетной записи электронной почты Office 365, чтобы вы могли создавать и администрировать сообщения электронной почты, задачи, события календаря и собрания, контакты, запросы и многое другое. |
| [![Значок API ][sftp-ssh-icon]<br> **SFTP-SSH**][sftp-ssh-doc] | Подключитесь к SFTP-серверам, к которым можно получить доступ из Интернета с помощью SSH, чтобы можно было работать с файлами и папками. |
| [![Значок API ][sharepoint-online-icon]<br> **SharePoint <br> Online**][sharepoint-online-doc] | Подключитесь к SharePoint Online, чтобы можно было управлять файлами, вложениями, папками и т. д. |
| [![Значок API ][azure-queues-icon]<br> ** <br> очереди Azure**][azure-queues-doc] | Подключитесь к учетной записи хранения Azure, чтобы можно было создавать очереди и сообщения и управлять ими. |
| [![Значок API ][ftp-icon]<br> **FTP**][ftp-doc] | Подключитесь к FTP-серверам, к которым можно получить доступ из Интернета, чтобы можно было работать с файлами и папками. |
| [![Значок API ][file-system-icon]<br> **Файловая <br> система**][file-system-doc] | Подключитесь к локальной общей папке, чтобы можно было создавать файлы и управлять ими. |
| [![Значок API ][azure-event-hubs-icon]<br> **концентраторы событий Azure**][azure-event-hubs-doc] | Использование событий и их публикация в концентраторе событий. Например, получите выходные данные из приложения логики с помощью соединителя Центров событий, а затем отправьте эти сведения любому поставщику аналитики в реальном времени. |
| [![Значок API служба " ][azure-event-grid-icon]<br> **Сетка событий Azure** <br> **Grid** "][azure-event-grid-doc] | Мониторинг событий, публикуемых службой "Сетка событий Azure", например при изменении ресурсов Azure или сторонних ресурсов. |
| [![Значок API ][salesforce-icon]<br> **Salesforce**][salesforce-doc] | Подключитесь к учетной записи Salesforce, чтобы вы могли создавать такие элементы, как записи, задания, объекты и многое другое, и управлять ими. |
|||
...
|___|___|___|___|___|
|---|---|---|---|---|
| [![Значок API ][biztalk-server-icon]<br> **BizTalk** <br> **Server**][biztalk-server-doc] | [![Значок API ][file-system-icon]<br> **Файловая <br> система**][file-system-doc] | [![Значок API ][ibm-db2-icon]<br> **IBM DB2**][ibm-db2-doc] | [![Значок API ][ibm-informix-icon]<br> **IBM** <br> **Informix**][ibm-informix-doc] | [![Значок API ][mysql-icon]<br> **MySQL**][mysql-doc] |
| [![Значок API ][oracle-db-icon]<br> **Oracle DB**][oracle-db-doc] | [![Значок API ][postgre-sql-icon]<br> **PostgreSQL**][postgre-sql-doc] | [![Значок API ][sharepoint-server-icon]<br> **SharePoint <br> Server**][sharepoint-server-doc] | [![Значок API ][sql-server-icon]<br> **SQL <br> Server**][sql-server-doc] | [![Значок API ][teradata-icon]<br> **Teradata**][teradata-doc] |
||||||
...
|___|___|___|___|
|---|---|---|---|
| [![Значок API ][as2-icon]<br> ** <br> декодирование AS2**][as2-doc] | [![Значок API ][as2-icon]<br> ** <br> кодирование AS2**][as2-doc] | [![Значок API ][edifact-icon]<br> ** <br> декодирование EDIFACT**][edifact-decode-doc] | [![Значок API ][edifact-icon]<br> **EDIFACT <br> Encoding**][edifact-encode-doc] |
| [![Значок API ][flat-file-decode-icon]<br> ** <br> декодирование неструктурированного файла**][flat-file-decode-doc] | [![Значок API ][flat-file-encode-icon]<br> ** <br> Кодировка неструктурированного файла**][flat-file-encode-doc] | [![Значок API ][integration-account-icon]<br> ** <br> учетная запись интеграции**][integration-account-doc] | [![Значок API ][liquid-icon]<br> **преобразования жидкостей** <br> **transforms**][json-liquid-transform-doc] |
| [![Значок API ][x12-icon]<br> ** <br> декодирование X12**][x12-decode-doc] | [![Значок API ][x12-icon]<br> **X12 <br> Encoding**][x12-encode-doc] | [![Значок API ][xml-transform-icon]<br> **преобразования XML-кода** <br> **transforms**][xml-transform-doc] | [![Значок API ][xml-validate-icon]<br> ** <br> Проверка XML**][xml-validate-doc] |
|||||
...
|___|___|___|___|___|
|---|---|---|---|---|
| [![Значок API ][as2-icon]<br> **AS2**][as2-doc] | [![Значок API Служба ][azure-automation-icon]<br> ** <br> автоматизации Azure**][azure-automation-doc] | [![Значок API ][azure-blob-storage-icon]<br> ** <br> хранилище BLOB-объектов Azure**][azure-blob-storage-doc] | [![Значок API ][azure-cosmos-db-icon]<br> **Azure Cosmos <br> DB**][azure-cosmos-db-doc] | [![Значок API ][azure-event-hubs-icon]<br> ** <br> концентраторы событий Azure**][azure-event-hubs-doc] |
| [![Значок API служба " ][azure-event-grid-icon]<br> ** <br> Сетка событий Azure** "][azure-event-grid-doc] | [![Значок API ][azure-file-storage-icon]<br> ** <br> хранилище файлов Azure**][azure-file-storage-doc] | [![Значок API ][azure-key-vault-icon]<br> ** <br> хранилище ключей Azure**][azure-key-vault-doc] | [![Значок API ][azure-monitor-logs-icon]<br> **Azure Monitor <br> журналы**][azure-monitor-logs-doc] | [![Значок API ][azure-service-bus-icon]<br> **служебная <br> шина Azure**][azure-service-bus-doc] |
| [![Значок API ][azure-sql-data-warehouse-icon]<br> ** <br> хранилище данных SQL Azure**][azure-sql-data-warehouse-doc] | [![Значок API ][azure-table-storage-icon]<br> ** <br> хранилище таблиц Azure**][azure-table-storage-doc] | [![Значок API ][azure-queues-icon]<br> ** <br> очереди Azure**][azure-queues-doc] | [![Значок API ][edifact-icon]<br> **EDIFACT**][edifact-doc] | [![Значок API ][file-system-icon]<br> **Файловая <br> система**][file-system-doc] |
| [![Значок API ][ftp-icon]<br> **FTP**][ftp-doc] | [![Значок API ][ibm-3270-icon]<br> **IBM 3270**][ibm-3270-doc] | [![Значок API ][ibm-db2-icon]<br> **IBM DB2**][ibm-db2-doc] | [![Значок API ][ibm-mq-icon]<br> **IBM MQ**][ibm-mq-doc] | [![Значок API ][sap-icon]<br> **SAP**][sap-connector-doc] |
| [![Значок API ][sftp-ssh-icon]<br> **SFTP-SSH**][sftp-ssh-doc] | [![Значок API ][smtp-icon]<br> **SMTP**][smtp-doc] | [![Значок API ][sql-server-icon]<br> **SQL <br> Server**][sql-server-doc] | [![Значок API ][x12-icon]<br> **X12**][x12-doc] |
||||||
...
[biztalk-server-doc]: /connectors/biztalk/ "Подключитесь к BizTalk Server, чтобы можно было запускать приложения на основе BizTalk параллельно с Azure Logic Apps"
