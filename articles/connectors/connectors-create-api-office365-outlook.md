...
> [!NOTE]
> Чтобы автоматизировать задачи для @outlook.com учетной @hotmail.com записи или, используйте [соединитель Outlook.com](../connectors/connectors-create-api-outlook.md).
...
1. В триггере задайте значения **частоты** и **интервала** . Чтобы добавить другие доступные свойства триггера, такие как **Часовой пояс**, выберите эти свойства в списке **Добавить новый параметр** .
   ...
   Например, если требуется, чтобы триггер пропускал календарь каждые 15 минут, задайте для параметра **Frequency** значение **минуты**, а для **Interval** `15`параметра Интервал — значение. 
