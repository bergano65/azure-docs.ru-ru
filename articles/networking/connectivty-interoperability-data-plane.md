---
title: 'Совместимость в Azure : анализ плоскости данных'
...
---
# <a name="interoperability-in-azure--data-plane-analysis"></a>Совместимость в Azure : анализ плоскости данных
...
В этом маршруте трассировки в качестве первого прыжка указана конечная точка туннеля от шлюза ExpressRoute до MSEE. Второй переход — это ip-адрес шлюза удаленного VNet. Диапазон IP-адресов, в который входит адрес второго прыжка, не объявляется в центральной виртуальной сети. Третий прыжок — виртуальная машина в удаленной виртуальной сети.
...
В этом маршруте, первый переход концентратор VNet ExpressRoute шлюз туннель конечная точка mSEE. Второй переход — это ip-адрес шлюза удаленного VNet. Диапазон IP-адресов, в который входит адрес второго прыжка, не объявляется в центральной или периферийной виртуальной сети. Третий прыжок — виртуальная машина в удаленной виртуальной сети.
...
Как говорилось ранее, в тестовой конфигурации VPN-подключение типа "сеть — сеть" используется в качестве резервного, а основным служит подключение ExpressRoute между локальным расположением 1 и центральной виртуальной сетью. Чтобы проверить траекторию резервного копирования данных, давайте навесем сбой связи ExpressRoute между локальным основным маршрутизатором Location 1 и соответствующим маршрутизатором MSEE. Чтобы вызвать сбой соединения ExpressRoute, завершите работу интерфейса CE, подключенного к MSEE:
...
<!--Image References-->
[1]: ./media/backend-interoperability/HubVM-SpkVM.jpg "Просмотр подключения сети от концентратора VNet к спицу VNet"
[2]: ./media/backend-interoperability/HubVM-BranchVM.jpg "Представление сетевого наблюдения за подключением от концентратора VNet к ветке VNet"
[3]: ./media/backend-interoperability/HubVM-BranchVM-Grid.jpg "Представление сетки сетевого наблюдения от концентратора VNet к ветке VNet"
[4]: ./media/backend-interoperability/Loc1-HubVM.jpg "Представление сетевого монитора подключения от места 1 VM до концентратора VNet через ExpressRoute 1"
[5]: ./media/backend-interoperability/Loc1-HubVM-S2S.jpg "Представление возможностей сетевого мониторинга подключения от места 1 VM до концентратора VNet через VPN-сайт"
