...
Клиенты могут выбрать развертывание [Azure WAF с помощью шлюза приложений](../web-application-firewall/ag/ag-overview.md) , который обеспечивает региональную защиту сущностей в общедоступном и частном адресном пространстве. Клиенты также могут развернуть [WAF Azure с помощью передней дверцы](../web-application-firewall/afds/afds-overview.md) , которая обеспечивает защиту на границе сети до общедоступных конечных точек.
...
- Реализуйте балансировку интернет-трафика на общедоступные серверы, выполнив действия, описанные в статье [Создание подсистемы балансировки нагрузки для Интернета на портале Azure](../load-balancer/quickstart-load-balancer-standard-public-portal.md?toc=%2fazure%2fnetworking%2ftoc.json).