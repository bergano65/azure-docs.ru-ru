...
- [Подключение между ресурсами Azure](#connectivity). Свяжите вместе ресурсы Azure в облаке в защищенной частной виртуальной сети.
- [Подключение к Интернету](#internet-connectivity). Обменивайтесь данными между ресурсами Azure через Интернет.
- [Подключение к локальной среде](#on-premises-connectivity). Подключайте локальные сети к ресурсам Azure через виртуальную частную сеть в Интернете или выделенное подключение к Azure.
- [Направление трафика и балансировка нагрузки](#load-balancing). Загружайте балансировку трафика к серверам в одно расположение, и прямой трафик на серверы в другое.
- [Безопасность](#security) — Реализуйте фильтрацию сетевого трафика между подсетями или отдельными виртуальными машинами.
...
- [Управляемость.](#manageability) Отслеживайте сетевые ресурсы Azure и управляйте ими.
- [Средства развертывания и настройки](#tools). Используйте веб-портал или кросс платформенные средства командной строки для развертывания и настройки сетевых ресурсов.
...
- **Портал Azure** Графический пользовательский интерфейс, выполняемый в браузере. Откройте [портал Azure](https://portal.azure.com).
- **Azure PowerShell**. Программы командной строки для управления Azure на компьютерах Windows. Дополнительные сведения об Azure PowerShell см. в [этой статье](/powershell/azure/overview?view=azurermps-3.8.0?toc=%2fazure%2fnetworking%2ftoc.json).
- **Интерфейс командной строки Azure (CLI)**. Программы командной строки для управления Azure на компьютерах Linux, macOS или Windows. Дополнительные сведения об Azure CLI см. в [этой статье](/cli/azure/get-started-with-azure-cli?toc=%2fazure%2fnetworking%2ftoc.json).
