...
В этом разделе описаны службы, обеспечивающие подключение ресурсов Azure, подключение от локтевой сети к ресурсам Azure и ветка для подключения филиалов в Azure - Virtual Network (VNet), Virtual WAN, ExpressRoute, VPN Gateway, Виртуальная сеть NAT Gateway, Azure DNS, служба Azure Peering и Azure Bastion.
...
|[VPN-шлюз](#vpngateway)|Отправляет зашифрованный трафик между виртуальной сетью Azure и локальным местоположением через общедоступный Интернет.|<p>[Подключения "сеть — сеть"](../vpn-gateway/vpn-gateway-howto-site-to-site-resource-manager-portal.md)</p> <p>[Соединения VNet-to-VNet](../vpn-gateway/vpn-gateway-howto-vnet-vnet-resource-manager-portal.md)</p> <p>[Подключения "точка — сеть"](../vpn-gateway/vpn-gateway-howto-point-to-site-resource-manager-portal.md)</p>|
...
|[Бастион Azure](#bastion)|Наймите безопасное и бесшовное подключение RDP/SSH к вашим виртуальным машинам непосредственно на портале Azure через TLS. При подключении через Azure Bastion ваши виртуальные машины не нуждаются в общедоступном IP-адресе|<p>[Создание узла-бастиона Azure](../bastion/bastion-create-host-portal.md)</p><p>[Подключитесь с помощью SSH к Linux VM](../bastion/bastion-connect-vm-ssh.md)</p><p>[Подключение с помощью RDP к Windows VM](../bastion/bastion-connect-vm-rdp.md)</p>|
...
Служба Бастион Azure — это новая полностью управляемая платформой PaaS-служба, которая подготавливается в виртуальной сети. Он обеспечивает безопасное и бесшовное подключение RDP/SSH к вашим виртуальным машинам непосредственно на портале Azure через TLS. При подключении с помощью Бастиона Azure виртуальным машинам не требуются общедоступные IP-адреса. Для получения дополнительной информации, [см. Что такое Azure Бастион?](../bastion/bastion-overview.md).
...
В этом разделе описаны сетевые службы в Azure, которые помогают защитить ваши сетевые ресурсы - Защита ваших приложений, использующих любые или комбинации этих сетевых служб в Azure - Private Link, DDoS-защиту, брандмауэр, группы сетевой безопасности, веб-приложения Firewall и виртуальные сетевые конечные точки.

|Служба|Зачем использовать?|Сценарий|
|---|---|---|
|[Защита от DDoS](#ddosprotection) |Высокая доступность для ваших приложений с защитой от избыточных сборов IP-трафика|[Управление защитой DDOS Azure](../virtual-network/manage-ddos-protection.md)|
...
|[Конечные точки службы для виртуальной сети](#serviceendpoints)|Позволяет ограничить доступ к сети некоторым ресурсам службы Azure виртуальной сетевой подсети|[Настройка конечных точек служб виртуальной сети](../virtual-network/tutorial-restrict-network-access-to-resources-powershell.md)|
...
|[Монитор ЭкспрессРут](#expressroutemonitor)|Обеспечивает мониторинг производительности, доступности и использования сети в режиме реального времени, помогает с автоматическим открытием топологии сети, обеспечивает более быструю изоляцию неисправностей, обнаруживает переходные проблемы сети, помогает анализировать исторические характеристики производительности сети, поддерживает мультиподписку|<p>[Настройка решения "Монитор производительности сети" для ExpressRoute](../expressroute/how-to-npm.md)</p><p>[Мониторинг, метрики и оповещения в ExpressRoute](../expressroute/expressroute-monitoring-metrics-alerts.md)</p>|
