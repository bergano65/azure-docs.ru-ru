...
![Общие сведения о службе Azure Monitor](media/monitoring-data-collection/overview.png)
...
В Azure собранные метрики попадают в базу данных метрик Azure Monitor. Это база данных временных рядов, оптимизированная для быстрого извлечения данных и хранения значений метрик в течение 93 дней. Копировать метрики в службу Log Analytics для анализа и отслеживания долгосрочных тенденций.
...
Как было сказано выше, метрики реагируют быстрее, чем журналы, позволяя создавать оповещения с меньшими задержками и затратами. Служба Log Analytics собирает значительное количество числовых данных, которые подходят для метрик, но не хранятся в базе данных метрик Azure.  Распространенным примером являются данные о производительности, полученные от агентов и решений по управлению. Некоторые из этих значений можно скопировать в базу данных метрик, где они будут доступны для оповещений и анализа с помощью обозревателя метрик.
