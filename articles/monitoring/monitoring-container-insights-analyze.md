---
title: Мониторинг производительности кластера AKS с помощью Azure Monitor для контейнеров (предварительная версия) | Документация Майкрософт
...
ms.locfileid: "50214160"
---
# <a name="understand-aks-cluster-performance-with-azure-monitor-for-containers-preview"></a>Общие сведения о мониторинге производительности кластера AKS с помощью Azure Monitor для контейнеров (предварительная версия)
...
![Пример перехода из узла к контроллеру в представление производительности](./media/monitoring-container-insights-analyze/drill-down-node-controller.png)
...
Иерархия записи начинается с контроллером, и при его развертывании вы просматриваете один или несколько модулей pod.  Разверните pod, и в последней строке отобразится контейнер, сгруппированный в этот pod. Из развернутого контроллера можно перейти к узлу, который запущен, чтобы просмотреть данные производительности, отобранные для этого узла. Щелкните значение в столбце **Узел** для конкретного контролера.   

![Пример перехода из узла к контроллеру в представление производительности](./media/monitoring-container-insights-analyze/drill-down-controller-node.png)
...
Из контейнера можно перейти к сведениям в pod или узел, чтобы просмотреть данные производительности, отобранные для этого объекта. Щелкните значение в столбце **Pod** или **Узел** для конкретного контролера.   
