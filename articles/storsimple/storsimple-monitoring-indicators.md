...
| --- | --- | --- | --- | --- |
| Красно-желтый |OFF |OFF |Н/Д |Питание от электросети отсутствует, используется резервный источник питания; или питание от электросети включено, но модули контроллера извлечены. |
| Зеленый |ON |ON |Н/Д |Состояние тестирования с включенным питанием ПУ (5 с) |
| Зеленый |OFF |OFF |Н/Д |Питание включено, все функционирует нормально |
| Зеленый |ON |Н/Д |Светодиодные индикаторы сбоя блока питания с охлаждением (БПО), светодиодные индикаторы сбоя вентилятора |Любой сбой БПО, сбой вентилятора, слишком высокая или слишком низкая температура |
| Зеленый |ON |Н/Д |Светодиодные индикаторы модуля ввода-вывода |Любой сбой модуля контроллера |
| Зеленый |ON |Н/Д |Н/Д |Логический сбой корпуса |
...
| Сбой БПО (сбой вентилятора) |OFF |OFF |ON |Н/Д |
...
| --- | --- | --- | --- |
| Диск не установлен |OFF |OFF |Отсутствуют |
| Диск установлен и функционирует |Мигание при активности |X |Отсутствуют |
| Задана идентификация устройства с поддержкой SES |ON |Мигание с интервалом 1 секунда |Отсутствуют |
...
| S2 |Режим напоминания: кратковременный сигнал |Отсутствуют |
| S3 |Режим с отключенным звуком: без звука |Отсутствуют |
...
| Состояние | Severity | Сигнал | Светодиодный индикатор ПУ |
| --- | --- | --- | --- |
...
| БПО удален |Ошибка конфигурации |Отсутствуют |Сбой модуля |
...
| Извлечен интерфейсный модуль SBB |Предупреждение |Отсутствуют |Сбой модуля |
...
| Диск удален |Предупреждение |Отсутствуют |Сбой модуля |
