...
1. В учетной записи службы автоматизации щелкните **Variables** (Переменные), &gt;**Добавить переменную** и добавьте приведенные ниже переменные. Вы также можете зашифровать эти ресурсы. Переменные зависят от плана восстановления. Если имя плана восстановления, который вы создадите на следующем шаге, — TestPlan, то у переменных будут имена TestPlan-StorSimRegKey, TestPlan-AzureSubscriptionName и т. п.
   ...
   - **BaseUrl:** URL-адрес диспетчера ресурсов для облака Azure. Получить с помощью **Get-AzEnvironment | SELECT-Object Name, Resourcemanagerurll** командлета.
   - _RecoveryPlanName_ **- ResourceGroupName**: группа диспетчера ресурсов с ресурсом StorSimple.
   - _RecoveryPlanName_ **- ManagerName**: ресурс StorSimple с устройством StorSimple.
   - _RecoveryPlanName_ **- DeviceName**: устройство StorSimple, для которого необходимо выполнить отработку отказа.
   - _RecoveryPlanName_ **- DeviceIpAddress**: IP-адрес устройства (можно найти на вкладке **Устройства** в разделе диспетчера устройств StorSimple в группе &gt; **Параметры** &gt; **Сеть** &gt; **Параметры DNS**).
   - _RecoveryPlanName_ **- VolumeContainers**: строка разделенных запятыми контейнеров томов, содержащихся на устройстве для отработки отказа, например volcon1, volcon2, volcon3.
   - _RecoveryPlanName_ **- TargetDeviceName**: облачное устройство StorSimple, на которое нужно выполнить отработку отказа контейнеров.
   - _RecoveryPlanName_ **- TargetDeviceIpAddress**: IP-адрес целевого устройства (его можно найти в разделе **Виртуальная машина** &gt; в группе **Параметры** &gt; на вкладке **Сетевые подключения**).
   - _RecoveryPlanName_ **- StorageAccountName**: имя учетной записи хранения, где будет храниться сценарий (который должен выполняться на виртуальной машине с выполненной отработкой отказа). Ею может быть любая учетная запись хранения, в которой есть пространство для временного хранения сценария.
   - _RecoveryPlanName_ **- StorageAccountKey**: ключ доступа к вышеупомянутой учетной записи хранения.
   - _RecoveryPlanName_ **- VMGUIDS**: после включения защиты Azure Site Recovery назначает каждой виртуальной машине уникальный идентификатор с подробными сведениями о виртуальной машине, для которой была выполнена отработка отказа. Чтобы получить VMGUID, откройте вкладку **Службы восстановления** и последовательно выберите **Защищенный элемент** &gt; **Группы защиты** &gt; **Компьютеры** &gt; **Свойства**. При наличии нескольких виртуальных машин добавьте идентификаторы GUID в формате строки с разделителями-запятыми.
...
1. В группе **Управление** выберите параметр **Планы восстановления (Site Recovery)** и создайте план восстановления, выполнив указанные ниже действия.
   ...
   - Щелкните правой кнопкой мыши элемент **Группа 1: запуск** и выберите пункт **Действие при учете**, а затем добавьте все следующие сценарии:  
...
- Время ожидания задания клонирования. Время ожидания выполнения сценария StorSimple истечет, если клонирование томов длится дольше, чем установлено для ограничения сценария Azure Site Recovery (сейчас это 120 минут).
- Ошибка синхронизации времени. Сценарий StorSimple будет отображать сообщения о том, что процессы резервного копирования завершились ошибкой, несмотря на то, что на портале они выполнены успешно. Причиной этому может быть то, что время на устройстве StorSimple не отвечает текущему времени часового пояса.
