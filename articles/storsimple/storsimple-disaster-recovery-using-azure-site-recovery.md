...
1. Создайте файловые ресурсы, используя тома, взятые из хранилища StorSimple. Дополнительные сведения см. в статье [Использование службы StorSimple Manager для управления томами](storsimple-manage-volumes.md).
   ...
   1. Откройте портал Azure и выберите вкладку **Volumes and Devices.** Нажмите **Автоматическая настройка.** После этого должен отобразиться созданный том.
...
1. В учетной записи службы автоматизации щелкните **Variables (Переменные), ** &gt; **Добавить переменную** и добавьте приведенные ниже переменные. Вы также можете зашифровать эти ресурсы. Переменные зависят от плана восстановления. Если имя плана восстановления, который вы создадите на следующем шаге, — TestPlan, то у переменных будут имена TestPlan-StorSimRegKey, TestPlan-AzureSubscriptionName и т. п.
   ...
   - **BaseUrl.** URL-адрес диспетчера ресурсов для облака Azure. Получить с помощью **Get-AzEnvironment (ru) Название объекта, ResourceManagerUrl** cmdlet.
   - _RecoveryPlanName_**-ResourceGroupName.** Группа диспетчера ресурсов с ресурсом StorSimple.
   - _RecoveryPlanName_**-ManagerName**: Ресурс StorSimple, который имеет устройство StorSimple.
   - _RecoveryPlanName_**-DeviceName**: Устройство StorSimple, которое должно быть сбой более.
   - _RecoveryPlanName_**-DeviceIpAddress**: IP-адрес устройства (это можно найти во вкладке &gt; **"Устройства"** в разделе StorSimple Device Manager **Настройки** &gt; **группы** &gt; настроек Сети **DNS Настройки).**
   - _RecoveryPlanName_**-VolumeContainers**: Запятая отделенная строка объемных контейнеров, присутствующих на устройстве, которые должны быть несудавшимся; например: volcon1, volcon2, volcon3.
   - _RecoveryPlanName_**-TargetDeviceName**: Облачный прибор StorSimple, на котором контейнеры должны быть неведомы.
   - _RecoveryPlanName_**-TargetDeviceIpAddress**: IP-адрес целевого устройства (это можно найти во &gt; вкладке **группы настройки группы настройки** **виртуальной машины).** &gt; **Networking**
   - _RecoveryPlanName_**-StorageAccountName**: Имя учетной записи хранилища, в котором будет храниться скрипт (который должен работать на сбой над VM). Ею может быть любая учетная запись хранения, в которой есть пространство для временного хранения сценария.
   - _RecoveryPlanName_**-StorageAccountKey**: Ключ доступа для вышеуказанной учетной записи хранения.
   - _RecoveryPlanName_**-VMGUIDS**: После защиты VM, Восстановление сайта Azure присваивает каждому VM уникальный идентификатор, который дает подробную информацию о сбой над VM. Чтобы получить VMGUID, откройте вкладку **Службы восстановления** и последовательно выберите **Защищенный элемент** &gt; **Группы защиты** &gt; **Компьютеры** &gt; **Свойства**. При наличии нескольких виртуальных машин добавьте идентификаторы GUID в формате строки с разделителями-запятыми.
...
1. В группе **Управление** выберите параметр **Планы восстановления (Site Recovery)** и создайте план восстановления, выполнив указанные ниже действия.
   ...
   - Добавьте ручное действие после четырех сценариев, указанных выше, в том же разделе **Группа 1: последующие действия** . Это действие позволяет проверить, все ли работает правильно. Это действие должно быть добавлено только как часть теста failover (так что только выберите **тест Failover** флажок).
    
   - После ручного действия добавьте скрипт **очистки,** используя ту же процедуру, что и для других рун. **Сохраните** план восстановления.
