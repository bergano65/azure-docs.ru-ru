...
1. В учетной записи службы автоматизации щелкните **Variables** (Переменные), &gt;**Добавить переменную** и добавьте приведенные ниже переменные. Вы также можете зашифровать эти ресурсы. Переменные зависят от плана восстановления. Если имя плана восстановления, который вы создадите на следующем шаге, — TestPlan, то у переменных будут имена TestPlan-StorSimRegKey, TestPlan-AzureSubscriptionName и т. п.
   ...
   - **BaseUrl:** URL-адрес диспетчера ресурсов для облака Azure. Получение с помощью **Get-азенвиронмент | SELECT — имя объекта, командлет ResourceManagerUrl** .
   ...
   - _RecoveryPlanName_ **-Девицеипаддресс**: IP-адрес устройства (можно найти на вкладке **Устройства** в разделе диспетчера устройств StorSimple в группе &gt; **Параметры** &gt; **Сеть** &gt; **Параметры DNS**).
   ...
   - _RecoveryPlanName_ **-Таржетдевиценаме**: облачное устройство StorSimple, на которое нужно выполнить отработку отказа контейнеров.
   - _RecoveryPlanName_ **-Таржетдевицеипаддресс**: IP-адрес целевого устройства (его можно найти в разделе **Виртуальная машина** &gt; в группе **Параметры** &gt; на вкладке **Сетевые подключения**).
   ...
   - _RecoveryPlanName_ **-Вмгуидс**: после включения защиты Azure Site Recovery назначает каждой виртуальной машине уникальный идентификатор с подробными сведениями о виртуальной машине, для которой была выполнена отработка отказа. Чтобы получить VMGUID, откройте вкладку **Службы восстановления** и последовательно выберите **Защищенный элемент** &gt; **Группы защиты** &gt; **Компьютеры** &gt; **Свойства**. При наличии нескольких виртуальных машин добавьте идентификаторы GUID в формате строки с разделителями-запятыми.
