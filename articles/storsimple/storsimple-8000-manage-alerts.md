...
* **Для локальных данных на вашем устройстве**: В течение некоторого времени будет прерываться, и операции чтения будут обслуживаться. Однако по мере увеличения числа операций ввода-вывода до превышающих заданные ограничения могут возникнуть сбои операций чтения.
...
* **Для данных в облаке**: Для большинства ошибок подключения возвращается ошибка. После восстановления подключения обработка операций ввода-вывода возобновляется без необходимости подключения тома пользователем. В редких случаях может потребоваться вмешательство пользователя для включения тома на портале Azure.
* **Для облачных моментальных снимков выполняется**: Операция повторяется несколько раз в течение 4 – 5 часов и завершится ошибкой, если подключение не восстанавливается, облачные моментальные снимки.
