...
* **Имя**— уникальное имя, назначенное шаблону полосы пропускания при его создании.
* **Расписание**— число расписаний, содержащихся в рассматриваемом шаблоне полосы пропускания.
* **Используется**— число томов, использующих шаблоны полосы пропускания.
...
2. В колонке **Добавление шаблона пропускной способности** выполните следующие действия.
    ...
    2. Задайте расписание для пропускной способности. Чтобы создать расписание, выполните следующие действия.
        ...
        1. Из списка выпадающих списков выберите **Дни** недели, для них настроено расписание. Здесь можно выбрать несколько дней.        
        ...
        4. Укажите **коэффициент пропускной способности**. Это значение пропускной способности в Мбит/с, которое используется устройством StorSimple в операциях с облаком (скачивание и передача). Укажите в этом поле число от 1 до 1000.
...
**В**. Что происходит с элементами управления в период между выполнением расписаний? (Одно расписание завершено, а другое не начато.)

**A**. В таких случаях элементы управления пропускной способностью не будут использоваться. Это означает, что устройство может использовать неограниченную пропускную способность при размещении данных по уровням в облаке.

**В**. Можно ли изменить шаблоны пропускной способности на автономном устройстве?

**A**. Вы не сможете изменить шаблоны пропускной способности в контейнерах томов, если соответствующее устройство находится вне сети.

**В**. Можно ли изменить шаблон пропускной способности, связанный с контейнером томов, если связанные тома находятся вне сети?

**A**. Вы можете изменить шаблон пропускной способности, связанный с контейнером томов, если его тома находятся вне сети. Обратите внимание, что если тома находятся вне сети, данные устройства не будут размещены по уровням в облаке.

**В**. Можно ли удалить шаблон по умолчанию?

**A**. Хотя вы можете удалить шаблон по умолчанию, это не рекомендуется. Использование шаблона по умолчанию, в том числе измененных версий, отслеживается. Данные отслеживания анализируются и со временем используются для усовершенствования шаблона по умолчанию.

**В**. Как определить, нужно ли изменить шаблоны пропускной способности?

**A**. Один из признаков того, что нужно изменить шаблоны пропускной способности, — замедление или неоднократное прерывание работы сети. В этом случае просмотрите данные использования хранилища и сети в диаграммах производительности операций ввода-вывода и пропускной способности сети.
