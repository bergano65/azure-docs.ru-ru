...
2. Настройте сеть: мастер установки позволяет настроить параметры сети для сетевого интерфейса DATA 0 на устройстве StorSimple. В их число входят следующие параметры.
...
StorSimple включает в себя несколько средств, которые можно использовать для устранения неполадок в решении StorSimple. Сюда входит следующее.
...
Дополнительные сведения об использовании этого командлета см. в статье [Get-NetAdapter](/powershell/module/netadapter/get-netadapter?view=win10-ps) в справочнике по командлетам Windows PowerShell.
...
3. Убедитесь в работоспособности сетевого интерфейса:
   ...
   * Если канал не работает, статус **ifindex** будет показывать, что интерфейс отключен. Затем необходимо будет проверить сетевое подключение порта к устройству и коммутатору. Также необходимо будет исключить поврежденные кабели. 
   * Если вы подозреваете, что произошел сбой порта DATA 0 на активном контроллере, это можно проверить, подключившись к порту DATA 0 на контроллере 1. Чтобы проверить это, отключите сетевой кабель на задней панели устройства от контроллера 0, подключите его к контроллеру 1 и снова выполните командлет Get-NetAdapter.
     В случае неисправности контроллера порта DATA 0 [обратитесь в службу поддержки пользователей Майкрософт](storsimple-8000-contact-microsoft-support.md) для дальнейших действий. Возможно, потребуется заменить контроллер в системе.
