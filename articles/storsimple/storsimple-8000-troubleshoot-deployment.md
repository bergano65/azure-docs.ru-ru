...
| --- | --- | --- | --- |
| 1 |Invoke-HcsSetupWizard: Эта команда может выполняться только на активном контроллере. |Настройка выполнялась на пассивном контроллере. |Выполните эту команду на активном контроллере. Дополнительные сведения см. в разделе [Определение активного контроллера устройства](storsimple-8000-controller-replacement.md#identify-the-active-controller-on-your-device). |
...
| 3 |Invoke-HcsSetupWizard: Имеется конфликт IP-адресов с другой системой в сети (исключение из HRESULT: 0x80070263). |IP-адрес, предоставленный для DATA 0, уже используется другой системой. |Укажите новый IP-адрес, который еще не используется. |
...
| 7 |Invoke-HcsSetupWizard: Осталось доступных конечных точек. (Исключение из HRESULT: 0x800706D9) |Функциональные возможности кластера не работают. |[Обратитесь в службу поддержки Майкрософт](storsimple-8000-contact-microsoft-support.md) , чтобы узнать, что делать дальше. |
...
| 5 |Invoke-HcsSetupWizard: Не в сети ресурс кластера (исключение из HRESULT: 0x8007138c) |Ресурсы кластера отключены от сети. |[Обратитесь в службу поддержки Майкрософт](storsimple-8000-contact-microsoft-support.md) , чтобы узнать, что делать дальше. |
...
* `Get-NetAdapter`: Для определения работоспособности сетевых интерфейсов с помощью этого командлета.
* `Test-Connection`: Этот командлет используется для проверки сетевого подключения внутри и вне сети.
* `Test-HcsmConnection`: Используйте этот командлет для проверки подключения успешно зарегистрированного устройства.
* `Sync-HcsTime`: Используйте этот командлет для отображения времени устройства и время синхронизации с NTP-сервером.
* `Enable-HcsPing` и `Disable-HcsPing`: Используйте эти командлеты позволяют узлам для проверки связи сетевые интерфейсы на устройстве StorSimple. По умолчанию сетевые интерфейсы устройства StorSimple не отвечают на запросы проверки связи.
...
* `Get-HcsRoutingTable`: Используйте этот командлет для отображения таблица маршрутизации локальных IP-адрес.
