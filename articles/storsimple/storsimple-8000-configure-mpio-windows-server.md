...
3. В мастере **Добавление ролей и компонентов** выполните следующие шаги.
   ...
   3. На странице **Select destination server** (Выбор целевого сервера) выберите **Select a server from the server pool** (Выбрать сервер из пула серверов). Сервер должен быть найден автоматически. Нажмите кнопку **Далее**.
   ...
   6. На странице **подтвердите выбор установки,** а затем выберите **перезагрузить сервер назначения автоматически, если это необходимо,** как показано ниже. Нажмите кнопку **Установить**.
...
5. В диалоговом окне **MPIO Properties** щелкните вкладку **MPIO Devices.** Нажмите **Добавить**.
...
11. В диалоговом окне **Дополнительные параметры**:
    ...
    1. В **списке локальных адаптеров** выберите Microsoft iSCSI Initiator.
...
> [!IMPORTANT]
> **Мы рекомендуем не смешивать 1 интерфейсы сети GbE и 10 GbE. Если вы используете два сетевых интерфейса, оба интерфейса должны быть одинакового типа.**
...
12. Для просмотра устройств, представленных в сеансах, выберите вкладку **«Устройства».** Чтобы настроить политику MPIO для выбранного устройства, нажмите **MPIO**. Откроется диалоговое окно **Сведения об устройстве**. На вкладке **MPIO** можно выбрать соответствующие параметры **Политики балансировки нагрузки**. Можно также просмотреть **активный** или **ждущий** тип пути.
