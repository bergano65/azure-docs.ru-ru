...
| --- | --- | --- | --- | --- |
| TCP 80 (HTTP) |Выходной |WAN |Нет |Исходящий порт используется для доступа к Интернету и извлечения обновлений. <br></br>Пользователи могут настраивать исходящий веб-прокси. |
| TCP 443 (HTTPS) |Выходной |Глобальная сеть |Да |Исходящий порт используется для доступа к данным в облаке. <br></br>Пользователи могут настраивать исходящий веб-прокси. |
| UDP 53 (DNS) |Выходной |Глобальная сеть |В некоторых случаях. См. примечания. |Этот порт требуется, только если вы используете веб-DNS-сервер. <br></br> Обратите внимание, что при развертывании файлового сервера рекомендуется использовать локальный DNS-сервер. |
| UDP 123 (NTP) |Выходной |Глобальная сеть |В некоторых случаях. См. примечания. |Этот порт требуется, только если вы используете веб-NTP-сервер.<br></br> Обратите внимание, что при развертывании файлового сервера рекомендуется синхронизировать время с контроллерами домена Active Directory. |
| TCP 80 (HTTP) |In |Локальная сеть |Да |Это входящий порт для локального пользовательского интерфейса на устройстве StorSimple, используемый для локального управления. <br></br> Обратите внимание, что запрос доступа к локальному пользовательскому интерфейсу по протоколу HTTP будет автоматически перенаправлен на HTTPS. |
| TCP 443 (HTTPS) |In |Локальная сеть |Да |Это входящий порт для локального пользовательского интерфейса на устройстве StorSimple, используемый для локального управления. |
| TCP 3260 (iSCSI) |In |Локальная сеть |Нет |Этот порт используется для доступа к данным по iSCSI. |
...
| --- | --- |
| `https://*.storsimple.windowsazure.com/*`<br>`https://*.accesscontrol.windows.net/*`<br>`https://*.servicebus.windows.net/*` <br>`https://login.windows.net`|Служба диспетчера устройств StorSimple<br>Служба управления доступом<br>Служебная шина Azure<br>Служба проверки подлинности|
