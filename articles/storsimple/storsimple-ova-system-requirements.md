---
title: Microsoft Azure StorSimple требования к системе для виртуального массива
...
* Системные требования для устройств StorSimple серии 8000. см. [здесь](./storsimple-8000-system-requirements.md).
...
| **Компонент** | **Version** | **Дополнительные требования и примечания** |
| --- | --- | --- |
...
Поддерживается только хранилище блочных BLOB-объектов Azure. Страничные BLOB-объекты не поддерживаются. Дополнительные сведения о [блочных и страничных BLOB-объектах](/rest/api/storageservices/understanding-block-blobs--append-blobs--and-page-blobs).
...
| **Номер порта <sup>1</sup>** | **Входящий или исходящий** | **Область порта** | **Обязательное** | **Примечания** |
| --- | --- | --- | --- | --- |
| TCP 80 (HTTP) |Исходящий |Глобальная сеть |Нет |Исходящий порт используется для доступа к Интернету и извлечения обновлений. <br></br>Пользователи могут настраивать исходящий веб-прокси. |
| TCP 443 (HTTPS) |Исходящий |Глобальная сеть |Да |Исходящий порт используется для доступа к данным в облаке. <br></br>Пользователи могут настраивать исходящий веб-прокси. |
| UDP 53 (DNS) |Исходящий |Глобальная сеть |В некоторых случаях. См. примечания. |Этот порт требуется, только если вы используете веб-DNS-сервер. <br></br> Обратите внимание, что при развертывании файлового сервера рекомендуется использовать локальный DNS-сервер. |
| UDP 123 (NTP) |Исходящий |Глобальная сеть |В некоторых случаях. См. примечания. |Этот порт требуется, только если вы используете веб-NTP-сервер.<br></br> Обратите внимание, что при развертывании файлового сервера рекомендуется синхронизировать время с контроллерами домена Active Directory. |
| TCP 80 (HTTP) |В |Локальная сеть |Да |Это входящий порт для локального пользовательского интерфейса на устройстве StorSimple, используемый для локального управления. <br></br> Обратите внимание, что запрос доступа к локальному пользовательскому интерфейсу по протоколу HTTP будет автоматически перенаправлен на HTTPS. |
...
| --- | --- |
| `https://*.storsimple.windowsazure.com/*`<br>`https://*.accesscontrol.windows.net/*`<br>`https://*.servicebus.windows.net/*` <br>`https://login.windows.net`|Служба диспетчера устройств StorSimple<br>Служба контроля доступа<br>Служебная шина Azure<br>Служба проверки подлинности|
