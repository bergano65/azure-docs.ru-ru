---
title: Настройка MPIO на storSimple Linux хос
...
A. Как правило, не видя каких-либо многопатовых путей предполагает проблему с многопатовым daemon, `multipath.conf` и это, скорее всего, что любая проблема здесь лежит в файле.

Кроме того, стоит проверить, что вы можете увидеть некоторые диски после подключения к цели, так как нет ответа от многополосных списков также может означать, что у вас нет дисков.

* Повторно просканируйте шину SCSI, выполнив такую команду:
  
    `$ rescan-scsi-bus.sh`(часть sg3_utils пакета)
...
* Чтобы определить, принадлежит ли диск устройству StorSimple, выполните такую команду:
    ...
    Это вернет строку, которая определит, является ли это диском StorSimple.
...
Для получения дополнительной информации, перейдите на [устранение неполадок для мультипатов](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/6/html/dm_multipath/mpio_admin-troubleshoot).
