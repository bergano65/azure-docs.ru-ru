...
* **Высокий уровень доступности**. Он обеспечивает альтернативный путь, если происходит сбой какой-либо элемент пути ввода-вывода (например, кабеля, коммутатора, сетевого интерфейса или контроллера).
* **Балансировка нагрузки**: В зависимости от конфигурации, устройство хранения он может повысить производительность путем обнаружения нагрузки ввода-вывода и динамически перераспределения этих нагрузок.
...
* **Ядра**: Основным компонентом является *устройства mapper* , перенаправляет ввод-вывод и обеспечивает отказоустойчивость каналов и их групп.

* **Пространство пользователя**: Это *multipath-tools* многопутевых устройств, управление, предписывая устройства модуль, что делать. Инструменты состоят из следующих компонентов.
...
- **Системные параметры** *(по умолчанию)* : Можно переопределить значения по умолчанию на уровне системы.
- **Список запрещенных устройств** *(blacklist)* : Можно указать список устройств, не должен управлять компонент device-Mapper.
- **Исключения из списка запрещенных** *(blacklist_exceptions)* : Можно указать определенные устройства будет считаться многоканального ввода-вывода устройств, даже если содержатся в списке запрещенных.
- **Параметры контроллера хранилища** *(устройства)* : Можно указать параметры конфигурации, которые будут применяться к устройствам, содержащих сведения о поставщике и продукте.
- **Параметры определенных устройств** *(multipaths)* : В этом разделе можно использовать для точной настройки параметров отдельных LUN.
...
1. Установите *iSCSI-initiator-utils* на сервер CentOS. Чтобы установить *iSCSI-initiator-utils*, сделайте следующее.
   ...
   1. Установите *iSCSI-initiator-utils*. Тип:
      
       `yum install iscsi-initiator-utils`
   1. После успешной установки *iSCSI-Initiator-utils* запустите службу iSCSI. Тип:
   ...
   1. Чтобы гарантировать работу инициатора iSCSI во время загрузки, включите службу с помощью команды `chkconfig` .
       ...
       `chkconfig iscsi on`
   1. Чтобы убедиться в правильности установки, выполните следующую команду:
...
1. Установите *device-mapper-multipath*. Тип:
...
* Наличие как минимум двух активных интерфейсов для iSCSI. Чтобы убедиться в наличии двух активных интерфейсов iSCSI на устройстве StorSimple, выполните на классическом портале Azure для устройства StorSimple следующие действия.
  ...
  1. Выберите службу StorSimple Manager, щелкните **Устройства** и выберите нужное устройство StorSimple. Щелкните **Настройка** и проверьте параметры сетевого интерфейса. На снимке экрана ниже показаны два сетевых интерфейса с поддержкой iSCSI. Здесь оба интерфейса 10 GbE (DATA 2 и DATA 3) поддерживают iSCSI.
     ...
     1. Убедитесь, что сетевые интерфейсы имеют одинаковую скорость — 1 GbE или 10 GbE.
     1. Запишите IPv4-адреса интерфейсов с поддержкой iSCSI и сохраните их для последующего использования на узле.
...
### <a name="step-1-configure-multipathing-for-automatic-discovery"></a>Шаг 1. Настройка автоматического обнаружения доступных каналов
...
1. Запустите службу многоканального ввода-вывода. Тип:
...
1. Включите автоматическое обнаружение каналов ввода-вывода. Тип:
...
### <a name="step-2-configure-multipathing-for-storsimple-volumes"></a>Шаг 2. Настройка многоканального ввода-вывода для томов StorSimple
...
1. Найдите в файле multipath.conf раздел blacklist_exceptions. В этот список исключений нужно добавить ваше устройство StorSimple. Раскомментируйте соответствующие строки в этом файле, как показано ниже (укажите только свою модель устройства).
...
### <a name="step-3-configure-round-robin-multipathing"></a>Шаг 3. Настройка циклического многоканального ввода-вывода
...
### <a name="step-4-enable-multipathing"></a>Шаг 4. Включить многоканального ввода-вывода
...
### <a name="step-5-verify-multipathing"></a>Шаг 5. Проверьте многоканального ввода-вывода
1. Сначала убедитесь, что соединение iSCSI с устройством StorSimple установлено.
   ...
   2\. Подключитесь к устройству с помощью IQN целевого объекта. StorSimple здесь является целевым объектом iSCSI. Тип:
...
1. Том предоставляется серверу CentOS с устройства StorSimple. Дополнительные сведения см. в разделе [шаг 6: Создание тома](storsimple-8000-deployment-walkthrough-u2.md#step-6-create-a-volume) на портале Azure на устройстве StorSimple.

1. Проверьте доступные пути. Тип:
...
Дополнительные сведения см. в статье [Устранение неполадок многоканального ввода-вывода](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/6/html/dm_multipath/mpio_admin-troubleshoot).
