...
> * Если вы настраиваете RBS для фермы SharePoint, которая имеет очень большое количество баз данных (больше 200), страница **Центральной администрации SharePoint** может отойти. Если это произойдет, обновите страницу. Это не влияет на процесс настройки.
...
Кроме того, вы можете `RBS Migrate()` использовать cmdlet Microsoft PowerShell, включенный в SharePoint. Дополнительные сведения см. в статье о [переносе содержимого в RBS и из RBS](https://technet.microsoft.com/library/ff628255.aspx).
