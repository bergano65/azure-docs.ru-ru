...
6. На странице **Параметры устройства** :
   ...
   3. Если ваше устройство является файловым сервером, необходимо присоединить его к домену. Введите **доменное имя.**
...
8. Щелкните **Применить**. Параметры устройства будут проверены и применены.
...
10. (Необязательно) Настройте параметры времени для своего устройства, такие как часовой пояс, а также основной и дополнительный NTP-серверы. NTP-серверы необходимы, так как устройство должно синхронизировать время, чтобы проходить проверку подлинности в облачных службах.
    ...
    1. Из списка выпадающих данных выберите **часовой пояс** на основе географического местоположения, в котором развертывается устройство. Часовой пояс по умолчанию для устройства — тихоокеанское стандартное время (PST). Устройство будет использовать заданный часовой пояс для всех запланированных операций.
...
11. Настройте параметры облака для устройства. На этом этапе вы завершите настройку локального устройства и зарегистрируете его в службе диспетчера устройств StorSimple.
    ...
    2. Если это первое устройство, которое вы регистрируете в этой службе, вы получите **ключ шифрования данных службы**. Скопируйте этот ключ и сохраните его в безопасном месте. Этот ключ вместе с ключом регистрации службы требуется для регистрации дополнительных устройств в службе диспетчера устройств StorSimple. 
       ...
       Если это не первое устройство, которое вы регистрируете в службе, необходимо указать ключ шифрования данных службы. Для получения дополнительной информации обратитесь, чтобы получить [ключ шифрования данных службы](storsimple-ova-web-ui-admin.md#get-the-service-data-encryption-key) на локальном веб-ui.
...
1. Перейдите на службу StorSimple Device Manager, а затем перейдите на **управление > устройствами.** В колонке **Устройства** выберите устройство, которое вы только что создали. Для него должно отображаться состояние **Готово к настройке**.
...
3. На панели команд щелкните **Настроить**. Откроется колонка **Настройка**. В колонке **Настройка** выполните следующие действия.
   ...
   4. Щелкните **Настроить обязательные параметры**, чтобы указать данные учетной записи хранения, которую следует использовать с устройством. Щелкните **Добавить**, если учетная запись хранения не настроена. **Убедитесь, что используемая учетная запись хранилища поддерживает блок капли. Капли страниц не поддерживаются.** Дополнительные сведения о [блочных и страничных BLOB-объектах](https://docs.microsoft.com/rest/api/storageservices/understanding-block-blobs--append-blobs--and-page-blobs).
...
4. В колонке **добавления учетных данных учетной записи хранения** выполните следующие действия. 
    ...
    4. Включите TLS для обеспечения безопасного сетевого канала связи между устройством и облаком.
...
1. Нажмите сочетание клавиш ![](./media/storsimple-virtual-array-deploy3-fs-setup/image22.png) + R. В окне выполнения укажите *&#92;&#92;&lt;имя файлового сервера&gt;* в качестве пути, заменив *имя файлового сервера* именем устройства, присвоенным файловому серверу. Нажмите кнопку **ОК**.
...
Узнайте, как использовать локальный веб-чат-миг для [администрирования виртуального массива StorSimple.](storsimple-ova-web-ui-admin.md)
