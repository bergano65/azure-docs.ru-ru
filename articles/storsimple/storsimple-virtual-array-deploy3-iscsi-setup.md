...
7. (Необязательно) Настройте прокси-сервер доступа в Интернет. Хотя использовать прокси-сервер доступа в Интернет не обязательно, следует знать, что, если в вашей сети он имеется, настройку для работы с ним можно выполнить только в этом разделе.
   ...
   1. Предоставь **веб-прокси URL** в этом формате: *http:\//host-IP-адрес* или *номер F'DN:Port*. Обратите внимание, что URL-адреса HTTP не поддерживаются.
...
4. В окне **свойств инициатора iSCSI** на вкладке **Конечные объекты** найдите элемент **Обнаруженные конечные объекты**. (Каждый том будет обнаруженной целью.) Статус устройства должен отображаться как **неактивный.**
...
2. В окне **iSCSI Initiator Properties** на вкладке **Configuration** выберите и скопируйте строку из поля **Имени инициатора.**
