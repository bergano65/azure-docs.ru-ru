...
* У вас есть ключ регистрации службы из службы диспетчера устройств StorSimple, который вы создали для управления виртуальными массивами StorSimple. Дополнительные сведения см. в разделе **Step 2: Get the service registration key** (Шаг 2. Получение регистрационного ключа службы) статьи [Deploy StorSimple Virtual Array — Prepare the Azure portal](storsimple-virtual-array-deploy1-portal-prep.md#step-2-get-the-service-registration-key) (Развертывание виртуального массива StorSimple. Подготовка портала Azure)
...
* [Шаг 1. Завершение настройки локального веб-интерфейса и регистрация устройства](#step-1-complete-the-local-web-ui-setup-and-register-your-device)
...
## <a name="step-1-complete-the-local-web-ui-setup-and-register-your-device"></a>Шаг 1.: Завершение настройки локального веб-интерфейса и регистрация устройства
...
2. Войдите в веб-интерфейс виртуального устройства с правами администратора **StorSimpleAdmin**. Введите пароль администратора устройства, который вы изменили в соответствии с инструкциями, приведенными в разделе "Step 3: Start the virtual array and get the IP" (Шаг 3. Запуск виртуального массива и получение IP-адреса) статьи [Deploy StorSimple Virtual Array — Provision a virtual device in Hyper-V](storsimple-virtual-array-deploy2-provision-hyperv.md) (Развертывание виртуального массива StorSimple — подготовка виртуального в Hyper-V) или [Deploy StorSimple Virtual Array — Provision a virtual device in VMware](storsimple-virtual-array-deploy2-provision-vmware.md) (Развертывание виртуального массива StorSimple — подготовка в VMware).
...
3. Откроется **главная** страница. На этой странице описаны различные параметры, необходимые для настройки и регистрации виртуального устройства в службе диспетчера устройств StorSimple. Обратите внимание, что настройки в приведенных ниже разделах интерфейса **Параметры сети**, **Параметры веб-прокси** и **Параметры времени** являются необязательными. Обязательны только настройки в разделах **Параметры устройства** и **Настройки облака**.
    ...
    ![Домашняя страница](./media/storsimple-virtual-array-deploy3-iscsi-setup/image5.png)
...
7. (Необязательно) Настройте прокси-сервер доступа в Интернет. Хотя использовать прокси-сервер доступа в Интернет не обязательно, следует знать, что, если в вашей сети он имеется, настройку для работы с ним можно выполнить только в этом разделе.
   ...
   1. Укажите **URL-адрес веб-прокси** в следующем формате: *http:\//хост-ИП Address* или *FQDN: номер порта*. Обратите внимание, что URL-адреса HTTP не поддерживаются.
...
9. Настройте параметры облака для устройства. На этом этапе вы завершите настройку локального устройства и зарегистрируете его в службе диспетчера устройств StorSimple.
   ...
   1. Введите **регистрационный ключ службы**, полученный на **Step 2: Get the service registration key** (Шаг 2. Получение регистрационного ключа службы) статьи [Deploy StorSimple Virtual Array — Prepare the Azure portal](storsimple-virtual-array-deploy1-portal-prep.md#step-2-get-the-service-registration-key) (Развертывание виртуального массива StorSimple. Подготовка портала Azure).
   ...
   3. Щелкните **Зарегистрировать**. Это приведет к перезапуску устройства. Может потребоваться подождать 2–3 минуты, прежде чем устройство будет зарегистрировано. После перезапуска устройства откроется страница входа.
      ...
      ![Зарегистрировать устройство](./media/storsimple-virtual-array-deploy3-iscsi-setup/image11.png)
...
12. В колонке **Устройства** убедитесь, что устройство успешно подключено к службе, посмотрев на его состояние. Состояние устройства должно быть такое: **Готово к настройке**.
    ...
    ![Зарегистрировать устройство](./media/storsimple-virtual-array-deploy3-iscsi-setup/deployis1m.png)
...
## <a name="step-3-add-a-volume"></a>Шаг 3. Добавить том
...
1. В колонке **Устройства** выберите устройство, которое вы только что настроили в качестве сервера iSCSI. Щелкните **…** (или щелкните правой кнопкой мыши в этой строке). Затем в контекстном меню выберите **Добавить том**. Можно также щелкнуть **+ Добавить том** на панели команд. Откроется колонка **Добавить том**.
    ...
    ![Добавить том](./media/storsimple-virtual-array-deploy3-iscsi-setup/deployis10m.png)
...
3. Чтобы добавить новый подключенный узел, щелкните **Добавить**, введите обычное имя и полное имя iSCSI узла, а затем нажмите кнопку **Добавить**. Если у вас нет IQN, перейдите к [ Приложение А. Получение IQN узла Windows Server](#appendix-a-get-the-iqn-of-a-windows-server-host).
   
      ![Добавить том](./media/storsimple-virtual-array-deploy3-iscsi-setup/deployis15m.png)
4. После завершения настройки тома нажмите кнопку **ОК**. Будет создан том с указанными настройками. Вы увидите соответствующее уведомление. По умолчанию для тома включены наблюдение и резервное копирование.
     ...
     ![Добавить том](./media/storsimple-virtual-array-deploy3-iscsi-setup/deployis18m.png)
5. Чтобы убедиться, что том был создан, перейдите к колонке **Тома**. Том должен отображаться в списке.
   
   ![Добавить том](./media/storsimple-virtual-array-deploy3-iscsi-setup/deployis20m.png)
...
5. Выберите целевое устройство и нажмите кнопку **Подключить**. После подключения устройства состояние должно измениться на **Подключено**. (Дополнительные сведения об использовании инициатора iSCSI (Майкрософт) см. в статье [Установка и настройка инициатора Microsoft iSCSI][1].
...
## <a name="appendix-a-get-the-iqn-of-a-windows-server-host"></a>Приложение А. Получение IQN узла Windows Server
