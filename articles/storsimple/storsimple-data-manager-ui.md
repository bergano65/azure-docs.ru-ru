---
title: UI-менеджера данных Microsoft Azure StorSimple
...
В этой статье объясняется, как применять пользовательский интерфейс диспетчера данных StorSimple для преобразования данных, хранящихся на устройствах StorSimple серии 8000. Преобразованные данные могут быть использованы другими службами Azure, такими как службы мультимедиа Azure, Azure HDInsight, Машинное обучение Azure и Когнитивный поиск Azure.
...
3. Для новой службы укажите следующее:
   ...
   2. Выберите **подписку** из списка выпадающих данных. Подписка привязана к учетной записи для выставления счетов. Это поле будет автоматически заполнено (без возможности выбора) при наличии только одной подписки.
...
2. Нажмите **и определение вакансии**.
...
7. Из раскрывающегося списка для поля **Репозиторий данных** выберите созданный вами репозиторий данных. 
   ...
   4. Служба преобразования данных работает только на последнем снимке данных, которые перемещаются в Azure.
...
### <a name="view-logs-after-job-completion"></a>Просмотр журналов после завершения выполнения задания

После завершения задания можно просмотреть состояние задания. Статус задания может быть **успешным,** **частично успешно и** не **удалось**. Можно просмотреть список файлов, которые были успешно скопированы, и файлы, которые не были скопированы. Эти списки доступны в контейнере под названием **"Storsimple-data-manager-joblogs"** в вашей учетной записи целевого хранилища. В этом контейнере можно найти папку с тем же именем, что и определение работы. В рамках этого будет создана папка для каждого выполнения задания, которая будет содержать ваши списки. Название этой папки будет GUID задания, которое вы можете получить со страницы данных о работе. Кроме того, в большинстве случаев вы увидите ссылку на журналы копий на самой странице заданий.
Есть 2 набора файлов csv, которые вы увидите в этой папке. Все файлы, которые начинаются с **copiedfilelist ...** Все файлы, которые начинаются с **неудавшегося файла...,** содержат файлы, которые не были в состоянии быть скопированы, наряду с сообщением об ошибке.
