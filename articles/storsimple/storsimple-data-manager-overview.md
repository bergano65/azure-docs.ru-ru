...
Примите во внимание следующие рекомендации.
 ...
 - Подключите определение задания и диспетчер данных к региону, содержащему учетную запись хранения StorSimple. Если это невозможно, подключите диспетчер данных к ближайшему региону Azure, а затем создайте определение задания в том же регионе, что и учетная запись хранения StorSimple. 
    ...
    Если ваша учетная запись хранения SorSimple не находится в 26 регионах, поддерживающих создание определения рабочих мест, мы рекомендуем вам не запускать StorSimple Data Manager, поскольку вы видите длинные запоздалые и потенциальные расходы на выход.
    
Корпорация Майкрософт стремится обеспечить доступность служб Azure во всех регионах. Однако в определенном регионе могут возникать незапланированные перебои в обслуживании в течение коротких периодов времени. В таких случаях можно создать вопрос об менеджере данных и определении задания в регионе, который не подвержен проблеме сбой, и запустить задание преобразования. В таком сценарии может возникнуть дополнительная задержка, но это может быть вашей стратегией восстановления в редких случаях регионального сбоя.
