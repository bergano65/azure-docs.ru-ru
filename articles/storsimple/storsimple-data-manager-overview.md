...
description: Узнайте о решении Диспетчер данных StorSimple и о том, как можно использовать эту службу для создания приложений, использующих данные StorSimple и другие службы Azure.
services: storsimple
...
Примите во внимание следующие рекомендации.
 ...
 - Подключите определение задания и диспетчер данных к региону, содержащему учетную запись хранения StorSimple. Если это невозможно, подключите диспетчер данных к ближайшему региону Azure, а затем создайте определение задания в том же регионе, что и учетная запись хранения StorSimple. 
    ...
    Если ваша учетная запись хранения StorSimple не находится в 26 регионах, поддерживающих создание определения задания, не рекомендуется запускать Диспетчер данных StorSimple, так как вы видите долгие задержки и возможный расход на исходящий трафик.
    
Корпорация Майкрософт стремится обеспечить постоянную доступность служб Azure во всех регионах. Однако незапланированные сбои службы могут возникать в течение коротких периодов в определенном регионе. В таких случаях можно отобразить Диспетчер данных и определение задания в регионе, на котором не влияет сбой, и запустить задание преобразования. В таком сценарии может возникать дополнительная задержка, но это может быть стратегия восстановления в редких случаях региональной простоя.
...
Диспетчер данных StorSimple не собирает и не отображает никаких персональных данных. Дополнительные сведения см. в политике конфиденциальности Майкрософт в [центре управления безопасностью](https://www.microsoft.com/trustcenter).

## <a name="known-limitations"></a>Известные ограничения

Для службы действуют следующие ограничения.
- Диспетчер данных StorSimple пока не поддерживает тома, зашифрованные BitLocker. При попытке запустить службу с зашифрованным диском будут возникать сбои заданий.
- Некоторые метаданные файлов (включая списки управления доступом) не будут сохраняться в преобразованных данных.
- Эта служба поддерживает только тома NTFS.
- Значения длины пути к файлам должны быть меньше 256 символов. В противном случае задание завершится ошибкой.
