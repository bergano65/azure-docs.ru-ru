...
1. На классическом портале Azure выберите элементы **Параметры > Отслеживание > Работоспособность оборудования**. Проверьте состояние компонентов PCM под **общими компонентами,** чтобы определить, какой PCM не удалось:
