...
* **Повышенная устойчивость к сбоям в облаке**: Этот выпуск содержит несколько исправлений, касающихся аварийного восстановления, резервного копирования, восстановления и распределение по уровням в случае сбоя подключения облака. 
* **Улучшенная производительность восстановления**: Этот выпуск содержит исправления ошибок, которые значительно сократили время выполнения задания восстановления.
...
* **Новые образы виртуальных дисков**: Новый VHD, VHDX- и VMDK теперь доступны с помощью классического портала Azure. Эти образы можно скачать для подготовки новых устройств с Обновлением 0.1.
* **Повышение точности отображения сведений о состоянии задания на портале**: В более ранней версии программного обеспечения состояние задания на портале отображалось менее детально. В этом выпуске эта проблема устранена.
* **Присоединение к домену**: Исправления ошибок, связанных с присоединение к домену и переименованием устройства.
...
| 3 |Пакет поддержки |В более ранней версии программного обеспечения коллекция вспомогательных пакетов выдавала ошибку, если размеры пакетов превышали 1 ГБ. В этом обновлении эта проблема исправлена. |
...
| **6.** |Многоуровневые тома или общие папки |Блокировка диапазона байтов для приложений, работающих с многоуровневыми томами StorSimple, не поддерживается. Если блокировка диапазона байтов включена, распределение по уровням StorSimple не будет работать. |Рекомендованные меры включают следующие: <br></br>Отключите блокировку диапазона байтов в логике приложения.<br></br>Выберите помещение данных для этого приложения в локально закрепленные тома, а не в многоуровневые тома.<br></br>*Предупреждение*. При использовании локально закрепленных томов и блокировка диапазона байтов включена, имейте в виду, что локально закрепленного тома может быть online, еще до завершения восстановления. В таких случаях нужно дождаться завершения восстановления. |
