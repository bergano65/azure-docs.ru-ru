---
title: Failover и аварийного восстановления для StorSimple Виртуальный массив
...
Аварийное восстановление организовано с помощью функции сбоя устройства и инициируется из лезвия **устройств.** В этой колонке представлены все устройства StorSimple, подключенные к службе диспетчера устройств StorSimple. Для каждого устройства отображается понятное имя, состояние устройства, выделенная и максимальная емкость, тип и модель.
...
5. В колонке **Отключение** вы увидите запрос подтверждения. Деактивация устройства является *постоянным* процессом, который не может быть отменен. Также отобразится напоминание о том, что нужно перевести общие папки или тома на узле в автономный режим. Введите для подтверждения имя устройства и щелкните **Отключить**.
...
10. В колонке **Устройство для отработки отказа** выполните следующие действия:
    ...
    2. Из списка выпадающих устройств выберите **целевое устройство.** В этом списке отображаются только устройства с достаточной емкостью.

    3. Установите флажок **Я понимаю, что эта операция приведет к отработке отказа с переносом данных на целевое устройство**. 

    4. Нажмите **Fail более**.
...
Если в период от запуска до завершения аварийного восстановления устройства будет прервано подключение к облаку, произойдет сбой. Вы получаете уведомление о сбое. Устройство, которое использовалось как целевое для аварийного восстановления, будет помечено как *непригодное для использования*. Его нельзя будет использовать для аварийного восстановления в дальнейшем.
