...
| --- | --- | --- | --- | --- |
| 1 |Производительность узлов |В более ранней версии проблемы с производительностью на стороне узла возникали во время создания локально закрепленного тома и во время преобразования многоуровневого тома в локально закрепленный. В этом выпуске эти проблемы исправлены, благодаря чему улучшается производительность узлов во время процедур создания и преобразования томов. |Да |Нет |
| 2 |Локально закрепленные тома |В редких случаях создание локально закрепленного тома приводило к сбою системы. В этом обновлении эта ошибка устранена. |Да |Нет |
| 3 |Распределение по уровням |При перемещении метаданных для устройств StorSimple Cloud Appliance (8010 и 8020) на уровни в облаке ранее возникали случайные сбои. В этом обновлении эта проблема исправлена. |Нет |Да |
| 4 |Создание моментального снимка |Ранее возникали проблемы, связанные с созданием добавочных моментальных снимков в сценариях с большими томами и минимальным обновлением данных или без него. В этом обновлении эти проблемы исправлены. |Да |Да |
| 5 |Проверка подлинности Openstack |При использовании Openstack в качестве поставщика облачных служб у пользователей иногда возникала ошибка, связанная с проверкой подлинности, при которой средство синтаксического анализа JSON приводило к сбою. В этом обновлении эта ошибка устранена. |Да |Нет |
| 6 |Копирование на стороне узла |В более ранних версиях ПО при копировании данных с одного тома на другой иногда возникала ошибка, связанная с временем ODX. Это могло привести к отработке отказа контроллера, и система потенциально могла перейти в режим восстановления. В этом обновлении эта ошибка устранена. |Да |Нет |
| 7 |Инструментарий управления Windows (WMI) |В предыдущих версиях ПО наблюдались несколько случаев сбоя веб-прокси с исключением "Сбой загрузки поставщика \<ManagementException>". Эта ошибка была связана с проблемой доступа к памяти WMI. В этом выпуске она исправлена. |Да |Нет |
| 8 |Update |В редких случаях в предыдущих версиях ПО пользователь получал сообщение "CisPowershellHcsscripterror" при попытке проверки или установки обновлений. В этом обновлении эта проблема исправлена. |Да |Да |
| 9 |Вспомогательный пакет |В этом выпуске улучшен способ сбора и передачи пакета поддержки. |Да |Да |
...
| 15 |Командлеты Azure PowerShell и локально закрепленные тома |Создать локально закрепленный том с помощью командлетов Azure PowerShell невозможно. (Любой том, который создан с помощью Azure PowerShell, будет многоуровневым.) |Всегда используйте службу StorSimple Manager для настройки локально закрепленных томов. |Да |Нет |
...
Дополнительные сведения о том, как установить обновления драйвера, Storport, Spaceport и встроенного ПО диска, см. в статье [Установка обновления 2.2 на устройство StorSimple](./storsimple-8000-install-update-5.md).
...
Узнайте, как [установить обновление 2.2](./storsimple-8000-install-update-5.md) на устройство StorSimple.