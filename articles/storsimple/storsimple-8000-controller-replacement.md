...
При замене одного контроллера сначала необходимо снять неисправный контроллер. (Оставшимся контроллером в устройстве является активный контроллер.) При вставке контроллера замены возникают следующие действия:
...
5. Чтобы убедиться, что замена успешна, на портале Azure перейдите на устройство, а затем перейдите к **мониторингу** > **работоспособности оборудования**и убедитесь, что контроллер 0 и контроллер 1 здоровы (статус зеленый).
...
На портале Azure перейдите на устройство, а затем **пройдите** > **Hardware health**в разделе **Контроллеры.** Здесь можно проверить, какой контроллер активен.
