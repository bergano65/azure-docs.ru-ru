...
| Порт № <sup>1, 2</sup> | Входящий или исходящий | Область порта | Обязательное значение | Примечания |
| --- | --- | --- | --- | --- |
| TCP 80 (HTTP)<sup>3</sup> |Выходной |WAN |Нет |<ul><li>Исходящий порт используется для доступа к Интернету и извлечения обновлений.</li><li>Пользователи могут настраивать исходящий веб-прокси.</li><li>Чтобы разрешить выполнение обновлений системы, этот порт также должен быть открыт для фиксированных IP-адресов контроллера.</li></ul> |
| TCP 443 (HTTPS)<sup>3</sup> |Выходной |Глобальная сеть |Да |<ul><li>Исходящий порт используется для доступа к данным в облаке.</li><li>Пользователи могут настраивать исходящий веб-прокси.</li><li>Чтобы разрешить выполнение обновлений системы, этот порт также должен быть открыт для фиксированных IP-адресов контроллера.</li><li>Этот порт также используется обоими контроллерами для сборки мусора.</li></ul> |
| UDP 53 (DNS) |Выходной |Глобальная сеть |В некоторых случаях. См. примечания. |Этот порт требуется, только если вы используете веб-DNS-сервер. |
| UDP 123 (NTP) |Выходной |Глобальная сеть |В некоторых случаях. См. примечания. |Этот порт требуется, только если вы используете веб-NTP-сервер. |
| TCP 9354 |Выходной |Глобальная сеть |Да |Исходящий порт используется устройством StorSimple для взаимодействия со службой диспетчера устройств StorSimple. |
| 3260 (iSCSI) |In |Локальная сеть |Нет |Этот порт используется для доступа к данным по iSCSI. |
| 5985 |In |Локальная сеть |Нет |Диспетчер моментальных снимков StorSimple использует входящий порт для взаимодействия с устройством StorSimple.<br>Этот порт также используется при удаленном подключении к Windows PowerShell для StorSimple по протоколу HTTP. |
| 5986 |In |Локальная сеть |Нет |Этот порт используется, когда вы удаленно подключаетесь к Windows PowerShell для StorSimple по протоколу HTTPS. |
...
| --- | --- | --- |
| `https://*.storsimple.windowsazure.com/*`<br>`https://*.accesscontrol.windows.net/*`<br>`https://*.servicebus.windows.net/*`<br>`https://login.windows.net` |Служба диспетчера устройств StorSimple<br>Служба управления доступом<br>Служебная шина Azure<br>Служба проверки подлинности |Сетевые интерфейсы с поддержкой облака |
...
| `https://*.partners.extranet.microsoft.com/*`<br>`https://dcupload.microsoft.com/`<br>`https://*.support.microsoft.com/` |Пакет поддержки |Сетевые интерфейсы с поддержкой облака |

#### <a name="url-patterns-for-azure-government-portal"></a>Шаблоны URL-адресов для портала Azure для государственных организаций

| Шаблон URL-адреса | Компонент или функция | IP-адреса устройств |
| --- | --- | --- |
| `https://*.storsimple.windowsazure.us/*`<br>`https://*.accesscontrol.usgovcloudapi.net/*`<br>`https://*.servicebus.usgovcloudapi.net/*`<br>`https://login.microsoftonline.us` |Служба диспетчера устройств StorSimple<br>Служба управления доступом<br>Служебная шина Azure<br>Служба проверки подлинности |Сетевые интерфейсы с поддержкой облака |
...
* С точки зрения повторных попыток iSCSI имеет приоритет над облаком.
    ...
    Рассмотрим следующий пример. На устройстве StorSimple включены два сетевых интерфейса: Data 0 и Data 1. Для интерфейса "Данные 0" включена поддержка облака, а для интерфейса "Данные 1" включена поддержка как облака, так и iSCSI. Никакие другие сетевые интерфейсы на этом устройстве не имеют включенной поддержки облака или iSCSI.
...
* Настройте StorSimple с [двумя узлами конфигурации кластера файлового сервера][1]. При удалении одиночных точек сбоя и создании избыточности на стороне узла все решение становится высокодоступным.
