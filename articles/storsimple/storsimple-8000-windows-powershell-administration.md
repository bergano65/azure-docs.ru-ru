...
1. **Войти в систему с полным доступом**: позволяет подключиться к пространству выполнения **SSAdminConsole** на локальном контроллере (если указаны соответствующие учетные данные). (Локальный контроллер — это контроллер, к который вы в настоящее время получаете доступ через серийную консоль вашего устройства StorSimple.) Эта опция также может быть использована для того, чтобы позволить службе Майкрософт получить доступ к неограниченному запущенной области (сессии поддержки) для устранения любых возможных проблем с устройством. Дополнительные сведения см. в статье [Запуск сеанса поддержки](storsimple-8000-contact-microsoft-support.md#start-a-support-session-in-windows-powershell-for-storsimple).
...
Для подключения к устройству StorSimple можно использовать удаленное взаимодействие Windows PowerShell. При таком способе подключения меню не отображается. (Меню появляется только при подключении к устройству с помощью последовательной консоли. Подключение удаленно перенесет вас непосредственно к эквиваленту "вариант 1 - полный доступ" на серийной консоли.) С помощью реметики Windows PowerShell вы подключаетесь к определенному запущенной области. Также можно указать язык интерфейса.
...
> [!NOTE]
> При работе с виртуальными узлами Microsoft Azure и облачными устройствами StorSimple можно использовать удаленное взаимодействие Windows PowerShell и виртуальный узел для подключения к облачному устройству. Если вы настроили месторасположения общего на хосте, на котором можно сохранить информацию от сеанса Windows PowerShell, вы должны знать, что _принцип «Все»_ включает в себя только проверенных пользователей. Поэтому, если вы настроили долю, чтобы разрешить доступ _всем,_ и вы подключитесь без указания учетных данных, будет использован неустановленный принцип Anonymous, и вы увидите ошибку. Чтобы решить эту проблему, активируйте в общем ресурсе гостевую учетную запись и предоставьте ей полный доступ к общему ресурсу либо укажите действительные учетные данные вместе с соответствующим командлетом Windows PowerShell.
...
Получить справку в этом интерфейсе можно точно так же, как в Windows PowerShell; большинство командлетов справки здесь работают. Вы можете найти Справку для Windows PowerShell онлайн: [Microsoft.PowerShell.Core](/powershell/module/Microsoft.PowerShell.Core/).
