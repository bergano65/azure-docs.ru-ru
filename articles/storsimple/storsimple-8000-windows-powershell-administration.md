---
title: PowerShell для управления устройствами StorSimple
...
4. В области **Категория** выберите **Преобразование**.
5. В области списка **Удаленная кодировка** выберите **UTF-8**.
6. В разделе **Обработка символов чертежей** выберите **Использовать точки кода чертежей Unicode**. Правильный выбор параметров PuTTY показан на снимке экрана ниже.
...
> [!IMPORTANT]
> Текст заголовка можно использовать для определения, является ли подключенное устройство _Активным_ или _Пассивным_.
...
1. **Войти в систему с полным доступом**: позволяет подключиться к пространству выполнения **SSAdminConsole** на локальном контроллере (если указаны соответствующие учетные данные). (Локальный контроллер — это контроллер, к которому в данный момент осуществляется доступ через последовательную консоль устройства StorSimple). Этот параметр также можно использовать, чтобы разрешить служба поддержки Майкрософт доступ к неограниченному пространству выполнения (сеанс поддержки), чтобы устранить возможные проблемы с устройством. Дополнительные сведения см. в статье [Запуск сеанса поддержки](storsimple-8000-contact-microsoft-support.md#start-a-support-session-in-windows-powershell-for-storsimple).
...
Для подключения к устройству StorSimple можно использовать удаленное взаимодействие Windows PowerShell. При таком способе подключения меню не отображается. (Меню появляется только при подключении к устройству с помощью последовательной консоли. Удаленное подключение переводит вас непосредственно к эквиваленту "вариант 1 — полный доступ" в последовательной консоли.) При удаленном взаимодействии Windows PowerShell вы подключаетесь к определенному пространству выполнения. Также можно указать язык интерфейса.

Язык интерфейса устройства независим от языка, заданного с использованием параметра **Изменить язык** в меню последовательной консоли. Если язык не задан, удаленный сеанс PowerShell автоматически выберет языковой стандарт устройства, к которому вы подключаетесь.

> [!NOTE]
> При работе с виртуальными узлами Microsoft Azure и облачными устройствами StorSimple можно использовать удаленное взаимодействие Windows PowerShell и виртуальный узел для подключения к облачному устройству. Если вы настроили общую папку на узле, на котором будет храниться информация из сеанса Windows PowerShell, следует иметь в виду, что субъект _Everyone_ включает только пользователей, прошедших проверку подлинности. Таким образом, если вы настроили общий ресурс для предоставления доступа _всем пользователям_ и подключаетесь без указания учетных данных, будет использоваться анонимный субъект, не прошедший проверку подлинности, и появится сообщение об ошибке. Чтобы решить эту проблему, активируйте в общем ресурсе гостевую учетную запись и предоставьте ей полный доступ к общему ресурсу либо укажите действительные учетные данные вместе с соответствующим командлетом Windows PowerShell.
...
Получить справку в этом интерфейсе можно точно так же, как в Windows PowerShell; большинство командлетов справки здесь работают. Справку по Windows PowerShell Online можно найти в [Microsoft. PowerShell. Core](/powershell/module/Microsoft.PowerShell.Core/).
...
1. Запустите Windows PowerShell через пункт **Запуск от имени администратора**.
