...
[Пакет Azure SDK для PHP](https://github.com/Azure/azure-sdk-for-php) состоит из нескольких компонентов. В этой статье будут использоваться два из них: Azure PowerShell и Эмуляторы Azure. Эти два компонента можно установить при помощи установщика веб-платформы Майкрософт. Подробнее: [Установка и настройка Azure PowerShell](/powershell/azure/overview).
...
3. ИСПОЛЬЗУЕМЫХ Если среда выполнения PHP использует [драйверы Майкрософт для PHP для SQL Server][sqlsrv drivers], необходимо настроить веб-роль для установки [SQL Server Native Client 2012][sql native client] во время подготовки. Для этого добавьте установщик [sqlncli.msi x64] в папку `bin` в корневом каталоге веб-роли. Скрипт запуска, описанный на следующем шаге, запустит установщик без вмешательства пользователя при подготовке роли. Если среда выполнения PHP не использует драйверы Microsoft для PHP для SQL Server, можно удалить следующую строку из скрипта, показанного на следующем шаге.

        msiexec /i sqlncli.msi /qn IACCEPTSQLNCLILICENSETERMS=YES
4. Определите задачу запуска, которая настраивает [службы IIS (IIS)][iis.net] на использование среды выполнения PHP для обработки запросов к `.php` страницам. Для этого откройте файл`setup_web.cmd` (в файле `bin` корневого каталога веб-роли) в текстовом редакторе и замените его содержимое на следующий скрипт:
...
3. ИСПОЛЬЗУЕМЫХ Если среда выполнения PHP использует [драйверы Майкрософт для PHP для SQL Server][sqlsrv drivers], необходимо настроить рабочую роль для установки [SQL Server Native Client 2012][sql native client] во время подготовки. Для этого добавьте установщик [sqlncli.msi x64] в корневой каталог рабочей роли. Скрипт запуска, описанный на следующем шаге, запустит установщик без вмешательства пользователя при подготовке роли. Если среда выполнения PHP не использует драйверы Microsoft для PHP для SQL Server, можно удалить следующую строку из скрипта, показанного на следующем шаге.
...
[определения службы (CSDEF)]: https://msdn.microsoft.com/library/windowsazure/ee758711.aspx
[конфигурации службы (CSCFG)]: https://msdn.microsoft.com/library/windowsazure/ee758710.aspx
[iis.net]: https://www.iis.net/
[sql native client]: https://docs.microsoft.com/sql/sql-server/sql-server-technical-documentation
[sqlsrv drivers]: https://php.net/sqlsrv
[sqlncli.msi x64]: https://go.microsoft.com/fwlink/?LinkID=239648
