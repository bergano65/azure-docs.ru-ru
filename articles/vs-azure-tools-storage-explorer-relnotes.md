...
* Операции для Blobs, ADLS Gen2 и управляемых дисков используют интегрированную AzCopy. Более конкретно, следующие операции выполняются с помощью AzCopy:
   ...
   * Управляемые диски
   ...
   Кроме того, в интегрированный опыт AzCopy добавлено несколько часто запрашиваемых функций:
...
* Если передача AzCopy содержит пропуски и отсутствие сбоев, исследователь хранилища теперь покажет значок предупреждения, чтобы подчеркнуть, что ошибки произошли. [#1490](https://www.github.com/Microsoft/AzureStorageExplorer/issues/1490)
...
### <a name="download-azure-storage-explorer-170"></a>Скачать Azure Storage Explorer 1.7.0
...
* Если вы используете VS для Mac и когда-либо создавали пользовательскую конфигурацию AAD, вы не сможете выполнить вход. Чтобы обойти эту проблему, удалите содержимое в папке ~/.IdentityService/AadConfigurations. Если это не разблокирует вас, пожалуйста, прокомментируйте [этот вопрос.](https://github.com/Microsoft/AzureStorageExplorer/issues/97)
* В Azurite еще не полностью реализованы все API-интерфейсы хранилища. Из-за этого могут возникнуть непредвиденные ошибки или поведение при использовании Azurite в хранилище разработки.
* В редких случаях фокус дерева мог задержаться на функции быстрого доступа. Разблокировать фокус можно командой "Обновить все".
...
* Отзывы об Обозревателе службы хранилища теперь доступны на GitHub. Вы можете связаться с нашей страницей вопросов, нажав кнопку обратной связи в левом нижнем углу или перейдя к [https://github.com/Microsoft/AzureStorageExplorer/issues](https://github.com/Microsoft/AzureStorageExplorer/issues). Вы можете оставить свои предложения и отзывы, сообщать о проблемах и задавать вопросы в любых других формах.
* Если вы столкнулись с проблемами сертификата TLS/SSL и не можете найти сертификат обидеть, теперь можно запустить Storage Explorer с командной строки с флагом. `--ignore-certificate-errors` При запуске с этим флагом, Storage Explorer будет игнорировать ошибки сертификата TLS/SSL.
...
* Во время передачи большого двоичного объекта редактор для целевого контейнера больших двоичных объектов иногда не обновлялся должным образом. Теперь эта проблема устранена.
* Поддержка нескольких видов строк подключения и универсальных кодов ресурса (URI) SAS возвращена. Мы исправили все известные проблемы, но если возникают какие-либо дополнительные проблемы, отправьте нам отзыв.
* У некоторых пользователей в версии 0.9.0 уведомление об обновлении не работало. Эта проблема была исправлена, и для тех, кто пострадал от ошибки, вы можете вручную скачать последнюю версию Storage Explorer [здесь](https://azure.microsoft.com/features/storage-explorer/)
...
* Исправлено: Open in Portal теперь работает с Azure China 21Vianet, Azure Германия, правительством США Azure и Azure Stack
...
* Теперь вы можете добавить доверенные Базовые-64 закодированные сертификаты X.509 TLS/SSL, перейдя на Edit -&gt; SSL Сертификаты -&gt; Импортные сертификаты
...
* Теперь можно открыть несколько редакторов на разных вкладках. Однокликающе, чтобы открыть временную вкладку; дважды щелкните, чтобы открыть постоянную вкладку. Вы также можете нажать на временную вкладку, чтобы сделать его постоянной вкладкой
