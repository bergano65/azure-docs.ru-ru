...
description: Ознакомьтесь с заметками о выпуске для последней версии Обозреватель службы хранилища Microsoft Azure. Также показаны заметки о выпуске для предыдущих версий.
services: storage
...
* При использовании RBAC Обозреватель службы хранилища требуются разрешения уровня управления для доступа к ресурсам хранилища. Дополнительные сведения см. в разделе [руководство по устранению неполадок](./storage/common/storage-explorer-troubleshooting.md) .
...
* Для запуска Обозреватель службы хранилища в Linux необходимо сначала установить определенные зависимости. Дополнительные сведения см. в разделе [руководство по устранению неполадок](./storage/common/storage-explorer-troubleshooting.md?tabs=1804#linux-dependencies) обозреватель службы хранилища.
...
* Обозреватель службы хранилища теперь имеет пользовательский интерфейс выделенных параметров. Доступ к нему можно получить либо в меню Правка > Параметры, либо щелкнув значок параметров (шестеренки) на левой вертикальной панели инструментов. Эта функция является первым шагом для предоставления разнообразных [параметров, запрошенных пользователем](https://github.com/microsoft/AzureStorageExplorer/labels/%3Abulb%3A%20setting%20candidate). Начиная с этого выпуска поддерживаются следующие параметры:
  ...
  * Включить AzCopy
  * Длительность AzCopy SAS если есть другие параметры, которые вы хотели бы добавить, [откройте вопрос на сайте GitHub](https://github.com/microsoft/AzureStorageExplorer/issues/new?assignees=&labels=%3Abulb%3A%20setting%20candidate&template=feature_request.md&title=) с описанием параметра, который вы хотите просмотреть.
* Обозреватель службы хранилища теперь поддерживает управляемые диски. Вы можете выполнить следующие действия:
...
* При управлении моментальными снимками BLOB-объектов средства чтения с экрана не считывают метку времени, связанную с моментальным снимком. Эта ошибка исправлена. #1202
...
* Обозреватель службы хранилища теперь имеет пользовательский интерфейс выделенных параметров. Доступ к нему можно получить либо в меню Правка > Параметры, либо щелкнув значок параметров (шестеренки) на левой вертикальной панели инструментов. Эта функция является первым шагом для предоставления разнообразных [параметров, запрошенных пользователем](https://github.com/microsoft/AzureStorageExplorer/labels/%3Abulb%3A%20setting%20candidate). Начиная с этого выпуска поддерживаются следующие параметры:
    ...
    * Длительность AzCopy SAS
    ...
    Если есть другие параметры, которые вы хотели бы добавить, [откройте вопрос на сайте GitHub с описанием параметра, который вы хотите увидеть](https://github.com/microsoft/AzureStorageExplorer/issues/new?assignees=&labels=%3Abulb%3A%20setting%20candidate&template=feature_request.md&title=).
* Обозреватель службы хранилища теперь поддерживает управляемые диски. Вы можете выполнить следующие действия:
    * Отправка локального виртуального жесткого диска на новый диск
    * Загрузка диска
    * Копирование и вставка дисков в группах ресурсов и регионах
    ...
    * Создание моментального снимка диска
    ...
    Передача, скачивание и копирование дисков в разных регионах на базе AzCopy V10.
* Теперь Обозреватель службы хранилища можно установить с помощью хранилища прикрепления в Linux. При установке через хранилище привязкой устанавливаются все зависимости, включая .NET Core. Сейчас мы проверили, что Обозреватель службы хранилища хорошо работает на Ubuntu и CentOS. Если возникли проблемы при установке из хранилища прикрепления в другой дистрибутивов Linux, [откройте вопрос на сайте GitHub](https://github.com/microsoft/AzureStorageExplorer/issues/new?assignees=&labels=snaps&template=bug-report.md&title=). Дополнительные сведения об установке из хранилища привязкой см. в нашем разделе [руководства по началу работы](./vs-azure-tools-storage-manage-with-storage-explorer.md). [#68](https://www.github.com/Microsoft/AzureStorageExplorer/issues/68)
...
* При управлении моментальными снимками BLOB-объектов средства чтения с экрана не считывают метку времени, связанную с моментальным снимком. Эта ошибка исправлена. [#1202](https://www.github.com/Microsoft/AzureStorageExplorer/issues/1202)
...
* При использовании RBAC Обозреватель службы хранилища требуются разрешения уровня управления для доступа к ресурсам хранилища. Дополнительные сведения см. в разделе [руководство по устранению неполадок](./storage/common/storage-explorer-troubleshooting.md) .
* Отключение от ресурса, подключенного через URI SAS, например контейнера больших двоичных объектов, может привести к ошибке, которая предотвращает правильное отображение других подключенных объектов. Чтобы обойти эту проблему, просто обновите узел группы. Для получения дополнительных сведений ознакомьтесь с исправлением № 537.
* Если вы используете VS для Mac и когда-либо создали пользовательскую конфигурацию AAD, вы не сможете войти в систему. Чтобы обойти эту проблему, удалите содержимое в папке ~/.IdentityService/AadConfigurations. Если это не разблокирует вашу учетную запись, оставьте комментарий об этой проблеме.
* В Azurite еще не полностью реализованы все API-интерфейсы хранилища. Из-за этого могут возникнуть непредвиденные ошибки или поведение при использовании Azurite в хранилище разработки.
* В редких случаях фокус дерева мог задержаться на функции быстрого доступа. Разблокировать фокус можно командой "Обновить все".
* Загрузка из папки OneDrive не работает из-за ошибки в NodeJS. Ошибка была исправлена, но еще не интегрирована в Electron. Чтобы обойти эту проблему при передаче и скачивании данных с использованием контейнера больших двоичных объектов, можно использовать экспериментальную функцию AzCopy.
* При нацеливании на Azure Stack передача определенных файлов в качестве добавляемых больших двоичных объектов может завершиться ошибкой.
...
### <a name="hotfixes"></a>Исправления
* Выполнен откат версии API управления ресурсами Azure для разблокирования пользователей Azure для US Gov организаций. [696](https://github.com/Microsoft/AzureStorageExplorer/issues/696)
...
* Обозреватель службы хранилища теперь поддерживает обратимое удаление. Вы можете выполнить следующие действия:
...
* Если вы используете VS для Mac и когда-либо создавали пользовательскую конфигурацию AAD, вы не сможете выполнить вход. Чтобы обойти эту проблему, удалите содержимое в папке ~/.IdentityService/AadConfigurations. Если этот параметр не разблокируется, добавьте комментарий к [этой ошибке](https://github.com/Microsoft/AzureStorageExplorer/issues/97).
* В Azurite еще не полностью реализованы все API-интерфейсы хранилища. Из-за этого могут возникнуть непредвиденные ошибки или поведение при использовании Azurite в хранилище разработки.
* В редких случаях фокус дерева мог задержаться на функции быстрого доступа. Разблокировать фокус можно командой "Обновить все".
...
* В Обозревателе службы хранилища теперь поддерживаются уровни доступа учетных записей хранения только для больших двоичных объектов и GPV2. Дополнительные сведения об уровнях доступа см. [здесь](./storage/blobs/storage-blob-storage-tiers.md).
* Время начала действия больше не требуется при создании SAS.

### <a name="fixes"></a>Исправления
* Получение подписок для учетных записей US Gov организаций было нарушено. Теперь эта проблема устранена. [#61](https://github.com/Microsoft/AzureStorageExplorer/issues/61)
...
* Отзывы об Обозревателе службы хранилища теперь доступны на GitHub. Вы можете перейти на нашу страницу проблем, нажав кнопку с отзывом в левом нижнем углу или перейдя по адресу [https://github.com/Microsoft/AzureStorageExplorer/issues](https://github.com/Microsoft/AzureStorageExplorer/issues) . Вы можете оставить свои предложения и отзывы, сообщать о проблемах и задавать вопросы в любых других формах.
* Если вы используете проблемы с сертификатами TLS/SSL и не можете найти сертификат, вызывающий ошибку, теперь можно запустить Обозреватель службы хранилища из командной строки с `--ignore-certificate-errors` флагом. При запуске с этим флагом Обозреватель службы хранилища пропускает ошибки сертификатов TLS/SSL.
* Теперь в контекстном меню есть пункт "Скачать" для таких элементов, как большие двоичные объекты и файлы.
* Улучшена доступность и поддержка программ чтения с экрана. Если вам необходимы специальные возможности, см. нашу [документацию по специальным возможностям](./vs-azure-tools-storage-explorer-accessibility.md) для получения дополнительных сведений.
...
* Домен конечных точек для Azure для US Gov организаций был неправильным. Эта ошибка исправлена.
...
* Поддержка предварительной версии для Azure Cosmos DB:
    * [Интерактивная документация](./cosmos-db/storage-explorer.md)
...
* Обозреватель хранилищ стал более удобным. Доступные улучшения:
...
* Во время передачи большого двоичного объекта редактор для целевого контейнера больших двоичных объектов иногда не обновлялся должным образом. Теперь эта проблема устранена.
* Поддержка нескольких видов строк подключения и универсальных кодов ресурса (URI) SAS возвращена. Мы исправили все известные проблемы, но если возникают какие-либо дополнительные проблемы, отправьте нам отзыв.
* У некоторых пользователей в версии 0.9.0 уведомление об обновлении не работало. Эта проблема исправлена, и для тех, которые затронули ошибку, вы можете вручную скачать последнюю версию Обозреватель службы хранилища [здесь](https://azure.microsoft.com/features/storage-explorer/)
...
* Теперь вы можете добавить доверенные сертификаты X. 509 TLS/SSL с кодировкой Base-64, перейдя в Edit- &gt; SSL Certificates- &gt; Import Certificates.
...
* Вы можете войти в учетные записи Azure для Китая (21Vianet), Германии и US Gov организаций.
