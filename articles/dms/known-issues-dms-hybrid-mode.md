...
3. В разделе certmgr для сертификатов локального компьютера предоставьте разрешения закрытого ключа для новой учетной записи для **ключа гибридного приложения DMS** и **пары ключей обработчика сценариев DMS** .
    ...
    б. Щелкните правой кнопкой мыши запись **ключа гибридного приложения DMS** , наведите указатель на пункт **все задачи**, а затем выберите **Управление закрытыми ключами**.

    в. На вкладке **Безопасность** выберите **Добавить**, а затем введите имя учетной записи.

    . Используйте те же действия для предоставления разрешения на закрытый ключ для новой учетной записи в сертификат **пары ключей механизма сценария DMS** .
