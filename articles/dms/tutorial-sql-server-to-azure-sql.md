---
title: Руководство. Миграция SQL Server в автономный режим в отдельную базу данных SQL
...
В этой статье описывается автономная миграция с SQL Server в базу данных в базе данных SQL Azure. [Перенос данных из SQL Server в Базу данных SQL Azure с помощью DMS в онлайн-режиме](tutorial-sql-server-azure-sql-online.md).
...
- Создайте базу данных в базе данных SQL Azure, выполнив действия, описанные в статье [Создание базы данных в базе данных SQL Azure с помощью портал Azure](https://docs.microsoft.com/azure/sql-database/sql-database-single-database-get-started).
...
- Создайте [правило брандмауэра](https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure) IP на уровне сервера для базы данных SQL Azure, чтобы разрешить Azure Database Migration Service доступ к целевым базам данных. Укажите диапазон подсети виртуальной сети, используемый для Azure Database Migration Service.
...
- Убедитесь, что учетные данные, используемые для подключения к целевому экземпляру базы данных SQL Azure, имеют разрешение CONTROL DATABASE для целевых баз данных.

## <a name="assess-your-on-premises-database"></a>Оценка локальной базы данных

Перед переносом данных из экземпляра SQL Server в отдельную базу данных или базу данных в составе пула в базе данных SQL Azure необходимо оценить SQL Server базу данных на наличие проблем с блокировкой, которые могут препятствовать миграции. В Помощнике по миграции данных версии 3.3 или более поздней выполните инструкции по [оценке миграции SQL Server](https://docs.microsoft.com/sql/dma/dma-assesssqlonprem), чтобы завершить оценку локальной базы данных. Ниже приведена сводка необходимых действий:
...
5. В диалоговом окне **Add sources** (Добавление источников) выберите **AdventureWorks2012**, а затем щелкните **Add** (Добавить) и выберите **Start Assessment** (Начать оценку).
    ...
    Для баз данных в базе данных SQL Azure оценки обозначают проблемы с контролем четности компонентов и проблемы с блокировкой миграции для развертывания в отдельной базе данных или в составе пула.
...
> [!NOTE]
> Перед созданием проекта миграции в Помощник по миграции данных убедитесь, что в Azure уже подготовлена база данных, как указано в предварительных требованиях. В рамках этого руководства для базы данных SQL Azure используется имя **AdventureWorksAzure**, но вы можете назвать ее иначе.
...
3. Найдите миграцию и выберите **зарегистрировать** для **Microsoft. Migration**.
...
1. В меню портал Azure или на **домашней** странице выберите **создать ресурс**. Найдите и выберите **Azure Database Migration Service**.
...
2. На экране **Azure Database Migration Services** выберите созданный экземпляр Azure Database Migration Service.
...
1. Нажмите кнопку **сохранить**, а затем на экране **сведения о целевом объекте миграции** укажите сведения о подключении для целевой базы данных SQL Azure, которая является предварительно подготовленной базой данных SQL Azure, для которой была развернута схема **AdventureWorks2012** , с помощью помощник по миграции данных.
...
3. На экране **Выбрать таблицы** выберите **Сохранить**, разверните список таблиц и просмотрите список затронутых полей.
    ...
    Azure Database Migration Service автоматически выбирает все пустые исходные таблицы, существующие в целевом экземпляре базы данных SQL Azure. Чтобы повторно перенести таблицы, уже содержащие данные, необходимо явным образом выбрать их в этой колонке.

    ![Выбор таблиц](media/tutorial-sql-server-to-azure-sql/dms-configure-setting-activity2.png)

4. Нажмите кнопку **сохранить**, на экране **Сводка по миграции** в текстовом поле **имя действия** укажите имя для действия миграции.
...
- Выберите **Запустить миграцию**.
    ...
    Откроется окно действие миграции, в котором будет находиться **состояние** действия **Ожидание**.
...
3. Проверьте целевые базы данных в целевой базе данных SQL Azure.
