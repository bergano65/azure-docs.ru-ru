---
title: "Учебник: Перемотайте S'L Server в автономный режим в единую базу данных S'L"
titleSuffix: Azure Database Migration Service
description: Узнайте, как выполнять миграцию с локального SQL Server в отдельную базу данных или базу данных в пуле в автономной Базе данных SQL Azure с помощью Azure Database Migration Service.
services: dms
author: HJToland3
ms.author: jtoland
manager: craigg
...
ms.locfileid: "80298915"
---
# <a name="tutorial-migrate-sql-server-to-a-single-database-or-pooled-database-in-azure-sql-database-offline-using-dms"></a>Учебник: Переносите сервер S'L в единую базу данных или объединенную базу данных в базе данных Azure S'L в автономном режиме с помощью DMS
...
В этой статье описывается автономный перенос данных из SQL Server в отдельную базу данных или базу данных в пуле в Базе данных SQL Azure. [Перенос данных из SQL Server в Базу данных SQL Azure с помощью DMS в онлайн-режиме](tutorial-sql-server-azure-sql-online.md).
...
- Создайте [правило брандмауэра](https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure) для IP-адресов на уровне сервера для сервера Базы данных SQL Azure, чтобы предоставить службе Azure Database Migration Service доступ к целевым базам данных. Предоставьте диапазон подсетей виртуальной сети, используемой для миграционной службы базы данных Azure.
...
5. В диалоговом окне **Add sources** (Добавление источников) выберите **AdventureWorks2012**, а затем щелкните **Add** (Добавить) и выберите **Start Assessment** (Начать оценку).
    ...
    Для отдельных баз данных или баз данных в пуле в Базе данных SQL Azure оценка идентифицирует проблемы с равенством функций и проблемы, блокирующие миграцию, для развертывания в одной из таких баз данных.
...
3. Поиск миграции, а затем выберите **Регистрация** для **Microsoft.DataMigration**.
...
1. В меню портала Azure или на **главной** странице выберите **«Создайте ресурс».** Поиск и выберите **миграционную службу базы данных Azure.**
...
1. В меню портала Azure выберите **все сервисы.** Поиск и выбор **миграционных служб базы данных Azure.**

     ![Поиск всех экземпляров Azure Database Migration Service](media/tutorial-sql-server-to-azure-sql/dms-search.png)

2. На экране **миграционных служб базы данных Azure** выберите созданную службу миграции базы данных Azure.
...
3. На экране **Выбрать таблицы** выберите **Сохранить**, разверните список таблиц и просмотрите список затронутых полей.
    ...
    Служба миграции базы данных Azure выбирает все пустые таблицы исходных данных, которые существуют в экземпляре целевой базы данных Azure S'L. Чтобы повторно перенести таблицы, уже содержащие данные, необходимо явным образом выбрать их в этой колонке.

    ![Выбор таблиц](media/tutorial-sql-server-to-azure-sql/dms-configure-setting-activity2.png)

4. Выберите **Сохранить**, на экране **резюме миграции,** в текстовом поле **имя действия,** указать имя для миграции деятельности.
...
- Выберите **Запустить миграцию**.
    ...
    Появляется окно миграционной активности, а **состояние** действия **находится в ожидании**.
