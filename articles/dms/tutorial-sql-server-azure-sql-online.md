---
title: "Учебник: Перемотайте сервер S'L в режиме онлайн в единую базу данных"
...
ms.locfileid: "80298970"
---
# <a name="tutorial-migrate-sql-server-to-a-single-database-or-pooled-database-in-azure-sql-database-online-using-dms"></a>Учебник: Переносите сервер S'L в единую базу данных или объединенную базу данных в базе данных Azure S'L в режиме онлайн с помощью DMS
...
- Убедитесь, что правила виртуальной сетевой группы безопасности не блокируют следующие входящие порты связи в службу миграции базы данных Azure: 443, 53, 9354, 445, 12000. Для получения более подробной информации о фильтрации [Filter network traffic with network security groups](https://docs.microsoft.com/azure/virtual-network/virtual-networks-nsg)трафика трафика Azure Virtual Network NSG см.
...
- Убедитесь, что учетные данные, используемые для подключения к целевому экземпляру базы данных Azure S'L, имеют разрешение CONTROL DATABASE на целевых экземплярах базы данных Azure S'L.
...
5. В диалоговом окне **Add sources** (Добавление источников) выберите **AdventureWorks2012**, а затем щелкните **Add** (Добавить) и выберите **Start Assessment** (Начать оценку).
    ...
    - Категория **паритета функций Сервера S'L** предоставляет полный набор рекомендаций, альтернативные подходы, доступные в Azure, и смягчающие шаги, которые помогут вам спланировать усилия в ваших миграционных проектах.
...
6. Выберите **Далее,** под **подключением к целевому серверу,** укажите детали целевого соединения для базы данных Azure S'L, выберите **Connect,** а затем выберите базу данных **AdventureWorksAzure,** которую вы предварительно зарезервировали в базе данных Azure S'L.
...
5. Выберите существующую виртуальную сеть или создайте новую.
    ...
    Виртуальная сеть предоставляет миграционной службе базы данных Azure доступ к исходной серверу S'L и целевому экземпляру базы данных Azure S'L.
