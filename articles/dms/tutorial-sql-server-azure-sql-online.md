---
title: Учебник. Миграция SQL Server в оперативный режим в отдельную базу данных SQL
titleSuffix: Azure Database Migration Service
description: Узнайте, как выполнить оперативную миграцию из SQL Server в базу данных SQL Azure с помощью Azure Database Migration Service.
services: dms
author: pochiraju
ms.author: rajpo
manager: craigg
...
- Убедитесь, что правила группы безопасности сети виртуальной сети не блокируют следующие порты входящих подключений для Azure Database Migration Service: 443, 53, 9354, 445, 12000. Дополнительные сведения о фильтрации трафика в виртуальной сети Azure NSG см. в статье [Фильтрация сетевого трафика с помощью групп безопасности сети](https://docs.microsoft.com/azure/virtual-network/virtual-networks-nsg).
...
- Создайте [правило брандмауэра](https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure) уровня сервера для базы данных SQL Azure, чтобы разрешить Azure Database Migration Service доступ к целевым базам данных. Укажите диапазон подсети виртуальной сети, используемый для Azure Database Migration Service.
...
- Убедитесь, что учетные данные, используемые для подключения к целевому экземпляру базы данных SQL Azure, имеют разрешение CONTROL DATABASE на целевые экземпляры базы данных SQL Azure.
...
Прежде чем можно будет перенести данные из экземпляра SQL Server в базу данных SQL Azure, необходимо оценить SQL Server базу данных на наличие блокирующих проблем, которые могут препятствовать миграции. В Помощнике по миграции данных версии 3.3 или более поздней выполните инструкции по [оценке миграции SQL Server](https://docs.microsoft.com/sql/dma/dma-assesssqlonprem), чтобы завершить оценку локальной базы данных.
...
5. В диалоговом окне **Add sources** (Добавление источников) выберите **AdventureWorks2012**, а затем щелкните **Add** (Добавить) и выберите **Start Assessment** (Начать оценку).
    ...
    - Категория **контроля четности SQL Server функций** содержит исчерпывающий набор рекомендаций, альтернативных подходов, доступных в Azure, и меры, которые помогут вам спланировать усилия в проектах миграции.
    - Категория **проблемы совместимости** определяет частично поддерживаемые или неподдерживаемые функции, отражающие проблемы совместимости, которые могут препятствовать миграции SQL Server баз данных в базу данных SQL Azure. Также предлагаются рекомендации, которые помогут вам решить эти проблемы.
...
> [!NOTE]
> Перед созданием проекта миграции в DMA убедитесь, что в Azure уже подготовлена база данных SQL, как указано в предварительных требованиях. В рамках этого руководства для базы данных SQL Azure используется имя **AdventureWorksAzure**, но вы можете назвать ее иначе.
...
6. Нажмите кнопку **Далее**, в разделе **Подключение к целевому серверу**укажите сведения о целевом подключении для базы данных SQL Azure, нажмите кнопку **подключить**, а затем выберите базу данных **Адвентуреворксазуре** , предварительно подготовленную в базе данных SQL Azure.
...
5. Выберите существующую виртуальную сеть или создайте новую.
    ...
    Виртуальная сеть предоставляет Azure Database Migration Service с доступом к исходному SQL Server и целевому экземпляру базы данных SQL Azure.
...
1. Нажмите кнопку **сохранить**, а затем на экране **сведения о целевом объекте миграции** укажите сведения о подключении для целевой базы данных SQL Azure, которая является предварительно подготовленной базой данных SQL Azure, для которой была развернута схема **AdventureWorks2012** , с помощью DMA.
