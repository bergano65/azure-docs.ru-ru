---
title: Предпосылки для миграционной службы лазурных баз данных
...
* Убедитесь, что правила виртуальной сетевой группы безопасности (NSG) не блокируют следующие порты связи 443, 53, 9354, 445, 12000. Для получения более подробной информации о виртуальной сети NSG фильтрации трафика, [см.](https://docs.microsoft.com/azure/virtual-network/virtual-networks-nsg)
...
* При установке SQL Server Express протокол TCP/IP отключен по умолчанию. Включите его, выполнив инструкции в статье [Включение или отключение сетевого протокола сервера](https://docs.microsoft.com/sql/database-engine/configure-windows/enable-or-disable-a-server-network-protocol#SSMSProcedure).
    ...
    > [!IMPORTANT]
    > Для создания экземпляра службы миграции баз данных Azure требуется доступ к настройкам виртуальных сетей, которые обычно не входят в одну и ту же группу ресурсов. В результате пользователю, создавему экземпляр DMS, требуется разрешение на уровне подписки. Чтобы создать необходимые роли, которые можно назначить по мере необходимости, запустите следующий сценарий:
    >
    > ```
    >
    > $readerActions = `
...
* Создайте экземпляр экземпляра базы данных Azure S'L, который вы делаете, следуя подробной информации в статье [Создание базы данных Azure S'L на портале Azure.](https://docs.microsoft.com/azure/sql-database/sql-database-get-started-portal)
...
* Создайте [правило брандмауэра](https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure) уровня сервера для сервера Базы данных SQL Azure, чтобы предоставить службе Azure Database Migration Service доступ к целевым базам данных. Предоставьте диапазон подсетей виртуальной сети, используемой для миграционной службы базы данных Azure.
...
## <a name="prerequisites-for-migrating-sql-server-to-an-azure-sql-database-managed-instance"></a>Предпосылки для переноса сервера S'L в управляемый экземпляр базы данных Azure S'L

* Создайте экземпляр базы данных Azure S'L, управляемую экземпляром, следуя подробной информации в статье [Создание управляемой базы данных Azure S'L на портале Azure.](https://aka.ms/sqldbmi)
