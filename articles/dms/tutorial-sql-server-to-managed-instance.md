---
title: "Учебник: Перемотайте сервер S'L в управляемый экземпляр S'L"
...
---
# <a name="tutorial-migrate-sql-server-to-an-azure-sql-database-managed-instance-offline-using-dms"></a>Учебник: Переносите сервер S'L в управляемый экземпляр базы данных Azure S'L в автономном режиме с помощью DMS

Azure Database Migration Service можно использовать для переноса баз данных из локального экземпляра SQL Server в [Управляемый экземпляр Базы данных SQL Azure](../sql-database/sql-database-managed-instance.md). Для дополнительных методов, которые могут потребовать [SQL Server instance migration to Azure SQL Database managed instance](../sql-database/sql-database-managed-instance-migrate.md)некоторых ручных усилий, см.
...
- Создайте экземпляр базы данных Azure S'L, управляемый экземпляром, следуя подробной информации в статье [Создание управляемого экземпляра базы данных Azure S'L на портале Azure.](https://aka.ms/sqldbmi)
- Убедитесь, что для подключения к исходному серверу SQL Server и целевому управляемому экземпляру используются имена, входящие в серверную роль sysadmin.
    ...
    >[!NOTE]
    >По умолчанию служба миграции базы данных Azure поддерживает только миграционные логины S'L. Тем не менее, можно включить возможность переноса логинов Windows:
    >
    >- Обеспечение того, чтобы в целевом экземпляре базы данных S'L был доступ к считывани AAD, который может быть настроен через портал Azure пользователем с ролью **Администратора компании**или **Глобального администратора.**
    >- Настройка экземпляра службы миграции базы данных Azure для включения миграций входа пользователей/групп Windows, настроенных через портал Azure, на странице Конфигурации. После включения этой настройки перезапустите службу для вступления изменений в силу.
    >
    > После перезапуска службы логины пользователя/группы Windows отображаются в списке логинов, доступных для миграции. Для любого пользователя/группы входа в Windows, которые вы мигрируете, вам предлагается предоставить связанное доменное имя. Учетные записи пользователей службы (учетная запись с доменным именем NT AUTHORITY) и виртуальные учетные записи пользователей (имя учетной записи с доменным именем NT SERVICE) не поддерживаются.
...
1. Выберите имена для входа, которые хотите перенести, на экране **Select logins** (Выбор имени входа).
    ...
    >[!NOTE]
    >По умолчанию служба миграции базы данных Azure поддерживает только миграционные логины S'L. Для поддержки миграционных логинов Windows смотрите раздел **Prerequisites** этого руководства.
...
1. На экране **Configure migration settings** (Настройка параметров миграции) укажите следующие сведения:
    ...
    |**Имя пользователя** | Убедитесь, что пользователь Windows имеет полное право доступа к указанной ранее сетевой папке. Миграционная служба базы данных Azure будет выдавать себя за учетные данные пользователя для загрузки резервных файлов в контейнер хранения Azure для восстановления работы. В случае выбора для миграции баз данных с поддержкой TDE указанный ранее пользователь Windows должен иметь встроенную учетную запись администратора, а режим [Контроль учетных записей](https://docs.microsoft.com/windows/security/identity-protection/user-account-control/user-account-control-overview) должен быть отключен для Azure Database Migration Service, чтобы отправлять и удалять файлы сертификатов. |
    |**Пароль** | Пароль для пользователя |
    |**Параметры учетной записи хранения** | SAS URI, предоставляющая службе миграции базы данных Azure доступ к контейнеру учетных записей хранилища, в который служба загружает файлы резервного копирования, и используется для переноса баз данных в управляемый экземпляр базы данных Azure S'L. [Узнайте, как получить SAS URI для контейнера с каплями.](https://docs.microsoft.com/azure/vs-azure-tools-storage-explorer-blobs#get-the-sas-for-a-blob-container)|
