...
- Создайте контейнер больших двоичных объектов и получите его URI SAS, следуя инструкциям, приведенным в разделе [Получение SAS для контейнера больших двоичных объектов](../vs-azure-tools-storage-explorer-blobs.md#get-the-sas-for-a-blob-container), и не забудьте установить все разрешения (чтение, запись, удаление, перечисление) в окне политик при создании URI SAS. Это позволяет Azure Database Migration Service обращаться к контейнеру учетной записи хранения для отправки файлов резервных копий, которые используются для переноса баз данных в Управляемый экземпляр SQL.

    > [!NOTE]
    > Azure Database Migration Service не поддерживает использование маркера SAS на уровне учетной записи при настройке параметров учетной записи хранения на этапе [Настройка параметров миграции](#configure-migration-settings).
...
5. Выберите существующую виртуальную сеть или создайте новую.
    ...
    Подробные сведения см. в статье [Сетевые топологии для переноса Управляемого экземпляра Azure SQL с помощью Azure Database Migration Service](./resource-network-topologies.md).
...
1. На экране **Сведения о целевом объекте переноса** задайте сведения о подключении для целевого объекта, роль которого здесь выполняет предварительно подготовленный Управляемый экземпляр SQL, в который вы переносите базу данных **AdventureWorks2012**.
    ...
    Если Управляемый экземпляр SQL еще не подготовлен, щелкните [эту ссылку](../azure-sql/managed-instance/instance-create-quickstart.md), чтобы подготовить его. Вы можете продолжить создание проекта, а затем, когда Управляемый экземпляр SQL будет готов, вернуться к этому проекту, чтобы выполнить миграцию.
...
1. На экране **Configure migration settings** (Настройка параметров миграции) укажите следующие сведения:
    ...
    |**Параметры учетной записи хранения** | Код URI SAS, который позволяет Azure Database Migration Service обращаться к контейнеру учетной записи хранения для отправки файлов резервных копий, используемых для переноса базы данных в Управляемый экземпляр SQL. [Получение SAS для контейнера больших двоичных объектов](../vs-azure-tools-storage-explorer-blobs.md#get-the-sas-for-a-blob-container). Этот URI SAS должен определять контейнер больших двоичных объектов, а не учетную запись хранения.|
