---
title: Учебник. Загрузка данных в Хранилище данных SQL Azure | Документация Майкрософт
description: В данном учебнике Azure портала и SQL Server Management Studio для загрузки хранилища данных WideWorldImportersDW из глобального Azure BLOB-объектов в хранилище данных SQL Azure.
...
3. Заполните форму для создания хранилища данных SQL, указав следующие сведения:   
   ...
   | ------- | --------------- | ----------- | 
   | **Имя базы данных** | SampleDW | Допустимые имена баз данных см. в статье об [идентификаторах базы данных](/sql/relational-databases/databases/database-identifiers). | 
...
4. Щелкните **Сервер**, чтобы создать и настроить новый сервер для новой базы данных. Заполните форму для **создания сервера**, указав следующую информацию. 
    ...
    | **Location** | Любое допустимое расположение | Дополнительные сведения о регионах Azure см. [здесь](https://azure.microsoft.com/regions/). |
...
2. В диалоговом окне **Подключение к серверу** введите следующие значения.
    ...
    | Вход | Учетная запись администратора сервера | Это учетная запись, которая была указана при создании сервера. |
...
5. В обозревателе объектов разверните папку **Базы данных**. Затем разверните **Системные базы данных** и **master**, чтобы просмотреть объекты в базе данных master.  Разверните **SampleDW** для просмотра объектов в новой базе данных.
...
Выполните следующие скрипты SQL и укажите сведения о данных, которые требуется загрузить. К этим сведениям относится информация о расположении данных, формате содержимого данных и определении таблицы для данных. Данные находятся в глобальной BLOB-объектов Azure.
...
8. В обозревателе объектов разверните базу данных SampleDW, чтобы просмотреть список внешних таблиц, которые вы создали.
...
В этом разделе используются внешние таблицы, вы определили для загрузки демонстрационных данных из большого двоичного объекта Azure в хранилище данных SQL.  
...
В этом разделе создается таблицы wwi.dimension_Date и wwi.fact_Sale. Он также создает хранимые процедуры, которые могут вызвать миллионов строк в таблицах wwi.dimension_Date и wwi.fact_Sale.
...
Используйте хранимые процедуры, созданной для создания миллионов строк в таблице wwi.fact_Sale и соответствующих данных в таблице wwi.dimension_Date. 
...
2. Запустите эту процедуру для заполнения wwi.fact_Sale с 100 000 строк за каждый день 2000 года.
...
2. Если вы хотите сохранить данные в хранилище, то можете приостановить работу вычислительных ресурсов, когда не используете хранилище данных. По Приостановка вычислений, можно, только плата за хранилище данных, и можно будет возобновить вычисления всякий раз, когда вы будете готовы работать с данными. Чтобы приостановить работу вычислительных ресурсов, нажмите кнопку **Приостановить**. Если работа хранилища данных приостановлена, то отобразится кнопка **Запуск**.  Чтобы возобновить работу вычислительных ресурсов, нажмите кнопку **Запуск**.
