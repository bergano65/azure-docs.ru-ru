...
Если вы еще мало знакомы с хранилищем данных SQL Azure, некоторые разделы могут показаться вам очень сложными.  Разделы здесь в основном расположены в порядке важности.  Если сперва вы уделите внимание нескольким первым основным понятиям, то будете подготовлены к дальнейшему материалу.  По мере ознакомления с хранилищем данных SQL и более привычным знакомым, вернемся и рассмотрим еще несколько концепций.  Вам не понадобится много времени, чтобы разобраться с этими вопросами.
...
Дополнительные сведения см. в статьях [Загрузка данных в хранилище данных Azure SQL][Load data], [Руководство по использованию PolyBase в хранилище данных SQL][Guide for using PolyBase], [Azure SQL Data Warehouse loading patterns and strategies][Azure SQL Data Warehouse loading patterns and strategies] (Стратегии и шаблоны загрузки данных в хранилище данных SQL Azure), [Загрузка данных с помощью фабрики данных Azure][Load Data with Azure Data Factory], [Перемещение данных в хранилище данных Azure SQL и из него с помощью фабрики данных Azure][Move data with Azure Data Factory], [CREATE EXTERNAL FILE FORMAT][CREATE EXTERNAL FILE FORMAT] (Создание формата внешнего файла) и [Функция Create Table As Select (CTAS) в хранилище данных SQL][Create table as select (CTAS)].
...
См. также [классы ресурсов для управления рабочей](resource-classes-for-workload-management.md)нагрузкой, [sys. DM _pdw_waits][sys.dm_pdw_waits]
