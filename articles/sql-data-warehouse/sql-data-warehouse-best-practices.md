...
Если вы еще мало знакомы с хранилищем данных SQL Azure, некоторые разделы могут показаться вам очень сложными.  Разделы здесь в основном расположены в порядке важности.  Если сперва вы уделите внимание нескольким первым основным понятиям, то будете подготовлены к дальнейшему материалу.  При получении ближе познакомиться и знакомы с использованием хранилища данных SQL, вернитесь и взгляните на следующих понятий.  Вам не понадобится много времени, чтобы разобраться с этими вопросами.
...
См. также [управление статистикой таблиц][Manage table statistics], [CREATE STATISTICS][CREATE STATISTICS], [UPDATE STATISTICS][обновление СТАТИСТИКИ]
...
См. также [загрузки данных][Load data], [Guide for using PolyBase][Guide for using PolyBase], [хранилище данных SQL Azure, стратегии и шаблоны загрузки][хранилище данных SQL Azure, стратегии и шаблоны загрузки], [ Загрузка данных с помощью фабрики данных Azure][Load Data with Azure Data Factory], [Move data with Azure Data Factory][Move data with Azure Data Factory], [CREATE EXTERNAL FILE FORMAT][CREATE EXTERNAL FILE FORMAT], [создать таблицу, как select (CTAS)][Create table as select (CTAS)]
...
См. также [Общие сведения о таблицах][Table overview], [Table distribution][Table distribution], [выбору распределения таблицы][выбору распределения таблицы], [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]
...
См. также [основные сведения о транзакциях][Understanding transactions], [Optimizing transactions][Optimizing transactions], [секционирование таблиц][секционирование таблиц], [TRUNCATE TABLE][TRUNCATE TABLE] , [ALTER TABLE][ALTER TABLE], [Создать таблицу, как select (CTAS)][создать таблицу, как select (CTAS)]
...
См. также [Общие сведения о таблицах][Table overview], [Table data types][Table data types], [CREATE TABLE][CREATE TABLE]
...
См. также [индексирование таблиц в][Table indexes], [Columnstore indexes guide][Columnstore indexes guide], [перестроении индексов columnstore][перестроении индексов columnstore]
...
См. также [мониторинг рабочей нагрузки с помощью динамических административных представлений][Monitor your workload using DMVs], [LABEL][LABEL], [параметр][параметр], [sys.dm_exec_sessions][sys.dm_exec_sessions], [sys.dm_pdw_exec_requests][sys.dm_pdw_exec_requests], [sys.dm_pdw_request_steps][sys.dm_pdw_request_steps], [sys.dm_pdw_sql_requests][sys.dm_pdw_sql_requests], [sys.dm_pdw_dms_workers], [DBCC PDW_SHOWEXECUTIONPLAN][DBCC PDW_SHOWEXECUTIONPLAN], [sys.dm_pdw_waits][sys.dm_pdw_waits]
