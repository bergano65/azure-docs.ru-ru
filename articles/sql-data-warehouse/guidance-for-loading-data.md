...
description: Рекомендации и оптимизация производительности для загрузки данных в SQL Analytics
...
Как упоминалось ранее, Загрузка с помощью Polybase обеспечит максимальную пропускную способность хранилища данных SQL. Если вы не можете использовать Polybase для загрузки и должны использовать API SQLBulkCopy (или BCP), рекомендуется увеличить размер пакета для лучшей пропускной способности. хорошим правилом для бегунка является размер пакета в диапазоне от 100 до 1м строк.
