...
Ресурсы ЦП хранилища данных SQL Azure, памяти и ввода-ВЫВОДА объединяются в единицы вычисления вызывается единиц использования хранилища данных (Dwu). Единица DWU представляет собой абстрактный, нормализованный объем вычислительных ресурсов и производительности. Изменение вашего уровня обслуживания изменяет количество Dwu, доступные в систему, которая в свою очередь влияет на производительность и стоимость использования системы.

Для повышения производительности можно увеличить число единиц использования хранилища данных. Производительности сокращения единиц использования хранилища данных. Использование хранилища и операции вычисления оплачиваются отдельно, независимо от изменений количества единиц использования хранилища данных.

Производительность единиц DWU основана на следующих метриках рабочей нагрузки хранилища данных:

- Насколько быстро стандартный запрос хранения данных можно сканировать большое количество строк и затем выполнять сложное объединение. Это требует интенсивного использования операций ввода-вывода и ресурсов ЦП.
- Насколько быстро хранилище данных может принимать данные из больших двоичных объектов хранилища Azure или Azure Data Lake. Это сетевая операция, которая требует интенсивного использования ресурсов ЦП.
- Насколько быстро [ `CREATE TABLE AS SELECT` ](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse) команды T-SQL можно скопировать таблицу. Эта операция предусматривает чтение данных из хранилища, их распространение в различные узлы устройства и повторную запись в хранилище. Эта операция выполняется ЦП и требует интенсивного использования операций ввода-вывода и сетевых ресурсов.
...
В T-SQL параметр SERVICE_OBJECTIVE определяет уровень обслуживания и уровня производительности для хранилища данных.
...
По мере увеличения единиц использования хранилища данных вы линейно увеличивайте количество вычислительных ресурсов. 2-го поколения обеспечивает наилучшую производительность запросов и максимальном масштабе. 2-го поколения систем также использовать большинство кэша.
...
3. Определите любые дополнительные требования для периодов пиковой активности. Рабочие нагрузки, которые показывают значительные пики и спады в активности может потребоваться масштабировать часто.
...
Чтобы изменить число единиц Dwu или Cdwu, используйте [AzSqlDatabase набора](/powershell/module/az.sql/set-azsqldatabase) командлета PowerShell. В приведенном ниже примере для базы данных MySQLDW, размещенной на сервере MyServer, устанавливается цель уровня обслуживания DW1000.
...
Чтобы изменить число DWU, используйте REST API [создания или обновления базы данных](/rest/api/sql/databases/createorupdate). В следующем примере цель уровня обслуживания для DW1000 для базы данных MySQLDW, размещенной на сервере MyServer. Сервер находится в группе ресурсов Azure с именем ResourceGroup1.
...
Это динамическое административное Представление возвращает сведения о различных операциях управления хранилищем данных SQL, таких как операции и состояние операции, которое может быть либо значение IN_PROGRESS или COMPLETED.

## <a name="the-scaling-workflow"></a>Масштабирование рабочего процесса

Когда вы запускаете операцию масштабирования, система сначала прерывает все открытые сеансы, выполняется откат всех открытых транзакций, чтобы обеспечить согласованное состояние. Операции масштабирования можно выполнить только после завершения отката транзакций.  

- Для операции масштабирования система отсоединяет все вычислительные узлы, подготавливает дополнительные вычислительные узлы и затем повторно подключается к уровню хранилища.
- При операции уменьшения масштаба система отсоединяет все вычислительные узлы и затем повторно подключается только необходимые узлы уровню хранилища.
