...
description: Рекомендации и примеры использования секций таблиц в SQL Analytics
...
При секционировании таблиц с **кластеризованными индексами columnstore** важно учитывать, сколько строк будет содержать каждая секция. Для оптимального сжатия и производительности таких таблиц требуется как минимум 1 миллион строк на одно распределение и одну секцию. Перед созданием секций SQL Analytics уже делит каждую таблицу на 60 распределенных баз данных. Таким образом, секционирование таблицы выступает дополнением к распределениям, созданным в фоновом режиме. В этом примере, если таблица фактов продаж содержала 36 ежемесячных секций и при условии, что база данных SQL Analytics содержит 60, то таблица фактов Sales должна 60 000 000 содержать строки в месяц или 2 100 000 000 строки, если все месяцы заполнены. Если в таблице намного меньше строк, чем рекомендуемый минимум на одну секцию, уменьшите количество секций, чтобы на каждую из них приходилось больше строк. Дополнительные сведения см. в статье [Индексирование таблиц в хранилище данных SQL](sql-data-warehouse-tables-index.md). В ней рассматриваются запросы, с помощью которых можно оценить качество кластеризованных индексов columnstore.

## <a name="syntax-differences-from-sql-server"></a>Отличия синтаксиса от SQL Server
SQL Analytics предоставляет способ определения секций, которые проще, чем SQL Server. Функции секционирования и схемы не используются в SQL Analytics, так как они находятся в SQL Server. Вам нужно только определить секционированный столбец и граничные точки. Хотя синтаксис секционирования может немного отличаться от синтаксиса SQL Server, основные понятия одинаковые. SQL Server и SQL Analytics поддерживают один столбец секционирования для таблицы, который может быть секционирован по диапазону. Дополнительные сведения о секционировании см. в статье [Секционированные таблицы и индексы](/sql/relational-databases/partitions/partitioned-tables-and-indexes).
...
Чтобы перенести определения секций SQL Server в SQL Analytics, просто сделайте следующее:
...
Если вы переносите секционированную таблицу из экземпляра SQL Server, количество строк в каждой секции можно запросить с помощью приведенного ниже алгоритма SQL. Помните, что если в SQL Analytics используется одна и та же гранулярность секционирования, число строк на секцию уменьшается на 60.  
...
SQL Analytics поддерживает разделение, слияние и переключение секций. Для каждой из этих функций используется инструкция [ALTER TABLE](/sql/t-sql/statements/alter-table-transact-sql).
...
Загрузка данных в секции с переключением секций — это удобный способ размещения новых данных в таблице, невидимой пользователям при переключении новых данных.  В занятых системах может быть проблематично работать с конфликтами блокировки, связанными с переключением секций.  Чтобы очистить существующие данные в секции, необходимо `ALTER TABLE`, чтобы они использовались для переключения данных.  Затем для переключения новых данных требовалось еще одно `ALTER TABLE`.  В SQL Analytics параметр `TRUNCATE_TARGET` поддерживается в команде `ALTER TABLE`.  При использовании `TRUNCATE_TARGET` команда `ALTER TABLE` перезаписывает существующие данные в секции новыми данными.  Ниже приведен пример, в котором используется `CTAS` для создания новой таблицы с существующими данными, вставки новых данных, а затем переключения всех данных обратно в целевую таблицу с перезаписью существующих данных.
