...
description: Рекомендации по проектированию реплицируемых таблиц в SQL Analytics
...
При проектировании таблиц необходимо максимально четко понять, как устроены данные и как выполняются запросы к данным.  Например, рассмотрим следующие вопросы:
...
![Реплицированная таблица](media/guidance-for-using-replicated-tables/replicated-table.png "Реплицированная таблица")  
...
- Таблица содержит частые операции вставки, обновления и удаления. Для выполнения этих операций языка обработки данных DML требуется перестроение реплицированной таблицы. Перестроение часто может привести к снижению производительности.
...
Если у вас уже есть таблицы с циклическим перебором, рекомендуется преобразовать их в реплицируемые таблицы, если они соответствуют критериям, описанным в этой статье. Реплицированные таблицы эффективнее по сравнению с циклическими таблицами, поскольку они исключают необходимость в перемещении данных.  Таблица с циклическим перебором всегда требует перемещения данных для соединений. 
...
SQL Analytics реализует реплицированную таблицу, сохраняя главную версию таблицы. Для этого оно копирует главную версию в одну базу данных распространителя на каждый вычислительный узел. При изменении SQL Analytics сначала обновляет главную таблицу. Затем оно перестраивает таблицы на каждом вычислительном узле. Перестроение реплицированной таблицы включает в себя копирование таблицы на каждый вычислительный узел и последующее перестроение индексов.  Например, у реплицированной таблицы на DW400 имеются 5 копий данных.  Мастер-копия и полные копии на каждом вычислительном узле.  Все данные хранятся в базах данных распространителя. SQL Analytics использует эту модель для поддержки более быстрых инструкций изменения данных и гибких операций масштабирования. 

Перестроение требуется после следующего:
- Загрузка или изменение данных
- Экземпляр SQL Analytics масштабируется до другого уровня
...
- [CREATE TABLE как SELECT (SQL Analytics)](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse)
