...
Реплицированные таблицы для таблицы измерения в схеме «звезда». Обычно таблицы измерений соединены с таблицами фактов, которые распределяются по-другому, чем в таблице измерения.  Измерения — обычно такого размера, который позволяют хранить и поддерживать несколько копий. В измерениях хранятся описательные данные, которые изменяются редко, например имя и адрес клиента, а также сведения о продукте. Медленно изменяющихся данных приводит к меньшим объемом обслуживания реплицированной таблицы. 
...
- Хранилище данных масштабируется достаточно часто. Масштабирование хранилища данных изменяет количество вычислительных узлов, который вносит перестроение реплицированной таблицы.
...
Например, в этом запросе содержится сложный предикат.  Он выполняется быстрее, когда данные находятся в распределенная таблица вместо реплицированной таблицы. В этом примере данные могут быть циклического перебора.
...
Если вас уже есть циклические таблицы, рекомендуется их преобразования в реплицированные таблицы, если они отвечают критериям, обозначенным в этой статье. Реплицированные таблицы эффективнее по сравнению с циклическими таблицами, поскольку они исключают необходимость в перемещении данных.  Для соединений в циклической таблице всегда требуется перемещение данных. 
