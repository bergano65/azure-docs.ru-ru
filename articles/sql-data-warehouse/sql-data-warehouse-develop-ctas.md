...
description: Пояснение и примеры инструкции CREATE TABLE как SELECT (CTAS) в SQL Analytics для разработки решений.
...
ms.locfileid: "78195912"
---
# <a name="create-table-as-select-ctas-in-sql-analytics"></a>CREATE TABLE в качестве SELECT (CTAS) в SQL Analytics

В этой статье объясняется, CREATE TABLE как инструкция T-SQL в SQL Analytics для разработки решений. Здесь также приведены примеры кодов.
...
Выберите... В не позволяет изменять метод распределения или тип индекса как часть операции. `[dbo].[FactInternetSales_new]` создается с использованием типа распределения по умолчанию ROUND_ROBIN и табличной структуры по умолчанию КЛАСТЕРИЗОВАНного индекса COLUMNSTORE.
...
Вы также можете использовать CTAS для обхода ряда неподдерживаемых функций, перечисленных ниже. Этот метод часто может оказаться полезным, так как он не только будет соответствовать коду, но и часто будет работать быстрее в SQL Analytics. Эта производительность является результатом полностью параллелизации проекта. Сценарии включают:
...
SQL Analytics не поддерживает присоединение ANSI в предложении `FROM` инструкции `UPDATE`, поэтому вы не можете использовать предыдущий пример, не изменяя его.
...
CTAS является одной из самых важных инструкций в SQL Analytics. Научитесь его применять. См. [документацию по CTAS](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse).
