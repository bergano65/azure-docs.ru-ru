...
description: Узнайте, как оптимизировать производительность транзакционного кода в SQL Analytics и свести к минимуму риск длительных откатов.
...
ms.locfileid: "78195844"
---
# <a name="optimizing-transactions-in-sql-analytics"></a>Оптимизация транзакций в SQL Analytics
Узнайте, как оптимизировать производительность транзакционного кода в SQL Analytics и свести к минимуму риск длительных откатов.

## <a name="transactions-and-logging"></a>Транзакции и ведение журнала
Транзакции представляют собой важную часть ядра реляционной СУБД. SQL Analytics использует транзакции во время изменения данных. Эти транзакции могут быть явными или неявными. Одиночные инструкции INSERT, UPDATE и DELETE являются примерами неявных транзакций. В явных транзакциях используются инструкции BEGIN TRAN, COMMIT TRAN или ROLLBACK TRAN. Явные транзакции обычно используются, когда несколько инструкций изменения нужно связать в одну атомарную единицу. 

SQL Analytics фиксирует изменения в базе данных с помощью журналов транзакций. У каждого распределения есть свой журнал транзакций. Запись в журналы транзакций выполняется автоматически. Настраивать что-либо не требуется. Этот процесс гарантирует запись, но в то же время увеличивает нагрузку на систему. Чтобы минимизировать этот эффект, нужно написать эффективный код транзакции. Такой код можно условно разделить на две категории.
...
> [!NOTE]
> Для повторного создания больших таблиц можно воспользоваться функциями управления рабочей нагрузкой SQL Analytics. Дополнительные сведения см. в разделе [Классы ресурсов для управления рабочими нагрузками](resource-classes-for-workload-management.md).
> 
> 
...
Дополнительные сведения об уровнях изоляции и ограничениях транзакций см. [в статье транзакции в SQL Analytics](sql-data-warehouse-develop-transactions.md) .  Рекомендации по использованию хранилища данных SQL Azure см. в [этой статье](sql-data-warehouse-best-practices.md).
