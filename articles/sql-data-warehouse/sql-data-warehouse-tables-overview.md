...
Схемы являются хорошим способом группы таблиц, использовать таким же образом, совместно.  Если вы переносите несколько баз данных из решения в локальной среде в хранилище данных SQL, лучше всего перенести все интеграции таблиц фактов, измерений и одну схему в хранилище данных SQL. Например, можно сохранить все таблицы в примере хранилища данных [WideWorldImportersDW](/sql/sample/world-wide-importers/database-catalog-wwi-olap) в рамках одной схемы, которая называется WWI. В следующем коде создается [пользовательская схема](/sql/t-sql/statements/create-schema-transact-sql) с именем WWI.
...
Временная таблица существует, только пока отрыт сеанс. Чтобы запретить другим пользователям видеть ваши временные результаты и уменьшить потребность в очистке можно использовать временную таблицу.  Временные таблицы используют локальное хранилище для обеспечивают высокую производительность.  Дополнительные сведения см. в статье [Временные таблицы в хранилище данных SQL](sql-data-warehouse-tables-temporary.md).
...
Основная функция хранилища данных SQL является способом это хранение и обработка таблиц между [дистрибутивов](massively-parallel-processing-mpp-architecture.md#distributions).  Хранилище данных SQL поддерживает три метода для распространения данных, циклический перебор (по умолчанию), хэш и реплицированы.

### <a name="hash-distributed-tables"></a>Таблицы с хэш-распределением
Хэш-распределенная таблица распределяет строки на основе значения в столбце распределения. Хэш-распределенная таблица предназначена для обеспечения высокой производительности запросов в больших таблицах. Существует несколько факторов, которые следует учитывать при выборе столбца распределения. 
...
| Промежуточная        | Используйте циклический перебор для промежуточных таблиц. Загрузка с помощью инструкции CTAS выполняется быстро. Когда данные находятся в промежуточной таблице, используйте INSERT... ВЫБЕРИТЕ для перемещения данных в рабочие таблицы. |
...
Оптимизатор запросов использует статистику уровня столбца при создании плана выполнения запроса. Чтобы повысить производительность запросов, важно иметь статистику по отдельным столбцам, особенно столбцам, используемым в объединениях запросов. [Создание статистики](https://docs.microsoft.com/azure/sql-data-warehouse/sql-data-warehouse-tables-statistics#automatic-creation-of-statistics) происходит автоматически.  Однако обновление статистики не происходит автоматически. Обновите статистику после добавления или изменения значительного числа строк. Например, обновите статистику после загрузки. Дополнительные сведения см. в статье [Управление статистикой таблиц в хранилище данных SQL](sql-data-warehouse-tables-statistics.md).
