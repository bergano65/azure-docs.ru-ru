...
Качество группы строк определяется количеством строк в группе строк. Увеличить объем памяти, могут увеличить количество строк в индекс columnstore сжимает в каждой группе строк.  Эти способы можно использовать, чтобы оптимизировать степень сжатия и производительность запросов для индексов columnstore.
...
- DICTIONARY_SIZE. Эта причина усечения указывает на то, что усечение групп строк вызвано наличием по крайней мере одного столбца строки с широкими строками и (или) со строками с высокой кратностью. Размер словаря ограничен объемом памяти 16 МБ, поэтому когда этот предел достигается, группа строк сжимается. Если складывается такая ситуация, рекомендуется поместить проблемный столбец в отдельную таблицу.
...
Подробные сведения о требованиях к памяти columnstore см. в [видео-руководстве по настройке масштабирования хранилища данных SQL Azure](https://channel9.msdn.com/Events/Ignite/2016/BRK3291).
