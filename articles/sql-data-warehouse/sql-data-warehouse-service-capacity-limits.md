...
| Подключение к базе данных |Максимальное количество одновременных сеансов откройте |1024<br/><br/>Количество одновременно открытых сеансов будет зависеть от выбранных DWU. DWU600c и выше поддерживает не более 1024 открыть сеансы. DWU500c и ниже поддерживают максимальное количество одновременных сеансов откройте ограничение в 512. Обратите внимание: есть ограничения на количество запросов, которые могут выполняться одновременно. При превышении ограничения на число параллельных запросов запрос помещается во внутреннюю очередь для ожидания обработки. |
...
| Категория | Описание | Максимум |
|:--- |:--- |:--- |
| Нагрузка Polybase |МБ в строке |1<br/><br/>Polybase загружает строки размером меньше 1 МБ. Загрузка в таблицы с помощью индекса Clustered Columnstore (CCI) типы данных LOB не поддерживается.<br/><br/> |
