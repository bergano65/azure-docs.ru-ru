...
description: Рекомендации по проектированию распределенных хэш-таблиц и распределенных циклов с циклическим перебором в SQL Analytics.
...
ms.locfileid: "78199450"
---
# <a name="guidance-for-designing-distributed-tables-in-sql-analytics"></a>Руководство по проектированию распределенных таблиц в SQL Analytics
Рекомендации по проектированию распределенных хэш-таблиц и распределенных циклов с циклическим перебором в SQL Analytics.

В этой статье предполагается, что вы знакомы с концепциями распределения данных и перемещения данных в SQL Analytics.  Дополнительные сведения см. в разделе [архитектура массовой параллельной обработки SQL Analytics (MPP)](massively-parallel-processing-mpp-architecture.md). 
...
Поскольку идентичные значения всегда хэшируются в одно и то же распределение, аналитика SQL имеет встроенные знания о расположении строк. SQL Analytics использует эти знания для уменьшения перемещения данных во время запросов, что повышает производительность запросов. 
...
В руководстве [Загрузка данных в Нью-Йорке такси](load-data-from-azure-blob-storage-using-polybase.md#load-the-data-into-your-data-warehouse) приводится пример загрузки данных в промежуточную таблицу с циклическим перебором в SQL Analytics.
...
- [CREATE TABLE как SELECT (SQL Analytics)](https://docs.microsoft.com/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse)
