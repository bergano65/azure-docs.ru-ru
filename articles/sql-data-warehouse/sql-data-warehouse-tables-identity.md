...
description: Рекомендации и примеры использования свойства IDENTITY для создания суррогатных ключей в таблицах в SQL Analytics.
...
ms.locfileid: "78199433"
---
# <a name="using-identity-to-create-surrogate-keys-in-sql-analytics"></a>Использование удостоверения для создания суррогатных ключей в SQL Analytics

Рекомендации и примеры использования свойства IDENTITY для создания суррогатных ключей в таблицах в SQL Analytics.

## <a name="what-is-a-surrogate-key"></a>Что такое суррогатный ключ

Суррогатный ключ таблицы представляет собой столбец с уникальным идентификатором каждой строки. Ключ не генерируется из данных таблицы. Модели данных, например, создают суррогатные ключи в таблицах при проектировании моделей SQL Analytics. Чтобы просто и эффективно достичь этой цели, не оказывая влияния на производительность загрузки, можно использовать свойство IDENTITY.  

## <a name="creating-a-table-with-an-identity-column"></a>Создание таблицы со столбцом IDENTITY

Свойство IDENTITY предназначено для масштабирования всех распределений в базе данных SQL Analytics, не влияя на производительность нагрузки. Поэтому реализация IDENTITY ориентирована на достижение этих целей.
...
Свойство IDENTITY не гарантирует порядок, в котором распределяются суррогатные значения, что отражает поведение SQL Server и базы данных SQL Azure. Однако в SQL Analytics отсутствие гарантии является более произносится.
...
SQL Analytics поддерживает синтаксис `SET IDENTITY_INSERT <your table> ON|OFF`. Этот синтаксис позволяет явно вставить значения в столбец IDENTITY.
...
Дополнительные сведения о загрузке данных см. в разделе [Разработка операций извлечения, загрузки и преобразования (ELT) для SQL Analytics](design-elt-data-loading.md) и [Загрузка рекомендаций](guidance-for-loading-data.md).
...
В SQL Analytics не поддерживаются следующие связанные функции:
