...
Чем больше хранилищу данных SQL Azure известно о данных, тем быстрее оно выполняет запросы, связанные с ними. После загрузки данных в хранилище данных SQL, для сбора статистики данных — одна из самых важных вещей, которые помогут вам оптимизировать запросы. Оптимизатор запросов хранилища данных SQL основан на стоимости. Он сравнивает стоимость разных планов запросов и затем выбирает план с наименьшей стоимостью. В большинстве случаев он выбирает такой план, который будет быстрее выполняться. Например если оптимизатор определяет, что запрос выполняет фильтрацию по дата будет возвращать одну строку он выберет один план. Если он оценивает, что выбранной даты будет возвращать 1 миллион строк, то возвращается другой план.

## <a name="automatic-creation-of-statistic"></a>Автоматическое создание статистики

Если включен параметр AUTO_CREATE_STATISTICS базы данных, хранилище данных SQL анализирует входящие запросы пользователей к отсутствующие статистические данные. Если статистика отсутствует, оптимизатор запросов создает статистику по отдельным столбцам в предикате или соединения условие запроса чтобы улучшить оценку кратности для плана запроса. Автоматическое создание статистики в настоящее время включено по умолчанию.

Вы можете установите флажок, если хранилище данных AUTO_CREATE_STATISTICS, настроить, выполнив следующую команду:
...
Если хранилище данных не содержит AUTO_CREATE_STATISTICS настроена, мы рекомендуем включить это свойство, выполнив следующую команду:
...
Автоматическое создание статистики выполняется синхронно, поэтому вы можете понести немного снижает производительность, если отсутствует статистика столбцов. Время создания статистики для одного столбца зависит от размера таблицы. Чтобы избежать снижения производительности, измеряемые, особенно в сравнение производительности, следует убедиться, что статистика будут созданы во-первых, выполнив тестовой рабочей нагрузки до профилирования в системе.

> [!NOTE]
> Создание статистики будут записаны в [sys.dm_pdw_exec_requests](/sql/relational-databases/system-dynamic-management-views/sys-dm-pdw-exec-requests-transact-sql?view=azure-sqldw-latest) в контексте другого пользователя.

При автоматическом создании статистики используется следующий формат: _WA_Sys_<8-разрядный идентификатор столбца в шестнадцатеричном формате>_<8-разрядный идентификатор таблицы в шестнадцатеричном формате>. Можно просмотреть статистику, которые уже были созданы, выполнив [DBCC SHOW_STATISTICS](/sql/t-sql/database-console-commands/dbcc-show-statistics-transact-sql?view=azure-sqldw-latest) команды:

```sql
DBCC SHOW_STATISTICS (<table_name>, <target>)
```

Table_name — имя таблицы, которая содержит статистические данные для отображения. Это не может быть внешняя таблица. Целью является имя целевого индекса, статистики или столбца, для которого отображаются статистические данные.

## <a name="updating-statistics"></a>Обновление статистики

Лучше всего обновлять статистику в столбцах дат каждый день, когда добавляются новые даты. При каждой загрузке строк в хранилище данных добавляются новые даты загрузки или даты транзакций. Это изменяет распределение данных и делает статистику устаревшей. И наоборот Статистика по столбцу страны в таблице клиентов может никогда не должны обновляться, так как распределение значений не меняется. Если предположить, что распределение между клиентами постоянно, добавление новых строк в вариант таблицы не изменит распределение данных. Тем не менее если хранилище данных содержит только одной стране или регионе и появляются данные из новой страны или региона, в результате чего данные из разных стран и регионов, хранящихся, необходимо обновить статистику по столбцу страны или региона.
...
| **Выборка** |  Не более 1 млрд строк, используйте выборку по умолчанию (20 процентов). </br> Свыше 1 миллиарда строк используйте выборку данных о двух процентов. |
...
Отсутствует динамическое административное представление для определения, изменения данных в таблице с момента последнего обновления. Известному возрасту статистики позволяет получить часть изображения. Можно использовать следующий запрос, чтобы определить время последнего обновления статистики в каждой таблице.

> [!NOTE]
> Если распределение значений для столбца изменилось, статистику, независимо от того, время последнего обновления необходимо обновить.
...
Чтобы создать статистику для всех столбцов в таблице, используя параметры по умолчанию, выполните хранимую процедуру.
...
Ниже приведен простой метод обновления всех объектов статистики для таблицы.
...
Инструкция UPDATE STATISTICS очень прост в использовании. Просто помните, что она обновляет *всю* статистику для таблицы и, следовательно, может выполнять больше работы, чем необходимо. Если производительность не является проблемой, это самый простой и эффективный способ гарантировать, что статистика в актуальном состоянии.
