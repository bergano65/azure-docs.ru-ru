...
3. Вы увидите раздел **Редактор** для вашего предложения с четырьмя частями для заполнения: **параметры предложения**, **планы**, **Marketplace**и **Поддержка**. Прочтите инструкции по заполнению этих разделов.
...
|Поле  |Description  |
|---------|---------|
|**Title**     | Понятное имя плана для отображения. Максимальная длина составляет 50 символов.        |
...
Наконец, добавьте в план одну или несколько записей **Авторизация**. Авторизации определяют сущности, которые имеют доступ к ресурсам и подпискам для клиентов, купивших план, и назначают роли, предоставляющие определенные уровни доступа. Дополнительные сведения о поддерживаемых ролях см. в разделе [Арендаторы, роли и пользователи в сценариях Azure Lighthouse](../concepts/tenants-users-roles.md).
...
- **Идентификатор объекта Azure AD**: идентификатор Azure AD пользователя, группы пользователей или приложения, которым будут предоставлены определенные разрешения (как описано в определении роли) для ресурсов клиентов.
...
|Поле  |Description  |
|---------|---------|
|**Title**     |  Название предложения (часто длинное официальное имя). Этот заголовок будет отображаться на видном месте в marketplace. Максимальная длина составляет 50 символов. В большинстве случаев это **имя** должно совпадать с именем, введенным в разделе **Параметры предложения**.       |
...
## <a name="publish-your-offer"></a>Опубликуйте приложение
...
> [!IMPORTANT]
> Делегирование должно быть выполнено с помощью не гостевой учетной записи в клиенте пользователя, которая имеет [встроенную роль владельца](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#owner) для подключаемой подписки (или которая содержит группы ресурсов, которые подключаются). Чтобы просмотреть всех пользователей, которые могут делегировать подписку, пользователь в клиенте клиента может выбрать подписку в портал Azure, открыть **Управление доступом (IAM)** , [вывести список всех ролей](../../role-based-access-control/role-definitions-list.md#list-all-roles), а затем выбрать **владелец** , чтобы просмотреть всех пользователей с этой ролью.
...
> [!NOTE]
> В настоящее время подписки (или группы ресурсов в подписке) нельзя делегировать, если в подписке используется Azure Databricks. Аналогично, если подписка (или группы ресурсов в подписке) уже была делегирована, в настоящее время в этой подписке невозможно будет создать рабочую область Databricks.
