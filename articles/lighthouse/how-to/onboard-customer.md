...
После обновления файла параметров пользователь должен развернуть в арендаторе клиента шаблон Azure Resource Manager в формате развертывания на уровне подписки. Для каждой подписки, которую необходимо подключить (или для каждой подписки, содержащей группы ресурсов, которые необходимо подключить), требуется отдельное развертывание. Развертывание можно выполнить с помощью PowerShell или Azure CLI, как показано ниже.
