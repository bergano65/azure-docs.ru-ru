...
Командлет Azure PowerShell [Get-AzSubscription](https://docs.microsoft.com/powershell/module/Az.Accounts/Get-AzSubscription?view=azps-3.5.0) отображает **tenantID** для каждой подписки, позволяя определить, относится ли возвращенная подписка к арендатору поставщика услуг или арендатору управляемого клиента.

Аналогичным образом команды Azure CLI, такие как [az account list](https://docs.microsoft.com/cli/azure/account?view=azure-cli-latest#az-account-list), отображают атрибуты **homeTenantId** и **managedByTenants**.
...
- Пользователи в управляющем арендаторе не будут иметь доступ для просмотра сведений о выставлении счетов для делегированной подписки клиента, даже если у них есть встроенная роль, которая обычно разрешает это. Это связано с тем, что доступ к сведениям о выставлении счетов требует дополнительных действий, которые в настоящее время поддерживаются только для пользователей в пределах одного арендатора.
