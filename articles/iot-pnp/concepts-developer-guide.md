...
- `name`: имя программирования интерфейса.
...
С ДТДЛ вы описываете возможности устройства с помощью интерфейсов. Интерфейсы описывают _Свойства_, _телеметрию_и _команды_ , которые компонент реализует:
...
- `@type`: определяет тип возможности: `Telemetry`, `Property`или `Command`.
...
- `iothub-interface-id`: идентификатор интерфейса, на котором определена эта команда, например `urn:example:AssetTracker:1`.
...
Чтобы использовать устройство IoT Plug and Play, подключенное к центру Интернета вещей, используйте центр Интернета вещей REST API или один из пакетов SDK для языка Интернета вещей. В следующих примерах используется центр Интернета вещей REST API. Текущая версия API — `2019-07-01-preview`. Присоедините `?api-version=2019-07-01-preview` к вызовам остальных PI.

Чтобы получить значение свойства устройства, например версию встроенного по (`fwVersion`) в интерфейсе `DeviceInformation` в термостата, используется REST API Digital двойников.

Если устройство термостата называется `t-123`, вы получаете все свойства всех интерфейсов, реализованных устройством, с помощью вызова REST API GET:

```REST
GET /digitalTwins/t-123/interfaces
```

Как правило, доступ ко всем свойствам всех интерфейсов осуществляется с помощью этого шаблона REST API, где `{device-id}` — это идентификатор устройства:

```REST
GET /digitalTwins/{device-id}/interfaces
```

Если известно имя интерфейса, например `deviceInformation`, и вы хотите получить свойства для этого интерфейса, вызовите запрос к определенному интерфейсу по имени:

```REST
GET /digitalTwins/t-123/interfaces/deviceInformation
```

Как правило, доступ к свойствам определенного интерфейса можно получить с помощью этого шаблона REST API, где `device-id` является идентификатором для устройства, а `{interface-name}` — именем интерфейса:

```REST
GET /digitalTwins/{device-id}/interfaces/{interface-name}
```

Вы можете напрямую вызывать команды для устройств IoT Plug and Play. Если в интерфейсе `Thermostat` на устройстве `t-123` имеется команда `restart`, ее можно вызвать с помощью вызова REST API POST:
...
- `interface-name`: имя интерфейса из раздела Implements в модели возможностей устройства.
