...
- **Тип пакета SDK для устройства**. Если вы выбрали CMak как тип проекта, это будет действие для настройки того, как созданный код будет включать в себя пакет SDK для устройств с Azure IoT C в `CMakeLists.txt`:
...
1. На панели инструментов **Общие** найдите раскрывающийся список **конфигураций** . Выберите **Управление конфигурацией** , чтобы добавить параметр CMAK для проекта.
    ...
    ![откорректировать конфигурацию;](media/howto-develop-with-vs-vscode/vs-manage-config.png)
