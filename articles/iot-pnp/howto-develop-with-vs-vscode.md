...
- **Тип проекта**. Генератор кода также создает проект CMak или Arduino. В настоящее время поддерживаются следующие типы проектов:
    ...
    - **Проект CMAK в Windows**: для проекта устройства, который использует [CMAK](https://cmake.org/) как систему сборки в Windows. Этот параметр создается `CMakeLists.txt` с конфигурациями пакета SDK для устройств в той же папке, что и код на языке C.
    - **Проект CMAK в Linux**: для проекта устройства, использующего пакет [адменистрирования диспетчера подключений (CMAK](https://cmake.org/) ) в качестве системы сборки в Linux. Этот параметр создается `CMakeLists.txt` с конфигурациями пакета SDK для устройств в той же папке, что и код на языке C.
...
- **Тип пакета SDK для устройства**. Если вы выбрали CMak как тип проекта, это будет действие для настройки того, как созданный код будет включать пакет SDK для устройств с Azure IoT C в `CMakeLists.txt` :
...
1. В VS Code используйте **сочетание клавиш CTRL + SHIFT + P** , чтобы открыть палитру команд, введите и выберите **C/C++: Edit Configurations (JSON)** , чтобы открыть **c_cpp_properties.jsв** файле.
...
Чтобы создать код устройства вместе с пакетом SDK для устройства C в Windows с помощью CMak и компиляторов Visual Studio C/C++ в командной строке, ознакомьтесь с [кратким руководством по Plug and Play IOT](./quickstart-create-pnp-device-windows.md). Ниже показано, как создать код устройства вместе с пакетом SDK для Vcpkgа устройства в Visual Studio как проект CMak.

1. Выполните действия, описанные в [кратком руководстве](https://docs.microsoft.com/azure/iot-pnp/quickstart-create-pnp-device-windows#prepare-the-development-environment) , чтобы установить пакет SDK для устройств Azure IOT для C через Vcpkg.

1. Установите [Visual Studio 2019 (Community, Professional или Enterprise)](https://visualstudio.microsoft.com/downloads/) — убедитесь, что включен компонент **диспетчера пакетов NuGet** и Рабочая нагрузка " **Разработка классических приложений на C++** ".

1. Откройте Visual Studio, выберите **файл > открыть > CMAK...** , чтобы открыть `CMakeLists.txt` в папке, содержащей созданный код.
...
1. В **Обозреватель решений**щелкните правой кнопкой мыши `CMakeLists.txt` в корневой папке и выберите в контекстном меню пункт **построить** , чтобы создать ЗАГЛУШКУ кода с помощью пакета SDK для устройства.
...
1. Используйте [Homebrew](https://brew.sh) для установки всех зависимостей:
