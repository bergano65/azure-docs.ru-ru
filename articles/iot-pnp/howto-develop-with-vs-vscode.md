...
- **Тип пакета SDK для устройства**. Если вы выбрали CMak как тип проекта, это будет действие для настройки того, как созданный код будет включать пакет SDK для устройств с Azure IoT `CMakeLists.txt`C в:
...
1. `cmake` Создайте подкаталог в папке, содержащей созданную заглушку кода, и перейдите к этой папке:
