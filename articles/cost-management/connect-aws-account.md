...
4. Рядом с пунктом **Options** (Параметры) выберите **Require external ID** (Требовать внешний идентификатор). Вставьте уникальное значение, которое было скопировано ранее из поля **External ID** (Внешний идентификатор), в Cloudyn. Нажмите кнопку **Далее: разрешения**.  
