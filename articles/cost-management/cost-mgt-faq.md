...
> Администратора (1) может быть пользователем Microsoft Cloud Solution Provider или соглашением Enterprise. Организации приступить к работе с Cloudyn.  Администратор регистрируется на портале Azure и выполняет вход на портал Cloudyn. Как тот, кто регистрирует службы Cloudyn и знаки на портал Cloudyn, становится администратора (1) *Главный администратор*. Администратор1 не создает какие-либо пользовательские учетные записи. Тем не менее на портале Cloudyn, они создайте учетные записи Azure и настроить иерархии сущностей. Администратора (1) информирует администратора (2), администратором клиента, который им необходим для регистрации с Cloudyn и выполните вход на портал Cloudyn.
>
> Администратор2 регистрируется через портал Azure. Тем не менее при попытке войти на портал Cloudyn, они получают сообщение об ошибке ссылкой, учетная запись **приостановлено**. Главный администратор (администратор1) получает уведомление о приостановке учетной записи. Администратор1 должен активировать учетную запись администратора2 и предоставить *доступ к соответствующим сущностям от имени администратора*. Он также должен разрешить доступ к управлению пользователями и активировать учетную запись пользователя.
