...
Контрольные значения [как назначить доступ в Azure Cost Management](https://www.youtube.com/watch?v=J997ckmwTa8) видео, чтобы узнать о назначении прав для просмотра затрат и расходов с помощью управления доступом на основе ролей в Azure.
...
## <a name="cross-tenant-authentication-issues"></a>Проблемы проверки подлинности по всем клиентам

В настоящее время управление затратами Azure обеспечивает ограниченную поддержку для проверки подлинности между клиентами. В некоторых случаях при попытке проверки подлинности по клиентам, может появиться **отказано в доступе** ошибка при анализе затрат. Эта проблема может возникать, если Настройка управления доступом на основе ролей (RBAC) в другом клиенте подписку и затем попытайтесь просмотреть данные о затратах.

*Чтобы обойти проблему*: После настройки RBAC между клиентами, подождите час. Затем попытайтесь просмотреть затраты в анализе затрат или предоставлять управление затратами доступ пользователям в обоих клиентах.  
