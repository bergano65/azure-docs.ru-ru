...
| Назначение | API клиента Microsoft соглашения (MCA) |
| --- | --- |
...
| Назначение | Старый предложения | Новое предложение |
| --- | --- | --- |
...
| Свойства старого | Новое свойство | Примечания |
| --- | --- | --- |
...
| ConsumedService | consumedService | Точное строковые значения могут отличаться. |
...
| Плата за повтора | Нет | &nbsp;  |
| Расположение | location | &nbsp;  |
| MeterCategory | meterCategory | Точное строковые значения могут отличаться. |
| Значение MeterId | meterId | Точная строка значения отличаются. |
| Значение MeterName | meterName | Точное строковые значения могут отличаться. |
| MeterRegion | meterRegion | Точное строковые значения могут отличаться. |
| MeterSubCategory | meterSubCategory | Точное строковые значения могут отличаться. |
| Месяц | Нет | Выполняет синтаксический анализ месяц из даты. |
| Название предложения | Нет | Используйте параметры publisherName и productOrderName. |
| OfferId | Нет | &nbsp;  |
| Номер заказа | Нет | &nbsp;  |
...
| ServiceTier | meterSubCategory | Точное строковые значения могут отличаться. |
...
| UnitOfMeasure | unitOfMeasure | Точная строка значения отличаются. |
| usageEndDate | date | &nbsp;  |
...
| (новая) | значение свойства publisherType | Тип добавляемого издателя, для покупок. Пустым для использования. |
...
| Название единицы измерения | meterName | Имя индикатора. Выражают развертываемый ресурс службы Azure. |
| Категория единицы измерения  | служба | Имя категории, классификации для единицы измерения. То же, как служба в прайс-лист соглашения Майкрософт клиента. Точная строка значения отличаются. |
| Подкатегория единицы измерения | meterSubCategory | Имя категории подклассификация индикатора. На основе классификации дифференциации набор основных функций в службе. Например основные SQL DB vs базы данных SQL Standard. |
...
| Номер компонента | _Неприменимо_ | Вместо partnumber: используйте productOrderName и снабжены для однозначной идентификации цена за профиля выставления счетов. Поля перечислены в счете MCA вместо partnumber: в MCA счета. |
