...
| Резервирование покупок | [/ресерватиончаржес](/rest/api/billing/enterprise/billing-enterprise-api-reserved-instance-charges) | Microsoft. Billing/Биллингаккаунтс/Биллингпрофилес/Transactions |
...
| Накладные | Microsoft. Billing/Биллингаккаунтс/Биллингпрофилес/счета |
...
| --- | --- | --- |
| ИД учетной записи | Н/Д | Создатель подписки не будет отслеживанием. Используйте Инвоицесектионид (то же, что и departmentId). |
...
| Использованная служба | consumedService | Точные строковые значения могут отличаться. |
| ИД использованной службы | Отсутствуют | &nbsp; |
| Центр затрат | costCenter | &nbsp; |
| Дата и Усажестартдате | date | &nbsp;  |
| День | Отсутствуют | Анализирует дату начала дня. |
| ИД отдела | invoiceSectionId | Точные значения различаются. |
| Название отдела | invoiceSectionName | Точные строковые значения могут отличаться. При необходимости настройте разделы счета для соответствия подразделениям. |
| Расширенные затраты и стоимость | costInBillingCurrency | &nbsp;  |
| InstanceId | resourceId | &nbsp;  |
| Является повторяющимся платежом | Отсутствуют | &nbsp;  |
| Местоположение | расположение | &nbsp;  |
| Категория единицы измерения | meterCategory | Точные строковые значения могут отличаться. |
| ИД единицы измерения | meterId | Точные строковые значения различаются. |
| Имя единицы измерения | meterName | Точные строковые значения могут отличаться. |
| Регион единицы измерения | meterRegion | Точные строковые значения могут отличаться. |
| Подкатегория единицы измерения | meterSubCategory | Точные строковые значения могут отличаться. |
| Месяц | Отсутствуют | Анализирует дату начала месяца. |
| Название предложения | Отсутствуют | Используйте publisherName и Продуктордернаме. |
| OfferId | Отсутствуют | &nbsp;  |
| Номер заказа | Отсутствуют | &nbsp;  |
| PartNumber | Отсутствуют | Используйте meterId и Продуктордернаме для уникальной идентификации цен. |
| Имя плана | productOrderName | &nbsp;  |
| Продукт | Продукт |   |
| ИД продукта | productId | Точные строковые значения различаются. |
...
| ИД расположения ресурса | Отсутствуют | &nbsp;  |
...
| Имя службы | meterCategory | Точные строковые значения могут отличаться. |
...
| Tags | теги | Свойство Tags применяется к корневому объекту, а не к свойству вложенных свойств. |
| Единица измерения | unitOfMeasure | Точные строковые значения различаются. |
| usageEndDate | date | &nbsp;  |
| Год | Отсутствуют | Синтаксический анализ года с даты. |
...
| новую | инвоицеид | Уникальный идентификатор счета. Пусто для текущего открытого месяца. |
...
| новую | сервицепериоденддате | Дата окончания для приобретенной службы. |
| новую | сервицепериодстартдате | Дата начала для приобретенной службы. |
...
| Метод | URI запроса |
| --- | --- |
| ПОМЕСТИТЬ | `https://management.azure.com/providers/Microsoft.Billing/billingAccounts/{billingAccountId}/billingProfiles/{billingProfileId}/pricesheet/default/download?api-version=2018-11-01-preview&startDate=2019-01-01&endDate=2019-01-31&format=csv` |

Используйте API прайс-листа для просмотра всех данных прайс-листа служб потребления Azure в формате JSON:

| Метод | URI запроса |
| --- | --- |
| ПОМЕСТИТЬ | `https://management.azure.com/providers/Microsoft.Billing/billingAccounts/{billingAccountId}/billingProfiles/{billingProfileId}/pricesheet/default/download?api-version=2018-11-01-preview&startDate=2019-01-01&endDate=2019-01-31&format=json` |

Использование API возвращает прайс-лист для всей учетной записи. Однако можно также получить сокращенную версию прайс-листа в формате PDF. Сводка включает в себя использование Azure и услуги по потреблению рынка, которые выставляются за конкретный счет. Счет определяется с помощью {Инвоицеид}, который совпадает с **номером счета** , отображаемым в PDF-файлах сводки по счетам. Рассмотрим пример.
...
| Метод | URI запроса |
| --- | --- |
| ПОМЕСТИТЬ | `https://management.azure.com/providers/Microsoft.Billing/billingAccounts/2909cffc-b0a2-5de1-bb7b-5d3383764184/billingProfiles/2dcffe0c-ee92-4265-8647-515b8fe7dc78/invoices/{invoiceId}/pricesheet/default/download?api-version=2018-11-01-preview&format=csv` |

Для просмотра сведений о счете с помощью API прайс-листа в формате JSON:

| Метод | URI запроса |
| --- | --- |
| ПОМЕСТИТЬ | `https://management.azure.com/providers/Microsoft.Billing/billingAccounts/2909cffc-b0a2-5de1-bb7b-5d3383764184/billingProfiles/2dcffe0c-ee92-4265-8647-515b8fe7dc78/invoices/{invoiceId}/pricesheet/default/download?api-version=2018-11-01-preview&format=json` |
...
| Метод | URI запроса |
| --- | --- |
| ПОМЕСТИТЬ | `https://management.azure.com/providers/Microsoft.Billing/billingAccounts/{billing AccountId}/billingProfiles/{billingProfileId}/pricesheet/default/download?api-version=2018-11-01-preview&format=csv` |

Для просмотра оценочных цен на услуги потребления с помощью API прайс-листа в формате JSON:

| Метод | URI запроса |
| --- | --- |
| ПОМЕСТИТЬ | `https://management.azure.com/providers/Microsoft.Billing/billingAccounts/{billing AccountId}/billingProfiles/{billingProfileId}/pricesheet/default/download?api-version=2018-11-01-preview&format=json` |
...
| --- | --- | --- |
| Идентификатор измерения | _meterId_ | Уникальный идентификатор для счетчика. То же, что и meterId. |
| Название измерения | meterName | Имя индикатора. Счетчик представляет ресурс, развертываемый службой Azure. |
| Категория измерения  | служба | Имя категории классификации для счетчика. То же, что и служба, в прайс – листе соглашения для клиентов Майкрософт. Точные строковые значения различаются. |
| Подкатегория измерения | meterSubCategory | Имя категории подклассификации измерения. На основе классификации набора функций высокого уровня, отличающегося от службы. Например, Basic SQL DB и стандартная база данных SQL. |
...
| Артикул | _Неприменимо_ | Вместо Партнумбер используйте Продуктордернаме и MeterId для уникальной идентификации цены для профиля выставления счетов. Поля перечислены в счете MCA вместо Партнумбер в счетах MCA. |
...
| единица измерения | Не применяется Можно проанализировать из Унитофмеасуре. |
...
Если для отчетов о затратах используется Power BI, необходимо перейти к следующим параметрам:

- Приложение Power BI Microsoft Azure Consumption Insights
- Соединитель Azure Consumption Insights Desktop


Этот соединитель рекомендуется использовать для организаций, которым требуются наиболее гибкие возможности. Однако для быстрой настройки также доступен Power BI приложение.

- Установка [приложения Power BI Microsoft Azure consumption Insights](https://appsource.microsoft.com/product/power-bi/pbi_azureconsumptioninsights.pbi-azure-consumptioninsights?tab=overview)
- [Подключение к соединителю Azure Consumption Insights](/power-bi/desktop-connect-azure-consumption-insights)

Старый пакет содержимого и соединитель для аналитики использования в настоящее время работают на уровне регистрации. Требуется по крайней мере доступ для чтения. Новое приложение Power BI "аналитика потребления" и новый соединитель Azure Consumption Insights доступны для пользователей профиля выставления счетов. Группы, которым требуются дополнительные возможности для просмотра затрат или просмотра затрат в профилях выставления счетов, должны использовать для [анализа затрат](https://ms.portal.azure.com/#blade/Microsoft_Azure_CostManagement/Menu/costanalysis) портал Azure.
