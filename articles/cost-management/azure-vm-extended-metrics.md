...
Например, вам может потребоваться отслеживать процент используемой памяти и загрузки ЦП виртуальных машин Azure. Метрики виртуальной машины Azure соответствуют процентам использования _ЦП_ и _\% \ память выделенных байтов_.

> [!NOTE]
> Расширенный сбор данных метрик поддерживается только с помощью мониторинга Azure на уровне гостя. Cloudyn несовместим с расширением виртуальной машины журналов Azure Monitor.
