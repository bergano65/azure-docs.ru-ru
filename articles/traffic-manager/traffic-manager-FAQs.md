...
Иерархию стран или регионов, которую использует диспетчер трафика, см. [здесь](traffic-manager-geographic-regions.md). Эта страница регулярно дополняется при внесении изменений, но эту же информацию можно получить программным способом с помощью [REST API диспетчера трафика Azure](/rest/api/trafficmanager/). 
...
Нет, диспетчер трафика не может гарантировать, что географический регион, который мы получаем из исходного IP-адреса запроса DNS, всегда будет соответствовать расположению пользователя по следующим причинам:

- Во-первых, как описано в предыдущем вопросе, полученный нами исходный IP-адрес обычно принадлежит сопоставителю DNS, который выполняет поиск от имени пользователя. Географическое расположение сопоставителя DNS может достаточно точно обозначать географическое местоположение пользователя, но они могут и различаться в зависимости от сферы действия сопоставителя DNS и особенностей конкретной службы сопоставителя DNS, выбранной пользователем. Например, клиент, расположенный в Малайзии, может указать в параметрах устройства использование службы сопоставителя DNS, DNS-сервер которой в Сингапур может получить разрешение на обработку запросов для этого пользователя или устройства. В этом случае диспетчер трафика может видеть только IP-адрес сопоставителя, соответствующий расположению Сингапур. См. также ответ о поддержке адресов подсети клиента, представленный на этой странице.
...
Если в профиле используется географическая маршрутизация, регион можно назначить только одной точке в этом профиле. Если эта конечная точка не является вложенным типом с присоединенным к нему дочерним профилем, то в случае, если эта конечная точка переходит в неработоспособное состояние, диспетчер трафика продолжит отправлять ему трафик, так как альтернативой не отправлять какой-либо трафик лучше. Диспетчер трафика не выполняет отработку отказа в другую конечную точку, даже если назначенный регион является родительским для региона, назначенного конечной точке, которая была неработоспособной (например, если конечная точка, имеющая регион "Испания", неработоспособна, мы не отработкой отказа на другую конечную точку, которой назначен регион "Европа"). Таким образом диспетчер трафика будет учитывать ограничения, связанные с географическими регионами, которые клиент установил в своем профиле. Чтобы использовать отработку отказа на другую конечную точку, когда конечная точка становится неработоспособной, рекомендуется назначать географические регионы вложенным профилям с несколькими конечными точками, а не отдельным конечным точкам. Таким образом, в случае сбоя конечной точки во вложенном дочернем профиле трафик будет перенаправлен на другую конечную точку в том же профиле.
...
-    В случае нотации CIDR IP-адрес перед "/" должен быть начальным адресом этого диапазона (например, 1.2.3.0/24 является допустимым, но 1.2.3.4.4/24 является недопустимым).
...
Если в профиле с маршрутизацией подсети есть отключенная конечная точка, диспетчер трафика будет вести себя так, будто этой конечной точки и сопоставлений подсети не существует. Если запрос, сопоставленный с сопоставлением IP-адресов, получен и конечная точка отключена, диспетчер трафика вернет резервную конечную точку (одна без сопоставлений) или если такая конечная точка отсутствует, вернет ответ NXDOMAIN.
...
Другим вариантом использования многозначной маршрутизации является то, что конечная точка является "сдвоенным подключением" к адресам IPv4 и IPv6 и вы хотите предоставить вызывающей стороне возможность выбора из, когда инициируется подключение к конечной точке.
...
Нет, не нужно использовать диспетчер трафика. Сторона маршрутизации диспетчера трафика работает отдельно от реальной части измерения пользователя, хотя у них есть отличная идея в одном веб-свойстве, они не обязательно должны быть.
...
Нет, для работы Измерения на стороне пользователей не требуется размещать серверные компоненты в Azure. Изображение размером в один пиксель, который скачивает скрипт измерений, и соответствующая служба в разных регионах Azure размещается и управляется Azure. 
...
Монитор Azure позволяет оценить обобщенную информацию о трафике для вашего профиля и конечных точек. Также в нем вы можете отслеживать состояние работоспособности конечных точек по результатам проверки работоспособности. Если вам необходимо выйти за рамки этих пользователей и понять, как пользователи подключаются к Azure на региональном уровне, для этого можно использовать Представление трафика.
...
Если вы используете внешние конечные точки, размещенные за пределами регионов Azure, вы можете настроить в профиле диспетчера трафика сопоставление этих точек с регионами Azure, которые будут использоваться для оценки характеристик задержки (это обязательное условие, если вы хотите использовать метод маршрутизации по производительности). Если эта служба сопоставления регионов Azure используется, то при создании выходных данных Представление трафика будут использоваться метрики задержки региона Azure. Если не указан ни один регион Azure, сведения о задержке для этих внешних конечных точек будут пустыми.
...
На портале управления Azure отображается текущее состояние мониторинга для каждой конечной точки, а также всего профиля. Эти сведения также можно получить с помощью [REST API](/rest/api/trafficmanager/) отслеживания трафика, [командлетов PowerShell](/powershell/module/az.trafficmanager) и [кроссплатформенного интерфейса командной строки Azure](/cli/azure/install-classic-cli).

Azure Monitor может использоваться для отслеживания работоспособности конечных точек и просмотра их визуального представления. Дополнительные сведения об использовании Azure Monitor см. в статье [Обзор метрик в Microsoft Azure](../azure-monitor/data-platform.md).
...
### <a name="what-happens-when-an-incoming-querys-record-type-is-different-from-the-record-type-associated-with-the-addressing-type-of-the-endpoints"></a>Что происходит, если тип записи входящего запроса отличается от типа записи, связанной с типом адресации конечных точек?
...
Обычно заголовки узлов указываются, чтобы диспетчер трафика правильно распределял запросы по конечным точкам в среде с несколькими клиентами. Другой вариант использования заключается в определении запросов диспетчера трафика из журналов запросов к протоколу HTTP (S) конечной точки.
...
| --- | --- | --- |
| Отключена. Дочерний профиль отключен. |Остановлена |Состояние родительской конечной точки — «Остановлено», а не «Отключено». Состояние «Отключено» указывает, что вы отключили конечную точку в родительском профиле. |
