...
### <a name="does-traffic-manager-support-sticky-sessions"></a>Поддерживает ли диспетчер трафика «липкие» сеансы?
...
Как поясняется в [How Traffic Manager Works](../traffic-manager/traffic-manager-how-it-works.md), диспетчер трафика работает на уровне DNS. Так как клиенты подключаются к конечным точкам службы напрямую, использование диспетчера трафика при установленном соединении никак не влияет на производительность.
...
### <a name="can-i-use-traffic-manager-with-a-naked-domain-name"></a>Могу ли я использовать traffic Manager с «голым» доменным именем?

Да. Чтобы узнать, как создать псевдоним записи для доменного имени вершины для ссылки на профиль менеджера трафика Azure, [см.](../dns/tutorial-alias-tm.md)
...
Нет, диспетчер трафика не может гарантировать, что географический регион, который мы выводим из исходного IP-адреса запроса DNS, всегда будет соответствовать местоположению пользователя по следующим причинам:

- Во-первых, как описано в предыдущем вопросе, полученный нами исходный IP-адрес обычно принадлежит сопоставителю DNS, который выполняет поиск от имени пользователя. Географическое расположение сопоставителя DNS может достаточно точно обозначать географическое местоположение пользователя, но они могут и различаться в зависимости от сферы действия сопоставителя DNS и особенностей конкретной службы сопоставителя DNS, выбранной пользователем. Например, клиент, находящийся в Малайзии, может указать в настройках своего устройства службу DNS resolver, чей DNS-сервер в Сингапуре может быть выбран для обработки разрешений запросов для этого пользователя/устройства. В этом случае диспетчер трафика может видеть только IP-адрес решатера, соответствующий местонахождению Сингапура. См. также ответ о поддержке адресов подсети клиента, представленный на этой странице.
...
Если в профиле используется географическая маршрутизация, регион можно назначить только одной точке в этом профиле. Если эта конечная точка не является вложенным типом с прикрепленным к ней профилем ребенка, если эта конечная точка будет нездоровой, диспетчер движения продолжает отправлять трафик на нее, так как альтернатива не отправки трафика не лучше. Менеджер трафика не подводит к другой конечной точке, даже если назначенный регион является "родителем" региона, назначенного конечной точке, которая прошла неработоспособно (например, если конечная точка, в которой есть регион Испания, идет нездоровой, мы не можем перейти к другой конечной точке, которая имеет регион Европы, возложенных на него). Таким образом диспетчер трафика будет учитывать ограничения, связанные с географическими регионами, которые клиент установил в своем профиле. Чтобы использовать отработку отказа на другую конечную точку, когда конечная точка становится неработоспособной, рекомендуется назначать географические регионы вложенным профилям с несколькими конечными точками, а не отдельным конечным точкам. Таким образом, в случае сбоя конечной точки во вложенном дочернем профиле трафик будет перенаправлен на другую конечную точку в том же профиле.
...
## <a name="traffic-manager-subnet-traffic-routing-method"></a>Методы маршрутизации трафика в подсети с помощью диспетчера трафика
...
-    В случае нотации CIDR IP-адрес перед '/' должен быть стартовым адресом этого диапазона (например, 1.2.3.0/24 действителен, но 1.2.3.4.4/24 не действителен)
...
Если в профиле с маршрутизацией подсети есть отключенная конечная точка, диспетчер трафика будет вести себя так, будто этой конечной точки и сопоставлений подсети не существует. Если получен запрос, соответствующий его IP-адресу, и конечная точка отключена, диспетчер трафика вернет обратную точку (один без отображения) или если такая конечная точка отсутствует, вернет ответ NXDOMAIN.
...
Другое использование для метода разгрома MultiValue — это если конечная точка является «двойной домашней» для обоих адресов IPv4 и IPv6, и вы хотите предоставить вызывающему оба варианта на выбор, когда он инициирует подключение к конечной точке.
...
## <a name="real-user-measurements"></a>Измерения на стороне пользователей
...
Нет, ему не нужно использовать диспетчера трафика. Сторона разгрома traffic Manager работает отдельно от части измерения реальных пользователей, и хотя это отличная идея, чтобы они оба в том же веб-собственности, они не должны быть.
...
Нет, для работы в Azure не требуется размещение какого-либо компонента на стороне сервера. Изображение размером в один пиксель, который скачивает скрипт измерений, и соответствующая служба в разных регионах Azure размещается и управляется Azure. 
...
Монитор Azure позволяет оценить обобщенную информацию о трафике для вашего профиля и конечных точек. Также в нем вы можете отслеживать состояние работоспособности конечных точек по результатам проверки работоспособности. Когда вам нужно выйти за рамки этих и понять опыт вашего вашего конечном пользователя подключения к Azure на региональном уровне, traffic View может быть использован для достижения этой цели.
...
Если вы используете внешние конечные точки, размещенные за пределами регионов Azure, вы можете настроить в профиле диспетчера трафика сопоставление этих точек с регионами Azure, которые будут использоваться для оценки характеристик задержки (это обязательное условие, если вы хотите использовать метод маршрутизации по производительности). Если у него есть это отображение региона Azure, при создании вывода traffic View будут использоваться метрики задержки региона Azure. Если не указан ни один регион Azure, сведения о задержке для этих внешних конечных точек будут пустыми.
...
### <a name="how-do-i-move-my-traffic-manager-profiles-azure-endpoints-to-a-different-resource-group-or-subscription"></a>Как переместить конечные точки профиля Azure в другой группе ресурсов или подписку?

Конечные точки Azure, связанные с профилем диспетчера трафика, отслеживаются по идентификаторам ресурсов. Когда ресурс Azure, используемый в качестве конечной точки (например, Public IP, Classic Cloud Service, WebApp или другой профиль менеджера трафика, используемый в вложенном виде), перемещается в другую группу ресурсов или подписку, его идентификатор ресурса изменяется. Сейчас в этом сценарии необходимо обновить профиль диспетчера трафика, сначала удалив, а затем добавив конечные точки в профиль.
...
* Сертификаты sNI сервера не проверяются
...
### <a name="what-happens-when-an-incoming-querys-record-type-is-different-from-the-record-type-associated-with-the-addressing-type-of-the-endpoints"></a>Что происходит, когда тип записи входящего запроса отличается от типа записи, связанного с типом адресации конечных точек?
...
|Объект |    A / CNAME |    Целевая конечная точка|
|Объект |    AAAA |    NODATA |
...
|Объект |    Сочетание A и AAAA |    Только целевые конечные точки типа A|
...
### <a name="i-stopped-an-web-application-endpoint-in-my-traffic-manager-profile-but-i-am-not-receiving-any-traffic-even-after-i-restarted-it-how-can-i-fix-this"></a>Я остановил конечную точку веб-приложения в моем профиле менеджера трафика, но я не получаю никакого трафика даже после того, как я перезапустил его. Как это исправить?

При остановке конечная точка веб-приложений Azure диспетчер трафика прекращает проверку своей работоспособности и перезапускает проверку работоспособности только после того, как обнаруживает, что конечная точка перезапущена. Во избежание этой задержки отключите и повторно включите эту конечную точку в профиле диспетчера трафика после перезагрузки.
...
При использовании протокола мониторинга ТСР диспетчер трафика запускает трехстороннее подтверждение TCP, отправляя запрос SYN в конечную точку на указанном порте. Затем он ждет ответа SYN-ACK от конечной точки в течение определенного периода времени (указано в настройках тайм-аута).

- Если ответ SYN-ACK получен в течение периода тайм-аута, указанного в настройках мониторинга, то эта конечная точка считается здоровой. FIN или FIN-ACK — это ожидаемый ответ от менеджера трафика, когда он регулярно завершает розетку.
- Если ответ SYN-ACK получен после указанного тайм-аута, диспетчер трафика ответит RST на сбросить соединение.
...
Обычно заголовки узлов указываются, чтобы диспетчер трафика правильно распределял запросы по конечным точкам в среде с несколькими клиентами. Другой случай использования этого заключается в идентификации запросов диспетчера трафика из журналов запросов HTTP (S) конечной точки
