...
Для работы диспетчера трафика обязательно требуется запись DNS CNAME для сопоставления с личным именем DNS. Например, можно сопоставить `www.contoso.com` с DNS-именем профиля диспетчера трафика `contoso.trafficmanager.net`. Кроме того, профиль диспетчера трафика возвращает еще одну запись DNS CNAME, которая указывает клиенту конечную точку для подключения.
