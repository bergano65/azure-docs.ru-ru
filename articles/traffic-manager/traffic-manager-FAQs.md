...
Да. Чтобы узнать, как создать запись псевдонима для вершине имя вашего домена для ссылки на профиль диспетчера трафика Azure, см. в разделе [настройки записи псевдонима для поддержки вершине доменных имен с помощью диспетчера трафика](../dns/tutorial-alias-tm.md).
...
* Сертификаты SNI на стороне сервера не проверяются.
...
|--|--|--|
|ЛЮБОЙ |  A / AAAA / CNAME |  Целевая конечная точка| 
...
|Входящий запрос|    Тип конечной точки | Предоставленный ответ|
|--|--|--|
|ЛЮБОЙ |  Сочетание A и AAAA | Целевые конечные точки|
...
### <a name="i-stopped-an-web-application-endpoint-in-my-traffic-manager-profile-but-i-am-not-receiving-any-traffic-even-after-i-restarted-it-how-can-i-fix-this"></a>Отключения конечной точки веб-приложения в профиль диспетчера трафика, но я не получаю любой трафик, даже после его перезапуска. Как это исправить?

При остановке конечную точку приложения с веб-Azure диспетчер трафика прекращает проверку их состояния и перезапускает данную проверку только после перезапуска конечной точки. Во избежание этой задержки отключите и повторно включите эту конечную точку в профиле диспетчера трафика после перезагрузки.
...
№ Использование вложенных профилей не отражается на производительности.
