...
5. Возвращается запись DNS CNAME с именем выбранной конечной точки. В этом случае позвольте нам предположить, что contoso-eu.cloudapp.net возвращается.
6. Теперь рекурсивная служба DNS находит серверы доменных имен для домена cloudapp.net. Он связывается с этими серверами имен, чтобы запросить запись DNS "contoso-eu.cloudapp.net". Возвращается запись DNS "A", содержащая IP-адрес конечной точки службы на основе ЕС.
