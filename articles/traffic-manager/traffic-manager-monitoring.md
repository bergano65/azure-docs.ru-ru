...
| --- | --- | --- | --- |
| отключено |Enabled |Неактивен |Профиль отключен. Хотя конечная точка остается в состоянии "Включено", состояние профиля ("Отключено") имеет более высокий приоритет. Конечные точки в отключенных профилях не отслеживаются. Для запроса DNS возвращается код ответа NXDOMAIN. |
...
| Enabled |По крайней мере одна конечная точка находится в состоянии пониженной функциональности. |Понижено |Просмотрите значения состояния отдельных конечных точек, чтобы определить, какие конечные точки необходимо рассмотреть. |
| Enabled |По крайней мере одна конечная точка находится в сети. Отсутствуют конечные точки в состоянии пониженной функциональности. |В сети |Служба принимает трафик. Никаких действий от пользователя не требуется. |
| Enabled |По крайней мере одна конечная точка находится в состоянии проверки конечной точки. Нет конечных точек, находящихся в сети или в состоянии пониженной функциональности. |Проверка конечных точек |Это переходное состояние возникает при создании или включении профиля. Работоспособность конечной точки проверяется в первый раз. |
| Enabled |Все конечные точки профиля отключены или остановлены, либо в профиле не определены конечные точки. |Неактивен |Конечные точки не активны, но профиль включен. |
...
- Если используется протокол мониторинга TCP: 
    - В ответ на запрос SYN, отправленный диспетчером трафика для попытки установки соединения, получен ответ, отличный от ACK или SYN-ACK.
...
* [Является ли диспетчер трафика устойчивым к сбоям в регионе Azure?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#is-traffic-manager-resilient-to-azure-region-failures)

* [Как выбирается расположение группы ресурсов, влияющее на диспетчер трафика?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#how-does-the-choice-of-resource-group-location-affect-traffic-manager)

* [Разделы справки определить текущее состояние работоспособности каждой конечной точки?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#how-do-i-determine-the-current-health-of-each-endpoint)
...
* [Используется ли при добавлении конечной точки IP-адрес или DNS-имя?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#do-i-use-an-ip-address-or-a-dns-name-when-adding-an-endpoint)
...
* [Можно ли использовать разные типы адресации конечной точки в одном профиле?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#can-i-use-different-endpoint-addressing-types-within-a-single-profile)

* [Что происходит, если тип записи входящего запроса отличается от типа записи, связанной с типом адресации конечных точек?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#what-happens-when-an-incoming-querys-record-type-is-different-from-the-record-type-associated-with-the-addressing-type-of-the-endpoints)

* [Можно ли использовать профиль с конечными точками, адресованными по протоколу IPv4 или IPv6, во вложенном профиле?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#can-i-use-a-profile-with-ipv4--ipv6-addressed-endpoints-in-a-nested-profile)

* [Вы остановили конечную точку веб-приложения в своем профиле диспетчера трафика, но не получаете никакого трафика даже после его перезапуска. Как это исправить?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#i-stopped-an-web-application-endpoint-in-my-traffic-manager-profile-but-i-am-not-receiving-any-traffic-even-after-i-restarted-it-how-can-i-fix-this)

* [Можно ли использовать диспетчер трафика, даже если у моего приложения нет поддержки HTTP или HTTPS?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#can-i-use-traffic-manager-even-if-my-application-does-not-have-support-for-http-or-https)

* [Какие конкретные ответы требуются для конечной точки при использовании мониторинга TCP?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#what-specific-responses-are-required-from-the-endpoint-when-using-tcp-monitoring)
...
* [Как указать различные параметры мониторинга для разных конечных точек в профиле?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#how-can-i-specify-different-monitoring-settings-for-different-endpoints-in-a-profile)

* [Как можно назначить заголовки HTTP для проверки работоспособности диспетчера трафика для моих конечных точек?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#how-can-i-assign-http-headers-to-the-traffic-manager-health-checks-to-my-endpoints)
...
* [Какие IP-адреса исходят из проверки работоспособности?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#what-are-the-ip-addresses-from-which-the-health-checks-originate)

* [Сколько проверок работоспособности конечной точки можно получить от диспетчера трафика?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#how-many-health-checks-to-my-endpoint-can-i-expect-from-traffic-manager)

* [Как получить уведомления при отключении одной из конечных точек?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#how-can-i-get-notified-if-one-of-my-endpoints-goes-down)
