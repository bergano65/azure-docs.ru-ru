---
title: Настройка подсети трафика - Менеджер трафика Azure
...
1. В верхнем, левом углу портала Azure выберите **Создать ресурс** > **Compute** > **Windows Server 2016 VM.**
...
В этом разделе вы устанавливаете сервер IIS на два ВМ - *myIISVMEastUS*  & *myIISVMWEurope,* а затем обновляете страницу сайта по умолчанию. На настроенной странице веб-сайта отображается имя виртуальной машины, к которой вы подключаетесь при посещении веб-сайта в веб-браузере.
...
6. На рабочем столе сервера перейдите на **windows Administrative Tools**>**Server Manager.**
...
Диспетчер трафика направляет трафик пользователя на основе имени DNS конечных точек службы. В этом разделе вы настраиваете имена DNS для серверов IIS - *myIISVMEastUS* и *myIISVMWEurope*.
...
Добавьте два ВМ с серверами IIS - *myIISVMEastUS* & *myIISVMWEurope* для маршрутизации пользовательского трафика на основе подсети запроса пользователя.
...
3. Введите или выберите следующие значения, примите значения по умолчанию для остальных параметров и нажмите кнопку **ОК**:
    ...
    | Целевой ресурс          | **Выберите общедоступный IP-адрес,** чтобы показать список ресурсов с общедоступными IP-адресами по той же подписке. В разделе **Ресурс** выберите общедоступный IP-адрес с именем *myIISVMEastUS-ip*. Это общедоступный IP-адрес виртуальной машины сервера IIS в восточной части США.|
...
4. Нажмите кнопку **ОК**.
5. При входе в систему может появиться предупреждение о сертификате. Если вы получили предупреждение, выберите **Да** или **Продолжить**.
1. В веб-браузере на виртуальной машине *myVMEastUS* введите DNS-имя вашего профиля диспетчера трафика, чтобы просмотреть веб-сайт. Так как IP-адрес VM *myVMEastUS* связан с конечным пунктом *myIISVMEastUS*, веб-браузер запускает сервер сайта Test - *myIISVMEastUS.*

   ![Тестирование профиля диспетчера трафика](./media/traffic-manager-subnet-routing-method/test-traffic-manager.png)

2. Затем подключитесь к виртуальной машине *myVMWestEurope*, расположенной в **Западной Европе**, выполнив шаги 1–5, и перейдите к доменному имени профиля диспетчера трафика с этой виртуальной машины. Так как IP-адрес VM *myVMWestEurope* ассоциируется с конечным пунктом *myIISVMEastUS*, веб-браузер запускает сервер сайта Test - *myIISVMWEurope*.
