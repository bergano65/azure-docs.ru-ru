...
В этом разделе создайте две виртуальные машины *myIISVMEastUS* и *myIISVMWestEurope* в **восточной части США** и **Западная Европа** регионах Azure.
...
6. Выполните действия, чтобы создать вторую виртуальную Машину с именем *myIISVMWestEurope*, с помощью **группы ресурсов** имя *myResourceGroupTM2*, **расположение**из *Западная Европа*и все остальные параметры так же, как *myIISVMEastUS*.
...
В этом разделе описано, установка сервера IIS на две виртуальные машины *myIISVMEastUS* и *myIISVMWestEurope*, а затем обновите страницу веб-сайт по умолчанию. На настроенной странице веб-сайта отображается имя виртуальной машины, к которой вы подключаетесь при посещении веб-сайта в веб-браузере.
...
9. Повторите шаги 1 – 8, с путем создания RDP-подключение с виртуальной Машиной *myIISVMWestEurope* в *myResourceGroupTM2* группу ресурсов, чтобы установить IIS и настроить его веб-страницы по умолчанию.
...
Диспетчер трафика направляет трафик пользователя на основе имени DNS конечных точек службы. В этом разделе Настройка DNS-имена для серверов IIS - *myIISVMEastUS* и *myIISVMWestEurope*.
...
В этом разделе описано создание виртуальной Машины (*myVMEastUS* и *myVMWestEurope*) в каждом регионе Azure (**восточной части США** и **Западная Европа**). Эти виртуальные машины будут использоваться для тестирования того, как диспетчер трафика направляет трафик к ближайшему серверу IIS при переходе к веб-сайту.
...
Добавьте две виртуальные машины под управлением IIS серверы — *myIISVMEastUS* & *myIISVMWestEurope* осуществлять маршрутизацию трафика пользователя с ближайшей конечной точкой для пользователя.
...
4. Повторите шаги 2 и 3, чтобы добавить другую конечную точку с именем *myWestEuropeEndpoint* общедоступного IP-адреса *myIISVMWestEurope ip* связанный с сервером IIS с именем виртуальной Машины  *myIISVMWestEurope*.
...
2. Просмотрите диспетчер трафика в действии следующим образом:
    ...
    - Из тестовой виртуальной Машине (*myVMWestEurope*), расположенную в **Западная Европа** регион, в веб-браузер, перейдите к DNS-имя профиля диспетчера трафика.
...
2. Затем подключитесь к виртуальной машине *myVMWestEurope*, расположенной в **Западной Европе**, выполнив шаги 1–5, и перейдите к доменному имени профиля диспетчера трафика с этой виртуальной машины. Так как виртуальная машина находится в **Западная Европа**, вы теперь будете перенаправлены на веб-сайта, размещенного на ближайший сервер IIS *myIISVMWestEurope* , расположенную в **Западная Европа**.
