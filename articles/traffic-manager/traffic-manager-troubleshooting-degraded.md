...
* Диспетчер трафика считает, что конечная точка работает В СЕТИ, только если из пути проверки получен ответ HTTP 200. Если приложение возвращает любой другой код ответа HTTP, вы должны добавить этот код ответа к [диапазонам ожидаемого статусного кода](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-monitoring#configure-endpoint-monitoring) в профиле Traffic Manager.
* 30x перенаправление ответа рассматривается как сбой, если вы не указали это как действительный код ответа в [диапазоне ожидаемого статуса кода](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-monitoring#configure-endpoint-monitoring) вашего профиля менеджера трафика. Диспетчер движения не зондирует цель перенаправления.
...
Для этого примера мы хотим увидеть ответ от\/нашего зонда URL: http: /watestsdp2008r2.cloudapp.net:80/Probe. Проблема продемонстрирована в приведенном ниже примере PowerShell.
...
[Что такое менеджер трафика](traffic-manager-overview.md)
