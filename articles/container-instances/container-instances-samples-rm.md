...
description: Поиск образцов шаблонов шаблонов управления ресурсами Azure для развертывания экземпляров контейнеров Azure в различных конфигурациях
ms.topic: article
ms.date: 03/07/2019
...
|-|-|
| **Приложения** ||
| [Wordpress][app-wp] | Создает веб-сайт WordPress и его базу данных MyS'L в контейнерной группе. Содержимое сайта WordPress и база данных MySQL сохраняются в общей папке Azure. Также создает шлюз приложения, чтобы разоблачить доступ к сети для WordPress. |
...
| [Секрет][vol-secret] | Развертывает контейнер Linux с использованием сертификата PFX, подключенного в качестве секретного тома. |
| **Сеть** ||
...
| [Развертывание контейнерной группы с виртуальной сетью (предварительный просмотр)][net-vnet] | Развертывает новую виртуальную сеть, подсеть, сетевой профиль и группу контейнеров. |
| **Ресурсы Azure** ||
...
[Лазурный CLI][deploy-cli]

[Лазурная силаШелл][deploy-powershell]
