...
При создании экземпляра контейнера при необходимости можно указать команду для переопределения инструкции командной строки по умолчанию, помогут в образ контейнера. Это поведение аналогично аргументу `--entrypoint` командной строки для `docker run`.
...
* Диспетчер ресурсов шаблон или файл YAML или один из пакетов SDK Azure: Укажите свойство командной строки в виде массива строк. Пример. массив `["python", "myscript.py", "arg1", "arg2"]` JSON в шаблоне диспетчер ресурсов. 
