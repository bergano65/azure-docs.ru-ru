---
title: Развертывание изображения контейнера из реестра контейнеров Azure
description: Узнайте, как развертывать контейнеры в экземплярах контейнеров Azure, вытягивая изображения контейнеров из реестра контейнеров Azure.
...
[Реестр контейнеров Azure](../container-registry/container-registry-intro.md) — это управляемая служба реестра контейнеров на базе Azure, используемая для хранения частных образов контейнеров Docker. В этой статье описывается, как вытягивать изображения контейнеров, хранящихся в реестре контейнеров Azure при развертывании в экземплярах контейнеров Azure. Рекомендуемый способ настройки доступа к реестру — создание основного и пароля службы Active Directory Azure и учетных данных входа в хранилище ключей Azure.

## <a name="prerequisites"></a>Предварительные требования

**Реестр контейнеров Azure**: Вам нужен реестр контейнеров Azure - и по крайней мере одно изображение контейнера в реестре - для завершения шагов в этой статье. Если вам нужен реестр, см. сведения о его создании в статье [Краткое руководство. Создание реестра контейнеров с использованием Azure CLI](../container-registry/container-registry-get-started-azure-cli.md).
...
В производственном сценарии, где вы предоставляете доступ к "безголовым" службам и приложениям, рекомендуется настроить доступ к реестру с помощью [основного сервиса.](../container-registry/container-registry-auth-service-principal.md) Директор службы позволяет обеспечить [элементный контроль доступа](../container-registry/container-registry-roles.md) к изображениям контейнеров. Например, вы можете настроить субъект-службу с доступом только на извлечение к реестру.

Реестр контейнеров Azure предоставляет дополнительные [варианты аутентификации.](../container-registry/container-registry-authentication.md)

> [!NOTE]
> Вы не можете проверить подлинность в реестре контейнеров Azure для вытягивания изображений во время развертывания контейнерной группы с помощью [управляемой идентификации,](container-instances-managed-identity.md) настроенной в той же группе контейнеров.
...
Обновите переменную `RES_GROUP` именем существующей группы ресурсов, в которой будет создано хранилище ключей, а `ACR_NAME` — именем вашего реестра контейнеров. Для краткости команды в этой статье предполагают, что регистратура, хранилище ключей и экземпляры контейнеров создаются в одной и той же группе ресурсов.
...
Теперь создайте основной службы и храните его учетные данные в хранилище ключей.
...
Затем храните *appId* директора службы в хранилище, которое является **именем пользователя,** которое вы передаете в реестр контейнеров Azure для проверки подлинности.
...
Свойства реестра контейнеров Azure можно указать в шаблоне менеджера `imageRegistryCredentials` ресурсов Azure, включив свойство в определение группы контейнеров. Например, можно указать учетные данные реестра напрямую:
...
Для получения полных параметров [Resource Manager template reference](/azure/templates/Microsoft.ContainerInstance/2018-10-01/containerGroups)группы контейнеров см.    
