...
Если вы хотите убедиться, что служба "экземпляры контейнеров Azure" может прослушивать порт, настроенный в образе контейнера, протестируйте развертывание образа `aci-helloworld`, предоставляющего порт. Также запустите приложение `aci-helloworld`, чтобы оно прослушивает порт. `aci-helloworld` принимает необязательную переменную среды `PORT`, чтобы переопределить порт по умолчанию 80, который он прослушивает. Например, чтобы проверить порт 9000, задайте [переменную среды](container-instances-environment-variables.md) при создании группы контейнеров:
