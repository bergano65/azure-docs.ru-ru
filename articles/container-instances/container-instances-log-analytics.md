...
1. Под **настройками**выберите **Расширенные настройки**
1. Выберите**серверы Windows** **Connected Sources** > (или **Linux Servers**- ID и ключи одинаковы для обоих)
...
В следующих примерах показаны два способа создания контейнерной группы, состоящей из одного [свободно госконтейнера:][fluentd] Azure CLI и Azure CLI с шаблоном YAML. Свободно контейнер производит несколько строк вывода в конфигурации по умолчанию. Так как эти выходные данные направляются в рабочую область Log Analytics, такой контейнер отлично подходит для демонстрации процессов просмотра журналов и обращения к ним.
...
Вы увидите несколько результатов, отображаемых запросом. Если сначала вы не видите результатов, подождите несколько минут, а затем выберите кнопку **Run** для выполнения запроса снова. По умолчанию записи журнала отображаются в формате **Таблица**. Вы также можете развернуть любую строку, чтобы просмотреть содержимое отдельной записи журнала.
...
Чтобы увидеть примеры результатов запроса, вставьте следующий запрос в текстовое поле запроса и выберите кнопку **«Запуск»** для выполнения запроса. Этот запрос отображает все записи журнала, у которых поле Message (Сообщение) которого содержит слово warn (Предупреждение):
