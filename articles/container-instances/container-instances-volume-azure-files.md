...
Следующий шаблон YAML определяет группу контейнеров с одним контейнером, созданным с помощью образа `aci-hellofiles`. Контейнер подключает файловый ресурс Azure *acishare* , созданный ранее в качестве тома. Где указано, введите имя и ключ хранилища для учетной записи хранения, в которой размещена общая папка. 

Как и в примере CLI, значение `dnsNameLabel` должно быть уникальным в регионе Azure, где создается экземпляр контейнера. При необходимости обновите значение в файле YAML.
...
Затем для каждого контейнера, в котором вы хотите подключить том, заполните массив `volumeMounts` в разделе `properties` определения контейнера.

Следующий шаблон диспетчер ресурсов определяет группу контейнеров с одним контейнером, созданным с помощью образа `aci-hellofiles`. Контейнер подключает файловый ресурс Azure *acishare* , созданный ранее в качестве тома. Где указано, введите имя и ключ хранилища для учетной записи хранения, в которой размещена общая папка. 

Как и в предыдущих примерах, значение `dnsNameLabel` должно быть уникальным в регионе Azure, где создается экземпляр контейнера. При необходимости обновите значение в шаблоне.
