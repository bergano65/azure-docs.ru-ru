---
title: Развертывание экземпляра контейнеров с поддержкой GPU
description: Узнайте, как развертывать экземпляры контейнеров Azure для запуска вычислительно-интенсивных контейнерных приложений с использованием ресурсов Графического процессора.
ms.topic: article
ms.date: 02/19/2020
...
Для выполнения определенных вычислительно-интенсивных рабочих нагрузок в контейнерных инстанциях Azure развернуть [группы контейнеров](container-instances-container-groups.md) с *ресурсами GPU.* Экземпляры контейнеров в группе могут получить доступ к одному графическому процессору NVIDIA Tesla или нескольким при выполнении рабочих нагрузок контейнеров, таких как CUDA и приложения глубокого обучения.

В этой статье показано, как добавлять ресурсы gPU при развертывании контейнерной группы с помощью [файла YAML](container-instances-multi-container-yaml.md) или [шаблона менеджера ресурсов.](container-instances-multi-container-group.md) Можно также указать ресурсы графического процессора при развертывании экземпляра контейнера с помощью портала Azure.
...
**Дополнительные ограничения:** ресурсы GPU не могут быть использованы при развертывании группы контейнеров в [виртуальную сеть.](container-instances-vnet.md)
...
* **Количество** графических процессоров : **1,** **2**или **4**.
* **SKU** - GPU SKU: **K80**, **P100**, или **V100**. Каждый номер SKU сопоставляется с графическим процессором NVIDIA Tesla в одном из следующих семейств виртуальных машин с поддержкой GPU Azure:
...
При развертывании ресурсов графического процессора, набора процессора и ресурсов памяти, подходящих для рабочей нагрузки, до максимальных значений, показанных в предыдущей таблице. Эти значения в настоящее время больше, чем процессор и ресурсы памяти, доступные в контейнерных группах без ресурсов GPU.  
...
* **Драйверы CUDA**. Экземпляры контейнеров с ресурсами GPU предварительно снабжены драйверами NVIDIA CUDA и средами выполнения контейнеров, поэтому вы можете использовать образы контейнеров, разработанные для рабочих нагрузок CUDA.
  ...
  На данном этапе мы поддерживаем CUDA 9.0. Например, для файла Docker можно использовать следующие базовые изображения:
  * [nvidia/cuda:9.0-base-ubuntu16.04](https://hub.docker.com/r/nvidia/cuda/)
  * [тензорпоток/тензорпоток: 1.12.0-gpu-py3](https://hub.docker.com/r/tensorflow/tensorflow)
...
Чтобы развернуть группу контейнеров с ресурсами GPU, также можно воспользоваться [шаблоном Resource Manager](container-instances-multi-container-group.md). Начните с создания файла с именем `gpudeploy.json` и скопируйте в него приведенный ниже код JSON. Этот пример развертывает экземпляр контейнера с графическим процессором V100, который выполняет задание обучения [TensorFlow](https://www.tensorflow.org/) против набора данных MNIST. Запросов ресурсов достаточно для запуска рабочей нагрузки.
