...
Каждому экземпляру контейнера в группе назначаются ресурсы, указанные в запросе ресурса. Однако максимальное количество ресурсов, используемых экземпляром в группе, может отличаться при настройке свойства необязательного [ограничения ресурсов][resource-limits] . Ограничение ресурсов экземпляра должно быть больше или равно значению обязательного свойства [запроса ресурса][resource-requests] .
...
* Если указать ограничение для экземпляра, максимальное использование экземпляра может быть больше, чем у запроса, вплоть до заданного предела. Соответственно, использование ресурсов другими экземплярами в группе может уменьшиться. Максимальное ограничение ресурсов, которое можно задать для экземпляра, — общее количество ресурсов, выделенных группе.
...
В этом сценарии можно задать ограничение ресурсов, равное 2 процессорам для экземпляра. Такая конфигурация позволяет контейнеру использовать до 2 ЦП, если он доступен.
