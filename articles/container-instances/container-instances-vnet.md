---
title: Развертывание контейнерной группы в виртуальную сеть Azure
...
[Виртуальная сеть Azure](../virtual-network/virtual-networks-overview.md) предоставляет безопасные, частные сети для ваших ресурсов Azure и на территории. Развертывание групп контейнеров в виртуальной сети Azure позволяет контейнерам безопасно обмениваться данными с другими ресурсами в виртуальной сети.
...
> [!IMPORTANT]
> Развертывание контейнерных групп в виртуальной сети, как правило, доступно для производственных нагрузок только в следующих регионах: **Восточная часть США, южная центральная часть США и Западная часть США 2**. В других регионах, где функция доступна, виртуальные развертывания сети в настоящее время в предварительном просмотре, с общей доступности планируется в ближайшем будущем. Предварительные версии предоставляются при условии, что вы принимаете [дополнительные условия использования][terms-of-use]. 
...
* Вы не можете использовать [управляемую идентификацию](container-instances-managed-identity.md) в группе контейнеров, развернутой в виртуальной сети.
* Вы не можете включить [зонд живости](container-instances-liveness-probe.md) или [зонд готовности](container-instances-readiness-probe.md) в контейнерной группе, развернутой в виртуальной сети.
* Из-за дополнительных сетевых ресурсов развертывание группы контейнеров в виртуальной сети, как правило, происходит медленнее, чем развертывание стандартного экземпляра контейнера.
...
### <a name="unsupported-networking-scenarios"></a>Неподдерживаемые сетевые сценарии 

* **Балансизатор загрузки Azure** - Размещение баланса загрузки Azure перед экземплярами контейнеров в сетевой группе контейнеров не поддерживается
* **Виртуальная сеть пиринга**
  * Вглядывание VNet не будет работать для ACI, если сеть, в которую вглядывается ACI VNet, использует публичное IP-пространство. Заглянувая сеть нуждается в частном IP-пространстве RFC 1918 для того, чтобы VNet вглядывался в работу. 
  * Вы можете заглянуть в VNet только в один другой VNet
* **Виртуальная маршрутизирующая маршрутизм сетевого трафика** - пользовательские маршруты не могут быть настроены вокруг общедоступных I-плеепов. Маршруты могут быть настроены в рамках частного пространства ИС делегированной подсети, в которой развертываются ресурсы ACI 
* **Группы сетевой безопасности** - Правила безопасности исходящих в НСГ, применяемые к подсети, делегированной в Azure Container Instances, в настоящее время не соблюдаются 
* **Публичная метка IP или DNS** - Контейнерные группы, развернутые в виртуальной сети, в настоящее время не поддерживают размещение контейнеров непосредственно в Интернет ею с общедоступным IP-адресом или полностью квалифицированным доменным именем
* **Внутреннее разрешение имен** - Разрешение имен для ресурсов Azure в виртуальной сети через внутренний DNS Azure не поддерживается
...
1. Создайте подсеть в существующей виртуальной сети, используйте существующую подсеть, в которой уже развернута группа контейнеров, или используйте существующую подсеть, опустевшая от *всех* других ресурсов
...
Выполните следующую команду [az container create][az-container-create], которая задает настройки для новой виртуальной сети и подсети. Необходимо предоставить название группы ресурсов, которая была создана в регионе, где [доступны](#virtual-network-deployment-limitations)развертывания контейнерных групп в виртуальной сети. Эта команда развертывает общедоступный контейнер [Microsoft aci-helloworld,][aci-helloworld] который запускает небольшой веб-сервер Node.js, обслуживающий статичную веб-страницу. В следующем разделе вы развернете вторую группу контейнеров в той же подсети и протестируете связь между двумя экземплярами контейнеров.
...
Можно также развернуть группу контейнеров в существующую виртуальную сеть с помощью файла YAML, шаблона менеджера ресурсов или другого программного метода, например, с Python SDK. Для развертывания в подсети виртуальной сети следует указать несколько дополнительных свойств в YAML:
...
Развертывание группы контейнеров с [группой аз создать][az-container-create] команду, указывая имя файла YAML для `--file` параметра:
...
Эта функция в настоящее время требует нескольких дополнительных команд для удаления сетевых ресурсов, созданных ранее. Если вы использовали примеры команд из предыдущих разделов этой статьи для создания виртуальной сети и подсети, можно удалить эти сетевые ресурсы с помощью следующего скрипта. Сценарий предполагает, что группа ресурсов содержит единую виртуальную сеть с одним сетевым профилем.

Перед выполнением скрипта задайте в качестве переменной `RES_GROUP` имя группы ресурсов, содержащей виртуальную сеть и подсеть, которые следует удалить. Обновите название виртуальной сети, `aci-vnet` если вы не использовали имя, предложенное ранее. Скрипт отформатирован для оболочки Bash. Если вы предпочитаете использовать другую оболочку, например PowerShell или командную строку, измените методы доступа и присваивание значения переменной соответствующим образом.
...
* [Подсети](../virtual-network/virtual-network-manage-subnet.md)
* [Конечные точки обслуживания](../virtual-network/virtual-network-service-endpoints-overview.md)
