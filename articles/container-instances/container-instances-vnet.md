...
Хотя эта функция находится на этапе предварительной версии, при развертывании групп контейнеров в виртуальной сети применяются следующие ограничения. 
...
* **Пиринг виртуальных сетей** — пиринг виртуальной сети не будет работать для ACI, если сеть, к которой подключена виртуальная сеть ACI, использует пространство общедоступного IP-адреса. Одноранговая сеть требует наличия частного IP-пространства АДРЕСНЫЕ пространства RFC1918, чтобы обеспечить работу пиринга. Кроме того, в настоящее время вы можете выполнить одноранговую виртуальную сеть только для одной виртуальной сети.
* **Маршрутизация трафика виртуальной сети** . маршруты клиентов не могут настраиваться на основе общедоступных IP-адресов. Маршруты можно настроить в частном IP-пространстве в делегированной подсети, в которой развернуты ресурсы ACI 
...
Выполните следующую команду [AZ Container Create][az-container-create] , которая указывает параметры для новой виртуальной сети и подсети. Необходимо указать имя группы ресурсов, которое было создано в регионе, [поддерживающем](#preview-limitations) группы контейнеров в виртуальной сети. Эта команда развертывает общедоступный контейнер Microsoft [ACI-HelloWorld][aci-helloworld] , в котором выполняется небольшой веб-сервер Node. js, обслуживающий статичную страницу. В следующем разделе вы развернете вторую группу контейнеров в той же подсети и протестируете связь между двумя экземплярами контейнеров.
...
> [!NOTE]
> Если при попытке удаления сетевого профиля появляется сообщение об ошибке, подождите 2-3 дней, чтобы платформа автоматически воспринимала проблемы, и повторите попытку удаления. Если у вас по-прежнему возникают проблемы с удалением сетевого профиля, [откройте службу поддержки рекест.](https://azure.microsoft.com/support/create-ticket/)
