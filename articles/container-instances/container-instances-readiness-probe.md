...
Развертывание включает свойство `command`, определяющее начальную команду, которая выполняется при первом запуске контейнера. Это свойство принимает массив строк. Эта команда имитирует время, когда веб-приложение выполняется, но контейнер не готов. 

Сначала запускается сеанс оболочки и запускается команда `node` для запуска веб-приложения. Он также запускает команду в спящий режим в течение 240 секунд, после чего он создает файл с именем `ready` в каталоге `/tmp`:
