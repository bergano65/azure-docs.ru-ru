...
Сначала запускается сеанс оболочки и выполняется `node` команда для запуска веб-приложения. Она также запускает команду в спящий режим в течение 240 секунд, после чего он создает файл `ready` с именем `/tmp` в каталоге:
...
Этот файл YAML определяет, `readinessProbe` который поддерживает команду `exec` готовности, которая действует как проверка готовности. В этом примере команда готовности проверяет наличие `ready` файла в `/tmp` каталоге.
...
В этом примере в течение первых 240 секунд команда Readiness завершается ошибкой при проверке существования `ready` файла. Код состояния вернул сигнал о том, что контейнер не готов.
