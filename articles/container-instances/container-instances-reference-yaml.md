...
| инитконтаинерс | массиве | Нет | Контейнеры init для группы контейнеров. - [Объект Инитконтаинердефинитион](#initcontainerdefinition-object) |
...
| .   | массиве | Нет    | Команда, которую необходимо выполнить в контейнере init в Exec Form. -String |
| environmentVariables | массиве  | Нет |Переменные среды, которые необходимо задать в контейнере init. - [Объект EnvironmentVariable](#environmentvariable-object)
| волумемаунтс |массиве   | Нет    | Подключения тома, доступные для контейнера init. - [Объект Волумемаунт](#volumemount-object)
