---
title: Введение в модель ресурсов ресурсов службы Azure
...
Приложения Сетки Service Fabric на основе модели ресурсов упаковываются в виде контейнеров Docker.  Сетка Service Fabric является общей мультитенантной средой, а контейнеры обеспечивают высокий уровень изоляции.  Описываются эти приложения с помощью формата JSON или YAML (который затем преобразуется в формат JSON). При развертывании приложения Mesh в сетку отдела данных Отдела азов до 2000 -00 JSON, используемый для описания приложения, является шаблоном менеджера ресурсов Azure. Ресурсы сопоставляются с ресурсами Azure.  При развертывании приложения Mesh в кластер Service Fabric (вавтономное или размещенное в Azure) приложение, используемое для описания приложения, является форматом, похожим на шаблон управления ресурсами Azure.  После развертывания приложениями Сетки можно управлять с помощью интерфейсов HTTP или Azure CLI. 
