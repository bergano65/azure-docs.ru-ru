---
title: Общие вопросы для лазурной службы Ткань сетки
...
В настоящее время плата за развертывание приложений или контейнеров для просмотра mesh отсутствует. Пожалуйста, следите за обновлениями в мае для включения в выставление счетов. Однако мы рекомендуем удалять развертываемые ресурсы и не оставлять их в ней, если вы не будете активно тестировать их.
...
- Количество заявок: 5
- Коры в приложении: 12
- Общая оперативная память на приложение: 48 ГБ
- Конечные точки сети и ingress: 5
- Объемы Azure, которые можно прикрепить: 10
- Количество реплик службы: 3
...
### <a name="what-container-images-are-supported"></a>Какие изображения контейнеров поддерживаются?
...
> [!NOTE]
> Инструментирование Visual Studio для Mesh пока не поддерживает развертывание в Windows Server 2019 и 1809 контейнерах.

### <a name="what-types-of-applications-can-i-deploy"></a>Какие типы приложений можно развернуть? 

Вы можете развернуть все, что работает в контейнерах, которые вписываются в ограничения, наложенные на ресурс приложения (см. выше для получения дополнительной информации о квотах). Если мы обнаружим, что вы используете Mesh для выполнения незаконных рабочих нагрузок или злоупотребления системой (т.е. майнинга), то мы оставляем за собой право прекратить развертывание и блокировать подписку от выполнения службы. Пожалуйста, обратитесь к нам, если у вас есть какие-либо вопросы о запуске определенной рабочей нагрузки. 
...
- Если само по себе название службы не работает, попробуйте полностью квалифицированное имя: ServiceName.ApplicationName.
...
Для других известных проблем DNS с запуском кластера разработки Service Fabric на Windows 10 см.: [Отопечение контейнеров Windows](/azure/service-fabric/service-fabric-how-to-debug-windows-containers) и [известные проблемы DNS.](https://docs.microsoft.com/azure/service-fabric/service-fabric-dnsservice#known-issues)
...
### <a name="vs-tooling-has-limited-support-for-windows-containers"></a>VS Tooling имеет ограниченную поддержку контейнеров Windows

Инструментальная студия Visual Studio поддерживает только развертывание Windows Контейнеры с базовой версией ОС Windows Server 1709 и 1803 сегодня. 
...
Чтобы узнать больше о сервисной сетке Fabric, прочитайте [обзор.](service-fabric-mesh-overview.md)
