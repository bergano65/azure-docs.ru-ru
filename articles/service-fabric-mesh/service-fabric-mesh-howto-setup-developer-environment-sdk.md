---
title: Настройка среды разработчиков Windows для сетки сервисных тканей
...
Следующие инструкции помогут вам получить все установлено на основе версии Windows вы работаете.

[!INCLUDE [preview note](./includes/include-preview-note.md)]

## <a name="visual-studio"></a>Visual Studio

Visual Studio 2017 или позже требуется для развертывания приложений Service Fabric Mesh. [Его версия должна быть не ниже 15.6.0][download-visual-studio] и включать следующие рабочие нагрузки.
...
Если у вас уже есть установленный Docker, убедитесь, что используете последнюю версию. Докер может подсказать вам, когда новая версия выходит, но проверить вручную, чтобы убедиться, что у вас есть последняя версия.
...
Если Hyper-V не включен на вашей машине, установщик Docker предложит включить его. Если будет предложено, щелкните **OK**.
...
Если вы используете Visual Studio, вы можете пропустить этот раздел, потому что Visual Studio создаст локальный кластер для вас, если у вас его нет.

Для лучшей производительности отладки при создании и запуске одного приложения Service Fabric одновременно создайте кластер локальной разработки с одним уемничением. Если вы работаете одновременно несколько приложений, создайте кластер локальной разработки из пяти узлов. Этот кластер должен быть запущен при развертывании или при отладке проекта Сетки Service Fabric.
