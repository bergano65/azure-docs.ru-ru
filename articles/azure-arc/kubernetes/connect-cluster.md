...
* Кластер Kubernetes настроен и запущен.
* Возможность доступа с помощью kubeconfig и полномочия администратора кластера. 
...
* Установлены последние версии расширений*connectedk8s* и *k8sconfiguration*.
...
## <a name="install-azure-cli-extensions"></a>Установка расширений Azure CLI
...
Чтобы обновить расширения до последних версий, выполните следующие команды.
...
* `deployment.apps/clusteridentityoperator` хранит сертификат управляемого удостоверения службы (MSI), который используется другими агентами для взаимодействия с Azure.
...
Команда Azure CLI `az connectedk8s delete` удаляет ресурс `Microsoft.Kubernetes/connectedCluster` из Azure. При этом Azure CLI удаляет из Azure и все связанные ресурсы `sourcecontrolconfiguration`. Azure CLI использует для удаления агентов в кластере команду helm uninstall.

Портал Azure удаляет из Azure ресурс `Microsoft.Kubernetes/connectedcluster` и все связанные с ним ресурсы `sourcecontrolconfiguration`.

Чтобы удалить агенты из кластера, необходимо выполнить команду `az connectedk8s delete` или `helm uninstall azurearcfork8s`.
