...
В связи с Kubernetes Гитопс — это практика объявления требуемого состояния конфигураций кластера Kubernetes (развертываний, пространств имен и т. д.) в репозитории Git. За этим объявлением следует развертывание этих конфигураций кластера с помощью опроса и на основе запроса, используя оператор. 

В этой статье рассматривается настройка рабочих процессов Гитопс в кластерах Kubernetes с поддержкой ARC в Azure.

Подключение между кластером и репозиторием Git создается `Microsoft.KubernetesConfiguration/sourceControlConfigurations` в Azure Resource Manager в качестве ресурса расширения. Свойства ресурса `sourceControlConfiguration` представляют, где и как ресурсы Kubernetes должны передаваться из Git в кластер. `sourceControlConfiguration`Данные хранятся в зашифрованном виде в базе данных Azure Cosmos DB для обеспечения конфиденциальности данных.
...
Вы можете создать несколько `sourceControlConfiguration` ресурсов в одном кластере Kubernetes с поддержкой дуги Azure для достижения нескольких целей. Ограничьте развертывания до соответствующих пространств имен, создав каждый из них `sourceControlConfiguration` с другой `namespace` областью.

Репозиторий Git может содержать:
...
Типичный набор сценариев включает определение базовой конфигурации для Организации, в том числе общие роли и привязки Azure, агенты наблюдения или ведения журнала, а также службы на уровне кластера.

Один и тот же шаблон можно использовать для управления более крупной коллекцией кластеров, которая может быть развернута в разнородных средах. Например, имеется один репозиторий, который определяет базовую конфигурацию для Организации, которая применяется к нескольким кластерам Kubernetes одновременно. [Политика Azure позволяет автоматизировать](use-azure-policy.md) создание `sourceControlConfiguration` с помощью определенного набора параметров во всех ресурсах Kubernetes, поддерживающих службу Arc Azure, в пределах области (подписки или группы ресурсов).

Чтобы узнать, как применить набор конфигураций с областью действия, выполните следующие действия `cluster-admin` .
