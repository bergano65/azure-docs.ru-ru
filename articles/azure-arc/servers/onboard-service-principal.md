...
description: Из этой статьи вы узнаете, как подключить компьютеры к Azure с помощью серверов с поддержкой ARC (Предварительная версия) с помощью субъекта-службы.
ms.date: 07/23/2020
ms.topic: conceptual
...
По завершении этого процесса вы сможете успешно подключить гибридные компьютеры к серверам с поддержкой ARC (Предварительная версия).
