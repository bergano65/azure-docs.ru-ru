...
- Windows Server 2012 R2 и более поздних версий (включая Windows Server Core)
...
Чтобы обеспечить безопасность данных при передаче в Azure, настоятельно рекомендуется настроить компьютер для использования протокола TLS 1,2. Более старые версии протоколов TLS/SSL оказались уязвимы. Хотя они все еще используются для обеспечения обратной совместимости, применять их **не рекомендуется**. 
...
Пакет агента подключенного компьютера Azure для Windows и Linux можно скачать из указанных ниже расположений.
...
Агент подключенного компьютера Azure для Windows и Linux можно обновить до последнего выпуска вручную или автоматически в зависимости от ваших требований. Для Windows обновление агента можно автоматически выполнить с помощью Центр обновления Windows и для Ubuntu с помощью программы командной строки [APT](https://help.ubuntu.com/lts/serverguide/apt.html) .

### <a name="agent-status"></a>Состояние агента

Агент подключенного компьютера отправляет службе регулярное сообщение пульса каждые 5 минут. Если он не получен в течение 15 минут, компьютер считается автономным, а состояние автоматически изменится на " **отключено** " на портале. При получении следующего сообщения пульса от агента подключенного компьютера его состояние автоматически изменится на **подключено**.
