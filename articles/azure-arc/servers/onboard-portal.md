...
description: Из этой статьи вы узнаете, как установить агент и подключить компьютеры к Azure с помощью серверов с поддержкой ARC (Предварительная версия) из портал Azure.
ms.date: 08/07/2020
ms.topic: conceptual
...
1. На странице **Создать скрипт** выберите подписку и группу ресурсов, которые следует использовать для управления компьютером в Azure. Выберите расположение Azure, в котором будут храниться метаданные компьютера.
    ...
    >[!NOTE]
    >Серверы с поддержкой ARC (Предварительная версия) Azure поддерживают только следующие регионы:
    >- EastUS
    >- WestUS2
    >- WestEurope
    >- SoutheastAsia
