...
И обратите внимание, что модуль нового рабочего процесса (postgres01w-2) размещен на том же узле, что и координатор. 
...
|----|----|----
|начальный загрузчик — jh48b|Служба, которая обрабатывает входящие запросы на создание, изменение и удаление настраиваемых ресурсов, таких как управляемые экземпляры SQL Server, PostgreSQLные группы серверов и контроллеры данных.|AKS-ажентпул-42715708-vmss000003
|Control-гвмбс||AKS-ажентпул-42715708-vmss000002
|контролдб-0|Хранилище данных контроллера, используемое для хранения конфигурации и состояния контроллера данных.|AKS-ажентпул-42715708-vmss000001
|контролвд — zzjp7|Служба "Watch собаки" контроллера, которая следит за доступностью контроллера данных.|AKS-ажентпул-42715708-vmss000000
|логсдб-0|Экземпляр эластичного поиска, который используется для хранения всех журналов, собираемых для всех модулей данных служб ARC. Elasticsearch, получает данные из `Fluentbit` контейнера каждого Pod|AKS-ажентпул-42715708-vmss000003
|логсуи — 5fzv5|Экземпляр Kibana, расположенный поверх базы данных эластичного поиска, чтобы предоставить графический пользовательский интерфейс log Analytics.|AKS-ажентпул-42715708-vmss000003
|метриксдб-0|Экземпляр InfluxDB, который используется для хранения всех метрик, собранных во всех модулях данных служб ARC. InfluxDB, получает данные из `Telegraf` контейнера каждого Pod|AKS-ажентпул-42715708-vmss000000
|метриксдк — 47d47|Набор управляющих программ, развернутый на всех узлах Kubernetes в кластере для сбора метрик уровня узла об узлах.|AKS-ажентпул-42715708-vmss000002
|метриксдк — 864kj|Набор управляющих программ, развернутый на всех узлах Kubernetes в кластере для сбора метрик уровня узла об узлах.|AKS-ажентпул-42715708-vmss000001
|метриксдк — l8jkf|Набор управляющих программ, развернутый на всех узлах Kubernetes в кластере для сбора метрик уровня узла об узлах.|AKS-ажентпул-42715708-vmss000003
|метриксдк — nxm4l|Набор управляющих программ, развернутый на всех узлах Kubernetes в кластере для сбора метрик уровня узла об узлах.|AKS-ажентпул-42715708-vmss000000
|метриксуи — 4fb7l|Экземпляр Grafana, расположенный на базе базы данных InfluxDB для представления графического пользовательского интерфейса панели мониторинга.|AKS-ажентпул-42715708-vmss000003
|мгмтпрокси — 4qppp|Уровень прокси веб-приложения, расположенный перед экземплярами Grafana и Kibana.|AKS-ажентпул-42715708-vmss000002
...
Как описано выше, узлы координатора (Pod 1) в группе серверов postgres в службе "Дуга Azure" используют те же физические ресурсы, что и третий рабочий узел (Pod 4) группы серверов. Это приемлемо, поскольку узел координатор обычно использует очень мало ресурсов в сравнении с тем, что может использоваться рабочим узлом. По этой причине необходимо тщательно выбрать:
...
Давайте Опишите модуль postgres01w-3, чтобы определить, на каком физическом узле он размещен:
