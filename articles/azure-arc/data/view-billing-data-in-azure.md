...
Дополнительные сведения о различиях между [режимами подключения](/docs/connectivity.md)см. в этой статье.
...
1. Создайте службу данных с поддержкой дуги Azure, если она еще не создана. Например, создайте один из следующих элементов:
   ...
   - [Создание группы серверов PostgreSQL в службе "Дуга Azure" с поддержкой геомасштабирования](create-postgresql-hyperscale-server-group.md)
...
1. Убедитесь, что для фильтра даты установлен **этот месяц** или какой-либо другой диапазон времени, который имеет смысл в отношении времени создания ресурсов службы данных.
...
1. Выберите либо создать новую учетную запись хранения, либо создать новую, заполнив форму, чтобы указать учетную запись хранения, контейнер и путь к каталогу для экспорта файлов данных выставления счетов, а затем нажмите кнопку Далее.
1. Щелкните Создать.
