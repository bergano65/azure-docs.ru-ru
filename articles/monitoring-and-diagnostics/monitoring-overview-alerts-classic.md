...
Для данных журнала диагностики, доступных через Azure Monitor, перенаправьте данные в Log Analytics (прежнее название — OMS) и используйте оповещение о запросе Log Analytics. Теперь в Log Analytics используется [новый метод оповещения](monitoring-overview-alerts.md). 
