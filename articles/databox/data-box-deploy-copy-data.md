...
description: В этом руководстве объясняется, как установить подключение к главному компьютеру и скопировать данные с него в Azure Data Box, используя SMB и локальный пользовательский веб-интерфейс.
services: databox
author: alkohli
ms.service: databox
ms.subservice: pod
...
|Типы службы хранилища Azure  | Общие папки Data Box            |
|-------------------|--------------------------------------------------------------------------------|
...
3. Чтобы получить доступ к общим папкам, связанным с вашей учетной записью хранения ( *utsac1* в следующем примере) на главном компьютере, откройте окно командной строки. В командной строке введите:
...
4. Нажмите клавиши WINDOWS+R. В окне **Выполнить** введите `\\<device IP address>`. Нажмите кнопку **ОК** , чтобы открыть проводник.
    ...
    ![Подключение к общей папке с помощью проводника](media/data-box-deploy-copy-data/connect-shares-file-explorer1.png)

    Должны отобразиться папки, в том числе общие.
    
    ![Общие папки в проводнике](media/data-box-deploy-copy-data/connect-shares-file-explorer2.png)
...
* Если вы хотите сохранить метаданные (списки управления доступом, метки времени и атрибуты файлов) при передаче данных в Файлы Azure, следуйте инструкциям из статьи [Сохранение файловых списков управления доступом, атрибутов и меток времени с помощью Azure Data Box](data-box-file-acls-preservation.md).  
* Если данные, отправляемые Data Box, также отправляются другим приложением за пределами Data Box, это может привести к сбоям заданий отправки и повреждению данных.
...
![Уведомление об ошибке копирования на странице "Подключиться и копировать"](media/data-box-deploy-copy-data/view-errors-1.png)

Выберите **Скачать список ошибок**.

![Скачивание и просмотр ошибок на странице "Подключиться и копировать" 2](media/data-box-deploy-copy-data/view-errors-2.png)
...
![Скачивание и просмотр ошибок на странице "Подключиться и копировать" 3](media/data-box-deploy-copy-data/view-errors-3.png)
