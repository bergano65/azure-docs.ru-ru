...
description: Описание Azure Data Box функций безопасности устройства, службы и данных, которые находятся в Data Box.
services: databox
author: alkohli
ms.service: databox
ms.subservice: pod
...
- **Облачное хранилище** — место хранения данных в облаке Azure. Это обычно учетная запись хранения, связанная с созданным ресурсом Azure Data Box.

На следующей схеме показан поток данных через решение Azure Data Box из локальной среды в Azure и различные функции безопасности, как данные проходят через решение. Этот поток предназначен для импорта Data Box.

![Безопасность при импорте Data Box](media/data-box-security/data-box-security-import.png)

На следующей схеме показан порядок экспорта Data Box.

![Data Box экспорт безопасности](media/data-box-security/data-box-security-export.png)
...
- Управляет доступом к устройствам с помощью ключа доступа для разблокировки устройства. Этот ключ защиты защищен ключом шифрования. Для защиты ключа доступа можно использовать собственный ключ, управляемый клиентом. Дополнительные сведения см. в статье [Использование ключей, управляемых клиентом, в Azure Key Vault для Azure Data Box](data-box-customer-managed-encryption-key-portal.md).
- доступ к учетным данным для копирования данных на устройство и из него. Каждый доступ к странице **учетные данные устройства** в портал Azure записывается в [журналы действий](data-box-logs.md#query-activity-logs-during-setup).
- Вы можете использовать собственные пароли для доступа к устройствам и совместному использованию. Дополнительные сведения см. в разделе [учебник. порядок Azure Data Box](data-box-deploy-ordered.md).
...
Устройство с Data Box позволяет использовать собственные сертификаты и устанавливать их для подключения к локальному веб-ИНТЕРФЕЙСу и хранилищу больших двоичных объектов. Дополнительные сведения см. в статье [Использование собственных сертификатов с Data Box и Data Box Heavy устройствами](data-box-bring-your-own-certificates.md).

### <a name="data-box-data-protection"></a>Защита данных Data Box

Входные и выходные данные Data Box защищены следующими функциями:

- 256-битное шифрование AES для неактивных данных; В среде с высоким уровнем безопасности можно использовать двойное шифрование на основе программного обеспечения. Дополнительные сведения см. в разделе [учебник. порядок Azure Data Box](data-box-deploy-ordered.md).
- зашифрованные протоколы могут использоваться для данных в процессе использования; Мы рекомендуем использовать SMB 3,0 с шифрованием для защиты данных при копировании на них с серверов данных.
- Защита очистки данных с устройства после отправки в Azure завершена. Удаление данных осуществляется в соответствии с рекомендациями, приведенными в [Приложении А для жестких дисков ATA из стандарта NIST 800-88r1](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-88r1.pdf). Событие удаления данных фиксируется в [журнале заказов](data-box-logs.md#download-order-history).

### <a name="data-box-service-protection"></a>Защита службы Data Box

Служба Data Box защищена с помощью следующих функций.

- Для доступа к службе Data Box требуется, чтобы у вашей организации была подписка Azure, которая включает Data Box. Подписка обеспечивает доступ к функциям, предоставляемым на портале Azure.
...
- Служба Data Box хранит пароль для разблокировки, который используется для разблокировки устройства в службе.
...
- **Сведения о заказе** — когда заказ создан, адрес доставки, электронная почта и контактные данные пользователя сохраняются в портал Azure. Сохраненная информация включает следующее.
