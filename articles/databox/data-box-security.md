...
По мере передачи данных в этом решении фиксируются события и создаются журналы. Дополнительные сведения см. в статье об [отслеживании и ведении журнала событий в Azure Data Box](data-box-logs.md).
...
- безопасное удаление данных с устройства после завершения передачи данных в Azure. Удаление данных осуществляется в соответствии со стандартами NIST 800-88r1. Событие удаления данных фиксируется в [журнале заказов](data-box-logs.md#download-order-history).
...
- Доступ к заказу Data Box можно контролировать с помощью механизма управления доступом на основе ролей (RBAC). Дополнительные сведения см. в разделе о [настройке управления доступом для заказа Data Box](data-box-logs.md#set-up-access-control-on-the-order).
...
|[NIST SP 800-88r1](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-88r1.pdf)      | Для очистки данных         |
