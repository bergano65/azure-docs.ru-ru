...
- Если вы указали управляемых дисков в том порядке, просмотрите следующие факторы:

    - Вы может иметь только один управляемый диск с указанным именем в группе ресурсов все готовые папки и в разных диск Data Box. Это означает, что дисков VHD, переданных в предварительно созданных папки должны иметь уникальные имена. Убедитесь, что имя не соответствует уже существующего управляемого диска в группе ресурсов. Если виртуальные жесткие диски имеют одинаковые имена, только один виртуальный жесткий ДИСК преобразуется в управляемый диск с таким именем. Другие виртуальные жесткие диски, передаются как страничных BLOB-объектов в промежуточную учетную запись хранения.
    - Всегда копировать виртуальные жесткие диски в одну из предварительно созданных папок. При копировании виртуальных жестких дисков за пределами этих папок или в папке, которую вы создали виртуальные жесткие диски, передаются в учетную запись хранения Azure как страничных BLOB-объектов и не управляемые диски.
    - Можно отправить только фиксированные VHD для создания управляемых дисков. Виртуальные жесткие диски динамических, виртуальные жесткие диски разностный или VHDX файлы не поддерживаются.
