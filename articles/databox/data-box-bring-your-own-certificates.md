...
6. Установите новый сертификат на клиентском компьютере, который вы используете для доступа к локальному веб-ИНТЕРФЕЙСу. Инструкции см. [в разделе Импорт сертификатов в клиент](#import-certificates-to-client).

7. Если вы изменили сертификат для локального веб-интерфейса, необходимо перезапустить браузер, а затем локальный веб-интерфейс. Этот шаг необходим, чтобы избежать проблем с кэшем SSL.
