...
|`getaddrinfo ENOTFOUND <accountname>.blob.<serialnumber>.microsoftdatabox.com` |Убедитесь, что имя `<accountname>.blob.<serialnumber>.microsoftdatabox.com` конечной точки Добавлено в файл hosts по указанному пути: <li>`C:\Windows\System32\drivers\etc\hosts`в Windows или </li><li> `/etc/hosts`в Linux.</li>|
...
|Сообщение об ошибке  |Рекомендованное действие |
|---------|---------|
|Команда AzCopy зависает в течение минуты, после чего отобразится следующее сообщение об ошибке: <br>Не удалось перечислить каталог https://... Не удалось разрешить удаленное имя`<accountname>.blob.<serialnumber>.microsoftdatabox.com`|Убедитесь, что имя `<accountname>.blob.<serialnumber>.microsoftdatabox.com` конечной точки Добавлено в файл hosts по адресу:. `C:\Windows\System32\drivers\etc\hosts`|
|Команда AzCopy зависает в течение минуты, после чего отобразится следующее сообщение об ошибке: <br>Ошибка при анализе расположения источника. Базовое соединение закрыто: не удалось установить доверительные отношения для защищенного канала SSL/TLS.|Импортируйте сертификат TLS/SSL для устройства в хранилище сертификатов системы. Дополнительные сведения см. в подразделе [Загрузка сертификата](data-box-deploy-copy-data-via-rest.md#download-certificate).|
...
|Сообщение об ошибке  |Рекомендованное действие |
|---------|---------|
|Команда AzCopy зависает в течение 20 минут, после чего отобразится следующее сообщение об ошибке: <br>Ошибка при анализе расположения `https://<accountname>.blob.<serialnumber>.microsoftdatabox.com/<cntnr>`источника. Отсутствует такое устройство или адрес|Убедитесь, что имя `<accountname>.blob.<serialnumber>.microsoftdatabox.com` конечной точки Добавлено в файл hosts по адресу:. `/etc/hosts`|
