...
Существуют следующие варианты экспорта этих файлов: 

- Вы можете передавать файлы, которые не удалось скопировать по сети. 
- Если размер данных превышает объем используемой емкости устройства, происходит частичная копия, и все файлы, которые не были скопированы, перечислены в этом журнале. Этот журнал можно использовать в качестве входного XML-файла, чтобы создать новый Data Box порядок, а затем скопировать эти файлы.

### <a name="verbose-log"></a>Подробный журнал

*Подробный журнал* содержит список всех файлов, успешно экспортированных из учетной записи хранения Azure. Журнал также содержит размер файла и вычисление контрольной суммы.
