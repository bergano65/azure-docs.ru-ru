---
title: Администрирование Azure Data Box/Azure Data Box Heavy с помощью локального веб-мина
description: Описывает, как использовать локальный веб-интерфейс для администрирования устройств Data Box и Data Box Heavy
services: databox
author: alkohli
ms.service: databox
ms.subservice: pod
...
---
# <a name="use-the-local-web-ui-to-administer-your-data-box-and-data-box-heavy"></a>Используйте локальный веб-ui для администрирования коробки данных и коробки данных Heavy

В этой статье описаны некоторые задачи конфигурации и управления, которые могут быть выполнены на устройствах Data Box и Data Box Heavy. Управлять устройствами Data Box и Data Box Heavy можно через uI портала Azure и локальный веб-интерфейс устройства. В этой статье речь идет о задачах, выполняемых с использованием локального пользовательского веб-интерфейса.

Для начальной конфигурации устройства используется локальный веб-мина для Box data Box и Data Box Heavy. Вы также можете использовать локальный веб-чат-файл, чтобы закрыть или перезапустить устройство, выполнить диагностические тесты, обновить программное обеспечение, просмотреть журналы копий и создать пакет журналов для поддержки Майкрософт. На устройстве Data Box Heavy с двумя независимыми узлами можно получить доступ к двум отдельным локальным веб-файлам, соответствующим каждому узлу устройства.
...
Если возникают проблемы с устройством, из системных журналов можно создать пакет поддержки. Служба поддержки Майкрософт использует этот пакет для устранения неполадок. Для создания пакета поддержки следующие действия:
...
Вы можете отключить или перезапустить устройство с помощью локального веб-мина. Перед тем как перезапустить устройство, рекомендуется отключить автономный доступ к ресурсам на хосте, а затем на устройстве. Это уменьшает возможность повреждения данных. Убедитесь, что при завершении работы устройства не выполняется копирование данных.

Чтобы выключить устройство, примите следующие меры.
...
Билль материалов (BOM) или файлы манифеста содержат список файлов, которые копируются в Box или Data Box Heavy. Эти файлы генерируются при подготовке устройства к отправке.

Прежде чем начать, убедитесь, что ваше устройство завершило **Подготовка к отправке** шаг. Чтобы скачать файл метки порядка байтов или манифеста, сделайте следующее:

1. Перейдите на локальный веб-файл ы для вашего устройства. Вы увидите, что устройство завершило подготовку к отправке. По завершении подготовки состояние устройства отобразится как **Все готово к поставке**.
...
> [!NOTE]
> На коробке данных Heavy присутствуют два набора файлов (BOM-файлы), соответствующие двум узлам на устройстве.
...
Этот файл содержит список всех файлов, которые были скопированы на box data Box или Data Box Heavy. В этом файле значение *crc64* соотносится с контрольной суммой, созданной для соответствующего файла.
...
1. В правом верхнем углу локального веб-ui вашего устройства перейдите в **настройки.**
...
- Узнайте, как [управлять коробкой данных и коробкой данных пулеметчиком через портал Azure.](data-box-portal-admin.md)
