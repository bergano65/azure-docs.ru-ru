...
1. В локальном веб-ИНТЕРФЕЙСе перейдите в службу **поддержки**. При необходимости выберите **включить дамп памяти**. Затем выберите **создать пакет поддержки**.

    Дамп памяти — это содержимое памяти устройства, которое сохраняется после сбоя системы.

    Не следует выбирать параметр **включить дамп памяти** , если только поддержка не запрашивается. Сбор пакета поддержки, содержащего дамп памяти, и включение в него конфиденциальных данных занимает много времени.

    ![Создание пакета поддержки 1](media/data-box-local-web-ui-admin/create-support-package-1.png)

    Создание пакета поддержки. Эта операция занимает несколько минут, если включены только системные журналы. При включении дампа памяти он занимает гораздо больше времени.
