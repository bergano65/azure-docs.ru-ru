...
description: Из этого учебника вы узнаете, как присоединить кабели и подключиться к Azure Data Box, а также как включить это устройство.
services: databox
author: alkohli
ms.service: databox
ms.subservice: pod
...
1. Вы разместили заказ на Azure Data Box.
    ...
    - Сведения о заказе на экспорт см. в статье [Руководство Заказ Azure Data Box](data-box-deploy-export-ordered.md).
1. Вы получили Data Box; состояние заказа на портале **Доставлено**. 
...
1. Вы ознакомились с [руководством по безопасности Data Box](data-box-safety.md).
1. Вы получили один шнур питания с заземлением для использования с устройством хранения размером 100 ТБ.
1. У вас есть главный компьютер, который используется для копирования данных (заказ на импорт) в Data Box или из него (заказ на экспорт). На главном компьютере должно быть следующее ПО:
...
1. Data Box нужно разместить на плоской поверхности. Если вы хотите расположить устройство на стандартной полке в стойке центра обработки данных, в ней должен быть слот на 7 U. Устройство можно разместить в стойке горизонтально или вертикально.
1. Требуются следующие кабели для подключения Data Box к главному компьютеру:
...
> [Копирование данных на Azure Data Box для выполнения заказа на импорт](./data-box-deploy-copy-data.md)

либо

> [!div class="nextstepaction"]
> [Копирование данных из Azure Data Box для выполнения заказа на экспорт](./data-box-deploy-export-copy-data.md)
