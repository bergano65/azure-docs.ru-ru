---
title: Управление Azure Data Box, Azure Data Box большой с помощью портала Azure | Документация Майкрософт
description: Описывает способ администрирования Azure Data Box и интенсивной поле данных Azure с помощью портала Azure.
services: databox
author: alkohli
ms.service: databox
ms.subservice: pod
...
ms.locfileid: "66427717"
---
# <a name="use-the-azure-portal-to-administer-your-azure-data-box-and-azure-data-box-heavy"></a>Для администрирования Azure Data Box и интенсивной поле данных Azure с помощью портала Azure

Эта статья относится к Azure Data Box и Azure Data Box большой. В этой статье описываются некоторые сложные рабочие процессы и задачи управления, которые могут выполняться на устройстве Azure Data Box. Можно управлять устройство Data Box на портале Azure или с помощью локального пользовательского веб-интерфейса.

В этой статье речь идет о задачах, выполняемых с использованием портала Azure. Управление заказами, управлять устройство Data Box с помощью портала Azure и отслеживание состояния заказа прошло до завершения.


## <a name="cancel-an-order"></a>Отмена заказа

Возможно, после размещения заказа вам нужно будем отменить его по различным причинам. Можно отменить заказ только перед его обработкой. После обработки заказа и подготовки устройство Data Box, не поддерживается Отмена заказа.
...
Клонирование полезно в определенных ситуациях. Например, пользователь использовал Data Box для передачи данных. Как создается больше данных, то есть потребность в другое устройство Data Box для передачи этих данных в Azure. В этом случае заказ можно просто клонировать.
...
Если отображение дисплея E-ink вашего Data Box не работает и не отображается транспортная этикетка для возврата, необходимо ее загрузить. Имеется без отображения рукописного ввода E на большой поле данных и поэтому этот рабочий процесс не применяется к большой поле данных.
...
2. Щелкните **Скачать журнал заказов**. В скачанном журнале вы увидите запись журналов отслеживания от перевозчика. Будет существовать два набора журнала, соответствующий двумя узлами на большой поле данных. Если вы прокрутите страницу этого журнала до конца, то увидите ссылки на такие элементы:
...
- Узнайте, как [Устранение Data Box и поле данных в большой проблемы](data-box-troubleshoot.md).
