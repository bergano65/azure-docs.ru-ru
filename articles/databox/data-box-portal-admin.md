---
title: Управляйте ящиком данных Azure, Azure Data Box Heavy через портал Azure (ru) Документы Майкрософт
description: Описывает, как использовать портал Azure для администрирования ящика данных Azure и Azure Data Box Heavy.
services: databox
author: alkohli
ms.service: databox
ms.subservice: pod
...
ms.locfileid: "79259998"
---
# <a name="use-the-azure-portal-to-administer-your-azure-data-box-and-azure-data-box-heavy"></a>Используйте портал Azure для администрирования коробки данных Azure и Azure Data Box Heavy

Эта статья относится как к Коробке данных Azure, так и к коробке данных Azure Heavy. В этой статье описаны некоторые сложные рабочие процессы и задачи управления, которые могут быть выполнены на устройстве Azure Data Box. Управлять устройством Data Box можно через портал Azure или через локальный веб-файл ы.

В этой статье речь идет о задачах, выполняемых с использованием портала Azure. Используйте портал Azure для управления заказами, управления устройством Data Box и отслеживания состояния заказа по мере его завершения.


## <a name="cancel-an-order"></a>Отмена заказа

Возможно, после размещения заказа вам нужно будем отменить его по различным причинам. Можно отменить заказ только перед его обработкой. После обработки заказа и подготовки устройства Data Box отменить заказ невозможно.
...
Клонирование полезно в определенных ситуациях. Например, пользователь использовал Data Box для передачи данных. По мере того, как создается больше данных, необходимо, чтобы другое устройство Box данных перевалывало эти данные в Azure. В этом случае заказ можно просто клонировать.
...
Если отображение дисплея E-ink вашего Data Box не работает и не отображается транспортная этикетка для возврата, необходимо ее загрузить. На папке Данных на коробке данных нет дисплея, и поэтому этот рабочий процесс не применяется к Data Box Heavy.
...
2.  Загрузится новая транспортная этикетка для возврата, которая выглядит следующим образом. Сохраните этикетку и распечатайте ее. Сложите и вставьте этикетку в прозрачный рукав на устройстве. Убедитесь, что этикетка является видимой. Удалите все наклейки, которые находятся на устройстве от предыдущей доставки.
...
2. Щелкните **Скачать журнал заказов**. В скачанном журнале вы увидите запись журналов отслеживания от перевозчика. На коробке данных Heavy будет два набора журналов, соответствующих двум узлам. Если вы прокрутите страницу этого журнала до конца, то увидите ссылки на такие элементы:
   ...
   - **Журналы аудита** - содержат информацию о том, как включить и получить доступ к акциям в окне данных, когда он находится за пределами центра обработки данных Azure.
...
|Завершено с ошибками| Копирование данных завершено, но во время процесса произошли ошибки. <br> Просмотрите журналы копирования, используя путь, указанный на портале Azure. Смотрите [примеры журналов копий при загрузке, завершенной с ошибками.](https://docs.microsoft.com/azure/databox/data-box-logs#upload-completed-with-errors)   |
|завершено с предупреждениями.| Копия данных была завершена, но ваши данные были изменены. В данных были некритические ошибки с именами или именами файлов, которые были исправлены путем изменения имен файлов или кабл. <br> Просмотрите журналы копирования, используя путь, указанный на портале Azure. Обратите внимание на изменения в ваших данных. Смотрите [примеры журналов копий при загрузке, завершенной с предупреждениями.](https://docs.microsoft.com/azure/databox/data-box-logs#upload-completed-with-warnings)   |
...
- Узнайте, как [проблемы с устранением данных и проблемы Сводочная коробка данных.](data-box-troubleshoot.md)
