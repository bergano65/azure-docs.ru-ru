...
4. Проверьте, доступна ли служба Azure Data Box в вашем регионе. Введите или выберите следующие сведения и нажмите кнопку **Применить**.
    ...
    |групп ресурсов;     |    Выберите имеющуюся группу ресурсов. <br> Группа ресурсов — это логический контейнер для ресурсов, которые могут управляться или развертываться вместе.         |
...
1. Если вы хотите включить двойное шифрование на основе программного обеспечения, в области **безопасности**выберите **включить двойное шифрование для заказа**. 

   Программное шифрование выполняется в дополнение к шифрованию данных AES-256 на Data Box.

   > [!NOTE]
   > Включение этого параметра может потребовать больше времени на обработку заказов и копирование данных. Этот параметр нельзя изменить после создания заказа.

   ![Экран безопасности для импорта полей данных, двойное шифрование](media/data-box-deploy-export-ordered/azure-data-box-export-05b.png)
