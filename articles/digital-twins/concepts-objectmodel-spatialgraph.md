...
- **Пробелы** — это виртуальные или физические расположения, например `Tenant` `Customer` `Region`,,, и `Venue`.
...
- **Онтологиес** представляет набор `Default`расширенных типов, `Building` `BACnet`например,, и. `EnergyGrid`
...
- **Роли** — это наборы разрешений, назначенных пользователям и устройствам на пространственном графе, `Space Administrator`например `User Administrator`,, `Device Administrator`и.
...
- **Конечные точки** — это расположения, в которых можно маршрутизировать сообщения телеметрии и Цифровые двойников события, например `Event Hub` `Service Bus`,, и `Event Grid`.
