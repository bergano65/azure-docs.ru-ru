...
| `ResourceID` | Строковый тип | Идентификатор ресурса Azure Resource Manager для ресурса, в котором произошло событие |
| `OperationName` | Строковый тип  | Тип действия, выполняемого во время события |
| `OperationVersion` | Строковый тип | Версия API, использованная во время события |
| `Category` | Строковый тип | Тип выдаваемый ресурса |
| `ResultType` | Строковый тип | Результат события |
| `ResultSignature` | Строковый тип | Код состояния HTTP для события |
| `ResultDescription` | Строковый тип | Дополнительные сведения о событии |
| `DurationMs` | Строковый тип | Время, затраченное на выполнение события в миллисекундах |
| `CallerIpAddress` | Строковый тип | Маскированный исходный IP-адрес для события |
| `CorrelationId` | Guid | Клиент предоставил уникальный идентификатор для события |
| `Level` | Строковый тип | Серьезность ведения журнала события |
| `Location` | Строковый тип | Регион, в котором произошло событие |
...
| `Level` | Строковый тип | Серьезность ведения журнала события |
| `Location` | Строковый тип | Регион, в котором произошло событие |
| `EndpointName` | Строковый тип | Имя конечной точки исходящего трафика, созданной в Azure Digital двойников |
