...
2. Настройка локальных учетных данных Azure для предоставления доступа к вашему экземпляру
3. Запустите обозреватель ADT и настройте его для подключения к экземпляру. Вы будете использовать **имя узла** исходного экземпляра Azure Digital двойников, который вы перемещаете.
...
Замените *URL-адрес ADT* в соответствии с новым экземпляром. Измените это значение на "чтение *https://{новое имя узла}* ".
