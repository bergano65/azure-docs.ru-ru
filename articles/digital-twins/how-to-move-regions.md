...
Перейдите к примеру: [Azure Digital двойников (ADT) Explorer](/samples/azure-samples/digital-twins-explorer/digital-twins-explorer/). Нажмите кнопку *Download ZIP* (Загрузить ZIP-файл), чтобы загрузить файл *ZIP* этого примера кода с именем _**Azure_Digital_Twins__ADT__explorer.zip**_ на свой компьютер. Распакуйте файл.

Затем установите и настройте разрешения для обозревателя ADT. Для этого следуйте инструкциям в разделе [*Настройка Azure Digital двойников и ADT Explorer*](quickstart-adt-explorer.md#set-up-azure-digital-twins-and-adt-explorer) в кратком руководстве по цифровому двойников в Azure. В этом разделе рассматриваются следующие шаги:
1. Настройка экземпляра Digital двойников для Azure (вы можете пропустить эту часть, так как у вас уже есть экземпляр)
2. Настройка **регистрации приложения Azure AD** для предоставления доступа к вашему экземпляру
3. Настройка разрешений для запуска обозревателя ADT на компьютере
4. Запустите обозреватель ADT и настройте его для подключения к экземпляру. Вы будете использовать **имя узла** исходного перемещаемого экземпляра Azure Digital ДВОЙНИКОВ и **идентификатор клиента** и **идентификатор клиента** из регистрации приложения.
...
Вы можете повторно использовать ту же регистрацию приложения, поэтому достаточно заменить *URL-адрес ADT* для отражения нового экземпляра. Измените это значение на "чтение *https://{новое имя узла}* ".
