...
ms.locfileid: "85392205"
---
# <a name="dtdl-client-side-parser-library"></a>Библиотека синтаксического анализатора ДТДЛ на стороне клиента

[Модели](concepts-models.md) в Azure Digital двойников определяются с помощью языка определения цифровых двойников на основе JSON-LD (дтдл). В случаях, когда полезно анализировать модели, в NuGet.org в качестве клиентской библиотеки предоставляется библиотека синтаксического анализа ДТДЛ: [Microsoft. Azure. дигиталтвинс. Parser](https://nuget.org/packages/Microsoft.Azure.DigitalTwins.Parser/).

Эта библиотека предоставляет доступ к моделям для определения ДТДЛ, фактически действуя в качестве эквивалента отражения C# для ДТДЛ. Эту библиотеку можно использовать независимо от любого [пакета SDK Azure Digital двойников](how-to-use-apis-sdks.md), особенно для проверки дтдл в визуальном или текстовом редакторе. Прежде чем пытаться передать их в службу, полезно убедиться, что файлы определения модели действительны.
...
Существует пример кода, который может проверить документы модели, чтобы убедиться, что ДТДЛ является допустимым. Он основан на библиотеке анализатора ДТДЛ и не зависит от языка. Его можно найти здесь: [Пример средства проверки дтдл](https://docs.microsoft.com/samples/azure-samples/dtdl-validator/dtdl-validator).

Образец проверяющего элемента управления можно использовать в качестве программы командной строки для проверки дерева каталогов файлов ДТДЛ. Он также предоставляет интерактивный режим. В исходном коде приведены примеры использования библиотеки анализатора.
...
## <a name="use-the-parser-library-in-code"></a>Использование библиотеки синтаксического анализатора в коде

Кроме того, библиотеку анализатора можно использовать напрямую, например, для проверки моделей в собственном приложении или для создания динамических, управляемых моделями пользовательского интерфейса, панелей мониторинга и отчетов.
