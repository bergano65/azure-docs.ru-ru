...
1. На этапе _проверки_ входящее сообщение телеметрии преобразуется в общепринятый формат [**объекта передачи данных**](https://en.wikipedia.org/wiki/Data_transfer_object). На этом этапе также выполняется проверка устройств и датчиков.
