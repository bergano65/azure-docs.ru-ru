...
>[!NOTE]
> В этом примере для проверки подлинности используется `DefaultAzureCredential`. Сведения о других типах учетных данных см. в документации для [библиотек проверки подлинности платформы удостоверений Майкрософт](../active-directory/develop/reference-v2-libraries.md) или в статье Azure Digital Twins о [проверке подлинности клиентских приложений](how-to-authenticate-client.md).
...
Чтобы упростить создание связей, в этом примере кода используется пространство имен `Azure.DigitalTwins.Core.Serialization`. Вы добавили его в проект ранее с помощью инструкции `using`:
