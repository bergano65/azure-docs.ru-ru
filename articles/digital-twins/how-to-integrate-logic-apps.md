...
В оставшейся части этого раздела будут рассмотрены следующие шаги:
...
:::image type="content" source="media/how-to-integrate-logic-apps/add-client-secret.png" alt-text="Добавить секрет клиента":::
...
:::image type="content" source="media/how-to-integrate-logic-apps/client-secret-value.png" alt-text="Копировать значение секрета клиента":::
...
На следующей странице *создание Logic Apps пользовательского соединителя* выберите подписку и группу ресурсов, а также имя и расположение развертывания для нового соединителя. Нажмите кнопку " *Проверка" + "создать*". 

:::image type="content" source="media/how-to-integrate-logic-apps/create-logic-apps-custom-connector.png" alt-text="Страница "Создание Logic Apps пользовательского соединителя" в портал Azure.":::

Откроется вкладка *Просмотр и создание* , где можно нажать кнопку *создать* внизу, чтобы создать ресурс.

:::image type="content" source="media/how-to-integrate-logic-apps/review-logic-apps-custom-connector.png" alt-text="Вкладка "Проверка и создание" страницы "Проверка Logic Apps пользовательского соединителя" в портал Azure. Выделение вокруг кнопки "создать"":::
...
* **Общие сведения**
    - Icon: отправка значка, который вам нравится
    - Цвет фона значка: Введите шестнадцатеричный код в формате "#xxxxxx" для своего цвета.
...
Перейдите на страницу [Logic Apps (потребление)](https://portal.azure.com/#blade/HubsExtension/BrowseResourceBlade/resourceType/Microsoft.Logic%2Fworkflows) в портал Azure (вы можете использовать эту ссылку или найти ее на панели поиска портала). Нажмите кнопку *Добавить* , чтобы создать приложение логики.

:::image type="content" source="media/how-to-integrate-logic-apps/create-logic-app.png" alt-text="Страница "Logic Apps (потребление)" в портал Azure. Нажмите кнопку "Добавить"":::

На следующей странице *Logic Apps (потребление)* введите подписку, группу ресурсов. Кроме того, выберите имя приложения логики и укажите расположение.

Нажмите кнопку " _проверить и создать_ ".
...
* _идентификатор_. Укажите *идентификатор двойника* цифрового двойника в экземпляре, который требуется обновить в приложении логики.
* _двойника_: в этом поле вводится текст, требуемый для выбранного запроса API. Для *дигиталтвинсупдате*это тело в виде кода исправления JSON. Дополнительные сведения о структурировании исправления JSON для обновления двойника см. в разделе " [Обновление цифрового двойника](how-to-manage-twin.md#update-a-digital-twin) " статьи " *Управление цифровыми двойниковми*".
