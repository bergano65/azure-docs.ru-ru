...
ms.locfileid: "84725590"
---
# <a name="query-the-azure-digital-twins-twin-graph"></a>Запрос к графу Azure Digital двойников двойника

В этой статье подробно описано, как использовать [язык хранилища запросов Digital двойников](concepts-query-language.md) для запроса информации в [графе двойника](concepts-twins-graph.md) . Запросы выполняются на графе с помощью [**API-интерфейсов запросов**](how-to-use-apis-sdks.md)двойников Azure Digital.
...
При выполнении запросов, основанных на связях Digital двойников, язык хранилища запросов Azure Digital двойников имеет специальный синтаксис.
...
Вспомним, что с возможностями [модели](concepts-models.md) цифровых двойников Azure отношения не существуют независимо от двойников. Это означает, что язык хранилища запросов Digital двойников `JOIN` немного отличается от общего SQL `JOIN` , так как связи здесь не могут запрашиваться отдельно и должны быть привязаны к двойника.
...
Аналогично тому, как цифровые двойников имеют свойства, описанные в ДТДЛ, связи могут также иметь свойства. Язык хранилища запросов Microsoft Azure Digital двойников позволяет фильтровать и проекцию связей путем назначения псевдонима связи в `JOIN` предложении. 
...
* В общедоступной предварительной версии глубина обхода графа ограничена: `JOIN` для каждого запроса допускается только один.
