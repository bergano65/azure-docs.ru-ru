...
* Используйте операторы сравнения запросов: `AND` / `OR` / `NOT` , `IN` / `NOT IN` , `STARTSWITH` / `ENDSWITH` , `=` , `!=` , `<` , `>` , `<=` , `>=` .
* Использовать продолжение: экземпляр объекта запроса создается с размером страницы (до 100). Вы можете получить цифровое двойников по одной странице за раз, повторив вызовы `nextAsTwin` метода.
