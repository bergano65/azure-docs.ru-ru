---
title: Как создать функции, определяемые пользователями - в Azure Digital Twins Документы Майкрософт
...
Параметр **телеметрии** предоставляет атрибуты **SensorId** и **Сообщения,** соответствующие сообщению, отправленному датчиком. Параметр **ExecutionContext** предоставляет следующие атрибуты.
...
1. [Запросите API системы](./security-create-manage-role-assignments.md#retrieve-all-roles) для всех ролей, чтобы получить идентификатор роли, которую необходимо назначить определяемой пользователем функции. Сделайте это, создавая аутентифицированный запрос HTTP GET.
    ...
    ```
   Сохраните идентификатор требуемой роли. Он будет принят как JSON тела`YOUR_DESIRED_ROLE_IDENTIFIER`атрибут **roleId** ( ) ниже.

1. **objectId** `YOUR_USER_DEFINED_FUNCTION_ID`() будет пользовательским идентификатором функции, который был создан ранее.
1. Найти значение **пути** `YOUR_ACCESS_CONTROL_PATH`( ) путем запроса пробелов с `fullpath`.
...
1. Вставьте возвращенное значение `spacePaths` в **path**, чтобы создать назначение роли определяемой пользователем функции, сделав аутентифицированный HTTP-запрос POST в следующее расположение.
    ...
    | YOUR_USER_DEFINED_FUNCTION_TYPE_ID | Идентификатор с указанием`UserDefinedFunctionId`типа функции, определяемой пользователем () |
