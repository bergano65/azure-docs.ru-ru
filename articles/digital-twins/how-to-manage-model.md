...
После передачи модели в экземпляр весь интерфейс модели является неизменяемым. Это означает отсутствие традиционного «редактирования» моделей.

Вместо этого, если вы хотите внести изменения в модель в Azure Digital двойников, то способ сделать это — отправить **более новую версию** той же модели. Во время предварительной версии при попыток использовать версию модели можно только удалить поля, а не добавлять новые поля (для добавления новых полей нужно просто [создать новую модель](#create-models)).
...
Затем передайте новую версию модели в экземпляр. Он займет старую версию, а новые двойников, создаваемые с помощью этой модели, будут использовать обновленную версию.
