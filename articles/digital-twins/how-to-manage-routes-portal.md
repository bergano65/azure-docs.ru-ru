...
Определение маршрута события может содержать следующие элементы:
...
Если идентификатор маршрута отсутствует, сообщения не направляются за пределы Azure Digital двойников.
Если имеется идентификатор маршрута и фильтр имеет значение `true` , все сообщения направляются в конечную точку.
Если имеется идентификатор маршрута и добавлен другой фильтр, сообщения будут отфильтрованы на основе фильтра.
...
На открывшейся странице *Создание маршрута события* выберите в поле _имя_ минимальное имя маршрута и выберите _конечную точку_ , которую вы хотите использовать для создания маршрута из раскрывающегося списка.

:::image type="content" source="media/how-to-manage-routes-portal/create-event-route-no-filter.png" alt-text="Снимок экрана создания маршрута события для экземпляра.":::
...
Чтобы добавить фильтр при создании маршрута событий, используйте раздел _Добавление фильтра маршрутов событий_ страницы *Создание маршрута события* . 
...
Чтобы использовать основные фильтры, разверните параметр _типы событий_ и установите флажки, соответствующие событиям, по которым вы хотите выполнить фильтрацию. 
