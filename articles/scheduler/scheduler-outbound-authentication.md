...
| **тип** | Да | Тип проверки подлинности. Для обычной проверки подлинности это значение равно `Basic`. | 
| **Пользователя** | Да | Имя пользователя для проверки подлинности | 
| **пароль** | Да | Пароль для проверки подлинности |
|||| 
...
|---------|-------------|
| **authentication** (родительский элемент) | Объект проверки подлинности для использования обычной проверки подлинности |
| **тип** | Тип проверки подлинности. Для обычной проверки подлинности это значение равно `Basic`. |
| **Пользователя** | Имя пользователя, прошедшего проверку подлинности |
||| 
...
| **Арендатор** | Да | Идентификатор клиента Azure AD. Идентификатор клиента Azure AD можно узнать, выполнив команду `Get-AzureAccount` в Azure PowerShell. |
| **Аудитории** | Да | Этот параметр имеет значение `https://management.core.windows.net/`. | 
| **Clientid** | Да | Идентификатор клиента для приложения Azure AD | 
| **Секрет** | Да | Секретные данные клиента, запрашивающего маркер. | 
|||| 
...
| **Арендатор** | Идентификатор клиента Azure AD. |
| **Аудитории** | Этот параметр имеет значение `https://management.core.windows.net/`. |
