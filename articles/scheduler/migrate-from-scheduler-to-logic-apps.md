...
Дополнительные сведения см. в статье [Что такое Azure Logic Apps](../logic-apps/logic-apps-overview.md) или попробуйте создать свое первое приложение логики в этом кратком руководстве: [Создайте свое первое приложение логики](../logic-apps/quickstart-create-first-logic-app-workflow.md).
...
1. На [портале Azure](https://portal.azure.com) создайте пустое приложение логики в конструкторе приложений логики. 
   ...
   Основные шаги, выполните [краткое руководство: Создание первого автоматизированного рабочего процесса с помощью Azure Logic Apps на портале Azure](../logic-apps/quickstart-create-first-logic-app-workflow.md).
...
| Метод запроса | URL-адрес | Основной текст | Заголовки |
|----------------|-----|------|---------| 
| **POST** | <*endpoint-URL*> | **raw** (Без форматирования) <p>**JSON (application/json)** <p>В поле **raw** (Без форматирования) введите полезные данные, которые нужно отправить в запросе. <p>**Примечание**. Этот параметр автоматически настраивает **заголовки** значения. | **Ключ**: Content-Type <br>**Значение**: application/json
 |||| 
...
>
> ![Отклик](./media/migrate-from-scheduler-to-logic-apps/postman-response.png)
...
**Вопрос**. Когда планировщик Azure прекращает работу? <br>
**Ответ**. Планировщик Azure планируется выведен из эксплуатации 30 сентября 2019 г.

**Вопрос**. Что произойдет с Мои коллекции заданий планировщика и задания, после завершение службы? <br>
**Ответ**. Все задания и коллекции заданий планировщика будут удалены из системы.

**Вопрос**. У для резервного копирования или выполнения других задач, перед миграцией Мои задания планировщика в Logic Apps? <br>
**Ответ**. Как рекомендуется всегда создавать резервные копии работы. Убедитесь, что созданные вами приложения логики работают, как ожидалось, прежде чем удалять или отключать задания планировщика. 

**Вопрос**. Есть ли это средство, которое может помочь мне перенести Мои задания из планировщика в Logic Apps? <br>
**Ответ**. Каждое задание планировщика является уникальным, поэтому не существует универсального средства. Тем не менее будут доступны различные сценарии, которые можно будет изменить для ваших нужд. Доступность сценариев можно будет проверить позже.

**Вопрос**. Где можно получить поддержку по переносу Мои задания планировщика? <br>
**Ответ**. Ниже приведены некоторые способы для получения поддержки. 
