...
> [!IMPORTANT]
> [Azure Logic Apps](../logic-apps/logic-apps-overview.md) заменяет планировщик Azure, который выводится из [эксплуатации](#retire-date). Чтобы продолжить работу с заданиями, настроенными в планировщике, выполните миграцию в Azure Logic Apps как можно скорее, следуя этой статье. 
>
...
1. В поле поиска введите `when a http request` , чтобы найти триггер запроса. В списке триггеров выберите триггер **При получении HTTP-запроса**.

   ![Добавление триггера запросов](./media/migrate-from-scheduler-to-logic-apps/request-trigger.png)

1. Для триггера запроса можно дополнительно предоставить схему JSON, которая помогает конструктору приложений логики понять структуру входных данных, включенных в входящий вызов триггера запроса, и сделает выводы более удобными для выбора в рабочем процессе.

   В поле **схема JSON текста запроса** введите схему, например:
   ...
   1. В разделе **введите или вставьте образец полезных данных JSON**, укажите полезные данные примера и выберите **Готово**, например:
...
1. Введите отметку времени, когда требуется запустить рабочий процесс приложения логики.
   ...
   Если щелкнуть внутри поля **метки времени** , появится список динамического содержимого, в котором можно дополнительно выбрать выходные данные триггера.
...
|----------------|-----|------|---------|
| **POST** | <*URL-адрес конечной точки*> | **чтен** <p>**JSON (application/json)** <p>В поле **необработанные** введите полезные данные, которые необходимо отправить в запросе. <p>**Примечание**. Этот параметр автоматически настраивает значения **Headers** (Заголовки). | **Ключ**: Content-Type <br>**Значение**: Application/JSON |
|||||
