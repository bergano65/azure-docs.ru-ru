---
title: Основные понятия, термины и сущности
...
| Сущность | Описание |
|--------|-------------|
| **Работу** | Определяет одно повторяющееся действие с простой или сложной стратегией выполнения. К действиям относятся HTTP-запросы, запросы к очереди хранилища, запросы к очереди служебной шины и запросы к разделу служебной шины. | 
...
| **История работы** | Описывает сведения о выполнении задания, например состояние и все данные ответов. |
||| 
...
| [**Действий**](#action) | Да | Подробные сведения об основном действии, которые могут включать объект **errorAction**. | 
...
| [**Повторения**](#recurrence) | нет | Такие сведения, как частота и интервал, для повторяющегося задания. | 
| [**повторная попыткаПолитика**](#retry-policy) | нет | Сведения о том, как часто повторять действие. | 
| [**Государства**](#state) | Да | Сведения о текущем состоянии задания. |
| [**Статус**](#status) | Да | Сведения о текущем состоянии задания, которым управляет служба. |
||||
...
В объекте **startTime** можно указать время начала и смещение часового пояса в [формате ISO 8601](https://en.wikipedia.org/wiki/ISO_8601).
...
|----------|----------|-------|-------------| 
| **Частота** | Да, если используется **recurrence** | "Minute", "Hour", "Day", "Week", "Month", "Year". | Единица измерения времени между выполнениями. | 
| **Интервал** | нет | От 1 до 1000 включительно. | Положительное целое число, которое определяет количество единиц времени между каждым повторением на основании значения **frequency**. | 
| **Расписание** | нет | Различается | Сведения о более сложных и расширенных расписаниях. См. параметры **hours**, **minutes**, **weekDays**, **months** и **monthDays**. | 
| **Часов** | нет | От 1 до 60. | Массив, содержащий часы, который задает время запуска задания. | 
| **Минут** | нет | От 0 до 59 | Массив, содержащий минуты, который задает время запуска задания. | 
| **months** | нет | От 1 до 12. | Массив, содержащий месяцы, который задает время запуска задания. | 
| **monthDays** | нет | Различается | Массив, содержащий дни месяца, который задает время запуска задания. | 
| **Будние дни** | нет | "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday". | Массив, содержащий дни недели, который задает время запуска задания. | 
| **count** | нет | <*Ни один*> | Количество повторений. По умолчанию используется бесконечное повторение. Невозможно одновременно использовать **count** и **endTime**; применяется правило, по которому задание завершается раньше. | 
| **endTime** | нет | <*Ни один*> | Дата и время, когда нужно прекратить повторение. По умолчанию используется бесконечное повторение. Невозможно одновременно использовать **count** и **endTime**; применяется правило, по которому задание завершается раньше. | 
||||
