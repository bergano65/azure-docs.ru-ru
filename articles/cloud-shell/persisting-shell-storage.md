...
Для обеспечения безопасности каждому пользователю следует подготовить собственную учетную запись хранения.  Для управления доступом на основе ролей Azure (Azure RBAC) пользователи должны иметь права доступа участника или выше на уровне учетной записи хранения.
...
Чтобы найти текущий регион, можно запустить `env` Bash и найти переменную `ACC_LOCATION` или запустить PowerShell `$env:ACC_LOCATION` . Файловые ресурсы получают образ размером 5 ГБ, создаваемый для хранения каталога `$Home`.
...
Клиенты должны выбрать основной регион, если только у них нет необходимости хранить свои данные в определенном регионе. Если у них есть такое требование, следует использовать дополнительный регион хранения.

### <a name="secondary-storage-regions"></a>Дополнительные регионы хранения
Если используется дополнительный регион хранилища, связанная учетная запись хранения Azure находится в другом регионе, где находится Cloud Shell компьютер, к которому вы подключаетесь. Например, Мария может задать расположение своей учетной записи хранения в восточной части Канады, в дополнительном регионе, но компьютер, к которому она подключена, по-прежнему находится в основном регионе. Его неактивных данных находятся в Канаде, но обрабатываются в США.

> [!NOTE]
> Если используется дополнительный регион, доступ к файлам и время запуска для Cloud Shell могут выполняться медленнее.

`(Get-CloudDrive | Get-AzStorageAccount).Location`Чтобы увидеть расположение общего файлового ресурса, пользователь может запустить в PowerShell.
...
Учетные записи хранения, создаваемые в Cloud Shell, помечаются тегом `ms-resource-usage:azure-cloud-shell`. Если вы хотите запретить пользователям создавать учетные записи хранения в Cloud Shell, создайте [политику ресурсов Azure на основе тегов](../governance/policy/samples/index.md) для этого конкретного тега.
...
При подключении имеющегося файлового ресурса учетные записи хранения должны находится в выбранном регионе Cloud Shell. Извлеките расположение, запустив `env` и установив `ACC_LOCATION` .
...
[Сведения о тегах хранилища](../azure-resource-manager/management/tag-resources.md) <br>