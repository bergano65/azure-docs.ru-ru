...
Чтобы найти текущий регион, можно запустить `env` в Bash и найти переменную `ACC_LOCATION`или из среды выполнения PowerShell `$env:ACC_LOCATION`. Файловые ресурсы получают образ размером 5 ГБ, создаваемый для хранения каталога `$Home`.
...
Чтобы увидеть расположение общего файлового ресурса, пользователь может запустить `(Get-CloudDrive | Get-AzStorageAccount).Location` в PowerShell.
...
### <a name="list-clouddrive"></a>Список `clouddrive`
...
> [!WARNING]
> Хотя при выполнении этой команды ресурсы не удаляются, ручное удаление группы ресурсов, учетной записи хранения или файлового ресурса, сопоставленного с Cloud Shell, приведет к удалению образа диска каталога `$Home` и всех файлов в файловом ресурсе. Это действие нельзя отменить.
...
Примечание. Если необходимо определить функцию в файле и вызвать ее из командлетов PowerShell, необходимо добавить оператор dot. Пример: . .\MyFunctions.ps1
