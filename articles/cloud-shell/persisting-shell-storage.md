...
Чтобы найти текущий регион, можно запустить `env` Bash и найти переменную `ACC_LOCATION`или запустить `$env:ACC_LOCATION`PowerShell. Файловые ресурсы получают образ размером 5 ГБ, создаваемый для хранения каталога `$Home`.
...
Чтобы увидеть расположение общего `(Get-CloudDrive | Get-AzStorageAccount).Location` файлового ресурса, пользователь может запустить в PowerShell.
...
1. Выполните команду `clouddrive unmount`.
...
[Сведения о тегах хранилища](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-using-tags) <br>
