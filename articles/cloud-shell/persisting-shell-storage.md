---
title: Неупорные файлы в облачной оболочке Azure Документы Майкрософт
...
С помощью расширенных параметров можно привязать существующие ресурсы. При выборе региона Cloud Shell необходимо выбрать резервную учетную запись хранения, расположенную в этом же регионе. Например, если назначенный регион — это Западная ЧАСТЬ США, то вы должны связать долю файлов, которая также находится в Пределах Западной США.
...
Обудоболочка использует долю файлов Azure в учетной записи хранения внутри указанной подписки. Благодаря унаследованные разрешения, пользователи с достаточными правами доступа к подписке смогут получить доступ ко всем учетным записям хранения и файловым долям, содержащимся в подписке.

Пользователи должны блокировать доступ к своим файлам, установив разрешения на учетной записи хранилища или на уровне подписки.

## <a name="supported-storage-regions"></a>Поддерживаемые регионы хранилища
Чтобы найти текущий регион, вы можете `env` `ACC_LOCATION`работать в Bash `$env:ACC_LOCATION`и найти переменную, или от PowerShell перспективе . Файловые ресурсы получают образ размером 5 ГБ, создаваемый для хранения каталога `$Home`.
...
Клиенты должны выбрать основной регион, если у них нет требования о том, чтобы их данные в состоянии покоя хранились в определенном регионе. Если они имеют такое требование, следует использовать вторичный регион хранения.

### <a name="secondary-storage-regions"></a>Вторичные области хранения
При использовании вторичной области хранения связанная учетная запись хранения Azure находится в другом регионе, как машина оболочки облака, в которую вы их устанавливаете. Например, Джейн может установить свой счет хранения, который будет расположен в Канаде Восток, вторичный регион, но машина она установлена по-прежнему находится в первичном регионе. Ее данные в покое находится в Канаде, но обрабатывается в Соединенных Штатах.

> [!NOTE]
> При использовании вторичного региона доступ к файлам и время запуска для Cloud Shell могут быть более медленными.

Пользователь может `(Get-CloudDrive | Get-AzStorageAccount).Location` запустить в PowerShell, чтобы увидеть местоположение своего файла Доля.
...
В облачной оболочке можно `clouddrive`запустить команду под названием, которая позволяет вручную обновлять долю файла, установленную в Cloud Shell.
...
При подключении имеющегося файлового ресурса учетные записи хранения должны находится в выбранном регионе Cloud Shell. Извлеките расположение, `env` запустив и проверив `ACC_LOCATION`.
...
Примечание: Если вам нужно определить функцию в файле и вызвать ее из cmdlets PowerShell, то оператор точек должен быть включен. Например: .\MyFunctions.ps1

## <a name="next-steps"></a>Дальнейшие действия
[Краткое руководство по Cloud Shell](quickstart.md) <br>
[Введение в хранилище Microsoft Azure](../storage/files/storage-files-introduction.md) <br>
[Подробнее о тегах хранения](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-using-tags) <br>
