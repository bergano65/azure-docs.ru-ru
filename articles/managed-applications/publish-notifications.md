...
ОТПРАВКА | Succeeded | Полная подготовка управляемого приложения прошла удачно после размещения.
ОТПРАВКА | Failed | Сбой помещения подготовки экземпляра приложения в любой момент.
PATCH | Succeeded | После успешного исправления в экземпляре управляемого приложения для обновления тегов, политики JIT-доступа или управляемого удостоверения.
УДАЛИТЬ | Удаление | Как только пользователь инициирует удаление экземпляра управляемого приложения.
УДАЛИТЬ | Удалена | После полного и успешного удаления управляемого приложения.
УДАЛИТЬ | Failed | После любой ошибки в процессе отмены инициализации, блокирующей удаление.
...
биллингдетаилс | Сведения о выставлении счетов для экземпляра управляемого приложения. Содержит Ресаурцеусажеид, которые можно использовать для запроса сведений об использовании в Marketplace.
