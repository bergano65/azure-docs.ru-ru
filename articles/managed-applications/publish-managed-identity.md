...
---|---|---
authorizationAudience | *Нет* | URI идентификатора приложения целевого ресурса. Это также `aud` утверждении (аудитории), выданного маркера. Значение по умолчанию — "https://management.azure.com/"
userAssignedIdentities | *Нет* | Список назначаемого пользователем управляемого удостоверения для получения маркера для. Если не указан, `listTokens` возвращает токен для управляемого удостоверения, назначенное системой.
...
Ответ будет содержать массив маркеров в разделе `value` свойство:

Параметр | Описание
---|---
access_token | Запрашиваемый маркер доступа.
expires_in | Количество секунд, в который маркер доступа будет действительна.
expires_on | Период времени после истечения срока действия маркера доступа. Это представляется как количество секунд с начала эпохи.
not_before | Период времени, когда маркер доступа вступает в силу. Это представляется как количество секунд с начала эпохи.
authorizationAudience | `aud` (Аудитории) маркер доступа был запрос. Это совпадает со значением, представленных на `listTokens` запроса.
resourceId | Идентификатор ресурса Azure для выданного токена. Это может быть идентификатор управляемого приложения или идентификатор назначаемого пользователем удостоверения.
