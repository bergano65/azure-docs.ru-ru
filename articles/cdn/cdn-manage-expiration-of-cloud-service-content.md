...
Чтобы использовать атрибут **cacheControlMaxAge**, присвойте атрибуту **cacheControlMode** значение `UseMaxAge`. Этот параметр добавляет HTTP-заголовок и директиву (`Cache-Control: max-age=<nnn>`) в ответ. У значения интервала времени для атрибута **cacheControlMaxAge** следующий формат: `<days>.<hours>:<min>:<sec>`. Его значение преобразуется в секунды и `Cache-Control` `max-age` используется в качестве значения директивы. Для получения дополнительной `<clientCache>` информации об элементе см. [Клиент Кэш \<клиентCache>](https://www.iis.net/ConfigReference/system.webServer/staticContent/clientCache).  
...
* [Ознакомьте документацию для **класса HttpCachePolicy**](/dotnet/api/system.web.httpcachepolicy)  
