...
Когда эти правила заданы, запрос для _ &lt;конечной точки hostname&gt;_. azureedge.net/Home/Index.HTML активирует настраиваемое правило кэширования #2, которое имеет значение: **Set, если отсутствует** , и 3 дня. Поэтому если в файле *index.html* есть заголовки HTTP `Cache-Control` или `Expires`, они учитываются. Если эти заголовки не заданы, файл кэшируется 3 дня.
