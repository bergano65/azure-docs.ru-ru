...
description: Узнайте, как можно оптимизировать загрузку больших файлов в сети доставки содержимого Azure. Эта статья содержит несколько сценариев.
services: cdn
documentationcenter: ''
author: asudbring
manager: danielgi
...
Сеть доставки содержимого кэширует все фрагменты по мере их получения. Весь файл не кэшируется в кэше сети доставки содержимого. Последующие запросы файла или диапазонов байт обслуживаются из кэша сети доставки содержимого. Если в сети доставки содержимого отсутствуют некоторые фрагменты файла, запрошенные недостающие фрагменты предварительно загружаются с сервера-источника. Эта оптимизация полагается на способность сервера-источника поддерживать запросы диапазона байтов. Если сервер-источник не поддерживает запросы в диапазоне байтов, запросы на скачивание данных, размер которых превышает 8 МБ, завершатся ошибкой. 
...
| Caching  | Общая веб-доставка | Оптимизация больших файлов 
--- | --- | --- 
