...
description: 'Можно просмотреть способы использования CDN с помощью следующих отчетов: Пропускная способность, переданные данные, попадания, состояния кэша, эффективность кэша, переданных данных IPV4/IPV6.'
services: cdn
documentationcenter: ''
author: zhangmanling
manager: erikre
...
* TCP_HIT: Обслуживаться с пограничного сервера. Объект найден в кэше и не превысил максимальный возраст.
* TCP_MISS: Подано с сервера-источника. Объект не найден в кэше, ответ получен от сервера-источника.
* TCP_EXPIRED _MISS: Подано с сервера-источника после сверки с оригиналом. Объект найден в кэше, однако он превысил установленный максимальный возраст. Повторная сверка с сервером-источником привела к тому, что объект кэша был замещен новым откликом от источника.
* TCP_EXPIRED _HIT: Получено с Edge после сверки с оригиналом. Объект находился в кэше, однако он превысил максимальный возраст, заданный для него. Повторная сверка с сервером-источником привела к тому, что объект кэша остался неизменным.
...
* TCP_EXPIRED_HIT: Это состояние передается, когда запрос, который предназначен для ресурса с истекшим TTL был обработан непосредственно от POP клиенту. Например, так происходит при превышении максимального возраста (max-age) ресурса. 
...
* TCP_EXPIRED_MISS: Это состояние передается, когда новая версия просроченного ресурса передается от POP клиенту. Этот статус возникает, когда истекает TTL для кэшированного ресурс-контейнера (например, истек срок, заданный в параметре max-age), а сервер-источник возвращает более новую версию этого ресурса. Клиенту вместо кэшированной версии передается новая версия ресурса. Кроме того, она кэшируется на граничном сервере и на клиенте.
* CONFIG_NOCACHE: Это состояние указывает, что клиентская конфигурация на пограничном POP не актива предотвращает кэширование.
* NONE — это состояние указывает на то, что проверка актуальности содержимого кэша не была выполнена.
* TCP_CLIENT_REFRESH_MISS: Это состояние передается в том случае, если HTTP-клиента, например, браузер принудительно заставляет Граничный POP извлекать новую версию устаревший ресурс на сервере-источнике. По умолчанию серверы не позволяют клиенту HTTP инициировать принудительное получение пограничными серверами новой версии ресурса с сервера-источника.
* TCP_PARTIAL_HIT: Это состояние передается, когда запрос по диапазону байтов приводит нажатия для частично кэшированного ресурс-контейнера. Запрошенный диапазон байтов немедленно передается от POP клиенту.
* UNCACHEABLE: Это состояние передается в том случае, если ОС `Cache-Control` и `Expires` заголовки указывают, что он не должен кэшироваться в точке Присутствия или с помощью клиента HTTP. Эти типы запросов обслуживаются на сервере-источнике.
...
* Запросы, которые отклоняются из-за параметров фильтрации страны или региона.
...
Рассчитывается по формуле: (TCP_ HIT / (TCP_ HIT + TCP_MISS)) * 100
