...
- в необходимом запросе для обработчика правил включена [функция заголовков ответа Debug Cache](cdn-verizon-premium-rules-engine-reference-features.md#debug-cache-response-headers);
...
- Код состояния: Указывает, как запрошенное содержимое было обработано CDN, что представлено кодом состояния кэша.
...
- MASeconds. Указывает max-age (в секундах), как определено в заголовках Cache-Control запрошенного содержимого.
...
- UnixTime: Указывает метку времени кэширования запрошенного содержимого во времени Unix (также известный как времени POSIX или Unix epoch). Метка времени кэша указывает начальную дату и время, с которых будет рассчитываться срок жизни ресурса. 
...
- ддд, ДД МММ ГГГГ ЧЧ: мм: GMT: Указывает метку времени кэширования запрошенного содержимого. Дополнительные сведения см. в описании термина UnixTime выше.

- CASeconds: Указывает количество секунд, истекших с метку времени кэширования.

- RTSeconds. Указывает число оставшихся секунд, для которого кэшированное содержимое будет считаться новым. Это значение вычисляется следующим образом: RTSeconds = max-age — срок действия кэша.

- RTTimePeriod: Преобразует оставшееся значение срока ЖИЗНИ (то есть RTSeconds) в приблизительный эквивалент большей единицы измерения (например, дни).

- ExpiresSeconds. Указывает количество секунд, оставшихся до даты и времени, указанного в `Expires` заголовок ответа. Если заголовок ответа `Expires` не включен в ответ, тогда значение этого термина — *none*.
