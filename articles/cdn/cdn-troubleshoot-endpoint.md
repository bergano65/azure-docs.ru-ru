...
Тем не менее рассмотрим такой URL-адрес для ранее тестируемого исходного файла: http:\//www.contoso.com:8080/file.txt. Обратите внимание на часть *:8080* в конце сегмента имени узла. Что номер указывает, что браузер на использование порта 8080 для подключения к веб-сервером по www\.contoso.com, поэтому вам потребуется ввести *8080* в **HTTP-порт** поля. Важно отметить, что эти параметры портов влияют только на определение порта, используемого конечной точкой для получения сведений из источника.
