...
Переопределение | Если выбран этот параметр и правило соответствует, то значение TTL, возвращенное из источника, перезаписывается значением, указанным в действии.
...
-------|------------------|------
Append | Если выбран этот параметр и правило соответствует, заголовок, указанный в поле **имя заголовка** , добавляется в запрос с указанным значением. Если заголовок уже существует, значение добавляется к существующему значению. | Строка
Overwrite | Если выбран этот параметр и правило соответствует, заголовок, указанный в поле **имя заголовка** , добавляется в запрос с указанным значением. Если заголовок уже существует, указанное значение перезаписывает существующее значение. | Строка
...
Действие | Имя заголовка HTTP | Значение
-------|------------------|------
Append | Если выбран этот параметр и правило соответствует, заголовок, заданный в поле **имя заголовка** , добавляется в ответ с использованием указанного **значения**. Если заголовок уже существует, **значение** добавляется к существующему значению. | Строка
Overwrite | Если выбран этот параметр и правило соответствует, заголовок, заданный в поле **имя заголовка** , добавляется в ответ с использованием указанного **значения**. Если заголовок уже существует, **значение** перезаписывает существующее значение. | Строка
...
Поле | Описание 
------|------------
Type | Выберите тип ответа для возврата запрашивающему инициатору: найдено (302), перемещено (301), временное перенаправление (307) и постоянное перенаправление (308).
