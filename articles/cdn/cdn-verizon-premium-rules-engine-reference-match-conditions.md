---
title: Azure CDN из условия соответствия обработчика правил Verizon Premium | Документация Майкрософт
description: Справочная документация по сети доставки содержимого Azure из условий соответствия обработчика правил Verizon Premium.
services: cdn
author: asudbring
...
ms.locfileid: "81253447"
---
# <a name="azure-cdn-from-verizon-premium-rules-engine-match-conditions"></a>Azure CDN из условия соответствия обработчика правил Verizon Premium

В этой статье приведены подробные описания доступных условий соответствия для сети доставки содержимого (CDN) Azure из [обработчика правил](cdn-verizon-premium-rules-engine.md)Verizon Premium.
...
Имя | Назначение
-----|--------
[Постоянно](#always) | Применяет набор функций по умолчанию ко всем запросам.
...
[Страна](#country) | Определяет запросы, которые берутся из указанных стран или регионов.
...
[Метод Request](#request-method) | Определяет запросы по их HTTP-методу.
...
[В начало](#reference-for-rules-engine-match-conditions)
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "число".
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия IP-адреса клиента.
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия параметру файла cookie.
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "регулярное выражение параметра cookie".
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "страна":
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "устройство".
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия предметной области.
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "литерал заголовка запроса".
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия шаблону заголовка запроса.
...
### <a name="request-method"></a>Метод Request
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "Каталог URL-пути".
...
- Используйте параметр **Относительно**, чтобы указать, где начинается точка сравнения URL-адресов: перед точкой доступа к содержимому или после нее. Точка доступа к содержимому представляет собой часть пути и отображается между именем узла Verizon CDN и относительным путем к запрошенному ресурсу (например, /800001/CustomerOrigin). Она определяет расположение по типу сервера (например, источник CDN или клиента) и номеру счета заказчика.
  ...
  Этот URL-адрес указывает на следующее имя узла CDN Verizon:\/http:/WPC.0001. &lt;домен&gt;/800001/myorigin/**MyFolder**/index.htm
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "расширение URL-пути".
...
Спецификация значения: HTML-код ASP ASPX PHP
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "имя файла URL-пути".
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "литерал URL-пути".
...
- Используйте параметр **Относительно**, чтобы указать, где начинается точка сравнения URL-адресов: перед точкой доступа к содержимому или после нее. 
  ...
  - **Корневой домен**. Указывает, что точка сравнения URL-адресов начинается непосредственно после имени узла CDN.
    ...
    Например: http:\//WPC.0001. &lt;&gt;/**800001/myorigin/MyFolder/index.htm** домена

  - **Исходный домен**. Указывает, что точка сравнения URL-адресов начинается после точки доступа к содержимому (например, /000001 или /800001/myorigin). Так как запись CNAME \*.azureedge.net создана относительно исходного каталога в имени узла Verizon CDN по умолчанию, пользователи Azure CDN должны использовать значение **Исходный домен**. 

    Пример: https:\//&lt;Endpoint&gt;. azureedge.NET/**MyFolder/index.htm**
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "регулярное выражение URL-пути".
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия подстановочного знака URL-пути.
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "литерал запроса URL-адреса".
...
- Для некоторых знаков требуется кодирование в формат URL-адреса. Используйте знак процента для кодирования таких знаков в формат URL-адреса:
   ...
   Символ | Кодирование в формат URL-адреса
   ----------|---------
   Пробел     | %20
   &         | %25
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия параметра запроса URL-адреса.
...
------|-------|--------
User (Пользователь)  | Ivan   | Для соответствия этому шаблону строка запроса запрошенного URL-адреса должна быть "?user=ivan."
User (Пользователь)  | *     | Для соответствия этому шаблону строка запроса запрошенного URL-адреса должна содержать параметр пользователя.
Электронная почта | Ivan\* | Для соответствия этому шаблону строка запроса запрошенного URL-адреса должна содержать параметр адреса электронной почты, который начинается с "Ivan".
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "регулярное выражение запроса URL-адреса".
...
- Для некоторых знаков требуется кодирование в формат URL-адреса. Используйте знак процента для кодирования таких знаков в формат URL-адреса:
   ...
   Символ | Кодирование в формат URL-адреса | Значение
   ----------|--------------|------
   Пробел     | %20          | \%20
   &         | %25          | \%25
...
Параметр **соответствует**/**не соответствует** определяет условия, при которых будет выполняться условие соответствия "шаблон запроса URL-адреса".
...
- В целях этого параметра строка запроса начинается с первого знака после разделителя вопросительного знака (?) строки запроса.
- Значения параметров могут включать [подстановочные знаки](cdn-verizon-premium-rules-engine-reference.md#wildcard-values):
   - Каждый шаблон значения параметра может содержать одну или несколько звездочек (*), где каждая звездочка может соответствовать последовательности одного или нескольких знаков.
   - Для некоторых знаков требуется кодирование в формат URL-адреса. Используйте знак процента для кодирования таких знаков в формат URL-адреса:
