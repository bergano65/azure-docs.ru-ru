...
Адрес эл. почты | Ivan\* | Для соответствия этому шаблону строка запроса запрошенного URL-адреса должна содержать параметр адреса электронной почты, который начинается с "Ivan".
