...
1. В [портал Azure](https://portal.azure.com)перейдите к профилю CDN. На предыдущем шаге вы могли прикрепить его к панели мониторинга. В противном случае профиль CDN можно найти, выбрав **Все службы**, а затем **Профили CDN**. Выберите профиль, к которому планируется добавить конечную точку, в области **Профили CDN**. 
...
3. В поле **Имя** введите уникальное имя новой конечной точки CDN. Это имя используется для доступа к кэшированным ресурсам в домене _\<endpointname>_ . azureedge.NET.
...
8. Для параметров **Протокол** и **Порт источника** укажите протоколы и порты, используемые для доступа к ресурсам в сервере-источнике. Необходимо указать хотя бы один протокол (HTTP или HTTPS). Используйте предоставленный CDN домен ( _\<endpointname>_ . azureedge.NET) для доступа к содержимому HTTPS. 
...
9. Для параметра **Оптимизация для** выберите тип оптимизации, который наилучшим образом соответствует сценарию и типу содержимого, которое должна предоставлять конечная точка. Дополнительные сведения см. в разделе [Optimize Azure CDN for the type of content delivery](cdn-optimization-overview.md) (Оптимизация Azure CDN для типа доставки содержимого).
    ...
    - **Azure CDN Standard из профилей Майкрософт** :
    ...
    - **Azure CDN "Стандартный" из профилей Akamai** :
       - [**Общая веб-доставка**](cdn-optimization-overview.md#general-web-delivery)
...
10. Для создания конечной точки нажмите кнопку **Добавить**.
    ...
    - Для **Azure CDN Standard от Verizon** и **Azure CDN Premium из профилей Verizon** распространение обычно завершается в течение 30 минут. 
