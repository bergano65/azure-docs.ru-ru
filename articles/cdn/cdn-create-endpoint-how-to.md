...
3. В поле **Имя** введите уникальное имя новой конечной точки CDN. Это имя используется для доступа к кэшированным ресурсам в домене  _\<имя_конечной_точки >_ . azureedge.net.
...
8. Для параметров **Протокол** и **Порт источника** укажите протоколы и порты, используемые для доступа к ресурсам в сервере-источнике. Необходимо указать хотя бы один протокол (HTTP или HTTPS). Использовать домен, предоставленный сетью CDN ( _\<имя_конечной_точки >_ . azureedge.net) для доступа к содержимому HTTPS. 
