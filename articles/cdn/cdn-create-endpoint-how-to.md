...
3. В поле **Имя** введите уникальное имя новой конечной точки CDN. Это имя используется для доступа к кэшированным ресурсам в _ \<конечном домене>_.azureedge.net.
...
8. Для параметров **Протокол** и **Порт источника** укажите протоколы и порты, используемые для доступа к ресурсам в сервере-источнике. Необходимо указать хотя бы один протокол (HTTP или HTTPS). Для доступа к содержимому HTTPS используйте домен, предоставленный CDN_\<(конечная точка>_.azureedge.net). 
...
9. Для параметра **Оптимизация для** выберите тип оптимизации, который наилучшим образом соответствует сценарию и типу содержимого, которое должна предоставлять конечная точка. Дополнительные сведения см. в разделе [Optimize Azure CDN for the type of content delivery](cdn-optimization-overview.md) (Оптимизация Azure CDN для типа доставки содержимого).
    ...
    - **Стандарт Azure CDN из** профилей Майкрософт:
