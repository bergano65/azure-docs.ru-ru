...
ms.locfileid: "79252120"
---
# <a name="optimize-azure-cdn-for-the-type-of-content-delivery"></a>Оптимизация Azure CDN для типа доставки содержимого

При передаче содержимого большой аудитории очень важно оптимизировать доставку содержимого. [Сеть доставки контента Azure (CDN)](cdn-overview.md) может оптимизировать интерфейс доставки в зависимости от типа контента. В качестве содержимого может выступать веб-сайт, поток мультимедиа, видео или большой загружаемый файл. При создании конечной точки CDN сценарий оптимизации указывается в параметре **Оптимизировано для**. Выбранный сценарий оптимизации определяет, каким образом будет оптимизировано содержимое, доставляемое из конечной точки сети доставки содержимого.
...
> [!NOTE]
> Динамическое ускорение сайта от Microsoft предлагается через [сервис Front Door Azure.](https://docs.microsoft.com/azure/frontdoor/front-door-overview)
...
2. В соответствии с SETTINGS выберите **Оптимизацию.** Затем выберите тип из **Оптимизированного** списка.
...
> [!NOTE]
> Если конечная точка CDN используется преимущественно для доставки видео по запросу, используйте этот тип оптимизации. Основное различие между этим типом оптимизации и общим типом оптимизации потоковой передачи мультимедиа — это тайм-аут повтора подключения. Тайм-аут гораздо короче для работы с сценариями потоковой передачи.
>
...
> [!NOTE]
> Динамическое ускорение сайта от корпорации Майкрософт предлагается через [службу front Door Azure,](https://docs.microsoft.com/azure/frontdoor/front-door-overview) которая является глобальной службой [anycast,](https://en.wikipedia.org/wiki/Anycast) используя частную глобальную сеть Microsoft для доставки рабочих нагрузок приложения.
