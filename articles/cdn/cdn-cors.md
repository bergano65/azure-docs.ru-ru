...
1. Браузер отправляет запрос CORS с дополнительным заголовком HTTP-запроса **Origin**. Значение этого заголовка является источником, обслужившим родительскую страницу, который определяется как сочетание *протокола, * *домена* и *порта*.  Когда страница https//www.contoso.com\:попытается получить доступ к данным пользователя в fabrikam.com происхождения, следующий заголовок запроса будет отправлен fabrikam.com:
...
В Azure CDN Standard от корпорации Майкрософт можно создать правило в [движке стандартных правил](cdn-standard-rules-engine-reference.md) для проверки заголовка **Origin** по запросу. Если это действительное происхождение, ваше правило установит заголовок **Access-Control-Allow-Origin** с желаемым значением. В этом случае заголовок **Access-Control-Allow-Origin** с сервера происхождения файла игнорируется, а движок правил CDN полностью управляет разрешенным происхождением CORS.

![Пример правил со стандартным механизмом правил](./media/cdn-cors/cdn-standard-cors.png)

> [!TIP]
> Можно добавить дополнительные действия в правило для изменения дополнительных заголовков ответов, таких как **Access-Control-Allow-Methods.**
> 

На **Azure CDN Standard от Akamai**, единственным механизмом, позволяющим использовать несколько истоков без использования происхождения подстановочных знаков, является использование [кэширования строки запроса.](cdn-query-string.md) Включите параметр строки запроса для конечной точки CDN, а затем используйте уникальную строку для запросов от каждого разрешенного домена. В результате CDN будет кэшировать отдельный объект для каждой уникальной строки запроса. Однако этот подход не является идеальным, так как он приведет к появлению нескольких копий одного файла в кэше CDN.  

### <a name="azure-cdn-premium-from-verizon"></a>Azure CDN уровня "Премиум" от Verizon
Используя движок правил Verizon Premium, необходимо [создать правило](cdn-rules-engine.md) для проверки заголовка **Origin** по запросу.  Если это допустимый источник, то ваше правило установит заголовок **Access-Control-Allow-Origin** в соответствии с источником, указанным в запросе.  Если происхождение, указанное в заголовке **Origin,** не разрешено, правило должно исключить заголовок **Access-Control-Allow-Origin,** что приведет к отклонению запроса браузером. 

Есть два способа сделать это с движком правил Premium. В обоих случаях заголовок **Access-Control-Allow-Origin** из файла сервера-источника игнорируется, и разрешенными источниками CORS управляет только обработчик правил CDN.
