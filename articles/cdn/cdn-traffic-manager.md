---
title: Отработка отказа в нескольких конечных точках Azure CDN с помощью диспетчера трафика
...
2.  В своем профиле Azure CDN выберите первую конечную точку CDN (Akamai). Выберите **Добавить личный домен** и введите *cdndemo101. дустидогпеткаре. Online*. Убедитесь, что флажок, подтверждающий допустимость пользовательского домена, отображается зеленым цветом. 
    ...
    > [!NOTE]
    > Чтобы включить SSL для **Azure CDN из профилей Akamai** , необходимо напрямую зарегистрировать личный домен в конечной точке. cdnverify для включения SSL еще не поддерживается. 
    >
