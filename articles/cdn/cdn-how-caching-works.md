...
- При использовании в HTTP-ответе от клиента к POP CDN:
     ...
     - **Azure CDN уровня "Стандартный" от Akamai** поддерживает перечисленные ниже директивы `Cache-Control` и игнорирует все остальные.
         ...
         - `no-cache`. Кэширует содержимое, но проверяет содержимое каждый раз перед его доставкой из кэша. Аналогично параметру `Cache-Control: max-age=0`.
...
| **Длительность кэша CDN** | 2 дня |7 дней | None | 7 дней | None | 1 день | Год |
