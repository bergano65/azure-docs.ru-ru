...
4. Выберите, какие ресурсы нужно удалить из пограничных узлов.  Чтобы очистить все ресурсы, установите флажок **Очистить все** .  В противном случае в поле **Путь** введите путь к каждому ресурсу, который нужно очистить. В поле "Путь" поддерживаются следующие форматы.
    1. **Очистка отдельного URL-адреса**. Очистить отдельный ресурс, указав полный URL-адрес с или без расширения файла, например,`/pictures/strasbourg.png`; `/pictures/strasbourg`
   ...
   > > Очистка отдельного URL-адрес `@"^\/(?>(?:[a-zA-Z0-9-_.%=\(\)\u0020]+\/?)*)$";`  
...
* [Справочник по CDN REST API Azure - Очистка и предварительная загрузка конечной точки](/rest/api/cdn/endpoints)
