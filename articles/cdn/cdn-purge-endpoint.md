...
description: Узнайте, как очистить все кэшированное содержимое конечной точки сети доставки содержимого Azure. Граничные узлы кэшируют ресурсы, пока не истечет срок жизни.
services: cdn
documentationcenter: ''
author: asudbring
manager: danielgi
...
3. В колонке "Очистка" выберите адрес службы для удаления из раскрывающегося списка URL-адресов.
   ...
   > 
   > 
4. Выберите, какие ресурсы нужно удалить из пограничных узлов.  Если вы хотите очистить все ресурсы, установите флажок **Очистить все** .  В противном случае в поле **Путь** введите путь к каждому ресурсу, который нужно очистить. В поле "Путь" поддерживаются следующие форматы.
    1. **Очистка одного URL-адреса**. Очистите отдельный ресурс, указав полный URL-адрес с расширением или без него, например, `/pictures/strasbourg.png` ; `/pictures/strasbourg`
