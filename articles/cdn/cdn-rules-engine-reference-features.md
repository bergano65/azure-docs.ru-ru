...
Value|Результат
--|--
Enabled | Заголовок ответа Age включен в ответ, отправляемый инициатору запроса.
...
Value|Результат
--|--
Enabled|Позволяет точке подключения удовлетворять запросы на регулирование ширины полосы пропускания.
...
**Поведение по умолчанию:** включена.
...
**Назначение:** регулирует пропускную способность для ответа от точки подключения.
...
**Назначение:** определяет, нужно ли пропустить запроса кэширование.

Value|Результат
--|--
Enabled|Приводит к тому, что все запросы достигают сервера-источника, даже если содержимое ранее было кэшировано на точках подключения.
...
- По умолчанию кэшируются только запросы, размер которых меньше 14 КБ. Используйте функцию размер текста кэшируемого запроса, чтобы задать максимальный размер текста запроса.
...
**Назначение:** контролирует создание `Cache-Control` заголовки точкой подключения при включенной функции External Max-Age.
...
Value|Результат
--|--
Перезаписать|Обеспечивает выполнение следующих действий:<br/> — Перезаписывает `Cache-Control` заголовок, созданный на сервере-источнике. <br/>— Добавляет `Cache-Control` заголовка, созданный функцией External Max-Age в ответ.
Пропустить|Гарантирует, что `Cache-Control` заголовок, созданный функцией External Max-Age никогда не добавляется в ответ. <br/> Если сервер-источник создает `Cache-Control` заголовок, он проходит через конечному пользователю. <br/> Если сервер-источник не создает `Cache-Control` заголовка, а затем этот параметр может привести к заголовок ответа содержит `Cache-Control` заголовка.
Добавить при отсутствии|Если `Cache-Control` заголовок не был получен с сервера-источника, а затем этот параметр добавляет `Cache-Control` заголовка, созданный функцией External Max-Age. Этот параметр полезен для обеспечения того, что все ресурсы назначены `Cache-Control` заголовка.
Удалить| Этот параметр гарантирует, что `Cache-Control` заголовок не включен в заголовок ответа. Если `Cache-Control` заголовок уже присвоен, то он удаляется из ответа заголовка.
...
**Назначение:** определяет, включает или исключает параметры строки запроса, связанный с запросом ключа кэша.
...
--|--
 ...
 Исключить все  |Указывает, что из ключа кэша исключаются все параметры строки запроса. Эта конфигурация дублирует поведение на странице Query-String Caching кэширования по умолчанию «standard-cache».  
...
Чтобы продублировать поведение на странице Query-String Caching кэширования строки запроса «no-cache», создайте правило, содержащее условие соответствия шаблона запроса URL-адреса и функцию отключения кэша. Для условия соответствия шаблона запроса URL-адреса укажите звездочку (*).
...
- **Пример запроса:** http://wpc.0001.&lt; домена&gt;/800001/Origin/folder/asset.htm?sessionid=1234 & language = EN & userid = 01
...
Новый путь|Определяет относительный путь для нового ключа кэша. Относительный путь можно задать, выбрав базовый исходный путь, а затем определив шаблон регулярного выражения. Этот относительный путь можно создать динамически помощи [переменных HTTP](cdn-http-variables.md).

**Поведение по умолчанию:** ключ кэша запроса определяется URI запроса.
...
**Назначение:** позволяет примечания для добавления в правиле.
...
**Назначение:** определяет, что происходит, когда запрос завершается частичным промахом кэша в точке Присутствия.
...
Value|Результат
--|--
Enabled|Восстанавливает поведение по умолчанию. Поведение по умолчанию — принудительное получение точкой подключения фоновых данных ресурса с сервера-источника. После этого ресурс появится в локальном кэше точки подключения.
...
- Прежде чем добавить эту функцию в правило, убедитесь, что задан параметр сжатие отключено на странице "сжатие" для платформы, к которому применяется это правило.
...
**Назначение:** определяет, может ли ответ включать [заголовки ответа X-EC-Debug](cdn-http-debug-headers.md), который предоставляет сведения о политике кэширования для запрошенного ресурса.
...
Value|Результат
-|-
Enabled|Запросы для заголовков ответа debug cache возвратят ответ, включающий заголовок X-EC-Debug.
Отключено|Заголовок ответа X-EC-Debug будут исключены из ответа.
...
**Назначение:** определяет интервал максимального срока по умолчанию точки подключения до повторной сверки кэша исходного сервера. Другими словами, это количество времени, которое пройдет, прежде чем точка подключения проверит, соответствует ли кэшированный ресурс ресурсу, хранящемуся на сервере-источнике.

Основные сведения

- Это действие выполняется только для ответов на сервере-источнике, который не присвоил Указание максимального срока в `Cache-Control` или `Expires` заголовка.
- Это действие не будет выполняться для ресурсов, которые невозможно кэшировать.
- Это действие не влияет на повторные сверки браузера с кэшем точки подключения. Эти типы повторных сверок определяются `Cache-Control` или `Expires` заголовков, отправляемых в браузер, который может настраиваться с помощью функции External Max-Age.
...
- Задать для единицы времени значение «Выкл.» назначит интервал внутреннего максимального срока по умолчанию 7 дней для запросов, которые не было назначено Указание максимального срока в их `Cache-Control` или `Expires` заголовка.
...
Value | Результат
------|-------
Enabled| Приводит к отклонению всех запросов, соответствующих критериям сопоставления, с ответом 403 (запрещено).
...
**Назначение:** контролирует создание `Expires` заголовки точкой подключения при включенной функции External Max-Age.
...
Value|Результат
--|--
Перезаписать|Обеспечивает выполнение следующих действий:<br/>— Перезаписывает `Expires` заголовок, созданный на сервере-источнике.<br/>— Добавляет `Expires` заголовка, созданный функцией External Max-Age в ответ.
Пропустить|Гарантирует, что `Expires` заголовок, созданный функцией External Max-Age никогда не добавляется в ответ. <br/> Если сервер-источник создает `Expires` заголовок, он будет передаваться конечному пользователю. <br/>Если сервер-источник не создает `Expires` заголовка, а затем этот параметр может привести к заголовок ответа содержит `Expires` заголовка.
Добавить при отсутствии| Если `Expires` заголовок не был получен с сервера-источника, а затем этот параметр добавляет `Expires` заголовка, созданный функцией External Max-Age. Этот параметр полезен для обеспечения того, что все ресурсы будут назначаться `Expires` заголовка.
Удалить| Гарантирует, что `Expires` заголовок не включен в заголовок ответа. Если `Expires` заголовок уже присвоен, то он удаляется из ответа заголовка.

**Поведение по умолчанию:** перезаписи
...
**Назначение:** определяет интервал максимального срока для браузера для повторной сверки кэша точки подключения. Другими словами, это промежуток времени, которое пройдет, прежде чем браузер сможет проверить наличие новой версии ресурса на точке подключения.

Включение этой функции будет создавать `Cache-Control: max-age` и `Expires` заголовки из точки присутствия и отправлять их в HTTP-клиент. По умолчанию эти заголовки перезапишут заголовки, созданные на сервере-источнике. Тем не менее обработка заголовка Cache-Control и функции истечения срока действия обработки заголовка могут использоваться для изменения этого поведения.

Основные сведения

- Это действие не влияет на повторные сверки кэша точки подключения с сервером-источником. Эти типы повторных сверок определяются `Cache-Control` и `Expires`  заголовков получен с сервера-источника и может настраиваться с помощью Default Internal Max-Age и Force Internal Max-Age функции.
- Настройте эту функцию, указав целое значение и выбрав нужную единицу времени (секунды, минуты, часы и т. д.).
- Задав для этой функции отрицательное значение приводит к точкам подключения для отправки `Cache-Control: no-cache` и `Expires` время, заданные в прошлом с каждым ответом в браузер. Несмотря на то, что HTTP-клиент не будет кэшировать ответ, этот параметр не повлияет на способность точек подключения кэшировать ответ с сервера-источника.
- Если задать для единицы времени значение "Выкл.", эта функция будет отключена.  `Cache-Control` И `Expires`  заголовки, кэшированные с помощью ответа с сервера-источника будет передаваться браузера.

**Поведение по умолчанию:** Off
...
Value|Результат
-|-
Enabled|Запросы можно перенаправлять.
...
**Назначение:** определяет интервал максимального срока для повторной сверки кэша сервера исходного точки подключения. Другими словами, это количество времени, которое пройдет, прежде чем точка подключения сможет проверить, соответствует ли кэшированный ресурс ресурсу, хранящемуся на сервере-источнике.

Основные сведения

- Эта функция переопределяет интервал максимального срока, определенного в `Cache-Control` или `Expires` заголовки, созданные на сервере-источнике.
- Эта функция не влияет на повторные сверки браузера с точкой подключения. Эти типы повторных сверок определяются `Cache-Control` или `Expires` заголовков, отправляемых в браузер.
...
**Назначение:** определяет типы форматов файлов H.264, которые могут использоваться для потоковой передачи содержимого.

Основные сведения

- Определяют набор допустимых расширений имен файлов H.264 в параметре расширений файлов. Расширения файлов параметр переопределяет поведение по умолчанию. Настраивая этот параметр, включите эти расширения имен файлов, чтобы обеспечить поддержку MP4 и F4V. 
- Добавляйте точку при указании каждого расширения имени файла (например, _.mp4_, _.f4v_).

**Поведение по умолчанию:** Поэтапная загрузка HTTP поддерживает мультимедиа MP4 и F4V по умолчанию.
...
**Назначение:** определяет, будут ли без кэша HTTP-клиента перенаправляться запросы на сервер-источник.

Запрос без кэша происходит, когда в HTTP-клиент отправляет `Cache-Control: no-cache` и/или `Pragma: no-cache` заголовков в HTTP-запроса.

Value|Результат
--|--
Enabled|Это позволяет перенаправлять запросы HTTP-клиента без кэша на сервер-источник, который будет возвращать заголовки и текст ответов от точки подключения обратно в HTTP-клиент.
...
Состояние кэша, которое сообщается для запроса, который можно перенаправить на сервер-источник, из-за этой функции — `TCP_Client_Refresh_Miss`. Отчет состояния кэша, который доступен в основном модуле создания отчетов, предоставляет статистические данные по состоянию кэша. Это позволяет отслеживать количество и процент запросов, перенаправляемых на сервер-источник при применении этой функции.
...
- Набор допустимых кодов состояния для этой функции являются: 200, 203, 300, 301, 302, 305, 307, 400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 417, 500, 501, 502, 503, 504 и 505.
- Отключите эту функцию, присвоив ей пустое значение.

**Поведение по умолчанию:** поведение по умолчанию — учитывать приведенные выше директивы.
...
По умолчанию этот код состояния возвращается, если запрос указанного диапазона байтов не может удовлетворить POP и поле заголовка запроса If-Range не указано.

Value|Результат
-|-
Enabled|Предотвращает ответ на запрос байтового диапазона с кодом состояния 416 запрошенный диапазон невыполним точек подключения. Вместо этого серверы предоставят запрошенный ресурс и возвратят ответ 200 ОК клиенту.
...
**Назначение:** контролирует, как долго после обычного истечения срока действия кэшированный ресурс может обслуживаться с точки подключения, если точка подключения не сможет выполнить повторную сверку кэшированного ресурса с сервера-источника.

Как правило по истечении времени максимального срока ресурса POP отправит запрос повторной проверки на сервер-источник. Сервер-источник затем отправит либо 304 не изменено, чтобы выдать точке подключения новый аренды для кэшированного ресурса, ответ 200 ОК для предоставления точке подключения обновленную версию кэшированного ресурса.
...
Обратите внимание на то, что этот интервал времени начинается по истечении максимального срока ресурса, не при неудачной повторной проверке. Таким образом максимальный период, в течение которого актива может обрабатываться без успешной повторной проверки, объема времени, заданного параметром сочетание max-age, а также максимальное значение устаревания. Например если ресурс был кэширован в 9:00 с максимальным сроком 30 минут и максимальным значением устаревания 15 минут, затем Неудачная повторная попытка проверки в 9:44 приведет к конечному пользователю получать устаревший кэшированный ресурс, а Неудачная повторная попытка проверки в 9:46 приведет к en d пользователь, получивший 504 истекло время ожидания шлюза.

Любое значение, настроенное для этой функции заменяется `Cache-Control: must-revalidate` или `Cache-Control: proxy-revalidate` заголовки, полученные с сервера-источника. Если хоть один из этих заголовков получен от сервера-источника при изначальном кэшировании ресурса, точка подключения не будет обрабатывать устаревший кэшированный ресурс. В этом случае если точка подключения не сможет выполнить повторную сверку с источником по истечении интервала максимального срока ресурса, POP возвращает ошибка 504 истекло время ожидания шлюза.

Основные сведения

- Настройте эту функцию, сделав следующее:
    - Выберите код состояния, к которому будет применено максимальное значение устаревания.
    - Укажите целое значение, а затем выберите нужную единицу времени (секунды, минуты, часы и т. д.). Это значение определяет внутреннее максимальное значение устаревания, будут применены.
...
**Назначение:** определяет, будет ли строка запроса сохранена вместе с URL-адреса в журналах доступа.

Value|Результат
-|-
Enabled|Позволяет хранить строки запроса во время записи URL-адресов в журнале доступа. Если URL-адрес не содержит строку запроса, этот параметр не будет иметь влияния.
...
Delete (Удалить)|Удаляет указанный заголовок запроса.|**Значение заголовка запроса (клиент):**<br/>Значение1<br/>**Конфигурация заголовка запроса Modify Client:**<br/>удаление указанного заголовка запроса.<br/>**Результат:**<br/>указанный заголовок запроса не будет перенаправляться на сервер-источник.

Основные сведения

- Убедитесь, что значение, указанное в параметре Name, точно совпадать с заголовком запроса.
- В целях определения заголовка регистр не учитывается. Например, все следующие варианты `Cache-Control` имя заголовка можно использовать для его идентификации:
    - cache-control;
    - CACHE-CONTROL;
    - cachE-Control.
...
Delete (Удалить)|Удаляет указанный заголовок ответа.|**Значение заголовка ответа (клиент):**<br/>Значение1<br/>**Конфигурация заголовка ответа Modify Client:**<br/>удаление указанного заголовка ответа.<br/>**Результат:**<br/>указанный заголовок ответа не будет перенаправляться инициатору запроса.

Основные сведения

- Убедитесь, что значение, указанное в параметре Name, точно совпадать с заголовком ответа. 
- В целях определения заголовка регистр не учитывается. Например, все следующие варианты `Cache-Control` имя заголовка можно использовать для его идентификации:
    - cache-control;
    - CACHE-CONTROL;
    - cachE-Control.
...
Value|Результат
-|-
Enabled|Запросы могут создавать частично кэшированное содержимое.
...
**Назначение:** определяет, подойдет ли кэшированное содержимое для ранней повторной сверки в до истечения его срока действия.
...
**Поведение по умолчанию:** Off. Повторную проверку можно провести только после истечения срока жизни кэшированного содержимого.
...
**Назначение:** определяет набор [заголовков запросов HTTP в Verizon](cdn-verizon-http-headers.md) , перенаправляемых от точки подключения к серверу-источнику.
...
**Поведение по умолчанию:** заголовки запроса для всех относящихся к CDN, пересылаются на сервер-источник.
...
Value|Результат
--|--
Enabled|В результате происходит повторное получение ресурса с сервера-источника точкой подключения.
...
По умолчанию только включено кэширование для ответов 200 ОК.

Укажите ряд необходимых кодов состояния, разделенных пробелами.

Основные сведения

- Включите функцию Ignore Origin No-Cache. Если эта функция не включена, то ответы 200 ОК не будут кэшироваться.
- Набор допустимых кодов состояния для этой функции являются: 203, 300, 301, 302, 305, 307, 400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 417, 500, 501, 502, 503, 504 и 505.
- Эта функция не может использоваться, чтобы отключить кэширование для ответов, создающих код состояния 200 ОК.
...
**Назначение:** добавляет пользовательский заголовок, который определяет запрашивающего клиента по IP-адресу к запросу.

Параметр Header name определяет имя пользовательского заголовка запроса хранения IP-адрес клиента.

Эта функция позволяет серверу-источнику клиента узнать IP-адреса клиента с помощью пользовательского заголовка запроса. Если запрос обрабатывается из кэша, сервер-источник не получит IP-адрес клиента. Таким образом рекомендуется использование этой функции с ресурсами, которые не кэшируются.

Убедитесь, что указанное имя заголовка не совпадает с перечисленными ниже именами.

- Стандартные имена заголовков запросов. Список стандартных имен заголовков можно найти в [RFC 2616](https://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html).
...
**Назначение:** определяет, будет ли доставлено содержимое с истекшим сроком, когда произошла ошибка во время повторной сверки кэша или при получении запрошенного содержимого с сервера-источника клиента.

Value|Результат
-|-
Enabled|При возникновении ошибки во время подключения к серверу-источнику инициатору запроса передается устаревшее содержимое.
...
**Назначение:** повышает производительность, позволяя точкам подключения обслуживать устаревшее содержимое для запрашивающей стороны во время повторной сверки.

Основные сведения

- Поведение этой функции зависит от выбранной единицы времени.
    - **Единица времени:** укажите интервал времени и выберите единицу времени (например, секунд, минут, часов, и т.д.) чтобы разрешить доставку устаревшего содержимого. Этот тип установки позволяет CDN увеличивать продолжительность времени, он может предоставлять содержимое до начала проверки по следующей формуле: **Срок ЖИЗНИ** + **устаревших при повторной сверке времени** 
    - **OFF:** выберите «Выкл.» Чтобы требовать повторной проверки перед обработкой запроса устаревшее содержимое может обслуживаться.
        - Не указывайте промежуток времени, так как он неприменим и будет игнорироваться.

**Поведение по умолчанию:** Off. Повторная сверка должна проводиться перед обслуживанием запрошенного содержимого.
...
**Назначение:** определяет, применяется ли на запрос проверки подлинности на основе маркеров.
...
Ключ шифрования, который используется для шифрования и расшифровки значений маркера, определяется первичным ключом и параметрами ключа резервного копирования на странице аутентификации на основе маркеров. Имейте в виду, что ключи шифрования специфичны для конкретной платформы.
...
Value | Результат
------|---------
Enabled | Защищает запрошенное содержимое с помощью проверки подлинности на основе маркеров. Принимаются запросы только от тех клиентов, которые предоставляют допустимый маркер и соответствуют требованиям. На транзакции FTP проверка подлинности на основе маркеров не распространяется.
...
**Назначение:** определяет тип ответа, возвращаемого пользователю при отклонении запроса из-за проверки подлинности на основе маркеров.
...
403|Запрещено|Это сообщение является стандартное сообщение о состоянии 403 Forbidden которое неавторизованный пользователь увидит при попытке доступа к защищенному содержимому.
...
Эта функция поддерживает перенаправление URL-адреса на URL-адрес определяемых пользователем, если он настроен для возврата кода состояния 3xx. Этот определяемый пользователем URL-адрес можно указать, сделав следующее:

1. Выберите код ответа 3xx для компонента кода отказа при проверки подлинности маркеров.
2. Выберите «Расположение» из параметра необязательного имени заголовка.
3. Значение параметра необязательного значения заголовка нужный URL-адрес.

Если URL-адрес не определен для кода состояния 3xx, стандартная страница ответа для кода состояния 3xx будет возвращаться для пользователя.

URL-адрес перенаправления применяется только для кодов ответов 3xx.

Параметра необязательного значения заголовка поддерживает буквенно-цифровые символы, кавычки и пробелы.

#### <a name="authentication"></a>Authentication

Эта функция поддерживает возможность включения заголовка WWW-Authenticate при ответе на неавторизованный запрос на содержимое, защищенное с помощью проверки подлинности на основе маркеров. Если в заголовке WWW-Authenticate было присвоено «базовый» в конфигурации, а затем неавторизованный пользователь будут запрашиваться учетные данные учетной записи.
...
2. Выберите «WWW-Authenticate» из параметра необязательного имени заголовка.
3. Значение параметра необязательного значения заголовка в «базовый».

В заголовке WWW-Authenticate применим только для кодов ответов 401.
...
**Назначение:** определяет, являются ли регистр в сравнениях URL-адрес, выполненных с помощью проверки подлинности на основе маркеров.
...
Value|Результат
---|----
Enabled|Приводит к тому, что точка подключения не учитывает регистр знаков при сравнении URL-адресов для параметров аутентификации на основе маркеров.
...
- Параметр Value определяет имя параметра строки запроса, через который может быть указан маркер.
- Значение не может быть установлен в «ec_token.»
- Убедитесь, что имя, указанное в параметре Value содержит только допустимые символы URL-адрес.

Value|Результат
----|----
Enabled|Параметр Value определяет имя параметра строки запроса, через который следует определять маркеры.
...
Настоятельно рекомендуется использовать абсолютный URL-адрес. Использование относительного URL-адреса может привести к перенаправлению URL-адресов CDN по недопустимому пути.
...
Этот URL-адрес перенаправления можно получить с помощью следующей конфигурации: ![URL-адрес перенаправления](./media/cdn-rules-engine-reference/cdn-rules-engine-redirect.png)
...
- Все соответствующие запросы будут перенаправляться в edge, URL-адрес записи CNAME, определенный в параметре Destination. 
    - Пример сценария №1: 
...
- Переменная схемы запроса (% {scheme}) используется в параметре "Назначение", которая гарантирует, что схема запроса останется неизменной после перенаправления.
...
**Назначение:** перезаписывает URL-АДРЕСЕ запроса.
...
-|-
 ...
 Место назначения  |Определите относительный URL-адрес, на который будут перезаписываться указанные выше запросы. <br/>    1. Выбор точки доступа к содержимому, который определяет сервер-источник. <br/>    2. Определение относительного пути с помощью следующих компонентов: <br/>        -Шаблон регулярного выражения <br/>        - [Переменные HTTP](cdn-http-variables.md) <br/> <br/> Замените значения, записанные из исходного шаблона в целевом шаблоне с помощью $_n_ где _n_ определяет значение в порядке, в котором оно было записано. Например, $1 представляет собой первое значение, записанное из исходного шаблона, а $2 — второе значение. 
...
Этот URL-адрес перенаправления можно получить с помощью следующей конфигурации: ![URL-адрес перенаправления](./media/cdn-rules-engine-reference/cdn-rules-engine-rewrite.png)
...
Этот URL-адрес перенаправления можно получить с помощью следующей конфигурации: ![URL-адрес перенаправления](./media/cdn-rules-engine-reference/cdn-rules-engine-to-lowercase.png)
