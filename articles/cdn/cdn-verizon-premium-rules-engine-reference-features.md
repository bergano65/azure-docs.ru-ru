---
title: Azure CDN от Verizon Premium функции правил Документы Майкрософт
description: Справочная документация для Azure CDN от функций движка правил Verizon Premium.
services: cdn
author: asudbring
...
ms.locfileid: "81253498"
---
# <a name="azure-cdn-from-verizon-premium-rules-engine-features"></a>Azure CDN от функций движка правил Verizon Premium
...
[Дрожь пропускной способности](#bandwidth-throttling) | Регулирует пропускную способность для передачи ответа от точки подключения.
[Не использовать кэш](#bypass-cache) | Определяет, нужно ли пропустить кэширование для запроса.
[Обработка заголовка Cache-Control](#cache-control-header-treatment) | Контролирует создание заголовков `Cache-Control` точкой подключения при включенной функции External Max-Age.
...
[Полная заполнение кэша](#complete-cache-fill) | Определяет, что происходит, когда запрос завершается частичным промахом кэша в точке подключения.
...
[Запрос на четное кэш](#honor-no-cache-request) | Определяет, перенаправляются ли запросы без кэша HTTP-клиента на сервер-источник.
...
[Частичное совместное использование кэша](#partial-cache-sharing) | Определяет, может ли запрос создавать частично кэшированное содержимое.
...
Имя | Назначение
-----|--------
[Поддержка перенаправлений](#follow-redirects) | Определяет, можно ли перенаправлять запросы к имени узла, определенном в заголовке Location, возвращенным сервером-источником клиента.
[URL Перенаправление](#url-redirect) | Перенаправляет запросы через заголовок Location.
[Переопределение URL-адреса](#url-rewrite)  | Перезаписывает URL-адрес запроса.

## <a name="azure-cdn-from-verizon-premium-rules-engine-features-reference"></a>Azure CDN от Verizon Premium правила двигателя особенности ссылки
...
Значение|Результат
--|--
Активировано | Заголовок ответа Age включен в ответ, отправляемый инициатору запроса.
Выключено | Заголовок ответа Age исключен из ответа, отправляемого инициатору запроса.

**Поведение по умолчанию**: Отключено.

[В начало](#azure-cdn-from-verizon-premium-rules-engine-features)
...
Значение|Результат
--|--
Активировано|Позволяет точке подключения удовлетворять запросы на регулирование ширины полосы пропускания.
Выключено|Приводит к игнорированию параметров регулирования пропускной способности точки подключения. Запрашиваемое содержимое предоставляется обычным способом (т. е. без регулирования пропускной способности).
...
**Поведение по умолчанию:** Отключен.
...
Значение|Результат
--|--
Активировано|Приводит к тому, что все запросы достигают сервера-источника, даже если содержимое ранее было кэшировано на точках подключения.
Выключено|Приводит к тому, что точки подключения кэшируют ресурсы в соответствии с политикой кэширования, заданной в заголовках ответов.
...
Значение|Результат
--|--
Overwrite|Обеспечивает выполнение следующих действий:<br/> — перезапись заголовка `Cache-Control`, созданного на сервере-источнике; <br/>— добавление заголовка `Cache-Control`, созданного функцией External Max-Age, в ответ.
...
- **Запрос на выборку:** http://wpc.0001.&lt;Domain&gt;/800001/Origin/folder/asset.htm?sessionid-1234&language-EN&userid-01
...
- **Тип:** исключить.
- **Параметр (ы):** сеансированный userid
...
Новый путь|Определяет относительный путь для нового ключа кэша. Относительный путь можно задать, выбрав базовый исходный путь, а затем определив шаблон регулярного выражения. Этот относительный путь может быть динамически построен с помощью [переменных HTTP.](cdn-http-variables.md)
...
Значение|Результат
--|--
Активировано|Восстанавливает поведение по умолчанию. Поведение по умолчанию — принудительное получение точкой подключения фоновых данных ресурса с сервера-источника. После этого ресурс появится в локальном кэше точки подключения.
Выключено|Предотвращает получение фоновых данных для ресурса в точке подключения. В результате при выполнении следующего запроса к ресурсу из региона точка подключения запрашивает информацию в кэше пользовательского сервера-источника.
...
-|-|-
Заголовок запроса|`%{[RequestHeader]()}[i]()` | %{Accept-Encoding}i <br/> «Реферер») <br/> %{Authorization}i
...
Значение|Результат
-|-
Активировано|Запросы для заголовков ответа Debug Cache возвратят ответ, который включает заголовок X-EC-Debug.
Выключено|Заголовок ответа X-EC-Debug будет исключен из ответа.
...
Значение | Результат
------|-------
Активировано| Приводит к отклонению всех запросов, соответствующих критериям сопоставления, с ответом 403 (запрещено).
Выключено| Восстанавливает поведение по умолчанию. Поведение по умолчанию предполагает разрешение серверу-источнику определять тип возвращаемого ответа.
...
Значение|Результат
--|--
Overwrite|Обеспечивает выполнение следующих действий:<br/>— перезапись заголовка `Expires`, созданного на сервере-источнике;<br/>— добавление заголовка `Expires`, созданного функцией External Max-Age, в ответ.
...
Значение|Результат
-|-
Активировано|Запросы можно перенаправлять.
Выключено|Запросы не будут перенаправляться.
...
Значение|Результат
--|--
Активировано|Это позволяет перенаправлять запросы HTTP-клиента без кэша на сервер-источник, который будет возвращать заголовки и текст ответов от точки подключения обратно в HTTP-клиент.
Выключено|Восстанавливает поведение по умолчанию. Поведение по умолчанию — предотвратить перенаправление запросов без кэша на сервер-источник.
...
Значение|Результат
-|-
Активировано|Блокирует ответы от точек подключения на неправильно сформированный запрос байтового диапазона с кодом состояния 416 (запрос на диапазон не может быть удовлетворен). Вместо этого серверы предоставят запрошенный ресурс и возвратят ответ 200 ОК клиенту.
Выключено|Восстанавливает поведение по умолчанию. Поведение по умолчанию — учитывать код состояния 416 (Запрошенный диапазон невыполним).
...
Значение|Результат
-|-
Активировано|Позволяет хранить строки запроса во время записи URL-адресов в журнале доступа. Если URL-адрес не содержит строку запроса, этот параметр не будет иметь влияния.
Выключено|Восстанавливает поведение по умолчанию. Поведение по умолчанию — игнорировать строки запроса во время записи URL-адресов в журнал доступа.
...
-|-|-
Append|Указанное значение будет добавлено в конец имеющегося значения заголовка запроса.|**Значение заголовка запроса (клиент):**<br/>Значение1<br/>**Значение заголовка запроса (обработчик правил):**<br/>Значение2 <br/>**Новое значение заголовка запроса:** <br/>Value1Value2
Overwrite|Для значения заголовка запроса будет присвоено указанное значение.|**Значение заголовка запроса (клиент):**<br/>Значение1<br/>**Значение заголовка запроса (обработчик правил):**<br/>Значение2<br/>**Новое значение заголовка запроса:**<br/> Значение2 <br/>
...
Параметр|Описание|Пример
-|-|-
Append|Указанное значение будет добавлено в конец имеющегося значения заголовка ответа.|**Значение заголовка ответа (клиент):**<br />Значение1<br/>**Значение заголовка ответа (обработчик правил):**<br/>Значение2<br/>**Новое значение заголовка ответа:**<br/>Value1Value2
Overwrite|Заголовку ответа будет присвоено указанное значение.|**Значение заголовка ответа (клиент):**<br/>Значение1<br/>**Значение заголовка ответа (обработчик правил):**<br/>Значение2 <br/>**Новое значение заголовка ответа:**<br/>Значение2 <br/>
...
Значение|Результат
-|-
Активировано|Запросы могут создавать частично кэшированное содержимое.
Выключено|Запросы могут создавать только полностью кэшированную версию запрошенного содержимого.
...
**Поведение по умолчанию:** Выключить. Повторную проверку можно провести только после истечения срока жизни кэшированного содержимого.
...
Значение|Результат
--|--
Активировано|В результате происходит повторное получение ресурса с сервера-источника точкой подключения.
Выключено|Восстанавливает поведение по умолчанию. Поведение по умолчанию — обслуживать допустимые ресурсы кэша по запросу.

Эта функция не является обязательной для надлежащего кэширования и доставки содержимого, но может использоваться в качестве обходного решения. Например, генераторы динамического содержимого на серверах-источниках могут случайным образом привести к получению ответов нулевой длины, отправляемых точкам подключения. Эти типы ответов, как правило, кэшируются точками подключения. Если вы знаете, что ответ на 0 байт никогда не является действительным ответом на такой контент, эта функция может предотвратить эти типы активов от пособнии для ваших клиентов.
...
Значение|Результат
-|-
Активировано|При возникновении ошибки во время подключения к серверу-источнику инициатору запроса передается устаревшее содержимое.
Выключено|Ошибка сервера-источника перенаправляется инициатору запроса.
...
**Поведение по умолчанию:** Выключить. Повторная сверка должна проводиться перед обслуживанием запрошенного содержимого.
...
Значение | Результат
------|---------
Активировано | Защищает запрошенное содержимое с помощью проверки подлинности на основе маркеров. Принимаются запросы только от тех клиентов, которые предоставляют допустимый маркер и соответствуют требованиям. На транзакции FTP проверка подлинности на основе маркеров не распространяется.
Выключено| Восстанавливает поведение по умолчанию. Поведение по умолчанию заключается в том, что конфигурация проверки подлинности на основе маркеров может определять, будет ли запрос защищен.
...
Значение|Результат
---|----
Активировано|Приводит к тому, что точка подключения не учитывает регистр знаков при сравнении URL-адресов для параметров аутентификации на основе маркеров.
Выключено|Восстанавливает поведение по умолчанию. Поведение по умолчанию предполагает учет регистра при сравнении URL-адресов для проверки подлинности на основе маркеров.
...
Значение|Результат
----|----
Активировано|Параметр Value определяет имя параметра строки запроса, через который следует определять маркеры.
Выключено|Маркер можно указать как неопределенный параметр строки запроса в URL-адресе запроса.

**Поведение по умолчанию:** Отключен. Маркер можно указать как неопределенный параметр строки запроса в URL-адресе запроса.
