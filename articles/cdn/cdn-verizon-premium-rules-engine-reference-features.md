...
[Не учитывать запрос без кэширования](#honor-no-cache-request) | Определяет, перенаправляются ли запросы без кэша HTTP-клиента на сервер-источник.
...
[Общий доступ к частичному кэшу](#partial-cache-sharing) | Определяет, может ли запрос создавать частично кэшированное содержимое.
...
Отключен | Заголовок ответа Age исключен из ответа, отправляемого инициатору запроса.
...
Отключен|Приводит к игнорированию параметров регулирования пропускной способности точки подключения. Запрашиваемое содержимое предоставляется обычным способом (т. е. без регулирования пропускной способности).
...
Отключен|Приводит к тому, что точки подключения кэшируют ресурсы в соответствии с политикой кэширования, заданной в заголовках ответов.
...
- **Пример запроса:** http://wpc.0001.&lt;D омаин&gt;/800001/Origin/Folder/Asset.htm? SessionID = 1234&Language = EN&UserID = 01
...
Новый путь|Определяет относительный путь для нового ключа кэша. Относительный путь можно задать, выбрав базовый исходный путь, а затем определив шаблон регулярного выражения. Этот относительный путь может быть динамически создан с помощью [переменных HTTP](cdn-http-variables.md).
...
### <a name="comment"></a>Добавление примечаний
...
Отключен|Предотвращает получение фоновых данных для ресурса в точке подключения. В результате при выполнении следующего запроса к ресурсу из региона точка подключения запрашивает информацию в кэше пользовательского сервера-источника.
...
Отключен|Заголовок ответа X-EC-Debug будет исключен из ответа.
...
Отключен| Восстанавливает поведение по умолчанию. Поведение по умолчанию предполагает разрешение серверу-источнику определять тип возвращаемого ответа.
...
Отключен|Запросы не будут перенаправляться.
...
Отключен|Восстанавливает поведение по умолчанию. Поведение по умолчанию — предотвратить перенаправление запросов без кэша на сервер-источник.
...
Отключен|Восстанавливает поведение по умолчанию. Поведение по умолчанию — учитывать код состояния 416 (Запрошенный диапазон невыполним).
...
Отключен|Восстанавливает поведение по умолчанию. Поведение по умолчанию — игнорировать строки запроса во время записи URL-адресов в журнал доступа.
...
Удалить|Удаляет указанный заголовок запроса.|**Значение заголовка запроса (клиент):**<br/>Значение1<br/>**Конфигурация заголовка запроса Modify Client:**<br/>удаление указанного заголовка запроса.<br/>**Результат:**<br/>указанный заголовок запроса не будет перенаправляться на сервер-источник.
...
Удалить|Удаляет указанный заголовок ответа.|**Значение заголовка ответа (клиент):**<br/>Значение1<br/>**Конфигурация заголовка ответа Modify Client:**<br/>удаление указанного заголовка ответа.<br/>**Результат:**<br/>указанный заголовок ответа не будет перенаправляться инициатору запроса.
...
Отключен|Запросы могут создавать только полностью кэшированную версию запрошенного содержимого.
...
**Поведение по умолчанию:** Автоном. Повторную проверку можно провести только после истечения срока жизни кэшированного содержимого.
...
Отключен|Восстанавливает поведение по умолчанию. Поведение по умолчанию — обслуживать допустимые ресурсы кэша по запросу.
...
Отключен|Ошибка сервера-источника перенаправляется инициатору запроса.
...
**Поведение по умолчанию:** Автоном. Повторная сверка должна проводиться перед обслуживанием запрошенного содержимого.
...
Отключен| Восстанавливает поведение по умолчанию. Поведение по умолчанию заключается в том, что конфигурация проверки подлинности на основе маркеров может определять, будет ли запрос защищен.
...
Отключен|Восстанавливает поведение по умолчанию. Поведение по умолчанию предполагает учет регистра при сравнении URL-адресов для проверки подлинности на основе маркеров.
...
Отключен|Маркер можно указать как неопределенный параметр строки запроса в URL-адресе запроса.

**Поведение по умолчанию:** Доступ. Маркер можно указать как неопределенный параметр строки запроса в URL-адресе запроса.
