...
- Страна: Разрешение или отклонение запросов, поступающих из стран/регионов, указанных по их [код страны](/previous-versions/azure/mt761717(v=azure.100)).
- URL-адрес: Разрешить только запросы, которые соответствуют указанному ресурсу или пути.
- "Узел": Разрешение или отклонение запросов, которые используются указанные узлы в заголовке запроса.
- Источник ссылки. Разрешение или отклонение запроса из указанного источника ссылки.
- IP-адрес: Разрешить только запросов, отправленных с конкретного IP-адреса или IP-подсеть.
- Протокол: Разрешение или отклонение запросов, в зависимости от протокола, используемого для запроса содержимого.
...
2. Наведите указатель мыши на элемент **Большой HTTP-объект** и щелкните **Аутентификация на основе маркеров** во всплывающем элементе. Затем можно настроить ключ шифрования и параметры шифрования следующим образом.
   ...
   3. Выберите минимальную версию шифрования для каждого ключа в списке **Минимальная версия шифрования**, а затем щелкните **Обновить**.
      - **V2**: Указывает, что ключ может использоваться для создания маркеров версии 2.0 и 3.0. Этот режим следует использовать только при переходе с ключа шифрования устаревшей версии 2.0 на ключ версии 3.0.
      - **V3**: (Рекомендуется) Указывает, что ключ может использоваться только для создания маркеров версии 3.0.

      ![Настройка ключей для аутентификации на основе маркеров CDN](./media/cdn-token-auth/cdn-token-auth-setupkey.png)
    
   4. Используйте инструмент шифрования для настройки параметров шифрования и создания маркера. С его помощью можно разрешить или запретить запросы в соответствии с на время истечения срока действия, страны, источника ссылки, протокола и IP-адрес клиента (в любой комбинации). Хотя количество и сочетание параметров, которые можно объединить для формирования маркера, не ограничено, общая длина маркера не должна превышать 512 символов. 
      ...
      >       </ul></li>
      >       <li>Входное значение `/pictures/`: Только запросы, содержащие `/pictures/` путь разрешены. Например, `http://www.mydomain.com/pictures/city/strasbourg.png`.</li>
      >       <li>Входное значение `/pictures/city/strasbourg.png`: Разрешены только запросы для этого конкретного пути и ресурса.</li>
      >    </ul>
      > </tr>
      > <tr>
      >    <td><b>ec_country_allow</b></td> 
      >    <td>Разрешает только запросы, отправленные из одной или нескольких указанного странах и регионах. Отклоняются запросы, поступающие из всех других странах и регионах. Укажите двухбуквенный [код каждой страны в формате ISO 3166](/previous-versions/azure/mt761717(v=azure.100)), разделяя значения запятой. Не добавляйте пробелы. Например, если вы хотите разрешить доступ только из США и Франции, введите в поле `US,FR`.</td>
      > </tr>
      > <tr>
      >    <td><b>ec_country_deny</b></td> 
      >    <td>Отклоняет запросы, отправленные из одной или нескольких указанного стран и регионов. Разрешены запросы, поступающие из всех других странах и регионах. Реализация аналогична параметру <b>ec_country_allow</b>. Если код страны указан в параметрах <b>ec_country_allow</b> и <b>ec_country_deny</b>, то параметр <b>ec_country_allow</b> имеет больший приоритет.</td>
      > </tr>
      > <tr>
   ...
   6. Выберите версию шифрования из **версию шифрования** списка: **V2** для версии 2 или **V3** для версии 3 (рекомендуется). 
...
4. В подсистеме правил можно также включить дополнительные функции аутентификации на основе маркеров. Чтобы включить какую-либо из приведенных ниже функций, выберите ее из списка **Features** (Функции), а затем выберите **Enabled** (Включено).
   ...
   - **[Код отказа при проверки подлинности маркера](cdn-verizon-premium-rules-engine-reference-features.md#token-auth-denial-code)** : Определяет тип ответа, возвращаемого пользователю при отклонении запроса. Заданные здесь правила переопределяют код ответа, настроенный в разделе **Custom Denial Handling** (Пользовательская обработка отказов) на странице аутентификации на основе маркеров.

   - **[Аутентификации на основе маркеров не учитывать регистр URL-адрес](cdn-verizon-premium-rules-engine-reference-features.md#token-auth-ignore-url-case)** : Определяет, учитывается ли регистр в URL-адрес, используемый для проверки токена.

   - **[Параметр маркера проверки подлинности](cdn-verizon-premium-rules-engine-reference-features.md#token-auth-parameter)** : Переименовывает параметр строки запроса аутентификации на основе маркеров в запрошенного URL-адреса. 
