...
- Страна: Разрешить или отклонить запросы, которые поступают из стран/регионов, указанных их [кодом страны.](/previous-versions/azure/mt761717(v=azure.100))
...
2. Наведите указатель мыши на элемент **Большой HTTP-объект** и щелкните **Аутентификация на основе маркеров** во всплывающем элементе. Затем можно настроить ключ шифрования и параметры шифрования следующим образом.
   ...
   1. Создайте один или несколько ключей шифрования. В ключе шифрования учитывается регистр, и он может содержать любое сочетание буквенно-цифровых символов. Любые другие типы символов, включая пробелы, не допускаются. Максимальная длина составляет 250 символов. Чтобы убедиться, что ваши ключи шифрования являются случайными, рекомендуется создавать их с помощью [инструмента OpenSSL.](https://www.openssl.org/) 
   ...
   4. Используйте инструмент шифрования для настройки параметров шифрования и создания маркера. С помощью инструмента шифрования можно разрешить или отклонить запросы в зависимости от времени истечения срока действия, страны/региона, реферера, протокола и иС клиента (в любой комбинации). Хотя количество и сочетание параметров, которые можно объединить для формирования маркера, не ограничено, общая длина маркера не должна превышать 512 символов. 
      ...
      >    <td>Отклоняет запросы из указанного источника ссылки. Реализация такая же, как и параметр <b>ec_ref_allow.</b> Если источник ссылки указан в параметрах <b>ec_ref_allow</b> и <b>ec_ref_deny</b>, то параметр <b>ec_ref_allow</b> имеет больший приоритет.</td>
      > </tr>
      > <tr> 
