...
4. В подсистеме правил можно также включить дополнительные функции аутентификации на основе маркеров. Чтобы включить какую-либо из приведенных ниже функций, выберите ее из списка **Features** (Функции), а затем выберите **Enabled** (Включено).
   ...
   - **[Код отказа при проверки подлинности маркера](cdn-rules-engine-reference-features.md#token-auth-denial-code)**: Определяет тип ответа, возвращаемого пользователю при отклонении запроса. Заданные здесь правила переопределяют код ответа, настроенный в разделе **Custom Denial Handling** (Пользовательская обработка отказов) на странице аутентификации на основе маркеров.

   - **[Аутентификации на основе маркеров не учитывать регистр URL-адрес](cdn-rules-engine-reference-features.md#token-auth-ignore-url-case)**: Определяет, учитывается ли регистр в URL-адрес, используемый для проверки токена.

   - **[Параметр маркера проверки подлинности](cdn-rules-engine-reference-features.md#token-auth-parameter)**: Переименовывает параметр строки запроса аутентификации на основе маркеров в запрошенного URL-адреса. 
