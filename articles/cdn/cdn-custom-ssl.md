...
Кроме того, необходимо связать личный домен Azure CDN с конечной точкой CDN. Дополнительные сведения см. в статье [Руководство. Добавление личного домена к конечной точке Azure CDN](cdn-map-content-to-custom-domain.md) 
...
1. На [портале Azure](https://portal.azure.com) перейдите к своему профилю **Azure CDN уровня "Стандартный" от Майкрософт**, **Azure CDN уровня "Стандартный" от Akamai**, **Azure CDN уровня "Стандартный" от Verizon** и **Azure CDN уровня "Премиум" от Verizon**.
...
3. Выберите **Получить разрешения секретов** и откройте список **Разрешения сертификатов**, чтобы разрешить CDN использовать такие разрешения для получения и вывода списка сертификатов. 
...
7. *Как работает продление сертификатов с помощью собственного сертификата?*
    ...
    Чтобы обеспечить развертывание нового сертификата в инфраструктуре PoP, просто отправьте новый сертификат в Azure KeyVault, а затем в параметрах SSL на Azure CDN выберите самую новую версию сертификата и нажмите кнопку "Сохранить". Azure CDN затем распространит ваш новый обновленный сертификат. 
