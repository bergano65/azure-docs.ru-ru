...
Кроме того, необходимо связать личный домен Azure CDN с конечной точкой CDN. Дополнительные сведения см. в разделе [учебник. Добавление пользовательского домена в конечную точку Azure CDN](cdn-map-content-to-custom-domain.md).
...
2. В PowerShell запустите следующую команду:
...
6. *20 июня 2018 Azure CDN от Verizon начал использовать выделенный сертификат с SNI TLS/SSL по умолчанию. Что происходит с существующими личными доменами с использованием сертификата альтернативного имени субъекта (SAN) и протокола TLS/SSL на основе IP-адреса?*
...
7. *Как работает продление сертификатов с помощью собственного сертификата?*
    ...
    Чтобы обеспечить развертывание нового сертификата в инфраструктуре PoP, просто отправьте новый сертификат в Azure KeyVault, а затем в параметрах SSL на Azure CDN выберите самую новую версию сертификата и нажмите кнопку "Сохранить". Затем Azure CDN распространит ваш обновленный сертификат. 
