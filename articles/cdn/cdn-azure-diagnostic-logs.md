...
- Экспорт данных в журналы Azure Monitor и просмотр данных в своем рабочем пространстве Log Analytics
...
Войдите на [портал Azure](https://portal.azure.com). Если для рабочего процесса не включена CDN, [создайте профиль и конечную точку Azure CDN](cdn-create-new-endpoint.md) перед продолжением.
...
### <a name="logging-with-azure-monitor"></a>Регистрация с azure Monitor
...
2. Выберите **настройку** для настройки журналов Azure Monitor. 
...
12. Для просмотра новой рабочей области Log Analytics выберите **Основная аналитика** со страницы конечной точки CDN.
    ...
    Теперь рабочая область Log Analytics готова для записи данных в журнал. Для того, чтобы использовать эти данные, необходимо использовать [решение журналов Azure Monitor,](#consuming-diagnostics-logs-from-a-log-analytics-workspace)охватываемое позднее в этой статье.
...
1.  Откройте **исследователь хранения данных Microsoft Azure**
...
|Месяц| 2-значное обозначение номера месяца 01 января ... 12-декабрь|
...
Azure Monitor — это служба Azure, которая отслеживает облачные и штатные среды для поддержания их доступности и производительности. Она собирает данные, формируемые ресурсами в облачных и локальных средах, а также другими средствами мониторинга, и на их основе предоставляет аналитические сведения для нескольких источников. 

Чтобы использовать Azure Monitor, необходимо [включить журнал в](#enable-logging-with-azure-storage) рабочее пространство Analytics журналов Azure, о котором говорилось ранее в этой статье.
...
Вы можете установить решения мониторинга с рынка Azure, выбрав ссылку **Get it в** нижней части каждого решения.

### <a name="add-an-azure-monitor-cdn-monitoring-solution"></a>Добавьте решение мониторинга CDN Azure Monitor

Выполните следующие действия, чтобы добавить решение мониторинга Azure Monitor:
...
* [Диагностические журналы Azure](https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitoring-overview-of-diagnostic-logs)
