...
description: В этом руководстве показано, как создать кластер виртуальной машины Linux с подсистемой балансировки нагрузки в Azure с помощью модулей Terraform и HCL
...
> [!NOTE]
> Если вы [используете переменные среды Terraform](terraform-install-configure.md) или выполняете это руководство в [Azure Cloud Shell](terraform-cloud-shell.md), пропустите этот раздел.
...
1. Настройте [субъект-службу Azure AD](terraform-install-configure.md#set-up-terraform-access-to-azure), чтобы предоставить Terraform возможность подготовить ресурсы в Azure. При создании субъекта запишите следующие значения: идентификатор подписки, клиент, идентификатор приложения и пароль.
