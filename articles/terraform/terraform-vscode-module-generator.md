...
1. Создайте папку $GOPATH, если она еще не существует. Для этого введите `mkdir ~/go`.

1. Создайте папку в папке $GOPATH, введя `mkdir ~/go/src`. Эта папка будет использоваться для хранения и упорядочения различных папок проекта, которые вы создадите, например папки <имя_модуля>, которую мы создадим на следующем шаге.

1. Создайте папку для хранения вашего модуля Terraform. Для этого введите `mkdir ~/go/src/<your-module-name>`.

    >[!NOTE]
    >В этом примере мы выбрали `my-module-name` в качестве имени папки.

1. Перейдите в папку модуля, введя `cd ~/go/src/<your-module-name>`.
