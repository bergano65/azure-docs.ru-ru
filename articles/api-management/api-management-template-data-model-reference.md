...
|`subscriptionKeyParameterNames`|объект|Необязательное свойство, которое может использоваться для указания пользовательских имен для параметров запроса и (или) заголовка, содержащих ключ подписки. Это свойство должно содержать хотя бы одно из двух следующих свойств.<br /><br /> `{   "subscriptionKeyParameterNames":   {     "query": “customQueryParameterName",     "header": “customHeaderParameterName"   } }`|  
...
|`required`|boolean|Указывает, требуется ли заголовок.|  
|`readOnly`|boolean|Указывает, доступен ли заголовок только для чтения.|  
...
|`ShowAll`|boolean|Указывает, следует ли показывать все результаты на одной странице.|  
...
|`AllowMultipleSubscriptions`|boolean|Указывает, может ли пользователь одновременно иметь несколько подписок на этот продукт.|  
...
|`CanBeCancelled`|boolean|Указывает, может ли текущий пользователь отменить подписку.|  
|`IsAwaitingApproval`|boolean|Указывает, ожидает ли подписка утверждения.|  
...
|`CanBeRenewed`|boolean|Указывает, можно ли обновить подписку для текущего пользователя.|  
|`HasExpired`|boolean|Указывает, истек ли срок действия подписки.|  
|`IsRejected`|boolean|Указывает, отклонен ли запрос на подписку.|  
...
|`IsBasicAccount`|boolean|Значение true, если эта учетная запись зарегистрирована с использованием электронной почты и пароля. Значение false, если учетная запись зарегистрирована с помощью поставщика.|  
...
|`RememberMe`|boolean|Указывает, следует ли сохранять сведения о текущем пользователе.|  
|`RegistrationEnabled`|boolean|Указывает, включена ли регистрация.|  
|`DelegationEnabled`|boolean|Указывает, включен ли делегированный вход.|  
...
|`UserRegistrationTermsEnabled`|boolean|Указывает, включены ли условия использования.|  
...
|Свойство|type|Описание|  
|--------------|----------|-----------------|  
|`PasswordConfirm`|boolean|Значение, используемое элементом управления для [регистрации](api-management-page-controls.md#sign-up).|  
|`Password`|строка|Пароль учетной записи пользователя.|  
...
|`ConsentAccepted`|boolean|Значение, используемое элементом управления для [регистрации](api-management-page-controls.md#sign-up).|  
|`Email`|строка|Электронная почта. Не может быть пустым и должно быть уникальным в пределах экземпляра службы. Максимальная длина составляет 254 символа.|  
|`FirstName`|строка|Имя. Не может быть пустым. Максимальная длина составляет 100 символов.|  
|`LastName`|строка|Фамилия. Не может быть пустым. Максимальная длина составляет 100 символов.|  
