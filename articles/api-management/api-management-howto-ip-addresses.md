...
Общедоступные IP-адреса используются для внутреннего взаимодействия по порту `3443` — для управления конфигурацией (например, с помощью Azure Resource Manager). В конфигурации внешней виртуальной сети они также используются для трафика API среды выполнения. Когда запрос отправляется из службы управления API в общедоступную серверную часть (с выходом в Интернет), общедоступный IP-адрес будет отображаться в качестве источника запроса.
