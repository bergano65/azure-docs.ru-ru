...
Служба управления API каждую минуту передает метрики, позволяя отслеживать состояние и работоспособность API-интерфейсов практически в реальном времени. Ниже приведены две самые часто используемые метрики. Список всех доступных метрик см. в разделе со списком [поддерживаемых метрик](https://docs.microsoft.com/azure/azure-monitor/platform/metrics-supported#microsoftapimanagementservice).
