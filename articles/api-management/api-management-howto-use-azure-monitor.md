...
| durationMs | integer | Количество миллисекунд с момента поступления запроса в шлюз до момента полной отправки ответа. Он включает clienTime, cacheTime и backendTime. |
...
| responseCode | integer | Код состояния ответа HTTP, отправленного клиенту |
...
| backendResponseCode | integer | Код HTTP-ответа, полученного от серверной части |
| backendProtocol | строка | Версия протокола HTTP для запроса, отправленного в серверную часть | 
| requestSize | integer | Число байтов, полученных от клиента во время обработки запроса | 
| responseSize | integer | Число байтов, отправленных клиенту во время обработки запроса | 
| cache | строка | Состояние участия кэша управления API в обработке запроса (т. е. попадания, пропуски, ничего) | 
| cacheTime | integer | Время в миллисекундах, затраченное на все операции ввода-вывода кэша службы управления API (подключение, отправка и получение байтов) | 
| backendTime | integer | Время в миллисекундах, затраченное на все операции ввода-вывода серверной части (подключение, отправка и получение байтов) | 
| clientTime | integer | Время в миллисекундах, затраченное на все операции ввода-вывода клиента (подключение, отправка и получение байтов) | 
...
| elapsed | integer | Время в миллисекундах, истекшее с момента поступления запроса в шлюз и до момента возникновения ошибки | 
