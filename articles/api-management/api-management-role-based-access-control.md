...
Сейчас служба управления API предоставляет три встроенные роли, к которым скоро будут добавлены еще две. Эти роли можно назначать в разных областях, включая подписку, группу ресурсов и отдельный экземпляр службы управления API. Например, если вы назначаете роль "API Management Service Reader" пользователю на уровне группы ресурсов, пользователь прочитает доступ ко всем экземплярам управления API внутри группы ресурсов. 
...
| Читатель службы управления API | ✓ | | || Имеет доступ на чтение к службам и сущностям управления API. |
| Оператор услуг управления API | ✓ | | ✓ | | Может управлять службами управления API, но не может управлять сущностями.|
| Редактор службы управления API<sup>*</sup> | ✓ | ✓ | |  | Может управлять сущностями управления API, но не может управлять службами.|
| Менеджер по контенту управления API<sup>*</sup> | ✓ | | | ✓ | Может управлять порталом разработчика. Имеет доступ на чтение к службам и сущностям.|
...
<sup>\*Роль редактора службы будет доступна после того, как мы переодненим весь разбивательный доступ с существующего портала издателя на портал Azure. Роль менеджера контента будет доступна после рефакторинга портала издателя, чтобы содержать только функциональные возможности, связанные с управлением порталом разработчика.</sup>  
...
Чтобы узнать больше об управлении доступом на основе ролей в Azure, см. следующие статьи:
  ...
  * [Операции поставщика ресурсов ресурсов менеджера ресурсов Azure](../role-based-access-control/resource-provider-operations.md#microsoftapimanagement)
