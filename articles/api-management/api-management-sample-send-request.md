...
Использование автономного запроса связано с некоторыми компромиссами. Если по какой-либо причине запрос завершается ошибкой, сообщение о сбое не выводится. В этой конкретной ситуации сложности, связанные с необходимостью наличия вторичной системы сообщений о сбоях и дополнительными издержками при ожидании ответа, являются неоправданными. В ситуациях, где проверка ответа имеет важное значение, лучшим вариантом будет политика [запроса на отправку](/azure/api-management/api-management-advanced-policies#SendRequest) .
...
Основная задача API управления состоит в защите серверных ресурсов. Если сервер авторизации, используемый API-интерфейсом, создает [маркеры JWT](https://jwt.io/) в рамках потока OAuth2, как это делается в [Azure Active Directory](../active-directory/hybrid/whatis-hybrid-identity.md), с помощью политики `validate-jwt` можно проверить допустимость маркера. Некоторые серверы авторизации создают так называемые [маркеры ссылок](https://leastprivilege.com/2015/11/25/reference-tokens-and-introspection/), для проверки которых требуется обратный вызов к серверу авторизации.
...
Первым действием является извлечение маркера из заголовка авторизации. Значение заголовка должно быть отформатировано с помощью схемы авторизации `Bearer` — согласно [RFC 6750](https://tools.ietf.org/html/rfc6750#section-2.1) сначала указывается одиночный пробел, за которым следует маркер авторизации. К сожалению, существуют случаи, когда схема авторизации опускается. Чтобы учесть этот момент во время анализа, служба управления API разделяет значение заголовка и в возвращенном массиве строк выбирает последнюю строку. Это возможное решение позволяет справиться с неправильно отформатированными заголовками авторизации.
...
Кроме того, если сервер авторизации не включает "активное" поле, чтобы указать, является ли маркер допустимом, используйте инструмент, как Postman, чтобы определить, какие свойства установлены в действительном маркере. Например, если допустимый ответ маркера содержит свойство под названием "expires_in", проверьте, существует ли это имя свойства в ответе сервера авторизации таким образом:

<, когда условие» ((IResponse)контекст. Переменные »токенстейт»). Body.As<JObject>(). Недвижимость ("expires_in") - ноль)">
...
Для формирования составного запроса можно использовать политику [возврата ответа](/azure/api-management/api-management-advanced-policies#ReturnResponse). Элемент `set-body` может применять выражение для создания нового `JObject` со всеми представлениями компонентов, внедренными в качестве свойств.
