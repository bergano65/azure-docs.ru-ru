...
> [!TIP]
> Политики ограничения доступа можно использовать в разных областях для различных целей. Например, можно обезопасить весь API с `validate-jwt` помощью аутентификации AAD, применяя политику `claims` на уровне API, или применить ее на уровне операции API и использовать для более детального управления.

## <a name="check-http-header"></a><a name="CheckHTTPHeader"></a>Проверьте заголовок HTTP
...
## <a name="limit-call-rate-by-subscription"></a><a name="LimitCallRate"></a>Ограничьте скорость вызова по подписке
...
> [!CAUTION]
> Из-за распределенного характера архитектуры регулирования, ограничение скорости никогда не бывает полностью точным. Разница между настроенными и реальным числом разрешенных запросов варьируется в зависимости от объема запросов и скорости, задержки бэкэнда и других факторов.
...
| api        | Добавьте один или несколько из этих элементов, чтобы наложить ограничение скорости вызова на AIS внутри продукта. Ограничения частоты вызовов продукта и API применяются раздельно. Ссылаться на API можно с помощью `name` или `id`. Если указаны оба атрибута, `id` будет использоваться, а `name` — игнорироваться.                    | нет       |
| операции  | Добавьте один или несколько из этих элементов, чтобы наложить ограничение частоты вызовов на операции в Рамках API. Ограничения частоты вызовов продукта, API и операции применяются раздельно. Ссылаться на операцию можно с помощью `name` или `id`. Если указаны оба атрибута, `id` будет использоваться, а `name` — игнорироваться. | нет       |
...
-   **Сферы политики:** продукт, api, эксплуатация
...
| name              | Описание   | Обязательно |
| ----------------- | ------------- | -------- |
| ограничение скорости по ключам | Корневой элемент. | Да      |
...
В следующем примере политика разрешает запросы, поступающие либо с одного IP-адреса, либо с диапазона указанных IP-адресов
...
## <a name="set-usage-quota-by-subscription"></a><a name="SetUsageQuota"></a>Установите квоту использования по подписке
...
| api       | Добавьте один или несколько из этих элементов, чтобы навязать квоту вызова на AIS внутри продукта. Квоты для продукта и вызовов API применяются раздельно. Ссылаться на API можно с помощью `name` или `id`. Если указаны оба атрибута, `id` будет использоваться, а `name` — игнорироваться.                    | нет       |
| операции | Добавьте один или несколько из этих элементов, чтобы навязать квоту вызова на операции в рамках API. Квоты для продукта, API и вызовов операций применяются раздельно. Ссылаться на операцию можно с помощью `name` или `id`. Если указаны оба атрибута, `id` будет использоваться, а `name` — игнорироваться. | нет       |
...
#### <a name="simple-token-validation"></a>Простая проверка токенов
...
В этом примере показано, как использовать политику [Validate JWT](api-management-access-restriction-policies.md#ValidateJWT) для авторизации доступа к операциям на основе значения требований токенов.
...
| header-name                     | Имя заголовка НТТР, содержащего маркер.                                                                                                                                                                                                                                                                                                                                                                                                         | Один `header-name`из `query-parameter-name` `token-value` , или должны быть указаны. | Недоступно                                                                               |
| query-parameter-name            | Имя параметра запроса, содержащего маркер безопасности.                                                                                                                                                                                                                                                                                                                                                                                                     | Один `header-name`из `query-parameter-name` `token-value` , или должны быть указаны. | Недоступно                                                                               |
| токен-значение                     | Выражение, возвращающая строку, содержащую токен JWT                                                                                                                                                                                                                                                                                                                                                                                                     | Один `header-name`из `query-parameter-name` `token-value` , или должны быть указаны. | Недоступно                                                                               |
...
| require-scheme                  | Название схемы токенов, например, "Bearer". Когда задан этот атрибут, политики обеспечивают присутствие указанной схемы в значении заголовка Authorization.                                                                                                                                                                                                                                                                                    | нет                                                                               | Недоступно                                                                               |
...
| выходно-переменный-имя вывода      | Строка. Имя переменной контекста, которая получит значение [`Jwt`](api-management-policy-expressions.md) маркера как объект типа при успешной проверке токенов                                                                                                                                                                                                                                                                                     | нет                                                                               | Недоступно                                                                               |
