---
title: Безопасные услуги бэк-энда с помощью проверки подлинности сертификата клиента
...
API Management позволяет обеспечить доступ к бэк-энд-сервису API с помощью сертификатов клиентов. В этом руководстве описывается, как на портале Azure управлять сертификатами в экземпляре службы управления API Azure. Также объясняется, как настроить API для доступа к внутренней службе с помощью сертификата.
...
В этом руководстве описано, как настроить для экземпляра службы API Management проверку подлинности с помощью сертификата клиента при доступе из API к фоновой службе. Прежде чем выследить за шагом в этой статье, вы должны настроить свой бэк-энд службы для проверки подлинности сертификата клиента[(для настройки проверки подлинности сертификата в службе приложений Azure обратитесь к этой статье][to configure certificate authentication in Azure WebSites refer to this article]). Вам потребуется доступ к сертификату и паролю, чтобы передать их в службу управления API.

## <a name="upload-a-certificate"></a><a name="step1"> </a>Загрузить сертификат

> [!NOTE]
> Вместо загруженного сертификата можно использовать сертификат, хранящийся в службе [Azure Key Vault,](https://azure.microsoft.com/services/key-vault/) как показано в этом [примере.](https://github.com/galiniliev/api-management-policy-snippets/blob/galin/AkvCert/examples/Look%20up%20Key%20Vault%20certificate%20using%20Managed%20Service%20Identity%20and%20call%20backend.policy.xml)
...
2. Выберите **Сертификаты** из меню.
3. Щелкните **+ Добавить**.
...
После загрузки сертификата он отображается в **Сертификатах.**  Если у вас несколько сертификатов, сохраните отпечаток необходимого сертификата, чтобы [настроить API для аутентификации шлюза на основе сертификата клиента][Configure an API to use a client certificate for gateway authentication].
...
Если вы используете самозаверяющие сертификаты, вам потребуется отключить проверку цепочки сертификатов, чтобы служба управления API могла взаимодействовать с серверной системой. В противном случае будет возвращена ошибка с кодом 500. Для настройки этого можно [`New-AzApiManagementBackend`](https://docs.microsoft.com/powershell/module/az.apimanagement/new-azapimanagementbackend) использовать (для нового [`Set-AzApiManagementBackend`](https://docs.microsoft.com/powershell/module/az.apimanagement/set-azapimanagementbackend) задней части) или (для существующего `-SkipCertificateChainValidation` задней `True`части) смдлеты PowerShell и установить параметр.
