...
1. Перейдите к [Azure портал регистрации приложений](https://go.microsoft.com/fwlink/?linkid=2083908) страницы. 
...
1. После появления страницы **Регистрация приложения** введите сведения о регистрации приложения: 
    ...
    - В **поддерживаемые типы учетных записей** выберите **учетных записей в любой организации directory**. 

1. Оставьте **URI перенаправления** разделе %)) пустым.
...
1. В **URI перенаправления** выберите `Web` и введите URL-адрес `https://contoso5.portal.azure-api.net/signin`
...
1. В списке страниц для клиентского приложения, выберите **сертификаты и секреты**и выберите **новый секрет клиента**.

2. В разделе **Добавление секрета клиента**, предоставляют **описание**. Выберите, когда срок действия ключа и выберите **добавить**.
...
2. Выберите `client-app`и в список страниц для приложения перейдите в раздел **разрешения API**.

3. Выберите **добавить разрешение**.

4. В разделе **Выбор API**найдите и выберите `backend-app`.

5. В разделе **делегированные разрешения**, выберите нужные разрешения для `backend-app`.

6. Выберите **Добавление разрешений** 
...
6. Затем укажите **URL-адрес конечной точки авторизации** и **URL-адрес конечной точки маркера**. Эти значения можно извлечь на странице **конечных точек** в клиенте Azure Active Directory. Снова перейдите на страницу **Регистрация приложений** и выберите **Конечные точки**.
    ...
    >[!NOTE]
    > Используйте здесь конечные точки **v1**.
