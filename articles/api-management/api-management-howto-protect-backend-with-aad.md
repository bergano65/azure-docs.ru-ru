...
1. При необходимости на странице **разрешения API** выберите **предоставить согласие администратора для \<имени клиента>** , чтобы предоставить согласие от имени всех пользователей в этом каталоге. 
...
1. Если используются конечные точки **версии 2** , используйте область, созданную для внутреннего приложения, в поле **область по умолчанию** . Также убедитесь, что для [`accessTokenAcceptedVersion`](/azure/active-directory/develop/reference-app-manifest#accesstokenacceptedversion-attribute) свойства задано значение `2` в [манифесте приложения](/azure/active-directory/develop/reference-app-manifest).
...
```
> [!NOTE]
> Этот `openid-config` URL-адрес соответствует конечной точке v1. Для конечной точки `openid-config`версии 2 Используйте `https://login.microsoftonline.com/common/v2.0/.well-known/openid-configuration`.
