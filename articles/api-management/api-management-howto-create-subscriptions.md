...
При публикации API в службе управления API Azure самый простой способ защитить доступ к этим интерфейсам API — использовать ключи подписки. Клиентские приложения, которым требуется использовать опубликованные API, должны включать действительный ключ подписки в HTTP-запросах при выполнении вызовов к этим API. Чтобы получить ключ подписки для доступа к API, нужна подписка. Для получения дополнительной информации о подписках смотрите [подписки в управлении API Azure.](api-management-subscriptions.md)
...
4. Дополнительно выберите, должна ли подписка быть связана с пользователем.
...
+ Изучите другие [концепции](api-management-terminology.md) управления API.
+ Следите за [нашими учебниками,](import-and-publish.md) чтобы узнать больше об управлении API.
+ Проверьте нашу [страницу часто задаваемых вопросов](api-management-faq.md) для общих вопросов.