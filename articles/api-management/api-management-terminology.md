...
* **Frontend API**/**APIM API** - API APIM не размещает API, он создает фасады для ваших API для того, чтобы настроить фасад в соответствии с вашими потребностями, не касаясь API задней части. Дополнительные сведения см. в статье об [импорте и публикации API](import-and-publish.md).
