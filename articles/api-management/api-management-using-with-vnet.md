...
| Исходные и конечные порты | Направление          | Транспортный протокол |   [Сервис Теги](../virtual-network/security-overview.md#service-tags) <br> Ресурс и назначение   | Цель\*( )                                                 | Тип виртуальной сети |
|------------------------------|--------------------|--------------------|---------------------------------------|-------------------------------------------------------------|----------------------|
...
+ **Метрики и мониторинг работоспособности**: Исходящие сетевые подключения к конечным точкам мониторинга Azure, которые решаются в следующих доменах. Как показано в таблице, эти URL-адреса представлены в теге службы AzureMonitor для использования в группах сетевой безопасности.
...
+ **Региональные теги служб :** правила NSG, позволяющие исходящие подключения к хранению, S'L, и события концентраты теги могут использовать региональные версии тех тегов, соответствующих региону, содержащий API управления экземпляр (например, Storage.WestUS для API управления экземпляр в западном регионе США). В многорегионе развертывания, NSG в каждом регионе должны разрешить трафик на теги обслуживания для этого региона и первичного региона.
...
+ **Принудить туннельный трафик к локальным брандмауэру с помощью экспресс-маршрута или сетевого виртуального прибора**: общая конфигурация клиента заключается в определении их собственного маршрута по умолчанию (0.0.0.0/0), который заставляет весь трафик из aPI Management делегировать подсеть, чтобы течь через локальный брандмауэр или к виртуальному прибору сети. Этот поток трафика неизменно прерывает подключение к службе управления API Azure, так как исходящий трафик или блокируется локально, или преобразуется с помощью NAT в нераспознаваемый набор адресов, которые больше не относятся к различным конечным точкам Azure. Решение требует, чтобы вы сделали несколько вещей:
...
## <a name="related-content"></a><a name="related-content"> </a>Похожий контент
