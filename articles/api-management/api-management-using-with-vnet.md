...
* **Добавочные операции обновления.** При внесении изменений в вашей сети, см. [NetworkStatus API](https://docs.microsoft.com/rest/api/apimanagement/2019-01-01/networkstatus), чтобы убедиться, что служба управления API не потерял доступ к критически важным ресурсам, которых она зависит. Состояние подключения должно обновляться каждые 15 минут.
