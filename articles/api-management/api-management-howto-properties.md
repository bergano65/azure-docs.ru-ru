...
| --- | --- | --- |
| `Display name` |строка |Буквенно-цифровая строка, используемая для ссылки на свойство в политиках. |
| `Value` |строка |Значение свойства. Не может быть пустым или состоять только из пробелов. |
| `Secret` |Логическое|Определяет, является ли значение секретом и должно ли быть зашифровано.|
| `Tags` |Массив строк |Необязательные теги, которые (если указаны) можно использовать для фильтрации списка свойств. |
