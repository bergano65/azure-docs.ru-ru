...
| -------------- | --------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `Display name` | string          | Используется для ссылки на именованное значение в политиках. Строка от одного до 256 символов. Допускаются только буквы, цифры, точки и тире. |
| `Value`        | string          | Фактическое значение. Не должно быть пустым или состоять только из пробелов. Не более 4096 символов.                                        |
...
| ---------- | -------------------------- | ------ | ------------- |
| Значение      | 42                         | Нет  | важнейшие номера |
| Учетные данные | ••••••••••••••••••••••     | Да   | безопасность      |
| Expression | @(DateTime.Now.ToString()) | Нет  |               |
