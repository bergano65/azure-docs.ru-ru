...
После завершения развертывания вы должны увидеть **частный** виртуальный IP-адрес и **общедоступный** виртуальный IP-адрес службы управления API в колонке обзор. **Частный** виртуальный IP-адрес — это IP-адрес с балансировкой нагрузки в делегированной подсети управления API `gateway`, `portal`к `management` которой `scm` можно получить доступ конечные точки, и. **Общедоступный** виртуальный IP-адрес используется **только** для управления трафиком `management` плоскости в конечную точку через порт 3443 и может быть заблокирован в [ApiManagement][ServiceTags] сервицетаг.
