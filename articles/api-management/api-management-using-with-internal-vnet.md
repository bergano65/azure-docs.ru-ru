...
После завершения развертывания вы должны увидеть **частный** виртуальный IP-адрес и **общедоступный** виртуальный IP-адрес службы управления API в колонке обзор. **Частный** виртуальный IP-адрес — это IP-адрес с балансировкой нагрузки в пределах делегированной подсети управления API, через которую можно получить доступ к конечным точкам `gateway`, `portal`, `management` и `scm`. **Общедоступный** виртуальный IP-адрес используется **только** для того, чтобы трафик плоскости управления `management` конечной точке через порт 3443 и мог быть заблокирован в сервицетаг [ApiManagement][ServiceTags] .
