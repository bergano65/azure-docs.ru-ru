...
1. Используйте местозаполнители для URL-адресов ответа теперь https://localhost, например, мы будем обновлять эти URL-адреса позже.
...
1. Перейдите к любой операции в разделе `Echo API` на портале разработчика и выберите команду **попробовать** , чтобы открыть консоль разработчика.
...
1. После успешного входа в запрос добавляется `Authorization: Bearer` заголовок с маркером доступа из Azure AD B2C, закодированного в Base64. 
