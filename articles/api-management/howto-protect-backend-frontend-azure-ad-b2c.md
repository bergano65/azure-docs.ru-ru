...
Мы будем использовать протокол OpenID Connect Connect с Azure AD B2C, наряду с управлением API для защиты серверной части функций Azure с помощью Еасяус.
...
1. Задать WebApp/Web API и разрешить неявный потоку значение "Да"
...
1. Откройте API серверной части из списка приложений и выберите вкладку *ключи* (в разделе Общие) и нажмите кнопку "создать ключ", чтобы создать ключ проверки подлинности.
...
1. Затем в разделе "поставщики удостоверений" установите флажок "Регистрация идентификатора пользователя" и нажмите кнопку ОК. 
...
1. Вставьте идентификатор приложения API внутреннего сервера (из Azure AD B2C в поле "идентификатор клиента").
...
1. Добавьте три (или два, если используется модель использования управления API) идентификаторы приложений, записанные ранее для Azure AD B2C приложений в разрешенные аудитории маркеров. Этот параметр указывает Еасяус, какие значения утверждений AUD разрешены в полученных маркерах.
1. Нажмите кнопку ОК, а затем — сохранить.

   > [!NOTE]
   > Теперь API-интерфейс функции развернут и должен вызывать ошибки 401 или 403 для несанкционированных запросов и возвращать данные при появлении допустимого запроса.
   > Но у нас по-прежнему нет IP-безопасности, если у вас есть допустимый ключ, который можно вызвать из любого места. в идеале мы хотим принудительно выполнять все запросы через Управление API.
   > Кроме того, если вы используете уровень потребления для управления API, вы не сможете выполнять блокировку с помощью виртуального IP-адреса, так как для этого уровня нет выделенного статического IP, вам придется полагаться на метод блокировки вызовов API через общий секретный ключ функции. , поэтому шаги 11-14 будут невозможны.
...
1. Откройте колонку портала управления API и выберите экземпляр управления API.
...
1. Вернитесь к стандартному клиенту Azure AD в портал Azure, чтобы мы могли снова настроить элементы в подписке, а затем открыть *колонку управления API*, а затем открыть *свой экземпляр*.
...
1. Проверьте тип предоставления *неявной проверки подлинности* и при необходимости снимите флажок для параметра Тип предоставления кода авторизации.
...
1. Прокрутите вниз и заполните *дополнительный параметр Body* с именем Resource и идентификатором клиента API функции из Azure AD B2C регистрации приложения.
...
1. Выберите операцию API ниже "все API".
