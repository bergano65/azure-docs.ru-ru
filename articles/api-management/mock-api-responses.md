...
2. Щелкните **+ Add Operation** (+ Добавить операцию).
    ![Ответ макетированной операции](./media/mock-api-responses/mock-api-responses-add-operation.png)
...
4. Щелкните значок карандаша в окне **Обработка входящих запросов**.
5. На вкладке **Макетирование** выберите **Статические ответы** в качестве значения параметра **Поведение макетирования**.
6. В текстовом поле **API Management returns the following response:** (Служба управления API возвращает следующий ответ:) введите **200 OK, application/json**. Этот параметр указывает, что API должен возвращать пример ответа, определенный в предыдущем разделе.
