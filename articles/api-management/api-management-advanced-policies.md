...
| Атрибут                                              | ОПИСАНИЕ                                                                                               | Обязательно для заполнения |
| ------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- | -------- |
...
| --------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | -------- | ----------- |
| timeout="целое число"                       | Возникает время ожидания для заголовки HTTP-ответа, возвращаемые во внутреннюю службу, прежде чем выдать ошибку времени ожидания в секундах. Минимальное значение — 0 секунд. Значения больше 240 секунд может оказаться учитываться как базовой инфраструктуры сети, могут удалить неактивные соединения после этой даты. | Нет        | Нет |
...
| buffer-request-body="true &#124; false" | Когда, равным «true» запроса буферизируется, а также будет повторно использоваться на [повторите](api-management-advanced-policies.md#Retry). | Нет        | false       |
...
| Атрибут | ОПИСАНИЕ                                                                                        | Обязательно для заполнения | значение по умолчанию |
| --------- | -------------------------------------------------------------------------------------------------- | -------- | ------- |
| key       | Строка. Допустимое выражение. Задает область параллелизма. Используется несколькими политиками. | Yes      | Н/Д     |
| max-count | Целое число. Указывает максимальное количество запросов для ввода политики.           | Yes      | Н/Д     |
...
`retry` Политики выполняет свои дочерние политики один раз и затем повторяет их выполнение до попытки `condition` становится `false` или повторите `count` будет исчерпан.
...
| Атрибут        | ОПИСАНИЕ                                                                                                                                           | Обязательно для заполнения | значение по умолчанию |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------- |
| condition        | Логический литерал или [выражение](api-management-policy-expressions.md), указывающее, что нужно сделать: прекратить повторные попытки (`false`) или продолжить (`true`).      | Yes      | Н/Д     |
| count            | Положительное число, определяющее максимальное число повторных попыток.                                                                                | Yes      | Н/Д     |
| interval         | Положительное значение в секундах, определяющее интервал ожидания между повторными попытками.                                                                 | Yes      | Н/Д     |
...
| first-fast-retry | Если значение `true` , первая попытка повтора выполняется немедленно.                                                                                  | Нет        | `false` |
...
| ignore-error                    | Если получено значение true и запрос завершился ошибкой:<br /><br /> — Если response-variable-name указано, он будет содержать значение null.<br />— Если response-variable-name не указан, контекст. Запрос не будет обновляться.                                                                                                                                                                                                                                                                                                                                                                                   | Нет        | false    |
| name                            | Указывает имя заголовка, которое должно быть установлено.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Yes      | Н/Д      |
| exists-action                   | Указывает действие, которое должно быть выполнено, когда заголовок уже задан. Атрибут должен иметь одно из следующих значений:<br /><br /> override — заменяет значение имеющегося заголовка.<br />-skip — не заменяет существующее значение заголовка.<br />-append — добавляет значение к значению имеющегося заголовка.<br />delete — удаляет заголовок из запроса.<br /><br /> Если установлено значение `override`, перечисление нескольких записей с одним и тем же именем будет приводить к тому, что заголовок будет устанавливаться в соответствии со всеми записями (будут перечисляться несколько раз). В результате будут установлены только перечисленные значения. | Нет        | override |
...
| Атрибут         | ОПИСАНИЕ                                            | Обязательно для заполнения | значение по умолчанию |
| ----------------- | ------------------------------------------------------ | -------- | ------- |
| url="string"      | URL-адрес прокси-сервера в виде http://host:port.             | Yes      | Н/Д     |
...
| Атрибут       | ОПИСАНИЕ                                                | Обязательно для заполнения | значение по умолчанию |
| --------------- | ---------------------------------------------------------- | -------- | ------- |
| code="целое число"  | Код состояния HTTP для возврата.                            | Yes      | Н/Д     |
| reason="строка" | Описание причины для возврата кода состояния. | Yes      | Н/Д     |
...
`trace` Политики добавляет строку в [инспектора API](https://azure.microsoft.com/documentation/articles/api-management-howto-api-inspector/) выходных данных. Политика будет выполняться, только если инициирована трассировка, т. е. заголовок запроса `Ocp-Apim-Trace` присутствует и имеет значение `true`, а заголовок запроса `Ocp-Apim-Subscription-Key` присутствует и содержит допустимый ключ, связанный с учетной записью администратора.
...
| Атрибут | ОПИСАНИЕ                                                                             | Обязательно для заполнения | значение по умолчанию |
| --------- | --------------------------------------------------------------------------------------- | -------- | ------- |
| источник    | Строковый литерал, понятный средству просмотра трассировки и указывающий источник сообщения. | Yes      | Н/Д     |
