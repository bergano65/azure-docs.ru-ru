...
Политика выполняет дочерние политики один раз, а затем повторяет их выполнение до тех пор, пока `condition` `count` не будет исчерпана повторная попытка `false`. `retry`
