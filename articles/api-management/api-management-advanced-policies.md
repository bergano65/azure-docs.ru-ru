...
-   [Trace](#Trace) - Добавляет пользовательские следы в выход [API Inspector,](https://azure.microsoft.com/documentation/articles/api-management-howto-api-inspector/) телеметрии Application Insights и диагностические журналы.
-   [Подождите](#Wait) - Ожидается прилагаемый [запрос Отправить,](api-management-advanced-policies.md#SendRequest) [Получить значение из кэша,](api-management-caching-policies.md#GetFromCacheByKey)или [политики потока управления](api-management-advanced-policies.md#choose) для завершения перед началом.

## <a name="control-flow"></a><a name="choose"></a>Контрольный поток
...
#### <a name="example"></a><a name="ChooseExample"></a>Примере
...
### <a name="usage"></a><a name="ChooseUsage"></a>Использования
...
## <a name="forward-request"></a><a name="ForwardRequest"></a>Запрос на переумотани

Политика `forward-request` перенаправляет входящий запрос во внутреннюю службу, указанную в [контексте](api-management-policy-expressions.md#ContextVariables) запроса. URL-адрес службы бэкэнда указан в [настройках](https://azure.microsoft.com/documentation/articles/api-management-howto-create-apis/#configure-api-settings) API и может быть изменен с помощью политики [обслуживания заданного бэкэнда.](api-management-transformation-policies.md)
...
Следующая политика уровня API перенаправляет все запросы API на бэкэнд-сервис с интервалом тайм-аута 60 секунд.
...
Эта политика уровня операций явным образом перенаправляет все запросы во внутреннюю службу с временем ожидания 120 секунд и не наследует родительскую политику уровня API. Если служба бэкэнда отвечает кодом состояния ошибки от 400 до 599 включительно, будет срабатывать раздел [по ошибке.](api-management-error-handling-policies.md)
...
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------- |
| timeout="целое число"                             | Количество времени в секундах, чтобы ждать заголовки ответов HTTP, которые будут возвращены службой бэкэнда, прежде чем будет поднята ошибка тайм-аута. Минимальное значение составляет 0 секунд. Значения, превышающее 240 секунд, могут не соблюдаться, так как базовая сетевая инфраструктура может отказаться от простоесоединений после этого времени. | нет       | None    |
| последующие перенаправления "ложные &#124; правда"          | Указывает, что происходит с перенаправлениями от внутренней службы: шлюз выполняет их или они возвращаются вызывающему объекту.                                                                                                                                                                                                    | нет       | false   |
| буфер-запрос-тело "ложный &#124; правда"       | При наборе на "истинный" запрос буферизируется и будет повторно использоваться при [повторной попытке.](api-management-advanced-policies.md#Retry)                                                                                                                                                                                               | нет       | false   |
| сбой-на-ошибка-статус-код "ложный &#124; правда" | При наборе на истинные триггеры [по ошибке](api-management-error-handling-policies.md) раздел для ответов коды в диапазоне от 400 до 599 включительно.                                                                                                                                                                      | нет       | false   |
...
| Атрибут | Описание                                                                                        | Обязательно | Значение по умолчанию |
| --------- | -------------------------------------------------------------------------------------------------- | -------- | ------- |
| ключ       | Строка. Допустимое выражение. Задает область параллелизма. Используется несколькими политиками. | Да      | Недоступно     |
...
Любую строку можно использовать как значение для регистрации в Центрах событий. В этом примере дата и время, имя службы развертывания, идентификатор запроса, IP-адрес и `contoso-logger` имя операции для всех входящих вызовов регистрируются в концентратор событий Logger, зарегистрированный в ID
...
| Атрибут     | Описание                                                               | Обязательно                                                             |
| ------------- | ------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| logger-id     | Идентификатор Logger, зарегистрированный в службе управления API.         | Да                                                                  |
...
## <a name="retry"></a><a name="Retry"></a>Повторить

Политика `retry` выполняет свои политики в отношении детей один раз, `false` а `count` затем повторно выполняет их исполнение до тех пор, пока повторная попытка `condition` не станет или повторная попытка не будет исчерпана.
...
| first-fast-retry | При установке `true` на, первая попытка повтора выполняется немедленно.                                                                                  | нет       | `false` |
...
Эта политика может использоваться в следующих [разделах](https://azure.microsoft.com/documentation/articles/api-management-howto-policies/#sections) и [областях](https://azure.microsoft.com/documentation/articles/api-management-howto-policies/#scopes). Обратите внимание, что эта политика наследует ограничения использования дочерних политик.
...
| exists-action | Указывает действие, которое должно быть выполнено, когда заголовок уже задан. Атрибут должен иметь одно из следующих значений:<br /><br /> - переопределение - заменяет значение существующего заголовка.<br />- пропустить - не заменяет существующее значение заголовка.<br />- приложение - прикладывает значение к существующему значению заголовка.<br />- удалить - удаляет заголовок из запроса.<br /><br /> Если установлено значение `override`, перечисление нескольких записей с одним и тем же именем будет приводить к тому, что заголовок будет устанавливаться в соответствии со всеми записями (будут перечисляться несколько раз). В результате будут установлены только перечисленные значения. | нет       | override |
...
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------- |
| mode="строка"                   | Определяет, является ли это новым запросом или копией текущего запроса. В исходящем режиме mode=copy не инициализирует текст запроса.                                                                                                                                                                                                                                                                                                                                                                                                                                                                | нет       | Создать      |
| response-variable-name="строка" | Имя переменной контекста, которая примет объект response. Если переменная не существует, она будет создана после успешного выполнения [`context.Variable`](api-management-policy-expressions.md#ContextVariables) политики и станет доступной через коллекцию.                                                                                                                                                                                                                                                                                                                          | Да      | Недоступно      |
...
| ignore-error                    | Если получено значение true и запрос завершился ошибкой:<br /><br /> - Если было указано имя ответа-переменная, оно будет содержать нулевую стоимость.<br />- Если ответ-переменный-имя не было указано, контекст. Запрос не будет обновляться.                                                                                                                                                                                                                                                                                                                                                                                   | нет       | false    |
| name                            | Указывает имя заголовка, которое должно быть установлено.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Да      | Недоступно      |
| exists-action                   | Указывает действие, которое должно быть выполнено, когда заголовок уже задан. Атрибут должен иметь одно из следующих значений:<br /><br /> - переопределение - заменяет значение существующего заголовка.<br />- пропустить - не заменяет существующее значение заголовка.<br />- приложение - прикладывает значение к существующему значению заголовка.<br />- удалить - удаляет заголовок из запроса.<br /><br /> Если установлено значение `override`, перечисление нескольких записей с одним и тем же именем будет приводить к тому, что заголовок будет устанавливаться в соответствии со всеми записями (будут перечисляться несколько раз). В результате будут установлены только перечисленные значения. | нет       | override |
...
## <a name="set-variable"></a><a name="set-variable"></a>Набор переменных
...
### <a name="example"></a><a name="set-variableExample"></a>Примере
...
Политика `trace` добавляет пользовательский след в вывод aPI Inspector, телеметрии Application Insights и/или диагностические журналы.

-   Политика добавляет пользовательский след к выходу [API Inspector](https://azure.microsoft.com/documentation/articles/api-management-howto-api-inspector/) при срабатывании трассировки, т.е. `Ocp-Apim-Trace` заголовок запроса присутствует и устанавливается на истину, и `Ocp-Apim-Subscription-Key` заголовок запроса присутствует и содержит действительный ключ, позволяющий отслеживать.
-   Политика создает телеметрию [Trace](https://docs.microsoft.com/azure/azure-monitor/app/data-model-trace-telemetry) в Application Insights, когда [включена интеграция Application Insights](https://docs.microsoft.com/azure/api-management/api-management-howto-app-insights) и `severity` уровень, указанный в политике, находится на уровне или выше `verbosity` уровня, указанного в диагностической настройке.
-   Политика добавляет свойство в ввод журнала, когда [диагностические журналы включены](https://docs.microsoft.com/azure/api-management/api-management-howto-use-azure-monitor#diagnostic-logs) и уровень серьезности, указанный в политике, находится на уровне или выше уровня многословия, указанного в диагностической настройке.
...
### <a name="example"></a><a name="traceExample"></a>Примере
...
| message  | Строка или выражение для регистрации.                                                                                                                 | Да      |
| метаданные | Добавляет пользовательское свойство в телеметрию Application Insights [Trace.](https://docs.microsoft.com/azure/azure-monitor/app/data-model-trace-telemetry) | нет       |

### <a name="attributes"></a>Атрибуты

| Атрибут | Описание                                                                                                               | Обязательно | Значение по умолчанию |
| --------- | ------------------------------------------------------------------------------------------------------------------------- | -------- | ------- |
| source    | Строковый литерал, понятный средству просмотра трассировки и указывающий источник сообщения.                                   | Да      | Недоступно     |
| severity  | Определяет уровень тяжести следа. Разрешенные значения `verbose` `information`, `error` (от самых низких до самых высоких). | нет       | Подробный |
| name      | Имя свойства.                                                                                                     | Да      | Недоступно     |
| значение     | Значение свойства.                                                                                                    | Да      | Недоступно     |
