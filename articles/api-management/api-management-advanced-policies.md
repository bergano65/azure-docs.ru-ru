...
Любую строку можно использовать как значение для регистрации в Центрах событий. В этом примере Дата и время, имя службы развертывания, идентификатор запроса, IP-адрес и имя операции для всех входящих вызовов записываются в журнал концентратора событий, зарегистрированный с помощью идентификатора `contoso-logger`.
...
| partition-id  | Указывает индекс раздела, куда должны отправляться сообщения.             | Необязательно. Этот атрибут не может использоваться, если используется `partition-key`. |
| partition-key | Указывает значение, используемое для назначения секции при отправке сообщений. | Необязательно. Этот атрибут не может использоваться, если используется `partition-id`.  |
...
`retry` политика выполняет свои дочерние политики один раз, а затем повторяет их выполнение до тех пор, пока `condition` не `false` или не будет исчерпана повторная попытка `count`.
...
| first-fast-retry | Если задано значение `true`, то первая повторная попытка выполняется немедленно.                                                                                  | Нет       | `false` |
...
| Атрибут              | Описание                                                                                                                                                                          | Обязательно  |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- |
| response-variable-name | Имя переменной контекста, на которую ссылается, например, вышестоящая политика [send-request](api-management-advanced-policies.md#SendRequest) и которая содержит объект `Response`. | Необязательно. |
...
Политика `trace` добавляет пользовательскую трассировку в выходные данные инспектора API, Application Insights телеметрии и (или) журналы диагностики.

-   Политика добавляет пользовательскую трассировку в выходные данные [инспектора API](https://azure.microsoft.com/documentation/articles/api-management-howto-api-inspector/) при срабатывании трассировки, т. е. `Ocp-Apim-Trace` заголовок запроса имеется и имеет значение true, а `Ocp-Apim-Subscription-Key` заголовок запроса имеется и содержит допустимый ключ, позволяющий выполнять трассировку.
-   Политика создает данные телеметрии [трассировки](https://docs.microsoft.com/azure/azure-monitor/app/data-model-trace-telemetry) в Application Insights, если [Application Insights интеграция](https://docs.microsoft.com/azure/api-management/api-management-howto-app-insights) включена, а уровень `severity`, указанный в политике, не превышает уровень `verbosity`, указанный в параметре диагностики.
-   Политика добавляет свойство в запись журнала при включении [журналов диагностики](https://docs.microsoft.com/azure/api-management/api-management-howto-use-azure-monitor#diagnostic-logs) и уровень серьезности, указанный в политике, превышает уровень детализации, указанный в параметре диагностики, или выше.
...
| серьезность  | Задает степень серьезности трассировки. Допустимые значения: `verbose`, `information`, `error` (от наименьшего к наибольшему). | Нет       | Подробный |
