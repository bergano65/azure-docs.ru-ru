...
#### <a name="example"></a><a name="ChooseExample"></a>Например
...
Любую строку можно использовать как значение для регистрации в Центрах событий. В этом примере в журнал концентратора событий, зарегистрированный с `contoso-logger` идентификатором, заносятся Дата и время, имя службы развертывания, идентификатор запроса, IP-адрес и имя операции для всех входящих вызовов.
...
| content-type | Указывает значение заголовка ответа `Content-Type` и позволяет выбрать соответствующий пример или схему. | Нет       | Отсутствуют    |
...
Политика выполняет дочерние политики один раз, а затем повторяет их выполнение до тех пор, пока `count` не `false` будет исчерпана повторная попытка `condition` `retry`
...
| first-fast-retry | Если задано `true` значение, первая повторная попытка выполняется немедленно.                                                                                  | Нет       | `false` |
...
### <a name="example"></a><a name="set-variableExample"></a>Например
...
### <a name="example"></a><a name="traceExample"></a>Например
