...
| buffer-Response = "false &#124; true" | Влияет на обработку фрагментированных откликов. Если задано значение "false", каждый блок, полученный от серверной части, немедленно возвращается вызывающему. Если задано значение "true", блоки помещаются в буфер (8 КБ, если не обнаружен конец потока) и только затем возвращается вызывающему объекту. | нет | Да |
...
| значение     | Значение свойства.                                                                                                    | Да      | Н/Д     |
