...
## <a name="open-api"> </a>Ограничения импорта OpenAPI/Swagger
...
-   единственными поддерживаемыми расширениями являются `x-ms-paths` и `x-servers`.
...
-   ключевое слово `Produces`, которое описывает типы MIME, возвращаемые API, не поддерживается. 

### <a name="open-api-v2"> </a>OpenAPI версии 2

-   Поддерживается только формат JSON.

### <a name="open-api-v3"> </a>OpenAPI версии 3
...
Для каждой операции, найденной в документе OpenAPI, будет создана новая операция с именем ресурса Azure, а отображаемое имя имеет значение `operationId` и `summary` соответственно. `operationId` значение нормализовано согласно правилам, описанным ниже. `summary` значение импортируется как есть и его длина ограничена 300 символами.

Если `operationId` не указано (т. е. отсутствует, `null`или пусто), значение имени ресурса Azure будет создано путем объединения метода HTTP и шаблона пути, например `get-foo`.

Если `summary` не указан (т. е. отсутствует, `null`или пуста), `display name` значение будет установлено в `operationId`. Если параметр `operationId` не указан, значение отображаемого имени будет создано путем объединения метода HTTP и шаблона пути, например `Get - /foo`.

### <a name="update-an-existing-api-via-openapi-import"></a>Обновление существующего API с помощью импорта OpenAPI

Во время импорта существующий API изменяется в соответствии с API, описанным в документе OpenAPI. Каждая операция в документе OpenAPI сопоставляется с существующей операцией, сравнивая значение `operationId` с именем ресурса Azure существующей операции.
...
- Обязательно укажите свойство `operationId` для каждой операции.
- Не следует менять `operationId` после первоначального импорта.
...
- Замените каждую последовательность символов, отличных от буквенно-цифровых, одним тире, например `GET-/foo/{bar}?buzz={quix}` будет преобразован в `get-foo-bar-buzz-quix-`.
- Обрезать дефисы на обеих сторонах, например `get-foo-bar-buzz-quix-` станет `get-foo-bar-buzz-quix`
