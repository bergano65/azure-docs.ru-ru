...
|Название|Описание|Обязательное значение|Значение по умолчанию|
|----------|-----------------|--------------|-------------|
|from|Строка, которую нужно найти.|Да|Н/Д|
...
|Название|Описание|Обязательное значение|Значение по умолчанию|
|----------|-----------------|--------------|-------------|
|base-url|Новый базовый URL-адрес внутренней службы.|Должен присутствовать `backend-id` один из `base-url` или.|Н/Д|
|backend-id|Идентификатор серверной части для перенаправления. (Внутренние сущности управляются через [API](https://docs.microsoft.com/rest/api/apimanagement/2019-01-01/backend) и [PowerShell](https://www.powershellgallery.com/packages?q=apimanagement).)|Должен присутствовать `backend-id` один из `base-url` или.|Н/Д|
