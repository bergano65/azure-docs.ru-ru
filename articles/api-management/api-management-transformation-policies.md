...
##  <a name="transformation-policies"></a><a name="TransformationPolicies"></a>Политика трансформации
...
> [!NOTE]
> Управление бэкэндом можно с помощью [API](https://docs.microsoft.com/rest/api/apimanagement/2019-12-01/backend) управления и [PowerShell.](https://www.powershellgallery.com/packages?q=apimanagement)
...
|Имя|Описание|Обязательно|Значение по умолчанию|
|----------|-----------------|--------------|-------------|
|base-url|Новый базовый URL-адрес внутренней службы.|Один `base-url` из `backend-id` или должны присутствовать.|Н/Д|
|backend-id|Идентификатор серверной части для перенаправления. (Сущности backend управляются через [API](https://docs.microsoft.com/rest/api/apimanagement/2019-12-01/backend) и [PowerShell.)](https://www.powershellgallery.com/packages?q=apimanagement)|Один `base-url` из `backend-id` или должны присутствовать.|Н/Д|
...
#### <a name="transform-json-using-a-liquid-template"></a>Преобразование JSON с помощью шаблона Liquid
```xml
{
"order": {
...
#### <a name="example---adding-header-override-existing"></a>Пример - добавление заголовка, переопределение существующих
...
```
#### <a name="example---removing-header"></a>Пример - удаление заголовка
...
> [!NOTE]
> Несколько значений заголовка соплут на строку CSV, например:`headerName: value1,value2,value3`
>
...
>
> В случае этих исключений несколько значений заголовка не будут скатированы в одну строку и будут переданы в виде отдельных заголовков, например:`User-Agent: value1`
>`User-Agent: value2`
>`User-Agent: value3`
