...
> [!NOTE]
> Управление бэкэндом можно с помощью [API](https://docs.microsoft.com/rest/api/apimanagement/2019-01-01/backend) управления и [PowerShell.](https://www.powershellgallery.com/packages?q=apimanagement)
...
|name|Описание|Обязательно|Значение по умолчанию|
|----------|-----------------|--------------|-------------|
|base-url|Новый базовый URL-адрес внутренней службы.|Один `base-url` из `backend-id` или должны присутствовать.|Недоступно|
|backend-id|Идентификатор серверной части для перенаправления. (Сущности backend управляются через [API](https://docs.microsoft.com/rest/api/apimanagement/2019-01-01/backend) и [PowerShell.)](https://www.powershellgallery.com/packages?q=apimanagement)|Один `base-url` из `backend-id` или должны присутствовать.|Недоступно|
