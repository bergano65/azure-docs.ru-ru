...
|Имя|Описание|Обязательно|По умолчанию|
|----------|-----------------|--------------|-------------|
|from|Искомая строка.|Да|Н/Д|
|до|Строка для замены. Укажите строку замены с нулевой длиной для удаления строки поиска.|Да|Н/Д|
...
> [!NOTE]
> Внутренним объектам можно управлять с помощью [API](https://docs.microsoft.com/rest/api/apimanagement/2019-01-01/backend) управления и [PowerShell](https://www.powershellgallery.com/packages?q=apimanagement).
...
|Имя|Описание|Обязательно|По умолчанию|
|----------|-----------------|--------------|-------------|
|base-url|Новый базовый URL-адрес внутренней службы.|Должен присутствовать один из `base-url` или `backend-id`.|Н/Д|
|backend-id|Идентификатор серверной части для перенаправления. (Внутренние сущности управляются через [API](https://docs.microsoft.com/rest/api/apimanagement/2019-01-01/backend) и [PowerShell](https://www.powershellgallery.com/packages?q=apimanagement).)|Должен присутствовать один из `base-url` или `backend-id`.|Н/Д|
...
|Имя|Описание|Обязательно|По умолчанию|
|----------|-----------------|--------------|-------------|
|template (шаблон)|Используется для изменения режима шаблона, в котором будет выполняться политика set-body. В настоящее время поддерживается только одно значение:<br /><br />- liquid — политика set-body будет использовать подсистему шаблонов Liquid. |Нет||
...
|---------------|-----------------|--------------|-------------|
|template (шаблон)|Фактический URL-адрес веб-службы с любыми параметрами строки запроса. При использовании выражений все значение должно быть выражением.|Да|Н/Д|
