...
|----------|-----------------|--------------|-------------|
|apply|Для атрибута нужно задать одно из следующих значений:<br /><br /> always — всегда применять преобразование;<br />content-type-json — преобразовать только в том случае, если в заголовке ответа Content-Type указано наличие формата JSON.|Yes|Н/Д|
...
|ИМЯ|ОПИСАНИЕ|Обязательно для заполнения|значение по умолчанию|
|----------|-----------------|--------------|-------------|
|kind|Для атрибута нужно задать одно из следующих значений:<br /><br /> javascript-friendly — преобразованный JSON имеет форму, понятную разработчикам JavaScript;<br />direct — преобразованный JSON отражает структуру исходного XML-документа.|Yes|Н/Д|
|apply|Для атрибута нужно задать одно из следующих значений:<br /><br /> always — всегда применять преобразование;<br />content-type-xml — преобразовать только в том случае, если в заголовке ответа Content-Type указано наличие формата XML.|Yes|Н/Д|
...
|ИМЯ|ОПИСАНИЕ|Обязательно для заполнения|значение по умолчанию|
|----------|-----------------|--------------|-------------|
|from|Строка, которую нужно найти.|Yes|Н/Д|
|значение|Строка замены. Укажите строку замены с нулевой длиной для удаления строки поиска.|Yes|Н/Д|
...
> [!NOTE]
> Сущности серверной части может осуществляться с помощью управления [API](https://docs.microsoft.com/en-us/rest/api/apimanagement/backend) и [PowerShell](https://www.powershellgallery.com/packages?q=apimanagement).
...
|ИМЯ|ОПИСАНИЕ|Обязательно для заполнения|значение по умолчанию|
|----------|-----------------|--------------|-------------|
|base-url|Новый базовый URL-адрес внутренней службы.|Один из `base-url` или `backend-id` должен присутствовать.|Н/Д|
|backend-id|Идентификатор серверной части для перенаправления. (Управляемые сущности серверной части через [API](https://docs.microsoft.com/en-us/rest/api/apimanagement/backend) и [PowerShell](https://www.powershellgallery.com/packages?q=apimanagement).)|Один из `base-url` или `backend-id` должен присутствовать.|Н/Д|
...
> [!NOTE]
> Несколько значений заголовка объединяются в строку CSV-ФАЙЛ, например: `headerName: value1,value2,value3`
>
...
>
> В случае те исключения несколько значений заголовка не будут объединены в одну строку и будет передаваться как отдельные заголовки, например: `User-Agent: value1`
>`User-Agent: value2`
>`User-Agent: value3`
...
|name|Указывает имя заголовка, которое должно быть установлено.|Yes|Н/Д|
...
|name|Указывает имя параметра запроса, которое должно быть установлено.|Yes|Н/Д|
...
|Атрибут|ОПИСАНИЕ|Обязательно для заполнения|значение по умолчанию|
|---------------|-----------------|--------------|-------------|
|шаблон|Фактический URL-адрес веб-службы с любыми параметрами строки запроса. При использовании выражений все значение должно быть выражением.|Yes|Н/Д|
