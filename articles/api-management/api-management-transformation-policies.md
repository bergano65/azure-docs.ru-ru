...
> [!NOTE]
> Сущности серверной части может осуществляться с помощью управления [API](https://docs.microsoft.com/rest/api/apimanagement/backend) и [PowerShell](https://www.powershellgallery.com/packages?q=apimanagement).
...
|backend-id|Идентификатор серверной части для перенаправления. (Управляемые сущности серверной части через [API](https://docs.microsoft.com/rest/api/apimanagement/backend) и [PowerShell](https://www.powershellgallery.com/packages?q=apimanagement).)|Один из `base-url` или `backend-id` должен присутствовать.|Н/Д|
