...
|Имя|Описание|Обязательный|По умолчанию|
|----------|-----------------|--------------|-------------|
|из|Строка, которую нужно найти.|Да|Недоступно|
|значение|Строка замены. Укажите строку замены с нулевой длиной для удаления строки поиска.|Да|Недоступно|
...
|Имя|Описание|Обязательный|По умолчанию|
|----------|-----------------|--------------|-------------|
|base-url|Новый базовый URL-адрес внутренней службы.|Должен присутствовать один из `base-url` или `backend-id` .|Недоступно|
|backend-id|Идентификатор серверной части для перенаправления. (Внутренние сущности управляются через [API](https://docs.microsoft.com/rest/api/apimanagement/2019-12-01/backend) и [PowerShell](https://www.powershellgallery.com/packages?q=apimanagement).)|Должен присутствовать один из `base-url` или `backend-id` .|Недоступно|
...
|name|Указывает имя параметра запроса, которое должно быть установлено.|Да|Недоступно|
