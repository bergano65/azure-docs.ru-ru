...
|значение|Строка для замены. Укажите строку замены с нулевой длиной для удаления строки поиска.|Да|Недоступно|
...
|backend-id|Идентификатор серверной части для перенаправления. (Внутренние сущности управляются через [API](https://docs.microsoft.com/rest/api/apimanagement/2019-01-01/backend) и [PowerShell](https://www.powershellgallery.com/packages?q=apimanagement).)|Должен присутствовать один из `base-url` или `backend-id`.|Недоступно|
...
|attribute|Description|Обязательно|По умолчанию|
|---------------|-----------------|--------------|-------------|
|template (шаблон)|Фактический URL-адрес веб-службы с любыми параметрами строки запроса. При использовании выражений все значение должно быть выражением.|Да|Недоступно|
