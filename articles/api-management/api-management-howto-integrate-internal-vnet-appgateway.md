...
В этой статье рассказывается о том, как использовать единый сервис управления API как для внутренних, так и для внешних потребителей и сделать ее единым фронтпередом как для в помещениях, так и для облачных API. Также вы узнаете, как предоставить некоторое подмножество этих API-интерфейсов (в нашем примере они выделены зеленым цветом) для внешнего использования, применив функцию маршрутизации, предоставляемую Шлюзом приложений.

В первом примере конфигурации все API-интерфейсы управляются только из виртуальной сети. Внутренние потребители (выделены оранжевым цветом) могут обращаться ко всем внутренним и внешним API. Трафик никогда не выходит в Интернет. Высокопроизводительная связь осуществляется по схемам Express Route.
...
* Убедитесь, что у вас установлена последняя версия Azure PowerShell. Смотрите инструкции по установке на [install Azure PowerShell.](/powershell/azure/install-az-ps) 
...
* **Пул серверов бэк-энда:** Это внутренний виртуальный IP-адрес службы управления API.
* **Настройки пула серверов бэк-энда:** Каждый пул имеет такие настройки, как порт, протокол и сродство на основе файлов cookie. Эти параметры применяются ко всем серверам в этом пуле.
...
* **Слушатель:** Слушатель имеет передний порт, протокол (http или Https, эти значения чувствительны к случаям) и имя сертификата TLS/SSL (при настройке разгрузки TLS).
...
> [!WARNING]
> Чтобы предотвратить нарушение спецификации OpenAPI на портале разработчиков, необходимо отключить правило `942200 - "Detects MySQL comment-/space-obfuscated injections and backtick termination"`брандмауэра.
...
Ниже приводится следующий пример, как создать виртуальную сеть с помощью менеджера ресурсов.
...
> [!IMPORTANT]
> [Новый портал разработчиков](api-management-howto-developer-portal.md) также требует подключения к конечной точке управления API Management в дополнение к приведенным ниже шагам.

### <a name="step-1"></a>Шаг 1

Инициализировать следующие переменные с деталями сертификатов с закрытыми ключами для доменов. В этом примере используется `api.contoso.net` и `portal.contoso.net`.  
...
Создайте и установите объекты конфигурации хоста для прокси и для портала.  
...
> [!NOTE]
> Для настройки устаревшего подключения `-HostnameType DeveloperPortal` портала разработчика необходимо заменить `-HostnameType Portal`на .
...
Создайте конфигурацию IP-адреса шлюза приложения под названием **gatewayIP01.** При запуске шлюз приложений получает IP-адрес из настроенной подсети. Затем шлюз маршрутизирует сетевой трафик на IP-адреса из пула внутренних IP-адресов. Помните, что для каждого экземпляра требуется отдельный IP-адрес.
...
Создайте прослушиватели HTTP для Шлюза приложений. Назначайте им сертификаты IP-адреса, порт и TLS/SSL.
...
Загрузите сертификат, который будет использоваться на ресурсах бэкэнда с поддержкой TLS. Это тот же сертификат, который вы указали выше на шаге 4.
...
## <a name="summary"></a><a name="summary"> </a> Краткое описание
Управление API Azure, настроенное в VNET, обеспечивает единый интерфейс шлюза для всех настроенных API, независимо от того, размещаются ли они в помещениях или в облаке. Интеграция шлюза приложений с управлением API дает вам дополнительную гибкость, позволяя избирательно предоставлять доступ к API-интерфейсам через Интернет. Также вы можете использовать брандмауэр веб-приложения в качестве интерфейсного компонента для экземпляра службы управления API.

## <a name="next-steps"></a><a name="next-steps"> </a> Следующие шаги
* Дополнительные сведения о шлюзе приложений Azure:
  * [Обзор шлюзов приложений](../application-gateway/application-gateway-introduction.md)
