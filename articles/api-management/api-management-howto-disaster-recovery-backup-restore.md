...
Чтобы обеспечить восстановление в случае проблем с доступностью в регионе, в котором размещена ваша служба управления API, будьте готовы в любой момент создать ее в другом регионе. В зависимости от цель времени восстановления может потребоваться сохранить резервной службы в одном или нескольких регионах. Также можно попробовать для поддержания ее конфигурации и содержимого в соответствии с активной службой в соответствии с вашей целевой точки восстановления. Служба резервного копирования и возможностей по восстановлению предоставляет необходимые строительные блоки для реализации стратегии аварийного восстановления.

Операции резервного копирования и восстановления могут также использоваться для репликации конфигурации службы управления API между рабочих средах, например для разработки и промежуточной. Имейте в виду эти данные среды выполнения, например пользователей и подписки будут скопированы, который может всегда быть нежелательно.

В этом руководстве показано, как для автоматизации резервного копирования и восстановления и убедитесь в успешной проверке подлинности резервного копирования и восстановления запросы с помощью Azure Resource Manager.

> [!IMPORTANT]
> Восстановление операция не меняет настройки пользовательского имени узла целевой службы. Рекомендуется использовать же пользовательского имени узла и сертификат TLS активными и резервными служб, так что после завершения операции восстановления, трафик можно, назначаемые резервный экземпляр, простое изменение DNS-запись CNAME.
>
> Операции резервного копирования не захватывает предварительно вычисленные данные журнала, используемых в отчетах, в колонке аналитики на портале Azure.
...
> [!NOTE]
> Операции резервного копирования и восстановления могут также выполняться с помощью PowerShell _резервного копирования AzApiManagement_ и _AzApiManagement восстановления_ команд соответственно.
