...
Приложение PHP, которое использует службу Twilio и выполняется в Azure, не отличается от других приложений PHP, которые используют службу Twilio. Хотя службы Twilio Services основаны на интерфейсе RESTFUL и могут вызываться из PHP несколькими способами, в этой статье основное внимание уделяется использованию служб Twilio с [библиотекой Twilio для PHP из GitHub][twilio_php]. Дополнительные сведения об использовании библиотеки Twilio для PHP см. в разделе [https://www.twilio.com/docs/libraries/php][twilio_lib_docs].

Подробные инструкции по созданию и развертыванию приложения Twilio/PHP в Azure доступны на странице [как позвонить с помощью Twilio в приложении PHP в Azure][howto_phonecall_php].
...
1. Скачайте библиотеку Twilio для PHP из GitHub ([https://github.com/twilio/twilio-php][twilio_php]) и добавьте в приложение каталог **служб** .
...
Дополнительные сведения можно найти по адресу [https://github.com/twilio/twilio-php/blob/master/README.md][twilio_github_readme].
...
* **Примечание**. Сведения об устранении ошибок при проверке SSL-сертификата см. в разделе[http://readthedocs.org/docs/twilio-php/en/latest/usage/rest.html][ssl_validation] 
...
Когда приложение инициирует вызов API Twilio, Twilio отправляет ваш запрос на URL-адрес, который должен возвратить ответ TwiML. В приведенном выше примере используется URL-адрес [https://twimlets.com/message][twimlet_message_url], предоставленный Twilio. (Хотя TwiML предназначается для использования службой Twilio, его можно также просмотреть в браузере. Например [https://twimlets.com/message][twimlet_message_url] , щелкните, чтобы просмотреть пустой [https://twimlets.com/message?Message%5B0%5D=Hello%20World][twimlet_message_url_hello_world] `<Response>` элемент; в качестве другого примера щелкните `<Response>` , чтобы просмотреть элемент, содержащий `<Say>` элемент.)
...
Дополнительные сведения о TwiML см. по адресу [https://www.twilio.com/docs/api/twiml][twiml_reference]. 
...
Дополнительные сведения об использовании Twilio в Azure с PHP см. в статье [Создание телефонного звонка с помощью Twilio в приложении PHP в Azure][howto_phonecall_php].
