...
## <a name="create-a-php-application"></a><a id="create_app"></a>Создание приложения PHP
Приложение PHP, которое использует службу Twilio и выполняется в Azure, не отличается от других приложений PHP, которые используют службу Twilio. Хотя службы Twilio поддерживают интерфейс REST и могут вызываться из PHP несколькими способами, в этой статье основное внимание уделяется использованию служб Twilio с [библиотекой Twilio для PHP из GitHub][twilio_php]. Для получения дополнительной информации об использовании библиотеки Twilio для PHP см. [https://www.twilio.com/docs/libraries/php][twilio_lib_docs]
...
1. Загрузите библиотеку Twilio для PHP от GitHub ()[https://github.com/twilio/twilio-php][twilio_php]и добавьте каталог **услуг** в приложение.
...
Для получения дополнительной [https://github.com/twilio/twilio-php/blob/master/README.md][twilio_github_readme]информации см.
...
* **Примечание.** Чтобы устранить ошибки проверки сертификата SSL, перейдите по адресу [http://readthedocs.org/docs/twilio-php/en/latest/usage/rest.html][ssl_validation]. 
...
Когда приложение инициирует вызов API Twilio, Twilio отправляет ваш запрос на URL-адрес, который должен возвратить ответ TwiML. В приведенном выше примере используется [https://twimlets.com/message][twimlet_message_url]URL-адрес, предоставленный Twilio. (Хотя TwiML предназначается для использования службой Twilio, его можно также просмотреть в браузере. Например, [https://twimlets.com/message][twimlet_message_url] нажмите кнопку, чтобы увидеть пустой `<Response>` элемент; в качестве еще [https://twimlets.com/message?Message%5B0%5D=Hello%20World][twimlet_message_url_hello_world] одного `<Response>` примера нажмите кнопку, чтобы увидеть элемент, содержащий `<Say>` элемент.)
...
Для получения дополнительной информации [https://www.twilio.com/docs/api/twiml][twiml_reference]о TwiML см. 
