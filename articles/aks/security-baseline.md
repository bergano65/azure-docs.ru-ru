...
Дополнительные сведения см. в статье [Обзор базовой конфигурации безопасности Azure](/azure/security/benchmarks/security-baselines-overview-v1).
...
Используйте рабочую область Log Analytics Azure Monitor, чтобы просматривать журналы и выполнять запросы к данным журнала. Журналы Azure Monitor включены и управляются в портал Azure или с помощью интерфейса командной строки, а также работают как кластеры AKS с доступом на основе ролей (Azure RBAC) и не поддерживают RBAC.
...
**Руководство**. Интеграция проверки подлинности пользователей для кластеров службы Azure Kubernetes Service (AKS) с Azure Active Directory (Azure AD). Войдите в кластер AKS с помощью маркера проверки подлинности Azure AD. Настройте управление доступом на основе ролей Kubernetes (RBAC) для административного доступа к сведениям о конфигурации Kubernetes (kubeconfig) и разрешениям, пространствам имен и ресурсам кластера. 
...
Управление доступом на основе (RBAC) следует использовать в службах Kubernetes Services
...
Обратитесь к публикации NIST: руководство по тестированию, обучению и применению программ для ИТ-специалистов 
- [Планы и возможности](https://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-84.pdf)
