---
title: Динамически создать громкость дисков Azure
titleSuffix: Azure Kubernetes Service
description: Узнайте, как динамически создавать постоянный объем с помощью дисков Azure в службе Azure Kubernetes (AKS)
...
* Класс хранения *default* подготавливает диск Azure ценовой категории "Стандартный".
    * Стандартное хранилище поддерживается HDD и обеспечивает экономичное хранилище, в то же время выполняя. Стандартные диски идеально подходят для экономичности разработчика и тестовой рабочей нагрузки.
...
Эти классы хранения по умолчанию не позволяют обновлять размер громкости после создания. Чтобы включить эту возможность, добавьте *allowVolumeExpansion: истинную* строку в один из классов хранения данных по умолчанию или создайте собственный пользовательский класс хранения. С помощью `kubectl edit sc` команды можно отсвазать существующий класс хранения. Для получения дополнительной информации о классах хранения и создании собственных, [см.][storage-class-concepts]
...
> [!NOTE]
> Утверждения постоянных томов указаны в ГиБ, но счета за использование управляемых дисков Azure выставляются по номеру SKU для определенного размера. Эти SKUs варьируются от 32GiB для дисков S4 или P4 до 32TiB для дисков S80 или P80 (в предварительном просмотре). Число операций ввода-вывода в секунду и пропускная способность управляемого диска уровня "Премиум" зависят как от номера SKU, так и от размера экземпляров узлов в кластере AKS. Дополнительные сведения см. на странице [Цены на управляемые диски][managed-disk-pricing-performance].
...
После создания утверждения постоянного тома и успешной подготовки диска можно создать группу pod с доступом к диску. Приведенный ниже манифест создает базовый модуль pod NGINX, который использует утверждение постоянного тома *azure-managed-disk* для подключения диска Azure по пути `/mnt/azure`. Для контейнеров Windows Server (в настоящее время в предварительном просмотре в AKS) укажите *mountPath* с помощью конвенции пути Windows, например *'D:'.*
...
> [Плагин Kubernetes для дисков Azure][azure-disk-volume]
