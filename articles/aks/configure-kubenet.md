...
> [!NOTE]
> Кубераутер позволяет включить сетевую политику при использовании кубенет и может быть установлена в качестве управляющей программы в кластере AKS. Имейте в виду, что KUBE-маршрутизатор по-прежнему находится в бета-версии, и корпорация Майкрософт не предлагает поддержку для этого проекта.
...
Теперь назначьте субъекту-службе для вашего кластера AKS разрешения *участника* в виртуальной сети с помощью команды [AZ Role назначение Create][az-role-assignment-create] . Укажите собственные  *\<AppID >* , как показано в выходных данных предыдущей команды для создания субъекта-службы.
...
Вы создали виртуальную сеть и подсеть, а также создали и назначили разрешения для субъекта-службы, чтобы использовать эти сетевые ресурсы. Теперь создайте кластер AKS в виртуальной сети и подсети с помощью команды [AZ AKS Create][az-aks-create] . Определите собственные  *\<идентификаторы* субъекта-службы > и  *\<пароль >* , как показано в выходных данных предыдущей команды для создания субъекта-службы.
