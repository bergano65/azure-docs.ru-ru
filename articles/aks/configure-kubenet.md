...
По умолчанию AKS кластеров под управлением Windows используйте [kubenet][kubenet], и виртуальной сеть и подсеть Azure создаются автоматически. При использовании *kubenet* узлы получают IP-адрес из подсети виртуальной сети Azure. Объекты pod получают IP-адрес из логически отличного адресного пространства в подсеть узлов виртуальной сети Azure. Затем настраивается преобразование сетевых адресов (NAT) таким образом, чтобы объекты pod могли получить доступ к ресурсам в виртуальной сети Azure. Исходный IP-адрес трафика преобразуется в первичный IP-адрес узла. Такой подход значительно уменьшает количество IP-адресов, которые необходимо зарезервировать в пространстве сети для объектов pod.

С помощью [интерфейс сетевые подключения контейнеров Azure (CNI)][cni-networking], каждая группа контейнеров pod получает IP-адрес из подсети и может осуществляться непосредственно. Эти IP-адреса должны быть уникальными во всей сети, и их следует планировать заранее. Для каждого узла предусмотрен параметр конфигурации, в котором указывается максимальное число объектов pod, которые он поддерживает. Затем на каждом узле заранее резервируется эквивалентное число IP-адресов для этого узла. Этот подход требует дополнительного планирования и часто приводит к исчерпанию IP-адресов или необходимости повторно создавать кластеры в подсети большего размера по мере увеличения потребностей вашего приложения.

В этой статье показано, как с помощью сети *kubenet* создать и использовать подсеть виртуальной сети для кластера AKS. Дополнительные сведения о сети вариантов и соображений, см. в разделе [сети основные понятия Kubernetes и AKS][aks-network-concepts].

> [!WARNING]
> Чтобы использовать пулы узлов Windows Server (в настоящее время в предварительной версии в AKS), необходимо использовать Azure CNI. Использование kubenet в качестве модели сети не доступен для контейнеров Windows Server.
...
Для обеспечения подключения к локальным ресурсам, оба *kubenet* и *Azure CNI* сети подходов можно использовать [пиринга виртуальных сетей Azure][vnet-peering] or [ExpressRoute connections][express-route]. Следует тщательно планировать диапазоны IP-адресов, чтобы избежать перекрытия и неправильной маршрутизации трафика. Например, во многих локальных сетях используется диапазон *10.0.0.0/8*, который объявляется через подключение ExpressRoute. Рекомендуется для создания кластеров AKS в подсети виртуальной сети Azure за пределами этого диапазона адресов, например *172.16.0.0/16*.
...
Дополнительные сведения, помогающие решить, какую модель сети для использования, см. в разделе [сравнение моделей сетей и их объем поддержки][network-comparisons].
...
Чтобы приступить к работе с *kubenet* и собственные подсети виртуальной сети, сначала создайте группу ресурсов с помощью [Создание группы az][az-group-create] команды. В следующем примере создается группа ресурсов с именем *myResourceGroup* в расположении *eastus*.

```azurecli-interactive
az group create --name myResourceGroup --location eastus
```

Если у вас нет существующей виртуальной сети и подсети для использования, создайте этим сетевым ресурсам с помощью [Создание виртуальной сети az сети][az-network-vnet-create] команды. В следующем примере виртуальная сеть называется *myVnet* с префиксом адреса *192.168.0.0/16*. Подсеть создается именованный *myAKSSubnet* с префиксом адреса *192.168.1.0/24*.
...
Чтобы назначить правильный делегирования в оставшихся шагах, используйте [az сети vnet show][az-network-vnet-show] and [az network vnet subnet show][az-network-vnet-subnet-show] команды, чтобы получить необходимые идентификаторы ресурсов. Эти идентификаторы ресурсов хранятся как переменные и указываются в оставшихся действиях:
...
Теперь назначение субъекта-службы для кластера AKS *участник* разрешения для виртуальной сети с помощью [Создание назначений ролей az][az-role-assignment-create] команды. Предоставить свой собственный  *\<appId >* как показано в выходных данных из предыдущей команды для создания субъекта-службы:
...
Вы создали виртуальную сеть и подсеть, а также создали и назначили разрешения для субъекта-службы, чтобы использовать эти сетевые ресурсы. Теперь создание кластера AKS в виртуальной сети и подсети с помощью [создать az aks][az-aks-create] команды. Определить собственный субъект-службу  *\<appId >* и  *\<пароль >* , как показано в выходных данных из предыдущей команды для создания субъекта-службы.

Приведенные ниже диапазоны IP-адресов также определяются в процессе создания кластера:

* *--service-cidr* используется для назначения IP-адреса внутренним службам в кластере AKS. Этот диапазон IP-адресов не должен использоваться где-либо еще в сетевой среде. Этот диапазон включает в себя все диапазоны локальной сети при подключении или планируете подключиться, виртуальным сетям Azure с помощью Express Route или сеть-сеть VPN-подключение.

* Адрес *--dns-service-ip* должен быть адресом *.10* диапазона IP-адресов вашей службы.

* Диапазон *--pod-cidr* должен быть большим адресным пространством, которое не используется где-либо еще в сетевой среде. Этот диапазон включает в себя все диапазоны локальной сети при подключении или планируете подключиться, виртуальным сетям Azure с помощью Express Route или сеть-сеть VPN-подключение.
    ...
    * Диапазон IP-адресов объектов pod используется для назначения адресного пространства */24* каждому узлу в кластере. В следующем примере *--pod cidr* из *10.244.0.0/16* назначает первый узел *10.244.0.0/24*, второй узел *10.244.1.0/24*и третий узел *10.244.2.0/24*.
...
* *--Адрес для моста docker* позволяет AKS узлы взаимодействуют с базовой платформы управления. Этот IP-адрес не должен находиться в пределах диапазона IP-адресов виртуальной сети вашего кластера и перекрывать другие диапазоны адресов, используемые в сети.
