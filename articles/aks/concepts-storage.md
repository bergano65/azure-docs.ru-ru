...
В AKS создаются 4 первоначальные Сторажеклассес:

- *по умолчанию* — использует хранилище Azure стандардссд для создания управляемого диска. Политика reclaimа указывает, что базовый диск Azure удаляется при удалении постоянного тома, который использовался для удаления.
- *managed-premium* — использует службу хранилища Azure ценовой категории "Премиум" для создания управляемого диска. Политика повторного утверждения указывает, что базовый диск Azure удаляется при удалении постоянного тома, который использовался для удаления.
- *азурефиле* — использует хранилище Azure уровня "Стандартный" для создания файлового ресурса Azure. Политика reclaimа указывает, что базовый файловый ресурс Azure удаляется при удалении постоянного тома, который использовался для удаления.
- *азурефиле-Premium* — использует хранилище Azure класса Premium для создания файлового ресурса Azure. Политика reclaimа указывает, что базовый файловый ресурс Azure удаляется при удалении постоянного тома, который использовался для удаления.
