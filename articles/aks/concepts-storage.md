...
- *Диски Azure* можно использовать для создания ресурсов *DataDisk* в Kubernetes. Диски могут использовать службу хранилища Azure ценовой категории "Премиум" на основе высокопроизводительных дисков SSD или службу хранилища Azure ценовой категории "Стандартный" на основе обычных жестких дисков. Для большинства производственных рабочих нагрузок и рабочих нагрузок разработки следует использовать службу хранилища ценовой категории "Премиум". Azure Disks монтируются как *ReadWriteOnce*, поэтому доступны только для одного стручка. Для объемов хранения, к которым можно получить доступ одновременно несколькими стручками, используйте файлы Azure.
- *Файлы Azure* можно использовать для подключения к элементам pod общей папки SMB 3.0, размещенной в учетной записи хранения Azure. Служба файлов обеспечивает обмен данными между несколькими узлами и элементами pod. Файлы могут использовать хранилище Azure Standard, поддерживаемое обычными HDD- или хранилищем Azure Premium, подкрепленное высокопроизводительными SSD-файлами.
> [!NOTE] 
> Azure Files поддерживает премиум-хранилище в кластерах AKS, запускаемых Kubernetes 1.13 или выше.
...
- *default* — использует службу хранилища Azure ценовой категории "Стандартный" для создания управляемого диска. Политика reclaim указывает, что базовый Azure Disk удаляется при удалении используемого объема.
- *managed-premium* — использует службу хранилища Azure ценовой категории "Премиум" для создания управляемого диска. Политика reclaim снова указывает на то, что базовый Azure Disk удаляется при удалении используемого объема.
