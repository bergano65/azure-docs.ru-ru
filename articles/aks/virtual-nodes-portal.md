...
Используйте [az aks get-credentials][az-aks-get-credentials] команду, чтобы настроить `kubectl` для подключения к кластеру Kubernetes. В следующем примере возвращаются учетные данные для имени кластера *myAKSCluster* в группе ресурсов *myResourceGroup*.
...
Создайте в Azure Cloud Shell файл с именем `virtual-node.yaml` и скопируйте в него следующий код YAML. Чтобы запланировать контейнера на узле, [nodeSelector][node-selector] and [toleration][toleration] определены. Они включают выполнение группы pod в этом виртуальном узле и позволяют убедиться, что функция успешно включена.
...
Используйте [kubectl get pods][kubectl-get] с `-o wide` аргумент для вывода списка модулей и запланированных узла. Обратите внимание, что pod `virtual-node-helloworld` был назначен на узел `virtual-node-linux`.
