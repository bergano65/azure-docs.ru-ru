...
Требуется Azure CLI версии 2.0.76 или более поздней. Чтобы узнать версию, выполните команду  `az --version`. Если вам необходимо выполнить установку или обновление, см. статью  [Установка Azure CLI][install-azure-cli].
...
При создании кластера с помощью команды [AZ AKS Create][az-aks-create] `--zones` параметр определяет, в какие узлы агента зоны развертываются. Компоненты плоскости управления AKS для кластера также распределяются между зонами в самой высокой доступной конфигурации при определении `--zones` параметра во время создания кластера.
...
Обратите внимание, что в более новых версиях Kubernetes (1.17.0 и более поздних версий) `topology.kubernetes.io/zone` AKS использует более новую метку в `failure-domain.beta.kubernetes.io/zone`дополнение к устаревшей.

## <a name="verify-pod-distribution-across-zones"></a>Проверка распределения Pod между зонами

Как описано в [хорошо известных метках, Annotations и таинтс][kubectl-well_known_labels], Kubernetes использует эту `failure-domain.beta.kubernetes.io/zone` метку для автоматического распределения модулей Pod в контроллере или службе репликации в разных доступных зонах. Чтобы протестировать это, можно увеличить масштаб кластера с 3 до 5 узлов, чтобы убедиться в правильности распространения Pod:
...
Как видно из предыдущих выходных данных, первый модуль выполняется на узле 0, который находится в зоне `eastus2-1`доступности. Второй модуль выполняется на узле 2, который соответствует `eastus2-3`, а третий — в узле 4 в. `eastus2-2` Без какой-либо дополнительной настройки Kubernetes правильно распределяет модули Pod по всем трем зонам доступности.
