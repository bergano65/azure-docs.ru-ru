...
Если вы не использовали ACI ранее, зарегистрируйте поставщик служб в подписке. Можно проверить состояние регистрации поставщика ACI с помощью [список поставщиков az][az-provider-list] команды, как показано в следующем примере:
...
Теперь создайте дополнительную подсеть для виртуальных узлов с помощью [Создание подсети виртуальной сети az сети][az-network-vnet-subnet-create] команды. В следующем примере создается подсеть с именем *myVirtualNodeSubnet* с префиксом адреса *10.241.0.0/16*.
...
Сначала получите идентификатор ресурса виртуальной сети с помощью [az сети vnet show][az-network-vnet-show]:

```azurecli-interactive
az network vnet show --resource-group myResourceGroup --name myVnet --query id -o tsv
```

Чтобы предоставить надлежащих прав доступа в кластере AKS для использования виртуальной сети, Создание назначения роли с помощью [Создание назначений ролей az][az-role-assignment-create] команды. Замените `<appId`> и `<vnetId>` на значения, собранных на предыдущих двух шагах.
...
Разверните кластер AKS в подсеть AKS, созданную на предыдущем шаге. Получите идентификатор для этой подсети с помощью [show подсети виртуальной сети сети az][az-network-vnet-subnet-show]:
...
Чтобы включить виртуальные узлы, теперь использовать [az aks enable-надстройки для][az-aks-enable-addons] команды. В следующем примере используется подсеть с именем *myVirtualNodeSubnet*, созданная на предыдущем шаге:
...
Создайте файл `virtual-node.yaml` и скопируйте в него следующий код YAML. Чтобы запланировать контейнера на узле, [nodeSelector][node-selector] and [toleration][toleration] определены.
...
Запускает приложение с [kubectl применить][kubectl-apply] команды.

```console
kubectl apply -f virtual-node.yaml
```

Используйте [kubectl get pods][kubectl-get] с `-o wide` аргумент для вывода списка модулей и запланированных узла. Обратите внимание, что pod `aci-helloworld` был назначен на узел `virtual-node-aci-linux`.
...
> [!NOTE]
> При использовании образов, хранящихся в реестре контейнеров Azure, [настроить и использовать секрет Kubernetes][acr-aks-secrets]. Текущее ограничение виртуальных узлов — что нельзя использовать интеграции Azure AD аутентификация субъекта-службы. Если вы не используете секрет, модули pod, запланированные на виртуальные узлы, не будут запускаться и сообщат об ошибке `HTTP response status code 400 error code "InaccessibleImage"`.
...
Если вы больше не хотите использовать виртуальные узлы, их можно отключить с помощью [az aks disable-надстройки для][az aks disable-addons] команды. 
...
- [Использовать инструмент автомасштабирования горизонтальной pod Kubernetes][aks-hpa]
- [Использовать инструмент автомасштабирования кластера Kubernetes][aks-cluster-autoscaler]
