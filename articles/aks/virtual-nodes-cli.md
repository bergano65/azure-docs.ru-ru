...
Теперь создайте дополнительную подсеть для виртуальных узлов с помощью команды [AZ Network vnet подсети Create][az-network-vnet-subnet-create] . В следующем примере создается подсеть с именем *myVirtualNodeSubnet* с префиксом адреса *10.241.0.0/16*.
...
Сначала получите идентификатор ресурса виртуальной сети с помощью команды [AZ Network vnet показывать][az-network-vnet-show]:

```azurecli-interactive
az network vnet show --resource-group myResourceGroup --name myVnet --query id -o tsv
```

Чтобы предоставить правильный доступ к виртуальной сети в кластере AKS, создайте назначение роли с помощью команды [AZ Role назначение Create][az-role-assignment-create] . Замените `<appId`> и `<vnetId>` на значения, собранных на предыдущих двух шагах.
...
Разверните кластер AKS в подсеть AKS, созданную на предыдущем шаге. Получите идентификатор этой подсети с помощью команды [AZ Network vnet Subnet показывать][az-network-vnet-subnet-show]:
...
Чтобы включить виртуальные узлы, теперь используйте команду [AZ AKS Enable-надстроек][az-aks-enable-addons] . В следующем примере используется подсеть с именем *myVirtualNodeSubnet*, созданная на предыдущем шаге:
...
Создайте файл `virtual-node.yaml` и скопируйте в него следующий код YAML. Чтобы запланировать контейнер на узле, определяются [нодеселектор][node-selector] и [допуска][toleration] .
...
Используйте команду [kubectl Get][kubectl-get] Pod с аргументом `-o wide` для вывода списка модулей Pod и запланированного узла. Обратите внимание, что pod `aci-helloworld` был назначен на узел `virtual-node-aci-linux`.
...
Если вы больше не хотите использовать виртуальные узлы, их можно отключить с помощью команды [AZ AKS Disable-надстроек][az aks disable-addons] . 

Сначала удалите модуль HelloWorld, выполняющийся на виртуальном узле:
