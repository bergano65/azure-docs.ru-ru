...
На портале Azure Monitor включены и управляются журналы Azure Monitor. Чтобы включить сбор журналов для основных компонентов Kubernetes в своем кластере AKS, откройте портал Azure в веб-браузере и выполните шаги ниже.
...
1. Выберите кластер AKS, например *myAKSCluster,* а затем выберите **диагностическую настройку.**
1. Введите имя, например *myAKSClusterLogs*, а затем выберите **Отправить в Log Analytics**.
1. Выберите существующее рабочее пространство или создайте новое. При создании рабочего пространства укажите имя рабочего пространства, группу ресурсов и местоположение.
1. В списке доступных журналов выберите журналы, которые вы хотите включить. Общие журналы включают *kube-аписервер,* *кубе-контроллер-менеджер*, и *кубе-планировщик*. Вы можете включить дополнительные журналы, такие как *kube-audit* и *cluster-autoscaler*. Когда рабочая область Log Analytics будет включена, вы сможете вернуть или изменить собранные журналы.
1. Когда настройка будет завершена, нажмите кнопку **Сохранить**, чтобы включить сбор выбранных журналов.

На следующем скриншоте портала приведено окно *настроек диагностики:*
...
Включение журналов диагностики и их отображение в рабочей области Log Analytics может занять несколько минут. На портале Azure выберите группу ресурсов для рабочего пространства Log Analytics, например *myResourceGroup,* а затем выберите ресурс аналитики журналов, например *myAKSLogs.*
...
Для получения дополнительной информации о том, как запросить и отфильтровать данные журнала, [просмотрите или проанализируйте данные, собранные с помощью поиска журналов аналитики.][analyze-log-analytics]
...
| *Время*                   | Метка времени — время передачи журнала |
| *Категории*               | Имя контейнера или компонента, генерирующего журнал |
| *эксплуатацияИмя*          | Всегда *Служба контейнеров Майкрософт, управляемые кластеры, журналы диагностики, чтение* |
...
| *properties.containerID* | ID докер контейнера этот журнал пришел из |

## <a name="log-roles"></a>Роли журнала

| Роль                     | Описание |
|--------------------------|-------------|
| *aksService*             | Имя дисплея в журнале аудита для работы плоскости управления (от hcpService) |
| *мастер-клиент*           | Имя дисплея в журнале аудита для MasterClientCertificate, сертификат, который вы получаете от az aks получить учетные данные |
| *нодеклиент*             | Имя дисплея ClientCertificate, которое используется узлами агента |
