...
1. Выберите **Azure Active Directory** > **Регистрация приложений** > **новую регистрацию**.
    ...
    a. Присвойте приложению имя, например *аксазуреадсервер*.

    b. Для **поддерживаемых типов учетных записей**выберите **учетные записи только в этом каталоге Организации**.
    
    c. Выберите **Интернет** для типа URI перенаправления, а затем введите любое значение в формате URI, например *https://aksazureadserver*.

    d. По завершении выберите **Регистрация** .
...
3. В левой области приложения Azure AD выберите **сертификаты & секреты**.
    ...
    a. Выберите **+ новый секрет клиента**.

    b. Добавьте описание ключа, например *AKS Azure AD Server*. Выберите срок действия, а затем нажмите кнопку **Добавить**.

    c. Обратите внимание на значение ключа, которое отображается только в данный момент. При развертывании кластера AKS с поддержкой Azure AD это значение называется секретом серверного приложения.

4. В левой области приложения Azure AD выберите **разрешения API**и щелкните **+ Добавить разрешение**.

    a. В разделе **API-интерфейсы Майкрософт**выберите **Microsoft Graph**.

    b. Выберите **делегированные разрешения**и установите флажок для **каталога > каталог. чтение. все (чтение данных каталога)**.

    c. Если по умолчанию делегированное разрешение **пользователя > пользователя. Read (вход и чтение профиля пользователя)** не существует, установите флажок рядом с ним.

    d. Выберите **разрешения приложения**и установите флажок для **каталога > Directory. Read. All (чтение данных каталога)**.
...
5. В левой области приложения Azure AD выберите **предоставить API**, а затем выберите **+ Добавить область**.
    ...
    a. Введите **имя области**, **Отображаемое имя согласия администратора**, а затем **Описание согласия администратора** , например *аксазуреадсервер*.

    b. Убедитесь, что для параметра **состояние** установлено значение **включено**.
...
1. Выберите **Azure Active Directory** > **Регистрация приложений** > **новую регистрацию**.
    ...
    a. Присвойте приложению имя, например *аксазуреадклиент*.

    b. Для **поддерживаемых типов учетных записей**выберите **учетные записи только в этом каталоге Организации**.

    c. Выберите **Интернет** для типа URI перенаправления, а затем введите любое значение в формате URI, например *https://aksazureadclient*.

    >[!NOTE]
    >Если вы создаете новый кластер с поддержкой RBAC для поддержки Azure Monitor для контейнеров, добавьте следующие два URL-адреса перенаправления в этот список в качестве типов **веб-** приложений. Первый базовый URL-адрес должен иметь `https://afd.hosting.portal.azure.net/monitoring/Content/iframe/infrainsights.app/web/base-libs/auth/auth.html` значение, а второй базовый URL-адрес должен быть`https://monitoring.hosting.portal.azure.net/monitoring/Content/iframe/infrainsights.app/web/base-libs/auth/auth.html`
    >
    >Если вы используете эту функцию в Azure для Китая, то первым базовым URL-адресом должно быть `https://afd.hosting.azureportal.chinaloudapi.cn/monitoring/Content/iframe/infrainsights.app/web/base-libs/auth/auth.html` , а для второго базового URL-адреса должно быть`https://monitoring.hosting.azureportal.chinaloudapi.cn/monitoring/Content/iframe/infrainsights.app/web/base-libs/auth/auth.html`
    >
...
2. В левой области приложения Azure AD выберите **разрешения API**и щелкните **+ Добавить разрешение**.
    ...
    a. Выберите **Мои API**, а затем выберите приложение сервера Azure AD, созданное на предыдущем шаге, например *аксазуреадсервер*.

    b. Выберите **делегированные разрешения**, а затем установите флажок рядом с приложением сервера Azure AD.

    ![Настройка разрешений для приложения](media/aad-integration/select-api.png)

    c. Выберите **Добавить разрешения**.

    d. В разделе **предоставление согласия**выберите **предоставить согласие администратора**. Эта кнопка недоступна, если текущая учетная запись не является администратором клиента. При предоставлении разрешений на портале отображается следующее уведомление:
