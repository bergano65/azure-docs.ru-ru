...
1. В левой области навигации приложения Azure AD, выберите **проверки подлинности**.
...
1. Соответствующий идентификатор объекта или имя участника-пользователя, зависимости, является ли учетная запись пользователя в одном клиенте Azure AD или не определен.
2. Пользователь не может быть членом более чем 200 групп.
3. Секрет, определенные в регистрации приложения для сервера совпадает со значением, настроенные с помощью `--aad-server-app-secret`
