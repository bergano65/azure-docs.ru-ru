...
Операции кластера ограничены, когда выполняются активные операции обновления или была предпринята попытка обновления, но в дальнейшем произошел сбой. Для диагностики выполнения `az aks show -g myResourceGroup -n myAKSCluster -o table` проблемы, чтобы получить подробные сведения о состоянии кластера. В зависимости от результата:

* Если кластер активно обновляется, дождитесь завершения операции. Если это выполнено успешно, попробуйте выполнить ранее неудачную операцию.
* Если произошел сбой обновления кластера, выполните действия, описанные выше.
