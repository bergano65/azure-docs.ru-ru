...
1. Обновление AKS или AKS Нодепул
   ...
   По умолчанию кластеры AKS устанавливают значение максимального всплеска напряжения ("буфер обновления"), равное одному (1), но это поведение может быть настроено путем установки [максимального значения всплеска для пула узлов](upgrade-cluster.md#customize-node-surge-upgrade) , что увеличит количество доступных IP-адресов, необходимых для завершения обновления.
...
Обычно это обусловлено истечением срока действия учетных данных субъекта-службы. [Обновите учетные данные для кластера AKS.](update-credentials.md)
...
- В VMSS существует множество кластеров или нодепулс AKS. Обычно рекомендуется использовать менее 20-30 кластеров в данной подписке.
...
В Linux может появиться сообщение об ошибке неверного формата DevicePath. Пример:
...
В Windows может появиться ошибка неверного номера устройства DevicePath (LUN). Пример:
