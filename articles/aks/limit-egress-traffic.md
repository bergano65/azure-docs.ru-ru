...
> [!IMPORTANT]
> ***. BLOB.Core.Windows.NET и aksrepos.azurecr.IO** больше не требуют правил полного доменного имени для блокировки исходящего трафика.  Для существующих кластеров [выполните операцию обновления кластера][aks-upgrade] с помощью `az aks upgrade` команды, чтобы удалить эти правила.
...
|----------------------------|-----------|----------|
| *. HCP. \<location\>. azmk8s.IO | HTTPS: 443, TCP: 22, TCP: 9000, UDP: 1194 | Этот адрес необходим для взаимодействия с сервером API узла <->. Замените * \<расположение\> * на регион, в котором развернут кластер AKS. |
| *. Тун. \<location\>. azmk8s.IO | HTTPS: 443, TCP: 22, TCP: 9000, UDP: 1194 | Этот адрес необходим для взаимодействия с сервером API узла <->. Замените * \<расположение\> * на регион, в котором развернут кластер AKS. |
...
| Полное доменное имя.                       | Порт      | Использование      |
|----------------------------|-----------|----------|
| *. HCP. \<location\>. CX.prod.Service.azk8s.CN | HTTPS: 443, TCP: 22, TCP: 9000, UDP: 1194 | Этот адрес необходим для взаимодействия с сервером API узла <->. Замените * \<расположение\> * на регион, в котором развернут кластер AKS. |
| *. Тун. \<location\>. CX.prod.Service.azk8s.CN | HTTPS: 443, TCP: 22, TCP: 9000, UDP: 1194 | Этот адрес необходим для взаимодействия с сервером API узла <->. Замените * \<расположение\> * на регион, в котором развернут кластер AKS. |
...
## <a name="required-by-windows-server-based-nodes-enabled"></a>Требуется для узлов на базе Windows Server, которые включены
