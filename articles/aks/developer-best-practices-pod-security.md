---
title: Лучшие практики безопасности Pod
...
Эта лучшая практика статье сосредоточена на том, как обеспечить стручки в AKS. Вы узнаете, как выполнять следующие задачи:
...
**Наилучший вариант**. Не определяйте учетные данные в коде приложения. Используйте управляемые удостоверения ресурсов Azure, чтобы позволить группе pod запрашивать доступ к другим ресурсам. Кроме того, для хранения и извлечения цифровых ключей и учетных данных следует применить цифровое хранилище, например Azure Key Vault. Pod управляемых идентификаторов предназначен для использования с Linux стручки и контейнер изображения только.
...
Следующие [связанные проекты с открытым исходным кодом AKS][aks-associated-projects] позволяют автоматически аутентифицировать стручки или запрашивать учетные данные и ключи из цифрового хранилища:

* управляемые удостоверения для ресурсов Azure;
* драйвер FlexVol для Azure Key Vault.

Связанные проекты с открытым исходным кодом AKS не поддерживаются технической поддержкой Azure. Они предоставляются для сбора отзывов и ошибок от нашего сообщества. Эти проекты не рекомендуются для использования в производстве.

### <a name="use-pod-managed-identities"></a>Использование управляемых удостоверений для групп pod

Управляемый имитатор для ресурсов Azure позволяет аутентифицировать себя в отношении служб Azure, поддерживающих его, таких как хранилище или S'L. Группе pod присваивается удостоверение Azure, которое позволяет проходить аутентификацию Azure Active Directory и получать цифровой маркер. Этот цифровой маркер предоставляется другим службам Azure, которые проверяют авторизацию группы pod для доступа к соответствующей службе и выполнения запрошенных действий. Такой подход позволяет, в частности, не использовать секреты в строках подключения к базам данных. На следующей схеме показан упрощенный рабочий процесс при использовании управляемых удостоверений группы pod:
...
Для получения дополнительной информации об идентификационных данных стручка [см. Наверяйте кластер AKS для использования управляемых идентификационных данных стручка и с помощью приложений][aad-pod-identity]
...
Azure Key Vault с FlexVol предназначен для использования с приложениями и сервисами, работающими на стручках и узлах Linux.
