...
**Наилучший вариант**. Не определяйте учетные данные в коде приложения. Используйте управляемые удостоверения ресурсов Azure, чтобы позволить группе pod запрашивать доступ к другим ресурсам. Кроме того, для хранения и извлечения цифровых ключей и учетных данных следует применить цифровое хранилище, например Azure Key Vault. Модуль управляемого удостоверения предназначен для использования с Linux модулей и только для образов контейнера.
...
Следующие [связанные AKS проектов с открытым кодом] [ aks-associated-projects] позволяют автоматическую проверку подлинности модулей или запрос учетных данных и ключи из цифровой хранилища:

* управляемые удостоверения для ресурсов Azure;
* драйвер FlexVol для Azure Key Vault.

Связанные проекты с открытым кодом AKS не поддерживаются по технической поддержке Azure. Они предназначены для сбора отзывов и ошибки нашего сообщества. Эти проекты не рекомендуются для использования в рабочей среде.
...
Дополнительные сведения об удостоверениях pod см. в разделе [настроить кластер AKS, чтобы использовать удостоверения управляемых pod и с приложениями][aad-pod-identity]
...
Azure Key Vault, выполнив FlexVol предназначен для использования с приложениями и службами под управлением Linux модулей и узлов.
