...
В процессе разработки и выполнения приложений в службе контейнеров Azure (AKS) важно не забывать о безопасности групп pod. Приложению должно быть предоставлено минимальное число требуемых привилегий. Защита личных данных играет важнейшую роль для клиентов. Вы вряд ли захотите, чтобы учетные данные, например строки подключения к базам данных, ключи, секреты и сертификаты свободно предоставлялись внешнему миру, где злоумышленник могут воспользоваться ими в злонамеренных целях. Не добавляйте учетные данные в код и не внедряйте в образы контейнеров. Такой подход создает риск компрометации и снижает возможность замены учетных данных, так как для этого придется перестраивать образы контейнеров.
