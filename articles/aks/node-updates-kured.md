...
В этой статье показано, как использовать с открытым кодом [kured (управляющая программа перезагрузки KUbernetes)] [ kured] для отслеживания для узлов Linux, которые требуют перезагрузки, затем автоматически обрабатывать изменения расписания выполнения модулей и узел процесс перезагрузки.
...
В этой статье описывается, как использовать `kured` для перезагрузки узлов Linux автоматически в рамках процесса обновления безопасности. Чтобы перейти на последнюю версию Kubernetes, вы можете [обновить кластер AKS][aks-upgrade].
