...
> * Держите узлы в актуальном состоянии и автоматически применяйте патчи безопасности
...
Вы также можете использовать [интеграцию служб Azure Kubernetes с Центром безопасности,][security-center-aks] чтобы помочь обнаружить угрозы и просматривать рекомендации по обеспечению безопасности кластеров AKS.
...
## <a name="process-linux-node-updates-and-reboots-using-kured"></a>Обработка обновлений узлов Linux и перезагрузки с помощью kured

**Рекомендации по наилучшей практике** - AKS автоматически загружает и устанавливает исправления безопасности на каждом узеле Linux, но при необходимости не перезагружается автоматически. Используйте `kured` для отслеживания ожидающих перезагрузок, затем безопасно заблокируйте и очистите узел, чтобы перегрузить его, примените обновления и обеспечьте максимальную безопасность с учетом ОС. Для узлов Windows Server (в настоящее время в предварительном просмотре в AKS) регулярно выполняйте операцию обновления AKS, чтобы безопасно окороточить и слить стручки и развернуть обновленные узлы.

Каждый вечер, Linux узлов в AKS получить патчи безопасности доступны через их канал обновления дистро. Это настраивается автоматически, так как узлы развернуты в кластере AKS. Чтобы свести к минимуму нарушения и возможные последствия для рабочих нагрузок, узлы не перезагружается автоматически, если исправление или обновление ядра этого требуют.

Проект с открытым исходным кодом от Weaveworks [kured (KUbernetes REboot Daemon)][kured] следит за ожидающими перезагрузками узлов. Когда узло Linux применяет обновления, требующие перезагрузки, узла безопасно оцепление и осушеное для перемещения и планирования стручков на других узлах кластера. После перезагрузки узел добавляется обратно в кластер и Kubernetes возобновляет планирование групп pod, содержащихся на нем. Для минимизации сбоев в работе `kured` может перезагрузить только один узел за раз.
