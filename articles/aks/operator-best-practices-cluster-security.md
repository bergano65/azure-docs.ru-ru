---
title: Рекомендации по обеспечению безопасности кластера
...
**Рекомендации**. Обеспечение безопасного доступа к серверу API Kubernetes является одним из самых важных факторов для защиты кластера. Интегрируйте Kubernetes управления доступом на основе ролей (Kubernetes RBAC) с Azure Active Directory для управления доступом к серверу API. Эти элементы управления позволяют защитить AKS так же, как и доступ к подпискам Azure.
...
Используйте интеграцию с Kubernetes RBAC и Azure AD для защиты сервера API и предоставляйте наименьшее количество разрешений, требуемых для набора ресурсов с заданной областью, например единое пространство имен. Различным пользователям или группам в Azure AD могут быть предоставлены разные роли Kubernetes. Эти детализированные разрешения позволяют ограничивать доступ к серверу API и предоставляют четкий журнал аудита выполненных действий.

Рекомендуется использовать группы для предоставления доступа к файлам и папкам, а также отдельным удостоверениям, *использовать членство в группах Azure* AD для привязки пользователей к ролям Kubernetes, а не отдельным *пользователям*. При изменении членства в группах пользователей их права доступа в кластере AKS будут меняться соответственно. Если вы связываете пользователя непосредственно с ролью, его функциональные обязанности могут измениться. Членство в группах Azure AD будет обновлено, но разрешения для кластера AKS не будут отражать это. В таком случае пользователю предоставляется больше разрешений, чем ему требуется.

Дополнительные сведения об интеграции Azure AD, Kubernetes RBAC и Azure RBAC см. в статье рекомендации [по проверке подлинности и авторизации в AKS][aks-best-practices-identity].
...
**Рекомендации по** обновлению новых компонентов и исправлению ошибок. регулярно обновляйте версию Kubernetes в кластере AKS.
...
Настоятельно рекомендуется протестировать новые дополнительные версии в среде тестирования разработки, чтобы можно было проверить, что Рабочая нагрузка будет продолжать работоспособную работу с новой версией Kubernetes. Kubernetes может стать устаревшими API, например в версии 1,16, которые могут полагаться на ваши рабочие нагрузки. При переводе новых версий в рабочую среду рекомендуется использовать [несколько пулов узлов в отдельных версиях](use-multiple-node-pools.md) и обновлять отдельные пулы по одному за раз, чтобы постепенно выполнить обновление в кластере. При запуске нескольких кластеров одновременно обновите один кластер, чтобы поочередно отслеживать влияние и изменения.
...
Проект с открытым исходным кодом от Weaveworks [kured (KUbernetes REboot Daemon)][kured] следит за ожидающими перезагрузками узлов. Когда узел Linux применяет обновления, требующие перезагрузки, узел безопасно блокируются и загружается для перемещения и планирования модулей Pod на других узлах в кластере. После перезагрузки узел добавляется обратно в кластер и Kubernetes возобновляет планирование групп pod, содержащихся на нем. Для минимизации сбоев в работе `kured` может перезагрузить только один узел за раз.
