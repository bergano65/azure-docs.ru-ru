---
title: Используйте сертификаты TLS для входа
...
В этой статье для установки контроллера входящего трафика NGINX и примера веб-приложения используется Helm. Поэтому необходимо инициализировать Helm в кластере AKS и использовать учетную запись службы для Tiller. Убедитесь, что вы используете последний выпуск Helm. Для инструкций по [обновлению][helm-install]см. Для получения дополнительной информации о настройке и использовании Helm [см.][use-helm]
...
> [!TIP]
> Если имя хоста, указанное в процессе запроса сертификата, имя CN, не совпадает с хостом, определенным в маршруте входа, вы вгоняете контроллер отображения предупреждения *о поддельном сертификате контроллера Kubernetes Ingress.* Убедитесь в том, что имена сертификата и узла в маршруте входящего трафика совпадают.
...
Дополнительный путь с адресом не был предоставлен, поэтому */* контроллер входа по умолчанию по умолчанию к маршруту. Первое демонстрационное приложение возвращается, как показано в следующем сокращенном примере выходных данных:
