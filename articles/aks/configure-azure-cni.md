...
По умолчанию AKS кластеров под управлением Windows используйте [kubenet][kubenet], и виртуальную сеть и подсеть создаются автоматически. При использовании *kubenet* узлы получают IP-адрес из подсети виртуальной сети. Затем настраивается преобразование сетевых адресов (NAT) на узлах, а модули получают IP-адрес, "скрытый" за IP-адресом узла. Такой подход уменьшает количество IP-адресов, которые необходимо зарезервировать в пространстве сети для модулей pod.
...
В этой статье показано, как использовать сеть *Azure CNI*, чтобы создавать и использовать подсеть виртуальной сети для кластера AKS. Дополнительные сведения о сети вариантов и соображений, см. в разделе [сети основные понятия Kubernetes и AKS][aks-network-concepts].
...
* Кластерах AKS не имеете права использовать `169.254.0.0/16`, `172.30.0.0/16`, `172.31.0.0/16`, или `192.0.2.0/24` диапазон адресов службы Kubernetes.
...
> [!NOTE]
> Минимальное значение в приведенной выше таблице четко реализовывались алгоритмы давности служба AKS. Не удается задать значение maxPods ниже показано, как это можно предотвратить кластера начиная минимальных.

* **Azure CLI.** Укажите `--max-pods` аргумент при развертывании кластера с [создать az aks][az-aks-create] команды. Максимальное значение — 250.
...
**Диапазон адресов службы Kubernetes.** Это набор виртуальных IP-адресов, который Kubernetes назначает внутренний [служб][services] в кластере. Можно использовать любой диапазон частных адресов, который отвечает следующим требованиям:
...
* Не должно быть в диапазоне `169.254.0.0/16`, `172.30.0.0/16`, `172.31.0.0/16`, или `192.0.2.0/24`

Не рекомендуется указывать диапазон адресов службы в той же виртуальной сети, что и кластер, хотя технически это возможно. Перекрывающиеся диапазоны IP-адресов могут привести к непредсказуемому поведению. Дополнительные сведения см. в разделе этой статьи [с вопросами и ответами](#frequently-asked-questions). Дополнительные сведения о службах Kubernetes см. в разделе [служб][services] в документации по Kubernetes.
...
**Адрес моста Docker.** IP-адрес и маска сети, назначаемые мосту Docker. Мост Docker позволяет узлам AKS взаимодействовать с базовой платформой управления. Этот IP-адрес не должен находиться в пределах диапазона IP-адресов виртуальной сети вашего кластера и перекрывать другие диапазоны адресов, используемые в сети.
...
Используйте [создать az aks][az-aks-create] с `--network-plugin azure` аргумент для создания кластера с интенсивной работы в сети. Измените значение `--vnet-subnet-id`, указав идентификатор подсети, полученный на предыдущем шаге.
...
* *Можно ли развернуть виртуальные машины в подсети моего кластера?*
  ...
  № Развертывание виртуальных машин в подсети, используемой кластером Kubernetes, не поддерживается. Виртуальные машины можно развернуть в одной и той же виртуальной сети, но в разных подсетях.

* *Можно ли настроить политики сети для каждого контейнера pod?*

  Да, политики сети Kubernetes в AKS. Чтобы приступить к работе, см. в разделе [защиты трафика между модулей с помощью политик сети в AKS][network-policy].
...
- [Создайте входящий контроллер статического общедоступного IP-адреса и настройка Let's Encrypt для автоматического создания TLS-сертификаты][aks-ingress-static-tls]

### <a name="aks-engine"></a>Обработчик AKS

[Модуль службы Azure Kubernetes (AKS ядро)][aks-engine] — это проект с открытым исходным кодом, приводит к возникновению ошибки шаблоны Azure Resource Manager, которые можно использовать для развертывания кластеров Kubernetes в Azure.

Кластеры Kubernetes, созданные средством AKS поддерживает оба [kubenet][kubenet] and [Azure CNI][cni-networking] подключаемых модулей. Таким образом, обработчик AKS поддерживает оба сценария сети.
