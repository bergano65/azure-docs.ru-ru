...
* В настоящее время не может использоваться несколько пулов узлов (в настоящее время в предварительной версии в AKS).
