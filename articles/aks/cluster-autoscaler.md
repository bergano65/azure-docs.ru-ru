...
Если вам нужен новый кластер AKS, выполните команду [az aks create][az-aks-create]. Чтобы включить и настроить Автомасштабирование кластера в пуле узлов для кластера, используйте параметр *--Enable-Cluster-Автомасштабирование* и укажите Node *--min-Count* и *--Max-Count*.
...
|----------------------------------|------------------------------------------------------------------------------------------|---------------|
| интервал сканирования                    | Частота повторной вычисления кластера для увеличения или уменьшения масштаба                                    | 10 с    |
...
| Балансировка-аналогичных групп узлов | Обнаружение схожих пулов узлов и балансировка количества узлов между ними | false |
...
При включении автомасштабирования кластера в пулах узлов в кластере эти кластеры также будут использовать профиль автомасштабирования кластера. Пример:
...
При создании кластера также можно использовать параметр *cluster-ScaleY-Profile* . Пример:
...
1. Настройте правило для журналов ресурсов для принудительной отправки журналов автоматического масштабирования кластера в Log Analytics. [Здесь подробно описаны инструкции](https://docs.microsoft.com/azure/aks/view-master-logs#enable-resource-logs), которые следует установить `cluster-autoscaler` при выборе параметров для "журналы".
...
Автоматическое масштабирование кластера также записывает состояние работоспособности в ConfigMap с именем `cluster-autoscaler-status`. Чтобы получить эти журналы, выполните следующую `kubectl` команду. Состояние работоспособности будет выводиться для каждого пула узлов, настроенного с помощью автомасштабирования кластера.
