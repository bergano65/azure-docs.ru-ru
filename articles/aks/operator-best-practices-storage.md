...
Чтобы выбрать правильный уровень хранилища, важно понимать требования к производительности приложения и шаблоны доступа. Дополнительные сведения о размерах управляемые диски и уровни производительности, см. в разделе [обзоре управляемых дисков Azure][managed-disks]
...
Тип используемого хранилища определяется в *классах хранения* Kubernetes. Затем класс хранения указывается в спецификации группы pod или развертывания. Сочетание этих определений используется для создания соответствующего хранилища и его подключения к группе pod. Дополнительные сведения см. в разделе [классы хранения в AKS][aks-concepts-storage-classes].
...
| Тип и размер узла | Виртуальных ЦП | Память, ГиБ | Максимальное число дисков данных | Максимальное число некэшированных дисковых операций ввода-вывода в секунду | Максимальная некэшированнае пропускная способность (Мбит/с) |
|--------------------|------|--------------|----------------|------------------------|--------------------------------|
...
Дополнительные сведения о доступных размерах виртуальных Машин, см. в разделе [размеры виртуальных машин Linux в Azure][vm-sizes].
...
Основные понятия о том, как динамически создавать и использовать тома, см. в разделе [утверждения постоянного тома][aks-concepts-storage-pvcs].

Чтобы просмотреть эти тома в действии, см. в разделе как динамически создавать и использовать постоянный том с [диски Azure][dynamic-disks] or [Azure Files][dynamic-files].
...
Дополнительные сведения о параметрах класса хранения см. в разделе [хранилища освободить политики][reclaim-policy].

## <a name="secure-and-back-up-your-data"></a>Защита и резервное копирование данных

**Советы и рекомендации** — резервные копии данных, используя соответствующий инструмент для хранения типа, например Velero или Azure Site Recovery. Проверяйте целостность и безопасность этих резервных копий.

Если приложения хранят и используют данные на постоянных дисках или в файлах, следует регулярно выполнять резервное копирование или моментальные снимки этих данных. Диски Azure предоставляют встроенные технологии создания моментальных снимков. Для операций с моментальными снимками может потребоваться подключить приложения к потоковой записи на диск. [Velero][velero] can back up persistent volumes along with additional cluster resources and configurations. If you can't [remove state from your applications][remove-state], резервное копирование данных из постоянного тома и регулярно тестировать операции восстановления, чтобы проверить целостность данных и процессы, необходимые.

Выясните ограничения разных подходов к резервному копированию данных и оцените необходимость замораживать данные перед созданием моментального снимка. Резервные копии данных не всегда позволят восстановить среду приложения в кластерном развертывании. Дополнительные сведения об этих сценариях см. в разделе [советы и рекомендации для бизнеса непрерывности и аварийного восстановления в AKS][best-practices-multi-region].

## <a name="next-steps"></a>Дальнейшие действия

Эта статья посвящена рекомендациям по организации хранения в Службе контейнеров Azure. Дополнительные сведения об основах хранения в Kubernetes см. в разделе [концепции хранилища для приложений в AKS][aks-concepts-storage].
