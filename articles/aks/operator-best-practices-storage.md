...
| Неструктурированные данные, операции файловой системы | [БлобФуза][blobfuse] | Да | Да | Да | нет |
...
Утверждение постоянного тома позволяет динамически создавать хранилища, когда они потребуются. Базовые диски Azure создаются только при получении запроса от групп pod. В определении стручка вы запрашиваете объем, который должен быть создан и прикреплен к назначенному пути крепления.
...
**Рекомендации по наилучшей практике** - Резервное копирование данных с помощью соответствующего инструмента для вашего типа хранения, такого как Velero или Восстановление сайта Azure. Проверяйте целостность и безопасность этих резервных копий.

Если приложения хранят и используют данные на постоянных дисках или в файлах, следует регулярно выполнять резервное копирование или моментальные снимки этих данных. Диски Azure предоставляют встроенные технологии создания моментальных снимков. Возможно, вам придется искать приложения для промывки записей на диск, прежде чем выполнить операцию моментального снимка. [Velero][velero] может резервное копирование постоянных объемов наряду с дополнительными кластерными ресурсами и конфигурациями. Если вы не можете [запретить учет состояния в приложениях][remove-state], создавайте резервные копии данных из постоянных томов и регулярно проверяйте операции восстановления, чтобы гарантировать целостность данных и надежность процессов.
