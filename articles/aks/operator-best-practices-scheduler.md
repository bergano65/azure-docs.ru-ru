...
Ограничения и запросы на получение ресурсов помещаются в спецификацию pod. Эти ограничения используются в планировщике Kubernetes во время развертывания для поиска доступного узла в кластере. Эти ограничения и запросы работают на уровне отдельных pod. Дополнительные сведения о том, как определить эти значения, см. в разделе [запросы ресурсов определить pod и ограничения][resource-limits]
...
Дополнительные сведения о доступных ресурсов объектов, областей и приоритеты, см. в разделе [квоты ресурсов в Kubernetes][k8s-resource-quotas].
...
Дополнительные сведения об использовании бюджеты перебоев в работе pod см. в разделе [указывать бюджет перебоев в работе приложения][k8s-pdbs].
...
* [Изоляция Мультитенантность и кластера][aks-best-practices-cluster-isolation]
