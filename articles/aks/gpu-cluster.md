...
Опишите один из узлов, чтобы подтвердить, что процессоры GPU планируются. Для этого можно использовать раздел `Capacity`. Например, `nvidia.com/gpu:  1`. Если процессоры GPU не отображаются, обратитесь к разделу **Устранение неполадок** ниже.
...
Следующий манифест задания включает ограничение ресурса `nvidia.com/gpu: 1`. 
...
В некоторых сценариях в разделе Capacity могут не отображаться ресурсы GPU. Например: после обновления кластера до Kubernetes версии 1.10 или создания нового кластера Kubernetes версии 1.10 ожидаемый ресурс `nvidia.com/gpu` отсутствует в разделе `Capacity` при выполнении `kubectl describe node <node-name>`. 

Чтобы устранить эту проблему, примените после подготовки или обновления указанный ниже набор daemonset. `nvidia.com/gpu` отобразится как планируемый ресурс. 

Скопируйте манифест и сохраните его с именем **nvidia-device-plugin-ds.yaml**. Для указанного ниже тега образа `image: nvidia/k8s-device-plugin:1.10` обновите тег в соответствии с используемой версией Kubernetes. Например, для Kubernetes версии 1.11 используйте тег `1.11`.
...
Выполните команду [kubectl apply][kubectl-apply], чтобы создать daemonset.
...
Хотите выполнять рабочие нагрузки машинного обучения в Kubernetes? Дополнительные сведения см. в лабораториях Kubeflow.

> [!div class="nextstepaction"]
> [Лаборатории Kubeflow][kubeflow-labs]
