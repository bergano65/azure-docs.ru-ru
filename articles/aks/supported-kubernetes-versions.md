...
Например, если представлены AKS *1.13.x* в настоящее время поддержка также предоставляется для *1.12.a* + *1.12.b*, *1.11.c*  +  *1.11d*, *1.10.e* + *1.10F* (где выпусков обозначенные буквами исправлений являются две последние стабильные сборки).

Когда выпускается новая дополнительная версия, самая старая поддерживаемая дополнительная версия и выпуск исправлений удаляются. за 30 дней до выпуска новый дополнительный номер версии и прекращения использования новой версии, объявление будет выполнен с использованием [каналов Azure обновления][azure-update-channel]. В примере выше where *1.13.x* будет выпущена, выведенного из эксплуатации версии являются *1.9.g* + *1.9.h*.

При развертывании кластера AKS на портале или с помощью Azure CLI в кластере всегда используется дополнительная версия n-1 и последнее исправление. Например, если служба AKS поддерживает *1.13.x*, *1.12.a* + *1.12.b*, *1.11.c*  +   *1.11d*, *1.10.e* + *1.10F*, версия по умолчанию для новых кластеров — *1.11.b*.
...
- Если поддерживаются версии AKS *1.13.x*, *1.12.a* + *1.12.b*, *1.11.c*  +  *1.11d*, и *1.10.e* + *1.10F* , и используется *1.9.g* или *1.9.h*, не соответствуют цели уровня Обслуживания.
- Если обновление с *1.9.g* или *1.9.h* для *1.10.e* или *1.10.f* завершается успешно, вы снова находитесь в цели уровня Обслуживания.
