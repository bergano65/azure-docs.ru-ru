...
**Можно ли пропустить версию при обновлении?**

Нет, следуя рекомендациям kubernetes, AKS разрешает обновление только для немедленного перехода на следующий пакет исправлений или дополнительный номер версии. В портал Azure будут показаны только те версии, до которых вы можете выполнить обновление, а на интерфейсе командной строки можно `az aks get-upgrades -n MyAKSCluster -g MyResourceGroup` Просмотреть доступные обновления из текущей версии.

**Как выполнить обновление до поддерживаемой версии, если у меня несколько версий за последнюю поддерживаемую версию?**

Чтобы остаться в пределах поддержки, следует избегать использования нескольких версий из списка, поддерживаемого в настоящее время, но если вы находитесь в этой ситуации, AKS всегда позволит выполнить обновление до минимальной поддерживаемой версии.
