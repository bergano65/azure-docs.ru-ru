...
## <a name="can-i-spread-an-aks-cluster-across-regions"></a>Можно ли распределить кластер AKS по регионам?

Нет. Кластеры AKS являются региональными ресурсами и не могут охватывать регионы. Рекомендации по созданию архитектуры, включающей в себя несколько регионов, см. в статье рекомендации [по обеспечению непрерывности бизнес-процессов и аварийного восстановления][bcdr-bestpractices] .

## <a name="can-i-spread-an-aks-cluster-across-availability-zones"></a>Можно ли распределить кластер AKS между зонами доступности?

Да. Кластер AKS можно развернуть в одной или нескольких [зонах доступности в тех][availability-zones] [регионах, которые их поддерживают][az-regions].
...
- Используйте [Разрешенные диапазоны IP-адресов сервера API][api-server-authorized-ip-ranges] , если вы хотите поддерживать общедоступную конечную точку для сервера API, но ограничьте доступ набором доверенных IP-адресов.
- Используйте [частный кластер][private-clusters] , если требуется ограничить доступ к серверу API *только* из виртуальной сети.
...
Да, вы можете использовать разные размеры виртуальных машин в кластере AKS, создав [несколько пулов узлов][multi-node-pools].
...
AKS строится на основе нескольких ресурсов инфраструктуры Azure, включая масштабируемые наборы виртуальных машин, виртуальные сети и управляемые диски. Это позволяет использовать многие основные возможности платформы Azure в управляемой среде Kubernetes, предоставляемой AKS. Например, большинство типов виртуальных машин Azure можно использовать непосредственно с AKS, а резервирование Azure — для автоматического получения скидок на эти ресурсы.

Чтобы включить эту архитектуру, каждое развертывание AKS охватывает две группы ресурсов:

1. Вы создадите первую группу ресурсов. Эта группа содержит только ресурс службы Kubernetes. Поставщик ресурсов AKS автоматически создает вторую группу ресурсов во время развертывания. Пример второй группы ресурсов — *MC_myResourceGroup_myAKSCluster_eastus*. Сведения о том, как указать имя второй группы ресурсов, см. в следующем разделе.
1. Вторая группа ресурсов, называемая *группой ресурсов узла*, содержит все ресурсы инфраструктуры, связанные с кластером. Эти ресурсы включают виртуальные машины узла Kubernetes, виртуальную сеть и хранилище. По умолчанию группа ресурсов узла имеет такое же имя, как *MC_myResourceGroup_myAKSCluster_eastus*. AKS автоматически удаляет ресурс узла при каждом удалении кластера, поэтому его следует использовать только для ресурсов, которые совместно используют жизненный цикл кластера.
...
Да. По умолчанию AKS будет наименовать группу ресурсов узла *MC_resourcegroupname_clustername_location*, но можно также указать собственное имя.
...
## <a name="can-i-use-admission-controller-webhooks-on-aks"></a>Можно ли использовать веб-перехватчики контроллера допусков на AKS?

Да, вы можете использовать веб-перехватчики контроллера допусков на AKS. Рекомендуется исключить внутренние пространства имен AKS, помеченные **меткой плоскости управления.** Например, добавьте приведенный ниже элемент в конфигурацию веб-перехватчика.
...
## <a name="can-admission-controller-webhooks-impact-kube-system-and-internal-aks-namespaces"></a>Могут ли веб-перехватчики контроллеров допусков влиять на KUBE-System и внутренние пространства имен AKS?

Чтобы защитить стабильность системы и предотвратить влияние пользовательских контроллеров на использование внутренних служб в Kube-System, пространство имен AKS имеет **принудительный**прием, который автоматически исключает внутренние пространства имен KUBE-System и AKS. Эта служба гарантирует, что пользовательские контроллеры допуска не влияют на службы, работающие в Kube-System.

Если у вас есть критически важный вариант использования для развертывания на KUBE-System (не рекомендуется), который необходимо включить в веб перехватчик вашего пользовательского допуска, вы можете добавить следующую метку или аннотацию, чтобы их не проигнорировал.

Метка: ```"admissions.enforcer/disabled": "true"``` или Аннотация:```"admissions.enforcer/disabled": true```

## <a name="is-azure-key-vault-integrated-with-aks"></a>Интегрируется ли Azure Key Vault с AKS?

В настоящее время AKS не интегрирован с Azure Key Vault. Однако [поставщик Azure Key Vault для хранилища секретов CSI][csi-driver] обеспечивает прямую интеграцию с Kubernetes Pods для Key Vault секретов.
...
Поддержка пула узлов в Windows Server включает некоторые ограничения, которые являются частью вышестоящего сервера Windows Server в Kubernetes проекте. Дополнительные сведения об этих ограничениях см. [в статье контейнеры Windows Server в разделе ограничения AKS][aks-windows-limitations].
...
Убедитесь, что срок действия субъекта-службы не истек.  См. статью: [AKS Service Principal](https://docs.microsoft.com/azure/aks/kubernetes-service-principal) и [AKS Update Credential](https://docs.microsoft.com/azure/aks/update-credentials).

## <a name="my-cluster-was-working-but-suddenly-cannot-provision-loadbalancers-mount-pvcs-etc"></a>Мой кластер работал, но внезапно не смог подготавливать LoadBalancers, подключать виртуальные каналы и т. д.? 
