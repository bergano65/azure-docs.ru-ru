...
| -------------------|------------|----------------------------|-------------|
| Учетная запись администратора прежних версий с использованием сертификата клиента| **Роль администратора Azure Kubernetes**. Эта роль позволяет `az aks get-credentials` использовать с `--admin` флагом, который скачивает [устаревший сертификат администратора кластера (не в Azure AD)](control-kubeconfig-access.md) в учетную запись пользователя `.kube/config` . Это единственная цель "роль администратора Azure Kubernetes".|Недоступно|Если вы постоянно блокируете, не имея доступа к допустимой группе Azure AD с доступом к кластеру.| 
