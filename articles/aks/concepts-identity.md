...
> [!NOTE]
> Любые действия кластера, выполняемые службой поддержки Майкрософт, выполняются с согласия пользователя во встроенной роли Kubernetes ["Edit" (изменение](https://kubernetes.io/docs/reference/access-authn-authz/rbac/#user-facing-roles) имени) `aks-support-rolebinding` . С этой ролью поддержка AKS включена для изменения конфигурации кластера и ресурсов для устранения неполадок и диагностики кластеров, но роль не может изменять разрешения, а также не создавать роли или привязки ролей. Доступ к роли доступен только в рамках активных запросов в службу поддержки с JIT-доступом. Узнайте больше о [политиках поддержки AKS](support-policies.md).
