...
В службе Azure Kubernetes (AKS) узлы одной и той же конфигурации сгруппированы в *пулы узлов.* Пулы узлов содержат базовые VMs, которые управляют приложениями. Пулы системных узлов и пулы узлов пользователя представляют два различных режима пула узлов для кластеров AKS. Пулы системных узлов служат основной цели размещения критически важных системных стручков, таких как CoreDNS и туннель. Пулы узлов пользователей служат основной цели размещения стручков приложения. Тем не менее, стручки приложений могут быть запланированы в пулах системных узлов, если вы хотите иметь только один пул в кластере AKS. Каждый кластер AKS должен содержать по крайней мере один пул системных узлов с по крайней мере одним узлами. 
