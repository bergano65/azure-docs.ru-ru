...
При создании кластера клиент определяет рабочие узлы Kubernetes, создаваемые AKS. На этих узлах выполняются рабочие нагрузки клиентов. Пользователи владеют и могут просматривать или изменять рабочие узлы.

Так как узлы кластера клиентов выполняют частный код и хранят конфиденциальные данные, служба поддержки Майкрософт могут обращаться к ним только в ограниченном виде. Служба поддержки Майкрософт не может войти в, выполнить команды в или просмотреть журналы для этих узлов без явного разрешения или поддержки клиента.
...
Корпорация Майкрософт предоставляет техническую поддержку для следующих действий:
...
* Вопросы об использовании Kubernetes. Например, служба поддержки Майкрософт не предоставляет советов о том, как создавать пользовательские входные контроллеры, использовать рабочие нагрузки приложений или применять сторонние или программные пакеты или средства с открытым кодом.
  > [!NOTE]
...
* Стороннее программное обеспечение с закрытым исходным кодом. Это программное обеспечение может включать средства проверки безопасности и сетевые устройства или программное обеспечение.
* Проблемы с многооблачными или многопоставщикными сборками. Например, корпорация Майкрософт не поддерживает проблемы, связанные с запуском федеративного решения для поставщиков облачных решений.
...
> [!NOTE]
> Если компонент плоскости управления не работает в рабочем узле, команде AKS может потребоваться перезагрузить весь рабочий узел. Из-за ограниченного доступа к активной рабочей нагрузке и данным клиента команда AKS перезапускает рабочий узел только в том случае, если клиент повышает эту неполадку. Когда это возможно, команда AKS работает, чтобы предотвратить появление приложением необходимой перезагрузки.
...
### <a name="security-issues-and-patching"></a>Проблемы безопасности и установка исправлений

Если в одном или нескольких компонентах AKS обнаружен изъян в системе безопасности, Группа AKS отправит все затронутые кластеры, чтобы устранить эту неполадку. Кроме того, группа предоставит пользователям руководство по обновлению.
...
Для функций и функций, требующих расширенного тестирования и отзывов пользователей, корпорация Майкрософт выпускает новые функции предварительной версии или функции, лежащие в основе флага компонента. Рассмотрите эти возможности как предварительные или бета-версии компонентов.

Функции предварительной версии или функции-флаги не предназначены для рабочей среды. Текущие изменения в API и поведении, исправления ошибок и другие изменения могут привести к нестабильной работе кластеров и простоев.
