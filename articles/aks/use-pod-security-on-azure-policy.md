...
В этом документе предполагается, что у вас есть следующие ссылки, которые развернуты в приведенном выше примере.
...
* Azure CLI 2,12 или выше
* Кластер AKS с установленной версией 1,15 или более поздней с надстройкой политики Azure.

## <a name="overview-of-securing-pods-with-azure-policy-for-aks"></a>Общие сведения о защите модулей Pod с помощью политики Azure для AKS
...
- [Причины несоответствия](../governance/policy/how-to/determine-non-compliance.md#compliance-reasons) для этого [режима поставщика ресурсов](../governance/policy/concepts/definition-structure.md#resource-provider-modes) недоступны
...
Существует 11 встроенных индивидуальных политик Azure и двух встроенных инициатив, которые специально защищают модули Pod в кластере AKS.
...
Дополнительные пользовательские пространства имен можно исключить из оценки во время создания, обновления и аудита. Эти исключения следует использовать при наличии специализированных модулей Pod, которые выполняются в санкционированном пространстве имен и не вызывают срабатывание нарушений аудита.
