...
Чтобы создать кластер AKS, использующий Azure AD, см. статью [интеграция Azure Active Directory с AKS][aks-aad].
...
Затем создается Ролебиндинг, связывающий пользователя Azure AD *developer1\@contoso.com* с ролебиндинг, как показано в следующем манифесте YAML:
...
Когда *developer1\@contoso.com* проходит проверку подлинности в кластере AKS, они имеют полные разрешения на доступ к ресурсам в пространстве имен *Finance-App* . Таким образом, вы логически разделяете и контролируете доступ к ресурсам. RBAC Kubernetes следует использовать вместе с интеграцией с Azure AD, как описано в предыдущем разделе.

Чтобы узнать, как использовать группы Azure AD для управления доступом к ресурсам Kubernetes с помощью RBAC, см. раздел [Управление доступом к ресурсам кластера с помощью элементов управления доступом на основе ролей и удостоверений Azure Active Directory в AKS][azure-ad-rbac].
...
Сведения об использовании удостоверений Pod см. в разделе [удостоверения Azure Active Directory для приложений Kubernetes][aad-pod-identity].
