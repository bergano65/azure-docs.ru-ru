...
При работе приложений в Службе Azure Kubernetes (AKS) может потребоваться увеличить или уменьшить количество вычислительных ресурсов. При изменении числа экземпляров приложения может потребоваться изменить и число базовых узлов Kubernetes. Возможно, вам также потребуется быстро предоставить большое количество дополнительных экземпляров приложений.
...
Можно вручную масштабировать реплики (элементы pod) и узлы, чтобы проверить, как приложение реагирует на изменение доступных ресурсов и состояния. Масштабирование ресурсов вручную также позволяет задать объем используемых ресурсов (например, количество узлов), чтобы обеспечить фиксированные расходы. Для ручного масштабирования вы определяете количество реплик или узлов. Затем API Kubernetes планирует создавать дополнительные стручки или дренажные узлы на основе этой реплики или количества узлов.

При масштабировании узлов API Kubernetes вызывает соответствующий API Azure Compute, привязанный к типу вычислений, используемому кластером. Например, для кластеров, построенных на шкале VM, устанавливается логика для выбора узлов для удаления, определяемой API наборов шкалы VM. Чтобы узнать больше о том, как узлы выбираются для удаления по шкале вниз, [см.](https://docs.microsoft.com/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-faq#if-i-reduce-my-scale-set-capacity-from-20-to-15-which-vms-are-removed)
...
Когда средство горизонтального автомасштабирования pod проверяет API метрик раз в 30 секунд, предыдущие события масштабирования могут оказаться незавершенными к началу следующей проверки. Такое поведение может привести к тому, что горизонтальный автоскейлор стручка изменит количество реплик до того, как предыдущее событие масштаба может получить рабочую нагрузку приложения, и требует соответствующее изменение ресурса.

Чтобы свести к минимуму эти события гонки, устанавливаются значения охлаждения или задержки. Эти значения определяют, сколько средству горизонтального автомасштабирования pod необходимо ждать после события масштабирования, прежде можно будет активировать другое событие масштабирования. Такое поведение позволяет ввести в действие новый отсчет реплики, а API метрик и отразить распределенную рабочую нагрузку. По умолчанию задержка событий увеличения масштаба составляет 3 минуты, а задержка событий уменьшения масштаба составляет 5 минут.

В настоящее время вы не можете настроить эти значения охлаждения по умолчанию.

## <a name="cluster-autoscaler"></a>Средство автомасштабирования кластера

Чтобы реагировать на изменяющиеся требования стручка, Kubernetes имеет кластерный автоскейлор, который регулирует количество узлов на основе запрашиваемых вычислительных ресурсов в пуле узлов. По умолчанию автоскмастер кластера проверяет сервер API Metrics каждые 10 секунд на наличие необходимых изменений в коле узлов. Если средство автомасштабирования кластера определяет, что требуется внести изменение, количество узлов в кластере AKS увеличивается или уменьшается соответствующим образом. Средство автомасштабирования кластера работает кластерами AKS с поддержкой RBAC под управлением Kubernetes 1.10.x или более поздней версии.
...
Если узла не хватает вычислительных ресурсов для запуска запрошенного стручка, этот модуль не может прогрессировать в процессе планирования. Стручок не может начаться, если в пуле узлов не доступны дополнительные вычислительные ресурсы.

Когда кластерный автоскейлза замечает стручки, которые не могут быть запланированы из-за ограниченности ресурсов пула узлов, количество узлов в пуле узлов увеличивается, чтобы обеспечить дополнительные вычислительные ресурсы. Когда эти дополнительные узлы успешно развертываются и становятся доступны для использования в пуле узлов, элементы pod добавляются в расписание для запуска на них.
...
Кластерный автоскейлтакжес также отслеживает состояние планирования стручков для узлов, которые недавно не получили новых запросов на планирование. Этот сценарий указывает на то, что пул узлов имеет больше вычислительных ресурсов, чем требуется, и количество узлов может быть уменьшено.
...
ACI позволяет быстро развернуть экземпляры контейнеров без дополнительных затрат на инфраструктуру. При подключении к AKS служба ACI становится защищенным логическим расширением кластера AKS. Компонент [виртуальных узлов,][virtual-nodes-cli] основанный на [Virtual Kubelet,][virtual-kubelet]устанавливается в кластере AKS, который представляет ACI как виртуальный узло Kubernetes. После этого Kubernetes может планировать запуск элементов pod в качестве экземпляров ACI на виртуальных узлах, а не в качестве элементов pod на узлах виртуальных машин непосредственно в кластере AKS. Виртуальные узлы в настоящее время находятся в предварительном просмотре в AKS.
