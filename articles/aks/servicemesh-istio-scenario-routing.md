...
В этой статье предполагается, что у вас есть кластер AKS (Kubernetes версии `1.13` и выше с поддержкой RBAC), к которому установлено подключение `kubectl`. Также в кластере потребуется установить Istio.
