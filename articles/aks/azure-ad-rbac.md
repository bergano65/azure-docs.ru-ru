---
title: Управление ресурсами кластера с помощью RBAC и Azure AD в службе Kubernetes Azure
...
> [!TIP]
> Если появляется сообщение об ошибке, например `Principal 35bfec9328bd4d8d9b54dea6dac57b82 does not exist in the directory a5443dcd-cd0e-494d-a387-3039b419f0d5.`, подождите несколько секунд, пока идентификатор объекта группы Azure AD будет распространен через каталог, а затем повторите команду `az role assignment create`.
...
В следующем примере создается пользователь с отображаемым именем *AKS dev* и именем участника-пользователя (UPN) `aksdev@contoso.com`. Обновите имя участника-пользователя, включив в него проверенный домен для вашего клиента Azure AD (замените *contoso.com* своим доменом) и предоставьте собственные учетные данные безопасного `--password`:
...
Сначала сбросьте контекст *kubeconfig* с помощью команды [AZ AKS Get-Credential][az-aks-get-credentials] . В предыдущем разделе вы настроили контекст с помощью учетных данных администратора кластера. Пользователь с правами администратора обходит запросы на вход в Azure AD. Без параметра `--admin` применяется пользовательский контекст, требующий проверки подлинности всех запросов с помощью Azure AD.
...
В качестве запроса на вход введите учетные данные для собственной учетной записи `appdev@contoso.com`, созданной в начале статьи. После успешного входа маркер учетной записи кэшируется для будущих `kubectl` команд. Расписание NGINX успешно запланировано, как показано в следующем примере выходных данных:
...
Теперь попробуйте просмотреть модули Pod за пределами пространства имен *dev* . Снова используйте команду [kubectl Get][kubectl-get] Pod, чтобы увидеть `--all-namespaces` следующим образом:
...
Попытайтесь запланировать и просмотреть модули Pod в назначенном пространстве имен *выполняются* . При появлении запроса выполните вход с помощью собственных учетных данных `opssre@contoso.com`, созданных в начале статьи:
