...
Служба Azure Kubernetes (AKS) предоставляет управляемую службу Kubernetes, которая упрощает выполнение важнейших задач развертывания и управления, в том числе координацию обновлений. Главными узлами кластера AKS управляет платформа Azure, а вы оплачиваете работу только тех узлов AKS, где фактически выполняется ваше приложение. Служба AKS основана на использовании обработчика Службы Azure Kubernetes с открытым кодом (aks-engine).
...
Если вам требуются определенные параметры настройки для главного узла кластера или прямой доступ к ним, следует развернуть собственный кластер Kubernetes на основе [aks-engine][aks-engine].
