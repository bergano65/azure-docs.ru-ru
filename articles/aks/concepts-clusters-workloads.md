...
AKS предоставляет главный кластер с одним клиентом с выделенным сервером API, планировщиком и т. д. Вы определяете число и размер узлов, а Платформа Azure настраивает безопасное взаимодействие между главным кластером и узлами кластера. Взаимодействие с главным узлом кластера происходит через API Kubernetes, например `kubectl` или панель мониторинга Kubernetes.
