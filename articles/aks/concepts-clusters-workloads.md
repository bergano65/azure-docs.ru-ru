...
Разработка приложений все больше ориентируется на контейнеры, и значение оркестрации и управления для взаимосвязанных ресурсов непрерывно растет. Kubernetes сейчас является ведущей платформой надежного планирования рабочих нагрузок для отказоустойчивых приложений. Служба Azure Kubernetes (AKS) — это управляемая среда Kubernetes, упрощающая развертывание приложений на основе контейнеров и управление ими.
...
Служба Azure Kubernetes (AKS) предоставляет управляемую службу Kubernetes, которая упрощает выполнение важнейших задач развертывания и управления, в том числе координацию обновлений. Главными узлами кластера AKS управляет платформа Azure, а вы оплачиваете работу только тех узлов AKS, где фактически выполняется ваше приложение. Служба AKS основана на обработчике Службы контейнеров Azure с открытым кодом (acs-engine).
...
При создании кластера AKS автоматически создается и настраивается главный узел кластера. Главный узел предоставляется в формате управляемых ресурсов Azure и не привязан к пользователю. Плата за работу главного узла кластера не взимается. Оплачиваются только узлы, входящие в кластер AKS.
...
Главный узел является управляемым, а значит вам не нужно самостоятельно настраивать такие компоненты, как хранилище высокой доступности *etcd*. Но при этом вы лишаетесь возможности напрямую обращаться к главному узлу кластера. Обновления Kubernetes оркестрируются через Azure CLI или портал Azure. Первыми обновляются главные узлы кластера, а затем рядовые узлы. Для устранения возникших неполадок вы можете изучить журналы главного узла кластера с помощью Azure Log Analytics.

Если вам требуются определенные параметры настройки для главного узла кластера или прямой доступ к ним, следует развернуть собственный кластер Kubernetes на основе [acs-engine][acs-engine].
...
- *Среда выполнения контейнера* — это компонент, поддерживающий выполнение контейнерных приложений и их взаимодействие с дополнительными ресурсами, такими как виртуальная сеть и хранилище. В AKS в качестве среды выполнения контейнера используется Docker.
...
Сейчас для виртуальных машин AKS в кластере используется образ на основе Ubuntu Linux. Когда вы создаете кластер AKS или масштабируете количество узлов, платформа Azure создает и настраивает необходимое количество виртуальных машин. Вам не придется выполнять настройку вручную.

Если вы хотите использовать другую операционную систему узла, контейнерную среду выполнения или пользовательские пакеты, следует развернуть собственный кластер Kubernetes на основе [acs-engine][acs-engine]. Компоненты и параметры конфигурации становятся доступными для вышестоящего обработчика `acs-engine` раньше, чем начинается официальная поддержка в кластерах AKS. Например, если вы хотите использовать контейнеры Windows или отличную от Docker контейнерную среду выполнения, настройте и разверните с помощью `acs-engine` пользовательский кластер Kubernetes, соответствующий вашим потребностям.
...
Такое резервирование приводит к тому, что приложениям предоставляется меньше ресурсов ЦП и памяти, чем содержит сам узел. Если возникнут ограничения на ресурсы, связанные с количеством запущенных приложений, такое резервирование гарантирует доступность ЦП и памяти для основных компонентов Kubernetes. Резервирование ресурсов изменить невозможно.
...
Когда вы масштабируете или обновляете кластер AKS, выбранные действия применяются именно к пулу узлов по умолчанию. При операциях обновления все запущенные контейнеры поочередно переносятся в другие узлы в том же пуле узлов, пока все узлы не будут успешно обновлены.
