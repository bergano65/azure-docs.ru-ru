---
title: Создание контейнера Windows Server в кластере Azure Kubernetes Service (AKS)
...
ms.locfileid: "82208114"
---
# <a name="create-a-windows-server-container-on-an-azure-kubernetes-service-aks-cluster-using-the-azure-cli"></a>Создание контейнера Windows Server в кластере Azure Kubernetes Service (AKS) с помощью Azure CLI
...
Для использования контейнеров Windows Server требуется расширение CLI *AKS-Preview* версии 0.4.12 или более поздней. Установите расширение Azure CLI *AKS-Preview* с помощью команды [AZ Extension Add][az-extension-add] , а затем проверьте наличие доступных обновлений с помощью команды [AZ Extension Update][az-extension-update] :
...
* Кластер AKS может содержать не более 100 узлов в каждом пуле узлов.
...
Чтобы запустить кластер AKS, который поддерживает пулы узлов для контейнеров Windows Server, кластер должен использовать сетевую политику, которая использует подключаемый модуль сети [Azure CNI][azure-cni-about] (дополнительно). Дополнительные сведения о планировании требуемых диапазонов подсетей и сетевых факторов см. в статье [Настройка сетей Azure CNI][use-advanced-networking]. Используйте команду [AZ AKS Create][az-aks-create] , приведенную ниже, чтобы создать кластер AKS с именем *myAKSCluster*. Эта команда создает необходимые сетевые ресурсы, если они не существуют.
...
По умолчанию кластер AKS создается с пулом узлов, который может выполнять контейнеры Linux. Используйте `az aks nodepool add` команду, чтобы добавить дополнительный пул узлов, который может выполнять контейнеры Windows Server наряду с пулом узлов Linux.
...
Приведенная выше команда создает пул узлов с именем *нпвин* и добавляет его в *myAKSCluster*. При создании пула узлов для запуска контейнеров Windows Server значение по умолчанию для *node-VM-size* равно *Standard_D2s_v3*. Если вы решили задать параметр *node-VM-size* , просмотрите список [ограниченных размеров виртуальных машин][restricted-vm-sizes]. Минимальный рекомендуемый размер — *Standard_D2s_v3*. Приведенная выше команда также использует подсеть по умолчанию в виртуальной сети по `az aks create`умолчанию, создаваемой при запуске.
