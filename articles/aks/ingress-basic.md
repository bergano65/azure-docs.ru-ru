...
В этой статье показано, как развернуть [входящего контроллера NGINX][nginx-ingress] в кластер Azure Kubernetes Service (AKS). Затем в кластере AKS запускаются два приложения, каждое из которых доступно по одному IP-адресу.
