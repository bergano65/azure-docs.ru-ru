...
description: Используйте службу Azure Kubernetes (AKS) для создания и запуска задания Apache Spark для крупномасштабной обработки данных.
author: lenadroid
ms.topic: conceptual
ms.date: 10/18/2019
ms.author: alehall
...
Создайте директора службы для кластера. После его создания вам понадобится appId и пароль Службы Для следующей команды.

```azurecli
az ad sp create-for-rbac --name SparkSP
```

Создайте кластер AKS с узлами размера `Standard_D3_v2`и значениями appId и пароля, передаваемого как основные и клиентско-секретные параметры.
...
Создайте учетную запись службы, которая имеет достаточно ели разрешений для выполнения задания.
...
Для этого найдите `dockerfile` для образа Spark в каталоге `$sparkdir/resource-managers/kubernetes/docker/src/main/dockerfiles/spark/`. Добавьте `ADD` выписку для `jar` задания `WORKDIR` `ENTRYPOINT` Spark где-то между и декларациями.
