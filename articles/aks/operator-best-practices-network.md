...
> [!div class="checklist"]
> * сравнение базового и расширенного сетевых режимов в AKS;
...
**Советы и рекомендации.** Для интеграции с существующими виртуальными или локальными сетями используйте расширенные сетевые функции AKS. Эта сетевая модель также позволяет изолировать ресурсы и элементы управления в среде предприятия.
...
* **Базовая организация сети** — Azure управляет ресурсами виртуальной сети по мере развертывания кластера и использует подключаемый модуль Kubernetes [kubenet][kubenet].
* **Расширенная организация сети** — развертывание в существующей виртуальной сети с использованием подключаемого модуля Kubernetes [Azure Container Networking Interface (CNI)][cni-networking]. Группы pod получают отдельные IP-адреса, которые можно направлять к другим службам сети или локальным ресурсам.
...
Для большинства развертываемых служб в рабочей среде следует использовать расширенную организацию сети. Такая сетевая модель позволяет разделять контроль ресурсов и управление ими. С точки зрения безопасности для управления ресурсами и их защиты часто требуется задействовать разные группы сотрудников. Расширенная организация сети позволяет подключаться к существующим ресурсам Azure, локальным ресурсам или другим службам напрямую через IP-адреса, назначенные каждой группе pod.

При использовании расширенной организации сети ресурс виртуальной сети размещается в отдельной группе ресурсов относительно кластера AKS. Делегируйте разрешения субъекту-службе AKS для доступа к этим ресурсам и управления ими. Субъект-служба, используемый кластером AKS, должен иметь по крайней мере разрешения [Участник сетей](../role-based-access-control/built-in-roles.md#network-contributor) в подсети в виртуальной сети. Если вы хотите определить [пользовательскую роль](../role-based-access-control/custom-roles.md) вместо того, чтобы использовать встроенную роль участника сети, требуются следующие разрешения:
  * `Microsoft.Network/virtualNetworks/subnets/join/action`
  * `Microsoft.Network/virtualNetworks/subnets/read`

См. дополнительные сведения о [делегировании прав доступа другим ресурсам Azure в AKS][sp-delegation].

Так как каждый узел и узел pod получают собственный IP-адрес, спланируйте диапазоны адресов для подсетей AKS. Подсеть должна быть достаточно большой, чтобы предоставить IP-адреса каждому узлу, группам pod и сетевым ресурсам, которые вы развертываете. Каждый кластер AKS должен быть размещен в собственной подсети. Чтобы обеспечить возможность подключения к локальным или одноранговым сетям в Azure, не используйте диапазоны IP-адресов, которые пересекаются с существующими сетевыми ресурсами. Существуют стандартные ограничения числа групп pod, запущенных на каждом узле как при базовой, так и при расширенной организации сети. Для обработки событий масштабирования или обновления кластеров потребуются также дополнительные IP-адреса, доступные для использования в назначенной подсети.

Дополнительные сведения о планировании необходимых IP-адресов см. в руководстве по [настройке расширенных параметров сети в AKS][advanced-networking].

### <a name="basic-networking-with-kubenet"></a>Базовая организация сети с помощью Kubenet

Хотя для базовой организации сети не требуется настраивать виртуальные сети до развертывания кластера, у этого варианта есть определенные недостатки:

* Узлы и группы pod размещаются в разных IP-подсетях. Для маршрутизации трафика между группами pod и узлами применяются определяемая пользователем маршрутизация (UDR) и IP-пересылка. Такая дополнительная маршрутизация снижает производительность сети.
* Подключения к существующим локальным сетям или пиринговые подключения к другим виртуальным сетям Azure являются сложными.

Базовая организация сети подходит для небольших рабочих нагрузок при разработке или тестировании, так как не нужно создавать виртуальную сеть и подсети отдельно от кластера AKS. При работе с простыми веб-сайтами с небольшим трафиком или переносе рабочих нагрузок в контейнеры (lift-and-shift) также можно пользоваться преимуществами простых в управлении кластеров AKS, развертываемых в рамках базовой организации сети. Для большинства развертываемых служб в рабочей среде следует спланировать и применить расширенную организацию сети.
