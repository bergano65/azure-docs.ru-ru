...
* **Сети Kubenet** -Azure управляет ресурсами виртуальной сети кластера развертывается, и использует [kubenet][kubenet] подключаемый модуль Kubernetes.
* **Сети Azure CNI** — развертывание в существующей виртуальной сети и использует [сетевой интерфейс Azure контейнера (CNI)][cni-networking] подключаемый модуль Kubernetes. Группы pod получают отдельные IP-адреса, которые можно направлять к другим службам сети или локальным ресурсам.
...
Контроллер входящего трафика — это управляющая программа, которая выполняется на узле AKS и отслеживает входящие запросы. Затем трафик распределяется на основе правил, определенных в ресурсе входящего трафика. Самый распространенный контроллер входящего трафика основан на [NGINX]. AKS не накладывает ограничений для определенного контроллера, поэтому вы можете использовать другие контроллеры, такие как [профиль][contour], [HAProxy][haproxy], или [Traefik][traefik].

Входящие контроллеры необходимо запланировать на узле Linux. Узлы Windows Server (в настоящее время в предварительной версии в AKS) не следует выполнять контроллеру входящего трафика. Используйте имя узла в yaml-ФАЙЛ манифеста или развертывания диаграмма Helm для указания, что ресурс следует запускать на узле под управлением Linux. Дополнительные сведения см. в разделе [использовать селекторы узла к элементу управления, где модулей запланированы в AKS][concepts-node-selectors].
...
Подсистема балансировки нагрузки или ресурсы входящего трафика будут и работают в кластере AKS для более точного распределения трафика. Шлюзом приложений можно централизованно управлять как контроллером входящего трафика с определением ресурса. Чтобы начать работу, [создайте приложения шлюза входящий контроллер][app-gateway-ingress].
...
Чтобы приступить к работе с политиками, см. в разделе [защиты трафика между POD, содержащихся в службе Azure Kubernetes (AKS) с помощью политик сети][use-network-policies].
...
Сеть управления для узла-бастиона должна быть также защищена. Используйте [Azure ExpressRoute][expressroute] or [VPN gateway][vpn-gateway] для подключения к локальной сети и управление доступом с помощью групп безопасности сети.

## <a name="next-steps"></a>Следующие шаги

В этой статье описаны вопросы, связанные с безопасностью и подключению сетей. Дополнительные сведения об основах сети в Kubernetes см. в разделе [сети основные понятия для приложений в службе Azure Kubernetes (AKS)][aks-concepts-network]
