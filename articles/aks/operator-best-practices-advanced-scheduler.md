...
При управлении кластерами в Cлужбе Azure Kubernetes (AKS) часто возникает необходимость изолировать команды и рабочие нагрузки. Планировщик Kubernetes предоставляет расширенные функции, которые позволяют контролировать, какие стручки могут быть запланированы на определенных узлах, или как мульти-стручки приложений могут быть надлежащим образом распределены по кластеру. 
...
Эта поддержка различных узлов обеспечивается с помощью нескольких пулов узлов. Кластер AKS предоставляет один или несколько пулов узлов.
...
### <a name="behavior-of-taints-and-tolerations-in-aks"></a>Поведение поросятостей и толерантностей в АКС

При обновлении пула узлов в AKS, загрязнения и терпимости следуют установленному шаблону при применении к новым узлам:

- **Кластеры по умолчанию, которые используют виртуальные наборы масштабов машин**
  - Допустим, у вас есть двухузловый кластер - *node1* и *node2.* Вы модернизируете пул узлов.
  - Создаются два дополнительных узла, *node3* и *node4,* и пятна передаются соответственно.
  - Исходный *узла1* и *узла2* удаляются.

- **Кластеры без поддержки набора набора виртуального масштаба машины**
  - Опять же, предположим, что у вас есть двухузловый кластер - *node1* и *node2.* При обновлении создается дополнительный узла *(node3).*
  - Загрязнения от *node1* применяются к *node3,* затем *узла1* удаляется.
  - Создается еще один новый уденст (названный *node1*, так как предыдущий *уде1* был удален), и *ноуч2* наносится на новый *уде1.* Затем *узла* удаляется.
  - По сути *узло1* становится *node3,* а *node2* — *node1.*

При масштабировании пула узлов в AKS, пятна и терпимости не переносятся по конструкции.
...
В следующем примере для подобия узла задано значение *requiredDuringSchedulingIgnoredDuringExecution*. Этот параметр требует, чтобы планировщик Kubernetes использовал узел с соответствующей меткой. Если нет ни одного подходящего узла, модуль pod ожидает назначения. Чтобы стручок был запланирован на другом узлах, можно вместо этого установить значение *для предпочтительногоDuringSchedulingIgnoreDuringExecutionExecution:*
