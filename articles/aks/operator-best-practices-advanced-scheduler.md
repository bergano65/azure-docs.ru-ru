...
Эта поддержка для различных узлов предоставляется с помощью нескольких пулов узлов. Кластер AKS предоставляет один или несколько пулов узлов. Поддержка нескольких пулов узлов в AKS сейчас доступна в предварительной версии.
...
- **Кластеры по умолчанию без поддержки масштабирования виртуальных машин**
  - Предположим, что у вас есть два узла: Cluster- *NODE1* и *NODE2*. При обновлении создается дополнительный узел (*Node3*).
...
- **Кластеры, использующие масштабируемые наборы виртуальных машин**
  - Опять же, предположим, что у вас есть два узла: Cluster- *NODE1* и *NODE2*. Вы обновляете пул узлов.
