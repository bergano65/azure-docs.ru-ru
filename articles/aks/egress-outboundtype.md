...
* Во время предварительной версии `outboundType` кластеры AKS должны использовать Azure CNI. Кубенет является настраиваемым, для использования необходимо вручную сопоставить таблицу маршрутов с подсетью AKS.
* Для установки `outboundType` требуются кластеры AKS с `vm-set-type` `VirtualMachineScaleSets` и `load-balancer-sku` `Standard`.
* Для установки `outboundType` значение `UDR` требуется определяемый пользователем маршрут с допустимыми исходящими подключениями для кластера.
* При установке `outboundType` в значение `UDR` подразумевается, что входящий IP-адрес входящего трафика, направляемый в подсистему балансировки нагрузки, может **не совпадать** с исходящим адресов назначения кластера.
...
Если `userDefinedRouting` установлен, AKS не будет автоматически настраивать пути выхода. Ожидается, что **пользователь**должен выполнить следующие действия.
...
Теперь замените `APPID` и `PASSWORD` ниже идентификатором субъекта-службы и паролем субъекта-службы, созданными в результате предыдущего вывода команды. Мы будем ссылаться на идентификатор ресурса виртуальной сети, чтобы предоставить разрешения субъекту-службе, чтобы AKS мог развернуть в ней ресурсы.
...
Наконец, кластер AKS можно развернуть в существующей подсети, выделенной для кластера. Целевая подсеть для развертывания в определяется переменной среды `$SUBNETID`.
