...
Кошка черепахового сети открытым исходным кодом и решений для безопасности сети основал по Tigera, предлагает обработчик политик сети, который может реализовать правила политик сети Kubernetes. В этой статье показано, как установить обработчик Кошка черепахового сетевой политики и создания политик сети Kubernetes для управления потоком трафика между POD, содержащихся в AKS.
...
Политики сети — это ресурсы Kubernetes, которые позволяют контролировать поток трафика между контейнерами pod. Вы можете разрешить или запретить трафик на основе параметров как назначенные метки, пространство имен или трафик порта. Сетевые политики определяются как манифесты YAML. Эти политики могут быть включены в манифест шире, также создает развертывания или службы.
