...
| Дополнительные функции                      | None                       | Расширенная модель политики, состоящая из глобальной сетевой политики, глобального сета сети и конечной точки хоста. Для получения дополнительной `calicoctl` информации об использовании CLI для управления этими расширенными функциями, см. [calicoctl user reference][calicoctl] |
...
| Ведение журнала                                  | Правила, добавленные/ удаленные в IPTables, регистрируются на каждом хосте под */var/log/azure-npm.log* | Для получения дополнительной [информации][calico-logs] см. |
