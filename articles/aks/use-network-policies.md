...
* Создает кластер AKS в определенной виртуальной сети и включает сетевую политику.
    * Используется параметр политики сети *Azure* . Чтобы вместо этого использовать Калико в качестве параметра сетевой политики, используйте параметр `--network-policy calico`. Примечание. Калико можно использовать с `--network-plugin azure` или `--network-plugin kubenet`.
...
В командной строке оболочки используйте `wget`, чтобы убедиться, что вы можете получить доступ к веб-странице NGINX по умолчанию:
...
В командной строке оболочки используйте `wget`, чтобы увидеть, что политика сети теперь запрещает трафик:
...
В командной строке оболочки используйте `wget`, чтобы убедиться, что сетевая политика разрешает трафик:
