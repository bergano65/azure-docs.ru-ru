---
title: Защита трафика Pod с помощью политики сети
...
* Создает кластер AKS в определенной виртуальной сети и включает сетевую политику.
    * Используется параметр политики сети *Azure* . Чтобы вместо этого использовать Калико в качестве параметра сетевой политики, используйте `--network-policy calico` параметр. Примечание. Калико можно использовать с `--network-plugin azure` или. `--network-plugin kubenet`

Обратите внимание, что вместо использования субъекта-службы можно использовать управляемое удостоверение для разрешений. Дополнительные сведения см. в статье о том, [как использовать управляемые удостоверения](use-managed-identity.md).
