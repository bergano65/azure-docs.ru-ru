...
Ниже приведен список предварительных требований для предоставления общего доступа к данным из источника SQL. 

#### <a name="prerequisites-for-sharing-from-azure-sql-database-or-azure-synapse-analytics-formerly-azure-sql-dw"></a>Предварительные требования для совместного использования базы данных SQL Azure или Azure синапсе Analytics (ранее — хранилище Azure SQL)
Вы можете выполнить пошаговую [демонстрацию](https://youtu.be/hIE-TjJD8Dc) , чтобы настроить необходимые компоненты.

* База данных SQL Azure или аналитика Azure синапсе (ранее — хранилище Azure SQL) с таблицами и представлениями, к которым вы хотите предоставить общий доступ.
...
* Разрешение на доступ к базе данных для управляемого удостоверения общего ресурса данных. Это можно обеспечить следующим образом. 
    1. В портал Azure перейдите к SQL Server и задайте себя в качестве **администратора Azure Active Directory**.
    1. Подключитесь к базе данных или хранилищу данных SQL Azure с помощью [редактора запросов](../azure-sql/database/connect-query-portal.md#connect-using-azure-active-directory) или SQL Server Management Studio с проверкой подлинности Azure Active Directory. 
...
* Пользователь базы данных SQL Azure с доступом **"db_datareader"** для навигации и выбора таблиц и (или) представлений, к которым требуется предоставить общий доступ. 

* SQL Server доступ к брандмауэру. Это можно обеспечить следующим образом. 
    1. В портал Azure перейдите к SQL Server. В области навигации слева выберите *брандмауэры и виртуальные сети* .
...
#### <a name="prerequisites-for-sharing-from-azure-synapse-analytics-workspace-sql-pool"></a>Необходимые условия для совместного использования из пула SQL Azure синапсе Analytics (Рабочая область)

* Пул SQL Azure синапсе Analytics (Рабочая область) с таблицами, к которым вы хотите предоставить общий доступ. Совместное использование представления в настоящее время не поддерживается.
* Разрешение на запись в пул SQL в рабочей области синапсе, которая имеется в *Microsoft. синапсе/workspaces/склпулс/Write*. Это разрешение назначено роли **участника**.
* Разрешение на доступ к управляемому удостоверению общего ресурса данных для доступа к пулу синапсе рабочей области SQL. Это можно обеспечить следующим образом. 
    1. В портал Azure перейдите в рабочую область синапсе. Выберите SQL Active Directory администратор на левой панели навигации и задайте себя в качестве **администратора Azure Active Directory**.
    1. Откройте синапсе Studio и выберите *Управление* в левой области навигации. В разделе Безопасность выберите *Управление доступом* . Назначьте себя **администратору SQL** или роли **администратора рабочей области** .
    1. В синапсе Studio в области навигации слева выберите *Разработка* . Выполните следующий скрипт в пуле SQL, чтобы добавить управляемое удостоверение ресурса общих ресурсов в качестве db_datareader. 
...
* Доступ к брандмауэру рабочей области синапсе. Это можно обеспечить следующим образом. 
    1. В портал Azure перейдите в рабочую область синапсе. Выберите *брандмауэры* в области навигации слева.
    1. Щелкните **вкл** . *, чтобы разрешить службам Azure и ресурсам доступ к этой рабочей области*.
    1. Щелкните **+ Добавить IP-адрес клиента**. IP-адрес клиента можно изменить. При следующем совместном использовании данных SQL из портала Azure может потребоваться повторение этого процесса. Вы можете также добавить диапазон IP-адресов.
    1. Нажмите кнопку **Сохранить**. 
...
1. Выберите свою рабочую область SQL Server или синапсе, введите учетные данные при появлении соответствующего запроса и нажмите кнопку **Далее** , чтобы вернуться к объекту, к которому вы хотите предоставить общий доступ, и выберите "добавить наборы данных". 
...
#### <a name="prerequisites-for-receiving-data-into-azure-sql-database-or-azure-synapse-analytics-formerly-azure-sql-dw"></a>Необходимые условия для получения данных в базу данных SQL Azure или Azure синапсе Analytics (ранее — хранилище Azure SQL)
Вы можете выполнить пошаговую [демонстрацию](https://youtu.be/aeGISgK1xro) , чтобы настроить необходимые компоненты.

* База данных SQL Azure или аналитика Azure синапсе (ранее — хранилище Azure SQL).
* Разрешение на запись в базы данных на сервере SQL, которое присутствует в *Microsoft.Sql/servers/databases/write*. Это разрешение назначено роли **участника**. 
* Разрешение на управляемое удостоверение общего ресурса данных для доступа к базе данных SQL Azure или Azure синапсе Analytics. Это можно обеспечить следующим образом. 
    1. В портал Azure перейдите к SQL Server и задайте себя в качестве **администратора Azure Active Directory**.
    1. Подключитесь к базе данных или хранилищу данных SQL Azure с помощью [редактора запросов](../azure-sql/database/connect-query-portal.md#connect-using-azure-active-directory) или SQL Server Management Studio с проверкой подлинности Azure Active Directory. 
...
#### <a name="prerequisites-for-receiving-data-into-azure-synapse-analytics-workspace-sql-pool"></a>Необходимые условия для получения данных в пул SQL Azure синапсе Analytics (Рабочая область)

* Пул SQL Azure синапсе Analytics (Рабочая область).
* Разрешение на запись в пул SQL в рабочей области синапсе, которая имеется в *Microsoft. синапсе/workspaces/склпулс/Write*. Это разрешение назначено роли **участника**.
* Разрешение на управляемое удостоверение ресурса общей папки для доступа к пулу SQL рабочей области синапсе. Это можно обеспечить следующим образом. 
    1. В портал Azure перейдите в рабочую область синапсе. Выберите SQL Active Directory администратор на левой панели навигации и задайте себя в качестве **администратора Azure Active Directory**.
    1. Откройте синапсе Studio и выберите *Управление* в левой области навигации. В разделе Безопасность выберите *Управление доступом* . Назначьте себя **администратору SQL** или роли **администратора рабочей области** .
    1. В синапсе Studio в области навигации слева выберите *Разработка* . Выполните следующий скрипт в пуле SQL, чтобы добавить управляемое удостоверение ресурса общих ресурсов в качестве "db_datareader, db_datawriter, db_ddladmin". 
