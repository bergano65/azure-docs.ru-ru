...
- Вы имеете полный контроль над именами ролей; нет списка, к которому должны придерживаться ваши роли.
...
В приведенном выше примере `content-security-policy` добавляется новый заголовок, который `cache-control` изменяет значение сервера по умолчанию, а `x-dns-prefetch-control` заголовок удаляется.
