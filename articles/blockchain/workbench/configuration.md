...
| Поле | Description | Обязательно для заполнения |
|-------|-------------|:--------:|
...
| Description | Описание приложения. | Нет |
...
| Workflows | Коллекция рабочих процессов [Workflows](#workflows). Каждый рабочий процесс выступает в качестве конечного автомата для управления потоком бизнес-логики. | Да |
...
| Поле | Description | Обязательно для заполнения | Максимальная длина |
|-------|-------------|:--------:|-----------:|
| Name | Уникальное имя рабочего процесса. Для соответствующего смарт-контракта должно использоваться то же самое имя **Name**, что и для применимого класса контракта. | Да | 50 |
| DisplayName | Понятное отображаемое имя рабочего процесса. | Да | 255 |
| Description | Описание рабочего процесса. | Нет | 255 |
| Initiators | Коллекция ролей [ApplicationRoles](#application-roles). Роли, назначаемые пользователям, которым разрешено создавать контракты в рабочем процессе. | Да | |
| StartState | Имя первоначального состояния рабочего процесса. | Да | |
| properties | Коллекция [идентификаторов](#identifiers). Представляет данные, которые могут считываться вне сети или визуализируются в средстве пользователя. | Да | |
| Конструктор | Определяет входные параметры для создания экземпляра рабочего процесса. | Да | |
| Функции Azure | Коллекция [функций](#functions), которые могут выполняться в рабочем процессе. | Да | |
...
| массив    | Одномерный массив типа integer, bool, money или time. Массивы могут быть статическими или динамическими. Используйте **ElementType**, чтобы указать тип данных элементов в массиве. Ознакомьтесь с [примером конфигурации](#example-configuration-of-type-array). |
| bool     | Логический тип данных. |
| контракт | Адрес типа контракта. |
...
| Description | Описание функции. | Нет | 255 |
...
| Поле | Description | Обязательно для заполнения | Максимальная длина |
|-------|-------------|:--------:|-----------:|
| Name | Уникальное имя состояния. Для соответствующего смарт-контракта должно использоваться то же самое имя **Name**, что и для применимого состояния. | Да | 50 |
| DisplayName | Понятное отображаемое имя состояния. | Да | 255 |
| Description | Описание состояния. | Нет | 255 |
...
| Style | Визуальная подсказка, указывающая, каким является состояние: успешным или неуспешным. Есть два допустимых значения: `Success` или `Failure`. | Да | |
...
| Description | Описание перехода. | Нет |
| Функция | Имя функции, которая инициирует переход. | Да |
...
| Поле | Description | Обязательно для заполнения | Максимальная длина |
|-------|-------------|:--------:|-----------:|
| Name | Уникальное имя роли приложения. Для соответствующего смарт-контракта должно использоваться то же самое имя **Name**, что и для применимой роли. Имена базового типа зарезервированы. Роли приложения нельзя присвоить имя, которое уже используется для типа [Type](#type)| Да | 50 |
| Description | Описание роли приложения. | Нет | 255 |
...
| Description | Описание свойства или параметра. | Нет | 255 |
