...
| Поле | ОПИСАНИЕ | обязательные |
|-------|-------------|:--------:|
| ApplicationName | Уникальное имя приложения. Для соответствующего смарт-контракта должно использоваться то же самое имя **ApplicationName**, что и для применимого класса контракта.  | Yes |
...
| Поле | ОПИСАНИЕ | обязательные | Максимальная длина |
|-------|-------------|:--------:|-----------:|
| имя | Уникальное имя рабочего процесса. Для соответствующего смарт-контракта должно использоваться то же самое имя **Name**, что и для применимого класса контракта. | Yes | 50 |
| DisplayName | Понятное отображаемое имя рабочего процесса. | Yes | 255 |
...
| свойства | Коллекция [идентификаторов](#identifiers). Представляет данные, которые могут считываться вне сети или визуализируются в средстве пользователя. | Yes | |
| Конструктор | Определяет входные параметры для создания экземпляра рабочего процесса. | Yes | |
| Функции | Коллекция [функций](#functions), которые могут выполняться в рабочем процессе. | Yes | |
| States | Коллекция [состояний](#states) рабочего процесса. | Yes | |

См. [пример файла конфигурации](#configuration-file-example).

## <a name="type"></a>введите
...
Если вы определяете в конфигурации свойство как тип array, необходимо добавить явно определяемую функцию get для возвращения общедоступного свойства типа array в Solidity. Например,
...
| Поле | ОПИСАНИЕ | обязательные |
|-------|-------------|:--------:|
| parameters | Коллекция [идентификаторов](#identifiers), необходимых для инициирования смарт-контракта. | Yes |
...
| Поле | ОПИСАНИЕ | обязательные | Максимальная длина |
|-------|-------------|:--------:|-----------:|
| имя | Уникальное имя функции. Для соответствующего смарт-контракта должно использоваться то же самое имя **Name**, что и для применимой функции. | Yes | 50 |
| DisplayName | Понятное отображаемое имя функции. | Yes | 255 |
| ОПИСАНИЕ | Описание функции. | Нет | 255 |
| parameters | Коллекция [идентификаторов](#identifiers), соответствующих параметрам функции. | Yes | |
...
| Поле | ОПИСАНИЕ | обязательные | Максимальная длина |
|-------|-------------|:--------:|-----------:|
| имя | Уникальное имя состояния. Для соответствующего смарт-контракта должно использоваться то же самое имя **Name**, что и для применимого состояния. | Yes | 50 |
| DisplayName | Понятное отображаемое имя состояния. | Yes | 255 |
...
| Поле | ОПИСАНИЕ | обязательные | Максимальная длина |
|-------|-------------|:--------:|-----------:|
| имя | Уникальное имя роли приложения. Для соответствующего смарт-контракта должно использоваться то же самое имя **Name**, что и для применимой роли. Имена базового типа зарезервированы. Роли приложения нельзя присвоить имя, которое уже используется для типа [Type](#type)| Yes | 50 |
...
| Поле | ОПИСАНИЕ | обязательные | Максимальная длина |
|-------|-------------|:--------:|-----------:|
| имя | Уникальное имя свойства или параметра. Для соответствующего смарт-контракта должно использоваться то же самое имя **Name**, что и для применимого свойства или параметра. | Yes | 50 |
| DisplayName | Понятное отображаемое имя свойства или параметра. | Yes | 255 |
