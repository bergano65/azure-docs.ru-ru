---
title: Шаблоны интеграции смарт-контракта — Azure Блокчейн Workbench
...
В этом разделе рассматриваются закономерности, связанные с аспектами REST API, которые отправляют транзакции в распределенную книгу учета и шаблоны, которые запрашивают данные о транзакциях из базы данных *цепочки* Azure блокчейн Workbench.
...
Эта доставка происходит с помощью процесса, показанного ранее, где:
...
![На схеме показано изменение состояния в контракте, в результате которого событие переходит к распределенной книге. Затем блокчейн Workbench выберет событие и опубликует его.](./media/integration-patterns/transition-external-process.png)
...
![На схеме показано сообщение о P I из внешней системы, которое забирается Блокчейн Workbench через шину обслуживания. Затем блокчейн Workbench отправляет сообщение в виде транзакции в распределенную главную книгу от имени агента. Он передается в контракт, где происходит изменение состояния.](./media/integration-patterns/direct-delivery.png)
...
-   Запросы записываются в представления базы данных, чтобы упростить их использование внешними системами, службами, отчетами, инструментами для разработчиков и корпоративными инструментами повышения производительности.
