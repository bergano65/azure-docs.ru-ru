...
* План службы приложений (стандарт)
* Application Insights
* Сетка событий Azure
* Хранилище ключей Azure
* Служебная шина Azure
* База данных SQL (Standard S0) + логического сервера SQL
* Учетная запись хранения Azure (Standard-LRS)
* Масштабируемый набор с объемом, равным 1 виртуальных машин
* Группы ресурсов виртуальной сети (с помощью группа подсистемы балансировки нагрузки, сетевой безопасности, общедоступный IP-адрес виртуальной сети)
* Необязательно: Служба Azure Blockchain (по умолчанию основные B0)
...
6. Выполните основные настройки.
    ...
    | Пароль базы данных и Блокчейна | Укажите пароль, который используется для доступа к созданной базе данных в ходе развертывания. Пароль должен соответствовать три из четырех следующих требований: длина должна находиться в диапазоне от 12 & 72 символов, одну строчную букву, одну прописную букву, одну цифру и один специальный символ, не число sign(#), percent(%), запятой (,), star(*), Обратная кавычка (\`), дважды quote("), одинарные, дефис и semicolumn(;) |
...
8. В разделе **Дополнительные параметры** выберите, хотите ли вы создать новую сеть блокчейн или использовать имеющуюся сеть PoA.
    ...
    *Создавать новые* развертываются кворума служба Azure Blockchain книги с sku basic по умолчанию.

    ![Дополнительные параметры для новой сети блокчейн](media/deploy/advanced-blockchain-settings-new.png)

    | Параметр | Описание  |
    |---------|--------------|
    | Служба Azure Blockchain, Ценовая категория | Выберите **основные** или **стандартный** уровень служб Azure Blockchain, который используется для Blockchain Workbench |
    ...
    | Выбор виртуальной машины | Выберите предпочтительный производительность хранилища и размер виртуальной Машины для вашей сети блокчейнов. Выберите меньший размер виртуальной машины, например *Standard DS1 v2*, если вы используете подписку с высокими ограничениями на использование служб, например подписку Azure ценовой категории "Бесплатный". |
...
4. В манифесте также измените значение **Oauth2AllowImplicitFlow** на **true**.
