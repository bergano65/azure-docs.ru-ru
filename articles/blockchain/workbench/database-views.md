...
| UploadedByUserFirstName          | nvarchar(50)  | Yes         | Первое имя пользователя, который отправил контракт |
...
| ContractLedgerIdentifier                 | nvarchar(255) | Yes         | Уникальный идентификатор, связанный с развернутой версией смарт-контракта для указанного распределенного реестра. Например, Ethereum. |
...
| ContractActionExecutedByUserExternalId   | nvarchar(255) | Yes         | Внешний идентификатор пользователя, который выполняет действия контракта. По умолчанию этот идентификатор является guid, представляющий свою личность в консорциума Azure Active Directory. |
...
| TransactionIsWorkbenchTransaction        | bit           | Yes         | Немного, которое определяет, если транзакция является транзакцией Azure Blockchain Workbench |
...
| StateName                          | nvarchar(50)  | Yes         | Если это свойство содержит состояние контракта, это отображаемое имя для состояния. Если оно не связанно с состояние, значение будет null. |
...
| StateValue                         | nvarchar(255) | Yes         | Если это свойство содержит состояние, это значение состояния. Если оно не связанно с состояние, значение будет null. |
...
| WorkflowDescription                | nvarchar(255) | Yes         | Описание рабочего процесса |
| ContractLedgerImplementationId     | nvarchar(255) | Yes         | Уникальный идентификатор, связанный с развернутой версией смарт-контракта для указанного распределенного реестра. Например, Ethereum. |
| ContractId                         | int           | Нет           | Уникальный идентификатор контракта |
...
| StateName                          | nvarchar(50)  | Yes         | Если это свойство содержит состояние, его отображаемое имя для состояния. Если оно не связанно с состояние, значение будет null. |
| StateDisplayName                   | nvarchar(255) | Нет           | Если это свойство содержит состояние, это отображаемое имя для состояния. Если оно не связанно с состояние, значение будет null. |
| StateValue                         | nvarchar(255) | Yes         | Если это свойство содержит состояние, это значение состояния. Если оно не связанно с состояние, значение будет null. |
