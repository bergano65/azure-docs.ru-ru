...
| category  |Для службы Azure Блокчейн допустимыми значениями являются **проксилогс** и **аппликатионлогс**. |
...
| time | Дата и время (UTC) выполнения операции. |
| Идентификатора  | Ресурс службы Azure Блокчейн, для которого включены журналы.|
| category  |Для службы Azure Блокчейн значение может быть **проксилогс** и **аппликатионлогс**.  |
| operationName  | Имя операции, которую представляет это событие.   |
| Уровень журнала  | По умолчанию служба Блокчейн Azure включает **информационный** уровень ведения журнала.   |
| ноделокатион  | Регион Azure, в котором развернут член блокчейн.  |
| блоккчаинноденаме  | Имя узла члена службы Azure Блокчейн, в котором выполняется операция.   |
...
|---|---|---|---|
| Ожидающие транзакции   | Count  |  Average | Число транзакций, ожидающих осуществляет интеллектуальный анализ.   |
...
| Активные подключения  | Count  | Average  |  Текущее число активных клиентских подключений, включая ожидающие подключения.    |
|Обработанные соединения    | Count  | Sum  | Общее число обработанных соединений. Как правило, значение параметра совпадает с принятыми подключениями, если не достигнуто никаких ограничений на ресурсы.     |
...
|Ограничение памяти       |  Объем   | Average    | Максимальный объем памяти, доступный для процесса блокчейн на каждом узле. |
|Использование памяти     | Объем  |  Average | Объем используемой в среднем памяти для всех узлов.  |
| Процент использования памяти     | Процент   | Average  |  Процент используемой в среднем памяти для всех узлов.       |
|Использование хранилища      | Объем   | Average  | Количество ГБ хранилища, используемое в среднем на всех узлах.       |
