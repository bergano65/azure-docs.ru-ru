...
## <a name="open-truffle-project"></a>Открытие проекта Truffle
...
С помощью Web3 подключитесь к узлу транзакций по умолчанию и создайте учетную запись. Вы можете получить строку подключения Web3 на портале Azure.
...
1. Создайте новую учетную запись на узле транзакций по умолчанию. Замените пароль надежным паролем.
    ...
    Запишите полученный адрес учетной записи и пароль для использования в следующем разделе.
...
Добавьте код конфигурации в раздел конфигурации **module.exports**.
...
В папке **contracts** создайте файл с именем `SimpleStorage.sol`. Добавьте следующий код.
...
В папке **migrations** создайте файл с именем `2_deploy_simplestorage.js`. Добавьте следующий код.
...
| \<Адрес учетной записи\> | Адрес учетной записи, созданной в узле транзакций по умолчанию.

В этом примере начальное значение переменной **storeData** равно 42.

Переменная **privateFor** определяет узлы, которым доступен контракт. В этом примере учетная запись узла транзакций по умолчанию может передавать частную транзакцию узлу **alpha**. Вам нужно добавить открытые ключи для всех участников частной транзакции. Если не включить переменные **privateFor:** и **from:**, транзакции смарт-контракта станут общедоступными, и их смогут просматривать все участники консорциума.
...
Из-за конфиденциальной природы контракта его значения можно получить с помощью запросов только с узлов, которые объявлены в переменной **privateFor**. В этом примере мы можем отправить запрос узлу транзакций по умолчанию, так как учетная запись существует в таком узле. С помощью консоли Truffle подключитесь к узлу транзакций по умолчанию.

```bash
truffle console --network defaultnode
```

Выполните команду, которая возвращает значение экземпляра контракта.

```bash
SimpleStorage.deployed().then(function(instance){return instance.get();})
```

При успешном выполнении запроса узла транзакций по умолчанию будет возвращено значение 42.
...
Так как мы объявили открытый ключ узла **alpha** в переменной **privateFor**, мы можем отправить запрос узлу **alpha**. С помощью консоли Truffle подключитесь к узлу **alpha**.
...
При успешном выполнении запроса к узлу **alpha** будет возвращено значение 42.
...
Так как мы не объявили открытый ключ узла **beta** в переменной **privateFor**, мы не сможем отправить запрос узлу **beta** из-за конфиденциальности контракта. С помощью консоли Truffle подключитесь к узлу **beta**.
...
Выполнение запроса к узлу **beta** завершится ошибкой, так как контракт является частным.
...
Создайте файл с именем `sampletx.js`. Сохраните его в корневом каталоге проекта.

Этот скрипт задает для переменной **storedData** значение 65. Добавьте код в новый файл.
...
С помощью Truffle выполните скрипт для узла транзакций по умолчанию.
...
При успешном выполнении транзакции будет возвращено значение 65.
...
Из-за конфиденциальности транзакций их можно выполнять только на тех узлах, которые были объявлены в переменной **privateFor**. В этом примере мы можем выполнить транзакции, так как мы объявили открытый ключ узла **alpha** в переменной **privateFor**. С помощью Truffle выполните транзакцию на узле **alpha**.
