...
1. Укажите параметры для участника Блокчейна Azure.
    ...
    Регион | Выберите регион для создания группы ресурсов. Все участники консорциума должны находится в одном расположении. Доступные расположения для развертывания: *westeurope, eastus, southeastasia, westeurope, northeurope, westus2* и *japaneast*. Компоненты могут быть недоступны в некоторых регионах. Диспетчер данных блокчейна Azure доступен в таких регионах Azure: восточная часть США и Западная Европа.
    Имя участника блокчейн-сети | Выберите уникальное имя для участника в службе "Блокчейн Azure". Имя участника блокчейн-сети может содержать только строчные буквы и цифры. Первый символ должен быть буквой. Значение должно иметь длину от 2 до 20 символов.
    ...
    Пароль участника | Пароль для узла транзакций по умолчанию участника. Этот пароль используется для обычной проверки подлинности при подключении к общедоступной конечной точке узла транзакций участника блокчейна.
    Пароль учетной записи для управления консорциумом | Пароль учетной записи консорциума используется для шифрования закрытого ключа для учетной записи Ethereum, созданного для вашего участника. Он используется для управления консорциумом.
    ...
    Имя SKU | Настройка и стоимость узла для новой службы. Используйте **B0** для выбора ценовой категории "Базовый", а **S0** — для выбора ценовой категории "Стандартный".
    Расположение | Выберите расположение для создания участника. По умолчанию для выбора расположения группы ресурсов используется `[resourceGroup().location]`. Все участники консорциума должны находится в одном расположении. Доступные расположения для развертывания: *westeurope, eastus, southeastasia, westeurope, northeurope, westus2* и *japaneast*. Компоненты могут быть недоступны в некоторых регионах. Диспетчер данных блокчейна Azure доступен в таких регионах Azure: восточная часть США и Западная Европа.

1. Чтобы проверить и развернуть шаблон, выберите **Просмотр и создание**.
