...
* Установите последнюю [Azure CLI](https://docs.microsoft.com/cli/azure/install-azure-cli) и войдите с помощью `az login`.
...
Экземпляр Диспетчер данных Блокчейн отслеживает узел транзакции службы Блокчейн Azure. Экземпляр регистрирует все необработанные данные о блоках и транзакциях из узла транзакций. Блокчейн Диспетчер данных публикует сообщение **равблоккандтрансактионмсг** , которое представляет собой надмножество информации, возвращенной из запросов Web3. ETH- [Block](https://web3js.readthedocs.io/en/v1.2.0/web3-eth.html#getblock) и- [Transaction](https://web3js.readthedocs.io/en/v1.2.0/web3-eth.html#gettransaction) .
...
Пример конфигурации JSON для создания входного ресурса в регионе " *Восточная часть США* ", подключенного \<к\>элементу блокчейн.
...
| resourceId | Узел транзакции, к которому подключены входные данные. Замените \<идентификатор\>подписки, \<группу\>ресурсов и \<член\> блокчейн значениями для ресурса узла транзакции. Вход подключается к узлу транзакции по умолчанию для члена службы Azure Блокчейн. |
...
Пример конфигурации JSON для создания выходного ресурса в регионе *восточной части США* , подключенного к разделу сетки событий с \<именем "Сетка\>событий".
...
| resourceId | Ресурс, к которому подключены выходные данные. Замените \<идентификатор\>подписки, \<группу\>ресурсов и \<элемент\> блокчейн значениями для ресурса сетки событий. |
...
| ids | Идентификатор ресурса наблюдателя. Замените \<идентификатор\>подписки, \<группу\>ресурсов и \<имя\> наблюдателя значениями для ресурса наблюдателя.|
...
| ids | Имя наблюдателя. Замените \<идентификатор\>подписки, \<группу\>ресурсов и \<имя\> наблюдателя значениями для ресурса наблюдателя. |
