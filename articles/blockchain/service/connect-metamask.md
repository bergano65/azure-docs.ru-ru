...
1. Выберите **Запуск**. 
    ...
    В MetaMask для вашей **среды** устанавливается значение **Injected Web3**, а для **учетной записи** — имя сети.
...
1. Чтобы скомпилировать контракт, выберите **Compile > Start to compile** (Компиляция > Начать компиляцию). После успешной компиляции отображается зеленое поле с именем контракта.

    ![Компиляция](./media/connect-metamask/compile.png)

1. Чтобы реализовать контракт, выберите вкладку **Запуск**. Выберите контракт **simple** и щелкните **Deploy** (Развернуть).

    ![Настраиваемая сеть RPC](./media/connect-metamask/deploy.png)
...
1. Чтобы выполнить транзакцию **add** с блокчейном, введите число, которое нужно добавить, и нажмите кнопку **add**.
...
1. Выберите **Confirm** (Подтвердить) для выполнения транзакции с блокчейном.
...
При работе с этим кратким руководством вы подключились к узлу транзакций службы "Блокчейн Azure", развернули смарт-контракт и отправили транзакцию в блокчейн с помощью расширения браузера MetaMask. Ознакомьтесь со следующим руководством, чтобы узнать, как развернуть и отправить транзакцию с помощью Truffle.
