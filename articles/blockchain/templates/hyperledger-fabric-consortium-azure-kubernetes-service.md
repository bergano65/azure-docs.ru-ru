...
```
Эта команда требует времени для загрузки всех пакетов. После успешного выполнения команды можно увидеть `node_modules` папку в текущем каталоге. Все необходимые пакеты загружаются в `node_modules` папку.

### <a name="generate-connection-profile-and-admin-profile"></a>Создать профиль подключения и профиль администратора

Создать `profile` каталог внутри `app` папки
...
Он создаст профиль подключения и администратора `profile` Организации в папке Profile с именем `<orgname>-ccp.json` и `<orgname>-admin.json` соответственно.
...
```
Приведенная выше команда выполняет Импортадмин. js для импорта удостоверения пользователя администратора в бумажник. Сценарий считывает удостоверение администратора из профиля `<orgname>-admin.json` администратора и импортирует его в бумажник для выполнения операций хлф.

Скрипты используют бумажник файловой системы для хранения удостоверений. Он создает бумажник в соответствии с путем, указанным в поле ". бумажник" в профиле подключения. По умолчанию поле ". бумажник" инициализируется с помощью `<orgname>`, а это означает, что `<orgname>` папка с именем создается в текущем каталоге для хранения удостоверений. Если вы хотите создать бумажник на другом пути, измените поле ". бумажник" в профиле подключения перед выполнением команды "регистрация пользователя администратора" и любых других операций ХЛФ.
...
- Задайте `ORGNAME` для `<peerOrg1Name>` параметра значение `installCC` и выдать команду.
- Задайте `ORGNAME` для `<peerOrg2Name>` параметра значение `installCC` и выдать команду.
...
```
Передайте имя функции создания экземпляра и список аргументов, разделенных `<instantiateFunc>` запятыми, в и `<instantiateFuncArgs>` соответственно. Например, в [фабркар чаинкоде](https://github.com/hyperledger/fabric-samples/blob/release/chaincode/fabcar/fabcar.go), чтобы создать экземпляр чаинкоде `<instantiateFunc>` со `"Init"` значением и `<instantiateFuncArgs>` в пустую строку. `""`
...
```
Передайте имя функции вызова и список аргументов, разделенных `<invokeFunction>` запятыми, в и `<invokeFuncArgs>` соответственно. Продолжим работу с примером чаинкоде фабкар, чтобы вызвать функцию `<invokeFunction>` инитледжер `"initLedger"` со `<invokeFuncArgs>` значением `""`и до.
...
Передайте имя функции запроса и список аргументов, разделенных `<queryFunction>` запятыми, в и `<queryFuncArgs>` соответственно. Опять же, `fabcar` выполнив чаинкоде как ссылку, можно запросить все автомобили в мировом состоянии `<queryFunction>` , `"queryAllCars"` для `<queryArgs>` которых `""`задано значение и.
