...
Вариант | Модель службы | Общий вариант использования
-------|---------------|-----------------
Шаблоны решений | IaaS | Шаблоны решений — это Azure Resource Manager шаблоны, которые можно использовать для предоставления полностью настроенной топологии сети блокчейн. Шаблоны развертывают и настраивают Microsoft Azure службы вычислений, сети и хранения для заданного типа сети блокчейн.
...
[Azure Блокчейн Workbench](../workbench/overview.md) | IaaS и PaaS | Azure Blockchain Workbench (предварительная версия) — это совокупность служб и возможностей Azure, которые позволяют создавать и развертывать приложения блокчейн для совместного использования бизнес-процессов и данных с другими организациями. Используйте Azure Блокчейн Workbench для создания прототипа решения блокчейн или подтверждения концепции приложения блокчейн.
...
Subscription | Подписка, для которой необходимо развернуть сеть консорциума |
...
F16s|Диск SSD категории "Премиум"|high|high|low
...
Дополнительные параметры | Расширенные параметры Ethereum | Включение
...
Параметр | Description | Пример значения
----------|-------------|--------------
Мониторинг | Параметр для включения мониторинга | Включение
...
Идентификатор существующей рабочей области log Analytics (подключитесь к существующим Azure Monitor журналов = присоединение к существующему)|Идентификатор существующего экземпляра журналов Azure Monitor||Нет данных
Первичный ключ существующего log Analytics (подключение к существующим журналам Azure Monitor: соединение с существующим)|Первичный ключ, используемый для подключения к существующему экземпляру журналов Azure Monitor||Нет данных
...
Децентрализованное приложение системы управления — это набор предварительно развернутых [смарт-контрактов](https://github.com/Azure-Samples/blockchain/tree/master/ledger/template/ethereum-on-azure/permissioning-contracts) и веб-приложение, с помощью которого осуществляется управление центрами сертификации сети. Центры разбиваются на удостоверения администраторов и проверяющие узлы.
...
## Разработка Ethereum<a id="tutorials"></a>
...
* Установите `npm install -g truffle@v5.0.5`Труффле v 5.0.5. Для работы с Truffle нужно установить несколько инструментов, включая [Node.js](https://nodejs.org) и [Git](https://git-scm.com/). Дополнительные сведения см. в [документации по труффле](https://github.com/trufflesuite/truffle).
...
1. Измените каталог на новую папку `HelloWorld`.
1. Инициализируйте новый проект Труффле, используя командную `truffle init`.
...
1. Создайте файл в именованном `postBox.sol` в подкаталоге **Contracts** проекта труффле.
...
1. Так как мы используем поставщик бумажника Труффле HD, установите модуль в проекте с помощью команды `npm install truffle-hdwallet-provider --save`.
...
1. Добавьте новую миграцию, чтобы развернуть новый контракт. Создайте файловый `2_deploy_contracts.js` в подкаталоге **migrations** проекта труффле.
...
1. Добавьте следующее содержимое в **сендтрансактион. js**.
...
Дополнительные решения Azure Блокчейн см. в [документации по Azure блокчейн](https://docs.microsoft.com/azure/blockchain/).
