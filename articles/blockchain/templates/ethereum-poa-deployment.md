...
Для выполнения каких-либо транзакций с помощью децентрализованного приложения системы управления вам необходимо использовать кошелек Ethereum.  Самый простой подход — использовать кошелек в браузере, например [MetaMask](https://metamask.io). Однако, учитывая, что это смарт-контракты, развернутые в сети, вы также можете автоматизировать свои взаимодействия с контрактом системы управления.

После установки MetaMask перейдите в децентрализованное приложение системы управления в браузере.  Вы можете найти URL-адрес в сообщении электронной почты о подтверждении развертывания или в выходных данных развертывания на портале Azure.  Если у вас нет установленного в браузере кошелька, вы не сможете выполнить никаких действий, однако все равно сможете прочитать состояние администратора.  

#### <a name="becoming-an-admin"></a>Как стать администратором
Если вы первый участник, который выполнил развертывание в сети, вы автоматически становитесь администратором, а ваши равноправные узлы будут указаны в списке как проверяющие элементы управления.  При присоединении к сети большинство (более 50 %) имеющегося состава администраторов должны отдать свои голоса за вас как за администратора.  Если вы не хотите становиться администратором, то узлы по-прежнему будут синхронизировать и проверять блокчейн, но без участия в процессе создания блока. Чтобы начать процесс голосования за получение прав администратора, щелкните __Номинация__ и введите адрес и псевдоним Ethereum.
...
На вкладке __Администраторы__ отображается текущий набор администраторов, а также предоставляется возможность проголосовать против них.  Как только администратор потеряет более 50 % голосов, его роль администратора будет удалена из сети.  Все узлы проверяющего элемента управления, которыми владеет этот администратор, потеряют состояние проверяющего элемента управления и станут узлами транзакций в сети.  Администратора могут удалить по ряду причин, однако консорциум должен заранее согласовать политику.
...
При выборе вкладки __Validators__ (Проверяющие элементы управления) в меню в левой области появятся текущие развернутые узлы равенства для этого экземпляра и их текущее состояние (тип узла).  Обратите внимание, что каждому участнику консорциума будет доступен разный набор проверяющих элементов управления в этом списке, так как это представление отображает текущего участника развернутого консорциума.  Если это новый развернутый экземпляр и вы еще не добавили проверяющие элементы управления, вам будет предложено использовать параметр "Add Validators" (Добавить проверяющие элементы управления).  При выборе этого параметра будет автоматически выбран регионально сбалансированный набор узлов равенства, которые и будут назначены набору проверяющих элементов управления.  Если вы развернули больше узлов, чем позволяет доступная емкость, остальные узлы станут узлами транзакций в сети.
...
В правом верхнем углу находится псевдоним и идентикон учетной записи Ethereum.  Если вы являетесь администратором, вам доступна возможность обновить свой псевдоним.
...
Обратите внимание, что виртуальная машина и уровень хранилища повлияют на производительность сети.  Мы рекомендуем следующие номера SKU в зависимости от требуемой эффективности затрат:
...
В колонке мониторинга вы можете настроить ресурс Log Analytics сети. Агент мониторинга будет собирать и оценивать полезные метрики и журналы из сети, предоставляя возможность быстро проверять работоспособность сети и заниматься устранением неполадок.

  Имя параметра|ОПИСАНИЕ|Допустимые значения|Значения по умолчанию
  ---|---|---|---
Мониторинг|Параметр включения мониторинга|"Включить" или "Отключить"|Включение
Connect to existing Log Analytics (Подключиться к имеющемуся экземпляру Log Analytics)|Создание нового экземпляра Log Analytics или присоединение существующего экземпляра|"Создать" или Join existing (Присоединить имеющийся)|Создание
Расположение мониторинга (Connect to existing Log Analytics (Подключиться к имеющемуся экземпляру Log Analytics) = "Создать")|Регион, куда будет развернут новый экземпляр Log Analytics.|Все регионы Log Analytics|Нет данных
Existing Log Analytics Workspace Id (Идентификатор имеющейся рабочей области Log Analytics) (Connect to existing Log Analytics = Join Existing (Подключиться к имеющемуся экземпляру Log Analytics = Присоединить имеющийся))|Идентификатор рабочей области имеющегося экземпляра Log Analytics||Нет данных
Existing Log Analytics Primary Key (Первичный ключ имеющегося экземпляра Log Analytics) (Connect to existing Log Analytics = Join Existing (Подключиться к имеющемуся экземпляру Log Analytics = Присоединить имеющийся))|Первичный ключ, с помощью которого выполняется подключение к имеющемуся экземпляру Log Analytics.||Нет данных
...
Если используется развертывание на основе общедоступного IP-адреса, это сделать просто. Если используется развертывание за VPN, сведения о подключении сети (как часть развертывания нового участника) см. в разделе о [подключении шлюза виртуальной сети](#connecting-vnet-gateways).  После завершения развертывания используйте [децентрализованное приложение системы управления](#governance-dapp), чтобы стать администратором сети.
...
Вы можете проигнорировать этот шаг, если развертывание было выполнено с помощью параметров по умолчанию общедоступного IP-адреса. Если сеть частная, ее участники используют разные подключения шлюза виртуальной сети. Прежде чем участник сможет присоединиться к сети и просматривать трафик транзакций, имеющийся участник должен выполнить окончательную настройку на своем VPN-шлюзе, чтобы принимать подключения. Это означает, что узлы Ethereum присоединяемого участника не будут работать до установки подключения. Советуем создавать избыточные сетевые подключения (одноранговая сеть) в консорциуме, чтобы уменьшить вероятность единой точки отказа.
...
По умолчанию доступ к портам SSH запрещен правилом безопасности сетевой группы. Это сделано по соображениям безопасности. Чтобы получить доступ к экземплярам виртуальных машин в сети PoA, значение этого правила следует изменить на \"Разрешить\".
