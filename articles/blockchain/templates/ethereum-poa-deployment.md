...
Для тех, кто не знаком с блокчейн-сообществом, выпуск этого решения — отличная возможность познакомиться с этой технологией в простой и настраиваемой среде Azure. Proof-of-work (POW) — это механизм защиты от атак Сивиллы, который автоматически регулирует сеть и разрешает участие на основе вычислительных затрат. Лучше всего его применять в анонимных открытых сетях блокчейнов, в которых майнинг криптовалюты может влиять на защиту. Однако в частных сетях и сетях консорциумов эфир не имеет ценности. Другой протокол, Подтверждение полномочия, более подходящий для разрешенных сетей, где известны все участники консенсус и надежными. Не требующий майнинга в привычном его понимании, этот механизм более эффективен. Однако подход к византийской отказоустойчивости по-прежнему сохраняется.

### <a name="consortium-governance"></a>Управление консорциумом

Так как доказательство записи зоны полагается на списком разрешенных сетевых центров, Поддерживайте работоспособность сети, очень важно для обеспечения справедливого вносить изменения в этот список разрешений. Каждое развертывание поставляется с набором смарт контрактов и портал для управления блокчейна список разрешенных. Когда за предложенное изменение проголосует большинство участников консорциума, изменение задействуется. Этот механизм позволяет добавлять новых участников консенсуса или удалять скомпрометированных прозрачным образом, что способствует формированию открытой сети.

### <a name="admin-account"></a>Учетная запись администратора

Во время развертывания узлов подтверждения записи зоны запрашивается для адреса Ethereum администратора. Создать и защитить эту учетную запись Ethereum можно с помощью нескольких различных механизмов. Добавив этот адрес как удостоверение сети, вы сможете выполнять с помощью этой учетной записи задачи администрирования. Кроме того, эта учетная запись администратора позволяет делегировать участие в консенсусе на узлы проверяющего элемента управления, которые создаются в рамках этого развертывания. Так как используется только общедоступный адрес Ethereum, каждый администратор может легко для защиты закрытых ключей способом, который соответствует модели их нужно было безопасности.
...
Поскольку каждый член будет иметь несколько узлов проверяющий элемент управления, работающих одновременно, и каждый узел должен иметь допустимое удостоверение, очень важно, что проверяющие элементы управления можно безопасно получить уникальное удостоверение active в сети. Чтобы облегчить эту задачу, мы создали Store удостоверений, который разворачивается в подписке каждого члена, безопасно содержит созданный Ethereum удостоверений. После развертывания контейнера оркестрации будет создание Ethereum закрытого ключа для каждого проверяющего элемента управления и сохранять их в хранилище ключей Azure. Перед запуском узел равенства сначала получает аренду на неиспользуемое удостоверение, чтобы убедиться, что его не выберет другой узел. Это удостоверение предоставляется клиенту, который наделяет его полномочиями для создания блоков. Если на виртуальной машине размещения произошел сбой, аренда удостоверения освобождается, за счет чего его сможет получить другой узел.

### <a name="bootnode-registrar"></a>Регистратор Bootnode

Чтобы обеспечить простоту подключения, на [конечной точке API данных](#data-api) каждого участника размещается набор сведений о соединении. Эти данные включают список bootnodes, которые предоставляются как узлы пиринга для присоединяемого члена. Периодически мы обновляем этот список загрузочных узлов, входящий в состав API данных.

### <a name="bring-your-own-operator"></a>Использование собственного оператора

Зачастую участники консорциума хотят участвовать в управлении сети, но не хотят настраивать и обслуживать свою инфраструктуру. В отличие от традиционных систем использование одного оператора в сети имеет отрицательный эффект для децентрализованной модели блокчейн-систем. Вместо найма централизованного посредника для управления сетью каждый участник консорциума может делегировать управление инфраструктурой любому оператору по своему выбору. Это позволяет гибридной модели, где каждый элемент можно работать с собственной инфраструктуры или делегировать операции к другому партнеру. Рабочий процесс делегированной операции выглядит следующим образом:
...
Для выполнения любых операций по управлению DApp, необходимо использовать Ethereum wallet.  Самый простой подход — использовать кошелек в браузере, например [MetaMask](https://metamask.io). Однако, учитывая, что это смарт-контракты, развернутые в сети, вы также можете автоматизировать свои взаимодействия с контрактом системы управления.

После установки MetaMask перейдите в децентрализованное приложение системы управления в браузере.  Вы можете найти URL-адрес в сообщении электронной почты о подтверждении развертывания или в выходных данных развертывания на портале Azure.  Если у вас в браузере wallet установки вы не сможете выполнять любые действия; Тем не менее по-прежнему можно считывать состояние администратора.  

#### <a name="becoming-an-admin"></a>Как стать администратором
Если вы первый элемент, который развернут в сети, затем будет автоматически стать администратором, и четность узлов будет указано в качестве проверяющих элементов управления.  Если вы присоединяетесь к сети, необходимо получить одобрен администратор большинством (более 50%) существующего набора администратора.  Если вы не хотите становиться администратором, то узлы по-прежнему будут синхронизировать и проверять блокчейн, но без участия в процессе создания блока. Чтобы начать процесс голосования за получение прав администратора, щелкните __Номинация__ и введите адрес и псевдоним Ethereum.

![Назначение](./media/ethereum-poa-deployment/governance-dapp-nominate.png)

#### <a name="candidates"></a>Кандидаты
Перейдя на вкладку __Кандидаты__, вы увидите текущий набор администраторов-кандидатов.  Как только кандидат получит большинство голосов текущих администраторов, он получит повышение до администратора.  Чтобы проголосовать за кандидата, выберите строку и щелкните "Vote in" (Выбрать) в верхней области.  Если вы передумаете во время голосования, можно выбрать кандидата и щелкнуть "Rescind vote" (Отменить выбор).

![Кандидаты](./media/ethereum-poa-deployment/governance-dapp-candidates.png)


#### <a name="admins"></a>Администраторы
На вкладке __Администраторы__ отображается текущий набор администраторов, а также предоставляется возможность проголосовать против них.  После администратор теряет более чем 50% поддержка, они будут удалены как администратор в сети.  Все узлы проверяющего элемента управления, которыми владеет этот администратор, потеряют состояние проверяющего элемента управления и станут узлами транзакций в сети.  Администратор может быть удален по различным причинам. Тем не менее именно консорциума согласовать политику, заранее.
...
При выборе вкладки __Validators__ (Проверяющие элементы управления) в меню в левой области появятся текущие развернутые узлы равенства для этого экземпляра и их текущее состояние (тип узла).  Каждый член консорциума будут иметь другой набор проверяющих элементов управления в этом списке, так как это представление представляет текущий элемент развернутой консорциума.  Если вы еще не добавили вашей проверяющие элементы управления, это вновь развернутый экземпляр, отобразится параметр для «Добавление проверяющих элементов управления».  При выборе этого будет автоматически выбрать регионально Сбалансированный набор узлов с контролем четности и назначить их в набор проверяющего элемента управления.  Если вы развернули больше узлов, чем позволяет доступная емкость, остальные узлы станут узлами транзакций в сети.
...
В правом верхнем углу находится псевдоним и идентикон учетной записи Ethereum.  Если вы являетесь администратором вы получите возможность изменить ваш псевдоним.
...
Адрес электронной почты (необязательно)|После завершения развертывания вы получите уведомление по электронной почте с информацией об этом процессе.|Допустимый адрес электронной почты|Н/Д
Имя пользователя виртуальной машины|Имя пользователя администратора каждой развернутой виртуальной машины (только буквенно-цифровые символы)|1–64 символов|Н/Д
...
Пароль (тип проверки подлинности = пароль)|Пароль учетной записи администратора для каждой развернутой виртуальной машины.  Пароль должен содержать три из следующих элементов: одна строчная буква, одна прописная буква, одна цифра и один специальный знак. Хотя все виртуальные машины изначально имеют одинаковый пароль, вы можете изменить пароль после инициализации.|12–72 символов|Н/Д
Ключ SSH ("Тип аутентификации" = "Открытый ключ")|Ключ защищенной оболочки, используемый для удаленного входа.||Н/Д
Подписка|Подписка, для которой необходимо развернуть сеть консорциума||Н/Д
Группа ресурсов|Группа ресурсов, куда будет развертываться сеть консорциума.||Н/Д
Размещение|Регион Azure для группы ресурсов.||Н/Д
...
Ниже приведено подробное описание каждого параметра.
  ...
  Первый регион|Первый регион для развертывания сети консорциума|Все разрешенные регионы Azure|Н/Д
  Второй регион|Второй регион развертывания сети консорциума (отображается только в том случае, если для количества регионов указано значение 2)|Все разрешенные регионы Azure|Н/Д
  Третий регион|Третий регион развертывания сети консорциума (отображается только в том случае, если для количества регионов указано значение 3)|Все разрешенные регионы Azure|Н/Д
  Четвертый регион|Четвертый регион развертывания сети консорциума (отображается только в том случае, если для количества регионов указано значение 4)|Все разрешенные регионы Azure|Н/Д
  Пятый регион|Пятый регион развертывания сети консорциума (отображается только в том случае, если для количества регионов указано значение 5)|Все разрешенные регионы Azure|Н/Д
...
Виртуальная машина и уровень хранилища повлияет на производительность сети.  Мы рекомендуем следующие номера SKU в зависимости от требуемой эффективности затрат:

  Номер SKU виртуальной машины|Уровень хранилища|Цена|Пропускная способность|Задержка
  ---|---|---|---|---
  F1|SSD (цен. категория "Стандартный")|низкая|низкий|высокий
  D2_v3|SSD (цен. категория "Стандартный")|средняя|средняя|средняя
  F16s|SSD (цен. категория "Премиум")|высокий|высокий|низкий
...
Ниже приведено подробное описание каждого параметра.
  ...
  Имя параметра|Описание|Допустимые значения|Значения по умолчанию
  ---|---|---|---
Consortium Member ID (Идентификатор участника консорциума)|Идентификатор, связанный с каждым участником сети консорциума, который используется для настройки пространства IP-адресов, чтобы избежать конфликтов. Если сеть частная, идентификатор участника должен быть уникальным среди разных организаций в одной сети.  Уникальный идентификатор участника необходим даже в том случае, когда та же организация развертывает участников в нескольких регионах. Запишите значения этого параметра, так как необходимо поделиться им с другими участниками соединения, чтобы убедиться, что нет конфликта.|0–255|Н/Д
...
Admin Ethereum Address (Адрес администратора Ethereum)|Адрес учетной записи Ethereum, которая используется в системе управления PoA.  Мы советует создавать адрес Ethereum с помощью MetaMask.|42 буквенно-цифровых символа, начиная с 0|Н/Д
Дополнительные параметры|Расширенные параметры Ethereum|"Включить" или "Отключить"|Отключить
...
Transaction Permission Contract (Контракт разрешения транзакций) ("Расширенные параметры" = "Включить")|Байт-код контракта разрешения транзакций. Ограничивает развертывание смарт-контрактов и выполнение в список разрешенных Ethereum учетных записей.|Contract bytecode (Байт-код контракта)|Н/Д

Ниже приведен пример развертывания: ![ethereum параметры](./media/ethereum-poa-deployment/ethereum-settings.png)

#### <a name="monitoring"></a>Наблюдение

Колонка "Мониторинг" можно настроить ресурс журналы Azure Monitor для вашей сети. Агент мониторинга будет собирать и оценивать полезные метрики и журналы из сети, предоставляя возможность быстро проверять работоспособность сети и заниматься устранением неполадок.

[!INCLUDE [azure-monitor-log-analytics-rebrand](../../../includes/azure-monitor-log-analytics-rebrand.md)]

  Имя параметра|Описание|Допустимые значения|Значения по умолчанию
  ---|---|---|---
Наблюдение|Параметр включения мониторинга|"Включить" или "Отключить"|Включить
Подключаться к существующим журналам Azure Monitor|Создать новый экземпляр журналы Azure Monitor или присоединиться к существующему экземпляру|"Создать" или Join existing (Присоединить имеющийся)|Создать
Отслеживать расположение (подключение к существующим журналам Azure Monitor — создайте новый)|Регион, где новый монитор Azure регистрирует экземпляр будет развертываться.|Все Azure Monitor регистрирует регионов|Н/Д
Идентификатор рабочей области существующая log analytics (подключение к существующим журналам Azure Monitor = присоединиться к существующей)|Идентификатор рабочей области из существующей Azure Monitor регистрирует экземпляр||Н/Д
Первичный ключ существующая log analytics (подключение к существующим журналам Azure Monitor = присоединиться к существующей)|Первичный ключ, используемый для подключения в существующий экземпляр журналы Azure Monitor||Н/Д
...
После завершения развертывания, пользователю доступны необходимые параметры с помощью подтверждение по электронной почте или через портал Azure. В этих параметрах доступны такие данные:
...
После успешного завершения развертывания и подготовлены все ресурсы в группе ресурсов можно просмотреть выходные параметры.
...
Если используется развертывание на основе общедоступного IP-адреса, это сделать просто. Если развертывание за виртуальную частную сеть, см. в разделе [подключение шлюза виртуальной сети](#connecting-vnet-gateways) сделать сетевое подключение как часть развертывания нового члена.  После завершения развертывания используйте [децентрализованное приложение системы управления](#governance-dapp), чтобы стать администратором сети.
...
Вы можете проигнорировать этот шаг, если развертывание было выполнено с помощью параметров по умолчанию общедоступного IP-адреса. Если сеть частная, ее участники используют разные подключения шлюза виртуальной сети. Перед членом можно подключиться к сети и трафике транзакций см. в разделе, существующего члена необходимо выполнить до окончательной конфигурации на своих VPN-шлюза должен принимать подключения. Это означает, что узлы Ethereum присоединяемого члена не будет работать до установления соединения. Рекомендуется для создания избыточных сетевых подключений (сетка) в consortium, чтобы уменьшить вероятность единой точкой отказа.
...
Чтобы завершить подключение, имеющийся участник должен выполнить приведенный ниже сценарий PowerShell. Мы рекомендуем с помощью Azure Cloud Shell, которая находится на панели навигации сверху справа на портале.
...
По умолчанию доступ к портам SSH запрещен правилом безопасности сетевой группы. Это сделано по соображениям безопасности. Для доступа к экземпляров виртуальных машин в сети PoA, вам потребуется изменить это правило к \"разрешить\"
...
После развертывания, затем выберите экземпляр в группе ресурсов. DNS-имя доступа к диспетчеру трафика отображается на вкладке "Обзор".
...
Повторите эти шаги для каждого региона в развернутой сети. После определения конечных точек \"включена\" состояния, они будут автоматически загружаются и региона с балансировкой на DNS-имя диспетчера трафика. Теперь вы можете использовать это DNS-имя и параметр \[CONSORTIUM\_DATA\_URL\] на других шагах документации.

### <a name="data-api"></a>API данных

Каждый участник консорциума размещает информацию, на основе которой выполняется подключение к сети. Существующего члена предоставит [CONSORTIUM_DATA_URL] перед его развертыванием. После развертывания присоединяемый участник получит сведения из интерфейса JSON на следующей конечной точке:

`<CONSORTIUM_DATA_URL>/networkinfo`

Ответ будет содержать сведения, полезные для присоединения к члены (происхождения блок, проверяющий элемент управления задать контракт ABI, bootnodes) и сведения, полезные для существующего члена (адреса проверяющего элемента управления). Мы советуем расширить консорциум несколькими поставщиками облачных служб, используя такую стандартизацию. Этот API возвращает ответ в формате JSON со следующей структурой:
```json
{
  "$id": "",
  "type": "object",
