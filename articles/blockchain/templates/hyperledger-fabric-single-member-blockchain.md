...
Сеть состоит из узлов трех типов.

1. **Узел участника**. Узел, на котором запущена служба членства в Fabric, которая регистрирует участников сети и управляет ими. Этот узел может быть помещен в кластер для повышения масштабируемости и уровня доступности, но в этой лаборатории мы используем только один узел участника.
...
Чтобы начать, щелкните **Hyperledger Fabric Single Member Blockchain** (Блокчейн с одним участником Hyperledger Fabric) и нажмите кнопку **Создать**, которая открывает колонку мастера **Основные сведения**.
...
Затем в разделе **Network size and performance** (Размер и производительность сети) укажите размер сети консорциума. Например, количество участвующих, проверяющих и одноранговых узлов. Выберите параметры инфраструктуры и размер вашей виртуальной машины.
...
- Параметр _SSH-TO-FIRST-VM_ (Сеанс SSH с первой ВМ) предоставляет вам предварительно собранную команду SSH со всеми параметрами, необходимыми для подключения к первой виртуальной машине в вашей сети (для Hyperledger Fabric это будет узел Fabric-CA).
