---
title: Руководство. Фильтрация и анализ данных с помощью вычислений в Azure Stack Edge с GPU | Документация Майкрософт
description: Узнайте, как настроить роль вычислений на устройстве GPU Azure Stack Edge и использовать ее для преобразования данных перед отправкой в Azure.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
- Устройство Azure Stack Edge активировано, как описано в статье [Активация Azure Stack Edge](azure-stack-edge-gpu-deploy-activate.md).
...
1. Добавьте общую папку Edge на устройство следующим образом:
    ...
    > [!NOTE]
    > Чтобы подключить общую папку NFS для вычислений, нужно настроить сеть вычислений в той же подсети, что и виртуальный IP-адрес NFS. Сведения о настройке сети вычислений см. в статье [Включение сети вычислений в Azure Stack Edge](azure-stack-edge-gpu-deploy-configure-network-compute-web-proxy.md).
...
Можно добавить пользовательский или готовый модуль. На этом устройстве Edge нет пользовательских модулей. Чтобы узнать, как создать пользовательский модуль, ознакомьтесь со статьей [Разработка на C# модуля IoT Edge для перемещения файлов в Azure Stack Edge](azure-stack-edge-j-series-create-iot-edge-module.md).

В этом разделе вы добавите пользовательский модуль к устройству IoT Edge, которое было создано с помощью руководства [Разработка на C# модуля IoT Edge для перемещения файлов в Azure Stack Edge](azure-stack-edge-j-series-create-iot-edge-module.md). Этот пользовательский модуль позволяет получить файлы из локальной общей папки Edge на устройстве Edge и перенести их в облачную общую папку Edge на устройстве. Затем облачный ресурс переносит файлы в учетную запись Azure, связанную с облачным ресурсом.
