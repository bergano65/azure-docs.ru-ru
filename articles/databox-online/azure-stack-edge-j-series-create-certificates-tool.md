...
6. Отправьте эти файлы в центр сертификации (как внутренний, так и общедоступный). Убедитесь, что ваш центр сертификации создает сертификаты с помощью созданного запроса, который соответствует требованиям к сертификату для сертификатов [узлов](azure-stack-edge-j-series-manage-certificates.md#node-certificates), [конечным точкам](azure-stack-edge-j-series-manage-certificates.md#endpoint-certificates)и [локальным сертификатам пользовательского интерфейса](azure-stack-edge-j-series-manage-certificates.md#local-ui-certificates), которые соответствуют потребностям сертификата узла Azure Stack.
