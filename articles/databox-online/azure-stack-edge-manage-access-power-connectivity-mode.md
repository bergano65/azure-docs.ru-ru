---
title: Azure Stack доступ пограничных устройств, режим питания и подключение | Документация Майкрософт
description: В этой статье описывается управление режимами доступа, питания и подключения для Azure Stack пограничных устройств, которые помогают передавать данные в Azure.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
В этой статье описывается, как управлять режимом доступа, питания и подключения для Azure Stackной стороны. Эти операции выполняются с помощью локального веб-интерфейса или портала Azure.
...
Доступ к устройству Azure Stack пограничным устройства управляется с помощью пароля устройства. Пароль можно изменить с помощью локального веб-интерфейса. Также можно сбросить пароль устройства в портал Azure.
...
При создании ключа активации для Azure Stack пограничной устройства или выполнения любых операций, требующих учетных данных, необходимо иметь разрешения на Azure Active Directory API Graph. Операции, которым требуются учетные данные, могут быть:
