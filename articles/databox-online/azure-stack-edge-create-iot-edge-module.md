...
В этой статье описано, как создать модуль IoT Edge для развертывания с помощью устройства Azure Stackного периметра. Azure Stack ребро — это решение для хранения данных, которое позволяет обрабатывать данные и передавать их по сети в Azure.

Вы можете использовать Azure IoT Edge модули с Azure Stackным ребром для преобразования данных по мере их перемещения в Azure. Модуль, используемый в этой статье, реализует логику копирования файла из локальной общей папки в облачную общую папку на устройстве Azure Stack пограничных устройств.
...
Устройство Azure Stack погранично может развертывать и запускать IoT Edge модули. Модули Edge — это по сути контейнеры Docker, предназначенные для выполнения конкретной задачи, например для приема сообщений от устройства, преобразования сообщений или отправки сообщений в Центр Интернета вещей. В этой статье вы создадите модуль, который копирует файлы из локальной общей папки в облачную общую папку на устройстве Azure Stack пограничных устройств.

1. Файлы записываются в локальный общий ресурс на Azure Stack пограничном устройстве.
...
![Как работает модуль Azure IoT Edge на Azure Stack границе](./media/azure-stack-edge-create-iot-edge-module/how-module-works-1.png)
...
- Azure Stack пограничной устройства, на котором работает.

    - Связанный с этим устройством ресурс Центра Интернета вещей.
    - Настроенная на устройстве роль вычислений Edge.
    Дополнительные сведения см. в подразделе [Настройка вычислений](azure-stack-edge-deploy-configure-compute.md#configure-compute) для Azure Stackной стороны.
...
3. В палитре команд введите и выполните команду **Azure IoT Edge: New IoT Edge Solution** (Azure IoT Edge: создать решение IoT Edge). В палитре команд укажите следующие сведения для создания решения:
    ...
    2. Введите имя своего решения или примите имя по умолчанию **EdgeSolution**.
        ...
        ![Создание решения 1](./media/azure-stack-edge-create-iot-edge-module/create-new-solution-1.png)
    ...
    4. Замените имя модуля, предложенное по умолчанию, любым именем на ваш выбор. В нашем примере это **FileCopyModule**.
        ...
        ![Создание решения 2](./media/azure-stack-edge-create-iot-edge-module/create-new-solution-2.png)
    ...
    5. В качестве репозитория образов первого модуля укажите Реестр контейнеров Azure, который вы создали в предыдущем разделе. Замените **localhost:5000** скопированным значением имени входа на сервер.
        ...
        ![Создание решения 3](./media/azure-stack-edge-create-iot-edge-module/create-new-solution-3.png)

4. Выберите **Файл > Открыть папку**.

    ![Создание решения 4](./media/azure-stack-edge-create-iot-edge-module/create-new-solution-4.png)
...
5. Найдите и выберите папку **EdgeSolution**, которую вы создали ранее. Окно VS Code загружает рабочую область решения IoT Edge, в которой размещены пять элементов верхнего уровня. В этой статье мы не будем изменять папку **.vscode**, а также файлы **.gitignore**, **.env** и **deployment.template.json**.
    ...
    ![Создание решения 5](./media/azure-stack-edge-create-iot-edge-module/create-new-solution-5.png)
...
2. Войдите в DOCKER, введя следующую команду в интегрированном терминале.
    ...
    ![Сборка и отправка решения IoT Edge](./media/azure-stack-edge-create-iot-edge-module/build-iot-edge-solution-1.png)
...
3. Предоставив учетные данные, вы сможете отправить образ модуля в Реестр контейнеров Azure. В обозревателе VS Code щелкните правой кнопкой мыши файл **module.json** и выберите действие **Build and Push IoT Edge solution** (Собрать и отправить решение IoT Edge).
    ...
    ![Выбор платформы](./media/azure-stack-edge-create-iot-edge-module/select-platform.png)
