---
title: Учебник по подготовке портала Azure и среды центра обработки данных к развертыванию Azure Stack Edge с GPU| Документация Майкрософт
description: Первый учебник по развертыванию Azure Stack Edge с GPU включает подготовку портала Azure.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "89254684"
---
# <a name="tutorial-prepare-to-deploy-azure-stack-edge-with-gpu"></a>Руководство по подготовке к развертыванию Azure Stack Edge с GPU 

Это первый учебник из серии учебников по развертыванию, необходимых для полного развертывания Azure Stack Edge с GPU. В этом учебнике содержатся сведения о подготовке портала Azure для развертывания ресурса Azure Stack Edge.
...
|  | |
|**Учебники по развертыванию** |Эти учебники необходимы для развертывания устройства Azure Stack Edge в рабочей среде. |
|**[1. Подготовка портала Azure для Azure Stack Edge](azure-stack-edge-gpu-deploy-prep.md)** |Создайте и настройте ресурсы Azure Stack Edge, прежде чем устанавливать физическое устройство Azure Stack Box Edge. |
|**[2. Установка Azure Stack Edge](azure-stack-edge-gpu-deploy-install.md)**|Распаковка, установка в стойку и подключение физического устройства Azure Stack Edge.  |
|**[3. Подключение к Azure Stack Edge](azure-stack-edge-gpu-deploy-connect.md)** |После установки устройства подключитесь к локальному пользовательскому веб-интерфейсу устройства.  |
|**[4. Настройка параметров сети для Azure Stack Edge](azure-stack-edge-gpu-deploy-configure-network-compute-web-proxy.md)** |Настройте сеть, включая параметры вычислительной сети и веб-прокси для вашего устройства.   |
|**[5. Настройка параметров устройства для Azure Stack Edge](azure-stack-edge-gpu-deploy-set-up-device-update-time.md)** |Назначьте имя устройства и домен DNS, настройте сервер обновлений и время устройства. |
|**[6. Настройка параметров безопасности для Azure Stack Edge](azure-stack-edge-gpu-deploy-configure-certificates.md)** |Настройка сертификатов для устройства. Используйте сертификаты, созданные устройством, или подключите собственные сертификаты.   |
|**[7. Активация Azure Stack Edge](azure-stack-edge-gpu-deploy-activate.md)** |Используйте ключ активации из службы, чтобы активировать устройство. Устройство готово к настройке общих папок SMB или NFS или подключению через REST. |
...
Теперь можно приступить к сбору сведений о конфигурации программного обеспечения для устройства Azure Stack Edge.

## <a name="deployment-configuration-checklist"></a>Контрольный список конфигурации развертывания

Перед развертыванием устройства Azure Stack Edge необходимо будет собрать данные для настройки программного обеспечения на нем. Заблаговременная подготовка некоторых из этих сведений поможет оптимизировать процесс развертывания устройства в вашей среде. Используйте [контрольный список конфигурации развертывания Azure Stack Edge](azure-stack-edge-gpu-deploy-checklist.md) для записи сведений о конфигурации при развертывании устройства.
...
- У вас должен быть доступ с правами владельца или участника на уровне группы ресурсов к Azure Stack Edge или Шлюзу Azure Data Box, Центру Интернета вещей и ресурсам службы хранилища Azure.
...
2. В левой области выберите **+ Создать ресурс**. Найдите и выберите **Azure Stack Edge или Шлюз Azure Data Box**. Нажмите кнопку **создания**. Если возникнут проблемы, ознакомьтесь со статьей [Устранение неполадок с заказами в Azure Stack Edge](azure-stack-edge-troubleshoot-ordering.md).

3. Выберите подписку, которую вы хотите использовать для устройства Azure Stack Edge. Выберите страну, в которую вы хотите отправить это физическое устройство. Выберите **Показать устройства**.

    ![Создание ресурса 1](media/azure-stack-edge-gpu-deploy-prep/create-resource-1.png)

4. Выберите тип устройства. В разделе **Azure Stack Edge Commercial** выберите **Azure Stack Edge with GPU** (Azure Stack Edge с GPU) и щелкните **Регистрация**. 

    ![Создание ресурса 2](media/azure-stack-edge-gpu-deploy-prep/create-resource-2.png)

5. Отображается краткая форма. Заполните форму и выберите **Отправить**. Корпорация Майкрософт включит вашу подписку.

    ![Создание ресурса 3](media/azure-stack-edge-gpu-deploy-prep/create-resource-3.png)

5. После включения подписки вы сможете продолжить создание ресурсов. В колонке **Выбор типа устройства** щелкните **Выбрать**.
...
> [Руководство по установке Azure Stack Edge](./azure-stack-edge-gpu-deploy-install.md)
