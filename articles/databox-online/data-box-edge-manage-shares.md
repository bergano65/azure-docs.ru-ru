...
4. Для получения легкого доступа к общим папкам из модулей пограничных вычислений используйте локальную точку подключения. Чтобы пограничный модуль мог использовать вычисление с локальной точкой подключения, выберите **Use the share with Edge compute** (Использование общей папки с вычислениями Edge).
