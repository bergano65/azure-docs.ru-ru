...
У вас должен быть `User`ный доступ к Active Directory клиенту, так как вам нужна возможность `Read all directory objects`. Вы не можете быть гостевым пользователем, так как у него нет разрешений на `Read all directory objects`. Если вы являетесь гостевым компьютером, то такие операции, как создание ключа активации, создание общей папки на Data Box Edge устройстве, создание пользователя, Настройка роли вычислений ребра, сброс пароля устройства завершатся неудачей.
...
*Подписка \<имя подписки > не имеет разрешений на регистрацию поставщиков ресурсов: Microsoft. Датабокседже.*
...
Для Data Box Edge устройства `Microsoft.DataBoxEdge` должны быть зарегистрированы. Чтобы зарегистрировать `Microsoft.DataBoxEdge`, администратор подписки должен выполнить следующую команду:
