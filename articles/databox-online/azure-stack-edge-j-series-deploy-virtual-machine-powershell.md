...
> [!IMPORTANT]
> Эта подписка не подключена или не связана с вашей подпиской Azure и находится локально на вашем устройстве.

Эта подписка будет использоваться для развертывания виртуальных машин.
...
> [!NOTE]
> Задайте значение `BlobType` страница, чтобы создать управляемый диск из виртуального жесткого диска. Задается `BlobType` для блокировки при записи в многоуровневые учетные записи хранения с помощью AzCopy.

Образы дисков можно загрузить из Marketplace. Подробные инструкции см. в статье [Получение образа виртуального диска из Azure Marketplace](azure-stack-edge-j-series-create-virtual-machine-image.md).

Ниже приведен пример выходных данных, использующих AzCopy 7,3. Дополнительные сведения об этой команде см. в подразделе [Отправка VHD-файла в учетную запись хранения с помощью AzCopy](../devtest-labs/devtest-lab-upload-vhd-using-azcopy.md).
...
Дополнительные сведения см. на странице [Dv2 Series on общего назначения sizes VM](../virtual-machines/dv2-dsv2-series.md#dv2-series).

## <a name="unsupported-vm-operations-and-cmdlets"></a>Неподдерживаемые операции и командлеты виртуальной машины

Расширение, масштабируемые наборы, группы доступности, моментальные снимки не поддерживаются.
...
Чтобы проверить, правильно ли задана переменная среды для AzCopy, выполните следующие действия.

1. Запустите "azcopy env"
