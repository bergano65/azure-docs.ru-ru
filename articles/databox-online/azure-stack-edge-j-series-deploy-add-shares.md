---
title: Учебник по передаче данных в общие папки с помощью GPU Azure Stack Edge | Документация Майкрософт
description: Узнайте, как добавлять общие папки на устройстве GPU Azure Stack Edge и подключаться к ним.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "89268933"
---
# <a name="tutorial-transfer-data-via-shares-with-azure-stack-edge-gpu"></a>Руководство по передаче данных через общие папки с помощью GPU Azure Stack Edge
...
2. Добавьте следующую запись в файл **hosts**, указав соответствующие значения для вашего устройства: 
    ...
    ![Файл hosts в проводнике](media/azure-stack-edge-j-series-deploy-add-shares/client-hosts-file-2.png)
...
2. После установки NFS-клиента используйте следующую команду, чтобы подключить общую папку типа NFS, созданную на устройстве Azure Stack Edge:
   ...
   `sudo mount -t nfs -o sec=sys,resvport <device IP>:/<NFS share on device> /home/username/<Folder on local Linux computer>`
    ...
    В следующем примере показано, как выполнить подключение к папке на устройстве Azure Stack Edge через NFS. `10.10.10.60` — IP-адрес устройства. Общая папка `mylinuxshare2` подключена на виртуальной машине ubuntuVM. `/home/azurestackedgeubuntuhost/edge` представляет собой точку подключения общей папки.
