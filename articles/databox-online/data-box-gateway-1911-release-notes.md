---
title: Заметки о выпуске Azure Stack & Шлюз Azure Data Box 1911 | Документация Майкрософт
description: Описание критических открытых проблем и способов их устранения для Azure Stack пограничных и Шлюз Data Box запуска выпуска 1911.
services: databox
author: alkohli
ms.service: databox
ms.subservice: gateway
ms.topic: article
ms.date: 03/13/2020
ms.author: alkohli
ms.openlocfilehash: 7dc2963717865157848dec3a1a0e8bddeff045ef
ms.sourcegitcommit: 829d951d5c90442a38012daaf77e86046018e5b9
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 10/09/2020
ms.locfileid: "90055900"
---
# <a name="azure-stack-edge-and-azure-data-box-gateway-1911-release-notes"></a>Заметки о выпуске Azure Stack и Шлюз Azure Data Box 1911

В следующих заметках о выпуске указываются критические открытые проблемы и Устраненные проблемы, связанные с выпуском 1911 для Azure Stack пограничных и Шлюз Data Box.

Заметки о выпуске постоянно обновляются: обнаруживаются и добавляются критические проблемы, требующие временного решения. Перед развертыванием Azure Stack ребра или Шлюз Data Box внимательно проверьте сведения, содержащиеся в заметках о выпуске.

Этот выпуск соответствует версиям программного обеспечения:

- **Azure Stack пограничных 1911 (1.6.1049.786)**
- **Шлюз Data Box 1911 (1.6.1049.786)**

> [!NOTE]
> Обновление 1911 можно применить только ко всем устройствам, на которых выполняются общедоступные версии программного обеспечения или более поздней версии.

## <a name="whats-new"></a>Новые возможности

В выпуске 1911 отсутствуют новые функции, исправлены только ошибки.

## <a name="known-issues-in-this-release"></a>Известные проблемы в этом выпуске

В этом выпуске не указаны новые проблемы. Все указанные в выпуске проблемы были перенесены из предыдущих выпусков. Чтобы просмотреть список известных проблем, перейдите к разделу [Известные проблемы в общедоступном выпуске](data-box-gateway-release-notes.md#known-issues-in-ga-release).

## <a name="next-steps"></a>Дальнейшие шаги

- [Подготовка к развертыванию Azure Stack пограничных устройств](azure-stack-edge-deploy-prep.md)
- [Подготовка к развертыванию Шлюз Azure Data Box](data-box-gateway-deploy-prep.md)
