---
title: Руководство по подключению, настройке и активации устройства Azure Stack Edge на портале Azure | Документация Майкрософт
description: В руководстве по развертыванию Azure Stack Edge вам предлагается подключить, настроить и активировать физическое устройство.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "84608440"
---
# <a name="tutorial-connect-set-up-and-activate-azure-stack-edge"></a>Руководство по подключению, настройке и активации Azure Stack Edge 

В этом учебнике описывается, как подключить, настроить и активировать устройство Azure Stack Edge с помощью локального пользовательского веб-интерфейса.
...
* Вы установили физическое устройство, как описано в статье [Руководство по установке Azure Stack Edge](azure-stack-edge-deploy-install.md).
* У вас есть ключ активации из службы Azure Stack Edge, который вы создали для управления устройством Azure Stack Edge. Дополнительные сведения см. в разделе [Подготовка к развертыванию Azure Stack Edge](azure-stack-edge-deploy-prep.md).
...
3. (Необязательно.) В левой области выберите **Параметры веб-прокси**, а затем настройте сервер веб-прокси. Хотя конфигурация веб-прокси не является обязательной, если вы используете веб-прокси, вы можете настроить его только на этой странице.
   ...
   b. В поле **Проверка подлинности** выберите **Нет** или **NTLM**. Если вы включили вычисление и используете модуль IoT Edge на устройстве Azure Stack Edge, рекомендуется задать для проверки подлинности веб-прокси значение **Нет**. **NTLM** не поддерживается.
   ...
   > Обычно прозрачные прокси-серверы без проблем работают с Azure Stack Edge.
...
6. В левой области выберите **Настройки облака**, а затем активируйте свое устройство с помощью службы Azure Stack Edge на портале Azure.
    ...
    1. В поле **Ключ активации** введите ключ активации, который вы получили при работе с разделом [Получение ключа активации](azure-stack-edge-deploy-prep.md#get-the-activation-key) для Azure Stack Edge.
...
> [Руководство по передаче данных с помощью Azure Stack Edge](./azure-stack-edge-deploy-add-shares.md).
