---
title: Требования к системе для Microsoft Azure Stack пограничных Pro R | Документация Майкрософт
description: Сведения о требованиях к программному обеспечению и сети для Azure Stack пограничных Pro R
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "96467522"
---
# <a name="azure-stack-edge-pro-r-system-requirements"></a>Требования к системе для Azure Stack пограничных Pro R

В этой статье описываются важные требования к системе для решения R для Azure Stack ребры и для клиентов, подключающихся к Azure Stack пограничному Pro R. Рекомендуется внимательно ознакомиться с информацией перед развертыванием Azure Stack пограничной Pro R. При развертывании и последующей операции вы можете обращаться к этим сведениям по мере необходимости.

Требования к системе для Azure Stack пограничных Pro R включают:
...
|----------|-----------|------------|----------|----------|
| TCP 443 (HTTPS)| Исходящий       | Глобальная сеть        | Да      | Исходящий трафик открыт для IoT Edge подготовки. Эта конфигурация необходима при использовании созданных вручную сценариев или Службы подготовки устройств к добавлению в Центр Интернета вещей Azure.|
...
Нередко сетевые администраторы могут настраивать правила брандмауэра на основе шаблонов URL-адресов, чтобы фильтровать входящий и исходящий трафик. Устройство Azure Stack пограничной Pro R и служба зависят от других приложений Майкрософт, таких как служебная шина Azure, Azure Active Directory контроля доступа, учетных записей хранения и серверов Центр обновления Майкрософт. Шаблоны URL-адресов, связанных с этими приложениями, можно использовать для настройки правил брандмауэра. Важно понимать, что шаблоны URL-адресов, связанных с этими приложениями, могут меняться. Эти изменения требуют от администратора сети отслеживания и обновления правил брандмауэра для Azure Stack пограничных Pro R в качестве и при необходимости.

Мы рекомендуем настроить правила брандмауэра для исходящего трафика, основываясь на фиксированных IP-адресах Azure Stack пограничных Pro R в большинстве случаев. Тем не менее, чтобы задать расширенные правила брандмауэра, необходимые для создания безопасных сред, можно использовать приведенные ниже сведения.
...
Используйте свой опыт при разработке и тестировании решения, чтобы убедиться в наличии достаточной емкости на устройстве с Azure Stack пограничным Pro R, и получите оптимальную производительность на устройстве.
...
- Метрики вычислений, доступные в портал Azure. Перейдите к ресурсу Azure Stack пограничной Pro R и перейдите к разделу **мониторинг метрик >**. Чтобы понять доступные ресурсы и способ их использования, ознакомьтесь с разрядом ЦП **использование вычислительной памяти** и **граничных вычислительных ресурсов в процентах** .
- [Команды мониторинга, доступные через интерфейс PowerShell устройства](azure-stack-edge-gpu-connect-powershell-interface.md#debug-kubernetes-issues-related-to-iot-edge).

Наконец, убедитесь, что вы проверили свое решение на наборе данных и количественно изверите производительность Azure Stack пограничных Pro R перед развертыванием в рабочей среде.

## <a name="next-step"></a>Следующий шаг

- [Развертывание Azure Stack ребра Pro R](azure-stack-edge-pro-r-deploy-prep.md)