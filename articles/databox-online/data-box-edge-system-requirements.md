...
- Команды мониторинга, доступные через интерфейс PowerShell устройства, например:
    ...
    - `dkrdbe stats`для получения активного потока статистики использования ресурсов контейнеров. Команда поддерживает показатели использования ЦП, памяти, ограничения памяти и сетевых операций ввода-вывода.
    - `dkrdbe system df`для получения сведений об используемом объеме дискового пространства. 
    - `dkrdbe image prune`для очистки неиспользуемых образов и освобождения места.
    - `dkrdbe ps --size`для просмотра приблизительного размера выполняющегося контейнера. 
