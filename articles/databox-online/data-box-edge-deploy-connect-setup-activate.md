---
title: Руководство по подключению, настройке и активации устройства Azure Data Box Edge на портале Azure | Документация Майкрософт
description: В руководстве по развертыванию Data Box Edge вам предлагается подключить, настроить и активировать физическое устройство.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "79128844"
---
# <a name="tutorial-connect-set-up-and-activate-azure-data-box-edge"></a>Учебник. подключение, Настройка и активация Azure Data Box Edge 
...
3. Откройте окно браузера и перейдите к локальному пользовательскому веб-интерфейсу на устройстве по адресу `https://192.168.100.10`.  
    ...
    Отображается сообщение об ошибке или предупреждение, указывающее на проблему с сертификатом безопасности веб-сайта. 
...
3. (Необязательно.) В левой области выберите **Параметры веб-прокси**, а затем настройте сервер веб-прокси. Хотя конфигурация веб-прокси не является обязательной, если вы используете веб-прокси, вы можете настроить его только на этой странице.
   ...
   > [!NOTE]
   > Прокси-файлы автоматической настройки (PAC) не поддерживаются. Файл PAC определяет, каким образом веб-браузеры и другие агенты пользователей могут автоматически выбирать соответствующий прокси-сервер (метод доступа) для получения заданного URL-адреса.
   > Прокси, которые пытаются перехватить и прочесть весь трафик (затем повторно подписывать все с собственной сертификацией) несовместимы, так как сертификат прокси-сервера не является доверенным.
   > Обычно прозрачные прокси-серверы хорошо работают с Azure Data Box Edge.
...
6. В левой области выберите **Настройки облака**, а затем активируйте свое устройство с помощью службы Data Box Edge на портале Azure.
    ...
    2. Нажмите кнопку **Применить**.
        ...
        ![Страница "Настройки облака" в локальном пользовательском веб-интерфейсе](./media/data-box-edge-deploy-connect-setup-activate/set-up-activate-6.png)
    ...
    3. Сначала устройство активируется. Затем выполняется проверка на наличие критических обновлений, которые устанавливаются автоматически при их доступности. При этом вы получите соответствующее уведомление.
        ...
        ![Обновленная страница "Настройки облака" в локальном пользовательском веб-интерфейсе](./media/data-box-edge-deploy-connect-setup-activate/set-up-activate-7.png)
    ...
    4. После завершения обновления вам, возможно, придется подождать несколько минут. Страница будет обновлена с указанием того, что устройство успешно активировано.
        ...
        ![Обновленная страница "Настройки облака" в локальном пользовательском веб-интерфейсе](./media/data-box-edge-deploy-connect-setup-activate/set-up-activate-8.png)
