...
Чтобы статически подготавливать ПС, необходимо создать общую папку на устройстве. Выполните следующие действия, чтобы предоставить ПС для общей папки SMB или NFS. 
...
1. Запишите имя общей папки. При создании этой общей папки в кластере Kubernetes, соответствующем созданному вами общему ресурсу SMB или NFS, автоматически создается объект постоянного тома. 
...
2. Скопируйте и сохраните в виде `mysql-pv.yml` файла в той же папке, где был сохранен `mysql-deployment.yml` . Чтобы использовать общий ресурс SMB или NFS, созданный ранее с помощью `kubectl` , задайте в `volumeName` поле в объекте PVC имя общей папки. 
...
В предыдущем YAML-файле создается служба, позволяющая модулю в кластере получить доступ к базе данных. Параметр службы кластера: None позволяет разрешить DNS-имя службы непосредственно на IP-адрес Pod. Это оптимально, если у вас есть только один модуль, за которым работает служба, и вы не собираетесь увеличивать число модулей Pod.
