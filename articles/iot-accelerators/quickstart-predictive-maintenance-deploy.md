...
Самолеты компании Fabrikam снабжены датчиками, которые отслеживают состояние двигателей во время полета. С этих датчиков Fabrikam годами собирает данные о функционировании и сбоях двигателей. С помощью этих сведений специалисты Fabrikam по анализу и обработке данных разработали модель, которая прогнозирует остаточный срок эксплуатации (RUL — Remaining Useful Life) двигателя самолета. В модели используется корреляция между данными четырех датчиков двигателя и износом двигателя (что в конечном счете и приводит к сбою). Компания Fabrikam продолжает проводить регулярные проверки безопасности, но теперь она может использовать модели для вычисления RUL каждого двигателя после каждого рейса. Fabrikam теперь может прогнозировать будущие точки сбоя и планировать обслуживание, что минимизирует время нахождения самолета на земле. Этот процесс не только снижает эксплуатационные расходы, но и обеспечивает безопасность пассажиров и экипажа.
...
Щелкните **Создать**, чтобы начать развертывание. Этот процесс занимает по крайней мере пять минут:
...
![Панель решения](./media/quickstart-predictive-maintenance-deploy/solutionpaneldetails.png)
