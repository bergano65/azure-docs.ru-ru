...
description: В этой статье содержатся сведения о настройке управления доступом на основе ролей (RBAC) в акселераторе решений для удаленного мониторинга.
author: dominicbetts
manager: timlt
ms.author: dobett
...
ms.locfileid: "92078074"
---
# <a name="configure-role-based-access-control-in-the-remote-monitoring-solution-accelerator"></a>Настройка управления доступом на основе ролей в акселераторе решений для удаленного мониторинга

В этой статье содержатся сведения о настройке управления доступом на основе ролей в акселераторе решений для удаленного мониторинга. Управление доступом на основе ролей позволяет ограничить доступ отдельных пользователей к определенным функциям решения.
...
1. Проверьте, являетесь ли вы владельцем приложения, щелкнув приложение, а затем — **Владельцы**. На следующем снимке экрана владельцем приложения **contoso-rm4** является **администратор Contoso**:
    ...
    ![Снимок экрана с выбранным параметром Управление владельцами, в котором отображаются владельцы приложения Contoso r m 4.](media/iot-accelerators-remote-monitoring-rbac/owners.png)
...
1. Выберите свое приложение и щелкните **Манифест**. Отобразятся две имеющиеся [роли приложения](/azure/architecture/multitenant-identity/app-roles), определенные для приложения:
...
В этой статье вы узнали, как реализуется управление доступом на основе ролей в акселераторе решений для удаленного мониторинга.
