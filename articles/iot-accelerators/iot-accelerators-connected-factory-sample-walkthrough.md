...
Прокси-сервер и издатель OPC реализованы как модули на основе [Edge Интернета вещей Azure][lnk-Azure-IoT-Gateway]. К каждой виртуальной производственной линии подключен шлюз.
...
Производственная линия изготавливает детали. Она состоит из нескольких станций: сборка, тестирование и упаковка.

Выполняемое моделирование обновляет данные, которые отображаются на узлах OPC UA. Работа всех станций виртуальной производственной линии координируется с помощью MES через OPC UA.

## <a name="simulated-manufacturing-execution-system"></a>Виртуальная система управления производственными процессами

Система MES отслеживает каждую станцию на производственной линии через OPC UA для обнаружения изменений в состоянии станции. Она вызывает методы OPC UA для управления станциями и передает продукты от одной станции к другой до завершения цикла.

## <a name="gateway-opc-publisher-module"></a>Модуль издателя OPC шлюза

Модуль издателя OPC подключается к серверам OPC UA на станции и подписывается на узлы OPC, которые должны быть опубликованы. Этот модуль выполняет следующее:
...
Этот модуль использует методы устройства Центра Интернета вещей для передачи пакетированных данных TCP/IP на уровне приложения, чтобы с помощью SSL/TLS обеспечить доверие конечной точки, шифрование и целостность данных.
...
TSI отправляет запрос к данным узла с использованием **SearchSpan** на основе времени и группирует результаты по значениям **OPC UA ApplicationUri**, **OPC UA NodeId** или **OPC UA DisplayName**.

Чтобы получить данные для датчиков общей эффективности оборудования и ключевых показателей эффективности, а также диаграммы временных рядов, это решение выполняет статистическую обработку по количеству событий, общей сумме (**Sum**), среднему (**Avg**), минимальному (**Min**) или максимальному (**Max**) значению.

Временные ряды создаются с помощью другого процесса. Решение вычисляет значения общей эффективности оборудования и ключевых показателей эффективности на основе базовых данных станции и суммирует результаты по производственным линиям, фабрикам и предприятию в целом.
...
11. Веб-приложение подключенной фабрики возвращает интерфейс браузера OPC со сведениями OPC UA, полученными с сервера OPC UA, в веб-браузер для отображения.
    - Когда пользователь просматривает пространство адресов OPC и применяет функции к размещенным в нем узлам, клиент интерфейса браузера OPC выполняет вызовы AJAX через HTTPS, защищенные с помощью токенов защиты от подделки, для получения данных из веб-приложения подключенной фабрики.
