...
ms.locfileid: "46369517"
---
# <a name="tutorial-conduct-a-root-cause-analysis-on-an-alert"></a>Руководство. Анализ первопричин с помощью оповещений

Из этого руководства вы узнаете, как использовать акселератор решений удаленного мониторинга для выявления первопричины. Вы увидите, что оповещение активируется на информационной панели решения удаленного мониторинга, а затем используйте обозреватель Аналитики временных рядов Azure для выявления первопричины.

В этом руководстве используется два имитированных устройства для грузовых автомобилей, которые отправляют телеметрические данные местоположения, высоты над уровнем моря, скорости и температуры. Грузовиками управляет организация под названием Contoso, и они подключены к акселератору решений для удаленного мониторинга. Как оператору Contoso вам необходимо понять, почему один из грузовиков (delivery-truck-02) отправил оповещение о низкой температуре.
...
При применении фильтра на карте и на панели телеметрии отображаются только те грузовики, которые отвечают условиям фильтра на странице **Панель мониторинга**. Вы можете увидеть, что к акселератору решений подключены два грузовика, включая **truck-02**.
...
Отфильтруйте устройства, введя в поле фильтра **delivery-truck** и выбрав **температуру** в качестве **Меры** на левой панели:
...
Вы увидите то же представление, которое было на панели удаленного мониторинга, и теперь вы можете приблизить интервал времени, в котором было вызвано предупреждение:
...
На новой панели измените имя новой метки на **Устройства**, чтобы оно соответствовало предыдущей. Выберите **высоту над уровнем моря** в качестве **Меры** и **iothub-connection-device-id** как значение **Split By** (Разделение по), чтобы добавить данные телеметрии высоты над уровнем моря на представление:
...
Если взглянуть на потоки в текущем представлении, вы увидите, что профили высоты над уровнем моря для двух грузовиков сильно отличаются. Кроме того, температура в **delivery-truck-02** падает, когда грузовик достигает большой высоты над уровнем моря. Вы удивитесь, узнав, что грузовики должны были пройти по одному и тому же пути.
...
Маршруты грузовиков обычно оптимизируются заранее. Вы понимаете, что схемы движения транспорта, погода и другие непредсказуемые события могут вызывать задержки, а выбор маршрута до последней минуты остается за водителем, который исходит из самого оптимального решения. Однако, так как температура ресурсов внутри транспортного средства является решающем значением, следует создать дополнительное правило в своем решении удаленного мониторинга, чтобы убедиться, что вы получите предупреждение, если средняя высота в 1-минутном интервале превышает 350 футов:
...
Из этого руководства вы узнали, как с помощью обозревателя Аналитики временных рядов с акселератора решений удаленного мониторинга диагностировать первопричину предупреждения. Чтобы узнать, как выявлять и устранять проблемы с подключенными устройствами с помощью акселератора решений, перейдите к следующему руководству.
