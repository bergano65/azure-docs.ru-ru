...
Откройте палитру команд (CTRL+SHIFT+P) и введите **ADL: Open Sample Script** (ADL: открыть пример сценария). При этом откроется другой экземпляр этого примера. Вы также можете изменить, настроить и отправить скрипт в этом экземпляре.
...
2. Введите **ADL: Compile Script** (ADL: компилировать сценарий). Результаты компиляции отображаются в окне **Вывод**. Вы также можете щелкнуть файл сценария правой кнопкой мыши и выбрать **ADL: Compile Script** (ADL: компилировать сценарий), чтобы выполнить компиляцию задания U-SQL. Результат компиляции отобразится в области **Вывод**.
 
**Отправка скрипта U-SQL**

1. Нажмите клавиши CTRL+SHIFT+P, чтобы открыть палитру команд. 
2. Введите **ADL: Submit Job** (ADL: отправить задание). Вы также можете щелкнуть файл сценария правой кнопкой мыши и выбрать **ADL: Submit Job** (ADL: отправить задание). 
...
1. Нажмите клавиши CTRL+SHIFT+P, чтобы открыть палитру команд. 
2. Введите **ADL: Set Default Context** (ADL: задать контекст по умолчанию). Можно также щелкнуть редактор сценариев правой кнопкой мыши и выбрать **ADL: Set Default Context** (ADL: задать контекст по умолчанию).
...
1. Нажмите клавиши CTRL+SHIFT+P, чтобы открыть палитру команд. 
2. Введите **ADL: Set Script Parameters** (ADL: задать параметры сценария).
3. Откроется файл xxx_settings.json со следующими свойствами:
   ...
   - **optionalSettings**:
        ...
        - **degreeOfParallelism**. Коэффициент параллелизма в диапазоне от 1 до 150. Значением по умолчанию является максимально допустимый коэффициент параллелизма в учетной записи Azure Data Lake Analytics. 
...
1. Нажмите клавиши CTRL+SHIFT+P, чтобы открыть палитру команд. 
2. Введите **ADL: Set Git Ignore** (ADL: установить игнорирование Git).
...
## <a name="work-with-code-behind-files-c-sharp-python-and-r"></a>Работа с файлами кода программной части: C Sharp, Python и R
...
Вы можете зарегистрировать сборку, используя команду **ADL: Register Assembly** (ADL: зарегистрировать сборку) или **ADL: Register Assembly (Advanced)** (ADL: зарегистрировать сборку (дополнительно)).

**Регистрация с помощью команды ADL: Register Assembly** (ADL: зарегистрировать сборку)
1.  Нажмите клавиши CTRL+SHIFT+P, чтобы открыть палитру команд.
2.  Введите **ADL: Register Assembly** (ADL: зарегистрировать сборку). 
...
Более удобный способ вызова команды **ADL: Register Assembly** (ADL: зарегистрировать сборку) — щелкнуть правой кнопкой мыши DLL-файл в проводнике. 

**Регистрация с помощью команды ADL: Register Assembly (Advanced) (ADL: зарегистрировать сборку (дополнительно))**
1.  Нажмите клавиши CTRL+SHIFT+P, чтобы открыть палитру команд.
2.  Введите **ADL: Register Assembly (Advanced)** (ADL: зарегистрировать сборку (дополнительно)). 
3.  Укажите локальный путь к сборке. 
...
Еще один способ запуска команды **ADL: Register Assembly (Advanced)** (ADL: зарегистрировать сборку (дополнительно)) — щелкнуть правой кнопкой мыши DLL-файл в проводнике. 
...
1.  Нажмите клавиши CTRL+SHIFT+P, чтобы открыть палитру команд. 
2.  Введите **ADL: Вход**. Сведения для входа появятся в правом нижнем углу.
...
Для выхода введите команду **ADL: Logout** (ADL: выход).
...
Вы можете создать сценарий извлечения файлов в формате CSV, TSV и TXT с помощью команды **ADL: Create EXTRACT Script** (ADL: создать сценарий извлечения) или через обозреватель Azure Data Lake.

**Создание скрипта извлечения с помощью команды**

1. Нажмите клавиши CTRL+SHIFT+P, чтобы открыть палитру команд, и введите **ADL: Create EXTRACT Script** (ADL: создать сценарий извлечения).
...
1. Нажмите клавиши CTRL+SHIFT+P, чтобы открыть палитру команд.
2. Введите **ADL: List Accounts** (ADL: вывести список учетных записей). Учетные записи отображаются в области **Вывода**.

**Доступ к метаданным Azure Data Lake Analytics**

1.  Нажмите клавиши CTRL+SHIFT+P, а затем введите **ADL: List Tables** (ADL: вывести список таблиц).
...
1.  Откройте палитру команд (CTRL+SHIFT+P) и выберите **ADL: Show Jobs** (ADL: показать задания). 
...
1. Щелкните редактор сценариев правой кнопкой мыши и выберите **ADL: List Path** (ADL: вывод пути).
...
Вы можете скачать файл с помощью команды **ADL: Download File** (ADL: скачать файл) или **ADL: Download File (Advanced)** (ADL: скачать файл (дополнительно)).

**Скачивание файла с помощью команды ADL: Download File (Advanced)** (ADL: скачать файл (дополнительно))
...
**Скачивание файла с помощью команды ADL: Download File** (ADL: скачать файл)
...
-   **Технология автозаполнения IntelliSense для метаданных Data Lake Analytics**. Средства Data Lake скачивают метаданные Data Lake Analytics в локальное хранилище. Функция IntelliSense автоматически заполняет объекты метаданными Data Lake Analytics. К этим объектам относятся база данных, схема, таблица, представление, функция с табличным значением, процедуры и сборки C#.
...
-   **Маркер ошибки IntelliSense**. Средства Data Lake подчеркивают ошибки изменения в файлах U-SQL и C#. 
...
- [Учебник. по началу работы с Azure Data Lake Analytics с помощью портала Azure](data-lake-analytics-get-started-portal.md)
- [Учебник. по разработке сценариев U-SQL с помощью средств Data Lake для Visual Studio](data-lake-analytics-data-lake-tools-get-started.md)
