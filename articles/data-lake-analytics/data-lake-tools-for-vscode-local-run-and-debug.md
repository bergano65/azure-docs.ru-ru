...
1. Нажмите клавиши CTRL + SHIFT + P, чтобы открыть палитру команд, а затем введите **ADL: Скачайте локальный пакет выполнения @ no__t-0, чтобы скачать пакеты.  
...
Для первого пользователя используйте **ADL: Скачайте локальный пакет выполнения @ no__t-0, чтобы скачать локальные пакеты выполнения, если не [настроена локальная среда выполнения U-SQL](#set-up-the-u-sql-local-run-environment).

1. Нажмите клавиши CTRL + SHIFT + P, чтобы открыть палитру команд, а затем введите **ADL: Запустите службу локального запуска @ no__t-0.   
...
4. Нажмите клавиши CTRL + SHIFT + P, чтобы открыть палитру команд, и введите **ADL: Отправьте задание @ no__t-0, а затем выберите **Local (локальный** ), чтобы отправить задание в локальную учетную запись.
...
1. Используйте @no__t 0ADL: Скачайте локальный пакет выполнения @ no__t-0, чтобы скачать локальные пакеты выполнения, если не [настроена локальная среда выполнения U-SQL](#set-up-the-u-sql-local-run-environment).
2. Установите пакет SDK для .NET Core 2,0, как предложено в окне сообщения, если оно не установлено.
 
   @ no__t-1reminder устанавливает DotNet @ no__t-2
...
1. Нажмите клавиши CTRL + SHIFT + P, чтобы открыть палитру команд, а затем введите **ADL: Запустите службу локального запуска @ no__t-0. Откроется консоль команд. Убедитесь, что установлено значение параметра **DataRoot**.
2. Установите точку останова в коде программной части C#.
3. Вернитесь в редактор скриптов, щелкните правой кнопкой мыши и выберите **ADL: Локальная отладка @ no__t-0.
