...
2. Откройте свою учетную запись Data Lake Analytics и в разделе __Мониторинг__ выберите **Журналы диагностики**. Затем выберите __Turn on diagnostics__ (Включить диагностику).
    ...
    ![Снимок экрана, на котором показано выбранное действие "журналы диагностики" и отмечено флажком "включить диагностику для получения сведений о следующих журналах".](./media/data-lake-analytics-diagnostic-logs/turn-on-logging.png)
...
| Имя | Тип | Описание |
| --- | --- | --- |
| time |Строковый тип |Метка времени журнала (в формате UTC). |
| resourceId |Строковый тип |Идентификатор ресурса, с которым была выполнена операция |
| категория |Строковый тип |Категория журнала. Например, **Requests**. |
| operationName |Строковый тип |Имя операции, добавленной в журнал. Например, GetAggregatedJobHistory. |
| resultType |Строковый тип |Состояние операции. Например, 200. |
| callerIpAddress |Строковый тип |IP-адрес клиента, отправившего запрос. |
| correlationId |Строковый тип |Идентификатор журнала. С помощью этого значения можно группировать связанные записи журнала. |
...
| Имя | Тип | Описание |
| --- | --- | --- |
| HttpMethod |Строковый тип |Метод HTTP, использованный для операции. Например, GET. |
| путь |Строковый тип |Путь выполнения операции. |
| RequestContentLength |INT |Длина содержимого HTTP-запроса. |
| ClientRequestId |Строковый тип |Идентификатор, однозначно определяющий данный запрос. |
| StartTime |Строковый тип |Время получения запроса сервером. |
| EndTime |Строковый тип |Время отправки ответа сервером. |
...
| time |Строковый тип |Метка времени журнала (в формате UTC). |
| resourceId |Строковый тип |Идентификатор ресурса, с которым была выполнена операция |
| категория |Строковый тип |Категория журнала. Например, **Audit**. |
| operationName |Строковый тип |Имя операции, добавленной в журнал. Например, JobSubmitted. |
| resultType |Строковый тип |Подсостояние состояния задания (operationName). |
| resultSignature |Строковый тип |Дополнительные сведения о состоянии задания (operationName). |
| удостоверение |Строковый тип |Пользователь, который запросил операцию. Например, susan@contoso.com. |
...
| Имя | Тип | Описание |
| --- | --- | --- |
| JobId |Строковый тип |Идентификатор, присвоенный заданию. |
| JobName |Строковый тип |Имя, указанное для задания. |
| JobRunTime |Строковый тип |Среда выполнения, используемая для обработки задания. |
| SubmitTime |Строковый тип |Время отправки задания (в формате UTC). |
| StartTime |Строковый тип |Время начала выполнения задания после отправки (в формате UTC). |
| EndTime |Строковый тип |Время завершения задания. |
| Parallelism |Строковый тип |Количество единиц Data Lake Analytics, запрошенных для этого задания во время отправки. |
