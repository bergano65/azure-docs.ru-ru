---
title: Разработка проекта базы данных U-S'L - Озеро данных Azure
...
3.  Добавить **управляемые зависимости** и **дополнительные файлы,** если таковые имеются. При добавлении дополнительных файлов средство будет использовать относительный путь, чтобы позже обеспечить возможность определения сборки на локальном компьютере и компьютере построения. 
...
Узнайте больше о [том, как создать проект базы данных U-S'L с командной строкой MSBuild и задачей сборки служб Azure DevOps.](data-lake-analytics-cicd-overview.md)
...
2.  В **развертывании U-S'L Database Wizard**выберите **учетную запись ADLA,** на которую необходимо развернуть базу данных. Поддерживаются оба типа учетных записей — локальные и ADLA.
...
1.  Открытый **исследователь серверов**. Затем разверните **учетную запись Azure Data Lake Analytics**, для которой требуется выполнить развертывание базы данных.
...
- [Как настроить конвейер CI/CD для аналитики озер данных Azure](data-lake-analytics-cicd-overview.md)
- [Как протестировать код аналитики озер данных Azure](data-lake-analytics-cicd-test.md)
