---
title: Управление политиками аналитики аналитики озер данных Azure
description: Узнайте, как использовать политики учетных записей для контроля использования учетной записи Data Lake Analytics, например, максимальных AUs и максимальных заданий.
...
---
# <a name="manage-azure-data-lake-analytics-using-account-policies"></a>Управление аналитикой озер данных Azure с помощью политик и средств учетных записей

Политики учетных записей помогают контролировать использование ресурсов учетной записи Azure Data Lake Analytics. Эти политики позволяют вам контролировать стоимость использования Azure Data Lake Analytics. Например, с помощью этих политик вы можете предотвратить неожиданное повышение расходов, ограничив количество AU, которые учетная запись может одновременно использовать.
...
Эта политика ограничивает количество заданий одновременно. По умолчанию установлено значение 20. Если в вашей учетной записи Data Lake Analytics есть доступные единицы использования аналитики, новые задания будут запускаться немедленно до тех пор, пока общее количество выполняемых заданий не достигнет значения этой политики. Когда вы достигаете максимального количества заданий, которые могут выполняться одновременно, последующие задания стоят в приоритетном порядке до тех пор, пока одно или несколько выполняемых заданий не будут завершены (в зависимости от имеющихся AUs).
...
Политики уровня занятости позволяют контролировать максимальный AUs и максимальный приоритет, который отдельные пользователи (или члены определенных групп безопасности) могут устанавливать на должности, которые они представляют. Эта политика позволит контролировать затраты пользователей. Политики также позволяют контролировать эффект, который запланированные задания могу оказать на рабочие задания с высоким приоритетом, запущенные в одной учетной записи Data Lake Analytics.
