...
Перед выполнением примеров U-SQL в этом документе прочтите и выполните [Tutorial: Разработка скриптов U-SQL с помощью средств Data Lake для Visual Studio @ no__t-0. Это руководство описывает механизм использования U-SQL с помощью средств Azure Data Lake для Visual Studio.
...
* **Переменные набора строк**: Каждое выражение запроса, создающее набор строк, может быть назначено переменной. U-SQL использует в сценарии шаблон именования переменных T-SQL (пример: `@searchlog`).
