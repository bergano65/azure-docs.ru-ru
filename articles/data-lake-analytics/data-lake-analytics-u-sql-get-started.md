...
* Дополнительные сведения о **синтаксисе языка U-SQL** см. в [справочнике по языку U-SQL](https://go.microsoft.com/fwlink/p/?LinkId=691348).
...
* Оператор **EXTRACT** считывает данные из файла и определяет схему для чтения. `Extractors.Tsv` имеет встроенные средства извлечения U-SQL для файлов вкладку запятыми. Можно разрабатывать пользовательские средства извлечения.
* Оператор **OUTPUT** записывает данные из набора строк в файл. `Outputters.Csv()` имеет встроенные средства вывода U-SQL, чтобы создать файл с разделителями запятыми. Вы можете разработать собственные средства вывода.
