...
И наконец, важно помнить, что в каждой базе данных U-SQL может быть только одна версия любой сборки. Например если требуется, и версии 7 и 8 библиотеки NewtonSoft Json.NET, необходимо зарегистрировать их в двух разных базах данных. Кроме того, каждый скрипт может ссылаться только на одну версию библиотеки DLL для каждой сборки. В этом отношении U-SQL соблюдает семантику C# управления сборками и версиями.

## <a name="use-user-defined-functions-udf"></a>Использование определяемых пользователем функций. Определяемая пользователем функция
...
## <a name="use-user-defined-types-udt"></a>Использование пользовательских типов данных (UDT)
...
* Форматировщик типа — обязательный параметр, определяющий форматировщика для UDT. Здесь нужно передать тип интерфейса `IFormatter`.
...
* **Десериализация** — обратная сериализация данных в предоставленном потоке, в ходе которого воспроизводится граф объектов.
...
* **Persistent** — указывает, что контекст источника или назначения является материализованным хранилищем.
...
## <a name="use-user-defined-aggregates-udagg"></a>Использование определяемых пользователем статистических функций (UDAGG)
...
* TResult — тип возвращаемого значения метода Terminate.
...
## <a name="use-user-defined-objects-udo"></a>Использование определяемых пользователем объектов (UDO)
...
* *Extractors.Text()*: извлекает данные из текстовых файлов с разделителями разного кодирования.

* *Extractors.Csv()*: извлекает данные из файлов данных с разделителями-запятыми (CSV) разного кодирования.

* *Extractors.Tsv()*: извлекает данные из файлов со значением с разделением знаками табуляции (TSV) разного кодирования.
...
* *Outputters.Text()*: записывает данные в текстовые файлы с разделителями разного кодирования
* *Outputters.Csv()*: записывает данные в файлы с разделителями-запятыми (CSV) разного кодирования.
* *Outputters.Tsv()*: записывает данные в файлы со значением с разделением знаками табуляции (TSV) разного кодирования.
