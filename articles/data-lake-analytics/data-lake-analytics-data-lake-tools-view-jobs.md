---
title: Используйте браузер вакансий & просмотр ездовые работы - Аналитика azure Data Lake
...
* Граф задания.
  ...
  * Воспроизведение задания
      ...
      Data Lake Analytics запускает задания и архивы вершин, работающих информацию о заданиях, например, когда вершины запускаются, останавливаются, не удались и как они повторяются и т.д. Вся информация автоматически регистрируется в хранилище запросов и хранится в его профиле работы. Профиль задания можно скачать, нажав кнопку Load Profile ("Загрузить профиль"), после чего можно просматривать воспроизведение задания.
...
* Metadata Operations ("Операции с метаданными").
    ...
    Вы можете выполнить некоторые операции метаданных в скрипте U-S'L, такие как создание базы данных, падение таблицы и т.д. Эти операции отображаются в операции Метаданных после компиляции. Здесь можно искать утверждения, создавать сущности, а также перетаскивать сущности сюда.
...
* Представление выполнения вершин.
    ...
    Здесь показываются сведения о выполнении вершин. Профиль работы архивирует каждый журнал выполнения вершины, например, общее чтение/написание данных, время выполнения, состояние и т.д. С помощью этого представления вы можете получить более подробную информацию о том, как работа побежала. Дополнительные сведения см. в статье[Использование представления выполнения вершин в инструментах Data Lake для Visual Studio](data-lake-analytics-data-lake-tools-use-vertex-execution-view.md).
