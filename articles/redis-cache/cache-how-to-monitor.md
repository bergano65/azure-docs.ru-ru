...
>[!NOTE]
>В дополнение к архивированию метрик кэша в хранилище вы можете настроить для них [потоковую передачу в концентратор событий или отправку в журнал Log Analytics](../monitoring-and-diagnostics/monitoring-rest-api-walkthrough.md#retrieve-metric-values).
>
>
...
| Задержка кэша (предварительная версия) | Задержка кэша определяется на основе задержки между узлами кэша. Этот показатель измеряется в микросекундах и имеет три измерения: Avg, Min и Max, которые, соответственно, представляют среднее, минимальное и максимальное время задержки кэша за указанный интервал создания отчетов. |
