...
2. [**Настройте управляющую программу DOCKER и управляющую программу IoT Edge на устройстве.**](#configure-the-daemons)

   IoT Edge использует на устройстве две управляющие программы, для которых необходимо выполнять веб-запросы через прокси-сервер. Управляющая программа IoT Edge отвечает за взаимодействие с центром Интернета вещей. Управляющая программа значок Кита отвечает за управление контейнерами, поэтому обменивается данными с реестрами контейнеров.

   Этот шаг является одноразовым процессом настройки устройства IoT Edge при его первой настройке.

3. [**Настройка свойств агента IoT Edge в файле config. YAML на устройстве**](#configure-the-iot-edge-agent)
