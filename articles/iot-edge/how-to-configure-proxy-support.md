...
Введите следующий текст, заменив ** \<URL-адрес прокси>** адресом и портом прокси-сервера. Сохранитесь и закройте редактор.
...
Откройте окно PowerShell от имени администратора и выполните следующую команду, чтобы внести изменения в реестр с помощью новой переменной среды. Замените ** \<URL-адрес прокси>** адресом и портом прокси-сервера.
...
Помимо модулей edgeAgent и edgeHub, для других модулей может потребоваться настройка прокси-сервера. Это модули, которым необходим доступ к ресурсам Azure, кроме центра Интернета вещей, например хранилища BLOB-объектов, и в файле манифеста развертывания должна быть указана HTTPS_PROXYая переменная для этого модуля.
