...
1. Устройство шлюза должно иметь возможность безопасного подключения к подчиненным устройствам, получать подключения от подчиненных устройств и маршрутизировать сообщения в соответствующее место назначения. Дополнительные сведения см. [в разделе Настройка устройства IOT Edge для](how-to-create-transparent-gateway.md)работы в качестве прозрачного шлюза.
...
2. Следуйте инструкциям в разделе [Создание устройства x. 509 для центра Интернета вещей](../iot-hub/iot-hub-security-x509-get-started.md#create-an-x509-device-for-your-iot-hub) статьи *Настройка безопасности x. 509 в центре Интернета вещей Azure*. В этом разделе вы выполните следующие действия: 
   ...
   1. Добавить устройство. Укажите имя в нижнем регистре для **идентификатора устройства**и выберите тип проверки подлинности **X. 509, подписанный центром сертификации**. 
...
Пример программы Python, выполняющей проверку подлинности в центре Интернета вещей с помощью сертификатов X. 509, см. в статье пример [iothub_client_sample_x509. корректировки](https://github.com/Azure/azure-iot-sdk-python/blob/master/device/samples/iothub_client_sample_x509.py) пакета SDK для центра Интернета вещей Java. Некоторые ключевые строки этого примера приведены здесь, чтобы продемонстрировать процесс проверки подлинности.
