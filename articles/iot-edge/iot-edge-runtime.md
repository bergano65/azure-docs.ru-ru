...
* **имажепуллполици** — указывает, пытается ли агент IOT EDGE автоматически запрашивать последнюю версию образа для модуля. Если значение не указано, используется значение по умолчанию — *OnCreate*. Возможные значения:
...
IoT Edge собирает анонимные данные телеметрии из среды выполнения узла и системных модулей для улучшения качества продукта. Эти сведения называются телеметрии качества среды выполнения (РКТ). РКТ периодически отправляется в центр Интернета вещей в качестве сообщений, отправляемых с устройства в облако, из агента IoT Edge. Сообщения РКТ не отображаются в обычных телеметрических данных клиента и не потребляют квоты на сообщения.

Полный список метрик, собранных edgeAgent и edgeHub, доступен в [разделе Доступные метрики статьи метрики времени выполнения Access IOT Edge](how-to-access-built-in-metrics.md#available-metrics). Подмножество этих метрик собираются агентом IoT Edge как часть РКТ. Метрики, собранные как часть РКТ, включают тег `ms_telemetry` .

В рамках анонимной передачи все личные или организационные сведения, такие как имена устройств и модулей, удаляются перед отправкой.

Частота по умолчанию РКТ — одно сообщение, отправляемое в центр Интернета вещей каждые 24 часа и локальную коллекцию на edgeAgent каждый час.

Если вы хотите отказаться от РКТ, это можно сделать двумя способами:
