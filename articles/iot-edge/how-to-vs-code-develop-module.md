...
Существует два способа отладки модулей, написанных на C#, Node. js или Java, в Visual Studio Code: можно подключить процесс в контейнере модуля или запустить код модуля в режиме отладки. Для отладки модулей, написанных на Python или C, можно присоединяться только к процессу в контейнерах Linux AMD64.
...
1. Подготовьте свою среду к отладке в соответствии с требованиями языка разработки, установите точку останова в модуле и выберите необходимую конфигурацию отладки:
   - **C#**
     - В интегрированном терминале Visual Studio Code измените каталог на ***&lt;имя модуля&gt;*** папке, а затем выполните следующую команду, чтобы создать приложение .NET Core.
     ...
     - Перейдите в представление отладки Visual Studio Code, выбрав **Вид > Отладка**. Выберите конфигурацию отладки ***&lt;имя вашего модуля&gt;* Local Debug (.NET Core)** (Локальная отладка (.NET Core)) из раскрывающегося списка.
        ...
        > [!NOTE]
        > Если `TargetFramework` .NET Core не согласуется с путем к программе в `launch.json`, необходимо вручную обновить путь к программе в `launch.json`, чтобы сопоставить `TargetFramework` в CSPROJ-файле, чтобы Visual Studio Code мог успешно запустить эту программу.
...
1. Если вы разрабатываете свой модуль на языке Python, сначала выполните следующие шаги:
   ...
   - Добавьте следующую строку кода в обратный вызов, который необходимо отладить:
     ...
     Например, если вы хотите отладить функцию `receive_message_listener`, вставьте следующую строку кода, как показано ниже:
...
1. Откройте файл модуля для используемого языка разработки и добавьте точку останова:
   ...
   - **Python**: добавьте точку останова в файл `main.py`в методе обратного вызова, где была добавлена строка `ptvsd.break_into_debugger()`.
   - **В. Добавьте**точку останова в файл `main.c`.
