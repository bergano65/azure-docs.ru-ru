...
В этом учебнике используется пример развертывания модуля **C на устройстве Linux**. Мы выбрали этот пример, так как для него нужно выполнить минимальное количество предварительных требований, так что вы можете узнать о средствах разработки, не беспокоясь о том, установлены ли у вас нужные библиотеки. Как только вы поймете принципы разработки, вы сможете выбрать предпочитаемый язык или службу Azure, чтобы ознакомиться с подробностями. 
...
В этом учебнике мы используем шаблон модуля C, потому что для его установки нужно выполнить минимальное количество предварительных требований. 
...
1. Откройте файл **main.c**, который находится в папке **modules/SampleModules/** . 

2. Пакет SDK для центра Интернета вещей для C использует функцию [SetInputMessageCallback](https://docs.microsoft.com/azure/iot-hub/iot-c-sdk-ref/iothub-module-client-ll-h/iothubmoduleclient-ll-setinputmessagecallback), чтобы инициализировать очереди ввода модуля. Найдите эту функцию в файле main.c.

3. Просмотрите конструктор функции SetInputMessageCallback и убедитесь, что очередь ввода **input1** инициализируется в коде. 

   ![Поиск входного имени в конструкторе SetInputMessageCallback](./media/tutorial-develop-for-linux/declare-input-queue.png)

4. Очереди вывода модуля инициализируются аналогичным образом. Найдите функцию [SendEventToOutputAsync](https://docs.microsoft.com/azure/iot-hub/iot-c-sdk-ref/iothub-module-client-ll-h/iothubmoduleclient-ll-sendeventtooutputasync) в файле main.c. 

5. Просмотрите конструктор функции SendEventToOutputAsync и убедитесь, что очередь вывода **output1** инициализируется в коде. 
...
* Ваш компьютер использует тот же тип контейнеров, который вы создаете? Этот учебник предназначен для устройств IoT Edge с Linux, поэтому на боковой панели в Visual Studio Code должно отображаться **amd64** или **arm32v7**, а на Docker Desktop должны запускаться контейнеры Linux. Контейнеры Windows не поддерживают модули в Visual Studio Code. 
