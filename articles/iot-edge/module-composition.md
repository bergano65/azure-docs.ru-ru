---
title: Развертывание & маршрутов модуля с манифестами развертывания — Azure IoT Edge
...
* Модуль **агента IOT Edge** двойника, который включает в себя три компонента:
...
* Требуемые свойства любого дополнительного модуля двойников (необязательно).
...
Определите, как среда выполнения IoT Edge устанавливает модули в развертывании. Агент IoT Edge — это компонент среды выполнения, который управляет установкой, обновлением и отчетами о состоянии для устройства IoT Edge. Таким образом, модуль $edgeAgent двойника содержит сведения о конфигурации и управлении для всех модулей. Эти сведения включают параметры конфигурации для самого агента IoT Edge.
