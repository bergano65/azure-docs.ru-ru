...
- [Azure CLI](https://docs.microsoft.com/cli/azure/?view=azure-cli-latest&preserve-view=true) с `azure-iot` установленным расширением CLI. Дополнительные сведения см. [в статье инструкции по установке расширения Azure IOT для Azure CLI Azure](https://docs.microsoft.com/cli/azure/azure-cli-reference-for-iot).
...
Чтобы клиент MQTT сам пропускал проверку подлинности, ему сначала нужно отправить пакет CONNECT в брокер MQTT, чтобы инициировать подключение в его имени. Этот пакет предоставляет три части сведений для проверки подлинности: a `client identifier` , `username` и `password` :
...
-   `client identifier`Поле представляет имя устройства или модуля в центре Интернета вещей. Для этого используется следующий синтаксис:
   ...
   Обратите внимание, что брокер не будет разрешать подключение двух клиентов с использованием одних и тех же учетных данных. Брокер отключит уже подключенный клиент, если второй клиент подключается с использованием тех же учетных данных.
...
Каждая инструкция политики авторизации состоит из комбинации `identities` , `allow` или `deny` , `operations` и `resources` :
...
- `allow` или `deny` Определите, следует ли разрешать или запрещать операции.
...
- Инструкции авторизации оцениваются в порядке, отличном от того, который отображается в определении JSON. Он начинается с просмотра `identities` , а затем выбора первой инструкции Allow или Deny, соответствующей запросу. В случае конфликтов между операторами allow и Deny инструкция DENY побеждает.
- В политике авторизации можно использовать несколько переменных (например, подстановки):
    - `{{iot:identity}}` представляет идентификатор подключенного в данный момент клиента. Например `myDevice` , в случае с устройством, `myEdgeDevice/SampleModule` в случае с модулем.
    - `{{iot:device_id}}` представляет идентификатор подключенного в данный момент устройства. Например `myDevice` , в случае с устройством, `myEdgeDevice` в случае с модулем.
    - `{{iot:module_id}}` представляет идентификатор подключенного в данный момент модуля. Например, "" в случае устройства, `SampleModule` в случае с модулем.
...
Вопросы авторизации для центра Интернета вещей обрабатываются немного иначе, чем в пользовательских разделах. Вот основные моменты, которые следует помнить:
...
Обратите внимание, что в первом примере используется порт 1883 (MQTT), например без протокола TLS. Еще один пример с портом 8883 (МКТТС), например с включенным протоколом TLS, показан в следующем разделе.
