---
title: Развертывание Пользовательского визуального распознавания на устройстве Azure IoT Edge | Документация Майкрософт
description: Узнайте, как создать модель компьютерного зрения в качестве контейнера с помощью Пользовательского визуального распознавания и IoT Edge.
services: iot-edge
author: kgremban
manager: philmea
ms.author: kgremban
...
ms.locfileid: "51566488"
---
# <a name="tutorial-perform-image-classification-at-the-edge-with-custom-vision-service"></a>Руководство. Классификация изображений в пограничной системе с помощью Пользовательского визуального распознавания
...
* В качестве устройства Edge можно использовать компьютер, на котором ведется разработка, или виртуальную машину. Для этого выполните действия, описанные в кратком руководстве для устройства [Linux](quickstart-linux.md).
* В настоящее время модуль Пользовательского визуального распознавания доступен только в качестве контейнера Linux для архитектуры x64. 

Облачные ресурсы.

* [Центр Интернета вещей](../iot-hub/iot-hub-create-through-portal.md) цен. категории "Стандартный" в Azure. 
* Реестр контейнеров. В этом руководстве используется [Реестр контейнеров Azure](https://docs.microsoft.com/azure/container-registry/). 
    * Узнайте учетные данные [учетной записи администратора](../container-registry/container-registry-authentication.md#admin-account) для реестра контейнеров.
...
* Расширение [Azure IoT Edge](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-edge) для Visual Studio Code.
...
4. Создайте проект со следующими значениями:
   ...
   | Группа ресурсов | Примите **ограниченную пробную версию** по умолчанию. |
...
5. Нажмите кнопку **Upload 10 files** (Передать 10 файлов). 
   ...
   ![Отправка файлов с тегом hemlock](./media/tutorial-deploy-custom-vision/upload-hemlock.png)
...
1. После тренировки классификатора щелкните **Экспортировать** на странице производительности классификатора. 
   ...
   ![Экспорт классификатора изображений](./media/tutorial-deploy-custom-vision/export.png)
...
2. В окне встроенного терминала введите следующую команду для установки (или обновления) пакета **cookiecutter**, который вы используете для создания шаблона модуля python IoT Edge в VS Code:
...
Предварительные требования для этого руководства, указанные в реестре контейнеров, который необходим для хранения образов контейнеров для созданных модулей. Необходимо предоставить учетные данные для доступа к реестру в двух местах: в Visual Studio Code, чтобы можно было создавать и передавать образы в реестр, а также в манифесте развертывания, чтобы устройство Edge IoT могло извлечь и развернуть изображения. 

Если вы используете реестр контейнеров Azure, убедитесь, что знаете имя пользователя, сервер входа и пароль для [учетной записи администратора](../container-registry/container-registry-authentication.md#admin-account). 

1. В Visual Studio Code откройте интегрированный терминал, последовательно выбрав элементы **Вид** > **Терминал**. 

2. Введите следующую команду в окне интегрированного терминала: 

    ```csh/sh
    docker login -u <registry username> <registry login server>
    ```

3. При запросе укажите пароль для реестра и нажмите клавишу **ВВОД**.

4. Откройте файл **.env** в папке решений. Этот файл игнорируется git и хранит учетные данные реестра, чтобы вам не требовалось жестко задавать их в шаблоне файла развертывания. 

5. Укажите имя пользователя и пароль для реестра контейнеров без кавычек вокруг значений. 
...
Затем в Visual Studio Code настройте доступ к Центру Интернета вещей. 
...
Наконец, выберите устройство и разверните решение.
...
В Visual Studio Code щелкните правой кнопкой мыши имя устройства IoT Edge и выберите **Start monitoring D2C message** (Начать мониторинг сообщений D2C). 
