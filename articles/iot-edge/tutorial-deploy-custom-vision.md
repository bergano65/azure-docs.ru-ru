...
> [!NOTE]
> Возможно, что изначально в выходных данных модуля cameraCapture будут отображаться некоторые ошибки подключения. Это вызвано задержкой между развертыванием и запуском модулей.
>
> Модуль cameraCapture будет автоматически повторять попытки до успешного подключения. После этого вы должны увидеть приведенные ниже сообщения о классификации изображений.
