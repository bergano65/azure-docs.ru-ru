...
2. В окне встроенного терминала введите следующую команду для установки (или обновления) пакета **cookiecutter**, который вы используете для создания шаблона решения IoT Edge в VS Code:
    ...
    ```
   >[!Note]
   >Убедитесь, каталог установки cookiecutter будет включен в переменную `Path` среды, чтобы его можно было вызвать из командной строки. Как правило, на Windows нужно добавить путь `%APPDATA%\Python\PythonVersion\Scripts`, где вместо PythonVersion должна быть версия Python.
