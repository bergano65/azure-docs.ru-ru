...
## <a name="iot-edge-behind-a-gateway-cannot-perform-http-requests-and-start-edgeagent-module"></a>IoT Edge за шлюзом не удается выполнить HTTP-запросы и запустить модуль edgeAgent

**Наблюдаемое поведение**

Управляющая программа IoT Edge активна с допустимым файлом конфигурации, но не может запустить модуль edgeAgent. Команда `iotedge list` возвращает пустой список. Отчет о журналах управляющей программы IoT Edge `Could not perform HTTP request` .

**Основная причина:**

IoT Edge устройства, расположенные за шлюзом, получают свои образы модулей с родительского IoT Edge устройства, указанного в `parent_hostname` поле файла config. YAML. Эта `Could not perform HTTP request` ошибка означает, что дочернее устройство не может связаться с родительским устройством через HTTP.

**Решение:**

Убедитесь, что родительский IoT Edge устройство может принимать входящие запросы от дочернего устройства IoT Edge. Откройте сетевой трафик на портах 443 и 6617 для запросов, поступающих от дочернего устройства.
