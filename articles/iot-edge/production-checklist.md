...
Дополнительные сведения см. в разделе [хранилище узлов для системных модулей](offline-capabilities.md#host-storage-for-system-modules).
...
@no__t 0Option: Установка глобальных ограничений, относящихся ко всем модулям контейнеров @ no__t-0
...
@no__t 0Option: Настройка параметров журнала для каждого модуля контейнера @ no__t-0
...
* Настройте модуль контейнеров для отправки журналов в [журнал](https://docs.docker.com/config/containers/logging/journald/) `systemd`, задав `journald` в качестве драйвера ведения журнала по умолчанию. 
