...
В этой статье показано, как протестировать автоматическую подготовку на устройстве Linux IoT Edge с помощью доверенный платформенный модуль (TPM) (TPM). Вы можете автоматически подготовить Azure IoT Edge устройства для [службы подготовки устройств](../iot-dps/index.yml). При необходимости ознакомьтесь с [процессом автоматической подготовки](../iot-dps/concepts-auto-provisioning.md), прежде чем продолжить.
