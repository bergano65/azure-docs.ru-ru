---
title: Автоматическая подготовка устройств Linux с помощью DPS - Edge Интернета вещей Azure | Документация Майкрософт
...
Устройства Azure IoT Edge может быть автоматически подготовлен с помощью [службы подготовки устройств](../iot-dps/index.yml). При необходимости ознакомьтесь с [процессом автоматической подготовки](../iot-dps/concepts-auto-provisioning.md), прежде чем продолжить. 

В этой статье показано, как проверить автоматической подготовки баз данных на имитированном устройстве IoT Edge, сделав следующее: 
...
В этом разделе создайте новую виртуальную машину Linux на Hyper-V. Вы настроили этой виртуальной машины с помощью имитации доверенного платформенного МОДУЛЯ, которые можно использовать для тестирования принцип работы автоматической подготовки с помощью Edge Интернета вещей. 
...
2. В диспетчере Hyper-V еще раз выберите **New** > **виртуальной машины** в **действия** меню.

3. В ходе работы **Мастера создания виртуальной машины** укажите следующие настройки.

   1. **Укажите поколение**. Выберите **Поколение 2**. Виртуальные машины поколения 2 есть вложенные виртуализация включена, который необходим для выполнения IoT Edge на виртуальной машине.
...
4. Выберите **Готово** мастера, чтобы создать виртуальную машину.
...
После создания виртуальной Машины, откройте его параметры, чтобы включить виртуальный доверенный платформенный модуль (TPM), которая позволяет autoprovision устройства. 

1. Выберите виртуальную машину, а затем откройте его **параметры**.
...
1. Запустить виртуальную машину и подключиться к нему.

2. Следуйте указаниям в запросах на виртуальной машине, чтобы завершить процесс установки и перезагрузите компьютер. 

3. Войдите на виртуальную Машину, а затем выполните действия, описанные в [настроить среду разработки Linux](https://github.com/Azure/azure-iot-sdk-c/blob/master/doc/devbox_setup.md#linux) для установки и создания пакет SDK для устройств Azure IoT для C. 

   >[!TIP]
   >В этой статье будет скопировать и вставить данные из виртуальной машины, который не задачу за счет подключения приложение диспетчера Hyper-V. Вы можете подключиться к виртуальной машине с помощью диспетчера Hyper-V, один раз для получения IP-адрес: `ifconfig`. Затем можно использовать IP-адрес для подключения по протоколу SSH: `ssh <username>@<ipaddress>`.
...
3. Выберите **Добавить отдельную регистрацию** и выполните следующие действия для настройки регистрации.  
   ...
   2. Укажите **ключ подтверждения** и **идентификатор регистрации** , скопированный из виртуальной машины.
   
   3. Выберите **True** объявить, что эта виртуальная машина — устройство IoT Edge. 
   
   4. Выберите связанный **Центр Интернета вещей**, к которому планируется подключение устройства. Вы можете выбрать несколько концентраторов, а устройство будет назначен один из них в соответствии с политикой выбран. 
...
Теперь, когда запись регистрации существует для этого устройства, среда выполнения IoT Edge может выполнять автоматическую подготовку устройства во время установки. 
...
* [Установка среды выполнения Azure IoT Edge в Linux (ARM32v7/armhf)](how-to-install-iot-edge-linux-arm.md)
...
Вы можете проверить, что было использовано индивидуальной регистрации, которая создана в службе подготовки устройств. Перейдите к экземпляру службы подготовки устройств на портале Azure. Откройте сведения о регистрации для отдельной регистрации, который вы создали. Обратите внимание, что находится в состоянии регистрации **назначенный** и указан идентификатор устройства. 
