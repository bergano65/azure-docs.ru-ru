...
Дополнительные сведения см. [в разделе Знакомство со средой выполнения Azure IOT EDGE и ее архитектурой](iot-edge-runtime.md).

В этой статье перечислены действия по установке среды выполнения Azure IoT Edge на устройстве Ubuntu Linux x64 (Intel/AMD) IoT Edge. Список поддерживаемых операционных систем AMD64 см. в [Azure IOT Edge поддерживаемых системах](support.md#operating-systems) .
...
Используйте следующие разделы для установки последней версии службы Azure IoT Edge на устройствах. 
...
Установите конфигурацию репозитория. Выберите фрагмент кода **16,04** или **18,04** , соответствующий выпуску Ubuntu:

> [!NOTE]
> Убедитесь, что выбран фрагмент кода из поля правильный код для вашей версии Ubuntu.
...
Многие производители встраиваемых устройств доставляют образы устройств, которые содержат пользовательские ядра Linux, в которых могут отсутствовать функции, необходимые для совместимости среды выполнения контейнеров. Если при установке рекомендуемой среды выполнения контейнеров [значок Кита](https://github.com/moby/moby) возникли проблемы, вы можете устранить неполадки в конфигурации ядра Linux с помощью скрипта [Check-config](https://raw.githubusercontent.com/moby/moby/master/contrib/check-config.sh) , предоставленного в официальном [репозитории значок Кита GitHub](https://github.com/moby/moby) . выполнение на устройстве следующих команд.
...
В результате будут предоставлены подробные выходные данные, которые содержат сведения о состоянии функций ядра, используемых средой выполнения значок Кита. Необходимо убедиться, что все элементы в разделе `Generally Necessary` и `Network Drivers` включены, чтобы обеспечить полную совместимость ядра с средой выполнения значок Кита.  Если вы определили недостающие компоненты, их можно включить, перестроив ядро из источника и выбрав связанные модули для включения в соответствующий файл kernel. config.  Аналогичным образом, если вы используете генератор конфигурации ядра, например дефконфиг или менуконфиг, вам потребуется найти и включить соответствующие функции и перестроить ядро соответствующим образом.  После развертывания недавно измененного ядра запустите сценарий Check-config еще раз, чтобы убедиться, что идентифицированные компоненты успешно включены.
...
Если вы хотите установить определенную версию Azure IoT Edge, можно ориентироваться на файлы компонентов непосредственно из репозитория IoT Edge GitHub. Чтобы получить все компоненты IoT Edge на устройстве, выполните следующие действия: подсистема значок Кита и интерфейс командной строки, либиоссм и, наконец, управляющая программа безопасности IoT Edge.

1. Перейдите к [Azure IOT Edge](https://github.com/Azure/azure-iotedge/releases)выпускам и найдите целевую версию для выпуска. 

2. Разверните раздел **активы** для этой версии.

3. В любом выпуске могут быть обновления ядра значок Кита. Если вы видите файлы, которые начинаются с **значок Кита-Engine** и **значок Кита-CLI**, используйте следующие команды для обновления этих компонентов. Если вы не видите ни одного файла значок Кита и вы еще не установили значок Кита на устройстве, вернитесь назад к более старым ресурсам выпуска, пока они не будут найдены. 
