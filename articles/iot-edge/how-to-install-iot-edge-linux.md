...
## <a name="install-iot-edge-and-container-runtimes"></a>Установка IoT Edge и сред выполнения контейнеров
...
Выполните действия, описанные в этом разделе, если требуется установить определенную версию значок Кита и среду выполнения Azure IoT Edge, которая недоступна через `apt-get install` . Список пакетов Майкрософт содержит только ограниченный набор последних версий и их подверсий, поэтому эти действия предназначены для всех, кто хочет установить более старую версию или версию-кандидат.

С помощью фигурных команд можно ориентироваться на файлы компонентов непосредственно из репозитория IoT Edge GitHub. Чтобы получить все компоненты IoT Edge на устройстве, выполните следующие действия: подсистема значок Кита и интерфейс командной строки, либиоссм и, наконец, управляющая программа безопасности IoT Edge.
