...
Первым шагом при устранении неполадок IOT Edge должно быть использование `check` команды, которая выполняет сбор данных о конфигурации и тестах подключения для распространенных проблем. Команда доступна в [выпуске Release 1.0.7](https://github.com/Azure/azure-iotedge/releases/tag/1.0.7) и более поздних версий. `check`

Вы можете выполнить `check` команду, как показано ниже, или `--help` включить флаг, чтобы просмотреть полный список параметров:
...
* Проверки конфигурации: Проверка сведений, которые могут препятствовать подключению пограничных устройств к облаку, включая проблемы с *config. YAML* и подсистемой контейнеров.
...
На портале перейдите к разделу **сведения об** > устройстве**набор модулей** > **Настройка дополнительных параметров среды выполнения ребра**. Создайте переменную среды для модуля концентратора ребра с именем *оптимизефорперформанце* , для которого задано *значение false*.
...
Убедитесь, что для отправки сообщений в edgeHub пользовательский модуль IoT Edge всегда использует один и тот же идентификатор процесса. Например `ENTRYPOINT` , убедитесь, что `CMD` вместо команды в файле `CMD` DOCKER используется один идентификатор процесса для модуля, и другой идентификатор процесса для команды `ENTRYPOINT` bash, запускающей основную программу, в то время как при вызове ИДЕНТИФИКАТОР одного процесса.
