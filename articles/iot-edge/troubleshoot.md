...
Можно просматривать сообщения, отправляемые через Центр Edge Интернета вещей и получить подробные сведения из подробных журналов контейнеров в среде выполнения. Чтобы включить ведение подробных журналов в этих контейнерах, задайте значение `RuntimeLogLevel` в файле конфигурации YAML. Чтобы открыть этот файл, выполните следующее.
...
Сетевая конфигурация сети узла препятствует получению доступа к сети агентом Edge Интернета вещей. Сначала агент пытается подключиться через AMQP (порт 5671). Если это не удается, он пытается подключиться через WebSocket (порт 443).
...
## <a name="iot-edge-hub-fails-to-start"></a>Сбой запуска центра Edge Интернета вещей
...
### <a name="resolution"></a>Способы устранения:
Начиная с версии 1.0.7 все процессы модуля разрешено подключаться. Если обновление до 1.0.7 невозможно, выполните следующие действия. Дополнительные сведения см. в разделе [1.0.7 выпуска журнала изменений](https://github.com/Azure/iotedge/blob/master/CHANGELOG.md#iotedged-1).
...
Укажите DNS-сервера для вашей среды в параметры ядра контейнера, которые будут применены ко всем модулям контейнер запущен обработчиком. Создайте файл с именем `daemon.json` указание DNS-сервер для использования. Пример:
...
Можно задать DNS-сервер для каждого модуля *createOptions* в развертывании IoT Edge. Пример:
