...
Первым шагом при устранении неполадок IoT Edge должно быть использование команды `check`, которая запускает набор тестов конфигурации и подключения для распространенных проблем. Команда `check` доступна в [выпуске 1.0.7](https://github.com/Azure/azure-iotedge/releases/tag/1.0.7) и более поздних версиях.
