...
Вы можете убедиться, что модуль sqlFunction был успешно отправлен в реестр контейнеров. На портале Azure перейдите в реестр контейнеров. Выберите **Репозитории** и выполните поиск по имени **sqlFunction**. Другие два модуля (tempSensor и sql) не помещены в реестр контейнеров, так как вы уже использовали ссылки на их репозитории в реестрах Майкрософт.
