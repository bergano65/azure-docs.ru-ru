...
* Устройство AMD64 [с Linux, на котором выполняется Azure IoT Edge](quickstart-linux.md);
  * Устройства ARM, такие как Raspberry Pis, не могут выполнять SQL Server. Если вы хотите использовать SQL на устройстве ARM, вы можете зарегистрироваться, чтобы использовать предварительную версию [Базы данных SQL Azure для пограничных вычислений](https://azure.microsoft.com/services/sql-database-edge/).
...
3. В строке 35 замените строку **\<sql connection string\>** следующей строкой. Свойство **Источник данных** ссылается на контейнер SQL Server, которого пока не существует. Он будет создан в следующем разделе, и будет иметь имя — **SQL**.
...
После указания службе Visual Studio Code создать решение, сначала она запишет данные в шаблон развертывания, а затем в новой папке **config** создаст файл deployment.json. Затем выполняются две команды в интегрированном терминале: `docker build` и `docker push`. Команда "build" создает код и контейнеризирует модуль. Команда "push" передает код в реестр контейнеров, который вы указали при инициализации решения.
