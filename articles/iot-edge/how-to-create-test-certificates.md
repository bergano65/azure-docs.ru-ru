...
8. Убедитесь, что OpenSSL установлен правильно, и убедитесь в отсутствии конфликтов имен с существующими сертификатами. При возникновении проблем в выходных данных сценария должно быть описано, как их исправить в системе.
...
Каждому IoT Edgeу устройству в рабочей среде требуется сертификат ЦС устройства, на который ссылается файл config. YAML. Сертификат ЦС устройства отвечает за создание сертификатов для модулей, выполняющихся на устройстве. Это также то, как устройство IoT Edge проверяет свое удостоверение при подключении к подчиненным устройствам. 
...
Имя устройства шлюза, передаваемое в эти скрипты, не должно совпадать с параметром HostName в файле config. YAML. Сценарии помогают избежать проблем, добавляя строку ". ca" к имени устройства шлюза, чтобы предотвратить конфликт имен в случае, если пользователь настроит IoT Edge с тем же именем в обоих местах. Однако все же рекомендуется не использовать одинаковые имена.
...
## <a name="create-x509-certs-for-downstream-devices"></a>Создание сертификатов X. 509 для подчиненных устройств
...
Проверить подлинность устройства IoT с помощью сертификатов X. 509 можно двумя способами: с помощью самозаверяющих сертификатов или с помощью сертификатов, подписанных центром сертификации. Для самостоятельно подписанной аутентификации X. 509, которая иногда называется проверкой подлинности отпечатков, необходимо создать новые сертификаты для размещения на устройстве IoT.
