...
3. Создайте сертификаты, необходимые для сценария, который необходимо протестировать:
   * [Создайте IOT Edge сертификаты удостоверений устройств](#create-iot-edge-device-identity-certificates) для автоматической подготовки с помощью службы подготовки устройств для центра Интернета вещей.
...
Сертификаты удостоверений устройств используются для подготовки устройств IoT Edge с помощью службы подготовки устройств для центра Интернета вещей Azure (DPS).
...
Имя, передаваемое команде **create_edge_device_ca_certificate** , не должно совпадать с именем параметра HostName в файле config. YAML или идентификатором устройства в центре Интернета вещей.
