...
Если на устройстве Windows уже установлен OpenSSL, этот шаг можно пропустить, но убедитесь, что openssl.exe доступен в переменной среды PATH.
...
2. Создайте сертификат ЦС устройства IoT Edge и закрытый ключ с помощью следующей команды. Укажите имя сертификата ЦС.
   ...
   Эта команда создает несколько файлов сертификатов и ключей. Следующий сертификат и пара ключей должны быть скопированы на устройство IoT Edge и ссылаться на него в файле config. YAML:

   * `<WRKDIR>\certs\iot-edge-device-<CA cert name>-full-chain.cert.pem`
   * `<WRKDIR>\private\iot-edge-device-<CA cert name>.key.pem`

Имя, передаваемое команде **New-кацертседжедевице** , не должно совпадать с именем параметра HostName в файле config. YAML или идентификатором устройства в центре Интернета вещей.
Сценарий помогает избежать проблем, добавляя строку ". ca" к имени сертификата, чтобы избежать конфликта имен в случае, если пользователь настраивает IoT Edge с использованием того же имени в обоих местах.
...
Имя, передаваемое команде **create_edge_device_certificate** , не должно совпадать с именем параметра HostName в файле config. YAML или идентификатором устройства в центре Интернета вещей.
Сценарий помогает избежать проблем, добавляя строку ". ca" к имени сертификата, чтобы избежать конфликта имен в случае, если пользователь настраивает IoT Edge с использованием того же имени в обоих местах.
Однако все же рекомендуется не использовать одинаковые имена.
