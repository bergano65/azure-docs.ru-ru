...
Например, если вы использовали примеры сценариев для [создания демонстрационных сертификатов](how-to-create-test-certificates.md), скопируйте следующие файлы на устройство IOT-погранично:
...
Для этих двух автоматически создаваемых сертификатов имеется возможность установки флага **auto_generated_ca_lifetime_days** в файле config. YAML для настройки количества дней существования сертификатов.

>[!NOTE]
>Существует третий автоматически созданный сертификат, создаваемый диспетчером IoT Edge безопасности, **сертификатом сервера центра IOT Edge**. Этот сертификат всегда имеет 90 дней, но автоматически обновляется до истечения срока действия. Значение **auto_generated_ca_lifetime_days** не влияет на этот сертификат.

Чтобы настроить истечение срока действия сертификата, отличное от 90 дней по умолчанию, добавьте значение в поле "дни" в раздел " **Сертификаты** " файла config. YAML.
...
Если вы указали собственные сертификаты ЦС устройств, это значение по-прежнему применяется к сертификату рабочей нагрузки, при условии, что заданное значение времени существования короче времени существования сертификата ЦС устройства.

После указания флага в файле config. YAML выполните следующие действия.

1. Удалите содержимое `hsm` папки.
