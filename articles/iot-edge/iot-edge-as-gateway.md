...
Шлюзы в решениях IoT Edge обеспечивают подключение устройств и аналитику края для устройств IoT, которые в противном случае не имели бы этих возможностей. Azure IoT Edge может использоваться для удовлетворения любых потребностей в ioT-шлюзе, независимо от того, связано ли это с подключением, идентификацией или краевой аналитикой. Шаблоны шлюза в этой статье относятся только к характеристикам подключения к нисходящему устройству и идентификации устройства, а не как данные устройства обрабатываются на шлюзе.
...
* **Аналитика на грани** - Использование служб ИИ локально для обработки данных, поступающих от устройств вниз по течению, без отправки полной телеметрии в облако. Находите данные и реагируйте на запросы локально, а также отправляйте подмножество данных в Центр Интернета вещей.
...
* **Оффлайн-поддержка** - Устройство шлюза хранит сообщения и двойные обновления, которые не могут быть доставлены в IoT Hub.

Шлюз, который выполняет преобразование протокола, также может выполнять пограничную аналитику, изоляцию устройства, сглаживание трафика и автономную поддержку имеющихся устройств и новых устройств с ограниченными ресурсами. Многие имеющиеся устройства производят данные, которые могут предоставить бизнес-аналитику, однако они не были разработаны с учетом возможности подключения к облаку. Непрозрачные шлюзы позволяют разблокировать и использовать эти данные в решении IoT.
