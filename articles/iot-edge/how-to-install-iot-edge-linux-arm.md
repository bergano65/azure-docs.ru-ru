...
Следуйте приведенным ниже, чтобы установить самую последнюю версию службы Azure IoT Edge на устройствах ARM в Linux. 
...
Если вы хотите установить определенную версию Edge Интернета вещей Azure, можно обращаться и файлы компонентов непосредственно из репозитория GitHub Edge Интернета вещей. Используйте тот же `curl` команд, перечисленных в предыдущих разделах, чтобы получить все IoT Edge компоненты на устройстве: ядро Кита и интерфейса командной строки, libiothsm и наконец управляющей программы безопасности IoT Edge. Единственное отличие заключается в том, что вы замените **aka.ms** URL-адреса с помощью ссылки, указывающие непосредственно до версии каждого компонента, который вы хотите использовать.

Перейдите к [Edge Интернета вещей Azure освобождает](https://github.com/Azure/azure-iotedge/releases)и найти версию выпуска, вы будете работать. Разверните **активы** раздела для версии и выберите файлы, которые соответствуют ваше устройство IoT Edge на устройстве архитектуру. Каждый выпуск Edge Интернета вещей содержит **iotedge** и **libiothsm** файлов. Не все выпуски включают **Кита ядра** или **cli Кита**. Если у вас еще нет Кита обработчик контейнеров установлены, просмотрите более старых версий, пока не найдете ту, которая включает компоненты Кита. 
