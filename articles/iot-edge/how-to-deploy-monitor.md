...
Существует два типа модулей, которые можно добавить в развертывание. Первый — это модуль, основанный на службе Azure, например учетной записи хранения или Stream Analytics. Второй — это модуль, использующий ваш собственной код. Вы можете добавить несколько модулей любого типа в развертывание. 

Если вы создаете развертывание без модулей, с устройств будут удалены все установленные модули. 

>[!NOTE]
>Функции Azure еще не поддерживают автоматическое развертывание службы Azure. Вы можете вручную добавить эту службу к своему развертыванию, применив пользовательское развертывание модуля. 
...
1. В разделе **Параметры реестра контейнеров** на этой странице введите имена и учетные данные всех реестров частных контейнеров, которые содержат образы модулей для этого развертывания. Агент Edge сообщает об ошибке 500, если не может найти учетные данные для реестра контейнеров с образом Docker.
1. В разделе **Модули развертывания** на этой странице щелкните **Добавить**.
...
1. Изучите список развертывания. Для каждого развертывания можно просмотреть следующие сведения:
   ...
   * **Пользовательские метрики**. Число устройств Edge в данных отчетов о развертывании для любых метрик, определенных вами для развертывания.
