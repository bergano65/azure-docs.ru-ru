...
ms.locfileid: "56877459"
---
# <a name="what-is-azure-iot-edge"></a>Что такое Azure IoT Edge

Служба Azure IoT Edge позволяет перенести облачную аналитику и пользовательскую бизнес-логику на устройства, чтобы организация могла сосредоточиться на бизнес-аналитике и не тратить время на управление данными. При настройке вашего программного обеспечения Интернета вещей вы можете развернуть его на устройствах с помощью стандартных контейнеров и отслеживать работу всех компонентов из облака.
...
В каких целях вы будете использовать устройство Azure IoT Edge, зависит от вас. Среда выполнения часто используется для развертывания решений ИИ на шлюзах, которые позволяют агрегировать и обрабатывать данные из разных локальных устройств. Но такая модель развертывания — это только один из вариантов. Конечные устройства также можно использовать как устройства Azure IoT Edge, независимо от того, подключены они к шлюзу или непосредственно к облаку.
