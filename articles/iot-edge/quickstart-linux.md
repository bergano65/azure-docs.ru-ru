...
![Архитектура, используемая при работе с этим кратким руководством](./media/quickstart-linux/install-edge-full.png)
...
![Создание Центра Интернета вещей](./media/quickstart-linux/create-iot-hub.png)
...
![Регистрация устройства](./media/quickstart-linux/register-device.png)
...
Так как устройства IoT Edge отличаются от стандартных устройств Интернета вещей поведением и управлением, укажите в удостоверении, что это устройство IoT Edge, с помощью флага `--edge-enabled`. 
...
1. Чтобы создать устройство с именем **myEdgeDevice** в Центре Интернета вещей, введите следующую команду в Azure Cloud Shell.
   ...
   Если отобразится сообщение об ошибке с ключами политики iothubowner, убедитесь, что в Cloud Shell установлена последняя версия расширения azure-cli-iot-ext. 
...
![Регистрация устройства](./media/quickstart-linux/start-runtime.png)
...
![Регистрация устройства](./media/quickstart-linux/deploy-module.png)
...
Вы также можете просматривать сообщения, которые поступают в Центр Интернета вещей, используя [расширение Azure IoT Toolkit для Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-toolkit). 
...
Если вы создали виртуальную машину и Центр Интернета вещей в новой группе ресурсов, можно удалить эту группу и все связанные с ней ресурсы. Внимательно проверьте содержимое группы ресурсов, чтобы убедиться в том, что ничего нужного в ней не содержится. Если вы не хотите удалять всю группу, можно удалить отдельные ресурсы.
