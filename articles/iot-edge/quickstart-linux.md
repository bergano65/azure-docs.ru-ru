...
Все шаги, описанные в этом разделе, выполняются на устройстве IoT Edge. Если в качестве устройства IoT Edge используется компьютер, вы можете перейти к следующему разделу. Если используется виртуальная машина или дополнительное оборудование, вам нужно подключиться к этому устройству. 
...
Модуль датчика температуры может ожидать подключения к центру Edge, если последней строкой в журнале оказывается **Using transport Mqtt_Tcp_Only**. Попробуйте остановить модуль, и пусть агент Edge перезапустит его. Для остановки можно воспользоваться командой `sudo docker stop SimulatedTemperatureSensor`.
...
Удалите контейнеры, созданные на устройстве средой выполнения IoT Edge. 
