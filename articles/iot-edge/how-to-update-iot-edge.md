...
>[!NOTE]
>`-OfflineInstallationPath` Параметр ищет файл с именем **Микрософт-Азуре-иотедже. cab** в указанном каталоге. Начиная с версии IoT Edge-RC4, можно использовать два CAB-файла: один для устройств AMD64 и один для ARM32. Скачайте правильный файл для устройства, а затем переименуйте файл, чтобы удалить суффикс архитектуры.

Чтобы получить дополнительные сведения о параметрах обновления, используйте `Get-Help Update-IoTEdge -full` команду или обратитесь ко [всем параметрам установки](how-to-install-iot-edge-windows.md#all-installation-parameters).
