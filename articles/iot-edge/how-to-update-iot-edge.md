...
Если вы хотите выполнить обновление до определенной версии управляющей программы безопасности, найдите версию, которую вы хотите использовать, из [IOT Edge выпусков](https://github.com/Azure/azure-iotedge/releases). В этой версии выберите соответствующие файлы **либиоссм-STD** и **iotedge** для устройства. Для каждого файла щелкните правой кнопкой мыши ссылку на файл и скопируйте адрес ссылки. Используйте адрес ссылки для установки конкретных версий этих компонентов:
...
Если вы хотите выполнить обновление до определенной версии управляющей программы безопасности, найдите версию, которую вы хотите использовать, из [IOT Edge выпусков](https://github.com/Azure/azure-iotedge/releases). В этой версии Скачайте файл **Микрософт-Азуре-иотедже. cab** . Затем используйте параметр `-OfflineInstallationPath`, чтобы указать расположение локального файла. Например:
...
Для получения дополнительных сведений о параметрах обновления используйте команду `Get-Help Update-IoTEdge -full` или обратитесь ко [всем параметрам установки](how-to-install-iot-edge-windows.md#all-installation-parameters).
...
Агент IoT Edge и модули концентратора имеют версии-КАНДИДАТы, помеченные одинаковым соглашением. Например, **MCR.Microsoft.com/azureiotedge-Hub:1.0.7-RC2**.

В качестве предварительных версий версии-кандидата не входят в качестве последней версии, предназначенной для обычных установщиков. Вместо этого необходимо вручную ориентироваться на ресурсы для версии RC, которую требуется протестировать. В большинстве случаев установка или обновление версии-КАНДИДАТа выполняется так же, как и для любой другой конкретной версии IoT Edge за исключением одного отличия для устройств Windows. 

В версии-кандидате сценарий PowerShell, который позволяет установить IoT Edgeную управляющую программу безопасности на устройстве Windows и управлять ею, может отличаться от функциональности последней общедоступной версии. Кроме загрузки файла IoT Edge. cab для версии-КАНДИДАТа, также Скачайте сценарий **иотеджесекуритидаемон. ps1** . Используйте [точку с точкой](https://docs.microsoft.com/powershell/module/microsoft.powershell.core/about/about_scripts?view=powershell-7#script-scope-and-dot-sourcing) для запуска скачанного скрипта в текущем источнике. Например: 
