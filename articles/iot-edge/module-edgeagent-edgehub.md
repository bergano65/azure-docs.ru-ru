...
В этой статье представлены требуемые и отображаемые в отчете свойства двойников модулей для среды выполнения. Дополнительные сведения о развертывании модулей на устройствах IoT Edge см. в разделе [сведения о развертывании модулей и устанавливать маршруты в IoT Edge](module-composition.md).

Двойник модуля, включает в себя: 

* **Требуемые свойства**. Серверной частью решения можно задать нужные свойства, а модуль можно считать их. Модуль также можно получать уведомления об изменениях в требуемых свойствах. Требуемые свойства используются совместно с сообщаемыми свойствами для синхронизации модуля конфигурации или условий.

* **Сообщаемые свойства**. Модуль может задать сообщаемые свойства, и серверной частью решения можно считывать и запрашивать их. Сообщаемые свойства используются совместно с требуемыми свойствами для синхронизации модуля конфигурации или условий. 
...
| modules.{ИД_модуля}.restartPolicy | {«Никогда» \| «при сбое» \| «on-unhealthy» \| «всегда»} | Да |
...
Последний элемент информации, копию текущей требуемые свойства, полезно определить ли устройство применил требуемые свойства или по-прежнему работает предыдущего манифеста развертывания.
...
| lastDesiredStatus.code | Данный код ссылается на последние требуемые свойства, используемые агентом Edge Интернета вещей. Допустимые значения: `200` — успех, `400` — недопустимая конфигурация,`412` — недопустимая версия схемы, `417` — нужные свойства пусты, `500` — сбой. |
...
| systemModules.edgeHub.exitCode | Код завершения, сообщаемый контейнером концентратора Edge Интернета вещей, если контейнер завершает работу |
...
| modules.{ИД_модуля}.exitCode | Код завершения, сообщаемый контейнером модуля, если контейнер завершает работу |
...
| storeAndForwardConfiguration.timeToLiveSecs | Время в секундах концентратора IoT Edge сохраняет сообщения, если отключен от конечных точек маршрутизации, является ли центр Интернета вещей или локального модуля. Значение может быть любым положительным целым числом. | Да |
...
| lastDesiredStatus.code | Код состояния, ссылающийся на последние требуемые свойства, используемые концентратором Edge Интернета вещей. Допустимые значения: `200` — успех, `400` — недопустимая конфигурация, `500` — сбой. |
| lastDesiredStatus.description | Текстовое описание состояния. |
