...
Чтобы развернуть модули с помощью Visual Studio Code, сохраните манифест развертывания на локальный компьютер в формате JSON. Путь к файлу, указанный в следующем разделе, потребуется вам для запуска команды, которая применяет конфигурацию к устройству.
