...
* В качестве устройства IoT Edge можно использовать виртуальную машину Azure. Для этого выполните действия, описанные в кратком руководстве для устройства [Linux](quickstart-linux.md) или [Windows](quickstart.md). 
   ...
   >[!TIP]
   >В этом учебнике Visual Studio Code используется для разработки модуля C с помощью контейнеров Linux. Если вы хотите разрабатывать на C для контейнеров Windows, необходимо использовать Visual Studio 2017. Дополнительные сведения см. в статье [Use Visual Studio 2017 to develop and debug modules for Azure IoT Edge (Preview)](how-to-visual-studio-develop-module.md) (Разработка и отладка модулей для Azure IoT Edge (предварительная версия) с помощью Visual Studio 2017).
...
Добавьте код к модулю C, позволяющий ему проверить, превысила ли температура компьютера безопасный порог. При слишком высокой температуре модуль добавляет параметр предупреждения в сообщение перед отправкой данных в Центр Интернета вещей. 
