...
1. Взгляните на типы модулей из раскрывающегося меню:
   ...
   * **Модуль IOT Edge** . Вы предоставляете имя модуля и URI образа контейнера. Например, универсальный код ресурса (URI) образа для примера модуля `mcr.microsoft.com/azureiotedge-simulated-temperature-sensor:1.0`симулатедтемпературесенсор —. Если образ модуля хранится в частном реестре контейнеров, добавьте учетные данные на этой странице для доступа к образу.
