...
1. На верхней панели выберите **задать модули**.
...
На вкладке **Routes (маршруты** ) определяется, как сообщения передаются между модулями и центром Интернета вещей. Сообщения формируются с помощью пар "имя-значение". По умолчанию маршрут называется **Route** и определяется как **из/мессажес/* в $upstream * *. Это означает, что все сообщения, выводимые любыми модулями, отправляются в центр Интернета вещей.  
