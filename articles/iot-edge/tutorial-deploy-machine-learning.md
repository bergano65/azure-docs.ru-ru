...
> Ниже приведены шаги по ее отключению. Однако это не подходит для использования в рабочей среде. Эти шаги необходимы только на устройствах с Linux. 
...
Создайте переменную среды IOTEDGE_HOST с адресом management_uri. (Чтобы установить ее на постоянной основе, добавьте в `/etc/environment`.) Например: 

```cmd/sh
export IOTEDGE_HOST="http://172.17.0.1:15580"
```

Перезапустите службу IoT Edge, чтобы изменения вступили в силу.
...
Теперь вы знаете, где хранится образ контейнера Машинного обучения. В следующем разделе описано, как он работает в качестве развернутого модуля на устройстве IoT Edge. 
