...
После создания сертификата удостоверения устройства необходимо иметь два файла: CER или PEM, который содержит открытую часть сертификата, и файл CER или PEM с закрытым ключом сертификата.

Для подготовки вручную с помощью X. 509 вам потребуются следующие файлы:

* Два набора сертификатов удостоверений устройств и закрытых ключей. Для среды выполнения IoT Edge предоставляется один набор файлов сертификатов или ключей.
* Отпечатки, взятые из сертификатов удостоверений устройств. Значения отпечатков — это 40-шестнадцатеричные символы для хэшей SHA-1 или 64-шестнадцатеричных символов для хэшей SHA-256. Оба отпечатки предоставляются в центре Интернета вещей во время регистрации устройства.
