...
В этой статье показано, как с помощью функций непрерывной интеграции и непрерывного развертывания Azure DevOps Services и Microsoft Team Foundation Server (TFS) быстро и эффективно выполнять сборку, тестирование и развертывание приложений в Azure IoT Edge. 
...
7. Сохраните эти проекты и запишите их после изменения в репозиторий Azure DevOps или TFS.
...
В рамках этого раздела вы создадите конвейер сборки, настроенный для автоматического запуска при записи данных после изменения в пример решения IoT Edge. При этом будут автоматически выполняться содержащиеся внутри модульные тесты.

1. Войдите в учетную запись организации Azure DevOps (**https://**_ваша_учетная_запись_**.visualstudio.com**) и откройте проект, в который внесен пример приложения.

    ![Код записи после изменения](./media/how-to-ci-cd/init-project.png)

1. Перейдите на страницу [Azure IoT Edge для Azure DevOps](https://marketplace.visualstudio.com/items?itemName=vsc-iot.iot-edge-build-deploy) на сайте Azure DevOps Marketplace. Нажмите кнопку **Получить бесплатно** и следуйте указаниям в мастере, чтобы установить это расширение для своей организации Azure DevOps или скачать его в TFS.

    ![Установка расширения](./media/how-to-ci-cd/install-extension.png)

1. В Azure DevOps откройте центр **Сборка и выпуск** и на вкладке **Сборки** выберите **+ Новый конвейер**. Или, если конвейеры сборки уже есть, нажмите кнопку **+ Создать**. 

    ![Новая сборка](./media/how-to-ci-cd/add-new-build.png)

1. При получении запроса выберите тип источника **Azure DevOps Git**. Затем выберите проект, репозиторий и ветвь, где находится ваш код. Выберите **Продолжить**.

    ![Выберите Azure DevOps Git](./media/how-to-ci-cd/select-vsts-git.png)
...
1. Сохраните новый конвейер сборки и поставьте в очередь новую сборку. Нажмите кнопку **Save & queue** (Сохранить и поставить в очередь).

1. Выберите ссылку на сборку на появившейся панели сообщений. Или же перейдите к конвейеру сборки, чтобы просмотреть последнее поставленное в очередь задание сборки.
