...
> [!NOTE]
> Используемые в этой статье «шлюза имя» должно совпадать имя, как использовать в качестве имени узла в файле config.yaml IoT Edge. Имя шлюза должно разрешаться в IP-адрес, с помощью DNS или записи файла host. Связи на основе протокола используется (MQTTS:8883 / AMQPS:5671 / HTTPS:433) должен быть между подчиненное устройство и прозрачным IoT Edge. Если используется брандмауэр между ними, соответствующие порт должен быть открыт.
...
1. Такое имя шлюза, добавляемой в конце соединения string так же, как имя узла в файле config.yaml IoT Edge на устройстве шлюза?
2. Такое имя шлюза, сопоставляются с IP-адрес? Можно разрешить intenmittent подключений с помощью DNS или путем добавления записи файла host на конечном устройстве.
3. Открыты порты в брандмауэре? Связи на основе протокола используется (MQTTS:8883 / AMQPS:5671 / HTTPS:433) должен быть между подчиненное устройство и прозрачным IoT Edge.
