...
* Устройство или виртуальная машина Linux для работы в качестве устройства IoT Edge. Рекомендуется использовать виртуальную машину [Azure IoT Edge on Ubuntu](https://azuremarketplace.microsoft.com/en-us/marketplace/apps/microsoft_iot_edge.iot_edge_vm_ubuntu), предоставляемую корпорацией Майкрософт, которая предварительно установит среду выполнения IoT Edge. Создайте эту виртуальную машину, используя следующую команду:
...
## <a name="connect-the-iot-edge-device-to-iot-hub"></a>Подключение устройства IoT Edge к Центру Интернета вещей
...
* Если вы используете Azure IoT Edge на виртуальной машине Ubuntu, примените строку подключения устройства, которую вы скопировали ранее, для удаленной настройки устройства IoT Edge:

   ```azurecli-interactive
   az vm run-command invoke -g IoTEdgeResources -n EdgeVM --command-id RunShellScript --script '/etc/iotedge/configedge.sh "{device_connection_string}"'
   ```

   Для выполнения оставшихся инструкций получите общедоступный IP-адрес, который был выведен командой создания. Общедоступный IP-адрес можно также узнать на странице общих сведений о виртуальной машине на портале Azure. Подключитесь к виртуальной машине, используя приведенную ниже команду. Замените **{publicIpAddress}** адресом своей виртуальной машины. 

   ```azurecli-interactive
   ssh azureuser@{publicIpAddress}
   ```

* Если вы используете IoT Edge на локальном компьютере или устройстве ARM32, откройте файл конфигурации, расположенный в папке /etc/iotedge/config.yaml, и обновите переменную **device_connection_string** значением, скопированным ранее, затем перезапустите IoT Edge безопасности управляющей программы, чтобы применить внесенные изменения:
...
Вы также можете просматривать сообщения, которые поступают в Центр Интернета вещей, с помощью [расширения Azure IoT Hub Toolkit для Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-toolkit) (прежнее название — "расширение Azure IoT Toolkit"). 
