...
* **Разработка и тестирование**. для сценариев разработки и тестирования Azure IOT Edge с контейнерами Windows можно установить на любой SKU (Pro, Enterprise, Server и т. д.) для windows Build 17763, поддерживающего функцию контейнеров.
...
6. Выполните `Initialize-IoTEdge` команду для подготовки устройства с удостоверением в центре Интернета вещей. Укажите строку подключения устройства для подготовки вручную или выберите один из методов, описанных в предыдущем разделе [автоматической подготовки](#option-2-install-and-automatically-provision) .
   ...
   Если устройство перезагружено после выполнения `Deploy-IoTEdge` , перед запуском запустите сценарий PowerShell с помощью точки `Initialize-IoTEdge` .
...
После новой установки единственным модулем, который вы должны увидеть, является **edgeAgent**. После первого [развертывания модулей IOT Edge](how-to-deploy-modules-portal.md) на устройстве будет запущен другой системный модуль **edgeHub**.
...
| Параметр | Допустимые значения | Комментарии |
| --------- | --------------- | -------- |
| **Вручную** | Отсутствуют | **Параметр Switch**. Если тип подготовки не указан, по умолчанию используется значение вручную.<br><br>Объявляет, что вы предоставите строку подключения устройства для подготовки устройства вручную. |
| **Сможет** | Отсутствуют | **Параметр Switch**. Если тип подготовки не указан, по умолчанию используется значение вручную.<br><br>Объявляет, что вы предоставите идентификатор области службы подготовки устройств (DPS) и идентификатор регистрации устройства для подготовки через DPS.  |
