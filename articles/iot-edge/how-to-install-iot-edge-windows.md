...
5. **Initialize IoTEdge** команда настраивает среду выполнения IoT Edge на вашем компьютере. По умолчанию используется Подготовка вручную с помощью контейнеров Windows. 
...
6. **Initialize IoTEdge** команда настраивает среду выполнения IoT Edge на вашем компьютере. По умолчанию используется Подготовка вручную с помощью контейнеров Windows. Использовать `-Dps` флаг для использования в службе подготовки устройств вместо Подготовка вручную.
...
Команды Initialize-IoTEdge настраивает Edge Интернета вещей с помощью своей строкой подключения устройства и данные о выполнении. Большая часть информации, создаваемой этой командой хранится в файле iotedge\config.yaml. Команда инициализации принимает эти общие параметры, среди прочего. Полный список, используйте команд `Get-Help Initialize-IoTEdge -full`. 
