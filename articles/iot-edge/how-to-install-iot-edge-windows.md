...
1. Команда **Initialize-IoTEdge** настраивает среду выполнения IoT Edge на вашем компьютере. По умолчанию при выполнении команды применяется подготовка вручную с помощью контейнеров Windows. `-Dps` Используйте флаг, чтобы использовать службу подготовки устройств вместо подготовки вручную.
...
1. При появлении запроса укажите идентификатор области в службе подготовки устройств и идентификатор регистрации с устройства, которые должны быть получены на шаге 1.
