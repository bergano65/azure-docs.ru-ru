...
Во время установки загружаются три файла:
...
* Установка визуального пакета c'' redistributable (VC время выполнения)

Если устройство будет находиться в автономном режиме во время установки, или если вы хотите установить конкретную версию IoT Edge, вы можете скачать эти файлы заранее на устройство. Когда пришло время установить, укажите сценарий установки в каталоге, который содержит загруженные файлы. Установщик сначала проверяет этот каталог, а затем загружает только ненайденные компоненты. Если все файлы доступны в автономном режиме, вы можете установить без подключения к Интернету.

Вы также можете использовать параметр пути установки в автономном режиме для обновления IoT Edge. Дополнительные сведения см. в разделе [Обновление управляющей программы безопасности и среды выполнения IoT Edge](how-to-update-iot-edge.md).
...
4. Дополнительно загрузите установщик для Visual C' redistributable. Например, в скрипте PowerShell используется эта версия: [vc_redist.x64.exe.](https://download.microsoft.com/download/0/6/4/064F84EA-D1DB-4EAA-9A5C-CC2F0FF6A638/vc_redist.x64.exe) Сохраните установщик в той же папке на устройстве IoT, что и файлы IoT Edge.

5. Чтобы установить с автономными компонентами, [точка источника](https://docs.microsoft.com/powershell/module/microsoft.powershell.core/about/about_scripts?view=powershell-7#script-scope-and-dot-sourcing) локальной копии сценария PowerShell. Затем используйте `-OfflineInstallationPath` параметр как `Deploy-IoTEdge` часть команды и предоставьте абсолютный путь к каталогу файлов. Например,
   ...
   Команда развертывания будет использовать все компоненты, найденные в предоставленном локальном каталоге файлов. Если либо файл .cab, либо установщик Visual C', отсутствуют, он попытается загрузить их.

6. Запустите `Initialize-IoTEdge` команду, чтобы предоставить устройство с идентификацией в Концентраторе IoT. Либо предоставьте строку подключения устройства для ручной подготовки, либо выберите один из методов, описанных в предыдущем разделе [автоматического обеспечения.](#option-2-install-and-automatically-provision)

   Если устройство перезапустилось `Deploy-IoTEdge`после запуска, точка источника `Initialize-IoTEdge`PowerShell сценарий снова перед запуском.

Для получения дополнительной информации о варианте установки в автономном режиме, пройдите вперед, чтобы узнать обо [всех параметрах установки.](#all-installation-parameters)
...
| **ПерезагрузкаНенужно** | Нет | Этот флаг позволяет скрипту развертывания перезапустить машину без запроса, если это необходимо. |
...
| Параметр | Допустимые значения | Комментарии |
| --------- | --------------- | -------- |
| **Силы** | Нет | Этот флаг вынуждает неустановку в случае неудачной предыдущей попытки удалить.
| **ПерезагрузкаНенужно** | Нет | Этот флаг позволяет удалив скрипт перезапустить машину без побуждения, если это необходимо. |
