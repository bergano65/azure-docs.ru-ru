---
title: Медиа-службы Azure — источник Event Grid
...
---
# <a name="azure-media-services-as-an-event-grid-source"></a>Медиа-службы Azure как источник сетки событий
...
### <a name="monitoring-job-output-state-changes"></a>Мониторинг изменений состояния состояния выполнения задания

Задание может содержать несколько выходных работ (если настроить преобразование с несколькими выходными работами). Если требуется отслеживать детали индивидуального вывода задания, прослушайте событие изменения вывода задания.

Каждая **работа** будет на более высоком уровне, чем **JobOutput**, таким образом, события вывода рабочих мест увольняют внутри соответствующей работы. 

Сообщения об `JobFinished`ошибке в , `JobCanceled` `JobError` вывести агрегированные результаты для каждого вывода задания - когда все они закончены. Принимая во время вывода задания, события срастая по мере завершения каждой задачи. Например, если у вас есть выход кодирования, за которым следует выход ВидеоАналитики, вы получите два события, срабатывающие в качестве событий вывода рабочих мест, до финального события JobFinished с агрегированными данными.
...
### <a name="monitoring-job-output-progress"></a>Мониторинг хода выполнения работ
...
> [!NOTE]
> Все события уровня трека поднимаются после подключения живого кодера.

Типы событий на уровне треков:
...
| Microsoft.Media.LiveEventIncomingStreamsOutOfSync | Медиа-сервер обнаруживает, что аудио- и видеопотоки не синхронизированы. Используйте в качестве предупреждения, поскольку пользовательский опыт не может быть затронут. |
| Microsoft.Media.LiveEventIncomingVideoStreamsOutOfSync | Медиа-сервер обнаруживает, что любой из двух видеопотоков, поступающих от внешнего кодера, не синхронизирован. Используйте в качестве предупреждения, поскольку пользовательский опыт не может быть затронут. |
| Microsoft.Media.LiveEventIngestHeartbeat | Публикуется каждые 20 секунд для каждой дорожки, когда выполняется событие прямой трансляции. Предоставляет сводку работоспособности приема.<br/><br/>После того, как кодеран был первоначально подключен, событие сердцебиения продолжает излучать каждые 20 сек ли кодер по-прежнему подключен или нет. |
...
| state | строка | Новое состояние задания в этом событии. Например, "Расписание: задание готово к запуску" или "Завершено: задание закончено".|
...
Вы можете найти коды результатов ошибки в [кодах ошибок события в реальном времени.](../media-services/latest/live-event-error-codes.md)
...
- [Коды ошибок, связанных с трансляцией](../media-services/latest/live-event-error-codes.md)
