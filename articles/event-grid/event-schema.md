...
Источники событий отправляют события в службу "Сетка событий Azure" в массиве, в котором может быть несколько объектов событий. При отправке событий в службу "Сетка событий" общий размер массива не может превышать 1 МБ. Каждое событие в массиве ограничен 64 КБ (общедоступная) или 1 МБ (Предварительная версия). Если событие или массив превышает пределы размера, поступает ответ **413 (слишком большой объем полезных данных)** .
