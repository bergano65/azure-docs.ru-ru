...
description: Описывает свойства и схему, которые присутствуют для всех событий.События включают пять обязательных свойств строки и обязательный объект данных.
services: event-grid
author: banisadr
manager: timlt
...
| -------- | ---- | -------- | ----------- |
| Раздел | строка | Нет, но если включено, должен точно соответствовать идентификатору менеджера ресурсов Azure, который соответствует идентификатору менеджера ресурсов Event Grid. Если это не включено, Event Grid будет штамп на событие. | Полный путь к ресурсу для источника событий. Это поле защищено от записи. Это значение предоставляет служба "Сетка событий". |
...
| идентификатор | строка | Да | Уникальный идентификатор события. |
| . | объект | нет | Данные события, относящиеся к поставщику ресурсов. |
| dataVersion | строка | Нет, но будет штамп с пустой стоимости. | Версия схемы для объекта данных. Версию схемы определяет издатель. |
| metadataVersion | строка | Не требуется, но если включено, должны `metadataVersion` соответствовать схеме сетки событий точно (в настоящее время, только `1`). Если это не включено, Event Grid будет штамп на событие. | Версия схемы для метаданных события. Служба "Сетка событий" определяет схему свойств верхнего уровня. Это значение предоставляет служба "Сетка событий". |
...
* [Медиа-услуги](../media-services/latest/media-services-event-schemas.md?toc=%2fazure%2fevent-grid%2ftoc.json)
