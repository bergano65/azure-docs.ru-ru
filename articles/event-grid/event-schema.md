...
Источники событий отправляют события в службу "Сетка событий Azure" в массиве, в котором может быть несколько объектов событий. При отправке событий в службу "Сетка событий" общий размер массива не может превышать 1 МБ. Каждое событие в массиве ограничено 1 МБ. Если событие или массив превышает пределы размера, поступает ответ **413 (слишком большой объем полезных данных)**. При выполнении операций за 64 КБ увеличивается. Таким образом, события свыше 64 КБ приводят к оплате операций, как будто они были несколькими событиями. Например, событие, 130 КБ, вызовет операции, как будто это было 3 отдельными событиями.
...
| Свойство | Type | Обязательный | Описание |
| -------- | ---- | -------- | ----------- |
| Раздел | строка | Нет, но если он указан, он должен точно соответствовать разделу сетки событий Azure Resource Manager ID. Если не включена, то сетка событий будет отмечаться на событие. | Полный путь к ресурсу для источника событий. Это поле защищено от записи. Это значение предоставляет служба "Сетка событий". |
...
| . | object | Нет | Данные события, относящиеся к поставщику ресурсов. |
...
| metadataVersion | строка | Не требуется, но если он указан, должен точно соответствовать схеме `metadataVersion` сетки событий (в настоящее время `1`только). Если не включена, то сетка событий будет отмечаться на событие. | Версия схемы для метаданных события. Служба "Сетка событий" определяет схему свойств верхнего уровня. Это значение предоставляет служба "Сетка событий". |
