...
description: Подписки Azure, реестр контейнеров, пользовательские разделы, концентраторы событий, центр Интернета вещей, Key Vault, службы мультимедиа, группы ресурсов, служебная шина, хранилище, карты, Конфигурация приложения, сигнал R, Машинное обучение.
services: event-grid
author: spelluru
...
| [Схема событий службы "Сетка событий Azure" для подписок](event-schema-event-hubs.md) | Содержит сведения о полях в событиях в Центрах событий. |
...
Интеграция Key Vault со службой "Сетка событий Azure" сейчас доступна в предварительной версии. 
...
| [Схема событий службы "Сетка событий Azure" для подписок](event-schema-key-vault.md) | Показывает поля в Key Vault событиях. |

## <a name="media-services"></a>Службы Media Services
...
| [Схема событий службы "Сетка событий Azure" для подписок](../media-services/latest/media-services-event-schemas.md?toc=%2fazure%2fevent-grid%2ftoc.json) | Содержит сведения о полях в событиях Служб мультимедиа. |
...
| [Схема событий службы "Сетка событий Azure" для подписок](event-schema-service-bus.md) | Содержит сведения о полях в событиях служебной шины. |
...
>[!NOTE]
> Интеграцию событий поддерживают только учетные записи хранения типа **StorageV2 (версия 2, общего назначения)** и **BlobStorage**. **Хранилище (общее назначение** версии 1) *не поддерживает интеграцию со службой "* сетка событий".
...
| [Схема событий службы "Сетка событий Azure" для подписок](event-schema-blob-storage.md) | Содержит сведения о полях в событиях хранилища BLOB-объектов. |
...
| [Схема событий службы "Сетка событий Azure" для подписок](event-schema-app-configuration.md) | Показывает поля в событиях конфигурации приложения Azure. |
...
| [Схема событий службы "Сетка событий Azure" для подписок](event-schema-azure-signalr.md) | Показывает поля в событиях службы Azure SignalR. |
