...
description: Подписки Azure, Реестр контейнеров, пользовательские темы, концентраторы событий, IoT Hub, Key Vault, Медиа-сервисы, группы ресурсов, Сервисный автобус, Хранение, Карты, Конфигурация приложений, Сигнал R, Машинное обучение.
services: event-grid
author: spelluru
...
Подпишитесь на события IoT Hub, чтобы реагировать на события, созданные, удаленные, подключенные, отключенные и телеметрические устройства.
...
## <a name="key-vault-preview"></a>Ключевое Убежище (предварительный просмотр)

Интеграция Key Vault со службой "Сетка событий Azure" сейчас доступна в предварительной версии. 

Подпишитесь на события Key Vault, которые должны быть уведомлены, когда срок действия секрета истекает, срок действия секрета истекает или секрет имеет новую версию. 
...
| [Схема событий службы "Сетка событий Azure" для подписок](event-schema-key-vault.md) | Отображает поля в событиях Key Vault. |
...
>[!NOTE]
> Интеграцию событий поддерживают только учетные записи хранения типа **StorageV2 (версия 2, общего назначения)** и **BlobStorage**. **Хранение (общее назначение v1)** *не* поддерживает интеграцию с Event Grid.
...
Подпишитесь на события Azure App Configuration, чтобы реагировать на события изменения ключевой ценности.
...
| [Схема событий службы "Сетка событий Azure" для подписок](event-schema-app-configuration.md) | Отображает поля в событиях конфигурации приложений Azure. |

## <a name="azure-signalr"></a>Azure SignalR
Подписаться на события службы Azure SignalR для реагирования на события подключения клиентов.
...
| [Схема событий службы "Сетка событий Azure" для подписок](event-schema-azure-signalr.md) | Отображает поля в событиях службы Azure SignalR. |

## <a name="azure-machine-learning"></a>Машинное обучение Azure

Подпишитесь на события рабочего пространства Azure Machine Learning, чтобы ответить на регистрацию моделей.
...
| [Схема событий Azure Event Grid для машинного обучения Azure](event-schema-machine-learning.md) | Отображает поля в событиях машинного обучения Azure. |
