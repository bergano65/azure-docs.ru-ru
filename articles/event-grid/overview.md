...
Сетка событий Azure развертывается для обеспечения максимальной доступности, предопределенным образом распределяясь между несколькими доменами сбоя в каждом регионе, а также между в зонами доступности (в регионах, которые их поддерживают). Сейчас служба "Сетка событий Azure" предоставляется во всех общедоступных регионах. Она пока не предоставляется в облаках Azure для Китая от 21Vianet, Azure для Германии или Azure для государственных организаций.
...
* [Служебная шина](event-handlers.md#service-bus-queue-preview) (предварительная версия)
