...
В следующих разделах подробно описано, как эти параметры защищаются и проходят проверку подлинности.
...
По умолчанию разрешены только соединения по протоколу HTTPS. Это поведение можно переопределить с помощью **inbound__serverAuth__tlsPolicy** конфигурации. В следующей таблице записаны возможные значения этого свойства.

| Возможные значения | Описание |
| ---------------- | ------------ |
| Strict | По умолчанию. Включает только протокол HTTPS
| Активировано | Включает HTTP и HTTPS
| Отключен | Включает только HTTP
...
По умолчанию модуль сетки событий настроен на прием только проверки подлинности на основе сертификатов. При запуске модуль "Сетка событий" получает "Трустбундле" из управляющей программы IoT Edge безопасности и использует его для проверки любого сертификата клиента. Сертификаты клиентов, которые не разрешаются в эту цепь, будут `UnAuthorized`отклонены с помощью.

### <a name="certificate-based-client-authentication"></a>Проверка подлинности клиента на основе сертификата

Проверка подлинности на основе сертификата включена по умолчанию. Можно отключить проверку подлинности на основе сертификатов с помощью свойства **inbound__clientAuth__clientCert__enabled**. В следующей таблице записаны возможные значения.

| Возможные значения | Описание |
| ----------------  | ------------ |
| Да | По умолчанию. Требует, чтобы все запросы в модуль "Сетка событий" представим сертификат клиента. Кроме того, необходимо будет настроить **inbound__clientAuth__clientCert__source**.
| false | Не требовать, чтобы клиент выприсутствовал сертификат.

В следующей таблице записаны возможные значения для **inbound__clientAuth__clientCert__source**

| Возможные значения | Описание |
| ---------------- | ------------ |
| IoT Edge | По умолчанию. Использует Трустбундле IoT Edge для проверки всех сертификатов клиента.

Если клиент по умолчанию представляет собой самозаверяющий, модуль сетки событий отклонит такие запросы. Можно разрешить использовать самозаверяющие сертификаты клиентов с помощью свойства **inbound__clientAuth__clientCert__allowUnknownCA** . В следующей таблице записаны возможные значения.

| Возможные значения | Описание |
| ----------------  | ------------|
| Да | По умолчанию. Позволяет успешно предоставлять самозаверяющие сертификаты.
| false | Запросы будут завершаться неудачей, если представлены самозаверяющие сертификаты.

>[!IMPORTANT]
>В рабочих сценариях может потребоваться задать для **inbound__clientAuth__clientCert__allowUnknownCA** **значение false**.

### <a name="sas-key-based-client-authentication"></a>Проверка подлинности клиента на основе ключа SAS

Помимо проверки подлинности на основе сертификата, модуль сетки событий также может выполнять проверку подлинности на основе ключей SAS. Ключ SAS подобен секрету, настроенному в модуле сетки событий, который должен использоваться для проверки всех входящих вызовов. Клиентам необходимо указать секрет в заголовке HTTP "AEG-SAS-Key". Запрос будет отклонен, `UnAuthorized` если он не совпадает.

Конфигурация для управления проверкой подлинности на основе ключа SAS **inbound__clientAuth__sasKeys__enabled**.

| Возможные значения | Описание  |
| ----------------  | ------------ |
| Да | Разрешает проверку подлинности на основе ключей SAS. Требуется **inbound__clientAuth__sasKeys__key1** или **inbound__clientAuth__sasKeys__key2**
| false | По умолчанию. Проверка подлинности на основе ключа SAS отключена.

 **inbound__clientAuth__sasKeys__key1** и **inbound__clientAuth__sasKeys__key2** — это ключи, настроенные модулем сетки событий для проверки входящих запросов. Необходимо настроить по крайней мере один из ключей. Клиент, выполняющий запрос, должен будет представлять ключ как часть заголовка запроса "**AEG-SAS-Key**". Если настроены оба ключа, клиент может предоставить либо один из ключей.
...
Конфигурация контроля исходящей проверки подлинности клиента **outbound__clientAuth__clientCert__enabled**.

| Возможные значения | Описание |
| ----------------  | ------------ |
| Да | По умолчанию. Требует, чтобы все исходящие запросы из модуля сетки событий присутствовали сертификат. Необходимо настроить **outbound__clientAuth__clientCert__source**.
| false | Модуль сетки событий не требуется для представления его сертификата.

Конфигурация, управляющая источником сертификата, — **outbound__clientAuth__clientCert__source**.
...
Конфигурация для управления политикой назначения веб-перехватчика **outbound__webhook__httpsOnly**.

| Возможные значения | Описание |
| ----------------  | ------------ |
| Да | По умолчанию. Разрешает только подписчики с конечной точкой HTTPS.
| false | Разрешает подписчикам с конечной точкой HTTP или HTTPS.

По умолчанию модуль "Сетка событий" будет проверять сертификат сервера подписчика. Проверку можно пропустить, переопределив **outbound__webhook__skipServerCertValidation**. Возможны следующие значения:

| Возможные значения | Описание |
| ----------------  | ------------ |
| Да | Не проверять сертификат сервера подписчика.
| false | По умолчанию. Проверка сертификата сервера подписчика.

Если сертификат подписчика является самозаверяющим, модуль сетки событий по умолчанию будет отклонять такие подписчики. Чтобы разрешить самозаверяющий сертификат, можно переопределить **outbound__webhook__allowUnknownCA**. В следующей таблице записаны возможные значения.
...
>[!IMPORTANT]
>В рабочих сценариях необходимо задать для **outbound__webhook__allowUnknownCA** **значение false**.
...
Ниже приведены принципы использования GUID при настройке.
