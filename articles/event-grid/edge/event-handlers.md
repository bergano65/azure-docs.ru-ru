...
Для публикации в облачной конечной точке службы "Сетка событий Azure" задайте для `endpointType` значение `eventGrid` и укажите:
...
Чтобы опубликовать данные в модуле концентратора ребра, задайте для `endpointType` значение `edgeHub` и укажите:
...
## <a name="storage-queues"></a>Очереди хранилища
