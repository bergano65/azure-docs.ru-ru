...
Метрики будут доступны на `5888/metrics` модуля для HTTP и `4438/metrics` для HTTPS. Например, `http://<modulename>:4438/metrics?api-version=2019-01-01-preview` для HTTP. На этом этапе модуль метрик может опрашивать конечную точку для сбора метрик, как в этом [примере архитектуры](https://github.com/veyalla/ehm).
...
| деливерифаилурекаунтс | Число неудачных попыток доставки событий в настроенную конечную точку
...
| експиредкаунтс | Число событий, которые не удалось доставить 