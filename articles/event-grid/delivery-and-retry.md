...
## <a name="delayed-delivery"></a>Отложенной доставки

Как конечную точку возникают сбои доставки, сетка событий начинается задержка доставки и повторной попытке события для этой конечной точки. Например, при сбое первых десяти события, публикуемые для конечной точки "Сетка событий" будет предполагается, что конечная точка возникают проблемы с и приведет к задержке всех последующих попыток *и новые* доставок некоторое время — в некоторых случаях до нескольких часов .

Функциональное назначение отложенной доставки является защита неработоспособные конечные точки, а также системы "Сетка событий". Без задержки и задержка доставки неработоспособные конечные точки политику повторов сетки событий и возможности тома может легко превысить системы.
...
Случае сетка события считает **только** следующие коды ответов HTTP как успешных доставок. Все другие состояния коды считаются ошибках доставки и будет выполнена повторно или deadlettered соответствующим образом. При получении кодом состояния успешного выполнения, случае сетка события считает доставку завершения.
...
Не в наборе выше (200 204), все другие коды считаются ошибками и будет повторена. Некоторые из них имеют политики повторов, определенных, привязанным к ним описанные ниже, все остальные следуйте стандартной экспоненциальной отсрочки модели. Важно помнить, что из-за высокой степенью параллелизма природа архитектуры сетки событий, поведение повтора является недетерминированным. 

| status code | Поведение при повторе |
| ------------|----------------|
| 400 — недопустимый запрос | Повтор через 5 минут или больше (недоставленных сообщений немедленно в том случае, если программа установки недоставленных сообщений) |
| 401 — недостаточно прав | Повтор через 5 минут или больше |
| 403. Запрещено | Повтор через 5 минут или больше |
| 404 — не найдено | Повтор через 5 минут или больше |
| 408 — Истекло время ожидания запроса | Повторите попытку через 2 минуты или больше |
| 413 — размер запрашиваемой сущности слишком большой | Выполняет повторную попытку через 10 секунд или более (недоставленных сообщений немедленно в том случае, если программа установки недоставленных сообщений) |
| 503 — Служба недоступна | Повторите попытку через 30 секунд или несколько |
| Все остальные | Выполняет повторную попытку через 10 секунд или более |
