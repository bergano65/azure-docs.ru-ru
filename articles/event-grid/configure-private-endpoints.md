...
> [!IMPORTANT]
> Функции закрытых конечных точек доступны для разделов и доменов только на уровне Premium. Сведения о переходе с уровня "базовый" на уровень "Премиум" см. в статье [Обновление ценовой категории](update-tier.md) . 
...
3. На странице **ресурсов** выполните следующие действия. 
...
4. На странице **Конфигурация** выберите подсеть в виртуальной сети, в которую требуется развернуть закрытую конечную точку. 
...
5. На странице **теги** создайте теги (имена и значения), которые нужно связать с ресурсом частной конечной точки. Затем в нижней части страницы нажмите кнопку **Просмотр и создание** . 
6. На странице **Просмотр и создание**проверьте все параметры и выберите **создать** , чтобы создать частную конечную точку. 
...
| Действие службы | Состояние частной конечной точки объекта-получателя службы | Description |
|--|--|--|
...
- для `group-ids`укажите `topic` или `domain`. В предыдущем примере используется `topic`. 
...
- Раздел "Сетка событий Azure" (уровень "Премиум")
