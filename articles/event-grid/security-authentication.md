...
Если при создании подписки на события отображается сообщение об ошибке, например "попытка проверить указанную конечную точку HTTPS: \//конечная точка — не пройдена. Дополнительные сведения см. по адресу https: \//, то есть. MS/есвалидатион ". Это означает, что в подтверждении проверки произошла ошибка. Чтобы устранить эту ошибку, проверьте следующие аспекты:
