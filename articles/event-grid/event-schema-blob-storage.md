...
| рекурсивные | string | `True` выполнить операцию со всеми дочерними каталогами; в противном случае `False`. <br>Отображается только для событий, запускаемых в учетных записях хранения BLOB-объектов с иерархическим пространством имен. |
