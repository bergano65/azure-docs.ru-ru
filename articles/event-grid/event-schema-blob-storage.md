...
> [!NOTE]
> Использование конечной точки DFS *`(abfss://URI) `* для учетных записей с включенным неиерархическим пространством имен не приведет к формированию событий. Для таких учетных записей события будут создаваться только конечной точкой большого двоичного объекта *`(wasb:// URI)`* .
