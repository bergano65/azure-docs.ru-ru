...
Работу доменов событий можно легко объяснить на примере. Допустим, вы управляете строительной техникой Contoso, где вы производите тракторы, рытье оборудования и другой тяжелой техники. В рамках ведения бизнеса вы отправляете клиентам сведения в режиме реального времени об обслуживании оборудования, работоспособности системы, обновлениях контрактов. Вся эта информация поступает на различные конечные точки, включая ваше приложение, конечные точки клиента и другие настроенные компоненты инфраструктуры.
...
RBAC в доменах событий работает так же, как [управляемый контроль доступа](security-authorization.md) в остальной части Сетки событий и Azure. Используйте RBAC для создания и применения определений настраиваемых ролей в доменах событий.

### <a name="built-in-roles"></a>Встроенные роли

В целях упрощения RBAC для работы с доменами событий в службе "Сетка событий" доступны два определения встроенных ролей. Это **Участник EventGrid EventSubscription (предварительная версия)** и **Читатель EventGrid EventSubscription (предварительная версия)**. Вы назначаете эти роли пользователям, которым необходимо подписаться на разделы в вашем домене событий. Вы присваивываете назначение роли только теме, на которую пользователям необходимо подписаться.
...
Чтобы опубликовать события на любую тему в домене событий, нажмите события к конечному пункту домена [так же, как для пользовательской темы.](./post-to-custom-topic.md) Единственное различие заключается в том, что вы должны указать раздел, в который хотите отправить событие.
...
Вот ограничения и квоты, связанные с доменами событий:

- 100 000 тем на домен события 
- 100 доменов событий по подписке Azure 
- 500 подписок на события по теме в домене события
- 50 подписок области доменов 
- 5000 событий в секунду приема (в домен)

Если эти ограничения вас не устраивают, обратитесь к команде разработчиков, [askgrid@microsoft.com](mailto:askgrid@microsoft.com)открыв билет поддержки или отправив электронное письмо. 

## <a name="pricing"></a>Цены
Домены событий используют те же [цены операций,](https://azure.microsoft.com/pricing/details/event-grid/) что и все другие функции в Event Grid.
...
* Чтобы узнать о настройке доменов событий, создании тем, создании подписок на события и публикации событий, [см.](./how-to-event-domains.md)
