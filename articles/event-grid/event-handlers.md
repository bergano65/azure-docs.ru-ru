...
description: Описание поддерживаемых обработчиков событий для службы "Сетка событий Azure". Служба автоматизации Azure, функции, концентраторы событий, гибридные подключения, Logic Apps, служебная шина, хранилище очередей, веб-перехватчики.
services: event-grid
author: spelluru
...
| [Руководство. Автоматизация изменения размера передаваемых изображений с помощью сетки событий](resize-images-on-storage-blob-upload-event.md) | Пользователи отправляют изображения в учетную запись хранения через веб-приложение. При создании большого двоичного объекта в хранилище служба "Сетка событий" отправляет событие приложению-функции, которое изменяет размер отправленного изображения. |
