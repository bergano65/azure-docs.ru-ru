...
description: Описание поддерживаемых обработчиков событий для службы "Сетка событий Azure". Обработчик выполняет последующую обработку полученного события.
services: event-grid
author: spelluru
