...
Поля в следующем фрагменте конфигурации являются новыми и должны быть включены в конфигурацию кластера. `managementSubnetCidr`должен находиться в виртуальной сети кластера и использоваться в Azure для управления кластером.
...
Частный кластер можно развернуть с помощью примеров сценариев, приведенных ниже. После развертывания кластера выполните `cluster get` команду и просмотрите `properties.FQDN` свойство, чтобы определить частный IP-адрес сервера API OpenShift.
...
При изменении DNS-серверах имен в виртуальной сети кластера необходимо будет выполнить обновление в кластере со `properties.RefreshCluster` свойством, имеющим значение, `true` чтобы можно было повторно создать образ виртуальных машин. Это обновление позволит им выбрать новый серверах имен.
