---
title: Часто задаваемые вопросы об Azure Active Directory B2C
...
В Azure AD B2C также добавлена поддержка [пользовательских политик](active-directory-b2c-overview-custom.md). Эти пользовательские политики позволяют разработчикам создавать собственные политики с любыми поставщиками удостоверений, который поддерживает [OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html) или SAML.
...
1. Удалить все **маршруты пользователей (политики)** в клиенте Azure AD B2C.
1. Удалить все **приложений** зарегистрирован в клиенте Azure AD B2C.
1. Затем войдите в [портала Azure](https://portal.azure.com/) как администратор подписки. Используйте же рабочая или учебная учетная запись или ту же учетную запись Майкрософт, которая использовалась для регистрации в Azure.
...
1. Затем выберите всех пользователей (кроме пользователя, от имени которого вы вошли, — администратора подписки). Выберите **удалить** в нижней части страницы и выберите **Да** при появлении запроса.
1. В разделе **управление**выберите **регистрация приложений** (или **регистрация приложений (для прежних версий)** ).
1. Выберите **просмотреть все приложения**
1. Выберите приложение, с именем **b2c-extensions-app**выберите **удалить**, а затем выберите **Да** при появлении запроса.
1. В разделе **управление**выберите **параметры пользователя**.
1. В разделе **подключений к учетной записи LinkedIn**выберите **нет**, а затем выберите **Сохранить**.
1. В разделе **управление**выберите **свойства**
1. В разделе **управление доступом для ресурсов Azure**выберите **Да**, а затем выберите **Сохранить**.
1. Выйдите из портала Azure и затем войти снова обновить доступ.
1. Выберите **Azure Active Directory** в меню слева.
1. На **Обзор** выберите **удалить каталог**. Следуйте инструкциям на экране инструкциям, чтобы завершить процесс.
