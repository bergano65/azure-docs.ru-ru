...
В Azure AD B2C также добавлена поддержка [пользовательских политик](active-directory-b2c-overview-custom.md). Эти пользовательские политики позволяют разработчику создавать собственные политики с любым поставщиком удостоверений, поддерживающим [OpenID Connect Connect](https://openid.net/specs/openid-connect-core-1_0.html) или SAML.
...
1. В разделе **подключения к учетной записи LinkedIn**выберите **нет**, а затем нажмите кнопку **сохранить**.
