...
| Настройка. Форготпассвордлинклокатион <sup>2</sup>| нет| Отображает ссылку Забыли пароль. Возможные значения: `AfterInput` (по умолчанию). ссылка отображается в нижней части страницы или `None` удаляет ссылку "Забыли пароль".|
...
Примечания.
1. Доступен для определения содержимого типа [DataUri](contentdefinitions.md#datauri) `unifiedssp`или `unifiedssd`.
1. Доступен для определения содержимого типа [DataUri](contentdefinitions.md#datauri) `unifiedssp`или `unifiedssd`. [Макет страницы версии](page-layout.md) 1.1.0 и выше.
