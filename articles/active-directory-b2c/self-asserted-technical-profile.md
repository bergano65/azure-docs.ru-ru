...
* Первое утверждение вывода создает ссылку на элемент управления `emailVerificationControl` отображением, который собирает и проверяет адрес электронной почты.
* Пятый запрос на отображение создает ссылку на элемент управления `phoneVerificationControl` отображением, который собирает и проверяет номер телефона.
...
Если конечная политика, которая наследует этот базовый объект `officeNumber` , в дальнейшем указывает как утверждение на **Отображение** :
