...
### <a name="output-claims-sign-up-or-sign-in-page"></a>Страница регистрации или регистрации выходных утверждений

На комбинированной странице регистрации и входа Обратите внимание на следующее при использовании элемента определения содержимого [DataUri](contentdefinitions.md#datauri) , определяющего `unifiedssp` `unifiedssd` тип страницы или.

- Подготавливаются только утверждения имени пользователя и пароля.
- Первые два исходящих утверждения должны быть именем пользователя и паролем (в указанном порядке). 
- Любые другие утверждения не подготавливаются к просмотру; для этих утверждений необходимо либо установить `defaultValue` или вызвать технический профиль проверки форм утверждений. 
...
| Настройка. Форготпассвордлинклокатион <sup>2</sup>| Нет| Отображает ссылку Забыли пароль. Возможные значения: `AfterLabel` (по умолчанию) отображает ссылку непосредственно после метки или после поля ввода пароля при отсутствии метки,  `AfterInput` отображает ссылку после поля ввода пароля, `AfterButtons` отображает ссылку в нижней части формы после кнопок или `None` удаляет ссылку "Забыли пароль".|
