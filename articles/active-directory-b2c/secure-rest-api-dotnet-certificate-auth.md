---
title: Защита службы RESTFUL с помощью сертификатов клиента
...
В связанной статье вы [создадите службу RESTful](rest-api-claims-exchange-dotnet.md), взаимодействующую с Azure Active Directory B2C (Azure AD B2C).
...
* Выполните действия, описанные в статье [Azure Active Directory B2C. Интеграция обмена утверждениями REST API в путях взаимодействия пользователей Azure AD B2C как проверка входных данных](rest-api-claims-exchange-dotnet.md).
...
7. В поле **Пароль** введите пароль сертификата.
    ...
    ![Отправка ключа политики на странице Создание ключа в портал Azure](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-cert-upload.png)
...
5. Сразу после завершения элемента `<Metadata>` добавьте следующий фрагмент XML-кода:
    ...
    ![Настройка XML-элементов ClientCertificate для аутентификации](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-cert-tech-profile.png)
...
3. Проверьте процесс, введя **Test** в поле **Имя**.
    ...
    ![Выделено выделенное текстовое поле имени и отображается ошибка проверки входных данных](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-test.png)
...
![Добавление кода для проверки сертификата](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-code.png)
...
Если необходимо устранить неполадки на этом шаге, ознакомьтесь со статьей [Azure Active Directory B2C: сбор журналов](troubleshoot-with-application-insights.md).
...
* После того как вы ознакомитесь с [пошаговым руководством по началу работы с пользовательскими политиками](custom-policy-get-started.md), рекомендуем создать свой сценарий, используя собственные файлы пользовательской политики. Для справки мы предоставили [образцы файлов политики](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-rest-api-netfw-secure-cert).
