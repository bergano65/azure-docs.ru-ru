...
Преобразует тип `phoneNumber` данных в тип `string` данных.
...
В этом примере утверждение Целлфоненумбер с типом `phoneNumber` значения преобразуется в утверждение целлфоне с типом значения. `string`
...
| Элемент | TransformationClaimType | Тип данных | Примечания |
| ---- | ----------------------- | --------- | ----- |
| InputClaim | фоненумберстринг | строка |  Строковое утверждение для номера телефона. Номер телефона должен быть в международном формате, заканчиваться ведущим символом "+" и кодом страны. Если предоставлено `country` входящее утверждение, номер телефона указывается в локальном формате (без кода страны). |
