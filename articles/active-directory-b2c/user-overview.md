...
description: Узнайте о типах учетных записей пользователей, которые можно использовать в Azure Active Directory B2C.
...
В Azure Active Directory B2C (Azure AD B2C) существует несколько типов учетных записей, которые могут быть созданы. Активный каталог Azure, Active Directory B2B и Active Directory B2C делятся на типы учетных записей пользователей, которые могут быть использованы.
...
- **Учетная запись потребителя** - Учетная запись потребителя используется пользователем приложений, зарегистрированных в Azure AD B2C. Учетные записи потребителей могут быть созданы:
  - Пользователь, проходящий через поток пользователей регистрации в приложении Azure AD B2C
  - Использование API графика Microsoft Graph
...
Потребитель может входить в приложение, защищенное Azure AD B2C, однако не имеет доступа к таким ресурсам Azure, как портал Azure. Потребитель может использовать локальную учетную запись или федеративную учетную запись, такую как Facebook или Twitter. Учетная запись потребителя создается с помощью [регистрации или ввоза пользовательского потока,](user-flow-overview.md)с помощью API Microsoft Graph или с помощью портала Azure.
...
Для получения дополнительной информации об управлении учетными записями потребителей [см.](manage-user-accounts-graph-api.md)

### <a name="migrate-consumer-user-accounts"></a>Миграция учетных записей потребителей

Возможно, вам потребуется перенести существующие учетные записи потребителей из любого поставщика удостоверений в Azure AD B2C. Для получения дополнительной информации [см.](user-migration.md)
