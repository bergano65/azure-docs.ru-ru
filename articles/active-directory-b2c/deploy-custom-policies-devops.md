...
> [!IMPORTANT]
> Управление Azure AD B2C пользовательскими политиками с помощью конвейера Azure в настоящее время использует операции **предварительной версии** , доступные в КОНЕЧНОЙ точке API `/beta` Microsoft Graph. Использование этих API для приложений в рабочей среде не поддерживается. Дополнительные сведения см. в [справочнике по конечной точке бета-версии Microsoft Graph REST API](https://docs.microsoft.com/graph/api/overview?toc=./ref/toc.json&view=graph-rest-beta).
...
Если у вас уже есть регистрация приложения, которая используется для задач автоматизации, убедитесь, что ей было предоставлено разрешение политики**политики** >  **Microsoft Graph** > **. ReadWrite. TrustFramework** в **разрешениях API** регистрации приложения.
...
1. Введите следующие значения для задачи "скрипт PowerShell":
    ...
    * **Аргументы:**
        ...
        Введите следующие значения **аргументов**. Замените `{alias-name}` на псевдоним, указанный в предыдущем разделе.
...
Если задача завершается успешно, добавьте задачи развертывания, выполнив предыдущие шаги для каждого файла настраиваемой политики. Измените значения `-PolicyId` аргументов `-PathToFile` и для каждой политики.
