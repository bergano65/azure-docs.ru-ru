---
title: Настройка требований сложности паролей
...
Во время регистрации или сброса паролей пользователю необходимо ввести пароль, отвечающий требованиям сложности. Правила сложности пароля применяются согласно потоку пользователя. Можно иметь один поток пользователя требует четырехзначный контактный во время регистрации в то время как другой поток пользователя требует восемь строки символа во время регистрации. Например, вы можете использовать поток пользователя с разной сложностью пароля для взрослых и детей.
...
2. Выберите **значок каталога и подписки** в панели инструментов портала, а затем выберите каталог, содержащий ваш клиент Azure AD B2C.
3. На портале Azure ищите **Azure AD B2C.**
...
- **3 из 4: Нижний символ, Персонаж Верхнего регистра, номер (0-9), Символ гарантирует,** что пароль содержит по крайней мере три типа символов. например цифра, знак нижнего и верхнего регистра.
