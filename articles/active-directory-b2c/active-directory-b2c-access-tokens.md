---
title: Запрос маркеров доступа в Azure Active Directory B2C | Документация Майкрософт
...
2. Щелкните **+Добавить** в верхней части меню.
3. Введите **имя** приложения, которое будет служить его описанием для пользователей. Например, введите Contoso API.
4. Переведите переключатель **Include web app / web API** (Включить веб-приложение или веб-API) в положение **Да**.
5. Введите произвольное значение для **URL-адресов ответа**. Например, введите `https://localhost:44316/`. Значение не имеет значения, так как API не должен получать маркер непосредственно из Azure AD B2C.
6. Введите **URI идентификатора приложения**. Это идентификатор, используемый для веб-API. Например, введите notes в поле. **URI идентификатора приложения** будет `https://{tenantName}.onmicrosoft.com/notes`.
7. Чтобы зарегистрировать приложение, щелкните **Создать** .
8. Щелкните приложение, которое вы только что создали, и скопируйте глобальный уникальный **идентификатор клиента приложения**. Позже он вам потребуется в коде.
...
2. Щелкните **Опубликованные области**. Здесь вы определяете разрешения (области), которые могут быть предоставлены другим приложениям.
3. При необходимости добавьте **значения области** (например, read). По умолчанию будут определены области user_impersonation. Их можно игнорировать. Введите описание области в столбце **Имя области**.
...
2. Зарегистрируйте клиентское приложение ([веб-приложение](active-directory-b2c-app-registration.md) или [собственный клиент](active-directory-b2c-app-registration.md)), если у вас еще его нет. Если вы выбрали это руководство в качестве отправной точки, необходимо зарегистрировать клиентское приложение.
3. Щелкните **Доступ через API**.
4. Щелкните **Добавить**.
