...
Последовательность проверки учетных данных пароля владельца ресурса — это последовательность стандартной аутентификации OAUTH, где приложение (проверяющая сторона) меняет допустимые учетные данные, такие как идентификатор пользователя и пароль, для токена идентификатора, токена доступа и токена обновления. 
...
4. Нажмите кнопку **все** и выберите **вход с помощью ROPC**.
...
```
> [!NOTE]
> При создании пользователей с помощью Graph API, приложение должно иметь «openid», «offline_access» и «профиль» разрешений Microsoft Graph.
