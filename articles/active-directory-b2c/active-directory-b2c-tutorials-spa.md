...
Для обновления приложения можно использовать текущий интерфейс **приложений** или новый объединенный интерфейс **Регистрация приложений (предварительная версия**). [Дополнительные сведения об интерфейсе предварительной версии](http://aka.ms/b2cappregintro).
...
1. Щелкните **Регистрация приложений (предварительная версия)** , выберите вкладку **Собственные приложения**, а затем выберите приложение *webapp1*.
1. Щелкните **Проверка подлинности**, а затем выберите **Попробовать новый пользовательский интерфейс** (если это показано).
1. В разделе **Интернет** выберите ссылку **Добавление URI**, введите `http://localhost:6420`, а затем щелкните **Сохранить**.
1. Щелкните **Обзор**.
...
1. В рамках рабочего процесса регистрации появляется страница для сбора данных и проверки личности пользователя с использованием адреса электронной почты. Рабочий процесс регистрации также собирает пароль пользователя и запрашиваемые атрибуты, определенные в потоке пользователя.
    ...
    ![Страница регистрации, представленная потоком входа и регистрации пользователя](./media/active-directory-b2c-tutorials-spa/azure-ad-b2c-sign-up-workflow.png)
