...
## <a name="2-get-a-token"></a>2. Получить токен
После получения кода авторизации можно использовать `code`, чтобы получить маркер для целевого ресурса путем отправки запроса POST на конечную точку `/token`. В Azure AD B2C можно [запросить токены доступа для других API,](access-tokens.md#request-a-token) как обычно, указав их область (ы) в запросе.
...
| --- | --- | --- |
|(арендатор)| Обязательно | Имя вашего арендатора Azure AD B2C|
|(политика)| Обязательно| Поток пользователя, который использовался для получения кода авторизации. Другой поток пользователя в этом запросе использовать нельзя. |
| client_id |Обязательно |Идентификатор приложения, назначенный вашему приложению на [портале Azure](https://portal.azure.com).|
| client_secret | Да, в веб-приложениях | Секрет приложения, который был создан на [портале Azure](https://portal.azure.com/). Секреты клиентов используются в этом потоке для сценариев Web App, где клиент может надежно хранить секрет клиента. Для сценариев Native App (общественный клиент) секреты клиента не могут быть надежно сохранены и, следовательно, не используются в этом вызове. Если вы используете секрет клиента, пожалуйста, измените его на периодической основе. |
...
## <a name="3-use-the-token"></a>3. Используйте токен
...
## <a name="4-refresh-the-token"></a>4. Обновить токен
...
| --- | --- | --- |
|(арендатор)| Обязательно | Имя вашего арендатора Azure AD B2C|
|(политика) |Обязательно |Поток пользователя, который использовался для получения исходного маркера обновления. Другой поток пользователя в этом запросе использовать нельзя. |
| client_id |Обязательно |Идентификатор приложения, назначенный вашему приложению на [портале Azure](https://portal.azure.com). |
| client_secret | Да, в веб-приложениях | Секрет приложения, который был создан на [портале Azure](https://portal.azure.com/). Секреты клиентов используются в этом потоке для сценариев Web App, где клиент может надежно хранить секрет клиента. Для сценариев Native App (общественный клиент) секреты клиента не могут быть надежно сохранены и, следовательно, не используются в этом вызове. Если вы используете секрет клиента, пожалуйста, измените его на периодической основе. |
