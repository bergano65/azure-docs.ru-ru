...
| end_session_endpoint | Нет | URL-адрес конечной точки конечного сеанса. Значение метаданных authorization_endpoint имеет приоритет над `end_session_endpoint` указанным в конечной точке конфигурации OpenID Connect. |
...
|token_endpoint_auth_method| Нет| Указывает, как Azure AD B2C отправляет заголовок проверки подлинности в конечную точку маркера. Возможные значения: `client_secret_post` (по умолчанию), `private_key_jwt` (общедоступная Предварительная версия) и `client_secret_basic` (общедоступная Предварительная версия). Дополнительные сведения см. в [разделе OpenID Connect Connect Client Authentication](https://openid.net/specs/openid-connect-core-1_0.html#ClientAuthentication). |
