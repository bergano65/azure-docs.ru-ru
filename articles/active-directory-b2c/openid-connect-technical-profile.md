...
| IdTokenAudience | Нет | Аудитория id_token. Если указано, Azure AD B2C проверяет, равен `aud` ли утверждение в токене, возвращенном поставщиком удостоверений, указанному в метаданных идтокенаудиенце.  |
...
| issuer | Нет | Уникальный идентификатор поставщика удостоверений OpenID Connect Connect. Значение метаданных издателя имеет приоритет над `issuer` указанным в конечной точке конфигурации OpenID Connect.  Если указано, Azure AD B2C проверяет, равен `iss` ли утверждение в токене, возвращенном поставщиком удостоверений, указанному в метаданных издателя. |
