...
| --------- | -------- | ----------- |
| Включено | Нет  | Возможные значения: `true` или `false`. |
...
| Атрибут | Обязательно | ОПИСАНИЕ |
| --------- | -------- | ----------- |
| DefaultLanguage | Yes | Язык, который будет использоваться по умолчанию для локализованных ресурсов. |
...
| Атрибут | Обязательно | ОПИСАНИЕ |
| --------- | -------- | ----------- |
| Идентификатор | Yes | Идентификатор, который используется для уникальной идентификации локализованных ресурсов. |
...
| Атрибут | Обязательно | ОПИСАНИЕ |
| --------- | -------- | ----------- |
| ElementType | Yes | Ссылается на элемент ClaimType или элемент пользовательского интерфейса в файле политики. |
| ElementId | Yes | Строка, содержащая ссылку на тип утверждения, который уже определен в разделе ClaimsSchema и используется, если **ElementType** имеет значение ClaimType. |
| TargetCollection | Yes | Целевая коллекция. |
...
| Атрибут | Обязательно | ОПИСАНИЕ |
| --------- | -------- | ----------- |
| текст | Yes | Понятная для пользователя отображаемая строка, которая размещается в пользовательском интерфейсе для этого параметра. |
| Значение | Yes | Строковое значение утверждения, связанное с этим вариантом. |
