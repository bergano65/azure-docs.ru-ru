---
title: Примеры и определения API отчетов об использовании
...
* **b2cAuthenticationCount**: количество операций аутентификации за период времени. По умолчанию — за последние 30 дней.  (Необязательно. начальное и конечное `TimeStamp` параметры определяют определенный период времени.) Выходные данные включают `StartTimeStamp` (самая ранняя дата действия для этого клиента) и `EndTimeStamp` (Последнее обновление).
