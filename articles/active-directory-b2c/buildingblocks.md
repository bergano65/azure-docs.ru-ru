...
- InputValidation - Этот элемент позволяет выполнять агрегации булеаев, которые похожи *и* *или .*
...
- [DisplayControls](display-controls.md) - Определяет элементы управления, которые будут отображаться на странице. Элементы управления дисплеями имеют специальную функциональность и взаимодействуют с техническими профилями проверки. Элементы управления дисплея в настоящее время находятся в **предварительном просмотре**.
