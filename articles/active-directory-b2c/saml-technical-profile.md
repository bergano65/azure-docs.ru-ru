...
### <a name="subject-name-output-claim"></a>Претензия к выходу на имя объекта

Чтобы прочитать утверждение SAML **NameId** в **субъекте** как нормализованную претензию, установите претензию **PartnerClaimType** к значению атрибута. `SPNameQualifier` Если `SPNameQualifier`атрибут не представлен, установите претензию **PartnerClaimType** к значению атрибута. `NameQualifier` 
...
Если `SPNameQualifier` оба `NameQualifier` атрибута не представлены в утверждении SAML, `assertionSubjectName`установите претензию **PartnerClaimType.** Убедитесь, что **NameId** является первым значением в XML утверждения. При определении нескольких утверждений Azure AD B2C выбирает значения субъекта из последнего утверждения.

Ниже приводится претензии, возвращенные поставщиком идентификационных данных SAML:
