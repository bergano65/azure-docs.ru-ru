---
title: Настройка регистрации и всасывайтесь в OpenID Connect
titleSuffix: Azure AD B2C
description: Навлаживайте регистрацию и вскакивайте с любым поставщиком идентификационных данных OpenID Connect (IdP) в Azure Active Directory B2C.
...
[OpenID Connect](openid-connect.md) — это протокол проверки подлинности, встроенный поверх OAuth 2.0, который можно использовать для безопасного вхасывания пользователя. Большинство поставщиков идентификационных данных, которые используют этот протокол, поддерживаются в Azure AD B2C. В этой статье объясняется, как можно добавить пользовательских поставщиков удостоверений OpenID Connect в потоки пользователя.
...
1. Убедитесь, что вы используете каталог, содержащий ваш клиент Azure AD B2C, нажав на фильтр **подписки каталога** и выбрав каталог, содержащий ваш арендатор.
...
> [!NOTE]
> Секрет клиента не обязателен. Тем не менее, вы должны ввести секрет клиента, если вы хотите использовать [поток кода авторизации,](https://openid.net/specs/openid-connect-core-1_0.html#CodeFlowAuth)который использует секрет для обмена кода для токена.
...
## <a name="claims-mapping"></a>Отображение претензий
...
* **Идентификатор пользователя**: Введите требование, которое предоставляет *уникальный идентификатор* для пользователя, вписаваемого в пользователя.
* **Имя дисплея**: Введите требование, которое предоставляет *имя дисплея* или *полное имя* пользователя.
* **Имя:** Введите требование, которое предоставляет *имя* пользователя.
* **Фамилия**: Введите требование, которое предоставляет *фамилию* пользователя.
