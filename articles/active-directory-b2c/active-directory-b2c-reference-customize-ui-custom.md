...
5. Выберите соответствующую ценовую категорию, группу ресурсов и подписку. Убедитесь, что флажок **Закрепить на начальной панели** установлен. Щелкните **Create**(Создать).
...
9. Задайте для параметра **Тип доступа** значение **BLOB-объект**. Щелкните **Create**(Создать).
...
3.  При появлении запроса укажите:  Имя учетной записи хранения *storageAccountName*, например *contoso369b2c*.
    ...
    в)  Имя контейнера хранилища BLOB-объектов *containerName*, например *b2c*.
...
1. Откройте браузер и перейдите на страницу *unified.html*, используя полный URL-адрес расположения в вашей учетной записи хранения `https://<storageAccountName>.blob.core.windows.net/<containerName>/unified.html`. Пример: https://contoso369b2c.blob.core.windows.net/b2c/unified.html.
2. Перейдите к https://test-cors.org. Этот сайт позволяет проверить, включен ли механизм CORS на используемой странице.  
   <!--
