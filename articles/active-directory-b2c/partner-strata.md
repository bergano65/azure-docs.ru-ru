...
| 7. | Orchestrator оценивает политики доступа и вычисляет значения атрибутов, которые будут включаться в заголовки HTTP, перенаправляемые в приложение. На этом шаге Orchestrator может обратиться к дополнительным поставщикам атрибутов, чтобы получить сведения, необходимые для правильного задания значений заголовка.|
| 8. | Orchestrator задает значения заголовков и отправляет запрос в приложение.|
| 9. | Теперь пользователь прошел проверку подлинности и имеет доступ к приложению.|
...
1. **Регистрация приложения**
   ...
   а. [Зарегистрируйте Orchestrator как приложение](https://docs.microsoft.com/azure/active-directory-b2c/tutorial-register-applications?tabs=app-reg-ga) в Azure AD B2C клиенте.
   >[!Note]
