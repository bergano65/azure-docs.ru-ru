...
В определении содержимого измените значение `LoadUri` на. `https://<app_name>.azurewebsites.net/home/unified` Ваша пользовательская `ContentDefinition` политика должна выглядеть, как в следующем фрагменте кода:
