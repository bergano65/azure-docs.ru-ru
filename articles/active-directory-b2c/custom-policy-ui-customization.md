...
В определении содержимого измените значение `LoadUri` на `https://<app_name>.azurewebsites.net/home/unified`. `ContentDefinition` настраиваемой политики должен выглядеть, как в следующем фрагменте кода:
...
Azure AD B2C отправляет ISO-код с двумя буквами для языка, `fr` для французского:
