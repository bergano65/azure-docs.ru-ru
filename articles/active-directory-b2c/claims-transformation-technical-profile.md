---
title: Определение технического профиля трансформации претензий
...
Технический профиль преобразования утверждений позволяет выполнять преобразование утверждений на любом шаге оркестрации на пути взаимодействия пользователя. В следующем примере шаг оркестрации вызывает один из технических профилей с удаленной связью, такой как **UnLink-Facebook-OAUTH**. Этот технический профиль вызывает технический профиль преобразования претензий **RemoveAlternativeSecurityIdByIdentityProvider**, который генерирует новый **alternativeSecurityIds2** претензии, которая содержит список пользователей социальных идентичностей, удаляя личность Facebook из коллекций.
