...
|ScriptExecution| 0:1| Поддерживаемые режимы выполнения [JavaScript.](javascript-samples.md) Возможные `Allow` значения: `Disallow` или (по умолчанию).
...
|EnforceIdTokenHintOnLogout| нет|  Заставить передать ранее выданный токен ID в конечную точку выхода в качестве намека на текущую проверку подлинности сеанса конечномго пользователя с клиентом. Возможные значения: `false` (по умолчанию) или `true`. Для получения дополнительной информации [см.](openid-connect.md)  |
...
| Элемент | Вхождения | Описание |
| ------- | ----------- | ----------- |
| DisplayName | 1:1 | Строка, содержащая название технического профиля. |
...
Ниже приводится следующий пример, как определить опору OpenID Connect. Сведения об имени субъекта настраиваются как `objectId`:
