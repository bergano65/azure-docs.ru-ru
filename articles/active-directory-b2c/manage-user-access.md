...
description: Узнайте, как идентифицировать несовершеннолетних, собрать дату рождения и данные о стране/регионе, а также получить согласие с условиями использования в приложении с помощью Azure AD B2C.
...
ms.locfileid: "78184525"
---
# <a name="manage-user-access-in-azure-active-directory-b2c"></a>Управление доступом пользователей в Azure Active Directory B2C

В этой статье обсуждается, как управлять доступом пользователей к приложениям с помощью Azure Active Directory B2C (Azure AD B2C). Управление доступом в приложении включает следующие аспекты:
...
- Сбор данных о рождении и страна/регионе от пользователей.
...
1. Операция [Microsoft Graph API](https://docs.microsoft.com/graph/use-the-api) идентифицирует пользователя как несовершеннолетнего и возвращает данные пользователя в приложение в виде неподписанного токена JSON.

2. Приложение обрабатывает токен JSON и показывает экран несовершеннолетнему, уведомляя их о том, что требуется согласие родителей, и запрашивает согласие родителя в Интернете.

3. Azure AD B2C показывает интерфейс входа в систему, с помощью которого пользователь может выполнить обычный вход, и издает маркер приложению, которое должно включать **legalAgeGroupClassification = "minorWithParentalConsent"**. Приложение собирает адрес электронной почты родителя и проверяет, что тот является взрослым. Для этого используется надежный источник информации, например государственный орган, выдавший удостоверение личности, водительские права или кредитную карту. Если проверка выполнена успешно, приложение предложит несовершеннолетнему пользователю выполнить вход, используя интерфейс Azure AD B2C. Если разрешение отклонено (например, если **legalAgeGroupClassification = "minorWithoutParentalConsent"**), Azure AD B2C возвратит приложению маркер JSON (не дающий права входа), чтобы снова повторить процесс получения согласия. Также есть возможность настроить поток пользователя, позволяющий взрослому или самому несовершеннолетнему получить доступ к учетной записи несовершеннолетнего, например отправляя код регистрации на сохраненный адрес электронной почты несовершеннолетнего или взрослого.

4. Приложение позволяет несовершеннолетнему пользователю отозвать согласие.

5. Когда несовершеннолетний или взрослый отменяет согласие, Microsoft Graph API может быть использован для изменения **согласияProvidedForMinor** **отказать**. Кроме того, приложение может удалить учетную запись несовершеннолетнего при отзыве разрешения. Также есть возможность настроить потока пользователя так, чтобы прошедший аутентификацию несовершеннолетний (или его родитель, войдя с учетной записью несовершеннолетнего) мог отозвать согласие. Azure AD B2C сохраняет для параметра **consentProvidedForMinor** значение **denied**.

Дополнительные сведения об утверждениях **legalAgeGroupClassification**, **consentProvidedForMinor** и **ageGroup** см. в разделе [User resource type](https://developer.microsoft.com/graph/docs/api-reference/beta/resources/user) (Типы ресурсов пользователя). Дополнительные сведения о пользовательских атрибутах см. в статье [Azure Active Directory B2C: использование настраиваемых атрибутов для сбора данных о потребителях](user-flow-custom-attributes.md). При обращении к расширенным атрибутам с помощью API Microsoft Graph необходимо использовать длинную версию атрибута, *например, extension_18b70cf9bb834edd8f38521c2583cd86_dateOfBirth:* *2011-01-01T00:00:00 .*

## <a name="gather-date-of-birth-and-countryregion-data"></a>Сбор данных о дате рождения и данных по стране/региону

Приложения могут полагаться на Azure AD B2C для сбора информации о дате рождения (DOB) и информации о стране/регионе от всех пользователей во время регистрации. Если эта информация отсутствует, приложение может запросить у пользователя ввести ее во время следующей проверки подлинности (входа). Пользователи не могут продолжать работу без предоставления информации о добе и стране/регионе. Azure AD B2C использует эту информацию для определения того, считается ли данное лицо несовершеннолетним в соответствии с нормативными стандартами этой страны/региона.

Индивидуальный поток пользователей может собирать информацию о DOB и страна/регионе и использовать преобразование требований Azure AD B2C для определения **возрастной группы** и сохранения результата (или сохранения информации о DOB и страна/регионе напрямую) в каталоге.
...
2. Если для элемента выбранной страны присутствует узел **MinorConsent**: 
...
Если приложение надежно собрало данные DOB или страны/региона другими методами, приложение может использовать API Graph для обновления записи пользователя с помощью этой информации. Пример:
...
2. Создайте необходимый флажок с надписью "Принять условия использования" и запишите полученное значение во время регистрации пользователя. Это можно сделать, используя как встроенные, так и настраиваемые потоки пользователей.
...
![Диаграмма диаграммы потока, показывающая рекомендуемый поток пользователя принятия](./media/manage-user-access/user-flow.png)
...
- Например, пользовательская политика, которая реализует запрос [A B2C IEF Custom Policy - Sign Up and Sign In with 'Terms of Use' prompt](https://github.com/azure-ad-b2c/samples/tree/master/policies/sign-in-sign-up-versioned-tou)на использование, см.
