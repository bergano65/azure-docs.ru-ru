...
description: Узнайте, как найти дополнительную информацию, получить дату рождения и данные о стране или регионе и принять условия использования в приложении с помощью Azure AD B2C.
...
В этой статье описывается управление доступом пользователей к приложениям с помощью Azure Active Directory B2C (Azure AD B2C). Управление доступом в приложении включает следующие аспекты:
...
- Сбор данных о рождении и стране или регионе от пользователей.
...
1. [Microsoft Graphная операция API](https://docs.microsoft.com/graph/use-the-api) идентифицирует пользователя как дополнительный и возвращает данные пользователя приложению в форме токена JSON без знака.

2. Приложение обрабатывает маркер JSON и отображает дополнительный экран, уведомляя их о необходимости разрешения родителей и запрашивая согласие родителя в сети.
...
5. Когда либо дополнительный, либо Взрослый отменяет согласие, можно использовать Microsoft Graph API, чтобы изменить **консентпровидедформинор** на **отклоненный**. Кроме того, приложение может удалить учетную запись несовершеннолетнего при отзыве разрешения. Также есть возможность настроить потока пользователя так, чтобы прошедший аутентификацию несовершеннолетний (или его родитель, войдя с учетной записью несовершеннолетнего) мог отозвать согласие. Azure AD B2C сохраняет для параметра **consentProvidedForMinor** значение **denied**.

Дополнительные сведения об утверждениях **legalAgeGroupClassification**, **consentProvidedForMinor** и **ageGroup** см. в разделе [User resource type](https://developer.microsoft.com/graph/docs/api-reference/beta/resources/user) (Типы ресурсов пользователя). Дополнительные сведения о пользовательских атрибутах см. в статье [Azure Active Directory B2C: использование настраиваемых атрибутов для сбора данных о потребителях](user-flow-custom-attributes.md). При адресации расширенных атрибутов с помощью Microsoft Graph API необходимо использовать длинную версию атрибута, например *extension_18b70cf9bb834edd8f38521c2583cd86_dateOfBirth*: *2011-01-01T00:00:00Z*.

## <a name="gather-date-of-birth-and-countryregion-data"></a>Сбор данных о дате рождения и стране или регионе

Приложения могут полагаться на Azure AD B2C, чтобы собрать дату рождения (DOB) и сведения о стране или регионе от всех пользователей во время регистрации. Если эта информация отсутствует, приложение может запросить у пользователя ввести ее во время следующей проверки подлинности (входа). Пользователи не могут продолжать работу без предоставления сведений о DOB и стране или регионе. Azure AD B2C использует сведения, чтобы определить, считается ли лицо незначительным в соответствии с нормативными стандартами этой страны или региона.

Настраиваемый поток пользователя может собирать сведения о DOBе и стране или регионе и использовать преобразование утверждений Azure AD B2C, чтобы определить **возрастная группа** и сохранить результат (или непосредственно сохранить сведения о DOB и стране или регионе) в каталоге.
...
Если приложение надежно собрало данные DOB или страны или региона с помощью других методов, приложение может использовать API Graph для обновления записи пользователя этими сведениями. Пример:
...
![Схема диаграммы Flow, показывающая рекомендуемый пользовательский поток принятия](./media/manage-user-access/user-flow.png)
...
- Пример пользовательской политики, в которой реализуется запрос на использование, см. [в разделе B2C инфраструктура процедур идентификации Custom Policy (регистрация и вход с помощью запроса "условия использования"](https://github.com/azure-ad-b2c/samples/tree/master/policies/sign-in-sign-up-versioned-tou)).
