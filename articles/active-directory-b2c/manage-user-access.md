...
description: Узнайте, как определить несовершеннолетним, собирать даты рождения и страны или региона данных и получить принятие условий использования в приложении с помощью Azure AD B2C.
...
- Идет сбор рождения и страны или региона данные пользователей.
...
2. Приложение обрабатывает маркер JSON и показан экран, на дополнительный, уведомлением о том, что требуется согласие родителя и запрос на получение согласия родительского узла в Интернете. 
...
## <a name="gather-date-of-birth-and-countryregion-data"></a>Дата рождения и страны или региона данных сбора

Приложения могут использовать Azure AD B2C для сбора Дата рождения (DOB) и страну или регион из всех пользователей во время регистрации. Если эта информация отсутствует, приложение может запросить у пользователя ввести ее во время следующей проверки подлинности (входа). Пользователям не удается продолжить без предоставления своих данных DOB и страны или региона. Azure AD B2C использует сведения, чтобы определить, считается ли отдельные minor согласно нормативным стандартам страны/региона. 

Поток пользовательских может собирать сведения о DOB и страну или регион и использование преобразования для определения утверждений Azure AD B2C **возрастным группам** и сохранить результат (или напрямую сохранять данные DOB и страны или региона) в каталог.
...
Если приложение надежно собирал DOB или страны или региона данных другими способами, приложение может использовать Graph API для обновления записи пользователя с помощью этой информации. Например:
...
![Поток пользователя для предоставления согласия](./media/manage-user-access/user-flow.png) 
