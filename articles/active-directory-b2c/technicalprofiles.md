...
| DisplayClaims | 0:1 | Список ранее определенных ссылок на типы претензий, представленные поставщиком претензий, относятся к [самоутвержденному техническому профилю.](self-asserted-technical-profile.md) Функция DisplayClaims в настоящее время находится в **предварительном просмотре**. |
...
| ValidationTechnicalProfiles | 0:n | Список ссылок на другие технические профили, которые этот технический профиль использует для проверки. Для получения дополнительной информации смотрите [технический профиль проверки](validation-technical-profile.md)|
...
| IncludeInSso | 0:1 |  Следует ли использовать этот технический профиль, применять поведение одного входного (SSO) для сеанса или вместо этого требовать явного взаимодействия. Этот элемент действителен только в профилях SelfAsserted, используемых в техническом профиле проверки. Возможные значения: `true` (по умолчанию) или `false`. |
...
| Атрибут | Обязательно | Описание |
| --------- | -------- | ----------- |
| name | Да | Имя допустимого протокола, поддерживаемого в Azure AD B2C и используемого в составе технического профиля. Возможные `OAuth1`значения: `OAuth2` `SAML2`, `OpenIdConnect` `Proprietary`, `None`, |
...
Элемент **DisplayClaims** содержит следующий элемент:

| Элемент | Вхождения | Описание |
| ------- | ----------- | ----------- |
| DisplayClaim | 1:n | Ожидаемый тип входящего утверждения. |

Функция DislayClaims в настоящее время находится в **предварительном просмотре**.
...
| DisplayControlReferenceId | нет | Идентификатор [элемента управления дисплея,](display-controls.md) уже определенный в разделе ClaimsSchema в файле политики или файле родительской политики. |
| Обязательно | нет | Указывает, требуется ли претензия дисплея. |

**DisplayClaim** требует, чтобы вы `ClaimTypeReferenceId` `DisplayControlReferenceId`указали либо a, либо .
...
| Атрибут | Обязательно | Описание |
| --------- | -------- | ----------- |
| ReferenceId | Да | Идентификатор технического профиля, уже определенный в файле политики или файле родительской политики. |
