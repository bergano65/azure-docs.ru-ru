...
| Обязательный | Нет | Указывает, требуется ли утверждение на отображение. |

Для **дисплайклаим** требуется указать `ClaimTypeReferenceId` либо `DisplayControlReferenceId`.
