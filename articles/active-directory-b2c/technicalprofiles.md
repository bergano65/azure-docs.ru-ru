...
| инклудеинссо | 0:1 |  Следует ли использовать этот технический профиль в качестве поведения единого входа (SSO) для сеанса или вместо этого требовать явное взаимодействие. Возможные значения: `true` (по умолчанию) или `false`. |
...
| attribute | Обязательно для заполнения | Description |
| --------- | -------- | ----------- |
| Имя | Да | Имя допустимого протокола, поддерживаемого в Azure AD B2C и используемого в составе технического профиля. Возможные значения: `OAuth1`, `OAuth2`, `SAML2`, `OpenIdConnect`, `Proprietary`, `session management`, `self-asserted`или `None`. |
...
| Обязательно для заполнения | Нет | Указывает, требуется ли утверждение на отображение. |
