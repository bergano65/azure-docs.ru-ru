...
|---------|---------|---------|
| Идентификатор | Yes | Уникальный идентификатор технического профиля. Этот идентификатор позволяет указать технический профиль в других элементах в файле политики. Например, в элементах **OrchestrationSteps** и **ValidationTechnicalProfile**. |
...
| Атрибут | Обязательно для заполнения | ОПИСАНИЕ |
| --------- | -------- | ----------- |
| Name | Yes | Имя допустимого протокола, поддерживаемого в Azure AD B2C и используемого в составе технического профиля. Возможные значения: `OAuth1`, `OAuth2`, `SAML2`, `OpenIdConnect`, `WsFed`, `WsTrust`, `Proprietary`, `session management`, `self-asserted` или `None`. |
...
| Атрибут | Обязательно для заполнения | ОПИСАНИЕ |
| --------- | -------- | ----------- |
| Ключ | Yes | Ключ метаданных. Список элементов метаданных см. в описании каждого типа технических профилей. |
...
| StorageReferenceId | Yes | Идентификатор контейнера хранения ключей, который можно указывать в других элементах файла политики. |
...
| Атрибут | Обязательно для заполнения | ОПИСАНИЕ |
| --------- | -------- | ----------- |
| ReferenceId | Yes | Идентификатор преобразования утверждений, который уже определен в файле политики или файле родительской политики. |
...
| Атрибут | Обязательно для заполнения | ОПИСАНИЕ |
| --------- | -------- | ----------- |
| ClaimTypeReferenceId | Yes | Идентификатор типа утверждения, который уже определен в разделе ClaimsSchema файла политики или файла родительской политики. |
...
| Атрибут | Обязательно для заполнения | ОПИСАНИЕ |
| --------- | -------- | ----------- |
| ReferenceId | Yes | Идентификатор технического профиля, который уже определен в файле политики или файле родительской политики. |
...
| Атрибут | Обязательно для заполнения | ОПИСАНИЕ |
| --------- | -------- | ----------- |
| ClaimType | Yes | Идентификатор типа утверждения, который уже определен в разделе ClaimsSchema файла политики. |
