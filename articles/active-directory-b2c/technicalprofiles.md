...
| Атрибут | Обязательно | Описание |
| --------- | -------- | ----------- |
| Имя | Да | Имя допустимого протокола, поддерживаемого в Azure AD B2C и используемого в составе технического профиля. Возможные значения: `OAuth1`, `OAuth2`, `SAML2`, `OpenIdConnect`, `Proprietary`или `None`. |
| Handler | Нет | Если имя протокола имеет значение `Proprietary`, здесь должно быть указано полное имя сборки, которую Azure AD B2C использует для определения обработчика протокола. |
...
Для **дисплайклаим** необходимо указать либо `ClaimTypeReferenceId`, либо `DisplayControlReferenceId`.
