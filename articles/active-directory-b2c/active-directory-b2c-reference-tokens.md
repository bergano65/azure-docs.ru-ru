...
| Version (версия) | `ver` | `1.0` | Версия маркера Идентификации, как определено с помощью Azure AD B2C. |
...
| Ссылка на класс контекста проверки подлинности | `acr` | Не применяется | Используется только с старой политики. |
...
| Область | `scp` | `Read`| Разрешения, предоставленные ресурс для маркера доступа. Несколько предоставленных разрешений разделяются пробелом. |
