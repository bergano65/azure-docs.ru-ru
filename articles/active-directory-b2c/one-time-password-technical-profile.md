...
Первый режим этого технического профиля — создание кода. Ниже приведены параметры, которые можно настроить для этого режима. Созданные коды и попытки будут отслеживаниться в рамках сеанса. 
...
| --------- | -------- | ----------- |
| кодикспиратионинсекондс | Нет | Время в секундах до истечения срока действия кода. Минимум: `60` ; Максимум: `1200` ; По умолчанию: `600` . При каждом указании кода (с помощью `ReuseSameCode` или нового кода) срок действия кода увеличивается. Это время также используется для задания времени ожидания повторных попыток (при достижении максимального количества попыток пользователь блокирует попытки получить новые коды до истечения этого времени). |
