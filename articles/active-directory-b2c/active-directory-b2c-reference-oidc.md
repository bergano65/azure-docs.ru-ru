...
[OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html) расширяет возможности протокола *авторизации* OAuth 2.0 и позволяет использовать его в качестве протокола *проверки подлинности*, что позволяет вам выполнять единый вход с помощью OAuth. В этом протоколе вводится понятие *маркера идентификации*. Это токен безопасности, который позволяет клиенту выполнять идентификацию пользователя и получать основные сведения о профиле пользователя.
...
Узнайте, как [получить собственный клиент, приложение и потоки пользователей B2C](#use-your-own-b2c-directory).
