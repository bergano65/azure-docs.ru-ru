...
| id_token_hint| Нет | Ранее выданный маркер идентификатора для передачи в конечную точку выхода в качестве подсказки о текущем сеансе, прошедшем проверку подлинности конечного пользователя, с клиентом. Гарантирует, что в параметрах приложения Azure AD B2C будетзарегистрированныйURL-адресответа.`post_logout_redirect_uri` `id_token_hint` |
