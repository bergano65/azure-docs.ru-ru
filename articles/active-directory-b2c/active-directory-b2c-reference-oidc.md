...
| scope | ДА | Список областей с разделителями-пробелами. Область `openid` определяет разрешение на вход пользователя и получение данных о пользователе в форме маркеров идентификации. Область `offline_access` является необязательной для веб-приложений. Это означает, что для расширенного доступа к ресурсам приложению потребуется *маркер обновления* . |
...
| redirect_uri | Нет | Параметр `redirect_uri` приложения, в котором приложения могут отправлять и получать ответы на проверку подлинности. Он должен точно соответствовать одному из параметров `redirect_uri`, зарегистрированных в портал Azure, за исключением того, что он должен быть закодирован URL-адресом. |
...
После того как пользователь завершит поток пользователя, в приложение будет возвращен ответ с указанным параметром `redirect_uri` с помощью метода, указанного в параметре `response_mode`. Ответ одинаков для каждого из предыдущих случаев, независимо от потока пользователя.
...
| refresh_token | Токен обновления OAuth 2.0. Приложение может использовать этот токен для получения дополнительных маркеров после истечения срока действия текущего маркера. Маркеры обновления можно использовать для получения доступа к ресурсам в течение продолжительного периода времени. Для получения маркера обновления область `offline_access` должна быть использована как в запросах авторизации, так и в маркерах. |
...
| id_token_hint| Нет | Ранее выданный маркер идентификатора для передачи в конечную точку выхода в качестве подсказки о текущем сеансе, прошедшем проверку подлинности конечного пользователя, с клиентом. @No__t-0 гарантирует, что `post_logout_redirect_uri` является зарегистрированным URL-адресом ответа в параметрах приложения Azure AD B2C. |
