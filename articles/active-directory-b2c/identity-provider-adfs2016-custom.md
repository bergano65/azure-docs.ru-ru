---
title: Добавление ADFS в качестве поставщика идентификационных данных SAML с помощью пользовательских политик
...
- Для того, чтобы Azure принял пароль файла .pfx, пароль должен быть зашифрован с помощью опции TripleDES-SHA1 в утилите Windows Certificate Store Export, в отличие от AES256-SHA256.
...
Чтобы определить учетную запись ADFS в качестве поставщика утверждений, добавьте ее в элемент **ClaimsProviders** в файле расширения политики. Для получения дополнительной информации [см.](saml-technical-profile.md)
...
1. Найдите `<ClaimsProviders>` раздел и добавьте следующий фрагмент XML. Если ваша политика `SM-Saml-idp` уже содержит технический профиль, перейдите к следующему шагу. Для получения дополнительной [single sign-on session management](custom-policy-reference-sso.md)информации см.
...
2. Добавьте следующий элемент **ClaimsExchange**, убедившись, что для идентификатора используется то же значение, которое использовалось для **TargetClaimsExchangeId**.
    ...
    Обновитите значение **TechnicalProfileReferenceId** в идентификатор технического профиля, созданный ранее. Например, `Contoso-SAML2`.
...
11. Укажите **имя правила утверждения**. Для **магазина Attribute**, выберите Активный каталог, добавить следующие **претензии,** а затем нажмите **Закончить** и **OK**.
    ...
    Обратите внимание, что эти имена не будут отображаться в исходящих выпадающих типа претензий. Вы должны вручную ввести их дюйма (Выпадение на самом деле является editable).
