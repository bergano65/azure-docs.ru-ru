...
- Создайте корректное содержимое HTML и добавьте в раздел `<div id="api"></div>` пустой элемент `<body>`. Этот элемент обозначает место, в которое будет добавляться содержимое Azure AD B2C. Ниже приведен пример страницы с минимальным числом элементов.
...
Чтобы использовать активы фирменной символики компании в пользовательском HTML-коде, добавьте следующие теги за пределами тега `<div id="api">`:
