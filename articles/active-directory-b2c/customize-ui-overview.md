...
- Если вы используете [встроенные политики](active-directory-b2c-reference-policies.md) для функций регистрации или входа, сброса пароля или изменения профиля в приложении, используйте [портал Azure, чтобы настроить пользовательский интерфейс](tutorial-customize-ui.md).
...
Служба Azure AD B2C выполняет код в браузере пользователя и использует современный метод — [общий доступ к ресурсам независимо от источника (CORS)](http://www.w3.org/TR/cors/). Во время выполнения содержимое загружается с URL-адреса, указанного в политике. Для разных страниц указываются разные URL-адреса. После объединения содержимого, загруженного с URL-адреса, с HTML-фрагментом, вставленным из Azure AD B2C, оно отображается клиенту.
...
- Если вы используете встроенные политики, то вы можете приступить к настройке пользовательского интерфейса с помощью руководства [Руководство по настройке пользовательского интерфейса приложений в Azure Active Directory B2C](tutorial-customize-ui.md).
- Если вы используете настраиваемые политики, то вы можете приступить к настройке пользовательского интерфейса с помощью статьи [Настройка пользовательского интерфейса приложения с помощью настраиваемой политики в Azure Active Directory B2C](active-directory-b2c-ui-customization-custom.md).
