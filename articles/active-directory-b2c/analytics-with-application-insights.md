...
| ----------------- | -----|
| AppInsights — общие | Общий набор параметров, включаемых во все технические профили Azure Insights. |
| AppInsights — Сигнинрекуест | Записывает `SignInRequest` событие с набором утверждений при получении запроса на вход. |
...
| AppInsights — Сигнинкомплете | Записывает `SignInComplete` событие при успешном завершении проверки подлинности, когда маркер был отправлен в приложение проверяющей стороны. |
