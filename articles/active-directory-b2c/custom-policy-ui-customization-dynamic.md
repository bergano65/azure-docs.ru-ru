...
Эта статья посвящена настройке пользовательского интерфейса Azure AD B2C с помощью *динамического содержимого*, используя пользовательские политики. Сведения по началу работы см. в статье [Azure Active Directory B2C. Настройка пользовательского интерфейса с помощью настраиваемой политики](custom-policy-ui-customization.md).

>[!NOTE]
>В статье об Azure AD B2C [Настройка пользовательского интерфейса с помощью настраиваемой политики](custom-policy-ui-customization.md) содержатся следующие основные сведения:
...
В статье [Azure Active Directory B2C. Настройка пользовательского интерфейса с помощью настраиваемой политики](custom-policy-ui-customization.md) выполнялась отправка файлов HTML5 в хранилище BLOB-объектов Azure. Эти файлы HTML5 являются статичными и отображают то же HTML-содержимое для каждого запроса.
...
3. Присвойте имя приложению, например *Contoso.AADB2C.UI*, а затем нажмите кнопку **ОК**.
    ...
    ![Создание проекта Visual Studio](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project1.png)

4. Выберите шаблон **Веб-приложение**.

5. В параметрах проверки подлинности выберите вариант **Без аутентификации**.

    ![Выбор шаблона "Веб-приложение"](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project2.png)
...
1. Щелкните правой кнопкой мыши папку Views/Home, а затем выберите **Добавить** > **Новый элемент**.
    ...
    ![Добавление элемента меню "добавить новый элемент" в Visual Studio](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view1.png)
...
5. Выберите **Добавить**.
    ...
    ![Диалоговое окно "Добавление нового элемента" в Visual Studio с выделенной страницей представления MVC](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view2.png)
...
10. Скопируйте содержимое файла и вставьте его ниже определения макета. Код должен выглядеть следующим образом:
    ...
    ![файл unified.cshtml после добавления содержимого HTML5](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-edit-view1.png)
...
![элемент img со значением пользовательского background_background_image src](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-static-background.png)
...
1. Откройте **Controllers\HomeController.cs** и добавьте следующий метод:
    ...
    ![Изменение контроллера для отображения представления](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-controller-view.png)
...
1. В **обозревателе решений** щелкните правой кнопкой мыши проект **Contoso.AADB2C.UI**, а затем выберите **Опубликовать**.
    ...
    ![Публикация в службе приложений Microsoft Azure](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish1.png)

2. Выберите плитку **Служба приложений Microsoft Azure** и нажмите кнопку **Опубликовать**.

    ![Создание службы приложений Microsoft Azure](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish2.png)
...
4. Нажмите кнопку **Создать**, чтобы начать создавать ресурсы Azure.
    ...
    ![Предоставление свойств службы приложений](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish3.png)
...
1. На [портале Azure](https://portal.azure.com/) выберите **Службы приложений**, а затем щелкните имя своего приложения API.
    ...
    ![Выбор приложения API на портале Azure](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS1.png)

2. В разделе **Параметры** в области **API** выберите **CORS**.

    ![В портал Azure меню службы приложений выбран пункт меню CORS](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS2.png)
...
4. Щелкните **Сохранить**.
    ...
    ![Страница параметров CORS с выделенной звездочкой в разрешенных источниках](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS3.png)
...
6. Измените значение `LoadUri` с _~/tenant/default/unified_ на _https://<имя_приложения>.azurewebsites.net/home/unified_.
    ...
    ![Пример фрагмента XML с выделенным элементом LoadUri](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-content-definition.png)
...
2. Откройте **B2C_1A_signup_signin**, отправленную вами пользовательскую политику проверяющей стороны, а затем выберите **Run Now** (Запустить сейчас).
    ...
    ![Политика регистрации или входа в систему](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo1.png)
...
2. Найдите элемент `<img>` с идентификатором `background_background_image` и замените значение `src` на `@ViewData["background"]`.
    ...
    ![элемент img с выделенным значением src ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-dynamic-background.png)
...
5. Добавьте параметр строки запроса _campaignId_ в универсальный код ресурса (URI). Например, добавьте `&campaignId=hawaii`, как показано на следующем рисунке:
    ...
    ![URI с выделенным параметром строки запроса campaignId](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-campaignId-param.png)

6. Нажмите клавишу **ВВОД**, чтобы отобразить фон Гавайев.

    ![Страница входа в систему с пользовательским фоном Гавайи Image](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo2.png)

7. Измените значение на *Tokyo* и нажмите клавишу **ВВОД**.
    В браузере отобразится фон Токио.

    ![Страница входа в систему с пользовательским фоном в Токио](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo3.png)
...
* После того как вы ознакомитесь с [пошаговым руководством по началу работы с пользовательскими политиками](custom-policy-get-started.md), рекомендуем создать свой сценарий, используя собственные файлы пользовательской политики. Для справки мы предоставили [образцы файлов политики](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-ui-customization).
