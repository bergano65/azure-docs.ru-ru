...
| domain_hint |Обязательно |Может иметь значение `consumers` или `organizations`.  Для обновления и получения маркеров в скрытом IFRAME включите в запрос значение `domain_hint`.  Извлеките `tid` утверждение из маркера идентификатора более раннего входа, чтобы определить, какое значение использовать (область `profile` требуется для получения утверждения `tid`). Если значение утверждения `tid` — `9188040d-6c67-4c5b-b112-36a304b66dad`, то используйте `domain_hint=consumers`.  Или используйте `domain_hint=organizations`. |
