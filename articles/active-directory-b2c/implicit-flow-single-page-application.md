...
| domain_hint |Обязательное значение |Может иметь значение `consumers` или `organizations`.  Для обновления и получения маркеров в скрытом IFRAME включите `domain_hint` значение в запрос.  Извлеките `tid` утверждение из маркера идентификатора более раннего входа, чтобы определить, какое значение использовать ( `profile` область требуется для получения `tid` утверждения). Если значение утверждения `tid` — `9188040d-6c67-4c5b-b112-36a304b66dad`, то используйте `domain_hint=consumers`.  В противном случае используйте коллекцию `domain_hint=organizations`. |
...
| state | Нет | Если запрос содержит параметр `state`, то в ответе должно отображаться то же значение. Приложение должно проверить, совпадают ли `state` значения в запросе и ответе. |
