---
title: Часто задаваемые вопросы (Часто задаваемые вопросы) для Azure Active Directory B2C
description: Ответы на часто задаваемые вопросы об Azure Active Directory B2C.
...
ms.locfileid: "79264405"
---
# <a name="azure-ad-b2c-frequently-asked-questions-faq"></a>Azure AD B2C: часто задаваемые вопросы

Эта страница часто отвечает на часто задаваемые вопросы об Aazure Active Directory B2C (Azure AD B2C). Следите за обновлениями.
...
Azure AD и Azure AD B2C являются отдельными предложениями продуктов и не могут совместно существовать в одном клиенте. Клиент Azure AD представляет организацию. Клиент Azure AD B2C представляет коллекцию удостоверений для использования с приложениями проверяющей стороны. Добавляя **новый поставщик OpenID Connect** в **рамках Azure AD B2C > поставщиков identity** или с пользовательскими политиками, Azure AD B2C может подавать сяртер на Azure AD, позволяя аутентифицировать сотрудников в организации.
...
Azure AD B2C нельзя использовать для проверки подлинности пользователей Microsoft Office 365. Azure AD — это решение корпорации Майкрософт для управления доступом сотрудников к приложениям SaaS, и в нем есть функции, предназначенные для этой цели, такие как лицензирование и условный доступ. Azure AD B2C предоставляет платформу управления удостоверениями и доступом для создания веб-и мобильных приложений. После настройки Azure AD B2C для включения в федерацию с клиентом Azure AD этот клиент может управлять доступом сотрудников к приложениям, которые используют Azure AD B2C.
...
В настоящее время мы поддерживаем несколько поставщиков социальной идентичности, включая Amazon, Facebook, GitHub (предварительный просмотр), Google, LinkedIn, учетную запись Майкрософт (MSA), Кью (предварительный просмотр), Twitter, WeChat (предварительный просмотр) и Weibo (предварительный просмотр). Мы оцениваем добавление поддержки для других популярных поставщиков социальной идентичности на основе спроса клиентов.

Azure AD B2C также поддерживает [пользовательские политики.](custom-policy-overview.md) Пользовательские политики позволяют создавать свою собственную политику для любого поставщика идентификационных данных, который поддерживает [OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html) или SAML. Начните работу с пользовательскими политиками, ознакомившись с [начальным пакетом пользовательской политики](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack).
...
* **Логотип баннера**: Показано в правом нижнем углу.
* **цвет фона**: отображается вверху.

    ![Скриншот индивидуальной электронной почты проверки](./media/faq/company-branded-verification-email.png)
...
1. Откройте лезвие **Active Directory Azure.**
...
Для записи инструмента миграции можно использовать API Microsoft Graph. Дополнительные сведения см. в [руководстве по переносу пользователей](user-migration.md).
...
Поток пользователя с использованием паролей Azure AD B2C для локальных учетных записей основан на политике для Azure AD. В потоках пользователей для регистрации, входа и сброса пароля в Azure AD B2C предусмотрено использование надежного пароля, срок действия которого не истекает. Для получения более подробной информации смотрите [политики и ограничения паролей в active-каталоге Azure.](https://docs.microsoft.com/azure/active-directory/authentication/concept-sspr-policy)
...
Нет служба Azure AD Connect не предназначена для работы с Azure AD B2C. Для миграции пользователей можно использовать [API Microsoft Graph.](manage-user-accounts-graph-api.md) Дополнительные сведения см. в [руководстве по переносу пользователей](user-migration.md).
...
Прочитайте [сравнить B2B совместное сотрудничество и B2C в Azure AD,](../active-directory/b2b/compare-with-b2c.md) чтобы узнать больше о применении соответствующих функций для ваших внешних сценариев идентификации.
...
Да, см. [настройку языка](user-flow-language-customization.md). Мы предоставляем переводы для 36 языков, и вы можете переопределить любую строку в соответствии со своими потребностями.
...
В настоящее время нет. Мы планируем добавить эту функцию в будущем. Проверка домена на вкладке **Домены** на портале Azure не подходит для достижения этой цели. Однако, с b2clogin.com, мы предлагаем [нейтральный домен верхнего уровня,](b2clogin.md)и, таким образом, внешний вид может быть реализован без упоминания Microsoft.

### <a name="how-do-i-delete-my-azure-ad-b2c-tenant"></a>Как удалить клиент Azure AD B2C?

Выполните следующие действия, чтобы удалить арендатора Azure AD B2C.

Вы можете использовать текущий опыт **приложений** или наш новый унифицированный **опыт регистрации приложений (Preview).** [См. дополнительные сведения о новом интерфейсе](https://aka.ms/b2cappregintro).

#### <a name="applications"></a>[Приложения](#tab/applications/)

1. Вопиюсь на [портал Azure](https://portal.azure.com/) в качестве *администратора подписки.* Используйте ту же учетную запись работы или школы или ту же учетную запись Майкрософт, которую вы использовали для регистрации в Azure.
...
1. Удалите все **потоки (политики) пользователей** в вашем арендаторе Azure AD B2C.
1. Удалите все **приложения,** зарегистрированные в вашем арендаторе Azure AD B2C.
1. Выберите **Активный каталог Azure** в левом меню.
1. В разделе **Управление** выберите **Пользователи**.
1. Выберите каждого пользователя по очереди (исключите пользователя *Администратора подписки,* в который вы в настоящее время подписаны в качестве). Выберите **Удалить** в нижней части страницы и выбрать **ДА,** когда предложено.
1. Под **управлением**выберите **регистрации приложений** (или **регистрации приложений (Наследие).**
1. Выберите **Просмотр всех приложений**
1. Выберите приложение под названием **b2c-расширения-приложение,** выберите **Удалить,** а затем выберите **Да,** когда предложено.
1. Под **управлением**выберите **настройки пользователя.**
1. Если присутствует, под **LinkedIn соединения учетной записи**, выберите **Нет,** а затем выберите **Сохранить**.
...
1. Выйти из портала Azure, а затем войти обратно, чтобы обновить доступ.
1. Выберите **Активный каталог Azure** в левом меню.
1. На странице **Обзор** выберите **каталог «Удалить».** Следуйте инструкциям на экране, чтобы завершить процесс.
...
1. Выберите фильтр **Каталог и подписка** в верхнем меню, а затем выберите каталог, содержащий клиент Azure AD B2C.
1. В меню слева выберите **Azure AD B2C**. Либо щелкните **Все службы**, а затем найдите и выберите **Azure AD B2C**
1. Удалите все **потоки пользователей (политики)** в вашем нанимателье Azure AD B2C.
1. Выберите **регистрацию приложений (Preview),** затем выберите вкладку **«Все приложения».**
1. Удалите все зарегистрированные приложения.
1. Удалите **b2c-расширение-приложение**.
1. В разделе **Управление** выберите **Пользователи**.
1. Выберите каждого пользователя по очереди (исключите пользователя *Администратора подписки,* в который вы в настоящее время подписаны в качестве). Выберите **Удалить** в нижней части страницы и выбрать **Да,** когда предложено.
1. Выберите **Активный каталог Azure** в левом меню.
1. Под **управлением**выберите **настройки пользователя.**
