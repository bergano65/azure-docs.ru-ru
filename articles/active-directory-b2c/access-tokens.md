...
Если параметр **response_type** `/authorize` в запросе содержит `token`, то параметр **области** должен включать по крайней мере одну область ресурсов, отличную от `openid` , и `offline_access` которая будет предоставлена. В противном `/authorize` случае запрос завершится ошибкой.
