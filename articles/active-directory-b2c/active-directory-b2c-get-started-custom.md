...
4. Выберите **Секрет**, введите секрет Facebook, указанный на сайте developers.facebook.com, или задайте значение `0` в качестве заполнителя. Это значение является секретом, а не ИДЕНТИФИКАТОРом приложения.
5. Для параметра **Использование ключа** выберите **Подпись**.
6. Нажмите кнопку **Создать**.
...
7. Выберите **выбрать API**, выполните поиск и выберите **IdentityExperienceFramework**, а затем нажмите кнопку **выбрать**.
...
1. Выполните действия, описанные в статье [Настройка регистрации и входа с помощью учетной записи Facebook](active-directory-b2c-setup-fb-app.md) для настройки приложения Facebook.
...
1. В разделе **пользовательские политики**выберите **B2C_1A_signup_signin**.
1. Выберите **выполнить сейчас** и выберите Facebook для входа с помощью Facebook и протестируйте настраиваемую политику. Или вызовите политику непосредственно из зарегистрированного приложения.
