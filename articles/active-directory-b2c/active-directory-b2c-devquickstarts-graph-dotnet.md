---
title: Использование API Graph в Azure Active Directory B2C | Документация Майкрософт
...
3. В области навигации слева выберите **все службы**, щелкните **Регистрация приложений**и нажмите кнопку **создать регистрацию**.
4. Следуйте инструкциям на экране, а затем создайте новое приложение.
    1. Добавить подходящее имя
    2. Выбрать **учетные записи только в этом каталоге Организации**
    3. Выберите в качестве типа приложения " **Интернет** " и укажите **URL-адрес для входа** `https://B2CGraphAPI`(например,), так как он не подходит для этого примера.
...
6. В меню Параметры выберите **сертификаты & секреты**.
7. В разделе **секреты клиента** щелкните **новый секрет клиента**, введите описание секрета и выберите длительность, а затем нажмите кнопку **добавить**. Скопируйте значение секрета (также известное как секрет клиента) для использования в более позднем разделе.
...
> [!NOTE]
> Предоставление разрешений может занять несколько минут.
>
>

## <a name="configure-delete-or-update-password-permissions-for-your-application"></a>Настройка разрешений на удаление или обновление паролей для вашего приложения
Сейчас разрешение *Чтение и запись данных каталога* **не** включает возможность удаления пользователей или обновления их паролей. Если вы хотите предоставить приложению возможность удалять пользователей или обновлять пароли, необходимо выполнить следующие дополнительные действия, требующие использования PowerShell. В противном случае перейдите к следующему разделу.
...
Теперь мы используем **идентификатор приложения** в приведенном ниже скрипте, чтобы назначить приложению роль администратора учетных записей пользователей. Идентификаторы этих ролей известны, поэтому можно просто ввести **идентификатор приложения** в приведенный ниже скрипт.
...
Теперь ваше приложение также имеет разрешения на удаление пользователей или обновление паролей от клиента B2C.
...
> [!NOTE]
> Если у учетных записей, переносимых из существующего хранилища пользователей, надежность пароля ниже, чем [высокая надежность пароля, обеспечиваемая Azure AD B2C](/previous-versions/azure/jj943764(v=azure.100)), можно отключить требование надежного пароля с помощью значения `DisableStrongPassword` свойства `passwordPolicies`. Например, можно изменить запрос создания пользователя, приведенный выше, следующим образом: `"passwordPolicies": "DisablePasswordExpiration, DisableStrongPassword"`.
>
>
