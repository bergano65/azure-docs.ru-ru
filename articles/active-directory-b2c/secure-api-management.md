...
1. Запишите значение в **идентификаторе приложения для идентификатора** *APP1* или другого ранее созданного приложения.

  ![Расположение идентификатора приложения B2C в портал Azure](media/secure-apim-with-b2c-token/portal-02-app-id.png)
...
Теперь для использования в следующем разделе должны быть записаны два URL-адреса: общедоступная конечная точка конфигурации OpenID Connect Connect и URI издателя. Пример:
...
1. В разделе **входящая обработка**выберите **\< / ,чтобыоткрытьредакторкодаполитики\>** .
1. Поместите следующий `<validate-jwt>` тег `<inbound>` в политику.

    1. Обновите `<openid-config>` значение в элементе с помощью хорошо известного URL-адреса конфигурации политики. `url`
    1. Обновите элемент, указав идентификатор приложения, созданного ранее в клиенте B2C (например, "веб-APP1"). `<audience>`
    1. Обновите `<issuer>` элемент с помощью конечной точки издателя токена, записанной ранее.
...
Сначала требуется маркер, выданный Azure AD B2C для использования в `Authorization` заголовке в POST. Его можно получить с помощью функции " **Запустить сейчас** " пользовательского потока регистрации или входа, который должен быть создан в качестве одного из необходимых компонентов.
...
1. Выберите **Подписки**.
1. Выберите многоточие для **продукта: Без**ограничений, выберите **Показать/скрыть ключи**.
1. Запишите **первичный ключ** для продукта. Этот ключ используется для `Ocp-Apim-Subscription-Key` заголовка HTTP-запроса в POST.
...
1. Создание нового `GET` запроса в [POST](https://www.getpostman.com/). В поле URL-адрес запроса укажите конечную точку списка динамиков API, опубликованного в качестве одного из необходимых компонентов. Пример:
...
1. Затем добавьте следующие заголовки:
    ...
    | Ключ | Значение |
    | --- | ----- |
    | `Authorization` | Закодированное ранее значение токена с `Bearer ` префиксом (включая пробел после "Bearer") |
...
Теперь, когда вы выполнили успешный запрос, проверьте случай сбоя, чтобы убедиться, что вызовы API с недопустимым токеном отклоняются должным образом. Один из способов выполнить тест — добавить или изменить несколько символов в значении маркера, а затем выполнить тот же `GET` запрос, что и раньше.
...
1. Нажмите кнопку **Send (отправить** ), чтобы выполнить запрос. При наличии недопустимого маркера ожидаемым результатом будет `401` код неавторизованного состояния:
...
Если отображается `401` код состояния, вы проверили, что только вызывающие объекты с действительным маркером доступа, выпущенные Azure AD B2C, могут успешно выполнять запросы к API управления API Azure.

## <a name="support-multiple-applications-and-issuers"></a>Поддержка нескольких приложений и издателей

Несколько приложений обычно взаимодействуют с одним REST API. Чтобы разрешить нескольким приложениям вызывать API, добавьте идентификаторы `<audiences>` приложений в элемент в политике входящих подключений APIM.
...
Аналогично, для поддержки нескольких издателей маркеров добавьте их универсальные коды ресурсов `<audiences>` (URI конечной точки) в элемент в политике входящих подключений APIM.
...
Если у вас есть API APIM, который проверяет маркеры, выданные устаревшей `login.microsoftonline.com` конечной точкой, необходимо перенести API и приложения, вызывающие его, для использования маркеров, выданных [b2clogin.com](b2clogin.md).
