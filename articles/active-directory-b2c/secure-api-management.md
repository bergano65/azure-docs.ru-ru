...
1. Поместите следующий `<validate-jwt>` тег в `<inbound>` политику.

    1. Обновите `url` значение в `<openid-config>` элементе с помощью хорошо известного URL-адреса конфигурации политики.
    1. Обновите `<audience>` элемент, указав идентификатор приложения, созданного ранее в клиенте B2C (например, "веб- *APP1*").
...
Если отображается код `401` состояния, вы проверили, что только вызывающие объекты с действительным маркером доступа, выпущенные Azure AD B2C, могут успешно выполнять запросы к API управления API Azure.

## <a name="support-multiple-applications-and-issuers"></a>Поддержка нескольких приложений и издателей

Несколько приложений обычно взаимодействуют с одним REST API. Чтобы разрешить API принимать маркеры, предназначенные для нескольких приложений, добавьте идентификаторы приложений в `<audiences>` элемент в политике входящих подключений APIM.
...
Аналогично, для поддержки нескольких издателей маркеров добавьте их универсальные коды ресурсов `<issuers>` (URI конечной точки) в элемент в политике входящих подключений APIM.
