...
Эти пользовательские поездки можно рассматривать как шаблоны, доступные для удовлетворения основных потребностей различных заинтересованных сторон сообщества. Поездки пользователей облегчают определение части политики, полагающейся стороной. Политика может определять несколько путей взаимодействия пользователя. Каждый путь взаимодействия пользователя представляет собой последовательность шагов оркестрации.
...
Шаги оркестровки могут быть условно выполнены на основе предварительных условий, определенных в элементе шага оркестровки. Например, вы можете проверить выполнение шага оркестровки только в том случае, если существует конкретная претензия или если претензия равна или не указана.
...
Следующие предварительные условия позволяют проверить, выполнил ли пользователь вход с помощью учетной записи социальной сети. Предпринята попытка найти учетную запись пользователя в каталоге. Если пользователь входит или регистрируется с локальной учетной записью, пропустите этот шаг оркестровки.
...
Элемент Preconditions позволяет проверить несколько предварительных условий. В следующем примере проверяется, существуют ли элементы objectId или email. Если первое условие верно, путешествие переходят к следующему этапу оркестровки.
...
| Атрибут | Обязательно | Описание |
| --------- | -------- | ----------- |
| DisplayOption| нет | Контролирует поведение случая, когда доступен выбор поставщика единого поставщика претензий. Возможные значения: `DoNotShowSingleProvider` (по умолчанию) , пользователь немедленно перенаправляется к федеративному поставщику идентификационных данных. Или `ShowSingleProvider` Azure AD B2C представляет страницу ввне с выбором единого поставщика идентификационных данных. Чтобы использовать этот атрибут, версия `urn:com:microsoft:aad:b2c:elements:contract:providerselection:1.0.0` определения [содержимого](page-layout.md) должна быть и выше.|
