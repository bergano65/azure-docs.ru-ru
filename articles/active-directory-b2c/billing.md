...
### <a name="move-by-un-linking-and-re-linking"></a>Перемещение путем отмены связывания и повторного связывания

Если исходная и Целевая подписки связаны с разными клиентами Azure Active Directory, перемещаться с помощью Azure Resource Manager, как описано выше, нельзя. Тем не менее можно добиться того же конечного результата, отменив связь клиента Azure AD B2C с исходной подпиской и повторно связав его с конечной подпиской. Этот метод является надежным, поскольку единственным удаляемым объектом является *ссылка для выставления счетов*, а не сам клиент Azure AD B2C. Ни один из пользователей, приложений, потоков пользователей и т. п. не будет затронуты.
...
1. На странице **Обзор** нажмите кнопку **Удалить** . Это *не* приводит к удалению связанных пользователей или приложений клиента Azure AD B2C. Он просто удаляет связь выставления счетов из исходной подписки.
...
1. Ресурс Azure AD B2C теперь перемещен в целевую подписку Azure (связанную с целевой Azure Active Directory) и будет взиматься из этой подписки.
