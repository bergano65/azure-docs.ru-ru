...
1. Пытается выйти из федеративных поставщиков удостоверений:
   - OpenID Connect Connect — если конечная точка хорошо известной конфигурации поставщика удостоверений `end_session_endpoint` указывает расположение.
   - SAML — если метаданные поставщика удостоверений содержат `SingleLogoutService` расположение.
...
Приложения должны ответить на него, удалив любой сеанс, который идентифицирует пользователя, и возвратив ответ `200`. Если вы хотите поддерживать единый выход в приложении, необходимо реализовать `LogoutUrl` в коде приложения. `LogoutUrl` можно настроить на портале Azure.

1. Перейдите к [портал Azure](https://portal.azure.com).
1. Выберите свой активный каталог B2C, щелкнув свою учетную запись в правом верхнем углу страницы.
1. На панели навигации слева щелкните **Azure AD B2C**, выберите **Регистрация приложений**, а затем выберите свое приложение.
...
- Узнайте, как [настроить поведение сеанса в настраиваемой политике](custom-policy-manage-sso-and-token-config.md#session-behavior-and-sso).