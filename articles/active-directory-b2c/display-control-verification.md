...
- `DisplayClaims`
  ...
  - **Код проверки** — утверждение проверочного кода, предоставленное пользователем после отправки кода. Это утверждение должно быть задано как обязательное, а `ControlClaimType` должно быть установлено в значение `VerificationCode`.
