...
- `DisplayClaims`
  ...
  - **Код проверки** — утверждение проверочного кода, предоставленное пользователем после отправки кода. Это утверждение должно быть задано как обязательное, `ControlClaimType` а значение должно быть `VerificationCode`равно.
