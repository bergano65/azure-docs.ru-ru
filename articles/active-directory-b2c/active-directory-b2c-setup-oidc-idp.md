...
> [!NOTE]
> Секрет клиента не обязателен. Тем не менее, секрет клиента необходимо ввести, если требуется применить [последовательность с использованием кода авторизации](https://openid.net/specs/openid-connect-core-1_0.html#CodeFlowAuth), в которой секрет используется для обмена кода на маркер.
