...
| Сценарий | Роль Azure AD B2C | Пошаговые инструкции |
| -------- | ----------------- | ------- |
...
| Моим пользователям требуется единый вход с помощью соответствующего SAML поставщика удостоверений, такого как ADFS, Salesforce или Shibboleth.  | **Azure AD B2C выступает в качестве поставщика услуг (SP)**<br />Azure AD B2C выступает в качестве поставщика услуг при подключении к поставщику удостоверений SAML. Это прокси-сервер федерации между приложением и поставщиком удостоверений SAML.  | <ul><li>[Настройка входа с помощью ADFS в качестве IdP SAML с помощью настраиваемых политик](active-directory-b2c-custom-setup-adfs2016-idp.md)</li><li>[Настройка входа с помощью поставщика SAML SalesForce с помощью настраиваемых политик](active-directory-b2c-setup-sf-app-custom.md)</li></ul> |

## <a name="prerequisites"></a>Технические условия

* Выполните действия, описанные в разделе [Приступая к работе с пользовательскими политиками в Azure AD B2C](active-directory-b2c-get-started-custom.md). Вам потребуется настраиваемая политика *SocialAndLocalAccounts* из начального пакета настраиваемой политики, описанного в этой статье.
...
Чтобы создать отношение доверия между поставщиком услуг и Azure AD B2C, необходимо предоставить сертификаты X509 и их закрытые ключи.
...
1. Выполните эту команду PowerShell, чтобы создать самозаверяющий сертификат. Измените аргумент `-Subject` соответствующим образом для приложения и Azure AD B2C имя клиента. Можно также настроить `-NotAfter` дату, чтобы указать другой срок действия сертификата.
...
1. Открыть **Управление сертификатами пользователей** > **текущего пользователя** > **Личные** > **Сертификаты** > *yourappname.yourtenant.onmicrosoft.com*
1. Выберите **действие** > сертификата > **все задачи** > **экспорта**
1. Выберите **да** > **Далее** > **да, экспортировать закрытый ключ** > **Далее** .
...
Теперь добавьте возможность для клиента, чтобы выдать токены SAML.

Откройте `SocialAndLocalAccounts\` **`TrustFrameworkExtensions.xml`** в начальном пакете настраиваемой политики.

Откройте раздел `<ClaimsProviders>` и добавьте следующий фрагмент XML-кода.

Можно изменить значение метаданных `IssuerUri`. Это URI издателя, который возвращается в ответе SAML от Azure AD B2C. Приложение проверяющей стороны должно быть настроено на принятие URI издателя во время проверки утверждения SAML.
...
1. Добавьте следующий фрагмент кода XML непосредственно перед элементом `<RelyingParty>`. Этот код XML перезаписывает шаг 7 в пути взаимодействия пользователя _SignUpOrSignIn_ . При настройке пути взаимодействия пользователя путем добавления или удаления действий оркестрации убедитесь, что число (в элементе `order`) соответствует указанному в пути взаимодействия пользователя для шага издателя маркера.
...
1. Замените весь элемент `<TechnicalProfile>` в элементе `<RelyingParty>` следующим техническим XML-документом профиля.
...
1. Обновите `tenant-name` именем клиента Azure AD B2C.
...
Метаданные политики Azure AD B2C доступны по следующему URL-адресу. Замените `tenant-name` именем клиента Azure AD B2C и `policy-name` именем (ID) политики:
...
1. В разделе **URI перенаправления**выберите **веб**и введите `https://localhost`. Это значение можно изменить позже в манифесте регистрации приложения.
1. Выберите **предоставить согласие администратора для разрешений OpenID Connect и offline_access**.
...
`identifierUris` — это коллекция строк, содержащая определяемые пользователем универсальные коды ресурсов (URI), которые уникально идентифицируют веб-приложение в его Azure AD B2Cном клиенте. Поставщик услуг должен задать это значение в элементе `Issuer` запроса SAML.
...
Для работы с этим руководством, в котором используется тестовое приложение SAML, используйте следующее значение для `samlMetadataUrl`:
...
Если не указать URI метаданных, можно явно указать URL-адрес ответа. Это необязательное свойство представляет `AssertionConsumerServiceUrl` (URL-адрес`SingleSignOnService` в метаданных поставщика услуг), а `BindingType` предполагается `HTTP POST`.
...
Это необязательное свойство представляет `Logout` URL-адрес (URL-адрес`SingleLogoutService` в метаданных проверяющей стороны), а `BindingType` для этого предполагается `Http-Redirect`.

Для работы с этим руководством, в котором используется тестовое приложение SAML, оставьте `logoutUrl` установленным в значение `https://samltestapp2.azurewebsites.net/logout`:
...
* **Издатель**: `https://tenant-name.onmicrosoft.com/policy-name`
...
Выберите **имя входа** , и появится экран входа конечного пользователя. После входа в пример приложения возвращается утверждение SAML.
