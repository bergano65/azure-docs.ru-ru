...
Эта статья является продолжением статьи о перемещении пользователей и посвящена перемещению социальных удостоверений. Прежде чем начать, ознакомьтесь со статьей [Azure Active Directory B2C. Миграция пользователей](user-migration.md).
...
5. Выполните следующий запрос GET. Замените значение userObjectId скопированным идентификатором пользователя. **Получить** https://graph.windows.net/tenant-name.onmicrosoft.com/users/userObjectId
...
https://graph.windows.net/tenant-name.onmicrosoft.com/users/userObjectId **исправлений**
...
Дополнительные сведения об использовании этого примера приложения см. в статье [Azure Active Directory B2C. Миграция пользователей](user-migration.md).
