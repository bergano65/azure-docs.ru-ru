...
При использовании Azure Active Directory B2C (Azure AD B2C) вместе с Azure Application Insights можно получить подробные и настраиваемые журналы событий для пути взаимодействия пользователя. Вы узнаете, как выполнять следующие задачи:
...
Этот поставщик использует технический профиль для определения события из Azure AD B2C. Профиль определяет имя события, утверждения, которые записаны, и ключ инструментирования. Чтобы опубликовать событие, технический профиль добавляется в качестве `orchestration step` в нестандартном пути взаимодействия пользователя.
...
> [!IMPORTANT]
> Замените ключ инструментирования в техническом профиле `AzureInsights-Common` идентификатором GUID, который предоставляется ресурсом Application Insights.
