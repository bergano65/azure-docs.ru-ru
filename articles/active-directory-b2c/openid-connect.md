...
После того, как пользователь завершит поток пользователя, в приложении в указанном параметре будет возвращен ответ с `redirect_uri` помощью метода, указанного в `response_mode` параметре. Ответ одинаков для каждого из предыдущих случаев, независимо от потока пользователя.
...
| state | Если в запрос включен параметр `state`, идентичное значение должно содержаться и в ответе на этот запрос. Приложение должно проверить, совпадают ли `state` значения в запросе и ответе. |
...
- Проверьте `aud` утверждение, чтобы убедиться, что маркер идентификатора был выдан для вашего приложения. Его значение должно быть ИДЕНТИФИКАТОРом приложения.
- Проверьте `iat` утверждения и, `exp` чтобы убедиться, что срок действия МАРКЕРа идентификатора не истек.
...
Срок действия маркеров идентификатора истекает через короткий период времени. Обновите токены после истечения срока их действия, чтобы продолжить доступ к ресурсам. Токен можно обновить, отправив другой `POST` запрос в `/token` конечную точку. На этот раз укажите параметр `refresh_token` вместо параметра `code`:
...
| client_id | Да | Идентификатор приложения, назначенный приложению [портал Azure](https://portal.azure.com/) . |
| client_secret | Да, в веб-приложениях | Секрет приложения, созданный в [портал Azure](https://portal.azure.com/). Секреты клиента используются в этом потоке для сценариев веб-приложений, где клиент может безопасно хранить секрет клиента. Для сценариев собственного приложения (общедоступного клиента) секретность клиента не может быть безопасно сохранена, поэтому она не используется в этом вызове. При использовании секрета клиента следует периодически менять его. |
| grant_type | Да | Тип предоставления, который должен быть `refresh_token` для этой части потока кода авторизации. |
| refresh_token | Да | Исходный маркер обновления, полученный во второй части последовательности. `offline_access`Для получения маркера обновления область должна использоваться как в запросах авторизации, так и на маркерах. |
...
| id_token_hint| Нет | Ранее выданный маркер идентификатора для передачи в конечную точку выхода в качестве подсказки о текущем сеансе, прошедшем проверку подлинности конечного пользователя, с клиентом. `id_token_hint`Гарантирует, что в `post_logout_redirect_uri` параметрах приложения Azure AD B2C будет зарегистрированный URL-адрес ответа. |
| client_id | Нет* | Идентификатор приложения, назначенный приложению [портал Azure](https://portal.azure.com/) .<br><br>\**Это необходимо при использовании `Application` конфигурации единого входа изолирования и необходимости _маркера идентификатора_ в запросе на выход `No` .* |
...
После выхода пользователя перенаправляется на URI, указанный в `post_logout_redirect_uri` параметре, независимо от URL-адресов ответа, указанных для приложения. Однако при передаче допустимого `id_token_hint` значения Azure AD B2C проверяет, соответствует ли значение `post_logout_redirect_uri` одному из настроенных URI перенаправления приложения перед выполнением перенаправления. Если соответствующий URL-адрес ответа не был настроен для приложения, выводится сообщение об ошибке, и пользователь не перенаправляется.
