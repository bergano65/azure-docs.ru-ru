...
Если атрибуты `SPNameQualifier` или `NameQualifier` не представлены в утверждении SAML, задайте для `assertionSubjectName`утверждения **партнерклаимтипе** значение. Убедитесь, что **NameId** является первым значением в XML утверждения. При определении нескольких утверждений Azure AD B2C выбирает значения субъекта из последнего утверждения.
