...
Возрастная обработка в Azure Active Directory B2C (Azure AD B2C) позволяет идентифицировать несовершеннолетних, которые хотят использовать ваше приложение. Вы можете заблокировать вход в приложение для несовершеннолетних. Пользователи могут вернуться в приложение и указать возрастную группу и состояние родительского согласия. Azure AD B2C может блокировать вход несовершеннолетних, выполняемый без родительского согласия. Также Azure AD B2C можно настроить так, чтобы приложение самостоятельно выбирало действия для несовершеннолетних.

После того как вы включите возраст gating в потоке [пользователей,](user-flow-overview.md)пользователи спрашивают, когда они родились и в какой стране / регионе они живут. Пользователь, который еще не вводил эту информацию, должен будет ввести ее для следующего входа. Правила применяются при каждом входе пользователя.
...
| -------------- | ------------------- | ----------------- | --------- |
| Значение по умолчанию | None | None | 18 |
| AE | ОАЭ | None | 21 |
| AT | Австрия | 14 | 18 |
| BE | Бельгия | 14 | 18 |
| BG | Болгария | 16 | 18 |
...
1. Убедитесь, что вы используете каталог, содержащий ваш клиент Azure AD B2C, выбрав фильтр **подписки каталога в** верхнем меню. Выберите каталог, содержащий ваш клиент.
