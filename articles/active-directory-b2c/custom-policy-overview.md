...
| Настройка пользовательского интерфейса | Полная настройка пользовательского иммунитета, включая HTML, CSS и JavaScript.<br><br>Поддержка нескольких языков для пользовательских строк. | Аналогично |
...
| Поставщики удостоверений | Предопределено локальному или социальному поставщику и большинству поставщиков идентификационных данных OIDC, таким как федерация с арендаторами Azure Active Directory. | OIDC, SAML, OAuth в соответствии со стандартами.  Аутентификация также возможна с помощью интеграции с REST AIS. |
...
Потоки пользователей в Azure AD B2C следуют шаблону файлов, изображенным выше, но разработчик видит только файл RP, в то время как портал Azure вносит изменения в фон файла расширений.

Хотя существует три типа файлов политики, вы не ограничены только тремя файлами. Возможно, у вас может быть несколько файлов каждого типа файлов. Например, если вы не хотите вносить изменения в файл расширения, можно создать файл Extensions2 для дальнейшего расширения файла расширений.
...
Система Identity Experience Framework — это полностью настраиваемая, основанная на политике, облачная платформа Azure, которая организует доверие между сущностями в стандартных форматах протокола, таких как OpenID Connect, OAuth, SAML, и несколько нестандартных, например, на основе REST API на основе системных обменов претензий. Эта платформа обеспечивает удобные возможности с поддержкой HTML и CSS.
