...
Azure AD B2C поддерживает протоколы [OpenID Connect и OAuth 2.0](protocols-overview.md) для пути взаимодействия пользователя. В реализации OpenID Connect в Azure AD B2C приложение запускает путь взаимодействия пользователя, отправляя запросы проверки подлинности в Azure AD B2C.

Результатом запроса к Azure AD B2C является маркер безопасности, например [маркер идентификатора или маркер доступа](tokens-overview.md). Этот маркер безопасности определяет идентификатор пользователя. Маркеры предоставляются из конечных точек Azure AD B2C, таких как конечные точки `/token` или `/authorize`. Эти маркеры позволяют получить доступ к утверждениям, которые можно применить для проверки удостоверения и предоставления доступа к защищенным ресурсам.
...
Локализация подробно описана в статье [Настройка языка в Azure Active Directory B2C](user-flow-language-customization.md).
...
Дополнительные сведения см. в статье [Включение многофакторной идентификации в Azure Active Directory B2C](custom-policy-multi-factor-authentication.md).
...
Вы можете настраивать сложность пароля как для [потоков пользователей](user-flow-password-complexity.md), так и для [пользовательских политик](custom-policy-password-complexity.md).
