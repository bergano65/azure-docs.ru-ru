---
title: Определение технического профиля эмитента JWT в пользовательской политике
titleSuffix: Azure AD B2C
description: Определите технический профиль для эмитента веб-токенов JSON (JWT) в пользовательской политике в Azure Active Directory B2C.
...
|RefreshTokenUserJourneyId| нет | Идентификатор пользовательского путешествия, который должен быть выполнен во время `/token` [обновления запроса токена доступа](authorization-code-flow.md#4-refresh-the-token) POST к конечной точке. |
