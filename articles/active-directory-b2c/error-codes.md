...
| `AADB2C90007` | Приложение, связанное с идентификатором{0}клиента "", не имеет зарегистрированных URI перенаправления. |
| `AADB2C90008` | Запрос не содержит параметр идентификатора клиента. |
| `AADB2C90010` | Запрос не содержит параметр области. |
| `AADB2C90011` | Идентификатор{0}клиента "", указанный в запросе, не совпадает с идентификатором клиента{1}"", зарегистрированным в политике. |
...
| `AADB2C90067` | URI{0}перенаправления после выхода из системы имеет недопустимый формат. Укажите URL-адрес на основе HTTPS,https://example.com/returnнапример "", или для собственных клиентов используйте URI собственного клиента IETF "urn: IETF: WG: OAuth: 2.0: OOB". |
...
| `AADB2C90075` | Обмен утверждениями "{0}", указанным на шаге{1}"", вернул ответ об ошибке{2}HTTP с кодом "{3}" и причиной "". |
...
| `AADB2C90080` | Истек срок действия предоставленного разрешения GRANT. Выполните повторную проверку подлинности и повторите попытку. Текущее время: {0}, предоставление времени выдачи {1}:, предоставление скользящего времени окончания срока {2}действия окна:. |
...
| `AADB2C90120` | В запросе указан недопустимый параметр максимального срока "{0}". Максимальный возраст должен быть целым числом в{1}диапазоне от "{2}" до "" включительно. |
| `AADB2C90122` | Входные данные{0}для "", полученного в запросе, не прошли проверку HTTP-запроса. Убедитесь, что входные данные не содержат таких символов, как < или &. |
...
| `AADB2C90149` | Не удалось{0}загрузить скрипт "". |
...
| `AADB2C90155` | Запрос многофакторной проверки подлинности завершился с{0}причиной "". |
| `AADB2C90156` | Запрос многофакторной проверки подлинности завершился с{0}причиной "". |
...
| `AADB2C90168` | Запрос HTTP-перенаправления не содержит обязательный параметр "{0}" для подписанного запроса. |
...
| `AADB2C90242` | В техническом профиле SAML{0}"" указывается партнерентити CDATA, который не может быть загружен{1}по причине "". |
...
| `AADB2C90288` | UserJourney с идентификатором{0}"", указанный в{1}техническом профиле "" для активации токена обновления{2}для клиента "", не существует{3}в политике "" или в ее базовых политиках. |
...
| `AADB2C99005` | Запрос содержит недопустимый параметр области, который содержит недопустимый{0}символ "". |
| `AADB2C99006` | Azure AD B2C не удается найти приложение расширений с идентификатором приложения{0}"". Дополнительные сведения https://go.microsoft.com/fwlink/?linkid=851224 см. по адресу. |
...
| `AADB2C99015` | В профиле "{0}" в{1}политике "" в{2}клиенте "" отсутствуют все inputclaim, необходимые для потока учетных данных для пароля владельца ресурса. |
