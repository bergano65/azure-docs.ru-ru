...
- Если у вас еще нет учетная запись Майкрософт, создайте его [https://www.live.com/](https://www.live.com/).
...
1. Добавьте следующий элемент **ClaimsExchange**, убедившись, что для идентификатора используется то же значение, которое использовалось для **TargetClaimsExchangeId**.
    ...
    Обновите значение **течникалпрофилереференцеид** , чтобы оно совпадало со `Id` значением в элементе **техническом профиле** поставщика утверждений, который вы добавили ранее. Например, `MSA-OIDC`.
