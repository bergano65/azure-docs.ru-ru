...
1. Добавьте следующий элемент **ClaimsExchange**, убедившись, что для идентификатора используется то же значение, которое использовалось для **TargetClaimsExchangeId**.
    ...
    Обновите значение **течникалпрофилереференцеид** , чтобы оно совпадало с значением `Id` в элементе **техническом профиле** поставщика утверждений, который вы добавили ранее. Например, `MSA-OIDC`.
