...
2. Нажмите кнопку **+ контейнер**.
3. Для **имя**, введите *корневой*. Это может быть имя по своему выбору, например *wingtiptoys*, но мы используем *корневой* в этом примере для простоты.
4. Для **уровня общего доступа**выберите **BLOB-объектов**, затем **ОК**.
5. Нажмите кнопку **корневой** открыть новый контейнер.
6. Щелкните **Отправить**.
7. Щелкните значок папки рядом с элементом **Выбор файла**.
8. Найдите и выберите **customize-ui.html** созданному ранее в разделе Настройка пользовательского интерфейса страницы.
9. Если вы хотите отправить во вложенную папку, разверните **Дополнительно** и введите имя папки в **передать папку**.
10. Щелкните **Отправить**.
11. Выберите **customize-ui.html** загруженный большой двоичный объект.
12. Справа от **URL-адрес** текстовое поле, выберите **Копировать в буфер обмена** значок, чтобы скопировать URL-адрес в буфер обмена.
13. В веб-браузер перейдите к URL-адрес, который вы скопировали, чтобы убедиться, что большой двоичный объект, загруженный доступен. Если он недоступен, например, при возникновении `ResourceNotFound` ошибку, убедитесь, что является значение типа доступа контейнера **BLOB-объектов**.
...
Ниже приведены шаги по использованию примера. 
1. Клонируйте репозиторий на локальном компьютере. Выберите шаблон папке sample_templates. Можно использовать `wingtip` или `contoso`.
2. Отправить все файлы в `css`, `fonts`, и `images` папок в хранилище BLOB-объектов, как описано в предыдущих разделах. 
3. Затем откройте каждый \*HTML-файл в корневом каталоге либо `wingtip` или `contoso` (зависимости от того что вы выбрали на первом шаге) и замените все вхождения "http://localhost" с URL-адреса файлов css, изображений и шрифтов, загруженный на шаге 2.
4. Сохранить \*.html, файлы и отправлять их в хранилище BLOB-объектов.
5. Теперь измените файл расширения, как упоминалось ранее, в [Изменение расширения файла](#modify-the-extensions-file).
6. Если вы видите отсутствуют шрифтов, изображений или css, проверьте ссылки в политику расширений и \*HTML-файлы.

### <a name="content-defintion-ids"></a>Определение содержимого идентификаторы
