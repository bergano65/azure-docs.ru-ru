...
| AdminHelpText | 0:1 | Описание типа претензии, которое может быть полезно для администраторов, чтобы понять его цель. |
...
|Дата| Представляет момент времени, обычно выраженный как дата дня. Значение даты соответствует конвенции ISO 8601.|
|dateTime|Представляет текущее время, обычно выраженное как дата и время суток. Значение даты соответствует конвенции ISO 8601.|
|длительность|Представляет интервал времени в годах, месяцах, днях, часах, минутах и секундах. Формат, `PnYnMnDTnHnMnS`где `P` указывает положительное, `N` или для отрицательного значения. `nY`это количество лет, за которыми следует буквальное `Y`. `nMo`это количество месяцев, за `Mo`которыми следует буквальное . `nD`это количество дней, за которыми следует буквальное `D`. Примеры: `P21Y` 21 год. `P1Y2Mo`составляет один год и два месяца. `P1Y2Mo5D`составляет один год, два месяца и пять дней.  `P1Y2M5DT8H5M620S`представляет один год, два месяца, пять дней, восемь часов, пять минут и двадцать секунд.  |
|phoneNumber|Представляет номер телефона. |
|INT| Представляет число между -2,147,483,648 и 2,147,483,647|
|long| Представляет число от -9,223,372,036,854,775,808 до 9,223,372,036,854,775,807 |
|строка| Представляет текст как последовательность из частей кода UTF-16.|
|stringCollection|Представляет коллекцию `string`.|
|userIdentity| Представляет идентификацию пользователя.|
...
| `Regex` | нет | Если **`Type`** `Regex`установлен, укажите регулярное выражение для использования.
...
![Номер телефона претензии показаны в браузере с первых шести цифр, замаскированных Xs](./media/claimsschema/mask.png)
...
![Электронная заявка, показанная в браузере с символами, замаскированными звездочками](./media/claimsschema/mask-regex.png)
...
Элемент **Enumeration** определяет доступные параметры для пользователя, чтобы выбрать для претензии в `CheckboxMultiSelect` `DropdownSingleSelect`пользовательском `RadioSingleSelect`интерфейсе, такие как значение в , или . Кроме того, можно определить и локализовать доступные параметры с помощью элемента [LocalizedCollections.](localization.md#localizedcollections) Чтобы найти элемент из коллекции **enumeration** претензий претензий претензий [претензий Претензий претензий GetMappedValueFromLocalizedCollection.](string-transformations.md#getmappedvaluefromlocalizedcollection)
...
![Управление выпадающим образом отображается в браузере и показывает значение по умолчанию](./media/claimsschema/dropdownsingleselect.png)
...
| HelpText | нет | Сообщение об ошибке для пользователей, если регулярная проверка выражения не удается. |
...
![TextBox, показывающий сообщение об ошибке, вызванное ограничением regex](./media/claimsschema/pattern.png)

### <a name="userinputtype"></a>UserInputType

Azure AD B2C поддерживает различные типы элементов для ввода данных пользователем, такие как текстовое поле, поле пароля и раскрывающийся список, которые можно использовать при вводе данных для типа утверждения вручную. При сборе информации от пользователя с помощью [самоутверждаемого технического профиля](self-asserted-technical-profile.md) и [элементов управления дисплеем](display-controls.md)необходимо указать **UserInputType.**

Элемент **UserInputType,** доступный типу ввода пользователя:

| UserInputType | Поддерживаемый claimType | Описание |
| --------- | -------- | ----------- |
|CheckboxMultiSelect| `string` |Мульти выберите выпадающую коробку. Значение претензии представлено в строке делимитедора запятой выбранных значений. |
|DateTimeDropdown | `date`, `dateTime` |Выпадающие падения, чтобы выбрать день, месяц и год. |
|DropdownSingleSelect |`string` |Одновыражаемый выпадающий ящик. Значение претензии является выбранным значением.|
|EmailBox | `string` |Поле ввода электронной почты. |
|Paragraph | `boolean`, `date`, `dateTime`, `duration`, `int`, `long`, `string`|Поле, показы которого отображает текст только в теге абзаца. |
|Пароль | `string` |Текстовое окно паролей.|
|RadioSingleSelect |`string` | Коллекция радиокнопок. Значение претензии является выбранным значением.|
|readOnly | `boolean`, `date`, `dateTime`, `duration`, `int`, `long`, `string`| Текстовое окно только для чтения. |
|TextBox |`boolean`, `int`, `string` |Однолинейный текстовый ящик. |
...
![TextBox, показывающий свойства, указанные в типе претензии](./media/claimsschema/textbox.png)
...
![EmailBox, показывающий свойства, указанные в типе претензии](./media/claimsschema/emailbox.png)
...
Тип элемента для ввода данных пользователем **Paragraph** используется для предоставления поля, которое отображает текст только в теге параграфа.  Например, &lt;p&gt;текст&lt;/p&gt;. **Параграф пользователя** ввода типа `OutputClaim` самоутверждаемого технического `Required` `false` профиля, должны установить атрибут (по умолчанию).
