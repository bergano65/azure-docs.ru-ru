...
| `Regex` | Нет | Если **`Type`** параметр имеет значение `Regex`, укажите регулярное выражение для использования.
...
Тип элемента для ввода данных пользователем **Paragraph** используется для предоставления поля, которое отображает текст только в теге параграфа.  Например, &lt;p&gt;текст&lt;/p&gt;. Пользовательский тип `OutputClaim` входных данных в виде **абзаца** самостоятельно утвержденного технического профиля должен задавать `Required` атрибут `false` (по умолчанию).
