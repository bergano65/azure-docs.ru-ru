...
| TenantObjectId | нет | Уникальный идентификатор объектов для арендатора Azure Active Directory B2C (Azure AD B2C). |
...
| DeploymentMode | нет | Возможные значения: `Production` `Development`, или . Значение по умолчанию — `Production`. Это свойство используется при отладке политики. Для получения дополнительной [информации см.](troubleshoot-with-application-insights.md) |
| UserJourneyRecorderEndpoint | нет | Конечная точка, которая используется, когда для **DeploymentMode** задано значение `Development`. Значение должно быть равно `urn:journeyrecorder:applicationinsights`. Для получения дополнительной [информации см.](troubleshoot-with-application-insights.md) |
...
![Диаграмма, показывающая модель наследования политики рамочного управления трастовых рамок](./media/trustframeworkpolicy/custom-policy-Inheritance-model.png)
...
Дополнительные сведения см. в статье [Azure Active Directory B2C: начало работы с настраиваемыми политиками](custom-policy-get-started.md).
...
Приложение проверяющей стороны, например веб-приложение, мобильное или классическое приложение, вызывает [политику проверяющей стороны](relyingparty.md). Этот файл выполняет определенную задачу, например вход, сброс пароля или изменение профиля. Политика проверяющей стороны настраивает список утверждений, которые получает приложение проверяющей стороны в составе выданного маркера. Одна и та же политика может использоваться в нескольких приложениях. Все приложения получают одинаковую консику с претензиями, и пользователь проходит через одно и то же путешествие пользователя. В одном приложении может использоваться несколько политик.
...
Файл политики **SocialAndLocalAccounts** в [начальном пакете](custom-policy-get-started.md#custom-policy-starter-pack) содержит пути взаимодействия пользователя SignUpOrSignIn, ProfileEdit и PasswordReset. Вы можете добавить больше поездок пользователей для других сценариев, таких как изменение адреса электронной почты или ссылка и отсоединение социальной учетной записи.
...
![Диаграмма, показывающая поток выполнения политики](./media/trustframeworkpolicy/custom-policy-execution.png)
