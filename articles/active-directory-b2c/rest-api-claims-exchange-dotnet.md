---
title: Интеграция обмена REST APIми утверждениями в пути взаимодействия пользователя
...
* Отправка входящих утверждений и их чтение в коде
...
1. В окне **Новый проект** выберите **Visual C#**  > **Интернет** > **Веб-приложение ASP.NET (.NET Framework)** .
1. В поле **Имя** введите имя приложения (например, *Contoso.AADB2C.API*), а затем нажмите кнопку **ОК**.

    ![Создание нового проекта Visual Studio в Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-create-project.png)

1. В окне **Создание веб-приложения ASP.NET** выберите шаблон **Веб-API** или **Приложение Azure API**.

    ![Выбор шаблона веб-API в Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-select-web-api.png)
...
1. В обозревателе решений щелкните правой кнопкой мыши папку **Модели**, выберите **Добавить**, а затем щелкните **Класс**.
    ...
    ![Пункт меню "добавить класс", выбранный в Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-model.png)

1. Назовите класс `InputClaimsModel`, а затем добавьте следующие свойства в класс `InputClaimsModel`.
...
1. Создайте модель `OutputClaimsModel`, а затем добавьте следующие свойства в класс `OutputClaimsModel`.
...
1. Создайте еще одну модель, `B2CResponseContent`, которая будет использоваться для создания сообщений об ошибках проверки входных данных. Добавьте следующие свойства в класс `B2CResponseContent`, укажите отсутствующие ссылки и сохраните файл.
...
1. В обозревателе решений щелкните правой кнопкой мыши папку **Контроллеры**, выберите **Добавить**, а затем щелкните **Контроллер**.
    ...
    ![Добавление нового контроллера в Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-1.png)

1. В окне **Добавление шаблона** выберите **Web API Controller - Empty** (Контроллер веб-API — пустой) и нажмите кнопку **Добавить**.

    ![Выбор контроллера веб-API 2 — пустой в Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-2.png)

1. В окне **Добавление контроллера** присвойте контроллеру имя **IdentityController** и нажмите кнопку **Добавить**.

    ![Ввод имени контроллера в Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-3.png)

    В результате формирования шаблона создается файл с именем *IdentityController.cs* в папке *Controllers*.

1. Если файл *IdentityController.cs* еще не открыт, дважды щелкните его, а затем замените содержимое этого файла приведенным ниже кодом.
...
1. В обозревателе решений щелкните правой кнопкой мыши проект **Contoso.AADB2C.API**, а затем выберите **Опубликовать**.
    ...
    ![Публикация в службе приложений Microsoft Azure с помощью Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-1.png)

1. В окне **Публикация** выберите **Служба приложений Microsoft Azure**, а затем — **Опубликовать**.

    ![Создание новой Microsoft Azure службы приложений с помощью Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-2.png)
    ...
    > [!TIP]
    > Дополнительные сведения см. в разделе о публикации статьи [Создание веб-приложения ASP.NET в Azure](../app-service/app-service-web-get-started-dotnet-framework.md).

1. В поле **Имя веб-приложения** введите уникальное имя (допустимые символы: a–z, 0–9 и дефис (-)). URL-адрес веб-приложения: http://<app_name>.azurewebsites.NET, где *app_name* — это имя вашего веб-приложения. Можно принять имя, созданное автоматически, если оно является уникальным.

    ![Настройка свойств службы приложений](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-3.png)

1. Чтобы начать создавать ресурсы Azure, нажмите кнопку **Создать**.
    После создания веб-приложения ASP.NET мастер опубликует его в Azure и запустит приложение в браузере по умолчанию.

1. Скопируйте URL-адрес веб-приложения.
...
1. Повторите последний шаг, используя файл SignUpOrSignIn.xml.
...
2. Откройте **B2C_1A_signup_signin**, отправленную вами пользовательскую политику проверяющей стороны, а затем выберите **Run Now** (Запустить сейчас).
    ...
    ![Страница настраиваемой политики B2C_1A_signup_signin в портал Azure](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-run.png)

3. Проверьте процесс, введя **Test** в поле **Имя**.
    В верхней части страницы Azure AD B2C отобразится сообщение об ошибке.

    ![Проверка указанной проверки ввода имени на странице входа для входа](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-test.png)
...
Ваша следующая задача — защитить API RESTFUL с использованием обычной или проверки подлинности на основе сертификата клиента. Дополнительные сведения о защите API см. в следующих статьях:
...
Сведения обо всех элементах, доступных в техническом профиле RESTFUL, см. в статье [справочник: технический профиль RESTful](restful-technical-profile.md).
