...
description: Узнайте о типах приложений, которые можно использовать с помощью Azure Active Directory B2C.
...
---
# <a name="application-types-that-can-be-used-in-active-directory-b2c"></a>Типы приложений, которые могут быть использованы в Active Directory B2C

Активный каталог Azure B2C (Azure AD B2C) поддерживает аутентификацию для различных современных архитектур приложений. Все они основаны на стандартных отраслевых протоколах: [OAuth 2.0](protocols-overview.md) или [OpenID Connect](protocols-overview.md). В этой статье описаны типы приложений, которые можно создавать, независимо от языка или платформы, которые вы предпочитаете. Это также поможет понять сценарии высокого уровня, прежде чем приступать к созданию приложений.
...
Каждый запрос, отправляемый в Azure AD B2C, определяет **поток пользователей** (встроенная политика) или **пользовательскую политику,** контролирующая поведение Azure AD B2C. Оба типа политики позволяют создать настраиваемый набор пользовательских интерфейсов.
...
9. Безопасные данные возвращаются в веб-приложение.
...
Многие архитектуры включают в себя интерфейс веб-API, которому требуется вызывать другой нисходящий веб-API. При этом Azure AD B2C защищает оба интерфейса. Этот сценарий распространен среди местных клиентов, которые имеют веб-aPI бэк-энди и вызывает онлайн-службы Microsoft, такие как Microsoft Graph API.
