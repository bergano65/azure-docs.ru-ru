...
Azure Active Directory B2C (Azure AD B2C) поддерживает проверку подлинности для различных современных архитектур приложений. Все они основаны на стандартных отраслевых протоколах: [OAuth 2.0](protocols-overview.md) или [OpenID Connect](protocols-overview.md). В этой статье описываются типы приложений, которые можно создавать независимо от предпочитаемого языка или платформы. Это также поможет понять сценарии высокого уровня, прежде чем приступать к созданию приложений.

Каждое приложение, которое использует Azure AD B2C, должно быть зарегистрировано в [клиенте Azure AD B2C](tutorial-create-tenant.md) с помощью [портала Azure](https://portal.azure.com/). Процесс регистрации собирает и присваивает следующие значения.
...
1. Приложение направляет пользователя в конечную точку версии 2.0 для выполнения [политики](user-flow-overview.md).
...
Все типы токенов и утверждений, доступных для приложения, описаны в справочнике [Azure AD B2C: справочник по маркерам](tokens-overview.md).

В веб-приложениях выполнение любой [политики](user-flow-overview.md) включает в себя следующие действия.
...
Чтобы увидеть этот сценарий в действии, изучите один из примеров кода входа в веб-приложение в разделе [Приступая к работе](overview.md).

Помимо простого входа в систему, приложению веб-сервера может потребоваться доступ к некоторым серверным веб-службам. В этом случае веб-приложение может выполнять несколько иной [поток OpenID Connect](openid-connect.md) и получать токены, используя коды авторизации и токены обновления. Этот сценарий представлен ниже в разделе [Веб-API](#web-apis).

## <a name="single-page-applications"></a>Одностраничные приложения
Многие современные веб-приложения создаются как одностраничные приложения на стороне клиента ("одностраничные приложения"). Разработчики пишут их с помощью JavaScript или платформы SPA, например, под углом, Vue и реагируют на них. Эти приложения работают в веб-браузере и имеют различные характеристики проверки подлинности, чем традиционные веб-приложения на стороне сервера.

Azure AD B2C предоставляет **два** параметра для включения одностраничных приложений для входа пользователей и получения маркеров для доступа к серверным службам или веб-API:

### <a name="authorization-code-flow-with-pkce"></a>Поток кода авторизации (с PKCE)
- [Поток кода авторизации OAuth 2,0 (с PKCE)](./authorization-code-flow.md). Поток кода авторизации позволяет приложению обмениваться кодом авторизации для маркеров **идентификации** , представляющих пользователя, прошедшего проверку подлинности, и маркеры **доступа** , необходимые для вызова защищенных API. Кроме того, он возвращает маркеры **обновления** , которые предоставляют долгосрочный доступ к ресурсам от имени пользователей без необходимости взаимодействия с этими пользователями. 

Это **рекомендуемый** подход. Наличие маркеров обновления с ограниченным временем жизни позволяет приложению адаптироваться к [современным ограничениям на конфиденциальность файлов cookie браузера](../active-directory/develop/reference-third-party-cookies-spas.md), например Safari ITP.

Чтобы воспользоваться преимуществами этого потока, приложение может использовать библиотеку проверки подлинности, которая поддерживает ее, например [MSAL.js 2. x](https://github.com/AzureAD/microsoft-authentication-library-for-js/tree/dev/lib/msal-browser).

<!-- ![Single-page applications-auth](./media/tutorial-single-page-app/spa-app-auth.svg) -->
![Одностраничные приложения — проверка подлинности](./media/tutorial-single-page-app/active-directory-oauth-code-spa.png)

### <a name="implicit-grant-flow"></a>Поток неявного предоставления
- [Неявный поток OAuth 2,0](implicit-flow-single-page-application.md). Некоторые платформы, например [MSAL.js 1. x](https://github.com/AzureAD/microsoft-authentication-library-for-js/tree/dev/lib/msal-core), поддерживают только поток неявного предоставления. Неявный поток предоставления позволяет приложению получать **идентификаторы** и маркеры **доступа** . В отличие от потока кода авторизации, неявный поток предоставления не возвращает **маркер обновления**. 

Этот поток проверки подлинности не включает сценарии приложений, в которых используются межплатформенные платформы JavaScript, такие как электронное и собственное реагирование. В этих сценариях требуются дополнительные возможности для взаимодействия с собственными платформами.
...
Интерфейс веб-API может использовать маркер для проверки удостоверения объекта, вызывающего API, и получения сведений о нем из утверждений, которые закодированы в маркере. Все типы токенов и утверждений, доступных для приложения, описаны в [справочнике по токенам Azure AD B2C](tokens-overview.md).
