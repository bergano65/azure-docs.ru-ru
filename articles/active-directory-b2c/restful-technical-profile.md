...
| алловинсекуреаусинпродуктион| Нет| Указывает, можно ли задать для `AuthenticationType` значение `none` в рабочей среде (для`DeploymentMode` параметра [TrustFrameworkPolicy](trustframeworkpolicy.md) задано значение `Production`или не указано). Возможные значения: true или false (по умолчанию). |
