...
ms.locfileid: "80051495"
---
# <a name="contentdefinitions"></a>ContentDefinitions

[!INCLUDE [active-directory-b2c-advanced-audience-warning](../../includes/active-directory-b2c-advanced-audience-warning.md)]

Вы можете настроить внешний вид любого [самоподтвержденного технического профиля](self-asserted-technical-profile.md). Azure Active Directory B2C (Azure AD B2C) запускает код в браузере клиента и использует современный подход под названием Cross-Origin Resource Sharing (CORS).
...
| RecoveryUri | 1:1 | Строка, содержащая URL-адрес страницы HTML для отображения ошибки, связанной с определением содержимого. В настоящее время не `~/common/default_page_error.html`используется значение должно быть . |
...
Вы можете включить [код клиента JavaScript,](javascript-samples.md) вставив `contract` между `elements` и типстраницы. Например, `urn:com:microsoft:aad:b2c:elements:contract:page-name:version`.

[!INCLUDE [active-directory-b2c-public-preview](../../includes/active-directory-b2c-public-preview.md)]

В [части версии](page-layout.md) `DataUri` указан пакет содержимого, содержащего HTML, CSS и JavaScript для элементов пользовательского интерфейса в вашей политике. Если вы намереваетесь включить клиентский код JavaScript, элементы, на которые вы основываете JavaScript, должны быть неизменяемыми. Если они не являются неизменяемыми, любые изменения могут привести к неожиданному поведению на ваших страницах пользователей. Чтобы предотвратить эти проблемы, заполните использование макета страницы и укажите версию макета страницы. Это гарантирует, что все определения содержимого, на основе которого вы основывали JavaScript, неизменяемы. Даже если вы не собираетесь включать JavaScript, вам все равно нужно указать версию макета страницы для ваших страниц.

Следующий пример показывает **DataUri** версии: `selfasserted` `1.2.0`
...
Формат значения должен содержать слово `contract`: _urn:com:microsoft:aad:b2c:elements:**contract**:page-name:version_. Чтобы указать макет страницы в пользовательских политиках, в которых используется старое значение **DataUri,** используйте следующую таблицу для перехода в новый формат.
...
| --------- | -------- | ----------- |
| DisplayName | нет | Строка, содержащая название определения содержимого. |
...
Например, настроить пользовательский интерфейс с помощью определений содержимого см.:

[Настройка пользовательского интерфейса приложения с помощью пользовательской политики](custom-policy-ui-customization.md)
