...
Azure AD B2C позволяет расширить набор атрибутов, хранящихся в каждой учетной записи пользователя. Эти атрибуты можно также считывать и записывать с помощью [Microsoft API Graph](microsoft-graph-operations.md).
...
1. Выберите **Атрибуты пользователя**, а затем — настраиваемый атрибут (например, ShoeSize). Щелкните **Save** (Сохранить).
