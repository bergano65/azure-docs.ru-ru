...
Преобразование « **ассертстрингклаимсарикуал** Claims» всегда выполняется из [технического профиля проверки](validation-technical-profile.md) , который вызывается [самостоятельно подтвержденным техническим профилем](self-asserted-technical-profile.md)или [дисплайконртол](display-controls.md). Метаданные `UserMessageIfClaimsTransformationStringsAreNotEqual` самостоятельно утвержденного технического профиля позволяют управлять сообщением об ошибке, представленным пользователю.
...
| Элемент | TransformationClaimType | Тип данных | Примечания |
| ---- | ----------------------- | --------- | ----- |
| InputClaim | inputClaim1 | string | Изменяемый параметр. |
...
| Элемент | TransformationClaimType | Тип данных | Примечания |
|----- | ----------------------- | --------- | ----- |
| InputParameter | value | string | Строка, которую необходимо задать. Этот входной параметр поддерживает [выражения преобразования строковых утверждений](string-transformations.md#string-claim-transformations-expressions). |
...
| InputParameter | stringFormat | string | Формат строки, включая параметр {0}. Этот входной параметр поддерживает [выражения преобразования строковых утверждений](string-transformations.md#string-claim-transformations-expressions).  |
| outputClaim | outputClaim | string | Параметр ClaimType, который создается после вызова этого преобразования утверждений. |
...
| InputParameter | stringFormat | string | Формат строки, включая параметры {0} и {1}. Этот входной параметр поддерживает [выражения преобразования строковых утверждений](string-transformations.md#string-claim-transformations-expressions).   |
| outputClaim | outputClaim | string | Параметр ClaimType, который создается после вызова этого преобразования утверждений. |
...
| Элемент | TransformationClaimType | Тип данных | Примечания |
| ---- | ----------------------- | --------- | ----- |
| outputClaim | Имя локализованной строки | string | Список типов утверждений, созданных после вызова этого преобразования утверждений. |
...
| outputClaim | inputParameterId | string | Параметр ClaimType, который будет создан после вызова этого преобразования утверждений. Значение сопоставления `Id`. |
...
Если `errorOnFailedLookup` входной параметр имеет значение `true`, преобразование утверждений **LookupValue** всегда выполняется из [технического профиля проверки](validation-technical-profile.md) , который вызывается [самостоятельно подтвержденным техническим профилем](self-asserted-technical-profile.md)или [дисплайконртол](display-controls.md). Метаданные `LookupNotFound` самостоятельно утвержденного технического профиля позволяют управлять сообщением об ошибке, представленным пользователю.
...
| Элемент | TransformationClaimType | Тип данных | Примечания |
| ---- | ----------------------- | --------- | ----- |
| outputClaim | claim_to_null | string | Для утверждения задано значение NULL. |
...
| InputParameter | matchTo | string | Регулярное выражение для сопоставления. |
| InputParameter | outputClaimIfMatched | string | Значение, которое необходимо задать, если строки равны. |
| outputClaim | outputClaim | string | Если регулярное выражение совпадает, это выходное утверждение содержит значение `outputClaimIfMatched` входного параметра. Или null, если совпадений нет. |
...
| Элемент | TransformationClaimType | Тип данных | Примечания |
| ---- | ----------------------- | --------- | ----- |
| InputClaim | InputClaim | string | Тип утверждения, для которого выполняется поиск. |
|InputParameter|contains|string|Искомое значение.|
|InputParameter|ignoreCase|string|Указывает, должно ли это сравнение учитывать регистр сравниваемой строки.|
| outputClaim | outputClaim | string | ClaimType, который создается после вызова ClaimsTransformation. Логический индикатор, если подстрока встречается во входном утверждении. |
...
| Элемент | TransformationClaimType | Тип данных | Примечания |
| ---- | ----------------------- | --------- | ----- |
| InputClaim | InputClaim | string | Тип утверждения, который содержит строку. |
...
| ---- | ----------------------- | --------- | ----- |
| InputClaim | InputClaim | string | Тип утверждения, который содержит строку. |
| InputParameter | старое_значение | string | Искомая строка. |
| InputParameter | newValue | string | Строка для замены всех вхождений `oldValue` |
...
| InputParameter | разделитель | string | Строка, используемая в качестве разделителя, например запятая `,`. |
| outputClaim | outputClaim | string | Строка, состоящая из элементов коллекции строк `inputClaim`, разделенных входным параметром `delimiter`. |
...
| Элемент | TransformationClaimType | Тип данных | Примечания |
| ---- | ----------------------- | --------- | ----- |
| InputClaim | InputClaim | string | Тип строкового утверждения, который содержит подстрочные строки для разбиения. |
| InputParameter | разделитель | string | Строка, используемая в качестве разделителя, например запятая `,`. |
