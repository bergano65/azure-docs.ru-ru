---
title: Примеры преобразования строки для пользовательских политик
titleSuffix: Azure AD B2C
description: Строка требует примеры трансформации для схемы интерфейса identity Experience (IEF) Active Directory B2C.
...
В этой статье приводятся примеры использования преобразований строк и требований в схеме интерфейса Identity Experience в Azure Active Directory B2C (Azure AD B2C). Дополнительные сведения см. в статье о [преобразовании утверждений](claimstransformations.md).
...
Преобразование **assertStringClaimsAreEqual** претензий всегда выполняется из [технического профиля проверки,](validation-technical-profile.md) который называется [самоутверждаемым техническим профилем,](self-asserted-technical-profile.md)или [DisplayConrtol.](display-controls.md) Метаданные `UserMessageIfClaimsTransformationStringsAreNotEqual` самоутвержденного технического профиля контролируют сообщение об ошибке, представленное пользователю. Сообщения об ошибках могут быть [локализованы.](localization-string-ids.md#claims-transformations-error-messages)
...
- Входящие утверждения:
  - **входная претензия1**:someone@contoso.com
  - **входная претензия2**:someone@outlook.com
- Входные параметры:
...
- Исходящие утверждения:
  - **выходная претензия**:cpim_5164db16-3eee-4629-bfda-dcc3326790e9@b2cdemo.onmicrosoft.com
...
| InputParameter | stringFormat | строка | Формат строки, включая параметры {0} и {1}. Этот параметр ввода поддерживает [выражения преобразования строки.](string-transformations.md#string-claim-transformations-expressions)   |
| outputClaim | outputClaim | строка | Параметр ClaimType, который создается после вызова этого преобразования утверждений. |
...
## <a name="getlocalizedstringstransformation"></a>GetLocalizedStringsТрансформация
...
1. Определите [строку локализации](localization.md) и связывай ее с [самоутверждаемым-техническим профилем.](self-asserted-technical-profile.md)
1. Элемент должен быть установлен `GetLocalizedStringsTransformationClaimType`на . `ElementType` `LocalizedString`
1. Это `StringId` уникальный идентификатор, который вы определяете, и использовать его позже в преобразовании претензий.
1. При преобразовании претензий укажите список претензий, которые будут установлены с локализованной строкой. Это `ClaimTypeReferenceId` ссылка на ClaimType, уже определенный в разделе ClaimsSchema в политике. Это `TransformationClaimType` название локализованной строки, как `StringId` определено `LocalizedString` в элементе.
1. В [самоутвержденном техническом профиле](self-asserted-technical-profile.md)или преобразовании входных данных [управления дисплеем](display-controls.md) или преобразованиях вывода, сделайте ссылку на преобразование претензий.

![GetLocalizedStringsТрансформация](./media/string-transformations/get-localized-strings-transformation.png)

Следующий пример смотрит на тему электронной почты, тело, ваше кодовое сообщение, и подпись электронной почты, из локализованных строк. Эти утверждения позже используются пользовательским шаблоном проверки электронной почты.

Определите локализованные строки для английского (по умолчанию) и испанского языка.
...
Преобразование претензий устанавливает значение *объекта* типа претензии со значением `StringId` *email_subject.*
...
- Исходящие утверждения:
    - **restrictionValueClaim**: Вы не можете войти, потому что вы несовершеннолетний.
...
| outputClaim | inputParameterId | строка | Параметр ClaimType, который будет создан после вызова этого преобразования утверждений. Значение соответствия `Id`. |
...
При `errorOnFailedLookup` `true`настройке параметра ввода преобразование требований **LookupValue** всегда выполняется из [технического профиля проверки,](validation-technical-profile.md) который называется [самоутверждаемым техническим профилем](self-asserted-technical-profile.md)или [DisplayConrtol.](display-controls.md) Метаданные `LookupNotFound` самоутвержденного технического профиля контролируют сообщение об ошибке, представленное пользователю.

![Выполнение AssertStringClaimsAreEqual](./media/string-transformations/assert-execution.png)

В следующем примере выполняется поиск доменного имени в одной из коллекций inputParameters. Преобразование требований ищет доменное имя в идентификаторе и возвращает его значение (идентификатор приложения) или поднимает сообщение об ошибке.
...
- Входящие утверждения:
    - **вхотливый Паразамилид**: live.com
- Входные параметры:
    - **contoso.com**: 13c15f79-8fb1-4e29-a6c9-be0d36ff19f1.
    - **microsoft.com**: 0213308f-17cb-4398-b97e-01da7bd4804e.
    - **test.com**: c7026f88-4299-4cdb-965d-3f166464b8a9.
    - **errorOnFailedLookup**: правда
- Ошибка:
    - Не найдено совпадений для значения ввода претензии в списке идентификаторов параметров ввода и ошибкаOnFailedLookup не соответствует действительности.
...
Используйте эту трансформацию претензий, чтобы удалить ненужные данные из пакета свойств претензий, чтобы файл cookie сеанса был меньше. В следующем примере удаляется значение типа утверждения `TermsOfService`.
...
- Входные параметры:
    - **matchTo**: "0-9"{4,16}$"
    - **выходНаяclaimIfMatched**: "isPhone"
- Исходящие утверждения:
...
- Входные параметры:
    - **matchTo**: V1.
    - **строка Сравнение**: ordinalIgnoreCase
...
|InputParameter|ignoreCase|строка|Уточняется, следует ли игнорировать это сравнение в случае сравнения строки.|
| outputClaim | outputClaim | строка | ClaimType, который создается после вызова ClaimsTransformation. Индикатор boolean, если подстрока происходит в пределах входиной претензии. |

Используйте это преобразование требований, чтобы проверить, содержит ли тип требования строки подстроку. Следуя примеру, `roles` проверите, содержит ли тип требования строки значение **админ.**
...
- Входящие утверждения:
    - **inputclaim**: "Админ, Approver, редактор"
- Входные параметры:
    - **содержит**: "админ",
    - **ignoreCase**: правда
- Исходящие утверждения:
...
## <a name="stringsubstring"></a>СтрунныйСубъект
...
| outputClaim | outputClaim | Логическое | Строка, эквивалентная подстроке длины, которая начинается в startIndex в данном экземпляре, или Пустая, если стартовый индекс равен длине этого экземпляра, а длина равна нулю. |

Например, получить приставку страны номера телефона.
...
- Входящие утверждения:
    - **вводпретензии**: "1644114520"
- Входные параметры:
    - **стартовыйиндекс**: 0
    - **длина**: 2
- Исходящие утверждения:
    - **выходная претензия**: "No1"
...
| InputParameter | newValue | строка | Строка для замены всех случаев`oldValue` |
| outputClaim | outputClaim | Логическое | Строка, эквивалентная текущей строке, за исключением того, что все экземпляры oldValue заменяются на newValue. Если oldValue не найдено в текущем экземпляре, метод возвращает текущий экземпляр без изменений. |

Например, нормализовать номер телефона, `-` удалив символы
...
- Входящие утверждения:
    - **вводпретензии**: "164-411-452-054"
- Входные параметры:
    - **oldValue**: "-"
    - **длина**: ""
- Исходящие утверждения:
    - **выходная претензия**: "164411452054"
...
| outputClaim | outputClaim | строка | Строка, состоящая из `inputClaim` членов коллекции строк, `delimiter` делегированная параметром ввода. |

В следующем примере приводится строка сбора ролей пользователя и преобразует его в строку разграничества запятых. Этот метод можно использовать для хранения коллекции строк в учетной записи пользователя Azure AD. Позже, когда вы читаете учетную `StringSplit` запись из каталога, используйте для преобразования строки делимитера запятой обратно в строку строки.
...
- Входящие утверждения:
  - **inputclaim**: " Админ", "Автор", "Читатель"
- Входные параметры:
  - **делимитер**: ","
- Исходящие утверждения:
  - **выходная претензия**: "Админ, Автор, Читатель"


## <a name="stringsplit"></a>СтрунСплит
...
| InputParameter | разделитель | строка | Строка для использования в качестве сепаратора, например, запятая. `,` |
| outputClaim | outputClaim | stringCollection | Строка, элементы которой содержат подстроки в этой `delimiter` строке, которые разграничены параметром ввода. |

В следующем примере приводится строка делимитедора запятой ролей пользователя и преобразует ее в сбор строк.
...
## <a name="string-claim-transformations-expressions"></a>Строка претензии преобразования выражения
Выражения преобразования претензий в пользовательских политиках Azure AD B2C предоставляют контекстную информацию об идентификаторе клиента и идентификаторе технического профиля.

  | Выражение | Описание | Пример |
 | ----- | ----------- | --------|
 | `{TechnicalProfileId}` | Техническое имя profileId. | Facebook-OAUTH |
