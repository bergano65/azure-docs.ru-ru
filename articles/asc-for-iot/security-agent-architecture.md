---
title: Основные сведения о ASC для агента-архитектура безопасности Интернета вещей Preview | Документация Майкрософт
description: Общие сведения о безопасности архитектура агента для агентов, используемых в ASC для службы Интернета вещей.
services: ascforiot
...
ASC для Интернета вещей предоставляет эталонной архитектуры для агентов безопасности, журналов, обрабатывать, статистической обработки и отправки данных безопасности через Центр Интернета вещей.
...
Агенты безопасности поддерживают следующие возможности решений Интернета вещей:

- Собирает события необработанной безопасности от нижележащей ОС (Linux, Windows). Дополнительные сведения о сборщиках данных для системы безопасности, см. в разделе [ASC для настройки агента IoT](concept-agent-configuration.md).
...
- Удаленной настройки посредством использования платформы **ascforiot** двойник модуля. Дополнительные сведения см. в разделе [Настройка ASC для Интернета вещей агента](concept-agent-configuration.md).

ASC для агентов безопасности Интернета вещей разрабатываются в виде проектов с открытым кодом и доступны из GitHub: 

- [IoT-ASC-агента C](https://github.com/Azure/IoT-ASC-Agent-C) 
...
- Использовать примеры [настроить решение](quickstart-configure-your-solution.md)
- Понять [методы проверки подлинности агента безопасности](concept-security-agent-authentication-methods.md)
- Выберите и разверните [агент безопасности](select-deploy-agent.md)
