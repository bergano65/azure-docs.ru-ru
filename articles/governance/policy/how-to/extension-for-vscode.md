...
1. Запустите команду Subscription из палитры команд или нижнего колонтитула окна.
   ...
   - Нижний колонтитул окна
     ...
     В нижнем колонтитуле окна в нижней части экрана выберите сегмент, который соответствует **Azure: \<ваша учетная запись\>**.
...
При выборе ресурса с помощью интерфейса поиска или при выборе его в TreeView расширение политики Azure открывает JSON-файл, представляющий этот ресурс, и все его диспетчер ресурсов значения свойств.

После открытия ресурса при наведении указателя мыши на имя или значение свойства диспетчер ресурсов отображается псевдоним политики Azure, если он существует. В этом примере ресурс является типом `Microsoft.Compute/virtualMachines` ресурса, а свойство **Properties. storageProfile. imageReference. предложение** наводится на него. При наведении указателя мыши отображаются совпадающие псевдонимы.
