...
При аудите системы Windows гостевая конфигурация применяет модуль ресурсов [Desired State Configuration (DSC)](/powershell/scripting/dsc/overview/overview) для создания файла конфигурации. Эта конфигурация DSC определяет состояние, в котором должен находиться компьютер.
Если оценка конфигурации проходит неудачно, то срабатывает политика **auditIfNotExists** и компьютер считается **несоответствующим**.
...
> Чтобы развернуть расширение в большом масштабе на всех компьютерах Windows, назначьте приведенные ниже определения политик.
...
Свойство Reasons также нужно добавить в качестве внедренного класса в MOF-файл схемы для ресурса.
...
> [!Note]
> В последней версии модуля гостевой конфигурации добавлены новые параметры:
...
>   - Если этот параметр не указан, по умолчанию используется категория гостевой конфигурации.
> Эти функции предоставляются в предварительной версии и требуют для работы модуль гостевой конфигурации версии 1.20.1, который можно установить с помощью `Install-Module GuestConfiguration -AllowPrerelease`.
...
Ресурс DSC нужно создать самостоятельно, если сообщество еще не предложило готового решения.
...
Командлет `New-GuestConfigurationPackage` создает пакет. Для содержимого от сторонних разработчиков укажите параметр **FilesToInclude**, чтобы добавить содержимое InSpec в пакет. Параметр **ChefProfilePath**, как для пакетов Linux, использовать не нужно.
...
## <a name="converting-windows-group-policy-content-to-azure-policy-guest-configuration"></a>Преобразование содержимого групповой политики Windows в гостевую конфигурацию в службе "Политика Azure"

Гостевая конфигурация при аудите компьютеров Windows использует синтаксис PowerShell Desired State Configuration. Сообщество DSC опубликовало средства для преобразования экспортированных шаблонов групповой политики в формат DSC. Применяя это средство в сочетании с командлетами гостевой конфигурации, описанными выше, вы можете преобразовать содержимое групповой политики Windows, упаковать и опубликовать ее для аудита в службе "Политика Azure". Дополнительные сведения об использовании этого средства см. в статье [Краткое руководство. Преобразование групповой политики в DSC](/powershell/scripting/dsc/quickstarts/gpo-quickstart).
После преобразования содержимого выполните описанные выше действия, как для любого содержимого DSC, чтобы создать пакет и опубликовать его в качестве Политики Azure.
