...
Назначьте разрешения для экземпляра, чтобы предоставить назначенному пользователю доступ к секретам, хранящимся в Key Vault. Подробные инструкции см. [в разделе Установка и получение секретного кода — .NET](../../../key-vault/quick-create-net.md#assign-permissions-to-your-application-to-read-secrets-from-key-vault).
...
- **Версия.** При запуске командлета командлета `New-GuestConfigurationPolicy` необходимо указать номер версии, превышающий текущую публикацию.  Это приведет к обновлению версии назначения гостевой конфигурации в новом файле политики, чтобы расширение знало, что пакет был обновлен.
- **контенсаш**: Это автоматически `New-GuestConfigurationPolicy` обновляется командлетом.  Это хэш-значение пакета, созданного `New-GuestConfigurationPackage`.  Это должно быть верно для `.zip` публикуемого файла.  Если обновляется только `contentUri` свойство, например в случае, когда кто-либо может внести изменение в определение политики с портала вручную, расширение не примет пакет содержимого.
...
Это гарантирует, что все свойства будут обновлены правильно.
...
После преобразования содержимого шаги, описанные выше, помогут создать паккаже и опубликовать его как политику Azure, как и для любого содержимого DSC.
