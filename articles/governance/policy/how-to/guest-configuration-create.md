...
- Оболочк. Если она еще не установлена, выполните [эти инструкции](/powershell/scripting/install/installing-powershell).
...
1. Наконец, в настраиваемом ресурсе используйте идентификатор клиента, созданный выше, для доступа к Key Vault с помощью маркера, доступного на компьютере.
   ...
   @No__t-0 и URL-адрес экземпляра Key Vault можно передать в ресурс в качестве [свойств](/powershell/scripting/dsc/resources/authoringresourcemof#creating-the-mof-schema) , чтобы ресурс не обновлялся для нескольких сред или если значения необходимо изменить.
