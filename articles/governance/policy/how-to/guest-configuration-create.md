...
- **Path**: Output folder path. Это необязательный параметр. If not specified, the package is created in current directory.
...
For Linux policies, include the property **AttributesYmlContent** in your configuration and overwrite the values accordingly. The Guest Configuration agent automatically creates the YaML file used by InSpec to store attributes. Ознакомьтесь с примером ниже.
