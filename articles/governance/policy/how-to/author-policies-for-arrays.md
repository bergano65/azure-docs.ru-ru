...
Это означает, что при ссылке на элемент counted Array с `field()` функцией внутри `where` условия **она возвращает массив с одним элементом**. Хотя это и не может быть интуитивно понятным, оно согласуется с идеями того, что псевдонимы массивов всегда возвращают коллекцию выбранных свойств. Пример:
