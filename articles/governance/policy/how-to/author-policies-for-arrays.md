...
Псевдонимы, имеющие **\[\*\]** присоединенные к их имени, указывают, что этот **тип** является _массивом_. Вместо того чтобы оценивать значение всего массива, **\[\*\]** позволяет оценивать каждый элемент массива по отдельности с помощью логического и между ними. Существует три стандартных сценария для оценки каждого элемента, которые полезны в следующих случаях: _нет_, _любой_или _все_ элементы совпадают.
Для сложных сценариев используйте [Count](../concepts/definition-structure.md#count).
