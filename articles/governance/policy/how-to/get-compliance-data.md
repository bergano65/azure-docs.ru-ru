...
Результаты завершенного цикла оценки отражаются в поставщике ресурсов `Microsoft.PolicyInsights` с помощью операций `PolicyStates` и `PolicyEvents`. Дополнительные сведения об операциях REST API Policy Insights см. [здесь](/rest/api/policy-insights/).
...
Как часть новой **общедоступной предварительной версии** для несоответствующих ресурсов доступны данные журнала изменений за последние четырнадцать дней. Журнал изменений содержит подробные сведения о том, когда было обнаружено изменение и _отличия между визуальными элементами_ для каждого изменения. Обнаружение изменений активируется, когда свойства Resource Manager несоответствующего ресурса добавляются, удаляются или изменяются.

1. Запустите службу "Политика Azure" на портале Azure, щелкнув **Все службы**, а затем выполнив поиск и выбрав **Политика**.

1. На странице **Обзор** или **Соответствие** выберите политику, которая является _несоответствующей_.

1. На вкладке **Соответствие ресурсов** страницы **Соответствие политики** выберите ресурс, который является _несоответствующим_.
...
1. На странице **Соответствие ресурсов** выберите вкладку **Журнал изменений (предварительная версия)**. После этого отобразится список обнаруженных изменений (при наличии).
   ...
   ![Журнал изменений политики — вкладка](../media/getting-compliance-data/change-history-tab.png)

1. Выберите одно из обнаруженных изменений. _Отличия между визуальными элементами_ для несоответствующих ресурсов представлены на странице **Журнал изменений**.

   ![Журнал изменений политики — отличия между визуальными элементами](../media/getting-compliance-data/change-history-visual-diff.png)

_Отличия между визуальными элементами_ позволяют обнаружить изменения ресурса. Обнаруженные изменения могут быть не связаны с тем, что сделало ресурс несоответствующим для выбранной политики.
...
Модуль Azure PowerShell для политики доступен в коллекции PowerShell как [Az.PolicyInsights](https://www.powershellgallery.com/packages/Az.PolicyInsights). Используя PowerShellGet, вы можете установить модуль с помощью `Install-Module -Name Az.PolicyInsights` (установите последнюю версию [Azure PowerShell](/powershell/azure/install-az-ps)):
...
Если у вас есть рабочая область [Log Analytics](../../../log-analytics/log-analytics-overview.md) со связанным с подпиской решением `AzureActivity`, вы также можете просмотреть результаты проверки несоответствия из циклов оценки, используя простые запросы Azure Data Explorer и таблицу `AzureActivity`. С учетом подробных сведений о несоответствии в Azure Monitor вы также можете настроить оповещения для отслеживания несоответствия.
