...
ms.locfileid: "79280642"
---
# <a name="get-compliance-data-of-azure-resources"></a>Получайте данные о соответствии ресурсам Azure

Одним из наибольших преимуществ Политики Azure является анализ и контроль, который эта служба выполняет над ресурсами в подписке или [группе управления](../../management-groups/overview.md) подписками. Этот контроль может быть реализован разными способами, такими как предотвращение создания ресурсов в неправильном месте, принудительное использование общих и согласованных тегов, а также аудит существующих ресурсов на предмет соответствующих конфигураций и параметров. Во всех случаях данные генерируются политикой Azure, чтобы можно было понять состояние соответствия среде.

Существует несколько способов доступа к информации о соответствии, которая создается при назначении политик и инициатив:

- Использование [портала Azure](#portal)
...
> [!WARNING]
> Если состояние соответствия регистрируется как **не зарегистрированное,** убедитесь, что поставщик ресурсов **Microsoft.PolicyInsights** зарегистрирован и что пользователь имеет соответствующие разрешения управления доступом на основе ролей (RBAC), как описано [в RBAC в Azure Policy.](../overview.md#rbac-permissions-in-azure-policy)

## <a name="evaluation-triggers"></a>Триггеры оценки

Результаты завершенного цикла оценки отражаются в поставщике ресурсов `Microsoft.PolicyInsights` с помощью операций `PolicyStates` и `PolicyEvents`. Более подробную информацию об операциях API Azure Policy Insights REST можно получить, [см.](/rest/api/policy-insights/)
...
- **Конфликт**: Две или более политики существуют с противоречивыми правилами. Например, две политики, добавив один и тот же тег с разными значениями.
- **Не запущен:** Цикл оценки не начался для политики или ресурса.
- **Не зарегистрировано:** Поставщик ресурсов политики Azure не зарегистрирован или в аккаунте, внесенном в систему, нет разрешения на чтение данных о соответствии.

Политика Azure использует поля **типа** и **имени** в определении, чтобы определить, совпадает ли ресурс. Если ресурс совпадает, он считается применимым и будет иметь состояние **Соответствует** или **Не соответствует**. Если **Тип** или **Имя** является единственным свойством в определении, приемлемыми и подлежащими оценке считаются все ресурсы.
...
![Пример соответствия политики со страницы «Соответствие»](../media/getting-compliance-data/simple-compliance.png)
...
![Пример страницы соответствия требованиям политики Azure](../media/getting-compliance-data/compliance-page.png)
...
![Пример страницы «Детали соответствия требованиям политики Azure»](../media/getting-compliance-data/compliance-details.png)
...
> [!NOTE]
> Для политики AKS Engine показан ресурс — это группа ресурсов.

![Пример событий соответствия политики Azure](../media/getting-compliance-data/compliance-events.png)

Для ресурсов [режима поставщика ресурсов,](../concepts/definition-structure.md#resource-provider-modes) на вкладке **соответствия ресурсу,** выбор ресурса или нажатие права на строку и выбор **деталей соответствия View** открывает детали соответствия компонента. На этой странице также предлагаются вкладки для просмотра политик, назначенных этому ресурсу, событий, событий компонентов и истории изменений.

![Пример комплаенс-соответствия компонентаю политики Azure](../media/getting-compliance-data/compliance-components.png)

Вернитесь на страницу соответствия ресурса, нажмите на строку события, о которых вы хотели бы собрать более подробную информацию и выбрать **журналы активности Show.** Откроется страница журнала действий с предварительно отфильтрованными результатами поиска, где отображаются подробные сведения о назначениях и событиях. В журнале действий содержатся дополнительный контекст и сведения об этих событиях.

![Пример журнала о соответствии действиям по вопросам политики Azure](../media/getting-compliance-data/compliance-activitylog.png)

### <a name="understand-non-compliance"></a>Понимание несоблюдения

<a name="change-history-preview"></a>

Когда ресурсы определены как **несоответствующие,** существует множество возможных причин. Чтобы определить причину **несоблюдения** ресурса или найти ответственное изменение, [см.](./determine-non-compliance.md)

## <a name="command-line"></a>Командная строка

Такую же информацию можно получить на портале с помощью REST API (в том числе с [ARMClient),](https://github.com/projectkudu/ARMClient)Azure PowerShell и Azure CLI (предварительный просмотр).
Для получения подробной информации об API REST можно ознакомиться в справке [Azure Policy Insights.](/rest/api/policy-insights/) На страницах справки по REST API есть зеленая кнопка "Попробовать" для каждой операции, которую можно опробовать непосредственно в браузере.

Используйте ARMClient или аналогичный инструмент для обработки аутентификации в Azure для примеров REST API.

### <a name="summarize-results"></a>Суммирование результатов

С помощью REST API формирование сводных данных можно выполнить по контейнеру, определению или назначению. Вот пример обобщения на уровне подписки с помощью Azure Policy Insight [в Суммировать для подписки:](/rest/api/policy-insights/policystates/summarizeforsubscription)
...
Для получения дополнительной информации о событиях политики запросов см. [Azure Policy Events](/rest/api/policy-insights/policyevents)

### <a name="azure-powershell"></a>Azure PowerShell

Модуль Azure PowerShell для политики Azure доступен в галерее PowerShell под [as Az.PolicyInsights.](https://www.powershellgallery.com/packages/Az.PolicyInsights)
...
Если у вас есть [рабочее пространство Log Analytics](../../../log-analytics/log-analytics-overview.md) `AzureActivity` с [решением Activity Log Analytics,](../../../azure-monitor/platform/activity-log-collect.md) привязанным к подписке, вы также `AzureActivity` можете просматривать результаты несоблюдения цикла оценки с помощью простых запросов Kusto и таблицы. С учетом подробных сведений о несоответствии в журналах Azure Monitor вы также можете настроить оповещения для отслеживания несоответствия.


![Соответствие требованиям политики Azure с помощью журналов Azure Monitor](../media/getting-compliance-data/compliance-loganalytics.png)
