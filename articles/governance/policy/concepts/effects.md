...
Аудит — это последнее действие проверки политики при создании или обновлении ресурса. В диспетчер ресурсов режиме политика Azure отправляет ресурс поставщику ресурсов. Аудит работает одинаково для запроса ресурса и для цикла оценки. Политика Azure добавляет операцию `Microsoft.Authorization/policies/audit/action` в журнал действий и помечает ресурс как несовместимый.
...
- Добавьте или замените значения определенных псевдонимов (Предварительная версия).
  - Используйте `Get-AzPolicyAlias | Select-Object -ExpandProperty 'Aliases' | Where-Object { $_.DefaultMetadata.Attributes -eq 'Modifiable' }`.
    в Azure PowerShell, чтобы получить список псевдонимов, которые можно использовать с параметром MODIFY.
...
|Remove |Удаляет заданное свойство или тег из ресурса. |
