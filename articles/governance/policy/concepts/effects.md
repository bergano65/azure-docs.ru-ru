...
Пример 2. Удалите тег `env` и добавьте тег `environment` или замените существующие теги `environment` значением параметризованного значения:
...
Этот результат используется с *режимом* определения политики для `Microsoft.Kubernetes.Data`. Он используется для передачи правил управления допуском привратника версии 3, определенных с помощью [инфраструктуры ограничений непрозра](https://github.com/open-policy-agent/frameworks/tree/master/constraint#opa-constraint-framework) , чтобы [Открыть агент политики](https://www.openpolicyagent.org/) (непрозра) для самостоятельно управляемых кластеров Kubernetes в Azure.
...
- **ограничение** [обязательный]
  - Реализация CRD шаблона ограничения. Использует параметры, передаваемые через **значения** , как `{{ .Values.<valuename> }}`. В приведенном ниже примере это будет `{{ .Values.cpuLimit }}` и `{{ .Values.memoryLimit }}`.
...
Этот результат используется с *режимом* определения политики для `Microsoft.ContainerService.Data`. Он используется для передачи правил управления допуском привратника версии 2, определенных с помощью [Рего](https://www.openpolicyagent.org/docs/latest/policy-language/#what-is-rego) , для [открытия агента политики](https://www.openpolicyagent.org/) (Непрозра) в [службе Kubernetes Azure](../../../aks/intro-kubernetes.md).
