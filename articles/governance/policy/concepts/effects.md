...
Этот эффект используется с *режимом* определения политики `Microsoft.Kubernetes.Data`. Он используется для передачи правил контроля входа Gatekeeper v3, определенных с [помощью OPA Constraint Framework](https://github.com/open-policy-agent/frameworks/tree/master/constraint#opa-constraint-framework) для [open Policy Agent](https://www.openpolicyagent.org/) (OPA) в кластеры Kubernetes на Azure.

> [!NOTE]
> [Политика Azure для Kubernetes](aks-engine.md) находится в предварительном просмотре и поддерживает только встроенные определения политики.
...
Каждые 15 минут завершается полное сканирование кластера, о результатах которых сообщается в Azure Policy.
...
Пример: Правило контроля входа Gatekeeper v3 для установки процессора контейнера и ограничений ресурсов памяти в Kubernetes.
...
> [!IMPORTANT]
> [Политика Azure для Kubernetes](rego-for-aks.md) находится в предварительном просмотре и поддерживает только встроенные определения политики. Встроенные политики относятся к категории **Kubernetes.** Эффект **EnforceRegoPolicy** и связанные с ним политики категории **Kubernetes** Service _унижаются._ Вместо этого используйте обновленный эффект [EnforceOPAConstraint.](#enforceopaconstraint)
