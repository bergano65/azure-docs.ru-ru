...
Одна из инициатив, доступных в политике Azure, позволяет выполнять аудит параметров операционной системы в виртуальных машинах с помощью "базовых показателей" от корпорации Майкрософт.  Определение *[Предварительная версия]: аудит виртуальных машин Windows, которые не соответствуют базовым параметрам безопасности Azure,* включает полный набор правил аудита на основе параметров из Active Directory групповая политика.

Большинство параметров доступны в качестве параметров.  Эта функция позволяет настроить аудит для согласования политики с требованиями организации или сопоставление политики со сторонними сведениями, такими как отраслевые стандарты.

Некоторые параметры поддерживают диапазон целочисленных значений.  Например, параметр максимального срока действия пароля может быть установлен с помощью оператора Range для предоставления гибкости владельцам компьютеров.  Можно было бы проследить, что действующий параметр групповая политика, требующий от пользователя изменения паролей, должен быть не более 70 дней, но не может быть менее 1 дня.  Как описано в информационном подсказке для параметра, чтобы сделать это значение действующего аудита, установите значение "1, 70".

При назначении политики с помощью шаблона Azure Resource Manager дплоймент можно использовать файл параметров для управления этими параметрами из системы управления версиями.
С помощью такого средства, как Git, для управления изменениями политик аудита с комментариями при каждом возврате, будет документировать свидетельство относительно того, почему назначение должно находиться в исключении с ожидаемым значением.

#### <a name="applying-configurations-using-guest-configuration"></a>Применение конфигураций с помощью гостевой конфигурации

Последняя функция политики Azure настраивает параметры внутри компьютеров.
Определение Настройка часового *пояса на компьютерах Windows* вносит изменения в компьютер, настраивая часовой пояс.

При назначении определений, начинающихся с *Configure*, необходимо также назначить определение *необходимые условия для включения гостевой политики конфигурации на виртуальных машинах Windows.*
Вы можете комбинировать эти определения в инициативе, если вы решили.

#### <a name="assigning-policies-to-machines-outside-of-azure"></a>Назначение политик компьютерам за пределами Azure

Политики аудита, доступные для гостевой конфигурации, включают тип ресурса **Microsoft. хибридкомпуте/Machines** .  Все компьютеры, подключенные к службе "Дуга Azure", которые находятся в области назначения, будут включены автоматически.
