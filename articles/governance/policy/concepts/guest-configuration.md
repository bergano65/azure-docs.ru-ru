...
Для списков IP-адресов можно загрузить [диапазоны IP-адреса и теги службы Azure](https://www.microsoft.com/download/details.aspx?id=56519). Файл обновляется еженедельно и содержит развернутые в настоящее время диапазоны и все предстоящие изменения диапазонов IP-адресов. Вам нужно только разрешить исходящий доступ к IP-адресам в регионах, где развернуты виртуальные машины.

> [!NOTE]
> В JSON-файле Azure Ranges and Service Tags указаны диапазоны IP-адресов, используемые в Microsoft Azure центрах обработки данных. Этот файл включает диапазоны вычисления, хранения и SQL. Обновленный файл публикуется еженедельно. Он отражает развернутые в настоящее время диапазоны и все предстоящие изменения IP-адресов. Новые диапазоны, появившиеся в файле, не будут использоваться в центрах обработки данных по крайней мере одну неделю. Скачивайте новый XML-файл каждую неделю и вносите соответствующие изменения на своем сайте, чтобы правильно определять службы, выполняемые в Azure. Пользователям Azure ExpressRoute следует обратить внимание, что этот файл используется для того, чтобы обновлять протокол BGP в пространстве Azure в первую неделю каждого месяца.
