...
После того как назначение **DeployIfNotExists** соответствует требованиям, назначение политики **аудита** использует локальные средства проверки, чтобы определить, является ли назначение конфигурации совместимым или несоответствующим.
...
> [!NOTE]
> Политика **DeployIfNotExists** необходима, чтобы политика **аудита** возвращала результаты.
> Без **DeployIfNotExists**политика **аудита** отображает "0 из 0" ресурсов в качестве состояния.
