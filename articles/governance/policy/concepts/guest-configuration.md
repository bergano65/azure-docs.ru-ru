...
Клиент гостевой конфигурации проверяет наличие нового содержимого каждые 5 минут. После получения назначения гостя параметры проверяются с 15-минутным интервалом. Результаты отправляются поставщику ресурсов гостевой конфигурации после завершения аудита. Когда происходит [триггер оценки](../how-to/get-compliance-data.md#evaluation-triggers) политики, состояние компьютера записывается в поставщик ресурсов гостевой конфигурации. Это обновление заставляет политику Azure оценивать свойства диспетчера ресурсов Azure. Оценка политики Azure по требованию получает последнее значение от поставщика ресурсов гостевой конфигурации. Однако это не вызывает нового аудита конфигурации в машине.
...
> [!IMPORTANT]
> Конфигурация гостей может проводить аудит узлов с поддержкой ОС. Если вы хотите провести аудит виртуальных машин, которые используют пользовательское изображение, вам необходимо дублировать определение **DeployIfNotExists** и изменить раздел **If,** чтобы включить свойства изображения.
...
## <a name="built-in-resource-modules"></a>Встроенные ресурсные модули

При установке расширения гостевой конфигурации модуль PowerShell 'GuestConfiguration' включен в последнюю версию модулей ресурсов DSC. Этот модуль можно загрузить из галереи PowerShell, используя ссылку 'Руководство Загрузка' со страницы модуля [GuestConfiguration](https://www.powershellgallery.com/packages/GuestConfiguration/). Формат файла '.nupkg' может быть переименован в '.zip', чтобы распаковать и просмотреть.
