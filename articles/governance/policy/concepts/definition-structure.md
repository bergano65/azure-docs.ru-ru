...
**Режим** — настроить в зависимости от политики для работы с Azure Resource Manager свойство или свойство поставщика ресурсов.
...
### <a name="resource-provider-modes"></a>Режимы поставщика ресурсов

В настоящее время поддерживается только режим поставщика ресурсов `Microsoft.ContainerService.Data` управления правилами допуском контроллера на [службе Azure Kubernetes](../../../aks/intro-kubernetes.md).
...
- `type`. Определяет, является ли параметр **строка**, **массива**, **объект**, **логическое**, **целое число**, **float**, или **datetime**.
...
- **EnforceRegoPolicy**: настраивает контроллер приемной комиссии откройте агент политики в службе Azure Kubernetes (Предварительная версия)
...
«Normal» псевдоним представляет поле как одно значение. Данное поле используется для сценариев сравнения точное совпадение, если весь набор значений должен содержать ровно, как определено, не больше и не меньше.
