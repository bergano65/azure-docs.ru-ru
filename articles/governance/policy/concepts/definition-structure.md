...
ms.locfileid: "82613308"
---
# <a name="azure-policy-definition-structure"></a>Структура определения службы "Политика Azure"

Политика Azure устанавливает соглашения для ресурсов. Определения политик описывают [условия](#conditions) соответствия ресурсов и последствия выполнения условия. Условие сравнивает [поле](#fields) свойства ресурса с требуемым значением. Доступ к полям свойств ресурсов осуществляется с помощью [псевдонимов](#aliases). Поле свойства ресурса является либо однозначным полем, либо [массивом](#understanding-the--alias) из нескольких значений. Вычисления условий различаются для массивов.
...
- `all`: Оценка групп ресурсов, подписок и всех типов ресурсов
- `indexed`: оцениваются только типы ресурсов, которые поддерживают теги и расположение.

Например, ресурс `Microsoft.Network/routeTables` поддерживает теги и расположение и вычисляется в обоих режимах. Однако ресурс `Microsoft.Network/routeTables/routes` не может быть помечен и не вычисляется `Indexed` в режиме.
...
`indexed` следует использовать при создании политик, которые будут принудительно применять теги или расположения. Это не обязательно, но помешает отображению ресурсов, которые не поддерживают теги и расположения, в качестве несоответствующих в результатах проверки соответствия. Исключением являются **группы ресурсов** и **подписки**. Политики, которые принудительно используют расположение или теги в группе ресурсов или подписке, `all` должны устанавливать **режим** и, `Microsoft.Resources/subscriptions` в частности, указывать на `Microsoft.Resources/subscriptions/resourceGroups` тип или. Пример см. в статье о [принудительном применении тегов к группам ресурсов](../samples/enforce-tag-rg.md). Список ресурсов, поддерживающих теги, см. в статье [Поддержка тегов для ресурсов Azure](../../../azure-resource-manager/management/tag-support.md).

### <a name="resource-provider-modes-preview"></a><a name="resource-provider-modes" />Режимы поставщиков ресурсов (Предварительная версия)
...
- `Microsoft.Kubernetes.Data`для управления кластерами AKS Engine Kubernetes в Azure.
...
- `metadata`: Определяет подсвойства, которые в основном используются портал Azure для вывода сведений, понятных пользователю:
  ...
  - `version`: (Необязательно) отслеживает сведения о версии содержимого определения политики.

    > [!NOTE]
    > Служба политики Azure использует `version`свойства, `preview`и `deprecated` , чтобы передать уровень изменения встроенному определению политики или инициативе и состоянию. Формат `version` имеет вид: `{Major}.{Minor}.{Patch}`. Конкретные состояния, такие как _устаревшие_ или _предварительные версии_, добавляются к `version` свойству или в другое свойство в качестве **логического значения**.

  - `category`: (Необязательно) определяет, в какой категории в портал Azure отображается определение политики.
...
В правиле политики вы ссылаетесь на параметры с помощью `parameters` следующего синтаксиса функции:
...
> [!NOTE]
> Во время создания или обновления определения политики **идентификатор**, **тип**и **имя** определяются свойствами, которые являются внешними по отношению к JSON и не являются обязательными в JSON-файле. Выборка определения политики с помощью пакета SDK Возвращает свойства **ID**, **Type**и **Name** как часть JSON, но все они являются сведениями только для чтения, относящимися к определению политики.
...
Для **less**, **лессорекуалс**, **больше**и **греатерорекуалс**, если тип свойства не совпадает с типом условия, выдается ошибка. Сравнения строк выполняются с помощью `InvariantCultureIgnoreCase`.
...
При использовании условий **Match** и **notMatch** укажите `#` для соответствия цифре `?` , буквы, `.` соответствия любому символу и любой другой символ в соответствии с фактическим символом. Несмотря на то, что **Match** и **notMatch** чувствительны к регистру, все остальные условия, в которых вычисляется _StringValue_ , не учитывают регистр. Другие варианты без учета регистра доступны для условий **matchInsensitively** и **notMatchInsensitively**.

В значении **and** поля массива ** \] псевдонимов каждый элемент массива вычисляется отдельно с помощью логического элемента и между \[ \* ** элементами. Дополнительные сведения см. [в \[ \* \] разделе Вычисление псевдонима](../how-to/author-policies-for-arrays.md#evaluating-the--alias).
...
В указанном ниже примере `concat` используется для создания поля подстановки тега для тега со значением параметра **tagName**. Если этот тег не существует **, то с** помощью функции `resourcegroup()` Lookup для добавления тега используется значение того же именованного тега, что и в родительской группе ресурсов с аудитом.
...
Использование _функций шаблона_ в **значении** позволяет использовать множество сложных вложенных функций. Если результатом _функции шаблона_ является ошибка, оценка политики завершится сбоем. Завершившаяся сбоем оценка — это неявный **запрет**. Пример **значения** , которое не удается выполнить в определенных сценариях:
...
В примере правила политики выше используется [подстрока ()](../../../azure-resource-manager/templates/template-functions-string.md#substring) для сравнения первых трех символов **имени** с **ABC**. Если **имя** короче трех символов, `substring()` функция приводит к ошибке. Эта ошибка приводит к тому, что политика станет **отклонением** .

Вместо этого используйте функцию [if ()](../../../azure-resource-manager/templates/template-functions-logical.md#if) , чтобы проверить, что первые три символа **имени** , равные **ABC** , не допускают возникновения ошибки в **имени** , меньшем трех символов.
...
При измененном правиле политики `if()` проверяет длину **имени** перед попыткой получить `substring()` значение с менее чем тремя символами. Если **имя** слишком короткое, возвращается значение "не начинается с ABC", а затем сравнивается с **ABC**. Ресурс с коротким именем, который не начинается с **ABC** , по-прежнему не проходит правило политики, но больше не вызывает ошибку во время вычисления.

### <a name="count"></a>Счетчик

Условия, которые подсчитывает, сколько элементов массива в полезных данных ресурса соответствуют условному выражению, могут быть сформированы с помощью выражения **Count** . Распространенными сценариями является проверка того, что по крайней мере один из "," только один из "," все "или" ни одного из "элементов массива не удовлетворяет условию. Функция **Count** вычисляет каждый [ \[ \* \] элемент массива псевдонимов](#understanding-the--alias) для выражения условия и суммирует _истинные_ результаты, которые затем сравниваются с оператором выражения. Выражения **Count** могут быть добавлены до 3 раз в одно определение **класс policyrule** .
...
- **Count. Where** (необязательно): выражение условия для индивидуальной проверки [ \[ \* \] ](#understanding-the--alias) каждого члена массива псевдонимов **Count. Field**. Если это свойство не указано, все элементы массива с путем "Field" оцениваются в _значение true_. В этом свойстве можно использовать любое [условие](../concepts/definition-structure.md#conditions) .
...
Все [функции шаблонов диспетчер ресурсов](../../../azure-resource-manager/templates/template-functions.md) доступны для использования в правиле политики, за исключением следующих функций и определяемых пользователем функций:
...
Следующая функция доступна для использования в правиле политики, но отличается от использования в шаблоне Azure Resource Manager:

- `utcNow()`-В отличие от шаблона диспетчер ресурсов, его можно использовать вне defaultValue.
...
- `field(fieldName)`
  - **fieldname**: [обязательный параметр] строка — имя извлекаемого [поля](#fields)
...
- `requestContext().apiVersion`
  - Возвращает версию API запроса, который активировал вычисление политики (пример: `2019-09-01`).
    Это будет версия API, которая использовалась в запросе на размещение или исправление для оценки при создании или обновлении ресурсов. Последняя версия API всегда используется во время оценки соответствия для существующих ресурсов.
...
- Расширение политики Azure для Visual Studio Code (рекомендуется)
  ...
  :::image type="content" source="../media/extension-for-vscode/extension-hover-shows-property-alias.png" alt-text="Расширение политики Azure для Visual Studio Code" border="false":::

- Azure Resource Graph

  Используйте `project` оператор для вывода **псевдонима** ресурса.
...
Несколько доступных псевдонимов имеют версию, которая отображается в виде "нормального" имени, а другая ** \[ \* ** — к ней. Пример:
...
** \[ Псевдоним позволяет сравнить со значением каждого элемента массива и конкретными свойствами \* ** каждого элемента. Такой подход позволяет сравнивать свойства элементов для сценариев "If None of", "If any of", или "Если все из". Для более сложных сценариев используйте выражение условия [Count](#count) . С **помощью\[\*ипрулес**в качестве примера можно проверить, что _action_ каждое действие _отклоняется_, но не беспокоиться о том, сколько правил существует или какое _значение_ является IP-адресом.
Этот пример правила проверяет наличие совпадений **ипрулес\[\*\]. Value** с **10.0.4.1** и применяет **еффекттипе** только в том случае, если не найдено хотя бы одно совпадение:
...
> [!NOTE]
> После назначения инициативы невозможно изменить параметры уровня инициативы. Из-за этого рекомендуется задать значение **DefaultValue** при определении параметра.
