...
### <a name="resource-provider-modes-preview"></a><a name="resource-provider-modes" />Режимы поставщика ресурсов (предварительная версия)

Сейчас в предварительной версии поддерживаются следующие режимы поставщика ресурсов:

- `Microsoft.ContainerService.Data` для управления правилами контроллера допуска в [Службе Azure Kubernetes](../../../aks/intro-kubernetes.md). Политики, использующие этот режим поставщика ресурсов, **должны** использовать действие [EnforceRegoPolicy](./effects.md#enforceregopolicy). Этот устаревший режим _не рекомендуется использовать_.
- `Microsoft.Kubernetes.Data` для управления кластерами Kubernetes в Azure или вне Azure. Политики, использующие этот режим поставщика ресурсов, **должны** использовать действие [EnforceOPAConstraint](./effects.md#enforceopaconstraint).
- `Microsoft.KeyVault.Data` для управления хранилищами и сертификатами в [Azure Key Vault](../../../key-vault/general/overview.md).

> [!NOTE]
> Режимы поставщика ресурсов поддерживают только встроенные определения политик и не поддерживают инициативы в предварительной версии.
...
- `metadata`: Определяет вложенное свойство, которое в основном используется для отображения понятной пользователям информации на портале Azure:
  ...
  - `category`: (Необязательно.) Определяет, в какой категории портал Azure отображает определение политики.
...
Некоторые поддерживаемые _типы ресурсов_ **Get-AzResourceProvider** не возвращает. Это следующие.
...
При использовании условий **match** и **notMatch** укажите `#` для сопоставления цифры, `?` для буквы, `.` для сопоставления любого знака и любой другой знак для сопоставления соответствующего фактического знака. **match** и **notMatch** учитывает регистр, а все остальные условия, в которых оценивается _stringValue_, не учитывают регистр. Другие варианты без учета регистра доступны для условий **matchInsensitively** и **notMatchInsensitively**.
...
- **count.where** (необязательное): выражение условия для индивидуального вычисления каждого [\[\*\] псевдонима](#understanding-the--alias) элемента массива **count.field**. Если это свойство не указано, то все элементы массива с путем field оцениваются как _true_. В этом свойстве можно использовать любое [condition](../concepts/definition-structure.md#conditions) (условие).
  Внутри этого свойства можно использовать [логические операторы](#logical-operators) для создания сложных требований к оценке.
- **\<condition\>** (обязательное): значение сравнивается с числом элементов, которые соответствуют выражению условия **count.where**. Следует использовать числовое [condition](../concepts/definition-structure.md#conditions).
...
Пример 5. Убедиться, что все члены массива строк соответствуют выражению условия.
...
Пример 6. Использовать **field** в **value**, чтобы убедиться, что все члены массива объектов соответствуют выражению условия.
...
Пример 7. Убедиться, что хотя бы один элемент массива соответствует нескольким свойствам в выражении условия.
...
- **ЕEnforceOPAConstraint** (предварительная версия): настраивает контроллер допуска агента открытой политики с Gatekeeper v3 для самостоятельно управляемых кластеров Kubernetes в Azure (предварительная версия);
- **EnforceRegoPolicyи** (предварительная версия): настраивает контроллер допуска агента открытой политики с Gatekeeper v3 в Службе Azure Kubernetes;
...
Следующая функция доступна для использования в правиле политики, но такое использование отличается от использования в шаблоне Azure Resource Manager.

- `utcNow()` — в отличие от шаблона Resource Manager, ее можно использовать вне defaultValue.
  - Возвращает строку с текущей датой и временем в универсальном формате даты и времени ISO 8601: «yyyy-MM-ddTHH:mm:ss.fffffffZ».

Следующие функции доступны только в правилах политик.

- `addDays(dateTime, numberOfDaysToAdd)`
  - **dateTime**: [обязательная] строка — строка в универсальном формате даты и времени ISO 8601: "yyyy-MM-ddTHH:mm:ss.fffffffZ".
...
- `requestContext().apiVersion`
  - Возвращает версию API запроса, который активировал оценку политики (пример: `2019-09-01`).
    Это будет версия API, которая использовалась в запросе PUT/PATCH для оценки при создании или обновлении ресурсов. Последняя версия API всегда используется во время оценки соответствия для существующих ресурсов.
