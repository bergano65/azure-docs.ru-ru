...
Например, `Microsoft.Network/routeTables` ресурсов поддерживает теги и расположение и вычисляется в обоих режимах. Однако `Microsoft.Network/routeTables/routes` ресурсов не могут быть помечены в режиме `Indexed`.
...
При использовании условий **Match** и **notMatch** укажите `#` для сопоставления цифры, `?` для буквы `.`, чтобы сопоставить любой символ, и любой другой символ в соответствии с фактическим символом. Несмотря на то, что **Match** и **notMatch** чувствительны к регистру, все остальные условия, в которых вычисляется _StringValue_ , не учитывают регистр. Другие варианты без учета регистра доступны для условий **matchInsensitively** и **notMatchInsensitively**.

В **\[\*\] поле массива псевдонимов** каждый элемент массива вычисляется по отдельности с помощью логического элемента **и** между элементами. Дополнительные сведения см. [в разделе оценка \] псевдонима \[\*](../how-to/author-policies-for-arrays.md#evaluating-the--alias).
...
В этом примере правила политики используют **value** для сравнения результата функции `resourceGroup()` и возвращенного свойства **name** с условием **like** для `*netrg`. Правило запрещает любой ресурс, не являющийся **типом** `Microsoft.Network/*`, в любой группе ресурсов, имя которой заканчивается на `*netrg`.
...
В этом примере правила политики используется **значение** для проверки того, что результат нескольких вложенных функций **равен** `true`. Это правило запрещает любой ресурс, в котором нет хотя бы трех тегов.
...
Условия, которые подсчитывает, сколько элементов массива в полезных данных ресурса соответствуют условному выражению, могут быть сформированы с помощью выражения **Count** . Распространенными сценариями является проверка того, что по крайней мере один из "," только один из "," все "или" ни одного из "элементов массива не удовлетворяет условию. Функция **Count** вычисляет каждый [\[\*\] член массива псевдонимов](#understanding-the--alias) для выражения условия и суммирует _истинные_ результаты, которые затем сравниваются с оператором выражения.
...
- `requestContext().apiVersion`
  - Возвращает версию API запроса, который активировал вычисление политики (пример: `2019-09-01`). Это будет версия API, которая использовалась в запросе на размещение или исправление для оценки при создании или обновлении ресурсов. Последняя версия API всегда используется во время оценки соответствия для существующих ресурсов.
...
> [!NOTE]
> После назначения инициативы невозможно изменить параметры уровня инитативе. Из-за этого рекомендуется задать значение **DefaultValue** при определении параметра.
