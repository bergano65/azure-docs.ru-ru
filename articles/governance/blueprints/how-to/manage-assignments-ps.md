...
1. Запустите следующие команды из командной строки PowerShell **с правами администратора**.
   ...
   > [!NOTE]
   > Если **AZ. Accounts** уже установлено, может потребоваться использовать `-AllowClobber` для принудительной установки.

1. Убедитесь, что модуль импортирован и является правильной версией (0.2.5):
...
Командлет `Get-AzBlueprint` получает одно или несколько определений схем. Командлет может получать определения схем из группы управления с `-ManagementGroupId {mgId}` или подпиской с `-SubscriptionId {subId}`. Параметр **Name** получает определение схемы, но должен использоваться с **ManagementGroupId** или **SubscriptionId**. **Версию** можно использовать с **именем** , чтобы более явно определить, какое определение схемы возвращается. Вместо **версии**параметр `-LatestPublished` извлекает последнюю опубликованную версию.

В следующем примере `Get-AzBlueprint` используется для получения всех версий определения схемы с именем "101-чертежи-Definition-Subscription" из конкретной подписки, представленной в виде `{subId}`:
...
### <a name="example-1-provide-parameters"></a>Пример 1 Предоставление параметров

В следующем примере создается новое назначение версии "1,1" определения схемы "My-чертеж", полученного с `Get-AzBlueprint`, устанавливает расположение управляемого удостоверения и объекта назначения в "westus2", блокирует ресурсы с помощью _аллресаурцесреадонли_. и задает хэш-таблицы для **параметра** и **ресаурцеграуппараметер** в конкретной подписке, представленной в виде `{subId}`:
...
При удалении назначения схемы это действие обрабатывается командлетом `Remove-AzBlueprintAssignment`. Командлет принимает либо **имя** , либо **InputObject** , чтобы указать назначение схемы для удаления. Идентификатор **SubscriptionId** является _обязательным_ и должен быть указан во всех случаях.

Следующий пример извлекает существующее назначение схемы с `Get-AzBlueprintAssignment`, а затем удаляет его из конкретной подписки, представленной в виде `{subId}`:
