...
Связанный Key Vault должен присутствовать в той же подписке, в которой находится схема.
Key Vault также необходимо **предоставить доступ к Azure Resource Manager для развертывания шаблона** на странице **Политики доступа** Key Vault. Инструкции о том, как включить эту функцию, см. в разделе [Включение развертывания шаблона](../../../managed-applications/key-vault-access.md#enable-template-deployment). Дополнительные сведения об Azure Key Vault см. в [этой статье](../../../key-vault/key-vault-overview.md).
...
1. Щелкните правой кнопкой мыши схему, которую нужно назначить. Выберите **Назначить схему** или щелкните нужную схему, а затем нажмите кнопку **Назначить схему**.
