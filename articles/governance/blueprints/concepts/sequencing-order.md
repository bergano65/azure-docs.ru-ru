...
При составлении определений больших чертежей может потребоваться создание ресурсов в определенном порядке. Наиболее распространенный шаблон использования этого сценария — когда определение чертежа включает несколько шаблонов azure Resource Manager. Для поддержки этого в службе Blueprints предусмотрена возможность определить порядок последовательности.
