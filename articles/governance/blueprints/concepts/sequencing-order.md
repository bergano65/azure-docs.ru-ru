...
При составлении крупных определений схем может потребоваться создать ресурсы в определенном порядке. Наиболее распространенный шаблон использования этого сценария состоит в том, что определение схемы включает несколько шаблонов Azure Resource Manager. Схемы Azure обрабатывают этот шаблон, позволяя определять порядок последовательности.
