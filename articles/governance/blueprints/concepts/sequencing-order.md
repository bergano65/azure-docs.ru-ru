...
Для определения порядка в коде JSON определяется свойство `dependsOn`. Определение схемы, для групп ресурсов и объекты артефакт поддерживает это свойство. `dependsOn` является массив строк имен артефакта, определенного артефакта должна быть создана до его создания.
