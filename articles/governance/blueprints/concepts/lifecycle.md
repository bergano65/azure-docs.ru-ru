...
description: Узнайте о жизненном цикле, через который проходит определение чертежа, и подробностях о каждом этапе, включая обновление и удаление назначений чертежей.
ms.date: 07/30/2019
ms.topic: conceptual
...
Как и многие ресурсы в Azure, схемы в Azure Blueprints обладают обычным жизненным циклом. Они создаются, развертываются и, наконец, удаляются, когда они больше не требуются или нерелевантны. Azure Blueprints поддерживает стандартные операции жизненного цикла. На их основе предоставляются дополнительные уровни состояния, которые поддерживают традиционные конвейеры непрерывной интеграции и непрерывного развертывания для организаций, которые управляют основным элементом DevOps "Инфраструктура как код".
...
Схема в режиме **черновика**, которая никогда не публиковалась, будет отображаться на странице **Определения схем** со значком, отличным от значка для ранее **опубликованной** схемы. **Последняя версия** отображается в виде **проекта** для этих никогда не публиковавшихся чертежей.
...
1. Выберите определения Blueprint со **страницы** слева и используйте параметры фильтра, чтобы найти чертеж, который вы хотите удалить версию. Выберите его, чтобы открыть страницу отсеивания.

1. Выберите вкладку **«Опубликованные версии»** и найдите версию, которая вы хотите удалить.

1. Нажмите на версию, чтобы удалить и выбрать **Удалить эту версию.**
...
### <a name="unassigning-assignments"></a>Неназначение назначений

Если план больше не нужен, он может быть неназначен из группы управления или подписки. Во время неназначения чертежа происходит следующее:

- Удаление [блокировки ресурсов чертежа](resource-locking.md)
- Удаление объекта назначения чертежа
- (Условно) Если использовалась **управляемая идентификация, назначенная системой,** она также удаляется

> [!NOTE]
> Все ресурсы, развернутые в назначении чертежей, остаются в силе, но больше не защищены чертежами Azure.
