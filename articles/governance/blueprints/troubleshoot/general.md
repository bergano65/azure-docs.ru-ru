...
Чтобы передать функцию в качестве параметра, направьте всю строку с `[` так, чтобы параметр схемы был похож на `[[resourceGroup().tags.myTag]`. Escape-символ заставляет схемы обрабатывать значение как строку при обработке. Затем служба "схемы" помещает функцию в артефакт, что делает ее динамически требуемой. Дополнительные сведения см. [в разделе синтаксис и выражения в шаблонах Azure Resource Manager](../../../azure-resource-manager/templates/template-expressions.md).
...
Назначения схемы в состоянии, отличном от терминала, автоматически помечаются как **неудачные** после _6-часового_ времени ожидания. После того, как время ожидания изменило состояние назначения схемы, удаление может быть повторено.
