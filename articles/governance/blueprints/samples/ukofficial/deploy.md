---
title: Пример — ОФИЦИАЛЬНЫЙ часть соединенного Королевства и Великобритании Отделения схемы, в которой — развертывание действия
description: Разверните шаги ОФИЦИАЛЬНЫЙ часть соединенного Королевства и примеров в рамках схемы Отделения часть соединенного Королевства.
services: blueprints
author: DCtheGeek
ms.author: dacoulte
ms.date: 06/26/2019
...
ms.locfileid: "67453207"
---
# <a name="deploy-the-uk-official-and-uk-nhs-blueprint-samples"></a>Развертывание ОФИЦИАЛЬНЫЙ часть соединенного Королевства и примеров в рамках схемы Отделения часть соединенного Королевства

Чтобы развернуть ОФИЦИАЛЬНЫЙ часть соединенного Королевства и примеров в рамках схемы Отделения часть соединенного Королевства, необходимо выполнить следующие действия:
...
1. Найти **Великобритании ОФИЦИАЛЬНЫЙ** или **Отделения Великобритании** примера схемы в _другие примеры_ и выберите **использовать этот пример**.
...
Теперь в вашей среде создана копия образца схемы. Она создана в режиме **Черновик**, и прежде чем назначить и развернуть эту копию, ее необходимо **опубликовать**. Копия примера в рамках схемы можно настроить для вашей среды и требований, но это изменение может отойти от его стандартный.

1. Выберите **Все службы**, а затем выполните поиск по запросу **Политика** и выберите этот пункт на панели слева. На странице **Политика** выберите **Схемы**.

1. В меню слева выберите страницу **Определения схем**. Примените фильтры, чтобы найти копию примера схемы, и выберите его.

1. В верхней части страницы выберите **Опубликовать схему**. В правой части новой страницы укажите **версию** для копии примера схемы. Это свойство позволяет вносить изменения позднее. Укажите **изменить заметки** такие, как «первая версия публикации из образца схемы КОРОЛЕВСТВО или Отделения Великобритания.» Затем щелкните **Опубликовать** в нижней части страницы.
...
|-|-|-|-|
|Схема инициативы для Великобритании ОФИЦИАЛЬНЫЙ или Отделения часть соединенного Королевства|Назначение политики |Типы ресурсов в диагностические журналы аудита (политика: Схема инициативы для Великобритании ОФИЦИАЛЬНЫЙ или Отделения Великобритании) |Список типов ресурсов для аудита, если параметр журнала диагностики включено Примечание.  Допустимые значения см. в разделе [поддерживаемые службы, схемы и категории для журналов диагностики Azure](../../../../azure-monitor/platform/diagnostic-logs-schema.md). |
|[Предварительная версия.] Развертывание агента Log Analytics для виртуальных машин Linux |Назначение политики |Необязательно: Список образов виртуальных Машин, на которые установлена поддерживаемая ОС Linux, чтобы добавить область действия (политика: [Предварительная версия]: Развернуть агент Log Analytics для виртуальных машин Linux) |(Необязательно) Значение по умолчанию — _none_. Дополнительные сведения см. в разделе [создать рабочую область Log Analytics на портале Azure](../../../../azure-monitor/learn/quick-create-workspace.md). |
|[Предварительная версия.] Развертывание агента Log Analytics для виртуальных машин Windows |Назначение политики |Необязательно: Список образов виртуальных Машин, на которые установлена поддерживаемая ОС Windows, чтобы добавить область действия (политика: [Предварительная версия]: Развернуть агент Log Analytics для виртуальных машин Windows) |(Необязательно) Значение по умолчанию — _none_. Дополнительные сведения см. в разделе [создать рабочую область Log Analytics на портале Azure](../../../../azure-monitor/learn/quick-create-workspace.md). |

## <a name="next-steps"></a>Дальнейшие действия

Теперь, когда вы ознакомились с шаги по развертыванию ОФИЦИАЛЬНОЙ часть соединенного Королевства и примеров в рамках схемы Отделения часть соединенного Королевства, см. следующие статьи, чтобы узнать о Обзор и сопоставление элемента управления.

> [!div class="nextstepaction"]
> [ОФИЦИАЛЬНЫЙ часть соединенного Королевства и Великобритании Отделения чертежей - Обзор](./index.md)
> [ОФИЦИАЛЬНЫЙ часть соединенного Королевства и проекты Отделения Великобритании - контролировать сопоставление](./control-mapping.md)
