---
title: Пример — схема FedRAMP высокого уровня — сопоставление элементов управления
description: Сопоставление элементов управления примера схемы FedRAMP высокого уровня. Каждый элемент управления сопоставляется с одной или несколькими политиками Azure, которые помогают выполнять оценку.
ms.date: 01/31/2020
ms.topic: sample
...
ms.locfileid: "76941352"
---
# <a name="control-mapping-of-the-fedramp-high-blueprint-sample"></a>Сопоставление элементов управления примера схемы FedRAMP высокого уровня.

В статье подробно описано, как пример схемы Azure Blueprints "FedRAMP высокого уровня" сопоставляется с элементами управления FedRAMP высокого уровня. Дополнительные сведения об элементах управления см. в разделе [FedRAMP Security Controls Baseline](https://www.fedramp.gov/) (базовые показатели элементов управления безопасности FedRAMP).

Далее представлены сопоставления для элементов управления **FedRAMP высокого уровня**. Используйте панель навигации справа для перехода непосредственно к сопоставлению конкретных элементов управления. Многие сопоставленные элементы управления реализуются с помощью инициативы [Политика Azure](../../../policy/overview.md). Чтобы просмотреть полную инициативу, откройте раздел **Политика** на портале Azure и перейдите на страницу **Определения**. Найдите и выберите функцию **\[Предварительная версия\]: Аудит FedRAMP высокого уровня контролирует и развертывает определенные виртуальные расширения для поддержки требований аудита** встроенной политики.
...
> Историю изменений можно просмотреть на странице [журнала фиксаций в GitHub](https://github.com/MicrosoftDocs/azure-docs/commits/master/articles/governance/blueprints/samples/fedramp-h/control-mapping.md).
...
В этой схеме содержатся определения политик, которые регистрируют записи с анализом оценки уязвимостей на виртуальных машинах, масштабируемых наборах виртуальных машин, управляемых экземплярах SQL и серверах SQL Server.
Эти определения политики также выполняют аудит журналов диагностики, что позволяет подробно анализировать операции, выполненные с ресурсами Azure. Эти полезные сведения предоставляют информацию в реальном времени о состоянии безопасности развернутых ресурсов и помогают вам приоритезировать действия по исправлению.
Для получения подробных сведений о проверке уязвимостей и мониторинге рекомендуется использовать Azure Sentinel и Центр безопасности Azure.

- \[Предварительная версия.\] На виртуальных машинах должна быть включена Оценка уязвимостей
- \[Предварительная версия.\] Включение Azure Monitor для виртуальных машин
- \[Предварительная версия.\] Включение Azure Monitor для масштабируемых наборов виртуальных машин (VMSS)
...
Теперь, когда вы ознакомились с сопоставлением элементов управления схемы "FedRAMP высокого уровня", узнайте о схеме и развертывании этого образца из следующих статей:

> [!div class="nextstepaction"]
> [Общие сведения о примере схемы FedRAMP высокого уровня](./index.md)
> [Deploy the FedRAMP High blueprint sample](./deploy.md) (Развертывание примера схемы FedRAMP высокого уровня)
