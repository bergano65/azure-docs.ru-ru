...
Чтобы продемонстрировать работу заголовков, рассмотрим ответ на запрос с заголовком и значениями параметров `x-ms-user-quota-remaining: 10` и. `x-ms-user-quota-resets-after: 00:00:03`

- В течение следующих 3 секунд можно отправить не более 10 запросов без регулирования.
- В течение 3 секунд `x-ms-user-quota-remaining` значения и `x-ms-user-quota-resets-after` будут сброшены в `15` и `00:00:05` соответственно.
...
Если вы регулируете указанные выше рекомендации, вы можете обратиться к группе по адресу [resourcegraphsupport@microsoft.com](mailto:resourcegraphsupport@microsoft.com).
