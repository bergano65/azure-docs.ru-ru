...
description: Научитесь использовать язык запроса resource Graph, чтобы исследовать ресурсы и узнать, как они связаны.
ms.date: 10/18/2019
ms.topic: conceptual
...
Свойства говорят нам дополнительную информацию о виртуальном ресурсе машины себя, все, от SKU, Os, диски, теги, и группа ресурсов и подписки это член.
...
> [!NOTE]
> Еще один способ получить номер SKU — воспользоваться свойством **Microsoft.Compute/virtualMachines/sku.name** из раздела **aliases**. Смотрите [псевдонимы Show](../samples/starter.md#show-aliases) и [отображайте различные примеры значений псевдонимов.](../samples/starter.md#distinct-alias-values)
...
Этот набор запросов сначала находит и хранит все сетевые интерфейсы (NIC) ресурсы, подключенные к виртуальным машинам. Затем запросы используют список NICs, чтобы найти каждый ресурс IP-адреса, который является общедоступным IP-адресом, и сохранить эти значения. Наконец, в запросах указан список общедоступных IP-адресов.
...
Используйте файл (Azure CLI) или переменную (Azure PowerShell) в следующем запросе, чтобы получить сведения о связанных с сетими ресурсах, где есть общедоступный IP-адрес, прикрепленный к NIC.
...
Наконец, используйте список общедоступных ресурсов IP-адреса, хранящихся в файле (Azure CLI) или переменной (Azure PowerShell), чтобы получить фактический общедоступный IP-адрес от связанного объекта и отображения.
...
Чтобы узнать, как выполнить эти шаги `join` в одном запросе с оператором, ознакомьтесь [с виртуальными машинами List с их сетевым интерфейсом и общедоступным](../samples/advanced.md#join-vmpip) выборкой IP.

## <a name="next-steps"></a>Дальнейшие действия

- Подробнее о [языке запроса.](query-language.md)
