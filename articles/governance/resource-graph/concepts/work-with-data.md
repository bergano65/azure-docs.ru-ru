...
> [!NOTE]
> При использовании функции **Skip**рекомендуется упорядочить результаты по крайней мере по одному столбцу с `asc` или `desc`. Без сортировки возвращаемые результаты являются случайными и не повторяются.
