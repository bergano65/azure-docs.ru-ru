...
Если необходимо разбить результирующий набор на меньшие наборы записей для обработки или если он превышает максимально допустимое значение возвращаемых записей (_1000_), используйте разбиение на страницы. [REST API](/rest/api/azureresourcegraph/resourcegraph(2018-09-01-preview)/resources/resources) **QueryResponse** предоставляет значения, указывающие, был ли разбит набор результатов: **resultTruncated** и **$skipToken**.
 ...
 **resultTruncated** имеет значение **true**, если разбиение по страницам отключено или невозможно из-за отсутствия столбца `id` или недостаточного количества ресурсов для запроса. Когда свойство **resultTruncated** равно **true**, свойство **$skipToken** не задается.
