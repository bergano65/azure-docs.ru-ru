...
> [!NOTE]
> При **первом**использовании рекомендуется упорядочить результаты по крайней мере по одному столбцу с помощью `asc` или. `desc` Без сортировки возвращаемые результаты являются случайными и не повторяются.
...
> [!NOTE]
> При использовании функции **Skip**рекомендуется упорядочить результаты по крайней мере по одному столбцу с помощью `asc` или `desc`. Без сортировки возвращаемые результаты являются случайными и не повторяются.
 ...
 **ресулттрункатед** имеет **значение true** , если подкачка отключена или невозможна из `id` -за отсутствия столбца или если доступно меньше ресурсов, чем запрос запроса. Если **ресулттрункатед** имеет **значение true**, свойство **$skipToken** не задано.
...
Результаты из Azure CLI предоставляются по умолчанию в JSON. Результаты в Azure PowerShell являются **PSCustomObject** по умолчанию, но их можно быстро преобразовать в формат JSON с помощью `ConvertTo-Json` командлета. Для других пакетов SDK результаты запроса можно настроить для вывода формата _обжектаррай_ .
