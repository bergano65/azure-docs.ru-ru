...
- **resourceId** \[требуется\]resourceId: ресурс Azure для поиска изменений.
- **interval** \[требуемый\]интервал: свойство с _датами начала_ и _окончания_ для проверки события изменения с помощью часового **пояса Zulu (Z)**.
...
- **ChangeType** — описывает тип изменений, обнаруженных для всей записи об изменениях между **бефореснапшот** и **афтерснапшот**. Значения: _Создание_, _Обновление_и _Удаление_. Массив свойств **пропертичанжес** включается только при обновлении **ChangeType** . _Update_
- **пропертичанжес** — этот массив свойств содержит сведения обо всех свойствах ресурсов, которые были обновлены между **бефореснапшот** и **афтерснапшот**.
  ...
  - **афтервалуе** — значение свойства ресурса в **афтерснапшот**. Не отображается, **changeType** когда ChangeType _удаляется._
