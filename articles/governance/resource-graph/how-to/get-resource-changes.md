...
- **resourceId** \[required @ no__t-2: Ресурс Azure для поиска изменений.
- **interval** \[required @ no__t-2: Свойство с датами _начала_ и _окончания_ для проверки события изменения с помощью часового **пояса Zulu (Z)** .
- **фетчпропертичанжес** (необязательно): Логическое свойство, которое задает, содержит ли объект ответа изменения свойств.
...
- **пропертичанжес** — этот массив свойств содержит сведения обо всех свойствах ресурсов, которые были обновлены между **бефореснапшот** и **афтерснапшот**.
  ...
  - **чанжекатегори** — описывает, что делало изменение. Значения: _Система_ и _пользователь_.
  - **ChangeType** — описывает тип изменений, обнаруженных для отдельного свойства ресурса.
    Значения: _Вставка_, _Обновление_, _Удаление_.
