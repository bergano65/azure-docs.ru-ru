...
> [!NOTE]
> Будучи функцией _предварительной версии_, некоторые объекты `type` имеют свойства, которые отсутствуют в Resource Manager. Дополнительные сведения о расширенных свойствах см. в [этом разделе](./concepts/query-language.md#extended-properties).
...
Чтобы использовать Resource Graph, необходимо иметь соответствующие права для [управления доступом на основе ролей в Azure](../../role-based-access-control/overview.md) (Azure RBAC) как минимум с доступом на чтение ресурсов, которые вы планируете запрашивать. Без минимального разрешения `read` для объекта или группы объектов Azure результаты не будут возвращаться.
