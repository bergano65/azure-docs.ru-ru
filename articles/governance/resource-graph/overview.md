...
Azure Resource Graph дает вам доступ к свойствам, возвращаемым поставщиками ресурсов, без необходимости вызывать каждый поставщик ресурсов отдельно. Чтобы получить список поддерживаемых типов ресурсов, выполните поиск по значению **Да** в таблице [Ресурсы для развертываний в полном режиме](../../azure-resource-manager/templates/complete-mode-deletion.md). Дополнительные типы ресурсов находятся в связанных [таблицах Resource Graph](./concepts/query-language.md#resource-graph-tables). Альтернативный способ просмотра поддерживаемых типов ресурсов — с помощью [обозревателя схемы обозревателя Azure Resource Graph](./first-query-portal.md#schema-browser).
