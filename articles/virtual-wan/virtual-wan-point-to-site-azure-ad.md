---
title: 'Нанастройка аутентификации Azure AD для VPN-соединения пользователя: Virtual WAN'
description: Узнайте, как настроить активную проверку подлинности каталога Azure для VPN пользователя.
...
---
# <a name="configure-azure-active-directory-authentication-for-user-vpn"></a>Нанастройка azure Active Directory authentication для пользовательского VPN

В этой статье показано, как настроить аутентификацию Azure AD для VPN пользователя в виртуальном WAN для подключения к вашим ресурсам в Azure через VPN-соединение OpenVPN. Проверка подлинности Azure Active Directory доступна только для шлюзов, использующих протокол OpenVPN и клиенты под управлением Windows.

Этот тип подключения требует, чтобы клиент был настроен на клиентском компьютере. Для получения дополнительной информации о виртуальном WAN, [см.](virtual-wan-about.md)
...
* Если у вас нет подписки На Azure, создайте [бесплатную учетную запись](https://azure.microsoft.com/free/?WT.mc_id=A261C142F).
...
3. На странице **Создание глобальной сети** на вкладке **Основные сведения** заполните следующие поля:
   ...
   * **Группа ресурсов** - Создание новых или использование существующих.
   ...
   * **Тип:** Стандартный. При создании глобальной сети (цен. категория "Базовый") можно создать только концентратор той же ценовой категории. Концентраторы (цен. категория "Базовый") поддерживают только VPN-подключения типа "сеть — сеть".
