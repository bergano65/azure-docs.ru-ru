...
- Исключить зависимость между существующими локальными центрами обработки данных, которые служат конечными точками VPN-канала, и в то же время сохранить перечисленные ниже пути подключения.
...
- Общие службы (любая общая функция, необходимая для нескольких периферийных серверов), которые использует Contoso. Например, контроллеры домена Windows Server на виртуальных машинах в рамках инфраструктуры как услуги (IaaS).
...
Первый шаг подразумевает развертывание концентратора виртуальной глобальной сети в каждом регионе. Настройте концентратор виртуальной глобальной сети с помощью VPN-шлюза и шлюза ExpressRoute, как описано в следующих статьях.  
...
**Рис. 5. Перенос виртуального ЦОД в звездообразной топологии в виртуальную локальную сеть, шаг 2**
...
Теперь подключите концентратор виртуальной глобальной сети к существующим каналам ExpressRoute и настройте VPN-подключения типа "сеть — сеть" через Интернет к любым удаленным филиалам.
...
**Рис. 6. Перенос виртуального ЦОД в звездообразной топологии в виртуальную локальную сеть, шаг 3**
...
**Рис. 7. Перенос виртуального ЦОД в звездообразной топологии в виртуальную глобальную сеть, шаг 4**
...
**Рис. 8. Перенос виртуального ЦОД в звездообразной топологии в виртуальную глобальную сеть, шаг 5**
...
**б**. Подключите сегменты виртуальных сетей к концентратору виртуальной глобальной сети через подключения к виртуальной сети.
...
### <a name="old-hub-becomes-shared-services-spoke"></a>Старый концентратор становится сегментом для общих служб
...
**Рис. 10. Перенос виртуального ЦОД в звездообразной топологии в виртуальную глобальную сеть, шаг 7**
...
Путь 1 описывает поток трафика из филиала в Азии, подключенного через VPN типа "сеть — сеть", к виртуальной сети Azure в регионе "Юго-Восточная Азия".
...
Путь 2 описывает поток трафика из главного европейского офиса, подключенного через ExpressRoute, к виртуальной сети Azure в регионе "Юго-Восточная Азия".

Трафик направляется следующим образом:
- Главный европейский офис подключен через канал ExpressRoute категории "Премиум" к виртуальному концентратору глобальной сети в регионе "Западная Европа".
...
Путь 3 описывает поток трафика из локального ЦОД в Азии, подключенного к частной глобальной сети, к европейскому филиалу, подключенному через канал типа "сеть — сеть".

Трафик направляется следующим образом:
- ЦОД в Азии подключен к локальной частной службе глобальной сети.
- Канал ExpressRoute локально завершается в частной глобальной сети, которая подключается к виртуальному концентратору глобальной сети в регионе "Юго-Восточная Азия".
- Глобальное подключение типа "концентратор — концентратор" обеспечивает транзитную передачу трафика.

![Последовательность 3](./media/migrate-from-hub-spoke-topology/flow3.png)


### <a name="path-4"></a>Путь 4
Путь 4 описывает последовательность трафика из виртуальной сети Azure в регионе "Юго-Восточная Азия" в виртуальную сеть Azure в регионе "Западная Европа".
...
Путь 5 описывает последовательность трафика от пользователей, подключенных в роуминге через VPN типа "точка — сеть", к виртуальной сети Azure в регионе "Западная Европа".

Трафик направляется следующим образом:
- Пользователи ноутбуков и мобильных устройств используют клиент OpenVPN для прозрачного подключения к VPN-шлюзу типа "точка — сеть" в регионе "Западная Европа".
...
- Сопоставьте политику брандмауэра с концентратором виртуальной глобальной сети Azure.
- Этот шаг позволяет применить существующий концентратор виртуальной глобальной сети в качестве защищенного виртуального концентратора и развертывать необходимые ресурсы Брандмауэра Azure.
...
Путь 6 описывает защищенный поток трафика между виртуальными сетями в пределах одного региона.
...
Путь 7 описывает поток трафика из виртуальной сети Azure в Интернет или в сторонние службы безопасности.
