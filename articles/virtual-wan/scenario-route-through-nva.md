...
ms.locfileid: "100519802"
---
# <a name="scenario-route-traffic-through-an-nva"></a>Scenario: перенаправление трафика через сетевой виртуальный модуль
...
* "NVA виртуальных сетей" для виртуальных сетей, где пользователи развернули NVA и подключили другие виртуальные сети в качестве периферийных (VNet 2 и VNet 4 на **рис. 2** далее в статье).
* «NVAные периферийные» для виртуальных сетей, подключенных к виртуальной сети NVA (VNet 5, VNet 6, VNet 7 и VNet 8 на **рис. 2** далее в статье).
* "Non-NVA виртуальных сетей" для виртуальных сетей, подключенных к виртуальной глобальной сети, которая не имеет NVA или других виртуальных сетей, связанных с ними (VNet 1 и VNet 3 на **рис. 2** далее в статье).
...
| Исходный тип             | на:|   *NVA спицы*|*NVA виртуальных сетей*|*Не NVA виртуальных сетей*|*Ветви*|
|---|---|---|---|---|---|
| **NVA спицы**   | &#8594; | Через NVA VNet | Пиринг | Через NVA VNet | Через NVA VNet |
| **NVA виртуальных сетей**    | &#8594; | Пиринг | Прямой доступ | Прямой доступ | Прямой доступ |
| **Не NVA виртуальных сетей**| &#8594; | Через NVA VNet | Прямой доступ | Прямой доступ | Прямой доступ |
| **Ветви**     | &#8594; | Через NVA VNet | Прямой доступ | Прямой доступ | Прямой доступ |

Каждая ячейка в матрице подключения описывает, как виртуальная сеть или ветвь (сторона "от" потока, заголовки строк в таблице) взаимодействуют с целевой виртуальной сетью или ветвью ("to"), заголовки столбцов в таблице заменяются курсивом. "Direct" означает, что подключение обеспечивается встроенной виртуальной глобальной сетью, "пиринг" означает, что подключение предоставляется User-Definedным маршрутом в виртуальной сети, "over NVA VNet" означает, что подключение проходит через NVA, развернутый в виртуальной сети NVA. Рассмотрим следующий пример.
...
* NVA виртуальных сетей узнает о своих собственных NVAях, но не о NVA периферийных зонах, подключенных к другим NVA виртуальных сетей. Например, на рис. 2 Далее в этой статье виртуальная сеть 2 знает о виртуальной сети 5 и виртуальной сети 6, но не о других периферийных зонах, таких как VNet 7 и VNet 8. Статический маршрут требуется для добавления префиксов других периферийных серверов в NVA виртуальных сетей
* Аналогичным образом, ветви и NVA виртуальных сетей не будут иметь знания о каких-либо периферийных NVA, так как NVA периферийные зоны не подключены к виртуальным концентраторам глобальной сети. В результате также потребуется использовать статические маршруты.
