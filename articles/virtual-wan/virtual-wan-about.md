...
* **Интегрированные решения для подключения в центрально-лучевой сети:** Автоматизируйте настройку типа "сеть — сеть" и подключение между локальными сайтами и центром Azure.
...
* **Интуитивно понятные способы устранения неполадок:** Вы можете увидеть сквозной поток в Azure, а затем использовать эти сведения для выполнения необходимых действий.
...
**Дополнительные ресурсы Виртуальной глобальной сети**
  ...
  * **Сайт:** Этот ресурс используется только для подключений "сеть — сеть". Ресурс узла **vpnsite**. Он представляет локальное VPN-устройство и его параметры. При работе с участником Виртуальной глобальной сети экспорт информации в Azure осуществляется автоматически.

## <a name="connectivity"></a>Типы подключения

Виртуальная глобальная сеть поддерживает следующие типы подключения: VPN типа "сеть — сеть", VPN пользователя (точка-сеть) и ExpressRoute.
