---
title: Руководящие принципы автоматизации виртуальных партнеров Azure WAN (ru) Документы Майкрософт
...
ms.locfileid: "77190415"
---
# <a name="automation-guidelines-for-virtual-wan-partners"></a>Руководящие принципы автоматизации для виртуальных партнеров WAN
...
* Просмотр [APIs REST,](#additional) которые используются для автоматизации подключения к Virtual WAN Azure.
...
* [REST API](https://docs.microsoft.com/rest/api/virtualwan/virtualhubs) для автоматизации создания виртуального концентратора
* [REST API](https://docs.microsoft.com/rest/api/virtualwan/vpngateways) автоматизировать VPN шлюз Azure для виртуального WAN
* [REST API](https://docs.microsoft.com/rest/api/virtualwan/vpnconnections) подключит VPNSite к концентратору VPN Azure
* [Политики IPsec по умолчанию](#default)

## <a name="customer-experience"></a><a name ="ae"></a>Опыт работы с клиентами
...
Определите ожидаемое качество обслуживания клиентов для Виртуальной глобальной сети Azure.
  ...
  7. Рассмотрите возможность устранения неполадок и мониторинга интерфейса управления CPE. Типичные сценарии включают в себя "Клиент не может получить доступ к ресурсам Azure из-за проблемы CPE", "Показать параметры IPsec на стороне CPE" и т.д.

## <a name="automation-details"></a><a name ="understand"></a>Детали автоматизации

###  <a name="access-control"></a><a name="access"></a>Контроль доступа
...
Необходимо спроектировать пользовательский интерфейс, чтобы загрузить информацию о филиале (на территории сайта) в Azure. Для создания информации о сайте в Virtual WAN можно использовать [rest AIS](https://docs.microsoft.com/rest/api/virtualwan/vpnsites) для VPNSite. Вы можете выбрать все VPN-устройства или SDWAN для филиалов или выбрать соответствующие настройки устройств.

### <a name="device-configuration-download-and-connectivity"></a><a name="device"></a>Загрузка конфигурации устройства и подключение

Этот шаг включает в себя загрузку конфигурации Azure и настройку подключения с устройства филиала к Виртуальной глобальной сети Azure. На этом этапе клиент, который не использует поставщик, может вручную загрузить конфигурацию Azure и применить ее к своему локальному VPN-устройству или SDWAN. Вам как поставщику следует автоматизировать этот этап. Просмотр [скачать REST AIS](https://docs.microsoft.com/rest/api/virtualwan/vpnsitesconfiguration/download) для получения дополнительной информации. Контроллер устройства может вызвать REST API 'GetVpnConfiguration' для загрузки конфигурации Azure.
...
**Примечания к конфигурации**
  ...
  * VPN-соединение использует маршрутную конфигурацию и поддерживает как протоколы IKEv1, так и IKEv2.
...
* **vpnSiteConnections**. В этом разделе представлены следующие сведения:
    ...
    * **IP-адреса** VPN-шлюзов виртуального концентратора. Так как каждое подключение VPN-шлюза состоит из 2 туннелей в конфигурации "активный — активный", в этом файле будут указаны оба IP-адреса. В данном примере вы видите Instance0 и Instance1 для каждого сайта.<br>Пример
        ...
        ```
    * **Детали конфигурации соединения Vpngateway,** такие как BGP, предварительно общий ключ и т.д. PSK — это предварительно общий ключ, автоматически генерируемый для вас. Вы всегда можете изменить подключение на странице обзора для настраиваемого ключа PSK.
...
## <a name="connectivity-details"></a><a name="default"></a>Детали подключения
