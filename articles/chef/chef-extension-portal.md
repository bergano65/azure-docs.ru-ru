...
1. На вкладке **Установка расширения** укажите приведенные ниже значения, после чего щелкните **ОК**.
    ...
    - **Имя узла Chef** . Введите имя узла Chef.
    - **Список выполнения** — введите список выполнения Chef, который добавляется на компьютер. Это значение можно оставить пустым.
    - **Имя клиента проверки** — введите имя клиента проверки Chef. Например, `tarcher-validator`.
    ...
    - **Файл конфигурации клиента** — выберите файл конфигурации для Chef-Client. Это значение можно оставить пустым.
    - **Chef Client version** (Версия клиента Chef). Введите версию клиента Chef, которую необходимо установить. Это значение можно оставить пустым, при этом устанавливается последняя версия.
    ...
    - **Секретный код в зашифрованном контейнере данных** — выберите файл, содержащий секрет для зашифрованного контейнера данных, к которому должен получить доступ этот компьютер. Это значение можно оставить пустым.
    - **SSL-сертификат сервера Chef** . Выберите сертификат SSL, назначенный серверу Chef. Это значение можно оставить пустым.
...
1. При отображении вкладки **создать** отображается сводка по выбранным и введенным параметрам. Проверьте сведения, а также **условия использования**и нажмите кнопку **создать**.
