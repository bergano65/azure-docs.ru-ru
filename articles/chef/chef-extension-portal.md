...
- **Подписка Azure:** Если у вас нет подписки Нацу, создайте [бесплатную учетную запись](https://azure.microsoft.com/free/?ref=microsoft.com&utm_source=microsoft.com&utm_medium=docs&utm_campaign=visualstudio) перед началом.
...
1. На вкладке **Установка расширения** укажите приведенные ниже значения, после чего щелкните **ОК**.
    ...
    - **Имя узла шеф-повара** - Введите имя узла шеф-повара.
    - **Список выполнения** - Введите список запуска Chef, который добавляется в машину. Это значение можно оставить пустым.
    - **Имя клиента валидации** - Введите имя клиента проверки шеф-повара. например, `tarcher-validator`.
    ...
    - **Файл конфигурации клиента** - Выберите файл конфигурации для шеф-повара-клиента. Это значение можно оставить пустым.
    - **Chef Client version** (Версия клиента Chef). Введите версию клиента Chef, которую необходимо установить. Это значение можно оставить пустым, который устанавливает последнюю версию.
    ...
    - **Секрет зашифрованного пакета данных** - Выберите файл, содержащий секрет для зашифрованного мешка данных, к которому эта машина должна получить доступ. Это значение можно оставить пустым.
    - **Сертификат шеф-повара Server SSL** - Выберите сертификат SSL, присвоенный вашему серверу шеф-повара. Это значение можно оставить пустым.

      ![Установка сервера Chef на виртуальной машине Linux](./media/chef-extension-portal/install-extension.png)

1. При отображении **вкладки Extensions** выберите **OK.**

1. При отображении **вкладки «Настройки»** выберите **OK.**

1. При отображении вкладки **«Создание»** отображается резюме выбранных и введенных вариантов. Проверьте информацию, а также **условия использования**и выберите **Создать**.
