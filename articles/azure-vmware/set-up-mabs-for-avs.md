...
:::image type="content" source="media/avs-backup/deploy-mabs-avs-diagram.png" alt-text="Архитектура развертывания AVS" border="false":::
...
| 60                      | 200 ГБ                | 10-процентное изменение                  | 10 500            | SSD (цен. категория "Премиум") * (8 ТБ или превышает размер на диск) | DS3_V2     |
...
1. В меню слева выберите **все службы**.
   ...
   ![Выбор "Все службы"](../backup/media/backup-create-rs-vault/click-all-services.png)

1. В диалоговом окне **все службы** введите **службы восстановления** и в списке выберите **хранилища служб восстановления** .

   ![Ввод и выбор "Хранилища служб восстановления"](../backup/media/backup-create-rs-vault/all-services.png)

   После этого отобразится список хранилищ служб восстановления в подписке.

1. На панели мониторинга **хранилища служб восстановления** нажмите кнопку **Добавить**.

   ![Добавление хранилища Служб восстановления](../backup/media/backup-create-rs-vault/add-button-create-vault.png)
...
1. Введите значения для **имени**, **подписки**, **группы ресурсов**и **расположения**.
   ...
   ![Настройка хранилища Служб восстановления](../backup/media/backup-create-rs-vault/create-new-vault-dialog.png)

   - **Имя**: введите понятное имя для идентификации хранилища. Это имя должно быть уникальным в пределах подписки Azure. Укажите имя, которое содержит по крайней мере два символа, но не более 50 символов. Оно должно начинаться с буквы и может содержать только буквы, цифры и дефисы.
...
1. Когда все будет готово к созданию хранилища Служб восстановления, выберите **Создать**.
   ...
   ![Создание хранилища Служб восстановления](../backup/media/backup-create-rs-vault/click-create-button.png)
   ...
   ![Обновление списка хранилищ Служб восстановления](../backup/media/backup-create-rs-vault/refresh-button.png)
...
1. Выберите тип репликации хранилища и нажмите кнопку **сохранить**.
   ...
   ![Задание конфигурации хранилища для нового хранилища](../backup/media/backup-try-azure-backup-in-10-mins/recovery-services-vault-backup-configuration.png)
...
1. Если хранилище служб восстановления уже открыто, перейдите к следующему шагу. Если хранилище служб восстановления не открыто, но вы находитесь в портал Azure, в главном меню выберите **Обзор**.
   ...
   1. Элементы списка фильтруются автоматически по мере ввода символов. Когда вы увидите пункт **Хранилища служб восстановления**, щелкните его.
...
1. в котором нужно выбрать хранилище.
   ...
   ![Открыть панель мониторинга хранилища](../backup/media/backup-azure-microsoft-azure-backup/vault-dashboard.png)

   По умолчанию открывается параметр **Параметры** . Если параметр закрыт, выберите **Параметры** , чтобы открыть его.

   ![Параметр "открыть параметры хранилища"](../backup/media/backup-azure-microsoft-azure-backup/vault-setting.png)

1. Выберите **резервное копирование** , чтобы открыть мастер **Начало работы** .

   ![Мастер открытия начало работы](../backup/media/backup-azure-microsoft-azure-backup/getting-started-backup.png)

1. В открывшемся окне выполните следующие действия.

   1. В меню **где работает Рабочая нагрузка?** выберите **локальный**.

      :::image type="content" source="media/avs-backup/deploy-mabs-on-premises-workload.png" alt-text="Где работает Рабочая нагрузка?":::
   ...
   1. Выберите **Подготовка инфраструктуры** , чтобы скачать и установить Azure Backup Server и учетные данные хранилища.
      ...
      :::image type="content" source="media/avs-backup/deploy-mabs-prepare-infrastructure.png" alt-text="Подготовка инфраструктуры":::
...
1. В открывшемся окне **Подготовка инфраструктуры** выполните следующие действия.
   ...
   :::image type="content" source="media/avs-backup/deploy-mabs-prepare-infrastructure2.png" alt-text="Подготовка инфраструктуры — Azure Backup Server":::
...
1. На странице загрузки выберите все файлы и нажмите кнопку **Далее**.
   ...
   ![Выбор файлов в центре загрузки](../backup/media/backup-azure-microsoft-azure-backup/downloadcenter.png)
...
1. Нажмите кнопку **извлечь** , чтобы начать процесс извлечения.
   ...
   ![Мастер установки Microsoft Azure Backup](../backup/media/backup-azure-microsoft-azure-backup/extract/03.png)
...
1. В окне установки в разделе **Установка**выберите **Microsoft Azure Backup** , чтобы открыть мастер установки.
   ...
   ![Кнопка установки мастера установки Microsoft Azure Backup](../backup/media/backup-azure-microsoft-azure-backup/launch-screen2.png)
...
1. Щелкните **проверить еще раз** , чтобы определить, выполнены ли требования к оборудованию и программному обеспечению для Azure Backup Server. Если это выполнено успешно, нажмите кнопку **Далее**.
   ...
   ![Проверка предварительных требований Azure Backup Server](../backup/media/backup-azure-microsoft-azure-backup/prereq/prereq-screen2.png)
...
1. Пакет установки Azure Backup Server входит в состав необходимых SQL Server двоичных файлов. При запуске новой Azure Backup Server установки выберите **установить новый экземпляр SQL Server с этим** параметром установки. Затем выберите **проверить и установить**.
   ...
   ![Проверка SQL Server Azure Backup Server](../backup/media/backup-azure-microsoft-azure-backup/sql/01.png)
   ...
   * **URL-адрес портала**: **виртуальный каталог** должен **быть \<SQLInstanceName> Reports_**.
   ...
   [Подробнее](/sql/reporting-services/report-server/configure-and-administer-a-report-server-ssrs-native-mode?view=sql-server-2017) о конфигурации SSRS.
...
1. Укажите расположение для установки файлов Microsoft Azure Backup Server и нажмите кнопку **Далее**.
   ...
   ![Параметры установки Microsoft Azure Backup установки](../backup/media/backup-azure-microsoft-azure-backup/space-screen.png)

1. Укажите надежный пароль для ограниченных локальных учетных записей пользователей и нажмите кнопку **Далее**.

   ![Настройка параметров безопасности Microsoft Azure Backup](../backup/media/backup-azure-microsoft-azure-backup/security-screen.png)
...
1. Выберите, следует ли использовать Центр обновления Майкрософт для проверки наличия обновлений, и нажмите кнопку **Далее**.
   ...
   ![Microsoft Azure Backup установки Центр обновления Майкрософт](../backup/media/backup-azure-microsoft-azure-backup/update-opt-screen2.png)
...
1. Введите место для сохранения парольной фразы, а затем нажмите кнопку **Далее** , чтобы зарегистрировать сервер.
   ...
   ![Установка Microsoft Azure Backup установки](../backup/media/backup-azure-microsoft-azure-backup/final-install/venus-installation-screen.png)
