...
description: Сведения о ключевых возможностях управления доступом на основе ролей для решения Azure VMware (AVS)
ms.topic: conceptual
ms.date: 06/30/2020
...
ms.locfileid: "85660904"
---
# <a name="role-based-access-control-rbac-for-azure-vmware-solution-avs"></a>Управление доступом на основе ролей (RBAC) для решения Azure VMware (AVS)

В vCenter и ESXi локального развертывания администратор имеет доступ к administrator@vsphere.local учетной записи vCenter и может иметь дополнительные Active Directory (AD) пользователей и групп. Однако в развертывании решения Azure VMware (AVS) администратор не имеет доступа к учетной записи администратора, но может назначать пользователей и группы AD роли CloudAdmin в vCenter.  Кроме того, пользователь частного облака AVS не имеет разрешения на доступ или настройку конкретных компонентов управления, поддерживаемых и управляемых Майкрософт, таких как кластеры, узлы, хранилища данных и распределенные виртуальные коммутаторы.


В AVS v vCenter имеет встроенного локального пользователя с именем cloudadmin, назначенного встроенной роли CloudAdmin. Локальный пользователь cloudadmin используется для настройки дополнительных пользователей в AD. Роль CloudAdmin в целом имеет право создавать рабочие нагрузки в частном облаке (виртуальные машины, пулы ресурсов, хранилища данных и сети) и управлять ими. Роль CloudAdmin в AVS имеет специальный набор привилегий vCenter, отличающийся от других облачных решений VMware.   

> [!NOTE]
> Сейчас AVS не предлагает настраиваемые роли на сервере vCenter или на портале AVS. 

## <a name="avs-cloudadmin-role-on-vcenter"></a>Роль CloudAdmin AVS в vCenter

Вы можете просмотреть привилегии, предоставленные роли CloudAdmin AVS в частном облаке "AVS".
...
1. В списке ролей выберите **CloudAdmin** и щелкните **привилегии**. 
   ...
   :::image type="content" source="media/rbac-cloudadmin-role-privileges.png" alt-text="Просмотр привилегий роли CloudAdmin в клиенте vSphere":::

Роль CloudAdmin в AVS имеет следующие права доступа к vCenter. Подробное описание каждой привилегии см. в [документации по продукту VMware](https://docs.vmware.com/en/VMware-vSphere/7.0/com.vmware.vsphere.security.doc/GUID-ED56F3C4-77D0-49E3-88B6-B99B8B437B62.html) .
...
| **Global** | Отменить задачу<br />Глобальный тег<br />Health<br />Событие журнала<br />Управление настраиваемыми атрибутами<br />Диспетчеры служб<br />Задать настраиваемый атрибут<br />Системный тег |
