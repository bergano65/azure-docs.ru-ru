...
Да. при условии, что система, на которой он установлен, может получить доступ к частному облаку vCenter и использовать общедоступный DNS-сервер (для разрешения имен узлов ESXi).
...
#### <a name="what-is-the-correct-storage-policy-for-the-dedup-setup"></a>Какова правильная политика хранения для установки дедупликации?
...
Перед созданием ресурса решения Azure VMware необходимо отправить запрос в службу поддержки, чтобы узлы были распределены. Подтверждение запроса и выделение узлов занимает до пяти рабочих дней. Если у вас уже есть частное облако Решения Azure VMware и вы хотите выделить для него больше узлов, выполните те же действия.
