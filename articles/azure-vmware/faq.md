...
### <a name="why-does-azure-vmware-solution-use-a-public-4-byte-autonomous-system-number-asn"></a>Почему решение Azure VMware использует общедоступный 4-байтовый номер автономной системы (ASN)?
Решение VMware для Azure использует официально зарегистрированный общедоступный 4-байтный ASN, чтобы убедиться в отсутствии конфликтов с локальным использованием частного ASN в маршруте маршрутизации клиента к решению Azure VMware.
 
### <a name="how-can-i-use-expressroute-to-connect-to-azure-vmware-solution-if-the-on-premises-expressroute-carrier-partnersisps-dont-support-4-byte-asn"></a>Как использовать ExpressRoute для подключения к решению VMware для Azure, если локальные партнеры и поставщики услуг не поддерживают 4-байтовый ASN?
Единственный способ подключения к решению VMware для Azure через ExpressRoute — для вашей среды, а локальные партнеры и поставщики услуг на основе ExpressRoute поддерживают 4-байтовый ASN или имеют обратную совместимость из 4 байта в 2-байтовый ASN в объявлении пути BGP с префиксом ASN.
