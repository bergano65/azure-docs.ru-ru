...
2. При выборе учетной записи NetApp можно просматривать различные параметры. Например, если выбрать **contoso-anf2**, вы увидите его параметры. 
    ...
    :::image type="content" source="media/net-app-files/net-app-settings.png" alt-text="Просмотр параметров учетной записи NetApp."::: 

3. Выберите **Пулы ресурсов** , чтобы проверить настроенный пул. 

    :::image type="content" source="media/net-app-files/capacity-pools.png" alt-text="Проверьте настроенный пул.":::
...
4. Выберите **тома** (см. снимок экрана из шага 2), чтобы просмотреть тома, созданные в пуле ресурсов.  
    ...
    :::image type="content" source="media/net-app-files/azure-net-app-volumes.png" alt-text="Azure NetApp Files том.":::

5. Выберите том, чтобы просмотреть его конфигурацию.  

    :::image type="content" source="media/net-app-files/configuration-of-volume.png" alt-text="Сведения о конфигурации тома.":::
...
2. Введите необходимые сведения для имени пула томов, уровня обслуживания и размера диска (FQDN) или IP-адреса и веса. Выберите **Добавить**.

    :::image type="content" source="media/net-app-files/add-capacity-pool.png" alt-text="Добавьте пул емкости в файлы NetApp.":::
...
4. Заполните обязательные поля, как показано на снимке экрана ниже.
    ...
    :::image type="content" source="media/net-app-files/create-a-volume.png" alt-text="Добавьте тома в пул ресурсов.":::
...
6. Если это общий ресурс NFS, добавьте исходный IP-адрес, из которого будет осуществляться доступ к общему ресурсу типа протокола. Выберите **Review + create** (Просмотреть и создать). 
    ...
    :::image type="content" source="media/net-app-files/select-volume-details.png" alt-text="Выберите сведения о томе.":::
 
    В разделе Azure NetApp Files в портал Azure общий ресурс NFS готов к использованию.

    :::image type="content" source="media/net-app-files/share-ready.png" alt-text="Общая папка NFS готова.":::
...
- Откройте командную строку и выполните команду, чтобы отобразить файловый ресурс NFS. На следующих снимках экрана показан общий диск, сопоставленный на виртуальной машине Windows в решении VMware Azure.  
    ...
    :::image type="content" source="media/net-app-files/share-mapped-to-windows-vm.png" alt-text="Общий доступ к диску, сопоставленному на виртуальной машине Windows в решении VMware для Azure.":::
 
    :::image type="content" source="media/net-app-files/mapped-to-windows-drive.png" alt-text="Общий доступ к диску, сопоставленному на виртуальной машине Windows.":::
...
3. Подключите файловую систему: `sudo mount -t nfs -o rw,hard,rsize=1048576,wsize=1048576,vers=3,tcp 10.22.3.4:/ANFPOOL ANFPOOL`
    ...
    :::image type="content" source="media/net-app-files/root-test.png" alt-text="Корневой тест.":::
...
2. В разделе Azure NetApp Files выберите **Active Directory подключения**.
    ...
    :::image type="content" source="media/net-app-files/active-directory-connections.png" alt-text="Active Directory подключения."::: 
...
3. Выберите **присоединиться** к общему ресурсу SMB для Active Directory. На следующем снимке экрана показаны сведения о домене общего ресурса SMB.
    ...
    :::image type="content" source="media/net-app-files/active-directory-connect-details.png" alt-text="Сведения о домене общего ресурса SMB."::: 

    Файловый ресурс SMB Azure готов к использованию.  

    :::image type="content" source="media/net-app-files/smb-file-share-details.png" alt-text="Общая папка SMB готова."::: 
...
- См. статью [интеграция решения VMware с Azure в центральную и периферийную архитектуру](concepts-avs-hub-and-spoke-integration.md) .
