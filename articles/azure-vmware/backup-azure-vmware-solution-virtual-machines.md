...
В этой статье мы создадим резервные копии виртуальных машин VMware, работающих в решении VMware для Azure, с Azure Backup Server. Во-первых, тщательно пройдите по [настройке Microsoft Azure Backup Server для решения VMware для Azure](set-up-backup-server-for-azure-vmware-solution.md).
...
1. На странице **Готово** проверьте параметры и нажмите кнопку **Закрыть**.
   ...
   Сервер vCenter Server отображается в списке **рабочий сервер** с помощью:
   - Тип как **сервер VMware** 
   - Состояние агента — **ОК** 
...
1. Укажите, как долго следует выполнять резервное копирование данных на диск.
   ...
   :::image type="content" source="media/azure-vmware-solution-backup/new-protection-group-specify-short-term-goals.png" alt-text="Укажите краткосрочные цели для защиты на основе диска":::
...
1. На странице " **Проверка выделения хранилище дисков** " проверьте место на диске, предоставленное для резервных копий виртуальных машин.
   ...
   - **Место на диске:** Рекомендуемый объем дискового пространства для группы защиты. Если вы хотите изменить этот параметр, выберите пустое пространство, превышающее предполагаемое значение для каждого из источников данных.
   - **Сведения о пуле носителей:** Отображает состояние пула носителей, включая общий и оставшийся размер диска.

   :::image type="content" source="media/azure-vmware-solution-backup/review-disk-allocation.png" alt-text="Проверка места на диске, заданного в пуле носителей":::
...
1. На странице **Указание расписания оперативного резервного копирования** укажите, как часто требуется выполнять резервное копирование данных из локального хранилища в Azure. 
...
1. На странице **Указание политики оперативного хранения** укажите, как долго следует хранить точки восстановления, созданные из резервных копий в Azure.
...
- В области задач " **наблюдение** " выполните следующие действия.
   - В разделе **оповещения** можно отслеживать ошибки, предупреждения и общие сведения.  Вы можете просматривать активные и неактивные предупреждения, а также настраивать уведомления по электронной почте.
   - В разделе **задания** можно просмотреть задания, запущенные Azure Backup Server для определенного защищенного источника данных или группы защиты. Можно отслеживать ход выполнения задания или проверять ресурсы, потребляемые заданиями.
...
:::image type="content" source="media/azure-vmware-solution-backup/monitor-backup-jobs.png" alt-text="Отслеживайте состояние заданий резервного копирования в Azure Backup Server":::
...
- **Восстановление в альтернативное расположение (ALR)**: используйте, если ИСХОДная виртуальная машина отсутствует или вы не хотите перебеспокоить ИСХОДную виртуальную машину. Укажите расположение узла ESXi, пул ресурсов, папку, хранилище данных хранилища и путь к нему. Чтобы отличить восстановленную виртуальную машину от исходной виртуальной машины, Azure Backup Server добавляет recoverd *к имени* виртуальной машины.
...
1. В области **Обзор** просмотрите или отфильтруйте элементы, чтобы найти нужную виртуальную машину для восстановления. После выбора виртуальной машины или папки в окне * * точки восстановления для панели будут отображены доступные точки восстановления.

   ![Доступные точки восстановления](../backup/media/restore-azure-backup-server-vmware/recovery-points.png)

1. В области **точки восстановления для** выберите дату, когда была сделана точка восстановления. Даты, за которые доступные точки восстановления, выделены полужирным шрифтом. Кроме того, можно щелкнуть виртуальную машину правой кнопкой мыши и выбрать пункт **Показывать все точки восстановления** , а затем выбрать точку восстановления из списка.
...
1. Перед восстановлением из точки восстановления в сети убедитесь, что промежуточное расположение содержит достаточно свободного места для размещения полного несжатого размера виртуальной машины, которую требуется восстановить. Промежуточное расположение можно просмотреть или изменить, запустив **Мастер настройки параметров подписки**.
   ...
   :::image type="content" source="media/azure-vmware-solution-backup/mabs-recovery-folder-settings.png" alt-text="Azure Backup Server параметры папки восстановления":::
...
1. На странице **Выбор типа восстановления** либо восстановите на исходном экземпляре, либо в новое расположение.
...
- [Устранение неполадок при настройке резервного копирования в Azure Backup Server](../backup/backup-azure-mabs-troubleshoot.md).
