...
> [!NOTE]
> Для достижения наилучшей производительности приема данных размер сжатых больших двоичных *объектов,* отправленных для приема, должен быть передан. Поскольку уведомления в сетке событий содержат только основные сведения, сведения о размере должны быть явно переданы. Сведения о размере без сжатия можно предоставить, установив свойство `rawSizeBytes` метаданных большого двоичного объекта с *несжатым* размером данных в байтах.

### <a name="ingestion-properties"></a>Свойства приема

[Свойства приема](https://docs.microsoft.com/azure/kusto/management/data-ingestion/#ingestion-properties) большого двоичного объекта можно указать с помощью метаданных большого двоичного объекта.
...
| `kustoTable` |  Имя существующей целевой таблицы. Переопределяет `Table`, установленный в колонке `Data Connection`. |
| `kustoDataFormat` |  Формат данных. Переопределяет `Data format`, установленный в колонке `Data Connection`. |
| `kustoIngestionMappingReference` |  Имя существующего сопоставления приема, которое будет использоваться. Переопределяет `Column mapping`, установленный в колонке `Data Connection`.|
...
| `kustoExtentTags` | Строка, представляющая [теги](/azure/kusto/management/extents-overview#extent-tagging) , которые будут присоединены к результирующему экстенту. |
| `kustoCreationTime` |  Переопределяет [$IngestionTime](/azure/kusto/query/ingestiontimefunction?pivots=azuredataexplorer) для большого двоичного объекта в формате строки ISO 8601. Используется для заполнения. |
