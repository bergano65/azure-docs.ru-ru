...
1. В окне **Создание подписки на событие** на вкладке **Основные** введите следующие значения:
    ...
    **Параметр** | **Рекомендуемое значение** | **Описание поля**
    ...
    | Тип раздела | *Учетная запись хранилища* | Тип раздела сетки событий. |
    ...
    | Определенные типы событий | *Blob создан* | Позволяет указать, о каких конкретных событиях нужно получать уведомления. |
...
Вы можете указать [свойства проглатывания](https://docs.microsoft.com/azure/kusto/management/data-ingestion/#ingestion-properties) капли через метаданные капли.
...
| `kustoExtentTags` | Строка, представляющая [теги,](/azure/kusto/management/extents-overview#extent-tagging) которые будут прикреплены в результирующей степени. |
| `kustoCreationTime` |  Оверлет$IngestionTime для капли, отформатированный как строка ISO 8601. [$IngestionTime](/azure/kusto/query/ingestiontimefunction?pivots=azuredataexplorer) Используйте для заправки. |
