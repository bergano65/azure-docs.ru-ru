...
ms.topic: quickstart
ms.date: 1/23/2019
ms.openlocfilehash: 366cef8a5b87c7cba4d3115a6dbae45e800d3f3a
ms.sourcegitcommit: a7331d0cc53805a7d3170c4368862cad0d4f3144
ms.translationtype: HT
ms.contentlocale: ru-RU
ms.lasthandoff: 01/30/2019
ms.locfileid: "55302873"
---
...
* Войдите в учетную запись хранения на портале Azure.
* На вкладке **События** щелкните **Подписка на события**.
 ...
 Укажите следующие значения.
    ...
    **Параметр** | **Рекомендуемое значение** | **Описание поля**
    ...
    | Подписка на все типы событий | *Снять пометку* | Позволяет указать, нужно ли получать уведомления обо всех событиях. |
    ...
    | | |
* Если вы хотите отслеживать файлы из определенного контейнера, щелкните вкладку **Дополнительные функции** и установите фильтры для уведомлений. * Поле **Тема начинается с** — это префикс *литерала* контейнера больших двоичных объектов (так как применен шаблон *startswith*, этот префикс может охватывать несколько контейнеров). Подстановочные знаки не допускаются.
        Значение *нужно* установить в таком виде: *`/blobServices/default/containers/`*[префикс контейнера]. * Поле **Тема заканчивается на** — это суффикс *литерала* большого двоичного объекта. Подстановочные знаки не допускаются.
...
Теперь подключимся к сетке событий с помощью Azure Data Explorer, так чтобы данные, поступающие в контейнер больших двоичных объектов, потоком передавались в тестовую таблицу.
...
1. Заполните форму, указав следующую информацию, затем нажмите кнопку **Создать**.
    ...
    **Параметр** | **Рекомендуемое значение** | **Описание поля**
    ...
    | Учетная запись хранения | *gridteststorage* | Имя учетной записи хранения, созданной на первом шаге.|
...
Сохраните следующие данные в файл и используйте его с приведенным ниже скриптом.
