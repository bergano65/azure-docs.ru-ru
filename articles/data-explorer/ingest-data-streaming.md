...
## <a name="supported-streaming-ingestion-types"></a>Поддерживаемые типы приема потоковой передачи
...
* Производительность приема потоковой передачи и масштабирование емкости с увеличением размера виртуальной машины и кластера. Для одного узла D11 Рекомендуемая нагрузка составляет до 20 запросов в секунду. Для одного D14 узла Рекомендуемая нагрузка составляет до 150 запросов в секунду.
* В настоящее время поддержка поддерживается только для SKU с 8 и 16 ядрами (D13, D14, уровня 8 и L16).
