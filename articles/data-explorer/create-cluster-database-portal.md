...
1. Заполните форму основных сведений о кластере, указав следующую информацию.
    ...
    **Параметр** | **Рекомендуемое значение** | **Описание поля**
    ...
    | Resource group | Ваша группа ресурсов | Воспользуйтесь существующей группой ресурсов или создайте новую. |
    ...
    | Зоны доступности | *1*, *2* и (или) *3* | Для регионов с поддержкой избыточности в пределах зоны выберите зоны, в которых будут размещаться экземпляры (необязательно). Узлы кластера по умолчанию создаются в одном центре обработки данных. Используйте [Зоны доступности Azure](/azure/availability-zones/az-overview), чтобы разместить экземпляры кластера в разных зонах доступности в одном регионе. Выбрав несколько зон доступности, можно исключить единую точку отказа и обеспечить высокий уровень доступности. 
