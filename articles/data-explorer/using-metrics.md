...
Максимальная длина непрерывного экспорта |    Count   | Max   | Задержку (в минутах), сообщаемую заданиями непрерывного экспорта в кластере. | None |
Число ожидающих непрерывного экспорта | Count | Max   | Число ожидающих заданий непрерывного экспорта. Эти задания готовы к запуску, но ожидают в очереди, возможно, из-за недостаточной емкости. 
...
**Метрика** | **Единица измерения** | **Агрегирование** | **Описание метрики** | **Dimensions** |
|---|---|---|---|---|
| Обработано событий (для концентраторов событий и IoT) | Count | Max, min, Sum | Общее число событий, считанных из концентраторов событий и обработанных кластером. События разбиваются на события, отклоненные, и на события, принятые ядром кластера. | EventStatus |
...
| Результат приема | Count | Count | Общее число успешно выполненных операций приема. Используйте **разделение Apply** , чтобы создать контейнеры результатов успеха и сбоя и проанализировать измерения (**значение** > **состояние**).| IngestionResultDetails |
...
**Метрика** | **Единица измерения** | **Агрегирование** | **Описание метрики** | **Dimensions** |
|---|---|---|---|---|
Скорость приема данных потоковой передачи |    Count   | ратерекуестсперсеконд | Общий объем данных, принимаемых в кластер. | None |
Длительность приема потоковой передачи   | Миллисекунды  | AVG, Max, min | Общая длительность всех запросов приема потоковой передачи. | None |
Частота запросов приема потоковой передачи   | Count | Счетчик, AVG, Max, min, Sum | Общее число запросов приема потоковой передачи. | None |
Поток приема результатов | Count | Avg   | Общее число запросов приема потоковой передачи по типу результата. | Результат |
| | | | |
