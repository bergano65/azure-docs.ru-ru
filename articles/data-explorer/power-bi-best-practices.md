...
* **Параллелизм** . Обозреватель данных Azure представляет собой линейную масштабируемую платформу данных, поэтому вы можете повысить производительность отрисовки панели мониторинга, увеличив параллелизм сквозного потока следующим образом:
...
* **Эффективные срезы** — вы можете использовать [срезы синхронизации](https://docs.microsoft.com/power-bi/visuals/power-bi-visualization-slicers#sync-and-use-slicers-on-other-pages) , чтобы не допустить, чтобы отчеты загрузили данные перед тем, как будете готовы. После структурирования набора данных, размещения всех визуальных элементов и пометки всех срезов можно выбрать срез синхронизации для загрузки только необходимых данных.
...
Эти параметры выдают [инструкции SET](/azure/kusto/query/setstatement) с запросом на изменение ограничений по умолчанию для запросов.
  ...
  * **Отключение усечения результирующего набора** приводит к созданию `set notruncation`
...
1. Замените соответствующую часть запроса параметром. Разделите запрос на несколько частей и объедините их обратно с помощью & знака вместе с параметром.
