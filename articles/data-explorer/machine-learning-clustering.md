...
Процесс диагностики является сложным и длительным и осуществляется экспертами области. Этот процесс включает в себя получение и присоединение дополнительных данных из различных источников за один и тот же период времени, поиск изменений в распределении значений по нескольким измерениям, составление графиков дополнительных переменных и другие методы, основанные на знаниях домена и Интуиции. Поскольку эти сценарии диагностики являются общими в Azure Data Explorer, плагины машинного обучения доступны, чтобы сделать фазу диагностики проще и сократить продолжительность RCA.

Azure Data Explorer имеет три плагина машинного обучения: [`autocluster`](/azure/kusto/query/autoclusterplugin), [`basket`](/azure/kusto/query/basketplugin)и [`diffpatterns`](/azure/kusto/query/diffpatternsplugin). Все плагины реализуют алгоритмы кластеризации. Плагины `autocluster` и `basket` плагины кластера `diffpatterns` одного набора записей и плагин кластеров различия между двумя наборами записей.
...
Несмотря на то, что исключений меньше тысячи, все равно трудно найти общие сегменты, так как в каждом столбце есть несколько значений. Вы можете [`autocluster()`](/azure/kusto/query/autoclusterplugin) использовать плагин, чтобы мгновенно извлечь небольшой список общих сегментов и найти интересные кластеры в течение двух минут всплеска, как видно из следующего запроса:
...
Вы также можете [`basket()`](/azure/kusto/query/basketplugin) использовать плагин, как видно из следующего запроса:
