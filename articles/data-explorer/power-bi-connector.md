...
1. На экране **Обозреватель данных Azure (Kusto)** заполните форму следующими сведениями.
    ...
    **Параметр** | **Value** | **Описание поля**
    |---|---|---|
    | Кластер | *https://help.kusto.windows.net* | URL-адрес кластера help. Для других кластеров URL-адрес имеет вид *https://\<Имя_кластера\>.\<Регион\>.kusto.windows.net*. |
    | Database | Не указывайте | База данных, размещенная в кластере, к которому вы подключаетесь. Это будет указано позже. |
    ...
    | Дополнительные параметры. | Не указывайте | Варианты запроса, например размер результирующего набора. |
