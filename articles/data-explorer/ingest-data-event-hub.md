...
* [Пример приложения](https://github.com/Azure-Samples/event-hubs-dotnet-ingest), создающего данные и отправляющего их в концентратор событий.
...
1. Нажмите следующую кнопку, чтобы начать развертывание. Чтобы следовать инструкциям в этой статье, мы рекомендуем открыть ссылку в другой вкладке или окне.
...
1. Заполните форму, указав следующую информацию.
    ...
    **Параметр** | **Рекомендуемое значение** | **Описание поля**
    ...
    | Расположение | *Западная часть США* | Выберите *Западная часть США* для этого краткого руководства. Для производственной системы выберите регион, лучше всего соответствующий вашим потребностям. Создайте пространство имен концентратора событий в одном расположении с кластером Kusto, чтобы обеспечить оптимальную производительность (крайне важно для пространств имен концентраторов событий с высокой пропускной способностью).
...
1. Заполните форму, указав следующую информацию, затем нажмите кнопку **Создать**.
    ...
    **Параметр** | **Рекомендуемое значение** | **Описание поля**
    ...
    | Целевая таблица | Не активируйте флажок **My data includes routing info** (Мои данные содержат сведения о маршрутизации). | Существует два варианта маршрутизации: *статическая* и *динамическая*. В этом кратком руководстве используется статическая маршрутизация (по умолчанию), в которой можно указать имя таблицы, формат файла и сопоставление. Вы также можете использовать динамическую маршрутизацию, где данные содержат необходимые сведения о маршрутизации. |
...
Теперь, когда Azure Data Explorer и концентратор событий соединены, используйте загруженный [пример приложения](https://github.com/Azure-Samples/event-hubs-dotnet-ingest) для создания данных.
...
1. Вернитесь к примеру приложения и остановите его, когда оно достигнет сообщения 99.
