...
description: В этой статье вы узнаете, как прием данных (загрузка) в обозреватель данных Azure из концентратора событий.
author: orspod
ms.author: orspodek
ms.reviewer: mblythe
...
В этой статье вы можете создать демонстрационные данные и отправить его в концентратор событий. Первым шагом является создание концентратора событий. Это делается с помощью шаблона Azure Resource Manager на портале Azure.
...
1. Чтобы увидеть содержимое сообщений, выполните следующий запрос:
    ...
    > [!NOTE]
    > В Azure Data Explorer настроена политика агрегирования (пакетной обработки) для приема данных, предназначенных для оптимизации процесса. Эта политика настроена на 5 минут, по умолчанию, могут возникать задержки. См. в разделе [Пакетная обработка политики](/azure/kusto/concepts/batchingpolicy) для параметры статистической обработки. См. в разделе [потоковой передачи политики](/azure/kusto/concepts/streamingingestionpolicy) для приема данных с помощью ни один агрегат.
