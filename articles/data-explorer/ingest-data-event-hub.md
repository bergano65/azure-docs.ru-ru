...
1. Выберите **Прием данных** и **Добавить подключение к данным**. Заполните форму, указав следующую информацию. По завершении нажмите кнопку **Создать**.
    ...
    **Целевая таблица:**
     ...
     **Параметр** | **Рекомендуемое значение** | **Описание поля**
    |---|---|---|
    | Table | *TestTable* | Таблица, созданная в базе данных **TestDatabase**. |
    | Формат данных | *JSON* | Поддерживаются форматы Avro, CSV, JSON, многострочные JSON, ПСВ, СОХСВ, СКСВ, TSV, ТСВЕ, TXT, ORC и PARQUET. |
    | Сопоставление столбцов | *TestMapping* | [Сопоставление](/azure/kusto/management/mappings) , созданное в **тестдатабасе**, которое сопоставляет входящие данные JSON с именами столбцов и типами данных **тесттабле**. Требуется для JSON или многострочного JSON и необязательно для других форматов.|
    | | |
