...
## <a name="sign-in-to-the-azure-portal"></a>Вход на портал Azure
 ...
 DevOps Projects создает конвейер CI/CD в Azure Pipelines.  Вы можете создать новую бесплатную организацию Azure DevOps или использовать существующую. DevOps Projects также создает ресурсы Azure в требуемой подписке Azure.
...
Веб-приложение для контейнеров является целью развертывания по умолчанию. Исполняющая среда, которую вы выбрали на предыдущих шагах, определяет тип цели развертывания службы Azure, доступной здесь. 
...
На предыдущем шаге с помощью функции DevOps Projects был автоматически настроен весь конвейер CI/CD. При необходимости изучите и настройте конвейер. Чтобы составить представление о конвейерах сборки и выпуска, сделайте следующее:
...
1. Выберите поле **Состояние** и щелкните значок **многоточия** (...).  
        В появившемся меню содержится несколько параметров, таких как постановка в очередь новой сборки, приостановка сборки и правка конвейера сборки.
...
1. Выберите **Триггеры**.  
         DevOps Projects автоматически создает триггер CI, а при каждой фиксации в репозитории создается новая сборка.  При желании выберите включение или исключение ветвей из процесса непрерывной интеграции.
...
1. В левой части окна выберите **Задачи**.   
Задачи — это действия, выполняемые при развертывании. В этом примере задача заключается в развертывании в Службу приложений Azure.
...
После настройки процесса CI/CD автоматически были созданы конвейеры сборки и выпуска. Вы можете изменить эти конвейеры сборки и выпуска в соответствии с потребностями вашей команды. Дополнительные сведения о конвейере CI/CD доступны по этой ссылке:
