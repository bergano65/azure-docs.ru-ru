---
title: Создание конвейера CI/CD для Ruby on Rails с помощью проекта Azure DevOps | Быстрое начало
description: Проект DevOps упрощает начало работы с Azure. Это поможет вам запустить веб-приложение Ruby в службе Azure за несколько быстрых шагов.
ms.prod: devops
...
ms.locfileid: "47405086"
---
# <a name="create-a-cicd-pipeline-for-ruby-on-rails-with-the-azure-devops-project"></a>Создание конвейера CI/CD для Ruby on Rails с помощью проекта Azure DevOps

Настройте непрерывную интеграцию (CI) и непрерывную доставку (CD) для вашего приложения Ruby on Rails с помощью проекта **Azure DevOps**.  Проект Azure DevOps упрощает первоначальную конфигурацию конвейера сборки и выпуска Azure DevOps Services.
...
1. Щелкните значок **Создать ресурс** на панели навигации слева, затем выполните поиск по строке **Проект DevOps**.  Выберите **Создать**.
    ...
    ![Начало непрерывной поставки](_img/azure-devops-project-ruby/fullbrowser.png)
...
1. Выберите платформу приложений **Ruby on Rails**.  Когда все будет готово, щелкните **Далее**.

1. **Веб-приложение в Linux** является целью развертывания по умолчанию.  При необходимости можно выбрать веб-приложение для контейнеров.  Исполняющая среда, которую вы выбрали на предыдущих шагах, определяет тип цели развертывания службы Azure, доступной здесь.  Выберите необходимую **целевую службу**.  Когда все будет готово, щелкните **Далее**.
...
1. За несколько минут на портале Azure загружается **панель мониторинга проекта Azure DevOps**.  Пример приложения настраивается в репозитории в организации Azure DevOps Services, выполняется сборка, а ваше приложение развертывается в Azure.  Эта панель мониторинга обеспечивает видимость **репозитория кода**, **конвейера CI/CD Azure** и **приложения в Azure**.  В правой части панели мониторинга выберите **Обзор**, чтобы просмотреть запущенное приложение.
    ...
    ![Представление панели мониторинга](_img/azure-devops-project-ruby/dashboardnopreview.png) 
...
1. В левой части браузера перейдите к файлу **app/views/pages/home.html.erb**.
