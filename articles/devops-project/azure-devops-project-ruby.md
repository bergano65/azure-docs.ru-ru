---
title: Краткое руководство. Создание конвейера CI/CD для приложения Ruby on Rails с помощью Azure DevOps Projects
description: Azure DevOps Projects помогает быстро приступить к работе с Azure. Вы можете запустить веб-приложение Ruby в службе Azure, выполнив несколько простых действий.
ms.prod: devops
...
ms.locfileid: "52264528"
---
# <a name="create-a-cicd-pipeline-for-ruby-on-rails-by-using-azure-devops-projects"></a>Создание конвейера CI/CD для приложения Ruby on Rails с помощью Azure DevOps Projects

Настройте непрерывную интеграцию (CI) и непрерывную поставку (CD) для приложения Ruby on Rails с помощью Azure DevOps Projects. DevOps Projects упрощает первоначальную настройку конвейера сборки и выпуска Azure DevOps.
...
Служба DevOps Projects создает конвейер CI/CD в Azure Repos. Вы можете создать новую организацию Azure DevOps или использовать существующую. DevOps Projects также создает ресурсы Azure в требуемой подписке Azure.
...
1. **Веб-приложение в Linux** является целью развертывания по умолчанию.  
    Дополнительно можно выбрать **Веб-приложение для контейнеров**. Платформа приложения, которую вы выбрали ранее, определяет доступный тип объекта развертывания в службе Azure. 
...
1. Выберите подписку Azure и расположение, введите имя приложения и нажмите **Готово**.  
    Через несколько минут панель мониторинга DevOps Projects отобразится на портале Azure. Пример приложения настраивается в репозитории в организации Azure DevOps, затем выполняется сборка и приложение развертывается в Azure. 
...
Служба Azure DevOps Projects создает репозиторий Git в Azure Pipelines или на сайте GitHub. Выполните следующие действия, чтобы просмотреть репозиторий и внести изменения в код приложения.

1. В левой части панели мониторинга DevOps Projects щелкните ссылку на ветвь master.  
...
Служба Azure DevOps Projects автоматически настраивает полный конвейер CI/CD в вашей организации Azure DevOps. При необходимости изучите и настройте конвейер. Чтобы ознакомиться с конвейерами сборки и выпуска Azure DevOps, сделайте следующее:
...
1. В области слева выберите **Задачи**.  
    Задачи являются действиями, выполняемыми в процессе развертывания. В этом примере задача была создана для выполнения развертывания в Службе приложений Azure.
...
Ненужный экземпляр Службы приложений Azure и связанные ресурсы, созданные в рамках того руководства, можно удалить. Для этого воспользуйтесь функцией **Удалить** на панели мониторинга DevOps Projects.
