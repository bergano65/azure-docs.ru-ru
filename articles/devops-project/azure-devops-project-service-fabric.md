---
title: Развертывание приложения ASP.NET в Azure Service Fabric с помощью проекта Azure DevOps | Руководство по Azure DevOps Services
description: Проект DevOps упрощает начало работы с Azure. Проект Azure DevOps упрощает развертывание вашего приложения ASP.NET Core на платформе Azure Service Fabric до нескольких простых шагов.
ms.author: mlearned
ms.manager: douge
ms.prod: devops
...
Проект Azure DevOps представляет собой упрощенный интерфейс, к которому вы подключаете существующий код и репозиторий Git. Или можно выбрать один из примеров приложений, чтобы создать конвейер непрерывной интеграции (CI) и непрерывной поставки (CD) в Azure.  Проект DevOps автоматически создает такие ресурсы Azure, как Azure Service Fabric, а также создает и настраивает в Azure DevOps Services конвейер выпуска, который содержит настроенный контейнер CI/CD, и ресурс Azure Application Insights для мониторинга.
...
Проект Azure DevOps создает конвейер CI/CD.  Вы можете создать **новую организацию Azure DevOps Services** или использовать **существующую организацию**.  Проект Azure DevOps также создает **ресурсы Azure**, такие как кластер Service Fabric, в **подписке Azure** по вашему выбору.
...
1. Этот процесс может занять несколько минут.  Он подготовит пример приложения ASP.NET Core в репозитории, настроенном для организации Azure DevOps Services, создаст кластер Service Fabric, выполнит конвейер CI/CD и развернет приложение в Azure.  
    ...
    Эта панель мониторинга содержит сведения о **репозитории кода** Azure DevOps Services, **конвейере CI/CD Azure DevOps Services** и **кластере Service Fabric**.  Вы можете настроить дополнительные параметры в Azure DevOps Services.  В правой части панели мониторинга выберите **Обзор**, чтобы просмотреть запущенное приложение.

## <a name="examine-the-azure-devops-services-ci-pipeline"></a>Изучение конвейера CI в Azure DevOps Services

Проект Azure DevOps автоматически настраивает конвейер CI/CD Azure DevOps Services в организации Azure DevOps Services.  Вы можете изучить и настроить конвейер.  Выполните следующие шаги, чтобы лучше понять конвейеры сборки в Azure DevOps Services.
...
1. Через несколько минут **начинается сборка**, а затем выпуск выполняет развертывание изменений.  Вы можете отслеживать **Состояние сборки** на панели мониторинга проекта DevOps или в браузере с помощью функции ведения в реальном времени журнала об Azure DevOps Services.
...
При необходимости вы можете изменить конвейеры CI/CD Azure, если это потребуется для вашей команды. Вы также можете использовать этот шаблон CI/CD в качестве шаблона для других своих проектов.  Вы научились выполнять следующие задачи:
