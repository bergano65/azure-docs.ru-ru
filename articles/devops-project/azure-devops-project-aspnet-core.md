...
Настройте непрерывную интеграцию (CI) и непрерывную доставку (CD) для вашего приложения .NET Core или ASP.NET с помощью проекта **Azure DevOps**.  Проект Azure DevOps упрощает первоначальную конфигурацию конвейера сборки и выпуска Azure DevOps Services.
...
1. В браузере перейдите к **панели мониторинга Azure DevOps**.  Теперь вы должны увидеть, что сборка выполняется.  Внесенные изменения автоматически создаются и развертываются через конвейер CI/CD Azure DevOps Services.

## <a name="examine-the-azure-devops-services-cicd-pipeline"></a>Изучение конвейера CI/CD в Azure DevOps Services

Проект Azure DevOps автоматически настраивает полный конвейер CI/CD Azure DevOps Services в организации Azure DevOps Services.  При необходимости изучите и настройте конвейер.  Выполните следующие шаги, чтобы ознакомиться с конвейерами сборки и выпуска Azure DevOps Services.
...
1. В этом представлении **рассмотрите различные задачи** для конвейера сборки.  Сборка выполняет различные задачи, такие как выборка источников из репозитория Azure Repos Git, восстановление зависимостей и публикация результатов, используемых для развертывания.
