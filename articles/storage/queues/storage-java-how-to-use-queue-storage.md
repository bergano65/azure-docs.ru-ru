---
title: Как использовать хранилище очередей из Java - Azure Storage
description: Узнайте, как использовать хранилище очередей для создания и удаления очередей, а также ввода, получения и удаления сообщений в клиентской библиотеке Azure Storage для Java.
...
В этом руководстве показано, как реализовать типичные сценарии с использованием службы хранения очередей Azure. Примеры написаны на Java и используют [пакет SDK службы хранилища Azure для Java][Azure Storage SDK for Java]. Сценарии включают **вставки,** **заглядывание,** **получение**и **удаляние** сообщений очереди, а также **создание** и **удаляние** очередей. Дополнительные сведения об очередях см. в разделе [Дальнейшие действия](#next-steps).
...
В этом руководстве будут использоваться функции хранения, которые могут быть запущены в локальном приложении Java или в коде, запущенном в веб-приложении Azure.

Для этого необходимо установить пакет SDK для Java (JDK) и создать учетную запись хранения Azure в подписке Azure. После этого необходимо убедиться, что система разработки отвечает минимальным требованиям и зависимостям, перечисленным в [SDK Хранилища Azure для][Azure Storage SDK for Java] Java-репозитория GitHub. Если ваша система отвечает указанным требованиям можно приступить к выполнению инструкций по загрузке библиотек хранилища Azure для Java из репозитория и их установке на своей системе. После выполнения этих задач можно создать Java-приложение, использующее примеры в этой статье.
...
Объект **Cloud'UтейлиКлиент** позволяет получать справочные объекты для очередей. Следующий код создает объект **CloudQueueClient**. (Примечание. Есть и другие способы создания объектов **CloudStorageAccount**. Дополнительные сведения см. в разделе **CloudStorageAccount** в [справочнике по пакету SDK для клиента службы хранилища Azure].)

Используйте объект **Cloud'ueclient,** чтобы получить ссылку на очередь, в какую вы хотите использовать. Очередь можно создать, если она не существует.
...
Чтобы вставить сообщение в существующую очередь, сначала создайте новый объект **CloudQueueMessage**. Затем позвоните в метод **addMessage.** **Облачное сообщение** может быть создано либо из строки (в формате UTF-8), либо из массива байт. Вот код, который создает очередь (если она не существует) и вставляет сообщение "Здравствуйте, Мир".
...
Код удаляет сообщение из очереди в два этапа. При вызове метода **retrieveMessage**вы получаете следующее сообщение в очереди. Сообщение, возвращаемое методом **retrieveMessage** , становится невидимым для другого кода, считывающего сообщения из этой очереди. По умолчанию это сообщение остается невидимым в течение 30 секунд. Чтобы закончить удаление сообщения из очереди, необходимо также вызвать **deleteMessage.** Этот двухэтапный процесс удаления сообщения позволяет удостовериться, что если коду не удастся обработать сообщение из-за сбоя оборудования или программного обеспечения, другой экземпляр кода сможет получить то же сообщение и повторить попытку. Ваш код вызывает **deleteMessage** сразу после обработки сообщения.
...
В следующем примере кода метод **retrieveMessages** используется для получения 20 сообщений в одном вызове. Затем он обрабатывает каждое сообщение, используя **для** цикла. Пример также задает время ожидания невидимости в размере пяти минут (300 секунд) для каждого сообщения. Пять минут начинаются для всех сообщений в то же время, поэтому, когда пять минут прошло с момента вызова для **получения сообщений**, любые сообщения, которые не были удалены станет видимым снова.
...
* [SDK для хранения данных Azure для Java][Azure Storage SDK for Java]
