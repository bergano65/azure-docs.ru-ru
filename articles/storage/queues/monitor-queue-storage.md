---
title: Мониторинг хранилища очередей Azure | Документация Майкрософт
...
Замените `<storage-service-resource--id>` заполнитель в этом фрагменте идентификатором ресурса службы очередей. ИД ресурса можно найти на портале Azure, открыв страницу **свойств** учетной записи хранения.
...
Замените `<storage-service-resource--id>` заполнитель в этом фрагменте на службу хранилища очереди идентификаторов ресурсов. ИД ресурса можно найти на портале Azure, открыв страницу **свойств** учетной записи хранения.
...
Доступ к журналам ресурсов можно получить либо в виде очереди в учетной записи хранения, либо в виде данных события, либо с помощью запросов аналитики журнала.
...
Записи журнала создаются только при получении запроса к конечной точке службы. Например, если учетная запись хранения имеет действие в конечной точке очереди, но не в ее таблице или конечных точках BLOB-объектов, то создаются только журналы, относящиеся к службе очередей. Журналы службы хранилища Azure содержат подробные сведения об успешных и неудачных запросах к службе хранилища. Эта информация может использоваться для мониторинга отдельных запросов и диагностики неполадок в службе хранилища. Запросы вносятся в журнал наилучшим возможным образом.
...
Дополнительные сведения см. в статье [Начало работы с Log Analytics в Azure Monitor](../../azure-monitor/log-query/log-analytics-tutorial.md).

Данные хранятся в таблице **сторажекуеуелогс** .

#### <a name="sample-kusto-queries"></a>Примеры запросов Kusto

Ниже приведены некоторые запросы, которые можно ввести на панели **поиска журналов** , чтобы упростить мониторинг хранилища очередей. Эти запросы поддерживают [новый язык](../../azure-monitor/log-query/log-query-overview.md).
