---
title: Как использовать хранилище очередей от Ruby - Azure Storage
...
Лазурный модуль будет считывать переменные среды в среде **АЗURE\_\_STORAGE ACCOUNT** и **АЗURE\_STORAGE\_ACCESS_KEY** информации, необходимой для подключения к учетной записи хранения Azure. Если эти переменные среды не заданы, необходимо указать сведения об учетной записи перед использованием объекта **Azure::QueueService** с помощью следующего кода:
...
Вы можете просмотреть сообщение в начале очереди, не удаляя его из нее, вызвав метод **peek\_messages()**. По умолчанию, **заглянуть\_сообщения ()** заглядывает в одно сообщение. Вы также можете указать количество сообщений для просмотра.
...
2. Чтобы закончить удаление сообщения из очереди, необходимо также позвонить **delete_message()**.

Этот двухэтапный процесс удаления сообщения позволяет удостовериться, что если коду не удастся обработать сообщение из-за сбоя оборудования или программного обеспечения, другой экземпляр кода сможет получить то же сообщение и повторить попытку. Ваш код вызывает **удаление\_сообщения ()** сразу после обработки сообщения.
...
* Посетите [SDK Azure для](https://github.com/WindowsAzure/azure-sdk-for-ruby) репозитория Ruby на GitHub
