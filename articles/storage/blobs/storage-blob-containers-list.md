...
Если ваша учетная запись хранения содержит более 5000 контейнеров или указано значение `maxresults` таким, что операция перечисления возвращает подмножество контейнеров в учетной записи хранения, служба хранилища Azure возвращает *токен продолжения* со списком контейнеров. Маркер продолжения — это непрозрачное значение, которое можно использовать для получения следующего набора результатов из службы хранилища Azure.
