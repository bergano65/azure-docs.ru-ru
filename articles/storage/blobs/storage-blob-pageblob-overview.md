...
Основные возможности страничных BLOB-объектов Azure: интерфейс REST, устойчивость базового хранилища и возможности эффективной миграции в Azure. Подробнее эти возможности рассматриваются в следующем разделе. Кроме того, страничные BLOB-объекты Azure сейчас поддерживаются в хранилищах двух типов: класса Premium и Standard. Хранилище уровня "премиум" предназначена для рабочих нагрузок, которым требуется согласованная высокая производительность и низкой задержкой, поэтому уровня "премиум" страничных BLOB-объектов идеально подходит для сценариев высокой производительности хранилища. Учетные записи хранилища класса Standard являются более экономически эффективным для выполнения рабочих нагрузок задержками.
...
* Управление добавочными моментальными снимками, направляемыми приложением. С помощью моментальных снимков страничного BLOB-объекта и REST API приложения могут сохранять контрольные точки приложения без дорогостоящего дублирования данных. Служба хранилища Azure поддерживает локальные моментальные снимки для страничных BLOB-объектов, для которых не требуется копирование всего BLOB-объекта. Общедоступные API-интерфейсы этих моментальных снимков также обеспечивают доступ к разностным данным между моментальными снимками и их копирование.
* Динамическая миграция приложения и данных из локальной среды в облако: Скопируйте данные в локальной среде и использовать интерфейсы REST API для записи непосредственно в большой двоичный объект страницы Azure при локальном каталоге, виртуальная машина продолжает работать. После этого вы можете быстро выполнить отработку отказа на виртуальную машину Azure с использованием этих данных. Таким образом можно перенести виртуальные машины, и виртуальные диски из локальной среды в облако с минимальным временем простоя, так как перенос данных происходит в фоновом режиме, а вы продолжаете использовать виртуальную Машину. время простоя, необходимое для отработки отказа, составит (в минутах).
...
Чтобы изменить размер страничного BLOB-объекта после ее создания, используйте [изменение размера](/dotnet/api/microsoft.azure.storage.blob.cloudpageblob.resize) метод. Запрошенный размер должен быть кратен 512 байтам.
...
На рисунке показана операция чтения со смещением 256 и размером диапазона диапазона 4352. Возвращенные данные выделены оранжевым цветом. Возвращение нулей для страниц NUL.
...
Помимо разнообразных интерфейсов REST API страничных BLOB-объектов также предоставляют общий доступ, устойчивость и повышенную безопасность. Далее мы рассмотрим эти преимущества подробнее. 
...
Хранилища класса Standard и Premium — это надежные хранилища, в которых данные страничных BLOB-объектов всегда реплицируются для обеспечения устойчивости и высокого уровня доступности. Дополнительные сведения об избыточности службы хранилища Azure см. в этой [документации](../common/storage-redundancy.md). Azure постоянно доставлялись надежность корпоративного уровня дисков IaaS и страничных BLOB-объектов, с ведущим в отрасли нулевым [показателем процента сбоев в](https://en.wikipedia.org/wiki/Annualized_failure_rate).
