...
Чтобы приступить к разработке с использованием страничных BLOB-объектов, см. статью [Краткое руководство по передаче, скачиванию и составлению списка больших двоичных объектов с помощью .NET](./storage-quickstart-blobs-dotnet.md). Например, рассмотрим получение доступа к страничным BLOB-объектам с помощью клиентской библиотеки хранилища для .NET. 
...
Чтобы изменить размер страничного BLOB-объекта после создания, используйте метод [изменения размера](/dotnet/api/azure.storage.blobs.specialized.pageblobclient.resize) . Запрошенный размер должен быть кратен 512 байтам.
