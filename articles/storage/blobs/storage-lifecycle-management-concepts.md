...
| Имя параметра | Тип параметра | Примечания | Обязательный |
|----------------|----------------|-------|----------|
...
| блобиндексматч | Массив значений словаря, состоящий из ключа тега индекса больших двоичных объектов и условий значения для сопоставления. Каждое правило может определять до 10 условий тега индекса больших двоичных объектов. Например, если вы хотите сопоставить все большие двоичные объекты `Project = Contoso` с `https://myaccount.blob.core.windows.net/` для правила, блобиндексматч имеет `{"name": "Project","op": "==","value": "Contoso"}`значение. | Если не определить Блобиндексматч, правило применяется ко всем BLOB-объектам в учетной записи хранения. | нет |
...
Некоторые данные должны быть просрочены только в том случае, если они явно помечены для удаления. Политику управления жизненным циклом можно настроить для истечения срока действия данных, помеченных атрибутами ключ/значение индекса больших двоичных объектов. В следующем примере показана политика, которая удаляет все блочные BLOB `Project = Contoso`-объекты, помеченные. Дополнительные сведения об индексе больших двоичных объектов см. в статье [Управление и поиск данных в хранилище BLOB-объектов Azure с помощью индекса больших двоичных объектов (Предварительная версия)](storage-manage-find-blobs.md).
