...
|----------------|----------------|-------|----------|
| name           | Строка |Имя правила может содержать до 256 буквенно-цифровые символы. В именах правил учитывается регистр.  Имя должно быть уникальным в пределах политики. | Да |
| Включено | Логическое | Необязательное логическое значение, разрешающее правило для временного отключена. Значение по умолчанию имеет значение true, если оно не задано. | False | 
| Тип           | Значение перечисления | Текущий тип допустимым является `Lifecycle`. | Да |
