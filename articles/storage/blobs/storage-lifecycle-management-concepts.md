...
Компонент управления жизненным циклом доступна во всех глобальных регионах Azure и Azure для государственных организаций.
...
|----------------|----------------|-------|----------|
| `name`         | Строка, |Имя правила может содержать до 256 буквенно-цифровые символы. В именах правил учитывается регистр.  Имя должно быть уникальным в пределах политики. | True |
| `enabled`      | Логическое | Необязательное логическое значение, разрешающее правило для временного отключена. Значение по умолчанию имеет значение true, если оно не задано. | False | 
...
**Я вручную восстановлен архивные большой двоичный объект, как исключить ее перемещение обратно на архивном уровне временно?**  
При перемещении большого двоичного объекта из уровня доступа одного на другой уровень доступа, не изменяет время последнего изменения. Если вы вручную повторно заполнить большой двоичный объект архивные "Горячий" уровень, он будет возвращены для архивирования уровня подсистемой управления жизненным циклом. Ее можно избежать, отключив правилом, которое временно влияет на этот большой двоичный объект. Большой двоичный объект можно скопировать в другое место, если он должен находиться в "Горячий" уровень без возможности восстановления. Чтобы включить правило, большой двоичный объект можно безопасно переместить обратно для архивирования уровня. 
