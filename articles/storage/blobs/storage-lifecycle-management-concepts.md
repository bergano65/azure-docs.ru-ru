...
| Имя параметра | Тип параметра | Примечания | обязательные |
|----------------|----------------|-------|----------|
...
| `enabled`      | Логическое значение. | Необязательное логическое значение, которое позволяет временно отключить правило. Значение по умолчанию — true, если оно не задано. | Ложь | 
...
|-------------|-------------|-------|-------------|
| blobTypes   | Массив предустановленных значений перечисления. | Текущий выпуск поддерживает `blockBlob`. | Yes |
