...
|----------------|----------------|-------|----------|
| `name`         | Строковый тип |Имя правила может включать до 256 буквенно-цифровых символов. В именах правил учитывается регистр. Имя должно быть уникальным в пределах политики. | True |
...
| енаблеаутотиертохотфромкул | Поддержка больших двоичных объектов в настоящее время на изящном уровне        | Не поддерживается |
| tierToArchive               | Поддержка BLOB-объектов, размещенных на горячем или холодном уровне доступа | Не поддерживается |
| "Удалить"                      | Поддерживается для `blockBlob` и `appendBlob`  | Поддерживается     |
