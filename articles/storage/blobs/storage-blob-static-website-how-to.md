...
3. Включить размещение статических веб-сайтов.
   ...
   * Замените `<error-document-name>` заполнитель именем документа ошибки, который будет отображаться пользователям, когда браузер запрашивает страницу несуществующего сайта.

   * Замените `<index-document-name>` заполнитель именем документа индекса. Этот документ обычно имеет формат index. HTML.
...
3. Нажмите кнопку **Отправить** , чтобы отправить файлы.
   ...
   ![Отправка файлов](media/storage-blob-static-website/storage-blob-static-website-upload.png)

4. Если вы хотите, чтобы браузер отображал содержимое файла, убедитесь, что его тип содержимого имеет значение `text/html`. 

   ![Проверка типов содержимого](media/storage-blob-static-website/storage-blob-static-website-content-type.png)

   >[!NOTE]
   > Обозреватель службы хранилища автоматически присваивает этому свойству `text/html` значение для наиболее распознаваемых расширений `.html`, таких как. Однако в некоторых случаях это необходимо сделать самостоятельно. Если не задать для `text/html`этого свойства значение, браузер предложит пользователям загрузить файл, а не подготовит его к просмотру. Чтобы задать это свойство, щелкните правой кнопкой мыши файл и выберите пункт **Свойства**.

### <a name="azure-cli"></a>[Azure CLI](#tab/azure-cli)

Передача объектов в контейнер *$Web* из исходного каталога.

> [!NOTE]
> Если вы используете Azure Cloud Shell, обязательно добавьте `\` escape-символ при ссылке на `$web` контейнер (например, `\$web`). Если вы используете локальную установку Azure CLI, вам не придется использовать escape-символ.
...
> [!NOTE] 
> Если браузер запрашивает у пользователей команду на загрузку файла вместо отображения содержимого, можно добавить `--content-type 'text/html; charset=utf-8'` к команде. 

* Замените значение заполнителя `<storage-account-name>` именем вашей учетной записи хранения.

* Замените `<source-path>` заполнитель путем к расположению файлов, которые требуется передать.
...
> [!NOTE] 
> Если браузер запрашивает у пользователей команду на загрузку файла вместо отображения содержимого, можно добавить `-Properties @{ ContentType = "text/html; charset=utf-8";}` к команде.

* Замените значение `<path-to-file>` заполнителя полным путем к файлу, который требуется передать (например, `C:\temp\index.html`).

* Замените значение `<blob-name>` заполнителя именем, которое нужно присвоить результирующему большому двоичному объекту (например `index.html`:).
...
1. Щелкните **метрики** в разделе **монитор** в меню учетной записи хранения.
...
2. Нажмите кнопку времени, выберите интервал времени и нажмите кнопку **Применить**.
...
7. Установите флажок рядом с **жетвебконтент** в селекторе *значений* , чтобы заполнить отчет метрики.
   ...
   >[!NOTE]
   > Флажок **жетвебконтент** отображается только в том случае, если этот элемент API использовался в течение заданного промежутка времени. На портале отображаются только элементы API, используемые в течение заданного промежутка времени, чтобы сосредоточиться только на тех элементах, которые возвращают данные. Если вы не можете найти конкретный член API в этом списке, разверните интервал времени.
