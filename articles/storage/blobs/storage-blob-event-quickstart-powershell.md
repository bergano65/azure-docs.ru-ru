...
---
# <a name="quickstart-route-storage-events-to-web-endpoint-with-powershell"></a>Быстрый запуск: События хранения маршрута в конечную точку с PowerShell
...
Для работы с этой статьей требуется последняя версия Azure PowerShell. Если вам нужно установить или обновить, [см. Установить и настроить Azure PowerShell.](/powershell/azure/install-Az-ps)
...
Вы подписываетесь на тему, чтобы сообщить Event Grid, какие события вы хотите отслеживать. Следующий пример подписывается на созданную учетную запись хранения и передает URL-адрес из веб-приложения в качестве конечной точки для уведомления о событии. Конечная точка веб-приложения должна содержать суффикс `/api/updates/`.
