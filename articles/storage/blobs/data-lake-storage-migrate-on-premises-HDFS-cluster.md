...
4. Задайте для переменной `azjars` оболочки расположение `hadoop-azure` JAR-файлов и `azure-storage` . Эти файлы можно найти в каталоге установки Hadoop.

    Чтобы определить, существуют ли эти файлы, используйте следующую команду: `ls -l $<hadoop_install_dir>/share/hadoop/tools/lib/ | grep azure`. `<hadoop_install_dir>` Замените заполнитель на путь к каталогу, в котором установлен Hadoop. Обязательно используйте полные пути.
...
5. Создайте контейнер хранилища, который будет использоваться для копирования данных. Также следует указать целевой каталог в составе этой команды. На этом этапе это может быть фиктивный каталог назначения.
    ...
    * `<blob_service_endpoint>` Замените заполнитель именем конечной точки службы BLOB-объектов.

    * `<account_key>` Замените заполнитель ключом доступа вашей учетной записи.

    * `<container-name>` Замените заполнитель именем своего контейнера.

    * `<destination_directory>` Замените заполнитель именем каталога, в который необходимо скопировать данные.
...
7. Скопируйте данные из хранилища HDFS в Data Box хранилище BLOB-объектов в созданный ранее контейнер. Если каталог, в который выполняется копирование, не найден, команда автоматически создаст его.
    ...
    * `<exlusion_filelist_file>` Замените заполнитель именем файла, который содержит список исключений файлов.

    * `<source_directory>` Замените заполнитель именем каталога, содержащего данные, которые необходимо скопировать.

    * `<destination_directory>` Замените заполнитель именем каталога, в который необходимо скопировать данные.

    Параметр используется, чтобы `hadoop-azure*.jar` сделать и зависимые `azure-storage*.jar` файлы доступными для `distcp`. `-libjars` Это может быть уже для некоторых кластеров.
...
* Чтобы использовать кластер Hadoop на основе Azure, выполните следующую команду DistCp:
    ...
    * Замените заполнители `<dest_account>`иименами исходной и целевой учетных записей хранения. `<source_account>`

    * Замените заполнители `<dest_container>`иименами исходного и целевого контейнеров. `<source_container>`

    * Замените заполнители `<dest_path>`ипутями исходного и конечного каталогов. `<source_path>`

    * `<source_account_key>` Замените заполнитель ключом доступа учетной записи хранения, содержащей данные.
...
Сведения о создании субъекта-службы [см. в разделе как Создание приложения Azure Active Directory и субъект-службы с доступом к ресурсам с помощью портала](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal).
...
* При выполнении действий, описанных в разделе [Получение значений для входа](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal#get-values-for-signing-in) статьи, идентификатор приложения и значения секрета клиента в текстовый файл. Они вам скоро понадобятся.
...
1. `copy-acls.py` Скачайте скрипт. См. раздел [Загрузка вспомогательных сценариев и настройка пограничных узлов для их запуска в](#download-helper-scripts) этой статье.
...
* Замените заполнители `<client-secret>`ина идентификатор приложения и секрет клиента, собранные при создании субъекта-службы. `<application-id>`
