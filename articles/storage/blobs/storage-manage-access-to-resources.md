...
* **Полный общий доступ для чтения.** Все данные контейнера и BLOB-объектов могут считываться с помощью анонимного запроса. Клиенты могут перечислять большие двоичные объекты внутри контейнера с помощью анонимного запроса, но не могут перечислять контейнеры в учетной записи хранения.
