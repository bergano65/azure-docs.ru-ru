...
Можно настроить пользовательский домен для доступа к BLOB-данным в учетной записи хранения Azure. Конечной точкой по умолчанию для хранилища BLOB-объектов является *\<storage-account-name>.blob.core.windows.net*. Можно также использовать конечную точку веб-сайта, которая создается как часть [компонента статических веб-сайтов](storage-blob-static-website.md). При сопоставлении личного домена и поддомена, например *www\.contoso.com*, с двоичным объектом или веб-конечной точкой вашей учетной записи хранения, пользователи могут использовать этот домен для доступа к данным большого двоичного объекта в вашей учетной записи хранения.
...
| Корневой контейнер | http:\//mystorageaccount.BLOB.Core.Windows.NET/myblob или http:\//mystorageaccount.BLOB.Core.Windows.NET/$root/миблоб | http:\//www.contoso.com/myblob или http:\//www.contoso.com/$root/миблоб |
| Интернет |  http:\//мисторажеаккаунт. [ зона]. Web. Core. Windows. NET/$Web/[индексдок] или http:\//мисторажеаккаунт. [ зона]. Web. Core. Windows. NET/[индексдок] или http:\//мисторажеаккаунт. [ зона]. Web. Core. Windows. NET/$Web или http:\//мисторажеаккаунт. [ зона]. Web. Core. Windows. NET/ | http:\//www.contoso.com/$Web или http:\/\//www.contoso.com/или http:/www.contoso.com/$Web/[индексдок] или http:\//ВВВ.Контосо.ком/[индексдок] |
...
Вы можете указать для личного домена префикс поддомена (например, www\.contoso.com) с конечной точкой большого двоичного объекта для вашей учетной записи хранения одним из двух способов: 
...
Первым и самым простым методом является создание записи канонического имени (CNAME), сопоставляющей личный домен и поддомен непосредственно с конечной точкой больших двоичных объектов. Запись CNAME — это компонент службы доменных имен (DNS), позволяющий сопоставить исходный и конечный домены. В нашем примере исходный домен — это собственный пользовательский домен и поддомен (например,*www\.contoso.com*). Конечный домен является конечной точкой службы BLOB-объектов (например, *mystorageaccount.blob.core.windows.net*).
...
1. Создайте новую запись CNAME, введите псевдоним дочернего домена, например **www** или **Photos** (поддомен является обязательным, корневые домены не поддерживаются), а затем укажите имя узла.  
...
1. В области **Личный домен** в текстовом поле введите имя личного домена, включая поддомен.  
   Например, если доменом является *contoso.com* , а псевдонимом поддоменом является *www*, **введите\.www contoso.com**. Если используется поддомен *photos*, введите **photos.contoso.com**.
...
1. В области **Личный домен** в текстовом поле введите имя личного домена, включая поддомен.  
   Не включайте *asverify*. Например, если доменом является *contoso.com* , а псевдонимом поддоменом является *www*, **введите\.www contoso.com**. Если используется поддомен *photos*, введите **photos.contoso.com**.
