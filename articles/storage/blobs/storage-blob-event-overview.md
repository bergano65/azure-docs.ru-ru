...
События хранения Azure позволяют приложениям реагировать на события, такие как создание и удаление капли. При этом не требуется сложный код или дорогостоящие и неэффективные службы опроса.

События перемещаются с помощью [Azure Event Grid](https://azure.microsoft.com/services/event-grid/) для подписчиков, таких как Azure Functions, Azure Logic Apps или даже для вашего собственного http-слушателя. Самая лучшая часть вы только оплачиваете для чего вы используете.

Хранилище Blob отправляет события в Event Grid, которая обеспечивает надежную доставку событий вашим приложениям с помощью богатых политик повторений и мертвых букв.
...
Если вы хотите попробовать это сейчас, увидеть любой из этих статей quickstart:
