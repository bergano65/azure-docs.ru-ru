...
Вы можете создать области шифрования с помощью ключа, управляемого корпорацией Майкрософт, или с помощью управляемого клиентом ключа, который хранится в Azure Key Vault. Чтобы создать область шифрования с ключом, управляемым клиентом, необходимо сначала создать хранилище ключей Azure и добавить ключ, который предполагается использовать для этой области. В хранилище ключей должны быть включены свойства **обратимого удаления** и **очистки** , и они должны находиться в том же регионе, что и учетная запись хранения. См. статью [Использование ключей, управляемых клиентом, в Azure Key Vault для управления шифрованием службы хранилища Azure](../common/encryption-customer-managed-keys.md).
...
1. Выберите тип шифрования: **ключи, управляемые корпорацией Майкрософт** , или **ключи, управляемые клиентом**.
    ...
    - Если вы выбрали **ключи, управляемые клиентом**, укажите хранилище ключей, ключ и версию ключа, которые будут использоваться для этой области шифрования, как показано на следующем рисунке.
...
Дополнительные сведения о настройке управляемых клиентом ключей с помощью Azure Key Vault для шифрования службы хранилища Azure см. [в статье Настройка ключей, управляемых клиентом, с помощью Azure Key Vault с помощью портал Azure](../common/storage-encryption-keys-portal.md).
...
Чтобы создать новую область шифрования, защищенную управляемыми клиентом ключами с помощью Azure Key Vault, сначала настройте управляемые клиентом ключи для учетной записи хранения. Необходимо назначить управляемое удостоверение учетной записи хранения, а затем использовать управляемое удостоверение, чтобы настроить политику доступа для хранилища ключей, чтобы у учетной записи хранения были разрешения на доступ к ней. Дополнительные сведения см. в статье о [настройке ключей, управляемых клиентом, с помощью Azure Key Vault с помощью PowerShell](../common/storage-encryption-keys-powershell.md).

Чтобы настроить ключи, управляемые клиентом, для использования с областью шифрования, необходимо включить в хранилище ключей свойства **обратимого удаления** и **очистки** . Хранилище ключей должно находиться в том же регионе, что и учетная запись хранения. См. статью [Использование ключей, управляемых клиентом, в Azure Key Vault для управления шифрованием службы хранилища Azure](../common/encryption-customer-managed-keys.md).
...
Чтобы создать новую область шифрования, защищенную управляемыми клиентом ключами с помощью Azure Key Vault, сначала настройте управляемые клиентом ключи для учетной записи хранения. Необходимо назначить управляемое удостоверение учетной записи хранения, а затем использовать управляемое удостоверение, чтобы настроить политику доступа для хранилища ключей, чтобы у учетной записи хранения были разрешения на доступ к ней. Дополнительные сведения см. [в статье Настройка ключей, управляемых клиентом, с использованием Azure Key Vault с помощью Azure CLI](../common/storage-encryption-keys-cli.md).
...
Чтобы получить список всех областей шифрования в группе ресурсов по учетной записи хранения, используйте синтаксис конвейера следующим образом:
...
Можно создать отдельный большой двоичный объект с собственной областью шифрования, если только контейнер не настроен так, чтобы все большие двоичные объекты использовали его область по умолчанию. Чтобы принудительно использовать область по умолчанию контейнера для всех больших двоичных объектов в контейнере, задайте `-PreventEncryptionScopeOverride` для параметра значение `true` .
...
Чтобы создать контейнер с областью шифрования по умолчанию с Azure CLI, вызовите команду [AZ Storage Container Create](/cli/azure/storage/container#az-storage-container-create) , указав область для `--default-encryption-scope` параметра. Можно создать отдельный большой двоичный объект с собственной областью шифрования, если только контейнер не настроен так, чтобы все большие двоичные объекты использовали его область по умолчанию. Чтобы принудительно использовать область по умолчанию контейнера для всех больших двоичных объектов в контейнере, задайте `--prevent-encryption-scope-override` для параметра значение `true` .
...
Чтобы изменить ключ, защищающий область шифрования от ключа, управляемого Майкрософт, к ключу, управляемому клиентом, сначала убедитесь, что вы включили ключи, управляемые клиентом, с Azure Key Vault для учетной записи хранения. Дополнительные сведения см. в статье о [настройке ключей, управляемых клиентом, с помощью Azure Key Vault с помощью PowerShell](../common/storage-encryption-keys-powershell.md). Затем вызовите команду **Update-азсторажеенкриптионскопе** и передайте `-KeyUri` `-KeyvaultEncryption` Параметры и:
...
Чтобы изменить ключ, защищающий область шифрования от ключа, управляемого Майкрософт, к ключу, управляемому клиентом, сначала убедитесь, что вы включили ключи, управляемые клиентом, с Azure Key Vault для учетной записи хранения. Дополнительные сведения см. [в статье Настройка ключей, управляемых клиентом, с использованием Azure Key Vault с помощью Azure CLI](../common/storage-encryption-keys-cli.md). Затем вызовите команду **AZ Storage Account Encryption-Scope Update** , передайте `--key-uri` параметр и передайте `--key-source` параметр со значением `Microsoft.KeyVault` :
...
Чтобы отключить область шифрования с помощью PowerShell, вызовите команду Update-Азсторажеенкриптионскопе и включите `-State` параметр со значением `disabled` , как показано в следующем примере. Чтобы повторно включить область шифрования, вызовите ту же команду с `-State` параметром, для которого задано значение `enabled` . Не забудьте заменить значения заполнителей в примере собственными значениями:
