...
* **Шаг 1: Определите общую память, доступную для очереди приложения YARN 'по умолчанию'** - Первый шаг состоит в том, чтобы определить память, доступную для очереди приложения YARN по умолчанию. Эта информация доступна на портале Ambari, связанном с кластером. Перейдите к YARN и откройте вкладку конфигураций, чтобы определить объем памяти YARN, доступный для очереди приложения по умолчанию. Это общая доступная память для задания DistCp (которое фактически является заданием MapReduce).

* **Шаг 2. Расчет количества модулей сопоставления.** Чтобы узнать значение **m**, разделите целую часть общего объема памяти YARN на размер контейнера YARN. Сведения о размере контейнера YARN также доступны на портале Ambari. Перейдите к YARN и просмотрите вкладку Configs. Размер контейнера YARN отображается в этом окне. Уравнение для получения количества модулей сопоставления (**m**):
...
* **Количество картографов**: С портала Ambari вы определяете, что размер контейнера YARN составляет 3072 МБ для кластерного узла D14. Таким образом, число модулей сопоставления равно:
