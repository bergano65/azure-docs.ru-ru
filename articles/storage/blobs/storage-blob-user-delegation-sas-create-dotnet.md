...
Команда @no__t – 0 возвращает список свойств субъекта-службы в формате JSON. Скопируйте эти значения, чтобы их можно было использовать для создания необходимых переменных среды на следующем шаге.
...
Добавьте следующие директивы `using` в код, чтобы использовать предварительные версии клиентских библиотек Azure Identity и Azure Storage.
...
Чтобы проверить подлинность субъекта-службы, создайте экземпляр класса [дефаултазурекредентиал](/dotnet/api/azure.identity.defaultazurecredential) . Конструктор `DefaultAzureCredential` считывает ранее созданные переменные среды.
