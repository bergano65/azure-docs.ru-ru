...
Функциональные возможности хранения на **рис. 1** обеспечивает Data Lake Storage 1-го поколения (**рис. 2**). Обратите внимание на то, как различные компоненты потока данных взаимодействуют с Data Lake Storage 1-го поколения с помощью REST API или пакета SDK для Java. Также обратите внимание, как сквозные функциональные компоненты взаимодействуют с Data Lake Storage 1-го поколения. Средство подготовки использует шаблоны ресурсов Azure, в то время как компонент мониторинга, которая использует журналы Azure Monitor использует рабочие данные, поступающие от Gen1 хранилища Озера данных.
...
В этой таблице описаны наборы API, доступные для пользовательских приложений. Чтобы упростить немного подробнее, мы запятыми эти наборы API в два типа: управления API и файл API системы.

API-интерфейсы управления позволяют управлять учетными записями, при API-интерфейсы позволяют работать с файлов и папок в файловой системе.
...
| Java SDK — файловая система                  | [Ссылка](https://docs.microsoft.com/java/api/overview/azure/datalake)                                                                                                                                                                                                                                         | *Пока недоступно*                                                | *Пока недоступно*                                                                                                                                             |
...
| PowerShell — управление и файловой системы | [Ссылка](https://docs.microsoft.com/powershell/module/az.datalakestore)                                                                                                                                                                                                                        | Управление — не поддерживается файловая система — *еще не доступна*        | Управление. *Доступно сейчас*. [Ссылка](https://docs.microsoft.com/powershell/module/az.storage) <br><br>Файловая система: *еще не доступна* |
...
| Интерфейс командной строки — файловая система                       | [Ссылка](https://docs.microsoft.com/cli/azure/dls/fs?view=azure-cli-latest)                                                                                                                                                                                                                                               | *Пока недоступно*                                                | *Пока недоступно*                                                                                                                                             |
...
|------------------------------------|-----------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Перенос по методу lift-and-shift**                 | [Фабрика данных Azure](https://docs.microsoft.com/azure/data-factory/load-azure-data-lake-storage-gen2-from-gen1) | Управляемая облачная служба                                                                                                                | Списки управления доступом и данные можно скопировать на данный момент.                                                                                                                                                                                                                                                                      |
...
Основная задача здесь — для преобразования URI, с префиксами `adl://` в URI, имеющих `abfss://` префикс.
...
Описывается схема URI для доступа к файлам Gen2 хранилища Озера данных [здесь](../../hdinsight/hdinsight-hadoop-use-data-lake-storage-gen2.md) в подробности, но вообще говоря, это `abfss://<FILE_SYSTEM_NAME>@<ACCOUNT_NAME>.dfs.core.widows.net/<PATH>`.
...
После завершения переноса заключительные действия включают в себя тщательную проверку. Это может включать в том числе для проверки данных будет скопирован на надежно, проверка ACL имеет правильное значение, проверка конвейеры end-to-end функционируют правильно д. После выполнения проверок можно отключить старые конвейеры, удалить исходные учетные записи Data Lake Storage 1-го поколения и полностью перейти к работе с решениями на основе Data Lake Storage 2-го поколения.
