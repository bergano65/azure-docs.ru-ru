...
> [!NOTE]
> Веб-канал изменений находится в общедоступной предварительной версии и доступен в регионах **westcentralus** и **westus2** . См. раздел " [условия](#conditions) " этой статьи. Чтобы зарегистрироваться в предварительной версии, ознакомьтесь с разделом [регистрация подписки](#register) в этой статье.
...
Поддержка веб-канала изменений хорошо подходит для сценариев, обрабатывающих данные на основе измененных объектов. Например, приложения могут:
  ...
  - Обновите вторичный индекс, синхронизируйте его с кэшем, поисковым модулем или любым другим сценарием управления содержимым.
  ...
  - Создавайте подключенные конвейеры приложений, реагирующие на события изменения или Запланируйте выполнение на основе созданного или измененного объекта.

> [!NOTE]
> [События хранилища BLOB-объектов](storage-blob-event-overview.md) предоставляют одноразовые события в реальном времени, которые позволяют функциям или приложениям Azure реагировать на изменения, происходящие в большом двоичном объекте. Веб-канал изменений предоставляет устойчивую упорядоченную модель журнала изменений. Изменения в веб-канале изменений становятся доступными в веб-канале изменений в течение нескольких минут после изменения. Если приложение должно реагировать на события гораздо быстрее, попробуйте использовать [события хранилища BLOB-объектов](storage-blob-event-overview.md) . События хранилища BLOB-объектов позволяют функциям или приложениям Azure реагировать на отдельные события в режиме реального времени.

## <a name="enabling-and-disabling-the-change-feed"></a>Включение и отключение веб-канала изменений

Чтобы начать запись изменений, необходимо включить веб-канал изменений. Отключите веб-канал изменений для отмены записи изменений. Вы можете включать и отключать изменения с помощью шаблонов Azure Resource Manager на портале или PowerShell.
...
3. Выберите **шаблоны развертывания**, выберите **создать**, а затем — создать **собственный шаблон в редакторе**.

5. В редакторе шаблонов вставьте следующий код JSON. Замените заполнитель `<accountName>` именем вашей учетной записи хранения.
...
```
4. Нажмите кнопку **сохранить** , укажите группу ресурсов учетной записи, а затем нажмите кнопку **приобрести** , чтобы включить веб-канал изменений.
...
- В каждой учетной записи хранения имеется только один канал изменений для службы BLOB-объектов. 

- Изменения фиксируются только на уровне службы BLOB-объектов.
...
- Учетные записи с иерархическим пространством имен не поддерживаются.

## <a name="consuming-the-change-feed"></a>Использование веб-канала изменений
...
>[!NOTE]
> В текущем выпуске контейнер **$blobchangefeed** не отображается в Обозреватель службы хранилища или портал Azure. 

Клиентские приложения могут использовать веб-канал изменений с помощью библиотеки обработчика канала изменения больших двоичных объектов, предоставляемой вместе с пакетом SDK. 

См. статью [Обработка журналов веб-канала изменений в хранилище BLOB-объектов Azure](storage-blob-change-feed-how-to.md).

## <a name="understanding-change-feed-organization"></a>Общие сведения об организации веб-канала изменений

<a id="segment-index"></a>

### <a name="segments"></a>Сегменты

Веб-канал изменений — это журнал изменений, которые организованы в **почасовые** *сегменты* (см. [спецификации](#specifications)). Это позволяет клиентскому приложению использовать изменения, происходящие в определенные диапазоны времени, без необходимости выполнять поиск по всему журналу.

Доступный ежечасный сегмент веб-канала изменений описывается в файле манифеста, который указывает пути к файлам веб-канала изменений для этого сегмента. В списке `$blobchangefeed/idx/segments/` виртуального каталога показаны сегменты, упорядоченные по времени. Путь сегмента описывает начало почасового диапазона времени, который представляет сегмент. (См. [спецификации](#specifications)). Этот список можно использовать для фильтрации сегментов интересующих вас журналов.
...
> [!NOTE]
> `$blobchangefeed/idx/segments/1601/01/01/0000/meta.json` автоматически создается при включении веб-канала изменений. Этот файл можно спокойно проигнорировать. Он всегда пуст. 
...
> [!NOTE]
> Если вы перечислите контейнеры в своей учетной записи хранения, контейнер `$blobchangefeed` появится только после включения функции веб-канала изменений в учетной записи. После включения веб-канала изменений необходимо подождать несколько минут, прежде чем можно будет увидеть контейнер.
...
- Измените записи событий, в которых `eventType` имеет значение `Control` являются внутренними системными записями и не отображают изменения объектов в вашей учетной записи. Их следует игнорировать.

- Значения в контейнере свойств `storageDiagnonstics` предназначены только для внутреннего использования и не предназначены для использования приложением. Приложения не должны иметь контрактную зависимость от этих данных.
...
- Веб-канал изменений фиксирует только операции создания, обновления, удаления и копирования.
- Изменения записей событий для любого отдельного изменения могут появиться в веб-канале изменений несколько раз.
- Вы пока не можете управлять временем существования файлов журнала веб-канала изменений, задав для них политику хранения на основе времени.
- Свойство `url` файла журнала всегда пусто.
