...
> [!NOTE]
> [События хранилища BLOB-объектов](storage-blob-event-overview.md) предоставляют одноразовые события в реальном времени, которые позволяют функциям или приложениям Azure реагировать на изменения, происходящие в большом двоичном объекте. Веб-канал изменений предоставляет устойчивую упорядоченную модель журнала изменений. Изменения в вашем веб-канале изменений становятся доступными в веб-канале изменений в течение нескольких минут после изменения. Если приложение должно реагировать на события гораздо быстрее, попробуйте использовать [события хранилища BLOB-объектов](storage-blob-event-overview.md) . События хранилища BLOB-объектов позволяют функциям или приложениям Azure реагировать на отдельные события в режиме реального времени.

## <a name="enable-and-disable-the-change-feed"></a>Включение и отключение веб-канала изменений

Чтобы начать сбор изменений, необходимо включить веб-канал изменений в учетной записи хранения. Отключите веб-канал изменений для отмены записи изменений. Вы можете включать и отключать изменения с помощью шаблонов Azure Resource Manager на портале или PowerShell.

Ниже приведены некоторые моменты, которые следует учитывать при включении веб-канала изменений.

- Для службы BLOB-объектов в каждой учетной записи хранения, хранящейся в контейнере **$blobchangefeed** , существует только один канал изменений.
...
- Включить веб-канал изменений могут только учетные записи хранилища GPv2 и BLOB-объектов. Учетные записи хранения GPv1, учетные записи Premium Блоккблобстораже и учетные записи с включенным иерархическим пространством имен сейчас не поддерживаются.
...
Веб-канал изменений оптимизирован для разработки приложений, так как только успешные события создания, изменения и удаления больших двоичных объектов записываются в журнал веб-канала изменений. Ведение журнала аналитики регистрирует все успешные и неудачные запросы во всех операциях, включая операции чтения и перечисления. Используя веб-канал изменений, не нужно беспокоиться о фильтрации шума журнала при большой учетной записи транзакции и сосредоточиться только на событиях изменения больших двоичных объектов.

### <a name="should-i-use-change-feed-or-storage-events"></a>Следует ли использовать веб-канал изменений или события хранилища?
Вы можете использовать обе функции в качестве веб-канала изменений и [события хранилища BLOB-объектов](storage-blob-event-overview.md) аналогичным образом, а основное различие заключается в задержке, упорядочении и хранении записей событий. Веб-канал изменений записывает записи в журнал веб-канала изменений каждые несколько минут, одновременно обеспечивая порядок операций изменения больших двоичных объектов. События хранилища отправляются в режиме реального времени и могут быть не упорядочены. События веб-канала изменений надежно хранятся в учетной записи хранения, а события хранилища являются временными и потребляются обработчиком событий, если они не сохранены явным образом.
