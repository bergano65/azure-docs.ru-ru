...
2. Убедитесь, что установленная версия PowerShell `5.1` или более поздняя, с помощью следующей команды.    
...
4. Закройте, а затем снова откройте консоль PowerShell.

5. Установите **AZ. Storage,** выполнив предварительную версию модуля.
...
Получите идентификатор клиента, секрет клиента и идентификатор клиента. Для этого см. статью [Получение маркера из Azure AD для авторизации запросов из клиентского приложения](../common/storage-auth-aad-app.md). В рамках этого процесса необходимо присвоить субъекту безопасности одну из следующих ролей [RBAC (Управление доступом на основе ролей)](../../role-based-access-control/overview.md) . 
...
Рекурсивно задайте список ACL с помощью `Set-AzDataLakeGen2AclRecursive` командлета.
...
Настройте список ACL рекурсивно, вызвав метод **даталакедиректориклиент. сетакцессконтролрекурсивеасинк** . Передайте этот метод [List](/dotnet/api/system.collections.generic.list-1) в список [пасакцессконтролитемс](/dotnet/api/azure.storage.files.datalake.models.pathaccesscontrolitem). Каждый [пасакцессконтролитемс](/dotnet/api/azure.storage.files.datalake.models.pathaccesscontrolitem) определяет запись ACL.
...
Настройте список ACL рекурсивно, вызвав метод **даталакедиректориклиент. set_access_control_recursive** .
...
Рекурсивно обновите список ACL, вызвав метод **даталакедиректориклиент. упдатеакцессконтролрекурсивеасинк** . 
...
Рекурсивно обновите список ACL, вызвав метод **даталакедиректориклиент. update_access_control_recursive** . 
...
Удалите записи ACL, вызвав метод **даталакедиректориклиент. ремовеакцессконтролрекурсивеасинк** . 

В этом примере удаляется запись ACL из списка ACL каталога с именем `my-parent-directory` . 
...
Удалите записи ACL, вызвав метод **даталакедиректориклиент. remove_access_control_recursive** . 
