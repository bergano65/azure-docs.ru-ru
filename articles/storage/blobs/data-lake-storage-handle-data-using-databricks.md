...
В рамках этого руководства вы выполните операцию извлечения, преобразования и загрузки данных для их перемещения из учетной записи службы хранилища Azure с Azure Data Lake Storage 2-го поколения в Хранилище данных SQL Azure с помощью Azure Databricks.

На следующем рисунке показан поток в приложении.

![Azure Databricks с Data Lake Storage Gen2 и хранилищем данных SQL](./media/data-lake-storage-handle-data-using-databricks/databricks-extract-transform-load-sql-datawarehouse.png "Azure Databricks с Data Lake Storage Gen2 и хранилищем данных SQL")
...
* Создайте хранилище данных SQL Azure, создайте правило брандмауэра на уровне сервера и подключитесь к серверу с правами администратора. Следуйте указаниям, приведенным здесь — [Краткое руководство. Создание хранилища данных SQL Azure](../../sql-data-warehouse/create-data-warehouse-portal.md).
...
* [Создайте учетную запись Data Lake Storage Gen2 Azure](data-lake-storage-quickstart-create-account.md)
...
2. В левой области выберите **Рабочая область**. В раскрывающемся списке **Рабочая область** выберите **Создать** > **Notebook** (Записная книжка).
    ...
    ![Создание записной книжки в Databricks](./media/data-lake-storage-handle-data-using-databricks/databricks-create-notebook.png "Create notebook in Databricks")
...
3. В диалоговом окне **создания записной книжки** введите имя записной книжки. Выберите **Scala** в качестве языка, а затем выберите созданный ранее кластер Spark.
    ...
    ![Создание записной книжки в Databricks](./media/data-lake-storage-handle-data-using-databricks/databricks-notebook-details.png "Create notebook in Databricks")

    Нажмите кнопку **Создать**.

4. Введите следующий код в первую ячейку и выполните его. Не забудьте заменить значения заполнителей, показанные в примере в скобках, собственными значениями.
...
5. Чтобы выполнить ячейку кода, нажмите клавиши **SHIFT+ВВОД**.
...
> [!NOTE]
> Если у вас еще нет учетной записи Azure Data Lake Storage Gen2, ознакомьтесь с [кратким руководством по ее созданию](./data-lake-storage-quickstart-create-account.md).
...
2. Затем передайте данные примера в учетную запись хранения. Метод, который используется для отправки данных в учетную запись хранения, зависит от того, включено ли иерархическое пространство имен.
    ...
    Если иерархическое пространство имен включено для учетной записи службы хранилища Azure, то для обработки загрузки можно использовать Фабрику данных Azure, distp или AzCopy (версия 10). Сейчас версия AzCopy 10 доступна только в предварительной версии. Чтобы использовать AzCopy, вставьте следующий код в окно командной строки:
...
>[Потоковая передача данных в Azure Databricks при помощи службы "Центры событий"](../../azure-databricks/databricks-stream-from-eventhubs.md)
