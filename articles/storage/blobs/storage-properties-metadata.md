...
>
> Пары имя/значение метаданных являются допустимыми заголовками HTTP и поэтому должен соответствовать всем ограничениям, относящихся к заголовкам HTTP. Имена метаданных должны быть допустимыми именами заголовок HTTP, может содержать только символы ASCII и должны обрабатываться без учета регистра. Значения метаданных, содержащих отличные от ASCII символы должны быть Base64 или URL-адреса.
...
* [Клиентская библиотека хранилища Azure для .NET](/dotnet/api/?term=Microsoft.WindowsAzure.Storage)
* [Клиентская библиотека хранилища Azure для пакета .NET NuGet](https://www.nuget.org/packages/WindowsAzure.Storage/)
