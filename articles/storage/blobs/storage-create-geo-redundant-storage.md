...
4. Заполните форму учетной записи хранения следующей информацией, как показано на изображении ниже, и нажмите кнопку **Создать**:
   ...
   | **Имя учетной записи хранения** | mystorageaccount | Уникальное значение учетной записи хранения. |
   ...
   | **Уровень доступа** | Горячий | Значения "Горячий" достаточно для примера сценария. |
...
Этот пример требует безопасного хранения имени и ключа учетной записи хранения. Храните их в переменных среды локального компьютера, на котором будет выполняться пример. Чтобы создать переменные среды, в зависимости от операционной системы, используйте либо пример для Linux, либо для Windows. 
...
Необходимо также установить требуемые зависимости. Для этого откройте командную строку, перейдите к папке с примерами и введите `npm install`.
...
1. Откройте оболочку и перейдите в папку **storage-blobs-java-v10-tutorial-master** в клонированном каталоге.
