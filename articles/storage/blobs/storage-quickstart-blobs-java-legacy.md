---
title: 'Быстрый запуск: Библиотека хранения данных Azure Blob для Java'
description: Создайте учетную запись хранения и контейнер в хранилище объектов (больших двоичных объектов). Затем используйте библиотеку клиентов Azure Storage для Java, чтобы загрузить каплю в Хранилище Azure, загрузить каплю и перечислить капли в контейнере.
...
ms.locfileid: "76906485"
---
# <a name="quickstart-manage-blobs-with-java-v8-sdk"></a>Быстрый запуск: Управление капли с Java v8 SDK

Из этого краткого руководства вы узнаете, как управлять большими двоичными объектами с использованием Java. Большие двоичные объекты — это объекты, которые могут содержать большие объемы текстовых или двоичных данных, включая изображения, документы, потоковое мультимедиа и архивные данные. Вы будете загружать, загружать и перечислять капли. Вы также будете создавать, устанавливать разрешения и удалять контейнеры.

## <a name="prerequisites"></a>Предварительные требования

- Учетная запись Azure с активной подпиской. [Создайте учетную запись](https://azure.microsoft.com/free/?ref=microsoft.com&utm_source=microsoft.com&utm_medium=docs&utm_campaign=visualstudio) бесплатно.
- Учетная запись хранения Azure. [Создайте учетную запись хранения.](../common/storage-account-create.md)
- IDE, который имеет Maven интеграции. В этом руководстве используется среда разработки [Eclipse](https://www.eclipse.org/downloads/) с конфигурацией для разработчиков Java.
...
Как только проект завершает импорт, откройте **AzureApp.java** (расположен в **blob'sstart.blobAzureApp** внутри `accountname` **src/main/java),** и замените и `accountkey` внутри строки. `storageConnectionString` Затем запустите приложение. Конкретные инструкции по выполнению этих задач описаны в следующих разделах.
...
После проверки файлов нажмите клавишу **Enter,** чтобы завершить демонстрацию и удалить тестовые файлы. Теперь вы знаете, что делает этот пример. Давайте откроем файл **AzureApp.java** и изучим его код.
...
* Создайте экземпляр объекта [CloudStorageAccount,](/java/api/com.microsoft.azure.management.storage.storageaccount) указывающей на учетную запись хранилища.
...
* Создайте экземпляр объекта **CloudBlobClient,** который указывает на [службу Blob](/java/api/com.microsoft.azure.storage.blob._cloud_blob_client) в вашей учетной записи хранения.

    **CloudBlobClient** предоставляет вам точку доступа к службе blob, что позволяет установить и получить доступ к свойствам хранения Blob программно. С помощью **CloudBlobClient** можно создать экземпляр объекта **CloudBlobContainer**, который необходим для создания контейнеров.

* Создайте экземпляр объекта [CloudBlobContainer](/java/api/com.microsoft.azure.storage.blob._cloud_blob_container), представляющий контейнер, к которому осуществляется доступ. Используйте контейнеры для организации больших двоичных объектов аналогично папкам для упорядочения файлов на компьютере.
...
Этот пример использует [CreateIfNotExists,](/java/api/com.microsoft.azure.storage.blob._cloud_blob_container.createifnotexists) потому что мы хотим создать новый контейнер каждый раз, когда образец запущен. В производственной среде, где вы используете один и тот же контейнер в приложении, лучше подать вызов **CreateIfNotExists** только один раз. Вы можете также создать контейнер заранее, чтобы не создавать его в коде.
...
> [Примеры кода Java](https://docs.microsoft.com/java/api/overview/azure/storage?view=azure-java-legacy)
> API[для Java](../common/storage-samples-java.md)
