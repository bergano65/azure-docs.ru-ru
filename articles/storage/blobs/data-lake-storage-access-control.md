...
description: Узнайте, как работает управление доступом в Azure Data Lake Storage 2-го поколения. Поддерживается управление доступом на основе ролей Azure (Azure RBAC) и список ACL, аналогичный стандарту POSIX.
author: normesta
...
Сведения о назначении ролей субъектам безопасности в области действия учетной записи хранения см. в статье [использование портал Azure для назначения роли Azure для доступа к данным BLOB-объектов и очередей](https://docs.microsoft.com/azure/storage/common/storage-auth-aad-rbac-portal?toc=%2fazure%2fstorage%2fblobs%2ftoc.json).

> [!NOTE]
> Гостевой пользователь не может создать назначение роли.
...
Использование назначений ролей Azure является мощным механизмом управления разрешениями на доступ. это очень грубый механизм относительно списков ACL. Наименьшая степень гранулярности для Azure RBAC — на уровне контейнера, и она будет оцениваться с более высоким приоритетом, чем списки ACL. Таким образом, если назначить роль субъекту безопасности в области контейнера, то этот субъект безопасности будет иметь уровень авторизации, связанный с этой ролью, для всех каталогов и файлов в этом контейнере, независимо от назначений ACL.

Когда субъекту безопасности предоставляются разрешения на доступ к данным Azure RBAC через [встроенную роль](https://docs.microsoft.com/azure/storage/common/storage-auth-aad?toc=%2fazure%2fstorage%2fblobs%2ftoc.json#built-in-rbac-roles-for-blobs-and-queues)или пользовательскую роль, эти разрешения оцениваются первыми при авторизации запроса. Если запрошенная операция авторизована назначениями роли Azure субъекта безопасности, авторизация немедленно разрешается и никакие дополнительные проверки списка ACL не выполняются. Кроме того, если у субъекта безопасности нет назначения роли Azure или операция запроса не соответствует назначенному разрешению, то выполняется проверка списка управления доступом, чтобы определить, является ли субъект безопасности полномочным для выполнения запрошенной операции.
...
В случае с общим ключом вызывающий объект эффективно получает доступ "суперпользователя", что означает полный доступ ко всем операциям на всех ресурсах, включая настройку владельца и изменение списков управления доступом.
...
> [!NOTE]
> Списки управления доступом применяются только к субъектам безопасности в одном клиенте. 
...
|PowerShell|[Использование PowerShell для управления каталогами, файлами и списками ACL в Azure Data Lake Storage 2-го поколения](data-lake-storage-directory-file-acl-powershell.md)|
|Azure CLI|[Использование Azure CLI для управления каталогами, файлами и списками ACL в Azure Data Lake Storage 2-го поколения](data-lake-storage-directory-file-acl-cli.md)|
