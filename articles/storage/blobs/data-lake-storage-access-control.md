...
Как правило, ресурсы Azure ограничены ресурсами верхнего уровня (например: Учетные записи хранения Azure). В случае с хранилищем Azure и, следовательно, Azure Data Lake Storage 2-го поколения этот механизм был расширен до ресурса контейнера (файловой системы).
...
Использование назначений ролей RBAC является мощным механизмом управления разрешениями на доступ. это очень грубый механизм относительно списков ACL. Наименьший уровень детализации для RBAC — уровень файловой системы, который оценивается с более высоким приоритетом, чем списки управления доступом. Таким образом, если назначить роль субъекту безопасности в области файловой системы, этот субъект безопасности будет иметь уровень авторизации, связанный с этой ролью, для всех каталогов и файлов в этой файловой системе, независимо от назначений ACL.
...
> [!NOTE]
> Если субъекту безопасности назначено встроенное назначение роли "владелец данных BLOB-объекта хранилища", то субъект безопасности считается *суперпользователем-пользователем* и получает полный доступ ко всем операциям изменения, включая задание владельца каталога или файл, а также списки ACL для каталогов и файлов, для которых они не являются владельцами. Доступ суперпользователя — единственный разрешенный способ изменить владельца ресурса.
...
> [!NOTE]
> Если разрешения предоставляются только с помощью списков управления доступом (без RBAC), то чтобы предоставить субъекту-службе доступ на чтение или запись к файлу, необходимо предоставить субъекту-службе разрешения на **выполнение** этой файловой системы и для каждой папки в иерархии папок, которые привести к файлу.
...
* **Тип 1.** Корневой каталог "/". Этот каталог создается при создании файловой системы Data Lake Storage 2-го поколения. В данном случае группа владельцев закрепляется за пользователем, создавшим файловую систему, если это было сделано с помощью OAuth. Если файловая система создана с использованием общего ключа, SAS учетной записи или SAS службы, то владельцем и группой владельцев присваивается значение **$superuser**.
