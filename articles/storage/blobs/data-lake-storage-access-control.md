...
Как правило, эти ресурсы Azure ограничены по ресурсам верхнего уровня (например: Учетные записи хранения Azure). В случае службы хранилища Azure и, следовательно, Azure Data Lake Storage 2-го поколения, этот механизм расширен до ресурса файловой системы.

Чтобы узнать, как назначить роли для субъектов безопасности в рамках вашей учетной записи хранения, см. в разделе [проверка подлинности доступа к Azure BLOB-объекты и очереди, используя Azure Active Directory](https://docs.microsoft.com/azure/storage/common/storage-auth-aad?toc=%2fazure%2fstorage%2fblobs%2ftoc.json).
...
> [!NOTE]
> Если явно назначенной субъекту безопасности [данные в хранилище BLOB-объектов]() назначение встроенной роли владельца, а затем субъекта безопасности считается *суперпользователь* и предоставляется полный доступ ко все изменения операции, включая задание владельца каталога или файла, а также списки ACL для каталогов и файлов, для которых они не является владельцем. Доступ суперпользователя — единственный разрешенный способ изменить владельца ресурса.
...
|--------|-----------|
|обозреватель хранилищ Azure    |[Установка разрешений на уровне файлов и каталогов в Azure Data Lake Storage 2-го поколения с помощью Обозревателя службы хранилища Azure](https://docs.microsoft.com/azure/storage/blobs/data-lake-storage-how-to-set-permissions-storage-explorer)|
