...
ms.locfileid: "66243591"
---
# <a name="access-control-in-azure-data-lake-storage-gen2"></a>Управление доступом в Azure Data Lake Storage 2-го поколения

Azure Data Lake хранения 2-го поколения реализована модель контроля доступа, поддерживает управление доступом Azure на основе ролей (RBAC) и POSIX по принципу управления доступом (ACL). В этой статье приведены общие сведения о модели управления доступом в Data Lake Storage 2-го поколения.

<a id="azure-role-based-access-control-rbac" />

## <a name="role-based-access-control"></a>Управление доступом на основе ролей

RBAC использует назначения ролей, чтобы эффективно применять наборы разрешений для *субъекты безопасности*. Объект *субъекта безопасности* — это объект, представляющий пользователя, группы, субъекта-службы или управляемого удостоверения, который определен в Azure Active Directory (AD), запрашивает доступ к ресурсам Azure.

Как правило, эти ресурсы Azure ограничены по ресурсам верхнего уровня (например: Учетные записи хранения Azure). В случае хранилища Azure и, следовательно, Gen2 хранилища Озера данных Azure этот механизм была расширена для ресурса контейнера (файловая система).

Чтобы узнать, как назначить роли для субъектов безопасности в рамках вашей учетной записи хранения, см. в разделе [предоставить доступ к BLOB-объектов и очереди данных Azure с помощью RBAC на портале Azure](https://docs.microsoft.com/azure/storage/common/storage-auth-aad-rbac-portal?toc=%2fazure%2fstorage%2fblobs%2ftoc.json).

### <a name="the-impact-of-role-assignments-on-file-and-directory-level-access-control-lists"></a>Влияние назначений ролей в списках управления доступом на уровне файлов и каталогов

Хотя с помощью назначений ролей RBAC — это мощный механизм для управления разрешениями доступа, это весьма очень точный механизм по отношению к ACL. Наименьший уровень детализации для RBAC — уровень файловой системы, который оценивается с более высоким приоритетом, чем списки управления доступом. Таким образом Если назначить роль субъекту безопасности в рамках файловой системы, что субъект безопасности имеет уровень авторизации, связанных с этой ролью, для всех каталогов и файлов в этой файловой системе, независимо от назначения списка управления Доступом.

Когда субъект безопасности разрешения RBAC данных через [встроенная роль](https://docs.microsoft.com/azure/storage/common/storage-auth-aad?toc=%2fazure%2fstorage%2fblobs%2ftoc.json#built-in-rbac-roles-for-blobs-and-queues), или через пользовательскую роль, эти разрешения вычисляются в первую очередь при авторизации запроса. Если запрошенная операция получает разрешение путем назначения RBAC для субъекта безопасности авторизации немедленно разрешенных и не потребуется дополнительных выполняются проверки списка управления Доступом. Кроме того Если операция запроса не совпадает с назначено разрешение участника безопасности не поддерживает назначение RBAC, затем ACL проверок чтобы определить, авторизован ли субъект безопасности для выполнения запрошенной операции.

> [!NOTE]
> Если субъект безопасности, явно назначенной назначение встроенной роли владельца данных хранилища BLOB-объектов, то субъект безопасности считается *суперпользователь* и предоставляется полный доступ ко всем операциям изменения, включая настройку владелец каталога или файла, а также списки ACL для каталогов и файлов, для которых они не является владельцем. Доступ суперпользователя — единственный разрешенный способ изменить владельца ресурса.

## <a name="shared-key-and-shared-access-signature-sas-authentication"></a>Проверка подлинности Shared Key и подписи общего доступа (SAS)

Azure Data Lake хранения 2-го поколения поддерживает Shared Key и SAS методов проверки подлинности. Характеристика эти методы проверки подлинности является удостоверение не связан с объектом, вызывающим и поэтому не может быть выполнена безопасности участника разрешение авторизации на основе.
...
Субъект безопасности можно связать с уровнем доступа для файлов и каталогов. Эти связи, сохраняются в *список управления доступом (ACL)* . Каждого файла и каталога в вашей учетной записи хранения имеет список управления доступом.

Если вы назначили роль субъекту безопасности на уровне учетной записи хранения, можно использовать списки управления доступом для предоставления что этого участника безопасности с повышенными правами доступа для конкретных файлов и каталогов.

Нельзя использовать списки управления доступом для обеспечения уровня доступа, который меньше, чем уровень, предоставленные назначение ролей. Например, если вы назначаете [участник для данных больших двоичных объектов хранилища](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#storage-blob-data-contributor-preview) перечислены роли для субъекта безопасности, то нельзя использовать управление доступом для предотвращения этого участника безопасности производить запись в каталог.

### <a name="set-file-and-directory-level-permissions-by-using-access-control-lists"></a>Набор файлов и каталогов разрешения на уровне с помощью списков управления доступом

Чтобы задать разрешения на уровне файлов и каталогов, см. в любой из следующих статей:

|Если вы хотите использовать это средство:    |См. в статье:    |
|--------|-----------|
|Обозреватель службы хранилища Azure    |[Установка разрешений на уровне файлов и каталогов в Azure Data Lake Storage 2-го поколения с помощью Обозревателя службы хранилища Azure](https://docs.microsoft.com/azure/storage/blobs/data-lake-storage-how-to-set-permissions-storage-explorer)|
|REST API    |[PATH - обновление](https://docs.microsoft.com/rest/api/storageservices/datalakestoragegen2/path/update)|

> [!IMPORTANT]
> Если участником безопасности является *службы* субъекта, необходимо использовать идентификатор объекта субъекта-службы, а не идентификатор объекта зарегистрированного приложения связанные. Чтобы получить идентификатор объекта субъекта-службы, откройте Azure CLI, а затем используйте следующую команду: `az ad sp show --id <Your App ID> --query objectId`. не забудьте заменить `<Your App ID>` заполнитель идентификатора приложения для регистрации вашего приложения.

### <a name="types-of-access-control-lists"></a>Типы списков контроля доступа
...
Список ACL по умолчанию представляют собой шаблоны области ACL, связанные с каталогом, которые определяют доступ списки управления доступом для дочерних элементов, которые создаются в этом каталоге. У файлов нет списков ACL по умолчанию.
...
### <a name="levels-of-permission"></a>Уровни доступа
...
> [!NOTE]
> Если вы предоставляете доступ, используя только списки управления доступом (не RBAC), а затем предоставить чтения субъекта-службы или доступ на запись в файл, необходимо предоставить субъекту-службе **Execute** разрешения в файловой системе и для каждой папки в Иерархия папок, которые приводят к файлу.
...
Ниже перечислены некоторые распространенные сценарии, помогут вам понять, какие разрешения требуются для выполнения определенных операций в учетной записи хранения.
...
- С именем управляемых удостоверений
- все остальные пользователи.

Удостоверения пользователей и групп являются удостоверениями Azure Active Directory (AAD). Поэтому, если не указано иное, *пользователя*, в контексте Gen2 хранилища Озера данных, можно ссылаться на пользователем Azure AD, службы субъекта, управляемого удостоверения или группы безопасности.
...
В списках управления доступом POSIX каждый пользователь связан с *основной группой*. Например пользователь «Alice» может быть участником группы «Финансы». Алиса может принадлежать к нескольким группам, но одна группа всегда назначается как их основную группу. В интерфейсе POSIX, когда пользователь Aлиса создает файл, группа владельцев этого файла задается для основной группы этого пользователя, которой в данном случае является группа finance. В противном случае разрешения группы владельцев будут аналогичны разрешениям, назначенным другим пользователям или группам.
...
* **Тип 1.** Корневой каталог "/". Этот каталог создается при создании файловой системы Data Lake Storage 2-го поколения. В данном случае группа владельцев закрепляется за пользователем, создавшим файловую систему, если это было сделано с помощью OAuth. Если в файловой системе создается с помощью общего ключа SAS учетной записи и SAS службы, а затем присваивается имя владельца и группы владельцев **$superuser**.
...
> [!NOTE]
> Группа владельцев не может изменить списки ACL для файла или каталога.  Хотя группа владельцев установлено значение пользователя, который создал учетную запись с вариантом в корневом каталоге **вариант 1** выше, одной учетной записи не может предоставлять разрешения группе владельцев. Разрешение можно назначить допустимой группе пользователей, если это применимо.

### <a name="access-check-algorithm"></a>Алгоритм проверки доступа

Следующий псевдокод представляет алгоритма проверки доступа для учетных записей хранения.
...
№ Управление доступом с помощью списков управления доступом включен для учетной записи хранения, до тех пор, пока иерархическое пространство имен (HNS) функция включена.
