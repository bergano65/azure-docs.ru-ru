...
- На уровне учетной записи хранения один раз в день происходит определение правил для выполнения.
...
Политика управления жизненным циклом поддерживает учетные записи GPv2 (общего назначения, версия 2) и учетные записи хранилища BLOB-объектов. Существующую учетную запись GPv1 можно обновить до GPv2 одним щелчком мыши без простоев на портале Azure. Дополнительные сведения об учетных записях хранения см. в статье [Общие сведения об учетной записи хранения](../common/storage-account-overview.md).  
...
3. В колонке "Параметры" под службой BLOB-объектов щелкните **Управление жизненным циклом**, чтобы просмотреть или изменить политики.
...
Каждому правилу необходимы три параметра.
...
Каждое определение правила состоит из набора фильтров и набора действий. [Набор фильтра](#rule-filters) используется для ограничения действий правила в наборе объектов в контейнере или имен объектов. [Набор операций](#rule-actions) вводит в действие уровень или удаляет действия в отфильтрованном наборе объектов.

### <a name="sample-rule"></a>Пример правила
Следующий пример правила фильтрует учетную запись так, чтобы она выполняла действия только на `container1/foo`. Для всех объектов, которые существуют внутри `container1` **AND** начинается с `foo`, выполняются следующие действия: 
...
| prefixMatch | Массив строк, по которым выполняется сопоставление префиксов. Строка префикса должно начинаться с имени контейнера. Например, если все большие двоичные объекты в разделе https://myaccount.blob.core.windows.net/container1/foo/.. должны совпадать для правила, prefixMatch будет container1/foo. | Если prefixMatch не определен, то для всех BLOB-объектов в учетной записи применяются правила. | Нет  |
