...
В этом руководстве вы будете использовать подключаемый модуль службы хранилища Azure для Hudson CI, предоставляемый корпорацией Майкрософт.
...
* Решение непрерывной интеграции Hudson.
  ...
  1. На компьютере с поддержкой Java [Скачайте WAR-файл Hudson](https://www.eclipse.org/hudson/download.php).
  ...
  5. После завершения начальной настройки отмените запущенный экземпляр Hudson WAR, снова запустите WAR-файл Hudson и снова откройте панель мониторинга Hudson, `http://localhost:8080/`которая будет использоваться для установки и настройки подключаемого модуля службы хранилища Azure.
...
3. В разделе **Microsoft Azure Storage Account Configuration** (Настройка учетной записи хранения Azure):
    ...
    a. Введите имя учетной записи хранения, которую можно получить из [портал Azure](https://portal.azure.com).
   
    b. Введите ключ учетной записи хранения, который также можно получить из [портал Azure](https://portal.azure.com).
   
    c. Используйте значение по умолчанию для параметра **Blob Service Endpoint URL** (URL-адрес конечной точки службы BLOB-объектов) при использовании глобального облака Azure. Если вы используете другое облако Azure, используйте конечную точку, как указано в [портал Azure](https://portal.azure.com) учетной записи хранения.
   
    d. Щелкните **Validate storage credentials** (Проверить учетные данные хранилища) для проверки вашей учетной записи хранения.
   
    e. [Необязательно] Если у вас есть дополнительные учетные записи хранения, которые требуется сделать доступными для решения Hudson CI, нажмите **Добавить дополнительные учетные записи хранения**.
...
6. В поле **имя учетной записи хранения**выберите учетную запись хранения для использования.
7. В поле **имя контейнера**укажите имя контейнера. (Контейнер будет создан, если он еще не существует при передаче артефактов сборки.) Можно использовать переменные среды, поэтому в этом примере в качестве имени контейнера введите **$ {JOB_NAME}** .
...
10. **Чтобы получить список артефактов для отправки**, введите **Text/*. txt**.
...
* **Учетная запись хранения**. весь доступ к службе хранилища Azure осуществляется через учетную запись хранения. Это самое высокоуровневое пространство имен для доступа к BLOB-объектам. Учетная запись может содержать неограниченное число контейнеров при условии, что их общий размер не достигает 100 ТБ.
* **Контейнер**. контейнер предоставляет Группирование набора больших двоичных объектов. Все BLOB-объекты должны содержаться в контейнере. Учетная запись может содержать неограниченное число контейнеров. Контейнер может хранить неограниченное количество больших двоичных объектов.
* **BLOB**— файл любого типа и размера. Существует два типа BLOB-объектов, которые могут храниться в хранилище Azure: блочные и страничные. Большинство файлов являются блочными BLOB-объектами. Единичный блочный BLOB-объект может иметь размер до 200 ГБ. В этом учебнике используются блочные BLOB-объекты. Страничные BLOB-объекты, другой BLOB-объект могут иметь размер до 1 ТБ. Они более эффективны, когда диапазоны байтов в файле часто изменяются. Дополнительные сведения о BLOB-объектах см. в разделе [Основные сведения о блочных, добавочных и страничных BLOB-объектах](https://msdn.microsoft.com/library/azure/ee691964.aspx).
* **Формат URL-адреса**. К BLOB-объектам можно обратиться, используя следующий формат URL-адреса:
  
    `http://storageaccount.blob.core.windows.net/container_name/blob_name`
  
    (Приведенный выше формат применяется для глобального облака Azure. Если вы используете другое облако Azure, используйте конечную точку в [портал Azure](https://portal.azure.com) , чтобы определить конечную точку URL-адреса.)
  
    В указанном выше формате `storageaccount` представляет имя вашей учетной записи хранения, `container_name` представляет имя контейнера, а `blob_name` представляет имя BLOB-объекта соответственно. В имени контейнера можно использовать несколько путей, разделенных косой чертой **/**. Примером имени контейнера в данном учебнике было имя **MyJob**, а имя **${BUILD\_ID}/${BUILD\_NUMBER}** использовалось в качестве общего виртуального пути, в результате чего BLOB-объект имеет следующий URL-адрес:
