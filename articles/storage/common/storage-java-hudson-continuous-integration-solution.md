...
3. В разделе **Microsoft Azure Storage Account Configuration** (Настройка учетной записи хранения Azure):
    ...
    а. Введите имя учетной записи хранения, которую можно получить из [портал Azure](https://portal.azure.com).
   
    б. Введите ключ учетной записи хранения, который также можно получить из [портал Azure](https://portal.azure.com).
    ...
    . Щелкните **Validate storage credentials** (Проверить учетные данные хранилища) для проверки вашей учетной записи хранения.
...
7. Для поля **Container name** задайте имя контейнера. (Контейнер будет создан, если он еще не существует при передаче артефактов сборки.) Можно использовать переменные среды, поэтому в этом примере в качестве имени контейнера введите **$ {JOB_NAME}** .
