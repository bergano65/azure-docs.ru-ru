...
Кроме того, в некоторых сценариях SAS вам потребуется для авторизации доступа к исходному объекту в операции копирования:
...
Если клиент предоставляет URI SAS в службу хранилища Azure как часть запроса, то служба проверяет параметры и подпись SAS, чтобы убедиться, что он является допустимым для аутентификации запроса. Если служба подтверждает эту подпись, выполнение запроса разрешается. В противном случае запрос отклоняется и происходит ошибка с кодом 403 (запрещено).
...
* **Версия службы.** Обязательный параметр, который указывает версию службы хранилища для авторизации запроса.
...
* **Подпись.** Подпись составляется из других параметров, указанных в маркере, а затем шифруется. Эта подпись используется для авторизации доступа к указанным ресурсам хранилища.
...
| Подпись |`sig=Z%2FRHIX5Xcg0Mq2rqI3OlWTjEg2tYkboXr1P9ZUXDtkk%3D` |Используется для авторизации доступа к большому двоичному объекту. Подпись представляет собой код HMAC, рассчитанный по алгоритму SHA256 с использованием ключа и преобразования строки в символы. Полученное значение преобразуется в кодировку Base64. |
...
* **Динамический SAS**. При создании динамического SAS время начала, время окончания срока действия и разрешения для SAS указываются в URI SAS (или подразумеваются в случае, когда опускается время начала). Этот тип можно использовать для SAS учетной записи и SAS службы.
...
Клиент, владеющий SAS, может использовать его для авторизации запроса к учетной записи хранения, для которой у него нет ключей учетной записи. SAS можно добавить в строку подключения и непосредственно использовать в соответствующем конструкторе или методе.
...
Несколько конструкторов и перегрузок методов из клиентской библиотеки службы хранилища Azure поддерживают параметр SAS, что позволяет авторизовать запрос к службе с использованием SAS.
