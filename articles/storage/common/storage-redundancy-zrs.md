...
Ваши данные будут по-прежнему доступны, даже если зона становится недоступной. Корпорация Майкрософт рекомендует и дальше следовать рекомендациям по устранению временных сбоев. Эти рекомендации включают в себя внедрение политик повтора с экспоненциальной задержкой.
...
Миграция данных в хранилище, избыточное между зонами (ZRS), или из него требует другой стратегии. Миграция ZRS включает физическое перемещение данных из одной метки хранилища в несколько меток в пределах региона.

Имеется два основных варианта перемещения данных в хранилище ZRS или из него: 
...
4. Укажите следующие значения в разделе **Проблема**: 
    ...
    - **Категория**. Выберите **Migrate to ZRS within a region** (Миграция в ZRS в пределах региона).
    - **Заголовок**. Введите описательный заголовок, например **Миграция учетной записи ZRS**.
    - **Сведения**. Введите дополнительные сведения в поле **Сведения**, например "нужна миграция из [LRS, GRS] в ZRS в регионе ___". 
5. Щелкните **Далее**.
