---
title: Создание высокодоступных приложений с хранилищем, избыточным в виде зоны (ZRS)
...
ms.locfileid: "76719098"
---
# <a name="build-highly-available-applications-with-zone-redundant-storage-zrs"></a>Создание высокодоступных приложений с хранилищем, избыточным в виде зоны (ZRS)

[!INCLUDE [storage-common-redundancy-ZRS](../../../includes/storage-common-redundancy-zrs.md)]

## <a name="support-coverage-and-regional-availability"></a>Доступность в регионах и действие поддержки

В настоящее время ZRS поддерживает стандартные типы учетных записей хранения общего назначения v2, Филестораже и Блоккблобстораже. Дополнительные сведения о типах учетных записей хранения см. в статье [Общие сведения об учетной записи хранения](storage-account-overview.md).

Учетные записи общего назначения версии 2 ZRS поддерживают блочные и недисковые BLOB-объекты, стандартные общие файловые ресурсы, таблицы и очереди.
...
- Динамическая миграция поддерживается только для учетных записей хранения, использующих репликацию LRS. Если ваша учетная запись использует GRS или RA-GRS, перед продолжением необходимо сначала изменить тип репликации учетной записи на LRS. Этот промежуточный шаг удаляет вторичную конечную точку, предоставляемую GRS/RA-GRS.
...
**Можно ли запросить динамическую миграцию учетных записей геоизбыточного хранилища (GRS/RA-GRS) в ZRS?**

Динамическая миграция поддерживается только для учетных записей хранения, использующих репликацию LRS. Если ваша учетная запись использует GRS или RA-GRS, перед продолжением необходимо сначала изменить тип репликации учетной записи на LRS. Этот промежуточный шаг удаляет вторичную конечную точку, предоставляемую GRS/RA-GRS. Кроме того, перед отправкой запроса на динамическую миграцию в ZRS убедитесь, что ваши приложения или рабочие нагрузки больше не нуждаются в доступе к вторичной конечной точке только для чтения, и измените тип репликации учетных записей хранения на локально избыточное хранилище (LRS). Дополнительные сведения см. в разделе [изменение стратегии репликации](https://docs.microsoft.com/azure/storage/common/storage-redundancy#changing-replication-strategy) .
