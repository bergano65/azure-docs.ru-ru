...
|**/d:**<TargetDirectories\>|**Обязательный.** Один или несколько разделенных точкой с запятой каталогов, содержащих исходные импортированные файлы. Также может использоваться диск для импорта, но это не является обязательным, если доступны альтернативные расположения исходных файлов.|  
|**/bk:**<ключ_BitLocker\>|**Необязательный элемент.** Укажите ключ BitLocker, чтобы разблокировать зашифрованный диск, на котором расположены исходные файлы.|  
...
|**/PathMapFile:**<DrivePathMapFile\>|**Необязательный элемент.** Путь к текстовому файлу, который может использоваться для устранения неоднозначностей при наличии нескольких файлов с одинаковым именем, импортируемых в одно задание. При первом запуске средства файл может быть заполнен всеми неоднозначными именами. При последующих запусках средства этот файл будет использоваться для устранения неоднозначностей.|  
