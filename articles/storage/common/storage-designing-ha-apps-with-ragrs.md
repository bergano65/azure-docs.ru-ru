---
title: Разработка высокодоступных приложений с использованием геоизообразующих хранилищ
titleSuffix: Azure Storage
description: Узнайте, как использовать геоизмерие в области геоизмового хранилища для проектирования высокодоступного приложения, которое является достаточно гибким для обработки перебоев.
services: storage
author: tamram
ms.service: storage
ms.topic: conceptual
...
---
# <a name="designing-highly-available-applications-using-read-access-geo-redundant-storage"></a>Проектирование высокодоступных приложений с использованием геоизущегося хранилища для чтения доступа

Характерной чертой облачных инфраструктур, таких как служба хранилища Azure, является то, что они предоставляют высокодоступную платформу для размещения приложений. Разработчикам облачных приложений необходимо внимательно обдумать, как эффективно использовать эту платформу, чтобы обеспечить высокий уровень доступности приложений пользователям. В этой статье основное внимание уделяется тому, как разработчики могут использовать один из вариантов геоизообразуемого репликации Azure, чтобы гарантировать, что их приложения для хранения Azure очень доступны.

Учетные записи хранения, настроенные для геоизованной репликации, синхронно реплицируются в первичном регионе, а затем асинхронно реплицируются во вторичную область, которая находится на расстояние сотен миль. Azure Storage предлагает два типа гео-избыточной репликации:

* [Гео-зона-избыточное хранилище (ГЗРС) (предварительный просмотр)](storage-redundancy.md) обеспечивает репликацию для сценариев, требующих как высокой доступности, так и максимальной долговечности. Данные синхронно реплицируются в трех зонах доступности Azure в основном регионе с использованием избыточного хранилища зон (ЗРС), а затем синхронно реплицируются во вторичную область. Для чтения доступа к данным во вторичном регионе, включите считывающий доступ к геозоне-избыточного хранения (RA-G'S).
* [Геоизбыточное хранилище (GRS)](storage-redundancy.md) обеспечивает межрегиональную репликацию для защиты от региональных отключений. Данные реплицируются синхронно три раза в первичном регионе с использованием локально избыточного хранилища (LRS), а затем реплицируются синхронно во вторичную область. Для чтения доступа к данным во вторичном регионе включите геоизмовую хранилище (RA-GRS).

В этой статье показано, как создать приложение для обработки сбой в основной области. Если основная область становится недоступной, приложение может адаптироваться для выполнения считывательных операций против вторичного региона. Перед началом работы убедитесь, что ваша учетная запись хранилища настроена для RA-GRS или RA-G'S.
...
## <a name="application-design-considerations-when-reading-from-the-secondary"></a>Соображения дизайна приложений при чтении из вторичного
...
Имейте в виду эти ключевые моменты при разработке приложения для RA-GRS или RA-G'S:
...
* Библиотеку клиентов хранения можно использовать для чтения и записи данных в первичном или вторичном регионе. Можно также автоматически перенаправлять запросы на чтение в дополнительный регион, если истекло время ожидания запроса на чтение в основном регионе.
...
> [!NOTE]
> Сбой в работе учетной записи с управлением клиентом (предварительный просмотр) пока не доступен в регионах, поддерживающих G'S/RA-G'S, поэтому в настоящее время клиенты не могут управлять событиями сбоя с работы с помощью учетных записей G'S и RA-G'S. Во время предварительного просмотра корпорация Майкрософт будет управлять любыми событиями сбоя, влияющими на учетные записи G'S/RA-G'S.
...
Например, предположим, что клиент успешно отправил обновление, но в основном регионе произошел сбой, прежде чем обновление распространилось на дополнительный регион. Когда клиент просит прочитать данные обратно, он получает устаревшие данные из вторичного региона вместо обновленных данных. На этапе разработки приложения вам нужно решить, допустимо ли это, и если допустимо, — определить способ уведомления клиента. 
...
Чтобы эффективно подготовиться к сбою в основной области, необходимо иметь возможность обрабатывать как невыполненные запросы на чтение, так и невыполненные запросы на обновление (с обновлением в данном случае означаемих вставок, обновлений и удаляний). Если первичный регион выходит из строя, запросы на чтение могут быть перенаправлены во вторичный регион. Но запросы на обновление невозможно перенаправить в дополнительный центр обработки данных, так как он доступен только для чтения. По этой причине необходимо разработать приложение, выполняемое в режиме только для чтения.
...
Библиотека клиентов Azure Storage помогает определить, какие ошибки можно повторить. Например, ошибка 404 (ресурс не найден) не будет повторена, поскольку ее повторная попытка вряд ли приведет к успеху. С другой стороны, ошибка 500 может быть повторена, потому что это ошибка сервера, и проблема может быть просто переходной проблемой. Для получения дополнительных сведений ознакомьтесь с [открытым кодом класса ExponentialRetry](https://github.com/Azure/azure-storage-net/blob/87b84b3d5ee884c7adc10e494e2c7060956515d0/Lib/Common/RetryPolicies/ExponentialRetry.cs) в клиентской библиотеке службы хранилища для .NET. (Найдите метод ShouldRetry.)

### <a name="read-requests"></a>Запросы на чтение

В случае проблем с основным хранилищем запросы на чтение можно перенаправлять в дополнительное хранилище. Как было отмечено выше в разделе [Использование данных, согласованных в конечном счете](#using-eventually-consistent-data), для вашего приложения должно быть приемлемым чтение потенциально устаревших данных. Если вы используете библиотеку клиента хранения для доступа к данным из вторичного, можно указать поведение повторной попытки запроса на чтение, установив значение для свойства **LocationMode** на одно из следующих:
...
При установке **LocationMode** на **PrimaryThenSecondary,** если первоначальный запрос на чтение в основную конечную точку не удается с ошибкой, которая может быть повторена, клиент автоматически делает еще один запрос чтения на вторичную конечную точку. Если ошибка возникла из-за времени ожидания сервера, то клиенту придется дождаться, пока не истечет время ожидания, прежде чем служба вернет ему сообщение об ошибке, допускающей повторение.
...
| T2       | Транзакция Б:<br>Update<br> обновление сущности сотрудника<br> в основном регионе.  |                                | T1                 | Транзакция Б записана в основной регион,<br> но еще не реплицирована.  |
| T3       | Транзакция В:<br> Update <br>administrator<br>сущности роли в<br>primary |                    | T1                 | Транзакция В записана в основной регион,<br> но еще не реплицирована.  |
...
Чтобы узнать, как проверить последнее время синхронизации, [см.](last-sync-time-get.md)
...
* Для получения дополнительной информации о том, как читать из вторичного региона, [Azure Storage Redundancy Options and Read Access Geo-Redundant Storage](https://blogs.msdn.microsoft.com/windowsazurestorage/2013/12/11/windows-azure-storage-redundancy-options-and-read-access-geo-redundant-storage/)включая другой пример того, как устанавливается свойство Последнее время синхронизации, см.

* Полный пример, показывающий, как сделать переключатель между первичными [Azure Samples – Using the Circuit Breaker Pattern with RA-GRS storage](https://github.com/Azure-Samples/storage-dotnet-circuit-breaker-pattern-ha-apps-using-ra-grs)и вторичными конечными точками, см.
