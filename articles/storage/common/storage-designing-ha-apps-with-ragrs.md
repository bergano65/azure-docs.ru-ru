...
Ниже показан пример что может произойти при обновлении сведений сотрудника, чтобы сделать их членом *Администраторы* роли. В этом примере требуется обновить сущность **employee** и изменить в сущности **administrator role** общее число администраторов. Обратите внимание на то, как обновления применяются не по порядку в дополнительном регионе.
...
## <a name="getting-the-last-sync-time"></a>Получение последней синхронизации

Для получения последней синхронизации, чтобы определить, когда данные последней операции записи на сервер-получатель можно использовать PowerShell или Azure CLI.

### <a name="powershell"></a>PowerShell

Чтобы получить время последней синхронизации учетной записи хранения с помощью PowerShell, проверьте учетной записи хранения **GeoReplicationStats.LastSyncTime** свойство. Не забудьте заменить значения заполнителей собственными значениями:
...
Чтобы получить время последней синхронизации учетной записи хранения с помощью Azure CLI, проверьте учетной записи хранения **geoReplicationStats.lastSyncTime** свойство. Используйте `--expand` для возврата значения для свойств вложен в узел **geoReplicationStats**. Не забудьте заменить значения заполнителей собственными значениями:
