...
> [!IMPORTANT]
> Включение правил брандмауэра для учетной записи хранения заблокирует доступ для входящих запросов к данным, в том числе из других служб Azure.  Сюда входит использование портала, запись журналов и т. д.  Службам Azure, которые работают внутри виртуальной сети, может быть предоставлен доступ за счет подсети экземпляра службы.  Брандмауэр блокирует службы Azure, которые не работают внутри виртуальной сети.  Ограниченное количество сценариев можно включить с помощью описанного ниже механизма [исключений](https://docs.microsoft.com/azure/storage/common/storage-network-security#exceptions).  Для доступа к порталу это необходимо будет сделать на компьютере, размещенном в пределах настроенной доверенной границы (на основе протокола IP или виртуальной сети).
>
...
Правила IP-сети можно применять только для **общедоступных** IP-адресов в Интернете.  Диапазоны IP-адресов, зарезервированные для частных сетей (как определено в документе [RFC 1918](https://tools.ietf.org/html/rfc1918#section-3)), запрещено использовать в правилах IP.  Частные сети включают адреса, начинающиеся с *10.\**, *172.16.\** - *172.31.\** и *192.168\**.

> [!NOTE]
> Правила IP-сети не затронут запросы, исходящие из того же региона Azure, к которому относится учетная запись хранения.  Используйте [правила виртуальной сети](#grant-access-from-a-virtual-network) для разрешения запросов из того же региона.
>
