...
- Ресурсы некоторых служб, **зарегистрированные в подписке**, могут обращаться к учетным записям хранения **в одной подписке** только для операций выбора, таких как запись журналов или резервное копирование.
- Экземплярам ресурсов некоторых служб можно предоставить явный доступ к учетной записи хранения, назначив экземпляру ресурса [**роль RBAC**](storage-auth-aad.md#assign-rbac-roles-for-access-rights) .


При включении **разрешения доверенных служб Майкрософт...** исключение эти службы (если они зарегистрированы в вашей подписке) получают доступ к учетной записи хранения для операций выбора, как описано ниже.
...
| Azure HDInsight          | Microsoft.HDInsight        | Подготавливает начальное содержимое файловой системы по умолчанию для нового кластера HDInsight. [Дополнительные сведения](https://azure.microsoft.com/blog/enhance-hdinsight-security-with-service-endpoints/) |
...
| Azure Stream Analytics         | Microsoft.StreamAnalytics       | Позволяет записывать данные из задания потоковой передачи в хранилище BLOB-объектов. Эта функция в настоящее время находится на стадии предварительной версии. [Дополнительные сведения](../../stream-analytics/blob-output-managed-identity.md) |
