...
При создании параметра диагностики на портале Azure можно выбрать ресурс из списка. Для PowerShell или Azure CLI необходимо использовать ИД ресурса типа хранилища. ИД ресурса можно найти на портале Azure, открыв страницу **свойств** учетной записи хранения.
...
Журналы для Data Lake Storage 2-го поколения не отображаются в выделенной таблице. Это связано с тем, что Data Lake Storage 2-го поколения не является службой. Это набор возможностей, которые можно включить в учетной записи хранения BLOB-объектов. Если вы включили эти возможности, журналы будут по прежнему отображаться в таблице Сторажеблоблогс. 
