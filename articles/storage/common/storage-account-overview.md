...
### <a name="block-blob-storage-accounts"></a>Блок учетные записи хранения BLOB-объектов

Учетную запись хранения BLOB-объектов блока — специализированные записи хранения для хранения неструктурированных данных объектов в виде блочных BLOB-объектов. Этой учетной записи хранения введите поддерживает блочные и добавления больших двоичных объектов, но не это страничных BLOB-объектов, таблиц и очередей.

По сравнению с общего назначения версии 2 и учетные записи хранения BLOB-объектов, блок учетные записи хранения BLOB-объектов предоставляют низким и согласованное задержки и более высокие скорости транзакций.

Блок учетные записи хранения BLOB-объектов в настоящее время не поддерживают распределение по уровням "Горячий", "холодный" или архивный уровни доступа.

### <a name="filestorage-preview-storage-accounts"></a>Учетные записи хранения хранилища (Предварительная версия)

Учетной записи хранения хранилища — это специализированная учетная запись для хранения и создание файловых ресурсов уровня "премиум". Учетные записи хранения хранилища предлагают уникальный выделенный характеристики например больших операций ввода-ВЫВОДА. Дополнительные сведения об этих характеристиках см. в разделе [уровни производительности в общую папку файл](../files/storage-files-planning.md#file-share-performance-tiers) раздел файлов, руководство по планированию.
...
Блок учетные записи хранения BLOB-объектов обеспечивают уровень производительности premium для хранения блочных BLOB-объектов и добавочных BLOB-объектов.

Учетные записи хранения хранилища (Предварительная версия) предоставляют уровень производительности premium для файловых ресурсов Azure.
...
* **Горячий** уровень доступа оптимизирован для частого доступа к объектам в учетной записи хранения. Доступ к данным в "Горячий" уровень — самое экономичное, а затраты на хранение в более поздней версии. Новые учетные записи хранения создаются на горячем уровне по умолчанию.
* **Холодный** уровень доступа оптимизирован для хранения больших объемов данных, которые используются редко и хранятся не менее 30 дней. Хранение данных в "холодном" уровне является более экономичной, но доступ к этим данным может быть более затратным, чем доступ к данным в "Горячий" уровень.
* **Архивный** уровень доступен только для отдельных блочных BLOB-объектов. Архивный уровень оптимизирован для данных, конфигурация может выдержать извлечении задержку в несколько часов и будет оставаться на архивном уровне менее 180 дней. Хранение данных на архивном уровне наиболее экономично, но доступ к этим данным затратнее, чем доступ к данным на горячем и холодном уровнях.

Если нужно изменить шаблон использования данных, вы в любое время можете изменить уровень доступа. Дополнительные сведения об уровнях доступа см. в разделе [хранилище BLOB-объектов Azure: горячий, холодный и архивный уровни доступа](../blobs/storage-blob-storage-tiers.md).
...
> [!NOTE]
> Большой двоичный объект и учетные записи хранения BLOB-объектов предоставляют только конечную точку службы BLOB-объектов.
...
- **Azure Active Directory:** Используйте учетные данные Azure Active Directory (Azure AD) для проверки подлинности пользователя, группы или другое удостоверение для доступа к данным BLOB-объектов и очередей. Если аутентификация удостоверения выполнена успешно, Azure AD возвращает токен для использования при авторизации запроса в хранилище BLOB-объектов Azure или хранилище очередей. Дополнительные сведения см. в статье об [аутентификации доступа к службе хранилища Azure с помощью Azure Active Directory](storage-auth-aad.md).
...
* Чтобы узнать, как создать учетную запись хранения Azure общего назначения, см. в разделе [создать учетную запись хранилища](storage-quickstart-create-account.md).
* Узнайте, как создать учетную запись хранилища BLOB-объектов блока, см. в разделе [создать учетную запись хранилища BLOB-объектов блока](../blobs/storage-blob-create-account-block-blob.md).
