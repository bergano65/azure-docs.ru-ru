...
| Закрытое | Если после отправки вам всех дисков обнаруживаются ошибки, произошедшие во время выполнения задания, то оно переходит в состояние **Closed**. Задание в состоянии **Closed** автоматически удаляется через 90 дней. |
...
В службе "Импорт и экспорт" не предусмотрено соглашение об уровне обслуживания, однако служба стремится выполнить копирование в течение 7–10 дней после получения дисков. Помимо состояния, опубликованного на портал Azure, для отслеживания хода выполнения задания можно использовать интерфейсы API. Параметр "процент завершения" в вызове API операции " [список заданий](/previous-versions/azure/dn529083(v=azure.100)) " обеспечивает ход копирования в процентах.
