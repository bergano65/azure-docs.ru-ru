...
Для получения дополнительной информации и рекомендаций, когда емкость данных охватывает несколько заданий импорта дисков, обратитесь в службу поддержки майкрософт.
...
Вы можете проигнорировать поле "Lease Expired". Служба импорта и экспорта арендует большой двоичный объект во время отправки, чтобы большой двоичный объект не обновлялся во время других процессов в параллельном режиме. Состояние "Lease expired" (Срок аренды истек) подразумевает, что служба импорта и экспорта больше не выполняет отправку и большой двоичный объект доступен для использования.
...
> [!NOTE]
> Центры обработки данных в Индии требуют декларации письмо на бланке (доставка challan), чтобы вернуть диски. Чтобы организовать необходимый пропуск, необходимо также забронировать билет с выбранным оператором связи и поделиться информацией с центром обработки данных.

### <a name="are-there-any-restrictions-with-shipping-and-returning-my-drive-internationally"></a>Существуют ли какие-либо ограничения с доставкой и возвращением моего диска на международном уровне?
...
После завершения загрузки процесс возврата диска (ы) на международный адрес может занять больше времени, чем обычные 2-3 дня, необходимые для местной доставки. На этапе, указанном на портале Azure как Packaging, команда Data Box обеспечивает наличие правильной документации для обеспечения соответствия груза различным международным требованиям к импорту и экспорту.

### <a name="are-there-any-special-requirements-for-delivering-my-disks-to-a-datacenter"></a>Существуют ли какие-либо особые требования к доставке моих дисков в центр обработки данных?

Требования зависят от конкретных ограничений центра обработки данных Azure.

- Есть несколько сайтов, таких как Австралия, Германия и Великобритания на юге, которые требуют, чтобы входящие идентификационный номер центра обработки данных Майкрософт был записан на посылке по соображениям безопасности. Перед отправкой дисков или дисков в центр обработкиadbops@microsoft.comданных свяжитесь с Azure DataBox Operations (), чтобы получить этот номер. Без этого номера пакет будет отклонен.
- Центры обработки данных в Индии требуют персональные данные водителя, такие как удостоверение личности правительства или Proof No. (например, PAN, AADHAR, DL), имя, контакт и номер автомобиля, чтобы получить пропуск на вход в ворота. Чтобы избежать задержек с доставкой, сообщите об этих требованиях своему перевозчику.
...
Используйте портал Azure или [исследователь хранения для](https://docs.microsoft.com/azure/vs-azure-tools-storage-manage-with-storage-explorer) доступа к данным, наданным учетной записью хранения Azure.  
...
- Размер Max Page Blob составляет 8 ТБ.
...
Нет. Сервис Azure Import/Export использует шифрование AES-128 BitLocker.
