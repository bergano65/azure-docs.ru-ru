...
| **Пример** | `azcopy bench 'https://mystorageaccount.blob.core.windows.net/mycontainer/myBlobDirectory/'` |
...
Чтобы проверить текущее значение этой переменной, используйте значение `azcopy env`. Если значение пустое, то можно считать, какое значение используется, просмотрев начало любого файла журнала AzCopy. Выбранное значение и причина, по которой он был выбран, включаются в отчет.
...
По умолчанию файлы журнала и плана находятся в каталоге `%USERPROFILE$\.azcopy` в Windows или каталоге `$HOME$\.azcopy` в Mac и Linux, но при необходимости вы можете изменить это расположение.
...
По умолчанию файлы плана и журнала расположены в каталоге `%USERPROFILE$\.azcopy` в Windows или в каталоге `$HOME$\.azcopy` на компьютерах Mac и Linux. Это расположение можно изменить.
...
Чтобы проверить текущее значение этой переменной, используйте значение `azcopy env`. Если значение пустое, файлы планов записываются в расположение по умолчанию.
...
Чтобы удалить файлы плана и журнала, связанные только с одним заданием, используйте `azcopy jobs rm <job-id>`. Замените заполнитель `<job-id>` в этом примере на идентификатор задания.
