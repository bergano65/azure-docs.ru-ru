...
Чтобы просмотреть подробные справочные сведения об этой команде, `azcopy bench -h` введите и нажмите клавишу ВВОД.

### <a name="optimize-throughput"></a>Оптимизация пропускной способности

Вы можете использовать `cap-mbps` флаг в командах, чтобы поместить потолк на скорость передачи данных. Например, следующая команда возобновляет задание и пропускную способность с прописной `10` буквы на мегабайты (МБ) в секунду. 

```azcopy
azcopy jobs resume <job-id> --cap-mbps 10
```

Пропускная способность может уменьшиться при передаче небольших файлов. Можно увеличить пропускную способность, задав переменную `AZCOPY_CONCURRENCY_VALUE` среды. Эта переменная указывает количество одновременных запросов, которые могут быть выполнены.  

Если на компьютере установлено менее 5 ЦП, значение этой переменной будет равно `32`. В противном случае значение по умолчанию равно 16, умноженному на число ЦП. Максимальное значение по умолчанию для этой переменной `3000`равно, но можно вручную задать это значение выше или ниже. 
...
Задайте переменную `AZCOPY_BUFFER_GB` среды, чтобы указать максимальный объем системной памяти, который будет использоваться AzCopy при скачивании и отправке файлов.
...
Для этого используйте команду [azcopy Copy](storage-ref-azcopy-copy.md) , а затем установите для `--overwrite` `ifSourceNewer`флага значение. AzCopy будет сравнивать файлы при копировании без выполнения сквозной проверки и сравнения. Это обеспечивает производительность в тех случаях, когда требуется сравнить большое количество файлов.
...
По умолчанию файлы плана и журнала находятся в `%USERPROFILE%\.azcopy` каталоге в Windows или в `$HOME$\.azcopy` каталоге на компьютерах Mac и Linux. Это расположение можно изменить.
...
Чтобы удалить файлы плана и журнала, связанные только с одним заданием, `azcopy jobs rm <job-id>`используйте. Замените `<job-id>` заполнитель в этом примере идентификатором задания.
