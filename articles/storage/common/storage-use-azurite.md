...
Следующая команда запускает образ DOCKER Азурите. Параметр `-p 10000:10000` перенаправляет запросы с порта 10000 хоста компьютера на экземпляр DOCKER.
...
В следующем примере параметр `-v c:/azurite:/data` указывает *c:/азурите* в качестве расположения сохраненных данных азурите. Перед выполнением команды DOCKER необходимо создать каталог *c:/азурите*.
...
Азурите поддерживает пользовательские имена и ключи учетных записей хранения, задавая переменную среды `AZURITE_ACCOUNTS` в следующем формате: `account1:key1[:key2];account2:key1[:key2];...`.
...
> [!NOTE]
> Учетная запись хранения `devstoreaccount1` по умолчанию отключена при задании пользовательских учетных записей хранения.

> [!NOTE]
> Обновите строку подключения соответствующим образом при использовании настраиваемых имен и ключей учетной записи.

> [!NOTE]
> Используйте ключевое слово `export` для задания переменных среды в среде Linux, используйте `set` в Windows.

### <a name="storage-explorer"></a>Обозреватель хранилища
