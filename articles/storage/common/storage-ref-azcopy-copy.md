...
- azcopy CP "/Пас/то/file.txt" "https://[учетная запись]. BLOB. Core. Windows. NET/[контейнер]/[путь/к/большой двоичный объект]"
...
- azcopy CP "/Пас/то/file.txt" "https://[учетная запись]. BLOB. Core. Windows. NET/[контейнер]/[путь/кому/большой двоичный объект]"--MD5

Отправьте один файл с помощью маркера SAS:

- azcopy CP "/Пас/то/file.txt" "https://[учетная запись]. BLOB. Core. Windows. NET/[контейнер]/[путь/к/BLOB-объект]? [SAS] "

Отправка одного файла с помощью маркера SAS и конвейера (только блочные BLOB-объекты):
  
- Cat "/Пас/то/file.txt" | azcopy CP "https://[учетная запись]. BLOB. Core. Windows. NET/[контейнер]/[путь/к/BLOB-объект]? [SAS] "
...
- azcopy CP "https://[учетная запись]. BLOB. Core. Windows. NET/[контейнер]/[путь/к/большой двоичный объект]" "/Пас/то/file.txt"

Скачайте один файл с помощью маркера SAS:

- azcopy CP "https://[учетная запись]. BLOB. Core. Windows. NET/[контейнер]/[путь/к/BLOB-объект]? [SAS] ""/Пас/то/file.txt "
...
- azcopy CP " https://s3.amazonaws.com/ [сегмент]/[Папка]" "https://[дестаккаунт]. BLOB. Core. Windows. NET/[контейнер]/[путь/к/каталог]? [ SAS] "--recursive = true

Дополнительные сведения о https://docs.aws.amazon.com/AmazonS3/latest/user-guide/using-folders.html заполнителе [Папка] см. здесь.
...
**--BACKUP**                               Активирует Windows "Себаккуппривилеже для отправки" или "Сересторепривилеже для скачивания", чтобы разрешить AzCopy просматривать все файлы, независимо от разрешений файловой системы и восстанавливать все разрешения. Требует, чтобы учетная запись, выполняющая AzCopy, уже соработала эти разрешения (например, имеет права администратора или является членом группы "операторы резервного копирования"). Этот флаг позволяет активировать привилегии, которые у учетной записи уже есть.
...
**--Строка исключения-путь** исключает эти пути при копировании. Этот параметр не поддерживает подстановочные знаки (*). Проверяет относительный префикс пути (например: myFolder; myFolder/Субдирнаме/file.pdf). При использовании в сочетании с обходом учетной записи пути не включают имя контейнера.
...
Строка **--include-Path** включает в себя только эти пути при копировании. Этот параметр не поддерживает подстановочные знаки (*). Проверяет относительный префикс пути (например: myFolder; myFolder/Субдирнаме/file.pdf).
...
**--Preserve-SMB-строка разрешений** false по умолчанию. Сохраняет списки ACL SMB между ресурсами, которые поддерживаются (файлы Windows и Azure). Для загрузки необходимо также использовать `--backup` флаг для восстановления разрешений, когда новый владелец не будет пользователем, на котором выполняется AzCopy. Этот флаг применяется к файлам и папкам, если только не указан фильтр только для файлов (например, `include-pattern` ).

**--Preserve-SMB — строка сведений** false по умолчанию. Сохраняет сведения о свойстве SMB (время последней записи, время создания, биты атрибутов) между ресурсами, поддерживающими SMB (файлы Windows и Azure). Будут переданы только биты атрибутов, поддерживаемые службой "файлы Azure". все остальные будут пропущены. Этот флаг применяется к файлам и папкам, если только не задан фильтр только для файлов (например, шаблон include). Данные, передаваемые для папок, совпадают с файлами, за исключением времени последней записи, которое никогда не сохранялось для папок.

**--Preserve — владелец**                       Оказывает воздействие только на загрузку данных и только при `--preserve-smb-permissions` использовании. Если задано значение true (значение по умолчанию), владелец файла и группа сохраняются в файлах для загрузки. Если этот флаг имеет значение false, `--preserve-smb-permissions` все равно сохранит списки ACL, но владелец и группа будут основываться на пользователе, на котором выполняется AzCopy.
