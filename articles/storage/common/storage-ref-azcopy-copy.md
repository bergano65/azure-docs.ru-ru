...
- azcopy CP "https://s3.amazonaws.com/[сегмент]/[Папка]" "https://[дестаккаунт]. BLOB. Core. Windows. NET/[контейнер]/[путь/папка]? [SAS] "--recursive = true
...
**--Preserve — владелец**                       Оказывает воздействие только на загрузку данных и только при `--preserve-smb-permissions` использовании. Если задано значение true (значение по умолчанию), владелец файла и группа сохраняются в файлах для загрузки. Если этот флаг имеет значение false, все `--preserve-smb-permissions` равно сохранит списки ACL, но владелец и группа будут основываться на пользователе, на котором выполняется AzCopy.
