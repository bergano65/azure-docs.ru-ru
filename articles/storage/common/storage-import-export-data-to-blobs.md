...
7. Чтобы подготовить диск, выполните следующую команду. **В зависимости от размера данных это может занять от нескольких часов до нескольких дней.**
    ...
    |/blobtype     |Этот параметр указывает тип больших двоичных объектов, в которые необходимо импортировать данные. Для блочных BLOB-объектов `BlockBlob` это и для страничных BLOB- `PageBlob`объектов.         |
