...
- [Скачайте WAImportExport версии 1](https://www.microsoft.com/download/details.aspx?id=42659) в систему Windows. Распакуйте содержимое в папку по умолчанию: `waimportexportv1`. Пример: `C:\WaImportExportV1`.
...
6.  Чтобы подготовить диск, выполните следующую команду. **В зависимости от размера данных это может занять от нескольких часов до нескольких дней.** 
    ...
    |/srcdir:     |Буква отправляемого диска и `:\`. Пример: `D:\`.         |
    |/dstdir:     |Имя целевого контейнера в службе хранилища Azure.         |
    |/blobtype     |Этот параметр указывает тип больших двоичных объектов, в которые необходимо импортировать данные. Для блочных BLOB-объектов это `BlockBlob`, а для страничных BLOB-объектов — `PagaBlob`.         |
