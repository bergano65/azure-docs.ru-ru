---
title: Мониторинг учетной записи хранения Azure в портал Azure | Документация Майкрософт
...
>
> Учетные записи хранения блоков производительности Premium не поддерживают аналитические метрики хранилища, но поддерживают ведение журнала. Ведение журнала можно включить программно с помощью REST API или клиентской библиотеки. Если вы хотите просмотреть метрики с учетными записями хранилища BLOB-объектов производительности уровня "Премиум", рассмотрите возможность использования [метрик хранилища Azure в Azure Monitor](storage-metrics-in-azure-monitor.md).
>
...
1. Выберите **тип** данных метрик для каждой **службы**, которую нужно отслеживать, и **политику хранения** для данных. Можно также отключить мониторинг, задав для параметра **Состояние** значение **Откл**.
   ...
   > [!WARNING]
   > За удаление данных метрик вручную взимается плата. Устаревшие данные аналитики (срок которых превышает значение, установленное политикой хранения) удаляются системой бесплатно. Рекомендуется устанавливать политику хранения в зависимости от того, как долго нужно хранить данные аналитики хранилища для вашей учетной записи. Дополнительные сведения см. [в статье выставление счетов за метрики хранилища](storage-analytics-metrics.md#billing-on-storage-metrics) .
   >
...
Метрики, выбранные в окне **Диагностика** , определяют разрешение метрик, доступных для вашей учетной записи.
