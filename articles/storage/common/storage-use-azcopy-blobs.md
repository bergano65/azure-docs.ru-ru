...
>-  Учетные записи хранения блочных BLOB-объектов уровня "Премиум" не поддерживают уровни доступа. Исключите уровень доступа большого двоичного объекта из операции копирования, задав для `s2s-preserve-access-tier` `false` значение ( `--s2s-preserve-access-tier=false`например,).
