...
> [!NOTE]
> AzCopy по умолчанию загружает данные в виде блоковых капли. Чтобы загрузить файлы в виде приложения Blobs `--blob-type=[BlockBlob|PageBlob|AppendBlob]`или Page Blobs, используйте флаг.
> AzCopy по умолчанию загружает ваши данные, чтобы унаследовать уровень доступа к учетной записи. Чтобы загрузить файлы на определенный `--block-blob-tier=[Hot|Cool|Archive]` [уровень доступа,](../blobs/storage-blob-storage-tiers.md)используйте флаг.
