...
При создании учетной записи хранения вы выбираете первичный регион для учетной записи. Выбор вторичного региона определяется на основе первичного региона, его невозможно изменить. Дополнительные сведения см. в статье [Геоизбыточное хранилище (GRS). Межрегиональная репликация для службы хранилища Azure](storage-redundancy.md).

**Где можно получить дополнительные сведения о шифровании службы хранилища Azure?**  
...
- [Укажите ключ шифрования для запроса к хранилищу BLOB-объектов (Предварительная версия)](encryption-customer-provided-keys.md)
...
- Используйте маркер подписанного URL-адрес (SAS), чтобы предоставить доступ к ресурсу.
...
**Как использовать пользовательский домен HTTPS с моей учетной записью хранения? Например, как сделать, чтобы ссылка "https:\//mystorageaccountname.blob.core.windows.net/images/image.gif" имела вид "https:\//www.contoso.com/images/image.gif"** ?

TLS/SSL в настоящее время не поддерживаются в учетных записях хранения с личными доменами.
...
- При использовании геоизбыточного хранилища с доступом на чтение данные доступны из дополнительного региона в любое время. Используйте один из следующих методов.  
  ...
  - **Маркер SAS**: используйте маркер SAS для доступа к данным из конечной точки. Дополнительные сведения см. в статье [Использование подписанных URL-адресов (SAS)](storage-sas-overview.md).
