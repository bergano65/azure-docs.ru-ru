...
## <a name="copy-upload-or-download"></a>Копировать, загружать или загружать

**Как создать сценарий для копирования файлов из одного контейнера в другой?**

Для копирования файлов между контейнерами можно использовать AzCopy. См. следующий пример.
...
Скрипт автоматизации предназначен для развертывания Azure Resource Manager вместо операций с содержимым хранилища. Дополнительные сведения см. в статье [Развертывание ресурсов с использованием шаблонов Resource Manager и Azure PowerShell](../../azure-resource-manager/templates/deploy-powershell.md).
...
## <a name="migration-or-backup"></a>Миграция или резервное копирование
...
2.  Используйте [AzCopy](https://azure.microsoft.com/blog/azcopy-5-1-release/) для копирования содержимого из исходного контейнера с каплями в другой контейнер с каплями.
...
**Я должен каждый день перемещать журналы с диска D в свою учетную запись хранения Azure. Как это автоматизировать?**

Это можно сделать с помощью команды AzCopy и создания задачи в планировщике. Отправьте файлы в учетную запись Azure с помощью пакетного скрипта AzCopy. Для получения дополнительной [How to configure and run startup tasks for a cloud service](../../cloud-services/cloud-services-startup-tasks.md)информации см.
...
> [!NOTE]
> AzCopy CLI не работает вместе с переключателем **шаблона** при копировании с одной капли Azure в другую.
>
> Вы можете непосредственно скопировать и изменить команду AzCopy и перекрестно проверить, что **шаблон** соответствует источнику. Также убедитесь, что подстановочные знаки **/S** работают. Для получения дополнительной [информации](storage-use-azcopy.md)см.
...
-  [Руководство по безопасности хранения данных Azure](../blobs/security-recommendations.md)

-  [Шифрование службы хранения данных ВСУ для данных в остальных](storage-service-encryption.md)
...
**Как перейти со стандартной учетной записи хранилища на премиум-аккаунт? Как понизить значение с премиум-аккаунта хранения до стандартной учетной записи хранилища?**
...
-   Предоставьте пользователю первичный или вторичный ключ для учетной записи хранения. См. сведения о том, как [управлять ключами доступа к учетной записи хранения](storage-account-keys-manage.md).
...
**Как использовать пользовательский домен HTTPS с моей учетной записью хранения? Например, как сделать так,\/чтобы "https: /mystorageaccountname.blob.core.windows.net/images/image.gif" отображались как "https:\//www.contoso.com/images/image.gif"?**

TLS/SSL в настоящее время не поддерживается на учетных записях хранения с пользовательскими доменами.
Однако можно использовать пользовательские домены не HTTPS. Для получения дополнительной информации [см.](../blobs/storage-custom-domain-name.md)

## <a name="access-to-storage"></a>Доступ к хранилищам
...
**Как получить доступ к данным, хранящимся в дополнительном регионе, для реплицированной учетной записи хранения (например, хранилище, избыточное между зонами, геоизбыточное хранилище или географически избыточное хранилище с доступом на чтение)?**

-   Если вы используете хранилище, избыточное между зонами или геоизбыточное хранилище, вы не можете получить доступ к данным из дополнительного региона, если не инициируете отработку отказа в этом регионе. Дополнительные сведения об отработке отказа см. в статье [Disaster recovery and storage account failover (preview) in Azure Storage](storage-disaster-recovery-guidance.md) (Аварийное восстановление и отработка отказа учетной записи хранения (предварительная версия) в службе хранилища Azure).
...
-   При использовании геоизбыточного хранилища с доступом на чтение данные доступны из дополнительного региона в любое время. Используйте один из следующих методов.  
    ...
    - **AzCopy**: добавьте **-secondary** к имени учетной записи хранения в URL-адресе для доступа к дополнительной конечной точке. Пример:  

      https://storageaccountname-secondary.blob.core.windows.net/vhds/BlobName.vhd

    - **Токен SAS**: используйте токен SAS для доступа к данным из конечной точки. Дополнительные сведения см. в статье [Использование подписанных URL-адресов (SAS)](storage-sas-overview.md).

**Как использовать FTP для доступа к данным, соданным в учетной записи хранилища?**
...
Если вы хотите только скачивать данные без использования Обозревателя службы хранилища или аналогичного приложения, можно использовать токен SAS. Дополнительные сведения см. в статье [Использование подписанных URL-адресов (SAS)](storage-sas-overview.md).
