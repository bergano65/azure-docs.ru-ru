...
## <a name="copy-upload-or-download"></a>Копирование, отправка или скачивание
...
**Каждый день мне приходится перемещать журналы с диска D в учетную запись хранения Azure. Разделы справки автоматизировать это?**

Это можно сделать с помощью команды AzCopy и создания задачи в планировщике. Отправьте файлы в учетную запись Azure с помощью пакетного скрипта AzCopy. Дополнительные сведения см. [в статье Настройка и запуск задач запуска для облачной службы](../../cloud-services/cloud-services-startup-tasks.md).
...
> [!NOTE]
> Интерфейс командной строки AzCopy не работает вместе с параметром **шаблона** при копировании из одного большого двоичного объекта Azure в другой.
>
> Вы можете непосредственно скопировать и изменить команду AzCopy и перекрестно проверить, что **шаблон** соответствует источнику. Также убедитесь, что подстановочные знаки **/S** работают. Дополнительные сведения см. в разделе [AzCopy Parameters](storage-use-azcopy.md).
...
-  [Руководство по безопасности службы хранилища Azure](../blobs/security-recommendations.md)
...
**Разделы справки обновить учетную запись хранения уровня "Стандартный" до учетной записи хранения Premium? Разделы справки переходить от учетной записи хранения класса Premium к стандартной учетной записи хранения?**
...
**Разделы справки использовать пользовательский домен HTTPS с учетной записью хранения? Например, как сделать так, чтобы "https:\//mystorageaccountname.BLOB.Core.Windows.NET/Images/Image.gif" отображался как "HTTPS\/:/www.contoso.com/Images/Image.gif"?**

В настоящее время TLS/SSL не поддерживается в учетных записях хранения с пользовательскими доменами.
...
## <a name="access-to-storage"></a>Доступ к хранилищу
...
**Разделы справки использовать FTP для доступа к данным в учетной записи хранения?**
