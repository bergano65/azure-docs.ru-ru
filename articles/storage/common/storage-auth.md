...
- **Интеграция Azure Active Directory (Azure AD)** для больших двоичных объектов и очередей. Azure AD предоставляет управление доступом на основе ролей (RBAC) для управления доступом клиента к ресурсам в учетной записи хранения. Дополнительные сведения об интеграции Azure AD с большими двоичными объектами и очередями см. в статье [авторизация доступа к BLOB-объектам и очередям Azure с помощью Azure Active Directory](storage-auth-aad.md).
...
- **Локальные службы домен Active Directory (AD DS или локальная AD DS) (Предварительная версия** ) для файлов Azure. Служба файлов Azure поддерживает проверку подлинности по протоколу SMB с помощью AD DS. Среда AD DS может размещаться на локальных компьютерах или на виртуальных машинах Azure. Доступ к файлам по протоколу SMB поддерживается с помощью учетных данных AD DS из компьютеров, присоединенных к домену, локально или в Azure. Можно использовать сочетание RBAC для управления доступом на уровне общего доступа и DACL NTFS для принудительного применения разрешений на уровне каталогов и файлов. Дополнительные сведения о проверке подлинности файлов Azure с помощью доменных служб см. в [обзоре](../files/storage-files-active-directory-overview.md).
