...
|**/d:<целевой_каталог\>**|Обязательный параметр. Каталог, который нужно проверить и восстановить. Обычно это корневой каталог экспортируемого диска, но также может быть общая сетевая папка, в которой хранится копия экспортированных файлов.|  
...
|**/sn:<учетная_запись_хранения\>**|Обязательный параметр. Имя учетной записи хранения для задания экспорта.|  
...
|**/CopyLogFile:<файл_журнала\>**|Обязательный параметр. Путь к файлу журнала копирования диска. Этот файл создается службой импорта и экспорта Windows Azure. Его можно скачать из хранилища BLOB-объектов, связанного с заданием. Файл журнала копирования содержит сведения о BLOB-объектах или файлах с ошибками, которые требуется восстановить.|  
