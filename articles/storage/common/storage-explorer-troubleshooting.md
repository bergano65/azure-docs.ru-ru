...
2. Перейдите в раздел **Edit** > **SSL Certificates** > **Import Certificates**, а затем используйте средство выбора файлов для поиска, выбора и открытия CER-файла.
...
Если вы не можете найти самозаверяющие сертификаты, выполнив эти действия, обратитесь к нам через средство обратной связи. Можно также открыть Обозреватель службы хранилища из командной строки с помощью флага `--ignore-certificate-errors`. При открытии с этим флагом Обозреватель службы хранилища игнорирует ошибки сертификата.
...
1. В меню перейдите к **предварительной версии** > **использовать код устройства вход**.
...
> [!NOTE]
> Если вы когда-либо импортировали SSL-сертификаты, создайте резервную копию содержимого каталога `certs`. Позже вы сможете использовать резервную копию, чтобы повторно импортировать сертификаты SSL.
...
1. Запустите Обозреватель хранилища. В меню выберите пункт **справка** > **переключить средства для разработчиков**.
...
4. После сохранения текущих имен соединений задайте для параметра в Средства для разработчиков значение `[]`.
