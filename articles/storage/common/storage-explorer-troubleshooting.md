...
#### <a name="list-storage-account-keys"></a>Получение списка ключей учетной записи хранения
...
2. Вы запустили приложение, которое вставляет самозаверяющий сертификат SSL в получаемые сообщения HTTPS. К примерам приложений, которые внедряют сертификаты, относятся антивирусные программы и программное обеспечение для проверки трафика.
...
Если вы знаете, откуда поступают сертификат, можно попробовать следующие действия, чтобы найти его.
...
4. Найдите самозаверяющие сертификаты. Если вы знаете, какие из сертификатов являются самозаверяющими, найдите сертификаты, субъект `("s:")` и издателя `("i:")` одинаковы.
...
Если не удается найти самозаверяющие сертификаты, используя описанные выше шаги, свяжитесь с нами через средство обратной связи для получения дополнительных сведений. Вы также можете запустить обозреватель службы хранилища из командной строки с `--ignore-certificate-errors` флаг. При запуске с этим флагом Обозреватель службы хранилища будет игнорировать ошибки сертификата.

## <a name="sign-in-issues"></a>Проблемы при входе

### <a name="blank-sign-in-dialog"></a>Пустое окно регистрации

Пустой диалоговые окна входа в систему чаще всего вызваны ADFS запросом обозреватель службы хранилища для выполнения перенаправления, который не поддерживается программой Electron. Чтобы обойти эту проблему, можно попытаться использовать поток кода устройства для входа в систему. Для этого сделайте следующее:
...
Если вы в цикле повторную проверку подлинности, или были изменены имя участника-пользователя одного из своих учетных записей, выполните следующие действия.
...
6. Откроется всплывающее окно с сообщением примерно такого содержания: "Центр обслуживания хочет получить доступ к цепочке ключей". Если его, введите пароль учетной записи администратора Mac и выбрать пункт **всегда разрешать** (или **Разрешить** Если **всегда разрешать** недоступна).
7. Попробуйте выполнить вход.

### <a name="general-sign-in-troubleshooting-steps"></a>Общие действия по устранению неполадок входа

* Если у вас в macOS, и никогда не появится окно входа в систему через «ожидание проверки подлинности...» диалоговое окно, затем повторите [следующие действия](#mac-keychain-errors)
...
* Если вы используете Windows и иметь доступ к 2019 г. Visual Studio на одном компьютере и входа, попробуйте войти в Visual Studio 2019. После успешного входа в систему на 2019 г. Visual Studio можно открыть обозреватель службы хранилища и ваша учетная запись на панели учетной записи.
...
Если вам не удается получить подписки после успешного входа, попробуйте следующие методы устранения неполадок.
...
* Убедитесь, что вы вошли с использованием правильного Azure среду (Azure, Azure для Китая компанией 21Vianet, Azure для Германии, Azure для государственных организаций США или пользовательскую среду).
* При подключении через прокси-сервер, убедитесь, что правильно настроить прокси-сервер обозревателя службы хранилища.
* Попробуйте удалить и снова добавить учетную запись.
* Если есть ссылка «Подробнее», найдите и посмотрите, какие сообщения об ошибках выводятся для клиентов, которые не. Если you'ren't убедитесь что делать с ошибкой сообщений см. в разделе, а затем вы можете [начните обсуждение проблемы на GitHub](https://github.com/Microsoft/AzureStorageExplorer/issues).

## <a name="cant-remove-attached-account-or-storage-resource"></a>Не удается удалить присоединенные учетную запись или ресурс хранилища

Если вы не удается удалить подключенную учетную запись или ресурс хранилища через пользовательский Интерфейс, можно вручную удалить все подключенные ресурсы, удалив следующие папки:
...
* Убедитесь, что можно подключиться к порталу для среды Azure, которую вы пытаетесь использовать
...
Если у вас есть средства работы с сетью, такие как Fiddler для Windows, можно диагностировать проблемы следующим образом:
...
* Убедитесь, что прокси-сервер не блокирует трафик к конечным точкам Azure management или ресурсов.
...
Если вы подключены к Azure через учетную запись-посредник, проверьте правильность параметров прокси-сервера. Если вы получаете право доступа к ресурсу от владельца подписки или учетной записи, убедиться, что прочитали или список разрешений для этого ресурса.

## <a name="connection-string-doesnt-have-complete-configuration-settings"></a>Строка подключения не имеют полный параметры конфигурации

Если появляется следующее сообщение об ошибке, это возможно, что у вас нет необходимых разрешений для получения ключей для учетной записи хранения. Чтобы в этом убедится, перейдите на портал и найдите учетную запись хранения. Быстро, это можно сделать, щелкнув правой кнопкой мыши на узле для вашей учетной записи хранения и выбрав «В открытых в портал». После этого перейдите к колонке "Ключи доступа". Если у вас нет разрешений на просмотр ключи, вы увидите страницу с сообщением «У вас нет доступа». Чтобы обойти эту проблему, вы можете получить ключ учетной записи от кого-нибудь и подключение с именем и ключом, или можно попросить кого-нибудь для SAS учетной записи хранения и использовать для присоединения учетной записи хранения.
