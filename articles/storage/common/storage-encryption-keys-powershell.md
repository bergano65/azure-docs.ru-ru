...
При настройке шифрования с помощью управляемых клиентом ключей можно выбрать автоматический поворот ключа, используемого для шифрования при изменении версии в связанном хранилище ключей. Кроме того, можно явно указать версию ключа, которая будет использоваться для шифрования до тех пор, пока не будет обновлена версия ключа вручную.
...
Чтобы настроить шифрование для автоматического вращения ключей, управляемых клиентом, установите модуль [AZ. Storage](https://www.powershellgallery.com/packages/Az.Storage) , версия 2.0.0 или более поздняя.

Чтобы автоматически поворачивать ключи, управляемые клиентом, не указывайте версию ключа при настройке ключей, управляемых клиентом, для учетной записи хранения. Вызовите [Set-азсторажеаккаунт](/powershell/module/az.storage/set-azstorageaccount) , чтобы обновить параметры шифрования учетной записи хранения, как показано в следующем примере, и включите параметр **-кэйваултенкриптион** , чтобы включить управляемые клиентом ключи для учетной записи хранения. Не забудьте заменить значения заполнителей в квадратных скобках собственными значениями и использовать переменные, определенные в предыдущих примерах.
...
### <a name="configure-encryption-for-manual-rotation-of-key-versions"></a>Настройка шифрования для ручного смены основных версий
