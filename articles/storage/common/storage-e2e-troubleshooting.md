---
title: Устранение проблемных операций с помощью диагностики и анализа сообщений
...
* **Аналитика хранения данных Azure**. [Аналитика службы хранилища Azure](/rest/api/storageservices/Storage-Analytics) обеспечивает метрики и ведение журнала для службы хранилища Azure.
...
В этом учебнике мы рассмотрим сценарий, где метрики службы хранилища Azure указывают на низкий процент успешных операций для приложения, выполняющего обращения к службе хранилища Azure. Метрика с низким процентом успеха (показанная как **PercentSuccess** на [портале Azure](https://portal.azure.com) и в таблицах метрик) отслеживает успешное управление операций, но возвращает код состояния HTTP, который превышает 299. В файлах журналов хранилища на стороне сервера такие операции отображаются с состоянием транзакции **ClientOtherErrors**(Другие ошибки клиента). Дополнительные сведения о метрике низкого процента успешных операций см. в разделе [Метрики содержат низкие значения PercentSuccess, или в записях журналов аналитики присутствуют операции с состоянием транзакции ClientOtherErrors](storage-monitoring-diagnosing-troubleshooting.md#metrics-show-low-percent-success).
...
#### <a name="status-code-404-not-found-examples"></a>Примеры кода состояния 404 (не найдено)
...
#### <a name="status-code-409-conflict-examples"></a>Примеры кода состояния 409 (конфликт)
...
#### <a name="status-code-412-precondition-failed-examples"></a>Примеры кода состояния 412 (необходимое условие не выполнено)
...
Во-первых, нам нужно настроить журналы и метрики хранилища Azure, чтобы у нас были данные со стороны службы для анализа. Вы можете настроить журнал и метрики различными способами - через [портал Azure,](https://portal.azure.com)с помощью PowerShell, или программно. См [Метрики включить](storage-analytics-metrics.md#enable-metrics-using-the-azure-portal) и [Включить журнал](storage-analytics-logging.md#enable-storage-logging) для получения подробной информации о настройке регистрации и метрик.
...
После того, как приложение работает в течение определенного периода времени, можно просмотреть диаграммы метрик, которые отображаются на [портале Azure,](https://portal.azure.com) чтобы посмотреть, как работает ваш сервис.
...
Можно использовать программу командной строки AzCopy для скачивания этих файлов журнала на стороне сервера на локальный компьютер. Например, вы можете использовать следующую команду для загрузки файлов журнала для операций капли, которые `C:\Temp\Logs\Server`имели место 2 января 2015 года в папку; заменить `<storageaccountname>` имя учетной записи хранилища:
...
Дополнительные сведения об импорте данных журнала в анализатор сообщений (Майкрософт) см. в разделе [Получение данных сообщений](https://technet.microsoft.com/library/dn772437.aspx) на сайте TechNet.
...
2. На ленте панели инструментов выберите **New Viewer,** затем выберите **Аналитическую сетку,** чтобы открыть новую вкладку. Новая вкладка отображает все данные в файлах журнала без сгруппирования, фильтрации или правил цвета.
