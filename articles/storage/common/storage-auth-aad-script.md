---
title: Выполните команды Azure CLI или PowerShell под удостоверением Azure AD для доступа к службе хранилища Azure | Документация Майкрософт
...
Azure CLI поддерживает `--auth-mode` параметра для операции с данными в службе хранилища Azure:
...
1. Перед созданием контейнера, назначить [участник для данных больших двоичных объектов хранилища (Предварительная версия)](../../role-based-access-control/built-in-roles.md#storage-blob-data-contributor-preview) роли себе. Несмотря на то, что вы являетесь владельцем учетной записи, вы должны явные разрешения для выполнения операций с данными в учетной записи хранения. Дополнительные сведения о назначении роли RBAC см. в разделе [предоставить доступ к контейнерам Azure и очереди с RBAC на портале Azure (Предварительная версия)](storage-auth-aad-rbac.md).

    > [!IMPORTANT]
    > На этапе предварительной версии Azure AD с поддержкой больших двоичных объектов и очередей назначения ролей RBAC может занять до 5 минут для распространения.
