...
[Библиотека клиентов Azure Storage для .NET](/dotnet/api/overview/azure/storage?view=azure-dotnet) поддерживает шифрование данных в клиентских приложениях перед загрузкой в Azure Storage и расшифровку данных при загрузке клиенту. Библиотека также поддерживает интеграцию с [Azure Key Vault](https://azure.microsoft.com/services/key-vault/) для управления ключами хранения данных.
...
Загрузка зашифрованного капли включает в себя получение содержимого всей капли с помощью методов удобства **DownloadTo**/**BlobReadStream.** Зашифрованный ключ CEK расшифровывается и используется вместе с ключом IV (который в данном случае хранится как метаданные большого двоичного объекта) для передачи расшифрованных данных обратно пользователям.

Загрузка произвольного диапазона (методы**DownloadRange)** в зашифрованном blob включает в себя корректировку диапазона, предоставляемого пользователями, чтобы получить небольшое количество дополнительных данных, которые могут быть использованы для успешной расшифровки запрашиваемого диапазона.
