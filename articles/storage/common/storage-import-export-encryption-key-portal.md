...
2. У вас уже есть Azure Key Vault с ключом, который можно использовать для защиты ключа BitLocker. Сведения о создании Key Vault с помощью портала Azure см. в статье [Краткое руководство. Настройка и получение секрета из Azure Key Vault с помощью портала Azure](../../key-vault/secrets/quick-create-portal.md).
    ...
    - Существующее хранилище Key Vault должно иметь ключ RSA размером 2048 бит или более. Дополнительные сведения о **ключах Key Vault** см. в статье [Сведения о ключах, секретах и сертификатах](../../key-vault/about-keys-secrets-and-certificates.md#key-vault-keys).
