...
| Код ошибки     |Подробности     | Восстанавливаемых?    |
|----------------|------------|-----------------|
| кмкерроракцессревокед | Доступ к ключу, управляемому клиентом, отозван.                                                       | Да, проверить, если: <ol><li>Хранилище ключей по-прежнему имеет MSI в политике доступа.</li><li>Политика доступа включает разрешения на получение, перенос и распаковку.</li><li>Если хранилище ключей находится в виртуальной сети за брандмауэром, установите флажок **разрешить включение доверенных служб Майкрософт** .</li><li>Проверьте, сброшен ли MSI ресурса задания для `None` использования API.<br>Если да, то снова установите значение `Identity = SystemAssigned` . Это повторно создает удостоверение для ресурса задания.<br>После создания нового удостоверения включите `Get` `Wrap` разрешения, и `Unwrap` для нового удостоверения в политике доступа хранилища ключей.</li></ol>                                                                                            |
| кмкерроркэйдисаблед      | Ключ, управляемый клиентом, отключен.                                         | Да, включив версию ключа     |
| кмкерроркэйнотфаунд      | Не удается найти ключ, управляемый клиентом. | Да, если ключ был удален, но все еще находится в пределах продолжительности очистки, используется команда [Отменить удаление ключа хранилища ключей](https://docs.microsoft.com/powershell/module/az.keyvault/undo-azkeyvaultkeyremoval).<br>Кроме <ol><li>Да, если пользователь имеет резервную копию ключа и восстанавливает его.</li><li>Нет, в противном случае.</li></ol>
| кмкеррорваултнотфаунд |Не удается найти хранилище ключей для управляемого клиентом ключа. |   Если хранилище ключей удалено:<ol><li>Да, если она находится в течение длительного времени защиты, выполнив действия, описанные в статье [восстановление хранилища ключей](https://docs.microsoft.com/azure/key-vault/general/soft-delete-powershell#recovering-a-key-vault).</li><li>Нет, если он выходит за пределы продолжительности очистки от вирусов.</li></ol><br>В противном случае, если хранилище ключей было перенесено на другой клиент, его можно восстановить, выполнив одно из следующих действий.<ol><li>Верните хранилище ключей к старому клиенту.</li><li>Установите `Identity = None` , а затем присвойте значение `Identity = SystemAssigned` . Это приведет к удалению и повторному созданию удостоверения после создания нового удостоверения. Включите `Get` `Wrap` разрешения, и `Unwrap` для нового удостоверения в политике доступа хранилища ключей.</li></ol>|
