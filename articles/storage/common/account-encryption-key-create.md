...
Учетная запись хранения должна иметь тип общего назначения v2 и должна быть настроена для локально избыточного хранилища (LRS). Вы можете создать учетную запись хранения и настроить ее для использования ключа шифрования учетной записи с помощью Azure CLI или шаблона Azure Resource Manager.
