...
1. В окне **Добавление назначения ролей** выберите роль службы хранилища Azure, которую необходимо назначить. Затем найдите субъект безопасности, которому нужно назначить эту роль.
    ...
    ![Снимок экрана, показывающий, как назначить роль Azure](media/storage-auth-aad-rbac-portal/add-rbac-role.png)

1. Щелкните **Сохранить**. Удостоверение, которому назначена роль RBAC, будет отображаться в списке под этой ролью. Например, на следующем изображении показано, что у добавленных пользователей теперь есть разрешения на чтение для данных в контейнере с именем *sample-container*.
