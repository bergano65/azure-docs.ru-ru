...
description: Сведения о способах защиты учетных записей хранения Azure, включая безопасность и авторизацию в плоскости управления, безопасность сети, шифрование и т. д.
services: storage
author: tamram
ms.service: storage
ms.topic: article
...
Служба хранилища Azure предоставляет исчерпывающий набор возможностей обеспечения безопасности, которые позволяют организациям создавать и развертывать безопасные приложения.

- Все данные (включая метаданные), записанные в хранилище Azure, автоматически шифруются с помощью [Шифрование службы хранилища (SSE)](storage-service-encryption.md). Дополнительные сведения см. в статье [объявление о шифровании по умолчанию для больших двоичных объектов Azure, файлов, таблиц и очередей хранилища](https://azure.microsoft.com/blog/announcing-default-encryption-for-azure-blobs-files-table-and-queue-storage/).
- Azure Active Directory (Azure AD) и управление доступом на основе ролей (RBAC) поддерживаются как для операций управления ресурсами, так и для операций с плоскостью данных.   
    ...
    - Интеграция Azure AD поддерживается для операций с данными большого двоичного объекта и очереди. Роли RBAC могут быть ограничены подпиской, группой ресурсов, учетной записью хранения, отдельным контейнером или очередью. Роли могут быть назначены субъекту безопасности или управляемому удостоверению для ресурсов Azure. Дополнительные сведения см. в статье об [аутентификации доступа к службе хранилища Azure с помощью Azure Active Directory](storage-auth-aad.md).
- Данные могут быть защищены при передаче между приложением и Azure с помощью [шифрования на стороне клиента](../storage-client-side-encryption.md), HTTPS или SMB 3,0.  
...
- Безопасность сетевого уровня между компонентами приложения и хранилищем можно включить с помощью брандмауэра хранилища, конечных точек службы или частных конечных точек.

В этой статье содержатся общие сведения о функциях безопасности, которые можно использовать в службе хранилища Azure. Ссылки на статьи содержат дополнительные сведения о каждой возможности.

Ниже приведены области, описанные в этой статье.

* [Безопасность плоскости управления](#management-plane-security) — защита доступа на уровне ресурсов к учетной записи хранения

  Плоскость управления состоит из операций, используемых для управления учетной записью хранения. В этом разделе рассматривается модель развертывания с помощью Azure Resource Manager и использование управления доступом на основе ролей (RBAC) для контроля доступа к учетным записям хранения. Кроме того, здесь представлены сведения об управлении ключами учетной записи хранения и их повторном создании.

* [Безопасность сети](#network-security) — защита доступа на уровне сети к учетной записи хранения

  В этом разделе описывается, как можно защитить доступ на уровне сети к конечным точкам служб хранилища. Здесь рассказывается о том, как можно использовать брандмауэр службы хранилища, чтобы разрешить доступ к данным из конкретных виртуальных сетей или диапазонов IP-адресов. В нем также рассматривается использование конечных точек службы и частных конечных точек с учетными записями хранения.

* [Авторизация](#authorization) — авторизация доступа к данным

  В этом разделе описывается доступ к объектам данных в вашей учетной записи хранения, таким как большие двоичные объекты, файлы, очереди и таблицы, с использованием подписанных URL-запросов и хранимых политик доступа. Будут рассмотрены подписанные URL-адреса как уровня службы, так и уровня учетной записи. Вы также узнаете, как ограничить доступ к определенному IP-адресу (или диапазону IP-адресов), как запретить использование любых протоколов, кроме HTTPS, и как отозвать подписанный URL-адрес, не дожидаясь истечения срока его действия.

* [Шифрование при передаче](#encryption-in-transit)

  В этом разделе рассматривается, как можно защитить данные при передаче в службу хранилища Azure или из нее. Также описаны рекомендуемые способы применения протокола HTTPS и методы шифрования, используемые протоколом SMB 3.0 для файловых ресурсов Azure. Мы также обсудим шифрование на стороне клиента, которое позволяет шифровать данные перед передачей в хранилище и расшифровывать данные после их передачи из хранилища.
...
Существует несколько причин повторного создания ключей учетной записи хранения.

* Вы можете периодически повторно создавать их для обеспечения безопасности.
* Вы можете повторно создать ключи учетной записи хранения, если безопасность приложения или сети скомпрометирована.
* Еще один экземпляр для повторного создания ключа — это то, что члены группы, имеющие доступ к ключам, оставляют. Подписанные URL-адреса были разработаны в основном для решения этой ситуации: необходимо предоставить общий доступ к строке подключения SAS уровня учетной записи или токену, а не к общим ключам доступа с большинством пользователей или приложений.

#### <a name="key-regeneration-plan"></a>План повторного создания ключей
Не следует повторно создавать ключ доступа без планирования. Внезапное повторное создание ключа может заблокировать доступ к учетной записи хранения для существующих приложений, что приводит к существенному сбою. Учетные записи хранения Azure предоставляют два ключа, что позволяет повторно создавать один ключ за раз.

Перед повторной созданием ключей убедитесь, что у вас есть список всех приложений, зависящих от учетной записи хранения, а также все другие службы, которые используются в Azure. Например, если вы используете службы мультимедиа Azure, используйте учетную запись хранения, после повторного создания ключа необходимо заново синхронизировать ключи доступа со службой мультимедиа. Если вы используете такое приложение, как Обозреватель хранилищ, необходимо также указать новые ключи для этих приложений. Если у вас есть виртуальные машины, файлы VHD которых хранятся в учетной записи хранения, повторное создание ключей не повлияет на их работу.
...
Другой вариант — поместить ключ учетной записи хранения в [хранилище ключей Azure](https://azure.microsoft.com/services/key-vault/) в качестве секрета, чтобы приложения извлекали его оттуда. При повторном создании ключа и обновлении хранилища ключей Azure приложения не нужно будет развертывать повторно, так как они будут получать новый ключ из хранилища автоматически. Приложение может считывать ключ каждый раз, когда оно ему требуется, или же приложение может кэшировать его в памяти и если оно завершается сбоем при его использовании, извлеките ключ из Azure Key Vault.

Хранилище ключей Azure обеспечивает еще один уровень безопасности для ключей к хранилищу данных. Использование Key Vault позволяет избежать записи ключей хранения в файлах конфигурации приложений. Это также предотвращает предоставление ключей всем пользователям, у которых есть доступ к этим файлам конфигурации.

Azure Key Vault также имеет преимущество использования Azure AD для управления доступом к ключам. Вы можете предоставить доступ к конкретным приложениям, которые должны получать ключи из Key Vault, не предоставляя их другим приложениям, которым не требуется доступ к ключам.
...
Сетевая безопасность позволяет ограничить доступ к данным в учетной записи хранения Azure из выбранных сетей. Вы можете использовать брандмауэр службы хранилища Azure, чтобы ограничить доступ клиентов от конкретных диапазонов общедоступных IP-адресов, выбрать виртуальные сети (виртуальных сетей) в Azure или конкретные ресурсы Azure. Вы также можете создать закрытую конечную точку для учетной записи хранения в виртуальной сети, которой требуется доступ, и блокировать доступ через общедоступную конечную точку.

Правила доступа к сети для учетной записи хранения можно настроить на вкладке [брандмауэры и виртуальные сети](storage-network-security.md) в портал Azure. С помощью брандмауэра хранилища можно запретить доступ к общедоступному Интернет и предоставить доступ к выбранным клиентам на основе настроенных правил сети.

[Частные конечные точки](../../private-link/private-endpoint-overview.md) также можно использовать для частного и безопасного подключения к учетной записи хранения из виртуальной сети с помощью [частных ссылок](../../private-link/private-link-overview.md).

Правила брандмауэра хранилища применяются только к общедоступной конечной точке учетной записи хранения. Подсеть, в которой размещается частная конечная точка для учетной записи хранения, получает неявный доступ к учетной записи при утверждении создания этой частной конечной точки.

> [!NOTE]
> Правила брандмауэра хранилища неприменимы к операциям управления хранилищем, проводимым с помощью портал Azure и API управления хранилищем Azure.

### <a name="access-rules-for-public-ip-address-ranges"></a>Правила доступа для диапазонов общедоступных IP-адресов
Брандмауэр службы хранилища Azure можно использовать для ограничения доступа к учетной записи хранения из конкретных диапазонов общедоступных IP-адресов. Правила IP-адресов можно использовать для ограничения доступа к определенным веб-службам, которые обмениваются данными с фиксированной конечной точкой общедоступного IP-адреса, или для выбора локальных сетей.

### <a name="access-rules-for-azure-virtual-networks"></a>Правила доступа для виртуальных сетей Azure
По умолчанию учетные записи хранения принимают подключения от клиентов в любой сети. Можно ограничить доступ клиента к данным в учетной записи хранения к выбранным сетям с помощью брандмауэра хранилища. [Конечные точки службы](../../virtual-network/virtual-network-service-endpoints-overview.md) позволяют выполнять маршрутизацию трафика из виртуальной сети Azure в учетную запись хранения. 

### <a name="granting-access-to-specific-trusted-resource-instances"></a>Предоставление доступа к конкретным доверенным экземплярам ресурсов
Вы можете разрешить [подмножеству доверенных служб Azure](storage-network-security.md#trusted-microsoft-services) доступ к учетной записи хранения через брандмауэр, используя строгую проверку подлинности на основе типа ресурса службы или экземпляра ресурса.

Для служб, поддерживающих доступ на основе экземпляров ресурсов через брандмауэр хранилища, только выбранный экземпляр может получить доступ к данным в учетной записи хранения. В этом случае служба должна поддерживать проверку подлинности экземпляра ресурса с помощью [управляемых идентификаторов](../../active-directory/managed-identities-azure-resources/overview.md), назначенных системой.

### <a name="using-private-endpoints-for-securing-connections"></a>Использование частных конечных точек для защиты подключений
Служба хранилища Azure поддерживает частные конечные точки, которые обеспечивают безопасный доступ к учетной записи хранения из виртуальной сети Azure. Частные конечные точки присваивают службе хранилища частный IP-адрес из адресного пространства виртуальной сети. При использовании частных конечных точек строка подключения к хранилищу перенаправляет трафик, предназначенный для учетной записи хранения, на частный IP-адрес. Соединение между частной конечной точкой и учетной записью хранения использует закрытую ссылку. С помощью частных конечных точек можно блокировать утечка данных из виртуальной сети.

Локальные сети, подключенные через VPN или [которыми](../../expressroute/expressroute-locations.md) частный пиринг и другие одноранговые виртуальные сети, также могут получить доступ к учетной записи хранения через закрытую конечную точку. Частная конечная точка для учетных записей хранения может быть создана в виртуальной сети в любом регионе, что обеспечивает безопасный глобальный доступ. Вы также можете создать частные конечные точки для учетных записей хранения в других [Azure Active Directory](../../active-directory/fundamentals/active-directory-whatis.md) клиентах.
...
* Это руководство по использованию клиентской библиотеки .NET для создания подписей общего доступа и хранимых политик доступа.
