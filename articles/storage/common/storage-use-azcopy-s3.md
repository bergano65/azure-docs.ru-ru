...
> [!IMPORTANT]
> Сейчас эта функция доступна в предварительной версии. Если вы решили удалить данные из контейнеров S3 после операции копирования, убедитесь, что данные были правильно скопированы в учетную запись хранения перед удалением данных.
...
В составе команды AzCopy `copy` можно указать необязательный `s2s-invalid-metadata-handle` флаг, указывающий, как вы хотите управлять файлами, в которых метаданные файла содержат несовместимые имена ключей. В следующей таблице описывается каждое значение флага.
