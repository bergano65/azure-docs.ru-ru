...
>
> Также можно использовать виртуальные URL-адреса в стиле (например, `http://bucket.s3.amazonaws.com`). 
>
...
Кроме того, так как AzCopy выполняет копирование файлов, он проверяет наличие конфликтов именования и пытается разрешить их. Например, если есть контейнеры с именем `bucket-name` и `bucket.name`, AzCopy разрешает контейнер с именем `bucket.name` сначала в `bucket-name`, а затем в `bucket-name-2`.
...
В рамках команды AzCopy `copy` можно указать значение для необязательного флага `s2s-invalid-metadata-handle`, который указывает, как следует выполнять обработку файлов, в которых метаданные файла содержат несовместимые имена ключей. В следующей таблице описывается каждое значение флага.
