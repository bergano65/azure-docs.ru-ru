...
Azure Monitor предоставляет несколько способов доступа к метрикам. Вы можете получить к ним доступ с [портала Azure,](https://portal.azure.com)APIs Azure Monitor (REST и .NET) и аналитических решений, таких как концентраторы событий. Дополнительные сведения см. в статье [Обзор метрик в Microsoft Azure](../../monitoring-and-diagnostics/monitoring-overview-metrics.md).
...
### <a name="access-metrics-with-the-net-sdk"></a>Показатели доступа с помощью .NET SDK

Azure Monitor предоставляет [.NET SDK](https://www.nuget.org/packages/Microsoft.Azure.Management.Monitor/) для чтения определения и значений метрики. [Пример кода](https://azure.microsoft.com/resources/samples/monitor-dotnet-metrics-api/) демонстрирует использование пакета SDK с различными параметрами. Необходимо использовать `0.18.0-preview` или более поздней версии для метрик хранилища. Идентификатор ресурсов используется в .NET SDK. Дополнительные сведения см. в статье, посвященной обзору идентификатора ресурса для служб в службе хранилища.

В следующем примере показано, как использовать Azure Monitor .NET SDK для чтения метрик хранения данных.

#### <a name="list-account-level-metric-definition-with-the-net-sdk"></a>Перечислите определение уровня учетной записи с помощью .NET SDK
...
#### <a name="read-metric-values-with-the-net-sdk"></a>Читать значения метрики с помощью .NET SDK
...
#### <a name="read-multi-dimensional-metric-values-with-the-net-sdk"></a>Читать многомерные метрические значения с помощью .NET SDK
...
| Имя метрики | Описание |
| ------------------- | ----------------- |
| BlobCapacity | Объем хранилища BLOB-объектов, используемый в учетной записи хранения. <br/><br/> Единица измерения: байт <br/> Тип агрегирования: средний <br/> Пример значения: 1024 <br/> Размеры: **BlobType**, и **BlobTier** ([Определение](#metrics-dimensions)) |
| BlobCount    | Количество больших двоичных объектов, хранящихся в учетной записи хранения. <br/><br/> Единица измерения: число <br/> Тип агрегирования: средний <br/> Пример значения: 1024 <br/> Размеры: **BlobType**, и **BlobTier** ([Определение](#metrics-dimensions)) |
...
| Исходящие | Объем исходящих данных. Этот номер включает в себя выход к внешнему клиенту из Хранилища Azure, а также выход в Azure. В результате это количество не отражает исходящий трафик для тарификации. <br/><br/> Единица измерения: байт <br/> Тип агрегирования: итог <br/> Применимые измерения: GeoType, ApiName и Authentication ([определение](#metrics-dimensions)) <br/> Пример значения: 1024 |
...
| **BlobTier** | Хранилище Azure предлагает различные уровни доступа, которые позволяют хранить данные объектов blob наиболее экономично. Подробнее читайте в [узлах Azure Storage.](../blobs/storage-blob-storage-tiers.md) Поддерживаются следующие значения: <br/> <li>**Горячий**: Горячий ярус</li> <li>**Прохладный**: Прохладный ярус</li> <li>**Архив**: Архив ный уровень</li> <li>**Премиум**: Премиум-уровень для блока капли</li> <li>**P4/P6/P10/P15/P20/P30/P40/P50/P60/P60**: Типы уровней для премиум-страницы</li> <li>**Стандарт**: Тип уровня для стандартной страницы Blob</li> <li>**Неявный**: Тип уровня для учетной записи хранения v1 общего назначения</li> |
| **GeoType** | Транзакции из основного или дополнительного кластера. Доступные значения включают **Начальные** и **Вторичные.** Применимо к геоизбыточному хранилищу с доступом только для чтения (RA-GRS) при считывании объектов из дополнительного клиента. |
| **ОтветТип** | Тип ответа транзакции. Доступные значения: <br/><br/> <li>**ServerOtherError**: Все другие ошибки на стороне сервера, за исключением описанных </li> <li>**ServerBusyError**: Проверенный запрос, который вернул код состояния HTTP 503. </li> <li>**ServerTimeoutError**: Приуроченный аутентифицированный запрос, который вернул код статуса HTTP 500. Время ожидания истекло из-за ошибки на стороне сервера. </li> <li>**АвторизацияОшибка**: Аутентифицированный запрос, который не удалось из-за несанкционированного доступа к данным или сбоя авторизации. </li> <li>**NetworkError**: Аутентифицированный запрос, который не удалось из-за ошибок сети. Чаще всего происходит, когда клиент преждевременно закрывает подключение до истечения времени ожидания. </li>  <li>**ClientAccountBandwidthThrottlingError**: Запрос регулируется на пропускную способность за превышение [пределов масштабируемости учетной записи хранилища.](scalability-targets-standard-account.md)</li><li>**ClientAccountRequestThrottlingError**: Запрос регулируется по ставке запроса за превышение [пределов масштабируемости хранилища.](scalability-targets-standard-account.md)<li>**ClientThlingError**: Другие ошибки регулирования на стороне клиента. ClientAccountBandwidthThrottlingError и ClientAccountRequestThrottlingError исключены.</li> <li>**ClientTimeoutError**: Приуроченный аутентифицированный запрос, который вернул код статуса HTTP 500. Если для времени ожидания сети клиента или времени ожидания запроса задано меньшее значение, чем требовалось для службы хранения, это предусмотренное время ожидания. В противном случае оно будет отмечено как ServerTimeoutError.</li> </li> <li>**ClientOtherError**: Все другие ошибки на стороне клиента, кроме описанных. </li> <li>**Успех**: Успешный запрос</li> <li> **SuccessWithThrottling**: Успешный запрос, когда клиент SMB получает задушил в первой попытке (ы), но успешно после повторений.</li> |
| **ApiName** | Имя операции. Пример: <br/> <li>**СозданиеКонтейнера**</li> <li>**DeleteBlob**</li> <li>**GetBlob**</li> Для всех имен операций. Дополнительные сведения см. в этой [статье](/rest/api/storageservices/storage-analytics-logged-operations-and-status-messages). |
| **Проверка подлинности** | Тип аутентификации, используемой в транзакциях. Доступные значения: <br/> <li>**AccountKey**: Транзакция проверяется ключом к учетной записи хранилища.</li> <li>**SAS**: Транзакция проверяется с помощью общих подписей доступа.</li> <li>**OAuth**: Транзакция проверяется с помощью токенов доступа OAuth.</li> <li>**Аноним:** Транзакция запрашивается анонимно. Он не включает предполетные запросы.</li> <li>**AnonymousPreflight**: Транзакция является предполетным запросом.</li> |
