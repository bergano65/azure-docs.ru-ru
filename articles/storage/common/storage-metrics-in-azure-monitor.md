...
Azure Monitor предоставляет несколько способов доступа к метрикам. К ним можно получить доступ из [портал Azure](https://portal.azure.com), Azure Monitor API-интерфейсов (RESTful и .NET) и решений анализа, таких как концентраторы событий. Дополнительные сведения см. в статье [Обзор метрик в Microsoft Azure](../../monitoring-and-diagnostics/monitoring-overview-metrics.md).

Метрики включены по умолчанию, и вы можете получить доступ к данным за последние 93 дня. Если необходимо хранить данные метрик в течение длительного периода времени, вы можете архивировать их в учетную запись хранения Azure. Для настройки используйте [параметры диагностики](../../azure-monitor/platform/platform-logs-overview.md) в Azure Monitor.
...
В следующем примере показано, как использовать пакет SDK для .NET Azure Monitor для чтения метрик хранилища.

#### <a name="list-account-level-metric-definition-with-the-net-sdk"></a>Вывод списка определения метрик на уровне учетной записи с помощью пакета SDK для .NET
...
#### <a name="read-multi-dimensional-metric-values-with-the-net-sdk"></a>Чтение значений многомерных метрик с помощью пакета SDK для .NET
...
```
* Идентификатор ресурса служба очередей
```
