...
description: Узнайте, как настроить инструмент подготовки и исправления дисков для службы импорта и экспорта Azure. Эта статья относится к версии 1 средства импорта и экспорта.
author: alkohli
services: storage
ms.service: storage
ms.topic: how-to
...
-   Перед созданием задания импорта это средство можно использовать для копирования данных на жесткие диски, поставляемые в центр обработки данных Windows Azure.  
...
При подготовке дисков для задания импорта необходимо выполнить следующие предварительные требования.  

-   У вас должна быть активная подписка Azure.  

-   Ваша подписка должна включать учетную запись хранения с достаточным объемом свободного места для хранения импортируемых файлов.  
...
-   Вам потребуется один или несколько дисков, содержащих данные для импорта или пустых 3,5-дюймовых жестких дисков SATA, подключенных к компьютеру копирования.  
...
- Hddid.dll  
  ...
  Скопируйте эти файлы в рабочий каталог, например `c:\WAImportExport`. Затем откройте окно командной строки в режиме администратора и укажите в качестве текущего каталога текущий каталог.  
