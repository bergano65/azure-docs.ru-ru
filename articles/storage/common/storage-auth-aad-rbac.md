---
title: Управлять правами доступа Azure AD к контейнерам и очереди с помощью RBAC (Предварительная версия) — службы хранилища Azure с помощью портала Azure | Документация Майкрософт
...
ms.locfileid: "57431862"
---
# <a name="grant-access-to-azure-containers-and-queues-with-rbac-in-the-azure-portal-preview"></a>Предоставление доступа к контейнерам Azure и очереди с RBAC на портале Azure (Предварительная версия)
...
1. Назначьте соответствующую роль RBAC хранилища Azure для предоставления доступа к контейнерам или очереди. Доступ на чтение, назначить **модуль чтения данных больших двоичных объектов (Предварительная версия)** или **модуль чтения данных очереди (Предварительная версия)** роли. Для чтения, записи и доступ на удаление, назначение **участник для данных больших двоичных объектов (Предварительная версия)** или **участник для данных очереди (Предварительная версия)** роли. Можно также назначить пользовательскую роль.
...
Когда вы назначаете встроенные или пользовательские роли для службы хранилища Azure для субъекта безопасности, вы предоставляете разрешения субъекта безопасности, чтобы выполнять операции с данными в вашей учетной записи хранения. Встроенный **модуль чтения данных** роли предоставляют разрешения на чтение для данных в контейнере или очереди, тогда как встроенные в **участник для данных** роли предоставляют чтение, запись и удаление разрешений на контейнер или очередь. Разрешения действуют в указанный ресурс.  

Например, если вы назначаете **участник для данных больших двоичных объектов хранилища (Предварительная версия)** роли для пользователя Mary на уровне контейнера с именем **образец контейнера**, затем Мэри предоставляются разрешения на чтение, запись и удаление доступ ко всем в этом контейнере больших двоичных объектов.

Тем не менее, если Владимир хочет просмотреть большой двоичный объект на портале Azure, то **участник для данных больших двоичных объектов хранилища (Предварительная версия)** роли сама по себе не будет предоставлять достаточные разрешения на доступ к на портале в большой двоичный объект, чтобы просмотреть его. Дополнительные необходимые разрешения Azure AD для перемещения по порталу и просматривать другие ресурсы, существуют ли видимым.
...
1. Из **назначение доступа к** раскрывающегося списка, выберите **пользователя, группы или субъекта-службы Azure AD**.
...
## <a name="use-azure-ad-credentials-with-the-portal"></a>Использовать учетные данные Azure AD с помощью портала

Для доступа к BLOB-объектов или очередей на портале Azure, используя свои учетные данные Azure AD, воспользуйтесь ссылками предварительной версии, показано на следующем рисунке:

![Доступа к BLOB-объектов или очередей учетные данные Azure AD на портале](media/storage-auth-aad-rbac/access-data-azure-ad.png)

При доступе к BLOB-объектов или очереди данных, с помощью ссылки рабочей среде, а не ссылки предварительной версии, портал Azure использует ключ учетной записи для авторизации доступа, а не с помощью Azure AD.
