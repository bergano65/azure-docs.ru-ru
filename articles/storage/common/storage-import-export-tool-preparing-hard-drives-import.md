...
- **Конфигурация компьютера**
  ...
  - На компьютере должна быть установлена платформа .NET Framework 4. См. в разделе [часто задаваемые вопросы о](#faq) по проверке, если на компьютере установлена платформа .NET Framework.
...
| Disposition | **[Необязательный параметр]** rename &#124; no-overwrite &#124; overwrite <br/> Это поле определяет поведение копирования во время импорта при передаче данных с диска в учетную запись хранения. Доступные параметры: rename&#124, overwrite&#124, no-overwrite. Если ничего не указано, то по умолчанию используется параметр rename. <br/><br/>**rename** — при наличии двух объектов с одинаковым именем создает копию в целевом расположении.<br/>overwrite — заменяет более старый файл новым. Приоритет имеет последний измененный файл.<br/>**no-overwrite** — пропускает запись имеющегося файла.|
...
Каталог журнала определяет каталог, используемый для хранения файлов подробный журналов и некоторых временных файлов манифеста. Если каталог не указан, вместо него будет использоваться текущий каталог. В этот каталог записываются подробные журналы.
...
Перейдите к упомянутому выше каталогу на своем целевом компьютере, где нужно запустить средство. Найдите папку, имя которой начинается с v4. Если эта папка отсутствует, это означает, что на компьютере не установлена платформа .NET 4. Вы можете скачать .NET 4 на виртуальной машине с помощью [Microsoft .NET Framework 4 (веб-установщик)](https://www.microsoft.com/download/details.aspx?id=17851).
