...
* [Рекомендации по устранению неполадок]
  ...
  * [Клиент получает сообщения HTTP 409 (с конфликтом)]
  ...
  * [Возникла другая ошибка службы хранилища]
...
* [Приложения]
  * [Приложение 1. Использование Fiddler для записи трафика HTTP и HTTPS]
  * [Приложение 2. Использование Wireshark для записи сетевого трафика]
  * [Приложение 3. Отслеживание сетевого трафика с помощью Microsoft Message Analyzer]
  * [Приложение 4. Просмотр метрик и данных журнала с помощью Excel]
...
Вы можете выбрать, какие почасовые метрики должны отображаться в [портал Azure](https://portal.azure.com) и настроить правила, уведомляющие администраторов по электронной почте каждый раз, когда Почасовая метрика превысит определенный порог. Дополнительные сведения см. в статье [Создание оповещений для служб Azure с помощью портала Azure](/azure/monitoring-and-diagnostics/monitoring-overview-alerts).

Рекомендуется просмотреть [Azure Monitor для хранилища](../../azure-monitor/insights/storage-insights-overview.md) (Предварительная версия). Это функция Azure Monitor, которая обеспечивает полный мониторинг учетных записей хранения Azure, обеспечивая единое представление производительности, емкости и доступности служб хранилища Azure. Для этого не требуется включать или настраивать ничего, и вы можете немедленно просмотреть эти метрики из предварительно определенных интерактивных диаграмм и других включенных визуализаций.
...
В [портал Azure](https://portal.azure.com)можно добавить правила генерации оповещений, чтобы уведомить пользователя о том, что **доступность** службы ниже указанного порогового значения.
...
В [портал Azure](https://portal.azure.com)можно добавить правила оповещений, чтобы уведомить вас, если какая-либо из метрик производительности для этой службы ниже или превышает указанное пороговое значение.
...
* [Коды ошибок службы BLOB-объектов](https://msdn.microsoft.com/library/azure/dd179439.aspx)
...
* [Коды ошибок службы файлов](https://msdn.microsoft.com/library/azure/dn690119.aspx)
...
## <a name="end-to-end-tracing"></a><a name="end-to-end-tracing"></a>Сквозная трассировка
...
* В журнале на стороне клиента, создаваемом клиентской библиотекой хранилища, идентификатор запроса клиента отображается в поле **идентификатор запроса клиента** каждой записи журнала, относящейся к запросу.
* В трассировке сети, например в записи, захваченной Fiddler, идентификатор запроса клиента отображается в сообщениях запроса как значение заголовка HTTP **x-MS-Client-Request-ID** .
...
* В трассировке сети, например в записи, захваченной Fiddler, идентификатор запроса сервера отображается в ответных сообщениях как значение заголовка HTTP **x-MS-Request-ID** .
* В журнале на стороне клиента, создаваемом клиентской библиотекой хранилища, идентификатор запроса сервера отображается в столбце " **текст операции** " для записи журнала, в которой отображаются сведения об ответе сервера.
...
Для служб таблиц и очередей алгоритм Nagle также может вызвать высокий **AverageE2ELatency** по сравнению с **значения averageserverlatency**. Дополнительные сведения см. в разделе [алгоритма Nagle, который не является понятным для небольших запросов](https://blogs.msdn.com/b/windowsazurestorage/archive/2010/06/25/nagle-s-algorithm-is-not-friendly-towards-small-requests.aspx). Вы можете отключить алгоритм Нейгла в коде с помощью класса **ServicePointManager** в пространстве имен **System.Net**. Это следует сделать до выполнения вызовов к службам таблиц или очередей в приложении, так как на открытые подключения это не распространяется. Приведенный ниже пример взят из метода **Application_Start** в рабочей роли.
...
Ошибки регулирования происходят при превышении целевых показателей масштабируемости для службы хранилища. Регулирование службы хранилища происходит для того, чтобы ни один из клиентов не мог использовать службу хранилища в ущерб другим. Дополнительные сведения см. в разделе [целевые показатели масштабируемости и производительности для учетных записей](scalability-targets-standard-account.md) хранения уровня "Стандартный". подробные сведения о целевых показателях масштабируемости для учетных записей хранения и целевых показателей производительности для секций в учетных
...
* [Постоянное увеличение PercentThrottlingError ошибки]
...
| --- | --- | --- | --- | --- |
| Microsoft. Azure. Storage |Данные |3 |85d077ab -… |Начинается выполнение операции с расположением «Основное» в режиме расположения PrimaryOnly. |
| Microsoft. Azure. Storage |Данные |3 |85d077ab -… |Запуск синхронного запроса к<https://domemaildist.blob.core.windows.netazureimblobcontainer/blobCreatedViaSAS.txt?sv=2014-02-14&sr=c&si=mypolicy&sig=OFnd4Rd7z01fIvh%2BmcR6zbudIH2F5Ikm%2FyhNYZEmJNQ%3D&api-version=2014-02-14> |
| Microsoft. Azure. Storage |Данные |3 |85d077ab -… |Waiting for response (Ожидание ответа). |
...
| Microsoft. Azure. Storage |Данные |3 |85d077ab -… |Response received. Код состояния = 403; идентификатор запроса = 9d67c64a-64ed-4b0d-9515-3b14bbcdc63d; Content-MD5 = ; ETag = . |
...
| Microsoft. Azure. Storage |Данные |3 |85d077ab -… |Проверка необходимости в повторной попытке выполнения операции. Число повторных попыток = 0; код состояния HTTP = 403; исключение = удаленный сервер вернул ошибку 403 (Запрещено). |
| Microsoft. Azure. Storage |Данные |3 |85d077ab -… |Следующим установлено расположение «Основное» в соответствием с режимом расположения. |
...
* Соответствует ли параметр версии в ключе SAS (например, **SV = 2015-04-05**) версии используемой клиентской библиотеки хранилища? Рекомендуется всегда использовать последнюю версию [клиентской библиотеки хранилища](https://www.nuget.org/packages/WindowsAzure.Storage/).
...
В этом примере журнал показывает, что клиент переводит запросы из метода **CreateIfNotExists** (идентификатор запроса — e2d06d78...) в запросы из метода **UploadFromStream** (de8b1c3c-...). Это происходит потому, что клиентское приложение асинхронно вызывает эти методы. Измените код с асинхронными вызовами в клиенте таким образом, чтобы он создавал контейнер до того, как попытается передать какие-либо данные в большой двоичный объект в этом контейнере. В идеале нужно создавать все контейнеры заранее.
...
Сведения об исключении в клиенте включают идентификатор запроса (7e84f12d...), назначенный службой таблиц для запроса. Эти сведения можно использовать для поиска сведений о запросе в журналах хранилища на стороне сервера путем поиска в столбце **request-ID-Header** в файле журнала. Кроме того, можно проверить метрики для того, чтобы выяснить, когда возникают такие ошибки, а затем выполнить поиск в файлах журналов по зафиксированному в метриках времени. Эта запись журнала указывает на то, что удаление не выполнено и возвращено сообщение о состоянии HTTP (404) Client Other Error (Другая ошибка клиента). Эта же запись журнала также включает идентификатор запроса, сформированный клиентом в столбце **Client-Request-ID** (813ea74f...).
...
* [Ошибка "значение одного из заголовков HTTP имеет неправильный формат" при использовании эмулятора хранения]
...
## <a name="appendices"></a><a name="appendices"></a>Приложениями
...
[Fiddler](https://www.telerik.com/fiddler) — это удобное средство для анализа трафика HTTP и HTTPS между вашим клиентским приложением и используемой службой хранилища Azure.
...
Чтобы ограничить объем трафика, Fiddler записи, можно использовать фильтры, настроенные на вкладке **фильтры** . На следующем снимке экрана показан фильтр, который фиксирует только трафик, отправленный в конечную точку хранилища **contosoemaildist.Table.Core.Windows.NET** .
...
[Средства ведения журналов хранилища]: #storage-logging-tools
...
[Временное увеличение в PercentThrottlingError]: #transient-increase-in-PercentThrottlingError
[Постоянное увеличение PercentThrottlingError ошибки]: #permanent-increase-in-PercentThrottlingError
...
[Клиент получает сообщения HTTP 409 (с конфликтом)]: #the-client-is-receiving-409-messages
...
[Метрики емкости показывают непредвиденное увеличение использования емкости хранилища.]: #capacity-metrics-show-an-unexpected-increase
...
[Ошибка "значение одного из заголовков HTTP имеет неправильный формат" при использовании эмулятора хранения]: #error-HTTP-header-not-correct-format
...
[Возникла другая ошибка службы хранилища]: #you-have-a-different-issue-with-a-storage-service

[Приложения]: #appendices
[Приложение 1. Использование Fiddler для записи трафика HTTP и HTTPS]: #appendix-1
[Приложение 2. Использование Wireshark для записи сетевого трафика]: #appendix-2
[Приложение 3. Отслеживание сетевого трафика с помощью Microsoft Message Analyzer]: #appendix-3
[Приложение 4. Просмотр метрик и данных журнала с помощью Excel]: #appendix-4
[Приложение 5. мониторинг с помощью Application Insights для Azure DevOps]: #appendix-5
