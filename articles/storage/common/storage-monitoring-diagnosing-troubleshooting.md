...
* Проверьте, нет ли сбоев рабочей роли, считывающей сообщения из очереди. Если клиент очереди **вызывает метод инвисибилититимеаут, но** не отвечает на подтверждение, сообщение остается невидимым в очереди, пока не истечет период **invisibilityTimeout** . После этого сообщение снова станет доступным для обработки.
...
На снимке экрана ниже показан пример трассировки **Local Link Layer** с несколькими **информационными** сообщениями в столбце **DiagnosisTypes**. Если щелкнуть значок в столбец **DiagnosisTypes** , откроется подробное сообщение. В этом примере передано сообщение сервера #305, так как оно не получило подтверждение от клиента:
