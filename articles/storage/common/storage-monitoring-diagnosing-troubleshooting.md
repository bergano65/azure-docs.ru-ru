...
* [Мониторинг службы хранения данных]
  * [Мониторинг работоспособности службы]
  * [Потенциал мониторинга]
  * [Мониторинг доступности]
  * [Мониторинг производительности]
* [Диагностика проблем хранения данных]
  * [Вопросы здоровья службы]
  * [Проблемы с производительностью]
  * [Диагностика ошибок]
  * [Проблемы с эмулятором хранения]
  * [Инструменты хранения данных для хранения данных]
  * [Использование инструментов для ведения сетевого журнала]
* [Сквозной трассировки]
  * [Сопоставление данных журнала]
  * [Идентификатор запроса клиента]
  * [Идентификатор запроса сервера]
  * [Метки времени]
* [Рекомендации по устранению неполадок]
  * [Метрики показывают высокий показатель средней задержки E2E и низкий показатель средней задержки сервера]
  * [Метрики показывают низкий показатель средней задержки E2E и низкий показатель средней задержки сервера, но клиент наблюдает большую задержку]
  ...
  * [Клиент получает сообщения HTTP 403 Forbidden (запрещено)]
  * [Клиент получает HTTP 404 (не найдено) сообщения]
  * [Клиент получает http 409 (Конфликт) сообщения]
  ...
  * [Показатели емкости свидетельствуют о неожиданном увеличении использования емкости хранилища]
  ...
  * [У вас другая проблема со службой хранения данных]
...
* [Приложения]
  * [Приложение 1: Использование Скрипача для захвата трафика HTTP и HTTPS]
  * [Приложение 2: Использование Wireshark для захвата сетевого трафика]
...
Раздел[Приложения]включает сведения об использовании других средств, например Wireshark и Netmon для анализа обмена сетевыми пакетами, Fiddler — для анализа сообщений HTTP и HTTPS, а также Microsoft Message Analyzer — для корреляции данных журналов.

## <a name="monitoring-your-storage-service"></a><a name="monitoring-your-storage-service"></a>Мониторинг службы хранилища
Если вы знакомы с системным монитором Windows, то можете считать, что метрики хранилища — эквивалент счетчиков службы хранилища Azure. Они предоставляют полный набор метрик (счетчиков в терминологии системного монитора Windows), позволяющих оценить, например, доступность службы, общее количество запросов к службе, а также процент успешно обработанных запросов. Полный список доступных метрик см. в статье [Storage Analytics Metrics Table Schema](https://msdn.microsoft.com/library/azure/hh343264.aspx) (Схема таблицы метрик аналитики хранилища). Вы можете настроить службу хранилища так, чтобы она собирала и агрегировала метрики каждый час или каждую минуту. Дополнительные сведения о том, как включить метрики и отслеживать учетные записи хранения, см. в статье [Enabling storage metrics and viewing metrics data](https://go.microsoft.com/fwlink/?LinkId=510865) (Включение метрик хранилища и просмотр данных метрик).

Вы можете выбрать, какие почасовые метрики вы хотите отобразить на [портале Azure,](https://portal.azure.com) и настроить правила, которые уведомляют администраторов по электронной почте всякий раз, когда почасовая метрика превышает определенный порог. Дополнительные сведения см. в статье [Создание оповещений для служб Azure с помощью портала Azure](/azure/monitoring-and-diagnostics/monitoring-overview-alerts).

Рекомендуем просмотреть [Azure Monitor для хранения](../../azure-monitor/insights/storage-insights-overview.md) (предварительный просмотр). Это особенность Azure Monitor, которая предлагает комплексный мониторинг учетных записей хранилища Azure, предоставляя единое представление о производительности, емкости и доступности служб хранения Azure. Это не требует от вас, чтобы включить или настроить что-нибудь, и вы можете сразу же просматривать эти метрики из заранее определенных интерактивных диаграмм и других визуализаций включены.
...
Ранее эта информация, наряду с данными журналов, была доступна на [панели мониторинга служб Azure](https://status.azure.com).
...
На [портале Azure](https://portal.azure.com)можно добавить правила оповещения, чтобы уведомить вас, если какая-либо из метрик производительности для этой службы опустится ниже или превышает указанный вами порог.
...
* [Общие коды ошибок API-интерфейса REST](https://msdn.microsoft.com/library/azure/dd179357.aspx)
* [Коды ошибки службы Blob](https://msdn.microsoft.com/library/azure/dd179439.aspx)
* [Коды ошибок службы очередей](https://msdn.microsoft.com/library/azure/dd179446.aspx)
* [Коды ошибок для службы таблиц](https://msdn.microsoft.com/library/azure/dd179438.aspx)
* [Коды ошибки файловой службы](https://msdn.microsoft.com/library/azure/dn690119.aspx)
...
Ведение журналов хранилища подразумевает запись в журнал на стороне сервера запросов к хранилищу в вашей учетной записи хранения Azure. Дополнительные сведения о том, как включить ведение журнала на стороне сервера, и о доступе к данным журнала см. в статье [Enabling Storage Logging and Accessing Log Data](https://go.microsoft.com/fwlink/?LinkId=510867) (Включение ведения журнала и доступ к данным журнала хранилища).

С помощью клиентской библиотеки хранилища для .NET вы можете собирать на стороне клиента данные журнала, связанные с операциями хранилища, которые выполняются приложением. Дополнительные сведения см. в статье [Client-side Logging with the .NET Storage Client Library](https://go.microsoft.com/fwlink/?LinkId=510868) (Ведение журналов на стороне клиента с помощью клиентской библиотеки хранилища для .NET).
...
* [Microsoft Network Monitor (Netmon)](https://www.microsoft.com/download/details.aspx?id=4865) и [Wireshark](https://www.wireshark.org/) — бесплатные средства для анализа сетевых протоколов, с помощью которых можно просматривать подробную информацию о пакетах для широкого спектра сетевых протоколов. Дополнительные сведения о Wireshark см. в [Приложение 2. Отслеживание сетевого трафика с помощью Wireshark](#appendix-2).
...
## <a name="end-to-end-tracing"></a><a name="end-to-end-tracing"></a>Сквозной трассировки
...
* В журнале на стороне клиента, который создает Библиотека клиента, идентификатор запроса клиента отображается в поле **идентификатора запроса клиента** каждой записи журнала, относящейся к запросу.
* В сетевом следе, таком как идентификатор, захваченный Fiddler, идентификатор запроса клиента отображается в сообщениях запроса как значение **заголовка x-ms-client-request-id** HTTP.
...
Служба хранения данных автоматически генерирует инотые икидеи запроса сервера.
...
* В сетевом следе, таком как идентификатор, захваченный Fiddler, идентификатор запроса сервера отображается в сообщениях ответа как значение **заголовка x-ms-request-id** HTTP.
* В журнале на стороне клиента, который создает Библиотека клиентов хранения, идентификатор запроса сервера отображается в столбце **«Операция Текст»** для входа в журнал, показывающий детали ответа сервера.
...
---
### <a name="metrics-show-high-averagee2elatency-and-low-averageserverlatency"></a><a name="metrics-show-high-AverageE2ELatency-and-low-AverageServerLatency"></a>Метрики показывают высокий показатель средней задержки E2E и низкий показатель средней задержки сервера
...
Для служб таблицы и очереди алгоритм Nagle также может вызвать высокую **averageE2ELatency** по сравнению с **AverageServerLatency:** для получения дополнительной информации см. [Алгоритм публикации Nagle не подходит к малым](https://blogs.msdn.com/b/windowsazurestorage/archive/2010/06/25/nagle-s-algorithm-is-not-friendly-towards-small-requests.aspx)запросам. Вы можете отключить алгоритм Нейгла в коде с помощью класса **ServicePointManager** в пространстве имен **System.Net**. Это следует сделать до выполнения вызовов к службам таблиц или очередей в приложении, так как на открытые подключения это не распространяется. Приведенный ниже пример взят из метода **Application_Start** в рабочей роли.
...
Ошибки регулирования происходят при превышении целевых показателей масштабируемости для службы хранилища. Регулирование службы хранилища происходит для того, чтобы ни один из клиентов не мог использовать службу хранилища в ущерб другим. Для получения дополнительной информации см. [целевые показатели масштабируемости и производительности для стандартных учетных записей хранения](scalability-targets-standard-account.md) данных о целевых показателях масштабируемости для учетных записей и целевых показателей производительности для разделов в учетных записях хранилища.
...
* [Переходное увеличение в PercentThrottlingОшибка]
...
Если в значении метрики **PercentThrottlingError** наблюдаются пиковые скачки, совпадающие с периодами высокой активности приложения, следует реализовать стратегию повторения с экспоненциальной, а не линейной задержкой в клиенте. Это позволит снизить мгновенную нагрузку на секцию и сгладить пиковые скачки трафика. Для получения дополнительной информации о том, как реализовать политики повторная попытка с помощью библиотеки клиентов хранения, [см.](/dotnet/api/microsoft.azure.storage.retrypolicies)
...
### <a name="the-client-is-receiving-http-403-forbidden-messages"></a><a name="the-client-is-receiving-403-messages"></a>Клиент получает сообщения HTTP 403 Forbidden (запрещено)
...
| --- | --- | --- | --- | --- |
| Microsoft.Azure.Хранение |Сведения |3 |85d077ab -… |Начинается выполнение операции с расположением «Основное» в режиме расположения PrimaryOnly. |
| Microsoft.Azure.Хранение |Сведения |3 |85d077ab -… |Запуск синхронного запроса на<https://domemaildist.blob.core.windows.netazureimblobcontainer/blobCreatedViaSAS.txt?sv=2014-02-14&sr=c&si=mypolicy&sig=OFnd4Rd7z01fIvh%2BmcR6zbudIH2F5Ikm%2FyhNYZEmJNQ%3D&api-version=2014-02-14> |
| Microsoft.Azure.Хранение |Сведения |3 |85d077ab -… |Waiting for response (Ожидание ответа). |
| Microsoft.Azure.Хранение |Предупреждение |2 |85d077ab -… |При ожидании ответа возникло исключение: удаленный сервер вернул ошибку 403 (Запрещено). |
| Microsoft.Azure.Хранение |Сведения |3 |85d077ab -… |Response received. Код состояния = 403; идентификатор запроса = 9d67c64a-64ed-4b0d-9515-3b14bbcdc63d; Content-MD5 = ; ETag = . |
...
* Соответствует ли параметр версии в ключе SAS (например, **sv'2015-04-05)** версии библиотеки клиентского хранилища, которая вы используете? Мы рекомендуем вам всегда использовать последнюю версию [библиотеки клиентов хранения.](https://www.nuget.org/packages/WindowsAzure.Storage/)
...
В этом примере журнал показывает, что клиент переплетывает запросы из метода **CreateIfNotExists** (request ID e2d06d78...) с запросами от метода **UploadFromStream** (de8b1c3c-...). Это взаимодействие происходит потому, что клиентское приложение ссылается на эти методы асинхронно. Измените код с асинхронными вызовами в клиенте таким образом, чтобы он создавал контейнер до того, как попытается передать какие-либо данные в большой двоичный объект в этом контейнере. В идеале нужно создавать все контейнеры заранее.
...
Чтобы решить эту проблему JavaScript обходным путем, вы можете настроить для службы хранилища, к которой обращается клиент, общий доступ к ресурсам независимо от источника (CORS). Дополнительные сведения см. в статье [Cross-Origin Resource Sharing (CORS) Support for Azure Storage Services](https://msdn.microsoft.com/library/azure/dn535601.aspx) (Поддержка общего доступа к ресурсам независимо от источника (CORS) для служб хранилища Azure).
...
Детали исключения в клиенте включают идентификатор запроса (7e84f12d...), назначенный службой таблицы для запроса: вы можете использовать эту информацию, чтобы найти данные запроса в журналах хранения сервера путем поиска в столбце **запроса-id-header** в файле журнала. Кроме того, можно проверить метрики для того, чтобы выяснить, когда возникают такие ошибки, а затем выполнить поиск в файлах журналов по зафиксированному в метриках времени. Эта запись журнала указывает на то, что удаление не выполнено и возвращено сообщение о состоянии HTTP (404) Client Other Error (Другая ошибка клиента). Та же запись журнала также включает идентификатор запроса, генерируемый клиентом в столбце **клиента-запрос-id** (813ea74f...).
...
* [Ошибка "Значение для одного из заголовков HTTP не в правильном формате" при использовании эмулятора хранилища]
* [Запуск эмулятора хранилища требует административных привилегий]
...
[Fiddler](https://www.telerik.com/fiddler) является полезным инструментом для анализа трафика HTTP и HTTPS между вашим клиентским приложением и службой хранения Azure, которая используется.
...
Чтобы ограничить объем трафика, который захватывает Скрипач, можно использовать фильтры, настроенные во вкладке **Фильтры.** На следующем скриншоте показан фильтр, который фиксирует только трафик, отправленный в конечную точку **хранения contosoemaildist.table.core.windows.net:**
...
### <a name="appendix-3-using-microsoft-message-analyzer-to-capture-network-traffic"></a><a name="appendix-3"></a>Приложение 3. Отслеживание сетевого трафика с помощью Microsoft Message Analyzer
...
Чтобы настроить сеанс трассировки веб-трафика HTTP и HTTPS с помощью анализатора сообщений Майкрософт, запустите анализатор, а затем в меню **Файл** выберите пункт **Захват и трассировка**. В списке доступных сценариев трассировки выберите пункт **Web Proxy**(Веб-прокси). Затем в панели **конфигурации trace Scenario** в текстовом ящике **HostnameFilter** добавьте имена конечных точек хранения (эти имена можно найти [на портале Azure).](https://portal.azure.com) Например, если ваша учетная запись хранения Azure имеет имя **contosodata**, добавьте в поле **Фильтр имен узлов** следующий текст.
...
Дополнительные сведения о трассировке **веб-прокси** в анализаторе сообщений Майкрософт см. в разделе [Microsoft-PEF-WebProxy Provider](https://technet.microsoft.com/library/jj674814.aspx) (Поставщик Microsoft-PEF-WebProxy).
...
Дополнительные сведения о трассировке Local Link Layer в анализаторе сообщений Майкрософт см. в разделе [Microsoft-PEF-NDIS-PacketCapture Provider](https://technet.microsoft.com/library/jj659264.aspx) (Поставщик Microsoft-PEF-NDIS-PacketCapture).
...
* [Формат журнала аналитики хранения](/rest/api/storageservices/storage-analytics-log-format)

<!--Anchors-->
[Введение]: #introduction
[Как организовано это руководство]: #how-this-guide-is-organized

[Мониторинг службы хранения данных]: #monitoring-your-storage-service
[Мониторинг работоспособности службы]: #monitoring-service-health
[Потенциал мониторинга]: #monitoring-capacity
[Мониторинг доступности]: #monitoring-availability
[Мониторинг производительности]: #monitoring-performance

[Диагностика проблем хранения данных]: #diagnosing-storage-issues
[Вопросы здоровья службы]: #service-health-issues
...
[Инструменты хранения данных для хранения данных]: #storage-logging-tools
[Использование инструментов для ведения сетевого журнала]: #using-network-logging-tools

[Сквозной трассировки]: #end-to-end-tracing
[Сопоставление данных журнала]: #correlating-log-data
...
[Метрики показывают высокий показатель средней задержки E2E и низкий показатель средней задержки сервера]: #metrics-show-high-AverageE2ELatency-and-low-AverageServerLatency
[Метрики показывают низкий показатель средней задержки E2E и низкий показатель средней задержки сервера, но клиент наблюдает большую задержку]: #metrics-show-low-AverageE2ELatency-and-low-AverageServerLatency
...
[Переходное увеличение в PercentThrottlingОшибка]: #transient-increase-in-PercentThrottlingError
[Постоянное увеличение ошибки PercentThrottlingError]: #permanent-increase-in-PercentThrottlingError
...
[Клиент получает сообщения HTTP 403 Forbidden (запрещено)]: #the-client-is-receiving-403-messages
[Клиент получает HTTP 404 (не найдено) сообщения]: #the-client-is-receiving-404-messages
...
[Клиент получает http 409 (Конфликт) сообщения]: #the-client-is-receiving-409-messages
...
[Показатели емкости свидетельствуют о неожиданном увеличении использования емкости хранилища]: #capacity-metrics-show-an-unexpected-increase
...
[Ошибка "Значение для одного из заголовков HTTP не в правильном формате" при использовании эмулятора хранилища]: #error-HTTP-header-not-correct-format
[Запуск эмулятора хранилища требует административных привилегий]: #storage-emulator-requires-administrative-privileges
...
[У вас другая проблема со службой хранения данных]: #you-have-a-different-issue-with-a-storage-service

[Приложения]: #appendices
[Приложение 1: Использование Скрипача для захвата трафика HTTP и HTTPS]: #appendix-1
[Приложение 2: Использование Wireshark для захвата сетевого трафика]: #appendix-2
[Приложение 3. Отслеживание сетевого трафика с помощью Microsoft Message Analyzer]: #appendix-3
[Приложение 4: Использование Excel для просмотра метрик и данных журналов]: #appendix-4
[Приложение 5: Мониторинг с помощью анализа приложений для Azure DevOps]: #appendix-5
