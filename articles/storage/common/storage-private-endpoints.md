...
Клиенты в виртуальной сети должны использовать одну и ту же строку подключения для учетной записи хранения даже при использовании частной конечной точки.
...
> [!IMPORTANT]
> Используйте ту же строку подключения для подключения к учетной записи хранения через частные конечные точки, как и в противном случае. Не подключайтесь к учетной записи хранения, используя URL-адрес поддомена "*привателинк*".

> [!TIP]
> При использовании пользовательского или локального DNS-сервера следует настроить записи ресурсов DNS для частных конечных точек в зоне DNS, соответствующей поддомену "привателинк" службы хранилища.
