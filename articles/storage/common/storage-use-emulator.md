...
| --- | --- | --- | --- |
| **Начало** |Запускает эмулятор хранения. |`AzureStorageEmulator.exe start [-inprocess]` |*-* Повторная обработка: запускает эмулятор в текущем процессе вместо создания нового процесса. |
| **Stop** |Останавливает эмулятор хранения. |`AzureStorageEmulator.exe stop` | |
...
* Теперь геоизбыточное хранилище с доступом для чтения (RA-GRS) поддерживается и в эмуляторе хранения. Интерфейсы API `Get Queue Service Stats` ,и`Get Table Service Stats` поддерживаются для вторичной учетной записи и всегда возвращают значение элемента ответа ластсинктиме как текущее время в соответствии с базовой базой данных SQL. `Get Blob Service Stats` Для программного доступа к вторичному расположению с помощью эмулятора хранения используйте клиентскую библиотеку хранилища для .NET 3.2 или более поздней версии. Подробнее см. в справочнике по клиентской библиотеке хранилища Microsoft Azure для .NET.
