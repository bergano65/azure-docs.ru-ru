...
[Клиентская библиотека службы хранилища Azure для Java](https://mvnrepository.com/artifact/com.microsoft.azure/azure-storage) поддерживает шифрование данных в клиентских приложениях перед их отправкой в службу хранилища Azure и их расшифровку во время скачивания клиентом. Библиотека также поддерживает интеграцию с [Azure Key Vault](https://azure.microsoft.com/services/key-vault/) для управления ключами хранения данных.
...
Загрузка зашифрованного капли включает в себя получение содержимого всей капли с помощью методов **удобства загрузки**/**OpenInputStream.** Зашифрованный ключ CEK расшифровывается и используется вместе с ключом IV (который в данном случае хранится как метаданные большого двоичного объекта) для передачи расшифрованных данных обратно пользователям.

Загрузка произвольного диапазона (методы**загрузкиДиапазона)** в зашифрованном blob включает в себя корректировку диапазона, предоставляемого пользователями, чтобы получить небольшое количество дополнительных данных, которые могут быть использованы для успешной расшифровки запрашиваемого диапазона.  
...
4. Из-за этих двух дополнительных зарезервированных свойств, необходимых для шифрования, пользователи могут иметь только 250 пользовательских свойств вместо 252. Общий размер сущности должен быть меньше 1 МБ.  
   ...
   Что касается таблиц, то в дополнение к политике шифрования пользователи должны указать свойства, которые необходимо зашифровать. Это можно сделать путем указания атрибута [Encrypt] \(для сущностей POCO, которые являются производными от TableEntity) или с помощью сопоставителя шифрования в параметрах запроса. Сопоставитель шифрования — это делегат, который получает ключ раздела, ключ строки и имя свойства, а затем возвращает логическое значение, которое указывает, следует ли это свойство шифровать. Во время шифрования клиентская библиотека использует эти сведения, чтобы решить, следует ли шифровать свойство перед отправкой. Делегат также обеспечивает возможность логики в отношении того, как шифруются свойства. (Например, если X, а затем шифровать свойства A; в противном случае шифровать свойства A и B.) Обратите внимание, что нет необходимости предоставлять эту информацию при чтении или запросе сущностей.
...
3. Во время создания политики шифрования используйте кэширующий сопоставитель в качестве входных данных.
   Дополнительные сведения об использовании хранилища ключей см. в разделе с примерами кода шифрования.
...
В дополнение к созданию политики шифрования и установке ее на параметры запроса, необходимо либо указать **encryptionResolver** в **TableRequestOptions,** либо установить атрибут «Encrypt» на геттере и сеттере сущности.
...
* Скачайте [клиентскую библиотеку хранилища Azure для пакета Maven Java](https://mvnrepository.com/artifact/com.microsoft.azure/azure-storage)  
...
* Скачайте библиотеку Maven хранилища ключей Azure для пакетов Maven Java.
  * [Основной](https://mvnrepository.com/artifact/com.microsoft.azure/azure-keyvault-core) пакет.
  * [Клиентский](https://mvnrepository.com/artifact/com.microsoft.azure/azure-keyvault) пакет
* Просмотрите [документацию по хранилищу ключей Azure](../../key-vault/key-vault-overview.md)
