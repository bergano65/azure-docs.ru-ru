...
[Клиентская библиотека службы хранилища Azure для Java](https://search.maven.org/artifact/com.azure/azure-storage-blob-cryptography) поддерживает шифрование данных в клиентских приложениях перед их отправкой в службу хранилища Azure и их расшифровку во время скачивания клиентом. Библиотека также поддерживает интеграцию с [Azure Key Vault](https://azure.microsoft.com/services/key-vault/) для управления ключами учетной записи хранения.
...
Загрузка зашифрованного большого двоичного объекта включает извлечение содержимого всего большого двоичного объекта с помощью методов **download** / **опенинпутстреам** . Зашифрованный ключ CEK расшифровывается и используется вместе с ключом IV (который в данном случае хранится как метаданные большого двоичного объекта) для передачи расшифрованных данных обратно пользователям.
...
Помимо создания политики шифрования и ее настройки в параметрах запроса необходимо либо указать **EncryptionResolver** в **TableRequestOptions**, либо задать атрибут [Encrypt] в методах получения и задания сущности.
...
* Скачайте библиотеку Maven хранилища ключей Azure для пакетов Maven Java.
  ...
  * Пакет [клиента](https://mvnrepository.com/artifact/com.microsoft.azure/azure-keyvault)
