---
title: Настройка строки соединения
titleSuffix: Azure Storage
description: Настройка строки подключения для учетной записи хранения Azure. Строка подключения содержит информацию, необходимую для авторизации доступа к учетной записи хранилища из приложения во время выполнения с помощью общей авторизации Ключ.
services: storage
author: tamram
ms.service: storage
ms.topic: article
...
ms.locfileid: "79268357"
---
# <a name="configure-azure-storage-connection-strings"></a>Настройка строк подключения службы хранилища Azure

Строка подключения включает в себя информацию авторизации, необходимую для доступа к данным в учетной записи хранилища Azure во время выполнения с помощью общей авторизации Key. Настройка строки подключения позволяет выполнить следующие действия:
...
## <a name="view-and-copy-a-connection-string"></a>Просмотр и копирование строки соединения

[!INCLUDE [storage-view-keys-include](../../../includes/storage-view-keys-include.md)]

## <a name="store-a-connection-string"></a>Хранение строки соединения
...
* Строку соединения можно хранить в переменной среды.
...
## <a name="configure-a-connection-string-for-the-storage-emulator"></a>Настройка строки соединения для эмулятора хранилища
...
## <a name="configure-a-connection-string-for-an-azure-storage-account"></a>Настройка строки соединения для учетной записи хранения Azure
...
> [!TIP]
> Данные о строках подключения учетной записи хранения можно найти на [портале Azure](https://portal.azure.com). Перейдите к**клавишам** **SETTINGS** > Access в лезвии меню учетной записи хранилища, чтобы увидеть строки подключения как для первичных, так и для вторичных ключей доступа.
>
...
Для создания строки подключения для службы хранения в регионах или экземплярах с различными суффиксами конечных точек, например для Azure China 21Vianet или Azure Government, используйте следующий формат строки соединения. Укажите, следует ли подключаться к учетной записи хранения через HTTPS (рекомендуется) или HTTP. Замените `myAccountName` именем своей учетной записи хранения, замените `myAccountKey` ключом доступа своей учетной записи и замените `mySuffix` суффиксом URI:
...
Вот пример строки подключения для служб хранения данных в Azure China 21Vianet:
...
* [Использование подписей общего доступа (SAS)](storage-sas-overview.md)
