...
2. Пессимистичный параллелизм — До начала обновления приложение блокирует доступ к объекту, не позволяя пользователю изменить данные до снятия блокировки. Например в сценарии репликации данных главный/подчиненный, где только образец будет выполнять обновления master будет обычно содержать монопольную блокировку в течение продолжительного периода времени для данных, чтобы убедиться, что никто не может обновлять ее.
