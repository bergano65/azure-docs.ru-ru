---
title: Передача данных в библиотеке движения данных для .NET
titleSuffix: Azure Storage
description: Используйте библиотеку Движения данных для перемещения или копирования данных в или из содержимого кабы и файлов. Копируйте данные в хранилище Azure из локальных файлов, а также внутри учетной записи хранения и из одной такой учетной записи в другую. Легко переносите данные в хранилище Azure.
services: storage
author: tamram
ms.service: storage
ms.devlang: dotnet
...
Библиотека Azure Storage Data Movement — это кросс-платформная библиотека с открытым исходным кодом, предназначенная для высокопроизводительной загрузки, загрузки и копирования кабы и файлов. Библиотека движения данных предоставляет удобные методы, которые недоступны в клиентской библиотеке Azure Storage для .NET. Эти методы обеспечивают возможность установить количество параллельных операций, отслеживать ход передачи, легко возобновить отмененный перевод и многое другое.
...
## <a name="add-the-data-movement-library-to-your-project"></a>Добавление библиотеки движения данных в проект

1. Добавьте последнюю версию библиотеки `dependencies` Движения `<project-name>.csproj` данных в раздел файла. На момент написания статьи это версия `"Microsoft.Azure.Storage.DataMovement": "0.6.2"`.
...
## <a name="upload-a-local-file-to-a-blob"></a>Загрузить локальный файл в каплю
...
## <a name="set-the-number-of-parallel-operations"></a>Установить количество параллельных операций

Одной из особенностей, предлагаемых библиотекой Движения данных, является возможность установить количество параллельных операций для увеличения пропускной способности передачи данных. По умолчанию библиотека Движения данных устанавливает количество параллельных операций до 8 - количество ядер на машине.
...
Знание того, сколько времени потребовалось для передачи данных, полезно. Тем не менее, быть в состоянии видеть ход передачи *во время* операции передачи было бы еще лучше. Чтобы получить такую возможность, необходимо создать объект `TransferContext`. Объект `TransferContext` бывает двух типов: `SingleTransferContext` и `DirectoryTransferContext`. Первый предназначен для передачи одного файла, а второй — для передачи каталога файлов.
...
Еще одной удобной функцией, предлагаемой библиотекой Data Movement, является возможность возобновления отмененного перевода. Давайте добавим код, позволяющий временно отменить передачу, введя `c`, а через 3 секунды возобновить ее.
...
## <a name="transfer-a-local-directory-to-blob-storage"></a>Передача локального каталога в хранилище Blob

Было бы разочаровывающим, если бы библиотека Движения данных могла передавать только один файл за один раз. К счастью, это не так. Библиотека движения данных предоставляет возможность передачи каталога файлов и всех его субдиректоров. Давайте добавим код, позволяющий сделать это.
...
## <a name="copy-a-file-from-url-to-a-blob"></a>Копирование файла из URL в каплю
...
Еще одной особенностью, уникально предоставляемой библиотекой Движения данных, является возможность копирования с одного ресурса хранения Azure на другой.
...
Приложение для перемещения данных завершено. [Полный пример кода можно найти на сайте GitHub](https://github.com/azure-samples/storage-dotnet-data-movement-library-app).

## <a name="next-steps"></a>Дальнейшие действия

[Справочная документация библиотеки движения данных Лазурного хранилища данных](https://azure.github.io/azure-storage-net-data-movement).
