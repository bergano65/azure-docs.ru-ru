...
## <a name="azure-importexport-use-cases"></a>Случаи использования импорта/экспорта Azure
...
* **Распространение контента**: Быстро отправлять данные на сайты клиентов.
* **Резервное копирование:** Возьмите резервные копии данных для хранения в Azure Storage.
...
* **Инструмент WAImportExport**: это программа командной строки, которая выполняет следующие действия:
  ...
  * Шифрует данные на диске с помощью 128-битного BitLocker AES. Вы можете использовать внешний протектор ключа для защиты ключа BitLocker.
...
> [!NOTE]
> Для местных (в пределах страны/региона) отгрузок, пожалуйста, поделитесь счетом внутреннего перевозчика.
>
> Для зарубежных (за пределами страны/региона) отправления, пожалуйста, поделитесь международным аккаунтом перевозчика.
...
Данные на диске шифруются с помощью 128-битного шифрования Привода BitLocker AES. Это шифрование защитит данные во время пересылки.
...
* Можно включить шифрование диска BitLocker вручную. Для этого следует указать ключ шифрования в файле *driveset.csv* во время запуска окна командной строки инструмента WAImportExport в процессе подготовки диска. Ключ шифрования BitLocker может быть дополнительно защищен с помощью внешнего протектора ключа (также известного как управляемый ключом Майкрософт) или ключом управления клиентом. Для получения дополнительной информации, см. как [использовать управляемый ключ омички клиента для защиты ключа BitLocker.](storage-import-export-encryption-key-portal.md)

В случае экспорта после копирования данных на диски и перед их отправкой вам служба выполняет шифрование с помощью технологии BitLocker. Ключ шифрования предоставляется пользователю на портале Azure. Диск должен быть разблокирован с помощью инструмента WAImporExport с помощью ключа.
...
[Стандартная плата за транзакцию хранения](https://azure.microsoft.com/pricing/details/storage/) применяется при импорте, а также экспорте данных. Стандартные сборы за выход также применимы наряду с транзакционными сборами за хранение при экспорте данных из Azure Storage. Для получения дополнительной информации о расходах на выемку, [см.](https://azure.microsoft.com/pricing/details/data-transfers/)
