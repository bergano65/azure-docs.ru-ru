...
Каждый разработчик приложений, использующий службу хранилища Azure, должен прочесть эту статью и проверить, что их приложения следуют всем проверенным методикам, приведенным ниже.  
...
| &nbsp; | Все службы |Сети |[Имеют ли клиентские устройства достаточно высокую пропускную способность и низкую степень задержки для достижения необходимой производительности?](#subheading2) |
| &nbsp; | Все службы |Сети |[Имеют ли клиентские устройства достаточно высокое качество связи?](#subheading3) |
| &nbsp; | Все службы |Сети |[Расположено ли клиентское приложение "вблизи" учетной записи хранения?](#subheading4) |
...
| &nbsp; | Все службы |Кэширование |[Данные кэширования приложений используются постоянно и изменяются редко?](#subheading7) |
| &nbsp; | Все службы |Кэширование |[Ваше приложение группирует обновления (кэширует на стороне клиента, а затем загружает в виде большого массива)?](#subheading8) |
...
| &nbsp; | Все службы |Сервис |[Вы используете последние версии клиентских библиотек и инструментов, предоставленных корпорацией Microsoft?](#subheading13) |
...
| &nbsp; | BLOB-объекты |Копирование BLOB-объектов |[Используете ли вы импорт и экспорт Azure для перемещения больших объемов данных?](#subheading19) |
...
| &nbsp; | Таблицы |Вставить, обновить, удалить |[Для сущностей, которые всегда будут извлекаться вместе и могут быть записаны в пакеты (например, данные временных рядов), рассматривали ли вы возможность использования больших двоичных объектов вместо таблиц?](#subheading38) |
| &nbsp; | Списки ожидания |Целевые показатели масштабируемости |[Вы достигаете целевых показателей масштабируемости для сообщений в секунду?](#subheading39) |
| &nbsp; | Списки ожидания |Конфигурация |[Вы отключили алгоритм Nagle для повышения производительность небольших запросов?](#subheading40) |
| &nbsp; | Списки ожидания |Размер сообщения |[Ваши сообщения сжаты для повышения производительности очереди?](#subheading41) |
| &nbsp; | Списки ожидания |Массовое получение |[Вы получаете множество сообщений при выполнении одной операции "Получить"?](#subheading42) |
| &nbsp; | Списки ожидания |Частота опроса |[Частота опроса достаточна для уменьшения наблюдаемой задержки приложения?](#subheading43) |
| &nbsp; | Списки ожидания |Сообщение об обновлении |[Используете ли вы UpdateMessage для хранения хода выполнения обработки сообщений, избегая повторной обработки всего сообщения при возникновении ошибки?](#subheading44) |
| &nbsp; | Списки ожидания |Архитектура |[Используете ли вы очереди для того, чтобы сделать все приложение более масштабируемым, не допуская длительных рабочих нагрузок на критическом пути и масштабируя их самостоятельно?](#subheading45) |
...
Сама Служба хранилища Azure работает с ограничением в 250 учетных записей хранения в регионе для каждой подписки. При достижении этого ограничения вы больше не сможете создать любые учетные записи хранения в сочетании с этой подпиской или регионом.
...
#### <a name="sub1bandwidth"></a>Целевая масштабируемость полосы пропускания для всех служб

На момент написания данной статьи целевыми показателями пропускной способности в США для учетной записи геоизбыточного хранилища (GRS) является скорость в 10 гигабит в секунду (Гбит/с) на входе (данные, отправленные в учетную запись хранения) и 20 Гбит/с на выходе (данные, отправленные из учетной записи хранения). Для учетной записи локально избыточного хранилища (LRS) эти ограничения выше и составляют до 20 Гбит/с на входе и 30 Гбит/с на выходе.  Международные ограничения пропускной способности могут быть ниже. О них можно узнать на [странице целевых показателей масштабируемости](https://msdn.microsoft.com/library/azure/dn249410.aspx).  Для получения дополнительной информации о параметрах избыточности хранилищ см. ссылки в разделе "Полезные ресурсы" далее в этой статье.  

#### <a name="what-to-do-when-approaching-a-scalability-target"></a>Действия при достижении целевого показателя масштабируемости

Если вы приближаетесь к лимиту учетных записей хранения, которые возможны для определенной комбинации подписки и региона, оцените свое приложение и использование учетных записей хранения и определите, применимо ли какое-либо из этих условий.

* Использование учетных записей хранения в качестве неуправляемых дисков и добавление этих дисков в виртуальные машины. В этом случае мы рекомендуем использовать [управляемые диски](../../virtual-machines/windows/managed-disks-overview.md), так как они обрабатывают масштабируемость диска хранилища без необходимости создавать отдельные учетные записи хранения и управлять ими.
* Использование одной учетной записи хранения для каждого клиента в целях изоляции данных. В этом случае мы рекомендуем использовать контейнеры хранилища для каждого клиента, а не все учетные записи хранения. Служба хранилища Azure теперь позволяет указать управление доступом на основе ролей для каждого [контейнера](storage-auth-aad-rbac-portal.md).
* Использование нескольких сегментов учетных записей хранения для большей масштабируемости входящего и исходящего трафика, операций ввода-вывода в секунду и емкости. В этом сценарии, если это возможно, мы рекомендуем воспользоваться преимуществами [увеличенных лимитов](https://azure.microsoft.com/blog/announcing-larger-higher-scale-storage-accounts/) стандартных учетных записей хранения, чтобы уменьшить количество учетных записей хранения, необходимых для вашей рабочей нагрузки.
...
* Дополнительные сведения о параметрах избыточности хранилищ см. в статье [Репликация службы хранилища Azure](storage-redundancy.md) и записи блога [Microsoft Azure Storage Redundancy Options and Read Access Geo Redundant Storage](https://blogs.msdn.com/b/windowsazurestorage/archive/2013/12/11/introducing-read-access-geo-replicated-storage-ra-grs-for-windows-azure-storage.aspx) (Параметры избыточности службы хранилища Microsoft Azure и геоизбыточное хранилище с доступом на чтение).
...
Для масштабирования и распределения нагрузки в системы в хранилище Azure используется схема секционирования на основе диапазонов. Ключ секции (учетная запись + контейнер + BLOB) используется для секционирования данных в диапазоны, и эти диапазоны распределяются по всей системе. Это означает, что используются такие соглашения об именовании, как лексическая Сортировка (например, *мипайролл*, *миперформанце*, *мемплойис*и т. д.) или использование меток времени (*log20160101*, *log20160102*, *log20160102*и т. д.). сами секции могут совместно размещаться на одном сервере секций, пока операция балансировки нагрузки не разбивает их на меньшие диапазоны. Например, все большие двоичные объекты в контейнере могут обслуживаться одним сервером до тех пор, пока нагрузка на эти большие двоичные объекты не потребует дополнительной повторной балансировки диапазонов секций. Точно так же группу незначительно нагруженных учетных записей с именами, упорядоченными в лексическом порядке, может обслуживать один и тот же сервер, пока нагрузка на одну или несколько из этих учетных записей не потребует разбить их между несколькими серверами секций. Каждая операция балансировки нагрузки может вызывать задержку вызовов хранилища во время ее выполнения. Способность системы управлять внезапной интенсивностью трафика в секцию ограничивается масштабируемостью односекционного сервера до тех пор, пока операция балансировки нагрузки не запустится и не выполнит повторную балансировку диапазона ключей секций.

Частоту выполнения таких операций можно уменьшить.  

* Если возможно, используйте больший размер большого двоичного объекта размещения или размещение блоков размещения (более 4 MiB для стандартных учетных записей и более 256 КИБ для учетных записей уровня "Премиум") для активации блочного BLOB-объекта высокой пропускной способности (ХТББ). ХТББ обеспечивает высокую производительность, что не влияет на именование секций.
* Внимательно проверьте используемое соглашение об именовании учетных записей, контейнеров, больших двоичных объектов, таблиц и очередей. Попробуйте предварительно исправить имена учетной записи, контейнера или BLOB-объекта с помощью 3-значного хэша, используя функцию хэширования, которая лучше подходит для ваших потребностей.  
* Если данные упорядочены по меткам времени или числовым идентификаторам, убедитесь в том, что вы используете инкрементируемые (или декрементируемые) шаблоны трафика. Эти шаблоны не подходят для системы секционирования на основе диапазонов и могут приводить к тому, что весь трафик будет направляться в одну и ту же секцию, препятствуя эффективной балансировке нагрузки. Например, при наличии ежедневных операций, использующих объект большого двоичного объекта с меткой времени, например *ГГГГММДД*, весь трафик для этой ежедневной операции направляется на один объект, который обслуживается одним сервером секций. Проверьте, соответствуют ли ограничения по BLOB-объектам и ограничения по секциям вашим потребностям и, если нужно, разбейте эту операцию на несколько BLOB-объектов. Точно так же, если в таблицах хранятся данные временных рядов, весь трафик может направляться в последнюю часть ключевого пространства имен. Если необходимо использовать метки времени или числовые идентификаторы, добавьте префикс в идентификатор с 3-значным хэшем или в случае с метками времени добавьте в часть времени, например *ссииииммдд*, значение секунд. Если операции перечисления и запросов выполняются регулярно, выберите функцию хэширования, ограничивающую число запросов. В других случаях может быть достаточно случайного префикса.  
* Дополнительные сведения о схеме секционирования, используемой в службе хранилища Azure, [см. в статье Служба хранилища Azure: A Highly Available Cloud Storage Service with Strong Consistency](https://sigops.org/sosp/sosp11/current/2011-Cascais/printable/11-calder.pdf) (Хранилище Microsoft Azure: высокодоступная служба облачного хранения со строгой согласованностью).
...
Дополнительные сведения см. в записи блога [Web Services: Concurrent Connections ](https://blogs.msdn.com/b/darrenj/archive/2005/03/07/386655.aspx) (Веб-службы: параллельные подключения).  

#### <a name="subheading10"></a>Увеличение минимального количества потоков при использовании синхронного кода с асинхронными задачами

Этот код увеличит минимальное число потоков в пуле потоков:  

```csharp
ThreadPool.SetMinThreads(100,100); //(Determine the right number for your application)  
```

Дополнительные сведения см. в статье [ThreadPool.SetMinThreads Method (Int32, Int32)](https://msdn.microsoft.com/library/system.threading.threadpool.setminthreads%28v=vs.110%29.aspx) (Метод ThreadPool.SetMinThreads (Int32, Int32)).  

#### <a name="subheading11"></a>Воспользуйтесь преимуществами .NET 4,5 и более поздней сборки мусора
...
Дополнительные сведения см. в статье [Обзор усовершенствований, направленных на повышение производительности](https://msdn.microsoft.com/magazine/hh882452.aspx).  
...
Всегда используйте самые последние версии клиентских библиотек и инструментов, предоставляемых корпорацией Microsoft. На момент написания данной статьи существуют клиентские библиотеки, доступные для .NET, Windows Phone, Windows Runtime, Java и C++, а также библиотеки предварительного просмотра для других языков. Кроме того, корпорация Майкрософт выпустила командлеты PowerShell и команды CLI Azure для работы с хранилищем Azure. Корпорация Майкрософт активно разрабатывает эти инструменты с учетом производительности, поддерживает их в соответствии с последними версиями служб и обеспечивает в них поддержку проверенных и производительных методов работы.  

### <a name="retries"></a>Повторы

#### <a name="subheading14"></a>Ошибки регулирования и занятости сервера
...
#### <a name="subheading15"></a>Ошибки, не допускающие повторения
...
Для получения дополнительной информации о кодах ошибок хранилища см. статью [Status and Error Codes](https://msdn.microsoft.com/library/azure/dd179382.aspx) (Коды ошибок и состояний) на веб-сайте Microsoft Azure.  

## <a name="blobs"></a>BLOB-объекты

В дополнение к проверенным методикам для [всех служб](#allservices) , описанных выше, следующие проверенные методики применяются специально для службы BLOB-объектов.  

### <a name="blob-specific-scalability-targets"></a>Целевые показатели масштабируемости для большого двоичного объекта
...
#### <a name="subheading17"></a>Копировать BLOB-объект
...
Дополнительную информацию см. в статье [Copy Blob](https://msdn.microsoft.com/library/azure/dd894037.aspx) (Копирование большого двоичного объекта).  
...
Для больших объемов данных (более 1 ТБ) служба хранилища Azure предлагает службу импорта и экспорта, которая позволяет размещать или загружать данные хранилищ BLOB-объектов путем доставки жестких дисков.  Можно поместить свои данные на жесткий диск и отправить его в корпорацию Microsoft для размещения в сети или отправить пустой жесткий диск в корпорацию Microsoft для загрузки данных из сети.  Дополнительную информацию см. в статье [Использование службы импорта и экспорта Azure для передачи данных в хранилище BLOB-объектов](../storage-import-export-service.md).  Это может быть гораздо более эффективным, чем отправка или загрузка такого объема данных по сети.  

### <a name="subheading20"></a>Использование метаданных

Служба BLOB-объектов поддерживает запросы HEAD, которые могут включать метаданные о большом двоичном объекте. Например, если приложению необходимы EXIF-данные из фотографии, оно может получить фотографию и извлечь их из нее. Для сохранения пропускной способности и повышения производительности приложение может сохранить данные EXIF в метаданных большого двоичного объекта при загрузке фотографии приложением: можно получить данные EXIF, находящиеся в метаданных, используя только запрос HEAD, тем самым экономя значительную пропускную способность и время обработки, необходимые для извлечения данных EXIF каждый раз при чтении большого двоичного объекта. Это было бы полезным в ситуациях, когда необходимы только метаданные, а не полное содержимое BLOB-объекта.  В большом двоичном объекте могут храниться только 8 КБ метаданных (служба не будет принимать запрос на хранение большего, чем этот, объема данных), поэтому если данные превышают этот размер, использование такого подхода становится невозможным.  

### <a name="rapid-uploading"></a>Быстрая отправка

Чтобы быстро передать большие двоичные объекты, ответьте на первый вопрос: вы отправляете один большой двоичный объект или много?  Чтобы правильно выбрать метод работы, используйте сведения из приведенного ниже руководства.  

#### <a name="subheading21"></a>Быстрая отправка одного BLOB-объекта

Чтобы быстро отправить один большой BLOB-объект, клиентское приложение должно отправить его блоки или страницы параллельно (помня о целевых показателях масштабируемости для отдельных BLOB-объектов и учетной записи хранения в целом).  Обратите внимание на то, что этой способностью обладают клиентские библиотеки хранилища RTM, официально предоставляемые корпорацией Майкрософт (.NET, Java).  Для установки уровня параллелизма для каждой библиотеки используйте указанный ниже объект или свойство:  

* .NET: Установите ParallelOperationThreadCount для использования в объекте BlobRequestOptions.
...
Дополнительные сведения см. в статье [Understanding Block Blobs, Append Blobs, and Page Blobs](https://msdn.microsoft.com/library/azure/ee691964.aspx) (Основные сведения о блочных, добавочных и страничных BLOB-объектах).  
...
### <a name="subheading24"></a>Целевые показатели масштабируемости для конкретной таблицы
...
#### <a name="entities-per-second-storage-account"></a>Сущностей в секунду (учетная запись хранения)
...
#### <a name="entities-per-second-partition"></a>Сущностей в секунду (секция)
...
#### <a name="subheading26"></a>Отключение Nagle
...
##### <a name="subheading29"></a>Данные с высоким трафиком
...
###### <a name="point-queries"></a>Запросы точек

Точечный запрос извлекает ровно одну сущность. Он делает это, указав как ключ раздела, так и ключ строки сущности. Эти запросы являются эффективными, и их следует использовать везде, где это возможно.  

###### <a name="partition-queries"></a>Запросы к разделам
...
###### <a name="table-queries"></a>Запросы таблиц
...
##### <a name="subheading31"></a>Плотность запросов
...
##### <a name="subheading37"></a>Хранение рядов данных в одной сущности
...
### <a name="subheading40"></a>Отключение Nagle
...
Производительность и масштабируемость очереди уменьшаются по мере увеличения размера сообщений. В сообщении следует размещать только ту информацию, которая необходима получателю.  

### <a name="subheading42"></a>Получение пакетов
...
### <a name="subheading44"></a>Сообщение об обновлении

Операцию **обновить сообщение** можно использовать для увеличения времени ожидания невидимости или обновления сведений о состоянии сообщения. Хотя это и является мощным, помните, что каждая операция **обновления сообщения** подсчитывается в целевую цель масштабируемости. Тем не менее ее использование может быть гораздо более эффективным, чем наличие рабочего процесса, который передает работу из одной очереди в другую по мере завершения каждого этапа работы. С помощью операции **обновления сообщений** приложение сохраняет состояние задания в сообщении, а затем продолжает работу, вместо того, чтобы повторно поочередно выполнять это сообщение для следующего шага задания при каждом завершении действия.  

Дополнительные сведения см. в разделе [Change the contents of a queued message ](../queues/storage-dotnet-how-to-use-queues.md#change-the-contents-of-a-queued-message) (Изменение содержимого сообщения в очереди).  
