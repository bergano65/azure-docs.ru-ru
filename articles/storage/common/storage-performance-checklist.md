...
ms.locfileid: "67295745"
---
# <a name="microsoft-azure-storage-performance-and-scalability-checklist"></a>Контрольный список производительности и масштабируемости службы хранилища Microsoft Azure
...
Каждый разработчик приложения, с помощью службы хранилища Azure занимает время в этой статье и проверьте, что их приложение следует всем проверенным методикам, перечисленным ниже.  
...
### <a name="subheading47"></a>Соглашения об именовании секций

Для масштабирования и распределения нагрузки в системы в хранилище Azure используется схема секционирования на основе диапазонов. Ключ секции (учетной записи и контейнера больших двоичных объектов) используется для секционирования, данные на диапазоны и эти диапазоны распределяются во всей системе. Это означает, что соглашения об именовании, такие как лексическое упорядочение (например, *mypayroll*, *myperformance*, *myemployees*т. д.) или с использованием отметок времени ( *log20160101*, *log20160102*, *log20160102*т. д.), будут применяться для секций, которые могут размещаться на одном сервере секционирования, пока Операция балансировки нагрузки разбивает их на более мелкие диапазоны. Например все большие двоичные объекты в контейнере может предоставляться на одном сервере пока нагрузка на эти большие двоичные объекты требует дальнейшей повторной балансировки диапазонов секций. Точно так же группу незначительно нагруженных учетных записей с именами, упорядоченными в лексическом порядке, может обслуживать один и тот же сервер, пока нагрузка на одну или несколько из этих учетных записей не потребует разбить их между несколькими серверами секций. Каждая операция балансировки нагрузки может вызывать задержку вызовов хранилища во время ее выполнения. Способность системы справляться с внезапным трафика в секцию, ограничивается масштабируемость единственного сервера секционирования, пока операция балансировки нагрузки в запущена и повторно балансирует диапазона ключей разделов.

Частоту выполнения таких операций можно уменьшить.  

* По возможности используйте Put Blob или Put Block размера (больше 4 МБ для стандартных учетных записей и больше, чем 256 Киб для учетных записей "премиум") для активации большого двоичного объекта блока высокой пропускной способностью (HTBB). HTBB предоставляет высокой производительности приема, которые не подвержены именование разделов.
* Внимательно проверьте используемое соглашение об именовании учетных записей, контейнеров, больших двоичных объектов, таблиц и очередей. Рассмотрите возможность добавления префикса учетной записи, контейнера или имена больших двоичных объектов с хэшем из 3 цифр, используя функцию хэширования, наиболее подходящий.  
* Если данные упорядочены по меткам времени или числовым идентификаторам, убедитесь в том, что вы используете инкрементируемые (или декрементируемые) шаблоны трафика. Эти шаблоны не подходят для системы секционирования на основе диапазонов и могут приводить к тому, что весь трафик будет направляться в одну и ту же секцию, препятствуя эффективной балансировке нагрузки. Например при наличии ежедневных операций, использующие большого двоичного объекта с меткой времени, такие как *ГГГГММДД*, а затем весь трафик этой ежедневной операции направляется на один объект, который обслуживается единственного сервера секционирования. Проверьте, соответствуют ли ограничения по BLOB-объектам и ограничения по секциям вашим потребностям и, если нужно, разбейте эту операцию на несколько BLOB-объектов. Точно так же, если в таблицах хранятся данные временных рядов, весь трафик может направляться в последнюю часть ключевого пространства имен. Если необходимо использовать метки времени или числовые идентификаторы, префикс идентификатор с трехзначный хэш трехзначного префиксное секунд даты типа время например *ssyyyymmdd*. Если операции перечисления и запросов выполняются регулярно, выберите функцию хэширования, ограничивающую число запросов. В других случаях может быть достаточно случайного префикса.  
* Дополнительные сведения о схеме секционирования, используемый в службе хранилища Azure, см. в разделе [хранилища Azure: A Highly Available Cloud Storage Service with Strong Consistency](https://sigops.org/sosp/sosp11/current/2011-Cascais/printable/11-calder.pdf) (Хранилище Microsoft Azure: высокодоступная служба облачного хранения со строгой согласованностью).
...
#### <a name="subheading10"></a>Увеличить минимальное количество потоков, если с помощью синхронного кода с помощью асинхронных задач

Этот код будет увеличить минимальное количество потоков в пуле потоков:  
...
#### <a name="subheading11"></a>Воспользуйтесь преимуществами .NET 4.5 и более поздней версии сборки мусора
...
### <a name="subheading13"></a>Хранилище клиентских библиотек и средств
...
#### <a name="subheading14"></a>Регулирование и сервером ошибки занятости
...
#### <a name="subheading15"></a>Неповторяемых ошибок
...
В дополнение к проверенным практическим подходам [все службы](#allservices) было описано выше, следующие проверенные подходы применяются специально для службы BLOB-объектов.  

### <a name="blob-specific-scalability-targets"></a>Целевые показатели масштабируемости, характерные для BLOB-объектов
...
#### <a name="subheading16"></a>Пропускная способность больших двоичных объектов и операции
...
#### <a name="subheading17"></a>Копирование большого двоичного объекта
...
Служба BLOB-объектов поддерживает запросы head, которые могут включать метаданные большого двоичного объекта. Например, если приложению необходимы EXIF-данные из фотографии, оно может получить фотографию и извлечь их из нее. Для сохранения пропускной способности и повышения производительности приложение может сохранить данные EXIF в метаданных большого двоичного объекта при загрузке фотографии приложением: можно получить данные EXIF, находящиеся в метаданных, используя только запрос HEAD, тем самым экономя значительную пропускную способность и время обработки, необходимые для извлечения данных EXIF каждый раз при чтении большого двоичного объекта. Это было бы полезным в ситуациях, когда необходимы только метаданные, а не полное содержимое BLOB-объекта.  В большом двоичном объекте могут храниться только 8 КБ метаданных (служба не будет принимать запрос на хранение большего, чем этот, объема данных), поэтому если данные превышают этот размер, использование такого подхода становится невозможным.  
...
Чтобы быстро отправить большие двоичные объекты, — для ответа на первый вопрос: вы отправляете один большой двоичный объект или много?  Чтобы правильно выбрать метод работы, используйте сведения из приведенного ниже руководства.  
...
### <a name="subheading24"></a>Целевые показатели масштабируемости, связанные с таблицами
...
#### <a name="subheading26"></a>Отключить Nagle
...
###### <a name="table-queries"></a>Запросы к таблицам
...
##### <a name="limiting-the-amount-of-data-returned"></a>Ограничение объема возвращаемых
...
Кроме того, приложение может хранить данные о загрузке ЦП для каждого часа в виде отдельного свойства одной сущности: для ежечасного обновления приложение может использовать один вызов **InsertOrMerge Upsert**, чтобы обновить значение за последний час. Для построения этих данных приложение достаточно извлечь одну сущность, а не 24, выполняя эффективный запрос (см. в разделе выше обсуждение на [область запроса](#subheading30)).
...
### <a name="subheading39"></a>Целевые показатели масштабируемости
...
### <a name="subheading40"></a>Отключить Nagle
...
Очередь производительность и масштабируемость уменьшить увеличении размера сообщения. В сообщении следует размещать только ту информацию, которая необходима получателю.  
...
### <a name="subheading44"></a>Обновление сообщения

Можно использовать **сообщения обновление** операции, чтобы увеличить время ожидания невидимости или обновить сведения о состоянии сообщения. Это мощная, помните, что каждая **сообщение об обновлении** операции учитывается при подсчете целевого показателя масштабируемости. Тем не менее ее использование может быть гораздо более эффективным, чем наличие рабочего процесса, который передает работу из одной очереди в другую по мере завершения каждого этапа работы. С помощью **сообщение об обновлении** операция позволяет приложению сохранить состояние работы в сообщение и затем продолжить работу, а не requeuing сообщение для следующего шага задания, всякий раз по завершении шага.  
