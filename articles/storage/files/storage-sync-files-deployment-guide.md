...
* Модули Az и AzureRM PowerShell.
    - Модуль Az можно установить, следуя инструкциям из этой статьи: [Установите и настройте Azure PowerShell](https://docs.microsoft.com/powershell/azure/install-Az-ps). 
    - Модуль AzureRM PowerShell можно установить, выполнив следующий командлет PowerShell:
...
# [<a name="portal"></a>Microsoft Azure](#tab/azure-portal)
Чтобы развернуть службу синхронизации хранилища, перейдите к статье [портала Azure](https://portal.azure.com/), нажмите кнопку *создать ресурс* и выполните поиск службы синхронизации файлов Azure. В результатах поиска выберите **Служба синхронизации файлов Azure**, а затем щелкните **Создать**, чтобы открыть вкладку **Развертывание службы синхронизации хранилища**.

В открывшейся области введите следующие сведения:

- **Name**. Уникальное имя (в подписке) для службы синхронизации хранилища.
...
Конечная точка облака является указателем файлового ресурса Azure. Все конечные точки сервера будут синхронизироваться с облачной конечной точкой, превращая ее в центр. Учетная запись хранения файлового ресурса Azure должна располагаться в том же регионе, что и служба синхронизации хранилища. Весь файловый ресурс Azure будет синхронизирован с одним исключением: будет подготовлена специальная папка, сопоставимая со скрытой папкой System Volume Information на томе NTFS. Этот каталог называется ".SystemShareInformation". Он содержит важные метаданные синхронизации, которые не синхронизируются с другими конечными точками. Не используйте и не удаляйте его!
...
- **Путь**. Путь в Windows Server, синхронизируемый как элемент группы синхронизации.
- **Уровни в облаке**. Параметр, позволяющий включать и отключать распределение по уровням в облаке. С помощью этого параметра редко используемые файлы можно распределить по уровням компонента "Файлы Azure".
...
- [Добавить или удалить конечную точку сервера синхронизации файлов Azure](storage-sync-files-server-endpoint.md)
- [Регистрация или Отмена регистрации сервера в службе синхронизации файлов Azure](storage-sync-files-server-registration.md)
