...
*Конечная точка сервера* представляет конкретное расположение на *зарегистрированном сервере*, например папку на томе сервера или в корне тома. В томе могут располагаться несколько конечных точек сервера, если их пространства имен не перекрываются (например, F:\sync1 и F:\sync2). Политики распределения по уровням облака можно настроить отдельно для каждой конечной точки сервера. При добавлении расположения сервера с имеющимся набором файлов в качестве конечной точки сервера в группу синхронизации эти файлы будут объединены с любыми другими файлами, которые уже находятся в других конечных точках в группе синхронизации.
