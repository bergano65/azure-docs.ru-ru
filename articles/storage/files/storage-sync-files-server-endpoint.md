...
*Конечная точка сервера* представляет определенное место на *зарегистрированном сервере,* например папку на громкости сервера или корень громкости. В томе могут располагаться несколько конечных точек сервера, если их пространства имен не перекрываются (например, F:\sync1 и F:\sync2). Политики распределения по уровням облака можно настроить отдельно для каждой конечной точки сервера. При добавлении расположения сервера с имеющимся набором файлов в качестве конечной точки сервера в группу синхронизации эти файлы будут объединены с любыми другими файлами, которые уже находятся в других конечных точках в группе синхронизации.
...
```
Указание `-Order CloudTieringPolicy` вспомнит последние недавно измененные файлы в первую очередь.
Другие необязательные, но полезные параметры для рассмотрения являются:
* `-ThreadCount`определяет, сколько файлов может быть отозвано параллельно.
