...
ms.locfileid: "82176096"
---
# <a name="planning-for-an-azure-file-sync-deployment"></a>Планирование развертывания службы синхронизации файлов Azure

:::row:::
    :::column:::
        [![Собеседование и демонстрационные материалы, посвященные Синхронизация файлов Azureию для игр!](./media/storage-sync-files-planning/azure-file-sync-interview-video-snapshot.png)](https://www.youtube.com/watch?v=nfWLO7F52-s)
    :::column-end:::
    :::column:::
        ...
        В этой статье рассказывается, как Синхронизация файлов Azure концепции и функции. Когда вы знакомы с Синхронизация файлов Azure, ознакомьтесь с руководством по [развертыванию синхронизация файлов Azure](storage-sync-files-deployment-guide.md) , чтобы испытать эту службу.        
    :::column-end:::
:::row-end:::

Файлы будут храниться в облаке в [файловых ресурсах Azure](storage-files-introduction.md). Файловые ресурсы Azure можно использовать двумя способами: путем прямого подключения этих файловых ресурсов Azure (SMB) без сервера или путем кэширования файловых ресурсов Azure в локальной среде с помощью Синхронизация файлов Azure. Выбор варианта развертывания изменяет аспекты, которые необходимо учитывать при планировании развертывания. 
...
Чтобы включить функцию синхронизации в Windows Server, необходимо установить загружаемый Синхронизация файлов Azure агент. Агент Синхронизация файлов Azure предоставляет два основных компонента: `FileSyncSvc.exe`, фоновую службу Windows, ответственную за отслеживание изменений в конечных точках сервера и запуск сеансов синхронизации, а `StorageSync.sys`также фильтр файловой системы, обеспечивающий распределение по уровням облака и быстрое аварийное восстановление.  
...
Сторонние продукты, которые работают аналогично BitLocker, в том, что они находятся под томом NTFS, должны также полностью прозрачно работать с Синхронизация файлов Azure. 
...
> [!NOTE]
> Синхронизация файлов Azure служба удалит поддержку TLS 1.0 и 1,1 в августе 2020. Все поддерживаемые версии агента Синхронизация файлов Azure уже используют TLS 1.2 по умолчанию. Использование более ранней версии TLS может произойти, если протокол TLS 1.2 был отключен на сервере или используется прокси-сервер. Если вы используете прокси-сервер, рекомендуем проверить конфигурацию прокси-сервера. Синхронизация файлов Azure регионы служб, добавленные после 5/1/2020, будут поддерживать TLS 1.2, а поддержка TLS 1.0 и 1,1 будут удалены из существующих регионов в августе 2020.  Дополнительные сведения см. в разделе [руководство по устранению неполадок](storage-sync-files-troubleshoot.md#tls-12-required-for-azure-file-sync).
...
| Public | Франция | Южная Франция * | `francesouth` |
...
| Public | ЮАР | Западная Южная Африка * | `southafricawest` |
| Public | ОАЭ | Центральная часть ОАЭ * | `uaecentral` |
...
- Создайте конечную точку сервера только на новом файловом сервере и скопируйте данные из старого файлового ресурса с помощью `robocopy`. В зависимости от топологии общих файловых ресурсов на новом сервере (количество общих папок на каждом томе, как освободить каждый том и т. д.) может потребоваться временное предоставление дополнительного хранилища, так как ожидается, что от старого сервера `robocopy` до нового сервера в локальном центре обработки данных будет выполняться быстрее, чем синхронизация файлов Azure переместит данные в Azure.
