...
Прежде чем развертывать службу синхронизации файлов Azure, следует оценить, появлялось ли она стала совместима с вашей системой, используя командлет оценки службы синхронизации файлов Azure. Этот командлет проверяет на наличие потенциальных проблем с файловой системы и набора данных, таких как неподдерживаемые символы или неподдерживаемая версия операционной системы. Обратите внимание, что проверки охватывают большинство, но не все возможности, описанные ниже. Мы рекомендуем прочесть всю оставшуюся часть этого раздела очень внимательно, чтобы осуществить развертывание надлежащим образом. 

Командлет оценки можно установить при установке модуля Az PowerShell, который можно установить, следуя приведенным ниже указаниям: [Установите и настройте Azure PowerShell](https://docs.microsoft.com/powershell/azure/install-Az-ps).
...
- Сервер под управлением Windows Server 2012 R2, Windows Server 2016 или Windows Server 2019.
    ...
    |---------|----------------|------------------------------|
    | Windows Server 2019 | Datacenter и Standard | Full и Core |
    | Windows Server 2016 | Datacenter и Standard | Full и Core |
    | Windows Server 2012 R2 | Datacenter и Standard | Full и Core |
...
**Версия агента 5.0.2.0 или более поздней версии**   
Дедупликация данных поддерживается в томах с включенным распределением по уровням в облаке в Windows Server 2016 и Windows Server 2019. Включение дедупликации данных на томе с распределением по уровням облака позволяет кэшировать несколько файлов локальных без подготовки больший объем хранилища. 

При включении дедупликации данных на томе с распределением по уровням облака, задает выполнение дедупликации оптимизированных файлов в расположение конечной точки сервера, будет многоуровневым аналогичную обычный файл на основе облака, распределение по уровням параметры политики. После дедупликации, оптимизированные файлы, был перемещен, задания сбора мусора дедупликации данных будут выполняться автоматически для освобождения места на диске, удалив неиспользуемые блоки, на которые больше не другие файлы в томе.

Обратите внимание, что экономия тома применяются только для сервера. не будет быть дедуплицированных данных в файловый ресурс Azure.
...
**Примечания**
- Если Дедупликация данных установлен до установки агента службы синхронизации файлов Azure, требуется перезагрузка для поддержки дедупликации данных и уровней в облаке на одном томе.
- При включении дедупликации данных в томе после облако включено распределение по уровням, исходное задание оптимизации дедупликации оптимизирует файлы на томе, которые уже не перемещаются и будет иметь следующие последствия в облаке распределение по уровням:
    - Политика свободное место будет продолжать файлы по уровням согласно свободное пространство на томе с помощью тепловой карты.
    - Политика даты будет пропускать распределение по уровням файлов, возможно, в противном случае подходящих для распределения по уровням, из-за оптимизации задания дедупликации, обращаться к файлам.
- Для текущих заданий оптимизации дедупликации, распределения по уровням с помощью политики даты облака будет получить задержан дедупликации данных [MinimumFileAgeDays](https://docs.microsoft.com/powershell/module/deduplication/set-dedupvolume?view=win10-ps) параметр, если файл, не перемещаются. 
    - Пример: Если параметр MinimumFileAgeDays составляет 7 дней и Дата политику распределения по уровням облака — 30 дней, политика будет распределять файлы уровням 37 дней после даты.
    - Примечание. Если файл перемещается службой синхронизации файлов Azure, задания оптимизации дедупликации будет пропускать этот файл.
- Если на сервере под управлением Windows Server 2012 R2 с установленным агентом службы синхронизации файлов Azure обновляется до Windows Server 2016 или Windows Server 2019, для поддержки дедупликации данных и уровней в облаке на том же томе необходимо выполнить следующие действия:  
    - Удаление агента службы синхронизации файлов Azure для Windows Server 2012 R2 и перезапустите сервер.
    - Скачайте агент службы синхронизации файлов Azure для новой версии операционной системы сервера (Windows Server 2016 или Windows Server 2019).
    - Установите агент службы синхронизации файлов Azure и перезапустите сервер.  
    
    Примечание. Параметры конфигурации службы синхронизации файлов Azure на сервере удаляются после удаления и переустановки агента.
...
> [!Note]  
> Антивирусного программного обеспечения можно проверить совместимость своих продуктов и службы синхронизации файлов Azure с помощью [набор тестов для синхронизации антивирусной программы Azure файл совместимости](https://www.microsoft.com/download/details.aspx?id=58322), которая доступна для загрузки в центре загрузки Майкрософт.
...
| Аризона (для обслуживания государственных организаций США) | Аризона |
| Техас (для обслуживания государственных организаций США) | Техас |
| Правительство штата Вирджиния | Виргиния |
| Западная Европа | Нидерланды |
| Западно-центральная часть США | Вайоминг |
| Запад США | Калифорния |
| Западный регион США 2 | Вашингтон |
...
| US Gov (Айова)         | Правительство штата Вирджиния    |
