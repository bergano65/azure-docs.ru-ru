...
- **FileSyncSvc. exe**— фоновая служба, отвечающая за отслеживание изменений в конечных точках сервера и запуск сеансов синхронизации в Azure.
...
| Южная Бразилия | Состояние-Паулу Сан |
| Центральная Канада | Toronto |
...
Чтобы защититься от потери региона Azure, Синхронизация файлов Azure интегрируется с [географически избыточным хранилищем](../common/storage-redundancy.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json) (GRS). GRS работает с использованием репликации асинхронных блоков между хранилищем в основном регионе, в которым вы обычно работаете, и хранилищем в парном дополнительном регионе. В случае аварии, в результате которой регион Azure временно или постоянно переходит в автономный режим, корпорация Майкрософт будет отработка отказа хранилища в парный регион. 
...
|---------------------|--------------------|
| Восточная Австралия      | Юго-Восточная часть Австралии|
| Юго-Восточная часть Австралии | Восточная Австралия     |
...
| Северная часть ЮАР;  | Западная Южная Африка  |
| Западная Южная Африка   | Северная часть ЮАР; |
...
## <a name="recommended-azure-file-sync-machine-configuration"></a>Рекомендуемая конфигурация компьютера Синхронизация файлов Azure

Требования к Синхронизация файлов Azure машине определяются количеством объектов в пространстве имен и обновлением набора данных. Один сервер можно подключить к нескольким группам синхронизации, а количество объектов, перечисленных в следующих учетных записях, — для полного пространства имен, к которому подключен сервер. Например, конечная точка сервера A с 10 000 000 объектами + Server Endpoint B с 10 000 000 объектами = 20 000 000 Objects. Для этого примера развертывания мы рекомендуем 8CPU, 16GiB память для стабильного состояния и (если возможно) 48GiB памяти для первоначальной миграции.
...
В следующей таблице мы предоставили как размер пространства имен, так и преобразование в емкость для типичных общих файловых ресурсов общего назначения, где средний размер файла — 512KiB. Если размер файлов меньше, рекомендуется добавить дополнительную память для того же объема емкости. Настройка конфигурации памяти на основе размера пространства имен.
...
| 5        | 2.4     | 2        | 16 (начальная синхронизация)/4 (типичное обновление)    |
| 10       | 4.8     | 4        | 32 (начальная синхронизация)/8 (типичное обновление)   |
| 30       | 14,3    | 8        | 48 (начальная синхронизация)/16 (типичное обновление)   |
| 50       | 23,8    | 16       | 64 (начальная синхронизация)/32 (типичное обновление)  |
| 100*     | 47,7   | 32       | 128 (начальная синхронизация)/32 (типичное обновление)  |

\*более 100 000 000 файлов, & каталоги не проверялись. Это мягкое ограничение.
