...
- Если вы планируете использовать Azure PowerShell, [установите последнюю версию](https://docs.microsoft.com/powershell/azure/install-az-ps).
...
#### <a name="tags"></a>Теги
Теги — это пары "имя-значение", которые позволяют классифицировать ресурсы и просматривать консолидированный счет за счет применения одного и того же тега к нескольким ресурсам и группам ресурсов. Они необязательны и могут быть применены после создания учетной записи хранения.
...
Чтобы создать учетную запись хранения с помощью PowerShell, мы будем использовать командлет `New-AzStorageAccount`. У этого командлета есть множество параметров. отображаются только необходимые параметры. Дополнительные сведения о дополнительных параметрах см. в [документации по командлету`New-AzStorageAccount`](/powershell/module/az.storage/new-azstorageaccount).
...
Чтобы создать учетную запись хранения, способную хранить стандартные общие файловые ресурсы Azure, мы будем использовать следующую команду. Параметр `-SkuName` относится к требуемому типу избыточности. Если требуется геоизбыточная учетная запись хранения, избыточная или геозона, необходимо также удалить параметр `-EnableLargeFileShare`.
...
Чтобы создать учетную запись хранения, способную хранить файловые ресурсы Azure уровня "Премиум", мы будем использовать следующую команду. Обратите внимание, что параметр `-SkuName` изменен на включение `Premium` и требуемый уровень избыточности локально избыточного (`LRS`). `-Kind` параметр `FileStorage`, а не `StorageV2`, так как общие файловые ресурсы класса Premium должны создаваться в учетной записи хранения Филестораже, а не в учетной записи хранения GPv2.
...
Чтобы создать учетную запись хранения с помощью Azure CLI, мы будем использовать команду AZ Storage Account Create. Эта команда имеет много параметров. отображаются только необходимые параметры. Дополнительные сведения о дополнительных параметрах см. в [документации по командам`az storage account create`](/cli/azure/storage/account).
...
Чтобы создать учетную запись хранения, способную хранить стандартные общие файловые ресурсы Azure, мы будем использовать следующую команду. Параметр `--sku` относится к требуемому типу избыточности. Если требуется геоизбыточная учетная запись хранения, избыточная или геозона, необходимо также удалить параметр `--enable-large-file-share`.
...
Чтобы создать учетную запись хранения, способную хранить файловые ресурсы Azure уровня "Премиум", мы будем использовать следующую команду. Обратите внимание, что параметр `--sku` изменен на включение `Premium` и требуемый уровень избыточности локально избыточного (`LRS`). `--kind` параметр `FileStorage`, а не `StorageV2`, так как общие файловые ресурсы класса Premium должны создаваться в учетной записи хранения Филестораже, а не в учетной записи хранения GPv2.
...
Выберите **создать** , чтобы завершить создание новой общей папки. Обратите внимание, что если ваша учетная запись хранения находится в виртуальной сети, вы не сможете успешно создать файловый ресурс Azure, если клиент также не находится в виртуальной сети. Вы также можете обойти это ограничение на момент времени с помощью командлета Azure PowerShell `New-AzRmStorageShare`.

# <a name="powershell"></a>[PowerShell](#tab/azure-powershell)
Файловый ресурс Azure можно создать с помощью командлета [`New-AzRmStorageShare`](/powershell/module/az.storage/New-AzRmStorageShare) . Следующие команды PowerShell предполагают, что вы установили переменные `$resourceGroupName` и `$storageAccountName`, как указано выше в разделе Создание учетной записи хранения с Azure PowerShell. 

> [!Important]  
> Для файловых ресурсов уровня "Премиум" параметр `-QuotaGiB` относится к подготовленному размеру файлового ресурса. Подготовленный размер общей папки — это сумма, за которую будет взиматься плата, независимо от использования. Счета за стандартные общие ресурсы выставляются на основе использования, а не подготовленного размера.
...
# <a name="azure-cli"></a>[Azure CLI](#tab/azure-cli)
Прежде чем можно будет создать файловый ресурс Azure с Azure CLI, необходимо получить ключ учетной записи хранения для авторизации операции создания файлового ресурса в. Это можно сделать с помощью команды [`az storage account keys list`](/cli/azure/storage/account/keys) :
...
Получив ключ учетной записи хранения, вы можете создать файловый ресурс Azure с помощью команды [`az storage share create`](/cli/azure/storage/share) . 

> [!Important]  
> Для файловых ресурсов уровня "Премиум" параметр `--quota` относится к подготовленному размеру файлового ресурса. Подготовленный размер общей папки — это сумма, за которую будет взиматься плата, независимо от использования. Счета за стандартные общие ресурсы выставляются на основе использования, а не подготовленного размера.
...
Эта команда завершится ошибкой, если учетная запись хранения содержится в виртуальной сети, а компьютер, с которого вы вызываете эту команду, не является частью виртуальной сети. Это ограничение на момент времени можно обойти с помощью командлета Azure PowerShell `New-AzRmStorageShare`, как описано выше, или путем выполнения Azure CLI с компьютера, который является частью виртуальной сети, включая подключение через VPN.
