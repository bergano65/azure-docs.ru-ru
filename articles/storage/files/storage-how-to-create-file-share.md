...
Чтобы создать учетную запись хранения с помощью PowerShell, мы будем `New-AzStorageAccount` использовать командлет. У этого командлета есть множество параметров. отображаются только необходимые параметры. Дополнительные сведения о дополнительных параметрах см. в [ `New-AzStorageAccount` документации по командлетам](/powershell/module/az.storage/new-azstorageaccount).
...
Чтобы создать учетную запись хранения, способную хранить файловые ресурсы Azure уровня "Премиум", мы будем использовать следующую команду. Обратите внимание `-SkuName` , что параметр изменился так `Premium` , чтобы он включал и, и требуемый уровень`LRS`избыточности локально избыточного (). Параметр `-Kind` имеет значение `FileStorage` , `StorageV2` так как общие файловые ресурсы уровня "Премиум" должны создаваться в учетной записи хранения филестораже, а не в учетной записи хранения GPv2.
...
Чтобы создать учетную запись хранения, способную хранить файловые ресурсы Azure уровня "Премиум", мы будем использовать следующую команду. Обратите внимание `--sku` , что параметр изменился так `Premium` , чтобы он включал и, и требуемый уровень`LRS`избыточности локально избыточного (). Параметр `--kind` имеет значение `FileStorage` , `StorageV2` так как общие файловые ресурсы уровня "Премиум" должны создаваться в учетной записи хранения филестораже, а не в учетной записи хранения GPv2.
...
> [!Important]  
> Для файловых ресурсов уровня " `-QuotaGiB` Премиум" параметр ссылается на подготовленный размер общей папки. Подготовленный размер общей папки — это сумма, за которую будет взиматься плата, независимо от использования. Счета за стандартные общие ресурсы выставляются на основе использования, а не подготовленного размера.
...
> [!Important]  
> Для файловых ресурсов уровня " `--quota` Премиум" параметр ссылается на подготовленный размер общей папки. Подготовленный размер общей папки — это сумма, за которую будет взиматься плата, независимо от использования. Счета за стандартные общие ресурсы выставляются на основе использования, а не подготовленного размера.
...
Эта команда завершится ошибкой, если учетная запись хранения содержится в виртуальной сети, а компьютер, с которого вы вызываете эту команду, не является частью виртуальной сети. Вы можете обойти это ограничение на момент времени с помощью командлета Azure PowerShell `New-AzRmStorageShare` , как описано выше, или путем выполнения Azure CLI с компьютера, который является частью виртуальной сети, включая через VPN-подключение.
