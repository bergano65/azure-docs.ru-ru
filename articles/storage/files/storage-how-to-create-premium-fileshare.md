...
1. Оставьте **репликации** присвоено значение по умолчанию **локально избыточное хранилище (LRS)**.
    ...
    ![Как создать учетную запись хранения для общей папки "премиум"](media/storage-how-to-create-premium-fileshare/premium-files-storage-account.png)
...
Для взаимодействия с уровня "премиум" общей папки с помощью PowerShell, вам потребуется установить последнюю версию модуля Az.Storage.
...
### <a name="add-the-preview-storage-cli-extension"></a>Добавление предварительной версии хранилища расширение интерфейса командной строки

Так как файловые ресурсы уровня "премиум" — это функция предварительной версии, необходимо добавить расширение предварительной версии в оболочке. Для этого с помощью Cloud Shell или локальный оболочки введите следующую команду: `az extension add --name storage-preview`
