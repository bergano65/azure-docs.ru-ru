...
- Утилиты CIFS (cifs-utils) не устанавливаются на клиенте.
- На клиенте не установлена минимальная необходимая версия SMB или CIFS 2.1.
- На клиенте не поддерживается шифрование SMB 3.0. В предыдущей таблице представлен список дистрибутивов Linux, которые поддерживают монтаж с помощью шифрования. В других дистрибутивах нужно использовать ядро 4.11 или более поздней версии.
...
<a id="mounterror13"></a>
## <a name="mount-error13-permission-denied-when-you-mount-an-azure-file-share"></a>"Ошибка горы (13): Разрешение отказано" при установке раздела файла Azure
...
**\<имя файла> «разрешение отказано» Диск квота превышена**
...
- Используйте правильный метод копирования:
    ...
    - Использование cp или dD с параллельом может улучшить скорость копирования, количество потоков зависит от случая использования и рабочей нагрузки. Ниже приведены шесть примеров: 
    - cp пример (cp будет использовать размер блока по умолчанию `find * -type f | parallel --will-cite -j 6 cp {} /mntpremium/ &`файловой системы в качестве размера фрагмента): .
    - пример dD (эта команда явно устанавливает размер куска до 1 MiB):`find * -type f | parallel --will-cite-j 6 dd if={} of=/mnt/share/{} bs=1M`
    - Инструменты третьих сторон с открытым исходным кодом, такие как:
        - [GNU Параллель](https://www.gnu.org/software/parallel/).
        - [Fpart](https://github.com/martymac/fpart) - Сортирует файлы и упаковывает их в разделы.
        - [Fpsync](https://github.com/martymac/fpart/blob/master/tools/fpsync) - Использует Fpart и инструмент копирования для порождения нескольких экземпляров для переноса данных из src_dir в dst_url.
        - [Multi](https://github.com/pkolano/mutil) - Многопоточный CP и md5sum на основе coreutils GNU.
- Установка размера файла заранее, вместо того, чтобы каждый писать расширяющейся записи, помогает улучшить скорость копирования в сценариях, где размер файла известен. Если необходимо избежать расширения записей, можно установить `truncate - size <size><file>` размер файла назначения с помощью команды. После этого `dd if=<source> of=<target> bs=1M conv=notrunc`команда будет копировать исходный файл без повторного обновления размера целевого файла. Например, можно установить размер файла назначения для каждого файла, который вы хотите скопировать (предположим, что доля установлена под /mnt/share):
    - `$ for i in `` find * -type f``; do truncate --size ``stat -c%s $i`` /mnt/share/$i; done`
    - а затем - копировать файлы без расширения записывает параллельно:`$find * -type f | parallel -j6 dd if={} of =/mnt/share/{} bs=1M conv=notrunc`
...
Функция шифрования протокола SMB 3.0 для Linux появилась в ядре версии 4.11. Эта функция позволяет подключать файловый ресурс Azure из локальной среды или другого региона Azure. Некоторые дистрибутивы Linux, возможно, отодвигали изменения от ядра 4.11 к более старым версиям ядра Linux, которые они поддерживают. Чтобы помочь в определении того, поддерживает ли ваша версия Linux SMB 3.0 с помощью шифрования, обратитесь к [Use Azure Files с Linux.](storage-how-to-use-files-linux.md) 
...
### <a name="cause-1-caching"></a>Причина 1: Кэшинг

Одной из возможных причин снижения производительности может быть отключенное кэширование. Кэширование может быть полезно, если вы получаете доступ к файлу повторно, в противном случае, это может быть накладные расходы. Проверьте, используете ли вы кэш, прежде чем отключить его.
...
В некоторых сценариях параметр подключения **serverino** может привести к тому, что команда **ls** формирует статистику для каждой записи каталога. Такое поведение приводит к снижению производительности при перечислении большого каталога. Параметры подключения можно просмотреть в записи **/etc/fstab**.
...
### <a name="cause-2-throttling"></a>Причина 2: Дрожь

Возможно, вы испытываете регулирование и ваши запросы отправляются в очередь. Вы можете проверить это, используя [метрики хранения Azure в Azure Monitor.](../common/storage-metrics-in-azure-monitor.md)

### <a name="solution-for-cause-2"></a>Решение для причины 2

Убедитесь, что ваше приложение находится в [пределах целевых показателей масштаба Azure Files.](storage-files-scale-targets.md#azure-files-scale-targets)
...
Тем не менее эти изменения могут быть перенесены еще не во все дистрибутивы Linux. Если вы используете популярный дистрибутив Linux, вы можете проверить [файлы использования Azure с Linux,](storage-how-to-use-files-linux.md) чтобы увидеть, какая версия дистрибутива имеет необходимые изменения ядра.
...
## <a name="cifs-vfs-error--22-on-ioctl-to-get-interface-list-when-you-mount-an-azure-file-share-by-using-smb-30"></a>"CIFS VFS: ошибка -22 на ioctl, чтобы получить список интерфейсов", когда вы устанавливаете долю файла Azure с помощью SMB 3.0

### <a name="cause"></a>Причина
Эта ошибка регистрируется, поскольку файлы Azure [в настоящее время не поддерживают многоканальный SMB.](https://docs.microsoft.com/rest/api/storageservices/features-not-supported-by-the-azure-file-service)

### <a name="solution"></a>Решение
Эту ошибку можно пропускать.
