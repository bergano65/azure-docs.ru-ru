...
description: Устранение неполадок службы файлов Azure в Linux. См. раздел распространенные проблемы, связанные с файлами Azure, при подключении из клиентов Linux и просмотр возможных решений.
author: jeffpatt24
ms.service: storage
...
| --- | :---: | :---: |
| **Сервер Ubuntu** | 14.04 или более поздней версии | 16.04+ |
...
В одном файле или каталоге имеется квота 2 000 открытых дескрипторов. По достижении этого ограничения вы увидите соответствующую ошибку.
...
- Используйте командлет PowerShell [Get-азсторажефилехандле](/powershell/module/az.storage/get-azstoragefilehandle) для просмотра открытых дескрипторов.

- Чтобы закрыть открытые дескрипторы, используйте командлет PowerShell [Close-азсторажефилехандле](/powershell/module/az.storage/close-azstoragefilehandle) . 
...
Возможно, вы испытываете регулирование и ваши запросы отправляются в очередь. Это можно проверить, используя [метрики службы хранилища Azure в Azure Monitor](../blobs/monitor-blob-storage.md).
...
### <a name="cause"></a>Причина:
Эта ошибка регистрируется, так как служба файлов Azure [в настоящее время не поддерживает многоканальный протокол SMB](/rest/api/storageservices/features-not-supported-by-the-azure-file-service).

### <a name="solution"></a>Решение
Эту ошибку можно пропускать.


### <a name="unable-to-access-folders-or-files-which-name-has-a-space-or-a-dot-at-the-end"></a>Не удается получить доступ к папкам или файлам, имена которых имеют пробел или точку в конце
...
Используйте параметр мапчарс в общем ресурсе при подключении общей папки в Linux: 
