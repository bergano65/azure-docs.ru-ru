...
Службы [файлов Azure](storage-files-introduction.md)   поддерживает проверку подлинности на основе удостоверений через протокол SMB с помощью двух типов доменных служб: локальных домен Active Directory служб (AD DS) и Azure Active Directory доменных служб (Azure AD DS). Мы настоятельно рекомендуем ознакомиться с [разделом "как работает](./storage-files-active-directory-overview.md#how-it-works) ", чтобы выбрать подходящую службу домена для проверки подлинности. Программа установки различается в зависимости от выбранной службы домена. В этой серии статей основное внимание уделяется включению и настройке локальной AD DS для проверки подлинности с помощью файловых ресурсов Azure.
...
- Выберите или создайте [среду AD DS](/windows-server/identity/ad-ds/get-started/virtual-dc/active-directory-domain-services-overview) и [синхронизируйте ее с Azure AD](../../active-directory/hybrid/how-to-connect-install-roadmap.md) с Azure AD Connect. 
...
- Домен — присоединение локального компьютера или виртуальной машины Azure к локальной AD DS. Сведения о том, как присоединиться к домену, см. в разделе [Присоединение компьютера к домену](/windows-server/identity/ad-fs/deployment/join-a-computer-to-a-domain).
...
Если вы планируете включить любые сетевые конфигурации в общем файловом ресурсе, рекомендуем ознакомиться со статьей рекомендации по работе с [сетью](./storage-files-networking-overview.md) и завершить соответствующую конфигурацию, прежде чем включать проверку подлинности AD DS.
