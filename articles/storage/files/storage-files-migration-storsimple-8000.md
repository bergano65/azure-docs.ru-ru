...
:::row:::
    :::column:::
        ![На рисунке показано время, необходимое для инициализации виртуальной машины и предоставления виртуальной машине клонирования тома (или нескольких).](media/storage-files-migration-storsimple-shared/storsimple-8000-migration-phase-2.png)
    :::column-end:::
    :::column:::
...
* [Узнайте, как масштабировать Windows Server на основе количества элементов (файлов и папок), которые необходимо синхронизировать.](storage-sync-files-planning.md#recommended-system-resources)
    ...
    **Примечание.** Ранее связанная статья представляет собой таблицу с диапазоном памяти сервера (ОЗУ). Ориентация на большое число для виртуальной машины Azure. Вы можете ориентироваться на меньшее число для локального компьютера.
...
> [!IMPORTANT]
> Часто локальный сервер Windows используется для работы с локальным устройством StorSimple. В такой конфигурации можно включить функцию[дедупликации данных](https://docs.microsoft.com/windows-server/storage/data-deduplication/install-enable)на этом сервере Windows. **Если вы использовали дедупликацию данных с данными StorSimple, убедитесь, что вы также включили дедупликацию данных на этой виртуальной машине Azure.** Не путайте эту дедупликацию на уровне файлов с встроенной дедупликацией на уровне блока Сторсимплес, для которой никаких действий не требуется.

> [!IMPORTANT]
> Чтобы оптимизировать производительность, разверните **быстрый диск ОС** для ОБЛАЧНОЙ виртуальной машины. База данных синхронизации будет храниться на диске ОС для всех томов данных. Кроме того, убедитесь, что вы создали **большой диск операционной системы**. В зависимости от количества элементов (файлов и папок) на томах StorSimple диску ОС может потребоваться **несколько сотен гиб** пространства для размещения базы данных синхронизации.

### <a name="expose-the-storsimple-8020-volumes-to-the-azure-vm"></a>Предоставление томов StorSimple 8020 виртуальной машине Azure
...
:::row:::
    :::column:::
        ![Иллюстрация, показывающая необходимость определения и подготовки нескольких файловых ресурсов Azure и локального создания Windows Server в качестве замены устройств StorSimple.](media/storage-files-migration-storsimple-shared/storsimple-8000-migration-phase-3.png)
    :::column-end:::
    :::column:::
...
:::row:::
    :::column:::
        ![На рисунке показано, как получить виртуальную машину, подключенную с помощью Синхронизация файлов Azure, и начать первый цикл перемещения файлов из клонов томов StorSimple.](media/storage-files-migration-storsimple-shared/storsimple-8000-migration-phase-4.png)
    :::column-end:::
    :::column:::
...
1. Создайте новый каталог на системном диске виртуальной машины. Синхронизация файлов Azure сведения должны храниться там, а не на клонах подключенного тома. Пример: `"C:\syncmetadata"`
...
:::row:::
    :::column:::
        ![Иллюстрация, в которой показано, как сократить время простоя с помощью нескольких клонов томов и сообщить о завершении синхронизации.](media/storage-files-migration-storsimple-shared/storsimple-8000-migration-phase-5.png)
    :::column-end:::
    :::column:::
...
![Иллюстрация, на которой показано, как кэш Windows Server был включен в состояние устройства и не оставляет никакого файла за последним параметром Robocopy.](media/storage-files-migration-storsimple-shared/storsimple-8000-migration-phase-6.png)
...
При наличии развертывания DFS-N можно указать пространства имен ДФН-Namespace для новых расположений папок сервера. Если у вас нет развертывания DFS-N и вы построили устройство 8100 8600 локально с помощью Windows Server, вы можете отключить этот сервер от домена и присоединить новый сервер Windows к домену с помощью AFS, присвоить ей то же имя сервера, что и старый сервер, и те же имена общих папок, а затем вырезание на новый сервер остается прозрачным для пользователей. , групповая политика или скрипты.
