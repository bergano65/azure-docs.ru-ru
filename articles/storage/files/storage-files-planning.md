...
Стандартные файловые ресурсы обеспечиваются жесткие диски (HDD). Стандартные файловые ресурсы обеспечивают надежную работу для рабочих нагрузок ввода-ВЫВОДА, которые менее чувствительны к характеристикам производительности, такие как файловые ресурсы общего назначения и сред разработки и тестирования. Файловые ресурсы уровня "Стандартный" доступны только в модели выставления счетов с оплатой по мере использования.

Стандартные файловые ресурсы до 5 Тиб размер доступны как предложение общедоступной версии. Хотя большего размера общих папок, которые являются любых общих ресурсов, размер которых превышает 5 Тебибайт, максимум до 100 Tиб, в настоящее время доступны в предварительной.

> [!IMPORTANT]
> См. в разделе [. подключитесь к общим большего размера (уровня "стандартный")](#onboard-to-larger-file-shares-standard-tier) раздел для действия, чтобы решить эту задачу, а также область и ограничения предварительной версии.

### <a name="premium-file-shares"></a>Файловые ресурсы уровня "премиум"

Файловые ресурсы уровня "премиум" обеспечиваются твердотельные накопители (SSD). Файловые ресурсы уровня "премиум" предоставляют согласованная высокая производительность и низкой задержкой и в пределах одной цифры в миллисекундах для большинства операций ввода-ВЫВОДА для ИНТЕНСИВНЫХ по вводу-выводу рабочих нагрузок. Это делает их подходящий для самых разных рабочих нагрузок баз данных, размещение веб-сайтов и сред разработки. Файловые ресурсы уровня "Премиум" доступны только в подготовленной модели выставления счетов. Файловые ресурсы уровня "премиум" используется модель развертывания, отдельно от стандартных файловых ресурсов.
...
> [!NOTE]
> Производительность общие папки файла подчиняется ограничениям для компьютера сети, доступную пропускную способность сети, объемы ввода-ВЫВОДА, параллелизма, среди прочих факторов. Чтобы добиться максимальной производительности масштабирования, распределите нагрузку между несколькими виртуальными машинами. См. в статье [руководство по устранению неполадок](storage-troubleshooting-files-performance.md) некоторых распространенных проблем с производительностью и обходные пути.
...
Стандартные общие файлы Azure поддерживают три варианта избыточности данных: локально избыточное хранилище (LRS), избыточное хранилище (ZRS) и геоизбыточное хранилище (GRS).

Файловых ресурсах Azure уровня "премиум" поддерживают только локально избыточное хранилище (LRS).
...
## <a name="onboard-to-larger-file-shares-standard-tier"></a>Файловые ресурсы подключены к большего размера (уровня "стандартный")

Этот раздел относится только к общим папкам стандартный файл. Все файловые ресурсы уровня "премиум" доступны с 100 Tиб как предложение общедоступной версии.

### <a name="restrictions"></a>Ограничения

- Необходимо создать новую учетную запись хранения общего назначения (Невозможно расширить существующие учетные записи хранения).
- LRS на GRS учетной записи преобразования можно на любой новой учетной записи хранения, созданные после принятия подписку для предварительной версии общих ресурсов большего размера файлов не будет.

### <a name="regional-availability"></a>Доступность по регионам

Стандартные файловые ресурсы доступны во всех регионах до 5 Тиб. В определенных регионах он доступен с ограничением в 100 ТБ, эти регионы перечислены в следующей таблице:

|Регион  |Поддерживаемые избыточности  |Поддерживает существующие учетные записи хранения  |
|---------|---------|---------|
|Юго-Восточная Азия     |LRS|Нет         |
|Западная Европа     |LRS|Нет         |
|Западный регион США 2     |LRS, ZRS|Нет         |

Чтобы помочь нам определить приоритеты новых регионах и функциях, заполните это [опроса](https://aka.ms/azurefilesatscalesurvey).

### <a name="steps-to-onboard"></a>Процедуры подключения

Чтобы зарегистрировать подписку для предварительной версии общих ресурсов большего размера файлов, необходимо использовать Azure PowerShell. Вы можете использовать [Azure Cloud Shell](https://shell.azure.com/) или установить [модуля Azure PowerShell локально](https://docs.microsoft.com/powershell/azure/install-Az-ps?view=azps-2.4.0) для выполнения следующих команд PowerShell:

Во-первых убедитесь, что выбрана подписка, которую вы хотите зарегистрировать в предварительной версии:
...
Затем зарегистрироваться в предварительной версии, используя следующие команды:

```powershell
Register-AzProviderFeature -FeatureName AllowLargeFileShares -ProviderNamespace Microsoft.Storage
Register-AzResourceProvider -ProviderNamespace Microsoft.Storage
```
Подписки утверждается автоматически, когда обе команды запускаются.

Чтобы проверить состояние регистрации, необходимо выполнить следующую команду:

```powershell
Get-AzProviderFeature -FeatureName AllowLargeFileShares -ProviderNamespace Microsoft.Storage
```

Может занять до 15 минут для свой статус, чтобы обновить до **зарегистрирован**. После вашего статуса **зарегистрирован**, можно использовать функцию.

### <a name="use-larger-file-shares"></a>Использование большего размера общих папок

Чтобы приступить к использованию большего размера общих папок, создайте новую учетную запись хранения общего назначения версии 2 и новый файловый ресурс.

## <a name="data-growth-pattern"></a>Модель роста данных

В настоящее время максимальный размер для файлового ресурса Azure — 5 Тиб (100 ТБ на этапе предварительной версии). Из-за этого ограничения при развертывании файлового ресурса Azure следует учитывать ожидаемый рост объема данных.
