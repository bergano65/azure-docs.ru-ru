...
Все общие папки можно быстро по крайней мере 100 операций ввода-ВЫВОДА и целевой пропускную способность 100 Миб/с. Общие папки должны быть подготовлены с шагом 1 ГБ. Минимальный размер — 100 Гиб, следующего размера 101 ГИБ и т. д.

> [!TIP]
> Базовые операции ввода-ВЫВОДА = 100 + 1 * подготовлены Гиб. (До максимум 100 000 операций ввода-вывода).
>
> Повышение предела = 3 * базовых показателей операций ввода-ВЫВОДА. (До максимум 100 000 операций ввода-вывода).
>
> Коэффициент исходящих = 60 Миб/с + 0.06 подготовленной Гиб (до 6 Гиб/с)
>
> скорость приема = 40 Миб/с + 0,04 подготовленной Гиб (до 4 Гиб)

Размер общей папки может быть увеличено в любое время и уменьшить в любое время, но может быть уменьшен каждые 24 часа с момента последнего увеличения. Изменение масштаба операций ввода-ВЫВОДА и пропускной способности начнут действовать в течение 24 часов после изменения размера.
...
|Емкость (ГБ) | Базовые показатели операций ввода-вывода в секунду | Ограничения пакета | Исходящий трафик (Миб/с) | Входящий трафик (Миб/с) |
|---------|---------|---------|---------|---------|
|100         | 100     | 300     | 66   | 44   |
...
|100,000 *    | 100 000 | До 100 000 | 6,204 | 4,136   |

В настоящее время файл размеры общего ресурса до 5 Тиб доступны в общедоступной предварительной версии, хотя размеры до 102 Тиб доступны в ограниченной общедоступной предварительной версии, чтобы запросить доступ к ограниченной общедоступной предварительной версии полный [этот опрос.](https://aka.ms/azurefilesatscalesurvey)
...
Кредиты накапливаются в сегменте расширительных всякий раз, когда трафик к папкам значениями ниже базовых показателей операций ввода-ВЫВОДА. Например общую папку 100 Гиб имеет 100 базовых показателей операций ввода-ВЫВОДА. Если фактический трафика в общей папке 40 операций ввода-ВЫВОДА для указанным интервалом 1 секунда, 60 неиспользуемых операций ввода-ВЫВОДА кредит в контейнер пакета. Эти кредиты будут использоваться позже при операции приведет к превышению базовых показателей операций ввода-вывода.

> [!TIP]
> Размер блока ограничение расширительных = Baseline_IOPS * 2 * 3600.
...
- Осталось с нуля, когда отсутствуют либо кредиты или базовых показателей операций ввода-ВЫВОДА, используются.

Новый файл общих папок, начинающихся полное число кредитов в его контейнере пакетов.
...
* [Развертывание службы синхронизации файлов Azure](storage-sync-files-deployment-guide.md)
