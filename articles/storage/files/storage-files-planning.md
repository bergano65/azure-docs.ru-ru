...
> - Доступны только для LRS.
> - Доступно в трех регионах: Западная часть США 2, Западная Европа и ЮГО-Восточная Азия областей.
...
Если вы хотите подключить к предварительной версии этих больших размеров файлов общего ресурса, отправьте это [формы](https://aka.ms/azurefilesatscalesurvey). 

### <a name="premium-file-shares-preview"></a>Общие файлы уровня "Премиум"

Файловые ресурсы уровня "премиум" (Предварительная версия) обеспечиваются твердотельные накопители (SSD). Файловые ресурсы уровня "премиум" предоставляют согласованная высокая производительность и низкой задержкой и в пределах одной цифры в миллисекундах для большинства операций ввода-ВЫВОДА для ИНТЕНСИВНЫХ по вводу-выводу рабочих нагрузок. Это делает их пригодными для широкого круга рабочих нагрузок, таких как базы данных, размещение веб-сайтов, среды разработки и т. д. Файловые ресурсы уровня "Премиум" доступны только в подготовленной модели выставления счетов. Файловые ресурсы уровня "премиум" используется модель развертывания, отдельно от стандартных файловых ресурсов.
