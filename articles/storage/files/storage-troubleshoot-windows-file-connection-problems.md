...
## <a name="unable-to-mount-azure-files-with-ad-credentials"></a>Невозможно установить файлы Azure с учетными данными AD 

### <a name="self-diagnostics-steps"></a>Шаги самодиагностики
Во-первых, убедитесь, что вы следовали через все четыре шага, чтобы [включить Azure файлов AD аутентификации.](https://docs.microsoft.com/azure/storage/files/storage-files-identity-auth-active-directory-enable)

Во-вторых, попробуйте [усваить долю файла Azure с ключом учетной записи хранилища.](https://docs.microsoft.com/azure/storage/files/storage-how-to-use-files-windows) Если вы не смогли смонтировать, загрузите [AzFileDiagnostics.ps1,](https://gallery.technet.microsoft.com/Troubleshooting-tool-for-a9fa1fe5) чтобы помочь вам проверить клиентскую среду, обнаружить несовместимую конфигурацию клиента, которая может привести к сбою доступа для Azure Files, дает директивные рекомендации по самоисправлению и, сбор следов диагностики.

В-третьих, можно запустить смдлет Debug-AzStorageAccountAuth для проведения набора основных проверок конфигурации AD с зарегистрированным пользователем AD. Это cmdlet поддерживается на [версии AzFilesHybrid v0.1.2 .](https://github.com/Azure-Samples/azure-files-samples/releases) Вам нужно запустить этот cmdlet с пользователем AD, который имеет разрешение владельца на учетную запись целевого хранилища.  
```PowerShell
...
```
Cmdlet выполняет эти проверки ниже в последовательности и обеспечивает руководство для сбоев:
1. CheckPort445Connectivity: проверьте, что порт 445 открыт для подключения к SMB
2. CheckDomainJoined: проверить, что клиентская машина является доменом, объединенным с AD
3. CheckADObject: подтвердить, что зарегистрированный на пользователя имеет действительное представление в домене AD, что учетная запись хранилища связана с
4. CheckGetKerberosTicket: попытка получить билет Kerberos для подключения к учетной записи хранения 
5. CheckADObjectPasswordIsCorrect: убедитесь, что пароль, настроенный на идентификационный элемент AD, представляющий учетную запись хранилища, соответствует ключу бордюра учетной записи хранилища
6. CheckSidHasAadUser: проверьте, что зарегистрированный на AD пользователь синхронизирован с Azure AD

Мы активно работаем над расширением этой диагностики cmdlet, чтобы обеспечить лучшее руководство по устранению неполадок.
