...
- Создали учетную запись хранения Azure с необходимыми параметрами устойчивости шифрования в нужном регионе. Пошаговые инструкции по созданию учетной записи хранения см. в разделе о [создании учетной записи хранения](../common/storage-account-create.md?toc=%252fazure%252fstorage%252ffiles%252ftoc.json).
...
4. Создайте CSV-файл набора данных. CSV-файл набора данных представляет собой сопоставление пути к данным в локальной среде с нужной общей папкой Azure, в которую должны быть скопированы данные. Например, следующий CSV-файл набора данных сопоставляет локальный файловый ресурс ("F:\shares\scratch") с общей папкой Azure ("MyAzureFileShare"):
    ...
    С помощью учетной записи хранения можно указать несколько общих ресурсов. Дополнительные сведения см. в разделе [Подготовка CSV-файла набора данных](/previous-versions/azure/storage/common/storage-import-export-tool-preparing-hard-drives-import?toc=%252fazure%252fstorage%252ffiles%252ftoc.json).
...
5. Создайте CSV-файл набора дисков. CSV-файл набора дисков содержит перечень дисков, доступных агенту локального экспорта. Например, следующий CSV-файл набора дисков перечисляет диски `X:`, `Y:` и `Z:`, которые будут использоваться в задании локального экспорта:
    ...
    Дополнительные сведения см. в статье [Подготовка CSV-файла набора дисков](/previous-versions/azure/storage/common/storage-import-export-tool-preparing-hard-drives-import?toc=%252fazure%252fstorage%252ffiles%252ftoc.json).
...
2. Используйте `robocopy` в командной строке, чтобы перенести данные в общую папку Azure.
    ...
    Robocopy располагает значительным количеством параметров для изменения поведения копирования. Дополнительные сведения см. на странице руководства по [Robocopy](/windows-server/administration/windows-commands/robocopy).
...
1. Скачайте [последнюю версию AzCopy для Windows](https://aka.ms/downloadazcopy) или [Linux](../common/storage-use-azcopy-v10.md?toc=/azure/storage/files/toc.json#download-azcopy).
2. Используйте `azcopy` в командной строке, чтобы перенести данные в общую папку Azure. В Windows используется следующий синтаксис: 
    ...
    AzCopy располагает значительным количеством параметров для изменения поведения копирования. Дополнительные сведения см. в статье Начало [работы с AzCopy](../common/storage-use-azcopy-v10.md?toc=%252fazure%252fstorage%252ffiles%252ftoc.json).
