...
| Версия Windows        | Версия SMB | Возможность подключения на виртуальной машине Azure | Локально подключаемый сервер |
|------------------------|-------------|-----------------------|-----------------------|
...
<sup>1</sup> Windows 10, версии 1507, 1607, 1709, 1803, 1809, 1903 и 1909.  
<sup>2</sup> Windows Server, версии 1809, 1903 и 1909.  
<sup>3</sup> Обычная поддержка Майкрософт для Windows 7 и Windows Server 2008 R2 закончилась. Дополнительную поддержку обновлений безопасности можно приобрести только через [программу расширенного обновления безопасности (ЕСУ)](https://support.microsoft.com/help/4497181/lifecycle-faq-extended-security-updates). Мы настоятельно рекомендуем переходить с этих операционных систем.

> [!Note]  
> Мы всегда рекомендуем использовать последнюю версию KB для своей версии Windows.

## <a name="prerequisites"></a>Предварительные условия 
* **Имя учетной записи хранения**. для подключения файлового ресурса Azure потребуется имя учетной записи хранения.

* **Ключ учетной записи хранения**. для подключения файлового ресурса Azure потребуется основной (или вторичный) ключ хранилища. В настоящее время ключи SAS для подключения не поддерживаются.

* **Убедитесь, что порт 445 открыт**: для протокола SMB требуется, чтобы TCP-порт 445 был открыт, в противном случае подключение будет невозможно. Вы можете проверить, блокирует ли ваш брандмауэр порт 445, с помощью командлета `Test-NetConnection`. Вы можете узнать о [различных способах решения заблокированного порта 445 здесь](https://docs.microsoft.com/azure/storage/files/storage-troubleshoot-windows-file-connection-problems#cause-1-port-445-is-blocked).
...
В отличие от других общих папок SMB, с которыми вы могли взаимодействовать, например папок, размещенных на серверах Windows Server, Linux Samba или на устройстве NAS, в настоящее время аутентификация Kerberos с помощью идентификатора Active Directory (AD) или Azure Active Directory (AAD) для общих папок Azure не поддерживается, хотя мы [работаем](https://feedback.azure.com/forums/217298-storage/suggestions/6078420-acl-s-for-azurefiles) над этой функцией. Вместо этого для доступа к вашей общей папке Azure нужно использовать ключ учетной записи хранения, содержащей эту папку. Ключ учетной записи хранения — это ключ администратора для учетной записи хранения, включая разрешения администратора для всех файлов и папок в общей папке, к которым осуществляется доступ, а также для всех файловых ресурсов и других ресурсов хранилища (BLOB-объектов, очередей, таблиц и т. д.), которые содержатся в вашей учетной записи хранения. Если этого недостаточно для вашей рабочей нагрузки, служба [Синхронизация файлов Azure](storage-sync-files-planning.md) может помочь урегулировать отсутствие проверки подлинности Kerberos и поддержки ACL до тех пор, пока не будет доступна общедоступная поддержка ACL и аутентификация Kerberos на основе AAD.
...
Хранение учетных данных другого пользователя на компьютере — это просто: при входе в учетную запись просто выполните следующую команду PowerShell:
...
Выполните следующие команды в обычном сеансе PowerShell (не с повышенными привилегиями), чтобы подключить файловый ресурс Azure. Не забудьте заменить `<your-resource-group-name>`, `<your-storage-account-name>`, `<your-file-share-name>` и `<desired-drive-letter>` правильными данными.
...
1. Перейдите к элементу **этот компьютер** в левой части окна. Меню, доступные на ленте, изменятся. В меню "Компьютер" выберите раздел **Подключение сетевого диска**.
...
1. Выберите букву диска и введите UNC- `<storageAccountName>.file.core.windows.net/<fileShareName>`путь в формате UNC. Например: `anexampleaccountname.file.core.windows.net/example-share-name`.
...
Если вы уже создали моментальные снимки общего файлового ресурса вручную или автоматически с помощью скрипта или такой службы, как Azure Backup, вы можете просмотреть предыдущие версии общего ресурса, каталога или определенного файла из подключенного общего файлового ресурса в Windows. Вы можете создать моментальный снимок общего ресурса из [портал Azure](storage-how-to-use-files-portal.md), [Azure PowerShell](storage-how-to-use-files-powershell.md)и [Azure CLI](storage-how-to-use-files-cli.md).
...
|-------------------------------------------|----------------------|-----------------------------|
| Windows Server 2019                       | Отключен             | Удаление с помощью функции Windows |
| Windows Server версии 1709 или выше            | Отключен             | Удаление с помощью функции Windows |
| Windows 10 версии 1709 или выше                | Отключен             | Удаление с помощью функции Windows |
