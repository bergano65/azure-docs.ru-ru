...
<sup>3</sup> Регулярная поддержка Microsoft для Windows 7 и Windows Server 2008 R2 закончилась. Дополнительную поддержку обновлений безопасности можно приобрести только через [программу расширенного обновления безопасности (ESU).](https://support.microsoft.com/help/4497181/lifecycle-faq-extended-security-updates) Мы настоятельно рекомендуем мигрировать от этих операционных систем.
...
* **Убедитесь, что порт 445 открыт**: для протокола SMB требуется, чтобы TCP-порт 445 был открыт, в противном случае подключение будет невозможно. Вы можете проверить, блокирует ли ваш брандмауэр порт 445, с помощью командлета `Test-NetConnection`. Вы можете узнать о [различных способах обхода заблокирован ный порт 445 здесь](https://docs.microsoft.com/azure/storage/files/storage-troubleshoot-windows-file-connection-problems#cause-1-port-445-is-blocked).

    Следующий код PowerShell предполагает, что у вас установлен модуль Azure PowerShell, см. [Для получения](https://docs.microsoft.com/powershell/azure/install-az-ps) дополнительной информации см. Не забудьте заменить `<your-storage-account-name>` и `<your-resource-group-name>` соответствующими именами для вашей учетной записи хранения.
...
2. Перейдите к **данному** элементу ПК на левой стороне окна. Меню, доступные на ленте, изменятся. В меню "Компьютер" выберите раздел **Подключение сетевого диска**.
...
3. Скопируйте путь КООН из панели **Connect** на портале Azure. 
...
Если вы уже создали моментальные снимки общего файлового ресурса вручную или автоматически с помощью скрипта или такой службы, как Azure Backup, вы можете просмотреть предыдущие версии общего ресурса, каталога или определенного файла из подключенного общего файлового ресурса в Windows. Вы можете сделать снимок общего обмена с [портала Azure,](storage-how-to-use-files-portal.md) [Azure PowerShell](storage-how-to-use-files-powershell.md)и [Azure CLI.](storage-how-to-use-files-cli.md)
...
> Применяется к Windows Server 2019, Windows Server полугодовой канал (версии 1709 и 1803), Windows Server 2016, Windows Server 2012 R2
