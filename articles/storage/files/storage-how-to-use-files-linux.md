...
### <a name="minimum-recommended-versions-with-corresponding-mount-capabilities-smb-version-21-vs-smb-version-30"></a>Минимальный рекомендуемый версии с соответствующие возможности подключения (версия SMB версии SMB 2.1 и 3.0)
...
* **Выберите разрешения для каталога и файлов подключаемого общедоступного ресурса**. В приведенных ниже примерах используется разрешение `0777`, чтобы все пользователи имели разрешения на чтение, запись и выполнение файлов. Вы можете заменить его другим [разрешения chmod](https://en.wikipedia.org/wiki/Chmod) требуемым образом, хотя это будет означать потенциально ограничения доступа. Если вы используете другие разрешения, можно также использовать uid и gid, чтобы сохранить доступ для локальных пользователей и групп по своему усмотрению.

> [!NOTE]
> Если разрешение каталогов и файлов с dir_mode и file_mode не заданы явно, по умолчанию 0755.
