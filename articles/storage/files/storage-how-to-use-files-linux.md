...
* <a id="install-cifs-utils"></a>**Убедитесь, что пакет CIFS-utils установлен.**  
    ...
    В **Fedora**, **Red Hat Enterprise Linux 8 +** и **CentOS 8 +**, используйте диспетчер `dnf` пакетов:

    ```bash
    sudo dnf install cifs-utils
    ```

    В более старых версиях **Red Hat Enterprise Linux** и **CentOS**используйте диспетчер `yum` пакетов:
...
Проверить, поддерживает ли дистрибутив Linux параметр Module, `disable_legacy_dialects` можно с помощью следующей команды.
...
Чтобы выгрузить модуль, сначала отключите все общие ресурсы SMB (с `umount` помощью команды, описанной выше). Вы можете выделить все подключенные общие папки SMB в системе с помощью следующей команды:
