...
Скорость производительности составляет 7 объектов в секунду. Клиенты могут оценить время, необходимое для завершения первоначального перечисления изменений в облаке, определив количество элементов в облачной общей папке и используя следующую формулу для получения времени в днях. 

   **Время (в днях) для начального перечисления облака = (число объектов в облачной конечной точке)/(7 * 60 * 60 * 24)**
