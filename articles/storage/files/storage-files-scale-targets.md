...
> [!Important]  
> Использование учетной записи хранения общего назначения из других служб хранения влияет на ваши доли файлов Azure в вашей учетной записи хранения. Например, при достижении максимальной емкости учетной записи хранения в службе хранилища BLOB-объектов Azure вы не сможете создать файлы в общей папке Azure, даже если ее размер не превышает максимального.

## <a name="azure-files-scale-targets"></a>Целевые показатели масштабируемости службы файлов Azure

Для файлов Azure необходимо учитывать три категории ограничений: учетные записи, акции и файлы.

Например: с премиум-файлакций, одна доля может достичь 100000 IOPS и один файл может масштабировать до 5000 IOPS. Таким образом, если у вас есть три файла в одной акции, максимальный IOPS вы можете получить от этой доли составляет 15000.

### <a name="standard-storage-account-limits"></a>Стандартные лимиты на хранение

Ознакомьтесь с разделом [целевых показателей шкалы хранения данных Azure](#azure-storage-account-scale-targets) для этих ограничений.

### <a name="premium-filestorage-account-limits"></a>Ограничения учетной записи Премиум FileStorage

[!INCLUDE [azure-storage-limits-filestorage](../../../includes/azure-storage-limits-filestorage.md)]

> [!IMPORTANT]
> Ограничения на хранение счетов распространяются на все акции. Масштабирование до максимального для учетных записей FileStorage достижимо только в том случае, если на учетную запись FileStorage имеется только одна доля.
...
| Размер набора данных| 4,7 ТБО |
| Средний размер файлов | 200 киБ (самый большой файл: 100 GiB) |
| Пропускная способность отправки | 20 объектов в секунду в группе синхронизации |
