...
При включении распределения по уровням в облаке на конечной точке сервера вы устанавливаете политику с учетом свободного пространства. Она всегда приоритетнее других политик, включая политику на основе даты. В качестве опционов можно включить политику даты для каждой точки конечных точек сервера в этом объеме. Эта политика управляет тем, что только файлы, к которым обращаются (т.е. прочитанные или написанные) в пределах дней, описываемых этой политикой, будут храниться на местах. Файлы, не доступные с указанным количеством дней, будут многоуровневыми. 

Cloud Tiering использует последнее время доступа, чтобы определить, какие файлы должны быть многоуровневыми. Драйвер фильтра облачного уровня (storagesync.sys) отслеживает последнее время доступа и регистрирует информацию в магазине тепла, многоуровневом облаке. Вы можете увидеть тепловой магазин с помощью местного Смдлета PowerShell.
...
> [!IMPORTANT]
> Последний доступ к метке времени не является свойством, отслеживаемым NTFS, и поэтому не виден по умолчанию в File Explorer. Не используйте последнюю модифицированную отметку времени в файле, чтобы проверить, работает ли политика даты как ожидалось. Этот штамп только треки пишет, а не читает. Используйте cmdlet показано, чтобы получить последний доступ времени штамп для этой оценки.

> [!WARNING]
> Не включайте функцию NTFS отслеживания последнего доступа к тайне для файлов и папок. Эта функция отключена по умолчанию, поскольку она имеет большое влияние на производительность. Azure File Sync будет отслеживать время последнего доступа автоматически и очень эффективно и не использует эту функцию NTFS.

Имейте в виду, что политика свободного пространства всегда имеет приоритет, и когда нет достаточносвободного пространства на том, чтобы сохранить столько дней стоит файлов, как описано в политике даты, Azure File Sync будет продолжать многоуровневый холодный файлов до тех пор, пока объем свободного пространства процент будет выполнена.
