...
Для агентов версии 9. x и более новых версий минимальный размер файла на уровень зависит от размера кластера файловой системы (в два раза размер кластера файловой системы). Например, если размер кластера файловой системы NTFS равен 4 КБ, то результирующий минимальный размер файла для уровня равен 8 КБ. Для агентов версии 8. x и более ранних версий минимальный размер файла для уровня равен 64 КБ.
...
Существует несколько способов определения перемещения файла в файловый ресурс Azure.
   ...
   *  **Проверьте атрибуты файла.**
     Щелкните файл правой кнопкой мыши, перейдите в раздел **Сведения** и прокрутите вниз до свойства **Атрибуты**. Перемещенному файлу будут заданы следующие атрибуты:     
        ...
        | O | Offline | Указывает, что некоторое (или все) содержимое файла не хранится на диске. После полного отзыва файла служба синхронизации файлов Azure удаляет этот атрибут. |
...
* `-Order CloudTieringPolicy`в первую очередь будет отозван недавно измененные файлы.  
