...
1. [Форум по службе хранилища Azure](https://social.msdn.microsoft.com/forums/azure/home?forum=windowsazuredata).
...
<a id="agent-installation-failures"></a>**Устранение ошибок установки агента**  
...
<a id="server-registration-missing-subscriptions"></a>**При регистрации сервера не перечисляются все подписки Azure**  
При регистрации сервера с помощью ServerRegistration. exe подписки отсутствуют, если щелкнуть раскрывающийся список подписка Azure.

Эта проблема возникает из-за того, что ServerRegistration. exe в настоящее время не поддерживает среды с несколькими клиентами. Эта проблема будет исправлена в будущем Синхронизация файлов Azure обновления агента.

Чтобы решить эту проблему, используйте следующие команды PowerShell для регистрации сервера:
...
<a id="server-already-registered"></a>**При регистрации сервера отображается следующее сообщение: "Этот сервер уже зарегистрирован".** 
...
<a id="web-site-not-trusted"></a>**При регистрации сервера отображаются многочисленные ответы "веб-сайт не является доверенным". Важно?**  
...
<a id="server-registration-missing"></a>**Сервер не указан в списке "зарегистрированные серверы" в портал Azure**  
...
2. Щелкните правой кнопкой мыши файловый ресурс Azure и выберите пункт **изменить метаданные**.
3. Щелкните правой кнопкой мыши **SyncService**и выберите пункт **Удалить**.
...
5. В списке **поставщик ресурсов** выберите **авторизация Майкрософт**. 
    * **Назначение роли** должно иметь разрешения на **Чтение** и **запись** .
    * **Определение роли** должно иметь разрешения на **Чтение** и **запись** .

<a id="-2134375898"></a>**Сбой создания конечной точки сервера, ошибка: "Мгмтсервержобфаилед" (код ошибки:-2134375898 или 0x80c80226)**  
...
<a id="-2147024894"></a>**Сбой создания конечной точки сервера, ошибка: "Мгмтсервержобфаилед" (код ошибки:-2147024894 или 0x80070002)**  
Эта ошибка возникает, если указан недопустимый путь к конечной точке сервера. Убедитесь, что указанный путь к конечной точке сервера ведет к локально подключенному тому NTFS. Обратите внимание, Синхронизация файлов Azure не поддерживает подключенные диски в качестве пути к конечной точке сервера.

<a id="-2134375640"></a>**Сбой создания конечной точки сервера, ошибка: "Мгмтсервержобфаилед" (код ошибки:-2134375640 или 0x80c80328)**  
Эта ошибка возникает, если указанный путь к конечной точке сервера не является томом NTFS. Убедитесь, что указанный путь к конечной точке сервера ведет к локально подключенному тому NTFS. Обратите внимание, Синхронизация файлов Azure не поддерживает подключенные диски в качестве пути к конечной точке сервера.

<a id="-2134347507"></a>**Сбой создания конечной точки сервера, ошибка: "Мгмтсервержобфаилед" (код ошибки:-2134347507 или 0x80c8710d)**  
...
<a id="-2134376345"></a>**Сбой создания конечной точки сервера, ошибка: "Мгмтсервержобфаилед" (код ошибки:-2134376345 или 0x80C80067)**  
Эта ошибка возникает, если достигнуто максимальное число конечных точек на сервер. Сейчас служба "Синхронизация файлов Azure" поддерживает до 30 конечных точек на сервер. Дополнительные сведения см. в разделе [Синхронизация файлов Azure Scale targets](https://docs.microsoft.com/azure/storage/files/storage-files-scale-targets#azure-file-sync-scale-targets).

<a id="-2134376427"></a>**Сбой создания конечной точки сервера, ошибка: "Мгмтсервержобфаилед" (код ошибки:-2134376427 или 0x80c80015)**  
...
<a id="-2160590967"></a>**Сбой создания конечной точки сервера, ошибка: "Мгмтсервержобфаилед" (код ошибки:-2160590967 или 0x80c80077)**  
Эта ошибка возникает, если путь к конечной точке сервера содержит потерянные многоуровневые файлы. Если конечная точка сервера была недавно удалена, дождитесь завершения очистки потерянных многоуровневых файлов. Событие с ИДЕНТИФИКАТОРом 6662 регистрируется в журнале событий телеметрии после запуска очистки потерянных многоуровневых файлов. Событие с ИДЕНТИФИКАТОРом 6661 регистрируется после завершения очистки потерянных многоуровневых файлов, и можно создать новую конечную точку сервера с помощью пути. Если создание конечной точки сервера завершается сбоем после записи события с ИДЕНТИФИКАТОРом 6661, удалите потерянные многоуровневые файлы, выполнив действия, описанные в [многоуровневых файлах, после удаления раздела серверной конечной точки](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#tiered-files-are-not-accessible-on-the-server-after-deleting-a-server-endpoint) .

<a id="-2134347757"></a>**Удаление конечной точки сервера завершается сбоем со следующей ошибкой: "Мгмтсервержобекспиред" (код ошибки:-2134347757 или 0x80c87013)**  
...
```
<a id="server-endpoint-noactivity"></a>**Конечная точка сервера имеет состояние работоспособности "нет активности" или "ожидание", а состояние сервера в колонке "зарегистрированные серверы" отображается в автономном режиме.**  

Эта проблема может возникать, если процесс монитора синхронизации хранилища (Азуресторажесинкмонитор. exe) не выполняется или серверу не удается получить доступ к службе Синхронизация файлов Azure.

На сервере, который отображается как "отображается в автономном режиме" на портале, просмотрите событие с ИДЕНТИФИКАТОРом 9301 в журнале событий телеметрии (в разделе приложения и Сервицес\микрософт\филесинк\ажент в Просмотр событий), чтобы определить, почему серверу не удается получить доступ к службе Синхронизация файлов Azure. 

- Если **жетнекстжоб завершено с состоянием: 0** — зарегистрировано, сервер может взаимодействовать со службой Синхронизация файлов Azure. 
...
- Если **жетнекстжоб завершено с состоянием:-2134347756** , то сервер не может связаться со службой Синхронизация файлов Azure из-за брандмауэра или прокси-сервера. 
    - Если сервер находится за брандмауэром, убедитесь, что порт 443 для исходящих подключений разрешен. Если брандмауэр блокирует трафик к определенным доменам, убедитесь, что домены, перечисленные в [документации](https://docs.microsoft.com/azure/storage/files/storage-sync-files-firewall-and-proxy#firewall) по брандмауэру, доступны.
    - Если сервер находится за прокси-сервером, настройте параметры прокси-сервера на уровне компьютера или приложения, выполнив действия, описанные в [документации по](https://docs.microsoft.com/azure/storage/files/storage-sync-files-firewall-and-proxy#proxy)прокси-серверу.
...
- Если **жетнекстжоб завершено с состоянием:-2134347764** в журнале, сервер не может связаться со службой Синхронизация файлов Azure из-за просроченного или удаленного сертификата.  
    ...
    ```
<a id="endpoint-noactivity-sync"></a>**Конечная точка сервера имеет состояние работоспособности "нет активности", а состояние сервера в колонке "зарегистрированные серверы" — "в сети"**  
...
Конечная точка сервера не может записывать в журнал действия синхронизации в течение нескольких часов из-за ошибки или недостатка системных ресурсов. Проверьте, установлена ли последняя [версия агента](https://docs.microsoft.com/azure/storage/files/storage-files-release-notes) синхронизация файлов Azure. Если проблема будет повторяться, отправьте запрос в службу поддержки.

> [!Note]  
> Если состояние сервера в колонке "зарегистрированные серверы" отображается в автономном режиме, выполните действия, описанные в [конечной точке сервера, с состоянием работоспособности "нет активности" или "ожидание", а состояние сервера в колонке "зарегистрированные серверы" отображается в автономном режиме](#server-endpoint-noactivity) .
...
2. Когда сервер полностью синхронизирован с облаком и отсутствуют какие-либо изменения в синхронизации в любом направлении, вы увидите пустые сеансы синхронизации. Они обозначаются событиями отправки и загрузки, в которых все поля Sync * (Синкфилекаунт, Синкдиркаунт, Синктомбстонекаунт и Синксизебитес) равны нулю, что означает, что синхронизация не выполнялась. Обратите внимание, что эти пустые сеансы синхронизации могут не происходить на серверах с высоким уровнем обработки, так как всегда существует что-то новое для синхронизации. Если действия синхронизации отсутствуют, они должны выполняться каждые 30 минут. 
...
Если Перитемерроркаунт на сервере или файлах, не синхронизированных на портале, больше 0 для любого сеанса синхронизации, это означает, что некоторые элементы не синхронизируются. Файлы и папки могут иметь характеристики, препятствующие синхронизации. Эти характеристики могут быть постоянными и требуют явного действия для возобновления синхронизации, например удаления неподдерживаемых знаков из имени файла или папки. Также эти характеристики могут быть временными, то есть синхронизация файла или папки возобновляется автоматически. Например, файлы с открытыми дескрипторами автоматически возобновят синхронизацию при закрытии файла. Когда механизм службы "Синхронизация файлов Azure" обнаруживает такую проблему, создается журнал ошибок, который можно проанализировать, чтобы перечислить элементы, которые в настоящее время не синхронизируются должным образом.
...
> [!Note]  
> Если сценарий Филесинцеррорсрепорт. ps1 возвращает сообщение "не найдены ошибки в файлах или не указаны ошибки для каждого элемента для группы синхронизации, причина может быть такой:
>
>- Причина 1. в последнем завершенном сеансе синхронизации отсутствуют ошибки для каждого элемента. На портале вскоре будет отображаться 0 файлов, которые не синхронизируются. 
>    - Проверьте [событие с идентификатором 9102](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=server%2Cazure-portal#broken-sync) в журнале событий телеметрии, чтобы убедиться, что перитемерроркаунт имеет значение 0. 
>
>- Причина 2. журнал событий Итемресултс на сервере, упакованный из-за слишком большого количества ошибок для каждого элемента, и журнал событий больше не содержит ошибок для этой группы синхронизации.
>    - Чтобы избежать этой проблемы, увеличьте размер журнала событий Итемресултс. Журнал событий Итемресултс можно найти в разделе "приложения и службы Логс\микрософт\филесинк\ажент" в Просмотр событий. 
...
|---------|-------------------|--------------|-------|-------------|
| 0x80070043 | — 2147942467 | ERROR_BAD_NET_NAME | Многоуровневый файл на сервере недоступен. Такая ошибка возникает, если многоуровневый файл не был отозван перед удалением конечной точки сервера. | Сведения об устранении этой проблемы см. в разделе [многоуровневые файлы недоступны на сервере после удаления конечной точки сервера](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#tiered-files-are-not-accessible-on-the-server-after-deleting-a-server-endpoint). |
| 0x80c80207 | -2134375929 | ECS_E_SYNC_CONSTRAINT_CONFLICT | Изменение файла или каталога пока невозможно синхронизировать, так как зависимая папка еще не синхронизирована. Этот элемент будет синхронизирован после синхронизации зависимых изменений. | Действия не требуются. Если ошибка повторяется в течение нескольких дней, используйте сценарий PowerShell Филесинцеррорсрепорт. ps1, чтобы определить причину, по которой зависимая папка еще не синхронизирована. |
| 0x80c80284 | — 2134375804 | ECS_E_SYNC_CONSTRAINT_CONFLICT_SESSION_FAILED | Изменение файла или каталога пока невозможно синхронизировать, так как зависимая папка еще не синхронизирована, и не удалось выполнить сеанс синхронизации. Этот элемент будет синхронизирован после синхронизации зависимых изменений. | Действия не требуются. Если ошибка повторяется, проверьте сбой сеанса синхронизации. |
...
| 0x80c8031d | -2134375651 | ECS_E_CONCURRENCY_CHECK_FAILED | Файл был изменен, но это изменение еще не обнаружено синхронизацией. После обнаружения этого изменения синхронизация будет восстановлена. | Действия не требуются. |
...
| 0x80070003 | — 2147942403 | ERROR_PATH_NOT_FOUND | Невозможно синхронизировать удаление файла или каталога, так как элемент уже удален в месте назначения, и синхронизация не учитывает изменения. | Действия не требуются. Синхронизация прекращает регистрировать эту ошибку после того, как на целевом компьютере выполняется обнаружение изменений, и синхронизация обнаруживает, что элемент был удален. |
| 0x80c80205 | — 2134375931 | ECS_E_SYNC_ITEM_SKIP | Файл или каталог пропущен, но будут синхронизированы во время следующего сеанса синхронизации. Если при скачивании элемента появляется сообщение об ошибке, имя файла или каталога больше вероятно недопустимо. | Никаких действий не требуется, если при отправке файла выдается сообщение об ошибке. Если при скачивании файла появляется сообщение об ошибке, переименуйте файл или каталог. Дополнительные сведения см. в разделе [Обработка неподдерживаемых знаков](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#handling-unsupported-characters). |
| 0x800700B7 | — 2147024713 | ERROR_ALREADY_EXISTS | Невозможно синхронизировать создание файла или каталога, так как этот элемент уже существует в назначении, и синхронизация не учитывает изменения. | Действия не требуются. Синхронизация прекращает запись в журнал этой ошибки после того, как обнаружение изменений выполняется в месте назначения, и при синхронизации учитывается этот новый элемент. |
...
| 0x80070017 | — 2147024873 | ERROR_CRC | Не удается синхронизировать файл из-за ошибки CRC. Эта ошибка может возникать, если многоуровневый файл не был отозван до удаления конечной точки сервера или если файл поврежден. | Сведения об устранении этой проблемы см. в разделе [многоуровневые файлы недоступны на сервере после удаления конечной точки сервера](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#tiered-files-are-not-accessible-on-the-server-after-deleting-a-server-endpoint) для удаления потерянных многоуровневых файлов. Если ошибка повторяется после удаления потерянных многоуровневых файлов, запустите [программу chkdsk](https://docs.microsoft.com/windows-server/administration/windows-commands/chkdsk) на томе. |
...
Сеансы синхронизации могут завершаться сбоем по различным причинам, включая перезагрузку или обновление сервера, моментальные снимки VSS и т. д. Несмотря на то, что эта ошибка выглядит так, что требует дальнейших действий, можно спокойно проигнорировать эту ошибку, если она не будет сохранена в течение нескольких часов.

<a id="-2147012889"></a>**Не удалось установить подключение к службе.**    
...
2. [Убедитесь, что файловый ресурс Azure существует.](#troubleshoot-azure-file-share)
...
<a id="-2134364014"></a>**Не удалось выполнить синхронизацию, так как учетная запись хранения заблокирована.**  
...
Эта ошибка возникает, если учетная запись хранения имеет [блокировку ресурса](https://docs.microsoft.com/azure/azure-resource-manager/management/lock-resources)только для чтения. Чтобы устранить эту проблему, удалите блокировку ресурсов только для чтения в учетной записи хранения. 
...
Эта ошибка возникает при возникновении проблемы с внутренней базой данных, используемой Синхронизация файлов Azure. При возникновении этой проблемы создайте запрос в службу поддержки, и мы свяжемся с вами, чтобы помочь вам устранить эту проблему.
...
5. Выберите **файлы** , чтобы просмотреть список файловых ресурсов.
6. Щелкните многоточие в конце строки для общего файлового ресурса Azure, на который ссылается облачная конечная точка.
7. Проверьте, чтобы значение в поле **Использование** было ниже значения в поле **Квота**. Примечание. Если не указана альтернативная квота, квота будет соответствовать [максимальному размеру файлового ресурса Azure](storage-files-scale-targets.md).
...
<a id="-2146762487"></a>**Серверу не удалось установить безопасное соединение. Облачная служба получила непредвиденный сертификат.**  
...
Эта ошибка может возникать, если в организации используется прокси-сервер, завершающийся TLS, или если вредоносная сущность перехватывает трафик между сервером и службой Синхронизация файлов Azure. Если вы уверены, что это ожидается (так как ваша организация использует прокси-сервер, заканчивающийся TLS), проверку сертификата можно пропустить с помощью переопределения реестра.
...
Установив это значение реестра, агент Синхронизация файлов Azure будет принимать локально доверенный TLS/SSL-сертификат при передаче данных между сервером и облачной службой.

<a id="-2147012894"></a>**Не удалось установить подключение к службе.**  
...
Эта ошибка возникает из-за того, что облачная конечная точка была создана с содержимым, уже существующим в общем файловом ресурсе Azure. Синхронизация файлов Azure необходимо проверить файловый ресурс Azure для всего содержимого, прежде чем разрешить конечной точке сервера продолжать свою первоначальную синхронизацию.
...
<a id="-2134376373"></a>**Служба сейчас недоступна.**  
...
Эта ошибка возникает, если произошел сбой синхронизации из-за исключения. Если ошибка повторяется в течение нескольких часов, создайте запрос в службу поддержки.
...
Убедитесь, что у вас есть последняя версия агента Синхронизация файлов Azure. Начиная с агента V10, Синхронизация файлов Azure поддерживает перемещение подписки в другой клиент Azure Active Directory.
 
После получения последней версии агента необходимо предоставить приложению Microsoft. StorageSync доступ к учетной записи хранения (см. раздел [обеспечение доступа к учетной записи хранения у синхронизация файлов Azure](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot#troubleshoot-rbac)).
...
1. Загрузите средство [PsExec](https://docs.microsoft.com/sysinternals/downloads/psexec) .
2. Выполните следующую команду из командной строки с повышенными привилегиями, чтобы запустить командную строку с помощью системной учетной записи: **PsExec. exe-i-s-d cmd** 
...
<a id="-2145844941"></a>**Не удалось выполнить синхронизацию, так как HTTP-запрос был перенаправлен**  
...
<a id="-2134364027"></a>**Во время автономной пересылки данных истекло время ожидания, но оно по-прежнему выполняется.**  
...
Эта ошибка возникает, когда операция приема данных превышает время ожидания. Эту ошибку можно пропустить, если выполняется синхронизация (Апплиедитемкаунт больше 0). См. [разделы справки мониторинг хода выполнения текущего сеанса синхронизации](#how-do-i-monitor-the-progress-of-a-current-sync-session).

### <a name="common-troubleshooting-steps"></a>Распространенные действия по устранению неполадок
<a id="troubleshoot-storage-account"></a>**Убедитесь, что учетная запись хранения существует.**  
...
<a id="troubleshoot-azure-file-share"></a>**Убедитесь, что файловый ресурс Azure существует.**  
# <a name="portal"></a>[Портал](#tab/azure-portal)
1. Щелкните **Обзор** в оглавлении слева, чтобы вернуться на страницу Главная учетная запись хранения.
2. Выберите **файлы** , чтобы просмотреть список файловых ресурсов.
...
1. Убедитесь, что служба **Microsoft. StorageSync** или **Гибридная синхронизация файлов Service** (старое имя приложения) присутствует в списке с ролью **читателя и доступа к данным** . 
    ...
    Если **Microsoft. StorageSync** или **гибридная служба Синхронизация файлов** не отображается в списке, выполните следующие действия.

    - Нажмите кнопку **Добавить**.
    - В поле **Роль** выберите **Модуль чтения и доступ к данным**.
    - В поле **Выбор** введите **Microsoft. StorageSync**, выберите роль и нажмите кнопку **сохранить**.
...
> [!Important]  
> Обратите внимание, что экраны файлов следует использовать только для блокировки создания символов, не поддерживаемых Синхронизация файлов Azure. Если в других сценариях используются экраны файлов, синхронизация будет непрерывно пытаться скачать файлы из общего файлового ресурса Azure на сервер и будет заблокирована из-за экранного файла, что приведет к повышению исходящего трафика данных. 
...
- Файлы могут не быть отозваны. Это означает, что Синхронизация файлов Azure фильтр файловой системы (StorageSync. sys) не может загрузить данные, когда пользователь пытается получить доступ к файлу, который был многоуровневый.
...
- Сбои облачного хранилища
    ...
    - *Недоступность учетной записи хранения*. Эта ошибка обычно возникает при удалении учетной записи хранения, если у нее еще есть общая папка Azure, которая является облачной конечной точкой в группе синхронизации. 
- Ошибки сервера 
...
- Идентификатор события 9003 обеспечивает распределение ошибок для конечной точки сервера. Например, общее количество ошибок, ErrorCode и т. д. Обратите внимание, что в журнал заносится одно событие для каждого кода ошибки.
...
- Идентификатор события 9006 обеспечивает распределение ошибок отзыва для конечной точки сервера. Например, общее количество неудачных запросов, код ошибки и т. д. Обратите внимание, что в журнал заносится одно событие для каждого кода ошибки.
...
> [!NOTE]
> Идентификатор события 9003 регистрируется один раз в час в журнале событий телеметрии при сбое распределения файла по уровням (на один код ошибки регистрируется одно событие). Ознакомьтесь с разделом [об ошибках уровней и исправлениях](#tiering-errors-and-remediation) , чтобы узнать, указаны ли действия по исправлению для кода ошибки.

### <a name="tiering-errors-and-remediation"></a>Распределение ошибок и исправлений по уровням

| HRESULT | HRESULT (десятичное число) | Строка ошибки | Проблема | Исправление |
|---------|-------------------|--------------|-------|-------------|
| 0x80c86043 | — 2134351805 | ECS_E_GHOSTING_FILE_IN_USE | Не удалось прослойировать файл, так как он используется. | Действия не требуются. Файл будет многоуровневый, если он больше не используется. |
| 0x80c80241 | — 2134375871 | ECS_E_GHOSTING_EXCLUDED_BY_SYNC | Не удалось прослойировать файл, так как он исключен синхронизацией. | Действия не требуются. Файлы в списке исключений синхронизации не могут быть многоуровневые. |
| 0x80c86042 | — 2134351806 | ECS_E_GHOSTING_FILE_NOT_FOUND | Не удалось прослойировать файл, так как он не был найден на сервере. | Действия не требуются. Если ошибка повторяется, проверьте, существует ли файл на сервере. |
| 0x80c83053 | — 2134364077 | ECS_E_CREATE_SV_FILE_DELETED | Не удалось прослойировать файл, так как он был удален в общем файловом ресурсе Azure. | Действия не требуются. Файл должен быть удален на сервере при следующем запуске сеанса синхронизации загрузки. |
| 0x80c8600e | — 2134351858 | ECS_E_AZURE_SERVER_BUSY | Не удалось прослойировать файл из-за проблемы с сетью. | Действия не требуются. Если ошибка повторяется, проверьте сетевое подключение к общему файловому ресурсу Azure. |
| 0x80072ee7 | -2147012889 | WININET_E_NAME_NOT_RESOLVED | Не удалось прослойировать файл из-за проблемы с сетью. | Действия не требуются. Если ошибка повторяется, проверьте сетевое подключение к общему файловому ресурсу Azure. |
| 0x80070005 | -2147024891 | ERROR_ACCESS_DENIED | Не удалось прослойировать файл из-за ошибки отказа в доступе. Эта ошибка может возникать, если файл находится в папке репликации DFS-R только для чтения. | Служба синхронизации файлов Azure не поддерживает конечные точки сервера на уровне папок репликации DFS-R только для чтения. Дополнительные сведения см. в разделе [руководств по планированию](https://docs.microsoft.com/azure/storage/files/storage-sync-files-planning#distributed-file-system-dfs) . |
| 0x80072efe | — 2147012866 | WININET_E_CONNECTION_ABORTED | Не удалось прослойировать файл из-за проблемы с сетью. | Действия не требуются. Если ошибка повторяется, проверьте сетевое подключение к общему файловому ресурсу Azure. |
| 0x80c80261 | — 2134375839 | ECS_E_GHOSTING_MIN_FILE_SIZE | Не удалось прослойировать файл, так как размер файла меньше поддерживаемого размера. | Если версия агента меньше 9,0, минимальный поддерживаемый размер файла — 64 КБ. Если версия агента — 9,0 и более поздняя, минимальный поддерживаемый размер файла зависит от размера кластера файловой системы (размер кластера с двойным размером файловой системы). Например, если размер кластера файловой системы равен 4 КБ, минимальный размер файла — 8 КБ. |
| 0x80c83007 | — 2134364153 | ECS_E_STORAGE_ERROR | Не удалось прослойировать файл из-за проблемы с хранилищем Azure. | Если ошибка повторяется, откройте запрос в службу поддержки. |
| 0x800703e3 | — 2147023901 | ERROR_OPERATION_ABORTED | Не удалось прослойировать файл, так как он был отозван одновременно. | Действия не требуются. Файл будет распределяться по уровням после завершения отзыва, и файл больше не используется. |
| 0x80c80264 | — 2134375836 | ECS_E_GHOSTING_FILE_NOT_SYNCED | Не удалось прослойировать файл, так как он не синхронизирован с общим файловым ресурсом Azure. | Действия не требуются. Файл будет иметь уровень после синхронизации с общим файловым ресурсом Azure. |
| 0x80070001 | — 2147942401 | ERROR_INVALID_FUNCTION | Не удалось прослойировать файл, так как не работает драйвер фильтра уровня облака (storagesync. sys). | Чтобы устранить эту проблему, откройте командную строку с повышенными привилегиями и выполните следующую команду:`fltmc load storagesync`<br>Если не удается загрузить драйвер фильтра storagesync при выполнении команды флтмк, удалите агент Синхронизация файлов Azure, перезапустите сервер и переустановите агент Синхронизация файлов Azure. |
| 0x80070070 | — 2147024784 | ERROR_DISK_FULL | Не удалось прослойировать файл из-за недостатка места на диске в томе, где находится конечная точка сервера. | Чтобы устранить эту проблему, освободите не менее 100 МБ дискового пространства на томе, где находится конечная точка сервера. |
| 0x80070490 | — 2147023728 | ERROR_NOT_FOUND | Не удалось прослойировать файл, так как он не синхронизирован с общим файловым ресурсом Azure. | Действия не требуются. Файл будет иметь уровень после синхронизации с общим файловым ресурсом Azure. |
| 0x80c80262 | — 2134375838 | ECS_E_GHOSTING_UNSUPPORTED_RP | Не удалось выполнить профилирование файла, так как это неподдерживаемая точка повторного анализа. | Если файл является точкой повторного анализа дедупликации данных, выполните действия, описанные в [руководстве по планированию](https://docs.microsoft.com/azure/storage/files/storage-sync-files-planning#data-deduplication) , чтобы включить поддержку дедупликации данных. Файлы с точками повторного анализа, отличными от дедупликации данных, не поддерживаются и не будут распределены по уровням.  |
| 0x80c83052 | — 2134364078 | ECS_E_CREATE_SV_STREAM_ID_MISMATCH | Не удалось прослойировать файл, так как он был изменен. | Действия не требуются. После синхронизации измененного файла с общим файловым ресурсом Azure будет выполнено профилирование. |
| 0x80c80269 | — 2134375831 | ECS_E_GHOSTING_REPLICA_NOT_FOUND | Не удалось прослойировать файл, так как он не синхронизирован с общим файловым ресурсом Azure. | Действия не требуются. Файл будет иметь уровень после синхронизации с общим файловым ресурсом Azure. |
| 0x80072ee2 | -2147012894 | WININET_E_TIMEOUT | Не удалось прослойировать файл из-за проблемы с сетью. | Действия не требуются. Если ошибка повторяется, проверьте сетевое подключение к общему файловому ресурсу Azure. |
| 0x80c80017 | -2134376425 | ECS_E_SYNC_OPLOCK_BROKEN | Не удалось прослойировать файл, так как он был изменен. | Действия не требуются. После синхронизации измененного файла с общим файловым ресурсом Azure будет выполнено профилирование. |
| 0x800705aa | — 2147023446 | ERROR_NO_SYSTEM_RESOURCES | Не удалось прослойировать файл из-за недостатка системных ресурсов. | Если ошибка повторяется, выясните, какое приложение или драйвер режима ядра исчерпали системные ресурсы. |
...
> [!NOTE]
> Идентификатор события 9006 регистрируется один раз в час в журнале событий телеметрии при сбое отзыва файла (на один код ошибки регистрируется одно событие). Ознакомьтесь с разделом [ошибки отзыва и исправлением](#recall-errors-and-remediation) , чтобы узнать, указаны ли действия по исправлению для кода ошибки.
...
| 0x80c86030 | -2134351824 | ECS_E_AZURE_FILE_SHARE_NOT_FOUND | Не удалось вспомнить файл, так как файловый ресурс Azure недоступен. | Убедитесь, что общая папка существует и доступна. Если файловый ресурс был удален и создан повторно, выполните действия, описанные в статье [Синхронизация, так как файловый ресурс Azure был удален и создан повторно](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#-2134375810) , чтобы удалить и повторно создать группу синхронизации. |
| 0x800705aa | — 2147023446 | ERROR_NO_SYSTEM_RESOURCES | Не удалось отозвать файл из-за недостатка системных ресурсов. | Если ошибка повторяется, выясните, какое приложение или драйвер режима ядра исчерпали системные ресурсы. |
| 0x8007000e | — 2147024882 | ERROR_OUTOFMEMORY | Не удалось отозвать файл из-за нехватки памяти. | Если ошибка повторяется, выясните, какое приложение или драйвер режима ядра вызывают нехватку памяти. |
| 0x80070070 | — 2147024784 | ERROR_DISK_FULL | Не удалось отозвать файл из-за недостатка места на диске. | Чтобы устранить эту проблему, освободите место на томе, переместив файлы на другой том, увеличив размер тома или перенесите файлы на уровень с помощью командлета Invoke-Сторажесинкклаудтиеринг. |
...
*Вариант 2. Подключите файловый ресурс Azure и скопируйте файлы локально, которые были потеряны на сервере*
...
### <a name="tls-12-required-for-azure-file-sync"></a>Для Синхронизация файлов Azure требуется TLS 1,2

Параметры TLS можно просмотреть на сервере, просмотрев [параметры реестра](https://docs.microsoft.com/windows-server/security/tls/tls-registry-settings). 

Если вы используете прокси-сервер, обратитесь к документации прокси-сервера и убедитесь, что она настроена на использование TLS 1.2.
...
Если проблема не устранена, запустите средство AFSDiag и отправьте его выходные данные в виде ZIP-файла инженеру службы поддержки, назначенному вашему случаю для дальнейшей диагностики.
...
1. Откройте окно PowerShell, а затем выполните следующие команды (нажимайте клавишу ВВОД после каждой команды).
    ...
    > [!NOTE]
    >AFSDiag создаст выходной каталог и временную папку в нем перед сбором журналов и удалит временную папку после выполнения. Укажите расположение выхода, которое не содержит данных.
