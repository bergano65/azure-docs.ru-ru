...
На сервере, который отображается как "отображается в автономном режиме" на портале, просмотрите событие с ИДЕНТИФИКАТОРом 9301 в журнале событий телеметрии (расположенном в разделе Applications and Сервицес\микрософт\филесинк\ажент в Просмотр событий), чтобы определить причину, по которой сервер не может получить доступ к Синхронизация файлов Azure служеб. 
...
| 0x80070017 | — 2147024873 | ERROR_CRC | Не удается синхронизировать файл из-за ошибки CRC. Эта ошибка может возникать, если многоуровневый файл не был отозван до удаления конечной точки сервера или если файл поврежден. | Сведения об устранении этой проблемы см. в разделе [многоуровневые файлы недоступны на сервере после удаления конечной точки сервера](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#tiered-files-are-not-accessible-on-the-server-after-deleting-a-server-endpoint) для удаления потерянных многоуровневых файлов. Если ошибка повторяется после удаления многоуровневого файла потерянные, запустите [программу chkdsk](https://docs.microsoft.com/windows-server/administration/windows-commands/chkdsk) на томе. |
...
1. Скачайте средство [PsExec](https://docs.microsoft.com/sysinternals/downloads/psexec).
...
Эта ошибка возникает, когда длительность операции приема данных превышает время ожидания. Эту ошибку можно пропустить, если выполняется синхронизация (Апплиедитемкаунт больше 0). См. [разделы справки мониторинг хода выполнения текущего сеанса синхронизации](#how-do-i-monitor-the-progress-of-a-current-sync-session).
...
| HRESULT | HRESULT (десятичное число) | Строка ошибки | Проблема | Исправление |
|---------|-------------------|--------------|-------|-------------|
| 0x80c86043 | — 2134351805 | ECS_E_GHOSTING_FILE_IN_USE | Не удалось прослойировать файл, так как он используется. | Действия не требуются. По завершении использования файл будет помещен в многоуровневое хранилище. |
| 0x80c80241 | — 2134375871 | ECS_E_GHOSTING_EXCLUDED_BY_SYNC | Не удалось прослойировать файл, так как он исключен синхронизацией. | Действия не требуются. Файлы из списка исключений синхронизации не могут храниться в многоуровневом хранилище. |
| 0x80c86042 | — 2134351806 | ECS_E_GHOSTING_FILE_NOT_FOUND | Не удалось прослойировать файл, так как он не был найден на сервере. | Действия не требуются. Если ошибка повторится, проверьте, существует ли файл на сервере. |
| 0x80c83053 | — 2134364077 | ECS_E_CREATE_SV_FILE_DELETED | Не удалось прослойировать файл, так как он был удален в общем файловом ресурсе Azure. | Действия не требуются. Файл должен быть удален на сервере при следующем запуске сеанса синхронизации скачивания. |
| 0x80c8600e | — 2134351858 | ECS_E_AZURE_SERVER_BUSY | Не удалось прослойировать файл из-за проблемы с сетью. | Действия не требуются. Если ошибка повторится, проверьте сетевое подключение к общей папке Azure. |
| 0x80072ee7 | -2147012889 | WININET_E_NAME_NOT_RESOLVED | Не удалось прослойировать файл из-за проблемы с сетью. | Действия не требуются. Если ошибка повторится, проверьте сетевое подключение к общей папке Azure. |
...
| 0x80072efe | — 2147012866 | WININET_E_CONNECTION_ABORTED | Не удалось прослойировать файл из-за проблемы с сетью. | Действия не требуются. Если ошибка повторится, проверьте сетевое подключение к общей папке Azure. |
| 0x80c80261 | — 2134375839 | ECS_E_GHOSTING_MIN_FILE_SIZE | Не удалось прослойировать файл, так как размер файла меньше поддерживаемого размера. | Если версия агента меньше 9.0, то минимальный поддерживаемый размер файла — 64 КБ. Если версия агента — 9.0 и выше, то минимальный поддерживаемый размер файла зависит от размера кластера файловой системы и составляет два кластера файловой системы. Например, если размер кластера файловой системы равен 4 КБ, минимальный размер файла — 8 КБ. |
...
| 0x80c80264 | — 2134375836 | ECS_E_GHOSTING_FILE_NOT_SYNCED | Не удалось прослойировать файл, так как он не синхронизирован с общим файловым ресурсом Azure. | Действия не требуются. Файл будет помещен в многоуровневое хранилище после синхронизации с общей папкой Azure. |
...
| 0x80070490 | — 2147023728 | ERROR_NOT_FOUND | Не удалось прослойировать файл, так как он не синхронизирован с общим файловым ресурсом Azure. | Действия не требуются. Файл будет помещен в многоуровневое хранилище после синхронизации с общей папкой Azure. |
| 0x80c80262 | — 2134375838 | ECS_E_GHOSTING_UNSUPPORTED_RP | Не удалось выполнить профилирование файла, так как это неподдерживаемая точка повторного анализа. | Если файл является точкой повторного анализа дедупликации данных, следуйте инструкциям в [руководстве по планированию](https://docs.microsoft.com/azure/storage/files/storage-sync-files-planning#data-deduplication), чтобы включить поддержку дедупликации данных. Файлы с точками повторного анализа, отличными от точек повторного анализа дедупликации данных, не поддерживаются и не будут помещены в многоуровневое хранилище.  |
| 0x80c83052 | — 2134364078 | ECS_E_CREATE_SV_STREAM_ID_MISMATCH | Не удалось прослойировать файл, так как он был изменен. | Действия не требуются. Файл будет помещен в многоуровневое хранилище после синхронизации измененного файла с общей папкой Azure. |
| 0x80c80269 | — 2134375831 | ECS_E_GHOSTING_REPLICA_NOT_FOUND | Не удалось прослойировать файл, так как он не синхронизирован с общим файловым ресурсом Azure. | Действия не требуются. Файл будет помещен в многоуровневое хранилище после синхронизации с общей папкой Azure. |
| 0x80072ee2 | -2147012894 | WININET_E_TIMEOUT | Не удалось прослойировать файл из-за проблемы с сетью. | Действия не требуются. Если ошибка повторится, проверьте сетевое подключение к общей папке Azure. |
| 0x80c80017 | -2134376425 | ECS_E_SYNC_OPLOCK_BROKEN | Не удалось прослойировать файл, так как он был изменен. | Действия не требуются. Файл будет помещен в многоуровневое хранилище после синхронизации измененного файла с общей папкой Azure. |
| 0x800705aa | — 2147023446 | ERROR_NO_SYSTEM_RESOURCES | Не удалось прослойировать файл из-за недостатка системных ресурсов. | Если ошибка повторится, выясните, какое приложение или драйвер режима ядра исчерпали системные ресурсы. |
...
| 0x80070036 | — 2147024842 | ERROR_NETWORK_BUSY | Не удалось отозвать файл из-за проблемы в сети.  | Если ошибка повторится, проверьте сетевое подключение к общей папке Azure. |
...
| 0x800705aa | — 2147023446 | ERROR_NO_SYSTEM_RESOURCES | Не удалось отозвать файл из-за недостатка системных ресурсов. | Если ошибка повторится, выясните, какое приложение или драйвер режима ядра исчерпали системные ресурсы. |
| 0x8007000e | — 2147024882 | ERROR_OUTOFMEMORY | Не удалось отозвать файл из-за инсуффЦиент памяти. | Если ошибка повторится, выясните, какое приложение или драйвер режима ядра исчерпали память. |
| 0x80070070 | — 2147024784 | ERROR_DISK_FULL | Не удалось отозвать файл из-за недостатка места на диске. | Чтобы устранить эту проблему, освободите место в томе. Для этого переместите файлы в другой том, увеличьте размер тома или переместите файлы в многоуровневое хранилище, выполнив командлет Invoke-StorageSyncCloudTiering. |
...
Если приведенные выше условия не выполнены, восстановление доступа невозможно, так как многоуровневые файлы на сервере уже стали потерянными. Выполните приведенные ниже инструкции, чтобы удалить потерянные многоуровневые файлы.
