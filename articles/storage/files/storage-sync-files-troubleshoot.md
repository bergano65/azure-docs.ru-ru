...
<a id="server-already-registered"></a>**При регистрации сервера во время установки агента службы "Синхронизация файлов Azure" отображается следующее сообщение: "This server is already registered" (Сервер уже зарегистрирован)** 
...
* Чтение: Получить назначение ролей
* Запись: Создать назначение ролей
...
<a id="server-endpoint-createjobfailed"></a>**Создание конечной точки сервера завершается сбоем с такой ошибкой: "MgmtServerJobFailed" (код ошибки: 2134375898)**  
...
|---------|-------------------|--------------|-------|-------------|
| 0x80c80207 | -2134375929 | ECS_E_SYNC_CONSTRAINT_CONFLICT | На этом этапе невозможно синхронизировать изменение файла или каталога, потому что зависимая папка еще не синхронизирована. Этот элемент будет синхронизирован после синхронизации зависимых изменений. | Никаких действий не требуется. |
| 0x7b | 123 | ERROR_INVALID_NAME | Недопустимое имя файла или каталога. | Переименуйте соответствующий файл или каталог. Дополнительные сведения см. в разделе [Обработка неподдерживаемых знаков](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#handling-unsupported-characters). |
| 0x8007007b | -2147024773 | STIERR_INVALID_DEVICE_NAME | Недопустимое имя файла или каталога. | Переименуйте соответствующий файл или каталог. Дополнительные сведения см. в разделе [Обработка неподдерживаемых знаков](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#handling-unsupported-characters). |
...
| 0x80c8031d | -2134375651 | ECS_E_CONCURRENCY_CHECK_FAILED | Файл был изменен, но механизм синхронизации еще не обнаружил это изменение. Синхронизация восстановится после обнаружения изменения. | Никаких действий не требуется. |
...
| 0x80070005 | -2147024891 | E_ACCESSDENIED | Эта ошибка может возникать по следующим причинам: файл зашифрован, решение не поддерживается (например, NTFS EFS), файл есть удаление в состоянии ожидания или файл находится в папке только для чтения репликации DFS-R | Если файл зашифрован программой неподдерживаемый решением, расшифровать файл и использовать это решение шифрования, поддерживаемые. Список поддерживаемых решений см. в разделе [Решения для шифрования](https://docs.microsoft.com/azure/storage/files/storage-sync-files-planning#encryption-solutions) руководства по планированию. Если файл находится в состоянии ожидания удаления, он будет удален после закрытия всех открытых дескрипторов файла. Если файл расположен в папке только для чтения репликации DFS-R, службе синхронизации файлов Azure не поддерживает конечные точки сервера для папок DFS-R репликация только для чтения. См. в разделе [руководство по планированию](https://docs.microsoft.com/azure/storage/files/storage-sync-files-planning#distributed-file-system-dfs) Дополнительные сведения.
| 0x20 | 32 | ERROR_SHARING_VIOLATION | Невозможно синхронизировать файл, так как он используется. По завершении использования файла он будет синхронизирован. | Никаких действий не требуется. |
...
Если в сценарии PowerShell **FileSyncErrorsReport.ps1** отображаются ошибки из-за неподдерживаемых знаков (коды ошибок 0x7b и 0x8007007b), нужно удалить или переименовать в соответствующих именах файла знаки, которые привели к ошибке. PowerShell, скорее всего, выведет эти знаки как вопросительные знаки или пустые прямоугольники, так как большинство из этих знаков не имеют стандартного визуального кодирования. [Средство оценки](storage-sync-files-planning.md#evaluation-tool) можно использовать, чтобы идентифицировать знаки, которые не поддерживаются.
...
Никаких действий не требуется. Когда файл или в общей папке (облачной конечной точки) восстанавливается с помощью Azure Backup, синхронизации блокируется до завершения обнаружения изменений в общую папку Azure. Обнаружение изменений выполняется немедленно в том случае, когда восстановление завершится, и длительность основан на количество файлов в файловом ресурсе.
...
- Произошел сбой при удалении конечной точки сервера.
- Истек срок действия сертификата, используемого для проверки подлинности. 
...
Если время сервера указано правильно, выполните следующие шаги, чтобы решить проблему.
...
Убедитесь, что путь существует, указан на локальном томе NTFS и не является точкой повторного анализа или существующей конечной точкой сервера.
...
1. Проверьте наличие **гибридной службы "Синхронизация файлов"** в списке с ролью **модуля чтения и доступа к данным**. 
    ...
    ![Снимок экрана службы синхронизации файлов гибридного субъекта-службы на вкладке управления доступа к учетной записи хранения](media/storage-sync-files-troubleshoot/file-share-inaccessible-3.png)
