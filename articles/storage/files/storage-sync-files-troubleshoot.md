...
На сервере, который отображается как "Появляется в автономном режиме" на портале, посмотрите на Event ID 9301 в журнале событий Telemetry (находится в приложении и службах(Microsoft-FileSync'Agent in Event Viewer), чтобы определить, почему сервер не может получить доступ к службе синхронизации файлов Azure. 
...
Эта ошибка может произойти, если организация использует прокси-сервер TLS или если вредоносная организация перехватывает трафик между сервером и службой синхронизации файлов Azure. Если вы уверены, что это ожидаемо (потому что ваша организация использует прокси-сервер TLS, вы пропускаете проверку сертификата с переопределением реестра.
...
Установив значение реестра, агент Azure File Sync принимает любой локально доверенный сертификат TLS/SSL при передаче данных между сервером и облачным сервисом.
