...
2. **Выберите "подключиться к серверу" в меню "перейти"**: используя UNC-путь из предварительных требований, преобразуйте начальную обратную`\\`косую `smb://` черту () в и все`\`другие обратные косые черты (`/`), чтобы переслать косую черту (). Ссылка должна выглядеть следующим образом: ![диалоговое окно "Подключение к серверу"](./media/storage-how-to-use-files-mac/mount-via-finder-2.png)
