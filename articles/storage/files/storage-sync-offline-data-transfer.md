...
Перед началом передачи данных в автономном режиме:

- Перенести данные массового в один или несколько файловых ресурсов Azure, перед включением синхронизации со службой синхронизации файлов Azure.
- Если вы планируете использовать Data Box для массового переноса, просмотрите [предварительные условия для развертывания для поля данных](../../databox/data-box-deploy-ordered.md#prerequisites).
- Планируйте топологию окончательный службы синхронизации файлов Azure. Дополнительные сведения см. в разделе [Планирование развертывания службы синхронизации файлов Azure](storage-sync-files-planning.md).
- Выберите учетную запись хранения Azure или учетных записей, которые будут содержать общие папки, которые вы хотите синхронизировать с. Перенести данные массового временной промежуточной общих папок, находящихся в одной учетной записи хранения или учетные записи. Можно использовать только окончательный общей папке и промежуточную общую папку, в одну и ту же учетную запись хранения.
- Создайте новую связь синхронизации с расположением сервера. Существующее отношение синхронизации нельзя использовать для переноса массовых данных.
