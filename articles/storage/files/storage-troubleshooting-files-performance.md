...
- Для виртуальных машин Linux увеличьте время ожидания кэша записи каталога, указав **актимео =\<с >** в качестве параметра подключения. По умолчанию это одна секунда, поэтому может помочь большее значение, например три или пять.
...
1. В [портал Azure](https://portal.azure.com)щелкните **монитор**. 

2. Щелкните **оповещения** , а затем — **+ новое правило генерации оповещений**.

3. Щелкните **выбрать** , чтобы выбрать **учетную запись хранения или файловый** ресурс, содержащий общую папку, для которой необходимо создать оповещение, и нажмите кнопку **Готово**. Например, если имя учетной записи хранения — Contoso, выберите ресурс Contoso/File.

4. Нажмите кнопку **Добавить** , чтобы добавить условие.

5. Вы увидите список сигналов, поддерживаемых для учетной записи хранения, и выберите метрику **транзакции** .

6. В колонке **Настройка логики сигнала** перейдите к измерению **тип ответа** , щелкните раскрывающийся список **значения измерения** и выберите **сукцессвиссроттлинг** (для SMB) или **ClientThrottlingError** (для остальных). 

  > [!NOTE]
  > Если значение измерения Сукцессвиссроттлинг или ClientThrottlingError отсутствует в списке, это означает, что ресурс не был отрегулирован.  Чтобы добавить значение измерения, щелкните **+** рядом с раскрывающимся списком **значения измерений** , введите **сукцессвиссроттлинг** или **ClientThrottlingError**, нажмите кнопку **ОК** , а затем повторите шаг #6.

7. Перейдите к измерению общей **папки** , щелкните раскрывающийся список **значения измерения** и выберите Общие папки, по которым необходимо создать оповещение. 

  > [!NOTE]
  > Если общая папка является стандартным общим ресурсом, раскрывающийся список значений измерений будет пустым, так как метрики для общих папок недоступны для стандартных файловых ресурсов. Предупреждения регулирования для стандартных файловых ресурсов будут активированы, если какая-либо общая папка в учетной записи хранения регулируется, а предупреждение не определит, какой файловый ресурс был отрегулирован. Так как метрики для общего ресурса недоступны для стандартных файловых ресурсов, рекомендуется использовать одну общую папку для каждой учетной записи хранения. 

8. Определите **Параметры оповещений** (пороговое значение, оператор, гранулярность и частота агрегирования), которые используются для вычисления правила генерации оповещений метрики, и нажмите кнопку **Готово**.

  > [!TIP]
  > Если используется статическое пороговое значение, диаграмма метрик может помочь определить разумное пороговое значение, если общая папка в данный момент регулируется. Если используется динамическое пороговое значение, на диаграмме метрики будут отображаться рассчитанные пороговые значения на основе последних данных.

9. Добавьте **группу действий** (электронную почту, SMS и т. д.) в предупреждение либо выбрав существующую группу действий, либо создав новую группу действий.
