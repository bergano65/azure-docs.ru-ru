...
2. [Форум хранения azure](https://social.msdn.microsoft.com/forums/azure/home?forum=windowsazuredata).
...
* <a id="files-versus-blobs"></a>
  **Почему для хранения данных предпочтительнее использовать файловый ресурс Azure, а не хранилище BLOB-объектов Azure?**  
    ...
    Более подробное описание различий между файлами Azure и хранилищем Azure Blob можно узнать в [описании основных служб хранения azure.](../common/storage-introduction.md) Дополнительные сведения о хранилище BLOB-объектов Azure см. в статье [Общие сведения о хранилище BLOB-объектов](../blobs/storage-blobs-introduction.md).
...
* <a id="files-versus-disks"></a>**Почему предпочтительнее использовать файловый ресурс Azure, а не диски Azure?**  
    ...
    Дополнительные сведения о способе настройки высокопроизводительного и высокодоступного файлового сервера в Azure см. в записи блога о [развертывании гостевых кластеров виртуальной машины IaaS в Microsoft Azure](https://blogs.msdn.microsoft.com/clustering/2017/02/14/deploying-an-iaas-vm-guest-clusters-in-microsoft-azure/). Более подробное описание различий между файлами Azure и дисками Azure можно ознакомьтесь [с основными службами хранения данных Azure.](../common/storage-introduction.md) Дополнительные сведения о дисках Azure см. в статье [Обзор компонента "Управляемые диски" Azure](../../virtual-machines/windows/managed-disks-overview.md).
...
* <a id="redundancy-options"></a>
  **Какие параметры избыточности хранилища поддерживаются в службе "Файлы Azure"?**  
    В настоящее время Azure Files поддерживает локально избыточное хранилище (LRS), избыточное хранилище зон (ЗРС), геоизбыточное хранилище (GRS) и геозонно-избыточное хранилище (ГЗРС) (предварительный просмотр). В будущем мы планируем включить поддержку геоизбыточного хранилища с доступом на чтение (RA-GRS), но о конкретных сроках пока говорить рано.

* <a id="tier-options"></a>
  **Какие уровни хранилища поддерживаются службой файлов Azure?**  
    Azure Files поддерживает два уровня хранения: премиум и стандартный. Стандартные файловые акции создаются в учетных записях хранения общего назначения (GPv1 или GPv2), а премиальные файлные акции создаются в учетных записях хранения FileStorage. Узнайте больше о том, как создать [стандартные файлы и](storage-how-to-create-file-share.md) [премиум-файлы.](storage-how-to-create-premium-fileshare.md) 
    
    > [!NOTE]
    > Нельзя создавать файлы Azure из учетных записей хранения Blob или учетных записей общего назначения *премиум-класса* (GPv1 или GPv2). Акции файлов Standard Azure должны создаваться только в *стандартных* учетных записях общего назначения, а премиальные файлы Azure должны создаваться только в учетных записях хранения FileStorage. *Премиум* общего назначения (GPv1 и GPv2) хранения счетов предназначены только для премиум-страницы капли. 

* <a id="give-us-feedback"></a>
  **Я действительно хочу увидеть определенную функцию, добавленную в Файлы Azure. Можете ли вы добавить его?**  
  ...
  **Поддерживает ли Azure Files блокировку файлов?**  
    Да, Azure Files полностью поддерживает блокировку файлов в стиле SMB/Windows, [см.](https://docs.microsoft.com/rest/api/storageservices/managing-file-locks) 
...
* <a id="afs-change-detection"></a>
  **Я создал файл непосредственно в моем файле Azure, используя SMB или на портале. Сколько времени требуется для синхронизации файла с серверами в группе синхронизации?**  
    [!INCLUDE [storage-sync-files-change-detection](../../../includes/storage-sync-files-change-detection.md)]

* <a id="afs-conflict-resolution"></a>**Если один и тот же файл изменяется на двух серверах примерно в одно и то же время, что произойдет?**  
    ...
    Например, файл CompanyReport.docx при первом конфликте станет CompanyReport-CentralServer.docx, где CentralServer является расположением, в котором запись изменений произошла раньше. Для второго конфликта соответственно будет использоваться имя CompanyReport-CentralServer-1.docx. Azure File Sync поддерживает 100 конфликтных файлов на файл. Как только будет достигнуто максимальное количество конфликтных файлов, файл не сможет синхронизироваться до тех пор, пока количество файлов конфликтов не будет меньше 100.
...
* <a id="sizeondisk-versus-size"></a>
  **Почему *размер на диске* для файла не соответствует свойству *размера* после использования Azure File Sync?**  
...
* <a id="afs-files-excluded"></a>
  **Какие файлы или папки автоматически исключены в службе "Синхронизация файлов Azure"?**  
  Посмотреть [файлы пропущены](storage-sync-files-planning.md#files-skipped).
...
* <a id="afs-os-support"></a>
  **Могу ли я использовать службу "Синхронизация файлов Azure" с Windows Server 2008 R2, Linux или своим запоминающим устройством, подключаемым к сети (NAS)?**  
    В настоящее время Azure File Sync поддерживает только Windows Server 2019, Windows Server 2016 и Windows Server 2012 R2. В данный момент мы ничего не планируем, но мы с радостью рассмотрим предложения по поводу поддержки дополнительных платформ, проанализировав потребности клиентов. Сообщите нам о платформах, для которых нужно реализовать поддержку, на портале [UserVoice службы файлов Azure](https://feedback.azure.com/forums/217298-storage/category/180670-files).
...
* <a id="afs-do-not-delete-server-endpoint"></a>
  **У меня возникла проблема с синхронизацией файлов Azure на моем сервере (синхронизация, многоуровневое облако и т.д.). Должен ли я удалить и воссоздать конечную точку сервера?**  
...
* <a id="afs-ntfs-acls"></a>
  **Сохраняет ли служба синхронизации файлов Azure наряду с данными, хранящимися в службе файлов Azure, списки управления доступом NTFS для каталогов или файлов?**
    ...
    С 24 февраля 2020 года новые и существующие AC-системы, многоуровневые синхронизацией файлов Azure, будут сохраняться в формате NTFS, а изменения ACL, внесенные непосредственно в раздел файлов Azure, будут синхронизированы со всеми серверами в группе синхронизации. Любые изменения в ОКЛ, внесенные в файлы Azure, будут синхронизироваться с помощью синхронизации файлов Azure. При копировании данных в файлы Azure убедитесь, что вы используете SMB для доступа к акции и сохранения своих ACL. Существующие инструменты на основе REST, такие как AzCopy или Storage Explorer, не сохраняются.

    Если вы включили резервное копирование Azure в управляемых файлах файлов, апли могут продолжать восстанавливаться в рамках резервного копирования, восстанавливая рабочий процесс. Это работает либо для всей доли или отдельных файлов / каталогов.

    Если вы используете снимки как часть самоуправляемого решения резервного копирования для файлов, управляемых синхронизацией файлов, ваши ACL не могут быть восстановлены должным образом в NTFS ACLs, если снимки были сделаны до 24 февраля 2020 года. Если это происходит, обратитесь в службу поддержки Azure.
    
## <a name="security-authentication-and-access-control"></a>Безопасность, проверка подлинности и управление доступом
* <a id="ad-support"></a>
**Поддерживается ли проверка подлинности и доступом на основе идентификационных данных и контроль доступа в Azure Files?**  
    
    Да, Azure Files поддерживает проверку подлинности и контроля доступа на основе идентификации. Вы можете выбрать один из двух способов использования элемента управления доступом на основе идентификационных данных: на фотослужбы домена Active Directory Domain Services (предварительный просмотр) или службы домена Active Directory Domain (Azure AD DS). В непредпомянении Active Directory Domain Services (AD DS) поддерживается аутентификация с помощью доменных компьютеров AD DS, как на месте, так и в Azure, для доступа к файлам Azure через SMB. Аутентификация Azure AD DS по сравнению с SMB для файлов Azure Позволяет доменным vMs-миноностам Azure AD DS получить доступ к акциям, каталогам и файлам с использованием учетных данных Azure AD. Для получения более подробной информации смотрите [обзор поддержки аутентификации на основе файлов Azure для доступа к SMB.](storage-files-active-directory-overview.md) 

    В службе файлов Azure предусмотрено два дополнительных способа управления доступом:

    - Вы можете использовать подписанные URL-адреса (SAS), чтобы создать маркеры со специальными разрешениями, которые будут допустимы для использования в течение определенного интервала времени. Например, вы можете создать маркер с доступом только для чтения для одного конкретного файла со сроком действия 10 минут. Любой пользователь, обладающий этим маркером, пока он действует, получит доступ только для чтения к этому файлу на 10 минут. Ключи подписи общего доступа поддерживаются только через REST API или в клиентских библиотеках. Вам необходимо подключить файловый ресурс Azure через протокол SMB с помощью ключей учетной записи хранения.
    ...
    Вы можете обратиться к [авторизующим доступу к хранилищеaz](https://docs.microsoft.com/azure/storage/common/storage-auth?toc=%2fazure%2fstorage%2fblobs%2ftoc.json) для полного представления всех протоколов, поддерживаемых службами хранения Azure. 

* <a id="ad-support-devices"></a>
**Поддерживает ли доступ к SMB-доступу Azure AD с помощью учетных данных Azure AD с устройств, объединенных или зарегистрированных в Azure AD, службы функциональности Azure Files Active Directory Domain Services (Azure AD DS) с помощью доступа к SMB с помощью учетных данных Azure AD с устройств, объединенных или зарегистрированных в Azure AD?**
...
**Существуют ли API REST, которые поддерживают получение, настройку или копирование списков управления доступом NTFS для каталогов или файлов?**
    ...
    Да, мы поддерживаем REST API, которые получают, устанавливают или копируют NTFS ACLs для каталогов или файлов при использовании API [REST 2019-07-07](https://docs.microsoft.com/rest/api/storageservices/versioning-for-the-azure-storage-services#version-2019-07-07) (или позже).

* <a id="ad-vm-subscription"></a>
**Могу ли я получить доступ к файлам Azure с учетными данными Azure AD из VM по другой подписке?**

    Если подписка, под которой развернута доля файла, связана с тем же арендатором Azure AD, что и развертывание DD Azure AD, к которому кигоняшне соединен аименной, можно получить доступ к файлам Azure, используя те же учетные данные Azure AD. Это ограничение накладывается не на подписку, а на связанный клиент Azure AD.
    
* <a id="ad-support-subscription"></a>
**Могу ли я включить проверку подлинности AD DS Azure или на месте AD DS для файлов Azure с помощью арендатора Azure AD, который отличается от основного арендатора файла Azure?**

    Нет, Azure Files поддерживает только AD DS Azure AD или на пределенционную интеграцию AD DS с арендатором Azure AD, который находится в той же подписке, что и доля файлов. С клиентом Azure AD может быть связана только одна подписка. Это ограничение распространяется как на Azure AD DS, так и на методы проверки подлинности AD DS. При использовании предварительного AD DS для проверки подлинности [учетные данные AD DS должны быть синхронизированы с Azure AD,](../../active-directory/hybrid/how-to-connect-install-roadmap.md) с которым связана учетная запись хранилища.

* <a id="ad-linux-vms"></a>
**Поддерживает ли Azure AD DS или предприимжение подлинности AD DS для файлов Azure: Linux VMs?**

    Нет, аутентификация от Linux VMs не поддерживается.

* <a id="ad-aad-smb-afs"></a>
**Поддерживают ли файлы, управляемые Azure File Sync, либо Azure AD DS, либо предприимжение подлинности AD DS (предварительный просмотр)?**

    Да, можно включить проверку подлинности Azure AD DS или на месте AD DS на файле, управляемом Azure File Sync. Изменения в каталоге/файле NTFS ACLs на локальных файловых серверах будут многоуровневыми в Файлы Azure и наоборот.

* <a id="ad-aad-smb-files"></a>
**Как проверить, включил ли я аутентификацию AD DS в своей учетной записи хранения и получить информацию о домене?**

    Для получения инструкций, смотрите [здесь](https://docs.microsoft.com/azure/storage/files/storage-files-identity-auth-active-directory-enable#1-enable-ad-authentication-for-your-account).
    
* <a id="encryption-at-rest"></a>
**Как обеспечить шифрование неактивных данных файлового ресурса Azure?**  

    Да. Для получения дополнительной [информации](../common/storage-service-encryption.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json)см.
...
**Можно ли указать только на чтение или только для записи разрешения на папках в рамках доли?**  
...
**Можно ли внедрить ограничения IP-адресов для файлового ресурса Azure?**  
    ...
    Да. Доступ к общему файловому ресурсу Azure можно ограничить на уровне учетной записи хранения. Для получения дополнительной информации [см.](../common/storage-network-security.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json)
...
* <a id=""></a>
**Поддерживает ли аутентификация Azure Files Azure AD Linux VMs?**

    Нет, аутентификация от Linux VMs не поддерживается.

* <a id="ad-multiple-forest"></a>
**Поддерживает ли напредпочная аутентификация AD DS для файлов Azure интеграцию с средой AD DS с использованием нескольких лесов?**    

    Проверка подлинности AD DS в Azure Files интегрируется только с лесом службы домена, на которую зарегистрирована учетная запись хранилища. Для поддержки аутентификации из другого леса в вашей среде необходимо правильно настроить лесной траст. Способ регистрации файлов Azure в DS AD почти такой же, как обычный файловый сервер, где он создает идентификацию (учетную запись входа в компьютер или службу) в AD DS для проверки подлинности. Разница лишь в том, что зарегистрированный SPN учетной записи хранилища заканчивается "file.core.windows.net", которая не совпадает с суффиксом домена. Проконсультируйтесь с администратором домена, чтобы узнать, требуется ли обновление политики dNS для включения нескольких мер проверки подлинности леса из-за разного суффикса домена.

* <a id=""></a>
**Какие регионы доступны для проверки подлинности AD DS Azure Files (предварительный просмотр)?**

    Для получения подробной информации обратитесь к [региональной доступности AD DS.](storage-files-identity-auth-active-directory-enable.md#regional-availability)
    
* <a id="ad-aad-smb-afs"></a>
**Могу ли я использовать активную проверку (AD) Azure Files Active Directory (AD) на файловых долях, управляемых Azure File Sync?**

    Да, можно включить аутентификацию AD на резолютивном файле, управляемом синхронизацией файлов Azure. Изменения в каталоге/файле NTFS ACLs на локальных файловых серверах будут многоуровневыми в Файлы Azure и наоборот.
...
* <a id="ad-aad-smb-files"></a>
**Есть ли какая-то разница в создании учетной записи компьютера или учетной записи журнала для входа в систему службы, чтобы представлять мою учетную запись хранения в AD?**

    Создание [учетной записи компьютера](https://docs.microsoft.com/windows/security/identity-protection/access-control/active-directory-accounts#manage-default-local-accounts-in-active-directory) (по умолчанию) или [учетной записи входа в службу](https://docs.microsoft.com/windows/win32/ad/about-service-logon-accounts) не имеет никакого значения в отношении того, как проверка подлинности будет работать с файлами Azure. Вы можете сделать свой собственный выбор в пользу того, как представлять учетную запись хранилища в качестве удостоверения личности в среде AD. По умолчанию DomainAccountType набор в Join-AzStorageAccountAccountForAuth cmdlet является учетная запись компьютера. Тем не менее, возраст истечения срока действия пароля, настроенный в вашей среде AD, может отличаться для учетной записи входа в журнал ы компьютера или службы, и вы должны принять это во внимание для [обновления пароля вашей учетной записи хранилища в AD.](https://docs.microsoft.com/azure/storage/files/storage-files-identity-auth-active-directory-enable#5-update-ad-account-password)

## <a name="on-premises-access"></a>Локальный доступ

* <a id="port-445-blocked"></a>
**Мой isP или ИТ-файлы Port 445 не смонтируют сярприт файлов Azure. Что мне делать?**

    Вы можете узнать о [различных способах обхода заблокирован ный порт 445 здесь](https://docs.microsoft.com/azure/storage/files/storage-troubleshoot-windows-file-connection-problems#cause-1-port-445-is-blocked). Azure Files разрешает соединения только с помощью SMB 3.0 (с поддержкой шифрования) из-за пределов региона или центра обработки данных. Протокол SMB 3.0 ввел много функций безопасности, включая шифрование канала, которое очень безопасно для использования через Интернет. Однако его возможно, что порт 445 был заблокирован из-за исторических причин уязвимостей, найденных в нижних версиях SMB. В идеальном случае порт должен быть заблокирован только для sMB 1.0 трафика и SMB 1.0 должны быть выключены на всех клиентов.
...
**Как установить общий файл Azure на локальной машине?**  
...
**Какие размеры доступны для файлов Azure?**  
    Размеры файлов Azure (премиум и стандартный) могут масштабироваться до 100 TiB. [Ознакомьтесь с разделом Onboard to larger file shares (стандартный уровень)](storage-files-planning.md#enable-standard-file-shares-to-span-up-to-100-tib) в руководстве по планированию инструкций по бортовой посадке на большие файлные акции для стандартного уровня.

* <a id="lfs-performance-impact"></a>
**Влияет ли расширение квоты на совместное наем файла на мои рабочие нагрузки или синхронизацию файлов Azure?**
    
    Нет. Расширение квоты не повлияет на рабочие нагрузки или синхронизацию файлов Azure.
...
**Моя производительность замедляется, когда я распаковываю файлы в Файлах Azure. Что мне делать?**  
...
**Почему после подключения файлового ресурса Azure в Windows Server 2012 R2 или Windows 8.1 снижается производительность?**  
    Имеется известная проблема при подключении файлового ресурса Azure в Windows Server 2012 R2 и Windows 8.1 с накопительным обновлением за апрель 2014 г. для Windows 8.1 и Windows Server 2012 R2. Для достижения оптимальной производительности убедитесь, что на всех экземплярах Windows Server 2012 R2 и Windows 8.1 есть это исправление. (Вы всегда должны получать патчи Windows через обновление Windows.) Для получения дополнительной информации, см связанные Microsoft Knowledge Base статьи [Медленная производительность при доступе к Лазурные файлы из Windows 8.1 или Server 2012 R2](https://support.microsoft.com/kb/3114025).
...
* <a id="rest-rename"></a>
**Есть ли операция переименования в REST API?**  
    На данный момент нет.

* <a id="nested-shares"></a>
**Могу ли я настроить вложенные акции? Другими словами, доля под акцией?**  
    Нет. Доля файлов *— это* виртуальный драйвер, который можно смонтировать, поэтому вложенные акции не поддерживаются.
