...
* <a id="afs-change-detection"></a>
  **я создал файл непосредственно в моем файловом ресурсе Azure с помощью SMB или на портале. Сколько времени требуется для синхронизации файла с серверами в группе синхронизации?**  
...
* <a id="afs-do-not-delete-server-endpoint"></a>
  **возникли проблемы с Синхронизация файлов Azure на моем сервере (синхронизация, распределение по уровням облака и т. д.). Следует ли удалить и повторно создать конечную точку сервера?**  
...
* <a id="afs-ntfs-acls"></a>
  **Сохраняет ли служба синхронизации файлов Azure наряду с данными, хранящимися в службе файлов Azure, списки управления доступом NTFS для каталогов или файлов?**
    ...
    Если вы используете моментальные снимки в рамках самостоятельно управляемого решения резервного копирования для файловых ресурсов, управляемых с помощью синхронизации файлов, списки ACL могут быть неправильно восстановлены в ACL NTFS, если моментальные снимки были сделаны до 24 февраля 2020. В этом случае рассмотрите возможность обращения в службу поддержки Azure.
...
**поддерживает ли служба "файлы Azure" проверку подлинности и управление доступом на основе удостоверений?**  
    ...
    Да, служба файлов Azure поддерживает проверку подлинности и контроль доступа на основе удостоверений. Можно выбрать один из двух способов использования управления доступом на основе удостоверений: Azure Active Directory доменные службы (Azure AD DS) (GA) или Active Directory (AD) (Предварительная версия). Azure AD DS проверку подлинности по SMB для файлов Azure AD DS позволяет виртуальным машинам Windows, присоединенным к домену, присоединяться к ресурсам, каталогам и файлам с помощью учетных данных Azure AD AD поддерживает аутентификацию с помощью компьютеров, присоединенных к домену AD (локально или в Azure), для доступа к файловым ресурсам Azure через SMB. Дополнительные сведения см. в статье [Поддержка проверки подлинности на основе удостоверений в службе файлов Azure для доступа по протоколу SMB](storage-files-active-directory-overview.md). 
...
**поддерживает ли служба проверки подлинности azure AD DS службы Azure Active Directory файлов Azure доступ к SMB с использованием учетных данных Azure AD с устройств, присоединенных к Azure AD или зарегистрированных** в ней?
...
**можно ли включить Azure "файлы Azure" AD DS или проверку подлинности AD в клиенте Azure AD, который отличается от основного клиента, с которым связан файловый ресурс?**
    ...
    Нет, служба "файлы Azure" поддерживает только AD DS Azure или интеграцию AD с клиентом Azure AD, который находится в той же подписке, что и файловый ресурс. С клиентом Azure AD может быть связана только одна подписка. Это ограничение относится как к Azure AD DS, так и к методам проверки подлинности AD. При использовании AD для проверки подлинности учетные данные AD должны быть синхронизированы с Azure AD, с которой связана учетная запись хранения.

* <a id="ad-linux-vms"></a>
**поддерживает ли служба "AD DS Azure" файлы Azure или проверку подлинности AD на виртуальных машинах Linux?**

    Нет, проверка подлинности с виртуальных машин Linux не поддерживается.

* <a id="ad-multiple-forest"></a>
**поддерживает ли служба проверки подлинности Active Directory для файлов Azure интеграцию с средой AD с использованием нескольких лесов?**    

    Проверка подлинности AD в службе файлов Azure интегрируется только с лесом доменной службы AD, в которую зарегистрирована учетная запись хранения. Для поддержки проверки подлинности из другого леса AD в вашей среде должно быть настроено доверие леса должным образом. Регистрация файлов Azure в службе домена AD в основном совпадает с обычным файловым сервером, где в AD создается учетная запись для проверки подлинности. Единственное отличие заключается в том, что зарегистрированное имя участника-службы учетной записи хранения заканчивается на "file.core.windows.net", который не соответствует суффиксу домена.

    Обратитесь к администратору домена, чтобы узнать, требуется ли обновление политики маршрутизации DNS для включения проверки подлинности нескольких лесов.

* <a id=""></a>
 **, какие регионы доступны для проверки подлинности Active Directory для службы файлов Azure (Предварительная версия)?**

    Дополнительные сведения см. в статье о [доступности в регионе AD](storage-files-active-directory-domain-services-enable.md#regional-availability) .

* <a id="ad-aad-smb-afs"></a>
**можно ли использовать службу файлов Azure для проверки подлинности azure AD DS или Active Directory (AD) (Предварительная версия) в общих папках, управляемых с помощью синхронизация файлов Azure?**
...
**поставщик услуг Интернета или блокирует порт 445, что отдает сбой при подключении файлов Azure. Что мне делать?**
...
**производительность снижается при распаковке файлов в службе файлов Azure. Что мне делать?**  
