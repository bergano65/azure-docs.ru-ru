...
* <a id="tier-options"></a>
  **Какие уровни хранилища поддерживаются службой файлов Azure?**  
    Служба файлов Azure поддерживает два уровня: premium и standard. Стандартные общие папки создаются в целом (GPv1 и GPv2) учетные записи хранения назначения и "премиум" Общие папки создаются в учетных записях хранения в хранилище файлов. Дополнительные сведения о создании [стандартные файловые ресурсы](storage-how-to-create-file-share.md) и [файловых ресурсов уровня "премиум"](storage-how-to-create-premium-fileshare.md). 
    
    > [!NOTE]
    > Не удается создать файловые ресурсы Azure из учетных записей хранения BLOB-объектов или *уровня "премиум"* общего назначения (GPv1 и GPv2) учетные записи хранения. Стандартные файловые ресурсы Azure должны, созданные в *стандартный* только и "премиум" файловые ресурсы Azure должны быть созданы в хранилище учетных записей хранения только учетные записи общего назначения. *"Премиум"* общего назначения (GPv1 и GPv2) учетных записей хранения предназначены для уровня "премиум" страничных BLOB-объектов только. 
...
**Какие размеры доступны для файловых ресурсов Azure?**  
    Размеры общей папки службы файлов Azure ("премиум" и "стандартный") можно масштабировать до 100 ТБ. Размеры уровня "премиум" Общие папки для файлов до 100 Tиб доступны как предложение общедоступной версии. Стандартный файл размеры общие ресурсы до 5 Тиб доступны в виде общедоступной версии, время предлагает размеры до 100 Tиб, доступны в предварительной версии. См. в разделе [. подключитесь к общим большего размера (уровня "стандартный")](storage-files-planning.md#onboard-to-larger-file-shares-standard-tier) предварительной версии для уровня "стандартный" использует раздел руководства по планированию для инструкции по подключению к файлу большего размера.
