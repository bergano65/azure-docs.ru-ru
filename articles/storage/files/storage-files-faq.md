...
**Поддерживается ли проверка подлинности и доступом на основе идентификационных данных и контроль доступа в Azure Files?**  
    ...
    Да, Azure Files поддерживает проверку подлинности и контроля доступа на основе идентификации. Вы можете выбрать один из двух способов использования элемента управления доступом на основе идентификационных данных: Active Directory (AD) (предварительный просмотр) или службы домена Active Directory (Azure AD DS) (GA). AD поддерживает аутентификацию с помощью объединенных спомощьами домена AD, как на месте, так и в Azure, для доступа к файлам Azure через SMB. Аутентификация Azure AD DS по сравнению с SMB для файлов Azure Позволяет доменным vMs-миноностам Azure AD DS получить доступ к акциям, каталогам и файлам с использованием учетных данных Azure AD. Для получения более подробной информации смотрите [обзор поддержки аутентификации на основе файлов Azure для доступа к SMB.](storage-files-active-directory-overview.md) 
...
**Могу ли я включить dD DS Azure Files AD или aD-аутентификацию с нанимателем Azure AD, который отличается от основного элемента, с которым связан доля файла?**

    Нет, Azure Files поддерживает dS dS или AD Azure AD только с арендатором Azure AD, который находится в той же подписке, что и доля файлов. С клиентом Azure AD может быть связана только одна подписка. Это ограничение распространяется как на методы проверки подлинности Azure AD DS, так и на методы аутентификации AD. При использовании AD для проверки подлинности учетные данные AD должны быть синхронизированы с AD Azure AD, с которым связана учетная запись хранилища.

* <a id="ad-linux-vms"></a>
**Поддерживает ли Azure Files Azure AD DS или aD аутентификацию Linux VMs?**

    Нет, аутентификация от Linux VMs не поддерживается.

* <a id="ad-aad-smb-afs"></a>
**Могу ли я использовать аутентификацию Azure Files Azure AD DD или активную проверку подлинности каталога (AD) на файловых долях, управляемых Azure File Sync?**

    Да, можно включить проверку подлинности Azure AD DS или AD в файле, управляемом синхронизацией файлов Azure. Изменения в каталоге/файле NTFS ACLs на локальных файловых серверах будут многоуровневыми в Файлы Azure и наоборот.
...
**Поддерживает ли аутентификация Azure Files AD интеграцию с средой AD с использованием нескольких лесов?**    

    Аутентификация AD Azure Files только интегрируется с лесом доменного сервиса AD, на который зарегистрирована учетная запись хранилища. Для поддержки аутентификации из другого леса AD ваша среда должна правильно настроить доверие к лесу. Способ регистрации Файлов Azure в доменном службе AD в основном такой же, как и обычный файловый сервер, где он создает идентификационную учетную запись (компьютер или учетную запись входа в службу обслуживания) в AD для проверки подлинности. Разница лишь в том, что зарегистрированный SPN учетной записи хранилища заканчивается "file.core.windows.net", которая не совпадает с суффиксом домена. Проконсультируйтесь с администратором домена, чтобы узнать, требуется ли обновление политики dNS для включения нескольких мер проверки подлинности леса из-за разного суффикса домена.

* <a id=""></a>
**Какие регионы доступны для проверки подлинности AD AD Azure Files (предварительный просмотр)?**

    Для получения подробной информации обратитесь к [региональной доступности AD.](storage-files-identity-auth-active-directory-enable.md#regional-availability)
