...
| &nbsp; |Целевые показатели масштабируемости |[Вы приближаетесь к целевым показателям масштабируемости для сущностей в секунду?](#targets-for-data-operations) |
...
Дополнительные сведения о целевых показателях масштабируемости для службы таблиц см. в статье [Целевые показатели масштабируемости и производительности для хранилища таблиц](scalability-targets.md).
...
### <a name="targets-for-data-operations"></a>Целевые объекты для операций с данными

Хранилище Azure распределяет нагрузку по мере увеличения трафика к вашей учетной записи хранения. Но если в трафике наблюдаются внезапные пики, вы не сможете немедленно получить весь объем пропускной способности. Во время пиков следует ожидать регулирования и истечения времени ожидания по мере того, как служба хранилища Azure автоматически распределяет нагрузку по таблице. Медленное повышение, как правило, дает лучшие результаты, так как предоставляет системе время для распределения нагрузки.

#### <a name="entities-per-second-storage-account"></a>Масштабирование сущностей в секунду (учетная запись хранения)
...
#### <a name="entities-per-second-partition"></a>Масштабирование сущностей в секунду (раздел)
...
- [Целевые показатели масштабируемости и производительности для учетных записей хранения ценовой категории "Стандартный"](../common/scalability-targets-standard-account.md?toc=%2fazure%2fstorage%2ftables%2ftoc.json)
