...
* Чтобы найти всех сотрудников отдела продаж с адресом электронной почты, начинающимся с буквы "a", используйте следующее условие фильтрации: $filter=(PartitionKey eq 'Sales') and (RowKey ge 'email_a') and (RowKey lt 'email_b').  
  ...
  Синтаксис фильтрации, используемый в приведенных выше примерах, взят из REST API службы таблиц. Дополнительные сведения см. в статье [Query Entities](https://msdn.microsoft.com/library/azure/dd179421.aspx) (Сущности запроса).  
...
Вы ожидаете большой объем транзакций против этих сущностей и хотите свести к минимуму риск того, что служба столовой задушит вашего клиента.  
...
* Чтобы найти всех сотрудников отдела продаж с идентификатором сотрудника в диапазоне **000100** до **000199** отсортированы в использовании заказа идентификатора сотрудника: $filter "(PartitionKey eq 'empid_Sales') и (RowKey ge '000100') и (RowKey le '000199')  
...
* Как правило, лучше хранить дубликаты данных и гарантировать, что вы можете получить все данные, необходимые с помощью одного запроса, чем использовать один запрос, чтобы найти сущность с помощью вторичного индекса, а другой, чтобы найти необходимые данные в первичном индексе.  
...
* Сущность, хранящаяся в службе таблицы, еще не проиндексирована с помощью службы когнитивного поиска Azure.  
...
### <a name="context-and-problem"></a>Контекст и проблема
Служба таблиц автоматически индексирует сущности с помощью значений **PartitionKey** и **RowKey**. Это позволяет клиентскому приложению эффективно извлекать сущность с помощью запроса точки. Например, используя приведенную ниже структуру таблицы, клиентское приложение может эффективно получить отдельный объект сотрудника, используя имя отдела и идентификатор сотрудника **(PartitionKey** и **RowKey).**  
...
Для первого варианта вы создаете каплю для каждой уникальной фамилии, и в каждом хранилище blob список значений **PartitionKey** (отдел) и **RowKey** (идентификатор сотрудника) для сотрудников, которые имеют эту фамилию. При добавлении или удалении сотрудника следует убедиться, что содержимое соответствующего большого двоичного объекта согласовано с сущностями сотрудников.  
...
Свойство **EmployeeIDs** содержит список идов сотрудников, с фамилией, хранящейся в **RowKey**.  

Ниже описан процесс, которого следует придерживаться при добавлении нового сотрудника при использовании второго варианта. В этом примере мы добавляем сотрудника с ID 000152 и фамилией Джонс в отделе продаж:  
...
2. Создайте транзакцию группы сущностей (т.е. пакетную операцию), которая вставляет новую сущность сотрудника (значение**PartitionKey** "Продажи" и **значение RowKey** "000152"), и обновляет сущность индекса **(значение PartitionKey** "Продажи" и **RowKey** значение "Jones"), добавляя новый идентификатор сотрудника в список в поле EmployeeIDs. Сведения о транзакциях группы сущностей см. в разделе "Транзакции группы сущностей".  
...
В третьем случае использовать транзакции группы сущностей для обеспечения согласованности нельзя, поскольку сущности индексов находятся в разделе, отличном от раздела с сущностями сотрудников. Убедитесь, что сущности индекса в конечном итоге согласуются с сущностями сотрудников.  
...
* Учитывая, что отдельный объект имеет максимальный размер 1 МБ, опцион #2 и опцион #3 в решении предполагают, что список идентиматобов сотрудников для любой данной фамилии никогда не превышает 1 МБ. Если размер идентификаций сотрудников, вероятно, превышает 1 МБ, используйте опцию #1 и храните данные индекса в хранилище каброй.  
...
Используйте сложные значения **RowKey,** чтобы клиент мог искать связанные данные с помощью одного точечного запроса.  
...
Обратите внимание, что **RowKey** теперь является соединением ключа, состоящего из идентификатора сотрудника, и года данных обзора, которые позволяют получить данные о производительности сотрудника и просмотреть данные с помощью одного запроса для одной сущности.  
...
Например, чтобы получить 10 последних претензий по расходам, представленных сотрудником, можно использовать обратное значение тика, полученное из текущей даты/времени. В следующем примере кода C# показан один из способов создания подходящего значения «обратных тиков» для свойства **RowKey** , которое выполняет сортировку с самого последнего значения и до самого первого.  
...
Обратите внимание, что в этом примере **PartitionKey** и **RowKey** являются составными ключами. **PartitionKey** использует как отдел, так и идентификатор сотрудника для распределения журналов по нескольким разделам.  
...
Можно использовать LIN-сервис для извлечения нескольких объектов из службы таблицы при работе со стандартной библиотекой Microsoft Azure Cosmos Table Standard Library. 

```azurecli
dotnet add package Microsoft.Azure.Cosmos.Table
```

Чтобы приведенные ниже примеры работали, необходимо включить пространства имен:
...
EmployeeTable — это объект CloudTable, который\<реализует метод Create's ITableEntity>(), который возвращает\<>>. Объекты этого типа реализуют i'Sемерируемые и позволяют использовать как выражения запросов ЛИНА, так и синтаксис обозначений точек.

Извлечение нескольких сущностей и быть достигнуто путем указания запроса с положением **where.** Чтобы исключить просмотр таблицы, в предложение where необходимо всегда включать значение **PartitionKey** и, если возможно, значение **RowKey**, которые позволят избежать просмотров таблиц и разделов. Служба таблиц поддерживает ограниченный набор операторов сравнения («больше», «больше или равно», «меньше», «меньше или равно», «равно», «не равно»), которые могут использоваться в предложении where. 
...
Следующий пример кода показывает эквивалентную функциональность без использования синтаксиса LIN':  
...
Для получения других свойств необходимо использовать метод **TryGetValue** на **свойстве Свойств** класса **DynamicTableEntity.**  
