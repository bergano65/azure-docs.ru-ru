...
* Чтобы найти всех сотрудников отдела продаж с адресом электронной почты, начинающимся с буквы "a", используйте следующее условие фильтрации: $filter=(PartitionKey eq 'Sales') and (RowKey ge 'email_a') and (RowKey lt 'email_b').  
  ...
  Обратите внимание, что синтаксис фильтрации, используемый в приведенных выше примерах, взят из REST API службы таблиц. Дополнительные сведения см. в статье о [сущностях запроса](https://msdn.microsoft.com/library/azure/dd179421.aspx).  
...
Можно использовать LINQ для извлечения нескольких сущностей из службы таблиц, при работе с Microsoft Azure Cosmos таблицы стандартной библиотеки. 

```cli
dotnet add package Microsoft.Azure.Cosmos.Table
```

Чтобы сделать ниже рабочие примеры, вам потребуется для включения пространств имен:
...
EmployeeTable является объектом CloudTable, который реализует CreateQuery<ITableEntity>() метод, возвращающий TableQuery<ITableEntity>. Объекты этого типа реализации IQueryable и разрешить с помощью нотации синтаксиса выражения запросов LINQ и точка.

Извлечение нескольких сущностей и сделать, указав запрос с **где** предложение. Чтобы исключить просмотр таблицы, в предложение where необходимо всегда включать значение **PartitionKey** и, если возможно, значение **RowKey**, которые позволят избежать просмотров таблиц и разделов. Служба таблиц поддерживает ограниченный набор операторов сравнения («больше», «больше или равно», «меньше», «меньше или равно», «равно», «не равно»), которые могут использоваться в предложении where. 
...
Если вы знаете тип сущности, сохраненной с конкретными значениями **RowKey** и **PartitionKey**, то при извлечении сущности можно указать ее тип, как показано в предыдущих двух примерах извлечения сущностей типа **EmployeeEntity**: [Выполнение запроса точки с помощью клиентской библиотеки хранилища](#executing-a-point-query-using-the-storage-client-library) и [Извлечение нескольких сущностей с помощью LINQ](#retrieving-multiple-entities-using-linq).  
