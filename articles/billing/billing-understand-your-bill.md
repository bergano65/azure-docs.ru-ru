...
Эта статья не относится к клиентам с соглашением Enterprise (EA). Если вы используете Соглашение Enterprise, воспользуйтесь статьей [Understand your bill for Azure customers with an Enterprise Agreement](billing-understand-your-bill-ea.md) (Расшифровка счета для клиентов Azure с соглашением Enterprise).  
...
Чтобы просмотреть распределение расходов по дням, перейдите в раздел **Ежедневное использование** CSV-файла. Установите фильтр по значению Scheduler (Планировщик) в разделе *Категория счетчика*. Вы увидите, в какие дни использовался этот счетчик и какой потребления он зафиксировал. Также для сравнения приведены сведения о *ресурсе* и *группе ресурсов*. Значения в разделе *Потреблено* должны в сумме давать значение, указанное в счете.
...
### <a name="option-2-compare-the-usage-and-costs-with-the-azure-portal"></a>Вариант 2. Сравнение данных о потреблении и затратах с порталом Azure
...
Azure не включает стоимость ресурсов в счета напрямую. Затраты на ресурс всегда вычисляются по данным одного или нескольких счетчиков. Эти счетчики используются для отслеживания потребления ресурса на протяжении всего времени его существования. Затем эти измерения используются для выставления счетов.

Например при создании любого ресурса Azure, например виртуальной машины, вместе с ним создаются один или несколько экземпляров счетчиков. Эти измерения используются для отслеживания использования ресурса на протяжении времени. Каждый счетчик создает записи о потреблении, на основе которых Azure выставляет счета.
...
После создания виртуальной машины каждый из этих счетчиков начнет передавать данные о потреблении. Эти данные и соответствующая стоимость отслеживаются в системе учета затрат Azure.
