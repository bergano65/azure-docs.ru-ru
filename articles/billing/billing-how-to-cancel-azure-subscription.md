...
ms.locfileid: "66480235"
---
# <a name="cancel-your-subscription-for-azure"></a>Отмена подписки в Azure

Только подписка Azure [учетной записи администратора](billing-subscription-transfer.md#whoisaa) может отменить подписку Azure. Администратор подписки Azure можно также [назначить другому пользователю, от имени администратора подписки](billing-add-change-azure-subscription-administrator.md#assign-a-user-as-an-administrator-of-a-subscription) таким образом, чтобы они может отменить подписку. После отмены подписки вы больше не сможете получать доступ к службам и ресурсам Azure.
...
* Необходимо удалить все ресурсы и все группы ресурсов. Удаление является обязательным, прежде чем вы можете отменить подписку. Каждая группа ресурсов необходимо удалять отдельно. При удалении группы ресурсов необходимо подтвердить удаление, введя имя группы ресурсов.
* Если у вас есть все пользовательские роли, которые ссылаются на эту подписку в `AssignableScopes`, необходимо обновить эти пользовательские роли, чтобы удалить подписку. Если вы попытаетесь обновить пользовательскую роль после отмены подписки, может появиться ошибка. Дополнительные сведения см. в разделе [Устранение неполадок с помощью пользовательских ролей](../role-based-access-control/troubleshooting.md#problems-with-custom-roles) и [пользовательские роли для ресурсов Azure](../role-based-access-control/custom-roles.md).
...
После отмены, выставление счетов прекращается немедленно. Но отображение отмены на портале может занять до 10 минут. Если вы отменяете подписку в середине расчетного периода, мы отправим вам итоговый счет в день расчета после истечения периода тарификации.
