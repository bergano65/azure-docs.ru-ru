...
1. В меню в верхней части окна портала Cloudyn выберите **Затраты** > **Анализ затрат** > **Actual Cost Over Time** (Фактические затраты за период времени). 
2. В разделе **Groups** (Группы) задайте **Service** (Служба), а в поле **Filter on the service** (Фильтр по службе) — **Azure/VM**. 
3. В верхнем правом углу отчета выберите **Действия**, а затем выберите **Планировать отчет**.
4. Чтобы отправить отчет по электронной почте с заданным интервалом, выберите вкладку **Планирование** в диалоговом окне отчета **Save or Schedule this** (Сохранение или создание расписания для отчета). Обязательно установите флажок **Send via email** (Отправить по электронной почте). Все теги, параметры группирования и фильтрации, которые вы использовали, включаются в отправляемый отчет. 
5. На вкладке **Пороговое значение** выберите **Actual Cost vs. Threshold** (Пороговое значение по фактическим затратам). 
