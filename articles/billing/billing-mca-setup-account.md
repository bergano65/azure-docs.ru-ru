...
2. Если кто-то другой подписывал это соглашение от имени вашей организации и у вас нет этого электронного сообщения, выполните вход по предоставленной ниже ссылке. Замените здесь заполнитель **enrollmentNumber** реальным номером Соглашения Enterprise, которое вы продлили.
...
3. Выберите администраторов предприятия, которым вы хотите отправить запрос.
   ...
   ![Снимок экрана с приглашением для администраторов предприятия](./media/billing-mca-setup-account/ea-mca-invite-admins.png)
...
Тот пользователь в вашей организации, который подписал Клиентское соглашение Майкрософт, автоматически назначается владельцем профиля выставления счетов. Обратитесь к нему с просьбой добавить вас в качестве владельца, чтобы можно было завершить настройку.  <!-- Todo Are there any next steps -->
...
1. Учетная запись выставления счетов используется для управления выставлением счетов по Клиентскому соглашению Майкрософт. Дополнительные сведения об учетной записи выставления счетов см. в [этой статье](billing-mca-overview.md#your-billing-account).
...
| --- | --- |
| **Администратор предприятия (только для чтения = нет)** | **— Владелец профиля выставления счетов** </br> Управление всеми данными в профиле выставления счетов </br> - **Владелец раздела счета для всех разделов счета** </br> Управление всеми данными в разделах счета |
| **Администратор предприятия (только для чтения = да)** | **— Читатель профиля выставления счетов** </br> — Просмотр всех данных в учетной записи выставления счетов</br>**— Читатель раздела счета для всех разделов счета**</br> — Просмотр всех данных в разделе счета|
...
При подписывании Клиентского соглашения Майкрософт для новой учетной записи выставления счетов выбирается клиент Azure Active Directory. Если у вашей организации нет клиентов, создается новый клиент. Клиент представляет организацию в каталоге Azure Active Directory. Глобальные администраторы клиентов в организации используют клиент для управления доступом к приложениям и данным в организации.
...
Завершите настройку учетной записи выставления счетов не позднее истечения срока действия Соглашения о регистрации для Соглашения Enterprise. Когда срок действия Соглашения о регистрации истечет, службы в подписках Azure будут работать без перерывов. Но за них будет взиматься плата по розничным тарифам.
...
3. Выберите **Начать переход** на последнем шаге настройки. После выбора действия "начать переход" выполняется следующее:
    ...
    ![Снимок экрана с мастером настройки](./media/billing-mca-setup-account/ea-mca-set-up-wizard.png)
    ...
    - Если у вас есть резервирования Azure, они будут перемещены в новую учетную запись с такими же скидками и сроками действия. Скидка на резервирование будет по-прежнему применяться во время перехода.

4. Вы можете отслеживать текущее состояние перехода на странице **Состояние перехода**.

   ![Снимок экрана с состоянием перехода](./media/billing-mca-setup-account/ea-mca-set-up-status.png)

## <a name="validate-billing-account-set-up"></a>Проверка настройки учетной записи выставления счетов
...
3. Выберите профиль выставления счетов. В зависимости от уровня вашего доступа вам, возможно, потребуется выбрать учетную запись выставления счетов. В учетной записи выставления счетов щелкните **Профили выставления счетов** и выберите нужный профиль выставления счетов.

4. Выберите **Подписки Azure** слева.

   ![Снимок экрана со списком подписок](./media/billing-mca-setup-account/billing-mca-subscriptions-post-transition.png)
...
3. Выберите раздел счета. В зависимости от уровня вашего доступа вам, возможно, потребуется выбрать учетную запись выставления счетов или профиль выставления счетов.  В учетной записи выставления счетов или в профиле выставления счетов щелкните **Разделы счета** и выберите нужный раздел счета.

    ![Снимок экрана со списком разделов счета после перехода](./media/billing-mca-setup-account/billing-mca-invoice-sections-post-transition.png)

4. В области слева выберите **Все продукты**.

5. Выполните поиск по слову **Зарезервировано**.

    ![Снимок экрана со списком подписок после перехода](./media/billing-mca-setup-account/billing-mca-azure-reservations-post-transition.png)

Резервирования Azure, которые были перенесены из Соглашения о регистрации для Соглашения Enterprise в новую учетную запись, отображаются на странице "Все продукты". Повторите эти шаги для всех разделов счета, чтобы проверить правильность переноса всех резервирований Azure из Соглашения о регистрации для Соглашения Enterprise. Если вы считаете, что какого-то резервирования Azure не хватает, [обратитесь в службу поддержки Azure](https://portal.azure.com/?#blade/Microsoft_Azure_Support/HelpAndSupportBlade), чтобы переместить недостающее резервирование в новую учетную запись выставления счетов.
...
3. Выберите профиль выставления счетов, созданный для Соглашения о регистрации. В зависимости от уровня вашего доступа вам, возможно, потребуется выбрать учетную запись выставления счетов.  В учетной записи выставления счетов щелкните **Профили выставления счетов** и выберите нужный профиль выставления счетов.

4. В области слева выберите **Управление доступом (IAM)** .

   ![Снимок экрана, демонстрирующий доступ администраторов предприятия после перехода](./media/billing-mca-setup-account/billing-mca-ea-admins-access-post-transition.png)
...
3. Выберите раздел счета. Разделы счета имеют те же имена, что и соответствующие подразделения в Соглашении о регистрации для Соглашения Enterprise. В зависимости от уровня доступа вам, возможно, придется выбрать учетную запись выставления счетов или профиль выставления счетов. В учетной записи выставления счетов или в профиле выставления счетов щелкните **Разделы счета** и выберите нужный раздел счета.

   ![Снимок экрана со списком разделов счета после перехода](./media/billing-mca-setup-account/billing-mca-invoice-sections-post-transition.png)

4. В области слева выберите **Управление доступом (IAM)** .

    ![Снимок экрана, демонстрирующий доступ администраторов подразделений и учетных записей после перехода](./media/billing-mca-setup-account/billing-mca-department-account-admins-access-post-transition.png)
