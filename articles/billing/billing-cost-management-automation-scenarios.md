...
-   [API RateCard](https://msdn.microsoft.com/library/azure/mt219005.aspx). API для получения тарифов по разным показателям для клиентов Web Direct. Полученные данные в сочетании со сведениями о потреблении ресурсов позволят вручную вычислить ожидаемые расходы. 

-   [API неоплачиваемого потребления](https://msdn.microsoft.com/library/azure/mt219003.aspx). Этот API позволяет получить необработанные сведения о потреблении перед применением алгоритмов Azure для оценки и выставления счетов.
...
- [API использования](https://msdn.microsoft.com/library/Mt219003.aspx) предоставляет необработанные сведения по потреблению Azure, прежде чем они будут обработаны системой измерения затрат Azure. Эти данные могут не иметь корреляции с показателями затрат или потребления, которые видны после системы измерений затрат Azure.
