...
| --- | --- | --- |
| Резервирование покупок | Доступные в этом представлении.<br>  Чтобы получить этот фильтр данных по ChargeType = &quot;покупки&quot;. <br> Ссылаться на ReservationID или ReservationName знать, какие резервирования, то плата взимается.  | Не применимо к этому представлению. <br> Затраты на покупку не поставляются в данных с поправкой на амортизацию. |
...
| Неиспользуемые резервирования (предоставляет количество часов, резервирование не был использован в день и денежное значение отходов) | Неприменимо в этом представлении. | Доступные в этом представлении.<br> Для получения этих данных, фильтрацию по ChargeType = &quot;UnusedReservation&quot;.<br>  Ссылаться на ReservationID или ReservationName, чтобы знать, какие резервирование было недостаточно. Это объем резервирования была напрасной в день.  |
...
| **Использование, получившее скидка на резервирование** | Замените {метрика} ActualCost<br>Замените {фильтр}: properties/reservationId%20ne%20 |
| **Не удалось получить скидки резервирования для использования** | Замените {метрика} ActualCost<br>Замените {фильтр}: properties/reservationId%20eq%20 |
| **Амортизированные расходы (использование и покупки)** | Замените {метрика} AmortizedCost |
| **Неиспользуемые резервирования отчета** | Замените {метрика} AmortizedCost<br>Замените {фильтр}: properties/ChargeType%20eq%20'UnusedReservation " |
| **Резервирование покупок** | Замените {метрика} с {фильтр} ActualCostReplace: properties/ChargeType%20eq%20'Purchase "  |
| **Возвраты платежей** | Замените {метрика} ActualCost<br>Замените {фильтр}: properties/ChargeType%20eq%20'Refund " |
