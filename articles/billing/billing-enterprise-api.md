...
|-|-|
|Authorization| Укажите значение в следующем формате: **bearer {API_KEY}** <br/> Пример: bearer eyr....09| 
...
* **Прейскурант**. [API прейскуранта](/rest/api/billing/enterprise/billing-enterprise-api-pricesheet) предоставляет соответствующий тариф для каждой метрики в отдельной регистрации и за определенный расчетный период.

* **Сведения о** зарезервированном экземпляре. [API использования](/rest/api/billing/enterprise/billing-enterprise-api-reserved-instance-usage) зарезервированного экземпляра возвращает данные об использовании зарезервированного экземпляра. В [API](/rest/api/billing/enterprise/billing-enterprise-api-reserved-instance-usage) -интерфейсе зарезервированных экземпляров отображаются созданные транзакции выставления счетов. 
...
|-|-|-|
|200| OK|Без ошибок|
|401| Неавторизованный| Ключ API не найден, недопустимый формат, срок действия истек и т. д.|
|404| Недоступно| Не найдена конечная точка отчетов|
|400| Неправильный запрос| Недопустимые параметры — диапазоны дат, числа EA и т. д.|
