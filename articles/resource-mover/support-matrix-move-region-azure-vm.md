...
Группы доступности | Поддерживается | Поддерживается.
...
Политики RBAC Azure | Не поддерживается | Политики управления доступом на основе ролей Azure (Azure RBAC) на виртуальных машинах не копируются на виртуальную машину в целевом регионе.
...
В этой таблице приведены сведения о поддержке диска Azure VM OS, диска данных и временного диска. Важно наблюдать за ограничениями дисков виртуальной машины и целевыми объектами для [управляемых дисков](../virtual-machines/disks-scalability-targets.md) , чтобы избежать проблем с производительностью.

> [!NOTE]
> Размер целевой виртуальной машины должен быть больше или равен исходной виртуальной машине. Параметры, используемые для проверки: количество дисков данных, число сетевых карт, количество доступных ЦП, объем памяти в ГБ. Если она сн'т, выдается ошибка.
...
Временный диск | Не поддерживается | Временный диск всегда исключается из процесса подготовки.<br/><br/> Не сохраняйте долговременные данные на временном диске. [Подробнее](../virtual-machines/managed-disks-overview.md#temporary-disk).
...
Виртуальные машины с включенным шифрованием дисков Azure | Поддерживается | [Дополнительные сведения](tutorial-move-region-encrypted-virtual-machines.md)
Виртуальные машины, использующие шифрование на стороне сервера с ключом, управляемым клиентом | Поддерживается | [Дополнительные сведения](tutorial-move-region-encrypted-virtual-machines.md)
...
Общедоступная подсистема балансировки нагрузки | Поддерживается | Укажите существующий ресурс в целевом регионе или создайте новый ресурс в процессе подготовки.  
Общедоступный IP-адрес | Поддерживается | Укажите существующий ресурс в целевом регионе или создайте новый ресурс в процессе подготовки.<br/><br/> Общедоступный IP-адрес зависит от региона и не будет храниться в целевом регионе после перемещения. Помните об этом при изменении параметров сети (включая правила балансировки нагрузки) в целевом расположении.
