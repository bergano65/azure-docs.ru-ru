...
Вывод на консоль на локальном прокси-сервере устройства, который подключается к управляющей программе SSH по адресу <code>IP_address:22</code>: ![Замещающий текст](./media/quickstart-device-streams-proxy-c/device-console-output.PNG "Вывод локального прокси-сервера устройства")
