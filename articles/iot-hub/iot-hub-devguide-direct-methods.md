...
* *Код состояния HTTP*:
  ...
  * 404 указывает, что идентификатор устройства является недопустимым или что устройство не было подключено к сети при вызове прямого метода, `connectTimeoutInSeconds` и в дальнейшем (используйте сопровождающее сообщение об ошибке, чтобы понять причину ошибки);
  * 504 означает, что время ожидания шлюза вызвано тем, что устройство не `responseTimeoutInSeconds`отвечает на вызов прямого метода в.
