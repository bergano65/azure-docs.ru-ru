...
* *Код состояния HTTP*:
  ...
  * 404 указывает, что идентификатор устройства является недопустимым или что устройство не было подключено к сети при вызове прямого метода, а для `connectTimeoutInSeconds` впоследствии (используйте сообщение об ошибке, чтобы разобраться в основной причине);
...
Сообщение AMQP приходит на ссылку приема, представляющую запрос метода. Содержание раздела.
