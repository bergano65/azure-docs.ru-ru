---
title: Заказать события подключения устройства fr Azure IoT Концентратор ж / Azure Космос DB
...
* Активная учетная запись Azure. Если у вас его нет, вы можете [создать бесплатную учетную запись.](https://azure.microsoft.com/pricing/free-trial/)
...
* Коллекция в базе данных. Пошаговое руководство см. в разделе [Добавление коллекции](../cosmos-db/create-sql-api-java.md#add-a-container). При создании коллекции `/id` используйте ключ раздела.
...
1. На [портале Azure](https://portal.azure.com)выберите **«Создайте ресурс»,** выберите **Интеграцию,** а затем **Logic App.**
...
3. Выберите **Создать**, чтобы создать приложение логики.
   ...
   > [!NOTE]
   > Чтобы снова найти и открыть логическое приложение, выберите **группы ресурсов** и выберите группу ресурсов, используемую для этого приложения. Затем выберите новое приложение логики. Это открывает логику App Designer.

4. В Logic App Designer прокрутите вправо, пока не увидите общие триггеры. Под **шаблонами**выберите **Blank Logic App,** чтобы вы могли создавать свое логическое приложение с нуля.
...
1. В разъемах и триггерах панели поиска, введите **HTTP** и нажмите Enter.
...
4. Вставьте пример кода JSON ниже в текстовое поле, а затем нажмите кнопку **Готово**:
   ...
   ![Вставить образец полезной нагрузки JSON](./media/iot-hub-how-to-order-connection-state-events/paste-sample-payload.png)
...
1. Выберите **новый шаг,** затем **встроенный,** затем найдите и выберите **Состояние.** Нажмите в **Выберите значение,** и появится окно, показывающее динамическое содержимое — поля, которые могут быть выбраны. Заполните поля, как показано ниже, чтобы выполнить это только для событий, связанных с устройством и отключенным устройством:

   * Выберите значение: **eventType** - выберите это из полей в динамическом содержимом, которое появляется при нажатии на это поле.
   * Изменение "равно" **к концу с**.
   * Выберите значение: **nected**.

     ![Заполнение условия](./media/iot-hub-how-to-order-connection-state-events/condition-detail.png)

2. В, **если верно** диалог, нажмите на **Добавить действие**.
...
3. Поиск Cosmos DB и выберите **Azure Cosmos DB - Выполнение процедуры хранения**

   ![Поиск Cosmos DB](./media/iot-hub-how-to-order-connection-state-events/cosmosDB-search.png)

4. Заполните **космосд-соединение** для **названия соединения** и выберите запись в таблице, а затем выберите **Создать.** Вы видите панель **процедуры выполнения.** Введите значения для полей:

   **Идентификатор базы данных**: Тодолист

   **Идентификатор коллекции**: Элементы

   **Sproc ID**: LatestDeviceConnectionState

5. Выберите **Добавить новый параметр**. В выпадающем виде, происля ягоды рядом с **ключом раздела** и **параметрами для сохраненной процедуры,** затем нажмите где-либо еще на экране; он добавляет поле для значения ключа раздела и поле для параметров для сохраненной процедуры.

   ![Заполнение действия приложения логики](./media/iot-hub-how-to-order-connection-state-events/logicapp-stored-procedure.png)

6. Теперь введите значение ключа раздела и параметры, указанные ниже. Обязательно положите в скобки и двойные цитаты, как показано на рисунке. Возможно, вам придется нажать **Добавить динамическое содержимое,** чтобы получить действительные значения, которые вы можете использовать здесь.

   ![Заполнение действия приложения логики](./media/iot-hub-how-to-order-connection-state-events/logicapp-stored-procedure-2.png)

7. В верхней части панели, где он говорит **для каждого**, под **Выберите выход из предыдущих шагов,** убедитесь, что **он тело** выбрано.

   ![заполнить логику приложение для каждого](./media/iot-hub-how-to-order-connection-state-events/logicapp-foreach-body.png)
...
3. Выберите **подписку на событие**.

   ![Создание подписки на события](./media/iot-hub-how-to-order-connection-state-events/event-subscription.png)

4. Заполните **сведения о подписке на события**: Предоставьте описательное имя и выберите **схему Event Grid.**

5. Заполните поля **типов событий.** В списке выпадающих данных выберите только **отключенные** устройства и **устройства** из меню. Нажмите в любом другом месте на экране, чтобы закрыть список и сохранить ваши выборы.

   ![Установка типов событий для поиска](./media/iot-hub-how-to-order-connection-state-events/set-event-types.png)

6. Для **получения деталей конечных точек**выберите тип endpoint как **Web Hook** и нажмите на выбранную конечную точку и вставьте URL-адрес, который вы скопировали из приложения логики, и подтвердите выбор.

   ![Выберите URL-адрес конечных точек](./media/iot-hub-how-to-order-connection-state-events/endpoint-select.png)

7. Теперь форма должна выглядеть так же, как и в следующем примере:
...
2. Выберите **«Добавить»** в верхней части панели.
...
5. Вы можете добавить несколько устройств с разными идентификаторами.
   ...
   ![Устройства, добавленные в концентратор](./media/iot-hub-how-to-order-connection-state-events/AddIoTDevice.png)

6. Нажмите на устройство еще раз; теперь строки соединения и ключи будут заполнены. Скопируйте значение из поля **Connection string -- primary key** (Строка подключения — первичный ключ) для последующего использования.
...
1. В области кодирования замените заполнитель в строке 15 строкой подключения концентратора Azure IoT, сохраненной в конце предыдущего раздела.

   ![Вставить в строке соединения устройства](./media/iot-hub-how-to-order-connection-state-events/raspconnstring.png)

2. Выполнить приложение, выбрав **Run**.

Вы видите нечто похожее на следующий вывод, который показывает данные датчика и сообщения, отправляемые в концентратор IoT.
