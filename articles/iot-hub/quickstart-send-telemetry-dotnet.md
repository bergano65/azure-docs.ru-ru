...
Примеры приложений, запускаемые в рамках этого краткого руководства, написаны на языке C#. На компьютере, на котором ведется разработка, необходимо установить пакет SDK для .NET Core версии 3.0 или более поздней.
...
> [!NOTE]
> Рекомендуется установить пакет SDK для .NET Core версии 3.0 или более поздней для компилирования кода службы Центра событий, используемого для чтения данных телеметрии в этом кратком руководстве. Можно использовать пакет SDK для .NET Core версии 2.1, если для кода службы установить языковую версию для предварительного просмотра, как указано в разделе [Чтение данных телеметрии из центра](#read-the-telemetry-from-your-hub).
...
2. Откройте файл **ReadDeviceToCloudMessages.cs** в любом текстовом редакторе. Обновите указанные ниже переменные и сохраните изменения в файле.
    ...
    | `EventHubName`                | Замените значение переменной записанным ранее путем, совместимым с Центрами событий. |
    | `IotHubSasKey`                | Замените значение переменной записанным ранее первичным ключом службы. |

    > [!NOTE]
    > Если используется пакет SDK для .NET Core 2.1, необходимо установить языковую версию для предварительного просмотра, чтобы скомпилировать код. Для этого откройте файл **read-d2c-messages.csproj** и задайте для параметра `<LangVersion>` значение `preview`.
...
4. Создайте и запустите внутреннее приложение, выполнив в окне терминала на локальном компьютере следующие команды:
    ...
    ![Запуск внутреннего приложения](media/quickstart-send-telemetry-dotnet/read-device-to-cloud.png)
