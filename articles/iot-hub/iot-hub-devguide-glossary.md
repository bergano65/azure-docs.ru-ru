...
description: Руководство для разработчиков - глоссарий, объясняющий некоторые общие термины, используемые в статьях Azure IoT Hub.
author: robinsh
manager: philmea
ms.author: robinsh
ms.service: iot-hub
...
## <a name="azure-iot-explorer"></a>Лазурный IoT Explorer

[Исследователь Azure IoT](https://github.com/Azure/azure-iot-explorer) используется для просмотра телеметрии, которую устройство отправляет, работает с свойствами устройств и командами вызова. Вы также можете использовать explorer для взаимодействия с устройствами и тестирования, а также для управления плагинами и воспроизведением устройств.
...
[Портал Microsoft Azure](https://portal.azure.com) — это центральное место, где можно предоставлять ресурсы Azure и управлять ими. Содержимое портала упорядочено с помощью _колонок_.
...
[Менеджер ресурсов Azure](../azure-resource-manager/management/overview.md) позволяет работать с ресурсами в вашем решении в группе. Вы можете развертывать, обновлять или удалять ресурсы решения в рамках одной скоординированной операции.

## <a name="azure-service-bus"></a>Служебная шина Azure

[Service Bus](../service-bus/index.md) обеспечивает связь с облачными сообщениями с корпоративными сообщениями и ретрансляционную связь, которая помогает подключать решения с облаком. В некоторых учебниках, посвященных Центру Интернета вещей, упоминаются [очереди](../service-bus-messaging/service-bus-messaging-overview.md) служебной шины.
...
Исследователь устройства был заменен [на Azure IoT Explorer,](https://github.com/Azure/azure-iot-explorer)который используется для просмотра телеметрии, которую устройство отправляет, работает с свойствами устройства и командами вызова. Вы также можете использовать explorer для взаимодействия с устройствами и тестирования, а также для управления плагинами и воспроизведением устройств.
...
Подготовка устройств — это процесс добавления [исходных данных устройства](#device-data) в магазины вашего решения. Чтобы включить новое устройство в концентратор, необходимо добавить идентификатор устройства и ключи в [реестр идентификации](#identity-registry)IoT Hub. В рамках процесса подготовки может потребоваться инициализировать данные устройства в других хранилищах решения.
...
Чтобы считывать сообщения от [устройства к облаку,](#device-to-cloud) отправленные в концентратор IoT, можно подключиться к конечной точке концентратора и использовать любой совместимый с событиями метод для чтения этих сообщений. Методы, совместимые с Центрами событий, предусматривают использование [пакетов SDK для Центров событий](../event-hubs/event-hubs-programming-guide.md) и службы [Azure Stream Analytics](../stream-analytics/stream-analytics-introduction.md).

## <a name="field-gateway"></a>Полевой шлюз

Полевой шлюз обеспечивает подключение устройств, которые не могут подключаться непосредственно к [Концентратору IoT](#iot-hub) и обычно развертываются локально с вашими устройствами. Для получения дополнительной [What is Azure IoT Hub?](about-iot-hub.md) информации см.
...
Задняя часть решения может использовать [задания](iot-hub-devguide-jobs.md) для планирования и отслеживания действий на наборе устройств, зарегистрированных в концентраторе IoT. Мероприятия включают обновление устройства близнецов [желаемых свойств,](#desired-properties)обновление устройства твин [теги](#tags), и ссылаясь [на прямые методы.](#direct-method) [Центр Интернета вещей](#iot-hub) также использует  для [импорта и экспорта](iot-hub-devguide-identity-registry.md#import-and-export-device-identities) из [реестра удостоверений](#identity-registry).
...
[МЗТТ](https://mqtt.org/) является одним из протоколов обмена сообщениями, которые [IoT Концентрподдерживает](#iot-hub) для общения с устройствами. Дополнительные сведения о протоколах обмена сообщениями, которые поддерживает Центр Интернета вещей, см. в статье [Отправка и получение сообщений в Центре Интернета вещей](iot-hub-devguide-messaging.md).

## <a name="operations-monitoring"></a>Мониторинг операций

[Мониторинг операций](iot-hub-operations-monitoring.md) IoT Hub позволяет отслеживать состояние операций в вашем концентраторе IoT в режиме реального времени. [IoT Концентратор](#iot-hub) отслеживает события по нескольким категориям операций. Вы можете выбрать отправку событий из одной или нескольких категорий в конечную точку Центра Интернета вещей для обработки. Вы можете отслеживать данные на наличие ошибок или настроить более сложную обработку на основе закономерностей в данных.
...
Шлюз протокола обычно развертывается в облаке и предоставляет услуги по переводу протоколов для устройств, подключенных к [Концентратору IoT.](#iot-hub) Для получения дополнительной [What is Azure IoT Hub?](about-iot-hub.md) информации см.
