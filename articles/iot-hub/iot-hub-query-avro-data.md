...
ms.locfileid: "66257740"
---
# <a name="query-avro-data-by-using-azure-data-lake-analytics"></a>Запрос данных Avro с помощью Azure Data Lake Analytics

В этой статье показано, как запросить данные Avro, чтобы повысить эффективность маршрутизации сообщений из Центра Интернета вещей Azure в службы Azure. [Маршрутизация сообщений](iot-hub-devguide-messages-d2c.md) позволяет фильтровать данные с помощью сложных запросов на основе свойства сообщения, текста сообщения, тегов двойников устройств и свойств двойников устройств. Дополнительные сведения о возможности выполнения запросов в маршрутизации сообщений, см. в статье [сообщений синтаксисе запросов маршрутизации](iot-hub-devguide-routing-query-syntax.md).

Проблемой было, что при центра Интернета вещей Azure направляет сообщения в хранилище BLOB-объектов Azure, по умолчанию центр Интернета вещей записывает содержимое в формате Avro, который имеет свойство тело сообщения и свойство сообщения. Формат Avro не используется для любых конечных точек. Хотя формат Avro прекрасно подходит для хранения данных и сообщений, использовать его для опроса данных сложно. в отличие от формата JSON или CSV. Центр Интернета вещей теперь поддерживает запись данных в хранилище BLOB-объектов в JSON, а также AVRO.

Дополнительные сведения см. в разделе [с помощью хранилища BLOB-объектов Azure как конечную точку маршрута](iot-hub-devguide-messages-d2c.md#azure-blob-storage).
