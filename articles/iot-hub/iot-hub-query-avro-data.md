...
ms.locfileid: "73605615"
---
# <a name="query-avro-data-by-using-azure-data-lake-analytics"></a>Запрос данных Avro с помощью Azure Data Lake Analytics

В этой статье показано, как запросить данные Avro, чтобы повысить эффективность маршрутизации сообщений из Центра Интернета вещей Azure в службы Azure. [Маршрутизация сообщений](iot-hub-devguide-messages-d2c.md) позволяет фильтровать данные с помощью сложных запросов на основе свойства сообщения, текста сообщения, тегов двойников устройств и свойств двойников устройств. Чтобы узнать больше о возможностях запроса в реутивации сообщений, смотрите статью о [синтаксисе запроса на запрос сообщения.](iot-hub-devguide-routing-query-syntax.md)

Проблема заключается в том, что, когда Azure IoT Hub направляет сообщения в хранилище Azure Blob, по умолчанию IoT Hub записывает содержимое в формате Avro, в котором есть как свойство тела сообщений, так и свойство сообщений. Формат Avro не используется для других конечных точек. Хотя формат Avro прекрасно подходит для хранения данных и сообщений, использовать его для опроса данных сложно. в отличие от формата JSON или CSV. IoT Hub теперь поддерживает написание данных для хранения Blob в JSON, а также AVRO.

Для получения дополнительной информации [см. Использование хранилища Azure в качестве конечной точки для разгрома.](iot-hub-devguide-messages-d2c.md#azure-storage)
