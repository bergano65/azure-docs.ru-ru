...
1. Отключите доступ к общедоступной сети с помощью IP-фильтра, чтобы заблокировать все IP-адреса.
1. Отключите встроенную конечную точку концентратора событий, [настроив маршрутизацию, исключающую отправку данных на нее](iot-hub-devguide-messages-d2c.md).
...
6. На появившейся странице укажите имя конечной точки, выберите контейнер, который вы хотите использовать в хранилище BLOB-объектов, укажите кодировку и формат имени файла. Выберите **Назначено системой** в качестве **типа проверки подлинности** для конечной точки хранилища. Нажмите кнопку **Создать** .
...
Теперь вы можете использовать REST API службы Интернета вещей Azure для [создания заданий импорта/экспорта](https://docs.microsoft.com/rest/api/iothub/service/jobclient/getimportexportjobs) для получения сведений об использовании функций массового импорта и экспорта. В тексте запроса необходимо указать `storageAuthenticationType="identityBased"` и использовать `inputBlobContainerUri="https://..."` и `outputBlobContainerUri="https://..."` в качестве входного и выходного URL-адресов учетной записи хранения, соответственно.
