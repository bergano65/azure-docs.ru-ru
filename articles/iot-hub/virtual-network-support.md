...
> [!NOTE]
> Функции центра Интернета вещей, описанные в этой статье, в настоящее время доступны для центра Интернета вещей, созданного в следующих регионах: Восточная часть США, Юго-Центральный регион США и Западная часть США 2.
...
Ниже приведены необходимые условия.
...
* Центру Интернета вещей необходимо назначить управляемое удостоверение службы во время подготовки концентратора. Следуйте инструкциям по [созданию концентратора с управляемым удостоверением службы](#create-a-hub-with-managed-service-identity).
...
### <a name="create-a-hub-with-managed-service-identity"></a>Создание концентратора с управляемым удостоверением службы

Управляемое удостоверение службы может быть назначено вашему центру во время подготовки ресурсов (в настоящее время эта функция не поддерживается для существующих концентраторов). Для этой цели необходимо использовать шаблон ресурсов ARM ниже:
...
После замены значений для `name`ресурсов, `location`, `SKU.name` и `SKU.tier`можно использовать Azure CLI для развертывания ресурса в существующей группе ресурсов с помощью:
...
Теперь вы можете использовать REST API Интернета вещей Azure для [создания заданий импорта](https://docs.microsoft.com/rest/api/iothub/jobclient/getimportexportjobs) и получения сведений об использовании функций полного импорта и экспорта. Обратите внимание, что в тексте запроса необходимо указать `storageAuthenticationType="identityBased"` и использовать `inputBlobContainerUri="https://..."` и `outputBlobContainerUri="https://..."` в качестве URL-адреса ввода и вывода учетной записи хранения соответственно.


Пакет SDK для центра Интернета вещей Azure также поддерживает эту функцию в диспетчере реестра клиента службы. В следующем фрагменте кода показано, как инициировать задание импорта или задание экспорта в с помощью C# пакета SDK.
...
Чтобы использовать эту версию пакетов SDK для Azure IoT с ограниченным регионом с поддержкой C#виртуальной сети для, Java и Node. js:

1. Создайте переменную среды с именем `EnableStorageIdentity` и присвойте ей значение `1`.

2. Скачайте пакет SDK:
...
* [Отправка файла](./iot-hub-devguide-file-upload.md)
