...
* **Получение двойника устройства{b> <b}по идентификатору**. Эта операция возвращает документ двойника устройства, включая теги, а также требуемые и сообщаемые системные свойства.
...
* Все ключи в объектах JSON кодируются в кодировке UTF-8, с учетом регистра и до 1 КБ в длину. Разрешено использовать все знаки, кроме управляющих знаков Юникода (сегменты C0 и C1), а также `.`, `$` и SP.
...
* Все объекты JSON в тегах, требуемых и сообщаемых свойствах могут иметь максимальную глубину 10. Например, допустим следующий объект:
