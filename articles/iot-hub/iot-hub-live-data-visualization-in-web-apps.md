...
2. Теперь подготавливайте веб-приложение в плане службы приложений. `--deployment-local-git` Параметр позволяет отправлять и развертывать код веб-приложения из репозитория Git на локальном компьютере. Имя веб-приложения должно быть глобально уникальным и может содержать буквы верхнего и нижнего регистра, цифры и дефисы. Убедитесь, что для `--runtime` параметра указана версия node 10,6 или более поздняя, в зависимости от используемой версии среды выполнения Node. js. Для получения списка поддерживаемых `az webapp list-runtimes` сред выполнения можно использовать команду.
...
* Из веб-приложения в портал Azure в разделе **средства разработки** выберите **консоль** и проверьте версии node и NPM с `node -v` помощью `npm -v`и.

* Если появится сообщение об ошибке "не удается найти пакет", то вы могли выполнить шаги в неопределенном порядке. При развертывании сайта (с `git push`) запускается `npm install`служба приложений, которая выполняется на основе текущей установленной версии узла. Если это позднее изменилось в конфигурации, необходимо внести небессмысленные изменения в код и отправить его снова.
