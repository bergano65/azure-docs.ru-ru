...
1. Запустите приложение имитированного устройства, выполнив в окне терминала на локальном компьютере следующие команды:
    ...
    ![Запуск виртуального устройства](media/quickstart-send-telemetry-python/simulated-device.png)
...
> [!NOTE]
> На следующих шагах используется синхронная выборка **read_device_to_cloud_messages_sync.py**. Те же действия можно выполнить с использованием асинхронной выборки **read_device_to_cloud_messages_async.py**.

1. В другом окне терминала на локальном компьютере перейдите в корневую папку примера проекта Python. Затем перейдите в папку **iot-hub\Quickstarts\read-d2c-messages**.

2. Откройте файл **read_device_to_cloud_messages_sync.py** в любом текстовом редакторе. Обновите указанные ниже переменные и сохраните изменения в файле.

    | Переменная | Значение |
    | -------- | ----------- |
    | `EVENTHUB_COMPATIBLE_ENDPOINT` | Замените значение переменной записанной ранее конечной точкой, совместимой с Центрами событий. |
    | `EVENTHUB_COMPATIBLE_PATH`     | Замените значение переменной записанным ранее путем, совместимым с Центрами событий. |
    | `IOTHUB_SAS_KEY`                | Замените значение переменной записанным ранее первичным ключом службы. |
...
4. Создайте и запустите внутреннее приложение, выполнив в окне терминала на локальном компьютере следующие команды:
    ...
    ![Запуск внутреннего приложения](media/quickstart-send-telemetry-python/read-device-to-cloud.png)
