...
1. Добавьте следующие поля в класс **Program** . Замените значение `{iot hub connection string}` заполнителя строкой подключения центра Интернета вещей, скопированным ранее в поле [Получение строки подключения для центра Интернета вещей](#get-the-iot-hub-connection-string).
...
1. Нажмите кнопку **Обзор**, найдите и выберите **Microsoft. Azure. Devices. Client**. Выберите пункт **Установить**.
...
1. Добавьте следующие поля в класс **Program** . Замените значение `{device connection string}` заполнителя строкой подключения устройства, записанной ранее в поле [Регистрация нового устройства в центре Интернета вещей](#register-a-new-device-in-the-iot-hub).
...
> [!NOTE]
> Для простоты в этом руководстве не реализуются политики повтора. В рабочем коде следует реализовать политики повтора (например, экспоненциальную [отработку отказа](/azure/architecture/best-practices/transient-faults)), как это было предложено при обработке временных сбоев.
...
1. После подключения устройства и ожидания вызова метода щелкните правой кнопкой мыши проект **TriggerReboot** , выберите пункт **Отладка**, а затем выберите пункт **запустить новый экземпляр**.
