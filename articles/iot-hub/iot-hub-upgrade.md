...
* Измените размер Центра Интернета вещей. Например, перейти от уровня B1 к уровню B2, чтобы увеличить количество сообщений, которые каждая единица может поддерживать в день.

* Перейдите на более высокий уровень. Например, перейти с уровня B1 на уровень S1 для доступа к расширенным функциям с той же емкостью обмена сообщениями.

Все эти изменения можно вносить, не прерывая выполняющиеся операции.

Если вы хотите понизить свой концентратор IoT, вы можете удалить единицы и уменьшить размер концентратора IoT, но вы не можете понизить до более низкого уровня. Например, можно перейти с уровня S2 на уровень S1, но не с уровня S2 на уровень B1. Только один тип [издания Iot Hub](https://azure.microsoft.com/pricing/details/iot-hub/) в пределах уровня может быть выбран в IoT Hub. Например, можно создать центр Интернета вещей с несколькими единицами уровня S1, но не с комбинацией единиц из разных выпусков, например S1 и B3 или S1 и S2.

Эти примеры помогут понять, как настраивать Центр Интернета вещей при изменениях решения. Для получения конкретной информации о возможностях каждого уровня всегда следует ссылаться на [цены Azure IoT Hub.](https://azure.microsoft.com/pricing/details/iot-hub/)
...
Максимальный предел раздела для базового уровня IoT Hub и стандартного уровня IoT Hub составляет 32. Для большинства центров Интернета вещей достаточно четырех секций. Ограничение на количество секций устанавливается при создании центра Интернета вещей. Оно привязывает сообщения, отправляемые с устройства в облако, к числу одновременно работающих модулей чтения этих сообщений. Это значение сохраняется при переходе с уровня "Базовый" на уровень "Стандартный".
