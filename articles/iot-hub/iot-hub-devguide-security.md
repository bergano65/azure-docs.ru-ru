...
Вы можете выдавать [разрешения](#iot-hub-permissions) следующим образом:
...
> [!NOTE]
> Безопасность для поставщика ресурсов Центра Интернета вещей Azure обеспечивается с помощью подписки Azure (это касается всех поставщиков в [Azure Resource Manager](../azure-resource-manager/management/overview.md)).
...
При использовании [AMQP](https://www.amqp.org/) Центр Интернета вещей поддерживает механизм [SASL PLAIN](https://tools.ietf.org/html/rfc4616) и стандарт [защиты AMQP на основе утверждений](https://www.oasis-open.org/committees/download.php/50506/amqp-cbs-v1%200-wd02%202013-08-12.doc).
...
Пароль (Вы можете создать токен SAS с командой расширения CLI [az iot, генерировать токен sas-sas,](/cli/azure/ext/azure-cli-iot-ext/iot/hub?view=azure-cli-latest#ext-azure-cli-iot-ext-az-iot-hub-generate-sas-token)или [инструменты Azure IoT для визуального кода studio):](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-tools)
...
Центр Интернета вещей использует маркеры безопасности для проверки подлинности устройств и служб, чтобы избежать отправки ключей по сети. Кроме того, маркеры безопасности ограничены по времени и области действия. [SDK Azure IoT](iot-hub-devguide-sdks.md) автоматически генерируют токены, не требуя специальной конфигурации. В некоторых случаях требуется напрямую создать и использовать маркеры безопасности. Ниже приведены соответствующие сценарии.
...
IoT Hub также позволяет устройствам аутентифицировать подлинность с помощью IoT Hub с помощью [сертификатов X.509.](iot-hub-devguide-security.md#supported-x509-certificates)
...
Ниже приведены инструкции по установке для предварительных условий.
...
> [!NOTE]
> Можно создать токен SAS с командой расширения CLI [az iot, генерируемой токеном sas-sas-token,](/cli/azure/ext/azure-cli-iot-ext/iot/hub?view=azure-cli-latest#ext-azure-cli-iot-ext-az-iot-hub-generate-sas-token)или [инструментами Azure IoT для Visual Studio Code.](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-tools)
...
* [службы маркеров](iot-hub-devguide-security.md#custom-device-and-module-authentication), используемые для реализации настраиваемых схем аутентификации.
...
Вы можете выполнить аутентификацию для устройства в Центре Интернета вещей при помощи любого сертификата X.509. Для этого отправьте отпечаток сертификата или данные центра сертификации (ЦС) в Центр Интернета вещей. Аутентификация с помощью отпечатков пальцев сертификата проверяет, что представленный отпечаток пальца соответствует настроенной отпечатку пальца. При аутентификации с использованием центра сертификации проверяется цепочка сертификатов. В любом случае, рукопожатие TLS требует, чтобы устройство было иметь действительный сертификат и закрытый ключ. Обратитесь к спецификации TLS для деталей, например: [RFC 5246 - Протокол протокола 1.2 протокола транспортного уровня (TLS).](https://tools.ietf.org/html/rfc5246/)
...
* **Самостоятельно сформированный и самозаверяющий сертификат X-509**. Производитель устройства или внутренний специалист по развертыванию может создать эти сертификаты и сохранить соответствующий закрытый ключ (и сертификат) на устройстве. Для этого вы можете использовать такие инструменты, как [OpenSSL](https://www.openssl.org/) и служебная программа [Windows SelfSignedCertificate](/powershell/module/pkiclient/new-selfsignedcertificate). Работает только с аутентификацией при помощи отпечатков. 
...
[Пакет SDK службы Интенета вещей Azure для C#](https://github.com/Azure/azure-iot-sdk-csharp/tree/master/iothub/service) (версия 1.0.8+) поддерживает регистрацию устройства, которое использует сертификат X.509 для аутентификации. Другие интерфейсы API (например, импорт и экспорт устройств) также поддерживают сертификаты X.509.
...
[Пакет SDK для устройств Azure IoT для .NET](https://github.com/Azure/azure-iot-sdk-csharp/tree/master/iothub/device) (версия 1.0.11+) поддерживает использование сертификатов X.509.
...
[Реестр удостоверений](iot-hub-devguide-identity-registry.md) Центра Интернета вещей позволяет настроить контроль доступа и учетные данные безопасности для каждого устройства и (или) модуля с помощью [маркеров](iot-hub-devguide-security.md#security-tokens). Если в решении Интернета вещей уже есть настраиваемый реестр удостоверений и (или) схема аутентификации, то рекомендуем создать *службу маркеров*, чтобы интегрировать эту инфраструктуру с Центром Интернета вещей. Таким образом можно использовать и другие функции IoT в решении.
...
* В статье [Справочник — язык запросов Центра Интернета вещей для двойников устройств, заданий и маршрутизации сообщений](iot-hub-devguide-query-language.md) описывается язык запросов, который можно использовать для получения сведений о двойниках устройств и заданиях из Центра Интернета вещей.
...
* [RFC 5246 - Версия протокола протокола 1.2 протокола «Транспортный слой» (TLS)](https://tools.ietf.org/html/rfc5246/) содержит дополнительную информацию об аутентификации TLS.
...
* [Основные сведения о двойниках устройств (предварительная версия)](iot-hub-devguide-device-twins.md)
* [Вызов прямого метода на устройстве (предварительная версия)](iot-hub-devguide-direct-methods.md)
...
* [Начало работы с концентратором Azure IoT](quickstart-send-telemetry-node.md)
