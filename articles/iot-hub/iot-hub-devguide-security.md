...
Пароль (можно создать маркер SAS с помощью команды расширения интерфейса командной строки [az iot hub generate-sas-token](/cli/azure/ext/azure-iot/iot/hub#ext-azure-iot-az-iot-hub-generate-sas-token) или [расширения "Средства Интернета вещей Azure" для Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-tools)).
...
> [!NOTE]
> Можно создать маркер SAS с помощью команды расширения интерфейса командной строки [az iot hub generate-sas-token](/cli/azure/ext/azure-iot/iot/hub#ext-azure-iot-az-iot-hub-generate-sas-token) или [расширения "Средства Интернета вещей Azure" для Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-tools).
...
Для аутентификации устройство может использовать только один из вариантов: либо сертификат X.509, либо маркер безопасности. При использовании проверки подлинности на сертификате X. 509 убедитесь, что у вас есть стратегия по обработке смены сертификатов при истечении срока действия существующего сертификата.
...
[Пакет SDK службы Интернета вещей Azure для C#](https://github.com/Azure/azure-iot-sdk-csharp/tree/master/iothub/service) (версия 1.0.8+) поддерживает регистрацию устройства, которое использует сертификат X.509 для проверки подлинности. Другие интерфейсы API (например, импорт и экспорт устройств) также поддерживают сертификаты X.509.

Для настройки сертификатов X.509 для устройств можно также использовать команду интерфейса командной строки расширения [az iot hub device-identity](/cli/azure/ext/azure-iot/iot/hub/device-identity).
...
* [RFC 5246 — протокол TLS версии 1.2](https://tools.ietf.org/html/rfc5246/) предоставляет дополнительные сведения о проверке подлинности TLS.
