...
1. Создайте пустую папку под названием **dm-get-started.**

2. В папке **dm-get-started** создайте проект Maven под названием **триггер-перезагрузка,** используя следующую команду в запросе команды:

    ```cmd/sh
    mvn archetype:generate -DgroupId=com.mycompany.app -DartifactId=trigger-reboot -DarchetypeArtifactId=maven-archetype-quickstart -DinteractiveMode=false
    ```

3. В запросе команды перейдите к папке **перезагрузки триггера.**

4. Используя текстовый редактор, откройте файл **pom.xml** в папке **триггер-перезагрузки** и добавьте следующую зависимость в узло **зависимых.** Эта зависимость позволит вам использовать в приложении пакет iot-service-client для обмена данными с Центром Интернета вещей:
...
7. Используя текстовый редактор, откройте **триггер-перезагрузку- src-main-java-com-mycompany-App.java** исходный файл.
...
9. Добавьте следующие переменные уровня класса в класс **Приложения.** Заменить `{youriothubconnectionstring}` строку подключения Концентратора IoT, скопированную ранее в [строке подключения концентратора IoT:](#get-the-iot-hub-connection-string)
...
12. Чтобы вызвать метод перезагрузки прямой на смоделированном устройстве, замените код в **основном** методе следующим кодом:
...
15. Сохраните и закройте **триггер-перезагрузку- src-main-java-com-mycompany-app.java** файл.

16. Выполните сборку внутреннего приложения **trigger-reboot** и исправьте ошибки (при наличии). В запросе команды перейдите к папке **перезагрузки триггера** и запустите следующую команду:
...
1. В папке **dm-get-started** создайте проект Maven, **называемый смоделированным устройством,** используя следующую команду в вашей команде:
...
3. Используя текстовый редактор, откройте файл **pom.xml** в папке **с моделируемым устройством** и добавьте следующую зависимость в узло **зависимостей.** Эта зависимость позволит вам использовать в приложении пакет iot-service-client для обмена данными с Центром Интернета вещей:
...
7. Используя текстовый редактор, откройте смоделированное устройство, которое является основным файлом **смоделированного устройства.**
...
9. Добавьте следующие переменные уровня класса в класс **Приложения.** Заменить `{yourdeviceconnectionstring}` строку подключения устройства, отмеченную в [Регистре, новое устройство в разделе концентратора IoT:](#register-a-new-device-in-the-iot-hub)
...
14. Чтобы реализовать прямой метод на устройстве, добавьте указанный ниже вложенный класс в класс **App**. Когда смоделированное приложение получает вызов непосредственного метода **перезагрузки,** оно возвращает подтверждение вызывающему абоненту, а затем запускает поток для обработки перезагрузки:
...
16. Чтобы мгновенно уотить **DeviceClient,** замените код в **основном** методе следующим кодом:
...
20. Создайте приложение **simulated-device** и исправьте все ошибки. В запросе команды перейдите к папке **с моделируемым устройствами** и запустите следующую команду:
...
1. В запросе команды в папке **с моделируемым устройством** запустите следующую команду, чтобы начать прослушивание вызовов метода перезагрузки из концентратора IoT:
...
2. В запросе команды в папке **перезагрузки триггера** выполнить следующую команду, чтобы вызвать метод перезагрузки на смоделированном устройстве из концентратора IoT:
