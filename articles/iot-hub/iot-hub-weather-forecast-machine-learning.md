...
1. Нажмите кнопку **выполнить** , чтобы проверить шаги модели. Выполнение этого шага может занять несколько минут.

   ![Запустите эксперимент, чтобы проверить шаги.](media/iot-hub-weather-forecast-machine-learning/run-experiment.png)

1. Щелкните **настроить** > **прогнозную веб**-службу. Откроется схема прогнозного эксперимента.
...
1. В [портал Azure](https://portal.azure.com/)щелкните **создать ресурс** > **"Интернет вещей"** > **Stream Analytics задание**.
...
В Stream Analytics задание щелкните **запустить** > **сейчас** > **запустить**. После успешного запуска состояние задания **Остановлено** изменится на **Выполняется**.
