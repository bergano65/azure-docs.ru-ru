...
* *Abandon* Отмените сообщение, что приведет к тому, что центр Интернета вещей поместит сообщение обратно в очередь с состоянием, установленным в *очередь*. Устройства, подключающиеся по протоколу MQTT, не могут отменять сообщения, передаваемых из облака на устройство.
...
| Негативная | Если сообщение, переданное из облака на устройство, достигает *недоставленного* состояния, центр Интернета вещей создает обратное сообщение. |
...
| StatusCode         | Обязательная строка, используемая в сообщениях отзыва, созданных центром Интернета вещей: <br/> *Success* <br/> *Срок действия истек* <br/> *деливерикаунтексцеедед* <br/> *Отклонено* <br/> *Очистке* |
...
* **Azure CLI**: воспользуйтесь командой [AZ IOT центра обновления](https://docs.microsoft.com/cli/azure/iot/hub?view=azure-cli-latest#az-iot-hub-update) :
