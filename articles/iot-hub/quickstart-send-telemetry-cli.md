...
*  Выполните команду [az extension add](/cli/azure/extension?view=azure-cli-latest#az-extension-add&preserve-view=true), чтобы добавить Расширение Интернета вещей Microsoft Azure для Azure CLI в оболочку CLI. Расширение Интернета вещей добавляет в Azure CLI специальные команды Центра Интернета вещей, IoT Edge и службы подготовки устройств Интернета вещей (DPS).
...
1. Выполните команду [az group create](/cli/azure/group?view=azure-cli-latest#az-group-create&preserve-view=true), чтобы создать группу ресурсов. В следующей команде создается группа ресурсов с именем *MyResourceGroup* в расположении *eastus*. 

    ```azurecli
    az group create --name MyResourceGroup --location eastus
    ```

1. Создайте Центр Интернета вещей с помощью команды [az iot hub create](/cli/azure/iot/hub?view=azure-cli-latest#az-iot-hub-create&preserve-view=true). Создание Центра Интернета вещей может занять несколько минут. 
...
1. В первом сеансе CLI выполните команду [az iot hub device-identity create](/cli/azure/ext/azure-iot/iot/hub/device-identity?view=azure-cli-latest#ext-azure-iot-az-iot-hub-device-identity-create&preserve-view=true). При этом создается удостоверение имитированного устройства. 
...
1. Во втором сеансе CLI выполните команду [az iot hub monitor-events](/cli/azure/ext/azure-iot/iot/hub?view=azure-cli-latest#ext-azure-iot-az-iot-hub-monitor-events&preserve-view=true). Начнется мониторинг имитированного устройства. В выходных данных отображаются данные телеметрии, которые имитированное устройство отправляет в центр Интернета вещей.
...
1. Во втором сеансе CLI выполните команду [az iot device c2d-message send](/cli/azure/ext/azure-iot/iot/device/c2d-message?view=azure-cli-latest#ext-azure-iot-az-iot-device-c2d-message-send&preserve-view=true). С ее помощью отправляется сообщение из центра Интернета вещей в виртуальное устройство. Сообщение содержит строку и две пары "ключ-значение".  
...
1. Выполните команду [az group delete](/cli/azure/group?view=azure-cli-latest#az-group-delete&preserve-view=true). При этом будут удалены созданные группа ресурсов, центр Интернета вещей и регистрация устройства.

    ```azurecli
    az group delete --name MyResourceGroup
    ```
1. Выполните команду [az group list](/cli/azure/group?view=azure-cli-latest#az-group-list&preserve-view=true), чтобы подтвердить удаление группы ресурсов.  
