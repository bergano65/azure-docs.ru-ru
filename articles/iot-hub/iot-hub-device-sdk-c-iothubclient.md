...
При вызове **IoTHubClient\_CreateFromConnectionString** базовые библиотеки создают новый поток, выполняемый в фоновом режиме. Этот поток отправляет события в Центр Интернета вещей и принимает сообщения из него. При работе с интерфейсами API **LL** такой поток не создается. Создание фонового потока — это удобный инструмент для разработчиков. Вам не нужно беспокоиться о явной отправке событий и получении сообщений из центра Интернета вещей — это происходит автоматически в фоновом режиме. В отличие от этого, API-интерфейсы **LL** предоставляют явный контроль над связью с центром Интернета вещей, если это необходимо.
...
API-интерфейсы **LL** не создают фоновый поток. Вместо этого для явной отправки и получения данных из Центра Интернета вещей необходимо вызывать новый интерфейс API. Это продемонстрировано в следующем примере.

** \_ \_ Пример \_ HTTP-приложения iothub Client** , который входит в состав пакета SDK, демонстрирует интерфейсы API нижнего уровня. В этом примере мы отправляем события в Центр Интернета вещей с помощью кода, аналогичного приведенному ниже.
...
Причина, по которой **IoTHubClient \_ LL \_ DoWork** часто вызывается в цикле, заключается в том, что каждый раз, когда он вызывается, он отправляет *некоторые* буферизованные события в центр Интернета вещей и извлекает *следующее* сообщение, поставленное в очередь для устройства. При каждом вызове не гарантируется отправка всех буферизованных событий или получение всех сообщений в очереди. Если вам нужно отправить все события из буфера, а затем заняться другой обработкой, замените этот цикл следующим кодом.
...
Если сообщение отклонено, оно также отправляется в Центр Интернета вещей, но при этом указывается, что сообщение следует доставить повторно. Как правило, сообщение будет отменяться, если возникла ошибка, но вы хотите повторить попытку обработки сообщения. В отличие от этого, отклонение сообщения уместно при возникновении неустранимой ошибки (или если вы просто решите, что не нужно обрабатывать сообщение).
...
В этой строке указаны четыре значения: имя центра Интернета вещей, суффикс центра IoT, идентификатор устройства и ключ общего доступа. При создании экземпляра центра IoT на портале Azure вы получаете полное доменное имя (FQDN) центра IoT. Из этого имени мы получаем имя центра IoT (первая часть FQDN) и суффикс центра IoT (остальная часть FQDN). При регистрации устройства в центре Интернета вещей вы получаете идентификатор устройства и общий ключ доступа (как описано в [предыдущей статье](iot-hub-device-sdk-c-intro.md)).
...
* **SetBatching** (логическое значение). Если значение равно **true**, то данные, отправляемые в Центр Интернета вещей, передаются пакетами. При значении **false** сообщения отправляются по отдельности. Значение по умолчанию — **false**. Поддержка пакетной обработки через AMQP/AMQP-WS, а также добавление системных свойств в сообщениях D2C поддерживается.
