...
* **SetBatching** (логическое значение). Если значение равно **true**, то данные, отправляемые в Центр Интернета вещей, передаются пакетами. При значении **false** сообщения отправляются по отдельности. Значение по умолчанию — **false**. Пакетная обработка по протоколу AMQP / AMQP WS, а также добавление свойств системы сообщений D2C поддерживается.
