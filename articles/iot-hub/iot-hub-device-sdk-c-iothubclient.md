...
В имени API всех этих функций содержатся символы **LL**. Другая часть **LL** имени, параметры каждой из этих функций идентичны с их не-LL-аналогами. Однако в работе этих функций есть одна важная особенность.

При вызове **IoTHubClient\_CreateFromConnectionString** базовые библиотеки создают новый поток, выполняемый в фоновом режиме. Этот поток отправляет события в Центр Интернета вещей и принимает сообщения из него. При работе с интерфейсами API **LL** такой поток не создается. Создание фонового потока — это удобный инструмент для разработчиков. Вам не нужно беспокоиться о явной отправке событий и получении сообщений от IoT Hub - это происходит автоматически в фоновом режиме. В отличие от этого, **APIs LL** дают вам явный контроль над коммуникацией с IoT Hub, если это необходимо.
...
AA-разносчивки **LL** не создают фонового потока. Вместо этого для явной отправки и получения данных из Центра Интернета вещей необходимо вызывать новый интерфейс API. Это продемонстрировано в следующем примере.

**Выборка\_клиента\_\_iothub http,включенная** в SDK демонстрирует AI более низкого уровня. В этом примере мы отправляем события в Центр Интернета вещей с помощью кода, аналогичного приведенному ниже.
...
Причина, по которой **IoTHubClient\_\_LL DoWork** часто вызывается в цикле, заключается в том, что каждый раз, когда он вызывается, он отправляет *некоторые* буферизированные события в Концентратор IoT и получает *следующее* сообщение, составленное в очереди для устройства. Каждый вызов не гарантирует отправку всех буферизированных событий или для извлечения всех сообщений в очереди. Если вам нужно отправить все события из буфера, а затем заняться другой обработкой, замените этот цикл следующим кодом.
...
Этот код вызывает **IoTHubClient\_LL\_DoWork** до тех пор, пока все события из буфера не будут отправлены в Центр Интернета вещей. Обратите внимание, что это не означает, что все сообщения из очереди получены. Одной из причин этого является то, что проверка "всех" сообщений не является детерминированной действия. Что происходит при извлечении «всех» сообщений и отправке другого сообщения на устройство непосредственно после этого? Лучше такие моменты решать через запрограммированное время ожидания. Например, функция обратного вызова сообщения может сбросить таймер при каждом вызове. Затем можно написать логику для продолжения обработки, если, например, ни одно сообщение не было получено за последние *X* секунд.

Когда вы закончите входить в события и получать сообщения, обязательно позвоните в соответствующую функцию для очистки ресурсов.

```C
IoTHubClient_LL_Destroy(iotHubClientHandle);
```

В основном есть только один набор AIS для отправки и получения данных с фоновым потоком и другой набор AIS, который делает то же самое без фонового потока. Многие разработчики предпочитают интерфейсы API, не относящие к нижнему уровню. Тем не менее, API нижнего уровня полезны, когда разработчику нужно контролировать передачу данных по сети. Например, некоторые устройства собирают данные в течение продолжительного периода и передают события только через заданные интервалы времени (скажем, раз в час или раз в день). Интерфейсы API нижнего уровня позволяют вам четко контролировать, когда будут отправляться и приниматься данные из Центра Интернета вещей. Другие разработчики отдадут предпочтение простоте интерфейсов API нижнего уровня. Все действия происходят в основном потоке, а не в фоновом режиме.
...
В предыдущем примере мы прикрепляем свойства к событию, которое отправляем в Концентратор IoT. Свойства можно также прикреплять к сообщениям, получаемым из Центра Интернета вещей. Чтобы извлечь свойства из сообщения, в функции обратного вызова сообщения используйте код, подобный указанному ниже.
...
Если сообщение отклонено, оно также отправляется в Центр Интернета вещей, но при этом указывается, что сообщение следует доставить повторно. Обычно вы откажетесь от сообщения, если столкнетесь с ошибкой, но хотите попытаться обработать сообщение снова. В отличие от этого, отклонение сообщения является целесообразным, когда вы столкнулись с невосстановимой ошибкой (или если вы просто решите, что не хотите обрабатывать сообщение).
...
* **SetBatching** (логическое значение). Если значение равно **true**, то данные, отправляемые в Центр Интернета вещей, передаются пакетами. При значении **false** сообщения отправляются по отдельности. По умолчанию **является ложным.** Поддерживается пакетирование над АМЗП / АМЗП-WS, а также добавление системных свойств в сообщения D2C.
...
Параметр пакетной обработки является важным параметром. По умолчанию библиотека передает события по отдельности (одно событие — это то, что вы передаете в **IoTHubClient\_LL\_SendEventAsync**). Если параметр пакетной обработки имеет значение **true**, библиотека собирает из буфера максимально возможное количество событий (вплоть до максимального размера сообщения, который принимается Центром Интернета вещей).  Пакет событий отправляется в Центр Интернета вещей за один вызов HTTPS (отдельные события объединяются в массив JSON). Включение пакетирования обычно приводит к значительному повышению производительности, так как вы сокращаете сетевые обходные поездки. При этом также существенно экономится пропускная способность, так как вы отправляете с пакетом событий только один набор заголовков HTTPS, а не набор заголовков для каждого отдельного события. Если у вас нет конкретной причины, чтобы сделать иначе, как правило, вы хотите включить пакетирование.

## <a name="next-steps"></a>Следующие шаги

В этой статье подробно описывается поведение библиотеки **IoTHubClient,** найденной в **устройстве SDK SDK Для C.** С помощью этой информации вы должны иметь хорошее представление о возможностях библиотеки **IoTHubClient.** Во второй статье этой серии — [Пакет SDK для устройств Azure IoT для C — дополнительные сведения о сериализаторе](iot-hub-device-sdk-c-serializer.md), подробно рассказано о библиотеке **serializer**.
