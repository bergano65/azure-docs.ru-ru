...
Компании, внедряющие Центр Интернета вещей Azure, ожидают надежной работы ресурсов. Чтобы помочь вам внимательно отслеживать действия, Центр Интернета вещей полностью интегрирован с [Azure Monitor](../azure-monitor/index.yml) и службой [Работоспособность ресурсов Azure](../service-health/resource-health-overview.md). Эти две службы работают, чтобы предоставить вам данные, необходимые для поддержания работы ioT-решений в рабочем состоянии.

Azure Monitor является единым источником мониторинга и ведения журнала для всех служб Azure. Можно отправить диагностические журналы, генерируемые Azure Monitor, в журналы Azure Monitor, концентраторы событий или хранилище Azure для пользовательской обработки. Параметры метрик и диагностик Azure Monitor позволяют отслеживать производительность ресурсов в режиме реального времени. Прочтите эту статью, чтобы узнать, как [пользоваться Azure Monitor](#use-azure-monitor) с помощью Центра Интернета вещей. 
...
Служба Работоспособность ресурсов Azure поможет выполнить диагностику и получить необходимую поддержку, если неполадки Azure влияют на ваши ресурсы. Панель мониторинга обеспечивает текущее и прошлое состояние работоспособности для каждого из концентраторов IoT. Продолжить раздел в нижней части этой статьи, чтобы узнать, как [использовать здоровье ресурсов Azure](#use-azure-resource-health) с вашим концентратором IoT. 
...
Параметры диагностики Azure Monitor заменяют операции мониторинга Центра Интернета вещей. Если вы сейчас используете мониторинг операций, необходимо перенести рабочие процессы. Дополнительную информацию см. в статье [Перенос Центра Интернета вещей для перехода с операций мониторинга к параметрам диагностики](iot-hub-migrate-to-diagnostics-settings.md).
...
Категория распределенной трассировки позволяет отследить идентификаторы корреляции для сообщений, содержащих заголовок контекста трассировки. Чтобы полностью включить эти журналы, код на стороне клиента должен быть обновлен, следуя [анализу и диагностике приложений IoT с помощью распределенного отслеживания (предварительного просмотра) Концентратора IoT.](iot-hub-distributed-tracing.md)

Обратите `correlationId` внимание, что соответствует предложению [W3C Trace Context,](https://github.com/w3c/trace-context) где оно `trace-id` содержит, а также `span-id`.
...
| **устройствоId** | Строка 7-битовых букв и цифр ASCII | Удостоверение устройства |
...
В `properties` разделе этот журнал содержит дополнительную информацию о проникновении сообщений.
...
| Свойство | Тип | Описание |
|--------------------|-----------------------------------------------|------------------------------------------------------------------------------------------------|
| **конечная точка** | Строка | Имя конечной точки маршрутизации |
| **конечная точкаТип** | Строка | Тип конечной точки маршрутизации |
...
Настройка конфигурации IoT Hub отслеживает события и ошибки для набора функций автоматического управления устройствами.
...
### <a name="device-streams-preview"></a>Потоки устройств (Предварительный просмотр)

Категория потоков устройств отслеживает взаимодействия запроса и ответа, отправленные на отдельные устройства.
...
2. Перейдите к**здоровью ресурсов здоровья** **службы.** > 

3. Из выпадающих коробок выберите подписку, а затем выберите **IoT Концентризм** в качестве типа ресурса.
