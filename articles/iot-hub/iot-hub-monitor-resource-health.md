...
Компании, внедряющие Центр Интернета вещей Azure, ожидают надежной работы ресурсов. Чтобы помочь вам сохранить закрывающий контроль над операциями, центр Интернета вещей полностью интегрирован с [Azure Monitor](../azure-monitor/index.yml) и [работоспособность ресурсов Azure](../service-health/resource-health-overview.md). Эти две службы работают для предоставления данных, необходимых для обеспечения работоспособности решений Интернета вещей в рабочем состоянии.
...
Также Центр Интернета вещей предоставляет собственные метрики, которые можно использовать для оценки состояния ресурсов Интернета вещей. Дополнительные сведения см. в статье [сведения о метриках центра Интернета вещей](iot-hub-metrics.md).
...
Параметры диагностики Azure Monitor заменяют операции мониторинга Центра Интернета вещей. Если вы сейчас используете мониторинг операций, необходимо перенести рабочие процессы. Дополнительные сведения см. [в статье миграция из Operations Monitoring в параметры диагностики](iot-hub-migrate-to-diagnostics-settings.md).

Дополнительные сведения о конкретных метриках и событиях, которые Azure Monitor отслеживать, см. в разделе [Поддерживаемые метрики с Azure Monitor](../azure-monitor/platform/metrics-supported.md) и [поддерживаемыми службами, схемами и категориями для журналов диагностики Azure](../azure-monitor/platform/diagnostic-logs-schema.md).
...
> [!NOTE]
> Для состояния надежного подключения устройства проверьте [пульс устройства](iot-hub-devguide-identity-registry.md#device-heartbeat).
...
Обратите внимание, что `correlationId` соответствует предложению [контекстной трассировки W3C](https://github.com/w3c/trace-context) , где оно содержит `trace-id`, а также `span-id`.
...
В разделе `properties` этот журнал содержит дополнительные сведения о входящих сообщениях.
...
Центр Интернета вещей Azure указывает состояние работоспособности на региональном уровне. Если региональный сбой влияет на Центр Интернета вещей, отображается состояние работоспособности **Неизвестно**. Дополнительные сведения см. [в разделе Типы ресурсов и проверки работоспособности в службе работоспособности ресурсов Azure](../service-health/resource-health-checks-resource-types.md).
...
* [Удаленный мониторинг Интернета вещей и уведомления с Azure Logic Apps подключением центра Интернета вещей и почтового ящика](iot-hub-monitoring-notifications-with-azure-logic-apps.md)
