...
Пакет SDK для устройств Azure IoT для C написан на языке ANSI C (C99), что обеспечивает максимальную переносимость. Эта функция делает библиотеки хорошо пригодными для работы на нескольких платформах и устройствах, особенно там, где минимизация диска и памяти является приоритетом.

Этот пакет SDK протестирован на множестве платформ (подробные сведения см. в [каталоге устройств, сертифицированных по программе Microsoft Azure Certified for IoT](https://catalog.azureiotsolutions.com/)). В этой статье приведены пошаговые инструкции с примерами кода, выполняемого на платформе Windows. Для других поддерживаемых платформ код будет аналогичным.
...
Существует несколько инструментов с открытым исходным кодом, которые помогут вам управлять концентратором IoT.

* Приложение для Windows под названием [Azure IoT Explorer.](https://github.com/Azure/azure-iot-explorer)

* Кроссплатформенное расширение Visual Studio Code, которое называется [Средства Интернета вещей Azure](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-tools).
...
В этом руководстве описывается использование графического *обозревателя устройств*. Если вы разрабатываете решения в VS Code, можно воспользоваться *Средствами Интернета вещей Azure для VS Code*. Вы также можете использовать расширение IoT для инструмента *Azure CLI 2.0,* если вы предпочитаете использовать инструмент CLI.
...
1. При выполнении программы вы увидите следующий интерфейс:
   ...
   ![Снимок экрана с двойником устройства в Device Explorer](./media/iot-hub-device-sdk-c-intro/DeviceExplorerTwinConfigTab.png)

1. Введите в первое поле **строку подключения Центра Интернета вещей** и нажмите кнопку **Обновить**. Теперь средство настроено для обмена данными с Центром Интернета вещей. 

**Строку подключения** можно найти в разделе **IoT Hub Service (Служба Центра Интернета вещей)** > **Параметры** > **Политика общего доступа** > **iothubowner**.

1. Настроив строку подключения Центра Интернета вещей, перейдите на вкладку **Управление**.

   ![Снимок экрана с двойником устройства и элементами управления в Device Explorer](./media/iot-hub-device-sdk-c-intro/DeviceExplorerTwinManagementTab.png)

Здесь вы можете управлять устройствами, зарегистрированными в Центре Интернета вещей.

1. Чтобы создать устройство, нажмите кнопку **Создать**. Откроется диалоговое окно с заполненным набором ключей (первичных и вторичных). Введите **идентификатор устройства**, а затем нажмите кнопку **Создать**.

   ![Снимок экрана с созданием устройства](./media/iot-hub-device-sdk-c-intro/CreateDevice.png)

1. Когда устройство будет создано, список зарегистрированных устройств обновится. Он будет включать только что созданное устройство. Если щелкнуть новое устройство правой кнопкой мыши, появится следующее меню:

   ![Результат щелчка правой кнопкой мыши по двойнику устройства Device Explorer](./media/iot-hub-device-sdk-c-intro/DeviceExplorerTwinManagementTab_RightClick.png)
...
Выполнив описанные выше действия, вы будете готовы к запуску кода. В большинстве примеров в верхней части основного исходного файла есть константа для ввода строки подключения. Например, соответствующая строка из **iothub_client\_образцов\_iothub_convenience_sample** приложения выглядит следующим образом.
...
Версия **образцов iothub_client\_Windows\_iothub_convenience_sample** приложения включает в себя следующее решение Visual Studio:

  ![Обозреватель решений Visual Studio](./media/iot-hub-device-sdk-c-intro/iothub-client-sample-mqtt.png)

> [!NOTE]
> Если Visual Studio попросит вас перенацелить проект на последнюю версию, примите запрос.
...
Можно найти реализацию для примерного приложения в **iothub_client\_образцах\_iothub_convenience_sample** исходного файла.
...
Реализация примерного применения можно найти **в\_\_iothub_client образцов iothub_convenience_sample** файле.
...
Модель в библиотеке **serializer** определяет сообщения, которые ваше устройство может отправлять в Центр Интернета вещей, а также сообщения, которые оно может получать (на языке моделирования называются *действия*). Модель определяется с помощью набора макросов C, как в **iothub_client\_образцах\_iothub_convenience_sample** примера применения:
...
В этой статье были рассмотрены основы использования библиотек в **устройстве SDK SDK Azure для C**. Он предоставил вам достаточно информации, чтобы понять, что включено в SDK, его архитектуру, и как начать работу с образцами Windows. В следующей статье также рассказывается о пакете SDK и приводятся [дополнительные сведения о библиотеке IoTHubClient](iot-hub-device-sdk-c-iothubclient.md).
