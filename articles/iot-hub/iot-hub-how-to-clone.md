...
Это общий метод, который мы рекомендуем использовать для перемещения центра Интернета вещей из одного региона в другой. Для маршрутизации сообщений предполагается, что ресурсы не перемещаются в новый регион. Дополнительные сведения см. в [разделе Маршрутизация сообщений](#how-to-handle-message-routing).
   ...
   1. Добавьте все, что не было экспортировано в шаблон. 
       ...
       [Дополнения сообщений](iot-hub-message-enrichments-overview.md) также не экспортируются в шаблон. Они используются совместно с сообщениями маршрутизации и должны обновляться вручную в новом концентраторе при обновлении конфигурации маршрутизации сообщений.
...
> [!NOTE]
> Если в центре используются [улучшения сообщений](iot-hub-message-enrichments-overview.md), их необходимо настроить вручную в новом центре Интернета вещей, так как они не экспортируются с помощью шаблона диспетчер ресурсов.
...
1. Выберите **Развертывание шаблона (развертывание с помощью пользовательских шаблонов)** . Откроется экран для Шаблоны развертывания. Нажмите кнопку **Создать**. Появится следующий экран:
...
Это можно сделать несколькими способами. Вы изначально использовали [службу подготовки устройств (DPS)](/azure/iot-dps/about-iot-dps.md)для подготовки устройств, или это не так. Если вы сделали это, это не сложно. Если это не так, это может быть очень сложно. 
