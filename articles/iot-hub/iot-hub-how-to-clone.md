...
1. После получения ключа учетной записи хранения поместите его в шаблон в предложении `AccountKey=****` вместо звездочек. 

1. Для очередей служебной шины получите ключ общего доступа, соответствующий SharedAccessKeyName. Вот ключ и `SharedAccessKeyName` в JSON:
...
1. Выберите **Развертывание шаблона (развертывание с помощью пользовательских шаблонов)**. Откроется экран для Шаблоны развертывания. Выберите **Создать**. Появляется следующий экран:
...
1. Найдите новый шаблон, который вы редактировали, и выберите его, а затем нажмите кнопку **Открыть**. Шаблон загружается в окно редактирования. Нажмите кнопку **Сохранить**. 
...
1. Используйте примеры C# для Интернета вещей на этой странице: [примеры Azure IOT для C#](https://azure.microsoft.com/resources/samples/azure-iot-samples-csharp/). Скачайте ZIP-файл и распакуйте его на компьютере. 
...
1. Проверьте наличие ошибок импорта и экспорта, перейдя в учетную запись хранения Azure в [портал Azure](https://portal.azure.com) и просматривая `devicefiles` контейнер для `ImportErrors.log` . Если этот файл пуст (размер равен 0), ошибки отсутствуют. Если вы попытаетесь импортировать одно устройство несколько раз, оно отклоняет устройство второй раз и добавляет сообщение об ошибке в файл журнала.
