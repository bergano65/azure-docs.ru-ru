...
Теперь вы напишете консольное приложение для .NET, которое отправляет сообщения из облака в приложение устройства.
...
1. В обозревателе решений щелкните правой кнопкой мыши новое решение и выберите пункт **Управление пакетами NuGet**.
...
1. Добавьте следующие поля в класс **Program** . Замените заполнитель строкой подключения центра Интернета вещей, скопированной в разделе [Получение строки подключения центра Интернета вещей](#get-the-iot-hub-connection-string).
...
1. Добавьте следующий метод в класс **Program**. Задайте имя устройства, которое использовалось при определении устройства в руководстве [Отправка данных телеметрии с устройства в центр Интернета вещей](quickstart-send-telemetry-dotnet.md).
...
1. В разделе **Общие свойства** > **Запускаемый проект** выберите **Несколько запускаемых проектов**, а затем выберите действие **Запуск** для **ReadDeviceToCloudMessages**, **SimulatedDevice** и **SendCloudToDevice**. Нажмите кнопку **ОК** , чтобы сохранить изменения.
