...
| **обновить** |Если устройство с указанным **идентификатором**уже существует, существующая информация перезаписывается предоставленными входными данными без учета значения **ETag** . <br/>Если устройство не существует, в файл журнала записывается ошибка. |
...
Статья клонирования содержит рабочий пример, связанный с ним, который находится в примерах IOT C# на этой странице: [примеры Azure IOT для C# ](https://azure.microsoft.com/resources/samples/azure-iot-samples-csharp/), а также проект, импортекспортдевицессампле. Вы можете скачать пример и испытать его. Здесь приведены инструкции по [клонированию статьи центра Интернета вещей](iot-hub-how-to-clone.md) .
