---
title: Импорт/экспорт идентификаторов устройств Azure IoT hub Документы Майкрософт
description: Как использовать службу SDK Azure IoT для выполнения оптовых операций против реестра идентификационных данных для импорта и экспорта идентификаторов устройств. Операции импорта позволяют создавать, обновлять и удалять удостоверения устройств в пакетном режиме.
author: robinsh
manager: philmea
ms.service: iot-hub
services: iot-hub
...
ms.locfileid: "79371585"
---
# <a name="import-and-export-iot-hub-device-identities-in-bulk"></a>Import and export IoT Hub device identities in bulk (Импорт и экспорт удостоверений устройств центра Интернета вещей в пакетном режиме)

В каждом Центре Интернета вещей есть реестр удостоверений, который можно использовать для создания в службе уникальных ресурсов устройства. Реестр удостоверений обеспечивает доступ к конечным точкам, обращенным к устройствам. В этой статье описывается массовый импорт удостоверений устройств в реестр удостоверений и массовый экспорт удостоверений из реестра. Чтобы увидеть рабочий образец в СЗ и узнать, как можно использовать эту возможность при клонировании концентратора в другой регион, [см.](iot-hub-how-to-clone.md)

> [!NOTE]
> IoT Hub недавно добавил поддержку виртуальной сети в ограниченном числе регионов. Эта функция обеспечивает импортно-экспортные операции и устраняет необходимость передачи ключей для проверки подлинности.  Изначально виртуальная сетевая поддержка доступна только в этих регионах: *WestUS2,* *EastUS*и *SouthCentralUS.* Чтобы узнать больше о поддержке виртуальной сети [IoT Hub Support for virtual networks](virtual-network-support.md)и вызовах API для ее реализации, см.
...
Вы можете использовать класс **RegistryManager** для запросов состояния **задания** с помощью возвращенных метаданных **JobProperties**. Для создания экземпляра класса **RegistryManager** используйте метод **CreateFromConnectionString.**
...
## <a name="device-importexport-job-limits"></a>Ограничения на импорт/экспорт устройств

Для всех уровней IoT Hub допускается только 1 работа по импорту или экспорту активных устройств. IoT Hub также имеет ограничения для скорости операций с рабочими местами. Чтобы узнать больше, [см. Справка - Квоты IoT концентратор и регулирование](iot-hub-devguide-quotas-throttling.md).

## <a name="export-devices"></a>Экспорт устройств

Используйте метод **ExportDevicesAsync** для экспорта полного реестра идентификационных данных концентратора IoT в контейнер Azure Storage с помощью общей подписи доступа (SAS). Для получения дополнительной информации об подписях общего доступа см. [Грант ограниченный доступ к ресурсам хранения Azure с использованием общих подписей доступа (SAS).](../storage/common/storage-sas-overview.md)
...
| importMode | Описание |
| --- | --- |
| **createOrUpdate** |Если устройство не существует с указанным **идентификатором,** оно недавно зарегистрировано. <br/>Если устройство уже существует, имеющиеся данные заменяются предоставленными входными данными вне зависимости от значения **ETag** . <br> Вместе с данными устройства пользователь может дополнительно указать данные двойника устройства. Etag близнеца, если указано, обрабатывается независимо от etag устройства. При несоответствии с etag существующего близнеца в файле журнала записывается ошибка. |
| **Создать** |Если устройство не существует с указанным **идентификатором,** оно недавно зарегистрировано. <br/>Если устройство уже существует, в файл журнала записывается ошибка. <br> Вместе с данными устройства пользователь может дополнительно указать данные двойника устройства. Etag близнеца, если указано, обрабатывается независимо от etag устройства. При несоответствии с etag существующего близнеца в файле журнала записывается ошибка. |
| **Обновление** |Если устройство уже существует с указанным **идентификатором,** существующая информация перезаписана с предоставленными входными данными без учета значения **ETag.** <br/>Если устройство не существует, в файл журнала записывается ошибка. |
| **updateIfMatchETag** |Если устройство уже существует с указанным **идентификатором,** существующая информация перезаписана с предоставленными входными данными только при наличии соответствия **ETag.** <br/>Если устройство не существует, в файл журнала записывается ошибка. <br/>Если имеется несовпадение **ETag** , в файл журнала записывается ошибка. |
| **createOrUpdateIfMatchETag** |Если устройство не существует с указанным **идентификатором,** оно недавно зарегистрировано. <br/>Если устройство уже существует, имеющиеся данные заменяются предоставленными входными данными только при совпадении **ETag** . <br/>Если имеется несовпадение **ETag** , в файл журнала записывается ошибка. <br> Вместе с данными устройства пользователь может дополнительно указать данные двойника устройства. Etag близнеца, если указано, обрабатывается независимо от etag устройства. При несоответствии с etag существующего близнеца в файле журнала записывается ошибка. |
| **Удалить** |Если устройство уже существует с указанным **идентификатором,** оно удаляется без учета значения **ETag.** <br/>Если устройство не существует, в файл журнала записывается ошибка. |
| **deleteIfMatchETag** |Если устройство уже существует с указанным **идентификатором,** оно удаляется только при наличии совпадения **ETag.** Если устройство не существует, в файл журнала записывается ошибка. <br/>Если имеется несовпадение ETag, в файл журнала записывается ошибка. |
...
В этой статье вы узнали, как выполнять массовые операции с реестром удостоверений в Центре Интернета вещей. Многие из этих операций, в том числе способ перемещения устройств из одного концентратора в другой, используются в [устройствах управления, зарегистрированных в разделе концентратора IoT Как клонировать концентратор IoT.](iot-hub-how-to-clone.md#managing-the-devices-registered-to-the-iot-hub) 

Статья клонирования имеет рабочий образец, связанный с ним, который находится в образцах IoT C на этой странице: [Azure IoT Образцы для C ,](https://azure.microsoft.com/resources/samples/azure-iot-samples-csharp/)с проектом Является ImportExportDevicesSample. Вы можете скачать образец и попробовать его; Есть инструкции в статье [Как клонировать IoT концентратор](iot-hub-how-to-clone.md) статьи.

Чтобы узнать больше об управлении концентратором Azure IoT, ознакомьтесь со следующими статьями:

* [Метрики Центра Интернета вещей](iot-hub-metrics.md)
* [Журналы концентратора IoT](iot-hub-monitor-resource-health.md)
