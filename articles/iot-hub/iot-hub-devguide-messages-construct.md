...
description: Руководство для разработчиков - описывает формат и ожидаемое содержание сообщений IoT Hub.
author: ash2017
manager: briz
ms.service: iot-hub
services: iot-hub
...
Для реализация отправки сообщений с устройства в облако Центр Интернета вещей использует модель потоковой передачи сообщений. Сообщения IoT Hub «устройство к облаку» больше похожи на *события* [Event Hubs,](/azure/event-hubs/) чем *на сообщения* [Службы Автобус](/azure/service-bus-messaging/) в том, что существует большой объем событий, проходящих через службу, которые могут быть прочитаны несколькими читателями.
...
* Вы можете штамповать сообщения с информацией, которая поступает в свойства приложения. Для получения дополнительной [информации,](iot-hub-message-enrichments-overview.md)пожалуйста, смотрите сообщения обогащения .

Дополнительные сведения о способах кодировки и декодирования сообщений, отправляемых с помощью разных протоколов, см. в статье, посвященной [пакетам SDK для Azure IoT](iot-hub-devguide-sdks.md).

## <a name="system-properties-of-d2c-iot-hub-messages"></a>Системные свойства сообщений **Концентратора D2C** IoT

| Свойство | Описание  |Пользовательский набор?|Ключевое слово для  </br>запрос на разгром|
| --- | --- | --- | --- |
...
| iothub-enqueuedtime |Дата и время получения сообщения [от устройства к облаку](iot-hub-devguide-d2c-guidance.md) от IoT Hub. | нет | enqueuedTime |
...
| iothub-соединение-модуль-id |Центр Интернета вещей устанавливает этот идентификатор в сообщениях, отправляемых с устройства в облако. Он содержит **модуль** устройства, отправивого сообщение. | нет | подключениеModuleId |
| iothub-connection-auth-generation-id |Центр Интернета вещей устанавливает этот идентификатор в сообщениях, отправляемых с устройства в облако. Он содержит **соединениеDeviceGenerationId** (в как вне [свойств идентификации устройства)](iot-hub-devguide-identity-registry.md#device-identity-properties)устройства, которое отправило сообщение. | нет |подключениеDeviceGenerationId |
| iothub-connection-auth-method |Центр Интернета вещей устанавливает этот метод проверки подлинности в сообщениях, отправляемых с устройства в облако. Это свойство содержит сведения о методе проверки подлинности, используемом для аутентификации устройства, отправляющего сообщение.| нет | соединениеАутМетод |

## <a name="system-properties-of-c2d-iot-hub-messages"></a>Системные свойства сообщений **C2D** IoT Hub

| Свойство | Описание  |Пользовательский набор?|
| --- | --- | --- |
...
Центр Интернета вещей измеряет размер сообщения вне зависимости от протокола, учитывая только фактический объем полезных данных. Размер байтов рассчитывается как сумма следующих значений:
...
* **iothub-соединение-устройство-id**
* **iothub-connection-auth-generation-id**
* **iothub-connection-auth-method**

Первые два свойства содержат параметры **deviceId** и **generationId** исходного устройства (согласно разделу [Device identity properties](iot-hub-devguide-identity-registry.md#device-identity-properties) (Свойства удостоверений устройств)).
...
* Сведения об ограничениях на размер сообщений в Центре Интернета вещей см. в справочнике [по квотам и регулированию Центра Интернета вещей](iot-hub-devguide-quotas-throttling.md).
