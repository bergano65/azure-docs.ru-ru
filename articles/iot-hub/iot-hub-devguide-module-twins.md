...
Центр Интернета вещей обеспечивает ограничение размера в 8 КБ на значение `tags`и максимальный размер 32 КБ для каждого значения `properties/desired` и `properties/reported`. Эти итоги являются исключительно для элементов только для чтения, таких как `$etag`, `$version`и `$metadata/$lastUpdated`.
...
Теги содержат ETag (согласно [RFC7232](https://tools.ietf.org/html/rfc7232)), являющийся представлением JSON тега. ETag можно использовать в операциях условного обновления в серверной части решения, чтобы обеспечить согласованность.
