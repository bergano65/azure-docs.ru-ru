...
Жизненный цикл двойника модуля связан с соответствующим [удостоверением модуля](iot-hub-devguide-identity-registry.md). Двойники модуля неявно создаются и удаляются при создании или удалении удостоверения модуля в Центре Интернета вещей.
...
* **Пожеланные свойства**. Используются совместно с сообщаемыми свойствами для синхронизации конфигурации или условий модуля. В серверной части решения можно задать нужные свойства, а приложение модуля может считывать их. Приложение модуля может также получать уведомления об изменениях в нужных свойствах.

* **Сообщенные свойства**. Используется совместно с требуемыми свойствами для синхронизации конфигурации или условий модуля. Приложение модуля может задать сообщаемые свойства, а серверная часть решения может считывать и запрашивать их.

* **Свойства удостоверений модулей**. Корень документа JSON двойника модуля содержит свойства только для чтения из соответствующего удостоверения модуля, хранимые в [реестре удостоверений](iot-hub-devguide-identity-registry.md).

![Представление двойника устройства в архитектуре](./media/iot-hub-devguide-device-twins/module-twin.jpg)
...
В предыдущем примере требуемые и сообщаемые свойства двойника модуля `telemetryConfig` используются в серверной части решения и приложении для модуля, чтобы синхронизировать конфигурацию телеметрии для этого модуля. Пример:
...
3. Серверная часть решения может отслеживать результаты операции изменения конфигурации на множестве модулей, [отправляя запросы](iot-hub-devguide-query-language.md) на двойники модуля.
...
* **Получение уведомлений двойника**. Эта операция позволяет серверной части решения получать уведомления при изменении двойника. Для этого в решении Интернета вещей необходимо создать маршрут и присвоить источнику данных значение *twinChangeEvents*. По умолчанию уведомления двойника не отправляются, то есть такие маршруты не существуют. Если скорость изменения слишком высока или имеются какие-либо другие причины (например, внутренние сбои), Центр Интернета вещей может отправлять только одно уведомление, которое содержит все изменения. Таким образом, если приложению требуется надежный аудит и регистрация всех промежуточных состояний, следует использовать сообщения, отправляемые с устройства в облако. Уведомление двойника содержит свойства и текст.
  ...
  - Свойства
    ...
    iothub-message-schema | twinChangeУведомление |
...
Помимо выполнения этих операций серверная часть решения может отправлять запросы к двойникам устройств с помощью [языка запросов Центра Интернета вещей](iot-hub-devguide-query-language.md) по типу SQL.
...
Все предыдущие операции требуют разрешения **ModuleConnect**, как указано в статье [Управление доступом к Центру Интернета вещей](iot-hub-devguide-security.md).

[Пакеты SDK для устройств Azure IoT](iot-hub-devguide-sdks.md) упрощают использование указанных выше операций на многих языках и платформах.
...
* **Ключи**: Все ключи в объектах JSON являются чувствительными к случаям 64 байт UTF-8 строки UNICODE. Разрешено использовать все знаки, кроме управляющих знаков Юникода (сегменты C0 и C1), а также `.`, `$` и SP.

* **Значения**: Все значения в объектах JSON могут быть следующих типов JSON: булеан, число, строка, объект. Запрещено использовать массивы.

    * Ряды могут иметь минимальное значение -450359962770496 и максимальное значение 450359962770495.

    * Значения струн закодированы UTF-8 и могут иметь максимальную длину 512 байтов.

* **Глубина**: Все объекты JSON в тегах, желаемые и сообщенные свойства могут иметь максимальную глубину 5. Например, допустим следующий объект:
...
IoT концентратор обеспечивает ограничение размера 8 `tags`КБ на стоимость, и 32 `properties/desired` КБ ограничение размера каждого на стоимость и `properties/reported`. Эти итоги являются эксклюзивными только `$etag`для `$version`чтения `$metadata/$lastUpdated`элементов, таких как , и .

Размер близнецов вычисляется следующим образом:

* Для каждого свойства в документе JSON IoT Hub кумулятивно вычисляет и добавляет длину ключа и стоимость свойства.

* Ключи свойства считаются строками UTF8.

* Простые значения свойств считаются закодированными строками UTF8, числовыми значениями (8 байт) или значениями Boolean (4 байта).

* Размер закодированных строк UTF8 вычисляется путем подсчета всех символов, за исключением символов управления UNICODE (сегменты C0 и C1).
...
Центр Интернета вещей сохраняет метку времени последнего обновления для каждого объекта JSON в требуемых и сообщаемых свойствах двойника модуля. Метки времени указываются в формате UTC и кодируются в формате [ISO8601](https://en.wikipedia.org/wiki/ISO_8601) (`YYYY-MM-DDTHH:MM:SS.mmmZ`).
Пример:
...
Теги имеют ETag, в соответствии [с RFC7232](https://tools.ietf.org/html/rfc7232), который представляет jSON представления тега. ETag можно использовать в операциях условного обновления в серверной части решения, чтобы обеспечить согласованность.
...
* [Приступая к работе с удостоверением Центра Интернета вещей и двойником модуля с использованием серверной части и устройства .NET](iot-hub-csharp-csharp-module-twin-getstarted.md)
