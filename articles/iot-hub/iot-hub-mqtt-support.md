...
* [MQTT версии 3.1.1](https://mqtt.org/) на порту 8883;
...
Если вы используете [пакеты SDK для устройств](https://github.com/Azure/azure-iot-sdks), перехода с протокола AMQP на MQTT требуется изменить параметр протокола в инициализации клиента, как уже говорилось ранее.
...
* MQTT не поддерживает *Отклонить* операций при получении [сообщений из облака на устройство](iot-hub-devguide-messaging.md). Если вашему приложению серверной части для получения ответа от приложения для устройства, рассмотрите возможность использования [прямые методы](iot-hub-devguide-direct-methods.md).

## <a name="using-the-mqtt-protocol-directly-as-a-device"></a>Непосредственное использование протокола MQTT (как устройство)

Если устройство не может использовать пакеты SDK для устройств, оно может подключаться к общедоступным конечным точкам устройства по протоколу MQTT по порту 8883. В **CONNECT** пакетов, устройство должно использовать следующие значения:
...
* В поле **Пароль** укажите маркер SAS. Формат маркера SAS аналогичен описанному для протоколов HTTPS и AMQP:
  ...
  > [!NOTE]
  > При использовании аутентификации с помощью сертификата X.509 пароли маркеров SAS не требуются. Дополнительные сведения см. в разделе [Настройка сертификата безопасности X.509 в центре Интернета вещей Azure](iot-hub-security-x509-get-started.md)

  Дополнительные сведения о способах создания маркеров SAS см. в разделе описания устройств раздела [маркеры безопасности центра Интернета вещей с помощью](iot-hub-devguide-security.md#use-sas-tokens-in-a-device-app).

  При тестировании, можно также использовать кроссплатформенного [средства Интернета вещей Azure для Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-tools) или [Device Explorer](https://github.com/Azure/azure-iot-sdk-csharp/blob/master/tools/DeviceExplorer) для быстрого создания маркера SAS, который можно скопировать и вставить в собственный код:
...
Чтобы установить соединение TLS, может потребоваться скачать корневой сертификат DigiCert Baltimore и добавить ссылку на него. Этот сертификат используется в Azure для обеспечения безопасного подключения. Вы можете найти этот сертификат в [Azure-iot-sdk-c](https://github.com/Azure/azure-iot-sdk-c/blob/master/certs/certs.c) репозитория. Дополнительные сведения об этих сертификатах можно найти на [веб-сайте Digicert](https://www.digicert.com/digicert-root-certificates.htm).

Пример реализации с помощью версии Python [библиотеку Paho MQTT](https://pypi.python.org/pypi/paho-mqtt) фонда Eclipse может выглядеть следующим образом.
...
> [!NOTE]
> Это `{property_bag}` элемент использует ту же кодировку, что строки запросов в протоколе HTTPS.
...
Дополнительные сведения см. в разделе [обмена сообщениями руководство разработчика](iot-hub-devguide-messaging.md).
...
Идентификатором запроса может быть любое допустимое значение для значения свойства сообщения, как указано в [центра Интернета вещей, обмен сообщениями руководство разработчика](iot-hub-devguide-messaging.md), а состояние проверяется как целое число.
...
Дополнительные сведения см. в разделе [устройства двойники](iot-hub-devguide-device-twins.md).
...
> [!IMPORTANT]
> Центр Интернета вещей создает уведомления об изменении только в том случае, если устройства подключены. Убедитесь, что [повторном подключении устройства](iot-hub-devguide-device-twins.md#device-reconnection-flow) для нужного свойства продолжали синхронизироваться между центром Интернета вещей и приложением для устройства.
...
Дополнительные сведения см. в разделе [прямой метод в руководстве разработчика](iot-hub-devguide-direct-methods.md).

### <a name="additional-considerations"></a>Дополнительные замечания

Наконец, обратите внимание, если вам нужно настраивать поведение протокола MQTT на стороне облака, следует ознакомиться с [шлюз протокола Azure IoT](iot-hub-protocol-gateway.md). Это программное обеспечение позволяет развернуть шлюз протокола с высокой производительностью, который взаимодействует непосредственно с Центром Интернета вещей. Шлюз протокола Azure IoT позволяет настроить протокол устройства для уже существующих развертываний MQTT или других настраиваемых протоколов. Однако при этом подходе необходимо запустить настраиваемый шлюз протокола и управлять им.

## <a name="next-steps"></a>Дальнейшие действия

Дополнительные сведения о протоколе MQTT см. в разделе [документации по MQTT](https://mqtt.org/documentation).
