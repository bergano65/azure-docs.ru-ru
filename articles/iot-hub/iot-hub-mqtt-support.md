...
• PnPMQTTWin32: содержит код для отправки сообщения телеметрии с помощью & подключения к Интернету с подключением к центру Интернета вещей, которые можно создавать и запускать на компьютере Windows. Дополнительные сведения о подключении к Интернету IoT & воспроизвести [здесь](https://docs.microsoft.com/en-us/azure/iot-pnp/overview-iot-plug-and-play)

**Для Linux:**

• Мкттлинукс: содержит код и сценарий сборки для запуска в Linux (WSL, Ubuntu и Raspbian были протестированы на данный момент).
...
• Эта папка содержит две команды, которые используются с служебной программой mosquitto_pub, предоставляемой Mosquitto.org.

Mosquitto_sendmessage: Отправка простого текстового сообщения в центр Интернета вещей Azure, который выступает в качестве устройства.
