...
description: Узнайте о языке запроса запроса на запрос ioT Hub, который можно использовать для применения богатых запросов к сообщениям для получения важных для вас данных.
author: ash2017
ms.service: iot-hub
services: iot-hub
ms.topic: conceptual
...
| iothub-интерфейс-имя | строка | Это значение устанавливается пользователем и представляет собой название цифрового двойного интерфейса, который реализует телеметрическое сообщение. Чтобы запросить, используйте `$interfaceName`. Эта функция доступна в рамках [публичного предварительного просмотра IoT Plug and Play.](../iot-pnp/overview-iot-plug-and-play.md) |

Как описано в разделе [Сообщения центра Интернета вещей](iot-hub-devguide-messages-construct.md), существуют дополнительные системные свойства в сообщении. В дополнение к **contentType**, **contentEncoding**, и **enqueuedTime**, **подключениеDeviceId** и **connectionModuleId** также могут быть запрошены.
...
Полный список поддерживаемых операторов и функций отображается в [Выражении и условиях.](iot-hub-devguide-query-language.md#expressions-and-conditions)
...
> [!NOTE] 
> Это показывает, как обрабатывать кодирование тела в JavaScript. Если вы хотите увидеть образец в C, загрузите [образцы Azure IoT C.](https://github.com/Azure-Samples/azure-iot-samples-csharp/archive/master.zip) Распакуйте файл master.zip. Решение Visual *Studio, симулирующееProgram.cs файл,* показывает, как кодировать и отправлять сообщения в концентратор IoT. Это тот же образец, который используется для тестирования направлении сообщений, как это объясняется в учебнике по [направлению сообщений.](tutorial-routing.md) В нижней части Program.cs, он также имеет метод для чтения в одном из закодированных файлов, расшифровать его, и записать его обратно, как ASCII, чтобы вы могли прочитать его. 
...
Маршрутизация сообщений позволяет запрашивать теги и свойства [двойников устройств](iot-hub-devguide-device-twins.md), которые являются объектами JSON. Поддерживается также запрос на модульный близнец. Ниже приведен пример свойств и тегов двойников устройств.
...
Запрос на сообщение близнец должен быть префиксирован с `$twin`. Выражение запроса также может объединять ссылку на свойство или тег двойника со ссылкой на текст, системные свойства сообщения и ссылкой на свойства приложения сообщения. Мы рекомендуем использовать уникальные имена в тегах и свойствах, так как запрос не учитывает регистр. Это относится как к близнецам устройств, так и к близнецам-модулям. Кроме того, в качестве имен свойств не рекомендуется использовать `twin`, `$twin`, `body` или `$body`. Например, все выражения, приведенные ниже, допустимы: 
...
Запрос на реутинг на тело или устройство twin с периодом в полезной нагрузке или имени свойства не поддерживается.

## <a name="next-steps"></a>Дальнейшие действия

* Подробнее о [разгроме сообщений.](iot-hub-devguide-messages-d2c.md)
* Попробуйте учебник по [разгрому сообщений.](tutorial-routing.md)
