...
description: Узнайте о языке запросов маршрутизации сообщений центра Интернета вещей, который можно использовать для применения сложных запросов к сообщениям для получения нужных данных.
author: ash2017
ms.service: iot-hub
services: iot-hub
ms.topic: conceptual
...
Маршрутизация сообщений позволяет запрашивать теги и свойства [двойников устройств](iot-hub-devguide-device-twins.md), которые являются объектами JSON. Также поддерживается выполнение запросов в модуле двойника. Ниже приведен пример свойств и тегов двойников устройств.
...
Запрос к тексту сообщения должен начинаться с `$twin`. Выражение запроса также может объединять ссылку на свойство или тег двойника со ссылкой на текст, системные свойства сообщения и ссылкой на свойства приложения сообщения. Мы рекомендуем использовать уникальные имена в тегах и свойствах, так как запрос не учитывает регистр. Это относится как к двойниковам устройств, так и к двойников модуля. Кроме того, в качестве имен свойств не рекомендуется использовать `twin`, `$twin`, `body` или `$body`. Например, все выражения, приведенные ниже, допустимы: 
