...
Например, можно задать путь двойника, `properties.desired.chiller-water` а затем предоставить следующее содержимое JSON: 
...
Можно также задать отдельные параметры, указав весь путь двойника и указав значение без квадратных скобок. Например, с помощью пути `properties.desired.chiller-water.temperature`двойника задайте для `66`содержимого значение. Затем создайте новый параметр двойника для свойства нажима. 
...
Если вы создаете метрику для создания отчетов о настроенных модулях, выберите `moduleId` из `devices.modules`. Пример:
