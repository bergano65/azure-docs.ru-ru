...
* Создайте приложение для устройств, которое реализует прямой метод под названием **LockDoor,** который можно вызвать в приложении бэк-энда.
...
* **SimulateDeviceMethods**. Это приложение подключается к концентратору IoT и реализует прямой метод **LockDoor.**

* **ScheduleJob**. Это приложение использует задания для вызова **LockDoor** прямой метод и обновить устройство близнецов желаемых свойств на нескольких устройствах.
...
1. В Visual Studio выберите **Создать новый проект,** а затем выбрать шаблон проекта **Console App (.NET Framework).** Нажмите кнопку **Далее**, чтобы продолжить.

1. В **настройке нового проекта,** назовите проект *SimulateDeviceMethods,* а затем выберите **Создать.**

    ![Налажить проект SimulateDeviceMethods](./media/iot-hub-csharp-csharp-schedule-jobs/configure-device-app.png)
...
1. Добавьте следующий код для реализации прямого метода на устройстве:
...
1. Добавьте следующий метод для реализации устройства близнецов слушателя на устройстве:
...
> [!NOTE]
> Чтобы все было просто, этот учебник не реализует никаких политик повторную попытку. В производственном коде следует реализовать политики повторной попытки (например, повторение подключения), как это предлагается при [обработке переходных сбоев.](/azure/architecture/best-practices/transient-faults)
>
...
1. В **настройке нового проекта**назовите проект *ScheduleJob.* Для **решения** **выберите Добавить в решение,** а затем выберите **Создать.**
    ...
    ![Назовите и напланируйте проект ScheduleJob](./media/iot-hub-csharp-csharp-schedule-jobs/config-schedule-job-app.png)

1. В Solution Explorer нажмите правой кнопкой мыши на **проектЕ ScheduleJob,** а затем выберите **пакеты Manage NuGet.**

1. В **наборе NuGet менеджер**, выберите **Просмотр**, поиск и выбор **Microsoft.Azure.Devices**, а затем выберите **Установить**.
...
1. Добавьте следующие поля в класс **Program** . Замените заполнители строкой подключения Концентратора IoT, которую вы скопировали ранее в [строке подключения концентратора IoT](#get-the-iot-hub-connection-string) и названии устройства.
...
1. В Visual Studio Solution Explorer нажмите правой кнопкой мыши ваше решение, а затем выберите **Set StartUp Projects.**

1. Выберите**проект запуска** **Common Properties,** > а затем выберите **несколько проектов запуска.**

1. Убедитесь, что `SimulateDeviceMethods` находится в верхней части списка, за которым следует `ScheduleJob`. Установите оба своих действия, чтобы **начать** и выбрать **OK**.
