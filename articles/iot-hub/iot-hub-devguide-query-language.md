...
Центр Интернета вещей предоставляет мощный язык SQL-подобные для получения сведений о [двойников](iot-hub-devguide-device-twins.md), [двойников модулей](iot-hub-devguide-module-twins.md), [заданий](iot-hub-devguide-jobs.md), и [маршрутизациисообщений](iot-hub-devguide-messages-d2c.md). В этой статье представлены:
...
Обработку запросов для двойников модулей похож на обработку запросов для двойников устройств, но с помощью другой коллекции или пространстве имен. а не из **устройств**, запрос из **devices.modules**:
...
**Из < из_спецификации >** предложение можно предположить, что только три значения: **С устройств** выполнять запросы к двойникам устройств, **из devices.modules** выполнять запросы к двойникам модуля, или **FROM devices.jobs** для сведения о запросе задания на устройство.
