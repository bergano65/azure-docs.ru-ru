...
IoT концентратор обеспечивает мощный язык, похожий на S'L, для получения информации о [близнецах устройств,](iot-hub-devguide-device-twins.md) [близнецах-модулях,](iot-hub-devguide-module-twins.md) [заданиях](iot-hub-devguide-jobs.md)и [разгроме сообщений.](iot-hub-devguide-messages-d2c.md) В этой статье представлены:
...
[Близнецы устройств](iot-hub-devguide-device-twins.md) и [близнецы модулей](iot-hub-devguide-module-twins.md) могут содержать произвольные объекты JSON как теги, так и свойства. Центр Интернета вещей позволяет выполнять запросы к двойникам устройств и двойникам модулей как к одному документу JSON, содержащему все сведения о двойниках.
...
Запрос на близнецы модулей аналогичен запросу на близнецах устройств, но с использованием другого пространства коллекции/имен; вместо **устройств**вы запросите с **устройств.модули:**
...
> [!IMPORTANT]
> Результаты запросов могут поступать с задержкой в несколько минут и не учитывать последние значения в двойниках устройств. Если запрос отдельных близнецов устройства по ID, используйте [двойной REST API.](https://docs.microsoft.com/rest/api/iothub/service/twin/getdevicetwin) Этот API всегда возвращает последние значения и имеет более высокие ограничения регулирования. Вы можете выпустить API REST напрямую или использовать эквивалентную функциональность в одном из [SDK-сервиса Azure IoT.](iot-hub-devguide-sdks.md#azure-iot-hub-service-sdks)
...
Положение **FROM <from_specification>** может предполагать только три значения: from **devices** to query device twins, FROM **devices.modules** to query module twins или FROM **devices.jobs** to query job per-device details.
...
| hexadecimal_literal |Число, выраженное строкой '0x', сопровождается строкой гексадецимальных цифр. |
...
| IS_DEFINED | Возвращает логическое значение, указывающее, назначено ли свойству значение. Это поддерживается только тогда, когда значение является примитивным типом. Примитивные типы включают строку, boolean, число, или `null`. DateTime, типы объектов и массивы не поддерживаются. |
