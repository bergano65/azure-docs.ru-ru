...
В этом примере устройство вызывает функцию **Complete** для уведомления центра Интернета вещей о том, что сообщение обработано, и что его можно безопасно удалить из очереди устройства. Вызов **complete** не требуется, если используется транспорт MQTT, и его можно опустить. Он необходим для AMQP и HTTPS.
