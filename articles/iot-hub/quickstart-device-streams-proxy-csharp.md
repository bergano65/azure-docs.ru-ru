...
[Потоки устройств Центра Интернета вещей](./iot-hub-device-streams-overview.md) позволяют службам и приложениям устройств безопасным и подходящим методом обмениваться данными с брандмауэром. Это краткое руководство включает две программы C#, которые отправляют трафик SSH и RDP через поток устройств, установленный через Центр Интернета вещей. Общие сведения о настройке см. на [этой странице](./iot-hub-device-streams-overview.md#local-proxy-sample-for-ssh-or-rdp).

Сначала мы опишем настройку для SSH (используя порт 22). Затем опишем, как изменить заданный порт настройки для RDP. Так как потоки устройств не зависят от приложений и протоколов, тот же пример можно изменить (обычно путем изменения коммуникационных портов) для размещения других типов трафика приложений.
...
3. Локальный прокси-сервер устройства подключается к управляющей программе SSH (SSHD), прослушивающей порт 22 на устройстве (это можно настроить, как описано [ниже](#run-the-device-side-application)).
...
| `Port` | Локальный порт, к которому будет подключаться клиент SSH. В этом примере мы используем порт `2222`, но его можно заменить другими произвольными числами. |
...
| `RemoteHostName` | IP-адрес, на котором включен сервер SSH (это будет `localhost`, если IP-адрес тот, на котором запущен локальный прокси-сервер устройства). |
| `RemotePort` | Порт, используемый протоколом приложения (по умолчанию это будет порт 22 для SSH).  |
...
Вывод на консоль на стороне службы (локальный прокси-сервер службы прослушивает порт 2222): ![Замещающий текст](./media/quickstart-device-streams-proxy-csharp/service-console-output.png "Вывод локального прокси-сервера службы")

Вывод на консоль на локальном прокси-сервере устройства, который подключается к управляющей программе SSH по адресу <code>IP_address:22</code>: ![Замещающий текст](./media/quickstart-device-streams-proxy-csharp/device-console-output.png "Вывод локального прокси-сервера устройства")

Вывод на консоль клиентской программы SSH (клиент SSH связывается с управляющей программой SSH, подключаясь к порту 22, где прослушивает локальный прокси-сервер службы): ![Замещающий текст](./media/quickstart-device-streams-proxy-csharp/ssh-console-output.png "Вывод программы клиента SSH")
...
|----------------|-----------------|
| `DeviceConnectionString` | Строка подключения созданного ранее устройства. |
| `RemoteHostName` | IP-адрес, на котором включен сервер RDP (это будет `localhost`, если IP-адрес тот, на котором запущен локальный прокси-сервер устройства). |
| `RemotePort` | Порт, используемый протоколом приложения (по умолчанию это будет порт 3389 для RDP).  |
