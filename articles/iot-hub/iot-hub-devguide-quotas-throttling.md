...
| Передачи с устройства в облако | Более 100 операций отправки в секунду или 12 операций отправки в секунду на единицу <br/> Например, два модуля S1 имеют значение 2\*12 = 24/с, но у вас есть по крайней мере 100 операций отправки в секунду между вашими единицами. В девяти единицах S1 имеется 108 операций отправки в секунду (9\*12) по всем единицам. | 120 операций отправки в секунду на единицу | 6 000 операций отправки в секунду на единицу |
...
| Операция | Предельное количество |
| --------- | ----- |
...
| IoT Edge автоматические развертывания<sup>1</sup> | 20 модулей на развертывание. 100 развертывания (включая многоуровневые развертывания) для каждого центра платных номеров SKU. 10 развертываний на каждый центр свободных номеров SKU. |
