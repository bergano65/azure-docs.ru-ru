...
description: Узнайте о рекомендациях по управлению автоматическими устройствами для минимизации повторяющихся и сложных задач, связанных с управлением устройствами IoT в масштабе.
author: Philmea
ms.author: philmea
ms.date: 06/28/2019
ms.topic: conceptual
...
* **Оператор решений IoT:** После развертывания решения IoT требуется длительная эксплуатация, мониторинг, модернизация и техническое обслуживание. Эти задачи может выполнять команда внутри компании, в которую входят специалисты по информационным технологиям, группы по эксплуатации и обслуживанию оборудования, а также специалисты в определенной области, которые следят за работой всей инфраструктуры Интернета вещей.
...
Автоматическое управление устройством включает в себя множество преимуществ, связанных с [двойниками устройств](iot-hub-devguide-device-twins.md) и [двойниками модулей](iot-hub-devguide-module-twins.md), для синхронизации требуемых и сообщаемых состояний между облаком и устройствами. [Автоматические конфигурации устройств](iot-hub-auto-device-config.md) автоматически обновляют большие наборы близнецов и суммируют прогресс и соответствие требованиям. Ниже приведены основные действия по разработке и использованию автоматического управления устройствами.
...
* **Реализация [автоматических конфигураций устройств](iot-hub-auto-device-config.md).** Автоматические конфигурации устройств развертывают и отслеживают изменения конфигурации в больших наборах устройств Интернета вещей с использованием двойников устройств.

   Автоматические конфигурации устройств определяют наборы двойников устройств с помощью **целевого состояния**, которое является запросом к тегам двойников устройств или сообщаемым свойствам. **Целевое содержимое** — это набор требуемых свойств, который будет установлен в указанных двойниках устройств. Целевое содержимое должно соответствовать структуре двойников устройств, определяемой изготовителем оборудования Центра Интернета вещей или интегратором. **Метрики** представляют запросы по свойствам устройства, о которых сообщается, и должны также согласовываться с двойной структурой устройства, определяемой производителем/интегратором оборудования IoT.

   Автоматические конфигурации устройств работают впервые вскоре после создания конфигурации, а затем с интервалом в пять минут. Они также извлекают выгоду из IoT Hub выполнения устройств двойной операции со скоростью, которая никогда не будет превышать [регулирование пределы](iot-hub-devguide-quotas-throttling.md) для устройства близнецов читает и обновляет.
