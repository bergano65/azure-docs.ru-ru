...
Клиентский код, выполняемый на устройстве, `temperatureAlert`задает свойство приложения для каждого сообщения телеметрии, которое оно отправляет в центр Интернета вещей. Когда клиентский код обнаруживает температуру выше 30 C, он устанавливает для этого свойства значение `true`; в противном случае свойству присваивается значение `false`.
...
1. Щелкните **Создать**. Дождитесь завершения развертывания, прежде чем переходить к следующему шагу.
...
1. Щелкните **Создать**. После успешного создания конечной точки перейдите к следующему шагу.
