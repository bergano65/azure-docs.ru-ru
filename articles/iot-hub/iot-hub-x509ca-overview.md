...
Независимо от того, как вы получаете сертификат X.509 CA, убедитесь, что сохранить его соответствующие частные ключ секрет и защищены во все времена.  Это необходимо для создания отношений доверия при проверке подлинности на основе ЦС X.509.
...
Сертификат устройства (также называемый сертификатом листа) должен иметь набор *имя объекта* для **идентификатора устройства,** который использовался при регистрации устройства IoT в концентраторе Azure IoT. Эта настройка необходима для проверки подлинности.
...
Узнайте здесь, как [зарегистрировать сертификат CA](iot-hub-security-x509-get-started.md#register-x509-ca-certificates-to-your-iot-hub)
