...
* **Полевые шлюзы**. Протоколы MQTT и HTTPS не позволяют подключать по TLS-соединению одновременно несколько устройств (на каждом из которых используются собственные учетные данные). Эти протоколы не вполне подходят для [ситуаций с использованием полевого шлюза](iot-hub-devguide-endpoints.md#field-gateways), так как требуют отдельного TLS-подключения между полевым шлюзом и Центром Интернета вещей для каждого подключенного устройства.
