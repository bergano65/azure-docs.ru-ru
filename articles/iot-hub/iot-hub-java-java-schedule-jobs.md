...
* Прямые методы: [Общие сведения о прямых методах и информация о вызове этих методов из Центра Интернета вещей](iot-hub-devguide-direct-methods.md) и [Использование прямых методов на устройстве Интернета вещей (Node.js)](quickstart-control-device-java.md).
...
* Создайте приложение бэк-энда, которое создает задание для вызова прямого метода **lockDoor** на нескольких устройствах. Еще одно задание отправляет обновления требуемых свойств на несколько устройств.
...
**расписание рабочих мест,** которые используют задания для вызова **lockDoor** прямого метода и обновления устройства близнецов желаемых свойств на нескольких устройствах.
...
1. На машине разработки создайте пустую папку под названием **iot-java-schedule-jobs.**

2. В папке **iot-java-schedule-jobs** создайте проект Maven, **называемый расписанием,** используя следующую команду в вашей команде. Обратите внимание, что это одна длинная команда.

   ```cmd/sh
   mvn archetype:generate -DgroupId=com.mycompany.app -DartifactId=schedule-jobs -DarchetypeArtifactId=maven-archetype-quickstart -DinteractiveMode=false
   ```

3. В запросе команды перейдите к папке **расписания рабочих мест.**

4. Используя текстовый редактор, откройте файл **pom.xml** в папке **расписания и** добавьте следующую зависимость в узло **зависимостей.** Эта зависимость позволяет использовать пакет **iot-service-client** в приложении для связи с концентратором IoT:
...
7. Используя текстовый редактор, откройте **файл расписания-задания.**
...
9. Добавьте следующие переменные уровня класса в класс **Приложения.** Замените `{youriothubconnectionstring}` строку подключения концентратора IoT, которую вы скопировали ранее в [строке подключения концентратора IoT:](#get-the-iot-hub-connection-string)
...
15. Чтобы последовательно запускать и контролировать две задания, замените код в **основном** методе следующим кодом:
...
16. Сохранить и закрыть **расписание рабочих мест src-main-java-com-mycompany-app-App.java** файл

17. Создайте приложение **schedule-jobs** и исправьте обнаруженные ошибки. В запросе команды перейдите к папке **расписания и** заданиям и запустите следующую команду:
...
1. В папке **iot-java-schedule-jobs** создайте проект Maven, называемый **смоделированным устройством,** используя следующую команду в запросе команды. Обратите внимание, что это одна длинная команда.
...
3. Используя текстовый редактор, откройте файл **pom.xml** в папке **с моделируемым устройством** и добавьте следующие зависимости в узло **зависимостей.** Эта зависимость позволяет использовать пакет **iot-устройство-клиент** в приложении для связи с концентратором IoT:
...
9. Добавьте следующие переменные уровня класса в класс **Приложения.** Заменить `{yourdeviceconnectionstring}` строку подключения устройства, скопированную ранее в [Регистре, новое устройство в разделе концентратора IoT:](#register-a-new-device-in-the-iot-hub)
...
14. Замените код в **основном** методе следующим кодом на:
...
17. Сохранить и закрыть **смоделированное устройство src-main-java-com-mycompany-app.java** файл.

18. Создайте приложение **simulated-device** и исправьте все ошибки. В запросе команды перейдите к папке **с моделируемым устройствами** и запустите следующую команду:
...
1. В запросе команды в папке **с моделируемым устройством** запустите следующую команду, чтобы запустить приложение устройства, прослушивающее для желаемых изменений свойств и прямых вызовов метода:
