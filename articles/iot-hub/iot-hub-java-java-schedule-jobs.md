...
* Создайте серверное приложение, которое создает задание для вызова прямого метода **lockDoor** на нескольких устройствах. Еще одно задание отправляет обновления требуемых свойств на несколько устройств.
...
**Schedule — задания** , которые используют задания для вызова прямого метода **lockDoor** и обновления требуемых свойств двойникаа устройства на нескольких устройствах.
...
1. На компьютере разработки создайте пустую папку с именем **IOT-Java-Scheduler-Jobs**.

2. В папке **IOT-Java-Schedule-Jobs** создайте проект Maven с именем **Schedule-Jobs** , выполнив в командной строке следующую команду. Обратите внимание, что это одна длинная команда.

   ```cmd/sh
   mvn archetype:generate -DgroupId=com.mycompany.app -DartifactId=schedule-jobs -DarchetypeArtifactId=maven-archetype-quickstart -DinteractiveMode=false
   ```

3. В командной строке перейдите в папку **Schedule-Jobs** .

4. С помощью текстового редактора откройте файл **POM. XML** в папке **Schedule-Jobs** и добавьте следующую зависимость в узел **зависимости** . Эта зависимость позволяет использовать в приложении пакет **IOT-Service-Client** для взаимодействия с центром Интернета вещей:
...
7. Откройте файл **счедуле-жобс\срк\маин\жава\ком\микомпани\апп\апп.Жава** в текстовом редакторе.
...
9. Добавьте следующие переменные уровня класса в класс **app** . Замените `{youriothubconnectionstring}` строкой подключения к центру Интернета вещей, скопированной ранее в [поле получение строки подключения для центра Интернета вещей](#get-the-iot-hub-connection-string):
...
15. Чтобы последовательно запустить и отслеживать два задания, замените код в методе **Main** следующим кодом:
...
16. Сохранение и закрытие файла **счедуле-жобс\срк\маин\жава\ком\микомпани\апп\апп.Жава**

17. Создайте приложение **schedule-jobs** и исправьте обнаруженные ошибки. В командной строке перейдите в папку **Schedule-Jobs** и выполните следующую команду:
...
9. Добавьте следующие переменные уровня класса в класс **app** . Замените `{yourdeviceconnectionstring}` строкой подключения устройства, скопированной ранее в разделе [Регистрация нового устройства в центре Интернета вещей](#register-a-new-device-in-the-iot-hub) :
...
1. В командной строке в папке **имитации устройства** выполните следующую команду, чтобы запустить приложение устройства, слушающее требуемые изменения свойств и прямые вызовы методов:
...
* Отправка данных телеметрии с устройств с помощью учебника [Приступая к работе с центром Интернета вещей](quickstart-send-telemetry-java.md) .
