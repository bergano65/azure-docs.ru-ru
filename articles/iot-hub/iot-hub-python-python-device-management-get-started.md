...
1. Добавьте следующие инструкции `import` в начале файла **dmpatterns_getstarted_device.js**.
...
1. Добавьте переменные, в том числе переменную **CONNECTION_STRING** и инициализацию клиента.  Замените connection string строкой подключения своего устройства.  
...
1. Добавьте следующую функцию обратных вызовов, чтобы реализовать прямой метод на устройстве.
...
1. Запустите прослушиватель прямого метода и подождите.
...
1. Сохраните и закройте файл **dmpatterns_getstarted_device.py**.
...
1. Добавьте следующие инструкции `import` в начале файла **dmpatterns_getstarted_service.py**.
...
1. Добавьте следующие объявления переменных. Только замените значения заполнителей для параметров _IoTHubConnectionString_ и _deviceId_.
...
1. Добавьте следующую функцию для вызова метода устройства, чтобы перезагрузить целевое устройство, а затем выполнить запрос к двойникам устройства и получить сведения о времени последней перезагрузки.
...
1. Сохраните и закройте файл **dmpatterns_getstarted_service.py**.
...
1. В консоли отобразится ответ устройства на прямой метод.
