...
* Двойники устройств и свойства: [Приступая к работе с двойниками устройств (предварительная версия)](iot-hub-node-node-twin-getstarted.md) и [Руководство. Настройка устройств с помощью требуемых свойств (предварительная версия)](tutorial-device-twins.md).

* Прямые методы: [Руководство разработчика IoT Hub - прямые методы](iot-hub-devguide-direct-methods.md) и [учебник: прямые методы](quickstart-control-device-node.md)
...
* **simDevice.js**, который подключается к вашему концентратору IoT с идентификацией устройства и получает прямой метод **lockDoor.**
...
5. Добавьте переменную **connectionString**, чтобы создать с ее помощью экземпляр **клиента**. Замените `{yourDeviceConnectionString}` значение заполнителя строкой соединения устройства, скопиваемым ранее.
...
В этом разделе вы создаете консольное приложение Node.js, которое инициирует удаленную **блокировку** на устройстве с помощью прямого метода и обновляет свойства близнеца устройства.
...
4. Добавьте следующие "требующие" выписки в начале файла **scheduleJobService.js:**
...
5. Добавьте следующие объявления переменных. Замените `{iothubconnectionstring}` значение заполнителя значением, скопированным в [строке подключения концентратора IoT.](#get-the-iot-hub-connection-string) Если вы зарегистрировали устройство, отличающееся от **myDeviceId,** не забудьте изменить его в состоянии запроса.
...
3. Вы видите реакцию устройства на прямой метод и состояние задания в консоли.

   Ниже показана реакция устройства на прямой метод:

   ![Вывод приложения с моделируемым устройством](./media/iot-hub-node-node-schedule-jobs/sim-device.png)

   Ниже показаны задания планирования службы для прямого обновления метода и устройства, а также задания, выполняемые до завершения:
...
Чтобы продолжить работу с IoT Концентратор и шаблоны управления устройствами, такие как удаленное обновление прошивки воздуха, [см.](tutorial-firmware-update.md)

Чтобы продолжить знакомство с Центром Интернета вещей, см. сведения в статье [Приступая к работе с архитектурой Azure IoT Edge в Linux](../iot-edge/tutorial-simulate-device-linux.md).
