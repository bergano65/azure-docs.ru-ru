---
title: Потоки концентратора Azure IoT Документы Майкрософт
description: Обзор потоков устройств Azure IoT Hub, которые облегчают безопасные двунаправленные туннели TCP для различных сценариев связи между облаками и устройствами.
author: robinsh
services: iot-hub
ms.service: iot-hub
ms.topic: conceptual
...
!["Обзор потоков устройств IoT Hub"](./media/iot-hub-device-streams-overview/iot-hub-device-streams-overview.png )
...
* **Безопасное подключение, благоприятное для брандмауэра:** Устройства IoT могут быть достигнуты из конечных точек обслуживания без открытия входящего порта брандмауэра на устройстве или в сетевых периметрах (только исходящие подключения к IoT Концентратору необходимо через порт 443).

* **Аутентификация:** Как устройства, так и стороны обслуживания туннеля должны аутентифицироваться с помощью IoT Hub, используя соответствующие учетные данные.

* **Шифрование:** По умолчанию потоки устройств IoT Hub используют соединения с поддержкой TLS. Это гарантирует, что трафик всегда шифруется, независимо от того, использует ли приложение шифрование.

* **Простота подключения:** Во многих случаях использование потоков устройств устраняет необходимость сложной настройки виртуальных частных сетей для обеспечения подключения к устройствам IoT.

* **Совместимость со стеком TCP/IP:** Потоки устройств IoT Hub могут вместить трафик приложений TCP/IP. Это означает, что эту функцию может использовать широкий диапазон запатентованных протоколов, а также протоколов, основанных на стандартах.

* **Простота использования в частных настройках сети:** Служба может общаться с устройством, ссылаясь на идентификатор устройства, а не на IP-адрес устройства. Это полезно в ситуациях, когда устройство находится в частной сети и имеет частный IP-адрес или когда IP-адрес назначается динамически и неизвестен на стороне службы.

## <a name="device-stream-workflows"></a>Рабочие потоки устройств
...
!["Процесс рукопожатия потока устройства"](./media/iot-hub-device-streams-overview/iot-hub-device-streams-handshake.png)
...
!["Конечные точки потока устройств"](./media/iot-hub-device-streams-overview/device-stream-in-portal.png)

Кроме того, информация о конечных точках может быть получена с помощью Azure CLI в разделе свойств концентратора, в частности, `property.hostname` и `property.deviceStreams` клавишах.
...
> [!NOTE]
> Убедитесь, что вы установили Azure CLI 2.0.57 или новее. Вы можете скачать последнюю версию со страницы [Install Azure CLI.](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest)
>
...
Как упоминалось в начале этой статьи, устройство создает исходящие соединения с IoT Концентр потоковой конечной точки во время процесса инициации потоков устройства. Ваши брандмауэры устройства или сети должны разрешать исходящие подключения к шлюзу потоковой передачи через порт 443 (обратите внимание, что обмен данными осуществляется через подключение WebSocket, зашифрованное с помощью протокола TLS).

Хостимя конечной точки потоковой передачи устройства можно найти на портале Azure IoT Hub под вкладкой «Обзорный ![поток».](./media/iot-hub-device-streams-overview/device-stream-in-portal.png)
...
Можно настроить журналы Azure Monitor для сбора журналов активности потоков устройств в концентраторе IoT. Это может быть очень полезно в сценариях устранения неполадок.

Выполните ниже шаги для настройки журналов Azure Monitor для действий потока потоков устройств вашего Концентратора:

1. Перейдите на вкладку *Параметры диагностики* в Центре Интернета вещей и щелкните ссылку *Включить диагностику*.

   !["Включение журналов диагностики"](./media/iot-hub-device-streams-overview/device-streams-diagnostics-settings-pane.png)

2. Укажите имя для параметров диагностики и выберите параметр *Отправить в Log Analytics*. Вы будете руководствоваться, чтобы выбрать существующий ресурс рабочего пространства журнала Analytics или создать новый. Кроме того, установите флажок напротив *DeviceStreams* в списке.

    !["Включить журналы потоков устройств"](./media/iot-hub-device-streams-overview/device-streams-configure-diagnostics.png)
...
3. Теперь вы можете просматривать журналы потоков устройств на вкладке *Журналы* на портале Центра Интернета вещей. Журналы действий потоков устройств будут отображаться в таблице `AzureDiagnostics`, и для них установлено значение `Category=DeviceStreams`.
   ...
   Как показано ниже, личность целевого устройства и результат операции также доступны в журналах.

   !["Системы потока доступа к устройствам"](./media/iot-hub-device-streams-overview/device-streams-view-logs.png)

## <a name="regional-availability"></a>Доступность по регионам

Во время публичного предварительного просмотра потоки устройств IoT Hub доступны в центральных регионах США, Центральной Части США EUAP, Северной Европе и юго-восточной Азии. Убедитесь, что центр создан в одном из этих регионов.
...
## <a name="iot-hub-device-stream-samples"></a>Образцы потока устройств IoT концентратора

На странице IoT Hub есть два [образца быстрого запуска.](/azure/iot-hub) Они демонстрируют использование потоков устройств приложениями.

* Образец *эха* демонстрирует программное использование потоков устройств (позвонив напрямую sDK API).
...
В примере echo показано программное использование потоков устройства для отправки и получения байтов между службой и приложением устройства. Обратите внимание, что вы можете использовать программы обслуживания и устройства на разных языках. Например, можно использовать программу C-устройства с помощью программы обслуживания C.

Вот образцы эхо:

* [Программа обслуживания и обслуживания](quickstart-device-streams-echo-csharp.md)

* [Программа обслуживания Node.js](quickstart-device-streams-echo-nodejs.md)

* [C программа устройства](quickstart-device-streams-echo-c.md)

### <a name="local-proxy-sample-for-ssh-or-rdp"></a>Локальная выборка прокси (для SSH или RDP)
...
!["Настройка прокси-потока устройства для SSH/RDP"](./media/iot-hub-device-streams-overview/iot-hub-device-streams-ssh.png)
...
* Выбор порта 22 зависит от протокола и специфичен для SSH в этом случае. В случае с RDP необходимо использовать порт 3389. Это можно настроить в предоставленных примерах программ.
...
* [Программа обслуживания и обслуживания](quickstart-device-streams-proxy-csharp.md)

* [Программа обслуживания Node.js](quickstart-device-streams-proxy-nodejs.md)

* [C программа устройства](quickstart-device-streams-proxy-c.md)

## <a name="next-steps"></a>Дальнейшие действия

Используйте ссылки ниже, чтобы узнать больше о потоках устройств.

> [!div class="nextstepaction"]
> [Потоки устройств на IoT-шоу (канал 9)](https://nam06.safelinks.protection.outlook.com/?url=https%3A%2F%2Fchannel9.msdn.com%2FShows%2FInternet-of-Things-Show%2FAzure-IoT-Hub-Device-Streams&data=02%7C01%7Crezas%40microsoft.com%7Cc3486254a89a43edea7c08d67a88bcea%7C72f988bf86f141af91ab2d7cd011db47%7C1%7C0%7C636831125031268909&sdata=S6u9qiehBN4tmgII637uJeVubUll0IZ4p2ddtG5pDBc%3D&reserved=0)
