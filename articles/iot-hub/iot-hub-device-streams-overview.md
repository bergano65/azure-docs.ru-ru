...
Вы можете настроить журналы Azure Monitor, чтобы получить журнал действий потоков устройств в центре Интернета вещей. Это может быть очень полезно в сценариях устранения неполадок.

Выполните следующие действия, чтобы настроить журналы Azure Monitor для действий потока устройства центра Интернета вещей:
...
2. Укажите имя для параметров диагностики и выберите параметр *Отправить в Log Analytics*. Вам будет предложено выбрать существующий ресурс Log Analytics рабочей области или создать новый. Кроме того, установите флажок напротив *DeviceStreams* в списке.

    !["Включить журналы потоков устройства"](./media/iot-hub-device-streams-overview/device-streams-configure-diagnostics.png)
...
* [C#Программа обслуживания и обслуживания](quickstart-device-streams-proxy-csharp.md)
