...
![«Обзор потоков устройства центра Интернета вещей»](./media/iot-hub-device-streams-overview/iot-hub-device-streams-overview.png )
...
## <a name="device-stream-workflows"></a>Рабочие процессы устройства потока
...
![«Процесс подтверждения поток устройства»](./media/iot-hub-device-streams-overview/iot-hub-device-streams-handshake.png)
...
> [!NOTE]
> Убедитесь, что вы установили Azure CLI 2.0.57 или новее. Можно загрузить последнюю версию из [установить Azure CLI](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest) страницы.
>

## <a name="allow-outbound-connectivity-to-the-device-streaming-endpoints"></a>Разрешить исходящие подключения к устройству, конечные точки потоковой передачи

Как упоминалось в начале этой статьи, устройство создает исходящее подключение к конечной точке потоковой передачи, центр Интернета вещей во время процесса запуска потоков устройства. Ваши брандмауэры устройства или сети должны разрешать исходящие подключения к шлюзу потоковой передачи через порт 443 (обратите внимание, что обмен данными осуществляется через подключение WebSocket, зашифрованное с помощью протокола TLS).

Имя узла конечной точки потоковой передачи устройства можно найти на портале Центра Интернета вещей Azure на вкладке "Обзор". ![«Конечные точки потока на устройство»](./media/iot-hub-device-streams-overview/device-stream-in-portal.png)
...
3. Теперь вы можете просматривать журналы потоков устройств на вкладке *Журналы* на портале Центра Интернета вещей. Журналы действий потоков устройств будут отображаться в таблице `AzureDiagnostics`, и для них установлено значение `Category=DeviceStreams`.
   ...
   Как показано ниже, удостоверение на целевом устройстве и результат операции также доступно в журналах.
...
## <a name="iot-hub-device-stream-samples"></a>Примеры поток устройств центра Интернета вещей

Существует два [примеров](/azure/iot-hub) предлагаемые на этой странице центра Интернета вещей. Они демонстрируют использование потоков устройства приложениями.
...
В примере echo показано программное использование потоков устройства для отправки и получения байтов между службой и приложением устройства. Обратите внимание на то, что можно использовать программы, службы и устройства на разных языках. Например, можно использовать программу C устройств с C# служебной программы.

Ниже приведены примеры echo.

* [C#службы и служебной программы](quickstart-device-streams-echo-csharp.md)

* [Служебная программа node.js](quickstart-device-streams-echo-nodejs.md)

* [Устройства программы на языке C](quickstart-device-streams-echo-c.md)

### <a name="local-proxy-sample-for-ssh-or-rdp"></a>Образец локальный прокси-сервера (для SSH или удаленного рабочего СТОЛА)
...
![«Устройство настройки прокси-сервера поток для SSH/RDP»](./media/iot-hub-device-streams-overview/iot-hub-device-streams-ssh.png)
...
* Выбор порта 22 в данном случае это зависит от протокола и относящиеся к SSH. В случае с RDP необходимо использовать порт 3389. Это можно настроить в предоставленных примерах программ.
...
* [C#службы и служебной программы](quickstart-device-streams-proxy-csharp.md)

* [Служебная программа node.js](quickstart-device-streams-proxy-nodejs.md)
...
> [Потоки данных устройств Интернета вещей Показать (Channel 9)](https://nam06.safelinks.protection.outlook.com/?url=https%3A%2F%2Fchannel9.msdn.com%2FShows%2FInternet-of-Things-Show%2FAzure-IoT-Hub-Device-Streams&data=02%7C01%7Crezas%40microsoft.com%7Cc3486254a89a43edea7c08d67a88bcea%7C72f988bf86f141af91ab2d7cd011db47%7C1%7C0%7C636831125031268909&sdata=S6u9qiehBN4tmgII637uJeVubUll0IZ4p2ddtG5pDBc%3D&reserved=0)