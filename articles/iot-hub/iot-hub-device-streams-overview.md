...
- **Простота подключения.** Благодаря потокам устройств устраняется необходимость в сложной установке виртуальных частных сетей для подключения к устройствам Интернета вещей.
...
5. Служба уведомляется о результатах принятия потока устройством и устанавливает собственное WebSocket-подключение к конечной точке потоковой передачи. Аналогичным образом она получает URL-адрес конечной точки потоковой передачи и информацию об аутентификации из Центра Интернета вещей.
...
Здесь приведены два примера для демонстрации использования потоков устройств приложениями. В примере *echo* демонстрируется программное использование потоков устройств (путем вызова API пакета SDK). В примере с *локальным прокси-сервером* демонстрируется использование функции пакета SDK для туннелирования трафика (такого как SSH, RDP или веб) готового приложения через потоки устройств.

### <a name="echo-sample"></a>Пример echo
В примере echo показано программное использование потоков устройства для отправки и получения байтов между службой и приложением устройства. По приведенным ниже ссылкам можно перейти к кратким руководствам (вы можете использовать программы службы и устройства на разных языках, например, программа устройства на языке C может работать со служебной программой на языке C#):
...
| NodeJS | [Ссылка](quickstart-device-streams-echo-nodejs.md) | -                                                        |
...
В этом разделе описывается использование потоков устройства для реализации сценариев SSH (случай с RDP или другими протоколами клиента или сервера аналогичен использованию соответствующего порта протокола).

В этой конфигурации используются две программы *локального прокси-сервера* (показаны на рисунке ниже), а именно: *локальный прокси-сервер устройства* и *локальный прокси-сервер службы*. Локальные прокси отвечают за выполнение [подтверждения запуска потока устройства](#Device-stream-creation-flow) с помощью Центра Интернета вещей и взаимодействуют с клиентом и управляющей программой SSH, используя обычное программирование сокета клиента и сервера.
...
2. Устройство принимает инициацию потоковой передачи, а к конечной точке потоковой передачи Центра Интернета вещей устанавливается туннель (как обсуждалось выше).
...
4. Локальный прокси-сервер службы ожидает передачи данных на указанный порт, в частности новых SSH-подключений от пользователя (в примере используется произвольный порт — 2222). Пользователь указывает клиент SSH на порте локального прокси-сервера в localhost.

### <a name="notes"></a>Примечания
- На описанных выше шагах создается сквозной туннель между клиентом SSH (справа) и управляющей программой SSH (слева). 
...
Воспользуйтесь ссылками ниже, чтобы узнать, как запустить программы локального прокси-сервера на выбранном языке. Аналогично примеру echo можно запустить локальные прокси-серверы службы и устройства на разных языках, так как они полностью совместимы.
