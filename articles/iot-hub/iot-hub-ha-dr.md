...
>
> - При маршрутизации в хранилище мы рекомендуем прикрепить контейнер хранилища, а затем выполнить итерацию по ним, чтобы убедиться, что все контейнеры считаны без принятия каких либо предположений раздела. Диапазон секций может измениться во время отработки отказа, инициированного Майкрософт, или при отработке отказа вручную. Чтобы узнать, как перечислить список BLOB-объектов, см. раздел [Маршрутизация в службу хранилища Azure](iot-hub-devguide-messages-d2c.md#azure-storage).
