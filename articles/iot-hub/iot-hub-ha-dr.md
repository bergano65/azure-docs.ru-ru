...
### <a name="manual-failover-preview"></a>Отработка отказа вручную (предварительная версия)
...
| Отработка отказа вручную |От 10 мин до 2 ч|См. сведения в таблице RPO выше|Да|Очень низкая. Необходимо просто запустить эту операцию на портале.|Нет|
| Обеспечение высокого уровня доступности между регионами |< 1 мин|Зависит от частоты репликации пользовательского решения обеспечения высокого уровня доступности|Нет|Высокая|> 1x стоимости 1 Центра Интернета вещей|
