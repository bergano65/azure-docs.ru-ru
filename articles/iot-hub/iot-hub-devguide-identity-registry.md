...
Более сложная реализация может включать сведения из [Azure Monitor](../azure-monitor/index.yml) и [работоспособность ресурсов Azure](../service-health/resource-health-overview.md) для выявления устройств, пытающихся подключаться или обмениваться данными, но при этом происходит сбой. Дополнительные сведения см. в статье [мониторинг центра Интернета вещей](monitor-iot-hub.md) и [Проверка работоспособности ресурсов центра Интернета вещей](iot-hub-azure-service-health-integration.md#check-health-of-an-iot-hub-with-azure-resource-health). Внедряя в решение шаблон пульса, учитывайте [квоты и ограничения Центра Интернета вещей](iot-hub-devguide-quotas-throttling.md).
...
| lastActivityTime |Только для чтения |Временной индикатор, показывающий дату и время последнего подключения устройства, получения сообщения на устройство и отправки сообщения с него. Это свойство в конечном итоге согласуется, но может быть задержано до 5 – 10 минут. По этой причине ее не следует использовать в рабочих сценариях. |
