...
| Передача файлов | Передача файлов в службу хранилища Azure не тарифицируется Центром Интернета вещей. Сообщения об инициации и завершении передачи файлов тарифицируются по фактически переданному объему сообщений с шагом в 4 КБ. Например, передача файла объемом 10 МБ взимается как два сообщения в дополнение к стоимости хранилища Azure. |
| Прямые методы | Успешные запросы метода взимаются в 4-КБ куски, и ответы взимаются в 4-КБ куски в качестве дополнительных сообщений. Запросы на отключенные устройства тарифицируются как сообщения блоками по 4 КБ. Например, метод с телом 4-КБ, который приводит к ответу без тела от устройства, заряжается как два сообщения. Метод с текстом размером 6 КБ, который возвращает ответ размером 1 КБ от устройства, тарифицируется как два сообщения запроса плюс еще одно сообщение для ответа. |
...
Каждую минуту устройство отправляет в Центр Интернета вещей одно сообщение из устройства в облако размером 1 КБ, которое затем считывается службой Azure Stream Analytics. Задняя часть решения вызывает метод (с полезной нагрузкой 512 байт) на устройство каждые 10 минут, чтобы вызвать определенное действие. Устройство отвечает на метод с результатом в 200 байт.
