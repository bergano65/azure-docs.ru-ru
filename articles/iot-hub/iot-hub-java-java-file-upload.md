...
[Телеметрия отправки с устройства на быстрый запуск концентратора IoT](quickstart-send-telemetry-java.md) и [отправка сообщений об об лактапой](iot-hub-java-java-c2d.md) к устройству с помощью учебника IoT Hub показывает основную функциональность обмена сообщениями от IoT и облачных сообщений от облака к устройству. В статье [Руководство. Настройка маршрутизации сообщений с Центром Интернета вещей](tutorial-routing.md) описан способ надежного хранения сообщений, отправляемых с устройства в облако, в хранилище BLOB-объектов Azure. Тем не менее в некоторых случаях не просто сопоставить данные, отправляемые устройством, с относительно небольшими сообщениями, отправляемыми из устройства в облако, которые принимает Центр Интернета вещей. Пример:
...
* **смоделированная ,** модифицированная версия приложения, созданная в «Отправить сообщения облачного с устройства с помощью учебника IoT Hub». Это приложение передает файл в хранилище с помощью универсального кода ресурса (URI) SAS, предоставленного вашим Центром Интернета вещей.
...
В этом разделе вы измените приложение устройства, созданное в [сообщении от облака к устройству с помощью IoT Hub,](iot-hub-java-java-c2d.md) чтобы загрузить файл в концентратор IoT.
...
7. Добавьте следующие переменные уровня класса в класс **Приложения.** Замените `{Your IoT Hub connection string}` значение заполнителя строкой подключения концентратора IoT, которую вы скопировали ранее в [строке подключения концентратора IoT:](#get-the-iot-hub-connection-string)
