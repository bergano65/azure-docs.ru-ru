...
Этот учебник основан на коде в [отправки сообщений из облака на устройство с центром Интернета вещей](iot-hub-java-java-c2d.md) руководству, чтобы показать, как использовать [возможности центра Интернета вещей для передачи файлов](iot-hub-devguide-file-upload.md) отправить файл в [BLOB-объектов Azure хранилище](../storage/index.yml). В этом руководстве описаны следующие процедуры.
...
[Отправки данных телеметрии с устройства в центре Интернета вещей](quickstart-send-telemetry-java.md) быстрого запуска и [отправки сообщений из облака на устройство с центром Интернета вещей](iot-hub-java-java-c2d.md) руководстве демонстрации базовых функций из устройства в облако и облаком и устройством обмена сообщениями Интернета вещей Концентратор. В статье [Руководство. Настройка маршрутизации сообщений с Центром Интернета вещей](tutorial-routing.md) описан способ надежного хранения сообщений, отправляемых с устройства в облако, в хранилище BLOB-объектов Azure. Тем не менее в некоторых случаях не просто сопоставить данные, отправляемые устройством, с относительно небольшими сообщениями, отправляемыми из устройства в облако, которые принимает Центр Интернета вещей. Пример:
...
* **simulated-device**, измененную версию приложения, созданного в руководстве [Send cloud-to-device messages с центром Интернета вещей]. Это приложение передает файл в хранилище с помощью универсального кода ресурса (URI) SAS, предоставленного вашим Центром Интернета вещей.
...
В этом разделе вы измените приложение устройства, созданный в [отправки сообщений из облака на устройство с центром Интернета вещей](iot-hub-java-java-c2d.md) для отправки файла в центр Интернета вещей.
