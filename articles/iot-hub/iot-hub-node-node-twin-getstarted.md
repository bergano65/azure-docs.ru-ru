...
4. Добавьте следующий код в файл **AddTagsAndQuery. js** . Замените `{iot hub connection string}` строкой подключения центра Интернета вещей, скопированной в [поле получение строки подключения центра Интернета вещей](#get-the-iot-hub-connection-string).
...
4. Добавьте следующий код в файл **ReportConnectivity. js** . Замените `{device connection string}` строкой подключения устройства, скопированным при создании удостоверения устройства **myDeviceId** в окне [Регистрация нового устройства в центре Интернета вещей](#register-a-new-device-in-the-iot-hub).
