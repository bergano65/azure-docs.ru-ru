...
> [!NOTE]
> С 11 марта 2019 года функция мониторинга операций удаляется из интерфейса портала Azure концентратора IoT. Приведенные ниже шаги больше не применяются. Чтобы мигрировать, убедитесь, что правильные категории включены в диагностических настройках Azure Monitor выше.

После тестирования новых параметров диагностики в рабочем процессе можно отключить функцию мониторинга операций. 
...
Кроме того, настройки диагностики предлагают пять новых категорий для отслеживания. После обновления приложений для имеющейся схемы, добавьте также новые категории:
...
Для мониторинга подключения и отключения событий устройства в производстве рекомендуется подписаться на [ **отключенное устройство** в](iot-hub-event-grid.md#event-types) Event Grid, чтобы получать оповещения и контролировать состояние соединения устройства. Используйте это [руководство](iot-hub-how-to-order-connection-state-events.md), чтобы узнать, как интегрировать события подключения и отключения устройства из Центра Интернета вещей в решение Интернета вещей.
