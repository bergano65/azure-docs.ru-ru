...
В первой статье этой серии появились [Общие сведения о пакете SDK для устройств Azure IOT для C](iot-hub-device-sdk-c-intro.md). В следующей статье представлено более подробное описание [пакета SDK для устройств Azure IOT для C--IoTHubClient](iot-hub-device-sdk-c-iothubclient.md). Этой статьей завершается изучение пакета SDK, в ней более подробно рассматривается оставшийся компонент — библиотека **serializer** .
...
Мы отправляем температуру, имеющую тип **температуривент**, и эта структура содержит элемент **температуры** и **времени** . Это напрямую отражается в сериализованных данных.
...
В этом случае мы исключили макросы **DECLARE\_STRUCT** и просто определяем элементы данных из нашего сценария, используя простые типы из языка моделирования.
...
Взгляните на эту модель в контексте объектно-ориентированного подхода. В этом случае мы моделируем физическое устройство (термостата), и это устройство включает такие атрибуты, как **температура** и **влажность**.
...
В результате будет создано такое же сериализованное событие, как и при определении **TemperatureEvent** с атрибутами **Temperature** и **Time**(модель 1). В этом случае мы смогли создать точно такое же сериализованное событие с помощью другой модели (модель 2), так как мы назвали **сериализацию** другим способом.
...
До сих пор в этой статье рассказывалось только об отправке событий в Центр Интернета вещей, но не об их получении. Причина этого заключается в том, что нам нужно знать о получении сообщений, как описано в статье [пакет SDK для устройств Azure IOT для C](iot-hub-device-sdk-c-intro.md). В этой статье вы помните, что вы обрабатываете сообщения путем регистрации функции обратного вызова сообщения:
...
Если вы клонировать репозиторий Azure-IOT-SDK-c из GitHub и выпустили команду `git submodule update --init`, вы найдете эту общую библиотеку служебной программы здесь:
...
Двумя ключевыми параметрами, с которыми следует думать, являются **nArithmetic** и **nMacroParameters**, которые определены в этих двух строках в макросах\_utils.TT:
...
Кроме того, в этой серии завершается серия из трех частей, посвященных разработке приложений с помощью **пакета SDK для устройств Azure IOT для C**. Эта информация должна быть достаточной, чтобы не только приступить к работе, но и получить полное представление о принципах работы API. Дополнительные сведения можно получить из нескольких примеров в пакете SDK, не описанных в этой статье. [Документация по пакету SDK для устройств Azure IoT](https://github.com/Azure/azure-iot-sdk-c) — хороший источник дополнительной информации.
