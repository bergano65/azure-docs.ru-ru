...
В этом случае мы исключили макросы **объявления\_структуры** и просто определяем элементы данных из нашего сценария, используя простые типы из языка моделирования.
...
Программа в этом решении создает файл **macro\_utils.h**. В пакет SDK входит файл\_макро utils. h, который используется по умолчанию. Это решение позволяет изменить ряд параметров, а затем повторно создать файл заголовка с учетом этих изменений.

Двумя ключевыми параметрами, с которыми следует думать, являются **nArithmetic** и **nMacroParameters**, которые определены в этих двух строках в\_макросах utils.TT:
...
Аналогично, когда вы закончите работу с библиотекой, последний вызов, который вы сделаете, заключается в **деинициализации сериализатора\_**:
