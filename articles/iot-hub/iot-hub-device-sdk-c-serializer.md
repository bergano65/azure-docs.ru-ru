...
Первая статья в этой серии представила Введение в [Azure IoT устройство SDK для C](iot-hub-device-sdk-c-intro.md). В следующей статье содержится более подробное описание [устройства SDK Azure IoT для C - IoTHubClient](iot-hub-device-sdk-c-iothubclient.md). Этой статьей завершается изучение пакета SDK, в ней более подробно рассматривается оставшийся компонент — библиотека **serializer** .
...
Во [вводной](iot-hub-device-sdk-c-intro.md) статье этого курса вы познакомились с языком моделирования **пакета SDK для устройств Azure IoT для C** на примере приложения **simplesample\_amqp**.
...
Все, что происходит «внутри» пространства имен, определяет модель. В данном случае создается одна модель для анемометра. Опять-таки, модели можно присвоить любое имя, но обычно оно соответствует имени устройства или типу данных для взаимодействия с Центром Интернета вещей.  
...
В этой выборке не показаны дополнительные типы данных, поддерживаемые SDK. Об этом мы поговорим далее.
...
| int8\_t |8-битный многокомнатный |
| int16\_t |16-битный многораз |
...
Начнем с последнего типа данных. Макрос **DECLARE\_STRUCT** позволяет определять сложные типы данных, которые являются группировками других простых типов. Эти группировки позволяют определить модель, которая выглядит следующим образом.
...
В основном, мы присваиваем значение каждому члену структуры **тестирования,** а затем вызываем **SendAsync** для отправки события данных **теста** в облако. **SendAsync** является вспомогательной и служит для отправки одного события, инициируемого изменением данных, в Центр Интернета вещей:
...
Используя эту информацию, вы сможете определять модели с использованием различных типов данных, включая сложные типы (мы даже можем включить одни сложные типы в другие). Но сериализованный код JSON, созданный в примере, приведенном выше, вызывает один важный вопрос. *Каким образом* при отправке данных библиотека **serializer** определяет способ формирования JSON? Об этом моменте мы поговорим далее.
...
Мы будем использовать жесткие значения температуры и влажности в коде образца, но представьте, что мы фактически извлекаем эти значения, пробуя соответствующие датчики на термостате.
...
Этот код представляет собой подмножество помощника **SendAsync,** описанного в предыдущем разделе, поэтому мы не будем переходить его снова здесь.
...
Мы отправляем температуру, которая имеет тип **TemperatureEvent**, и что структура содержит **температуру** и **время** члена. Это напрямую отражается в сериализованных данных.
...
Серифицированная форма, отправленная в Концентратор IoT, отображается следующим образом:
...
Теперь давайте изменим модель так, чтобы она включала те же данные, но с другой структурой.
...
В этом случае мы исключили макросы **\_DECLARE STRUCT** и просто определяем элементы данных из нашего сценария, используя простые типы из языка моделирования.

В данный момент не обращайте внимания на событие **Time**. С этим в сторону, вот код для входа **Температура:**
...
Другими словами, можно предположить, что этот код равнозначен отправке событий **Temperature** и **Humidity** по отдельности. Это просто удобство, чтобы передать оба события **SERIALIze** в тот же вызов. Однако это не так. Вместо этого, приведенный выше код отправляет в Центр Интернета вещей одно событие, инициируемое изменением данных:
...
Более того, мы не моделировали эти события, в которых **температура** и **влажность** находятся в одной структуре:
...
Это поведение проще понять, если знать о допущениях библиотеки **serializer** . Чтобы понять это, давайте вернемся к нашей модели:
...
Взгляните на эту модель в контексте объектно-ориентированного подхода. В этом случае мы моделируем физическое устройство (термостат), и это устройство включает в себя такие атрибуты, как **температура** и **влажность.**
...
Иногда вам нужно отправлять в облако значения только *некоторых* свойств модели (особенно это касается случаев, когда модель содержит большое количество событий, инициируемых изменением данных). Полезно отправлять только подмножество событий данных, например, в нашем предыдущем примере:

```C
{"Temperature":75, "Time":"2015-09-17T18:45:56Z"}
```

В результате будет создано такое же сериализованное событие, как и при определении **TemperatureEvent** с атрибутами **Temperature** и **Time**(модель 1). В этом случае мы смогли создать точно такое же сериализованное событие с помощью другой модели (модель 2), потому что мы называли **SERIALII** по-другому.
...
Выбор наиболее подходящего подхода зависит от того, как вы воспринимаете модель. Если вы отправляете "события" в облако и каждое событие содержит определенный набор свойств, то первый подход имеет большой смысл. В этом случае с помощью макроса **DECLARE\_STRUCT** определяется структура каждого события, а затем они включаются в модель с помощью макроса **WITH\_DATA**. Теперь события можно отправлять, как описано в первом примере. В этом подходе вы передайте только одно событие данных **SERIALIzER.**

Если вы рассматриваете свою модель с точки зрения объектно-ориентированного подхода, вам лучше использовать второй метод. В этом случае элементы, определенные с помощью макроса **WITH\_DATA**, будут свойствами вашего объекта. Вы передаете любое подмножество событий **SERIALI,** что вам нравится, в зависимости от того, сколько вашего состояния "объекта" вы хотите отправить в облако.
...
До сих пор в этой статье рассказывалось только об отправке событий в Центр Интернета вещей, но не об их получении. Причина этого заключается в том, что то, что нам нужно знать о получении сообщений, в значительной степени было описано в статье [Azure IoT устройство SDK для C](iot-hub-device-sdk-c-intro.md). Напомним из этой статьи, что вы обрабатываете сообщения путем регистрации функции обратного вызова сообщения:

```C
IoTHubClient_SetMessageCallback(iotHubClientHandle, IoTHubMessage, myWeather)
```

Затем вы пишете функцию обратного вызова, вызываемую при получении сообщения:
...
Если вы используете библиотеку **Serializer,** важная часть SDK, о которой следует знать, находится в библиотеке azure-c-shared-utility.

Если вы клонировали репозиторий Azure-iot-sdk-c от GitHub и выдали `git submodule update --init` команду, то вы найдете эту общую библиотеку утилиты здесь:
...
Эта папка содержит решение Visual Studio с именем **macro\_utils\_h\_generator.sln**:
  ...
  ![Снимок экрана решения Visual Studio maco_utils_h_generator](media/iot-hub-device-sdk-c-serializer/01-macro_utils_h_generator.png)

Программа в этом решении создает файл **macro\_utils.h**. Есть файл макроса\_по умолчанию utils.h, включенный в SDK. Это решение позволяет изменить ряд параметров, а затем повторно создать файл заголовка с учетом этих изменений.

Два ключевых параметра, которые будут касаться являются **nArithmetic** и **nMacro Parameters**, которые определяются в этих двух строках, найденных в макро-utils.tt:\_
...
Как говорилось ранее, **DECLARE\_MODEL** является обычным макросом C. Имя модели и инструкция **WITH\_DATA** (еще один макрос) являются параметрами макроса **DECLARE\_MODEL**. **nMacroParameters** определяет, сколько параметров можно включить в макрос **DECLARE\_MODEL**. Фактически этот параметр определяет максимальное количество событий, инициируемых изменением данных, и объявлений действий. Таким образом, ограничение в 124 элемента означает возможность определения модели, в которой будет приблизительно по 60 действий и событий, инициируемых изменением данных. Если превысить этот предел, вы получите следующую ошибку компиляции:
  ...
  ![Снимок экрана с ошибками компилятора параметров макроса](media/iot-hub-device-sdk-c-serializer/02-nMacroParametersCompilerErrors.png)
...
Параметр **nArithmetic** скорее предназначен для настройки внутренних механизмов макроязыка, чем для работы приложения.  Он определяет общее допустимое количество элементов в модели, включая макрос **DECLARE_STRUCT**. Если при компиляции программы вы получаете подобные сообщения об ошибках, попробуйте увеличить значение **nArithmetic**.
   ...
   ![Снимок экрана с ошибками арифметического компилятора](media/iot-hub-device-sdk-c-serializer/03-nArithmeticCompilerErrors.png)
...
Чтобы использовать новую версию macro\_utils.h, удалите пакет NuGet **serializer** из решения, заменив его проектом Visual Studio **serializer**. Это позволяет вашему коду выполнить компиляцию для исходного кода библиотеки serializer. Сюда входит обновленный macro\_utils.h. Если необходимо выполнить это действие для **simplesample\_amqp**, следует сначала удалить из решения пакет NuGet для библиотеки serializer:
   ...
   ![Снимок экрана удаления пакета NuGet для библиотеки serializer](media/iot-hub-device-sdk-c-serializer/04-serializer-github-package.png)
...
Когда все будет готово, ваше решение должно выглядеть следующим образом:
   ...
   ![Снимок экрана решения Visual Studio simplesample_amqp](media/iot-hub-device-sdk-c-serializer/05-serializer-project.png)
...
Аналогичным образом, когда вы закончите работать с библиотекой, последний звонок, который вы сделаете, это **serializer\_deinit:**
...
В этой статье подробно описаны уникальные аспекты библиотеки **сериализаторов,** содержащиеся в **устройстве SDK SDK Azure для C.** С предоставленной информацией вы должны иметь хорошее понимание того, как использовать модели для отправки событий и получать сообщения от IoT Концентратор.

Это также завершает серию из трех частей о том, как разрабатывать приложения с устройством **Azure IoT SDK для C.** Это должно быть достаточно информации, чтобы не только начать, но дать вам полное понимание того, как работают AA. Дополнительные сведения можно получить из нескольких примеров в пакете SDK, не описанных в этой статье. [Документация по пакету SDK для устройств Azure IoT](https://github.com/Azure/azure-iot-sdk-c) — хороший источник дополнительной информации.
