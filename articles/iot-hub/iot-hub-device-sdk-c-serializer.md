...
В этом примере не показаны дополнительные типы данных, поддерживаемые пакетом SDK. Об этом мы поговорим далее.
...
По сути, мы присваиваем значение каждому элементу структуры **теста** , а затем вызываю **SendAsync** для отправки события **тестовых** данных в облако. **SendAsync** является вспомогательной и служит для отправки одного события, инициируемого изменением данных, в Центр Интернета вещей:
...
Мы будем использовать жестко запрограммированные значения температуры и влажности в образце кода, но представьте, что мы фактически извлекаем эти значения, выполнив выборку соответствующих датчиков на термостата.
...
Этот код является подмножеством вспомогательного метода **SendAsync** , описанного в предыдущем разделе, поэтому мы не будем передвигаться сюда.
...
Сериализованная форма, которая отправляется в центр Интернета вещей, выглядит следующим образом:
...
Теперь изменим модель так, чтобы она включала те же данные, но с другой структурой.
...
В этом случае мы исключили макросы **объявления \_ структуры** и просто определяем элементы данных из нашего сценария, используя простые типы из языка моделирования.

В данный момент не обращайте внимания на событие **Time**. В этом случае код для входной **температуры**выглядит следующим образом:
...
Другими словами, можно предположить, что этот код равнозначен отправке событий **Temperature** и **Humidity** по отдельности. Это просто удобный способ передачи обоих событий для **сериализации** в одном вызове. Однако это не так. Вместо этого, приведенный выше код отправляет в Центр Интернета вещей одно событие, инициируемое изменением данных:
...
Более того, мы не будем моделировать эти события, в которых **температура** и **влажность** находятся в одной и той же структуре:
...
Иногда вам нужно отправлять в облако значения только *некоторых* свойств модели (особенно это касается случаев, когда модель содержит большое количество событий, инициируемых изменением данных). Полезно отправить только подмножество событий данных, как в предыдущем примере:
...
Выбор наиболее подходящего подхода зависит от того, как вы воспринимаете модель. Если вы отправляете "события" в облако, а каждое событие содержит определенный набор свойств, то первый подход имеет много смысла. В этом случае с помощью макроса **DECLARE\_STRUCT** определяется структура каждого события, а затем они включаются в модель с помощью макроса **WITH\_DATA**. Теперь события можно отправлять, как описано в первом примере. При таком подходе к **сериализатору**будет передавать только одно событие данных.

Если вы рассматриваете свою модель с точки зрения объектно-ориентированного подхода, вам лучше использовать второй метод. В этом случае элементы, определенные с помощью макроса **WITH\_DATA**, будут свойствами вашего объекта. Вы передаете любое подмножество событий для **сериализации** в зависимости от того, какая часть состояния объекта будет отправлена в облако.
...
Затем вы пишете функцию обратного вызова, которая вызывается при получении сообщения:
...
Если вы используете библиотеку **сериализатора** , важная часть пакета SDK, о которой следует помнить, находится в библиотеке Azure-c-Shared-Utility.

Если вы клонировать репозиторий Azure-IOT-SDK-c из GitHub и выпустили `git submodule update --init` команду, вы найдете эту общую библиотеку служебной программы здесь:
...
Программа в этом решении создает файл **macro\_utils.h**. В \_ пакет SDK входит файл макро utils. h, который используется по умолчанию. Это решение позволяет изменить ряд параметров, а затем повторно создать файл заголовка с учетом этих изменений.

Двумя ключевыми параметрами, с которыми следует думать, являются **nArithmetic** и **nMacroParameters**, которые определены в этих двух строках в макросах \_ utils.TT:
...
Аналогично, когда вы закончите работу с библиотекой, последний вызов, который вы сделаете, заключается в ** \_ деинициализации сериализатора**:
