...
* Политики **Чтение реестра** и **Запись в реестр** предоставляют права на чтение и запись в реестре удостоверений. Эти разрешения используются облачными службами бэк-энда для управления идентификаторами устройств. Если вы выберете разрешение на запись, разрешение на чтение добавляется автоматически.

* **Служба подключения предоставляет** разрешение на доступ к конечным точкам службы. Это разрешение используется облачными службами бэк-энда для отправки и получения сообщений с устройств, а также для обновления и чтения двойных данных устройства и модуля.

* Политика **Подключение устройства** предоставляет разрешения на отправку и получение сообщений через конечные точки Центра Интернета вещей на стороне устройства. Это разрешение используется устройствами для отправки и получения сообщений из концентратора IoT, обновления и чтения двойных и модульных двойных данных устройства и выполнения загрузок файлов.

Чтобы добавить новую политику к списку уже существующих, нажмите кнопку **Создать** .

Для получения более подробной [информации](./iot-hub-devguide-security.md#iot-hub-permissions)о доступе, предоставленном конкретными разрешениями, см.
...
Нажмите на вкладку **пользовательских конечных точек.** Вы видите все пользовательские конечные точки, уже созданные. На этой странице можно добавить новые конечные точки или удалить существующие. 
