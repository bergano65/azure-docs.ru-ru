...
**Цены и масштабирование**: Задайте количество вещей единиц или переход на другой уровень, можно использовать это свойство. 

**Мониторинг операций**: Отключение мониторинга категорий, таких как ведение журнала для событий, связанных с устройства в облако или сообщений из облака на устройство.

**IP-фильтра**: Задать диапазон IP-адресов, которые будет принят или отклонен центром Интернета вещей.

**Свойства**. Предоставляет список свойств, можно скопировать и использовать в другом месте, например идентификатор ресурса, группы ресурсов, расположение и т. д.
...
* Политики **Чтение реестра** и **Запись в реестр** предоставляют права на чтение и запись в реестре удостоверений. Эти разрешения используются серверными облачными службами позволяют управлять удостоверениями устройств. Если вы выберете разрешение на запись, разрешение на чтение добавляется автоматически.

* **Подключение службы** политика предоставляет разрешение на доступ к конечным точкам службы. Это разрешение используется серверными облачными службами для отправки и получения сообщений с устройств, а также относительно обновления и чтения данные двойника модуля и двойника устройства.

* Политика **Подключение устройства** предоставляет разрешения на отправку и получение сообщений через конечные точки Центра Интернета вещей на стороне устройства. Это разрешение используется устройства для отправки и получения сообщений из центра Интернета вещей, обновить и считывать данные двойника модуля и двойника устройства и отправки.

Чтобы добавить новую политику к списку уже существующих, нажмите кнопку **Создать** .

Более подробные сведения о предоставляются определенные разрешения, см. в разделе [разрешения центра Интернета вещей](./iot-hub-devguide-security.md#iot-hub-permissions).
