...
2. Добавьте эти значения в список для конечной точки Контососторажеендпоинтенричед.
   ...
   | Key | Значение | Конечная точка (раскрывающийся список) |
   | ---- | ----- | -------------------------|
...
1. В области **изменение шаблона** выберите **загрузить файл**. Откроется проводник Windows. Нахождение **template_messageenrichments.js** файла в распакованном файле репозитория в **/ИОТ-ХУБ/туториалс/раутинг/симулатеддевице/ресаурцес**. 
...
Чтобы удалить группу ресурсов, используйте команду [az group delete](https://docs.microsoft.com/cli/azure/group?view=azure-cli-latest#az-group-delete). Помните, что `$resourceGroup` в начале этого руководства было задано **контосоресаурцесмсжен** .
