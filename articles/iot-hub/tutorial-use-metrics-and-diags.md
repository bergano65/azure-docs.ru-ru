...
ms.locfileid: "78893252"
---
# <a name="tutorial-set-up-and-use-metrics-and-diagnostic-logs-with-an-iot-hub"></a>Руководство. Настройка и использование метрик и журналов диагностики с помощью центра Интернета вещей
...
>[!NOTE]
>При создании удостоверения устройства может появиться следующее сообщение об ошибке: *не найдены ключи для политики Iothubowner центра Интернета вещей контосотессуб*. Чтобы устранить эту ошибку, обновите расширение Azure CLI для Интернета вещей и снова выполните две последние команды скрипта. 
>
...
3. Убедитесь в правильности подписки и группы ресурсов. В разделе **Тип ресурса** снимите флажок **Выбрать все**, а затем найдите в списке **Центр Интернета вещей** и установите флажок напротив него. (Он помещает флажок *выбрать все* еще раз, просто проигнорируя его.) В разделе **ресурс**выберите имя концентратора. Экран должен выглядеть следующим образом: 
...
2. Чтобы перейти к классическим оповещениям отсюда, щелкните **Просмотреть классические оповещения**. 
    ...
    **Подписка**: Оставьте этот набор полей текущим подписку.

    **Источник**: задайте для этого поля значение *метрики*.

    **Группа ресурсов**. Задайте для этого поля текущую группу ресурсов *контосоресаурцес*. 

    **Тип ресурса**. Задайте для этого поля значение центр Интернета вещей. 

    **Ресурс**: выберите центр Интернета вещей *контосотессуб*.

3. Щелкните **Добавить оповещение метрики (классическое)** , чтобы настроить новое оповещение.

    Заполните следующие поля.

    **Имя**: укажите имя для правила оповещения, например данные *телеметрии*.

    **Описание**: Укажите описание оповещения, например *оповещение при наличии отправленных сообщений телеметрии 1000*. 

    **Источник**: задайте *метрики*.
...
4. Заполните следующие поля после диаграммы:
   ...
   **Условие**: задайте значение *больше*.

   **Пороговое значение**: значение 1000.

   **Период**: значение *за последние 5 минут*.

   **Получатели уведомлений по электронной почте**: Поместите здесь свой адрес электронной почты. 
...
5. Теперь настройте другое оповещение для параметра *Total number of messages used* (Общее количество используемых сообщений). Эта метрика полезна, если вы хотите отправить оповещение, когда количество используемых сообщений приближается к квоте, установленной для Центра Интернета вещей. Так вы узнаете, что центр вскоре начнет отклонять сообщения.
   ...
   **Имя**: укажите имя для правила генерации оповещений, например *число сообщений, используемых в сообщениях*.

   **Описание**: введите описание оповещения, например *оповещение при получении сообщения о близком к квоте*.
...
6. Под диаграммой заполните следующие поля:
   ...
   **Period**: задайте для этого поля значение *за последние 5 минут*. 
