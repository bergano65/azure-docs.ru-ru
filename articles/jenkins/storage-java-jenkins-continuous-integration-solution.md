...
description: В этом учебнике показано, как использовать службу Azure blob в качестве репозитория для создания артефактов, созданных решением непрерывной интеграции Jenkins.
keywords: Дженкинс, лазурный, devops, хранения, cicd
ms.topic: article
ms.date: 08/13/2019
...
* Учетная запись хранения Azure. Если у вас еще нет учетной записи хранения, ее можно создать с помощью процедуры [Создание учетной записи хранения](../storage/common/storage-account-create.md).
...
3. В разделе **Microsoft Azure Storage Account Configuration** (Настройка учетной записи хранения Azure):
   1. Введите имя учетной записи хранилища, которое можно получить на [портале Azure.](https://portal.azure.com)
   2. Введите ключ учетной записи хранилища, также полученный с [портала Azure](https://portal.azure.com).
   ...
   6. Выберите **Сохранить,** чтобы сохранить настройки.
...
7. Для поля **Container name** задайте имя контейнера. (Контейнер будет создан, если он еще не существует при загрузке артефактов сборки.) Можно использовать переменные среды, поэтому `${JOB_NAME}` для этого примера введите в качестве имени контейнера.
...
8. Щелкните **Make new container public by default** (Сделать новый контейнер общедоступным по умолчанию) для этого примера. (Если вы хотите использовать частный контейнер, необходимо создать подписанный URL-адрес. Это выходит за рамки данной статьи. Дополнительные сведения о подписанных URL-адресах см. в статье [Использование подписанных URL-адресов (SAS)](../storage/common/storage-sas-overview.md)).
...
12. Выберите **Сохранить,** чтобы сохранить настройки.
