...
В этой статье пользователи [службы Машинного обучения Azure](overview-what-is-azure-ml.md) могут ознакомиться с подробными сведениями о предварительно настроенных ограничениях на ресурсы Azure для подписки. В ней также представлены инструкции по запросу увеличения квоты для каждого типа ресурсов. Эти ограничения применяются во избежание перерасхода бюджета из-за мошенничества, а также для соблюдения ограничений по объемам ресурсов Azure.
...
Для каждой подписки Azure существует ограничение на количество виртуальных машин, которые можно подготовить в своих службах или изолированно. Это ограничение на уровне региона на общее количество ядер, а также на основе каждого семейства.
...
Более подробный и актуальный список квот см. в [статье, посвященной квотам на уровне среды Azure](https://docs.microsoft.com/azure/azure-resource-manager/management/azure-subscription-service-limits).
...
> [!NOTE]
> Эта функция доступна только для выпуска "Корпоративный". Если в подписке есть рабочая область уровня ["Базовый" и "Корпоративный"](overview-what-is-azure-ml.md#sku), вы можете использовать ее для задания квот только для корпоративных рабочих областей. В рабочих областях уровня "Базовый" по-прежнему будет существовать квота уровня подписки, что является поведением по умолчанию.
>
...
Вычислительная среда Машинного обучения Azure управляется отдельно от других квот на ресурсы Azure в вашей подписке. Чтобы просмотреть эту квоту, необходимо выполнить детализацию Службы машинного обучения Azure.  
