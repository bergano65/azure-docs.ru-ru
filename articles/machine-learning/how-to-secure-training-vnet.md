...
[1. Обзор виртуальной сети](how-to-network-security-overview.md)  >  [2. Обеспечьте безопасность рабочей области](how-to-secure-workspace-vnet.md)  >  **3. Обеспечьте безопасность среды обучения**  >  [4. Обеспечьте безопасность окружения](how-to-secure-inferencing-vnet.md)   >  [5. Включить функциональные возможности студии](how-to-enable-studio-virtual-network.md)
...
> * При развертывании экземпляра COMPUTE в рабочей области частной связи доступ к нему можно получить только из виртуальной сети. Если вы используете пользовательский DNS или файл hosts, добавьте запись для `<instance-name>.<region>.instances.azureml.ms` с частным IP-адресом частной конечной точки рабочей области. Дополнительные сведения см. в статье о [пользовательской службе DNS](./how-to-custom-dns.md) .
> * Подсеть, используемая для развертывания вычислительного кластера или экземпляра, не должна быть делегирована какой-либо другой службе, такой как ACI
> * Политики конечной точки службы виртуальной сети не работают для учетных записей хранения вычислительных кластерных систем и экземпляров
...
> * Один общедоступный IP-адрес. Если политика Azure запрещает создание общедоступного IP-адреса, то развертывание кластера или экземпляров завершится сбоем.
...
> Эти ресурсы ограничены [квотами ресурсов](../azure-resource-manager/management/azure-subscription-service-limits.md) в подписке. Если группа ресурсов виртуальной сети заблокирована, то удаление вычислительных кластеров или экземпляров завершится ошибкой. Подсистема балансировки нагрузки не может быть удалена до тех пор, пока не будет удален вычислительный кластер или экземпляр.
...
Конкретные сведения об использовании Azure Databricks с виртуальной сетью см. в разделе [Развертывание Azure Databricks в виртуальной сети Azure](/azure/databricks/administration-guide/cloud-configurations/azure/vnet-inject).
...
* [Создание и администрирование виртуальных сетей Azure для виртуальных машин Linux](../virtual-machines/linux/tutorial-virtual-network.md)

* [Расширение возможностей HDInsight с помощью виртуальной сети Azure](../hdinsight/hdinsight-plan-virtual-network-deployment.md).
...
Используйте правила входящего трафика по умолчанию для группы безопасности сети. Дополнительные сведения см. в описании стандартных правил безопасности в статье [о группах безопасности](../virtual-network/network-security-groups-overview.md#default-security-rules).
