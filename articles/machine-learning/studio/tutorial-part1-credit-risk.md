...
description: Подробное пошаговое руководство по созданию решения для прогнозной аналитики в Студии машинного обучения Azure для оценки кредитных рисков. Это первое руководство в серии, состоящей из трех частей.  В нем рассказывается, как создать рабочую область, отправить данные и запустить эксперимент.
...
В этом руководстве подробно описывается процесс разработки решения прогнозной аналитики. Вы разработаете простую модель в Студии машинного обучения.  Затем развернете модель в качестве веб-службы машинного обучения Azure.  Эта развернутая модель позволяет создавать прогнозы на основе новых данных. Это **первое руководство в серии, состоящей из трех частей**.
...
Оценка кредитных рисков — это сложная задача. Мы немного упростим ее для этого руководства. На примере этой задачи вы создадите решение прогнозной аналитики с помощью Машинного обучения Microsoft Azure. Для этого вы используете Студию машинного обучения Azure и веб-службу машинного обучения.  
...
> [!div class="checklist"]
> * создание рабочей области Студии машинного обучения;
...
Если вы не использовали **Студию машинного обучения**, прежде чем приступать к работе с этим руководством, [создайте свой первый эксперимент по обработке и анализу данных в Студии машинного обучения Azure](create-experiment.md). При ознакомлении со Студией машинного обучения вы научитесь выполнять базовые операции: переносить модули в эксперимент, соединять их друг с другом, запускать эксперимент и просматривать результаты.


> [!TIP] 
> Рабочую копию эксперимента, разработанного в этом руководстве, можно найти в [Коллекции решений ИИ Azure](https://gallery.azure.ai). Перейдите по ссылке **[Tutorial - Predict credit risk](https://gallery.azure.ai/Experiment/Walkthrough-Credit-risk-prediction-1)** (Руководство. Прогнозирование кредитных рисков) и нажмите кнопку **Open in Studio** (Открыть в Студии), чтобы скачать в рабочую область Студии машинного обучения копию этого эксперимента.
> 


## <a name="create-a-machine-learning-studio-workspace"></a>Создание рабочей области Студии машинного обучения

Чтобы использовать Студию машинного обучения, требуется рабочая область Студии машинного обучения Microsoft Azure. Такая рабочая область содержит инструменты, необходимые для создания, публикации экспериментов и управления ими.  

Инструкции по созданию рабочей области см. в статье [Создание рабочей области Студии машинного обучения Azure и предоставление к ней общего доступа](create-workspace.md).
...
![Выбор рабочей области в Студии](./media/tutorial-part1-credit-risk/open-workspace.png)
...
В исходном наборе данных используется формат с разделителями-пробелами. Студия машинного обучения лучше работает с файлами, в которых используются разделители-запятые (CSV-файлами), поэтому вы преобразуете набор данных, заменив пробелы запятыми.  
...
После преобразования данных в формат CSV вам необходимо отправить их в Студию машинного обучения. 
...
Следующий этап в этом руководстве — создание эксперимента в Студии машинного обучения, который использует отправленный вами набор данных.  
...
1. Выберите [Изменить метаданные][edit-metadata], затем в области **Свойства** справа от холста щелкните **Launch column selector** (Запустить средство выбора столбцов).
...
1. Подключите левый порт вывода модуля [Split Data][split] (Разделение данных) к первому порту ввода (Dataset1) модуля[Выполнить сценарий R][execute-r-script].

1. Дважды щелкните модуль [Выполнить сценарий R][execute-r-script] и введите комментарий "Задать корректировку стоимости".
