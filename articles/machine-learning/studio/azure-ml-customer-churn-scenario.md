...
description: Тематическое исследование разработки интегрированной модели анализа и оценки оттока клиентов с помощью Azure Machine Learning Studio (классика).
...
---
# <a name="analyze-customer-churn-using-azure-machine-learning-studio-classic"></a>Анализ клиентов Churn с помощью Azure машинного обучения Studio (классический)

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

## <a name="overview"></a>Обзор
В этой статье представлена эталонная реализация проекта анализа оттока клиентов, построенного с помощью Azure Machine Learning Studio (классический). Кроме того, здесь рассматриваются соответствующие универсальные модели для комплексного решения проблем оттока промышленных клиентов. Здесь также измеряется точность моделей, построенных с использованием машинного обучения, и оцениваются направления дальнейшего развития.  

### <a name="acknowledgements"></a>Благодарности
Этот эксперимент был разработан и протестирован Сержем Бергером (Serge Berger), главным специалистом по данным корпорации Майкрософт, и Роджером Баргой (Roger Barga), бывшим менеджером по продуктам в Microsoft Azure Machine Learning Studio (классика). Группа составителей документации Azure с благодарностью подтверждает их квалификацию и выражает признательность за предоставление этого технического документа.
...
![Схема взаимодействия модели оттока](./media/azure-ml-customer-churn-scenario/churn-2.png)
...
## <a name="implementing-the-modeling-archetype-in-machine-learning-studio-classic"></a>Внедрение арочима моделирования в студии машинного обучения (классический)
Как лучше всего можно реализовать интегрированный подход моделирования и оценки, принимая во внимание описанную проблему? В этом разделе мы продемонстрируем, как мы это сделали, используя Azure Machine Learning Studio (классический).  
...
На следующей диаграмме показан созданный мы прототип, в котором используются четыре алгоритма скоринга в Machine Learning Studio (классический) для прогнозирования оттока. Причиной внедрения многомодельного подхода является не только необходимость создания классификатора множества для повышения уровня точности, но и обеспечение защиты от чрезмерно близкой подгонки, а также улучшение выбора функций директивного характера.  

![Скриншот, изображающий сложное рабочее пространство Studio (классическое) со многими взаимосвязанными модулями](./media/azure-ml-customer-churn-scenario/churn-3.png)

*Рисунок 5: Прототип метода моделирования оттока*  

В следующих разделах приведены более подробная информация о модели оценки прототипа, которую мы реализовали с помощью Machine Learning Studio (классический).  
...
Наиболее важным моментом является то, что весь процесс, включая ETL, выбор функций и моделирование, был реализован в Машинной студии обучения (классический), используя источники данных в Microsoft Azure.   
...
> Тем не менее в примере эксперимента [Telco Customer Churn](https://gallery.azure.ai/Experiment/31c19425ee874f628c847f7e2d93e383) из [коллекции решений ИИ Azure](https://gallery.azure.ai/) для построения аналогичной модели используются общедоступные данные.
> 
...
*Рисунок 8: Создание моделей в студии машинного обучения (классический)*  
...
Оцениваемый набор данных также отправлялся в сопоставимую модель, созданную с помощью SAS Enterprise Miner 12 для настольных систем. Мы измерили точность модели SAS и всех четырех моделей Machine Learning Studio (классических).  
...
Как правило, реализация в Azure Machine Learning Studio (классическая) отстает от SAS по точности примерно на 10-15% (Area Under Curve или AUC).  
...
* Студия машинного обучения (классическая) неправильная классификация составила 15-20% для топ-200-300.  

В сфере телекоммуникаций очень важно уделять внимание только клиентам с самым высоким уровнем риска оттока. Для этого им предлагаются услуги поддержки или другие специальные режимы и условия. В этом отношении реализация Machine Learning Studio (классическая) достигает результатов наравне с моделью SAS.  
...
Мы сравнили скорость, с которой данные были забиты с помощью машинного обучения Studio (классические) модели и сопоставимые модели, созданные с помощью настольного издания SAS Enterprise Miner 12.1.  
...
Модели, размещенные в Machine Learning Studio (классический) превзошли SAS на 15-25% по скорости исполнения, но точность была в значительной степени на одном уровне.  
...
* Получение показателей для четырех основных категорий:
  ...
  * **Деятельность**. Получение всей возможной информации об использовании, касающейся объекта, например количества входов.
...
Тем не менее, обещание самообслуживания аналитики с помощью машинного обучения Studio (классический) является то, что четыре категории информации, градуированные по разделению или отделу, стать ценным источником для машинного обучения о оттоке.  

Еще одна захватывающая возможность, поступающая в Azure Machine Learning Studio (классический) является возможность добавить пользовательский модуль в хранилище предопределенных модулей, которые уже доступны. Она, по сути, позволяет выбрать библиотеки и создавать шаблоны для вертикальных рынков. Это важный дифференциатор Azure Machine Learning Studio (классический) на рынке.  
...
В этой статье описывается разумный подход к решению распространенной проблемы, связанной с оттоком клиентов, на основе единой схемы. Мы рассмотрели прототип для скоринговых моделей и внедрили его с помощью Azure Machine Learning Studio (классический). Наконец, мы оценили точность и производительность решения прототипа на уровне сопоставимых алгоритмов в SAS.  

 

## <a name="references"></a>Ссылки
Прогнозная аналитика: Помимо прогнозов, W. McKnight, Управление информацией, июль/август 2011 года, стр.18-20.  

Статья в Википедии: [Точность и точность](https://en.wikipedia.org/wiki/Accuracy_and_precision)
