...
description: Подробное руководство по созданию решения прогнозной аналитики для оценки кредитных рисков в Студии машинного обучения Azure. Это руководство представляет собой вторую часть серии, состоящей из трех частей. В нем объясняется, как обучать и оценивать модели.
...
ms.locfileid: "56453493"
---
# <a name="tutorial-2-train-credit-risk-models---azure-machine-learning-studio"></a>Руководство 2. Обучение моделей кредитных рисков в Студии машинного обучения Azure

В этом руководстве подробно описывается процесс разработки решения прогнозной аналитики. Мы создадим в Студии машинного обучения простую модель,  а затем развернем ее в качестве веб-службы машинного обучения Azure.  Развернутая модель позволяет создавать прогнозы на основе новых данных. Это руководство представляет собой **вторую часть серии, состоящей из трех частей**.
...
Оценка кредитных рисков — это сложная задача. Мы немного упростим ее для этого руководства. На примере этой задачи вы создадите решение прогнозной аналитики с помощью Студии машинного обучения Microsoft Azure. Для этого вы используете Студию машинного обучения Azure и веб-службу машинного обучения.  
...
В [первой части руководства](tutorial-part1-credit-risk.md) вы создали рабочую область Студии машинного обучения, отправили в нее данные и создали эксперимент.

В этой части руководства вы выполните следующие задачи:
...
Существует множество моделей, которые можно выбрать. Чтобы просмотреть доступные модели, разверните узел **Машинное обучение** в палитре модулей, а затем **Initialize Model** (Инициализация модели) и узлы под ним. Для этого эксперимента необходимо выбрать модули [Two-Class Support Vector Machine][two-class-support-vector-machine] (Двухклассовый метод опорных векторов) и [Two-Class Boosted Decision Tree][two-class-boosted-decision-tree] (Двухклассовое увеличивающееся дерево решений).
...
1. Найдите модуль [Train Model][train-model] (Обучение модели), перетащите его на холст, а затем подключите точку вывода модуля [Two-Class Boosted Decision Tree][two-class-boosted-decision-tree] (Двухклассовое увеличивающееся дерево решений) к левому порту ввода модуля [Train Model][train-model] (Обучение модели).
...
1. Соедините левый выход левого модуля [Выполнить сценарий R][execute-r-script] с правым портом ввода модуля [Обучение модели][train-model]. Напомним, что в этом руководстве вы [использовали для обучения данные, поступающие от левой стороны](#train) модуля разделения данных.
   
   > [!TIP]
   > Для этого эксперимента вам не нужны два входа и один выход модуля [Выполнить сценарий R][execute-r-script], поэтому их можно оставить неподключенными. 
   > 
   > 
...
Теперь необходимо сообщить модулю [Обучение модели][train-model], что модель должна прогнозировать значения кредитного риска (поле Credit Risk).
...
1. Соедините ввод этого модуля с левым портом вывода левого модуля [Execute R Script][execute-r-script] (Выполнение скрипта R). Обратите внимание, что порт вывода любого модуля может быть подключен к нескольким модулям.

1. Соедините левый выходной порт модуля [Normalize Data][normalize-data] (Нормализация данных) с правым входным портом второго модуля [Train Model][train-model] (Обучение модели).
...
1. Подключите модуль [Train Model][train-model] (Обучение модели), который подключен к модулю [Two-Class Support Vector Machine][two-class-boosted-decision-tree] (Двухклассовый метод опорных векторов), к левому порту ввода нового модуля [Score Model][score-model] (Оценка модели).

1. Соедините правый модуль [Execute R Script][execute-r-script] (Выполнение скрипта R), который возвращает данные для тестирования, с правым входным портом модуля [Score Model][score-model] (Оценка модели).
...
1. Соедините выход модуля SVM (т. е. порт вывода модуля [Train Model][train-model] (Обучение модели), подключенного к модулю [Two-Class Support Vector Machine][two-class-support-vector-machine] (Двухклассовый метод опорных векторов)) со входным портом второго модуля [Score Model][score-model] (Оценка модели).
...
1. Соедините левый выход второго модуля [Normalize Data][normalize-data] (Нормализация данных) с правым входным портом второго модуля [Score Model][score-model] (Оценка модели).
...
Чтобы проанализировать и сравнить два результата оценки, используйте модуль [Evaluate Model][evaluate-model] (Анализ модели).  

1. Найдите модуль [Evaluate Model][evaluate-model] (Анализ модели) и перетащите его на холст.

1. Подключите порт вывода модуля [Score Model][score-model] (Оценка модели), связанного с модулем увеличивающегося дерева решений, к левому входному порту модуля [Evaluate Model][evaluate-model] (Анализ модели).

1. Соедините второй модуль [Score Model][score-model] (Оценка модели) с левым входным портом.  
...
- [Как оценить эффективность модели в Студии машинного обучения Azure](evaluate-model-performance.md)
- [Выбор параметров для оптимизации алгоритмов в Студии машинного обучения Azure](algorithm-parameters-optimize.md)
- [Интерпретация результатов модели в Студии машинного обучения Azure](interpret-model-results.md)
