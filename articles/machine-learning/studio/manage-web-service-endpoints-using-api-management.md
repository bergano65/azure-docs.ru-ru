---
title: Управление веб-сервисами с помощью Управления API
...
* Рабочая область, служба и ключ API для эксперимента AzureML, развернутого в качестве веб-службы. Подробную информацию о том, как создать эксперимент AzureML, можно узнать на [quickstart Studio.](create-experiment.md) Для получения информации о том, как развернуть эксперимент Studio (классический) в качестве веб-службы, ознакомьтесь с тем, [как](deploy-a-machine-learning-web-service.md) развернуть эксперимент AzureML в качестве веб-службы. Инструкции по созданию и тестированию простого эксперимента AzureML и его развертывания в качестве веб-службы содержатся также в дополнении А.
...
4. Нажмите **Ответы** > **ADD** слева и выберите **200 OK**.
...
5. Нажмите **Ответы** > **ADD** слева и выберите **200 OK**.
6. Нажмите **Сохранить**.
...
6. Нажмите **Ответы** > **ADD** слева и выберите **200 OK**.
7. Нажмите **Сохранить**.
...
Операции могут быть вызваны из портала разработчика, где предоставляется удобный способ для просмотра и проверки операций API. На этом этапе вы вызовете метод **RRS Execute,** который был добавлен в **API-версию AzureML Demo.** 
...
3. Выберите операцию **RRS Execute** . Нажмите **Попробуйте его**.
...
Во-первых, используя браузер по вашему выбору, перейдите к: [https://studio.azureml.net/](https://studio.azureml.net/) и введите свои учетные данные, чтобы войти в систему. Затем создайте пустой эксперимент.
...
![Подключение модуля набора данных обзоров книг к модулю колонки проекта](./media/manage-web-service-endpoints-using-api-management/project-columns.png)
...
![Выберите столбцы с использованием имен столбцов](./media/manage-web-service-endpoints-using-api-management/select-columns.png)
