...
![Схема Студии машинного обучения Azure (классической): создание экспериментов, чтение данных из различных источников, запись оцененных данных, запись моделей.](./media/what-is-ml-studio/azure-ml-studio-diagram.jpg)
...
[Машинное обучение Azure](../overview-what-is-azure-ml.md) предоставляет как пакеты SDK **, так и** конструктор Машинного обучения Azure (предварительная версия), позволяющие быстро подготавливать данные, а также обучать и развертывать модели машинного обучения. Этот конструктор предоставляет аналогичный способ перетаскивания, что и в Студии (классической). Тем не менее, в отличие от собственной вычислительной платформы среды Студии (классической), конструктор использует ваши собственные вычислительные ресурсы и полностью интегрирован в Машинное обучение Azure.

Ниже приведена краткая сравнительная характеристика.

|| Студия машинного обучения (классическая версия) | Машинное обучение Azure |
|---| --- | --- |
| Интерфейс перетаскивания | Да | Да, [конструктор Машинного обучения Azure (предварительная версия)](../concept-designer.md) |
...
|Обучение целевых объектов вычислений| Собственный целевой объект, поддерживающий только ЦП| Поддерживает вычисления Машинного обучения Azure (GPU или ЦП), а также виртуальные машины Notebook.<br/>([Другие вычисления, поддерживаемые пакетом SDK](../concept-compute-target.md#train))|
|Вывод целевых объектов вычислений| Собственный формат веб-службы, без возможности настройки |  Служба Azure Kubernetes и вычислительная среда AML <br/>([Другие вычисления, поддерживаемые пакетом SDK](../how-to-deploy-and-where.md)) |
| ML-конвейер | Не поддерживается | Поддерживаемые [конвейеры](../concept-ml-pipelines.md) |
...
| Формат модели | Собственный формат, только Студия (классическая) | Стандартный формат в зависимости от типа задания обучения |
|Автоматизированное обучение моделей и настройка гиперпараметров | нет | Пока отсутствует в конструкторе <br/> ([Поддерживается пакетом SDK и целевой страницей рабочей области](../concept-automated-ml.md)) | 

Воспользуйтесь конструктором, выполнив действия в [учебнике по прогнозированию цены на автомобили с помощью конструктора](../tutorial-designer-automobile-price-train-score.md).
...
Модуль — это алгоритм, который можно выполнять с данными. В Студии машинного обучения Azure (классической) имеется ряд модулей, начиная с функций ввода данных для процессов обучения, оценки и проверки. Ниже приведены примеры включенных модулей.
