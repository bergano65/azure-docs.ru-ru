...
Машинное обучение Azure Studio (классическая модель) предоставляет средства, необходимые для разработки модели прогнозной аналитики, а затем эксплуатацию ее путем развертывания в качестве веб-службы Azure.

Для этого используйте Studio (классическая модель) для создания эксперимента, который называется *обучающим экспериментом* , в котором вы научитесь обучать, оценивать и редактировать модели. Когда модель будет полностью вас устраивать, вы подготовите ее к развертыванию, преобразовав обучающий эксперимент в *прогнозный*, который предназначен для обработки пользовательских данных.

Пример этого процесса см. в [руководстве 1. прогнозирование кредитного риска](tutorial-part1-credit-risk.md).
...
1. Сохраняет обученную модель в разделе **Trained Models** (обученные модели) на палитре модулей (слева от холста эксперимента). Затем он заменяет алгоритм машинного обучения и модули [обучения модели][train-model] на сохраненную обученную модель.
...
* **Подготовка.** Возможно, эти модули понадобятся для обработки входных данных, которые пользователь отправит для оценки. Действие **Set Up Web Service** (Настройка веб-службы) не затрагивает эти модули. Вам нужно самостоятельно решить, что с ними делать.
    ...
    Например, в этом примере образец набора данных может содержать отсутствующие значения, поэтому модуль [очистки отсутствующих данных][clean-missing-data] был добавлен к ним. Кроме того, этот образец набора данных содержит столбцы, не используемые для обучения модели. Поэтому был добавлен модуль [Select Columns in DataSet (выбор столбцов в наборе данных][select-columns] ), чтобы исключить из потока данных лишние столбцы. Если известно, что данные, которые будут отправлены для оценки через веб-службу, не будут иметь отсутствующих значений, можно удалить модуль [очистить недостающие данные][clean-missing-data] . Однако, поскольку модуль [Выбор столбцов в наборе данных][select-columns] помогает определить столбцы данных, которые предположительно обученная модель, этот модуль должен остаться.
...
* **Оценка** . в этом примере модуль [Split Data (разделение данных][split] ) используется для разделения потока данных на тестовые данные и обучающие данные. В прогнозном эксперименте мы больше не обучить, поэтому [разделение данных][split] можно удалить. Аналогичным образом, второй модуль [оценки модели][score-model] и модуль « [Анализ модели][evaluate-model] » используются для сравнения результатов из тестовых данных, поэтому эти модули не нужны в прогнозном эксперименте. Однако оставшийся модуль [оценки модели][score-model] необходим для возврата результатов оценки через веб-службу.
...
Например, чтобы получить только результаты оценки, а не весь вектор входных данных, добавьте модуль [Select Columns in DataSet (выбор столбцов в наборе данных][select-columns] ), чтобы исключить все столбцы, кроме результатов оценки. Затем переместите модуль **выходные данные веб-службы** в выходные данные модуля [Select Columns in DataSet (выбор столбцов в наборе данных][select-columns] ). Измененный эксперимент будет выглядеть так:
...
Если вам известно, что в эксперименте есть модули, которые не будут нужны при оценке, их можно удалить. Например, так как мы переместили модуль **ввода веб-службы** в точку после модулей обработки данных, мы можем удалить модуль [Clean Missing Data (очистка недостающих данных][clean-missing-data] ) из прогнозного эксперимента.
...
Распространенным примером является настройка модуля [Импорт данных][import-data] , чтобы пользователь развернутой веб-службы мог указать другой источник данных при доступе к веб-службе. Или настройки модуля [экспорта данных][export-data] , чтобы можно было указать другое назначение.
...
Дополнительные сведения о параметрах веб-службы и способах их использования см. [в разделе Использование параметров веб-службы машинное обучение Azure][webserviceparameters].
