...
description: Как подготовить подготовленную модель для развертывания в качестве веб-сервиса, преобразовав учебный эксперимент Machine Learning Studio (классический) в прогностический эксперимент.
...
---
# <a name="how-to-prepare-your-model-for-deployment-in-azure-machine-learning-studio-classic"></a>Как подготовить модель к развертыванию в студии машинного обучения Azure (классический)

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

Студия машинного обучения Azure (классическая) предоставляет инструменты, необходимые для разработки модели прогностической аналитики, а затем ее эксплуатации, развернув ее в качестве веб-сервиса Azure.

Для этого вы используете Studio (классический) для создания эксперимента, называемого *учебным экспериментом,* в котором вы тренируетесь, забиваете и отсвачиваете свою модель. Когда модель будет полностью вас устраивать, вы подготовите ее к развертыванию, преобразовав обучающий эксперимент в *прогнозный*, который предназначен для обработки пользовательских данных.

Вы можете увидеть пример этого процесса в [учебнике 1: Предсказать кредитный риск.](tutorial-part1-credit-risk.md)
...
Аналогичным образом при нажатии кнопки **Настройка веб-службы** модуль выходных данных веб-службы по умолчанию помещается в нижнюю часть потока данных. В этом примере веб-служба вернет пользователю выход модуля [Score Model,][score-model] который включает полный вектор входных данных плюс результаты подсчета очков.
