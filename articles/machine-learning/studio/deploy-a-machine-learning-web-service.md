...
description: Как преобразовать обучаемый эксперимент в прогностический эксперимент, подготовить его к развертыванию, а затем развернуть в виде веб-сервиса Azure Machine Learning Studio (классический).
...
ms.locfileid: "79218129"
---
# <a name="deploy-an-azure-machine-learning-studio-classic-web-service"></a>Развертывание веб-сервиса Azure Machine Learning Studio (классический)

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

Студия машинного обучения Azure (классическая) позволяет создавать и тестировать прогностический аналитический решение. Затем эти решение можно развернуть как веб-службу.

Студия машинного обучения (классические) веб-сервисы обеспечивают интерфейс между приложением и машинной студией обучения (классическая) модель оценки рабочего процесса. Внешнее приложение может общаться с моделью оценки рабочего процесса Machine Learning Studio в режиме реального времени. Звонок в веб-сервис Machine Learning Studio (классический) возвращает результаты прогнозирования внешнему приложению. Чтобы вызвать веб-службу, передайте ключ API, созданный при развертывании этой службы. Студия машинного обучения (классический) веб-сервис основан на REST, популярном выборе архитектуры для проектов веб-программирования.

Студия машинного обучения Azure (классическая) имеет два типа веб-сервисов:

* Служба запроса-ответа (RRS): Низкая задержка, высокомасштабируемая служба, которая оценивает одну запись данных.
* Служба исполнения пакетов (BES): асинхронная служба, которая оценивает пакет записей данных.
...
* **[Создайте обучающие эксперименты]** - In Studio (классический) вы можете обучить и протестировать модель прогностической аналитики, используя данные обучения, которые вы поставляете, используя большой набор встроенных алгоритмов машинного обучения.
...
Для обучения модели прогностической аналитики используется Azure Machine Learning Studio (классический) для создания учебного эксперимента, в котором вы включаете различные модули для загрузки обучающих данных, подготовки данных по мере необходимости, применения алгоритмов машинного обучения и оценки Результаты. Вы можете выполнять итерации эксперимента и испытывать разные алгоритмы машинного обучения, чтобы сравнить и оценить результаты.
...
* [Создайте простой эксперимент в студии машинного обучения Azure (классический)](create-experiment.md)
* [Разработать прогностиче-решение с помощью студии машинного обучения Azure (классический)](tutorial-part1-credit-risk.md)
* [Импортируйте свои обучающие данные в студию машинного обучения Azure (классический)](import-data.md)
* [Управление итерациями экспериментов в студии машинного обучения Azure (классический)](manage-experiment-iterations.md)
...
Для получения дополнительной информации о том, как выполнить это преобразование, [см. Как подготовить модель для развертывания в Azure Machine Learning Studio (классический)](convert-training-experiment-to-scoring-experiment.md).
...
Чтобы развернуть прогнозный эксперимент, в нижней части области эксперимента щелкните **Run** (Выполнить). После завершения эксперимента щелкните **Deploy Web Service** (Развернуть веб-службу) и выберите **Deploy Web Service [новую]**.  Открывается страница развертывания портала машинного обучения (классический) веб-сервис.

> [!NOTE] 
> Для развертывания новой веб-службы у вас должен быть достаточный уровень разрешений в подписке, в которую выполняется развертывание веб-службы. Для получения дополнительной информации [см.](manage-new-webservice.md) 

### <a name="web-service-portal-deploy-experiment-page"></a>Веб-сервис портал Развертывание Страница эксперимента
...
Нажмите **Развертывание** и страница **Быстрый запуск** для вашего веб-службы открывается.
...
Чтобы протестировать новый веб-сервис, нажмите **Test веб-сервис** под общими задачами. На странице тестирования веб-службу можно проверить двумя способами: как службу RRS (запрос-ответ) или как службу BES (пакетное выполнение).
...
![Введите соответствующие значения для тестирования веб-сервиса](./media/publish-a-machine-learning-web-service/figure-5-test-request-response.png)

Для тестирования службы BES щелкните **Batch**(Пакетная служба). На странице тестирования "Batch" (Пакетная служба) в разделе входных данных нажмите кнопку "Browse" (Обзор) и выберите CSV-файл, содержащий необходимые примеры значений. Если у вас нет файла CSV, и вы создали прогностический эксперимент с помощью Machine Learning Studio (классический), вы можете загрузить набор данных для прогностического эксперимента и использовать его.

Чтобы загрузить набор данных, откройте машинную студию обучения (классический). Откройте прогнозный эксперимент и щелкните правой кнопкой мыши входные данные своего эксперимента. В контекстном меню выберите **dataset** (набор данных), а затем выберите **Download** (Скачать).

![Скачать набор данных с холста Studio (классический)](./media/publish-a-machine-learning-web-service/figure-7-mls-download.png)
...
![Проверьте работу пакета исполнения с веб-порталом службы](./media/publish-a-machine-learning-web-service/figure-6-test-batch-execution.png)

<!--![Test the web service](./media/publish-a-machine-learning-web-service/figure-3.png)-->

На странице **CONFIGURATION** можно изменить описание, название, обновить ключ учетной записи хранилища и включить выборочные данные для веб-сервиса.

![Настройка веб-сервиса](./media/publish-a-machine-learning-web-service/figure-8-arm-configure.png)

### <a name="access-your-new-web-service"></a>Доступ к новой веб-службе

После развертывания веб-сервиса из Machine Learning Studio (классический), вы можете отправлять данные в службу и получать ответы программно.
...
Для получения дополнительной информации о доступе к веб-сервису Machine Learning Studio (классический) [см.](consume-web-services.md)

### <a name="manage-your-new-web-service"></a>Управление новой веб-службой

Управлять новыми веб-сервисами можно с помощью портала веб-сервисов Machine Learning Studio (классический). Со [страницы главного портала](https://services.azureml.net/)нажмите **веб-сервисы**. На странице веб-служб можно удалить службу или скопировать ее. Для наблюдения за конкретной службой выберите службу и щелкните **Dashboard**(Панель мониторинга). Для наблюдения за выполнением пакетных заданий, связанных с веб-службой, щелкните **Batch Request Log**(Журнал пакетных запросов).
...
![Развертывание веб-сервиса от Studio (классический)](./media/publish-a-machine-learning-web-service/figure-2.png)

### <a name="test-your-classic-web-service"></a>Тестирование классической веб-службы

Вы можете протестировать веб-сервис на портале машинного обучения (классический) веб-сервис или в студии машинного обучения (классический).
...
Вы можете нажать на ссылку предварительного просмотра **теста,** чтобы протестировать свой сервис на портале Web-сервисов Azure Machine Learning Studio (классический), как показано ранее в разделе Новый веб-сервис.
...
Включив ведение журнала, вы сможете диагностировать сбои, которые происходят при обращении к веб-службе. Для получения дополнительной информации [см.](web-services-logging.md)

![Включить регистрацию на портале веб-сервисов](./media/publish-a-machine-learning-web-service/figure-4.png)
...
После развертывания веб-сервиса из Azure Machine Learning Studio (классический) можно отправлять данные в службу и получать ответы за программу.
...
Для получения дополнительной информации [см. Управление Azure Machine Learning Studio (классическое) рабочее пространство](manage-workspace.md) и управление [веб-службой с помощью портала Web Services Azure Machine Learning Studio (классический).](manage-new-webservice.md)
...
Один из вариантов обновления веб-службы — переобучить модель программно. Для получения дополнительной информации, см [Retrain машинного обучения Studio (классические) модели программно](/azure/machine-learning/studio/retrain-machine-learning-model).

## <a name="next-steps"></a>Дальнейшие действия

* Для получения более подробной информации о том, как работает развертывание, см., [как модель Machine Learning Studio (классическая) переходит от эксперимента к оперативной web-службе.](model-progression-experiment-to-web-service.md)

* Для получения подробной информации о том, как подготовить модель к развертыванию, см. [Как подготовить модель к развертыванию в Azure Machine Learning Studio (классический).](convert-training-experiment-to-scoring-experiment.md)

* Существует несколько способов использования REST API и доступа к веб-службе. Узнайте, [как использовать веб-сервис Azure Machine Learning Studio (классический)](consume-web-services.md).

<!-- internal links -->
[Создание учебного эксперимента]: #create-a-training-experiment
[Преобразование обучающего эксперимента в прогнозный]: #convert-the-training-experiment-to-a-predictive-experiment
[Новый веб-сервис]: #deploy-it-as-a-new-web-service
[Классический веб-сервис]: #deploy-it-as-a-classic-web-service
[Новые функции]: #deploy-it-as-a-new-web-service
