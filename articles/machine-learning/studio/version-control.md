...
description: Применяйте лучшие практики управления жизненным циклом приложений в студии машинного обучения Azure (классический)
...
---
# <a name="application-lifecycle-management-in-azure-machine-learning-studio-classic"></a>Управление жизненным циклом приложений в студии машинного обучения Azure (классический)

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

Azure Machine Learning Studio (классическая) — это инструмент для разработки экспериментов машинного обучения, которые используются на облачной платформе Azure. Это решение позволяет объединить в рамках одной платформы возможности Visual Studio IDE и масштабируемой облачной службы. Стандартные методы управления жизненным циклом приложений (ALM) можно включить в программу Azure Machine Learning Studio (классическая) от версии различных ресурсов до автоматизированного выполнения и развертывания. В этой статье рассматриваются некоторые возможности и подходы.
...
В модели выполнения эксперимента По обучению Машинного обучения Azure Machine Learning Studio (классический) непреложный снимок эксперимента представляется планировщику работ всякий раз, когда вы нажимаете **Run** в редакторе эксперимента. Чтобы просмотреть этот список моментальных снимков, нажмите кнопку **Журнал выполнения** на панели команд в представлении редактора экспериментов.
...
Обученная модель в Azure Machine Learning Studio (классическая) сериализуется`.iLearner`в формат, известный как файл iLearner (), и хранится в учетной записи хранения Azure Blob, связанной с рабочим пространством. Получить копию iLearner можно с помощью API переобучения. В [этой статье](/azure/machine-learning/studio/retrain-machine-learning-model) объясняется, как работает API переобучения. Основные действия:
...
Можно развернуть два типа веб-сервисов из эксперимента Azure Machine Learning Studio (классический). Классическая веб-служба тесно связана с экспериментом и рабочей областью. Новая веб-служба использует платформу Azure Resource Manager. Она больше не связана с исходным экспериментом или с рабочей областью.
...
При создании новой веб-службы на основе Azure Resource Manager конструкция конечной точки недоступна. Вместо этого можно создавать файлы определения веб-службы (WSD) в формате JSON, из прогностического эксперимента с помощью командного пункта [Export-AmlWebServiceDefinitionFromExperiment](https://github.com/hning86/azuremlps#export-amlwebservicedefinitionfromexperiment) PowerShell или с помощью командного пункта [*Export-AzMlWebservice*](https://docs.microsoft.com/powershell/module/az.machinelearning/export-azmlwebservice) PowerShell из развернутого веб-сервиса, основанного на ресурсах.
...
Важным аспектом управления жизненным циклом приложений является возможность автоматического выполнения и развертывания приложения. В студии машинного обучения Azure (классическая) вы можете сделать это с помощью [модуля PowerShell.](https://aka.ms/amlps) Вот пример сквозных шагов, которые имеют отношение к стандартному процессу автоматического выполнения/развертывания ALM с помощью [модуля PowerShell, разработанного](https://aka.ms/amlps)в рамках системы автоматического обучения. Для выполнения каждого шага используется один или несколько командлетов PowerShell.

1. [Загрузите набор данных.](https://github.com/hning86/azuremlps#upload-amldataset)
...
4. [Выполнить учебный эксперимент](https://github.com/hning86/azuremlps#start-amlexperiment).
...
* Загрузите модуль [Azure Machine Learning Studio (классический) PowerShell](https://aka.ms/amlps) и начните автоматизировать свои задачи ALM.
