...
Отправьте ZIP-файл в качестве набора данных в Studio (классическая модель). Затем создайте и запустите эксперимент, использующий код Python в файле Hello. zip, присоединив его к третьему входному порту модуля **EXECUTE Python Script (выполнение скрипта** ), как показано на следующем рисунке.
