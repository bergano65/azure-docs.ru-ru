...
Эта тема объясняет, как визуализировать и интерпретировать результаты прогнозирования в Azure Machine Learning Studio (классический). Когда вы обучите модель и получите с ее помощью прогнозы (оценки модели), этот результат нужно интерпретировать.



В студии машинного обучения Azure Machine Learning (классический) существует четыре основных вида моделей машинного обучения(классический):
...
В этом документе объясняется интерпретация результатов прогнозирования для каждого из этих модулей. Для обзора этих модулей см. [Как выбрать параметры для оптимизации алгоритмов в Azure Machine Learning Studio (классический)](algorithm-parameters-optimize.md).

В этом разделе рассматривается интерпретация прогноза, а не оценка модели. Для получения дополнительной информации о [How to evaluate model performance in Azure Machine Learning Studio (classic)](evaluate-model-performance.md)том, как оценить модель, см.

Если вы новичок в Azure Machine Learning Studio (классический) и нуждаетесь в помощи в создании простого эксперимента, чтобы начать работу, [см.](create-experiment.md)
...
Студия машинного обучения Azure (классическая) имеет различные модули для работы с каждым из этих типов классификации, но методы интерпретации результатов их прогнозирования схожи.

### <a name="two-class-classification"></a>Двуклассовая классификация
**Пример эксперимента**

Пример проблемы с двухклассовой классификацией — классификация цветов ириса. Задача — классификация цветов ириса по их признакам. Набор данных Iris, представленный в Azure Machine Learning Studio (классический), представляет собой подмножество популярного [набора данных Iris,](https://en.wikipedia.org/wiki/Iris_flower_data_set) содержащий экземпляры только двух видов цветов (классы 0 и 1). Для каждого цветка можно выделить четыре признака: длина наружной доли околоцветника, ширина наружной доли околоцветника, длина внутренней доли околоцветника и ширина внутренней доли околоцветника.
...
16 столбцов слева представляют значения признаков тестового набора. Столбцы с такими именами, как "Scored Probabilities for Class «XX»" (Оцененные вероятности для класса "XX") аналогичны столбцу со значениями оцененной вероятности в двухклассовом эксперименте. Они показывают вероятность того, что соответствующая запись будет принадлежать определенному классу. Например, для первой записи существует вероятность 0.003571, что это "А", 0.000451 вероятность того, что это "B" и так далее. Последний столбец (с оцененными метками) полностью соответствует одноименному столбцу в двухклассовом эксперименте. Класс с наибольшим показателем вероятности выбирается в качестве прогнозируемого класса соответствующей записи. Например, для первой записи наметив омичнем является "F", поскольку она имеет наибольшую вероятность быть "F" (0.916995).
...
Когда вы опубликуете эту веб-службу и введете некоторые входные значения признаков, вы получите результат, который выглядит как на рис. 10. Это рукописное письмо, с извлеченными 16 функциями, по прогнозам, будет "T" с вероятностью 0.9715.
...
Есть несколько вещей, которые мы можем сделать с [поездом Matchbox Рекомендатель][train-matchbox-recommender] модуль в Azure машинного обучения Studio (классический):
...
Типичный эксперимент Azure Machine Learning Studio (классический) для рекомендательной системы выглядит как рисунок 20. Дополнительные сведения об использовании этих модулей системы рекомендаций см. на странице справки в статьях [Train matchbox recommender][train-matchbox-recommender] (Обучение модели рекомендаций Matchbox) и [Score matchbox recommender][score-matchbox-recommender] (Оценка модели рекомендаций Matchbox).
