---
title: Создание нескольких конечных точек для модели
...
ms.locfileid: "79218185"
---
# <a name="use-powershell-to-create-studio-classic-models-and-web-service-endpoints-from-one-experiment"></a>Используйте PowerShell для создания студийных (классических) моделей и конечных точек веб-сервиса из одного эксперимента

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

При машинном обучении часто возникает следующая задача: требуется создать множество моделей с одинаковым рабочим процессом обучения и одинаковым алгоритмом. Но для их обучения нужно использовать разные входные наборы данных. В этой статье показано, как это сделать в масштабе в Azure Machine Learning Studio (классический) с помощью одного эксперимента.
...
Это может быть лучшим подходом, но вы не хотите создавать 1000 учебных экспериментов в Azure Machine Learning Studio (классический) с каждым из них, представляющих уникальное место. Такая задача не только трудоемка, но и малоэффективна, поскольку каждый эксперимент имеет одинаковые компоненты, за исключением набора данных для обучения.

К счастью, вы можете сделать это с помощью [Azure Machine Learning Studio (классический) переподготовки API](/azure/machine-learning/studio/retrain-machine-learning-model) и автоматизации задачи с [Azure Machine Learning Studio (классический) PowerShell.](powershell-module.md)
...
[Обучающий эксперимент](https://gallery.azure.ai/Experiment/Bike-Rental-Training-Experiment-1) для этого примера представлен в [Cortana Intelligence Gallery](https://gallery.azure.ai). Откройте этот эксперимент в рабочей комнате [Azure Machine Learning Studio (классическое).](https://studio.azureml.net)
...
![Модуль reader импортирует данные из капли Azure](./media/create-models-and-endpoints-with-powershell/reader-module.png)
...
![Модуль подготовленной модели выводит на выходной модуль Web service](./media/create-models-and-endpoints-with-powershell/web-service-output.png)
