...
* **Эксперименты** - Галерея содержит широкий спектр экспериментов, которые были разработаны в Azure Machine Learning Studio (классический). К ним относятся как эксперименты, демонстрирующие конкретный метод машинного обучения, так и готовые решения для сложных задач машинного обучения.
...
* **Пользовательские модули** - Вы можете скачать пользовательские модули в вашей студии (классическое) рабочее пространство для использования в ваших собственных экспериментов.
...
Вы можете скачать **эксперименты,** **пользовательские модули,** и **ноутбуки Jupyter** в свой собственный машинный обучения Studio (классический) рабочее пространство для использования в разработке собственных решений.
...
Чтобы загрузить ресурс из studio (классический):

1. В студии (классический), выберите **НОВОЕ**.
...
1. Наведите на этот ресурс указатель мыши и выберите **Open in Studio** (Открыть в Студии).
    ![Эксперимент Open Gallery из студии машинного обучения (классический)](./media/gallery-how-to-use-contribute-publish/open-experiment-from-studio.png)

После того, как ресурс находится в вашем рабочем пространстве, вы можете настроить и использовать его, как вы бы все, что вы создаете в studio (классический).
...
Чтобы продемонстрировать методы аналитики, или дать другим толчок на их решения, вы можете внести свой вклад **эксперименты** вы разработали в studio (классический).
...
1. Откройте свой эксперимент в студии (классический).

1. В списке действий под холстом эксперимента выберите **Publish to Gallery** (Опубликовать в коллекции).

1. В коллекции введите описательные значения для полей **Name** (Имя) и **Tags** (Тэги). Выделите конкретные методы, которые вы использовали, или проблему, которую вы решаете. Примером описательного названия эксперимента является "Двоичная классификация: анализ настроений Twitter".
...
Если необходимо, в эксперименте, опубликованном в коллекции, можно внести изменения в рабочий процесс (модули, параметры и т. д.). В машинном обучении Studio (классический), внести любые изменения, которые вы хотели бы внести в эксперимент, а затем опубликовать еще раз. Опубликованный эксперимент будет обновлен с учетом изменений.
...
1. Выберите **новый пункт**.

1. На странице **Description** (Описание) в раскрывающемся списке **Item type** (Тип элемента) выберите **Tutorial** (Руководство) или **Collection** (Коллекция). Введите имя, краткое содержание, подробное описание и любые теги, которые могут помочь другим пользователям найти этот ресурс. Нажмите **Далее**.

1. На странице **Image Selection** (Выбор изображения) выберите изображение, которое будет отображаться для этого ресурса. Можно отправить свой файл изображения или выбрать готовое изображение. Желательно, чтобы изображение помогало пользователям понять содержимое и назначение этого ресурса. Нажмите **Далее**.
...
**Что происходит с набором данных, который я использовал в эксперименте? Является ли набор данных также опубликован в галерее?**
...
**У меня есть эксперимент, в ходе которого используется модуль импортных данных для получения данных из Azure HDInsight или сервера S'L. Он использует мои учетные данные для получения данных. Могу ли я опубликовать такой эксперимент? Как я могу быть уверен, что мои учетные данные не будут общими?**
