...
Архитектура моделей нейронной сети, создаваемых в Машинное обучение Azure Studio (классическая модель), может быть расширена с помощью net #. Можно выполнить следующие действия.
...
+ Арность свертки составляет три: длина `InputShape`кортежей, `KernelShape` `Stride`, и. `Sharing`
