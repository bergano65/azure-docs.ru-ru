...
description: После развертывания службы машинного обучения из Azure Machine Learning Studio (классика) служба RESTFul Web может использоваться как служба запроса-ответа в режиме реального времени, либо как служба пакетного исполнения.
...
ms.locfileid: "79218233"
---
# <a name="how-to-consume-an-azure-machine-learning-studio-classic-web-service"></a>Как использовать веб-сервис Azure Machine Learning Studio (классический)

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

После развертывания прогностической модели Azure Machine Learning Studio (классической) модели web-службы можно использовать REST API для отправки данных и получения прогнозов. Данные можно отправлять в режиме реального времени или пакетном режиме.

Более подробную информацию о том, как создать и развернуть веб-сервис Machine Learning, можно найти здесь:

* Для учебника о том, как создать эксперимент в машинном обучении Studio (классический), [см.](create-experiment.md)
...
Студия машинного обучения Azure (классическая) имеет два типа услуг:

* Служба запроса-ответ (RRS) - Низкая задержка, высокомасштабируемая служба, которая обеспечивает интерфейс для моделей без состояния, созданных и развернутых из студии машинного обучения (классика).
* Служба пакетного выполнения (BES) — асинхронная служба, оценивающая пакет записей данных.

Для получения дополнительной информации о веб-сервисах машинного обучения можно [ознакомиться на веб-службе машинного обучения.](deploy-a-machine-learning-web-service.md)
...
Вопием на портале [веб-сервисов Microsoft Azure Machine Learning.](https://services.azureml.net)
...
### <a name="classic-web-service"></a>Классическая веб-служба
 Вы также можете получить ключ для классического веб-сервиса от Machine Learning Studio (классический).

#### <a name="machine-learning-studio-classic"></a>Студия машинного обучения (классическая версия)
1. В студии машинного обучения (классический) нажмите **WEB SERVICES** слева.
2. Щелкните веб-службу. **Ключ API** указан на вкладке **Панель мониторинга**.

## <a name="connect-to-a-machine-learning-web-service"></a><a id="connect"></a>Подключение к веб-службе машинного обучения
...
**Справка по API машинного обучения.** Страница справки по API машинного обучения Azure создается при развертывании веб-службы. [См. Учебник 3: Развертывание модели кредитного риска.](tutorial-part3-credit-risk-deploy.md)
...
На [портале веб-сервисов машинного обучения Azure](https://services.azureml.net/):
...
Чтобы подключиться к веб-службе машинного обучения, используйте **httpClient,** передавая ScoreData. Данные набора содержат Вектор свойств, n-мерный вектор числовых параметров, представляющий Данные набора. Службу машинного обучения можно аутентифицировать с помощью ключа API.

Для подключения к веб-службе машинного обучения необходимо установить пакет **Microsoft.AspNet.WebApi.Client** NuGet.

**Установка Microsoft.AspNet.webApi.Client NuGet в визуальной студии**

1. Опубликуйте набор данных Загрузки из набора UCI: Adult 2 веб-службы.
2. Нажмите **Инструменты** > **NuGet пакет менеджер** > **менеджер консоли**.
...
2. Назначьте apiKey ключ из веб-службы. Смотрите **Получить ключ авторизации** выше.
...
2. Назначьте apiKey ключ из веб-службы. В начале этой статьи смотрите **раздел «Получите ключ авторизации»** в начале этой статьи.
3. Назначьте serviceUri универсальный код ресурса запроса.
