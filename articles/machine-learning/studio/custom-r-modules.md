...
description: Эта тема описывает, как автор и развернуть пользовательские R Studio (классический). Здесь поясняется, что такое пользовательский R-модуль, и какие файлы используются для его создания.
...
---
# <a name="define-custom-r-modules-for-azure-machine-learning-studio-classic"></a>Определите пользовательские R-модули для студии машинного обучения Azure (классический)

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

Эта тема описывает, как автор и развернуть пользовательские R Studio (классический). Здесь поясняется, что такое пользовательский R-модуль, и какие файлы используются для его создания. В этом разделе также описан способ создания файлов, определяющих модуль, и регистрации модуля для его развертывания в рабочей области Машинного обучения. Затем более подробно описываются элементы и атрибуты, используемые в определении пользовательского модуля. Кроме этого, здесь рассматриваются способы использования дополнительных функций, файлов и нескольких наборов выходных данных. 



## <a name="what-is-a-custom-r-module"></a>Что такое пользовательский R-модуль?
**Пользовательский модуль** — это модуль, определяемый пользователем, который может быть загружен в рабочее пространство и выполнен в рамках эксперимента Azure Machine Learning Studio (классический). **Пользовательский R-модуль** представляет собой настраиваемый модуль, который выполняет определяемую пользователем R-функцию. **R** — это язык программирования, предназначенный для статистических вычислений и работы с графикой. Его широко используют специалисты по статистике, а также обработке и анализу данных для реализации алгоритмов. В настоящее время R — это единственный язык, поддерживаемый в пользовательских модулях, но в следующих выпусках планируется добавить поддержку и других языков.

Пользовательские модули имеют **первоклассный статус** в Azure Machine Learning Studio (классический) в том смысле, что они могут быть использованы так же, как и любой другой модуль. Их можно выполнять с другими модулями, включенными в опубликованный эксперимент или визуализации. Вы можете управлять алгоритмом, реализуемым модулем, используемыми портами ввода и вывода, параметрами моделирования и другими параметрами, определяющими поведение во время выполнения. Эксперимент с пользовательскими модулями также можно опубликовать в коллекции решений ИИ Azure, чтобы упростить его совместное использование.
...
Чтобы предоставить эту `CustomAddRows` функцию в виде модуля Azure Machine Learning Studio (классический), необходимо создать файл определения XML, чтобы указать, как должен выглядеть и вести себя модуль Custom Add **Rows.** 
...
Чтобы зарегистрировать их в рабочем пространстве машинного обучения, перейдите в рабочее пространство в Azure Machine Learning Studio (классический), нажмите кнопку **«NEW»** внизу и выберите **MODULE -> от ЗИП ПАКЕТ,** чтобы загрузить новый модуль **Custom Add Rows.**
...
Результаты модуля могут быть детерминированными или недетерминированными.** По умолчанию все модули считаются детерминированными. То есть, учитывая неизменный набор входных параметров и данных, модуль должен вернуть те же результаты eacRAND или время выполнения функции. Учитывая такое поведение, Azure Machine Learning Studio (классическая) перезапускает модули, помеченные как детерминированные, если параметр или входные данные изменились. При возвращении кэшированных результатов эксперименты можно проводить гораздо быстрее.
...
Для пользовательских модулей R идентификатор для порта Цип не должен соответствовать каким-либо параметрам функции R. Это связано с тем, что ZIP-файл автоматически извлекается в рабочий каталог R.
...
**Стандартные выходные порты** сопоставляются со значениями, выдаваемыми функцией R, что потом может использоваться последующими модулями. *DataTable* — пока единственный поддерживаемый стандартный выходной порт. (Поддержка *учащихся* и *преобразований* не предвидится.) Выход *DataTable* определяется как:
...
И верните список объектов в список в правильном порядке в 'CustomAddRows.R':
...
Как и в области входов и выходов, крайне важно, чтобы каждый из параметров имеет уникальные значения идентификатора, связанные с ними. В нашем примере quickstart связанный идентификатор/параметр был *своп.*
...
**ColumnPicker** — параметр выбора столбца. Этот тип воспроизводится в UX в виде средства выбора столбца. Элемент **свойств** используется здесь для указания идентификатора порта, из которого выбраны столбцы, где тип порта целевого порта должен быть *DataTable.* Результат выбора столбца передается функции R в виде списка строк, содержащих имена выбранных столбцов. 
...
* *Требуемые свойства*: **portId** - соответствует идентификатору элемента ввода с типом *DataTable.*
...
* *Необязательные свойства*:
  * **по умолчанию** - Значение свойства по умолчанию должно соответствовать значению идентификатора одного из элементов **элемента.**

### <a name="auxiliary-files"></a>Вспомогательные файлы
Любой файл, помещенный в ZIP-файл пользовательского модуля, будет доступен для использования во время выполнения. Вся структура каталогов сохраняется. Это означает, что поиск файлов работает одинаково локально и в Azure Machine Learning Studio (классическое) исполнение. 
...
Например, например, вы хотите удалить любые строки с nAs из набора данных, а также удалить любые дубликаты строк, прежде чем выводить их в CustomAddRows, и вы уже написали функцию R, которая делает это в файле RemoveDupNARows.R:
...
Затем загрузите почтовый файл, содержащий 'CustomAddRows.R', 'CustomAddRows.xml' и 'RemoveDupNARows.R' в качестве пользовательского R-модуля.
