...
Чтобы предоставить эту функцию `CustomAddRows` в качестве модуля Машинное обучение Azure Studio (классическая модель), необходимо создать XML-файл определения, чтобы указать, как должен выглядеть и работать модуль **пользовательского добавления строк** . 
