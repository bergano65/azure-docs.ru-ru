...
description: В этом разделе описывается создание и развертывание пользовательских R Studio. Здесь поясняется, что такое пользовательский R-модуль, и какие файлы используются для его создания.
...
Чтобы предоставить данную `CustomAddRows` функцию как модуль Студии машинного обучения Azure, необходимо создать XML-файл определения, чтобы задать вид и поведение модуля **Добавление пользовательских строк**. 
...
Результаты модуля могут быть детерминированными или недетерминированными.** По умолчанию все модули считаются детерминированными. То есть предъявляется неизменяемый набор входных параметров и данных, модуль должен выдать же другими результаты или функция времени запуска. Это означает, что Студия машинного обучения Azure повторяет выполнение только модулей, помеченных как детерминированные, если параметр или входные данные были изменены. При возвращении кэшированных результатов эксперименты можно проводить гораздо быстрее.
...
Для пользовательских модулей R идентификатор ZIP-порта соответствовать параметрам функции r. нет. Это связано с тем, что ZIP-файл автоматически извлекается в рабочий каталог R.
...
**Стандартные выходные порты**. Сопоставляются со значениями, выдаваемыми R-функцией, что потом может использоваться последующими модулями. *DataTable* — пока единственный поддерживаемый стандартный выходной порт. (Ожидается поддержка *Learners* и *Transforms*.) Выходной объект *DataTable* определяется следующим образом:
...
**Выходные данные визуализации**. Вы можете также указать выходной порт типа *Визуализация*, отображающий выходные данные от графического устройства и консоли выходных данных R. Этот порт не является частью выходных данных функции R и не влияет на порядок других типов выходных портов. Для добавления порта визуализации в пользовательские модули добавьте элемент **Output** со значением *Visualization* для его атрибута **type**:
...
Как и входные и выходные данные, крайне важно, у каждого параметра уникальных значений идентификатора, связанных с ними. В нашем примере быстрого запуска привязанным идентификатором или параметром был *замены*.
...
**ColumnPicker** — параметр выбора столбца. Этот тип воспроизводится в UX в виде средства выбора столбца. **Свойство** элемент используется для указания идентификатора порта, из которой выбираются столбцы, где должен быть тип порта назначения *DataTable*. Результат выбора столбца передается функции R в виде списка строк, содержащих имена выбранных столбцов. 
...
* *Обязательные свойства*: **portId** -совпадает с Идентификатором элемента входных данных с типом *DataTable*.
...
* *Необязательные свойства*:
  * **по умолчанию** -значение свойства по умолчанию должен соответствовать значению идентификатора от одного из **элемент** элементов.

### <a name="auxiliary-files"></a>Вспомогательные файлы
Любой файл, помещенный в ZIP-файл пользовательского модуля, будет доступен для использования во время выполнения. Вся структура каталогов сохраняется. Это означает, что вызов файлов работает как локально, так и при выполнении в Студии машинном обучении Azure. 
...
* Временная файловая система. Файлы, записанные при выполнении пользовательского модуля, не сохраняются между несколькими запусками одного и того же модуля.
* Нет доступа к сети
