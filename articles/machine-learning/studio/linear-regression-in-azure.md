...
2. Создание эксперимента и использовать [Выбор столбцов в наборе данных][select-columns] модуля для выбора тех же признаков данных, которые используются в Excel 
3. Использовать [разбиение данных][split] модуля (с *относительное выражение* режима) для разделения данных на те же наборы обучающих данных, как в Excel 
4. Проведение эксперимента с [линейной регрессии][linear-regression] модуля (по умолчанию только с параметрами), Документирование и сравнение результатов с результатами модели регрессии Excel
...
* При использовании [линейной регрессии][linear-regression] модуля в Studio два метода:
...
Определив необходимую модель Excel, мы приступили к оптимизации модели линейной регрессии машинного обучения. Мы использовали модуль [Filter-Based Feature Selection][filter-based-feature-selection] чтобы усовершенствовать Выбор исходных данных элементов и он помогли нам добиться повышения производительности 4,6% средней абсолютной погрешности. Для будущих проектов мы будем использовать эту функцию, что позволит нам сэкономить недель в итерации атрибутов данных для поиска правильного набора возможностей, используемых для моделирования. 

Далее мы планируем включить дополнительные алгоритмы, такие как [Bayesian][bayesian-linear-regression] or [Boosted Decision Trees][boosted-decision-tree-regression] в нашем эксперименте для сравнения производительности. 
...
Параллельное выполнение экспериментов Студии и регрессии Excel позволило узнать много нового. Создание базовой модели в Excel и ее сравнение с моделями с помощью машинного обучения [линейной регрессии][linear-regression] помогло нам изучить Studio, мы обнаружили возможности усовершенствования производительности выбора и модели данных. 

Мы также поняли, что рекомендуется использовать [Filter-Based Feature Selection][filter-based-feature-selection] ускорять проекты прогноз на будущее. Применив выборку признаков к данным, можно создать улучшенную модель Студии с более высокой общей производительностью. 
