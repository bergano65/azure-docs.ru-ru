---
title: Приступая к работе с R
titleSuffix: Azure Machine Learning Studio
description: Используйте этот учебник программирования R, чтобы приступить к работе с помощью языка R со студией машинного обучения Azure для создания решения прогнозирования.
...
ms.locfileid: "64726546"
---
# <a name="getting-started-with-the-r-programming-language-in-azure-machine-learning-studio"></a>Приступая к работе с R, языке программирования в студии машинного обучения Azure

<!-- Stephen F Elston, Ph.D. -->

## <a name="introduction"></a>Общие сведения

Это руководство поможет вам начать расширение студии машинного обучения Azure с помощью языка программирования R. Выполнив приведенные в этом руководстве инструкции, вы научитесь создавать, тестировать и выполнять код R в Студии машинного обучения. Во время работы с руководством вы создадите полноценное прогнозное решение, используя язык R в Студии машинного обучения.  

В Студии машинного обучения Microsoft Azure содержится множество эффективных модулей машинного обучения и обработки данных. Высокоэффективный язык R считается универсальным языком аналитики. К счастью, возможности аналитики и обработки данных, которыми располагает Студия машинного обучения Azure, можно расширить благодаря использованию языка R. Это сочетание позволяет объединить масштабируемость при развертывании Студии машинного обучения с гибкостью и глубиной аналитики языка R.
...
В этом руководстве мы работа с производству молочных продуктов в Калифорнии и ценах на данных. Эти данные включают в себя информацию о производстве нескольких молочных продуктов и цену молочного жира — эталонного товара за каждый месяц.

Данные, используемые в этой статье, а также скрипты R, можно загрузить из [репозитории MachineLearningSamples-записных книжек/studio-samples](https://github.com/Azure-Samples/MachineLearningSamples-Notebooks/tree/master/studio-samples). Данные в файле `cadairydata.csv` основаны на информации, собранной университетом Висконсина в [ https://dairymarkets.com ](https://dairymarkets.com).

### <a name="organization"></a>План

Вам потребуется несколько шагов, чтобы научиться создавать, тестировать и выполнять код R для анализа и обработки данных в среде Студии машинного обучения Azure.  

* Сначала мы изучим основы использования языка R в Студии машинного обучения Azure.
* Затем рассмотрим различные аспекты ввода и вывода данных, кода R и графики в среде Студии машинного обучения Azure.
...
В этом разделе вы ознакомитесь с основами работы с языком программирования R в Студии машинного обучения. Язык R — мощный инструмент для создания настраиваемых модулей анализа и обработки данных в среде Студии машинного обучения Azure.
...
В Студии машинного обучения сценарии на языке R выполняются в модуле [Выполнить сценарий R][execute-r-script]. Пример модуля [Выполнить сценарий R][execute-r-script] в Студии машинного обучения показан на рис. 1.
 ...
 ![Язык программирования R: в Студии машинного обучения выбран модуль "Выполнение сценария R"](./media/r-quickstart/fig1.png)
...
* В нижней части правой панели отображаются свойства модуля [Выполнить сценарий R][execute-r-script]. Журналы ошибок и вывода можно просмотреть, выбрав соответствующие элементы в этой области.

Мы Конечно, рассмотрим [выполнение скрипта R] [ execute-r-script] более подробно в оставшейся части этой статьи.

При работе с более сложными функциями R для изменения, тестирования и отладки кода я рекомендую использовать RStudio. Как и в случае разработки любого программного обеспечения, создавайте код пошагово, отдельными фрагментами, и тестируйте его на небольших, простых проверочных примерах. Затем вырезайте и вставляйте готовые функции в окно сценария модуля [Выполнить сценарий R][execute-r-script]. Такой подход позволяет в полной мере использовать как интегрированную среду разработки (IDE) RStudio, так и возможности Студии машинного обучения Azure.  

#### <a name="execute-r-code"></a>Выполнение кода R

Любой код R в [выполнение скрипта R] [ execute-r-script] модуль будет выполняться при запуске эксперимента, выбрав **запуска** кнопки. Когда код выполнен, на значке модуля [Выполнить сценарий R][execute-r-script] появляется флажок.

#### <a name="defensive-r-coding-for-azure-machine-learning"></a>Защищенное программирование на R для Машинного обучения Azure

При разработке кода R для, скажем, веб-службы с помощью Студии машинного обучения Azure необходимо предусмотреть, как ваш код будет реагировать на непредвиденные входные данные и исключения. Чтобы не усложнять, я не стал добавлять проверку или обработку исключений во все примеры кода. Однако далее я приведу несколько примеров функций, использующих возможности языка R для обработки исключений.  

Если вам требуется более полное освещение обработке исключений на R, советую прочитать соответствующие разделы книги викхема Wickham, перечислены ниже в [Дополнительные материалы](#appendixb).
...
Большая часть выходных данных при выполнении кода R на платформе Студии машинного обучения Azure находится в файле output.log. Дополнительная информация отображается в файле error.log.  

Если при выполнении вашего кода в Студии машинного обучения происходит сбой, в первую очередь нужно просмотреть файл error.log. В этом файле могут быть полезные сообщения об ошибках, которые помогут вам понять и исправить свою ошибку. Чтобы просмотреть файл error.log, выберите **Просмотр журнала ошибок** на **панели свойств** для [выполнение скрипта R] [ execute-r-script] содержащего ошибку.
...
Выполнить этот код не удается, возникает условие ошибки. Выбрав **Просмотр журнала ошибок** на **панели свойств** появится всплывающее окно, показанный на рис. 2.

  ![Всплывающее окно сообщения об ошибке](./media/r-quickstart/fig2.png)

*Рис. 2. Всплывающее окно сообщения об ошибке*

Похоже, нужно просмотреть файл output.log, чтобы прочитать сообщение об ошибке в коде на R. Выберите [выполнение скрипта R] [ execute-r-script] , а затем выберите **просмотреть файл output.log** товара **панели свойств** справа. Откроется новое окно браузера, где мы увидим следующее:
...
Если вам пока не понятна последняя строка кода, не останавливайтесь. В оставшейся части этой статьи мы подробно рассмотрим использование R в студии.

### <a name="introduction-to-rstudio"></a>Основные сведения об RStudio

RStudio — широко используемый среда IDE для языка R. Я буду использовать RStudio для изменения, тестирования и отладки кода R, используемые в данном руководстве. Когда код на R будет готов и проверен, его можно будет просто вырезать из редактора RStudio и вставить в модуль [выполнения скрипта R][execute-r-script] в Студии машинного обучения.  
...
Учебника введение RStudio доступен по [в интегрированной среде разработки RStudio](https://support.rstudio.com/hc/sections/200107586-Using-RStudio).

Я представляют некоторые дополнительные сведения об использовании RStudio в [руководство по документации по RStudio](#appendixa) ниже.  
...
Полный код для этого раздела находится в [репозитории MachineLearningSamples-записных книжек/studio-samples](https://github.com/Azure-Samples/MachineLearningSamples-Notebooks/tree/master/studio-samples).
...
1. Запустите среду Студии машинного обучения Azure.
1. Выберите **+ создать** в левом нижнем углу экрана и выберите **набора данных**.
...
1. Установите флажок.
1. После передачи набора данных, вы должны увидеть новый набор данных, выбрав **наборы данных** вкладки.  
...
1. Выберите **+ создать** в левом нижнем углу и выберите **эксперимента**, затем **пустой эксперимент**.
...
Рассмотрим данные, загруженные в наш эксперимент. В эксперименте, выберите выходной **набора данных cadairydata.csv** и выберите **визуализировать**. Вы увидите нечто вроде этого (рис. 4):  
...
> [!NOTE]
> Студия машинного обучения Azure обрабатывает файлы ZIP-архива, как если бы они находились в каталоге src/, поэтому необходимо добавлять имя этого каталога в начало имен файлов. Например, если ZIP-файл содержит файлы `yourfile.R` и `yourData.rdata` в корне, вы будете рассматривать их как `src/yourfile.R` и `src/yourData.rdata` при использовании `source` и `load`.

Мы уже обсуждали Загрузка наборов данных в [загрузки набора данных](#loading). После создания и тестирования сценария R, описанного в предыдущем разделе, сделайте следующее:
...
1. Создайте ZIP-файл и скопируйте в него свой скрипт. В Windows, щелкните правой кнопкой мыши файл и выберите **отправку**, а затем **сжатая папка**. Будет создан ZIP-файл, содержащий файл "simpleplot.R".

1. Добавьте свой файл в **наборы данных** в студии машинного обучения, указав тип файла **ZIP**. Теперь вы увидите ZIP-файл среди своих наборов данных.

1. Перетащите ZIP-файл из **наборов данных** на **холст студии машинного обучения**.
...
1. Обязательно выберите **Сохранить**.
...
Выполните эксперимент, выбрав **запуска** кнопки. По завершении выполнения выберите [выполнение скрипта R] [ execute-r-script] модуля, а затем выберите **просмотреть выходной журнал** на панели «Свойства». В браузере откроется новая страница с содержимым файла output.log. Прокрутив ее вниз, вы увидите следующее:
...
Завершив эксперимент, выберите порт вывода результата Dataset1, а затем выберите **визуализировать**. Вы увидите нечто вроде этого (рис. 6):
...
Чтобы просмотреть выходные данные устройства R, выберите порт, а затем на **визуализировать**. На рисунке 7 показаны стандартный вывод данных и стандартный вывод ошибок.
...
Полный код на R для этого раздела доступен в [репозитории MachineLearningSamples-записных книжек/studio-samples](https://github.com/Azure-Samples/MachineLearningSamples-Notebooks/tree/master/studio-samples).
...
Таблицы данных R поддерживают массу возможностей фильтрации. Наборы данных можно разбивать на подмножества, используя логическую фильтрацию по строкам или столбцам. Во многих случаях вам потребуются сложные критерии фильтрации. Ссылки в [Дополнительные материалы](#appendixb) ниже содержится множество примеров фильтрации таблиц данных.  
...
В этом разделе мы выполним несколько простых преобразований значений в некоторых столбцах таблицы данных. Язык R поддерживает практически любые преобразования значений. Ссылки в [Дополнительные материалы](#appendixb) ниже содержится множество примеров.
...
Полный код на R для этого раздела находится в [репозитории MachineLearningSamples-записных книжек/studio-samples](https://github.com/Azure-Samples/MachineLearningSamples-Notebooks/tree/master/studio-samples).
...
Учитывая небольшой объем данных, создадим модель для тренда в RStudio, а затем вырежем результат и вставим его в Студии машинного обучения Azure. RStudio предоставляет интерактивную среду для такого рода интерактивного анализа.
...
По значениям P (`Pr(>|t|)`) в этих выходных данных видно, опустив член не может оказаться слишком дорогим. С помощью функции `update()` изменим эту модель, опустив член со степенью 2.
...
Для проверки корректности создадим график временных рядов данных молочного производства Калифорнии с использованием полученной линии тренда. Для создания модели и построения графика мы добавили приведенный ниже код в модуль [Выполнить сценарий R][execute-r-script] в Студии машинного обучения Azure (а не RStudio). Результат показан на рис. 23.
...
Построим еще один график временного ряда данных по молочному производству Калифорнии, чтобы проверить работу модели. Чтобы создать модель и построить график, мы добавили следующий код в модуль [Выполнить сценарий R][execute-r-script] в Студии машинного обучения Azure.
...
График, полученный в результате выполнения этого кода в Студии машинного обучения Azure, приведен на рис. 24.
...
## <a id="appendixa"></a>Руководство по документации по RStudio

RStudio хорошо описана. Ниже приведены некоторые ссылки на ключевые разделы документации по RStudio, чтобы приступить к работе.

* **Создание проектов** -можно организовывать и управление кодом R в проекты с помощью RStudio. См. в разделе [проектов с помощью](https://support.rstudio.com/hc/articles/200526207-Using-Projects) подробные сведения. Я рекомендую следовать этим инструкциям и создать проект для примеров кода R в этой статье.  
* **Редактирование и выполнение кода R** -RStudio предоставляет интегрированную среду для редактирования и выполнения кода R. См. в разделе [редактирование и выполнение кода](https://support.rstudio.com/hc/articles/200484448-Editing-and-Executing-Code) подробные сведения.
* **Отладка** -RStudio располагает эффективными возможностями отладки. См. в разделе [отладки с помощью RStudio](https://support.rstudio.com/hc/articles/200713843-Debugging-with-RStudio) Дополнительные сведения об этих функциях. Сведения о функции устранения неполадок точек останова, см. в разделе [Устранение неполадок точек останова](https://support.rstudio.com/hc/articles/200534337-Breakpoint-Troubleshooting).
...
* [R для начинающих](https://cran.r-project.org/doc/contrib/Paradis-rdebuts_en.pdf) по Emmanuel Paradis — хорошее начало.  
* [Введение в язык R](https://cran.r-project.org/doc/manuals/R-intro.html) в. н. Венаблес (W. N. Venables) и др. — Переход в немного подробнее.
...
* **Art of R Programming: Tour of Statistical Software Design** по Нормана Мэтлофа является отличным введением в программировании на языке R.  
* **Настольная книга по R** , Paul Teetor предоставляет подход и решение с помощью языка R.  
* **R в действии** по Robert Kabacoff является другой полезная книга. Сопутствующий [веб-сайт Quick R](https://www.statmethods.net/) – это полезный ресурс.
* **R Inferno** по (Patrick Burns) является юмористическом адский Патрика Бернса, которыми можно столкнуться при программировании на R. Книга доступна по адресу [R Inferno](https://www.burns-stat.com/documents/books/the-r-inferno/).
* Если вы хотите углубиться в сложные темы языка, взглянем на книги **расширенные R** по (Hadley Wickham). Интернет-версию эта книга доступна по адресу [ http://adv-r.had.co.nz/ ](http://adv-r.had.co.nz/).

Каталог пакетов временных рядов на R можно найти в [CRAN представления задач: Время анализа рядов](https://cran.r-project.org/web/views/TimeSeries.html). Информацию по определенным пакетам объектов временных рядов см. в документации к этим пакетам.

Книга **Introductory Time Series** with R пола каупертвейта (Paul cowpertwait) и Эндрю Меткалфа введение в использование языка R для анализа временных рядов. Множество других теоретических работ содержат примеры на языке R.

Ниже приведены некоторые полезные ресурсы в Интернете.

* На сайте DataCamp обучение R происходит в вашем браузере с помощью видеоуроков и упражнений по программированию. Здесь представлены интерактивные учебники по последним методам кодирования на R и пакетам R. Бесплатных [интерактивное руководство по R](https://www.datacamp.com/courses/introduction-to-r).
* [Изучить программирование на R, полное руководство](https://www.programiz.com/r-programming) от programiz:.
* Быстрый [руководство по R](https://www.cyclismo.org/tutorial/R/) по Kelly Black из университета кларксон:.
* Существуют по 60 ресурсов по R: [R верхней языковых ресурсов, чтобы улучшить свои навыки данных](https://www.computerworld.com/article/2497464/business-intelligence-60-r-resources-to-improve-your-data-skills.html).
