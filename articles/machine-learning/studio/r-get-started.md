...
description: Используйте этот учебник по программированию R, чтобы начать использовать язык R с Azure Machine Learning Studio (классический) для создания решения для прогнозирования.
...
ms.locfileid: "79218012"
---
# <a name="getting-started-with-the-r-programming-language-in-azure-machine-learning-studio-classic"></a>Начало работы с языком программирования R в студии машинного обучения Azure (классический)
...
Этот учебник поможет вам начать расширение Azure Machine Learning Studio (классический) с помощью языка программирования R. Следуйте этому учебнику по программированию R для создания, тестирования и выполнения R-кода в Studio (классический). При работе с учебником вы создадите полное решение для прогнозирования, используя язык R в Studio (классический).  

Студия машинного обучения Azure (классическая) содержит множество мощных модулей машинного обучения и обработки данных. Высокоэффективный язык R считается универсальным языком аналитики. К счастью, аналитика и манипуляции с данными в Studio (классический) могут быть расширены с помощью R. Эта комбинация обеспечивает масштабируемость и простоту развертывания Studio (классический) с гибкостью и глубокой аналитикой R.
...
В этом руководстве мы будем работать с Калифорнии молочного производства и ценовых данных. Эти данные включают в себя информацию о производстве нескольких молочных продуктов и цену молочного жира — эталонного товара за каждый месяц.

Данные, используемые в этой статье, наряду с R-скриптами, можно загрузить из [MachineLearningSamples-Notebooks/studio-образцов.](https://github.com/Azure-Samples/MachineLearningSamples-Notebooks/tree/master/studio-samples) Данные в `cadairydata.csv` файле были первоначально синтезированы из [https://dairymarkets.com](https://dairymarkets.com)информации, доступной из Университета Висконсина в .

### <a name="organization"></a>План

Мы будем прогрессировать через несколько шагов, как вы узнаете, как создавать, тестировать и выполнять аналитики и обработки данных R код в Azure Machine Learning Studio (классический) среды.  

* Сначала мы изучим основы использования языка R в среде Azure Machine Learning Studio (классическая).
* Затем мы перерастаем к обсуждению различных аспектов ввоза/от данных, R-кода и графики в среде Azure Machine Learning Studio (классическая).
...
## <a name="interact-with-r-language-in-machine-learning-studio-classic"></a><a id="mlstudio"></a>Взаимодействие с языком R в студии машинного обучения (классический)

Этот раздел перенесет некоторые основы взаимодействия с языком программирования R в среде Машинной Студии Обучения (классическая). Язык R является мощным инструментом для создания индивидуальных модулей аналитики и обработки данных в среде Azure Machine Learning Studio (классическая).

Для разработки, тестирования и отладки небольших фрагментов кода на языке R мы будем использовать RStudio. Этот код затем вырезается и вставляется в модуль [Execute R Script,][execute-r-script] готовый к запуску в Azure Machine Learning Studio (классический).  

### <a name="the-execute-r-script-module"></a>Модуль «Выполнение сценария R»

В студии машинного обучения (классический) R-скрипты выполняются в модуле [Execute R Script.][execute-r-script] На рисунке 1 показан пример модуля [Execute R Script][execute-r-script] в студии машинного обучения (классический).

 ![R язык программирования: Модуль execute R Script, выбранный в студии машинного обучения (классический)](./media/r-quickstart/fig1.png)

*Рисунок 1. Студия машинного обучения (классическая) среда, показывающая выбранный модуль Execute R Script.*

Ссылаясь на рисунке 1, давайте рассмотрим некоторые из ключевых частей машинного обучения Studio (классический) среды для работы с [модулем Execute R Script.][execute-r-script]
...
* В нижней части правой панели отображаются свойства модуля [Выполнить сценарий R][execute-r-script]. Вы можете просмотреть журналы ошибок и вывода, выбрав соответствующие пятна этого стекла.

Мы, конечно, будем обсуждать [сценарий выполнения R][execute-r-script] более подробно в остальной части этой статьи.

При работе с более сложными функциями R для изменения, тестирования и отладки кода я рекомендую использовать RStudio. Как и в случае разработки любого программного обеспечения, создавайте код пошагово, отдельными фрагментами, и тестируйте его на небольших, простых проверочных примерах. Затем вырезайте и вставляйте готовые функции в окно сценария модуля [Выполнить сценарий R][execute-r-script]. Такой подход позволяет использовать как интегрированную среду разработки RStudio (IDE), так и мощь студии машинного обучения Azure (классическая).  

#### <a name="execute-r-code"></a>Выполнение кода R

Любой R-код в модуле [Execute R Script][execute-r-script] будет выполняться при запуске эксперимента, выбрав кнопку **Run.** Когда код выполнен, на значке модуля [Выполнить сценарий R][execute-r-script] появляется флажок.

#### <a name="defensive-r-coding-for-azure-machine-learning"></a>Защищенное программирование на R для Машинного обучения Azure

Если вы разрабатываете R-код, например, веб-сервис с помощью Azure Machine Learning Studio (классический), вы должны обязательно планировать, как ваш код будет иметь дело с неожиданным вводом данных и исключениями. Чтобы не усложнять, я не стал добавлять проверку или обработку исключений во все примеры кода. Однако далее я приведу несколько примеров функций, использующих возможности языка R для обработки исключений.  

Если вам нужно более полное лечение обработки R исключения, я рекомендую вам прочитать применимые разделы книги Уикхем, перечисленные ниже в [Дальнейшем чтении](#appendixb).

#### <a name="debug-and-test-r-in-machine-learning-studio-classic"></a>Отчерепно и тест R в машинном обучении Studio (классический)

Повторюсь: я рекомендую использовать RStudio для отладки и тестирования небольших фрагментов кода на R. Но бывают случаи, когда необходимо отслеживать проблемы кода непосредственно в модуле [Выполнить сценарий R][execute-r-script]. Кроме того, это хорошая практика, чтобы проверить свои результаты в машинном обучении Studio (классический).

Выход от выполнения вашего R-кода и на платформе Azure Machine Learning Studio (классическая) находится в основном в output.log. Дополнительная информация отображается в файле error.log.  

Если ошибка происходит в машинном обучении Studio (классический) во время запуска Вашего R-кода, ваш первый курс действий должен быть смотреть на error.log. В этом файле могут быть полезные сообщения об ошибках, которые помогут вам понять и исправить свою ошибку. Для просмотра error.log выберите **журнал ошибок View** на **панели свойств** для сценария [Выполнения R,][execute-r-script] содержащего ошибку.
...
Выполнить этот код не удается, возникает условие ошибки. Выбор **журнала ошибок представления** на **панели свойств** создает дисплей, показанный на рисунке 2.

  ![Всплывающее окно сообщения об ошибке](./media/r-quickstart/fig2.png)

*Рис. 2. Всплывающее окно сообщения об ошибке*

Похоже, нужно просмотреть файл output.log, чтобы прочитать сообщение об ошибке в коде на R. Выберите [сценарий выполнения R,][execute-r-script] а затем выберите элемент **view output.log** на **панели свойств** справа. Откроется новое окно браузера, где мы увидим следующее:
...
#### <a name="packages-in-machine-learning-studio-classic"></a>Пакеты в студии машинного обучения (классический)
...
Если вам пока не понятна последняя строка кода, не останавливайтесь. В остальной части этой статьи мы будем подробно обсуждать использование R в студии (классический) среде.

### <a name="introduction-to-rstudio"></a>Основные сведения об RStudio

RStudio является широко используемым IDE для R. Я буду использовать RStudio для редактирования, тестирования и отладки некоторых R-кода, используемых в этом руководстве. После того, как R-код будет протестирован и готов, вы можете просто вырезать и вставить из редактора RStudio в машинное обучение Studio (классический) [Выполнить R Script][execute-r-script] модуль.  

Если на вашем компьютере еще не установлен язык R, советую сделать это сейчас. Бесплатные загрузки языка R с открытым исходным кодом доступны [https://www.r-project.org/](https://www.r-project.org/)в Сети Всеобъемлющего R Archive Network (CRAN) по адресу . Доступны файлы для скачивания для Windows, Mac OS и Linux или UNIX. Выберите любое зеркало и следуйте инструкциям по загрузке. CRAN также содержит множество полезных пакетов для аналитики и обработки данных.
...
Учебник введение в RStudio доступна на [использование RStudio IDE](https://support.rstudio.com/hc/sections/200107586-Using-RStudio).

Я предоставляю некоторую дополнительную информацию об использовании RStudio в [Руководстве по документации RStudio](#appendixa) ниже.  
...
Полный код для этого раздела находится в [MachineLearningSamples-Notebooks/студии-образцы](https://github.com/Azure-Samples/MachineLearningSamples-Notebooks/tree/master/studio-samples).

### <a name="load-and-check-data-in-machine-learning-studio-classic"></a>Загрузка и проверка данных в студии машинного обучения (классический)

#### <a name="load-the-dataset"></a><a id="loading"></a>Загрузка набора данных

Начнем с загрузки файла **csdairydata.csv** в студию машинного обучения Azure (классический).

1. Запустите свою среду Для машинного обучения Azure (классическая).
1. Выберите **НОВЫЙ в** левом нижнем нисбыте экрана и выберите **набор данных.**
...
1. Выберите контрольный знак.
1. После загрузки набора данных следует увидеть новый набор данных, выбрав вкладку **Datasets.**  

#### <a name="create-an-experiment"></a>Создание эксперимента

Теперь, когда у нас есть некоторые данные в машинном обучении Studio (классический), мы должны создать эксперимент, чтобы сделать анализ.  

1. Выберите **- NEW** в левом нижнем нижнем и выберите **Эксперимент**, затем **пустой эксперимент**.
...
Рассмотрим данные, загруженные в наш эксперимент. В эксперименте выберите выход **набора данных cadairydata.csv** и выберите **визуализацию.** Вы увидите нечто вроде этого (рис. 4):  

![Сводка набора данных cadairydata.csv](./media/r-quickstart/fig4.png)

*Рис. 4. Сводка набора данных cadairydata.csv*

Здесь представлено много полезной информации. Также видны первые несколько строк этого набора данных. Если выбрать какой-либо столбец, в разделе «Статистика» появится более подробная информация об этом столбце. Например, строка типа функций показывает нам, какие типы данных Azure Machine Learning Studio (классические) назначены столбцоку. Такой беглый просмотр данных — хорошая проверка перед началом серьезной работы.

### <a name="first-r-script"></a>Первый сценарий R

Давайте создадим простой первый R-скрипт для экспериментов в Azure Machine Learning Studio (классический). Я создал и протестировал такой сценарий в RStudio:  
...
Теперь мне нужно перенести этот скрипт в Azure Machine Learning Studio (классический). Можно было бы его просто вырезать и вставить. Но в данном случае я перенесу свой скрипт R при помощи ZIP-файла.
...
> [!NOTE]
> Студия машинного обучения Azure (классическая) рассматривает файлы на молнии так, как будто они находятся в каталоге src/, поэтому вам нужно префиксировать имена файлов с этим именем каталога. Например, если ZIP-файл содержит файлы `yourfile.R` и `yourData.rdata` в корне, вы будете рассматривать их как `src/yourfile.R` и `src/yourData.rdata` при использовании `source` и `load`.

Мы уже обсуждали загрузку наборов данных в [наборе данных Load.](#loading) После создания и тестирования сценария R, описанного в предыдущем разделе, сделайте следующее:
...
1. Создайте ZIP-файл и скопируйте в него свой скрипт. На Windows можно нажать правой кнопкой мыши на файл и выбрать **Отправить,** а затем **сжатую папку.** Будет создан ZIP-файл, содержащий файл "simpleplot.R".

1. Добавьте файл в **наборы данных** в Студии машинного обучения Azure (классический), указав тип как **zip.** Теперь вы увидите ZIP-файл среди своих наборов данных.

1. Перетащите и падение почтовый файл из **наборов данных** на **ML Studio (классический) холст**.
...
1. Убедитесь, что вы **выбираете Сохранить**.
...
Выполните эксперимент, выбрав кнопку **Run.** Когда выполнение завершается, выберите модуль [Execute R Script,][execute-r-script] а затем выберите **журнал вывода просмотра** на панели свойств. В браузере откроется новая страница с содержимым файла output.log. Прокрутив ее вниз, вы увидите следующее:
...
После запуска эксперимента выберите выходной порт Result Dataset1, а затем выберите **Визуализацию.** Вы увидите нечто вроде этого (рис. 6):
...
Для просмотра вывода R-устройства выберите порт, а затем **визуализировать.** На рисунке 7 показаны стандартный вывод данных и стандартный вывод ошибок.
...
Полный R-код для этого раздела доступен в [MachineLearningSamples-Notebooks/studio-образцах.](https://github.com/Azure-Samples/MachineLearningSamples-Notebooks/tree/master/studio-samples)
...
Я удалил строку, создававшую матрицу точечной диаграммы, и добавил строку, преобразующую тип столбца Month в фактор. В моем эксперименте я просто вырежу код на R и вставлю его в окно кода модуля [Выполнить сценарий R][execute-r-script]. Вы также можете обновить почтовый файл и загрузить его в Azure Machine Learning Studio (классический), но это занимает несколько шагов.  
...
Проблема в том, что кодирование столбца "Месяц" было бессистемным. В некоторых случаях месяц называется апреля, а в других он сокращенкак апреля. Мы можем решить эту проблему путем обрезки строки до 3 символов. Теперь строка кода выглядит так:
...
Таблицы данных R поддерживают массу возможностей фильтрации. Наборы данных можно разбивать на подмножества, используя логическую фильтрацию по строкам или столбцам. Во многих случаях вам потребуются сложные критерии фильтрации. Ссылки [ниже](#appendixb) содержат обширные примеры фильтрации фреймов данных.  
...
> [!NOTE]
> С этого момента я буду показывать только дополнительный код, который добавляется в модуль [Выполнить сценарий R][execute-r-script]. Новые строки будут добавляться **перед** функцией `str()`. Я использую эту функцию для проверки результатов в Azure Machine Learning Studio (классический).
...
В этом разделе мы выполним несколько простых преобразований значений в некоторых столбцах таблицы данных. Язык R поддерживает практически любые преобразования значений. Ссылки в [Далее ниже](#appendixb) содержат обширные примеры.
...
Полный R-код для этого раздела находится в [MachineLearningSamples-Notebooks/студии-образцах.](https://github.com/Azure-Samples/MachineLearningSamples-Notebooks/tree/master/studio-samples)
...
Учитывая небольшой масштаб данных, я построю модель для тренда в RStudio, а затем вырежу и вставьте полученную модель в Azure Machine Learning Studio (классическая). RStudio предоставляет интерактивную среду для такого рода интерактивного анализа.
...
Из значений`Pr(>|t|)`P () в этом выводе мы видим, что квадратный термин может быть незначительным. С помощью функции `update()` изменим эту модель, опустив член со степенью 2.
...
Для проверки корректности создадим график временных рядов данных молочного производства Калифорнии с использованием полученной линии тренда. Я добавил следующий код в Azure Machine Learning Studio (классический) [Выполнить R Script][execute-r-script] модели (не RStudio), чтобы создать модель и сделать сюжет. Результат показан на рис. 23.
...
Построим еще один график временного ряда данных по молочному производству Калифорнии, чтобы проверить работу модели. Я добавил следующий код в Azure Machine Learning Studio (классический) [Выполнить R Script][execute-r-script] для создания модели и сделать сюжет.
...
Запуск этого кода в Azure Machine Learning Studio (классический) создает сюжет, показанный на рисунке 24.
...
## <a name="guide-to-rstudio-documentation"></a><a id="appendixa"></a>Руководство по документации RStudio

RStudio довольно хорошо документированы. Вот некоторые ссылки на ключевые разделы документации RStudio, чтобы вы начали.

* **Создание проектов** - Вы можете организовать и управлять своим R-кодом в проекты с помощью RStudio. Подробнее об этом можно [узнать об использовании проектов.](https://support.rstudio.com/hc/articles/200526207-Using-Projects) Я рекомендую вам следовать этим направлениям и создать проект для примеров R-кода в этой статье.  
* **Редактирование и выполнение R-кода** - RStudio обеспечивает интегрированную среду для редактирования и выполнения R-кода. Для получения подробной информации смотрите [Код редактирования и выполнения.](https://support.rstudio.com/hc/articles/200484448-Editing-and-Executing-Code)
* **Отладка** - RStudio включает в себя мощные возможности отладки. Дополнительную информацию об этих функциях можно узнать в [rStudio.](https://support.rstudio.com/hc/articles/200713843-Debugging-with-RStudio) Для получения информации о функциях устранения неполадок, [см.](https://support.rstudio.com/hc/articles/200534337-Breakpoint-Troubleshooting)

## <a name="further-reading"></a><a id="appendixb"></a>Дополнительные сведения

Этот учебник по программированию R охватывает основы того, что вам нужно использовать язык R с Azure Machine Learning Studio (классический). Если вы еще не знакомы с языком R, на ресурсе CRAN можно найти два вводных курса:

* [R для начинающих](https://cran.r-project.org/doc/contrib/Paradis-rdebuts_en.pdf) Эммануэль Паради является хорошим местом для начала.  
* [Введение в R](https://cran.r-project.org/doc/manuals/R-intro.html) В. Н. Венаблес (W. N. Venables) и др. — идет в немного больше глубины.
...
* **Искусство R программирования: Тур статистического программного обеспечения Дизайн** Норман Matloff является отличным введением в программировании в R.  
* **R Cookbook** Пола Титора предоставляет проблему и решение подхода к использованию R.  
* **R в действии** Роберт Kabacoff является еще одной полезной вступительной книги. Компаньон [Быстрый R веб-сайт](https://www.statmethods.net/) является полезным ресурсом.
* **R Inferno** Патрик Бернс является удивительно юмористической книги, которая занимается рядом сложных и трудных тем, которые могут возникнуть при программировании в R. Книга доступна бесплатно в [R Inferno](https://www.burns-stat.com/documents/books/the-r-inferno/).
* Если вы хотите глубоко погрузиться в передовые темы в R, посмотрите на книгу **Расширенный R** Хэдли Уикхем. Онлайн-версия этой книги доступна [http://adv-r.had.co.nz/](http://adv-r.had.co.nz/)бесплатно по адресу .

Каталог пакетов серий времени R можно найти в [обзоре задач CRAN: Анализ временных рядов.](https://cran.r-project.org/web/views/TimeSeries.html) Информацию по определенным пакетам объектов временных рядов см. в документации к этим пакетам.

Книга **Вводная серия времени** с R Пол Cowpertwait и Эндрю Metcalfe обеспечивает введение в использовании R для анализа временных рядов. Множество других теоретических работ содержат примеры на языке R.

Вот некоторые большие интернет-ресурсы:

* На сайте DataCamp обучение R происходит в вашем браузере с помощью видеоуроков и упражнений по программированию. Здесь представлены интерактивные учебники по последним методам кодирования на R и пакетам R. Возьмите бесплатный [интерактивный учебник R](https://www.datacamp.com/courses/introduction-to-r).
* [Изучите R Программирование, Окончательное руководство](https://www.programiz.com/r-programming) от Programiz.
* Быстрый [R Учебник](https://www.cyclismo.org/tutorial/R/) Келли Блэк из Университета Кларксона.
* На языковых ресурсах Top R, перечисленных в Top R, более 60 ресурсов R [для улучшения навыков передачи данных.](https://www.computerworld.com/article/2497464/business-intelligence-60-r-resources-to-improve-your-data-skills.html)
