...
**[Microsoft Azure Machine Learning Studio Памятка по алгоритмам](algorithm-cheat-sheet.md)** помогает выбрать правильный машинного обучения алгоритм для решений прогнозной аналитики из машинного обучения Azure Библиотека Studio алгоритмов.
В этой статье рассматриваются способы использования этой памятки.
...
Ниже приведен пример эксперимента из [Коллекции решений ИИ Azure](https://gallery.azure.ai/), в котором используется несколько алгоритмов для одних и тех же данных, а затем сравниваются результаты: [Compare Multi-class Classifiers: Letter recognition](https://gallery.azure.ai/Details/a635502fc98b402a890efe21cec65b92) (Сравнение многоклассовых классификаторов: распознавание букв).
...
Линейность используется во многих алгоритмах машинного обучения. Алгоритмы линейной классификации предполагают, что классы могут быть разделены прямой линией (или ее аналогом для большего числа измерений). К ним относятся логистическая регрессия и метод опорных векторов (в том виде, в котором это реализовано в Студии машинного обучения Azure).
...
Несмотря на то, что он включает в себя «Регрессия» в имени, логистическая Регрессия является действительно мощное средство для [двухклассовой](/azure/machine-learning/studio-module-reference/two-class-logistic-regression) и [мультиклассовых](/azure/machine-learning/studio-module-reference/multiclass-logistic-regression) классификации. Это быстрый и простой метод. Тот факт, что в нем вместо прямой линии используется S-образная кривая, позволяет естественным образом использовать его для разделения данных на группы. Логистическая регрессия приводит к появлению линейных границ классов, поэтому при ее использовании убедитесь, что вам комфортно с линейной аппроксимацией.
...
Так как пространство функций можно подразделить на регионы произвольного размера, легко представить такое разделение, при котором в одном регионе будет только одна точка данных. Это крайний пример чрезмерно высокой точности. Чтобы избежать этого, большие наборы деревьев создаются с особой математической осторожностью, чтобы убедиться, что деревья не связаны друг с другом. Средним для этого "леса решений" является дерево, что позволяет избежать чрезмерно высокой точности. Леса решений могут использовать большой объем памяти. Джунгли решений — это вариант, который используют меньший объем памяти за счет небольшого увеличения времени обучения.
...
Методы опорных векторов находят границу, которая разделяет классы с как можно большей шириной. Если два класса нельзя четко разделить, алгоритмы найдут наилучшую границу, которую смогут. По мере записи в студии машинного обучения, [двухклассовые методы опорных Векторов](/azure/machine-learning/studio-module-reference/two-class-support-vector-machine) делает это только прямой линией (в SVM говорят, используется линейное ядро).
Так как это выполняется с помощью линейной аппроксимации, время выполнения достаточно мало. Где действительно проявляют себя — с данными, функций, как текст или геномные данные. В этих случаях методы опорных векторов позволяют разделить классы быстрее и с меньшей избыточной точностью, чем большинство других алгоритмов; кроме того, они используют небольшой объем памяти.
...
Другой продукт Microsoft Research, [двухклассовый локально глубокий метод опорных векторов](/azure/machine-learning/studio-module-reference/two-class-locally-deep-support-vector-machine) , представляет собой нелинейный вариант метода опорных векторов, который сохраняет большую часть скорости и эффективного использования памяти линейной версии метода. Он идеально подходит для случаев, в которых линейный подход не дает достаточно точных результатов. Разработчики сохранили скорость метода путем разбиения задачи на несколько малых задач метода Опорных векторов. Подробнее узнать о том, как они это сделали, можно в [полном описании](http://proceedings.mlr.press/v28/jose13.html) .
...
При наличии очень конкретной цели вам может повезти со специализированным алгоритмом. В коллекции Студии машинного обучения Azure есть алгоритмы, которые специализируются на:
...
Студия машинного обучения Azure также включает доступ к мощной платформе машинного обучения под названием [Vowpal Wabbit](/azure/machine-learning/studio-module-reference/train-vowpal-wabbit-version-7-4-model).
...
* Список всех категорий алгоритмов, доступных в студии машинного обучения, см. в разделе [Инициализация модели](/azure/machine-learning/studio-module-reference/machine-learning-initialize-model) в справке по алгоритмам и модулям студии машинного обучения.

* Полный список всех алгоритмов и модулей машинного обучения, доступных в студии машинного обучения, расположенных в алфавитном порядке, см. в разделе [Полный список модулей студии машинного обучения](/azure/machine-learning/studio-module-reference/a-z-module-list) в справке по алгоритмам и модулям студии машинного обучения.
