...
1. Вызов веб-службы обучения.  Вызывается служба пакетного выполнения (BES), а не служба отклика на запросы (RRS). Сделать вызов можно с помощью примера кода C#, приведенного на странице справки по интерфейсу API. 
2. Найдите значения параметров *BaseLocation*, *RelativeLocation* и *SasBlobToken*. Эти значения возвращаются в выходных данных после вызова веб-службы обучения. 
...
3. Обновите добавленную конечную точку веб-службы оценки с помощью новой обученной модели. С помощью примера кода, приведенного в статье "Программное переобучение моделей машинного обучения", обновите новую конечную точку, добавленную вами в модель оценки, с помощью повторно обученной модели из веб-службы обучения.
...
**Вариант 2. Использование портала веб-служб Машинного обучения Azure**
...
* Не нужно исправлять веб-службу обучения. Исправить нужно веб-службу оценки.
* Исправлять нужно не ту конечную точку, которая используется по умолчанию в веб-службе, а добавленную вами новую конечную точку веб-службы оценки.
