...
Для развертывания в качестве веб-службы необходимо создать конфигурацию вывода (`InferenceConfig`) и конфигурацию развертывания. Вывод или оценка модели — это этап, в котором развернутая модель используется для прогнозирования, чаще всего в рабочих данных. В конфигурации вывода указываются скрипты и зависимости, необходимые для обслуживания модели. В конфигурации развертывания укажите сведения о том, как обслуживать модель на целевом объекте вычислений.
...
Конфигурация вывода описывает, как настроить модель для выполнения прогнозов. В следующем примере показано, как создать конфигурацию вывода. В этой конфигурации указывается среда выполнения, сценарий записи и (необязательно) файл среды conda:
...
Перед развертыванием необходимо определить конфигурацию развертывания. __Конфигурация развертывания зависит от целевого объекта вычислений, в котором будет размещена веб-служба__. Например, при локальном развертывании необходимо указать порт, на котором служба принимает запросы.
