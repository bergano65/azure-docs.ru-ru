...
Из этого руководства, состоящего из двух частей, вы узнаете, как использовать графический интерфейс Машинного обучения Azure для разработки и развертывания решения прогнозной аналитики для прогнозирования цены любого автомобиля. 
...
Из [второй части](ui-tutorial-automobile-price-deploy.md) этого руководства вы узнаете, как развертывать прогнозную модель в виде веб-службы Azure и прогнозировать цены любого автомобиля на основании отправленных технических характеристик. 

> [!Note]
>Готовая версия этого руководства доступна в качестве примера конвейера.
>
>Вы можете найти ее в **графическом интерфейсе рабочей области**. В разделе **Новый конвейер** выберите **Sample 1 - Regression: Automobile Price Prediction (Basic)** (Пример 1. Регрессия. Прогнозирование цен на автомобили (простой вариант)).
...
1. Щелкните **Визуальный интерфейс**.

    ![Снимок экрана рабочей области, демонстрирующий доступ к ее графическому интерфейсу](./media/ui-tutorial-automobile-price-train-score/launch-visual-interface.png)
...
В этом интерфейсе есть несколько примеров наборов данных, с которыми можно поэкспериментировать. Для этого урока используйте образец набора данных **Automobile price data (Raw)** . 
...
Обычно требуется предварительная обработка набора данных перед его анализом. Возможно, вы заметили некоторые пропущенные значения при визуализации набора данных. Чтобы модель смогла правильно проанализировать данные, необходимо очистить эти недостающие значения. Вам нужно удалить столбцы с большим количеством отсутствующих значений и все строки с отсутствующими значениями.
...
1. В разделе **Свойства** выберите **Параметры** > **Комментарий** и введите "Исключение нормализованных потерь".
...
> [!TIP]
> Удаление недостающих значений из входных данных — необходимое условие для использования большинства модулей в визуальном интерфейсе.
...
1. В области "Свойства" введите "Удаление строк с отсутствующими значениями" в поле **Комментарий**.  
...
Теперь, завершив предварительную обработку данных, вы можете создать прогнозную модель. Вы будете использовать свои данные для обучения модели. Затем вы протестируете модель, чтобы увидеть, насколько точно она может прогнозировать цены.
...
Разделите набор данных на два набора, один из которых потребуется для обучения модели, а второй — для тестирования.
...
1. В области свойств для поля **Fraction of rows in the first output dataset** (Доля строк в первом наборе выходных данных) введите значение 0,7.
    ...
    Это означает, что мы применим 70 % данных для обучения модели и оставим 30 % для тестирования.

1. В области свойств введите текст "Разделение данных на набор для обучения (0,7) и набор для тестирования (0,3)." в поле **Комментарий**.
...
1. Разверните узел **Machine Learning Algorithms** (Алгоритмы машинного обучения).
    ...
    Появится несколько категорий модулей, которые можно использовать для инициализации алгоритма обучения.

1. Для этого эксперимента выберите модуль **Регрессия** > **Линейная регрессия** и перетащите его на холст.
...
1. В диалоговом окне **Столбец метки** разверните раскрывающееся меню и выберите **Имена столбцов**. В текстовое поле введите значение **price** (цена). Именно значение цены будет прогнозировать наша модель.
...
1. Введите **score model** в поле поиска, чтобы найти модуль **Score Model** (Оценка модели), и перетащите этот модуль на холст эксперимента. 
...
1. Введите **evaluate** в поле поиска, чтобы найти модуль **Evaluate Model** (Оценка модели), и перетащите модуль на холст конвейера. 
...
1. Откройте выходные данные модуля **Score Model** (Оценка модели), щелкнув модуль **Score Model** (Оценка модели).

1. В области **Свойства** щелкните **Выходы** > **Визуализировать**. На порту вывода будут показаны прогнозируемые значения цены вместе с известными значениями проверочных данных.
...
1. Откройте выходные данные модуля **Evaluate Model** (Оценка модели), щелкнув модуль **Score Model** (Оценка модели).

1. В области **Свойства** щелкните **Выходы** > **Визуализировать**, а затем выберите **Визуализировать**.
