...
description: Узнайте, какая рабочая область — и зачем нужен для службы машинного обучения Azure.
...
ms.locfileid: "65989835"
---
# <a name="what-is-an-azure-machine-learning-workspace"></a>Что такое рабочей областью машинного обучения Azure?
...
Рабочая область ведет журнал учебных запусков, включая журналы, метрики, выходные данные и скрипты моментальный снимок. Используйте эти сведения для выбора обучающего запуска с оптимальной моделью.  

При наличии модели, но следует его зарегистрировать с рабочей областью. Используйте для зарегистрированной модели и оценки скриптов развертывания экземпляры контейнеров Azure, службе Azure Kubernetes или перепрограммируемые пользователем компоненты массива (FPGA) как конечную точку HTTP на основе REST. Можно также развернуть модель устройства Azure IoT Edge, как модуль.
