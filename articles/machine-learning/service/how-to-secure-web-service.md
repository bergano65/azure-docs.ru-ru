...
В этой статье показано, как защитить развертывание, развернутое с помощью Машинное обучение Azure.

Используйте [протокол HTTPS](https://en.wikipedia.org/wiki/HTTPS) , чтобы ограничить доступ к s и защитить данные, отправляемые клиентами. Протокол HTTPS обеспечивает безопасность обмена данными между клиентом и путем шифрования обмена данными между ними. Шифрование использует [протокол TLS](https://en.wikipedia.org/wiki/Transport_Layer_Security). Протокол TLS иногда по-прежнему называется *SSL* (SSL), который был предшественником TLS.

> [!TIP]
> В пакете SDK для Машинное обучение Azure используется термин "SSL" для свойств, связанных с безопасной связью. Это не значит, что не использует *TLS*. SSL — это просто более широко распознанный термин.
...
> [!WARNING]
> Если вы не используете протокол HTTPS для, данные, отправляемые в службу и из нее, могут быть видны другим пользователям в Интернете.
>
...
Это общий процесс защиты:

1. Получите имя домена.

2. Получение цифрового сертификата.

3. Развертывание или обновление с включенным SSL.

4. Обновите DNS, чтобы указать на.
...
При запросе сертификата необходимо указать полное доменное имя адреса, который планируется использовать для (например, www\.contoso.com). Адрес, который был отмечен в сертификате, и адрес, используемый клиентами, сравнивается с целью проверки удостоверения. Если эти адреса не совпадают, клиент получает сообщение об ошибке.
...
### <a name="deploy-on-aks-and-field-programmable-gate-array-fpga"></a>Развертывание в AKS и программируемом массиве Gates (FPGA)
  ...
  > [!NOTE]
  > Сведения в этом разделе также применяются при развертывании безопасного для конструктора. Если вы не знакомы с использованием пакета SDK для Python, см. статью [что такое пакет SDK для машинное обучение Azure для Python?](https://docs.microsoft.com/python/api/overview/azure/ml/intro?view=azure-ml-py).
...
Затем необходимо обновить DNS, чтобы она указывала на.
...
+ **Для AKS:**
  ...
  [![Машинное обучение Azure: защита с помощью SSL](./media/how-to-secure-web-service/aks-public-ip-address.png)](./media/how-to-secure-web-service/aks-public-ip-address-expanded.png)
