...
1. Создайте сценарий обучения. Этот код использует ```%%writefile%%``` для записи кода обучения в файл ```train.py``` в папке скрипта.
...
#### <a name="predicted-vs-true"></a>Прогнозируемые и истинные значения
