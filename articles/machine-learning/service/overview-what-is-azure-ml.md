...
Для обучения и развертывания без кода попробуйте:
...
|Эксперимент| Масштаб (ограничение на объем данных для обучения: 10 ГБ) | Масштабирование с помощью целевого объекта вычислений|
|Модули для интерфейса| Много | Начальный набор популярных модулей|
|Обучение целевых объектов вычислений| Собственный целевой объект, только ЦП|Вычислительная среда AML (GPU/ЦП)<br/> Виртуальные машины записных книжек |
|Вывод целевых объектов вычислений| Собственный формат веб-службы, без возможности настройки | Служба Azure Kubernetes (формирование выводов в режиме реального времени) <br/>Вычисление AML (пакетное получение выводов) |
|ML-конвейер| Не поддерживается | Создание конвейера <br/> Опубликованный конвейер <br/> Конечная точка конвейера <br/> [What are Azure Machine Learning pipelines?](concept-ml-pipelines.md) (Что такое конвейеры Машинного обучения Azure?)|
|Операции машинного обучения| Развертывание базовой модели и управление ею | Настраиваемое развертывание, управление версиями модели и конвейера|
|Модель| Собственный формат. Не может использоваться за пределами Studio | Стандартный формат, зависит от задания обучения|
|Автоматизированное обучение моделей и настройка гиперпараметров | Нет | Пока отсутствует в графическом интерфейсе. <br/> (Поддерживается пакетом SDK для Python и целевой страницей рабочей области.) |
