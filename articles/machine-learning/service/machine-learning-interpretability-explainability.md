...
ms.locfileid: "58919730"
---
# <a name="azure-machine-learning-interpretability-sdk"></a>Interpretability машинного обучения Azure SDK

Узнайте, как объяснить, почему ваша модель прибывает прогнозы он делает. Interpretability пакета SDK для Azure Machine Learning можно объяснить модели, что важно по следующим причинам:
...
* Во время **обучения**: Конструкторы моделей и оценивающих требуют interpretability средства для объяснения выходные данные модели заинтересованным лицам, чтобы установить доверие. Interpretability инструменты позволяют выполнять отладку модели:

    * Его поведение соответствует целей и задач?
    * Настроена ли он?
...
Interpretability пакета SDK для Azure Machine Learning включает в себя технологии разработанных корпорацией Майкрософт и проверенные сторонних библиотек (например, ФИГУРЫ и ТРАВЯНОЙ). Он обеспечивает общий API интеграции библиотеки и интегрируется со службами машинного обучения Azure. 

Используя этот пакет SDK, вы сможете разъяснить моделей машинного обучения **глобально ко всем данным**, или **локально на точку данных** с использованием технологий современных образом-удобное и масштабируемое.

## <a name="how-does-it-work"></a>Как это работает?

Azure Machine Learning Interpretability могут применяться для понимания поведения глобального модели или конкретного прогноза. Первое из них называется глобального объяснение и называется локальной объяснение.
...
* Интерактивные визуализации для:

    * Прогнозирование
    * Функция и прогнозирования связей
    * Показатели важности компонентов относительные значения глобально, так и локально
...
__Meta explainers__ автоматически выбрать подходящий прямой пояснения и создавать лучшие объяснение информацию, на основе заданной модели и наборы данных. Meta explainers использовать все библиотеки (ФИГУРЫ, ТРАВЯНОЙ, GA2M, имитировал т. д.), которые мы интегрированных или разработаны. Ниже приведены explainers meta, доступных в пакете SDK.

* **Табличные пояснения**: Используется с наборы табличных данных.
* **Текст с пояснениями**: Используется с наборами данных текста.
* **Изображение с пояснениями** использовании наборы данных изображений.
...
* **KNN быстрое пояснение к**. В случае, где должен быть быстрее, чем один оценки/прогноз объяснение можно использовать метод KNN. Во время глобального объяснения примеры инициализации и соответствующие компоненты top-k сохраняются. Чтобы создать описание для каждого образца оценки, метод KNN используется найти наиболее похожий пример примеров инициализации, и функции top-k наиболее похожий пример возвращаются в виде функции top-k пример оценки.
...
Описание функции принимать в качестве входных данных и модели и конвейеры. Если модель, модель необходимо реализовать прогнозирующую функцию `predict` или `predict_proba` , отвечающем соглашению Scikit. Если конвейер (имя сценария конвейера), функция объяснение предполагается, что выполняемый сценарий конвейера возвращает прогноз.

### <a name="local-and-remote-compute-target"></a>Локальная и удаленная целевого объекта вычислений

Пакет SDK Interpretability машины обучения предназначен для работы с оба целевых объекта локальных и удаленных вычислений. 

* Если запустить **локально**, пакет SDK не обращается к службам Azure.

* Если запустить **удаленно**, в служб машинного обучения Azure выполните журнала записываются сведения о запуске. После входа в эту информацию, отчеты и визуализации из объяснение легко доступны на портале рабочей области машинного обучения Azure для анализа пользователем.
...
2. Вызовите пояснения. При создании экземпляра объекта с пояснениями, передаете модели и обучающих данных. При необходимости можно передать интересующую нас характеристику. Если с помощью классификации, передайте имена классов выходных данных.
...
2. Чтобы отправить обучающих запусках, выполните действия, описанные в [настроить целевые объекты вычисления для обучения модели](how-to-set-up-training-targets.md#amlcompute) статьи. Чтобы создать целевой объект вычислений машинного обучения Azure, выполните действия, а затем отправьте сеансе обучения.
