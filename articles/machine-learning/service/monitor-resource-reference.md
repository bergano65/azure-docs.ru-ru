...
### <a name="amlcomputerclusterevents-table"></a>Таблица Амлкомпутерклустеревентс
...
| Version (версия) | Версия образа, используемая при создании узла или виртуальной машины |
...
| Модель | Единица измерения | Description (Описание) |
| ----- | ----- | ----- |
| Сбой развертывания модели | Количество | Число неудачных развертываний моделей. |
| Развертывание модели начато | Количество | Число запущенных развертываний моделей. |
| Развертывание модели прошло | Количество | Число удачных развертываний модели. |
| Не удалось зарегистрировать модель | Количество | Число неудачных регистраций модели. |
| Регистрация модели прошла | Количество | Число удачных регистраций в модели. |

**Квота**

Сведения о квотах используются только для Машинное обучение Azureных вычислений.

| Метрика | Единица измерения | Description (Описание) |
| ----- | ----- | ----- |
| Активные ядра | Количество | Количество активных ядер вычислений. |
| Активные узлы | Количество | Количество активных узлов. |
| Бездействующие ядра | Количество | Число бездействующих ядер вычислений. |
| Бездействующие узлы | Количество | Число бездействующих узлов вычислений. |
| Освобождение ядер | Количество | Число покидает ядер. |
| Покинуть узлы | Количество | Количество покидает узлов. |
| Вытесненные ядра | Количество | Число ядер с вытеснением. |
| Прерванные узлы | Количество | Количество вытесненных узлов. |
| Процент использования квоты | Процент | Процент используемой квоты. |
| Всего ядер | Количество | Общее число ядер. |
| Всего узлов | Количество | Общее число узлов. |
| Непригодные для использования ядра | Количество | Число неиспользуемых ядер. |
| Неиспользуемые узлы | Количество | Число неиспользуемых узлов. |

Ниже приведены измерения, которые можно использовать для фильтрации метрик квот:

| Измерение | Метрики, доступные с | Description (Описание) |
| ---- | ---- | ---- |
...
| Метрика | Единица измерения | Description (Описание) |
| ----- | ----- | ----- |
| Завершенные запуски | Количество | Число завершенных запусков. |
| Неудачные запуски | Количество | Число неудачных выполнений. |
| Запущенные запуски | Количество | Число запущенных запусков. |

Ниже приведены измерения, которые можно использовать для фильтрации метрик выполнения.

| Измерение | Description (Описание) |
| ---- | ---- |
