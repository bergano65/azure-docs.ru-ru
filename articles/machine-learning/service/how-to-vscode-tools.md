...
1. В правом нижнем углу окна появится уведомление, показывающее, что пакета SDK для Azure Machine Learning устанавливается автоматически. Только что созданный среда Python является локальной и частной, и имеет необходимые компоненты Visual Studio Code для работы со службой машинного обучения Azure.
...
Перед началом обучения и развертывания моделей машинного обучения в Visual Studio Code, необходимо создать [рабочей области службы машинного обучения Azure](concept-azure-machine-learning-architecture.md#workspace) в облаке. Эта рабочая область будет содержать модели и ресурсы. 
