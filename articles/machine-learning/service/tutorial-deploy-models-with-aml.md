...
ms.locfileid: "53312176"
---
# <a name="tutorial--deploy-an-image-classification-model-in-azure-container-instance"></a>Руководство.  Развертывание модели классификации изображений в Экземплярах контейнера Azure
...
Теперь вы готовы развернуть модель как веб-службу в [Экземплярах контейнеров Azure](https://docs.microsoft.com/azure/container-instances/). Веб-служба — это образ, в данном случае это образ Docker, который инкапсулирует логику оценки и саму модель. 

В этой части вы будете использовать службу "Машинное обучение Azure", чтобы научиться выполнять следующие задачи:
...
Выполните обучение модели в следующей записной книжке: [Руководство 1. Обучение модели классификации изображений с помощью Службы машинного обучения Azure](tutorial-train-models-with-aml.md).  
...
Создайте скрипт оценки, называемый score.py. Он нужен в вызове веб-службы, чтобы показать, как использовать модель.
...
Создайте файл конфигурации развертывания и укажите необходимое для контейнера службы "Экземпляры контейнеров" количество ЦП и объем ОЗУ в гигабайтах. Так как он зависит от модели, для многих моделей по умолчанию обычно бывает достаточно одного ядра и одного ГБ оперативной памяти. Если в дальнейшем вам потребуется больше ресурсов, создайте образ еще раз и повторно разверните службу.
...
1. Создание образа с использованием:
...
Ниже приведен результат одной случайной выборки тестовых изображений: ![Рисунок, показывающий результаты](./media/tutorial-deploy-models-with-aml/results.png)
...
+ Дополнительные сведения обо всех вариантах развертывания для Службы машинного обучения Azure, включая ACI, Службу Azure Kubernetes, ППВМ и IoT Edge, см. в [этой статье](how-to-deploy-and-where.md).

+ Узнайте, как служба "Машинное обучение Azure" может автоматически выбирать и настраивать подходящий для вашей модели алгоритм, а также создавать эту модель. Ознакомьтесь с руководством по [автоматическому выбору алгоритма](tutorial-auto-train-models.md). 