...
ms.locfileid: "65851122"
---
# <a name="manage-and-request-quotas-for-azure-resources"></a>Управление квотами для ресурсов Azure и их запрашивание

Как и в других службах Azure, существуют ограничения на некоторые ресурсы, связанные со службой "Машинное обучение Azure". Эти ограничения в диапазоне от ограничение на число рабочих областей, вы можете создать для ограничения на фактический базовые ресурсы вычисления, который используется для обучения модели или определение и оценка. 
...
Помните об этих квотах при разработке и увеличении масштаба ресурсов Службы машинного обучения Azure для производственных рабочих нагрузок. Например если кластер не удается достичь целевое количество узлов, которые вы указали, затем может достигнуто ограничение ядер вычислений машинного обучения Azure для вашей подписки. Если ограничение или квоту требуется сделать выше значения ограничения по умолчанию, бесплатно отправьте запрос в службу поддержки клиентов. Из-за ограничений по объемам ресурсов Azure ограничения не могут быть увеличены выше значения максимального предела, как показано в таблицах ниже. Если столбца "Максимальное ограничение" нет, то ресурс не имеет настраиваемого ограничения.
...
Доступные ресурсы:
+ Выделенных ядер на регион имеют ограничение по умолчанию в 24-300 в зависимости от вашего типа предложения подписки.  Количество выделенных ядер для каждой подписки можно увеличить. Чтобы обсудить варианты увеличения, обратитесь в службу поддержки Azure.

+ Ядра с низким приоритетом на регион имеют ограничение по умолчанию в 24-300, в зависимости от вашего типа предложения подписки.  Количество ядер с низким приоритетом для каждой подписки можно увеличить. Чтобы обсудить варианты увеличения, обратитесь в службу поддержки Azure.
...
| Задание максимального времени существования на узле низкий приоритет | 1 день<sup>2</sup> |
...
<sup>2</sup> задания на узле низкого приоритета может обслуживаться любое время, имеется ограничение емкости. Рекомендуется реализовать контрольные точки в задании.
...
1. На левой панели выберите **службы машинного обучения** и из появившегося списка выберите любой рабочей области
