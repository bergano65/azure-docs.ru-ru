...
На рисунке ниже показано, как: ![visulize-Data ](./media/resource-known-issues/aml-visualize-data.png)
...
Иногда при обращении за помощью полезно предоставить диагностические сведения. Чтобы просмотреть некоторые журналы, перейдите на [портал Azure](https://portal.azure.com) и перейдите в рабочую область и выберите **рабочую область > эксперимент > запустить > журналы**.  Эти сведения можно также найти в разделе **эксперименты** на [целевой странице рабочей области (Предварительная версия)](https://ml.azure.com).
...
В большинстве случаев это исключение означает, что в одном из процессов, вызвавших завершение хоровод, возникло базовое исключение. Каждый рейтинг в задании MPI получает собственный выделенный файл журнала в МАШИНном обучении Azure. Эти журналы имеют имя `70_driver_logs`. В случае распределенного обучения имена журналов добавляются в суффиксы `_rank`, чтобы облегчить их различение. Чтобы найти точную ошибку, которая привела к хоровод завершению работы, просмотрите все файлы журнала и найдите `Traceback` в конце файлов driver_log. Один из этих файлов предоставит фактическое базовое исключение. 
