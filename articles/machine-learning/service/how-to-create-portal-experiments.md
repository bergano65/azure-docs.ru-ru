...
description: Создавайте, управляйте и развертывайте автоматические эксперименты машинного обучения на целевой странице рабочей области Машинное обучение Azure (Предварительная версия).
...
ms.locfileid: "72900746"
---
# <a name="create-explore-and-deploy-automated-machine-learning-experiments-with-azure-machine-learnings-workspace-landing-page-preview"></a>Создание, просмотр и развертывание автоматических экспериментов машинного обучения с помощью целевой страницы рабочей области Машинное обучение Azure (Предварительная версия)

 В этой статье вы узнаете, как создавать, изучать и развертывать автоматические эксперименты машинного обучения на целевой странице рабочей области Машинное обучение Azure без единой строки кода. Автоматизированное машинное обучение автоматизирует процесс выбора наилучшего алгоритма для конкретных данных, что позволяет быстро создать модель машинного обучения. Дополнительные [сведения об автоматизированном машинном обучении](concept-automated-ml.md).
...
* Рабочая область машинного обучения Azure. См. раздел [создание машинное обучение Azure рабочей области](how-to-manage-workspace.md).

## <a name="get-started"></a>Начать

1. Войдите на [целевую страницу рабочей области](https://ml.azure.com/workspaceportal/). 

1. Выберите подписку и рабочую область. 

1. Перейдите к левой панели. Выберите **Автоматический ML** в разделе **Автор** .

[панель навигации![портал Azure](media/how-to-create-portal-experiments/nav-pane.png)](media/how-to-create-portal-experiments/nav-pane-expanded.png)

 Если вы впервые выполняете эксперименты, вы увидите экран Начало **работы** . 
...
1. Выберите вычисление для задания профилирования и обучения данных. Список существующих вычислений доступен в раскрывающемся списке. Чтобы создать новое вычисление, следуйте инструкциям на шаге 4.
...
1. Выберите набор данных из контейнера хранилища или создайте его, загружая файл с локального компьютера в контейнер. Общедоступная Предварительная версия поддерживает только локальные передачи файлов и учетные записи хранения BLOB-объектов Azure.
    ...
    1. Чтобы создать новый набор данных из файла на локальном компьютере вычислений, нажмите кнопку **Обзор** и выберите файл. 

    1. Присвойте набору данных уникальное имя и укажите дополнительное описание. 

    1. Нажмите кнопку **Далее** , чтобы отправить его в контейнер хранилища по умолчанию, который автоматически создается в рабочей области, или выберите контейнер хранилища, который вы хотите использовать для эксперимента. 
    ...
    1. Проверьте правильность **параметров и формы предварительного просмотра** . Форма создается на основе типа файла. 
        ...
        Сервис кодирования| Определяет, какой бит следует использовать в таблице схемы символов, чтобы считать набор данных.
...
1. Используемых Дополнительные параметры: дополнительные параметры, которые можно использовать для лучшего управления обучающими заданиями. В противном случае применяются значения по умолчанию, основанные на выборе эксперимента и данных. 
    ...
    Условия выхода| При выполнении любого из этих условий задание обучения останавливается. <br> *Время учебного задания (в минутах)* . продолжительность выполнения задания обучения.  <br> *Максимальное число итераций*: максимальное число конвейеров (итераций) для проверки в задании обучения. Задание не будет выполняться больше указанного числа итераций. <br> *Порог оценки метрики*: минимальный показатель метрики для всех конвейеров. Это гарантирует, что если у вас есть определенная Целевая метрика, вы не тратите больше времени на задание обучения, чем это нужно.
...
Максимальное количество| Максимальное значение столбца. 
...
1. Существует несколько вариантов развертывания. 
    ...
    + Вариант 1. чтобы развернуть оптимальную модель (в соответствии с заданными критериями метрики), выберите пункт развернуть оптимальную модель на странице сведений о запуске.

    + Вариант 2. чтобы развернуть конкретную итерацию модели из этого эксперимента, выполните детализацию по модели, чтобы открыть страницу сведений о выполнении, и выберите пункт развернуть модель.
