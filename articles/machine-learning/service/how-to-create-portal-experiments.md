...
1. Заполнение области " **развертывание модели** ".
    ...
    Description (Описание)| Введите описание, чтобы лучше понять, для чего предназначено это развертывание.
    Тип вычислений| Выберите тип конечной точки, которую требуется развернуть: *Azure Kubernetes Service (AKS)* или *экземпляр контейнера Azure (ACI)* .
    Имя| *Применимо только к AKS:* Выберите имя кластера AKS, в который вы хотите выполнить развертывание.
    Включение проверки подлинности | Выберите этот параметр, чтобы разрешить проверку подлинности на основе маркеров или на основе ключей.
    ...
    Меню *Дополнительно* предоставляет возможности развертывания по умолчанию, такие как сбор данных и параметры использования ресурсов. Если вы хотите переопределить эти значения по умолчанию, это можно сделать в этом меню.
