...
Хотя конвейеры ML могут обучать модели, они также могут подготавливать данные перед обучением и развертыванием моделей после обучения. Одним из основных вариантов использования конвейеров является пакетная оценка. Дополнительные сведения см. в разделе [Pipelines: Оптимизируйте рабочие процессы машинного обучения @ no__t-0.
...
> [!NOTE]
> Чтобы указать максимальное число используемых узлов, обычно необходимо задать `node_count` на число узлов. В настоящее время (04/04/2019) ошибка не может быть связана с этой ошибкой. В качестве обходного решения используйте свойство `amlcompute._cluster_max_node_count` конфигурации запуска. Например, `run_config.amlcompute._cluster_max_node_count = 5`.
...
1. В разделе __Приложения__ выберите __Вычисления__.
    ...
    [Вкладка "![View COMPUTE"](./media/how-to-set-up-training-targets/azure-machine-learning-service-workspace.png)](./media/how-to-set-up-training-targets/azure-machine-learning-service-workspace-expanded.png)
...
> [!TIP]
> В этом примере по умолчанию используется только один узел целевого объекта вычислений для обучения. Чтобы использовать более одного узла, задайте для параметра `node_count` конфигурации запуска необходимое число узлов. Например, следующий код задает количество узлов, используемых для обучения, до четырех:
>
> ```python
...
Эта команда создает вложенную папку `.azureml`, которая содержит файлы конфигурации запуска шаблона для различных целевых объектов вычислений. Вы можете скопировать и изменить эти файлы, чтобы настроить конфигурацию, например добавить пакеты Python или изменить параметры DOCKER.  
...
Дополнительные сведения об этих командах CLI ```az ml``` и полном наборе аргументов см. [в справочной документации](reference-azure-machine-learning-cli.md).
