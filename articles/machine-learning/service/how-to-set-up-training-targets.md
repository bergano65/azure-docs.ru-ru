...
>[!NOTE]
> Код в этой статье протестирован с помощью пакета SDK для Машинного обучения Azure версии 1.0.6.
...
|Целевой объект вычислений для обучения| Ускорение GPU | Автоматическая регистрация.<br/> Настройка гиперпараметров | Автоматическая регистрация.</br> машинное обучение; | Поддержка конвейеров|
|----|:----:|:----:|:----:|:----:|
...
|[Аналитика озера данных Azure](how-to-create-your-first-pipeline.md#adla)| &nbsp; | &nbsp; | &nbsp; | ✓[*](#pipeline-only) |
|[Azure HDInsight](#hdinsight)| &nbsp; | &nbsp; | &nbsp; | ✓ |

<a id="pipeline-only"></a>__*__ Azure Databricks и Azure Data Lake Analytics можно использовать __только__ в конвейере. 

>Вы создаете целевые объекты вычислений для конвейеров машинного обучения, как показано в этой статье, но используете эти вычислительные ресурсы на этапах конвейера вместо указанных ниже способов.  Кроме того, не все шаги конвейера используют конфигурацию запуска, описанную в этой статье.  Дополнительные сведения об использовании целевых объектов вычисления в конвейере см. в разделе [Создание и запуск конвейера машинного обучения](how-to-create-your-first-pipeline.md).
...
В средах, управляемых пользователями, вы несете ответственность за настройку среды и установку каждого пакета, который необходим вашему обучающему сценарию на целевом объекте вычислений. Если ваша учебная среда уже настроена (например, на локальном компьютере), вы можете пропустить этот шаг настройки, установив значение `user_managed_dependencies` на True. Conda не будет проверять вашу среду или устанавливать что-либо за вас.
...
1. **Создание**. Создайте DSVM перед ее использованием для обучения модели. Подробные сведения о создании этого ресурса см. в разделе [Подготовка виртуальной машины Linux (Ubuntu) для обработки и анализа данных](https://docs.microsoft.com/en-us/azure/machine-learning/data-science-virtual-machine/dsvm-ubuntu-intro).
...
* [Повторное использование существующих целевых объектов вычислений](#portal-reuse).
...
### <a id="portal-reuse"></a>Повторное использование существующих целевых объектов вычислений
...
* Обзор справочника по пакету SDK [класса RunConfiguration](https://docs.microsoft.com/en-us/python/api/azureml-core/azureml.core.runconfig.runconfiguration?view=azure-ml-py).
