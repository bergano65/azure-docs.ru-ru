...
Модули Azure IoT Edge основаны на образах контейнеров. Чтобы развернуть модель на устройство IoT Edge, выполните следующие действия для регистрации модели в рабочей области службы "Машинное обучение Azure" и создайте образ Docker. 
...
1. Зарегистрируйте модель в своей рабочей области. Вместо текста по умолчанию введите путь, имя, теги и описание вашей модели:
    ...
    > [!IMPORTANT]
    > Если для обучения модели использовалось Машинное обучение Azure, эта модель уже может быть зарегистрирована в рабочей области. В этом случае пропустите шаг. Чтобы просмотреть список моделей, зарегистрированных в этой рабочей области, используйте `Model.list(ws)`.
