---
title: Руководство по регрессионной модели. Автоматическое обучение моделей
...
Если у вас еще нет подписки Azure, создайте бесплатную учетную запись, прежде чем начинать работу. Попробуйте [бесплатную или платную версию Службы машинного обучения Azure](http://aka.ms/AMLFree).
...
![Сведения о выполнении мини-приложения Jupyter](./media/tutorial-auto-train-models/automl-dash-output.png)
...
Создайте точечную диаграмму для отображения прогнозируемых значений затрат в сравнении с фактическими значениями затрат. Следующий код использует функцию `distance` в качестве оси X и поездку `cost` как ось Y. Первые 100 прогнозируемых и фактических значений затрат создаются в виде отдельных рядов, чтобы сравнивать дисперсию прогнозируемых затрат при каждом значении расстояния поездки. Просматривая график видно, что отношение расстояния и затрат почти линейное, а прогнозируемые значения затрат в большинстве случаев очень близки к фактической цене для той же длительности поездки.
...
Вычислите значение `root mean squared error` результатов. Преобразуйте эти значения в список с помощью кадра данных `y_test`, чтобы сравнить их с прогнозируемыми. Функция `mean_squared_error` принимает два массива значений и вычисляет значение среднеквадратической погрешности между ними. Квадратический корень результата выдает ошибку в тех же единицах, что и переменная y (затраты) и примерно указывает, насколько прогнозы отличаются от фактического значения.
