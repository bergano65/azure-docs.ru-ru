...
![Архитектура Azure службы машинного обучения и рабочий процесс](./media/concept-azure-machine-learning-architecture/workflow.png)

## <a name="workflow"></a>Рабочий процесс

Машинного обучения модели рабочего процесса обычно выполняется в следующей последовательности:

1. **Обучение**
    + Разработка машинного обучения, обучение скрипты в **Python** или в визуальный интерфейс.
...
1. **Пакет** — когда обнаружен категории "Удовлетворительно" запуск, регистрация сохраненной модели в **реестра модели**.

1. **Проверка** - **запроса эксперимента** для метрики журнала текущих и прошлых запусков. Если метрики не указывают желаемый результат, необходимо вернуться обратно к шагу 1 и выполнить итерацию сценариев.

1. **Развертывание** -разработать скрипт оценки, который использует эту модель и **развертывание модели** как **веб-службы** в Azure или **устройство IoT Edge**.

1. **Монитор** -мониторинг для **данные о смещении** между обучающих наборов данных и вывод данных развернутой модели. При необходимости, цикл обратно к выполнению этапа 1, чтобы переобучить модель новыми данными обучения.

## <a name="tools-for-azure-machine-learning"></a>Средства для машинного обучения Azure 

Эти средства можно используйте для машинного обучения Azure:

+  Взаимодействие со службой в любой среде Python с [Azure Machine Learning и пакет SDK для Python](https://docs.microsoft.com/python/api/overview/azure/ml/intro?view=azure-ml-py).
+ Автоматизация выполнения машинного обучения действий с [интерфейса командной строки Azure Machine Learning](https://docs.microsoft.com/azure/machine-learning/service/reference-azure-machine-learning-cli).
+ Написать код в Visual Studio Code с помощью [расширение Azure Machine Learning VS Code](how-to-vscode-tools.md) 
+ Используйте [визуальный интерфейс (Предварительная версия) для службы машинного обучения Azure](ui-concept-visual-interface.md) для выполнения действий рабочего процесса без написания кода.
...
Невозможно удалить зарегистрированный модели, который используется с активным развертыванием.
