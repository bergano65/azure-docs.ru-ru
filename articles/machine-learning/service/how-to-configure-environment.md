...
* [Visual Studio Code](#vscode). Если вы используете Visual Studio Code, [расширение машинное обучение Azure](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai) включает в себя расширенную языковую поддержку для Python, а также функции, обеспечивающие более удобную и эффективную работу с машинное обучение Azureной службой.
...
| Среда выполнения Databricks |всегда| Любая немашинная среда выполнения (не МАШИНное обучение 4. x, 5. x) |
...
1. Отслеживать ошибки до тех пор, покасостояние не изменится на "подключено", что может занять несколько минут.  Если этот шаг завершается ошибкой:
