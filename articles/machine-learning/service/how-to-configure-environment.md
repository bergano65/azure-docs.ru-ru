...
* [Записные книжки Azure](#aznotebooks). Служба Jupyter Notebook, размещенная в облаке Azure. Это самый простой способ начать работу, так как пакет SDK для Машинного обучения Azure уже установлен.
...
1. [Создайте библиотеку](https://docs.databricks.com/user-guide/libraries.html#create-a-library) для установки и подключения к кластеру пакета SDK Машинного обучения Azure для пакета Python PyPi `azureml-sdk[databricks]`.  
    По завершении библиотека присоединится, что будет выглядеть так, как показано на следующем рисунке. Обратите внимание на эти [распространенные проблемы с Databricks](resource-known-issues.md#databricks).
   ...
   b. Выберите имя кластера в таблице. 
