...
* Собственные [записной книжки на основе облака виртуальной Машины](#notebookvm): Используйте вычислительных ресурсов в рабочей станции для запуска записные книжки Jupyter. Это самый простой способ начать работу, так как пакет SDK для Машинного обучения Azure уже установлен.
...
Рабочая область службы машинного обучения Azure. Чтобы создать рабочую область, см. в разделе [создать рабочую область службы машинного обучения Azure](setup-create-workspace.md). Рабочая область — все что нужно приступить к работе с собственным [сервер записных книжек на основе облака](#notebookvm), [DSVM](#dsvm), [Azure Databricks](#aml-databricks), или [записных книжек Azure](#aznotebooks).
...
## <a id="notebookvm"></a>Собственные облачные записной книжки виртуальной Машины

Виртуальная машина записной книжки (Предварительная версия) — Azure безопасные, облачной рабочей станции, предоставляются по обработке и анализу данных на сервере записной книжки Jupyter, JupyterLab и полностью подготовленные среде машинного Обучения. 

Записная книжка виртуальной Машины: 

+ **Безопасность**. Так как доступ к виртуальной Машине и записной книжки защищен HTTPS и Azure Active Directory по умолчанию, ИТ-специалисты могут легко обеспечивать единого входа и другие функции безопасности, такие как многофакторная проверка подлинности.

+ **Предварительно настроенные**. Эта полностью подготовиться среда машинного Обучения Python рисует его pedigree из популярных Data Science VM IaaS и включает в себя:
  + Azure ML пакет SDK для Python (последняя версия)
  + Автоматическая настройка для работы с рабочей областью
  + На сервере записной книжки Jupyter
  + Записная книжка JupyterLab интегрированной среды разработки
  + Предварительно настроенные драйверы GPU 
  + Выбор платформ глубокого обучения
 

  Если в код, виртуальная машина включает в себя учебники и образцы, которые облегчают изучение и узнайте, как использовать службы машинного обучения Azure. Примеры записных книжек хранятся в учетной записи хранилища BLOB-объектов Azure к рабочей области, что делает их совместного использования на виртуальных машинах. При запуске, они также иметь доступ к хранилищам данных и вычислительные ресурсы рабочей области. 

+ **Простая установка**: Создайте в любое время с в рабочей области машинного обучения Azure. Укажите только имя и укажите тип виртуальной Машины Azure. Попробуйте прямо сейчас с этим [краткое руководство: Использование облачного сервера записных книжек для начала работы с Машинным обучением Azure](quickstart-run-cloud-notebook.md).

+ **Настраиваемые**. Во время управляемого и безопасного виртуальная машина, которая вы сохраняете полный доступ к возможностям оборудования и настройте его в ваше сердце желания. Например быстро создайте последнюю версию NVidia V100 на базе виртуальной Машины, чтобы выполнить отладку пошаговые novel архитектуры нейронных сетей.

В противном случае плата за ВМ записной книжки, [остановка виртуальной Машины записную книжку](quickstart-run-cloud-notebook.md#stop-the-notebook-vm). 
...
* **Загрузите файл**: На [портале Azure](https://ms.portal.azure.com) выберите **Скачать config.json** в разделе **Обзор** рабочей области.
