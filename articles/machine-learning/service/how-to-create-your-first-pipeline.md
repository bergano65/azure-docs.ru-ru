...
Промежуточные данные (или выходные данные шага) представляет объект [PipelineData](https://docs.microsoft.com/python/api/azureml-pipeline-core/azureml.pipeline.core.pipelinedata?view=azure-ml-py). `output_data1` создается как выходные данные шага и используется в качестве входных данных одного или нескольких последующих шагов.  `PipelineData` представляет зависимость данных между шагами и неявно определяет порядок выполнения шагов в конвейере.
...
После определения шагов следует создать конвейер, добавив в него некоторые или все созданные шаги.
...
При отправке конвейера проверяются зависимости для каждого шага и создается моментальный снимок папки, указанной в качестве исходного каталога, который передается в Службу машинного обучения Azure.  Если исходный каталог не указан, передается текущий локальный каталог.
...
При первом запуске конвейера происходит следующее.

* На целевой объект вычислений скачивается моментальный снимок проекта из хранилища BLOB-объектов, связанного с рабочей областью.
* Создается образ Docker, соответствующий каждому шагу в конвейере.
* Образ Docker для каждого шага скачивается на целевой объект вычислений из реестра контейнеров.
* Если в шаге указан объект `DataReference`, то подключается хранилище данных. Если подключение не поддерживается, то данные копируются на целевой объект вычислений.
* Шаг выполняется на целевом объекте вычислений, указанном в определении этого шага. 
* Создаются артефакты, указанные в шаге (журналы, stdout и stderr, метрики и выходные данные). Затем эти артефакты передаются в хранилище данных пользователя по умолчанию для хранения.

![Запуск эксперимента в виде конвейера](./media/how-to-create-your-first-pipeline/run_an_experiment_as_a_pipeline.png)
...
Все опубликованные конвейеры используют конечную точку REST для вызова запуска конвейера из внешних систем, таких как клиенты не на платформе Python. Эта конечная точка обеспечивает "управляемую повторяемость" для сценариев пакетной оценки и переобучения.
