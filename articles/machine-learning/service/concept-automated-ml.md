...
Создание прогнозов является составной частью любого бизнеса, будь то требование доход, инвентаризации, продажи или клиента. Автоматически использует машинное обучение количество объединенные методики и подходы к рекомендуем прогноза временные ряды высокого качества. Временных рядов экспериментов в машинном обучении автоматическая рассматриваются как проблема многофакторная Регрессия. После временных рядов значений, «сводятся» в качестве дополнительных измерений для регрессора вместе с другими предикторов. 

Этот подход, в отличие от методы классические временных рядов, имеет преимущество — естественным образом включения нескольких контекстных переменных, а также их связь друг с другом во время обучения. В реальных приложениях прогнозирования нескольких факторов может повлиять на прогноз. Например при прогнозирование продаж, взаимодействие между исторические тенденции, курса обмена валют и цена совместно диске продаж результат. Дополнительные преимущества — немедленно применить все последние нововведения в моделях регрессии для прогнозирования.

Насколько далеко в будущее прогноза должны расширять (период прогноза) является частью спецификации основных прогноза. Требуется параметр `max_horizon` в эксперименте определяет количество периодов единицы (в зависимости от интервала времени обучающих данных, например, ежемесячно, еженедельно forecaster должна прогнозировать out. 

Автоматические ML узнает один, но часто внутренне ветвями модели для всех элементов в горизонты набора данных и прогнозирования. Больше данных, таким образом, доступных для оценки параметров модели и обобщения невидимым серии становится возможным. 

Признаки, извлеченные из обучающих данных играют важную роль. Автоматические машинного Обучения выполняет стандартные этапы предварительной обработки и создает дополнительные функции временных рядов (например, год, месяц, день недели и т.д.) для записи сезонные и максимально увеличить точность прогнозирования. 

Если подходит для вашего сценария, можно настроить автоматическое машинного Обучения для создания задержек (`target_lags`) или статистическая обработка данных скользящего интервала (`target_rolling_window_size`) из целевого объекта (`y_value`) за последние значения. 
