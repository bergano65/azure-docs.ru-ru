...
> [!IMPORTANT]
> Возможности пространства имен `azureml.contrib` быстро изменяются, так как мы работаем над улучшением службы. Поэтому все, что доступно в этом пространстве имен, считается предварительными версиями компонентов, поддержка которых корпорацией Майкрософт ограничена.
>
> Если необходимо протестировать их в локальной среде разработки, можно установить эти компоненты в пространстве имен участников сообщества, используя следующую команду.
> 
> ```shell
