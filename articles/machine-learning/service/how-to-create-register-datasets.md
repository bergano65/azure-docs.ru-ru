...
* [Рабочая область службы машинное обучение Azure](https://docs.microsoft.com/azure/machine-learning/service/setup-create-workspace)
...
Загрузите файлы с локального компьютера, указав путь к файлу или папке с [`auto_read_files()`](https://docs.microsoft.com/python/api/azureml-core/azureml.core.dataset(class)?view=azure-ml-py#auto-read-files-path--include-path-false-) помощью метода `Dataset` из класса.  Этот метод выполняет следующие действия, не требуя указывать тип файла или аргументы синтаксического анализа:
