...
description: Сведения об эффективной настройке гиперпараметров для модели глубокого или машинного обучения с помощью службы "Машинное обучение Azure". Вы узнаете, как определить область поиска параметров, указать основную метрику для оптимизации и раннего завершения плохо выполняемых запусков.
ms.author: swatig
author: swatig007
ms.reviewer: sgilley
...
Выборка Байеса поддерживает `choice` `uniform`только распределения по области поиска. `quniform`
...
Пакет SDK для Машинное обучение Azure предоставляет [мини](https://docs.microsoft.com/python/api/azureml-widgets/azureml.widgets.rundetails?view=azure-ml-py) -приложение записной книжки, которое визуализирует ход выполнения обучения. Следующий фрагмент кода визуализирует все запуски по настройке гиперпараметров в одном отображении, создаваемом в записной книжке Jupyter:
