...
Автоматические машинного Обучения можно использовать для объединения подходы и технические приемы и получить рекомендуется, высокого качества временные ряды прогноза. Автоматические эксперимента временных рядов рассматривается как проблема многофакторная Регрессия. Последние значения временных рядов выполняется «сведение» в качестве дополнительных измерений для регрессора вместе с другими предикторов. 
...
Вы можете [Настройка](#config) насколько далеко в будущее прогноза должны расширять (период прогноза), а также задержка и многое другое. Автоматические ML узнает один, но часто внутренне ветвями модели для всех элементов в горизонты набора данных и прогнозирования. Больше данных, таким образом, доступных для оценки параметров модели и обобщения невидимым серии становится возможным. 

Признаки, извлеченные из обучающих данных играют важную роль. И автоматических машинного Обучения выполняет стандартные этапы предварительной обработки и создает дополнительные функции временных рядов для записи сезонные и максимально увеличить точность прогнозирования. 
...
|`max_horizon`|Определяет максимальный требуемый период прогноза в единицах частоты временных рядов. Единицы основаны на интервал времени обучающих данных, например, ежемесячно, еженедельно, что forecaster должен прогнозировать.|✓|
