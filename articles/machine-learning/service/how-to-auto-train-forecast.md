...
Этот набор данных является простым примером еженедельно данные о продажах для у организации есть два хранилища, А и б. Кроме того, имеется функция для `week_of_year` , обеспечивая модели для определения еженедельного сезонности. Поле `week_starting` представляет очистки временных рядов с еженедельной частотой, а в поле `sales_quantity` целевой столбец для выполнения прогнозов. Считывать данные в таблице данных Pandas, а затем используйте `to_datetime` функции, чтобы убедиться, временной ряд `datetime` типа.
...
В этом случае данные уже отсортированы по возрастанию по полю время `week_starting`. Тем не менее при настройке эксперимент, убедитесь, что нужное время столбец отсортирован по возрастанию для построения допустимых временных рядов. Предположим, что данные содержат 1000 записей и добавьте детерминированным разбиения в данные для создания обучающих и проверочных наборов данных. Разделяйте целевое поле `sales_quantity` для создания прогноза обучения и тестовым групп.
...
Создайте необходимые параметры временных рядов как объект словаря. Задайте `time_column_name` для `week_starting` поле в наборе данных. Определение `grain_column_names` параметр, чтобы убедиться, что **две отдельные группы временных рядов** создаются для наших данных — одну для магазина А и б. Наконец, задайте `max_horizon` до 50 для прогнозирования для всего теста значение.
...
Теперь, когда общая точность модели будет определено, реалистичные следующим шагом является использование модели для прогнозирования неизвестных будущих значений. Просто введите набор данных в тот же формат, что проверочный набор `X_test` , но в будущих значений даты и времени и полученный прогноза набора не спрогнозированные значения для каждого шага временных рядов. Предположим, были последние записи в наборе данных временных рядов для начала недели 12/31/2018. Для прогнозирования спроса на следующую неделю (или любое количество периодов для прогнозирования, < = `max_horizon`), создайте одно время записи ряда для каждого магазина, для начала недели 01/07/2019 г.
