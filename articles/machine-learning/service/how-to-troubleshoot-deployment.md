...
* Изучите политики доступа Key Vault, а затем используйте эти политики, `accessPolicies` чтобы задать свойство шаблона.
...
> [!IMPORTANT]
> Для локальных развертываний веб-служб требуется работающая установка DOCKER в локальной системе. Перед развертыванием локальной веб-службы необходимо запустить DOCKER. Сведения об установке и использовании DOCKER см. в разделе [https://www.docker.com/](https://www.docker.com/).

> [!WARNING]
> Локальные развертывания веб-служб не поддерживаются в рабочих сценариях.

Для локального развертывания измените код, используя `LocalWebservice.deploy_configuration()` для создания конфигурации развертывания. Затем используйте `Model.deploy()` для развертывания службы. В следующем примере модель, которая содержится в `model` переменной, развертывается в качестве локальной веб-службы:
...
1. Во время отладки может потребоваться внести изменения в файлы в образе без его повторного создания. Чтобы установить текстовый редактор (VIM) в образе DOCKER, создайте новый текстовый файл `Dockerfile.steps` и используйте в качестве содержимого файла следующее:
...
1. Чтобы создать образ, использующий `Dockerfile.steps` файл, `docker_file` используйте параметр при создании образа. В следующем примере показано, как это сделать:

    > [!NOTE]
    > В этом примере предполагается, что `ws` указывает на рабочую область машинное обучение Azure и которая `model` является развертываемой моделью. `myenv.yml` Файл содержит зависимости conda, созданные на шаге 1.
...
В этом примере текста реестр имеет `myregistry` имя, а образ `myimage`называется. Версия образа — `1`.
...
1. Чтобы пройти проверку подлинности в реестре контейнеров Azure (запись контроля доступа), который содержит образ, используйте следующую команду. Замените `myregistry` на тот, который возвращается при регистрации образа:

    ```azurecli
    az acr login --name myregistry
    ```

1. Чтобы скачать образ на локальный DOCKER, используйте следующую команду. Замените `myimagepath` на расположение, возвращенное при регистрации образа:

    ```bash
    docker pull myimagepath
    ```

    Путь к образу должен быть похож `myregistry.azurecr.io/myimage:1`на. Где `myregistry` находится реестр, `myimage` является изображением, а `1` — версией образа.
    ...
    Время, необходимое для завершения загрузки, зависит от скорости подключения к Интернету. Во время процесса отображается состояние загрузки. После завершения загрузки можно использовать `docker images` команду, чтобы убедиться, что она загружена.

1. Чтобы упростить работу с изображением, используйте следующую команду, чтобы добавить тег. Замените `myimagepath` на значение Location из шага 2.
