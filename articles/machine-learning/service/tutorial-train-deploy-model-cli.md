...
* `train-sklearn.py`: сценарий обучения. Этот файл обучает модель.
...
В терминале или командной строке используйте следующие команды, чтобы изменить каталоги на `mlops` Directory, а затем подключиться к рабочей области:
...
В этом примере для обучения модели используется виртуальная машина записной книжки Машинное обучение Azure. Чтобы создать виртуальную машину записной книжки, используйте следующую команду:
...
> [!IMPORTANT]
> Имя целевого объекта вычислений (`cpu` в данном случае) важно; на него ссылается файл `.azureml/sklearn.runconfig`, используемый в следующем разделе.

## <a name="submit-the-training-run"></a>Отправить обучающий запуск

Чтобы начать обучение на целевом `cpu` вычислений, перейдите в каталог `model-training` и выполните следующую команду:
...
Параметр `-c sklearn` указывает файл `.azureml/sklearn.runconfig`. Как упоминалось ранее, этот файл содержит сведения, используемые для настройки среды, используемой обучающим запуском. Если просмотреть этот файл, вы увидите, что он ссылается на `cpu`ный целевой объект вычислений, созданный ранее. В нем также указано количество узлов, используемых при обучении (`"nodeCount": "4"`), и содержится `"condaDependenciees"` раздел, в котором перечислены пакеты Python, необходимые для запуска сценария обучения.
...
Если вы просматриваете `train-sklearn.py`, вы заметите, что он также использует альфа-значение при сохранении обученных моделей в файл. В этом случае он обучает несколько моделей. Первый из них должен быть самым лучшим. Просмотрев приведенные выше выходные данные и код, модель с альфа-составляющей 0,95 была сохранена как `./outputs/ridge_0.95.pkl`

Модель была сохранена в каталоге `./outputs` в целевом объекте вычислений, где он был обучен. В этом случае виртуальная машина записной книжки Машинное обучение Azure в облаке Azure. Процесс обучения автоматически отправляет содержимое каталога `./outputs` из целевого объекта вычислений, в котором выполняется обучение в рабочей области Машинное обучение Azure. Он хранится в рамках эксперимента (`myexperiment` в этом примере).
...
Первая команда скачивает зарегистрированную модель в текущий каталог. Имя файла `ridge_0.95.pkl`— это файл, на который указывает ссылка при регистрации модели. Вторая команда регистрирует локальную модель (`-p "ridge_0.95.pkl"`) с тем же именем, что и Предыдущая регистрация (`mymodel`). На этот раз возвращенные данные JSON выводит версию как 2.

## <a name="deploy-the-model"></a>Развертывание модели

Чтобы развернуть модель, перейдите в каталог `model-deployment`, а затем используйте следующую команду:
...
Если вы планируете продолжить использовать рабочую область Машинное обучение Azure, но хотите избавиться от `cpu`ного целевого объекта вычислений, созданного для обучения, используйте следующую команду:
