...
### <a name="visual-interface"></a>Графический интерфейс
+ **Функции предварительной версии**
  + Добавленный модуль «Execute R script» в визуальный интерфейс.

### <a name="azure-machine-learning-sdk-for-python-v1048"></a>Машинного обучения Azure SDK для Python v1.0.48

+ **Новые функции**
  + **azureml-opendatasets**
    + **azureml-contrib-opendatasets** теперь доступен как **azureml opendatasets**. Старого пакета можно по-прежнему работать, но мы рекомендуем с помощью **azureml opendatasets** двигаться вперед, более широкие возможности и улучшения.
    + Этот новый пакет позволяет зарегистрировать открытых наборов данных в качестве набора данных в рабочей области AML и использовать любые функциональные возможности, которые предлагает набор данных.
    + Он также включает существующие возможности, такие как использование открытых наборов данных в качестве кадров данных Pandas или SPARK, и расположение JOIN для несколько наборов данных, такие как погода.

+ **Функции предварительной версии**
    + HyperDriveConfig теперь могут принимать объект конвейера в качестве параметра для поддержки настройки гиперпараметров с помощью конвейера.

+ **Исправления ошибок и улучшения**
  + **azureml-train-automl**
    + Исправлена ошибка, о потере типы столбцов после преобразования.
    + Исправлена ошибка, чтобы разрешить y_query быть тип объекта, содержащего None (s) в начале. 
    + Исправлена проблема в процедуры выбора совокупности, которая без необходимости росла результирующего набора, даже если показатели остается постоянным.
    + Исправлена проблема с параметрами whitelist_models и blacklist_models в AutoMLStep.
    + Исправлена проблема, которая препятствовала использованию предварительной обработки при AutoML использовался бы в контексте конвейеров машинного Обучения Azure.
  + **azureml-opendatasets**
    + Перемещенный azureml-contrib-opendatasets для opendatasets машинного обучения Azure.
    + Допускается открытого набора данных классов регистрироваться для рабочей области AML и легко использовать возможности AML набора данных.
    + Улучшенная NoaaIsdWeather значительно улучшение производительности в версии не SPARK.
  + **azureml-explain-model**
    + Обновленные объекты interpretability электронной документации.
    + Добавлен batch_size имитировать пояснения при include_local = False для потоковой передачи глобального объяснения в пакеты, чтобы сократить время выполнения DecisionTreeExplainableModel.
    + Исправлена проблема, где `explanation.expected_values` иногда возвращает значение с плавающей запятой, а не список с значение с плавающей запятой в нем.
    + Добавлена ожидаемые значения для выходных данных для отражения пояснения в automl объяснить библиотека модели.
    + Важность функции перестановки исправлена в том случае, когда аргумент преобразования для получения необработанных признаков важности.
    + Добавлен batch_size имитировать пояснения при include_local = False для потоковой передачи глобального объяснения в пакеты, чтобы сократить время выполнения DecisionTreeExplainableModel для библиотеки explainability модели.
  + **azureml-core**
    + Добавлена возможность присоединить DBFS хранилища данных в интерфейсе командной строки машинного обучения Azure.
    + Исправлена проблема с передачей в хранилище данных, в которой создается пустой папке, если `target_path` к работе с `/`.
    + Включена сравнение двух наборов данных.
    + Модели и Image delete теперь предоставляет дополнительные сведения о получении вышестоящих объектов, зависящих от них, если delete произошел сбой из-за вышестоящего зависимость.
    + Рекомендуется использовать параметр неиспользуемые RunConfiguration в auto_prepare_environment.
  + **azureml-mlflow**
    + Более эффективное использование ресурсов из удаленных запусков, использующих azureml.mlflow.
    + Улучшена Документация пакета mlflow машинного обучения Azure.
    + Исправлена проблема, в котором mlflow.log_artifacts("my_dir") бы сохранить артефакты в разделе «my_dir/артефакта пути» вместо «артефакта пути».
  + **azureml-dataprep**
    + Объекты потока данных может теперь быть выполнена итерация создания последовательности записей.
    + Исправлена проблема, где `Dataflow.read_pandas_dataframe` бы ошибкой, когда `in_memory` аргумент имеет значение True.
    + Улучшенное взаимодействие с pandas кадров данных с помощью индексов столбцов без строк.
    + Предоставляемые `set_diagnostics_collection()` для программный Включение и отключение сбора данных телеметрии.
    + Добавлен набор значений и bottomValues приводятся сводные данные.
  + **azureml-pipeline-core**
    + Параметр hash_paths для всех шагов конвейера является устаревшим и будет удален в будущем. По умолчанию содержимое исходный_каталог хэшируется (за исключением файлов, перечисленных в .amlignore или gitignore-файл)
    + Продолжение улучшение модуль и ModuleStep для поддержки определенных модулей типа вычислений, в процессе подготовки к интеграции RunConfiguration и дальнейшие изменения для разблокировки их использования в конвейерах.
  + **azureml-pipeline-steps**
    + AzureBatchStep: Улучшенная документация отношении входных и выходных данных.
    + AzureBatchStep: Значение по умолчанию delete_batch_job_after_finish изменилось на true.
  + **azureml-train-core**
    + Как целевой объект вычислений для автоматической настройки Гиперпараметров теперь допустимы строки.
    + Рекомендуется использовать параметр неиспользуемые RunConfiguration в auto_prepare_environment.
    + Нерекомендуемые параметры `conda_dependencies_file_path` и `pip_requirements_file_path` для `conda_dependencies_file` и `pip_requirements_file` соответственно.
  + **azureml-opendatasets**
    + Улучшить NoaaIsdWeather значительно улучшение производительности в версии не SPARK.
    
## <a name="2019-07-01"></a>2019-07-01

### <a name="azure-machine-learning-data-prep-sdk-v117"></a>Пакет SDK v1.1.7 подготовки данных машинного обучения Azure

Мы отменены изменения, улучшенная производительность, как она вызывала проблемы для некоторых клиентов, с помощью Azure Databricks. Если возникли проблемы в Azure Databricks, можно обновить до версии 1.1.7, с помощью одного из описанных ниже методов:
1. Выполните следующий сценарий для обновления: `%sh /home/ubuntu/databricks/python/bin/pip install azureml-dataprep==1.1.7`
2. Повторного создания кластера, которая установит последнюю версию пакета SDK для подготовки данных.

## <a name="2019-06-25"></a>2019-06-25

### <a name="azure-machine-learning-sdk-for-python-v1045"></a>Машинного обучения Azure SDK для Python v1.0.45

+ **Новые функции**
  + Добавление модели суррогат дерева принятия решений, чтобы имитировать пояснения в пакете объяснить модели машинного обучения Azure
  + Возможность указать версию CUDA должны быть установлены на образы выводов. Поддержка CUDA 9.0, 10.0 и 9.1.
  + Сведения о Azure ML обучения базовые образы теперь доступны со [репозитория GitHub Azure ML контейнеры](https://github.com/Azure/AzureML-Containers) и [DockerHub](https://hub.docker.com/_/microsoft-azureml)
  + CLI добавлена поддержка расписания конвейера. Выполните «az ml конвейера -h» для получения дополнительных сведений
  + Добавить пользовательский параметр пространства имен Kubernetes конфигурации развертывания веб-службы AKS и интерфейса командной строки.
  + Параметр hash_paths не рекомендуется для всех шагов конвейера
  + Model.Register теперь поддерживает регистрации несколько отдельных файлов в качестве одной модели с использованием `child_paths` параметра.
  
+ **Функции предварительной версии**
    + Оценки explainers теперь можно при необходимости сохранить conda и pip сведения для более надежного сериализации и десериализации.
    + Исправление ошибки для селектора компонентов автоматически.
    + Обновленные mlflow.azureml.build_image на новый API, устанавливает исправления ошибок, предоставляемых в новой реализации.

+ **Критические изменения**

+ **Исправления ошибок и улучшения**
  + Удален paramiko зависимость от core машинного обучения Azure. Предупреждения об устаревании добавлены для целевого объекта вычислений устаревших присоединении методов.
  + Повысить производительность run.create_children
  + В соответствуют пояснения с двоичный классификатор устранить порядок вероятности при использовании для масштабирования фигуры значения вероятности преподавателя
  + Улучшенная обработка ошибок и сообщений для автоматических машинного обучения. 
  + Устранена проблема времени ожидания итерации для автоматических машинного обучения.
  + Повышена производительность преобразования временных рядов для автоматических машинного обучения.

## <a name="2019-06-24"></a>2019-06-24

### <a name="azure-machine-learning-data-prep-sdk-v116"></a>Пакет SDK v1.1.6 подготовки данных машинного обучения Azure

+ **Новые функции**
  + Добавлены функции суммирования для максимальных значений (`SummaryFunction.TOPVALUES`) и значений (`SummaryFunction.BOTTOMVALUES`).

+ **Исправления ошибок и улучшения**
  + Значительно повышена производительность при `read_pandas_dataframe`.
  + Исправлена ошибка, которые могли бы вызвать `get_profile()` на потоке данных, указывающий на двоичные файлы, переход на другой.
  + Предоставляемые `set_diagnostics_collection()` для программный Включение и отключение сбора данных телеметрии.
  + Изменена работа `get_profile()`. Значения NaN теперь игнорируются минимальное, среднее, Std и функции Sum, которая соответствует поведение Pandas.
...
### <a name="azure-machine-learning-data-prep-sdk-v114"></a>Пакет SDK v1.1.4 подготовки данных машинного обучения Azure
