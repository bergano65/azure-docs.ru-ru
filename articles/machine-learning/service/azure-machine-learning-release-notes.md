...
+ **Изменения**
  ...
  + `read_pandas_dataframe()` Теперь требует `temp_folder` указывать.
...
+ **новые функции;**
  + `to_bool` функция теперь позволяет несовпадающие значения для преобразования в значения ошибки. Это новое поведение при несоответствии по умолчанию для `to_bool` и `set_column_types`. Предыдущим поведением по умолчанию было преобразование несовпадающих значений в False.
...
+ **Критические изменения** 
  * `SummaryFunction.N` был переименован в `SummaryFunction.Count`.
...
+ Целевой объект вычислений Машинного обучения Azure
  ...
  + [Дополнительные сведения об этом вычислений](how-to-set-up-training-targets.md#amlcompute)
...
+ **Новые возможности**
   ...
   * Можно получить все столбцы типов данных из потока данных или профиля данных путем вызова `.dtypes`
   * Можно получить число строк из потока данных или профиля данных путем вызова `.row_count`
