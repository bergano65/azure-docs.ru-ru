---
title: Руководство по выполнению скрипта Python Hello World
...
ms.locfileid: "90929711"
---
# <a name="tutorial-run-hello-world-python-script-part-2-of-4"></a>Руководство по выполнению скрипта Python Hello World (часть 2 из 4)

В рамках этого учебника вы узнаете, как использовать пакет SDK для Python Машинного обучения Azure, чтобы отправить и запустить скрипт Python Hello World.

Этот учебник является **второй частью учебной серии из четырех частей**, в рамках которой вы ознакомитесь с основами Машинного обучения Azure и выполните задачи машинного обучения на основе заданий в Azure. Этот учебник создан на основе работы, выполненной при изучении [учебника по настройке локального компьютера для Машинного обучения Azure (часть 1)](
tutorial-1st-experiment-sdk-setup-local.md).

Изучив данный учебник, вы научитесь:

> [!div class="checklist"]
> * создавать и запускать скрипт Python Hello World локально;
> * создавать скрипт элемента управления Python для отправки Hello World в Машинное обучение Azure;
> * применять концепции Машинного обучения Azure в сценарии элемента управления;
> * отправлять и выполнять Hello World;
> * просматривать выходные данные кода в облаке.

## <a name="prerequisites"></a>Prerequisites

- Выполните инструкции из [первой части учебника по настройке локального компьютера](tutorial-1st-experiment-sdk-setup-local.md), если у вас еще нет рабочей области Машинного обучения Azure.
...
Создайте новый подкаталог с именем `src` в каталоге `tutorial` для хранения кода, который необходимо запустить в вычислительном кластере Машинного обучения Azure. В подкаталоге `src` создайте скрипт Python `hello.py`:
...
Вы можете запустить свой код локально (с преимуществом интерактивной отладки кода), используя предпочтительную интегрированную среду разработки или терминал:
...
*Скрипт элемента управления* позволяет выполнять скрипт `hello.py` в облаке.  Скрипт элемента управления позволяет контролировать, как и где выполняется код машинного обучения.  
...
:::row:::
   ...
   :::column-end:::
   :::column span="2":::
       Отправляет скрипт. Эта отправка называется [запуском](https://docs.microsoft.com/python/api/azureml-core/azureml.core.run(class)?view=azure-ml-py&preserve-view=true).  Запуск инкапсулирует одно выполнение кода. Используйте запуск, чтобы отслеживать ход выполнения скрипта, записывать выходные данные, анализировать результаты, визуализировать метрики и многое другое.
   :::column-end:::
:::row-end:::
:::row:::
   :::column span="":::
...
## <a name="monitor-your-code-in-the-cloud-using-studio"></a>Мониторинг кода в облаке с помощью студии

Выходные данные будут содержать ссылку на Студию машинного обучения Azure, которая выглядит примерно так: `https://ml.azure.com/experiments/hello-world/runs/<run-id>?wsid=/subscriptions/<subscription-id>/resourcegroups/<resource-group>/workspaces/<workspace-name>`.

Щелкните ссылку и перейдите на вкладку **Выходные данные и журналы**. Там можно увидеть файл `70_driver_log.txt`:
