...
Автоматический ML предоставляет пользователям собственные модели временного и глубокого обучения в рамках системы рекомендаций. Эти сведения включают:
+ профет
+ Auto-ARIMA
+ форекастткн
...
При наличии больших данных модели глубокого обучения, такие как Форекастткн Майкрософт, могут улучшить показатели результирующей модели. 

Собственные знания временных рядов также предоставляются в составе автоматизированного ML. Профет работает наилучшим образом с временными рядами, которые обладают сильными сезонными эффектами и несколькими временами исторических данных. Профет является точным & быстрым, надежным для выбросов, отсутствующими данными и значительными изменениями в ряде временных рядов. 

Интегрированное скользящее среднее с авторегрессией (ARIMA) является популярным статистическим методом для прогнозирования временных рядов. Этот метод прогнозирования обычно используется в краткосрочных сценариях прогнозирования, где данные показывают доказательства таких тенденций, как циклы, которые могут быть непредсказуемыми и сложными для моделирования или прогнозирования. Функция Auto-ARIMA преобразует данные в нестационарные данные для получения согласованных и надежных результатов.
...
Этот набор данных представляет собой простой пример ежедневных данных о продажах для компании с двумя разными магазинами: A и B. Кроме того, существует функция для `week_of_year`, которая позволит модели обнаруживать еженедельные сезонности. Поле `day_datetime` представляет собой чистый временной ряд с ежедневной частотой, а поле `sales_quantity` является целевым столбцом для выполнения прогнозов. Прочтите данные в таблицу данных Pandas, а затем используйте функцию `to_datetime`, чтобы убедиться, что временной ряд имеет тип `datetime`.
...
| Параметр | Description | Обязательно для заполнения |
|-------|-------|-------|
...
Создайте параметры временных рядов в качестве объекта Dictionary. Установите `time_column_name` в поле `day_datetime` в наборе данных. Определите параметр `grain_column_names`, чтобы обеспечить создание **двух отдельных групп временных рядов** для данных. один для хранения A и б. Наконец, задайте для `max_horizon` значение 50, чтобы прогнозировать весь набор тестов. Задайте для окна "прогноз" 10 периодов с `target_rolling_window_size`и укажите одну задержку для целевых значений в две точки вперед с помощью параметра `target_lags`. Рекомендуется задать для `max_horizon`, `target_rolling_window_size` и `target_lags` значение "Auto", которое автоматически определит эти значения. В приведенном ниже примере для этих параметров использовались параметры Auto. 
...
> [!NOTE]
> Поддержка DNN для прогнозирования в автоматизированных Машинное обучение доступна в предварительной версии.

Чтобы использовать DNN для прогнозирования, необходимо присвоить параметру `enable_dnn` в Аутомлконфиг значение true. 

Чтобы использовать DNN, мы рекомендуем использовать в качестве целевого объекта вычислений кластер AML с номерами SKU GPU и по крайней мере два узла. Дополнительные сведения см. в [документации по вычислений AML](how-to-set-up-training-targets.md#amlcompute). Дополнительные сведения о размерах виртуальных машин, которые включают GPU, см. в статье [размеры виртуальной машины, оптимизированные для GPU](https://docs.microsoft.com/azure/virtual-machines/linux/sizes-gpu) .

Чтобы обеспечить достаточное время для завершения обучения DNN, рекомендуется установить время ожидания эксперимента не менее чем в несколько часов.
...
Кроме того, можно использовать функцию `forecast()` вместо `predict()`, которая позволит указать, когда должны начинаться прогнозы. В следующем примере сначала заменяются все значения в `y_pred` `NaN`. Источник прогноза будет находиться в конце обучающих данных в этом случае, как обычно при использовании `predict()`. Однако если заменить только вторую половину `y_pred` с `NaN`, функция будет оставлять числовые значения в первой половине неизменного, но прогнозировать значения `NaN` во второй половине. Функция возвращает прогнозируемые значения и выданные функции.
...
Вычислите корень СРЕДНЕКВАДРАТИЧНОЙ ПОГРЕШНОСТИ (среднее значение ошибки в корне) между `actual_labels` фактическими значениями и прогнозируемыми значениями в `predict_labels`.
...
> [!NOTE]
> Невозможно прогнозировать значения для количества периодов, превышающих `max_horizon`. Модель должна быть повторно обучена с большим горизонтом для прогнозирования будущих значений за пределами текущего горизонта.
