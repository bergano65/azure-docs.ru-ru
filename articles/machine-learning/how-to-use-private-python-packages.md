...
### <a name="consume-a-repository-of-packages-from-azure-devops-feed"></a>Использование репозитория пакетов из веб-канала Azure DevOps
...
### <a name="consume-a-repository-of-packages-from-private-storage"></a>Использование репозитория пакетов из частного хранилища

Вы можете использовать пакеты из учетной записи хранения Azure в брандмауэре вашей организации. Такая учетная запись хранения может содержать проверенный набор пакетов для корпоративного использования или внутреннее зеркало общедоступных пакетов.
...
Чтобы настроить такое частное хранилище, сделайте следующее:
 ...
 2. Создайте учетную запись хранения и [запретите общий доступ](https://docs.microsoft.com/azure/storage/common/storage-network-security).
 2. Размещение пакетов Python, которые вы хотите использовать, в контейнер в учетной записи хранения 
 3. [Разрешение доступа к учетной записи хранения из виртуальной сети рабочей области](https://docs.microsoft.com/azure/storage/common/storage-network-security#grant-access-from-a-virtual-network) 

Затем можно сослаться на пакеты в определении среды Машинное обучение Azure по полному URL-адресу в хранилище больших двоичных объектов Azure.
