...
С помощью поля можно `Run.parent` получить доступ к запуску, который запустил текущий дочерний запуск. Распространенный вариант использования такой ситуации заключается в том, что вы хотите объединить результаты журнала в одном месте. Обратите внимание, что дочерние операции запускаются асинхронно, и не гарантируется порядок или синхронизация за пределами того, что родитель должен дождаться завершения его дочерних запусков.
