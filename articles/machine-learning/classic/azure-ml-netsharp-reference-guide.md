...
+ Все модули нейронной сети в Студия машинного обучения Microsoft Azure (классическая модель): [многоклассовая нейронная сеть](/azure/machine-learning/studio-module-reference/multiclass-neural-network), [Многоклассовая нейронная](/azure/machine-learning/studio-module-reference/two-class-neural-network)сеть и [регрессия нейронных сетей](/azure/machine-learning/studio-module-reference/neural-network-regression)
+ Функции нейронной сети в Microsoft ML Server: [NeuralNet](/machine-learning-server/r-reference/microsoftml/neuralnet) и [Ркснеуралнет](/machine-learning-server/r-reference/microsoftml/rxneuralnet)для языка R и [rx_neural_network](/machine-learning-server/python-reference/microsoftml/rx-neural-network) для Python.
...
Архитектура моделей нейронной сети, создаваемых в Машинное обучение Azure Studio (классическая модель), может быть расширена с помощью net #. Вы можете выполнить следующие действия:
...
+ **UpperPad** и **LowerPad** (необязательно): позволяют более тонко управлять объемом заполнения. **Важно.** Эти атрибуты можно определить только в том случае, если значение свойства **Padding** выше **_не_*определено. Значения должны быть кортежами с целочисленными значениями с длиной, которая является арностью пакета. При указании этих атрибутов добавляются «фиктивные» узлы в нижние и верхние концы каждого измерения входного слоя. Число узлов, добавляемых к нижним и верхним концам в каждом измерении, определяется символами _* LowerPad**[i] и **UpperPad**[i] соответственно.
...
+ Если ядро содержит нейроны в той же карте, что и **_x_*_, схема нормализации называется _* той же нормализацией карт**. Чтобы определить нормализацию в той же карте, первая координата в **InputShape** должна иметь значение 1.

+ Если ядро содержит нейроны в той же пространственной позицией, что и **_x_*_, но нейроны находятся в других картах, схема нормализации называется _* между картами нормализация**. Этот тип нормализации ответа реализует форму латерального торможения, похожую на форму, найденную у реальных нейронов, создавая конкуренцию за большие уровни активации среди выходов нейронов, вычисленных на различных картах. Чтобы задать межкарточную нормализацию, первая координата должна быть целым числом (больше единицы и не больше количества карт), а остальные координаты должны иметь значения, равные 1.
