...
description: Эффективно настраивайте гиперпараметры для модели глубокого обучения /машинного обучения с помощью Azure Machine Learning. Вы узнаете, как определить пространство поиска параметров, указать первичную метрику для оптимизации и досрочно прекратить плохо выполняемые запуски.
ms.author: swatig
author: swatig007
ms.reviewer: sgilley
...
ms.locfileid: "80398267"
---
# <a name="tune-hyperparameters-for-your-model-with-azure-machine-learning"></a>Настройка гиперпараметров для модели с помощью машинного обучения Azure
[!INCLUDE [applies-to-skus](../../includes/aml-applies-to-basic-enterprise-sku.md)]

Эффективно настраивайте гиперпараметры для модели с помощью Машинного обучения Azure.  Настройка гиперпараметров включает следующие шаги:
...
Каждый гиперпараметр может быть дискретным или непрерывным и имеет распределение значений, описанных [выражением параметра.](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.hyperdrive.parameter_expressions?view=azure-ml-py)
...
Вы также можете указать метод выборки параметров для определения пространства гиперпараметров. Azure Machine Learning поддерживает случайную выборку, выборку сетки и байесовскую выборку.

#### <a name="picking-a-sampling-method"></a>Выбор метода выборки

* Выборка сетки может быть использована, если ваше пространство гиперпараметра может быть определено как выбор среди дискретных значений и если у вас есть достаточный бюджет для исчерпывающего поиска по всем значениям в определенном пространстве поиска. Кроме того, можно использовать автоматизированное досрочное прекращение плохо выполняемых трасс, что уменьшает потери ресурсов.
* Случайная выборка позволяет гиперпараметру включать как дискретные, так и непрерывные гиперпараметры. На практике он дает хорошие результаты в большинстве случаев, а также позволяет использовать автоматизированное досрочное прекращение плохо выполняющих работает. Некоторые пользователи выполняют первоначальный поиск с помощью случайной выборки, а затем итеративно уточнить пространство поиска для улучшения результатов.
* Байесовская выборка использует знания предыдущих образцов при выборе значений гиперпарамов, эффективно пытаясь улучшить заявленную первичную метрику. Байесовская выборка рекомендуется при наличии достаточного бюджета для изучения пространства гиперпарамсов - для достижения наилучших результатов с Байесовской выборкой мы рекомендуем использовать максимальное количество трасс, превышающего или равное 20 раз большем количеству настроенных гиперпараметров. Обратите внимание, что байесовская выборка в настоящее время не поддерживает какую-либо политику досрочного прекращения.

#### <a name="random-sampling"></a>Случайная выборка

При случайной выборке значения гиперпараметров выбираются случайным образом из определенного пространства поиска. [Случайная выборка](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.hyperdrive.randomparametersampling?view=azure-ml-py) позволяет место поиска включать как дискретные, так и непрерывные гиперпараметры.
...
[Выборка сетки](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.hyperdrive.gridparametersampling?view=azure-ml-py) выполняет простой поиск сетки по всем возможным значениям в определенном пространстве поиска. Ее можно использовать только с гиперпараметрами, указанными с помощью `choice`. Например, следующее пространство имеет в общей сложности шесть образцов:
...
[Байесовская выборка](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.hyperdrive.bayesianparametersampling?view=azure-ml-py) основана на алгоритме оптимизации Байеса и делает разумный выбор на значениях гиперпарамов для следующего образца. Образец выбирается на основе результатов для предыдущих образцов так, чтобы новый образец улучшал основную оцениваемую метрику.
...
Байесовская `choice`выборка поддерживает только, `uniform`и `quniform` распределения по пространству поиска.
...
Машинное обучение Azure поддерживает следующие политики раннего прекращения.

### <a name="bandit-policy"></a>Политика Bandit

[Бандит](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.hyperdrive.banditpolicy?view=azure-ml-py#definition) – это политика прекращения, основанная на вялым факторе/вялой сумме и интервале оценки. Эта политика досрочно завершает все запуски, где основная метрика не находится в пределах указанного коэффициента или количества резерва времени в отношении учебного запуска с самой высокой эффективностью. Эта политика принимает следующие параметры конфигурации:
...
[Медианная остановка](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.hyperdrive.medianstoppingpolicy?view=azure-ml-py) — это политика досрочного прекращения, основанная на запущенных средних показателях первичных метрик, о которых сообщается в ходе выполнения. Эта политика вычисляет средние значения среди всех обучающих прогонов и завершает прогоны, эффективность которых хуже медианы средних показателей. Она принимает следующие параметры конфигурации:
* `evaluation_interval`: частота применения политики (необязательный параметр).
* `delay_evaluation`: задерживает первую оценку политики для определенного количества интервалов (необязательный параметр).
...
[Выбор Трунца](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.hyperdrive.truncationselectionpolicy?view=azure-ml-py) отменяет определенный процент самых низких выполнения запусков в каждом интервале оценки. Прогоны сравниваются в зависимости от их эффективности по основной метрике, и X % прогонов с наименьшей эффектностью завершаются. Эта политика принимает следующие параметры конфигурации:
...
В этом примере политика раннего завершения применяется в каждом интервале, начиная с оценочного интервала 5. Запуск будет прекращен с интервалом 5, если его производительность с интервалом 5 находится в самом низком 20% производительности всех запусков с интервалом 5.
...
Если политика не указана, служба настройки гиперпараметра позволит выполнить все обучающие заезды до завершения.

### <a name="picking-an-early-termination-policy"></a>Выбор политики досрочного прекращения
...
* Если вы ищете более агрессивную экономию от досрочного прекращения, вы можете использовать политику бандитов с более строгой (меньшей) допустимой вялой или политикой выбора с большим процентом усечения.
...
Как только вы определите конфигурацию настройки гиперпараметра, [отправьте эксперимент:](https://docs.microsoft.com/python/api/azureml-core/azureml.core.experiment%28class%29?view=azure-ml-py#submit-config--tags-none----kwargs-)
...
`experiment_name`это имя, которое вы присваивают `workspace` вашему эксперименту по настройке гиперпараметра, и это рабочее пространство, в котором вы хотите создать эксперимент (Для получения дополнительной информации об экспериментах, см. [Как работает Azure Machine Learning?)](concept-azure-machine-learning-architecture.md)

## <a name="warm-start-your-hyperparameter-tuning-experiment-optional"></a>Теплое начало вашего гиперпарамтаметра тюнинг эксперимент (по желанию)

Часто поиск наилучших значений гиперпарамса для модели может быть итеративным процессом, в ходе которого требуется несколько трасс настройки, которые учатся на предыдущих трассах настройки гиперпарамта. Повторное использование знаний из этих предыдущих запусков ускорит процесс настройки гиперпарамта, тем самым снизив затраты на настройку модели и потенциально улучшит первичную метрику полученной модели. При теплом запуске гиперпараметрического тюнинг-эксперимента с байесовской пробой, испытания из предыдущего запуска будут использоваться в качестве предварительных знаний, чтобы разумно выбрать новые образцы, для улучшения первичной метрики. Кроме того, при использовании выборки Random или Grid любые решения о досрочном прекращении будут использовать метрики из предыдущих запусков для определения плохо выполняемых тренировочных запусков. 

Azure Machine Learning позволяет тепло начать процесс настройки гиперпараметра, используя знания до 5 ранее завершенных / отмененных гиперпарамстных запусков родителей. Можно указать список родительских трасс, которые вы хотите согреть, начиная с использования этого фрагмента:
...
Кроме того, могут быть случаи, когда индивидуальные тренировочные запуски эксперимента по настройке гиперпарамстрима отменяются из-за бюджетных ограничений или терпят неудачу по другим причинам. Теперь можно возобновить такие индивидуальные тренировочные запуски с последнего контрольно-пропускного пункта (при условии, что ваш учебный сценарий обрабатывает контрольно-пропускные пункты). Возобновление индивидуального тренировочного запуска будет использовать ту же конфигурацию гиперпараметра и монтировать папку выходов, используемую для этого запуска. Сценарий обучения `resume-from` должен принять аргумент, содержащий контрольно-пропускной пункт или файлы модели, из которых можно возобновить учебный запуск. Вы можете возобновить индивидуальные тренировочные забеги, используя следующий фрагмент:
...
Вы можете настроить свой гиперпараметр настройки эксперимента для теплого начала с предыдущего `resume_from` `resume_child_runs` эксперимента или возобновить индивидуальные учебные запуски с использованием дополнительных параметров и в конфигурации:
...
SDK Azure Machine Learning предоставляет [виджет ноутбука,](https://docs.microsoft.com/python/api/azureml-widgets/azureml.widgets.rundetails?view=azure-ml-py) который визуализирует ход выполнения учебных программ. Следующий фрагмент кода визуализирует все запуски по настройке гиперпараметров в одном отображении, создаваемом в записной книжке Jupyter:
...
После завершения всех трасс настройки гиперпарамта [определите наиболее эффективную конфигурацию](/python/api/azureml-train-core/azureml.train.hyperdrive.hyperdriverun?view=azure-ml-py#get-best-run-by-primary-metric-include-failed-true--include-canceled-true--include-resume-from-runs-true-----typing-union-azureml-core-run-run--nonetype-) и соответствующие значения гиперпарамов:
...
В этой папке обратитесь к блокнотам для поездов-гиперпарамстепов:
