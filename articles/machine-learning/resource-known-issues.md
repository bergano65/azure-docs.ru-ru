...
* **Установка PIP: депендеЦиес не гарантируется в соответствии с однострочной установкой**: это известное ограничение PIP, так как в нем отсутствует действующий сопоставитель зависимостей при установке в виде одной строки. Первая уникальная зависимость — это единственная из них. Например, если вы устанавливаете Azure-ML-перечеркивание, требующее версии > 1,0 и azureml-Training-аутомл, для которой требуется версия < 1,2, а последняя версия — 1,3, когда пользователь устанавливает пакеты в одну строку, как показано ниже, все обновляется до 1,3, несмотря на то, что для пакета azureml-Learning-аутомл требуется более старая версия. 

    * Вы увидите непоследовательные депендеЦиес с однострочной установкой.
    ```python
       pip install azure-ml-datadrift, azureml-train-automl
     ```
   
    * Чтобы обеспечить установку соответствующих версий для пакетов, установите с помощью нескольких строк, как в следующем коде. Порядок не имеет значения.
