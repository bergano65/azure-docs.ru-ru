...
1. Перейдите на вкладку Фильтры и прокрутите вниз до пункта дополнительные фильтры. В **разделе** и **значении** укажите типы свойств, по которым требуется выполнить фильтрацию. Здесь можно увидеть, что событие будет запущено только тогда, когда тип запуска является запуском конвейера или этапом конвейера.  
