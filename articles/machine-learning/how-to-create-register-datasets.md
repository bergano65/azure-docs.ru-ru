...
Если вы не можете получить достаточно большое виртуальное место для данных, у вас есть два варианта: использовать такую платформу, как Spark или Даск, для обработки данных "недостаточно памяти", т. е. Таблица данных загружается в раздел ОЗУ по секциям и обрабатываются с окончательным результатом собрано в конце. Если это слишком большое значение, Spark или Даск позволяет масштабировать кластер, который по-прежнему можно использовать в интерактивном режиме. 
...
2|3|Истина|3|Хеиккинен, промах. лаина|Женский|26,0|0|0|СТОН/O2. 3101282|7,9250||S
...
Используйте метод [`from_files()`](https://docs.microsoft.com/python/api/azureml-core/azureml.data.dataset_factory.filedatasetfactory?view=azure-ml-py#from-files-path--validate-true-) класса `FileDatasetFactory` для загрузки файлов в любом формате и создания незарегистрированного филедатасет. Если хранилище находится за виртуальной сетью или брандмауэром, задайте параметр `validate =False` в методе `from_files()`. Это обходит этап начальной проверки и гарантирует, что вы сможете создать набор данных на основе этих защищенных файлов.
