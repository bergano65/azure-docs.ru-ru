...
Машинное обучение Azure [вычислительным экземпляром](concept-compute-instance.md) является защищенная облачная Рабочая станция Azure, которая предоставляет специалистам по обработке и анализу данных Jupyter портативный компьютер, JupyterLab и полностью подготовленную среду машинного обучения.
...
Чтобы приостанавливаться к выплатам за вычислительные ресурсы, необходимо [Отключить вычислительный экземпляр](tutorial-1st-experiment-bring-data.md#clean-up-resources).
...
Если вы используете локальный компьютер (который может также быть удаленной виртуальной машиной), создайте среду Anaconda и установите пакет SDK. Пример:
...
1. Выполните следующие команды в новой среде, чтобы включить ядра Python, относящиеся к конкретной среде. Это обеспечит ожидаемое поведение при импорте ядра и пакета при работе с записными книжками Jupyter в средах Anaconda:
...
1. Клонирование [репозитория GitHub](https://github.com/Azure/MachineLearningNotebooks) для набора примеров записных книжек.
