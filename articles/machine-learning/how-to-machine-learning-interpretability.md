...
Доступ к классам интерпретации предоставляется через следующий пакет SDK: (сведения об [установке пакетов SDK для машинное обучение Azure](/python/api/overview/azure/ml/install?preserve-view=true&view=azure-ml-py))
