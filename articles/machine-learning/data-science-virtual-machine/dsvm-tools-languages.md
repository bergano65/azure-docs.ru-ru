...
| Настройка и установка на DSVM  | Создаются две `conda` глобальные среды: <br /> * Среда `root` , расположенная `/anaconda/` в, находится в Python 3,7. <br/> * Среда `python2` , расположенная `/anaconda/envs/python2` в, находится в Python 2,7.       |
...
**Java**: OpenJDK доступен как в выпусках Linux, так и Windows DSVM и задается в пути. Чтобы использовать Java, введите команду `javac` или `java` в командной строке в Windows или в оболочке bash в Linux.
