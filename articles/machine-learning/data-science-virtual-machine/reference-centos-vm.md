---
title: 'Справка: CentOS Data Science Виртуальная машина'
titleSuffix: Azure Data Science Virtual Machine
description: Подробная информация об инструментах, включенных в CentOS Data Science Виртуальная машина
author: gvashishtha
...
ms.locfileid: "80475481"
---
# <a name="reference-centos-linux-data-science-virtual-machine"></a>Справка: CentOS (Linux) Наука о данных Виртуальная машина
...
Плата за программное обеспечение, включенное в образ DSVM, не взимается. Вы платите только за оборудование Azure в зависимости от размера виртуальной машины, которая подготавливается с образом DSVM. Более подробную информацию о тарифах на вычисления можно узнать в списке Azure Marketplace с использованием виртуальной [машины Data Science для Linux.](https://azure.microsoft.com/marketplace/apps/microsoft-dsvm.linux-data-science-vm-ubuntu)
...
Чтобы открыть консоль R, в оболочке введите **R**. Эта команда перенесет вас в интерактивную среду. Для разработки программы R обычно используется текстовый редактор, например Emacs или vi. Затем скрипты запускаются в R. RStudio предоставляет полноценную графическую интегрированную среду для разработки программ R.
...
Anaconda Python устанавливается вместе со средами Python версий 3.5 и 2.7. Среда 2.7 называется _корнем,_ а среда 3.5 — _py35._ Этот дистрибутив содержит базовую версию Python, а также примерно 300 наиболее популярных математических и инженерных пакетов и пакетов аналитики данных.
...
Сервер записной книжки Jupyter доступен с любого узла. Введите **\//\<https: Имя DNS DSVM или IP-адрес:8000/\>**.
...
* LaTeX устанавливается через пакет texlive, вместе с пакетом дополнений Emacs под названием [AUCTeX.](https://www.gnu.org/software/auctex/manual/auctex/auctex.html) Этот пакет упрощает создание документов LaTeX в Emacs. 
...
* **Библиотеки Azure**: На DSVM предустановлены следующие библиотеки:
  ...
  * **Java**: Список библиотек Azure Java можно найти в каталоге /dsvm/sdk/AzureSDKJava на DSVM. Основные библиотеки — это API-интерфейсы для службы хранилища Azure и API-интерфейсы для управления Azure, драйверы Azure Cosmos DB и драйверы JDBC для SQL Server.  
...
Машинное обучение Azure — полностью управляемая облачная служба, с помощью которой вы сможете создавать, развертывать и совместно использовать решения прогнозной аналитики. Вы строите свои эксперименты и модели из студии машинного обучения Azure (классика). Чтобы получить доступ к машинному обучению Azure из веб-браузера dSVM, перейдите на microsoft [Azure Machine Learning.](https://studio.azureml.net)

После вступления в программу Azure Machine Learning Studio (классическая) можно использовать холст экспериментов для создания логического потока для алгоритмов машинного обучения. Также предоставляется доступ к приложению Jupyter Notebook, которое размещено в Машинном обучении Azure. Ноутбук может беспрепятственно работать с экспериментами в Azure Machine Learning Studio (классический). 
...
В DSVM включены некоторые инструменты и алгоритмы машинного обучения, которые были предварительно скомпилированы и установлены локально. Сюда входит следующее.

* **Microsoft Cognitive Toolkit**: Набор инструментов глубокого обучения.
* **Vowpal Wabbit**— алгоритм быстрого онлайн-обучения.
* **XGBoost**: Инструмент, который обеспечивает оптимизированные, увеличенные алгоритмы дерева.
...
* **R**: Богатая библиотека функций машинного обучения доступна для R. Предустановленные библиотеки включают lm, glm, randomForest и rpart. С помощью команды `install.packages(<lib name>)` можно установить другие библиотеки.
...
1. Откройте вкладку **Log** (Журнал), чтобы показать код R, созданный для предыдущих операций. (Из-за ошибки в текущем выпуске Rattle, вы должны вставить **#** символ перед Экспорт **омичи этого журнала** в тексте журнала.)
