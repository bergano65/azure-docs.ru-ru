...
В Dsvm существует несколько других библиотек машинного обучения, таких как популярный пакет `scikit-learn`, который входит в состав дистрибутива Anaconda Python для Dsvm. Чтобы извлечь список пакетов, доступных в Python, R и Julia, запустите соответствующие диспетчеры пакетов.
...
| Распространенные способы применения      | Общая библиотека машинного обучения      |
| Настройка и установка      |  Установлена с поддержкой GPU.   |
| Запуск и использование      | Как библиотека Python (2,7 и 3,5), пакет R и программа командной строки по пути (`C:\dsvm\tools\xgboost\bin\xgboost.exe` для Windows и `/dsvm/tools/xgboost/xgboost` для Linux).    |
