...
1. Введите следующую информацию на каждом шаге работы мастера:
    ...
    1. **Основные сведения**:
       ...
       * **Size.** Этот параметр должен автоматически заполняться размером, подходящим для общих рабочих нагрузок. Дополнительные сведения см. в статье [Размеры виртуальных машин Linux в Azure](../../virtual-machines/linux/sizes.md).
...
>[!NOTE]
> Если на этом этапе появляется сообщение об ошибке 500, скорее всего, в имени пользователя используются прописные буквы. Это известная особенность взаимодействия между концентратором JupyterHub и средством PAMAuthenticator, которое он использует. 
...
* Изучите систематический подход к созданию аналитических решений с помощью [процесса обработки и анализа данных группы](https://aka.ms/tdsp).
