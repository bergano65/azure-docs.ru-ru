...
Ubuntu DSVM работает на базе [JupyterHub](https://github.com/jupyterhub/jupyterhub), многопользовательского сервера Jupyter. Чтобы подключиться, перейдите к https:\/и в вашей-vm-ip:8000 на ноутбуке или рабочего стола, введите имя пользователя и пароль, который использовался для создания виртуальной Машины и выполните вход. Для просмотра и испытания доступно много примеров записных книжек.

Кроме того, доступен JupyterLab, следующее поколение записных книжек Jupyter, и JupyterHub. Для доступа к нему, войдите в JupyterHub, затем перейдите по URL-адрес https:\/и в вашей-vm-ip:8000/пользователя, your-имя пользователя и лабораторных. JupyterLab можно установить как на сервер записных книжек по умолчанию, добавив следующую строку в */etc/jupyterhub/jupyterhub_config.py*:
...
Caffe2 — это платформа глубокого обучения от Facebook на основе Caffe. Она доступна на Python 2.7 корневой среде Conda. Чтобы активировать его, выполните следующую команду из оболочки:
...
TensorFlow — это библиотека глубокого обучения от Google. Он является библиотекой программного обеспечения с открытым исходным кодом для числовых вычислений с применением графиков потоков данных. TensorFlow доступен в среде Python 3.5 (py35), а несколько примеров объектов Notebook доступно в JupyterHub.
...
Чтобы запустить консоль R, просто введите **R** в оболочке. Эта команда выполняет интерактивную среду. Для разработки программы R обычно используется редактор, такой как Emacs или vi, а затем сценарии запускаются в R. RStudio предоставляет полноценную графическую интегрированную среду разработки для программ R.
...
Если вы программируете на R для Spark, можно использовать Microsoft R Server, SparkR или sparklyr. 
...
Вы можете остановить Hadoop связанных служб, когда они не нужны, выполнив ```systemctl stop hadoop-namenode hadoop-datanode hadoop-yarn```

Пример, демонстрирующий, как разрабатывать и тестировать с использованием MRS в удаленном контексте Spark (который является изолированным экземпляром Spark на DSVM), доступен в */dsvm/samples/MRS* каталога.

### <a name="ides-and-editors"></a>Интегрированные среды разработки и редакторы

У вас есть выбор несколько редакторов кода, включая vi/VIM, Emacs, PyCharm, RStudio и IntelliJ. IntelliJ, RStudio и PyCharm — графические редакторы, и для их использования необходимо войти в графический рабочий стол. На рабочем столе есть меню и значки для запуска этих редакторов.
...
Для подключения к различным базам данных (Microsoft SQL Server и MySQL) и для выполнения SQL-запросов предоставлен графический клиент SQL — **SQuirrel SQL**. Позволяет выполнять SQuirrel SQL из сеанс графического рабочего стола (например, с помощью клиента X2Go) с помощью значка на рабочем столе, или при помощи следующую команду в оболочке:
...
Чтобы запустить средство с простым примером, используйте следующие команды:
...
Откроется графический интерфейс с набором вкладок. Ниже приведены шаги краткого руководства в Rattle используется пример набора данных о погоде и построить модель. В некоторых действиях будет предложено автоматически скачать и установить необходимые пакеты R, которые еще не установлены в системе.
