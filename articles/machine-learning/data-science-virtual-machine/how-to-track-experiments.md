...
Файл конфигурации содержит такие сведения, как имя рабочей области, подписка и т. д., и это означает, что вам не нужно жестко кодировать эти параметры.
...
Если щелкнуть запуск, будут отображаться другие сведения, а также выбранная модель в выходных данных и __журналах__ .

## <a name="deploy-model-in-azure-machine-learning"></a>Развертывание модели в Машинное обучение Azure

В этом разделе описано, как развертывать модели, обученные на DSVM, в Машинное обучение Azure.

### <a name="step-1-create-inference-compute"></a>Шаг 1. Создание вычислений вывода

В меню слева в [AzureML Studio](https://ml.azure.com) щелкните " __вычисления__ ", а затем вкладку " __кластеры вывода__ ". Затем щелкните __+ создать__ , как показано ниже:

![Создание вычислений вывода](./media/how-to-track-experiments/mlflow-experiments-6.png)

В __новой области кластера вывода__ введите сведения для:
...
При регистрации модели в коде с помощью `register_model` мы указали платформу как sklearn. Машинное обучение Azure не поддерживает развертывание кода для следующих платформ:
...
Мы выполним развертывание модели в кластере вывода (служба Azure Kubernetes), созданном на шаге 1. Заполните приведенные ниже сведения, указав имя службы и имя кластера вычислений AKS (созданного на шаге 1). Также рекомендуется увеличить __емкость резерва ЦП__ до 1 (от 0,1) и __емкость резерва памяти__ равна 1 (из 0,5). это можно сделать, щелкнув __Дополнительно__ и заполнив сведения. Затем нажмите кнопку __развернуть__.
...
После успешного развертывания модели вы должны увидеть следующее (чтобы перейти на эту страницу, щелкните конечные точки в меню слева, > затем щелкните имя развернутой службы):

![Использование модели](./media/how-to-track-experiments/mlflow-experiments-8.png)

Обратите внимание, что состояние развертывания переходит от __перехода__ к __работоспособному__. Кроме того, в этом разделе содержатся сведения о конечной точке и URL-адресах Swagger, которые разработчик приложения может использовать для интеграции модели машинного обучения в свои приложения.

Вы можете проверить конечную точку с помощью [POST](https://www.postman.com/). Кроме того, можно использовать пакет SDK для AzureML:
