...
* [**X2Go**](https://wiki.x2go.org/doku.php) , установленный на компьютере с открытым сеансом Xfce. Дополнительные сведения см. [в статье Установка и Настройка клиента X2Go](linux-dsvm-intro.md#install-and-configure-the-x2go-client).
...
1. Нажмите кнопку **Выполнить**. В одном окне графа отобразятся обе диаграммы плотности, где ясно, что в сообщениях электронной почты часто встречаются такие же слова, как и для _удаления_.
