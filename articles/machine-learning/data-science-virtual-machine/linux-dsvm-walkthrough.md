...
* Такие столбцы, как **word\_freq\_* Word***, указывают процент слов в сообщении электронной почты, соответствующих *слову*. Например, если **word\_freq\_сделать** **1**, значит, 1% всех слов в электронном письме было *внесено*.
* Такие столбцы, как **char\_freq\_* char***, указывают процент всех символов в письме, которые являются *char*.
...
1. Нажмите кнопку **Execute**(Выполнить). В одном окне графа отобразятся обе диаграммы плотности, где ясно, что в сообщениях электронной почты часто встречаются такие _же слова,_ как и для _удаления_.
...
Вернитесь на вкладку **Cluster (кластер** ). Выберите **кмеанс**и задайте для **параметра число кластеров** значение **4**. Нажмите кнопку **Execute**(Выполнить). Результаты появятся в окне вывода. Один кластер имеет высокую частоту от _Георгия_ до _HP_и, вероятно, является законным бизнес-сообщением.
