...
* Инструменты машинного обучения
  ...
  * [Rattle](https://togaware.com/rattle/) (аналитическое средство для R, упрощающее процесс обучения): этот инструмент облегчает процесс анализа данных и работы со средствами машинного обучения в R, предоставляя графический пользовательский интерфейс для исследования данных и моделирования с автоматическим созданием кода R.
...
* **Учетная запись хранения Azure**. Сведения о создании записи см. в статье [Create a storage account](../../storage/common/storage-quickstart-create-account.md) (Создание учетной записи хранения). Если вы не хотите использовать имеющуюся учетную запись хранения, то можно настроить новую учетную запись при создании виртуальной машины.
...
1. В нижней части страницы нажмите кнопку **Создать**. Откроется мастер.![configure-data-science-vm](./media/linux-dsvm-intro/configure-linux-data-science-virtual-machine.png)
1. В следующих разделах приведены входные данные для всех шагов мастера (номера см. справа на предыдущем рисунке) по созданию виртуальной машины Майкрософт для обработки и анализа данных. Ниже приведено описание входных данных, необходимых для настройки на каждом из шагов.
   ...
   * **Подписка**: Если у вас есть несколько подписок, выберите ту, в которой будет создана виртуальная машина и для которой будут выставляться счета. Вам необходимо иметь права на создание ресурсов для этой подписки.
...
При желании для установки [20 самых популярных пакетов R](https://www.kdnuggets.com/2015/06/top-20-r-packages.html) можно воспользоваться R-скриптом. Этот сценарий можно запустить в интерактивном интерфейсе R, для подключения к которому, как уже упоминалось, необходимо ввести **R** в оболочке.  
...
Чтобы установить дополнительные библиотеки Python, необходимо выполнить команду ```conda``` или ```pip```, используя sudo, и указать полный путь к диспетчеру пакетов Python (Conda или PIP) для установки в правильную среду Python. Пример:
...
Вы можете остановить Hadoop связанных служб, когда они не нужны, выполнив ```systemctl stop hadoop-namenode hadoop-datanode hadoop-yarn``` пример, демонстрирующий, как разрабатывать и тестировать с использованием MRS в удаленном контексте Spark (который является изолированным экземпляром Spark на DSVM), доступен в `/dsvm/samples/MRS` каталог. 
...
**VIM** и **Emacs** — текстовые редакторы. В Emacs мы установили пакет надстройки ESS, который упрощает работу с R в редакторе Emacs. Дополнительные сведения см. в описании пакета [ESS](https://ess.r-project.org/).
...
**sqlcmd**. Это служебная программа, которая позволяет из командной строки выполнять инструкции Transact-SQL, системные процедуры и файлы сценариев. Эта программа использует ODBC для выполнения пакетов Transact-SQL.
...
1. Откроется диалоговое окно с вопросом, хотите ли вы использовать пример погодных данных. Нажмите кнопку **Yes** (Да), чтобы загрузить пример.
1. Перейдите на вкладку **Model** (Модель).
1. Нажмите кнопку **Execute** (Выполнить) для построения дерева принятия решений.
1. Нажмите кнопку **Draw** (Нарисовать) для отображения дерева принятия решений.
1. Щелкните переключатель **Forest** (Лес) и нажмите кнопку **Execute** (Выполнить) для создания случайного леса.
1. Перейдите на вкладку **Evaluate** (Оценка).
1. Щелкните переключатель **Risk** (Риск) и нажмите кнопку **Execute** (Выполнить) для отображения двух диаграмм производительности "Risk (Cumulative)" (Риск (накопительный)).
1. Щелкните вкладку **Log** (Журнал), чтобы показать код R, созданный для предыдущих операций.
...
1. Нажмите кнопку **Export** (Экспортировать), чтобы сохранить файл R-скрипт под именем *weather_script.R* в домашней папке.
...
* Изучите систематический подход к созданию комплексных аналитических решений с помощью [группового процесса обработки и анализа данных](https://docs.microsoft.com/azure/machine-learning/team-data-science-process/).
* Перейдите в [коллекцию Cortana Analytics](https://gallery.cortanaanalytics.com) , чтобы найти примеры машинного обучения и анализа данных с использованием Cortana Analytics Suite.
