...
## <a name="1-explore-data--develop-models-with-microsoft-ml-server"></a>1. Изучите данные, & разрабатывать модели с помощью Microsoft ML Server
...
## <a name="2-use-jupyter-notebooks"></a>2. Использование записных книжек Jupyter
...
## <a name="3-train--deploy-models-with-azure-machine-learning"></a>3. Обучение & развертывание моделей с помощью Машинное обучение Azure
...
## <a name="4-manage-azure-resources"></a>4. Управление ресурсами Azure
...
+ **Портал Azure**: используйте браузер и укажите [портал Azure](https://portal.azure.com)

+ **Azure PowerShell**. Используйте сценарии Windows PowerShell. Запустите Azure PowerShell из ярлыка на рабочем столе или из меню "Пуск", озаглавленного "Microsoft Azure PowerShell". Полные сведения см. в [документации по Microsoft Azure PowerShell](../../powershell-azure-resource-manager.md) . 

## <a name="5-extend-storage-with-shared-file-systems"></a>5. Расширение хранилища с помощью общих файловых систем
Специалисты по обработке и анализу данных могут использовать большие наборы данных, код или другие ресурсы вместе с участниками группы. В самой виртуальной машине DSVM доступно около 45 ГБ свободного места. Чтобы увеличить объем хранилища, можно использовать службу файлов Azure и либо подключить к одному или нескольким экземплярам DSVM, либо получить к ней доступ через REST API.  Вы также можете использовать [портал Azure](../../virtual-machines/windows/attach-managed-disk-portal.md) или [Azure PowerShell](../../virtual-machines/windows/attach-disk-ps.md) для добавления дополнительных выделенных дисков данных. 
...
## <a name="6-share-code-in-github"></a>6. Совместное использование кода в GitHub
...
## <a name="7-access-many-azure-data--analytics-services"></a>7. Доступ к многим службам аналитики & данных Azure
### <a name="azure-blob"></a>BLOB-объект Azure
...
**Azure Data Lake Explorer** можно использовать для [передачи данных из локальных файлов на виртуальной машине в хранилище Data Lake](https://docs.microsoft.com/azure/data-lake-store/data-lake-store-get-started-portal).
...
## <a name="8-power-bi-reports--dashboards"></a>8. Power BI отчеты & панели мониторинга 
Вы можете визуализировать файл JSON О вулканах из предыдущего Cosmos DB примера в Power BI Desktop, чтобы получить наглядную информацию о данных. Подробные указания см. в статье о [Power BI](../../cosmos-db/powerbi-visualize.md). Ниже приведены основные действия.
...
Вы можете приступить к созданию отчетов и визуализации с помощью модели данных. Указания по созданию отчетов см. в этой [статье о Power BI](../../cosmos-db/powerbi-visualize.md#build-the-reports).

## <a name="9-dynamic-dsvm-scaling"></a>9. Динамическое масштабирование DSVM 
