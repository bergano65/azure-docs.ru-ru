...
> [!NOTE]
> Экземпляры вычислений (Предварительная версия) доступны только для рабочих областей с регионом " **северо-центральная часть США**", " **Восток США 2**", " **северная Европа** " или " **Южная часть Соединенного Королевства**" с поддержкой других регионов в ближайшее время.
...
1. В записной книжке отображается URL-адрес и порт, на котором запущена служба. Например, `https://localhost:6789`. Можно также выполнить ячейку, содержащую `print('Local service port: {}'.format(local_service.port))`, чтобы отобразить порт.
