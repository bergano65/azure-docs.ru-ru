...
1. Выберите **создать новое вычисление** , чтобы настроить контекст вычислений для этого эксперимента.
    ...
    Нажмите кнопку **создания**. Создание нового вычислений может занять несколько минут.
...
При настройке экспериментов можно включить дополнительные параметры `feauturization`. 

|Конфигурация Добавление признаков | Description |
| ------------- | ------------- |
|"феаутуризатион" = ' Феатуризатионконфиг '| Указывает, что следует использовать настраиваемый шаг добавление признаков. [Узнайте, как настроить добавление признаков](how-to-configure-auto-train.md#customize-feature-engineering).|
|"феаутуризатион" = "OFF"| Указывает, что шаг добавление признаков не должен выполняться автоматически.|
|"феаутуризатион" = "Auto"| Указывает, что в ходе предварительной обработки следующие шаги снятие данных и добавление признаков выполняются автоматически.|
