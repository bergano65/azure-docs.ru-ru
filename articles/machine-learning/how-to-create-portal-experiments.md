...
1. Используемых Дополнительные настройки: дополнительные параметры, которые можно использовать для лучшего управления обучающим заданием. В противном случае применяются значения по умолчанию, основанные на выборе эксперимента и данных. 
...
При настройке экспериментов можно включить дополнительные параметры `Preprocess`. Это означает, что в рамках предварительной обработки следующих шагов снятие данных и добавление признаков выполняются автоматически.
...
Автоматизированное машинное обучение предлагает снятие данных для выявления потенциальных проблем с данными (например, отсутствующие значения, несбалансированность класса) и помогает предпринять корректирующие действия для улучшения результатов. Существует множество рекомендаций, которые можно применять для достижения надежных результатов. 
...
---|---|---
Отсутствующие значения&nbsp;&nbsp;добавления отсутствующих |**Выполнено** <br> <br> **Фиксированная**|    Отсутствуют отсутствующие значения во входных&nbsp;столбцах <br> <br> В некоторых столбцах отсутствуют значения
Перекрестная проверка|**Договорились**|Если явный набор проверки не указан
Высокое&nbsp;количество элементов&nbsp;обнаружение&nbsp;компонентов|  **Выполнено** <br> <br>**Договорились**|   Не обнаружено компонентов с высоким количеством элементов <br><br> Обнаружены входные столбцы с большим количеством элементов
Определение баланса класса |**Выполнено** <br><br><br>**Оповещения** |Классы сбалансированы в обучающих данных; Набор данных считается сбалансированным, если каждый класс имеет хорошее представление в наборе данных, как измеряется числом и отношением выборок. <br> <br> Несбалансированные классы в обучающих данных
Согласованность данных временных рядов|**Выполнено** <br><br><br><br> **Фиксированная** |<br> Были проанализированы выбранные значения {горизонта, запаздывания, чередующегося окна} и не обнаружены потенциальные проблемы нехватки памяти. <br> <br>Выбранные значения {горизонта, запаздывания, чередующегося окна} были проанализированы и потенциально могут привести к нехватке памяти для эксперимента. Запаздывание или пошаговое окно отключено.
