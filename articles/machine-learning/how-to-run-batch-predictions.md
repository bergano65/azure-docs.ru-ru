...
### <a name="how-to-access-other-files-in-init-or-run-functions"></a>Доступ к другим файлам в функциях `init()` или `run()`
...
- `run_invocation_timeout`: Время вызова метода `run()` в секундах. Значение по умолчанию — `60`.
...
См. инструкции по [отладке и устранению неполадок с ParallelRunStep](how-to-debug-batch-predictions.md).
