...
Azure Container Instance не поддерживает шифрование диска. Если вам нужно шифрование диска, мы рекомендуем вместо этого развернуть в [экземпляре службы Azure Kubernetes.](how-to-deploy-azure-kubernetes-service.md) В этом случае можно также использовать поддержку Azure Machine Learning для элементов управления доступом на основе ролей для предотвращения развертывания в azure Container Instance в подписке.
