...
[![Проверка подлинности в Машинное обучение Azure](media/concept-enterprise-security/authentication.png)](media/concept-enterprise-security/authentication-expanded.png#lightbox)
...
|---|---|---|---|
|Key|Ключи являются статическими и не нуждаются в обновлении. Ключи можно создать повторно вручную.|По умолчанию отключено| По умолчанию включено|
...
> [!WARNING]
> Машинное обучение Azure поддерживается совместной работой Azure Active Directory бизнес-бизнес, но в настоящее время не поддерживается для совместной работы Azure Active Directory.
...
Дополнительные сведения см. [в статье как безопасно выполнять эксперименты и вывод в изолированной виртуальной сети](how-to-enable-virtual-network.md).

Вы также можете включить частную связь Azure для вашей рабочей области. Частная ссылка позволяет ограничить обмен данными с рабочей областью из виртуальной сети Azure. Дополнительные сведения см. [в разделе Настройка частной ссылки](how-to-configure-private-link.md).
...
Этот `hbi_workspace` флаг управляет объемом данных, собираемых корпорацией Майкрософт в целях диагностики, и обеспечивает дополнительное шифрование в управляемых средах Майкрософт. Кроме того, он обеспечивает следующие действия:

* Запускает шифрование локального временного диска в кластере Амлкомпуте, если вы не создали предыдущие кластеры в этой подписке. В противном случае необходимо создать запрос в службу поддержки, чтобы включить шифрование временных дисков для кластеров вычислений. 
...
* Включает IP-фильтрацию для того, чтобы базовые пулы пакетов не можно было вызывать из внешних служб, отличных от Азуремачинелеарнингсервице
...
Если необходимо __повернуть или отозвать__ ключ, это можно сделать в любое время. При смене ключа учетная запись хранения начнет использовать новый ключ (последняя версия) для шифрования неактивных данных. При отмене (отключении) ключа учетная запись хранения позаботится о неудачных запросах. Для вступления в силу смены или отзыва обычно требуется час.
...
* Включите возможности управления ключами, управляемыми клиентом, для Cosmos DB. В настоящее время необходимо запросить доступ, чтобы использовать эту возможность. Для этого обратитесь к [cosmosdbpm@microsoft.com](mailto:cosmosdbpm@microsoft.com).
...
Этот Cosmos DB экземпляр создается в группе ресурсов, управляемой корпорацией Майкрософт в вашей подписке. Имя управляемой группы ресурсов — в формате `<AML Workspace Resource Group Name><GUID>`.
...
Если необходимо __повернуть или отозвать__ ключ, это можно сделать в любое время. При повороте ключа Cosmos DB начнет использовать новый ключ (последняя версия) для шифрования неактивных данных. При отмене (отключении) ключа Cosmos DB позаботится о сбое запросов. Для вступления в силу смены или отзыва обычно требуется час.
...
Развернутый ресурс экземпляра контейнера Azure (ACI) можно зашифровать с помощью управляемых клиентом ключей. Управляемый клиентом ключ, используемый для ACI, может храниться в Azure Key Vault рабочей области. Сведения о создании ключа см. в разделе [Шифрование данных с помощью управляемого клиентом ключа](../container-instances/container-instances-encrypt-data.md#generate-a-new-key).

Чтобы использовать ключ при развертывании модели в экземпляре контейнера Azure, создайте новую конфигурацию развертывания с помощью `AciWebservice.deploy_configuration()`. Укажите ключевые сведения, используя следующие параметры:
...
Дополнительные сведения об использовании ключа, управляемого клиентом, с ACI см. в разделе [Шифрование данных с помощью управляемого клиентом ключа](../container-instances/container-instances-encrypt-data.md#encrypt-data-with-a-customer-managed-key).
...
Машинное обучение Azure использует TLS для защиты внутренней связи между различными Машинное обучение Azure микрослужбами. Все доступ к хранилищу Azure также осуществляется по безопасному каналу.

Для защиты внешних вызовов конечной точки оценки Машинное обучение Azure использует TLS. Дополнительные сведения см. [в статье Использование TLS для защиты веб-службы с помощью машинное обучение Azure](https://docs.microsoft.com/azure/machine-learning/how-to-secure-web-service).
...
Корпорация Майкрософт также рекомендует не хранить конфиденциальные сведения (например, секретные ключи учетной записи) в переменных среды. Переменные среды записываются в журнал, шифруются и хранятся в США. Аналогичным образом при именовании [run_id](https://docs.microsoft.com/python/api/azureml-core/azureml.core.run%28class%29?view=azure-ml-py)не следует включать конфиденциальные сведения, такие как имена пользователей или имена секретных проектов. Эти сведения могут отображаться в журналах телеметрии, доступных для служба поддержки Майкрософт инженеров.

Вы можете отказаться от сбора диагностических данных, задав `hbi_workspace` для `TRUE` параметра значение во время подготовки рабочей области. Эта функция поддерживается при использовании пакетов SDK для AzureML Python, интерфейса командной строки, API-интерфейсов RESTFUL или шаблонов Azure Resource Manager.
...
* [Защита веб-служб Машинное обучение Azure с помощью TLS](how-to-secure-web-service.md)
* [Использование Машинное обучениеной модели, развернутой в качестве веб-службы](how-to-consume-web-service.md)
* [Использование Машинное обучение Azure с брандмауэром Azure](how-to-access-azureml-behind-firewall.md)
