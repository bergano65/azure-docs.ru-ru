...
|---|---|---|---|
|Клавиши|Ключи статичны и не нуждаются в обновлении. Ключи можно регенерировать вручную.|По умолчанию отключено| По умолчанию включено|
...
Флаг `hbi_workspace` контролирует объем данных, собираемых корпорацией Майкрософт для диагностических целей, и позволяет дополнительно шировать в управляемых средах Майкрософт. Кроме того, она позволяет следующим образом:

* Начинается шифрование локального скретч-диска в кластере Amlcompute при условии, что вы не создали никаких предыдущих кластеров в этой подписке. В противном случае необходимо повысить стоимость подтверждаемого билета для шифрования диска с нуля ваших вычислительных кластеров 
* Очищает локальный диск царапины между трассами
* Безопасно передает учетные данные для учетной записи хранилища, реестра контейнеров и SSH-аккаунта из уровня выполнения в вычислительные кластеры с помощью хранилища ключей
* Позволяет фильтровать IP для обеспечения того, чтобы базовые пакетные пулы не могли быть вызваны какими-либо внешними службами, кроме AzureMachineLearningService
...
Azure Machine Learning использует TLS для обеспечения внутренней связи между различными микрослужбами Машинного обучения Azure. Весь доступ к хранилищам Azure также осуществляется через защищенный канал.

Для обеспечения внешних вызовов в конечную точку Azure Machine Learning использует TLS. Для получения дополнительной информации [см.](https://docs.microsoft.com/azure/machine-learning/how-to-secure-web-service)
