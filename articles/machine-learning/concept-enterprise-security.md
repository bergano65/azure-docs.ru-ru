...
Большинство проверок подлинности для Машинное обучение Azure ресурсов используют Azure Active Directory (Azure AD) для проверки подлинности и управление доступом на основе ролей Azure (Azure RBAC) для авторизации. Исключения из этого:
...
Вы можете создать несколько рабочих областей, и каждая рабочая область может совместно использоваться несколькими пользователями. Вы можете контролировать, к каким функциям или операциям пользователи рабочей области могут получить доступ, назначив учетную запись Azure AD ролям Azure. Ниже перечислены встроенные роли.
...
Дополнительные сведения об использовании RBAC с Kubernetes см. в статье [Управление доступом Azure Role-Based для авторизации Kubernetes](../aks/manage-azure-rbac.md).
