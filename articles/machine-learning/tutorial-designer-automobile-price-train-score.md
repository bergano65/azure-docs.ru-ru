---
title: Руководство. прогнозированию цен на автомобили с помощью конструктора
...
ms.locfileid: "75532877"
---
# <a name="tutorial-predict-automobile-price-with-the-designer-preview"></a>Руководство. прогнозированию цен на автомобили с помощью конструктора (предварительная версия)
...
В первой части описано, как настраивать среду, перетаскивать модули на интерактивный холст и соединять их вместе, чтобы создать конвейер Машинного обучения Azure.
...
Из [второй части](tutorial-designer-automobile-price-deploy.md) этого учебника вы узнаете, как развернуть прогнозную модель в виде конечной точки вывода в реальном времени и прогнозировать цену любого автомобиля на основании отправленных технических характеристик. 
...
1. В верхней части холста выберите имя конвейера по умолчанию **Pipeline-Created-on**. Переименуйте его на более понятное. Например, *Automobile price prediction*. Имя не должно быть уникальным.
...
1. На панели "Свойства" справа от холста щелкните **Параметры** > **Изменить столбец**.
...
1. Введите **split data** в поле поиска, чтобы найти модуль **Split Data** (Разделение данных). Присоедините его к левому порту модуля **очистки недостающих данных**.
...
1. В области свойств для поля **Fraction of rows in the first output dataset** (Доля строк в первом наборе выходных данных) введите значение 0,7.
    ...
    Таким образом, мы применим 70 % данных для обучения модели и 30 % для тестирования.
...
1. В области "Свойства" выберите **Выходные данные** > **Визуализировать**.
...
1. На панели "Свойства" выберите **Выходные данные** > **Визуализировать**.
