...
Теперь, когда конвейер полностью настроен, можно выполнить прогон конвейера, чтобы обучить модель машинного обучения. Вы можете инициировать выполнение конвейера в любой момент при построении конвейеров в конструкторе. Это можно сделать, чтобы проверить выполнение по ходу работы, а также удостовериться, что конвейер работает, как ожидалось.
