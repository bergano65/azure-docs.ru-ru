...
description: Сведения о том, как обойти и устранить распространенные проблемы развертывания Docker с помощью Службы Azure Kubernetes и Экземпляров контейнеров Azure, используя Машинное обучение Azure.
...
ms.locfileid: "83873384"
---
# <a name="troubleshooting-azure-machine-learning-azure-kubernetes-service-and-azure-container-instances-deployment"></a>Устранение неполадок развертывания Службы Azure Kubernetes и Экземпляров контейнера Azure с помощью Машинного обучения Azure

Ознакомьтесь со сведениями о том, как обойти или устранить распространенные проблемы развертывания Docker с помощью Экземпляров контейнера Azure (ACI) и С Azure Kubernetes (AKS), используя Машинное обучение Azure.

При развертывании модели в Машинном обучении Azure система выполняет ряд задач.

Рекомендуемый и самый современный подход к развертыванию модели — использование API [Model.deploy()](https://docs.microsoft.com/python/api/azureml-core/azureml.core.model%28class%29?view=azure-ml-py#deploy-workspace--name--models--inference-config-none--deployment-config-none--deployment-target-none--overwrite-false-) с объектом [среды](how-to-use-environments.md) в качестве входного параметра. В этом случае наша служба создаст для вас базовый образ Docker на этапе развертывания и соединит необходимые модели в один вызов. Основные задачи развертывания:
...
Обратите внимание, что при определении собственной спецификации conda формата YAML необходимо составить список azureml-defaults с версией >= 1.0.45 в качестве зависимости pip. Этот пакет содержит функции, необходимые для размещения модели в качестве веб-службы.
...
При развертывании службы в целевой объект вычислений Службы Azure Kubernetes Машинное обучение Azure попытается запланировать службу с запрошенным количеством ресурсов. Если через 5 минут в кластере не появятся доступные узлы с соответствующим количеством доступных ресурсов, развертывание завершится неудачно с сообщением `Couldn't Schedule because the kubernetes cluster didn't have available resources after trying for 00:05:00`. Вы можете устранить эту ошибку, добавив больше узлов, изменив SKU своих узлов или изменив требования к ресурсам службы. 

В сообщении об ошибке обычно указывается ресурс, количество которого необходимо увеличить. Например, если отображается сообщение об ошибке `0/3 nodes are available: 3 Insufficient nvidia.com/gpu`, это означает, что для службы требуются GPU, а в кластере имеется 3 узла, у которых нет доступных GPU. Эту проблему можно решить, добавив больше узлов при использовании SKU-номера GPU. В противном случае переключитесь на SKU с поддержкой GPU или измените среду, убрав необходимость использования GPU.  
...
Вы можете увеличить время ожидания или попытаться ускорить службу, изменив файл score.py для удаления ненужных вызовов. Если проблема не исчезнет, используйте сведения из этой статьи для отладки файла score.py. Код может зависнуть или попасть в бесконечный цикл.
