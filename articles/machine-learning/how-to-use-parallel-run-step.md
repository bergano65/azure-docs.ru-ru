...
-  `run(mini_batch)`: Эта функция будет выполняться для каждого экземпляра `mini_batch`.
    ...
    -  `response`: метод run() должен возвращать Pandas DataFrame или массив. Для append_row output_action эти возвращаемые элементы добавляются в общий выходной файл. Для summary_only содержимое элементов игнорируется. Для всех выходных действий каждый возвращаемый элемент обозначает один успешный запуск входного элемента во входном мини-пакете. Пользователь должен убедиться в том, что в выполнение включено достаточно данных, чтобы сопоставить входные данные для выполнения. Выполнение вывода будет записано в выходной файл и для него не гарантируется правильный порядок. Для сопоставления со входными данными пользователю нужно использовать какой-либо ключ.
