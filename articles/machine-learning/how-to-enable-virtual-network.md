...
1. На странице __брандмауэров и виртуальных сетей__ выполняй следующие действия:
    ...
    - В __рамках виртуальных сетей__выберите __существующую виртуальную сетевую__ ссылку Add. Это действие добавляет виртуальную сеть, в которой находится ваш компьютер (см. шаг 1).
        ...
        > [!IMPORTANT]
        > Учетная запись хранилища должна находиться в той же виртуальной сети, что и вычислительные экземпляры или кластеры, используемые для обучения или выводов.
...
- Для __вычисляемого экземпляра__также добавьте последующие элементы:
...
1. На [портале Azure](https://portal.azure.com)выберите рабочее пространство для машин Azure Machine Learning.

1. В разделе __Приложения__ выберите __вычислить,__ а затем выберите __Добавить вычисление.__

1. Чтобы настроить этот вычислительный ресурс для использования виртуальной сети, сделайте следующие действия:

    а. Для __конфигурации сети__выберите __Advanced__.

    b. В списке выпадающих групп __ресурсов__ выберите группу ресурсов, содержащую виртуальную сеть.

    c. В __списке__ выпадающих сетей Virtual выберите виртуальную сеть, содержащую подсеть.

    d. В списке выпадающих __подсетей__ выберите подсеть для использования.
...
1. На [портале Azure](https://portal.azure.com)убедитесь, что NSG, которая управляет виртуальной сетью, имеет входящее правило, включенное для машинного обучения __Azure, используя AzureMachineLearning__ в качестве **ИСТОЧНИК.**

    [![Лазурное машинное обучение Добавить вычислительное стекло](./media/how-to-enable-virtual-network/aks-vnet-inbound-nsg-aml.png)](./media/how-to-enable-virtual-network/aks-vnet-inbound-nsg-aml.png#lightbox)

1. Выберите рабочее пространство машинного обучения Azure.

1. В разделе __Приложения__ выберите __вычислить,__ а затем выберите __Добавить вычисление.__

1. Чтобы настроить этот вычислительный ресурс для использования виртуальной сети, сделайте следующие действия:

    - Для __конфигурации сети__выберите __Advanced__.

    - В списке выпадающих групп __ресурсов__ выберите группу ресурсов, содержащую виртуальную сеть.

    - В __списке__ выпадающих сетей Virtual выберите виртуальную сеть, содержащую подсеть.

    - В списке выпадающих __подсетей__ выберите подсеть.

    - В поле __адресного диапазона kubernetes Service__ введите адресный диапазон услуг Kubernetes. Этот диапазон адресов использует диапазон IP-адресов Classless Inter-Domain Routing (CIDR) для определения IP-адресов, доступных для кластера. Он не должен пересекаться с любыми диапазонами IP-адресов подсети (например, 10.0.0.0/16).

    - В __ip-адресе сервиса Kubernetes DNS__ введите IP-адрес сервиса Kubernetes DNS. Это IP-адрес, назначенный службе DNS Kubernetes. Он должен находиться в пределах диапазона адресов сервиса Kubernetes (например, 10.0.0.10).

    - В адресной коробке __докерского моста__ введите адрес моста Докер. Этот IP-адрес, назначенный мосту Docker. Он не должен быть ни в каких диапазонах IP-адресов подсети или в диапазоне адресов службы Kubernetes (например, 172.17.0.1/16).
...
При использовании виртуальной сети с Azure Machine Learning __не__ введомы реестр контейнеров Azure для рабочего пространства в виртуальной сети. Эта конфигурация не поддерживается.
