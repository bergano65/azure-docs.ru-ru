...
ms.locfileid: "83816307"
---
# <a name="secure-your-machine-learning-lifecycles-with-private-virtual-networks"></a>Защитите свои жизненные циклы машинного обучения с помощью частных виртуальных сетей
[!INCLUDE [applies-to-skus](../../includes/aml-applies-to-basic-enterprise-sku.md)]

Из этой статьи вы узнаете, как изолировать задания в области экспериментирования, обучения, анализа и оценки в системе Машинного обучения Azure в виртуальной сети Azure. Кроме того, вы изучите некоторые *продвинутые настройки безопасности*, которые необязательны для базовых или экспериментальных вариантов использования.

> [!WARNING]
> Если базовое хранилище находится в виртуальной сети, пользователи не смогут использовать веб-интерфейс студии Машинного обучения Azure, включая следующее:
> - конструктор перетаскивания;
> - пользовательский интерфейс автоматизированного машинного обучения;
> - пользовательский интерфейс маркировки данных;
> - пользовательский интерфейс для наборов данных.
> - Записные книжки
> 
> При попытке отобразится сообщение об ошибке, аналогичное следующему: `__Error: Unable to profile this dataset. This might be because your data is stored behind a virtual network or your data does not support profile.__`

## <a name="what-is-a-vnet"></a>Что такое виртуальная сеть?

**Виртуальная сеть** выполняет роль границы безопасности, изолируя ресурсы Azure от общедоступного Интернета. Также вы можете подключить виртуальную сеть Azure к локальной сети. Объединяя сети, вы получаете возможность безопасно обучать модели и обращаться к развернутым моделям для формирования выводов.

Для обучения и развертывания моделей система Машинного обучения Azure использует другие службы Azure для получения вычислительных ресурсов ([целевых объектов вычисления](concept-compute-target.md)). Эти целевые объекты могут создаваться в виртуальной сети. Например, можно использовать вычислительные ресурсы Машинного обучения Azure для обучения модели с последующим развертыванием модели в службе Azure Kubernetes (AKS). 
...
Чтобы использовать [управляемый целевой объект вычислений **Машинного обучения Azure**](concept-compute-target.md#azure-machine-learning-compute-managed) или [вычислительный экземпляр  **Машинного обучения Azure**](concept-compute-instance.md) в виртуальной сети, должны выполняться следующие требования к сети.
...
Вам не нужно указывать группы безопасности сети на уровне подсети, так как пакетная служба Azure настраивает собственные группы. Если указанной подсети уже назначены группы безопасности сети или брандмауэр, настройте правила безопасности для входящего и исходящего трафика, как описано выше.
...
> [!NOTE]
> Если вы планируете использовать предоставляемые Майкрософт образы Docker по умолчанию и включите управляемые пользователем зависимости, необходимо также использовать __тег службы__ __MicrosoftContainerRegistry.Region_Name__ (например, MicrosoftContainerRegistry.EastUS).
>
...
1. На портале Azure перейдите в хранилище, подключенное к вашей рабочей области.
...
1. На странице __Брандмауэры и виртуальные сети__ выполните следующие действия.
    ...
    - В разделе __Виртуальные сети__ щелкните ссылку __Добавить существующую виртуальную сеть__. Это действие добавляет виртуальную сеть, в которой размещается вычислительная среда (см. шаг 1).
...
В следующем фрагменте кода показано, как **создать новый кластер AKS**, а затем обновить его для использования частного IP-адреса или внутреннего балансировщика нагрузки:
...
> [!NOTE]
> В настоящее время невозможно настроить балансировщик нагрузки при выполнении операции __присоединения__ в существующем кластере. Чтобы изменить балансировщик нагрузки, необходимо сначала присоединить кластер, а затем выполнить операцию обновления.
...
Azure Data Lake Storage второго поколения — это набор возможностей для аналитики больших данных, созданный на основе хранилища BLOB-объектов Azure. Он может использоваться для хранения данных, используемых для обучения моделей в системе Машинного обучения Azure. 

Чтобы использовать Data Lake Storage Gen 2 внутри виртуальной сети рабочей области Машинного обучения Azure, выполните следующие действия.

1. Создайте учетную запись Azure Data Lake Storage 2-го поколения. Для получения дополнительной информации обратитесь к разделу [Создание учетной записи Azure Data Lake Storage 2-го поколения](../storage/blobs/data-lake-storage-quickstart-create-account.md).

1. Выполните шаги 2–4 в предыдущем разделе [Использование учетной записи хранения для своей рабочей области](#use-a-storage-account-for-your-workspace), чтобы разместить учетную запись в виртуальной сети.

При использовании Машинного обучения Azure с Data Lake Storage 2-го поколения в виртуальной сети следуйте инструкциям ниже.

* Если вы используете __SDK для создания набора данных__ и система, в которой выполняется код __, не является виртуальной сетью__, используйте параметр `validate=False`. Этот параметр пропускает проверку, которая завершается ошибкой, если система не находится в той же виртуальной сети, что и учетная запись хранения. Дополнительные сведения см. в описании метода [from_files()](https://docs.microsoft.com/python/api/azureml-core/azureml.data.dataset_factory.filedatasetfactory?view=azure-ml-py#from-files-path--validate-true-).

* При использовании экземпляра или кластера вычислительных ресурсов Машинного обучения Azure для обучения модели с использованием набора данных он должен находиться в той же виртуальной сети, что и учетная запись хранения.
...
1. На странице **Хранилище ключей**в левой области выберите __Брандмауэры и виртуальные сети__.
