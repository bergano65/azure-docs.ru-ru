...
> [!WARNING]
> Если у вас есть специалисты по обработке данных, которые используют конструктор АЗур ногой машинного обучения, они получат ошибку при визуализации данных из учетной записи хранения внутри виртуальной сети. Следующий текст — это ошибка, которую они получают:
>
> __Ошибка: Невозможно профилировать этот набор данных. Это может быть связано с тем, что ваши данные хранятся за виртуальной сетью или ваши данные не поддерживают профиль.__
...
> [!TIP]
> Экземпляр или кластер машинного обучения автоматически выделяет дополнительные сетевые ресурсы __в группе ресурсов, содержащей виртуальную сеть.__ Для каждого экземпляра вычислений или кластера служба выделяет следующие ресурсы:
> 
> * Одна группа безопасности сети
> * Один общедоступный IP-адрес
> * Один балансировщик нагрузки
> 
> В случае кластеров эти ресурсы удаляются (и воссоздаются) каждый раз, когда кластер масштабируется до 0 узлов, однако в случае, если ресурсы удерживаются до полного удаления экземпляра (остановка не удаляет ресурсы). 
