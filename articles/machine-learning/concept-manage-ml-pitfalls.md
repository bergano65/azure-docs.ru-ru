...
> [!NOTE]
> Перекрестная проверка по умолчанию не включена. Ее необходимо настроить в параметрах автоматического машинного обучения. Однако после настройки перекрестной проверки и предоставления набора данных для проверки процесс выполняется автоматически. См. 
...
Так как алгоритмы классификации обычно оцениваются по точности, проверка уровня точности модели — хороший способ определения того, повлияли ли на нее несбалансированные данные. Была ли точность особенно высокой для одних классов, но слишком низкой для других?
...
В рамках задачи упрощения рабочего процесса машинного обучения автоматизированное машинное обучение предоставляет встроенные возможности для работы с несбалансированными данными. 

- **Столбец веса**. Автоматизированное машинное обучение поддерживает взвешенный столбец в качестве входных данных: вес строк данных меняется в большую или меньшую сторону, из-за чего важность классов повышается или понижается.

- Алгоритмы, используемые автоматизированным машинным обучением, могут правильно обрабатывать дисбаланс до 20:1. Это означает, что наиболее распространенный класс может иметь в 20 раз больше строк, чем наименее распространенный.

Ниже приведены дополнительные методы обработки несбалансированных данных, используемые за пределами автоматизированного машинного обучения. 
...
- Использование метрики производительности, которая лучше работает с несбалансированными данными. Например, показатель F1 — это взвешенное среднее значение точности и полноты. Точностью измеряется точность значения классификатора — чем ниже точность, тем больше ложноположительных результатов. Полнотой измеряется степень полноты классификатора — чем меньше полнота, тем больше ложноотрицательных результатов. 
