...
- [Azure Machine Learning Workbench](../service/overview-what-is-azure-ml.md).
    - Чтобы установить эту программу и создать учетные записи Экспериментирования и Управления моделями, выполните инструкции из [краткого руководства по установке и созданию](../desktop-workbench/quickstart-installation.md).
- Пакет SDK для Python и Azure CLI для [Batch AI](https://github.com/Azure/BatchAI).
    - Необходимо выполнить действия, описанные в следующих разделах [файла сведений с инструкциями по работе с Batch AI](https://github.com/Azure/BatchAI/tree/master/recipes):
        ...
        - Регистрация поставщиков ресурсов Batch AI (в разделе, посвященному выполнению инструкций с помощью Azure CLI).
    ...
    - На момент написания этой статьи для Azure Machine Learning Workbench и Azure Batch AI используются отдельные вилки Azure CLI. Для ясности мы называем версию CLI для Workbench как "CLI, запускаемый из Azure Machine Learning Workbench", а общедоступную версию (которая включает Batch AI) — Azure CLI.
...
Этот пример был протестирован на 10 компьютерах под управлением Windows. Его можно запустить с любого компьютера под управлением Windows, включая виртуальные машины обработки и анализа данных Azure. Azure CLI был установлен из файла MSI, согласно [этим инструкциям](https://github.com/Azure/azure-sdk-for-python/wiki/Contributing-to-the-tests#getting-azure-credentials). Могут потребоваться небольшие изменения (например, изменения в путях к файлам) при выполнении этого примера на macOS.
...
Ожидая передачи файла учетной записи хранения и развертывания кластера Spark, можно подготовить сетевой файловый сервер искусственного интеллекта пакетной службы и кластер GPU. Откройте Azure CLI с повышенными привилегиями и выполните следующую команду:
