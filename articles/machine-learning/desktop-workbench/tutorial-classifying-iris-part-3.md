---
title: Руководство по развертыванию модели для Службы машинного обучения Azure
description: Из этого полного руководства вы узнаете, как использовать Службу машинного обучения Azure. Это третья часть серии руководств. В ней рассматривается развертывание модели.
...
1. Убедитесь, что поставщик ресурсов Azure **Microsoft.ContainerRegistry** зарегистрирован в вашей подписке. Зарегистрировать этот поставщик ресурсов нужно до создания среды на шаге 3. С помощью следующей команды вы можете проверить, зарегистрирован ли он:
   ...
   В третьей строке выходных данных отображается **"registrationState": "Registering"**. Подождите немного и повторяйте команду **show**, пока в выходных данных не отобразится состояние **"registrationState": "Registered"**.
...
1. Создайте веб-службу в режиме реального времени, используя следующую команду:
   ...
   ```
   При помощи этой команды создается идентификатор веб-службы, который понадобится позднее. Опустите выходной каталог, если работаете в записной книжке.
   ...
   * `--collect-model-data true`: этот параметр позволяет включить сбор данных.
...
1. Создайте манифест.
   ...
   ```
   После выполнения этой команды вы получите идентификатор манифеста.  Опустите выходной каталог, если работаете в записной книжке.
...
1. Вы можете использовать эти данные из хранилища BLOB-объектов Azure. Программное обеспечение Майкрософт и средства с открытым кодом можно использовать разными способами.
   ...
   * [Hive.](https://docs.microsoft.com/azure/hdinsight/hdinsight-hadoop-linux-tutorial-get-started) Загрузите данные в формате CSV в таблицу Hive и выполняйте SQL-запросы прямо к большим двоичным объектам.
