...
Для развертывания модели потребуются перечисленные ниже компоненты.

* **Скрипт входа**. Этот скрипт принимает запросы, оценивает запросы с помощью модели и возвращает результаты.

    > [!IMPORTANT]
    > * Скрипт относится только к модели. Он должен понимать формат данных входящего запроса, формат данных, ожидаемых моделью, и формат данных, возвращаемых клиентам.
    >
...
* **Конфигурация развертывания** для целевого объекта вычислений, в котором размещается развернутая модель. Эта конфигурация описывает такие параметры, как память и ЦП, необходимые для выполнения модели.

Эти компоненты инкапсулируются в *конфигурацию зависимостей* и *конфигурацию развертывания*. В конфигурации зависимостей указываются скрипт входа и другие зависимости. Если для развертывания используется пакет SDK, эти конфигурации определяются программно. При использовании интерфейса командной строки (CLI) они определяются в файлах JSON.
