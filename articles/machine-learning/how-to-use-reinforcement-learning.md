...
Запустите этот код в любой из следующих сред. Чтобы ускорить процесс, рекомендуем использовать вычислительную операцию Машинного обучения Azure. Доступные примеры записных книжек с подкреплением можно быстро клонировать и применить для выполнения вычислительной операции Машинного обучение Azure.
 ...
 - Собственный сервер Jupyter Notebook
    ...
    - Установите [пакет SDK для Машинного обучения Azure](https://docs.microsoft.com/python/api/overview/azure/ml/install?view=azure-ml-py).
    - Установите [пакет SDK обучения с подкреплением для Машинного обучения Azure](https://review.docs.microsoft.com/python/api/azureml-contrib-reinforcementlearning/?view=azure-ml-py&branch=harnvirdebug): `pip install --upgrade azureml-contrib-reinforcementlearning`.
...
Создайте [эксперимент](https://docs.microsoft.com/python/api/azureml-core/azureml.core.experiment.experiment?view=azure-ml-py), чтобы отслеживать выполнение обучения с подкреплением. В Машинном обучении Azure эксперименты — это логические коллекции связанных попыток, используемые для упорядочения журналов выполнения, прочих журналов, выходных данных и многого другого.
...
Для вычислений головной узел использует одну [виртуальную машину `STANDARD_NC6`](https://docs.microsoft.com/azure/virtual-machines/nc-series) (ВМ). Он оснащен 6 виртуальными ЦП, поэтому может распределять работу между ними.
...
В этом примере для целевого рабочего вычислительного узла используются четыре [виртуальные машины `STANDARD_D2_V2`](https://docs.microsoft.com/azure/virtual-machines/nc-series). На каждом рабочем узле доступно 2 ЦП, что в совокупности составляет 8 доступных ЦП для параллельной работы.
...
В этом разделе вы узнаете, как использовать [ReinforcementLearningEstimator](https://review.docs.microsoft.com/python/api/azureml-contrib-reinforcementlearning/azureml.contrib.train.rl.reinforcementlearningestimator?view=azure-ml-py&branch=harnvirdebug) для отправки задания обучения в Машинное обучение Azure.

Машинное обучение Azure использует классы оценщика для инкапсуляции сведений о конфигурации выполнения. Это позволяет легко указать, как настроить выполнение скрипта. Дополнительные сведения о шаблоне оценщика Машинного обучения Azure см. в статье [Обучение моделей с помощью оценщика](how-to-train-ml-models.md).
...
Головной узел — это [Standard_NC6](https://docs.microsoft.com/azure/virtual-machines/nc-series) с 6 виртуальными ЦП. Рабочий кластер — это 4 [виртуальные машины Standard_D2_V2](https://docs.microsoft.com/azure/cloud-services/cloud-services-sizes-specs#dv2-series) с двумя ЦП на каждой, всего 8 ЦП. Однако из числа рабочих процессоров необходимо вычесть 1 ЦП, так как один из их нужно выделить для роли головного узла. 6 ЦП + 8 ЦП – 1 головной ЦП = 13 одновременных рабочих процессов. Машинное обучение Azure использует кластеры головного и рабочих узлов, чтобы различать вычислительные ресурсы. Однако Ray не различает головной и рабочие процессы, а все ЦП доступны для выполнения рабочих потоков.
...
Команда [run](https://docs.microsoft.com/python/api/azureml-core/azureml.core.run(class)?view=azure-ml-py) обрабатывает журнал выполнения для выполняющихся или завершенных заданий. 
