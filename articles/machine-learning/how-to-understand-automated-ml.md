...
В этой статье вы узнаете, как просматривать и оценивать результаты автоматизированного машинного обучения, Аутомл экспериментов. Эти эксперименты состоят из нескольких запусков, где каждый запуск создает модель. Чтобы помочь вам оценить каждую модель, Аутомл автоматически создает метрики производительности и диаграммы, относящиеся к типу эксперимента. 

Например, Аутомл предоставляет различные диаграммы для моделей классификации и регрессии. 
...
В следующей таблице перечислены метрики производительности модели, которые Аутомл вычисляет для каждой модели классификации, создаваемой для вашего эксперимента. 
...
Более высокая кривая предсказания, то есть более высокая модель выше базовой, указывает на лучшее выполнение модели. 
...
В следующей таблице перечислены метрики производительности модели, которые Аутомл вычисляет для каждой модели регрессии или прогнозирования, созданной для вашего эксперимента. 
...
### <a name="what-does-a-good-model-look-like"></a>Как выглядит хорошая модель?
Эта диаграмма позволяет оценить эффективность модели: чем ближе прогнозируемые значения к графику y=x, тем выше точность прогнозной модели.
