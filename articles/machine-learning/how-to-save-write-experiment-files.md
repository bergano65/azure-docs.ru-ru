...
Необходимо использовать указанный каталог сценариев| Создайте `.amlignore` файл, чтобы исключить файлы из моментального снимка эксперимента, которые не являются частью исходного кода. Добавьте имена файлов в `.amlignore` файл и поместите его в тот же каталог, в котором находится сценарий обучения. Файл использует тот же [синтаксис и шаблоны](https://git-scm.com/docs/gitignore) , что и `.gitignore` файл. `.amlignore`
...
Если хранилище данных не требуется, запишите файлы в папку `./outputs` и/или. `./logs`
