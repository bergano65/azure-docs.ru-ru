...
Существует несколько подклассов `PipelineStep`. Кроме того `AutoMLStep`, в этой статье будет показана `PythonScriptStep` подготовка данных и другая для регистрации модели.

Предпочтительный способ первоначального перемещения данных _в_ конвейер машинного обучения — `Dataset` с объектами. Для перемещения данных _между_ шагами предпочтительным способом является `PipelineData` объекты. Для использования с `AutoMLStep` `PipelineData` объект должен быть преобразован в `PipelineOutputTabularDataset` объект. Дополнительные сведения см. [в статье входные и выходные данные из конвейеров ML](how-to-move-data-in-out-of-pipelines.md).
...
`Pipeline` Выполняется в `Experiment`. Конвейер `Run` имеет для каждого этапа дочерний `StepRun`элемент. Выходные данные автоматизированного ML `StepRun` — это обучающие показатели и модель с наивысшей производительностью.
...
Так как `output_path` параметр является полным, функция `os.makedirs()` используется для подготовки структуры каталогов. На этом этапе можно использовать `DataFrame.to_csv()` для записи выходных данных, но Parquet файлы более эффективны. Эта эффективность может быть непростой для такого небольшого набора данных, но **PyArrow** использование функций `from_pandas()` и `write_table()` PyArrow пакета является лишь еще несколькими нажатиями `to_csv()`.
...
В приведенном выше фрагменте кода создается `PipelineOutputTabularDataset` высокопроизводительный `PipelineOutputFileDataset` результат выполнения этапа подготовки данных.
...
|| [Записная `PipelineOutputTabularDataset` книжка с методом](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/machine-learning-pipelines/nyc-taxi-data-regression-model-building/nyc-taxi-data-regression-model-building.ipynb) |
...
Выходные данные `AutoMLStep` являются окончательными показателями модели с более высокой производительностью и самой моделью. Чтобы использовать эти выходные данные в дальнейших шагах `PipelineData` конвейера, подготавливайте объекты для их получения.
...
В приведенном выше фрагменте `PipelineData` кода создаются два объекта для метрик и выходных данных модели. Каждому `type` `TrainingOutput` из них присваивается имя, которое назначается хранилищу данных по умолчанию, полученному ранее, и связывается с конкретным из `AutoMLStep`. Так как мы `pipeline_output_name` присваиваем эти `PipelineData` объекты, их значения будут доступны не только на этапе отдельного конвейера, но и из конвейера в целом, как будет рассмотрено ниже в разделе «анализ результатов конвейера». 
...
train_step = AutoMLStep(name='AutoML_Classification',
    ...
    allow_reuse=True)
```
В фрагменте кода показана идиома `AutoMLConfig`, обычно используемая с. Аргументы, которые являются более гибкими (длинных), задаются в отдельном словаре, а значения с меньшей вероятностью изменяются непосредственно в `AutoMLConfig` конструкторе. В этом случае, укажите `automl_settings` краткий запуск: выполнение будет прерываться только после 2 итераций или 15 минут, в зависимости от того, что происходит раньше.

`automl_settings` Словарь передается в `AutoMLConfig` конструктор как кваргс. Другие параметры не являются сложными:

- `task`для этого примера `classification` задано значение. Другие допустимые значения `regression` — и.`forecasting`
...
>[!Important]
> Необходимо задать значение `passthru_automl_config` , `False` если `AutoMLStep` для входа используются `PipelineOutputTabularDataset` объекты.

В этом примере автоматизированный процесс машинного обучения выполнит перекрестные проверки на `training_data`. Число перекрестных проверок можно контролировать с помощью `n_cross_validations` аргумента. Если вы уже разделили обучающие данные как часть шагов подготовки данных, вы можете задать собственное `validation_data` `Dataset`значение.
...
Модель регистрируется в `Workspace`. Вы, вероятно, знакомы с `Workspace.from_config()` использованием для входа в рабочую область на локальном компьютере, но существует другой способ получить рабочую область из работающего конвейера ml. `Run.get_context()` Возвращает активный `Run`объект. Этот `run` объект предоставляет доступ ко многим важным объектам, включая `Workspace` использованный здесь.
...
`PipelineParameter` При регистрации `PythonScriptStep` модели используется для одного из своих аргументов. Параметры конвейера — это аргументы конвейеров, которые могут быть легко заданы во время отправки выполнения. После объявления они передаются как обычные аргументы. 
...
Создание и запуск конвейера, содержащего объект `AutoMLStep` , не отличается от обычного конвейера. 
...
Если вы выполнили следующие инструкции вместе с этой статьей, то у вас будет `run` экземпляр объекта. Но вы также можете получить готовые `Run` объекты из `Workspace` , используя `Experiment` объект.

Рабочая область содержит полную запись всех экспериментов и запусков. Можно использовать портал для поиска и скачивания выходных данных экспериментов или использования кода. Чтобы получить доступ к записям из исторических запусков, используйте Машинное обучение Azure, чтобы найти идентификатор требуемого запуска. С помощью этого идентификатора можно выбрать конкретное `run` значение для `Workspace` и. `Experiment`
...
Вам пришлось бы изменить строки в приведенном выше коде на особенности своего исторического выполнения. В приведенном выше фрагменте предполагается `ws` , что вы `Workspace` присвоены `from_config()`значение, соответствующее стандарту. Полученный эксперимент извлекается напрямую, а затем код находит интересующий `Run` его объект, сопоставляя `run.id` значение.
...
Каждый `Run` объект содержит `StepRun` объекты, содержащие сведения о выполнении отдельных шагов конвейера. `run` Поиск `StepRun` объекта для `AutoMLStep`. Метрики и модель извлекаются с использованием имен по умолчанию, которые доступны, даже если не `PipelineData` передать объекты в `outputs` параметр `AutoMLStep`. 
