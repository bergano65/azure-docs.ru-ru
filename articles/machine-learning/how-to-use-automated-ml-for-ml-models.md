...
Ограждения данных применяются при включении автоматической фетурации или автоматической проверке. Ограждения данных помогают выявлять потенциальные проблемы с данными (например, отсутствующие значения, дисбаланс классов) и принимать корректирующие меры для улучшения результатов. Есть много передовой практики, которые доступны и могут быть применены для достижения надежных результатов. Пользователи могут просматривать данные ограждения в студии в вкладке ```show_output=True``` **Data ограждения** автоматизированного запуска ML или путем установки при отправке эксперимента с помощью Python SDK. В следующей таблице описаны защищенные данные, поддерживаемые в настоящее время, и связанные с ними статусы, с которыми пользователи могут столкнуться при отправке эксперимента.
...
Ограждения данных будут отображать одно из трех состояний: 'Пройдено', 'Сделано, или 'Alerted'.
...
Предыдущая версия Автоматизированного ML отображалась в четвертом состоянии: 'Fixed'. Новые эксперименты не будут отображать это состояние, и все ограждения, которые отображаются 'Fixed' состояние теперь будет отображать 'Done'.   
