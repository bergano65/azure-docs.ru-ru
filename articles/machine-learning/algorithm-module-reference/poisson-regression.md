...
> [!TIP]
> Если цель не заключается в подсчете количеств, то возможно распределение Пуассона является не подходящим способом. Попробуйте [другие модули регрессии в конструкторе](https://docs.microsoft.com/azure/machine-learning/algorithm-module-reference/module-reference#machine-learning-algorithms). 
...
1. Добавьте модуль **регрессии Пуассона** в конвейер в конструкторе (Предварительная версия). Этот модуль можно найти в разделе **алгоритмы машинное обучение**в категории **регрессии** .
...
5. Весовой коэффициент использования **L1** и **вес "основной" L2**: значения типа, используемые для упрощения L1 и L2. *Регуляризация* добавляет ограничения алгоритма относительно аспектов модели, которые не зависят от данных для обучения. Регуляризация обычно используется, чтобы избежать переобучения. 
    ...
    Хорошее описание работы с уровнями "L1" и "L2" см. в разделе "сравнение L1 [и L2" для машинное обучение](https://msdn.microsoft.com/magazine/dn904675.aspx).
