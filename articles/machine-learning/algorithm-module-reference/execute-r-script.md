---
title: 'Выполнить сценарий R: Справочник по модулям'
titleSuffix: Azure Machine Learning service
description: Узнайте, как использовать модуль "выполнение скрипта R" в Машинное обучение Azure Service для выполнения кода R.
...
ms.locfileid: "70128801"
---
# <a name="execute-r-script"></a>Выполнение сценария R

В этой статье описывается, как использовать модуль « **выполнение скрипта r** » для выполнения кода R в эксперименте с визуальным интерфейсом.
...
- Создание моделей с использованием алгоритмов, которые не реализованы как автономные модули в визуальном интерфейсе

## <a name="r-version-support"></a>Поддержка версии R

Визуальный интерфейс Машинное обучение Azure службы использует распределение R по CRAN (всеобъемлющую сеть с архивом R). Текущая используемая версия — CRAN 3.5.1.
...
Чтобы установить дополнительные пакеты R, используйте `install.packages()` метод. Не забудьте указать репозиторий CRAN. Пакеты устанавливаются для каждого модуля « **выполнение скрипта r** » и не являются общими для других модулей « **выполнение скрипта r** ».
...
Наборы данных, хранящиеся в визуальном интерфейсе, автоматически преобразуются в кадр с данными R при загрузке с этим модулем.
...
1. Подключите любые входные данные, необходимые для скрипта. Входные значения необязательны и могут включать данные и дополнительный код R.
    ...
    * **DataSet1**: Сослаться на первый `dataframe1`вход как. Входной набор данных должен быть отформатирован в виде CSV-файла, TSV, ARFF, либо можно подключить набор данных Машинное обучение Azure.

    * **DataSet2**: Сослаться на второй `dataframe2`вход как. Этот набор данных также должен быть отформатирован в виде файла CSV, TSV, ARFF или набора данных Машинное обучение Azure.
...
1.  **Случайное начальное значение**: Введите значение, которое будет использоваться в среде R как значение случайного начального значения. Этот параметр эквивалентен вызову `set.seed(value)` в коде R.  

1. Запустите эксперимент.  

## <a name="results"></a>Результаты

Модули **выполнения сценариев r** могут возвращать несколько выходов, но они должны быть представлены в виде кадров данных R. Данные автоматически преобразуются в наборы данных визуальных интерфейсов для обеспечения совместимости с другими модулями.

Стандартные сообщения и ошибки из R возвращаются в журнал модуля.

## <a name="sample-scripts"></a>Примеры сценариев

Существует множество способов расширения эксперимента с помощью пользовательского скрипта R.  В этом разделе приведен пример кода для распространенных задач.
...
1. Все файлы, содержащиеся в ZIP-файле, доступны во время выполнения эксперимента. 
...
Объекты R можно передавать между экземплярами модуля **выполнить сценарий R** с помощью внутреннего механизма сериализации. В этом примере предполагается, что необходимо переместить объект R с `A` именем между двумя модулями **выполнение скрипта r** .

1. Добавьте в эксперимент первый модуль **выполнить сценарий R** и введите следующий код в текстовом поле **Скрипт r** , чтобы создать сериализованный объект `A` в виде столбца в таблице выходных данных модуля:  
    ...
    Явное преобразование в целочисленный тип выполняется потому, что функция сериализации выводит данные в `Raw` формате R, который не поддерживается визуальным интерфейсом.
