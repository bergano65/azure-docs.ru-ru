...
* [Оценка или развертывание модели](#build-inference-pipeline-that-uses-n-grams-to-deploy-a-real-time-endpoint) , использующей n-граммы.
...
### <a name="build-inference-pipeline-that-uses-n-grams-to-deploy-a-real-time-endpoint"></a>Конвейер определения сборки, использующий n-граммы для развертывания конечной точки в режиме реального времени

Обучающий конвейер, который содержит **функцию извлечения N-грамм из модели текста** и **оценки** для создания прогноза на основе тестового набора данных, построена в следующей структуре:

:::image type="content" source="./media/module/extract-n-gram-training-pipeline-score-model.png" alt-text="Пример извлечения N-граммного конвейера обучения" border="true":::

**Режим словаря** для **функции извлечения N-грамм** в кружке из текстового модуля — **CREATE**, а **режим словаря** модуля, который подключается к модулю **оценки модели** , доступен **только для чтения**.

После успешной отправки обучающего конвейера можно зарегистрировать выходные данные модуля Circle в качестве набора данных.

:::image type="content" source="./media/module/extract-n-gram-output-voc-register-dataset.png" alt-text="зарегистрировать набор данных" border="true":::

Затем можно создать конвейер вывода в режиме реального времени. После создания конвейера вывода необходимо настроить конвейер вывода вручную следующим образом:

:::image type="content" source="./media/module/extract-n-gram-inference-pipeline.png" alt-text="конвейер вывода" border="true":::
