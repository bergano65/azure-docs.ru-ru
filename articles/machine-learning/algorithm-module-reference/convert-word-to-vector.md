...
ms.locfileid: "84753893"
---
# <a name="convert-word-to-vector-module"></a>Преобразование слова в векторный модуль

В этой статье описывается, как использовать модуль преобразование слова в векторы Машинное обучение Azure конструктора (Предварительная версия) для следующих задач.
...
В целом, преобразование слов в векторы или векторы слов — это процесс обработки естественного языка (NLP). В процессе используются языковые модели или методы для отображения слов в пространстве вектора, то есть для представления каждого слова по вектору вещественных чисел. В то же время, слова с аналогичными значениями позволяют использовать похожие представления.

Внедрения слов можно использовать в качестве начальных входных данных для NLPных задач, таких как классификация текста и анализ тональности.
...
+ Word2Vec — одна из самых популярных методик для изучения внедрения слов с помощью неполной нейронной сети. Теория обсуждается в этой статье, которая доступна как загрузка в формате PDF: [эффективная оценка представлений Word в пространстве векторов, Mikolov, (Tomas, et al](https://arxiv.org/pdf/1301.3781.pdf). Реализация в этом модуле основана на [библиотеке женсим для Word2Vec](https://radimrehurek.com/gensim/models/word2vec.html).

+ Теория Фасттекст объясняется в этой статье, которая доступна в виде PDF-файла: [обогащение векторов слов с информацией о подсловах, Божановски, Пиотр, et al](https://arxiv.org/pdf/1607.04606.pdf). Реализация в этом модуле основана на [библиотеке женсим для фасттекст](https://radimrehurek.com/gensim/models/fasttext.html).
...
5. Если **стратегия Word2Vec** — **Женсим Word2Vec** или **женсим фасттекст**:
    ...
    + Для параметра **число эпох**укажите количество эпох (итераций) в совокупности. Этот параметр соответствует `iter` параметру в женсим.
...
6. Для параметра **максимальный размер словаря**укажите максимальное число слов в созданном словаре.
    ...
    Если более уникальных слов нет, обрезать редко используемые слова.
...
В следующем примере показано, как работает модуль преобразование слова в вектор Vector. Он применяет этот модуль с параметрами по умолчанию к предварительно обработанному набору данных Википедии SP 500, предоставленному в Машинное обучение Azure (Предварительная версия).

### <a name="source-dataset"></a>Исходный набор данных

Набор данных содержит столбец Category, а также полный текст, полученный из Википедии. В этой таблице приведено лишь несколько репрезентативных примеров.
...
+ Разница между интерактивным обучением и предварительно обученной моделью:
    ...
    Если в качестве стратегии направления слов выбрана предварительно обученная модель специализированный, она суммирует словарь из входного набора данных и создает вектор внедрения для каждого слова из предварительно обученной модели. Без интерактивного обучения использование предварительно обученной модели может сэкономить время обучения. Он обеспечивает лучшую производительность, особенно если размер входного набора данных относительно мал.

+ Размер внедрения:

    Как правило, длина внедрения слова равна многим сотням (например, 100, 200, 300) для достижения хорошей производительности. Причина заключается в том, что небольшой размер внедрения означает небольшое векторное пространство, что может привести к конфликтам внедрения слов.  

    Для предварительно обученных моделей длина внедрений слов исправлена. В этой реализации размер внедрения специализированный-wiki-гигаворд-100 равен 100.
...
Список ошибок, характерных для модулей конструктора (Предварительная версия), см. в разделе [машинное обучение коды ошибок](designer-error-codes.md).
