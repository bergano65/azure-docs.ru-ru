...
description: Узнайте, как использовать модуль перекрестной проверки модели в Машинное обучение Azure для перекрестной проверки оценок параметров для моделей классификации или регрессии путем секционирования данных.
...
ms.locfileid: "90898568"
---
# <a name="cross-validate-model"></a>Перекрестная проверка модели

В этой статье описывается использование модуля перекрестная проверка модели в конструкторе Машинное обучение Azure. *Перекрестная проверка* — это методика, часто используемая в машинном обучении для оценки вариативности набора данных и надежности любой модели, обученной этими данными.  
...
1. Перекрестная проверка случайным образом делит обучающие данные на сгибы. 
   ...
   Если вы ранее не секционировали определенный набор данных, по умолчанию алгоритм создает 10 сверток. Чтобы разделить набор данных на другое число сверток, можно использовать модуль [Partition и Sample](partition-and-sample.md) , а также указать, сколько сверток следует использовать.  
...
2. Соедините выходные данные любой модели классификации или регрессии. 
    ...
    Например, если вы используете **два класса с повышенным деревом принятия решений** для классификации, настройте модель с нужными параметрами. Затем перетащите соединитель из порта **обученной модели** классификатора в соответствующий порт модели перекрестной проверки. 
...
4.  На правой панели перекрестной проверки модели нажмите кнопку **изменить столбец**. Выберите один столбец, содержащий метку класса, или прогнозируемое значение. 
...
Чтобы просмотреть результаты, щелкните правой кнопкой мыши модуль перекрестная проверка модели в конвейере. Выберите **визуализировать результаты оценки**.
...
Чтобы просмотреть результаты, щелкните правой кнопкой мыши модуль перекрестная проверка модели в конвейере. Выберите **визуализировать результаты оценки по сгибу**.
