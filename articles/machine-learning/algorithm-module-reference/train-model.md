...
2. Добавьте модуль **обучение модели** в конвейер.  Этот модуль можно найти в категории **машинное обучение** . Разверните узел **обучение**, а затем перетащите модуль **обучение модели** в конвейер.
...
+ Чтобы использовать модель в других конвейерах, щелкните правой кнопкой мыши модель и выберите команду **сохранить модель**. Введите имя модели. 

    При этом модель сохраняется в виде моментального снимка, который не обновляется повторными запусками конвейера.
