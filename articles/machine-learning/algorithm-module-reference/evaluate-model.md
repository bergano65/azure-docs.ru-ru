...
1. Подключите выходные данные оцененного **набора данных** [модели оценки](./score-model.md) или результирующего набора данных для [назначения данных кластерам](./assign-data-to-clusters.md) к левому порту ввода для параметра **Оценка модели**. 
  > [!NOTE] 
  > Если для выбора части входного набора данных используются такие модули, как "Выбор столбцов в наборе данных", убедитесь, что столбец с фактическими метками (используемый в обучении), столбец "оцененные вероятности" и столбец "оцененные метки" существуют, чтобы вычислить метрики, такие как AUC, точность для обнаружения двоичной классификации и аномалий.
  > Фактический столбец меток, столбец "оцененные метки" для вычисления метрик для многоклассовой классификации и регрессии.
  > Столбец "назначения", столбцы "Дистанцестоклустерцентер No. X "(X — центроид индекс, от 0 до..., число средневзвешенных-1) существует для вычисления метрик для кластеризации.

2. Используемых Подключите выходные данные оцененного **набора данных** к [модели оценки](./score-model.md) или результирующему набору данных для параметра назначить данные в кластеры для второй модели на **правый** порт ввода для параметра **Оценка модели**. Можно легко сравнивать результаты из двух разных моделей в одних и тех же данных. Два алгоритма ввода должны иметь одинаковый тип алгоритма. Или вы можете сравнить результаты двух различных выполнений по аналогичным данным с разными параметрами.
