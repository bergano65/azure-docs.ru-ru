---
title: 'Оцените модель: Справочник по модулям'
...
> [!TIP]
> Если вы не знакомы с оценкой модели, мы рекомендуем использовать серию видео Dr. Стивен Elston) в рамках [курса машинного обучения](https://blogs.technet.microsoft.com/machinelearning/2015/09/08/new-edx-course-data-science-machine-learning-essentials/) от EdX. 
...
2. Нажмите кнопку "вычислить модуль **модели** " и запустите эксперимент, чтобы создать оценки.
...
После выполнения **оценки модели**щелкните правой кнопкой мыши модуль и выберите **результаты оценки** , чтобы просмотреть результаты. Можно выполнить следующие действия:
...
- **Равна нулю одна ошибка (мзое)** указывает, был ли прогноз правильным.  Иными словами: `ZeroOneLoss(x,y) = 1` when `x!=y`; в противном случае — `0`.
