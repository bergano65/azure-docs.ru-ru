...
> [!IMPORTANT]
> Для [`provisioning_configuration()`](https://docs.microsoft.com/python/api/azureml-core/azureml.core.compute.akscompute?view=azure-ml-py), если вы выбираете пользовательские значения для `agent_count` и `vm_size`, а `cluster_purpose` не `DEV_TEST`, то необходимо убедиться, что `agent_count` умножение `vm_size` на больше или равно 12 виртуальным ЦП. Например, если вы используете `vm_size` "Standard_D3_v2" с 4 виртуальными процессорами, то следует выбрать `agent_count` не более 3.
>
