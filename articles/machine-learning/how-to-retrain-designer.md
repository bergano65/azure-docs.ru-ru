...
Конвейер, используемый в этой статье, является измененной версией [примера 3: прогноз дохода](samples-designer.md#classification). Конвейер использует модуль [Импорт данных](algorithm-module-reference/import-data.md) вместо образца набора данных, чтобы продемонстрировать, как обучать модели с помощью собственных данных.
