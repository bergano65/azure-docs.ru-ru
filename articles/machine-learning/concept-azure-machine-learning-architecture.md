...
1. **Обучение**
    + Разработка сценариев обучения машинного обучения в **Python**, **R**или с помощью визуального конструктора.
    + Создайте и настройте **целевой объект вычислений**.
    + **Отправьте скрипты** в настроенный целевой объект вычислений для выполнения в этой среде. Во время обучения скрипты могут считывать или записывать в **хранилища данных**. Журналы и выходные данные, созданные во время обучения, сохраняются как **выполняемые** в **рабочей области** и группируются в рамках **экспериментов**.
...
* [Рабочая область](#workspaces)
    ...
    * [DataSet & хранилища данных](#datasets-and-datastores)
...
Запуск — это однократный запуск сценария обучения. Как правило, эксперимент будет содержать несколько запусков.

Машинное обучение Azure записывает все запуски и сохраняет в эксперименте следующую информацию:
...
* [Обучение и регистрация моделей цепочек в масштабе с помощью машинное обучение Azure](how-to-train-ml-models.md).
...
### <a name="compute-instance-preview"></a><a name="compute-instance"></a>Вычислительный экземпляр (Предварительная версия)
...
Дополнительные сведения см. в разделе [Создание и регистрация машинное обучение Azure наборов данных](how-to-create-register-datasets.md).  Дополнительные примеры использования наборов данных см. в [примере записных книжек](https://github.com/Azure/MachineLearningNotebooks/tree/master/how-to-use-azureml/work-with-data/datasets-tutorial).
