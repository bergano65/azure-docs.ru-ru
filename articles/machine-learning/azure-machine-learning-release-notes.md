...
+ **Исправления ошибок и улучшения**
  ...
  + **azureml-mlflow**
    ...
    + Добавлена поддержка RBAC для операций AzureML-Млфлов. 
    
  + **azureml-pipeline-core**
    + Улучшена документация по методам Пипелинеаутпутфиледатасет. parse_ *.
    + Новый Kusto шаг и целевой объект Kusto вычислений.
...
+ **Исправления ошибок и улучшения**
  ...
  + **azureml-core**
    ...
    + Исправление ошибки в наборе данных. get_by_name, где будут отображаться теги для новейшей версии набора данных, даже если была получена определенная более старая версия.
...
+ **Исправления ошибок и улучшения**
  ...
  + **azureml-core**
    ...
    + Добавлен параметр [Environment. build_local ()](https://docs.microsoft.com/python/api/azureml-core/azureml.core.environment.environment?view=azure-ml-py&preserve-view=true) для отправки образа в реестр контейнеров рабочих областей AzureML.
...
+ **Исправления ошибок и улучшения**
  ...
  + **azureml-core**
    ...
    + Добавлены сведения о CreatedBy в объекты Model и Service. Доступ возможен через. created_by
...
+ **Исправления ошибок и улучшения**
  ...
  + **[azureml-core](https://docs.microsoft.com/python/api/azureml-core/azureml.core)**
    ...
    + Добавлены дополнительные поля в функцию [list_nodes ()](https://docs.microsoft.com/python/api/azureml-core/azureml.core.compute.amlcompute.amlcompute#list-nodes--) для отображения выполнения, закрытого, общедоступного IP-адреса, порта и т. д.
...
[`PipelineEndpoint`](https://docs.microsoft.com/python/api/azureml-pipeline-core/azureml.pipeline.core.pipeline_endpoint.pipelineendpoint?view=azure-ml-py&preserve-view=true)Было введено Добавление новой версии опубликованного конвейера с сохранением той же конечной точки.
...
+ **Новые функции**
  ...
  Список версий, поддерживаемых последней версией пакета SDK, см. в документации по средству [оценки DNN](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.dnn?view=azure-ml-py&preserve-view=true).
...
+ **Новые функции**
  + Метод *azureml. Core. Run. create_children* позволяет создавать множественные дочерние операции с одним вызовом с низкой задержкой.
...
+ **Новые функции**
  + Машинное обучение Azure теперь предоставляет поддержку первого класса для популярного каркаса DNN Framework. С помощью [`Chainer`](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.dnn.chainer?view=azure-ml-py&preserve-view=true) классов пользователи могут легко обучать и развертывать модели цепочек.
...
+ **Исправления ошибок и улучшения**
  + Мы добавили поддержку в конвейеры Машинное обучение Azure, чтобы задать для свойства source_directory_data_store нужное хранилище данных (например, хранилище BLOB-объектов) на [рунконфигуратионс](https://docs.microsoft.com/python/api/azureml-core/azureml.core.runconfig.runconfiguration?view=azure-ml-py&preserve-view=true) , предоставленное для [писонскриптстеп](https://docs.microsoft.com/python/api/azureml-pipeline-steps/azureml.pipeline.steps.python_script_step.pythonscriptstep?view=azure-ml-py&preserve-view=true). По умолчанию в качестве резервного хранилища используется хранилище файлов Azure, которое может выполняться в случае проблем регулирования, когда выполняется большое количество шагов одновременно.
