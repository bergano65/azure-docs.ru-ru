...
+ **Исправления ошибок и улучшения**
  ...
  + **azureml-core**
    ...
    + Переназначенный `grant_workspace_access` параметр при регистрации хранилищ данных. Задайте для него `True` доступ к данным из виртуальной сети из машинное обучение Studio.
      [Подробнее](https://docs.microsoft.com/azure/machine-learning/how-to-enable-studio-virtual-network)
  ...
  + **azureml-pipeline-core**
    + Пакет SDK для поддержки типа Синапсекомпуте и Синапсеспаркстеп. Клиенты могут запускать эксперименты и конвейеры в пуле синапсе Spark.
  + **azureml-pipeline-steps**
    + Пакет SDK для поддержки типа Синапсекомпуте и Синапсеспаркстеп. Клиенты могут запускать эксперименты и конвейеры в пуле синапсе Spark.
  + **azureml-синапсе**
    + Добавьте синапсе Magic и Спаркмонитор, чтобы разрешить пользователю отправлять задание Сянпсе и просмотреть ход выполнения задания в записной книжке.
...
+ **Новые функции**
  ...
  + Рабочая область: добавлен `hbi_workspace` флаг для рабочих областей с конфиденциальными данными, которые обеспечивают дальнейшее шифрование и отключает расширенную диагностику для рабочих областей. Мы также добавили поддержку для добавления собственных ключей для связанного экземпляра Cosmos DB, указав `cmk_keyvault` `resource_cmk_uri` Параметры и при создании рабочей области, которая создает экземпляр Cosmos DB в подписке при подготовке рабочей области. [Дополнительные сведения см. здесь.](./concept-data-encryption.md#azure-cosmos-db)
