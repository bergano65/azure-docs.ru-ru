...
1. В диалоговом окне **Импорт репозитория git** выберите **Git** в качестве **типа источника**и введите URL-адрес для репозитория общего шаблона группы в поле **URL-адрес клона**. URL-адрес — *https: \/ @ no__t-2 @ no__t-3server name >/\<organization name >/_git/\<repository name >* . Пример: *https: \//dev. Azure. com/датасЦиенцеунит/GroupCommon/_git/GroupProjectTemplate*. 
