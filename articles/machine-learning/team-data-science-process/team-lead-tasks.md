...
description: Структура задач, которые должен выполнить руководитель команды в командном проекте обработки и анализа данных.
author: marktab
manager: cgronlun
editor: cgronlun
...
>[AZURE.NOTE] Задачи в блоках 1 и 2 необходимо выполнить, если вы используете Azure DevOps в качестве платформы размещения кода и хотите, чтобы для команды был выделен отдельный командный проект. После выполнения этих задач все репозитории команды можно создавать в этом проекте. 
...
1. Создание **проекта** в Azure DevOps Services группы и двух командных репозиториев в проекте:
...
- **R1**: репозиторий **GroupProjectTemplate** в Git, который менеджер группы настраивает на сервере группы Azure DevOps.
...
- **D3**: локальный каталог, клонированный из R3, настроенный и скопированный обратно в R3.
- **D4**: локальный каталог, клонированный из R4, настроенный и скопированный обратно в R4.

Имена репозиториев и каталогов в этом руководстве указаны с учетом того, что вы хотите создать отдельный проект для своей команды из группы обработки и анализа данных. Но, как руководителю команды, вам доступны другие варианты.

- Вся группа может решить создать единый проект. Затем все проекты из всех групп обработки и анализа данных будут помещены в один проект. Чтобы реализовать такой сценарий, попросите администратора git выполнить следующие инструкции для создания отдельного проекта. Этот сценарий стоит применять в таких случаях:
...
- Команды могут использовать специальные шаблоны или программы для проекта, над которым работает вся группа. В этом случае руководители команд должны создать репозитории шаблонов проекта и/или репозитории программ команды в одном проекте. Назовите эти репозитории *<Имя_команды\>ProjectTemplate* и *< Имя_команды\>Utilities*, например: *TeamJohnProjectTemplate*и *TeamJohnUtilities*. 
...
- Менеджер группы должен настроить **сервер Azure DevOps Services группы** (или учетную запись группы на другой платформе для размещения кода).
...
## <a name="1-create-a-project-and-repositories"></a>1. Создание проекта и репозиториев

Выполните эти действия, если вы используете Azure DevOps в качестве платформы размещения кода для управления версиями и совместной работы. Выполнив инструкции этого раздела, вы создадите три артефакта в Azure DevOps Services своей группы:
...
- Перейдите на домашнюю страницу Azure DevOps Services группы по URL-адресу `https://<Azure DevOps Services Name\>.visualstudio.com`. 
- Нажмите кнопку **Создать**, чтобы создать проект. 

    ![2](./media/team-lead-tasks/team-leads-2-create-new-team.png)

- В окне создания проекта введите имя проекта (в этом примере это **MyTeam**). Для параметра **Шаблон процесса** выберите **Гибкий**, а для параметра **Управление версиями** — **Git**. 

    ![3](./media/team-lead-tasks/team-leads-3-create-new-team-2.png)

- Нажмите кнопку **Создать проект**. Создание проекта **MyTeam** займет меньше одной минуты. 
...
- На странице репозитория Git проекта щелкните стрелку вниз рядом с именем репозитория **MyTeam** и выберите **Управление репозиториями...**

    ![6](./media/team-lead-tasks/team-leads-6-rename-team-project-repo.png)

- На вкладке **Управление версиями** панели управления проекта выберите репозиторий **MyTeam**, а затем выберите **Переименовать репозиторий...** 
...
- Чтобы создать репозиторий *<имя_вашей_команды\>Служебные программы* в проекте, на вкладке **Управление версиями** панели управления проекта нажмите кнопку **Новый репозиторий...**  
...
- Убедитесь, что вы видите два новых репозитория Git, созданных в проекте **MyTeam**. В данном примере: 
...
## <a name="2-seed-your-projecttemplate-and-teamutilities-repositories"></a>2. Заполнение содержимым репозиториев ProjectTemplate и TeamUtilities
...
- Передайте содержимое локального каталога в репозитории команды: содержимое D3 и D4 добавляется в команду R3 и R4.


### <a name="initialize-the-team-repositories"></a>Инициализация репозиториев команды

На этом шаге вы инициализируете репозиторий шаблона проекта из репозитория шаблона проекта группы:
...
Эти команды клонируют репозиторий **GroupProjectTemplate** (R1) в Azure DevOps Services группы в локальный каталог в папке **GitRepos\GroupCommon** на локальном компьютере. После клонирования каталог **GroupProjectTemplate** (D1) создается в каталоге **GitRepos\GroupCommon**. Предполагается, что менеджер группы создал проект **GroupCommon**, и репозиторий **GroupProjectTemplate** находится в этом проекте. 


### <a name="clone-your-team-repositories-into-local-directories"></a>Клонирование репозиториев команды в локальные каталоги

Эти команды клонируют ваши репозитории **MyTeamProjectTemplate** (R3) и **MyTeamUtilities** (R4) в проекте **MyTeam** в Azure DevOps Services группы в каталоги **MyTeamProjectTemplate** (D3) и **MyTeamUtilities** (D4) в папке **GitRepos\MyTeam** на локальном компьютере. 
...
После клонирования в каталоге **GitRepos\MyTeam** будут созданы два каталога: **MyTeamProjectTemplate** (D3) и **MyTeamUtilities** (D4). Предполагается, что ваши репозитории программ и шаблонов проекта называются **MyTeamProjectTemplate** и **MyTeamUtilities**. 

### <a name="copy-the-group-project-template-content-to-the-local-project-template-directory"></a>Копирование содержимого шаблона проекта в локальный каталог шаблонов проекта группы
...
### <a name="customize-your-project-template-or-team-utilities-optional"></a>Настройка шаблонов проекта или программ команды (необязательно)
...
Файлы в репозитории MyTeamProjectTemplate служб Azure DevOps Services вашей группы почти мгновенно синхронизируются при выполнении этого сценария.
...
Рекомендуется сохранить этот текстовый файл в репозитории ProjectTemplate. Рекомендуется поместить его в каталог **Docs\DataDictionaries**. Таким образом к этому ресурсу данных смогут обращаться все проекты вашей команды. 
...
**Как вручную ввести сведения о хранилище файлов Azure.** Если у вас нет сведений о текстовом файле в хранилище файлов Azure, следуйте инструкциям на изображении экрана ниже, чтобы ввести необходимую информацию о подписке, учетной записи хранения и хранилище файлов Azure.
...
На домашней странице Azure DevOps Services группы щелкните **значок шестеренки** рядом с именем пользователя в правом верхнем углу. Затем выберите вкладку **Безопасность**. Здесь вы можете добавлять участников команды с разными разрешениями.
