...
Предприятиям требуется, чтобы важное оборудование работало и использовалось максимально эффективно для обеспечения окупаемости капиталовложений. Это могут быть любые устройства — от авиадвигателей, турбин, лифтов или промышленных систем охлаждения, которые стоят миллионы, до бытовых приборов, таких как копировальные аппараты, кофеварки или кулеры.
- По умолчанию большинство предприятий полагаются на _восстановительное обслуживание_, где детали заменяются по мере выхода из строя. Корректирующее обслуживание гарантирует, что ресурс деталей используется в полной мере (до полной непригодности), но при этом бизнес несет убытки от простоя, трудозатрат и незапланированного технического обслуживания (в нерабочее время или в неудобных расположениях).
...
- Цель _прогнозного обслуживания_ заключается в оптимизации баланса между восстановительным и профилактическим обслуживанием за счет предоставления возможности _моментальной_ замены компонентов. При этом заменяются только те компоненты, срок службы которых подходит к концу. Продлевая срок службы компонентов (по сравнению с профилактическим обслуживанием) и сокращая незапланированные затраты на техническое обслуживание и оплату труда (по сравнению с корректирующим обслуживанием), предприятия могут обеспечить экономию средств и конкурентные преимущества.
...
|:---------|---------------------------|
|_Задержка и отмена рейсов_ | Информацию о полете можно найти в маршруте полета и постраничных журналах. Данные о подучастках рейсов включают в себя такие сведения о маршрутизации, как Дата отправления и прибытия, время, аэропорт, лайоверс и т. д Журнал страниц содержит серию кодов ошибок и обслуживания, записанных сотрудниками технического обслуживания.|
...
|_Сбой банкомата_ | Показания датчиков для каждой транзакции (прием наличных средств или чека) и выдачи наличных. Сведения об измерении зазоров между заметками, толщина заметок, расстояние прибытия, проверка атрибутов и т. д. Записи об обслуживании, предоставляющие коды ошибок, сведения об исправлении, время последнего заполнения средства выручки.|
|_Сбои ветроэлектрической установки_ | Датчики отслеживают ветроэлектрической такие условия, как температура, направление обмотки, формирование питания, скорость генератора и т. д. Данные собираются из нескольких обмотк турбины из ферм ветер, расположенных в различных регионах. Как правило, каждая установка передает несколько показаний измерений с фиксированным временным интервалом.|
|_Сбои автоматического выключателя_ | Журналы технического обслуживания, которые включают в себя ремонтные, профилактические и систематические мероприятия. Операционные данные, которые включают в себя автоматические и ручные команды, отправленные на автоматические выключатели, например для включения и отключения. Метаданные устройства, такие как Дата изготовления, расположение, модель и т. д. Спецификации автоматического выключения, такие как уровни напряжения, географическое положение и окружающие условия.|
...
|_Поломка колес_ | Данные датчика, измеряющие ускорение колесика, экземпляры торможения, движущие расстояние, скорость и т. д. Статическая информация о колесах, таких как производитель, Дата изготовления. Данные о сбоях выводятся из базы данных заказа запчастей, которые отслеживают даты и количество заказов.|
...
![Рис. 1. Признаки для скользящих агрегатов](./media/cortana-analytics-playbook-predictive-maintenance/rolling-aggregate-features.png)

Рис. 1. Признаки для скользящих агрегатов
...
![Рис. 4. 4 Создание меток для регрессии](./media/cortana-analytics-playbook-predictive-maintenance/labelling-for-regression.png)

Рис. 4. 4 Создание меток для регрессии
...
![Рис. 5. Метки прогнозирования времени сбоя для многоклассовой классификации](./media/cortana-analytics-playbook-predictive-maintenance/labelling-for-multiclass-classification-for-failure-time-prediction.png)
...
![Рис. 6. Метки прогнозирования основной причины для многоклассовой классификации](./media/cortana-analytics-playbook-predictive-maintenance/labelling-for-multiclass-classification-for-root-cause-prediction.png)
...
| [Интерактивные семинары по Microsoft AI YouTube](https://www.youtube.com/watch?v=NvrH7_KKzoM&t=4s) | Общедоступные |
