...
description: Изучите данные о процессах и создайте функции, используя Python или S'L в виртуальной машине сервера S'L на Azure.
...
В этом документе описывается изучение данных и создание характеристик для данных, хранящихся в виртуальной машине SQL Server в Azure. Эта цель может быть завершена путем обработки данных с помощью S'L или с помощью языка программирования, такого как Python.
...
2. [Поколение функций](#sql-featuregen)
...
### <a name="feature-generation"></a><a name="sql-featuregen"></a>Поколение функций
В этом разделе мы опишем способы создания характеристик с помощью SQL:  

1. [Отсчет на основе поколения функций](#sql-countfeature)
...
### <a name="count-based-feature-generation"></a><a name="sql-countfeature"></a>Отсчет на основе поколения функций
В следующих примерах показаны два способа создания количественных характеристик. В первом способе используется условная сумма, а во втором — предложение where. Эти результаты могут быть соединены с исходной таблицей (с использованием первичных ключевых столбцов) для того, чтобы иметь функции подсчета наряду с исходными данными.
...
* Цифра единиц (один десятичный градус) дает положение с точностью до 111 километров (60 морских миль, или около 69 миль). Он может сказать вам примерно, в каком состоянии, стране или регионе вы находитесь.
...
Сведения о местоположении можно характеризовать следующим образом, выделяя информацию о регионе, местоположении и городе. Вы также можете позвонить в конечную точку REST, такую как API Bing Maps, доступный в [«Найти место по пункту»,](https://msdn.microsoft.com/library/ff701710.aspx) чтобы получить информацию о регионе/районе.
...
Использование языка Python для просмотра данных и создания характеристик, когда данные находятся в SQL Server, подобно обработке данных в большом двоичном объекте Azure с использованием Python, как описано в статье [Обработка больших двоичных данных Azure с применением методов расширенного анализа](data-blob.md). Загрузите данные из базы данных в рамку данных панд для дополнительной обработки. В этом разделе документирован процесс подключения к базе данных и загрузки данных во фрейм данных.
...
[Библиотека Pandas](https://pandas.pydata.org/) в Python предоставляет богатый набор структур данных и инструментов анализа данных для манипуляций с данными для программирования Python. Приведенный ниже код считывает результаты, возвращенные из базы данных SQL Server, во фрейм данных Pandas:
