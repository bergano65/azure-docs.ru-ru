...
description: Узнайте, как перемещать данные в или из хранилища Azure Blob, используя пакет функций интеграции серверов Для Azure.
...
После того, как клиенты переместили данные в облако, они смогут получить доступ к своим данным из любого сервиса Azure, чтобы использовать всю мощь набора технологий Azure. Впоследствии эти данные могут использоваться, например, в Машинном обучении Azure или в кластере HDInsight.

Примеры использования этих ресурсов Azure приведены в пошаговых пошаговых отвах [на s-L](sql-walkthrough.md) и [HDInsight.](hive-walkthrough.md)
...
Для выполнения задач, описанных в этой статье, необходимо настроить подписку Azure и учетную запись хранения Azure. Для загрузки или загрузки данных необходимо имя учетной записи и ключ учетной записи Хранилища Azure.
...
* Для получения инструкций по созданию **учетной записи хранилища** и получению учетной записи и ключевой информации [см.](../../storage/common/storage-create-storage-account.md)
...
* **Пакет интеграционных услуг для Azure 2014 или 2016**— эти разъемы могут быть загружены, соответственно, со страниц [интеграционных служб S'L Server 2014](https://www.microsoft.com/download/details.aspx?id=47366) и на страницах [интеграционных служб S'L Server 2016.](https://www.microsoft.com/download/details.aspx?id=49492)
...
В приведенном здесь примере используется общедоступный набор данных [Поездки в такси по Нью-Йорку](https://www.andresmh.com/nyctaxitrips/) . Набор данных состоит из информации об около 173 миллионах поездок в такси в Нью-Йорке в 2013 году. В нем используется два типа данных: сведения о поездках и тарифах. Как есть файл для каждого месяца, у нас есть 24 файлов, каждый из которых составляет около 2 ГБ несжатые.
...
| Поле | Описание |
| --- | --- |
| **AzureStorageConnection** |Уотрежьте существующий менеджер подключения к хранению Azure или создай новый, отображающий учетную запись Хранилища Azure, которая указывает на то, где размещаются файлы blob. |
| **BlobContainer** |Увеяет название контейнера blob, вмещаемом в загруженные файлы, как капли. |
...
| **Имени файла** |Указывает имя фильтра для выбора файлов с указанным шаблоном имени. Например, MySheet\*.xls\* включает в себя файлы MySheet001.xls и MySheetABC.xlsx. |
