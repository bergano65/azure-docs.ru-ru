...
В этом руководстве описывается шаг 1. Создайте репозиторий проекта и шаг 2. Начальное значение репозитория проекта из репозитория команды ProjectTemplate. 

Для шага 3: Создайте рабочий элемент функции для проекта и шаг 4. Добавление историй для этапов проекта см. в статье [гибкая разработка проектов обработки и анализа данных](agile-development.md).

Для шага 5: Создание и Настройка ресурсов хранения, анализа и общего ресурса (при необходимости) см. в разделе [Создание данных группы и ресурсов аналитики](team-lead-tasks.md#create-team-data-and-analytics-resources).

Для шага 6: Настройка управления безопасностью репозитория проекта см. в разделе [Добавление членов команды и настройка разрешений](team-lead-tasks.md#add-team-members-and-configure-permissions).
...
1. Перейдите на страницу **сводки** проекта команды по адресу *https: \/ @ no__t-3 @ no__t-4server name >/\<organization name >/\<team имя >* , например **https: \//dev. Azure. com/датасЦиенцеунит/myTeam.** Выберите **репозиториев** в левой области навигации. 
...
1. В диалоговом окне **Импорт репозитория git** выберите **Git** в качестве **типа источника**и введите URL-адрес репозитория **теамтемплате** в разделе **URL-адрес клона**. URL-адрес имеет значение *https: \/ @ no__t-2 @ no__t-3server name >/\<organization name >/\<team name >/_git/\<team имя репозитория template >* . Пример: **https: \//dev. Azure. com/датасЦиенцеунит/myTeam/_git/теамтемплате**. 
