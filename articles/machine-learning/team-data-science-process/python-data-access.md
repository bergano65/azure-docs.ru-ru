...
## <a name="datasetAccess"></a>Использование фрагментов кода для доступа к наборам данных
...
После запуска эксперимента в классической версии Машинное обучение Studio можно получить доступ к промежуточным наборам данных из выходных узлов модулей. Промежуточные наборы данных — это данные, создаваемые и используемые на промежуточных шагах после запуска инструмента моделирования.
...
Рабочая область — это точка входа для клиентской библиотеки Python. Укажите класс `Workspace` с ИДЕНТИФИКАТОРом рабочей области и маркером авторизации, чтобы создать экземпляр:
