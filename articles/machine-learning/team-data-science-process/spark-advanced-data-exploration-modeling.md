---
title: Расширенное исследование и моделирование данных с помощью Spark — процесс обработки и анализа данных группы
...
**Оптимизация параметров** — это проблема выбора набора параметров для алгоритма обучения, обычно с целью оптимизации показателя производительности алгоритма на независимом наборе данных. **Гиперпараметры** — это значения, которые должны быть указаны за пределами процедуры обучения модели. Предположения относительно этих значений могут повлиять на гибкость и точность моделей. Деревья принятия решений содержат гиперпараметры, такие как требуемая глубина и число листьев в дереве. Для методов опорных векторов (SVM) необходимо задать условие штрафа за неправильную классификацию. 

Распространенный способ выполнения оптимизации гиперпараметров, который используется здесь, — поиск в сетке или **перебор параметров**. Этот поиск проходит через подмножество пространства параметров для алгоритма обучения. Перекрестная проверка может предоставить метрику производительности, чтобы отсортировать оптимальные результаты, выданные алгоритмом поиска в сетке. Использование перекрестной проверки с перебором гиперпараметров помогает ограничить такие проблемы, как лжевзаимосвязи модели с обучающими данными, чтобы модель сохраняла возможность применения к общему набору данных, из которого извлекаются данные для обучения.
...
* Высокоприоритетные [деревья](https://spark.apache.org/docs/latest/ml-classification-regression.html#gradient-boosted-trees-gbts) (GBT) — это это совокупности деревьев принятия решений. GBT обучение деревьев принятия решений для сворачивания функции потери. GBT используется для регрессии и классификации и может работать с характеристиками категории, не требует масштабирования компонентов и позволяет собирать нелинейные и взаимодействия. Кроме того, его можно использовать для создания модели мультиклассовой классификации.
...
* **%% имя переменной SQL \<-o>** Выполняет запрос Hive к sqlContext. Если передан параметр -o, результат запроса сохраняется в контексте %%local Python в формате Pandas DataFrame.
...
* Создание случайной подвыборки данных и разбиение их на обучающие и проверочные наборы
...
### <a name="create-a-random-subsampling-of-the-data-and-split-it-into-training-and-testing-sets"></a>Создание случайной подвыборки данных и разбиение их на обучающие и проверочные наборы
С помощью кода в этом разделе создается случайная выборка данных (25 % всех данных). Хотя из-за небольшого размера используемого набора данных, этот шаг выполнять необязательно, мы все равно покажем, как создавать выборку данных. Это поможет вам в случае возникновения проблем. Когда выборки велики, выборка может сэкономить значительное время во время обучения моделей. Затем мы разобьем выборку данных на наборы для обучения (75 %) и тестирования (25 %), которые будут использоваться для двоичной классификации и регрессии.
...
Время, затраченное на выполнение приведенной выше ячейки: 0,31 секунд
...
Время, затраченное на выполнение приведенной выше ячейки: 0,13 секунд
...
1. Использование **универсального** пользовательского кода, который может применяться к любому алгоритму в MLlib и к любым наборам параметров в алгоритме. 
2. С использованием **функции конвейера pySpark CrossValidator**. CrossValidator имеет несколько ограничений для 1.5.0 Spark: 
   
   * Модели конвейера невозможно сохранить или сохранить для будущего использования.
...
> [!NOTE] 
> Перекрестная проверка не используется с тремя моделями регрессии, указанными в этом разделе. Это было подробно описано в отношении моделей логистической регрессии. Пример использования перекрестной проверки с эластичной сетью для линейной регрессии показан в приложении к этой статье.
...
* * Путь к файлам модели, который будет использоваться в записной книжке для потребления. ** Чтобы использовать и оценивать независимые наборы данных, необходимо скопировать и вставить имена файлов в записную книжку данных об использовании.
