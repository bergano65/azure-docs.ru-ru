---
title: Создание функций для данных в кластере Azure HDInsight Hadoop - Процесс получения данных team Data
...
1. [Поколение функций на основе частоты](#hive-frequencyfeature)
...
5. [Рассчитать расстояние между координатами GPS](#hive-gpsdistance)

### <a name="frequency-based-feature-generation"></a><a name="hive-frequencyfeature"></a>Генерация функций на основе частот
...
### <a name="extract-features-from-datetime-fields"></a><a name="hive-datefeatures"></a>Извлекайте особенности из полей времени даты
...
Этот запрос Hive предполагает, что * \<поле времени>дат* находится в формате времени по умолчанию.
...
В этом запросе, если * \<поле времени даты>* имеет шаблон, как *03/26/2015 12:04:39* `'MM/dd/yyyy HH:mm:ss'`, * \<шаблон поля> даты "должен* быть . Чтобы проверить шаблон поля даты и времени, можно выполнить такую команду:
...
### <a name="extract-features-from-text-fields"></a><a name="hive-textfeatures"></a>Извлечение функций из текстовых полей
...
Математические уравнения, с помощью которых рассчитывается расстояние между двумя координатами GPS, можно найти на сайте <a href="http://www.movable-type.co.uk/scripts/latlong.html" target="_blank">Movable Type Scripts</a> (создатель веб-сайта — Петер Лапису (Peter Lapisu)). В этом Javascript `toRad()` функция находится *всего lat_or_lon*pi/180, который преобразует степени в радианов. Здесь *lat_or_lon* — это широта или долгота. Так как в Hive нет функции `atan2`, но есть функция `atan`, функция `atan2` в приведенном выше запросе Hive реализована функцией `atan` с помощью определения из <a href="https://en.wikipedia.org/wiki/Atan2" target="_blank">Википедии</a>.
...
1. **Куча Java**: при работе с запросами на объединение крупных наборов данных или обработку длинных записей часто возникает ошибка, указывающая на то, что **в куче не хватает памяти**. Чтобы избежать этой ошибки, задайте нужные значения для параметров *mapreduce.map.java.opts* и *mapreduce.task.io.sort.mb*. Например:
    ...
    Этот параметр выделяет 4-ГБ памяти на пространство Ява кучи, а также делает сортировку более эффективной, выделяя больше памяти для него. Мы рекомендуем заняться настройкой того, сколько памяти куда выделяется, если произошли сбои задач, связанные с размером кучи.
...
2. **Оптимизация операций объединения в Hive**: так как операции объединения на платформе Map/Reduce обычно выполняются на этапе сокращения, в некоторых случаях эффективность этих операций можно существенно повысить, запланировав объединения на этапе сопоставления (они также называются "объединениями при сопоставлении"). Установите эту опцию:
   
       set hive.auto.convert.join=true;

3. **Определение количества модулей сопоставления в Hive**: хотя Hadoop позволяет задать число модулей сокращения, число модулей сопоставления, как правило, задать нельзя. Трюк, который позволяет некоторую степень контроля над этим числом, состоит в том, чтобы выбрать переменные Hadoop *mapred.min.split.size* и *mapred.max.split.size,* поскольку размер каждой задачи карты определяется следующим:
...
4. Ниже приведены некоторые другие **дополнительные возможности** по оптимизации производительности Hive. Эти параметры позволяют настроить память, выделенную для картирования и уменьшить задачи, и могут быть полезны в настройке производительности. Не забывайте, что значение *mapreduce.reduce.memory.mb* не может превышать объем физической памяти каждого рабочего узла в кластере Hadoop.
