...
Данные о поездках такси Нью-Йорка содержатся в сжатых CSV-файлах размером около 20 ГБ (примерно 48 ГБ в распакованном виде), которые включают в себя сведения о более 173 млн отдельных поездок и платежах за каждую поездку. Каждая запись поездки включает в себя пикап и высадки местах и раз, анонимизированный взломать (водитель) номер лицензии, и медальон (уникальный id такси) номер. Данные включают в себя все поездки за 2013 год и предоставляются в виде следующих двух наборов данных за каждый месяц:
...
**Подключитесь к аналитике Azure Synapse с помощью Visual Studio.** Для получения инструкций смотрите шаги 1 & 2 в [Connect to s'L Analytics в Azure Synapse Analytics.](../../synapse-analytics/sql/connect-overview.md)
...
> [!NOTE]
> Во избежание конфликтов с именами схем с теми, которые уже существуют в вашей Synapse Analytics Azure Azure, при чтении параметров непосредственно из файла sldW.conf к имени схемы добавляется 3-значное случайное число в название схемы из файла S'LDW.conf в качестве имени схемы по умолчанию для каждого запуска. Сценарий PowerShell может запросить имя схемы. Имя указывается по усмотрению пользователя.
>
>
...
> [!NOTE]
> В зависимости от географического положения вашей учетной записи хранения капли, процесс копирования данных из публичного кабы на ваш личный счет хранения может занять около 15 минут или даже больше, а процесс загрузки данных с вашей учетной записи хранилища в ваш Azure Azure Synapse Analytics может занять 20 минут или дольше.
>
>
...
Этот скрипт PowerShell также подключает информацию Azure Synapse Analytics в файлы примера исследования данных SQLDW_Explorations.sql, SQLDW_Explorations.ipynb и SQLDW_Explorations_Scripts.py, чтобы эти три файла были готовы к мгновенной опробованию после завершения сценария PowerShell.
...
Пример бинарной классификации эксперимента чтения данных непосредственно из базы данных Azure Synapse Analytics находится в рисунке ниже (не забудьте заменить имена таблицnyctaxi_trip и nyctaxi_fare имя схемы и таблицы имена, которые вы использовали в пошаговом пошаговом пошаговом походе). Подобные эксперименты можно сконструировать для задач мультиклассовой классификации и регрессии.
