...
1. **Двоичная классификация**: для прогнозирования того, оплачивается ли TIP для поездки, то есть *Tip\_сумма* , превышающая $0, является положительным примером, а *\_ный уровень* $0 — отрицательный пример.
...
**Установите Visual Studio и SQL Server Data Tools.** Инструкции см. в статье [Приступая к работе с Visual Studio 2019 для хранилища данных SQL](../../sql-data-warehouse/sql-data-warehouse-install-visual-studio.md).

**Подключитесь к Azure синапсе Analytics с помощью Visual Studio.** Инструкции см. в шагах 1 & 2 в статье [Подключение к хранилищу данных SQL Azure](../../sql-data-warehouse/sql-data-warehouse-connect-overview.md).
...
## <a name="getdata"></a>Загрузка данных в Azure синапсе Analytics
...
> [!NOTE]
> Чтобы избежать конфликта имен схем с теми, которые уже существуют в Azure Azure синапсе Analytics, при чтении параметров непосредственно из файла SQLDW. conf в имя схемы из файла SQLDW. conf добавляется случайное число из 3 цифр в качестве схемы по умолчанию. имя для каждого запуска. Сценарий PowerShell может запросить имя схемы. Имя указывается по усмотрению пользователя.
>
>
...
> [!NOTE]
> В зависимости от географического расположения учетной записи частного хранилища BLOB-объектов процесс копирования данных из общедоступного большого двоичного объекта в закрытую учетную запись хранения может занять около 15 минут или даже дольше, а также процесса загрузки данных из вашей учетной записи хранения в Azure. Azure синапсе Analytics может занять 20 минут или более.
>
>
...
Этот сценарий PowerShell также подключается к сведениям Azure синапсе Analytics в файлах примеров для просмотра данных SQLDW_Explorations. SQL, SQLDW_Explorations. ipynb и SQLDW_Explorations_Scripts. корректировка, чтобы эти три файла были готовы к попыток. сразу после завершения сценария PowerShell.

После успешного выполнения вы увидите экран, аналогичный этому:

![Результат успешного выполнения скрипта][20]

## <a name="dbexplore"></a>Исследование данных и проектирование характеристик в Azure синапсе Analytics
...
Пример эксперимента по двоичной классификации, считывающий данные непосредственно из базы данных Azure синапсе Analytics, приведен на рисунке ниже (не забудьте заменить имена таблиц nyctaxi_trip и nyctaxi_fare по имени схемы и именам таблиц, использованным в Пошаговое руководство). Подобные эксперименты можно сконструировать для задач мультиклассовой классификации и регрессии.
...
После создания эксперимента оценки Проверьте результаты и внесите необходимые изменения. Типичная корректировка заключается в замене входного набора данных или запроса на другой, исключающий поля меток, поскольку эти поля меток не будут сопоставляться со схемой при вызове службы. Также рекомендуется уменьшить размер входного набора данных и (или) запроса на несколько записей, чтобы указать входную схему. Для порта вывода обычно исключаются все поля ввода и включаются только оцененные **метки** и оцененные **вероятности** в выходных данных с помощью модуля [Выбор столбцов в наборе данных][select-columns] .
