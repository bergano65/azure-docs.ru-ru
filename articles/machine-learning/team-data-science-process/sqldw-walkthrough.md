...
Данные о поездках такси Нью-Йорка содержатся в сжатых CSV-файлах размером около 20 ГБ (примерно 48 ГБ в несжатом виде), которые включают в себя сведения о более 173 млн отдельных поездок и платежах за каждую поездку. В каждой записи о поездке есть следующие данные: расположение пунктов посадки и высадки, время посадки и высадки, анонимизированный номер лицензии водителя, номер медальона (уникальный идентификатор такси). Данные включают в себя все поездки за 2013 год и предоставляются в виде следующих двух наборов данных за каждый месяц:
...
2. Сохранить выбранные и сконструированные данные, которые планируется использовать для создания модели в новой таблице хранилища данных SQL, и использовать новую таблицу в модуле [Import Data][import-data] в машинное обучение Azure. Это уже сделано на одном из предыдущих шагов сценария PowerShell. Данные можно считать прямо из этой таблицы в модуле "Импорт данных".
...
При создании расширенных аналитических решений на Машинное обучение Azure с большими данными рекомендуется использовать следующую последовательность:
...
В этом упражнении мы уже изучили и разработали данные в хранилище данных SQL и решили принять выбор размера в Машинное обучение Azure Studio (классическая модель). Вот процедура, позволяющая создать одну или несколько моделей прогнозирования:

1. Данные можно получить в Машинное обучение Azure Studio (классическая модель) с помощью модуля [Импорт данных][import-data] , доступного в разделе **входные и выходные данные** . Дополнительные сведения см. на странице справочника по модулю [импорта данных][import-data] .
