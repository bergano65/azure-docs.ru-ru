...
Сведения о расположении можно включить в признаки, выделяя информацию о регионе, расположении и городе. Один раз может также вызвать конечную точку RESTFUL, например API Карт Bing ( `https://msdn.microsoft.com/library/ff701710.aspx` см. статью, чтобы получить сведения о регионе или районе).
