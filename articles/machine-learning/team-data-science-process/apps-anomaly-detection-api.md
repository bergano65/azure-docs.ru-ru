...
Чтобы использовать API-интерфейс, необходимо развернуть его в подписке Azure. Он будет размещаться там в роли веб-службы машинного обучения Azure.  Это можно сделать в [коллекции решений ИИ Azure](https://gallery.cortanaintelligence.com/MachineLearningAPI/Anomaly-Detection-2).  В результате в подписке Azure будут развернуты две веб-службы Машинного обучения Azure (и соответствующие ресурсы). Одна из них используется для обнаружения аномалий и сезонности, а вторая работает без обнаружения сезонности.  Когда завершится развертывание, этими интерфейсами API можно будет управлять на странице [веб-служб Машинного обучения Azure](https://services.azureml.net/webservices/).  На этой странице вы можете найти расположения конечных точек, ключи API и примеры кода для вызова API.  Более подробные инструкции см. [здесь](https://docs.microsoft.com/azure/machine-learning/machine-learning-manage-new-webservice).
...
На рисунке ниже показан пример аномалий, которые может обнаружить API оценки. В этом временном ряду есть 2 отчетливых изменения уровня и 3 пика. Красными точками обозначено время обнаружения изменений уровня, а черными точками — обнаруженные пики.
![API оценки][1]
...
| Средство обнаружения медленно развивающихся тенденций |Средство обнаружения медленно развивающихся тенденций |Обнаружение медленно развивающихся положительных тенденций согласно чувствительности набора |*trenddetector.sensitivity:* пороговое значение оценки средства обнаружения (по умолчанию: 3.25, 3.25–5 — допустимый диапазон для выбора значения; чем выше, тем меньшая чувствительность) |tscore: число с плавающей точкой, которое используется как показатель аномалий в тенденции |
| Средства обнаружения изменений уровня | Средство обнаружения двунаправленных изменений уровня |Обнаружение восходящих и нисходящих изменений уровня согласно чувствительности набора |*bileveldetector.sensitivity:* пороговое значение оценки средства обнаружения (по умолчанию: 3.25, 3.25–5 — допустимый диапазон для выбора значения; чем выше, тем меньшая чувствительность) |rpscore: число с плавающей точкой, которое используется как показатель аномалий в восходящем и нисходящем изменении уровня |
...
| detectors.spikesdips | Указывает, нужно ли выявлять только пики, только спады, или и то, и другое |Обе |перечисляемый |Both, Spikes, Dips |Обе |
...
| Data |Значения из необработанных данных или агрегированные (и/или) добавленные данные при применении агрегирования и/или добавления отсутствующих данных |
...
| tspikedetector.sensitivity |Чувствительность для средства обнаружения пиков TSpike |3 |integer |1–10 |3,5–5 (чем меньше значение, тем выше чувствительность) |
| zspikedetector.sensitivity |Чувствительность для средства обнаружения пиков ZSpike |3 |integer |1–10 |3,5–5 (чем меньше значение, тем выше чувствительность) |
| seasonality.enable |Указывает, нужно ли выполнять анализ сезонности |true |Логический |true, false |Зависит от временных рядов |
