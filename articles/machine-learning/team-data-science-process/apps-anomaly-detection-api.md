...
Чтобы вызвать API, вам нужно знать расположение его конечной точки и ключ API.  Эти два требования, а также пример кода для вызова API, доступны на странице [веб-служб машинное обучение Azure Studio (классическая модель)](https://services.azureml.net/webservices/) .  Чтобы получить эти сведения, перейдите к нужному API и щелкните вкладку "Использование".  API можно вызвать как API Swagger (то есть с параметром `format=swagger`URL-адреса) или как API-интерфейс, не являющийся Swagger (т. е. без `format` параметра URL-адреса).  В приведенных примерах кода используется формат Swagger.  Ниже представлен пример запроса и ответа без поддержки Swagger.  В этих примерах обращение выполняется к конечной точке с поддержкой сезонности.  Для другой конечной точки все выглядит точно так же.
...
Чтобы увидеть `ColumnNames` поле, необходимо включить `details=true` в запрос в качестве параметра URL-адреса.  В следующих таблицах описано назначение каждого из этих полей.
...
| Входные параметры | Описание | Параметр по умолчанию | Type | Допустимый диапазон значений | Рекомендуемый диапазон значений |
| --- | --- | --- | --- | --- | --- |
