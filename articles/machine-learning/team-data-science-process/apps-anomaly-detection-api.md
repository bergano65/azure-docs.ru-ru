...
ms.locfileid: "76721886"
---
# <a name="machine-learning-anomaly-detection-api"></a>API обнаружения аномалий в машинном обучении

> [!NOTE]
> Этот элемент находится в состоянии обслуживания. Мы рекомендуем использовать [службу API детектора аномалий](https://azure.microsoft.com/services/cognitive-services/anomaly-detector/) на основе коллекции машинное обучение алгоритмов в Azure Cognitive Services, чтобы обнаруживать аномалии для бизнес-, операционных и метрик Интернета вещей.
...
Чтобы использовать API-интерфейс, необходимо развернуть его в подписке Azure. Он будет размещаться там в роли веб-службы машинного обучения Azure.  Это можно сделать в [коллекции решений ИИ Azure](https://gallery.cortanaintelligence.com/MachineLearningAPI/Anomaly-Detection-2).  Это позволит развернуть две Машинное обучение Azure Studio (классические) веб-службы (и связанные с ними ресурсы) в подписке Azure — один для обнаружения аномалий с обнаружением сезонности и один без обнаружения сезонности.  После завершения развертывания вы сможете управлять API на странице [веб-служб машинное обучение Azure Studio (классическая модель)](https://services.azureml.net/webservices/) .  На этой странице вы можете найти расположения конечных точек, ключи API и примеры кода для вызова API.  Более подробные инструкции см. [здесь](https://docs.microsoft.com/azure/machine-learning/machine-learning-manage-new-webservice).

## <a name="scaling-the-api"></a>Масштабирование API
По умолчанию в развертывании будет существовать бесплатный план выставления счетов для разработки и тестирования, включающий 1 000 транзакций в месяц и 2 вычислительных часа в месяц.  Этот план вы можете сменить на любой другой в соответствии с потребностями.  Сведения о ценах на разные планы приводятся [на этой странице](https://azure.microsoft.com/pricing/details/machine-learning/) в разделе "Цены на производственный веб-API".
...
Веб-служба предоставляет API на основе RESTFUL по протоколу HTTPS, который может использоваться различными способами, включая веб-приложение или мобильные приложения, R, Python, Excel и т. д.  Вы отправляете данные временных рядов в эту службу через вызов REST API и выполняете сочетание трех типов аномалий, описанных ниже.

## <a name="calling-the-api"></a>Вызов интерфейса API
Чтобы вызвать API, вам нужно знать расположение его конечной точки и ключ API.  Эти два требования, а также пример кода для вызова API, доступны на странице [веб-служб машинное обучение Azure Studio (классическая модель)](https://services.azureml.net/webservices/) .  Чтобы получить эти сведения, перейдите к нужному API и щелкните вкладку "Использование".  Вы можете вызвать API в качестве API Swagger (то есть с параметром URL-адреса `format=swagger`) или как API, не относящийся к Swagger (т. е. без параметра URL-адреса `format`).  В приведенных примерах кода используется формат Swagger.  Ниже представлен пример запроса и ответа без поддержки Swagger.  В этих примерах обращение выполняется к конечной точке с поддержкой сезонности.  Для другой конечной точки все выглядит точно так же.
...
Чтобы увидеть поле `ColumnNames`, необходимо включить `details=true` в качестве параметра URL-адреса в запрос.  В следующих таблицах описано назначение каждого из этих полей.
...
На рисунке ниже показан пример аномалий, которые может обнаружить API оценки. Этот временный ряд содержит два отдельных изменения уровня и три пиковые пики. Красными точками обозначено время обнаружения изменений уровня, а черными точками — обнаруженные пики.
![API оценки][1]

### <a name="detectors"></a>Средства обнаружения
API обнаружения аномалий поддерживает функции обнаружения в трех широких категориях. Сведения об определенных входных параметрах и выходных данных для каждого средства обнаружения представлены в следующей таблице.

| Категория средства обнаружения | Средство обнаружения | Description | Входные параметры | Выходные данные |
| --- | --- | --- | --- | --- |
...
| Средство обнаружения медленно развивающихся тенденций |Средство обнаружения медленно развивающихся тенденций |Обнаружение медленно развивающихся положительных тенденций согласно чувствительности набора |*тренддетектор. Чувствительность:* порог для оценки детектора (значение по умолчанию: 3,25, 3,25 – 5 — разумный диапазон для выбора). Чем менее чувствительно к минимуму) |tscore: число с плавающей точкой, которое используется как показатель аномалий в тенденции |
| Средства обнаружения изменений уровня | Средство обнаружения двунаправленных изменений уровня |Обнаружение восходящих и нисходящих изменений уровня согласно чувствительности набора |*билевелдетектор. Чувствительность:* порог для оценки детектора (значение по умолчанию: 3,25, 3,25 – 5 — разумный диапазон для выбора). Чем менее чувствительно к минимуму) |rpscore: число с плавающей точкой, которое используется как показатель аномалий в восходящем и нисходящем изменении уровня |

### <a name="parameters"></a>Параметры
Подробные сведения об этих входных параметрах представлены в следующей таблице.

| Входные параметры | Description | Параметр по умолчанию | Тип | Допустимый диапазон значений | Рекомендуемый диапазон значений |
| --- | --- | --- | --- | --- | --- |
| detectors.historywindow |Глубина анализа (количество используемых точек данных) для вычисления показателей аномалий |500 |integer |10–2000 |Зависит от временных рядов |
| detectors.spikesdips | Указывает, нужно ли выявлять только пики, только спады, или и то, и другое |Оба варианта |перечисляемый |Both, Spikes, Dips |Оба варианта |
| bileveldetector.sensitivity |Чувствительность для средства обнаружения двунаправленных изменений уровня. |3.25 |double |Нет |3,25–5 (чем меньше значение, тем больше чувствительность) |
| trenddetector.sensitivity |Чувствительность для средства обнаружения положительных тенденций |3.25 |double |Нет |3,25–5 (чем меньше значение, тем больше чувствительность) |
| tspikedetector.sensitivity |Чувствительность для средства обнаружения пиков TSpike |3 |integer |1–10 |3,5–5 (чем меньше значение, тем выше чувствительность) |
| zspikedetector.sensitivity |Чувствительность для средства обнаружения пиков ZSpike |3 |integer |1–10 |3,5–5 (чем меньше значение, тем выше чувствительность) |
| postprocess.tailRows |Число последних точек данных, которые нужно сохранить в выходных результатах |0 |integer |0 (сохранить все точки данных) или укажите количество точек, которые следует сохранить в результатах |Н/Д |
...
На следующем рисунке показан пример аномалий, обнаруженных в сезонных временных рядах. Временные ряды имеют один пик (первую черную точку), два DIP (вторая черная точка и одна в конце) и одно изменение уровня (красная точка). Как DIP в середине временного ряда, так и изменение уровня определяются только после удаления сезонных компонентов из ряда.
![API сезонности][2]
...
| Входные параметры | Description | Параметр по умолчанию | Тип | Допустимый диапазон значений | Рекомендуемый диапазон значений |
| --- | --- | --- | --- | --- | --- |
| preprocess.aggregationInterval |Интервал агрегации в секундах для агрегирования входных временных рядов |0 (агрегирование не выполняется) |integer |0 — агрегирование пропускается, в противном случае используется значение больше 0 |От 5 минут до 1 дня, зависит от временных рядов |
| preprocess.aggregationFunc |Функция, используемая для агрегирования данных в указанный параметр AggregationInterval |mean |перечисляемый |mean, sum, length |Н/Д |
| preprocess.replaceMissing |Значения, используемые для добавления отсутствующих данных |lkv (последнее известное значение) |перечисляемый |zero, lkv, mean |Н/Д |
| detectors.historywindow |Глубина анализа (количество используемых точек данных) для вычисления показателей аномалий |500 |integer |10–2000 |Зависит от временных рядов |
| detectors.spikesdips | Указывает, нужно ли выявлять только пики, только спады, или и то, и другое |Оба варианта |перечисляемый |Both, Spikes, Dips |Оба варианта |
| bileveldetector.sensitivity |Чувствительность для средства обнаружения двунаправленных изменений уровня. |3.25 |double |Нет |3,25–5 (чем меньше значение, тем больше чувствительность) |
| postrenddetector.sensitivity |Чувствительность для средства обнаружения положительных тенденций |3.25 |double |Нет |3,25–5 (чем меньше значение, тем больше чувствительность) |
| negtrenddetector.sensitivity |Чувствительность для средства обнаружения отрицательных тенденций. |3.25 |double |Нет |3,25–5 (чем меньше значение, тем больше чувствительность) |
| tspikedetector.sensitivity |Чувствительность для средства обнаружения пиков TSpike |3 |integer |1–10 |3,5–5 (чем меньше значение, тем выше чувствительность) |
| zspikedetector.sensitivity |Чувствительность для средства обнаружения пиков ZSpike |3 |integer |1–10 |3,5–5 (чем меньше значение, тем выше чувствительность) |
| seasonality.enable |Указывает, нужно ли выполнять анализ сезонности |true |Логическое |true, false |Зависит от временных рядов |
| seasonality.numSeasonality |Максимальное число периодических циклов для обнаружения |1 |integer |1, 2 |1–2 |
| seasonality.transform |Указывает, будут ли перед обнаружением аномалий удаляться сезонные компоненты и компоненты тенденций |deseason |перечисляемый |none, deseason, deseasontrend |Н/Д |
| postprocess.tailRows |Число последних точек данных, которые нужно сохранить в выходных результатах |0 |integer |0 (сохранить все точки данных) или укажите количество точек, которые следует сохранить в результатах |Н/Д |
...
| ProcessedData |Один из следующих вариантов: <ul><li>Временные ряды данных с корректировкой сезонных колебаний, если обнаружены существенные сезонные колебания и активирована функция "deseason".</li><li>Временные ряды данных с корректировкой сезонных колебаний и тенденций, если обнаружены существенные сезонные колебания и активирована функция "deseasontrend".</li><li>в противном случае этот параметр будет таким же, как Оригиналдата</li> |
| TSpike |Двоичный показатель обнаружения пика с помощью средства обнаружения пиков TSpike |
| ZSpike |Двоичный показатель обнаружения пика с помощью средства обнаружения пиков ZSpike |
