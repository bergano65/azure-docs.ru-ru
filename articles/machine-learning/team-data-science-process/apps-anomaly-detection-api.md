...
> [!NOTE]
> Этот элемент находится на техническом обслуживании. Мы рекомендуем вам использовать [службу API Anomaly Detector,](https://azure.microsoft.com/services/cognitive-services/anomaly-detector/) работающая на основе галереи алгоритмов машинного обучения в Azure Cognitive Services для обнаружения аномалий от бизнес-, операционных и IoT-метрик.
...
Чтобы использовать API-интерфейс, необходимо развернуть его в подписке Azure. Он будет размещаться там в роли веб-службы машинного обучения Azure.  Это можно сделать в [коллекции решений ИИ Azure](https://gallery.cortanaintelligence.com/MachineLearningAPI/Anomaly-Detection-2).  Это развернет две Web-сервисы Azure Machine Learning Studio (классические) web-службы (и связанные с ними ресурсы) в подписку Azure - одну для обнаружения аномалий с обнаружением сезонности и одну без обнаружения сезонности.  После завершения развертывания вы сможете управлять A-Образов с страницы [веб-сервисов Azure Machine Learning Studio (классическая).](https://services.azureml.net/webservices/)  На этой странице вы можете найти расположения конечных точек, ключи API и примеры кода для вызова API.  Более подробные инструкции см. [здесь](https://docs.microsoft.com/azure/machine-learning/machine-learning-manage-new-webservice).

## <a name="scaling-the-api"></a>Масштабирование API
По умолчанию развертывание будет иметь бесплатный план выставления счетов Dev/Test, который включает 1000 транзакций в месяц и 2 вычислительных часа в месяц.  Этот план вы можете сменить на любой другой в соответствии с потребностями.  Сведения о ценах на разные планы приводятся [на этой странице](https://azure.microsoft.com/pricing/details/machine-learning/) в разделе "Цены на производственный веб-API".
...
Веб-сервис предоставляет API на основе REST по httpS, который можно использовать различными способами, включая веб-или мобильное приложение, R, Python, Excel и т.д.  Данные временных рядов отправляются в эту службу через вызов REST API и выполняеткомбинацию трех типов аномалий, описанных ниже.

## <a name="calling-the-api"></a>Вызов интерфейса API
Чтобы вызвать API, вам нужно знать расположение его конечной точки и ключ API.  Эти два требования, наряду с примерным кодом для вызова API, доступны на странице [веб-сервисов Azure Machine Learning Studio (классическая).](https://services.azureml.net/webservices/)  Чтобы получить эти сведения, перейдите к нужному API и щелкните вкладку "Использование".  Вы можете назвать API как API Swagger (т.е. с параметром `format=swagger`URL) или как неSwagger API (т.е. без параметра `format` URL).  В приведенных примерах кода используется формат Swagger.  Ниже представлен пример запроса и ответа без поддержки Swagger.  В этих примерах обращение выполняется к конечной точке с поддержкой сезонности.  Для другой конечной точки все выглядит точно так же.
...
Для просмотра `ColumnNames` поля необходимо включить `details=true` в запрос параметр URL-адреса.  В следующих таблицах описано назначение каждого из этих полей.
...
На рисунке ниже показан пример аномалий, которые может обнаружить API оценки. Эта серия времени имеет два различных изменения уровня и три шипа. Красными точками обозначено время обнаружения изменений уровня, а черными точками — обнаруженные пики.
![API оценки][1]

### <a name="detectors"></a>Средства обнаружения
API обнаружения аномалий поддерживает детекторы в трех широких категориях. Сведения об определенных входных параметрах и выходных данных для каждого средства обнаружения представлены в следующей таблице.
...
| Средство обнаружения медленно развивающихся тенденций |Средство обнаружения медленно развивающихся тенденций |Обнаружение медленно развивающихся положительных тенденций согласно чувствительности набора |*trenddetector.sensitivity:* порог на балле детектора (по умолчанию: 3.25, 3.25 - 5 является разумным диапазоном для выбора; Чем выше, тем менее чувствительны) |tscore: число с плавающей точкой, которое используется как показатель аномалий в тенденции |
| Средства обнаружения изменений уровня | Средство обнаружения двунаправленных изменений уровня |Обнаружение восходящих и нисходящих изменений уровня согласно чувствительности набора |*bileveldetector.sensitivity:* порог на балле детектора (по умолчанию: 3.25, 3.25 - 5 является разумным диапазоном, чтобы выбрать из; Чем выше, тем менее чувствительны) |rpscore: число с плавающей точкой, которое используется как показатель аномалий в восходящем и нисходящем изменении уровня |
...
На следующем рисунке показан пример аномалий, обнаруженных в сезонных временных рядах. Серия времени имеет один всплеск (первая черная точка), два провала (вторая черная точка и одна в конце) и одно изменение уровня (красная точка). Как падение в середине временных рядов, так и изменение уровня заметны только после того, как сезонные компоненты удаляются из серии.
![API сезонности][2]
