...
В этой статье показано, как перемещать данные из локальной базы данных серверов S'L в базу данных S'L Azure через Azure Blob Storage с помощью Azure Data Factory (ADF): этот метод является поддерживаемым устаревшим подходом, который имеет преимущества реплицируемой постановочной копии, хотя [мы предлагаем посмотреть на нашу страницу миграции данных для последних опций.](https://datamigration.microsoft.com/scenario/sql-to-azuresqldb?step=1)
...
* когда данные нуждаются в преобразованиях или бизнес-логика добавлена к ним при миграции.
...
Мы настраиваем конвейер ADF, который объединяет два действия переноса данных. Вместе они ежедневно перемещают данные между базой данных в облаке и базой данных Azure S'L в облаке. Эти два действия таковы:
...
> [!NOTE]
> Представленные здесь шаги были адаптированы из более подробного руководства, представленного группой ADF: [Копирование данных из предварительной базы данных S'L Server в хранилище Azure Blob](https://docs.microsoft.com/azure/data-factory/tutorial-hybrid-copy-portal/) к соответствующим разделам этой темы предоставляется при необходимости.
>
>
...
* Доступ к **базе данных SQL Azure**. Если необходимо настроить базу данных Azure S'L, тема, [начинающееся с базой данных Microsoft Azure S'L,](../../sql-database/sql-database-get-started.md) содержит информацию о том, как предоставить новый экземпляр базы данных Azure S'L.
...
## <a name="upload-the-data-to-your-on-premises-sql-server"></a><a name="upload-data"></a>Отправка данных в локальный SQL Server
...
Integration Runtime — это управляемая клиентами инфраструктура интеграции данных, используемая Фабрикой данных Azure для обеспечения возможностей интеграции данных в различных сетевых средах. Эта среда выполнения раньше называлась "Шлюз управления данными".

Чтобы настроить, [следуйте инструкциям для создания трубопровода](https://docs.microsoft.com/azure/data-factory/tutorial-hybrid-copy-portal#create-a-pipeline)
...
2. [Таблица Blob](#adf-table-blob-store)
3. [Таблица Лазурного излавлёза](#adf-table-azure-sql)
...
Сюда не были включены имена столбцов. Вы можете подвысить имена столбцов, включив их здесь (для получения подробной информации проверьте тему [документации ADF.](../../data-factory/copy-activity-overview.md)
...
Мы не использовали функциональность, предоставляемую ADF, для данных труб постепенно. Дополнительные сведения об этой и других возможностях ADF см. в [документации по ADF](https://azure.microsoft.com/services/data-factory/).
