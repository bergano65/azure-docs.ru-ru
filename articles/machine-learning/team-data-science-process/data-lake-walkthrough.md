...
Для построения и развертывания прогнозных моделей используется Машинное обучение Azure Studio (классическая модель). Для этого используется два подхода: первый — с помощью сценариев Python, а второй — с использованием таблиц Hive на кластере HDInsight (Hadoop).
...
В первой строке содержатся заголовки, поэтому необходимо их удалить и заменить типы столбцов на соответствующие. Обработанные данные можно сохранить в Azure Data Lake Storage с помощью **swebhdfs://data_lake_storage_name.azuredatalakestorage.NET/folder_name/file_name**_ или учетной записи хранилища BLOB-объектов Azure с помощью **wasb://container_name\@blob_storage_account_name. BLOB. Core. Windows. NET/BLOB_NAME**.
