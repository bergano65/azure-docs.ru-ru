...
Эти технологии используются в этом пошаговом шаге.
...
Аналитика озера данных — это также ключевая часть Cortana Analytics Suite. Она поддерживает работу с хранилищем данных SQL Azure, Power BI и фабрикой данных. Эта комбинация дает вам полную облачные большие данные и расширенную аналитическую платформу.

В начале этого пошагового руководства описано, как установить необходимые компоненты и ресурсы, которые требуются для выполнения задач обработки и анализа данных. Затем он излагает этапы обработки данных с помощью U-S'L и завершается показом того, как использовать Python и Hive с помощью Azure Machine Learning Studio (классический) для создания и развертывания прогностических моделей.
...
Студия машинного обучения Azure (классическая) используется для создания и развертывания прогностических моделей с использованием двух подходов: сначала со скриптами Python, а затем со таблицами Hive в кластере HDInsight (Hadoop).
...
* Хранение озер лазурных данных (ADLS)
* Аналитику озера данных Azure;
* учетную запись хранения BLOB-объектов;
* Студия машинного обучения Azure (классическая) учетная запись
...
### <a name="create-an-azure-data-lake-storage"></a>Создание хранилища озер лазурных данных
...
Создайте учетную запись хранилища BLOB-объектов Azure на [портале Azure](https://portal.azure.com). Для получения подробной информации смотрите раздел «Создание учетной записи хранения» в [учетных записях «О хранилищах Azure».](../../storage/common/storage-create-storage-account.md)

 ![5](./media/data-lake-walkthrough/5-Create-Azure-Blob.PNG)

### <a name="set-up-an-azure-machine-learning-studio-classic-account"></a>Настройка учетной записи Студии машинного обучения Azure (классическая)
Зарегистрируйтесь/в студии машинного обучения Azure (классика) со страницы [студии Azure Machine Learning.](https://azure.microsoft.com/services/machine-learning/) Нажмите кнопку **Начните прямо сейчас** и выберите Free Workspace ("Бесплатная рабочая область") или Standard Workspace ("Стандартная рабочая область"). Теперь вы можете создавать эксперименты в Студии машинного обучения Azure.
...
После завершения монтажа откройте Visual Studio. Вы должны увидеть вкладку Data Lake ("Озеро данных") в меню вверху. Ресурсы Azure должны отобразиться на левой панели при входе в учетную запись Azure.
...
Уникальный ключ для объединения trip\_data и trip\_fare состоит из следующих трех полей: medallion, hack\_license и pickup\_datetime. К необработанным файлам CSV можно получить доступ из капли хранения Azure. Сценарий U-SQL для этого объединения находится в разделе [Объединение таблиц trip и fare](#join) .
...
* [Присоединяйтесь к таблицам поездок и тарифов](#join)
* [Выборка данных](#sample)
* [Запуск рабочих мест в U-S'L](#run)
...
### <a name="data-ingestion-read-in-data-from-public-blob"></a><a name="ingest"></a>Заглатывание данных: Читайте в данных из публичной капли

Местоположение данных в blob Azure называется **\_wasb://container\@имя\_\_счета\_хранения капли name.blob.core.windows.net/blob_name** и может быть извлечено с помощью **Extractors.Csv()**. Замените свое собственное имя контейнера и\_имя\@учетной записи хранилища в следующих сценариях для имен контейнера blob\_имя учетной\_записи\_в адресе wasb. Поскольку имена файлов находятся в одном формате, можно использовать **данные\_о\_\{\*\}поездке .csv** для чтения во всех 12 файлах поездки.
...
В первой строке содержатся заголовки, поэтому необходимо их удалить и заменить типы столбцов на соответствующие. Вы можете сохранить обработанные данные в Хранилище озера Лазурных Данных с помощью **swebhdfs://data_lake_storage_name.azuredatalakestorage.net/folder_name/file_name,** либо в учетную запись хранения Azure Blob с помощью **wasb://container_name\@blob_storage_account_name.blob.core.windows.net/blob_name**.
...
Аналогичным образом можно выполнить чтение наборов данных о тарифах. Справа щелкните Хранилище озер лазурных данных, вы можете посмотреть свои данные на **портале Azure --> Data Explorer** или **File Explorer** в Visual Studio.
...
После считывания таблиц trip и fare можно проверить качество данных следующим образом. Полученные файлы CSV могут быть вывешивана в хранилище Azure Blob или хранилище озер данных Azure.
...
После редактирования скриптов U-S'L вы можете отправить их на сервер с помощью учетной записи Azure Data Lake Analytics. Выберите вкладку **Data Lake**, щелкните **Отправить задание**, а затем выберите свою учетную запись в поле **Analytics Account** (Учетная запись Аналитики), значение параметра **Параллелизм** и нажмите кнопку **Отправить**.

 ![12](./media/data-lake-walkthrough/12-submit-USQL.PNG)

Если задание успешно компилируется, вы сможете наблюдать за его состоянием в Visual Studio. После завершения задания вы даже можете воспроизвести процесс выполнения задания и узнать узкие шаги для повышения эффективности работы. Вы также можете открыть портал Azure, чтобы проверить состояние заданий U-SQL.
...
Для создания и развертывания моделей машинного обучения, используя Python, создайте записную книжки Jupyter на локальном компьютере или в студии машинного обучения Azure. Ноутбук Jupyter, представленный на [GitHub,](https://github.com/Azure/Azure-MachineLearning-DataScience/tree/master/Misc/AzureDataLakeWalkthrough) содержит полный код для изучения, визуализации данных, проектирования функций, моделирования и развертывания. В этой статье описаны только моделирование и развертывание.
...
Когда вы завершите создание модели машинного обучения, ее можно ввести в эксплуатацию. В качестве примера используется двоичная логистическая модель. Убедитесь, что scikit-узнать версию в локальной машине 0.15.1 (Azure машинного обучения Studio уже по крайней мере в этой версии).

* Найдите учетные данные рабочего пространства в настройках Azure Machine Learning Studio (классические). В студии машинного обучения Azure нажмите --> **кнопку «Настройки» Наизусть****Name** --> имен . **Settings**
...
* Получите учетные данные веб-службы:
        ...
        def NYCTAXIPredictor(trip_distance, passenger_count, payment_type_dummy_CRD, payment_type_dummy_CSH,payment_type_dummy_DIS, payment_type_dummy_NOC, payment_type_dummy_UNK, vendor_id_dummy_CMT, vendor_id_dummy_VTS ):
            pass
* Вызовите API веб-службы. Как правило, ждать 5-10 секунд после предыдущего шага.
...
Студия машинного обучения Azure (классическая) может считывать данные непосредственно из хранилища озер данных Azure, а затем использоваться для создания и развертывания моделей. При этом используется таблица Hive, которая указывает на хранилище озер лазурных данных. Для таблицы Hive необходимо подготовить отдельный кластер Azure HDInsight. 

### <a name="create-an-hdinsight-linux-cluster"></a>Создание кластера HDInsight на платформе Linux
Создайте кластер HDInsight (Linux) с помощью [портала Azure](https://portal.azure.com). Для получения подробной информации **см. кластер HDInsight с доступом к разделу хранения озер данных Azure** в [создании кластера HDInsight с помощью портала Azure.](../../data-lake-store/data-lake-store-hdinsight-hadoop-use-portal.md)

 ![18](./media/data-lake-walkthrough/18-create_HDI_cluster.PNG)

### <a name="create-hive-table-in-hdinsight"></a>Создание таблицы Hive в HDInsight
Теперь вы создаете таблицы Hive, которые будут использоваться в Azure Machine Learning Studio (классический) в кластере HDInsight, используя данные, хранящиеся в Azure Data Lake Storage на предыдущем этапе. Перейдите к новому кластеру HDInsight. Нажмите **Кнопка Настройки** --> **Свойства** --> **Кластер AAD Identity** --> **ADLS Доступ**, убедитесь, что ваша учетная запись хранения Azure data Lake хранилищ добавляется в список с чтением, записью и выполнением прав.
...
Вставьте следующие сценарии Hive, чтобы создать таблицу. Местонахождение источника данных находится в ссылке Azure Data Lake Storage: **adl:///data_lake_store_name.azuredatalakestore.net:443/folder_name/file_name**.
...
Когда запрос завершается, результаты должны увидеть следующим образом:
...
1. Получите данные в Студию машинного обучения Azure (классический) с помощью модуля **«Импортные данные»,** доступного в разделе **Ввод и выход данных.** Дополнительные сведения см. на странице справки [Импорт данных](https://msdn.microsoft.com/library/azure/4e1b0fe6-aded-4b3f-a36f-39b8862b9004/).
...
4. Введите кластер URI кластера HDInsight (это URI можно найти на портале Azure), учетные данные Hadoop, расположение выходных данных и имя учетной записи хранилища Azure/key/container.
...
После создания эксперимента нажмите **«Настройка веб-службы** --> **Predictive Web Service»**
...
Завершив это пошаговое решение, вы создали среду для создания масштабируемых сквозных решений в Azure Data Lake. Эта среда использовалась для анализа большого набора данных, в отношении которого выполнены ключевые шаги по обработке данных — от получения данных и обучения модели до развертывания модели в качестве веб-службы. Для обработки, изучения и выборочного образца данных использовался U-S'SL. Python и Hive использовались в Azure Machine Learning Studio (классический) для создания и развертывания прогностических моделей.
