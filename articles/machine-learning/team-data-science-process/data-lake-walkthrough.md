...
Data Lake Analytics — это также ключевая часть Cortana Analytics Suite. Поддерживается работа с хранилищем данных SQL Azure, Power BI и фабрикой данных. Это сочетание обеспечивает полную облачную платформу для работы с большими данными и расширенной аналитикой.
...
Расположение данных в большом двоичном объекте Azure указывается как **wasb://container\_имя\@\_хранилища BLOB-объектов\_учетная запись\_Name.BLOB.Core.Windows.NET/BLOB_NAME** , и их можно извлечь с помощью средств **извлечения. CSV ()** . Замените имя контейнера и имя учетной записи хранения в следующих скриптах для имени контейнера\_имя\@хранилище BLOB-объектов\_\_учетной записи\_имя в адресе wasb. Так как имена файлов имеют одинаковый формат, можно использовать **поездку\_данных\_\{\*\}. csv** для чтения во всех 12 файлах приема.
...
В первой строке содержатся заголовки, поэтому необходимо их удалить и заменить типы столбцов на соответствующие. Обработанные данные можно либо сохранить в Azure Data Lake Storage с помощью **swebhdfs://data_lake_storage_name. azuredatalakestorage. NET/folder_name/file_name**_, либо в учетную запись хранилища BLOB-объектов Azure с помощью **wasb://container_name\@blob_storage_account_name. BLOB. Core. Windows. NET/BLOB_NAME**.
...
* Найдите учетные данные рабочей области в параметрах Машинное обучение Azure Studio (классическая модель). В Студии машинного обучения Azure выберите **Параметры** --> **Имя** --> **Authorization Tokens** (Маркеры авторизации).
