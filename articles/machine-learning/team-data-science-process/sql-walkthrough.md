---
title: Сборка и развертывание модели в виртуальной машине SQL Server – командный процесс обработки и анализа данных
description: Создание и развертывание модели машинного обучения с помощью SQL Server на виртуальной машине Azure с общедоступным набором данных.
...
## <a name="nyc-taxi-trips-dataset-description"></a><a name="dataset"></a>NYC Такси Поездки Набор данных Описание
Данные NYC Taxi Trip составляют около 20 ГБ сжатого файла CSV (48 ГБ несжатых), включающих более 173 миллионов индивидуальных поездок и тарифы, оплаченные за каждую поездку. Каждая запись о поездке содержит расположение пунктов отправления и назначения и время, анонимизированный номер лицензии водителя и номер медальона (уникальный идентификатор такси). Данные включают в себя все поездки за 2013 год и предоставляются в виде следующих двух наборов данных за каждый месяц:
...
* Двоичная классификация: Предсказать, действительно ли чаевые были оплачены за поездку, то *есть,\_чаевые сумма,* которая больше, чем $ 0 является положительным примером, в то время как *чаевые\_сумма* $ 0 является негативным примером.
...
В этом уроке мы продемонстрируем параллельный объемный импорт данных на сервер S'L Server, исследование данных, проектирование функций и отбор проб с помощью студии управления серверами S'L, а также с помощью ноутбука IPython. [Примеры скриптов](https://github.com/Azure/Azure-MachineLearning-DataScience/tree/master/Misc/DataScienceProcess/DataScienceScripts) и файлы [IPython Notebook](https://github.com/Azure/Azure-MachineLearning-DataScience/tree/master/Misc/DataScienceProcess/iPythonNotebooks) предоставлены в общий доступ на GitHub. На этом же ресурсе доступен и образец файла IPython Notebook для работы с данными в BLOB-объектах.

Чтобы настроить среду научной обработки данных в Azure, выполните следующие действия.

1. [Создание учетной записи хранилища](../../storage/common/storage-account-create.md)
2. [Создание рабочего пространства для машинНого обучения Azure](../studio/create-workspace.md)
...
## <a name="get-the-data-from-public-source"></a><a name="getdata"></a>Получение данных из общедоступного источника
Чтобы получить [набор данных о поездках такси Нью-Йорка](https://www.andresmh.com/nyctaxitrips/) из общедоступного расположения, скопируйте данные на новую виртуальную машину. Для этого можно воспользоваться любым из методов, описанных в статье [Перемещение данных в хранилище больших двоичных объектов Azure и из него](move-azure-blob.md).

Чтобы скопировать данные с помощью AzCopy, выполните следующие действия.

1. Войдите на виртуальную машину (ВМ).
2. Создайте новый каталог на диске данных VM (Примечание: Не используйте временный диск, который поставляется с VM в качестве диска данных).
...
## <a name="bulk-import-data-into-sql-server-database"></a><a name="dbload"></a>Выполните массовый импорт данных в базу данных SQL Server
Производительность загрузки/переноса больших объемов данных в базу данных S'L и последующие запросы может быть улучшена с помощью *раздельных таблиц и*представлений. В этом разделе мы будем следовать инструкциям, описанным в разделе [Параллельный массовый импорт данных с помощью таблиц разделов SQL](parallel-load-sql-partitioned-tables.md) для создания новой базы данных и параллельной загрузки данных в секционированные таблицы.
...
2. Подключитесь с использованием проверки подлинности Windows.
    ...
    ![Подключение SSMS][12]
3. Если вы еще не изменили режим проверки подлинности SQL и не создали пользователя для входа в SQL, откройте файл сценария с именем **change\_auth.sql** в папке **Примеры скриптов**. Измените имя пользователя и пароль, используемые по умолчанию. Нажмите **Выполнить** в панели инструментов для запуска скрипта.
   
    ![Выполнение сценария][13]
4. Проверьте и/или измените используемые по умолчанию папки базы данных и журнала SQL Server, чтобы обеспечить хранение новых создаваемых баз данных на диске с данными. ВМ-изображение Сервера S'L Server, оптимизированное для нагрузок складирования данных, предварительно настроено с помощью дисков данных и журналов. Если ваша виртуальная машина не содержит диска с данными и вы добавили новые виртуальные жесткие диски в процессе установки ВМ, измените папки по умолчанию следующим образом.
...
6. Затем создайте две таблицы секционирования: по одной для trip\_data и trip\_fare. Откройте образец скрипта **create\_partitioned\_table.sql**, который выполнит следующие действия.
   ...
   * Создайте две таблицы секционирования для сопоставления со схемой секционирования: **nyctaxi\_trip** будет содержать данные trip\_data, а **nyctaxi\_fare** — данные trip\_fare.
     ...
     Нажмите **Выполнить,** чтобы запустить сценарий и создать разделенные таблицы.
7. Папка **Примеры сценариев** содержит два образца сценариев PowerShell, предоставленные для демонстрации параллельного массового импорта данных в таблицы SQL Server.
   ...
   * **bcp\_parallel\_nyctaxi.ps1** — предварительно настроенная версия универсального скрипта, которая может использоваться для загрузки обеих таблиц с данными о поездках на такси Нью-Йорка.  
...
9. Сценарий PowerShell сообщает о времени начала и завершения операций. После завершения всех операций массового импорта сообщается время завершения. Проверьте папку целевого журнала, чтобы убедиться, что объем импорта был успешным, то есть, никаких ошибок, зарегистрированных в папке целевого журнала.
10. Теперь база данных готова к просмотру, проектированию характеристик и другим операциям. Поскольку таблицы разделены в соответствии с полем **времени дат\_приема,** запросы, которые включают условия даты **пикапа\_** в пункте **WHERE,** выиграют от схемы раздела.
11. В среде **SQL Server Management Studio** изучите предоставленный образец скрипта **sample\_queries.sql**. Чтобы выполнить любой из выборочных запросов, выделите строки запроса, а затем нажмите **«Выполнить»** в панели инструментов.
...
## <a name="data-exploration-and-feature-engineering-in-sql-server"></a><a name="dbexplore"></a>Просмотр данных и проектирование характеристик в SQL Server
...
1. Сохраните окончательный запрос S'L, чтобы извлечь и отведать данные и копировать-вставить запрос непосредственно в модуль [данных импорта][import-data] в Azure Machine Learning, или
...
В этом разделе мы сохраним окончательный запрос для извлечения и отбытия данных. Второй метод демонстрируется в разделе [Просмотр данных и проектирование характеристик в IPython Notebook](#ipnb) .
...
Запросы на создание меток и просмотр преобразования географии можно также использовать для создания меток/характеристик путем удаления части, производящей подсчет. Дополнительные примеры проектирования характеристик на SQL содержатся в разделе [Просмотр данных и проектирование характеристик в IPython Notebook](#ipnb) . Более эффективно запускать запросы генерации функций на полном наборе данных или большом подмножестве, используя запросы S'L, которые работают непосредственно на экземпляре базы данных сервера S'L. Запросы могут выполняться в **студии управления серверами S'L,** IPython Notebook, или в любом инструменте или среде разработки, которые могут получить доступ к базе данных локально или удаленно.
...
## <a name="data-exploration-and-feature-engineering-in-ipython-notebook"></a><a name="ipnb"></a>Исследование данных и разработка функций в ноутбуке IPython
...
При работе с большими данными следуйте этой рекомендуемой последовательности:
...
1. Сохраните окончательный запрос S'L, чтобы извлечь и оторвать данные и скопировать запрос непосредственно в модуль [данных импорта][import-data] в Azure Machine Learning. Этот метод демонстрируется в разделе [Построение моделей в машинном обучении Azure](#mlmodel) .    
...
В этом разделе мы создадим новую таблицу для хранения отобранных и инженерных данных. Пример прямого SQL-запроса для построения модели содержится в разделе [Просмотр данных и проектирование характеристик в SQL Server](#dbexplore) .
...
В этом разделе мы исследуем распределение данных, используя 1% отобранных данных, которые сохраняются в новой таблице, созданной выше. Аналогичные исследования могут быть выполнены с помощью исходных таблиц, по желанию с помощью **TABLESAMPLE** для ограничения выборки исследования или путем ограничения результатов в данный период времени с использованием разделов **даты\_пикапа,** как показано в разделе Data Exploration and Feature Engineering в разделе [S'L Server.](#dbexplore)
...
Этот пример преобразует непрерывное численное поле в заданные диапазоны категорий, т.е. преобразует численное поле в категорическое поле.
...
Этот пример разбивает десятичное представление поля широты и/или долготы на несколько областных полей различной детализации, таких как страна/регион, город, город, блок и т.д. Новые геополя не отображаются в реальных местоположениях. Дополнительные сведения о сопоставлении местоположений геокодов см. в статье о [службах REST карт Bing](https://msdn.microsoft.com/library/ff701710.aspx).
...
В этом упражнении мы уже просмотрели и спроектировали данные на сервере SQL Server и определились с размером выборки для приема в Машинное обучение Azure. Чтобы построить одну или несколько моделей прогнозирования, мы решили:

1. Загрузите данные в Машинное обучение Azure, используя модуль [Импорт данных][import-data], доступный в разделе **Data Input and Output** (Ввод и вывод данных). Для получения дополнительной информации смотрите справочную страницу модуля [«Импортные данные».][import-data]
...
- [FOILing NYC's Taxi Trip Data Криса Ноунга](https://chriswhong.com/open-data/foil_nyc_taxi/)   
