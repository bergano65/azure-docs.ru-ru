---
title: Используйте кластер Azure HDInsight Hadoop на 1-ТБ-набор еданных - Процесс анализа данных команды
...
Данные Criteo — это набор данных прогнозирования кликов, который составляет 370 ГБ сжатого файла TSV с жипий(1,3 ТБ несжаты), включающий более 4,3 миллиарда записей. Эти данные получены на основе данных о переходах по ссылкам за 24 дня, предоставленных [Criteo](https://labs.criteo.com/downloads/download-terabyte-click-logs/). Для удобства специалистов по обработке и анализу данных данные, доступные для экспериментирования, были разархивированы.
...
**Определение:** *Clickthrough скорость (CTR):* Этот показатель представляет собой процент кликов в данных. В этом наборе Criteo коэффициент составляет около 3,3 % или 0,033.
...
> [!NOTE]
> Этот шаг обычно является задачей **админа.**
...
2. [Настройка кластеров Azure HDInsight Hadoop для обработки и анализа данных.](customize-hadoop-cluster.md) На этом шаге создается кластер Azure HDInsight Hadoop и на всех узлах устанавливается 64-разрядный дистрибутив Anaconda Python 2.7. При настройке кластера HDInsight следует выполнить два важных шага (описанных в этом разделе).
   ...
   * Свяжите учетную запись хранилища, созданную в шаге 1, с кластером HDInsight при ее создании. Эта учетная запись хранения используется для доступа к данным, которые можно обработать в пределах кластера.
   * После создания кластера включите удаленный доступ к головному узлу кластера. Помните, что учетные данные удаленного доступа, указанные здесь (в отличие от учетных данных, указанных при создании кластера): завершите следующие процедуры.
3. [Создайте рабочее пространство Azure Machine Learning Studio (классическое):](../studio/create-workspace.md)Это рабочее пространство Azure Machine Learning используется для построения моделей машинного обучения после первоначального исследования данных и отбора проб в кластере HDInsight.
...
Доступ к набору данных [Criteo](https://labs.criteo.com/downloads/download-terabyte-click-logs/) можно получить, щелкнув ссылку, приняв условия использования и указав имя. Снимок показан здесь:
...
Данные находятся в месте [хранения azure blob:](../../storage/blobs/storage-dotnet-how-to-use-blobs.md) wasb://criteo@azuremlsampleexperiments.blob.core.windows.net/raw/ wasb относится к расположению хранилища больших двоичных объектов Azure.

1. Данные в этом хранилище Azure blob состоят из трех subfolders незасеченных данных.
...
2. Необработанные данные gzip также доступны в основной папке *сырой /* как day_NN.Gz, где NN идет от 00 до 23.
...
Чтобы войти в головной узел кластера, узнайте его расположение, используя [портал Azure](https://ms.portal.azure.com) . Щелкните значок HDInsight в виде слона в левой части портала, а затем дважды щелкните имя кластера. Перейдите на вкладку **Конфигурация,** дважды щелкните значок CONNECT в нижней части страницы и введите учетные данные удаленного доступа по запросу, отведите вас к головному ноду кластера.

Вот как выглядит типичный первый вход в кластерный головной убор:
...
Следующий код создает базу данных "criteo", а затем генерирует четыре таблицы:
...
Все эти таблицы являются внешними, так что вы можете указать на их azure Blob Storage (wasb) местах хранения.

**ЛЮБОЙ запрос Hive можно выполнить двумя способами:**

* **Использование командной строки Hive REPL**: Первый из этого заключается в выдаче команды "улей" и копировании и вставке запроса в командной строке Hive REPL:

        cd %hive_home%\bin
        hive

     Теперь на командной строке REPL, резки и вставки запроса выполняет его.
* **Сохранение запросов в файл и выполнение команды:** Во-вторых, сохранение запросов в файл '.hql'[(образец&#95;улей&#95;создать&#95;criteo&#95;базы данных&#95;и&#95;tables.hql),](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_create_criteo_database_and_tables.hql)а затем выдать следующую команду для выполнения запроса:
...
Команда, чтобы сделать это похоже на один показано (обратитесь к [образцу&#95;улей&#95;рассчитывать&#95;criteo&#95;тест&#95;день&#95;23&#95;examples.hql):](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_count_criteo_test_day_23_examples.hql)
...
Процент положительных меток составляет около 3,3% (в соответствии с исходным набором данных).
...
Сочетание параметров LATERAL VIEW и explode в Hive служит для вывода выходных данных, подобных SQL, вместо обычного списка. В этой таблице первая колонка соответствует центру ячейки, а вторая — частоте бен.
...
Col15 имеет 19M уникальные ценности! Использование упрощенных методов, таких как "прямое кодирование" для кодирования таких многомерных категориальных переменных, невозможно. Для эффективного решения этой проблемы объясняется и демонстрируется эффективный и надежный метод, называемый [обучение с использованием счетчиков](https://blogs.technet.com/b/machinelearning/archive/2015/02/17/big-learning-made-easy-with-counts.aspx).
...
Интерес представляет также распределение количества пар категориальных переменных. Это можно определить с помощью кода в скрипте [sample&#95;hive&#95;criteo&#95;paired&#95;categorical&#95;counts.hql](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_criteo_paired_categorical_counts.hql):
...
Прежде чем перейти к машинному обучению Azure, следует обсудить последний важный компонент, касающийся таблицы счетчиков. В следующем подразделе таблица подсчетов подробно обсуждается.

## <a name="a-brief-discussion-on-the-count-table"></a><a name="count"></a> Краткое описание таблицы счетчиков
Как вы видели, несколько категориальных переменных имеют высокую размерность. В нашем пошаговом руководстве представлен метод под названием [обучение с использованием счетчиков](https://blogs.technet.com/b/machinelearning/archive/2015/02/17/big-learning-made-easy-with-counts.aspx) для кодирования этих переменных эффективным и надежным способом. Дополнительную информацию об этом методе можно получить, перейдя по указанной ссылке.
...
2. [Создайте эксперимент: очистите данные и сделайте их функцией со таблицами подсчета голосов](#step2)
...
Начните с выбора **эксперимента «New** -> **EXPERIMENT** -> Blank**Experiment**». Затем в поле **Поиск** в верхнем левом углу экрана введите запрос "Импорт данных". Перетащите модуль **Импорт данных** на холст эксперимента (в средней части экрана), чтобы использовать его для доступа к данным.
...
7. **Имя учетной записи хранилища Azure**: Учетная запись хранилища, связанная с кластером
8. **Ключ учетной записи хранилища Azure:** ключ учетной записи хранилища, связанный с кластером.
...
![Перетащите набор данных на основную панель](./media/hive-criteo-walkthrough/cl5tpGw.png)
...
### <a name="step-2-create-an-experiment-in-azure-machine-learning-to-predict-clicks--no-clicks"></a><a name="step2"></a>Шаг 2: Создайте эксперимент в Azure Machine Learning для прогнозирования кликов / без кликов
Наш эксперимент Azure Machine Learning Studio (классический) выглядит следующим образом:

![Эксперимент машинного обучения](./media/hive-criteo-walkthrough/xRpVfrY.png)

Теперь рассмотрим основные компоненты этого эксперимента. Перетащите наши сохраненные наборы данных и тестов на нашем экспериментальном холсте.
...
Теперь основное внимание уделяется такому важному моменту, как преобразование данных для обучения и тестовых данных перед созданием функций счетчика. Есть два модуля **Execute R Script,** используемых до того, как преобразование подсчета будет применено к нашим данным.
...
Выберите значения по умолчанию для эксперимента. По умолчанию имеют смысл и хороший способ получить быстрые базовые показатели производительности. Построив базу, можно повысить производительность, скорректировав желаемые параметры.
...
Наконец следует проанализировать производительность модели. Как правило, для задач двухклассовой (двоичной) классификации хорошо подходит мера AUC. Чтобы визуализировать эту кривую, подключите модуль **модели оценки** к модулю **модели оценки.** Если щелкнуть **Визуализировать** в модуле **Evaluate Model** (Анализ модели), отобразится подобная схема:

![Модуль «Анализ модели» и «Увеличивающееся дерево принятия решений»](./media/hive-criteo-walkthrough/0Tl0cdg.png)

В задачах двоичной (или двухклассовой) классификации хорошо подходит такая мера прогнозирования точности, как AUC (площадь под кривой). В следующих разделах продемонстрированы результаты использования этой модели на основе нашего тестового набора данных. Справа щелкните в выходный порт модуля **Evaluate Model,** а затем **визуализировать**.
...
Во-первых, сохранить нашу обученную модель в качестве объекта обученной модели, нажав на модуль **модели train** Model и используя опцию Save as **Trained Model.**
...
Так как таблица счетчиков имеет большой размер, на начальном этапе возьмите несколько строк тестовых данных и создайте из них выходные данные с признаками счетчиков. Этот вывод может служить форматом входных данных для нашего веб-сервиса, как показано здесь:
...
Во-первых, существенной структурой является модуль **Score Model,** который принимает наш обученный объект модели, и несколько строк входных данных, которые были созданы в предыдущих шагах с помощью модуля **Count Featurizer.** Используйте модуль "Выбор столбцов в наборе данных", чтобы отобразить расчетные метки и значения вероятности оценки.
...
Если щелкнуть ссылку **Запрос — ответ**, вы перейдете на страницу, которая содержит код на C#, Python и R, поставляемый изготовителем. Этот код можно использовать для вызова веб-службы. Ключ API на этой странице должен использоваться для проверки подлинности.
...
Ключ API по умолчанию был заменен клавишой API нашего веб-сервиса. Щелкнув **Запуск** в данной ячейке в iPython Notebook, вы получите следующий ответ:

![Ответ iPython](./media/hive-criteo-walkthrough/KSxmia2.png)

Для двух тестовых примеров, о которых было задано в рамке скрипта Python JSON, вы получаете ответы в виде "Scored Labels, Scored Probabilities". В этом случае значения по умолчанию были выбраны для кода, поставляемого изготовителем (значение 0 для всех числовых столбцов, строка "Значение" для всех категориальных столбцов).

В заключение наш пошаг-пошаговоц показывает, как обрабатывать крупномасштабный набор данных с помощью машинного обучения Azure. Вы начали работу с терабайтом данных, построили модель прогнозирования и развернули ее в облаке как веб-службу.
