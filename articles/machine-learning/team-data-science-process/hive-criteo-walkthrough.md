---
title: Использование кластеров Azure HDInsight Hadoop в процессе обработки и анализа данных группы с набором данных объемом 1 ТБ
...
Столбцы являются анонимными и используют ряд перечисляемых имен: от "Col1" (для столбца с меткой) до "Col40" (для последнего категориального столбца).
...
**Определение.** *Коэффициент выбора рекламного объявления (CTR).* Это процент щелчков в данных. В этом наборе Criteo коэффициент составляет около 3,3 % или 0,033.
...
1. **Двоичная классификация**. Прогнозирует, щелкнет ли пользователь рекламное объявление:
   ...
   * Класс 0: Без щелчка
   * Класс 1: Щелкните
2. **Регрессия:** Прогнозирует вероятность щелчка рекламного объявления на основании возможностей пользователя.
...
2. [Настройка кластеров HDInsight Hadoop в Azure для обработки и анализ данных](customize-hadoop-cluster.md). На этом этапе создается кластер Azure HDInsight Hadoop, на всех узлах которого установлена 64-разрядная версия Anaconda Python 2.7. При настройке кластера HDInsight следует выполнить два важных шага (описанных в этом разделе).
...
3. [Создание рабочей области Студии машинного обучения Azure](../studio/create-workspace.md). Эта рабочая область машинного обучения Azure используется для создания моделей машинного обучения после просмотра исходных данных и снижения частоты выборки в кластере HDInsight.

## <a name="getdata"></a>Получение и использование данных из общедоступного источника
Доступ к набору данных [Criteo](https://labs.criteo.com/downloads/download-terabyte-click-logs/) можно получить, щелкнув ссылку, приняв условия использования и указав имя. Ниже показан снимок того, как это выглядит.
...
1. **Использование командной строки Hive REPL**. Первый способ: подать команду hive, а затем скопировать и вставить запрос в командную строку Hive REPL. Для этого введите следующее:

        cd %hive_home%\bin
        hive

     Если скопировать и вставить запрос в командную строку REPL, он начнет выполняться.
2. **Сохранение запросов в файл и выполнение команды**. Второй способ: сохранить запросы в файл HQL ([sample&#95;hive&#95;create&#95;criteo&#95;database&#95;and&#95;tables.hql](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_create_criteo_database_and_tables.hql)), а затем выполнить следующую команду для отправки запроса.
...
Как было видно, несколько категориальных переменных обладают очень высокой размерностью. В нашем пошаговом руководстве представлен метод под названием [обучение с использованием счетчиков](https://blogs.technet.com/b/machinelearning/archive/2015/02/17/big-learning-made-easy-with-counts.aspx) для кодирования этих переменных эффективным и надежным способом. Дополнительную информацию об этом методе можно получить, перейдя по указанной ссылке.
...
2. [Создание эксперимента: очистка данных и сделать его функции в таблицах счетчиков](#step2)
...
5. **Пароль для учетной записи пользователя Hadoop**. Пароль для пользователя, выбранное во время введения кластера в эксплуатацию. Пароль для имени пользователя, выбранный во время введения кластера в эксплуатацию (НЕ пароль для удаленного доступа).
6. **Расположение выходных данных**. Выберите Azure
7. **Имя учетной записи хранения Azure**. Учетная запись хранения, которая связанная с кластером
8. **Ключ учетной записи хранения Azure**. Ключ учетной записи хранения, который связан с кластером
...
Наш эксперимент в Студии машинного обучения Azure выполняется следующим образом:
