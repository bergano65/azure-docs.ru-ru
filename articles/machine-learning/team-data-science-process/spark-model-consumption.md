---
title: Ввод моделей машинного обучения, созданных с помощью Spark, в эксплуатацию — командный процесс обработки и анализа данных | Документация Майкрософт
...
2. Создайте модели машинного обучения, которые будут забиты здесь, работая через [исследование данных и моделирование с](spark-data-exploration-modeling.md) темой Spark для кластера Spark 1.6 или ноутбуков Spark 2.0. 
...
Чтобы сохранить модели или файлы в хранилище BLOB-объектов, необходимо указать путь соответствующим образом. Контейнер по умолчанию, прикрепленный к кластеру Spark, можно ссылаться с помощью пути, начинающегося с: *"wasb//"/"*. В следующем примере кода задается расположение данных, которые необходимо считать, и путь для каталога хранилища модели, где сохраняются выходные данные модели: 
...
Ядра PySpark, входящие в состав записных книжек Jupyter, имеют предустановленный контекст. Поэтому вам не нужно четко устанавливать контексты Spark или Hive, прежде чем начать работать с приложением, которое вы разрабатываете. Эти контексты доступны по умолчанию:
...
С помощью функции [StringIndexer](https://spark.apache.org/docs/latest/ml-features.html#stringindexer) столбец меток строкового типа кодируется как столбец с индексами меток. При этом индексы упорядочиваются по частоте меток. 

[OneHotEncoder](https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.OneHotEncoder.html#sklearn.preprocessing.OneHotEncoder) сопоставляет столбец с индексами меток со столбцом двоичных векторов как минимум с одним отдельным значением. Благодаря этой кодировке алгоритмы, для которых требуются признаки со значениями (например, логистическая регрессия), можно применять к категориальным признакам.
...
[spark.mllib](https://spark.apache.org/mllib/) предусматривает использование метода случайного леса в моделях двоичной, мультиклассовой классификации и регрессии с применением как непрерывных, так и категориальных признаков. 
...
**spark.mllib** поддерживает GBTS для двоичной классификации и регрессии, используя как непрерывные, так и категоричные особенности. 

[Градиентные деревья boosting](https://spark.apache.org/docs/latest/ml-classification-regression.html#gradient-boosted-trees-gbts) (GBTS) являются ансамблями деревьев решений. GBTS поезд решения деревьев iteratively свести к минимуму функцию потери. GBTS может обрабатывать категоричные объекты, не требует масштабирования функций и может фиксировать нелинейные и функциональные взаимодействия. Этот алгоритм также может быть использован в настройках мультикласс-классификации.
...
В кластере Spark реализован механизм, который позволяет отправлять пакетные задания или интерактивные запросы удаленно через интерфейс REST с помощью компонента Livy. Livy включен по умолчанию в кластере Spark в HDInsight. Для получения дополнительной информации о Livy, см.: [Отправить Spark рабочих мест удаленно с помощью Livy](../../hdinsight/spark/apache-spark-livy-rest-interface.md). 
...
репозитория [GitHub](https://raw.githubusercontent.com/Azure/Azure-MachineLearning-DataScience/master/Misc/Spark/Python/ConsumeGBNYCReg.py) в большой двоичный объект кластера Spark. Используйте **Microsoft Azure Storage Explorer** или **AzCopy**, чтобы скопировать сценарий в большой двоичный объект кластера. В нашем случае мы загрузили сценарий в ***wasb:///example/python/ConsumeGBNYCReg.py***.   
...
Вы также можете добавить этот код Python в [функции Azure,](https://azure.microsoft.com/documentation/services/functions/) чтобы вызвать представление о работе Spark, которое оценивает каплю на основе различных событий, таких как таймер, создание или обновление капли. 
...
* С портала Azure создайте новое приложение Logic App, выбрав **приложение «Новый** -> **Веб» и «Мобильный** -> **логика».** 
