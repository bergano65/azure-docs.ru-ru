...
description: Техническое руководство по шаблону решений для прогностического обслуживания в аэрокосмической отрасли, коммунальных услугах и транспорте.
...
ms.locfileid: "79477176"
---
# <a name="technical-guide-to-the-solution-template-for-predictive-maintenance-in-aerospace"></a>Техническое руководство по шаблону решений для прогностического обслуживания в аэрокосмической отрасли
...
Шаблоны решений предназначены для ускорения процесса создания демо E2E. Развернутый шаблон предусматривает подписку с необходимыми компонентами, а затем строит отношения между ними. Он также заполняет конвейер данных примерами данных из приложения генератора данных, которое будет скачано и установлено на локальном компьютере после развертывания шаблона решения. Данные из генератора будут заполнять конвейер данных и начнут создавать прогнозы машинного обучения, которые можно визуализировать на панели мониторинга Power BI.
...
Для этого шаблона используемый источник данных генерируется из загруженного настольного приложения, которое вы используете локально после успешного развертывания.
...
Выполнить скрипты [Hive](https://blogs.msdn.com/b/bigdatasupport/archive/2013/11/11/get-started-with-hive-on-hdinsight.aspx) (организованные Azure Data Factory) с помощью HDInsight для предоставления агрегирования необработанных событий, архивированных с помощью ресурса Azure Stream Analytics.
...
Используйте [базу данных Azure S'L](https://azure.microsoft.com/services/sql-database/) для хранения прогнозов, полученных машинным обучением Azure, которые затем потребляются в панели мониторинга [Power BI.](https://powerbi.microsoft.com)
...
### <a name="azure-stream-analytics"></a><a name="azure-stream-analytics-1"></a>Аналитика Azure Stream
Используйте ресурс Azure Stream Analytics для обеспечения аналитики в режиме реального времени, читая данные из потоков данных и выходя данные в любое количество источников.
...
* На последнем шаге конвейера выполняется запись данных (например, прогнозов из системы машинного обучения) в базу данных SQL. Появление данных в базе данных SQL может занять не более трех часов. Одним из способов мониторинга объема данных, доступных в базе данных SQL, является использование [портала Azure](https://portal.azure.com/). На левой панели найдите значок ![](./media/predictive-maintenance-technical-guide/icon-SQL-databases.png) S'L DATABASES S'L и нажмите на нее. Затем найдите базу данных **pmaintenancedb** и щелкните ее. На следующей странице внизу нажмите на MANAGE.
...
Power BI подключается к базе данных Azure S'L в качестве источника данных, где хранятся результаты прогнозирования. 
...
2.    На этом этапе необходимым условием является загрузка и установка свободного программного обеспечения [Power BI рабочего стола.](https://powerbi.microsoft.com/documentation/powerbi-desktop-get-the-desktop/)
...
1. Получите учетные данные базы данных.
   ...
   * Вы увидите новую вкладку/окно браузера, отображая страницу портала Azure. Щелкните **Группы ресурсов** на левой панели.
...
2. Обновите источник данных файла отчета холодного пути с помощью Power BI Desktop.
   ...
   * В окне выскочить заменить **"Сервер"** и **"База данных"** на свой собственный сервер и имена баз данных, а затем нажмите **кнопку "ОК".** Для имени сервера укажите порт 1433 (**имя_решения.database.windows.net, 1433**). В поле базы данных оставьте **pmaintenancedb**. Игнорируйте предупреждения, которые отображаются на экране.
   * В следующем всплывающем окне вы увидите два варианта на левой панели (**Windows** и **База данных**). Нажмите **кнопку «База данных»,** заполните имя пользователя и **«Password»** (имя пользователя и пароль, которые вы ввели при первом развертывании решения и создании базы данных Azure S'L). **'Username'** В разделе ***Выберите уровень, к которому следует применить эти параметры:*** выберите уровень базы данных. Затем нажмите **кнопку "Подключение"**.
...
3. (Необязательно) Опубликуйте панель мониторинга холодного пути в [Power BI Online](https://www.powerbi.com/). Этот шаг нуждается в счете Power BI (или office 365).
   ...
   * Чтобы создать новую панель **+** мониторинга: щелкните знак рядом с разделом **Dashboards** на левом стеку. Введите имя "Демонстрация прогнозируемого обслуживания" для этой новой панели мониторинга.
   * После открытия отчета щелкните ![Значок "Закрепить"](./media/predictive-maintenance-technical-guide/icon-pin.png), чтобы закрепить все визуализации на панели мониторинга. Подробные инструкции см. в статье [Закрепление плитки на панели мониторинга Power BI из отчета](https://support.powerbi.com/knowledgebase/articles/430323-pin-a-tile-to-a-power-bi-dashboard-from-a-report).
     ...
     <br/>
   * Чтобы запланировать обновление данных, нависните над набором данных](./media/predictive-maintenance-technical-guide/icon-elipsis.png) **PredictiveMaintenanceAerospace,** нажмите ![значок Ellipsis, а затем выберите Обновление **расписания.**
     <br/>
     > [!NOTE]
     > Если вы видите предупреждающее сообщение, нажмите **«Изменить учетные данные»** и убедитесь, что учетные данные базы данных совпадают с данными, описанными в шаге 1.
     <br/>
...
1. Вот несколько шагов, чтобы создать одну из плиток выше - "Вид флота датчика 11 против порога 48,26" плитка:
   ...
   * Нажмите значок **Pin Visual** в правом верхнем углу этой диаграммы. Может отобразиться окно "ПИН-код для панели мониторинга" для выбора панели мониторинга. Выберите "Демонстрация прогнозируемого обслуживания", а затем нажмите кнопку "Закрепить".
   * Нависает над мышью над этой плиткой на приборной панели, нажмите значок "изменить" в правом верхнем углу, чтобы изменить свое название на "Вид на фальсификат 11 против порога 48,26" и подзаголовок "Средний по флоту с течением времени".
