...
description: Техническое руководством по шаблону решения для прогнозного обслуживания в Aerospace, служебных программах и транспорте.
...
---
# <a name="technical-guide-to-the-solution-template-for-predictive-maintenance-in-aerospace"></a>Техническое руководством по шаблону решения для прогнозируемого обслуживания в Aerospace
...
Шаблоны решений предназначены для ускорения процесса создания демонстрации E2E. Развернутый шаблон подготавливает подписку с необходимыми компонентами, а затем создает связи между ними. Он также заполняет конвейер данных примерами данных из приложения генератора данных, которое будет скачано и установлено на локальном компьютере после развертывания шаблона решения. Данные из генератора будут заполнять конвейер данных и начнут создавать прогнозы машинного обучения, которые можно визуализировать на панели мониторинга Power BI.
...
Для этого шаблона используемый источник данных создается на основе загруженного настольного приложения, запускаемого локально после успешного развертывания.
...
Выполнение скриптов [Hive](https://blogs.msdn.com/b/bigdatasupport/archive/2013/11/11/get-started-with-hive-on-hdinsight.aspx) (управляемых фабрикой данных Azure) с помощью HDInsight для предоставления агрегатов необработанных событий, архивированных с помощью ресурса Azure Stream Analytics.
...
Используйте [базу данных SQL Azure](https://azure.microsoft.com/services/sql-database/) для хранения прогнозов, полученных машинное обучение Azure, которые затем используются на панели мониторинга [Power BI](https://powerbi.microsoft.com) .
...
Используйте ресурс Azure Stream Analytics, чтобы обеспечить аналитику практически в реальном времени путем считывания из потоков данных и вывода данных в любое количество источников.
...
* На последнем шаге конвейера выполняется запись данных (например, прогнозов из системы машинного обучения) в базу данных SQL. Появление данных в базе данных SQL может занять не более трех часов. Одним из способов мониторинга объема данных, доступных в базе данных SQL, является использование [портала Azure](https://portal.azure.com/). На панели слева щелкните значок ![](./media/predictive-maintenance-technical-guide/icon-SQL-databases.png) SQL базы данных SQL и выберите его. Затем найдите базу данных **pmaintenancedb** и щелкните ее. На следующей странице внизу щелкните Управление.
...
Примечание. 
1.    При развертывании решения прогноз будет отображаться в базе данных в течение 3 часов. PBIX-файл, поставляемый с загрузкой генератора, содержит некоторые начальные данные, поэтому можно сразу создать панель мониторинга Power BI. 
2.    На этом шаге необходимо загрузить и установить бесплатное программное обеспечение [Power BI Desktop](https://powerbi.microsoft.com/documentation/powerbi-desktop-get-the-desktop/).
...
1. Получите учетные данные базы данных.
   ...
   * Вы увидите новую вкладку или окно браузера, в котором отображается страница портал Azure. Щелкните **Группы ресурсов** на левой панели.
...
2. Обновите источник данных файла отчета холодного пути с помощью Power BI Desktop.
   ...
   * Во всплывающем окне замените **"Server"** и **"Database"** собственными именами сервера и базы данных, а затем нажмите кнопку **"ОК"**. Для имени сервера укажите порт 1433 (**имя_решения.database.windows.net, 1433**). В поле базы данных оставьте **pmaintenancedb**. Игнорируйте предупреждения, которые отображаются на экране.
   * В следующем всплывающем окне вы увидите два варианта на левой панели (**Windows** и **База данных**). Щелкните **"база данных"**, введите **"Username"** и **"Password"** (имя пользователя и пароль, введенные при первом развертывании решения и создании базы данных SQL Azure). В разделе ***Выберите уровень, к которому следует применить эти параметры:*** выберите уровень базы данных. Затем нажмите кнопку **"подключить"**.
...
3. (Необязательно) Опубликуйте панель мониторинга холодного пути в [Power BI Online](https://www.powerbi.com/). Для этого шага требуется учетная запись Power BI (или учетная запись Office 365).
   ...
   * Чтобы создать новую панель мониторинга, щелкните **+** знак рядом с разделом **панели мониторинга** на левой панели. Введите имя "Демонстрация прогнозируемого обслуживания" для этой новой панели мониторинга.
   * После открытия отчета щелкните ![Значок "Закрепить"](./media/predictive-maintenance-technical-guide/icon-pin.png), чтобы закрепить все визуализации на панели мониторинга. Подробные инструкции см. в статье [Закрепление плитки на панели мониторинга Power BI из отчета](https://support.powerbi.com/knowledgebase/articles/430323-pin-a-tile-to-a-power-bi-dashboard-from-a-report).
     ...
     <br/>
   * Чтобы запланировать обновление данных, наведите указатель мыши на набор данных **PredictiveMaintenanceAerospace** , ![щелкните значок](./media/predictive-maintenance-technical-guide/icon-elipsis.png) с многоточием и выберите **запланировать обновление**.
     <br/>
     > [!NOTE]
     > Если появится предупреждение, нажмите кнопку **изменить учетные данные** и убедитесь, что учетные данные базы данных совпадают с описанными в шаге 1.
     <br/>
...
1. Ниже приведены некоторые шаги для создания одного из элементов, приведенных выше: «предельное представление датчика 11 и порогового значения 48,26».
   ...
   * Наведите указатель мыши на эту плитку на панели мониторинга, щелкните значок "Изменить" в правом верхнем углу, чтобы изменить его заголовок на "предельное представление датчика 11 и пороговое значение 48,26" и подзаголовок в "среднее время перекрытия по времени".
