...
- На следующем уровне компании практикуют _профилактическое обслуживание_, при котором определяется полезный срок службы компонента. Он обслуживается или заменяется в случае выхода из строя. Профилактическое обслуживание позволяет избежать незапланированных и катастрофических сбоев. Но высокие затраты на запланированное время простоя, недостаточное использование компонента в течение его полезного срока службы, и труд все еще остаются.
...
В отношении данных истории сбоев обычно задают два вопроса: (1) "Сколько событий сбоя требуется для обучения модели?" и (2) "Сколько записей считается "достаточно"?" Окончательных ответов нет, а есть только правила большого пальца. Для первого варианта, чем больше количество неудачных событий, тем лучше модель. Для второго точное количество событий сбоя зависит от данных и контекста решаемой проблемы. Но с другой стороны, если механизм выходит из строя слишком часто, компания заменит его, что сократит число случаев сбоя. Здесь также важны рекомендации предметного специалиста. Однако существуют способы справиться с проблемой _редких событий_. Они обсуждаются в разделе [Обработка несбалансированных данных](#handling-imbalanced-data).
...
История обслуживания актива содержит подробную информацию о замененных компонентах, выполненных ремонтных мероприятиях и т.д. Эти события фиксируют закономерности деградации. Отсутствие этой важной информации в данных для обучения может привести к ошибочным результатам модели. Журнал сбоев также можно найти в журнале обслуживания в виде специальных кодов ошибок или дат заказа деталей. Дополнительные источники данных, которые влияют на шаблоны отказов, должны быть исследованы и предоставлены экспертами в предметной области.

#### <a name="machine-operating-conditions"></a>Условия работы компьютера
Потоковые данные с датчиков (или из других источников) используемого оборудования являются важным источником данных. Ключевым предположением в PdM является то, что состояние здоровья машины ухудшается с течением времени во время ее обычной работы. Ожидается, что данные содержат изменяющиеся со временем признаки, которые отражают эту временную закономерность, и любые аномалии, которые ведут к ухудшению работоспособности. Временной аспект данных необходим для того, чтобы алгоритм мог изучать шаблоны наличия и отсутствия сбоев с течением времени. Основываясь на этих точках данных, алгоритм учится прогнозировать, сколько еще единиц времени оборудование сможет продолжать работать, до того как выйдет из строя.
...
|:---------|---------------------------|
|_Задержка и отмена рейсов_ | Информацию о полете можно найти в маршруте полета и постраничных журналах. Данные о летных ногах включают в себя такие детали вылета/прибытия, время, аэропорт, прокладки и т.д. Журнал страниц ы включает в себя ряд кодов ошибок и технического обслуживания, записанных наземным обслуживающим персоналом.|
...
|_Сбой банкомата_ | Показания датчиков для каждой транзакции (прием наличных средств или чека) и выдачи наличных. Информация об измерении разрыва между нотами, толщина нот, расстояние прибытия заметок, атрибуты проверки и т.д. Записи обслуживания, которые предоставляют коды ошибок, информацию о ремонте, последний раз, когда дозатор наличных средств был пополнен.|
|_Сбои ветроэлектрической установки_ | Датчики отслеживали условия турбины, такие как температура, направление ветра, генерируемая мощность, скорость генератора и т.д. Данные собираются с нескольких ветровых турбин с ветряных электростанций, расположенных в различных регионах. Как правило, каждая установка передает несколько показаний измерений с фиксированным временным интервалом.|
|_Сбои автоматического выключателя_ | Журналы технического обслуживания, которые включают в себя ремонтные, профилактические и систематические мероприятия. Операционные данные, которые включают в себя автоматические и ручные команды, отправленные на автоматические выключатели, например для включения и отключения. Метаданные устройства, такие как дата изготовления, местоположение, модель и т.д. Характеристики выключателя цепи, такие как уровни напряжения, геолокация, условия окружающей среды.|
...
|_Поломка колес_ | Данные датчиков, измеряющие ускорение колес, случаи торможения, расстояние вождения, скорость и т.д. Статическая информация о колесах, как производитель, изготовленная дата. Данные о сбоях выводятся из базы данных заказа запчастей, которые отслеживают даты и количество заказов.|
...
Проектирование признаков — это первый шаг перед моделированием данных. Его роль в процессе обработки и анализа данных [описана в данном разделе](https://docs.microsoft.com/azure/machine-learning/team-data-science-process/create-features). _Признак_ является прогнозным атрибутом модели, например температурой, давлением, вибрацией и т. д. Для PdM инженерия функций включает абстрагирование работоспособности машины по сравнению с историческими данными, собранными в течение значительной продолжительности. В этом смысле оно отличается от удаленного мониторинга, обнаружения аномалий и сбоев. 
...
- _Отказ части двигателя самолета:_ подвижного средства, стандартное отклонение, и сумма за прошедший день, неделю и т.д. Эта метрика должна быть определена вместе с экспертом области бизнеса.
...
Для каждой помеченной записи актива определяется окно размера _W-<sub>k,</sub> _ где _k_ является числом окон размера _W._ Агрегаты затем создаются более _k_ _акробатика окна_ _W-K, W-<sub>(k-1)</sub>... , W-<sub>2</sub>, W-<sub>1</sub> _ для периодов до метки времени записи. Можно выбрать большое значение _k_ для записи шаблонов долгосрочного замедления или небольшое — для записи краткосрочных воздействий (см. рис. 2).
...
| Идентификатор актива | Time | \<Колонки> | Метка |
| ---- | ---- | --- | --- |
| A123 |День 1 | . . . | . |
| A123 |День 2 | . . . | . |
| ...  |...   | . . . | . |
...
> - Dong, G. Liu, H. (редакторы), Художественная инженерия для машинного обучения и анализа данных (Chapman & Hall/CRC Data Mining and Knowledge Discovery Series), CRC Press, 2018.
...
Двоичная классификация используется для _прогнозирования вероятности того, что часть оборудования выходит из строя в течение будущего периода времени_ - называется _будущим периодом горизонта X_. X определяется бизнес-проблемой и данными, на данных, в консультации с экспертом домена. Примеры:
...
![Рис. 3. Создание меток для двоичной классификации](./media/predictive-maintenance-playbook/labelling-for-binary-classification.png)
...
- _Задержки рейсов_: X могут быть выбраны в качестве одного дня, чтобы предсказать задержки в ближайшие 24 часа. Всем рейсам в период 24 часов до сбоя присвоена метка 1.
...
![Рис. 6. Метки прогнозирования корневой причины для многоклассовой классификации](./media/predictive-maintenance-playbook/labelling-for-multiclass-classification-for-root-cause-prediction.png)
...
![Рис. 7. Разделение по принципу зависимости от времени для двоичной классификации](./media/predictive-maintenance-playbook/time-dependent-split-for-binary-classification.png)
...
- [Точность](https://en.wikipedia.org/wiki/Accuracy_and_precision) является наиболее популярной метрикой, используемой для описания производительности классификатора. Но точность чувствительна к распределению данных и является неэффективной мерой для сценариев с несбалансированными наборами данных. Вместо этого используются другие метрики. Для вычисления и обоснования точности модели используются такие инструменты, как [матрица неточностей](https://en.wikipedia.org/wiki/Confusion_matrix).
...
| # | Заголовок | Описание |
|--:|:------|-------------|
| 2 | [Шаблон решения для прогнозного обслуживания Azure](https://github.com/Azure/AI-PredictiveMaintenance) | Шаблон решения с открытым исходным кодом, демонстрирующий моделирование ML Azure и полную инфраструктуру Azure, способную поддерживать сценарии прогнозного обслуживания в контексте удаленного мониторинга IoT. |
...
| 6 | [Набор средств для работы с искусственным интеллектом для Azure IoT Edge](https://github.com/Azure/ai-toolkit-iot-edge) | ИИ в IoT Edge с помощью TensorFlow; набор инструментов пакетики глубокого обучения моделей в Azure IoT Edge-совместимых докерконтейнеров и подвергать эти модели как REST AIS.
...
|:-------------------|--------------|
| [Схема обучения PdM с помощью деревьев и случайного леса](https://aischool.microsoft.com/learning-paths/1H5vH5wAYcAy88CoQWQcA8) | Public | 
| [Схема обучения PdM с помощью глубинного обучения](https://aischool.microsoft.com/learning-paths/FSIXxYkOGcauo0eUO8qAS) | Public |
| [Разработка искусственного интеллекта в Azure](https://azure.microsoft.com/training/learning-paths/azure-ai-developer) | Public |
| [Курсы по изучению искусственного интеллекта Майкрософт](https://aischool.microsoft.com/learning-paths) | Public |
| [Обучение работе с ИИ Azure на GitHub](https://github.com/Azure/connectthedots/blob/master/readme.md) | Public |
| [LinkedIn Learning](https://www.linkedin.com/learning) | Public |
| [Вебинары Microsoft AI YouTube](https://www.youtube.com/watch?v=NvrH7_KKzoM&t=4s) | Public |
| [Канал об искусственном интеллекте Майкрософт](https://channel9.msdn.com/Shows/AI-Show) | Public |
