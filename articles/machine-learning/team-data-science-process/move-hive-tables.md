...
* Создана учетная запись хранения Azure. Если вам нужны инструкции, [см.](../../storage/common/storage-introduction.md)
...
* **загрузить** их в учетную запись хранилища Azure (или соответствующего контейнера) по умолчанию; варианты такой учетной записи отображаются в [элементе хранения Azure с темой кластеров Azure HDInsight.](../../hdinsight/hdinsight-hadoop-use-blob-storage.md) Процесс отправки CSV-файла в контейнер по умолчанию в учетной записи хранения описан на этой [странице](hive-walkthrough.md#upload).

## <a name="how-to-submit-hive-queries"></a><a name="submit"></a>Как отправить запросы Hive
...
Если запрос Hive сложный и содержит несколько строк, редактировать запросы в командной строке или командной консоли Hive нецелесообразно. Альтернативой является использование текстового редактора в головном узле кластера Hadoop для сохранения запросов Hive в файле '.hql' в локальном каталоге головного узла. Затем запрос Hive в файле '.hql' может `-f` быть представлен с помощью аргумента следующим образом:
...
Вы также можете использовать консоль запросов (редактор Hive), введя URL-адрес формы *https:\//\<Hadoop кластера имя>.azurehdinsight.net/Home/HiveEditor* в веб-браузере. Чтобы увидеть эту консоль, необходимо выполнить вход, а для этого вам потребуются ваши учетные данные для кластера Hadoop.
...
Запросы Hive используются в [репозитории GitHub](https://github.com/Azure/Azure-MachineLearning-DataScience/tree/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_create_db_tbls_load_data_generic.hql) и могут быть загружены оттуда.
...
* имя базы данных : имя базы данных, которую вы хотите создать. ** \<\>** Если вы просто хотите использовать базу данных по умолчанию, запрос *«создать базу данных...»* может быть опущен.
* имя таблицы : имя таблицы, которую необходимо создать в указанной базе данных. ** \<\>** Если вы хотите использовать базу данных по умолчанию, \<таблица\>может быть непосредственно передана * \<по\> имени таблицы* без имени базы данных.
* **полевой сепаратор\>: сепаратор, который разграничивает поля в файле данных для загрузки в таблицу Hive. \<**
* **линии сепаратора\>: сепаратор, который разграничает линии в файле данных. \<**
* **место\>хранения : Расположение хранилища Azure для сохранения данных таблиц Hive. \<** Если не указывать параметр *LOCATION \<storage location\>*, база данных и таблицы сохраняются в каталоге *hive/warehouse/* в контейнере по умолчанию для кластера Hive по умолчанию. Если вы хотите указать место хранения, то оно должно находиться в пределах контейнера по умолчанию для базы данных и таблиц. Это место должно быть отнесено к местоположению относительно контейнера по умолчанию кластера в формате *"wasb:///\<каталог 1>/"* или *\<"wasb:/// каталог 1>/каталог\<2>/"* и т.д. После выполнения запроса относительные каталоги создаются в контейнере по умолчанию.
...
* Путь **к данным\>blob : Если файл blob, который будет загружен на таблицу Hive, находится в контейнере по умолчанию кластера HDInsight Hadoop, путь к данным blob должен быть в формате "wasb:// каталог в этом контейнере>/ имя файла капли>". \<** * \<\> * *\<\<* В дополнительном контейнере кластера Hadoop под управлением службы HDInsight также может быть файл большого двоичного объекта. В этом случае * \<путь к\> данным blob* должен быть в формате *"wasb://\<имя контейнера \<>\<имя учетной записи хранения>.blob.core.windows.net/ blob имя файла>".*
...
При запросе разделенных таблиц рекомендуется добавить условие раздела в **начале** `where` положения, что повышает эффективность поиска.
...
> [!NOTE]
> Если * \<имя\>таблицы textFILE .\< внешнее имя\> таблицы текстов* имеет разделы, в ШАГе 3 `SELECT * FROM <database name>.<external textfile table name>` команда выбирает переменную раздела в качестве поля в наборе возвращенных данных. Вставка его в * \<название\>базы данных .\< Имя таблицы\> ORC* не удается, так как * \<имя\>базы данных\< . Название\> таблицы ORC* не имеет переменной раздела в качестве поля в таблице схемы. В этом случае необходимо специально выбрать поля, которые должны быть вставлены в * \<имя\>базы данных.\< Название\> таблицы ORC* следующим образом:
>
>
...
Безопасно отказаться от * \<внешнего\> названия таблицы текстовых файлов* при использовании следующего запроса после того, как все данные были вставлены в * \<имя\>базы данных.\< Название\>таблицы ORC*:
