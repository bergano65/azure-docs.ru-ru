...
В отрасли программного обеспечения уже долгое время стандартом считается "Модульное тестирование". Но для науки о данных часто не ясно, что означает «модульное тестирование» и как следует тестировать код для различных этапов жизненного цикла науки о данных, таких как:
...
1. Настройте автоматическую сборку и тестирование в Azure DevOps:
    ...
    d. Присвойте сборке имя и выберите агент. Вы можете выбрать по умолчанию здесь, если вы хотите использовать DSVM для завершения процесса сборки. Дополнительные сведения о настройке агентов см. в статье [Build and Release Agents](https://docs.microsoft.com/azure/devops/pipelines/agents/agents?view=vsts) (Агенты сборки и выпуска).
    
    ![Настройки для сборки и агента](./media/code-test/select_agent.PNG)

    д) Выберите **+** в левом стеле, чтобы добавить задачу для этой фазы сборки. Поскольку мы собираемся запустить скрипт Python **test1.py** для завершения всех проверок, эта задача использует команду PowerShell для запуска кода Python.
    ...
    ж. Выберите **очередь Сохранить &** для завершения процесса сборки конвейера.

    ![Кнопка Save & queue (Сохранить и поместить в очередь)](./media/code-test/save_and_queue_build_definition.PNG)

Теперь при каждой новой фиксации в репозитории кода будет автоматически запускаться процесс сборки (Здесь мы используем мастер в качестве репозитория, но вы можете определить любую ветвь.) Процесс выполняет **test1.py** файл в машине агента, чтобы убедиться, что все, что определено в коде, работает правильно. 
