...
Определите тип прогнозов, которые необходимо сделать на основе анализа данных, чтобы помочь уточнить необходимые задачи процесса. Ниже приведены три примера проблем прогнозирования, которые мы будем решать в этом пошаговом руководстве, на основе *суммы TIP\_*:
...
Здесь рассматривается использование AzCopy для передачи файлов, содержащих данные. Чтобы скачать и установить AzCopy, следуйте инструкциям в статье [Приступая к работе со служебной программой командной строки AzCopy](../../storage/common/storage-use-azcopy.md).
...
* ***контейнер по умолчанию для кластера Hadoop>\<*** Контейнер по умолчанию, используемый кластером. Имя контейнера по умолчанию обычно совпадает с именем самого кластера. Например, если кластер называется abc123.azurehdinsight.net, контейнером по умолчанию будет abc123.
...
Ниже приведено содержимое файла **\_файла c:\temp\Sample\_Hive Create\_DB\_и\_Tables. HQL** , в котором создается база данных Hive **nyctaxidb**, а также таблицы для **прохода** и **FARE**.
...
**Пример\_\_данных\_\_загрузки Hive в файл partitions. HQL содержит следующие команды Load:\_** **LOAD**
...
В следующем **примере\_файла\_HQL с\_невысоких частотой использования Hive** показана команда для запуска:
...
Эта процедура состоит из двух шагов. Сначала мы соединяем таблицы **nyctaxidb. поездку** и **nyctaxidb. FARE** на трех ключах, которые находятся во всех записях: **Medallion**, **Лицензия хакера\_** и Дата и **время отправки\_**. Затем мы создаем метку двоичной классификации **tipped** и метку мультиклассовой классификации **tip\_class**.
...
Запрос применяет стандартные функции Hive напрямую для создания следующих параметров времени из поля **даты\_отправки** .
- час дня;
- неделя года;
...
- **Двоичная классификация**: для прогнозирования того, оплачивается ли TIP для поездки.
  ...
  **Используемое средство обучения:** двухклассовая логистическая регрессия.
  ...
  a. Для задачи целевой меткой (меткой класса) будет **tipped** (чаевые выплачены). В исходном наборе данных сокращенной выборки есть несколько столбцов, содержащих целевые утечки сведений для данного эксперимента по классификации. В частности, **класс\_TIP**, **значение\_TIP**и **\_общая сумма** раскрывают сведения о целевой метке, недоступные во время тестирования. Мы удаляем эти столбцы из рассмотрения с помощью модуля [Select Columns in Dataset][select-columns] (Выбор столбцов в наборе данных).
...
- **Мультиклассовая классификация**: прогнозирование диапазона сумм чаевых за поездку с использованием заранее определенных классов.
  ...
  **Используемое средство обучения:** мультиклассовая логистическая регрессия.
  ...
  a. Для этой проблемы наша конечная метка (или класс) является **классом TIP\_**, который может принимать одно из пяти значений (0, 1, 2, 3, 4). Как и в случае двоичной классификации, у нас есть несколько столбцов с утечкой целевой информации для этого эксперимента. В частности, **tipped**, **tip\_amount** и **total\_amount** раскрывают информацию о целевой метке, которая недоступна во время тестирования. Мы удаляем эти столбцы с помощью модуля [Select Columns in Dataset][select-columns] (Выбор столбцов в наборе данных).
