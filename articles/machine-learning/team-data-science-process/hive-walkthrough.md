...
- **Двоичная классификация**. Прогнозирование того, были ли выплачены чаевые за поездку. Т. е. значение *tip\_amount* больше 0 $ — это положительный пример, а значение *tip\_amount* 0 $ — отрицательный пример.
   
        Class 0: tip_amount = $0
        Class 1: tip_amount > $0
- **Классификация по нескольким классам**. Прогнозирование диапазона суммы чаевых за поездку. Мы разделяем *tip\_amount* на пять классов:
...
- **Задача регрессии.** Прогнозирование суммы чаевых, выплаченных за поездку.  
...
1. [Создание учетной записи хранения](../../storage/common/storage-quickstart-create-account.md). Эта учетная запись хранения используется для хранения данных в хранилище BLOB-объектов Azure. Здесь находятся также и данные, используемые в кластерах HDInsight.
...
3. [Создание рабочей области Машинного обучения Azure.](../studio/create-workspace.md) Эта рабочая область используется для создания моделей машинного обучения. Эта задача решается после начального исследования данных и сокращения выборки с использованием кластера HDInsight.
...
* ***\<Имя учетной записи хранения кластера Hadoop >*** учетной записи хранения, связанной с кластером HDInsight.
* ***\<по умолчанию контейнере кластера Hadoop >*** контейнера по умолчанию, используемой вашим кластером. Обратите внимание, что имя контейнера по умолчанию обычно совпадает с именем самого кластера. Например, если кластер называется abc123.azurehdinsight.net, контейнером по умолчанию будет abc123.
* ***\<ключ учетной записи хранения >*** ключ учетной записи хранения, используемой вашим кластером.
...
### <a name="exploration-view-the-top-10-records-in-table-trip"></a>Изучение. Просмотр 10 верхних записей в таблице trip
> [!NOTE]
> Обычно эту задачу выполняет специалист по обработке и анализу данных.
> 
> 
...
### <a name="exploration-view-the-number-of-records-in-each-of-the-12-partitions"></a>Изучение. Просмотр количества записей в каждом из 12 разделов
> [!NOTE]
> Обычно эту задачу выполняет специалист по обработке и анализу данных.
> 
> 
...
### <a name="exploration-assessing-data-quality-by-checking-for-invalid-longitude-or-latitude-records"></a>Изучение. Оценка качества данных через проверку наличия недопустимых записей долготы или широты
> [!NOTE]
> Обычно эту задачу выполняет специалист по обработке и анализу данных.
> 
> 
...
### <a name="exploration-binary-class-distributions-of-trip-tips"></a>Изучение. Двоичные классовые распределения чаевых за поездки
> [!NOTE]
> Обычно эту задачу выполняет специалист по обработке и анализу данных.
> 
> 
...
### <a name="exploration-class-distributions-in-the-multiclass-setting"></a>Изучение. Классовые распределения при мультиклассовой настройке
> [!NOTE]
> Обычно эту задачу выполняет специалист по обработке и анализу данных.
> 
> 
...
### <a name="exploration-compute-the-direct-distance-between-two-longitude-latitude-locations"></a>Изучение. Вычисление расстояния по прямой между двумя местоположениями, заданными долготой и широтой
> [!NOTE]
> Обычно эту задачу выполняет специалист по обработке и анализу данных.
> 
> 
...
**Пароль учетной записи пользователя Hadoop**. Пароль, выбранный для кластера (не пароль для удаленного доступа).

**Расположение выходных данных**. Выбирается Azure.
...
- **Двоичная классификация**. Прогнозирование того, были ли выплачены чаевые за поездку.

  **Используемое средство обучения.** Двухклассовая регрессионная логистическая модель.
  ...
  2. Для этого эксперимента распределение наших целевых меток составляло примерно 1:1.
...
- **Классификация по нескольким классам**. Прогнозирование диапазона сумм чаевых за поездку с использованием заранее определенных классов.

  **Используемое средство обучения.** Мультиклассовая регрессионная логистическая модель
  ...
  2. Для этого эксперимента мы используем матрицу неточностей для проверки точности прогнозов. Вот как это работает:
...
- **Задача регрессии.** Прогнозирование суммы чаевых, выплаченных за поездку.
  ...
  **Используемое средство обучения.** Увеличенное дерево принятия решений
  ...
  2. Для задач регрессии мы измеряем величины точности прогноза, рассматривая квадратичную ошибку в прогнозах и коэффициент детерминации.
