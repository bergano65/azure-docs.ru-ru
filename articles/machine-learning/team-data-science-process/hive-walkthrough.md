...
Для пошагового представления, которое показывает, [Team Data Science Process - Using Azure HDInsight Hadoop Clusters on a 1-TB dataset](hive-criteo-walkthrough.md)как обрабатывать больший набор данных, см.

Вы также можете использовать ноутбук IPython для выполнения задач, представленных в пошаговом шаге, который использует набор данных 1-TB. Дополнительные сведения см. в статье [Criteo walkthrough using a Hive ODBC connection](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/iPythonNotebooks/machine-Learning-data-science-process-hive-walkthrough-criteo.ipynb) (Пошаговое руководство Criteo по использованию подключения Hive ODBC).
...
- В trip_data файлов CSV содержатся сведения о поездке: количество пассажиров, точки сбора и высадки, продолжительность поездки и продолжительность поездки. Вот несколько примеров записей:
...
- В trip_fare csV файлы содержат подробную информацию о тарифе, оплаченном за каждую поездку: тип оплаты, стоимость проезда, доплата и налоги, советы и сборы, а также общая сумма уплаченных. Вот несколько примеров записей:
...
Определите, какие прогнозы вы хотите сделать на основе анализа данных, чтобы помочь прояснить требуемые задачи процесса. Вот три примера проблем прогнозирования, которые мы рассматриваем в этом пошаговом пошаге, все на основе *суммы\_чаевых:*
...
1. [Создайте учетную запись хранения:](../../storage/common/storage-account-create.md)Эта учетная запись хранилища используется для хранения данных в хранилище Azure Blob. Здесь находятся также и данные, используемые в кластерах HDInsight.
...
Здесь рассматривается использование AzCopy для передачи файлов, содержащих данные. Чтобы загрузить и установить AzCopy, следуйте инструкциям при [начале работы с утилитой командной строки AzCopy.](../../storage/common/storage-use-azcopy.md)

1. Из окна запроса команд запустите следующие команды AzCopy, * \<заменив path_to_data_folder>* нужным пунктом назначения:

        "C:\Program Files (x86)\Microsoft SDKs\Azure\AzCopy\azcopy" /Source:https://nyctaxitrips.blob.core.windows.net/data /Dest:<path_to_data_folder> /S

1. По завершении копирования выбранная папка данных будет содержать в общей сложности 24 ZIP-файла. Распакуйте загруженные файлы в тот же каталог на локальном компьютере. Обратите внимание на папку, в которой располагаются распакованные файлы. Эта папка называется * \<путь\_\_к\_\> unzipped_data файлов* в следующем.
...
* *** \<path_to_data_folder>*** Каталог (наряду с траекторией) на вашей машине, которая содержит незастегнутые файлы данных.  
* ***имя учетной записи хадупского кластера>\<*** Учетная запись хранилища, связанная с кластером HDInsight.
* ***по умолчанию контейнер кластера>Хадуп>\<*** Контейнер по умолчанию, используемый кластером. Название контейнера по умолчанию обычно совпадает с тем же именем, что и сам кластер. Например, если кластер называется abc123.azurehdinsight.net, контейнером по умолчанию будет abc123.
* ***ключ>учетной записи хранения \<*** Ключ для учетной записи хранилища, используемой кластером.
...
Чтобы подготовить кластер для анализа исследовательских данных, загрузите файлы '.hql', содержащие соответствующие скрипты Hive, из [GitHub](https://github.com/Azure/Azure-MachineLearning-DataScience/tree/master/Misc/DataScienceProcess/DataScienceScripts) в локальный каталог (C:'temp) на головном узле. Откройте запрос команды из головного узла кластера и запустите следующие две команды:

    set script='https://raw.githubusercontent.com/Azure/Azure-MachineLearning-DataScience/master/Misc/DataScienceProcess/DataScienceScripts/Download_DataScience_Scripts.ps1'

    @powershell -NoProfile -ExecutionPolicy unrestricted -Command "iex ((new-object net.webclient).DownloadString(%script%))"

Эти две команды загружают все файлы '.hql', необходимые в этом пошаговом пункте в местный каталог ***C: «темп&#92;*** в головном узеле.
...
> [!NOTE]
> Эта задача обычно предназначена для админа.
> 
> 
...
Из запроса каталога Hive запустите следующую команду в командной строке Hadoop головного узла, который создает базу данных Ивии и таблицы:

    hive -f "C:\temp\sample_hive_create_db_and_tables.hql"

Вот содержание **C: «темп-образец\_\_улья\_создать\_\_dB и tables.hql** файл, который создает hive базы данных **nyctaxidb**, и таблицы **поездки** и **тариф**.
...
* Таблица **поездки** содержит сведения о каждой поездке (детали водителя, время подъема, расстояние поездки и время).
* Таблица **тарифа** содержит информацию о тарифе (стоимость проезда, сумма чаевых, сборы и доплаты).
...
**Данные\_\_о\_нагрузке образца\_улья\_через файл partitions.hql** содержат следующие команды **LOAD:**

    LOAD DATA INPATH 'wasb:///nyctaxitripraw/trip_data_${hiveconf:MONTH}.csv' INTO TABLE nyctaxidb.trip PARTITION (month=${hiveconf:MONTH});
    LOAD DATA INPATH 'wasb:///nyctaxifareraw/trip_fare_${hiveconf:MONTH}.csv' INTO TABLE nyctaxidb.fare PARTITION (month=${hiveconf:MONTH});

Ряд запросов Hive, используемых здесь в процессе исследования, включают изучение только одного или двух разделов. Однако эти запросы можно выполнять по всему набору данных.
...
Записи можно сохранить в файле для удобного просмотра с небольшим изменением к предыдущему запросу:
...
Эта команда производит этот вывод:
...
Точно такое же количество поездок в месяц возвращается для обоих наборов данных, обеспечивая первую проверку правильной загрузки данных.
...
Общее количество записей в обеих таблицах также одинаково, что обеспечивает вторую проверку правильной загрузки данных.

### <a name="exploration-trip-distribution-by-medallion"></a>Просмотр: распределение поездок по параметру medallion
> [!NOTE]
> Этот анализ, как правило, задача ученого данных.
> 
> 

В этом примере определяются медальоны (номера такси) с более чем 100 поездками за заданный период времени. Этот запрос выигрывает от доступа к секционированной таблице, так как в нем содержится условие, заданное переменной раздела **month**. Результаты запроса записываются в локальный файл, **queryoutput.tsv**, на `C:\temp` головном узлах.
...
> [!NOTE]
> Эта задача обычно предназначена для специалиста по обработке данных.
> 
> 

При изучении набора данных мы часто хотим изучить распределение групп значений. В этом разделе приводится пример того, как сделать этот анализ для такси и водителей.
...
Результаты запроса записываются в локальный **файл, C:'temp-queryoutput.tsv**.
...
Аргумент *-S* этой команды скрывает окно состояния задач Hive Map/Reduce. Эта команда полезна, поскольку делает экранную печать вывода запроса Hive более читаемым.
...
Следующий **\_образец\_улья\_наконечником частоты.hql** файл показывает команду для запуска:
...
В предыдущем запросе R — это радиус Земли в милях, а число пи (pi) преобразуется в радианы. Точки долготы широты фильтруются для удаления значений, которые находятся далеко от области NYC.
...
Эта процедура состоит из двух шагов. Сначала мы присоединяемся к **nyctaxidb.trip** и **nyctaxidb.fare** таблицы на трех клавишах, которые присутствуют во всех записях: **медальон**, **взломать\_лицензии**, и **пикап\_datetime**. Затем мы создаем метку двоичной классификации **tipped** и метку мультиклассовой классификации **tip\_class**.
...
Запрос применяет стандартные функции Hive непосредственно для генерации следующих параметров времени из поля **даты пикапа:\_**
- час дня;
- неделя года;
- будний день ('1' означает понедельник, и '7' стоит на воскресенье)
...
**HCatalog сервера URI**: Если название кластера **abc123**, то используйте: https://abc123.azurehdinsight.net.
...
**Расположение выходных данных**: Выбрано для Azure.

**Имя учетной записи хранилища Azure:** имя учетной записи хранилища по умолчанию, связанного с кластером.

**Имя контейнера Azure:** имя контейнера по умолчанию для кластера, как правило, совпадает с именем кластера. Для кластера под названием **abc123**, название abc123.
...
Поскольку наши данные, отобранные по умолчанию, находятся в контейнере по умолчанию, полученный запрос Hive от Machine Learning прост. Это **SELECT * FROM nyctaxidb.nyctaxi\_downsampled\_data**.
...
- **Двоичная классификация**: Чтобы предсказать, был ли оплатить сярвые чаевые за поездку.

  **Используемое средство обучения:** двухклассовая логистическая регрессия.

  а. Для задачи целевой меткой (меткой класса) будет **tipped** (чаевые выплачены). В исходном наборе данных сокращенной выборки есть несколько столбцов, содержащих целевые утечки сведений для данного эксперимента по классификации. В частности, **\_наконечник класса,** **количество\_чаевых,** и общая **\_сумма** раскрыть информацию о целевой метки, которая не доступна во время тестирования. Мы удаляем эти столбцы из рассмотрения с помощью модуля [Select Columns in Dataset][select-columns] (Выбор столбцов в наборе данных).
...
- **Мультиклассовая классификация**: прогнозирование диапазона сумм чаевых за поездку с использованием заранее определенных классов.
  ...
  **Используемое средство обучения:** мультиклассовая логистическая регрессия.
  ...
  а. Для этой проблемы, наша целевая (или класса) метка **наконечник\_класса**, который может принять одно из пяти значений (0,1,2,3,4). Как и в случае двоичной классификации, у нас есть несколько столбцов с утечкой целевой информации для этого эксперимента. В частности, **tipped**, **tip\_amount** и **total\_amount** раскрывают информацию о целевой метке, которая недоступна во время тестирования. Мы удаляем эти столбцы с помощью модуля [Select Columns in Dataset][select-columns] (Выбор столбцов в наборе данных).
  ...
  b. Для этого эксперимента мы используем матрицу путаницы, чтобы посмотреть на точности предсказания, как показано здесь:

  ![Матрица неточностей](./media/hive-walkthrough/cxFmErM.png)

  В то время как класс точности на распространенные классы хороши, модель не делает хорошую работу "обучения" на редких классов.

- **Задача регрессии:** Предсказать количество чаевых, оплаченных за поездку.
...
Доступ к этому учебнику и включенным в него скриптам предоставляет корпорация Майкрософт на основании лицензии свободного ПО. Для получения дополнительной информации смотрите файл **LICENSE.txt** в каталоге примера кода на GitHub.
