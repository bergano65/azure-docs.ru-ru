...
Узнайте, как создать классификатор машинного обучения без написания единой строки кода с помощью конструктора. В этом примере описывается обучение **многоклассового дерева принятия решений** для прогнозирования дохода от взрослых переписей (> = 50 000 или < = 50 000).

Так как вопрос отвечает "какой из них?" Это называется проблемой классификации. Однако можно применить тот же фундаментальный процесс для решения любых проблем машинного обучения — регрессии, классификации, кластеризации и т. д.
...
Набор данных содержит 14 функций и один столбец меток. Существует несколько типов функций, включая числовые и по категориям. На следующей диаграмме показан фрагмент из набора данных: ![данных](media/how-to-designer-sample-classification-predict-income/sample3-dataset-1225.png)
