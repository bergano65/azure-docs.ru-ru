...
1. __Создайте__ целевой объект вычислений, если он еще не создан.
...
Машинное обучение Azure обладает различной поддержкой в различных целевых объектах вычислений. Жизненный цикл разработки обычной модели начинается с разработки и экспериментов с небольшим объемом данных. На этом этапе мы рекомендуем использовать локальную среду. Например, ваш локальный компьютер или облачную виртуальную машину. При увеличении объема обучения по большим наборам данных или распределенному обучению рекомендуется использовать Машинное обучение Azure COMPUTE для создания кластера с одним или несколькими узлами, который автоматически масштабируется при каждой отправке выполнения. Можно также подключить собственный вычислительный ресурс, хотя не все сценарии могут поддерживаться. Подробнее об этом рассказывается ниже.
...
Конвейеры машинного обучения создаются на основе нескольких **шагов**, которые являются отдельными вычислительными единицами в конвейере. Каждый шаг может выполняться независимо и использовать изолированные ресурсы вычислений. Такой подход позволяет нескольким специалистам по обработке данных работать с одним и тем же конвейером без избыточных вычислений, а также упрощает использование различных типов вычислений и размеров для каждого шага.
...
Теперь, когда вы присоединяете расчеты и настроили выполнение, следующим шагом является [Отправка обучающего запуска](#submit).
...
> [!TIP]
> Кластеры обычно масштабируются до 100 узлов при наличии достаточной квоты для требуемого числа ядер. По умолчанию кластеры настроены с поддержкой взаимодействия между узлами кластера для поддержки заданий MPI, например. Однако вы можете масштабировать кластеры до тысяч узлов, просто [вызывая запрос в службу поддержки](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest)и запросив список разрешений вашу подписку, рабочую область или конкретный кластер для отключения взаимодействия между узлами. 

Машинное обучение Azureное вычисление можно использовать повторно во всех запусках. Результаты вычислений могут совместно использоваться другими пользователями в рабочей области и сохранены между запусками, автоматически масштабировать узлы в зависимости от числа отправленных запусков и max_nodes, установленных в кластере. Параметр min_nodes управляет минимально доступными узлами.
...
1. **Подключить**: чтобы подключить существующую виртуальную машину в качестве целевого объекта вычислений, необходимо указать идентификатор ресурса, имя пользователя и пароль для виртуальной машины. Идентификатор ресурса виртуальной машины можно создать с помощью идентификатора подписки, имени группы ресурсов и имени виртуальной машины в следующем строковом формате:`/subscriptions/<subscription_id>/resourceGroups/<resource_group>/providers/Microsoft.Compute/virtualMachines/<vm_name>`
...
1. **Присоединиться**: чтобы подключить кластер hdinsight в качестве целевого объекта вычислений, необходимо указать идентификатор ресурса, имя пользователя и пароль для кластера hdinsight. Идентификатор ресурса кластера HDInsight можно создать с помощью идентификатора подписки, имени группы ресурсов и имени кластера HDInsight в следующем строковом формате:`/subscriptions/<subscription_id>/resourceGroups/<resource_group>/providers/Microsoft.HDInsight/clusters/<cluster_name>`
...
1. Щелкните значок плюса (+), чтобы добавить целевой объект вычислений. 
1. Введите имя для целевого объекта вычислений. 
1. Выберите тип вычислений, подсоединяемых для __обучения__:
...
## <a name="submit-training-run-using-azure-machine-learning-sdk"></a><a id="submit"></a>Отправка учебного прогона с помощью пакета SDK для Машинное обучение Azure
...
Дополнительные сведения об этих ```az ml``` командах интерфейса командной строки см. [в справочной документации](reference-azure-machine-learning-cli.md).
