...
1. **Прикрепить**: Чтобы прикрепить существующую виртуальную машину в качестве вычислительной цели, необходимо предоставить полностью квалифицированное доменное имя (ФЗДН), имя пользователя и пароль для виртуальной машины. В приведенном примере замените \<fqdn> на общедоступное полное доменное имя виртуальной машины или общедоступный IP-адрес. Замените \<username> и \<password> именем пользователя и паролем SSH для виртуальной машины.

    > [!IMPORTANT]
    > Следующие регионы Azure не поддерживают присоединение виртуальной машины с использованием общедоступного IP-адреса VM. Вместо этого используйте идентификатор менеджера `resource_id` ресурсов Azure VM с параметром:
    >
    > * Восточная часть США
    > * Западная часть США 2
    > * Центрально-южная часть США
    >
    > Идентификатор ресурса VM может быть построен с помощью идентификатора `/subscriptions/<subscription_id>/resourceGroups/<resource_group>/providers/Microsoft.Compute/virtualMachines/<vm_name>`подписки, имени группы ресурсов и имени VM, используя следующий формат строки: .
...
1. **Прикрепить:** Чтобы прикрепить кластер HDInsight в качестве вычислительной цели, необходимо укажите имя хоста, имя пользователя и пароль для кластера HDInsight. В следующем примере пакет SDK используется для присоединения кластера к рабочей области. В примере замените \<имя кластера> именем кластера. Замените \<username> и \<password> именем пользователя и паролем SSH для кластера.

    > [!IMPORTANT]
    > Следующие регионы Azure не поддерживают присоединение кластера HDInsight с использованием общедоступного IP-адреса кластера. Вместо этого используйте идентификатор диспетчера ресурсов Azure кластера с параметром: `resource_id`
    >
    > * Восточная часть США
    > * Западная часть США 2
    > * Центрально-южная часть США
    >
    > Идентификатор ресурса кластера может быть построен с помощью идентификатора подписки, имени группы ресурсов и имени кластера с помощью следующего формата строки: `/subscriptions/<subscription_id>/resourceGroups/<resource_group>/providers/Microsoft.HDInsight/clusters/<cluster_name>`
