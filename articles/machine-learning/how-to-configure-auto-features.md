...
3. На шаге "выработка компонентов" Аутомл сравнивает БЕРТ с базовым (набором функций слов) и определяет, будет ли БЕРТ предоставляться точность. Если он определяет, что БЕРТ работает лучше, чем базовый план, Аутомл использует БЕРТ для текста Добавление признаков в качестве оптимальной стратегии Добавление признаков и переходит к феатуризинг всем данным. В этом случае в окончательной модели вы увидите «Претраинедтекстдннтрансформер».
