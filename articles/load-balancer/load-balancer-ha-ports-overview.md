...
- Объединение правила балансировки нагрузки портов с высоким уровнем доступности и правила балансировки нагрузки портов без высокой доступности, указывающие на те же серверные конфигурации, не поддерживается, если не включен параметр с плавающим IP-адресом.
...
- Узнайте, как настроить порты высокой доступности для ILB через портал (туториал-лоад-баланцер-Стандард-интернал-портал. MDL # Create-a-Load-балансировщик-Rule), [PowerShell](load-balancer-get-started-ilb-arm-ps.md#create-the-configuration-rules-probe-and-load-balancer), [CLI](load-balancer-get-started-ilb-arm-cli.md#create-the-load-balancer-rule)или [шаблоны](load-balancer-get-started-ilb-arm-template.md).
