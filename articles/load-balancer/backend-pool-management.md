...
* с использованием сочетания IP-адреса и идентификатора ресурса виртуальной сети (VNET).

Для настройки внутреннего пула следует выбрать сетевую карту, если вы используете виртуальные машины и масштабируемые наборы виртуальных машин. Этот метод создает наиболее прямую связь между ресурсом и внутренним пулом. 

В сценариях, где сетевую карту использовать невозможно, например с контейнерами или объектами pod Kubernetes, для настройки внутреннего пула следует применять сочетание IP-адреса и идентификатора виртуальной сети.
...
В сценариях с использованием контейнеров или предварительно заполненных внутренних пулов с IP-адресами следует применять сочетание IP-адресов и виртуальной сети.
...
Все управление внутренним пулом выполняется непосредственно на объекте внутреннего пула, как видно из приведенных ниже примеров.
  ...
  >[!IMPORTANT] 
  >Сейчас эта функция доступна в предварительной версии со следующими ограничениями:
  >* не более 100 добавляемых IP-адресов;
...
### <a name="resource-manager-template"></a>Шаблон Resource Manager
Создайте подсистему балансировки нагрузки, внутренний пул и заполните внутренний пул внутренними адресами:
```
{
...
Создайте виртуальную машину и подключенный сетевой интерфейс. Задайте для сетевого интерфейса один из внутренних IP-адресов:
```
{
