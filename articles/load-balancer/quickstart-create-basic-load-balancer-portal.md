...
Сначала создайте общедоступную подсистему балансировки нагрузки уровня "Базовый" с помощью портала. Имя и общедоступный IP-адрес, созданный вами, автоматически настроен в качестве внешнего интерфейса подсистемы балансировки нагрузки.
...
1. На панели **Создание подсистемы балансировки нагрузки** введите или выберите следующие значения:
   ...
   - **Общедоступный IP-адрес**. Выберите **Создать новый**. 
     - Поле **Общедоступный IP-адрес**. Введите *MyPublicIP*.
     - **Настроить общедоступный IP-адрес** > **Назначение**. Выберите **Динамический**.
   - **ResourceGroup**. Выберите **Создать новый**, а затем введите *MyResourceGroupLB* и нажмите кнопку **ОК**. 
...
1. На панели **Создание виртуальной сети** введите или выберите следующие значения:
   ...
   - **Имя**. Введите *MyVnet*.
   - **ResourceGroup**. В раскрывающемся списке **Выбрать существующее** выберите **MyResourceGroupLB**. 
   - **Подсети** > **Имя**. Введите *MyBackendSubnet*.
...
1. В разделе **Создание виртуальной машины** введите или выберите следующие значения на вкладке **Основные сведения**:
   - **Подписка** > **Группа ресурсов**. Раскройте список и выберите **MyResourceGroupLB**.
   ...
   - **Учетная запись администратора** > **Имя пользователя**. Введите *azureuser*.
   - **Учетная запись администратора** > **Пароль**. Введите *Azure1234567*. 
     В поле **Подтверждение пароля** введите пароль еще раз.
   
1. Выберите вкладку **Сети** или **Next: Disks** (Далее: диски), а затем **Next: Networking** (Далее: сеть). 
...
1. На странице **Добавление серверного пула** введите или выберите следующие значения:
   ...
   - **Сопоставлено с**. Раскройте список и выберите значение **Группа доступности**.
   - **Группа доступности**. Выберите значение **MyAvailabilitySet**.
...
1. На странице **Add a health probe** (Добавление пробы работоспособности) введите или выберите следующие значения:
   ...
   - **Протокол**. Раскройте список и выберите **HTTP**. 
   - **Порт**. Введите *80*. 
...
На каждой виртуальной машине откройте браузер и нажмите кнопку **ОК** в качестве ответа на любой запрос по настройке. 

Вставьте общедоступный IP-адрес подсистемы балансировки нагрузки в адресную строку браузера. В браузере появится страница веб-сервера IIS по умолчанию.
