...
* Измените выделение портов [SNAT](https://docs.microsoft.com/azure/load-balancer/load-balancer-outbound-connections#-sharing-ports-across-resources) .
...
### <a name="scenario-2-modify-snat-port-allocation"></a><a name="scenario2out"></a>Сценарий 2. изменение распределения портов [SNAT](https://docs.microsoft.com/azure/load-balancer/load-balancer-outbound-connections#-sharing-ports-across-resources)
...
При возникновении нехватки SNAT увеличьте число портов [SNAT](https://docs.microsoft.com/azure/load-balancer/load-balancer-outbound-connections#-sharing-ports-across-resources) , заданное по умолчанию (1024). 


Каждый общедоступный IP-адрес участвует в 64 000 временных портов. Число виртуальных машин в серверном пуле определяет количество портов, передаваемых на каждую виртуальную машину. Одна виртуальная машина во внутреннем пуле имеет доступ к максимуму 64 000 портов. Для двух виртуальных машин максимальное число портов [SNAT](https://docs.microsoft.com/azure/load-balancer/load-balancer-outbound-connections#-sharing-ports-across-resources) (32 000) может быть задано с помощью исходящего правила (2x 32 000 = 64 000). 


Для настройки портов SNAT, заданных по умолчанию, можно использовать правила для исходящего трафика. Вы предоставляете больше или меньше, чем выделение портов [SNAT](https://docs.microsoft.com/azure/load-balancer/load-balancer-outbound-connections#-sharing-ports-across-resources) по умолчанию. Каждый общедоступный IP-адрес из внешнего интерфейса правила для исходящего трафика составляет до 64 000 временных портов для использования в качестве портов [SNAT](https://docs.microsoft.com/azure/load-balancer/load-balancer-outbound-connections#-sharing-ports-across-resources) . 


Балансировщик нагрузки предоставляет порты [SNAT](https://docs.microsoft.com/azure/load-balancer/load-balancer-outbound-connections#-sharing-ports-across-resources) , кратные 8. Если вы предоставляете значение, которое не делится на 8, операция конфигурации будет отклонена. Каждое правило балансировки нагрузки и правило NAT для входящего трафика будут использовать диапазон из 8 портов. Если в правиле балансировки нагрузки или входящего трафика NAT используется один и тот же диапазон 8, никакие дополнительные порты не будут использоваться.


Если вы попытаетесь предоставить больше портов [SNAT](https://docs.microsoft.com/azure/load-balancer/load-balancer-outbound-connections#-sharing-ports-across-resources) , чем доступно в зависимости от числа общедоступных IP-адресов, операция настройки отклоняется. Например, если вы выдаете 10 000 портов на виртуальную машину и семь виртуальных машин в серверном пуле совместно используют один общедоступный IP-адрес, конфигурация отклоняется. Семь, умноженное на 10 000, превышает предельное число портов 64 000. Добавьте дополнительные общедоступные IP-адреса на интерфейсное правило исходящего трафика, чтобы включить этот сценарий. 


Вернитесь к [выделению порта по умолчанию](load-balancer-outbound-connections.md#preallocatedports) , указав 0 в качестве числа портов. Первые 50 экземпляров виртуальных машин получают порты 1024, 51-100 512 экземпляров виртуальных машин — до максимума. Дополнительные сведения о выделении портов SNAT по умолчанию см. в разделе [Таблица распределения портов SNAT](https://docs.microsoft.com/azure/load-balancer/load-balancer-outbound-connections#preallocatedports).
...
Используйте префикс или общедоступный IP-адрес для масштабирования портов [SNAT](https://docs.microsoft.com/azure/load-balancer/load-balancer-outbound-connections#-sharing-ports-across-resources) . Добавьте источник исходящих соединений в список разрешений или запретов.
...
1. Отключите исходящий [SNAT](https://docs.microsoft.com/azure/load-balancer/load-balancer-outbound-connections#-sharing-ports-across-resources) в правиле балансировки нагрузки. 
