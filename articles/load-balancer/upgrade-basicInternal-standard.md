...
* Создает стандартный внутренний номер SKU Load Balancer в указанном расположении. Обратите внимание, что [исходящее подключение](./load-balancer-outbound-connections.md) не будет предоставляться стандартным внутренним Load Balancer.
...
* Сценарий поддерживает только внутреннее Load Balancer обновление, когда исходящие подключения не требуются. Если для некоторых виртуальных машин требуется [исходящее подключение](./load-balancer-outbound-connections.md) , обратитесь к этой [странице](upgrade-InternalBasic-To-PublicStandard.md) за инструкциями. 
