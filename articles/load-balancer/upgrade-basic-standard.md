...
* Создает исходящие правила по умолчанию, которое позволяет исходящих подключения.

### <a name="caveatslimitations"></a>Ограничения оговорок

* Сценарий поддерживает только обновление баланса общественной нагрузки. Для обновления внутреннего базового баланса нагрузки, обратитесь к [этой странице](https://docs.microsoft.com/azure/load-balancer/upgrade-basicinternal-standard) для получения инструкций.
...
Скачать скрипт миграции из [галереи PowerShell](https://www.powershellgallery.com/packages/AzurePublicLBUpgrade/2.0).
