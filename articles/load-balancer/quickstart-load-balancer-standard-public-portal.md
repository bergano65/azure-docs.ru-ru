...
В этом разделе вы создадите службу Load Balancer (цен. категория "Стандартный"), помогающую распределять нагрузку виртуальных машин. Load Balancer (цен. категория "Стандартный") можно использовать в качестве внешней или внутренней подсистемы балансировки нагрузки. Load Balancer (цен. категория "Стандартный") поддерживает только общедоступные IP-адреса уровня "Стандартный". Общедоступные IP-адреса уровня "Базовый" не поддерживаются. Вместе с общедоступным экземпляром Load Balancer (цен. категория "Стандартный") необходимо также создать новый общедоступный IP-адрес уровня "Стандартный", настроенный в качестве внешнего интерфейсного (с именем *LoadBalancerFrontend* по умолчанию). 
