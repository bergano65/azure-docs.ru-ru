...
> [!IMPORTANT]
> Подсистема балансировки нагрузки в разных регионах сейчас доступна в предварительной версии и может быть развернута на портале. Войдите в, чтобы **https://preview.portal.azure.com** Просмотреть и развернуть компонент.. </br> </br>
...
* В настоящее время служба kubernetes Azure (AKS) не может быть интегрирована с Load Balancer в разных регионах. При настройке Load Balancer в нескольких регионах перед общедоступным Load Balancer, развернутым с помощью AKS, должно возникать утрата подключения.
