...
4. В правиле режим распределения изменяется путем изменения раскрывающегося списка **сохраняемость сеанса** .  Доступны следующие параметры.
...
Как было описано выше, установите для `LoadBalancerDistribution` элемента значение sourceIP для сходства двух кортежей, sourceIPProtocol для сходства с тремя кортежями или None для отсутствия сходства (сходство с пятью кортежами).
