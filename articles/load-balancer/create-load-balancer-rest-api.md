...
|---------|---------|---------|---------|--------|
|subscriptionId   |  path       |  True       |   string      |  Учетные данные подписки, позволяющие уникально идентифицировать подписку Microsoft Azure. Идентификатор подписки формирует часть URI для каждого вызова службы.      |
|resourceGroupName     |     path    | True        |  string       |   Имя группы ресурсов.     |
|loadBalancerName     |  path       |      True   |    string     |    Имя подсистемы балансировки нагрузки.    |
|api-version    |   запрос     |  True       |     string    |  Версия API клиента.      |
