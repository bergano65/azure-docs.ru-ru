...
description: Узнайте, как устранять известные проблемы с Azure Load Balancer.
...
На этой странице содержатся сведения об устранении неполадок для основных и стандартных вопросов Azure Load Balancer. См. дополнительные сведения в статье [Обзор Azure Load Balancer уровня "Стандартный" (предварительная версия)](load-balancer-standard-diagnostics.md).
...
Когда внешние клиенты на внутренних виртуальных машинах проходят через подсистему балансировки нагрузки, для связи будут использоваться IP-адреса клиентов. Убедитесь, что IP-адрес клиентов добавлен в список разрешений NSG. 
...
Для общедоступной подсистемы балансировки нагрузки IP-адреса клиентов Интернета будут использоваться для обмена данными между клиентами и серверными виртуальными машинами подсистемы балансировки нагрузки. Убедитесь, что IP-адрес клиентов разрешен в группе безопасности сети серверной виртуальной машины.

1. Откройте список групп безопасности сети, настроенных на виртуальной машине внутреннего пула. Дополнительные сведения см. в разделе [Управление группами безопасности сети](../virtual-network/manage-network-security-group.md) .
