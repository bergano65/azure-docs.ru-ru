...
description: Узнайте, как устранить известные проблемы с Помощью Azure Load Balancer.
...
Эта страница предоставляет информацию об устранении неполадок для основных и стандартных общих вопросов баланса загрузки Azure. См. дополнительные сведения в статье [Обзор Azure Load Balancer уровня "Стандартный" (предварительная версия)](https://docs.microsoft.com/azure/load-balancer/load-balancer-standard-diagnostics).
...
Когда внешние клиенты бэкэнд-вмпредов проходят через баланселизатор нагрузки, IP-адрес клиентов будет использоваться для связи. Убедитесь, что IP-адрес клиентов добавляется в список разрешений NSG. 
...
* Если в результате этих проверок связи ответ не получен, то выполните следующие действия:
    ...
    - Проанализируйте запись сетевых данных и проверьте наличие входящих и исходящих пакетов, связанных с запросом проверки связи. 
        ...
        - Если в пуле бэкэнда VM не наблюдается исходящих пакетов, VM необходимо проверить на наличие несвязанных проблем (например, приложение, блокирующее порт зонда). 
...
Для общественного баланса нагрузки, IP-адрес интернет-клиентов будет использоваться для связи между клиентами и балансом нагрузки бэкэнд VMs. Убедитесь, что IP-адрес клиентов разрешен в группе сетевой безопасности бэкэнда VM.

1. Откройте список групп безопасности сети, настроенных на виртуальной машине внутреннего пула. Для получения дополнительной информации [см.](../virtual-network/manage-network-security-group.md)
...
## <a name="symptom-cannot-change-backend-port-for-existing-lb-rule-of-a-load-balancer-which-has-vm-scale-set-deployed-in-the-backend-pool"></a>Симптом: не может изменить порт бэкэнда для существующего правила LB балансоилира нагрузки, который имеет набор шкалы VM, развернутый в пуле бэкэнда. 
### <a name="cause--the-backend-port-cannot-be-modified-for-a-load-balancing-rule-thats-used-by-a-health-probe-for-load-balancer-referenced-by-vm-scale-set"></a>Причина : Порт бэкэнда не может быть изменен для правила балансировки нагрузки, которое используется зондом работоспособности для балансиваля нагрузки, на которое ссылается набор шкалы VM.
**Резолюция** Для того, чтобы изменить порт, можно удалить зонд работоспособности, обновив набор шкалы VM, обновить порт, а затем настроить зонд работоспособности снова.
