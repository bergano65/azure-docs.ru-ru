...
#### <a name="is-the-data-path-up-and-available-for-my-load-balancer-frontend"></a>Является ли путь данных вверх и доступны для моего грузоподъемность Frontend?
<details><summary>Разверните</summary>

Метрика доступности доступности данных описывает работоспособность пути данных в регионе к узлам вычислений, где расположены ваши вс-банк. Эта метрика отражает работоспособность инфраструктуры Azure. Используя эту метрику, можно:
...
- определить, к чему относится событие: к службе или к базовой плоскости данных. Не путайте эту метрику со статусом зонда работоспособности ("Доступность бэкенд-инстанции").
...
Доступность datapath не удается по следующим причинам:
...
#### <a name="are-the-backend-instances-for-my-load-balancer-responding-to-probes"></a>Являются ли Бэкэнд-Инстанции для моего баланса нагрузки, реагирующие на зонды?
<details>
...
*Рисунок: Количество подключений к балансую sNAT*
</details>
...
*Рисунок: Количество балансового баланса нагрузки SYN*
</details>
...
*Рисунок: Количество байт-баланса нагрузки*
</details>
...
Вы можете сделать еще один шаг вперед и использовать метрику доступности «Путь данных», чтобы получить представление о том, как Azure рассматривает работоспособность базовой плоскости данных, отвечающего за конкретное развертывание. Если вы объедините обе метрики, можно будет определить, где возникла ошибка, как показано в этом примере:
...
- Инфраструктура, в которой размещалось ваши ВМ, была недоступна и составляла 0 процентов в начале графика. Позже инфраструктура была здоровой, и ВМ были доступны, и более одного VM был помещен в задней части. На эту информацию указывает синий след доступности пути данных, который позже был на 100 процентов. 
- Состояние зонда работоспособности, указанное фиолетовым следом, находится на уровне 0 процентов в начале графика. Обведенный области в зеленых подчеркивается, где состояние зонда здоровья стало здоровым, и в этот момент развертывание клиента был в состоянии принять новые потоки.
