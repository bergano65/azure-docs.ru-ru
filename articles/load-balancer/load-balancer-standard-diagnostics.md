...
Метрика доступности пути к данным определяет работоспособность пути к данным в пределах региона до узла вычислений, на котором находятся виртуальные машины. Эта метрика отражает работоспособность инфраструктуры Azure. Используя эту метрику, можно:
...
Доступность доступности пути к файлам завершается сбоем по следующим причинам:
...
<details>
  <summary>Разверните</summary>
Метрика использования SNAT показывает, сколько уникальных потоков установлено между источником Интернета и серверной виртуальной машиной или масштабируемым набором виртуальных машин, который находится за подсистемой балансировки нагрузки и не имеет общедоступного IP-адреса. Сравнивая это с метрикой распределения SNAT, можно определить, испытывает ли ваша служба нехватка SNAT или нет, и что приведет к сбою исходящего потока. 

Если метрики указывают на риск сбоя [исходящего потока](https://aka.ms/lboutbound) , сослаться на статью и принять меры по ее устранению, чтобы обеспечить работоспособность службы.
...
1. Выберите **Использование SNAT** и (или) **выделение SNAT** в качестве типа метрики и **среднего** значения в качестве агрегата.
    * По умолчанию это среднее число портов SNAT, выделенных или используемых для каждой серверной виртуальной машины или Вмссес, соответствующее всем интерфейсным общедоступным IP-адресам, сопоставленным с Load Balancer, агрегированным по протоколам TCP и UDP.
...
| Недоступно | Ресурс "Стандартный балансировщик нагрузки" неисправен. Для диагностики работоспособности выберите **Azure Monitor**  >  **метрики**Azure Monitor.<br>(Состояние "*недоступно* " может также означать, что ресурс не подключен к вашей стандартной подсистеме балансировки нагрузки.) |
