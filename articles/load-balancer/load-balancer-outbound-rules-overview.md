---
title: Правила выезда - Балансер загрузки Azure
description: С помощью этого пути обучения, начать использовать исходящие правила для определения исходящих сетевых адресных переводов.
...
- отправить ли сбросить TCP на простой тайм-аут
...
**передний** + **параметры** + **бэкэнд бассейн**
...
### <a name="tune-snat-port-allocation"></a><a name="snatports"></a> Настройка распределения портов SNAT
...
Каждый общедоступный IP-адрес из всех внешних интерфейсов правила исходящего трафика обеспечивает до 64 000 эфемерных портов в качестве портов SNAT.  Load Balancer выделяет порты SNAT в количестве, кратном 8. Если вы предоставляете значение, которое не делится на 8, операция конфигурации будет отклонена.  Если вы попытаетесь выделить больше портов SNAT, чем доступно на основе количества общедоступных IP-адресов, операция конфигурации будет отклонена.  Например, если вы выделяете 10 000 портов на VM и 7 ВМ в пулбэка, будет делиться одним общедоступным IP-адресом, конфигурация отклоняется (7 x 10 000 портов SNAT > 64 000 sNAT портов).  Для реализации этого сценария можно добавить больше общедоступных IP-адресов во внешний интерфейс правила для исходящего трафика.

Вы можете вернуться к [автоматическому распределению SNAT-портов на основе размера серверного пула](load-balancer-outbound-connections.md#preallocatedports), указав значение 0 для количества портов. В этом случае первые 50 экземпляров VM получат 1024 порта, 51-100 экземпляров VM получат 512 экземпляров и так далее в соответствии с таблицей.

### <a name="control-outbound-flow-idle-timeout"></a><a name="idletimeout"></a> Управление временем ожидания перед переходом в режим простоя для исходящего потока
...
### <a name="enable-tcp-reset-on-idle-timeout"></a><a name="tcprst"></a><a name="tcpreset"></a> Включить сбрасывание TCP на простое тайм-аут
...
Просмотрите [сбросить TCP на простое тайм-аут](https://aka.ms/lbtcpreset) для получения подробной информации, включая доступность региона.

### <a name="support-both-tcp-and-udp-transport-protocols-with-a-single-rule"></a><a name="proto"></a> Поддержка транспортных протоколов TCP и UDP с помощью единого правила
...
### <a name="disable-outbound-nat-for-a-load-balancing-rule"></a><a name="disablesnat"></a> Отключение NAT для исходящего трафика для правила балансировки нагрузки
...
### <a name="groom-outbound-connections-to-a-specific-set-of-public-ip-addresses"></a><a name="groom"></a> Оптимизация исходящих подключений для определенного набора общедоступных IP-адресов
...
### <a name="modify-snat-port-allocation"></a><a name="modifysnat"></a> Изменение распределения портов SNAT
...
### <a name="enable-outbound-only"></a><a name="outboundonly"></a> Включение только NAT для исходящего трафика
...
- Правила исходящих могут применяться только к первичной конфигурации IP-излгу NIC.  Несколько NICs поддерживаются.

## <a name="next-steps"></a>Дальнейшие действия

- Подробнее об использовании [load Balancer для исходящих соединений.](load-balancer-outbound-connections.md)
