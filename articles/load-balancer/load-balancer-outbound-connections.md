...
| Номера SKU | Сценарий | Метод | Протоколы IP | Описание |
| --- | --- | --- | --- | --- |
| Ценовые категории "Стандартный" и "Базовый" | [1. VM с общественным IP-адресом уровня instance (с или без баланса нагрузки)](#ilpip) | SNAT, маскировка портов не используется | TCP, UDP, ICMP, ESP | Azure использует общедоступный IP-адрес, назначенный IP-конфигурации сетевой карты в экземпляре. Доступны все временные порты экземпляра. При использовании Standard Load Balancer [правила исходящего](load-balancer-outbound-rules-overview.md) не поддерживаются, если в виртуальную машину присваивается общедоступный IP. |
| Ценовые категории "Стандартный" и "Базовый" | [2. Общественный баланс, связанный с VM (без публичного IP-адреса на примере)](#lb) | SNAT с маскировкой портов (PAT) с использованием подсистем балансировки нагрузки | TCP, UDP |Azure предоставляет общедоступные IP-адреса общедоступных подсистем балансировки нагрузки нескольким частным IP-адресам. Кроме того, Azure использует временные порты этих подсистем для PAT. Следует использовать [правила исходящего](load-balancer-outbound-rules-overview.md) для четкого определения исходящих подключений. |
| Ценовая категория "Базовый" или отсутствие номера SKU | [3. Автономный VM (без баланса нагрузки, без публичного IP-адреса)](#defaultsnat) | SNAT с маскировкой портов (PAT) | TCP, UDP | Azure автоматически назначает общедоступный IP-адрес для SNAT, предоставляет его нескольким частным IP-адресам в группе доступности и использует временные порты общедоступного адреса. Это резервный сценарий для первых двух. Мы не рекомендуем его использовать, если необходимо контролировать использование адресов и управлять им. |
...
### <a name="scenario-1-vm-with-public-ip-address"></a><a name="ilpip"></a>Сценарий 1: VM с публичным IP-адресом

В этом сценарии, VM имеет общественный IP назначен к ему. Так как здесь рассматриваются исходящие подключения, балансировка нагрузки виртуальной машины не имеет значения. Этот сценарий приоритетнее остальных. При использовании публичного IP-адреса VM использует общедоступный IP-адрес для всех исходящих потоков.  
...
Если приложение инициирует множество исходящих потоков и вы испытываете исчерпание порта SNAT, подумайте о назначении [публичного IP-адреса для смягчения ограничений SNAT.](#assignilpip) Сведения о проблеме см. в разделе [Управление нехваткой портов SNAT (PAT)](#snatexhaust).

### <a name="scenario-2-load-balanced-vm-without-a-public-ip-address"></a><a name="lb"></a>Сценарий 2: Сбалансированная с нагрузкой ВМ без публичного IP-адреса
...
Если вы не завершите эту конфигурацию правила, поведение, как описано в сценарии для [Standalone VM без публичного IP.](#defaultsnat) Для правильной работы правила не обязательно, чтобы был прослушиватель во внутреннем пуле для успешной проверки работоспособности.
...
Порты SNAT предварительно распределены, как описано в разделе [Понимание SNAT и PAT.](#snat) Они представляют собой ограниченный ресурс, который можно исчерпать. Важно понимать, как они [используются](#pat). Чтобы понять, как спроектировать архитектуру и избежать этой проблемы, см. сведения в разделе [Управление нехваткой портов SNAT (PAT)](#snatexhaust).
...
### <a name="multiple-combined-scenarios"></a><a name="combinations"></a>Несколько объединенных сценариев
...
### <a name="multiple-frontends-for-outbound-flows"></a><a name="multife"></a>Несколько интерфейсов для исходящих потоков
...
### <a name="availability-zones"></a><a name="az"></a>Зоны доступности
...
## <a name="understanding-snat-and-pat"></a><a name="snat"></a>Основные сведения о SNAT и PAT
...
#### <a name="tcp-snat-ports"></a><a name="tcp"></a>Порты SNAT TCP
...
#### <a name="udp-snat-ports"></a><a name="udp"></a>Порты SNAT UDP
...
После того, как порт был освобожден, порт доступен для повторного использования по мере необходимости.  Порты SNAT можно считать последовательностью от самого низкого к самому высокому доступному для данного сценария, и первый доступный порт SNAT используется для новых соединений. 
...
### <a name="ephemeral-port-preallocation-for-port-masquerading-snat-pat"></a><a name="preallocatedports"></a>Предварительное выделение временных портов для SNAT с маскировкой портов (PAT)
...
>[!NOTE]
> При использовании Load Balancer (цен. категории "Стандартный") с [множеством интерфейсов](load-balancer-multivip-overview.md) каждый внешний IP-адрес увеличивает количество доступных портов SNAT в предыдущей таблице. Например, бэкэнд бассейн 50 ВМ с 2 правилами балансировки нагрузки, каждый с отдельным IP-адресом переднего энда, будет использовать 2048 (2x 1024) SNAT портов в каждом правиле. См. подробности в разделе [Несколько интерфейсов для исходящих потоков](#multife).
...
- Если какой-либо сервер/клиент отправляет FINACK, порт SNAT будет выпущен через 240 секунд.
...
## <a name="problem-solving"></a><a name="problemsolving"></a> Решение проблем 
...
[Эфемерные порты,](#preallocatedports) используемые для [PAT,](#pat) являются исчерпывающим ресурсом, как описано в [Standalone VM без публичного IP-адреса](#defaultsnat) и [сбалансированного VM без публичного IP-адреса.](#lb) Вы можете контролировать использование эфемерных портов и сравнивать с текущим распределением, чтобы определить риск или подтвердить exhuastion SNAT с помощью [этого](https://docs.microsoft.com/azure/load-balancer/load-balancer-standard-diagnostics#how-do-i-check-my-snat-port-usage-and-allocation) руководства.
...
#### <a name="assign-a-public-ip-to-each-vm"></a><a name="assignilpip"></a>Назначить общественный IP каждому VM
Назначение публичного IP-адреса изменяет ваш сценарий на [общедоступный IP-адрес на VM.](#ilpip) Все временные порты общедоступного IP-адреса, используемые для каждой виртуальной машины, доступны для каждой из них. (В отличие от сценариев, где эфемерные порты публичного Ip-адреса совместно используются всеми вс-мистами, связанными с соответствующим пулом бэкэнда.) Есть компромиссы, которые следует рассмотреть, такие как дополнительные расходы на публичные IP-адреса и потенциальное влияние белого списка большого числа отдельных IP-адресов.
...
#### <a name="scale-out"></a><a name="scaleout"></a>Масштабирование
...
Для исходящих подключений задано время ожидания простоя 4 минуты. Этот тайм-аут регулируется с [помощью исходящих правил.](../load-balancer/load-balancer-outbound-rules-overview.md#idletimeout) Вы также можете использовать транспорт (например, TCP keepalives) или сохранение уровня приложения, чтобы обновить простой поток и при необходимости сбросить этот тайм-аут.  
...
## <a name="discovering-the-public-ip-that-a-vm-uses"></a><a name="discoveroutbound"></a>Обнаружение общедоступного IP-адреса, используемого виртуальной машиной
...
- Подробнее о [группах сетевой безопасности.](../virtual-network/security-overview.md)
