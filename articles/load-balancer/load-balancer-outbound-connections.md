...
Термины, используемые в этих сценариях. Дополнительные сведения см. в разделе [терминология](#terms).

* [Преобразование адресов исходной сети (SNAT)](#snat)
* [Маскировка портов (PAT)](#pat)
* Протокол TCP
* Протокол UDP
* Преобразование сетевых адресов
* Протокол управляющих сообщений Интернета
* Инкапсуляция протокола безопасности

### <a name="scenarios"></a>Сценарии

* [Сценарий 1](#scenario1) . Виртуальная машина с общедоступным IP-адресом.
* [Сценарий 2](#scenario2) . Виртуальная машина без общедоступного IP-адреса.
* [Сценарий 3](#scenario3) . Виртуальная машина без общедоступного IP-адреса и без подсистемы балансировки нагрузки "Стандартный".
...
| ---------- | ------ | ------------ |
| Общедоступная подсистема балансировки нагрузки или изолированная | [SNAT.](#snat) </br> [Маскировка портов](#pat) не используется. | TCP </br> UDP </br> ICMP </br> ESP |
...
### <a name="scenario-2---virtual-machine-without-public-ip"></a><a name="scenario2"></a>Сценарий 2. Виртуальная машина без общедоступного IP-адреса

| Сопоставления | Метод | Протоколы IP |
| ------------ | ------ | ------------ |
| Общедоступная подсистема балансировки нагрузки | Использование внешнего интерфейса балансировщика нагрузки для [SNAT](#snat) с [маскировкой портов (PAT)](#pat).| TCP </br> UDP |
...
### <a name="scenario-3---virtual-machine-without-public-ip-and-without-standard-load-balancer"></a><a name="scenario3"></a> Сценарий 3. Виртуальная машина без общедоступного IP-адреса и без стандартной подсистемы балансировки нагрузки

| Сопоставления | Метод | Протоколы IP |
| ------------ | ------ | ------------ |
|None </br> Базовая подсистема балансировки нагрузки | [SNAT](#snat) с [маскировкой портов (PAT)](#pat)| TCP </br> UDP | 

#### <a name="description"></a>Описание

Когда виртуальная машина создает исходящий поток, Azure преобразует исходный IP-адрес исходящего потока в общедоступный исходный IP-адрес. Этот общедоступный IP-адрес **не настраивается** и не может быть зарезервирован. Этот адрес не учитывается в ограничении ресурсов общедоступного IP-адреса подписки. 
...
Вы имеете полное декларативное управление исходящими подключениями для масштабирования и настройки этой возможности в своих целях. Этот раздел расширяет сценарий 2, как описано выше.
...
* [Сценарий 1](#scenario1out) . Настройка исходящих подключений к определенному набору общедоступных IP-адресов или префиксов.
* [Сценарий 2](#scenario2out) . изменение распределения портов [SNAT](#snat) .
* [Сценарий 3](#scenario3out) . Включение только исходящего трафика.
* [Сценарий 4](#scenario4out) . исходящий NAT только для виртуальных машин (без входящего трафика).
* [Сценарий 5](#scenario5out) . исходящий трафик NAT для внутренней подсистемы балансировки нагрузки уровня "Стандартный".
* [Сценарий 6](#scenario6out) . Включите протоколы TCP & UDP для исходящего трафика NAT с помощью общедоступной подсистемы балансировки нагрузки уровня "Стандартный".

### <a name="scenario-1"></a><a name="scenario1out"></a>Сценарий 1

| Сценарий |
| -------- |
| Настройка исходящих подключений к определенному набору общедоступных IP-адресов или префикса|
...
### <a name="scenario-2"></a><a name="scenario2out"></a>Сценарий 2

| Сценарий |
| -------- |
| Изменение распределения портов [SNAT](#snat) |
...
Вернитесь к [выделению порта по умолчанию](load-balancer-outbound-connections.md#preallocatedports) , указав 0 в качестве числа портов. Первые 50 экземпляров виртуальных машин получают порты 1024, 51-100 512 экземпляров виртуальных машин — до максимума.  Дополнительные сведения о выделении портов SNAT по умолчанию см. [выше](#snatporttable).

### <a name="scenario-3"></a><a name="scenario3out"></a>Сценарий 3

| Сценарий |
| -------- |
|  Включение только NAT для исходящего трафика |

#### <a name="details"></a>Подробности

Вы можете использовать общедоступную подсистему балансировки нагрузки уровня "Стандартный", чтобы предоставить исходящий трафик NAT для группы виртуальных машин. В этом случае используйте правило исходящего трафика отдельно, не требуя дополнительных правил.
...
### <a name="scenario-4"></a><a name="scenario4out"></a>Сценарий 4
...
### <a name="scenario-5"></a><a name="scenario5out"></a>Сценарий 5

| Сценарий |
| -------- |
| Исходящий трафик NAT для внутренней подсистемы балансировки нагрузки уровня "Стандартный" |
...
### <a name="scenario-6"></a><a name="scenario6out"></a>Сценарий 6

| Сценарий |
| -------- |
| Включение протоколов TCP & UDP для исходящего NAT с помощью общедоступной подсистемы балансировки нагрузки уровня "Стандартный" |
...
В пространстве общедоступного IP-адреса пять кортежей приведенного ниже потока должны быть уникальными:
