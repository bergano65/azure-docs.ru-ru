...
| Термин | Применимые протоколы | Сведения|
| ---------|---------| -------|
...
| Поведение при выпуске порта SNAT | TCP | Если клиент или сервер отправляют флажки FIN и ACK, порт SNAT освобождается через 240 секунд. Если вы видите RST, порт SNAT будет выпущен через 15 секунд. При достижении времени ожидания простоя порт освобождается.|
...
## <a name="outbound-rules"></a><a name="outboundrules"></a>Правила для исходящего трафика
