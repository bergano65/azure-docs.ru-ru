...
Чтобы отслеживать работоспособность исходящих подключений с помощью основные подсистемы балансировки нагрузки, можно использовать [журналы Azure Monitor для подсистемы балансировки нагрузки](load-balancer-monitor-log.md) и [журналы событий оповещений](load-balancer-monitor-log.md#alert-event-log) для отслеживания сообщений о нехватке портов SNAT.
...
- Если сервер/клиент отправляет FINACK, порт SNAT освобождается через 240 секунд.
- Если получен сегмент с флагом RST, порт SNAT освобождается через 15 секунд.
- Если достигнуто время ожидания простоя, порт освобождается.
