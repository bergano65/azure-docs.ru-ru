...
### <a name="scenario-2-load-balanced-vm-without-a-public-ip-address"></a><a name="lb"></a>Сценарий 2. Виртуальная машина с балансировкой нагрузки без общедоступного IP-адреса
...
Порты SNAT предварительно выделяются, как описано в разделе [Общие сведения о SNAT и Pat](#snat) . Они представляют собой ограниченный ресурс, который можно исчерпать. Важно понимать, как они [используются](#pat). Чтобы понять, как спроектировать архитектуру и избежать этой проблемы, см. сведения в разделе [Управление нехваткой портов SNAT (PAT)](#snatexhaust).
...
>[!NOTE]
> При использовании Load Balancer (цен. категории "Стандартный") с [множеством интерфейсов](load-balancer-multivip-overview.md) каждый внешний IP-адрес увеличивает количество доступных портов SNAT в предыдущей таблице. Например, серверный пул виртуальных машин 50 с 2 правилами балансировки нагрузки, каждый с отдельным внешним IP-адресом, будет использовать порты SNAT 2048 (2x 1024) для каждого правила. См. подробности в разделе [Несколько интерфейсов для исходящих потоков](#multife).
...
#### <a name="manually-allocate-snat-ports-to-maximize-snat-ports-per-vm"></a><a name ="manualsnat"></a>Вручную Выделите порты SNAT, чтобы максимально увеличить число портов SNAT на одну виртуальную машину
Как определено в предварительно [выделенных портах](#preallocatedports), балансировщик нагрузки будет автоматически распределять порты на основе числа виртуальных машин в серверной части. По умолчанию это сделано консервативно, чтобы обеспечить масштабируемость. Если известно максимальное число виртуальных машин в серверной части, можно вручную выделить порты SNAT, настроив их в каждом исходящем правиле. Например, если известно, что вы получите не более 10 виртуальных машин, можно выделить 6 400 портов SNAT на каждую виртуальную машину, а не по умолчанию 1 024. 
...
- Дополнительные сведения о [группах безопасности сети](../virtual-network/security-overview.md).
