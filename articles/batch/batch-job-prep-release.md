---
title: Создание задач для подготовки & выполнения заданий на COMPUTE Nodes в пакетной службе Azure
...
Может возникнуть необходимость хранить копию файлов журнала, создаваемых задачами, или, например, файлы аварийных дампов, создаваемые приложениями, в которых возникает сбой. Используйте **задачу выпуска задания** в таких случаях, чтобы сжать и передать эти данные в учетную запись [хранения Azure][azure_storage] .
...
Чтобы просмотреть задачи подготовки и выпуска заданий в действии, ознакомьтесь с примером проекта [жобпрепрелеасе][job_prep_release_sample] на сайте GitHub. Это консольное приложение выполняет следующее.
...
При запуске примера приложения можно использовать [портал Azure][portal] для просмотра свойств задания и его задач, а также для загрузки общего текстового файла, который изменяется задачами задания.
...
[Установка приложений и промежуточных данных на пакетных тестовых узлах][forum_post]
