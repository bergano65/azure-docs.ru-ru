---
title: Используйте вычислительно-интенсивные VMs Azure с пакетом
description: Как воспользоваться преимуществами виртуальных размеров машин HPC и GPU в пулах Azure Batch. Узнайте о зависимостях ОС и ознакомьтесь с несколькими примерами сценариев.
documentationcenter: ''
author: LauraBrenner
manager: evansma
editor: ''
...
Возможности RDMA и GPU размеров для ресурсоемких вычислений в пакетной службе поддерживаются только в некоторых операционных системах. (Список поддерживаемых операционных систем представляет собой подмножество тех, которые поддерживаются для виртуальных машин, созданных в этих размерах.) В зависимости от того, как вы создаете пул пакетов, возможно, потребуется установить или настроить дополнительный драйвер или другое программное обеспечение на узлах. В указанной ниже таблице представлены сведения об этих зависимостях. Перейдите по ссылке, чтобы просмотреть дополнительные сведения. Варианты настройки пулов пакетной службы см. далее в этой статье.
...
<sup>*</sup>RDMA-способных N-серии размеры также включают NVIDIA Tesla GPUs
...
## <a name="example-nvidia-gpu-drivers-on-windows-nc-vm-pool"></a>Пример: Драйверы графического процессора NVIDIA в пуле Windows NC VM
...
3. Отправьте пакет в учетную запись пакетной службы. Пошаговые инструкции см. в руководстве для [пакетов приложений](batch-application-packages.md). Укажите идентификатор приложения, такой как *GPUDriver,* и версию, такую как *411.82.*
...
| **Предлагаем** | WindowsServer |
| **Sku** | 2016-Datacenter |
| **Размер узла** | NC6 уровня "Стандартный" |
| **Ссылки на пакет приложения** | GPUDriver, версия 411.82 |
| **Start task enabled** (Включена задача запуска) | True<br>**Командная линия** - `cmd /c "%AZ_BATCH_APP_PACKAGE_GPUDriver#411.82%\\GPUDriverSetup.exe /s"`<br/>**Удостоверение пользователя.** Автоматический пользователь пула, администратор<br/>**Ожидать успешное выполнение.** True

## <a name="example-nvidia-gpu-drivers-on-a-linux-nc-vm-pool"></a>Пример: Драйверы Графического процессора NVIDIA в пуле Linux NC VM
...
3. Выполните действия по созданию [изображения общей галереи изображений](batch-sig-images.md) для Batch.
4. Создайте учетную запись пакетной службы в регионе, поддерживающем виртуальные машины NC.
5. Используя aIS-пакеты пакетов или портал Azure, создайте пул [с использованием пользовательского изображения](batch-sig-images.md) и с желаемым количеством узлов и масштабирования. В следующей таблице приведены примеры параметров пула для образа:

| Параметр | Значение |
| ---- | ---- |
| **Тип образа** | Пользовательский образ |
| **Пользовательское изображение** | *Имя образа* |
| **Номер SKU агента узла** | batch.node.ubuntu 16.04 |
| **Размер узла** | NC6 уровня "Стандартный" |

## <a name="example-microsoft-mpi-on-a-windows-h16r-vm-pool"></a>Пример: Microsoft MPI в пуле ВМ Windows H16r
...
1. Выполните действия по созданию [изображения общей галереи изображений](batch-sig-images.md) для Batch.
1. Используя aIS-пакеты пакетов или портал Azure, создайте пул [с помощью общей галереи изображений](batch-sig-images.md) и с желаемым количеством узлов и масштабирования. В следующей таблице приведены примеры параметров пула для образа:
...
## <a name="example-intel-mpi-on-a-linux-h16r-vm-pool"></a>Пример: Intel MPI на Linux H16r VM бассейн
