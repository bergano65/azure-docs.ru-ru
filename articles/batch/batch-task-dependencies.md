...
В предыдущем фрагменте кода «batchClient» является экземпляром класса [batchClient][net_batchclient] .

## <a name="create-dependent-tasks"></a>Создание зависимости задач
Чтобы создать задачу, зависящую от выполнения одной или нескольких родительских задач, можно указать, что эта задача "зависит" от других задач. В пакетной среде .NET настройте [CloudTask][net_cloudtask]. Свойство [DependsOn][net_dependson] с экземпляром класса [TaskDependencies][net_taskdependencies] :
...
> [!NOTE]
> По умолчанию задача считается успешно выполненной, когда она находится в состоянии **Выполнено** и ее **код выхода** равен `0`. В пакетной среде .NET это означает [CloudTask][net_cloudtask]. Свойство [State][net_taskstate] со значением `Completed` и [TaskExecutionInformation][net_taskexecutioninformation] CloudTask. Значение свойства [ExitCode][net_exitcode] — `0`. Чтобы изменить эти значения, см. раздел [Действия зависимостей](#dependency-actions).
> 
> 
...
При взаимосвязи "один к одному" задача зависит от успешного выполнения одной родительской задачи. Чтобы создать зависимость, укажите для [TaskDependencies][net_taskdependencies]один идентификатор задачи. [OnId][net_onid] статический метод при заполнении свойства [DependsOn][net_dependson] объекта [CloudTask][net_cloudtask].
...
При взаимосвязи "один ко многим" задача зависит от успешного выполнения нескольких родительских задач. Чтобы создать зависимость, укажите коллекцию идентификаторов задач для [TaskDependencies][net_taskdependencies]. [OnIds][net_onids] статический метод при заполнении свойства [DependsOn][net_dependson] объекта [CloudTask][net_cloudtask].
...
Чтобы создать зависимость, укажите идентификаторы первой и последней задач в диапазоне для [TaskDependencies][net_taskdependencies]. [OnIdRange][net_onidrange] статический метод при заполнении свойства [DependsOn][net_dependson] объекта [CloudTask][net_cloudtask].
...
Действие зависимости основано на условии выхода родительской задачи. Можно указать действие зависимости для любого из следующих условий выхода. Дополнительные сведения о .NET см. в описании класса [екситкондитионс][net_exitconditions] .
...
Чтобы указать действие зависимости в .NET, задайте [екситоптионс][net_exitoptions]. Свойство [DependencyAction][net_dependencyaction] для условия выхода. Свойство **DependencyAction** принимает одно из двух значений:
...
Пример проекта [TaskDependencies][github_taskdependencies] является одним из [примеров кода пакетной службы Azure][github_samples] на сайте GitHub. Это решение Visual Studio демонстрирует следующее:
