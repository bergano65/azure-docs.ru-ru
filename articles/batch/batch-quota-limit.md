...
| **Вычисление узлов в [пуле, созданном с помощью ресурса управляемого образа](batch-custom-images.md)**<sup>1</sup> ||
...
Выполните следующие действия, чтобы запросить увеличение квоты для своей учетной записи или подписки на [портале Azure][portal]. Тип увеличения квоты зависит от режима выделения пула в вашей учетной записи пакетной службы. Чтобы запросить увеличение квоты, необходимо включить ряд виртуальных машин, для которых требуется увеличить квоту. Когда применяется увеличение квоты, оно применяется ко всем сериям виртуальных машин.
...
1. Выберите **новый запрос** > поддержки**основы**.
...
1. В разделе **Подробные сведения** содержится следующая информация:
    ...
    a. В поле **Укажите сведения**укажите расположение, тип квоты и учетную запись пакетной службы.
...
* [Создание учетной записи пакетной службы Azure с помощью портал Azure](batch-account-create-portal.md)
