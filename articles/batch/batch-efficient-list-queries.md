...
Из этой статьи вы узнаете, как повысить производительность приложения пакетной службы Azure, уменьшив объем данных, возвращаемых службой при запросе заданий, задач, вычислительных узлов и других ресурсов с помощью библиотеки [.NET пакетной службы][api_net].
...
> [!NOTE]
> Пакетная служба поддерживает API для распространенных сценариев подсчета задач в задании и вычислительных узлов в пуле пакетной службы. Вместо того чтобы использовать запрос на вывод списка, вы можете вызвать операции [Получить количество задач][rest_get_task_counts] и [Вывести количество узлов в пуле][rest_get_node_counts]. Эти операции эффективнее, чем запрос списка, но возвращают более ограниченные сведения. См. статью о [подсчете задач и вычислительных узлов с учетом состояния](batch-get-resource-counts.md). 
...
* **Методы списка .NET**: каждому из .NET-методов API в этом столбце объект [ODATADetailLevel][odata] передается в качестве параметра.
...
Как показано в сведениях о затраченном времени, путем ограничения свойств и количества возвращаемых элементов можно значительно уменьшить время отклика на запрос. Этот и другие примеры проектов находятся в репозитории [azure-batch-samples][github_samples] на сервисе GitHub.

### <a name="batchmetrics-library-and-code-sample"></a>Библиотека BatchMetrics и пример кода
Помимо примера кода EfficientListQueries, приведенного выше, можно использовать проект [BatchMetrics][batch_metrics], находящийся в репозитории [azure-batch-samples][github_samples] на GitHub. Пример проекта BatchMetrics демонстрирует, как эффективно отслеживать ход выполнения задания пакетной службы Azure с помощью API пакетной службы.

Пример [BatchMetrics][batch_metrics] включает в себя проект библиотеки классов .NET, который вы можете использовать в собственных проектах, а также простую программу командной строки для проверки и демонстрации использования этой библиотеки.
