---
title: Создание решений с помощью шаблонов Visual Studio — пакетная служба Azure | Документация Майкрософт
...
* Шаблоны пакетной службы, доступные из [коллекции Visual Studio][vs_gallery] в виде расширений Visual Studio. Получить шаблоны можно двумя способами:
  
  * Установка шаблонов с помощью диалогового окна " **расширения и обновления** " в Visual Studio (Дополнительные сведения см. в разделе [Поиск и использование расширений Visual Studio][vs_find_use_ext]). С помощью диалогового окна **Расширения и обновления** найдите и скачайте следующие два модуля:
    
    * диспетчер заданий пакетной службы Azure с разделителем заданий;
    * обработчик задач пакетной службы Azure.
  * Скачайте шаблоны из коллекции в Интернете для Visual Studio: [Пакетная служба Microsoft Azure шаблоны проектов][vs_gallery_templates]
...
* Каталог платформы. Он содержит файлы, отвечающие за стандартную работу, выполняемую программой диспетчера заданий — распаковка параметров, Добавление задач в пакетное задание и т. д. Обычно не нужно изменять эти файлы.
...
Пакетная служба имеет простой механизм передачи параметров среды в задачу диспетчера заданий с помощью свойства `EnvironmentSettings` в [Microsoft. Azure. Batch. JobManagerTask][net_jobmanagertask].
...
Еще одним полезным инструментом в разработке решения пакетной службы являются [соглашения о пакетных файлах Azure][nuget_package]. Эта библиотека классов .NET (сейчас доступна предварительная версия) позволяет приложениям .NET пакетной службы легко сохранять выходные данные задач в службе хранилища Azure и извлекать их из нее. [Сохранение выходных данных заданий и задач пакетной службы Azure](batch-task-output.md) содержится полное описание библиотеки и сведения о ее использовании.
