...
1. Открыть Visual Studio и выбрать **файл** > **новый** > **проект**.
...
2. В Solution Explorer, правой кнопкой мыши решение, нажмите **Добавить** > **новый проект**.
...
* Каталог платформы. Это содержит файлы, ответственные за работу 'boilerplate', проделанную программой менеджера по работе - распаковка параметров, добавление задач к заданию пакета и т.д. Обычно не нужно изменять эти файлы.
...
По умолчанию при сбое не будет выполняться повторный запуск диспетчера заданий. В зависимости от логики менеджера по работе клиент может захотеть включить повторы с помощью *ограничений*/*maxTaskRetryCount.*
