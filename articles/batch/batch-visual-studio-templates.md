---
title: Создание решений с помощью шаблонов Visual Studio
...
* Шаблоны пакетной службы, доступные в [коллекции Visual Studio][vs_gallery] в качестве расширений Visual Studio. Получить шаблоны можно двумя способами:
  ...
  * Загрузите шаблоны из онлайн-коллекции для Visual Studio: [Шаблоны проектов пакетной службы Microsoft Azure][vs_gallery_templates]
...
> [!NOTE]
> Дополнительные сведения о задачах диспетчера заданий см. в разделе [Задания и задачи](jobs-and-tasks.md#job-manager-task).
> 
> 
...
Все сведения, возвращаемые исключением, записываются в файлы stdout.txt и stderr.txt. Дополнительные сведения см. в разделе [Обработка ошибок](error-handling.md).
...
Еще одно полезное средство в разработке решений пакетной службы — [соглашения об именовании пакетных файлов Azure][nuget_package]. Эта библиотека классов .NET (сейчас доступна предварительная версия) позволяет приложениям .NET пакетной службы легко сохранять выходные данные задач в службе хранилища Azure и извлекать их из нее. [Сохранение выходных данных заданий и задач пакетной службы Azure](batch-task-output.md) содержится полное описание библиотеки и сведения о ее использовании.
