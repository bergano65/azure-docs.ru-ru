...
4. Оставьте значения по умолчанию для остальных параметров и выберите **Создать**, чтобы создать учетную запись.
...
Через несколько минут значение состояния пула изменится на **Устойчивое** и узлы запустятся. Выберите раздел **Узлы**, чтобы проверить состояние узлов. Если состояние узла — **Idle** (Неактивен), он готов к выполнению задач. 
...
Выполнение предыдущих примеров задач завершается через несколько минут. Чтобы просмотреть выходные данные завершенной задачи, выберите **Файлы на узле**, а затем выберите файл `stdout.txt`. В этом файле показаны стандартные выходные данные задачи. Его содержимое должно выглядеть следующим образом:
