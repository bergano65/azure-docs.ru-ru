...
- **Выполняется** — задача назначена вычислительному узлу, но еще не завершена. Задача `running` считается, если ее состояние `preparing` — или `running`, как указано в операции [получения сведений о задаче][rest_get_task] .
...
- **Успешно выполненная** — задача, результатом выполнения которой является состояние `success`. Пакетная службы определяет, завершилась ли задача успешно или нет `TaskExecutionResult` , проверив свойство свойства [executionInfo][rest_get_exec_info] .
...
- **Старттаскфаилед** — узел, на котором произошел сбой [задачи запуска][rest_start_task] и исчерпаны все повторные попытки и для которых `waitForSuccess` задана задача запуска. Этот узел невозможно использовать для выполнения задач.
