...
* Операция [Получить количество задач][rest_get_task_counts] позволяет статистически подсчитать число активных, выполняющихся и завершенных задач в задании, а также успешно и неудачно выполненных задач. 
...
* Операция [Перечислить число узлов пула][rest_get_node_counts] возвращает число выделенных и низкоприоритетных вычислительных узлов в каждом пуле, находящихся в различных состояниях: создание, простой, вне сети, замещение, перезагрузка, повторное создание образа, запуск и другие. 
...
- **Выполняется** — задача назначена вычислительному узлу, но еще не завершена. Задача считается `running`, если она находится в состоянии `preparing` или `running`, как указывает операция [Получить информацию о задаче][rest_get_task].
