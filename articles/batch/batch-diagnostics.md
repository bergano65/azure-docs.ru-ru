...
3. Выберите одну или несколько метрик. Чтобы выбрать метрики для дополнительных ресурсов, используйте раскрывающиеся списки **Подписки**, **Группы ресурсов**, **Тип ресурсов** и **Ресурсы**.
    * Для метрик на основе счетчиков (например, "Количество выделенных ядер" или "Количество узлов с низким приоритетом") используйте агрегат "Среднее". Для метрик на основе событий (например, "События изменения размера пула") используйте агрегат "Количество".

> [!WARNING]
> Не используйте агрегат "Сумма", который суммирует значения всех точек данных, полученных в течение периода диаграммы.
> 
> 
...
Также с помощью [API-интерфейса REST](https://docs.microsoft.com/rest/api/monitor/) вы можете настроить оповещения практически в реальном времени. Дополнительные сведения см. в статье [Обзор оповещений в Microsoft Azure](../azure-monitor/platform/alerts-overview.md). Чтобы добавить в оповещение сведения о задании, задаче или пуле, изучите информацию о поисковых запросах из статьи [Реагирование на события с помощью оповещений Azure Monitor](../azure-monitor/learn/tutorial-response.md).
...
3. В разделе **Параметры диагностики** введите имя параметра и выберите расположение для журналов (существующую учетную запись хранения, концентратор событий или журналы Azure Monitor). Выберите **ServiceLog**, **AllMetrics** или оба параметра сразу.
