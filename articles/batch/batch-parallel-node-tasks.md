---
title: Выполнение задач параллельно для оптимизации вычислительных ресурсов - Azure Batch
...
Azure Batch позволяет устанавливать задачи на узел до (4x) числа основных узлов. Например, если для пула настроены узлы размера "Большой" (четыре ядра), для параметра `maxTasksPerNode` можно задать значение 16. Однако, независимо от количества ядер, которые есть узла, на узла не может быть более 256 задач. Сведения о количестве ядер для каждого узла см. в статье [Размеры для облачных служб](../cloud-services/cloud-services-sizes-specs.md). Дополнительные сведения об ограничениях службы см. в статье [Квоты и ограничения пакетной службы Azure](batch-quota-limit.md).
