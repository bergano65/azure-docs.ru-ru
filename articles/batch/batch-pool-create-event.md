...
|`imageType`|Строка|Метод развертывания для образа. Поддерживаемые значения — `virtualMachineConfiguration` или.`cloudServiceConfiguration`|
...
|`targetOSVersion`|Строка|Версия гостевой ОС Azure для установки на виртуальных машинах в пуле.<br /><br /> Значение **\*** по умолчанию указывает последнюю версию операционной системы для указанного семейства.<br /><br /> Другие допустимые значения см. в статье [Выпуски гостевой ОС Azure](https://azure.microsoft.com/documentation/articles/cloud-services-guestos-update-matrix/#releases).|
...
###  <a name="windowsconfiguration"></a><a name="bk_winconf"></a>виндовсконфигуратион

|Имя элемента|Тип|Примечания|
|------------------|----------|-----------|
|`enableAutomaticUpdates`|Логическое значение|Указывает, включено ли для виртуальной машины автоматическое обновление. Если это свойство не задано, используется значение по умолчанию true.|
