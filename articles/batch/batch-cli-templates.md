---
title: Выполнение заданий в конце с помощью шаблонов
...
Шаблоны пакетной службы создают существующую пакетную поддержку в [Azure CLI](batch-cli-get-started.md#json-files-for-resource-creation) для JSON-файлов, чтобы указать значения свойств при создании пулов, заданий, задач и других элементов. Шаблоны пакетной службы добавляют следующие возможности.
...
Например, [ffmpeg](https://ffmpeg.org/) — это популярное приложение, которое обрабатывает аудио- и видеофайлы. Ниже приведены инструкции по вызову приложение ffmpeg с помощью интерфейс командной строки для пакетной службы Azure, которое перекодирует исходные видеофайлы в файлы с другим разрешением.
...
-   **Параметры**
    ...
    -   Позволяют указывать значения свойств в теле шаблона. При использовании шаблона необходимо предоставить только значения параметров. Например, полное определение пула может быть помещено в тело и только один параметр для `poolId`; Поэтому для создания пула необходимо указать только строку идентификатора пула.
