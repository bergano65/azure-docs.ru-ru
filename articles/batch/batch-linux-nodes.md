...
ms.locfileid: "77917504"
---
# <a name="provision-linux-compute-nodes-in-batch-pools"></a>Подготовка вычислительных узлов Linux в пулах пакетной службы

Пакетная служба Azure позволяет выполнять параллельные вычислительные рабочие нагрузки на виртуальных машинах Linux и Windows. В этой статье подробно описано, как создавать пулы для вычислений с помощью клиентских библиотек [.NET][api_net] [для пакетной][py_batch_package] службы в пакетной службе.
...
Этот фрагмент кода явно создает [ImageReference][py_imagereference] и задает каждое из его свойств (издатель, предложение, SKU, версия). Однако в рабочем коде рекомендуется использовать метод [list_supported_images][py_list_supported_images] для определения и выбора из доступных комбинаций образа и номера SKU агента узла во время выполнения.
