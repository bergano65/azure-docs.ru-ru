...
Для предоставления вычислительных узлов в конфигурации виртуальной машины пакетная служба использует [масштабируемые наборы виртуальных машин](../virtual-machine-scale-sets/virtual-machine-scale-sets-overview.md). Вы можете указать образ из [Azure Marketplace][vm_marketplace] или подготовленный пользовательский образ. Для получения более подробной информации об пользовательских изображениях [см.](batch-sig-images.md)
...
В следующем фрагменте кода показан пример создания пула вычислительных узлов Ubuntu Server с помощью [клиентской библиотеки пакетной службы Microsoft Azure для Python][py_batch_package]. Справочную документацию для модуля Batch Python можно найти в [пакете azure.batch][py_batch_docs] на Read the Docs.

В этом фрагменте кода явно создается объект [ImageReference][py_imagereference] и указывается каждое из его свойств (publisher, offer, SKU, version). Однако в производственном коде мы рекомендуем использовать метод [list_supported_images][py_list_supported_images] для определения и выбора из доступных комбинаций SKU изображения и узла sKU во время выполнения.
...
Как упоминалось ранее, мы рекомендуем вместо явного создания [ImageReference][py_imagereference] использовать метод [list_supported_images][py_list_supported_images] динамически выбрать из поддерживаемых в настоящее время комбинаций изображений узла/Marketplace. Следующий фрагмент кода Python демонстрирует, как использовать этот метод.
...
Следующий фрагмент кода использует [PoolOperations.][net_pool_ops] [Метод ListSupportedImages][net_list_supported_images] для выбора из списка поддерживаемых в настоящее время комбинаций изображения и узла SKU. Этот способ является предпочтительным, так как список поддерживаемых сочетаний может меняться время от времени. Чаще всего добавляются поддерживаемые сочетания.
...
Хотя предыдущий фрагмент использует [PoolOperations][net_pool_ops]. [Метод ListSupportedImages][net_list_supported_images] динамически перечислить и выбрать из поддерживаемых комбинаций sKU изображения и узла (рекомендуется), вы также можете настроить [ImageReference][net_imagereference] явно:
...
Чтобы получить список всех поддерживаемых изображений виртуальных машин Marketplace для службы Batch и соответствующих агентов узлов, пожалуйста, используйте [list_supported_images][py_list_supported_images] (Python), [ListSupportedImages][net_list_supported_images] (Batch .NET) или соответствующий API на соответствующем языке SDK по вашему выбору.
...
Пакетная служба Azure основана на технологии виртуальных машин Azure и облачных служб Azure. Сама услуга Batch предлагается бесплатно, что означает, что с вас взимается плата только за вычислительные ресурсы (и связанные с ними затраты), которые потребляют решения Пакета. При выборе режима **Cloud Services Configuration** (Конфигурация облачных служб) плата взимается в зависимости от структуры [цен на облачные службы][cloud_services_pricing]. При выборе режима **Конфигурация виртуальной машины** плата взимается в зависимости от структуры [цен на виртуальные машины][vm_pricing].
