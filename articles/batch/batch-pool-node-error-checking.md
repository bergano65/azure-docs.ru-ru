...
Если на временном диске не хватает места (или почти заканчивается место на диске), узел перейдет в [неработоспособное](https://docs.microsoft.com/rest/api/batchservice/computenode/get#computenodestate) состояние и появится сообщение об ошибке узла (используйте уже имеющуюся ссылку) о том, что диск заполнен.
