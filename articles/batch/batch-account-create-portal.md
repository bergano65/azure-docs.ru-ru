...
1. Введите параметры **новой учетной записи пакетной службы**. Укажите следующие сведения:
    ...
    4.3. **Расположение.** Регион Azure, в котором будет создана учетная запись пакетной службы. Будут отображаться только те регионы, которые поддерживаются для вашей подписки и группы ресурсов.
...
В учетных записях пакетной службы в режиме пользовательской подписки квоты ядер не задаются по умолчанию. Квоты ядер необходимо установить вручную, так как обычные квоты ядер пакетной службы не применяются к учетным записям в режиме пользовательской подписки.
