...
1. Выберите **Создать службу** > **вычислительной** > **связи.**
...
1. Введите параметры **новой учетной записи пакетной службы**. Укажите следующие сведения:
    ...
    д) **Учетная запись хранения:** Дополнительная учетная запись хранения Azure, которую вы связываете с учетной записью Пакета. Для наилучшей производительности рекомендуется использовать учетную запись хранения общего назначения версии 2. Все параметры учетной записи хранения в пакетной службе см. в [обзоре функций пакетной службы](batch-api-basics.md#azure-storage-account). На портале выберите существующую учетную запись хранения или создайте новую.

      ![Создание учетной записи хранения][storage_account]

    е) **Режим распределения пула:** Во вкладке **Расширенные** настройки можно указать режим распределения пула как **службу пакетов** или **подписку пользователя.** В большинстве случаев можно использовать значение по умолчанию — **Пакетная служба**.
...
1. Выберите **все подписки** > **Subscriptions**служб и выберите подписку, которая вы хотите использовать для учетной записи Batch.
...
При создании учетной записи Batch в режиме подписки пользователя используйте группу ресурсов для хранилища ключей. Укажите **подписку пользователя** в режиме распределения пула, выберите хранилище ключей и проверьте поле, чтобы предоставить Azure Batch доступ к хранилищу ключей. 

Если вы предпочитаете предоставлять доступ к хранилищу ключей вручную, перейдите в раздел **политик access** of the key vault и выберите **политику добавления доступа** и поиск для Microsoft **Azure Batch.** После выбора необходимо настроить **разрешения Secret** с помощью меню drop down. Пакет Azure должен быть предоставлен как минимум **разрешений Get,** **List,** **Set**и **Delete.**

![Секретные разрешения для Azure Batch](./media/batch-account-create-portal/secret-permissions.png)


> [!NOTE]
> Убедитесь, что в **рамках политик access** для связанного ресурса Key **Vault** будут выбраны **виртуальные машины Azure для развертывания** и менеджер ресурсов Azure для флажков **шаблонов.**
> 
> ![Обязательная политика](./media/batch-account-create-portal/key-vault-access-policy.png) доступа к ключу Vault Это не является обязательным при создании учетной записи пакета на портале Azure. Опция выбрана по умолчанию.
...
* [Пакет PowerShell cmdlets](batch-powershell-cmdlets-get-started.md)
* [Лазурный CLI](batch-cli-get-started.md)
