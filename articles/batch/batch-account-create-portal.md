...
1. Введите параметры **новой учетной записи пакетной службы**. Укажите следующие сведения:
    ...
    б. **Группа ресурсов** — существующая группа ресурсов для новой учетной записи пакетной службы (при необходимости можно создать другую группу).
    ...
    . **Расположение** — регион Azure, в котором будет создана учетная запись пакетной службы. Будут отображаться только те регионы, которые поддерживаются для вашей подписки и группы ресурсов.
...
1. В [портал Azure][azure_portal]выберите **создать** > **Безопасность** > **Key Vault**.
...
> 
> ![обязательная политика доступа Key Vault](./media/batch-account-create-portal/key-vault-access-policy.png) это необязательно при создании учетной записи пакетной службы в портал Azure. Этот параметр выбран по умолчанию.
