...
Задание указывает [пул](nodes-and-pools.md#pools), в котором будет выполняться работа. Вы можете создавать отдельный пул для каждого задания или использовать один пул для множества заданий. Можно создать пул для каждого задания, связанного с [расписанием заданий](#scheduled-jobs), или один пул для всех заданий, связанных с расписанием заданий.

### <a name="job-priority"></a>приоритет задания

При создании задания ему при необходимости можно назначить приоритет. Пакетная служба использует значение приоритета задания, чтобы определить порядок планирования (для всех задач в задании), втихин каждый пул.

Чтобы обновить приоритет задания, вызовите [Update свойства операции задания](/rest/api/batchservice/job/update) (остальные пакеты) или измените [CloudJob. Priority](/dotnet/api/microsoft.azure.batch.cloudjob) (пакетная платформа .NET). Значения приоритета находятся в диапазоне от-1000 (низший приоритет) до 1000 (высший приоритет).

В пределах одного пула задания с более высоким приоритетом имеют приоритет планирования над заданиями с более низким приоритетом. Задачи в заданиях с более низким приоритетом, которые уже выполняются, не будут заменяться задачами с более высоким приоритетом. Задания с одинаковым уровнем приоритета имеют одинаковую вероятность планирования, а порядок выполнения задач не определен.

Задание с высоким приоритетом, которое выполняется в одном пуле, не влияет на планирование заданий, выполняемых в отдельном пуле или в другой учетной записи пакетной службы. Приоритет задания не применяется к [автопулам](nodes-and-pools.md#autopools), которые создаются при отправке задания.
...
По умолчанию задания остаются в активном состоянии после выполнения всех задач в задании. Это поведение можно изменить, чтобы задание автоматически завершалось после выполнения всех входящих в него задач. Задайте для свойства **onAllTasksComplete** задания ([OnAllTasksComplete](/dotnet/api/microsoft.azure.batch.cloudjob) в .NET пакетной службы) значение `terminatejob` * ', чтобы автоматически завершить задание, когда все его задачи находятся в состоянии завершено.

Пакетная служба считает задание *без* задач заданием, все задачи которого выполнены. Поэтому этот параметр чаще всего используется с [задачами диспетчера заданий](#job-manager-task). Если вы хотите использовать автоматическое завершение заданий без диспетчера заданий, сначала следует задать для свойства **onAllTasksComplete** нового задания значение `noaction` , а затем задать для него значение `terminatejob` * ' только после завершения добавления задач к заданию.
