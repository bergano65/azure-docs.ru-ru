...
> [!NOTE]
> Управляемое системой удостоверение, созданное в учетной записи пакетной службы, используется только для получения ключей, управляемых клиентом, из Key Vault. Это удостоверение недоступно для пулов пакетной службы. Сведения об использовании назначенного пользователем управляемого удостоверения в пуле см. в статье [Настройка управляемых удостоверений в пулах пакетной](managed-identity-pools.md)службы.
...
![Снимок экрана, показывающий экран "Добавление политики доступа".](./media/batch-customer-managed-key/key-permissions.png)
...
- **Назначено ли системой управляемое удостоверение учетной записи пакетной службы, доступной на этих узлах?** Нет. Управляемое системой удостоверение в настоящее время используется только для доступа к Azure Key Vault для ключа, управляемого клиентом. Сведения об использовании назначаемого пользователем управляемого удостоверения на кластерных узлах см. [в статье Настройка управляемых удостоверений в пулах пакетной](managed-identity-pools.md)службы.
