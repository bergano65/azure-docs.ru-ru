...
4. Потом они выполняют **команду координации** , указанную в параметрах для множественных экземпляров. Команда координации обычно используется для подготовки узлов к выполнению задачи. Это может быть запуск фоновых служб (например, [Microsoft MPI][msmpi_msdn] `smpd.exe`) и проверка готовности узлов к обработке сообщений между узлами.
5. *После* того, как основная задача и все подзадачи успешно выполнят команду координации, основная задача выполнит **команду приложения** на главном узле. Команда приложения — это командная строка, указанная для многоэкземплярной задачи. Ее выполняет только основная задача. В решении на основе [MS-MPI][msmpi_msdn]это место, где выполняется приложение с поддержкой MPI с помощью `mpiexec.exe`.

> [!NOTE]
> Хотя она функционально уникальна, "многоэкземплярная задача" не является уникальным типом задачи, например [StartTask][net_starttask] или [JobPreparationTask][net_jobprep]. Задача с несколькими экземплярами — это просто стандартная Пакетная задача ([CloudTask][net_task] в .NET Batch), для которой настроены параметры с несколькими экземплярами. В этой статье задача такого типа называется **задачей с несколькими экземплярами**.
>
>
...
Для выполнения приложений MPI с помощью задачи с несколькими экземплярами на вычислительных узлах в пуле сначала необходимо установить реализацию MPI (например, MS-MPI или Intel MPI). Это хороший момент использования [StartTask][net_starttask], который выполняется каждый раз, когда узел присоединяется к пулу, или перезапускается. Этот фрагмент кода создает StartTask, который указывает пакет установки MS-MPI в качестве [файла ресурсов][net_resourcefile]. Командная строка задачи запуска выполняется после скачивания файла ресурсов на узел. В этом случае командная строка выполняет автоматическую установку MS-MPI.
...
Обратите внимание, что в этой команде координации используется `start` . Это необходимо, так как приложение `smpd.exe` не возвращается сразу после выполнения. Без использования команды [Start][cmd_start] эта команда координации не будет возвращать значение и, следовательно, заблокирует выполнение команды приложения.
...
Пакетная среда создает несколько [переменных среды][msdn_env_var] , относящихся к задачам с несколькими экземплярами на вычислительных узлах, выделенных для задачи с несколькими экземплярами. Командные строки координации и приложения могут ссылаться на эти переменные среды, как и сценарии и программы, выполняемые с их помощью.
...
Полные сведения об этих и других переменных среды для пакетного узла для вычислений, включая их содержимое и видимость, см. в разделе [переменные среды для вычислений с узлами][msdn_env_var].

> [!TIP]
> Фрагмент кода Linux MPI для пакетной службы содержит пример использования некоторых из этих переменных среды. Скрипт Bash [-cmd][coord_cmd_example] загружает общие приложения и входные файлы из службы хранилища Azure, включает общую сетевую файловую систему (NFS) на главном узле и настраивает другие узлы, выделенные для многоэкземплярной задачи, как NFS-клиенты.
>
>
...
[Таскконстраинтс][net_taskconstraints] для задач с несколькими экземплярами, таких как свойства [MaxTaskRetryCount][net_taskconstraint_maxretry], [MaxWallClockTime][net_taskconstraint_maxwallclock]и [RetentionTime][net_taskconstraint_retention] , учитываются так же, как для стандартной задачи, и применяются к основной и ко всем подзадачам. Однако если изменить свойство [RetentionTime][net_taskconstraint_retention] после добавления задачи с несколькими экземплярами в задание, это изменение будет применено только к основной задаче. Все подзадачи продолжают использовать исходный [RetentionTime][net_taskconstraint_retention].
...
Пример кода [MultiInstanceTasks][github_mpi] в GitHub демонстрирует использование задачи с несколькими экземплярами для запуска приложения [MS-MPI][msmpi_msdn] на вычислительных узлах пакетной службы. Чтобы запустить пример, следуйте указаниям, приведенным в разделах [Подготовка](#preparation) и [Выполнение](#execution).

### <a name="preparation"></a>Подготовка
1. Выполните первые два шага, описанные в разделе [как скомпилировать и запустить простую программу MS-MPI][msmpi_howto]. Это поможет выполнить предварительные требования к следующему шагу.
2. Создайте *окончательную* версию примера программы MPI [MPIHelloWorld][helloworld_proj] . Это программа, которую будет выполнять на вычислительных узлах задача с несколькими экземплярами.
...
4. Используйте [портал Azure][portal] , чтобы создать [приложение](batch-application-packages.md) пакетной службы с именем "MPIHelloWorld", и укажите ZIP-файл, созданный на предыдущем шаге, как версия "1,0" пакета приложения. Дополнительные сведения см. в разделе [Передача приложений и управление ими](batch-application-packages.md#upload-and-manage-applications).
...
1. Скачайте [Azure-Batch-Samples][github_samples_zip] из GitHub.
...
5. *Необязательно*: Перед удалением ресурсов используйте [портал Azure][portal] или [Batch Explorer][batch_labs] , чтобы просмотреть пример пула, задания и задачи ("MultiInstanceSamplePool", "MultiInstanceSampleJob", "MultiInstanceSampleTask").
...
* В блоге о [поддержке MPI для Linux в пакетной службе Azure для][blog_mpi_linux]Microsoft HPC & содержатся сведения об использовании [OpenFOAM][openfoam] с пакетной службой. Примеры кода Python для примера OpenFOAM можно найти [на сайте GitHub][github_mpi].
