...
Если для образа создается новая виртуальная машина, для управляемого образа в качестве базового можно использовать (с последующей настройкой) образ из Azure Marketplace, поддерживаемый пакетной службой.  Чтобы вывести список ссылок на образы из Azure Marketplace, поддерживаемые пакетной службой Azure, см. команду из раздела о [получении списка номеров SKU агента узла](/rest/api/batchservice/account/listnodeagentskus). Сторонний образ нельзя использовать в качестве базового образа.
...
Чтобы создать управляемый образ на основе моментального снимка, используйте средства командной строки Azure, такие как команда [az image create](/cli/azure/image#az_image_create). Можно создать образ, указав моментальный снимок диска ОС и при необходимости один или несколько моментальных снимков дисков данных.
