---
title: Безопасный доступ к Key Vault с помощью пакетной службы Azure
...
Если у вас еще нет сертификата, самый простой способ получить его — создать самозаверяющий сертификат с помощью средства командной строки `makecert`.

Обычно `makecert` можно найти по этому пути: `C:\Program Files (x86)\Windows Kits\10\bin\<arch>`. Откройте командную строку от имени администратора и перейдите к `makecert`, используя следующий пример.
...
Далее необходимо назначить сертификат учетной записи пакетной службы. Назначение сертификата учетной записи позволяет нам назначить его пулам, а затем узлам. Проще всего это сделать, перейдя к учетной записи пакетной службы на портале, перейдя к разделу **Сертификаты**и выбрав **Добавить**. Отправьте файл `.pfx`, созданный в ходе [получения сертификата](#obtain-a-certificate) , и укажите пароль. По завершении сертификат добавляется в список, и вы можете проверить отпечаток.
