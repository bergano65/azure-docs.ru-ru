...
Независимо от того, разрабатывается ли распределенное вычислительное приложение или служба, которая выдает прямые вызовы [REST API][batch_rest_api] или вы используете один из [пакетов SDK для пакетной](batch-apis-tools.md#azure-accounts-for-batch-development)службы, вы будете использовать многие ресурсы и функции, описанные в этой статье.
...
1. Отправьте **файлы данных** , которые требуется обработать в учетной записи [хранения Azure][azure_storage] . В пакетную службу встроена поддержка доступа к хранилищу BLOB-объектов Azure, и эти файлы могут быть скачаны на [вычислительные узлы](#compute-node) в ходе выполнения задач.
...
Дополнительные сведения об учетных записях хранения см. в статье [Общие сведения об учетной записи хранения Azure](../storage/common/storage-account-overview.md).
...
На узлах может выполняться любой исполняемый файл или скрипт, поддерживаемый его операционной системой: Исполняемые файлы или скрипты включают \*. exe, \*. cmd, \*. bat и сценарии PowerShell для Windows, а также сценарии для двоичных файлов, оболочки и Python для Linux.
...
Ознакомьтесь с [зависимостями задач в пакетной службе Azure](batch-task-dependencies.md) и образце кода [TaskDependencies][github_sample_taskdeps] в репозитории [Azure-Batch-Samples][github_samples] , чтобы получить более подробные сведения об этой функции.
...
Пользовательские переменные среды можно задать на уровне задачи или задания, задав свойство *параметров среды* для этих объектов. Например, см. статью [Добавление задачи в задание][rest_add_task] (пакетная REST API) или свойства [CloudTask. EnvironmentSettings][net_cloudtask_env] и [CloudJob. CommonEnvironmentSettings][net_job_env] в пакетной службе .NET.

Клиентское приложение или служба могут получить переменные среды задачи, определяемые службой и пользовательские, с помощью операции [получения сведений о задаче][rest_get_task_info] (служебная служба) или доступа к свойству [CloudTask. EnvironmentSettings][net_cloudtask_env] (пакетной службы .NET). Процессы, которые выполняются на вычислительном узле, могут обращаться к этим и другим переменным среды узла, например с помощью привычного синтаксиса `%VARIABLE_NAME%` в Windows или `$VARIABLE_NAME` в Linux.
...
* **Задачи**. в каталоге **WORKITEMS** создается каталог для каждой задачи, выполняемой на узле. Доступ к нему осуществляется с помощью ссылки на переменную среды `AZ_BATCH_TASK_DIR`.
...
* Пересоздание **образа узла** ([остальное][rest_reimage] | [.NET][net_reimage])
