...
В следующих разделах обсуждаются ресурсы пакета, которые позволяют распределить вычислительный сценарий.
...
На узлах может выполняться любой исполняемый файл или скрипт, поддерживаемый его операционной системой: Исполнители или скрипты включают \*в себя \*скрипты \*.exe, .cmd, .bat и PowerShell для Windows - и бинарные файлы, оболочки и скрипты Python для Linux.
...
Оно называется *целевым*, так как в некоторых случаях пул не может достигнуть требуемого числа узлов. Например, это может произойти из-за достижения [квоты на ядра](batch-quota-limit.md) для учетной записи пакетной службы Или пул может не достичь цели, если вы применили формулу автоматического масштабирования к пулу, которая ограничивает максимальное количество узлов.
...
Для динамических рабочих нагрузок можно написать и применить [формулу автоматического масштабирования](#scaling-compute-resources) к пулу. По этой формуле пакетная служба периодически вычисляет и изменяет количество узлов в пуле в зависимости от выбранных вами параметров пула, заданий и задач.
...
Можно также указать *тип заполнения,* который определяет, равномерно ли пакет распределяет задачи по всем узлам в пуле, или упаковывает каждый узл с максимальным количеством задач перед назначением задач другому узлам.
...
* Вы можете задать некоторые **ограничения** для заданий.
    ...
    Пакетная служба может обнаруживать и повторно выполнять незавершенные задачи. В качестве ограничения можно указать **максимальное число повторных попыток задачи**, а также следует ли пытаться повторно выполнить задачу (*всегда* или *никогда*). Повторная попытка означает, что задача перерастает для выполнения снова.
...
* **приложения**: Содержит информацию о деталях пакетов приложений, установленных на вычислительном узлах. Задачи могут получить доступ к этому каталогу с помощью ссылки на переменную среды `AZ_BATCH_APP_PACKAGE` .

* **fsmounts**: Каталог содержит все файловые системы, которые установлены на вычислительном узлах. Задачи могут получить доступ к этому каталогу с помощью ссылки на переменную среды `AZ_BATCH_NODE_MOUNTS_DIR` . Для получения дополнительной [Mount a virtual file system on a Batch pool](virtual-file-mount.md)информации см.
...
* **летучих**: Этот каталог предназначен для внутренних целей. Нет никакой гарантии, что какие-либо файлы в этом каталоге или что сам каталог будет существовать в будущем.

* **workitems**: Этот каталог содержит каталоги для заданий и их задач на вычислительном узлах.

* **Задачи**: В каталоге **рабочих элементов** создается каталог для каждой задачи, которая выполняется на уде. К нему можно получить доступ, `AZ_BATCH_TASK_DIR` ссылаясь на переменную среды.
...
Кроме того, прерывистая проблема может привести к тому, что задача перестанет отвечать или слишком долго выполняется. Для задачи можно установить максимальный интервал выполнения. В случае его превышения пакетная служба прерывает выполнение приложения задачи.
...
* **Перезапустить узел** ([REST][rest_reboot] | [.NET][net_reboot])
    ...
    В некоторых случаях перезапуск узла может устранить некоторые скрытые проблемы, например задержки и сбои при выполнении процессов. Если пул использует задачу запуска или в задании используется задача подготовки к заданиям, они выполняются при перезагрузке узла.
