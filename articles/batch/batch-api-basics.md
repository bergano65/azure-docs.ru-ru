...
## <a name="azure-storage-account"></a>Учетная запись службы хранилища Azure
...
На узлах может выполняться любой исполняемый файл или скрипт, поддерживаемый его операционной системой: Исполняемые файлы или скрипты включают @no__t -0. exe, @no__t -1. cmd, \*. bat и сценарии PowerShell для Windows, а также сценарии для двоичных файлов, оболочки и Python для Linux.
...
* **фсмаунтс**: Каталог содержит все файловые системы, подключенные к кластерному узлу. Задачи могут получить доступ к этому каталогу с помощью ссылки на переменную среды `AZ_BATCH_NODE_MOUNTS_DIR` . Дополнительные сведения см. [в статье подключение виртуальной файловой системы к пулу пакетной](virtual-file-mount.md)службы.
...
* **рабочие**элементы: Этот каталог содержит каталоги для заданий и их задач на кластерном узле.
...
* **Перезагрузка узла** ([оставшийся][rest_reboot] | [.NET][net_reboot])
...
* Пересоздание **образа узла** ([оставшийся][rest_reimage] | [.NET][net_reimage])
