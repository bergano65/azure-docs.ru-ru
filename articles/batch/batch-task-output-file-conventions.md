...
Параметр `kind` метода [TaskOutputStorage](/dotnet/api/microsoft.azure.batch.conventions.files.taskoutputstorage).[ SaveAsync](/dotnet/api/microsoft.azure.batch.conventions.files.taskoutputstorage.saveasync#overloads) классифицирует сохраненные файлы. Существует четыре стандартных типа [типа taskoutputkind][net_taskoutputkind] : `TaskOutput`, `TaskPreview`, `TaskLog`, а `TaskIntermediate.` также можно определить пользовательские категории выходных данных.
...
Раздел с комментариями `Code to process data and produce output file(s)` — это просто заполнитель для обычного кода задачи. Например, вам может понадобиться код, который скачивает данные из службы хранилища Azure и выполняет с ними операцию преобразования или вычисления. Важная часть этого фрагмента кода демонстрирует, как можно заключить такой код в `using` блок для периодического обновления файла с помощью [метода savetrackedasync][net_savetrackedasync].
