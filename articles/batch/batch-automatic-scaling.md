...
ms.locfileid: "80054014"
---
# <a name="create-an-automatic-formula-for-scaling-compute-nodes-in-a-batch-pool"></a>Создание автоматической формулы для масштабирования вычислительных узлов в пуле пакетов
...
>
>Если вы создали учетную запись с конфигурацией пользовательской подписки, то она использует квоту ядер по подписке. Дополнительные сведения см. в разделе [Ограничения виртуальных машин](../azure-resource-manager/management/azure-subscription-service-limits.md#virtual-machines-limits) в статье [Подписка Azure, границы, квоты и ограничения службы](../azure-resource-manager/management/azure-subscription-service-limits.md).
>
>
...
### <a name="sample-autoscale-formulas"></a>Образцы автомасштабных формул

Ниже приведены примеры двух формул автомасштаба, которые могут быть скорректированы для работы для большинства сценариев. Переменные `startingNumberOfVMs` и `maxNumberofVMs` в примере формулы могут быть скорректированы с учетом ваших потребностей.

#### <a name="pending-tasks"></a>Задачи в ожидании
...
#### <a name="preempted-nodes"></a>Упреждаемые узлы 
...
Этот пример создает пул, который начинается с 25 низкоприоритетных узлов. Каждый раз, когда узла с низким приоритетом упреждается, он заменяется выделенным узлам. Как и в первом `maxNumberofVMs` примере, переменная предотвращает превышение пула на 25 ВМ. Этот пример полезен для использования преимуществ малоприоритетных ВМ, а также для обеспечения того, чтобы в течение всего срока службы пула возникал только фиксированное количество упреждений.
...
| $TargetLowPriorityNodes |Целевое количество вычислительных узлов с низким приоритетом для пула. Количество узлов с низким приоритетом указывается как целевое, так как оно может остаться недостижимым для пула. Например, это может произойти, если целевое количество узлов с низким приоритетом изменяется после вычисления формулы автоматического масштабирования до того, как пул достигнет изначального целевого количества. Это также может произойти, если целевое количество превышает квоту узлов или ядер в учетной записи пакетной службы. <br /><br /> Для получения дополнительной информации о низкоприоритетных вычислительных узлах [см.](batch-low-pri-vms.md) |
| $NodeDeallocationOption |Действие, выполняемое после удаления вычислительных узлов из пула. Возможны следующие значения:<ul><li>**requeue**-- Значение по умолчанию. Немедленно прекращает задачи и возвращает их в очередь задания, чтобы они были перенесены. Это действие гарантирует, что целевое число узлов достигается как можно быстрее, но может быть менее эффективным, так как любые выполнения задач будут прерваны и должны быть перезапущены, тратя любую работу, которую они уже сделали. <li>**terminate** — немедленное завершение задач с удалением из очереди заданий;<li>**taskcompletion** — ожидание завершения выполнения текущих задач с удалением узла из пула; Используйте эту опцию, чтобы избежать прерывания и повторного выполнения задач, тратя любую работу, проделанную задачей. <li>**retaineddata** — ожидание очистки всех сохраненных в узле данных локальных задач перед удалением узла из пула.</ul> |

> [!NOTE]
> Переменная `$TargetDedicatedNodes` также может быть указана с помощью псевдонима. `$TargetDedicated` Аналогичным образом, переменная `$TargetLowPriorityNodes` может быть `$TargetLowPriority`указана с помощью псевдонима . Если и полностью названная переменная, и ее псевдоним устанавливаются формулой, значение, назначенное полностью названной переменной, будет иметь приоритет.
>
>
...
| $ActiveTasks |Количество задач, готовых к выполнению, но еще не выполняющихся. Значение $ActiveTasks включает все задачи в активном состоянии, зависимости которых выполнены. Задачи в активном состоянии, зависимости которых не выполнены, не включаются в значение $ActiveTasks. Для задачи с несколькими экземплярами $ActiveTasks будет включать в себя количество экземпляров, установленных в задаче.|
...
| $CurrentLowPriorityNodes |Текущее количество низкоприоритетных вычислительных узлов, включая любые узлы, которые были упреждан. |
...
## <a name="obtain-sample-data"></a><a name="getsampledata"></a>Получение выборки данных
...
1. Уменьшать целевое количество выделенных вычислительных узлов в пуле при низкой загрузке ЦП.
1. Всегда ограничивать максимальное количество выделенных узлов на уровне 400.
1. При уменьшении количества узлов не удаляйте узлы, которые выполняют задачи; при необходимости подождите, пока задачи не закончим, чтобы удалить узлы.
...
## <a name="create-an-autoscale-enabled-pool-with-batch-sdks"></a>Создание пула с автоматической поддержкой с помощью пакетных SDK

Автоматическое использование пула можно настроить с помощью любого из [пакетных SDK,](batch-apis-tools.md#azure-accounts-for-batch-development) [Batch REST API](https://docs.microsoft.com/rest/api/batchservice/) [Batch PowerShell cmdlets](batch-powershell-cmdlets-get-started.md)и [Batch CLI.](batch-cli-get-started.md) В этом разделе можно привести примеры как для .NET, так и для Python.
...
Аналогичным образом, вы можете сделать пул с поддержкой автомасштабирования с помощью Python SDK по:

1. Создайте пул и укажите его конфигурацию.
1. Добавьте пул к клиенту службы.
1. Включите автомасштабвику в бассейне с формулой, которую вы пишете.
...
> [!TIP]
> Другие примеры использования Python SDK можно найти в [репозитории пакета Python quickstart](https://github.com/Azure-Samples/batch-python-quickstart) на GitHub.
>
>
...
Следующий фрагмент кода C'net использует библиотеку Batch .NET для печати информации о последнем автомасштабировании в пуле _myPool:_
...
$NodeDeallocationOption = taskcompletion;
```
`$curTime`может быть скорректирована с учетом `time()` вашего местного часового пояса, добавив к продукту и ваш UTC смещения. `TimeZoneInterval_Hour` Например, `$curTime = time() + (-6 * TimeInterval_Hour);` использовать для горного дневного времени (MDT). Имейте в виду, что смещение необходимо будет скорректировать в начале и конце летнего времени (если это применимо).
