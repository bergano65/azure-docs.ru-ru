...
#### <a name="preempted-nodes"></a>Прерванные узлы 
...
В этом примере создается пул, который начинается с 25 узлов с низким приоритетом. При каждом прерывании узла с низким приоритетом он заменяется выделенным узлом. Как и в первом примере, переменная `maxNumberofVMs` предотвращает превышение в пуле более 25 виртуальных машин. Этот пример полезен для использования с виртуальными машинами с низким приоритетом, а также для обеспечения того, что в течение всего времени существования пула произойдет только фиксированное количество вытеснений.
...
| $TargetLowPriorityNodes |Целевое количество вычислительных узлов с низким приоритетом для пула. Количество узлов с низким приоритетом указывается как целевое, так как оно может остаться недостижимым для пула. Например, это может произойти, если целевое количество узлов с низким приоритетом изменяется после вычисления формулы автоматического масштабирования до того, как пул достигнет изначального целевого количества. Это также может произойти, если целевое количество превышает квоту узлов или ядер в учетной записи пакетной службы. <br /><br /> Дополнительные сведения о кластерных узлах с низким приоритетом см. в статье [Использование виртуальных машин с низким приоритетом с пакетной службой](batch-low-pri-vms.md). |
| $NodeDeallocationOption |Действие, выполняемое после удаления вычислительных узлов из пула. Возможны следующие значения:<ul><li>**requeue**— значение по умолчанию. Немедленно прекращает выполнение задач и помещает их в очередь заданий, чтобы они были перепланированы. Это действие гарантирует, что целевое число узлов станет как можно быстрее, но может быть менее эффективным, так как все выполняющиеся задачи будут прерваны и их необходимо будет перезапустить, что приведет к тому, что все уже выполненные им операции не появятся. <li>**terminate** — немедленное завершение задач с удалением из очереди заданий;<li>**taskcompletion** — ожидание завершения выполнения текущих задач с удалением узла из пула; Используйте этот параметр, чтобы избежать прерывания и повторного постановки задач в очередь, что затратит все усилия, выполненные задачей. <li>**retaineddata** — ожидание очистки всех сохраненных в узле данных локальных задач перед удалением узла из пула.</ul> |

> [!NOTE]
> Переменную `$TargetDedicatedNodes` также можно указать с помощью псевдонима `$TargetDedicated`. Аналогичным образом переменную `$TargetLowPriorityNodes` можно указать с помощью псевдонима `$TargetLowPriority`. Если в формуле задаются как полностью именованная переменная, так и ее псевдоним, то значение, присвоенное переменной с полным именем, будет иметь приоритет.
>
>
...
1. При уменьшении числа узлов не удаляйте узлы, на которых выполняются задачи. При необходимости дождитесь завершения задач, чтобы удалить узлы.
...
* [BatchClient. PoolOperations. енаблеаутоскалеасинк][net_enableautoscaleasync] (пакетная платформа .NET)
* [Включение автоматического масштабирования в пуле][rest_enableautoscale] (REST API)
...
В C# этом фрагменте кода используется библиотека [.NET пакетной][net_api] службы для включения автоматического масштабирования в существующем пуле:
...
В этом фрагменте кода .NET для этой [пакетной][net_api] программы вычисляется формула автомасштабирования. Если в пуле не включено автомасштабирование, сначала необходимо его включить.
...
В этом примере размер пула изменяется на основе числа задач. Эта формула также учитывает значение [MaxTasksPerComputeNode][net_maxtasks] , установленное для пула. Такой подход полезен, если в пуле поддерживается [параллельное выполнение задач](batch-parallel-node-tasks.md).
