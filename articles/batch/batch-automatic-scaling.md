...
> [!NOTE]
> `$TargetDedicatedNodes` Переменную также можно указать с помощью псевдонима `$TargetDedicated`. Аналогичным образом `$TargetLowPriorityNodes` переменная может быть указана с помощью `$TargetLowPriority`псевдонима. Если в формуле задаются как полностью именованная переменная, так и ее псевдоним, то значение, присвоенное переменной с полным именем, будет иметь приоритет.
>
>
...
В следующем фрагменте кода C# используется библиотека .NET пакетной службы для вывода сведений о последнем запуске автомасштабирования в пуле _myPool_:
...
$NodeDeallocationOption = taskcompletion;
```
`$curTime`можно настроить в соответствии с местным часовым поясом, добавив `time()` к продукту `TimeZoneInterval_Hour` и смещению UTC. Например, используйте `$curTime = time() + (-6 * TimeInterval_Hour);` для летнего времени (для MDT). Помните, что смещение должно быть скорректировано в начале и в конце летнего времени (если применимо).
