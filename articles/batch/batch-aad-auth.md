---
title: Authenticate службы Azure Batch с помощью активного каталога Azure
description: Пакетная служба поддерживает Azure AD для аутентификации из пакетной службы. Узнайте, как проверить подлинность одним из двух способов.
services: batch
...
1. На портале Azure на панели навигации слева выберите **Все службы**. Выберите **регистрацию приложений**.
...
1. Выберите приложение и выберите **разрешения API.**
1. В разделе **разрешений API** выберите **добавление разрешения.**
1. В разделе **Выбор API** найдите API пакетной службы. Продолжайте поиск для каждой из этих строк, пока не найдете API:
    1. **Пакет Microsoft Azure**
    1. **ddbf3205-c6bd-46ae-8127-60eb93363864** — это идентификатор API пакетной службы.
1. Как только вы найдете API пакета, выберите его и **выберите.**
1. В **выберите разрешения**выберите флажок рядом с **службой пакетов Access Azure,** а затем выберите **разрешения «Добавить».**

В разделе **разрешений API** теперь показано, что приложение Azure AD имеет доступ как к Microsoft Graph, так и к API службы пакетов. Разрешения выдаются Microsoft Graph автоматически при первой регистрации приложения в Azure AD.
...
1. Запросите секрет для вашего приложения.
1. Присвоить приложению элемент управления доступом на основе ролей (RBAC).

### <a name="request-a-secret-for-your-application"></a>Запросить секрет для вашего приложения

Когда приложение проверяет подлинность с помощью принципала службы, оно отправляет идентификатор приложения и секрет в Azure AD. Вам необходимо создать и скопировать секретный ключ, чтобы использовать его из кода.

На портале Azure сделайте следующее:

1. На портале Azure на панели навигации слева выберите **Все службы**. Выберите **регистрацию приложений**.
1. Выберите приложение из списка регистраций приложений.
1. Выберите приложение, а затем выберите **Сертификаты & секреты.** В разделе **Секреты Клиента** выберите **Новый секрет клиента.**
1. Чтобы создать секрет, введите описание для секрета. Затем выберите срок действия для секрета либо один год, два года, или нет срока действия.
1. Выберите **Добавить,** чтобы создать и отобразить секрет. Копируйте секретное значение в безопасное место, так как вы не сможете получить к нему доступ снова после того, как вы покинете страницу.

    ![Создание секретного ключа](./media/batch-aad-auth/secret-key.png)

### <a name="assign-rbac-to-your-application"></a>Назначить RBAC в ваше приложение

Чтобы проверить подлинность с помощью принципала службы, необходимо назначить RBAC в приложение. Выполните следующие действия.

1. На портале Azure перейдите к учетной записи пакетной службы, используемой приложением.
1. В разделе **Настройки** учетной записи пакета выберите **элемент управления доступом (IAM).**
...
1. В поле **Выбрать** введите имя приложения. Выберите приложение из списка, а затем выберите **Сохранить**.
...
Пользовательская роль предоставляет пользователю детальное разрешение на отправку заданий, задач и многое другое. Это позволяет пользователям не допускать выполнения пользователями операций, влияющих на стоимость, таких как создание пулов или изменение узлов.

Можно использовать пользовательскую роль для предоставления разрешений пользователю, группе или обслуживанию Azure AD для следующих операций RBAC:
...
Пользовательские роли предназначены для пользователей, удостоверяющих подлинность Azure AD, а не учетных данных учетной записи Пакета (общий ключ). Обратите внимание, что учетные данные учетной записи Пакета дают полное разрешение учетной записи Batch. Также обратите внимание, что задания с использованием автопула требуют разрешения на уровне пула.

Вот пример пользовательского определения роли:
...
Для получения более подробной информации [Custom roles for Azure resources](../role-based-access-control/custom-roles.md)о создании пользовательской роли см.
...
[aad_auth_scenarios]: ../active-directory/active-directory-authentication-scenarios.md "Сценарии аутентификации для Azure AD"
