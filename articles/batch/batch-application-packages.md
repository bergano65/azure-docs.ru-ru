...
Если вы хотели структуре файла пакета приложения на у вычислительного узла, перейдите к учетной записи пакетной службы на портале. Из учетной записи пакетной службы, перейдите к **пулы**. Выберите пул, содержащий к узлам вычислений, которые вас интересуют.

![Узлы в пуле][13]

После выбора этого пула, перейдите к установленного пакета приложения на вычислительном узле. После этого данные пакета приложения находятся в папке **приложений** папки. Дополнительные папки на вычислительном узле содержат другие файлы, например задачи запуска, выходные файлы, вывод ошибок, и т.д.
...
Как вы видите, поля совпадают с **новое приложение** окна, но **идентификатор приложения** отключенного поля. Как и для нового приложения, укажите **версию** нового пакета, выберите ZIP-файл **пакета приложения**, а затем нажмите кнопку **OК**, чтобы передать пакет.
...
[13]: ./media/batch-application-packages/package-file-structure.png "Сведения об узле, на портале Azure COMPUTE"
[14]: ./media/batch-application-packages/package-file-structure-node.png "Файлы на вычислительном узле, на портале Azure"
