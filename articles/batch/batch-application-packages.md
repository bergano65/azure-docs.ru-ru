...
Функция пакетов приложений в пакетной службе Azure помогает управлять приложениями задач и их развертыванием для вычислений узлов в пулах. Пакеты приложений позволяют упростить код в решении пакетной службы, а также сократить расходы на управление приложениями, выполняемыми задачами. С помощью пакетов приложений вы можете передать несколько версий приложений, выполняемых вашими задачами (в том числе вспомогательные файлы), а также управлять ими, а затем автоматически развернуть одно или несколько таких приложений на вычислительных узлах в пуле.

Пакеты приложений могут помочь вашим клиентам выбирать приложения для своих заданий, указывая точную версию, используемую при обработке заданий с помощью службы с поддержкой пакетов. Вы также можете предоставить своим клиентам возможность отправлять и отслеживаниь своих собственных приложений в службе.
...
В этой статье объясняется, как отправлять пакеты приложений и управлять ими в портал Azure и как устанавливать их на расчетные узлы пула с помощью библиотеки [.NET пакетной](/dotnet/api/overview/azure/batch/client) службы.
...
> [!NOTE]
> Пулы пакетной службы, созданные до 5 июля, 2017 не поддерживают пакеты приложений (если они не были созданы после 10 марта 2016 с использованием конфигурации облачных служб).
>
...
В пакетной службе Azure *приложение* ссылается на набор двоичных файлов с возможностью управления версиями, которые могут автоматически скачиваться на вычислительные узлы в пуле. *Пакет приложения* ссылается на конкретный набор этих двоичных файлов, представляющий заданную версию приложения.
...
*Приложение* в пакете содержит один или несколько пакетов приложений и задает параметры конфигурации для приложения. Например, приложение может указать версию пакета приложения по умолчанию для установки на вычислительных узлах, а также определяет необходимость обновления или удаления пакетов.

*Пакет приложения* — это ZIP-файл, который содержит двоичные файлы приложения и вспомогательных файлов, необходимых задачам для запуска приложения. Каждый пакет приложения представляет определенную версию приложения. Поддерживается только формат ZIP.

Пакеты приложений можно указать на уровне пула или задачи. При создании пула или задачи можно указать один или несколько таких пакетов и (при необходимости) версию.
...
При использовании пакетов приложений задача запуска пула не должна указывать длинный список отдельных файлов ресурсов для установки на узлах. Кроме того, вам не нужно вручную управлять несколькими версиями этих файлов приложения в службе хранилища Azure или на узлах, а также создавать [URL-адреса SAS](../storage/common/storage-sas-overview.md), чтобы предоставить доступ к файлам в своей учетной записи хранения. Пакетная служба работает со службой хранилища Azure в фоновом режиме, обеспечивая хранение пакетов приложений и их развертывание на вычислительных узлах.

> [!NOTE]
> Общий размер задачи запуска не должен превышать 32 768 символов, включая файлы ресурсов и переменные среды. Если задача запуска превышает это ограничение, то можно воспользоваться пакетами приложений. Вы также можете создать ZIP-файл, содержащий файлы ресурсов, передать его в службу хранилища Azure в виде большого двоичного объекта, а затем извлечь из командной строки задачи запуска.
...
:::image type="content" source="media/batch-application-packages/app_pkg_02.png" alt-text="Снимок экрана: пункт меню "приложения" в портал Azure.":::
...
Чтобы просмотреть [структуру файлов](files-and-directories.md) пакета приложения на кластерном узле, перейдите к учетной записи пакетной службы в портал Azure. Выберите **Пулы** , а затем выберите пул, содержащий нужный вам расчетный узел. Затем выберите узел вычислений, на котором установлен пакет приложения, и откройте папку **приложения** .
...
- **Разрешить обновления**: указывает, можно ли [Обновить или удалить](#update-or-delete-an-application-package)пакеты приложений. По умолчанию используется значение **Да**. Если задано значение **нет**, обновления и удаления пакетов не будут разрешены для приложения, хотя можно добавлять новые версии пакетов приложений.
...
Чтобы создать новое приложение, добавьте пакет приложения и укажите новый уникальный идентификатор приложения.

В учетной записи пакетной службы выберите **приложения** и нажмите кнопку **Добавить**.

:::image type="content" source="media/batch-application-packages/app_pkg_05.png" alt-text="Снимок экрана нового процесса создания приложения в портал Azure.":::
...
:::image type="content" source="media/batch-application-packages/app_pkg_07.png" alt-text="Снимок экрана, показывающий параметры обновления и удаления для пакетов приложений в портал Azure.":::
...
> [!IMPORTANT]
> Если по какой-либо причине произошел сбой развертывания пакета приложения, то пакетная служба помечает узел как [непригодный](/dotnet/api/microsoft.azure.batch.computenode.state) и на этом узле не планируется выполнение каких-либо задач. В этом случае следует перезапустить этот узел, чтобы повторно инициировать развертывание пакета. Перезапуск узла также возобновляет планирование задач на узле.
