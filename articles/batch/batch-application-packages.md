...
>
> API-интерфейсы для создания пакетов приложений и управления ими являются частью библиотеки [.NET для управления пакетной][api_net_mgmt] службой. API-интерфейсы для установки пакетов приложений на кластерном узле являются частью библиотеки [.NET пакетной][api_net] службы. Схожие функции находятся в доступных API-интерфейсах пакетной службы для других языков. 
>
...
> - Учетная запись хранения Azure с **иерархическим пространством имен** с **включенным** параметром "включено" не может использоваться для пакетов приложений.

Пакетная служба использует службу хранилища Azure для хранения пакетов приложений в виде блочных BLOB-объектов. Вы платите [как обычная][storage_pricing] для данных блочного BLOB-объекта, и размер каждого пакета не может превышать максимальный размер блочного BLOB-объекта. Дополнительные сведения см. в статье [целевые показатели масштабируемости и производительности службы хранилища Azure для учетных записей хранения](../storage/blobs/scalability-targets.md). Вам обязательно следует учитывать размер и количество пакетов приложений, периодически удаляя устаревшие пакеты для минимизации расходов.
...
В пакетной среде .NET укажите один или несколько [CloudPool][net_cloudpool]. [ApplicationPackageReferences][net_cloudpool_pkgref] при создании нового пула или для существующего пула. Класс [ApplicationPackageReference][net_pkgref] указывает идентификатор и версию приложения для установки на расчетные узлы пула.
...
Чтобы установить пакет приложения задачи, настройте [CloudTask][net_cloudtask]задачи. [ApplicationPackageReferences][net_cloudtask_pkgref] , свойство:
...
Вы можете перечислить приложения и их пакеты в учетной записи пакетной службы с помощью [метода applicationoperations][net_appops]. Метод [ListApplicationSummaries][net_appops_listappsummaries] .
...
* [Пакетная REST API][api_rest] также предоставляет поддержку для работы с пакетами приложений. Например, см. элемент [applicationPackageReferences][rest_add_pool_with_packages] в разделе [Добавление пула в учетную запись][rest_add_pool] для получения сведений о том, как указать пакеты для установки с помощью REST API. Дополнительные сведения о получении сведений о приложении с помощью пакетного REST API см. в разделе [приложения][rest_applications] .
