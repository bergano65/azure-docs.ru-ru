...
> - Учетную запись хранения Azure, у которой параметру **Иерархическое пространство имен** задано значение **Включено**, нельзя использовать для пакетов приложений.
...
Чтобы просмотреть структуру файлов пакета приложения на вычислительном узле, перейдите к учетной записи пакетной службы на портале. В учетной записи пакетной службы выберите **Пулы**. Выберите пул, содержащий нужные вычислительные узлы.

![Узлы в пуле][13]

Выбрав пул, перейдите к вычислительному узлу, на котором установлен пакет приложения. Сведения о пакете приложения находятся в папке **applications**. В дополнительных папках на вычислительном узле содержатся другие файлы, такие как задачи запуска, выходные файлы, выходные данные ошибок и т. д.
...
[13]: ./media/batch-application-packages/package-file-structure.png "Сведения о вычислительном узле на портале Azure"
[14]: ./media/batch-application-packages/package-file-structure-node.png "Файлы на вычислительном узле, которые отображаются на портале Azure"
