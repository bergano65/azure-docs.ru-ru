...
![Предупреждение «Нет учетной записи хранилища» на портале Azure][9]
...
Мы рекомендуем создать учетную запись хранения *специально* для использования с учетной записью Batch и выбрать ее здесь. Созданную учетную запись хранения затем можно связать с учетной записью пакетной службы в окне **Учетная запись хранения**.
...
> - Учетная запись хранилища Azure с **иерархическим пространством имен,** установленным для **Enabled,** не может использоваться для пакетов приложений.

Пакетная служба использует службу хранилища Azure для хранения пакетов приложений в виде блочных BLOB-объектов. Вы [заряжаетесь как обычно][storage_pricing] для данных blob блока, и размер каждого пакета не может превышать максимальный размер blob блока. Для получения дополнительной информации смотрите [целевые показатели масштабируемости и производительности хранилища Azure Storage для учетных записей.](../storage/blobs/scalability-targets.md) Вам обязательно следует учитывать размер и количество пакетов приложений, периодически удаляя устаревшие пакеты для минимизации расходов.
...
* **Разрешить обновления**: значение, которое определяет возможность обновления, удаления и дополнения пакетов. Если для этого параметра задано значение **Нет**, возможность обновления и удаления пакетов будет отключена для приложения. Вы сможете только добавлять новые версии пакета приложения. По умолчанию **Да**.

Если вы хотите увидеть структуру файлов пакета приложений на вашем вычислительном узло, перейдите на свой аккаунт пакета на портале. С вашей учетной записи пакета перейдите в **пулы.** Выберите пул, содержащий интересующие вас вычислительный узла.

![Узлы в бассейне][13]

После того как вы выбрали пул, перейдите к вычислительному узлам, на который установлен пакет приложений. Оттуда детали пакета приложений находятся в папке **приложений.** Дополнительные папки на вычислительном узеле содержат другие файлы, такие как задачи запуска, выходные файлы, выход ошибок и т.д.
...
Как вы можете видеть, поля совпадают с полями нового окна **приложения,** но поле **идентификатора приложения** отключено. Как и для нового приложения, укажите **версию** нового пакета, выберите ZIP-файл **пакета приложения**, а затем нажмите кнопку **OК**, чтобы передать пакет.
...
[5]: ./media/batch-application-packages/app_pkg_05.png "Новое лезвие приложения на портале Azure"
...
[10]: ./media/batch-application-packages/app_pkg_10.png "Выберите лезвие учетной записи хранения на портале Azure"
[11]: ./media/batch-application-packages/app_pkg_11.png "Обновление лезвия пакета на портале Azure"
[12]: ./media/batch-application-packages/app_pkg_12.png "Диалоговое окно для подтверждения удаления пакета на портале Azure"
[13]: ./media/batch-application-packages/package-file-structure.png "Вычислите информацию о узеле на портале Azure"
[14]: ./media/batch-application-packages/package-file-structure-node.png "Файлы на вычислительном узеле, отображаемые на портале Azure"
