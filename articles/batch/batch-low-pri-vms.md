...
> [!NOTE]
> [Spot VMs](https://azure.microsoft.com/pricing/spot/) теперь доступны для [одного экземпляра VMs](https://docs.microsoft.com/azure/virtual-machines/linux/spot-vms) и [наборов масштаба VM.](https://docs.microsoft.com/azure/virtual-machine-scale-sets/use-spot) Spot VMs являются эволюцией низкоприоритетных VM, но отличаются тем, что цены могут варьироваться и дополнительная максимальная цена может быть установлена при распределении Spot VMs.
>
> Пулы Azure Batch начнут поддерживать Spot VMs в течение нескольких месяцев с их обычной доступностью с новыми версиями [AAP и инструментами пакетных AIS.](https://docs.microsoft.com/azure/batch/batch-apis-tools) Как только поддержка Spot VM будет доступна, малоприоритетные ВМ будут обесточены - они будут по-прежнему поддерживаться с помощью текущих AA и версий инструментов в течение по крайней мере 12 месяцев, чтобы дать достаточно времени для миграции в Spot VMs. 
>
> Spot VMs не будут поддерживаться для пулов [конфигурации облачных служб.](https://docs.microsoft.com/rest/api/batchservice/pool/add#cloudserviceconfiguration) Для использования Spot VMs пулы облачных сервисов необходимо будет перенести в пулы [виртуальной конфигурации машин.](https://docs.microsoft.com/rest/api/batchservice/pool/add#virtualmachineconfiguration)
...
-   Пул постоянно будет пытаться подключиться к целевым доступным низкоприоритетным узлам. При обнаружении емкости замены узлы сохраняют свои идентифицируются, но переизменяются, проходя через состояния **создания** и **запуска,** прежде чем они будут доступны для планирования задач.
...
* Начните планировать переход от низкоприоритетных ВМ к Spot VMs. Если вы используете низкоприоритетные виртуальные с пулами **конфигураций облачных сервисов,** запланируйте переход к пулам **конфигурации Виртуальной машины.**
