---
title: Запуск рабочих нагрузок на виртуальных машинах с низким приоритетом с низкой стоимостью
...
> [!NOTE]
> Теперь [Виртуальные](https://azure.microsoft.com/pricing/spot/) машины с [одним экземпляром](https://docs.microsoft.com/azure/virtual-machines/linux/spot-vms) и [масштабируемыми наборами виртуальных](https://docs.microsoft.com/azure/virtual-machine-scale-sets/use-spot)машин доступны для виртуальных машин. Плашечные виртуальные машины — это эволюция виртуальных машин с низким приоритетом, но они отличаются в разных ценах, и при выделении плашечных виртуальных машин может быть задана необязательная максимальная цена.
>
> Пулы пакетной службы Azure начнут поддерживать виртуальные машины в общедоступных виртуальных машинах с новыми версиями [API-интерфейсов и средств пакетной](https://docs.microsoft.com/azure/batch/batch-apis-tools)службы. После того как доступна несущее множество виртуальных машин, виртуальные машины с низким приоритетом будут устаревшими — они продолжат поддерживаться с использованием текущих API и версий инструментов в течение не менее 12 месяцев, чтобы обеспечить достаточное время для миграции на виртуальные машины. 
>
> Плашечные виртуальные машины не будут поддерживаться для пулов [конфигурации облачной службы](https://docs.microsoft.com/rest/api/batchservice/pool/add#cloudserviceconfiguration) . Для использования плашечных виртуальных машин пулы облачных служб потребуется перенести в пулы [конфигураций виртуальных машин](https://docs.microsoft.com/rest/api/batchservice/pool/add#virtualmachineconfiguration) .
...
-   Пул постоянно будет пытаться подключиться к целевым доступным низкоприоритетным узлам. При обнаружении заменяемой емкости узлы сохраняют свои идентификаторы, но повторно инициализируются путем **создания** и **запуска** состояний, прежде чем они будут доступны для планирования задач.
...
* Начните планировать перемещение с виртуальных машин с низким приоритетом на плашечные виртуальные машины. Если вы используете виртуальные машины с низким приоритетом с пулами **конфигурации облачной службы** , запланируйте переход на пулы **конфигураций виртуальных машин** .
