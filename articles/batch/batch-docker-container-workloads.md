...
Пакетная поддержка содержит образы Windows Server с поддержкой контейнеров. Обычно имена SKU образа имеют суффикс `-with-containers` или `-with-containers-smalldisk`. Кроме того, [API-интерфейс для перечисления всех поддерживаемых образов в пакетной](batch-linux-nodes.md#list-of-virtual-machine-images) службе будет заметка `DockerCompatible` возможности, если образ поддерживает контейнеры DOCKER.
