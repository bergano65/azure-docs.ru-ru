...
Для запуска задач контейнера на вычислительных узлах необходимо указать параметры для конкретного контейнера, например варианты выполнения контейнера, используемые образы и реестр.
...
Необязательный аргумент [ContainerRunOptions](/dotnet/api/microsoft.azure.batch.taskcontainersettings.containerrunoptions) содержит дополнительные аргументы для команды `docker create`, которую задача выполняет для создания контейнера.
...
Так как пакетная служба изменяет в контейнере используемый по умолчанию каталог, задача выполняется не в обычном для этого контейнера рабочем каталоге (например, в контейнере Windows по умолчанию используется `c:\`, а в контейнере Linux — `/`, а для отдельных образов контейнеров могут быть настроены другие каталоги). Чтобы убедиться, что приложения-контейнеры правильно выполняются в контексте пакетной службы, выполните одно из следующих действий. 

* Убедитесь, что в командной строке задачи или в рабочей папке контейнера используется абсолютный путь, и укажите его при необходимости.

* В файле ContainerSettings для задачи задайте рабочий каталог в параметрах выполнения контейнера. Например, `--workdir /app`.

В приведенном ниже фрагменте кода Python показана обычная командная строка, которая запущена в контейнере Ubuntu, извлеченном из центра Docker. Здесь параметр запуска контейнера `--rm` означает удаление контейнера после выполнения задачи.
