---
title: Контейнерные рабочие нагрузки - Пакет Azure
description: Узнайте, как запускать и масштабировать приложения из изображений контейнеров в Azure Batch. Создайте пул вычислительных узлов, поддерживающих задачи работы с бегущими контейнерами.
services: batch
author: LauraBrenner
manager: evansma
ms.service: batch
...
* **Версии SDK**: Пакетные SDK поддерживают изображения контейнеров по следующим версиям:
...
Пакет поддерживает изображения сервера Windows, которые имеют обозначения поддержки контейнеров. Как правило, эти имена sku `-with-containers` `-with-containers-smalldisk`суффиксс с или . Кроме того, [API, перечисляемый всеми поддерживаемыми изображениями в пакете,](batch-linux-nodes.md#list-of-virtual-machine-images) будет обозначать `DockerCompatible` возможность, если изображение поддерживает контейнеры Docker.
...
Для рабочих нагрузок контейнеров Linux Batch в настоящее время поддерживает следующие изображения Linux, опубликованные Microsoft Azure Batch в Azure Marketplace без необходимости в пользовательском изображении.

#### <a name="vm-sizes-without-rdma"></a>Размеры VM без RDMA

- Издателя:`microsoft-azure-batch`
  - Предлагаем:`centos-container`
  - Предлагаем:`ubuntu-server-container`

#### <a name="vm-sizes-with-rdma"></a>Размеры VM с RDMA

- Издателя:`microsoft-azure-batch`
  - Предлагаем:`centos-container-rdma`
  - Предлагаем:`ubuntu-server-container-rdma`

Эти изображения поддерживаются только для использования в пулах Пакетов Azure и предназначены для выполнения контейнеров Docker. Они содержат следующее:

* Предустановленное время выполнения контейнера Сакер-совместимый [Moby](https://github.com/moby/moby)

* Предварительно установленные драйверы NVIDIA GPU и время выполнения контейнера NVIDIA для оптимизации развертывания на VMs-ми серияaz Azure N

* Предварительно установленное/предварительно настроенное изображение с поддержкой размеров Infiniband RDMA `-rdma`VM для изображений с суффиксом . В настоящее время эти изображения не поддерживают размеры SR-IOV IB/RDMA VM.
