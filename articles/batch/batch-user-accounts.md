---
title: Выполнение задач с учетными записями пользователей
description: Сведения о типах учетных записей пользователей и их настройке.
ms.topic: how-to
ms.date: 08/20/2020
ms.custom: seodec18
...
>
> Подключение к узлу с конфигурацией виртуальной машины Linux по протоколу SSH описывается в разделе [Установка и настройка удаленного рабочего стола для подключения к виртуальной машине Linux в Azure](../virtual-machines/linux/use-remote-desktop.md). Подключение к узлам под управлением Windows по протоколу RDP описывается в разделе [Как подключиться к виртуальной машине Azure под управлением Windows и войти на нее](../virtual-machines/windows/connect-logon.md).<br /><br />
...
Задача в пакетной службе Azure всегда выполняется с учетной записью пользователя. По умолчанию задачи выполняются со стандартными учетными записями пользователей без разрешений администратора. В некоторых случаях может потребоваться настроить учетную запись пользователя, под которой должна выполняться задача. В этой статье обсуждаются типы учетных записей пользователей и способы их настройки для вашего сценария.
...
> [!IMPORTANT]
> В пакетной службе версии 2017-01-01.4.0 введено критическое изменение, требующее обновления кода для вызова этой версии. Если вы переносите код из более ранней версии пакетной службы, имейте ввиду, что свойство **runElevated** больше не поддерживается в клиентских библиотеках пакетной службы или REST API. Используйте новое свойство задачи **userIdentity** для указания уровня прав. Рекомендации по обновлению кода пакета при использовании одной из клиентских библиотек см. [в статье обновление кода до последней версии клиентской библиотеки пакетной](#update-your-code-to-the-latest-batch-client-library) службы.

## <a name="user-account-access-to-files-and-directories"></a>Доступ учетных записей пользователей к файлам и каталогам

Как автоматическая учетная запись пользователя, так и именованная учетная запись пользователя имеют доступ на чтение и запись к рабочему каталогу задачи, общему каталогу и каталогу задач с несколькими экземплярами. Учетные записи обоих типов имеют доступ на чтение к каталогам запуска и подготовки заданий.
...
Дополнительные сведения о доступе к файлам и каталогам из задачи см. в разделе [Файлы и каталоги](files-and-directories.md).
...
По умолчанию задачи выполняются в пакетной службе с автоматической учетной записью пользователя, как обычный пользователь без повышенного доступа и с областью пула. Область пула означает, что задача выполняется с автоматической учетной записью пользователя, доступной для любой задачи в пуле. Дополнительные сведения о области пула см. в разделе [Запуск задачи как автоматического пользователя с областью пула](#run-a-task-as-an-auto-user-with-pool-scope).

Альтернативой области действия пула является область задач. Если для области задачи настроена спецификация автоматических пользователей, пакетная служба создает автоматическую учетную запись пользователя для выполнения только этой задачи.
...
- Доступ без прав администратора к области задач
...
> [!NOTE]
> Две автоматические учетные записи пользователей пула — это отдельные учетные записи. Задачи, выполняемые в учетной записи администратора уровня пула, не могут обмениваться данными с задачами, выполняемыми с учетной записью Standard, и наоборот.
...
- Узнайте подробнее о [файлах и каталогах](files-and-directories.md) в пакетной службе Azure.
