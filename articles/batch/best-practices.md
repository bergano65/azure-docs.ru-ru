...
- **Обеспечьте актуальность пулов.**
    Необходимо изменить размер пулов на ноль каждые несколько месяцев, чтобы получить [последние обновления агента узла и исправления ошибок](https://github.com/Azure/Batch/blob/master/changelogs/nodeagent/CHANGELOG.md). Пул не будет принимать обновления агентов узлов, если он не создан повторно или его размер не приведен к 0 узлов вычислений. Перед повторным созданием или изменением размера пула рекомендуется скачать все журналы агентов узлов для отладки, как описано в разделе [Узлы](#nodes).
