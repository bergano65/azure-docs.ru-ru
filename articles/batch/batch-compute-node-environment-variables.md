---
title: Переменные среды выполнения задач - Azure Batch Документы Майкрософт
description: Переменные рекомендации и рекомендации среды выполнения задач для аналитики пакетов Azure.
services: batch
author: LauraBrenner
manager: evansma
ms.assetid: ''
...
ms.locfileid: "80053950"
---
# <a name="azure-batch-runtime-environment-variables"></a>Переменные среды выполнения пакета Azure
...
Для получения дополнительной информации об использовании переменных среды с пакетом [см.](https://docs.microsoft.com/azure/batch/batch-api-basics#environment-settings-for-tasks)
...
Чтобы получить текущее значение переменной `cmd.exe` среды, запустите `/bin/sh` на вычислительном узлах Windows или на узлах Linux:
...
| AZ_BATCH_APP_PACKAGE            | Префикс всех переменных среды пакета приложения. Например, если версия приложения "FOO" установлена на пул, переменная среды AZ_BATCH_APP_PACKAGE_FOO_1 (на Linux) или AZ_BATCH_APP_PACKAGE_FOO No1 (на Windows). AZ_BATCH_APP_PACKAGE_FOO_1 указывает на место загрузки пакета (папка). При использовании версии пакета приложения по умолчанию используйте переменную AZ_BATCH_APP_PACKAGE среды без номеров версий. Если в Linux, и название пакета приложения "Агент-linux-x64" и версия "1.1.46.0, имя среды на самом деле: AZ_BATCH_APP_PACKAGE_agent_linux_x64_1_1_46_0, используя подчеркивание и нижний случай. Дополнительные сведения см. [здесь](https://docs.microsoft.com/azure/batch/batch-application-packages#execute-the-installed-applications). | Любую задачу с помощью пакета связанного приложения. Доступно также для всех задач, если сам узел имеет пакеты приложений. | AZ_BATCH_APP_PACKAGE_FOO_1 (Linux) или AZ_BATCH_APP_PACKAGE_FOO No1 (Windows) |
...
| AZ_BATCH_JOB_PREP_WORKING_DIR   | Полный путь выполнения задачи по подготовке к работе [с каталогом][files_dirs] на узлах. | Все задачи, кроме задачи запуска и задачи подготовки задания. Доступна, только если для задания настроена задача подготовки задания. | C:\user\tasks\workitems\jobprepreleasesamplejob\job-1\jobpreparation\wd |
...
| AZ_BATCH_NODE_MOUNTS_DIR        | Полный путь [файловой системы](virtual-file-mount.md) уровня узла монтируется, где находятся все каталоги крепления. В долях файлов Windows используется дисковая буква, поэтому для Windows привод является частью устройств и дисков.  |  Все задачи, включая задачу запуска, имеют доступ к пользователю, учитывая, что пользователь знает о разрешениях крепления для установленного каталога. | В Ubuntu, например, расположение:`/mnt/batch/tasks/fsmounts` |
...
| AZ_BATCH_NODE_SHARED_DIR        | Полный путь [общего каталога][files_dirs] на узлах. Все задачи, которые выполняются на узле, имеют доступ на чтение и запись для этого каталога. У задач, выполняемых на других узлах, нет удаленного доступа к этому каталогу (это не "общий" сетевой каталог). | Все задачи. | C:\user\tasks\shared |
