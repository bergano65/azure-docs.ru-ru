...
| AZ_BATCH_CERTIFICATES_DIR       | Каталог в [рабочем каталоге задачи][files_dirs], где хранятся сертификаты для вычислительных узлов Linux. Эта переменная среды не применяется к вычисленным узлам Windows.                                                  | Все задачи.   |  /mnt/batch/tasks/workitems/batchjob001/job-1/task001/certs |
