...
|[`multiInstanceSettings`](#multiInstanceSettings)|Сложный тип|Указывает, что задача включает в себя несколько экземпляров и требует несколько вычислительных узлов.  Подробные сведения см. в статье о [`multiInstanceSettings`](/rest/api/batchservice/get-information-about-a-task).|
