---
title: 'Пример: Многоуровневые грани'
...
---
# <a name="example-multi-level-facets-in-azure-cognitive-search"></a>Пример: Многоуровневые аспекты в когнитивном поиске Azure

Схемы Azure Cognitive Search явно не поддерживают многоуровневые категории таксономии, но их можно приблизительно обблизить, манипулируя контентом перед индексацией, а затем применяя к результатам некоторые специальные обработки. 

## <a name="start-with-the-data"></a>Начало работы с данными

Пример в этой статье основан на предыдущем примере, [Моделируйте базу данных AdventureWorks Inventory,](search-example-adventureworks-modeling.md)чтобы продемонстрировать многоуровневую граненость в Azure Cognitive Search.
...
```
  ...
  ![Результаты запросов](./media/search-example-adventureworks/prod-query-results.png "Результаты запроса")

## <a name="indexing-to-a-collection-field"></a>Индексирование поля Collection

В индексе, содержащем эту структуру, создайте поле **Collection (Edm.String)** в схеме когнитивного поиска Azure для хранения этих данных, убедившись, что атрибуты поля включают в себя поисковые, фильтруемые, facetable и извлекаемые.
...
Значение count должно быть достаточно большим для возвращения всех возможных значений таксономии. Данные AdventureWorks содержат 41 уникальное значение таксономии, поэтому `count:50` достаточно.
  ...
  ![Граненый фильтр](./media/search-example-adventureworks/facet-filter.png "Граненый фильтр")
...
Объект **categories** теперь может использоваться для отображения свертываемого дерева таксономии с точными количествами.
  ...
  ![многоуровневый граненый фильтр](./media/search-example-adventureworks/multi-level-facet.png "многоуровневый граненый фильтр")

 
Каждая ссылка в дереве должна применять соответствующий фильтр. Пример:

+ **таксономия/любой** `(x:x eq 'Accessories')` возвращает все документы в филиале Аксессуары
+ **таксономия /любая** `(x:x eq 'Accessories|Bike Racks')` возвращает только документы с подкатегорией велосипедных стойки под ветвью аксессуаров.
...
[Пример: Смоделируй базу данных AdventureWorks inventory для когнитивного поиска Azure](search-example-adventureworks-modeling.md)