---
title: Краткое руководство Java. Создания, загрузки и запроса индексов с помощью API REST службы поиска Azure — службы поиска Azure
description: Описание способов создания индекса, загрузка данных и выполнение запросов с помощью API REST поиска Azure и Java.
services: search
author: jj09
manager: jlembicz
ms.service: search
...
ms.locfileid: "67450104"
---
# <a name="quickstart-create-an-azure-search-index-in-java"></a>Краткое руководство. Создание индекса службы поиска Azure в Java
...
## <a name="find-the-service-name-and-api-key-of-your-azure-search-service"></a>Найти имя службы и `api-key` службы поиска Azure
Все вызовы REST API службы поиска Azure необходимо предоставить URL-адрес службы и `api-key`. 
...
## <a name="configure-the-service-url-and-api-key"></a>Настройка URL-адрес службы и `api-key`
1. В **обозревателя проектов**, дважды щелкните **config.properties** для изменения параметров конфигурации, в котором содержится имя сервера и `api-key`.
2. См. в шагах выше в этой статье, в которых вы нашли URL-адрес службы и `api-key` в [портала Azure](https://portal.azure.com), чтобы получить значения, вы теперь введете в **config.properties**.
3. В **config.properties**, замените «API Key» `api-key` для вашей службы. Затем имя службы (первый компонент URL-адреса https://servicename.search.windows.net) заменяет "service name" в том же файле.
