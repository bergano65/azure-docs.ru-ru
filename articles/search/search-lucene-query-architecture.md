...
В следующем примере показан поисковый запрос, который можно отправить в Azure Когнитивный поиск с помощью [REST API](https://docs.microsoft.com/rest/api/searchservice/search-documents).  
...
> [!Note]
> Поисковый запрос выполняется независимо от всех полей, поддерживающих поиск, в индексе Когнитивный поиск Azure, за исключением случаев, `searchFields` когда поля задаются с помощью параметра, как показано в примере поискового запроса. Возвращаются документы, содержащие совпадение в любом из выбранных полей. 
