---
title: Архитектура полнотекстового запроса и механизма индексирования (Lucene)
titleSuffix: Azure Cognitive Search
description: Изучает основные понятия обработки запросов Lucene и извлечения документов для полнотекстового поиска, связанные с Azure Когнитивный поиск.
manager: nitinme
author: yahnoosh
ms.author: jlembicz
...
Эта статья предназначена для разработчиков, которым требуется более глубокое понимание работы полнотекстового поиска Lucene в Azure Когнитивный поиск. Для текстовых запросов Когнитивный поиск Azure в большинстве сценариев быстро предоставляет ожидаемые результаты. Но иногда вы можете получить и неточные результаты. В таких ситуациях понимание четырех этапов выполнения запросов Lucene (синтаксический анализ запроса, лексический анализ, поиск соответствующего документа и оценка) помогут определить характерные изменения параметров запроса или конфигурации индекса, которые приведут к желаемому результату. 

> [!Note] 
> Azure Когнитивный поиск использует Lucene для полнотекстового поиска, но интеграция с Lucene не является исчерпывающей. Мы выборочно предоставляем и расширяем функциональность Lucene, чтобы обеспечить важность сценариев для Когнитивный поиск Azure. 
...
На схеме ниже представлены компоненты, используемые для обработки поискового запроса. 
 ...
 ![Схема архитектуры запросов Lucene в Azure Когнитивный поиск][1]
...
|**Index** | Эффективная структура данных, используемая для хранения и упорядочивания доступных для поиска терминов, извлеченных из индексированных документов. |
...
Полный список поддерживаемых типов запросов см. в руководстве по [синтаксису запросов Lucene](/rest/api/searchservice/lucene-query-syntax-in-azure-search).
...
2. *Токены* являются выходными данными лексического анализа.
...
> [!Note]
> Когнитивный поиск Azure позволяет указать различные анализаторы для индексирования и поиска с помощью `indexAnalyzer` дополнительных `searchAnalyzer` параметров и полей. Если их не указать, анализатор с заданным свойством `analyzer` будет использоваться для двух операций — индексирования и поиска.  
...
> [!Note]
> Поисковый запрос выполняется независимо от всех полей, поддерживающих поиск, в индексе Когнитивный поиск Azure, за исключением случаев, когда поля задаются с помощью `searchFields` параметра, как показано в примере поискового запроса. Возвращаются документы, содержащие совпадение в любом из выбранных полей. 
...
Существует два способа настройки оценки релевантности в Azure Когнитивный поиск:

1. **Профили оценки** повышают позицию документов в ранжированном списке результатов на основе набора правил. В нашем примере мы можем рассмотреть документы, для которых найдены совпадения в заголовке поля, как более релевантные, чем те, которые содержат совпадения в поле описания. Кроме того, если бы для индекса было задано поле цены для каждого отеля, мы могли бы повысить позиции документов с более низкой стоимостью. Дополнительные сведения см. в статье о [добавлении профилей оценки в поисковый индекс](/rest/api/searchservice/add-scoring-profiles-to-a-search-index).
2. **Повышение приоритета слов** (доступно только в синтаксисе расширенных запросов Lucene) достигается с помощью оператора `^`, который можно применить к любой части дерева запроса. В нашем примере вместо поиска по префиксу " *воздух-условие* \* " можно выполнить поиск либо точного термина *Air* , либо префикса, но документы, соответствующие определенному термину, имеют более высокий приоритет, применяя повышение в отношении термина "запрос": * Air-условие ^ 2 | | кондиционирование * *. Дополнительные сведения см. в разделе о [повышении приоритета слов](/rest/api/searchservice/lucene-query-syntax-in-azure-search#bkmk_termboost).


### <a name="scoring-in-a-distributed-index"></a>Оценка в распределенном индексе

Все индексы в Azure Когнитивный поиск автоматически разбиваются на несколько сегментов, что позволяет быстро распределять индекс между несколькими узлами во время увеличения или уменьшения масштаба. Когда инициирован поисковый запрос, он выполняется отдельно для каждого сегмента. Затем результаты из каждого сегмента объединяются и упорядочиваются по оценке (если не определен другой порядок). Важно знать, что функция оценки анализирует частоту термина запроса по отношению к обратной частоте документа во всех документах в пределах определенного сегмента, а не всех сегментов.
...
+ Попробуйте использовать другой синтаксис запросов на основе примеров в статьях о [поиске документов](/rest/api/searchservice/search-documents#bkmk_examples) или [простом синтаксисе запросов](/rest/api/searchservice/simple-query-syntax-in-azure-search) в проводнике поиска на портале.

+ Если вы хотите настроить ранжирование в поисковом приложении, ознакомьтесь со статьей о [добавлении профилей оценки в поисковый индекс](/rest/api/searchservice/add-scoring-profiles-to-a-search-index).

+ Узнайте, как применить [лексические анализаторы для конкретного языка](/rest/api/searchservice/language-support).
