---
title: Полный текст запроса и архитектуры двигателя индексирования (Lucene)
titleSuffix: Azure Cognitive Search
description: Изучит концепции обработки запросов Lucene и поиска документов для полного поиска текста, связанных с когнитивным поиском Azure.
manager: nitinme
author: yahnoosh
ms.author: jlembicz
...
---
# <a name="how-full-text-search-works-in-azure-cognitive-search"></a>How full text search works in Azure Cognitive Search (Как выполняется полнотекстовый поиск в Когнитивном поиске Azure)

Эта статья предназначена для разработчиков, которым необходимо глубже понять, как работает полный поиск текста Lucene в Azure Cognitive Search. Для текстовых запросов Когнитивный поиск Azure в большинстве сценариев быстро предоставляет ожидаемые результаты. Но иногда вы можете получить и неточные результаты. В таких ситуациях понимание четырех этапов выполнения запросов Lucene (синтаксический анализ запроса, лексический анализ, поиск соответствующего документа и оценка) помогут определить характерные изменения параметров запроса или конфигурации индекса, которые приведут к желаемому результату. 

> [!Note] 
> Azure Cognitive Search использует Lucene для полного поиска текста, но интеграция Lucene не является исчерпывающей. Мы избирательно разоблачаем и расширяем функциональность Lucene, чтобы сценарии, важные для Azure Cognitive Search. 
...
На схеме ниже представлены компоненты, используемые для обработки поискового запроса. 
 ...
 ![Диаграмма архитектуры Lucene в Azure Cognitive Search][1]
...
Ниже приводится запрос на поиск, который можно отправить в Azure Cognitive Search с помощью [REST API.](https://docs.microsoft.com/rest/api/searchservice/search-documents)  
...
> [!Note]
> Azure Cognitive Search позволяет указать различные анализаторы для индексации и поиска по дополнительным `indexAnalyzer` и `searchAnalyzer` полевым параметрам. Если их не указать, анализатор с заданным свойством `analyzer` будет использоваться для двух операций — индексирования и поиска.  
...
> [!Note]
> Поисковый запрос выполняется независимо от всех полей поиска в индексе когнитивного поиска `searchFields` Azure, если только вы не ограничиваете поля, установленные параметром, как показано в примере запроса поиска. Возвращаются документы, содержащие совпадение в любом из выбранных полей. 
...
Существует два способа настройки баллов релевантности в Azure Cognitive Search:
...
2. **Повышение приоритета слов** (доступно только в синтаксисе расширенных запросов Lucene) достигается с помощью оператора `^`, который можно применить к любой части дерева запроса. В нашем примере, вместо поиска на префиксе *кондиционирования*\*воздуха , можно было бы искать либо точный термин *кондиционирования воздуха* или префикс, но документы, которые соответствуют точному сроку ранжируются выше, применяя импульс к термину запрос: «кондиционер»2» кондиционирования воздуха. Дополнительные сведения см. в разделе о [повышении приоритета слов](https://docs.microsoft.com/rest/api/searchservice/lucene-query-syntax-in-azure-search#bkmk_termboost).


### <a name="scoring-in-a-distributed-index"></a>Оценка в распределенном индексе

Все индексы в Azure Cognitive Search автоматически делятся на несколько осколков, что позволяет нам быстро распределить индекс между несколькими узлами во время масштабирования или масштабирования службы. Когда инициирован поисковый запрос, он выполняется отдельно для каждого сегмента. Затем результаты из каждого сегмента объединяются и упорядочиваются по оценке (если не определен другой порядок). Важно знать, что функция оценки анализирует частоту термина запроса по отношению к обратной частоте документа во всех документах в пределах определенного сегмента, а не всех сегментов.
...
В этой статье был рассмотрен полный поиск текста в контексте когнитивного поиска Azure. Мы надеемся, вы достаточно хорошо разобрались с этим вопросом и сможете применить полученные знания для разрешения распространенных проблем запросов. 
