...
2. **Ограничьте число полей с высокой кратностью**. Большое поле кратности состоит из поля с аспектом или фильтруемым полем, которое имеет значительное количество уникальных значений, и в результате потребляет значительные ресурсы при вычислении результатов. Например, задание идентификатора продукта или поля описания в качестве аспекта или фильтрации будет считаться большим количеством элементов, так как большинство значений из документа в документ являются уникальными. По возможности ограничивайте число полей с высокой кратностью.

3. **Повысьте уровень поиска**.  Переход на более высокий уровень Поиска Azure также может ускорить обработку медленно выполняемых запросов. Каждый более высокий уровень обеспечивает более быстрые процессоры и больше памяти, что оказывает положительное воздействие на производительность запросов.
...
Служба поиска Azure предлагает возможность анализировать и отслеживать производительность службы с помощью [анализа трафика поиска](search-traffic-analytics.md). При включении этой функции и добавлении инструментирования в клиентское приложение можно дополнительно зарегистрировать отдельные операции поиска, а также сводные метрики в учетной записи хранения Azure, которые затем можно обработать для анализа или визуализации в Power BI. Метрики захватывают такой способ предоставления статистики производительности, например среднего количества запросов или времени ответа на запросы. Кроме того, ведение журнала операций позволяет детализировать сведения об операциях поиска.

Аналитика трафика полезна для понимания ставок задержки с точки зрения поиска Azure. Так как метрики производительности запросов, регистрируемые в журнале, основаны на времени полного выполнения запроса в Поиске Azure (с момента поступления запроса до его отправки обратно), вы можете использовать их, чтобы определить источник проблем, порождающих задержки. Это могут быть задержки из-за службы Поиска Azure или, например, задержки сети.  
