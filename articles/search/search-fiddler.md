---
title: Краткое руководство. Postman и интерфейсы REST API — служба "Поиск Azure"
...
2. Скопируйте этот URL-адрес: `https://<YOUR-SEARCH-SERVICE-NAME>.search.windows.net/indexes/hotel?api-version=2019-05-06`.
...
В качестве целевого объекта укажите измененную версию этого URL-адреса: `https://<YOUR-SEARCH-SERVICE-NAME>.search.windows.net/indexes?api-version=2019-05-06`.
...
Измените команду на **PUT**. Скопируйте измененную версию этого URL-адреса: `https://<YOUR-SEARCH-SERVICE-NAME>.search.windows.net/indexes/hotel?api-version=2019-05-06`. Скопируйте указанное выше определение индекса в текст запроса. Теперь страница должна выглядеть, как на снимке экрана ниже. Нажмите **Execute** (Выполнить) вверху справа, чтобы отправить созданный запрос.
...
Ниже приведен пример запроса из статьи о [REST API для поиска документов](https://docs.microsoft.com/rest/api/searchservice/Search-Documents). Многие примеры запросов в этой статье включают пробелы, что является недопустимым в Fiddler. Замените все пробелы символом "+" перед вставкой в строку запроса и его выполнением в Fiddler:
