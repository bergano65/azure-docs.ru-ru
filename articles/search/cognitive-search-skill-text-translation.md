...
|---------------------|-------------|
| дефаулттолангуажекоде | Необходимости Код языка для перевода документов в документы, не указывающие на язык явным образом. <br/> См. [полный список поддерживаемых языков](https://docs.microsoft.com/azure/cognitive-services/translator/language-support). |
