...
ms.locfileid: "56007542"
---
# <a name="add-custom-analyzers-to-an-azure-search-index"></a>Добавление пользовательских анализаторов в индексы Поиска Azure

*Пользовательский анализатор* — это пользовательская комбинация создателя маркеров и дополнительных фильтров, используемая для настройки обработки текста в поисковой системе. Например, можно создать пользовательский анализатор с *фильтром знаков*, чтобы удалить HTML-разметку, прежде чем размечать текстовые входные данные.
...
## <a name="overview"></a>Обзор
 ...
 В службе "Поиск Azure" вы можете выбрать из набора стандартных независящих от языка анализаторов в таблице [Анализаторы](#AnalyzerTable) и зависимых от языка анализаторов, приведенных в статье [Добавление анализатора языка в индекс Поиска Azure](index-add-language-analyzers.md). Кроме того, вы можете определять собственные пользовательские анализаторы.  
...
## <a name="test-a-custom-analyzer"></a>Тестирование пользовательского анализатора
...
~~~~
 ...
 ## <a name="update-a-custom-analyzer"></a>Обновление пользовательского анализатора
...
## <a name="index-attribute-reference"></a>Справочные данные по атрибутам индекса
...
|CharFilters|Задайте один из стандартных фильтров знаков, перечисленных в таблице [Фильтры знаков](#CharFilter), или пользовательский фильтр знаков, указанный в определении индекса.|  
...
|[whitespace](https://lucene.apache.org/core/4_10_3/analyzers-common/org/apache/lucene/analysis/core/WhitespaceTokenizer.html)|(тип применяется только при наличии параметров) |Разбивает текст по пробелам. Маркеры длиной более 255 знаков разделяются.|  
 ...
 <sup>1</sup> Типы создателей маркеров всегда начинаются в коде с префиксов "#Microsoft.Azure.Search". Например, "ClassicTokenizer" фактически будет указан как "#Microsoft.Azure.Search.ClassicTokenizer". Мы удалили префикс, чтобы уменьшить ширину таблицы, но не забудьте включить его в код. Обратите внимание, что параметр tokenizer_type предоставляется только для создателей маркеров, которые можно настроить. Если параметров нет, как в случае с создателем буквенных маркеров, связанный тип #Microsoft.Azure.Search будет отсутствовать.
...
> [!NOTE]
> Необходимо настроить пользовательский анализатор так, чтобы он не создавал маркеры длиной более 300 знаков. Индексация документов с маркерами большей длины завершается сбоем. Чтобы обрезать их или игнорировать, используйте параметр **TruncateTokenFilter** или **LengthTokenFilter** соответственно.


## <a name="see-also"></a>См. также  
 [Руководство по REST Службы поиска Azure](https://docs.microsoft.com/rest/api/searchservice/)   
 [Анализаторы для обработки текста в службе "Поиск Azure" > раздел "Примеры"](https://docs.microsoft.com/azure/search/search-analyzers#examples)    
