...
Определение анализатора является частью большого индекса. Дополнительные сведения см. в статье [Create Index (Azure Search Service REST API)](/rest/api/searchservice/create-index) (Создание индексов (REST API службы "Поиск Azure")).
...
Чтобы увидеть, как анализатор разбивает текст на маркеры, можно использовать **операцию тестирования анализатора** в [REST API](/rest/api/searchservice/test-analyzer).
...
|Варианты|Это должны быть допустимые параметры стандартного анализатора, указанные в таблице [Анализаторы](#AnalyzerTable), приведенной ниже.|  
...
|Варианты|Это должны быть допустимые параметры заданного типа [фильтра знаков](#char-filters-reference).|  
...
|Варианты|Это должны быть допустимые параметры данного типа создателя маркеров, перечисленные в таблице [Создатели маркеров](#Tokenizers), приведенной ниже.|  
...
|Варианты|Это должны быть [фильтры маркеров](#TokenFilters) заданного типа.|  
