...
В этом примере используются следующие средства и службы. 

+ [Создайте когнитивный поискную службу](search-create-service-portal.md) или [найдите существующую](https://ms.portal.azure.com/#blade/HubsExtension/BrowseResourceBlade/resourceType/Microsoft.Search%2FsearchServices). 

+ [Создайте Azure Key Vault ресурс](../key-vault/secrets/quick-create-portal.md#create-a-vault) или найдите существующий. И Key Vault, и Когнитивный поиск должны находиться в одной подписке. Для хранилища ключей должна быть включена **Защита от** **обратимого удаления** и очистки.

+ [Azure Active Directory](../active-directory/fundamentals/active-directory-whatis.md) зарегистрировать приложение и создать секретную строку, используемую приложением для проверки подлинности. Если у вас ее нет, [Настройте новый клиент](../active-directory/develop/quickstart-create-new-tenant.md).
...
1. После создания регистрации приложения скопируйте идентификатор приложения. Необходимо предоставить эту строку приложению. 
   ...
   :::image type="content" source="media/search-manage-encryption-keys/cmk-application-id.png" alt-text="Создать новый ключ хранилища ключей":::
...
1. Скопируйте секрет приложения. При пошаговом выполнении образца вставьте это значение в **appsettings.js** файла.
   ...
   :::image type="content" source="media/search-manage-encryption-keys/cmk-application-secret.png" alt-text="Создать новый ключ хранилища ключей":::
...
1. Выберите **политики доступа** слева и щелкните **+ Добавить политику доступа** .
   ...
   :::image type="content" source="media/search-manage-encryption-keys/cmk-add-access-policy.png" alt-text="Создать новый ключ хранилища ключей":::

1. Щелкните **выбрать субъект** и выберите приложение, зарегистрированное с помощью Active Directory. Его можно найти по имени.

   :::image type="content" source="media/search-manage-encryption-keys/cmk-access-policy-permissions.png" alt-text="Создать новый ключ хранилища ключей":::
...
Чтобы добавить управляемый клиентом ключ на карту индекса или синонима, используйте REST API или пакет SDK для создания объекта, определение которого включает `encryptionKey` .
...
Сведения о создании новой схемы синонимов с помощью REST API можно найти на странице [Создание сопоставления синонимов (REST API)](/rest/api/searchservice/create-synonym-map), где единственное отличие заключается в указании сведений о ключе шифрования как части определения сопоставления синонимов: 
