...
> [!Note]
> Шифрование с помощью управляемых клиентом ключей выполняется в предварительной версии и не предназначено для использования в рабочей среде. Эта функция предоставляется в [версии REST API 2019-05-06-Preview](search-api-preview.md). Вы также можете использовать пакет SDK для .NET версии 8,0-Preview.
>
> Эта функция недоступна для бесплатных служб. Необходимо использовать службу поиска по оплатам, созданную в или после 2019-01-01. В настоящее время портал не поддерживается.

По умолчанию служба "Поиск Azure" шифрует неактивных пользовательских данных с помощью [ключей, управляемых службой](https://docs.microsoft.com/azure/security/fundamentals/encryption-atrest#data-encryption-models). Шифрование по умолчанию можно расширить с помощью дополнительного уровня шифрования, используя ключи, которые вы создаете и управляете в Azure Key Vault. В этой статье описаны шаги.
...
+ [POST](search-get-started-postman.md), [Azure PowerShell](search-create-index-rest-api.md) и [пакет SDK службы поиска Azure](https://aka.ms/search-sdk-preview) можно использовать для вызова REST API предварительной версии. В настоящее время отсутствует поддержка портала или пакета SDK для .NET для шифрования, управляемого клиентом.
...
>[!Note]
> Из-за особенностей шифрования с помощью функции, управляемой клиентом, служба поиска Azure не сможет получить данные, если ваш ключ хранилища ключей Azure будет удален. Чтобы предотвратить потери данных, вызванные случайным удалением ключей Key Vault, настоятельно рекомендуется включить обратимое удаление и очистку для выбранного хранилища ключей. Дополнительные сведения см. в разделе [Azure Key Vault обратимого удаления](https://docs.microsoft.com/azure/key-vault/key-vault-ovw-soft-delete).   
...
Запишите идентификатор ключа — он состоит из **URI значения ключа**, **имени ключа**и **версии ключа**. Они понадобятся вам для определения зашифрованного индекса в службе поиска Azure.
...
Поиск Azure поддерживает два способа назначения удостоверения: управляемое удостоверение или внешнее управляемое приложение Azure Active Directory. 
...
1. Щелкните **выбрать субъект** и выберите службу поиска Azure. Можно выполнить поиск по имени или по ИДЕНТИФИКАТОРу объекта, который отображался после включения управляемого удостоверения.
...
1. Щелкните **ключевые разрешения** и выберите *получить*, расобернуть *ключ* и *Ключ переноса*. Вы можете использовать *Azure Data Lake Storage или шаблон службы хранилища Azure* , чтобы быстро выбрать необходимые разрешения.
   ...
   Служба "Поиск Azure" должна быть предоставлена со следующими разрешениями на [доступ](https://docs.microsoft.com/azure/key-vault/about-keys-secrets-and-certificates#key-operations):
...
> [!Important]
> Зашифрованное содержимое в службе поиска Azure настроено для использования определенного ключа Azure Key Vault с определенной **версией**. При изменении ключа или версии необходимо обновить карту индекса или синонима, чтобы использовать новый кэй\версион **перед** удалением предыдущего кэй\версион. Если этого не сделать, то вы не сможете расшифровать содержимое после того, как теряется доступ к ключу.   

## <a name="5---encrypt-content"></a>5\. шифрование содержимого

Создание индекса или таблицы синонимов, зашифрованных с помощью ключа, управляемого клиентом, пока не поддерживается с помощью портал Azure. Используйте REST API поиска Azure, чтобы создать такой индекс или карту синонимов.
...
>[!Important] 
> Хотя **encryptionKey** нельзя добавить к существующим индексам службы поиска Azure или сопоставлениям синонимов, его можно обновить, предоставив разные значения для любого из трех данных хранилища ключей (например, обновление версии ключа). При переходе на новый ключ Key Vault или новую версию ключа необходимо сначала обновить любой индекс поиска Azure или карту синонимов, которая использует этот ключ, чтобы использовать новый кэй\версион **перед** удалением предыдущего кэй\версион. Если не выполнить это действие, будет отображена схема индекса или синонима, которую нельзя использовать, так как она не сможет расшифровать содержимое после потери доступа к ключу.   
> Восстановление разрешений на доступ к хранилищу ключей в дальнейшем приведет к восстановлению доступа к содержимому.

## <a name="aad-app"></a>Продвинут Использование приложения, управляемого извне Azure Active Directory

Если управляемое удостоверение невозможно, можно создать приложение Azure Active Directory с субъектом безопасности для службы поиска Azure. В частности, управляемое удостоверение неприменимо в следующих условиях:
...
Для поддержки таких топологий служба поиска Azure поддерживает использование приложений Azure Active Directory (AAD) для проверки подлинности между службой поиска и Key Vault.    
...
1. [Получите идентификатор приложения и ключ проверки](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal#get-values-for-signing-in) подлинности, так как они потребуются для создания зашифрованного индекса. Значения, которые потребуется указать, включают **идентификатор приложения** и **ключ проверки**подлинности.

>[!Important]
> При принятии решения об использовании приложения AAD с проверкой подлинности вместо управляемого удостоверения следует учитывать тот факт, что служба поиска Azure не может управлять вашим приложением AAD от вашего имени и управлять приложением AAD, например периодическое вращение ключ проверки подлинности приложения.
> При изменении приложения AAD или его ключа проверки подлинности любой индекс поиска Azure или схема синонимов, использующих это приложение, сначала должны быть обновлены для использования нового приложения Ид\кэй **перед** удалением предыдущего приложения или его ключа авторизации. перед отменой доступа к Key Vault.
