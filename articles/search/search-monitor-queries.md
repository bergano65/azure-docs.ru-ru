...
description: Отслеживайте метрики запросов для производительности и пропускной способности. Сбор и анализ данных строки запроса в журналах диагностики.
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
ms.locfileid: "77211156"
---
# <a name="monitor-query-requests-in-azure-cognitive-search"></a>Наблюдение за запросами запросов в Azure Когнитивный поиск

В этой статье объясняется, как измерять производительность запросов и тома с помощью метрик. Здесь также объясняется, как собираются входные термины, используемые в запросах. необходимые сведения необходимо для оценки служебной программы и эффективности поиска совокупности.

Исторические данные, которые веб-каналы поступают в метрики, сохраняются в течение 30 дней. Для более длительного хранения данных или для отчетов по рабочим данным и строкам запросов обязательно включите [параметр диагностики](search-monitor-logs.md) , указывающий параметр хранилища.

Ниже перечислены условия, которые обеспечивают максимальную целостность измерений данных.
...
+ По возможности используйте одну реплику, чтобы вычисления были ограничены одним компьютером. При использовании нескольких реплик метрики запроса вычисляются в среднем на нескольких узлах, некоторые из которых могут быть быстрее. При настройке производительности запросов один узел предоставляет более стабильную среду для тестирования.
...
Том измеряется как количество **поисковых запросов в секунду** (QPS), встроенная метрика, которую можно сообщить как среднее значение, количество, минимальное или максимальное значение запросов, выполняемых в течение одного минуты. Интервал в пределах одной минуты (TimeGrain = "PT1M") для метрик фиксируется в системе.
...
| Число | Число метрик, переданных в журнал в течение интервала в одну минуту. |
| Максимальное значение | Максимальное число поисковых запросов в секунду, зарегистрированных в течение минуты. |
| Минимум | Наименьшее количество поисковых запросов в секунду, зарегистрированных в течение минуты.  |
...
| Число | Число метрик, переданных в журнал в течение интервала в одну минуту. |
| Максимальное значение | Наиболее длительный запрос в образце. | 
| Минимум | Кратчайший запуск запроса в примере.  | 
| Всего | Общее время выполнения всех запросов в примере, выполненных в течение интервала (одна минута).  |
...
| Число | Число метрик, переданных в журнал в течение интервала в одну минуту. |
| Максимальное значение | Процент запросов, отбрасываемых в пределах интервала.|
| Минимум | Процент запросов, отбрасываемых в пределах интервала. |
| Всего | Процент запросов, отбрасываемых в пределах интервала. |
...
Для более глубокого изучения откройте обозреватель метрик из меню **мониторинг** , чтобы можно было послойировать, увеличить и визуализировать данные для изучения тенденций и аномалий. Дополнительные сведения об обозревателе метрик см. в этом [руководстве по созданию диаграммы метрик](https://docs.microsoft.com/azure/azure-monitor/learn/tutorial-metrics-explorer).
...
## <a name="query-strings-used-in-queries"></a>Строки запросов, используемые в запросах

При включении ведения журнала диагностики система фиксирует запросы в таблице **AzureDiagnostics** . В качестве необходимого компонента необходимо уже включить [ведение журнала диагностики](search-monitor-logs.md), указав рабочую область log Analytics или другой вариант хранения.

1. В разделе Мониторинг выберите **журналы** , чтобы открыть пустое окно запроса в log Analytics.

1. Выполните следующее выражение для поиска по запросу. поисковые операции, возвращая набор табличных результатов, состоящий из имени операции, строки запроса, запрошенного индекса и числа найденных документов. Последние две инструкции исключают строки запроса, состоящие из пустого или неуказанного поиска, через пример индекса, который вырезает шум в результатах.
...
1. При необходимости задайте фильтр столбца на *Query_s* для поиска по определенному синтаксису или строке. Например, можно отфильтровать `?api-version=2019-05-06&search=*&%24filter=HotelName` *)* .
...
## <a name="report-query-data"></a>Данные запроса отчета

Power BI — это средство аналитических отчетов, которое можно использовать для данных журнала, хранящихся в хранилище BLOB-объектов или в Log Analytics рабочей области.
