...
description: Пользовательский интерфейс извлечения данных для пользовательских навыков Web-Api в конвейере обогащения ИИ в Azure Cognitive Search.
manager: nitinme
author: luiscabrer
ms.author: luisca
...
ms.locfileid: "77500269"
---
# <a name="how-to-add-a-custom-skill-to-an-azure-cognitive-search-enrichment-pipeline"></a>Как добавить пользовательский навык в конвейер обогащения когнитивного поиска Azure

Конвейер [обогащения](cognitive-search-concept-intro.md) в Azure Cognitive Search может быть собран из [встроенных когнитивных навыков,](cognitive-search-predefined-skills.md) а также [пользовательских навыков,](cognitive-search-custom-skill-web-api.md) которые вы лично создаете и добавляете в конвейер. В этой статье узнайте, как создать пользовательский навык, который предоставляет интерфейс, позволяющий его включать в конвейер обогащения ИИ. 

Создание пользовательского навыка позволяет вставлять преобразования, уникальные для содержимого. Пользовательский навык выполняется независимо, применяя любой нужный вам шаг обогащения. Например, вы можете определить пользовательские сущности конкретных полей, создать настраиваемые модели классификации для разделения деловых и финансовых контрактов и документов, а также добавить навык распознавания речи, чтобы углубиться в содержимое звуковых файлов. Для пошагового примера см. [Пример: Создание пользовательского навыка для обогащения ИИ.](cognitive-search-create-custom-skill-example.md)
...
Если ответ от настраиваемых конечных точек навыка API Поиска в Интернете не будет получен в течение 30-секундного окна, то по умолчанию они истекают. Конвейер индексирования является синхронным и индексация будет вызывать ошибку времени ожидания, если ответ не будет получен в этом окне времени.  Можно настроить тайм-аут до 230 секунд, установив параметр тайм-аута:
...
Убедитесь, что URI является безопасным (HTTPS).
...
### <a name="1--web-api-input-format"></a>1. Формат ввода Web API
...
### <a name="2-web-api-output-format"></a>2. Формат вывода Web API
...
При создании обогатителя веб-API можно в составе запроса описать HTTP-заголовки и параметры. На фрагменте ниже показано, как параметры запроса и *дополнительные* заголовки HTTP могут быть описаны как часть определения набора навыков. Заголовки HTTP не являются обязательным требованием, но они позволяют добавлять дополнительные возможности конфигурации в свой навык и установить их из определения набора навыков.
...
Эта статья охватывает требования к интерфейсу, необходимые для интеграции пользовательского навыка в набор навыков. Нажмите на следующие ссылки, чтобы узнать больше об пользовательских навыков и skillset состава.
