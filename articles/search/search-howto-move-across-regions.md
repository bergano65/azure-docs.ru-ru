...
Иногда клиенты запросили перемещение существующей службы поиска в другой регион. В настоящее время отсутствуют встроенные механизмы или средства, помогающие выполнить эту задачу. Он остается ручным процессом, описанным ниже в этой статье.
...
## <a name="steps-for-moving-a-service"></a>Действия по перемещению службы

Если необходимо переместить службу поиска в другой регион, то ваш подход должен выглядеть примерно так, как описано ниже.

1. Определение связанных служб, чтобы понять, как все это повлияет на перемещение службы. Вы можете использовать службу хранилища Azure для ведения журнала, хранилища знаний или внешнего источника данных. Возможно, вы используете Cognitive Services для обогащения искусственного интеллекта. Доступ к службам в других регионах является распространенным, но поставляется с дополнительными платами за пропускную способность. Cognitive Services и Azure Когнитивный поиск должны находиться в одном регионе, если используется обогащение искусственного интеллекта.

1. Проведите инвентаризацию существующей службы, чтобы получить полный список объектов службы. Если вы включили ведение журнала, создайте и заархивируйте все отчеты, которые могут потребоваться для записи журнала.

1. Проверьте цены и доступность в новом регионе, чтобы обеспечить доступность Azure Когнитивный поиск и всех связанных служб, которые можно создать в одном регионе. Проверьте четность компонентов. Некоторые функции предварительной версии имеют ограниченный доступ.

1. Создание службы в новом регионе и повторная публикация из исходного кода всех существующих индексов, индексаторов, источников данных, навыков, хранилищ знаний и сопоставлений синонимов. Имена служб должны быть уникальными, поэтому нельзя повторно использовать существующее имя.
