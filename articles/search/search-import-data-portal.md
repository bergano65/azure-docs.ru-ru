...
Перед написанием кода можно использовать мастер для тестирования с созданием прототипов и проверки концепции. Мастер подключается к внешним источникам данных, выбирает данные для создания исходного индекса, а затем импортирует данные как документы JSON в индекс в службе поиска Azure. 
...
Мастер **импорта данных** подключается к внешнему источнику данных с помощью внутренней логики, предоставляемой индексаторами службы поиска Azure, которые поддерживают выбор источника, чтение метаданных, взлом документов для чтения содержимого и структуры, а также сериализацию содержимого в формате JSON для последующий импорт в службу поиска Azure.
...
|  Выделение | Описание |
| ---------- | ----------- |
| **Существующий источник данных** |Если у вас уже есть индексаторы, определенные в службе поиска, возможно, имеется определение существующего источника данных, которое можно использовать повторно. В службе поиска Azure объекты источника данных используются только в индексаторах. Объект источника данных можно создать программным способом или с помощью мастера **импорта данных** и использовать их по мере необходимости.|
| **Примеры**| Служба поиска Azure предоставляет два встроенных образца источников данных, которые используются в руководствах и кратких руководствах: база данных SQL с реальным пространством и база данных гостиниц, размещенная на Cosmos DB. Пошаговые инструкции по примерам гостиниц см. в разделе [Создание индекса в портал Azure](search-get-started-portal.md) кратком руководстве. |
...
Вы также можете запустить **Импорт данных** из других служб Azure, включая Azure Cosmos DB, базу данных SQL Azure и хранилище BLOB-объектов Azure. На панели навигации слева на странице обзора службы выберите **Добавить Поиск Azure**.
...
1. Подходит ли тип данных для входящих данных? Служба поиска Azure поддерживает [типы данных модели EDM](https://docs.microsoft.com/rest/api/searchservice/supported-data-types). Для данных SQL Azure существует [Диаграмма сопоставления](search-howto-connecting-azure-sql-database-to-azure-search-using-indexers.md#mapping-between-sql-and-azure-search-data-types) , в которой используются эквивалентные значения. Дополнительные сведения см. в разделе [сопоставления полей и преобразования](search-indexer-field-mappings.md).
...
> [Создание индекса службы поиска Azure с помощью портал Azure](search-get-started-portal.md)