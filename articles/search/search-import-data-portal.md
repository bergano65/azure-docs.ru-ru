...
Новичок в Поиске Azure? Воспользуйтесь [кратким руководством по Импорт, индексирования и запросов с помощью средств портала](search-get-started-portal.md) тестирования импорта и индексирования с помощью **импорта данных** и встроенные недвижимости примере набора данных.
...
Вы можно импортировать только из одной таблицы, представления базы данных или эквивалентной структуры данных, однако структура может включать иерархические или вложенные подструктуры. Дополнительные сведения см. в разделе [моделирование сложных типов](search-howto-complex-data-types.md).

Эта структура данных необходимо создать перед запуском мастера, и он должен содержать содержимое. Не запускайте **импорта данных** мастер на пустой источник данных.
...
| **Примеры**| Поиск Azure размещает свободного глобальной базы данных SQL Azure, можно использовать, чтобы узнать об импорте и запросов запросы в службе поиска Azure. См. [Краткое руководство. импорту, индексированию и отправке запросов с помощью средств портала](search-get-started-portal.md). |
...
| **База данных Cosmos** |Требования включают учетную запись, базу данных и коллекцию. Все документы в коллекции будут включены в индекс. Можно определить запрос для одноуровневые или фильтрации набора строк, или оставить запрос пустым. Запрос не требуется в этом мастере.|
