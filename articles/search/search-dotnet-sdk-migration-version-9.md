...
Версия 9 пакета SDK .NET для поиска Azure предназначена для версии 2019-05-06 REST API службы поиска Azure со следующими функциями:
...
`Field`Теперь класс был изменен, так что он также может представлять сложные поля.

Следующие свойства теперь допускают `bool` значение NULL:
...
Это обусловлено тем, что эти свойства теперь должны быть `null` в случае сложных полей. Если у вас есть код, считывающий эти свойства, необходимо подготовиться к обработке `null` . Обратите внимание, что все остальные свойства класса `Field` всегда были и продолжают допускать значения NULL, а некоторые из них также будут иметь значение `null` в случае сложных полей, в частности:
...
Конструктор без параметров для `Field` был выполнен `internal` . С этого момента `Field` во время создания каждой из них требуется явное имя и тип данных.
...
Базовый класс `ExtensibleEnum` был удален. Все классы, производные от него, теперь являются структурами, например `AnalyzerName` ,, `DataType` и `DataSourceType` . Их `Create` методы также были удалены. Можно просто удалить вызовы, `Create` так как эти типы могут быть неявно преобразованы из строк. Если это приводит к ошибкам компилятора, можно явно вызвать оператор преобразования с помощью приведения для неоднозначности типов. Например, можно изменить код следующим образом:
...
Если вы выполняете обновление с версии 8,0-Preview до версии 9, имейте в виду, что шифрование с помощью ключей, управляемых клиентом, было удалено, так как эта функция все еще доступна в предварительной версии. В частности, `EncryptionKey` `Index` `SynonymMap` были удалены свойства и.
...
Если вы используете "динамически типизированные" `Search` , `Suggest` или `Get` API, которые возвращают экземпляры типа `Document` , обратите внимание, что теперь они десериализует пустые массивы JSON в `object[]` вместо `string[]` .
