---
title: Обновление до пакета SDK .NET для поиска Azure версии 9 — Поиск Azure
...
ms.locfileid: "70183243"
---
# <a name="upgrade-to-the-azure-search-net-sdk-version-9"></a>Обновление до пакета SDK .NET для поиска Azure версии 9
...
* Функция " [Поиск](cognitive-search-concept-intro.md) со знаком" представляет собой функцию искусственного интеллекта в службе поиска Azure, используемую для извлечения текста из изображений, больших двоичных объектов и других неструктурированных источников данных. Это позволяет расширить возможности поиска в индексе поиска Azure.
...
Могут появиться дополнительные предупреждения о сборке, связанные с устаревшими методами или свойствами. В предупреждениях будет указано, что следует использовать вместо устаревшей функции. Например, если приложение использует `DataSourceType.DocumentDb` свойство, следует получить предупреждение о том, что этот член является устаревшим. Вместо этого используйте CosmosDb.
...
Теперь `Field` класс был изменен, так что он также может представлять сложные поля.
...
Это обусловлено тем, что эти свойства `null` теперь должны быть в случае сложных полей. Если у вас есть код, считывающий эти свойства, необходимо подготовиться к обработке `null`. Обратите внимание, что все `Field` остальные свойства класса всегда были и продолжают допускать значения NULL, а некоторые из `null` них также будут иметь значение в случае сложных полей, в частности:
...
Конструктор без параметров для `Field` был выполнен. `internal` С этого момента во время `Field` создания каждой из них требуется явное имя и тип данных.
...
`ExtensibleEnum` Базовый класс был удален. Все классы, производные от него, теперь являются структурами, `AnalyzerName`например `DataType`,, `DataSourceType` и. Их `Create` методы также были удалены. Можно просто удалить вызовы, `Create` так как эти типы могут быть неявно преобразованы из строк. Если это приводит к ошибкам компилятора, можно явно вызвать оператор преобразования с помощью приведения для неоднозначности типов. Например, можно изменить код следующим образом:
...
`AutocompleteMode` Свойство объекта`AutocompleteParameters` больше не может иметь значение null. Если у вас есть код, который присваивает этому свойству `null`значение, можно просто удалить его, и свойство будет автоматически инициализировано со значением по умолчанию.

Порядок параметров в `IndexAction` конструкторе был изменен, теперь этот конструктор создан автоматически. Вместо использования конструктора рекомендуется использовать заводские методы `IndexAction.Upload`, `IndexAction.Merge`и т. д.

### <a name="removed-preview-features"></a>Удалены функции предварительной версии

Если вы выполняете обновление с версии 8,0-Preview до версии 9, имейте в виду, что шифрование с помощью ключей, управляемых клиентом, было удалено, так как эта функция все еще доступна в предварительной версии. В `EncryptionKey` частности, были удалены `Index` свойства `SynonymMap` и.
...
> [!NOTE]
> Если вы создали зашифрованные индексы или сопоставления синонимов с помощью версии 8,0-Preview пакета SDK, вы по-прежнему сможете использовать их и изменять их определения с помощью версии 9 пакета SDK, не влияя на состояние шифрования. Версия 9 пакета SDK не будет отсылать `encryptionKey` свойство в REST API, и в результате REST API не изменит состояние шифрования ресурса. 

### <a name="behavioral-change-in-data-retrieval"></a>Изменение поведения при извлечении данных

Если вы `Search`используете "динамически типизированные", `Suggest`или `Get` API, которые возвращают экземпляры типа `Document`, `string[]`Обратите внимание, что теперь они десериализует пустые массивы JSON в `object[]` вместо.
