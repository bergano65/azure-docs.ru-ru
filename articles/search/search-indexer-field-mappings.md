...
Сопоставления полей определяются в пакете SDK для .NET с помощью класса [FieldMapping](/dotnet/api/microsoft.azure.search.models.fieldmapping) , содержащего свойства `SourceFieldName` и `TargetFieldName` , и необязательной `MappingFunction` ссылки.
...
В библиотеках .NET в Azure Когнитивный поиск предполагается полная .NET Framework, которая обеспечивает встроенную кодировку. `useHttpServerUtilityUrlTokenEncode`Параметры и `useHttpServerUtilityUrlTokenDecode` используют эту встроенную функциональность. Если вы используете .NET Core или другую платформу, мы рекомендуем настроить эти параметры для `false` непосредственного вызова функций кодирования и декодирования вашей платформы.
