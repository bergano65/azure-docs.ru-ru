...
|----------------|-------------|
| Шифрование при передаче <br>(HTTPS/TLS) | Когнитивный поиск Azure прослушивает HTTPS порт 443. Подключения к службам Azure по всей платформе зашифрованы. <br/><br/>Все взаимодействия между клиентом и службой Azure Когнитивный поиск используют шифрование TLS 1,2. Более ранние версии (1,0 или 1,1) не поддерживаются.|
...
| Шифрование при хранении <br>Ключами управляет пользователь | Шифрование с помощью управляемых клиентом ключей теперь общедоступно для служб поиска, созданных до 2019 января. Она не поддерживается в бесплатных (общих) службах.<br><br>Индексы Когнитивный поиск Azure и сопоставления синонимов теперь можно зашифровать неактивных с помощью управляемых клиентом ключей в Azure Key Vault. Дополнительные сведения см. в статье [Управление ключами шифрования в Azure когнитивный Поиск](search-security-manage-encryption-keys.md).<br><br>Эта функция не заменяет шифрование неактивных по умолчанию, а применяется в дополнение к нему.<br><br>Включение этой функции увеличит размер индекса и снизит производительность запросов. Исходя из наблюдений на дату, можно увидеть увеличение 30%-60% времени выполнения запроса, хотя фактическая производительность зависит от определения индекса и типов запросов. В связи с этим влиянием на производительность рекомендуется включить эту функцию только для индексов, которые действительно необходимы.
...
При наличии общедоступной службы и необходимости ограничения использования службы можно использовать правило ограничения IP-адресов в REST API версии управления: 2020-03-13, [ипруле](https://docs.microsoft.com/rest/api/searchmanagement/2019-10-01-preview/createorupdate-service#IpRule). Ипруле позволяет ограничить доступ к службе, определив IP-адреса, по отдельности или в пределах диапазона, которым вы хотите предоставить доступ к службе поиска. 

### <a name="private-access"></a>Частный доступ

[Частные конечные точки](https://docs.microsoft.com/azure/private-link/private-endpoint-overview) для Azure когнитивный Поиск позволяют клиенту в виртуальной сети безопасно получать доступ к данным в индексе поиска по [закрытой ссылке](https://docs.microsoft.com/azure/private-link/private-link-overview). Частная конечная точка использует IP-адрес из адресного пространства виртуальной сети для службы поиска. Сетевой трафик между клиентом и службой поиска проходит по виртуальной сети и частной связи в магистральной сети Майкрософт, что устраняет уязвимость в общедоступном Интернете.

[Виртуальная сеть Azure (VNet)](https://docs.microsoft.com/azure/virtual-network/virtual-networks-overview) обеспечивает безопасное взаимодействие между ресурсами с локальной сетью и Интернетом. 
...
| Создание, изменение, удаление объектов в службе: <br>индексы и элементы компонентов (включая определения анализатора, профили оценки, параметры CORS), индексаторы, источники данных, синонимы, средства подбора. | Ключ администратора, владелец RBAC или участник для ресурса  |
