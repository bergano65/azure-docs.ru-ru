---
title: Создание индекса службы поиска Azure в коде с помощью API .NET
description: Узнайте, как создать индекс для полнотекстового поиска с помощью пакета SDK для .NET для службы "Поиск Azure" и примера кода C#.
author: heidisteen
manager: cgronlun
ms.author: heidist
tags: azure-portal
...
Конечная точка URL-адреса и ключ API администратора вашей службы поиска. Служба поиска создана с обоими элементами, поэтому если вы добавили службу "Поиск Azure" в подписку, выполните следующие действия для получения необходимых сведений:

  1. На портале Azure на странице **обзора** службы поиска найдите URL-адрес. Пример конечной точки может выглядеть так: `https://mydemo.search.windows.net`.

  2. В разделе **Параметры** > **Ключи** получите ключ администратора, чтобы обрести полные права на службу. Существует два взаимозаменяемых ключа администратора, предназначенных для обеспечения непрерывности бизнеса-процессов на случай, если вам потребуется сменить один из них. Вы можете использовать первичный или вторичный ключ для выполнения запросов на добавление, изменение и удаление объектов.

  ![Получение конечной точки HTTP и ключа доступа](media/search-fiddler/get-url-key.png "Get an HTTP endpoint and access key")
...
В рамках этого краткого руководства вы создали пустой индекс службы "Поиск Azure" на основе схемы, определяющей типы данных и поведение полей. В следующем кратком руководстве этой серии рассматривается загрузка индекса с доступным для поиска содержимым.
