...
Этот прием не подходит в ситуациях, когда нужно выполнить поиск с использованием идентификатора и описания расположения. Пример.
...
В этом случае определите в индексе другое поле, объединяющее все данные в одну коллекцию.  В нашем примере это поле будет называться `locationsCombined`. Содержимое мы разделим с помощью значка `||`, хотя можно выбрать любой разделитель, который, по вашему мнению, будет уникальным набором символов для содержимого. Пример. 
...
Этот метод полезен в ряде сценариев, но не подходит в каждом случае.  Пример.
