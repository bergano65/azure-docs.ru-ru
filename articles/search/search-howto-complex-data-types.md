...
> [!Note]
> Поддержка сложных типов обычно доступна в `api-version=2019-05-06`. 
>
...
Все [правила переиндексации](search-howto-reindex.md) , применяемые к полям в целом, по-прежнему применяются к сложным полям. Повторное описание некоторых основных правил для добавления поля не требует перестроения индекса, но большинство изменений выполняется.
...
Параметр `$select` используется для выбора полей, возвращаемых в результатах поиска. Чтобы использовать этот параметр для выбора конкретных подполей сложного поля, включите родительское поле и дочернее поле, разделенные косой чертой (`/`).

    $select=HotelName, Address/City, Rooms/BaseRate

Если вы хотите, чтобы они были доступны в результатах поиска, поля должны быть помечены в индексе как доступные для получения. В инструкции `$select` можно использовать только поля, помеченные как доступные для получения.
...
Операции сортировки работают, когда поля содержат одно значение для каждого документа, является ли поле простым полем или вложенным полем сложного типа. Например, `Address/City` разрешено сортировать, так как имеется только один адрес в отеле, поэтому `$orderby=Address/City` будет сортировать Гостиницы по городу.
