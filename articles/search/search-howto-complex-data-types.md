...
Как и в случае с простыми полями верхнего уровня, простые вложенные поля сложных полей можно включать в фильтры только в том случае, если для `true` атрибута **FILTERED** задано значение в определении индекса. Дополнительные сведения см. в [справочнике по API создания индекса](/rest/api/searchservice/create-index).
