...
|`Weights`|необязательный параметр. Пара "имя-значение", указывающая имя поля и относительный вес. Относительный вес должен быть положительным целым числом или числом с плавающей запятой. Максимальное значение — int32.MaxValue.<br /><br /> Можно указать имя поля без соответствующего веса. Весовые коэффициенты используются для указания важности одного поля относительно другого.|  
