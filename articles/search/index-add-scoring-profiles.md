...
|`freshness` &#124; `boostingDuration`|Задает период истечения срока действия, после которого повышение для определенного документа будет остановлено. Синтаксис и примеры приведены далее в подразделе [Настройка boostingDuration](#bkmk_boostdur).|  
