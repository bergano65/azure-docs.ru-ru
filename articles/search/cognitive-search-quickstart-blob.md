---
title: Создание конвейера когнитивного поиска для индексирования под управлением искусственного интеллекта на портале Azure (Поиск Azure)
...
---
# <a name="quickstart-create-a-cognitive-search-pipeline-using-skills-and-sample-data"></a>Краткое руководство. Создание конвейера когнитивного поиска с использованием навыков и примера данных

Функция "Когнитивный поиск" (предварительная версия) добавляет навыки извлечения данных, обработки естественного языка и изображений в конвейер индексирования службы "Поиск Azure", упрощая поиск содержимого, по которому не выполняется обычный поиск или которое не структурировано. 

Конвейер когнитивного поиска интегрирует [ресурсы Cognitive Services](https://azure.microsoft.com/services/cognitive-services/), такие как [OCR](cognitive-search-skill-ocr.md), [распознавание языка](cognitive-search-skill-language-detection.md) или [распознавание сущностей](cognitive-search-skill-entity-recognition.md) в единый процесс индексирования. Алгоритмы искусственного интеллекта Cognitive Services используются для поиска шаблонов, компонентов и характеристики в исходных данных и возвращают структуры и текстовое содержимое, которые пригодны для применения в решениях полнотекстового поиска на базе Поиска Azure.
...
> * Примените [обозреватель поиска](search-explorer.md) для запроса по обогащенным данным.
...
>
> За операции с применением встроенных навыков взимается [плата по мере использования по существующим тарифам для служб Cognitive Services](https://azure.microsoft.com/pricing/details/cognitive-services/). За извлечение изображений взимается плата по тарифам для предварительной версии, как описано на странице [цен на службу "Поиск Azure"](https://go.microsoft.com/fwlink/?linkid=2042400). [Подробнее](cognitive-search-attach-cognitive-services.md).
...
+ [Хранилище BLOB-объектов Azure](https://azure.microsoft.com/services/storage/blobs/) предоставляет исходные данные.
+ [Cognitive Services](https://azure.microsoft.com/services/cognitive-services/) предоставляет функции искусственного интеллекта (эти ресурсы можно создать прямо в строке описания конвейера).
+ [Поиск Azure](https://azure.microsoft.com/services/search/) предоставляет конвейер обогащенного индексирования и многофункциональную систему поиска произвольного текста, которые можно использовать в пользовательских приложениях.
...
1. Создайте новую группу ресурсов, которая будет содержать все созданные в этом кратком руководстве ресурсы. Это упрощает очистку ресурсов после завершения работы с руководством.

1. В качестве расположения выберите один из [регионов](#supported-regions), в которых поддерживается функция когнитивного поиска.
...
1. В разделе ценовой категории можно выбрать **бесплатный** уровень, чтобы выполнить инструкции в руководствах. Для более глубокого анализа с использованием собственных данных создайте [платную службу](https://azure.microsoft.com/pricing/details/search/), например с уровнем **Базовый** или **Стандартный**. 
  ...
  ![Страница определения службы на портале](./media/cognitive-search-tutorial-blob/create-search-service2.png "Страница определения службы на портале")

  > [!NOTE]
  > Служба когнитивного поиска находится в общедоступной предварительной версии. Выполнение набора навыков в настоящее время доступно на всех уровнях, включая бесплатный. Вы можете выполнить ограниченное число обогащений без связывания платного ресурса Cognitive Services. [Подробнее](cognitive-search-attach-cognitive-services.md).
...
Конвейер обогащения извлекает из Azure источники данных, поддерживаемые [индексаторами службы "Поиски Azure"](search-indexer-overview.md). Обратите внимание на то, что в Хранилище таблиц Azure когнитивный поиск не поддерживается. В этом упражнении используется хранилище BLOB-объектов, чтобы продемонстрировать несколько типов содержимого.
...
1. Зарегистрируйтесь в хранилище BLOB-объектов Azure, создайте учетную запись хранения, войдите в обозреватель хранилища и создайте контейнер. Для уровня доступа установите значение **Контейнер**. Дополнительные сведения см. в разделе [Создание контейнера](../storage/blobs/storage-unstructured-search.md#create-a-container) в учебнике по поиску неструктурированных данных.

1. В созданном контейнере щелкните **Отправить**, чтобы передать примеры файлов.
...
+ Имя по умолчанию *azureblob-index*.
+ Ключ по умолчанию *metadata_storage_path* (это поле содержит уникальные значения).
+ Типы данных и атрибуты по умолчанию пригодны для сценариев полнотекстового поиска.

Возможно, стоит удалить атрибута **Retrievable** (Доступность) из поля `content`. Для больших двоичных объектах это поле может содержать несколько тысяч строк. Только представьте, насколько это осложнит просмотр файлов с объемным содержимым, например документов Word или PowerPoint в списке результатов поиска в формате JSON. 

Так как вы определили набор навыков, мастер предполагает, что вам потребуется не только исходное поле данных, но и поля созданных навыками выходных данных. По этой причине портал добавляет поля индекса для `content`, `people`, `organizations` и `locations`. Обратите внимание, что мастер автоматически включает для этих полей атрибуты **Retrievable** (Доступность для чтения) и **Searchable** (Доступность для поиска). **Доступность для поиска** означает, что по этому полю можно выполнять поиск. **Доступность для получения** означает, что его можно возвращать в результатах. 
