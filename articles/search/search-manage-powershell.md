...
Примеры в этой статье являются интерактивными и нуждаются в повышенных правах. Необходимо установить Azure PowerShell (модуль **AZ** ). Дополнительные сведения см. в разделе [Install Azure PowerShell](/powershell/azure/overview).
...
Его можно создать только по одному, указанному в качестве ключа `primary` или. `secondary` При использовании непрерывной службы не забудьте обновить весь код клиента для использования вторичного ключа при последовательном переходе по первичному ключу. Старайтесь не менять ключи, пока операции находятся в полете.
