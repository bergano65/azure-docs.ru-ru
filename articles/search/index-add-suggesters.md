---
title: Добавление запросов typeahead в индекс
...
ms.locfileid: "72790105"
---
# <a name="add-suggesters-to-an-index-for-typeahead-in-azure-cognitive-search"></a>Добавление предложить в индекс для typeahead в Azure Когнитивный поиск

В Azure Cognitive Search функциональность "поиск по типу" или typeahead основана на конструкции **отсылки,** которую вы добавляете к [индексу поиска.](search-what-is-an-index.md) Это список одного или нескольких полей, для которых вы хотите typeahead включен.

Suggester поддерживает два варианта typeahead: *автозаполнение,* которое завершает термин или фразу, которую вы вводите, и *предложения,* которые возвращают короткий список соответствующих документов.  

Следующий скриншот, взятый из первого приложения в образце [Си-Ква,](tutorial-csharp-type-ahead-and-suggestions.md) иллюстрирует typeahead. Автозаполнение предвидит, что пользователь может ввести в поле поиска. Фактическим ввозам является "tw", который автозаполнение заканчивается "в", решая как "двойной" в качестве перспективного термина поиска. Предложения визуализированы в списке выпадающих. Для предложений можно вывести любую часть документа, которая лучше всего описывает результат. В этом примере предложения — это названия отелей. 
...
### <a name="analyzer-restrictions-for-sourcefields-in-a-suggester"></a>Ограничения анализатора для sourceFields в предложить

Azure Cognitive Search анализирует содержимое поля, чтобы включить запрос на отдельных терминах. Предложения требуют индексации префиксов в дополнение к полным срокам, что требует дополнительного анализа исходных полей. Пользовательские конфигурации анализатора могут комбинировать любой из различных токенизаторов и фильтров, часто таким образом, что делает производство префиксов, необходимых для предложений невозможно. По этой причине Azure Cognitive Search предотвращает включение полей с пользовательскими анализаторами в подсказчик.

> [!NOTE] 
>  Если вам нужно обойти вышеуказанное ограничение, используйте два отдельных поля для одного и того же содержимого. Это позволит одному из полей иметь предложить, в то время как другое может быть настроено с пользовательской конфигурацией анализатора.
