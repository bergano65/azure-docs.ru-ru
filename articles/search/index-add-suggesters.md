...
Объект **средство подбора** представляет собой конструкцию в [индекс службы поиска Azure](search-what-is-an-index.md) , поддерживающий взаимодействие «поиска как то type». Он содержит список полей, для которых вы хотите включить входных данных typeahead query. Существует два возможных typeahead: [ *автозаполнения* ](search-autocomplete-tutorial.md) завершения термина или фразы, при вводе, [ *автозаполнения* ](search-autosuggest-example.md) предоставляет short список терминов или фраз, которые можно выбрать в качестве входных данных запроса. Несомненно, вы узнали эти поведения перед в коммерческих поисковых системах.

![Сравнение Visual функции автозаполнения и автозаполнения](./media/index-add-suggesters/visual-comparison-suggest-complete.png "сравнение Visual функции автозаполнения и автозаполнения")

Для реализации этого поведения в службе поиска Azure, имеется компонент индексов и запросов. 

+ В индексе добавите средство подбора. Можно использовать на портале, REST API или пакет SDK для .NET для создания средств подбора. 

+ В запросе укажите действие автозаполнения или автозаполнения. 
...
### <a name="index-rebuilds-for-existing-fields"></a>Перестроение индекса для существующих полей

Средства подбора содержат поля, и при добавлении средств подбора в существующий индекс или изменив его поля композицию, скорее всего нужно перестроить индекс.

| Действие | Влияние |
|--------|--------|
| Создать новые поля и создавать новые средства подбора смысловых вариантов одновременно, в то же обновление | Наименьшее влияние. Если индекс содержит ранее добавленные поля, добавлять новые поля и новые средства подбора смысловых вариантов не оказывает влияния на существующих полей. |
| Добавить существующие поля в индекс | Высокая степень влияния. Добавление поля изменяет определение поля, создавая [полное перестроение](search-howto-reindex.md).|

## <a name="use-a-suggester"></a>Используйте средство подбора

Как было отмечено выше можно использовать средство подбора для автозаполнения и автозаполнения. 
...
Примеры, демонстрирующие применение каждого запроса:

+ [Добавление автозаполнения для выбранных элементов для запроса раскрывающийся список](search-autosuggest-example.md)

+ [Добавить autocomplete к входным данным частичного термин в службе поиска Azure](search-autocomplete-tutorial.md) (в предварительной версии) 

## <a name="sample-code"></a>Пример кода

[DotNetHowToAutocomplete](https://github.com/Azure-Samples/search-dotnet-getting-started/tree/master/DotNetHowToAutocomplete) образец содержит оба C# и кода Java и демонстрирует это средство подбора конструкция автозаполнения, автозаполнения и применяемый для фасетной навигации. 
...
Ознакомьтесь с приведенным ниже, чтобы увидеть, как организуются запросы:

+ [Пример autosuggested запроса](search-autosuggest-example.md) 
