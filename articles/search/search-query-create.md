...
ms.locfileid: "97387434"
---
# <a name="create-a-query-in-azure-cognitive-search"></a>Создание запроса в Azure Когнитивный поиск

Сведения о средствах и API для создания запроса, методов, используемых для создания запроса, а также о том, как структура и содержимое индекса может повлиять на результаты запросов. Общие сведения о том, как выглядит запрос запроса, начинаются с [типов запросов и композиций](search-query-overview.md).

## <a name="choose-tools-and-apis"></a>Выбор средств и API

Для создания запросов на тестирование или рабочую нагрузку можно использовать любой из следующих средств и API-интерфейсов.

| Методика | Описание |
|-------------|-------------|
| Портал| [Обозреватель поиска (портал)](search-explorer.md) — это интерфейс запросов в портал Azure, который можно использовать для выполнения запросов к индексам в базовой службе поиска. Портал делает REST APIные вызовы в фоновом режиме. Можно выбрать любой индекс и поддерживаемую версию REST API, включая предварительные версии. Строка запроса может иметь простой и полный синтаксис, а также может включать критерии фильтра, аспекты, инструкции SELECT и Сеарчфиелд и searchMode. На портале при открытии индекса можно работать с обозревателем поиска наряду с определением JSON индекса на параллельных вкладках для упрощения доступа к атрибутам полей. Вы можете проверить, какие поля доступны для поиска, сортировки, фильтрации и для которых можно протестировать при тестировании запросов. Рекомендуется для раннего исследования, тестирования и проверки. <br/>[Подробнее.](search-explorer.md) |
| Средства веб-тестирования| [POST или Visual Studio Code](search-get-started-rest.md) являются надежными вариантами для составления запроса [поиска документов](/rest/api/searchservice/search-documents) в остальное. REST API поддерживает все программные операции в Когнитивный поиск Azure, и при использовании такого средства, как POST или Visual Studio Code, можно выдавать запросы в интерактивном режиме, чтобы понять, как это работает перед инвестиции в код. Средство веб-тестирования является хорошим выбором, если у вас нет прав участника или администратора в портал Azure. Если у вас есть URL-адрес поиска и ключ API запроса, можно использовать средства для выполнения запросов к существующему индексу. |
