---
title: Синонимы расширения запроса по индексу поиска
titleSuffix: Azure Cognitive Search
description: Создайте карту синонимов для расширения сферы действия поискового запроса в индексе когнитивного поиска Azure. В область поиска включаются эквивалентные термины, указанные в списке.
manager: nitinme
author: brjohnstmsft
ms.author: brjohnst
...
В Azure Cognitive Search расширение синонимов выполняется во время запроса. Можно добавить карту синонимов в службу, не прерывая текущие операции. Можно добавить свойство **synonymMaps** в определение поля, не перестраивая индекс.

## <a name="create-synonyms"></a>Создание синонимов

Для создания синонимов нет поддержки портала, но можно использовать REST API или .NET SDK. Для начала работы с REST мы рекомендуем [использовать Postman](search-get-started-postman.md) и формулировку запросов с помощью этого API: [Создание Synonym Maps.](https://docs.microsoft.com/rest/api/searchservice/create-synonym-map) Для разработчиков сc-разработчиками можно начать работу с [добавлением синонимов в Azure Cognitive Search с помощью C .](search-synonyms-tutorial-sdk.md)

В обязательном порядке, если вы используете [управляемые клиентом ключи](search-security-manage-encryption-keys.md) для шифрования на стороне службы на отдыхе, вы можете применить эту защиту к содержимому вашей синонимной карты.

## <a name="use-synonyms"></a>Использование синонимов

В Azure Cognitive Search поддержка синонимов основана на синонимных картах, которые вы определяете и загружаете в свой сервис. Эти карты представляют собой независимый ресурс (как индексы или источники данных) и могут использоваться любым полем, поддерживающим поиск, в любом индекс в службе поиска.
...
Для передачи карты синонимов в службу используется запрос POST или PUT. Правила должны быть разделены символами новой строки (\n). Вы можете определить до 5000 правил на карту синонимов в бесплатной услуге и 20 000 правил на карту во всех остальных SkUS. Каждое правило может содержать до 20 расширений.
...
Формат Solr поддерживает эквивалентные и явные сопоставления синонимов. Правила картирования соответствуют спецификации синоним-фильтра Apache Solr с открытым исходным кодом, описанной в этом документе: [SynonymFilter](https://cwiki.apache.org/confluence/display/solr/Filter+Descriptions#FilterDescriptions-SynonymFilter). Ниже приведен пример правила для эквивалентных синонимов.
```
USA, United States, United States of America
```
...
> [Создание синонимной карты](https://docs.microsoft.com/rest/api/searchservice/create-synonym-map)