...
На портале не поддерживается создание синонимов, но можно использовать REST API или пакет SDK для .NET. Чтобы начать работу с ОСТАВШЕЙся, мы рекомендуем [использовать POST](search-get-started-postman.md) и формулировку запросов с помощью этого API: [Создание](https://docs.microsoft.com/rest/api/searchservice/create-synonym-map)сопоставлений синонимов. Разработчикам можно приступить к работе с [добавлением синонимов в службе поиска C#Azure с помощью. ](search-synonyms-tutorial-sdk.md) C#
...
Формат Solr поддерживает эквивалентные и явные сопоставления синонимов. Правила сопоставления соответствуют спецификации синонимов с открытым исходным кодом в Apache Solr, описанном в этом документе: [SynonymFilter](https://cwiki.apache.org/confluence/display/solr/Filter+Descriptions#FilterDescriptions-SynonymFilter). Ниже приведен пример правила для эквивалентных синонимов.
```
USA, United States, United States of America
```
