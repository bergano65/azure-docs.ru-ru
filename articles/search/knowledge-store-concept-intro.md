---
title: Общие сведения о хранилище знаний — Поиск Azure
...
---
# <a name="what-is-knowledge-store-in-azure-search"></a>Хранилище знаний в службе "Поиск Azure"

Хранилище знаний — это дополнительный компонент службы "Поиск Azure" (сейчас предлагается общедоступная предварительная версия), который предназначен для хранения обогащенных документов и метаданных, созданных с помощью конвейера индексирования на базе ИИ [(когнитивный поиск)](cognitive-search-concept-intro.md). Хранилище знаний поддерживает учетная запись хранения Azure, которая настраивается как элемент конвейера. При активации служба поиска кэширует представление каждого обогащенного документа с помощью этой учетной записи хранения. 

Если вы ранее использовали когнитивный поиск, то знаете, что документ можно подвергнуть серии операций обогащения с помощью наборов навыков. Результатом этого процесса может быть индекс службы "Поиск Azure" или проекции документа в хранилище знаний (нововведение в этой предварительной версии).

Проекции — это механизм структурирования данных для их использования конечным приложением. [Обозреватель службы хранилища](https://docs.microsoft.com/azure/vs-azure-tools-storage-manage-with-storage-explorer?tabs=windows), предназначенный специально для службы хранилища Azure, или любое приложение, которое подключается к службе хранилища Azure, позволяют воспользоваться новыми возможностями, обеспечиваемыми обогащенными документами. Примерами использования могут служить конвейеры обработки и анализа данных или настраиваемая аналитика.
