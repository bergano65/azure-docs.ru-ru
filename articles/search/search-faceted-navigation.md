---
title: Добавьте граненые иерархии категорий навигации
titleSuffix: Azure Cognitive Search
description: Добавьте граненые навигации для самостоятельной фильтрации в поисковых приложениях, которые интегрируются с Azure Cognitive Search.
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
---
# <a name="how-to-implement-faceted-navigation-in-azure-cognitive-search"></a>Как реализовать граненые навигации в Azure Когнитивный поиск
...
Фасетная навигация представляет собой механизм фильтрации, обеспечивающий самоуправляемую детализированную навигацию в приложениях поиска. Термин "фасетная навигация" может быть неизвестен вам, но вы, вероятно, использовали этот механизм ранее. Как показано в следующем примере, фасетная навигации — это не что иное, как категории, с помощью которых фильтруются результаты.
 ...
 ![Демо-демо-портал когнитивного поиска Azure](media/search-faceted-navigation/azure-search-faceting-example.png "Демо-демо-портал когнитивного поиска Azure")

Фасетная навигация — это альтернативная точка входа в поиск. Ее можно использовать вместо того, чтобы вручную вводить сложные выражения поиска. Facets может помочь вам найти то, что вы ищете, обеспечивая при этом, что вы не получите нулевых результатов. Как разработчик, аспекты позволяют выявить наиболее полезные критерии поиска для навигации по индексу поиска. В онлайн-приложениях розничной торговли, граненые навигации часто построен над брендами, отделы (детская обувь), размер, цена, популярность, и рейтинги. 

Способ реализации фасетной навигации зависит от технологии поиска. В Azure Cognitive Search граненые навигации строятся во время запроса, используя поля, которые вы ранее приписывали в вашей схеме.
...
- Смотрите и тестируете рабочую демонстрацию онлайн на [Azure Cognitive Search Job Portal Demo.](https://aka.ms/azjobsdemo)

- Скачайте код из [репозитория примеров Azure на GitHub](https://github.com/Azure-Samples/search-dotnet-asp-net-mvc-jobs).

## <a name="get-started"></a>Начало работы
Если вы ранее не разрабатывали средства поиска, представьте, что фасетная навигация обеспечивает возможности самоуправляемого поиска. Это тип поиска сверления вниз, основанный на предопределенных фильтров, используемых для быстрого сужения результатов поиска с помощью точек и нажмите действия. 

### <a name="interaction-model"></a>Модель взаимодействия

Опыт поиска для граненых навигации является итеративным, так что давайте начнем с понимания его как последовательность запросов, которые разворачиваются в ответ на действия пользователя.
...
1. Запрос, отправленный в Azure Cognitive Search, определяет граненую структуру навигации через один или несколько параметров запроса граней. Например, запрос может включать элемент `facet=Rating` с необязательным параметром `:values` или `:sort`, дополнительно уточняющим представление данных.
...
В Azure Cognitive Search запрос указан через один или несколько параметров запроса (см. [Документы поиска](https://docs.microsoft.com/rest/api/searchservice/Search-Documents) для описания каждого из них). Ни один из параметров запроса не является обязательным, но при этом необходимо указать по крайней мере один параметр запроса.
...
Оба запроса являются действительными, но второй превосходит, если вы ищете не-мотели с парковкой в Сиэтле.
...
В коде приложения, который создает запрос поиска, выполняется граненые навигации с Azure Cognitive Search. Фасетная навигация зависит от определенных ранее элементов схемы.
...
Уровень представления данных в коде обеспечивает взаимодействие с пользователем. Он должен содержать составные части фасетной навигации, например метку, значения, флажки и счетчик. API Azure Cognitive Search REST является агностиком платформы, поэтому используйте любой нужный язык и платформу. Важно включить элементы пользовательского интерфейса, поддерживающие добавочное обновление. При этом состояние пользовательского интерфейса должно обновляться при выборе каждого дополнительного фасета. 
...
Azure Cognitive Search возвращает результаты поиска на основе одного или нескольких вводимых терминов, а также обновлений к граненой структуре навигации. В Azure Cognitive Search граненые навигации представляют собой одноуровневую конструкцию с значениями граней и подсчетом количества результатов для каждого из них.
...
Как вы можете видеть в схеме образца, выключен для строчных полей, `Facetable` которые не должны использоваться в качестве граней, таких как значения идентификатора. Отключение лица там, где вам не нужно, помогает сохранить размер индекса небольшим и, как правило, повышает производительность.
...
В веб-приложениях на уровне представления данных обычно используется технология AJAX, так как она позволяет обновлять добавочные изменения. Вы также можете использовать ASP.NET MVC или любой другой платформы визуализации, которые могут подключиться к службе когнитивного поиска Azure через HTTP. Пример приложения, на которое ссылаются в этой статье - **Демо-портал Azure Cognitive Search Job Portal** - оказывается ASP.NET mVC-приложении.
...
Как уже упоминалось, фасеты невозможно непосредственно вкладывать в иерархию. По умолчанию граненые навигации в Azure Cognitive Search поддерживают только один уровень фильтров. Однако это ограничение можно обойти. Вы можете кодировать иерархическую структуру фасета в `Collection(Edm.String)` с одной точкой входа для одной иерархии. Обход этого ограничения выходит за рамки данной статьи. 
...
**Повышение точности поиска с помощью фильтров**
 ...
 Используйте фильтры. Если вы полагаетесь только на выражения поиска в одиночку, то, по крайней случае, может привести к возвращению документа, не имеющем точного значения граней ни в одном из его полей.
...
* Параметр `&facet=city,count:5` указывает, что при применении фасета будут возвращены только первые 5 городов с наивысшим приоритетом. Рассмотрим пример запроса с поисковым термином "аэропорт" и 32 совпадения. Если при этом включить в запрос параметр `&facet=city,count:5`, то в результаты поиска, полученные при использовании фасета, будут включены только первые пять уникальных городов с наибольшим количеством документов.
...
> [!NOTE]
> Если имеются фасеты несколько типов, параметр `count` может привести к отображению различных данных. В следующей таблице приводится краткое изложение того, как этот термин используется в API-иности когнитивного поиска Azure, примере кода и документации. 
...
* "`@odata.count`"<br/>
   В ответе на запрос это значение указывает количество совпадений в результатах поиска. В среднем, это больше, чем сумма всех результатов аспекта вместе взятых, из-за наличия элементов, которые соответствуют термину поиска, но не имеют совпадений значения граней.

**Получение числа результатов для аспекта**

При добавлении фильтра в фасетный запрос вам может потребоваться сохранить инструкцию аспекта (например, `facet=Rating&$filter=Rating ge 4`). Технически, глицая оценка не нужна, но сохранение его возвращает количество граней значения для рейтингов 4 и выше. Например, если вы щелкнете "4", а запрос включает фильтр для отображения оценок, начиная с "4", будут возвращены количественные показатели для каждой оценки от 4 и выше.  

**Обеспечение точного числа результатов для аспекта**

При определенных обстоятельствах может обнаружить, что количество граней не соответствует наборам результатов (см. [Faceted навигации в Azure Cognitive Search (сообщение форума)](https://social.msdn.microsoft.com/Forums/azure/06461173-ea26-4e6a-9545-fbbd7ee61c8f/faceting-on-azure-search?forum=azuresearch)).
...
Хотя это поведение может измениться в любое время, если вы столкнулись с этим\<поведением сегодня, вы можете обойти его, искусственно раздувая количество: число> до большого числа, чтобы обеспечить полную отчетность от каждого осколок. Если значение параметра count: больше или равно количеству уникальных значений в поле, вы гарантировано получите точные результаты. Однако если имеется очень много документов, производительность снижается, поэтому выполняйте эти действия рассудительно.
...
Метки обычно определяются в HTML-файле или форме (`index.cshtml` в примере приложения). В Azure Cognitive Search нет API для меток навигации граней или любых других метаданных.

<a name="rangefacets"></a>

## <a name="filter-based-on-a-range"></a>Фильтрация по диапазону значений
В приложениях поиска часто используется фасетная навигация на основе диапазонов значений. Поддерживаются диапазоны числовых данных, а также значений даты и времени. Вы можете прочитать больше о каждом подходе в [поисковых документах (Azure Cognitive Search API).](https://docs.microsoft.com/rest/api/searchservice/Search-Documents)

Azure Cognitive Search упрощает построение диапазона, предоставляя два подхода для вычисления диапазона. Для обоих подходов Azure Cognitive Search создает соответствующие диапазоны с учетом предоставленных вами входов. Например, если указать диапазон значений 10|20|30, будут автоматически созданы диапазоны 0–10, 10–20, 20–30–10, 10–20 и 20–30. Приложение может при необходимости удалить все пустые интервалы. 
...
Каждый диапазон создается с использованием отправной точки 0 и конечной точки, равной значению из списка. Затем из всего диапазона удаляется предыдущий диапазон, что позволяет создавать дискретные интервалы. Azure Cognitive Search делает это как часть граненой навигации. Для создания каждого интервала не нужно писать код.
...
Обычно можно увидеть фильтры, которые помогут вам выбрать магазин, ресторан или пункт назначения в зависимости от его близости к текущему местоположению. Хотя этот тип фильтра может выглядеть как граненые навигации, это просто фильтр. Этот вопрос упоминается здесь для пользователей, которым нужно решить проблему с реализацией этого компонента.

Есть две геопространственные функции в Azure Cognitive Search, **geo.distance** и **geo.intersects**.
...
Вы можете найти примеры фильтров в [синтаксисе выражения OData (Azure Cognitive Search).](query-odata-filter-orderby-syntax.md)

<a name="tryitout"></a>

## <a name="try-the-demo"></a>Пробное использование демоверсии
Демо-портал Azure Cognitive Search Job Data Содержит примеры, приведенные в этой статье.
...
Смотреть [Azure Когнитивный поиск Глубокий погружение](https://channel9.msdn.com/Events/TechEd/Europe/2014/DBI-B410). Начиная с 45:25, на этой видеозаписи демонстрируется, как реализовать фасеты.
...
* [Фронт Конец проблем при реализации Граненый поиск - Часть 1](https://articles.uie.com/faceted_search2/)
