...
Оповещения, связанные с хранилищем, в настоящее время недоступны; потребление хранилища не агрегировано или не входит в **азуреметрикс**. Для получения уведомлений, связанных с ресурсами, необходимо создать пользовательское решение.

На портале вкладка « **Использование** » показывает доступность ресурсов относительно текущих [ограничений](search-limits-quotas-capacity.md) , накладываемых уровнем служб. 
...
Записанные в журнал события включают связанные с индексированием и запросами. Таблица **система диагностики Azure** в log Analytics собирает операционные данные, связанные с запросами и индексированием.
...
Поскольку индексы поиска являются компонентом более крупного клиентского приложения, для управления доступом к индексу не существует встроенной методологии для отдельных пользователей. Предполагается, что запросы поступают из клиентского приложения для запросов администратора или запроса. Административные операции чтения и записи включают создание, обновление и удаление объектов во всей службе. Операции только для чтения — это запросы к коллекции Documents, областью действия которых является один индекс. 

Таким образом, содержимое журналов будет ссылаться на вызовы с помощью ключей администратора или ключей запросов. Соответствующий ключ включается в запросы, исходящие из клиентского кода. Служба не поддерживает обработку токенов идентификации или олицетворения.
