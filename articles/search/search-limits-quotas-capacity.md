...
<sup>2</sup> Наличие очень большого числа элементов в сложных коллекциях на документ в данный момент приводит к высокой загрузке хранилища. Это известная проблема В то же время ограничение в 3000 считается надежной верхней границей для всех уровней служб. Это ограничение применяется только для операций индексирования, которые используют самую раннюю общедоступную версию API с поддержкой полей сложных типов (`2019-05-06`) или более поздние версии. Чтобы не прерывать работу клиентов, которые могут использовать более ранние предварительные версии API (которые поддерживают поля сложных типов), мы не будем применять это ограничение для операций индексирования, использующих эти API предварительных версий. Обратите внимание, что API предварительных версий не предназначены для использования в рабочих средах, и мы настоятельно рекомендуем всем клиентам перейти на последнюю общедоступную версию API.
...
Чтобы определить, существуют ли ограничения на количество документов для вашей службы, примените [REST API получения статистики по службе](https://docs.microsoft.com/rest/api/searchservice/get-service-statistics). Ограничения по документам будут указаны в ответе, а значение `null` обозначает отсутствие ограничений.
