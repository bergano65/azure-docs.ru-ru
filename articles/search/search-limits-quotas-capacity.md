...
Запросы API регулируется по мере того, как система приближается к пиковой емкости. Регулирование работает по-разному для разных API. API запросов (поиск, предложения и автозавершение) и API индексирования регулируются динамически в зависимости от нагрузки на службу. Интерфейсы API индекса и API операций службы имеют ограничения частоты статических запросов. 

Действуют следующие ограничения по запросам для операций, связанных с индексом.

+ Список индексов (GET/индексес): 3 в секунду на единицу поиска
...
Ограничения запросов статической скорости для операций, связанных со службой:

+ Статистика службы (GET/сервицестатс): 4 в секунду на единицу поиска
