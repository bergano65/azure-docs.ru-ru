...
> [!NOTE]
> Хотя максимальная емкость одного индекса обычно ограничена доступным хранилищем, на общем количестве документов, которые могут храниться в одном индексе, имеются максимальные верхние границы. Это ограничение составляет около 24 миллиардов документов на индекс для служб поиска Basic, S1, S2 и S3 и 2 миллиарда документов на индекс для поисковых служб S3HD. Каждый элемент сложной коллекции считается отдельными документами для целей этих ограничений.

<a name="document-limits"></a>

## <a name="document-limits"></a>Ограничения документов 

По состоянию на октябрь 2018 года в любом регионе больше нет ограничений на количество документов для любого нового сервиса, созданного на любом оплачиваемом уровне (Basic, S1, S2, S3, S3 HD). Старые службы, созданные до октября 2018 года, по-прежнему могут быть предметом ограничений по количеству документов.
...
| Индексатор BLOB-объектов: максимальное число символов в содержимом, извлеченном из BLOB-объекта |32 000 |64 000 |4&nbsp;миллиона |8&nbsp;миллионов |16&nbsp;миллионов |Н/Д |4&nbsp;миллиона |4&nbsp;миллиона |
