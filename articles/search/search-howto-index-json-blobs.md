---
title: Индексирование больших двоичных объектов JSON из индексатора больших двоичных объектов Azure для полнотекстового поиска
...
Мы рекомендуем использовать одну и ту же подписку Azure как для Azure Когнитивный поиск, так и для службы хранилища Azure (желательно в том же регионе).
...
Добавление когнитивных навыков не требуется для импорта документов JSON. Если нет необходимости [добавлять обогащение искусственного интеллекта](cognitive-search-concept-intro.md) в конвейер индексирования, этот шаг следует пропустить.
...
Этот шаг предоставляет сведения о соединении с источником данных, используемые индексатором. Источник данных — это именованный объект в Когнитивный поиск Azure, сохраняющий сведения о соединении. Тип источника данных, `azureblob`, определяет, какие поведения извлечения данных вызываются индексатором. 
...
+ [Индексаторы в Azure Когнитивный поиск](search-indexer-overview.md)
