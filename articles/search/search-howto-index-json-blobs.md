...
Большие двоичные объекты JSON в хранилище BLOB-объектов Azure обычно представляют собой один документ JSON или коллекцию сущностей JSON. Для коллекций JSON большой двоичный объект может иметь **массив** элементов JSON правильного формата. Большие двоичные объекты также могут состоять из нескольких отдельных сущностей JSON, разделенных символом новой строки. Индексатор больших двоичных объектов в службе поиска Azure может проанализировать любую такую конструкцию в зависимости от того, как в запросе задан параметр **parsingMode** .

Все режимы анализа JSON (`json`, `jsonArray`, `jsonLines`) теперь являются общедоступными. 
...
1. [Создайте контейнер больших двоичных объектов](https://docs.microsoft.com/azure/storage/blobs/storage-quickstart-blobs-portal) для хранения данных. Для общего уровня доступа можно задать любое из его допустимых значений.
...
На странице Обзор службы поиска Azure можно [запустить мастер](search-import-data-portal.md) из панели команд или щелкнуть **Добавить поиск Azure** в разделе **Служба BLOB-объектов** в левой области навигации вашей учетной записи хранения.
...
Этот шаг предоставляет сведения о соединении с источником данных, используемые индексатором. Источник данных — это именованный объект в поиске Azure, который сохраняет сведения о подключении. Тип `azureblob`источника данных определяет, какие поведения извлечения данных вызываются индексатором. 
...
> [!IMPORTANT]
> При использовании `json` `jsonArray` или врежимесинтаксическогоанализаслужбапоискаAzureпредполагает,чтовсебольшиедвоичныеобъектывисточникеданныхсодержатJSON.`jsonLines` Если необходима поддержка как объектов JSON, так и других объектов в одном источнике данных, сообщите нам об этом на [нашем сайте UserVoice](https://feedback.azure.com/forums/263029-azure-search).
...
Для массивов JSON, имеющих вложенные элементы, можно `documentRoot` указать, чтобы указать многоуровневая структура. Например, если большой двоичный объект выглядит следующим образом:
...
Опять же, обратите внимание, что сопоставления полей можно опустить, как `jsonArray` в режиме синтаксического анализа.
