...
В целях выставления счетов Когнитивный поиск имеет понятие *единицы поиска* (SU). SU — это произведение *реплик* и *секций* , используемых службой: **(R x P = SU)**. Количество серверов, умноженное на коэффициент оплаты **(SU * ставка = ежемесячные расходы)** , является основным определителем затрат, связанных с поиском. 
