---
title: Фильтр по языку в поисковом индексе
titleSuffix: Azure Cognitive Search
description: Критерии фильтрации для поддержки многоязычного поиска, отслеживание выполнения запросов в областях, специфичные для языка.
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
ms.locfileid: "74112066"
---
# <a name="how-to-filter-by-language-in-azure-cognitive-search"></a>Как фильтровать по языку в Azure Cognitive Search 

Основным требованием к многоязычному приложению поиска является возможность выполнять поиск и извлекать результаты на языке пользователя. В Azure Cognitive Search одним из способов удовлетворения языковых требований многоязычного приложения является создание ряда полей, предназначенных для хранения строк на определенном языке, а затем ограничение полного поиска текста только этими полями во время запроса.
...
Успех этого метода зависит от целостности содержимого поля. Azure Cognitive Search не переводит строки и не выполняет обнаружение языка. Вы должны убедиться, что поля содержат ожидаемые строки.

## <a name="define-fields-for-content-in-different-languages"></a>Определение полей для содержимого на разных языках

В Azure Cognitive Search запросы нацелены на один индекс. Разработчики, желающие предоставить языковые строки в одном поисковом интерфейсе, обычно задают выделенные поля для хранения значений: одно поле для строк на английском, одно для строк на французском и т. д. 
