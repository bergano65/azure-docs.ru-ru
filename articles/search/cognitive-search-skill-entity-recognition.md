...
|includeTypelessEntities | Если задано значение true, когда текст содержит известную сущность, но ее нельзя отнести к одной из поддерживаемых категорий, она будет возвращена как часть составного поля выходных данных `"entities"`. Значение по умолчанию — `false`. |
