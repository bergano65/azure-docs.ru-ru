...
Это элементы, которые хорошо известный, но не классифицируется как часть текущие поддерживаемые «категории». Для экземпляра «Windows 10» — это известное сущность (продукт), но не являются «Продукты» в категории, поддерживаемые в настоящее время. Значение по умолчанию — `false`. |
...
| namedEntities | Массив сложных типов, содержащий следующие поля: <ul><li>category</li> <li>значение (имя фактического объекта)</li><li>смещение (расположение, где оно было найдено в тексте);</li><li>confidence (Пока не используется. Будет задаваться значение -1.).</li></ul> |
