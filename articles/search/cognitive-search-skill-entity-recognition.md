...
| Ввод имени      | Описание                   |
|---------------|-------------------------------|
| languageCode  | Необязательно. Значение по умолчанию — `"en"`.  |
| текст          | Текст для анализа.          |

## <a name="skill-outputs"></a>Выходные данные навыка

> [!NOTE]
> Для некоторых языков категории сущностей не поддерживаются. Типы категорий сущностей `"Person"`, `"Location"`и `"Organization"` поддерживаются для полного списка языков, описанных выше. При извлечении типов `"Quantity"`, `"Datetime"`, `"URL"`и `"Email"` поддерживаются только _de_, _EN_, _ES_, _fr_и _zh-Ханс_ . Дополнительные сведения см. [в разделе Поддержка языков и регионов для API анализа текста](https://docs.microsoft.com/azure/cognitive-services/text-analytics/language-support).  

| Имя вывода     | Описание                   |
|---------------|-------------------------------|
| Люди      | Массив строк, где каждая строка представляет имя человека. |
| положения  | Массив строк, где каждая строка представляет расположение. |
