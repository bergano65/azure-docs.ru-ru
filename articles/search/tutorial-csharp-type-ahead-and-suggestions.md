---
title: Руководство по C# для функции автозаполнения и предложений — Поиск Azure
description: Это руководство основано на проекте "Разбиение результатов поиска на страницы (Поиск Azure)" для добавления автозаполнения и предложений. Целью является предоставление улучшенных возможностей для пользователя. Узнайте, как объединить раскрывающийся список предложений с функцией встроенного автозаполнения.
services: search
ms.service: search
ms.topic: tutorial
ms.author: v-pettur
...
4. В скрипте представления задайте для параметра **&fuzzy** значение true и снова запустите приложение. Теперь введите "po". Обратите внимание: при поиске предполагается, что одна буква неправильная!
    ...
    Подробные сведения о логике, используемой в поиске по нечетким соответствиям, приведены в статье о [синтаксисе запросов Lucene в службе "Поиск Azure"](https://docs.microsoft.com/azure/search/query-lucene-syntax).
...
Объединение функций автозаполнения и предложений является самой сложной задачей и, возможно, обеспечивает максимальное удобство для пользователя. Нам необходимо отобразить лучший вариант автозаполнения из Поиска Azure по мере набора текста. Кроме того, мы хотим увидеть несколько предложений в виде раскрывающегося списка.
...
> [C# Tutorial: Use facets to aid navigation - Azure Search](tutorial-csharp-facets.md) (Руководство по C#. Использование аспектов для улучшения навигации (Поиск Azure))
