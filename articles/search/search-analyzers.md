...
Следующий сегмент видео быстро пересылает объяснение того, как работает обработка текста в Azure Когнитивный поиск.
...
3. При необходимости вместо одного свойства **анализатора** можно задать различные анализаторы для индексирования и выполнения запросов с помощью параметров поля **индексанализер** и **сеарчанализер** . Для подготовки и извлечения данных можно использовать различные анализаторы, если одно из этих действий требует определенного преобразования, которое не требуется другому.

> [!NOTE]
> Нельзя использовать другой [анализатор языка](index-add-language-analyzers.md) во время индексирования, чем во время выполнения запроса для поля. Эта возможность зарезервирована для [пользовательских анализаторов](index-add-custom-analyzers.md). По этой причине, если вы попытаетесь задать для свойств **сеарчанализер** или **индексанализер** имя анализатора языка, REST API будет возвращать ответ об ошибке. Вместо этого следует использовать свойство **анализатора** .
