...
> * Получить предложения и автозаполнения, с помощью пакета SDK для .NET и REST API
...
1. В примере образец кода откройте файл решения AutocompleteTutorial.sln в Visual Studio.  Откройте файл Web.config в проекте AutocompleteTutorial и измените значения SearchServiceName и SearchServiceApiKey на указанные выше.

### <a name="running-the-sample"></a>Выполнение примера

Теперь вы готовы запустить пример приложения пример.  Откройте файл решения AutocompleteTutorial.sln в Visual Studio для выполнения этого примера.  Решение содержит проект ASP.NET MVC.  Нажмите клавишу F5, чтобы запустить проект и загрузить страницу в браузере по выбору.  В верхней части вы увидите параметр для выбора C# или JavaScript.  C# Параметр вызовы HomeController из браузера и использует пакет SDK для .NET для поиска Azure для получения результатов.  Параметр JavaScript вызывает REST API службы поиска Azure непосредственно из браузера.  Этот параметр обычно гораздо производительнее, так как контроллер не участвует в процессе.  Можно выбрать параметр, который подходит для ваших потребностей и языковых параметров.  На странице есть несколько примеров автозаполнения с инструкциями для каждого.  Для каждого примера есть рекомендуемый пример текста.  Попробуйте ввести несколько букв в каждом поле поиска и посмотрите, что произойдет.
...
Это простое текстовое поле ввода с классом для стилей, идентификатор для ссылки на JavaScript и замещающий текст.  Главное здесь — JavaScript.
...
Если сравнить это с примером выше, который вызывает контроллер Home, вы заметите несколько сходств.  Конфигурации автозаполнения для `minLength` и `position` абсолютно одинаковы.  А вот источник значительно отличается.  Вместо вызова метода предложить в контроллер home, запрос REST в функцию JavaScript создается и выполняется с помощью Ajax.  Затем ответ обрабатываются в success и используется в качестве источника.
...
Интегрируйте возможности предложений и автозаполнения в функцию поиска.  Рассмотрим, как с помощью пакета SDK для .NET или REST API напрямую поможет мощных возможностей поиска Azure для пользователей вводе чтобы повысить эффективность.
