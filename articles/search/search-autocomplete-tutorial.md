...
В этом примере сведения об использовании [предложения](https://docs.microsoft.com/rest/api/searchservice/suggestions) и [автозаполнения](https://docs.microsoft.com/rest/api/searchservice/autocomplete) для создания мощных функций поиска поле, поддерживающее поведения поиска как то type.

+ *Предложения* приведен список предлагаемых результаты, сформированные при вводе, где каждое из них — единственный результат, возвращаемый индекс, который совпадает с набранным до сих. 

+ *Автозаполнение*, [функцию предварительного просмотра](search-api-preview.md), «завершения», слово или фразу, пользователь вводит текст в настоящее время. Как и в случае с предложениями, завершенного слова или фразы объявлено в совпадение в индексе. 
...
* [Visual Studio 2017](https://visualstudio.microsoft.com/downloads/), любой выпуск. Пример кода и инструкции были протестированы в бесплатный выпуск Community.
...
## <a name="javascript-version-with-rest"></a>Версии JavaScript с помощью REST

Реализация JavaScript, откройте **IndexJavaScript.cshtml**. Обратите внимание, что функция автозаполнения пользовательского интерфейса jQuery также используется для поля поиска, сбор входных данных термин для поиска и выполнения асинхронных вызовов в службу поиска Azure для получения предложенный совпадений или завершения условия. 
...
1. [Создание службы поиска Azure](search-create-service-portal.md) или [найти существующую службу](https://ms.portal.azure.com/#blade/HubsExtension/BrowseResourceBlade/resourceType/Microsoft.Search%2FsearchServices) в рамках текущей подписки. Можно использовать это бесплатная служба, в этом примере. 
