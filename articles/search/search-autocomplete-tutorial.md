...
В этом примере показано поле поиска, поддерживающее поведение типа поиска. Есть две функции, которые вы можете использовать вместе или по отдельности:
...
Пример кода демонстрирует как предложения, так и автозаполнение, как в версиях языка СЗ, так и в JavaScript. 

Разработчики C# могут пошагово выполнить приложение на основе ASP.NET MVC, которое использует[пакет SDK .NET для Когнитивного поиска Azure](https://aka.ms/search-sdk). Логику для вызова автозаполнения и предложений можно найти в файле HomeController.cs. 

Для разработчиков JavaScript аналогичная логика представлена в файле IndexJavaScript.cshtml, где содержатся прямые вызовы [REST API Когнитивного поиска Azure](https://docs.microsoft.com/rest/api/searchservice/). 
...
Служба когнитивного поиска Azure не является обязательной для этого упражнения, поскольку в решении используется служба хостинга и демо-индекс NYCJobs. Если вы хотите создать этот индекс на собственном поисковом сервисе, [см.](#configure-app) В противном случае можно использовать существующий сервис и индекс для резервного копирования клиентского приложения JavaScript.
...
Пример кода для разработчиков как для разработчиков СК, так и JavaScript можно найти в [папке DotNetHowToAutoComplete,](https://github.com/Azure-Samples/search-dotnet-getting-started/tree/master/DotNetHowToAutocomplete) найденной в репозитории GitHub, запущенном **для поиска-дотнет.**

Выборка предназначена для существующего демо-поиска и заранее построенного индекса, населенного [демо-данными NYCJobs.](https://github.com/Azure-Samples/search-dotnet-asp-net-mvc-jobs) Индекс NYCJobs содержит [конструкцию средства подбора](index-add-suggesters.md), что является обязательным условием для использования предложений и (или) автозаполнения.

## <a name="run-the-sample"></a>Запуск примера

1. Откройте **AutocompleteTutorial.sln** в Visual Studio. Решение содержит ASP.NET проект MVC с подключением к существующему поисковому сервису и индексу.

1. Обновление пакетов NuGet:

   1. В Solution Explorer, правой кнопкой **мыши DotNetHowToAutoComplete** и выберите **Управление NuGet пакеты**.  
   1. Выберите вкладку **Обновления,** выберите все пакеты и нажмите **Обновление**. Примите любые лицензионные соглашения. Для обновления всех пакетов может потребоваться несколько пропусков.

1. Нажмите F5, чтобы запустить проект и загрузить страницу в браузере.
...
![Пример страницы запуска](media/search-autocomplete-tutorial/startup-page.png "Пример стартовой страницы в localhost")

Попробуйте ввести несколько букв в каждом поле поиска и посмотрите, что произойдет.

## <a name="query-inputs"></a>Входные данные запросов
...
Реализация автозаполнения и предложений JavaScript называет REST API, используя URI в качестве источника для указания индекса и операции. 
...
## <a name="create-an-nycjobs-index"></a>Создание индекса NYCJobs
