...
Образец представляет комплексное покрытия, автозаполнения, фасетная навигация и предложения кэширование на стороне клиента. Просмотр сведений и комментарии для полного описания образец предлагает.
...
Данный пример является простое текстовое поле ввода с классом для стилей, идентификатор для ссылки на JavaScript и замещающий текст.  Магия имеет встроенный JavaScript.
...
Приведенную выше строку сообщает функция автозаполнения пользовательского интерфейса jQuery, где можно получить список элементов для отображения под полем поиска. Поскольку этот проект является проектом MVC, он вызывает функцию предложить в HomeController.cs, который содержит логику для возвращения предложений запроса (Подробнее о предложить в следующем разделе). Эта функция также передает ряд параметров выделения элемента управления, нечеткое соответствие и термин. API JavaScript автозаполнения добавляет параметр термина.
...
Первое, что вы можете заметить, — это метод, в верхней части класса, который вызывается `InitSearch`. Этот метод создает клиент индекс прошедшего проверку подлинности HTTP для службы поиска Azure. Дополнительные сведения см. в разделе [использование поиска Azure в приложении .NET](https://docs.microsoft.com/azure/search/search-howto-dotnet-sdk).
...
Если вы сравните этот пример, в приведенном выше примере, вызывает контроллер Home, можно заметить несколько сходства.  Конфигурации автозаполнения для `minLength` и `position` абсолютно одинаковы. 
