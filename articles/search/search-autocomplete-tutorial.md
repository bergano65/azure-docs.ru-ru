...
+ *Запрос* с указанием API Autocomplete или Предложений по запросу.
...
Элементы запроса включают API[(Autocomplete REST](https://docs.microsoft.com/rest/api/searchservice/autocomplete) или [Suggestion REST),](https://docs.microsoft.com/rest/api/searchservice/suggestions)частичный запрос и предложить.
...
Для дальнейшего уточнения ответа включите дополнительные параметры в запросе. Следующие параметры применяются как к Autocomplete, так и к предложениям.

| Параметр | Использование |
|-----------|-------|
| **$select** | Если у вас есть несколько **sourceFields,** используйте`$select=GameTitle` **$select,** чтобы выбрать, какое поле способствует значения ( ). |
| **$filter** | Применить критерии соответствия`$filter=ActionAdventure`на набор результатов ( ). |
