...
* уровень службы [влияет на выставление счетов](search-sku-tier.md) и задает верхний лимит производительности;
* в зависимости от региона службы будет определяться доступность определенных сценариев. Если нужно реализовать интеллектуальный анализ знаний и обогащение с помощью ИИ, вам потребуется разместить службы Cognitive Services и, возможно, "Машинное обучение Azure" в одном регионе со службой "Когнитивный поиск Azure".
...
Если вы используете несколько служб Azure, выберите регион, в котором также размещена ваша служба данных или приложения. Это приводит к снижению или отсутствию расходов на пропускную способность для исходящих данных (за исходящие данные не взимается плата, если службы находятся в одном регионе).
