...
Индексатор Data Lake Storage 2-го поколения поддерживает обнаружение изменений. Это означает, что при выполнении индексатора он переиндексирует только измененные большие двоичные объекты, определяемые отметкой времени `LastModified` большого двоичного объекта.

> [!NOTE] 
> Data Lake Storage 2-го поколения позволяет переименовывать каталоги. При переименовании каталога метки времени для больших двоичных объектов в этом каталоге не обновляются. В результате индексатор не будет переиндексации этих больших двоичных объектов. Если требуется повторно индексировать большие двоичные объекты в каталоге после переименования каталога, поскольку теперь они имеют новые URL-адреса, необходимо обновить `LastModified` метку времени для всех больших двоичных объектов в каталоге, чтобы индексатор знал о необходимости повторного индексирования в ходе следующего запуска.
