...
При определении группы проекций таблицы один узел в дереве обогащения может быть разбит на несколько связанных таблиц. Добавление таблицы с исходным путем, который является дочерней для существующей проекции таблицы, приведет к тому, что дочерний узел будет разрезаться из родительского узла и будет запланировать в новую, но связанную таблицу. Это позволяет определить отдельный узел в навыке фигуры, который может быть источником для всех проекций таблицы.
...
+ Источник. узел из дерева обогащения, из которого вы исследуете свои дополнения. Обычно это выходные данные фигуры, но могут быть выходными данными любого из навыков.
