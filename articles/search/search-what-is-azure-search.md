...
+ поисковая система для индексирования и выполнения запросов;
+ постоянное хранилище для индексов поиска, принадлежащих пользователю;
+ язык запросов для создания простых и сложных запросов;
+ (необязательно) [обогащения на основе ИИ](cognitive-search-concept-intro.md), которые помогают создать пригодное для поиска содержимое на основе изображений, необработанного текста и файлов приложений;
+ (необязательно) средства интеграции с другими службами Azure для хранения данных, машинного обучения, ИИ и обеспечения безопасности.
...
+ При индексировании текст принимается в службе поиска и подготавливается для поиска. На внутреннем уровне входящий текст обрабатывается в маркерах безопасности и хранится в инвертированных индексах для быстрой проверки. Вы можете передать любое содержимое в виде документов JSON.
