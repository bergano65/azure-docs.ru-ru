---
title: Общие сведения о Поиске Azure
description: Поиск Azure — это полностью управляемая размещенная облачная служба поиска Майкрософт. Просмотрите описания функций, рабочий процесс разработки, сравнение с другими продуктами Майкрософт для поиска и инструкции по началу работы.
manager: nitinme
author: HeidiSteen
services: search
ms.service: search
...
ms.locfileid: "69034838"
---
# <a name="what-is-azure-search"></a>Что такое Поиск Azure?

Поиск Azure — это облачное решение "поиск как услуга", которое предоставляет разработчикам API и средства для реализации расширенных возможностей при работе с конфиденциальным и разнородным содержимым веб-, мобильных и корпоративных приложений. Пользовательский код вызывает прием данных (индексирование), выдает запросы и обрабатывает ответы. Процесс поиска определяется в клиентском коде на основе функций Поиска Azure, которые выполняют запросы по сохраненному индексу, который вы можете создавать, администрировать и хранить в Поиске Azure.

![Архитектура Поиска Azure](media/search-what-is-azure-search/azure-search-diagram.png "Архитектура Поиска Azure")
...
## <a name="when-to-use-azure-search"></a>Для чего вам нужен Поиск Azure

Поиск Azure хорошо подходит для следующих сценариев работы в приложениях.

+ Консолидация разнородных типов содержимого в единый закрытый индекс с поддержкой поиска. Запросы всегда выполняются по индексу, который вы создаете и отправляете вместе с документами. Этот индекс хранится в облаке службы Поиска Azure. Индекс можно заполнить потоками документов JSON из любого источника или платформы. Кроме того, для содержимого из Azure можно использовать *индексатор* для извлечения данных в индекс. Возможности определения индекса, управления и владения им являются основными причинами использовать Поиск Azure.
...
| Мониторинг и диагностика | [**Включите функции мониторинга**](search-monitor-usage.md), чтобы получать более подробные метрики, помимо основных, которые всегда отображаются на портале. Метрики количества запросов в секунду, задержки и регулирования записываются на страницах портала без дополнительной настройки. <br/><br/>[**Аналитика поискового трафика**](search-traffic-analytics.md) — другой вариант мониторинга, при котором собираются и анализируются данные со стороны сервера и клиента, на основе чего вы получаете полезные сведения о поисковых запросах, которые вводят пользователи. |
