...
Как должно быть назначено только одно поле в индексе **ключ** поле, которое однозначно определяет каждый документ.

Другие атрибуты определяют, как поле используется в приложении. Например **для поиска** атрибуту назначено для каждого поля, которые должны быть включены в полнотекстовый поиск. 

API-интерфейсов, используемых при построении индекса имеют различные поведения по умолчанию. Для [интерфейсов REST API](https://docs.microsoft.com/rest/api/searchservice/Create-Index), большинство атрибутов включены по умолчанию (например, **для поиска** и **извлекаемые** выполняются для строковых полей) и часто достаточно установить их, если Вы хотите их отключить. Для пакета SDK для .NET верно обратное. Для любого свойства, которые явно не задано по умолчанию используется для отключения соответствующий способ поиска, если вы специально не включите ее.
