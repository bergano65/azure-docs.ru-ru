---
title: Создание определения и основных понятий индекса
titleSuffix: Azure Cognitive Search
description: Общие сведения о терминах и концепциях индексов в Когнитивный поиск Azure, включая компоненты и физическую структуру.
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
ms.locfileid: "72794213"
---
# <a name="create-a-basic-index-in-azure-cognitive-search"></a>Создание базового индекса в Azure Когнитивный поиск

В Azure Когнитивный поиск *индекс* — это постоянное хранилище *документов* и других конструкций, используемых для фильтрованного и полнотекстового поиска в службе когнитивный Поиск Azure. Документ представляет собой единый блок доступных для поиска данных в индексе. Например, в интернет-магазине для каждой позиции продажи есть определенный документ, в агентстве новостей есть такой документ для каждой статьи и т. д. Можно сравнить эти понятия с более знакомыми понятиями баз данных. В таком случае *индекс* похож на *таблицу*, а *документы* — на *строки* в ней.

При добавлении или передаче индекса Azure Когнитивный поиск создает физические структуры на основе предоставленной схемы. Например, если поле в индексе помечено как доступное для поиска, для него создается инвертированный индекс. Позже, при добавлении или отправке документов или при отправке поисковых запросов в Когнитивный поиск Azure, вы отправляете запросы в службу поиска по указанному индексу. Загрузка полей со значениям документов называется *индексированием* или приемом данных.
...
В схеме Когнитивный поиск индекс Azure состоит из следующих элементов. 
...
Более подробные сведения о [поддерживаемых типах данных](https://docs.microsoft.com/rest/api/searchservice/Supported-data-types)для когнитивный Поиск Azure см. здесь.
...
## <a name="scoring-profiles"></a>Профили повышения
...
Хотя все индексы Когнитивный поиск Azure шифруются по умолчанию с помощью управляемых ключей Майкрософт, в Key Vault можно настроить шифрование индексов с помощью **управляемых пользователем ключей** . Дополнительные сведения см. в статье [Управление ключами шифрования в Azure когнитивный Поиск](search-security-manage-encryption-keys.md).
