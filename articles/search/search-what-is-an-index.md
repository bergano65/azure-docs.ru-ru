...
3. Скачайте схему индекса с помощью [REST API получения индекса](https://docs.microsoft.com/rest/api/searchservice/get-index) и веб-инструмента тестирования, например [Postman](search-get-started-postman.md). Теперь у вас есть индекс, созданный на портале, в формате JSON. 
   ...
   На этом этапе следует перейти к программированию. Портал плохо подходит для итерации, так как вы не можете изменить уже созданный индекс. Но для выполнения оставшихся задач можно использовать Postman и REST.
...
Размер индекса определяется размером отправляемых документов, а также конфигурацией индекса, например включением предложений и определением атрибутов для отдельных полей. На следующем снимке экрана показаны шаблоны хранения индекса, полученные в результате различных сочетаний атрибутов.
...
Индексы, поддерживающие фильтрацию и сортировку, пропорционально больше, чем поддержка полнотекстового поиска. Операции фильтрации и сортировки просматривают точные совпадения, что требует наличия нетронутых документов. Напротив, доступные для поиска поля, поддерживающие полнотекстовый поиск и поиск нечетких соответствий, используют инвертированные индексы, которые заполнены разбитыми на лексемы термины и занимают меньше места, чем целые документы. 
...
Хотя все индексы Когнитивный поиск Azure шифруются по умолчанию с помощью ключей, управляемых корпорацией Майкрософт, в Key Vault можно настроить шифрование индексов с помощью **управляемых клиентом ключей** . Дополнительные сведения см. в статье [Управление ключами шифрования в Azure когнитивный Поиск](search-security-manage-encryption-keys.md).
