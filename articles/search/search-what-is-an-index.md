...
## <a name="define-fields"></a>Определение полей

Документ поиска определяется `fields` коллекцией. Вам понадобятся поля для запросов и ключей. Вам, возможно, понадобится поле для поддержки фильтров, аспектов и сортировок. Вам также могут потребоваться поля для данных, которые пользователь не видит, например, могут потребоваться поля для маржи прибыли или маркетинговые акции, которые можно использовать для изменения рейтинга поиска.
...
Обеспечение фильтрации или сортировки полей также увеличивает потребление хранилища, так как отфильтрованные и отсортированные поля не размечены, чтобы последовательности символов могли быть сопоставлены буквально.

Кроме того, не отражается в приведенной выше таблице — это влияние [анализаторов](search-analyzers.md). Если вы используете лексему Едженграм для хранения точных последовательностей символов (a, AB, ABC, ABCD), размер индекса будет больше, чем при использовании стандартного анализатора.
...
Вы можете получить практический опыт создания индекса, используя практически любой пример или Пошаговое руководство для Когнитивный поиск. Для начала можно выбрать любое из кратких руководств в содержании.

Но вы также захотите ознакомиться с методологиями для загрузки индекса с данными. Определение индекса и стратегии импорта данных определяются совместно. В следующих статьях содержатся дополнительные сведения о загрузке индекса.
