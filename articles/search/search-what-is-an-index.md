...
1. Скачайте схему индекса с помощью инструкции [Get index (REST API)](/rest/api/searchservice/get-index) и средства веб-тестирования, например [POST](search-get-started-postman.md). Теперь у вас есть представление JSON индекса, которое можно адаптировать для кода.
