...
С помощью клиента API создайте [запрос индексатора Get](https://docs.microsoft.com/rest/api/searchservice/get-indexer) , чтобы получить текущую конфигурацию индексатора. При использовании предварительной версии API для получения индексатора в определениях добавляется свойство `cache`, для которого задано значение null.
...
По умолчанию свойство `cache` имеет значение null. Используйте клиент API для настройки конфигурации кэша (портал не поддерживает это обновление партикулате). 
...
[Обновление индексатора](https://docs.microsoft.com/rest/api/searchservice/2019-05-06-preview/update-indexer) с помощью запроса на размещение. тело запроса должно содержать обновленное определение индексатора со свойством Cache. Если вы получаете 400, проверьте определение индексатора, чтобы убедиться, что выполнены все требования (источник данных, навык, индекс).
...
Кроме того, для [запуска индексатора](https://docs.microsoft.com/rest/api/searchservice/run-indexer)можно использовать команду "оставшаяся".
...
Для изменения набора навыков можно использовать портал или API. Например, если используется преобразование текста, то простое встроенное изменение с `en` на `es` или на другой язык достаточно для проверки концепции добавочного уточнения.
...
Чтобы настроить добавочное углубление для нового индексатора, достаточно включить свойство `cache` в полезные данные определения индексатора при вызове [CREATE индексатора (2019-05-06-Preview)](https://docs.microsoft.com/rest/api/searchservice/2019-05-06-preview/create-indexer). 
...
| API           | Влияние кэша     |
|---------------|------------------|
| [Создание индексатора (2019-05-06-Preview)](https://docs.microsoft.com/rest/api/searchservice/2019-05-06-preview/create-indexer) | Создает и запускает индексатор при первом использовании, включая создание кэша, если определение индексатора указывает его. |
