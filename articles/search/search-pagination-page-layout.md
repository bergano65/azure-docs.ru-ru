...
Структура ответа определяется параметрами в документе запрос: [Поиск](/rest/api/searchservice/Search-Documents) в REST API или [классе документсеарчресулт](/dotnet/api/microsoft.azure.search.models.documentsearchresult-1) в пакете SDK для .NET.
