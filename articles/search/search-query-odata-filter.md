...
- Выражения фильтра коллекции с использованием `any` или `all`. Они применяют условия фильтра к полям коллекции. Дополнительные сведения см. [в разделе операторы коллекции OData в когнитивный Поиск Azure](search-query-odata-collection-operators.md).
- Логические выражения, объединяющие другие логические выражения с помощью операторов `and`, `or`и `not`. Дополнительные сведения см. [в статье логические операторы OData в Azure когнитивный Поиск](search-query-odata-logical-operators.md).
- Выражения сравнения, которые сравнивают поля или переменные диапазона с постоянными значениями с помощью операторов `eq`, `ne`, `gt`, `lt`, `ge`и `le`. Дополнительные сведения см. [в статье операторы сравнения OData в когнитивный Поиск Azure](search-query-odata-comparison-operators.md). Выражения сравнения также используются для сравнения расстояний между геопространственными координатами с помощью функции `geo.distance`. Дополнительные сведения см. [в статье геопространственные функции OData в Azure когнитивный Поиск](search-query-odata-geo-spatial-functions.md).
...
- Вызовы логических функций, включая:
  ...
  - `search.in`, который сравнивает переменную поля или диапазона с каждым значением в списке значений. Дополнительные сведения см. [в статье `search.in` функции OData в Azure когнитивный Поиск](search-query-odata-search-in-function.md).
...
- Пути к полям или переменные диапазона типа `Edm.Boolean`. Например, если в индексе имеется логическое поле с именем `IsEnabled` и вы хотите вернуть все документы, в которых это поле `true`, критерием фильтра может быть только имя `IsEnabled`.
...
Оператор `not` имеет наивысший приоритет всех--даже выше, чем операторы сравнения. Вот почему, если вы попытаетесь написать фильтр следующим образом:
...
Эта ошибка возникает из-за того, что оператор связан только с полем `Rating`, который имеет тип `Edm.Int32`, а не со всем выражением сравнения. Исправление заключается в том, чтобы поставить операнд `not` в круглые скобки:
...
> [!TIP]
> Использование [функции `search.in`](search-query-odata-search-in-function.md) вместо длинных точек сравнения на равенство может помочь избежать ограничения для предложения фильтра, так как вызов функции считается единственным предложением.
...
Найдите все гостиницы с тегом "WiFi" хотя бы в одном помещении (где каждая комната содержит теги, хранящиеся в поле `Collection(Edm.String)`):  
...
Найти документы со словом hostel и рейтингом, большим или равным 4, или документы со словом motel и рейтингом 5. Этот запрос не может быть выражен без функции `search.ismatchscoring`, так как он сочетает полнотекстовый поиск с операциями фильтрации с помощью `or`.
...
Найти документы с фразой "ocean view" или рейтингом 5. Запрос `search.ismatchscoring` будет выполняться только по отношению к полям `HotelName` и `Description`. Документы, которые соответствуют только второму предложению дизъюнкции, будут возвращены с `Rating`, равным 5. Эти документы будут возвращены с оценками, равными нулю, чтобы ясно, что они не соответствуют ни одной из оцененных частей выражения.
