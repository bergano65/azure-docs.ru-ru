...
Версия 10 пакета SDK для Azure Когнитивный поиск .NET предназначена для последней общедоступной версии REST API (`2019-05-06`) с этими обновлениями:
...
Теперь у формы "навык" можно разрешить консолидацию входных данных из вложенных контекстов. Чтобы включить это изменение, мы `InputFieldMappingEntry` изменили его, чтобы можно было создать экземпляр, указав только `Source` свойство или свойства `SourceContext` и. `Inputs`
...
- Допускается создание `InputFieldMappingEntry` только тех `Source` мест, где инициализирована только инициализация.
