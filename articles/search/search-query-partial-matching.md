...
ms.locfileid: "80666785"
---
# <a name="partial-term-search-and-patterns-with-special-characters---azure-cognitive-search-wildcard-regex-patterns"></a>Частичный поиск термина и шаблоны со специальными символами - Azure Cognitive Search (wildcard, regex, шаблоны)

*Частичный поиск термина* относится к запросам, состоящим из фрагментов термина, таких как первая, последняя или внутренняя части строки. *Шаблон* может быть комбинацией фрагментов, иногда с особыми символами, такими как тире или слэши, которые являются частью запроса. Общие случаи использования включают запрос на части номера телефона, URL- н.с. людей или кодов продуктов или сложных слов.

Частичный поиск может быть проблематичным, если индекс не имеет терминов в формате, необходимых для сопоставления шаблонов. На этапе анализа текста индексирования, используя анализатор стандарта по умолчанию, отбрасываются специальные символы, составные и составные строки расходятся, в результате чего запросы шаблонов терпят неудачу, когда не найдено совпадений. Например, такой номер `+1 (425) 703-6214` `"1"`телефона (токенизированный как , `"425"` `"703"`, ) `"3-62"` `"6214"`не будет отображаться в запросе, потому что это содержимое на самом деле не существует в индексе. 

Решение состоит в том, чтобы вызвать анализатор, который сохраняет полную строку, включая пробелы и специальные символы, если это необходимо, так что вы можете поддерживать частичные термины и шаблоны. Создание дополнительного поля для нетронутой строки, а также использование контент-сохраняющего анализатора, является основой решения.
...
+ [Поиск Wildcard](query-lucene-syntax.md#bkmk_wildcard) или [регулярные выражения,](query-lucene-syntax.md#bkmk_regex) которые ищут шаблон или части встроенной строки, включая суффикс. Wildcard и регулярные выражения требуют полного синтаксиса Lucene. 

  Некоторые примеры частичного поиска термина включают в себя следующее. Для запроса суффикса, учитывая термин "буквенное",`search=/.*numeric.*/`вы будете использовать поиск подстановочных знаков () для поиска совпадения. Для частичного термина, включающий символы, такие как фрагмент URL, может потребоваться добавить символы побега. В JSON, вперед `/` слэш сбежал `\`с отсталым черта . Таким образом, `search=/.*microsoft.com\/azure\/.*/` синтаксис для фрагмента URL "microsoft.com/azure/".
