...
+ Поиск Azure — это полностью управляемая облачная служба с 99,9% соглашений об уровне обслуживания (SLA) при подготовке с достаточной избыточностью (2 реплики для доступа на чтение, три реплики для чтения и записи).
...
Все эти действия можно выполнить с помощью примера кода с **индексом резервного копирования и восстановления** в этом [репозитории примера .NET поиска Azure](https://github.com/Azure-Samples/azure-search-dotnet-samples). 

Вы также можете [получить определение индекса](https://docs.microsoft.com/rest/api/searchservice/get-index) в любое время с помощью REST API поиска Azure.
...
Нет, если вы удалите индекс или службу поиска Azure, ее нельзя будет восстановить. При удалении службы "Поиск Azure" все индексы в службе будут удалены без возможности восстановления. Если удалить группу ресурсов Azure, содержащую одну или несколько служб поиска Azure, все службы будут удалены без возможности восстановления.  
...
В качестве альтернативы можно использовать пример кода с **индексом для резервного копирования** в этом [репозитории .NET для поиска Azure](https://github.com/Azure-Samples/azure-search-dotnet-samples) , чтобы создать резервную копию определения индекса и моментального снимка индекса в серии JSON-файлов. Позже при необходимости можно будет использовать средство и файлы для восстановления индекса.  
