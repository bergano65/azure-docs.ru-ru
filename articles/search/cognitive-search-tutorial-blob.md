---
title: Руководство по использованию REST. Создание конвейера обогащения ИИ с помощью Когнитивного поиска в Поиске Azure
description: Узнайте, как выполнять извлечение текста и обработку естественного языка по содержимому BLOB-объектов JSON с помощью Postman и REST API Поиска Azure.
manager: pablocas
author: luiscabrer
services: search
ms.service: search
...
ms.locfileid: "70102821"
---
# <a name="tutorial-add-structure-to-unstructured-content-with-cognitive-search"></a>Руководство по Добавление структуры в неструктурированное содержимое с помощью Когнитивного поиска

Если у вас есть неструктурированное текстовое или визуальное содержимое, функция [Когнитивный поиск](cognitive-search-concept-intro.md) в Поиске Azure поможет вам извлекать из него информацию и создавать новое содержимое, применимое для сценариев полнотекстового поиска и интеллектуального анализа. Хотя Когнитивный поиск может обрабатывать файлы изображений (JPG, PNG, TIFF), в этом руководстве внимание уделяется только текстовому содержимому, созданию новых полей на основе распознавания языка и текстовой аналитики. Все это позволит вам использовать информацию в запросах, аспектах и фильтрах.
...
> * Создайте конвейер, который извлекает текст, а также распознает язык, сущности и ключевые фразы.
...
> * Изучите содержимое с помощью полнотекстового поиска и расширенного синтаксиса запросов.
...
1. Выполните поиск по строке *учетная запись хранения* и выберите "Учетная запись хранения Microsoft".
   ...
   ![Создание учетной записи хранения](media/cognitive-search-tutorial-blob/storage-account.png "Create Storage account")
...
1. На вкладке "Основные сведения" нужно ввести следующие сведения. Сохраните значения по умолчанию для остальных параметров.
   ...
   + **Расположение.** Желательно выбрать то же расположение, которое использовалось для Поиска Azure и Cognitive Services. При использовании одного расположения плата за трафик не взимается.
...
1. Выберите *cog-search-demo* и щелкните **Отправить**, чтобы открыть папку с сохраненными файлами для скачивания. Выберите все файлы, не являющиеся изображениями. Их должно быть семь. Щелкните **ОК**, чтобы начать отправку.
   ...
   ![Отправка примеров файлов](media/cognitive-search-tutorial-blob/sample-files.png "Upload sample files")

1. Прежде чем выйти из службы хранилища Azure, получите строку подключения для создания подключения в Поиске Azure. 
...
Обогащение искусственного поиска в Когнитивном поиске основано на платформе Cognitive Services, которая включает службы "Анализ текста" для обработки естественного языка и "Компьютерное зрение" для обработки изображений. Если бы вы создавали реальный прототип или проект, на этом этапе нужно было бы создать Cognitive Services (в том же регионе, что и Поиск Azure) для связывания с операциями индексирования.

Но для нашего примера подготовку ресурсов можно пропустить, так как Поиск Azure может подключаться к Cognitive Services в фоновом режиме и предоставляет 20 бесплатных транзакций для каждого выполнения индексатора. В примере из этого руководства используется всего семь транзакций. Это значит, что нам достаточно уровня "Бесплатный". Для крупных проектов вам, скорее всего, нужно будет подготовить Cognitive Services на уровне S0 с оплатой по мере использования. См. сведения о [подключении Cognitive Services](cognitive-search-attach-cognitive-services.md).

### <a name="azure-search"></a>Поиск Azure

Третий компонент — Поиск Azure, который вы можете [создать на портале](search-create-service-portal.md). Для выполнения действий в этом пошаговом руководстве можно использовать уровень "Бесплатный". 
...
### <a name="get-an-admin-api-key-and-url-for-azure-search"></a>Получение ключа API и URL-адреса конечной точки для администрирования Поиска Azure
...
В разделе "Заголовки" присвойте параметру Content-type значение `application/json`, а в параметре `api-key` сохраните значение ключа администрирования для службы Поиск Azure. Единожды выполнив настройку заголовков, вы сможете применять их для всех запросов в этом руководстве.
...
В Поиске Azure обработка Искусственного интеллекта выполняется во время индексирования (или приема данных). В этой части пошагового руководства описано, как создать четыре объекта: источник данных, определение индекса, набор навыков и индексатор. 
...
[Индекс](https://docs.microsoft.com/rest/api/searchservice/create-index) предоставляет схему для создания физического выражения содержимого в инвертированных индексах и других конструкциях в Поиске Azure. Самым крупным компонентом индекса является коллекция полей, где указаны тип данных и атрибуты для определения содержимого и поведения в Поиске Azure.
...
1. Используйте запрос **GET** и указанный ниже URL-адрес, заменив в нем YOUR-SERVICE-NAME фактическим именем службы, чтобы найти вхождения термина или фразы. Этот запрос возвращает поле `content` и количество соответствующих документов.
   ...
   ![Выходные данные поля содержимого](media/cognitive-search-tutorial-blob/content-output.png "Content field output")
...
1. Второй запрос вернет несколько новых полей, созданных конвейером (persons, organizations, locations, languageCode). Для краткости мы не используем поле keyPhrases, но если вам нужны эти значения, добавьте его.
   ...
   ![Выходные данные конвейера](media/cognitive-search-tutorial-blob/pipeline-output.png "Pipeline output")
...
1. Чтобы изучить возможности этих полей, добавьте параметр аспекта, который будет возвращать агрегированные соответствующие документы по расположению.
   ...
   ![Выходные данные аспекта](media/cognitive-search-tutorial-blob/facet-output.png "Facet output")
