...
1. В качестве расположения выберите один из [поддерживаемых регионов](https://docs.microsoft.com/en-us/azure/search/cognitive-search-quickstart-blob#supported-regions) для функции "Когнитивный поиск".
...
> [!TIP]
> Теперь, прежде чем проделать большой объем работы, самое время убедиться, что служба поиска работает в одном из поддерживаемых расположений, в которых предоставляется предварительная версия функции: центрально-южная часть США или Западная Европа.
