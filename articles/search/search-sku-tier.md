...
ms.locfileid: "48785313"
---
# <a name="choose-a-pricing-tier-for-azure-search"></a>Выбор ценовой категории службы "Поиск Azure"

[Служба "Поиск Azure" подготавливается](search-create-service-portal.md) в ценовой категории или SKU, которые устанавливаются на все время существования службы. Доступны такие уровни: **Бесплатный**, **Базовый** и **Стандартный** (уровень **Стандартный** доступен для нескольких конфигураций и емкостей). Большинство клиентов начинают с **бесплатного** уровня для оценки, а затем переходят на уровень **Стандартный** для развертываний в среде разработки и рабочей среде. Вы можете ознакомиться со всеми краткими руководствами о **бесплатном** уровне, в том числе для ресурсоемкого когнитивного поиска. 
...
В службе "Поиск Azure" наиболее важное понятие при выставлении счетов — это *единица поиска* (ЕП). Так как работа службы "Поиск Azure" зависит от реплик и секций, нет смысла выставлять счет на основе лишь одного из этих компонентов. Вместо этого выставление счетов основано на всех частях. 

Единица поиска представляет собой продукт *реплик* и *секций*, используемых службой: **`(R X P = SU)`**.

Каждая служба имеет как минимум одну единицу поиска (одна реплика, умноженная на одну секцию). Максимальное количество единиц поиска для любой службы — 36. Это количество можно достичь несколькими способами, например: 6 секций x 6 реплик или 3 секции x 12 реплик. 

Чаще всего используется количество меньшее, чем общая емкость. Например, в службе с 3 репликами и 3 секциями выставляется счет за 9 единиц поиска. 
...
Чтобы снизить затраты, можно выбрать ценовую категорию с более низкой почасовой ставкой. Почасовые ставки на уровне S1 ниже, чем на уровнях S2 и S3. Вы можете подготовить службу, предназначенную для меньшего количества предполагаемых рабочих нагрузок. Если вам уже не хватает возможностей службы, создайте вторую службу большего уровня, перестройте свои индексы во второй службе, а затем удалите первую. Если вы выполняли планирование ресурсов для локальных серверов, вы знаете, что принято покупать больше, чтобы справиться с прогнозируемым ростом. Однако, используя облачную службу, вы можете эффективнее экономить средства, так как вы не будет ограничены конкретной покупкой. Вы всегда можете переключиться на более высокоуровневую службу, если текущей недостаточно.
