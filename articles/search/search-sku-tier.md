...
description: 'Служба поиска Azure может быть подготовлена в следующих SKU: Free, Basic, Standard и Standard доступны в различных конфигурациях ресурсов и уровнях емкости.'
services: search
author: HeidiSteen
manager: nitinme
tags: ''
...
Хотя все уровни, в том числе уровень Free, обычно предлагают функции четности, большие рабочие нагрузки могут требовать более высоких уровней. Например, [обогащение искусственного интеллекта с Cognitive Services](cognitive-search-concept-intro.md) имеет долгосрочные навыки, которые исполняются в бесплатной службе, если набор данных не является небольшим.
...
Дополнительные сведения о различных уровнях см. на [странице с ценами](https://azure.microsoft.com/pricing/details/search/), в разделе [ограничения службы в службе поиска Azure](search-limits-quotas-capacity.md) и на странице портала при подготовке службы.
...
В отличие от виртуальных машин или других ресурсов, которые можно "приостановить", чтобы избежать расходов, служба поиска Azure всегда доступна на оборудовании, выделенном для монопольного использования. Таким образом, создание службы — это оплачиваемое событие, которое запускается при создании службы и завершается при удалении службы. 

Минимальная стоимость — это первая единица поиска (одна реплика x одна секция). Это минимальное значение фиксируется во время существования службы, так как служба не может работать ни на что меньше этой конфигурации. Помимо минимума можно добавлять реплики и секции независимо друг от друга. Добавочное увеличение емкости с помощью реплик и секций увеличит счет на основе следующей формулы: [(количество реплик x секции x)](#search-units), где плата зависит от выбранной ценовой категории.
...
Использование [индексаторов службы поиска Azure](search-indexer-overview.md) может повлиять на выставление счетов в зависимости от расположения служб. При создании службы поиска Azure в том же регионе, в котором находятся данные, плата за исходящие данные может быть полностью устранена. Ниже приведены некоторые сведения на [странице цены](https://azure.microsoft.com/pricing/details/bandwidth/)на пропускную способность.

+ Корпорация Майкрософт не взимается за любые входящие данные в любой службе в Azure или на исходящие данные из службы поиска Azure.
+ В многосервисных решениях плата за данные, передаваемые по сети, не взимается, если все службы находятся в одном регионе.

Если службы находятся в разных регионах, плата взимается за исходящие данные. Эти расходы на самом деле не входят в счет поиска Azure. Они упомянуты здесь, потому что если вы используете данные или более обогащенные индексаторы для извлечения данных из разных регионов, вы увидите, что затраты отражены в общем счете.

### <a name="cognitive-search-ai-enrichment-with-cognitive-services"></a>Обогащенный Поиск в ИСКУССТВЕНном поиске с помощью Cognitive Services

Для [обогащения искусственного интеллекта с Cognitive Services](cognitive-search-concept-intro.md)следует запланировать [Подключение оплачиваемого ресурса Cognitive Services Azure](cognitive-search-attach-cognitive-services.md)в том же регионе, что и поиск Azure, на ценовой категории S0 для обработки с оплатой по мере использования. Нет фиксированных затрат, связанных с присоединением Cognitive Services. Вы платите только за необходимую обработку.

| Операция | Принцип выставления счетов |
|-----------|----------------|
| Взлом документов, извлечение текста | Бесплатный |
| Взлом документов, Извлечение изображений | Счета выставляются в соответствии с количеством образов, извлеченных из документов. В [конфигурации индексатора](https://docs.microsoft.com/rest/api/searchservice/create-indexer#indexer-parameters) **имажеактион** — это параметр, который запускает извлечение изображений. Если для **имажеактион** задано значение "нет" (по умолчанию), вы не будете получать оплату за извлечение изображений. Скорость извлечения изображений задокументирована на странице [сведений о ценах](https://azure.microsoft.com/pricing/details/search/) для поиска Azure.|
| [Предварительно собранные профессиональные навыки](cognitive-search-predefined-skills.md) | Счет выставляется по той же ставке, что и при выполнении задачи с помощью Cognitive Services напрямую. |
...
- Объедините все службы в одну группу ресурсов, например поиск Azure, Cognitive Services и любые другие службы Azure, используемые в решении. В портал Azure найдите группу ресурсов и используйте команды **управления затратами** для анализа фактических и прогнозируемых расходов.
...
Емкость и затраты на запуск службы связаны напрямую. Уровни накладывают ограничения на два уровня: хранилище и ресурсы. Следует подумать о обоих случаях, так как достигнуто максимально допустимое ограничение.
...
### <a name="step-1-develop-rough-estimates-by-using-the-free-tier"></a>Шаг 1.: Разрабатывайте приблизительные оценки с помощью уровня Free

Один из подходов к оценке емкости — начать с уровня Free. Помните, что бесплатная служба предлагает до трех индексов, 50 МБ хранилища и 2 минуты времени индексирования. Оценка предполагаемого размера индекса с этими ограничениями может быть непростой задачей. Ниже приведен подход, который можно выполнить.

+ [Создайте бесплатную службу](search-create-service-portal.md).
+ Подготовьте небольшой репрезентативный набор данных (например, 5 000 документов и размер выборки в 10 процентов).
+ [Создайте исходный индекс](search-create-index-portal.md) и запишите его размер на портале (например, 30 МБ).

Если образец является репрезентативным и 10 процентов от всего источника данных, индекс размером 30 МБ становится примерно 300 МБ, если все документы индексируются. Используя этот предварительный номер, вы можете вдвое больше вычислить бюджет для двух индексов (разработки и рабочей среды). Это дает общий объем в 600 МБ в требованиях к хранилищу. Это требование легко удовлетворяет базовому уровню, поэтому его можно запустить.

### <a name="step-2-develop-refined-estimates-by-using-a-billable-tier"></a>Шаг 2.: Разработка точных оценок с помощью оплачиваемого уровня

Некоторые клиенты предпочитают начать с выделенных ресурсов, которые могут поддерживать большую выборку и время обработки, а затем разрабатывать реалистичные оценки объема индексов, размера и запросов во время разработки. Изначально служба подготавливается на основе оценки наилучшего предположения. Затем, по мере развития проекта разработки, группы обычно узнают, находится ли существующая служба в рабочем окружении или превышает ее емкость для проектируемой рабочей нагрузки.
...
1. [Создайте службу на платном уровне](search-create-service-portal.md):
    ...
    + Started, at Basic или S1, если вы находитесь в начале кривой обучения.
...
1. [Отслеживайте хранилище, ограничения служб, объем запросов и задержку](search-monitor-usage.md) в портале. На портале отображаются запросы в секунду, регулируемые запросы и задержки поиска. Все эти значения помогут решить, выбран ли подходящий уровень. Кроме того, можно настроить глубокий мониторинг таких значений, как анализ с дополнительной информацией, включив [аналитику поискового трафика](search-traffic-analytics.md).
...
+ Узнайте, как создавать эффективные индексы и узнать, какие методы обновления имеют наименьшее воздействие. Используйте [аналитику поискового трафика](search-traffic-analytics.md) , чтобы получить подробные сведения о действиях запросов.
...
+ Помните, что единственным недостатком недоподготовки является то, что вам может потребоваться разорвать службу, если фактические требования больше ваших прогнозов. Чтобы избежать сбоев в работе службы, вы должны создать новую службу в одной и той же подписке на более высоком уровне и запустить ее параллельно, пока все приложения и запросы не будут направлены на новую конечную точку.

## <a name="next-steps"></a>Следующие шаги

Начните с уровня Free и создайте начальный индекс с помощью подмножества данных, чтобы понять его характеристики. Структура данных в службе поиска Azure является инвертированной структурой индекса. Размер и сложность инвертированного индекса определяются содержимым. Помните, что высокоизбыточное содержимое имеет тенденцию приводить к созданию индекса меньшего объема, чем более неоднородное содержимое. Поэтому характеристики содержимого, а не размер набора данных, определяют требования к хранению индекса.
