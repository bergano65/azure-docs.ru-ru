...
|*Реплик* | Экземпляры службы поиска, используемые главным образом для распределения операций запросов. Каждая реплика является одной копией индекса. При выделении трех реплик вы получите три копии индекса, доступные для обслуживания запросов запросов.|
