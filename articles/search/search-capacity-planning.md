...
> [!NOTE]
> Добавление дополнительных реплик или разделов увеличивает стоимость выполнения службы и может привести к небольшим изменениям в способе заказа результатов. Обязательно проверьте [калькулятор ценообразования,](https://azure.microsoft.com/pricing/calculator/) чтобы понять последствия выставления счетов добавления дополнительных узлов. Приведенная [ниже диаграмма](#chart) поможет вам перекрестно сопомнение количества единиц поиска, необходимых для определенной конфигурации. Для получения дополнительной информации о том, [Ordering results](search-pagination-page-layout.md#ordering-results)как дополнительные реплики влияют на обработку запросов, см.
