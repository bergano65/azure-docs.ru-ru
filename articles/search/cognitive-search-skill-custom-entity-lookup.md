...
description: Извлечение различных пользовательских сущностей из текста в конвейере поиска с помощью Когнитивный поиск. Этот навык сейчас находится на этапе общедоступной предварительной версии.
manager: nitinme
author: luiscabrer
ms.author: luisca
...
Этот навык не привязан к Cognitive Services API и может использоваться бесплатно в период действия предварительной версии. Однако вы все равно должны [присоединить ресурс Cognitive Services](./cognitive-search-attach-cognitive-services.md), чтобы переопределить дневное ограничение на обогащение. Ограничение ежедневно применяется для бесплатного доступа к Cognitive Services при доступе через Когнитивный поиск Azure.
...
|`globalDefaultCaseSensitive` | Используемых Значение, зависящее от регистра по умолчанию для навыка. Если `defaultCaseSensitive` значение сущности не указано, это значение станет `defaultCaseSensitive` значением для этой сущности. |
|`globalDefaultAccentSensitive` | Используемых Значение по умолчанию, учитывающее диакритические знаки для навыка. Если `defaultAccentSensitive` значение сущности не указано, это значение станет `defaultAccentSensitive` значением для этой сущности. |
|`globalDefaultFuzzyEditDistance` | Используемых Значение расстояния нечетких изменений по умолчанию для навыка. Если `defaultFuzzyEditDistance` значение сущности не указано, это значение станет `defaultFuzzyEditDistance` значением для этой сущности. |
...
| `languageCode`    | Необязательный элемент. По умолчанию — `"en"`.  |
...
Вы можете предоставить определение настраиваемых сущностей для поиска в файле Comma-Separated значение (CSV), указав путь к файлу и установив его в параметре навыка *ентитиесдефинитионури*  . Путь должен находиться в расположении HTTPS. Размер файла определения может доставлять до 10 МБ.
...
| `accentSensitive` | Используемых Значение по умолчанию — false. Логическое значение, указывающее, должны ли быть идентичны буквы с диакритическими знаками и без них, такие как «e» и «е». |
...
| `defaultCaseSensitive` | Используемых Изменяет значение чувствительности к регистру по умолчанию для этой сущности. Его можно использовать для изменения значения по умолчанию для всех псевдонимов caseSensitive. |
| `defaultAccentSensitive` | Используемых Изменяет значение чувствительности к диакритическим знакам по умолчанию для этой сущности. Его можно использовать для изменения значения по умолчанию для всех псевдонимов Акцентсенситиве значений.|
...
| `accentSensitive` | Используемых Действует так же, как параметр корневой сущности "Акцентсенситиве" выше, но применяется только к этому псевдониму. |
