...
|defaultLanguageCode |    Используемых Код языка входного текста, используемого для маркировки и отделения входного текста. Поддерживаются следующие языки: `da, de, en, es, fi, fr, it, ko, pt`. Значение по умолчанию —`en`Английский (). При передаче формата languagecode-countrycode используется только часть languagecode формата.  |
...
| псевдонимы | Используемых Массив сложных объектов, которые можно использовать для указания альтернативного написания или синонимов имени корневой сущности. |
