---
title: Советы по устранению неполадок с когнитивным поиском — Поиск Azure
...
```
## <a name="tip-4-looking-at-enriched-documents-under-the-hood"></a>Совет 4. Обращайте внимание на скрытые обогащенные документы 
...
Если проблема связана с размером файла, вы увидите ошибку, подобную этой: "Имя файла \<большого двоичного объекта >" имеет \<размер > байт, что превышает максимальный размер для извлечения документа для текущего уровня служб. " Дополнительные сведения об ограничениях индексатора см. в разделе [Ограничения служб](search-limits-quotas-capacity.md).
...
## <a name="tip-6-extend-processing-beyond-maximum-run-time-24-hour-window"></a>Совет 6. Увеличьте время выполнения обработки за пределы максимально допустимого значения (24-часовое окно)
...
В таком случае индексатор будет по расписанию возобновлять обработку последнего известного приемлемого документа. При использовании повторяющегося расписания индексатор может выполнять работу над необработанными изображением в течение нескольких часов или дней, пока все они не будут обработаны. Дополнительные сведения о синтаксисе расписания см. в разделе [Шаг 3. Create-a-index](search-howto-indexing-azure-blob-storage.md#step-3-create-an-indexer) или Узнайте [, как планировать индексаторы для поиска Azure](search-howto-schedule-indexers.md).
...
## <a name="tip-7-increase-indexing-throughput"></a>Совет 7. Увеличьте пропускную способность индексирования
