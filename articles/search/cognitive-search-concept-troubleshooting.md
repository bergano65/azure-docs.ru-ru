---
title: Советы по проектированию искусственного интеллекта
titleSuffix: Azure Cognitive Search
description: Советы и устранение неполадок для настройки конвейеров обогащения искусственного интеллекта в Azure Когнитивный поиск.
manager: nitinme
author: luiscabrer
ms.author: luisca
...
В этой статье содержится список советов и рекомендаций, которые помогут вам перейти к работе с возможностями обогащения искусственного интеллекта в Azure Когнитивный поиск. 

Если вы еще не сделали этого, [Познакомьтесь с руководством по вызову API-интерфейсов обогащения искусственного интеллекта](cognitive-search-quickstart-blob.md) , чтобы попрактиковаться в применении обогащений искусственного интеллекта к источнику данных большого двоичного объекта.
...
- Строка подключения должна быть указана правильно. При создании токенов SAS следует использовать формат, ожидаемый Azure Когнитивный поиск. Сведения о разных поддерживаемых форматах см. в руководстве по [определению учетных данных](
...
```
> [!NOTE]
> Рекомендуется задать для параметра Максфаиледитемс, Максфаиледитемспербатч значение 0 для рабочих нагрузок в рабочей среде.
...
## <a name="tip-5-looking-at-enriched-documents-under-the-hood"></a>Совет 5. взгляд на обогащенные документы внутри 
...
## <a name="tip-6-expected-content-fails-to-appear"></a>Совет 6. ожидаемое содержимое не отображается
...
Если проблема связана с размером файла, может появиться следующее сообщение об ошибке: "BLOB-объект \<file-name> " имеет размер в \<file-size> байтах, что превышает максимальный размер для извлечения документа для текущего уровня служб ". Дополнительные сведения об ограничениях индексатора см. в разделе [Ограничения служб](search-limits-quotas-capacity.md).
...
## <a name="tip-7-extend-processing-beyond-maximum-run-time-24-hour-window"></a>Совет 7. расширение обработки за пределами максимального времени выполнения (24-часовое окно)
...
В таком случае индексатор будет по расписанию возобновлять обработку последнего известного приемлемого документа. При использовании повторяющегося расписания индексатор может выполнять работу над необработанными изображением в течение нескольких часов или дней, пока все они не будут обработаны. Дополнительные сведения о синтаксисе расписания см. в разделе [планирование индексатора](search-howto-schedule-indexers.md).
