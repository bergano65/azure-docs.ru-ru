...
Если проблема связана с размером файла, может появиться следующее сообщение об ошибке: "имя файла большого двоичного объекта \<>" имеет размер \<файла> байт, что превышает максимальный размер для извлечения документа для текущего уровня служб. " Дополнительные сведения об ограничениях индексатора см. в разделе [Ограничения служб](search-limits-quotas-capacity.md).
