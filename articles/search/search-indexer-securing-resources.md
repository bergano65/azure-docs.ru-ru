...
- [Конечные точки службы](../virtual-network/virtual-network-service-endpoints-overview.md) не будут поддерживаться ни для одного ресурса Azure.
...
- [SQL Server на виртуальных машинах IaaS](./search-howto-connecting-azure-sql-iaas-to-azure-search-using-indexers.md#restrict-access-to-the-azure-cognitive-search)
...
Затем клиент должен располагать этот запрос на своем защищенном ресурсе и одобрить его. Как правило, это можно сделать либо с помощью портала, либо с помощью [REST API](/rest/api/virtualnetwork/privatelinkservices/updateprivateendpointconnection).
