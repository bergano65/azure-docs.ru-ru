...
При создании индекса аспекты включаются в зависимости от каждого поля, присвоив `facetable` атрибуту значение. `true` Как правило, `filterable` атрибут должен быть задан `true` для таких полей, чтобы приложение поиска может фильтровать по этим полям на основе аспектов, выбранных конечным пользователем. 
...
> [!Note]
> Это определение индекса копируется из [создания индекса когнитивный Поиск Azure с помощью REST API](https://docs.microsoft.com/azure/search/search-create-index-rest-api). Оно идентично, за исключением поверхностных различий в определениях полей. Атрибуты `filterable` и `facetable` `category`явно добавляются в поля, `tags`, `parkingIncluded` `smokingAllowed`, и `rating` . На практике `filterable` и `facetable` будут включены по умолчанию для этих полей при использовании REST API. При использовании пакета SDK для .NET эти атрибуты должны быть включены явным образом.
