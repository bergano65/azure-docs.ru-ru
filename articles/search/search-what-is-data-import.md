...
### <a name="indexing-actions-upload-merge-uploadormerge-delete"></a>Индексирование действий: отправка, объединение, uploadOrMerge, удаление
...
Каждый объект JSON в массиве value представляет документ для индексирования, Каждый из этих объектов содержит ключ документа и указывает нужное действие индексирования (отправка, объединение, удаление). В зависимости от выбранной операции для каждого документа будут включены только определенные поля.
