...
ms.locfileid: "61343348"
---
# <a name="custom-web-api-skill"></a>Пользовательский навык веб-API

**Пользовательский навык веб-API** позволяет расширять когнитивный поиск, вызывая конечную точку веб-API, обеспечивающую пользовательские операции. Аналогично встроенным навыкам **пользовательский навык веб-API** имеет входные и выходные данные. В зависимости от входных данных веб-API получает полезные данные JSON при работе индексатора и выводит полезные данные JSON в качестве ответа вместе с кодом успешного выполнения. В ответе должны содержаться выходные данные, указанные в вашем пользовательском навыке. Любой другой ответ считается ошибкой, и никакие обогащения не выполняются.
...
|--------------------|-------------|
| универсальный код ресурса | Универсальный код ресурса (URI) веб-API, в который будут отправляться полезные данные _JSON_. Допускается только схема URI **HTTPS**. |
...
Выходные данные соответствуют ответу, возвращенному от веб-API. Веб-API должен возвращать только полезные данные _JSON_ (можно проверить, просмотрев заголовок ответа `Content-Type`), которые должны соответствовать следующим ограничениям:
...
+ [Пример: создание пользовательского навыка с помощью API перевода текста](cognitive-search-create-custom-skill-example.md)