...
"Output" соответствует ответу, возвращенному веб-API. Веб-API должен возвращать полезные данные _JSON_ (проверенные путем просмотра заголовка `Content-Type` ответа) и должны соответствовать следующим ограничениям:
