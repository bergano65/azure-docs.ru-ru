---
title: Поиск содержимого таблицы Azure
titleSuffix: Azure Cognitive Search
description: Узнайте, как индексировать данные, хранящиеся в хранилище таблицы Azure, с помощью индекса когнитивного поиска Azure.
manager: nitinme
author: mgottein
ms.author: magottei
ms.devlang: rest-api
...
---
# <a name="how-to-index-tables-from-azure-table-storage-with-azure-cognitive-search"></a>Как индексировать таблицы из хранилища таблицы Azure с помощью когнитивного поиска Azure

В этой статье показано, как использовать Azure Cognitive Search для индексирования данных, хранящихся в хранилище таблицы Azure.
...
Источник данных определяет, какие данные индексировать, учетные данные, необходимые для доступа к данным, и политики, которые позволяют Azure Cognitive Search эффективно идентифицировать изменения в данных.
...
- **контейнер** устанавливает название таблицы и дополнительный запрос.
...
- **Строка подключения к учетной**записи с полным доступом: `DefaultEndpointsProtocol=https;AccountName=<your storage account>;AccountKey=<your account key>` Вы можете получить строку подключения с портала Azure, перейдя в**ключи** **к настройкам** >  **лезвия** > учетной записи хранилища (для классических учетных записей хранения) или в **настройках** > **доступа** к настройкам (для учетных записей управления ресурсами Azure.
- **Учетная запись хранения общего**доступа строка подключения к подписи: `TableEndpoint=https://<your account>.table.core.windows.net/;SharedAccessSignature=?sv=2016-05-31&sig=<the signature>&spr=https&se=<the validity end time>&srt=co&ss=t&sp=rl` общая подпись доступа должна иметь список и читать разрешения на контейнерах (таблицы в данном случае) и объектов (строки таблицы).
-  **Таблица общей подписью доступа**: `ContainerSharedAccessUri=https://<your storage account>.table.core.windows.net/<table name>?tn=<table name>&sv=2016-05-31&sig=<the signature>&se=<the validity end time>&sp=r` Общая подпись доступа должна иметь разрешения запроса (читай) на столе.
...
Этот индексатор выполняется каждые два часа. (Интервал расписания установлен на "PT2H".) Чтобы запустить индексер каждые 30 минут, установите интервал до "PT30M". Самый короткий интервал, который можно задать, составляет пять минут. Расписание является необязательным. Если оно не указано, то индексатор выполняется только один раз при его создании. Однако индексатор можно запустить по запросу в любое время.   
...
Иногда имена полей в существующем индексе отличаются от имен свойств в таблице. Для сопоставления имен свойств таблицы с именами полей в индексе поиска можно использовать сопоставления полей. Чтобы узнать больше о полях карт, см. [Полевые отображения поля Azure Cognitive Searchindexer, которые преодолевает различия между источниками данных и индексами поиска.](search-indexer-field-mappings.md)

## <a name="handle-document-keys"></a>Обработка ключей документа
В Azure Cognitive Search ключ документа однозначно идентифицирует документ. Каждый индекс поиска должен содержать только одно поле ключа типа `Edm.String`. Поле ключа является обязательным для каждого документа, который добавляется к индексу. (Фактически, это единственное обязательное для заполнения поле.)

Поскольку строки таблиц имеют сложный ключ, Azure `Key` Cognitive Search генерирует синтетическое поле, называемое соединением ключевых значений ключа раздела и строки. Например, если PartitionKey равен `PK1` и RowKey равен `RK1`, то значение поля `Key` равно `PK1RK1`.
...
По умолчанию Azure Cognitive Search использует `Timestamp >= HighWaterMarkValue`следующий фильтр запросов: . Поскольку в таблицах Azure отсутствует вторичный индекс в поле `Timestamp`, запрос такого типа требует просмотра всей таблицы и поэтому выполняется медленно для больших таблиц.
