...
> [!NOTE]
> По умолчанию большие двоичные объекты со структурированным содержимым, например JSON- или CSV-файлы, индексируются как один блок текста. Если вы хотите индексировать большие двоичные объекты JSON и CSV структурированным способом, см. в разделе [BLOB-объектов JSON, индексирование](search-howto-index-json-blobs.md) и [большие двоичные объекты для индексирования CSV](search-howto-index-csv-blobs.md) Дополнительные сведения.
>
...
> [!NOTE]
> Поиск Azure ограничивает количество извлекаемого текста в зависимости от ценовой категории: для уровня "Бесплатный" — 32 000 знаков, для ценовой категории "Базовый" — 64 000 знаков, а также 4 миллиона для категорий "Стандартный", "Стандартный S2" и "Стандартный S3". Предупреждение об усеченных документах отобразится в возвращенном состоянии индексатора.  
...
* Стандартные свойства метаданных большого двоичного объекта извлекаются в указанные ниже поля.
  ...
  * **метаданные\_хранения\_sas\_маркера** (Edm.String) — временный маркер, который может использоваться с [пользовательские методики](cognitive-search-custom-skill-interface.md) для получения доступа к BLOB-объекта. Этот маркер sas не должна должно быть сохранено для последующего использования, так как он может истечь срок действия.
