...
* Стандартные свойства метаданных большого двоичного объекта извлекаются в указанные ниже поля.
  ...
  * **метаданные\_хранения\_sas\_маркера** (Edm.String) — временный маркер SAS, который может использоваться с [пользовательские методики](cognitive-search-custom-skill-interface.md) для получения доступа к BLOB-объекта. Не следует хранить этот токен для последующего использования, так как он может срок действия.
