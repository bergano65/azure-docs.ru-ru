...
Следующий элемент в наборе навыков — массив навыков. Каждый навык можно представить в виде примитива обогащения. Каждый навык выполняет небольшую задачу в конвейере обогащения. Каждый из них принимает один или несколько элементов входных данных и возвращает выходные данные. Следующие несколько разделов сконцентрироваться на способах указания встроенных и пользовательских навыки, связывание навыки через входные и выходные ссылки. Входные данные могут поступать из исходных данных или из другого навыка. Выходные данные можно сопоставить с полем в индексе поиска или использовать в качестве входных для нижестоящего навыка.

## <a name="add-built-in-skills"></a>Встроенные новые знания и навыки

Давайте рассмотрим первый навыка, который является встроенной [навыков распознавания сущностей](cognitive-search-skill-entity-recognition.md):
...
* У каждого навыка встроенные `odata.type`, `input`, и `output` свойства. Свойства, связанные с определенным навыком, предоставляют дополнительные сведения, применимые к этому навыку. При распознавании сущностей предварительно обученной моделью `categories` представляет одну из сущностей фиксированного набора типов сущностей.

* Каждый навык должен иметь ```"context"```. Контекст представляет уровень, на котором выполняются операции. Выше навыка контекст является весь документ, это означает, что навык распознавания сущностей вызывается для каждого документа. Также на этом уровне создаются выходные данные. В частности, создаются ```"organizations"``` в качестве элемента ```"/document"```. В нижестоящих навыках можно ссылаться на только что созданную информацию как на ```"/document/organizations"```.  Если поле ```"context"``` явно не задано, контекстом по умолчанию является документ.

* Навык имеет один входной параметр text, исходный входной параметр для которого задан как ```"/document/content"```. Навык (распознавание сущностей) работает с *содержимого* создать поле каждого документа, предназначенного для стандартных индексатор BLOB-объектов Azure. 
...
Это определение представляет собой [пользовательский навык](cognitive-search-custom-skill-web-api.md), который вызывает веб-API в процессе обогащения. Для каждой организации, идентифицируемый распознавание сущностей этот навык вызывает веб-API, чтобы найти описание этой организации. Оркестрация времени вызова веб-API и включения в поток полученной информации обрабатывается внутренним модулем обогащения. При этом инициализация, необходимая для вызова этого настраиваемого API, должна быть задана в JSON (например, uri, httpHeaders и ожидаемые входные данные). Инструкции по созданию настраиваемого веб-API для конвейера обогащения см. в руководстве по [определению настраиваемого интерфейса](cognitive-search-custom-skill-interface.md).
...
До сих пор эта структура была только для внутреннего только памяти и используется только в индексы службы поиска Azure. Добавление базы знаний предоставляет способ сохранения фигурной усовершенствования для использования за пределами поиска.
...
Вы можете сохранить Обогащенные документы как таблицы с иерархических связей, которые сохраняются или в виде документов JSON в хранилище BLOB-объектов. Выходные данные из любого из навыков в квалификации могут поступать в качестве входных данных для проекции. Если вам нужны, чтобы проецировать данные на уровень конкретного фигуры, обновленный [shaper навыков](cognitive-search-skill-shaper.md) теперь можно моделировать сложные типы для использования. 
