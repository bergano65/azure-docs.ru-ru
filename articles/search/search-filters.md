...
В следующих примерах показаны несколько конструктивных шаблонов для сценариев фильтра. Дополнительные примеры можно найти в разделе [Примеры OData](https://docs.microsoft.com/rest/api/searchservice/odata-expression-syntax-for-azure-search#filter-examples).
...
Перестроение отдельных полей может не оказывать большого влияния и требовать только слияния, при котором имеющийся ключ документа и связанные значения отправляются в индекс, а остальная часть каждого документа остается нетронутой. При возникновении потребности перестроения см. в разделе [индексирования действия (отправка, merge, mergeOrUpload, удалить)](search-what-is-data-import.md#indexing-actions) для получения списка параметров.
