...
description: Узнайте, как импортировать данные из нескольких источников данных в один индекс службы "Когнитивный поиск Azure".
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
ms.locfileid: "72786480"
---
# <a name="c-tutorial-combine-data-from-multiple-data-sources-in-one-azure-cognitive-search-index"></a>Руководство по C#. Объединение данных из нескольких источников данных в один индекс службы "Когнитивный поиск Azure"
...
1. Откройте Visual Studio и установите пакет NuGet для службы "Когнитивный поиск Microsoft Azure", если он еще не установлен. В меню **Средства** выберите **Диспетчер пакетов NuGet**, а затем **Manage NuGet Packages for Solution...** (Управление пакетами NuGet для решения...). На вкладке **Просмотреть** найдите и установите **Microsoft.Azure.Search** (версия 9.0.1 или более поздняя). Для завершения установки нужно пройти серию дополнительных диалоговых окон.
...
1. Щелкните "Добавить контейнер" в строке меню. Выберите "Создать новую базу данных" и укажите для нее имя **hotel-rooms-db**. В качестве имени коллекции введите **hotels**, а имени ключа — **/HotelId**. Щелкните **ОК**, чтобы создать базу данных и контейнер.
   ...
   ![Добавление контейнера Azure Cosmos DB](media/tutorial-multiple-data-sources/cosmos-add-container.png "Добавление контейнера Azure Cosmos DB")
...
1. На панели отправки щелкните кнопку со значком папки, а затем перейдите к файлу **cosmosdb/HotelsDataSubset_CosmosDb.json** в папке проекта. Щелкните **ОК**, чтобы начать отправку.
   ...
   ![Выбор файла для отправки](media/tutorial-multiple-data-sources/cosmos-upload2.png "Выбор файла для передачи")
...
* создает индекс службы "Когнитивный поиск Azure" на основе структуры данных в классе C# Hotel (который также ссылается на классы Address и Room);
...
Может потребоваться дополнить структурированные данные индекса, полученные из одного источника данных, интеллектуально обогащенными данными из неструктурированных BLOB-объектов или содержимым для полнотекстового поиска. Следующий учебник демонстрируют, как применить Cognitive Services совместно со службой "Когнитивный поиск Azure", используя пакет SDK для .NET.

> [!div class="nextstepaction"]
> [Вызов API-интерфейсов Cognitive Services в конвейере индексирования службы "Когнитивный поиск Azure"](cognitive-search-tutorial-blob-dotnet.md)
