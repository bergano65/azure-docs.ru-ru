...
Задайте `enableReprocessing` свойство для управления обработкой входящих документов, уже представленных в кэше. Если `true` задано значение (по умолчанию), документы, которые уже находятся в кэше, повторно обрабатываются при перезапуске индексатора при условии, что обновление навыка влияет на этот документ. 

При `false`этом существующие документы не обрабатываются повторно, что позволяет эффективно расставлять приоритеты для нового входящего содержимого поверх существующего. Значение должно быть `enableReprocessing` задано `false` на временной основе. Чтобы обеспечить согласованность в совокупности, `enableReprocessing` должна быть `true` большая часть времени, гарантирующая, что все документы, как новые, так и существующие, действительны в соответствии с текущим определением набора навыков.
...
Если известно, что изменение набора навыков действительно является недоступным, следует переопределить оценку набора навыков, задав для `disableCacheReprocessingChangeDetection` `true`параметра значение:
