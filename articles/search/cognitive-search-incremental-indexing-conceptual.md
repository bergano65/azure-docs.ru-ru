...
+ Некоторые Индексаторы получают данные через запросы. Для запросов, извлекающих данные, [Обновление источника данных](https://docs.microsoft.com/rest/api/searchservice/update-data-source) поддерживает новый параметр `ignoreResetRequirement`запроса, который должен быть установлен в `true` если действие обновления не должно сделать кэш недействительным.
