...
1.  > Перейдите на [**портал**](https://apps.dev.microsoft.com) >  регистрации приложений**приложение для****добавления**приложения.
...
Чтобы уменьшить задержки в сети, при необходимости можно кэшировать ассоциации пользователей и групп, чтобы при выдаче запроса на поиск группы возвращались из кэша и не приходилось обращаться в AAD. Вы можете использовать [API пакетной службы AAD](https://developer.microsoft.com/graph/docs/concepts/json_batching) для отправки одного HTTP-запроса с несколькими пользователями и создания кэша.
