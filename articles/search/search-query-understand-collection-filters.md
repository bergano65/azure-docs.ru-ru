...
Вы можете получить Гостиницы, где одна комната — Deluxe, а другая комната упоминается в описании. Например, следующий `Id` документ `1` должен соответствовать запросу:
...
Таким образом, в отличие от приведенного выше фильтра, в котором по сути говорится о `Type` совпадении документов, в которых комната имеет значение "Deluxe `BaseRate` Room", и **что в этой комнате** меньше 100 ", поисковый запрос говорит «найти документы, `Rooms/Description` где `Rooms/Type` есть термин «Deluxe» и фразу «представление города». Нет концепции отдельных комнат, поля которых можно связать в последнем случае.
...
Например, рассмотрим поле коллекции строк, которое можно фильтровать, `seasons` например в индексе для Интернет-магазина. Некоторые документы, отправленные по этому индексу, могут выглядеть следующим образом:
...
Эта структура данных предназначена для ответа на один вопрос с высокой скоростью: в каком документе отображается указанный термин? Ответ на этот вопрос работает более похоже на проверку обычного равенства, чем на цикл по коллекции. По сути, именно поэтому для коллекций строк Когнитивный поиск Azure допускает `eq` только в качестве оператора сравнения внутри лямбда-выражения для. `any`
...
При фильтрации по `lt`коллекциям типов данных, которые поддерживают операторы, `gt` `le`, и `ge` , такие как `Collection(Edm.Int32)` , например, могут использоваться более широкий спектр выражений. `or` В частности `any`, можно использовать `and` и в, пока Базовые выражения сравнения объединяются в **диапазоны сравнения** с помощью `and`, которые затем объединяются с помощью. `or` Эта структура логических выражений называется [Дисжунктиве обычной формой (ДНФ)](https://en.wikipedia.org/wiki/Disjunctive_normal_form), которая в противном случае называется or and. И наоборот, лямбда-выражения для `all` этих типов данных должны быть в [был соединительным обычной форме (cnf)](https://en.wikipedia.org/wiki/Conjunctive_normal_form), в противном случае известной как "and of or". Azure Когнитивный поиск позволяет выполнять такие сравнения диапазонов, так как позволяет эффективно исполнять их с помощью инвертированных индексов, точно так же, как это может сделать быстрый поиск строк.

В сводке ниже приведены правила, которые можно использовать в лямбда-выражении.

- Внутри `any`, *положительные проверки* всегда разрешены, такие как равенство, сравнения диапазонов `geo.intersects`, `geo.distance` или по `lt` сравнению с or `le` (Представьте «близкость» как равенство, когда дело доходит до проверки расстояния).
