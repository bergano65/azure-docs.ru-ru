...
> [!NOTE]  
>  В целях безопасности в URI запроса не рекомендуется передавать конфиденциальные данные, например `api-key`. По этой причине Azure Когнитивный поиск принимает только ключ запроса в качестве `api-key` в строке запроса, и следует избегать этого, если содержимое индекса не должно быть общедоступным. Как правило, мы рекомендуем передавать `api-key` в заголовке запроса.  
...
> [!Note]
> Пример кода, в котором показано использование ключа запроса, можно найти в разделе [запрос индекса когнитивный поиск C#Azure в ](search-query-dotnet.md).
