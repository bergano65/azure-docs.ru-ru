...
При запуске Тексконв. exe с `--help` параметром будут перечислены все доступные параметры. Кроме того, Тексконв выводит используемые параметры при ее выполнении, чтобы понять, что именно происходит. Дополнительные сведения см. в этих выходных данных.
...
Чтобы собрать выходные данные из нескольких входных файлов, укажите каждый входной файл с `-in` помощью параметра с увеличивающимся номером:

```cmd
-in0 D:/img0.jpg -in1 D:/img1.jpg -in2 D:/img2.jpg ...
```

При `-right`сборке кубической карты из 2D-текстур можно также использовать, `-left`, `-top`, `-bottom` `-front`, `-back` или `-px`, `-nx` `-py` `-ny`,,, `-pz`,. `-nz`
