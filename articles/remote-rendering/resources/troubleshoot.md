...
**Модель превышает ограничения выбранной виртуальной машины, в частности максимальное количество многоугольников:**
...
При попытке компиляции образцов Unity (краткое руководство, Шовкасеапп,..) для HoloLens 2 мы наблюдали ложные сбои. Visual Studio выдаст о невозможности скопировать некоторые файлы, хотя они там находятся. Если вы столкнулись с этой проблемой:
* Удалите все временные файлы Unity из проекта и повторите попытку.
...
* Если это не поможет, то это может повлиять на шаг копирования. Чтобы настроить исключение, выполните команду реестра из командной строки (требуются права администратора):
    ```cmd
