...
В этом коде для получения маркера доступа мы используем [поток кода устройства](../../../../active-directory/develop/v2-oauth2-device-code.md). Этот поток позволяет пользователю войти в свою учетную запись Azure на компьютере или мобильном устройстве, после чего приложению HoloLens будет возвращен полученный в итоге маркер.
