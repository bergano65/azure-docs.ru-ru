...
Для Удаленной отрисовки Azure требуются предварительно обработанные трехмерные модели, а для создания виртуальной машины и загрузки модели для отрисовки Azure требуется несколько минут. Для максимального ускорения и непрерывности этого процесса необходимо заранее подготовить данные трехмерной модели и сеанс Удаленной отрисовки Azure.
...
Чтобы сократить время запуска, проще всего переместить создание и инициализацию виртуальной машины на как можно более ранний этап в рабочем процессе пользователя. Одна из стратегий — инициализировать сеанс, как только становится известно, что потребуется сеанс Удаленной отрисовки Azure. Часто это случается, когда пользователь начинает отправлять трехмерную модель в Хранилище BLOB-объектов Azure для использования с Удаленной отрисовкой Azure. В этом случае создание сеанса и подготовку виртуальной машины можно начать одновременно с передачей трехмерной модели таким образом, чтобы оба рабочих потока выполнялись параллельно.
...
В зависимости от варианта использования вам, скорее всего, потребуется определить и, возможно, поддерживать нужный уровень виртуальной машины Удаленной отрисовки Azure (уровня "Стандартный" или "Премиум") для каждого 3D-ресурса или группы ресурсов, которые будут просматриваться вместе в одном сеансе.  
