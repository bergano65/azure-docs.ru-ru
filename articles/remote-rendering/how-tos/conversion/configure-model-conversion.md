...
Если это не так, для этого параметра следует задать значение "SingleSided". Дополнительные сведения см. в разделе [ :::no-loc text="single sided"::: Подготовка к просмотру](../../overview/features/single-sided-rendering.md).
...
| Компонент :::no-loc text="Vertex"::: | Поддерживаемые форматы (полужирным шрифтом выделен формат по умолчанию) |
|:-----------------|:------------------|
