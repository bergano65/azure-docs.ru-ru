...
`box_materials_override.json`Файл помещается во входной контейнер, а `box.ConversionSettings.json` добавляется рядом с параметром `box.fbx` , который указывает преобразованию, где найти файл переопределения (см. раздел [Настройка преобразования модели](configure-model-conversion.md)):
