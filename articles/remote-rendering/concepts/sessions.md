...
Затем Удаленная отрисовка Azure пытается найти сервер, на котором можно разместить ваш сеанс. Для этого поиска используются два параметра. Во-первых, будут резервироваться серверы только в вашем [регионе](../reference/regions.md). Это обусловлено тем, что задержка в сети между регионами может быть слишком высокой, чтобы гарантировать приемлемый результат. Второй фактор — требуемый *размер*, который вы указали. В каждом регионе существует ограниченное количество серверов, которые могут выполнить запрос для размера ценовой категории *Стандартный* или *Премиум*. Следовательно, если все серверы запрошенного размера в вашем регионе в настоящее время используются, создание сеанса завершится ошибкой. Причину сбоя [можно запросить](../how-tos/session-rest-api.md#get-sessions-properties).

> [!IMPORTANT]
> Если вы запросили размер виртуальной машины ценовой категории *Стандартный* и запрос завершился ошибкой из-за высокой загрузки, это не означает, что запрос сервера ценовой категории *Премиум* тоже завершится ошибкой. Поэтому, если вам это подходит, можно попробовать запросить виртуальную машину ценовой категории *Премиум*.
