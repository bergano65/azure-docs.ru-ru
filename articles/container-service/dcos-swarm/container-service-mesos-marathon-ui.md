...
С помощью туннель Secure Shell (SSH) [установить](../container-service-connect.md), перейдите к http: \/ /localhost /. Загрузится веб-интерфейс DC/OS, а затем вы увидите сведения о кластере, в том числе об используемых ресурсах, активных агентах и запущенных службах.
...
Чтобы просмотреть пользовательский Интерфейс Marathon, перейдите к http: \/ /localhost/marathon. На этом экране можно запустить новый контейнер или другие приложения в кластере DC/OS службы контейнеров Azure. Здесь вы также можете просмотреть информацию о работе контейнеров и приложений.  
...
При переключении обратно в DC/OS веб-интерфейса (http:\//localhost/), вы увидите, что задача (в данном случае контейнер в формате Docker) выполняется в кластере DC/OS.
