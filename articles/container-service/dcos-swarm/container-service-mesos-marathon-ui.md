...
С безопасной оболочки (SSH) туннель\/ [создан](../container-service-connect.md), просматривать http: /localhost/. Загрузится веб-интерфейс DC/OS, а затем вы увидите сведения о кластере, в том числе об используемых ресурсах, активных агентах и запущенных службах.
...
Чтобы увидеть веб-удочку\/Марафона, просмотрите http:/localhost/marathon. На этом экране можно запустить новый контейнер или другие приложения в кластере DC/OS службы контейнеров Azure. Здесь вы также можете просмотреть информацию о работе контейнеров и приложений.  
...
При переключении обратно в веб-uI\/DC/OS (http:/localhost/), вы видите, что задача (в данном случае контейнер с форматом Docker) работает в кластере DC/OS.
