...
После подключения к кластеру службы контейнеров Azure, можно обращаться из DC/OS и соответствующим интерфейсам REST API http:\//localhost:local-порт. В этом документе для примера используется туннелирование через порт 80. Например, с конечной точкой Marathon можно связаться по адресу идентификаторы URI начиная с http: \/ /localhost/marathon/v2 /. 
...
> [!NOTE]
> URI — http: \/ /localhost/marathon/v2/apps/следуют идентификатор масштабируемого приложения. Если вы используете пример Nginx, которое предоставляется здесь URI будет http:\//localhost/marathon/v2/apps/nginx.
...
> [!NOTE]
> URI — http: \/ /localhost/marathon/v2/apps/следуют идентификатор масштабируемого приложения. Если вы используете пример Nginx, описанный здесь, URI будет http:\//localhost/marathon/v2/apps/nginx.
