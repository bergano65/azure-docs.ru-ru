...
[Пример 1. Создание сети периметра (также называется DMZ, демилитаризованной зоной или промежуточной подсетью) для защиты приложений с помощью групп безопасности сети (Nsg).](#example-1-build-a-perimeter-network-to-help-protect-applications-with-nsgs)</br>
[Пример 2. Создание сети периметра для защиты приложений с брандмауэром и группами безопасности сети.](#example-2-build-a-perimeter-network-to-help-protect-applications-with-a-firewall-and-nsgs)</br>
[Пример 3. Создание сети периметра для защиты сетей с брандмауэром, определяемый пользователем маршрут (UDR) и группы безопасности сети.](#example-3-build-a-perimeter-network-to-help-protect-networks-with-a-firewall-and-udr-and-nsg)</br>
[Пример 4. Добавление гибридного подключения "сеть сеть", виртуальные устройства виртуальная частная сеть (VPN).](#example-4-add-a-hybrid-connection-with-a-site-to-site-virtual-appliance-vpn)</br>
[Пример 5. Добавление гибридного подключения с сайтами, VPN-шлюза Azure.](#example-5-add-a-hybrid-connection-with-a-site-to-site-azure-vpn-gateway)</br>
[Пример 6. Добавление гибридного подключения с помощью ExpressRoute.](#example-6-add-a-hybrid-connection-with-expressroute)</br>
...
* **Распределенное подключение**: Клиенты могут установить распределенное подключение между виртуальной сети и несколькими локальными сайтами или другими виртуальными сетями в Azure. Чтобы создать подключение, клиенты могут использовать пиринг виртуальных сетей, VPN-шлюзы Azure, виртуальные сетевые устройства сторонних производителей или ExpressRoute. Azure поддерживает VPN типа "сеть — сеть" со стандартными протоколами IPsec/IKE, а также службу частного соединения ExpressRoute.
...
* **Сетевые виртуальные устройства** в Azure Marketplace: Устройства безопасности, такие как брандмауэры, подсистемы балансировки нагрузки и IDS/IPS, доступны в Azure Marketplace и коллекции образов виртуальных Машин. Клиенты могут развернуть эти устройства в своих виртуальных сетях, в частности на границах безопасности (в том числе в подсетях сети периметра), для получения многоуровневой защищенной сетевой среды.
...
* **Архитектура подсети:** Укажите виртуальную сеть, таким образом, чтобы вся подсеть была выделена для сети периметра, отделена от других подсетей в той же виртуальной сети. Это гарантирует, что трафик между сетью периметра и другими уровнями внутренних или частных подсетей будет проходить через брандмауэры или виртуальные устройства IDS и IPS.  Определяемые пользователем маршруты, расположенные на границах подсетей, должны перенаправить трафик в виртуальное устройство.
* **NSG:** Сама подсеть сети периметра должна быть открытой для взаимодействия с Интернетом, но это не означает, что клиенты должны применяться группы безопасности сети. Следуйте общим рекомендациям по безопасности для минимизации участков сети, открытых для доступа через Интернет. Ограничьте диапазон удаленных адресов, с которых разрешен доступ к развернутым службам, а также диапазон используемых протоколов приложений и открытых портов. При этом могут возникнуть обстоятельства, в которых полная блокировка невозможна. Например если клиенты имеют внешний веб-сайт в Azure, сети периметра следует разрешить входящие веб-запросы от любого общедоступного IP-адреса, но следует только на порты веб-приложения: TCP-подключения через порт 80 и TCP-подключения через порт 443.
* **Таблица маршрутизации:** Сама подсеть сети периметра должны иметь возможность взаимодействовать с Интернетом напрямую, но не должны допускать прямой обмен данными с задней окончания или в локальной сети минуя брандмауэра или устройства безопасности.
* **Конфигурация устройств безопасности:** Для маршрутизации и проверки пакетов между демилитаризованной зоной и остальной части защищенных сетях, например брандмауэра, IDS и IPS устройства для обеспечения безопасности устройств может использоваться как многосетевой. То есть они могут иметь отдельные сетевые адаптеры для сети периметра и внутренних подсетей. Сетевые адаптеры в сети периметра взаимодействуют с Интернетом напрямую. В них будут использоваться соответствующие сетевые группы безопасности и таблица маршрутизации сети периметра. Сетевые адаптеры, подключенные к внутренним подсетям, имеют более ограниченные сетевые группы безопасности и таблицы маршрутизации этих внутренних подсетей.
* **Функциональные возможности устройств безопасности:** Устройства безопасности, развернутые в сети периметра, обычно выполняют следующие функции:
  * Брандмауэр: Принудительное применение правил брандмауэра и политик контроля доступа к входящим запросам.
  * Обнаружения и предотвращения угроз: Обнаружение и устранение вредоносных атак из Интернета.
  * Аудит и ведение журнала. Ведение подробных журналов аудита и анализа.
  * Обратный прокси-сервер: Перенаправление входящих запросов на соответствующие серверы серверной части. Оно включает в себя сопоставление и преобразование адресов назначения на интерфейсных устройствах, обычно брандмауэрах, в адреса тыловых серверов.
  * Прокси-сервер переадресации: Преобразование сетевых адресов и аудит для обмена данными между виртуальной сети к Интернету.
  * Маршрутизатор: Перенаправляет трафик входящих, так и между подсетями в виртуальной сети.
  * VPN-устройства: В качестве VPN-шлюзы организациями для распределенных VPN-подключения между локальными сетями клиента и виртуальными сетями Azure.
  * VPN-сервера: Принимает VPN-клиентов, подключающихся к виртуальным сетям Azure.
...
* Одна граница: Один интерфейсный локальной сети, между виртуальной сетью и Интернетом.
* Две границы: Один на Интернет-границе сети периметра, а другой между подсеть сети периметра и внутренними подсетями в виртуальных сетях Azure.
* Три границы: На Интернет-границе сети периметра, другая между сетью периметра и внутренними подсетями и третья между внутренними подсетями и локальной сетью.
* N границ: Номер переменной. В зависимости от требований безопасности для конкретной сети может применяться любое количество границ безопасности.
...
## <a name="examples-building-security-boundaries-with-azure-virtual-networks"></a>Примеры. Создание границ безопасности виртуальных сетей Azure
...
- Виртуальная сеть с двумя подсетями: FrontEnd и BackEnd;
...
* одну группу ресурсов.
* Виртуальная сеть с тремя подсетями: SecNet, FrontEnd и BackEnd;
...
* Внешние правила (для входящего трафика).
  ...
  2. Правила RDP (для каждого сервера Windows): Эти четыре правила (по одному для каждого сервера) обеспечивают управление серверами через RDP. Четыре правила RDP можно объединить в одно, если виртуальное сетевое устройство поддерживает такую возможность.
  3. Правила трафика приложений: Существует два из этих правил, первый интерфейсный веб-трафика, а второе — для внутреннего трафика (например, веб-сервера к уровню данных). Параметры этих правил будут зависеть от архитектуры сети размещения серверов и характеристик потоков трафика (направление потока и используемые порты).
...
* Внутренние правила (для трафика внутри виртуальной сети)
  1. Исходящий трафик Интернета правила: Это правило разрешает трафик из любой сети для передачи в указанные сети. Обычно это правило является заданным в брандмауэре по умолчанию и при этом отключенным. В нашем примере это правило следует активировать.
  2. Правило DNS. Это правило разрешает передавать на DNS-сервер только трафик DNS (на порт 53). Для этой среды блокируется большая часть трафика от интерфейсной подсети к серверной части. В частности, это правило явно разрешает трафик DNS из любой локальной подсети.
  3. Подсеть для правила подсети: Это правило не позволяет любому серверу в подсети серверной части для подключения к любому серверу в интерфейсной подсети (но не наоборот).
* Резервное правило (для трафика, который не соответствует указанным выше правилам).
  1. Правило запрета любого трафика: Это правило всегда должно быть последнее правило (с точки зрения приоритет), и таким образом Если трафик не соответствует ни одному из приведенных выше правил они удаляются этим правилом. Это правило по умолчанию обычно уже установлено и активировано. Поэтому оно не требует дополнительных действий.
