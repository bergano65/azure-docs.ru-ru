...
| Название | Ограничение | Примечания |
| ---- | ----- | ----- |
...
| Название | Ограничение для нескольких клиентов | Ограничение среды службы интеграции | Примечания |
|------|--------------------|---------------------------------------|-------|
| Длительность выполнения | 90 дней | 366 дней | Длительность выполнения вычисляется с помощью времени начала выполнения и предельного значения, указанного в параметре рабочего процесса. *период* [**хранения журнала выполнения составляет в днях**](#change-duration). <p><p>Чтобы изменить предельное значение по умолчанию (90 дней), ознакомьтесь с разделом [изменение длительности выполнения](#change-duration). |
| Выполнение хранения в хранилище | 90 дней | 366 дней | Хранение запуска вычисляется с помощью времени начала выполнения и ограничения, указанного *в текущем времени* параметром рабочего процесса, [**срок хранения журнала выполнения в днях**](#change-retention). Независимо от того, завершается ли выполнение или истекает время ожидания, вычисление удержания всегда использует время начала выполнения. Если длительность выполнения превышает *Текущее* ограничение, запуск удаляется из журнала выполнения. <p><p>Если изменить этот параметр, текущее ограничение всегда будет использоваться для вычисления срока хранения, независимо от предыдущего ограничения. Например, если сократить лимит хранения от 90 дней до 30 дней, то в журнале выполнения будет удалено выполнение с 60 дней назад. Если вы увеличиваете срок хранения с 30 дней до 60 дней, в журнале запусков в течение следующего 40 дней период выполнения 20 дней остается прежним. <p><p>Чтобы изменить предельное значение по умолчанию (90 дней), см. статью [изменение выполнения сохранения в хранилище](#change-retention). |
...
### <a name="change-run-duration-and-run-retention-in-storage"></a>Изменение длительности выполнения и хранение в хранилище

Чтобы изменить предельное значение по умолчанию для длительности выполнения и срок хранения в хранилище, выполните следующие действия. Чтобы увеличить максимальное ограничение, [обратитесь к группе Logic Apps](mailto://logicappsemail@microsoft.com) , чтобы получить помощь в требованиях.
...
1. Перейдите на [портал Azure](https://portal.azure.com). В поле поиска на портале найдите и выберите **Logic Apps (приложения логики**).
...
1. Перетащите ползунок, чтобы изменить нужное количество дней.

1. Когда все будет готово, на панели инструментов **Параметры рабочего процесса** нажмите кнопку **сохранить**.
...
Ниже приведены ограничения пропускной способности для [SKU "Премиум" интегрированной среды сценариев](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md#ise-level).
...
> [!NOTE]
> [SKU для разработчиков ISE](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md#ise-level) не имеет опубликованных ограничений, не предоставляет возможности масштабирования и не использует соглашение об уровне обслуживания (SLA). Используйте этот номер SKU только для экспериментов, разработки и тестирования, а не для тестирования в рабочей среде и производительности.
...
| Название | Ограничение для нескольких клиентов | Ограничение среды службы интеграции | Примечания |
|------|--------------------|---------------------------------------|-------|
| Размер сообщения | 100 МБ | 200 МБ | Чтобы обойти это ограничение, см. раздел [Обработка больших сообщений](../logic-apps/logic-apps-handle-large-messages.md). Тем не менее некоторые соединители и API-интерфейсы могут не поддерживать фрагментацию и даже ограничение по умолчанию. <p><p>— Такие соединители, как AS2, X12 и EDIFACT, имеют собственные [ограничения сообщений B2B](#b2b-protocol-limits). <br>-Соединители ISE используют ограничение ISE, а не ограничения соединителей, не связанных с ISE. |
| Размер сообщения с фрагментацией | 1 ГБ | 5 ГБ | Это ограничение применяется к действиям, которые либо изначально поддерживают фрагментацию, либо позволяют включить фрагментацию в своей конфигурации среды выполнения. <p><p>Если вы используете интегрированную среду сценариев, подсистема Logic Apps поддерживает это ограничение, но соединители имеют собственные ограничения на объем блоков, например, см. [Справочник по API соединителя хранилища BLOB-объектов Azure](https://docs.microsoft.com/connectors/azureblob/). Дополнительные сведения о фрагментации см. [в разделе Handle Large messages with Фрагментирование](../logic-apps/logic-apps-handle-large-messages.md). |
|||||
...
Ниже приведены ограничения для приложения логики, которое начинается с триггера запроса и включает [Azure Active Directory Open authentication](../active-directory/develop/about-microsoft-identity-platform.md) (Azure AD OAuth) для авторизации входящих вызовов триггера запроса:

| Название | Ограничение | Примечания |
| ---- | ----- | ----- |
| Политики авторизации Azure AD | 5 | |
| Утверждения на политику авторизации | 10 | |
||||
...
|------|-------|
| Управляемые удостоверения для приложения логики | Удостоверение, назначенное системой, или одно пользовательское удостоверение |
| Число приложений логики с управляемым удостоверением в подписке Azure на регион | 250 |
|||
...
* В каждой ИНТЕГРИРОВАНной среде [разработки (Developer или Premium](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md#ise-level)) ограничен 5 общих учетных записей интеграции:
  ...
  |---------|----------------------------|
  | **Premium** | 5 всего — только [стандартные](../logic-apps/logic-apps-pricing.md#integration-accounts) учетные записи, включая одну стандартную учетную запись бесплатно. Учетные записи Free и Basic не разрешены. |
...
Цены см. на странице [цен на Logic Apps](https://azure.microsoft.com/pricing/details/logic-apps/). Чтобы узнать, как цены и данные о выставлении счетов для учетных записей интеграции, см. [Logic Apps модель ценообразования](../logic-apps/logic-apps-pricing.md#integration-accounts).
...
| Сопоставление (XSLT-файл) | 8 МБ | Чтобы передать файлы, размер которых превышает 2 МБ, используйте [REST API Azure Logic Apps — Maps](https://docs.microsoft.com/rest/api/logic/maps/createorupdate). <p><p>**Примечание**. объем данных или записей, которые могут быть успешно обработаны картой, зависит от размера сообщения и ограничений времени ожидания действия в Azure Logic Apps. Например, если вы используете действие HTTP, основанное на [размере сообщений HTTP и предельном времени ожидания](#request-limits), то карту могут обрабатывать данные до предельного размера HTTP-сообщения, если операция завершается в пределах предела времени ожидания HTTP. |
...
IP-адреса, которые Azure Logic Apps используют для входящих и исходящих вызовов, зависят от региона, в котором существует приложение логики. *Все* приложения логики в одном регионе используют одни и те же диапазоны IP-адресов. Некоторые вызовы [Power автоматизиру](https://docs.microsoft.com/power-automate/getting-started) , такие как запросы **http** и **http + OpenAPI** , проходят непосредственно через службу Azure Logic Apps и берутся из указанных здесь IP-адресов. Дополнительные сведения об IP-адресах, используемых службой автоматизации питания, см. [в разделе ограничения и настройка в Power автоматизиру](https://docs.microsoft.com/flow/limits-and-config#ip-address-configuration).

> [!TIP]
> Чтобы сократить сложность при создании правил безопасности, при необходимости можно использовать [теги службы](../virtual-network/service-tags-overview.md)вместо указания IP-адресов Logic Apps для каждого региона, описанных далее в этом разделе. Эти теги работают в регионах, где доступна служба Logic Apps:
>
> * **Логикаппсманажемент**: представляет префиксы входящих IP-адресов для службы Logic Apps.
> * **LogicApps**: представляет префиксы исходящего IP-адреса для службы Logic Apps.

* Для [Azure Китая 21vianet](https://docs.microsoft.com/azure/china/)фиксированные или зарезервированные IP-адреса недоступны для [настраиваемых соединителей](../logic-apps/custom-connector-overview.md) и [управляемых соединителей](../connectors/apis-list.md#managed-api-connectors), например для службы хранилища Azure, SQL Server, Office 365 Outlook и т. д.
...
* Для поддержки вызовов, которые делают [управляемые соединители](../connectors/apis-list.md#managed-api-connectors) , настройте брандмауэр со *всеми* [исходящими](#outbound) IP-адресами, используемыми этими соединителями, в зависимости от регионов, где существуют приложения логики. Эти адреса отображаются под заголовком **Исходящие** в этом разделе и сортируются по регионам.
...
В этом разделе перечислены входящие IP-адреса только для службы Azure Logic Apps. Если у вас есть Azure для государственных организаций, см. раздел [входящие IP-адреса Azure для государственных организаций](#azure-government-inbound).

> [!TIP]
> Чтобы сократить сложность при создании правил безопасности, при необходимости можно использовать [тег службы](../virtual-network/service-tags-overview.md) **логикаппсманажемент**вместо указания префиксов для входящего Logic Apps IP-адресов для каждого региона. Этот тег работает в регионах, где доступна служба Logic Apps.

<a name="multi-tenant-inbound"></a>

#### <a name="multi-tenant-azure---inbound-ip-addresses"></a>Несколько клиентов Azure — входящие IP-адреса
...
| Западная Южная Африка | 102.133.72.190, 102.133.72.145, 102.133.72.184, 102.133.72.173 |
...
В этом разделе перечислены исходящие IP-адреса для службы Azure Logic Apps и управляемых соединителей. Если у вас есть Azure для государственных организаций, см. раздел [Исходящие IP-адреса Azure для государственных организаций](#azure-government-outbound).

> [!TIP]
> Чтобы сократить сложность при создании правил безопасности, при необходимости можно использовать [тег службы](../virtual-network/service-tags-overview.md) **LogicApps**вместо указания префиксов исходящего Logic Apps IP-адресов для каждого региона. Этот тег работает в регионах, где доступна служба Logic Apps. Для управляемых соединителей необходимо продолжать использовать IP-адреса.

<a name="multi-tenant-outbound"></a>

#### <a name="multi-tenant-azure---outbound-ip-addresses"></a>Azure — исходящие IP-адреса с несколькими клиентами
...
| Западная Южная Африка | 102.133.72.98, 102.133.72.113, 102.133.75.169, 102.133.72.179, 102.133.72.37, 102.133.72.183, 102.133.72.132, 102.133.75.191 | 13.65.86.57, 104.214.19.48-104.214.19.63, 104.214.70.191, 102.133.72.85 |
