...
IP-адреса, которые Azure Logic Apps используют для входящих и исходящих вызовов, зависят от региона, в котором существует приложение логики. *Все* приложения логики в одном регионе используют одни и те же диапазоны IP-адресов. Некоторые вызовы [Power Automate](/power-automate/getting-started), такие как запросы **HTTP** и **HTTP + OpenAPI**, проходят непосредственно через службу Azure Logic Apps и поступают из перечисленных здесь IP-адресов. Дополнительные сведения об IP-адресах, используемых службой Power Automate, см. в статье [Ограничения и конфигурация в Power Automate](/flow/limits-and-config#ip-address-configuration).
