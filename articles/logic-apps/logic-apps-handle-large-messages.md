...
> [!NOTE]
> Для действий, использующих фрагментацию, нельзя передавать текст триггера или использовать выражения, такие как `@triggerBody()?['Content']`. Вместо этого для текста или содержимого JSON-файла можно попробовать использовать [действие **Compose**](../logic-apps/logic-apps-perform-data-operations.md#compose-action) или [создать переменную](../logic-apps/logic-apps-create-variables-store-values.md) для обработки этого содержимого. Если текст триггера содержит другие типы содержимого, например файлы мультимедиа, необходимо выполнить другие действия по обработке этого содержимого.
