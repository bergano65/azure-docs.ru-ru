...
description: Узнайте, как работать с большими размерами сообщений с помощью блоков в автоматизированных задачах и рабочих процессах, создаваемых с помощью Azure Logic Apps
services: logic-apps
...
> [!NOTE]
> Для действий, использующих фрагментацию, нельзя передавать тело триггера или использовать выражения, такие как `@triggerBody()?['Content']` в этих действиях. Вместо этого для текста или содержимого JSON-файла можно попробовать использовать [действие **создать** ](../logic-apps/logic-apps-perform-data-operations.md#compose-action) или [создать переменную](../logic-apps/logic-apps-create-variables-store-values.md) для ее обработки. Если текст триггера содержит другие типы содержимого, например файлы мультимедиа, необходимо выполнить другие действия по обработке этого содержимого.
...
1. Приложение логики отправляет начальный HTTP-запрос POST или PUT с пустым сообщением. Заголовок запроса содержит такие сведения о содержимом, которое приложение логики хочет передать поблочно:
   ...
   | **x-ms-content-length** | <*Длина содержимого*> | Целое число | Размер всего содержимого (в байтах) перед фрагментацией |
   ||||

2. Конечная точка возвращает ответ с кодом состояния "200" и такую дополнительную информацию:

   | Поле заголовка ответа конечной точки | Тип | Обязательный | Описание |
   |--------------------------------|------|----------|-------------|
...
3. Приложение логики создает и отправляет последующие сообщения HTTP PATCH с приведенными ниже сведениями.
   ...
   * Следующие сведения заголовка о блоке содержимого отправляются в каждом сообщении PATCH:
     ...
     | Поле заголовка запроса Logic Apps | Значение | Type | Описание |
     |---------------------------------|-------|------|-------------|
     | **Content-Range** | <*разнообраз*> | Строка | Диапазон байтов для текущего блока содержимого, включая начальное значение, конечное значение и общий размер содержимого, например "bytes=0-1023/10100" |
     | **Тип содержимого** | <*тип содержимого*> | Строка | Тип фрагментированного содержимого |
     | **Длина содержимого** | <*Длина содержимого*> | Строка | Длина размера в байтах для текущего блока |
     |||||

4. После каждого запроса на исправление конечная точка подтверждает уведомление для каждого блока, отвечая на код состояния "200" и следующие заголовки ответа:

   | Поле заголовка ответа конечной точки | Тип | Обязательный | Описание |
   |--------------------------------|------|----------|-------------|
   | **Разнообраз** | Строка | Да | Диапазон байтов для содержимого, полученного конечной точкой, например: "bytes = 0-1023" |   
   | **x-ms-chunk-size** | Целое число | Нет | Предлагаемый размер блока в байтах |
   ||||
