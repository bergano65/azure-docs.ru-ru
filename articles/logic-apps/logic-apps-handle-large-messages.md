...
> [!NOTE]
> Для действий, использующих фрагментацию, нельзя передавать тело триггера или использовать выражения, `@triggerBody()?['Content']` такие как в этих действиях. Вместо этого для текста или содержимого JSON-файла можно попробовать использовать [действие **создать** ](../logic-apps/logic-apps-perform-data-operations.md#compose-action) или [создать переменную](../logic-apps/logic-apps-create-variables-store-values.md) для ее обработки. Если текст триггера содержит другие типы содержимого, например файлы мультимедиа, необходимо выполнить другие действия по обработке этого содержимого.
...
4. После каждого запроса на исправление конечная точка подтверждает уведомление для каждого блока, отвечая на код состояния "200" и следующие заголовки ответа:
   ...
   | Поле заголовка ответа конечной точки | Тип | Обязательный | Описание |
   |--------------------------------|------|----------|-------------|
   | **Диапазон** | Строка | Да | Диапазон байтов для содержимого, полученного конечной точкой, например: "bytes = 0-1023" |   
   | **x-ms-chunk-size** | Целое число | Нет | Предлагаемый размер блока в байтах |
   ||||
