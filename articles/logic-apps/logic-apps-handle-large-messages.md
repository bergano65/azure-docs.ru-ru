...
> [!NOTE]
> Для действий, использующих фрагментацию, нельзя передавать текст триггера или использовать выражения, такие как `@triggerBody()?['Content']`, в этих действиях. Вместо этого для текста или содержимого JSON-файла можно попробовать использовать [действие **создать** ](../logic-apps/logic-apps-perform-data-operations.md#compose-action) или [создать переменную](../logic-apps/logic-apps-create-variables-store-values.md) для ее обработки. Если текст триггера содержит другие типы содержимого, например файлы мультимедиа, необходимо выполнить другие действия по обработке этого содержимого.
...
1. Приложение логики отправляет начальный HTTP-запрос POST или PUT с пустым сообщением. Заголовок запроса содержит такие сведения о содержимом, которое приложение логики хочет передать поблочно:
   ...
   | Поле заголовка запроса Logic Apps | Значение | Тип | Description |
   |---------------------------------|-------|------|-------------|
...
2. Конечная точка возвращает ответ с кодом состояния "200" и такую дополнительную информацию:
   ...
   | Поле заголовка ответа конечной точки | Тип | Обязательно для заполнения | Description |
   |--------------------------------|------|----------|-------------|
   | **x-ms-chunk-size** | Целое число | Нет | Предлагаемый размер блока в байтах |
   | **Местоположение** | String | Да | URL-адрес расположения для отправки сообщений HTTP PATCH |
   ||||
...
4. После каждого запроса на исправление конечная точка подтверждает уведомление для каждого блока, отвечая на код состояния "200" и следующие заголовки ответа:
   ...
   | Поле заголовка ответа конечной точки | Тип | Обязательно для заполнения | Description |
   |--------------------------------|------|----------|-------------|
   | **Range** | String | Да | Диапазон байтов для содержимого, полученного конечной точкой, например: "bytes = 0-1023" |   
   | **x-ms-chunk-size** | Целое число | Нет | Предлагаемый размер блока в байтах |
   ||||
