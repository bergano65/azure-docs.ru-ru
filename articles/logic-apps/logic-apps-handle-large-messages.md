...
1. Приложение логики отправляет начальный HTTP-запрос POST или PUT с пустым сообщением. Заголовок запроса содержит такие сведения о содержимом, которое приложение логики хочет передать поблочно:
   ...
   |---------------------------------|-------|------|-------------|
   | **x-ms-transfer-mode** | chunked | Строковое | Указывает, что содержимое передается блоками |
...
2. Конечная точка возвращает ответ с кодом состояния "200" и такую дополнительную информацию:
   ...
   | Поле заголовка ответа конечной точки | Type | Обязательное значение | Описание |
   |--------------------------------|------|----------|-------------|
   | **x-ms-chunk-size** | Integer | Нет | Предлагаемый размер блока в байтах |
   | **Location** | Строковое | Нет | URL-адрес расположения для отправки сообщений HTTP PATCH |
   ||||
...
3. Приложение логики создает и отправляет последующие сообщения HTTP PATCH с приведенными ниже сведениями.
   ...
   * Следующие сведения заголовка о блоке содержимого отправляются в каждом сообщении PATCH:
     ...
     |---------------------------------|-------|------|-------------|
     | **Content-Range** | <*диапазон*> | Строковое | Диапазон байтов для текущего блока содержимого, включая начальное значение, конечное значение и общий размер содержимого, например "bytes=0-1023/10100" |
     | **Content-Type** | <*тип_содержимого*> | Строковое | Тип фрагментированного содержимого |
     | **Content-Length** | <*длина_содержимого*> | Строковое | Длина размера в байтах для текущего блока |
     |||||
...
4. После каждого запроса на исправление конечная точка подтверждает уведомление для каждого блока, отвечая на код состояния "200" и следующие заголовки ответа:
   ...
   | Поле заголовка ответа конечной точки | Тип | Обязательное значение | Описание |
   |--------------------------------|------|----------|-------------|
   | **Range** | Строковое | Да | Диапазон байтов для содержимого, полученного конечной точкой, например: "bytes = 0-1023" |   
   | **x-ms-chunk-size** | Integer | Нет | Предлагаемый размер блока в байтах |
   ||||
