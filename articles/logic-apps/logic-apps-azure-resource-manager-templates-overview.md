...
* Другие ресурсы, необходимые для развертывания приложением логики
  ...
  Например, если приложение логики использует [учетную запись интеграции](../logic-apps/logic-apps-enterprise-integration-create-integration-account.md) для сценариев B2B, то объект верхнего уровня `parameters` шаблона объявляет параметр, который принимает идентификатор ресурса для этой учетной записи интеграции.
...
|-----------|----------|------|-------------|
| `state` | Да | Строка | Состояние приложения логики при развертывании. `Enabled` это означает, что приложение логики `Disabled` работает в реальном времени и означает, что приложение логики неактивно. Например, если вы не готовы к работе с приложением логики, но хотите развернуть черновую версию, можно использовать `Disabled` параметр. |
| `integrationAccount` | Нет | Объект | Если приложение логики использует учетную запись интеграции, которая хранит артефакты для сценариев "бизнес — бизнес" (B2B), этот объект `id` включает атрибут, указывающий идентификатор для учетной записи интеграции. |
...
* Внутри определения рабочего процесса `parameters` объект объявляет `$connections` параметр для значений соединения для использования в среде выполнения приложения логики. Кроме того, триггер или действие, которое создает соединение, использует соответствующие значения, которые проходят через `$connections` этот параметр.

* *За пределами* определения рабочего процесса, но по-прежнему *внутри* определения ресурса `parameters` приложения логики другой объект задает значения, которые будут `$connections` использоваться во время выполнения для параметра, ссылаясь на соответствующие параметры шаблона. Эти значения используют выражения шаблона для ссылки на ресурсы, которые безопасно хранят метаданные для подключений в приложении логики.
...
Для параметра подключения, который обрабатывает конфиденциальную информацию, пароли, ключи доступа или секреты, определение ресурса подключения включает `parameterValues` объект, указывающий эти значения в формате пар «имя-значение». Чтобы скрыть эти сведения, можно объявить или изменить параметры шаблона для этих значений с помощью типов параметров `securestring` или `secureobject` . Затем эти сведения можно сохранить в [Azure Key Vault](../azure-resource-manager/templates/key-vault-parameter.md). Чтобы передать эти значения в параметры шаблона, вы ссылаетесь на это хранилище ключей в [файле параметров](#template-parameter-files) , который используется шаблоном при развертывании.
...
| `token:grantType` | Запрошенный тип предоставления, который должен иметь `client_credentials`значение. Дополнительные сведения см. [в разделе Платформа Microsoft Identity и поток учетных данных клиента OAuth 2,0](../active-directory/develop/v2-oauth2-client-creds-grant-flow.md). |
|||

**Определения параметров шаблона**

Объект верхнего уровня `parameters` шаблона объявляет эти параметры для примера соединения:
