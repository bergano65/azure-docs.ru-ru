...
Чтобы проверить определения ресурсов для всех ресурсов в группе ресурсов Azure, [скачайте приложение логики из Azure в Visual Studio](../logic-apps/manage-logic-apps-with-visual-studio.md). это самый простой способ создать допустимый параметризованный шаблон приложения логики, который в основном готов к развертыванию, или выполнить следующие действия в портал Azure.

1. Войдите на [портал Azure](https://portal.azure.com) с помощью учетных данных учетной записи Azure.

1. Найдите группу ресурсов Azure, которая содержит приложение логики, подключения и другие ресурсы.

1. На панели инструментов группы ресурсов выберите **Обзор**, а затем выберите все ресурсы в группе ресурсов.

1. На панели инструментов группы ресурсов в разделе **Параметры**выберите **Экспорт шаблона**.

   На портале отобразятся определения для выбранных ресурсов. Дополнительные сведения см. [в статье экспорт одного и нескольких ресурсов в шаблон в портал Azure](../azure-resource-manager/templates/export-template-portal.md).
...
Когда приложение логики создает и использует подключения к другим службам и системам с помощью [управляемых соединителей](../connectors/apis-list.md), `resources` объект шаблона содержит определения ресурсов для этих подключений.
