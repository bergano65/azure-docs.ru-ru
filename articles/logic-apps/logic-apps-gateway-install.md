...
* Учетная запись и подписка Azure. Если у вас нет учетной записи Azure с подпиской, [зарегистрируйтесь для получения бесплатной учетной записи Azure](https://azure.microsoft.com/free/).

  * Учетная запись Azure должна принадлежать одному [арендатору или каталогу Azure Active Directory (Azure AD)](../active-directory/fundamentals/active-directory-whatis.md#terminology). Для установки и администрирования шлюза на локальном компьютере необходимо использовать одну и ту же учетную запись Azure.

  * Во время установки шлюза нужно войти с учетной записью Azure, что привяжет установку шлюза конкретно к этой учетной записи Azure. Позже на портале Azure необходимо будет использовать те же учетную запись Azure и арендатор Azure AD при создании ресурса шлюза Azure, который используется для регистрации и утверждения установки шлюза. Затем в Azure Logic Apps локальные триггеры и действия будут использовать этот ресурс шлюза для подключения к локальным источникам данных.

    > [!NOTE]
    > Вы можете связать между собой только одну установку шлюза и один ресурс шлюза Azure. Невозможно связать одну установку шлюза с несколькими учетными записями Azure или ресурсами шлюза Azure. Однако учетную запись Azure можно привязать к нескольким установкам шлюза и ресурсам шлюза Azure. В локальном триггере или действии можно выбрать одну из нескольких подписок Azure, а затем выбрать связанный ресурс шлюза.

  * Необходимо выполнить вход с рабочей учетной или учебной учетной записью, которая также называется учетной записью *организации* и выглядит так: `username@contoso.com`. Невозможно использовать учетные записи Azure B2B (гостевые) или личные учетные записи Майкрософт, например @hotmail.com или @outlook.com.

    > [!TIP]
    > Если вы зарегистрировались для использования предложения Office 365 и не предоставили рабочий электронный адрес, то ваш адрес может выглядеть так: `username@domain.onmicrosoft.com`. Ваша учетная запись хранится в арендаторе в Azure Active Directory (Azure AD). В большинстве случаев имя участника-пользователя для учетной записи Azure AD совпадает с адресом электронной почты.
    >
    > Чтобы использовать [подписку Visual Studio Standard](https://visualstudio.microsoft.com/vs/pricing/), связанную с учетной записью Майкрософт, сначала [создайте арендатор в Azure AD](../active-directory/develop/quickstart-create-new-tenant.md), или используйте каталог по умолчанию. Добавьте в каталог пользователя с паролем, а затем предоставьте этому пользователю доступ к своей подписке Azure. Затем вы можете войти в систему во время установки шлюза, используя эти имя пользователя и пароль.
...
1. Проверьте регион, который был выбран для облачной службы шлюза и [Служебной шины Azure](https://azure.microsoft.com/services/service-bus/), используемый для установки шлюза. По умолчанию этот регион совпадает с расположением арендатора Azure AD для вашей учетной записи Azure.
