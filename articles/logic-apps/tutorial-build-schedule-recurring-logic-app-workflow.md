...
2. В разделе **Создание приложения логики** предоставьте сведения о приложении логики, как показано и описано. По завершении выберите **Закрепить на панели мониторинга** > **Создать**.
   ...
   | **Расположение** | Восток США 2 | Регион для хранения сведений о приложении логики. | 
...
2. В фигуре **Повторение** нажмите кнопку **многоточия** (**...**) и выберите **Переименовать**. Переименуйте триггер, используя это описание:
...
4. Переименуйте действие с использованием этого описания:
...
5. Укажите сведения для действия **Get route** (Получить маршрут), как описано здесь, например:
   ...
   | Параметр | Значение | ОПИСАНИЕ |
   | ------- | ----- | ----------- |
   | **Waypoint 1 (Пункт 1)** | <*исходный пункт*> | Начало маршрута | 
   | **Waypoint 2 (Пункт 2)** | <*пункт назначения*> | Место назначения маршрута | 
   | **Избегать** | Нет | Все, чего нужно избегать на маршруте, например шоссе, платные дороги и т. д. | 
   | **Оптимизация** | timeWithTraffic | Параметр для оптимизации маршрута, например расстояние, время в пути в соответствии с текущей загрузкой дорог и т. д. Выберите параметр "timeWithTraffic" | 
   ...
   | **Travel mode (Способ передвижения)** | Driving (Движение на автомобиле) | Режим движения для маршрута. Выберите способ Driving (На автомобиле) | 
   | **Transit Date-Time (Дата и время пересадки)** | Нет | Применяется только для режима движения Transit (Транзит) | 
   | **Date-Time Type (Значение даты и времени)** | Нет | Применяется только для режима движения Transit (Транзит) | 
   |||| 
...
3. Переименуйте действие с использованием этого описания:
...
4. Укажите сведения о переменной, как описано здесь:
   ...
   | **type** | Целое число  | Тип данных для переменной. | 
...
2. Переименуйте условие, используя это описание: ```If travel time exceeds limit```
...
> [Управление запросами для списка рассылки](../logic-apps/tutorial-process-mailing-list-subscriptions-workflow.md)
