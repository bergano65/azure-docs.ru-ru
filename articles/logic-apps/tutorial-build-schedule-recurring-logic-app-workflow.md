...
---
# <a name="tutorial-create-automated-schedule-based-recurring-workflows-by-using-azure-logic-apps"></a>Руководство. cозданию автоматических, выполняемых по расписанию, повторяющихся рабочих процессов с помощью Azure Logic Apps
...
1. В разделе **Создание приложения логики** предоставьте сведения о приложении логики, как показано и описано. Когда все будет готово, выберите **Создать**.
   ...
   |----------|-------|-------------|
   | **Название** | LA-TravelTime | Имя приложения логики, которое может содержать только буквы, цифры, дефисы (`-`), символы подчеркивания (`_`), круглые скобки (`(`, `)`) и точки (`.`). В этом примере используется LA-TravelTime. |
   | **подписка** | <*Имя вашей подписки Azure*> | Имя подписки Azure. |
   | **группа ресурсов** | LA-TravelTime-RG | Имя [группы ресурсов Azure](../azure-resource-manager/management/overview.md), используемой для упорядочения связанных ресурсов. В этом примере используется "LA-TravelTime-RG". |
...
1. Если у вас нет подключения к Картам Bing, появится запрос на его установку. Укажите сведения о подключении и выберите **Создать**.
   ...
   | **Ключ API**. | Да | <*ваш ключ Карт Bing*> | Введите ключ Карт Bing, полученный ранее. Если у вас нет ключа Карт Bing, узнайте, [как получить его](https://msdn.microsoft.com/library/ff428642.aspx). |
   |||||
...
1. Теперь задайте значения для дополнительных свойств действия, как показано здесь.
   ...
   | Свойство | Обязательно | Значение | Description |
   |----------|----------|-------|-------------|
   | **Пункт маршрута 1** | Да | <*исходный пункт*> | Начало маршрута |
   | **Пункт маршрута 2** | Да | <*пункт назначения*> | Место назначения маршрута |
...
1. Укажите сведения о переменной, как описано здесь:
   ...
   | Свойство | Обязательно | Значение | Description |
   |----------|----------|-------|-------------|
   | **Название** | Да | travelTime | Имя для переменной. В этом примере используется "travelTime". |
   | **Тип** | Да | Целое число | Тип данных для переменной. |
   | **Value** | нет| Выражение, преобразующее текущее время в секундах в минуты (см. шаги в этой таблице). | Начальное значение для переменной |
   ||||
