...
1. В свойстве **Body** действия ответа добавьте маркер, представляющий параметр, указанный в относительных пути триггера.
   ...
   В свойстве **Body** введите `Postal Code: ` конечную пробел. В появившемся списке динамического содержимого выберите токен **PostalCode** .
...
После создания конечной точки HTTP можно запустить приложение логики, отправив HTTP- `POST` запрос на полный URL-адрес конечной точки. Приложения логики имеют встроенную поддержку конечных точек прямого доступа.
