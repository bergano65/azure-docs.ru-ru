---
title: Создание веб-AI & REST AIS для приложений логики Azure
...
Хотя Azure Logic Apps предлагает [сотни разъемов,](../connectors/apis-list.md) которые можно использовать в рабочих процессах логических приложений, можно вызвать AI, системы и службы, которые недоступны в качестве разъемов. Вы можете создавать собственные API с действиями и триггерами для использования в приложениях логики. Ниже приведены другие причины создания собственных интерфейсов API, которые можно вызывать из рабочих процессов приложения логики.
...
По сути, соединители — это веб-интерфейсы API, использующие REST для модульных интерфейсов, [формат метаданных Swagger](https://swagger.io/specification/) для документации и формат JSON для обмена данными. Поскольку разъемы являются APIs REST, которые передаются через конечные точки HTTP, вы можете использовать любой язык, например .NET, Java, Python или Node.js, для создания разъемов. Интерфейсы API также можно разместить в [службе приложений Azure](../app-service/overview.md). Это платформа как услуга (PaaS), предоставляющая один из лучших, самых простых и масштабируемых способов размещения API. 
...
> * [Руби](../app-service/containers/quickstart-ruby.md)
>
> Примеры приложений API для приложений логики см. в [репозитории GitHub для Azure Logic Apps](https://github.com/logicappsio) или [блоге](https://aka.ms/logicappsblog).
...
Пользовательские API и [пользовательские соединители](../logic-apps/custom-connector-overview.md) — это интерфейсы веб-API, использующие REST для модульных интерфейсов, [формат метаданных Swagger](https://swagger.io/specification/) для документации и формат JSON для обмена данными. А поскольку эти AA и разъемы являются REST AIS, которые передаются через конечные точки HTTP, вы можете использовать любой язык, например .NET, Java, Python или Node.js, для создания пользовательских AA и разъемов.
...
Вы также можете предложить зарегистрированные соединители для сертификации Майкрософт. Этот процесс проверяет, что зарегистрированные разъемы отвечают критериям для общественного пользования и делает эти разъемы доступными для пользователей power Automate и Microsoft Power Apps.
...
* [Регистрация настраиваемых соединителей в Azure Logic Apps](../logic-apps/logic-apps-custom-connector-register.md)
...
## <a name="improve-security-for-calls-to-your-apis-from-logic-apps"></a>Улучшение безопасности для вызовов aI-приложений из логических приложений

После создания пользовательских API настройте аутентификацию для API, чтобы обеспечить безопасность при их вызове из приложений логики. [Узнайте, как повысить безопасность вызовов пользовательских AI из логических приложений.](../logic-apps/logic-apps-custom-api-authentication.md)
...
Чтобы сделать пользовательские AI доступными для всех пользователей в приложениях Logic Apps, Power Automate и Microsoft Power Apps, необходимо добавить систему безопасности, зарегистрировать свои AI в качестве разъемов Logic App и назначить разъемы для [сертифицированной программы Microsoft Azure.](https://azure.microsoft.com/marketplace/programs/certified/logic-apps/) 

## <a name="get-support"></a>Получение поддержки

* Для конкретной помощи с [customapishelp@microsoft.com](mailto:customapishelp@microsoft.com)пользовательскими AIS, обратитесь .
