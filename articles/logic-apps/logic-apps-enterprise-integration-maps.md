---
title: Преобразование XML с картами XSLT
...
Для передачи данных XML между форматами для сценариев корпоративной интеграции в приложениях Azure Logic Apps приложение с логикой может использовать карты, или, более конкретно, расширяемые языковые преобразования (XSLT). Карта — это XML-документ, описывающий преобразование данных из XML-документа в другой формат. 
...
* Если карта ссылается на внешнюю сборку, необходимо передать *сборку и карту* в учетную запись интеграции. Убедитесь, что сначала вы [*загружаете сборку,*](#add-assembly)а затем загружайте карту, отсылает к сборке.
  ...
  * Для сборок необходим контейнер больших двоичных объектов Azure, в который можно отправить сборку, и расположение этого контейнера. Так вы сможете указать это расположение позже — при добавлении сборки в учетную запись интеграции. 
  Чтобы сделать это, вам понадобятся следующие элементы:
    ...
    | [Исследователь хранения azure](../vs-azure-tools-storage-manage-with-storage-explorer.md) | Это средство упрощает управление учетными записями хранения и контейнерами больших двоичных объектов. Чтобы использовать Обозреватель службы хранилища Azure, [скачайте и установите его](https://www.storageexplorer.com/). Затем подключите Обозреватель службы хранилища к учетной записи хранения, следуя шагам из статьи [Начало работы с Обозревателем службы хранилища](../vs-azure-tools-storage-manage-with-storage-explorer.md). Чтобы узнать больше, [см.](../storage/blobs/storage-quickstart-blobs-storage-explorer.md) <p>Можно также найти и выбрать свою учетную запись хранения на портале Azure. В меню учетной записи хранения выберите **Обозреватель службы хранилища**. |
    |||
...
> [!NOTE]
> При изменении сборки необходимо также обновить карту независимо от того, есть ли на карте изменения.
...
Чтобы добавить объемные сборки, отправьте свою сборку в контейнер больших двоичных объектов Azure в учетной записи хранения Azure. Ваши шаги для добавления сборок различаются на основе того, имеет ли ваш контейнер с каплей доступ к общедоступным чтениям. Итак, во-первых, проверьте, имеет ли ваш контейнер с каплями доступ к общедоступным, следуя следующим шагам: [Установите уровень публичного доступа для контейнера с каплями](../vs-azure-tools-storage-explorer-blobs.md#set-the-public-access-level-for-a-blob-container)
...
1. В контекстном меню контейнера больших двоичных объектов выберите **Set Public Access Level** (Настроить уровень общего доступа).
   ...
   * Если ваш контейнер с blob имеет по крайней мере открытый доступ, выберите **Cancel**и выполните следующие действия позже на этой странице: [Загрузить в контейнеры с открытым доступом](#public-access-assemblies)

     ![Открытый доступ](media/logic-apps-enterprise-integration-schemas/azure-blob-container-public-access.png)

   * Если контейнер с blob не имеет общедоступного доступа, выберите **Cancel**и выполните следующие действия позже на этой странице: [Загрузить в контейнеры без публичного доступа](#no-public-access-assemblies)
...
1. В **типе карты**выберите тип, например: **Liquid,** **XSLT,** **XSLT 2.0**или **XSLT 3.0**.
