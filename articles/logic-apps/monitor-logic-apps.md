...
1. Выберите приложение логики и щелкните **Обзор**.
   ...
   | Состояние | Описание |
   |--------|-------------|
   | **Клонения** | Рабочий процесс был запущен, но получил запрос на отмену |
   | **Failed** | Не удалось выполнить по крайней мере одно действие, и последующие действия в рабочем процессе не были настроены для обработки сбоя. |
   ...
   | **Waiting** | Рабочий процесс не запущен или приостановлен, например, из-за более раннего рабочего процесса, который еще работает. |
   |||
...
1. В меню приложения логики выберите **Обзор**. В разделе " **Сводка** " в разделе " **Оценка**" выберите **Просмотреть журнал триггеров**.
   ...
   | Состояние | Описание |
   |--------|-------------|
   | **Failed** | Произошла ошибка. Чтобы просмотреть созданные сообщения об ошибках для триггера, завершившегося сбоем, выберите соответствующую попытку активации и щелкните **Выходные данные**. Например, вы можете обнаружить недопустимые входные данные. |
