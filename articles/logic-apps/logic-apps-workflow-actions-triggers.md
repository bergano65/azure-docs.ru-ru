...
| <*splitOn-expression*> | Строка | Для триггеров, возвращающих массивы, это выражение ссылается на массив, который необходимо использовать, чтобы вы могли создать и запустить экземпляр рабочего процесса для каждого элемента массива, а не использовали цикл Foreach. <p>Например следующее выражение представляет элемент в массиве, возвращенное внутри тела триггера: `@triggerbody()?['value']` |
| <*operation-option*> | Строка | Поведение по умолчанию можно изменить, задав свойство `operationOptions`. Дополнительные сведения см. в разделе [Варианты операций](#operation-options). |
||||
...
*Примеры поведений для различных запросов*
...
| [**Объединение**](#join-action) | Создает строку из всех элементов в массиве и разделяет эти элементы определенным символом разделителя. | 
| [**Parse JSON**](#parse-json-action) | Создает понятные токены из свойств в содержимом JSON. Затем можно ссылаться на эти свойства, включая токены в приложение логики. | 
...
| Тип действия | ОПИСАНИЕ | 
|-------------|-------------| 
| [**По каждому элементу**](#foreach-action) | Выполняет те же действия в цикле для каждого элемента массива. | 
...
Это определение действие получает значения из переменной с помощью `variables()` функции в выражении и создает эту строку с этими значениями, которые разделяются запятыми: `"1,2,3,4"`
...
|----------|----------|-------|-------------| 
| **Тип** | Yes | "Basic" | Здесь используется тип проверки подлинности Basic. | 
| **Имя пользователя** | Yes | "@parameters('userNameParam')" | Имя пользователя для аутентификации доступа к целевой конечной точке службы |
...
| Свойство | Обязательно для заполнения | Value | ОПИСАНИЕ |
|----------|----------|-------|-------------|
| **Тип** | Yes | "ClientCertificate" | Тип аутентификации, используемый для сертификатов клиента Secure Sockets Layer (SSL). Хотя самозаверяющие сертификаты поддерживаются, они не допускаются для SSL. |
...
| Свойство | Обязательно для заполнения | Value | ОПИСАНИЕ |
|----------|----------|-------|-------------|
| **Тип** | Yes | `ActiveDirectoryOAuth` | Используемый тип проверки подлинности (ActiveDirectoryOAuth) для Azure Active Directory OAuth. |
| **Центр** | Нет  | <*URL-адрес для поставщика токена*> | URL-адрес центра, предоставляющего токен проверки подлинности. |
| **tenant** | Yes | <*ИД клиента*> | Идентификатор клиента Azure Active Directory. |
| **audience** | Yes | <*ресурс для авторизации*> | Ресурс, который вы хотите использовать для авторизации, например, `https://management.core.windows.net/` |
