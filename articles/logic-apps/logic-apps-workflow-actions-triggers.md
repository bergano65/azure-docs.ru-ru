...
| Отклик | Обязательно для заполнения | ОПИСАНИЕ | 
|----------|----------|-------------| 
...
| <*retry-behavior*> | Объект JSON | Настраивает режим повтора для прерывистых сбоев, которые имеют код состояния 408, 429 и 5XX, и для любых исключений при подключении. Дополнительные сведения см. в разделе [Политики повтора](../logic-apps/logic-apps-exception-handling.md#retry-policies). | 
| <*max-runs*> | Целое число  | По умолчанию все экземпляры рабочего процесса запускать одновременно или параллельно до [ограничение по умолчанию](../logic-apps/logic-apps-limits-and-config.md#looping-debatching-limits). Чтобы изменить это ограничение, задав новое значение <*count*>, см. раздел [Изменение параллелизма триггера](#change-trigger-concurrency). | 
| <*max-runs-queue*> | Целое число  | Если рабочий процесс уже выполняется максимальное количество экземпляров, которые можно изменить на основе `runtimeConfiguration.concurrency.runs` свойство, любой новых запусков помещаются в эту очередь [ограничение по умолчанию](../logic-apps/logic-apps-limits-and-config.md#looping-debatching-limits). Чтобы изменить предел по умолчанию, см. раздел [Изменение предела ожидающих запусков](#change-waiting-runs). | 
| <*operation-option*> | String | Поведение по умолчанию можно изменить, задав свойство `operationOptions`. Дополнительные сведения см. в разделе [Варианты операций](#operation-options). | 
|||| 
