...
1. Если вы еще не сделали этого, добавьте триггер или действие [, поддерживающее управляемые удостоверения](logic-apps-securing-a-logic-app.md#managed-identity-authentication).
   ...
   Например, предположим, что вы хотите выполнить [операцию snapshot BLOB](https://docs.microsoft.com/rest/api/storageservices/snapshot-blob) для большого двоичного объекта в учетной записи хранения Azure, где вы ранее настроили доступ к удостоверению, но [соединитель хранилища BLOB-объектов Azure](/connectors/azureblob/) в настоящее время не предоставляет эту операцию. Вместо этого можно использовать [действие HTTP](../logic-apps/logic-apps-workflow-actions-triggers.md#http-action) для выполнения операции или любой другой [службы BLOB-объектов, REST API операции](https://docs.microsoft.com/rest/api/storageservices/operations-on-blobs). Для проверки подлинности действие HTTP может использовать назначенное системой удостоверение, включенное для приложения логики. Действие HTTP также использует эти свойства для указания ресурса, к которому требуется получить доступ.

   * Свойство **URI** указывает URL-адрес конечной точки для доступа к целевому ресурсу Azure. Этот синтаксис URI обычно включает [идентификатор ресурса](../active-directory/managed-identities-azure-resources/services-support-managed-identities.md#azure-services-that-support-azure-ad-authentication) для ресурса или службы Azure.

   * Свойство **headers** указывает любые значения заголовка, которые требуется включить в запрос, например версию API для операции, которую требуется выполнить в целевом ресурсе.

   * Свойство **Queries** указывает любые параметры запроса, которые необходимо включить в запрос, например параметр для конкретной операции или конкретную версию API, если это необходимо.

   Таким образом, для выполнения [операции создания BLOB-объекта моментальных снимков](https://docs.microsoft.com/rest/api/storageservices/snapshot-blob)действие HTTP указывает следующие свойства:

   * **Метод**: определяет операцию `PUT`.

   * **URI**: указывает идентификатор ресурса для файла хранилища BLOB-объектов Azure в глобальной (общедоступной) среде Azure и использует следующий синтаксис:

     `https://{storage-account-name}.blob.core.windows.net/{blob-container-name}/{folder-name-if-any}/{blob-file-name-with-extension}`

   * **Заголовки**: указывает `x-ms-blob-type` как `BlockBlob` и `x-ms-version` в качестве `2019-02-02` для операции BLOB-объекта моментального снимка. Дополнительные сведения см. в статье [заголовки запросов — большой двоичный объект моментальных снимков](https://docs.microsoft.com/rest/api/storageservices/snapshot-blob#request) и [Управление версиями для служб хранилища Azure](https://docs.microsoft.com/rest/api/storageservices/versioning-for-the-azure-storage-services).

   * **Запросы**: указывает `comp` в качестве имени параметра запроса и `snapshot` в качестве значения параметра.
