...
* [Шаблоны Azure Resource Manager](#template-system-logic-app)
...
1. В меню приложения логики в разделе **Параметры** выберите **Идентификатор**. Выберите **система, назначенная** > **при** > **сохранении**. Когда Azure запросит подтверждение, выберите **Да**.
...
Чтобы автоматизировать создание и развертывание ресурсов Azure, таких как приложения логики, можно использовать [шаблоны Azure Resource Manager](../logic-apps/logic-apps-azure-resource-manager-templates-overview.md). Чтобы включить управляемое системой удостоверение для приложения логики в шаблоне, добавьте `identity` объект и `type` дочернее свойство в определение ресурса приложения логики в шаблоне, например:
...
Когда Azure создает определение ресурса приложения логики, `identity` объект получает следующие дополнительные свойства:
...
|-----------------|-------|-------------|
| `principalId` | <*Идентификатор участника*> | Глобальный уникальный идентификатор (GUID) объекта субъекта-службы для управляемого удостоверения, представляющего ваше приложение логики в клиенте Azure AD. Этот идентификатор GUID иногда отображается как "идентификатор объекта" или `objectID`. |
| `tenantId` | <*Azure-AD-идентификатор клиента*> | Глобальный уникальный идентификатор (GUID), представляющий клиент Azure AD, в который теперь входит приложение логики. В клиенте Azure AD субъект-служба имеет то же имя, что и экземпляр приложения логики. |
||||
...
* [Шаблоны Azure Resource Manager](#template-user-identity)
* Azure PowerShell
...
1. Укажите сведения об управляемом удостоверении, а затем выберите **создать**, например:
   ...
   |----------|----------|-------|-------------|
   | **Имя ресурса** | Да | <*назначенное пользователем удостоверение — имя*> | Имя, присваиваемое пользователю удостоверение. В этом примере используется "Fabrikam-User-Assigned-Identity". |
   | **Подписка** | Да | <*Имя подписки Azure*> | Имя подписки Azure, которую нужно использовать. |
   | **Группа ресурсов** | Да | <*Azure-Resource-Group-Name*> | Имя используемой группы ресурсов. Создайте новую группу или выберите существующую. В этом примере создается новая группа с именем "Fabrikam-Managed-identitys-RG". |
   | **Расположение** | Да | <*Azure-Region*> | Регион Azure, в котором хранятся сведения о ресурсе. В этом примере используется регион "Западная часть США". |
   |||||
...
Чтобы автоматизировать создание и развертывание ресурсов Azure, таких как приложения логики, можно использовать [шаблоны Azure Resource Manager](../logic-apps/logic-apps-azure-resource-manager-templates-overview.md), которые поддерживают [для проверки подлинности назначенные пользователем удостоверения](../active-directory/managed-identities-azure-resources/how-to-manage-ua-identity-arm.md). В `resources` разделе шаблона для определения ресурса приложения логики требуются следующие элементы:

* `identity` Объект со свойством `type` , для которого задано значение.`UserAssigned`

* Дочерний `userAssignedIdentities` объект, указывающий идентификатор ресурса удостоверения, который является другим дочерним объектом, `principalId` имеющим `clientId` свойства и.

В этом примере показано определение ресурса приложения логики для запроса HTTP-размещения и включает непараметризованный `identity` объект. Ответ на запрос размещения и последующие операции GET также имеют следующий `identity` объект:
...
| Property (JSON) | Значение | Описание |
|-----------------|-------|-------------|
| `principalId` | <*Идентификатор участника*> | Глобальный уникальный идентификатор (GUID) для назначаемого пользователем управляемого удостоверения в клиенте Azure AD |
| `clientId` | <*Идентификатор клиента*> | Глобальный уникальный идентификатор (GUID) для нового удостоверения приложения логики, используемого для вызовов во время выполнения |
||||

Если шаблон также содержит определение ресурса управляемого удостоверения, можно параметризовать `identity` объект. В этом примере показано, как `userAssignedIdentities` дочерний `userAssignedIdentity` объект ссылается на переменную, определенную `variables` в разделе шаблона. Эта переменная ссылается на идентификатор ресурса для назначенного пользователю удостоверения.
...
| Property (JSON) | Значение | Описание |
|-----------------|-------|-------------|
| `tenantId` | <*Azure-AD-идентификатор клиента*> | Глобальный уникальный идентификатор (GUID), представляющий клиент Azure AD, в котором теперь пользовательское удостоверение является членом. В клиенте Azure AD субъект-служба имеет то же имя, что и имя удостоверения, назначенное пользователем. |
| `principalId` | <*Идентификатор участника*> | Глобальный уникальный идентификатор (GUID) для назначаемого пользователем управляемого удостоверения в клиенте Azure AD |
| `clientId` | <*Идентификатор клиента*> | Глобальный уникальный идентификатор (GUID) для нового удостоверения приложения логики, используемого для вызовов во время выполнения |
||||
...
1. В меню ресурса выберите**назначения ролей** **управления доступом (IAM)** > , где можно проверить текущие назначения ролей для этого ресурса. На панели инструментов выберите **Добавить** > **Добавление назначения ролей**.
...
1. Если вы еще не сделали этого, добавьте [триггер или действие, поддерживающее управляемые удостоверения](logic-apps-securing-a-logic-app.md#managed-identity-authentication).
   ...
   |----------|----------|---------------|-------------|
   | **Method** | Да | `PUT`| Метод HTTP, используемый операцией BLOB-объекта моментального снимка |
   ...
   | **Заголовки** | Да, для службы хранилища Azure | `x-ms-blob-type` = `BlockBlob` <p>`x-ms-version` = `2019-02-02` | Значения `x-ms-blob-type` заголовка `x-ms-version` и, необходимые для операций службы хранилища Azure. <p><p>**Важно**. в исходящем триггере HTTP и запросах действий для службы хранилища Azure для заголовка `x-ms-version` требуется свойство и версия API для операции, которую требуется выполнить. <p>Дополнительные сведения см. в следующих статьях: <p><p>- [Заголовки запроса — большой двоичный объект моментальных снимков](https://docs.microsoft.com/rest/api/storageservices/snapshot-blob#request) <br>- [Управление версиями для служб хранилища Azure](https://docs.microsoft.com/rest/api/storageservices/versioning-for-the-azure-storage-services#specifying-service-versions-in-requests) |
...
1. Для некоторых триггеров и действий также отображается свойство **аудитория** для задания идентификатора целевого ресурса. Задайте для свойства **аудиторию** [идентификатор ресурса для целевого ресурса или службы](../active-directory/managed-identities-azure-resources/services-support-managed-identities.md#azure-services-that-support-azure-ad-authentication). В противном случае по умолчанию свойство **аудитория** использует идентификатор `https://management.azure.com/` ресурса, который является идентификатором ресурса для Azure Resource Manager.
   ...
   В этом примере свойству `https://storage.azure.com/` **аудитории** присваивается значение, чтобы маркеры доступа, используемые для проверки подлинности, были действительны для всех учетных записей хранения. Однако можно также указать URL-адрес `https://fabrikamstorageaccount.blob.core.windows.net`корневой службы для конкретной учетной записи хранения.
...
* [Шаблоны Azure Resource Manager](#template-disable)
* Azure PowerShell
...
1. В меню приложения логики в разделе **Параметры**выберите **удостоверение**, а затем выполните действия для своего удостоверения.
   ...
   * Выберите **система, назначенная** > **при** > **сохранении**. Когда Azure запросит подтверждение, выберите **Да**.
...
Если вы создали управляемое удостоверение приложения логики с помощью шаблона Azure Resource Manager, задайте для `identity` `type` `None`дочернего свойства объекта значение. Для управляемого системой удостоверения это действие также удаляет идентификатор субъекта из Azure AD.
