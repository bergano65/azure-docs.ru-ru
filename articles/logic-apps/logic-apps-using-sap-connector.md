...
ms.locfileid: "65136786"
---
# <a name="connect-to-sap-systems-from-azure-logic-apps"></a>Подключение к системам SAP из Azure Logic Apps

В этой статье показано, как доступ к SAP локальным ресурсам из внутри приложения логики с помощью соединителя SAP. Этот соединитель обрабатывает с SAP для классической освобождает таких r/3, ECC систем на предприятии. Соединитель также поддерживает интеграцию с SAP на новой HANA на основе систем SAP, таких как s/4 HANA, везде, где они размещаются - в локальной среде или в облаке.
Соединитель SAP поддерживает интеграцию сообщений или данных и из систем на основе SAP Netweaver через промежуточный документа (IDoc) интерфейса программирования приложений бизнеса (BAPI) или удаленный вызов функции (RFC).

Соединитель SAP использует <a href="https://support.sap.com/en/product/connectors/msnet.html">библиотека соединителя SAP .NET (NCo)</a> и предоставляет эти операции или действия:
...
Для всех указанных выше операций соединитель SAP поддерживает обычную проверку подлинности через имя пользователя и пароль. Соединитель также поддерживает <a href="https://help.sap.com/doc/saphelp_nw70/7.0.31/en-US/e6/56f466e99a11d1a5b00000e835363f/content.htm?no_cache=true"> Communications Network (SNC)</a>, который можно использовать для SAP Netweaver единого входа или для обеспечения дополнительной безопасности, предоставляемые на них продуктами обеспечения безопасности. 
...
* Если вы используете SNC с Single Sign-On (SSO), а затем убедитесь, что шлюз работает в качестве пользователя, который сопоставлен с пользователя SAP. Чтобы изменить учетную запись по умолчанию, выберите **изменить учетную запись** и введите учетные данные пользователя.

   ![Изменение учетной записи шлюза](./media/logic-apps-using-sap-connector/gateway-account.png)

* Если вы включаете SNC с помощью программного обеспечения безопасности, скопируйте библиотека SNC или файлы на том же компьютере, где установлен шлюз. Некоторые примеры продуктов SNC <a href="https://help.sap.com/saphelp_nw74/helpdata/en/7a/0755dc6ef84f76890a77ad6eb13b13/frameset.htm">sapseculib</a>, Kerberos, NTLM, и т. д.
...
2. В поле поиска введите слово sap в качестве фильтра. В списке триггеров выберите триггер: **When a message is received from SAP** (При получении сообщения от SAP)
...
1. В конструкторе приложений логики выберите под триггером пункты **Новый шаг** > **Добавить действие**. В поле поиска введите resource manager в качестве условия фильтра. Выбор действия **Create or update a resource** (Создать или изменить группу ресурсов).
...
* Библиотека SNC, который обеспечивает дополнительную безопасность функции был установлен в той же машине, что шлюз данных. Ниже перечислены некоторые примеры этих <a href="https://help.sap.com/saphelp_nw74/helpdata/en/7a/0755dc6ef84f76890a77ad6eb13b13/frameset.htm">sapseculib</a>, Kerberos, NTLM, и т. д.
...
Чтобы включить SNC для ваших запросов, или из системы SAP, выберите **использовать SNC** флажок в подключении SAP и предоставьте следующие свойства:
   ...
   | Свойство   | ОПИСАНИЕ |
   |------------| ------------|
   | **Библиотека SNC** | Имя библиотеки SNC или путь относительно расположения установки NCo или абсолютный путь. Как пример sapsnc.dll или.\security\sapsnc.dll или c:\security\sapsnc.dll  | 
   ...
   | **Меня зовут SNC** | В большинстве случаев это можно опустить. Установленное решение SNC обычно знает имени SNC. Только для решений, поддерживающих «удостоверения» может потребоваться указать идентификатор, который должен использоваться для этого конкретного назначения/сервера. |
   | **Имя партнера SNC** | Имя серверной части SNC |
   | **Качество SNC защиты** | Качество обслуживания, используемый для обмена данными SNC этого конкретного назначения/сервера. Значение по умолчанию определяется внутренней системе. Максимальное значение определяется по продуктам безопасности, используемый для SNC |
   |||
