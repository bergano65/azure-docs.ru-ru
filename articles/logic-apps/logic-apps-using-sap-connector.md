...
ms.locfileid: "76904658"
---
# <a name="connect-to-sap-systems-from-azure-logic-apps"></a>Подключение к системам SAP из Azure Logic Apps

> [!IMPORTANT]
> Более ранние версии соединителей сервера приложений SAP и сервера сообщений SAP запланированы до 29 февраля 2020. Текущий соединитель SAP объединяет эти предыдущие соединители SAP, чтобы не менять тип подключения, полностью совместимый с предыдущими соединителями, предоставляет множество дополнительных возможностей и по-прежнему использует библиотеку соединителей SAP .NET ( SAP NCo).
>
...
## <a name="send-message-to-sap"></a>Отправить сообщение в SAP
...
1. Если подключение уже существует, перейдите к следующему шагу, чтобы можно было настроить действие SAP. Тем не менее, если появится запрос на ввод сведений о подключении, предоставьте сведения, чтобы можно было создать подключение к локальному серверу SAP.
   ...
   1. В разделе " **шлюз данных** " на вкладке " **Подписка**" сначала выберите подписку Azure для ресурса шлюза, созданного в портал Azure для установки шлюза. 
   
   1. В разделе **шлюз подключений**выберите ресурс шлюза.
...
Ниже приведен пример, демонстрирующий извлечение отдельных iDoc из пакета с помощью [функции`xpath()`](./workflow-definition-language-functions-reference.md#xpath):
...
1. Чтобы извлечь отдельные IDOC, добавьте шаг, который создает переменную массива и сохраняет коллекцию IDOC, используя другое выражение `xpath()`:
   ...
   Каждый IDOC должен включать корневое пространство имен, что является причиной того, что содержимое файла упаковывается внутри элемента `<Receive></Receive` вместе с корневым пространством имен перед отправкой IDOC в подчиненное приложение, или SFTP-сервер в этом случае.
