...
ms.locfileid: "67458931"
---
# <a name="connect-to-sap-systems-from-azure-logic-apps"></a>Подключение к системам SAP из Azure Logic Apps

В этой статье показано, как доступ к SAP локальным ресурсам из внутри приложения логики с помощью соединителя SAP. Соединитель может работать с выпусками классической компании SAP, например r/3 и ECC систем локально. Соединитель также поддерживает интеграцию с систем новой SAP на базе HANA компании SAP, таких как s/4 HANA ли они размещены в локальной или в облаке. Соединитель SAP поддерживает интеграцию сообщений или данных и из систем на основе SAP NetWeaver через промежуточный документа (IDoc), интерфейс программирования приложений бизнеса (BAPI) или удаленный вызов функции (RFC).
...
Для выполнения этих операций соединитель SAP поддерживает обычную проверку подлинности через имена пользователей и пароли. Соединитель также поддерживает [Communications Network (SNC)](https://help.sap.com/doc/saphelp_nw70/7.0.31/e6/56f466e99a11d1a5b00000e835363f/content.htm?no_cache=true). SNC может использоваться для SAP NetWeaver единого входа (SSO) или для обеспечения дополнительной безопасности, предоставляемые на них продуктами обеспечения безопасности.

Соединитель SAP интегрируется с локальными системами SAP через [локальный шлюз данных](../logic-apps/logic-apps-gateway-connection.md). В сценариях отправки например, когда сообщение отправляется из приложения логики к системе SAP, шлюз данных выступает в качестве клиента RFC и перенаправляет запросы, полученные из приложения логики для SAP.
Аналогичным образом, в получения сценариев, шлюз данных действует как сервер RFC, получает запросы от SAP и пересылает их в приложение логики.
...
* Приложение логики, из которого требуется осуществлять доступ к системе SAP, и триггер, который запускает рабочий процесс приложения логики. Если вы знакомы с приложениями логики, см. в разделе [что такое Azure Logic Apps?](../logic-apps/logic-apps-overview.md) и [краткое руководство: Создание первого автоматизированного рабочего процесса с помощью Azure Logic Apps на портале Azure](../logic-apps/quickstart-create-first-logic-app-workflow.md).
* Ваш [сервера приложений SAP](https://wiki.scn.sap.com/wiki/display/ABAP/ABAP+Application+Server) или [сервер сообщений SAP](https://help.sap.com/saphelp_nw70/helpdata/en/40/c235c15ab7468bb31599cc759179ef/frameset.htm).
* Скачайте и установите последнюю версию [локального шлюза данных](https://www.microsoft.com/download/details.aspx?id=53127) на любом компьютере в локальной среде. Не забудьте настроить шлюз на портале Azure, прежде чем продолжить. Шлюз помогает безопасного доступа к локальным данным и ресурсам. Дополнительные сведения см. в разделе [Установка локального шлюза данных для Azure Logic Apps](../logic-apps/logic-apps-gateway-install.md).
* Если вы используете SNC с единым ВХОДОМ, убедитесь, что шлюз работает в качестве пользователя, который сопоставлен с пользователя SAP. Чтобы изменить учетную запись по умолчанию, выберите **изменить учетную запись**и введите учетные данные пользователя.

  ![Изменение учетной записи шлюза](./media/logic-apps-using-sap-connector/gateway-account.png)

* Если вы включаете SNC с помощью программного обеспечения безопасности, скопируйте библиотека SNC или файлы на том же компьютере, где установлен шлюз. Некоторые примеры продуктов SNC [sapseculib](https://help.sap.com/saphelp_nw74/helpdata/en/7a/0755dc6ef84f76890a77ad6eb13b13/frameset.htm), Kerberos и NTLM.
* Скачайте и установите последнюю версию библиотеки клиента SAP, которая в данный момент [соединитель SAP (NCo) 3.0.21.0 для Microsoft .NET Framework 4.0 и Windows 64 bit (x64)](https://softwaredownloads.sap.com/file/0020000001865512018), на одном компьютере с локальным шлюзом данных. Необходимо установить эту или более позднюю версию по следующим причинам:
  ...
  * Более ранние версии SAP NCo может стать взаимоблокировкой, при отправке сообщения более одного IDoc в то же время. Эта проблема блокирует все более поздние версии сообщения, отправляемые назначения SAP, которые вызывает сообщения для времени ожидания.
  ...
  * Как служба узла шлюза управления данными, так и Microsoft SAP Adapter используют .NET Framework 4.5. SAP NCo для .NET Framework 4.0 работает с процессами, использующими среду выполнения .NET версии от 4.0 до 4.7.1. NCo SAP для платформы .NET Framework 2.0 работает с процессами, которые используют среду выполнения .NET 2.0 до версии 3.5, но больше не работает с последнюю версию локального шлюза данных.

* Содержимое сообщения, которое можно отправить ваш сервер SAP, таких как образец файла IDoc, должно быть в формате XML и содержать пространство имен для SAP действие, которое вы хотите использовать.

<a name="add-trigger"></a>

## <a name="send-to-sap"></a>Отправка в SAP

В этом примере используется приложение логики, которое может запускаться по HTTP-запросу. Приложение логики отправляет IDoc на сервер SAP и возвращает ответ запросившей стороне, вызвавшей приложение логики. 

### <a name="add-an-http-request-trigger"></a>Добавить триггер HTTP-запроса
...
1. В поле поиска введите фильтр "http-запрос". Из **триггеры** выберите **получено при HTTP-запроса**.

   ![Добавление триггера HTTP-запроса](./media/logic-apps-using-sap-connector/add-trigger.png)

1. Теперь сохраните приложение логики, чтобы URL-адрес конечной точки можно создать для приложения логики. На панели инструментов конструктора выберите **Сохранить**.
...
### <a name="add-an-sap-action"></a>Добавить действие SAP
...
1. В поле поиска введите слово sap в качестве фильтра. Из **действия** выберите **отправить сообщение в SAP**.
  
   ![Выбор действия отправки в SAP](media/logic-apps-using-sap-connector/select-sap-send-action.png)

   Или вместо поиска, выберите **Enterprise** и выберите действие SAP.
...
1. Если потребуется ввести сведения о подключении, создайте подключение SAP. В противном случае если подключение уже существует, перейдите к следующему шагу, можно настроить действие SAP.

   **Создание подключения к локальным SAP**
...
1. Теперь найдите и выберите действие с сервера SAP.
    ...
    1. В **действие SAP** выберите значок папки. В списке файлов найдите и выберите сообщение SAP, которое следует использовать. Переходите по списку, используя стрелки.
       ...
       > [!TIP]
       > Укажите значение для **действие SAP** через редактор выражений. Таким образом, можно использовать те же действия для разных типов сообщений.
    ...
    1. Щелкните внутри поля **Входное сообщение**, чтобы отобразился список динамического содержимого. В этом списке в разделе **При получении HTTP-запроса** выберите поле **Текст**.
       ...
       Этот шаг включает содержимое текста из триггера HTTP-запрос и отправляет, выходные данные серверу SAP.

       ![Выбор поля "Текст"](./media/logic-apps-using-sap-connector/SAP-app-server-action-select-body.png)

       После завершения этого действия SAP выглядит следующим образом:

       ![Готовое действие SAP](./media/logic-apps-using-sap-connector/SAP-app-server-complete-action.png)

1. Сохраните приложение логики. На панели инструментов конструктора выберите **Сохранить**.

<a name="add-response"></a>

### <a name="add-an-http-response-action"></a>Добавить действие ответа HTTP
...
1. В поле поиска введите фильтр "ответ". Из **действия** выберите **ответа**.
...
1. На панели инструментов конструктора выберите **запуска**. Так приложение логики будет запущено вручную.

1. Вызвать приложение логики, отправив запрос HTTP POST на URL-адрес в триггер HTTP-запроса.
Включить при обработке вашего запроса содержимого сообщения. Для отправки запроса можно использовать, например, средство [Postman](https://www.getpostman.com/apps).
...
Теперь вы создали приложение логики, который может взаимодействовать с сервером SAP. Теперь с настроенным подключением SAP для приложения логики можно изучить другие доступные действия SAP, например BAPI и RFC.

## <a name="receive-from-sap"></a>Получение из SAP

В этом примере приложение логики, которое активируется, когда приложение получает сообщение от системы SAP.

### <a name="add-an-sap-trigger"></a>Добавить триггер SAP

1. На портале Azure создайте пустое приложение логики. Откроется конструктор приложений логики.

1. В поле поиска введите слово sap в качестве фильтра. Из **триггеры** выберите **при получении сообщения от SAP**.

   ![Добавление триггера SAP](./media/logic-apps-using-sap-connector/add-sap-trigger.png)

   Также можно перейти к **Enterprise** и выберите триггер:

   ![Добавление триггера SAP Enterprise вкладки](./media/logic-apps-using-sap-connector/add-sap-trigger-ent-tab.png)

1. Если потребуется ввести сведения о подключении, создайте подключение SAP. Если подключение уже существует, по-прежнему к следующему шагу, можно настроить действие SAP.

   **Создание подключения к локальным SAP**

   - Укажите сведения для подключения к серверу SAP. В качестве свойства **Шлюз данных** выберите шлюз данных, созданный на портале Azure при установке шлюза.
...
> [!NOTE]
> Триггер SAP не опрашивающий триггер, но вместо этого является триггером на базе веб-перехватчика. Триггер вызывается из шлюза, только если существует сообщение, поэтому опрос не требуется.

### <a name="test-your-logic-app"></a>Тестирование приложения логики

1. Чтобы активировать приложение логики, отправьте сообщение из системы SAP.

1. В меню приложения логики, выберите **Обзор**. Просмотрите **журнал запусков** для любого новых запусков приложения логики.
...
В этом примере используется приложение логики, которое может запускаться по HTTP-запросу. Действие SAP отправляет запрос к системе SAP для формирования схемы для BAPI и IDoc. Схемы, возвращаемые в ответе, передаются в учетную запись интеграции с помощью соединителя Azure Resource Manager.
...
### <a name="add-an-sap-action-to-generate-schemas"></a>Добавить действие SAP для создания схемы

1. В конструкторе приложений логики, см. в разделе триггера выберите **новый шаг**.

   ![Выберите «Новый шаг»](./media/logic-apps-using-sap-connector/add-action.png)

1. В поле поиска введите слово sap в качестве фильтра. Из **действия** выберите **создать схемы**.
  
   ![Выбор действия отправки в SAP](media/logic-apps-using-sap-connector/select-sap-schema-generator-action.png)

   Или вы также можете **Enterprise** и выберите действие SAP.
...
1. Укажите путь к артефакту, для которого требуется создать схему.
   ...
   Дополнительные сведения о действии SAP см. в разделе [сообщения схемы для операций IDOC](https://docs.microsoft.com/biztalk/adapters-and-accelerators/adapter-sap/message-schemas-for-idoc-operations).

1. Сохраните приложение логики. На панели инструментов конструктора выберите **Сохранить**.

### <a name="test-your-logic-app"></a>Тестирование приложения логики

1. На панели инструментов конструктора выберите **запуска** для активации выполнения для приложения логики.
...
### <a name="upload-schemas-to-an-integration-account"></a>Отправить схемы в учетной записи интеграции
...
1. В поле поиска введите «Resource Manager» в качестве фильтра. Выберите **создания или обновления ресурса**.
...
1. На панели инструментов конструктора выберите **запуска** вручную активировать приложение логики.

1. После успешного запуска, перейти в учетную запись интеграции и проверять, что созданные схемы существуют.

## <a name="enable-secure-network-communications"></a>Включить безопасное сетевое взаимодействие

Перед началом работы убедитесь, что вы выполнили ранее описанных [предварительные требования](#pre-reqs):

* Локальный шлюз данных установлен на компьютере, который находится в той же сети, что вашей системы SAP.
* Чтобы единый вход шлюз работает как пользователь, сопоставленный с пользователь SAP.
* Библиотека SNC, который обеспечивает дополнительную безопасность функции устанавливается на том же компьютере, что и шлюз данных. Некоторые примеры включают [sapseculib](https://help.sap.com/saphelp_nw74/helpdata/en/7a/0755dc6ef84f76890a77ad6eb13b13/frameset.htm), Kerberos и NTLM.

Чтобы включить SNC для ваших запросов, или из системы SAP, выберите **использовать SNC** в подключении к SAP и предоставить эти свойства:

   ![Конфигурирование соединений SNC SAP в соединении](media/logic-apps-using-sap-connector/configure-sapsnc.png)

   | Свойство | Описание |
   |----------| ------------|
   | **Путь к библиотеке SNC** | Библиотека SNC name или путь относительно расположения установки NCo или абсолютный путь. Примерами являются `sapsnc.dll` или `.\security\sapsnc.dll` или `c:\security\sapsnc.dll`. |
   | **SNC SSO** | При подключении через SNC SNC identity обычно используется для проверки подлинности вызывающего объекта. Другой вариант — для переопределения, чтобы сведения пользователя и пароль могут использоваться для проверки подлинности вызывающего объекта, но по-прежнему шифруется строке. |
   | **Меня зовут SNC** | В большинстве случаев это свойство может быть опущено. Установленное решение SNC обычно знает имени SNC. Только для решений, поддерживающих несколько удостоверений необходимо указать удостоверение для этого конкретного назначения или сервера. |
   | **Имя партнера SNC** | Имя для соединения SNC ПОД серверной части. |
   | **Качество SNC защиты** | Качество обслуживания, используемый для обмена данными SNC определенного места назначения или сервера. Значение по умолчанию определяется внутренней системе. Максимальное значение определяется по продуктам безопасности, используемый для SNC. |
   |||

   > [!NOTE]
   > Не задать переменные среды SNC_LIB и SNC_LIB_64 на компьютере, где у вас есть шлюз данных и библиотека SNC. Если задано, они имеют приоритет над значение библиотека SNC, переданное через соединитель.

<a name="safe-typing"></a>

## <a name="safe-typing"></a>Безопасный ввод

По умолчанию при создании подключения к SAP, строгую типизацию используется для проверки для недопустимых значений, выполняя проверки XML на соответствие схеме. Это поведение может помочь обнаружить проблемы ранее. **Безопасный ввод** параметр доступен для обеспечения обратной совместимости и только проверяет длину строки. Если вы выберете **безопасный ввод**, DATS типа и типа TIMS в SAP, рассматриваются как строки, а не как эквиваленты XML `xs:date` и `xs:time`, где `xmlns:xs="http://www.w3.org/2001/XMLSchema"`. Безопасный ввод влияет на поведение для создания все схемы, отправка сообщений для полезных данных «отправлено» и «получено» ответ и триггер. 

При использовании строгой типизации (**безопасный ввод** не включена), схема сопоставляет типы DATS и TIMS более простые типы XML:
...
Когда сообщения отправляются с **безопасный ввод** включен, DATS и TIMS ответ выглядит следующим образом:
...
* Только один отправить сообщение или вызов SAP работает с tRFC. Шаблон фиксации BAPI, например выполняя несколько вызовов tRFC в одном сеансе, не поддерживается.
* Триггер SAP не поддерживает принимающей пакета IDOC из SAP. Это действие может привести к сбою подключения RFC между системой SAP и шлюзом данных.
* Триггер SAP не поддерживает кластеры шлюза данных. В некоторых случаях отработка отказа узла шлюза данных, который обменивается данными с системами SAP могут отличаться от активного узла, что приводит к непредвиденному поведению. Для отправки сценариев поддерживаются кластеры шлюза данных.
...
* [Подключение к локальным системам](../logic-apps/logic-apps-gateway-connection.md) из Azure Logic Apps.
* Узнайте, как проверка, преобразование и использовать другие операции с сообщениями с [пакета интеграции Enterprise](../logic-apps/logic-apps-enterprise-integration-overview.md).
* Дополнительные сведения о других [соединители Logic Apps](../connectors/apis-list.md).
