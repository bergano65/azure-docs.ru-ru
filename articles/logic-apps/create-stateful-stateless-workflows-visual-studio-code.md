...
Для создания рабочих процессов приложения логики, которые интегрируются между приложениями, данными, облачными службами и системами, вы можете использовать Visual Studio Code и расширение Azure Logic Apps (Предварительная версия) для создания и запуска [рабочих процессов приложения логики без *отслеживания* *состояния* ](#stateful-stateless).
...
Приложения логики, созданные с помощью расширения общедоступной предварительной версии, используют новый тип ресурсов **приложения логики (Предварительная версия)** и на платформе среды выполнения [функций Azure](../azure-functions/functions-overview.md) . Этот новый тип ресурса может включать несколько рабочих процессов и аналогичен типу ресурса **приложение-функция** , который может включать несколько функций.
...
* Создавайте приложения логики для процессов интеграции и автоматизации из [300 + соединителей](/connectors/connector-reference/connector-reference-logicapps-connectors) для приложений и служб типа "программное обеспечение как услуга" (PaaS) и "платформа как услуга", а также соединителей для локальных систем.
  ...
  * Создавайте и развертывайте приложения логики, которые могут выполняться где угодно, так как служба Azure Logic Apps создает строки подключения для подписанного URL-адрес (SAS), которые эти приложения логики могут использовать для отправки запросов в конечную точку среды выполнения облачного подключения. Служба Logic Apps сохраняет эти строки подключения с другими параметрами приложения, что позволяет легко хранить эти значения в Azure Key Vault при развертывании в Azure.
    ...
    > [!NOTE]
    > По умолчанию ресурс **приложения логики (Предварительная версия)** автоматически включает [назначенное системой удостоверение](../logic-apps/create-managed-service-identity.md) для проверки подлинности подключений во время выполнения. Это удостоверение отличается от учетных данных проверки подлинности или строки подключения, используемой при создании соединения. Если отключить это удостоверение, подключения не будут работать во время выполнения.
...
Сейчас создание нового ресурса **приложения логики (Предварительная версия)** недоступно на Mac OS. Однако для Windows или другой операционной системы, такой как Linux, необходимо настроить это требование к хранилищу.
...
Перед созданием приложения логики создайте локальный проект, чтобы можно было управлять приложением логики и развертывать его из Visual Studio Code. Базовый проект очень похож на проект функций Azure, также известный как проект приложения-функции.
...
Затем откройте файл **workflow.js** в конструкторе приложений логики.
...
Прежде чем попытаться открыть файл определения рабочего процесса в конструкторе, если Visual Studio Code работает в Windows или Linux, убедитесь, что эмулятор хранения Azure работает. Для получения дополнительных сведений ознакомьтесь с [предварительными требованиями](#prerequisites).
...
1. В списке расположения найдите и выберите регион Azure, который будет использоваться для создания группы ресурсов и ресурсов. В этом примере используется **Западная Центральная часть США**.

   > [!NOTE]
   > В настоящее время поддерживаются не все регионы, хотя обновления выполняются. Дополнительные сведения см. на странице сведений об [известных проблемах](https://github.com/Azure/logicapps/blob/master/articles/logic-apps-public-preview-known-issues.md)расширения для предварительного просмотра.
...
Приложение логики можно опубликовать как новый ресурс, который автоматически создает дополнительные необходимые ресурсы, такие как [учетная запись хранения Azure, аналогично требованиям приложения-функции](../azure-functions/storage-considerations.md). Вы также можете опубликовать приложение логики в ранее развернутом ресурсе **приложения логики (Предварительная версия)** , который процесс развертывания перезаписывает в Azure.
...
1. Чтобы создать новый ресурс **приложения логики (Предварительная версия)** , выполните следующие действия.
   ...
   1. Укажите для нового приложения логики глобально уникальное имя, которое будет использоваться для ресурса **приложения логики (Предварительная версия)** .
...
Далее вы узнаете, [как найти развернутое приложение логики в портал Azure](#find-manage-deployed-workflows-portal) или [в Visual Studio Code](#find-manage-deployed-workflows-vs-code).
...
Чтобы включить журнал выполнения и мониторинг в развернутом ресурсе **приложения логики (Предварительная версия)** , выполните следующие действия.
...
1. Чтобы открыть развернутое приложение логики в портал Azure, в Visual Studio Code откройте контекстное меню приложения логики и выберите **Открыть на портале**.
...
1. Создайте контейнер DOCKER, используя файл DOCKER для рабочего процесса .NET и выполнив следующую команду:
   ...
   Например, ниже приведен пример файла DOCKER для рабочего процесса .NET, но замените значение <*Storage-Account-connection-string*> строкой подключения вашей учетной записи хранения Azure, которая была сохранена ранее, что выглядит следующим образом:
...
Чтобы упростить отладку рабочего процесса приложения логики без отслеживания состояния, можно включить журнал выполнения для этого рабочего процесса, а затем отключить журнал выполнения по завершении.
...
1. Добавьте `Workflow.<yourWorkflowName>.operationOptions` свойство и задайте для него значение `WithStatelessRunHistory` , например:
...
1. Чтобы отключить журнал выполнения по завершении, удалите `Workflow.<yourWorkflowName>.OperationOptions` свойство и его значение либо задайте для свойства `None` .
...
1. В поле **значение** введите следующее значение: `WithStatelessRunHistory`
   ...
   ![Снимок экрана, на котором показан ресурс портал Azure и приложения логики (Предварительная версия) с параметром "Конфигурация" > "новый параметр приложения" < "Добавить/изменить параметр приложения", а для параметра "Workflow. <Йоурворкфловнаме>Оператионоптионс" задано значение "Висстателессрунхистори".](./media/create-stateful-stateless-workflows-visual-studio-code/stateless-operation-options-run-history.png)
...
|-----------------|----------------|----------------|
| С отслеживанием состояния | С отслеживанием состояния | Асинхронный или синхронный с `operationOptions=DisableSynPattern` параметром |
...
* Создание нового ресурса **приложения логики (Предварительная версия)** сейчас недоступно на Mac OS.

* В этой предварительной версии не поддерживаются пользовательские соединители, триггеры на основе веб-перехватчиков и триггер скользящего окна.
...
* В портал Azure вы не можете создавать новые приложения логики с новым типом ресурсов **приложения логики (Предварительная версия)** . Эти приложения логики можно создавать только в Visual Studio Code. Однако после развертывания приложений логики с этим типом ресурса из Visual Studio Code в Azure можно [Добавить новые рабочие процессы в эти приложения логики](#add-workflows).

* Планы размещения **потребления** не поддерживаются для развертывания приложений логики.
