...
ms.locfileid: "94981055"
---
# <a name="create-stateful-or-stateless-workflows-in-visual-studio-code-with-the-azure-logic-apps-preview-extension"></a>Создание рабочих процессов с отслеживанием или без отслеживания состояния в Visual Studio Code с помощью расширения Azure Logic Apps (предварительная версия)
...
* Запускайте и отлаживать приложения логики локально в среде разработки Visual Studio Code.
...
* Чтобы запустить рабочий процесс, используйте [встроенный запрос, HTTP, концентраторы событий или триггер служебной шины](../connectors/apis-list.md), которые работают в собственном режиме со средой выполнения Logic Apps. Сейчас в этой предварительной версии не поддерживаются [Корпоративные соединители](../connectors/apis-list.md#enterprise-connectors), [триггеры локального шлюза данных](../connectors/apis-list.md#on-premises-connectors), триггеры на основе веб-перехватчика, триггеры скользящего окна, [пользовательские соединители](../connectors/apis-list.md#custom-apis-and-connectors), учетные записи интеграции, их артефакты и [их соединители](../connectors/apis-list.md#integration-account-connectors) . Функция "вызов функции Azure" недоступна, поэтому сейчас используйте *действие* HTTP, чтобы вызвать URL-адрес запроса для функции Azure.

  За исключением указанных ранее триггеров, рабочие процессы с *отслеживанием состояния* могут использовать триггеры и действия для [управляемых соединителей](../connectors/apis-list.md#managed-api-connectors), которые развертываются в Azure, а также встроенные триггеры и действия, выполняемые изначально с помощью среды выполнения Logic Apps. Однако рабочие процессы без *отслеживания состояния* в настоящее время поддерживают только *действия* для управляемых соединителей, а не триггеров. Хотя вы можете включить соединители в Azure для рабочего процесса без отслеживания состояния, конструктор не будет отображать триггеры управляемых соединителей для выбора.
...
1. Проверьте версии, установленные на компьютере, выполнив следующую команду:
   ...
   Убедитесь, что вы явно добавили **global.jsв** файл проекта в корневом расположении из Visual Studio Code. В противном случае конструктор не откроется.
...
1. В Visual Studio Code вернитесь на страницу обзора рабочего процесса.
   ...
   | **Waiting** | Выполнение не началось или приостановлено, например, из-за более раннего экземпляра рабочего процесса, который еще работает. |
   |||
...
1. Чтобы просмотреть состояние каждого шага в определенном запуске, а также входных и выходных данных этого шага, нажмите кнопку с многоточием (**...**) для этого запуска и выберите команду **отобразить выполнение**.
   ...
   | Waiting | ![Значок состояния "ожидание" действия][waiting-icon] | Применяется к действию веб-перехватчика, ожидающему входящего запроса от вызывающего. |
   ||||
