...
1. В разделе **Выбор действия**в поле поиска введите `as2 decode`и выберите **декодирование AS2 (v2)** .
...
1. В качестве значения свойства **заголовки сообщений** введите все заголовки, необходимые для действия AS2, которое описывается `headers` содержимым, полученным ТРИГГЕРОМ HTTP-запроса.
...
1. Теперь укажите ответы, которые будут возвращать результат операции **декодирования AS2** .
   ...
   1. В случае успеха при выполнении действия **декодирования AS2** в форме **If true** выберите **Добавить действие**. В разделе **Выбор действия**в поле поиска введите `response`и выберите **ответ**.
   ...
   1. В случае сбоя действия **декодирования AS2** в фигуре **If false** выберите **Добавить действие**. В разделе **Выбор действия**в поле поиска введите `response`и выберите **ответ**. Настройте действие **ответ** , чтобы вернуть требуемое состояние и ошибку.
...
1. В разделе **Выбор действия**в поле поиска введите `x12 decode`и выберите **декодировать сообщение X12**.
...
* [Мониторинг, создание файлов SFTP и управление ими с помощью SSH и Azure Logic Apps](../connectors/connectors-sftp-ssh.md)
