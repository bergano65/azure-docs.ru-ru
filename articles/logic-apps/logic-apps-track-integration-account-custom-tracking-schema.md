...
| repeatItemScopeName | Да | Строка | Повторение имени элемента, если действие находится внутри `foreach`цикла `until` или |
| repeatItemIndex | Да | Целое число | Указывает, что действие находится внутри цикла `foreach` или `until` и является повторяющимся индексом элемента. |
...
| eventTime | Да | DateTime | Время события в формате UTC: *гггг-мм-ddThh: mm: SS. 00000z)* |
