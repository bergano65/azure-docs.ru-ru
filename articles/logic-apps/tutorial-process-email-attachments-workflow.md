---
title: Создание рабочих процессов с помощью Функций Azure — Azure Logic Apps
description: Учебник. Автоматизирование рабочих процессы для обработки электронных писем и вложений в Azure Logic Apps, службе хранилища Azure и Функциях Azure
services: logic-apps
...
ms.locfileid: "72679179"
---
# <a name="tutorial-automate-handling-emails-and-attachments-with-azure-logic-apps"></a>Руководство по Обработка сообщений электронной почты и вложений с помощью Azure Logic Apps
...
1. Прежде чем создать контейнер хранилища, [создайте учетную запись хранения](../storage/common/storage-quickstart-create-account.md), указав приведенные параметры на вкладке **Основы** на портале Azure.
   ...
   На вкладке **Дополнительно** выберите следующий параметр.
...
1. По завершении выберите **Отзыв и создание**.

1. После развертывания учетной записи хранения в Azure получите ключ доступа к ней:
...
1. Перед созданием функции [создайте приложение-функцию](../azure-functions/functions-create-function-app-portal.md) с такими параметрами:
   ...
   | Параметр | Значение | ОПИСАНИЕ |
   | ------- | ----- | ----------- |
   | **Имя приложения** | <*function-app-name*> | В этом примере описательное, глобально уникальное имя приложения-функции — CleanTextFunctionApp, поэтому укажите другое имя, например MyCleanTextFunctionApp |
   ...
   Если приложение-функция не открывается автоматически после развертывания, найдите приложение на [портале Azure](https://portal.azure.com). 
   В главном меню Azure выберите **Приложения-функции** и выберите приложение-функцию.
...
2. В разделе **Приложения-функции** разверните приложение-функцию (в этом примере — CleanTextFunctionApp) и выберите **Функции**. На панели инструментов функций выберите **Новая функция**.

   ![Создание функции](./media/tutorial-process-email-attachments-workflow/function-app-new-function.png)

3. В разделе **Choose a template below or go to the quickstart** (Выберите шаблон ниже или перейдите к краткому руководству) выберите шаблон **Триггер HTTP**.
...
3. Теперь укажите критерии, используемые триггером для фильтрации новых сообщений электронной почты.
   ...
   1. Укажите эти параметры для проверки электронных сообщений.
...
2. В разделе **Выберите действие** введите "condition" в поле поиска. Выбор действия **Condition - Control** (Условие — контроль).
   ...
   В строке заголовка условия нажмите кнопку **многоточия** ( **…** ) и выберите **Переименовать**.
...
3. Выберите ранее созданное приложение-функцию, которое в этом примере называется CleanTextFunctionApp:
...
1. В разделе **Выберите действие** введите "for each" в поле поиска. Выбор действия **For each — Control** (for each — управление).
