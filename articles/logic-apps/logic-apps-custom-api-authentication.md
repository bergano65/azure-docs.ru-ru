---
title: Добавление аутентификации для обеспечения безопасности вызовов в пользовательские AIS
description: Как настроить аутентификацию для повышения безопасности вызовов пользовательских AI из приложений Azure Logic
services: logic-apps
...
---
# <a name="increase-security-for-calls-to-custom-apis-from-azure-logic-apps"></a>Повысить безопасность вызовов пользовательских AI из приложений логики Azure

Чтобы повысить безопасность вызовов AIS, можно настроить активную проверку подлинности Azure Active Directory (Azure AD) через портал Azure, чтобы не обновлять код. Вы можете также применить обязательную проверку подлинности с помощью кода API.

## <a name="authentication-options-for-your-api"></a>Параметры аутентификации для API

Вы можете улучшить безопасность вызовов на пользовательский API таким образом:
...
2. Убедитесь, что вы открыли каталог своего веб-приложения или приложения API.
   ...
   > [!TIP]
   > Для перехода между каталогами щелкните свой профиль и выберите другой каталог. Или выберите > **каталог коммутатора** **Обзора.**
...
1. Введите пароль и нажмите Enter.
...
Все еще необходимо создать итог приложения Azure AD для вашего веб-приложения или Приложения API, которое отличается от итлина приложения для приложения логики. Чтобы создать удостоверение приложения, выполните шаги на портале Azure, приведенные в части 2. 
...
Откройте определение приложения логики в представлении кода, перейдите к определению действия **HTTP,** найдите раздел **Авторизации** и включите следующие свойства:
...
В разделе **Авторизация** включите следующие свойства:
...
| Свойство | Обязательно | Описание |
| -------- | -------- | ----------- |
| `type` | Да | Тип проверки подлинности. Для сертификатов клиентов TLS/SSL `ClientCertificate`значение должно быть. |
