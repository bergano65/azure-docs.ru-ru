---
title: Создание и Управление переменными для хранения значений
...
Например, это выражение получает элементы из переменной массива, [созданной ранее в этой статье](#append-value) , с помощью функции `variables()`. Функция `string()` возвращает содержимое переменной в строковом формате: `"1, 2, 3, red"`
...
1. Чтобы циклически пройти все вложения, добавьте цикл *for each* .
   ...
   1. В разделе **Выберите действие** выберите **Встроенный**. В поле поиска введите `for each` в качестве фильтра поиска и выберите **для каждого из них**.
...
1. На панели инструментов конструктора приложений логики выберите **выполнить**. При этом приложение логики автоматически перезапустится.
...
При переключении из конструктора в редактор представления кода ниже показано, как действие **задать переменную** отображается в определении приложения логики в формате JSON. Этот пример изменяет текущее значение переменной `Count` на другое значение.
