...
1. Укажите эти сведения о переменной, как описано ниже.
   ...
   | **Type** | Да | <*тип переменной*> | Тип данных для переменной |
...
1. Укажите следующие сведения для увеличения переменной:
   ...
   |----------|----------|-------|--------------|
   | **имя**; | Да | <*имя переменной*> | Имя переменной |
   | **Значение** | Нет | <*Приращение значения*> | Значение, используемое для увеличения переменной. По умолчанию значение равно единице. <p><p>**Совет**. Указывать это значение необязательно, но рекомендуем сделать это, чтобы всегда знать конкретное значение, на которое увеличивается переменная. |
   ||||
...
| **Значение** | Нет | <*Приращение значения*> | Значение для уменьшения переменной. По умолчанию значение равно единице. <p><p>**Совет**. Указывать это значение необязательно, но рекомендуем сделать это, чтобы всегда знать конкретное значение, на которое уменьшается переменная. |
||||| 
