...
Вы можете настроить поведение действия "запуск после", чтобы действие выполнялось, когда состояние предшественника имеет `Succeeded`значение, `Failed` `Skipped` `TimedOut`, или любое из этих состояний. Например, чтобы отправить сообщение электронной почты после того, как `Add_a_row_into_a_table` действие предшественника « `Failed`Excel Online» `Succeeded`будет помечено, вместо этого измените поведение «запуск после», выполнив любое из следующих действий.
...
* В режиме конструктора нажмите кнопку с многоточием (**...**), а затем выберите **настроить выполнение после**.
  ...
  Чтобы указать, что действие выполняется при условии, `Failed` `Skipped` что действие-предшественник помечено как или `TimedOut`, выберите другие состояния:
...
* В представлении кода в определении JSON действия измените `runAfter` свойство, которое следует за следующим синтаксисом:
  ...
  Чтобы указать, что действие выполняется при условии, `Failed` `Skipped` что действие-предшественник помечено как или `TimedOut`, добавьте другие состояния:
...
По умолчанию при завершении всех действий области состояние области отмечается как `Succeeded`. Если Последнее действие в области имеет значение или `Failed` `Aborted`, то состояние области помечается `Failed`как.

Для перехвата исключений в `Failed` области и выполнения действий, которые обрабатывали эти ошибки, можно `runAfter` использовать свойство для `Failed` этой области. Таким образом, при сбое *каких-либо* действий в области и использовании `runAfter` свойства для этой области можно создать одно действие для перехвата сбоев.
...
Чтобы выполнить действие для каждого действия в области, которая `Failed` имеет результат, и для фильтрации массива результатов по невыполненным действиям, можно связать `@result()` выражение с действием [**Фильтрация массива**](logic-apps-perform-data-operations.md#filter-array-action) и цикл [**по каждому**](../logic-apps/logic-apps-control-flow-loops.md) элементу. Можно взять отфильтрованный массив результатов и выполнить действие для каждой ошибки с помощью `For_each` цикла.
...
Ниже приведен пример (для справочных целей) отдельного элемента `@result()` с отображением свойств `name`, `body` и `clientTrackingId`, проанализированных в примере выше. За `For_each` пределами `@result()` действия возвращает массив этих объектов.
