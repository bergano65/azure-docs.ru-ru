...
|-------|------|-------------|
| <*retry-policy-type*> | Строка | Необходимый тип политики повтора: `default`, `none`, `fixed` или `exponential` | 
...
Выражение `@result()` принимает единственный параметр (имя области) и возвращает массив всех результатов действия из этой области. Эти объекты действия включают в себя те же атрибуты, как  **\@actions()** объекта, например время начала действия, время окончания, состояние, входные данные, идентификаторы корреляции и выходные данные. Чтобы отправить контекст любых действий, которые не удалось в пределах области, вы можете легко связать  **\@result()** функционировать с **runAfter** свойство.

Чтобы запустить действие для каждого действия в область, которая содержит **Failed** результата, и чтобы отфильтровать массив результатов до завершившиеся сбоем действия, вы можете связать  **\@result()** с **[ Фильтрация массива](../connectors/connectors-native-query.md)** действия и [ **для каждого** ](../logic-apps/logic-apps-control-flow-loops.md) цикла. Можно настроить выполнение действия для каждой ошибки в отфильтрованном массиве результатов, используя цикл **Foreach**. 
