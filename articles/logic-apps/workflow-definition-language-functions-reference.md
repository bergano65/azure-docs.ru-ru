...
| Параметр | Обязательно | Тип | Описание |
| --------- | -------- | ---- | ----------- |
| <*startIndex*> | Да | Тип Integer | Целочисленное значение, которое является первым элементом массива. |
...
> [!NOTE]
>
> Если вы работаете в представлении кода, используйте escape-символ для двойных кавычек ("): обратную косую черту (\\). 
> Например, при сериализации выражения в виде строки JSON необходимо использовать escape-символы. 
> Но если вы работаете в конструкторе приложений логики или в редакторе выражений, вам не нужно использовать escape-символ, так как в базовое определение автоматически добавляется символ обратной косой черты, например:
> 
...
> 
> Следующие примеры применимы к выражениям, вводимым в редакторе выражений.
