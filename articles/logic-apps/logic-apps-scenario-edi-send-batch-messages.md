...
description: Отправка и получение сообщений EDI по партиям, группам или коллекциям с помощью пакетной обработки в приложениях Azure Logic Apps
services: logic-apps
...
---
# <a name="exchange-edi-messages-as-batches-or-groups-between-trading-partners-in-azure-logic-apps"></a>Обмен сообщениями EDI как партиями или группами между торговыми партнерами в приложениях Azure Logic Apps

В сценариях бизнес-бизнес -2B (B2B) партнеры часто обмениваются сообщениями группами или *партиями.* При создании решения пакетной обработки с помощью Logic Apps можно отправлять сообщения торговым партнерам и обрабатывать эти сообщения в пакетах. В этой статье показано, как выполнять пакетную обработку сообщений EDI, используя X12 в качестве примера, путем создания приложения логики для отправки пакета и приложения логики для получения пакета. 
...
4. Задайте свойства приложения логики для получения пакетов: 
   ...
   | **Граф сообщений** | 10 | Доступно только с условием отправки **На основе количества сообщений** | 
