---
title: Группируйте и запуская действия по сфере
description: Создание областных действий, которые работают на основе статуса группы в приложениях логики Azure
services: logic-apps
...
---
# <a name="run-actions-based-on-group-status-by-using-scopes-in-azure-logic-apps"></a>Запуск действий, основанных на статусе группы, с помощью областей в приложениях логики Azure
...
1. Вопийте на <a href="https://portal.azure.com" target="_blank">порталaz,</a>если вы еще не сделали этого. Создание пустого приложения логики.
...
1. Добавьте действие **Bing Maps — Get route** (Карты Bing — получить маршрут). 
   ...
   1. Если у вас еще нет подключения к Картам Bing, появится запрос на его установку.
      ...
      | **Ключ API** | <*ваш-Бинг-Карты-ключ*> | Введите ключ Карт Bing, полученный ранее. | 
      ||||  
   ...
   1. Настройте действие **Get route** (Получить маршрут), как показано в таблице под этим изображением:
      ...
      | Параметр | Значение | Описание |
      | ------- | ----- | ----------- |
      | **Waypoint 1 (Пункт 1)** | <*Начать*> | Введите начало маршрута. | 
      | **Waypoint 2 (Пункт 2)** | <*Конец*> | Введите место назначения маршрута. | 
      | **Избегать** | None | Введите то, чего нужно избегать на маршруте, например шоссе, платные дороги и т. д. Возможные значения см. в статье [Calculate a Route](https://msdn.microsoft.com/library/ff701717.aspx) (Расчет маршрута). | 
      | **Оптимизировать** | timeWithTraffic | Выберите параметр для оптимизации маршрута, например расстояние, время в пути в соответствии с информацией о текущей загрузке дорог и т. д. В этом примере используется это значение: "timeWithTraffic". | 
      | **Distance unit (Единица расстояния)** | <*Ваше предпочтение*> | Введите единицы измерения расстояния для расчета маршрута. В этом примере используется значение: "Mile" (Миля). | 
      | **Travel mode (Способ передвижения)** | Driving (Движение на автомобиле) | Введите режим движения по маршруту. В этом примере используется значение: "Driving" (Вождение). | 
      | **Transit Date-Time (Дата и время пересадки)** | None | Применяется только для режима движения Transit (Транзит). | 
...
1. [Добавьте условие](../logic-apps/logic-apps-control-flow-conditional-statement.md), чтобы проверить, превышает ли текущее время движения в соответствии с текущей загрузкой определенное время. 
   ...
   1. В самой правой колонке введите это значение сравнения, которое находится в секундах и эквивалентно 10 минут: **600**
...
1. В ветке **Если истинно** добавьте действие "Отправить сообщение" для поставщика электронной почты. 
   ...
   1. После того, как поле разрешится в формате JSON, добавьте **запятую** ()```,```следует номер, ```60``` чтобы преобразовать значение в **трафике Продолжительность трафика** от секунд до минут. 
   ...
   1. После решения выражения добавьте этот текст с ведущим пространством:``` minutes```
...
1. Добавьте область в нужное место в рабочем процессе. Например, чтобы добавить область между существующими шагами в рабочем процессе приложения логики, выполните следующие действия. 
   ...
   Выберите **знак** **+** плюс () > **Добавить действие.**
