...
1. Войдите в <a href="https://portal.azure.com" target="_blank">портал Azure</a>, если вы еще этого не сделали. Создание пустого приложения логики.
...
1. Добавьте действие **Bing Maps — Get route** (Карты Bing — получить маршрут). 
   ...
   1. Если у вас еще нет подключения к Картам Bing, появится запрос на его установку.
      ...
      | **Ключ API** | <*ключ-Bing-Map-Key*> | Введите ключ Карт Bing, полученный ранее. | 
      ||||  
   ...
   1. Настройте действие **Get route** (Получить маршрут), как показано в таблице под этим изображением:
      ...
      | Параметр | Значение | Описание |
      | ------- | ----- | ----------- |
      | **Waypoint 1 (Пункт 1)** | <*запустить*> | Введите начало маршрута. | 
      | **Waypoint 2 (Пункт 2)** | <*конце*> | Введите место назначения маршрута. | 
      ...
      | **Увеличить** | timeWithTraffic | Выберите параметр для оптимизации маршрута, например расстояние, время в пути в соответствии с информацией о текущей загрузке дорог и т. д. В этом примере используется это значение: "timeWithTraffic". | 
      | **Distance unit (Единица расстояния)** | <*настройки*> | Введите единицы измерения расстояния для расчета маршрута. В этом примере используется значение: "Mile" (Миля). | 
...
1. [Добавьте условие](../logic-apps/logic-apps-control-flow-conditional-statement.md), чтобы проверить, превышает ли текущее время движения в соответствии с текущей загрузкой определенное время. 
   ...
   1. В крайнем правом столбце введите значение сравнения, которое находится в секундах и эквивалентно 10 минутам: **600**
...
1. В ветке **Если истинно** добавьте действие "Отправить сообщение" для поставщика электронной почты. 
   ...
   1. После того как поле разрешается в формат JSON, добавьте **запятую** (```,```), за которой следует ```60``` число, чтобы преобразовывать значение в **трафике о длительности трафика** с секунд до минут. 
   ...
   1. После разрешения выражения добавьте этот текст в начальный пробел:``` minutes```
