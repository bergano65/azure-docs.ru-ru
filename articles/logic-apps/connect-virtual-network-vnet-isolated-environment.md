...
* [Виртуальная сеть Azure](../virtual-network/virtual-networks-overview.md). Если у вас нет виртуальной сети, узнайте, [как ее создать](../virtual-network/quick-create-portal.md).
  ...
  * Ваша виртуальная сеть должна иметь четыре *пустые* подсети для создания и развертывания ресурсов в интегрированной среде сценариев. Каждая подсеть поддерживает отдельный компонент Logic Apps для интегрированной среды сценариев. Вы можете создать эти подсети заранее или дождаться создания интегрированной среды сценариев, где вы сможете создавать подсети одновременно. Дополнительные сведения о [требованиях к подсети](#create-subnet).
...
|---------|-----------|-------------------|--------------------|-------------------------|-------|
| Интрасубнет связь | Входящий и исходящий | * | Адресное пространство виртуальной сети с подсетями ISE | Адресное пространство виртуальной сети с подсетями ISE | Требуется для того, чтобы трафик мог проходить в каждой подсети. <p><p>**Важно**. для обмена данными между компонентами в подсетях убедитесь, что открыты все порты в этих подсетях. |
| Взаимодействие между подсетями | Входящий и исходящий | 80, 443 | Виртуальная сеть | Виртуальная сеть | Для связи между подсетями |
| Поток данных из Azure Logic Apps | Исходящие | 80, 443 | Виртуальная сеть | Интернет | Порт зависит от внешней службы, с которой взаимодействует служба Logic Apps. |
...
| Поток данных в Azure Logic Apps | Входящий трафик | 443 | Внутренняя интегрированная среда сценариев: <br>Виртуальная сеть <p><p>Внешняя интегрированная среда сценариев: <br>Интернет | Виртуальная сеть | IP-адрес компьютера или службы, который вызывает триггеры запросов или веб-перехватчики в приложении логики. Закрытие или блокировка этого порта предотвращает вызовы HTTP в Logic Apps с триггерами запросов. |
| Журнал выполнения приложений логики | Входящий трафик | 443 | Внутренняя интегрированная среда сценариев: <br>Виртуальная сеть <p><p>Внешняя интегрированная среда сценариев: <br>Интернет | Виртуальная сеть | IP-адрес компьютера, с которого вы хотите просмотреть журнал выполнения приложения логики. Хотя закрытие или блокирование этого порта не мешает просмотру журнала выполнения, вы не сможете просматривать входные и выходные данные для каждого шага в этом журнале выполнения. |
...
| Служба работоспособности ресурса Azure | Исходящие | 1886 | Виртуальная сеть | AzureMonitor | Для публикации состояния работоспособности в Работоспособность ресурсов |
...
| Подсистема балансировки нагрузки Azure | Входящий трафик | * | AzureLoadBalancer | Виртуальная сеть | |
||||||
...
1. Укажите эти сведения для своей среды, а затем выберите **проверить и создать**, например:
   ...
   * Использует по крайней *мере `/27`* в адресном пространстве, так как для каждой подсети требуется минимум *32 адресов.* Например:
