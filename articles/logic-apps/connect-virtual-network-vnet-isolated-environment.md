...
* [Виртуальная сеть Azure](../virtual-network/virtual-networks-overview.md). Если у вас нет виртуальной сети, узнайте, [как ее создать](../virtual-network/quick-create-portal.md).
  ...
  * Ваша виртуальная сеть должна иметь четыре *пустые* подсети для создания и развертывания ресурсов в среде службы интеграции. Каждая подсеть поддерживает отдельный компонент Logic Apps, используемый в среде службы интеграции. Эти подсети можно создать заранее или дождаться создания среды службы интеграции, где можно создать эти подсети одновременно. Дополнительные сведения о [требованиях к подсети](#create-subnet).

  * Имена подсетей должны начинаться с буквы или знака подчеркивания, и в них не могут использоваться следующие символы: `<`, `>`, `%`, `&`, `\\`, `?`, `/`. 
  
  * Если вы хотите развернуть среду службы интеграции с помощью шаблона Azure Resource Manager, сначала убедитесь, что одна пустая подсеть делегирована в Microsoft.Logic/integrationServiceEnvironment. Это делегирование не нужно делать при развертывании с помощью портала Azure.
  ...
  * [ExpressRoute](../expressroute/expressroute-introduction.md) помогает расширить ваши локальные сети в облако Майкрософт и подключиться к облачным службам Майкрософт через частное подключение, которое обеспечивается поставщиком услуг подключения. В частности, ExpressRoute — это виртуальная частная сеть, которая направляет трафик по частной сети вместо общедоступного Интернета. Приложения логики могут подключаться к локальным ресурсам, которые находятся в одной виртуальной сети при подключении через ExpressRoute или виртуальную частную сеть. 
  
    При использовании ExpressRoute необходимо [создать таблицу маршрутов](../virtual-network/manage-route-table.md), которая содержит такой маршрут, и связать эту таблицу с каждой подсетью, используемой средой службы интеграции.

    **Имя**: <*имя_маршрута*><br>
    **Префикс адреса**: 0.0.0.0/0<br>
    **Определение следующего прыжка**. Интернет

    Эта таблица маршрутов необходима для того, чтобы компоненты Logic Apps могли взаимодействовать с другими зависимыми службами Azure, такими как служба хранилища Azure и база данных SQL Azure.
...
* Если нужно использовать пользовательские DNS-серверы для виртуальной сети Azure, [настройте эти серверы, выполнив следующие действия](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md) перед развертыванием среды службы интеграции в виртуальной сети. Дополнительные сведения об управлении параметрами DNS-сервера см. в разделе [Создание, изменение и удаление виртуальной сети](../virtual-network/manage-virtual-network.md#change-dns-servers).
  ...
  > [!NOTE]
  > При изменении DNS-сервера или его параметров необходимо перезапустить среду службы интеграции, чтобы среда службы интеграции могла получить эти изменения. Дополнительные сведения см. в разделе [Перезапуск среды службы интеграции](../logic-apps/ise-manage-integration-service-environment.md#restart-ISE).
...
1. В области **Среды службы интеграции** выберите **Добавить**.
   ...
   ![Найдите и выберите раздел "Среды службы интеграции"](./media/connect-virtual-network-vnet-isolated-environment/add-integration-service-environment.png)
...
1. Укажите следующие сведения для своей среды, а затем выберите **Просмотр и создание**, как показано здесь:
   ...
   | **Подсети** | Да | <*subnet-resource-list*> | Среде службы интеграции требуется четыре *пустых* подсети для создания и развертывания ресурсов в среде. Для создания каждой подсети [выполните действия, описанные в этой таблице](#create-subnet). |
   |||||
   ...
   **Создание подсети**
   ...
   Чтобы создавать и развертывать ресурсы в среде службы интеграции, эта среда должна иметь четыре *пустых* подсети, не делегированных какой бы то ни было службе. Каждая подсеть поддерживает отдельный компонент Logic Apps, используемый в среде службы интеграции. После создания среды вы *не можете изменить* эти адреса подсети. Каждая подсеть должна отвечать требованиям, перечисленным ниже.

   * Иметь имя, которое начинается с буквы или символа подчеркивания (без чисел) и в котором не используются следующие символы: `<`, `>`, `%`, `&`, `\\`, `?`, `/`.
