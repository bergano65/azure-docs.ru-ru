...
* Убедитесь, что все необходимые порты в виртуальной сети открыты, чтобы трафик мог проходить через интегрированную среду сценариев в этой виртуальной сети.
...
При использовании интегрированной среды сценариев с виртуальной сетью Azure общая проблема установки состоит в наличии одного или нескольких заблокированных портов. Соединители, используемые для создания подключений между средой ISE и целевой системой, также могут иметь собственные требования к портам. Например, если вы обмениваетесь данными с FTP-системой с помощью соединителя FTP, убедитесь, что порт, используемый в системе FTP, доступен, например, порт 21 для отправки команд. Чтобы убедиться в том, что интегрированная среда сценариев остается доступной и может работать правильно, откройте порты, указанные в таблице ниже. В противном случае, если все необходимые порты недоступны, интегрированная среда сценариев перестанет работать.
...
* Если вы создали новую виртуальную сеть и подсети без каких-либо ограничений, вам не нужно настраивать [группы безопасности сети (группы безопасности сети)](../virtual-network/security-overview.md#network-security-groups) в виртуальной сети для управления трафиком между подсетями.

* В существующей виртуальной сети при *необходимости* можно настроить группы безопасности сети, [отфильтруя сетевой трафик между подсетями](../virtual-network/tutorial-filter-network-traffic.md). При выборе этого маршрута в виртуальной сети, в которой необходимо настроить группы безопасности сети, убедитесь, что открыты порты, указанные в таблице ниже. При использовании [правил безопасности NSG](../virtual-network/security-overview.md#security-rules)требуются протоколы TCP и UDP.

* Если ранее в виртуальной сети уже существовали группы безопасности сети или брандмауэры, убедитесь, что открыты порты, указанные в таблице ниже. При использовании [правил безопасности NSG](../virtual-network/security-overview.md#security-rules)требуются протоколы TCP и UDP.

Ниже приведена таблица с описанием портов в виртуальной сети, используемых интегрированной средой ISE, и места их использования. [Теги службы Диспетчер ресурсов](../virtual-network/security-overview.md#service-tags) представляют собой группу ПРЕФИКСОВ IP-адресов, которые позволяют сократить сложность при создании правил безопасности.
...
| Поток данных в Azure Logic Apps | Входящий трафик | 443 | Конечные точки внутреннего доступа: <br>Виртуальная сеть <p><p>Конечные точки внешнего доступа: <br>Интернет <p><p>**Примечание**. Эти конечные точки относятся к параметру конечной точки, [выбранному во время создания интегрированной среды сценариев](#create-environment). Дополнительные сведения см. в разделе [доступ к конечной точке](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md#endpoint-access). | Виртуальная сеть | IP-адрес компьютера или службы, который вызывает любой триггер запроса или веб-перехватчик, существующий в приложении логики. Закрытие или блокировка этого порта предотвращает вызовы HTTP в Logic Apps с триггерами запросов. |
| Журнал выполнения приложений логики | Входящий трафик | 443 | Конечные точки внутреннего доступа: <br>Виртуальная сеть <p><p>Конечные точки внешнего доступа: <br>Интернет <p><p>**Примечание**. Эти конечные точки относятся к параметру конечной точки, [выбранному во время создания интегрированной среды сценариев](#create-environment). Дополнительные сведения см. в разделе [доступ к конечной точке](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md#endpoint-access). | Виртуальная сеть | IP-адрес компьютера, с которого вы просматриваете журнал выполнения приложения логики. Хотя закрытие или блокирование этого порта не мешает просмотру журнала выполнения, вы не сможете просматривать входные и выходные данные для каждого шага в этом журнале выполнения. |
