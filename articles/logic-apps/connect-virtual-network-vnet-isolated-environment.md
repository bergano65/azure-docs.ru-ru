...
| Цель | Направление | Конечные порты | Тег службы источника | Тег целевой службы | Примечания |
|---------|-----------|-------------------|--------------------|-------------------------|-------|
| Интрасубнет связь | Входящий и исходящий | * | - | - | **Важно**. для обмена данными между компонентами в подсетях убедитесь, что открыты все порты в этих подсетях. |
...
| Зависимость от службы хранилища Azure | Исход. | 80, 443 | Виртуальная сеть | Хранилище | |
...
| Azure Load Balancer | Вход. | * | AzureLoadBalancer | Виртуальная сеть | |
||||||
...
1. Укажите следующие сведения для своей среды, а затем выберите **Просмотр и создание**, как показано здесь:
   ...
   | **Дополнительная емкость** | Premium: <br>Да <p><p>Developer: <br>Не применяется | Premium: <br>от 0 до 10 <p><p>Developer: <br>Не применяется | Число дополнительных единиц обработки, используемых для этого ресурса ISE. Сведения о добавлении емкости после создания см. в разделе [Добавление емкости интегрированной среды сценариев](#add-capacity). |
