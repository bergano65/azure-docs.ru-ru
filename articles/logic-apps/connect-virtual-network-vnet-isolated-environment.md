...
1. Укажите следующие сведения для своей среды, а затем выберите **Просмотр и создание**, как показано здесь:
   ...
   | **Дополнительная емкость** | Yes | 0 до 10 | Число единиц дополнительной обработки, используемых для этого ресурса интегрированной среды Сценариев. Чтобы добавить емкость после ее создания, см. в разделе [ISE добавить емкость](#add-capacity). |
   | **Виртуальная сеть** | Yes | <*имя_виртуальной_сети_Azure*> | Виртуальная сеть Azure, в которую нужно внедрить среду, чтобы приложения логики в этой среде могли получить доступ к виртуальной сети. Если у вас нет подключения к сети, [сначала создать виртуальную сеть Azure](../virtual-network/quick-create-portal.md). <p>**Важно!** Внедрение среды службы интеграции можно выполнить *только* при ее создании. |
