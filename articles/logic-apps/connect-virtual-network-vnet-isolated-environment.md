...
При использовании интегрированной среды сценариев с существующей виртуальной сетью общая проблема установки состоит в наличии одного или нескольких заблокированных портов. Соединители, используемые для создания подключений между средой ISE и целевой системой, также могут иметь собственные требования к портам. Например, если для связи с системой FTP вы используете соединитель FTP, убедитесь в доступности порта, который нужен для этой системы FTP (обычно это порт 21).

Если вы создали виртуальную сеть и подсети без каких-либо ограничений, вам не нужно настраивать [группы безопасности сети (группы безопасности сети)](../virtual-network/security-overview.md) в виртуальной сети, чтобы вы могли управлять трафиком между подсетями. Для существующей виртуальной сети при *необходимости* можно настроить группы безопасности сети, [отфильтруя сетевой трафик между подсетями](../virtual-network/tutorial-filter-network-traffic.md). При выборе этого маршрута убедитесь, что интегрированная среда сценариев открывает определенные порты, как описано в следующей таблице в виртуальной сети с группы безопасности сети. Таким образом, для существующих группы безопасности сети или брандмауэров в виртуальной сети убедитесь, что они открывают эти порты. Таким образом, интегрированная среда сценариев остается доступной и может работать правильно, чтобы не потерять доступ к интегрированной среде сценариев. В противном случае, если все необходимые порты недоступны, интегрированная среда сценариев перестанет работать.
...
В этой таблице описаны порты в виртуальной сети, которые использует интегрированная среда сценариев и где они используются. [Теги службы Диспетчер ресурсов](../virtual-network/security-overview.md#service-tags) представляют собой группу ПРЕФИКСОВ IP-адресов, которые позволяют сократить сложность при создании правил безопасности.

> [!NOTE]
> Исходные порты являются временными, поэтому задайте для них `*` значение для всех правил.
