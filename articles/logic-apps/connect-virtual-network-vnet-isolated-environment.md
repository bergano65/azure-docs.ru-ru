...
| Цель | Direction | Конечные порты | Тег службы источника | Тег службы назначения | Примечания |
|---------|-----------|-------------------|--------------------|-------------------------|-------|
| Поток данных из Azure Logic Apps | Исходящие | 80, 443 | VirtualNetwork | Интернет | Порт зависит от внешней службы, с которой взаимодействует служба Logic Apps. |
| Azure Active Directory | Исходящие | 80, 443 | VirtualNetwork | AzureActiveDirectory | |
| Зависимость от службы хранилища Azure | Исходящие | 80, 443 | VirtualNetwork | Служба хранилища | |
| Взаимодействие между подсетями | Входящий и исходящий | 80, 443 | VirtualNetwork | VirtualNetwork | Для связи между подсетями |
| Поток данных в Azure Logic Apps | Входящие | 443 | Конечные точки внутреннего доступа: <br>VirtualNetwork <p><p>Конечные точки внешнего доступа: <br>Интернет <p><p>**Примечание**. Эти конечные точки относятся к параметру конечной точки, [выбранному во время создания интегрированной среды сценариев](#create-environment). Дополнительные сведения см. в разделе [доступ к конечной точке](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md#endpoint-access). | VirtualNetwork | IP-адрес компьютера или службы, который вызывает любой триггер запроса или веб-перехватчик, существующий в приложении логики. Закрытие или блокировка этого порта предотвращает вызовы HTTP в Logic Apps с триггерами запросов. |
| Журнал выполнения приложений логики | Входящие | 443 | Конечные точки внутреннего доступа: <br>VirtualNetwork <p><p>Конечные точки внешнего доступа: <br>Интернет <p><p>**Примечание**. Эти конечные точки относятся к параметру конечной точки, [выбранному во время создания интегрированной среды сценариев](#create-environment). Дополнительные сведения см. в разделе [доступ к конечной точке](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md#endpoint-access). | VirtualNetwork | IP-адрес компьютера, с которого вы просматриваете журнал выполнения приложения логики. Хотя закрытие или блокирование этого порта не мешает просмотру журнала выполнения, вы не сможете просматривать входные и выходные данные для каждого шага в этом журнале выполнения. |
...
| Обмен данными с диспетчером трафика Azure | Входящие | 443 | AzureTrafficManager | VirtualNetwork | |
| Конструктор Logic Apps — динамические свойства | Входящие | 454 | Интернет | VirtualNetwork | Запросы поступают из Logic Apps [доступа к входящим IP-адресам конечной точки в этом регионе](../logic-apps/logic-apps-limits-and-config.md#inbound). |
| Зависимость от управления службой приложений | Входящие | 454, 455 | AppServiceManagement | VirtualNetwork | |
| Развертывание соединителя | Входящие | 454 | азуреконнекторс | VirtualNetwork | Требуется для развертывания и обновления соединителей. Закрытие или блокировка этого порта приводит к сбою развертываний интегрированной среды сценариев и предотвращает обновления или исправления соединителя. |
| Развертывание политики соединителей | Входящие | 3443 | Интернет | VirtualNetwork | Требуется для развертывания и обновления соединителей. Закрытие или блокировка этого порта приводит к сбою развертываний интегрированной среды сценариев и предотвращает обновления или исправления соединителя. |
...
| Управление API — конечная точка управления | Входящие | 3443 | APIManagement | VirtualNetwork | |
| Зависимость от политики передачи журнала в концентратор событий и от агента мониторинга | Исходящие | 5672 | VirtualNetwork | EventHub | |
| Доступ к экземплярам кэша Azure для Redis между экземплярами ролей | Входящие <br>Исходящие | 6379-6383 | VirtualNetwork | VirtualNetwork | Кроме того, чтобы интегрированная среда сценариев работала с кэшем Azure для Redis, необходимо открыть эти [порты для исходящего и входящего трафика, описанные в статье вопросы и ответы по кэшу Azure для Redis](../azure-cache-for-redis/cache-how-to-premium-vnet.md#outbound-port-requirements). |
| Балансировщик нагрузки Azure | Входящие | * | AzureLoadBalancer | VirtualNetwork | |
||||||
...
1. Укажите следующие сведения для своей среды, а затем выберите **Просмотр и создание**, как показано здесь:
   ...
   | **Дополнительная емкость** | Premium: <br>Да <p><p>Developer: <br>Не применимо | Premium: <br>от 0 до 10 <p><p>Developer: <br>Не применимо | Число дополнительных единиц обработки, используемых для этого ресурса ISE. Сведения о добавлении емкости после создания см. в разделе [Добавление емкости интегрированной среды сценариев](#add-capacity). |
   ...
   **Создание подсети**
   ...
   > [!IMPORTANT]
   > 
   > Имена `<`подсетей должны начинаться с буквы или знака подчеркивания (без чисел) и не используют следующие символы:, `>`, `%`, `&` `\\` `?`,,, `/`.
   ...
   * Использует по крайней `/27` мере в адресном пространстве, так как в каждой подсети *должно быть как* *минимум 32 адресов* . Пример:
