...
* [Виртуальная сеть Azure](../virtual-network/virtual-networks-overview.md). Ваша виртуальная сеть должна иметь четыре *пустые* подсети, которые не делегируются ни одной службе для создания и развертывания ресурсов в интегрированной среде сценариев. Каждая подсеть поддерживает отдельный компонент Logic Apps, используемый в среде службы интеграции. Вы можете заранее создать подсети или подождать, пока вы создадите интегрированную среду сценариев, где вы сможете создавать подсети одновременно. Дополнительные сведения о [требованиях к подсети](#create-subnet).
  ...
  > * 169.254.169.254/32
  > 
  > Имена подсетей должны начинаться с буквы или знака подчеркивания, и в них не могут использоваться следующие символы: `<`, `>`, `%`, `&`, `\\`, `?`, `/`. Чтобы развернуть интегрированную среду сценариев с помощью шаблона Azure Resource Manager, сначала необходимо делегировать одну пустую подсеть в `Microsoft.Logic/integrationServiceEnvironment` . Это делегирование не нужно делать при развертывании с помощью портала Azure.
