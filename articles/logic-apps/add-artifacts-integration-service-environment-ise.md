---
title: Добавление артефактов в среды службы интеграции (Исес) в Azure Logic Apps
description: Добавление приложений логики, пользовательских соединителей и учетных записей интеграции в среду службы Integration Services (ISE) для доступа к виртуальным сетям Azure (виртуальных сетей), в то время как они остаются частными и изолированными от общедоступных или глобальных Azure
services: logic-apps
...
После создания [среды службы интеграции (ISE)](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md)добавьте артефакты, такие как приложения логики, учетные записи интеграции и пользовательские соединители, чтобы они могли получать доступ к ресурсам в виртуальной сети Azure.
...
## <a name="create-logic-apps-in-an-ise"></a>Создание приложений логики в интегрированной среде сценариев
...
## <a name="create-integration-accounts-in-an-ise"></a>Создание учетных записей интеграции в интегрированной среде сценариев

В зависимости от [номера SKU](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md#ise-level) , выбранного при создании, интегрированная среда разработки включает в себя конкретные сведения об использовании учетной записи интеграции без дополнительной оплаты. Приложения логики, которые существуют в среде служб Integration Services (ISE), могут ссылаться только на учетные записи интеграции, которые существуют в одной ISE. Таким образом, чтобы учетная запись интеграции работала с приложениями логики в интегрированной среде сценариев, как учетная запись интеграции, так и приложения логики должны использовать ту *же среду* , что и их расположение. Дополнительные сведения об учетных записях интеграции и исес [см. в статье](connect-virtual-network-vnet-isolated-environment-overview.md#create-integration-account-environment
)интеграция учетных записей с интегрированной средой сценариев.
...
1. Найдите и откройте интегрированную среду сценариев, если она еще не открыта. В меню интегрированной среды сценариев в разделе **Параметры**выберите **учетные записи** > интеграции**Добавить**.
   ...
   В главном меню Azure выберите создать**учетную запись интеграции** **интеграции** >  **ресурсов** > .
...
## <a name="create-custom-connectors-in-an-ise"></a>Создание настраиваемых соединителей в интегрированной среде сценариев
