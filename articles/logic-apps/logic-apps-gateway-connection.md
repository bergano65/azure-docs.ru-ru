...
* Установка шлюза еще не зарегистрирована и не заявлена другим существующим ресурсом шлюза Azure в портал Azure.

  При создании ресурса шлюза в портал Azure выберите установку шлюза, которая связывается с ресурсом шлюза и только с этим ресурсом шлюза. Каждый ресурс шлюза может ссылаться только на одну установку шлюза, которая может ссылаться только на одну учетную запись Azure. В Azure Logic Apps локальные триггеры и действия используют ресурс шлюза при подключении к локальным источникам данных. Если у вас есть доступ к подписке, вы можете выбрать один из разных подписок Azure, каждый из которых связан с другим ресурсом шлюза. Вашему приложению логики и ресурсу шлюза не обязательно использовать одну и ту же подписку Azure.
...
1. В разделе **шлюзы**в списке **подписки** выберите подписку Azure с нужным ресурсом шлюза.
   ...
   Если у вас есть доступ к подписке, вы можете выбрать один из разных подписок Azure, каждый из которых связан с другим ресурсом шлюза. Вашему приложению логики и ресурсу шлюза не обязательно использовать одну и ту же подписку Azure.
