---
title: Добавление циклов для повторения действий
...
Чтобы повторять действия, пока не будет выполнено условие или состояние не изменится, можно создать [цикл until](#until-loop). Приложение логики сначала запускает все действия внутри цикла, а затем проверяет состояние или состояние. Если условие выполнено, цикл прекращается. В противном случае он повторяется. Ограничения количества циклов until в одном выполнении приложения логики см. в статье [Ограничения и сведения о конфигурации для Azure Logic Apps](../logic-apps/logic-apps-limits-and-config.md). 
...
* Действия в цикле "Форах" используют[`@item()`](../logic-apps/workflow-definition-language-functions-reference.md#item) 
...
2. Добавьте цикл for each между триггером RSS и действием отправки сообщения. 
   ...
   1. Под полем поиска выберите **Все**. В поле поиска введите for each в качестве условия фильтрации. Из списка действий выберите это действие: **Для каждого - Контроль**
...
Чтобы запустить и повторить действия до тех пор, пока условие не будет выполнено или состояние не изменится, поместите эти действия в цикл "До" Приложение логики сначала запускает все действия внутри цикла, а затем проверяет состояние или состояние. Если условие выполнено, цикл прекращается. В противном случае он повторяется.
...
1. Создание пустого приложения логики. В конструкторе приложений логики под полем поиска выберите **Все**. В поле поиска введите recurrence. 
   Из списка триггеров выберите этот триггер: **Повторяю - Расписание**
...
1. В разделе триггера выберите **Добавить шаг**. 
   Поиск "переменных" и выберите это действие: **Инициализация переменной - Переменные**
...
1. Задайте для переменной следующие значения:
   ...
   | -------- | ----- | ----------- |
   | **Название** | Ограничение | Имя переменной | 
...
1. Под полем поиска выберите **Все**. Поиск "до", и выберите это действие: **До - Контроль**
...
1. Под полем поиска выберите **Все**. Поиск "переменных" и выберите это действие: **переменная приращения - Переменные**
...
1. Задайте свойства для действия отправки сообщения электронной почты. Добавьте переменную **Limit** в поле темы. Таким образом, вы сможете убедиться в том, что текущее значение переменной соответствует заданному условию, как в этом примере:
      ...
      | Свойство | Значение | Описание |
      | -------- | ----- | ----------- | 
      | **Кому** | *\<\@домен электронной почты>* | Адрес электронной почты получателя. Для тестировании используйте свой адрес электронной почты. | 
...
| -------- | ------------- | ----------- | 
| **Рассчитывать** | 60 | Максимальное количество итераций, выполняемых до выхода из цикла. Значение по умолчанию — 60 повторяемых операций. | 
| **Времени ожидания** | PT1H | Максимальное время выполнения цикла до выхода из него. Значение по умолчанию составляет один час и указывается в формате ISO 8601. <p>Значение времени ожидания вычисляется для каждой повторяющейся операции цикла. Если любое действие в цикле выполняется дольше предела времени ожидания, текущий цикл не останавливается. Тем не менее следующий цикл не начнется, потому что предельное условие не выполнено. | 
|||| 
