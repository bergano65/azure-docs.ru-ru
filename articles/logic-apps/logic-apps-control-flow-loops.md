...
1. В разделе триггера выберите **Добавить шаг**. Выполните поиск по слову variables и выберите действие **Initialize variable — Variables** (Инициализация переменной — переменные)
...
1. Создайте условие выхода из цикла. Для этого выберите переменную **Limit** и оператор **is equal**. Введите **10** в качестве значения для сравнения.
...
1. Под полем поиска выберите **Все**. Найдите и добавьте действие, которое отправляет электронные уведомления, например: 
