...
| **Timeout** | PT1H | Максимальное время выполнения цикла до выхода из него. Значение по умолчанию составляет один час и указывается в формате ISO 8601. <p>Значение времени ожидания вычисляется для каждой повторяющейся операции цикла. Если любое действие в цикле выполняется дольше предела времени ожидания, текущий цикл не останавливается. Тем не менее следующий цикл не начнется, потому что предельное условие не выполнено. | 
|||| 
