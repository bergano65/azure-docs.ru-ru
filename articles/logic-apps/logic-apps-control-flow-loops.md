...
* Действия в цикле foreach используют[`@item()`](../logic-apps/workflow-definition-language-functions-reference.md#item) 
...
2. Добавьте цикл for each между триггером RSS и действием отправки сообщения. 
   ...
   1. Под полем поиска выберите **Все**. В поле поиска введите for each в качестве условия фильтрации. В списке действия выберите это действие: **для каждого элемента управления** .
...
Чтобы выполнить и повторить действия до тех пор, пока условие не будет удовлетворено или изменится состояние, поставьте эти действия в цикл "Until". Приложение логики сначала выполняет все действия в цикле, а затем проверяет условие или состояние. Если условие выполнено, цикл прекращается. В противном случае он повторяется.
...
1. Создание пустого приложения логики. В конструкторе приложений логики под полем поиска выберите **Все**. В поле поиска введите recurrence. 
   В списке триггеры выберите этот триггер: **повторение — расписание** .
...
1. В разделе триггера выберите **Добавить шаг**. 
   Выполните поиск по фразе "переменные" и выберите это действие: **Инициализация** переменных переменных
...
1. Задайте для переменной следующие значения:
   ...
   | **Type** | Целое число | Тип данных переменной | 
   | **Значение** | 0 | Начальное значение переменной | 
   |||| 

1. В разделе действия **Инициализировать переменную** выберите **Новый шаг**. 

1. Под полем поиска выберите **Все**. Выполните поиск по слову "Until" и выберите это действие: **until-Control**
...
1. Под полем поиска выберите **Все**. Выполните поиск по фразе "переменные" и выберите это действие: **увеличить переменную-переменные**
...
1. В поле **Имя** выберите переменную **Limit**. В качестве **значения**введите "1". 
...
1. Задайте свойства для действия отправки сообщения электронной почты. Добавьте переменную **Limit** в поле темы. Таким образом, вы сможете убедиться в том, что текущее значение переменной соответствует заданному условию, как в этом примере:
      ...
      | Свойство | Значение | Описание |
      | -------- | ----- | ----------- | 
      | **Кому** | *\<адрес\@электронной почты —>домена* | Адрес электронной почты получателя. Для тестировании используйте свой адрес электронной почты. | 
      ...
      | **Организм** | <*Электронная почта — содержимое*> | Укажите содержимое сообщения, которое требуется отправить. Для этого примера введите любой текст. | 
      |||| 
