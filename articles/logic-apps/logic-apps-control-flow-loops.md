...
Чтобы обработать массив в приложении логики, можно создать [цикл for each](#foreach-loop). Этот цикл повторяет одно или несколько действий для каждого элемента в массиве. Ограничения на количество элементов массива, которые может обработать цикл foreach, см. в разделе [ограничения параллелизма, циклов и депакетирования](../logic-apps/logic-apps-limits-and-config.md#looping-debatching-limits).

Чтобы повторять действия, пока не будет выполнено условие или состояние не изменится, можно создать [цикл until](#until-loop). Приложение логики сначала запускает все действия в цикле, а затем проверяет условие или состояние. Если условие выполнено, цикл прекращается. В противном случае он повторяется. Дополнительные ограничения по умолчанию и максимальное количество циклов Until, которые может иметь запуск приложения логики, см. в разделе [ограничения параллелизма, циклов и депакетирования](../logic-apps/logic-apps-limits-and-config.md#looping-debatching-limits).
...
Цикл foreach повторяет одно или несколько действий с каждым элементом массива и работает только с массивами. Вот некоторые рекомендации при использовании цикла for each.

* Цикл foreach может обрабатывать ограниченное количество элементов массива. Для этого ограничения см. раздел [ограничения параллелизма, циклов и депакетирования](../logic-apps/logic-apps-limits-and-config.md#looping-debatching-limits).

* По умолчанию итерации в цикле foreach выполняются одновременно или параллельно. Это поведение отличается от [автоматического **применения к каждому** циклу](/power-automate/apply-to-each) , где итерации выполняются по одному или последовательно. Однако можно [настроить последовательные итерации цикла foreach](#sequential-foreach-loop). Например, если вы хотите приостановить следующую итерацию в цикле foreach с помощью [действия Delay](../connectors/connectors-native-delay.md), необходимо задать выполнение цикла последовательно.

  Исключением для поведения по умолчанию являются вложенные циклы, где итерации всегда выполняются последовательно, а не параллельно. Чтобы запустить выполнение операций в параллельном режиме для элементов вложенного цикла, создайте и [вызовите дочернее приложение логики](../logic-apps/logic-apps-http-endpoint.md).
...
> [Список соединителей можно просмотреть здесь](/connectors/). Если используется другая учетная запись электронной почты, общие шаги остаются неизменными, однако интерфейс может выглядеть несколько иначе. 
...
* **Count** : это наибольшее число циклов, выполняемых перед выходом из цикла. Дополнительные ограничения по умолчанию и максимальное количество циклов Until, которые может иметь запуск приложения логики, см. в разделе [ограничения параллелизма, циклов и депакетирования](../logic-apps/logic-apps-limits-and-config.md#looping-debatching-limits).

* **Время ожидания**. это значение является наибольшим временем выполнения цикла до выхода и указывается в [формате ISO 8601](https://en.wikipedia.org/wiki/ISO_8601). Значения по умолчанию и максимальные ограничения на значение **времени ожидания** см. в разделе [ограничения параллелизма, циклов и депакетирования](../logic-apps/logic-apps-limits-and-config.md#looping-debatching-limits).
...
Чтобы изменить эти ограничения, в действии цикл выберите **изменить ограничения**.
