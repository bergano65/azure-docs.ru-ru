---
title: Сообщения X12 для интеграции B2B
...
2. В главном меню Azure выберите **Все службы**. 
   Введите в поле поиска "интеграция" и выберите в результатах поиска **Учетные записи интеграции**.  
...
6. В колонке **Добавить** введите **имя** соглашения. 
   ...
   Заполните поля **Главный партнер**, **Главный идентификатор**, **Гостевой партнер** и **Гостевой идентификатор**. 
   Дополнительные сведения о свойствах см. в таблице ниже.
...
![Свойства идентификатора набора](./media/logic-apps-enterprise-integration-x12/x12-2.png)  
...
![Установка свойств подтверждения](./media/logic-apps-enterprise-integration-x12/x12-3.png) 
...
| Свойство | Описание |
| --- | --- |
| ISA11 Usage (Использование ISA11) |Указывает разделитель для использования в наборе транзакций. <p>Выберите **стандартный идентификатор**, чтобы использовать точку (.) для десятичного представления вместо десятичного представления входящего документа в конвейере получения EDI. <p>Выберите **Сепаратор повторения,** чтобы указать сепаратор для повторных возникновений простого элемента данных или повторяющейся структуры данных. Например, в качестве разделителя повторений обычно используется карет (^). Для схем HIPPA можно использовать только карет. |

### <a name="control-numbers"></a>Номера управления
...
![Установка свойств проверки для полученных сообщений](./media/logic-apps-enterprise-integration-x12/x12-36.png) 
...
### <a name="internal-settings"></a>Внутренние настройки
...
![Свойства идентификатора набора](./media/logic-apps-enterprise-integration-x12/x12-4.png)  
...
![Установка свойств подтверждения](./media/logic-apps-enterprise-integration-x12/x12-5.png)  
...
| Версия |Выберите версию X12. |
| Transaction Type (ST01) (Тип транзакции (ST01)) |Выберите тип транзакции. |
| SCHEMA |Выберите схему для использования. Схемы находятся в вашей учетной записи интеграции. Если вы сначала выберете схему, будут автоматически настроены версия и тип транзакции.  |
...
![Указать свойства контрольного номера](./media/logic-apps-enterprise-integration-x12/x12-8.png) 
...
| Prefix |Необязательно. Применяется к диапазону контрольных номеров наборов транзакций, используемых в подтверждении. Введите числовое значение в два поля посередине и буквы или цифры (при необходимости) в поля префикса и суффикса. Поля посередине являются обязательными и содержат минимальное и максимальное значения контрольного номера. |
...
### <a name="character-sets-and-separators"></a>Наборы персонажей и сепараторы
...
![Установка свойств проверки для отправки сообщений](./media/logic-apps-enterprise-integration-x12/x12-10.png) 
...
Для получения более подробной информации об этом разъеме, таких как действия и ограничения, описанные в файле Swagger разъема, смотрите [страницу ссылки разъема.](https://docs.microsoft.com/connectors/x12/) 
