...
Каждый URL-адрес содержит параметр запроса `sp`, `sv` и `sig`, как описано в следующей таблице.
...
В тексте включите `NotAfter`property, используя строку даты JSON. Это свойство возвращает URL-адрес обратного вызова, действительный только до даты и времени, заданной для параметра `NotAfter`.
...
При автоматизации развертывания приложений логики с помощью [шаблона Azure Resource Manager](../logic-apps/logic-apps-create-deploy-template.md)можно указать диапазоны IP-адресов с помощью раздела `accessControl` с разделом `triggers` в определении ресурса приложения логики, например:
...
При автоматизации развертывания приложений логики с помощью [шаблона Azure Resource Manager](../logic-apps/logic-apps-create-deploy-template.md)можно указать диапазоны IP-адресов с помощью раздела `accessControl` с разделом `contents` в определении ресурса приложения логики, например:
...
В базовом триггере или определении действия добавьте или обновите массив `runtimeConfiguration.secureData.properties` одним или обоими значениями:
...
При автоматизации развертываний с помощью [шаблонов Azure Resource Manager](../azure-resource-manager/resource-group-authoring-templates.md#parameters)можно определить защищенные параметры шаблона, которые оцениваются при развертывании с помощью типов `securestring` и `secureobject`. Чтобы определить параметры шаблона, используйте раздел верхнего уровня `parameters`, который является отдельным и отличным от раздела `parameters` определения рабочего процесса. Чтобы указать значения для параметров шаблона, используйте отдельный [файл параметров](../azure-resource-manager/resource-group-template-deploy.md#pass-parameter-values).
...
Шаблон диспетчер ресурсов для приложения логики содержит несколько разделов @no__t 0. Для защиты паролей, ключей, секретов и других конфиденциальных данных определите защищенные параметры на уровне шаблона и определения рабочего процесса, используя тип `securestring` или `secureobject`. Затем эти значения можно сохранить в [Azure Key Vault](../key-vault/key-vault-overview.md) и использовать [файл параметров](../azure-resource-manager/resource-group-template-deploy.md#pass-parameter-values) для ссылки на хранилище ключей и секрет. Затем шаблон извлекает эти сведения при развертывании. Дополнительные сведения см. в статье [использование Azure Key Vault для передачи безопасных значений параметров при развертывании](../azure-resource-manager/resource-manager-keyvault-parameter.md).

Ниже приведены дополнительные сведения об этих разделах `parameters`.
...
В этом примере шаблона есть несколько определений защищенных параметров, использующих тип `securestring`:
