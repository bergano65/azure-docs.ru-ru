...
#### <a name="hide-inputs-and-outputs-in-the-designer"></a>Скрытие входных и выходных данных в конструкторе
...
1. В триггере или действии, для которого необходимо скрыть конфиденциальные данные, нажмите кнопку с многоточием (**...**), а затем выберите **Параметры**.
...
#### <a name="hide-inputs-and-outputs-in-code-view"></a>Скрытие входных и выходных данных в представлении кода
...
* Чтобы скрыть выходные данные действия, которое скрывает входные данные или явно закрывает выходные данные, включите в это действие **защищенные выходные данные** вручную.
...
|---------------------|-----------------|----------|-------|-------------|
| **Аутентификация** | `type` | Да | Basic | Используемый тип проверки подлинности |
...
| Свойство (конструктор) | Property (JSON) | Обязательный | Значение | Описание |
|---------------------|-----------------|----------|-------|-------------|
| **Аутентификация** | `type` | Да | **Active Directory OAuth** <br>или диспетчер конфигурации служб <br>`ActiveDirectoryOAuth` | Тип проверки подлинности. Logic Apps в настоящее время соответствует [протоколу OAuth 2,0](../active-directory/develop/v2-overview.md). |
...
| **Владел** | `secret` | Да, но только для типа учетных данных "секрет" | <*Секрет клиента*> | Секрет клиента для запроса на авторизацию |
...
1. В триггере или действии, где вы хотите использовать управляемое удостоверение, укажите следующие значения свойств.
   ...
   | Свойство (конструктор) | Property (JSON) | Обязательный | Значение | Описание |
   |---------------------|-----------------|----------|-------|-------------|
   | **Аутентификация** | `type` | Да | **Управляемое удостоверение** <br>или диспетчер конфигурации служб <br>`ManagedServiceIdentity` | Используемый тип проверки подлинности |
