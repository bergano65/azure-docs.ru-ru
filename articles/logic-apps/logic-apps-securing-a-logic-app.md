...
description: Защита входных параметров, триггеров HTTP-запросов, журнала выполнения, операций приложения логики и подключений к другим службам в Azure Logic Apps
services: logic-apps
...
* [Доступ к триггерам HTTP-запросов](#secure-triggers)
...
## <a name="access-to-http-request-triggers"></a>Доступ к триггерам HTTP-запросов

Если приложение логики использует триггер на основе HTTP-запросов, например [запрос](../connectors/connectors-native-reqres.md) или триггер [перехватчика](../connectors/connectors-native-webhook.md) , можно ограничить доступ, чтобы только разрешенные клиенты могли запускать приложение логики. Все запросы, полученные приложением логики, шифруются и защищаются с помощью протокола SSL. 
...
Дополнительные сведения о защите доступа с помощью подписанного URL см. в следующих разделах этой статьи:
...
При создании или перечислении URL-адресов обратного вызова для триггеров, основанных на HTTP-запросах, можно указать ключ, используемый для подписи URL-адреса. Чтобы создать URL-адрес, подписанный указанным ключом, используйте [REST API Logic Apps](https://docs.microsoft.com/rest/api/logic/workflowtriggers), например:
...
> [!NOTE]
> Независимо от IP-адреса вы по-прежнему можете запускать приложение логики с триггером на основе HTTP-запросов, `/triggers/<trigger-name>/run` используя Azure REST API или через службу управления API. Однако этот сценарий по-прежнему требует проверки подлинности в REST API Azure. Все события отображаются в журнале аудита Azure. Убедитесь, что политики управления доступом настроены соответствующим образом.

#### <a name="restrict-incoming-ip-ranges-in-azure-resource-manager-template"></a>Ограничить диапазоны входящих IP-адресов в шаблоне Azure Resource Manager

При автоматизации развертывания приложений логики с помощью [шаблона Azure Resource Manager](../logic-apps/logic-apps-create-deploy-template.md)можно указать диапазоны IP-адресов, используя `accessControl` раздел с `triggers` разделом в определении ресурса приложения логики, например:
...
Чтобы добавить дополнительные протоколы авторизации в приложение логики, используйте [службу "Управление API Azure"](../api-management/api-management-key-concepts.md). Эта служба предоставляет возможность предоставить приложение логики в качестве API и предоставляет широкие возможности мониторинга, безопасности, политики и документации для любой конечной точки. Служба "Управление API" может предоставлять общедоступную или частную конечную точку для приложения логики, которое затем может использовать Azure Active Directory, сертификат, OAuth или другие методы безопасности. Когда служба "Управление API" получает запрос, она отправляет его в приложение логики, выполняя при этом необходимые преобразования или устанавливая ограничения. Чтобы разрешить приложению логики запускаться только с помощью службы "Управление API", можно использовать параметры диапазона входящих IP-адресов для приложения логики.

<a name="secure-operations"></a>

## <a name="access-to-logic-app-operations"></a>Доступ к операциям приложения логики

Вы можете разрешите отдельным пользователям или группам выполнять определенные операции, такие как управление, редактирование и просмотр приложений логики. Чтобы управлять своими разрешениями, используйте [Управление доступом на основе ролей Azure (RBAC)](../role-based-access-control/role-assignments-portal.md) для назначения настраиваемых или встроенных ролей участникам в вашей подписке Azure.

* [Создатель приложений логики](../role-based-access-control/built-in-roles.md#logic-app-contributor). Позволяет управлять приложениями логики, но не может изменить доступ к ним.

* [Оператор приложений логики](../role-based-access-control/built-in-roles.md#logic-app-operator). Позволяет читать, включать и отключать приложения логики, но не изменять и не обновлять их.

Чтобы предотвратить изменение или удаление приложения логики другими пользователями, можно использовать [блокировку ресурсов Azure](../azure-resource-manager/resource-group-lock-resources.md), которая запрещает другим пользователям изменять или удалять рабочие ресурсы.

<a name="secure-run-history"></a>

## <a name="access-to-run-history-data"></a>Доступ к данным журнала выполнения

Во время выполнения приложения логики все данные шифруются во время передачи с использованием [протокола TLS](https://azure.microsoft.com/updates/app-service-and-functions-hosted-apps-can-now-update-tls-versions/) и [неактивных](../security/fundamentals/encryption-atrest.md)данных. После завершения работы приложения логики можно просмотреть журнал этого запуска, включая шаги, которые были выполнены вместе с состоянием, длительностью, входом и выходом для каждого действия. Эти подробные сведения позволяют понять, как запущено приложение логики и где можно начать устранение любых возникающих проблем.

При доступе к журналу выполнения приложения логики Logic Apps выполняет проверку подлинности доступа и предоставляет ссылки на входные и выходные данные из запросов и ответов в выполнении приложения логики. Однако для действий, которые обрабатывали пароли, секреты, ключи или другие конфиденциальные сведения, необходимо запретить другим пользователям просматривать эти данные и получать к ним доступ. Например, если приложение логики получает секрет от [Azure Key Vault](../key-vault/key-vault-whatis.md) для использования при проверке подлинности действия HTTP, необходимо скрыть этот секрет из представления.
...
При автоматизации развертывания приложений логики с помощью [шаблона Azure Resource Manager](../logic-apps/logic-apps-create-deploy-template.md)можно указать диапазоны IP-адресов, используя `accessControl` раздел с `contents` разделом в определении ресурса приложения логики, например:
...
* В действии, когда вы защищаете входные данные или явно использует защищенные выходные данные, и это действие возвращает ответ с выходами, которые включают эти защищенные данные, необходимо вручную включить **защищенные выходные** данные в этом действии, чтобы защитить эти выходные данные.
...
При развертывании в разных средах рассмотрите возможность параметризации значений в определении рабочего процесса в зависимости от этих сред. Таким образом, можно использовать [шаблон Azure Resource Manager](../azure-resource-manager/resource-group-authoring-templates.md#parameters) для развертывания приложения логики, защиты конфиденциальной информации путем определения защищенных параметров и предоставления этих параметров отдельно через параметры шаблона с помощью [параметра. файл](../azure-resource-manager/resource-group-template-deploy.md#pass-parameter-values).

Например, при проверке подлинности HTTP-действий с помощью [Azure Active Directory](../logic-apps/logic-apps-workflow-actions-triggers.md#connector-authentication)можно определить и защитить параметры, которые принимают идентификатор клиента и секрет клиента, используемые для проверки подлинности. Чтобы определить эти параметры для приложения логики, используйте `parameters` раздел внутри определения рабочего процесса приложения логики. Чтобы защитить значения параметров, которые не должны отображаться при изменении приложения логики или просмотре журнала выполнения, определите параметры с помощью `securestring` типа или `secureobject` и при необходимости используйте кодировку. Параметры, имеющие этот тип, не возвращаются вместе с определением ресурса и недоступны при просмотре ресурса после развертывания. Чтобы получить доступ к этим значениям параметров во время `@parameters('<parameter-name>')` выполнения, используйте выражение внутри определения рабочего процесса. Это выражение вычисляется только во время выполнения и описывается [языком определения рабочего процесса](../logic-apps/logic-apps-workflow-definition-language.md).

> [!NOTE]
> Если вы используете параметр в заголовках или тексте HTTP-запроса, этот параметр может быть видимым при просмотре журнала выполнения приложения логики и исходящего HTTP-запроса. Кроме того, убедитесь, что политики доступа к содержимому заданы соответствующим образом. Заголовки авторизации никогда не отображаются во входных и выходных данных. Таким образом, если используется секрет, то он не извлекается.

Дополнительные сведения см. в подразделе [Защита параметров в определениях рабочих процессов](#secure-parameters-workflow) далее в этой статье.

При автоматизации развертываний с помощью [шаблонов Azure Resource Manager](../azure-resource-manager/resource-group-authoring-templates.md#parameters)можно определить защищенные параметры шаблона, которые оцениваются при развертывании с помощью `securestring` типов и. `secureobject` Чтобы определить параметры шаблона, используйте раздел верхнего уровня `parameters` шаблона, который является отдельным и отличным от `parameters` раздела определения рабочего процесса. Чтобы указать значения для параметров шаблона, используйте отдельный [файл параметров](../azure-resource-manager/resource-group-template-deploy.md#pass-parameter-values).

Например, при использовании секретов можно определить и использовать защищенные параметры шаблона, которые извлекают эти секреты из [Azure Key Vault](../key-vault/key-vault-whatis.md) при развертывании. Затем можно сослаться на хранилище ключей и секрет в файле параметров. Дополнительные сведения см. в следующих статьях:

* [Использование Azure Key Vault для передачи значений безопасных параметров при развертывании](../azure-resource-manager/resource-manager-keyvault-parameter.md)
...
Шаблон диспетчер ресурсов для приложения логики имеет несколько `parameters` разделов. Для защиты паролей, ключей, секретов и других конфиденциальных данных определите защищенные параметры на уровне шаблона и определения рабочего процесса с помощью `securestring` типа или. `secureobject` Затем эти значения можно сохранить в [Azure Key Vault](../key-vault/key-vault-whatis.md) и использовать [файл параметров](../azure-resource-manager/resource-group-template-deploy.md#pass-parameter-values) для ссылки на хранилище ключей и секрет. Затем шаблон извлекает эти сведения при развертывании. Дополнительные сведения см. в статье [использование Azure Key Vault для передачи безопасных значений параметров при развертывании](../azure-resource-manager/resource-manager-keyvault-parameter.md).

Ниже приведены дополнительные сведения об этих `parameters` разделах.

* На верхнем уровне `parameters` шаблона раздел определяет параметры для значений, используемых шаблоном при *развертывании*. Например, эти значения могут включать строки подключения для определенной среды развертывания. Затем эти значения можно хранить в отдельном [файле параметров](../azure-resource-manager/resource-group-template-deploy.md#pass-parameter-values), что упрощает изменение этих значений.
...
Ниже приведены некоторые способы защиты конечных точек, в которых приложению логики требуется доступ для отправки запросов.

* Добавление проверки подлинности для исходящих запросов.

  При работе с HTTP, HTTP + Swagger (Open API) или действием веб-перехватчика можно добавить проверку подлинности в запрос, отправленный приложением логики. Например, это может быть обычная аутентификация, аутентификация на основе сертификата или аутентификация Azure Active Directory. Дополнительные сведения см. в разделе [Проверка подлинности триггеров или действий](../logic-apps/logic-apps-workflow-actions-triggers.md#connector-authentication).
...
* Проверка подлинности и доступ к ресурсам в качестве управляемых удостоверений.

  Для доступа к ресурсам в других клиентах Azure Active Directory (Azure AD) приложение логики может проверить подлинность Вашего удостоверения без входа с помощью управляемого удостоверения (прежнее название — Управляемое удостоверение службы или MSI), а не учетных данных или секретов. Azure управляет этим удостоверением за вас и помогает защитить учетные данные, потому что вам не нужно предоставлять или сменять секреты. Дополнительные сведения о настройке и использовании назначенного системой управляемого удостоверения для приложения логики см. [в разделе Проверка подлинности и доступ к ресурсам с помощью управляемых удостоверений в Azure Logic Apps](../logic-apps/create-managed-service-identity.md).
