---
title: Добавление и запуск кода в Azure Logic Apps с помощью функций Azure
description: Добавление и запуск кода в Azure Logic Apps с помощью функций Azure
services: logic-apps
...
Если вы хотите запустить код, который выполняет конкретная задача в приложениях логики, можно создать свои собственные функции с ["функции Azure"](../azure-functions/functions-overview.md). Эта служба поможет вам создать Node.js, C#, и F# кода, поэтому вам не нужно создавать полные приложения или инфраструктуры для выполнения кода. Также можно [вызывать приложения логики из функций](#call-logic-app).
Служба "Функции Azure" обеспечивает бессерверные вычисления в облаке и является полезной при выполнении задач, приведенных ниже.
...
Создайте приложение-функцию Azure на [портале Azure](https://portal.azure.com). Оно должно находиться в той же подписке, что и приложение логики. Затем создайте свою функцию Azure.
