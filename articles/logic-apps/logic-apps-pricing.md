...
* Использование [учетной записи интеграции](../logic-apps/logic-apps-enterprise-integration-create-integration-account.md) без дополнительных затрат на основе [номера SKU интегрированной среды сценариев](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md#ise-level):
  ...
  * **SKU Premium**: Единая учетная запись интеграции [уровня Standard](../logic-apps/logic-apps-limits-and-config.md#artifact-number-limits)
...
* **Триггер опроса**: Этот триггер постоянно проверяет конечную точку на наличие сообщений, удовлетворяющих критериям создания экземпляра приложения логики и запуска рабочего процесса. Даже если не создается экземпляр приложения логики, Logic Apps учитывает каждый опрашивающий запрос как одно выполнение. Интервал опроса для триггера можно указать в конструкторе приложений логики.

  [!INCLUDE [logic-apps-polling-trigger-non-standard-metering](../../includes/logic-apps-polling-trigger-non-standard-metering.md)]

* **Триггер веб**-перехватчика: Этот триггер ожидает отправки клиентом запроса в определенную конечную точку. Каждый запрос, отправленный в конечную точку webhook, учитывается как выполненное действие. К триггерам веб-перехватчика относятся триггеры веб-перехватчика HTTP и триггеры запросов.
...
* **Базовый**. Для тех ситуаций, когда требуется только обработка сообщений или работа в качестве небольшого бизнес-партнера с отношением торгового партнера к более крупной бизнес-сущности.

* **Стандартный**. При наличии более сложных отношений B2B и увеличении числа сущностей, которыми необходимо управлять
