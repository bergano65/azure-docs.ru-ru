---
title: Схема для языка определения рабочего процесса — Azure Logic Apps
description: Справочник по схеме для языка определения рабочего процесса в Azure Logic Apps
services: logic-apps
...
| <*типа параметра* > | ДА | int, float, String, bool, массив, Object, SecureString, secureobject <p><p>**Примечание**. для всех паролей, ключей и секретов используйте типы `securestring` или `secureobject`, так как операция `GET` не возвращает эти типы. Дополнительные сведения о защите параметров см. в разделе [рекомендации по безопасности для действий и входных параметров](../logic-apps/logic-apps-securing-a-logic-app.md#secure-action-parameters). | Тип параметра. |
| <*по умолчанию > параметра-value* | ДА | То же, что `type` | Значение параметра по умолчанию, используемое, если при создании экземпляра рабочего процесса не указано значение. Атрибут `defaultValue` является обязательным, чтобы конструктор приложений логики мог правильно отобразить параметр, но можно указать пустое значение. |
| <*массив-с-допустимыми параметрами-values* > | Нет | Массив, | Массив со значениями, которые может принимать параметр. |
| <*параметр-описание* > | Нет | Объект JSON | Любые другие сведения о параметрах, например описание параметра. |
||||
...
| < >*типа ключа* | ДА | int, float, string, securestring, bool, массив, объект JSON | Тип возвращаемого значения в выходных данных. |
| <*значение ключа*> | ДА | То же, что и <*типа ключа* > | Возвращаемое значение в выходных данных. |
|||||
