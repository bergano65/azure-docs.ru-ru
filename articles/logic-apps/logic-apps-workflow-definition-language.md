...
|-----------|----------|------|-------------|
| <*имя параметра*> | Да | Строка | Имя параметра, который требуется определить |
| <*тип параметра*> | Да | int, float, String, bool, массив, Object, SecureString, secureobject <p><p>**Примечание**. для всех паролей, ключей и секретов используйте `securestring` типы или, `secureobject` так как `GET` операция не возвращает эти типы. Дополнительные сведения о защите параметров см. в разделе [рекомендации по безопасности для действий и входных параметров](../logic-apps/logic-apps-securing-a-logic-app.md#secure-action-parameters). | Тип параметра. |
...
| <*массив-с-допустимыми параметрами-Parameter*> | Нет | Массив | Массив со значениями, которые может принимать параметр. |
...
В `staticResults` атрибуте определите макет действия `outputs` и `status` действие, которое будет возвращено при включении статического параметра результата действия. В определении действия `runtimeConfiguration.staticResult.name` атрибут ссылается на имя статического определения результата внутри `staticResults` . Узнайте [, как тестировать приложения логики с помощью макетов данных, настроив статические результаты](../logic-apps/test-logic-apps-mock-data-static-results.md).
...
| Атрибут | Обязательно | Тип | Описание: |
|-----------|----------|------|-------------|
| <*статический-Result-Definition-имя*> | Да | Строка | Имя статического определения результата, на которое может ссылаться определение действия через `runtimeConfiguration.staticResult` объект. Дополнительные сведения см. в разделе [Настройки конфигурации среды выполнения](../logic-apps/logic-apps-workflow-actions-triggers.md#runtime-config-options). <p>Можно использовать любое уникальное имя. По умолчанию к этому уникальному имени добавляется номер, который увеличивается при необходимости. |
| <*Output-Attributes-and-Values-возвращено*> | Да | Различается | Требования к этим атрибутам зависят от различных условий. Например, если имеет значение `status` `Succeeded` , `outputs` атрибут содержит атрибуты и значения, возвращаемые действием в качестве фиктивных выходных данных. Если `status` имеет значение `Failed` , `outputs` атрибут включает `errors` атрибут, который является массивом с одним или несколькими `message` объектами ошибок, содержащими сведения об ошибках. |
| <*значения header*> | нет | JSON | Все значения заголовка, возвращаемые действием |
| <*состояние — возвращен код*> | Да | Строка | Код состояния, возвращенный действием |
| <*действие — состояние*> | Да | Строка | Состояние действия, например `Succeeded` или`Failed` |
|||||

Например, в этом определении HTTP-действия атрибуты ссылаются на атрибут, `runtimeConfiguration.staticResult.name` `HTTP0` `staticResults` где определены макеты выходных данных для действия. `runtimeConfiguration.staticResult.staticResultOptions`Атрибут указывает, что параметр статического результата имеет значение `Enabled` в действии HTTP.
...
Действие HTTP возвращает выходные данные в `HTTP0` определении внутри `staticResults` . В этом примере для кода состояния макет выходных данных имеет значение `OK` . Для значений заголовков макет выходных данных имеет значение `"Content-Type": "application/JSON"` . Для состояния действия макет выходных данных имеет значение `Succeeded` .
...
Вы также можете использовать значения, которые не существуют до начала времени выполнения. Чтобы представить эти значения, можно использовать *выражения*, которые вычисляются во время выполнения. Выражение — это последовательность, которая может содержать одну или несколько [функций](#functions), [операторов](#operators), [переменных](./logic-apps-create-variables-store-values.md), явных значений или констант. В определении рабочего процесса вы можете использовать выражение в любом месте в строчном значении JSON, указав его вместе с префиксом (\@). При вычислении выражения, представляющего значение JSON, тело выражения извлекается без символа \@ и всегда приводит к другому значению JSON.
...
* Дополнительные сведения, о том как программным способом создавать приложения логики и управлять ими с помощью [REST API рабочих процессов](/rest/api/logic/workflows).
