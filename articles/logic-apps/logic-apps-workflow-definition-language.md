...
| <*тип параметра*> | Да | int, float, String, bool, массив, Object, SecureString, secureobject <p><p>**Примечание**. для всех паролей, ключей и секретов используйте типы `securestring` или `secureobject` , так как `GET` операция не возвращает эти типы. Дополнительные сведения о защите параметров см. в разделе [рекомендации по безопасности для действий и входных параметров](../logic-apps/logic-apps-securing-a-logic-app.md#secure-action-parameters). | Тип параметра. |
...
| <*массив-с-допустимыми параметрами-Parameter*> | нет | Array | Массив со значениями, которые может принимать параметр. |
...
| <*Output-Attributes-and-Values-возвращено*> | Да | Различается | Требования к этим атрибутам зависят от различных условий. Например, если `status` имеет `Succeeded`значение, `outputs` атрибут содержит атрибуты и значения, возвращаемые действием в качестве фиктивных выходных данных. `status` Если `Failed`имеет значение `outputs` , атрибут включает `errors` атрибут, который является массивом с одним или несколькими объектами ошибок `message` , содержащими сведения об ошибках. |
...
Вы также можете использовать значения, которые не существуют до начала времени выполнения. Чтобы представить эти значения, можно использовать *выражения*, которые вычисляются во время выполнения. Выражение — это последовательность, которая может содержать одну или несколько [функций](#functions), [операторов](#operators), [переменных](https://docs.microsoft.com/azure/logic-apps/logic-apps-create-variables-store-values), явных значений или констант. В определении рабочего процесса вы можете использовать выражение в любом месте в строчном значении JSON, указав его вместе с префиксом (\@). При вычислении выражения, представляющего значение JSON, тело выражения извлекается без символа \@ и всегда приводит к другому значению JSON.
...
| <*ключ — значение*> | Да | То же, что и <*Key-Type*> | Возвращаемое значение в выходных данных. |
|||||
