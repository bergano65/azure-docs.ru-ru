...
|-----------|----------|------|-------------|
| <*имя параметра*> | Да | Строковое | Имя параметра, который требуется определить |
...
| <*Default — параметр-значение*> | Да | То же, что `type` | Значение параметра по умолчанию, используемое, если при создании экземпляра рабочего процесса не указано значение. `defaultValue` Атрибут является обязательным, чтобы конструктор приложений логики мог правильно отобразить параметр, но можно указать пустое значение. |
| <*array-with-permitted-parameter-values*> | Нет | Array | Массив со значениями, которые может принимать параметр. |
| <*Описание параметра*> | Нет | Объект JSON | Любые другие сведения о параметрах, например описание параметра. |
||||
...
| <*status-code-returned*> | Да | Строковое | Код состояния, возвращенный действием |
| <*action-status*> | Да | Строковое | Состояние действия, `Succeeded` например или`Failed` |
|||||
...
|-----------|----------|------|-------------|
| <*key-name*> | Да | Строковое | Имя ключа для возвращаемого значения в выходных данных. |
