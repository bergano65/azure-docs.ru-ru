...
| attribute | Тип | Описание |  
| --------- | ---- | ----------- |  
|мин|INT|Обязательный элемент. Минимальный номер порта в диапазоне. Возможный диапазон значений — от 1 до 65 535 включительно (пакет Azure SDK версии 1.7 и выше).|  
...
|storeLocation|строка|Обязательный элемент. Расположение хранилища сертификатов, в котором размещается этот сертификат на локальном компьютере. Возможными значениями являются `CurrentUser` и `LocalMachine`.|  
