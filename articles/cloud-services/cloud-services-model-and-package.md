...
* Я использую Visual Studio и хочу выполнить следующие действия.
  * [Создание облачного сервиса][vs_create]
...
Файл **ServiceDefinition.csdef** задает параметры, используемые Azure для настройки облачной службы. [Схема определения службы Azure (CSDEF-файл)](/previous-versions/azure/reference/ee758711(v=azure.100)) предоставляет допустимый формат для файла определения службы. В следующем примере показаны параметры, которые могут быть определены для веб-ролей и рабочих ролей.
...
Настройка параметров для облачной службы определяется значениями в файле **ServiceConfiguration.cscfg** . Вы указываете количество экземпляров, которые необходимо развернуть для каждой роли в этом файле. Значения для параметров конфигурации, которые определены в файле определения службы, добавляются в файл конфигурации службы. Отпечатки всех сертификатов управления, связанных с облачной службы, также добавляются в файл. [Схема конфигурации службы Azure (CSCFG-файл)](/previous-versions/azure/reference/ee758710(v=azure.100)) предоставляет допустимый формат файла конфигурации службы.
...
[Библиотека среды выполнения Azure](/previous-versions/azure/reference/mt419365(v=azure.100)) содержит пространство имен [Microsoft.WindowsAzure.ServiceRuntime](/previous-versions/azure/reference/ee741722(v=azure.100)), которое предоставляет классы для взаимодействия со средой Azure из роли. Класс [RoleEnvironment](/previous-versions/azure/reference/ee773173(v=azure.100)) определяет следующие события, которые вызываются до и после изменения конфигурации.

* Событие **[Changing](/previous-versions/azure/reference/ee758134(v=azure.100))**.  
...
* Событие **[Changed](/previous-versions/azure/reference/ee758129(v=azure.100))**.  
...
> [!NOTE]
> Максимальный размер пакета, который может быть развернут, составляет 600 МБ
