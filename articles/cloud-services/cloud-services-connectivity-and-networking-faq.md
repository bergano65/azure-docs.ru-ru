...
ms.locfileid: "61432965"
---
# <a name="connectivity-and-networking-issues-for-azure-cloud-services-frequently-asked-questions-faqs"></a>Подключение и сетевыми проблемами, для облачных служб Azure: Часто задаваемые вопросы (FAQ)
...
Модуль переопределения URL-адресов IIS можно использовать, чтобы перенаправлять трафик, поступающий на URL-адрес облачной службы по умолчанию (например, \*.cloudapp.net), к любому пользовательскому имени или URL-адресу. Поскольку модуль переопределения URL-адрес с включенной веб-ролей по умолчанию, и его правила настраиваются в файле web.config для приложения, его всегда можно просмотреть на виртуальной Машине независимо от любых перезагрузок или пересозданий образов. Дополнительные сведения см. в разделе:
...
При наличии более одного экземпляра каждой роли, связывание зарезервированного IP-адреса с облачной службой не будет вызывать простоев. Кроме того можно добавить в список разрешений диапазон IP-адрес центра обработки данных Azure. Можно найти все диапазоны IP-адресов Azure в [центра загрузки Майкрософт](https://www.microsoft.com/en-us/download/details.aspx?id=41653).
