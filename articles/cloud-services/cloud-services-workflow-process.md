...
8. Для полных веб-ролей IIS Вахостбутстраппер указывает Иисконфигуратор настроить IIS AppPool и указывает сайту на `E:\Sitesroot\<index>` , где — это `<index>` 0-индекс в количестве `<Sites>` элементов, определенных для службы.
