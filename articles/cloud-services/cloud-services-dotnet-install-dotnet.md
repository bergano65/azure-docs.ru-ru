...
Например, можно установить .NET Framework 4.6.2 на семейство Гостевой ОС 4, что не соответствует выпуску .NET Framework 4.6. (Семья Гостевых ОС 5 действительно поставляются с .NET Framework 4.6.) Для получения последней информации о выпусках ОС Azure Guest Можно ознакомиться с [новостями о выпуске оС Azure Guest.](cloud-services-guestos-update-matrix.md) 

>[!IMPORTANT]
>Azure SDK 2.9 содержит ограничение на развертывание .NET Framework 4.6 на семейство Гостевой ОС 4 или ранее. Исправление ограничения доступно на сайте [документации Майкрософт](https://github.com/MicrosoftDocs/azure-cloud-services-files/tree/master/Azure%20Targets%20SDK%202.9).
...
* [.NET Framework 4.7.2 веб-установщик](https://go.microsoft.com/fwlink/?LinkId=863262)
* [.NET Framework 4.6.2 веб-установщик](https://www.microsoft.com/download/details.aspx?id=53345)

Чтобы добавить установщик для *веб*-роли:
  1. В **обозревателе решений** в разделе **Роли** проекта облачной службы щелкните правой кнопкой мыши *веб-роль* и выберите **Добавить** > **Новая папка**. Создайте папку с названием **бен**.
  2. Нажмите правой папкой и выберите **Добавить** > **существующий элемент.** Выберите установщик .NET и добавьте его в папку bin.
...
> [!NOTE]
> Необходимо установить .NET Framework 4.6.2 на роль облачного сервиса, даже если ваше приложение нацелено на .NET Framework 4.6. Гостевая ОС включает [обновление 3098779](https://support.microsoft.com/kb/3098779) и [обновление 3097997](https://support.microsoft.com/kb/3097997) базы знаний. Проблемы могут возникнуть при запуске приложений .NET, если в верхней части обновлений Базы знаний установлена система .NET Framework 4.6. Чтобы избежать этих проблем, установите .NET Framework 4.6.2 вместо версии 4.6. Дополнительные сведения см. в статьях базы знаний [3118750](https://support.microsoft.com/kb/3118750) и [4340191](https://support.microsoft.com/kb/4340191).
> 
> 
...
С помощью задач запуска вы можете выполнять различные операции перед запуском роли. Установка платформы .NET Framework как части начальной задачи позволяет установить платформу до того, как будет запущено выполнение программного кода. Для получения дополнительной информации [Run startup tasks in Azure](cloud-services-startup-tasks.md)о задачах запуска см. 
...
2. Создайте файл с именем **install.cmd** и добавьте в него следующий скрипт установки.
   ...
   Скрипт путем поиска по реестру проверяет, установлена ли на компьютере выбранная версия .NET Framework. Если версия .NET Framework не установлена, открывается веб-установщик веб-сайта .NET Framework. Для помощи в устранении возникших проблем скрипт регистрирует все действия в файл startuptasklog-(текущая дата и время).txt, который хранится в локальном хранилище **InstallLogs**.
...
3. Добавьте файл install.cmd к каждой роли, используя **Добавить** > **существующий элемент** в Solution **Explorer,** как описано ранее в этой теме. 
...
* [Установки рамочной программы .NET][Troubleshooting .NET Framework Installations]
