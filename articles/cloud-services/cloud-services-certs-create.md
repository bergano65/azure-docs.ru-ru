...
>[!Note]
>В статье [об облачных сервисах - Конфигурация и управление](cloud-services-configuration-and-management-faq.md) есть полезная информация о сертификатах.
...
* Имя субъекта должно совпадать с именем домена, которое используется для обращения к облачной службе.
    ...
    > Вы не можете приобрести сертификат TLS/SSL для cloudapp.net (или для любого домена, связанного с Azure); Имя объекта должно соответствовать пользовательскому доменному имени, используемому для доступа к приложению. Например, **contoso.net**, а не **contoso.cloudapp.net**.
