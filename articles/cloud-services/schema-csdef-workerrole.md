---
title: Облачные службы Azure Def. WorkerRole Schema Документы Майкрософт
description: Роль работника Azure используется для обобщенных разработок и может выполнять фоновую обработку для веб-роли. Узнайте о схеме роли работника Azure.
...
| Атрибут | Тип | Описание |
| --------- | ---- | ----------- |
|аssemblyName|строка|Обязательный элемент. Путь и имя файла сборки, содержащей точку входа. Путь по отношению к папке `commandLine` ** \\%ROLEROOT% -Approot** (не указано ** \\% ROLEROOT% -Approot** в , предполагается). **%ROLEROOT%** — это переменная среды, поддерживаемая Azure и представляющая расположение корневой папки для роли. Папка ** \\%ROLEROOT% -Approot** представляет папку приложения для вашей роли.|
