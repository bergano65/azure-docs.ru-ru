...
|vmsize|строка|Необязательный элемент. Это значение используется для изменения размера виртуальной машины, выделяемой для роли. Значение по умолчанию — `Small`. Список возможных размеров виртуальной машины и их атрибутов см. в статье [Размеры для облачных служб](cloud-services-sizes-specs.md).|
...
|setReadyOnProcessStart|Логический|Обязательный. Указывает, должен ли экземпляр роли ожидать от программы командной строки сигнал о начале работы. Сейчас этот аргумент должен иметь значение `true`. Возможность установить значение `false` зарезервирована для будущего использования.|
