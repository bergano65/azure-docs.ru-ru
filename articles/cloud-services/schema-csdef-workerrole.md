...
|vmsize|string|Необязательный параметр. Это значение используется для изменения размера виртуальной машины, выделяемой для роли. Значение по умолчанию — `Small`. Список возможных размеров виртуальной машины и их атрибутов см. в статье [Размеры для облачных служб](cloud-services-sizes-specs.md).|
