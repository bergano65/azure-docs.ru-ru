...
Дополнительные сведения о сертификатах Azure см. в статье [Certificates overview for Azure Cloud Services](cloud-services-certs-create.md) (Общие сведения о сертификатах для облачных служб Azure). Для полного описания параметров OpenSSL [https://www.openssl.org/docs/apps/openssl.html](https://www.openssl.org/docs/apps/openssl.html)см.
...
Команда создает `.cer` файл и устанавливает его в хранилище **сертификатов Personal.** Дополнительные сведения см. в статье [Certificates overview for Azure Cloud Services](cloud-services-certs-create.md) (Общие сведения о сертификатах для облачных служб Azure).
...
При создании приложения и его запуске в Azure код и настройки вместе называются [облачной службой][cloud service] Azure. (Он был известен как *хостоввая служба* в предыдущих выпусках Azure.) Для создания нового хостового сервиса можно использовать метод **создания\_\_хосустового сервиса.** При этом укажите для нее имя (которое должно быть уникальным в Azure), метку (автоматически кодируется в base64), описание и расположение.
...
Удалять облачный сервис можно, передавая имя службы в метод **\_удаленного\_сервиса.**
...
Чтобы добавить изображение операционной системы в репозиторий изображения, используйте метод **добавления\_изображения\_os.**
...
Чтобы захватить изображение VM, вы сначала **вызовуйте метод изображения захвата\_vm.\_**
...
Теперь, когда вы изучили основы управления службами, вы можете получить доступ к [полной справочной документации API для Azure Python SDK](https://azure-sdk-for-python.readthedocs.org/) и легко выполнять сложные задачи для управления приложением Python.
