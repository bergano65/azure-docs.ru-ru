---
title: Настройте TLS для облачного сервиса Документы Майкрософт
description: Узнайте, как указать конечную точку HTTPS для веб-роли и как загрузить сертификат TLS/SSL для защиты приложения. В этих примерах используется портал Azure.
...
Transport Layer Security (TLS), ранее известный как шифрование Безопасный слой розетки (SSL), является наиболее часто используемым методом защиты данных, отправляемых через Интернет. Эта общая задача обсуждает, как указать конечную точку HTTPS для веб-роли и как загрузить сертификат TLS/SSL для защиты приложения.
...
## <a name="step-1-get-a-tlsssl-certificate"></a>Шаг 1: Получить сертификат TLS/SSL
Чтобы настроить TLS для приложения, сначала необходимо получить сертификат TLS/SSL, подписанный Сертификационным органом (CA), доверенным третьим лицом, выдавшим сертификаты для этой цели. Если у вас его еще нет, вам необходимо получить его от компании, которая продает сертификаты TLS/SSL.

Сертификат должен соответствовать следующим требованиям для сертификатов TLS/SSL в Azure:
...
2. В веб-браузере измените ссылку, указав **https** вместо **http**, и перейдите на страницу.
   ...
   > [!TIP]
   > Если требуется использовать TLS для развертывания, а не для развертывания производства, сначала необходимо определить URL-адрес, используемый для развертывания. После развертывания облачной службы URL-адрес промежуточной среды определяется по **идентификатору развертывания`https://deployment-id.cloudapp.net/` GUID и имеет следующий формат:**  
   >
