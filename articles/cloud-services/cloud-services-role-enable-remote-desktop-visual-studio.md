---
title: Использование Visual Studio включите удаленный рабочий стол для роли (облачные службы Azure)
...
5. Выберите сертификат из списка или выберите ** &lt;Создать... &gt;**. Если вы создаете новый сертификат, укажите для него понятное имя и нажмите **ОК**. Новый сертификат появится в раскрывающемся списке.
...
Когда Visual Studio общается с расширением RDP, она передает простой текстовый пароль через TLS. Но в файлах конфигурации проекта хранится только зашифрованный пароль. Чтобы получить его в обычном текстовом виде, необходимо сохранить локально и применить тот же сертификат, который использовался для его шифрования.
