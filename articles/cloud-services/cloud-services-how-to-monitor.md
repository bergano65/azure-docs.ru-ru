...
> [!IMPORTANT]
> Хотя все эти данные агрегируются в учетную запись хранилища, портал **не** предоставляет родной способ наметить данные. Настоятельно рекомендуем интегрировать в приложение другую службу, например Application Insights.
...
Затем перейдите к ресурсу **Учетная запись хранения (классика)**. Выберите**клавиши доступа** **к настройкам** > и скопируйте значение **строки основного соединения.** Это значение вы примените для облачной службы. 
