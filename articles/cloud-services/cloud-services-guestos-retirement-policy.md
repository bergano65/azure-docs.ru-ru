...
На 60 дней в продолжительности жизни, версия *"отключена*". Отключение означает, что версия удаляется из портала Azure. Ее больше нельзя указать в файле конфигурации CSCFG. Имеющиеся развертывания продолжают работать. Но новые развертывания, обновления кода и конфигурации имеющихся развертываний не разрешены.

Через некоторое время после того, как версия Гостевой ОС «истекает», и любые установки, все еще работающие с истекшим сроком действия версии, подвергаются проблемам безопасности и уязвимости. Как правило, срок действия выполняется партиями, поэтому период от отторжения до истечения может варьироваться.

Клиенты, которые настраивают свои услуги для обновления Гостевой ОС вручную, должны убедиться, что их службы работают на поддерживаемой Гостевой ОС. Если служба настроена на автоматическое обновление Гостевой ОС, базовая платформа обеспечит соответствие требованиям и будет обновлена до последней Гостевой ОС.
