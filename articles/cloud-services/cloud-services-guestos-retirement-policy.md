...
Через 60 дней в течение срока жизни версия*отключается*. Отключение означает, что версия удаляется из портала Azure. Ее больше нельзя указать в файле конфигурации CSCFG. Имеющиеся развертывания продолжают работать. Но новые развертывания, обновления кода и конфигурации имеющихся развертываний не разрешены.

Некоторое время после того как "отключено", версия гостевой ОС "истекает", и все установки с истекшим сроком действия будут подвержены проблемам безопасности и уязвимости. Как правило, истечение срока действия выполняется пакетами, поэтому период, в течение которого срок действия истекает, может отличаться.

Клиенты, которые настраивают свои службы для обновления гостевой ОС вручную, должны убедиться, что их службы работают в поддерживаемой гостевой ОС. Если служба настроена на автоматическое обновление гостевой ОС, то базовая платформа обеспечит соответствие требованиям и обновит ее до последней гостевой ОС.
