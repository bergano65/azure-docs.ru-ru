...
Для помощи в устранении таких сбоев выделения см. [См. Сбой выделения облачных служб: Решения](cloud-services-allocation-failures.md#solutions).
...
Кластеры в центре обработки данных могут иметь различные конфигурации типов машин (например, серия A, серия Av2, серия D, серия Dv2, серия G, серия H и т.д.). Однако не во всех кластерах будут присутствовать все виды виртуальных машин. Например, при попытке добавить ВМ серии D в облачную службу, которая уже развернута в кластере, в котором присутствует только серия A, может возникать ошибка выделения. Это также происходит при попытке изменить размер и SKU ВМ (например, при переключении из серии A в серию D).
...
Поскольку служба облака является классическим ресурсом, который не совместим напрямую с моделью управления ресурсами Azure, вы не можете связать его с учетными записями управления ресурсами Azure. Возможные сценарии: 
...
- Развертывание с помощью [портала Azure](https://portal.azure.com).
    ...
    Это будет работать с [портала Azure,](https://portal.azure.com) поскольку вызов проходит через прокси/шим, который позволяет общаться между менеджером ресурсов Azure и классическими ресурсами. 
