---
title: Карта виртуальных сетей между двумя регионами в восстановлении сайта Azure
description: Узнайте о картировании виртуальных сетей между двумя регионами Azure для восстановления аварийного спасения Azure VM с помощью восстановления сайта Azure.
...
В этой статье описан процесс сопоставления двух экземпляров виртуальной сети Azure, размещенных в разных регионах Azure, и настройки IP-адресации между ними. Сетевое отображение обеспечивает поведение по умолчанию для выбора целевой сети на основе исходной сети на момент включения репликации.
...
## <a name="set-up-network-mapping-manually-optional"></a>Настройка сетевого отображения вручную (необязательно)
...
3. Теперь создайте сетевое отображение в противоположном направлении. В нашем примере это означает, что источником теперь будет "Юго-Восточная Азия", а целевым регионом — "Восточная Азия".
...
- Если сетевое отображение уже произошло для исходной сети, отображаемые целевые сети всегда будут по умолчанию во время включения репликации для большего числа m. Вы можете изменить целевую виртуальную сеть, выбрав другие доступные варианты из выпадения. 
- Чтобы изменить виртуальную сеть по умолчанию для новых репликаций, необходимо изменить существующее сетевое картографическое.
- Если вы хотите изменить сетевое отображение из региона А в область B, убедитесь, что сначала удалите отображение сети из региона B в область А. После обратного удаления отображения карт измените сетевое отображение из региона А в область B, а затем создайте соответствующее обратное отображение.

>[!NOTE]
>* Изменение сетевого картирования только изменяет значения по умолчанию для новых репликаций VM. Это не влияет на целевые виртуальные выборы сети для существующих репликаций. 
>* Если вы хотите изменить целевую сеть для существующей репликации, перейдите к вычислительным и сетевым настройкам реплицированного элемента.
...
- Можно изменить целевую подсеть в настройках **вычислений и сети** для VM.
...
**Исходная и целевая подсети** | **Подробно**
--- | ---
