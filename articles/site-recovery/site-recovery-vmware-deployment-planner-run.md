---
title: Запустите планировщик развертывания для аварийного восстановления VMware с восстановлением сайта Azure
...
---
# <a name="run-the-deployment-planner-for-vmware-disaster-recovery"></a>Выполнить планировщик развертывания для аварийного восстановления VMware
...
| -Environment | (необязательно) Это целевая среда учетной записи хранения Azure. Этот параметр может иметь одно из трех значений: AzureCloud, AzureUSGovernment, AzureChinaCloud. Значение по умолчанию — AzureCloud. Используйте параметр, когда вашего целевого региона Azure является либо правительство Сша Azure, либо Azure China 21Vianet. |
...
Планировщик создает отчет в формате XLSM (файл Microsoft Excel с поддержкой макросов). В нем содержатся все рекомендации по развертыванию. Отчет называется `DeploymentPlannerReport_<unique numeric identifier>.xlsm` и помещается в указанный каталог.

>[!NOTE]
>Генерация отчетов требует Windows PC или Windows Server с Excel 2013 или позже. Десятичные символы на этой машине должны быть настроены как "." для получения сметы затрат. В случае, если у вас есть установка "," как десятичной символ, пожалуйста, перейдите на "Изменение даты, времени или числа форматов" в панели управления и перейдите на "Дополнительные настройки", чтобы изменить десятичной символ на ".".
...
| -StartDate | (Необязательно.) Дата и время начала в формате ММ-ДД-ГГГГ:ЧЧ:ММ (24-часовой цикл). *StartDate* должен быть указан вместе с *EndDate*. Если указать параметр StartDate, отчет будет создан на основе данных, собранных в период между датой начала и окончания. |
| -EndDate | (Необязательно.) Дата и время окончания в формате ММ-ДД-ГГГГ:ЧЧ:ММ (24-часовой цикл). *Дата окончания* должны быть указаны вместе с *StartDate*. Если указать параметр EndDate, отчет будет создан на основе данных, собранных в период между датой начала и окончания. |
...
* [Размещение< >хранения VM](site-recovery-vmware-deployment-planner-analyze-report.md#vm-storage-placement)
...
* [Проанализируйте генерируемый отчет](site-recovery-vmware-deployment-planner-analyze-report.md).
