...
2. Щелкните **выбрать компьютеры**и выберите компьютеры, которые требуется переместить с текущего на целевой сервер обработки. Сведения о среднем значении изменения данных отображаются возле каждой виртуальной машины. После этого щелкните **OK**. 
