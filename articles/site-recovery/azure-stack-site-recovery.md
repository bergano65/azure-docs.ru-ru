...
![На схеме показаны хранилища служб восстановления для двух клиентов в облаках, связанных с подписками клиентов, в общей инфраструктуре Azure Stack.](./media/azure-stack-site-recovery/architecture.png)
...
2. В разделе **Prepare source** (Подготовка источника) щелкните **+ Configuration Server** (+ Сервер конфигурации).
    ...
    ![Снимок экрана: диалоговое окно сервера конфигурации с сообщением "щелкните + сервер конфигурации на панели команд выше, чтобы настроить один...".](./media/azure-stack-site-recovery/plus-config-srv.png)
...
6. Скачайте ключ регистрации хранилища. При запуске программы единой установки вам потребуется ключ регистрации. Ключ действителен в течение пяти дней после создания.
    ...
    ![Снимок экрана: диалоговое окно добавления сервера с типом сервера, установленным на сервер конфигурации, и кнопка Скачать ключ регистрации хранилища.](./media/azure-stack-site-recovery/set-source2.png)
...
> [!NOTE]
> Сервер конфигурации можно также установить с помощью командной строки. [Дополнительные сведения](physical-manage-configuration-server.md#install-from-the-command-line).
>
...
8. Выберите **настроить сейчас для выбранных компьютеров** , чтобы применить параметр сети ко всем компьютерам, выбранным для защиты. Выберите **Отложить настройку**, если вы хотите выбрать сеть Azure отдельно для каждого компьютера.
...
13. Отслеживать ход выполнения задания **включения защиты** в **Settings**  >  **заданиях**"Параметры"  >  **Site Recovery задания**. После выполнения задания **Завершить подготовку защиты** виртуальная машина будет готова к отработке отказа.
...
7. Теперь вы можете подключиться к реплицированной виртуальной машине в Azure. [Дополнительные сведения](site-recovery-test-failover-to-azure.md#prepare-to-connect-to-azure-vms-after-failover).
...
Когда первичный сайт восстановится и заработает, можно восстановить размещение из Azure в Azure Stack. Для этого выполните действия, описанные [здесь](https://docs.microsoft.com/azure-stack/operator/site-recovery-failback?view=azs-2005).
