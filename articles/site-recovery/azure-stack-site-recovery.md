...
ms.locfileid: "80478518"
---
# <a name="replicate-azure-stack-vms-to-azure"></a>Репликация виртуальных машин Azure Stack в Azure

В этой статье показано, как настроить аварийное восстановление в Azure для виртуальных машин Azure Stack с помощью [службы Azure Site Recovery](site-recovery-overview.md).
...
> * **Шаг 5: Включить репликацию.** Настройте параметры репликации и включите репликацию виртуальных машин. Служба Mobility будет установлена ​​на виртуальную машину, когда репликация включена. Site Recovery выполняет начальную репликацию виртуальной машины, а затем начинается текущая репликация.
...
**Расположение** | **Компонент** |**Сведения**
--- | --- | ---
...
**Мобильный сервис** | Устанавливается на каждой виртуальной машине, которую нужно реплицировать. | В шагах в этой статье мы подготавливаем учетную запись, чтобы служба Mobility была установлена ​​автоматически на виртуальной машине, когда репликация включена. Если вы не хотите автоматически устанавливать службу, существует ряд других методов, которые можно использовать. [Подробнее](vmware-azure-install-mobility-service.md).
**Azure** | В Azure вам необходимо хранилище Служб восстановления, учетная запись хранения и виртуальная сеть. |  Реплицированные данные хранятся в учетной записи хранения. Виртуальные машины Azure добавляются в сеть Azure при отработке отказа.
...
**Ubuntu** | Сервер LTS версии 14.04 или 16.04. Просмотрите список [поддерживаемых ядер](vmware-physical-azure-support-matrix.md#ubuntu-kernel-versions)
...
- В брандмауэре Windows на виртуальной машине, которую требуется реплицировать, разрешите общий доступ к файлам и принтерам и инструментарий WMI.
    - Для этого выполните **wf.msc**, чтобы открыть консоль брандмауэра Windows. Правый клик **Входящие правила** > **Новое правило**. Щелкните **Предопределенные** и выберите пункт **общего доступа к файлам и принтерам** в списке. Завершите работу мастера, разрешите подключение и нажмите кнопку **Готово**.
...
- Включите подсистему SFTP и аутентификацию на основе пароля в файле sshd_config.
    ...
    3. Найдите строку, которая начинается с **подсистемы** и не комментируйте линию.
...
1. На портале Azure выберите **«Создайте** > **инструменты** > управления ресурсами» для**резервного копирования и восстановления сайта.**
2. В поле **Имя** введите понятное имя для определения хранилища.
...
5. Чтобы быстро получить доступ к хранилищу с панели мониторинга, выберите **Pin-код для создания панели мониторинга.** > **Create**

   ![Создание хранилища](./media/azure-stack-site-recovery/new-vault-settings.png)

   Новое хранилище отображается на **панели мониторинга** > **Все ресурсы**и на основной странице **хранилища служб восстановления.**
...
1. Нажмите **Подготовьте** > **источник**инфраструктуры .
...
Перед началом работы убедитесь, что часы на виртуальной машине [синхронизированы с сервером времени](/windows-server/networking/windows-time-service/windows-time-service-top). Если время отличается от местного более чем на пять минут, то регистрация завершится сбоем.
...
> [!NOTE]
> Сервер конфигурации можно также установить с помощью командной строки. [Подробнее](physical-manage-configuration-server.md#install-from-the-command-line).
>
> Имя учетной записи появится на портале через 15 (или больше) минут. Чтобы обновить немедленно, выберите **Настройка Сервер сервера** > ***имя*** > обновить**сервера**.

## <a name="step-4-set-up-the-target-environment"></a>Шаг 4. Настройка целевой среды

Выберите и проверьте целевые ресурсы.

1. В**целевой**группе **подготовки инфраструктуры** > выберите подписку Azure, которая требуется использовать.
...
1. Нажмите **Подготовьте** > **настройки репликации инфраструктуры.**
...
5. В поле **Периодичность создания моментальных снимков с согласованием приложений** укажите, с какой периодичностью будут создаваться моментальные снимки, согласованные на уровне приложений.
    ...
    - Снимок приложения — это моментальный снимок данных приложения внутри VM.
...
10. В **свойствах Properties** > **Configure**выберите учетную запись, которую сервер процесса будет использовать для автоматической установки Службы мобильности на машине.
11. В **настройках** > репликации**настройка параметров репликации**проверите выбранную правильную политику репликации.
12. Нажмите **Включить репликации**.
13. Отслеживайте ход выполнения задания **Включить защиту** в **настройках Заданий** > **Jobs** > **Задания восстановления сайта.** После выполнения задания **Finalize Protection** машина готова к сбою.
...
>
> Чтобы отслеживать добавленные VMs, проверьте последнее обнаруженное время для VMs в **конфигурационных серверах** > **Последний контакт в.** Чтобы добавить виртуальные машины, не дожидаясь запланированного обнаружения, выделите сервер конфигурации (не выбирая его) и нажмите кнопку **Обновить**.
...
Перед запуском тестовой отработки отказа проверьте свойства виртуальной машины и убедитесь, что виртуальная машина соответствует [требованиям Azure](vmware-physical-azure-support-matrix.md#azure-vm-requirements). Можно просмотреть и изменить свойства следующим образом:
...
3. В разделе **Вычисления и сеть** измените необходимые параметры.
    ...
    - Можно изменить имя виртуальной машины Azure, группу ресурсов, целевой размер, [группу доступности](/azure/virtual-machines/windows/tutorial-availability-sets) и параметры управляемого диска.
...
2. Операция отработки отказа обрабатывает данные с помощью указанной точки восстановления:
    ...
    - **Последнее приложение-последовательное**: Машина выходит из строя до последней точки восстановления приложения.
    - **Пользовательские**: Выберите точку восстановления, используемую для неудачи.
...
1. В **настройках** > **Реплицированные элементы**, нажмите на VM > **"Тест Failover**.
...
7. Теперь вы можете подключиться к реплицированной виртуальной машине в Azure. [Подробнее](site-recovery-test-failover-to-azure.md#prepare-to-connect-to-azure-vms-after-failover).
...
Если вы хотите подключиться к компьютеру в Azure после отработки отказа, [подготовьтесь к этому](site-recovery-test-failover-to-azure.md#prepare-to-connect-to-azure-vms-after-failover), прежде чем запустить отработку отказа.

Запустите отработку отказа, выполнив следующую команду:


1. В **настройках** > **Реплицированные элементы,** нажмите на машину > **Failover**.
...
5. Нажмите кнопку **ОК** , чтобы запустить отработку отказа. Вы можете следить за ходом сбоя на странице **Вакансии.**
...
6. Передайте виртуальный жесткий диск в Azure Stack с помощью [этих действий](/azure-stack/user/azure-stack-manage-vm-disks#use-powershell-to-add-multiple-disks-to-a-vm).
