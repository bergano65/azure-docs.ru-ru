...
- В брандмауэре Windows на виртуальной машине, которую требуется реплицировать, разрешите общий доступ к файлам и принтерам и инструментарий WMI.
    - Для этого выполните **wf.msc**, чтобы открыть консоль брандмауэра Windows. Щелкните правой кнопкой мыши **правила** > для входящих подключений**создать правило**. Щелкните **Предопределенные** и выберите пункт **общего доступа к файлам и принтерам** в списке. Завершите работу мастера, разрешите подключение и нажмите кнопку **Готово**.
...
- Включите подсистему SFTP и аутентификацию на основе пароля в файле sshd_config.
    ...
    3. Найдите строку, которая начинается с **подсистемы** , и раскомментируйте строку.
...
1. В портал Azure выберите **создать** > **средства** > управления ресурсами**резервное копирование и Site Recovery**.
...
5. Чтобы быстро получить доступ к хранилищу на панели мониторинга, выберите **закрепить на панели мониторинга** > **создать**.

   ![Создание хранилища](./media/azure-stack-site-recovery/new-vault-settings.png)

   Новое хранилище появится на **панели мониторинга** > **все ресурсы**и на главной странице **хранилища служб восстановления** .
...
5. В разделе **Ваши компьютеры виртуализированы?** выберите **Без виртуализации или иное**. Затем нажмите кнопку **OK**.
...
1. Щелкните **Подготовка инфраструктуры** > **источник**.
...
>
> Имя учетной записи появится на портале через 15 (или больше) минут. Чтобы немедленно выполнить обновление, выберите **серверы** > конфигурации***имя*** > сервера**обновить сервер**.

## <a name="step-4-set-up-the-target-environment"></a>Шаг 4. Настройка целевой среды

Выберите и проверьте целевые ресурсы.

1. В поле **Подготовка инфраструктуры** > **Цель**выберите подписку Azure, которую вы хотите использовать.
...
1. Щелкните **Подготовка инфраструктуры** > **Параметры репликации**.
...
5. В поле **Периодичность создания моментальных снимков с согласованием приложений** укажите, с какой периодичностью будут создаваться моментальные снимки, согласованные на уровне приложений.
    ...
    - Моментальный снимок с единообразием приложений — это моментальный снимок данных приложения в виртуальной машине на момент времени.
...
1. Выберите **репликация** > **источника**приложения.
...
10. В окне **Свойства** > **Настройка свойств**выберите учетную запись, которую сервер обработки будет использовать для автоматической установки службы Mobility Service на компьютере.
11. В области **Параметры** > репликации**Настройка параметров репликации**убедитесь, что выбрана правильная политика репликации.
12. Щелкните **включить репликацию**.
13. Отслеживать ход выполнения задания **включения защиты** в**заданиях** > " **Параметры** > "**Site Recovery задания**. После выполнения задания **Завершить подготовку защиты** виртуальная машина будет готова к отработке отказа.
...
>
> Чтобы отслеживать добавленные виртуальные машины, проверьте время последнего обнаружения для виртуальных машин на **серверах** > конфигурации**последний контакт в**. Чтобы добавить виртуальные машины, не дожидаясь запланированного обнаружения, выделите сервер конфигурации (не выбирая его) и нажмите кнопку **Обновить**.
...
2. Операция отработки отказа обрабатывает данные с помощью указанной точки восстановления:
    ...
    - **Последняя целостность приложений**. компьютер переключается на последнюю точку восстановления, совместимую с приложениями.
    - **Настраиваемый**: выберите точку восстановления, используемую для отработки отказа.
...
1. В окне **Параметры** > **реплицированные элементы**щелкните виртуальную машину > **+ Тестовая отработка отказа**.
...
1. В окне **Параметры** > **реплицированные элементы**щелкните компьютер > **отработки отказа**.
