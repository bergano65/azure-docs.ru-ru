---
title: Настройка аварийного восстановления VMware с помощью PowerShell в Azure Site Revoery
...
> - Создавайте учетные записи хранилища для хранения журналов репликации или данных и реплицировать vMs.
> - Выполнение отработки отказа. Настроите настройки сбоя, выполните настройки для репликации виртуальных машин.
...
Войти в подписку Azure с помощью cmdlet Connect-AzAccount:

```azurepowershell
Connect-AzAccount
```
Выберите подписку Azure, в которую нужно реплицировать виртуальные машины VMware. Используйте cmdlet Get-AzSubscription, чтобы получить список подписок Azure, к которых у вас есть доступ. Выберите подписку Azure для работы с помощью cmdlet Select-AzSubscription.
...
> [!TIP]
> Модуль Azure Site Recovery PowerShell (модуль Az.RecoveryServices) поставляется с простым в использовании псевдонимами для большинства cmdlets. Cmdlets в модуле принять форму * \<Операция **>-AzRecoveryServicesAsr**\<объект>* и имеют эквивалентные псевдонимы, которые принимают форму * \<Операция **>- ASR**\<объект>. * Вы можете заменить псевдонимы cmdlet для удобства использования.
...
В качестве альтернативы cmdlet Set-ASRVaultContext можно также использовать смдлет Import-AzRecoveryServicesrVaultSettingsFile для настройки контекста хранилища. Укажите путь, на котором находится файл ключа регистрации хранилища в качестве параметра-пути к импорту-AzRecoveryServicesrVaultSettingsFile cmdlet. Пример:
...
**Чтобы написать на управляемый диск, используйте [модуль Powershell Az.RecoveryServices 2.0.0.](https://www.powershellgallery.com/packages/Az.RecoveryServices/2.0.0-preview)** Это требует только создания учетной записи хранения журнала. Рекомендуется использовать стандартный тип учетной записи и избыточность LRS, так как он используется для хранения только временных журналов. Убедитесь, что учетная запись хранилища создана в том же регионе Azure, что и хранилище.

Если вы используете версию модуля Az.RecoveryServices старше 2.0.0, используйте следующие шаги для создания учетных записей хранения. Они будут использоваться позже для репликации виртуальных машин. Убедитесь, что эти учетные записи хранения создаются в том же регионе Azure, в котором находится хранилище. Этот шаг можно пропустить, если вы планируете использовать для репликации существующую учетную запись хранения.
...
* Учетная запись для воспроизведения виртуальной машины (только если вы реплицируете учетную запись хранения). 
* Хранилище журналов необходимо для защиты виртуальных машин к премиум-аккаунту или управляемому диску.
...
Узнайте, как автоматизировать дополнительные задачи с помощью [ссылки Azure Site Recovery PowerShell.](https://docs.microsoft.com/powershell/module/Az.RecoveryServices)
