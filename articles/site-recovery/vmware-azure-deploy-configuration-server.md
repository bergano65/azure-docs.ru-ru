...
1. В мастере управления сервером конфигурации щелкните **Настройка подключения** и выберите сетевой адаптер, который используется сервером обработки для получения трафика репликации от виртуальных машин. Затем нажмите кнопку **Save** (Сохранить). Этот параметр уже нельзя изменить после настройки. Настоятельно рекомендуется не изменять IP-адрес сервера конфигурации. Все IP-адреса, назначенные серверу конфигурации, должны быть СТАТИЧЕСКИМИ IP-адресами, а не IP-адресами DHCP.
...
Чтобы обновить сервер конфигурации до последней версии, выполните эти [инструкции](vmware-azure-manage-configuration-server.md#upgrade-the-configuration-server).
...
1. Можно ли использовать виртуальную машину, на которой установлен сервер конфигурации, для других целей?
...
5. Где можно найти последнюю версию сервера конфигурации?
    ...
    Ознакомьтесь с инструкциями по [обновлению сервера конфигурации на портале](vmware-azure-manage-configuration-server.md#upgrade-the-configuration-server). Ее можно скачать из [Центра загрузки Майкрософт](https://aka.ms/asrconfigurationserver).
...
8. Можно ли клонировать имеющийся сервер конфигурации и использовать его для оркестрации репликации?

    **Нет.** Использование клонированного компонента сервера конфигурации не поддерживается.

9. Можно ли изменить IP-адрес сервера конфигурации?
