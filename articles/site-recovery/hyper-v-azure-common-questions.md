...
[Отработка отказа](site-recovery-failover.md) не выполняется автоматически. Инициировать отработку отказа одним щелчком на портале или воспользоваться [PowerShell](/powershell/module/az.siterecovery) для активации отработки отказа.
