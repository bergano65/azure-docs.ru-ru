---
title: Общие вопросы для восстановления аварии Hyper-V с восстановлением сайта Azure
...
Да. Во время отработки отказа Site Recovery преобразует машины второго поколения в первое. При восстановлении размещения машина преобразуется обратно во второе поколение. [Читать далее](https://azure.microsoft.com/blog/2015/04/28/disaster-recovery-to-azure-enhanced-and-were-listening/).
...
Если вы являетесь администратором подписки, у вас есть необходимые разрешения на репликацию. Если нет, вам нужны разрешения на создание виртуальной машины Azure в группе ресурсов и виртуальной сети, которые указываются при настройке Site Recovery, и разрешения на запись в выбранную учетную запись хранения. Ознакомьтесь с [дополнительными сведениями](site-recovery-role-based-linked-access-control.md#permissions-required-to-enable-replication-for-new-virtual-machines).
...
### <a name="can-we-keep-on-premises-metadata-within-a-geographic-region"></a>Можем ли мы хранить данные о помещениях в пределах географического региона?
...
- Никаких конкретных шагов по кластеру делать не нужно.
...
Восстановление сайта воспроизводит данные из наместа в хранилище Azure через общедоступную точку или с помощью пиринга ExpressRoute Microsoft. Репликация через VPN-подключение "сеть — сеть" не поддерживается.

### <a name="can-i-replicate-to-azure-with-expressroute"></a>Можно ли реплицировать в Azure с помощью ExpressRoute?

Да, ExpressRoute можно использовать для репликации виртуальных машин в Azure. Восстановление сайта воспроизводит данные в учетную запись хранения Azure через общедоступную конечную точку, и вам необходимо настроить [пиринг Microsoft](../expressroute/expressroute-circuit-peerings.md#microsoftpeering) для репликации восстановления сайта. После отработки отказа виртуальных машин в виртуальную сеть Azure к ним можно получить доступ с помощью [частного пиринга](../expressroute/expressroute-circuit-peerings.md#privatepeering).


### <a name="why-cant-i-replicate-over-vpn"></a>Почему я не могу выполнить репликацию через VPN?

При репликации в Azure трафик репликации достигает конечных точек общедоступных учетных записей хранилища Azure. Таким образом, вы можете воспроизвести только через общественный интернет с ExpressRoute (Microsoft пиринг), и VPN не работает. 
...
- Failover состоит из двух частей:
...
После отработки отказа доступ к виртуальным машинам Azure можно получить через безопасное подключение к Интернету, VPN-подключение "сеть — сеть" или с помощью Azure ExpressRoute. Для подключения потребуется выполнить несколько подготовительных действий. Ознакомьтесь с [дополнительными сведениями](failover-failback-overview.md#connect-to-azure-after-failover).
...
[Failover](site-recovery-failover.md) не является автоматическим. Вы инициируете неудачи одним щелчком мыши на портале, или вы можете использовать [PowerShell,](/powershell/module/az.recoveryservices) чтобы вызвать сбой.
...
3. После завершения начальной синхронизации вы решаете выполнить отработку отказа. После его завершения, вы можете войти в помещение VM, чтобы проверить все работает, как ожидалось. На портале Azure видно, что виртуальные машины Azure остановлены.
...
Да, если отработка отказа выполнена в Azure, можно восстановить размещение в другом расположении, если изначальное расположение недоступно. Ознакомьтесь с [дополнительными сведениями](hyper-v-azure-failback.md#fail-back-to-an-alternate-location).
