...
[Отработка отказа](site-recovery-failover.md) не выполняется автоматически. Вы инициируете отработку отказа одним щелчком на портале или с помощью [PowerShell](/powershell/module/az.recoveryservices) можно запустить отработку отказа.
...
Да, если отработка отказа выполнена в Azure, можно восстановить размещение в другом расположении, если изначальное расположение недоступно. [Дополнительные сведения](hyper-v-azure-failback.md#failback-to-an-alternate-location-in-hyper-v-environment)
