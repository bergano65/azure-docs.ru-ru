...
### <a name="can-we-keep-on-premises-metadata-within-a-geographic-region"></a>Можно ли хранить локальные метаданные в географическом регионе?
...
- Никаких конкретных шагов должны выполняться в кластере.
...
При выполнении репликации в Azure трафик репликации достигает общедоступных конечных точек учетной записи хранения Azure. Таким образом репликацию можно выполнять только через общедоступный Интернет с помощью ExpressRoute (общедоступный пиринг), а VPN не будет работать. 
...
3. После завершения начальной синхронизации вы решаете выполнить отработку отказа. После ее завершения вы войти в локальной виртуальной машины для проверки правильности работы должным образом. На портале Azure видно, что виртуальные машины Azure остановлены.
