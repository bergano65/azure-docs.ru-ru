---
title: Отработка отказа во время аварийного восстановления с помощью Azure Site Recovery
...
|---|--|--|--|
|Плановая отработка отказа из-за предстоящего простоя центров обработки данных| При выполнении запланированного действия отсутствие потери данных для приложения| Для Hyper-V ASR реплицирует данные с частотой копирования, которая задается пользователем. Плановая отработка отказа используется, чтобы переопределить частоту и реплицировать последние изменения до запуска отработки отказа. <br/> <br/> 1. Планирование периода обслуживания в соответствии с процессом управления изменениями бизнеса. <br/><br/> 2. уведомлять пользователей о предстоящем простоях. <br/><br/> 3. Переведите приложение, работающее с пользователем, в автономный режим.<br/><br/>4. Инициируйте плановую отработку отказа с помощью портала ASR. Работа локальной виртуальной машины будет автоматически завершена.<br/><br/>Фактическая потеря данных приложения = 0 <br/><br/>Журнал точек восстановления можно также получить в окне сохранения для пользователя, который хочет использовать прежнюю точку восстановления. (Для Hyper-V период хранения составляет 24 часа.) Если репликация остановлена вне интервала времени окна сохранения, клиенты могут по-прежнему выполнить отработку отказа, используя последние доступные точки восстановления. | Для VMware ASR постоянно реплицирует данные с помощью CDP. Отработка отказа предоставляет пользователю возможность выполнить отработку отказа до последних данных (включая последующее завершение работы приложения)<br/><br/> 1. Планирование периода обслуживания в соответствии с процессом управления изменениями <br/><br/>2. Отправьте уведомления пользователям о предстоящем времени простоя. <br/><br/>3. Переведите приложение, работающее с пользователем, в автономный режим.<br/><br/>4. Запустите плановую отработку отказа, используя портал ASR, до последней точки после того, как приложение находится в автономном режиме. Используйте параметр "плановая отработка отказа" на портале и выберите последнюю точку для отработки отказа. Работа локальной виртуальной машины будет автоматически завершена.<br/><br/>Фактическая потеря данных приложения = 0 <br/><br/>Журнал точек восстановления в окне сохранения предоставляется для пользователя, который хочет использовать прежнюю точку восстановления. (Для VMware период хранения составляет 72 часа.) Если репликация остановлена вне интервала времени окна сохранения, клиенты могут по-прежнему выполнить отработку отказа, используя последние доступные точки восстановления.
|Отработка отказа из-за незапланированного простоя центра обработки данных (естественного или вследствие аварии ИТ) | Минимальные потери данных для приложения | 1. Инициация плана BCP в Организации <br/><br/>2. Запустите внеплановую отработку отказа с помощью портала ASR на последнюю или точку из окна сохранения (журнала).| 1. Инициируйте план BCP Организации. <br/><br/>2. Запустите внеплановую отработку отказа с помощью портала ASR на последнюю или точку из окна сохранения (журнала).


## <a name="run-a-failover"></a>Запуск отработки отказа
В этой процедуре описывается, как запустить отработку отказа для [плана восстановления](site-recovery-create-recovery-plans.md). В качестве альтернативы можно запустить отработку отказа для одной виртуальной машины или физического сервера на странице **Реплицированные элементы**, как описано [здесь](vmware-azure-tutorial-failover-failback.md#run-a-failover-to-azure).


![Переход на другой ресурс](./media/site-recovery-failover/Failover.png)
...
![Переход на другой ресурс](./media/site-recovery-failover/FailoverJob.png)
...
Azure Site Recovery поддерживает хранение букв дисков. [Узнайте больше](vmware-azure-exclude-disk.md#example-1-exclude-the-sql-server-tempdb-disk) о том, как это делается, если вы решили исключить некоторые диски.
