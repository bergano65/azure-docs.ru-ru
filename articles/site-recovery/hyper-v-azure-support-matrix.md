---
title: Поддержка аварийного восстановления вс-м вс-вс в Azure с восстановлением сайта Azure
...
--- | --- | ---
Hyper-V (без Virtual Machine Manager) |  Windows Server 2019, Windows Server 2016 (включая установку сервера сервера), Windows Server 2012 R2 с последними обновлениями | Если вы уже настроили Windows Server 2012 R2 или SCVMM 2012 R2 с помощью Azure Site Recovery и планируете обновить операционную систему, следуйте указаниям в [документации.](upgrade-2012R2-to-2016.md) 
Hyper-V (с Virtual Machine Manager) | Виртуальный машинный менеджер 2019, Виртуальный машинный менеджер 2016, Виртуальный машинный менеджер 2012 R2 | Если используется виртуальный машинный менеджер, хосты Windows Server 2019 должны управляться в Virtual Machine Manager 2019. Аналогичным образом, хосты Windows Server 2016 должны управляться в Virtual Machine Manager 2016.<br/><br/> Примечание: Failback к альтернативному местоположению не поддерживается для узлов Windows Server 2019.
...
**Действие** | **Подробно**
--- | ---
Изменение размера диска на реплицируемой виртуальной машине Hyper-V | Не поддерживается. Отключите репликацию, визуруйте изменение, а затем повторно включите репликацию для VM.
Добавление диска к реплицируемой виртуальной машине Hyper-V | Не поддерживается. Отключите репликацию, визуруйте изменение, а затем повторно включите репликацию для VM.

## <a name="hyper-v-network-configuration"></a>Конфигурация сети Hyper-V

**Компонент** | **Hyper-V с виртуальным менеджером машин** | **Hyper-V без виртуального менеджера машины**
--- | --- | ---
...
**Хранилище** | **Hyper-V с виртуальным менеджером машин** | **Hyper-V без виртуального менеджера машины**
--- | --- | --- 
NFS | Н/Д | Н/Д
SMB 3.0 | Да | Да
...
UEFI (EFI)<br></br>Мигрированный VM в Azure будет автоматически преобразован в bioS boot VM. VM должен быть запущен Windows Server 2012 и только позже. Диск ОС должен иметь до пяти разделов или меньше, а размер диска ОС должен быть менее 300 ГБ.| Да | Да
...
Диск: 4K логический и 512 байт физического сектора | Да |  Да
...
Шифрование в состоянии покоя (CMK) <br></br> (Только для отказа от управляемых дисков)| Да (через модуль PowerShell Az 3.3.0 и далее) | Да (через модуль PowerShell Az 3.3.0 и далее)
Хранилище уровня "Премиум" | Да | Да
Импортно-экспортный сервис | нет | нет
Учетные записи хранения Azure с включенным брандмауэром | Да. Для целевого хранения и кэша. | Да. Для целевого хранения и кэша.
Изменение учетной записи хранилища | Нет. Целевая учетная запись хранения Azure не может быть изменена после включения репликации. Чтобы изменить, отключить, а затем повторно включить аварийное восстановление. | нет


## <a name="azure-compute-features"></a>Вычислительные компоненты Azure

**Функция** | **Hyper-V с виртуальным менеджером машин** | **Hyper-V без виртуального менеджера машины**
--- | --- | ---
...
Архитектура операционной системы на виртуальной машине | 32-разрядный (Windows Server 2008)/64-разряд | Если не поддерживается, проверка на соответствие обязательным требованиям завершается ошибкой.
...
> [!NOTE]
> При репликации Hyper-VM из находного в Azure можно воспроизвести только одного арендатора AD из одной конкретной среды - Hyper-V сайта или Hyper-V с VMM, как это применимо.
