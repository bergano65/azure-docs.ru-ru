...
3. Выберите **защищенные элементы** -> **группы защиты** -> \<Имя_группы_защиты > "->" \<VirtualMachineName > Вы хотите восстановить размещение, и выберите **Плановая отработка отказа**.
...
8. После завершения начальной синхронизации, и вы будете готовы завершить работу виртуальной машины в Azure, щелкните **заданий** > \<задания плановой отработки отказа >> **полной отработки отказа** . Эта команда завершит работу машины Azure, передаст последние изменения на локальную виртуальную машину, которая после этого будет запущена.
