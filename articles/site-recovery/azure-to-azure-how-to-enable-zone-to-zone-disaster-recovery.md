...
ms.locfileid: "82891495"
---
# <a name="enable-zone-to-zone-disaster-recovery-for-azure-virtual-machines"></a>Включение аварийного восстановления зоны для виртуальных машин Azure

В этой статье описывается репликация, отработка отказа и восстановление размещения виртуальных машин Azure из одной зоны доступности в другой в пределах одного региона Azure.

>[!NOTE]
>
>- Site Recovery в настоящее время не поддерживает планы восстановления зоны для аварийного восстановления зоны. 
>- В настоящее время поддержка аварийного восстановления зоны в зоне ограничена двумя регионами: Юго-Восточная Азия и южная часть Соединенного Королевства.  
...
- В некоторых регионах, в которых нет парного региона в рамках одной юридической юрисдикции (например, Юго-Восточной Азии), аварийное восстановление зоны в зону может служить в качестве решения для аварийного восстановления, так как оно помогает обеспечить юридическую совместимость, так как ваши приложения и данные не пересекают национальные границы. 
...
|Шифрование дисков Azure v1 (двойной проход с AAD)     |     Поддерживается |
|Шифрование дисков Azure версии 2 (один проход, без AAD)    |    Поддерживается    |
|Неуправляемые диски    |    Не поддерживается    |
|Управляемые диски    |    Поддерживается    |
|Ключами управляет пользователь    |    Поддерживается    |
...
2. В списке операции выберите аварийное восстановление.
...
6. Изменяйте параметры по умолчанию, когда это необходимо. Для пользователей аварийного восстановления из Azure в Azure эта страница может показаться знакомой. Дополнительные сведения о параметрах, представленных в этой колонке, можно найти [здесь](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-tutorial-enable-replication) .
...
Группа Site Recovery и план группы управления производительностью Azure для достаточной мощности инфраструктуры. При запуске отработки отказа группы также помогают убедиться в том, что экземпляры виртуальных машин, защищенные Site Recovery, будут развернуты в целевой зоне.

**4. Какие операционные системы поддерживаются?**
Аварийное восстановление из зоны в зону поддерживает те же операционные системы, что и аварийное восстановление из Azure в Azure. См. таблицу поддержки [здесь](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-support-matrix).
...
Чтобы выполнить детализацию аварийного восстановления, выполните действия, описанные [здесь](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-tutorial-dr-drill).

Чтобы выполнить отработку отказа и повторно защитить виртуальные машины в дополнительной зоне, выполните действия, описанные [здесь](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-tutorial-failover-failback).

Чтобы выполнить восстановление размещения в основной зоне, выполните действия, описанные [здесь](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-tutorial-failback).