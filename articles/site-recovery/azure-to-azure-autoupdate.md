...
> [!NOTE]
> Начиная с 35 накопительный пакет обновления, вы можете выбрать существующую учетную запись службы автоматизации будут использоваться для обновления. До обновления Site Recovery создается эта учетная запись по умолчанию. Этот параметр доступен, когда вы включаете репликацию для виртуальной Машины. Если изменить этот параметр, будет применена для всех виртуальных машин Azure, защищенные в том же хранилище.
...
> [!Note]
> Любой из вариантов уведомит пользователя учетной записи службы автоматизации, используемой для управления обновлениями. Если вы используете эту функцию в хранилище в первый раз, по умолчанию создается учетной записи службы автоматизации. Кроме того можно настроить и выберите существующую учетную запись службы автоматизации. Все последующие включения репликации в том же хранилище использовать ранее созданный.
...
-  **Ошибка**: Запуска от имени Azure сертификат, используемый учетной записью службы автоматизации является с истекающим сроком действия. 

    Самозаверяющий сертификат, созданный для учетной записи запуска от имени, срок действия истекает один год с момента создания. Его можно обновить в любое время до истечения его срока действия. Если вы зарегистрировались для получения уведомлений по электронной почте, будет также получите сообщения электронной почты, когда действие требуется со стороны. Эта ошибка будет отображаться 2 месяцев до даты окончания срока действия и изменится на критическую ошибку, если истек срок действия сертификата. После истечения срока действия сертификата, автоматическое обновление не будет работать, пока же обновлении.

   **Рекомендуемое действие**: Щелкните «Исправить», а затем обновить сертификат, чтобы устранить эту проблему.
