...
Как указано в [архитектуре аварийного восстановления Azure-to-Azure,](azure-to-azure-architecture.md)служба мобильности установлена на всех виртуальных машинах Azure (VM), которые включены в репликацию из одного региона Azure в другой. При использовании автоматических обновлений каждый новый релиз обновляет расширение службы Mobility.

[!INCLUDE [updated-for-az](../../includes/updated-for-az.md)]

## <a name="how-automatic-updates-work"></a>Как работают автоматические обновления

При использовании восстановления сайта для управления обновлениями развертывается глобальная книга runbook (используется службами Azure) через учетную запись автоматизации, созданную в той же подписке, что и хранилище. В каждом хранилище используется одна учетная запись автоматизации. Для каждого VM в хранилище, runbook проверяет для активных автоматических обновлений. Если доступна новая версия расширения службы Mobility, обновление устанавливается.

Расписание выполнения по умолчанию происходит ежедневно в 12:00 в часовом поясе реплицируемой географии VM. Вы также можете изменить расписание runbook через учетную запись автоматизации.

> [!NOTE]
> Начиная с [обновления Rollup 35,](site-recovery-whats-new.md#updates-march-2019)вы можете выбрать существующую учетную запись автоматизации для использования для обновлений. До обновления Rollup 35, Восстановление сайта создало учетную запись автоматизации по умолчанию. Эту опцию можно выбрать только при включании репликации для VM. Он недоступен для VM, который уже включен репликации. Выбранная параметр применяется ко всем vMs Azure, защищенным в одном хранилище.

Включение автоматических обновлений не требует перезапуска вашего ИВ-ивки Azure и не влияет на текущее воспроизведение.

Биллинг за работу в учетной записи автоматизации основан на количестве минут выполнения работы, используемых в месяц. Выполнение задания занимает от нескольких секунд до примерно минуты каждый день и покрывается как свободные единицы. По умолчанию 500 минут включаются в качестве бесплатных единиц для учетной записи автоматизации, как показано в следующей таблице:
...
Существует несколько способов, с которыми восстановление сайта может управлять обновлениями расширения:

- [Управление как часть шага репликации включить](#manage-as-part-of-the-enable-replication-step)
...
:::image type="content" source="./media/azure-to-azure-autoupdate/enable-rep.png" alt-text="Настройки расширения":::
...
1. Из хранилища служб восстановления перейдите на **инфраструктуру восстановления** > **сайта.**
1. Под **для Azure Виртуальные машины** > **Расширение Обновления Настройки** > Позволяю**т.д. Восстановление сайта для управления,** выберите **на**.

   Чтобы управлять расширением вручную, выберите **Off**.

1. Щелкните **Сохранить**.

:::image type="content" source="./media/azure-to-azure-autoupdate/vault-toggle.png" alt-text="Настройки обновления расширения":::

> [!IMPORTANT]
> При выборе **управления восстановлением сайта**параметр применяется ко всем вхитам в хранилище.

> [!NOTE]
> Любой вариант уведомляет вас об учетной записи автоматизации, используемой для управления обновлениями. Если вы используете эту функцию в хранилище впервыйразовом упор, по умолчанию создается новая учетная запись автоматизации. Кроме того, можно настроить настройку и выбрать существующую учетную запись автоматизации. Все последующие такты для включения репликации в одном хранилище будут использовать ранее созданную учетную запись автоматизации. В настоящее время в меню выпадающих данных будут указаны только учетные записи автоматизации, которые находятся в той же группе ресурсов, что и хранилище.

> [!IMPORTANT]
> Следующий скрипт должен быть запущен в контексте учетной записи автоматизации.
Для учетной записи пользовательской автоматизации используйте следующий скрипт:
...
1. Выберите уведомление, чтобы открыть страницу выбора VM.
1. Выберите vMs, которые вы хотите обновить, а затем выберите **OK.** Для каждого выбранного VM будет застопождаться услуга Update Mobility.
...
Если вы не можете включить автоматические обновления, обратитесь к следующим распространенным ошибкам и рекомендуемым действиям:
...
- **Ошибка**: у вас нет разрешений на создание учетной записи запуска от имени Azure (субъекта-службы) и предоставление субъекту-службе роли участника.
  ...
  **Рекомендуемое действие**: Убедитесь, что учетная запись, подписанная, назначена в качестве участника и повторите попытку. Для получения дополнительной информации о назначении разрешений смотрите раздел [«Как: Используйте портал для создания приложения Azure AD и принципа службы, которое может получить доступ к ресурсам.](/azure/azure-resource-manager/resource-group-create-service-principal-portal#required-permissions)
  ...
  :::image type="content" source="./media/azure-to-azure-autoupdate/repair.png" alt-text="Кнопка восстановления службы восстановления сайта в настройках обновления расширения":::

- **Ошибка**: учетная запись запуска от имени не имеет разрешения на доступ к ресурсу служб восстановления.

  **Рекомендуемое действие**: Удалить, а затем [воссоздать Run As аккаунт](/azure/automation/automation-create-runas-account). Убедитесь, что приложение «Активный каталог» компании Automation Run As As,sAzure Active Directory может получить доступ к ресурсу служб восстановления.
...
- **Ошибка**: учетная запись запуска от имени не найдена. Не был создан или был удален один из следующих объектов: приложение Azure Active Directory, субъект-служба, роль, ресурс сертификата службы автоматизации, ресурс подключения службы автоматизации. Либо не совпадает отпечаток у сертификата и подключения.
  ...
  **Рекомендуемое действие**: Удалить, а затем [воссоздать Run As аккаунт](/azure/automation/automation-create-runas-account).
...
- **Ошибка**: Azure Run как сертификат, используемый учетной записью автоматизации, вот-вот истечет.
  ...
  **Рекомендуемые действия**: Для решения этой проблемы выберите **Repair,** а затем **возобновить сертификат**.

  :::image type="content" source="./media/azure-to-azure-autoupdate/automation-account-renew-runas-certificate.PNG" alt-text="возобновление сертификации":::

  > [!NOTE]
  > После обновления сертификата обновите страницу для отображения текущего состояния.
