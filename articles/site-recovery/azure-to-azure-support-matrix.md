...
ms.locfileid: "50241262"
---
# <a name="support-matrix-for-disaster-recovery-of-azure-vms-between-azure-regions"></a>Матрица поддержки для аварийного восстановления виртуальных машин Azure между регионами Azure
...
Шифрование дисков Azure для Windows | Виртуальные машины с включенным [шифрованием с помощью приложения Azure AD](https://aka.ms/ade-aad-app) поддерживаются. |
Шифрование дисков Azure для Linux | Не поддерживается |
"Горячее" добавление/удаление диска    | Не поддерживается | При добавлении или удалении диска данных с виртуальной машины вам необходимо сначала отключить репликацию для виртуальных машин, а затем включить ее снова.
...
Прокси-сервер, не прошедший проверку подлинности | Поддерживаются | [Узнайте больше](site-recovery-azure-to-azure-networking-guidance.md).   
Прокси-сервер, прошедший проверку подлинности | Не поддерживается | Невозможно реплицировать виртуальную машину, если она использует аутентифицированный прокси-сервер для исходящих подключений     
VPN "сайт — сайт" | Поддерживается с ExpressRoute или без него | Убедитесь, что определяемые пользователем маршруты и сетевые группы безопасности настраиваются так, чтоб трафик Site Recovery не перенаправлялся в локальную среду. [Узнайте больше](site-recovery-azure-to-azure-networking-guidance.md).
Подключение типа "виртуальная сеть — виртуальная сеть" | Поддерживаются |[Узнайте больше](site-recovery-azure-to-azure-networking-guidance.md). 
Конечные точки службы виртуальной сети | Поддерживаются | Обратите внимание, что брандмауэры службы хранилища Azure для виртуальных сетей не поддерживаются. Кроме того, разрешение доступа к определенным виртуальным сетям Azure для учетных записей хранения кэша, используемых для хранения реплицируемых данных, не поддерживается.
