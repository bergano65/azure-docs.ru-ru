...
- **Возможная причина** </br>
  - Не удается подключиться к конечным точкам Site Recovery из-за ошибки разрешения DNS.
...
> [!NOTE]
> Если виртуальные машины размещены за **стандартный** внутренней подсистемы балансировки нагрузки, то оно не будет доступа к IP-адресов Office 365 т. е. Login.microsoftonline.com по умолчанию. Либо измените его, чтобы **основные** внутренний тип подсистемы балансировки нагрузки или создайте out привязку, как упоминалось в [статье](https://aka.ms/lboutboundrulescli).
...
- **Возможная причина** </br>
  - Недопустимые параметры настраиваемого прокси-сервера и агента службы мобильности Azure Site Recovery была не автоматически определять параметры прокси-сервера из Internet Explorer
...
- **Способы устранения:**
  ...
  2. Если вы хотите задать прокси-сервер только для Azure Site Recovery Mobility Service, можно указать данные прокси-сервера в ProxyInfo.conf, расположенный в:</br>
  ...
  3. Файл ProxyInfo.conf должен содержать параметры прокси-сервера в таком формате INI.</br>
                ...
                *Port=567*</br>
  4. Агент Azure Site Recovery Mobility Service поддерживает только ***не прошедшие проверку подлинности учетных записей-посредников***.


### <a name="fix-the-problem"></a>Устранение проблемы
Чтобы разрешить [необходимые URL-адреса](azure-to-azure-about-networking.md#outbound-connectivity-for-urls) или [диапазоны IP-адресов](azure-to-azure-about-networking.md#outbound-connectivity-for-ip-address-ranges), выполните действия, описанные в [документации по настройке сети](site-recovery-azure-to-azure-networking-guidance.md).
...
## <a name="remove-the-virtual-machine-from-the-vault-completed-with-information--error-code-150225"></a>Удалите виртуальную машину из хранилища, в вводятся данные (код ошибки 150225)
Во время защиты виртуальной машины Azure Site Recovery создает некоторые ссылки на исходной виртуальной машины. Если снять защиту или отключить репликацию, Azure Site Recovery удаления этих ссылок в рамках задания очистки. Если на виртуальной машине установлена блокировка ресурса задание получает завершено со сведениями. Он сообщает, что виртуальная машина удалена из хранилища служб восстановления, но некоторые устаревшие ссылки не удалось очистить между исходным компьютером.

Это предупреждение можно игнорировать, если вы никогда не собираетесь защитить эту виртуальную машину в будущем. Тем не менее при наличии для защиты этой виртуальной машины позднее затем следует очистить ссылки как уже упоминалось в указанные ниже действия. 

**Если этого не сделать clean up затем:**

1.  Во время включения репликации через хранилище служб восстановления виртуальной машины не отображаются. 
2.  При попытке защитить виртуальную Машину через **виртуальной машины > Параметры > аварийного восстановления** будет происходить сбой с ошибкой "*невозможно включить репликацию из-за существующих ссылок на устаревшие ресурсы на виртуальной Машине*".
...
>[!NOTE]
>
>Azure Site Recovery не удалите исходную виртуальную машину или на нее влияют, любым способом, выполняя следующие действия.
>

1. Снять блокировку на виртуальной Машине или группе ресурсов. Пример: Ниже виртуальной Машины имя «MoveDemo» имеет блокировка ресурса, который необходимо удалить.

   ![Network_Selection_greyed_out](./media/site-recovery-azure-to-azure-troubleshoot/vm-locks.png)
2. Скачайте сценарий [удаления устаревшей конфигурации Azure Site Recovery](https://github.com/AsrOneSdk/published-scripts/blob/master/Cleanup-Stale-ASR-Config-Azure-VM.ps1).
3. Выполните сценарий *Cleanup-stale-asr-config-Azure-VM.ps1*.
4. Укажите имя подписки ID, группа ресурсов виртуальной Машины и виртуальную Машину в качестве параметра.
5. При запросе учетных данных Azure, укажите и проверьте, что сценарий выполняется без сбоев. 


## <a name="replication-cannot-be-enabled-because-of-the-existing-stale-resource-links-on-the-vm-error-code-150226"></a>Не удается включить репликацию из-за существующих ссылок на устаревшие ресурсы на виртуальной Машине (код ошибки 150226)

**Причина. Виртуальная машина имеет устаревшую конфигурацию, оставшиеся после предыдущей защиты Site Recovery**

Устаревшая конфигурация может остаться на виртуальной машине Azure в следующих случаях:

- Вы включили репликацию для виртуальной Машины Azure с помощью Site Recovery, а затем отключите репликацию, но **исходной виртуальной Машины было блокировки ресурсов**.
...
1. Снять блокировку на виртуальной Машине или группе ресурсов, если таковые имеются. *Например:* Ниже виртуальной Машины имя «MoveDemo» имеет блокировка ресурса, который необходимо удалить.
...
Azure Site Recovery в настоящее время предписаний, которые являются источниками область группы ресурсов и виртуальных машин должен находиться в том же расположении. Если это не так затем вы не сможет найти во время защиты виртуальной машины или группы ресурсов. 

**В качестве решения**, можно включить репликацию из виртуальной Машины, а не в хранилище служб восстановления. Перейдите к виртуальной Машине источник > Свойства > аварийное восстановление и включить репликацию.
...
> [!NOTE]
>
> Обновите модуль AzureRM.Resources перед использованием упомянутого ниже скрипта. Azure Site Recovery не удалите исходную виртуальную машину или на нее влияют, любым способом, выполняя следующие действия.
>

1. Снять блокировку на виртуальной Машине или группе ресурсов, если таковые имеются. *Например:* Ниже виртуальной Машины имя «MoveDemo» имеет блокировка ресурса, который необходимо удалить.

   ![Network_Selection_greyed_out](./media/site-recovery-azure-to-azure-troubleshoot/vm-locks.png)
2. Скачайте сценарий [Удалить устаревшую конфигурацию](https://github.com/AsrOneSdk/published-scripts/blob/master/Cleanup-Stale-ASR-Config-Azure-VM.ps1).
3. Выполните сценарий *Cleanup-stale-asr-config-Azure-VM.ps1*.
4. Укажите имя подписки ID, группа ресурсов виртуальной Машины и виртуальную Машину в качестве параметра.
5. При запросе учетных данных Azure, укажите и проверьте, что сценарий выполняется без сбоев.
