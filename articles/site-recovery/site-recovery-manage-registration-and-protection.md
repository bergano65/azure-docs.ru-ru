...
3. [Удалить сервер конфигурации](vmware-azure-manage-configuration-server.md#delete-or-unregister-a-configuration-server)
...
2. Удалите все сопоставления сетей, используемые облаками на этом сервере. В **инфраструктуре** > восстановления сайта для**системного центра VMM** > **Network Картирование**, право нажмите на сетевое картирование > **Удалить**.
3. Запишите идентификатор сервера VMM.
4. Отмените связь с политиками репликации для облаков на сервере VMM, который нужно удалить.  В **инфраструктуре** > восстановления сайта для**системного центра VMM** >  **Репликация политики**, дважды нажмите на связанную политику. Щелкните облако правой кнопкой мыши и выберите **Disassociate** (Отменить связь).
5. Удалите сервер VMM или его запущенный узел. В **инфраструктуре** > восстановления сайта для**системного центра VMM** > **VMM Servers**, правой кнопкой мыши на сервере > **удалить**.
6. Если сервер VMM был в отключенном состоянии, то скачайте и запустите [сценарий очистки](https://aka.ms/asr-cleanup-script-vmm) на этом сервере VMM. Откройте PowerShell с опцией **Run as Administrator,** чтобы изменить политику выполнения для области выполнения по умолчанию (LocalMachine). В скрипте укажите идентификатор сервера VMM, который требуется удалить. Скрипт удалит с сервера информацию о регистрации и связывании облаков.
...
2. Отмените политики для сайта Hyper-V. В **инфраструктуре** > восстановления сайта для политик**репликации****сайтов Hyper-V** >  — дважды щелкните связанную политику. Щелкните сайт правой кнопкой мыши и выберите **Отменить**.
3. Удалите узлы Hyper-V. В **инфраструктуре** > восстановления сайта для**Hyper-V сайтов** > **Hyper-V Хосты**, право йгнул на сервер > **удалить**.
4. После того как все узлы на сайте Hyper-V будут удалены, удалите сайт Hyper-V. В **инфраструктуре** > восстановления сайта для**Hyper-V сайтов** > **Hyper-V сайтов**, право нажмите на сайт > **удалить**.
...
> [!NOTE]
> Если вы уже сработали из-за VM и работаете в Azure, обратите внимание, что защита от службы не удаляет/влияет на сбой в VM.
## <a name="disable-protection-for-a-azure-vm-azure-to-azure"></a>Отменяемая защита для Azure VM (Azure to Azure)

-  В **защищенных элементах** > **Реплицированные элементы,** правой кнопкой мыши машина > отключить **репликацию.**
> [!NOTE]
