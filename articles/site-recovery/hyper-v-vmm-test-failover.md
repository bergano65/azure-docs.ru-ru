---
title: Выполните отработку аварийного восстановления Нхипер-V на дополнительный сайт с помощью Azure Site Recovery
...
| **Использовать существующую** | Тестовая виртуальная машина создается на том же узле, что и реплика виртуальной машины. Она не будет добавлена в облако.<br/><br/>Создайте сеть виртуальных машин, которая будет изолирована от вашей рабочей сети.<br/><br/>Если вы используете сеть на основе виртуальной локальной сети, то рекомендуется создать в VMM отдельную логическую сеть (которая не будет применяться в рабочих целях). Эта логическая сеть используется для создания сетей виртуальных машин для тестовой отработки отказа.<br/><br/>Логическая сеть должна быть связана по крайней мере с одним сетевым адаптером всех серверов Hyper-V, на которых размещаются виртуальные машины.<br/><br/>Сайты сети, добавляемые в виртуальную логическую сеть, должны быть изолированы.<br/><br/>При использовании логической сети на базе виртуализации сети Windows Azure Site Recovery автоматически создает изолированные сети виртуальных машин. | |
...
В этой процедуре описывается, как запустить тестовую отработку отказа для плана восстановления. Кроме того, можно запустить отработку отказа для одной виртуальной машины на вкладке **виртуальные машины** .

1. Выберите **планы восстановления**  >  *recoveryplan_name*. Щелкните **Failover**  >  **Тестовая отработка**отказа.
