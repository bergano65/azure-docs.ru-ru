---
title: Планирование потенциала для восстановления аварии Hyper-V с восстановлением сайта Azure
...
Планировщик развертывания сайта Azure (site-recovery-hyper-v-deployment-planner.md) для развертывания Hyper-V в Azure обеспечивает следующее:
...
4. Указав значения для исходной среды, вы увидите следующие выходные данные.
   ...
   * **Требуется количество серверов конфигурации:** отображается количество серверов конфигурации, необходимых для развертывания.
   ...
   * **100% дополнительное хранилище на Источнике**: Показывает, требуется ли дополнительное хранилище в исходном месте.
...
3. На листе **Workload Qualification** (Оценка рабочей нагрузки) введите необходимую информацию. Необходимо заполнить все помеченные поля.
   ...
   а. В **процессорных ядрах**укажите общее количество ядер на исходном сервере.
   ...
   d. В **Total Storage (в ГБ)** укажите общий размер хранилища VM. Например, если на исходном сервере есть три диска, каждый по 500 ГБ, общий размер хранилища составляет 1500 ГБ.
...
1. Открываемый лист **планировщика ресурсов** заполняется данными в зависимости от заданных вами параметров. Слово "Рабочая нагрузка" отображается в исходной ячейке **infra inputs,** чтобы показать, что входный данный является рабочим листом **квалификации рабочей нагрузки.**
