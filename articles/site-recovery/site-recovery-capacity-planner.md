...
3. На листе **Планировщик ресурсов** введите необходимую информацию. Заполните все поля, помеченные красными кружками на снимке экрана ниже.
   ...
   1\. В списке **Select your scenario** (Выберите сценарий) выберите пункт **Из Hyper-V в Azure** или **Из VMware или физического сервера в Azure**.
   ...
   В. Параметр **Compression** (Сжатие) не используется при репликации виртуальных машин Hyper-V в Azure. Чтобы использовать сжатие, воспользуйтесь устройством сторонних производителей, например Riverbed.

   Г. В **Retention in days** (Период хранения в днях) укажите, как долго следует хранить реплики (в днях).
...
4. Указав значения для исходной среды, вы увидите следующие выходные данные.
   ...
   * **100 % дополнительного хранилища в исходной среде**. Показывает, требуется ли дополнительное хранилище в исходной среде.
      ...
      ![Вывод](./media/site-recovery-capacity-planner/output.png)
...
3. На листе **Workload Qualification** (Оценка рабочей нагрузки) введите необходимую информацию. Необходимо заполнить все помеченные поля.
   ...
   1\. В поле **Processor Cores** (Ядра процессора) укажите общее число ядер исходного сервера.

   2\. В поле **Memory allocation (in MBs)** (Выделение памяти (в МБ)) укажите объем ОЗУ исходного сервера.

   В. В поле **Number of NICs** (Число сетевых карт) укажите количество сетевых карт исходного сервера.

   Г. В поле **Total Storage (in GB)** (Общий объем хранилища (в ГБ)) укажите общий размер хранилища виртуальной машины. Например, если на исходном сервере есть три диска, каждый по 500 ГБ, общий размер хранилища составляет 1500 ГБ.
