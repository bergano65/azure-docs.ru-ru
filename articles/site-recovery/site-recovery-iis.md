---
title: Настройка аварийного восстановления для веб-приложения IIS с помощью восстановления сайта Azure
...
Если используется статический IP-адрес, вы можете указать нужный IP-адрес для виртуальной машины. Чтобы установить IP-адрес, перейдите к **настройкам** > compute и Network TARGET**IP.**
...
![Скриншот, на который показано настройка привязки TLS/SSL](./media/site-recovery-iis/sslbinding.png)
...
#### <a name="tlsssl-certificate-binding-for-an-https-connection"></a>Привязка сертификата TLS/SSL для подключения HTTPS
Веб-сайт может иметь связанный сертификат TLS/SSL, который помогает обеспечить безопасную связь между веб-сервером и браузером пользователя. Если веб-сайт имеет подключение HTTPS, а также имеет связанный сайт HTTPS привязки к IP-адресу сервера IIS с привязкой сертификата TLS/SSL, необходимо добавить новый привязку сайта к сертификату с IP-адресом виртуальной машины IIS после неудачи.

Сертификат TLS/SSL может быть выдан в отношении следующих компонентов:

* Полного доменного имени веб-сайта.
* Имени сервера.
* Группового сертификата для доменного имени.  
* IP-адреса. Если сертификат TLS/SSL выдается на IP-адрес сервера IIS, другой сертификат TLS/SSL должен быть выдан на IP-адрес сервера IIS на сайте Azure. Необходимо создать дополнительную привязку TLS для этого сертификата. В связи с этим мы рекомендуем не использовать сертификат TLS/SSL, выданный на IP-адрес. Этот вариант используется не часто, а вскоре он будет объявлен устаревшим в соответствии с изменениями на форуме CA/Browser.
