---
title: Настройка аварийного восстановления для веб-приложения IIS с помощью Azure Site Recovery
...
* [Проектирование сети для аварийного восстановления](./concepts-on-premises-to-azure-networking.md)
...
Сертификат TLS/SSL может быть выдан для следующих компонентов:

* Полного доменного имени веб-сайта.
* Имени сервера.
* Группового сертификата для доменного имени.  
* IP-адреса. Если для IP-адреса сервера IIS выдается сертификат TLS/SSL, то для IP-адреса сервера IIS на сайте Azure должен быть выдан еще один сертификат TLS/SSL. Необходимо создать дополнительную привязку TLS для этого сертификата. Поэтому мы рекомендуем не использовать сертификат TLS/SSL, выданный для IP-адреса. Этот вариант используется не часто, а вскоре он будет объявлен устаревшим в соответствии с изменениями на форуме CA/Browser.
