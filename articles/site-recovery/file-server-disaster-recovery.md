...
* **Используйте Azure File Sync для воспроизведения файлов:** Если вы планируете использовать облако или уже используете Azure VM, вы можете использовать Azure File Sync. Azure File Sync предлагает синхронизацию полностью управляемых файлов в облаке, которые доступны через стандартный в отрасли протокол [Server Message Block](https://msdn.microsoft.com/library/windows/desktop/aa365233.aspx) (SMB). Файловые ресурсы Azure можно затем одновременно подключить к облачным или локальным развертываниям Windows, Linux и macOS. 
...
2. [Создайте общий файл.](https://docs.microsoft.com/azure/storage/files/storage-how-to-create-file-share)
...
1. [Подготовьте ресурсы Azure](tutorial-prepare-azure.md) для репликации наемных машин.
...
1. [Создайте учетную запись хранения в Azure](https://docs.microsoft.com/azure/storage/common/storage-create-storage-account?toc=%2fazure%2fstorage%2ffiles%2ftoc.json). Если вы выбрали геоизбыточное хранилище с доступом на чтение (рекомендуется) для учетных записей хранения, то у вас будет доступ только на чтение к своим данным из дополнительного региона в случае сбоя. Для получения дополнительной информации в Azure Storage можно ознакомиться на примере [аварийного восстановления и принудительного сбоя (предварительного просмотра) в Хранилище Azure.](../storage/common/storage-disaster-recovery-guidance.md?toc=%2fazure%2fstorage%2ffiless%2ftoc.json)
2. [Создайте общий файл.](https://docs.microsoft.com/azure/storage/files/storage-how-to-create-file-share)
