...
Правильная работа корпоративных приложений, таких как SharePoint, Dynamics AX и SAP, зависит от Active Directory и инфраструктуры DNS. При настройке аварийного восстановления приложений часто требуется восстановление системы Active Directory и Domain Name System (DNS) перед восстановлением других компонентов приложения, чтобы обеспечить правильную функциональность приложения.
...
- Необходимо настроить репликацию восстановления сайта по крайней мере на одной виртуальной машине (VM), в размещении контроллера домена или DNS.
...
1. Контроллер домена должен быть владельцем роли гибкого одного мастера (FSMO) для ролей, необходимых во время сбоя теста. В противном случае эти роли должны быть [заняты](https://support.microsoft.com/help/255504/using-ntdsutil-exe-to-transfer-or-seize-fsmo-roles-to-a-domain-control) после отработки отказа.
...
Создайте контроллер домена на дополнительном сайте. При продвижении сервера до роли контроллера домена укажите имя того же домена, который используется на основном сайте. Чтобы настроить параметры объекта связывания сайтов, в который добавляются сайты, можно использовать оснастку **Active Directory — сайты и службы**. Настраивая параметры связи между сайтами, можно указать время и периодичность репликации между двумя или несколькими сайтами. Для получения дополнительной информации смотрите [копирование расписания между сайтами.](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc731862(v=ws.11))
...
Чтобы избежать влияния на производственные нагрузки, сбой теста происходит в сети, изолированной от производственной сети.
...
Начиная с Windows Server 2012 в [Active Directory Domain Services (AD DS) предусмотрены дополнительные меры безопасности](/windows-server/identity/ad-ds/introduction-to-active-directory-domain-services-ad-ds-virtualization-level-100). Эти гарантии помогают защитить виртуализированные контроллеры доменов от откатов последовательности обновлений (USN), если базовая гипервизорная платформа поддерживает **VM-GenerationID.** Azure поддерживает **VM-GenerationID**. А это значит, что контроллеры доменов, которые работают под управлением Windows Server 2012 или более поздних версий на виртуальных машинах Azure, будут защищены дополнительными мерами безопасности.

Когда выполняется сброс **VM-GenerationID**, значение **InvocationID** базы данных AD DS также сбрасывается. Кроме того, пул относительного идентификатора (RID) отбрасывается, а `SYSVOL` папка помечается как неавторитетная. Для получения дополнительной информации [см. Введение в active Directory Domain Services виртуализации](/windows-server/identity/ad-ds/introduction-to-active-directory-domain-services-ad-ds-virtualization-level-100) и [безопасной виртуализации распределенных файлов репликации системы (DFSR)](https://techcommunity.microsoft.com/t5/storage-at-microsoft/safely-virtualizing-dfsr/ba-p/424671).
...
Так как этот контроллер домена будет использоваться только для тестовой отработки отказа, меры безопасности виртуализации не обязательны. Чтобы убедиться, что значение **VM-GenerationID** для виртуальной машины контроллера домена `DWORD` не изменится, вы можете изменить значение следующих до **4** в предварительном контроллере домена:
...
- Изменение значения **InvocationID:**

  :::image type="content" source="./media/site-recovery-active-directory/Event1109.png" alt-text="Изменение идентификатора вызова":::

- `SYSVOL`папка `NETLOGON` и акции недоступны.

  :::image type="content" source="./media/site-recovery-active-directory/sysvolshare.png" alt-text="Доля папок SYSVOL":::
...
1. В запросе команды запустите следующую `SYSVOL` команду, `NETLOGON` чтобы проверить, являются ли общие папки и папки:
...
1. Выполните заслуживающее доверия восстановление контроллера домена. Примите во внимание указанные ниже сведения.
    ...
    - Хотя мы не рекомендуем репликацию с помощью [службы репликации файлов (FRS),](https://techcommunity.microsoft.com/t5/storage-at-microsoft/the-end-is-nigh-for-frs-8211-updated-for-ws2016/ba-p/425379)если вы используете репликацию FRS, следуйте шагам для авторитетного восстановления. Процесс описан в статье об [использовании раздела реестра BurFlags для повторной инициализации службы репликации файлов](https://support.microsoft.com/kb/290762).

      Дополнительные сведения о BurFlags см. в этой [записи блога](/archive/blogs/janelewis/d2-and-d4-what-is-it-for).

    - Если используется репликация DFSR, выполните действия для заслуживающего доверия восстановления. Процесс описан в [Силе авторитетной и неавторитетной синхронизации для папки SYSVOL, воспроизведенной DFSR (например, "D4/D2" для FRS).](https://support.microsoft.com/kb/2218556)
...
1. Пропустите требование начальной синхронизации, задав следующему разделу реестра значение **0** на локальном контроллере домена. `DWORD` Если не существует, его можно создать под узла **параметрами.**
...
1. Отключите требование, чтобы сервер глобального каталога был доступен для проверки входа пользователя. Для этого в локальном контроллере домена присвойте следующему разделу реестра значение **1**. Если `DWORD` не существует, вы можете создать его под узла **Lsa.**

   `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Lsa\IgnoreGCFailures`

   Для получения дополнительной [How the Global Catalog Works](/previous-versions/windows/it-pro/windows-server-2003/cc737410(v=ws.10))информации см.
...
Если DNS находится не на той же виртуальной машине, что и контроллер домена, необходимо создать виртуальную машину DNS для тестовой отработки отказа. Вы можете использовать новый DNS-сервер и создать все необходимые зоны. Например, если домен Active `contoso.com`Directory — это, можно `contoso.com`создать зону DNS с именем. Записи DNS, соответствующие Active Directory, необходимо обновить следующим образом:
...
[Подробнее](site-recovery-workload.md) о защите рабочих нагрузок предприятия при восстановлении сайтов Azure.
