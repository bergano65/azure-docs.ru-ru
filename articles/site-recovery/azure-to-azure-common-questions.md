---
title: Часто задаваемые вопросы о аварийного восстановления Azure в Azure с помощью Azure Site Recovery
description: В этой статье содержатся ответы на часто задаваемые вопросы об аварийном восстановлении виртуальных машин Azure в другой регион Azure, с помощью Azure Site Recovery
author: asgang
manager: rochakm
...
ms.locfileid: "64925672"
---
# <a name="common-questions-azure-to-azure-disaster-recovery"></a>Часто задаваемые вопросы: Azure в Azure аварийного восстановления

В этой статье содержатся ответы на часто задаваемые вопросы об аварийном восстановлении виртуальных машин Azure в другой регион Azure с помощью [Site Recovery](site-recovery-overview.md). 
...
### <a name="where-can-i-find-best-practices-for-azure-vm-disaster-recovery"></a>Где можно найти рекомендации для аварийного восстановления виртуальных Машин Azure? 
...
Команда Site Recovery работает с группой управления мощности Azure для планирования достаточными возможностями инфраструктуры, и для защиты виртуальных машин, защищенных службой Site Recovery для успешно будет развернутой целевом регионе после запуска отработки отказа.
...
Да, вы можете исключить диски во время защиты с помощью PowerShell. Дополнительные сведения см. в разделе [статьи](azure-to-azure-exclude-disks.md)

### <a name="can-i-add-new-disks-to-replicated-vms-and-enable-replication-for-them"></a>Можно ли добавить новые диски для реплицированных виртуальных машин и включить для них репликацию?

Да, это поддерживается для виртуальных машин Azure с управляемыми дисками. При добавлении нового диска к виртуальной Машине Azure, которая включена для репликации, состояние работоспособности репликации для виртуальной Машины отображает предупреждение, с указанием Обратите внимание, что для защиты доступны один или несколько дисков на виртуальной Машине. Вы можете включить репликацию для добавленных дисков.
- Если включить защиту для всех добавленных дисков, это предупреждение исчезнет после начальной репликации.
- Если вы не захотите включить репликацию для диска, можно выбрать, чтобы отклонить предупреждение.
- При выполнении отработки отказа виртуальной Машины, к которому добавить диск и включить для нее репликацию, точки репликации покажет диски, которые доступны для восстановления. Например если у одного диска виртуальной Машины и добавить новую, точки репликации, которые были созданы перед добавлением диска покажет, что точка репликации состоит из «1 из 2 дисков».

Site Recovery не поддерживает «горячее удаление» диска из реплицированной виртуальной Машиной. При удалении диска виртуальной Машины, необходимо отключить и повторно включить репликацию для виртуальной Машины.
...
Site Recovery можно создает точки восстановления, согласованные с приложением с минимальным частотой в 1 час.
...
### <a name="how-is-capacity-assured-in-target-region-for-azure-vms"></a>Как емкость уверен в целевом регионе для виртуальных машин Azure?
Команда Site Recovery работает с группой управления мощности Azure для планирования инфраструктуры емкостью, достаточной, чтобы убедиться, что включен для аварийного восстановления виртуальных машин будет развертываться успешно в целевом регионе после запуска отработки отказа.
...
Не удается сохранить общедоступный IP-адрес рабочего приложения после отработки отказа.
...
- Можно сделать вручную или автоматизацию с помощью плана восстановления.
- Узнайте, как [настроить общедоступный IP-адресов после отработки отказа](concepts-public-ip-address-with-site-recovery.md#public-ip-address-assignment-using-recovery-plan).  

### <a name="can-i-retain-a-private-ip-address-during-failover"></a>Можно ли сохранить частный IP-адрес во время отработки отказа?
Да, вы можете сохранить частный IP-адрес. По умолчанию при включении аварийного восстановления для виртуальных машин Azure служба Site Recovery создает целевые ресурсы на основе параметров исходных ресурсов. — Для виртуальных машин Azure, настроены статические IP-адреса Site Recovery попытается подготовить же IP-адрес для целевой виртуальной Машине, в том случае, если он не используется.
Дополнительные сведения о [сохранение IP-адресов во время отработки отказа](site-recovery-retain-ip-azure-vm-failover.md).

### <a name="after-failover-why-is-the-server-assigned-a-new-ip-address"></a>После отработки отказа, почему серверу назначается новый IP-адрес?
...
Дополнительные сведения о [Настройка сетевого сопоставления и назначения IP-адресов для виртуальных сетей](azure-to-azure-network-mapping.md#set-up-ip-addressing-for-target-vms).
...
### <a name="what-happens-if-my-primary-region-experiences-an-unexpected-outage"></a>Что произойдет, если мой основной регион, произошел непредвиденный сбой?
...
После повторного включения защиты количество времени для восстановления размещения обычно похоже на время, необходимым для отработки отказа из основного региона в дополнительный регион.

## <a name="capacity"></a>емкость

### <a name="how-is-capacity-assured-in-target-region-for-azure-vms"></a>Как емкость уверен в целевом регионе для виртуальных машин Azure?
Команда Site Recovery работает с группой управления мощности Azure для планирования инфраструктуры емкостью, достаточной, позволяет обеспечить для аварийного восстановления виртуальных машин будет успешно развертываться в целевом регионе после запуска отработки отказа.

### <a name="does-site-recovery-work-with-reserved-instances"></a>Site Recovery работает с зарезервированными экземплярами?
Да, вы можете приобрести [зарезервировать экземпляры](https://azure.microsoft.com/pricing/reserved-vm-instances/) в регионе аварийного восстановления и Site Recovery операции отработки отказа будет использовать их. </br> Никаких дополнительных настроек не требуется.
...
Нет, Site Recovery не перехватывает реплицируемые данные и не получает сведения о компонентах, запущенных на виртуальных машинах. В службу Site Recovery передаются только метаданные, необходимые для управления репликацией и отработкой отказа.  
...
Да, оба шифрование передаваемых и [шифрования при хранении в Azure](https://docs.microsoft.com/azure/storage/storage-service-encryption) поддерживаются.
