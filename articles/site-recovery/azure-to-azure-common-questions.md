...
### <a name="how-is-capacity-guaranteed-in-target-region-for-azure-vms"></a>Как емкость гарантируется в целевом регионе для виртуальных машин Azure?
Команда Azure Site Recovery (ASR) работает с группой управления мощности Azure для планирования инфраструктуры емкостью, достаточной, с целью убедиться, что виртуальные машины защищены ASR для аварийного восстановления будет успешно развернута в регионе аварийного восстановления (DR), Каждый раз, когда инициируется ASR операции отработки отказа.
...
Согласованные с приложением точки восстановления создаются из согласованных с приложением моментальных снимков. Согласованные с приложением точки восстановления записывают те же данные, что и отказоустойчивые моментальные снимки, а также все данные в памяти и все выполняемые транзакции. Из-за дополнительного содержимого моментальные снимки, согласованные с приложением, создаются дольше других. Согласованные с приложениями точки восстановления рекомендуется использовать для операционных систем баз данных и таких приложений, как SQL Server.
...
Полное описание того, как Site Recovery обрабатывает адреса, см. в разделе [Настройка IP-адресации для целевых виртуальных машин](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-network-mapping#set-up-ip-addressing-for-target-vms).
