...
1. Откройте файл C:\ProgramData\ASR\home\svsystems\var\configurator_register_host_static_info.log. (Папка ProgramData может быть скрытой. Если вы не видите папку ProgramData в файле Explorer на вкладке **«Вид»** в разделе **«Шоу/скрыть»,** выберите флажок **скрытых элементов.)** Сбои могут быть вызваны несколькими проблемами.
...
5. Поиск **запроса строки столба: (7) - Не удалось подключиться к серверу**. Если вы ее нашли, сделайте следующее.
...
6. Если **запрос строки столба: (7) - Не удалось подключиться к серверу** не найдено, в том же файле журнала, ищите запрос строки: **(60) - Сертификат пера не может быть проверен с данными сертификатами CA.** Эта ошибка может возникнуть из-за истечения срока действия сертификата сервера конфигурации или не поддержки TLS 1.0 или более поздних протоколов. Это также может произойти, если брандмауэр блокирует связь TLS между исходной машиной и сервером конфигурации. Если вы ее нашли, сделайте следующее. 
...
7. В среде Linux, если значение платформы в <INSTALLATION_DIR\>/etc/drscout.conf повреждено, регистрация завершается сбоем. Чтобы определить эту проблему, откройте файл /var/log/ua_install.log. Выполните поиск строки **Aborting configuration as VM_PLATFORM value is either null or it is not VmWare/Azure** (Прерывание конфигурации, так как значением VM_PLATFORM является NULL, не VmWare или не Azure). Платформа должна иметь значение **VmWare** или **Azure**. Если поврежден файл drscout.conf, мы рекомендуем [удалить агент мобильности](vmware-physical-manage-mobility-service.md#uninstall-mobility-service), а затем установить его заново. Если неустановка не удается, выполните следующие шаги: a. Откройте файл Installation_Directory/uninstall.sh и закомментируйте вызов функции **StopServices**.
    ...
    d. [Удалите](vmware-physical-manage-mobility-service.md#uninstall-mobility-service) средство передвижения. После успешного удаления перезагрузите систему и попробуйте установить агент мобильности.
...
## <a name="acs50008-saml-token-is-invalid"></a>ACS50008: токен SAML недействителен
...
## <a name="failure-to-activate-windows-license-from-server-standard-evaluation-to-server-standard"></a>Неспособность активировать лицензию Windows от сервера Standard EVALUATION к стандарту сервера

1. В рамках развертывания сервера Configuration через OVF используется лицензия оценки, которая действительна в течение 180 дней. Необходимо активировать эту лицензию до истечения ее срока действия. В противном конце же время это может привести к частому закрытию сервера конфигурации и, таким образом, помехе для действий репликации.
...
4. Чтобы удалить сведения о зарегистрированных узлах и информацию об устаревших записях с сервера конфигурации, выполните следующую команду, используя исходный компьютер и IP-адрес устаревшей записи. 
    ...
    Если у вас есть запись исходного сервера "OnPrem-VM01" с ip-адресом 10.0.0.4, то используйте следующую команду вместо этого.
...
- Убедитесь, что в AAD параметр **User can create application** (Пользователь может создавать приложение) имеет значение *true*. Для получения дополнительной информации [см. Как: Используйте портал для создания приложения Azure AD и основного сервиса, который может получить доступ к ресурсам.](../active-directory/develop/howto-create-service-principal-portal.md#required-permissions)
...
TCP \<Заменить IP с CS IP здесь \<>:52739 Заменить IP с CS IP здесь>:443 SYN_SENT 
