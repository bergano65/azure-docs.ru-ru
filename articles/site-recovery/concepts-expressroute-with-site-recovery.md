---
title: Об использовании ExpressRoute с восстановлением сайта Azure
...
Схема ExpressRoute имеет несколько доменов маршрутики, связанных с ним. Дополнительные сведения о доменах маршрутизации ExpressRoute и их сравнение см. [здесь](../expressroute/expressroute-circuit-peerings.md#peeringcompare).
...
Восстановление сайта воспроизводит данные в учетную запись хранения Azure или репликирует управляемый диск в целевом регионе Azure в общедоступной конечной точке. Для использования ExpressRoute для трафика репликации восстановления сайта можно использовать [пиринг Microsoft](../expressroute/expressroute-circuit-peerings.md#microsoftpeering) или существующую [публичную пиринг](../expressroute/about-public-peering.md) (унипраженные для новых творений). Пиринг Майкрософт является рекомендуемым доменом маршрутизации для репликации. Обратите внимание, что репликация не поддерживается по сравнению с приватным пирингом.

Убедитесь, что [требования к сети](vmware-azure-configuration-server-requirements.md#network-requirements) для configuration Server также выполнены. Подключение к определенным URL-адресам требуется сервером конфигурации для оркестровки репликации восстановления сайта. ExpressRoute не может быть использован для этого подключения. 

В случае, если вы используете прокси в предварительном помещении и хотите использовать ExpressRoute для репликации трафика, вам необходимо настроить список обхода прокси на сервере конфигурации и серверах процессов. Выполните следующие действия:

- Скачать инструмент PsExec [отсюда,](https://aka.ms/PsExec) чтобы получить доступ к контексту пользователя системы.
- Открыть Internet Explorer в контексте системного пользователя, запустив следующую командную строку psexec-s-i "%programfiles% -Internet Explorer-iexplore.exe"
- Добавление параметров прокси в IE
- В список обходов добавьте URL-адрес хранения Azure blob.core.windows.net

Это гарантирует, что только репликация трафика течет через ExpressRoute в то время как связь может пройти через прокси.
