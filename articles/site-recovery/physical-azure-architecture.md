...
**Реплицированные серверы** | Служба Mobility Service устанавливается на каждом реплицируемом сервере. | Рекомендуем разрешить автоматическую установку с сервера обработки. Можно также установить службу вручную или использовать метод автоматического развертывания, например Configuration Manager.
...
4. Трафик реплицируется в общедоступные конечные точки службы хранилища Azure через Интернет. Кроме того, можно использовать [общедоступный пиринг](../expressroute/about-public-peering.md) Azure ExpressRoute. Репликация трафика через VPN типа "сеть — сеть" с локального сайта в Azure не поддерживается.
