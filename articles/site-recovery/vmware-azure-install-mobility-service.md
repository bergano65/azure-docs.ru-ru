---
title: Подготовьте исходные машины для установки Службы мобильности с помощью установки push для аварийного восстановления VMw и физических серверов в Azure Документы Майкрософт
description: Узнайте, как подготовить сервер к установке мобильного агента с помощью установки push для аварийного восстановления VMw и физических серверов в Azure с помощью службы восстановления сайта Azure.
...
---
# <a name="prepare-source-machine-for-push-installation-of-mobility-agent"></a>Подготовка исходной машины для установки подвижного агента
...
5. Запустите файл **cspsconfigtool.exe**. Он доступен в качестве ярлыка на рабочем столе и в "ProgramData%-ASR-home-svsystems-bin папку.
...
2. Создайте учетную запись, с помощью которой сервер обработки сможет получить доступ к компьютеру. Учетная запись должна быть **корневым** пользователем на исходном сервере Linux. Используйте эту учетную запись только для принудительной установки и обновлений.
...
4. Включите подсистему SFTP и проверку пароля в файле sshd_config. Для этого вопием в качестве **корня.**
5. В **/etc/ssh/sshd_config** файл, найдите строку, которая начинается с **PasswordAuthentication**.
6. Некомментируйте строку, и изменить значение **да**.
7. Найти строку, которая начинается с **подсистемы**, и не комментируйте линию.
...
После установки службы Mobility Service на портале Azure выберите **+ Репликация**, чтобы приступить к защите виртуальных машин. Подробнее о возможности репликации [для VMware VMs](vmware-azure-enable-replication.md) и [физических серверов.](physical-azure-disaster-recovery.md#enable-replication)
