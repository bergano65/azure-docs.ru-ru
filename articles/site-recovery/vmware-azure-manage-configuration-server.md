---
title: Управление сервером конфигурации для восстановления аварийности с помощью восстановления сайта Azure
...
ms.locfileid: "80672743"
---
# <a name="manage-the-configuration-server-for-vmware-vmphysical-server-disaster-recovery"></a>Управление сервером конфигурации для vMware VM/физического восстановления сервера аварийного управления
...
Вместе с шаблоном OVF предоставляется пробная лицензия сроком на 180 дней. Для бесперебойной работы необходимо активировать Windows с использованием приобретенной лицензии. Обновление лицензии может быть сделано либо с помощью автономного ключа, либо с помощью стандартного ключа KMS. Руководство доступно в [командной строке DISM Windows для запуска ОС.](https://docs.microsoft.com/windows-hardware/manufacture/desktop/dism-windows-edition-servicing-command-line-options) Чтобы получить ключи, обратитесь к [клиенту KMS, настроенному.](https://docs.microsoft.com/windows-server/get-started/kmsclientkeys)
...
* Восвай в VM, на котором он развернут, и запустите **менеджер конфигурации восстановления сайта Azure** с рабочего стола.
* Кроме того, вы можете получить доступ к серверу конфигурации удаленно с https://*ConfigurationServerName*/:44315/. Для входа необходимы учетные данные администратора.
...
1. Войти в конфигурацию сервера и запустить CSPSConfigtool.exe
...
1. После [входиный](#access-configuration-server), выберите **Управление виртуальными учетных данных машины**
2. Выберите учетную запись, которая вы хотите изменить, и нажмите **"Изменить"**
...
1. После [вхува](#access-configuration-server) на сервер конфигурации выберите **Управление подключением.**
...
- Вы можете [добавить дополнительный адаптер для VM,](vmware-azure-deploy-configuration-server.md#add-an-additional-adapter)но вы должны добавить его, прежде чем зарегистрировать сервер конфигурации в хранилище.
- Если необходимо добавить адаптер после того, как сервер конфигурации был зарегистрирован в хранилище, необходимо добавить адаптер в свойствах виртуальной машины. Затем вам нужно [перерегистрировать](#reregister-a-configuration-server-in-the-same-vault) сервер в хранилище.

## <a name="how-to-renew-ssl-certificates"></a>Как обновить SSL-сертификаты

Сервер конфигурации имеет встроенный веб-сервер, который организует деятельность агентов Mobility на всех защищенных машинах, встроенных/масштабных серверах процесса и подключенных к нему целевых серверах. Веб-сервер использует SSL-сертификат для аутентификации клиентов. Срок действия этого сертификата составляет три года, его можно обновить в любое время.
...
1. Для обновления в хранилище откройте > **сервер конфигурации** **инфраструктуры восстановления сайта.** Выберите нужный сервер конфигурации.
2. Убедитесь, что все серверы процесса масштабирования компонентов, мастер-серверы и мобильные агенты на всех защищенных машинах находятся на последних версиях и находятся в подключенном состоянии.
3. Теперь выберите **Сертификаты продления.**
4. Тщательно следуйте инструкциям на этой странице и нажмите хорошо, чтобы обновить сертификаты на выбранной конфигурации сервера и его связанных компонентов.

### <a name="if-certificates-have-already-expired"></a>Если срок действия сертификатов уже истек

1. Почтовый срок действия сертификатов **не может быть продлен с портала Azure.** Прежде чем приступить к работе, убедитесь, что все серверы процесса масштабирования компонентов, мастер-серверы и мобильные агенты на всех защищенных машинах находятся на последних версиях и находятся в подключенном состоянии.
2. **Соблюдайте эту процедуру только в том случае, если срок действия сертификатов уже истек.** Войти в конфигурацию сервера, перейти на C диск > программы данных > восстановление сайта > дома > svsystems > бен и выполнить "RenewCerts" исполнитель инструмент в качестве администратора.
3. Окно выполнения PowerShell всплывает и запускает обновление сертификатов. Это может занять до 15 минут. Не закрывайте окно до завершения обновления.
...
1. В хранилище, открыть **Управление** > **сайта Восстановление инфраструктуры** > **Настройка Серверы**.
...
6. При необходимости укажите данные прокси-сервера. Затем выберите **Регистрация**.
7. Откройте командную строку PowerShell с правами администратора и выполните следующую команду:
   ...
   ```
    ...
    >[!NOTE]
    >Для того, чтобы **вытащить последние сертификаты** с сервера конфигурации на сервер масштабирования, выполните команду *"\<Установка Drive (Восстановление сайта Microsoft Azure) - агент-cdpcli.exe>"--registermt*
...
1. В хранилище перейдите на **управление** >  > **серверами конфигурации инфраструктуры****восстановления сайта.**
...
4. В хранилище, открытые > **серверы конфигурации инфраструктуры** **восстановления сайта.**
...
> [!NOTE]
> Для принудительного удаления сервера конфигурации можно использовать опцию **-Force** в Remove-AzSiteRecoveryFabric.
...
1. На портале Azure перейдите на восстановление **служб Vault** > **Управление** > **инфраструктурой** > восстановления сайта**для VMware & физических серверов** > **конфигурации** машин
2. Щелкните сервер конфигурации, который нужно обновить.
3. На лезвии с деталями выбранного сервера конфигурации щелкните **Подробнее** > **Обновить сервер**.
4. Следите за ходом выполнения работы в рамках работ по**мониторингу** > сайта Recovery **Services Vault.** > **Site Recovery jobs**
