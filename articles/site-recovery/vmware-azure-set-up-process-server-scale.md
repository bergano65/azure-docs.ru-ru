...
9.24 версии рекомендации добавляется во время выбора сервера обработки для репликации на новый. Сервер обработки, будет отмечен Исправен "," Предупреждение "и" сигнализируя, на основе определенных критериев. Сведения о различных сценариях, которые могут повлиять на состояние сервера обработки, посетите [процессам выбора сервера](vmware-azure-manage-process-server.md#process-server-selection-guidance).
