---
title: Настройка сервера масштабирования во время аварийного восстановления VMw и физических серверов с помощью Azure Site Recovery Документы Microsoft Docs
description: В этой статье описывается, как настроить сервер масштабирования во время аварийного восстановления VMwи и физических серверов.
...
С версии 9.24 руководство добавляется при выборе сервера процессов для новых репликаций. Процесс сервера будет отмечен здоровый, предупреждение и критические на основе определенных критериев. Чтобы понять различные сценарии, которые могут влиять на состояние сервера процесса, просмотрите [оповещения сервера процесса.](vmware-physical-azure-monitor-process-server.md#process-server-alerts)
...
1. Вопийте на портале Azure и просматривайте хранилище служб восстановления.
2. Открытая **инфраструктура** > восстановления сайта**VMWare и физические машины** > **Конфигурация серверов** (под для VMware & физические машины).
3. Выберите сервер конфигурации, чтобы просмотреть подробные сведения о нем. Затем щелкните **+ Сервер обработки**.
4. В **Add Process server** >  **Выберите, где вы хотите развернуть сервер процесса,** выберите **развертывание сервера масштабирования процесса на месте.**
