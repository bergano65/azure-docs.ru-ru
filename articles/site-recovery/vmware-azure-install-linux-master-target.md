...
Оставить комментарий или задать вопрос можно с помощью формы в конце этой статьи или на [странице вопросов и ответов о Службах восстановления Azure на сайте Майкрософт](https://docs.microsoft.com/answers/topics/azure-site-recovery.html).
...
3. Отформатируйте диск и создайте файловую систему на новом диске: **mkfs.ext4 /dev/mapper/\<ИД Multipath диска хранения>** .
...
5. Создайте запись **fstab**, чтобы диск хранения автоматически подключался при каждом запуске системы.
    ...
    **/dev/mapper/\<ИД Multipath диска хранения> /mnt/retention ext4 rw 0 0**
