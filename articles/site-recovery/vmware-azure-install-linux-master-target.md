...
1.   Перейдите к [ссылка для скачивания](http://old-releases.ubuntu.com/releases/16.04.2/ubuntu-16.04.2-server-amd64.iso), выберите ближайшее зеркало и скачайте ISO-ОБРАЗА минимальной 64-разрядная Ubuntu 16.04.2.
...
1. Выберите **английского языка (США)** качестве страны происхождения клавиатуры, а затем выберите **ввод**.
...
5. Создайте запись **fstab**, чтобы диск хранения автоматически подключался при каждом запуске системы.
    ...
    **/dev/mapper/\<идентификатор multipath диска хранения >/mnt/хранения ext4 rw 0 0**
