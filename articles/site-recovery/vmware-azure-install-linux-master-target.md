---
title: Установка основного целевого сервера для сбоя Linux VM с помощью восстановления сайта Azure
...
1.   Перейдите к [ссылке загрузки,](http://old-releases.ubuntu.com/releases/16.04.2/ubuntu-16.04.2-server-amd64.iso)выберите ближайшее зеркало и скачать Ubuntu 16.04.2 минимальный 64-разрядный ISO.
...
1. Выберите **английский язык (США)** в качестве страны/региона происхождения для клавиатуры, а затем выберите **Enter**.
...
4. В левой панели выберите **Advanced** > **General,** а затем выберите кнопку **Параметры конфигурации** на нижней правой части экрана.
...
[Скачать последние Linux мастер целевых установок битов](https://aka.ms/latestlinuxmobsvc).
...
2. Копируйте необходимый файл установки с сервера процесса и сохраните его как **latestlinuxmobsvc.tar.gz** в вашем домашнем каталоге.


### <a name="apply-custom-configuration-changes"></a>Применение изменений настраиваемой конфигурации

Чтобы применить пользовательские изменения конфигурации, используйте следующие шаги в качестве пользователя ROOT:
...
3. Формат диска, а затем создать файловую систему на новом диске: **mkfs.ext4 /dev/mapper /\<Удержание диска многопотрообразной>. **
...
5. Создайте запись **fstab**, чтобы диск хранения автоматически подключался при каждом запуске системы.
    ...
    **/dev/mapper/\<Сохранение дисков многопоточный ид> /mnt/retention ext4 rw 0 0 0**
...
> [!NOTE]
> Перед установкой основного целевого сервера убедитесь, что файл **/etc/hosts** на виртуальной машине содержит записи, которые отображают локальное имя хоста на IP-адреса, которые связаны со всеми сетевыми адаптерами.
...
Запустите установщик. Он автоматически обнаружит, что на главном целевом сервере установлен агент. Для обновления выберите **Y**.  После завершения настройки проверьте версию основной цели, установленную с помощью следующей команды:
...
Теперь вы можете приступить к [повторной защите,](vmware-azure-reprotect.md)а затем failback.
