...
1.   Перейдите по [ссылке для скачивания](http://old-releases.ubuntu.com/releases/16.04.2/ubuntu-16.04.2-server-amd64.iso), выберите ближайшее зеркало и скачайте минимально 64-разрядный ISO Ubuntu 16.04.2.
...
1. Выберите **Английский (США)** в качестве страны или региона происхождения клавиатуры, а затем нажмите клавишу **Ввод**.
...
4. В левой области выберите **Advanced** > **General**(дополнительные общие), а затем нажмите кнопку **Параметры конфигурации** в правой нижней части экрана.
...
2. Скопируйте необходимый файл установщика с сервера обработки и сохраните его как **latestlinuxmobsvc. tar. gz** в домашнем каталоге.


### <a name="apply-custom-configuration-changes"></a>Применение изменений настраиваемой конфигурации

Чтобы применить пользовательские изменения конфигурации, выполните следующие действия в качестве ПРИВИЛЕГИРОВАНного пользователя:
...
3. Отформатируйте диск и создайте файловую систему на новом диске: **mkfs. ext4/Дев/маппер/\<идентификатор многопутевого диска для хранения данных>**.
...
5. Создайте запись **fstab**, чтобы диск хранения автоматически подключался при каждом запуске системы.
    ...
    **/Дев/маппер/\<retention Disks multipath ID>/МНТ/ретентион ext4 RW 0 0**
...
Запустите установщик. Он автоматически обнаружит, что на главном целевом сервере установлен агент. Чтобы выполнить обновление, выберите **Y**.  После завершения установки проверьте установленную версию главного целевого сервера, выполнив следующую команду:
...
Теперь можно продолжить повторное применение [защиты](vmware-azure-reprotect.md)с последующим восстановлением размещения.
