...
Параметр **VM Compatibility** (Совместимость виртуальной машины) имеет два значения: **Да** и **Да\***. **Да** \* — для экземпляров, в которых виртуальная машина подходит для [твердотельных накопителей](../virtual-machines/windows/disks-types.md)уровня "Премиум". За счет высокой активности обработки данных и обработки большого числа операций ввода-вывода в секунду на диске она подходит для категории P20 или P30, но из-за размера диска она сопоставляется с категорией P10 или P20. Служба хранилища Azure определяет, с каким типом диска хранилища класса Premium сопоставить диск виртуальной машины на основе его размера. Пример:
...
**Целевой объект хранилища репликации** | **Средний размер ввода-вывода исходного диска** |**Средняя скорость обработки данных исходного диска** | **Общее число обработанных данных с диска источника в день**
---|---|---|---
