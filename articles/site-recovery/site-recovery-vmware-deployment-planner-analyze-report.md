---
title: Проанализируйте отчет планировщика развертывания для восстановления аварийности VMware с помощью восстановления сайта Azure
description: В этой статье описывается, как анализировать отчет, генерируемый планировщиком развертывания восстановления для восстановления аварийности VMware в Azure, с помощью восстановления сайта Azure.
...
ms.locfileid: "79257515"
---
# <a name="analyze-the-deployment-planner-report-for-vmware-disaster-recovery-to-azure"></a>Проанализируйте отчет планировщика развертывания для восстановления аварийности VMware в Azure
...
**Desired RPO** (Требуемое значение RPO) — значение целевой точки восстановления для развертывания. По умолчанию необходимая пропускная способность рассчитывается для значений RPO, равных 15, 30 и 60 минутам. В зависимости от выбора в отчете обновляются затронутые значения. Если при генерации отчета вы использовали параметр *DesiredRPOinMin,* это значение отображается в результате Desired RPO.
...
На следующей диаграмме показано общее число учетных записей хранения Azure (уровня Standard и Premium), необходимых для защиты всех совместимых виртуальных машин. Чтобы узнать, какую учетную запись хранения необходимо использовать для каждой виртуальной машины, см. раздел "Размещение хранилища виртуальной машины". Если вы используете v2.5 планировщика развертывания, эта рекомендация показывает только количество стандартных учетных записей хранения кэша, которые необходимы для репликации, так как данные непосредственно записываются на управляемые диски.
...
>[!Note]
>Планировщик развертывания v2.5 рекомендует размещение для машин, которые будут реплицироваться непосредственно на управляемые диски.
...
**Тип хранения репликации:** стандартный или премиальный управляемый диск, который используется для репликации всех соответствующих ВМ, упомянутых в столбце **VMs to Place.**

**Тип учетной записи журнала**: Все журналы репликации хранятся в стандартной учетной записи хранилища.

**Предлагаемая префикс для учетной записи хранения:** предлагаемая префикс трех символов, которая может быть использована для именования учетной записи хранения кэша. Вы можете использовать собственный префикс, но для программы мы рекомендуем использовать префикс, соответствующий требованиям [соглашения об именовании секций учетных записей хранения](https://aka.ms/storage-performance-checklist).

**Рекомендуемое имя учетной записи журнала**: Имя учетной записи хранилища после включения предложенной приставки. Замените имя в угловых скобках (< и >) именем своей учетной записи.

**Резюме размещения**: Резюме дисков, необходимых для защиты VMs по типу хранилища. Она включает в себя общее количество VMs, общий размер на всех дисках и общее количество дисков.
...
Параметр **VM Compatibility** (Совместимость виртуальной машины) имеет два значения: **Да** и **Да\***. **Да,** \* для случаев, в которых VM подходит для [премиум SSDs](../virtual-machines/windows/disks-types.md). За счет высокой активности обработки данных и обработки большого числа операций ввода-вывода в секунду на диске она подходит для категории P20 или P30, но из-за размера диска она сопоставляется с категорией P10 или P20. Служба хранилища Azure определяет, с каким типом диска хранилища класса Premium сопоставить диск виртуальной машины на основе его размера. Пример:
...
**Asrseeddisk (Управляемый диск), созданный для репликации:** Имя диска, который создается при включании репликации. Он хранит данные и снимки в Azure.
...
* Неправильный размер диска данных или неправильный размер диска ОС. [Просмотрите](vmware-physical-azure-support-matrix.md#azure-vm-requirements) лимиты поддержки. 
...
* Средний отток данных превышает поддерживаемый предел оттока данных сайта в 20 МБ/с для среднего размера ввоза/о для диска.
...
**Тип OS**: Тип OS VM. Это может быть Windows, Linux или другая ОС в зависимости от выбранного шаблона в VMware vSphere при создании виртуальной машины.
...
**Цель хранения репликации** | **Средний размер ввода-вывода исходного диска** |**Средняя скорость обработки данных исходного диска** | **Общий отток исходных дисков в день**
---|---|---|---
