---
title: О планах восстановления в восстановлении сайта Azure
...
ms.locfileid: "79257775"
---
# <a name="about-recovery-plans"></a>Сведения о планах восстановления

В этой статье приводится обзор планов восстановления в [Azure Site Recovery](site-recovery-overview.md).

План восстановления собирает машины в группы восстановления с целью неудачи. План восстановления помогает определить систематический процесс восстановления путем создания небольших независимых модулей, для которых можно выполнять отработку отказа. Как правило, модуль представляет приложение в вашей среде.
...
- Планы восстановления используются для сбоя в Azure, но не могут быть использованы для сбоя из Azure.
- До 100 защищенных экземпляров могут быть добавлены в один план восстановления.
...
- Машины могут быть отожещены в нескольких планах восстановления, в которых последующие планы пропускают развертывание/запуск машины, если она была ранее развернута с помощью другого плана восстановления.
...
* Автоматизация задач восстановления для уменьшения цели времени восстановления (RTO).
* Проверка готовности к миграции или аварийному восстановлению путем добавления приложений в план восстановления.
* Запуск тестов на планы восстановления, чтобы обеспечить аварийное восстановление или миграция работает, как ожидалось.
...
- Если порядок задан, используется последовательная обработка. Действия работают параллельно по мере необходимости, чтобы улучшить восстановление приложения RTO.
...
### <a name="run-a-test-failover-on-recovery-plans"></a>Выполнить тест неудачу на планы восстановления
...
- Если вы обнаружите, что что-то пропустили, запустите очистку, а затем повторно запустите тест неудачу. 
...
- Приложения и их зависимости часто изменяются. Чтобы убедиться, что планы восстановления актуальны, выполняй тест-неудачу для каждого приложения каждый квартал.

    ![Снимок экрана примера тестового плана восстановления в Site Recovery](./media/recovery-plan-overview/rptest.png)

## <a name="watch-a-recovery-plan-video"></a>Смотреть видео плана восстановления

Смотреть быстрый пример видео, показывающее на кнопку failover для плана восстановления для двухуровневого WordPress приложение.
...
- [Запуск](site-recovery-failover.md) неудач. 
