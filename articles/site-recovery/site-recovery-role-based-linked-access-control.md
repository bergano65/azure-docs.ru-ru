---
title: Управление управлением элементом управления доступом на основе Azure в восстановлении сайта Azure
...
Контроль доступа на основе ролей Azure (RBAC) обеспечивает мелкозернистое управление доступом для Azure. Используя RBAC, можно разделить обязанности в команде и предоставлять пользователям только разрешения для выполнения определенных заданий.
...
> [!NOTE]
> Если вы позволяете репликацию для Azure VM и хотите разрешить восстановление сайта для управления обновлениями, то при включении репликации вы также можете создать новую учетную запись Automation, и в этом случае вам потребуется разрешение на создание учетной записи автоматизации в том же подписка, как хранилище, а также.
