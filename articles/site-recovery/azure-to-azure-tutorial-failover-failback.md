...
В этом руководстве описано, как выполнить отработку отказа виртуальной машины Azure в дополнительный регион Azure с помощью службы [Azure Site Recovery](site-recovery-overview.md). После выполнения отработки отказа нужно повторно включить защиту виртуальной машины. В этом руководстве описано следующее:
...
1. В разделе **Реплицированные элементы** выберите виртуальную машину, для которой требуется отработка отказа, а затем выберите **Отработка отказа**
   ...
   ![Снимок экрана с параметрами отработки отказа для виртуальной машины.](./media/azure-to-azure-tutorial-failover-failback/failover.png)
...
![Снимок экрана с отработкой отказа с добавленным диском.](./media/azure-to-azure-tutorial-failover-failback/failover-added.png)
...
2. В разделе **Хранилище** > **Реплицированные элементы** щелкните виртуальную машину, для которой проводилась отработка отказа, щелкните правой кнопкой мыши и выберите **Защитить повторно**.
   ...
   ![Снимок экрана с параметром повторной защиты для виртуальной машины.](./media/azure-to-azure-tutorial-failover-failback/reprotect.png)
