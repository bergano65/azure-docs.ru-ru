...
Требуется подписка Azure, хранилище служб восстановления, учетную запись хранения кэша, управляемые диски и виртуальной сети. Хранилище, учетная запись хранения кэша, управляемые диски и сеть должна располагаться в том же регионе.
...
### <a name="how-are-licensing-charges-handled-during-replication-after-failover"></a>Как обрабатываются лицензионные расходы во время репликации после отработки отказа?

Дополнительные сведения о лицензировании см. в часто задаваемых вопросах [здесь](https://aka.ms/asr_pricing_FAQ).
...
Вы можете использовать [Калькулятор цен](https://aka.ms/asr_pricing_calculator) для оценки затрат при использовании Azure Site Recovery. Подробные оценки затрат на запуск планировщика развертывания (https://aka.ms/siterecovery_deployment_planner) и анализировать [отчет об оценке затрат](https://aka.ms/asr_DP_costreport).

### <a name="is-there-any-difference-in-cost-when-i-replicate-directly-to-managed-disk"></a>Есть ли разница в стоимости при репликации непосредственно в управляемый диск?

Управляемые диски взимается немного отличается от учетных записей хранения. См. пример ниже для исходный диск размером 100 ГБ. Пример относится только к разностные расходы на хранение. Эти затраты не включают стоимость для моментальных снимков, хранилище кэша и транзакций.
...
Дополнительные сведения о [более подробно с ценообразованием для управляемых дисков](https://azure.microsoft.com/pricing/details/managed-disks/).

### <a name="do-i-incur-additional-charges-for-cache-storage-account-with-managed-disks"></a>Я понесу дополнительную плату для учетной записи хранения кэша с управляемыми дисками?

Нет, вы не взимается дополнительная плата за кэша. Кэш всегда является частью VMware, в архитектуру Azure. При репликации в стандартную учетную запись хранения, это хранилище кэша является частью одной целевой учетной записи хранения.
...
Да, период использования Azure Site Recovery значения не имеет. В течение первого 31-дневного периода использования Azure Site Recovery плата за каждый защищенный экземпляр не взимается. Например, если в течение последних 6 месяцев вы пользовались защитой 10 экземпляров и подключаете 11-й экземпляр к Azure Site Recovery, за использование Azure Site Recovery в течение начального периода (31 день) для этого экземпляра плата не взимается. За первые 10 экземпляров плата за Azure Site Recovery взимается, так как 31-дневный период их защиты истек.
...
Дополнительные сведения см. на станице [Цены на Восстановление сайтов Azure](https://aka.ms/asr_pricing_FAQ).

### <a name="is-there-a-cost-associated-to-perform-dr-drillstest-failover"></a>Взимается ли плата за выполнение аварийного восстановления или тестовой отработки отказа?

Отдельные затраты для аварийного восстановления отсутствуют. После создания виртуальной машины после тестовой отработки отказа будет взиматься плата за вычислительные ресурсы.
...
Данные реплицируются в хранилище Azure. При выполнении отработки отказа Site Recovery автоматически создает виртуальные машины Azure из учетной записи хранения или управляемый диск, в зависимости от конфигурации.
...
### <a name="can-i-protect-a-virtual-machine-that-has-docker-disk-configuration"></a>Можно ли защитить виртуальную машину с конфигурацией диска Docker?
...
Да, ExpressRoute можно использовать для репликации виртуальных машин в Azure. Site Recovery реплицирует данные в службу хранилища Azure через общедоступную конечную точку. Чтобы использовать ExpressRoute для репликации Site Recovery, необходимо настроить [общедоступный пиринг](../expressroute/expressroute-circuit-peerings.md#publicpeering) или [пиринг Майкрософт](../expressroute/expressroute-circuit-peerings.md#microsoftpeering). Пиринг Майкрософт является рекомендуемым доменом маршрутизации для репликации. Убедитесь, что [требования к сети](vmware-azure-configuration-server-requirements.md#network-requirements) также выполняются для репликации. После отработки отказа виртуальных машин в виртуальную сеть Azure к ним можно получить доступ с помощью [частного пиринга](../expressroute/expressroute-circuit-peerings.md#privatepeering).

### <a name="how-can-i-change-storage-account-after-machine-is-protected"></a>Как изменить учетную запись хранения после включения защиты машины?

Необходимо отключить и включить репликацию для обновления или перехода на предыдущую тип учетной записи хранения.

### <a name="can-i-replicate-to-storage-accounts-for-new-machine"></a>Можно ли реплицировать в учетные записи хранения для новой машины?

Нет, начиная с марта "19, можно реплицировать на управляемые диски в Azure с портала. Репликация в учетные записи хранения для новой машины доступна только через REST API и Powershell. Используйте API версии 2016-08-10 "или" 2018-01-10 для репликации в учетные записи хранения.
...
См. в статье о том, как [Azure Site Recovery упрощает аварийное восстановление с управляемыми дисками](https://azure.microsoft.com/blog/simplify-disaster-recovery-with-managed-disks-for-vmware-and-physical-servers/).

### <a name="how-can-i-change-managed-disk-type-after-machine-is-protected"></a>Как изменить тип управляемого диска защищенном компьютере?

Да, вы можете легко изменить тип управляемого диска. [Узнайте больше](https://docs.microsoft.com/azure/virtual-machines/windows/convert-disk-storage). Однако после изменить тип управляемого диска, следует убедиться, отложить точек восстановления, новые создаваться, если вам нужно выполните тестовую отработку отказа или отработки отказа после этого действия.

### <a name="can-i-switch-the-replication-from-managed-disks-to-unmanaged-disks"></a>Можно ли перейти с репликация с управляемыми дисками на неуправляемые диски?
...
При выполнении репликации в Azure трафик репликации достигает общедоступных конечных точек в хранилище Azure, таким образом репликацию можно выполнять только через общедоступный Интернет с помощью ExpressRoute (общедоступный пиринг) и VPN не будет работать.

### <a name="can-i-use-riverbed-steelheads-for-replication"></a>Можно ли использовать Riverbed SteelHeads для репликации?

Наш партнер, Riverbed, предоставляет подробные рекомендации по работе с Azure Site Recovery. См. в статье их [руководство по решению](https://community.riverbed.com/s/article/DOC-4627).
...
Да, вы можете сохранить IP-адрес при отработке отказа. Убедитесь, что вы указали целевой IP-адрес в колонке "Вычисления и сеть", перед выполнением отработки отказа. Кроме того, убедитесь, что машины завершили работу во время отработки отказа, чтобы избежать конфликтов протокола IP во время восстановления размещения.
...
Да, можно изменить тип и размер виртуальной Машины любое время перед выполнением отработки отказа, выбрав вычисления и сеть настройки элемента репликации на портале.
...
Настоятельно рекомендуется создать сервер обработки в Azure для цели восстановления размещения, чтобы избежать задержки передачи данных. Кроме того в случае, если разделены исходной сети виртуальных машин с сетью Azure с выходом на сервере конфигурации, затем очень важно для использования сервера обработки, созданные в Azure для восстановления размещения.
