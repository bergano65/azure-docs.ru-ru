...
- Для повторной защиты и восстановления размещения требуется VPN-подключение "сеть —сеть" (S2S) для репликации данных. Предоставьте сеть, через которую виртуальные машины в Azure, перенесенные при отработке отказа, смогут связываться (проверять связь) с локальным сервером конфигурации. Вы также можете развернуть сервер обработки в сети Azure виртуальной машины, для которой выполнена отработка отказа. Кроме того, этот сервер обработки должен иметь возможность взаимодействовать с локальным сервером конфигурации.
...
- Между сервером обработки и защищенной виртуальной машиной должна быть настроена сеть с низкой задержкой. Как правило, когда вы определяете, нужен ли сервер обработки в Azure, следует принять во внимание задержку:
    - Если настроено подключение Azure ExpressRoute, то для отправки данных можно использовать локальный сервер обработки, так как между ним и виртуальной машиной низкая задержка.
