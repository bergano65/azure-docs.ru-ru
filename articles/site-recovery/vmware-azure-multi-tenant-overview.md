---
title: Многотенантное восстановление с акситера VMware VM с восстановлением сайта Azure
...
[Azure Site Recovery](site-recovery-overview.md) поддерживает мультитенантные среды для подписки на арендаторов. Такие среды также поддерживаются для подписок клиентов, созданных и управляемых с помощью программы поставщика облачных решений Майкрософт (CSP).
...
1. Создайте новую роль, клонируя предопределенную роль *только для чтения,* а затем дайте ей удобное название (например, Azure_Site_Recovery, как показано в этом примере).
...
3. Теперь виртуальные машины подготовлены к репликации. Убедитесь, что в**виртуальных машинах** **Replicate** > Select отображаются только виртуальные банкоматы клиента.
