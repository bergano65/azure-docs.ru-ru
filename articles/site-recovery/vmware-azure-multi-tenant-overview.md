---
title: Аварийное восстановление нескольких клиентов виртуальной машины VMware с помощью Azure Site Recovery
...
[Azure Site Recovery](site-recovery-overview.md) поддерживает многоклиентские среды для подписок клиента. Такие среды также поддерживаются для подписок клиентов, созданных и управляемых с помощью программы поставщика облачных решений Майкрософт (CSP).
...
1. Создайте новую роль путем клонирования предопределенной роли *только для чтения* , а затем присвойте ей удобное имя (например, Azure_Site_Recovery, как показано в этом примере).
...
3. Теперь виртуальные машины подготовлены к репликации. Убедитесь, что в поле **реплицировать**  >  **Выбор виртуальных машин** отображаются только виртуальные машины клиента.
...
![Схема, показывающая архитектурное различие в выделенном решении для размещения, заключается в том, что инфраструктура каждого клиента настраивается только для этого клиента.](./media/vmware-azure-multi-tenant-overview/dedicated-hosting-scenario.png)  
