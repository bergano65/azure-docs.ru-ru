...
>[!NOTE]
> Восстановление сайта в настоящее время поддерживает ADE, с и без Azure Active Directory (AAD) для vMs под управлением операционных систем Windows и Linux.  Для машин под управлением ADE 1.1 (без AAD) vMs должны использовать управляемые диски. VMs с неуправляемыми дисками не поддерживаются. При переключении с ADE 0.1 (с AAD) на 1.1 необходимо отключить репликацию и включить репликацию для VM после включения 1.1.
...
4. В **настройках**можно настроить следующие настройки целевого сайта.
    ...
    - **Хранилища ключей шифрования:** По умолчанию восстановление сайта создает новое хранилище ключей в целевом регионе. Название имеет суффикс "asr", основанный на исходных ключах шифрования ключей VM. Если хранилище ключей, созданное восстановлением сайта Azure, уже существует, оно используется повторно.
