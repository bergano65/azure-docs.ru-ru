...
12 виртуальных ЦП (2 сокета по 6 ядер с частотой \@ 2,5 ГГц) | 18 ГБ | 600 ГБ | От 500 ГБ до 1 ТБ | Репликация от 100 до 150 компьютеров
16 виртуальных ЦП (2 сокета по 8 ядер с частотой \@ 2,5 ГГц) | 32 ГБ | 1 TБ | От 1 ТБ до 2 ТБ | Репликация от 150 до 200 компьютеров
...
--- | --- | --- | --- | ---
8 виртуальных ЦП (2 сокета по 4 ядра с частотой \@ 2,5 ГГц), 16 ГБ памяти | 4 виртуальных ЦП (2 сокета по 2 ядра с частотой \@ 2,5 ГГц), 8 ГБ памяти | 300 ГБ | 250 ГБ или менее | Вы можете реплицировать до 85 компьютеров.
8 виртуальных ЦП (2 сокета по 4 ядра с частотой \@ 2,5 ГГц), 16 ГБ памяти | 8 виртуальных ЦП (2 сокета по 4 ядра с частотой \@ 2,5 ГГц), 12 ГБ памяти | 600 ГБ | От 250 ГБ до 1 ТБ | Репликация от 85 до 150 компьютеров
12 виртуальных ЦП (2 сокета по 6 ядер с частотой \@ 2,5 ГГц), 18 ГБ памяти | 12 виртуальных ЦП (2 сокета по 6 ядер с частотой \@ 2,5 ГГц), 24 ГБ памяти | 1 TБ | От 1 ТБ до 2 ТБ | Репликация от 150 до 225 компьютеров
...
1. Откройте оснастку MMC в службе архивации Azure на компьютере, который выступает в качестве сервера обработки. По умолчанию ярлык для резервного копирования расположен на рабочем столе или в следующей папке: C:\Program Files\Microsoft Azure Recover Services Agent\bin.
...
Если вы увеличите развертывание так, что количество компьютеров-источников превысит 200 или объем ежедневно изменяемых данных превысит 2 ТБ, для обработки такой нагрузки потребуются дополнительные серверы обработки. Чтобы настроить сервер обработки, следуйте инструкциям из [этой статьи](vmware-azure-set-up-process-server-scale.md). После настройки сервера можно перевести исходные компьютеры на его использование.
...
Вам потребуется дополнительный главный целевой сервер в следующих случаях

1. Если необходимо защитить виртуальную машину под управлением Linux.
2. Если главный целевой сервер, доступный на сервере конфигурации, не имеет доступа к хранилищу данных виртуальной машины.
3. Если общее число дисков на главном целевом сервере (количество локальных дисков на сервере + количество защищаемых дисков) превышает 60 дисков.

Чтобы добавить новый главный целевой сервер для **виртуальной машины под управлением Linux**, [нажмите здесь](vmware-azure-install-linux-master-target.md).

Для **виртуальной машины под управлением Windows** выполните представленные ниже инструкции.

1. Перейдите в раздел **Хранилище служб восстановления** > **Инфраструктура Site Recovery** > **Серверы конфигурации**.
2. Щелкните нужный сервер конфигурации > **+ Главный целевой сервер**.![add-master-target-server.png](media/site-recovery-plan-capacity-vmware/add-master-target-server.png)
3. Загрузите унифицированную программу установки и запустите ее на виртуальной машине, чтобы настроить главный целевой сервер.
4. Выберите **Установка главного целевого сервера** > **Далее**. ![choose-MT.PNG](media/site-recovery-plan-capacity-vmware/choose-MT.PNG)
5. Выберите расположение установки по умолчанию > щелкните **Установить**. ![MT-installation](media/site-recovery-plan-capacity-vmware/MT-installation.PNG)
6. Щелкните **Перейти к конфигурации** для регистрации главного целевого сервера на сервере конфигурации. ![MT-proceed-configuration.PNG](media/site-recovery-plan-capacity-vmware/MT-proceed-configuration.PNG)
7. Введите IP-адрес сервера конфигурации и парольную фразу. [Щелкните здесь](vmware-azure-manage-configuration-server.md#generate-configuration-server-passphrase), чтобы узнать, как сгенерировать парольную фразу.![cs-ip-passphrase](media/site-recovery-plan-capacity-vmware/cs-ip-passphrase.PNG)
8. Нажмите кнопку **Зарегистрировать** и после регистрации кнопку **Готово**.
9. После успешной регистрации этот сервер будет отображаться на портале в разделе **Хранилище служб восстановления** > **Инфраструктура Site Recovery** > **Серверы конфигурации** > главные целевые серверы соответствующего сервера конфигурации.

 >[!NOTE]
 >Загрузить последнюю версию унифицированной программы установки главного целевого сервера для Windows можно [здесь](https://aka.ms/latestmobsvc).
