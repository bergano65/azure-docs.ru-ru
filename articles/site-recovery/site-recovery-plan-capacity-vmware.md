---
title: Планирование мощности для аварийного восстановления VMware с восстановлением сайта Azure
...
* **Влияние пропускной способности**: Вы можете влиять на пропускную способность, которая используется для репликации, используя несколько ключей реестра:
...
1. Откройте оснастку MMC в службе Azure Backup на компьютере, который выступает в качестве сервера обработки. По умолчанию ярлык для резервного копирования доступен на рабочем столе или в следующей папке: C:'Программные файлы »Microsoft Azure Recovery Services Agent'bin.
...
3. На вкладке **Throttling** выберите **Использование пропускной способности интернета для операций резервного копирования.** Задайте ограничения для рабочего и нерабочего времени. Допустимы значения в диапазоне от 512 Кбит/с до 1,023 Мбит/с.
...
Если вы увеличите развертывание так, что количество исходных компьютеров превысит 200 или объем ежедневно изменяемых данных превысит 2 ТБ, для обработки такой нагрузки потребуется добавить серверы обработки. Мы улучшили продукт в версии 9.24, чтобы обеспечить [оповещения сервера процесса](vmware-physical-azure-monitor-process-server.md#process-server-alerts) о том, когда настроить сервер процесса масштабирования. [Настройка сервера процесса](vmware-azure-set-up-process-server-scale.md) для защиты новых исходных машин или [баланса нагрузки.](vmware-azure-manage-process-server.md#move-vms-to-balance-the-process-server-load)

### <a name="migrate-machines-to-use-the-new-process-server"></a>Перенос компьютеров для использования нового сервера обработки

1. Выберите**серверы восстановления** **настроек.** >  Выберите сервер конфигурации и разверните список **Серверы обработки**.
...
1. Перейдите на > **серверы конфигурации инфраструктуры** **восстановления безопасности служб** > **восстановления.**
...
4. Выберите **Установить мастер-мишень** > **Далее.**
...
Когда регистрация завершается успешно, сервер зачисляется на портале Azure на**серверах конфигурации** > **инфраструктуры восстановления инфраструктуры** > Recovery **Services Recovery,** в главных целевых серверах сервера конфигурации.
