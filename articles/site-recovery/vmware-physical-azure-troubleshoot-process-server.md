...
--- | --- | --- 
![Работоспособно][green] | Нет  | Сервер обработки подключен и является работоспособным.
...
![Critical][red] |  Свободное место < 25% за последние 15 минут | 1. Следуйте инструкциям, связанным с предупреждением для этой проблемы.<br/> 2.3. Следуйте приведенным ниже инструкциям, чтобы устранить неполадки [подключения и репликации](#check-connectivity-and-replication).<br/> 3. Если проблема будет повторяться, запустите [планировщик развертывания](https://aka.ms/asr-v2a-deployment-planner) для репликации VMware или физического сервера.
