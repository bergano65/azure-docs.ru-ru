---
title: Управление сервером конфигурации для физических серверов в восстановлении сайта Azure
...
2. Просмотрите > **серверы конфигурации** **инфраструктуры восстановления сайта**(под для VMware & физических машин).
...
2. **Прежде чем начать,** выберите **Установить сервер конфигурации и процесс сервера.**
...
6. В **Проверке предпосылок,** Настройка выполняет проверку, чтобы убедиться, что установка может работать. Если появляется предупреждение о **глобальной синхронизации времени,** убедитесь, что время на часах системы (настройки**даты и времени)** совпадает с часовым поясом.
...
После завершения регистрации сервер отображается на лезвии > **серверов** **настроек**в хранилище.
...
|/BypassProxy|Необязательный|Указывает, что сервер конфигурации подключается к Azure без использования прокси-сервера.|Следует получить это значение от Venu.|
|/ProxySettingsFilePath|Необязательный|Параметры прокси-сервера (прокси-сервер по умолчанию с обязательной аутентификацией или пользовательский прокси-сервер).|Этот файл должен быть в формате, указанном ниже.|
|DataTransferSecurePort|Необязательный|Номер порта PSIP для данных репликации.| Допустимый номер порта (значение по умолчанию — 9433).|
|/SkipSpaceCheck|Необязательный|Пропуск проверки места на диске кэша.| |
...
|/ShowThirdpartyEULA|Необязательный|Отображает лицензионное соглашение со сторонним разработчиком. Если оно задано как источник данных, то все остальные параметры игнорируются.| |
...
> 1. [Отвадить защиту](site-recovery-manage-registration-and-protection.md#disable-protection-for-a-vmware-vm-or-physical-server-vmware-to-azure) для всех виртуальных машин под этим configuration Server.
> 2. [Отмежевать и](vmware-azure-set-up-replication.md#disassociate-or-delete-a-replication-policy) [удалить](vmware-azure-set-up-replication.md#disassociate-or-delete-a-replication-policy) все политики репликации с сервера конфигурации.
> 3. [Удалите](vmware-azure-manage-vcenter.md#delete-a-vcenter-server) все серверы vCenters/vSphere, связанные с configuration Server.


### <a name="delete-the-configuration-server-from-azure-portal"></a>Удаление сервера конфигурации на портале Azure
1. На портале Azure просматривайте > **серверы конфигурации** **инфраструктуры восстановления сайта**из меню Vault.
...
1. В хранилище откройте > **сервер конфигурации** **инфраструктуры восстановления сайта**и нажмите на требуемый сервер конфигурации.
2. Срок действия отображается в **соответствии со работой configuration Server**
