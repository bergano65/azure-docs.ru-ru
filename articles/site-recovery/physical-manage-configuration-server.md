...
2. В области **перед началом**нажмите кнопку **установить сервер конфигурации и сервер обработки**.
...
6. В окне **Проверка предварительных требований**программа установки выполняет проверку, чтобы убедиться, что установка может быть выполнена. Если появится предупреждение о **проверке глобальной синхронизации времени**, убедитесь, что время на системных часах (параметры**даты и времени** ) совпадает с часовым поясом.
...
После завершения регистрации сервер отобразится в колонке " **Settings** > **серверы** параметров" в хранилище.
...
> [!NOTE]
> Параметр **-Force** в Remove-азситерековерифабрик можно использовать для принудительного удаления или удаления сервера конфигурации.
