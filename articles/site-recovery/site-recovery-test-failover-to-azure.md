---
title: Выполнить тестовый сбой (упражнение восстановления аварийности) в Azure в восстановлении сайта Azure
...
ms.locfileid: "79257528"
---
# <a name="run-a-test-failover-disaster-recovery-drill-to-azure"></a>Выполнить тестовый сбой (упражнение восстановления аварийности) в Azure 
...
3. Выберите виртуальную сеть Azure, в которой будут созданы тестовые виртуальные машины.
    ...
    - Если тот же IP-адрес недоступен в подсети, виртуальная машина получает другой доступный IP-адрес в подсети. Ознакомьтесь с [дополнительными сведениями](#create-a-network-for-test-failover).
4. Если при отработке отказа с выполнением переноса в Azure включена функция шифрования данных, то в разделе **Ключ шифрования** выберите сертификат, который был выпущен, когда во время установки поставщика вы включили функцию шифрования. Вы можете проигнорировать этот шаг, если шифрование не включено.
5. Отслеживайте сбой на вкладке **«Вакансии».** Вы должны иметь возможность видеть машину тестовых реплик на портале Azure.
...
8. В **Notes**, запись и сохранить любые наблюдения, связанные с тестом неудачи.
...
4. **Начните**с этого шага: Этот шаг создает виртуальную машину Azure с использованием данных, обработанных на предыдущем этапе.
