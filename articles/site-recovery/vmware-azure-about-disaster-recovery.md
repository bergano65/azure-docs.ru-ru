---
title: Аварийное восстановление VMware с восстановлением сайта Azure
...
6. При сбоях VMs Azure создаются из данных VM в управляемых дисках или учетных записях хранения. Пользователи по-прежнему могут получать доступ к приложениям и рабочим нагрузкам из виртуальной машины Azure.
...
- Используете ли вы необходимые версии серверов виртуализации VMware при выполнении репликации виртуальных машин VMware? [Проверьте здесь](vmware-physical-azure-support-matrix.md#on-premises-virtualization-servers).
- Реплицируете ли вы компьютеры с поддерживаемой операционной системой? [Проверьте здесь](vmware-physical-azure-support-matrix.md#replicated-machines).
- Для аварийного восстановления Linux. Работают ли компьютеры с использованием поддерживаемых файловой системы и гостевого хранилища? [Проверить здесь](vmware-physical-azure-support-matrix.md#linux-file-systemsguest-storage)
- Соответствуют ли реплицируемые компьютеры требованиям Azure? [Проверьте здесь](vmware-physical-azure-support-matrix.md#azure-vm-requirements).
- Поддерживается ли конфигурация вашей сети? [Проверьте здесь](vmware-physical-azure-support-matrix.md#network).
- Поддерживается ли конфигурация вашего хранилища? [Проверьте здесь](vmware-physical-azure-support-matrix.md#storage).
...
2. Создайте сеть Azure, к которой присоединятся VMs Azure, когда они будут созданы из учетных записей или управляемых дисков после сбоя.
...
Узнайте, как настроить Azure, [проверив учетную запись,](tutorial-prepare-azure.md#verify-account-permissions)создав [сеть](tutorial-prepare-azure.md#set-up-an-azure-network)и [создав хранилище.](tutorial-prepare-azure.md#create-a-recovery-services-vault)
...
3. **Сервер конфигурации**. Необходимо настроить сервер конфигурации в исходной локальной среде:
    ...
    - Несколько других компонентов выполняются на компьютере сервера конфигурации.
        - Сервер процесса получает, оптимизирует и отправляет данные репликации на учетную запись хранения кэша в Azure. Этот сервер также используется для автоматической установки службы Mobility на компьютеры, которые требуется реплицировать, а также для выполнения автоматического обнаружения виртуальных машин на серверах VMware.
...
4. **Целевая среда:** Вы настраиваете целевую среду Azure, указав настройки подписки Azure и сети.
...
6. **Включить репликацию.** Включите репликацию для локальных компьютеров. Если вы создали учетную запись для установки службы Mobility, то установка начнется при запуске репликации для компьютера. 
