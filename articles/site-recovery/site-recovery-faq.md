...
Использование учетных записей хранения в качестве целевого хранилища не поддерживается Azure Site Recovery. Рекомендуется вместо этого использовать управляемые диски в качестве целевого хранилища для компьютеров. Управляемые диски поддерживают только тип LRS для обеспечения устойчивости данных.
...
### <a name="what-is-the-difference-between-complete-migration-commit-and-disable-replication"></a>В чем разница между полной миграцией, фиксацией и отключением репликации?

После отработки отказа виртуальной машины из исходного расположения в целевое расположение доступны три варианта выбора. Все три служат для разных целей.

1.  **Завершение миграции** означает, что вы больше не будете возвращаться к исходному расположению. Вы выполнили миграцию в целевой регион, и теперь все готово. При выборе пункта Завершение миграции триггеры фиксируются, а затем выполняется внутреннее отключение репликации. 
2.  **Фиксация** означает, что это не конец процесса репликации. Элемент репликации вместе со всей конфигурацией останется, и вы можете **повторно включить защиту** на более позднем этапе, чтобы обеспечить репликацию компьютеров обратно в исходный регион. 
3.  **Отключение репликации** приведет к отключению репликации и удалению всех связанных конфигураций. Он не влияет на уже существующий компьютер в целевом регионе.
