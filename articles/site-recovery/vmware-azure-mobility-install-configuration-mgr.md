...
1. Скопируйте эти файлы установки в общую сетевую папку:
   ...
   - Для Windows скопируйте файл _Microsoft-ASR_UA_version_Windows_GA_date_Release. exe_ в _Каталог mobsvcwindows_.
...
1. Как описано в следующих процедурах, скопируйте код в папки Windows или Linux. Предполагается, что:
   ...
   - Безопасная сетевая общая папка `\\ContosoSecureFS\MobilityServiceInstallers`.

### <a name="copy-code-to-the-windows-folder"></a>Копирование кода в папку Windows

Скопируйте следующий код:

- Сохраните код в папке _Каталог mobsvcwindows_ в файле _install. bat_.
- Замените заполнители `[CSIP]` в этом скрипте фактическими значениями IP-адреса сервера конфигурации.
...
1. В консоли Configuration Manager щелкните пакет правой кнопкой мыши и выберите команду **распространить содержимое**.
   ...
   ![Снимок экрана консоли Configuration Manager](./media/vmware-azure-mobility-install-configuration-mgr/sccm_distribute.png)

1. Выберите точки распространения, в которых должны копироваться пакеты. [Дополнительные сведения](/sccm/core/servers/deploy/configure/install-and-configure-distribution-points)
...
1. После завершения распространения пакета щелкните правой кнопкой мыши пакет, > **развернуть**.
   ...
   ![Снимок экрана консоли Configuration Manager](./media/vmware-azure-mobility-install-configuration-mgr/sccm_deploy.png)
...
1. В поле **укажите расписание для этого развертывания**настройте расписание. [Дополнительные сведения](/sccm/apps/deploy-use/deploy-applications#bkmk_deploy-sched)
...
1. Мониторинг хода выполнения развертывания в консоли Configuration Manager. Перейдите к разделу **мониторинг** > **развертываний** >  _\<имя пакета\>_ .
