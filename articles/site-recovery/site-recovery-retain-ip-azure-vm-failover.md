---
title: Храните IP-адреса после сбоя Azure VM при восстановлении сайта Azure
...
- Для уменьшения времени восстановления (RTO) компания использует узлы реплики для s'L Server Always On, контроллеров доменов и т.д. Эти узлы реплики находятся в другом VNet в целевом регионе, так что они могут установить VPN-сайт-сайт связи между источником и целевых регионов. Это невозможно, если в исходном и целевом регионах используется один и тот же диапазон IP-адресов.  
- Перед отработкой отказа используется следующая архитектура сети:
    ...
    - Дополнительный (целевой) регион Azure для восстановления — "Юго-Восточная Азия".
        ...
        - Реплики узлов для сервера S'L Всегда на, контроллер домена и т.д. расположены в **Subnet 4**.
...
- В Восточной Азии находится виртуальная сеть (**Исходная виртуальная сеть**) с диапазоном адресов 10.1.0.0/16.
  - В Восточной Азии рабочие нагрузки разделены между тремя подсетями **исходной виртуальной сети**:
    - **Подсеть 1**: 10.1.1.0/24
    - **Поднет 2**: 10.1.2.0/24
    - **Subnet 3**: 10.1.3.0/24, используя виртуальную сеть Azure с адресным пространством 10.1.0.0.16. Это **исходная виртуальная сеть**.
      - Дополнительный (целевой) регион Azure для восстановления — "Юго-Восточная Азия".
  - В Юго-Восточной Азии есть виртуальная сеть восстановления (**Виртуальная сеть восстановления**), идентичная **исходной виртуальной сети**.
- ВМ в Восточной Азии подключены к центру обработки данных с Azure ExpressRoute или VPN-сайтом.
