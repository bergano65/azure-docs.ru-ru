---
title: Аварийное восстановление VMware VMs/физических серверов на вторичный сайт с восстановлением сайта Azure
...
**Функция** | **Репликация в Azure** |**Репликация между центрами обработки данных VMware**
--|--|--
...
**Реплицируются** |Диск (Windows и Linux) |Том в Windows<br> Диск в Linux
...
**Мониторинга** |С портала Azure|С сервера конфигурации
...
- InMage_UA_8.0.7.0_SLES11-SP4-64_GA_03Dec2018_release.tar.gz
  ...
  2. **RX-сервер**: Копирование **RX_8.0.7.0_GA_Update_7_2965621_28Dec18.tar.gz** на сервер RX, и извлечь его. В извлеченной папке выполните команду **/Install**.
  3. **Конфигурация сервера и сервера процесса**: Копирование **CX_Windows_8.0.7.0_GA_Update_7_2965621_28Dec18.exe** на сервер конфигурации и процесс сервера. Дважды щелкните файл, чтобы запустить его.<br>
  4. **Windows Master Target server:** Для обновления унифицированного агента, скопируйте **InMage_UA_8.0.7.0_Windows_GA_27Dec2018_release.exe** на сервер. Дважды щелкните файл, чтобы запустить его. Этот файл может также использоваться для новой установки. То же обновление единого агента также можно применять к исходному серверу.
  ...
  5. **vContinuum сервер**: Копировать **InMage_Scout_vContinuum_MT_8.0.7.0_Windows_GA_27Dec2018_release.exe** к серверу.  Убедитесь, что вы закрыли мастер vContinuum. Дважды щелкните файл, чтобы запустить его.
  6. **Linux мастер целевой сервер**: Для обновления единого агента, скопировать **InMage_UA_8.0.7.0_RHEL6-64_GA_03Dec2018_release.tar.gz** на сервер Linux Master Target и извлечь его. В извлеченной папке выполните команду **/Install**.
  7. **Сервер источника Windows**: Для обновления унифицированного агента, скопируйте **InMage_UA_8.0.7.0_Windows_GA_27Dec2018_release.exe** на исходный сервер. Дважды щелкните файл, чтобы запустить его. 
  8. **Исходный сервер Linux**: чтобы обновить единый агент, скопируйте соответствующую версию файла единого агента на сервер Linux и извлеките его содержимое. В извлеченной папке выполните команду **/Install**.  Пример: Для RHEL 6.7 64-битный сервер, скопировать **InMage_UA_8.0.7.0_RHEL6-64_GA_03Dec2018_release.tar.gz** к серверу, и извлечь его. В извлеченной папке выполните команду **/Install**.
...
2. Дополнительные сведения об установке, защите и восстановлении см. в следующих документах:
   ...
   * [Руководство для пользователей](https://aka.ms/asr-scout-user-guide)
...
Обновлено: Декабрь 31, 2018 Скачать [Скаут обновление 7](https://aka.ms/asr-scout-update7).
...
* Исправлено: Windows Cluster/Standalone Machines имеют неправильную конфигурацию IP при восстановлении/DR-дрель.
* Исправлено: Иногда операция добавления диска не удается для кластера V2V.
...
* Исправлено: проблемы с безопасностью MyS'L смягчаются за счет обновления MyS'L до версии 5.7.23
...
Пожалуйста, следуйте инструкциям, приведенным в руководстве по [быстрой установке](https://aka.ms/asr-scout-quick-install-guide) для обновления версий PHP и MyS'L.
...
- UA update4 бит для RHEL5, OL5, OL6, SUSE 10, SUSE 11: UA_\<Linux OS>_8.0.4.0_GA_Update_4_9035261_26Sep16.tar.gz
  1. Извлеките ZIP-файлы.
  ...
  Обновление не нужно применять на Master Target подготовлен с **InMage_Scout_vContinuum_MT_8.0.1.0_Windows_GA_10Oct2017_release.exe,** как это новый га установщик со всеми последними изменениями.
