---
title: Аварийное восстановление виртуальных машин VMware или физических серверов на дополнительный сайт с помощью Azure Site Recovery
...
--|--|--
**Обязательные компоненты** |Служба Mobility Service на реплицированных компьютерах. Локальный сервер конфигурации, сервер обработки, главный целевой сервер, временный сервер обработки в Azure для восстановления размещения|Служба Mobility Service, сервер обработки, сервер конфигурации и главный целевой сервер
**Конфигурация и оркестрация** |Хранилище Служб восстановления на портале Azure | Использование vContinuum 
**Реплицировано** |Диск (Windows и Linux) |Том в Windows<br> Диск в Linux
**Кластер общих дисков** |Не поддерживается|Поддерживается
**Скорость обработки данных (средняя)** |10 МБ/с данных на диск<br> 25 МБ/с данных на виртуальную машину<br> [Дополнительные сведения](./site-recovery-vmware-deployment-planner-analyze-report.md#azure-site-recovery-limits) | > 10 МБ/с данных на диск  <br> > 25 МБ/с данных на виртуальную машину
**Мониторинг** |С портала Azure|С сервера конфигурации
**Матрица поддержки** | [Щелкните здесь для получения дополнительных сведений](./vmware-physical-azure-support-matrix.md)|[Скачайте матрицу совместимости с ASR Scout](https://aka.ms/asr-scout-cm)


## <a name="prerequisites"></a>Предварительные требования
Для работы с этим руководством сделайте следующее:

- [Ознакомьтесь](vmware-physical-secondary-support-matrix.md) с требованиями к поддержке для всех компонентов.
...
Скачайте ZIP-файл [обновления](https://aka.ms/asr-scout-update7) и файлы конфигурации для [обновления MySQL и PHP](https://aka.ms/asr-scout-u7-mysql-php-manualupgrade). ZIP-файл обновления содержит все базовые двоичные файлы и двоичные файлы накопительного пакета обновления для следующих компонентов: 
...
- InMage_UA_8.0.7.0_SLES11-SP4-64_GA_03Dec2018_release.tar.gz
  ...
  2. **Сервер RX**: Скопируйте **RX_8.0.7.0_GA_Update_7_2965621_28Dec18. tar. gz** на сервер RX и извлеките его. В извлеченной папке выполните команду **/Install**.
  3. **Сервер конфигурации и сервер обработки**: Скопируйте **CX_Windows_8.0.7.0_GA_Update_7_2965621_28Dec18.exe** на сервер конфигурации и сервер обработки. Дважды щелкните файл, чтобы запустить его.<br>
  4. **Главный целевой сервер Windows**. чтобы обновить единый агент, скопируйте **InMage_UA_8.0.7.0_Windows_GA_27Dec2018_release.exe** на сервер. Дважды щелкните файл, чтобы запустить его. Этот файл может также использоваться для новой установки. То же обновление единого агента также можно применять к исходному серверу.
  Применение обновления не требуется на главном целевом сервере, подготовленном с помощью файла **InMage_Scout_vContinuum_MT_8.0.7.0_Windows_GA_27Dec2018_release.exe**, так как это новый общедоступный установщик со всеми последними изменениями.
  5. **сервер vContinuum**: копирование **InMage_Scout_vContinuum_MT_8.0.7.0_Windows_GA_27Dec2018_release.exe** на сервер.  Убедитесь, что вы закрыли мастер vContinuum. Дважды щелкните файл, чтобы запустить его.
  6. **Главный целевой сервер Linux**. чтобы обновить единый агент, скопируйте **InMage_UA_8.0.7.0_RHEL6-64_GA_03Dec2018_release. tar. gz** на главный целевой сервер Linux и извлеките его. В извлеченной папке выполните команду **/Install**.
  7. **Исходный сервер Windows**. чтобы обновить единый агент, скопируйте **InMage_UA_8.0.7.0_Windows_GA_27Dec2018_release.exe** на исходный сервер. Дважды щелкните файл, чтобы запустить его. 
  8. **Исходный сервер Linux**: чтобы обновить единый агент, скопируйте соответствующую версию файла единого агента на сервер Linux и извлеките его содержимое. В извлеченной папке выполните команду **/Install**.  Пример. для RHEL 6,7 64-bit Server скопируйте **InMage_UA_8.0.7.0_RHEL6-64_GA_03Dec2018_release. tar. gz** на сервер и извлеките его. В извлеченной папке выполните команду **/Install**.
  9. После обновления сервера конфигурации, сервера обработки и сервера RX с помощью упомянутых выше установщиков необходимо вручную обновить библиотеки PHP и MySQL. Для этого выполните действия, описанные в разделе 7.4 [краткого руководства по установке](https://aka.ms/asr-scout-quick-install-guide).
...
2. Дополнительные сведения об установке, защите и восстановлении см. в следующих документах:
   ...
   * [Таблица совместимости](https://aka.ms/asr-scout-cm)
   ...
   * [Обновление библиотек MYSQL и PHP](https://aka.ms/asr-scout-u7-mysql-php-manualupgrade)

## <a name="updates"></a>Обновления

### <a name="site-recovery-scout-801-update-7"></a>Обновление 7 для Site Recovery Scout 8.0.1 
Обновлено: 31 декабря 2018 загрузить [Обновление 7 для Scout](https://aka.ms/asr-scout-update7).
...
#### <a name="bug-and-security-fixes"></a>Устранение ошибок и исправления безопасности.
* Исправлено: кластер или автономные компьютеры Windows имеют неверную конфигурацию IP-адресов при восстановлении или АВАРИЙном завершении.
* Исправлено. иногда сбой операции добавления диска для кластера V2V.
* Исправлено. Мастер vContinuum зависал на этапе восстановления, если главный целевой сервер работал на платформе Windows Server 2016.
* Исправлено. проблемы безопасности MySQL устранены путем обновления MySQL до версии 5.7.23

#### <a name="manual-upgrade-for-php-and-mysql-on-csps-and-rx"></a>Ручное обновление компонентов PHP и MySQL на сервере конфигурации, сервере обработки и сервере RX
Платформу сценариев PHP необходимо обновить до версии 7.2.10 на сервере конфигурации, сервере обработки и сервере RX.
Систему управления базами данных MySQL необходимо обновить до версии 5.7.23 на сервере конфигурации, сервере обработки и сервере RX.
Выполните действия, приведенные в руководстве по [быстрой установке](https://aka.ms/asr-scout-quick-install-guide) , чтобы обновить версии PHP и MySQL.
...
- Обновление 4 UA для RHEL5, OL5, OL6, SUSE 10, SUSE 11: UA_\<Linux OS>_8.0.4.0_GA_Update_4_9035261_26Sep16.tar.gz
  ...
  Обновление не нужно применять к главному целевому объекту, подготовленному с помощью **InMage_Scout_vContinuum_MT_8.0.1.0_Windows_GA_10Oct2017_release.exe** так как это новый общедоступный установщик, содержащий все последние изменения.
