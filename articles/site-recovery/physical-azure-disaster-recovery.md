...
Служба [Azure Site Recovery](site-recovery-overview.md) относится к стратегии аварийного восстановления, управляя репликацией, отработкой отказа и восстановлением размещения локальных компьютеров, а также виртуальных машин Azure (ВМ).
...
- Ознакомьтесь с [разрешениями](site-recovery-role-based-linked-access-control.md#permissions-required-to-enable-replication-for-new-virtual-machines) , необходимыми для репликации компьютеров в Azure.
...
- Для виртуальных машин Windows, если учетная запись домена не используется, отключите контроль удаленного доступа пользователей на локальном компьютере. Для этого в регистре в разделе **HKEY_LOCAL_MACHINE \софтваре\микрософт\виндовс\куррентверсион\полиЦиес\систем**добавьте параметр **LocalAccountTokenFilterPolicy**записи DWORD со значением 1.
...
На компьютере сервера конфигурации убедитесь, что системные часы синхронизированы с [сервером времени](https://technet.microsoft.com/windows-server-docs/identity/ad-ds/get-started/windows-time-service/windows-time-service). Значения времени должны совпадать. Если системные часы отстают или спешат в пределах 15 минут, установка может завершиться ошибкой.
...
После завершения регистрации сервер конфигурации отображается на странице**серверы** **параметров** > в хранилище.
...
1. Чтобы создать новую политику репликации, щелкните **Site Recovery** > **политики** > репликации инфраструктуры **+ Политика репликации**.
...
1. Щелкните **репликация** > **источника**приложения.
...
5. В поле **целевой объект**выберите подписку и группу ресурсов, в которых необходимо создать виртуальные машины Azure после отработки отказа. Выберите модель развертывания, которая будет использоваться в Azure (классическая или Resource Manager).
...
8. Выберите **настроить сейчас для выбранных компьютеров**, чтобы применить параметр сети ко всем компьютерам, выбранным для защиты. Щелкните **настроить позже** , чтобы выбрать сеть Azure на компьютер. 
...
10. В окне **Свойства** > **Настройка свойств**выберите учетную запись, которая будет использоваться сервером обработки для автоматической установки службы Mobility Service на компьютере.
11. В области **Параметры** > репликации**Настройка параметров репликации**убедитесь, что выбрана правильная политика репликации. 
12. Щелкните **включить репликацию**. Ход выполнения задания **включить защиту** можно отслеживать в**Site Recovery заданиях****задания** >  **параметров** > . После выполнения задания **завершить подготовку защиты** компьютер будет готов к отработке отказа.


Для мониторинга добавленных серверов можно проверить время последнего обнаружения для них на странице **серверы** > конфигурации**последний контакт**в. Чтобы добавить компьютеры, не дожидаясь запланированного времени обнаружения, выделите сервер конфигурации (не щелкая его) и щелкните **Обновить**.
