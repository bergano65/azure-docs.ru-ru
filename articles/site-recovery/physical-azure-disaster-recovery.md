---
title: Настройка аварийного восстановления физических наемных серверов с помощью Azure Site Recovery
...
Служба [восстановления сайтов Azure](site-recovery-overview.md) вносит свой вклад в стратегию аварийного восстановления, управляя и организуя репликацию, отказ и отказ от работы на местах, а также виртуальных машин Azure (VM).
...
- Просмотрите [требования к поддержке](vmware-physical-secondary-support-matrix.md) всех компонентов.
...
Получите [учетную запись](https://azure.microsoft.com/)Microsoft Azure .
...
- Просмотрите [разрешения,](site-recovery-role-based-linked-access-control.md#permissions-required-to-enable-replication-for-new-virtual-machines) необходимые для репликации машин в Azure.
...
Настройка [сети Azure.](../virtual-network/quick-create-portal.md)
...
Настройка [учетной записи хранения данных Azure.](../storage/common/storage-account-create.md)
...
- Для виртуальных машин Windows, если учетная запись домена не используется, отключите контроль удаленного доступа пользователей на локальном компьютере. Для этого в регистре **под HKEY_LOCAL_MACHINE»SOFTWARE-Microsoft-Windows-CurrentVersion -Политика ,** добавьте запись DWORD **LocalAccountTokenFilterPolicy**, со значением 1.
...
2. В меню ресурсов нажмите **«Восстановление** > сайта Подготовка**к цели защиты****инфраструктуры».** > 
...
1. Нажмите **На сайте Восстановление** > **Подготовка Источник инфраструктуры** > **Source**.
...
На сервере конфигурации убедитесь, что системные часы синхронизированы с [Time Server.](https://technet.microsoft.com/windows-server-docs/identity/ad-ds/get-started/windows-time-service/windows-time-service) Значения времени должны совпадать. Если системные часы отстают или спешат в пределах 15 минут, установка может завершиться ошибкой.
...
После завершения регистрации сервер конфигурации отображается на странице > **«Серверы** **настроек»** в хранилище.

## <a name="set-up-the-target-environment"></a>Настройка целевой среды

Выберите и проверьте целевые ресурсы.

1. Нажмите **«Подготовьте цель инфраструктуры»** > **Target**и выберите выбранную подписку Azure, которая вы хотите использовать.
...
1. Чтобы создать новую политику репликации, щелкните > **политики репликации инфраструктуры** >  **восстановления сайта****.**
...
1. Нажмите **Кнопка Коплицк приложение** > **Источник**.
...
5. В **Target**выберите подписку и группу ресурсов, в которой требуется создать VMs Azure после сбоя. Выберите модель развертывания, которая будет использоваться в Azure (классическая или Resource Manager).
...
8. Выберите **настройку для выбранных машин,** чтобы применить настройки сети для всех машин, которые вы выбираете для защиты. Выберите **наболее поздний выбор** сети Azure для одного машины. 
...
10. В **свойствах Properties** > **Configure**выберите учетную запись, которая будет использоваться сервером процесса для автоматической установки службы мобильности на машине.
11. В **настройках** > репликации**настройки репликации**проверяют выбранную политику репликации. 
12. Нажмите **Включить репликации**. Вы можете отслеживать ход выполнения задания **«Включить защиту»** в > **заданиях восстановления сайта** **«Настройки** > **вакансий».** После выполнения задания **Завершить подготовку защиты** виртуальная машина будет готова к отработке отказа.


Для мониторинга серверов, которые вы добавляете, вы можете проверить последнее обнаруженное время для них в **конфигурационных серверах** > **Последний контакт в**. Чтобы добавить компьютеры, не дожидаясь запланированного времени обнаружения, выделите сервер конфигурации (не щелкая его) и щелкните **Обновить**.

## <a name="next-steps"></a>Дальнейшие действия

[Выполнить аварийное восстановление дрель](tutorial-dr-drill-azure.md).
