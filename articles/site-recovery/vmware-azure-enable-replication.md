...
>[!NOTE]
>* Теперь репликация Azure Site Recovery выполняется непосредственно в управляемые диски для всех новых репликациями. Сервер обработки записывает журналы репликации для учетной записи хранения кэша в целевом регионе. Эти журналы используются для создания точек восстановления на управляемых дисках реплики. 
>* Во время отработки отказа клиентом выбранная точка восстановления используется для создания управляемого диска целевой объект.
>* Виртуальных машин, настроенных ранее для репликации в целевые учетные записи хранения не будут затронуты. 
>* Репликация в учетные записи хранения для новой машины доступна только через REST API и Powershell. Используйте API версии 2016-08-10 "или" 2018-01-10 для репликации в учетные записи хранения.
...
7. Выберите сеть и подсеть Azure, к которой будут подключаться развернутые виртуальные машины Azure после отработки отказа. Сеть должна располагаться в том же регионе, что и хранилище служб восстановления. Выберите **Настроить сейчас для всех выбранных компьютеров**, чтобы применить параметр сети ко всем защищаемым компьютерам. Выберите **Отложить настройку**, чтобы выбрать сеть Azure для каждого компьютера. Если у вас нет сети, ее необходимо создать. Чтобы создать сеть, используя диспетчер ресурсов, щелкните **Создать**. Выберите подсеть, если это применимо, затем нажмите кнопку **ОК**.
   
   ![Включение параметров целевого объекта репликации](./media/vmware-azure-enable-replication/enable-rep3.png)

8. В разделе **Виртуальные машины** > **Выбор виртуальных машин**, выберите каждую машину, которую нужно реплицировать. Можно выбрать только те компьютеры, для которых можно включить репликацию. Нажмите кнопку **ОК**. Если определенная виртуальная машина не отображается или ее нельзя выбрать, см. раздел [Исходный компьютер отсутствует в списке на портале Azure](https://aka.ms/doc-plugin-VM-not-showing) для устранения проблемы.

    ![Включение в репликацию выбранных виртуальных машин](./media/vmware-azure-enable-replication/enable-replication5.png)

9. В поле **Свойства** > **Настройка свойств** выберите учетную запись, используемую сервером обработки для автоматической установки службы Mobility Service на компьютере. Кроме того выберите тип целевого управляемого диска, который нужно реплицировать на основе ваших данных оттока шаблонов.
10. По умолчанию реплицируются все диски исходного компьютера. Чтобы исключить диски из репликации, снимите флажок **Include** флажок напротив все диски, не нужно реплицировать.  Нажмите кнопку **ОК**. Позже можно задать дополнительные свойства. Дополнительные сведения об исключении дисков см.в [этой статье](vmware-azure-exclude-disk.md).
...
12. Включите параметр **Multi-VM consistency** (Согласованность нескольких виртуальных машин), если хотите объединить компьютеры в группу репликации. Укажите имя группы и нажмите кнопку **ОК**. 
...
3. Приведенные далее свойства виртуальной машины можно изменить в разделах **Вычисления и сеть** > **Свойства вычислений**.
   ...
   * Размер целевой виртуальной Машины или типа виртуальной Машины — по умолчанию размер виртуальной Машины определяется в источнике размер виртуальной Машины. Основываясь на времени, требуемом перед отработкой отказа, можете выбрать другой размер виртуальной машины. Обратите внимание, что размер диска виртуальной машины основывается на размере начального диска и может быть изменен только после отработки отказа. Дополнительные сведения о размерах дисков и операциях ввода-вывода см. в статье [Целевые показатели масштабируемости и производительности для дисков ВМ под управлением Windows](../virtual-machines/windows/disk-scalability-targets.md).
