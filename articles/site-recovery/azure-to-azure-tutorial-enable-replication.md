...
2. На **домашней странице** или в меню портала Azure выберите **Создать ресурс**. Затем выберите **Средства управления** > **Backup и Site Recovery**.
3. В поле **Имя** укажите понятное имя для идентификации хранилища. Если у вас есть несколько подписок, выберите нужную.
4. Создайте группу ресурсов или выберите существующую. Укажите регион Azure. Сведения о поддерживаемых регионах см. в разделе о доступности по регионам на странице [Цены на Azure Site Recovery](https://azure.microsoft.com/pricing/details/site-recovery/).
...
Если вы используете группу безопасности сети (NSG), создайте правила NSG с тегом службы для доступа к службе хранилища, Azure Active Directory, службе Site Recovery и ее данным мониторинга. [Подробнее](azure-to-azure-about-networking.md#outbound-connectivity-for-ip-address-ranges).

Для управления исходящими подключениями по IP-адресу вместо правил NSG разрешите эти адреса в брандмауэре или прокси-сервере на основе IP-адресов, либо в правилах группы безопасности сети.

>[!NOTE]
>Рекомендуется всегда настраивать правила NSG с тегами службы для исходящего доступа.
...
5. Выберите **Исходная группа ресурсов** и нажмите **ОК**, чтобы сохранить настройки.
...
2. Чтобы переопределить параметры по умолчанию для целевых объектов, щелкните ссылку **Настроить** рядом с заголовком **Группа ресурсов, сеть, хранилище и доступность**.
...
4. Чтобы настроить параметры политики репликации, щелкните **Настроить** рядом с полем **Политика репликации** и при необходимости измените параметры.
...
В рамках этого руководства вы настроили аварийное восстановление для виртуальной машины Azure. Теперь вы можете инициировать аварийное восстановление, чтобы проверить, что отработка отказа работает правильно.
