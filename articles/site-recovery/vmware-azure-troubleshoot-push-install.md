...
* Включение общего доступа к файлам с помощью групповая политика:
  1. Перейдите в раздел **Пуск**, `gpmc.msc` введите и выполните поиск.
...
Эта ошибка возникает, когда учетная запись пользователя, используемая для установки, не `CSScript` имеет разрешений на выполнение команды. Предоставьте учетной записи пользователя необходимые разрешения на выполнение скрипта и повторите операцию.
...
В журнале, расположенном на сервере конфигурации _в\<к:\програмдата\асрсетуплогс\уплоадедлогс Дата-время,>UA_InstallLogFile. log_ , вы обнаружите следующее исключение:
...
1. С помощью редактора реестра `regedit.msc`, например, откройте реестр.
