...
Аппаратный модуль безопасности имеет вариант с перезагрузкой командной строки, тем не менее, при возникновении проблем зависания перезагрузки периодически и по этой причине рекомендуется для безопаснее перезагрузить, чтобы создавалось запрос в службу поддержки корпорации Майкрософт по переводу физически перезагрузки устройства. 
