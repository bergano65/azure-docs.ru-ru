...
### <a name="q-what-is-the-azure-dedicated-hsm-offering"></a>В: Что такое предложение Azure Dedicated HSM?
...
### <a name="q-what-hardware-is-used-for-dedicated-hsm"></a>В: Какое оборудование используется для выделенных HSM?
...
HSM используются для хранения криптографических ключей, которые используются для криптографических функций, таких как TLS (безопасность транспортного слоя), шифрование данных, PKI (инфраструктура открытых ключей), DRM (управление цифровыми правами) и подписание документов.
...
Компания Gemalto предоставляет все программное обеспечение для устройства HSM сразу после его подготовки корпорацией Майкрософт. Программное обеспечение доступно на [портале поддержки пользователей Gemalto](https://supportportal.gemalto.com/csm/). Клиенты, использующие службу выделенных устройств HSM, должны зарегистрироваться для получения поддержки от Gemalto и получить идентификатор клиента, позволяющий найти и скачать соответствующее ПО. Поддерживаемое клиентское программное обеспечение — это версия 7.2, которая совместима с проверенной версией прошивки 7.0.3 уровня FIPS 140-2 Уровня 3. 

### <a name="q-does-azure-dedicated-hsm-offer-password-based-and-ped-based-authentication"></a>В: Предлагает ли Azure Dedicated HSM проверку подлинности на основе паролей и PED?

В настоящее время служба выделенных устройств HSM Azure предоставляет только HSM с аутентификацией на основе пароля.

### <a name="q-will-azure-dedicated-hsm-host-my-hsms-for-me"></a>В: Будет ли Azure Dedicated HSM хозяйничает для меня HSMs-ми.
...
### <a name="q-does-azure-dedicated-hsm-support-payment-pineft-features"></a>В: Функции поддержки Azure Dedicated HSM (PIN/EFT) - функции?

Служба выделенных устройств HSM Azure использует устройства HSM 7 SafeNet Luna Network (модель A790). Эти устройства не поддерживают оплату HSM конкретной функциональности (например, PIN или EFT) или сертификации. Если вы хотите, чтобы служба Azure Dedicated HSM поддерживала оплату HSMs в будущем, передайте отзывы представителю учетной записи Майкрософт.

### <a name="q-which-azure-regions-is-dedicated-hsm-available-in"></a>В: В каких регионах Azure имеется специальный HSM?

По состоянию на конец марта 2019 года, выделенный HSM доступен в 14 регионах, перечисленных ниже. Планируются дальнейшие регионы, которые могут быть обсуждены через ваш представитель учетной записи Майкрософт.
...
Служба выделенных устройств HSM Azure — подходящий вариант для предприятий, переходящих на локальные приложения Azure, которые используют устройства HSM. Выделенные устройства HSM позволяют перенести приложение с минимальными изменениями. Если криптографические операции выполняются в коде приложения, выполняющемся в веб-приложении или на виртуальной машине Azure, то можно использовать выделенное устройство HSM. В целом, программное обеспечение, работающем в моделях IaaS (инфраструктура как услуга), которые поддерживают HSMs в качестве ключевого магазина, могут использовать Dedicate HSM, такие как шлюз приложения или менеджер трафика для бесключевого TLS, ADCS (Active Directory Certificate Services) или аналогичные инструменты PKI, инструменты/приложения, используемые для подписания документов, подписания кода или сервера S'L (IaaS), настроенного с TDE (прозрачное шифрование базы данных) с помощью основного ключа в HSM с помощью eKM (экстенированного поставщика ключей). Azure Key Vault подходит для приложений «рожденные в облаке» или для шифрования в остальных сценариях, где данные клиентов обрабатываются PaaS (платформа как услуга) или SaaS (программное обеспечение как служба) сценариев, таких как Office 365 Customer Key, Azure Information Protection, Azure Disk Encryption, azure Data Lake Store с помощью сервера клиента, шифрование хранилища azure
...
* Шлюз приложения или менеджер трафика для бесключевого TLS
...
Нет. Выделенный HSM предоставляется непосредственно в закрытом пространстве IP-адреса клиента, поэтому он не доступен другим службам Azure или Microsoft.
...
### <a name="q-what-is-the-tenantadmin-account-microsoft-uses-i-am-used-to-the-admin-user-being-admin-on-safenet-hsms"></a>В: Что использует учетная запись "tenantadmin", используемая корпорацией Майкрософт, я привык к тому, что пользователь-админ -админ" на HSMs SafeNet?

Устройство HSM поставляется с пользователем админ по умолчанию с обычным паролем по умолчанию. Корпорация Майкрософт не хотела использовать пароли по умолчанию, пока любое устройство находится в пуле ожидания, которое будет подготовлено клиентами. Это не отвечало бы нашим строгим требованиям безопасности. По этой причине мы устанавливаем надежный пароль, который отбрасывается во время подготовки. Кроме того, во время подготовки мы создаем нового пользователя в роли админа под названием "tenantadmin". Этот пользователь имеет пароль по умолчанию, и клиенты меняют это как первое действие при первом входе в новое устройство. Этот процесс обеспечивает высокую степень безопасности и сохраняет наше обещание единоличного административного контроля для наших клиентов. Следует отметить, что пользователь "tenantadmin" может быть использован для сбоя пароля пользователя админ, если клиент предпочитает использовать эту учетную запись. 
...
### <a name="q-is-it-possible-to-configure-high-availability-in-the-same-region-or-across-multiple-regions"></a>В: Можно ли настроить высокую доступность в одном регионе или в нескольких регионах?

Да. Настройка высокого уровня доступности выполняется в клиентском программном обеспечении HSM, предоставляемом компанией Gemalto. HSMs из того же VNET или других VNET s в том же регионе или в разных регионах, или на помещениях HSMs, подключенных к VNET с помощью сайта на сайт или пункт-точка VPN могут быть добавлены к той же конфигурации высокой доступности. Следует отметить, что это синхронизирует только ключевые материалы, а не конкретные элементы конфигурации, такие как роли.

### <a name="q-can-i-add-hsms-from-my-on-premises-network-to-a-high-availability-group-with-azure-dedicated-hsm"></a>В: Могу ли я добавить HSMs из моей сети в группу высокой доступности с Azure Dedicated HSM?

Да. Они должны соответствовать требованиям высокого уровня доступности для SafeNet Luna Network HSM 7.

### <a name="q-can-i-add-luna-56-hsms-from-on-premises-networks-to-a-high-availability-group-with-azure-dedicated-hsm"></a>В: Могу ли я добавить HSMs Luna 5/6 из штатных сетей в группу высокой доступности с Azure Dedicated HSM?
...
16 членов группы HA имеют under-gone, полное тестирование дроссельной заслонки с отличными результатами.
...
Для службы Выделенный HSM не предусмотрена какая-либо конкретная гарантия простоя. Корпорация Майкрософт обеспечивает доступ к устройству на уровне сети, поэтому действуют стандартные соглашения об уровне обслуживания для сетей Azure.
...
Поддержка предоставляется как корпорацией Майкрософт, так и Gemalto.  Если у вас есть проблемы с оборудованием или доступом к сети, выможете обратиться в корпорацию Майкрософт с запросом на поддержку, и если у вас есть проблемы с конфигурацией HSM, программное обеспечение и разработка приложений запросна на поддержку с Gemalto. Если у вас есть неопределенная проблема, подключите запрос на поддержку в корпорацию Майкрософт, и тогда Gemalto может быть привлечена по мере необходимости. 

### <a name="q-how-do-i-get-the-client-software-documentation-and-access-to-integration-guidance-for-the-safenet-luna-7-hsm"></a>В: Как получить клиентское программное обеспечение, документацию и доступ к инструкциям по интеграции для SafeNet Luna 7 HSM?

После регистрации на услугу будет предоставлен идентификатор клиента Gemalto, позволяющий зарегистрироваться на портале поддержки клиентов Gemalto. Это позволит получить доступ ко всему программному обеспечению и документации, а также позволяет запросы поддержки непосредственно с Gemalto.
...
### <a name="q-what-if-i-need-to-reboot-my-hsm"></a>В: Что делать, если мне нужно перезагрузить HSM?

HSM имеет опцию перезагрузки командной строки, однако, мы испытываем проблемы перезагрузки повесить с перерывами и по этой причине рекомендуется для безопасной перезагрузки, что вы поднимаете запрос на поддержку с Microsoft, чтобы устройство физически перезагрузки. 
...
### <a name="q-how-does-a-customer-ensure-that-when-an-hsm-is-deprovisioned-all-the-key-material-is-wiped-out"></a>Вопрос: Как клиент гарантирует, что при дезавуировании HSM все ключевые материалы будут уничтожены?
...
Используемая модель SafeNet Luna HSM 7 включает в себя лицензию на 10 разделов в стоимость услуги. Устройство имеет лимит в 100 разделов, и добавление разделов до этого предела повлекло бы за собой дополнительные расходы на лицензирование и потребовало бы установки нового лицензионного файла на устройстве.
...
Максимальное количество ключей является функцией доступной памяти. SafeNet Luna 7 модель A790 в использовании имеет 32 МБ памяти. Следующие числа также применимы к парам ключей при использовании асимметричных ключей.
