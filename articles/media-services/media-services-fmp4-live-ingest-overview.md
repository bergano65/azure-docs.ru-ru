...
## <a name="2-service-diagram"></a>2. Схема обслуживания
...
## <a name="3-bitstream-format--iso-14496-12-fragmented-mp4"></a>3. Формат Bitstream - ISO 14496-12 фрагментированный MP4
...
1. Раздел 3.3.2 в No 1 определяет дополнительную коробку под названием **StreamManifestBox** для живого глотания. Из-за логики маршрутизации в балансировщике нагрузки Azure не рекомендуется использовать это поле. Его НЕ ДОЛЖНО быть во время приема в службах мультимедиа. При наличии этого поля службы мультимедиа автоматически его игнорируют.
...
Это не исчерпывающий список всех возможных вариантов приема в данном примере. Фактически приемом прямой передачи поддерживается любое группирование дорожек в потоки. Заказчики и поставщики кодировщиков могут выбирать собственные реализации в зависимости от инженерной сложности, емкости кодировщика и соображений избыточности и отработки отказов. Однако в большинстве случаев во всей текущей презентации существует только одна звуковая дорожка, Таким образом, важно обеспечить работоспособность потоку, содержащему звуковую дорожку. Это соображение часто приводит к тому, что он ставит звуковую дорожку в свой собственный поток (как в варианте 2) или совмещает ее с самым низким количеством видеотрека (как в варианте 3). Кроме того, для повышения избыточности и отказоустойчивости передача звуковой дорожки в двух разных потоках (вариант 2 с избыточными звуковыми дорожками) или объединение звуковой дорожки по крайней мере с двумя видеодорожками с наименьшей скоростью (вариант 3 со звуком, включенным в не менее чем два видеопотока) настоятельно рекомендуются для приема прямой передачи в службах мультимедиа.

## <a name="7-service-failover"></a>7. Сбой обслуживания
...
1. После возникновения ошибки TCP:
    ...
    а. НЕОБХОДИМО закрыть текущее соединение, а также НЕОБХОДИМО создать новое соединение для нового запроса HTTP POST.
...
## <a name="8-encoder-failover"></a>8. Сбой кодера
...
## <a name="9-encoder-redundancy"></a>9. Избыточность кодера
...
## <a name="10-service-redundancy"></a>10. Увольнение службы
...
1. В **Live Server Manifest Box,** как это определено в разделе 6 в разделе «1», используйте параметр *parentTrackName* для указания имени родительского трека. Для получения более подробной информации см.
...
1. Учитывая разреженный характер сигнального события, мы рекомендуем сделать следующее:
    ...
    а. В начале транслируемого события кодировщик отправляет поля изначального заголовка службе, что позволит службе зарегистрировать разреженную дорожку в манифесте клиента.
...
1. Используйте отдельные потоки для отправки двух видеодорожек с наименьшей скоростью потока. Каждый из этих потоков SHOULD также содержат копию каждого уникального звукового трека. Например, когда поддерживаются несколько языков, эти потоки ДОЛЖНЫ содержать звуковые дорожки для каждого языка.
