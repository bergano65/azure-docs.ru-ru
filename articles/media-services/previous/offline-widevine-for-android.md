...
- В некоторых странах/регионах доступность Интернета и/или пропускная способность ограничены. Пользователи могут загрузить содержимое, чтобы иметь возможность смотреть его в достаточно высоком разрешении для удобного просмотра.
...
Не нужно копировать содержимое. Вы можете просто использовать одну копию контента и один ContentKeyAuthorizationPolicy, но два отдельных ContentKeyАвторитивизацияPolicyOption's:

1. IContentKeyАвторизацияПолитикаОпция 1: использует постоянную лицензию, и ContentKeyАвторитивизацияPolicyA1 1, которая содержит претензию, такую как license_type и "Persistent"
2. IContentKeyАвторизацияПолитикаОпция 2: использует непостоянную лицензию, и ContentKeyАвторитивизацияPolicyОграничение 2, которое содержит претензию, такую как license_type и "Непостоянный"

Таким образом при поступлении запроса на лицензию из клиентского приложения или от запроса на лицензию нет никаких различий. Однако для разных конечных пользователей/устройств STS должен иметь бизнес-логику для выпуска различных токенов JWT, содержащих различные утверждения (один из двух вышеуказанных license_type). Значение утверждений в токене JWT будет использоваться для определения на основе службы лицензирования того, какой тип лицензии выдать: постоянный или непостоянный.
...
Для уровней безопасности Widevine в документации Google [Widevine DRM Architecture Overview](https://storage.googleapis.com/wvdocs/Widevine_DRM_Architecture_Overview.pdf) определяется три различных уровня безопасности. Однако в [документации по службам мультимедиа Azure в шаблоне лицензии Widevine](https://docs.microsoft.com/azure/media-services/media-services-widevine-license-template-overview) описаны пять различных уровней безопасности. Что такое связь или сопоставление между двумя различными наборами уровней безопасности?

### <a name="answer"></a>Ответ

В [обзоре архитектуры Widevine DRM](https://storage.googleapis.com/wvdocs/Widevine_DRM_Architecture_Overview.pdf)от Google он определяет следующие три уровня безопасности:
