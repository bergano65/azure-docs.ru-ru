...
Необязательный элемент. Задает идентификатор ресурса служб мультимедиа, который содержит изображение листа. Значением по умолчанию является NULL. 
...
| --- | --- | --- |
| Запуск |Запуск |Нет (переходное состояние) |
| Выполнение |Готово (нет запущенных программ)<br/>or<br/>Потоковая передача (запущена по крайней мере одна программа) |Да |
| Остановка |Остановка |Нет (переходное состояние) |
| Остановлена |Остановлена |Нет |
