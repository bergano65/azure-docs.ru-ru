...
В службах мультимедиа Azure (AMS) **канал** представляет конвейер для обработки динамического потокового контента. **Канал** получает живые потоки ввода одним из двух способов:
...
## <a name="common-live-streaming-scenario"></a><a id="scenario"></a>Общий сценарий потоковой передачи в реальном времени
...
1. Подключите видеокамеру к компьютеру. Запуск и настройка на месте живой кодер, который может вывести **один** битратный поток в одном из следующих протоколов: RTMP или Smooth Streaming. 
...
* Однобитный **битрат RTMP**
* **Однобитный фрагментированный MP4** (Гладкая потоковая передача)
...
* [Телестрим Wirecast](media-services-configure-wirecast-live-encoder.md)
...
Получить URL-адреса приема можно сразу после создания канала. Чтобы получить эти URL-адреса, канал не должен находиться в состоянии **запуска.** Чтобы начать передавать данные в канал, его нужно перевести в состояние **Выполняется** . Когда канал начнет принимать данные, вы можете просмотреть поток с помощью URL-адреса предварительного просмотра.

У вас есть возможность глотать фрагментированный MP4 (Гладкая потоковая передача) живой эфир через подключение TLS. Чтобы глотать по TLS, не забудьте обновить URL-адрес для глотания до HTTPS. В настоящее время AMS не поддерживает TLS с пользовательскими доменами.  
...
* **Остановлено**. Это начальное состояние канала после его создания. В этом состоянии можно изменять свойства канала, но потоковая передача запрещена.
...
* **Бег**. Канал может обрабатывать динамические потоки.
...
| --- | --- | --- |
| Запуск |Запуск |Нет (переходное состояние) |
| Запущен |Готово (нет запущенных программ)<br/>или диспетчер конфигурации служб<br/>Потоковая передача (запущена по крайней мере одна программа) |Да |
| Остановка |Остановка |Нет (переходное состояние) |
| Остановлена |Остановлена |нет |
...
## <a name="considerations"></a><a id="Considerations"></a>Особенности
...
* По умолчанию в учетную запись служб мультимедиа можно добавить не более 5 каналов. Это мягкая квота для всех новых учетных записей. Для получения дополнительной [информации см.](media-services-quotas-and-limitations.md)
* Во время работы канала или связанных с ним программ входной протокол изменить нельзя. Если вам нужны другие протоколы, создайте отдельный канал для каждого из них.
...
[Доставка событий потоковой передачи в прямом эфире с помощью медиа-служб Azure](media-services-overview.md)
