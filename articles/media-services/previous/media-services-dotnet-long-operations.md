...
description: Службы мультимедиа Azure предлагают AIS, которые отправляют запросы в службы мультимедиа для начала операций (например, создают, запускают, останавливают или удаляют канал), эти операции являются длительными. В этом разделе содержатся сведения об опросе долговременных операций.
