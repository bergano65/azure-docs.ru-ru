...
Для неопросных методов используется такая схема наименования: Send*имя_операции*Operation (например, SendCreateOperation). Методы Send*имя_операции*Operation возвращают объект **IOperation**. Полученный объект содержит сведения, которые можно использовать для отслеживания операции. Отправить*Имя_операции*методы OperationAsync возвращают **задачи\<IOperation >**.
