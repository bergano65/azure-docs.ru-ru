...
Во время запуска канала можно [настроить кодировщик](media-services-configure-fmle-live-encoder.md).
...
1. Перейдите к интерфейсу Flash Media Live Encoder (FMLE) на используемом компьютере.
   ...
   * Формат: Частота кадров H.264: 30,00
   * Размер входных данных: 1280 x 720
