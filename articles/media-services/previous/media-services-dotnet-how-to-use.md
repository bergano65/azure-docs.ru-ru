...
1. Создайте в Visual Studio консольное приложение C#. Введите **имя,** **местоположение**и **имя решения,** а затем нажмите OK.
2. Создайте решение.
3. Используйте пакет **NuGet**, чтобы установить и добавить **расширения пакета SDK служб мультимедиа Azure для .NET** (**windowsazure.mediaservices.extensions**). Установка этого пакета, а также устанавливает **Media Services .NET SDK** и добавляет все другие необходимые зависимости.
    ...
    1. В Solution Explorer нажмите правой кнопкой мыши на название проекта и выберите **пакеты Управления NuGet.**
...
5. Добавьте ссылку на сборку **System.Configuration** . Эта сборка содержит System.Configuration. Класс **ConfigurationManager,** используемый для доступа к файлам конфигурации (например, App.config).
