...
description: В этой статье приводится обзор схемы метаданных метаданных мультимедиа-службы Azure Media Services.
author: Juliako
manager: femila
editor: ''
...
Задание кодирования связано с входным ресурсом-контейнером (или ресурсами-контейнерами), в котором нужно выполнить эти задачи.  После выполнения задачи создается выходной ресурс-контейнер.  Выходный актив содержит видео, аудио, эскизы, манифест и т.д. Выходный актив также содержит файл с метаданными о вхотеном активе. Имя XML-файла метаданных имеет следующий формат: &lt;ИД_ресурса-контейнера&gt;_metadata.xml (например, 41114ad3-eb5e-4c57-8d92-5354e2b7d4a4_metadata.xml), где &lt;ИД_ресурса-контейнера&gt; — это значение AssetId для входного ресурса-контейнера.  
...
| name | Тип | Описание |
| --- | --- | --- |
| **Название**<br /><br /> Обязательно |**xs:string** |Имя файла ресурса-контейнера. |
...
| **Метаданные**<br /><br /> minOccurs="0" maxOccurs="unbounded" |[MetadataType](media-services-input-metadata-schema.md#MetadataType) |Метаданные файла ресурса-контейнера, представленные в виде строки "ключ —значение". Пример:<br /><br /> **&lt;Метадата ключ »язык" значение »eng" /&gt;** |
...
| **Кодек** |**xs:string** |Строка кодека видеодорожки. |
...
| **Каналы**<br /><br /> Обязательно |**xs:int** |Количество (0 или более) аудиоканалов. |
...
| **Ширина**<br /><br /> Обязательно |**xs:int** |Ширина закодированного видео в пикселях. |
| **Высота**<br /><br /> Обязательно |**xs:int** |Высота закодированного видео в пикселях. |
...
| **Кадров**<br /><br /> Обязательно |**xs:decimal** |Измеренная частота видеокадров в формате 3F. |
...
## <a name="metadatatype"></a><a name="MetadataType"></a>МетадатаТип
...
| name | Тип | Описание |
| --- | --- | --- |
| **Ключ**<br /><br /> Обязательно |**xs:string** |Ключ в паре "ключ — значение". |
| **value**<br /><br /> Обязательно |**xs:string** |Значение в паре "ключ — значение". |

## <a name="programtype"></a><a name="ProgramType"></a>ПрограммаТип
...
| **PmtPid**<br /><br /> Обязательно |**xs:int** |Таблицы сопоставления программ (PMT) содержат сведения о программах.  Дополнительные сведения см. в разделе [PMT](https://en.wikipedia.org/wiki/MPEG_transport_stream#PMT). |
| **PcrPid**<br /><br /> Обязательно |**xs: int** |Используется декодером. Дополнительные сведения см. в разделе [PCR](https://en.wikipedia.org/wiki/MPEG_transport_stream#PCR). |
...
| **Исходный текст**<br /><br /> Обязательно |**xs: int** |Установите значение 1, чтобы указать, что это оригинал презентации. |
| **Комментарий**<br /><br /> Обязательно |**xs:int** |Установите значение 1, чтобы указать, что эта дорожка содержит дикторский текст. |
...
| **Принудительный**<br /><br /> Обязательно |**xs:int** |Установите значение 1, чтобы указать, что это принудительная презентация. |
| **HearingImpaired**<br /><br /> Обязательно |**xs:int** |Установите этот атрибут до 1, чтобы указать этот трек для людей, которые имеют мало слышащих. |
