...
> [!NOTE] 
> Многие из передовых медиа-услуг v2 функции Media Encoder Standard в настоящее время недоступны в v3. Для получения дополнительной [информации](https://docs.microsoft.com/azure/media-services/latest/media-services-v2-vs-v3#feature-gaps-with-respect-to-v2-apis)см.
...
* Элементы Jpg, Png и BmpImage обладают строковыми атрибутами Start, Step и Range, которые можно интерпретировать следующим образом:
  ...
  * Отметка времени, если атрибуты имеют формат ЧЧ:ММ:СС, например "Start" : "00:01:00".
    ...
    Кроме того, Start также поддерживает специальный Macro: «Лучший», который пытается определить первый «интересный» кадр содержимого ПРИМЕЧАНИЕ: (Шаг и диапазон игнорируются, когда старт установлен на «Лучший»)
...
## <a name="trim-a-video-clipping"></a><a id="trim_video"></a>Монтаж видео (обрезка)
...
## <a name="create-an-overlay"></a><a id="overlay"></a>Создание наложения
...
При использовании .NET добавьте две приведенные ниже функции в пример .NET, определенный в [этом](media-services-custom-mes-presets-with-dotnet.md#encoding_with_dotnet) разделе. Функция **UploadMediaFilesFromFolder** загружает файлы из папки (например, BigBuckBunny.mp4 и Image001.png) и устанавливает файл mp4 в основной файл в активе. Функция **EncodeWithOverlay** использует настраиваемый файл предустановки, который был передан в нее (например, приведенную ниже предустановку), для создания задачи кодирования.
...
## <a name="insert-a-silent-audio-track-when-input-has-no-audio"></a><a id="silent_audio"></a>Вставка звуковой дорожки с тишиной, если входные данные не содержат звука
...
## <a name="disable-auto-de-interlacing"></a><a id="deinterlacing"></a>Отключение автоматического устранения чересстрочной развертки
Клиентам не нужно ничего делать, если им нравится, чтобы содержимое переплетения автоматически де-переплетено. Если автоматическое устранение чересстрочной развертки включено (по умолчанию), стандартный кодировщик мультимедиа не выполняет автоматическое обнаружение чересстрочных кадров и устраняет чересстрочную развертку кадров, которые помечены как чересстрочные.

Автоматическое удаление чересстрочной развертки можно отключить. Использовать этот параметр не рекомендуется.
...
## <a name="audio-only-presets"></a><a id="audio_only"></a>Предустановки только для звука
...
## <a name="concatenate-two-or-more-video-files"></a><a id="concatenate"></a>Объединение двух или нескольких видеофайлов
...
## <a name="crop-videos-with-media-encoder-standard"></a><a id="crop"></a>Обрезка видео с помощью стандартного кодировщика мультимедиа
Ознакомьтесь с разделом [Обрезка видео с помощью стандартного кодировщика мультимедиа](media-services-crop-video.md) .

## <a name="insert-a-video-track-when-input-has-no-video"></a><a id="no_video"></a>Вставка видеодорожки, если во входных данных нет видео
...
Предположим, что вы используете несколько бикатов кодирования предварительного набора, такие как ["H264 Multiple Bitrate 720p"](media-services-mes-preset-h264-multiple-bitrate-720p.md) для кодирования всего каталога ввода для потоковой передачи, который содержит сочетание видеофайлов и аудио-файлы. В этом случае, если входные данные не содержат видео, может потребоваться указать кодировщику принудительно вставлять монохромную видеодорожку только с самой низкой скоростью, а не с каждой выходной скоростью. Для этого необходимо указать флаг **InsertBlackIfNoVideoBottomLayerOnly**.
...
## <a name="rotate-a-video"></a><a id="rotate_video"></a>Поворот видео
