...
>  **Время композиции**. Время образец поставляется на стороне клиента, как определено в [[ISO/IEC-14496-12]](https://go.microsoft.com/fwlink/?LinkId=183695).
> 
>   **CENC**. Общее шифрование, как определено во втором выпуске спецификации [ISO/IEC 23001-7].
> 
>   **Время декодирования**. Время образец должен быть декодирован в клиенте, как определено в [[ISO/IEC 14496-12:2008]](https://go.microsoft.com/fwlink/?LinkId=183695).
...
> 
>   **Презентация**. Набор всех **потоков** и связанных метаданных, необходимых для воспроизведения одного фильма. **Запрос**. Сообщение HTTP, отправленные клиентом на сервер, как определено в [[RFC2616]](https://go.microsoft.com/fwlink/?LinkId=90372) **ответа:** Сообщение HTTP, отправленных сервером клиенту, как определено в [[RFC2616]](https://go.microsoft.com/fwlink/?LinkId=90372)
> 
>   **Выборка**. Наименьшая фундаментальная единица (например, кадр), в которой сохраняется и обрабатывается файл **мультимедиа**.
> 
>   **MAY (ВОЗМОЖНО), SHOULD (СЛЕДУЕТ), MUST (НЕОБХОДИМО), SHOULD NOT (НЕ СЛЕДУЕТ), MUST NOT (НЕДОПУСТИМО)**. Эти термины (прописными буквами) используются в том случае, как описано в разделе [[RFC2119]](https://go.microsoft.com/fwlink/?LinkId=90317) либо может все инструкции использования необязательное поведение, следует или не следует.
