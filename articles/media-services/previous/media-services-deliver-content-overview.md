...
Для получения дополнительной информации, см [Динамическая упаковка](media-services-dynamic-packaging-overview.md).
...
> [!NOTE]
> Вы также можете передавать содержимое через соединение TLS. Для этого убедитесь, что URL-адреса потоковой передачи начинаются с HTTPS. Обратите внимание, что в настоящее время AMS не поддерживает TLS с пользовательскими доменами.  
> 

Прямую трансляцию через TLS можно только в том случае, если потоковая конечная точка, с которой вы доставляете свой контент, была создана после 10 сентября 2014 года. Если URL-адреса потоковой передачи основаны на конечных точках потоковой передачи, созданных после 10 сентября 2014 года, то они содержат "streaming.mediaservices.windows.net". Потоковые URL-адреса, содержащие "origin.mediaservices.windows.net" (старый формат) не поддерживают TLS. Если ваш URL находится в старом формате, и вы хотите иметь возможность потоковой передачи по TLS, создайте новую конечную точку потоковой передачи. Используйте URL-адреса на основе новой конечной точки потоковой передачи содержимого по TLS.

## <a name="streaming-url-formats"></a><a id="URLs"/>Форматы URL-адресов потоковой передачи
