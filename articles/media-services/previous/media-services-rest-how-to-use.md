...
| Header | Тип | Значение |
| --- | --- | --- |
| Authorization |Носитель |Носитель – единственный принимаемый механизм авторизации. Значение должно содержать маркер доступа, предоставленный Azure Active Directory. |
| x-ms-version |Десятичное |2.17 (или последняя версия)|
| DataServiceVersion |Десятичное |3.0 |
| MaxDataServiceVersion |Десятичное |3.0 |
...
| client-request-id |Строковое |Определяемое вызывающим объектом значение, по которому можно идентифицировать данный запрос. Если это значение указано, оно будет включено в ответное сообщении в качестве способа сопоставления запроса. <p><p>**Важно!**<p>Значения должны превышать 2096 Б (2 КБ). |
...
| --- | --- | --- |
| request-id |Строковое |Уникальный идентификатор текущей операции, создаваемый службой. |
| client-request-id |Строковое |Идентификатор, указанный вызывающим объектом в исходном запросе, если он имеется. |
...
| ПОМЕСТИТЬ |Создает объект на основе предоставленных данных или отправляет команду. |
