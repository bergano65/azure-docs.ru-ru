...
С помощью служб мультимедиа, можно воспользоваться преимуществами [динамической упаковки](media-services-dynamic-packaging-overview.md), которая позволяет рассылать потоков в реальном времени в формате MPEG DASH, HLS, и Smooth Streaming форматирует из вклад веб-канала, которые передаются в службу. Ваши зрители могут воспроизводить трансляцию с помощью любых проигрывателей, поддерживающих HLS, DASH и Smooth Streaming. Проигрыватель мультимедиа Azure можно использовать в веб-приложения или мобильные приложения для доставки потока в любой из этих протоколов.
...
## <a name="streaming-endpoints-channels-programs"></a>Потоковая передача конечных точек, каналы, программы
