...
description: В этой статье приводится обзор потоковой передачи в реальном времени с помощью служб Microsoft Azure Media.
...
С помощью Media Services вы можете воспользоваться [преимуществами динамической упаковки,](media-services-dynamic-packaging-overview.md)которая позволяет транслировать ваши прямые трансляции в форматах MPEG DASH, HLS и Smooth Streaming из канала вклада, который отправляется в службу. Ваши зрители могут воспроизводить трансляцию с помощью любых проигрывателей, поддерживающих HLS, DASH и Smooth Streaming. Для доставки потока в любом из этих протоколов можно использовать Проигрыватель мультимедиа Azure в мобильном или веб-приложении.
...
## <a name="streaming-endpoints-channels-programs"></a>Потоковые конечные точки, каналы, программы
...
* Локальный динамический кодировщик передает контент в многоскоростном формате **RTMP** или **Smooth Streaming** (фрагментированный формат MP4) в канал, настроенный для **сквозной** доставки. **Сквозная** доставка осуществляется, когда переданные потоки проходят через **каналы** без дополнительной обработки. Вы можете использовать следующие динамические кодировщики, выдающие содержимое в формате потоковой передачи Smooth Streaming с несколькими скоростями: MediaExcel, Ateme, Imagine Communications, Envivio, Cisco и Elemental. Следующие живые кодеры выход RTMP: Telestream Wirecast, Haivision, Teradek транскодеров.  Динамический кодировщик может также передавать односкоростной поток в канал, для которого не включено кодирование в реальном времени, но это не рекомендуется. При получении запроса службы мультимедиа предоставляют потоки клиентам.

  > [!NOTE]
  > Метод сквозной доставки — наиболее экономичный способ осуществления потоковой передачи в режиме реального времени, если передается ряд событий за длительный период времени и у вас уже есть локальные кодировщики. Ознакомьтесь с сведениями [о ценах.](https://azure.microsoft.com/pricing/details/media-services/)
  > 
  > 
* Локальный динамический кодировщик передает односкоростной поток в канал, который может осуществлять кодирование в реальном времени с помощью служб мультимедиа, в одном из следующих форматов: RTMP или Smooth Streaming (Fragmented MP4) . Следующие живые кодеры с выходом RTMP, как известно, работают с каналами этого типа: Telestream Wirecast. Затем канал кодирует входящий односкоростной поток в реальном времени в многоскоростной (адаптивный) видеопоток. При получении запроса службы мультимедиа предоставляют потоки клиентам.
...
| Price |Откройте [страницу цен](https://azure.microsoft.com/pricing/details/media-services/) и выберите вкладку Live Video |Посмотреть [страницу ценообразования](https://azure.microsoft.com/pricing/details/media-services/) |
...
При создании канала можно указать разрешенные IP-адреса в одном из следующих форматов: ipV4-адрес с 4 номерами, диапазон адресов CIDR.

### <a name="program"></a>Программа
[Программа](https://docs.microsoft.com/rest/api/media/operations/program) позволяет контролировать публикацию и хранение сегментов в прямом эфире. Каналы управляют программами. Отношение между каналом и программой очень похоже на традиционные мультимедиа, где канал передает постоянный поток контента, а программа ограничена временным событием на этом канале.
Вы можете указать количество часов, которые вы хотите сохранить записанный контент для программы, установив свойство **ArchiveWindowLength.** Это значение может быть задано в диапазоне от 5 минут до 25 часов.
...
* **Остановлено**. Это начальное состояние канала после его создания (если только автостарт не был выбран на портале). В этом состоянии биллинга не происходит. В этом состоянии можно изменять свойства канала, но потоковая передача запрещена.
...
* **Бег**. Канал может обрабатывать динамические потоки. Теперь оплата начисляется. Необходимо остановить работу канала, чтобы избежать дальнейшего начисления оплаты.
...
| Запущен |Готово (нет запущенных программ)<br/>или диспетчер конфигурации служб<br/>Потоковая передача (запущена по крайней мере одна программа) |YES |
...
[Службы мультимедиа Azure Фрагментированные спецификации MP4 Live Ingest](../media-services-fmp4-live-ingest-overview.md)

[Работа с каналами, которые позволяют выполнять живую кодировку с помощью медиаслужб Azure](media-services-manage-live-encoder-enabled-channels.md)
...
[Концепции медиа-услуг](media-services-concepts.md)
