...
description: В этой статье приводятся общие сведения о потоковой трансляции с помощью Cлужбы мультимедиа Microsoft Azure.
...
Благодаря службам мультимедиа вы можете использовать [динамическую упаковку](media-services-dynamic-packaging-overview.md), которая позволяет транслировать динамические потоки в форматах MPEG, HLS и Smooth Streaming из веб-канала публикации, отправляемого в службу. Ваши зрители могут воспроизводить трансляцию с помощью любых проигрывателей, поддерживающих HLS, DASH и Smooth Streaming. Для доставки потока в любом из этих протоколов можно использовать Проигрыватель мультимедиа Azure в мобильном или веб-приложении.
...
## <a name="streaming-endpoints-channels-programs"></a>Конечные точки потоковой передачи, каналы, программы
...
* Локальный динамический кодировщик передает контент в многоскоростном формате **RTMP** или **Smooth Streaming** (фрагментированный формат MP4) в канал, настроенный для **сквозной** доставки. **Сквозная** доставка осуществляется, когда переданные потоки проходят через **каналы** без дополнительной обработки. Вы можете использовать следующие динамические кодировщики, выдающие содержимое в формате потоковой передачи Smooth Streaming с несколькими скоростями: MediaExcel, Ateme, Imagine Communications, Envivio, Cisco и Elemental. Следующие динамические кодировщики выводят RTMP: Wirecast, HaiVision, Teradek.  Динамический кодировщик может также передавать односкоростной поток в канал, для которого не включено кодирование в реальном времени, но это не рекомендуется. При получении запроса службы мультимедиа предоставляют потоки клиентам.
  ...
  > 
  > 
* Локальный динамический кодировщик передает односкоростной поток в канал, который может осуществлять кодирование в реальном времени с помощью служб мультимедиа, в одном из следующих форматов: RTMP или Smooth Streaming (Fragmented MP4) . Следующие динамические кодировщики с данными RTMP известны для работы с каналами этого типа: Wirecast в потоке. Затем канал кодирует входящий односкоростной поток в реальном времени в многоскоростной (адаптивный) видеопоток. При получении запроса службы мультимедиа предоставляют потоки клиентам.
...
| Цена |Откройте [страницу цен](https://azure.microsoft.com/pricing/details/media-services/) и выберите вкладку Live Video |См. [страницу с ценами](https://azure.microsoft.com/pricing/details/media-services/) |
...
![Схема, на которой показаны основные части платформы M для рабочего процесса "сквозной".](./media/media-services-live-streaming-workflow/media-services-live-streaming-current.png)
...
При создании канала можно указать допустимые IP-адреса в одном из следующих форматов: IpV4-адрес с 4 числами, диапазон адресов CIDR.

### <a name="program"></a>Программа
[Программа](/rest/api/media/operations/program) позволяет управлять публикацией и хранением сегментов в активном потоке. Каналы управляют программами. Отношение между каналом и программой очень похоже на традиционные мультимедиа, где канал передает постоянный поток контента, а программа ограничена временным событием на этом канале.
