...
| --- | --- | --- |
| Идет запуск |Идет запуск |Нет (переходное состояние) |
| Выполняется |Готово (нет запущенных программ)<br/>или диспетчер конфигурации служб<br/>Потоковая передача (запущена по крайней мере одна программа) |да |
| Идет остановка |Идет остановка |Нет (переходное состояние) |
