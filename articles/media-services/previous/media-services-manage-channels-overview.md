...
Благодаря службам мультимедиа вы можете использовать [динамическую упаковку](media-services-dynamic-packaging-overview.md), которая позволяет транслировать динамические потоки в форматах MPEG, HLS и Smooth Streaming из веб-канала публикации, отправляемого в службу. Ваши зрители могут воспроизводить трансляцию с помощью любых проигрывателей, поддерживающих HLS, DASH и Smooth Streaming. Вы можете использовать Проигрыватель мультимедиа Azure в своих веб-приложениях или мобильных устройствах для доставки потока в любой из этих протоколов.
...
| Максимальное время выполнения |24x7 |8 часов |
...
* **Остановлен**. Это начальное состояние канала после его создания (если автозапуск не выбран на портале). В этом состоянии выставление счетов не выполняется. В этом состоянии свойства Канала можно обновить, но потоковая передача невозможна.
* **Запуск**. Канал запускается. В этом состоянии начисление оплаты не происходит. В этом состоянии не разрешаются обновления и потоковая передача. При возникновении ошибки Канал возвращается в состояние "Остановлено".
* **Выполняется**. Канал способен обрабатывать потоки в реальном времени. Теперь оплата начисляется. Необходимо остановить работу канала, чтобы избежать дальнейшего начисления оплаты.
* **Остановка**. Канал прекращает свою работу. В этом переходном состоянии оплата не начисляется. В этом состоянии не разрешаются обновления и потоковая передача.
* **Удаление**. Выполняется удаление Канала. В этом переходном состоянии оплата не начисляется. В этом состоянии не разрешаются обновления и потоковая передача.

В следующей таблице показано, как состояния Канала соотносятся с режимом выставления счетов.

| Состояние Канала | Индикаторы пользовательского интерфейса портала | Это начисление оплаты? |
| --- | --- | --- |
| Запуск |Запуск |Нет (переходное состояние) |
| Выполнение |Готово (нет запущенных программ)<br/>или<br/>Потоковая передача (как минимум одна программа запущена) |ДА |
| Остановлено. |Остановлено. |Нет (переходное состояние) |
