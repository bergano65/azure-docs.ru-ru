...
* [Урок 1.](https://code.msdn.microsoft.com/Smooth-Streaming-Client-0bb1471f) Простой мультимедиапроигрыватель для Windows 8 с потоковой передачей Smooth Streaming. 
* [Урок 2.](https://code.msdn.microsoft.com/A-simple-Windows-8-Smooth-ee98f63a) Простой мультимедиапроигрыватель для Windows 8 с потоковой передачей Smooth Streaming и элементом управления «Ползунок». 
...
### <a name="to-create-a-windows-store-project"></a>Чтобы создать проект Windows Store
...
### <a name="to-add-a-reference-to-the-smooth-streaming-client-sdk"></a>Чтобы добавить ссылку на клиентский пакет Smooth Streaming SDK
...
### <a name="to-design-the-player-user-interface"></a>Для разработки пользовательского интерфейса проигрывателя
...
### <a name="to-register-the-smooth-streaming-byte-stream-handler-and-pass-the-propertyset"></a>Для регистрации обработчика байтового потока Smooth Streaming и передать набора свойств propertyset
...
### <a name="to-add-the-adaptive-source-manager-level-event-handler"></a>Для добавления обработчиков событий уровня диспетчера адаптивных источников
...
### <a name="to-add-adaptive-source-level-event-handlers"></a>Для добавления обработчиков событий уровня адаптивного источника
...
### <a name="to-add-media-element-event-handlers"></a>Для добавления обработчиков событий MediaElement
...
### <a name="to-add-slider-bar-related-code"></a>Чтобы добавить полосы ползунка кода, связанного с
...
5. Добавьте в конце класса **MainPage** следующий код:
   ...
   > [!NOTE]
   > CoreDispatcher используется для внесения изменений в поток пользовательского интерфейса из других потоков. В случае узкое место в потоке разработчик может выбрать использовать отправитель, предоставленный элементом пользовательского интерфейса, следует обновить.  Пример:
...
2. Изменение кода в файле
3. Компиляция и тестирование приложения

### <a name="to-modify-the-xaml-file"></a>Изменение файла XAML
...
## <a name="lesson-4-select-smooth-streaming-tracks"></a>Урок 4. Выбор дорожек Smooth Streaming
