...
description: Узнайте, как получить мультимедийное содержимое в службах мультимедиа, создав и отгружая ресурсы с помощью функции RESTFUL.
...
- Кроме того, дополнительные сведения см. в статье [использование аутентификации Azure AD для доступа к API служб мультимедиа с помощью функции RESTful](./media-services-rest-connect-with-aad.md) .
...
1. Добавьте в среду параметры подключения. 
    ...
    Некоторые переменные, которые являются частью среды **MediaServices** [environment](postman-environment.md) , необходимо задать вручную, прежде чем можно будет начать выполнение операций, определенных в [коллекции](postman-collection.md).
    ...
    ![Снимок экрана, на котором показан значок "шестеренки", выбранный сверху справа, и первые пять переменных, выбранных на вкладке "среды управления".](./media/media-services-rest-upload-files/postman-import-env.png)
...
3. Изучите файл **AzureMediaServices.postman_environment.json** . Здесь вы увидите, что почти все операции в коллекции выполняют скрипт test. Эти скрипты принимают некоторые значения, полученные в ответе, и устанавливают соответствующие переменные среды.
    ...
    ```
4. В левой части окна **POST** щелкните **1. Получение маркера проверки подлинности AAD**  ->  **Получение маркера Azure AD для субъекта-службы** .

    Часть URL-адреса заполняется переменной среды **азуреадстсендпоинт** (ранее в этом руководстве вы задаете значения переменных среды, которые поддерживают коллекцию).

    ![Снимок экрана, на котором отображается «1». Получение маркера D auth — получение маркера Azure A D для субъекта-службы ", выбранного в окне" Post ", и выбранной кнопки" Отправить ".](./media/media-services-rest-upload-files/postment-get-token.png)
...
1. Выберите **AccessPolicy**  ->  **создать AccessPolicy для отправки** .
2. Нажмите кнопку **Отправить** .

    ![Снимок экрана, на котором в меню слева отображается "AccessPolicy-Create AccessPolicy для отправки" и выбрана кнопка "Отправить".](./media/media-services-rest-upload-files/postman-access-policy.png)
...
[Ресурс](/rest/api/media/operations/asset) — это контейнер для нескольких типов или наборов объектов в службах мультимедиа, включая видео, аудио, изображения, коллекции эскизов, текстовые дорожки и файлы скрытых субтитров. Для создания ресурса в REST API нужно отправить запрос POST службам мультимедиа и разместить любую информацию о свойствах ресурса в тексте запроса.

При создании ресурса можно добавить некоторые свойства, в том числе **Options** . Вы можете указать один из следующих методов шифрования: **None** (без шифрования — используется по умолчанию), **StorageEncrypted** (для содержимого, которое зашифровано с помощью шифрования хранилища на стороне клиента), **CommonEncryptionProtected** или **EnvelopeEncryptionProtected** . Для каждого зашифрованного актива нужно настроить политику доставки. Дополнительные сведения см. в разделе [Настройка политик доставки ресурсов](media-services-rest-configure-asset-delivery-policy.md).
...
1. Выберите **активы**  ->  **создать ресурс** .
2. Нажмите кнопку **Отправить** .

    ![Снимок экрана, показывающий, что в меню "коллекции" выбран пункт "активы — создать ресурс" и выбрана кнопка "Отправить".](./media/media-services-rest-upload-files/postman-create-asset.png)
...
1. Выберите **Локатор**  ->  **создать локатор SAS** .
2. Нажмите кнопку **Отправить** .
    ...
    ![Снимок экрана, показывающий, что в меню "коллекции" выбран пункт "Локатор — Создание локатора s" и выбрана кнопка "Отправить".](./media/media-services-rest-upload-files/postman-create-sas-locator.png)
...
- [Upload blobs to Blob storage](/previous-versions/azure/storage/storage-use-azcopy#upload-blobs-to-blob-storage) (Отправка больших двоичных объектов в хранилище BLOB-объектов)
...
1. Нажмите клавишу **+** , чтобы создать новую вкладку запроса.
...
5. Нажмите кнопку **Отправить** .
    ...
    ![Снимок экрана, на котором отображается выбранная вкладка "(Уплоаду R L)".](./media/media-services-rest-upload-files/postman-upload-file.png)
...
Чтобы убедиться, что файл успешно отправлен, получите значение [AssetFile](/rest/api/media/operations/assetfile) и сравните его параметры (например, **ContentFileSize** ) с теми значениями, которые соответствуют свойствам нового ресурса. 
