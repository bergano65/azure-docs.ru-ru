...
В этом разделе приведены типичные сценарии доставки содержимого [динамически](#live_scenarios) или по запросу. Здесь также приведены сведения о доступности функций мультимедиа и служб в центрах обработки данных.
...
|ИМЯ|Статус|Центры обработки данных
|---|---|---|
|Стандартная|Общая доступность|Все|
|"Премиум"|Общая доступность|Все|

### <a name="live-encoding"></a>Кодирование в реальном времени

#### <a name="availability"></a>Доступность

Доступно во всех центрах обработки данных, за исключением следующих регионов: Германия, Южная Бразилия, Западная Индия, Южная Индия и Центральная Индия. 
...
|Имя обработчика мультимедиа|Статус|Центры обработки данных
|---|---|---|
|Media Encoder Standard|Общая доступность|Все|
|Media Encoder Premium Workflow|Общая доступность|Все, кроме региона "Китай"|
...
|Имя обработчика мультимедиа|Статус|Центры обработки данных
|---|---|---|
|Azure Media Face Detector|Предварительная версия|Все|
|Azure Media Hyperlapse|Предварительная версия|Все|
|Индексатор мультимедийных данных Azure|Общая доступность|Все|
|Azure Media Motion Detector|Предварительная версия|Все|
|Azure Media OCR|Предварительная версия|Все|
|Azure Media Redactor|Предварительная версия|Все|
|Azure Media Stabilizer|Предварительная версия|Все|
|Azure Media Video Thumbnails|Предварительная версия|Все|
|Azure Media Indexer 2|Предварительная версия|Все, кроме регионов "Китай" и "Федеральное правительство США"|
...
|Шифрование|Статус|Центры обработки данных|
|---|---|---| 
|Хранилище|Общая доступность|Все|
|Ключи AES-128|Общая доступность|Все|
|FairPlay|Общая доступность|Все|
|PlayReady|Общая доступность|Все|
|Widevine|Общая доступность|Все, кроме Германии, федерального правительства США и Китая.
...
|Имя типа зарезервированных единиц|Статус|Центры обработки данных
|---|---|---|
|S1|Общая доступность|Все|
|S2|Общая доступность|Все регионы, кроме "Южная Бразилия" и "Западная Индия"|
|S3|Общая доступность|Все регионы, кроме "Западная Индия"|
