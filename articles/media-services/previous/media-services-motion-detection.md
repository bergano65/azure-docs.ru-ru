...
> [!NOTE]
> Медиапроцессор **Azure Media Motion Detector** будет удален. Для даты выхода на [legacy components](legacy-components.md) пенсию см.
...
| name | Параметры | Описание | Значение по умолчанию |
| --- | --- | --- | --- |
...
| start |Метка времени начала в тактах. |
...
| скобки |Каждая скобка представляет один интервал в событии. Пустые скобки для этого интервала означают, что движение не обнаружено. |
| Расположения |В этом новом элементе в разделе событий указывается место, где возникло движение. Оно более точное, чем зона обнаружения. |
