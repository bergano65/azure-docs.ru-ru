...
ms.locfileid: "77913010"
---
# <a name="detect-motions-with-azure-media-analytics"></a>Обнаружение движения с помощью медиа-аналитики Azure

> [!NOTE]
> **Azure Media Motion Detectorный** обработчик мультимедиа будет снят с учета. Дату выбытия см. в разделе [устаревшие компоненты](legacy-components.md) .
...
| Имя | Параметры | Описание | По умолчанию |
| --- | --- | --- | --- |
...
| Элемент | Описание |
| --- | --- |
| version |Версия API видео. Текущая версия — 2 |
| timescale |Количество тактов в секунду видео. |
| смещение |Смещение времени для отметки времени в тактах. В API видео версии 1.0 это значение всегда равно 0. В будущих поддерживаемых сценариях это значение может измениться. |
...
| interval |Интервал каждой записи события в тактах. |
...
| положения |В этом новом элементе в разделе событий указывается место, где возникло движение. Оно более точное, чем зона обнаружения. |
