---
title: Выполняйте прямую трансляцию с помощью медиаслужб Azure для создания многобитных потоков с порталом Azure Документы Майкрософт
...
---
# <a name="perform-live-streaming-using-media-services-to-create-multi-bitrate-streams-with-azure-portal"></a>Выполняйте прямую трансляцию с помощью медиа-сервисов для создания многобитных потоков с порталом Azure  
...
1. Подключите видеокамеру к компьютеру. <br/>Чтобы установить оборудование, ознакомьтесь с записью блога [Simple and portable event video gear setup]( https://link.medium.com/KNTtiN6IeT) (Настройка простого портативного оборудования для сьемки).
    ...
    Если у вас нет доступа к камере, такие инструменты, как [Telestream Wirecast](media-services-configure-wirecast-live-encoder.md) могут быть использованы для создания живой ленты из видеофайла.
...
* Для работы с этим учебником требуется учетная запись Azure. Если ее нет, можно создать бесплатную пробную учетную запись всего за несколько минут. 
  Для получения подробной информации [см.](https://azure.microsoft.com/pricing/free-trial/)
...
![глоток URL-адресов](./media/media-services-portal-creating-live-encoder-enabled-channel/media-services-ingest-urls.png)
