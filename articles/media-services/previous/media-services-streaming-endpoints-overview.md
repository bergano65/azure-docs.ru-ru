...
description: В этой статье приводится обзор конечных точек потоковой передачи медиаслужб Azure Media Services.
...
Службы мультимедиа Azure добавили следующие свойства в структуру потоковой должной точки: **CdnProvider,** **CdnProfile,** **StreamingEndpointVersion.** Подробный обзор этих свойств см. [здесь](https://docs.microsoft.com/rest/api/media/operations/streamingendpoint). 
...
Начиная с января 2017 года релиз Media Services, у вас есть два типа потоковой передачи: **Стандартный** (предварительный просмотр) и **Премиум**. Эти типы реализованы в конечной точке потоковой передачи версии 2.0.


|Тип|Описание|
|--------|--------|  
|**Стандартный**|Конечная точка потоковой передачи по умолчанию является **стандартным** типом, может быть изменена на тип Premium путем настройки потоковых единиц.|
|**Премиум** |Этот вариант подходит для специализированного использования, предусматривающего более крупный масштаб или управление данными. Вы переходите в тип **Premium,** регулируя потоковые блоки.<br/>Выделенные конечные точки потоковой передачи живут в изолированной среде и не конкурируют за ресурсы.|
...
|Стандартная конечная точка потоковой передачи (предварительный просмотр)|2.0|0|Да|Платно|
...
Масштабирование|Автоматически масштабируется до целевой пропускной способности.|Дополнительные потоковые единицы.
...
<sup>1</sup> используется непосредственно на конечную точку потоковой передачи, когда CDN не включен в конечную точку.<br/>
