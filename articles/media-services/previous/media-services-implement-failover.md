...
2. Создайте папку **SupportFiles** на том же уровне, где находится файл проекта **HandleRedundancyForOnDemandStreaming.csproj**. В папке **SupportFiles** создайте папки **OutputFiles** и **MP4Files**. Скопируйте MP4-файл в папку **MP4Files**. (В этом примере используется файл **BigBuckBunny.mp4**.) 
3. С помощью **Nuget** добавьте ссылки на библиотеки DLL, связанные со службами мультимедиа. В **главном меню Visual Studio** выберите **Инструменты** > **Library Package Manager** (Диспетчер пакетов библиотеки) > **Консоль диспетчера пакетов**. В окне консоли введите **Install-Package windowsazure.mediaservices** и нажмите клавишу ВВОД.
4. Добавьте другие ссылки, необходимые для этого проекта: System.Configuration, System.Runtime.Serialization и System.Web.
...
6. Добавьте раздел **appSettings** в файл в формате **CONFIG** и обновите значения в соответствии со значениями ключа и имени служб мультимедиа и хранилища. 
...
3. Следующие определения методов вызываются из метода Main.
