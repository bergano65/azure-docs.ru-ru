...
description: В этой статье показано, как реализовать сценарий потоковой передачи свыше с помощью медиаслужб Azure.
...
---
# <a name="implement-failover-streaming-with-media-services-v2"></a>Реализация сбоя потокового с медиа-услуг v2
...
3. Используйте **NuGet** для добавления ссылок на DLL, связанные с медиа-сервисами. В **главном меню визуальной студии**, выберите **TOOLS** > **NuGet пакет менеджер** > **менеджер консоли.** В окне консоли введите **Install-Package windowsazure.mediaservices** и нажмите клавишу ВВОД.
...
Пример в этой теме показывает четкую потоковую передачу. Если вы хотите сделать защищенную потоковую передачу, есть несколько других вещей, которые необходимо настроить, вам нужно использовать тот же **AssetDeliveryPolicy**, тот же **ContentKeyАвторитифицированию или** url-адрес внешнего ключевого сервера, и вам нужно дублировать ключи содержимого с тем же идентификатором.

Для получения дополнительной информации о защите контента [см.](media-services-protect-with-aes128.md)

## <a name="see-also"></a>См. также

[Используйте Webhooks Azure для мониторинга уведомлений о работе служб мультимедиа](media-services-dotnet-check-job-progress-with-webhooks.md)
