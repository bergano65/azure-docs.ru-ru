...
Службы мультимедиа поддерживают динамическую и статическую упаковку. При использовании статической упаковки необходимо создать копию содержимого в каждом формате, который необходим вашим клиентам. С динамической упаковкой, все, что вам нужно, это создать актив, который содержит набор адаптивных битрат MP4 или гладкие файлы потоковой передачи. Затем на основе формата, указанного в манифесте или запросе фрагмента, сервер потокового воспроизведения по запросу обеспечит передачу содержимого по протоколу, выбранному пользователями. В результате вы сможете хранить и оплачивать файлы только в одном формате, а службы мультимедиа выполнят сборку и будут обслуживать соответствующий ответ на основе запросов клиента.
...
Если у вас есть адаптивный набор bitrate MP4, вы можете воспользоваться динамической упаковки. Динамическая упаковка позволяет доставлять потоковое содержимое с помощью указанного протокола без дальнейшей упаковки. Дополнительные сведения см. в статье [Динамическая упаковка](media-services-dynamic-packaging-overview.md).
...
> Media Services создает файл ключа и помещает его в контейнер активов. Скопируйте файл /asset-containerguid \*.key на сервер (или создайте собственный файл ключа) и затем удалите файл \*.key из контейнера ресурса.
> 
> 
