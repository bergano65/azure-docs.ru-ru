...
* **Нет** = **0** - Шифрование не используется. При использовании этого параметра содержимое не защищено при передаче и хранении.
    Используйте этот параметр, если MP4-файл планируется доставить с помощью поэтапного скачивания.
* **StorageEncrypted** = **1** - Шифрует ваш прозрачный контент локально с помощью aES-256 битового шифрования, а затем загружает его в Azure Storage, где он хранится в зашифрованном виде. Активы, защищенные с помощью шифрования хранилища, автоматически расшифровываются и помещаются в систему зашифрованных файлов до кодирования и при необходимости повторно кодируются до отправки в виде нового выходного актива. Шифрование хранилища чаще всего используется, если нужно защитить входные файлы мультимедиа высокого качества с помощью стойкого шифрования при хранении на диске.
* **CommonEncryptionProtected** = **2** - Используйте эту опцию, если вы загружаете контент, который уже зашифрован и защищен общим шифрованием или PlayReady DRM (например, плавная потоковая передача, защищенная PlayReady DRM).
* **EnvelopeEncryptionProtected** = **4** - Используйте эту опцию, если вы загружаете HLS в зашифрованном виде с помощью AES. Файлы должны быть закодированы и зашифрованы с помощью Transform Manager.
...
## <a name="encode-the-source-file-into-a-set-of-adaptive-bitrate-mp4-files"></a><a id="encode"></a>Закодируйте исходный файл в набор адаптивных файлов bitrate MP4
