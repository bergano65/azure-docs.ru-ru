...
Имеется возможность приема потока с фрагментированным MP4 (Smooth Streaming) через TLS-подключение. Для приема по протоколу TLS необходимо обновить URL-адрес приема на HTTPS. В настоящее время вы не можете принимать RTMP через TLS.
