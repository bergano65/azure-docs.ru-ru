...
description: 'В этой статье описывается настройка канала, который получает динамический поток с поддержкой нескольких скоростей от локального кодировщика. Далее этот поток можно доставлять в клиентские приложения для воспроизведения с помощью одной или нескольких конечных точек потоковой передачи, используя один из следующих протоколов адаптивной потоковой передачи с переменной скоростью: HLS, Smooth Streaming, DASH.'
...
ms.locfileid: "61463810"
---
# <a name="working-with-channels-that-receive-multi-bitrate-live-stream-from-on-premises-encoders"></a>Работа с каналами, получающими динамические Многоскоростные потоки данных от локальных кодировщиков
...
>
>Рекомендуется использовать на локальных кодировщиков, см. сведения о [рекомендуется в локальной среде кодировщиков](media-services-recommended-encoders.md).
...
* **Основной URL-адрес.** Он задает полный URL-адрес основной конечной точки приема RTMP канала.
...
* **Остановлен**. Это начальное состояние канала после его создания. В этом состоянии свойства канала можно обновить, но потоковая передача невозможна.
* **Запуск**. Канал запускается. В этом состоянии не разрешаются обновления и потоковая передача. Если возникает ошибка, канал возвращается в состояние **Остановлен**.
* **Running**. Канал может обрабатывать динамические потоки.
* **Остановка**. Канал останавливается. В этом состоянии не разрешаются обновления и потоковая передача.
* **Удаление**. Канал удаляется. В этом состоянии не разрешаются обновления и потоковая передача.
...
[Рекомендуется использовать на локальных кодировщиков](media-services-recommended-encoders.md)
