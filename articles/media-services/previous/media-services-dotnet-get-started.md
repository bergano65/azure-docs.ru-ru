...
В службах мультимедиа цифровые файлы отправляются (или принимаются) в актив. Сущность **Активов** может содержать видео, аудио, изображения, коллекции эскизов, текстовые треки и закрытые файлы заголовков (и метаданные об этих файлах).  После загрузки файлов содержимое надежно хранится в облаке для дальнейшей обработки и потоковой передачи. Файлы в ресурсе называются **файлами ресурса**.
...
Метод **CreateFromFile** принимает **AssetCreationOptions**, который позволяет указать один из следующих вариантов создания активов:
...
«Потоковая потоковая передача имени-медиа-услуг имя учетной записи.streaming.mediaservices.windows.net/»locator ID/»filename.ism/Manifest **(формат-mpd-time-csf)**
