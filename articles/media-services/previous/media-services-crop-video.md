...
description: Обрезка заключается в выборе прямоугольного окна в пределах видеокадра и кодировании в нем пикселей. В этой статье показано, как обрезать видео с Media Encoder Standard.
...
Обрезка должна выполняться вручную. Вам нужно будет загрузить входное видео в подходящий инструмент редактирования, который позволяет выбрать кадры, представляющие интерес, положение курсора, чтобы определить смещения для обрезки прямоугольник, чтобы определить кодирование предустановки, которая настроена для этого конкретного видео, и т.д. Эта функция не предназначена для включения таких вещей, как: автоматическое обнаружение и удаление черных границ почтового ящика/столббокса в вашем входном видео.
