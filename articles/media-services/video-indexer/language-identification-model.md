...
Индексатор видео расшифровывает видео в соответствии с наиболее вероятным языком, если достоверность этого языка `> 0.6`. Если язык не может быть идентифицирован с уверенностью, предполагается, что речь составляет английский язык. 

Модель с главным языком доступна в JSON Insights как атрибут `sourceLanguage` (в разделе root/videos/Insights). Соответствующий показатель достоверности также доступен в атрибуте `sourceLanguageConfidence`.
...
* Если индексатор видео не может опознать язык с высокой достоверностью (`>0.6`), то резервный язык будет английским.
