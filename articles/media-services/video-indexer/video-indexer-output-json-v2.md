...
Если при вызове API **Get Video Index** отображается состояние ответа "OK", вы получите подробные выходные данные в формате JSON в виде содержимого ответа. В содержимом JSON представлены данные по выбранным аналитическим сведениям из видео. Аналитические сведения включают в себя: записи, Окрс, лица, темы, блоки и т. д. Каждый тип Insights содержит экземпляры диапазонов времени, которые показывают, когда аналитика отображается в видео. 
...
Экземпляры|Список диапазонов времени этого блока.|
...
|Экземпляры|Список диапазонов времени, в которых появилась эта строка. Если экземпляр является расшифровкой, будет отображаться только один экземпляр.|
...
|Язык|Язык OCR.|
|Экземпляры|Список диапазонов времени, где появилось это OCR (одно и то же OCR может появляться несколько раз).|
...
|Экземпляры|Список диапазонов времени, где появилось это ключевое слово (ключевое слово может появляться несколько раз).|
...
|Экземпляры|Это примеры того, где лицо появилось в заданном интервале времени. У каждого экземпляра также есть thumbnailsId. |
...
|Экземпляры|Список диапазонов времени, где появилась эта метка (метка может появляться несколько раз). У каждого экземпляра есть поле достоверности. |
...
|Экземпляры|Список диапазонов времени этой сцены (сцена может иметь только 1 экземпляр).|
...
|Экземпляры|Список временных диапазонов этого снимка (снимок может иметь только один экземпляр).|
...
|Экземпляры|Список диапазонов времени, в которых упоминается эта торговая марка. Каждый экземпляр содержит объект brandType, который указывает, появилась ли торговая марка при расшифровке или распознавании текста.|
...
|Экземпляры|Список диапазонов времени, в которых появились эти аудиоэффекты.|
...
|Экземпляры|Список диапазонов времени, в которых появилась эта тональность.|
...
|Экземпляры|Список диапазонов времени, где применяется модерация контента.|
...
|Экземпляры|Список диапазонов времени, когда проявлялась эта эмоция.|
...
|Экземпляры |В настоящее время Индексатор видео не выполняет индексацию темы по интервалам времени, поэтому в качестве интервала используется все видео.|
