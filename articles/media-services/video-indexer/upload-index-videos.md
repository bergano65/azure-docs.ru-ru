...
- URL-адрес должен указывать на допустимый файл мультимедиа, а не на веб-страницу, например ссылку на страницу `www.youtube.com`.
- Вы можете отправить до 60 фильмов в минуту.
...
Список форматов файлов, которые можно использовать с индексатором видео, см. в статье [контейнеры входных данных и форматы файлов](../latest/media-encoder-standard-formats.md#input-containerfile-formats) .

## <a name="a-idwebsiteupload-and-index-a-video-using-the-video-indexer-website"></a><a id="website"/>отправки и индексирования видео с помощью веб-сайта индексатора видео
...
2. Чтобы отправить видео, нажмите кнопку **Отправить** или щелкните ссылку.
    ...
    ![Отправка](./media/video-indexer-get-started/video-indexer-upload.png)
...
## <a name="a-idapisupload-and-index-with-api"></a>Передача и индексирование <a id="apis"/>с помощью API
...
- `VideoOnly`-индексировать и извлекать аналитические данные с использованием только видео (без звука)
...
* Ключ API — ключ API — это ваш личный ключ подписки управления API, который позволит получить маркер доступа для выполнения операций с учетной записью индексатора видео. 
    ...
    * Вход
    * Последовательно выберите **продукты** -> **авторизация** -> **авторизация** .
...
|Код состояния|ErrorType (в тексте ответа)|Description|
|---|---|---|
