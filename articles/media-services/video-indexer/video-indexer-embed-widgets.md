...
|Name|Определение|Описание|
|---|---|---|
|`accessToken`<sup>*</sup>|Строка|Предоставляет доступ к видео, которые используются только в учетной записи, используемой для внедрения мини-приложения.<br> Мини-приложению редактора требуется параметр `accessToken`.|
|`language`|Код языка|Управляет языком проигрывателя. По умолчанию используется значение `en-US`.<br/>Пример: `language=de-DE`.|
...
Заголовок окна iframe можно настроить, указав `&title=<YourTitle>` в URL-адресе для iframe. (Он настраивает HTML-\<title > значение).
