...
В этой статье показано, как внедрять графические элементы индексатора видео в приложения. Индексатор видео поддерживает внедрение трех типов мини-приложений в приложение: Сведения об *Автоприятии*, *проигрывателе*и *редакторе*. 
...
|`language`|Код языка|Управляет языком проигрывателя. Значение по умолчанию — `en-US`.<br/>Пример: `language=de-DE`.|
...
|Название|Определение|Описание|
|---|---|---|
|`accessToken`<sup>*</sup>|Строковое|Предоставляет доступ к видео, которые используются только в учетной записи, используемой для внедрения мини-приложения.<br> Для мини-приложения редактора `accessToken` требуется параметр.|
|`language`|Код языка|Управляет языком проигрывателя. Значение по умолчанию — `en-US`.<br/>Пример: `language=de-DE`.|
|`locale`|Короткий код языка|Управляет языком Insights. Значение по умолчанию — `en`.<br/>Пример: `language=de`.|
...
Заголовок окна iframe можно настроить, указав `&title=<YourTitle>` в URL-адресе для iframe. (Он настраивает заголовок HTML \<> значение).
...
Если вы хотите внедрить с другим языком, можно добавить `&captions=< Language | ”all” | “false” >` URL-адрес для внедрения проигрывателя. Если требуется использовать субтитры во всех заголовках доступных языков, используйте значение `all`. Чтобы субтитры отображались по умолчанию, передайте `&showCaptions=true`.

URL-адрес внедрения будет выглядеть так: 

`https://www.videoindexer.ai/embed/player/<accountId>/<videoId>/?captions=italian`

Если вы хотите отключить субтитры, можно передать `captions` значение параметра как. `false`
