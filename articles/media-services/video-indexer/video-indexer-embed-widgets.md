...
ms.locfileid: "68663744"
---
# <a name="embed-video-indexer-widgets-into-your-applications"></a>Внедрение мини-приложений Индексатора видео в приложения

В этой статье показано, как внедрить мини-приложения Индексатора видео в приложение. Индексатор видео поддерживает внедрение трех типов мини-приложений в приложение:Сведения об автоприятии, проигрывателе и **редакторе**. 
...
|---|---|---|
|`t`|Секунд с начала|Позволяет проигрывателю воспроизводить видео с определенной точки во времени.<br/>Пример: `t=60`.|
...
|`type`||Активирует обложку аудиопроигрывателя (видеочасть удаляется).<br/>Пример: `type=audio`.|
...
|`language`|Код языка|Язык элементов управления проигрывателя (значение по умолчанию — en-US)<br/>Пример: `language=de-DE`.|

### <a name="editor-widget"></a>Мини-приложение редактора 

Мини-приложение **редактора** позволяет создавать новые проекты и управлять аналитикой видео.

|Имя|Определение|Описание|
|---|---|---|
|`accessToken`<sup>*</sup>|Строковое|`accessToken` Параметр является обязательным при использовании мини-приложения редактора.<br/>Маркер доступа предоставляет доступ к видео, которые используются только в учетной записи, используемой для внедрения мини-приложения. |
|`language`|Код языка|Язык элементов управления проигрывателя (значение по умолчанию — en-US)<br/>Пример: `language=de-DE`.|
|`locale`|Короткий код языка|Управляет языком Insights. Значение по умолчанию — `en`. Например, `language=de`.|

<sup>*</sup>Владелец следует предоставлять `accessToken` с осторожностью. 
...
3. Нажмите кнопку "Внедрить", расположенную под видео.
    ...
    При выборе мини-приложения («автоаналитический», **проигрывателя**или **редактора**) создается внедренный код, который можно вставить в приложение.
...
### <a name="embed-widgets-in-your-application--blog-recommended"></a>Внедрение мини-приложений в приложение или блог (рекомендуется) 
...
При внедрении индексатора видео с помощью собственного [проигрыватель мультимедиа Azure](https://aka.ms/azuremediaplayer)можно использовать метод **жетвттурл** для получения скрытых субтитров (субтитров). Вы также можете вызвать метод JavaScript **getSubtitlesUrl** из подключаемого модуля AMP Индексатора видео (как показано выше). 
