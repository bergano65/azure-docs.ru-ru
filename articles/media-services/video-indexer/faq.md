...
description: В этой статье приводятся ответы на часто задаваемые вопросы о индексе видеоиндекса медиасервисов Azure.
...
* Определите бренды (например: Microsoft) из звуковых дорожек и текста на экране в видео.
...
### <a name="can-i-customize-video-indexer-models-for-my-specific-use-case"></a>Могу ли я настроить модели Video Indexer для моего конкретного случая использования? 

Да. В Video Indexer вы можете настроить некоторые из доступных моделей, чтобы лучше соответствовать вашим потребностям. 
...
### <a name="what-is-the-privacy-policy-for-video-indexer"></a>Какова политика конфиденциальности для Video Indexer?
...
### <a name="what-is-the-difference-between-private-and-public-videos"></a>В чем разница между частными и публичными видео? 

Когда видео загружается в Video Indexer, вы можете выбрать один из двух настроек конфиденциальности: частные и публичные. Общедоступные видео доступны для всех, включая анонимных и неопознанных пользователей. Частные из них ограничены только для членов учетной записи. 

### <a name="i-tried-to-upload-a-video-as-public-and-it-was-flagged-for-inappropriate-or-offensive-content-what-does-that-mean"></a>Я попытался загрузить видео в качестве общественности, и он был помечен за неуместный или оскорбительный контент, что это значит? 

При загрузке видео на Video Indexer алгоритмы и модели проводят автоматический анализ содержимого, чтобы убедиться, что небудет неприемлемого контента будет представлен публично. Если будет установлено, что видео является подозрительным, поскольку содержит явное содержание, его невозможно установить в качестве общедоступной. Тем не менее, пользователи учетной записи могут получить к нему доступ как к частному видео (просмотреть его, скачать информацию и извлеченные артефакты, а также выполнять другие операции, доступные для членов учетной записи).   

Для того, чтобы установить видео для публичного доступа, вы можете либо: 

* Создайте свой собственный слой интерфейса (например, приложение или веб-сайт) и используйте его для взаимодействия с службой Video Indexer. Таким образом, видео остается конфиденциальными на нашем портале, и ваши пользователи могут взаимодействовать с ним через ваш интерфейс. Например, вы все еще можете получить информацию или разрешить просмотр видео в собственном интерфейсе. 
* Запросить человеческий обзор содержимого, что приведет к снятию ограничения, если содержание не является явным. 

    Этот параметр можно изучить, если веб-сайт Video Indexer используется непосредственно пользователями в качестве слоя интерфейса, а также для публичного (непреддентированного) просмотра. 
...
### <a name="what-are-the-login-options-to-video-indexer-developer-portal"></a>Каковы варианты входа на портал Video Indexer Developer?

Вы можете войти в систему с помощью Azure AD, учетной записи Майкрософт, учетной записи Google или Facebook. 

После регистрации учетной записи электронной почты с помощью поставщика идентификационных данных вы не сможете использовать эту учетную запись электронной почты с другим поставщиком идентификационных данных.
...
### <a name="when-am-i-billed-for-using-video-indexer"></a>Когда мне выставляют счет за использование Video Indexer?

При отправке видео для индексации пользователь определяет индексирование как анализ видео, аудиоанализ или и то, и другое. Это позволит определить, какие SkUs будут взиматься. При наличии критической ошибки уровня во время обработки код ошибки будет возвращен в качестве ответа. В таком случае биллинга не происходит.  Критическая ошибка может быть вызвана ошибкой в нашем коде или критическим сбоем во внутренней зависимости службы. Ошибки, такие как неправильная идентификация или извлечение информации, не считаются критическими, и ответ возвращается. В любом случае, когда возвращается действительный (неошибодный код) ответ, происходит выставление счетов.
