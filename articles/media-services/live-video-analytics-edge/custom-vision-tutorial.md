...
1. Проверьте, установлен ли Docker; если нет — установите [Docker](https://docs.docker.com/get-docker/) для рабочего стола Windows.
1. Распакуйте загруженный файл в выбранном расположении. В командной строке перейдите к каталогу распакованных папок.
    ...
    1. `docker build -t cvtruck` 
        ...
        С помощью этой команды можно загрузить набор пакетов, создать образ Docker и пометить его как `cvtruck:latest`. 
    
        > [!NOTE]
        > При успешном выполнении вы увидите `- Successfully built <docker image id> and Successfully tagged cvtruck:latest.`. Если команда сборки завершилась неудачно, повторите попытку, так как иногда пакеты зависимостей не загружаются с первого раза.
    1. `docker  image ls`

        С помощью этой команды можно проверить, находится ли новый образ в локальном реестре.
    1. `docker run -p 127.0.0.1:80:80 -d cvtruck`
    
        С помощью этой команды можно опубликовать открытый порт Docker (80) на порте локального компьютера (80).
...
1. Щелкните правой кнопкой мыши файл "src/edge/deployment.customvision.template.json" и выберите **Generate IoT Edge Deployment Manifest** (Создание манифеста развертывания IoT Edge).
    ...
    > [!div class="mx-imgBorder"]
    > :::image type="content" source="./media/custom-vision-tutorial/deployment-template-json.png" alt-text="Создание манифеста развертывания IoT Edge":::
  
    В результате в папке src/edge/config должен быть создан файл манифеста с именем "deployment.customvision.amd64.json".
1. Откройте файл "src/edge/deployment.customvision.template.json" и найдите JSON-блок "registryCredentials". В этом блоке вы увидите адрес реестра контейнера Azure, а также имя пользователя и пароль.
1. Отправьте локальный контейнер Пользовательского визуального распознавания в реестр контейнеров Azure, выполнив в командной строке следующую команду.
    ...
    1. Пометьте изображение с помощью:<br/>`docker tag cvtruck   <address>/cvtruck`
    1. Отправьте изображение с помощью:<br/>`docker push <address>/cvtruck`

        В случае успешного выполнения вы должны увидеть сообщение "Отправлено" в командной строке, а также SHA для изображения. 
    1. Вы также можете узнать результат, проверив свой реестр контейнеров Azure на портале Azure. Здесь отображается имя репозитория вместе с тегом. 
1. Задайте строку подключения к Центру Интернета вещей. Для этого щелкните значок "Дополнительные действия" рядом с областью Центра Интернета вещей Azure в левом нижнем углу. Строку можно скопировать из файла appsettings.json. (Есть еще один рекомендуемый подход, позволяющий обеспечивать правильность настройки Центра Интернета вещей в VSCode. Он предполагает использование [команды выбора Центра Интернета вещей](https://github.com/Microsoft/vscode-azure-iot-toolkit/wiki/Select-IoT-Hub)).

    > [!div class="mx-imgBorder"]
    > :::image type="content" source="./media/custom-vision-tutorial/connection-string.png" alt-text="Строка соединения":::
1. Затем щелкните правой кнопкой мыши файл "src/edge/config/deployment.customvision.amd64.json" и выберите **Create Deployment for Single Device** (Создать развертывание для одного устройства). 

    > [!div class="mx-imgBorder"]
    > :::image type="content" source="./media/custom-vision-tutorial/deployment-amd64-json.png" alt-text="Создание развертывания для одного устройства":::
...
> [!div class="mx-imgBorder"]
> :::image type="content" source="./media/custom-vision-tutorial/start-monitoring.png" alt-text="Запуск мониторинга встроенной конечной точки события":::
...
1. Код файла operations.json начнет выполнение операций с вызова прямых методов GraphTopologyList и GraphInstanceList. Если вы очистили ресурсы после работы с предыдущими краткими руководствами, тогда этот процесс возвратит пустые списки, а затем приостановится. Чтобы продолжить, нажмите клавишу ВВОД.
    
   В окне ТЕРМИНАЛ показывается следующий набор вызовов прямых методов:
    
   * Вызов метода GraphTopologySet, который использует предыдущее значение topologyUrl.
   * Вызов метода GraphInstanceSet, который использует приведенный ниже код:
