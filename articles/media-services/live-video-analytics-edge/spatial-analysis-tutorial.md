...
1. Убедитесь, что вы настроили границы тома, поскольку это влияет на работу симулятора RTSP. Дополнительные сведения см. в разделе [Настройка подключений тома Docker](deploy-azure-stack-edge-how-to.md#optional-setup-docker-volume-mounts).
    ...
    1. [Подключитесь к общему ресурсу SMB](https://docs.microsoft.com/azure/databox-online/azure-stack-edge-deploy-add-shares#connect-to-an-smb-share) и скопируйте [пример видеофайла bulldozer.mkv](https://lvamedia.blob.core.windows.net/public/bulldozer.mkv) в локальную общую папку.
