...
* Аналитика — события, создаваемые в ходе анализа видео.
   ...
   * Примеры:
   ...
   ```
События, порожденные модулем, отправляются в [центр IOT Edge](https://docs.microsoft.com/azure/iot-edge/iot-edge-runtime#iot-edge-hub), откуда их можно направлять в другие назначения. 
...
Вы можете управлять [маршрутизацией всех указанных выше событий](https://docs.microsoft.com/azure/iot-edge/module-composition#declare-routes) с помощью требуемого свойства $edgeHub модуля двойника (в манифесте развертывания):
...
В приведенном выше примере Лваедже — это имя для аналитики в реальном времени в модуле IoT Edge, а правило маршрутизации соответствует схеме, определенной в [Declare Routes](https://docs.microsoft.com/azure/iot-edge/module-composition#declare-routes).
...
События происходят на пограничном устройстве, и их можно использовать на границе или в облаке. События, создаваемые функцией Live Video Analytics на IoT Edge, соответствуют [шаблону обмена сообщениями с потоковой передачей](https://docs.microsoft.com/azure/iot-hub/iot-hub-devguide-messages-construct) , установленному центром Интернета вещей Azure, со свойствами системы, свойствами приложения и телом.
...
|Раздел| аппликатионпроперти |string|    Azure Resource Manager путь к учетной записи служб мультимедиа.|
|subject|   аппликатионпроперти |string|    Вложенный путь к сущности, в которой создается событие.|
|eventTime| аппликатионпроперти|    string| Время создания события.|
|eventType| аппликатионпроперти |string|    Идентификатор типа события (см. ниже).|
|текст|текст  |объект|    Данные определенного события.|
|dataVersion    |аппликатионпроперти|   string  |{Major}. Дополнительный|
...
Как и в случае с другими IoT Edgeными модулями, можно также [изучить журналы контейнеров](https://docs.microsoft.com/azure/iot-edge/troubleshoot#check-container-logs-for-issues) на пограничном устройстве. Сведения, которые записываются в журналы, можно контролировать с помощью [следующих свойств модуля двойника](module-twin-configuration-schema.md) :
...
Сначала необходимо [связать хранилище модулей с хранилищем устройств](https://docs.microsoft.com/azure/iot-edge/how-to-access-host-storage-from-module#link-module-storage-to-device-storage) через креатеоптионс. Если изучить [шаблон манифеста развертывания](https://github.com/Azure-Samples/live-video-analytics-iot-edge-csharp/blob/master/src/edge/deployment.template.json) с помощью быстрого запуска, вы увидите следующее:
