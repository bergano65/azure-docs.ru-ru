---
title: Потоковая передача конечных точек (источник) в службах мультимедиа Azure | Документация Майкрософт
description: В службах мультимедиа Azure конечная точка потоковой передачи (источник) представляет динамической упаковки и потоковой передачи службы, которая может доставить содержимое непосредственно в приложение проигрывателя клиента или в сети доставки содержимого (CDN) для дальнейшего распространения.
...
ms.locfileid: "67296133"
---
# <a name="streaming-endpoints"></a>Конечные точки потоковой передачи 

В службах мультимедиа Microsoft Azure [конечной точки потоковой передачи](https://docs.microsoft.com/rest/api/media/streamingendpoints) представляет динамический (just-in-time) упаковки и происхождение службу, которая могла предоставлять в реальном времени и по требованию содержимое непосредственно в клиентское приложение проигрывателя, с помощью одного из распространенных протоколов потоковой передачи мультимедиа (HLS или DASH). Кроме того **конечной точки потоковой передачи** предоставляет возможности динамического шифрования (just-in-time) для лицензий DRM в отрасли.

Когда вы создаете учетную запись Служб мультимедиа, конечная точка потоковой передачи **по умолчанию** создается в остановленном состоянии. Удалить **стандартную** конечную точку потоковой передачи нельзя. Можно создать дополнительные конечные точки потоковой передачи в учетной записи (см. в разделе [квоты и ограничения](limits-quotas-constraints.md)). 
...
>  
> Плата взимается, только когда конечной точки потоковой передачи находится в состоянии выполнения.
...
Есть два типа **конечных точек потоковой передачи**: **Стандартный** (Предварительная версия) и **уровня "премиум"** . Тип определяется по количеству единиц масштабирования (`scaleUnits`), выделенных для конечной точки потоковой передачи. 

В таблице ниже описаны типы.  

|type|Единицы масштабирования|Описание|
|--------|--------|--------|  
|**Стандартный**|0|По умолчанию используется конечная точка потоковой передачи **стандартный** введите, можно изменить в тип "премиум" с помощью `scaleUnits`.|
|**Премиальный**|>0|**"Премиум"** конечными точками потоковой передачи не подходят для более сложных задач благодаря выделенной и масштабируемой пропускной способности. Перемещении **уровня "премиум"** тип перемещая `scaleUnits` (единиц потоковой передачи). `scaleUnits` предоставляют определенную выходную пропускную способность, которую можно приобрести с шагом приращения 200 Мбит/с. При использовании типа **Премиум** каждая включенная единица обеспечивает приложению дополнительную пропускную способность. |
...
Масштабирование|Автоматически масштабируется до целевой пропускной способности.|Дополнительные единицы su
