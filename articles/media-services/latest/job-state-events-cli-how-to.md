...
"Сетка событий Azure" — это служба обработки событий для облака. Эта служба использует [подписки на события](../../event-grid/concepts.md#event-subscriptions) для направления сообщений событий подписчикам с подписчиками. События Служб мультимедиа содержат все сведения, необходимые для реагирования на изменения в данных. Событие Служб мультимедиа можно определить, так как свойство eventType начинается с Microsoft.Media. Дополнительные сведения см. в статье [Схемы службы "Сетка событий Azure" для событий Служб мультимедиа](media-services-event-schemas.md).
...
Вы подписываетесь на статью, чтобы сообщить Event Grid, какие события вы хотите отслеживать. Следующий пример подписывается на созданную учетную запись Media Services и передает URL-адрес с веб-сайта, созданного вами в качестве конечной точки для уведомления о событии. 
