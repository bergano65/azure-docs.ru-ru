...
* Запоминающийся URL-адрес
    ...
    Чтобы указать этот `vanityUrl` режим, вы `true` установите `false`на время создания (по умолчанию это ). Вы также должны пройти свой собственный токен доступа ()`LiveEventInput.accessToken`во время создания. Вы указываете значение маркера, чтобы избежать случайного маркера в URL. Токен доступа должен быть действительной строкой GUID (с дефисом или без нее). После установки режима он не может быть обновлен.
