...
* Запоминающийся URL-адрес
    ...
    |CLI|[--именного-URL-адрес](https://docs.microsoft.com/cli/azure/ams/live-event?view=azure-cli-latest#az-ams-live-event-create)|[--маркер доступа](https://docs.microsoft.com/cli/azure/ams/live-event?view=azure-cli-latest#optional-parameters)|
...
После запуска событие прямой трансляции с получения участия в веб-канала его конечную точку предварительного просмотра можно использовать для предварительного просмотра и проверки, что вы получили обновляющегося потока перед дальнейшей публикацией. После проверки того, что поток предварительного просмотра является хорошим, чтобы сделать обновляющегося потока, доступных для доставки через одну или несколько (предварительно созданных) конечные точки потоковой передачи можно использовать событие прямой трансляции. Для этого создайте новый [Live вывода](https://docs.microsoft.com/rest/api/media/liveoutputs) на событие прямой трансляции. 
...
Связь между **событие прямой трансляции** и его **выходные данные Live** похоже на традиционное телевидение вещания переполняться канал (интерактивное событие) представляет собой постоянный поток видео и записи (Live Выходные данные), ограничиваются за определенный период времени (например, вечером новости от 18:30:00 до 7:00). Можно записать телевизора, с помощью цифровой записи видео (DVR) — эквивалентную возможность в реальном времени события управляется с помощью **archiveWindowLength** свойство. Это длительность временного диапазона ISO-8601 (например, PTHH:MM:SS), задающая объем DVR. Для нее можно задать значение от 3 минут до 25 часов.

Live выходной объект — как средство записи ленту, который будет перехватывать и записи обновляющегося потока в ресурс-контейнер в учетной записи служб мультимедиа. Записанное содержимое сохраняется в учетной записи хранения Azure, подключенной к вашей учетной записи, в контейнере, определенном ресурсом Asset. Выходные данные Live также позволяет контролировать некоторые свойства исходящего обновляющегося потока, например, какая часть потока сохраняется в записи архива (например, емкости облака DVR) и ли средства просмотра можно начать просмотр динамического потока. Архив на диске — это циклический архив «окна», содержит только объем содержимого, который указан в свойстве archiveWindowLength Live выходных данных. Содержимое, выходящее за рамки этого окна, автоматически удаляется из контейнера хранилища и не подлежит восстановлению. Можно создать несколько выходов Live (до трех максимум) на интерактивное событие с параметрами и длины архивации.  

Если вы опубликовали вывода Live **активов** с помощью **указатель потоковой передачи**, событие Live (до окна DVR) по-прежнему смогут просматривать до истечения срока действия или удаления, указатель потоковой передачи какое событие произойдет первым.
