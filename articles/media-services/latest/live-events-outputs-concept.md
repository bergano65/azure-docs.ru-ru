...
* [выходные данные потоковой трансляции](#live-outputs).
...
При использовании кодирования в реальном времени (значение событие прямой трансляции **стандартный**), предустановку кодирования определяет, как входящий поток кодируется в нескольких скоростей или слоев. Сведения см. в разделе [системных предустановок](live-event-types-comparison.md#system-presets).

> [!NOTE]
> В настоящее время единственное допустимое заранее установленное значение для стандартного типа события Live — *Default720p*. Если вам нужно использовать пользовательский динамический предустановку кодирования, обратитесь в службу amshelp@microsoft.com. Следует указать нужную таблицу, разрешения и скорости потока. Убедитесь, что имеется только один слой 720p, а не более 6 слоев.
