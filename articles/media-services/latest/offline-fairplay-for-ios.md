---
title: Оффлайн FairPlay потоковое для iOS с Azure Media Services v3
...
* Некоторые поставщики контента могут запретить доставку лицензии DRM за пределы страны/региона. Если пользователи хотят смотреть контент во время путешествия за пределами страны/региона, необходима загрузка в автономном режиме.
...
* Ознакомиться с интерактивной защитой содержимого FairPlay. 
    ...
    - Образец .NET, включающий конфигурацию потоковой передачи FPS в Интернете: [ConfigureFairPlayPolicyOptions](https://github.com/Azure-Samples/media-services-v3-dotnet-tutorials/blob/master/AMSV3Tutorials/EncryptWithDRM/Program.cs#L505)
...
* Пример кода: образец HLSCatalog (часть FPS Server SDK от Apple) для офлайн-режима FPS в версии 3.1 «Разработка»-HLSCatalog_With_FPS HLSCatalog. В примере приложения HLSCatalog следующие файлы кода, которые используются для реализации функций автономного режима:
