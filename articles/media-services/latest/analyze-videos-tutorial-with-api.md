...
Запуск задания занимает некоторое время. Поэтому вам пригодится уведомление. Получить уведомление о завершении [задания](https://docs.microsoft.com/rest/api/media/jobs) можно несколькими способами. Самый простой (который показан тут) — использовать опрос.
