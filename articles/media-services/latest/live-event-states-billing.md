...
|---|---|
|**Остановлено**| Это начальное состояние события прямой трансляции после создания (если для параметра автозапуска не задано значение true). В этом состоянии выставление счетов не выполняется. В этом состоянии можно изменять свойства трансляции, но потоковая передача невозможна.|
