...
В этом разделе показано, как создать событие потоковой трансляции **сквозного** типа (для которого параметр LiveEventEncodingType имеет значение None). Если вы хотите создать событие потоковой трансляции, поддерживающее кодирование в реальном времени, задайте для LiveEventEncodingType значение **Standard**. 
