...
&#42;Шифрование @no__t 1Dynamic поддерживает алгоритмы AES-128 Clear Key, CBCS и CENC. Дополнительные сведения см. в разделе [Матрица поддержки](#streaming-protocols-and-encryption-types). *.
...
2. Создайте [указатель потоковой передачи](streaming-locators-concept.md) , настроенный на потоковую передачу зашифрованного ресурса. 
   ...
   Указатель потоковой передачи должен быть связан с [политикой потоковой передачи](streaming-policy-concept.md). В примере мы устанавливаем `StreamingLocator.StreamingPolicyName` в политику "Predefined_MultiDrmCencStreaming". 
...
`ContentKeyId` имеет значение запрошенного ключа. Если вы хотите связать запрос с сущностью на стороне, можно использовать `AlternativeMediaId`. Например, `AlternativeMediaId` можно использовать для поиска разрешений.
...
Если вы получаете ошибку `MPE_ENC_ENCRYPTION_NOT_SET_IN_DELIVERY_POLICY`, убедитесь, что указана соответствующая политика потоковой передачи.
