...
* Динамический видеокодировщик, который преобразует сигналы с камеры (или другого устройства, например ноутбука) в потоки и отправляет их в службу потоковой трансляции. Эти сигналы могут включать стандарт рекламы SCTE-35 и подсказки Ad. 
...
### <a name="new-live-encoding-improvements"></a>Новые улучшения кодирования в реальном времени

Следующие новые улучшения были выполнены в последнем выпуске.

- Новый режим с низкой задержкой. Дополнительные сведения см. в разделе с описанием [задержки](#latency).
- Улучшенная поддержка RTMP (повышенная стабильность и поддержка исходного кодировщика).
- Безопасный прием RTMPS.

    При создании LiveEvent вы получаете четыре URL-адреса приема. 4 URL-адреса для приема практически идентичны, они имеют тот же токен потоковой передачи (AppId) с отличной частью номера порта. Два из URL-адресов являются первичными и резервными для RTMPS.   
...
| Поддержка неоднородных GPO |Yes |Нет — входные данные должны представлять собой фиксированные двухсекундные GOP |
...
В этом разделе рассматриваются типичные результаты, которые наблюдаются при использовании параметра малой задержки и различных проигрывателей. Результаты различаются в зависимости от задержки сети и CDN.

Чтобы использовать новую функцию малой задержки, в LiveEvent можно установить для параметра **StreamOptionsFlag** значение **LowLatency**. Когда поток запущен и работает, можно использовать демонстрационную страницу [Проигрывателя мультимедиа Azure](http://ampdemo.azureedge.net/) (AMP) и установить параметры воспроизведения так, чтобы использовался Low Latency Heuristics Profile (Эвристический профиль малой задержки).

### <a name="pass-through-liveevents"></a>Сквозные LiveEvent
...
|HLS.JS в проигрывателе Mixer|30 с|16 с|
