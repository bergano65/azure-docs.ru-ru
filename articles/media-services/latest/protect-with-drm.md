...
Ключ содержимого обеспечивает безопасность доступа к вашим ресурсам. При шифровании содержимого с помощью DRM необходимо создать [политику ключа содержимого](content-key-policy-concept.md) . Политика настраивает способ доставки ключа содержимого конечным клиентам. Ключ содержимого связан с указателем потоковой передачи. Службы мультимедиа также включают в себя службу доставки ключей, которая доставляет ключи шифрования и лицензии авторизованным пользователям.
...
Как правило, следует очищать все, кроме объектов, которые планируется повторно использовать (как правило, используются преобразования, Стреаминглокаторс и т. д.). Если вы хотите, чтобы учетная запись была очищена после эксперимента, удалите ресурсы, которые не планируется использовать повторно. Например, следующий код удаляет задания.
