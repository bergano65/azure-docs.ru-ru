...
Как правило, следует очищать все, кроме объектов, которые планируется повторно использовать (как правило, используются преобразования, Стреаминглокаторс и т. д.). Если учетную запись требуется очистить после эксперимента, удалите ресурсы, которые не планируется использовать повторно. Например, следующий код удаляет задание, созданные ресурсы и политику ключа содержимого:
