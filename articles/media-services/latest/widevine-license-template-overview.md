---
title: Azure Media Services v3 с обзором шаблона лицензий Widevine
...
ms.locfileid: "76705638"
---
# <a name="media-services-v3-with-widevine-license-template-overview"></a>Медиа Услуги v3 с обзором шаблона лицензии Widevine
...
Если существует уже существующая политика, нет необходимости указывать какие-либо значения в спецификации ключа содержимого. Уже существующая политика, связанная с этим контентом, используется для определения защиты от выходных данных, таких как Защита цифрового контента с высокой пропускной способностью (HDCP) и Система общего управления copy (CGMS). Если существующая политика не зарегистрирована на сервере лицензирования Widevine, поставщик содержимого может внедрить значения в запрос на лицензию.   
...
| content_key_specs <br/>ключ |Base64-<br/>Base64 |Содержимое ключа для использования для этого трека. Если указано, track_type или key_id не требуется. Поставщик содержимого с помощью этого параметра может вставить ключ содержимого для этой дорожки вместо того, чтобы сервер лицензирования Widevine создал или нашел ключ. |
