...
Чтобы воспользоваться функцией динамической упаковки, вам потребуется [ресурс](assets-concept.md) с набором MP4-файлов с адаптивной скоростью и файлов конфигурации потоковой передачи (ISM, ISMC, MPI и т. д.). Для получения файлов рекомендуется кодировать мезонинный (исходный) файл с помощью Служб мультимедиа. Чтобы видео в закодированном ресурсе стали доступны для воспроизведения в клиентах, необходимо создать [указатель потоковой передачи](streaming-locators-concept.md) и сформировать URL-адреса потоковой передачи. Затем, в зависимости от формата, указанного в манифесте клиента потоковой передачи (HLS, DASH или Smooth), вы получаете поток по выбранному протоколу.
...
### <a name="encoding-to-adaptive-bitrate-mp4s"></a>Кодирование в MP4-файлы с адаптивной скоростью
...
## <a name="delivery-codecs-support"></a>Поддержка кодеков доставки 
