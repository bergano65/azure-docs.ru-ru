...
Путь скачивания на представленном выше изображении нужен только для демонстрации того, что MP4-файл можно скачать напрямую через *конечную точку потоковой передачи* (источник), если в указателе потоковой передачи настроена [политика потоковой передачи](streaming-policy-concept.md) для скачивания.<br/>Динамический упаковщик не изменяет файл. 
...
Динамическая упаковка поддерживает аудиоданные, закодированные с помощью следующих протоколов:
