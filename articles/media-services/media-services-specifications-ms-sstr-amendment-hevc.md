---
title: Поправка к протоколу плавной потоковой передачи (MS-SSTR) для HEVC - Azure
...
В статье содержатся требования к технической реализации для сигнализации видеокодека HEVC (с использованием треков формата hev1 или hvc1) в манифесте Smooth Streaming, а нормативные ссылки обновляются для ссылки на текущие стандарты MPEG, которые включают HEVC, Общее шифрование HEVC, а также имена коробок для iSO Base Media File Format были обновлены в соответствии с последними спецификациями. 
...
>  **Время композиции:** Время представления образца у клиента, как это определено в [«ISO/IEC-14496-12».](https://go.microsoft.com/fwlink/?LinkId=183695)
> 
>   **CENC**: общее шифрование, как определено во втором выпуске спецификации [ISO/IEC 23001-7].
> 
>   **время декодирования:** Время, необходимое для декодирования образца на клиенте, как это определено в [«ISO/IEC 14496-12:2008».](https://go.microsoft.com/fwlink/?LinkId=183695)
...
> 
>   **Презентация:** набор всех **потоков** и связанных метаданных, необходимых для воспроизведения одного фильма. **запрос:** Сообщение HTTP, отправленное от клиента на сервер, как это определено в **ответе** на запрос [«RFC2616»:](https://go.microsoft.com/fwlink/?LinkId=90372) сообщение HTTP, отправленное с сервера клиенту, как это определено в [«RFC2616»](https://go.microsoft.com/fwlink/?LinkId=90372)
> 
>   **Образец:** наименьшая фундаментальная единица (например, кадр), в которой сохраняется и обрабатывается файл **мультимедиа**.
> 
>   **МОЖЕТ, ДОЛЖНЫ, ДОЛЖНЫ, НЕ ДОЛЖНЫ, НЕ ДОЛЖНЫ:** Эти термины (во всех крышках) используются, как описано в [«RFC2119»](https://go.microsoft.com/fwlink/?LinkId=90317) Все заявления о факультативном использовании поведения либо MAY, SHOULD, или SHOULD NOT.
...
>  (MS-SSTR) Протокол плавной потоковой передачи *v20140502*[https://msdn.microsoft.com/library/ff469518.aspx](https://msdn.microsoft.com/library/ff469518.aspx)
> 
...
> 
>   (MPEG4-RA) Регистрационный орган MP4, "MP4REG",[http://www.mp4ra.org](https://go.microsoft.com/fwlink/?LinkId=327787)
> 
>   (RFC2119) Брэднер, С., "Ключевые слова для использования в РЦР для обозначения уровней требований", BCP 14, RFC 2119, март 1997 года,[https://www.rfc-editor.org/rfc/rfc2119.txt](https://go.microsoft.com/fwlink/?LinkId=90317)
...
> 
>   (RFC3548) Джозефссон, С., эд., "База16, Base32 и Кодирование данных Base64", RFC 3548, июль 2003 г.,[https://www.ietf.org/rfc/rfc3548.txt](https://go.microsoft.com/fwlink/?LinkId=90432)
> 
>   (RFC5234) Крокер, Д., эд,, и Оверелл,., Дополненный BNF для синтаксиса спецификации: ABNF, STD 68, RFC 5234, январь 2008,[https://www.rfc-editor.org/rfc/rfc5234.txt](https://go.microsoft.com/fwlink/?LinkId=123096)
...
>   Исполнители могут гарантировать, что расширения не противоречат, регистрируя коды расширения с MPEG4-RA, как указано в [«ISO/IEC-14496-12»](https://go.microsoft.com/fwlink/?LinkId=183695)
...
> 1. (Без изменения)
> 
>    Рекомендуемое значение составляет 90000 для представления точной продолжительности видеокадров и фрагментов, содержащих дробное видео кадра (например, 30/1.001 Гц).
...
>
> * "hvc1": Видео образцы для этого трека использовать HEVC видео, используя формат описания образца 'hvc1', указанный в «ISO/IEC-14496-15».
> 
...
> 
>   * Когда TrackElement описывает видео HEVC, поле **FourCC** ДОЛЖНО равняться **"hev1"** или **"hvc1"**
> 
>   Поле **CodecPrivateData** SHALL содержит гекс-кодовое представление строки следующей последовательности байта, указанной в ABNF [(RFC5234):](https://go.microsoft.com/fwlink/?LinkId=123096) (без изменений от MS-SSTR)
> 
...
>   **Примечание**: Формат мультимедиа по умолчанию, запрошенный для **MinorVersion** 2 и 'hev1' или 'hvc1' является 'iso8' брендом ISO Base Media File Format, указанным в «ISO/IEC 14496-12» ISO Base Media File Format Fourth Edition, и «ISO/IEC 23001-7» Общее шифрование Второе издание.
