...
Если воспроизведение не может быть запущено или остановлено, будет вызвано событие ошибки `error()` , и функция возвратит код и необязательное сообщение, чтобы помочь разработчику приложения получить дополнительные сведения. `error().message`не отображается сообщение для пользователя.  Сообщение, отображаемое пользователю, основано на `error().code` битах 27-20, см. в таблице ниже.
...
| абортедеррхттпмикседконтентблоккед | 0x0100002 | Ошибка прерывания, смешанное содержимое заблокировано — обычно происходит при `http://` загрузке потока со `https://` страницы |
