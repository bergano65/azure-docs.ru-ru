...
> [!NOTE]
> Если вы уже используете HTML5 Шив, например [Modernizr](http://modernizr.com/) , можно включить проигрыватель мультимедиа Azure JavaScript в любом месте. Однако убедитесь, что ваша версия Modernizr включает в себя Шив для видео.
...
> [!IMPORTANT]
> **НЕ** используйте версию `latest` в рабочей среде, так как она может быть изменена по запросу. Замените `latest` на версию проигрыватель мультимедиа Azure. Например, замените `latest` на `2.1.1`. Вы можете [запросить требуемую версию Проигрывателя мультимедиа Azure](azure-media-player-changelog.md).

> [!NOTE]
> С момента `1.2.0` выпуска больше не требуется включать расположение в резервные конференции (оно автоматически выбирает расположение из относительного пути к файлу азуремедиаплайер. min. js). Вы можете изменить расположение резервных технических специалистов, добавив следующий скрипт в сценарий `<head>` после приведенных выше сценариев.
