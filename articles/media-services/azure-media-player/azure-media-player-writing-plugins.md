...
[Коллекция подключаемых](http//:aka.ms/ampplugins) модулей содержит несколько подключаемых модулей, которые сообщество уже участвовало в таких функциях, как маркеры времени, масштаб, аналитика и многое другое. Эта страница предоставляет доступ к подключаемым модулям и инструкциям по их настройке, а также демонстрации, которое показывает подключаемый модуль в действии. Если вы создаете замечательный подключаемый модуль, который вы считаете в нашей коллекции, вы можете отправить его, чтобы мы могли извлечь его.
