...
:::image type="content" source="./media/inspect-traffic-using-azure-firewall/shared-spoke.png" alt-text="Выделенная виртуальная сеть для частных конечных точек" border="true":::
...
:::image type="content" source="./media/inspect-traffic-using-azure-firewall/single-vnet.png" alt-text="Выделенная виртуальная сеть для частных конечных точек" border="true":::
...
:::image type="content" source="./media/inspect-traffic-using-azure-firewall/on-premises.png" alt-text="Выделенная виртуальная сеть для частных конечных точек" border="true":::
...
В разделе [Создайте рабочую область log Analytics в портал Azure](https://docs.microsoft.com/azure/azure-monitor/learn/quick-create-workspace) , чтобы создать рабочую область, если у вас ее нет в вашей подписке.
...
9. На вкладке **Конфигурация** введите или выберите следующие сведения:
    ...
    | Подписка | Выберите свою подписку. |
    | Зоны Частная зона DNS | Оставьте **privatelink.Database.Windows.NET**по умолчанию. |
...
2. Установите [SQL Server программ командной строки](https://docs.microsoft.com/sql/linux/quickstart-install-connect-ubuntu?view=sql-server-ver15#tools).
