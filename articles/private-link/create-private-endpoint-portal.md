...
В этом разделе вы создадите логический сервер SQL Server в Azure. 
...
## <a name="access-sql-database-privately-from-the-vm"></a>Частный доступ к базе данных SQL с виртуальной машины
...
В этом кратком руководстве вы создали виртуальную машину в виртуальной сети, логический сервер SQL Server и частную конечную точку для закрытого доступа. Вы подключились к одной виртуальной машине из Интернета и безопасно взаимодействовали с Базой данных SQL с помощью службы "Приватный канал". Дополнительные сведения о частных конечных точках см. в разделе [Что такое частная конечная точка Azure?](private-endpoint-overview.md).
