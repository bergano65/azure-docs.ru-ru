...
1. На обзорной странице для **myVM** выберите **Подключиться**, а затем — **Бастион**.

1. Нажмите синюю кнопку **Использовать Бастион**.

1. Введите имя пользователя и пароль, указанные при создании виртуальной машины.

1. После подключения откройте на сервере Windows PowerShell.

1. Введите `nslookup <cosmosdb-account-name>.documents.azure.com` и проверьте разрешение имен. Замените **\<cosmosdb-account-name>** именем учетной записи Cosmos DB, созданной на предыдущих шагах. 
...
1. Получите основную строку подключения Azure Cosmos DB на портале. Допустимая строка подключения имеет следующий формат:
   
   Для учетных записей API SQL: `https://<accountName>.documents.azure.com:443/;AccountKey=<accountKey>;` Для API Azure Cosmos DB для MongoDB: `mongodb://<accountName>:<accountKey>@cdbmongo36.mongo.cosmos.azure.com:10255/?ssl=true&replicaSet=globaldb&retrywrites=false`

1. Установите [Обозреватель службы хранилища Microsoft Azure](../vs-azure-tools-storage-manage-with-storage-explorer.md?tabs=windows&toc=%2fazure%2fstorage%2fblobs%2ftoc.json) на виртуальной машине.

1. После установки **Обозревателя службы хранилища Microsoft Azure** нажмите кнопку **Готово**.  Оставьте флажок установленным, чтобы открыть приложение.

1. На экране **Подключение к Службе хранилища Azure** выберите **Отмена**.

1. В Обозревателе службы хранилища щелкните правой кнопкой мыши элемент **Учетные записи Cosmos DB** и выберите **Подключение к Cosmos DB**.

1. В разделе **Выбрать API** оставьте значение по умолчанию **SQL**.

1. В поле раздела **Строка подключения** вставьте строку подключения из учетной записи Cosmos DB, скопированную на предыдущих шагах.

1. Выберите **Далее**.

1. Проверьте правильность используемых параметров в разделе **Сводка по соединению**.  

1. Выберите **Подключиться**.

1. Закройте подключение к **myVM**.
