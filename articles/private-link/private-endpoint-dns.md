...
description: Познакомьтесь с конфигурацией DNS для частной конечной точки Azure
...
- **Использовать файл узла (рекомендуется использовать его только для тестирования)** . Для переопределения DNS можно использовать файл узла на виртуальной машине.  
...
- **Использовать DNS-сервер переадресации (необязательно)** . Чтобы переопределить разрешение DNS для определенной частной конечной точки, можно использовать DNS-сервер переадресации. Если [DNS-сервер](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md#name-resolution-that-uses-your-own-dns-server) размещен в виртуальной сети, можно создать правило переадресации DNS, чтобы использовать частную зону DNS для упрощения настройки для всех ресурсов частной связи.
...
Службы Azure будут создавать каноническое имя DNS-записи (CNAME) на общедоступном DNS-сервере, чтобы перенаправить разрешение в предложенное имя частного домена. Разрешение можно переопределить с помощью частного IP-адреса частных конечных точек. 
 
Приложениям не нужно изменять URL-адрес подключения. При попытке разрешения с использованием общедоступной службы DNS DNS-сервер теперь будет разрешаться в частные конечные точки. Этот процесс не влияет на существующие приложения. 
...
| Storage Account (Microsoft.Storage/storageAccounts) / Файл (file, file_secondary) | privatelink.file.core.windows.net | file.core.windows.net |
...
| Файловая система Data Lake 2-го поколения (Microsoft.Storage/storageAccounts) / Файловая система Data Lake 2-го поколения (dfs, dfs_secondary) | privatelink.dfs.core.windows.net | dfs.core.windows.net |
...
| Концентратор событий Azure (Microsoft.EventHub/namespaces) / пространство имен | privatelink.servicebus.windows.net | servicebus.windows.net. |
...
Полное доменное имя служб автоматически разрешается в общедоступный IP-адрес, поэтому для разрешения в частный IP-адрес частной конечной точки необходимо соответствующим образом изменить конфигурацию DNS.
...
В зависимости от ваших предпочтений для разрешения DNS доступны следующие встроенные сценарии.
...
Эта конфигурация подходит для рабочих нагрузок виртуальных сетей без пользовательского DNS-сервера. В этом сценарии клиент запрашивает IP-адрес частной конечной точки для предоставленной Azure DNS [168.63.129.16](../virtual-network/what-is-ip-address-168-63-129-16.md). Azure DNS будет отвечать за разрешение DNS для частных зон DNS.


> [!NOTE]
> В этом сценарии используется база данных SQL Azure, рекомендуемая частной зоной DNS. Для других служб можно настроить эту модель, используя следующую ссылку [Конфигурации зоны DNS служб Azure](#azure-services-dns-zone-configuration).
...
> [!IMPORTANT]
> Для этой конфигурации требуется одна частная зона DNS. При создании нескольких зон с одинаковым именем для разных виртуальных сетей потребуется выполнить операции слияния записей DNS вручную.

В этом сценарии существует топология сети [по модели "звезда"](https://docs.microsoft.com/azure/architecture/reference-architectures/hybrid-networking/hub-spoke) с периферийными сетями, которые совместно используют общую частную конечную точку. Все периферийные виртуальные сети связаны с одной частной зоной DNS. 
...
Чтобы локальные рабочие нагрузки могли разрешать полное доменное имя частной конечной точки в частный IP-адрес, необходимо использовать DNS-сервер переадресации, чтобы обеспечить разрешение [общедоступной зоны DNS](#azure-services-dns-zone-configuration) службы Azure, развернутой в Azure.


Следующий сценарий подходит для локальной сети, в которой есть DNS-сервер переадресации в Azure, который, в свою очередь, отвечает за разрешение всех запросов DNS с помощью серверной переадресации на сервере DNS [168.63.129.16](../virtual-network/what-is-ip-address-168-63-129-16.md), предоставленном Azure. 
...
На следующей схеме показана последовательность разрешения DNS из локальной сети, в которой используется DNS-сервер переадресации, развернутый в Azure, где разрешение выполняется с помощью частной зоны DNS, [связанной с виртуальной сетью](../dns/private-dns-virtual-network-links.md).
...
Локальное решение DNS необходимо настроить для переадресации трафика DNS к Azure DNS через  [сервер условной переадресации](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md#name-resolution-that-uses-your-own-dns-server) ссылающейся на DNS-сервер переадресации, развернутый в Azure.
...
> [!IMPORTANT]
> Условную переадресацию следует выполнять для рекомендуемого [**сервера переадресации общедоступной зоны DNS**](#azure-services-dns-zone-configuration) Например: `database.windows.net`  вместо  **privatelink**.database.windows.net.
