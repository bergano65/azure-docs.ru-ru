---
title: Реплики чтения в Базе данных Azure для MariaDB
...
* Доступность по регионам. база данных Azure для MariaDB доступна в западной части США 2, центральном Франции, Северной и Германии. Однако их парные регионы недоступны.
...
При создании реплика наследует правила брандмауэра или конечную точку службы виртуальной сети главного сервера. Затем эти правила не зависят от главного сервера.
...
Эта метрика вычисляется с помощью метрики `seconds_behind_master`, доступной в команде `SHOW SLAVE STATUS` MariaDB.
...
Правила брандмауэра, правила виртуальной сети и параметры параметров наследуются от главного сервера реплике при создании реплики. После этого правила реплики будут независимыми.
