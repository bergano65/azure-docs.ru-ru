...
1. Прежде чем продолжать, ознакомьтесь с [требованиями к главному серверу](concepts-data-in-replication.md#requirements) . 
   ...
   Например, убедитесь, что исходный сервер допускает как входящий, так и исходящий трафик через порт 3306 и что исходный сервер имеет общедоступный **IP-адрес**, DNS является публично доступным или имеет полное доменное имя. 
