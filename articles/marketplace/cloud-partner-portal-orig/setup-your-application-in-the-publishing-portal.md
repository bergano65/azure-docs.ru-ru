...
1. Формы, которые нужно заполнить, отображаются в левой части представления "Редактор". Каждая «форма» состоит из набора полей, которые должны быть заполнены. Требуемые поля отмечены красной\*звездочкой ().
...
например, если издатель **"contoso"** издатели создает предложение с предложением ID **"образец-WebApp"**,\/он будет отображаться в AppSource как "https: / appsource.microsoft.com/marketplace/apps/contoso.sample-WebApp?tab=Overview"
...
* Is there more than one crm package in your package file (Файл пакета содержит более одного пакета CRM): выберите "Да" **только** в том случае, если вы намерены поддерживать несколько разных пакетов для разных версий CRM. Для большинства партнеров здесь следует выбрать вариант "Нет". Если вы выберете "Да", вам придется создать отдельные пакеты AppSource для каждой версии решения. _Примечание: Это не спрашивая, если у вас есть несколько файлов **zip.** Если у вас есть несколько файлов solution.zip, но только одна версия, вы все равно должны выбрать "Нет". Инструмент упаковки сведет их вместе автоматически._

### <a name="crm-package-availability"></a>Доступность пакета CRM

В этом разделе нужно выбрать, в каких регионах CRM будет доступен ваш пакет. Для получения информации о том, какие регионы CRM обслуживают страны/регионы, пожалуйста, ознакомьтесь с ссылкой:[https://o365datacentermap.azurewebsites.net/](https://o365datacentermap.azurewebsites.net/)

Примечание. Для развертывания в национальное облако Германии и национальное облака для US Gov организаций требуется СПЕЦИАЛЬНОЕ РАЗРЕШЕНИЕ и дополнительная ПРОВЕРКА в процессе сертификации
...
Выберите языки, которые поддерживает приложение. Если ваше приложение поддерживает дополнительные языки, которые не входят в [appsource@microsoft.com](mailto:appsource@microsoft.com) этот список, продолжайте публиковать ваше предложение и напишите нам по адресу: чтобы сообщить нам об этом.
...
* [Лазурный стол](./cloud-partner-portal-lead-management-instructions-azure-table.md)
