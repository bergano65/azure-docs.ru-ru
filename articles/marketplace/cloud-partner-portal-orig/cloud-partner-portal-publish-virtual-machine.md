...
-   [Создание и передача Windows VHD](https://docs.microsoft.com/azure/virtual-machines/virtual-machines-windows-prepare-for-upload-vhd-image?toc=/azure/virtual-machines/windows/toc.json)
...
После удовлетворения всех предварительных требований можно приступить к созданию предложения шаблона решения. Прежде чем начать, просмотрите следующее предложение и сведения о номере SKU.
...
Добавьте несколько номеров SKU в следующих сценариях:
- Если нужна поддержка различных моделей выставления счетов, таких как использование собственной лицензии (BYOL), оплата по мере использования (PAYG).
...
1. Настройте следующие поля **удостоверения предложения** в параметрах предложения.
    ...
    **Идентификатор издателя**
    ...
    Раскрывающийся список профилей издателей. В нем следует выбрать профиль издателя, от имени которого будет опубликовано это предложение. 
    >[!Note]
    >После активации предложения это поле блокируется.

    **Имя**

    Отображаемое имя для предложения. Это имя отображается в Azure Marketplace и на портале Azure. Его длина не должна превышать 50 символов. Следуйте таким инструкциям для имени предложения:
    -  Укажите для своего продукта узнаваемое название торговой марки. 
    - Не включайте в него название своей организации, если только это не требуется для продвижения продукта.
...
2. Выберите **Add a SKU** (Добавить номер SKU), чтобы ввести идентификатор номера SKU. Идентификатор номера SKU — это уникальный идентификатор номера SKU в рамках предложения. Он отображается в URL-адресах продукта, шаблонах Azure Resource Manager и отчетах о выставлении счетов. Для идентификатора номера SKU действуют такие требования:
    - Его длина не должна превышать 50 знаков.
...
**Public Azure (Общедоступные Azure)**
...
>-   Эта плата взимается с пользователя в дополнение к [ценам на виртуальные машины Azure](http://aka.ms/vmpricingdetails).
...
-   [Создание и отправка диска VHD Windows](https://docs.microsoft.com/azure/virtual-machines/virtual-machines-windows-prepare-for-upload-vhd-image?toc=/azure/virtual-machines/windows/toc.json)
...
-   Не размещайте текст на логотипе, например название компании или торговой марки. Логотип должен быть *плоским*, избегайте градиентов.
...
-   Как только предложение вносится в список, в имиджевый логотип программно вставляется отображаемое имя издателя, название плана, развернутая сводка предложения и кнопка "Создать". Не вводите текст при разработке имиджевого логотипа. Оставьте в правой области логотипа пустое место размером 415 x 100 пикселей, смещенное на 370 пикселей слева.
