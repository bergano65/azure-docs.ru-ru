---
title: Создание или изменение предложения | Документация Майкрософт
...
Артефакты, такие как образы и логотипы, должны отправляться, загружаясь в доступное расположение в Интернете, а затем включать каждый из них в качестве универсального кода ресурса (URI) в запросе отправки, как описано в примере выше. Система обнаружит, что эти файлы отсутствуют в хранилище Azure marketplace и загрузит их.  Таким образом вы обнаружите, что последующие запросы получения возвращают URL-адрес службы Azure marketplace для этих файлов.
