...
![Выберите изображение](./media/cloud-partner-portal-publish-virtual-machine/selectimage2.png)
...
3. Обновите объекты аудитории с ограниченным доступом для вашего предложения.
    ...
    **Если вы первоначально отправили список подписок для частного предложения из CSV-файла:**
    ...
    1\. Скачайте содержимое `restrictedAudience.uploadedCsvUri`. Содержимое — это просто CSV-файл с заголовками. Пример:

        type,id,description
        subscriptionId,541a269f-3df2-486e-8fe3-c8f9dcf28205,sub1
        subscriptionId,c0da499c-25ec-4e4b-a42a-6e75635253b9,sub2

    2\. При необходимости добавьте или удалите подписки в скачанном CSV-файле.

    В. Отправьте обновленный CSV-файл в расположение, например [хранилище BLOB-объектов Azure](../../storage/blobs/storage-blobs-overview.md) или [OneDrive](https://onedrive.live.com), и создайте ссылку только для чтения на файл. Это будет новый *SasUrl*.

    Г. Обновите ключ с помощью нового *SasUrl.* `restrictedAudience.uploadedCsvUri`
    ...
    **Если вы вручную указали исходный список подписок для частного предложения из Портал Cloud Partner:**
    ...
    1\. При необходимости добавьте или удалите записи в `restrictedAudience.manualEntries` списке для каждого объекта аудитории с ограниченным доступом.
