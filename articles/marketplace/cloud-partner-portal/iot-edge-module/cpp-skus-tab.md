---
title: Номера SKU модулей Azure IoT Edge | Azure Marketplace
...
|  **Поле**       |     **Описание**                                                          |
|  ---------       |     ---------------                                                          |
| **\* заголовка**        | Заголовок для этого номера SKU. Максимальная длина составляет 50 символов. <br/> Он будет отображаться на портале Azure и использоваться при развертывании как имя модуля по умолчанию (с удалением пробелов и специальных символов). Применение этого поля демонстрируется на изображениях ниже.|
...
| **Описание\***  | Краткое описание номера SKU. Максимальная длина — 3000 символов. Здесь НЕ НУЖНО описывать само предложение, это описание только для номера SKU. Оно будет отображаться в Azure Marketplace и на портале Azure. В портал Azure он будет добавлен в описание Marketplace, описывающее предложение, определенное на вкладке Marketplace.  Он может быть таким же, как и сводка SKU. Применение этого поля демонстрируется на изображениях ниже.|
| **Скрыть этот номер SKU\*** | Оставьте здесь вариант по умолчанию **Нет**. |
|  |  |
...
В следующей таблице приведены сведения о назначении, содержании и форматировании полей для разделов **сведений о репозитории изображений** и **версии образа**.  Обязательные поля помечены звездочкой (*).
...
| **\* пароля** | Пароль, связанный с реестром контейнеров. |
|    |  ***Версия образа***   |
| **Тег изображения или\* дайджеста** | Это поле должно содержать по меньшей мере тег `latest` и тег версии (например, начинаться со строки `xx.xx.xx-`, где xx — это число). Здесь должны быть [теги манифеста](https://github.com/estesp/manifest-tool) для нацеливания на несколько платформ. Все теги, указанные в теге манифеста, следует добавить в пакет для передачи. С помощью тегов вы можете включить несколько версий модуля IoT Edge. Все теги манифеста (за исключением `latest`) должны начинаться с `X.Y-` или `X.Y.Z-`, где X, Y и Z обозначают целые числа. Подробные сведения см. в разделе [о тегах и управлении версиями](./cpp-create-technical-assets.md) статьи "Prepare your IoT Edge module technical assets" (Подготовка технических ресурсов для модуля IoT Edge). <br/> Например, если тег `latest` указывает на тег, который указывает на `1.0.1-linux-x64`, `1.0.1-linux-arm32` и `1.0.1-windows-arm32`, следует добавить сюда все эти шесть тегов. |
|  |  |


### <a name="help-your-customers-launch-by-using-default-settings"></a>Помогите своим клиентам запускать с помощью параметров по умолчанию
