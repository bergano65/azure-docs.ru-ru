---
title: Тест-драйв Logic App Лазурный рынок
...
Документация тест-драйва для тестовых дисков Logic App в настоящее время все еще на GitHub для [операций](https://github.com/Microsoft/AppSource/blob/master/Setup-your-Azure-subscription-for-Dynamics365-Operations-Test-Drives.md) и [взаимодействия с клиентами,](https://github.com/Microsoft/AppSource/wiki/Setting-up-Test-Drives-for-Dynamics-365-app)туда, чтобы узнать больше.
...
Первое и самое важное поле заключается в том, чтобы переключиться, хотите ли вы тест формы со всеми необходимыми полями представлены для вас, чтобы заполнить. При выборе Нет форма становится **отключенной,** и если вы переиздаете с помощью тестового диска, тест-драйв удаляется из производства.
...
- **Регион** - *«Требуемое поле»* Область, выбранную вами, — это место, где развертываются ресурсы Logic App test Drive Logic.
...
- **Максимальное количество одновременных тестовых дисков** - *«Требуемое поле»* Количество экземпляров тест-драйва, которые уже развернуты и ожидают доступа в выбранном регионе. Клиенты могут мгновенно получить доступ к этим тестовым выпускам, не ожидая их развертывания.
...
- **Длительность тест-драйва (часы) -** *«Обязательное поле»* Длительность на сколько времени тест-драйв будет оставаться активным, в \# течение нескольких часов. Тестовый выпуск автоматически завершит работу по истечении этого периода.
...
1. Нажмите на вкладку **«Задания ролей».**  ![Активный каталог Azure, добавление нового принципа управления доступом](./media/azure-resource-manager-test-drive/SetupSub7_1.jpg)
...
> [!CAUTION]
> Вы не можете использовать предварительный просмотр регистрации приложений Azure, поскольку в настоящее время не генерируется закодированный ключ base64.
