---
title: Azure Resource Manager тестового диска | Azure Marketplace
...
Шаблон Azure Resource Manager (диспетчер ресурсов) представляет собой закодированный контейнер ресурсов Azure, которые вы разрабатываете для лучшего представления решения. Если вы не знакомы с шаблоном диспетчер ресурсов, ознакомьтесь с разделом [Общие сведения о](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-overview) шаблонах диспетчер ресурсов и [создании шаблонов диспетчер ресурсов](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-authoring-templates) , чтобы убедиться в том, что вы знаете, как создавать и тестировать собственные шаблоны.
...
Ниже приведен процесс создания Azure Resource Manager тестового диска.

1. Разработайте то, что нужно сделать вашим клиентам на схеме последовательности.
1. Определите, какие возможности вы хотите создавать клиентам.
1. Основываясь на указанных выше определениях, определите, какие части и ресурсы необходимы клиентам для выполнения таких задач: например, экземпляр D365 или веб-сайт с базой данных.
1. Создавайте проект локально и протестируйте работу.
1. Упакуйте интерфейс в развертывании шаблона ARM и оттуда:
    1. Определите, какие части ресурсов являются входными параметрами.
    1. Переменные;
    1. Какие выходные данные передаются в интерфейс пользователя.
1. Публикуйте, тестируйте и пройдите в реальном времени.
...
Проследите за достаточной продолжительностью, чтобы оценить, какие из лучших способов показывать в вашем продукте. В частности, для всех необходимых ресурсов, так как это упрощает упаковку шаблона диспетчер ресурсов.
...
После того как вы разработали нужный пакет ресурсов, теперь поступает написание и создание шаблона тестового диска диспетчер ресурсов.
...
|---|---|
| main-template.json                | HTTPS:\//\<\..\>. blob.core.windows.net/\<\..\>/Маин-Темплате.жсон  |
| templates/solution.json           | HTTPS:\//\<\..\>. blob.core.windows.net/\<\..\>/темплатес/солутион.жсон |
...
Первое и самое важное поле позволяет включить или отключить тестовый выпуск для вашего предложения. При нажатии **кнопки Да** остальная часть формы со всеми необходимыми полями будет заполнена. Если нажать кнопку " **нет",** форма становится неактивной, и при повторной публикации с отключенным тестовым диском тестовый диск удаляется из рабочей среды.
...
**Описание —** *требуется* , если вы записываете основное описание на тестовый диск. Клиент узнает о сценариях использования этого тестового выпуска. 

**Руководство пользователя —** *обязательно* . Это подробное пошаговое руководство по использованию тестового диска. Клиент ее откроет и сможет шаг за шагом выполнить точно те действия, которые требуются от него в тестовом выпуске. Очень важно, чтобы это содержимое было простым для понимания и выполнения! (Это должен быть файл в формате PDF.)

**Демонстрационное видео. Рекомендуемые видеоролики** , как и в руководстве пользователя, лучше включать в себя Видеоучебник по работе с тестовыми дисками. Клиент просмотрит его перед работой или во время работы с тестовым выпуском и сможет шаг за шагом выполнить точно те действия, которые от него требуются в тестовом выпуске. Очень важно, чтобы это содержимое было простым для понимания и выполнения!

- **Name** (Имя) — заголовок видео.
- **Link** (Ссылка) — должна быть встроенным URL-адресом видео или канала. Ниже показано, как получить внедренный URL-адрес:
...
**Instances (** *необходимые* экземпляры). здесь можно указать, сколько экземпляров вам нужно, в каких регионах и как быстро клиенты смогут получить тестовый выпуск.
...
**Длительность тестового диска (в часах) —** *требуемый* срок, в течение которого тестовый диск будет оставаться активным, в \# часов. Тестовый выпуск автоматически завершит работу по истечении этого периода.

**Шаблон тестового диска диспетчер ресурсов —** *требуется* отправить шаблон диспетчер ресурсов. Это файл, созданный в предыдущем разделе. Задайте для основного файла шаблона имя main-template.json и убедитесь, что шаблон Resource Manager содержит все необходимые выходные параметры для ключевых переменных. (Это должен быть ZIP-файл.)

**Сведения о доступе —** *требуется* после того, как клиент получит свой тестовый выпуск, предоставляются сведения о доступе. В этих инструкциях следует указать полезные параметры вывода из шаблона тестового выпуска Resource Manager. Чтобы включить параметры вывода, используйте двойные фигурные скобки (например, **{{имя_вывода}}** ). Так вы правильно вставите параметры в нужное расположение. (Здесь в клиентском интерфейсе рекомендуется выводить средства для HTML-форматирования строк.)
...
**Идентификатор подписки Azure —** *требуется* , чтобы предоставить доступ к службам Azure и портал Azure. Именно в подписке производится учет использования ресурсов и выставляются счета за услуги. Если у вас еще нет **отдельной** подписки Azure только для тестовых выпусков, создайте ее. Чтобы найти идентификаторы подписок Azure, войдите на портал Azure, а затем перейдите в раздел Subscriptions (Подписки) в меню слева. (Пример: a83645ac-1234-5ab6-6789-1h234g764ghty.)

![Подписки Azure](./media/azure-resource-manager-test-drive/subdetails2.png)

**Идентификатор клиента Azure AD —** *требуется* если у вас уже есть идентификатор клиента, его можно найти ниже в свойствах\> идентификатор каталога.
...
![Определение организации, домена и страны или региона для клиента Azure AD](./media/azure-resource-manager-test-drive/subdetails5.png)

![Подтверждение выбора](./media/azure-resource-manager-test-drive/subdetails6.png)

**Azure AD App идентификатор —** *необходимо* создать и зарегистрировать новое приложение. Мы будем использовать это приложение для выполнения операций с экземпляром тестового выпуска.
...
Нажмите кнопку "Сохранить". Последний шаг — скопировать идентификатор зарегистрированного приложения и вставить его в поле Test Drive (Тестовый выпуск).
...
1. Перейдите на вкладку **назначения ролей** .  ![добавить новый субъект контроля доступа](./media/azure-resource-manager-test-drive/SetupSub7_1.jpg)
