---
title: Тест-драйв менеджера ресурсов Azure (ru) Лазурный рынок
...
Шаблон Azure Resource Manager (Resource Manager) — это закодированный контейнер ресурсов Azure, который лучше всего представляет ваше решение. Если вы не знакомы с тем, что такое шаблон «Менеджер ресурсов», прочитайте [о понимании шаблонов «Менеджер ресурсов»](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-overview) и [авторизации шаблонов «Менеджер ресурсов»,](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-authoring-templates) чтобы убедиться, что вы знаете, как создавать и тестировать собственные шаблоны.
...
Вот процесс создания тест-драйва менеджера ресурсов Azure:

1. Дизайн того, что вы хотите, чтобы ваши клиенты делают в диаграмме потока.
1. Определите, какой опыт вы хотели бы, чтобы ваши клиенты построили.
1. Основываясь на вышеуказанных определениях, определите, какие части и ресурсы необходимы клиентам для достижения такого опыта: например, экземпляр D365 или веб-сайт с базой данных.
1. Создайте дизайн локально и проверьте его опыт.
1. Упакуйте опыт развертывания шаблона ARM, а оттуда:
    1. Определить, какие части ресурсов являются входные параметры;
    1. Какие переменные;
    1. Какие результаты предоставляются клиенту.
1. Публикуйте, тестируйте и выходите в прямой эфир.
...
Убедитесь в том, чтобы потратить достаточное количество времени на оценку того, что являются лучшими способами, чтобы показать свой продукт. В частности, вокруг всех необходимых ресурсов, которые вам понадобятся, так как это делает упаковку шаблона Resource Manager достаточно простой.
...
После того как вы разработали желаемый пакет ресурсов, теперь приходит написание и здание шаблона менеджер ресурсов тест-драйва.
...
Важно также отметить, что **все параметры являются необязательными,** так что если вы не\'хотите использовать какие-либо, вам не\'придется.
...
| **Пользователя**    | строка          | Новое случайное имя пользователя.| admin68876      |
| **пароль**    | защищенная строка    | Новый случайный пароль | Lp!ACS\^2kh     |
| **идентификатор сеанса**   | строка          | Уникальный идентификатор сеанса тестового выпуска (GUID)    | b8c8693e-5673-449c-badd-257a405a6dee |
...
Это означает, что каждый раз, когда Test Drive развертывает шаблон Resource Manager, он создает **новую группу ресурсов с уникальным именем** для всех своих\' ресурсов. Поэтому необходимо использовать функцию [uniquestring](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-template-functions#uniquestring), сцепленную с именами переменных в идентификаторах групп ресурсов, чтобы создавать случайные уникальные значения:
...
| scripts/warmup.ps1                | https:\//\<\... \>.blob.core.windows.net/\<\... \>/скрипты/warmup.ps1  |
...
Первое и самое важное поле позволяет включить или отключить тестовый выпуск для вашего предложения. При выборе **«Да»** остальная часть формы со всеми необходимыми полями представлена для заполнения. При выборе Нет форма становится **отключенной,** и если вы переиздаете с помощью тестового диска, тест-драйв удаляется из производства.
...
**Description (Описание) —** *обязательное*. Здесь следует ввести основное описание того, что содержится в данном тестовом выпуске. Клиент узнает о сценариях использования этого тестового выпуска. 
...
**Длительность тест-драйва (часы) -** *Обязательная* продолжительность \# времени на продолжительность, как долго тест-драйв будет оставаться активным, в течение нескольких часов. Тестовый выпуск автоматически завершит работу по истечении этого периода.
...
**Azure Subscription ID (Идентификатор подписки Azure) —** *обязательно*. Предоставляет доступ к службам Azure и порталу Azure. Именно в подписке производится учет использования ресурсов и выставляются счета за услуги. Если у вас еще нет **отдельной** подписки Azure только для тестовых выпусков, создайте ее. Чтобы найти идентификаторы подписок Azure, войдите на портал Azure, а затем перейдите в раздел Subscriptions (Подписки) в меню слева. (Пример: a83645ac-1234-5ab6-6789-1h234g764ghty.)
...
![Определите организацию, домен и страну/регион для арендатора Azure AD](./media/azure-resource-manager-test-drive/subdetails5.png)

![Подтверждение выбора](./media/azure-resource-manager-test-drive/subdetails6.png)

**Azure AD App ID (Идентификатор приложения Azure AD) —** *обязательно*. На следующем шаге создается и регистрируется новое приложение. Мы будем использовать это приложение для выполнения операций с экземпляром тестового выпуска.
...
1. Нажмите на вкладку **«Задания ролей».**  ![Добавление нового принципауправления доступа](./media/azure-resource-manager-test-drive/SetupSub7_1.jpg)
1. Нажмите **Добавить ролевые задания**.
1. Установите роль в качестве **вкладчика**.
...
Как только вы чувствуете себя комфортно с вашим Предварительный предложение, теперь пришло время **идти жить**! После публикации предложения выполняется процесс окончательного анализа корпорацией Майкрософт для повторной проверки всего процесса взаимодействия. Если по какой-либо причине предложение будет отклонено, мы отправим уведомление в отдел разработки с описанием того, что необходимо исправить.
