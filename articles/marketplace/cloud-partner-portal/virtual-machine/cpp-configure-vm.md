---
title: Нанастройка VM-м вм с Microsoft Azure для рынка Azure
...
Если вы выбрали один из виртуальных машин, предварительно настроенный с операционной системой (и дополнительно дополнительные услуги), то вы уже выбрали стандартный размер Azure VM, как описано во [вкладке Virtual machine SKUs.](./cpp-skus-tab.md)  Запуск решения с предварительно настроенной ОС является рекомендуемым подходом.  Но если вы устанавливаете операционную систему вручную, необходимо выбрать размер основного виртуального жесткого диска для образа виртуальной машины.
...
Для получения дополнительной информации о запуске sysprep [см.](https://docs.microsoft.com/azure/virtual-machines/windows/capture-image-resource#generalize-the-windows-vm-using-sysprep)

### <a name="linux"></a>Linux

Следующий двухступенчатый процесс обобщает Linux VM и передислоцирует его в отдельный VM. Эти два шага являются лишь основными аспектами процесса. Для получения дополнительной информации об этих двух шагах и почему они должны быть сделаны, [посмотреть, как создать образ виртуальной машины или VHD](../../../virtual-machines/linux/capture-image.md). Для создания VHD для предложения Azure Marketplace вы можете остановиться, когда дойдете до раздела «Создайте VM из захваченного изображения».
...
После того, как ваш VM обобщен, был deallocated, и вы создали изображение VM, вы готовы [развернуть виртуальную машину с виртуального жесткого диска.](./cpp-deploy-vm-vhd.md)
