...
|  User (Пользователь)              |  Имя пользователя, которое будет использоваться для входа в систему виртуальной машины         |
...
17. В разделе **Свойства** прокрутите вниз до раздела **Мультитенантный**. Выберите ответ **Да**.
...
20. Создание секретный ключ, выбрав текстовое поле **Описание** для ключа. Настройте следующие поля:
...
### <a name="to-create-and-get-a-token-using-postman"></a>Создание и получение маркера с помощью Postman
 ...
 Чтобы запросить Auth0 для маркеров для любого из ваших полномочных приложений, выполните операцию POST для [https://login.microsoftonline.com/common/oauth2/token](https://login.microsoftonline.com/common/oauth2/token) конечной точки с полезной нагрузкой в следующем формате:
...
Чтобы запросить Auth0 для маркеров для любого из ваших полномочных приложений, выполните операцию POST в конечной точке HTTPS\/:/soamtenant.auth0.com/OAuth/Token с полезной нагрузкой в следующем формате:
