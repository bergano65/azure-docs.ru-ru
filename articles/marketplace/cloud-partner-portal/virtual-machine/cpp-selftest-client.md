---
title: Самотестирование клиента для предварительной проверки виртуальной машины Лазурный рынок
...
В рамках процесса самотестирования вы создадите локальный клиент, который подключается к Azure Marketplace для проверки VM, работаемого в подписке Azure. Виртуальная машина может работать под управлением ОС Windows или Linux.
...
|      Поле         |    Описание    |
|  ---------------   |  ---------------  |
|  Авторизация     |  Строка "Bearer xxxx-xxxx-xxxxxxxx" содержит токен клиента Azure Active Directory (AD), который может быть создан с помощью PowerShell.          |
...
|  OS                |  Операционная система на виртуальной машине: `Linux` или `Windows`          |
...
3. На панели навигации слева щелкните **Все службы**, а затем выберите **Azure Active Directory**.
   ...
   **Чтобы получить сведения об арендаторе, сделайте следующее.**
   ...
   В **обзоре active Directory Azure**выисните "Свойства", а затем выберите **свойства.** На следующем снимке экрана вы видите пример этой страницы свойств.
...
3. В разделе **Создать** укажите обязательную информацию для следующих полей.
   ...
   - **Имя** - Введите дружественное название для приложения. Например, "SelfTestClient".
   - **Тип приложения** - Выберите **веб-приложение/API**
   - **Регистрация URL** - Введите\/"https: /isvapp.azurewebsites.net/selftest-vm"
...
10. В **рамках выбора API**введите классическую модель развертывания Windows Azure для поиска API.
...
13. Выберите **Access "Windows Azure Service Management API"**.
...
17. В разделе **Свойства** прокрутите вниз до раздела **Мультитенантный**. Нажмите кнопку **Да**.
...
20. Создание секретный ключ, выбрав текстовое поле **Описание** для ключа. Задайте значения в следующих полях:
    ...
    - Введите имя ключа. Например, "самопроверяйемый клиент"
    - В списке выпадающих **сроков EXPIRES** выберите "В 1 год".
    - Щелкните **Сохранить**, чтобы создать ключ.
    - В поле **Значение** скопируйте ключ.

      >[!Important]
      >Вы не сможете увидеть ключевое значение после выхода из формы **Ключей.**
...
### <a name="to-create-and-get-a-token-using-postman"></a>Создание и получение маркера с помощью Postman
 ...
 Чтобы запросить токены Auth0 для любого из ваших [https://login.microsoftonline.com/common/oauth2/token](https://login.microsoftonline.com/common/oauth2/token) авторизованных приложений, выполните операцию POST до конечной точки с полезной нагрузкой в следующем формате:
...
Чтобы запросить токены Auth0 для любого из ваших авторизованных\/приложений, выполните операцию POST на https: /soamtenant.auth0.com/oauth/token конечную точку с полезной нагрузкой в следующем формате:
