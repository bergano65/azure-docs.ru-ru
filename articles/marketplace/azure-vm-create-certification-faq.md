...
> [!Note]
> Если вы используете базовый образ Linux, который не используется в Azure Marketplace, убедитесь, что первые 2048 секторов (каждый сектор равен 512 байт) на виртуальном жестком диске пусты, поэтому Azure продолжит публикацию виртуальной машины в Azure Marketplace.  
...
## <a name="first-1mb-2048-sectors-each-sector-of-512-bytes-partition-linux-only"></a>Первая 1 МБ (2048 секторов, каждый сектор 512 байт) (только для Linux)

При отправке виртуального жесткого диска убедитесь, что первые 2048 секторов (1 МБ) виртуального жесткого диска пусты. В противном случае запрос завершится ошибкой. Обратите внимание, что это будет применимо к дискам загрузки и ОС, а не к дополнительным дискам данных.

>[!NOTE]
>Для некоторых специальных образов, например, созданных на основе базовых образов Azure, полученных из Azure Marketplace, или убедитесь, что первая 1 МБ (2048 секторов) виртуального жесткого диска пуста. 

### <a name="create-a-first-1mb-2048-sectors-each-sector-of-512-bytes-partition-on-an-empty-vhd"></a>Создание первой 1-мегабайтной секции (2048 секторов, каждый сектор 512 байт) на пустом виртуальном жестком диске
...
### <a name="create-a-first-mb-2048-sectors-each-sector-of-512-bytes-partition-by-moving-existing-data-on-vhd"></a>Создайте первые МБ (2048 секторов, каждый сектор размером 512 байт), переместив существующие данные на виртуальный жесткий диск
