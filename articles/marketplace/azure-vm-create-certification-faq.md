...
ms.locfileid: "93339540"
---
# <a name="vm-certification-troubleshooting"></a>Устранение неполадок с сертификацией виртуальной машины
...
Строка "конектикс" является частью спецификации VHD и определена как 8-байтный "cookie" в нижнем колонтитуле VHD, который определяет создателя файла. Все VHD-файлы, созданные корпорацией Майкрософт, имеют этот файл cookie. 

Большой двоичный объект в формате VHD должен иметь 512-разрядный нижний колонтитул. Это формат нижнего колонтитула VHD:
...
### <a name="vhd-specifications"></a>Спецификации VHD
Чтобы обеспечить возможность эффективной публикации, убедитесь, что **виртуальный жесткий диск соответствует следующим критериям.**
* Файл cookie должен содержать строку "конектикс"
* Необходимо исправить тип диска
* Виртуальный диск виртуального жесткого диска имеет размер не менее 20 МБ
* Виртуальный жесткий диск согласован (т. е. виртуальный размер должен быть кратен 1 МБ)
* Длина большого двоичного объекта VHD = размер виртуального и нижнего колонтитула VHD (512)

Здесь можно скачать спецификацию VHD [.](https://www.microsoft.com/download/details.aspx?id=23850)
...
Если вы хотите отправить запрос с образом отключенного образа SSH для процесса сертификации, выполните следующие действия.

1. Выполните набор средств Azure на своем образе. (Скачайте [последний набор средств](https://aka.ms/AzureCertificationTestTool)

2. Отправьте запрос в [службу поддержки](https://aka.ms/marketplacepublishersupport), прикрепите отчет набора средств и предоставьте сведения о предложении: имя предложения, имя издателя, идентификатор плана, номер SKU и версия.

3. Отправьте запрос на сертификацию повторно.
...
## <a name="steps-for-creating-first-mb-2048-kb-partition-only-for-linux-on-an-empty-vhd"></a>Шаги для создания раздела "первые МБ (2048 КБ)" (только для Linux) на пустом виртуальном жестком диске

Шаг 1. Создание виртуальной машины любого типа (пример: Ubuntu, центная ОС и т. д.). Заполните обязательные поля и щелкните "Далее: диски>" \
![Команда "Далее: диски"](./media/create-vm/vm-certification-issues-solutions-15.png)

Шаг 2. Создайте неуправляемый диск для указанной выше виртуальной машины.
![Создание неуправляемого диска](./media/create-vm/vm-certification-issues-solutions-16.png)

Обратите внимание, что можно либо использовать значения по умолчанию, либо указать любое значение для таких полей, как NIC, NSG и общедоступный IP-адрес.

Шаг 3. После создания виртуальной машины щелкните "диски" слева, как показано ниже, ![ щелкните "диски".](./media/create-vm/vm-certification-issues-solutions-17.png)

Шаг 4. Подключите VHD в качестве диска данных к указанной выше виртуальной машине, чтобы создать таблицу секционирования, как показано ниже.
![Подключение виртуального жесткого диска](./media/create-vm/vm-certification-issues-solutions-18.png)

Щелкните Добавить диск данных — > существующий BLOB-объект. > Обзор учетной записи хранилища VHD — > контейнер — > SELECT VHD-> нажмите кнопку ОК, как показано ниже.
![Выбор виртуального жесткого диска](./media/create-vm/vm-certification-issues-solutions-19.png)

Виртуальный жесткий диск будет добавлен в качестве диска данных LUN 0, а затем повторно запустите виртуальную машину после добавления диска.

Шаг 5. После повторного запуска виртуальной машины Войдите на виртуальную машину, используя выводимые данные (или любой другой клиент), и выполните команду "sudo-i", чтобы получить доступ к корневому каталогу.

![Вход на виртуальную машину](./media/create-vm/vm-certification-issues-solutions-20.png)

Шаг 6. выполните действия ниже, чтобы создать раздел на виртуальном жестком диске.

a) введите команду FDISK/Dev/SDB.

б) для просмотра существующего списка разделов из виртуального жесткого диска введите p

в) тип d для удаления всех существующих секций, доступных на виртуальном жестком диске (этот шаг можно пропустить, если он не требуется) ![ удалить все существующие секции](./media/create-vm/vm-certification-issues-solutions-21.png)

d) введите n, чтобы создать новую секцию, и выберите p для (основной раздел).

д) введите 2048 в качестве значения "First секторе", и вы можете оставить "последний сектор", так как он примет значение по умолчанию. Обратите внимание, что данные будут стерты до 2048 КБ.
           
>[!NOTE]
>* Обратите внимание, что при создании секции, как только существующие данные будут удалены до 2048 КБ, поэтому рекомендуется создать резервную копию виртуального жесткого диска перед выполнением приведенной выше команды.

Чтобы получить ссылку, найдите приведенный ниже снимок экрана.
![Удаленные данные](./media/create-vm/vm-certification-issues-solutions-22.png)

е) введите w, чтобы подтвердить создание секции. 

![Создание секции](./media/create-vm/vm-certification-issues-solutions-23.png)

ж) можно проверить таблицу секционирования, выполнив команду n FDISK/Dev/SDB и введя p, затем можно увидеть, как показано ниже, что секция создается с 2048 значением смещения. 

 ![2048 смещение](./media/create-vm/vm-certification-issues-solutions-24.png)

Шаг 7. Отключите виртуальный жесткий диск от виртуальной машины и удалите виртуальную машину.

         
## <a name="steps-for-creating-first-mb-2048-kb-partition-only-for-linux-by-moving-the-existing-data-on-vhd"></a>Шаги для создания раздела First МБ (2048 КБ) (только для Linux) путем перемещения существующих данных на виртуальный жесткий диск

Шаг 1. Создание виртуальной машины любого типа (пример: Ubuntu, центная ОС и т. д.). Заполните обязательные поля и щелкните "Далее: диски>" \
![Щелкните "Далее: диски>".](./media/create-vm/vm-certification-issues-solutions-15.png)
...
Шаг 4. Подключите VHD в качестве диска данных к указанной выше виртуальной машине, чтобы создать таблицу секционирования, как показано ниже.
![Таблица секционирования](./media/create-vm/vm-certification-issues-solutions-18.png)
...
Шаг 5. После повторного запуска виртуальной машины Войдите на виртуальную машину с помощью инструкции выполнив команду "sudo-i", чтобы получить доступ с корневым доступом. \
![Вход после перезапуска](./media/create-vm/vm-certification-issues-solutions-20.png)

Шаг 6. екскуте командный эхо "+ 1м," | сфдиск--Move-Data/Dev/SDC-N 1 ![ выполнить команду](./media/create-vm/vm-certification-issues-solutions-25.png)

>[!NOTE]
>* Обратите внимание, что выполнение приведенной выше команды может занять больше времени, так как зависит от размера диска.
