---
title: Географическая доступность и поддержка валют | Azure
description: Географическая доступность в Azure Marketplace и AppSource
services: Azure, Marketplace, Compute, Storage, Networking
author: qianw211
ms.service: marketplace
ms.topic: article
ms.date: 02/28/2019
ms.author: ellacroi
ms.openlocfilehash: 394ec230b836d55e4ce29a444566221c14efe296
ms.sourcegitcommit: 37343b814fe3c95f8c10defac7b876759d6752c3
ms.translationtype: HT
ms.contentlocale: ru-RU
ms.lasthandoff: 04/24/2019
ms.locfileid: "63762798"
---
# <a name="geographic-availability-and-currency-support"></a>Географическая доступность и поддержка валюты

Предложения Azure Marketplace можно приобрести в 141 географических регионах согласно адрес заказчика для выставления счетов и транзакции могут быть завершены в 17 валют. В таблице ниже перечислены каждого поддерживается географическое расположение его [альфа-код из двух цифр ISO 3166](https://en.wikipedia.org/wiki/List_of_ISO_3166_country_codes)и назначенных валюты.

|   Название страны/региона               |   ISO-2   |   Валюта   |
|-------------------------------------|-----------|--------------|
| Афганистан                         | AF        | USD          |
| Албания                             | AL        | USD          |
| Алжир                             | DZ        | USD          |
| Андорра                             | AD        | EUR          |
| Ангола                              | АО        | USD          |
| Аргентина                           | AR        | USD          |
| Армения                             | AM        | USD          |
| Австралия                           | AU        | AUD          |
| Австрия                             | AT        | EUR          |
| Азербайджан                          | AZ        | USD          |
| Бахрейн                             | BH        | USD          |
| Бангладеш                          | BD        | USD          |
| Барбадос                            | BB        | USD          |
| Беларусь                             | BY        | USD          |
| Бельгия                             | BE        | EUR          |
| Белиз                              | BZ        | USD          |
| Бермудские о-ва                             | BM        | USD          |
| Боливия                             | BO        | USD          |
| Босния и Герцеговина              | BA        | USD          |
| Ботсвана                            | BW        | USD          |
| Бразилия                              | BR        | БРАЗИЛЬСКИЙ РЕАЛ\*        |
| Бруней-Даруссалам                              | BN        | USD          |
| Болгария                            | BG        | EUR          |
| Камерун                            | CM        | USD          |
| Канада                              | CA        | CAD          |
| Кабо-Верде                          | CV        | USD          |
| Каймановы о-ва                      | KY        | USD          |
| Чили                               | CL        | USD          |
| Колумбия                            | CO        | USD          |
| Коста-Рика                          | CR        | USD          |
| Кот д'Ивуар                       | CI        | USD          |
| Хорватия                             | HR        | EUR          |
| Кюрасао                             | CW        | USD          |
| Кипр                              | CY        | EUR          |
| Чешская Республика                      | CZ        | EUR          |
| Дания                             | DK        | DKK          |
| Доминиканская Республика                  | DO        | USD          |
| Эквадор                             | EC        | USD          |
| Египет                               | EG        | USD          |
| Эль-Сальвадор                         | SV        | USD          |
| Эстония                             | EE        | EUR          |
| Эфиопия                            | ET        | USD          |
| Фарерские о-ва                       | FO        | DKK          |
| Фиджи                                | FJ        | AUD          |
| Финляндия                             | FI        | EUR          |
| Франция                              | СВ        | EUR          |
| Джорджия                             | GE        | USD          |
| Германия                             | DE        | EUR          |
| Гана                               | GH        | USD          |
| Греция                              | GR        | EUR          |
| Гватемала                           | GT        | USD          |
| Гондурас                            | HN        | USD          |
| Гонконг, САР                       | HK        | USD          |
| Венгрия                             | HU        | EUR          |
| Исландия                             | IS        | EUR          |
| Индия                               | IN        | INR          |
| Индонезия                           | ИД        | USD          |
| Ирак                                | IQ        | USD          |
| Ирландия                             | IE        | EUR          |
| Израиль                              | IL        | USD          |
| Италия                               | IT        | EUR          |
| Ямайка                             | JM        | USD          |
| Япония                               | JP        | JPY          |
| Иордания                              | JO        | USD          |
| Казахстан                          | KZ        | USD          |
| Кения                               | KE        | USD          |
| Корея                       | KR        | KRW          |
| Кувейт                              | KW        | USD          |
| Киргизия                          | KG        | USD          |
| Латвия                              | LV        | EUR          |
| Ливан                             | LB        | USD          |
| Ливия                               | LY        | USD          |
| Лихтенштейн                       | LI        | EUR          |
| Литва                           | LT        | EUR          |
| Люксембург                          | LU        | EUR          |
| Макао (САР)                           | MO        | USD          |
| Северная Македония                     | MK        | USD          |
| Малайзия                            | MY        | USD          |
| Мальта                               | MT        | EUR          |
| Маврикий                           | MU        | USD          |
| Мексика                              | MX        | USD          |
| Молдова                             | MD        | USD          |
| Монако                              | MC        | EUR          |
| Монголия                            | MN        | USD          |
| Черногория                          | ME        | USD          |
| Марокко                             | MA        | USD          |
| Намибия                             | Нет данных        | USD          |
| Непал                               | NP        | USD          |
| Нидерланды                         | NL        | EUR          |
| Никарагуа                           | NI        | USD          |
| Новая Зеландия                         | NZ        | NZD          |
| Нигерия                             | NG        | USD          |
| Норвегия                              | НЕТ        | NOK          |
| Оман                                | OM        | USD          |
| Пакистан                            | PK        | USD          |
| Палестинская Автономия               | PS        | USD          |
| Панама                              | PA        | USD          |
| Парагвай                            | PY        | USD          |
| Перу                                | PE        | USD          |
| Филиппины                         | PH        | USD          |
| Польша                              | PL        | EUR          |
| Португалия                            | PT        | EUR          |
| Пуэрто-Рико                         | PR        | USD          |
| Катар                               | QA        | USD          |
| Румыния                             | RO        | EUR          |
| Россия                              | RU        | RUB          |
| Руанда                              | RW        | USD          |
| Сент-Китс и Невис               | KN        | USD          |
| Саудовская Аравия                        | SA        | USD          |
| Сенегал                             | SN        | USD          |
| Сербия                              | RS        | USD          |
| Сингапур                           | SG        | USD          |
| Словакия                            | SK        | EUR          |
| Словения                            | SI        | EUR          |
| ЮАР                        | ZA        | USD          |
| Испания                               | ES        | EUR          |
| Шри-Ланка                           | LK        | USD          |
| Швеция                              | SE        | SEK          |
| Швейцария                         | CH        | CHF          |
| Тайвань                              | TW        | TWD          |
| Таджикистан                          | TJ        | USD          |
| Танзания                            | TZ        | USD          |
| Таиланд                            | ВП        | USD          |
| Тринидад и Тобаго                 | TT        | USD          |
| Тунис                             | TN        | USD          |
| Турция                              | TR        | USD          |
| Туркменистан                        | TM        | USD          |
| Ватикан (Ватикан)             | VA        | EUR          |
| Виргинские острова (США) | VI        | USD          |
| Уганда                              | UG        | USD          |
| Украина                             | UA        | USD          |
| Объединенные Арабские Эмираты                | AE        | USD          |
| Соединенное королевство                      | GB        | GBP          |
| США                       | США        | USD          |
| Уругвай                             | UY        | USD          |
| Узбекистан                          | UZ        | USD          |
| Венесуэла                           | VE        | USD          |
| Вьетнам                             | VN        | USD          |
| Йемен                               | YE        | USD          |
| Замбия                              | ZM        | USD          |
| Зимбабве                            | ZW        | USD          |
|   |   |   |

\* Для клиентов в Бразилии Marketplace через поставщиков облачных решений (CSP) использует долл. США.
