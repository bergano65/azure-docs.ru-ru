---
title: Поддержка географической доступности и валюты для коммерческой платформы Майкрософт
description: Поддержка географической доступности и валюты для транзакций на коммерческой платформе
ms.service: marketplace
ms.subservice: partnercenter-marketplace-publisher
ms.topic: reference
author: keferna
ms.author: keferna
ms.date: 07/29/2020
ms.openlocfilehash: b1baeaa31eddc13a68f2cc1d374b850f1c1a26bd
ms.sourcegitcommit: 1aef4235aec3fd326ded18df7fdb750883809ae8
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/12/2020
ms.locfileid: "88136343"
---
# <a name="geographic-availability-and-currency-support-for-commercial-marketplace"></a>Поддержка географической доступности и валюты для коммерческой платформы

## <a name="supported-geographic-locations"></a>Поддерживаемые географические расположения

Предложения на коммерческой платформе можно приобрести в 141 регионах в соответствии с адресом для выставления счетов клиента, а транзакции могут осуществляться в 17 валютах. В следующей таблице перечислены все поддерживаемые географические расположения, их [буквенно-цифровой код ISO 3166 2](https://en.wikipedia.org/wiki/List_of_ISO_3166_country_codes)и назначенная Валюта.

|   Название региона или страны               |   ISO-2   |   Валюта   |
|-------------------------------------|-----------|--------------|
| Афганистан                         | AF        | USD          |
| Албания                             | AL        | USD          |
| Алжир                             | DZ        | USD          |
| Андорра                             | AD        | EUR          |
| Ангола                              | АО        | USD          |
| Аргентина                           | AR        | USD          |
| Армения                             | AM        | USD          |
| Австралия                           | AU        | AUD          |
| Австрия                             | AT        | EUR          |
| Азербайджан                          | AZ        | USD          |
| Бахрейн                             | BH        | USD          |
| Бангладеш                          | BD        | USD          |
| Барбадос                            | BB        | USD          |
| Беларусь                             | BY        | USD          |
| Бельгия                             | BE        | EUR          |
| Белиз                              | BZ        | USD          |
| Бермудские о-ва                             | BM        | USD          |
| Боливия                             | BO        | USD          |
| Босния и Герцеговина              | BA        | USD          |
| Ботсвана                            | BW        | USD          |
| Бразилия                              | BR        | BRL\*        |
| Бруней-Даруссалам                              | BN        | USD          |
| Болгария                            | BG        | EUR          |
| Кабо-Верде                          | CV        | USD          |
| Камерун                            | CM        | USD          |
| Canada                              | CA        | CAD          |
| Каймановы о-ва                      | KY        | USD          |
| Чили                               | CL        | USD          |
| Китай\**                            | CN        | Недоступно          |
| Колумбия                            | CO        | USD          |
| Коста-Рика                          | CR        | USD          |
| Кот д'Ивуар                       | CI        | USD          |
| Хорватия                             | HR        | EUR          |
| Кюрасао                             | CW        | USD          |
| Кипр                              | CY        | EUR          |
| Чешская Республика                      | CZ        | EUR          |
| Дания                             | DK        | DKK          |
| Доминиканская Республика                  | DO        | USD          |
| Эквадор                             | EC        | USD          |
| Египет                               | EG        | USD          |
| Эль-Сальвадор                         | SV        | USD          |
| Эстония                             | EE        | EUR          |
| Эфиопия                            | ET        | USD          |
| Фарерские о-ва                       | FO        | DKK          |
| Фиджи                                | FJ        | AUD          |
| Финляндия                             | FI        | EUR          |
| Франция                              | СВ        | EUR          |
| Джорджия                             | GE        | USD          |
| Германия                             | DE        | EUR          |
| Гана                               | GH        | USD          |
| Греция                              | GR        | EUR          |
| Гватемала                           | GT        | USD          |
| Гондурас                            | HN        | USD          |
| Гонконг, САР                       | HK        | USD          |
| Венгрия                             | HU        | EUR          |
| Исландия                             | IS        | EUR          |
| Индия                               | IN        | INR          |
| Индонезия                           | ID        | USD          |
| Ирак                                | IQ        | USD          |
| Ирландия                             | IE        | EUR          |
| Израиль                              | IL        | USD          |
| Италия                               | IT        | EUR          |
| Ямайка                             | JM        | USD          |
| Япония                               | JP        | JPY          |
| Иордания                              | JO        | USD          |
| Казахстан                          | KZ        | USD          |
| Кения                               | KE        | USD          |
| Южная Корея                       | KR        | KRW          |
| Кувейт                              | KW        | USD          |
| Киргизия                          | KG        | USD          |
| Латвия                              | LV        | EUR          |
| Ливан                             | LB        | USD          |
| Ливия                               | LY        | USD          |
| Лихтенштейн                       | LI        | EUR          |
| Литва                           | LT        | EUR          |
| Люксембург                          | LU        | EUR          |
| Макао (САР)                           | MO        | USD          |
| Северная Македония                     | MK        | USD          |
| Малайзия                            | MY        | USD          |
| Мальта                               | MT        | EUR          |
| Маврикий                           | MU        | USD          |
| Мексика                              | MX        | USD          |
| Молдова                             | MD        | USD          |
| Монако                              | MC        | EUR          |
| Монголия                            | MN        | USD          |
| Черногория                          | ME        | USD          |
| Марокко                             | MA        | USD          |
| Намибия                             | Н/Д        | USD          |
| Непал                               | NP        | USD          |
| Нидерланды                         | NL        | EUR          |
| Новая Зеландия                         | NZ        | NZD          |
| Никарагуа                           | NI        | USD          |
| Нигерия                             | NG        | USD          |
| Норвегия                              | NO        | NOK          |
| Оман                                | OM        | USD          |
| Пакистан                            | PK        | USD          |
| Палестинская Автономия               | PS        | USD          |
| Панама                              | PA        | USD          |
| Парагвай                            | PY        | USD          |
| Перу                                | PE        | USD          |
| Филиппины                         | PH        | USD          |
| Польша                              | PL        | EUR          |
| Португалия                            | PT        | EUR          |
| Пуэрто-Рико                         | PR        | USD          |
| Катар                               | QA        | USD          |
| Румыния                             | RO        | EUR          |
| Россия                              | RU        | RUB          |
| Руанда                              | RW        | USD          |
| Сент-Китс и Невис               | KN        | USD          |
| Саудовская Аравия                        | SA        | USD          |
| Сенегал                             | SN        | USD          |
| Сербия                              | Сервер отчетов        | USD          |
| Сингапур                           | SG        | USD          |
| Словакия                            | SK        | EUR          |
| Словения                            | SI        | EUR          |
| ЮАР                        | ZA        | USD          |
| Испания                               | ES        | EUR          |
| Шри-Ланка                           | LK        | USD          |
| Швеция                              | SE        | SEK          |
| Швейцария                         | CH        | CHF          |
| Тайвань                              | TW        | TWD          |
| Таджикистан                          | TJ        | USD          |
| Танзания                            | TZ        | USD          |
| Таиланд                            | ВП        | USD          |
| Тринидад и Тобаго                 | TT        | USD          |
| Тунис                             | TN        | USD          |
| Турция                              | TR        | USD          |
| Туркменистан                        | TM        | USD          |
| Уганда                              | UG        | USD          |
| Украина                             | UA        | USD          |
| ОАЭ                | AE        | USD          |
| United Kingdom                      | ГБ        | GBP          |
| США                       | США        | USD          |
| США (США)                 | VI        | USD          |
| Уругвай                             | UY        | USD          |
| Узбекистан                          | UZ        | USD          |
| Ватикан             | VA        | EUR          |
| Венесуэла                           | VE        | USD          |
| Вьетнам                             | VN        | USD          |
| Йемен                               | YE        | USD          |
| Замбия                              | ZM        | USD          |
| Зимбабве                            | ZW        | USD          |
|   |   |   |

\* Для клиентов в Бразилии в Marketplace через поставщиков облачных решений (CSP) используется USD.

\** Только бесплатные образы виртуальных машин и образы с собственной лицензией.

## <a name="how-we-convert-currency"></a>Как мы преобразуем валюту

Для всех типов платных предложений независимые поставщики программного обеспечения имеют возможность вводить цены в долларах США или отправлять цены в местной валюте. Цены, указанные в долларах США, автоматически преобразуются в локальную валюту при сохранении страницы. Показатели использования центра партнеров обновляются ежедневно. Независимые поставщики программного обеспечения могут экспортировать цены и ознакомиться с преобразованными эквивалентами.

Чтобы настроить цену перед публикацией, просто экспортируйте электронную таблицу ценообразования, измените ее и отправьте с изменениями.

> [!NOTE]
> После публикации плана цены для отдельных рынков в плане нельзя будет изменить. Чтобы проверить правильность цен перед публикацией, экспортируйте электронную таблицу цен и изучите цены для каждого рынка.

## <a name="retrieving-currency-information-by-api"></a>Получение сведений о валюте по API

Отдельные цены (которые, в зависимости от того, как они были заданы), могут быть получены с помощью API. сведения о валюте не могут быть.

Дополнительные сведения о вводе цены для конкретных типов предложений см. в следующих статьях:

- [Создание предложения приложения Azure](partner-center-portal/create-new-azure-apps-offer.md)
- [Создание предложения для контейнера Azure](partner-center-portal/create-azure-container-offer.md)
- [Создание предложения виртуальной машины Azure](partner-center-portal/azure-vm-create-offer.md)
- [Создание предложения консультационных услуг](partner-center-portal/create-consulting-service-offer.md)
- [Создание Dynamics 365 для предложения "участие клиента" & PowerApps](partner-center-portal/create-new-customer-engagement-offer.md)
- [Создание предложения Dynamics 365 for Operations](partner-center-portal/create-new-operations-offer.md)
- [Создание предложения Dynamics 365 Business Central](partner-center-portal/create-new-business-central-offer.md)
- [Создание предложения модуля IoT Edge](partner-center-portal/azure-iot-edge-module-creation.md)
- [Создание предложения управляемой службы](partner-center-portal/create-new-managed-service-offer.md)
- [Создание приложения Power BI](partner-center-portal/create-power-bi-app-offer.md)
- [Создание предложения SaaS](partner-center-portal/create-new-saas-offer.md)
