---
title: Создание нового предложения Dynamics 365 Business Central в коммерческом магазине
description: Как создать новое предложение Dynamics 365 Business Central для перечисления или продажи в Azure Marketplace, AppSource или с помощью программы поставщика облачных решений (CSP), используя портал коммерческого рынка в центре партнеров Майкрософт.
author: ChJenk
manager: evansma
ms.author: v-chjen
...
Нажмите кнопку **+ создать предложение** , а затем выберите пункт меню **Dynamics 365 Business Central** . Появится диалоговое окно **новое предложение** .

### <a name="offer-id-and-alias"></a>Идентификатор предложения и псевдоним

- **Идентификатор предложения**. Уникальный идентификатор для каждого предложения в вашей учетной записи. Этот идентификатор будет отображаться для клиентов в URL-адресе предложения Marketplace и шаблонов Azure Resource Manager (если применимо). Идентификатор предложения должен состоять из строчных букв и цифр (включая дефисы и знаки подчеркивания, но без пробелов), не более 50 символов и не может быть изменен после выбора параметра " **создать**".  Например, если ввести *Test-предложение-1* , URL-адрес предложения будет `https://azuremarketplace.microsoft.com/marketplace/../test-offer-1`.
...
Чтобы включить тестовый выпуск, установите флажок **включить тестовый диск** . Затем необходимо настроить демонстрационную среду в [технической конфигурации тестового диска](#test-drive-technical-configuration) , чтобы позволить клиентам использовать ваше предложение в течение фиксированного периода времени. 
...
Укажите логотип вашего предложения в двух размерах: **Малый (48 x 48)** и **крупный (216 x 216)** .
...
На странице **доступность** можно найти сведения о том, где и как сделать ваше предложение доступным.
...
Помните, что даже если выбрать все рынки, местные законы, ограничения или другие факторы, могут помешать некоторым предложениям перечислено в некоторых странах и регионах.
...
- **Добавить**: Приложение надстройки расширяет возможности и существующие функциональные возможности Dynamics 365 Business Central. Дополнительные сведения см. [в разделе надстройки приложений](https://docs.microsoft.com/dynamics365/business-central/dev-itpro/developer/readiness/readiness-add-on-apps).
- **Подключение**: Приложение Connect можно использовать в сценарии, где необходимо установить подключение "точка — точка" между Dynamics 365 Business Central и сторонним решением или службой. Дополнительные сведения см. в разделе [Connect](https://docs.microsoft.com/dynamics365/business-central/dev-itpro/developer/readiness/readiness-connect-apps).
...
Требуется, если ваше предложение должно быть установлено вместе с другим расширением, которое уже Опубликовано в Marketplace. Если это так, отправьте свой файл `.app` или `.zip`.

### <a name="url-to-app-installation"></a>URL-адрес установки приложения

Если вы выбрали **Подключиться** выше, укажите здесь URL-адрес для установки приложения.
...
- **Длительность тестового диска** (обязательно): Введите период времени, в течение которого тестовый диск будет оставаться активным, в часы. Тестовый выпуск автоматически завершит работу по истечении этого периода. Эта длительность может быть задана только целым числом часов (например, "2" часов, "1,5" недопустим).
...
- **Длительность тестового диска** (обязательно): Введите период времени, в течение которого тестовый диск будет оставаться активным, определив количество часов. Через это количество часов сеанс завершится и больше не будет использовать одну из ваших лицензий. Мы рекомендуем использовать значение 2-24 часов в зависимости от сложности вашего предложения. Эта длительность может быть задана только целым числом часов (например, "2" часов, "1,5" недопустим).  Пользователь может запросить новый сеанс в случае нехватки времени и снова получить доступ к тестовому диску.

- **URL-адрес экземпляра** (обязательно): URL-адрес, по которому клиент начнет работу с тестового диска. Обычно это URL-адрес экземпляра Dynamics 365 с установленными образцами данных (например, https://testdrive.crm.dynamics.com) ).

- **URL-адрес Web API экземпляра** (обязательно): Извлеките URL-адрес Web API для экземпляра Dynamics 365, войдя в учетную запись Microsoft 365 и перейдя к **параметрам** \&GT; **Настройка** @no__t 3gt; **Ресурсы для разработчиков** @no__t — 5GT; **Экземпляр веб-API экземпляра (URL-адрес корня службы)** , скопируйте URL-адрес, приведенный здесь (например, https://testdrive.crm.dynamics.com/api/data/v9.0).
...
- **Регион** (обязательный, раскрывающийся список с одним выбором): В настоящее время существует 26 регионов, поддерживающих Azure, где можно сделать тестовый выпуск доступным. Ресурсы для приложения логики будут развернуты в выбранном регионе. Если приложение логики содержит пользовательские ресурсы, хранящиеся в определенном регионе, убедитесь, что выбран этот регион. Лучшим способом является полное развертывание приложения логики в вашей подписке Azure на портале и проверка правильности его работы перед тем, как сделать выбор.
...
Параметр **список Marketplace** на вкладке **тестовый** выпуск содержит языки, на которых доступен тестовый диск. Сейчас доступен только **Английский язык (США)** . Выберите имя языка, чтобы ввести сведения, описывающие возможности тестового диска.
...
Необходимо плоад файл `.pdf` со списком сценариев использования ключей вашего предложения в документе (формат PDF). Все сценарии, перечисленные здесь, могут быть проверены нашей группой проверки перед утверждением вашего предложения для Marketplace.

### <a name="app-tests-automation"></a>Автоматизация тестов приложений

При необходимости вы можете отправить файл **автоматизации тестирования приложения** здесь (. app).
...
- См. сведения о состоянии завершения для каждого раздела предложения.
    ...
    - Не завершено — означает, что раздел содержит ошибки, которые необходимо исправить, или требуется предоставить дополнительные сведения. Вернитесь в раздел (ы) и обновите его.
