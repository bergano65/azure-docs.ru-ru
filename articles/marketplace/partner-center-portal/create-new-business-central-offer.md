...
Требуется, если ваше предложение должно быть установлено вместе с другим расширением, которое уже Опубликовано в Marketplace. Если да, отправьте `.app` здесь файл или. `.zip`
...
Необходимо плоад `.pdf` файл, в котором перечислены сценарии использования ключей вашего предложения, перечисленные в документе (формат PDF). Все сценарии, перечисленные здесь, могут быть проверены нашей группой проверки перед утверждением вашего предложения для Marketplace.
