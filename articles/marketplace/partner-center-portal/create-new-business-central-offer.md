...
- **Длительность тестового диска** (обязательно). Введите период времени, в течение которого тестовый диск будет оставаться активным. Тестовый выпуск автоматически завершает работу по истечении указанного периода времени. Эта длительность может задаваться только целым числом часов (например, "2" часов, "1,5" недопустимо).
