---
title: Создание нового предложения Dynamics 365 Business Central в коммерческом магазине Майкрософт
description: Как создать новое предложение Dynamics 365 Business Central для перечисления или продажи в Azure Marketplace, Microsoft AppSource или с помощью программы поставщика облачных решений (CSP), используя портал коммерческого рынка в центре партнеров Майкрософт.
author: dsindona
ms.author: dsindona
...
Перечислите свои предложения клиентам бесплатно, указав допустимый URL-адрес (начиная с *http* или *HTTPS*), где они могут получить доступ к вашему приложению.  Пример: `https://contoso.com/my-app`
...
На странице со списком предложений вы определяете сведения о предложении (например, имя, описание, изображения и т. д.).
...
В этом разделе вы можете указать логотипы и изображения, которые будут использоваться при отображении предложения для клиента. Все изображения должны быть в формате PNG. Отправьте логотип предложения в два размера: **малый (48 x 48)** и **крупный (216 x 216)**.
...
- **Добавление**: надстройка расширяет возможности и существующие функциональные возможности Dynamics 365 Business Central. Дополнительные сведения см. [в разделе надстройки приложений](https://docs.microsoft.com/dynamics365/business-central/dev-itpro/developer/readiness/readiness-add-on-apps).
...
Если вы выбрали **Подключиться** выше, укажите здесь URL-адрес для установки приложения. Для подключенных служб, которые не нуждаются в установке, укажите URL-адрес целевой страницы службы или страницу регистрации.
...
- **Длительность тестового диска** (обязательно). Введите период времени, в течение которого тестовый диск будет оставаться активным. Тестовый выпуск автоматически завершит работу по истечении этого периода. Эта длительность может задаваться только целым числом часов (например, "2" часов, "1,5" недопустимо).
...
- **URL-адрес Web API экземпляра** (обязательный): получите URL-адрес Web API для экземпляра Dynamics 365, войдя в учетную запись Microsoft 365 и перейдя к **параметрам** \&gt; **Настройка** \&gt; **Ресурсы** \&для разработчиков gt; **Экземпляр веб-API экземпляра (URL-адрес корня службы)**, скопируйте URL-адрес, `https://testdrive.crm.dynamics.com/api/data/v9.0`приведенный здесь (например,).
...
Параметр **список Marketplace** , расположенный на вкладке **тестовый** выпуск, содержит сведения о процессе работы с тестовыми дисками.
...
Необходимо передать `.pdf` файл со списком сценариев использования ключа для вашего предложения в документе (формат PDF). Все сценарии, перечисленные здесь, могут быть проверены нашей группой проверки перед утверждением вашего предложения для Marketplace.
