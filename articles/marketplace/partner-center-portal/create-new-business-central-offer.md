...
Укажите, поддерживает ли ваше решение выпуск Essentials и (или) Premium для Microsoft Dynamics 365 Business Central. Необходимо выбрать по крайней мере одно значение.

### <a name="key-usage-scenario"></a>Сценарий использования ключа

`.pdf` Отправьте файл со списком сценариев использования ключей вашего предложения, перечисленных в документе (формат PDF). Все сценарии, перечисленные здесь, могут быть проверены нашей группой проверки перед утверждением вашего предложения для Marketplace.

### <a name="app-tests-automation"></a>Автоматизация тестов приложений

Также отправьте файл **автоматизации тестов приложения** (. app).

### <a name="test-accounts"></a>Тестовые учетные записи

Если тестовая учетная запись необходима для того, чтобы наша группа сертификации правильно просмотрела ваше предложение, отправьте файл PDF,. doc или. docx с информацией о **тестовых учетных записях** .
