...
Требуется, если ваше предложение должно быть установлено вместе с другим расширением, которое уже Опубликовано в Marketplace. Если да, отправьте его `.app` или `.zip` файл здесь.
...
Необходимо передать файл `.pdf`, в котором перечислены сценарии использования ключа вашего предложения, перечисленные в документе (формат PDF). Все сценарии, перечисленные здесь, могут быть проверены нашей группой проверки перед утверждением вашего предложения для Marketplace.
