...
Необходимо плоад файл `.pdf`, в котором перечислены сценарии использования ключей вашего предложения, перечисленные в документе (формат PDF). Все сценарии, перечисленные здесь, могут быть проверены нашей группой проверки перед утверждением вашего предложения для Marketplace.
