...
| `authorization`   | [Получение токена носителя JSON Web Token (JWT).](https://docs.microsoft.com/azure/marketplace/partner-center-portal/pc-saas-registration#get-a-token-based-on-the-azure-ad-app) Примечание. при выполнении HTTP-запроса префикс `Bearer`ся к маркеру, полученному по ссылке, на которую указывает ссылка. |
