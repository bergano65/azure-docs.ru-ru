...
- **Идентификатор предложения**: уникальный идентификатор для каждого предложения в вашей учетной записи. Этот идентификатор будет отображаться для клиентов в URL-адресе предложения Marketplace и шаблонов Azure Resource Manager (если применимо). Идентификатор предложения должен содержать строчные буквы и цифры (включая дефисы и символы подчеркивания, но не пробелы). Длина идентификатора ограничена 50 символами и не может быть изменена после нажатия на кнопку " **создать**". Например, если ввести *Test-предложение-1* , URL-адрес предложения будет иметь `https://azuremarketplace.microsoft.com/marketplace/../test-offer-1`значение.
...
Выберите минимум одну и не более трех категорий. **Категория** будет использоваться для размещения предложения в соответствующих областях поиска Marketplace. Обязательно вызовите, как ваше предложение поддерживает эти категории в описании предложения.
...
Логотип вашего предложения можно предоставить в трех размерах: **малый (48 x 48)**, **крупный (216 x 216)** и **Wide (255 x 115)**. Небольшие и крупные размеры являются обязательными.
...
Необходимо отправить PDF-файл со списком сценариев использования ключа предложения в документе (формат PDF). Все сценарии, перечисленные здесь, могут быть проверены нашей группой проверки перед утверждением вашего предложения для Marketplace.
