...
После установки состояния удержания выплаты **Вкл.** , все выплаты будут удерживаться, пока не переместить ползунок в положение **Выкл**. В этом случае вы будете включены в следующий ежемесячный цикл выплат (если достигнуто пороговое значение выплат). Например, если изменилась работа выплатам удержание, но бы хотели о выплатах, созданный в июне, а затем убедитесь, что для переключения о выплатах хранения состояния для **Off** до конца мая.
