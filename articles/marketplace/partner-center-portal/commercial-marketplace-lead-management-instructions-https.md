...
Если система управления взаимоотношениями с клиентами (CRM) явно не поддерживается в Партнерском центре для получения интересов Azure Marketplace и AppSource, вы можете использовать конечную точку HTTPS в MS Flow для обработки этих интересов. С точки зрения https эти интересы могут быть отправлены в виде уведомления по электронной почте или могут быть записаны в систему управления отношениями с клиентами (CRM), поддерживаемую MS Flow. Инструкции в этой статье будут провести вас через основной процесс для создания нового потока с помощью Microsoft Flow, который будет генерировать HTTP POST URL, который вы войдете в издательский портал для руководства управления > **httpS Endpoint URL** поля. Кроме того, включены инструкции о том, как вы можете проверить свой поток с помощью инструмента под названием [Почтальон,](https://www.getpostman.com/downloads/) который можно найти в Интернете.
...
4. Вставьте URL HTTP POST из потока, созданного в MS Flow, где он говорит *URL-адрес запроса Enter.*
