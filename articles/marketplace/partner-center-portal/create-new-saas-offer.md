...
![Обзор предложения в центре партнеров](./media/commercial-marketplace-offer-overview.png)
...
**Технические конфигурации** вкладке определяет технические сведения (URL-адрес веб-перехватчика, идентификатор клиента и идентификатор приложения), используемый для подключения к вашему предложению. Это подключение позволяет подготовить свое предложение как ресурс в подписке Azure клиента, если они решили получить его.

- **Целевой URL-адрес страницы** (обязательно): Определение сайта, URL-адрес, клиентам будут направляться должны переходить на после получения вашего предложения в Marketplace. Этот же URL-адрес выполняет роль конечной точки, где будут размещены API-интерфейсы подключения для совместного бизнеса с корпорацией Майкрософт.

- **Подключение веб-перехватчика** (обязательно): Для всех асинхронных событий, которые корпорация Майкрософт должна отправлять вам от имени клиента (например, Подписка Azure была восстановлена до момента недопустимый), необходимо предоставить веб-перехватчика подключения. Если у вас нет системы веб-перехватчика на месте, простейшая конфигурация — чтобы приложение логики конечной точки HTTP, который будет прослушивать все события, публиковать к нему и обрабатывать их соответствующим образом (например https://prod-1westus.logic.azure.com:443/work). Подробнее см. статью [Вызовы, срабатывания триггеров и создание вложенных рабочих процессов в приложениях логики](https://docs.microsoft.com/azure/logic-apps/logic-apps-http-endpoint).
...
- **Ключ приложения Azure AD** (обязательно): Введите в Azure Active Directory (AD) [ключ приложения](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal#get-application-id-and-authentication-key). Чтобы найти этот идентификатор, перейдите по адресу [портала Azure](https://portal.azure.com/), перейдите на вкладку Active Directory в меню слева, выберите **регистрация приложений** , а затем выберите **параметры**  >  **Ключи**.
