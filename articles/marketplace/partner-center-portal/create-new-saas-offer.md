...
Пример: Test-Offer-1
<br>В результате получается URL-адрес:`https://azuremarketplace.microsoft.com/marketplace/../test-offer-1`
...
#### <a name="saas-on-azure-billing-infrastructure-costs"></a>Затраты на инфраструктуру выставления счетов на SaaS в Azure
Если предложение SaaS размещается в Azure, то в качестве издателя необходимо учитывать сборы за использование инфраструктуры Azure, а также стоимость лицензирования программного обеспечения в виде единой номенклатуры затрат. Эта стоимость представляется как фиксированная ежемесячная плата клиенту. Управление использованием инфраструктуры Azure осуществляется и выставляется за вас, партнером напрямую. Фактическая стоимость использования инфраструктуры не видна пользователю. Как правило, издатели добавляют плату за использование инфраструктуры Azure в счета за лицензирование программного обеспечения. 

Плата за лицензирование программного обеспечения представлена в виде ежемесячной, повторяющейся платы с фиксированной курсовой ставкой на основе сайта и не зависит от лимита и потребления.
...
- **Канала Сделать предложение доступным в программе** CSP (флажок)
...
- **Включение тестового диска** Установка 
...
Выберите до двух (2) отраслей, используемых для группировки предложения в соответствующие области поиска Marketplace. Если ваше предложение не относится к отрасли, не выбирайте его. Вызовите, как ваше предложение поддерживает выбранные отрасли в описании предложения. 
...
- **Имя** (обязательно): Имя, заданное здесь, будет отображаться в качестве заголовка вашего предложения в выбранном Marketplace. Имя предварительно заполнено на основе предыдущей записи **нового предложения** .  Это может быть товарным знаком.  Он не должен содержать пробелы, эмодзи (если они не являются товарными знаками и символами авторского права) и не должны быть ограничены 50 символами.
- **Сводка по** (обязательно): Укажите краткое описание предложения, которое будет использоваться в результатах поиска в списке Marketplace. В этом поле можно указать до 100 символов текста.
...
1. Если вы хотите добавить маркированный **список элементов**, поместите текст в `<li>` Теги ниже. Вы можете копировать и `<li>` вставлять дополнительные маркированные элементы (элементы между тегами и `</li>` ) внутри `<ul>` тегов и `</ul>` . Убедитесь, что вы добавили `<ul></ul>`. 
...
1. Чтобы **Добавить** полужирное начертание, добавьте `<b>` в начало текста, который нужно выделить полужирным `</b>` шрифтом, и добавьте его в конец текста, который нужно выделить полужирным шрифтом. 
...
1. Чтобы добавить **разрывы строк** между содержимым, `<br>` добавьте перед содержимым, которое нужно начать с новой строки. Если вы хотите оставить пробел и убедиться, что содержимое начинается на новой строке, добавьте `<br><br>` перед содержимым. 
...
1. Если вы хотите **увеличить размер текста**, сначала выберите размер текста. Используйте приведенные ниже примеры. После выбора размера текста добавьте соответствующие `<H*></H*>` теги в начало и конец текста. 
...
- **Полезные ссылки** (необязательно): Дополнительные дополнительные интерактивные документы о приложении или связанных службах, указанных в списке, указываются **заголовок** и **URL-адрес**. Добавьте дополнительные полезные ссылки, щелкнув **+ Добавить URL-адрес**.

#### <a name="contact-information"></a>Контактные данные

- **Контакты**: Для каждого контактного лица клиента укажите **имя** сотрудника, **номер телефона**и адрес **электронной почты** .  (Они *не будут* отображаться в открытом виде). Для группы **контактов поддержки** также требуется **URL-адрес поддержки** .  (Эти сведения *будут* отображены в открытом виде).

**Контакт службы поддержки** (обязательно): Для общих вопросов о поддержке.

**Контактное лицо** по проектированию (обязательно): Для получения технических вопросов.

**Контактное лицо диспетчера каналов** (обязательно): Вопросы по торговым посредникам, связанным с программой CSP.
...
- **Образы** (необязательно): Существует несколько мест, где изображения логотипа вашего предложения могут отображаться в Marketplace, и для них требуются следующие размеры: 48 x 48 пикселей _(обязательный),_ средний: 90 x 90 пикселей, крупный: 216 x 216 пикселей _(обязательно),_ ширина: 255 x 115 пикселей и Hero: 815 x 290 пикселей. Все образы должны быть в. Формат PNG.
- **Снимки экрана** (обязательно): Добавление снимков экрана, демонстрирующих ваше предложение. Можно добавить до пяти (5) снимков экрана, размер которых должен составлять 1280 x 720 пикселей. Все образы должны быть в. Формат PNG.
- Видеоролики (необязательно): Добавьте ссылки на видео, демонстрирующие ваше предложение. Можно использовать ссылки на видео YouTube и (или) Vimeo, которые будут отображаться клиентам вместе с вашим предложением. Также необходимо будет ввести эскиз видео, размер которого составляет 1280 x 720 пикселей в формате PNG. Для каждого предложения можно отобразить не более четырех видео.
...
- **Определите аудиторию предварительного просмотра: Добавьте одну электронную учетную запись AAD/MSA в строку вместе с дополнительным описанием.**
...
- **Имя.** Предварительно заполнено на основе новой записи **плана** предварительного просмотра и будет отображаться в качестве заголовка "плана программного обеспечения" для вашего предложения, отображаемого в Marketplace.
...
**На основе места:** Включите доступ к вашему предложению с учетом цены на количество пользователей, обращающихся к предложению или занимающие рабочие *места.* Эта модель на основе этого места позволяет задать минимальное и максимальное число допустимых рабочих мест в зависимости от цены. Таким образом можно настроить различные ценовые точки на основе числа пользователей, настроив несколько планов.  Эти поля необязательные. Если оставить это поле пустым, число рабочих мест будет интерпретироваться как не имеющее ограничений (минимум 1 и максимум в том количестве, которое может поддерживать система). Эти поля могут быть изменены в рамках обновления плана.
...
- **Срок выставления счетов**: Ежемесячно или ежегодно
