---
title: Панель мониторинга использования в службе анализа коммерческих рынков Майкрософт
description: Сведения о том, как получить доступ ко всем виртуальным машинам, см. в статье Использование и отслеживание метрик оплаты. Перейдите на панель мониторинга "использование" в центре партнеров в разделе "коммерческие рынки".
author: dsindona
ms.author: dsindona
...
ms.locfileid: "82793012"
---
# <a name="usage-dashboard-in-microsoft-commercial-marketplace-analytics"></a>Панель мониторинга использования в службе анализа коммерческих рынков Майкрософт
...
Панель мониторинга использования представляет метрики для всех виртуальных машин (VM), которые обеспечивают использование и лимитное использование счетов. Эти метрики находятся на двух отдельных вкладках: использование виртуальной машины и лимитное использование счетов.
...
- Часы использования необработанных данных определяются как количество времени, в течение которого виртуальные машины работают в часах.
...
- На круговой диаграмме " **нормализованная загрузка по предложениям** " отображается разбивка нормализованных часов использования по предложениям в соответствии с выбранным диапазоном дат. Пять лучших предложений отображаются в Graph, а остальные группируются в категории **остальные** .
...
На этой диаграмме показана тенденция нормализованного использования для выбранных номеров SKU предложения. Предложение список лидеров отображает первые 50 предложений с самым высоким потреблением, отсортированные по часам использования. Номер SKU список лидеров отображает первые номера SKU 50 с наибольшим использованием для выбранного предложения.
...
- Если число записей превышает 1000, экспорт данных будет асинхронно помещен на страницу загрузки, которая будет доступна в течение следующих 30 дней.
...
- Примененные фильтры показывают выбор количества для выбранных фильтров. Примененные фильтры не отображаются для параметров по умолчанию.
...
- Чтобы просмотреть объединенное представление отзывов клиентов о предложениях на Microsoft AppSource и Azure Marketplace, см. раздел [панель мониторинга "оценки и рецензии" в статье анализ коммерческих рынков](./ratings-reviews.md).
