...
- **URL-адрес экземпляра** (обязательно). URL-адрес, по которому клиент начнет свой тестовый диск. Обычно это URL-адрес экземпляра Dynamics 365, на котором установлено приложение с демонстрационными данными ( `https://testdrive.crm.dynamics.com`например,).
