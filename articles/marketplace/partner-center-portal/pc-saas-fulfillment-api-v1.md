---
title: Выполнение SaaS API-интерфейсов версии 1 | Azure Marketplace
description: Объясняется, как создавать и управлять ими на предложение SaaS в Azure Marketplace, используя соответствующие API-интерфейсы выполнения v1.
...
ms.locfileid: "66258944"
---
# <a name="saas-fulfillment-apis-version-1-deprecated"></a>API-интерфейсы выполнения SaaS версии 1 (устаревшая версия)
...
> [!WARNING]
> Этот начальной версии API SaaS выполнения устарел. Вместо этого используйте [API V2 SaaS выполнения](./pc-saas-fulfillment-api-v2.md).  Это начальное версии API в настоящее время поддерживается только для того, для обслуживания существующих издателей. 
...
| x-ms-correlationid | Нет           | Уникальное строковое значение для операции на стороне клиента. Это поле сопоставляет все события из клиентской операции с событиями на стороне сервера. Если это значение не указано, оно создается случайным образом и возвращается в заголовке ответа. |
...
|------------------|---------------|--------------------------------------|
| planId           | Строковое значение (обязательное)        | Идентификатор плана пользователя службы SaaS, подписка на.  |
|  |  |  |
...
| status             | Enum          | Состояние операции: `In Progress`, `Succeeded` или `Failed`.          |
...
| **Имя параметра**     | **Тип данных** | **Описание**                               |
|------------------------|---------------|-----------------------------------------------|
| id                     | String        | Ресурс подписки идентификатор SaaS в Azure    |
| offerId                | String        | Идентификатор предложения, который подписан пользователь         |
| planId                 | String        | Идентификатор плана, который подписан пользователь          |
| saasSubscriptionName   | String        | Имя подписки SaaS                |
| saasSubscriptionStatus | Enum          | Состояние операции.  Один из следующих типов:  <br/> - `Subscribed`: подписка активна;  <br/> - `Pending`: пользователь создал ресурс, но независимый поставщик программного обеспечения еще не активировал его;   <br/> - `Unsubscribed`: пользователь отменил подписку;   <br/> - `Suspended`: пользователь приостановил подписку;   <br/> - `Deactivated`:  подписка Azure отключена.  |
| created                | DateTime      | Значение отметки времени создания подписки в формате UTC |
| lastModified           | DateTime      | Подписки изменить значение отметки времени в формате UTC |
|  |  |  |
