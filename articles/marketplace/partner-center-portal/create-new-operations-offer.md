---
title: Создание нового предложения Dynamics 365 для операций на коммерческом рынке
description: Как создать новое предложение Dynamics 365 для операций для листинга или продажи в Azure Marketplace, AppSource или через программу Cloud Solution Provider (CSP) с помощью портала Commercial Marketplace в партнерской центре Microsoft.
author: dsindona
ms.author: dsindona
...
ms.locfileid: "80294355"
---
# <a name="create-a-new-dynamics-365-for-operations-offer"></a>Создание нового предложения Dynamics 365 для операций

В этой теме объясняется, как создать новое предложение Dynamics 365 для операций. [Microsoft Dynamics 365 для финансов и операций](https://dynamics.microsoft.com/finance-and-operations) — это служба планирования ресурсов (ERP), которая поддерживает передовые финансы, операции, производство и управление цепочками поставок. Все предложения для Dynamics 365 для операций должны пройти через наш процесс сертификации.

Чтобы начать создание Dynamics 365 для предложений по операциям, убедитесь, что сначала [создадите учетную запись Центра партнеров](./create-account.md) и откроете [панель мониторинга Commercial Marketplace](https://partner.microsoft.com/dashboard/commercial-marketplace/offers)с выбранной страницей **Обзор.**
...
Выберите кнопку **«Новое предложение»,** а затем выберите пункт меню **Dynamics 365 для меню Operations.** Появится диалоговая коробка **Нового предложения.**

### <a name="offer-id-and-alias"></a>Идентификатор предложения и псевдоним

- **Идентификатор предложения**: Уникальный идентификатор для каждого предложения в вашей учетной записи. Этот идентификатор будет виден клиентам в URL-адресе для предложений на рынке и шаблонах Azure Resource Manager (если это применимо). Идентификатор предложения должен быть в нижнем регистре алфавитных символов (включая дефисы и подчеркнутые, но не белое пространство). Этот идентификатор ограничен 50 символами и не может быть изменен после выбора **Create.**  Например, если вы введете *тест-предложение-1* `https://azuremarketplace.microsoft.com/marketplace/../test-offer-1`здесь, URL-адрес предложения будет .
...
Выберите минимум одну и максимум три категории. Эти категории будут использоваться для размещения вашего предложения в соответствующих областях поиска рынка. Не забудьте позвонить, как ваше предложение поддерживает эти категории в описании предложения.
...
В этом разделе укажите имя, электронную почту и номер телефона для **контакта поддержки** и **инженерного контакта.** Эта информация не отображается клиентам, но будет доступна корпорации Майкрософт и может быть предоставлена партнерам CSP.

В разделе **контактподдержки поддержки** предоставьте **URL-адрес поддержки,** где партнеры CSP могут найти поддержку для вашего предложения.
...
Предоставьте логотип вашего предложения в двух размерах: **Малый (48 x 48)** и **Большой (216 x 216)**.
...
По умолчанию рынки не выбираются. Выберите хотя бы один рынок, чтобы опубликовать ваше предложение. Нажмите **Выберите все,** чтобы сделать ваше предложение доступным на всех возможных рынках, или выберите конкретные рынки, которые вы хотите добавить. После того как вы закончите, выберите **Сохранить**.
...
Имейте в виду, что даже если вы выберете все рынки, местные законы и ограничения или другие факторы могут помешать некоторым предложениям быть перечислены в некоторых странах и регионах.
...
### <a name="solution-identifier"></a>Идентификатор решений

Предоставьте идентификатор решения (GUID) для вашего решения.

Чтобы найти идентификатор решения:
1. В службах жизненного цикла Microsoft Dynamics (LCS) выберите **управление решениями.**
2. Выберите решение, а затем ищите **идентификатор решения** в **обзоре пакета.** Если идентификатор пуст, выберите **истрим** и переиздайте пакет, повторите попытку.

### <a name="release-version"></a>Версия выпуска

Выберите версию Dynamics 365 для финансов и операций, с которым работает это решение.
...
- **Регион** (обязательный список выпадающих данных по одному выбору): В настоящее время существует 26 регионов, поддерживаемых Azure, где тест-драйв может быть доступен. Ресурсы для приложения Logic будут развернуты в выбранном регионе. Если ваше приложение Logic app имеет какие-либо пользовательские ресурсы, хранящиеся в определенном регионе, убедитесь, что регион выбран здесь. Лучший способ убедиться, что у вас есть пользовательские ресурсы для вашего региона, это полностью развернуть приложение Logic App локально в подписке Azure на портале и убедиться, что оно функционирует правильно, прежде чем сделать этот выбор.
...
Загрузите [отчет об анализе настройки (CAR)](https://docs.microsoft.com/dynamics365/unified-operations/dev-itpro/dev-tools/customization-analysis-report) в этом разделе. Этот отчет генерируется путем анализа моделей настройки и расширения на основе заданного набора правил наилучшей практики.

Этот файл должен быть в формате .xls или .xlsx. Если у вас есть несколько отчетов, вы можете загрузить файл .zip, содержащий все отчеты.

### <a name="does-solution-include-localizations"></a>Включает ли решение локализацию?

Выберите **«Да»,** если решение позволяет использовать местные стандарты и политики (например, если оно учитывает различные правила расчета заработной платы, требуемые различными странами/регионами). В противном случае нажмите кнопку **Нет**.

### <a name="does-solution-enable-translations"></a>Позволяет ли решение переводы?

Ответьте **да,** если текст в вашем решении может быть переведен на другие языки. В противном случае нажмите кнопку **Нет**.
