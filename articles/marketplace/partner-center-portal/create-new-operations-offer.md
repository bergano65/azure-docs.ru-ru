---
title: Создание предложения Dynamics 365 for Operations в коммерческом магазине
...
3. На странице Обзор выберите **+ New предложение** > **Dynamics 365 для операций**.

    ![Показывает меню навигации слева.](./media/new-offer-dynamics-365-ops.png)
...
Выберите минимум одну и не более трех категорий. Эти категории будут использоваться для размещения предложения в соответствующих областях поиска Marketplace. Обязательно вызовите, как ваше предложение поддерживает эти категории в описании предложения.
...
Введите краткое описание предложения (до 100 символов). Это описание можно использовать в результатах поиска Marketplace.
...
В этом разделе Укажите имя, адрес электронной почты и номер телефона для **контакта службы поддержки** и **контактного лица по проектированию**. Эти сведения не отображаются для клиентов, но будут доступны корпорации Майкрософт и могут быть предоставлены партнерам CSP.

В разделе **контакт службы поддержки** укажите **URL-адрес поддержки** , по которому партнеры CSP могут найти поддержку для вашего предложения.

### <a name="supporting-documents"></a>Вспомогательные документы

Предоставьте по меньшей мере один (и до трех) связанные маркетинговые документы, такие как технические документы, буклеты, контрольные списки или презентации. Эти документы должны быть в формате PDF.
...
Укажите эмблему вашего предложения в двух размерах:
...
По умолчанию не выбраны рынки. Выберите по крайней мере один из рынков, чтобы опубликовать предложение. Щелкните **выбрать все** , чтобы сделать предложение доступным на всех возможных рынках, или выберите конкретные рынки, которые требуется добавить. После завершения нажмите кнопку **сохранить**.
...
- **Длительность тестового диска** (обязательно) — введите период времени, в течение которого тестовый диск будет оставаться активным, в часы. Тестовый выпуск автоматически завершит работу по истечении этого периода. Эта длительность может задаваться только целым числом часов (например, "2" часов; Недопустимый "1,5").
...
- **Регион** (требуется, раскрывающийся список с одним выбором). в настоящее время существует 26 регионов, поддерживающих Azure, где можно сделать тестовый диск доступным. Ресурсы для приложения логики будут развернуты в выбранном регионе. Если приложение логики содержит пользовательские ресурсы, хранящиеся в определенном регионе, убедитесь, что выбран этот регион. Лучший способ обеспечить доступность настраиваемых ресурсов для вашего региона — это полностью развернуть приложение логики в подписке Azure на портале и проверить, правильно ли он работает, прежде чем делать выбор.
...
- **Видео** (необязательно) — видео можно загрузить в YouTube или Vimeo и ссылаться на него с помощью изображения со ссылкой и эскизом (533 x 324 пикселей), чтобы пользователь мог просмотреть пошаговые инструкции, чтобы помочь им лучше понять тестовый диск, а также как успешно использовать возможности вашего предложения и понять сценарии, которые выделяют их преимущества.
  ...
  - **Эскиз** (изображение должно быть в формате PNG и 533 x 324 пикселей)
...
Отправьте [отчет об анализе настройки (автомобиль)](https://docs.microsoft.com/dynamics365/unified-operations/dev-itpro/dev-tools/customization-analysis-report) в этом разделе. Этот отчет создается путем анализа настроек и моделей расширения на основе предопределенного набора рекомендаций.
...
- См. сведения о состоянии завершения для каждого раздела предложения.
    ...
    - Не **завершено** — раздел содержит ошибки, которые необходимо исправить, или требуется предоставить дополнительные сведения. Вернитесь в раздел (ы) и обновите его.
