---
title: Создание технических ресурсов виртуальных машин Azure
...
ms.locfileid: "89646808"
---
# <a name="create-azure-virtual-machine-technical-assets"></a>Создание технических ресурсов виртуальных машин Azure
...
1. **Виртуальный жесткий диск операционной системы** — содержит операционную систему и решение, которое развертывается вместе с вашим предложением. Процесс подготовки виртуального жесткого диска зависит от того, является ли виртуальная машина на основе Linux или на основе Windows.

2. **Виртуальные жесткие диски с данными** — выделенное, постоянное хранилище для виртуальной машины. Не используйте виртуальный жесткий диск операционной системы (например, диск C:) для хранения постоянных данных.
...
5. Выберите изображение из раскрывающегося списка или выберите **Обзор все общедоступные и частные образы** для поиска или просмотра всех доступных образов виртуальных машин. Пример
    ...
    :::image type="content" source="media/vm/create-resource-group-example.png" alt-text="Показывает пример образа виртуальной машины.":::
...
6. Выберите размер развертываемой виртуальной машины, используя следующие рекомендации:
    ...
    :::image type="content" source="media/vm/create-virtual-machine.png" alt-text="Показывает выбор размера виртуальной машины.":::

7. В разделе **Диски** разверните раздел **Расширенный** и установите для параметра **Использовать управляемые диски** значение **Нет**.

    :::image type="content" source="media/vm/use-managed-disks.png" alt-text="Показывает параметр для использования управляемых дисков.":::
...
9. Чтобы завершить создание виртуальной машины, выполните [процедуру создания на портале Azure](https://docs.microsoft.com/azure/virtual-machines/windows/quick-create-portal).
    ...
    :::image type="content" source="media/vm/vm-generation.png" alt-text="Показывает параметр для выбора создания виртуальной машины.":::
...
8. В диалоговом окне конфигурации PuTTY введите IP-адрес или DNS-имя своей виртуальной машины.
    ...
    :::image type="content" source="media/vm/putty-configuration.png" alt-text="Показывает параметры терминала, выделяющие поля имя узла и порт.":::
...
| --- | --- | --- |
| >500 ТБ | Н/Д | Обратитесь в службу поддержки за исключением утверждения. |
