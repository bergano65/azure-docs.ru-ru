---
title: Управление интересами с помощью хранилища таблиц Azure — коммерческий рынок Майкрософт
description: Узнайте, как использовать хранилище таблиц Azure для настройки интересов для Microsoft AppSource и Azure Marketplace.
author: qianw211
...
1. Дождитесь подготовки учетной записи хранения. Этот процесс обычно занимает несколько минут. 

## <a name="create-a-table-in-your-storage-account"></a>Создание таблицы в учетной записи хранения

1. На **домашней** странице портал Azure выберите **Просмотреть все ресурсы** , чтобы получить доступ к вашей учетной записи хранения. Также можно выбрать **все ресурсы** в левой строке меню портал Azure.
...
## <a name="optional-use-power-automate-to-get-lead-notifications"></a>Используемых Использование Power автоматизиру для получения ведущих уведомлений

Вы можете использовать [автоматизацию](https://docs.microsoft.com/flow/) , чтобы автоматизировать уведомления при каждом добавлении интереса в таблицу службы хранилища Azure. Если у вас нет учетной записи, вы можете [зарегистрироваться для бесплатной учетной записи](https://flow.microsoft.com/).

### <a name="lead-notification-example"></a>Пример уведомления о потенциальном клиенте

В примере создается последовательность, которая автоматически отправляет уведомление по электронной почте при добавлении нового интереса в хранилище таблиц Azure. В этом примере при обновлении табличного хранилища настраивается повторение для отправки сведений об интересах каждый час.
...
1. В окне **Получение времени в прошлом** задайте для параметра **интервал** значение **1**. В раскрывающемся списке **единица времени** выберите значение **час**.
   ...
   В следующем наборе действий вы будете подключаться к таблице и настроили логику обработки для обработки новых интересов.

1. Выберите **+ новый шаг**. Затем найдите в окне **Выбор действия пункт** **получить сущности** .
...
1. В окне **хранилище таблиц Azure** введите сведения для следующих полей и выберите **создать**:
    ...
    * **Имя учетной записи хранения**: укажите имя учетной записи хранения для таблицы. Это имя можно найти на странице **ключи доступа** учетной записи хранения.
    * **Общий ключ хранилища**: Укажите значение ключа для своей учетной записи хранения в таблице. Это значение можно найти на странице **ключи доступа** учетной записи хранения.
   ...
   * **Таблица**: выберите имя таблицы (из таблицы [создать таблицу](#create-a-table-in-your-storage-account)). На следующем рисунке показан запрос, когда для этого примера выбрана таблица "маркетплацелеадс".
...
1. Во всплывающем окне **сведения о подключении** выберите **Таблица Azure** для **назначения интереса**. 
