...
5.    На *сборке запланированное* окно потока под *Повторение каждого* выбора "1" для интервала и "час" для частоты. Кроме того, дать потоку имя, если хотите. Нажмите кнопку **создания**.
...
* *Общий ключ хранения* — укажите ключевое значение для учетной записи магазина для таблицы Azure. Вы можете найти это на странице **ключей доступа** учетной записи хранилища.
    ![Хранилище таблицы Azure.](./media/commercial-marketplace-lead-management-instructions-azure-table/azure-table-storage.png)
...
* *Фильтровый запрос* - Выберите это поле и `Timestamp gt datetime'@{body('Get_past_time')}'` ![вставьте эту функцию в поле: Таблица Azure получает сущности - Фильтр-запрос.](./media/commercial-marketplace-lead-management-instructions-azure-table/azure-table-get-entities-filter-query.png)
