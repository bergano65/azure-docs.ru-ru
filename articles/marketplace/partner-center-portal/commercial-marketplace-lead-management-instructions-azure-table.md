...
19. В окне **Office 365 Outlook** введите сведения для следующих полей:
    ...
    1. **Тема** — укажите тему электронного письма. Пример: Новые потенциальные клиенты!
    1. Текст. Добавьте текст, который необходимо включить в каждый адрес электронной почты (необязательно), а затем вставьте в `body('Get_entities')?['value']`текст.
...
20. Выберите **Сохранить**, чтобы сохранить поток. Microsoft Flow автоматически проверит поток на наличие ошибок. Если они отсутствуют, поток запустится после его сохранения.
