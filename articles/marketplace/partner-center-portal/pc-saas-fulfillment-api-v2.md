...
В этой статье подробно описаны интерфейсы API выполнения, позволяющие партнерам продавать приложения SaaS в Microsoft AppSource и Azure Marketplace. Эти API-интерфейсы являются обязательными для transactических предложений SaaS.
...
|  `action`                | Действие, выполняемое над `Unsubscribe`ресурсом:, `Suspend`, `Reinstate`или `ChangePlan`,. `ChangeQuantity` `Transfer` |
|   |   |
...
##### <a name="deletebr-httpsmarketplaceapimicrosoftcomapisaassubscriptionssubscriptionidapi-versionapiversion"></a>Удалить<br> `https://marketplaceapi.microsoft.com/api/saas/subscriptions/<subscriptionId>?api-version=<ApiVersion>`
...
Позволяет издателю контролировать состояние `Subscribe`указанной асинхронной операции, активируемой триггером (например, `Unsubscribe` `ChangePlan`, или `ChangeQuantity`).
