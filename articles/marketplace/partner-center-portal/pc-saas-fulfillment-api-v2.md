...
Когда клиент инициирует покупку, он получает эти сведения в коде авторизации на интерактивной веб-странице клиента, использующей параметр URL-адреса. Например, `https://contoso.com/signup?token=..`, а URL-адрес целевой страницы в центре партнеров — `https://contoso.com/signup`. Код авторизации можно проверить и переключить на сервер для получения сведений о службе подготовки, вызвав API разрешения.  Когда служба SaaS завершает подготовку, она отправляет вызов активации, чтобы сообщить о завершении выполнения и выплата за использование клиента. 
...
#### <a name="unsubscribed"></a>Отменена 
...
В этом разделе документируется *API подписки* SaaS и *API операций*.  Значение параметра `api-version` для API-интерфейсов версии 2 — `2018-08-31`.  
...
|  `action`                | Действие, выполняемое над ресурсом: `unsubscribe`, `suspend`, `reinstate` или `changePlan`, `changeQuantity`, `transfer`.  |
|   |   |
...
|  authorization     |  [Получение токена носителя JSON Web Token (JWT)](https://docs.microsoft.com/azure/marketplace/cloud-partner-portal/saas-app/cpp-saas-registration#get-a-token-based-on-the-azure-ad-app). Например: "`Bearer <access_token>`". |
...
Код: 403<br>
Не авторизовано. Маркер проверки подлинности не указан или недопустим, либо запрос пытается получить доступ к приобретению, который не принадлежит текущему издателю.
...
API подписки поддерживает следующие операции HTTPS: **Get**, **POST**, **Patch**и **Delete**.
...
>[!Note]
>[Макеты API](#mock-apis) используются при первой разработке предложения, а реальные API-интерфейсы должны использоваться при публикации предложения.  Реальные API-интерфейсы и макеты API-интерфейсов отличаются первой строкой кода.  В реальном API имеется раздел `subscription`, в то время как этот раздел не существует для макетов API.

Полезные данные ответа для макета API:<br>
...
Код: 403<br>
Не авторизовано. Маркер проверки подлинности не указан или недопустим, либо запрос пытается получить доступ к приобретению, который не принадлежит текущему издателю. <br> 
...
>[!Note]
>Только план или количество могут быть исправлены одновременно, но не оба. Изменения в подписке с **обновлением** не `allowedCustomerOperations`.
...
##### <a name="deletebr-httpsmarketplaceapimicrosoftcomapisaassubscriptionssubscriptionid-api-versionapiversion"></a>Удаление<br> `https://marketplaceapi.microsoft.com/api/saas/subscriptions/<subscriptionId> ?api-version=<ApiVersion>`
...
Код: 400<br>
Удаление в подписке с **удалением** , не `allowedCustomerOperations`.
...
Позволяет издателю контролировать состояние указанной асинхронной операции, активируемой триггером (например, `subscribe`, `unsubscribe`, `changePlan` или `changeQuantity`).
...
- `unsubscribe` (при удалении ресурса)
...
Конечная точка узла: `https://marketplaceapi.microsoft.com/api` (проверка подлинности не требуется)<br/>
Версия API: `2018-09-15`<br/>
Пример URI: `https://marketplaceapi.microsoft.com/api/saas/subscriptions?api-version=2018-09-15` <br/>

Пути к конечной точке API одинаковы для макетов и реальных интерфейсов API, но версии API различны. Версия `2018-09-15` для макетной версии и `2018-08-31` для рабочей версии. 
