...
Правилу авторизации назначается *первичный* и *вторичный ключ*. Это криптографически строгие ключи. Не потеряйте их или утечек, они всегда будут доступны в [портал Azure][Azure portal]. Вы можете использовать любой из созданных ключей и создавать их повторно в любое время. Если вы повторно создадите или измените ключ в политике, все ранее выданные маркеры на его основе моментально станут недействительными. Тем не менее текущие подключения, созданные на основе этих маркеров, продолжат работать до истечения срока их действия.
...
**URI должен быть [закодирован](/dotnet/api/system.web.httputility.urlencode?view=netframework-4.8)в процентах.**
...
Полный рабочий пример приложения служебной шины, демонстрирующий конфигурацию и использующий авторизацию SAS, см. в следующем примере в нашем репозитории GitHub: [Управление очередями служебной шины Azure](https://github.com/Azure/azure-service-bus/tree/master/samples/DotNet/Microsoft.Azure.ServiceBus/ManagingEntities/SASAuthorizationRule).
...
| Начало прослушивания в пространстве имен |Прослушать |Любой адрес пространства имен |
...
| Получение сообщений из очереди |Прослушать |Любой допустимый адрес очереди |
| Прерывание или завершение обмена сообщениями после получения сообщения в режиме чтения с блокировкой |Прослушать |Любой допустимый адрес очереди |
| Откладывание сообщения для последующего получения |Прослушать |Любой допустимый адрес очереди |
| Назначение сообщению статуса недоставленного |Прослушать |Любой допустимый адрес очереди |
| Получение состояния, связанного с сеансом очереди сообщений |Прослушать |Любой допустимый адрес очереди |
| Назначение состояния, связанного с сеансом очереди сообщений |Прослушать |Любой допустимый адрес очереди |
| Планирование последующей доставки сообщений (например, [ScheduleMessageAsync()](/dotnet/api/microsoft.azure.servicebus.queueclient.schedulemessageasync#Microsoft_Azure_ServiceBus_QueueClient_ScheduleMessageAsync_Microsoft_Azure_ServiceBus_Message_System_DateTimeOffset_)) |Прослушать | Любой допустимый адрес очереди
| **Раздел** | | |
| Создать раздел |управление |Любой адрес пространства имен |
...
| **Подписка** | | |
| Создать подписку |Управление |Любой адрес пространства имен |
...
| Прерывание или завершение обмена сообщениями после получения сообщения в режиме чтения с блокировкой |Прослушать |../myTopic/Subscriptions/mySubscription |
| Откладывание сообщения для последующего получения |Прослушать |../myTopic/Subscriptions/mySubscription |
| Назначение сообщению статуса недоставленного |Прослушать |../myTopic/Subscriptions/mySubscription |
| Получение состояния, связанного с сеансом раздела |Прослушать |../myTopic/Subscriptions/mySubscription |
| Назначение состояния, связанного с сеансом раздела |Прослушать |../myTopic/Subscriptions/mySubscription |
| **Правила** | | |
