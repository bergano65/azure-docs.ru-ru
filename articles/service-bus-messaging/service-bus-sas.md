...
Следующие шаги показывают, как отправить маркер SAS с помощью протокола AMQP, используя библиотеку [AMQP.NET Lite](https://github.com/Azure/amqpnetlite) . Это полезно, если вы не можете использовать официальный пакет SDK служебной шины (например, на платформе WinRT, .NET Compact Framework, .NET Micro Framework и Mono), разрабатывая на C\#. Разумеется, эта библиотека полезна для ознакомления с тем, как работает защита на основе утверждений на уровне AMQP. Вы уже видели, как защита работает на уровне HTTP (с отправкой запроса HTTP POST и маркера SAS в заголовке Authorization). Если вам не нужны такие глубокие знания о AMQP, вы можете использовать официальный пакет SDK служебной шины с .NET Framework приложениями, что сделает это за вас.
