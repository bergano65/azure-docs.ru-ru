---
title: Контроль доступа к автобусу Azure с общими подписями доступа
...
> [!NOTE]
> Azure Service Bus поддерживает разрешение доступа к пространстве имен ногой service Bus и его сущностям с помощью Active Directory Azure (Azure AD). Разрешение пользователей или приложений с помощью токена OAuth 2.0, возвращенного Azure AD, обеспечивает превосходную безопасность и простоту использования над общими подписями доступа (SAS). С Azure AD нет необходимости хранить маркеры в коде и рисковать потенциальными уязвимостями безопасности.
>
> Корпорация Майкрософт рекомендует использовать Azure AD с приложениями Azure Service Bus, когда это возможно. Дополнительные сведения см. в следующих статьях:
> - [Authenticate и авторизуем приложение с помощью Active Directory Azure для доступа к объектам Azure Service Bus.](authenticate-application.md)
> - [Проверка управляемого итогового данных с помощью Active Directory Azure для доступа к ресурсам Azure Service Bus](service-bus-managed-service-identity.md)
...
* **`se`**- Момент истечения срока действия токенов. Количество секунд с начала эпохи UNIX (`00:00:00 UTC` 1 января 1970 года) до истечения срока действия маркера.
* **`skn`**- Название правила авторизации.
* **`sr`**- URI доступа к ресурсу.
* **`sig`**- Подпись.

Это `signature-string` хэш SHA-256, вычисленный на ресурс URI **(область,** как описано в предыдущем разделе) и рецензированное моментом истечения срока действия токенов, разделенное LF.
...
**URI должен быть [закодирован на процент.](https://msdn.microsoft.com/library/4fkewx0t.aspx)**
...
Следующие шаги показывают, как отправить токен SAS с протоколом Am-P с помощью [библиотеки AMQP.NET Lite.](https://github.com/Azure/amqpnetlite) Это полезно, если вы не можете использовать официальный сервис Bus SDK (например, на WinRT, .NET Compact Framework, .NET Micro Framework и Mono), разрабатываемый в C\#. Разумеется, эта библиотека полезна для ознакомления с тем, как работает защита на основе утверждений на уровне AMQP. Вы уже видели, как защита работает на уровне HTTP (с отправкой запроса HTTP POST и маркера SAS в заголовке Authorization). Если вам не нужны такие глубокие знания об АМЗП, вы можете воспользоваться официальным сервисным автобусом SDK с приложениями .NET Framework, которые сделают это за вас.
...
| Отправка сообщений в прослушиватель в пространстве имен |Send |Любой адрес пространства имен |
| **Очередь** | | |
...
| Отправка в очередь |Send |Любой допустимый адрес очереди |
...
| Отправка в раздел |Send |Любой допустимый адрес раздела |
| **Подписка** | | |
