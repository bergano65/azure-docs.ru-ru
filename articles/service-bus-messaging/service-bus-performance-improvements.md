...
## <a name="prefetching-and-receivebatch"></a>Предварительная выборка и ReceiveBatch

Предварительное получение нескольких сообщений вместе с основными понятиями, имеют похожую семантику для обработки сообщений в пакете (ReceiveBatch), существует ряд небольших различий, необходимо иметь в виду при использовании словарей.

Предварительная загрузка — конфигурации (или режим) на стороне клиента (QueueClient и SubscriptionClient) и ReceiveBatch является операцией (с семантикой запроса ответа).

При использовании этих вместе, рассматривайте следующие варианты-

* Упреждающей выборки должен быть больше или равно числу сообщений, чтобы получать от ReceiveBatch.
* Предварительная выборка может осуществляться до n/3 на количество сообщений, обработанных в секунду, где n — это длительность блокировки по умолчанию.

Существуют некоторые проблемы с входящим "жадные" подход (т. е. обновление счетчик предварительной выборки очень высокий), так как он подразумевает, что сообщение заблокировано для определенного получателя. Рекомендуется попробовать out упреждающую выборку значений между пороговыми значениями, упомянутых выше и эмпирически определить объем.

## <a name="multiple-queues"></a>Несколько очередей

Если ожидаемую нагрузку невозможно будет обработать с помощью одной секционированной очереди или раздела, необходимо использовать несколько сущностей обмена сообщениями. При использовании нескольких сущностей создайте выделенный клиент для каждой сущности. Не используйте один клиент для всех сущностей.

## <a name="development-and-testing-features"></a>Возможности для разработки и тестирования

В служебной шине имеется одна функция, используемая специально для разработки, которую **никогда не следует использовать в рабочих конфигурациях**: [TopicDescription.EnableFilteringMessagesBeforePublishing][].
