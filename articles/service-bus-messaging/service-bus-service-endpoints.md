...
ms.locfileid: "82117630"
---
# <a name="configure-virtual-network-service-endpoints-for-azure-service-bus"></a>Настройка конечных точек службы виртуальной сети для служебной шины Azure
...
2. В меню слева выберите пункт Параметры **сети** . По умолчанию выбран параметр **все сети** . Пространство имен принимает подключения с любого IP-адреса. То есть, такое значение параметра по умолчанию равноценно правилу, которое принимает диапазон IP-адресов 0.0.0.0/0. 

    ![Брандмауэр — выбран параметр "все сети"](./media/service-endpoints/firewall-all-networks-selected.png)
1. Выберите параметр **Выбранные сети** в верхней части страницы.
2. В разделе **Виртуальная сеть** страницы выберите **+ Добавить существующую виртуальную сеть**. 
...
6. Нажмите кнопку **сохранить** на панели инструментов, чтобы сохранить параметры. Подождите несколько минут, пока подтверждение отобразится в уведомлениях на портале. Кнопка **сохранить** должна быть отключена. 
