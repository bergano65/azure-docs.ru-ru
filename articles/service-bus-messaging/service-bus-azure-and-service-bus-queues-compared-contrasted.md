---
title: Сравните очереди хранения данных Azure и очереди в автобусах обслуживания
...
**Очереди хранения данных,** которые являются частью инфраструктуры [хранения данных Azure,](https://azure.microsoft.com/services/storage/) оснащены простым интерфейсом GET/PUT/PEEK на основе REST, обеспечивающим надежное и постоянное сообщение внутри служб и между ними.
...
| Гарантированный метод доставки |**Как минимум один раз** |**At-Least-Once** (с помощью режима получения PeekLock - это по умолчанию) <br/><br/>**At-most-Once** (с помощью режима receiveandDelete) <br/> <br/> Узнайте больше о различных [режимах приема](service-bus-queues-topics-subscriptions.md#receive-modes)  |
...
| API push-уведомлений |**Нет** |**Да**<br/><br/>[ОчередьClient.OnMessage](/dotnet/api/microsoft.servicebus.messaging.queueclient.onmessage#Microsoft_ServiceBus_Messaging_QueueClient_OnMessage_System_Action_Microsoft_ServiceBus_Messaging_BrokeredMessage__) и [MessageSessionHandler.OnMessage](/dotnet/api/microsoft.servicebus.messaging.messagesessionhandler.onmessage#Microsoft_ServiceBus_Messaging_MessageSessionHandler_OnMessage_Microsoft_ServiceBus_Messaging_MessageSession_Microsoft_ServiceBus_Messaging_BrokeredMessage__) сессий .NET API. |
...
| Степень детализации аренды или блокировки |**Уровень сообщения**<br/><br/>(у сообщений могут быть уникальные значения времени ожидания; при необходимости их можно обновлять в процессе обработки сообщения с помощью метода API [UpdateMessage](/dotnet/api/microsoft.azure.storage.queue.cloudqueue.updatemessage)) |**Уровень очереди**<br/><br/>(для каждой очереди определена степень детализации блокировки, применяемая ко всем сообщениям в ней, однако блокировку можно продлить с помощью метода API [RenewLock](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.renewlock#Microsoft_ServiceBus_Messaging_BrokeredMessage_RenewLock)) |
...
* Несоответствий в отношении обработки сообщений в контексте сеансов Service Bus можно избежать, используя состояние сеанса для хранения состояния приложения относительно хода обработки последовательности сообщений сеанса, а также с помощью транзакций вокруг урегулирования полученных сообщений и обновления состояния сеанса. Такая функция согласованности иногда маркируется *точно-после обработки* в продуктах других поставщиков, но сбои транзакций, очевидно, приведет к повторной доставке сообщений, и поэтому этот термин не является точно адекватным.
...
* Функциональность обнаружения дублирования, поддерживаемая очередями Service Bus, автоматически удаляет дубликаты сообщений, отправляемых в очередь или тему, в зависимости от значения свойства [MessageId.](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.messageid#Microsoft_ServiceBus_Messaging_BrokeredMessage_MessageId)
...
| Критерии сравнения | Очереди службы хранилища | Очереди служебной шины |
| --- | --- | --- |
| Максимальный размер очереди |**500 ТБ**<br/><br/>(ограничено [одной емкостью учетной записи хранилища)](../storage/common/storage-introduction.md#queue-storage) |**От 1 до 80 ГБ**<br/><br/>(определяется при создании очереди и [включении секционирования](service-bus-partitioning.md); см. раздел "Дополнительные сведения") |
...
| Максимальное количество очередей |**Неограниченно** |**10,000**<br/><br/>(на пространство имен службы) |
| Максимальное количество параллельных клиентов |**Неограниченно** |**Неограниченно**<br/><br/>(к взаимодействию на основе протокола TCP применяется единственное ограничение в 100 одновременных подключений) |
