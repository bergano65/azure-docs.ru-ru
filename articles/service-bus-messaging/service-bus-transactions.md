---
title: Обзор обработки транзакций в сервисном автобусе Azure
description: В этой статье представлен обзор обработки транзакций и отправки функции в автобусе службы Azure.
...
Чтобы обеспечить транзакционное перемещение в данных из очереди в обработчик, а затем в другую очередь, служебная шина поддерживает *передачи*. В операции передачи отправитель сначала отправляет сообщение в *очередь передачи,* и очередь о передаче немедленно перемещает сообщение в очередь назначения, используя ту же надежную реализацию передачи, на которую опирается возможность автоматического перенаправления. Сообщение никогда не фиксируется в журнале очереди передачи так, чтобы стать видимым для получателей очереди передачи.
...
Затем простая транзакция использует эти элементы, как в следующем примере. Чтобы сослаться на полный пример, направь [исходный код на GitHub:](https://github.com/Azure/azure-service-bus/tree/master/samples/DotNet/Microsoft.Azure.ServiceBus/TransactionsAndSendVia/TransactionsAndSendVia/AMQPTransactionsSendVia)
...
* [Цепь Сервис автобусных объектов с автоматической переадресировки](service-bus-auto-forwarding.md)
