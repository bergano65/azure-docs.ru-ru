...
1. Задайте следующие переменные среды для упрощения миграции команды.
   ...
   ```
    ...
    >[!IMPORTANT]
    > Имя после миграции (post_migration_dns_name) будет использоваться для доступа к старой после миграции стандартное пространство имен. Используется для очистки очереди и подписки, а затем удалите пространство имен.
