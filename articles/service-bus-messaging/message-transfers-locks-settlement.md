---
title: Передача сообщений, блоки и расчетов Azure Service Bus
description: В этой статье приводится обзор переводов сообщений, блокировок и операций расчетов Azure Service Bus.
...
> [!IMPORTANT]
> Важно отметить, что блокировка, которую PeekLock приобретает на сообщении, нестабильна и может быть потеряна в следующих условиях
>   * Обновление сервиса
>   * Обновление ОС
>   * Изменение свойств на объекте (Очередь, Тема, Подписка) при хранении блокировки.
>
> Когда блокировка будет потеряна, Azure Service Bus будет генерировать LockLostException, который будет всплывать на коде приложения клиента. В этом случае логика повторной попытки клиента по умолчанию должна автоматически вскакить и повторить операцию.
