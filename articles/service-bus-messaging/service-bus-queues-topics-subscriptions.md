---
title: Сообщения об обслуживании сервиса Azure - очереди, темы и подписки
description: В этой статье содержится обзор объектов обмена сообщениями Azure Service Bus (очередь, темы и подписки).
...
Вы можете указать два режима получения сообщений служебной шины: *ReceiveAndDelete* или *PeekLock*. В режиме [ReceiveAndDelete](/dotnet/api/microsoft.azure.servicebus.receivemode) операция приема является одноразовой; то есть, когда Service Bus получает запрос от потребителя, он отмечает сообщение как потребляемый и возвращает его в потребительское приложение. Режим **ReceiveAndDelete** представляет собой самую простую модель, которая лучше всего работает в ситуациях, когда приложение может не обрабатывать сообщение при сбое. Чтобы понять этот сценарий, рассмотрим сценарий, в котором объект-получатель выдает запрос на получение и выходит из строя до его обработки. Служебная шина помечает сообщение как использованное. Следовательно, когда после перезапуска приложение снова начнет обрабатывать сообщения, оно пропустит сообщение, использованное до сбоя.
...
Во многих ситуациях сообщения с определенными характеристиками должны обрабатываться разными способами. Для этого можно настроить подписки, обеспечивающие поиск сообщений с нужными свойствами, после чего можно определенным образом изменить эти свойства. Подписки служебной шины регистрируют все сообщения, отправленные в раздел, однако в виртуальную очередь подписки можно скопировать только подмножество этих сообщений. Это возможно благодаря использованию фильтров подписок. Такие изменения называются *действиями фильтров*. При создании подписки можно предоставить выражение фильтра, работающего на свойствах сообщения, как свойства системы (например, **Label),** так и пользовательские свойства приложения (например, **StoreName**.) В этом случае выражение фильтра S'L является необязательным; без выражения фильтра S'L любое действие фильтра, определенное в подписке, будет выполняться на всех сообщениях для этой подписки.
