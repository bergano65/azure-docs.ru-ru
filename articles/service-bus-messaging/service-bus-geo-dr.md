...
3. Создайте связь между основным и дополнительным пространствами имен, чтобы получить ***псевдоним***.
    ...
    >[!NOTE] 
    > При наличии [переноса пространства имен Standard служебной шины Azure в Azure Service Bus Premium](service-bus-migrate-standard-premium.md), а затем для создания аварийного восстановления необходимо использовать существующий псевдоним (т. е. служебной шины уровня Standard пространство имен строки подключения) Настройка с помощью **PS/CLI** или **REST API**.
    >
    >
    > Это обусловлено тем, во время миграции вашей Standard служебной шины Azure пространства имен подключения строка/DNS-имя сам становится псевдонимом для пространства имен уровня "премиум" для Azure Service Bus.
    >
    > Клиентские приложения должны использовать этот псевдоним (т. е. Standard служебной шины Azure пространство имен строка подключения), чтобы подключиться к пространству имен уровня "премиум", где сопряжение аварийного восстановления установлен и настроен.
    >
    > Если вы используете портал для настройки конфигурации аварийного восстановления, портале абстрагировать этот нюанс, от вас.
