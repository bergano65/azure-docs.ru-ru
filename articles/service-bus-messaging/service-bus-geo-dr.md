...
3. Создайте связь между основным и дополнительным пространствами имен, чтобы получить ***псевдоним***.
    ...
    >[!NOTE] 
    > Если вы [перенесли пространство имен Azure Service Bus Standard в Azure Service Bus Premium,](service-bus-migrate-standard-premium.md)необходимо использовать уже существующий псевдоним (т.е. строку подключения к пространству имен Service Bus Standard) для создания конфигурации аварийного восстановления через **PS/CLI** или **REST API.**
    >
    >
    > Это связано с тем, что во время миграции строка подключения к пространству имен Azure Service Service Standard/DNS сама по себе становится псевдонимом к вашему пространству имен Azure Service Bus Premium.
    >
    > Клиентские приложения должны использовать этот псевдоним (нат., напротив строки подключения к пространству имен Azure Service Service Standard) для подключения к пространству имен Premium, где была настроена система сопряжения аварийного восстановления.
    >
    > Если вы используете portal для настройки конфигурации аварийного восстановления, то портал абстрагирует сяобякнет эту оговорку от вас.
