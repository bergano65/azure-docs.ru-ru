---
title: Используйте АМЗП с API Службы сообщений Java & сервисной bus Azure
...
---
# <a name="use-the-java-message-service-jms-with-azure-service-bus-and-amqp-10"></a>Воспользуйтесь службой сообщений Java (JMS) с автобусом Azure Service и АМЗП 1.0
В этой статье объясняется, как использовать функции обмена сообщениями Azure Service Bus (очереди и публикации/подписки) из Java-приложений, использующих популярный стандарт API Службы сообщений Java (JMS). Существует [статья-компаньон,](service-bus-amqp-dotnet.md) в которая объясняет, как сделать то же самое с помощью API службы обслуживания Azure .NET. Эти два руководства можно использовать совместно для изучения обмена сообщениями между различными платформами с помощью AMQP 1.0.
...
Поддержка AM-P 1.0 в Azure Service Bus означает, что вы можете использовать очереди и публиковать/подписывать функции обмена сообщениями с различных платформ с помощью эффективного двоичного протокола. Кроме того, можно создавать приложения, содержащие компоненты, созданные с использованием разнообразных языков, платформ и операционных систем.

## <a name="get-started-with-service-bus"></a>Приступая к работе со служебной шиной
Это руководство предполагает, что у вас уже есть `basicqueue`пространство имен Service Bus, содержащее очередь с именем. Если вы этого не сможете, то можно [создать пространство имен и очередь](service-bus-create-namespace-portal.md) с помощью [портала Azure.](https://portal.azure.com) Дополнительные сведения о создании пространства имен и очередей служебной шины см. в статье [Приступая к работе с очередями служебной шины](service-bus-dotnet-get-started-with-queues.md).
...
Для получения информации о том, где скачать последнюю версию библиотеки клиентов [https://qpid.apache.org/download.html](https://qpid.apache.org/download.html)Apache spid JMS AM-P 1.0, посетите .
...
Ссылка на **ConnectionString,** соссылку в «Политике общего доступа» на [портале Azure](https://portal.azure.com) в строке **первичного подключения**
```java
...
Вы можете получить доступ ко всем исходным кодам и информации о конфигурации из [azure службы службы Автобус Образцы JMS Очередь quickstart](https://github.com/Azure/azure-service-bus/tree/master/samples/Java/qpid-jms-client/JmsQueueQuickstart)
...
## <a name="jms-topics-vs-service-bus-topics"></a>Темы JMS против темы автобусов обслуживания
Использование тем и подписок Azure Service Bus через API Службы сообщений Java (JMS) предоставляет основные возможности отправки и получения. Это удобный выбор при переносе приложений от других брокеров сообщений с AI, совместимыми с JMS, даже несмотря на то, что темы Service Bus отличаются от тем JMS и требуют некоторых корректировок. 

Темы Azure Service Bus направляют сообщения в названные, общие и прочные подписки, управляемые через интерфейс управления ресурсами Azure, инструменты командной строки Azure или через портал Azure. Каждая подписка позволяет использовать правила отбора до 2000 человек, каждое из которых может иметь состояние фильтра, а для фильтров S'L также действие преобразования метаданных. Каждое условие фильтра выбирает входного сообщения для копирования в подписку.  

Получение сообщений от подписок является идентичным приему сообщений из очередей. Каждая подписка имеет связанную очередь с мертвымписьмом и возможность юношной расписки сообщений в другую очередь или темы. 

Темы JMS позволяют клиентам динамически создавать недолговечных и долговечных подписчиков, что дополнительно позволяет фильтровать сообщения с селекторами сообщений. Эти нераспределенные сущности не поддерживаются Сервисным автобусом. Однако синтаксис правила фильтрации для Service Bus похож на синтаксис селектора сообщений, поддерживаемый JMS. 

Сторона издателя JMS Topic совместима с Service Bus, как показано в этом примере, но динамические подписчики не являются. Следующие топологические AMS-аПО не поддерживаются Сервисным автобусом. 
...
* Только один **Производитель сообщений** или **MessageConsumer** допускается в **сессии**. Если требуется создать несколько **MessageProducers** или **MessageConsumers** в приложении, создайте специальный **сеанс** для каждого из них.
* Нестабильные подписки темы в настоящее время не поддерживаются.
* В настоящее время **выборщики сообщений** не поддерживаются.
* Распределенные транзакции не поддерживаются (но транзакционные сеансы поддерживаются).
...
| receiveNoWait               | использовать метод получения (), предоставляемый Сервисным автобусом SDK, и указать очень низкий или нулевой тайм-аут |
...
* [Руководство для разработчиков сервисного автобуса АМЗП 1.0](service-bus-amqp-dotnet.md)
