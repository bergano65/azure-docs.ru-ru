...
## <a name="jms-topics-vs-service-bus-topics"></a>Разделы JMS vs. Разделы шины обслуживания
Использование разделов служебной шины Azure и подписок через сообщения службы Java (JMS) API предоставляет базовые отправки и получения возможности. Это подходит при переносе приложений из других брокеров сообщений с помощью соответствующих API JMS, несмотря на то, что разделы служебной шины, отличаются от разделов JMS и требуют небольшие изменения. 

Разделы служебной шины Azure маршрутизацию сообщений в именованный, совместно используемым и устойчивых подписок, управление которыми осуществляется через интерфейс управления ресурсами Azure, программы командной строки Azure, или на портале Azure. Каждая подписка позволяет для до 2000 правила выбора, каждый из который может иметь условие фильтра, а для фильтров SQL, действие преобразования также метаданных. Каждое совпадение условие фильтра выбирает входное сообщение, в который копируется tehj подписки.  

Получение сообщений из подписок идентичен, получение сообщений из очереди. Каждая подписка имеет связанную очередь недоставленных сообщений-, а также возможность автоматически перенаправлять сообщения к другой очереди или разделы. 

Разделы JMS разрешить клиентам динамически создавать ненадежной и устойчивых подписчиков, которые при необходимости разрешить фильтрацию сообщений с помощью селекторов сообщение. Эти отменен общий доступ к сущности служебной шины не поддерживаются. — Правило синтаксис SQL для служебной шины, тем не менее, очень похоже на синтаксис выделения сообщений, поддерживаемые JMS. 

На стороне издателя разделе совместима со служебной шиной, как показано в этом примере, но динамического подписчики не. Следующие API JMS, относящиеся к топологии не поддерживаются с помощью служебной шины. 
