...
description: В этой статье описывается, как использовать управляемые удостоверения для доступа к сущностям служебной шины Azure (очередям, темам и подпискам).
ms.topic: article
ms.date: 01/21/2021
...
Azure Active Directory (Azure AD) разрешает права доступа к защищенным ресурсам через [Управление доступом на основе ролей Azure (Azure RBAC)](../role-based-access-control/overview.md). Служебная шина Azure определяет набор встроенных ролей Azure, охватывающих общие наборы разрешений, используемых для доступа к сущностям служебной шины. Кроме того, можно определить пользовательские роли для доступа к данным.
...
- **Очередь**, **раздел** или **Подписка**. Назначение ролей применяется к определенной сущности служебной шины. В настоящее время портал Azure не поддерживает назначение пользователей, групп и управляемых удостоверений ролям Azure Service Bus на уровне подписки. Ниже приведен пример использования команды Azure CLI: [AZ-Role-назначение — создать](/cli/azure/role/assignment?#az-role-assignment-create) , чтобы назначить удостоверение роли Azure служебной шины: 
...
Чтобы авторизовать запрос к службе служебной шины из управляемого удостоверения в приложении, сначала настройте параметры управления доступом на основе ролей Azure (Azure RBAC) для этого управляемого удостоверения. Служебная шина Azure определяет роли Azure, которые включают разрешения на отправку и чтение из служебной шины. Когда роль Azure назначается управляемому удостоверению, управляемому удостоверению предоставляется доступ к сущностям служебной шины в соответствующей области.

Дополнительные сведения о назначении ролей Azure см. [в статьях аутентификация и авторизация с помощью Azure Active Directory для доступа к ресурсам](authenticate-application.md#azure-built-in-roles-for-azure-service-bus)служебной шины.
...
> [!NOTE]
> При использовании управляемого удостоверения строка подключения должна иметь формат: `Endpoint=sb://<NAMESPACE NAME>.servicebus.windows.net/;Authentication=Managed Identity` .
...
3.  Нажмите кнопку **Добавить**, а затем выберите **добавить назначение ролей**.
4.  На странице **Добавление назначения ролей** выполните следующие действия.
    1. В поле **роль** выберите роль служебной шины, которую требуется назначить. В этом примере это **владелец данных служебной шины Azure**.
