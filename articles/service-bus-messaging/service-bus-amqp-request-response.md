---
title: Операции запроса/реакции АМЗП 1.0 в автобусе службы Azure
description: В этой статье определяется список операций на запросах/ответах на основе АМЗП в microsoft Azure Service Bus.
...
Тело сообщения запроса должно состоять из раздела **значения amqp,** содержащего **карту** со следующими записями:  
...
Тело сообщения ответа должно состоять из раздела **значения амкп,** содержащего **карту** со следующими записями:  
...
Карта **описания правил** должна включать следующие записи, где **sql-фильтр** и **корреляционный фильтр** являются взаимоисключающими:  
...
Карта **корреляционных фильтров** должна включать по крайней мере одну из следующих записей:  
...
Карта **действий sql-правила** должна включать следующие записи:  
...
|receiver-settle-mode|ubyte|Да|**Режим урегулирования приемника,** указанный в ядре АМЗП v1.0.|  
