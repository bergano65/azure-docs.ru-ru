...
| Недоставленные сообщения| Количество недоставленных сообщений в очереди или разделе. <br/><br/> Единица измерения: число <br/> Тип агрегирования: средний <br/>Измерение: EntityName. |
| Запланированные сообщения| Число запланированных сообщений в очереди или разделе. <br/><br/> Единица измерения: число <br/> Тип агрегирования: средний  <br/> Измерение: EntityName. |

> [!NOTE]
> Значения для следующих метрик являются значениями на момент времени. Входящие сообщения, которые были использованы сразу после этого момента времени, могут не отражаться в этих метриках. 
> - Сообщения
> - Активные сообщения 
...
> 
> Ниже перечислены важные метрики, которые следует отслеживать при любых простоях пространства имен уровня "Премиум": **использование ЦП на пространство имен** и **Размер памяти для каждого пространства имен**. [Настройте оповещения](../azure-monitor/platform/alerts-metric.md) для этих метрик с помощью Azure Monitor.
> 
> Другая метрика, которую вы можете отслеживать: **регулируемые запросы**. Это не должно быть проблемой, если пространство имен остается в пределах памяти, ЦП и подключений через посредника. Дополнительные сведения см. [в статье регулирование на уровне Premium служебной шины Azure](service-bus-throttling.md#throttling-in-azure-service-bus-premium-tier) .
