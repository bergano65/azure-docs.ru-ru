...
Метрики включены по умолчанию, и вы можете получить доступ к данным за последние 30 дней. Если необходимо хранить данные в течение более длительного периода времени, можно архивировать данные метрик в учетную запись хранения Azure. Это значение задается в [параметрах диагностики](../azure-monitor/essentials/diagnostic-settings.md) Azure Monitor.
...
| Имя метрики | Описание |
| ------------------- | ----------------- |
|Входящие сообщения|Количество событий или сообщений, отправленных в служебную шину, за указанный период. Эта метрика не включает автоматические перенаправляемые сообщения.<br/><br/> Единица измерения: Count <br/> Тип агрегирования: Итог <br/> Измерение: имя сущности|
...
> 
> Важные метрики, по которым можно отслеживать любые сбои в пространстве имен уровня "Премиум": **Загрузка ЦП на пространство имен** и **Объем используемой памяти на пространство имен**. [Настройте оповещения](../azure-monitor/alerts/alerts-metric.md) для этих метрик с помощью Azure Monitor.
> 
