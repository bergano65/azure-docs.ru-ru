---
title: Авто-переадресировка сообщений Azure Service Bus
description: В этой статье описывается, как пригвартовать очередь или подписку на автобус Azure на другую очередь или тему.
...
> [!NOTE]
> При настройке автофорвардинга значение AutoDeleteOnIdle как на Источнике, так и в **пункте назначения** автоматически устанавливается на максимальное значение типа данных.
> 
>   - Что касается источника, то автофорвардинг выполняется как операция получения. Таким образом, источник, который имеет autoforwarding установки никогда не "просто".
>   - Со стороны назначения это делается для того, чтобы всегда было место для направления сообщения.
...
Для создания подписки, прикваченной к другой очереди или теме, создатель подписки должен иметь разрешения **Управления** как на источнике, так и на объекте назначения. Для отправки сообщений в исходный раздел требуется только разрешение на **отправку** для исходного раздела.
...
* [Разделенные сущности обмена сообщениями][Partitioned messaging entities].
