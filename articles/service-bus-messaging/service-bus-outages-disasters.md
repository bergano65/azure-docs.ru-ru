---
title: Изолировать приложения Azure Service Bus от простоев и стихийных бедствий
description: В этих статьях предоставляются методы защиты приложений от потенциального простоя шины Службы Azure.
...
Георепликация конечных точек [Azure Relay](../service-bus-relay/relay-what-is-it.md) позволяет службе, которая предоставляет конечную точку ретранслятора, быть доступной при наличии простоев service Bus. Для достижения георепликации служба должна создать две конечные точки ретрансляции в разных пространствах имен. Пространства имен должны находиться в разных центрах обработки данных, а две конечные точки должны иметь разные имена. Например, основная конечная точка может быть доступна в разделе **contosoPrimary.servicebus.windows.net/myQueue**, а ее аналог — в разделе **contosoSecondary.servicebus.windows.net/myQueue**.
