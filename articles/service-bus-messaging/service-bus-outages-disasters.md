...
* **Дублирование.** Предположим, что отправитель отправляет сообщение m в основную очередь. Служебная шина успешно обрабатывает m, но ей не удается отправить ответ. После истечения времени ожидания операции отправки отправитель отправляет копию сообщения m в дополнительную очередь. Если получатель смог получить первую копию m до того как основная очередь стала недоступной, то он получит обе копии m приблизительно в одно и то же время. Если получатель не смог получить первую копию m до того, как основная очередь стала недоступной, то сначала он получит вторую копию m, но затем получит первую копию m, когда основная очередь станет доступна.
