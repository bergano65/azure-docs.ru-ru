...
description: В этой статье объясняется, как можно обнаружить дубликаты в сообщениях Azure Service Bus. Дублировать сообщение можно игнорировать и упасть.
...
> [!NOTE]
> Если включено дубликатов обнаружения и не установлен ключ идентификатора сеанса или раздела, идентификатор сообщения используется в качестве ключа раздела. Если идентификатор сообщения также не установлен, библиотеки .NET и AM-P автоматически генерируют идентификатор сообщения для сообщения. Для получения дополнительной [информации см.](service-bus-partitioning.md#use-of-partition-keys)
...
В сценариях, где клиентский код не может повторно отправить сообщение с тем же *Сообщением,* что и раньше, важно проектировать сообщения, которые могут быть безопасно переработаны. Этот [блог о идемпотенции](https://particular.net/blog/what-does-idempotent-mean) описывает различные методы, как это сделать.
