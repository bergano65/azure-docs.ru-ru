...
Сейчас в служебной шине Azure для каждого подключения используется только один сеанс. Максимальный размер кадра служебной шины — 262 144 байта (256 КБ) для служебной шины ценовой категории "Стандартный" и Центров событий. Для служебной шины уровня Premium он составляет 1 048 576 (1 МБ). В служебной шине не установлены определенные окна регулирования на уровне сеанса, но предусмотрен регулярный сброс окна в рамках управления потоком на уровне связи (см. [следующий раздел](#links)).
...
### <a name="amqp-outbound-port-requirements"></a>Требования к исходящим портам АМЗП

Клиенты, которые используют соединения АМЗП через TCP, требуют открытия портов 5671 и 5672 в локальном брандмауэре. Наряду с этими портами может потребоваться открыть дополнительные порты, если включена функция [EnableLinkRedirect.](https://docs.microsoft.com/dotnet/api/microsoft.servicebus.messaging.amqp.amqptransportsettings.enablelinkredirect?view=azure-dotnet) `EnableLinkRedirect`это новая функция обмена сообщениями, которая помогает пропустить один-хоп при получении сообщений, тем самым помогая увеличить пропускную вылим. Клиент начнет общаться непосредственно с бэк-эндслужбой в диапазоне портов 104XX, как показано на следующем изображении. 

![Список портов назначения][4]

Клиент .NET не сможет выйти из строя с помощью SocketException ("Была предпринята попытка получить доступ к розетке, запрещенной его разрешениями доступа"), если эти порты заблокированы брандмауэром. Функция может быть отключена путем установки `EnableAmqpLinkRedirect=false` в строке подключения, что заставляет клиентов общаться с удаленной службой через порт 5671.
...
| "Клиент"; | Служебная шина |
| --- | --- |
| --> detach(<br/>handle={числовой дескриптор},<br/>закрытое**истинное**<br/>) |Нет действий |
| Нет действий |<-- detach(<br/>handle={числовой дескриптор},<br/>закрытое**истинное**<br/>) |
...
| durable |- |- |
| priority |- |- |
| ttl |Срок жизни сообщения |[Timetolive](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
| first-acquirer |- |- |
...
| Имя поля | Использование | Имя API |
| --- | --- | --- |
| message-id |Определяемый приложением идентификатор свободной формы для этого сообщения. Используется для обнаружения дубликатов. |[Messageid](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
...
| значение |Определяемый приложением идентификатор назначения, который не интерпретируется служебной шиной. |[Кому](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
| subject |Определяемый приложением идентификатор назначения сообщения, который не интерпретируется служебной шиной. |[Метка](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
| reply-to |Определяемый приложением индикатор пути ответа, который не интерпретируется служебной шиной. |[Replyto](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
| correlation-id |Определяемый приложением идентификатор корреляции, который не интерпретируется служебной шиной. |[Correlationid](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
| content-type |Определяемый приложением индикатор типа содержимого, который не интерпретируется служебной шиной. |[Contenttype](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
...
| absolute-expiry-time |Объявляет абсолютное время истечения срока действия сообщения. Игнорируется во входных данных (отмечается значение TTL заголовка), учитывается в выходных данных. |[ExpiresAtUtc](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
| creation-time |Объявляет время создания сообщения. Не используется служебной шиной. |Недоступно через API служебной шины. |
| group-id |Определяемый приложением идентификатор связанного набора сообщений. Используется для сеансов служебной шины. |[Sessionid](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
...
| x-opt-sequence-number | Определяемый службой уникальный номер, назначенный сообщению. | [ПоследовательностьНомер](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.sequencenumber?view=azure-dotnet) |
...
| transfer(<br/>delivery-id=0, ...)<br/>- AmqpValue **(Объявление()**)| ------> |  |
|  | <------ | disposition( <br/> first=0, last=0, <br/>state=**Declared**(<br/>**txn-id**={идентификатор транзакции}<br/>))|
...
| | <------ | disposition( <br/> first=57, last=57, <br/>государство принято **()**)|

#### <a name="sending-a-message-in-a-transaction"></a>Отправка сообщения в транзакции

Вся транзакционная работа выполняется `transactional-state` с состоянием транзакционной доставки, которое несет txn-id. В случае отправки сообщений транзакционное состояние осуществляется по системе передачи сообщения. 
...
| Логическая операция | "Клиент"; | Служебная шина |
| --- | --- | --- |
...
| Ключ | Необязательный | Тип значения | Содержимое значения |
| --- | --- | --- | --- |
| операции |нет |строка |**put-token** |
| type |нет |строка |Тип размещаемого маркера. |
...
| "Клиент"; | | Служебная шина |
| --- | --- | --- |
| attach(<br/>name={имя ссылки},<br/>role=sender,<br/>source={идентификатор связи клиента},<br/>целевой показатель **» (через-сущность)**,<br/>**properties=map [(<br/>com.microsoft:transfer-destination-address=<br/>{сущность-назначения} )]** ) | ------> | |
