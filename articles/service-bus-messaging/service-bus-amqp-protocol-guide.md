...
![Снимок экрана, показывающий сеанс, карригн соединение между двумя контейнерами.][2]
...
![Снимок экрана журнала, в котором показаны источник, назначение, порт источника, порт назначения и имя протокола. В строке Фиест целевой порт 10401 (0x28 A 1) выдается черным цветом.][4]
...
| "Клиент"; | Cлужебная шина |
| --- | --- |
| --> detach(<br/>handle={числовой дескриптор},<br/>закрыто =**true**<br/>) |Нет действий |
| Нет действий |<-- detach(<br/>handle={числовой дескриптор},<br/>закрыто =**true**<br/>) |
...
| значение |Определяемый приложением идентификатор назначения, который не интерпретируется служебной шиной. |[Чтобы](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
| subject |Определяемый приложением идентификатор назначения сообщения, который не интерпретируется служебной шиной. |[Label](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
...
| x-opt-via-partition-key | Определяемое приложением значение ключа раздела в случае, когда используется транзакция для отправки сообщений через очередь передачи. | [виапартитионкэй](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.viapartitionkey?view=azure-dotnet) |
...
| Клиент (контроллер) | Direction | Служебная шина (координатор) |
| :--- | :---: | :--- |
...
| transfer(<br/>delivery-id=0, ...)<br/>{Амкпвалуе (**Declare ()**)}| ------> |  |
...
| | <------ | disposition( <br/> first=57, last=57, <br/>State =**принято ()**)|

#### <a name="sending-a-message-in-a-transaction"></a>Отправка сообщения в транзакции

Вся транзакционная работа выполняется с состоянием доставки транзакций `transactional-state` , которое содержит транзакция-ID. В случае отправки сообщений состояние транзакции передается кадром передачи сообщения. 
...
| Создание пути "запрос-ответ" |Нет действий |\<-- attach(<br/>имя = {*имя ссылки*},<br/>handle={*числовой дескриптор*},<br/>role=**receiver**,<br/>source=null,<br/>target=”myentity”<br/>) |
...
| Создание пути "запрос-ответ" |Нет действий |\<-- attach(<br/>имя = {*имя ссылки*},<br/>handle={*числовой дескриптор*},<br/>role=**sender**,<br/>source=”myentity”,<br/>target=”myclient$id”<br/>) |
...
| "Клиент"; | Direction | Cлужебная шина |
| :--- | :---: | :--- |
