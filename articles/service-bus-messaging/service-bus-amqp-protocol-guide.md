...
Вся транзакционная работа выполняется с `transactional-state` состоянием доставки транзакций, которая содержит транзакция-ID. В случае отправки сообщений состояние транзакции передается кадром передачи сообщения. 
...
| Клавиши | Необязательно | Тип значения | Содержимое значения |
| --- | --- | --- | --- |
