...
Вся транзакционная работа выполняется с состоянием `transactional-state` доставки транзакций, которое содержит транзакция-ID. В случае отправки сообщений состояние транзакции передается кадром передачи сообщения. 
...
| Клавиши | Необязательный | Тип значения | Содержимое значения |
| --- | --- | --- | --- |
