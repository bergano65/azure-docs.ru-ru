...
-   *Фильтры SQL*. **SqlFilter** содержит SQL-подобное условное выражение, которое вычисляется в брокере для определяемых пользователем свойств и системных свойств каждого поступившего сообщения. Все системные свойства в условном выражении предваряются префиксом `sys.`. Подмножество на [языке S'L для](service-bus-messaging-sql-filter.md) тестов`EXISTS`условий фильтра`IS NULL`для существования свойств (), нулевых значений (), логических НЕ/И/ИЛИ, реляционных операторов, простой цифровой арифметики и простого шаблона текста, сопоставляя с `LIKE`.

-   *Фильтры корреляции*. **CorrelationFilter** содержит набор условий, которые сопоставляются с одним или несколькими из пользовательских и системных свойств для каждого поступающего сообщения. Общее использование заключается в сопоставлении с свойством **CorrelationId,** но приложение также может выбрать, чтобы соответствовать следующим свойствам:
    ...
    - **Contenttype**
     ...
     - **SessionId** 
     - **Кому**
     - любых свойств, определяемых пользователем. 
...
Все фильтры оценивают только свойства сообщения. Фильтры не могут оценить тело сообщения.

Сложные правила фильтрации требуют значительных вычислительных ресурсов. В частности, использование правил фильтра S'L приводит к снижению общей пропускной воды сообщений на уровне подписки, темы и пространства имен. Когда это возможно, приложения должны выбирать корреляционные фильтры по сравнению с фильтрами, похожими на S'L, потому что они гораздо более эффективны в обработке и оказывают меньшее влияние на пропускную работу.

## <a name="actions"></a>Действия

С помощью условий фильтров SQL можно определить действие, которое добавляет примечания с помощью добавления, удаления или изменения свойств и их значений. Это действие [использует выражение в стиле SQL](service-bus-messaging-sql-filter.md), отдаленно напоминающее синтаксис инструкции SQL UPDATE. Действие выполняется в сообщении после того, как оно было сопоставлено, и до того, как сообщение будет выбрано в подписку. Все изменения свойств сообщения действуют только для той копии сообщения, которая помещается в подписку.
...
> [!NOTE]
> В настоящее время портал Azure не позволяет указывать правила фильтрации для подписок. Для определения правил подписки можно использовать любой из поддерживаемых шаблонов SDK или менеджер ресурсов Azure. 

## <a name="next-steps"></a>Следующие шаги
Смотрите следующие образцы: 

- [.NET - Основные отправить и получить учебник с фильтрами](https://github.com/Azure/azure-service-bus/tree/master/samples/DotNet/GettingStarted/BasicSendReceiveTutorialwithFilters/BasicSendReceiveTutorialWithFilters)
- [.NET - Тематические фильтры](https://github.com/Azure/azure-service-bus/tree/master/samples/DotNet/Microsoft.Azure.ServiceBus/TopicFilters)
...
- [Введите скрипт](https://github.com/Azure/azure-sdk-for-js/blob/master/sdk/servicebus/service-bus/samples/typescript/src/advanced/topicFilters.ts)
