...
description: В этой статье содержатся ответы на некоторые часто задаваемые вопросы (часто задаваемые вопросы) о сервисном автобусе Azure.
...
---
# <a name="azure-service-bus---frequently-asked-questions-faq"></a>Автобус службы Azure - Часто задаваемые вопросы (Часто)
...
[Azure Service Bus](service-bus-messaging-overview.md) — это асинхронная облачная платформа обмена сообщениями, которая позволяет отправлять данные между разъединенными системами. Корпорация Майкрософт предлагает эту возможность в виде услуги, а значит вам не нужно развертывать собственное оборудование для ее использования.

### <a name="what-is-a-service-bus-namespace"></a>Что такое пространство имен служебной шины?
[Пространство имен](service-bus-create-namespace-portal.md) предоставляет контейнер для поиска ресурсов Service Bus в приложении. Создание пространства имен является первым и обязательным шагом перед началом работы со служебной шиной.
...
| -------- | ----- | ------- | 
| AMQP | 5671 и 5672 | Смотрите [руководство по протоколу АМЗП](service-bus-amqp-protocol-guide.md) | 
| SBMP | от 9350 до 9354 | Посмотреть [режим подключения](/dotnet/api/microsoft.servicebus.connectivitymode?view=azure-dotnet) |
| HTTP, HTTPS | 80, 443 | 
...
1. Выполнить следующую команду из запроса команды: 
    ...
    ```
2. Заметьте, что IP-адрес вернулся в `Non-authoritative answer`. Этот IP-адрес статичен. Единственный момент времени, который он изменит, это восстановление пространства имен в другом кластере.
...
### <a name="i-have-a-service-bus-standard-namespace-why-do-i-see-charges-under-resource-group-system"></a>У меня есть сервисный автобус Стандартный namespace. Почему я вижу сборы в группе ресурсов "$system"?
Недавно Azure Service Bus обновил компоненты биллинга. В связи с этим, если у вас есть пространство имен Service Bus Standard, вы можете видеть элементы строки для ресурса '/подписок/<azure_subscription_id>/resourceGroups/$system/providers/Microsoft.ServiceBus/namespaces/$system' в группе ресурсов "$system".

Эти сборы представляют собой базовую плату за подписку Azure, которая предоставила пространство имен Service Bus Standard. 

Важно отметить, что это не новые сборы, т.е. они существовали и в предыдущей модели биллинга. Единственное изменение заключается в том, что они в настоящее время перечислены в соответствии с "$system". Это делается из-за противоречий в новой системе биллинга, которая группирует сборы уровня подписки, не привязанные к определенному ресурсу, под идентификатором ресурса "$system".
...
Подписи общего доступа представляют собой механизм проверки подлинности на основе безопасных хэшей SHA-256 или URI. Информацию о том, как создавать собственные подписи в Node.js, PHP, Java, Python и C, можно получить в статье [«Общие подписи доступа».][Shared Access Signatures]
