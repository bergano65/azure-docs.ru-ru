...
>[!NOTE]
> Операция считывания Возвращает самое старое сообщение из части на основе его SequenceNumber. Для сущностей являются порядковый номер выдается относительно секции. Дополнительные сведения см. в разделе [последовательность сообщений и метки времени](../service-bus-messaging/message-sequencing.md).
