...
Каждая разделенная очередь или тема состоит из нескольких разделов. Каждая разделка хранится в другом магазине обмена сообщениями и обрабатывается другим брокером сообщений. Когда сообщение отправляется в разделитную очередь или тему, Service Bus назначает сообщение одному из разделов. Выбор фрагмента осуществляется произвольно или по ключу секции, который может указать отправитель.

Если клиент хочет получить сообщение из разделенной очереди или из подписки на раздельную тему, Service Bus запрашивает все разделы для сообщений, а затем возвращает первое сообщение, полученное из любого из магазинов обмена сообщениями, получателю. Служебная шина кэширует другие сообщения и возвращает их при получении дополнительных запросов. Клиент ничего не знает о секционировании, для него поведение секционированных очередей или разделов (например, чтение, завершение, откладывание, недоставление или предварительная выборка) аналогично поведению обычной сущности.
...
На уровне обмена сообщениями "Стандартный" можно создавать очереди и разделы служебной шины размером в 1, 2, 3, 4 или 5 ГБ (по умолчанию — 1 ГБ). С включенным разделом Service Bus создает 16 копий (16 разделов) сущности, каждый из которых указан в одном и том же размере. Поэтому при создании очереди размером 5 ГБ с 16 разделами максимальный размер очереди составит 80 ГБ (5 \* 16). Чтобы просмотреть максимальный размер секционированной очереди или раздела, откройте соответствующую запись на [портале Azure][Azure portal] в колонке **Обзор** для этой сущности.

### <a name="premium"></a>Premium

В пространстве имен уровня Premium сущности раздела не поддерживаются. Однако все еще можно создать разделы служебной шины размером в 1, 2, 3, 4, 5, 10, 20, 40 или 80 ГБ (по умолчанию — 1 ГБ). Чтобы просмотреть размер очереди или раздела, откройте соответствующую запись на [портале Azure][Azure portal] в колонке **Обзор** для этой сущности.
...
Когда сообщение добавляется в секционированную очередь или раздел, служебная шина проверяет наличие ключа секции. Если он находит один, он выбирает раздел на основе этого ключа. Если он не находит ключ раздела, он выбирает раздел на основе внутреннего алгоритма.

### <a name="using-a-partition-key"></a>Использование ключа секции

Некоторые сценарии, такие как сеансы или транзакции, требуют хранения сообщений в определенном разделе. Для всех этих сценариев необходимо использовать ключ секции. Все сообщения, использовавающие один и тот же ключ раздела, присваиваются тому же разделу. Если раздел временно недоступен, S-шина Service возвращает ошибку.
...
Чтобы дать Сервис-автобусу достаточно времени, чтобы выложить сообщение в другой раздел, значение [OperationTimeout,](/dotnet/api/microsoft.azure.servicebus.queueclient.operationtimeout) указанное клиентом, отправляя сообщение, должно быть больше 15 секунд. Рекомендуется установить свойство [OperationTimeout](/dotnet/api/microsoft.azure.servicebus.queueclient.operationtimeout) в значение по умолчанию 60 секунд.

Ключ раздела «прикрепляет» сообщение к определенному разделу. Если хранилище сообщений, вмещающее эту раздел, недоступно, Service Bus возвращает ошибку. При отсутствии ключа раздела Service Bus может выбрать другой раздел, и операция успешно. Поэтому рекомендуется не указывать ключ раздела, если он не является обязательным.
...
Если какой-либо из свойств, отслуживаемых в качестве ключа раздела, установлен, шины Service прикрепляет сообщение к определенному разделу. Это происходит независимо от того, используется ли транзакция. Рекомендуется не указывать ключ секции, если в этом нет необходимости.
...
* **Функции высокой согласованности:** Если объект использует такие функции, как сеансы, дубликат обнаружения или явного управления ключом раздела, то операции обмена сообщениями всегда направляются на определенный раздел. Если в каких-либо разделах наблюдается высокий трафик или базовый магазин неработоспособен, эти операции сбой и доступность уменьшается. В целом уровень согласованности гораздо выше, чем при использовании несекционированных сущностей, поэтому проблемы возникают лишь в некоторой части трафика. Дополнительные сведения см. в этом [обсуждении доступности и целостности](../event-hubs/event-hubs-availability-and-consistency.md).
* **Управление**: Операции, такие как Создание, Обновление и Удаление, должны выполняться на всех разделах сущности. Если какой-либо раздел неработоспособен, это может привести к сбоям для этих операций. Для операции Get информация, такая как количество сообщений, должна быть агрегирована из всех разделов. Если какой-либо раздел неработоспособен, статус доступности сущности сообщается как ограниченный.
* **Сообщения небольших объемов**. Чтобы получить все сообщения в таких сценариях, особенно при использовании протокола HTTP, может потребоваться выполнить несколько операций получения. Для получения запросов, передняя часть выполняет получение на всех разделах и кэширует все ответы, полученные. Это кэширование повышает производительность последующего запроса на получение при том же подключении, уменьшая задержки получения. Тем не менее при наличии нескольких подключений или использовании протокола HTTP для каждого запроса устанавливается новое подключение. Таким образом, нет никакой гарантии, что запрос будет выполняться на том же узле. Если все имеющиеся сообщения блокируются и кэшируются в другом внешнем интерфейсе, операция получения возвращает значение **NULL**. Когда в конечном итоге срок действия сообщений истекает, их снова можно получать. Рекомендуется выполнить проверку активности HTTP. При использовании раздела в малообъемных сценариях операции получения могут занять больше времени, чем ожидалось. Поэтому мы рекомендуем не использовать раздел ы в этих сценариях. Удалите все существующие разделивные сущности и воссоздайте их с отключением раздела для повышения производительности.
