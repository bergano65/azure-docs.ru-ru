...
Этот сценарий запускает трассировку [счетчиков производительности](/windows/win32/perfctrs/performance-counters-portal), которые указаны в файле RuleEngineConfig.json. Если виртуальная машина определяется как сервер, на котором работает SQL Server, выполняется трассировка счетчиков производительности. Это делается с помощью счетчиков, которые находятся в файле RuleEngineConfig.json. Этот сценарий также содержит данные диагностики производительности.
...
|--------------|-------------------------------|
| Количество операций ввода-вывода в секунду         | Запросов данных в секунду             |
...
| Размер ввода-вывода      | СР. байт/запрос данных       |
|              | Средн. байт/чтение               |
|              | Средн. байт/запись              |
| Пропускная способность   | Байтов в секунду                |
|              | Прочитано байтов в секунду                |
|              | Записано байтов в секунду               |
| Длина очереди | СР. Длина очереди чтения        |
|              | СР. Длина очереди записи       |
|              | СР. Длина очереди данных        |
...
| Трассировка производительности Diskspd * * _       |                            | Да                                |                          |                      |                      |
|       |                            |                         |                                                   |                      |                      |

### <a name="performance-diagnostics-trace-_"></a>Трассировка диагностики производительности (_)
...
### <a name="diskspd-benchmark-trace-_"></a>Трассировка производительности Diskspd (* * _)
...
-  Поддерживаются следующие операционные системы: _ Windows Server 2019
   * Windows Server 2016
...
По завершении трассировки или операции в папке с PerfInsights появится новый файл. Имя файла **PerformanceDiagnostics\_гггг-мм-дд\_чч-мм-сс-ммм.zip**. Вы можете отправить этот файл агенту поддержки для анализа или открыть отчет внутри ZIP-файла, чтобы просмотреть результаты и рекомендации.
...
![Снимок экрана вкладки "Обзор" отчета PerfInsights. ](media/how-to-use-perfInsights/pi-finding-tab.png)
 ![ Снимок экрана: вкладка "хранилище" в отчете PerfInsights.](media/how-to-use-perfInsights/pi-findings.png)
...
В перспективе физического диска (DiskMap) в таблице показаны все логические тома, которые размещены на диске. В следующем примере **physicaldrive2 размещены** запускает два логических тома, созданные в нескольких секциях (J и H):
...
![Снимок экрана вкладки SQL](media/how-to-use-perfInsights/pi-sql-tab.png)
...
Вкладка **результаты** содержит список всех найденных проблем с производительностью, связанных с SQL, а также рекомендации.
