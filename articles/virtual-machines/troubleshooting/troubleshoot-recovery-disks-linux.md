...
Если группа ресурсов и исходный моментальный снимок находятся в разных регионах, при запуске `az disk create`появится сообщение об ошибке "ресурс не найден". В этом случае необходимо указать `--location <region>`, чтобы создать диск в том же регионе, что и исходный моментальный снимок.
...
Этот сценарий присоединяет дисковый `myNewOSDisk` к виртуальной машине `MyTroubleshootVM`:
