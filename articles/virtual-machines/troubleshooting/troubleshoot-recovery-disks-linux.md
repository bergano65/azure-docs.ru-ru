...
1. Сфотографать с диска ОС VM.
1. Создайте диск из моментального снимка диска операционной системы.
1. Прикрепите и установите новый диск ОС на другой Linux VM для устранения неполадок.
1. Подключитесь к этой виртуальной машине. Оторитейте файлы или запускайте любые инструменты для устранения проблем на новом диске ОС.
1. Открепите и отсоедините новый диск ОС от устранения неполадок VM.
...
В следующих примерах замените имена параметров `myResourceGroup` своими значениями, такими как и `myVM`.
...
```
## <a name="take-a-snapshot-from-the-os-disk-of-the-affected-vm"></a>Сделать снимок с диска ОС пострадавшего VM
...
Если группа ресурсов и моментальный снимок исхода не находятся в одном регионе, `az disk create`при запуске — ошибка «Ресурс не найден». В этом случае необходимо `--location <region>` указать для создания диска в ту же область, что и моментальный снимок источника.

Теперь у вас есть копия исходного диска ОС. Вы можете смонтировать этот новый диск на другой Windows VM для устранения неполадок.

## <a name="attach-the-new-virtual-hard-disk-to-another-vm"></a>Прикрепите новый виртуальный жесткий диск к другому VM
В следующих нескольких шагах описывается использование другой виртуальной машины для устранения неполадок. Вы прикрепляете диск к этому устранению неполадок VM для просмотра и отображания содержимого диска. Этот процесс позволяет исправить любые ошибки конфигурации или просмотреть дополнительные файлы приложения или системного журнала.

Этот скрипт `myNewOSDisk` прикрепить `MyTroubleshootVM`диск к VM:
...
## <a name="fix-issues-on-the-new-os-disk"></a>Исправление проблем на новом диске ОС
...
## <a name="unmount-and-detach-the-new-os-disk"></a>Открепить и отсоединить новый диск ОС
...
2. Теперь отсоедините виртуальный жесткий диск от виртуальной машины. Выйдите из сеанса SSH на устранение неполадок VM:
...
Для замены дисков ОС можно использовать Azure CLI. Нет необходимости удалять и повторно создавать виртуальную машину.
