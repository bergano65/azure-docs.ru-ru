...
- [Как запланировать плановое техническое обслуживание на VMs Azure](../windows/classic/planned-maintenance-schedule.md)
...
При выполнении перезагрузки с портала Azure, Azure PowerShell, интерфейса командной строки или REST API вы можете найти событие в [журнале деятельности Azure.](../../azure-monitor/platform/platform-logs-overview.md)
...
Так как некоторые ошибки сервера узла могут возникать конкретно для этого сервера, в случае неоднократного перезапуска виртуальной машины ситуацию можно исправить, если заново развернуть машину на другом сервере узла. Эта операция может быть запущена с помощью опции **передислокации** на странице деталей VM или путем остановки и перезапуска VM на портале Azure.
...
Незапланированное техническое обслуживание включает в себя следующее:
...
Запросы на операции ввода-вывода последовательно регулируются на основе допустимых объемов, установленных для этих операций (в секунду). Если эти объемы превышают установленные ограничения для диска, виртуальные машины могут временно завершить работу. (Стандартное хранение дисков ограничено 500 IOPS.) Чтобы смягчить эту проблему, используйте полоску диска или настраивайте место для хранения внутри гостевого VM, в зависимости от рабочей нагрузки. Дополнительные сведения см. на странице [настройки виртуальных машин Azure для обеспечения оптимальной производительности хранилища](https://blogs.msdn.com/b/mast/archive/2014/10/14/configuring-azure-virtual-machines-for-optimal-storage-performance.aspx).
