...
description: Узнайте, как устранить проблему с подключением к удаленный рабочий стол Portal (RDP), которая блокирует входящий трафик брандмауэром гостевой операционной системы.
...
1.  Если агент Azure установлен и правильно работает на виртуальной машине, можно использовать параметр "сбросить конфигурацию только" в разделе " **Поддержка и устранение неполадок** > **сброса пароля** " в меню "виртуальная машина".
...
1.  Запросите профили брандмауэра, чтобы определить, задана ли для политики входящих подключений брандмауэра значение *блоккинбаундалвайс*:
...
2.  Измените *дефаултинбаундактион* , чтобы настроить эти профили для **разрешения** трафика. Для этого выполните следующую команду:
...
3.  После подключения системного диска к виртуальной машине восстановления убедитесь, что диск помечен как **подключенный** в консоли управления дисками. Запишите или запомните букву диска, которая присвоена подключенному диску ОС.
