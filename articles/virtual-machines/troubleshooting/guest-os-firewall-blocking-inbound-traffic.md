...
description: Узнайте, как устранить проблему подключения удаленного настольного портала (RDP), которую брандмауэр гостевой операционной системы блокирует входящий трафик.
...
1.  Если агент Azure установлен и работает правильно на VM, вы можете использовать опцию "Только конфигурация сбросить" в соответствии с паролем **"Поддержка и устранение** > **спажи"** в меню VM.
...
1.  Запрос профилей брандмауэра, чтобы определить, устанавливается ли входящий брандмауэр на *BlockInboundAlways:*
...
2.  Изменить *DefaultInboundAction,* чтобы настроить эти профили для **разрешить** трафик. Для этого выполните следующую команду:
...
3.  После того, как диск системы присоединен к восстановлению VM, убедитесь, что диск помечен как **Онлайн** в консоли управления диском. Запишите или запомните букву диска, которая присвоена подключенному диску ОС.
