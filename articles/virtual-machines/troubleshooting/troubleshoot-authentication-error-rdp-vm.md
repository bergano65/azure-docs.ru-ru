...
**Произошла ошибка аутентификации. С местным органом безопасности связаться нельзя.**

### <a name="error-message-2"></a>Сообщение об ошибке 2

**Удаленный компьютер, к который вы пытаетесь подключиться, требует аутентификации сетевого уровня (NLA), но контроллер домена Windows не может быть свещен для выполнения NLA. Если вы являетесь администратором на удаленном компьютере, вы можете отключить NLA, используя параметры на удаленной вкладке диалогового окна System Properties.**
