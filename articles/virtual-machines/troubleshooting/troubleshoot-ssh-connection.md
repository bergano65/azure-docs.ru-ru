...
1. [Сбросить конфигурацию SSH.](#reset-config)
...
4. Проверьте [здоровье ресурса VM.](../../resource-health/resource-health-overview.md)
   * Убедитесь, что виртуальная машина работоспособна.
   * Если у вас [включена диагностика загрузки,](boot-diagnostics.md)проверьте, что VM не сообщает об ошибках загрузки в журналах.
...
* [Azure VM Serial Console](https://aka.ms/serialconsolelinux) - серийная консоль VM будет работать независимо от конфигурации SSH, и предоставит вам интерактивную консоль для вашего VM. В самом деле, "не может SSH" ситуации, в частности, то, что серийная консоль была разработана, чтобы помочь решить. Дополнительные сведения см. ниже.
...
## <a name="use-the-azure-vm-serial-console"></a>Используйте серийную консоль Azure VM
[Azure VM Serial Console](./serial-console-linux.md) предоставляет доступ к текстовой консоли для виртуальных машин Linux. Вы можете использовать консоль для устранения неполадок соединения SSH в интерактивной оболочке. Убедитесь, что вы выполнили [предпосылки](./serial-console-linux.md#prerequisites) для использования Serial Console и попробуйте команды ниже, чтобы дальнейшее устранение неполадок sSH подключения.

### <a name="check-that-ssh-is-running"></a>Проверить, работает ли SSH
Вы можете использовать следующую команду, чтобы проверить, работает ли SSH на вашем VM:

```console
ps -aux | grep ssh
```

Если есть какой-либо выход, SSH запущен и работает.

### <a name="check-which-port-ssh-is-running-on"></a>Проверьте, на каком порте SSH работает

Вы можете использовать следующую команду, чтобы проверить, какой порт SSH работает на:

```console
sudo grep Port /etc/ssh/sshd_config
```

Ваш выход будет выглядеть примерно так:
...
* Проверьте работоспособность ресурсов виртуальной машины, чтобы выявить любые проблемы, связанные с платформой.<br>
     Выберите VM и прокрутите вниз **Настройки** > **Проверить здоровье.**
