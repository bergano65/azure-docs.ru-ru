...
1. [Выполните сброс конфигурации SSH](#reset-config).
...
4. Проверьте [работоспособность ресурсов виртуальной машины](../../resource-health/resource-health-overview.md).
   ...
   * Если [включена диагностика загрузки](boot-diagnostics.md), убедитесь, что виртуальная машина не сообщала об ошибках загрузки в журналах.
...
* [Azure последовательной консоли виртуальной Машины](https://aka.ms/serialconsolelinux) -последовательной консоли виртуальной Машины будет работать независимо от конфигурации SSH и вам будет предоставлена интерактивная консоль к виртуальной Машине. На самом деле, «не удается SSH» ситуации, в частности последовательную консоль была разработана для решения. Более подробное описание ниже.
...
### <a name="a-idreset-config-reset-the-ssh-configuration"></a><a id="reset-config" />Сброс конфигурации SSH
...
### <a name="a-idsecurity-rules-check-security-rules"></a><a id="security-rules" />Проверка правил безопасности

Используйте [проверку потока для IP-адреса](../../network-watcher/network-watcher-check-ip-flow-verify-portal.md), чтобы определить, блокирует ли правило в группе безопасности сети входящий или исходящий трафик виртуальной машины. Кроме того, вы можете просмотреть действующие правила группы безопасности и убедиться, что для порта SSH (22 по умолчанию) существует и является приоритетным правило NSG, разрешающее входящий трафик. Дополнительные сведения см. в разделе [Использование действующих правил безопасности для устранения проблем с потоком трафика в виртуальной машине](../../virtual-network/diagnose-network-traffic-filter-problem.md).
...
## <a name="use-the-azure-vm-serial-console"></a>С помощью последовательной консоли виртуальной Машины Azure
[Последовательной консоли для виртуальной Машины Azure](./serial-console-linux.md) предоставляет доступ к текстовой консоли для виртуальных машин Linux. Консоль можно использовать для устранения неполадок SSH-подключение в интерактивную оболочку. Убедитесь, выполнены [предварительные требования](./serial-console-linux.md#prerequisites) для использования последовательной консоли и повторите приведенные ниже команды для дальнейшего устранения неполадок подключения SSH.

### <a name="check-that-ssh-is-running"></a>Убедитесь, что запущена SSH
Чтобы проверить, работает ли SSH на виртуальной Машине, можно использовать следующую команду:
```
$ ps -aux | grep ssh
```
Если никаких выходных данных, SSH — приступить к работе.

### <a name="check-which-port-ssh-is-running-on"></a>Проверьте, какой порт прослушивает SSH
Чтобы проверить используемый порт SSH запущена на можно использовать следующую команду:
```
...
Если вы создали и отправили пользовательский образ диска Linux, убедитесь, что установлен [агент Linux для Microsoft Azure](../extensions/agent-windows.md) версии 2.0.5 или более поздней. В виртуальных машинах, созданных с помощью образов из коллекции, это расширение для доступа уже установлено и настроено.
...
Если вы этого еще не сделали, [установите интерфейс командной строки Azure и подключитесь к подписке Azure](../../cli-install-nodejs.md). Убедитесь, что используется режим Resource Manager следующим образом:
...
## <a name="a-idredeploy-vm-redeploy-a-vm"></a><a id="redeploy-vm" />Повторное развертывание виртуальной машины
...
* Дополнительные сведения об устранении неполадок на виртуальных машинах, созданных с помощью классической модели развертывания, см. в статье о [сбросе пароля или ключа SSH в виртуальных машинах Linux](../linux/classic/reset-access-classic.md).
