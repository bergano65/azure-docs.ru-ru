...
1. Выполните команду `sudo usermod -a -G sudo <username>` , чтобы предоставить привилегии нового корневого пользователя.
...
В RHEL GRUB всегда включается по умолчанию. Чтобы войти в GRUB, Перезагрузите виртуальную машину, выполнив `sudo reboot` команду, а затем нажмите любую клавишу. Должна отобразиться панель GRUB. Если это не так, убедитесь, что в файле GRUB () имеются следующие строки `/etc/default/grub` :

**Для RHEL 8**

>[!NOTE]
> Red Hat рекомендует использовать Грубби для настройки параметров командной строки ядра в RHEL 8 +. Сейчас невозможно обновить время ожидания GRUB и параметры терминала с помощью грубби. Чтобы изменить, обновите аргумент GRUB_CMDLINE_LINUX для всех загрузочных записей выполните команду `grubby --update-kernel=ALL --args="console=ttyS0,115200 console=tty1 console=ttyS0 earlyprintk=ttyS0 rootdelay=300"` . Дополнительные сведения можно найти [здесь](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html/managing_monitoring_and_updating_the_kernel/configuring-kernel-command-line-parameters_managing-monitoring-and-updating-the-kernel).
...
1. Убедитесь, что корневой пользователь может войти только через ttyS0, выполнив следующие действия: a. Выполните команду `edit /etc/ssh/sshd_config` и убедитесь, что для пермитрутлогин задано значение `no` .
...
В CentOS GRUB всегда включается по умолчанию. Чтобы войти в GRUB, Перезагрузите виртуальную машину, введя `sudo reboot` , а затем нажмите любую клавишу. Это действие отображает панель GRUB.
...
В Oracle Linux GRUB всегда включается по умолчанию. Чтобы войти в GRUB, Перезагрузите виртуальную машину, выполнив `sudo reboot` команду, и нажмите клавишу ESC. Это действие отображает панель GRUB. Если панель GRUB не отображается, убедитесь, что значение в `GRUB_TERMINAL` строке содержит *последовательную консоль* (то есть `GRUB_TERMINAL="serial console"` ). Перестройте GRUB с помощью `grub2-mkconfig -o /boot/grub/grub.cfg` .
