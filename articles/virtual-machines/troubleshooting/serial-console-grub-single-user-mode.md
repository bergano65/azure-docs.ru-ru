...
В RHEL GRUB всегда включается по умолчанию. Чтобы попасть в GRUB, перезагрузите виртуальную машину с помощью `sudo reboot` и нажмите любую клавишу. Вы увидите экран GRUB. Если он не отображается, убедитесь, что в файле GRUB (`/etc/default/grub`) имеются следующие строки:
...
> [!NOTE]
> В документации Red Hat можно найти инструкции для загрузки в режиме спасения, аварийном режиме, режиме отладки и для сброса пароля привилегированного пользователя. Щелкните [здесь](https://aka.ms/rhel7grubterminal), чтобы просмотреть эти инструкции.
...
1. Убедитесь, что есть строка с текстом`GRUB_TIMEOUT_STYLE=menu`
...
1. Найдите строку ядра — она начнется с`linux`
...
В Oracle Linux GRUB всегда включается по умолчанию. Чтобы попасть в GRUB, перезагрузите виртуальную машину с помощью `sudo reboot` и нажмите клавишу ESC. Вы увидите экран GRUB. Если вы не видите GRUB, убедитесь, что значение `GRUB_TERMINAL` строки содержит "последовательная консоль", например так:. `GRUB_TERMINAL="serial console"` Перестройте GRUB `grub2-mkconfig -o /boot/grub/grub.cfg`с помощью.
