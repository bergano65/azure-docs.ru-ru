...
Перезагрузка вашей виртуальной машины при открытой колонке последовательной консоли может быть выполнена с помощью команды `'b'`, если [SysRq](./serial-console-nmi-sysrq.md) включена, или нажатием кнопки "Перезагрузить" в колонке обзора (откройте виртуальную машину в новой вкладке браузера, чтобы выполнить перезагрузку, не закрывая колонку последовательной консоли). Следуйте инструкциям для конкретного дистрибутива ниже, чтобы узнать, чего ожидать от GRUB при перезагрузке.

## <a name="general-single-user-mode-access"></a>Общий доступ к однопользовательскому режиму
В ситуациях, когда вы не настроили в учетной записи проверку пароля, может требоваться доступ вручную к однопользовательскому режиму. Необходимо изменить конфигурацию GRUB, чтобы перейти в однопользовательский режим вручную. После этого перейдите к [следующему разделу](#-Use-Single-User-Mode-to-reset-or-add-a-password) для получения дальнейших инструкций.
...
После перехода в однопользовательский режим выполните следующие действия, чтобы добавить нового пользователя с привилегиями sudo:
...
1. Используйте `passwd <username>`, чтобы установить пароль для нового пользователя. Затем вы сможете войти в систему как новый пользователь.
