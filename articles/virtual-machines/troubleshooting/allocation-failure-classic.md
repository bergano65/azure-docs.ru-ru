...
Прежде чем объявить региональную виртуальную сеть, необходимо связать виртуальную сеть с территориальной группой. В результате вычисления ресурсов, помещенных в территориальную группу, привязываются к тем же ограничениям, которые описаны в разделе «сценарий выделения: Территориальная группа (сходство ВМ или службы) выше. Вычислительные ресурсы будут привязаны к одному кластеру.
