...
Функция Сисрк полезна для перезагрузки аварийно завершенной или зависой виртуальной машины непосредственно из последовательной консоли Azure. также она полезна при получении доступа к меню GRUB. Кроме того, перезапуск виртуальной машины из другого окна портала или сеанса SSH может привести к удалению текущего подключения консоли, тем самым истечения времени ожидания GRUB, которое используется для вывода меню GRUB.
...
Как показано здесь: ![Enable sysrq2](./media/virtual-machines-serial-console/enabling-sysrq-2.png)
...
## <a name="red-hat-74-grub-configuration"></a>Конфигурация GRUB Red\.Hat\+ 7 4
...
## <a name="red-hat-72-and-73-grub-configuration"></a>Конфигурация GRUB Red\.Hat 7 2\.и 7 3
...
Файл для изменения —/Бут/груб/груб.конф. `timeout` Значение определяет, как долго будет отображаться GRUB.
