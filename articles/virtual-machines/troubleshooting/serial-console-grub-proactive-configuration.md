...
Функция SysRq полезна для перезагрузки разбившегося или повешенного VM непосредственно с последовательной консоли Azure, также полезной для получения доступа к меню GRUB, альтернативнопере запуск VM из другого окна портала или сеанс ssh может отказаться от текущего подключения консоли таким образом, истекает Сроки GRUB тайм-аутов, к которым используются для отображения меню GRUB.
