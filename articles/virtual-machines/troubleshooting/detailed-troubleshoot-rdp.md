...
## <a name="source-3-cloud-service-endpoint-and-acl"></a>Источник 3: Конечная точка облачной службы и список управления доступом
...
> [!NOTE]
> Для виртуальных машин, созданных в Диспетчер ресурсов, перейдите [к источнику 4. Группы](#source-4-network-security-groups)безопасности сети.
...
* Конфигурация конечной точки для трафика удаленный рабочий стол на целевой виртуальной машине: Частный TCP-порт конечной точки должен совпадать с TCP-портом, который прослушивается службой удаленный рабочий стол виртуальной машины (по умолчанию — 3389).
* Список ACL для конечной точки удаленный рабочий стол трафика на целевой виртуальной машине: Списки управления доступом позволяют указать разрешенный или запрещенный входящий трафик из Интернета на основе его исходного IP-адреса. Если он неправильно настроен, входящие данные с удаленного рабочего стола на эту конечную точку могут блокироваться. Проверьте свои списки управления доступом и убедитесь в том, что входящий трафик с общедоступных IP-адресов вашего прокси-сервера или другого пограничного сервера разрешен. Дополнительные сведения см. в статье [Список управления доступом (ACL) конечной точки](../../virtual-network/virtual-networks-acl.md).
...
## <a name="source-4-network-security-groups"></a>Источник 4: группы сетевой безопасности;
...
## <a name="source-5-windows-based-azure-vm"></a>Источник 5: Виртуальная машина Azure на основе Windows
