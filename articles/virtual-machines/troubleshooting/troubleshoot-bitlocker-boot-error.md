...
2. [Подключите системный диск к виртуальной машине восстановления](troubleshoot-recovery-disks-portal-windows.md). Для выполнения команды [управления-Bde](https://docs.microsoft.com/windows-server/administration/windows-commands/manage-bde) на шаге 7 функция **шифрования Drive BitLocker** должна быть включена в восстановление VM.
...
7.  Чтобы разблокировать прикрепленный диск с помощью файла BEK, запустите следующую команду.
...
8. После того, как диск был успешно разблокирован с помощью ключа BEK, отсоедините диск от восстановления VM, а затем воссоздайте VM с помощью этого нового диска ОС.

    > [!NOTE]
    > Замена ДИСКа ОС не поддерживается для VMs с помощью шифрования диска.

9. Если новый VM по-прежнему не может загружаться нормально, попробуйте один из следующих шагов после разблокировки диска:

    - Приостановить защиту временно включить BitLocker OFF, запустив следующее:
...
2. Сохранить следующий сценарий для . Файл PS1:
    ...
    ```
3. Задайте параметры. Сценарий будет обрабатывать секрет KEK для создания ключа BEK, а затем сохранит его в локальной папке на виртуальной машине восстановления. Если вы получаете ошибки при запуске скрипта, просмотрите раздел [устранения неполадок скрипта.](#script-troubleshooting)
...
6. После того, как диск был успешно разблокирован с помощью ключа BEK, отсоедините диск от восстановления VM, а затем воссоздайте VM с помощью этого нового диска ОС. 

    > [!NOTE]
    > Замена ДИСКа ОС не поддерживается для VMs с помощью шифрования диска.

7. Если новый VM по-прежнему не может загружаться нормально, попробуйте один из следующих шагов после разблокировки диска:

    - Приостановить защиту, чтобы временно включить BitLocker OFF, запустив следующую команду:
    ...
    - Полностью расшифровать диск. Для этого выполните следующую команду:
                    ...
                    manage-bde -off F:
## <a name="script-troubleshooting"></a>Устранение неполадок в сценарии

**Ошибка: не удалось загрузить файл или сборку**

Эта ошибка возникает из-за того, что пути aDAL-агрегатов неверны. Если модуль АЗ установлен только для текущего пользователя, сборки ADAL `C:\Users\<username>\Documents\WindowsPowerShell\Modules\Az.Accounts\<version>`будут расположены в .

Вы также можете `Az.Accounts` искать папку, чтобы найти правильный путь.

**Ошибка: Get-AzKeyVaultSecret или Get-AzKeyVaultSecret не признается как название cmdlet**

Если вы используете старый модуль A' PowerShell, `Get-AzureKeyVaultSecret` необходимо `Get-AzureKeyVaultSecret`изменить две команды на и.

**Образцы параметров**

| Параметры  | Образец значения  |Комментарии   |
|---|---|---|
|  $keyVaultName | myKeyVault2112852926  | Название ключевого Убежища, в хранилище которого хранится ключ |
|$kekName   |mykey   | Имя ключа, который используется для шифрования VM|
|$secretName   |7EB4F531-5FBA-4970-8E2D-C11fd6B0C69D  | Название секрета ключа VM|
|$bekFilePath   |в: Збек-7EB4F531-5FBA-4970-8E2D-C11FD6B0C69D. Бек |Путь для написания файла BEK.|
|$adTenant  |contoso.onmicrosoft.com   | ФЗДН или GUID вашего каталога Azure Active, в котором размещается хранилище ключей |
