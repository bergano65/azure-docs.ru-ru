...
1. Создайте копию диска операционной системы из моментального снимка.
1. Подключите и подключите скопированный диск ОС к другой виртуальной машине Windows, а затем создайте на нем файлы конфигурации. Файлы помогут сбросить пароль.
1. Отключите и отсоедините скопированный диск ОС от виртуальной машины для устранения неполадок.
1. Замените диск операционной системы на затронутую виртуальную машину.

## <a name="detailed-steps-for-the-vm-with-resource-manager-deployment"></a>Подробные инструкции для виртуальной машины с развертыванием диспетчер ресурсов
...
1. Создайте моментальный снимок для диска операционной системы затронутой виртуальной машины, в нем создается диск из моментального снимка, после чего диск подключается к виртуальной машине для устранения неполадок. Дополнительные сведения см. [в разделе Устранение неполадок виртуальной машины Windows путем подключения диска ОС к виртуальной машине восстановления с помощью портал Azure](troubleshoot-recovery-disks-portal-windows.md).
...
5. В расположении `FixAzureVM.cmd` создайте файл `\Windows\System32` со следующим содержимым, заменив `<username>` и `<newpassword>` своими значениями:
...
6. В портал Azure отключите диск от виртуальной машины для устранения неполадок.

7. [Измените диск операционной системы для затронутой виртуальной машины](troubleshoot-recovery-disks-portal-windows.md#swap-the-os-disk-for-the-vm).
...
4. Создайте `gpt.ini` в `\Windows\System32\GroupPolicy` на диске исходной виртуальной машины (если `gpt.ini` существует, переименуйте в `gpt.ini.bak`):
   ...
   > [!WARNING]
   > Убедитесь, что вы случайно не создаете следующие файлы в `C:\Windows`, диске операционной системы для виртуальной машины для устранения неполадок. Создайте файлы на диске операционной системы исходной виртуальной машины, который подключен в качестве диска данных.
...
6. В расположении `FixAzureVM.cmd` создайте файл `\Windows\System32` со следующим содержимым, заменив `<username>` и `<newpassword>` своими значениями:
...
2. В режиме удаленного сеанса на новой виртуальной машине удалите следующие файлы, чтобы очистить среду:
    ...
    * От `%windir%\System32\GroupPolicy`
      * Удалите `gpt.ini` (если `gpt.ini` существовали ранее и вы переименовали его в `gpt.ini.bak`, переименуйте файл `.bak` обратно в `gpt.ini`).
