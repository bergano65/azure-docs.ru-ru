...
3.  Откройте экземпляр повышенной команды быстрого запроса **(Запуск в качестве администратора**). Затем выполните следующий сценарий. Этот скрипт предполагает, что дисковая буква, назначенная прикрепленному к прикрепленному диску ОС, является **F.** Замените письмо по мере необходимости значением в вашем VM.
