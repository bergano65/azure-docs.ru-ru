...
### <a name="example-3-a-vm-cannot-start-because-of-an-fstab-misconfiguration-or-because-the-disk-is-no-longer-attached"></a>Пример 3. невозможно запустить виртуальную машину из-за неfstabй конфигурации или из-за того, что диск больше не подключен
...
2. Использование последовательной консоли для использования [однопользовательского режима режима одного](https://docs.microsoft.com/azure/virtual-machines/linux/serial-console-grub-single-user-mode) пользователя
...
5. Измените или закомментируйте любые неправильные или ненужные строки в файле fstab, чтобы обеспечить правильную загрузку виртуальной машины.

6. Сохраните изменения в файле fstab.

7. Перезагрузите виртуальную машину с помощью приведенной ниже команды.
   
   ```
   # reboot -f
   ```
> [!Note]
   > Можно также использовать команду "Ctrl + x", которая также может перезагрузить виртуальную машину.


8. Если комментарий или исправление записей были успешными, система должна связаться с запросом bash на портале. Проверьте, можно ли подключиться к виртуальной машине.

### <a name="using-root-password"></a>Использование пароля root
