---
title: Устранение неисправного VM Azure с помощью вложенной виртуализации в Azure Документы Майкрософт
...
---
# <a name="troubleshoot-a-faulty-azure-vm-by-using-nested-virtualization-in-azure"></a>Устранение неисправного VM Azure с помощью вложенной виртуализации в Azure

В этой статье показано, как создать среду вложенной виртуализации в Microsoft Azure, чтобы можно было смонтировать диск неисправного VM на хосте Hyper-V (Rescue VM) для устранения неполадок.

## <a name="prerequisites"></a>Предварительные требования

Для того, чтобы установить неисправный VM, Rescue VM должен использовать тот же тип учетной записи хранения (Standard или Premium), что и неисправный VM.
...
1.  Создайте новую виртуальную машину спасения:
    ...
    -  То же место, учетная запись хранения и ресурсная группа, как неисправный VM.

    -  Выберите тот же тип хранилища, что и неисправный VM (Standard или Premium).

2.  После создания виртуальной машины спасения установите к ней подключение удаленного рабочего стола.

3.  В серверном менеджере выберите **Управление** > **Добавлением Ролей и Функций.**
...
6.  Выберите > **функции добавления** **роли Hyper-V.**
...
## <a name="step-2-create-the-faulty-vm-on-the-rescue-vms-hyper-v-server"></a>Шаг 2: Создайте неисправный VM на гипер-V сервере Rescue VM

1.  [Создайте диск моментального снимка](troubleshoot-recovery-disks-portal-windows.md#take-a-snapshot-of-the-os-disk) для диска ОС VM, который имеет проблемы, а затем прикрепите диск моментального снимка к recuse VM.

2.  Удаленный рабочий стол для спасательной ВМ.

3.  Откройте управление дисками (diskmgmt.msc). Убедитесь, что диск неисправного VM установлен в **автономном режиме.**
...
5.  В Hyper-V Manager, право йнюж спасения VM, а затем выбрать **новый** > **виртуальный машина** > **Следующая**.
...
14. В **физическом жестком диске**выберите диск неисправного VM, который вы прикрепили к Azure VM. Если диски в списке отсутствуют, с помощью параметра "Управление дисками" проверьте заданное состояние для диска ("Отключен").
...
## <a name="step-3-replace-the-os-disk-used-by-the-faulty-vm"></a>Шаг 3: Заменить диск ОС, используемый неисправным VM

1.  После возвращения виртуальной машины в оперативный режим завершите ее работу в диспетчере Hyper-V.

2.  [Открепите и отсоедините отремонтированный диск ОС.](troubleshoot-recovery-disks-portal-windows.md#unmount-and-detach-original-virtual-hard-disk
)
3.  [Замените диск ОС, используемый VM, на отремонтированный диск ОС.](troubleshoot-recovery-disks-portal-windows.md#swap-the-os-disk-for-the-vm
)
