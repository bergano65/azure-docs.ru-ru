...
> [!IMPORTANT]
> * Сценарии в этой статье применяются только к виртуальным машинам, которые используют [Azure Resource Manager](../../azure-resource-manager/management/overview.md).
...
Дополнительную документацию и инструкции см. в описании [az vm repair](/cli/azure/ext/vm-repair/vm/repair).
...
4. Выполните `az vm repair run`. Эта команда запустит указанный скрипт исправления на подключенном диске через виртуальную машину для исправления. Если вы используете руководство по устранению неполадок с указанным идентификатором запуска, используйте его здесь. в противном случае для просмотра доступных сценариев восстановления можно использовать команду AZ VM Repair List-Scripts. Используемые здесь группа ресурсов и имя виртуальной машины относятся к неработающей виртуальной машине, упоминавшейся на шаге 3. Дополнительные сведения о сценариях восстановления можно найти в [библиотеке сценариев восстановления](https://github.com/Azure/repair-script-library).
