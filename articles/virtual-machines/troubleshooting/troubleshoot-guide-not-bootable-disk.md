...
2. Введите *DiskPart* , чтобы запустить средство DiskPart.
3. Введите *List Disk* , чтобы вывести список дисков в системе и указать подключенный виртуальный жесткий диск ОС.
4. После того как подключенный виртуальный жесткий диск с ОС будет найден, введите *SEL Disk #* , чтобы выбрать диск.  См. рис. 2, где Disk 1 — подключенный виртуальный жесткий диск ОС.
...
5. После выбора диска введите *list partition (список* разделов), чтобы отобразить разделы выбранного диска.
6. После определения загрузочного раздела введите *SEL Partition #* , чтобы выбрать секцию.  Обычно размер загрузочного раздела составляет около 350 МБ.  См. рис. 3, где Partition 1 — загрузочный раздел.
...
2. Используйте следующую команду для запуска *программы chkdsk* на дисках и исправления ошибок:
   ...
   При добавлении команды "/f" исправляются все ошибки на диске. Обязательно замените <DRIVE LETTER> на букву подключенного виртуального жесткого диска ОС.
