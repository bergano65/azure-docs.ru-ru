...
Причина. Истек 120-дневный льготный период лицензирования для роли сервера удаленных рабочих столов, необходимо установить лицензии.
...
Причина. Клиент удаленного рабочего стола на компьютере не удается разрешить имя компьютера в параметрах RDP-файла.
...
Причина. Целевой виртуальной Машине не удается найти администратора безопасности в части имени пользователя учетные данные.

Если имя пользователя указывается в формате *Система_безопасности*\\*UserName* (пример: CORP\User1), *Система_безопасности* часть — имя компьютера виртуальной Машины (для локальной системы безопасности) или имя домена Active Directory.
...
## <a name="windows-security-error-your-credentials-did-not-work"></a>Ошибка безопасности Windows: Учетные данные не работает.
Причина. Целевой виртуальной Машине не удается проверить имя учетной записи и пароль.

Компьютер Windows принимает данные локальных учетных записей и учетных записей домена.

* Для локальных учетных записей, используйте *ComputerName*\\*UserName* синтаксис (пример: SQL1\Admin4798).
* Учетные записи домена, используйте *DomainName*\\*UserName* синтаксис (пример: CONTOSO\peterodman).
...
Причина. Учетная запись, которая используется для подключения нет прав входа в систему удаленный рабочий стол.
