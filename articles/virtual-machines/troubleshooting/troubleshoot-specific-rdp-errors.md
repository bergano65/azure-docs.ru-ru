...
* [Удаленная сессия была отключена из-за того, что для предоставления лицензии отсутствуют серверы лицензий удаленного рабочего стола.](#rdplicense)
...
* [Ошибка безопасности Windows: Ваши учетные данные не работали.](#wincred)
...
Когда ваше имя пользователя находится в форме *SecurityAuthority*\\*UserName* (пример: CORP-User1), часть *SecurityAuthority* — это либо имя компьютера VM (для местных органов безопасности), либо доменное имя Active Directory.
...
* Для локальных учетных записей используйте синтаксис *ComputerName*\\*UserName* (например, S'L1-Admin4798).
* Для учетных записей доменов используйте синтаксис *DomainName*\\*UserName* (пример: CONTOSO'peterodman).
