...
Причина. Срок действия лицензии 120-дневного периода лицензирования для роли сервера удаленный рабочий стол истек, и необходимо установить лицензии.
...
Причина. Клиент удаленный рабочий стол на компьютере не может разрешить имя компьютера в параметрах RDP-файла.
...
Причина. Целевая виртуальная машина не может выбрать центр безопасности в части имени пользователя ваших учетных данных.

Если имя пользователя указано в формате *секуритяусорити*\\*имя_пользователя* (пример: CORP\User1), часть *секуритяусорити* является либо именем компьютера виртуальной машины (для локального центра безопасности), либо Active Directory именем домена.
...
## <a name="windows-security-error-your-credentials-did-not-work"></a>Ошибка безопасности Windows: Ваши учетные данные не работали.
Причина. Целевая виртуальная машина не может проверить имя и пароль вашей учетной записи.
...
Причина. Учетная запись, используемая для подключения, не имеет удаленный рабочий стол прав входа.
