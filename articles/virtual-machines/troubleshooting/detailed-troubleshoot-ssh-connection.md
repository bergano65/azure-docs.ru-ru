...
* **Конфигурация конечной точки для SSH-трафика на целевой виртуальной машине.** Частный TCP-порт на конечной точке должен совпадать с TCP-портом, который прослушивает SSH-служба на виртуальной машине. (Порт по умолчанию — 22.) Проверьте номер порта SSH TCP на портале Azure, выбрав **виртуальные машины** > *VM-именных* > **конечных точек****настройки.** > 
...
Дополнительные сведения об устранении неполадок с доступом к приложению см. в статье [Устранение неполадок доступа к приложению, выполняющемуся в виртуальной машине Azure](../linux/troubleshoot-app-connection.md).
