...
>[!NOTE]
>Если вы используете VPN типа "сеть — сеть" и принудительное туннелирование, см. статью [Использование настраиваемых маршрутов Azure для включения активации KMS с принудительным туннелированием](../../vpn-gateway/vpn-gateway-about-forced-tunneling.md). 
>
>Если вы используете ExpressRoute и опубликован маршрут по умолчанию, см. статью [можно ли блокировать подключение к Интернету к виртуальным сетям, подключенным к каналам ExpressRoute?](../../expressroute/expressroute-faqs.md).
...
2. Если команда **slmgr.vbs /dlv** отображает канал RETAIL, то выполните следующие команды, чтобы задать [ключ установки клиента KMS](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj612867(v=ws.11)?f=255&MSPPError=-2147217396) для используемой версии Windows Server и принудительно повторить активацию: 
...
1. Скачайте и извлеките средство [PSping](/sysinternals/downloads/psping) в локальную папку на виртуальной машине, которая не активируется. 
...
5. Проверьте использование [наблюдателя за сетями следующий прыжок](../../network-watcher/network-watcher-next-hop-overview.md) , чтобы тип следующего прыжка из виртуальной машины на целевой IP-адрес 23.102.135.246 (для KMS.Core.Windows.NET) или IP-адрес соответствующей КОНЕЧНОЙ точки KMS, применяемой к вашему региону, — **Интернет**.  Если результат — VirtualAppliance или шлюза виртуальной сети, скорее всего, существует маршрут по умолчанию.  Обратитесь к администратору сети и поработайте с ними, чтобы определить правильный курс действий.  Это может быть [Настраиваемый маршрут](./custom-routes-enable-kms-activation.md) , если решение соответствует политикам вашей организации.
...
Если льготный период истек, а ОС Windows еще не активирована, то в Windows Server 2008 R2 и более поздних версиях Windows отобразятся дополнительные уведомления об активации. Фоновый рисунок рабочего стола будет оставаться черным, а Центр обновления Windows будет устанавливать только обновления системы безопасности и критические обновления без возможности установить необязательные обновления. Ознакомьтесь с разделом "Notifications" (Уведомления) в нижней части страницы [Licensing Conditions](/previous-versions/tn-archive/ff793403(v=technet.10)) (Условия лицензирования).   
