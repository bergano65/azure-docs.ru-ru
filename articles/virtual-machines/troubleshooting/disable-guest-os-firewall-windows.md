...
description: Изучите метод обхода для устранения неполадок, когда брандмауэр гостевой операционной системы фильтрует частичный или полный трафик на VM.
...
Описанное решение позволит вам сосредоточиться на исправлении реальной проблемы, то есть на правильной настройке правил брандмауэра. Это лучшая практика Microsoft, чтобы иметь компонент Windows Firewall включен. Настройка правил брандмауэра зависит от требуемого уровня доступа к VM.
...
1.  На устранение неполадок VM, запустите редактор реестра, а затем перейдите в**реестр сети** **файлов** > Connect .

2.  Откройте ветку *TARGET MACHINE*SYSTEM и укажите следующие значения:
...
3.  Перезапустите службу. Поскольку вы не можете сделать это с помощью удаленного реестра, вы должны использовать консоль удаленного обслуживания.
...
7.  Введите **частный IP-адрес (DIP)** проблемы VM.
