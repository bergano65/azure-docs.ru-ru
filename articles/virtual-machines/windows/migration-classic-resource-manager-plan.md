...
- **Развертывания веб-ролей и рабочих ролей.** Облачные службы, содержащие веб-роли и рабочие роли, невозможно переместить в Azure Resource Manager. Чтобы перенести содержимое ролей в веб-и рабочих, необходимо перенести сам код в новые службы приложений PaaS (эта дискуссия выходит за рамки этого документа). Если вы хотите оставить роли веб-сети/работника как есть, но перенести классические виртуальные в модели развертывания менеджера ресурсов, роли веб/рабочего должны быть сначала удалены из виртуальной сети, прежде чем может начаться миграция.  Типичным решением является простое перемещение экземпляров ролей веб-/работников в отдельную виртуальную сеть Classic, которая также связана с схемой ExpressRoute. В первом случае передислокации создайте новую виртуальную сеть Classic, переместите/передислоцируете роли веб/рабочих в эту новую виртуальную сеть, а затем удалите развертывания из перемещенной виртуальной сети. Не требует изменения кода. Вы можете использовать новую функцию [пиринга между виртуальными сетями](../../virtual-network/virtual-network-peering-overview.md), чтобы установить пиринг между классической виртуальной сетью, содержащей веб-роли и рабочие роли, и другими виртуальными сетями в одном регионе Azure (например, это может быть **уже перемещенная виртуальная сеть, так как пиринговые виртуальные сети переместить невозможно**). Это не повлияет на возможности и не приведет к потере производительности или штрафам за задержки или пропускную способность. С помощью [пиринга между виртуальными сетями](../../virtual-network/virtual-network-peering-overview.md) теперь можно легко переносить развертывания веб-ролей и рабочих ролей, не вызывая блокирование миграции в модель Azure Resource Manager.
