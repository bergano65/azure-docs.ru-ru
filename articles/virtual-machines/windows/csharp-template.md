...
2. В Visual Studio нажмите **Файл** > **Новый** > **проект**.
3. В **шаблонах** > **Visual C ,** выберите **Консоль App (.NET Framework)**, введите *myDotnetProject* для названия проекта, выберите местоположение проекта, а затем нажмите **OK**.
...
1. Нажмите **Инструменты** > **Nuget менеджер пакета**, а затем нажмите пакет менеджер **консоли**.
...
1. В Solution Explorer, правой кнопкой *мыши myDotnetProject* > **Добавить** > **новый пункт**, а затем выбрать **текстовый файл** в *визуальных элементов C .* Назовите файл *CreateVMTemplate.json*, а затем щелкните **Добавить**.
...
Чтобы указать значения параметров ресурса в шаблоне, необходимо создать файл параметров, содержащий значения.

1. В Solution Explorer, правой кнопкой *мыши myDotnetProject* > **Добавить** > **новый пункт**, а затем выбрать **текстовый файл** в *визуальных элементов C .* Назовите файл *Parameters.json*, а затем щелкните **Добавить**.
2. Добавьте следующий код JSON в созданный файл:
...
1. В Solution Explorer, правой кнопкой *мыши myDotnetProject* > **Добавить** > **новый пункт**, а затем выбрать **текстовый файл** в *визуальных элементов C .* Назовите файл *azureauth.properties* и щелкните **Добавить**.
...
4. Установите переменную среды в windows named AZURE_AUTH_LOCATION с полным способом создания файла авторизации, например, вы можете использовать следующую команду PowerShell:
...
1. Откройте файл Program.cs для созданного проекта. Затем добавьте их с помощью инструкций к существующим инструкциям в верхней части файла:
