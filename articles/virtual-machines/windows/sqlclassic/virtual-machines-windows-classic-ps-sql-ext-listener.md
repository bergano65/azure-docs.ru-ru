---
title: Настройка внешнего слушателя для групп доступности
...
---
# <a name="configure-an-external-listener-for-availability-groups-on-azure-sql-server-vms"></a>Настройка внешнего слушателя для групп доступности на VMs-серверах Azure S'L Server
> [!div class="op_single_selector"]
> * [Внутренний слушатель](../classic/ps-sql-int-listener.md)
> * [Внешний слушатель](../classic/ps-sql-ext-listener.md)
> 
> 
...
2. Щелкните вкладку **Конечные точки** каждой из ВМ.
3. Убедитесь, что **Имя** и **Общий порт** конечной точки прослушивателя, которые вы собираетесь использовать, еще не используются. В следующем примере используется имя MyEndpoint и порт 1433.
4. На локальном клиенте загрузите и [установите новейший модуль PowerShell.](https://azure.microsoft.com/downloads/)
...
Для того, чтобы получить доступ к слушателю из-за пределов виртуальной сети, вы должны использовать внешний / общественный баланс нагрузки (описанные в этой теме), а не ILB, который доступен только в том же VNet. В строке подключения укажите имя облачной службы. Например, если ваша облачная служба называется *mycloudservice*, оператор sqlcmd будет выглядеть так:
