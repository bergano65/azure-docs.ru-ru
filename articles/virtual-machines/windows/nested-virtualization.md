...
Чтобы настроить шлюз, необходимы некоторые сведения о сети:    
  * IPAddress. IP-адрес шлюза NAT определяет адрес IPv4 или IPv6, используемый в качестве адреса шлюза по умолчанию для подсети виртуальной сети. Универсальный формат — a.b.c.1 (например, "192.168.0.1"). Хотя окончательное положение не должно быть .1, как правило, (на основе длины префикса). Обычно следует использовать адресное пространство для частной сети RFC 1918. 
...
>[!IMPORTANT] 
>
>Гостевой агент Azure не поддерживается на вложенных ВМ и может вызвать проблемы как у host, так и для вложенных VM. Не устанавливайте агент Azure на вложенные VMs и не используйте изображение для создания вложенных VMs, в состав которого уже установлен гостевой агент Azure.
...
3. Войти на властную машину для гостей.
...
## <a name="set-up-intranet-connectivity-for-the-guest-virtual-machine"></a>Настройка подключения к интрасети для гостевой виртуальной машины

Инструкции по обеспечению прозрачного подключения между гостевыми виртуальными машинами и виртуальными машинами Azure см. [в этом документе](https://docs.microsoft.com/virtualization/hyper-v-on-windows/user-guide/nested-virtualization-azure-virtual-network).
