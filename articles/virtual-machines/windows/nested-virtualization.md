...
Чтобы настроить шлюз, необходимы некоторые сведения о сети:    
  * IPAddress. IP-адрес шлюза NAT определяет адрес IPv4 или IPv6, используемый в качестве адреса шлюза по умолчанию для подсети виртуальной сети. Универсальный формат — a.b.c.1 (например, "192.168.0.1"). Хотя конечное расположение не обязательно должно быть равно 0,1, обычно это (на основе длины префикса). Обычно следует использовать адресное пространство для частной сети RFC 1918. 
