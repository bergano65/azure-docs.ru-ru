...
> [!NOTE]
> Платформа Azure подключает файл ISO к DVD-ROM при создании виртуальной машины Windows из обобщенного образа.
> По этой причине DVD-диск должен быть включен в ОС в обобщенном образе. Если она отключена, виртуальная машина Windows будет заблокирована в OOBE.
...
1. Удалите все постоянные статические маршруты из таблицы маршрутизации.
   ...
   * Проверьте `Persistence Routes` разделы. Если имеется постоянный маршрут, удалите его с помощью `route delete` команды.
2. Удалите прокси-сервер WinHTTP.
    ...
    Если виртуальная машина должна работать с конкретным прокси-сервером, добавьте исключение прокси-сервера в IP-адрес Azure ([168.63.129.16](https://blogs.msdn.microsoft.com/mast/2015/05/18/what-is-the-ip-address-168-63-129-16/
)), чтобы виртуальная машина могла подключаться к Azure:
    ```
...
Убедитесь, что для каждой из приведенных ниже служб Windows заданы значения Windows по умолчанию. Эти службы являются минимальными, которые необходимо настроить для обеспечения подключения к виртуальной машине. Чтобы сбросить параметры загрузки, можно использовать приведенные ниже команды.
...
5. Создайте правило для сети платформы Azure.
   ...
   ``` 
6. Если виртуальная машина будет входить в домен, проверьте следующие политики Azure AD, чтобы убедиться, что прежние параметры не были отменены. 
...
4. Убедитесь, что репозиторий инструментарий управления Windows (WMI) (WMI) является согласованным:
    ...
    ```
    Если репозиторий поврежден, см. раздел [WMI: повреждение репозитория](https://blogs.technet.microsoft.com/askperf/2014/08/08/wmi-repository-corruption-or-not).
...
### <a name="determine-when-to-use-sysprep"></a>Определение необходимости использования Sysprep<a id="step23"></a>    
