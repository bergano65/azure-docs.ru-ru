...
Если вы используете хранилище (цен. категория "Стандартный") (HDD), то следует включить команду **TRIM**. Команда **TRIM** удаляет неиспользуемые блоки на диске, таким образом плата взимается только за ту часть хранилища, которая используется. Использование **TRIM** позволяет сократить затраты, если вы создаете большие файлы, а затем удаляете их. 

Чтобы проверить параметр **TRIM**, откройте командную строку на виртуальной машине Windows и введите следующую команду:
