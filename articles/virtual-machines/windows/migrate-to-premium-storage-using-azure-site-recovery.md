---
title: Перенос ваших вышированных компьютеров Windows в премиум-хранилище Azure с восстановлением сайта Azure
...
2. Выберите **Создать резервное** > **Management** > копирование ресурсов**и восстановление сайта (OMS).** Кроме того, вы можете выбрать**Услуги восстановления** >  **просмотра** > Vault**Add.**
   >[!NOTE]
   >Резервное копирование и восстановление сайта раньше входило в [пакет OMS](/azure/azure-monitor/terminology#april-2018---retirement-of-operations-management-suite-brand).
...
2. Перейдите к **хранилищам восстановления служб** > **настройки** > сайта**Восстановление** > Шаг 1: Подготовка цели**защиты****инфраструктуры.** > 
...
3. На виртуальной машине, которая используется в качестве сервера конфигурации, запустите программу унифицированной установки, чтобы установить сервер конфигурации и сервер обработки. Чтобы выполнить установку, вы можете [использовать снимки экрана](../../site-recovery/vmware-walkthrough-overview.md). Ниже приведены снимки экрана для шагов, необходимых в этом сценарии переноса данных.
   ...
   1. **Прежде чем начать,** выберите **Установить сервер конфигурации и процесс сервера.**
...
Выберите **«Подготовьте** > **цель инфраструктуры»** и укажите модель развертывания, которую необходимо использовать для вс-проектов после сбоя. В зависимости от конкретного сценария можно выбрать **классическую модель** или **модель развертывания Resource Manager**.
...
2. Включите репликацию следующим образом:
   1. Выберите**источник** **приложения** > Replicate . Включив репликацию впервые, выберите **+Replicate** (+ Репликация) в хранилище, чтобы включить репликацию для дополнительных машин.
   2. На шаг 1 навлажете **Source** в качестве сервера процессов.
...
При разработке среды службы хранилища Azure для каждой виртуальной машины в группе доступности рекомендуется использовать отдельные учетные записи хранения. Мы рекомендуем следовать рекомендациям наилучшую практику в нашье для [использования нескольких учетных записей для каждого набора доступности.](../linux/manage-availability.md) Распределение дисков виртуальной машины между несколькими учетными записями позволит повысить доступность хранилища. Кроме того, операции ввода-вывода будут распределены по инфраструктуре службы хранилища Azure.
...
Чтобы проверить, завершена ли репликация, выберите экземпляр восстановления сайта, а затем выберите **Настройки** > **Реплицированных Элементов.** Вы увидите состояние и прогресс процесса репликации в процентах. 
...
Вы можете увидеть состояние теста failover в **настройках** > **Задания** > *YOUR_FAILOVER_PLAN_NAME.* В открывшейся колонке вы увидите разбивку шагов и результаты выполнения (успешные и неудачные). Если какой-либо этап отработки отказа завершится ошибкой, щелкните его, чтобы просмотреть сообщение об ошибке. 
...
* [Создание и загрузка VHD Windows Server в Azure](upload-generalized-managed.md)
