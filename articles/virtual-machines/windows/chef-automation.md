---
title: Развертывание виртуальных машин Azure с помощью Chef
...
Создайте каталог с именем К:\чеф.

Скачайте и установите последнюю версию [Azure CLI](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest) на рабочую станцию.

## <a name="configure-azure-service-principal"></a>Настройка субъекта-службы Azure

В простейших терминах, а субъект-служба Azure — это учетная запись службы.   Мы будем использовать субъект-службу, чтобы помочь нам создавать ресурсы Azure на нашей рабочей станции Chef.  Чтобы создать соответствующий субъект-службу с необходимыми разрешениями, необходимо выполнить следующие команды в PowerShell:
...
Запишите значения SubscriptionID, TenantID, ClientID и секрет клиента (пароль, указанный выше), который потребуется позже. 
...
`organization-validator.pem` следует скачать отдельно, так как это закрытый ключ, а закрытые ключи не должны храниться на сервере Chef. В [Chef управление](https://manage.chef.io/)перейдите в раздел Администрирование и выберите параметр "сбросить ключ проверки", который предоставляет файл для загрузки отдельно. Сохраните файл в каталоге C:\chef.
...
validation_client_name "мйорг-проверяющий"

validation_key "# {current_dir}/мйорг.пем"

нож [: azure_tenant_id] = "0000000-1111-AAAA-BBBB-222222222222"

нож [: azure_subscription_id] = "11111111-bbbbb-КККК-1111-222222222222"

нож [: azure_client_id] = "11111111-bbbbb-КККК-1111-2222222222222"

нож [: azure_client_secret] = "#1234p $wdchef 19"


Эти строки гарантируют, что нож будет ссылаться на каталог справочники в разделе к:\чеф\кукбукс, а также использует субъект-службу Azure, созданный во время операций Azure.
...
На рабочем столе вы увидите CW PowerShell, которая представляет собой рабочую среду, загруженную вместе со средством, необходимым для взаимодействия с продуктами Chef. PowerShell во Вт делает доступными новые нерегламентированные команды, такие как `chef-run`, а также традиционные команды интерфейса командной строки Chef, такие как `chef`. Просмотрите установленную версию рабочей станции Chef и инструментов Chef с помощью `chef -v`. Вы также можете проверить версию рабочей станции, выбрав About Chef Workstation (О рабочей станции Chef) в приложении рабочей станции Chef.
...
Для этого используйте команду **нож azurerm Server Create** .
...
В приведенном выше примере создается виртуальная машина Standard_DS2_v2 с Windows Server 2016, установленная в регионе "Западная часть США". Подставьте свои переменные и запустите команду.

> [!NOTE]
> Мы автоматизируем работу правил сетевых фильтров конечной точки, используя параметр –tcp-endpoints в командной строке. Я открыл порты 80 и 3389, чтобы предоставить доступ к веб-странице и сеансу RDP.
>
>
...
После завершения развертывания общедоступный IP-адрес новой виртуальной машины будет отображаться после завершения развертывания. его можно скопировать и вставить в веб-браузер и просмотреть развернутый веб-сайт. Когда мы развернули виртуальную машину, мы открыли порт 80, чтобы он был доступен извне.   

![][11]

В этом примере используется изобретательный HTML-код.

Можно также просмотреть состояние узла [Chef управление](https://manage.chef.io/). 
