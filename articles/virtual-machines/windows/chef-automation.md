...
validation_client_name "мйорг-проверяющий элемент управления"
...
На рабочем столе вы увидите CW PowerShell, которая представляет собой рабочую среду, загруженную вместе со средством, необходимым для взаимодействия с продуктами Chef. PowerShell во Вт делает доступными новые нерегламентированные команды, такие `chef-run` как, а также традиционные команды интерфейса командной строки Chef `chef`, такие как. Просмотрите установленную версию рабочей станции Chef и инструментов Chef с помощью `chef -v`. Вы также можете проверить версию рабочей станции, выбрав About Chef Workstation (О рабочей станции Chef) в приложении рабочей станции Chef.
...
В приведенном выше примере будет создана виртуальная машина Standard_DS2_v2 с Windows Server 2016, установленная в регионе "Западная часть США". Подставьте свои переменные и запустите команду.
