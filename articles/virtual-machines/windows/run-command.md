...
> [!NOTE]
> Чтобы выполнение команд работало правильно, требуется подключение (через порт 443) к общедоступным IP-адресам Azure. Если расширение не имеет доступа к этим конечным точкам, сценарии могут успешно выполняться, но результаты не будут возвращаться. Если вы блокируете трафик на виртуальной машине, вы можете использовать [теги служб](../../virtual-network/security-overview.md#service-tags), чтобы разрешить трафик к общедоступным IP-адресам Azure с помощью тега `AzureCloud`.
...
Перейдите к виртуальной машине на [портале Azure](https://portal.azure.com) и выберите **Выполнить команду** под **ОПЕРАЦИИ**. Появится список доступных команд, выполняемых на виртуальной машине.
...
Выбрав команду, выберите **Выполнить**, чтобы выполнить сценарий. Сценарий выполняется и по завершении выдает результаты и возможные ошибки в окне вывода. На следующем рисунке показан пример сообщения после запуска команды **RDPSettings**.
...
Можно использовать одну из [встроенных ролей](../../role-based-access-control/built-in-roles.md) или создать [пользовательскую](../../role-based-access-control/custom-roles.md) роль для использования функции выполнения команд.
