---
title: Настройка прослушивателей группы доступности & балансировщика нагрузки (PowerShell)
...
В текущем [шаблоне Microsoft](virtual-machines-windows-portal-sql-alwayson-availability-groups.md) для группы доступности используется Load Balancer (цен. категория "Базовый") с базовыми IP-адресами.
   ...
   > [!NOTE]
   > Если для облака-свидетеля используется стандартный балансировщик нагрузки и служба хранилища Azure, необходимо настроить [конечную точку службы](https://docs.microsoft.com/azure/storage/common/storage-network-security?toc=%2fazure%2fvirtual-network%2ftoc.json#grant-access-from-a-virtual-network) . 
...
## <a name="example-script-add-an-ip-address-to-an-existing-load-balancer-with-powershell"></a><a name="Add-IP"></a>Пример сценария. Добавление IP-адреса в существующую подсистему балансировки нагрузки с помощью PowerShell
...
* Сведения об ограничениях балансировщика нагрузки см. в разделе **частный интерфейсный IP-адрес на подсистеме балансировки нагрузки** в разделе [ограничения сети — Azure Resource Manager](../../../azure-resource-manager/management/azure-subscription-service-limits.md#azure-resource-manager-virtual-networking-limits).
...
1. Перейдите к разделам группы доступности **AlwaysOn высокой доступности** | **группы** | доступности**прослушиватели**. 
...
1. Проверьте подключение с помощью программы **sqlcmd** . Например, следующий сценарий устанавливает подключение **sqlcmd** к первичной реплике с помощью прослушивателя с проверкой подлинности Windows:
...
* Создайте конечную точку службы при использовании подсистемы балансировки нагрузки "Стандартный" с хранилищем Azure для облака-свидетеля. Дополнительные сведения см. в статье [предоставление доступа из виртуальной сети](https://docs.microsoft.com/azure/storage/common/storage-network-security?toc=%2fazure%2fvirtual-network%2ftoc.json#grant-access-from-a-virtual-network).
