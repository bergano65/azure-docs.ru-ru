---
title: Настройка группы слушателей доступности & балансера нагрузки (PowerShell)
...
[Балансизатор нагрузки Azure](../../../load-balancer/load-balancer-overview.md) доступен в 2 SKUs: Basic & Standard. Рекомендуется использовать Load Balancer (цен. категория "Стандартный"). Если виртуальные машины находятся в группе доступности, разрешено использовать Load Balancer (цен. категория "Базовый"). Если виртуальные машины находятся в зоне доступности, требуется стандартный балансер нагрузки. Для Load Balancer (цен. категория "Стандартный") требуется, чтобы во всех виртуальных машинах использовались стандартные IP-адреса.
...
В текущем [шаблоне Microsoft](virtual-machines-windows-portal-sql-alwayson-availability-groups.md) для группы доступности используется Load Balancer (цен. категория "Базовый") с базовыми IP-адресами.
   ...
   > [!NOTE]
   > Для использования стандартного балансивизатора нагрузки и хранилища Azure для облака необходимо настроить [конечную точку службы.](https://docs.microsoft.com/azure/storage/common/storage-network-security?toc=%2fazure%2fvirtual-network%2ftoc.json#grant-access-from-a-virtual-network) 
...
## <a name="example-script-add-an-ip-address-to-an-existing-load-balancer-with-powershell"></a><a name="Add-IP"></a>Пример сценария: Добавление IP-адреса к существующему балансисатору нагрузки с PowerShell
...
* Для получения информации об ограничениях баланса нагрузки **см.** [Networking Limits - Azure Resource Manager](../../../azure-resource-manager/management/azure-subscription-service-limits.md#azure-resource-manager-virtual-networking-limits)
...
1. Перейдите к **всегда On HighOn Доступность** | **Доступность группы** | **доступности Группы слушателей**. 
...
1. В поле **Port** укажите номер порта для слушателя группы доступности, используя $EndpointPort, который вы использовали ранее (1433 был по умолчанию), затем нажмите **OK.**
...
1. Используйте **утилиту sqlcmd** для проверки соединения. Например, следующий скрипт устанавливает подключение **sqlcmd** к основной реплике через слушателя с аутентификацией Windows:
...
* Создайте конечную точку службы при использовании стандартного балансиста нагрузки с Azure Storage для облачного свидетеля. Для получения дополнительной информации [см.](https://docs.microsoft.com/azure/storage/common/storage-network-security?toc=%2fazure%2fvirtual-network%2ftoc.json#grant-access-from-a-virtual-network)
