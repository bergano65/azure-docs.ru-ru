...
- Целевые базы данных должны находиться либо в экземпляре SQL Server по умолчанию, либо в [правильно установленном](virtual-machines-windows-sql-server-iaas-faq.md#administration) именованном экземпляре. 

> [!NOTE]
> Автоматическая архивация зависит от **расширения агента SQL Server IaaS**. В текущей коллекции образов виртуальных машин SQL это расширение присутствует по умолчанию. Дополнительные сведения см. в статье [SQL Server IaaS Agent Extension](virtual-machines-windows-sql-server-agent-extension.md) (Расширение агента SQL Server IaaS).
...
| **Шифрование** |Включено/отключено (отключено) | Включает или отключает шифрование. Если шифрование включено, то сертификаты, используемые для восстановления резервной копии, сохраняются в указанной учетной записи хранения. Он использует тот же контейнер **автоматического резервного копирования** с тем же соглашением об именовании. При изменении пароля создается новый сертификат; при этом старый сертификат сохраняется для восстановления предыдущих резервных копий. |
...
На вкладке **параметры SQL Server** выберите **включить** в разделе **Автоматическая архивация**. На представленном ниже снимке экрана портала Azure показаны параметры **автоматического резервного копирования SQL**.
...
По завершении нажмите кнопку **Применить** в нижней части страницы параметры **резервного копирования** , чтобы сохранить изменения.
...
Чтобы включить шифрование, измените предыдущий скрипт, чтобы передать параметр **EnableEncryption** вместе с паролем (защищенной строкой) для параметра **CertificatePassword** . Следующий скрипт активирует параметры автоматической архивации их предыдущего примера и добавляет шифрование.
...
Во-первых, можно опрашивать состояние, вызывая [msdb.managed_backup.sp_get_backup_diagnostics](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/managed-backup-sp-get-backup-diagnostics-transact-sql). Или запросите возвращающую табличное значение функцию [msdb. managed_backup. fn_get_health_status](https://docs.microsoft.com/sql/relational-databases/system-functions/managed-backup-fn-get-health-status-transact-sql) .
