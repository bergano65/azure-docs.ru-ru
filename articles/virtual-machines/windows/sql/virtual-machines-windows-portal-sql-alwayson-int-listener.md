---
title: Настройка прослушивателей группы доступности & балансировщик нагрузки (портал Azure)
...
---
# <a name="configure-a-load-balancer-for-an-availability-group-on-azure-sql-server-vms"></a>Настройка подсистемы балансировки нагрузки для группы доступности на виртуальных машинах Azure SQL Server
...
3. В **серверных пулах**щелкните **Добавить** , чтобы создать пул внутренних адресов. 
...
3. Настройте пробу в колонке **Добавление пробы** . Для настройки пробы используйте следующие значения:
   ...
   | **Пределах** |*5* |
   | **Пороговое значение неработоспособности** |*2* |
...
3. В колонке **Add load balancing rules** (Добавление правил балансировки нагрузки) настройте правило балансировки нагрузки. Используйте следующие параметры: 
   ...
   | **протокол**; |**TCP** |
   | **порт**. |*1433* |
   | **Внутренний порт** |*1433*. это значение пропускается, так как это правило использует **плавающий IP-адрес (прямой возврат сервера)**. |
...
2. Перейдите в раздел " > **группы** > доступности с **высокой доступностью AlwaysOn****".**  
...
2. Проверьте подключение с помощью программы **sqlcmd** . Например, следующий сценарий устанавливает подключение **sqlcmd** к первичной реплике с помощью прослушивателя с проверкой подлинности Windows:
...
7. Добавьте пробу работоспособности, используя следующие параметры:
   ...
   |**Пределах** |Промежуток времени между повторными попытками выполнения пробы. Используйте значение по умолчанию (5).
   |**Пороговое значение неработоспособности** |Количество последовательных нарушений пороговых значений, после которого виртуальная машина считается неработоспособной.
...
10. Настройте новое правило балансировки нагрузки, используя следующие параметры:
    ...
    |**IP-адрес внешнего интерфейса** |Выберите IP-адрес, который вы создали. 
    |**протокол**; |TCP
    ...
    |**Проверка работоспособности** |Выберите пробу, которую вы создали.
    |**Сохранение сеанса** |Нет
...
4. [Сделайте SQL Server ресурс группы доступности зависимым от точки доступа клиента](#dependencyGroup).

5. [Сделайте ресурс точки доступа клиента зависимым от IP-адреса](#listname).
 
6. [Задайте параметры кластера в PowerShell](#setparam).
...
1. Создайте правило балансировки нагрузки со следующими параметрами:
   ...
   |**IP-адрес внешнего интерфейса** |Используйте тот же интерфейсный IP-адрес, что и для группы доступности.
   |**протокол**; |TCP
...
- [Настройка группы доступности SQL Server Always On на виртуальных машинах Azure в разных регионах](virtual-machines-windows-portal-sql-availability-group-dr.md)
