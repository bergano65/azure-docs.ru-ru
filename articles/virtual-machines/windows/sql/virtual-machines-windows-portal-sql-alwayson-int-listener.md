---
title: Настройка группы слушателей доступности & балансера нагрузки (портал Azure)
...
ms.locfileid: "79096330"
---
# <a name="configure-a-load-balancer-for-an-availability-group-on-azure-sql-server-vms"></a>Наверстируйте балансизатор нагрузки для группы доступности на VMs-серверах Azure S'L Server
...
5. В диалоговом окне **Создание подсистемы балансировки нагрузки** настройте подсистему балансировки нагрузки, задав следующие параметры:
   ...
   | Параметр | Значение |
   | --- | --- |
   | **Название** |Текст, представляющий имя балансировщика нагрузки. Например, **sqlLB**. |
...
3. На **backend пулов,** нажмите **Добавить,** чтобы создать бэк-конец адрес пула. 
...
3. Настройте пробу в колонке **Добавление пробы** . Для настройки пробы используйте следующие значения:
   ...
   | Параметр | Значение |
   | --- | --- |
   | **Название** |Текст, представляющий имя пробы. Например, **SQLAlwaysOnEndPointProbe**. |
   | **Протокол** |**Tcp** |
...
3. В колонке **Add load balancing rules** (Добавление правил балансировки нагрузки) настройте правило балансировки нагрузки. Используйте следующие параметры: 
   ...
   | Параметр | Значение |
   | --- | --- |
   | **Название** |Текст, представляющий имя правила балансировки нагрузки. Например, **SQLAlwaysOnEndPointListener**. |
   | **Протокол** |**Tcp** |
   | **Порт** |*1433* |
   | **Порт бэкенд** |*1433*. Это значение игнорируется, потому что это правило использует **Плавающий IP (прямой возврат сервера)**. |
   | **Проба** |Используйте имя пробы, созданной для этого балансировщика нагрузки. |
   | **Сохранение сеанса** |**Нет** |
   | **Простой тайм-аут (минуты)** |*4* |
   | **Плавающий IP (прямой возврат сервера)** |**Включен** |
...
2. Перейти к **Всегда На высокой доступности доступности** > **групп** > **доступности группы слушателей**.  
...
2. Используйте **утилиту sqlcmd** для проверки соединения. Например, следующий скрипт устанавливает подключение **sqlcmd** к основной реплике через слушателя с аутентификацией Windows:
...
7. Добавьте пробу работоспособности, используя следующие параметры:
   ...
   |Параметр |Значение
   |:-----|:----
   |**Название** |Имя для идентификации пробы.
   |**Протокол** |TCP
   ...
   |**Нездоровый порог** |Количество последовательных нарушений пороговых значений, после которого виртуальная машина считается неработоспособной.
...
10. Настройте новое правило балансировки нагрузки, используя следующие параметры:
    ...
    |Параметр |Значение
    |:-----|:----
    |**Название** |Имя для идентификации правила балансировки нагрузки. 
    |**IP-адрес Frontend** |Выберите IP-адрес, который вы создали. 
    |**Протокол** |TCP
    ...
    |**Порт бэкэнда** |Укажите то же значение, что и для параметра **Порт**.
    |**Бэкэнд бассейн** |Пул, содержащий виртуальные машины с экземплярами SQL Server. 
    |**Зонд здоровья** |Выберите пробу, которую вы создали.
    |**Сохранение сеанса** |None
    |**Простой тайм-аут (минуты)** |Оставьте значение по умолчанию (4).
    |**Плавающий IP (прямой возврат сервера)** | Активировано
...
3. [Настройка ресурса IP для группы доступности.](#congroup)

   >[!IMPORTANT]
   >При создании IP-адрес используйте IP-адрес, добавленный в подсистему балансировки нагрузки.  

4. [Сделайте ресурс группы доступности сервера S'L зависимым от точки доступа клиента.](#dependencyGroup)

5. [Сделайте ресурс точки доступа клиента зависимым от IP-адреса.](#listname)
 
6. [Установите параметры кластера в PowerShell.](#setparam)
...
1. Создайте правило балансировки нагрузки со следующими параметрами:
   ...
   |Параметр |Значение
   |:-----|:----
   |**Название** |Имя для идентификации правила балансировки нагрузки для распределенной группы доступности. 
   |**IP-адрес Frontend** |Используйте тот же интерфейсный IP-адрес, что и для группы доступности.
   |**Протокол** |TCP
   ...
   |**Порт бэкэнда** | 5022 — укажите то же значение, что и для параметра **Порт**.
   |**Бэкэнд бассейн** |Пул, содержащий виртуальные машины с экземплярами SQL Server. 
   |**Зонд здоровья** |Выберите пробу, которую вы создали.
   |**Сохранение сеанса** |None
   |**Простой тайм-аут (минуты)** |Оставьте значение по умолчанию (4).
...
- [Наймите группу доступности сервера S'L на виртуальных машинах Azure в разных регионах](virtual-machines-windows-portal-sql-availability-group-dr.md)
