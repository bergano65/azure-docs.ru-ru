...
| Уровень | Описание |
| --- | --- |
| **Уровень представления** |*Уровень представления* (веб-уровень, интерфейсный уровень) — это уровень, на котором пользователи взаимодействуют с приложением. |
| **Business** |*Бизнес-уровень* (средний уровень) — это уровень, на котором взаимодействуют друг с другом уровень представления и уровень данных; он включает основную функциональность системы. |
| **Данные** |*Уровень данных* — это в сущности сервер, на котором хранятся данные приложения (например сервер, на котором работает SQL Server). |
...
Рекомендуется размещать виртуальные машины, принадлежащие одному уровню, в одной и той же облачной службе и в одной и той же группе доступности. Например, разместите набор веб-серверов в облачной службе **CloudService1** и в группе доступности **AvailabilitySet1**, а набор серверов базы данных — в облачной службе **CloudService2** и в группе доступности **AvailabilitySet2**. Группа доступности в Azure позволяет помещать узлы с высоким уровнем доступности в отдельные домены сбоя и обновления.
...
Кроме того, мы рекомендуем настроить частную сеть для ваших виртуальных машин с помощью виртуальной сети Azure. Это позволит виртуальным машинам обмениваться друг с другом данными через частный IP-адрес. Для получения дополнительной информации [см.](../../../virtual-network/virtual-networks-overview.md)
...
Другой подход к реализации этой модели приложения состоит в использовании консолидированной веб-роли, содержащей компоненты уровня представления и бизнес-уровня, как показано на схеме ниже. Эта модель приложения подходит для приложений, которым требуется конструкция с отслеживанием состояния. Так как для веб-ролей и рабочих ролей Azure предоставляет вычислительные узлы без отслеживания состояния, мы рекомендуем реализовать логику для хранения состояний сеансов с помощью одной из следующих технологий: [кэширование Azure](https://azure.microsoft.com/documentation/services/azure-cache-for-redis/), [табличное хранилище Azure](../../../cosmos-db/table-storage-how-to-use-dotnet.md) или [База данных SQL Azure](../../../sql-database/sql-database-technical-overview.md).
