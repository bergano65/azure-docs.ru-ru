...
1. **Можно ли настраивать конфигурации, которых нет в коллекции виртуальных машин (например Windows 2008 R2 + SQL Server 2012)?**
   ...
   № С помощью портала Azure или [PowerShell](virtual-machines-windows-ps-sql-create.md) вы можете выбрать только один из представленных в коллекции образов виртуальных машин с SQL Server. 
...
1. **Можно ли настроить виртуальную машину для использования собственной лицензии SQL Server, если она была создана из одного из образов коллекции с оплатой по мере использования?**
   ...
   Да. Вы можете легко переходить с одной модели лицензирования на другую, независимо от изначально развернутого образа. Дополнительные сведения см. в статье [How to change the licensing model for a SQL virtual machine in Azure](virtual-machines-windows-sql-ahb.md) (Как изменить модель лицензирования для виртуальной машины SQL в Azure).
...
1. **Будет ли переход на другую модель лицензирования сопровождаться простоем SQL Server?**
   ...
   № [Изменение модели лицензирования](virtual-machines-windows-sql-ahb.md) не сопровождается простоем SQL Server, так как изменение вступает в силу немедленно и не требует перезапуска виртуальной машины. 

1. **Могут ли из подписок CSP активировать Преимущество гибридного использования Azure?**

   Да. [Изменение модели лицензирования](virtual-machines-windows-sql-ahb.md) доступно для подписок CSP. 
...
1. **Приведет ли регистрация виртуальной машины в новом поставщике ресурсов виртуальной машины SQL к дополнительным затратам?**
   ...
   № Поставщик ресурсов виртуальной машины SQL обеспечивает дополнительную управляемость для SQL Server на виртуальной машине Azure без каких-либо дополнительных затрат. 
...
1. **Что произойдет с ресурсом _* Microsoft.SqlVirtualMachine_*, если ресурс виртуальной машины будет перемещен или удален?** 
...
1. **Что происходит с виртуальной машиной при удалении ресурса _* Microsoft.SqlVirtualMachine_*?**
