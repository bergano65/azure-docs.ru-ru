...
1. **Будет ли переход на другую модель лицензирования сопровождаться простоем SQL Server?**
   ...
   Нет. [Изменение модели лицензирования](virtual-machines-windows-sql-ahb.md) не требует какого-либо времени простоя для SQL Server, так как изменение вступает в силу немедленно и не требует ПЕРЕЗАПУСКА виртуальной машины. Тем не менее, чтобы зарегистрировать SQL Server виртуальную машину с помощью поставщика ресурсов виртуальной машины SQL Server, [расширение IaaS SQL](virtual-machines-windows-sql-server-agent-extension.md) является необходимым условием и установка расширения SQL IaaS в _полном_ режиме перезапускает службу SQL Server. Таким образом, если необходимо установить расширение IaaS SQL, установите его в _упрощенном_ режиме для ограниченной функциональности или установите его в _полноэкранном_ режиме в течение периода обслуживания. Расширение SQL IaaS, установленное в _упрощенном_ режиме, может быть обновлено до _полного_ режима в любое время, но требует перезапуска службы SQL Server. 
...
1. **Что считается пассивным экземпляром?**
   ...
   Пассивный SQL Server экземпляр не обслуживает данные SQL Server клиентам или выполняет активные SQL Server рабочие нагрузки. Он используется только для синхронизации с сервером-источником и иным образом поддерживает пассивную базу данных в состоянии «горячего» резервирования. Если данные, такие как отчеты, обслуживаются клиентами, выполняющими активные SQL Server рабочие нагрузки, или выполнение любой работы, отличной от той, что указана в условиях продукта, это должен быть оплаченный лицензированный SQL Server экземпляр. На вторичном экземпляре допускается следующее действие: проверки согласованности базы данных или CheckDB, полные резервные копии, резервные копии журналов транзакций и мониторинг данных об использовании ресурсов. Вы также можете одновременно запустить первичный и соответствующий экземпляр аварийного восстановления в течение короткого периода тестирования аварийного восстановления каждые 90 дней.
   

1. **В каких сценариях может использоваться преимущество аварийного восстановления?**

   В этом [разделе приведены сценарии, в](https://aka.ms/sql2019licenseguide) которых можно использовать преимущества аварийного восстановления. Дополнительные сведения см. в разделе Условия продукта и обратитесь к контактным данным по лицензированию или менеджеру по учетным записям.

1. **Какие подписки поддерживают преимущество аварийного восстановления?**

   Полнофункциональные программы, предлагающие права на получение эквивалентной подписки по программе Software Assurance в виде фиксированной выгоды, поддерживают преимущество аварийного восстановления. Сюда входят. но не ограничивается значениями Open Value (OV), Open Value Subscription (OVS), Соглашение Enterprise (EA), соглашением Enterprise Subscription (EAS) и серверной и облачной регистрацией (SCE). Дополнительные сведения см. в разделе [условия продукта](https://www.microsoft.com/licensing/product-licensing/products) и обратитесь к контактным данным по лицензированию или менеджеру по учетным записям. 
...
1. **Можно ли установить второй экземпляр SQL Server на ту же виртуальную машину? Можно ли изменить установленные компоненты экземпляра по умолчанию?**

   Да. Установочный носитель SQL Server находится в папке на диске **C** . Запустите **файл Setup. exe** из этого расположения, чтобы добавить новые SQL Server экземпляры или изменить другие установленные компоненты SQL Server на компьютере. Обратите внимание, что некоторые функции, такие как автоматическая архивация, автоматическая установка исправлений и Azure Key Vaultная интеграция, работают только с экземпляром по умолчанию или именованным экземпляром, настроенным должным образом (см. вопрос 3). 
...
1. **Можно ли удалить экземпляр по умолчанию SQL Server?**
   ...
   Если вы решили удалить экземпляр по умолчанию, также удалите [расширение агента SQL Server IaaS](virtual-machines-windows-sql-server-agent-extension.md) . 
...
1. **Поддерживаются SQL Server экземпляры отказоустойчивого кластера (FCI) на виртуальных машинах Azure?**
   ...
   Да. Экземпляр отказоустойчивого кластера можно установить с помощью [файловых ресурсов (PFS) Premium](virtual-machines-windows-portal-sql-create-failover-cluster-premium-file-share.md) или [локальных дисковых пространств (S2D)](virtual-machines-windows-portal-sql-create-failover-cluster.md) для подсистемы хранения. Общие файловые ресурсы уровня "Премиум" обеспечивают операции ввода-вывода и пропускную способность, соответствующие потребностям многих рабочих нагрузок. Для рабочих нагрузок, интенсивно использующих ввод-вывод, рассмотрите возможность использования локальных дисковых пространств на основе управляемого уровня Premium или Ultra-Disks. Кроме того, можно использовать сторонние решения кластеризации или хранения, как описано в статье [Высокий уровень доступности и аварийное восстановление для SQL Server на виртуальных машинах Azure](virtual-machines-windows-sql-high-availability-dr.md#azure-only-high-availability-solutions).

   > [!IMPORTANT]
   > На данный момент _полное_ [расширение агента IaaS SQL Server](virtual-machines-windows-sql-server-agent-extension.md) не поддерживается для SQL Server FCI в Azure. Рекомендуется удалить _полное_ расширение с виртуальных машин, участвующих в FCI, и установить расширение в _упрощенном_ режиме. Это расширение поддерживает такие функции, как автоматическое резервное копирование и установка исправлений, а также некоторые функции портала для SQL Server. Эти функции не будут работать для SQL Server виртуальных машин после удаления _полного_ агента.
