...
1. **Как изменить до новой версии или выпуска SQL Server на виртуальной Машине Azure?**

   Клиенты программы Software Assurance имеют возможность обновления на месте своих SQL Server, запущенный на виртуальной Машине Azure с помощью установочного носителя на портале корпоративного лицензирования. Тем не менее в настоящее время нет способа для изменения выпуска экземпляра SQL Server. Создайте виртуальную машину Azure с нужный выпуск SQL Server и затем перенести базы данных на новый сервер, с помощью стандартных [методов переноса данных](virtual-machines-windows-migrate-sql.md).
