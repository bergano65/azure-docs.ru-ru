...
ms.locfileid: "80060109"
---
# <a name="configure-an-availability-group-on-azure-sql-server-virtual-machines-in-different-regions"></a>Налажить группу доступности на виртуальных машинах Azure S'L Server в разных регионах
...
1. [Налаживать соединение VNet-to-VNet с помощью портала Azure.](../../../vpn-gateway/vpn-gateway-howto-vnet-vnet-resource-manager-portal.md)
...
1. Добавление ресурса IP-адреса в кластер.

   Для этого можно использовать диспетчер отказоустойчивости кластеров. Выберите название кластера, затем нажмите правое нажатие на название кластера под **кластерными основными ресурсами** и выберите **Свойства:** 

   ![Свойства кластера](./media/virtual-machines-windows-portal-sql-availability-group-dr/cluster-name-properties.png)

   В поле диалога **Свойств** выберите **Добавить** под **IP-адресом,** а затем добавьте IP-адрес названия кластера из области удаленной сети. Выберите **OK** в диалоговом поле **IP-адреса,** а затем выберите **OK** снова в диалоговом окполе **Cluster Properties,** чтобы сохранить новый IP-адрес. 

   ![Добавление кластерного IP](./media/virtual-machines-windows-portal-sql-availability-group-dr/add-cluster-ip-address.png)


1. Добавьте IP-адрес в качестве зависимости для имени основного кластера.

   Еще раз откройте свойства кластера и выберите вкладку **«Зависимости».** 

   ![Свойства кластера](./media/virtual-machines-windows-portal-sql-availability-group-dr/cluster-ip-dependencies.png)

1. Добавление ресурса IP-адреса в роль группы доступности в кластере. 

   Нажмите правой кнопкой мыши роль группы доступности в Failover кластера менеджер, выберите **Добавить ресурсы**, **Больше ресурсов**, и выберите **IP-адрес**.
...
1. [Установите параметры кластера в PowerShell.](virtual-machines-windows-portal-sql-availability-group-tutorial.md#setparam)
...
Если вам не удается изменить строки подключения, можно настроить кэширование разрешения имен. Узнайте [об ошибке тайм-аута, и вы не можете подключиться к слушателю группы доступности S'L 2012 alwaysOn в среде с несколькими поднетами.](https://support.microsoft.com/help/2792139/time-out-error-and-you-cannot-connect-to-a-sql-server-2012-alwayson-av)
...
1. В **Object Explorer**, правой кнопкой мыши группы доступности, и нажмите **Failover ...**. Студия управления серверами S'L открывает мастеру сбой над сервером S'L.  
...
| ----- | ----- | ----- | ----- | -----
| Основной центр обработки данных | SQL-1 | Первичный | Синхронная | Автоматически
...
- [Выполните запланированное руководство Failover группы доступности (Сервер S'L)](https://msdn.microsoft.com/library/hh231018.aspx)
