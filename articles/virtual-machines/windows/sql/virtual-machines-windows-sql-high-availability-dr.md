...
* [Всегда на Failover кластерных инстанций](https://technet.microsoft.com/library/ms189134.aspx)
...
| --- | --- |
| **Группы доступности** |Реплики доступности, выполняемые на виртуальных машинах Azure в одном регионе, обеспечивают высокий уровень доступности.  Необходимо настроить виртуальную машину контроллера домена, так как для отказоустойчивой кластеризации Windows требуется домен Active Directory.<br/><br/> Для более высокой избыточности и доступности Чаты Пазуры могут быть развернуты в [различных зонах доступности,](../../../availability-zones/az-overview.md) как это описано в [обзоре группы доступности.](virtual-machines-windows-portal-sql-availability-group-overview.md) Если в группах доступности развернуты VMs-сервера s'L Server, то используйте [стандартный баланселист нагрузки](../../../load-balancer/load-balancer-standard-overview.md) для слушателя, как это описано в этих статьях - шаблоны [Azure S'L CLI](virtual-machines-windows-sql-availability-group-cli.md) & [Azure quickstart.](virtual-machines-windows-sql-availability-group-quickstart-template.md)<br/> ![Группы доступности](./media/virtual-machines-windows-sql-high-availability-dr/azure-only-ha-always-on.png)<br/>Дополнительные сведения см. в статье [Автоматическая настройка групп доступности AlwaysOn на виртуальных машинах Azure с использованием Resource Manager](virtual-machines-windows-portal-sql-alwayson-availability-groups.md). |
| **Экземпляры отказоустойчивого кластера** |Failover кластера instances (FCI), которые требуют совместного хранения, могут быть созданы в 4 различных способов.<br/><br/>1. Двухузловый кластер, работающий в Azure VMs с прикрепленным хранилищем с помощью [Windows Server \(2016 Storage Spaces Direct S2D\) ](virtual-machines-windows-portal-sql-create-failover-cluster.md) для предоставления виртуального SAN на основе программного обеспечения.<br/><br/> 2. Двухузловый кластер сбоя, работающий в Azure VMs с использованием [Premium File Share.](virtual-machines-windows-portal-sql-create-failover-cluster-premium-file-share.md) Премиум-файлы являются SSD-поддерживаемыми последовательно-низкой задержкой файловых акций, которые полностью поддерживаются для использования с Failover кластера Instance.<br/><br/>3. Двухузловый кластер сбоя, работающий в Azure VMs с хранением, поддерживаемым сторонним решением кластеризации. В конкретном примере, использующем SIOS DataKeeper, [см.](https://azure.microsoft.com/blog/high-availability-for-a-file-share-using-wsfc-ilb-and-3rd-party-software-sios-datakeeper/)<br/><br/>4. Двухузловый кластер сбоя, работающий в Azure VMs с удаленным хранилищем iSCSI Target, совместно с помощью ExpressRoute. Например, решение NetApp Private Storage (NPS) предоставляет цели iSCSI через ExpressRoute с Equinix для виртуальных машин Azure.<br/><br/>По любым вопросам, связанным с доступом к данным при отработке отказа, касающимся сторонних решений репликации данных и общих хранилищ, следует обратиться к их поставщикам.<br/><br/>|
...
| **Зеркальное использование базы данных** |Основной, зеркальный и другие серверы, выполняемые в разных центрах обработки данных для аварийного восстановления. Необходимо развернуть с помощью серверных сертификатов. Зеркальное отражение базы данных сервера S'L Server не поддерживается ни для сервера S'L Server 2008, ни для S'L Server 2008 R2 на Azure VM. <br/>![Зеркальное отображение базы данных](./media/virtual-machines-windows-sql-high-availability-dr/azure-only-dr-dbmirroring.png) |
...
## <a name="free-dr-replica-in-azure"></a>Бесплатная реплика DR в Azure

Если у вас есть [Software Assurance,](https://www.microsoft.com/licensing/licensing-programs/software-assurance-default?rtc=1&activetab=software-assurance-default-pivot:primaryr3)вы можете реализовать планы гибридного аварийного восстановления (DR) с помощью S'L Server без дополнительных затрат на лицензирование пассивного экземпляра DR.

На рисунке ниже настройка использует сервер S'L, работающий на виртуальной машине Azure, используя 12 ядер в качестве реплики аварийного восстановления для внутреннего развертывания сервера S'L, используя 12 ядер. В прошлом вам нужно было лицензировать 12 ядер сервера S'L для развертывания на территории и виртуальной машины Azure. Новое преимущество предлагает преимущества пассивных реплик, работающих на виртуальной машине Azure. Теперь вам нужно будет лицензировать только 12 ядер сервера S'L, работающих на месте, до тех пор, пока будут выполнены критерии аварийного восстановления для пассивной реплики на Azure Virtual Machine.

![Бесплатная реплика DR в Azure](media/virtual-machines-windows-sql-high-availability-dr/free-dr-replica-azure.png)

Для получения дополнительной информации смотрите [Условия лицензирования продукции](https://www.microsoft.com/licensing/product-licensing/products). 

Чтобы включить это преимущество, перейдите на [ресурс виртуальной машины сервера S'L Server,](virtual-machines-windows-sql-manage-portal.md#access-the-sql-virtual-machines-resource)выберите **настройку** в настройках, а затем выберите опцию **аварийного восстановления** в соответствии с **лицензией сервера S'L**Server. Выберите флажок, чтобы подтвердить, что этот S'L Server VM будет использоваться в качестве пассивной реплики, а затем выберите **Apply,** чтобы сохранить настройки. 

![Настройка реплики DR в Azure](media/virtual-machines-windows-sql-high-availability-dr/dr-replica-in-portal.png)
...
Группы доступности в Azure позволяют размещать узлы с высоким уровнем доступности в отдельные домены сбоя и домены обновления. Платформа Azure назначает каждой виртуальной машине в группе доступности домен обновления и домен сбоя. Эта конфигурация в пределах центра обработки данных обеспечит доступность не менее одной виртуальной машины и достижение показателя 99,95 % уровня обслуживания Azure при событиях запланированного и незапланированного обслуживания. Чтобы настроить настройку высокой доступности, поместите все участвующие виртуальные машины S'L в один и тот же набор доступности, чтобы избежать потери приложения или данных во время события обслуживания. В одной группе доступности можно разместить только узлы из одной облачной службы. Для получения дополнительной [информации см.](../manage-availability.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)

### <a name="high-availability-nodes-in-an-availability-zone"></a>Узлы высокой доступности в зоне доступности
Зоны доступности — уникальные физические расположения в пределах одного региона Azure. Каждая зона состоит из одного или нескольких центров обработки данных, оснащенных независимыми системами электроснабжения, охлаждения и сетевого взаимодействия. Физическое разделение зон доступности в регионе защищает приложения и данные от сбоев в работе центра обработки данных, обеспечивая доступность по крайней мере одной виртуальной машины и удовлетворяет 99,99% Azure SLA. Чтобы настроить высокую доступность, поместите участвующие виртуальные машины S'L, разбросанные по имеющимся зонам доступности в регионе. За передачу данных между виртуальными машинами, размещенными в пределах каждой зоны доступности, будет взиматься дополнительная плата. Для получения дополнительной [информации](/azure/availability-zones/az-overview)см. 
...
Для получения дополнительной информации см. [Группы доступности на настройке в Azure (GUI)](virtual-machines-windows-portal-sql-alwayson-availability-groups.md).
