...
| --- | --- |
| **Группы доступности** |Реплики доступности, выполняемые на виртуальных машинах Azure в одном регионе, обеспечивают высокий уровень доступности.  Необходимо настроить виртуальную машину контроллера домена, так как для отказоустойчивой кластеризации Windows требуется домен Active Directory.<br/><br/> Для повышения избыточности и доступности виртуальные машины Azure можно развернуть в разных [зонах доступности](../../../availability-zones/az-overview.md) , как описано в разделе [Общие сведения о группе доступности](virtual-machines-windows-portal-sql-availability-group-overview.md). Если SQL Server виртуальные машины в группе доступности развернуты в зонах доступности, используйте [стандартный балансировщик нагрузки](../../../load-balancer/load-balancer-standard-overview.md) для прослушивателя, как описано в этих статьях — шаблоны быстрого запуска[Azure](virtual-machines-windows-sql-availability-group-quickstart-template.md)для [виртуальной машины](virtual-machines-windows-sql-availability-group-cli.md) & Azure SQL.<br/> ![Группы доступности](./media/virtual-machines-windows-sql-high-availability-dr/azure-only-ha-always-on.png)<br/>Дополнительные сведения см. в статье [Автоматическая настройка групп доступности AlwaysOn на виртуальных машинах Azure с использованием Resource Manager](virtual-machines-windows-portal-sql-alwayson-availability-groups.md). |
| **Экземпляры отказоустойчивого кластера** |Экземпляры отказоустойчивого кластера (FCI), для которых требуется общее хранилище, можно создать четырьмя разными способами.<br/><br/>1. отказоустойчивый кластер с двумя узлами, работающий на виртуальных машинах Azure с подключенным хранилищем, с использованием [Windows Server 2016 \(Локальные дисковые пространства S2D\) ](virtual-machines-windows-portal-sql-create-failover-cluster.md) для предоставления программной виртуальной сети SAN.<br/><br/> 2. отказоустойчивый кластер с двумя узлами, работающий на виртуальных машинах Azure с помощью [файлового ресурса Premium](virtual-machines-windows-portal-sql-create-failover-cluster-premium-file-share.md). Общие файловые ресурсы уровня "Премиум" представляют собой согласованные файловые ресурсы с низкой задержкой, которые полностью поддерживаются для использования с экземпляром отказоустойчивого кластера.<br/><br/>3. отказоустойчивый кластер с двумя узлами, работающий на виртуальных машинах Azure, с хранилищем, поддерживаемым сторонним решением кластеризации. Конкретный пример, в котором используется SIOS, см. в разделе [высокий уровень доступности для файлового ресурса с помощью отказоустойчивой кластеризации и стороннего программного обеспечения SIOS](https://azure.microsoft.com/blog/high-availability-for-a-file-share-using-wsfc-ilb-and-3rd-party-software-sios-datakeeper/).<br/><br/>4. отказоустойчивый кластер с двумя узлами, работающий на виртуальных машинах Azure с удаленным хранилищем общих блоков iSCSI через ExpressRoute. Например, решение NetApp Private Storage (NPS) предоставляет цели iSCSI через ExpressRoute с Equinix для виртуальных машин Azure.<br/><br/>По любым вопросам, связанным с доступом к данным при отработке отказа, касающимся сторонних решений репликации данных и общих хранилищ, следует обратиться к их поставщикам.<br/><br/>|
...
Дополнительные сведения см. [в статье Настройка групп доступности в Azure (GUI)](virtual-machines-windows-portal-sql-alwayson-availability-groups.md).
