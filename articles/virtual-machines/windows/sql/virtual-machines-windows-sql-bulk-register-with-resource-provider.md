...
В этой статье описывается, как выполнить массовый регистрацию SQL Server виртуальной машины в Azure с помощью поставщика ресурсов виртуальной машины SQL, используя `Register-SqlVMs` командлет PowerShell.

`Register-SqlVMs` Командлет можно использовать для регистрации всех виртуальных машин в заданном списке подписок, групп ресурсов или списка конкретных виртуальных машин. Командлет зарегистрирует виртуальные машины в режиме _упрощенного_ управления, а затем создаст [отчет и файл журнала](#output-description). 
...
Откройте административный терминал PowerShell и перейдите к месту, где был `RegisterSqlVMs.psm1` сохранен файл. Затем выполните следующий командлет PowerShell, чтобы импортировать скрипт как модуль: 
...
При каждом использовании `Register-SqlVMs` командлета создается и отчет, и файл журнала. 

### <a name="report"></a>Report

Отчет создается в виде `.txt` файла с именем `RegisterSqlVMScriptReport<Timestamp>.txt` , в котором метка времени — это время запуска командлета. В отчете перечислены следующие сведения.
