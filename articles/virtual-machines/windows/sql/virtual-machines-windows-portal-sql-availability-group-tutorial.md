...
ms.locfileid: "71036527"
---
# <a name="tutorial-configure-always-on-availability-group-in-azure-vm-manually"></a>Учебник. Настройка группы доступности AlwaysOn на виртуальной машине Azure вручную
...
|![Square](./media/virtual-machines-windows-portal-sql-availability-group-tutorial/square.png)|Учетная запись службы агента SQL Server | Учетная запись домена |  
...
4. В мастере создания кластеров создайте кластер с одним узлом, поэтапно настроив параметры на всех страницах, как указано в таблице ниже.
   ...
   | Страница | Настройки |
   | --- | --- |
   | Перед началом |Использовать значения по умолчанию |
...
1. Настройте следующие параметры для балансировщика нагрузки:
   ...
   | **Location** |Используйте расположение, в котором находится виртуальная машина. |
...
1. Настройте пробу работоспособности прослушивателя, как показано ниже.
   ...
   | --- | --- |---
   | **Name** | Текст | SQLAlwaysOnEndPointProbe |
...
1. Настройте правила балансировки нагрузки прослушивателя, как показано ниже.
   ...
   | Параметр | Описание | Пример
   | --- | --- |---
   | **Name** | Текст | SQLAlwaysOnEndPointListener |
...
1. Задайте правила балансировки нагрузки IP-адресов ядра кластера следующим образом.
   ...
   | Параметр | Описание | Пример
   | --- | --- |---
   | **Name** | Текст | WSFCEndPoint |
   | **Frontend IP address** (Интерфейсный IP-адрес) | Выбор адреса |Используйте адрес, который был создан при настройке IP-адреса WSFC. Он отличается от IP-адреса прослушивателя |
   | **Протокол** | Выберите протокол TCP. |TCP |
