...
ms.locfileid: "80060180"
---
# <a name="tutorial-configure-availability-group-on-azure-sql-server-vm-manually"></a>Учебник: Настройка группы доступности на Azure S'L Server VM вручную
...
Прежде чем приступить к этому руководству, необходимо [настроить необходимые компонентов для создания групп доступности AlwaysOn на виртуальных машинах Azure](virtual-machines-windows-portal-sql-availability-group-prereq.md). Если эти предварительные условия уже выполнены, то можно перейти к [созданию кластера](#CreateCluster).
  ...
  >[!NOTE]
  > Многие из шагов, представленных в этом учебнике, теперь можно автоматизировать с помощью шаблонов [Azure S'L VM CLI](virtual-machines-windows-sql-availability-group-cli.md) и [Azure quickstart.](virtual-machines-windows-sql-availability-group-quickstart-template.md)
...
4. В мастере создания кластеров создайте кластер с одним узлом, поэтапно настроив параметры на всех страницах, как указано в таблице ниже.
   ...
   | Предупреждение проверки |Выберите **Номер, я не требую поддержки от корпорации Майкрософт для этого кластера, и поэтому не хочу запускать тесты проверки. При нажатии кнопки «Далее» продолжайте создавать кластер**. |
...
### <a name="set-the-windows-server-failover-cluster-ip-address"></a>Указание IP-адреса отказоустойчивого кластера Windows Server
  ...
  > [!NOTE]
  > На Windows Server 2019 кластер создает **имя распределенного сервера** вместо **названия кластерной сети.** Если вы используете Windows Server 2019, пропустите все шаги, которые относятся к названию ядра кластера в этом учебнике. Вы можете создать название кластерной сети с помощью [PowerShell.](virtual-machines-windows-portal-sql-create-failover-cluster.md#windows-server-2019) Для получения дополнительной информации ознакомься с блогом [Failover Cluster: Кластерный сетевой объект.](https://blogs.windows.com/windowsexperience/2018/08/14/announcing-windows-server-2019-insider-preview-build-17733/#W0YAxO8BfwBRbkzG.97) 
...
1. На странице **Предупреждение проверки** щелкните **Нет** (в рабочем сценарии вы должны выполнить проверочные тесты). Затем нажмите кнопку **Далее**.
...
1. В **диспетчере сервера** щелкните **Инструменты**. Открытое **управление компьютером**.
...
1. В окне **Разрешения для общей папки** выберите **Настройка разрешений доступа**. Нажмите **пользовательские ...**.
...
4. Щелкните **Применить**. Нажмите кнопку **ОК** во всплывающем диалоговом окне.
...
1. На первом сервере SQL Server в **диспетчере сервера** щелкните **Инструменты**. Открытое **управление компьютером**.
...
* Создание группы доступности **(AG1**) с синхронным коммитом, автоматическим сбой, и читаемые вторичные реплики
...
2. На странице **Введение** нажмите **Далее**. На странице **Указание имени группы доступности** в поле **Имя группы доступности** введите имя группы доступности (например, **AG1**). Нажмите кнопку **Далее**.
...
5. Открывается диалоговое окно **Подключение к серверу**. Введите имя второго сервера в поле **Имя сервера**. Нажмите кнопку **Подключить**.
...
8. На странице **Выбор начальной синхронизации данных** выберите **Полная** и укажите сетевую папку. Это должен быть [созданный вами общий ресурс для резервных копий](#backupshare). В этом примере это было, ** \\ \\ \<Первый сервер\>S'L «Резервное копирование\\**. Нажмите кнопку **Далее**.
...
1. Настройте следующие параметры для балансировщика нагрузки:
   ...
   | Параметр | Поле |
   | --- | --- |
   | **Название** |Используйте для подсистемы балансировки нагрузки текстовое имя, например **sqlLB**. |
   | **Тип** |Внутренние |
   | **Виртуальная сеть** |Используйте имя виртуальной сети Azure. |
   | **Подсети** |Используйте имя подсети, в которой находится виртуальная машина.  |
...
1. Настройте пробу работоспособности прослушивателя, как показано ниже.
   ...
   | --- | --- |---
   | **Название** | Text | SQLAlwaysOnEndPointProbe |
   ...
   | **Нездоровый порог** | Число последовательных сбоев пробы, после которых виртуальная машина будет считаться неработоспособной.  | 2 |
...
1. Настройте правила балансировки нагрузки прослушивателя, как показано ниже.
   ...
   | Параметр | Описание | Пример
   | --- | --- |---
   | **Название** | Text | SQLAlwaysOnEndPointListener |
   | **IP-адрес Frontend** | Выберите адрес. |Используйте адрес, который был создан при создании подсистемы балансировки нагрузки. |
   | **Протокол** | Выберите протокол TCP. |TCP |
   | **Порт** | Создайте порт для прослушивателя группы доступности | 1433 |
   | **Порт бэкенд** | Это поле не используется, если задан плавающий IP-адрес для прямого ответа от сервера. | 1433 |
   | **Проба** |Имя, указанное для пробы. | SQLAlwaysOnEndPointProbe |
   | **Стойкость сессии** | Раскрывающийся список. | **Нет** |
...
1. Настройте пробу работоспособности IP-адреса ядра кластера WSFC следующим образом.
   ...
   | Параметр | Описание | Пример
   | --- | --- |---
   | **Название** | Text | WSFCEndPointProbe |
   | **Протокол** | Выберите протокол TCP. | TCP |
...
1. Задайте правила балансировки нагрузки IP-адресов ядра кластера следующим образом.
   ...
   | Параметр | Описание | Пример
   | --- | --- |---
   | **Название** | Text | WSFCEndPoint |
   | **IP-адрес Frontend** | Выберите адрес. |Используйте адрес, который был создан при настройке IP-адреса WSFC. Он отличается от IP-адреса прослушивателя |
   | **Протокол** | Выберите протокол TCP. |TCP |
   ...
   | **Порт бэкенд** | Это поле не используется, если задан плавающий IP-адрес для прямого ответа от сервера. | 58888 |
...
## <a name="configure-the-listener"></a><a name="configure-listener"></a>Настройка слушателя

Далее нужно настроить прослушиватель группы доступности в отказоустойчивом кластере.

> [!NOTE]
> В этом руководстве показано, как создать отдельный прослушиватель с одним IP-адресом внутренней подсистемы балансировки нагрузки (ILB). Чтобы создать одного или нескольких слушателей с помощью одного или нескольких IP-адресов, [см. Лазурный .](virtual-machines-windows-portal-sql-ps-alwayson-int-listener.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)
>
>
