---
title: 'Учебник: Предпосылки для группы доступности'
...
ms.locfileid: "80060169"
---
# <a name="prerequisites-for-creating-always-on-availability-groups-on-sql-server-on-azure-virtual-machines"></a>Предпосылки для создания групп доступности Always On на сервере S'L на виртуальных машинах Azure
...
Вам понадобится учетная запись Azure. Вы можете [открыть бесплатную учетную запись Azure](https://signup.azure.com/signup?offer=ms-azr-0044p&appId=102&ref=azureplat-generic&redirectURL=https:%2F%2Fazure.microsoft.com%2Fget-started%2Fwelcome-to-azure%2F&correlationId=24f9d452-1909-40d7-b609-2245aa7351a6&l=en-US) или [активировать преимущества абонента Visual Studio.](https://docs.microsoft.com/visualstudio/subscriptions/subscriber-benefits)

## <a name="create-a-resource-group"></a>Создание группы ресурсов
1. Войдите на [портал Azure](https://portal.azure.com).
2. Нажмите, **+** чтобы создать новый объект на портале.
...
В решении используется виртуальная сеть с двумя подсетями. [Обзор виртуальной сети](../../../virtual-network/virtual-networks-overview.md) предоставляет более подробную информацию о сетях в Azure.

Создание виртуальной сети

1. На портале Azure в своей группе ресурсов щелкните **+ Добавить**. 

   ![Изменения](./media/virtual-machines-windows-portal-sql-availability-group-tutorial/02-newiteminrg.png)
...
4. В колонке **Виртуальная сеть** выберите модель развертывания **Resource Manager** и щелкните **Создать**.
   ...
   | --- | --- |
   | **Название** |autoHAVNET |
   | **Адресное пространство** |10.33.0.0/24 |
   | **Название подсети** |Административный |
   ...
   В примере используется имя подсети **Admin**. Эта подсеть предназначена для контроллеров доменов.
...
Новая виртуальная сеть имеет одну подсеть, названную **Admin**. Контроллеры доменов используют эту подсеть. Виртуальные машины SQL Server используют вторую подсеть, **SQL**. Чтобы настроить эту подсеть, сделайте следующее:
...
| --- | --- |
| **Название** |**autoHAVNET** |
| **Адресное пространство** |Это значение зависит от доступных адресных пространств в подписке. Стандартное значение — 10.0.0.0/16. |
| **Название подсети** |**администратор** |
...
Чтобы создать группу доступности, перейдите в колонку группы ресурсов и щелкните **Добавить**. Фильтр результатов, введя **набор доступности.** В результатах выберите **Группа доступности**, затем щелкните **Создать**.
...
После создания сетей, подсетей и наборов доступности вы готовы создать виртуальные машины для контроллеров доменов.
...
| **Расположение** |*Ваше местоположение* |
| **Размер** |DS1_V2 |
| **Хранилище** | **Использование управляемых дисков** - **Да** |
| **Виртуальная сеть** |autoHAVNET |
| **Подсети** |admin |
| **Общественный IP-адрес** |*Совпадает с именем виртуальной машины* |
| **Группа сетевой безопасности** |*Совпадает с именем виртуальной машины* |
| **Набор доступности** |adavailabilityset </br>**Домены сбоя**: 2 </br>**Домены обновления**: 2|
| **Диагностики** |Активировано |
| **Учетная запись хранения диагностики** |*Создается автоматически* |
...
8. Нажмите **OK,** а затем **закрыть,** чтобы совершить изменения. Теперь можно присоединить виртуальную машину к домену **corp.contoso.com**.
...
| --- | --- | --- | ---
|**Имя и фамилия** |Установка |SQLSvc1 | SQLSvc2
...
2. В **менеджере серверов**выберите **Инструменты,** а затем нажмите **Active Directory Administrative Center.**   
...
7. Нажмите кнопку **ОК**, а затем нажмите кнопку **ОК** еще раз. Закройте окно свойств **корпуса.**
...
| --- | --- | --- | --- |
| Выбор соответствующего элемента коллекции |**Windows Server 2016 Datacenter** |**Предприятие SP1 на Windows Server 2016 2016** |**Предприятие SP1 на Windows Server 2016 2016** |
...
| Конфигурация виртуальной машины **Параметры** |**Хранилище**: используйте управляемые диски.<br/>**Виртуальная сеть** = autoHAVNET<br/>**Подсеть** = sqlsubnet(10.1.1.0/24)<br/>**Общедоступный IP-адрес** создается автоматически.<br/>**Группа безопасности сети** = нет<br/>**Диагностика мониторинга** = включено<br/>**Учетная запись хранения для диагностики** = используйте автоматически созданную учетную запись хранения<br/>**Набор доступности** - sqlAvailabilitySet<br/> |**Хранилище**: используйте управляемые диски.<br/>**Виртуальная сеть** = autoHAVNET<br/>**Подсеть** = sqlsubnet(10.1.1.0/24)<br/>**Общедоступный IP-адрес** создается автоматически.<br/>**Группа безопасности сети** = нет<br/>**Диагностика мониторинга** = включено<br/>**Учетная запись хранения для диагностики** = используйте автоматически созданную учетную запись хранения<br/>**Набор доступности** - sqlAvailabilitySet<br/> |**Хранилище**: используйте управляемые диски.<br/>**Виртуальная сеть** = autoHAVNET<br/>**Подсеть** = sqlsubnet(10.1.1.0/24)<br/>**Общедоступный IP-адрес** создается автоматически.<br/>**Группа безопасности сети** = нет<br/>**Диагностика мониторинга** = включено<br/>**Учетная запись хранения для диагностики** = используйте автоматически созданную учетную запись хранения<br/>**Набор доступности** - sqlAvailabilitySet<br/> |
...
> [!NOTE]
> Предлагаемые здесь размеры компьютеров предназначены для тестирования групп доступности в виртуальных машинах Azure. Для достижения наилучшей производительности на производственных нагрузках см. рекомендации по размерам и конфигурации машин s'L Server в [рекомендациях по производительности для сервера S'L В виртуальных машинах Azure.](virtual-machines-windows-sql-performance.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)
>
>
...
### <a name="join-the-servers-to-the-domain"></a><a name="joinDomain"></a>Присоединиться к серверам в домен
...
2. В **серверном менеджере**нажмите **локальный сервер**.
3. Щелкните ссылку **Рабочая группа**.
4. В разделе **Имя компьютера** нажмите **Изменить**.
5. Установите флажок **Домен** и введите в текстовом поле **corp.contoso.com**. Нажмите кнопку **ОК**.
...
7. Когда будет выдано сообщение «Вас приветствует домен corp.contoso.com», нажмите **ОК**.
8. Щелкните **Закрыть**, а затем — **Перезагрузить сейчас** в появившемся диалоговом окне.
...
1. Дождитесь перезапуска виртуальной машины, а затем запустите RDP-файл еще раз на основном контроллере домена, чтобы войти на **sqlserver-0**, используя учетную запись **CORP\DomainAdmin**.
   >[!TIP]
   >Обязательно выполните вход с использованием учетной записи администратора домена. Ранее вы использовали учетную запись администратора BUILTIN. Теперь, когда сервер находится в домене, используйте учетную запись домена. В сеансе RDP укажите*имя пользователя* *DOMAIN.*\\

2. В **менеджере сервера**, выберите **инструменты**, а затем нажмите **Управление компьютером**.
...
1. Подключайтесь к серверу через протокол удаленного рабочего стола (RDP) с помощью учетной * \<записи MachineName\>«DomainAdmin».*
...
1. Щелкните правой кнопкой мыши **Имена входа**. Нажмите **Новый Войти**.

1. В **Логин - Новые**, нажмите **Поиск**.

1. Нажмите **Местоположение**.
...
Повторите эти действия на другой виртуальной машине SQL Server.
  ...
  >[!NOTE]
  > Этот шаг, наряду с фактическим присоединением VMs-сервера к кластеру failover, теперь можно автоматизировать с помощью шаблонов [Azure S'L VM CLI](virtual-machines-windows-sql-availability-group-cli.md) и [Azure quickstart.](virtual-machines-windows-sql-availability-group-quickstart-template.md)
...
- **Зонд балансосивов нагрузки Azure:**<br/>
   Любой доступный порт. В примерах часто используется 59999.
- **База данных, отражающая конечную точку:** <br/>
...
4. Для порта укажите **TCP** и введите соответствующие номера портов. См. следующий пример.
