...
В решении используется виртуальная сеть с двумя подсетями. В разделе [Обзор виртуальной сети](../../../virtual-network/virtual-networks-overview.md) содержатся дополнительные сведения о сетях в Azure.
...
Чтобы создать группу доступности, перейдите в колонку группы ресурсов и щелкните **Добавить**. Отфильтруйте результаты, введя группу **доступности**. В результатах выберите **Группа доступности**, затем щелкните **Создать**.
...
| **Размер** |DS1_V2 |
| **Хранилище** | **Использовать управляемые диски** - **Да** |
...
8. Нажмите кнопку **ОК**, а затем **Закрыть** , чтобы зафиксировать изменения. Теперь можно присоединить виртуальную машину к домену **corp.contoso.com**.
...
| --- | --- | --- | ---
|**Имя** |Установить |SQLSvc1 | SQLSvc2
|**User SamAccountName** |Установить |SQLSvc1 | SQLSvc2

Чтобы создать каждую учетную запись, сделайте следующее:

1. Войдите в виртуальную машину **ad-primary-dc**.
2. В **Диспетчер сервера**выберите **инструменты**, а затем — **центр администрирования Active Directory**.   
...
7. Нажмите кнопку **ОК**, а затем нажмите кнопку **ОК** еще раз. Закройте окно свойства **Corp** .
...
| --- | --- | --- | --- |
| Выбор соответствующего элемента коллекции |**Windows Server 2016 Datacenter** |**SQL Server 2016 с пакетом обновления 1 (SP1) Enterprise на Windows Server 2016** |**SQL Server 2016 с пакетом обновления 1 (SP1) Enterprise на Windows Server 2016** |
...
> [!NOTE]
> Предлагаемые здесь размеры компьютеров предназначены для тестирования групп доступности в виртуальных машинах Azure. Рекомендации по повышению производительности в производственных рабочих нагрузках см. в рекомендациях по SQL Server размера машины и конфигурации в разделе рекомендации по оптимизации [производительности для SQL Server на виртуальных машинах Azure](virtual-machines-windows-sql-performance.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json).
>
>
...
2. В **Диспетчер сервера**щелкните **локальный сервер**.
...
1. Дождитесь перезапуска виртуальной машины, а затем запустите RDP-файл еще раз на основном контроллере домена, чтобы войти на **sqlserver-0**, используя учетную запись **CORP\DomainAdmin**.
   >[!TIP]
   >Обязательно выполните вход с использованием учетной записи администратора домена. Ранее вы использовали учетную запись администратора BUILTIN. Теперь, когда сервер находится в домене, используйте учетную запись домена. В сеансе RDP укажите*имя пользователя* *домена*\\.
...
1. Подключитесь к серверу через протокол удаленного рабочего стола (RDP) с помощью учетной * \<записи\>MachineName \DomainAdmin* .
...
1. Щелкните правой кнопкой мыши **Имена входа**. Щелкните **создать имя входа**.

1. В меню **имя входа — создать**щелкните **Поиск**.
...
- **Проба балансировщика нагрузки Azure:**<br/>
