...
* Теперь вы хорошо понимаете принцип работы групп доступности. Дополнительные сведения см. в разделе [группы доступности Always On (SQL Server)](/sql/database-engine/availability-groups/windows/always-on-availability-groups-sql-server).

> [!NOTE]
> Если вы хотите использовать группы доступности с SharePoint, см. также раздел [настройка SQL Server 2012 группы доступности Always on для sharepoint 2013](/SharePoint/administration/configure-an-alwayson-availability-group).
>
>
...
* **Имя администратора.** Укажите здесь учетную запись пользователя с правами администратора домена, входящую в предопределенную роль системного администратора (sysadmin) для сервера SQL Server на обоих экземплярах SQL Server. Для работы с этим руководством используйте **DomainAdmin**.
...
* **Группа ресурсов**. Это имя группы, в которую входят все ресурсы Azure, созданные нашим шаблоном. В этом руководстве мы будем использовать имя **SQL-HA-RG**. Дополнительные сведения см. в разделе [Azure Resource Manager обзор](../../../azure-resource-manager/management/overview.md#resource-groups).
...
* **Имя группы доступности** — имя кластеризованного ресурса для группы доступности. Для работы с этим руководством используйте **contoso-AG**.
...
* **Порт прослушивателя группы доступности.** Это TCP-порт, на котором работает прослушиватель SQL Server. В этом руководстве используется порт по умолчанию **1433**.
...
* **File Share Witness virtual machine size** (Размер виртуальной машины файлового ресурса-свидетеля) — размер виртуальной машины для файлового ресурса-свидетеля. В этом руководстве используется **a1**.
* **SQL Storage account** (Учетная запись хранения SQL) — имя учетной записи хранения для данных SQL Server и дисков операционной системы. Для работы с этим руководством используйте **имя alwaysonsql01**.
* **DC Storage account** (Учетная запись хранения контроллера домена) — имя учетной записи хранения для контроллеров домена. Для работы с этим руководством используйте **имя alwaysondc01**.
* **SQL Server data disk size** (Размер диска данных SQL Server) — размер диска данных SQL Server в ТБ. Введите число от 1 до 4. Для работы с этим руководством используйте **1**.
* **Оптимизация хранилища** — параметры конфигурации хранилища для виртуальных машин SQL Server на основе типа рабочей нагрузки. В нашем примере все виртуальные машины с SQL Server используют хранилище класса Premium, а для диска Azure задан режим кэширования узла "Только для чтения". Кроме того, параметры SQL Server для рабочей нагрузки можно оптимизировать, выбрав один из трех таких параметров:
  ...
  * **Общая рабочая нагрузка** задает не определенные параметры конфигурации.
  * **Обработка транзакций** устанавливает флаги трассировки 1117 и 1118.
  * **Хранилище данных** устанавливает флаги трассировки 1117 и 610.

В этом руководстве используется **Общая рабочая нагрузка**.
...
* [Общие сведения о дисковых пространствах](https://technet.microsoft.com/library/hh831739.aspx)
...
Дополнительные сведения о рекомендациях по настройке SQL Server см. [в статье рекомендации по оптимизации производительности для SQL Server на виртуальных машинах Azure](virtual-machines-windows-sql-performance.md).
...
* **SQL Server Name Prefix** (Префикс имени SQL Server) — используется для создания имен виртуальных машин SQL Server. Для работы с этим руководством используйте **SQLServer**. Имена шаблонов для виртуальных машин SQL Server: *sqlserver-0* и *sqlserver-1*.
...
* **SQL Server service account user name** (Имя пользователя учетной записи службы SQL Server) — имя учетной записи домена для службы SQL Server. Для работы с этим руководством используйте **SqlService**.
* **Пароль** — пароль к учетной записи службы SQL Server.  Следует использовать сложный пароль. Подтвердите пароль.
* **SQL Auto Patching maintenance schedule** (Расписание автоматической установки исправлений на серверы SQL Server) — здесь указывается день недели, в который Azure будет автоматически устанавливать исправления на серверы SQL Server. Для работы с этим руководством введите **воскресенье**.
...
4. В колонке **SQLServer-0**нажмите кнопку **подключить**. В браузере отобразится сообщение с предложением открыть или сохранить объект удаленного подключения. Нажмите кнопку **Открыть**.
5. В окне **Подключение к удаленному рабочему столу** может появиться предупреждение о том, что издателя этого удаленного подключения невозможно определить. Нажмите кнопку **Соединить**.
