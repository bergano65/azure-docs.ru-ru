...
* Вы уже умеете подготавливать виртуальную машину SQL Server из коллекции виртуальных машин через графический интерфейс пользователя. Для получения более [подробной](virtual-machines-windows-portal-sql-server-provision.md)информации см.
* Теперь вы хорошо понимаете принцип работы групп доступности. Для получения дополнительной информации см. [Всегда на группы доступности (Сервер S'L)](/sql/database-engine/availability-groups/windows/always-on-availability-groups-sql-server).

> [!NOTE]
> Если вы заинтересованы в использовании групп доступности с SharePoint, также [см.](/SharePoint/administration/configure-an-alwayson-availability-group)
>
>
...
* **Имя администратора.** Укажите здесь учетную запись пользователя с правами администратора домена, входящую в предопределенную роль системного администратора (sysadmin) для сервера SQL Server на обоих экземплярах SQL Server. Для этого учебника используйте **DomainAdmin**.
...
* **Группа ресурсов**. Это имя группы, в которую входят все ресурсы Azure, созданные нашим шаблоном. В этом руководстве мы будем использовать имя **SQL-HA-RG**. Для получения дополнительной информации смотрите [обзор менеджера ресурсов Azure](../../../azure-resource-manager/management/overview.md#resource-groups).
...
* **Имя группы доступности** — имя кластеризованного ресурса для группы доступности. Для этого учебника используйте **Contoso-ag**.
* **Имя слушателя группы доступности** используется кластером и балансером внутренней нагрузки. Клиенты, которые подключаются к серверу SQL Server, могут использовать это имя для подключения к нужной реплике базы данных. Мы будем использовать имя **Contoso-listener**.
* **Порт прослушивателя группы доступности.** Это TCP-порт, на котором работает прослушиватель SQL Server. Для этого учебника используйте порт по умолчанию, **1433**.
...
* **File Share Witness virtual machine size** (Размер виртуальной машины файлового ресурса-свидетеля) — размер виртуальной машины для файлового ресурса-свидетеля. Для этого учебника используйте **A1**.
* **SQL Storage account** (Учетная запись хранения SQL) — имя учетной записи хранения для данных SQL Server и дисков операционной системы. Для этого учебника, используйте **alwaysonsql01**.
* **DC Storage account** (Учетная запись хранения контроллера домена) — имя учетной записи хранения для контроллеров домена. Для этого учебника используйте **alwaysondc01**.
* **SQL Server data disk size** (Размер диска данных SQL Server) — размер диска данных SQL Server в ТБ. Введите число от 1 до 4. Для этого учебника используйте **1**.
* **Оптимизация хранилища** — параметры конфигурации хранилища для виртуальных машин SQL Server на основе типа рабочей нагрузки. В нашем примере все виртуальные машины с SQL Server используют хранилище класса Premium, а для диска Azure задан режим кэширования узла "Только для чтения". Кроме того, параметры SQL Server для рабочей нагрузки можно оптимизировать, выбрав один из трех таких параметров:
  ...
  * **Общая рабочая нагрузка** не устанавливает никаких конкретных параметров конфигурации.
  * **Транзакционная обработка** устанавливает след флаг а 1117 и 1118.
  * **Складирование данных** устанавливает след флаг 1117 и 610.

Для этого учебника используйте **общую рабочую нагрузку.**
...
* [Обзор пространства хранения](https://technet.microsoft.com/library/hh831739.aspx)
...
Для получения более подробной информации [Performance best practices for SQL Server in Azure virtual machines](virtual-machines-windows-sql-performance.md)о рекомендациях конфигурации сервера S'L, см.
...
* **SQL Server Name Prefix** (Префикс имени SQL Server) — используется для создания имен виртуальных машин SQL Server. Для этого учебника используйте **sqlserver.** Имена шаблонов для виртуальных машин SQL Server: *sqlserver-0* и *sqlserver-1*.
...
* **SQL Server service account user name** (Имя пользователя учетной записи службы SQL Server) — имя учетной записи домена для службы SQL Server. Для этого учебника используйте **sqlservice.**
* **Пароль** — пароль к учетной записи службы SQL Server.  Следует использовать сложный пароль. Подтвердите пароль.
* **SQL Auto Patching maintenance schedule** (Расписание автоматической установки исправлений на серверы SQL Server) — здесь указывается день недели, в который Azure будет автоматически устанавливать исправления на серверы SQL Server. Для этого учебника, введите **воскресенье**.
...
### <a name="buy"></a>Купить
Это последнее лезвие содержит **условия использования,** и **политика конфиденциальности.** Просмотрите эту информацию. Когда вы подготовите все необходимое для создания виртуальных машин в Azure, в том числе ресурсы для группы доступности, щелкните **Создать**.
...
4. На лезвии для **sqlserver-0**, нажмите **Connect**. В браузере отобразится сообщение с предложением открыть или сохранить объект удаленного подключения. Нажмите кнопку **Открыть**.
5. В окне **Подключение к удаленному рабочему столу** может появиться предупреждение о том, что издателя этого удаленного подключения невозможно определить. Нажмите кнопку **Подключить**.
6. Служба безопасности Windows предложит вам ввести учетные данные для подключения к IP-адресу основного контроллера домена. Нажмите **Используйте другую учетную запись**. В поле **Имя пользователя** введите **contoso\DomainAdmin**. Эта учетная запись была настроена при указании имени администратора в шаблоне. Используйте сложный пароль, введенный во время настройки шаблона.
7. В окне **удаленного рабочего стола** может появиться предупреждение о том, что проверка подлинности удаленного компьютера не может быть выполнена из-за проблем с сертификатом безопасности. В нем будет отображено имя сертификата безопасности. Если вы следовали учебник, имя **sqlserver-0.contoso.com**. Нажмите кнопку **Да**.
