...
Для рабочих нагрузок изучите рекомендации по размерам машин SQL Server и их настройке в статье [Рекомендации по оптимизации производительности SQL Server в виртуальных машинах Azure](virtual-machines-windows-sql-performance.md).
...
* В разделе **Сеть** выберите входящие порты из списка **Select public inbound ports** (Выберите общедоступные входящие порты). Например, если вы хотите использовать подключение к удаленному рабочему столу виртуальной машины, выберите порт **RDP (3389)**.
