...
Существует несколько вкладок для настройки SQL Server виртуальной машины. В рамках данного учебника мы рассмотрим следующее: 

| Шаг | Описание |
| --- | --- |
| **Основы** |[Настройка основных параметров](#1-configure-basic-settings) |
| **Дополнительные возможности** |[Настройка дополнительных возможностей](#2-configure-optional-features) |
| **Параметры SQL Server** |[Настройка параметров SQL Server](#3-configure-sql-server-settings) |
| **Проверка и создание** | [Просмотр сводки.](#4-review--create) |
...
* В разделе **Сведения об экземпляре**:
    ...
    1. В этом разделе **параметрам доступности** присвоено значение _не требуется избыточность инфраструктуры_. Дополнительные сведения о вариантах доступности см. в разделе [доступность](../../windows/availability.md). 
...
* Azure включает **мониторинг загрузки** по умолчанию с той же учетной записью хранения, которая НАЗНАЧЕНА виртуальной машине. Эти параметры можно изменить здесь, а также включить диагностику **гостевых ОС**. 
...
На вкладке **параметры SQL Server** в разделе **Конфигурация хранилища**выберите **изменить конфигурацию** , чтобы указать требования к хранилищу.
