...
| **Дополнительные функции** |[Настройка дополнительных возможностей](#2-configure-optional-features) |
| **Параметры SQL Server** |[Настройка параметров SQL Server](#3-configure-sql-server-settings) |
| **Отзыв и создание** | [Просмотр сводки.](#4-review--create) |

## <a name="1-configure-basic-settings"></a>1. Настройка основных параметров.


На **основы** вкладке, укажите следующие сведения:
...
* В разделе **сведения об экземпляре**:
    ...
    1. Выберите расположение для вашей **регион**. 
    1. В данном руководстве оставьте **параметры доступности** присвоено _избыточность инфраструктуры, не требуется_. Чтобы получить дополнительные сведения о параметрах доступности, см. в разделе [Azure регионы и доступность](../../windows/regions-and-availability.md). 
    1. В **изображение** выберите _бесплатная лицензия SQL Server: SQL Server 2017 Developer on Windows Server 2016_ (Бесплатная лицензия на SQL Server: SQL Server 2017 Developer на базе Windows Server 2016).  
    1. Решили **изменить размер** для **размер** виртуальную машину и выберите **A2 основные** предложения. Не забудьте очистить ресурсы, когда вы закончили с ними, чтобы предотвратить все непредвиденные расходы. Для рабочих нагрузок изучите рекомендации по размерам машин SQL Server и их настройке в статье [Рекомендации по оптимизации производительности SQL Server в виртуальных машинах Azure](virtual-machines-windows-sql-performance.md).
...
* В разделе **правила для входящих портов**, выберите **разрешить выбранные порты** , а затем выберите **RDP (3389)** из раскрывающегося списка. 
...
| [Интеграции хранилища ключей Azure](#azure-key-vault-integration) |
...
| --- | --- | --- |
| **URL-адрес Key Vault** |Расположение хранилища ключей. |https:\//contosokeyvault.vault.azure.net/ |
...
| **Credential name (Имя учетных данных)** |**Учетное имя**. Интеграция AKV создает учетные данные в рамках SQL Server, с помощью которых VM будет получать доступ к хранилищу ключей. Выберите имя для этих учетных данных. |mycred1 |
...
На **просмотрите + создать** , просмотрите сводные данные и выберите **создать** для создания SQL Server, группу ресурсов и ресурсы, указанные для этой виртуальной Машины.
