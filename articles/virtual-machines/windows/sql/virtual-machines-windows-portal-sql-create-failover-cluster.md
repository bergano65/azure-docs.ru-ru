...
1. Создайте виртуальные машины в группе доступности.
   ...
   - **Ваша собственная лицензия (BYOL)**
...
1. [Создайте том](https://technet.microsoft.com/windows-server-docs/storage/storage-spaces/hyper-converged-solution-using-storage-spaces-direct#step-36-create-volumes).
   ...
   Локальные дисковые пространства автоматически создает пул носителей при его включении. Теперь все готово для создания тома. Командлет `New-Volume` PowerShell автоматизирует процесс создания тома. Этот процесс включает форматирование, добавление тома в кластер и создание общий том кластера (CSV). В этом примере создается CSV-файл размером 800 ГБ.
