---
title: Автоматизированное резервное копирование для виртуальных машин Azure 2014
...
| **Учетная запись хранилища** | Учетная запись хранения Azure. | Учетная запись хранения Azure для хранения файлов автоматической архивации в хранилище больших двоичных объектов. Там же создается контейнер для хранения всех файлов резервных копий. Имя файла резервной копии содержит дату, время и имя компьютера. |
...
На **вкладке настроек сервера S'L** прокрутите вниз, чтобы **Автоматизированное резервное копирование** и выберите **Включить.** На представленном ниже снимке экрана портала Azure показаны параметры **автоматического резервного копирования SQL**.
...
Для существующих виртуальных машин S'L Server можно включить и отключить автоматизированные резервные копии, изменить период хранения, указать учетную запись хранилища и включить шифрование с портала Azure. 

Перейдите на [ресурс виртуальных машин s'L](virtual-machines-windows-sql-manage-portal.md#access-the-sql-virtual-machines-resource) для вашей виртуальной машины S'L Server 2014, а затем выберите **резервные копирования.** 
...
После завершения работы выберите кнопку **Apply** в нижней части **страницы резервного копирования,** чтобы сохранить изменения.

Если автоматизированная архивация включается впервые, Azure настроит агент IaaS SQL Server в фоновом режиме. При этом портал Azure может не сообщать о том, что выполняется настройка автоматической архивации. Подождите несколько минут, пока агент будет установлен и настроен. После этого новые параметры отобразятся на портале Azure.
