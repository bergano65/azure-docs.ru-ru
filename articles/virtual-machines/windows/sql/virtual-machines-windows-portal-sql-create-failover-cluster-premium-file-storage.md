...
## <a name="step-2-mount-premium-file-share"></a>Шаг 2. Подключить файловый ресурс Premium
...
1. Выберите букву диска, которую вы хотите использовать, в раскрывающемся списке, а затем скопируйте две команды PowerShell из двух блоков команд PowerShell.  Вставьте их в текстовый редактор, например в Блокнот. 
...
1. Выполните команду `Test-NetConnection`, чтобы проверить возможность подключения к учетной записи хранения. Не выполняйте команду `cmdkey` из первого блока кода. 

   ```console
   example: Test-NetConnection -ComputerName  sqlvmstorageaccount.file.core.windows.net -Port 445
   ```

1. Выполните команду `cmdkey` *во втором* блоке кода, чтобы подключить общую папку как диск и сохранить ее. 
...
1. Откройте **проводник** и перейдите к **этому компьютеру**. Общая папка отображается в папке "Сетевые расположения": 

   :::image type="content" source="media/virtual-machines-windows-portal-sql-create-failover-cluster-premium-file-storage/file-share-as-storage.png" alt-text="Общая папка доступна в виде хранилища в проводнике":::

1. Откройте только что подключенный диск и создайте по крайней мере одну папку для размещения файлов данных SQL. 
1. Повторите эти действия на каждой виртуальной машине SQL Server, которая будет участвовать в кластере. 

  > [!IMPORTANT]
  > Не используйте один и тот же файловый ресурс как для файлов данных, так и для резервных копий. Выполните те же действия, чтобы настроить вторичный файловый ресурс для резервного копирования, если требуется выполнить резервное копирование баз данных в общую папку. 
...
Следующий пример PowerShell создает отказоустойчивый кластер для Windows Server 2019.  Дополнительные сведения см. в блоге [Failover кластера: Сетевой объект кластера @ no__t-0.  Обновите скрипт, введя имена узлов (имена виртуальных машин) и доступный IP-адрес из виртуальной сети Azure:
...
## <a name="step-4-test-cluster-failover"></a>Шаг 4. Тестовая отработка отказа кластера
...
## <a name="step-5-create-sql-server-fci"></a>Шаг 5. Создание экземпляра отказоустойчивого кластера SQL Server
...
## <a name="step-6-create-azure-load-balancer"></a>Шаг 6. Создание Azure Load Balancer
...
1. Настройте для балансировщика нагрузки следующие параметры.
   ...
   - **Регион**. Используйте то же расположение Azure, что используется для виртуальных машин.
   ...
   - **Назначение IP-адресов**. Назначение IP-адреса должно быть статическим. 
