...
[Database Migration Service](/dms/dms-overview) является параметром для клиентов, если миграция из локальной виртуальной машине Azure, выполнив обновление SQL Server до SQL Server 2012 и более.
...
- **Резервные копии SQL Server**: Резервные копии SQL Server можно использовать для восстановления SQL Server в случае региональные или сбои зоны. Так как функция управляемого резервного копирования не поддерживается для EOS SQL Server, клиенты должны будут выполнять резервное копирование вручную.
