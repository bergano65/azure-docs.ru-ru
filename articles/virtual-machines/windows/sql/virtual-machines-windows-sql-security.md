...
Выбранные параметры на портале используют правила безопасности для входящего трафика [группы безопасности сети](../../../virtual-network/security-overview.md) (NSG) виртуальной машины, чтобы разрешить или запретить сетевой трафик к виртуальной машине. Можно изменить существующие или создать новые правила входящего трафика NSG, чтобы разрешить трафик на порт SQL Server (по умолчанию — 1433). Можно также указать конкретные IP-адреса, которым разрешен обмен данными через этот порт.
...
Управляемые диски предлагают шифрование серверной стороны и шифрование azure Disk. [Шифрование серверной стороны](/azure/virtual-machines/windows/disk-encryption) обеспечивает шифрование на отдых и гарантирует ваши данные для выполнения ваших организационных обязательств по безопасности и соблюдению. [Лазурное шифрование дисков](/azure/security/fundamentals/azure-disk-encryption-vms-vmss) использует либо технологию Bitlocker или DM-Crypt, а также интегрируется с Azure Key Vault для шифрования как ОС, так и дисков данных. 
...
- Для виртуальных ресурсов менеджера vMs можно выбрать **систему безопасности** из [ресурса виртуальных машин S'L.](virtual-machines-windows-sql-manage-portal.md#access-the-sql-virtual-machines-resource) Это даст возможность изменить номер порта.
...
В дополнение к практике, описанной в этой теме, мы рекомендуем вам пересмотреть и внедрить рекомендации по безопасности как из традиционных практик безопасности на месте, так и из виртуальных методов безопасности. 

Для получения более подробной информации о методах обеспечения безопасности на месте можно ознакомиться на данных [по вопросам безопасности для установки сервера S'L](/sql/sql-server/install/security-considerations-for-a-sql-server-installation) и [центра безопасности.](/sql/relational-databases/security/security-center-for-sql-server-database-engine-and-azure-sql-database) 

Для получения дополнительной информации о безопасности виртуальных машин, [см.](/azure/security/fundamentals/virtual-machines-overview)
