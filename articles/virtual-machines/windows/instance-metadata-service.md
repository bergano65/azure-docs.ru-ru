...
description: Использование интерфейса rESTful для получения сведений о виртуальной Машине Windows вычислений, сети и ближайших мероприятиях обслуживания.
...
Сюда входят сведения о номере SKU, конфигурации сети и ближайших мероприятиях обслуживания. Дополнительные сведения о какая информация доступна, см. в разделе [API метаданных](#metadata-apis).
...
> [!IMPORTANT]
> Эта служба является **общедоступной** во всех регионах Azure.  Она регулярно получает обновления, чтобы предоставлять новые сведения об экземплярах виртуальной машины. Этой странице представлены актуальные сведения [API метаданных](#metadata-apis) доступны.
...
[Azure для Китая](https://azure.microsoft.com/global-infrastructure/china)                                                     | Общедоступная версия | 2017-04-02, 2017-08-01, 2017-12-01, 2018-02-01, 2018-04-02, 2018-10-01
...
[Общедоступный регион "Центрально-западная часть США"](https://azure.microsoft.com/regions/)                           | Общедоступная версия | 2017-04-02, 2017-08-01, 2017-12-01, 2018-02-01, 2018-04-02, 2018-10-01, 2019-02-01

Эта таблица обновляется при наличии обновлений службы, или когда доступны новые поддерживаемые версии.

> [!NOTE]
> 2019-02-01 в настоящее время начало разворачивается и будут доступны в других регионах чуть ниже.
...
## <a name="metadata-apis"></a>API-интерфейсы метаданных

#### <a name="the-following-apis-are-available-through-the-metadata-endpoint"></a>Следующие интерфейсы API доступны через конечную точку метаданных:

Data | Описание | Представленные версии
-----|-------------|-----------------------
...
instance | См. в разделе [экземпляр API](#instance-api) | 2017-04-02
...
##### <a name="the-following-compute-categories-are-available-through-the-instance-api"></a>Через API экземпляра доступны следующие категории вычислений:

> [!NOTE]
> Через конечную точку метаданных следующие категории осуществляется через экземпляр или вычислительной мощности

Data | Описание | Представленные версии
-----|-------------|-----------------------
azEnvironment | Когда виртуальная машина работает в среде Azure | 2018-10-01
customData | См. в разделе [пользовательские данные](#custom-data) | 2019-02-01
location | Регион Azure, в котором запущена виртуальная машина | 2017-04-02
name | Имя виртуальной машины. | 2017-04-02
offer | Предоставляют сведения об образе виртуальной Машины и развертывается присутствует только для образов из коллекции образов Azure | 2017-04-02
...
План | [Планирование](https://docs.microsoft.com/rest/api/compute/virtualmachines/createorupdate#plan) содержащий имя продукта и издателя для виртуальной Машины, если его образ Azure Marketplace | 2018-04-02
...
##### <a name="the-following-network-categories-are-available-through-the-instance-api"></a>Следующие категории сети доступны через API экземпляра.

> [!NOTE]
> Через конечную точку метаданных следующие категории осуществляется через экземпляр или сети/интерфейс
...
Ниже перечислены регионы и значения в среде Azure.
...
Теги могут быть применены к виртуальной Машине Azure, чтобы логически упорядочивать их в таксономию. Теги, назначаемый виртуальной Машине можно получить с помощью запроса ниже.
...
Служба метаданных экземпляров обеспечивают для виртуальной Машины, чтобы иметь доступ к его пользовательские данные. Двоичные данные должны быть меньше 64 КБ и предоставляется на виртуальную машину в кодировке base64.

Azure пользовательские данные могут быть вставлены на виртуальную машину через интерфейсы REST API, командлеты PowerShell, интерфейса командной строки Azure (CLI) или шаблон ARM.

Пример интерфейса командной строки Azure, см. в разделе [пользовательских данных и Cloud-Init в Microsoft Azure](https://azure.microsoft.com/blog/custom-data-and-cloud-init-on-windows-azure/).

Пример шаблона ARM, см. в разделе [развертывание виртуальной машины с помощью CustomData](https://github.com/Azure/azure-quickstart-templates/tree/master/101-vm-customdata).
...
В настоящее время пользовательские данные гарантированно были доступны во время начальной загрузки виртуальной машины. Если обновления выполняются на виртуальную машину, такие как добавление дисков или изменении размера виртуальной Машины, служба метаданных экземпляров не будет предоставлять пользовательские данные. Предоставление пользовательских данных постоянно через службу метаданных экземпляров в данный момент уже выполняется.
