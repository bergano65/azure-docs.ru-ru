...
description: Интерфейс RESTFUL для получения сведений о событиях по вычислению, сети и предстоящему обслуживанию виртуальных машин Windows.
...
В рамках сценария, обслуживаемого службой метаданных экземпляров, предоставляется гарантия того, что предоставленные данные поступают из Azure. Мы подписываем часть этой информации, чтобы гарантировать, что в Azure{1}{2}работает надлежащий образ Marketplace.
...
Существует известная ошибка, связанная с сертификатом, используемым для подписания. Сертификаты могут не иметь точного соответствия `metadata.azure.com` для общедоступного облака. Поэтому проверка сертификата должна допускать общее имя из любого поддомена `.metadata.azure.com`.
