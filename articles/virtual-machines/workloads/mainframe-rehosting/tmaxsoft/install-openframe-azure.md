...
- Запросите документацию по Опенфраме, <support@tmaxsoft.com>отправив сообщение электронной почты по адресу.
...
Открытый ключ может быть свободно предоставлен, но закрытый ключ должен храниться полностью в секрете и никогда не должен использоваться совместно с другой стороной. После создания ключей необходимо вставить **открытый ключ SSH** в конфигурацию — по сути, загружая его на виртуальную машину Linux. Он хранится внутри разрешенных\_ключей в каталоге \~/.SSH. домашнего каталога учетной записи пользователя. Затем виртуальная машина Linux сможет распознать и проверить подключение после предоставления соответствующего **закрытого ключа SSH** в SSH-клиенте (в нашем примере это выявление).
...
- Администратор вставит содержимое открытого ключа в файл ключей \~/.СШ/аусоризед.\_
...
3. Откройте профиль Bash\_в VI (`vi .bash_profile`) и вставьте в него следующий текст:
...
9. Теперь загрузите Тиберо с `tbboot`помощью. Пример:
...
3. Перейдите в каталог unixODBC-2.3.4 и создайте файл makefile с помощью проверки сведений о компьютере. Пример:
     ...
     По умолчанию unixODBC устанавливается в/usr/local, поэтому `--prefix` для изменения расположения передается значение. Аналогичным образом файлы конфигурации устанавливаются в/etc по умолчанию `--sysconfdir` , поэтому передает значение нужного расположения.
...
1. Убедитесь, что установка Тиберо прошла удачно, а затем убедитесь\_,\_что\_указан\_следующий\_файл установщика опенфраме Base7 0 Linux x86 64. bin и базовый файл конфигурации. Properties.
...
1. Убедитесь, что базовая установка прошла удачно, а затем\_убедитесь\_,\_что\_файл\_конфигурации\_опенфраме\_Batch7 0 Fix2 MVS Linux x86 64. bin и пакет. Properties установлены:
...
8. Для запуска `tmdown` и завершения работы пакетной службы используйте команду:
...
1. Убедитесь, что файл\_конфигурации\_опенфраме\_Tacf7\_0\_Fix2\_Linux x86 64. bin установлен в настоящее время.
...
6. В командной строке введите `tmboot` команду, чтобы перезапустить опенфраме. Выходные данные выглядят следующим образом:
...
1. Убедитесь, что Пакетная установка выполнена успешно, а затем убедитесь, что файл установщика **Sorting-bin\_-Sort 2sp3-Linux64-2123-opt. tar. gz** имеется.
...
1. Убедитесь, что установка пакета/в сети прошла удачно, а затем\_убедитесь\_в\_наличии\_файла\_установщика\_опенфраме COBOL3 0 40 Linux x86 64. bin.
...
11. Перезагрузите Опенфраме с `tmdown/tmboot` помощью команды.
...
1. Убедитесь, что установка пакета/в сети прошла удачно, а затем\_убедитесь\_в\_наличии\_файла\_установщика опенфраме ASM3 0 для Linux x86 64. bin.
...
1. Убедитесь, что базовая установка прошла удачно, а затем\_убедитесь\_,\_что\_файл\_конфигурации\_опенфраме OSC7 0 Fix2 Linux x86 64. bin установлен в настоящее время.
...
5. Проверьте файл Опенфраме\_OSC7\_0\_Fix2\_InstallLog. log. Он должен выглядеть примерно так:
...
10. Выполните `oscboot` команду, чтобы загрузить OSC. Выходные данные выглядят следующим образом:
...
11. Чтобы убедиться, что состояние процесса — "Готово" `tmadmin` , используйте команду в Si. Все процессы должны отображать РДИ в столбце **Status (состояние** ).

    ![Процессы, отображающие РДИ](media/tmadmin-02.png)

12. Завершите работу OSC с `oscdown` помощью команды.
...
1. Скачайте двоичный файл Ant `wget` с помощью команды. Пример:
...
9.  Обновите файл профиля\_bash с помощью переменных жеус, как показано ниже.
...
1. Убедитесь, что жеус успешно установлен, а затем убедитесь в наличии файла\_установщика\_OFGW7\_0 1 Generic. bin.
2. Запустите установщик. Пример:
...
1. Убедитесь в наличии файла\_установщика Generic. bin OFManager7.
2. Запустите установщик. Пример:
