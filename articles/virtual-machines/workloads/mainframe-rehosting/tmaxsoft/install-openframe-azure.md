...
- Запросите документацию Опенфраме, отправив сообщение электронной почты по адресу <support@tmaxsoft.com>.
...
1. Перейдите в портал Azure по адресу <https://portal.azure.com> и войдите в свою учетную запись.
...
Открытый ключ может быть свободно предоставлен, но закрытый ключ должен храниться полностью в секрете и никогда не должен использоваться совместно с другой стороной. После создания ключей необходимо вставить **открытый ключ SSH** в конфигурацию — по сути, загружая его на виртуальную машину Linux. Он хранится внутри полномочной @ no__t-0keys в каталоге \~/. SSH домашнего каталога учетной записи пользователя. Затем виртуальная машина Linux сможет распознать и проверить подключение после предоставления соответствующего **закрытого ключа SSH** в SSH-клиенте (в нашем примере это выявление).
...
- Администратор вставит содержимое открытого ключа в файл \~/. SSH/авторизовать @ no__t-1keys.
...
2.  Чтобы открыть PuTTYgen, перейдите в каталог установки в C: \\Program Files @ no__t-1PuTTY.
...
4.  После создания сохраните как открытый ключ, так и закрытый ключ. Вставьте содержимое открытого ключа в раздел **открытый ключ SSH** в области **основы создания виртуальной машины \>** (см. шаги 6 и 7 в предыдущем разделе).
...
7. В поле **имя узла**введите имя пользователя и общедоступный IP-адрес, который вы скопировали. Например, **username @ no__t-1publicip**.
...
3. Откройте. Bash @ no__t-0profile в VI (`vi .bash_profile`) и вставьте в него следующий текст:
...
6. Измените \$TB @ no__t-1HOME/Client/config/тбдсн. TBR и введите 127.0.0.1 вместо офлокалхост, как показано ниже:
...
1. Убедитесь, что установка Тиберо прошла удачно, а затем убедитесь в наличии следующего файла конфигурации Опенфраме @ no__t-0Base7 @ no__t-10 @ no__t-2Linux @ no__t-3x86\_64.bin и базового. Properties.
...
1. Убедитесь, что базовая установка прошла удачно, а затем убедитесь, что файл конфигурации Опенфраме @ no__t-0Batch7 @ no__t-10 @ no__t-2Fix2 @ no__t-3MVS @ no__t-4Linux @ no__t-5x86\_64.bin в пакете. Properties имеется:
...
1. Убедитесь в наличии файла конфигурации Опенфраме @ no__t-0Tacf7 @ no__t-10 @ no__t-2Fix2 @ no__t-3Linux @ no__t-4x86\_64.bin.
...
1. Убедитесь, что Пакетная установка выполнена успешно, а затем убедитесь, что файл установщика **prosort-bin-prosort\_2sp3-linux64-2123-opt.tar.gz** имеется.
...
1. Убедитесь, что установка пакета/в сети прошла удачно, а затем убедитесь в наличии файла установщика Опенфраме @ no__t-0COBOL3 @ no__t-10 @ no__t-240 @ no__t-3Linux @ no__t-4x86\_64.bin.
...
8. Перейдите к файлу конфигурации Опенфраме тжклрун. conf и откройте его в VI. Пример.
   ...
   Ниже приведен раздел СИСЛИБ после изменения:
     ...
     :${OFCOB_HOME}/lib
     ```
9. Проверьте файл Опенфраме @ no__t-0COBOL\_InstallLog.log в VI и убедитесь в отсутствии ошибок. Пример.
     ```
...
1. Убедитесь, что установка пакета/в сети прошла удачно, а затем убедитесь в наличии файла установщика Опенфраме @ no__t-0ASM3 @ no__t-10 @ no__t-2Linux @ no__t-3x86\_64.bin.
...
7. Откройте файл Опенфраме @ no__t-0ASM\_InstallLog.log в VI и убедитесь в отсутствии ошибок. Пример.
...
1. Убедитесь, что базовая установка прошла удачно, а затем убедитесь в наличии файла конфигурации Опенфраме @ no__t-0OSC7 @ no__t-10 @ no__t-2Fix2 @ no__t-3Linux @ no__t-4x86\_64.bin.
...
5. Проверьте файл Опенфраме @ no__t-0OSC7 @ no__t-10 @ no__t-2Fix2\_InstallLog.log. Должно отобразиться примерно следующее:
...
7. В разделах \#BASE и \#BATCH измените параметры, как показано ниже.
...
9. Чтобы запустить и завершить работу OSC, инициализируйте общую память CICS Region, введя в командной строке `osctdlinit OSCOIVP1`.

10. Выполните `oscboot`, чтобы загрузить OSC. Выходные данные выглядят примерно так:
...
11. Чтобы убедиться, что состояние процесса — "Готово", используйте команду `tmadmin` в Si. Все процессы должны отображать РДИ в столбце **Status (состояние** ).
...
2. Используйте служебную программу `tar`, чтобы извлечь двоичный файл и переместить его в соответствующее расположение. Пример.
...
4. Откройте профиль Bash в VI (`vi .bash_profile`) и обновите его с помощью следующих переменных:
...
9.  Обновите файл. Bash @ no__t-0profile с помощью переменных ЖЕУС, как показано ниже:
...
13. Проверьте веб-вход с помощью синтаксиса:
     ...
     Например, отображается <http://192.168.92.133:9736/webadmin/login.> экран входа в систему:
...
1. Убедитесь, что ЖЕУС успешно установлен, а затем убедитесь в наличии файла установщика OFGW7 @ no__t-00 @ no__t-11\_Generic.bin.
2. Запустите установщик. Пример.
...
1. Убедитесь, что файл установщика OFManager7\_Generic.bin имеется.
2. Запустите установщик. Пример.
