...
- Виртуальная машина с [Enterprise Developer](set-up-micro-focus-azure.md). Имейте в виду, что корпоративный разработчик имеет полный экземпляр Enterprise Server на нем для целей разработки и тестирования. Этот экземпляр является экземпляром Enterprise Server, используемый для получения демоверсии.
...
- [Visual Studio 2019](https://azure.microsoft.com/downloads/) с последним пакетом или [Visual Studio Community](https://visualstudio.microsoft.com/vs/community/), который можно загрузить бесплатно.
...
4. Выберите **World Wide Web Services**и проверьте следующие параметры:
...
6. Для **функции**, проверьте **Microsoft .NET framework 3.5.1**и проверьте следующие параметры:

    - Windows Communication Foundation HTTP активация
    - Не HTTP активация Windows Communication Foundation

7. Для **функции**, проверьте **Microsoft .NET framework 4.6**и проверьте следующие параметры:
...
Некоторые процессы Enterprise Server необходимо иметь возможность входить в SQL Server и создание баз данных и других объектов. Эти процессы использовать учетную запись локальной системы, поэтому необходимо предоставить полномочия системного администратора для этой учетной записи.
...
Создайте прослушиватель для сеансов TN3270, доступ к приложению BankDemo.
...
7. Для **имя**, введите **TN3270**. Для **порт**, введите **9024**. ESDEMO приложение использует порт 9230, поэтому необходимо использовать другой порт.
...
Последнее, что вам нужно будет настроить сеанс 3270, с помощью Rumba, эмулятор 3270. Этот шаг позволяет доступ к приложению BankDemo через прослушиватель, который вы создали.

1. Из Windows **запустить** меню, запустите Rumba рабочий стол.

2. В разделе **подключений** пункта меню, выберите **TN3270**.

3. Нажмите кнопку **вставить** и тип **127.0.0.1** для IP-адреса и **9024** для порта, определяемые пользователем.
