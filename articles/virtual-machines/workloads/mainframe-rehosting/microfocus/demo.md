...
6. Чтобы выбрать файл определения региона для базы данных в разных регионах, перейдите в **область\_банкдемо\_DB. config** на **языке C\\:\\пользователи\\\\общедоступные\\документы Micro\\Focus\\Enterprise\\Samples мэйнфрейм CICS\\DotNet\\банкдемо**.
...
2. Щелкните значок **Открыть файл** и выберите файл **силистенер. exe. config** . Этот файл будет изменен и загружается при каждом запуске Enterprise Server.
...
![Экран приветствия](media/14-demo.png)
![на дисплее в виде мэйнфреймов дисплей-Румба-подсистема](media/15-demo.png)
