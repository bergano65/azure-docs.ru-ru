...
Переключитесь на *привилегированного* пользователя:

   ```bash
   sudo su -
   ```
    
Добавьте пользователя Oracle в файл */etc/sudoers* :
...
1. В этом шаге предполагается, что у вас есть экземпляр Oracle (*Test*), который выполняется на виртуальной машине с именем *vmoracle19c*.
...
8.  Убедитесь, что база данных находится в режиме журнала архивации, чтобы включить оперативное резервное копирование.
    ...
    В режиме НОАРЧИВЕЛОГ выполните следующие команды:
...
Теперь Azure Backup предоставляет расширенную платформу предварительных сценариев и пост-Script Framework, где служба Azure Backup предоставляет Упакованные предварительные скрипты и посторонние скрипты для выбранных приложений. Azure Backup пользователям просто нужно присвоить имя приложению, а затем служба архивации виртуальных машин Azure автоматически вызовет соответствующие скрипты, предшествующие предварительной публикации. Упакованные предварительные и посторонние скрипты будут поддерживаться группой Azure Backup, поэтому пользователи могут быть уверены в поддержке, владении и допустимости этих сценариев. В настоящее время поддерживаемыми приложениями для расширенной платформы являются ***Oracle и MySQL** _, и в будущем ожидается больше типов приложений.

В этом разделе вы будете использовать Azure Backup расширенную платформу для создания моментальных снимков, соответствующих приложению, для работающей виртуальной машины и базы данных Oracle. База данных будет переведена в режим резервного копирования, что позволит выполнить транзакционно согласованное оперативное резервное копирование, в то время как Azure Backup создает моментальный снимок дисков виртуальной машины. Моментальный снимок будет полной копией хранилища, а не добавочной или копией моментального снимка записи, поэтому он является эффективным носителем для восстановления базы данных. Преимущество использования Azure Backupных моментальных снимков с согласованием приложений заключается в том, что они чрезвычайно быстро выполняются независимо от размера базы данных, а моментальный снимок можно использовать для операций восстановления сразу после выполнения, не дожидаясь его передачи в хранилище служб восстановления.

Чтобы использовать Azure Backup для резервного копирования базы данных, выполните следующие действия.

1. Подготовьте среду для создания резервной копии с постоянным приложением.
1. Настройте резервные копии, совместимые с приложениями.
1. Активация резервной копии виртуальной машины в соответствии с приложениями

### <a name="prepare-the-environment-for-application-consistent-backup"></a>Подготовка среды для резервного копирования с постоянным приложением

1. Перейдите к пользователю _ *root**:
...
2. Настройка пользовательской среды резервного копирования:
...
3. Настройте внешнюю проверку подлинности для нового пользователя резервной копии. 
   Пользователь резервного копирования должен иметь доступ к базе данных с использованием внешней проверки подлинности, так что не требует пароля.

   Сначала переключитесь на пользователя **Oracle** :
   ...
   Выходные данные должны отобразиться 
   ...
   >[!IMPORTANT] 
   >При получении сообщения об ошибке "ORA-46953: файл паролей имеет формат, отличный от 12,2".  При выполнении приведенной выше инструкции GRANT выполните следующие действия, чтобы перенести файл орапвд в формат 12,2:
   >
   >Выйдите из sqlplus, переместите файл пароля со старым форматом в новое имя, перенесите файл с паролем и удалите старый файл. После выполнения приведенных ниже команд Перезапустите операцию Grant выше в sqlplus.
...
### <a name="set-up-application-consistent-backups"></a>Настройка резервных копий, совместимых с приложениями  

1. Переключение на привилегированного пользователя 
   ```bash
   sudo su -
   ```

2. Создание резервной копии рабочего каталога с постоянным приложением
   ```bash
   if [ ! -d "/etc/azure" ]; then
...
3. Создайте файл в каталоге/etc/Azure с именем **рабочей нагрузки. conf** со следующим содержимым, который должен начинаться с `[workload]` . Следующая команда создаст файл и заполнит его содержимое:
   ...
   ```
1. Скачайте скрипты Преораклемастер. SQL и Постораклемастер. SQL из [репозитория GitHub](https://github.com/Azure/azure-linux-extensions/tree/master/VMBackup/main/workloadPatch/DefaultScripts) и скопируйте их в каталог/etc/Azure

4. Изменение разрешений для файла
   ```bash
   chmod 744 workload.conf preOracleMaster.sql postOracleMaster.sql 
   ```

### <a name="trigger-application-consistent-backup-of-the-vm"></a>Активация резервной копии виртуальной машины в соответствии с приложениями
...
1. Создайте хранилище служб восстановления.
   ...
   ```
2. Включение защиты резервного копирования для виртуальной машины
   ...
   ```
3. Запустите резервное копирование, чтобы запустить его сейчас, а не дожидаться активации резервной копии по расписанию по умолчанию (05:00 UTC). 
...
1. В портал Azure найдите элемент хранилища служб восстановления *myVault* и щелкните его.
...
2. В колонке **Обзор** выберите пункт **архивные элементы** и *_Виртуальная машина Azure_*"Select *", для которой в списке должно быть провер количество элементов резервного копирования.
...
3. На странице Архивация элементов (виртуальные машины Azure) в списке присутствует виртуальная машина _ *vmoracle19c**. Нажмите кнопку с многоточием справа, чтобы открыть меню и выбрать пункт **Восстановление файлов**.
...
1. Создайте точку подключения восстановления и скопируйте в нее скрипт.
    ...
    В следующем примере создайте каталог **_/RESTORE_* _ для подключения к моментальному снимку, переместите файл в каталог и измените файл, чтобы он принадлежал привилегированному пользователю и был создан исполняемый файл.
   ...
   ```bash
    ...
    В следующем примере показано, что вы должны увидеть после выполнения предыдущего скрипта. Когда появится запрос на продолжение, введите _ * Y * *.
...
1. Создайте учетную запись хранения для промежуточной подготовки:
   
   Настройка хранилища файлов в портал Azure

   В портал Azure выберите **_+ создать ресурс_* _, а затем найдите и выберите _*_учетная запись хранения_*_ .
    
   ![Страница добавления учетной записи хранения](./media/oracle-backup-recovery/storage-1.png)
    
    
   На странице Создание учетной записи хранения выберите существующую группу ресурсов _*_RG-Oracle_*_, присвойте имя учетной записи хранения _*_Оракресторе_*_ и выберите _*_Storage v2 (Женералпурпосе v2)_*_ для типа учетной записи. Измените репликацию на _*_локально избыточное хранилище (LRS)_*_ и установите для параметра производительность значение _*_Standard_*_. Убедитесь, что в качестве расположения выбран тот же регион, что и для всех остальных ресурсов в группе ресурсов. 
    
   ![Страница добавления учетной записи хранения](./media/oracle-backup-recovery/recovery-storage-1.png)
   
   Щелкните Проверка + создать, а затем — создать.

2. В портал Azure найдите элемент хранилища служб восстановления _myVault * и щелкните его.

    ![Элементы архивации хранилища myVault в хранилищах служб восстановления](./media/oracle-backup-recovery/recovery-service-06.png)
    
3.  В колонке **Обзор** выберите пункт **архивные элементы** и *_Виртуальная машина Azure_*"Select *", для которой в списке должно быть провер количество элементов резервного копирования.

    ![Число элементов архивации виртуальной машины Azure в хранилищах служб восстановления](./media/oracle-backup-recovery/recovery-service-07.png)

4.  На странице элементы архивации (виртуальные машины Azure) отобразится страница VM _ *vmoracle19c**. Щелкните имя виртуальной машины.
