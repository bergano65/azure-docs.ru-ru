...
- У вас есть [пакет диагностики](https://docs.oracle.com/cd/E11857_01/license.111/e11987/database_management.htm) для базы данных Oracle, вы хотите перенести
...
Ниже приведен пример того, как создать отчет AWR (Создание AWR отчетов с помощью диспетчера Enterprise Oracle, если текущая установка имеет одно):
...
- Использование виртуальных машин с [ускоренной сети](https://docs.microsoft.com/azure/virtual-network/create-vm-accelerated-networking-cli) для повышения производительности сети.
- Для некоторых Linux distrubutions, рассмотрите возможность включения [поддержка операций TRIM и UNMAP](https://docs.microsoft.com/azure/virtual-machines/linux/configure-lvm#trimunmap-support).
- Установка [Oracle Enterprise Manager](https://www.oracle.com/technetwork/oem/enterprise-manager/overview/index.html) на отдельной виртуальной машине.
- Огромные страницы не включены в linux по умолчанию. Рассмотрите возможность включения огромные страниц и задайте `use_large_pages = ONLY ` на базе данных Oracle. Это может помочь повысить производительность. Дополнительные сведения см. [здесь](https://docs.oracle.com/en/database/oracle/oracle-database/12.2/refrn/USE_LARGE_PAGES.html#GUID-1B0F4D27-8222-439E-A01D-E50758C88390).
...
- При использовании виртуальных машин серии M в хранилище класса Premium включите [ускоритель записи](https://docs.microsoft.com/azure/virtual-machines/linux/how-to-enable-write-accelerator) на повтор журналы диска.
...
Чтобы обеспечить максимальную пропускную способность, мы рекомендуем начать с **None** для кэширования узла. Помните, что для хранилища класса Premium необходимо отключить препятствующие факторы при подключении файловой системы с параметром **Только для чтения** или **Нет**. Внесите значения UUID дисков в файл /etc/fstab.
...
- *Политика NSG*. NSG можно определить с помощью подсети или сетевого адаптера. Проще организовать управление доступом на уровне подсети, как для обеспечения безопасности и принудительную маршрутизацию для таких вещей, как брандмауэры приложений.
