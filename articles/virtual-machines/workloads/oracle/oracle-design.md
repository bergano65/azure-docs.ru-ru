...
- У вас есть [пакет диагностики](https://docs.oracle.com/cd/E11857_01/license.111/e11987/database_management.htm) для базы данных Oracle, который вы хотите перенести
...
> | **Группа безопасности** |Средства ограничения портов и IP-адресов |[Группа сетевой безопасности (NSG)](https://azure.microsoft.com/blog/network-security-groups) |
...
Если у вас уже есть база данных Oracle и вы планируете перенести ее в Azure, это можно сделать несколькими способами. Если у вас есть [пакет диагностики](https://www.oracle.com/technetwork/oem/pdf/511880.pdf) для экземпляров Oracle, вы можете запустить отчет Oracle AWR, чтобы получить метрики (IOPS, Mbps, GiBs и так далее). Затем выберите виртуальную машину на основе собранных метрик. Также можно обратиться в группу обслуживания инфраструктуры, чтобы получить аналогичные сведения.
...
Ниже приводится пример того, как создать отчет AWR (Создание отчетов AWR с помощью вашего менеджера Oracle Enterprise Manager, если текущая установка имеет один):
...
#### <a name="1-estimate-vm-size-based-on-cpu-memory-and-io-usage-from-the-awr-report"></a>1. Оценить размер VM на основе процессора, памяти и использования ввоза/ввоза из отчета AWR
...
#### <a name="2-choose-a-vm"></a>2. Выберите VM
...
#### <a name="3-fine-tune-the-vm-sizing-with-a-similar-vm-series-based-on-the-acu"></a>3. Тонкая настройка размеров VM с аналогичной серией VM на основе ACU
...
- Для некоторых дисструций Linux рассмотрите возможность поддержки [TRIM/UNMAP.](https://docs.microsoft.com/azure/virtual-machines/linux/configure-lvm#trimunmap-support)
- Установите [менеджер предприятия Oracle](https://www.oracle.com/technetwork/oem/enterprise-manager/overview/index.html) на отдельной виртуальной машине.
- Огромные страницы не включены на linux по умолчанию. Рассмотрите возможность создания `use_large_pages = ONLY` огромных страниц и установки на Oracle DB. Это может помочь повысить производительность. Дополнительные сведения см. [здесь](https://docs.oracle.com/en/database/oracle/oracle-database/12.2/refrn/USE_LARGE_PAGES.html#GUID-1B0F4D27-8222-439E-A01D-E50758C88390).
...
Чтобы максимизировать пропускную выливку, мы рекомендуем начать с **None** для кэширования хоста. Помните, что для хранилища класса Premium необходимо отключить препятствующие факторы при подключении файловой системы с параметром **Только для чтения** или **Нет**. Внесите значения UUID дисков в файл /etc/fstab.
...
- *Политика NSG:* NSG можно определить для подсети или сетевого адаптера. Проще управлять доступом на уровне подсети, как для безопасности, так и для перемывания сил для таких брандмауэров, как брандмауэры приложений.
