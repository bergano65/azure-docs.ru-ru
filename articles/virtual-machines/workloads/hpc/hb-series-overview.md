...
Физически серия ХБ — это 2 * 32-ядерные ЦП ЕПИК 7551 для всего 64 физических ядер. Эти ядра 64 делятся на 16 доменов Пнума (по 8 на сокет), каждый из которых состоит из четырех ядер и называется "сложным ЦП" (или "КККС"). Каждый КККС имеет собственный кэш L3, что позволяет операционной системе видеть границу Пнума/Внума. Пара смежных Ккксс предоставляет доступ к двум каналам физической памяти DRAM (32 ГБ памяти DRAM на серверах серии ХБ).
...
См. Дополнительные сведения об [архитектуре AMD ЕПИК](https://bit.ly/2Epv3kC) и [архитектурах с несколькими микросхемами](https://bit.ly/2GpQIMb) в LinkedIn. Дополнительные сведения см. в описании [рекомендаций по настройке HPC для процессоров AMD ЕПИК](https://bit.ly/2T3AWZ9).
...
| Характеристики оборудования                | Виртуальная машина серии ХБ                     |
|----------------------------------|----------------------------------|
...
| Сеть                          | 50 ГБ Ethernet (40 Гб использования) Azure Second Смартник * * * |

## <a name="software-specifications"></a>Спецификации программного обеспечения

| Спецификации SW           |Виртуальная машина серии ХБ           |
|-----------------------------|-----------------------|
| Максимальный размер задания MPI            | 6000 ядер (масштабируемые наборы виртуальных машин 100) 12000 ядер (масштабируемые наборы виртуальных машин 200)  |
| Поддержка MPI                 | MVAPICH2, Опенмпи, МПИЧ, Platform MPI, Intel MPI  |
| Дополнительные платформы       | Унифицированная связь X, либфабрик, ПГАС |
| Поддержка хранилища Azure       | STD + Premium (Макс. 4 диска) |
