---
title: Производительность размер виртуальных Машин серии HB - виртуальных машинах Azure | Документация Майкрософт
description: Дополнительные сведения о производительности результаты тестирования для размеров серии HB виртуальных Машин в Azure.
...
Нескольких тестах производительности были запущены на размеры серии HB. Ниже приведены некоторые результаты тестирования производительности.


| Рабочая нагрузка                                        | HB                    |
|-------------------------------------------------|-----------------------|
| ПОТОК триады                                    | ~ 260 ГБ/с (32 33 ГБ/с в каждом CCX)  |
| Высокой производительности Linpack (HPL)                  | ~ 1000 гигафлопс (Rpeak), ~ 860 гигафлопс (Rmax) |
| RDMA задержка и пропускная способность                        | 2.35usec, 96,5 Гбит/с   |
| FIO на локальный NVMe SSD                           | ~1.7 считывает ГБИТ/с, ~1.0, записывает ГБИТ/с      |  
| IOR на 4 * Azure уровня "премиум" SSD (P30 управляемые диски, RAID0) **  | ~ 725 МБ/s считывает ~ 780 МБ/записывает   |
