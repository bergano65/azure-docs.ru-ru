---
title: Azure VMs высокой доступности для SAP NW на RHEL Документы Майкрософт
...
В Azure Marketplace доступен образ для Red Hat Enterprise Linux, который можно использовать для развертывания новых виртуальных машин. Все необходимые ресурсы можно развернуть с помощью шаблонов быстрого запуска с сайта GitHub. Шаблон развертывает виртуальные машины, балансизатор нагрузки, набор доступности и т.д. Выполните следующие действия, чтобы развернуть шаблон:
...
1. Задайте следующие параметры.
   ...
   1. Имя пользователя и пароль администратора или ключ SSH  
      Создается новый пользователь, который может быть использован для вхули в машину.
...
Сначала необходимо создать виртуальные машины для этого кластера. После этого вы создаете балансиватор нагрузки и используете виртуальные машины в пуле бэкэнда.
