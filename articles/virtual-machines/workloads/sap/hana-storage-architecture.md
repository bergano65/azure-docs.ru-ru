...
- **S144m и S192m**: С шагом 256 ГБ, с 512 ГБ самый маленький блок. Можно комбинировать шаги приращения 512 ГБ и 768 ГБ до достижения максимального значения объема памяти единицы.
- **Класс II типа**: С шагом 512 ГБ, с наименьшим стартовым блоком 2 ТБ. Можно комбинировать шаги приращения 512 ГБ, 1 TБ и 1,5 ТБ до достижения максимального значения объема памяти единицы.
...
Хранилище, используемое для HANA Large Instance, использует прозрачное шифрование данных, поскольку они хранятся на дисках с конца 2018 года. В предыдущих развертываниях можно выбрать зашифрованные тома. Если вы решили отказаться от этой опции, вы можете запросить, чтобы получить тома зашифрованы в Интернете. Переход от незашифрованных к зашифрованным томам прозрачен и не вызывает простоя. 

Для категории номеров SKU класса 1 том с загрузочным логическим номером LUN зашифрован. В редакции 3 HANA Large Instance марки, используя Тип II класса SKUs HANA Большой instance, вам нужно зашифровать загрузки LUN с методами ОС. В штампах Revision 4 HANA Large Instance, используя единицы Типа II, хранится объем загрузки LUN и шифруется в состоянии по умолчанию. 

## <a name="required-settings-for-larger-hana-instances-on-hana-large-instances"></a>Необходимые настройки для больших экземпляров HANA на крупных инстанциях HANA
...
> [!IMPORTANT]
> Для предотвращения того, чтобы HANA пыталась увеличить количество файлов данных за предельный размер 16 ТБ в хранилище HANA Large Instance, необходимо установить следующие параметры в файле конфигурации global.ini HANA
> 
> - datavolume_striping
