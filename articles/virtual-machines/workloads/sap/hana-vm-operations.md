...
Linux имеет несколько разных режимов планирования операций ввода-вывода. Обычно поставщики Linux и SAP рекомендуют задавать для планирования операций ввода-вывода для томов на диске режим **noop** (а не **cfq**). Дополнительные сведения см. в [примечании к SAP № 1984798](https://launchpad.support.sap.com/#/notes/1984787). 
...
Режимом UltraSSD дает возможность определения одного диска, удовлетворяющей диапазон пропускной способности объема операций ввода-ВЫВОДА и диска. Вместо того чтобы использовать диспетчеры логического тома, такие как диспетчер логических ТОМОВ или MDADM, на основе хранилища класса Premium для создания тома, отвечающие требованиям пропускную способность операций ввода-ВЫВОДА и хранения. Набор конфигурации можно запустить между режимом UltraSSD и "премиум". В результате вы можете ограничить использование режимом UltraSSD производительность важных/hana/Data и тома/hana/log и рассматриваются другие тома с хранилищем Azure Premium
...
Значения являются просто рассматривать в качестве отправной точки и должны вычисляться реальные потребности. С помощью Azure Ultra SSD удобен тем, что значения операций ввода-ВЫВОДА и пропускную способность можно адаптировать без необходимости завершать работу виртуальной Машины или остановка рабочей нагрузки применяются к системе.   
...
- Для **/hana/shared** вы можете создать высокодоступный кластер NFS под управлением SUSE Linux 12 SP3. Этот кластер узлов **/hana/shared** общих ресурсов NFS конфигурации горизонтального масштабирования и SAP NetWeaver или BW/4HANA центральные службы. Документация для создания такой конфигурации доступна в статье [Обеспечение высокого уровня доступности NFS на виртуальных машинах Azure в SUSE Linux Enterprise Server](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-nfs).
...
>[!NOTE]
>Отображаемый на рисунке высокодоступный кластер NFS пока поддерживается только в SUSE Linux. Учтите позднее будет решений высокой доступности NFS на базе Red Hat.
...
Проверьте, соответствует ли пропускная способность хранилища для разных предлагаемых томов рабочей нагрузки, которую вы хотите запустить. Если для рабочей нагрузки необходимы большие тома **/hana/data** и **/hana/log**, нужно увеличить количество виртуальных жестких дисков хранилища Azure класса Premium. Изменение размера тома с большим количеством VHD, чем перечисленные увеличение операций ввода-ВЫВОДА и пропускной способности ввода-вывода в пределах ограничений типа виртуальной машины Azure. Также примените ускоритель записи Azure к дискам, которые формируют том **/hana/log**.
...
- **После установки вам необходимо изменить файл global.ini и добавить в него параметр basepath_shared = no**. Этот параметр включает SAP HANA для запуска в масштабирования без «общий» **/hana/data** и **/hana/log** томов между узлами. Дополнительные сведения см. в [примечании к SAP № 2080991](https://launchpad.support.sap.com/#/notes/2080991).
...
Следуя этой процедуре настройки, установленная конфигурация горизонтального масштабирования будет применять не используемые совместно диски для выполнения томов **/hana/data** и **/hana/log**. Тогда как **/hana/shared** тома будет размещаться на высокой доступности для общей папки NFS.
...
Кроме сертификации SAP HANA на виртуальных машинах Azure серии M, в Microsoft Azure поддерживается решение SAP HANA Dynamic Tiering 2.0 (подробнее см. приведенную ниже ссылку на соответствующую документацию). Нет никакой разницы между установкой продукта и использованием его через, например, панель SAP HANA на виртуальной машине Azure. Но есть несколько важных аспектов, которые нужно обязательно соблюдать для включения официальной поддержки в Azure. Эти важные замечания описаны ниже. В статье сокращение от «DT 2.0» будет использоваться вместо полного имени динамическое распределение по уровням 2.0.
...
Дополнительные сведения будут рассматриваться в следующих разделах.
...
В соответствии с рекомендациями для DT 2.0 на каждое физическое ядро следует выделить пропускную способность дискового ввода-вывода не менее 50 МБ/с. Взглянув на спецификации для двух типов виртуальных Машин Azure, какие поддерживаются для DT 2.0, максимальный предел пропускной способности ввода-ВЫВОДА для виртуальной Машины внешний вид, например:
