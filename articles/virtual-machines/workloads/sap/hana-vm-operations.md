...
>[!IMPORTANT]
> Для того, чтобы использовать M208xx_v2 ВМ, необходимо быть осторожным при выборе изображения Linux из галереи изображений Azure VM. Для того, чтобы прочитать подробности, прочитайте статью [Память оптимизированных виртуальных размеров машины](../../mv2-series.md).
> 


### <a name="storage-configuration-for-sap-hana"></a>Конфигурация хранения для SAP HANA
Для конфигураций хранения и типов хранилищ, которые будут использоваться с SAP HANA в Azure, прочитайте документ [SAP HANA Azure виртуальных конфигураций машинного хранения](./hana-vm-operations-storage.md)
...
Чтобы узнать типы Azure VM, сертифицированные как для масштабирования OLAP, так и для S/4HANA, проверьте [аппаратный каталог SAP HANA.](https://www.sap.com/dmc/exp/2014-09-02-hana-hardware/enEN/iaas.html#categories=Microsoft%20Azure) Галочное пятно в столбце "Кластеризация" указывает на поддержку масштабирования. Тип приложения указывает, поддерживается ли масштаб OLAP или масштаб S/4HANA. Для получения подробной информации о узлах, сертифицированных в масштабе для каждого из VMs, проверьте детали записей в конкретном VM SKU, перечисленных в аппаратном каталоге SAP HANA.

Минимальные релизы ОС для развертывания масштабных конфигураций в Azure VMs, проверьте детали записей в конкретном VM SKU, указанном в аппаратном каталоге SAP HANA. Из n-узла oLAP масштабирования из конфигурации, один узла функционирует как мастер-узлы. Другие узлы до предела сертификации действуют как рабочий узлы. Дополнительные резервные узлы не учитываются в количестве сертифицированных узлов

>[!NOTE]
> Развертывание SAP HANA с резервным узлом Azure VM возможно только с помощью хранилища [файлов NetApp Azure.](https://azure.microsoft.com/services/netapp/) Ни одно другое сертифицированное хранилище Azure, сертифицированное SAP HANA, не позволяет конфигурацию резервных узлов SAP HANA
>

Для /hana/shared мы также рекомендуем использовать [файлы NetApp Azure](https://azure.microsoft.com/services/netapp/). 

Типичный базовый дизайн для одного узла в масштабируемой конфигурации будет выглядеть следующим образом:
...
- Для **/hana/shared**вы используете нативную службу NFS, предоставляемую через файлы NetApp Azure. 
- Все остальные объемы диска не разделяются между различными узлами и не основаны на NFS. Конфигурации установки и шаги для масштабирования установок HANA с необщими **/hana/data** и **/hana/log** предоставляются позже в этом документе. Для сертифицированных данных HANA, которые могут быть использованы, проверьте статью [SAP HANA Azure виртуальные конфигурации машинного хранения.](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage)


Размер объемов или дисков необходимо проверить документ [SAP HANA TDI Хранения требований,](https://www.sap.com/documents/2015/03/74cdb554-5a7c-0010-82c7-eda71af511fa.html)для размера, необходимого в зависимости от количества узлов работника. Документ выпускает формулу, которую необходимо применить, чтобы получить необходимую емкость объема

Другим критерием проектирования, отображаемым в графике конфигурации одного узла для масштабируемого SAP HANA VM, является VNet, или лучше конфигурация подсети. SAP настоятельно рекомендует отделить трафик к клиенту или приложению от трафика между узлами HANA. Как показано на рисунке, эта цель достигается за счет наличия двух разных виртуальных сетевых адаптеров, подключенных к виртуальной машине. Оба адаптера находятся в разных подсетях и имеют два разных IP-адреса. Затем вы будете управлять потоком трафика с помощью правил маршрутизации с использованием групп безопасности сети или определенных пользователем маршрутов.
...
>[!NOTE]
>SAP рекомендует разделить сетевой трафик на клиентскую/прикладную сторону и внутриузловый трафик, как описано в этом документе. Поэтому рекомендуется ввести архитектуру в действие, как показано в последней графике. Также проконсультируйтесь с группой по безопасности и соответствию требованиям, которые отклоняются от рекомендации 
>
...
- Развертывание новой или адаптирующей существующей инфраструктуры Azure VNet
- Развертывание новых VM с использованием управляемого премиум-хранилища Azure, объемов дисков Ultra и/или объемов NFS на основе ANF
...
- В случае использования Azure Premium Storage или Ultra-хранилища дисков с необщими дисками /hana/data и /hana/log необходимо изменить файл global.ini и добавить параметр "basepath_shared и нет" в файл global.ini. Этот параметр позволяет SAP HANA работать в масштабе без "общих" **/hana/data** и **/hana/log** томов между узлами. Дополнительные сведения см. в [примечании к SAP № 2080991](https://launchpad.support.sap.com/#/notes/2080991). Если вы используете объемы NFS на основе ANF для /hana/data и /hana/log, вам не нужно вносить это изменение
...
Подробная информация о настройке системы масштабирования SAP HANA с резервным узлом на SUSE Linux подробно описана в [системе развертывания sAP HANA с резервным узлом на Azure VMs с помощью файлов Azure NetApp на сервере SUSE Linux Enterprise Server.](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-scale-out-standby-netapp-files-suse) Эквивалентная документация для Red Hat можно найти в статье [Развертывание sAP HANA масштабная система с резервным узлом на Azure VMs с помощью Azure NetApp файлов на Red Hat Enterprise Linux](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-scale-out-standby-netapp-files-rhel). 
...
Кроме сертификации SAP HANA на виртуальных машинах Azure серии M, в Microsoft Azure поддерживается решение SAP HANA Dynamic Tiering 2.0 (подробнее см. приведенную ниже ссылку на соответствующую документацию). Нет никакой разницы между установкой продукта и использованием его через, например, панель SAP HANA на виртуальной машине Azure. Но есть несколько важных аспектов, которые нужно обязательно соблюдать для включения официальной поддержки в Azure. Эти важные замечания описаны ниже. На протяжении всей статьи вместо полного имени Dynamic Tiering 2.0 будет использоваться аббревиам "DT 2.0".
...
Более подробная информация будет разъяснена в следующих разделах.
...
В соответствии с рекомендациями для DT 2.0 на каждое физическое ядро следует выделить пропускную способность дискового ввода-вывода не менее 50 МБ/с. Глядя на спецификации для двух типов Azure VM, которые поддерживаются для DT 2.0 максимальный лимит пропускной мощности диска IO для VM выглядеть следующим образом:
...
- [SAP HANA тест на моментальный снимок хранилища](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-backup-storage-snapshots)
...
SAProuter следует установить на отдельной виртуальной машине, а не на виртуальной машине Jumpbox. Отдельная виртуальная машина должна иметь статический IP-адрес. Чтобы подключить ваш SAProuter к SAProuter, размещенный в SAP, обратитесь к SAP для получения IP-адреса. (SAProuter, который размещается SAP, является аналогом экземпляра SAProuter, который вы устанавливаете на VM.) Используйте IP-адрес SAP для настройки экземпляра SAProuter. В параметрах конфигурации требуется только TCP-порт 3299.
...
Если вы работаете под управлением SUSE Linux Enterprise Server или Red Hat, вы можете создать кластер Pacemaker с устройствами STONITH. С помощью этих устройств можно настроить конфигурацию SAP HANA, которая использует синхронную репликацию с репликацией системы HANA и автоматическим переходом на другой ресурс. Для получения дополнительной информации, указанной в разделе "Следующие шаги".

## <a name="next-steps"></a>Next Steps
Знакомьтесь со статьями, перечисленными
