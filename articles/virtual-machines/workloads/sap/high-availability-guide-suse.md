---
title: Azure VMs высокой доступности для SAP NetWeaver на SLES (ru) Документы Майкрософт
...
В конфигурациях примера команды установки и т.д. Используется номер экземпляра ASCS 00, номер экземпляра ERS 02 и SAP System ID NW1. Имена ресурсов (например, виртуальных машин или сетей) в примере предполагают, что вы использовали [конвергированный шаблон][template-converged] с идентификатором системы SAP NW1, чтобы создать эти ресурсы.
...
Все необходимые ресурсы можно развернуть с помощью шаблонов быстрого запуска с сайта GitHub. Шаблон развертывает виртуальные машины, балансизатор нагрузки, набор доступности и т.д. Выполните следующие действия, чтобы развернуть шаблон:

1. Откройте [шаблон ASCS/SCS Multi SID][template-multisid-xscs] или [конвергентный шаблон][template-converged] на портале Azure. 
   Шаблон ASCS/SCS создает только правила балансировки нагрузки для экземпляров SAP NetWeaver ASCS/SCS и ERS (только Linux), в то время как конвергентный шаблон также создает правила балансировки нагрузки для базы данных (например, Microsoft S'L Server или SAP HANA). Если вы планируете установить систему на основе SAP NetWeaver и базу данных на одних и тех же компьютерах, используйте [конвергированный шаблон][template-converged].
...
Сначала необходимо создать виртуальные машины для этого кластера NFS. После этого вы создаете балансиватор нагрузки и используете виртуальные машины в пуле бэкэнда.
...
1. Создание балансиватора нагрузки (внутренний, стандартный):  
   ...
   1. Правила балансировки нагрузки
      1. Правила балансировки нагрузки для ASCS
         ...
         1. Введите название нового правила балансоровы нагрузки (например, **nw1-lb-ascs)**
         1. Выберите адрес ip-версии, бэкэнд бассейн, и зонд здоровья вы создали ранее (например, **nw1-ascs-frontend**, **nw1-бэкэнд** и **nw1-ascs-hp)**
         1. Выберите **порты HA**
         1. Увеличьте время ожидания до 30 минут.
         1. **Не забудьте включить плавающий IP-адрес**.
         1. Нажмите кнопку "ОК"
         * Повторите выше, чтобы создать правила балансировки нагрузки для ERS (например, **nw1-lb-ers)**
1. Кроме того, если сценарий требует базового балансиватора нагрузки (внутреннего), выполните следующие действия:  
   1. Создайте IP-адреса внешнего интерфейса.
      1. IP-адрес 10.0.0.7 для ASCS
         ...
         1. Нажмите кнопку "ОК"
      1. IP-адрес 10.0.0.8 для ASCS ERS
         * Повторите приведенные выше действия, чтобы создать IP-адрес для ERS (например, **10.0.0.8** и **nw1-aers-frontend)**
   1. Создание внутреннего пула
      1. Выберите подсистему балансировки нагрузки, щелкните "Серверные пулы" и нажмите кнопку "Добавить".
      1. Введите имя нового внутреннего пула (например, **nw1-backend**).
      1. Щелкните "Добавить виртуальную машину".
...
1. **[A]** Установите соединитель SUSE.
   ...
   > [!NOTE]
   > Известная проблема с использованием тире в именах хоста фиксируется с версией **3.1.1** пакета **пакета sap-suse-кластер-разъем.** Убедитесь, что вы используете по крайней мере версию 3.1.1 пакета пакета sap-suse-cluster-connector, если используете кластерные узлы с тире в имени хоста. Иначе кластер не будет работать. 
...
При использовании архитектуры сервера enqueue 2[(ENSA2)](https://help.sap.com/viewer/cff8531bc1d9416d91bb6781e628d4e0/1709%20001/en-US/6d655c383abf4c129b0e5c8683e7ecd8.html)определите ресурсы следующим образом:
