---
title: Высокая доступность SAP HANA на Облачных Выцоттовых ВМ на SLES Документы Майкрософт
...
* SAP Note [2205917] рекомендовала настройки ОС для SUSE Linux Enterprise Server для SAP-приложений.
* SAP Note [1944799] имеет руководящие принципы SAP HANA для Корпоративного сервера SUSE Linux для SAP-приложений.
...
1. Задайте следующие параметры:
    ...
    - **Тип стека**: (Этот параметр применим только при использовании конвергентного шаблона.) Выберите тип стека SAP NetWeaver.
    - **Os Type** (Тип ОС). Выберите один из дистрибутивов Linux. Для этого примера выберите **SLES 12**.
    ...
    - **Имя пользователя и пароль админ:** Создается новый пользователь, который может быть использован для вху-на-аймин.
...
1. **[1]** Создайте базу данных клиента.
   ...
   Выполните следующую команду в\>качестве <ханасидад adm:
...
1. **Настройка** репликации системы на первом узлах:
   ...
   Резервное копирование баз данных\>в виде <адм ханасида:
...
1. **[2]** Настройте системную репликацию на втором узле
   ...
   Зарегистрируйте второй узел, чтобы запустить репликацию системы. Выполнить следующую команду,\>как <ханасид адм:
...
1. **[1]** Настройте репликацию системы на первом узле.
   ...
   Создайте основной сайт в\>качестве <ханасида:
...
1. Тест 1. Остановка основной базы данных на узле 1
   ...
   Выполнить следующие команды, как\><ханасид адм на узло hn1-db-0:
...
1. Тест 2. Остановка основной базы данных на узле 2
   ...
   Выполнить следующие команды, как\><ханасид адм на узло hn1-db-1:
