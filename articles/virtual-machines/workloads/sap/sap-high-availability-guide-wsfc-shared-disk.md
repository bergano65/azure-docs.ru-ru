---
title: Кластерный экземпляр SAP ASCS/SCS на WSFC с использованием общего диска в Azure Документы Майкрософт
...
* Центральные службы SAP.
    * Два процесса, сервер сообщения и enqueue, а также \<виртуальное имя хоста ASCS/SCS>, которое используется для доступа к этим двум процессам.
...
* Файлы глобального узла SAP.
  ...
  * Файловый ресурс sapmnt, который предоставляет доступ к глобальным файлам S:\usr\sap\\&lt;SID&gt;\SYS\... по следующему UNC-пути:
    ...
    \\\\\><ASCS/SCS виртуальное имя хозяина\\&lt;&gt;sapmnt\.SID .SYS ..
...
_**Рисунок 2:** Процессы, структура файлов и глобальная доля файлов sapmnt в экземпляре SAP ASCS/SCS_
...
_**Рисунок 3:** Архитектура SAP ASCS/SCS HA с общим диском_

> [!IMPORTANT]
> Следующие два компонента выполняются в одном экземпляре SAP ASCS/SCS.
>* То \<же имя виртуального хоста ASCS/SCS> используется для доступа к процессам sAP-сообщений и серверов, а также к файлам глобального хоста SAP через sapmnt-файл.
