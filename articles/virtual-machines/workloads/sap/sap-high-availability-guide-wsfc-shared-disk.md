...
![Рис. 1. Конфигурация отказоустойчивого кластера Windows в Azure без общего диска][sap-ha-guide-figure-1001]
...
* Центральные службы SAP.
    * Два процесса: сообщение и сервер постановки в очередь \<и имя виртуального узла ASCS/SCS >, которое используется для доступа к этим двум процессам.
    * Структура файла: Идентификатор\\&lt;\<безопасностиS:\usr\sap\ASCS/SCSномер&gt;экземпляра\>
...
![Рис. 2. Процессы, структура файлов и общая папка sapmnt общего узла для экземпляра SAP ASCS/SCS][sap-ha-guide-figure-8001]

_**Рис. 2.** Процессы, структура файлов и общая папка sapmnt общего узла для экземпляра SAP ASCS/SCS_
...
![Рис. 3. Архитектура высокой доступности SAP ASCS/SCS с общим диском][sap-ha-guide-figure-8002]

_**Рис. 3.** Архитектура высокой доступности SAP ASCS/SCS с общим диском_

> [!IMPORTANT]
> Следующие два компонента выполняются в одном экземпляре SAP ASCS/SCS.
>* То же \<имя виртуального узла ASCS/SCS > используется для доступа к процессам сервера сообщений SAP и постановки в очередь, а также файлов глобального узла SAP через файловый ресурс sapmnt.
>* Общий диск кластера с именем S: используется ими совместно.
>


![Рис. 4. Архитектура высокой доступности SAP ASCS/SCS с общим диском][sap-ha-guide-figure-8003]

_**Рис. 4.** Архитектура высокой доступности SAP ASCS/SCS с общим диском_
...
![Рис. 5. Конфигурация отказоустойчивой кластеризации Windows Server в Azure с помощью SIOS][sap-ha-guide-figure-1002]

_**Рис. 5.** Конфигурация отказоустойчивого кластера Windows в Azure с помощью SIOS_
...
* [Установка SAP NetWeaver высокого уровня доступности в отказоустойчивом кластере Windows и на общем диске для экземпляра SAP ASCS/SCS][sap-high-availability-installation-wsfc-shared-disk]
