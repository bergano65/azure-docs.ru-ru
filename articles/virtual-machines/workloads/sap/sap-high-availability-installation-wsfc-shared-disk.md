...
* [Подготовьте инфраструктуру Azure для SAP HA с помощью кластера сбоя Windows и общего диска для экземпляра SAP ASCS/SCS][sap-high-availability-infrastructure-wsfc-shared-disk]
...
## <a name="install-sap-with-a-high-availability-ascsscs-instance"></a><a name="31c6bd4f-51df-4057-9fdf-3fcbc619c170"></a>Установите SAP с экземпляром ASCS/SCS высокого уровня доступности.
...
2. Чтобы определить IP-адрес, назначенный виртуальному имени хоста, выберите **dNS Manager** > **Domain.**
...
2. Укажите порт пробы. По умолчанию для проб задан порт 0. В нашем примере мы используем порт пробы 62000.
   ...
   После того, как \<\> вы переведите роль кластера SAP SID в режиме онлайн, убедитесь, что **ProbePort** настроен на новое значение.
   ...
   _**Рисунок 4:** Зондирование кластерного порта после установки нового значения_
...
_**Рисунок 6:** Менеджер кластера Failover: \<\> кластерная группа SAP SID работает на кластерном узлах A_
...
_**Рисунок 7:** В SIOS DataKeeper повторите локальный объем из кластерного узла A в кластерный узлы B_
...
4. Перезапустите узел A кластера с помощью Azure PowerShell. Это инициирует автоматический переход кластерной группы SAP \<SID\> на другой ресурс с узла A на узел B кластера.
   ...
   _**Рисунок 9:** SIOData DataKeeper реплицирует локальный объем из кластерного узла B в кластерный узлы A_
