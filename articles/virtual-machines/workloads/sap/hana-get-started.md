...
Дополнительные сведения об операционных системах, поддерживаемых SAP Hana, см [. в разделе SAP Note 2235581-SAP HANA. Поддерживаемые операционные](https://launchpad.support.sap.com/#/notes/2235581/E)системы. На виртуальных машинах Azure поддерживаются только эти операционные системы. Следующие операционные системы поддерживаются для развертывания SAP HANA в Azure: 
...
* [Примечание SAP 171356. Программное обеспечение SAP в Linux: Общие сведения](https://launchpad.support.sap.com/#/notes/1984787).
* [Примечание SAP 1944799. SAP HANA рекомендации по установке](https://go.sap.com/documents/2016/05/e8705aae-717c-0010-82c7-eda71af511fa.html)операционной системы SLES.
* [Примечание SAP 2205917. Рекомендуемые параметры операционной системы SAP HANA DB для приложений](https://launchpad.support.sap.com/#/notes/2205917/E)SAP SLES 12.
* [Примечание SAP 1391070. Решения](https://launchpad.support.sap.com/#/notes/1391070)UUID Linux.
* [Примечание SAP 2009879. SAP HANA рекомендации для операционной системы](https://launchpad.support.sap.com/#/notes/2009879)Red Hat Enterprise Linux (RHEL).
...
Типы виртуальных машин Azure, сертифицированные SAP для SAP NetWeaver или уровня приложений S/4HANA, описаны в [статье SAP Note 1928533: Приложения SAP в Azure: Поддерживаемые продукты и типы](https://launchpad.support.sap.com/#/notes/1928533/E)виртуальных машин Azure.
...
На следующем снимке экрана можно увидеть, как **настроенные параметры ADM** изменили `numa_balancing` `transparent_hugepage` значения и в соответствии с требуемыми параметрами SAP Hana:
...
Необходимо избежать проблем с параметром идентификатора группы по умолчанию для `\<HANA SID\>adm user`, который создается средством HDBLCM. Перед установкой SAP HANA через HDBLCM, определите новую группу с именем `sapsys` с помощью идентификатора `1001`группы:
...
Такой подход обеспечивает достаточный объем памяти для корневой файловой системы. Обратите внимание, что системный администратор Hana имеет `1005` идентификатор пользователя и является частью `sapsys` группы с идентификатором `1001`, которая была определена до установки.

![Список всех выбранных ранее основных компонентов SAP HANA](./media/hana-get-started/image032.jpg)

`\<HANA SID\>adm user` Проверьте сведения в каталоге/etc/passwd. `azdadm`Найдите, как показано на следующем снимке экрана:
