...
1. Щелкните "Добавить".
1. Введите имя, выберите тип приложения "веб-приложение или API", введите URL-адрес входа (например, http:/ЛОКАЛХОСТ\/) и нажмите кнопку "создать".
...
Теперь тома NFS можно определить. Так как в одном пуле будут храниться тома для нескольких систем, следует выбрать схему именования с самообъяснением. Добавление идентификатора безопасности позволяет группировать связанные тома вместе. Для ASCS и экземпляра AS необходимы следующие подключения:/сапмнт/\<SID\>,/usr/SAP/\<SID\> и/Хоме/\<SID\>ADM. Необязательный/УСР/САП/транс для каталога Центрального транспорта, который по крайней мере используется всеми системами в одной альбомной ориентации.
