...
Теперь тома NFS можно определить. Так как в одном пуле будут храниться тома для нескольких систем, следует выбрать схему именования с самообъяснением. Добавление идентификатора безопасности позволяет группировать связанные тома вместе. Для ASCS и экземпляра AS требуются следующие подключения: */сапмнт/\<\>SID*, */usr/SAP/\<SID\>* и */Хоме/\<SID\>ADM*. При необходимости */УСР/САП/транс* требуется для центрального каталога транспорта, который по крайней мере используется всеми системами в одной альбомной ориентации.
