...
Теперь тома NFS можно определить. Так как в одном пуле будут храниться тома для нескольких систем, следует выбрать схему именования с самообъяснением. Добавление идентификатора безопасности позволяет группировать связанные тома вместе. Для ASCS и экземпляра AS необходимы следующие подключения: */сапмнт/\<SID @ no__t-2*, */usr/SAP/\<SID @ no__t-5*и */Хоме/\<sid @ no__t-8adm*. При необходимости */УСР/САП/транс* требуется для центрального каталога транспорта, который по крайней мере используется всеми системами в одной альбомной ориентации.
...
@no__t 0SAP лама точки подключения в Лама ](media/lama/sap-lama-ascs.png) (это пример. IP-адреса и путь экспорта отличаются от путей, которые использовались ранее).
