---
title: Высокая доступность для NFS на Azure VMs на SLES Документы Майкрософт
...
* Порт пробы:
  * Порт 61000 для NW1.
  * Порт 61001 для NW2.
* Правила балансировки нагрузки (при использовании базового балансивизатора нагрузки)
...
1. Задайте следующие параметры.
   ...
   2. Количество систем SAP  
      Введите количество систем SAP, которые будут использовать этот файловый сервер. Это позволит развернуть необходимое количество конфигураций фронтального, правил балансировки нагрузки, портов зонда, дисков и т.д.
   3. Тип ОС  
...
1. Создайте балансировора нагрузки (внутренний). Мы рекомендуем [стандартный балансер нагрузки.](https://docs.microsoft.com/azure/load-balancer/load-balancer-standard-overview)  
   1. Следуйте этим инструкциям для создания стандартного балансиватора нагрузки:
      ...
      1. Создайте внутренние пулы.
         1. подключена к основным сетевым интерфейсам всех виртуальных машин, которые должны быть частью кластера NFS.
            ...
            1. Введите название нового пула бэкэнда (например, **nw-backend)**
      ...
      1. правила балансировки нагрузки.
         ...
         1. Введите название нового правила балансоровы нагрузки (например, **nw1-lb)**
         1. Выберите IP-адрес переднего энда, пул бэкэнда и зонд здоровья, созданный ранее (например, **nw1-frontend.** **nw-backend** и **nw1-hp)**
         1. Выберите **порты HA**.
         1. Увеличьте время ожидания до 30 минут.
         1. **Не забудьте включить плавающий IP-адрес**.
         1. Нажмите кнопку "ОК"
         * Повторите вышеприведенные шаги, чтобы создать правило балансировки нагрузки для NW2
   1. Кроме того, если сценарий требует базового балансистора нагрузки, следуйте следующим инструкциям:
      1. Создайте IP-адреса внешнего интерфейса.
         1. IP-адрес 10.0.0.4 для NW1.
            1. Откройте подсистему балансировки нагрузки, выберите пул внешних IP-адресов и щелкните "Добавить".
            1. Введите имя нового внешнего пула IP-адресов (например, **nw1-frontend**).
...
1. **[A] ** Настройте обнаружение нескольких основных устройств DRBD.
   ...
   При использовании DRBD для синхронизации данных одного узла с другим может возникнуть нескольких основных устройств. Разделенный мозг — это сценарий, при котором оба кластерных узла способствовали тому, что устройство drbd было первичным и выходило из синхронизации. Это может быть редкая ситуация, но вы все еще хотите, чтобы справиться и решить раскол мозга как можно быстрее. Поэтому важно получать уведомления о возникновении нескольких основных устройств.
...
1. **[1]** Добавьте в конфигурацию кластера устройства NFS DRBD для системы SAP NW2.
   ...
   Вариант `crossmnt` в `exportfs` ресурсах кластера присутствует в нашей документации для обратной совместимости со старыми версиями SLES.  
