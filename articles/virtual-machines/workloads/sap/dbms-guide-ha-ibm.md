...
| [Продление версии SUSE Linux Enterprise с высоким уровнем доступности 12 SP3][sles-ha-guide] |
...
Убедитесь, что выбранная ОС поддерживается IBM DB2 LUW для IBM/SAP. Список поддерживаемых версий ОС для виртуальных машин Azure и выпусков DB2 доступен в примечании SAP [1928533]. Список выпусков ОС отдельных версий DB2 доступен в матрице доступности продуктов SAP. Мы настоятельно рекомендуем минимум SLES 12 SP3 из-за улучшений производительности, связанных с Azure, в этой или более поздних версиях SUSE Linux.
...
1. Измените среду оболочки для идентификатора безопасности\<DB2 > пользователя на */бин/КШ*. Рекомендуется использовать средство YaST. 
...
- **Миграция \<ресурса CRM > \<> узла:** Создает ограничения расположения и может вызвать проблемы с перенаправление
- **ресурс CRM Clear \<RES_NAME >** : Очищает ограничения расположения
- **> очистки \<ресурсов CRM RES_NAME**: Удаляет все ошибки ресурса.
