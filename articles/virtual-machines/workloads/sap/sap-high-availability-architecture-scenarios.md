...
Высокая доступность SAP в Azure отличается от высокой доступности SAP в локальной физической или виртуальной среде. Следующий документ [SAP NetWeaver высокого уровня доступности и непрерывности бизнес-процессов в виртуальных средах с VMware и Hyper-V в Microsoft Windows][sap-ha-bc-virtual-env-hyperv-vmware-white-paper] описываются стандартные конфигурации высокого уровня доступности SAP в виртуализированных среды на Windows.

Конфигурации высокой доступности SAP, интегрированной с SAPinst, для Linux (как и для Windows) не существует. Сведения о SAP высокого уровня доступности в локальной для Linux, см. в разделе [сведения о партнере высокого уровня доступности][sap-ha-partner-information].
...
Дополнительные сведения см. в разделе [Управление доступностью виртуальных машин Windows в Azure][azure-virtual-machines-manage-availability].
...
Дополнительные сведения см. в разделе [обзоре управляемых дисков Azure][azure-storage-managed-disks-overview].
...
Дополнительные сведения об этом подходе см. в разделе [перезапуск виртуальной Машины в инфраструктуре Azure, использовать для повышения доступности системы SAP][sap-higher-availability].
...
![Рис. 1. Сервер приложений SAP с высоким уровнем доступности][sap-ha-guide-figure-2000]
...
* Все виртуальные машины входят в один и тот же домен обновления.  
    ...
    Основные функциональные возможности, основанные на различные обновления и доменов сбоя в рамках единицы масштабирования Azure, уже были описаны выше в [домены обновления][planning-guide-3.2.2] раздел.
...
Дополнительные сведения см. в разделе [группы доступности Azure][planning-guide-3.2.3] разделе виртуальные машины Azure планированию и внедрению SAP NetWeaver документа.
...
* **Кластеризация экземпляра SAP ASCS/SCS с использованием кластерных общих дисков**. Дополнительные сведения об этой архитектуре см. в разделе [кластеризация экземпляра SAP ASCS/SCS в отказоустойчивом кластере Windows с помощью общего диска кластера][sap-high-availability-guide-wsfc-shared-disk].   

* **Кластеризация экземпляра SAP ASCS/SCS с использованием общей папки**. Дополнительные сведения об этой архитектуре см. в разделе [кластеризация экземпляра SAP ASCS/SCS в отказоустойчивом кластере Windows с помощью файлового ресурса][sap-high-availability-guide-wsfc-file-share].
...
> 
> Дополнительные сведения о кластеризации экземпляра SAP ASCS/SCS с помощью платформы кластера SLES см. в разделе [высокий уровень доступности для SAP NetWeaver на виртуальных машинах Azure в SUSE Linux Enterprise Server для приложений SAP][sap-suse-ascs-ha]. For alternative HA architecture on SLES, which doesn't require highly available NFS see [High-availability guide for SAP NetWeaver on SUSE Linux Enterprise Server with Azure NetApp Files for SAP applications][sap-suse-ascs-ha-anf].
...
* [SAP ASCS/SCS высокого уровня доступности с несколькими ИД безопасности экземпляра для отказоустойчивого кластера Windows Server и файлового ресурса][sap-ascs-ha-multi-sid-wsfc-file-share]

* [SAP ASCS/SCS высокого уровня доступности с несколькими ИД безопасности экземпляра для отказоустойчивого кластера Windows Server и общего диска][sap-ascs-ha-multi-sid-wsfc-shared-disk]
...
* [Ручная настройка группы доступности AlwaysOn на виртуальных машинах Azure с помощью Resource Manager][virtual-machines-windows-portal-sql-alwayson-availability-groups-manual]
...
Дополнительные сведения о кластеризации СУБД SAP HANA в Azure с помощью модели развертывания Azure Resource Manager, см. в разделе [высокий уровень доступности SAP HANA на виртуальных машинах (ВМ) Azure][sap-hana-ha].
