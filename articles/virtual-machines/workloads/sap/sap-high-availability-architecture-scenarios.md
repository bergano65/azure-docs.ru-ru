---
title: Архитектура Azure VMs HA и сценарии для SAP NetWeaver Документы Майкрософт
...
* **Использование перезагрузки инфраструктуры Azure VM для повышения *доступности* приложений SAP:** 
...
* **Высокое доступность приложения SAP:** 
...
- Вы не можете использовать [Load Balancer (цен. категория "Базовый")](https://docs.microsoft.com/azure/load-balancer/load-balancer-overview) для создания отказоустойчивых кластерных решений, основанных на службах отказоустойчивого кластера Windows или Linux Pacemaker. Вместо этого вам нужно использовать [SKU стандартную нагрузку Azure](https://docs.microsoft.com/azure/load-balancer/load-balancer-standard-availability-zones)
...
Для получения дополнительной информации смотрите [репликацию хранилища Azure][azure-storage-redundancy].
...
Для получения дополнительной информации смотрите [обзор управляемых дисков Azure][azure-storage-managed-disks-overview].
...
_**Рисунок 1:** Сервер приложений SAP с высокой доступностью_
...
* **Кластер SAP ASCS/SCS экземпляр с помощью ANF SMB доля**: Для получения дополнительной информации об этой архитектуре, см Кластер [кластера SAP ASCS / SCS экземпляр на Windows failover кластера с помощью ANF SMB доля файлов.](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-windows-netapp-files-smb)
...
> 
> Дополнительные сведения о кластеризации экземпляра SAP ASCS/SCS с помощью платформы кластера SLES см. в статье [Руководство по обеспечению высокого уровня доступности виртуальных машин Azure для SAP NetWeaver на SUSE Linux Enterprise Server для приложений SAP][sap-suse-ascs-ha]. Для альтернативной архитектуры HA на SLES, которая не требует высокой доступности NFS [см. Руководство по высокой доступности для SAP NetWeaver на SUSE Linux Enterprise Server с Azure NetApp Файлов для приложений SAP.][sap-suse-ascs-ha-anf]
...
> ![Windows][Logo_Windows] Windows
> 
> Multi-SID поддерживается WSFC, используя общую долю файлов и общий диск.
> 
> Для получения дополнительной информации об архитектуре с высокой доступностью multi-SID в Windows см.
...
> ![Linux][Logo_Linux] Linux
> 
> Кластеризация Multi-SID поддерживается в кластерах Linux Pacemaker для SAP ASCS/ERS, ограниченных **пятью** SAP SID в одном кластере.
> Для получения дополнительной информации об архитектуре с высокой доступностью multi-SID на Linux см.:

* [HA для SAP NW на Azure VMs на SLES для приложений SAP много-SID руководство](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-multi-sid)
