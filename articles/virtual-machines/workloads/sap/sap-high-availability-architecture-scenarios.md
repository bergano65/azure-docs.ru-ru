---
title: Архитектура высокой доступности виртуальных машин Azure и сценарии для SAP NetWeaver | Документация Майкрософт
...
Высокая доступность SAP в Azure отличается от высокой доступности SAP в локальной физической или виртуальной среде. Следующая статья [SAP NetWeaver высокая доступность и непрерывность бизнес-процессов в виртуальных средах с помощью VMware и Hyper-V в Microsoft Windows][sap-ha-bc-virtual-env-hyperv-vmware-white-paper] описывает стандартные конфигурации высокого уровня доступности SAP в виртуализованных средах Windows.

Конфигурации высокой доступности SAP, интегрированной с SAPinst, для Linux (как и для Windows) не существует. Сведения о высоком уровне доступности SAP в локальной среде для Linux см. в статье [сведения о партнерах высокого уровня доступности][sap-ha-partner-information].
...
В настоящее время предоставляется соглашение SLA для одной виртуальной машины с временем бесперебойной работы на уровне 99,9 % и хранилищем класса Premium. Чтобы получить представление о доступности одной виртуальной машины, можно создать продукт для различных доступных [соглашений об уровне обслуживания Azure][azure-sla].
...
Дополнительные сведения см. в статье [Обзор управляемых дисков Azure][azure-storage-managed-disks-overview].
...
Дополнительные сведения об этом подходе см. [в статье использование перезапуска ВИРТУАЛЬНОЙ машины инфраструктуры Azure для обеспечения более высокой доступности системы SAP][sap-higher-availability].
...
* Все виртуальные машины входят в один и тот же домен обновления.  
    ...
    Базовая функциональность, которая строится на разных доменах обновления и сбоя в пределах единицы масштабирования Azure, уже появилась в разделе [домены обновления][planning-guide-3.2.2] .
...
Дополнительные сведения см. в разделе [группы доступности Azure][planning-guide-3.2.3] статьи Планирование и реализация SAP NetWeaver для виртуальных машин Azure.
...
* **Кластеризация экземпляра SAP ASCS/SCS с помощью кластеризованных общих дисков**. Дополнительные сведения об этой архитектуре см. в статье [КЛАСТЕРИЗАЦИЯ экземпляра SAP ASCS/SCS в отказоустойчивом кластере Windows с помощью общего диска кластера][sap-high-availability-guide-wsfc-shared-disk].   

* **Кластеризация экземпляра SAP ASCS/SCS с помощью файлового ресурса**. Дополнительные сведения об этой архитектуре см. [в статье КЛАСТЕРИЗАЦИЯ экземпляра SAP ASCS/SCS в отказоустойчивом кластере Windows с помощью файлового ресурса][sap-high-availability-guide-wsfc-file-share].

* **Кластеризация экземпляра SAP ASCS/SCS с помощью общего ресурса SMB использовании**. Дополнительные сведения об этой архитектуре см. в статье кластер кластера с [экземпляром SAP ASCS/SCS в отказоустойчивом кластере Windows с помощью общей папки SMB использовании](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-windows-netapp-files-smb).
...
> 
> Дополнительные сведения о кластеризации экземпляра SAP ASCS/SCS с помощью платформы кластеров SLES см. в статье [высокий уровень доступности SAP NetWeaver на виртуальных машинах Azure SUSE Linux Enterprise Server для приложений SAP][sap-suse-ascs-ha]. Для альтернативной архитектуры с высоким уровнем доступности на SLES, для которой не требуется высокодоступная Поддержка NFS, см. рекомендации по обеспечению [высокого уровня надежности для SAP NetWeaver на SUSE Linux Enterprise Server с Azure NetApp Files для приложений SAP][sap-suse-ascs-ha-anf].
...
> ![Windows][Logo_Windows] Windows
> 
> С WSFC поддерживается Multi-SID, с использованием общего файлового ресурса и общего диска.
> 
> Дополнительные сведения об архитектуре с высоким уровнем доступности с несколькими ИД безопасности в Windows см. в следующих статьях:

* [Высокий уровень доступности экземпляра SAP ASCS/SCS с несколькими ИД безопасности для отказоустойчивой кластеризации Windows Server и файлового ресурса][sap-ascs-ha-multi-sid-wsfc-file-share]

* [Высокий уровень доступности экземпляра SAP ASCS/SCS с несколькими ИД безопасности для отказоустойчивой кластеризации Windows Server и общего диска][sap-ascs-ha-multi-sid-wsfc-shared-disk]

> ![Linux][Logo_Linux] Linux
> 
> Кластеризация с несколькими ИД безопасности поддерживается в кластерах Pacemaker Linux для SAP ASCS/ERS, ограниченных **пятью** идентификаторами безопасности SAP в одном кластере.
> Дополнительные сведения об архитектуре с высоким уровнем доступности с несколькими ИД безопасности в Linux см. в следующих статьях:
...
* [Настройка группы доступности AlwaysOn на виртуальных машинах Azure вручную с помощью диспетчер ресурсов][virtual-machines-windows-portal-sql-alwayson-availability-groups-manual]

* [Настройка внутреннего балансировщика нагрузки Azure для группы доступности AlwaysOn в Azure][virtual-machines-windows-portal-sql-alwayson-int-listener]

Дополнительные сведения о кластеризации SAP HANA СУБД в Azure с помощью модели развертывания Azure Resource Manager см. в статье [высокий уровень доступности SAP HANA на виртуальных машинах Azure][sap-hana-ha].
