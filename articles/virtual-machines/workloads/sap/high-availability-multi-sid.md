...
В сентябре 2016 г. Корпорация Майкрософт выпустила функцию, которая может управлять несколько виртуальных IP-адресов с помощью [внутреннего балансировщика нагрузки Azure][load-balancer-multivip-overview]. Эта функция уже существует во внешней подсистеме балансировки нагрузки Azure.

Если у вас есть развертывание SAP, внутренний балансировщик нагрузки можно использовать для создания конфигурации кластера Windows для SAP ASCS/SCS, как описано в статье [руководство для высокого уровня доступности SAP NetWeaver на виртуальных машинах Windows][sap-ha-guide].
...
Вы уже настроили кластер WSFC, который используется для одного экземпляра SAP ASCS/SCS, как описано в [руководство для высокого уровня доступности SAP NetWeaver на виртуальных машинах Windows][sap-ha-guide] и, как показано на следующей схеме.
...
Процедура создания записи DNS также описывается подробно в главном [руководство для высокого уровня доступности SAP NetWeaver на виртуальных машинах Windows][sap-ha-guide-9.1.1].
...
Для выполнения этой процедуры необходимо установить SIOS DataKeeper на компьютерах кластера WSFC. Если это программное решение установлено, теперь нужно настроить репликацию между компьютерами. Процесс описан подробно в главном [руководство для высокого уровня доступности SAP NetWeaver на виртуальных машинах Windows][sap-ha-guide-8.12.3.3].  
...
Полный процесс установки второй системы SAP SID2 описывается в главном [руководство для высокого уровня доступности SAP NetWeaver на виртуальных машинах Windows][sap-ha-guide-9].
...
6. Открытие портов брандмауэра Windows для экземпляра SAP ASCS/SCS и порта пробы.  
 На обоих узлах кластера, используемых для экземпляров SAP ASCS/SCS, откройте все порты брандмауэра Windows, используемые SAP ASCS/SCS. Эти порты подробно описаны в [руководство для высокого уровня доступности SAP NetWeaver на виртуальных машинах Windows][sap-ha-guide-8.8].  
 Кроме того, откройте порт пробы внутренней подсистемы балансировки нагрузки Azure. В нашем случае это порт 62350.

7. [Изменение типа запуска службы для экземпляра SAP ERS Windows][sap-ha-guide-9.4].

8. [Установка основного сервера приложений SAP][sap-ha-guide-9.5] на новой выделенной виртуальной Машины.

9. [Установка дополнительного сервера приложений SAP][sap-ha-guide-9.6] на новой выделенной виртуальной Машины.
...
- [Подсистема балансировки нагрузки нескольких виртуальных IP-адресов для Azure][load-balancer-multivip-overview]
- [Руководство для высокого уровня доступности SAP NetWeaver на виртуальных машинах Windows][sap-ha-guide]
