...
SAP HANA предоставляет API-интерфейс архивации, который позволяет интегрировать средства резервного копирования сторонних поставщиков непосредственно с SAP HANA. (Не входит в область данного руководством.) Прямо сейчас нет прямой интеграции SAP HANA со службой Azure Backup, доступной на основе этого API.
...
В Azure также доступно гибридное решение SAP HANA, где эта платформа выполняется на невиртуализированных физических серверах. Однако это SAP HANA руководстве по резервному копированию Azure охватывает чистую среду Azure, в которой SAP HANA работает на виртуальной машине Azure, а не SAP HANA на &quot;больших экземплярах.&quot; см. [в статье обзор SAP HANA (крупные экземпляры) и описание архитектуры в Azure](hana-overview-architecture.md) . Дополнительные сведения об этом решении резервного копирования в &quot;крупных экземплярах&quot; на основе моментальных снимков хранилища.
...
_Нет. в настоящее время резервные копии данных и журналов можно использовать только на основной стороне. Если автоматическая архивация журналов включена, после перенаправление на вторичную сторону резервные копии журналов будут автоматически записаны.&quot;_
...
Очень важно оценить размер резервной копии SAP HANA. Это позволяет улучшить производительность за счет определения максимального размера файла для нескольких файлов резервных копий в связи с параллелизмом во время копирования файлов. (Эти сведения описаны далее в этой статье.) Необходимо также решить, следует ли выполнить полное резервное копирование или разностное резервное копирование (добавочное или разностное).
