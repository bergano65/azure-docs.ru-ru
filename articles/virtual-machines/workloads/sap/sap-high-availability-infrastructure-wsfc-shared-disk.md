...
* [Руководство по архитектуре: Кластера экземпляра SAP ASCS/SCS в отказоустойчивом кластере Windows с помощью общего диска кластера][sap-high-availability-guide-wsfc-shared-disk]
...
Для производственных систем SAP, развертывание виртуальных машин Azure с помощью [подключением к корпоративной сети (распределенное развертывание)][planning-guide-2.2] с помощью VPN-шлюза Azure или Azure ExpressRoute.
...
Вы можете подготовить инфраструктуру и настроить SAP для поддержки сценария с несколькими идентификаторами безопасности. Например, вы можете добавить дополнительный экземпляр SAP ASCS/SCS в *имеющуюся* конфигурацию кластера. Дополнительные сведения см. в разделе [настроить дополнительный экземпляр SAP ASCS/SCS для существующей конфигурации кластера для создания конфигурации с несколькими ИД безопасности SAP в Azure Resource Manager][sap-ha-multi-sid-guide].
...
Чтобы настроить шаблон ASCS/SCS с несколькими ИД безопасности, в [шаблон ASCS/SCS с несколькими ИД безопасности][sap-templates-3-tier-multisid-xscs-marketplace-image] or [ASCS/SCS multi-SID template by using Managed Disks][sap-templates-3-tier-multisid-xscs-marketplace-image-md], введите значения для следующих параметров:
...
Чтобы настроить шаблон с несколькими ИД безопасности базы данных, в [шаблон базы данных с несколькими ИД безопасности][sap-templates-3-tier-multisid-db-marketplace-image] or [database multi-SID template by using Managed Disks][sap-templates-3-tier-multisid-db-marketplace-image-md], введите значения для следующих параметров:
...
Чтобы настроить этот шаблон с несколькими ИД безопасности серверов приложений в [шаблон с несколькими ИД безопасности серверов приложений][sap-templates-3-tier-multisid-apps-marketplace-image] or [application servers multi-SID template  by using Managed Disks][sap-templates-3-tier-multisid-apps-marketplace-image-md], введите значения для следующих параметров:
...
Имена виртуальных узлов pr1-ascs-vir и pr1-dbms-vir и связанные IP-адреса, используемые для управления кластером, создаются вместе с кластером. Сведения о том, как это сделать, см. в разделе [сбор узлов кластера в конфигурации кластера][sap-high-availability-infrastructure-wsfc-shared-disk-collect-cluster-config].
...
8. Введите имя второго узла кластера.
   ...
   Предупреждения о кворуме и дисках можно игнорировать. Можно задать кворум и более поздней версии, общих дисков, как описано в разделе [установить SIOS DataKeeper Cluster Edition для общего диска кластера SAP ASCS/SCS][sap-high-availability-infrastructure-wsfc-shared-disk-install-sios].
