...
* [Руководство по архитектуре: Кластеризация экземпляра SAP ASCS/SCS в отказоустойчивом кластере Windows с помощью общего диска кластера][sap-high-availability-guide-wsfc-shared-disk]
...
_**Рис. 1.** Настройка параметров Azure Resource Manager для SAP с высоким уровнем доступности_
  ...
  * **Виртуальные машины**:
    * виртуальные машины сервера приложений SAP: \<SAPSystemSID\>-di-\<номер\>;
    * виртуальные машины кластера ASCS/SCS: \<SAPSystemSID\>-ascs-\<номер\>;
    * кластер СУБД: \<SAPSystemSID\>-db-\<номер\>.
  ...
  * **Группы доступности** для:
    ...
    * виртуальные машины кластера SAP ASCS/SCS: \<SAPSystemSID\>-avset-ascs;
    * виртуальные машины кластера СУБД: \<SAPSystemSID\>-avset-db.
...
Для производственных систем SAP развертывайте виртуальные машины Azure с [подключением к корпоративной сети (][planning-guide-2.2] распределенными) с помощью VPN-шлюза Azure или Azure ExpressRoute.
...
Вы можете подготовить инфраструктуру и настроить SAP для поддержки сценария с несколькими идентификаторами безопасности. Например, вы можете добавить дополнительный экземпляр SAP ASCS/SCS в *имеющуюся* конфигурацию кластера. Дополнительные сведения см. [в статье Настройка дополнительного экземпляра SAP ASCS/SCS для существующей конфигурации кластера для создания конфигурации с несколькими идентификаторами безопасности SAP в Azure Resource Manager][sap-ha-multi-sid-guide].
...
- **Resource prefix** (Префикс ресурса).  Префикс ресурса используется в качестве префикса для всех ресурсов, созданных во время развертывания. Так как ресурсы принадлежат к нескольким системам SAP, префикс ресурса не является идентификатором безопасности одной системы SAP.  Длина префикса должна составлять от 3 до 6 символов.
...
- **SAP System Count** (Количество систем SAP). Выберите количество систем SAP, которые требуется установить в этом кластере.
...
- **Новая или имеющаяся подсеть**: Укажите, следует ли создать виртуальную сеть и подсеть или использовать имеющуюся подсеть. При наличии виртуальной сети, подключенной к локальной сети, выберите **ее**.
- **Subnet Id** (Идентификатор подсети). Чтобы развернуть виртуальную машину в имеющейся виртуальной сети с определенной подсетью, необходимо указать идентификатор этой определенной подсети. Идентификатор обычно выглядит следующим образом:
...
- **Размер системы SAP**: Задайте количество систем SAP, которые будет содержать новая система. Если вы не знаете, сколько систем SAP потребуется для системы, обратитесь к партнеру по технологиям или системному интегратору SAP.
- **Доступность системы**: Выберите **высокую доступность**.
- **Имя пользователя и пароль администратора**. Создайте пользователя, которого можно использовать для входа на компьютер.
...
Чтобы настроить шаблон серверов приложений с несколькими идентификаторами безопасности, в шаблоне серверы приложений с несколькими [идентификаторами безопасности][sap-templates-3-tier-multisid-apps-marketplace-image] или на [сервере приложений с использованием управляемых дисков][sap-templates-3-tier-multisid-apps-marketplace-image-md]введите значения для следующих параметров:
  ...
  -  **Sap System Id** (Идентификатор системы SAP). Введите идентификатор системы SAP, которую требуется установить. Идентификатор будет использоваться в качестве префикса для развертываемых ресурсов.
  -  **Тип ОС**: Выберите операционную систему виртуальных машин.
  -  **Размер системы SAP**: Количество систем SAP, которые предоставляет новая система. Если вы не знаете, сколько систем SAP потребуется для системы, обратитесь к партнеру по технологиям или системному интегратору SAP.
  -  **Доступность системы**: Выберите **высокую доступность**.
  -  **Имя пользователя и пароль администратора**. Создайте пользователя, которого можно использовать для входа на компьютер.
  -  **Subnet Id** (Идентификатор подсети). Введите идентификатор подсети, используемый во время развертывания шаблона ASCS/SCS, или идентификатор подсети, созданный в процессе развертывания шаблона ASCS/SCS.
...
Имена виртуальных узлов pr1-ascs-vir и pr1-dbms-vir и связанные IP-адреса, используемые для управления кластером, создаются вместе с кластером. Сведения о том, как это сделать, см. [в разделе Получение узлов кластера в конфигурации кластера][sap-high-availability-infrastructure-wsfc-shared-disk-collect-cluster-config].
...
3. Задайте для внутреннего балансировщика нагрузки **pr1-lb-dbms** IP-адрес имени виртуального узла экземпляра СУБД.
   ...
   ![Рис. 4. Настройка статических IP-адресов внутренней подсистемы балансировки нагрузки для экземпляра SAP ASCS/SCS][sap-ha-guide-figure-3003]

   _**Рис. 4.** Настройка статических IP-адресов внутренней подсистемы балансировки нагрузки для экземпляра SAP ASCS/SCS_
...
![Рис. 5. Правила балансировки нагрузки ASCS/SCS по умолчанию для внутренней подсистемы балансировки нагрузки Azure][sap-ha-guide-figure-3004]

_**Рис. 5.** Правила балансировки нагрузки ASCS/SCS по умолчанию для внутренней подсистемы балансировки нагрузки Azure_
...
2. Для всех правил балансировки нагрузки, относящихся к экземпляру SAP ASCS или SCS, измените следующие значения.
   ...
   * Внутренний порт
   ...
   ![Рисунок 6. Изменение правил балансировки нагрузки ASCS/SCS по умолчанию для внутренней подсистемы балансировки нагрузки Azure.][sap-ha-guide-figure-3005]

   _**Рисунок 6.** Изменение правил балансировки нагрузки ASCS/SCS по умолчанию для внутренней подсистемы балансировки нагрузки Azure_
...
![Рисунок 7. Добавление виртуальной машины в домен][sap-ha-guide-figure-3006]

_**Рисунок 7.** Добавление виртуальной машины в домен_
...
2. Настройте отказоустойчивый кластер с помощью диспетчера отказоустойчивости кластеров. В диспетчере отказоустойчивости кластеров щелкните **Создать кластер** и добавьте только имя первого кластера (узла A). Не добавляйте пока второй узел. Вы сделаете это позже.
   ...
   ![Рисунок 8. Добавление имени сервера или виртуальной машины первого узла кластера][sap-ha-guide-figure-3007]

   _**Рисунок 8.** Добавление имени сервера или виртуальной машины первого узла кластера_

3. Введите сетевое имя кластера (имя виртуального узла).

   ![Рисунок 9. Ввод имени кластера][sap-ha-guide-figure-3008]

   _**Рисунок 9.** Ввод имени кластера_

4. После создания кластера запустите проверочный тест.

   ![Рисунок 10. Проверка кластера][sap-ha-guide-figure-3009]

   _**Рисунок 10.** Проверка кластера_
   ...
   ![Рисунок 11. Предупреждение об отсутствии диска кворума][sap-ha-guide-figure-3010]

   _**Рисунок 11.** Предупреждение об отсутствии диска кворума_

   ![Рисунок 12. Для основного кластерного ресурса нужен новый IP-адрес][sap-ha-guide-figure-3011]

   _**Рисунок 12.** Для основного кластерного ресурса нужен новый IP-адрес_
...
5. Измените IP-адрес основной службы кластера. Кластер не может запуститься, пока не будет изменен IP-адрес основной службы кластера. Это связано с тем, что IP-адрес сервера указывает на один из узлов виртуальных машин. Его можно изменить на странице **Свойства** ресурса IP-адреса основной службы кластера.
   ...
   ![Рисунок 13. Использование окна "Свойства" для изменения IP-адреса][sap-ha-guide-figure-3012]

   _**Рисунок 13.** Использование окна **Свойства** для изменения IP-адреса_

   ![Рисунок 14. Назначение IP-адреса, зарезервированного для кластера][sap-ha-guide-figure-3013]

   _**Рисунок 14.** Назначение IP-адреса, зарезервированного для кластера_

6. Подключите имя виртуального узла кластера к сети.

   ![Рисунок 15. Основная служба кластера запущена и работает с правильным IP-адресом][sap-ha-guide-figure-3014]

   _**Рисунок 15.** Основная служба кластера запущена и работает с правильным IP-адресом_
...
7. Добавьте второй узел кластера.
   ...
   _**Рисунок 16.** Добавление второго узла кластера_

8. Введите имя второго узла кластера.

   ![Рисунок 17. Ввод имени второго узла кластера][sap-ha-guide-figure-3016]

   _**Рисунок 17.** Ввод имени второго узла кластера_
   ...
   ![Рисунок 18. Этот флажок не должен быть установлен][sap-ha-guide-figure-3017]

   _**Рисунок 18.** Этот флажок *не должен* быть установлен_

   Предупреждения о кворуме и дисках можно игнорировать. Вы задаете кворум и предоставляете общий доступ к диску позже, как описано в разделе [Install SIOS Me Cluster Edition for a Clustering Disk-The кластера SAP ASCS/SCS][sap-high-availability-infrastructure-wsfc-shared-disk-install-sios].

   ![Рисунок 19. Предупреждение о диске кворума можно игнорировать][sap-ha-guide-figure-3018]

   _**Рисунок 19.** Предупреждение о диске кворума можно игнорировать_
...
2. Добавьте объект имени кластера.
   ...
   ![Рисунок 20. Назначение разрешений для общего ресурса для объекта имени кластера][sap-ha-guide-figure-3019]

   _**Рисунок 20.** Назначение разрешений для общего ресурса для объекта имени кластера_
...
3. Чтобы добавить объект имени кластера в список, нажмите кнопку **Добавить**. Измените фильтр для добавления дополнительных объектов-компьютеров к показанным на рис. 22.
   ...
   ![Рисунок 21. Изменение типов объектов для добавления компьютеров][sap-ha-guide-figure-3020]

   _**Рисунок 21.** Изменение **типов объектов** для добавления компьютеров_

   ![Рисунок 22. Установка флажка "Компьютеры"][sap-ha-guide-figure-3021]

   _**Рисунок 22.** Установка флажка **Компьютеры**_
...
5. Откройте вкладку **Безопасность** общего ресурса и определите более детальные разрешения для объекта имени кластера.
   ...
   ![Рисунок 23. Настройка атрибутов безопасности для объекта имени кластера в кворуме на основе файлового ресурса][sap-ha-guide-figure-3022]

   _**Рисунок 23.** Настройка атрибутов безопасности для объекта имени кластера в кворуме на основе файлового ресурса_
...
1. Откройте мастер настройки параметров кворума.
   ...
   ![Рисунок 24. Запуск мастера настройки кворума кластера][sap-ha-guide-figure-3023]

   _**Рисунок 24.** Запуск мастера настройки кворума кластера_

2. На странице **Выбор параметра конфигурации кворума** установите флажок **Выбрать свидетель кворума**.

   ![Рисунок 25. Выбор конфигураций кворума][sap-ha-guide-figure-3024]

   _**Рисунок 25.** Выбор конфигураций кворума_

3. На странице **Выбрать свидетель кворума** установите флажок **Настроить файловый ресурс-свидетель**.

   ![Рисунок 26. Выбор файлового ресурса-свидетеля][sap-ha-guide-figure-3025]

   _**Рисунок 26.** Выбор файлового ресурса-свидетеля_
...
4. Введите путь UNC к файловому ресурсу (в нашем примере это \\domcontr-0\FSW). Чтобы просмотреть список изменений, которые можно внести, нажмите кнопку **Далее**.
   ...
   ![Рисунок 27. Определение расположения файлового ресурса-свидетеля][sap-ha-guide-figure-3026]

   _**Рисунок 27.** Определение расположения файлового ресурса-свидетеля_

5. Выберите нужные изменения и нажмите кнопку **Далее**. Конфигурацию кластера необходимо перенастроить, как показано на рис. 28:  

   ![Рисунок 28. Подтверждение изменения настройки кластера][sap-ha-guide-figure-3027]

   _**Рисунок 28.** Подтверждение изменения настройки кластера_
...
- Первый способ — с помощью мастера добавления ролей и компонентов в Windows, как показано на рис. 29:
  ...
  ![Рисунок 29. Установка .NET Framework 3.5 с помощью мастера добавления ролей и компонентов][sap-ha-guide-figure-3028]

  _**Рисунок 29.** Установка .NET Framework 3.5 с помощью мастера добавления ролей и компонентов_

  ![Рисунок 30. Индикатор выполнения установки .NET Framework 3.5 с помощью мастера добавления ролей и компонентов][sap-ha-guide-figure-3029]

  _**Рисунок 30.** Индикатор выполнения установки .NET Framework 3.5 с помощью мастера добавления ролей и компонентов_
...
1. Установите программное обеспечение SIOS на обоих узлах кластера.
   ...
   ![Рисунок 31. Первая страница установки SIOS DataKeeper][sap-ha-guide-figure-3031]

   _**Рисунок 31.** Первая страница установки SIOS DataKeeper_

2. В диалоговом окне выберите **Да**.

   ![Рисунок 32. Предупреждение DataKeeper об отключении службы][sap-ha-guide-figure-3032]

   _**Рисунок 32.** Предупреждение DataKeeper об отключении службы_

3. В диалоговом окне мы рекомендуем установить флажок **Domain or Server account** (Учетная запись домена или сервера).

   ![Рисунок 33. Выбор пользователя для SIOS DataKeeper][sap-ha-guide-figure-3033]

   _**Рисунок 33.** Выбор пользователя для SIOS DataKeeper_

4. Укажите имя учетной записи пользователя домена и пароль, созданные для SIOS DataKeeper.

   ![Рисунок 34. Ввод имени пользователя и пароля для установки SIOS DataKeeper][sap-ha-guide-figure-3034]

   _**Рисунок 34.** Ввод имени пользователя и пароля для установки SIOS DataKeeper_

5. Установите лицензионный ключ для своего экземпляра SIOS DataKeeper, как показано на рис. 35.

   ![Рисунок 35. Ввод ключа лицензии на использование SIOS DataKeeper][sap-ha-guide-figure-3035]

   _**Рисунок 35:** Ввод ключа лицензии на использование SIOS DataKeeper_
...
1. Запустите инструмент управления и настройки DataKeeper, а затем щелкните **Подключение к серверу**.
   ...
   ![Рисунок 36. Инструмент управления и настройки SIOS DataKeeper][sap-ha-guide-figure-3036]

   _**Рисунок 36.** Инструмент управления и настройки SIOS DataKeeper_
...
2. Введите имя или TCP/IP-адрес первого узла, к которому должен подключиться инструмент управления и настройки, а на втором шаге — для второго узла.
   ...
   ![Рисунок 37. Ввод имени или TCP/IP-адреса первого узла, к которому должен подключаться инструмент управления и настройки, а также второго узла на втором шаге][sap-ha-guide-figure-3037]

   _**Рисунок 37.** Ввод имени или TCP/IP-адреса первого узла, к которому должен подключаться инструмент управления и настройки, а также второго узла на втором шаге_

3. Создайте задание репликации между двумя узлами.

   ![Рисунок 38. Создание задания репликации][sap-ha-guide-figure-3038]

   _**Рисунок 38.** Создание задания репликации_

   Мастер поможет выполнить эту процедуру.

4. Определите имя задания репликации.

   ![Рисунок 39. Определение имени для задания репликации][sap-ha-guide-figure-3039]

   _**Рисунок 39.** Определение имени для задания репликации_

   ![Рисунок 40. Определение базовых данных для узла, который должен быть текущим исходным узлом][sap-ha-guide-figure-3040]

   _**Рисунок 40.** Определение базовых данных для узла, который должен быть текущим исходным узлом_

5. Определите имя, TCP/IP-адрес и дисковый том целевого узла.

   ![Рисунок 41. Определение имени, TCP/IP-адреса и дискового тома текущего целевого узла][sap-ha-guide-figure-3041]

   _**Рисунок 41.** Определение имени, TCP/IP-адреса и дискового тома текущего целевого узла_
...
7. Кроме того, нужно проверить параметр, который позволяет включить синхронный или асинхронный режим репликации. Для защиты конфигураций SAP ASCS/SCS требуется синхронная репликация.  
   ...
   ![Рисунок 42. Определение сведений о репликации][sap-ha-guide-figure-3042]

   _**Рисунок 42.** Определение сведений о репликации_
...
8. Определите, следует ли том, реплицируемый с помощью задания репликации, представить в конфигурации кластера WSFC как общий диск. Для конфигурации SAP ASCS/SCS выберите **Да**, чтобы кластер Windows воспринимал реплицируемый том как общий диск, который можно использовать в качестве тома кластера.
   ...
   ![Рисунок 43. Подтверждение согласия на выбор реплицируемого тома в качестве тома кластера][sap-ha-guide-figure-3043]

   _**Рисунок 43.** **Подтверждение** согласия на выбор реплицируемого тома в качестве тома кластера_

   После создания тома инструмент управления и настройки DataKeeper отобразят задание репликации как активное.

   ![Рисунок 44. Синхронное зеркальное отображение DataKeeper для общего диска SAP ASCS/SCS активно][sap-ha-guide-figure-3044]

   _**Рисунок 44.** Синхронное зеркальное отображение DataKeeper для общего диска SAP ASCS/SCS активно_

   Теперь диск отображается в диспетчере отказоустойчивости кластеров как диск DataKeeper, как показано на рис. 45:

   ![Рисунок 45. Реплицируемый диск DataKeeper отображается в диспетчере отказоустойчивости кластеров][sap-ha-guide-figure-3045]

   _**Рисунок 45.** Реплицируемый диск DataKeeper отображается в диспетчере отказоустойчивости кластеров_

## <a name="next-steps"></a>Следующие шаги

* [Установка SAP NetWeaver высокого уровня доступности с помощью отказоустойчивого кластера Windows и общего диска для экземпляра SAP ASCS/SCS][sap-high-availability-installation-wsfc-shared-disk]
