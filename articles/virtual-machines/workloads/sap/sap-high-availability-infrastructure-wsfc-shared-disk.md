...
_**Рис. 1.** Настройка параметров Azure Resource Manager для SAP с высоким уровнем доступности_
  ...
  * **Виртуальные машины**:
    ...
    * Виртуальные машины кластера ASCS/SCS: \<SAPSystemSID\> -ASCS-\<Number\>
    * Кластер СУБД: \<SAPSystemSID\> -DB-\<Number\>
  ...
  * **Учетные записи хранения Azure (только для неуправляемых дисков)**:
  ...
  * **Внутренний балансировщик нагрузки Azure**:
    * Со всеми портами для экземпляра ASCS/SCS и IP-адресом \<SAPSystemSID\> — Балансировка нагрузки-ASCS
    * Со всеми портами для SQL Server DBMS и IP-address \<SAPSystemSID\> -фунтов-DB

  * **Группа безопасности сети**: \<SAPSystemSID\> -NSG-ASCS-0  
    * С открытым внешним протокол удаленного рабочего столаным портом (RDP) для \<SAPSystemSID\> виртуальной машины-ASCS-0;
...
Для производственных систем SAP развертывайте виртуальные машины Azure с подключением к корпоративной сети с помощью VPN-шлюза Azure или Azure ExpressRoute.
...
- **Идентификатор подсети**. Если вы хотите развернуть виртуальную машину в существующей виртуальной сети с определенной подсетью, присвойте ей имя ID этой конкретной подсети. Идентификатор обычно выглядит следующим образом:

  /Subscriptions/ \<subscription id\> /ResourceGroups/ \<resource group name\> /провидерс/Микрософт.Нетворк/виртуалнетворкс/ \<virtual network name\> /субнетс/\<subnet name\>
...
#### <a name="sap-ascsscs-ports"></a><a name="0f3ee255-b31e-4b8a-a95a-d9ed6200468b"></a>Порты SAP ASCS/SCS
...
3. Задайте для внутреннего балансировщика нагрузки **pr1-lb-dbms** IP-адрес имени виртуального узла экземпляра СУБД.
   ...
   _**Рис. 4.** Настройка статических IP-адресов для внутренней подсистемы балансировки нагрузки для экземпляра SAP ASCS/SCS_
...
| Внутреннее сообщение ABAP/ *lbrule3900* |39\<InstanceNumber\> |3900 |
...
| Служба SAP Start Service ASCS HTTPS/ *Lbrule50014* |5\<InstanceNumber\>14 |50014 |
| Репликация в очередь/ *Lbrule50016* |5\<InstanceNumber\>16 |50016 |
| Служба SAP Start Service ERS HTTP *Lbrule51013* |5\<InstanceNumber\>13 |51013 |
| Служба SAP Start Service ERS HTTP *Lbrule51014* |5\<InstanceNumber\>14 |51014 |
| Служба удаленного управления Windows (WinRM) *Lbrule5985* | |5985 |
| Общая папка *Lbrule445* | |445 |
...
1. В портал Azure выберите правила балансировки нагрузки ** \<SID\> подсистемы**балансировки нагрузки ASCS  >  **Load Balancing Rules**.
...
## <a name="set-up-a-windows-server-failover-cluster-for-an-sap-ascsscs-instance"></a><a name="0d67f090-7928-43e0-8772-5ccbf8f59aab"></a> Установка отказоустойчивого кластера Windows Server для экземпляра SAP ASCS/SCS
...
4. После создания кластера запустите проверочный тест.
   ...
   _**Рис. 10.** Выполнение проверки кластера_
...
5. Измените IP-адрес основной службы кластера. Кластер не может запуститься, пока не будет изменен IP-адрес основной службы кластера. Это связано с тем, что IP-адрес сервера указывает на один из узлов виртуальных машин. Его можно изменить на странице **Свойства** ресурса IP-адреса основной службы кластера.
   ...
   _**Рис. 13.** В диалоговом окне **Свойства** измените IP-адрес._

   ![Рис. 14. Назначение IP-адреса, зарезервированного для кластера][sap-ha-guide-figure-3013]

   _**Рис. 14.** Назначьте IP-адрес, зарезервированный для кластера_
...
6. Подключите имя виртуального узла кластера к сети.
   ...
   _**Рис. 15.** Основная служба кластера запущена и работает с правильным IP-адресом._
...
8. Введите имя второго узла кластера.
   ...
   _**Рис. 17.** Введите имя второго узла кластера_
   ...
   _**Рис. 19.** Игнорировать предупреждения о кворуме диска_
...
3. Чтобы добавить объект имени кластера в список, нажмите кнопку **Добавить**. Измените фильтр для добавления дополнительных объектов-компьютеров к показанным на рис. 22.
   ...
   _**Рис. 21.** Изменение **типов объектов** для включения компьютеров_
...
5. Откройте вкладку **Безопасность** общего ресурса и определите более детальные разрешения для объекта имени кластера.
   ...
   _**Рис. 23.** Задание атрибутов безопасности для объекта имени кластера в кворуме файлового ресурса_
...
2. На странице **Выбор параметра конфигурации кворума** установите флажок **Выбрать свидетель кворума**.
   ...
   _**Рис. 25.** Конфигурации кворума, которые можно выбрать_
...
### <a name="add-net-framework-35"></a><a name="1c2788c3-3648-4e82-9e0d-e058e475e2a3"></a>Добавление .NET Framework 3.5
...
- Первый способ — с помощью мастера добавления ролей и компонентов в Windows, как показано на рис. 29:
  ...
  _**Рис. 30.** Индикатор хода выполнения установки при установке .NET Framework 3,5 с помощью мастера добавления ролей и компонентов_
...
1. Установите программное обеспечение SIOS на обоих узлах кластера.
   ...
   _**Рис. 31.** Первая страница установки SIOS_

2. В диалоговом окне выберите **Да**.

   ![Рис. 32. DataKeeper информирует об отключении службы][sap-ha-guide-figure-3032]

   _**Рис. 32.** При этом будет отображаться сообщение о том, что служба отключена._
...
3. В диалоговом окне мы рекомендуем установить флажок **Domain or Server account** (Учетная запись домена или сервера).
   ...
   _**Рис. 33.** Выбор пользователя для SIOS_
...
5. Установите лицензионный ключ для своего экземпляра SIOS DataKeeper, как показано на рис. 35.
   ...
   _**Рис. 35.** Введите ключ лицензии SIOS._
...
2. Введите имя или TCP/IP-адрес первого узла, к которому должен подключиться инструмент управления и настройки, а на втором шаге — для второго узла.
   ...
   _**Рис. 37.** Вставьте имя или TCP/IP-адрес первого узла, к которому должно подключиться средство управления и настройки, а на втором шаге — второй узел._
...
4. Определите имя задания репликации.
   ...
   _**Рис. 40.** Определите базовые данные для узла, который должен быть текущим исходным узлом_
...
5. Определите имя, TCP/IP-адрес и дисковый том целевого узла.
   ...
   _**Рис. 41.** Укажите имя, TCP/IP-адрес и дисковый том для текущего целевого узла_
...
8. Определите, следует ли том, реплицируемый с помощью задания репликации, представить в конфигурации кластера WSFC как общий диск. Для конфигурации SAP ASCS/SCS выберите **Да**, чтобы кластер Windows воспринимал реплицируемый том как общий диск, который можно использовать в качестве тома кластера.
   ...
   _**Рис. 43.** Выберите **Да** , чтобы задать реплицированный том как том кластера_
   ...
   _**Рис. 44.** Синхронное зеркальное отображение данных для общего диска SAP ASCS/SCS активно_
