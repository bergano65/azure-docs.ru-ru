---
title: Инфраструктура Azure для SAP ASCS/SCS с wsFC&общим диском Документы Майкрософт
...
_**Рис. 1.** Настройка параметров Azure Resource Manager для SAP с высоким уровнем доступности_
  ...
  * **Учетные записи хранения Azure (только неуправляемые диски)**:
  ...
  * **Группа сетевой безопасности** \<\>: SAPSystemSID -nsg-ascs-0  
...
Для производства систем SAP развертывайте виртуальные машины Azure с корпоративным подключением к сети с помощью Azure VPN Gateway или Azure ExpressRoute.
...
- **New Or Existing Subnet** (Новая или имеющаяся подсеть). Определяет, следует ли создать виртуальную сеть и подсеть или использовать имеющуюся подсеть. Если у вас уже есть виртуальная сеть, подключенная к вашей предварительной сети, выберите **существующую.**
- **Идентификатор подсети**.Чтобы развернуть виртуальную машину в имеющуюся виртуальную сеть с определенной подсетью, необходимо указать идентификатор этой определенной подсети. Идентификатор обычно выглядит следующим образом:
...
- **Os Type**: Выберите операционную систему виртуальных машин.
...
2. Выберите параметры в зависимости от типа сети. Дополнительные сведения см. в следующих ресурсах:
   ...
   * Для развертываний VM, которые изолированы в Azure: Развертывание дополнительной виртуальной машины в том же экземпляре виртуальной сети, который служит DNS-сервером. Добавьте IP-адреса этих виртуальных машин Azure, настроенных для запуска службы DNS.
...
3. Задайте для внутреннего балансировщика нагрузки **pr1-lb-dbms** IP-адрес имени виртуального узла экземпляра СУБД.
   ...
   _**Рисунок 4:** Установите статические IP-адреса для внутреннего баланса нагрузки для экземпляра SAP ASCS/SCS_
...
| --- | --- | --- |
| Enqueue сервер / *lbrule3200* |32\<номер_экземпляра\> |3200 |
...
| SAP запуск службы ASCS HTTPS / *Lbrule50014* |5\<номер_экземпляра\>14 |50014 |
| Служба постановки в очередь для репликации, *Lbrule50016* |5\<номер_экземпляра\>16 |50016 |
| SAP запуск службы ERS HTTP *Lbrule51013* |5\<номер_экземпляра\>13 |51013 |
| SAP запуск службы ERS HTTP *Lbrule51014* |5\<номер_экземпляра\>14 |51014 |
| Служба удаленного управления Windows (WinRM) *Lbrule5985* | |5985 |
| Доля файлов *Lbrule445* | |445 |
...
| SAP запуск службы SCS HTTPS / *Lbrule50114* |5\<номер_экземпляра\>14 |50114 |
...
_**Рисунок 5:** Правила балансировки нагрузки ASCS/SCS по умолчанию для баланса внутренней нагрузки Azure_
...
1. На портале Azure выберите >  ** \<правила\>балансировки балансировки нагрузки SID-lb-ascs.****Load Balancing Rules**
2. Для всех правил балансировки нагрузки, относящихся к экземпляру SAP ASCS или SCS, измените следующие значения.
   ...
   * Внутренний порт
   ...
   _**Рисунок 6:** Изменение правил балансировки по умолчанию ASCS/SCS для баланса внутренней нагрузки Azure_
...
4. После создания кластера запустите проверочный тест.
   ...
   _**Рисунок 10:** Выполнить проверку кластерной проверки_
...
5. Измените IP-адрес основной службы кластера. Кластер не может запуститься, пока не будет изменен IP-адрес основной службы кластера. Это связано с тем, что IP-адрес сервера указывает на один из узлов виртуальных машин. Его можно изменить на странице **Свойства** ресурса IP-адреса основной службы кластера.
   ...
   _**Рисунок 13:** В диалоговом окне **Свойств** измените IP-адрес_

   ![Рис. 14. Назначение IP-адреса, зарезервированного для кластера][sap-ha-guide-figure-3013]

   _**Рисунок 14:** Назначить IP-адрес, зарезервированный для кластера_
...
6. Подключите имя виртуального узла кластера к сети.
   ...
   _**Рисунок 15:** Основная служба кластера запущена и запущена с правильным IP-адресом_
...
8. Введите имя второго узла кластера.
   ...
   _**Рисунок 17:** Введите имя узла второго кластерного узла_
   ...
   _**Рисунок 19:** Игнорировать предупреждения о дисковом кворуме_
...
3. Чтобы добавить объект имени кластера в список, нажмите кнопку **Добавить**. Измените фильтр для добавления дополнительных объектов-компьютеров к показанным на рис. 22.
   ...
   _**Рисунок 21:** Изменение **типов объектов** для включения компьютеров_
...
5. Откройте вкладку **Безопасность** общего ресурса и определите более детальные разрешения для объекта имени кластера.
   ...
   _**Рисунок 23:** Установите атрибуты безопасности для объекта кластерного имени в кворуме общего файла_
...
1. Откройте мастер настройки параметров кворума.
   ...
   _**Рисунок 24:** Запустите настройку кластера Кворум Настройка мастера_
...
2. На странице **Выбор параметра конфигурации кворума** установите флажок **Выбрать свидетель кворума**.
   ...
   _**Рисунок 25:** Конфигурации кворума, которые вы можете выбрать из_
...
4. Введите путь UNC к файловому ресурсу (в нашем примере это \\domcontr-0\FSW). Чтобы просмотреть список изменений, которые можно внести, нажмите кнопку **Далее**.
   ...
   _**Рисунок 27:** Определение местоположения общего файла для общего участия свидетеля_
...
5. Выберите нужные изменения и нажмите кнопку **Далее**. Конфигурацию кластера необходимо перенастроить, как показано на рис. 28:  
   ...
   _**Рисунок 28:** Подтверждение того, что вы перенастроили кластер_
...
- Первый способ — с помощью мастера добавления ролей и компонентов в Windows, как показано на рис. 29:
  ...
  _**Рисунок 30:** Панель выполнения установки при установке .NET Framework 3.5 с помощью Мастера добавления ролей и функций_
...
2. В диалоговом окне выберите **Да**.
   ...
   _**Рисунок 32:** DataKeeper сообщает вам, что служба будет отключена_
...
5. Установите лицензионный ключ для своего экземпляра SIOS DataKeeper, как показано на рис. 35.
   ...
   _**Рисунок 35:** Введите клавишу лицензии SIOS DataKeeper_
...
2. Введите имя или TCP/IP-адрес первого узла, к которому должен подключиться инструмент управления и настройки, а на втором шаге — для второго узла.
   ...
   _**Рисунок 37:** Вставьте имя или адрес TCP/IP первого узла, к которому должен подключиться инструмент управления и конфигурации, а на втором этапе — второй узлы._
...
4. Определите имя задания репликации.
   ...
   _**Рисунок 39:** Определение названия задания репликации_

   ![Рис. 40. Определение базовых данных для узла, который должен быть текущим исходным узлом][sap-ha-guide-figure-3040]

   _**Рисунок 40:** Определите базовые данные для узла, который должен быть текущим узлам источника_
...
5. Определите имя, TCP/IP-адрес и дисковый том целевого узла.
   ...
   _**Рисунок 41:** Определите имя, адрес TCP/IP и объем диска текущего целевого узла_
...
8. Определите, следует ли том, реплицируемый с помощью задания репликации, представить в конфигурации кластера WSFC как общий диск. Для конфигурации SAP ASCS/SCS выберите **Да**, чтобы кластер Windows воспринимал реплицируемый том как общий диск, который можно использовать в качестве тома кластера.
   ...
   _**Рисунок 43:** Выберите **«Да»,** чтобы установить реплицированный том как кластерный том_
   ...
   _**Рисунок 44:** Синхронное зеркальное отражение DataKeeper для диска sAP ASCS/SCS является активным_
   ...
   _**Рисунок 45:** Менеджер кластера Failover показывает диск, реплицированный DataKeeper_
