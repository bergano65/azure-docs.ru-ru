...
Группы ресурсов в Azure Resource Manager можно использовать для управления всеми ресурсами приложения в подписке Azure. В рамках интегрированного подхода в группе ресурсов все ресурсы имеют одинаковый цикл жизни. Например, все они создаются одновременно и удаляются тоже одновременно. Дополнительные сведения о [группах ресурсов](../../../azure-resource-manager/management/overview.md#resource-groups).

### <a name="azure-internal-load-balancer-dependency-on-the-azure-resource-group"></a><a name="3e85fbe0-84b1-4892-87af-d9b65ff91860"></a>Зависимость внутренней подсистемы балансировки нагрузки Azure от группы ресурсов Azure
...
> [!IMPORTANT]
> Для установки приложений SAP настоятельно рекомендуется использовать модель развертывания с помощью Azure Resource Manager. Она предлагает преимущества, недоступные в классической модели развертывания. Дополнительные сведения о [моделях развертывания][virtual-machines-azure-resource-manager-architecture-benefits-arm]Azure.   
>
>

## <a name="windows-server-failover-clustering"></a><a name="8ecf3ba0-67c0-4495-9c14-feec1a2255b7"></a>Отказоустойчивая кластеризация Windows Server
...
## <a name="windows-server-failover-clustering-on-premises"></a><a name="fdfee875-6e66-483a-a343-14bbaee33275"></a>Отказоустойчивая кластеризация Windows Server в локальной среде
...
## <a name="windows-server-failover-clustering-in-azure"></a><a name="2ddba413-a7f5-4e4e-9a51-87908879c10a"></a>Отказоустойчивая кластеризация Windows Server в Azure
...
### <a name="shared-disk-in-azure-with-sios-datakeeper"></a><a name="1a464091-922b-48d7-9d08-7cecf757f341"></a>Общий диск в Azure с помощью SIOS
...
## <a name="sap-netweaver-high-availability-in-azure-infrastructure-as-a-service-iaas"></a><a name="2e3fec50-241e-441b-8708-0b1864f66dfa"></a>Высокий уровень доступности SAP NetWeaver в инфраструктуре "инфраструктура как услуга" (IaaS)
...
### <a name="high-availability-sap-application-server"></a><a name="93faa747-907e-440a-b00a-1ae0a89b1c0e"></a>Сервер приложений SAP с высоким уровнем доступности
...
### <a name="high-availability-sap-ascsscs-instance"></a><a name="f559c285-ee68-4eec-add1-f60fe7b978db"></a>Высокий уровень доступности экземпляра SAP ASCS/SCS
...
_**Рис. 5.** Высокий уровень доступности экземпляра SAP ASCS/SCS_

#### <a name="sap-ascsscs-instance-high-availability-with-windows-server-failover-clustering-in-azure"></a><a name="b5b1fd0b-1db4-4d49-9162-de07a0132a51"></a>Высокий уровень доступности экземпляра SAP ASCS/SCS с помощью отказоустойчивой кластеризации Windows Server в Azure
...
### <a name="high-availability-dbms-instance"></a><a name="ddd878a0-9c2f-4b8e-8968-26ce60be1027"></a>Экземпляр СУБД с высоким уровнем доступности
...
_**Рис. 9.** Шаблон 2 архитектуры SAP с высоким уровнем доступности с выделенным кластером для ASCS/SCS и выделенным кластером для СУБД_
...
_**Рис. 10.** Шаблон 3 архитектуры SAP с высоким уровнем доступности с выделенным кластером для различных экземпляров ASCS/SCS_
...
Для производственных систем SAP развертывайте виртуальные машины Azure с подключением к корпоративной сети с помощью VPN-подключения типа "сеть — сеть" или Azure ExpressRoute.
...
Чтобы настроить [шаблон ASCS/SCS с несколькими ИД безопасности][sap-templates-3-tier-multisid-xscs-marketplace-image] или [шаблон ASCS/SCS с несколькими ИД безопасности, использующий управляемые диски][sap-templates-3-tier-multisid-xscs-marketplace-image-md], введите в нем следующие значения параметров:
  ...
  -  **Число систем SAP**. Выберите количество систем SAP, которое требуется установить в этом кластере.
  ...
  -  **Новая или существующая подсеть**. Определите, следует ли создать виртуальную сеть и подсеть или использовать имеющуюся подсеть. Если у вас уже есть виртуальная сеть, подключенная к локальной сети, выберите пункт **существующие**.
  -  **Идентификатор подсети**. Если вы хотите развернуть виртуальную машину в существующей виртуальной сети с определенной подсетью, присвойте ей имя ID этой конкретной подсети. Идентификатор обычно выглядит следующим образом: /subscriptions/<*идентификатор_подписки*>/resourceGroups/<*имя_группы_ресурсов*>/providers/Microsoft.Network/virtualNetworks/<*имя_виртуальной_сети*>/subnets/<*имя_подсети*>
...
- **Идентификатор системы SAP**. Введите идентификатор системы SAP, которую вы хотите установить. Идентификатор будет использоваться в качестве префикса для развертываемых ресурсов.
- **Тип ОС**. Выберите операционную систему виртуальных машин.
...
- **Доступность системы**. Выберите **Ha**.
- **Имя пользователя и пароль администратора**. Создайте пользователя, которого можно использовать для входа на компьютер.
- **Идентификатор подсети**. Введите идентификатор подсети, который использовался при развертывании шаблона ASCS/SCS, или идентификатор подсети, которая была создана как часть развертывания шаблона ASCS/SCS.
...
1. В портал Azure выберите **Группа** > ресурсов**Параметры** > **сетевой карты** > **IP-адрес**.
2. В колонке **IP-адреса** в разделе **Назначение** выберите **Статический**. В поле **IP-адрес** введите нужный IP-адрес.
   ...
   _**Рис. 13.** Настройка статических IP-адресов для сетевой карты каждой виртуальной машины_
...
| --- | --- | --- |
| Сервер постановки в очередь, *lbrule3200* |32<*инстанценумбер*> |3200 |
| Сервер сообщений ABAP/ *lbrule3600* |36<*инстанценумбер*> |3600 |
| Внутреннее сообщение ABAP/ *lbrule3900* |39<*инстанценумбер*> |3900 |
| Сервер сообщений (HTTP), *Lbrule8100* |81<*инстанценумбер*> |8100 |
| Служба SAP Start Service ASCS HTTP/ *Lbrule50013* |5<*InstanceNumber*>13 |50013 |
| Служба запуска SAP ASCS (HTTPS), *Lbrule50014* |5<*InstanceNumber*>14 |50014 |
| Репликация в очередь/ *Lbrule50016* |5<*InstanceNumber*>16 |50016 |
...
_**Таблица 1.** Номера портов для экземпляров SAP NetWeaver ABAP ASCS_
...
| --- | --- | --- |
| Сервер постановки в очередь/ *lbrule3201* |32<*инстанценумбер*> |3201 |
| Сервер шлюза, *lbrule3301* |33<*инстанценумбер*> |3301 |
| Сервер сообщений Java/ *lbrule3900* |39<*инстанценумбер*> |3901 |
| Сервер сообщений (HTTP), *Lbrule8101* |81<*инстанценумбер*> |8101 |
| Служба SAP Start Service SCS HTTP/ *Lbrule50113* |5<*InstanceNumber*>13 |50113 |
| Служба запуска SAP SCS (HTTPS), *Lbrule50114* |5<*InstanceNumber*>14 |50114 |
| Репликация в очередь/ *Lbrule50116* |5<*InstanceNumber*>16 |50116 |
| Служба SAP Start Service ERS HTTP *Lbrule51113* |5<*InstanceNumber*>13 |51113 |
| Служба SAP Start Service ERS HTTP *Lbrule51114* |5<*InstanceNumber*>14 |51114 |
| Win RM, *Lbrule5985* | |5985 |
| Общий доступ к файлам, *Lbrule445* | |445 |
...
Установите IP-адрес балансировщика нагрузки **PR1-Load-СУБД** в IP-адрес имени виртуального узла экземпляра СУБД.
...
### <a name="set-up-a-windows-server-failover-clustering-cluster-for-an-sap-ascsscs-instance"></a><a name="0d67f090-7928-43e0-8772-5ccbf8f59aab"></a>Настройка отказоустойчивого кластера Windows Server для экземпляра SAP ASCS/SCS
...
4. После создания кластера запустите проверочный тест.
   ...
   _**Рис. 22.** Основному ресурсу кластера требуется новый IP-адрес_
...
8. Введите имя второго узла кластера.
   ...
   > [!IMPORTANT]
   > Убедитесь, что флажок **Добавить все подходящие хранилища в кластер** **не** установлен.  
   >
   >

   ![Рис. 28. Не следует устанавливать указанный флажок!][sap-ha-guide-figure-3017]

   _**Рис. 28.** **Не** выбирайте флажок_
...
##### <a name="create-a-file-share"></a><a name="06260b30-d697-4c4d-b1c9-d22c0bd64855"></a>Создание общей папки
...
### <a name="install-sios-datakeeper-cluster-edition-for-the-sap-ascsscs-cluster-share-disk"></a><a name="5c8e5482-841e-45e1-a89d-a05c0907c868"></a>Установка SIOS Clustering Edition для общего диска кластера SAP ASCS/SCS
...
1. В диспетчере DNS Windows создайте запись DNS для имени виртуального узла экземпляра ASCS/SCS.
   ...
   > [!IMPORTANT]
   > IP-адрес, назначаемый имени виртуального узла экземпляра ASCS/SCS, должен совпадать с IP-адресом, назначенным Azure Load Balancer (**<*SID*>-фунтов-ASCS**).  
   >
   >

   IP-адрес виртуального имени узла SAP ASCS/SCS (**PR1-ASCS-SAP**) совпадает с IP-адресом Azure Load Balancer (**PR1-фунтов-ASCS**).
...
2. Чтобы определить IP-адрес, назначенный имени виртуального узла, выберите > **домен** **диспетчера DNS**.
...
1. Проверьте текущую настройку **ProbePort** , выполнив следующую команду PowerShell. Выполните ее на одной из виртуальных машин в конфигурации кластера.
...
2. Укажите порт пробы. Номер порта пробы по умолчанию — **0**. В нашем примере мы используем порт пробы **62000**.
   ...
   После перевода роли кластера **SAP <*SID* > ** в режим «в сети» убедитесь, что для **ProbePort** задано новое значение.
...
#### <a name="open-the-windows-firewall-probe-port"></a><a name="4498c707-86c0-4cde-9c69-058a7ab8c3ac"></a>Открытие порта пробы брандмауэра Windows
...
Для **ProbePort** установлено значение **62000**. Теперь вы можете получить доступ к общей папке ** \\\ascsha-clsap\sapmnt** с других узлов, например из **ascsha-DBA**.
...
_**Рис. 60.** Изменение типа службы для экземпляра SAP ERS на "Отложенная автоматическая"_
...
### <a name="sap-ascsscs-instance-is-running-on-cluster-node-a"></a><a name="65fdef0f-9f94-41f9-b314-ea45bbfea445"></a>Экземпляр SAP ASCS/SCS работает на узле A кластера

Кластерная группа **SAP PR1** работает на узле A кластера. Например, для **PR1-ASCS-0**. Назначьте узлу A кластера общий диск S:, принадлежащий кластерной группе **SAP PR1** и используемый экземпляром ASCS/SCS.
...
В средстве управления и настройки средства сохранения данных SIOS можно увидеть, что данные общего диска синхронно реплицируются с исходного диска с тома S на узле A кластера на целевой том S на узле B кластера. Например, он реплицируется из **PR1-ASCS-0 [10.0.0.40]** в **PR1-ASCS-1 [10.0.0.41]**.
...
1. Выберите один из этих вариантов, чтобы инициировать отработку отказа> группы кластеров SAP <*SID* с узла a на узел B кластера.
...
4. перезапустив узел A кластера с помощью Azure PowerShell (это инициирует автоматическую отработку отказа кластерной группы SAP <*SID*> с узла A на узел B кластера).
   ...
   После отработки отказа> кластерная группа SAP <*SID* работает на узле B кластера. Например, он работает на **PR1-ASCS-1**.
   ...
   Теперь общий диск подключен к узлу B кластера. SIOS выполняет репликацию данных с диска исходного тома S на узле B кластера на целевой дисковый накопитель на узле A кластера. Например, репликация выполняется из **PR1-ASCS-1 [10.0.0.41]** в **PR1-ASCS-0 [10.0.0.40]**.
