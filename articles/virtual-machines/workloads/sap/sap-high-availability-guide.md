...
* Найти справа примечания к SAP и руководствах по установке, перечисленные в [ресурсы][sap-ha-guide-2] раздел. Это руководство дополняет документацию по установке SAP и примечания к SAP, которые являются основными ресурсами по установке и развертыванию ПО SAP на определенных платформах.
...
Перед началом работы необходимо выполнить все предварительные условия, описанные в следующих разделах. Кроме того, не забудьте проверить все ресурсы, перечисленные в [ресурсы][sap-ha-guide-2] раздел.
...
Дополнительные сведения о [ограничения для подписок Azure][azure-subscription-service-limits-subscription], включая общие ограничения по умолчанию и максимальные ограничения.
...
Дополнительные сведения о том, как [Управление доступностью виртуальных машин][virtual-machines-manage-availability].
...
Для производственных систем SAP, развертывание виртуальных машин Azure с помощью [подключением к корпоративной сети (распределенное развертывание)][planning-guide-2.2] с помощью Azure сеть VPN или Azure ExpressRoute.
...
Вы можете подготовить инфраструктуру и настроить SAP для поддержки сценария с **несколькими идентификаторами безопасности**. Например, вы можете добавить дополнительный экземпляр SAP ASCS/SCS в *имеющуюся* конфигурацию кластера. Дополнительные сведения см. в разделе [Настройка дополнительного экземпляра SAP ASCS/SCS в существующей конфигурации кластера для создания конфигурации с несколькими ИД безопасности SAP в Azure Resource Manager][sap-ha-multi-sid-guide].
...
Чтобы настроить шаблон ASCS/SCS с несколькими ИД безопасности, в [шаблон ASCS/SCS с несколькими ИД безопасности][sap-templates-3-tier-multisid-xscs-marketplace-image] or [ASCS/SCS multi-SID template using Managed Disks][sap-templates-3-tier-multisid-xscs-marketplace-image-md], введите значения для следующих параметров:
...
Чтобы настроить шаблон с несколькими ИД безопасности базы данных, в [шаблон базы данных с несколькими ИД безопасности][sap-templates-3-tier-multisid-db-marketplace-image] or [database multi-SID template using Managed Disks][sap-templates-3-tier-multisid-db-marketplace-image-md], введите значения для следующих параметров:
...
Чтобы настроить этот шаблон с несколькими ИД безопасности серверов приложений в [шаблон с несколькими ИД безопасности серверов приложений][sap-templates-3-tier-multisid-apps-marketplace-image] or [application servers multi-SID template using Managed Disks][sap-templates-3-tier-multisid-apps-marketplace-image-md], введите значения для следующих параметров:
...
Имена виртуальных узлов **pr1-ascs-vir** и **pr1-dbms-vir** и связанные IP-адреса, используемые для управления кластером, создаются вместе с кластером. Сведения о том, как это сделать, см. в разделе [сбор узлов кластера в конфигурации кластера][sap-ha-guide-8.12.1].
...
8. Введите имя второго узла кластера.
   ...
   Предупреждения о кворуме и дисках можно игнорировать. Можно задать кворум и более поздней версии, общих дисков, как описано в разделе [Установка SIOS DataKeeper Cluster Edition для общего диска кластера SAP ASCS/SCS][sap-ha-guide-8.12.3].
...
2. Чтобы сохранить порты по умолчанию для внутреннего балансировщика нагрузки Azure, выберите:
   ...
   * **Для системы ABAP+Java**: экземпляр **ASCS** с номером **00** и экземпляр **SCS** с номером **01**.
   ...
   Чтобы использовать номера экземпляров, отличные от 00 для экземпляра ABAP ASCS и 01 для Java SCS, сначала необходимо изменить правила, описанные в балансировки нагрузки по умолчанию подсистемы балансировки нагрузки Azure внутренняя [изменить правила для балансировки нагрузки ASCS/SCS по умолчанию Внутренний балансировщик нагрузки Azure][sap-ha-guide-8.9].
