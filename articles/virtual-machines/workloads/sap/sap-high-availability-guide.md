...
* Найдите правильные руководства и рекомендации по установке SAP, перечисленные в разделе [ресурсы][sap-ha-guide-2] . Это руководство дополняет документацию по установке SAP и примечания к SAP, которые являются основными ресурсами по установке и развертыванию ПО SAP на определенных платформах.
...
Перед началом работы необходимо выполнить все предварительные условия, описанные в следующих разделах. Кроме того, обязательно проверьте все ресурсы, перечисленные в разделе " [ресурсы][sap-ha-guide-2] ".
...
Узнайте больше о том [, как управлять доступностью виртуальных машин][virtual-machines-manage-availability].
...
Для производственных систем SAP развертывайте виртуальные машины Azure с [подключением к корпоративной сети (][planning-guide-2.2] распределенными) с помощью VPN-подключения типа "сеть — сеть" или Azure ExpressRoute.
...
Вы можете подготовить инфраструктуру и настроить SAP для поддержки сценария с **несколькими идентификаторами безопасности**. Например, вы можете добавить дополнительный экземпляр SAP ASCS/SCS в *имеющуюся* конфигурацию кластера. Дополнительные сведения см. в статье [Настройка дополнительного экземпляра SAP ASCS/SCS в существующей конфигурации кластера для создания конфигурации с несколькими идентификаторами безопасности SAP в Azure Resource Manager][sap-ha-multi-sid-guide].
...
Чтобы настроить шаблон ASCS/SCS с несколькими ИД безопасности в [шаблоне ASCS/SCS с несколькими идентификаторами безопасности][sap-templates-3-tier-multisid-xscs-marketplace-image] или в шаблоне с несколькими идентификаторами безопасности [ASCS/SCS с помощью управляемых дисков][sap-templates-3-tier-multisid-xscs-marketplace-image-md], введите значения для следующих параметров:
...
Чтобы настроить шаблон базы данных с несколькими ИД безопасности, в шаблоне базы данных с несколькими [идентификаторами безопасности][sap-templates-3-tier-multisid-db-marketplace-image] или в [шаблоне с несколькими идентификаторами безопасности, используя управляемые диски][sap-templates-3-tier-multisid-db-marketplace-image-md], введите значения для следующих параметров:
...
Чтобы настроить шаблон серверов приложений с несколькими идентификаторами безопасности, в шаблоне серверы приложений с несколькими [идентификаторами безопасности][sap-templates-3-tier-multisid-apps-marketplace-image] или на [сервере приложений с использованием управляемых дисков][sap-templates-3-tier-multisid-apps-marketplace-image-md]введите значения для следующих параметров:
...
2. Выберите параметры в зависимости от типа сети. Для получения дополнительных сведений см. следующие ресурсы:
   * [Подключение к корпоративной сети (][planning-guide-2.2]распределенное): Добавьте IP-адреса локальных DNS-серверов.  
   ...
   * Для развертываемых виртуальных машин, которые изолируются в Azure, сделайте следующее. Разверните в том же экземпляре виртуальной сети дополнительную виртуальную машину, которая будет выполнять функции DNS-сервера. Добавьте IP-адреса этих виртуальных машин Azure, настроенных для запуска службы DNS.
...
Имена виртуальных узлов **pr1-ascs-vir** и **pr1-dbms-vir** и связанные IP-адреса, используемые для управления кластером, создаются вместе с кластером. Сведения о том, как это сделать, см. [в разделе Получение узлов кластера в конфигурации кластера][sap-ha-guide-8.12.1].

Два других имени виртуальных узлов — **pr1-ascs-sap** и **pr1-dbms-sap** — и связанные IP-адреса можно вручную создать на DNS-сервере. Эти ресурсы используются кластеризованным экземпляром SAP ASCS/SCS и кластеризованным экземпляром СУБД. Сведения о том, как это сделать, см. в разделе [Создание имени виртуального узла для кластеризованного экземпляра SAP ASCS/SCS][sap-ha-guide-9.1.1].
...
8. Введите имя второго узла кластера.
   ...
   Предупреждения о кворуме и дисках можно игнорировать. Вы задаете кворум и предоставляете общий доступ к диску позже, как описано в разделе [Установка SIOS Clustering Edition для общего диска кластера SAP ASCS/SCS][sap-ha-guide-8.12.3].
...
2. Чтобы сохранить порты по умолчанию для внутреннего балансировщика нагрузки Azure, выберите:
   ...
   * **Для системы ABAP+Java**: экземпляр **ASCS** с номером **00** и экземпляр **SCS** с номером **01**.
   ...
   Чтобы использовать номера экземпляров, отличные от 00 для экземпляра ABAP ASCS и 01 для экземпляра Java SCS, сначала необходимо изменить правила балансировки нагрузки по умолчанию для внутренней подсистемы балансировки нагрузки Azure, описанные в разделе [изменение правил балансировки нагрузки ASCS/SCS по умолчанию для Azure. Внутренняя подсистема балансировки нагрузки][sap-ha-guide-8.9].
...
Необходимо добавить новый параметр профиля. Этот параметр профиля запрещает закрытие подключений между рабочими процессами SAP и сервером постановки в очередь, если они простаивают слишком долго. Мы упомянули сценарий проблемы в [разделе Добавление записей реестра на обоих узлах кластера экземпляра SAP ASCS/SCS][sap-ha-guide-8.11]. В этом разделе мы также внесли два изменения в некоторые основные параметры подключения по TCP/IP. На втором шаге необходимо настроить сервер постановки в очередь для отправки сигнала `keep_alive`, чтобы простой подключений не превышал пороговое значение, заданное во внутреннем балансировщике нагрузки Azure.
