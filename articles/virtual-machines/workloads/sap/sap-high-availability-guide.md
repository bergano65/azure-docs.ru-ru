...
Группы ресурсов в Azure Resource Manager можно использовать для управления всеми ресурсами приложения в подписке Azure. В рамках интегрированного подхода в группе ресурсов все ресурсы имеют одинаковый цикл жизни. Например, все они создаются одновременно и удаляются тоже одновременно. Подробнее о [группах ресурсов.](../../../azure-resource-manager/management/overview.md#resource-groups)

### <a name="azure-internal-load-balancer-dependency-on-the-azure-resource-group"></a><a name="3e85fbe0-84b1-4892-87af-d9b65ff91860"></a>Зависимость от баланса внутренней нагрузки Azure в группе ресурсов Azure
...
> [!IMPORTANT]
> Для установки приложений SAP настоятельно рекомендуется использовать модель развертывания с помощью Azure Resource Manager. Она предлагает преимущества, недоступные в классической модели развертывания. Подробнее о [моделях развертывания][virtual-machines-azure-resource-manager-architecture-benefits-arm]Azure.   
>
>

## <a name="windows-server-failover-clustering"></a><a name="8ecf3ba0-67c0-4495-9c14-feec1a2255b7"></a>Кластеризация Windows Server Failover
...
* **Узлы большинства**. Голосовать может каждый узел кластера. Кластер работает только при большинстве голосов (более половины). Этот вариант рекомендуется при наличии нечетного числа узлов. Пример: в кластере из 7 узлов могут отказать 3 узла, при этом в нем сохранится большинство голосов и кластер продолжит работу.  
* **Узлы и дисковое большинство**. Голосовать может каждый узел и указанный диск в хранилище кластера (диск-свидетель), когда они доступны и могут обмениваться данными. Кластер работает только при большинстве голосов (более половины). Этот режим целесообразно использовать в среде кластера с четным числом узлов. Пока половина узлов и диск находятся в сети, кластер находится в работоспособном состоянии.
* **Узла и файла Доля большинства**. Голосовать может каждый узел, а также указанный файловый ресурс, созданный администратором (файловый ресурс-свидетель), независимо от того, доступны ли они и могут ли обмениваться данными. Кластер работает только при большинстве голосов (более половины). Этот режим целесообразно использовать в среде кластера с четным числом узлов. Он похож на режим большинства узлов и дисков, только вместо диска-свидетеля используется файловый ресурс-свидетель. Данный режим прост в реализации, но если файловый ресурс не является высокодоступным, то он может оказаться единственной точкой отказа.
...
## <a name="windows-server-failover-clustering-on-premises"></a><a name="fdfee875-6e66-483a-a343-14bbaee33275"></a>Windows Server Failover Кластеризация на месте
...
_**Рисунок 1:** Пример конфигурации кластеризации Windows Server Failover для SAP ASCS/SCS в Azure_

### <a name="shared-storage"></a><a name="be21cf3e-fb01-402b-9955-54fbecf66592"></a>Совместное хранилище
...
### <a name="networking-and-name-resolution"></a><a name="ff7a9a06-2bc5-4b20-860a-46cdb44669cd"></a>Разрешение сети и имени
...
## <a name="windows-server-failover-clustering-in-azure"></a><a name="2ddba413-a7f5-4e4e-9a51-87908879c10a"></a>Кластеризация Windows Server Failover в Azure
...
### <a name="shared-disk-in-azure-with-sios-datakeeper"></a><a name="1a464091-922b-48d7-9d08-7cecf757f341"></a>Общий диск в Azure с SIOS DataKeeper
...
## <a name="sap-netweaver-high-availability-in-azure-infrastructure-as-a-service-iaas"></a><a name="2e3fec50-241e-441b-8708-0b1864f66dfa"></a>Высокая доступность SAP NetWeaver в Azure Infrastructure-as-a-Service (IaaS)
...
### <a name="high-availability-sap-application-server"></a><a name="93faa747-907e-440a-b00a-1ae0a89b1c0e"></a>Сервер приложений SAP с высокой доступностью
...
_**Рисунок 4:** Сервер приложений SAP с высокой доступностью_
...
### <a name="high-availability-sap-ascsscs-instance"></a><a name="f559c285-ee68-4eec-add1-f60fe7b978db"></a>Высокодоступность экземпляра SAP ASCS/SCS
...
_**Рисунок 5:** Высокодоступность экземпляра SAP ASCS/SCS_

#### <a name="sap-ascsscs-instance-high-availability-with-windows-server-failover-clustering-in-azure"></a><a name="b5b1fd0b-1db4-4d49-9162-de07a0132a51"></a>SAP ASCS/SCS экземпляр высокой доступности с Windows Server Failover кластеризации в Azure
...
### <a name="high-availability-dbms-instance"></a><a name="ddd878a0-9c2f-4b8e-8968-26ce60be1027"></a>Пример DBMS с высокой доступностью
...
_**Рисунок 9:** SAP высокой доступности Архитектурный шаблон 2, со специальным кластером для ASCS / SCS и выделенный кластер для DBMS_
...
_**Рисунок 10:** SAP высокой доступности Архитектурный шаблон 3, со специальным кластером для различных экземпляров ASCS/SCS_
...
* [Пользовательское изображение](https://github.com/Azure/azure-quickstart-templates/tree/master/sap-3-tier-user-image)
...
Для производства систем SAP развертывайте виртуальные машины Azure с корпоративным подключением к сети с помощью VPN Azure Site-to-Site или Azure ExpressRoute.
...
* [Пользовательское изображение](https://github.com/Azure/azure-quickstart-templates/tree/master/sap-3-tier-user-image-converged)
...
Чтобы настроить [шаблон ASCS/SCS с несколькими ИД безопасности][sap-templates-3-tier-multisid-xscs-marketplace-image] или [шаблон ASCS/SCS с несколькими ИД безопасности, использующий управляемые диски][sap-templates-3-tier-multisid-xscs-marketplace-image-md], введите в нем следующие значения параметров:
  ...
  -  **SAP Системный отсчет**. Выберите количество систем SAP, которое требуется установить в этом кластере.
  -  **Доступность системы**. Выберите **HA**.
  -  **Имя пользователя и пароль для админ-**. Создайте пользователя, которого можно использовать для входа на компьютер.
  -  **Новый или существующий подсети**. Определите, следует ли создать виртуальную сеть и подсеть или использовать имеющуюся подсеть. Если у вас уже есть виртуальная сеть, подключенная к вашей предварительной сети, выберите **существующую.**
  -  **Subnet Id**. Если вы хотите развернуть VM в существующую VNet, где у вас есть подсеть, определенная VM должны быть назначены, имя идентификатор этой конкретной подсети. Идентификатор обычно выглядит следующим образом: /subscriptions/<*идентификатор_подписки*>/resourceGroups/<*имя_группы_ресурсов*>/providers/Microsoft.Network/virtualNetworks/<*имя_виртуальной_сети*>/subnets/<*имя_подсети*>
...
- **Sap System Id**. Введите идентификатор системы SAP системы SAP, который вы хотите установить. Идентификатор будет использоваться в качестве префикса для развертываемых ресурсов.
- **Os Тип**. Выберите операционную систему виртуальных машин.
- **Dbtype**. Выберите тип базы данных, которую необходимо установить в кластере. Если вы хотите установить Microsoft SQL Server, выберите **SQL**. Чтобы установить SAP HANA на виртуальных машинах, выберите **HANA**. Выберите соответствующий тип операционной системы: **Windows** для SQL и Linux для HANA. Балансировщик нагрузки Azure, подключенный к виртуальным машинам, будет настроен для поддержки выбранного типа базы данных:
...
- **Доступность системы**. Выберите **HA**.
- **Имя пользователя и пароль для админ-**. Создайте пользователя, которого можно использовать для входа на компьютер.
- **Subnet Id**. Введите идентификатор подсети, использовавую во время развертывания шаблона ASCS/SCS, или идентификатор подсети, созданный в рамках развертывания шаблона ASCS/SCS.
...
2. Выберите параметры в зависимости от типа сети. Дополнительные сведения см. в следующих ресурсах:
   ...
   * Для развертывания VM, изолированного в Azure: Развертывание дополнительной виртуальной машины в том же экземпляре виртуальной сети, который служит DNS-сервером. Добавьте IP-адреса этих виртуальных машин Azure, настроенных для запуска службы DNS.
...
### <a name="host-names-and-static-ip-addresses-for-the-sap-ascsscs-clustered-instance-and-dbms-clustered-instance"></a><a name="9fbd43c0-5850-4965-9726-2a921d85d73f"></a>Имена хостов и статические IP-адреса для кластерных экземпляров SAP ASCS/SCS и экземпляра DBMS
...
1. На портале Azure выберите**IP-адрес****настройки сетевых** > **карт** >  **Группы ресурсов.** > 
2. В колонке **IP-адреса** в разделе **Назначение** выберите **Статический**. В поле **IP-адрес** введите нужный IP-адрес.
   ...
   _**Рисунок 13:** Установка статических IP-адресов для сетевой карты каждой виртуальной машины_
...
### <a name="set-a-static-ip-address-for-the-azure-internal-load-balancer"></a><a name="7a8f3e9b-0624-4051-9e41-b73fff816a9e"></a>Установите статический IP-адрес для баланса внутренней нагрузки Azure
...
При установке экземпляра SAP ASCS/SCS необходимо использовать экземпляр экземпляра по умолчанию **номер 00** для экземпляра ABAP ASCS и номер экземпляра по умолчанию **01** для экземпляра Java SCS.
...
| --- | --- | --- |
| Сервер постановки в очередь, *lbrule3200* |32<*InstanceНомер*> |3200 |
| ABAP Сервер сообщений / *lbrule3600* |36<*InstanceНомер*> |3600 |
| Внутреннее сообщение ABAP / *lbrule3900* |39<*InstanceНомер*> |3900 |
| Сервер сообщений (HTTP), *Lbrule8100* |81<*InstanceНомер*> |8100 |
| Служба sAP START ASCS HTTP / *Lbrule50013* |5<*InstanceNumber*>13 |50013 |
| Служба запуска SAP ASCS (HTTPS), *Lbrule50014* |5<*InstanceNumber*>14 |50014 |
| Репликация Enqueue / *Lbrule50016* |5<*InstanceNumber*>16 |50016 |
...
| --- | --- | --- |
| Enqueue Server / *lbrule3201* |32<*InstanceНомер*> |3201 |
| Сервер шлюза, *lbrule3301* |33<*InstanceНомер*> |3301 |
| Сервер сообщений Java / *lbrule3900* |39<*InstanceНомер*> |3901 |
| Сервер сообщений (HTTP), *Lbrule8101* |81<*InstanceНомер*> |8101 |
| SAP Старт сервис SCS HTTP / *Lbrule50113* |5<*InstanceNumber*>13 |50113 |
| Служба запуска SAP SCS (HTTPS), *Lbrule50114* |5<*InstanceNumber*>14 |50114 |
| Репликация Enqueue / *Lbrule50116* |5<*InstanceNumber*>16 |50116 |
| Служба старта SAP ERS HTTP *Lbrule51113* |5<*InstanceNumber*>13 |51113 |
| Служба старта SAP ERS HTTP *Lbrule51114* |5<*InstanceNumber*>14 |51114 |
| Win RM, *Lbrule5985* | |5985 |
| Общий доступ к файлам, *Lbrule445* | |445 |
...
Установите IP-адрес балансируктора **нагрузки pr1-lb-dbms** на IP-адрес виртуального имени хоста экземпляра DBMS.
...
1. На портале Azure выберите > **Load Balancing Rules** ** <правила балансировки балансировки нагрузки *SID*>-lb-ascs.**
2. Для всех правил балансировки нагрузки, относящихся к экземпляру SAP ASCS или SCS, измените следующие значения.
   ...
   * Внутренний порт
   ...
   _**Рисунок 16:** Изменение правил балансировки по умолчанию ASCS/SCS для баланса внутренней нагрузки Azure_
...
### <a name="set-up-a-windows-server-failover-clustering-cluster-for-an-sap-ascsscs-instance"></a><a name="0d67f090-7928-43e0-8772-5ccbf8f59aab"></a>Настройка кластерного кластера Windows Server Failover для экземпляра SAP ASCS/SCS
...
4. После создания кластера запустите проверочный тест.
   ...
   _**Рисунок 22:** Ресурсу основного кластера нужен новый IP-адрес_
...
8. Введите имя второго узла кластера.
   ...
   > [!IMPORTANT]
   > Убедитесь, что **добавление всех подходящих на это хранилище в кластерное флажок** **НЕ** выбрано.  
   >
   >

   ![Рис. 28. Не следует устанавливать указанный флажок!][sap-ha-guide-figure-3017]

   _**Рисунок 28:** Не **выбирайте** флажок_
...
### <a name="install-sios-datakeeper-cluster-edition-for-the-sap-ascsscs-cluster-share-disk"></a><a name="5c8e5482-841e-45e1-a89d-a05c0907c868"></a>Установка sIOS DataKeeper Кластерное издание для диска sAP ASCS/SCS кластера
...
- Первый способ — с помощью мастера добавления ролей и компонентов в Windows, как показано на рис. 39.
  ...
  _**Рисунок 39:** Установите рамочку .NET 3.5 с помощью мастера добавления ролей и функций_
...
### <a name="install-sap-with-a-high-availability-ascsscs-instance"></a><a name="31c6bd4f-51df-4057-9fdf-3fcbc619c170"></a>Установка SAP с высокой доступностью экземпляра ASCS/SCS
...
#### <a name="create-a-virtual-host-name-for-the-clustered-sap-ascsscs-instance"></a><a name="a97ad604-9094-44fe-a364-f89cb39bf097"></a>Создание виртуального имени хоста для кластерного экземпляра SAP ASCS/SCS

1. В диспетчере DNS Windows создайте запись DNS для имени виртуального узла экземпляра ASCS/SCS.

   > [!IMPORTANT]
   > IP-адрес, присвоенный виртуальному хост-названию экземпляра ASCS/SCS, должен быть таким же, как ip-адрес, назначенный Azure Load Balancer**<*(SID*>-lb-ascs).**  
   >
   >

   IP-адрес виртуального sAP ASCS/SCS хост-имя **(pr1-ascs-sap**) такой же, как IP-адрес Azure Load Balancer **(pr1-lb-ascs**).
...
2. Чтобы определить IP-адрес, присвоенный виртуальному имени хоста, выберите **dNS Manager** > **Domain.**

   ![Рис. 57. Новое виртуальное имя и TCP/IP-адрес для конфигурации кластера SAP ASCS/SCS][sap-ha-guide-figure-3047]

   _**Рисунок 57:** Новое виртуальное имя и адрес TCP/IP для конфигурации кластера SAP ASCS/SCS_

#### <a name="install-the-sap-first-cluster-node"></a><a name="eb5af918-b42f-4803-bb50-eff41f84b0b0"></a>Установка первого кластерного узла SAP
...
#### <a name="add-a-probe-port"></a><a name="10822f4f-32e7-4871-b63a-9b86c76ce761"></a>Добавить порт зонда
...
1. Проверьте текущую настройку **ProbePort,** запустив следующую команду PowerShell. Выполните ее на одной из виртуальных машин в конфигурации кластера.
...
2. Укажите порт пробы. Номер порта зонда по умолчанию **0**. В нашем примере мы используем порт зонда **62000**.
   ...
   После того как вы привведете **роль кластера SAP <*SID* > ** в Интернете, убедитесь, что **ProbePort** настроен на новое значение.
...
#### <a name="open-the-windows-firewall-probe-port"></a><a name="4498c707-86c0-4cde-9c69-058a7ab8c3ac"></a>Откройте порт зонда брандмауэра Windows
...
Для **ProbePort** установлено значение **62000**. Теперь вы можете получить доступ к файлу доля ** \\«ascsha-clsap'sapmnt** от других узлов, таких как из **ascsha-dbas**.
...
### <a name="install-the-second-cluster-node"></a><a name="8a276e16-f507-4071-b829-cdc0a4d36748"></a>Установка второго кластерного узла
...
_**Рисунок 60:** Изменение типа обслуживания для экземпляра SAP ERS на автоматическое отсроченное_
...
## <a name="test-the-sap-ascsscs-instance-failover-and-sios-replication"></a><a name="18aa2b9d-92d2-4c0e-8ddd-5acaabda99e9"></a>Проверьте сбой в сбой экземпляра SAP ASCS/SCS и репликацию SIOS
...
### <a name="sap-ascsscs-instance-is-running-on-cluster-node-a"></a><a name="65fdef0f-9f94-41f9-b314-ea45bbfea445"></a>Экземпляр SAP ASCS/SCS работает на кластерном узлах A

Кластерная группа **SAP PR1** работает на кластерном узлах А. Например, на **pr1-ascs-0**. Назначьте узлу A кластера общий диск S:, принадлежащий кластерной группе **SAP PR1** и используемый экземпляром ASCS/SCS.
...
В инструменте управления и конфигурации SIOS DataKeeper можно увидеть, что общие данные диска синхронно реплицируются из исходного диска громкости S на кластерном узло A в целевой томный диск S на кластерном узло B. Например, он реплицируется с **pr1-ascs-0 (10.0.0.40)** до **pr1-ascs-1 (10.0.0.41).**
...
### <a name="failover-from-node-a-to-node-b"></a><a name="5e959fa9-8fcd-49e5-a12c-37f6ba07b916"></a>Failover от узла A до узла B

1. Выберите один из этих вариантов, чтобы инициировать сбой sAP <*SID*> группы кластеров от кластерного узла A до кластерного узла B:
...
4. перезапустив узел A кластера с помощью Azure PowerShell (это инициирует автоматическую отработку отказа кластерной группы SAP <*SID*> с узла A на узел B кластера).
   ...
   После неудачи группа кластера SAP <*SID*> работает на кластерном узлах B. Например, он работает на **pr1-ascs-1**.
   ...
   Общий диск теперь установлен на кластерном уде B. SIOS DataKeeper реплицирует данные из исходного тома диска S на кластерном узлах B для целевого речения громкости S на кластерном узлах A. Например, он копируется с **pr1-ascs-1 (10.0.0.41)** до **pr1-ascs-0 (10.0.0.40).**
