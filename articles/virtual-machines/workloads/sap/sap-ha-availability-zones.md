...
- Вы не можете развертывать наборы доступности Azure в зоне доступности Azure, если вы не используете [группу размещения близости Azure.](https://docs.microsoft.com/azure/virtual-machines/linux/co-location) Как можно развернуть слой SAP DBMS и центральные службы в разных зонах и в то же время развернуть слой приложения SAP с использованием наборов доступности и при этом достичь непосредственной близости к ВМ, описано в статье [Azure Proximity Placement Groups для оптимальной задержки сети с приложениями SAP.](sap-proximity-placement-scenarios.md) Если вы не используете группы размещения близости Azure, вам необходимо выбрать одну или другую в качестве платформы развертывания для виртуальных машин.
...
Вам не нужно выполнять эти тесты вручную. Вы можете найти процедуру PowerShell [Проверка задержки зоны доступности,](https://github.com/Azure/SAP-on-Azure-Scripts-and-Utilities/tree/master/AvZone-Latency-Test) которая автоматизирует описанные тесты задержки. 
...
Эта архитектура развертывания называется активной/активной, поскольку развертывание активных серверов приложений SAP в двух или трех зонах. Экземпляр центральных служб SAP, использующий службу постановки в очередь для репликации, будет развертываться в двух зонах. Это справедливо и для уровня СУБД, который будет развертываться в тех же зонах, что и центральная служба SAP.
...
- Не используя [группу размещения proximity Azure,](https://docs.microsoft.com/azure/virtual-machines/linux/co-location)вы рассматриваете зоны доступности Azure как неисправность и обновляете домены для всех ВМ, поскольку наборы доступности не могут быть развернуты в зонах доступности Azure.
- Если вы хотите объединить зональные развертывания для уровня DBMS и центральных служб, но хотите использовать наборы доступности Azure для слоя приложений, необходимо использовать группы близости Azure, как описано в статье [Azure Proximity Placement Groups, для оптимальной задержки сети с приложениями SAP.](sap-proximity-placement-scenarios.md)
...
- Для всех виртуальных машин, которые вы развертываете, необходимо использовать [управляемые диски Azure.](https://azure.microsoft.com/services/managed-disks/) Неуправляемые диски не поддерживаются для зональных развертываний.
...
- Третья зона используется для размещения устройства SBD в случае, если вы создаете [кластер SUSE Linux Pacemaker](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-pacemaker#create-azure-fence-agent-stonith-device) или дополнительные экземпляры приложений.
- Для достижения согласованности времени выполнения критически важных бизнес-процессов можно попытаться направить определенные пакетные задания и пользователей в экземпляры приложений, находящихся в зоне с активным экземпляром DBMS, используя группы пакетных серверов SAP, группы логонов SAP или rFC. Но в случае зональной отработки отказа вам придется вручную перемещать эти группы в экземпляры, выполняемые на виртуальных машинах в одной зоне с виртуальной машиной базы данных.  
...
> [!IMPORTANT]
> В этом активном/активном сценарии дополнительные сборы за пропускную способность объявляются корпорацией Майкрософт с 04.01.2020. Проверьте [документ Информация о ценах на пропускную способность.](https://azure.microsoft.com/pricing/details/bandwidth/) Передача данных между слоем приложения SAP и слоем SAP DBMS является довольно интенсивной. Поэтому активный/активный сценарий может немного снизить затраты. Продолжайте проверять эту статью, чтобы получить точные расходы 
...
- Группы доступности невозможно развернуть в Зонах доступности Azure. Чтобы компенсировать это, можно использовать группы размещения близости Azure, как описано в статье [Группы размещения близости Azure для оптимальной задержки сети с приложениями SAP.](sap-proximity-placement-scenarios.md)
