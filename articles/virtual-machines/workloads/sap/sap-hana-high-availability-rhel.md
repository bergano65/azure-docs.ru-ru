---
title: Высокая доступность SAP HANA на Azure VMs на RHEL Документы Майкрософт
...
1. Создайте подсистему балансировки нагрузки (внутреннюю). Мы рекомендуем [стандартный балансер нагрузки.](https://docs.microsoft.com/azure/load-balancer/load-balancer-standard-overview)
...
1. При использовании стандартного балансивизатора нагрузки выполните следующие шаги конфигурации:
   1. Сначала создайте пула IP-адресов для интерфейсной части.
      ...
      1. Введите название нового пула IP-адреса (например, **hana-frontend).**
      1. Установите **назначение** на **Static** и введите IP-адрес (например, **10.0.0.13**).
      1. Щелкните **ОК**.
   ...
   1. Теперь создайте серверный пул.
      ...
      1. Введите название нового бэк-энда (например, **hana-backend).**
   ...
   1. Создайте зонд работоспособности.
      ...
      1. Введите название нового зонда здоровья (например, **hana-hp).**
   ...
   1. Далее создайте правила балансировки нагрузки:
   
      1. Откройте подсистему балансировки нагрузки, выберите **Правила балансировки нагрузки** щелкните **Добавить**.
      1. Введите название нового правила балансоостатка нагрузки (например, **hana-lb).**
      1. Выберите адрес ip-адреса переднего конца, пул бэк-энда и зонд здоровья, который вы создали ранее (например, **hana-frontend,** **hana-backend** и **hana-hp).**
      1. Выберите **порты HA**.
      1. Увеличьте **время ожидания** до 30 минут.
      1. Убедитесь в том, чтобы **включить плавающий IP**.
      1. Щелкните **ОК**.
...
1. Кроме того, если сценарий диктует использование базового балансиватора нагрузки, выполните следующие шаги конфигурации:
   ...
   1. Если используется SAP HANA 1.0, создайте правила балансировки нагрузки.
      ...
      1. Откройте подсистему балансировки нагрузки, выберите **Правила балансировки нагрузки** щелкните **Добавить**.
      1. Введите название нового правила балансоостатка нагрузки (например, hana-lb-3**03**15).
   ...
   1. Если используется SAP HANA 2.0, создайте правила балансировки нагрузки для системной базы данных.
      ...
      1. Откройте подсистему балансировки нагрузки, выберите **Правила балансировки нагрузки** щелкните **Добавить**.
      1. Введите название нового правила балансоостатка нагрузки (например, hana-lb-3**03**13).
      1. Выберите интерфейсный пул IP-адресов, серверный пул и зонд работоспособности, который вы создали ранее (например, **hana-frontend**).
   ...
   1. Если используется SAP HANA 2.0, создайте правила балансировки нагрузки для базы данных клиента.
      ...
      1. Откройте подсистему балансировки нагрузки, выберите **Правила балансировки нагрузки** щелкните **Добавить**.
      1. Введите название нового правила балансоостатка нагрузки (например, hana-lb-3**03**40).
      1. Выберите IP-адрес переднего энда, пул бэкэнда и зонд здоровья, созданный ранее (например, **hana-frontend).**
...
> Смотрите также SAP примечание [2382421](https://launchpad.support.sap.com/#/notes/2382421). 
...
1. **[A]** Настройте разметку диска (для **диспетчера логических томов**).
   ...
   Создайте логические тома. Линейный том создается при использовании `lvcreate` без параметра `-i`. Мы предлагаем создать полосатый том для повышения производительности ввоза/ои и привести размеры полосы к значениям, задокументированным в [конфигурациях хранения SAP HANA VM.](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage) Аргументом `-i` должно быть количество основных физических `-I` томов, а аргументом является размер полосы. В этом документе для тома данных используется 2 физических тома, поэтому аргумент `-i` имеет значение **2**. Размер полосы для объема данных составляет **256KiB.** Один физический том используется для объема `-i` `-I` журнала, поэтому для команд громкости журнала явно не используются коммутаторы или переключатели.  

   > [!IMPORTANT]
   > Параметр `-i`, значение которого соответствует числу базовых физических томов, необходимо указывать для всех томов данных, журналов или общих томов, для которых используется более одного физического тома. Используйте `-I` переключатель, чтобы указать размер полосы, при создании полосатого объема.  
   > Просматривайте [конфигурации хранения SAP HANA VM](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage) для рекомендуемых конфигураций хранения, включая размеры полос и количество дисков.  
...
1. **[A]** Установка SAP HANA
   ...
   * "Do you want to continue? (y/n)" (y/n): подтвердите введенные данные. Введите **y,** чтобы продолжить.
...
1. **[A]** Настройка брандмауэра
   ...
   Создайте правило брандмауэра для порта зонда балансировки нагрузки Azure.
...
1. **[1]** Создайте базу данных клиента.
   ...
   Если вы используете SAP HANA 2.0 или MDC, создайте базу данных клиента для системы SAP NetWeaver. Замените **NW1** SID вашей системы SAP.

   Выполните как <\>ханасид адм следующей команды:

   <pre><code>hdbsql -u SYSTEM -p "<b>passwd</b>" -i <b>03</b> -d SYSTEMDB 'CREATE DATABASE <b>NW1</b> SYSTEM USER PASSWORD "<b>passwd</b>"'
   </code></pre>

1. **Настройка** репликации системы на первом узлах:

   Резервное копирование баз данных\>в качестве <ханасида:
...
1. **[2]** Настройте системную репликацию на втором узле
   ...
   Зарегистрируйте второй узел, чтобы запустить репликацию системы. Выполнить следующую команду в\>качестве <ханасида:
...
1. **[1]** Проверка состояния репликации
   ...
   Проверьте состояние репликации и подождите, пока все базы данных будут синхронизированы. Если статус остается UNKNOWN, проверьте настройки брандмауэра.
...
1. **Создайте** нужных пользователей.

   Выполнить следующую команду в качестве корня. Убедитесь в том, чтобы заменить смелые строки (HANA System ID **HN1** и экземпляр номер **03**) со значениями установки SAP HANA:
...
1. **[A]** Создайте запись в хранилище ключей.
   ...
   Выполнить следующую команду в качестве корня, чтобы создать новую запись в магазине ключей:
...
1. **[1]** Создайте резервную копию базы данных.
   ...
   Резервное копирование баз данных в качестве корня:
...
1. **[1]** Настройте репликацию системы на первом узле.
   ...
   Создайте основной сайт в\>качестве <анасидов:
...
1. **[2]** Настройте репликацию системы на вторичном узле
   ...
   Зарегистрируйте вторичный сайт\>в качестве <ханасида:
...
> [!NOTE]
> Тайм-ауты в вышеуказанной конфигурации являются лишь примерами и, возможно, должны быть адаптированы к конкретной установке HANA. Например, может потребоваться увеличить время отойти от запуска, если для запуска базы данных SAP HANA требуется больше времени.  
...
Смотрите [статью Red Hat Knowledgebase 79523](https://access.redhat.com/solutions/79523) для описания того, как имитировать сбой сети. В этом примере мы используем сценарий net_breaker для полной блокировки доступа к сети.
...
* Чтобы узнать, как установить высокую доступность и планировать аварийное восстановление SAP HANA на Azure (большие экземпляры), [см. SAP HANA (крупные экземпляры) высокой доступности и аварийного восстановления на Azure](hana-overview-high-availability-disaster-recovery.md)
