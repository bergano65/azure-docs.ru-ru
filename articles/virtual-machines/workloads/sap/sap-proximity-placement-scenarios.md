...
Время, затраченное в сети на отправку такого запроса с уровня приложения на уровень базы данных и получение результирующего набора, оказывает значительное влияние на время, затрачиваемое на выполнение бизнес-процессов. Эта чувствительность к сетевой задержке заключается в том, что необходимо обеспечить оптимальную задержку в сети в проектах развертывания SAP. См. раздел [SAP Примечание #1100926. часто задаваемые вопросы: Производительность сети @ no__t-0. рекомендации по классификации сетевой задержки.
...
- Первая виртуальная машина Azure, развернутая в центре обработки данных. Вы можете считать, что первая виртуальная машина развернута в центре обработки данных на основе алгоритмов распределения Azure, которые в конечном итоге объединяются с определениями пользователей для определенной зоны доступности.
...
- При достижении оптимальной производительности системы SAP и ограничении одного центра обработки данных Azure для системы с помощью групп размещения, возможно, вы не сможете объединить все типы семейств виртуальных машин в пределах группы размещения. Эти ограничения возникают из-за того, что оборудование узла, необходимое для запуска определенного типа виртуальной машины, может отсутствовать в центре обработки данных, на который была развернута виртуальная машина привязки группы размещения.
...
- Для этого требуется тип виртуальной машины, который не может выполняться в конкретном центре обработки данных, к которому привязана группа размещения с учетом расположения.
