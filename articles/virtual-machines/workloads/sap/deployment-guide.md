...
1. **Параметры**:
   * **Хранилище**
     ...
     * **Использовать управляемые диски**. Если вы хотите использовать Управляемые диски, выберите "Да". Дополнительные сведения об управляемых дисках см. в разделе " [управляемые диски][planning-guide-managed-disks] " раздела "планирование по планированию".
   ...
   * **Расширения**. Можно установить расширения виртуальных машин, добавив их в развертывание. На этом шаге не нужно добавлять расширение. Расширения, необходимые для поддержки SAP, устанавливаются позже. См. раздел [Настройка расширения расширенного мониторинга Azure для SAP][deployment-guide-4.5] в этом руководством.
   * **Высокий уровень доступности** — Выберите группу доступности или введите параметры, чтобы создать ее. Дополнительные сведения см. в статье [группы доступности Azure][planning-guide-3.2.3].
   * **Мониторинг**
...
Если развертывание Azure подключено к локальному Active Directory или экземпляру DNS через VPN-подключение типа "сеть — сеть" Azure или ExpressRoute (это называется распределенным в [виртуальных машинах Azure планирование и внедрение SAP). NetWeaver][planning-guide]), предполагается, что виртуальная машина присоединяется к локальному домену. Дополнительные сведения о вопросах, касающихся этой задачи, см. в статье [Присоединение ВИРТУАЛЬНОЙ машины к локальному домену (только для Windows)][deployment-guide-4.3].

#### <a name="ec323ac3-1de9-4c3a-b770-4ff701def65b"></a>Настройка мониторинга

Чтобы убедиться, что SAP поддерживает вашу среду, настройте расширение мониторинга Azure для SAP, как описано в разделе [Настройка расширения расширенного мониторинга Azure для SAP][deployment-guide-4.5]. Проверьте предварительные требования для мониторинга SAP и требуемые минимальные версии ядра SAP и агента узла SAP в ресурсах, перечисленных в [ресурсах SAP][deployment-guide-2.2].

#### <a name="monitoring-check"></a>Проверка мониторинга

Проверьте, работает ли мониторинг, как описано в разделе [проверки и устранение неполадок для настройки сквозного мониторинга][deployment-guide-troubleshooting-chapter].
...
Если развертывание Azure подключено к локальному Active Directory или к экземпляру DNS через VPN-подключение типа "сеть — сеть" Azure или Azure ExpressRoute (это называется распределенным в [виртуальных машинах Azure планирование и внедрение SAP). NetWeaver][planning-guide]), предполагается, что виртуальная машина присоединяется к локальному домену. Дополнительные сведения о вопросах этого шага см. в статье [Присоединение ВИРТУАЛЬНОЙ машины к локальному домену (только для Windows)][deployment-guide-4.3].
...
* [**Шаблон двухуровневой конфигурации (только одна виртуальная машина)** (SAP-2-Tier-пользователь-диск)][sap-templates-2-tier-os-disk]
...
Чтобы обновить параметры мониторинга, обновите инфраструктуру мониторинга, выполнив действия, описанные в разделе [Настройка расширенного мониторинга Azure для SAP][deployment-guide-4.5].
...
После подготовки виртуальной машины, как описано в статье [сценарии развертывания виртуальных машин для SAP в Azure][deployment-guide-3], на виртуальной машине устанавливается агент виртуальной машины Azure. Теперь необходимо развернуть расширение расширенного мониторинга Azure для SAP, доступное в репозитории расширений Azure в глобальных центрах обработки данных Azure. Дополнительные сведения см. в статье [планирование и реализация SAP NetWeaver на виртуальных машинах Azure][planning-guide-9.1].
...
* Чтобы убедиться, что все шаги расширенного мониторинга Azure успешно выполнены и инфраструктура Azure предоставляет необходимые данные, выполните проверку готовности расширения расширенного мониторинга Azure для SAP, как описано в разделе [. Проверка готовности расширенного мониторинга Azure для SAP][deployment-guide-5.1].
...
Выполните проверку готовности расширения расширенного мониторинга Azure для SAP, как описано в статье [Проверка готовности расширения расширенного мониторинга Azure для SAP][deployment-guide-5.1]. Если проверка выполнена успешно и получены все соответствующие счетчики производительности, мониторинг Azure успешно настроен. Вы можете продолжить установку агента узла SAP, как описано в разделе Заметки SAP в ресурсах [SAP][deployment-guide-2.2]. Если проверка готовности указывает на отсутствие счетчиков, запустите проверку работоспособности для инфраструктуры мониторинга Azure, как описано в статье [Проверка работоспособности для конфигурации инфраструктуры мониторинга Azure][deployment-guide-5.2]. Дополнительные сведения о возможностях устранения неполадок см. в [статье Устранение неполадок мониторинга Azure для SAP][deployment-guide-5.3].
...
Если значение **состояния работоспособности** не равно " **ОК**", следуйте инструкциям в статье [Проверка работоспособности для конфигурации инфраструктуры мониторинга Azure][deployment-guide-5.2].
...
Если какая – либо из этих проверок не пройдена и подробные сведения о том, как повторно развернуть расширение, см. [в разделе Устранение неполадок инфраструктуры мониторинга Azure для SAP][deployment-guide-5.3].

### <a name="e2d592ff-b4ea-4a53-a91a-e5521edb6cd1"></a>Проверка работоспособности конфигурации инфраструктуры мониторинга Azure

Если некоторые данные мониторинга не были правильно доставлены, как указано в тесте, описанном в разделе [Проверка готовности для расширенного мониторинга Azure для SAP][deployment-guide-5.1], выполните `Test-AzVMAEMExtension` командлет, чтобы проверить, является ли инфраструктура мониторинга Azure и мониторинг. расширение для SAP настроено правильно.
...
Убедитесь, что для каждой проверки установлено состояние **ОК**. Если некоторые проверки не отображаются,выполните командлет Update, как описано в разделе [Настройка расширенного мониторинга Azure для SAP][deployment-guide-4.5]. Подождите 15 минут и повторите проверки, описанные в статье [Проверка готовности расширенного мониторинга Azure для SAP][deployment-guide-5.1] и [Проверка работоспособности для конфигурации инфраструктуры мониторинга Azure][deployment-guide-5.2]. Если проверки по-прежнему указывают на проблему с некоторыми или всеми счетчиками, см. раздел [Устранение неполадок инфраструктуры мониторинга Azure для SAP][deployment-guide-5.3].

> [!Note]
> Вы можете получить некоторые предупреждения в случаях, когда вы используете Управляемые диски Azure (цен. категория "Стандартный"). Предупреждения будут отображаться вместо тестов, возвращающих "ОК". Это нормально в случае с этим типом диска. См. также раздел [Устранение неполадок инфраструктуры мониторинга Azure для SAP][deployment-guide-5.3] .
> 
...
Если служба отсутствует, то расширение расширенного мониторинга Azure для SAP установлено неправильно. Повторно разверните расширение с помощью действий, описанных в сценарии развертывания, в [сценариях развертывания виртуальных машин для SAP в Azure][deployment-guide-3].
...
Недопустимая конфигурация. Перезапустите расширение мониторинга для виртуальной машины, как описано в разделе [Настройка расширенного мониторинга Azure для SAP][deployment-guide-4.5].
...
Если устранение неполадок с помощью SAP Note [1999351] не устраняет проблему, перезапустите `Set-AzVMAEMExtension` сценарий настройки, как описано в разделе [Настройка расширенного мониторинга Azure для SAP][deployment-guide-4.5]. Необходимо подождать час, так как счетчики аналитики или диагностики хранилищ создаются не сразу после включения. Если проблема не исчезла, откройте сообщение о поддержке клиентов SAP в компоненте BC-OP-NT-AZR для виртуальной машины Windows или BC-OP-LNX-AZR для виртуальной машины Linux.
