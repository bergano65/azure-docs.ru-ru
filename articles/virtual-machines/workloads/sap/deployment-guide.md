...
В зависимости от конфигурации локальной сети на виртуальной машине может потребоваться настроить прокси. Если виртуальная машина подключена к локальной сети через VPN или ExpressRoute, виртуальная машина может не получить доступ к Интернету и не сможет скачать необходимые расширения виртуальной машины или получить сведения о инфраструктуре Azure для агента узла SAP через расширение SAP. сведения об Azure см. [в разделе Настройка прокси-сервера][deployment-guide-configure-proxy].
...
* Чтобы убедиться, что все этапы расширения виртуальной машины Azure для конфигурации SAP выполнены успешно и инфраструктура Azure предоставляет необходимые данные, пройдите проверку готовности для расширения Azure для SAP, как описано в разделе [Проверка готовности. для расширения Azure для SAP][deployment-guide-5.1].
...
1. В командной строке измените каталог на папку установки расширения Azure для SAP. C:\\Packages\\Plugins\\Microsoft.AzureCAT.AzureEnhancedMonitoring.AzureCATExtensionHandler\\&lt;версия>\\drop
...
Если некоторые данные инфраструктуры не были правильно доставлены, как указано в тесте, описанном в разделе [Проверка готовности для расширения Azure для SAP][deployment-guide-5.1], выполните `Test-AzVMAEMExtension` командлет, чтобы проверить, являются ли инфраструктура Azure и расширение Azure для SAP настроен правильно.
...
Каталог \\var\\lib\\waagentнеимеетподкаталогадлярасширенияAzureдляSAP.\\
