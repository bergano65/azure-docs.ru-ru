---
title: Кластер SAP ASCS/SCS на WSFC с использованием файла в Azure Документы Майкрософт
...
* Файлы глобального узла SAP помещаются в файловый ресурс SMB. К ним можно получить доступ с использованием имени \<глобального узла SAP\>: \\\\&lt;глобальный узел SAP&gt;\sapmnt\\&lt;SID&gt;\SYS\..
...
При выборе пробелов для хранения Direct учитывайте следующие случаи использования:

- Виртуальные машины, используемые для создания кластера Storage Spaces Direct, должны быть развернуты в наборе доступности Azure.
- Для аварийного восстановления прямого кластера хранилищ можно использовать [службы восстановления сайта Azure.](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-support-matrix#replicated-machines---storage)
- Он не поддерживается, чтобы растянуть пространство для хранения прямого кластера в различных зонах доступности Azure.
...
> [!IMPORTANT]
> В облаке Azure каждый кластер, используемый для SAP и масштабированных файлов, должен быть развернут в своем собственном наборе доступности Azure или в зонах доступности Azure. Это гарантирует распределенное размещение этих виртуальных машин кластера в базовой инфраструктуре Azure. Развертывание зоны доступности поддерживается этой технологией.
>
...
* [Пространства для хранения данных Прямая в Windows Server 2016][s2d-in-win-2016]
