...
> [!IMPORTANT]
>Если настройка аварийного восстановления *не планируется*, рекомендуется использовать масштабируемый файловый ресурс в качестве решения для высокодоступного файлового ресурса в Azure.
>
...
* Рекомендуется отформатировать тома с помощью новой системы Resilient File System (ReFS).
    * Дополнительные сведения см. в разделе [1869038 Примечание SAP - поддержка файловой системы ReFs в SAP][1869038] and the [Choosing the file system][planning-volumes-s2d-choosing-filesystem] главе статьи планирование томов в локальных дисковых пространств.
    * Убедитесь, что установкой [накопительное обновление Майкрософт KB4025334][kb4025334].
...
* Чтобы обеспечить оптимальную производительность внутри сети виртуальных машин, необходимую для синхронизации дисков Локальных дисковых пространств, используйте тип виртуальной машины с высокой пропускной способностью сети.
    Дополнительные сведения см. в разделе [серии DSv2][dv2-series] and [DS-Series][ds-series] спецификации.
* Рекомендуется зарезервировать нераспределенную часть емкости в пуле хранения. Таким образом тома получат пространство для локального восстановления в случае отказа диска. В результате безопасность и производительность данных повысится.  Дополнительные сведения см. в разделе [Выбор размера томов][choosing-the-size-of-volumes-s2d].
* Виртуальные машины Azure масштабируемого файлового ресурса нужно развернуть в собственной группе доступности Azure.
...
>
> Дополнительные сведения см. в разделе [SAP Note 2492395 — Can изменить имя общей папки sapmnt?][2492395]
...
> [!IMPORTANT]
> В облаке Azure каждый кластер, используемый для SAP и масштабируемых файловых ресурсов, нужно развернуть в собственной группе доступности Azure. Это гарантирует распределенное размещение этих виртуальных машин кластера в базовой инфраструктуре Azure.
>
...
> [!IMPORTANT]
> Масштабируемый файловый ресурс — это рекомендуемое решение для высокодоступного файлового ресурса.
>
> Если вы планируете настроить аварийное восстановление для высокодоступного общего файлового ресурса, необходимо использовать универсальный файловый ресурс и SISO DataKeeper для общих дисков кластера.
>
...
* [Подготовка инфраструктуры Azure для SAP высокого уровня ДОСТУПНОСТИ с использованием отказоустойчивого кластера и файлового ресурса Windows для экземпляра SAP ASCS/SCS][sap-high-availability-infrastructure-wsfc-file-share]
* [Установите SAP NetWeaver высокого уровня ДОСТУПНОСТИ на Windows отказоустойчивого кластера и файлового ресурса для экземпляра SAP ASCS/SCS][sap-high-availability-installation-wsfc-shared-disk]
* [Развертывание двух узлов дисковых пространств масштабируемого файлового сервера для хранилища UPD в Azure][deploy-sofs-s2d-in-azure]
* [Дисковые пространства в Windows Server 2016][s2d-in-win-2016]
* [Deep Dive: Прямой томов в дисковых пространств][deep-dive-volumes-in-s2d]
