...
Вы можете отслеживать состояние работоспособности SAP HANA ландшафт как user ** \<Hana SID\>ADM** , вызвав сценарий SAP Python следующим образом. Возможно, потребуется адаптировать путь:
...
Чтобы использовать интерфейс браузера, замените ** \<Node\> ** фактическим SAP HANA узлом в следующем URL-адресе. Затем введите учетные данные для кластера (пользователь **cluster**):
