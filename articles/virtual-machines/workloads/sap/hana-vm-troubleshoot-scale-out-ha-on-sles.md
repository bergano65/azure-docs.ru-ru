...
SUSE опубликовал [подробное описание этой настройки, оптимизированной для производительности][sles-hana-scale-out-ha-paper].

Для типов виртуальных машин, которые поддерживаются для SAP HANA горизонтального масштабирования, проверьте [Каталог IaaS SAP HANA Certified][sap-hana-iaas-list].
...
Если вам нужна поддержка из SUSE, следуйте указаниям в этом [руководстве][suse-pacemaker-support-log-files]. Соберите всю информацию о кластере SAP HANA с высоким уровнем доступности (HA), как описано в этой статье. Службе поддержке SUSE нужна эта информация для дальнейшего анализа.
...
Каждая виртуальная машина содержит три виртуальных сетевых адаптера в соответствии с количеством подсетей. [Создание виртуальной машины Linux в Azure с несколькими сетевыми картами][azure-linux-multiple-nics] . Описание возможной проблемы маршрутизации в Azure при развертывании виртуальной машины Linux. Она касается только использования нескольких виртуальных сетевых адаптеров. Проблема разрешена SUSE по умолчанию в SLES 12 SP3. Дополнительные сведения см. [в статье Multi-NIC с Cloud-netconfig в EC2 и Azure][suse-cloud-netconfig].
...
Чтобы найти все порты, используемые в программном стеке SAP, включая SAP HANA, выполните поиск по [портам TCP/IP всех продуктов SAP][sap-list-port-numbers].
...
В этом руководстве по устранению неполадок описываются вопросы высокой доступности для SAP HANA в конфигурации горизонтального масштабирования. Еще один важный компонент ландшафта SAP, помимо базы данных, — стек SAP NetWeaver. Узнайте о [высокой доступности SAP NetWeaver на виртуальных машинах Azure, использующих SUSE Enterprise Linux Server][sap-nw-ha-guide-sles].
