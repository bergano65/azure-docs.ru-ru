---
title: SAP HANA масштабирования HSR-Pacemaker с SLES на Azure VMs устранение неполадок Документы Майкрософт
...
Чтобы найти правильные номера портов, можно посмотреть, например, в студии HANA в соответствии с **конфигурацией** или через выписку по S'L:
...
8. На экране **Подключенных целей,** вы должны увидеть запись для устройства SBD, как этот образец: **10.0.0.19:3260 iqn.2006-04.dbhso.local:dbhso**.
...
Вы можете отслеживать состояние ландшафта SAP HANA в качестве пользователя ** \<HANA SID\>adm,** вызывая следующий сценарий SAP Python. Возможно, потребуется адаптировать путь:
...
Чтобы использовать интерфейс браузера, замените ** \<узла\> ** фактическим узлом SAP HANA в следующем URL. Затем введите учетные данные для кластера (пользователь **cluster**):
