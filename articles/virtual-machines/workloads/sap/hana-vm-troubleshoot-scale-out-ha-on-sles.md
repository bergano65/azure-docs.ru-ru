...
Опубликованные SUSE [подробное описание установки оптимизированная][sles-hana-scale-out-ha-paper].

Для типов виртуальных машин, которые поддерживаются для горизонтального масштабирования SAP HANA, проверьте [IaaS directory, сертифицированные SAP HANA][sap-hana-iaas-list].
...
Если вам нужна поддержка от SUSE, выполните это [руководство][suse-pacemaker-support-log-files]. Соберите всю информацию о кластере SAP HANA с высоким уровнем доступности (HA), как описано в этой статье. Службе поддержке SUSE нужна эта информация для дальнейшего анализа.
...
Чтобы найти каждый порт, используемый в стеке программного обеспечения SAP, включая SAP HANA, выполните поиск [портов TCP/IP для всех продуктов SAP][sap-list-port-numbers].
...
Как уже упоминалось в разделе [Важные примечания](#important-notes), для проверки отработки отказа кластера или перенаправления SAP HANA HSR не следует использовать стандартное завершение работы. Вместо этого рекомендуем вызвать тревогу ядра, выполнить принудительный перенос ресурсов или завершить работу всех сетей на уровне ОС виртуальной машины. Можно также использовать команду **crm \<узел\> standby**. См. в разделе [документа SUSE][sles-12-ha-paper]. 
...
Дополнительные сведения можно найти в документах из SUSE как [к без простоя][sles-zero-downtime-paper] or [SAP HANA SR Performance Optimized Scenario][sles-12-for-sap]. В этих документах также приводятся примеры того, как вручную перенести основной сайт.
...
В этом руководстве по устранению неполадок описываются вопросы высокой доступности для SAP HANA в конфигурации горизонтального масштабирования. Еще один важный компонент ландшафта SAP, помимо базы данных, — стек SAP NetWeaver. Дополнительные сведения о [высокий уровень доступности для SAP NetWeaver на виртуальных машинах, использующих SUSE Enterprise Linux Server][sap-nw-ha-guide-sles].
