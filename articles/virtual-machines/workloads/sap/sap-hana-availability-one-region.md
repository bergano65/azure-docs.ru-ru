...
>[!IMPORTANT]
>При восстановлении работоспособности службы Azure виртуальные машины Linux, на которых находится гостевая ОС с ядром в критическом состоянии, не будут перезапущены. Параметры по умолчанию для обычно используемых выпусков Linux автоматически не перезапускают виртуальные машины или сервер, где ядро Linux находится в критическом состоянии. Вместо этого для ОС сохраняется критическое состояние ядра, чтобы подключить отладчик ядра для анализа. Azure следует такому поведению по умолчанию, не перезапуская виртуальную машину с подобной гостевой ОС автоматически. Предполагается, что такие случаи встречаются крайне редко. Вы можете перезаписать поведение по умолчанию, чтобы инициировался перезапуск виртуальной машины. Чтобы изменить поведение по умолчанию, задайте параметр "kernel.panic" в разделе /etc/sysctl.conf. Для этого параметра устанавливается время в секундах. Рекомендуем подождать 20–30 секунд, прежде чем запускать перезагрузку с помощью этого параметра. См. также раздел <https://gitlab.com/procps-ng/procps/blob/master/sysctl.conf>.
...
![Схема, на которой показана архитектура двух виртуальных машин с репликацией хранилища.](./media/sap-hana-availability-one-region/two_vm_storage_replication.PNG) 
...
В распространенной стандартной конфигурации доступности в пределах одного региона Azure у двух виртуальных машинах Azure под управлением SLES Linux есть определенный отказоустойчивый кластер. Кластер Linux SLES основан на платформе [Pacemaker](./high-availability-guide-suse-pacemaker.md) в сочетании с устройством [STONITH](./high-availability-guide-suse-pacemaker.md#create-azure-fence-agent-stonith-device). 
