---
title: Настройка Pacemaker на SLES в Azure (ru) Документы Майкрософт
...
1. Обновите SLES.
   ...
   > [!NOTE]
   > Возможно, потребуется перезагрузить ОС после обновления или обновления ОС. 
...
**`nfs`** используется для идентификации кластера NFS, **ascsnw1** используется для идентификации кластера ASCS **NW1,** **dbnw1** используется для идентификации кластера баз данных **NW1,** **nfs-0** и **nfs-1** являются хостименами узлов кластера NFS, **nw1-xscs-0** и **nw1-xscs-1** являются именами узлов кластера **NW1** ASCS, а **nw1-db-0** и **nw1-db-1** являются хостименами кластерных узлов базы данных. Замените их именами узлов кластера и SID системы SAP.
...
1. **Установка компонента,** необходимого для кластерных ресурсов

   <pre><code>sudo zypper in socat
   </code></pre>

1. **Установка** компонента лазурного фунта, необходимого для кластерных ресурсов

   <pre><code>sudo zypper in resource-agents
   </code></pre>

   > [!NOTE]
   > Проверьте версию ресурсо-агентов пакета и убедитесь, что минимальные требования версии выполнены:  
...
1. **[A]** Установите агенты ограждения
   ...
   >[!IMPORTANT]
   > При использовании Suse Linux Enterprise Server для SAP 15 следует знать, что необходимо активировать дополнительный модуль и установить дополнительный компонент, что является необходимым условием для использования Azure Fence Agent. Чтобы узнать больше о модулях и расширениях SUSE, смотрите [в модулях и расширениях.](https://www.suse.com/documentation/sles-15/singlehtml/art_modules/art_modules.html) Следуйте инструкциям по установке Azure Python SDK. 

   Следующие инструкции по установке Azure Python SDK применимы только для Suse Enterprise Server для SAP **15.**  

    - Если вы используете Bring-Your-Own-Подписка, следуйте этим инструкциям  
    ...
    sudo zypper in python3-azure-sdk
     ...
     - Если вы используете подписку Pay-As-You-Go, следуйте этим инструкциям  
...
1. **Отрегулируйте** настройки corosync.  
...
Назначьте субъекту-службе пользовательскую роль Linux Fence Agent Role, созданную в последней главе. Не используйте роль Владельца больше!

1. Перейти к[https://portal.azure.com](https://portal.azure.com)
...
## <a name="pacemaker-configuration-for-azure-scheduled-events"></a>Конфигурация кардиостимулятора для запланированных событий Azure

Azure предлагает [запланированные мероприятия.](https://docs.microsoft.com/azure/virtual-machines/linux/scheduled-events) Запланированные события предоставляются через службу мета-данных и позволяют приложению подготовиться к таким событиям, как отключение VM, передислокация VM и т.д. Ресурсный агент **[azure-events](https://github.com/ClusterLabs/resource-agents/pull/1161)** отслеживает запланированные события Azure. Если события обнаружены, агент попытается остановить все ресурсы на воздействии VM и переместить их в другой узло в кластере. Для этого необходимо настроить дополнительные ресурсы Pacemaker. 

1. **Убедитесь,** что пакет для агента **azure-events** уже установлен и обновлен. 

<pre><code>sudo zypper info resource-agents
</code></pre>

2. **Назначь** ресурсы в Pacemaker. 
...
sudo crm configure property maintenance-mode=false
   ...
   > [!NOTE]
   > После настройки ресурсов Pacemaker для агента azure-событий при помещании кластера в режим обслуживания или из нее, вы можете получить предупреждающие сообщения типа:  
     ВНИМАНИЕ: cib-bootstrap-опции: неизвестный атрибут 'hostName_ <strong>хостимя'</strong>  
     ВНИМАНИЕ: cib-bootstrap-опции: неизвестный атрибут 'лазурно-events_globalPullState'  
     ВНИМАНИЕ: cib-bootstrap-опции: неизвестный атрибут 'hostName_ <strong>хостимя'</strong>  
