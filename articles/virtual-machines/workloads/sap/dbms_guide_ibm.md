---
title: IBM Db2 Azure виртуальные машины по развертыванию СУБД для рабочей нагрузки SAP | Документация Майкрософт
description: IBM Db2 Azure виртуальные машины по развертыванию СУБД для рабочих нагрузок SAP
...
---
# <a name="ibm-db2-azure-virtual-machines-dbms-deployment-for-sap-workload"></a>IBM Db2 Azure виртуальные машины по развертыванию СУБД для рабочих нагрузок SAP
...
С помощью Microsoft Azure вы можете перенести существующие приложения SAP, работе в IBM Db2 для Linux, UNIX и Windows (LUW) на виртуальных машинах Azure. С SAP в IBM Db2 для LUW Администраторы и разработчики можно по-прежнему использовать те же разработки и средства администрирования, которые доступны локально.
Общие сведения о работе SAP Business Suite в IBM Db2 для LUW можно найти в сообщества SAP (SCN) в <https://www.sap.com/community/topic/db2-for-linux-unix-and-windows.html>.
...
## <a name="ibm-db2-for-linux-unix-and-windows-version-support"></a>IBM Db2 для Linux, UNIX и поддержка версий Windows
SAP в IBM Db2 для LUW в службах виртуальных машин Microsoft Azure поддерживается начиная с версии Db2 10.5.

Дополнительные сведения о поддерживаемых продуктах SAP и типах виртуальных машин Azure см. в примечании к SAP [1928533].

## <a name="ibm-db2-for-linux-unix-and-windows-configuration-guidelines-for-sap-installations-in-azure-vms"></a>IBM Db2 для Linux, UNIX и рекомендации по конфигурации Windows для установки SAP на виртуальных машинах Azure
...
Операторы с помощью дисков, на основе хранилища страничных BLOB-ОБЪЕКТОВ Azure или управляемых дисков, внесенные в [рекомендации по развертыванию Azure виртуальные машины СУБД рабочей нагрузки SAP](dbms_guide_general.md) относятся к развертываниям с СУБД Db2 также.
...
Функциональные возможности резервного копирования и восстановления для IBM Db2 для LUW поддерживается в так же, как в стандартных операционных системах Windows Server и Hyper-V.
...
>[!NOTE]
>Db2 на Windows не поддерживает технологию Windows VSS. В результате архивации виртуальной Машины согласованные приложения резервного копирования службы Azure, нельзя применять для виртуальных машин СУБД Db2 развертывается в.
...
Поддерживается Db2 высокий уровень доступности, аварийное восстановление (HADR). Если в конфигурации высокой доступности виртуальные машины имеют работающее разрешение имен, настройка в Azure не будет отличаться от настройки, которая выполняется локально. Мы не рекомендуем полагаться только на разрешение IP-адресов.
...
Для развертываний Db2 на Windows, настоятельно рекомендуется использовать функцию Azure, функции повышения производительности сети, как описано в документе [ускоренной сетью Azure](https://azure.microsoft.com/blog/maximize-your-vm-s-performance-with-accelerated-networking-now-generally-available-for-both-windows-and-linux/). Также обратите внимание на рекомендации, приведенные в документе [Вопросы развертывания СУБД для рабочей нагрузки SAP на виртуальных машинах Azure](dbms_guide_general.md). 
...
Для дисков, содержащих пути к хранилищу Db2, для которых и (saptmp) SAP каталогов необходимо указать физический диск размером сектора 512 КБ.
