...
ms.locfileid: "58763255"
---
# <a name="azure-hana-large-instances-control-through-azure-portal"></a>Контроль крупных экземпляров HANA в Azure через портал Azure
...
При отправке запроса развертывания крупных экземпляров HANA, вам будет предложено указать подписку Azure, вы подключаетесь к крупным экземплярам HANA также. Рекомендуется использовать ту же подписку, которая использовалась для развертывания уровень приложений SAP, который работает с единиц крупных экземпляров HANA.
...
Взглянув на различные атрибуты, указанные, эти атрибуты отличался едва ли атрибуты виртуальной Машины Azure. В заголовке левой стороне показаны группы ресурсов, регион Azure, имя подписки и идентификатор а также некоторые теги, которые были добавлены. По умолчанию единиц крупных экземпляров HANA имеют не назначен тег. В правой части заголовка имя единицы указывается, назначенный при было выполнено развертывание. А также IP-адрес отображается операционной системы. Как с виртуальными машинами, введите единицу крупного экземпляра HANA с числом ЦП потоки и память также отображаются сведения. Дополнительные сведения о разных единиц крупных экземпляров HANA, показаны ниже:
...
Первые несколько элементов данных, приведен в окне обзора. Но важной частью данных идентификатор канала ExpressRoute, который вы получаете при полученный первый развернутыми единицами, программисту. В некоторых случаях поддержка может получить запрос к этим данным. В нижней части экрана отображается запись важные данные. Данные, отображаемые является IP-адрес head хранилище NFS, который изолирует хранилище для вашего **клиента** в стеке крупных экземпляров HANA. Этот IP-адрес требуется также при изменении [файл конфигурации для хранения моментальных снимков резервных копий](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-backup-restore#set-up-storage-snapshots). 
