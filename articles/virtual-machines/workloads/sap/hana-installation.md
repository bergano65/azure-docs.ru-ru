...
**Первым шагом** после получения крупного экземпляра Hana и установления доступа и подключения к экземплярам является возврат портал Azure, отображаются ли экземпляры с правильными номерами SKU и ос в портал Azure. Чтение [крупных экземпляров Azure Hana с помощью портал Azure](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-li-portal) действий, необходимых для выполнения проверок.
