...
>[!NOTE]
> Если вы помещаете файлы журнала и файлы данных tempdb в созданную вами папку на диске D:\, убедитесь, что эта папка существует после перезагрузки виртуальной машины. С тех пор, как D: диск недавно инициализирован после перезагрузки VM все файлик и структуры каталога уничтожены. Возможность воссоздания возможных структур каталога на D: диск перед запуском службы сервера S'L описан в [этой статье](https://cloudblogs.microsoft.com/sqlserver/2014/09/25/using-ssds-in-azure-vms-to-store-sql-server-tempdb-and-buffer-pool-extensions/).
...
- Конфигурация аварийного восстановления из помещения в область Azure
- Сценарии пересечения из находиный до Azure. В этих случаях доставка журналов используется для синхронизации нового развертывания DBMS в Azure с текущей производственной системой на месте. В момент перехода рабочая среда завершает работу, и выполняется проверка передачи последних резервных копий журналов в развертывание СУБД Azure. Затем развертывание СУБД Azure открывается для рабочей среды.  
...
В тех случаях, когда вы выполняете неоднородную миграцию из другого DBMS, работающего на местах, в Windows/S'L Server, работающего в Azure, необходимо заблаговременно создать пустую целевую базу данных в сервере S'L. На следующем этапе вы примените функцию прозрачного шифрования данных для SQL Server, В то время как вы все еще работает ваша производственная система на месте. Причина такой последовательности в том, что процесс шифрования пустой базы данных может занять немало времени. Процессы импорта SAP импортируют данные в зашифрованную базу данных во время фазы простоя. Импорт зашифрованной базы данных требует гораздо меньше ресурсов, чем шифрование базы данных после завершения этапа экспорта в фазе простоя. У некоторых пользователей был негативный опыт применения прозрачного шифрования данных с нагрузкой SAP в базе данных. Поэтому мы рекомендуем выполнять прозрачное шифрование данных без рабочей нагрузки SAP в определенной базе данных.

В тех случаях, когда вы перемещаете базы данных SAP S'L Server из локализации в Azure, мы рекомендуем тестировать, на какой инфраструктуре можно быстрее применять шифрование. Для этого учитывайте следующие факты:
...
- В предыдущих выпусках SQL Server сжатие резервных копий уже не позволяло повысить эффективность при шифровании базы данных SQL Server. Такое поведение может перерасти в проблему, когда ваш план должен был зашифровать базу данных s'L Server на месте, а затем скопировать резервную копию в Azure для восстановления базы данных в Azure. Сжатие резервных копий SQL Server обычно имеет коэффициент 4.
...
Рассматривая применение шифрования TDE только без небольшой рабочей нагрузки SAP, следует проверить в вашей конкретной конфигурации, лучше ли применять TDE в вашей базе данных SAP на месте или сделать это в Azure. В Azure у вас определенно будет больше возможностей с точки зрения излишней подготовки инфраструктуры и сжатия инфраструктуры после применения прозрачного шифрования данных.
...
- [Расширяемое управление ключами с помощью Azure Key Vault (сервер S'L)](https://docs.microsoft.com/sql/relational-databases/security/encryption/extensible-key-management-using-azure-key-vault-sql-server?view=sql-server-2017).
...
- [Обслуживание подключения сервера sL & устранение неполадок.](https://docs.microsoft.com/sql/relational-databases/security/encryption/sql-server-connector-maintenance-troubleshooting?view=sql-server-2017)
...
## <a name="general-sql-server-for-sap-on-azure-summary"></a><a name="9053f720-6f3b-4483-904d-15dc54141e30"></a>Общий сервер S'L для SAP в резюме Azure
