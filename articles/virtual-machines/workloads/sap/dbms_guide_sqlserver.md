...
Перед продолжением рекомендуется ознакомиться с [этой][virtual-machines-sql-server-infrastructure-services] документацией.
...
* **Использование образов из Azure Marketplace.** Самый быстрый способ развернуть новую виртуальную машину Microsoft Azure — использовать образ из Azure Marketplace. В Azure Marketplace имеются образы, содержащие последние выпуски SQL Server. Образы, в которых уже установлен SQL Server, нельзя сразу же использовать для приложений SAP NetWeaver. Причина в том, что в таких образах заданы параметры сортировки по умолчанию для SQL Server, которые отличаются от параметров, требующихся для систем SAP NetWeaver. Чтобы использовать такие образы, проверьте действия, описанные в разделе [Использование образа SQL Server из Microsoft Azure Marketplace][dbms-guide-5.6]. 
...
* При использовании прослушивателя группы доступности виртуальные машины баз данных необходимо подключить к выделенному Load Balancer. Чтобы избежать назначения новых IP-адресов в Azure в случае, если обе виртуальные машины завершают работу, необходимо назначить статические IP-адреса сетевым интерфейсам этих виртуальных машин в конфигурации Always On (определение статического IP-адреса описывается в [этой][virtual-networks-reserved-private-ip] статье).
...
11. Установите и настройте Мониторинг узла SAP для Azure, как описано в разделе " [рекомендации по развертыванию][deployment-guide]".
