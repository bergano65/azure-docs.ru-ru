...
- Для SQL Server 2014: [Автоматическая архивация для виртуальных машин SQL Server 2014 (Resource Manager)](https://docs.microsoft.com/azure/virtual-machines/windows/sql/virtual-machines-windows-sql-automated-backup)
- Для SQL Server 2016/2017: [Автоматическая архивация версии 2 для виртуальных машин Azure (Resource Manager)](https://docs.microsoft.com/azure/virtual-machines/windows/sql/virtual-machines-windows-sql-automated-backup-v2)
...
* Выполните команду: Setup.exe /QUIET /ACTION=REBUILDDATABASE /INSTANCENAME=MSSQLSERVER /SQLSYSADMINACCOUNTS=`<local_admin_account_name`> /SQLCOLLATION=SQL_Latin1_General_Cp850_BIN2   
  * `<local_admin_account_name`> учетная запись, которая была определена как учетная запись администратора при развертывании виртуальной Машины в первый раз с помощью коллекции.
...
Если домен не поддерживается, то можно также применять сертификаты для базы данных, конечные точки зеркального отображения, как описано здесь: <https://docs.microsoft.com/sql/database-engine/database-mirroring/use-certificates-for-a-database-mirroring-endpoint-transact-sql>

Руководство для настройки зеркального отображения базы данных в Azure можно найти здесь: <https://docs.microsoft.com/sql/database-engine/database-mirroring/database-mirroring-sql-server> 
...
* Использование прослушивателя группы доступности возможно, только если в качестве гостевой ОС виртуальной машины используется Windows Server 2012 или более поздней версии. Для Windows Server 2012, необходимо убедиться, что следующее исправление: <https://support.microsoft.com/kb/2854082> 
...
В случаях, где перемещении баз данных SQL Server для SAP из локальной в Azure мы рекомендуем, тестирование производительности на какие инфраструктуры вы можете получить шифрования, используемых быстрым. Для этого учитывайте следующие факты:
...
- В предыдущих выпусках SQL Server сжатие резервных копий уже не позволяло повысить эффективность при шифровании базы данных SQL Server. Это поведение разработать проблем при был ваш план для шифрования SQL Server базы данных в локальной и скопируйте резервную копию в Azure, чтобы восстановить базу данных в Azure. Сжатие резервных копий SQL Server обычно имеет коэффициент 4.
...
Рассматривая Прозрачного шифрования данных нет на небольшой рабочей нагрузки для SAP только приложения, следует проверить в конкретной конфигурации, является ли лучше применить прозрачное шифрование данных для SAP базы данных в локальной или в Azure. В Azure у вас определенно будет больше возможностей с точки зрения излишней подготовки инфраструктуры и сжатия инфраструктуры после применения прозрачного шифрования данных.
