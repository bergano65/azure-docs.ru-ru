...
>[!NOTE]
> Если вы помещаете файлы журнала и файлы данных tempdb в созданную вами папку на диске D:\, убедитесь, что эта папка существует после перезагрузки виртуальной машины. С D:\ диск полностью инициализируется после перезагрузки виртуальной машины все структуры файлов и каталогов очищаются. Возможность воссоздать в конечном итоге структуры каталогов на D:\ диск перед запуском службы SQL Server описан в [этой статье](https://cloudblogs.microsoft.com/sqlserver/2014/09/25/using-ssds-in-azure-vms-to-store-sql-server-tempdb-and-buffer-pool-extensions/).
...
- Перерезание сценариев из локальной среды в Azure. В таких случаях доставка журналов используется для синхронизации нового развертывания СУБД в Azure с текущей рабочей системой в локальной среде. В момент перехода рабочая среда завершает работу, и выполняется проверка передачи последних резервных копий журналов в развертывание СУБД Azure. Затем развертывание СУБД Azure открывается для рабочей среды.  
...
- [Знакомство с SQL Server группы доступности Always on на виртуальных машинах Azure](https://docs.microsoft.com/azure/virtual-machines/windows/sql/virtual-machines-windows-portal-sql-availability-group-overview).
...
- [Настройте подсистему балансировки нагрузки для Always on группы доступности в Azure](https://docs.microsoft.com/azure/virtual-machines/windows/sql/virtual-machines-windows-portal-sql-alwayson-int-listener).
...
В случаях, когда выполняется разнородная миграция из другой СУБД, работающей локально, в Windows или SQL Server, работающий в Azure, необходимо создать пустую целевую базу данных в SQL Server заранее. На следующем этапе вы примените функцию прозрачного шифрования данных для SQL Server, Хотя Рабочая система по-прежнему выполняется локально. Причина такой последовательности в том, что процесс шифрования пустой базы данных может занять немало времени. Процессы импорта SAP импортируют данные в зашифрованную базу данных во время фазы простоя. Импорт зашифрованной базы данных требует гораздо меньше ресурсов, чем шифрование базы данных после завершения этапа экспорта в фазе простоя. У некоторых пользователей был негативный опыт применения прозрачного шифрования данных с нагрузкой SAP в базе данных. Поэтому мы рекомендуем выполнять прозрачное шифрование данных без рабочей нагрузки SAP в определенной базе данных.

В случаях, когда вы перемещаете базы данных SAP SQL Server из локальной среды в Azure, мы рекомендуем проверить, на какой инфраструктуре можно быстро применить шифрование. Для этого учитывайте следующие факты:
...
- В предыдущих выпусках SQL Server сжатие резервных копий уже не позволяло повысить эффективность при шифровании базы данных SQL Server. Это может привести к возникновению проблемы, когда ваш план зашифрует локальную базу данных SQL Server, а затем скопирует резервную копию в Azure для восстановления базы данных в Azure. Сжатие резервных копий SQL Server обычно имеет коэффициент 4.
...
Рассматривая применение шифрования TDE, но не только небольшое количество рабочих нагрузок SAP, следует протестировать конкретную конфигурацию, чтобы лучше применять TDE к локальной базе данных SAP или сделать это в Azure. В Azure у вас определенно будет больше возможностей с точки зрения излишней подготовки инфраструктуры и сжатия инфраструктуры после применения прозрачного шифрования данных.
...
- [Расширенное управление ключами с помощью Azure Key Vault (SQL Server)](https://docs.microsoft.com/sql/relational-databases/security/encryption/extensible-key-management-using-azure-key-vault-sql-server?view=sql-server-2017).
...
- [Соединитель SQL Server обслуживание & устранение неполадок](https://docs.microsoft.com/sql/relational-databases/security/encryption/sql-server-connector-maintenance-troubleshooting?view=sql-server-2017).
...
## <a name="general-sql-server-for-sap-on-azure-summary"></a><a name="9053f720-6f3b-4483-904d-15dc54141e30"></a>Общие сведения о SQL Server для SAP в Azure
