...
- Для HSR высокого уровня доступности клиентам необходимо настроить и настроить STONITH Device per SAP HANA HA Guide for [SLES](./high-availability-guide-suse-pacemaker.md) and [RHEL](./high-availability-guide-rhel-pacemaker.md).  Он не является предварительно настроенным как вариант ХЛИ.
...
| --- | --- | --- | --- |
| 1 | [Одноэкземплярная система с одним узлом](./hana-supported-scenario.md#single-node-with-one-sid) | Да | - |
| 2 | [Один узел с МКОС](./hana-supported-scenario.md#single-node-mcos) | Да | - |
| 3 | [Один узел с аварийным восстановлением с помощью репликации хранилища](./hana-supported-scenario.md#single-node-with-dr-using-storage-replication) | Нет | Репликация хранилища недоступна для виртуальной платформы Azure, измените текущее решение аварийного восстановления на HSR или резервное копирование или восстановление. |
| 4 | [Один узел с аварийным восстановлением (многоцелевые) с использованием репликации хранилища](./hana-supported-scenario.md#single-node-with-dr-multipurpose-using-storage-replication) | Нет | Репликация хранилища недоступна для виртуальной платформы Azure, измените текущее решение аварийного восстановления на HSR или резервное копирование или восстановление. |
| 5 | [HSR с STONITH для обеспечения высокой доступности](./hana-supported-scenario.md#hsr-with-stonith-for-high-availability) | Да | Нет предварительно настроенных SBD для целевых виртуальных машин.  Выберите и разверните решение STONITH.  Возможные варианты: агент ограждения Azure (поддерживается для [RHEL](./high-availability-guide-rhel-pacemaker.md), [SLES](./high-availability-guide-suse-pacemaker.md)), SBD |
| 6 | [Высокая доступность с помощью HSR, аварийное восстановление с репликацией хранилища](./hana-supported-scenario.md#high-availability-with-hsr-and-dr-with-storage-replication) | Нет | Замена репликации хранилища для нужд аварийного восстановления с помощью HSR или резервного копирования или восстановления |
| 7 | [Автоматическая отработка отказа узла (1+1)](./hana-supported-scenario.md#host-auto-failover-11) | Да | Использование использовании для общего хранилища с виртуальными машинами Azure |
| 8 | [Горизонтальное масштабирование с резервным узлом](./hana-supported-scenario.md#scale-out-with-standby) | Да | BW/4HANA с M128s, M416s, M416ms виртуальных машин, использующих использовании только для хранилища |
| 9 | [Горизонтальное масштабирование без резервного узла](./hana-supported-scenario.md#scale-out-without-standby) | Да | BW/4HANA с виртуальными машинами M128s, M416s, M416ms (с помощью или без использования использовании для хранилища) |
| 10 | [Масштабирование с помощью аварийного восстановления с использованием репликации хранилища](./hana-supported-scenario.md#scale-out-with-dr-using-storage-replication) | Нет | Замена репликации хранилища для нужд аварийного восстановления с помощью HSR или резервного копирования или восстановления |
| 11 | [Один узел с аварийным восстановлением с помощью HSR](./hana-supported-scenario.md#single-node-with-dr-using-hsr) | Да | - |
| 12 | [Один узел HSR в аварийное восстановление (оптимизированные затраты)](./hana-supported-scenario.md#single-node-hsr-to-dr-cost-optimized) | Да | - |
| 13 | [Высокая доступность и аварийное восстановление с помощью HSR](./hana-supported-scenario.md#high-availability-and-disaster-recovery-with-hsr) | Да | - |
| 14 | [HA и DR с HSR (оптимизированные затраты)](./hana-supported-scenario.md#high-availability-and-disaster-recovery-with-hsr-cost-optimized) | Да | - |
| 15 | [Горизонтальное масштабирование с помощью аварийного восстановления с использованием HSR](./hana-supported-scenario.md#scale-out-with-dr-using-hsr) | Да | BW/4HANA с M128s. M416s, M416ms виртуальные машины (с помощью или без использования использовании для хранилища) |
