...
После завершения подготовки командой Microsoft `Service Management` для сервера по умолчанию настраивается два расписания резервного копирования, чтобы создать резервную копию операционной системы на уровне файловой системы. Вы можете проверить расписания заданий резервного копирования с помощью следующей команды:
```
#crontab –l
```
