...
- **Сеть**: Изоляция клиентов в стеке инфраструктуры через виртуальные сети на одного клиента назначенного клиента. Клиент назначается только одному клиенту. Пользователь может иметь несколько клиентов. Сетевая изоляция клиентов запрещает взаимодействие между ними на уровне инфраструктуры, даже если клиенты принадлежат одному пользователю.
...
По состоянию на июль 2019 года мы различаем два различных изменения марок HANA Large Instance и расположение развертываний:

- "Ревизия 3" (Rev 3): Являются ли марки, которые были доступны для клиента, чтобы развернуть до июля 2019
- "Revision 4" (Rev 4): Новая марка, которая развернута в непосредственной близости от узлов Azure VM и которая до сих пор выпускалась в регионах Azure:
...
- [Обзор и архитектура SAP HANA (Большие инстанции) на Azure](hana-overview-architecture.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
...
- [SAP HANA (Большие инстанции) высокая доступность и аварийное восстановление на Azure](hana-overview-high-availability-disaster-recovery.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
- [SAP HANA (Большие инстанции) устранение и мониторинг на Azure](troubleshooting-monitoring.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
...
- [Резервное копирование ОС и восстановление для ТИП II SKUs пересмотра 3 марок](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/os-backup-type-ii-skus)
