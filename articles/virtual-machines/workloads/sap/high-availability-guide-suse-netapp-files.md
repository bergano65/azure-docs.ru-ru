...
Функция файлы Azure NetApp доступна в общедоступной предварительной версии в нескольких регионах Azure. Перед развертыванием службы файлов Azure NetApp, зарегистрироваться в предварительной версии Azure NetApp файлы, следуя [регистрации инструкции файлы Azure NetApp][anf-register]. 

### <a name="deploy-azure-netapp-files-resources"></a>Развертывание ресурсов Azure NetApp файлов  

Предполагается, что вы уже развернули [виртуальной сети Azure](https://docs.microsoft.com/en-us/azure/virtual-network/virtual-networks-overview). Имейте в виду, что ресурсы службы файлов Azure NetApp и виртуальные машины, куда будет подключен ресурсы службы файлов Azure NetApp, должны быть развернуты в той же виртуальной сети Azure.  

1. Если вы еще не сделали, запросить [подавать заявки на этапе предварительной версии Azure NetApp](https://docs.microsoft.com/en-gb/azure/azure-netapp-files/azure-netapp-files-register).  

2. Создайте учетную запись NetApp в выбранном регионе Azure, следуя [инструкции по созданию учетной записи NetApp](https://docs.microsoft.com/en-gb/azure/azure-netapp-files/azure-netapp-files-create-netapp-account).  
3. Настройте пул емкость NetApp службы файлов Azure, следуя [инструкции по настройке службы файлов Azure NetApp емкость пула](https://docs.microsoft.com/en-gb/azure/azure-netapp-files/azure-netapp-files-set-up-capacity-pool).  
...
4. Делегировать подсети, к файлам Azure NetApp, как описано в разделе [инструкции делегировать подсети в службу файлов Azure NetApp](https://docs.microsoft.com/en-gb/azure/azure-netapp-files/azure-netapp-files-delegate-subnet).  

5. Развертывание службы файлов Azure NetApp тома, в соответствии с [инструкции по созданию тома для службы файлов Azure NetApp](https://docs.microsoft.com/en-gb/azure/azure-netapp-files/azure-netapp-files-create-volumes). Выполнять развертывание томов в назначенным файлам Azure NetApp [подсети](https://docs.microsoft.com/en-us/rest/api/virtualnetwork/subnets). Имейте в виду, что ресурсы службы файлов Azure NetApp и виртуальные машины Azure должны находиться в той же виртуальной сети Azure. Например sapmnt<b>QAS</b>, usrsap<b>QAS</b>т. д. — это имена томов и sapmnt<b>qas</b>, usrsap<b>qas</b>т. д. являются filepaths для Azure Тома файлов NetApp.  
...
3. Проверка HAFailoverToNode
   ...
   Выполните следующие команды, как <sapsid>adm для переноса экземпляра ASCS.
