...
Исходный универсальный код ресурса (URI) портала выглядит так: http(s):`<Portalserver`>:5XX00/irj, где номер порта вычисляется как 50000 + (номер системы × 100). По умолчанию для системы SAP с номером 00 универсальный код ресурса (URI) будет выглядеть так: `<dns name`>.`<azure region`>.Cloudapp.azure.com:PublicPort/irj. Дополнительные сведения см. здесь: <https://help.sap.com/saphelp_nw70ehp1/helpdata/de/a2/f9d7fed2adc340ab462ae159d19509/frameset.htm>.
