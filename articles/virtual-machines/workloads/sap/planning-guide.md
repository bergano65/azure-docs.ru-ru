...
> [!NOTE]
> По умолчанию после развертывания виртуальной машины вы не можете изменять конфигурацию виртуальной сети. Параметры TCP/IP должны быть оставлены на DHCP-сервере Azure. Поведение по умолчанию — назначение динамических IP-адресов.
>
>
...
##### <a name="vpn-tunnel-device"></a>Устройство туннелирования VPN
...
#### <a name="multi-site-vpn"></a>VPN с несколькими сайтами
...
## <a name="different-ways-to-deploy-vms-for-sap-in-azure"></a>Различные способы развертывания ВМ для SAP в Azure
...
### <a name="remote-access-to-sap-systems"></a>Удаленный доступ к системам SAP
...
| Сервер сообщений |sapms`<sid`> см. ** |3600 |свободный sapms`<anySID`> |sid = ИД системы SAP |
| Шлюз |sapgw`<nn`> см. * |3301 |free |Шлюз SAP, используемый для взаимодействия CPIC и RFC |
| Маршрутизатор SAP |sapdp99 |3299 |free |Только имена службы CI (центрального экземпляра) можно заменить в файле /etc/services произвольными значениями после установки. |
...
### <a name="integration-of-azure-located-sap-instance-into-saprouter"></a>Интеграция размещенного в Azure экземпляра SAP в SAProuter
...
SAProuter позволяет взаимодействие по протоколу TCP/IP между участвующими системами, если отсутствует прямое соединение IP. Это преимущество избавляет от необходимости создавать полное подключение на уровне сети между партнерами по взаимодействию. SAProuter по умолчанию прослушивает порт 3299.
