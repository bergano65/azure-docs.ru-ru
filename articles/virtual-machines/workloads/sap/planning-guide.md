---
title: 'SAP на Azure: Руководство по планированию и внедрению'
...
Дополнительные ресурсы можно узнать в главе [«Ресурсы»][planning-guide-1.2] в этом документе.
...
Таким образом, предприятия должны тщательно продумать, какой поставщик облачных услуг выбрать для ведения таких критически важных бизнес-процессов. Azure является идеальной общедоступной облачной платформой для критически важных для бизнеса приложений SAP и бизнес-процессов. Учитывая широкий спектр инфраструктуры Azure, почти все существующие системы SAP NetWeaver и S/4HANA могут быть размещены в Azure уже сегодня. Azure предоставляет VMs с большим количеством терабайт памяти и более 200 процессоров. Помимо этого Azure предлагает [HANA Large Instances,](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-overview-architecture)которые позволяют масштабировать развертывание HANA до 24 ТБ и SAP HANA, масштабирование развертывания до 120 ТБ. Сегодня можно констатировать, что в Azure также можно выполнить практически все локальные сценарии SAP. 

Для точного описания сценариев и некоторых неподдерживаемых сценариев см. [SAP workload on Azure virtual machine supported scenarios](./sap-planning-supported-configurations.md)

Проверьте эти сценарии и некоторые условия, которые были названы не поддерживаемыми в справочной документации при планировании и разработке архитектуры, которые необходимо развернуть в Azure.

В целом наиболее распространенным шаблоном развертывания является кросс-предприим, как отображается

![VPN-подключение типа "сеть — сеть" (распределенное подключение)][planning-guide-figure-300]

Причина для многих клиентов применять кросс-предпосылок шаблон развертывания заключается в том, что наиболее прозрачным для всех приложений является расширение на основе помещения в Azure с помощью Azure ExpressRoute и относиться к Azure как к виртуальному центру обработки данных. По мере того, как все больше и больше ресурсов перемещаются в Azure, развертывание инфраструктуры и сетевой инфраструктуры Azure будет расти, а ресурсы на месте соответственно уменьшатся. Все прозрачно для пользователей и приложений.

Чтобы успешно развернуть системы SAP в Azure IaaS или любой другой службе IaaS, важно понимать, что предложения обычных аутсорсеров или хостеров существенно отличаются от предложений IaaS. В то время как традиционный хостер или аутсорсер адаптирует инфраструктуру (сеть, тип хранилища и сервера) к рабочей нагрузке, которую клиент хочет разместить, вместо этого клиент или партнер обязан охарактеризовать рабочую нагрузку и выбрать правильный Azure компоненты ВМ, хранения и сети для развертывания IaaS.

Для сбора данных для планирования развертывания в Azure важно:

- Оцените, какие продукты SAP поддерживаются в запущенных в Azure VMs
- Оцените, какие конкретные выпуски операционной системы поддерживаются с помощью конкретных выпусков Azure VMs для этих продуктов SAP
- Оцените, какие выпуски DBMS поддерживаются для ваших продуктов SAP с помощью специальных M-м и выв., вывешивают Azure
- Оцените, требуют ли некоторые из необходимых релизов OS/DBMS выполнять выпуск SAP, обновление пакета поддержки и обновление ядра, чтобы перейти к поддерживаемой конфигурации
- Оцените, нужно ли переходить на различные операционные системы для развертывания в Azure.

Подробная информация о поддерживаемых компонентах SAP в Azure, поддерживаемых блоками инфраструктуры Azure и связанных с ними выпусках операционных систем и dBMS-релизах разъясняется в статье [О том, что программное обеспечение SAP поддерживается для развертывания Azure.](./sap-supported-product-on-azure.md) Результаты оценки допустимых выпусков SAP, операционной системы и dBMS-релизов оказывают большое влияние на усилия по перемещению систем SAP в Azure. Результаты этой оценки определят, могут ли быть предприняты значительные усилия по подготовке в тех случаях, когда требуется обновление релиза SAP или изменения операционных систем.


## <a name="azure-regions"></a><a name="be80d1b9-a463-4845-bd35-f4cebdb5424a"></a>Регионы Azure
Службы Microsoft Azure собираются в регионах Azure. Регион Azure — это один или коллекция из центров обработки данных, которые содержат аппаратное обеспечение и инфраструктуру, которая запускает и размещает различные службы Azure. Эта инфраструктура включает в себя большое количество узлов, которые функционируют как вычислительные узлы или узлы хранения, или работают функциональность сети. 

Список различных регионов Azure можно уличить в статье [Azure .](https://azure.microsoft.com/global-infrastructure/geographies/) Не все регионы Azure предлагают одни и те же услуги. В зависимости от продукта SAP, который вы хотите запустить, и операционной системы и DBMS, связанных с ним, вы можете оказаться в ситуации, что определенный регион не предлагает VM типы, которые вам требуют. Это особенно верно для запуска SAP HANA, где вам обычно нужны vMs серии M/Mv2 VM. Эти семейства ВМ развернуты только в подмножестве регионов. Вы можете узнать, какие именно VM, типы типов, типы хранения Azure или другие службы Azure доступны в каких регионах с помощью сайта [Продукты доступны по регионам.](https://azure.microsoft.com/global-infrastructure/services/) При запуске планирования и наличии определенных регионов в качестве основного региона и, в конечном счете, вторичного региона необходимо сначала исследовать, доступны ли необходимые услуги в этих регионах. 

### <a name="availability-zones"></a>зоны доступности;
В некоторых регионах Azure реализована концепция под названием Зоны доступности. Зоны доступности — это физически разделенные расположения в пределах одного региона Azure. Каждая зона доступности состоит из одного или нескольких центров обработки данных, оснащенных независимыми системами электроснабжения, охлаждения и сетевого взаимодействия. Например, развертывание двух ВМ в двух зонах доступности Azure и внедрение платформы высокой доступности для системы SAP DBMS или SAP Central Services дают вам лучшую SLA в Azure. Для данной виртуальной машины SLA в Azure проверьте последнюю версию [виртуальных машин SLA](https://azure.microsoft.com/support/legal/sla/virtual-machines/). Поскольку в последние годы регионы Azure развивались и расширялись, топология регионов Azure, количество физических центров обработки данных, расстояние между этими центрами обработки данных и расстояние между зонами доступности Azure могут быть разными. И с этим задержка сети.

Принцип зон доступности не применяется к специфической службе [HANA крупных инстанций HANA.](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-overview-architecture) Соглашения об уровне обслуживания для крупных инстанций HANA можно найти в статье [SLA для SAP HANA на крупных инстанциях Azure](https://azure.microsoft.com/support/legal/sla/sap-hana-large/) 
...
При развертывании нескольких виртуальных машин в рамках одной системы SAP в microsoft Azure Virtual Machine Services можно повлиять на контроллер Azure Fabric Controller, чтобы развернуть приложение в различные домены неисправностей, тем самым удовлетворяя более высоким требованиям доступность SLAs. Но при этом вы не можете напрямую контролировать распределение доменов сбоя между единицами масштабирования Azure (наборами из сотен вычислительных узлов, узлов хранения и сетевых узлов) или распределение виртуальных машин между доменам сбоя. Для того чтобы направить контроллер ткани Azure на развертывание набора вс-м в различных доменах неисправностей, необходимо назначить набор доступности Azure для вс-ресурсов во время развертывания. Для получения дополнительной информации о наборах доступности Azure смотрите [наборы доступности Azure][planning-guide-3.2.3] в этом документе.


### <a name="upgrade-domains"></a><a name="fc1ac8b2-e54a-487c-8581-d3cc6625e560"></a>Обновление доменов
Домены обновления представляют логическую единицу, на основании которой определяется порядок обновления виртуальных машин, входящих в систему SAP, которая состоит из экземпляров SAP на нескольких виртуальных машинах. Когда происходит обновление, Microsoft Azure выполняет необходимые процессы поочередно для каждого домена обновления. Если вы во время развертывания распределите виртуальные машины по разным доменам обновления, это частично защитит систему SAP от потенциального простоя. Чтобы система Azure развернула виртуальные машины системы SAP в нескольких доменах обновления, следует задать специальный атрибут во время развертывания каждой виртуальной машины. Как и в случае с доменами сбоя, каждая единица масштабирования Azure включает множество доменов обновления. Чтобы контроллер структуры Azure развернул набор виртуальных машин в нескольких доменах обновления, вам следует назначить для виртуальных машин группу доступности Azure во время их развертывания. Для получения дополнительной информации о наборах доступности Azure приведены ниже [приведены наборы доступности Azure.][planning-guide-3.2.3]


### <a name="azure-availability-sets"></a><a name="18810088-f9be-4c97-958a-27996255c665"></a>Наборы доступности Azure
Виртуальные машины Azure в рамках одного набора доступных azure распространяются контроллером Azure Fabric по различным доменам неисправностей и обновлений. Распределение по разным доменам обновления и сбоя нужно для того, чтобы виртуальные машины системы SAP не завершали работу одновременно в ходе обслуживания инфраструктуры или в случае сбоя в одном домене сбоя. По умолчанию VMs не являются частью набора доступности. Участие VM в наборе доступности определяется во время развертывания или позже путем перенастройки и передислокации VM.

Чтобы понять концепцию наборов доступности Azure и способ, как наборы доступности относятся к доменам неисправностей и обновления, прочитайте [эту статью][virtual-machines-manage-availability]. 

При определении наборов доступности и попытке смешивать различные виртуальные машины различных семейств VM в пределах одного набора доступности могут возникнуть проблемы, которые мешают вам включить определенный тип VM в такой набор доступности. Причина в том, что набор доступности обязан масштабировать единицу, которая содержит определенный тип вычислительных узлов. И определенный тип вычислительного хоста может работать только определенные типы семейств VM. Например, если вы создадите набор доступности и развернете первый VM в набор доступности и вы выбираете тип VM семейства Esv3, а затем попытаетесь развернуть в качестве второго VM VM семейства M, вы будете отклонены во втором распределении. Причина в том, что VMs семейства Esv3 не работают на том же оборудовании, что и виртуальные машины семейства M. Та же проблема может возникнуть, когда вы пытаетесь изменить размер ВМ и попытаться переместить VM из семейства Esv3 в VM типа семьи M. В случае переразмерности в семейство VM, которое не может быть размещено на том же оборудовании хоста, необходимо отключить все вс-м в наборе доступности и изменить их размер, чтобы иметь возможность работать на другом типе машины-хозяина. Для SLAs виртуальных, которые развернуты в наборе доступности, проверьте статью [Виртуальные машины SLAs](https://azure.microsoft.com/support/legal/sla/virtual-machines/). 

Принцип набора доступности и соответствующего домена обновления и разлома не применяется к конкретной службе [HANA large Instances.](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-overview-architecture) Соглашения об уровне обслуживания для крупных инстанций HANA можно найти в статье [SLA для SAP HANA on Azure Large Instances](https://azure.microsoft.com/support/legal/sla/sap-hana-large/). 

> [!IMPORTANT]
> Концепции зон доступности Azure и наборов доступности Azure являются взаимоисключающими. Это означает, что можно развернуть пару или несколько вмвм в конкретную зону доступности или набор доступности Azure. Но не и то, и другое.


## <a name="azure-virtual-machine-services"></a>Услуги виртуальных машин Azure
Azure предлагает большое разнообразие виртуальных машин, которые можно выбрать для развертывания. Нет необходимости в авансовых закупках технологий и инфраструктуры. Лазурный VM-сервис, предлагающий услуги, упрощающие обслуживание и эксплуатацию приложений, предоставляя вычисления и хранение по требованию для размещения, масштабирования и управления веб-приложениями и подключенными приложениями. Управление инфраструктурой автоматизировано с платформой, которая предназначена для высокой доступности и динамического масштабирования в соответствии с потребностями использования с возможностью нескольких различных моделей ценообразования.

![Позиционирование служб виртуальных машин Microsoft Azure][planning-guide-figure-400]

С помощью виртуальных машин Azure корпорация Майкрософт позволяет развертывать пользовательские изображения серверов в Azure в качестве экземпляров IaaS. Или вы можете выбрать из богатого выбора расходных изображений операционной системы из галереи изображений Azure.

С точки зрения эксплуатации службы виртуальных машин Azure мало чем отличаются от виртуальных машин, развернутых на локальном компьютере. Вы несете ответственность за администрирование, операции, а также исправление конкретной операционной системы, работающей в Azure VM и его приложениях в том, что VM. Корпорация Майкрософт не предоставляет больше услуг, кроме хостинга, что VM на своей инфраструктуре Azure (Инфраструктура как услуга - IaaS). Для рабочей нагрузки SAP, которую вы как клиент развертываете, корпорация Майкрософт не предлагает больше, чем предложения IaaS. 

Платформа Microsoft Azure — это многотенантная платформа. В результате ресурсы хранения, сети и вычисления, хостующие вазовские визы, за некоторыми исключениями, совместно используются между арендаторами. Интеллектуальное регулирование и логика квотирования не позволяют клиентам существенно влиять на производительность, доступную для другого клиента ("шумный сосед"). Специально для сертификации платформы Azure для SAP HANA, Microsoft необходимо доказать изоляцию ресурсов в тех случаях, когда несколько VMs могут работать на одном и том же узеле на регулярной основе в SAP. Хотя логика в Azure пытается сохранить расхождения в пропускной способности опытных небольших, высоко распределенных платформ, как правило, вводят большие различия в доступности ресурсов / пропускной способности, чем клиенты могут возникнуть в их локальных развертываний. При планировании следует учитывать вероятность того, что система SAP в Azure будет испытывать более существенные отклонения, чем в локальной системе.

### <a name="azure-virtual-machines-for-sap-workload"></a>Виртуальные машины Azure для рабочей нагрузки SAP

Для рабочей нагрузки SAP мы сузили выбор до различных семейств VM, которые подходят для рабочей нагрузки SAP и рабочей нагрузки SAP HANA более конкретно. Как найти правильный тип VM и его способность работать с рабочей нагрузкой SAP описано в документе, что [программное обеспечение SAP поддерживается для развертывания Azure.](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-supported-product-on-azure) 

> [!NOTE]
> Типы VM, сертифицированные для рабочей нагрузки SAP, не переделывают cPU и ресурсы памяти.

Помимо выбора чисто поддерживаемых ВМ, вы также должны проверить, являются ли они доступны в определенном регионе на основе сайта [Продукты доступны по регионам](https://azure.microsoft.com/global-infrastructure/services/). Но что более важно, необходимо оценить, является ли:

- ЦП и ресурсы памяти различных типов VM 
- Пропускная способность IOPS различных типов VM
- Сетевые возможности различных типов VM
- Количество дисков, которые могут быть прикреплены
- Возможность использования определенных типов хранения Azure

соответствовать вашим потребностям. Большую часть этих данных можно найти [здесь (Linux)][virtual-machines-sizes-linux] и [здесь (Windows)][virtual-machines-sizes-windows] для конкретного типа VM.

В качестве модели ценообразования у вас есть несколько различных вариантов ценообразования, которые перечислять, как:

- с оплатой по мере использования;
- Один год зарезервирован
- Зарезервировано три года
- Точечные цены

Цены каждого из различных предложений с различными предложениями услуг вокруг операционных систем и различных регионах доступна на сайте [Linux Виртуальные машины Ценообразование](https://azure.microsoft.com/pricing/details/virtual-machines/linux/) и [Windows Виртуальные машины ценообразования](https://azure.microsoft.com/pricing/details/virtual-machines/windows/). Для получения подробной информации и гибкости одного года и трех лет зарезервированных экземпляров, проверьте следующие статьи:
...
Для получения дополнительной информации о спотовых цен, прочитайте статью [Azure Spot Виртуальные машины](https://azure.microsoft.com/pricing/spot/). Цены одного и того же типа VM также могут отличаться между различными регионами Azure. Некоторым клиентам стоило развернуть его в менее дорогом регионе Azure.

Кроме того, Azure предлагает концепции выделенного хоста. Специальная концепция узла дает вам больше контроля над циклами исправления, которые выполняются Azure. Вы можете время исправления в соответствии с вашим собственным графиком. Это предложение специально ориентировано на клиентов с рабочей нагрузкой, которая может не соответствовать обычному циклу рабочей нагрузки. Чтобы ознакомиться с концепциями предложений специального размещения Azure, прочитайте статью [Azure Dedicated Host](https://docs.microsoft.com/azure/virtual-machines/windows/dedicated-hosts). Использование этого предложения поддерживается для рабочей нагрузки SAP и используется несколькими клиентами SAP, которые хотят иметь больший контроль над исправлением инфраструктуры и возможными планами обслуживания корпорации Майкрософт. Для получения дополнительной информации о том, как корпорация Майкрософт поддерживает и патчи инфраструктуры Azure, которая хостов виртуальных машин, прочитайте статью [Обслуживание для виртуальных машин в Azure](https://docs.microsoft.com/azure/virtual-machines/maintenance-and-updates).

#### <a name="generation-1-and-generation-2-virtual-machines"></a>Виртуальные машины поколения 1 и 2
Гипервизор Microsoft способен обрабатывать два разных поколения виртуальных машин. Эти форматы называются **Поколение 1** и **Поколение 2**. **Поколение 2** было представлено в 2012 году с гипервизором Windows Server 2012. Azure начала использовать виртуальные машины поколения 1. При развертывании виртуальных машин Azure по умолчанию по-прежнему используется формат Generation 1. В то же время вы можете развернуть поколение 2 VM форматов, а также. В статье [«Поддержка ви-м визпоколения 2» в Azure](https://docs.microsoft.com/azure/virtual-machines/windows/generation-2) перечислены семейства VM Azure, которые могут быть развернуты как Поколение 2 VM. В этой статье также перечислены очень важные функциональные различия виртуальных машин поколения 2, поскольку они могут работать на частном облаке Hyper-V и Azure. В этой статье также перечислены функциональные различия между виртуальными машинами поколения 1 и VM-ми за йен, так как они работают в Azure. 

> [!NOTE]
> В Azure существуют функциональные различия в визажаниях в визажаниях поколения 1 и 2. Прочитайте статью [Поддержка для поколения 2 ВМ на Azure,](https://docs.microsoft.com/azure/virtual-machines/windows/generation-2) чтобы увидеть список этих различий.  
 
Перемещение существующего VM из одного поколения в другое невозможно. Чтобы изменить генерацию виртуальной машины, необходимо развернуть новый VM желаемого поколения и переустановить программное обеспечение, которое вы работаете в виртуальной машине поколения. Это влияет только на базовое изображение VM ИД и не влияет на диски данных или прикрепленные nFS или SMB-акции. Диски данных, NFS или SMB-акции, которые первоначально были назначены, например, на VM поколения 1

На данный момент вы столкнетесь с этой проблемой, особенно между VMs-ми-ми серии Azure M-Series и Mv2-Series VMs. Из-за ограничений в формате Generation 1 VM, большие VMs семейства Mv2 не могли быть предложены в формате Поколения 1, но должны быть предложены исключительно в поколении 2. С другой стороны, семейство M-Series VM еще не включено для развертывания в поколении 2. В результате, повторное увеличение размеров между M-серии и Mv2-серии виртуальных машин требует переустановки программного обеспечения в виртуальной машине, что вы ориентируетесь на другие семьи VM. Корпорация Майкрософт работает над тем, чтобы обеспечить развертывание VMs-ми-ми серий Для развертывания поколения 2. Развертывание M-серии VMs как поколение 2 VMs в будущем, собирается включить кажущейся менее повторного размера между M-серии и Mv2-серии виртуальных машин. В обоих направлениях, либо вверх-размер от M-серии до больших Mv2-серии виртуальных машин или вниз размера от больших Mv2-серии VMs для небольших M-серии VMs. Документация будет обновляться, как только M-серии ВМ могут быть развернуты в качестве поколения 2 VMs.    

 

### <a name="storage-microsoft-azure-storage-and-data-disks"></a><a name="a72afa26-4bf4-4a25-8cf7-855d6032157f"></a>Хранение: Хранение и диски данных Microsoft Azure
...
Виртуальные машины Azure предлагают временные диски после развертывания виртуальной машины. В случае перезагрузки VM все содержимое на этих дисках будет уничтожено. Таким образом, это учитывая, что файлы данных и журнал / переделать файлы баз данных ни при каких обстоятельствах должны быть расположены на этих неупорных дисков. Для некоторых баз данных возможны исключения: для них разрешается размещать на временных дисках табличные пространства tempdb и temp. Однако не следует использовать эти диски для виртуальных машин серии A, так как пропускная способность временных дисков с этим семейством виртуальных машин ограничена. Дополнительные сведения см. в статье [Сведения о временных дисках на виртуальных машинах Windows в Azure](https://blogs.msdn.microsoft.com/mast/2013/12/06/understanding-the-temporary-drive-on-windows-azure-virtual-machines/)
...
Можно загрузить существующий VHD из предварительного помещения или создать пустые из Azure и прикрепить эти VHD к развернутым VMs.
...
Поскольку эти VHD сохраняются, данные и изменения в этих VHDs являются безопасными при перезагрузке и воссоздании экземпляра виртуальной машины. Виртуальные жесткие диски остаются сохранными даже при удалении экземпляра. Позже их можно повторно развернуть или подключить к другим виртуальным машинам (кроме дисков с ОС).
...
Изучив часть [этой][virtual-machines-sizes-linux] (Linux) и [этой статьи][virtual-machines-sizes-windows] (Windows), посвященную виртуальным машинам серии DS, вы увидите, что существуют ограничения на объем тома данных для дисков в хранилище уровня Premium в зависимости от детализации уровня виртуальной машины. Различные VMs серии DS или GS-серии также имеют различные ограничения в количестве дисков данных, которые могут быть установлены. Эти ограничения описаны в упомянутой выше статье. Но по сути они означают, что вы не можете увеличить пропускную способность диска P30 в 32 раза, подключив 32 таких диска к одной виртуальной машине DS14. Пропускная способность ограничивается максимальным уровнем пропускной способности, указанным в статье для этого уровня виртуальной машины.
...
Более подробную информацию о учетных записях хранения Azure можно найти в [целевых показателях масштабируемости для стандартных учетных записей хранения данных](../../../storage/common/scalability-targets-standard-account.md) и [целевых показателях масштабируемости для учетных записей памяти премиум-страниц.](../../../storage/blobs/scalability-targets-premium-page-blobs.md) Читая эти статьи, вы понимаете, что существуют различия в ограничениях между стандартными учетными записями хранения Azure и учетными записями премиум-хранилищ. В первую очередь, различается объем данных, которые могут храниться в одной учетной записи хранения. В Стандартном хранилище объем больше, чем у Premium Storage. С другой стороны, учетная запись стандартного хранилища сильно ограничена в IOPS (см. колонку **Total Request Rate),** в то время как учетная запись премиум-хранилища Azure не имеет такого ограничения. Подробности и следствия этих различий мы опишем при обсуждении развертывания систем SAP, особенно серверов СУБД.
...
Управляемые диски — это ресурс нового типа в Azure Resource Manager, который можно использовать вместо виртуальных жестких дисков, хранимых в учетных записях хранения Azure. Управляемые диски автоматически согласуются с набором доступности виртуальной машины, к которой они прикреплены, и, следовательно, повышают доступность вашей виртуальной машины и услуг, работающих на виртуальной машине. Дополнительные сведения см. в [этой обзорной статье](https://docs.microsoft.com/azure/storage/storage-managed-disks-overview).
...
Для виртуальной машины Azure вы можете определить несколько виртуальных сетевых адаптеров (vNIC). Такая возможность позволяет настроить разделение сетевого трафика. Например, клиентский трафик можно направить через один виртуальный сетевой адаптер, а серверную часть трафика — через второй виртуальный сетевой адаптер. В зависимости от типа VM существуют различные ограничения для количества VNICs ВМ может быть назначен. Подробные сведения о параметрах, функциональных возможностях и ограничениях см. в следующих статьях:
...
* [Настройка подключения типа "точка — сеть" к виртуальной сети с использованием портала Azure](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-howto-point-to-site-resource-manager-portal)
...
**Шаг 3**. Сравните требования SAPS для сервера СУБД с возможностями SAPS, которые Azure предлагает для разных типов виртуальных машин. Сведения о SAPS для различных типов виртуальных машин Azure см. в примечании к SAP [1928533]. Важно сначала оценить виртуальную машину для СУБД, так как уровень базы данных в системе SAP NetWeaver для большинства развертываний не подлежит масштабированию. В отличие от этого, слой приложения SAP может быть уменьшен. Если ни один из поддерживаемых SAP типов Azure VM не может обеспечить требуемую SAPS, рабочая нагрузка запланированной системы SAP не может быть запущена в Azure. В этом случае следует развертывать систему локально или изменить объем рабочей нагрузки для системы.
...
Более подробные шаги по установке, обновлению и настройке cmdlets Azure PowerShell также можно найти в [этой главе Руководства по развертыванию.][deployment-guide-4.1]
...
Развертывание расширения Azure для SAP (см. главу [расширения Azure для SAP][planning-guide-9.1] в этом документе) возможно только через PowerShell или CLI. Поэтому при развертывании или администрировании системы SAP NetWeaver в Azure нужно обязательно установить и настроить PowerShell или интерфейс командной строки.  
...
Для клиентов, которые используют Linux и хотят управлять ресурсами Azure PowerShell, возможно, нет опции. Корпорация Майкрософт предлагает в качестве альтернативы интерфейс командной строки Azure.
...
* [Установка классического интерфейса командной строки Azure (Azure Classic CLI)][xplat-cli]
...
* [Управление ресурсами и группами ресурсов Azure с помощью интерфейса командной строки Azure][xplat-cli-azure-resource-manager]

Также читайте главу [Azure CLI для Linux VMs][deployment-guide-4.5.2] в [Руководстве по развертыванию][planning-guide] о том, как использовать Azure CLI для развертывания расширения Azure для SAP.


## <a name="first-steps-planning-a-deployment"></a>Первые шаги по планированию развертывания
Первым шагом в планировании развертывания является не проверка доступных для запуска SAP vMs. Первым шагом может быть тот, который занимает много времени, но самое главное, это работа с группами по соответствию и безопасности в вашей компании о том, какие условия границы для развертывания какого типа рабочей нагрузки SAP или бизнес-процесса в общедоступное облако. Если компания развернула другое программное обеспечение до того, как в Azure, процесс может быть простым. Если ваша компания находится в более начале пути, могут быть проведены более масштабные обсуждения, необходимые для того, чтобы выяснить условия границы, условия безопасности, которые позволяют размещать определенные данные SAP и бизнес-процессы SAP в общедоступном облаке.

В качестве полезной помощи вы можете указать на [предложения майкрософт](https://docs.microsoft.com/microsoft-365/compliance/offering-home) о соответствии для списка предложений, которые может предоставить корпорация Майкрософт. 

Другие области, вызывающие озабоченность, такие как шифрование данных для данных в состоянии покоя или другое шифрование в службе Azure, описаны в [обзоре шифрования Azure.](https://docs.microsoft.com/azure/security/fundamentals/encryption-overview)

Не стоит недооценивать этот этап проекта в планировании. Только при согласовании и правилах вокруг этой темы необходимо перейти к следующему этапу — планированию сетевой архитектуры, развертываемым в Azure.
...
* Для виртуальных жестких дисков, которые были подключены к виртуальной машине и должны быть снова подключены к виртуальной машине в Azure, также должен быть фиксированный формат. Прочитайте [эту статью (Linux)](../../linux/managed-disks-overview.md) и [эту статью (Windows)](../../windows/managed-disks-overview.md)для ограничения размера дисков данных. Динамические виртуальные жесткие диски, загружаемые с помощью командлетов PowerShell или интерфейса командной строки, будут преобразованы в статические виртуальные жесткие диски.
...
> ![Windows][Logo_Windows] Windows
>
> На последнем этапе войдите на виртуальную машину с помощью учетной записи администратора. Откройте окно команды Windows в качестве *администратора.* Перейдите к папке %windir%\windows\system32\sysprep и запустите программу sysprep.exe.
> Откроется небольшое окно. Важно проверить **опцию «Обобщение»** (по умолчанию не остановить) и изменить опцию выключения с его по умолчанию 'Перезагрузки' на 'закрытие'. В этой процедуре предполагается, что процесс sysprep выполняется локально в гостевой ОС виртуальной машины.
...
* Вопийте в подписке с *Connect-AzAccount*
* Установите подписку вашего контекста с *Set-AzContext* и параметр SubscriptionId или SubscriptionName - см.<https://docs.microsoft.com/powershell/module/az.accounts/set-Azcontext>
* Загрузите VHD с *помощью Add-AzVhd* на учетную запись хранения Azure - см.<https://docs.microsoft.com/powershell/module/az.compute/add-Azvhd>
* (Необязательно) Создание управляемого диска из VHD с *new-AzDisk* - см.<https://docs.microsoft.com/powershell/module/az.compute/new-Azdisk>
* Установите диск ОС нового VM-конфигурации на VHD или управляемый диск с *помощью Set-AzVMOSDisk* - см.<https://docs.microsoft.com/powershell/module/az.compute/set-Azvmosdisk>
* Создайте новый VM от VM-конфигурации с *New-AzVM* - см.<https://docs.microsoft.com/powershell/module/az.compute/new-Azvm>
* Добавление диска данных в новый VM с *Помощью-AzVMDataDisk* - см.<https://docs.microsoft.com/powershell/module/az.compute/add-Azvmdatadisk>

**Лазурный CLI**

* Войдите в подписку с помощью команды *az login*.
* Выберите подписку с *набором `<subscription name or id` учетных записей az-подписки*
...
* Загрузите VHD с Помощью PowerShell или Azure CLI
* (Необязательно) Создание управляемого диска из VHD с помощью PowerShell, Azure CLI или портала Azure
...
Чтобы загрузить существующий VM или VHD из предварительной сети, для того, чтобы использовать его в качестве изображения Azure VM, такое VM или VHD необходимо выполнить требования, перечисленные в главе [Подготовка к развертыванию VM с конкретным изображением клиента для SAP][planning-guide-5.2.2] этого документа.
...
* Установите подписку вашего контекста с *Set-AzContext* и параметр SubscriptionId или SubscriptionName - см.<https://docs.microsoft.com/powershell/module/az.accounts/set-Azcontext>
* Загрузите VHD с *помощью Add-AzVhd* на учетную запись хранения Azure - см.<https://docs.microsoft.com/powershell/module/az.compute/add-Azvhd>
* (Необязательно) Создание управляемого дискового изображения с VHD с *помощью New-AzImage* - см.<https://docs.microsoft.com/powershell/module/az.compute/new-Azimage>
* Задайте в качестве диска ОС в конфигурации новой виртуальной машины:
  * VHD с *Set-AzVMOSDisk -ИсточникImageUri -CreateOption fromImage* - см.<https://docs.microsoft.com/powershell/module/az.compute/set-Azvmosdisk>
  * Управляемый диск Изображения *Set-AzVMSourceImage* - см.<https://docs.microsoft.com/powershell/module/az.compute/set-Azvmsourceimage>
* Создайте новый VM от VM-конфигурации с *New-AzVM* - см.<https://docs.microsoft.com/powershell/module/az.compute/new-Azvm>
...
* Используйте *sysprep* в Windows или *waagent-deprovision* в Linux, чтобы подготовить к работе виртуальную машину. Процесс для Windows подробно описан в [техническом руководстве по Sysprep](https://technet.microsoft.com/library/cc766049.aspx), а для Linux — в разделе [Запись образа виртуальной машины Linux для его использования в качестве шаблона диспетчера ресурсов][capture-image-linux-step-2-create-vm-image].
* Загрузите VHD с Помощью PowerShell или Azure CLI
* (Необязательно) Создание управляемого дискового изображения с VHD с помощью PowerShell, Azure CLI или портала Azure
...
Во время загрузки виртуального жесткого диска или управляемого диска его нельзя использовать. А при загрузке диска, подключенного к виртуальной машине, следует отключить и освободить эту виртуальную машину. Если вы только хотите загрузить содержимое базы данных, которое, то должно быть использовано для настройки новой системы на месте, и если это приемлемо, что во время загрузки и установки новой системы, что система в Azure все еще может быть оперативной , вы могли бы избежать длительного простоя, выполняя резервную перепорку сжатой базы данных в диск и просто загрузить этот диск вместо того, чтобы также загружать базу ОС VM.
...
* Загрузка виртуального жесткого диска  
  После того, как система SAP остановлена и VM выключен, вы можете использовать Смдлет PowerShell Save-AzVhd на предварительной цели, чтобы загрузить диски VHD обратно в мир. Для этого следует определить URL-адрес виртуального жесткого диска, который можно найти в разделе хранилища на портале Azure (перейдите к учетной записи хранения и найдите тот контейнер хранения, в котором был создан нужный виртуальный жесткий диск). Также нужно определить, куда следует копировать виртуальный жесткий диск.
  ...
  Для получения более подробной информации о Save-AzVhd cmdlet, проверить здесь <https://docs.microsoft.com/powershell/module/az.compute/save-Azvhd>.
...
**Вывод: Для копирования или сохранения дисков, которые являются частью конфигурации системы SAP, необходимо остановить систему SAP, а также отключить развернутый VM. Только тогда вы можете скопировать или загрузить набор дисков, чтобы создать копию системы SAP в Azure или на месте.**
...
Для копирования виртуального жесткого диска вы можете использовать командлеты Azure PowerShell, как описано в [этой статье][storage-powershell-guide-full-copy-vhd]. Для создания нового управляемого диска используйте New-AzDiskConfig и New-AzDisk, как показано в следующем примере.
...
Для копирования VHD можно использовать Azure CLI. Для создания управляемого диска используйте команду *az disk create*, как показано в следующем примере.
...
##### <a name="powershell"></a>PowerShell
Вы также можете копировать виртуальные жесткие диски между подписками. Для получения дополнительной информации, прочитайте [эту статью][storage-powershell-guide-full-copy-vhd].

Далее представлен базовый логический поток командлета PowerShell.

* Создайте контекст учетной записи хранилища для учетной записи **исходного** хранилища с *помощью New-AzStorageContext* - см.<https://docs.microsoft.com/powershell/module/az.storage/new-AzStoragecontext>
* Создайте контекст учетной записи хранилища для **целевой** учетной записи хранилища с *помощью New-AzStorageContext* - см.<https://docs.microsoft.com/powershell/module/az.storage/new-AzStoragecontext>
...
Для примеров [см.][storage-powershell-guide-full-copy-vhd]
...
* Один или несколько дисков, которые содержат файл журнала DBMS базы данных SAP и файл журнала временного хранилища DBMS (если DBMS поддерживает это). Если журнал базы данных предъявляет высокие требования к скорости операций ввода-вывода, следует чередовать несколько дисков для достижения требуемого числа операций ввода-вывода в секунду.
...
> ![Linux][Logo_Linux] Linux
>
> * [Настройка программного обеспечения RAID на Linux][virtual-machines-linux-configure-raid]
...
>
> Если диски уже подключены, войдите на виртуальную машину и откройте диспетчер дисков Windows. Если автоматическое размещение не включено в соответствии с рекомендациями в главе [Настройка автоматического монтажа для прикрепленных дисков,][planning-guide-5.5.3]вновь прикрепленный том необходимо взять в режиме онлайн и инициализировать.
>
> ![Linux][Logo_Linux] Linux
>
...
Для окончательного развертывания и точных шагов, особенно в отношении развертывания расширения Azure для SAP, обратитесь к [Руководству по развертыванию.][deployment-guide]
...
1. Для подсети или сетевого интерфейса не определена группа безопасности сети. Сетевой трафик к виртуальным машинам Azure можно защитить с помощью так называемых "групп безопасности сети". Для получения дополнительной информации см. [Что такое группа сетевой безопасности (NSG)?][virtual-networks-nsg]
...
> Для этого:
>
> * Панель открытого управления (Open Control Panel) система и брандмауэр безопасности windows для **расширенных настроек.**
> * Теперь право нажмите на входящие правила и выбрал **новое правило**.
> * В следующем Волшебник решил создать новое правило **Порта.**
...
> * В следующей части мастера необходимо оставить элемент **Allow Connection** проверенным.
...
> * На последнем этапе мастера назовите правило и сохраните, нажав **на Finish.**
>
...
Графический пользовательский интерфейс SAP не подключается сразу к запущенным экземплярам SAP (через порт 32xx), а сначала подключается к процессу сервера сообщений SAP через открытый для него порт (порт 36xx). Ранее этот порт использовался сервером сообщений для внутреннего взаимодействия с экземплярами приложений. Чтобы предотвратить непреднамеренный обмен данными локальных серверов приложений с сервером сообщений в Azure, вы можете изменить порты внутренней связи. Настоятельно рекомендуется изменить внутреннюю связь между сервером сообщений SAP и его экземплярами приложения на другой номер порта на системах, которые были клонированы из внутренних систем, таких как клон разработки для тестирования проектов и т.д. Это можно сделать с параметром профиля по умолчанию:

> rdisp/msserv_internal
>
>

как описано в [настройках безопасности для сервера сообщений SAP](https://help.sap.com/saphelp_nwpi71/helpdata/en/47/c56a6938fb2d65e10000000a42189c/content.htm)
...
Шаблоны образцов можно использовать в репозитории Azure-quickstart-templates на GitHub.
...
* Налаживайте транспортные маршруты, группы и слои в обычном режиме.
...
Для того чтобы накормить часть инфраструктуры Azure критическиважных систем SAP в экземплярах SAP Host Agent, установленных в vMs, для развертывания VMS необходимо установить расширение Azure (VM) для SAP. Поскольку требования SAP были специфичны для приложений SAP, корпорация Майкрософт решила не вводить необходимую функциональность в Azure, а оставить пользователям развертывание необходимого расширения и конфигураций VM для своих виртуальных машин, работающих в Azure. Однако управление развертыванием и жизненным циклом расширения Azure VM для SAP будет в основном автоматизировано Azure.

#### <a name="solution-design"></a>Архитектура решения

Разработанное решение, позволяющее агенту SAP Host Agent получать необходимую информацию, основано на архитектуре платформы Azure VM Agent и Extension. Агент ВМ и платформа расширений используются для того, чтобы разрешить установку на виртуальной машине программных приложений, доступных в коллекции расширений для виртуальных машин Azure. Принципиальная идея этой концепции заключается в том, чтобы разрешить (в таких случаях, как расширение Azure для SAP), развертывание специальной функциональности в VM и конфигурацию такого программного обеспечения во время развертывания.

Агент ВМ Azure, который позволяет управлять на виртуальной машине некоторыми расширениями для виртуальных машин Azure, по умолчанию внедряется в виртуальные машины Windows при создании виртуальной машины на портале Azure. На виртуальных машинах SUSE, Red Hat или Oracle Linux агент и раньше входил в образ, доступный на Azure Marketplace. В случае, если можно загрузить Linux VM из наместера в Azure, агент VM должен быть установлен вручную.

Основные строительные блоки решения для предоставления информации об инфраструктуре Azure агенту SAP Host в Azure выглядят следующим образом:

![Компоненты расширения Microsoft Azure][planning-guide-figure-2400]

Как показано на диаграмме блока выше, одна часть решения размещается в галерее Azure VM Image и Azure Extension Gallery, которая является глобально реплицированным репозиторием, управляемым Azure Operations. Совместная группа SAP/MS, работающая над реализацией SAP Azure, обязана работать с Azure Operations для публикации новых версий расширения Azure для SAP.

Когда вы развертываете новую виртуальную машину Windows, в эту виртуальную машину автоматически добавляется агент виртуальной машины Azure. Функция этого агента заключается в координации загрузки и конфигурации расширенияaz ВМ. На виртуальных машинах Linux агент виртуальной машины Azure и раньше существовал в образе ОС, доступном в Azure Marketplace.
...
Чтобы подключить экземпляры SAP через SAProuter, необходимо дать строку SAProuter и имя хоста с любой попыткой подключения.
...
Первоначальный портал URI http(s):`<Portalserver`>:5XX00/irj, где порт формируется <https://help.sap.com/saphelp_nw70ehp1/helpdata/de/a2/f9d7fed2adc340ab462ae159d19509/frameset.htm>как документально SAP в .
...
* Высокая доступность серверов приложений (AS) SAP
  ...
  Экземпляры серверов приложений SAP являются избыточными компонентами. Каждый экземпляр SAP AS развертывается на отдельной виртуальной машине, которая работает в отдельных домене сбоя и домене обновления Azure (см. разделы [Домены сбоя][planning-guide-3.2.1] и [Домены обновления][planning-guide-3.2.2]). Это обеспечивается с помощью наборов доступности Azure (см. главу [Наборы доступности Azure).][planning-guide-3.2.3] Потенциальная недоступность домена сбоя или обновления Azure (запланированная или незапланированная) приведет к недоступности ограниченного количества виртуальных машин и их экземпляров SAP AS.
...
Для обеспечения полной доступности системы SAP нам необходимо защитить все критически важные компоненты системы SAP, например избыточные серверы приложений SAP, а также уникальные компоненты (например, Единая точка сбоя), такие как экземпляр SAP (A)SCS и DBMS.

#### <a name="high-availability-for-sap-application-servers"></a><a name="5d9d36f9-9058-435d-8367-5ad05f00de77"></a>Высокая доступность для серверов приложений SAP

В случае с экземплярами серверов приложений или диалогов SAP специальное решение для обеспечения высокой доступности не требуется. Высокая доступность достигается за счет избыточности и, как следствие, достаточного количества серверов на разных виртуальных машинах. Все они должны быть помещены в тот же набор доступности Azure, чтобы избежать того, что ВМ могут обновляться в то же время во время запланированного простоя обслуживания. Основные функциональные возможности, основанные на разделении доменов обновления и доменов сбоя в рамках единицы масштабирования Azure, уже были описаны выше в разделе [Домены обновления][planning-guide-3.2.2]. Наборы доступности Azure были представлены в [наборах доступности Azure][planning-guide-3.2.3] этого документа.

Не существует бесконечного количества доменов неисправностей и обновлений, которые могут быть использованы системой доступности Azure в блоке шкалы Azure. Это означает, что размещение нескольких всасок в один набор доступности рано или поздно более чем один VM попадает в тот же разлом или домен обновления.
...
* Разделение Виртуальных мв.к. одной системы SAP на три набора доступности возможно даже при всех виртуальных системах.
* Каждый слой (например, DBMS, ASCS, Application Servers) должен использовать специальный набор доступности.
* Все вм, работающие на экземплярах DBMS одной системы SAP, находятся в одном наборе доступности. Мы предполагаем, что используется несколько ВМ с экземплярами СУБД на систему, так как используются встроенные функции высокой доступности СУБД (например, SQL Server AlwaysOn или Oracle Data Guard).
...
* Все входной системы (A)SCS- экземпляр одной системы SAP находятся в одном наборе доступности. Для защиты экземпляра (A)SCS на этих ВМ настроена отказоустойчивая кластеризация Windows Server (WSFC).
...
* ВСЕ vMs, представляющие слой сервера приложений SAP, находятся в третьем наборе доступности.
...
* [Запуск и остановка SAP NetWeaver агентов по управлению](https://help.sap.com/saphelp_nwpi711/helpdata/en/49/9a15525b20423ee10000000a421938/content.htm)
...
В этой статье приводится пример того, как сделать сценарий этого процесса в PowerShell:<http://www.westerndevs.com/azure-snapshots/>
...
* Чтобы свести к минимуму проблемы в доменах неисправностей в инфраструктуре Azure или на обслуживании хоста, следует использовать наборы доступности Azure:
  * Рекомендуется иметь один набор доступности для слоя приложения SAP.
  * Рекомендуется иметь отдельный набор доступности для слоя SAP DBMS.
  * Не рекомендуется применять один и тот же набор доступности для vMs различных систем SAP.
...
* Резервное копирование VM, содержащего глобальный каталог системы SAP и вместе с ним все профили различных экземпляров, имеет смысл и должно выполняться с помощью резервного копирования Windows или, например, смолы на Linux. Так как между Windows Server 2008 (R2) и Windows Server 2012 (R2) существуют различия (в более новой версии Windows Server процесс резервного копирования упрощен), в качестве гостевой ОС мы рекомендуем использовать версию Windows Server 2012 (R2).
