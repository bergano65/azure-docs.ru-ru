...
* Во второй части подробно описаны различные сценарии, описанные в первой части.

Дополнительные ресурсы см. в разделе [ресурсы][planning-guide-1.2] в этом документе.
...
Таким образом, предприятия должны тщательно думать, какой поставщик облачных служб следует выбрать для выполнения таких критически важных бизнес-процессов. Azure — это идеальная Общедоступная облачная платформа для критически важных для бизнеса приложений SAP и бизнес-процессов. Учитывая широкий спектр инфраструктуры Azure, практически все существующие системы SAP NetWeaver и S/4HANA могут быть размещены в Azure уже сегодня. Azure предоставляет виртуальные машины с множеством терабайт памяти и более 200 ЦП. Помимо этого Azure предлагает [крупные экземпляры Hana](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-overview-architecture), которые позволяют развертывать развертывания Hana до 24TB и МАСШТАБИРОВАТЬ развертывание Hana до хранится 120 ТБ. Сегодня можно использовать практически все локальные сценарии SAP, которые также могут быть запущены в Azure. 

Более грубое описание сценариев и некоторые неподдерживаемые сценарии см. в статье Документирование [рабочей нагрузки SAP в поддерживаемых виртуальных машинах Azure](./sap-planning-supported-configurations.md).

Проверьте эти сценарии и некоторые условия, которые были названы как не поддерживаемые в документации по планированию и разработке архитектуры, которую вы хотите развернуть в Azure.

В общем, наиболее распространенным шаблоном развертывания является такой сценарий, как показано

![VPN-подключение типа "сеть — сеть" (распределенное подключение)][planning-guide-figure-300]

Многие клиенты применяют шаблон развертывания с распределенным развертыванием, так как он является наиболее прозрачным для всех приложений, чтобы расширить локальную среду в Azure с помощью Azure ExpressRoute и рассматривать Azure как виртуальный центр обработки данных. По мере того как все больше ресурсов перемещается в Azure, развернутая инфраструктура и сетевая инфраструктура Azure будут расти, а локальные ресурсы будут соответственно сокращены. Все, что прозрачно для пользователей и приложений.

Чтобы успешно развернуть системы SAP в Azure IaaS или любой другой службе IaaS, важно понимать, что предложения обычных аутсорсеров или хостеров существенно отличаются от предложений IaaS. В то время как традиционный поставщик услуг размещения или внешнего источника адаптирует инфраструктуру (сеть, хранилище и тип сервера) к рабочей нагрузке, которую клиент хочет разместить, это может быть обязанность клиента или партнера для того, чтобы изменять рабочую нагрузку и выбрать правильный Azure. Компоненты виртуальных машин, хранилища и сети для развертываний IaaS.

Чтобы собрать данные для планирования развертывания в Azure, важно:

- Оценка поддерживаемых продуктов SAP на виртуальных машинах Azure
...
- Оцените, требуется ли для некоторых из требуемых выпусков ОС и СУБД выполнение выпуска SAP, обновление пакетов поддержки и обновление ядра, чтобы получить поддерживаемую конфигурацию.
- Оцените, нужно ли переходить к разным операционным системам для развертывания в Azure.

Сведения о поддерживаемых компонентах SAP в Azure, поддерживаемых единицах инфраструктуры Azure и соответствующих выпусках операционных систем и выпусках СУБД, описаны в статье [что такое программное обеспечение SAP поддерживается для развертываний Azure](./sap-supported-product-on-azure.md). Результаты, полученные из оценки допустимых выпусков SAP, операционной системы и выпусков СУБД, оказывают значительное влияние на усилия по переносу систем SAP в Azure. Результатом этой оценки будет определение того, могут ли быть существенные подготовительные усилия в случаях, когда требуется обновление выпуска SAP или изменение операционных систем.


## <a name="azure-regions"></a><a name="be80d1b9-a463-4845-bd35-f4cebdb5424a"></a>Регионы Azure
Службы Microsoft Azure собираются в регионах Azure. Регион Azure — это одна или несколько коллекций из центров обработки данных, содержащих оборудование и инфраструктуру, которые работают и размещаются в разных службах Azure. Эта инфраструктура включает большое количество узлов, которые работают как ресурсы вычислений или узлы хранилища, а также выполняют сетевые функции. 

Список различных регионов Azure см. в статье [географические регионы Azure](https://azure.microsoft.com/global-infrastructure/geographies/). Не все регионы Azure предлагают одни и те же службы. В зависимости от продукта SAP, который вы хотите запустить, и операционной системы и СУБД, связанных с ней, вы можете поработать в ситуации, когда в определенном регионе не предлагаются требуемые типы виртуальных машин. Это особенно верно для запуска SAP HANA, где обычно требуются виртуальные машины серии виртуальных машин M/Mv2. Эти семейства виртуальных машин развертываются только в подмножестве регионов. Вы можете узнать, какие именно виртуальные машины, типы, типы хранения Azure или другие службы Azure доступны в тех регионах, где вы используете [продукты сайта, доступные по регионам](https://azure.microsoft.com/global-infrastructure/services/). По мере начала планирования и определенных регионов в качестве основного региона и в конечном счете, необходимо сначала исследовать, доступны ли необходимые службы в этих регионах. 

### <a name="availability-zones"></a>зоны доступности;
Некоторые регионы Azure реализовали концепцию, именуемую Зоны доступности. Зоны доступности — это физически разделенные расположения в пределах одного региона Azure. Каждая зона доступности состоит из одного или нескольких центров обработки данных, оснащенных независимыми системами электроснабжения, охлаждения и сетевого взаимодействия. Например, развертывание двух виртуальных машин в двух Зоны доступности Azure и реализация инфраструктуры высокого уровня доступности для системы СУБД SAP или центральных служб SAP обеспечивает наилучшее соглашение об уровне обслуживания в Azure. Для этого конкретного соглашения об уровне обслуживания для виртуальной машины в Azure проверьте последнюю версию [SLA](https://azure.microsoft.com/support/legal/sla/virtual-machines/). Так как регионы Azure были разработаны и расширены быстро за последние годы, топология регионов Azure, количество физических центров обработки данных, расстояние между ними и расстояние Зоны доступности Azure между ними могут отличаться. И с учетом задержки сети.

Принцип Зоны доступности не применяется к конкретной службе HANA [крупных экземпляров Hana](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-overview-architecture). Соглашения об уровне обслуживания для крупных экземпляров HANA можно найти в разделе [SLA для SAP HANA на крупных экземплярах Azure](https://azure.microsoft.com/support/legal/sla/sap-hana-large/) 
...
При развертывании нескольких виртуальных машин в составе одной системы SAP в Microsoft Azure службах виртуальных машин можно повлиять на контроллер структуры Azure на развертывание приложения в различных доменах сбоя, тем самым вызывая более высокие требования к Соглашения об уровне обслуживания. Но при этом вы не можете напрямую контролировать распределение доменов сбоя между единицами масштабирования Azure (наборами из сотен вычислительных узлов, узлов хранения и сетевых узлов) или распределение виртуальных машин между доменам сбоя. Чтобы контроллер структуры Azure мог развернуть набор виртуальных машин в разных доменах сбоя, необходимо назначить виртуальным машинам группу доступности Azure во время развертывания. Дополнительные сведения о группах доступности Azure см. в разделе [группы доступности Azure][planning-guide-3.2.3] в этом документе.
...
Виртуальные машины Azure в пределах одной группы доступности Azure распределяются контроллером структуры Azure по разным доменам сбоя и обновления. Распределение по разным доменам обновления и сбоя нужно для того, чтобы виртуальные машины системы SAP не завершали работу одновременно в ходе обслуживания инфраструктуры или в случае сбоя в одном домене сбоя. По умолчанию виртуальные машины не входят в группу доступности. Участие виртуальной машины в группе доступности определяется во время развертывания или позже при повторной настройке и повторном развертывании виртуальной машины.

Сведения о концепциях групп доступности Azure и способах связи групп доступности с доменами сбоя и обновления см. в [этой статье][virtual-machines-manage-availability]. 

При определении групп доступности и попытке смешивать различные виртуальные машины разных семейств виртуальных машин в одной группе доступности могут возникнуть проблемы, которые не позволяют включить определенный тип виртуальной машины в такую группу доступности. Причина заключается в том, что группа доступности привязана к единице масштабирования, содержащей определенный тип узлов вычислений. А определенный тип узла вычислений может выполнять только определенные типы семейств виртуальных машин. Например, если вы создаете группу доступности и развертываете первую виртуальную машину в группе доступности и выбираете тип виртуальной машины семейства Esv3, а затем пытаетесь выполнить развертывание в качестве второй виртуальной машины семейства M, вы будете отклоняться при втором выделении. Причина заключается в том, что виртуальные машины семейства Esv3 не работают на одном оборудовании с виртуальными машинами семейства M. Эта же проблема может возникнуть при попытке изменить размер виртуальных машин и попытаться переместить виртуальную машину из семейства Esv3 в тип виртуальной машины семейства M. В случае изменения размера для семейства виртуальных машин, которое не может быть размещено на одном и том же оборудовании, необходимо завершить работу всех виртуальных машин в группе доступности и изменить их размер, чтобы они могли работать на другом компьютере типа. Для соглашений об уровне обслуживания виртуальных машин, развернутых в группе доступности, см. статью [соглашения об уровне обслуживания для виртуальной машины](https://azure.microsoft.com/support/legal/sla/virtual-machines/). 

Принцип группы доступности и связанного обновления и домена сбоя не применяется к конкретной службе HANA [крупных экземпляров Hana](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-overview-architecture). Соглашения об уровне обслуживания для крупных экземпляров HANA можно найти в разделе [SLA для SAP HANA на крупных экземплярах Azure](https://azure.microsoft.com/support/legal/sla/sap-hana-large/). 

> [!IMPORTANT]
> Понятия Зоны доступности Azure и групп доступности Azure являются взаимоисключающими. Это означает, что можно либо развернуть пару или несколько виртуальных машин в определенной зоне доступности, либо группу доступности Azure. Но не оба.


## <a name="azure-virtual-machine-services"></a>Службы виртуальных машин Azure
Azure предлагает большое количество виртуальных машин, которые можно выбрать для развертывания. Нет необходимости в приобретении высокопроизводительных технологий и инфраструктуры. Предложение службы виртуальных машин Azure упрощает обслуживание и эксплуатацию приложений, предоставляя ресурсы по запросу и хранилища для размещения, масштабирования и управления веб-приложениями и подключенными приложениями. Управление инфраструктурой автоматизировано с помощью платформы, предназначенной для обеспечения высокого уровня доступности и динамического масштабирования в соответствии с потребностями использования с возможностью применения нескольких различных моделей ценообразования.

![Размещение служб виртуальных машин Microsoft Azure][planning-guide-figure-400]

С помощью виртуальных машин Azure Корпорация Майкрософт позволяет развертывать пользовательские образы сервера в Azure как экземпляры IaaS. Или вы можете выбрать из коллекции образов Azure широкий выбор используемых образов операционных систем.

С точки зрения эксплуатации службы виртуальных машин Azure мало чем отличаются от виртуальных машин, развернутых на локальном компьютере. Вы несете ответственность за администрирование, выполнение операций, а также установку исправлений определенной операционной системы, работающей на ВИРТУАЛЬНОЙ машине Azure и ее приложениях на этой ВМ. Корпорация Майкрософт не предоставляет больше служб, кроме размещения этой виртуальной машины в своей инфраструктуре Azure (инфраструктура как услуга — IaaS). Для рабочей нагрузки SAP, которую вы развертываете в качестве клиента, корпорация Майкрософт не предлагает никаких предложений. 

Microsoft Azureная платформа — это платформа с несколькими клиентами. В качестве результирующего хранилища, сети и вычислений, в которых размещаются виртуальные машины Azure, есть несколько исключений, совместно используемых клиентами. Интеллектуальное регулирование и логика квотирования не позволяют клиентам существенно влиять на производительность, доступную для другого клиента ("шумный сосед"). В частности, для сертификации платформы Azure для SAP HANA корпорации Майкрософт необходимо доказать изоляцию ресурсов в случаях, когда несколько виртуальных машин могут выполняться на одном узле на регулярной основе в SAP. Несмотря на то, что логика в Azure пытается удержать дисперсию пропускной способности небольших, высокообщие платформы обычно применяют большие дисперсии доступности ресурсов или пропускной способности, чем клиенты могут столкнуться с локальными развертываниями. При планировании следует учитывать вероятность того, что система SAP в Azure будет испытывать более существенные отклонения, чем в локальной системе.

### <a name="azure-virtual-machines-for-sap-workload"></a>Виртуальные машины Azure для рабочей нагрузки SAP

Для рабочей нагрузки SAP мы разделили выборку с различными семействами виртуальных машин, которые подходят для рабочей нагрузки SAP и SAP HANA более конкретно. Способ поиска правильного типа виртуальной машины и его возможности для работы с рабочей нагрузкой SAP описывается в документе, [что программное обеспечение SAP поддерживается для развертываний Azure](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-supported-product-on-azure). 

> [!NOTE]
> Типы виртуальных машин, сертифицированные для рабочей нагрузки SAP, не имеют избыточной подготовки ресурсов ЦП и памяти.

Помимо выбора только поддерживаемых виртуальных машин, необходимо также проверить, доступны ли они в определенном регионе, на основе [продуктов, доступных по регионам](https://azure.microsoft.com/global-infrastructure/services/). Но, что важнее, необходимо оценить следующее:

- Ресурсы ЦП и памяти различных типов виртуальных машин 
- Пропускная способность операций ввода-вывода для разных типов виртуальных машин
- Сетевые возможности различных типов виртуальных машин
- Число дисков, которые могут быть присоединены
- Возможность использовать определенные типы хранилищ Azure

в соответствии с вашими нуждами. Большинство этих данных можно найти [здесь (Linux)][virtual-machines-sizes-linux] и [здесь (Windows)][virtual-machines-sizes-windows] для конкретного типа виртуальной машины.

В качестве модели ценообразования можно использовать несколько различных вариантов ценообразования, например:

- с оплатой по мере использования;
- Зарезервировано на один год
- Зарезервировано три года
- Цены на точки
...
- [Как скидка на резервирование Azure применяется к виртуальным машинам](https://docs.microsoft.com/azure/cost-management-billing/manage/understand-vm-reservation-charges) 

Дополнительные сведения о ценах см. в статье [виртуальные машины Azure для машинного](https://azure.microsoft.com/pricing/spot/)характера. Цены на один и тот же тип виртуальной машины также могут отличаться в разных регионах Azure. Для некоторых клиентов было необходимо выполнить развертывание в менее дорогостоящем регионе Azure.

Кроме того, Azure предлагает основные понятия выделенного узла. Концепция выделенного узла предоставляет больше возможностей управления циклами исправлений, которые выполняются в Azure. Вы можете устанавливать исправления в соответствии с собственными расписаниями. Это предложение специально предназначено для клиентов с рабочей нагрузкой, которая может не соответствовать нормальному циклу рабочей нагрузки. Сведения о концепциях выделенного узла Azure см. в статье [выделенный узел Azure](https://docs.microsoft.com/azure/virtual-machines/windows/dedicated-hosts). Использование этого предложения поддерживается для рабочей нагрузки SAP и используется несколькими клиентами SAP, которым требуется более полный контроль над исправлениями инфраструктуры и планов обслуживания в конечном счете корпорации Майкрософт. Дополнительные сведения о том, как корпорация Майкрософт сохраняет и обновляет инфраструктуру Azure, в которой размещены виртуальные машины, см. в статье [Обслуживание виртуальных машин в Azure](https://docs.microsoft.com/azure/virtual-machines/maintenance-and-updates).

#### <a name="generation-1-and-generation-2-virtual-machines"></a>Виртуальные машины поколения 1 и 2
Низкоуровневая оболочка Майкрософт может работать с двумя различными поколениями виртуальных машин. Эти форматы называются **поколением 1** и **поколением 2**. **Поколение 2** было представлено в 2012 с помощью низкоуровневой оболочки Windows Server 2012. Azure запущен с использованием виртуальных машин поколения 1. При развертывании виртуальных машин Azure по умолчанию по-прежнему используется формат поколения 1. В то же время можно также развернуть форматы виртуальных машин поколения 2. В статье [Поддержка виртуальных машин поколения 2 в Azure](https://docs.microsoft.com/azure/virtual-machines/windows/generation-2) перечислены семейства виртуальных машин Azure, которые можно развернуть как виртуальную машину поколения 2. В этой статье также перечислены важные функциональные различия виртуальных машин версии 2, которые могут выполняться в частном облаке Hyper-V и Azure. Более важно в этой статье также перечислены функциональные различия между виртуальными машинами поколения 1 и ВМ поколения 2, как они выполняются в Azure. 

> [!NOTE]
> Существуют функциональные различия между виртуальными машинами поколения 1 и 2, работающими в Azure. Ознакомьтесь со статьей [Поддержка виртуальных машин поколения 2 в Azure](https://docs.microsoft.com/azure/virtual-machines/windows/generation-2) , чтобы просмотреть список этих различий.  
 
Перемещение существующей виртуальной машины из одного поколения в другое невозможно. Чтобы изменить поколение виртуальных машин, необходимо развернуть новую виртуальную машину требуемого поколения и повторно установить программное обеспечение, которое вы используете на виртуальной машине этого поколения. Это влияет только на базовый образ VHD виртуальной машины и не влияет на диски данных или подключенные общие папки NFS или SMB. Диски данных, NFS или общие папки SMB, которые изначально были назначены, например, на виртуальной машине поколения 1;

В данный момент эта проблема будет возникать в особенности между виртуальными машинами Azure серии M и Mv2. Из-за ограничений в формате виртуальной машины поколения 1 крупные виртуальные машины семейства Mv2 не могут быть предложены в формате поколения 1, но должны быть предложены исключительно в поколении 2. С другой стороны, семейство виртуальных машин серии M еще не включено для развертывания в поколении 2. В результате изменение размера между виртуальными машинами серии M и Mv2 требует повторной установки программного обеспечения на виртуальной машине, предназначенной для другого семейства виртуальных машин. Корпорация Майкрософт работает над развертыванием виртуальных машин серии M для развертываний поколения 2. Развертывание виртуальных машин серии M в качестве виртуальных машин поколения 2 в будущем позволит вам меньше изменять размер между виртуальными машинами серии M и Mv2. Как в обоих направлениях, так и в любом случае — от серии M до крупных виртуальных машин серии Mv2 или уменьшения размера с более крупных виртуальных машин серии Mv2 до небольших виртуальных машин серии M. Документация будет обновлена, как только виртуальные машины серии M можно будет развернуть как виртуальные машины версии 2.    
...
Виртуальные машины Azure предлагают временные диски после развертывания виртуальной машины. В случае перезагрузки виртуальной машины все содержимое этих дисков будет очищено. Таким образом, это значит, что файлы данных и файлы журнала/повтора баз данных не должны располагаться на этих нематериализованных дисках. Для некоторых баз данных возможны исключения: для них разрешается размещать на временных дисках табличные пространства tempdb и temp. Однако не следует использовать эти диски для виртуальных машин серии A, так как пропускная способность временных дисков с этим семейством виртуальных машин ограничена. Дополнительные сведения см. в статье [Сведения о временных дисках на виртуальных машинах Windows в Azure](https://blogs.msdn.microsoft.com/mast/2013/12/06/understanding-the-temporary-drive-on-windows-azure-virtual-machines/)
...
Можно передать существующий виртуальный жесткий диск из локальной среды или создать пустые из Azure и подключить эти виртуальные жесткие диски к развернутым виртуальным машинам.
...
По мере сохранения этих виртуальных жестких дисков данные и изменения в этих виртуальных жестких дисках могут быть надежными при перезагрузке и повторном создании экземпляра виртуальной машины. Виртуальные жесткие диски остаются сохранными даже при удалении экземпляра. Позже их можно повторно развернуть или подключить к другим виртуальным машинам (кроме дисков с ОС).
...
#### <a name="azure-premium-storage"></a><a name="ff5ad0f9-f7f4-4022-9102-af07aef3bc92"></a>Хранилище Azure уровня "Премиум"
...
Если вы извлеки часть виртуальных машин серии DS в [этой статье (Linux)][virtual-machines-sizes-linux] и в [этой статье (Windows)][virtual-machines-sizes-windows], то понимаете, что существуют ограничения объема данных для дисков хранилища класса Premium на уровне гранулярности виртуальной машины. На разных виртуальных машинах серии DS или GS также существуют разные ограничения числа дисков данных, которые можно подключить. Эти ограничения описаны в упомянутой выше статье. Но по сути они означают, что вы не можете увеличить пропускную способность диска P30 в 32 раза, подключив 32 таких диска к одной виртуальной машине DS14. Пропускная способность ограничивается максимальным уровнем пропускной способности, указанным в статье для этого уровня виртуальной машины.
...
#### <a name="managed-disks"></a><a name="c55b2c6e-3ca1-4476-be16-16c81927550f"></a>Управляемые диски

Управляемые диски — это ресурс нового типа в Azure Resource Manager, который можно использовать вместо виртуальных жестких дисков, хранимых в учетных записях хранения Azure. Управляемые диски автоматически согласовываются с набором доступности виртуальной машины, к которой они подключены, и поэтому увеличивают доступность виртуальной машины и служб, работающих на виртуальной машине. Дополнительные сведения см. в [этой обзорной статье](https://docs.microsoft.com/azure/storage/storage-managed-disks-overview).
...
Есть множество разных способов настроить в Azure разрешение имен и IP-адресов. Есть также возможность использовать службу Azure DNS вместо настройки собственного DNS-сервера. Дополнительные сведения можно найти в [этой статье][virtual-networks-manage-dns-in-vnet] и на [этой странице](https://azure.microsoft.com/services/dns/).
...
Поскольку сеть и разрешение имен являются важной частью развертывания базы данных для системы SAP, эта концепция более подробно обсуждается [в этом разделе.][dbms-guide]
...
Дополнительные сведения можно найти в [этой статье][resource-groups-networking] и на [этой странице](https://azure.microsoft.com/documentation/services/virtual-network/).
...
Для виртуальной машины Azure вы можете определить несколько виртуальных сетевых адаптеров (vNIC). Такая возможность позволяет настроить разделение сетевого трафика. Например, клиентский трафик можно направить через один виртуальный сетевой адаптер, а серверную часть трафика — через второй виртуальный сетевой адаптер. В зависимости от типа виртуальной машины существует ряд различных ограничений на количество vNIC, которое может быть назначено виртуальной машине. Подробные сведения о параметрах, функциональных возможностях и ограничениях см. в следующих статьях:

* [Создание виртуальной машины Windows с несколькими сетевыми картами][virtual-networks-multiple-nics-windows]
* [Создание виртуальной машины Linux с несколькими сетевыми картами][virtual-networks-multiple-nics-linux]
...
Дополнительную документацию см. в [этой статье][vpn-gateway-create-site-to-site-rm-powershell] .
...
При планировании и определении размеров решений SAP в Azure следует учитывать квоты для каждого размера виртуальной машины. Квоты виртуальных машин описаны [здесь (Linux)][virtual-machines-sizes-linux] и [здесь (Windows)][virtual-machines-sizes-windows].
...
**Шаг 3**. Сравните требования SAPS для сервера СУБД с возможностями SAPS, которые Azure предлагает для разных типов виртуальных машин. Сведения о SAPS для различных типов виртуальных машин Azure см. в примечании к SAP [1928533]. Важно сначала оценить виртуальную машину для СУБД, так как уровень базы данных в системе SAP NetWeaver для большинства развертываний не подлежит масштабированию. Уровень приложений SAP, напротив, можно масштабировать. Если ни один из типов виртуальных машин Azure, поддерживаемых SAP, не может доставить требуемый SAPS, то Рабочая нагрузка запланированной системы SAP не может быть запущена в Azure. В этом случае следует развертывать систему локально или изменить объем рабочей нагрузки для системы.

**Шаг 4**. как описано [здесь (Linux)][virtual-machines-sizes-linux] и [здесь (Windows)][virtual-machines-sizes-windows], Azure принудительно ПРИменяет квоту операций ввода-вывода на диск независимо от того, используется ли хранилище уровня "Стандартный" или "Премиум". В зависимости от типа виртуальной машины вы сможете подключить разное количество дисков данных. Таким образом, вы можете вычислить максимальное количество операций ввода-вывода в секунду, достижимое для всех типов виртуальных машин. В зависимости от структуры файлов базы данных вы можете использовать чередующиеся диски для одного тома в гостевой ОС. Но если фактическая скорость операций ввода-вывода в развернутой системе SAP превышает вычисленное максимальное значение для самого крупного типа виртуальных машин Azure, и у вас нет возможности компенсировать это значение увеличением объема памяти, рабочая нагрузка системы SAP может серьезно пострадать. Такое сочетание условий означает, что эту систему не следует развертывать в Azure.
...
В [этой статье][powershell-install-configure]описывается процесс включения локального рабочего стола или ноутбука для использования командлетов Azure PowerShell и настройки этих параметров для использования с подписками Azure.

Более подробные инструкции по установке, обновлению и настройке командлетов Azure PowerShell также можно найти в [этой главе руководства по развертыванию][deployment-guide-4.1].
...
Развертывание расширения Azure для SAP (см. раздел [расширение Azure для SAP][planning-guide-9.1] в этом документе) возможно только с помощью POWERSHELL или CLI. Поэтому при развертывании или администрировании системы SAP NetWeaver в Azure нужно обязательно установить и настроить PowerShell или интерфейс командной строки.  
...
Для клиентов, использующих Linux и желающих управлять ресурсами Azure PowerShell, может быть недоступен. Корпорация Майкрософт предлагает в качестве альтернативы интерфейс командной строки Azure.
...
* [Установка классической инфраструктуры CLI Azure][xplat-cli]
...
* [Использование классической инфраструктуры CLI Azure для Mac, Linux и Windows с Azure Resource Manager][xplat-cli-azure-resource-manager]

Также прочитайте главу [Azure CLI для виртуальных машин Linux][deployment-guide-4.5.2] в разделе [руководство по развертыванию][planning-guide] , где описано, как использовать Azure CLI для развертывания расширения Azure для SAP.


## <a name="first-steps-planning-a-deployment"></a>Первые шаги по планированию развертывания
Первый шаг в планировании развертывания — не проверять наличие виртуальных машин, доступных для запуска SAP. Первым шагом может быть использование времени, которое, однако, самое важное, — работать с группами обеспечения соответствия и безопасности в вашей компании с учетом требований к развертыванию типа рабочей нагрузки SAP или бизнес-процесса в общедоступном облаке. Если ваша компания развернула другое программное обеспечение до Azure, процесс может быть простым. Если ваша компания больше в начале поездки, то для определения граничных условий, условий безопасности, позволяющих размещать определенные данные SAP и бизнес-процессы SAP в общедоступном облаке, может потребоваться больше обсуждений.

В качестве полезной справки можно указать предложения по обеспечению [соответствия Майкрософт](https://docs.microsoft.com/microsoft-365/compliance/offering-home) , чтобы получить список предложений по соответствию, которые корпорация Майкрософт может предоставить. 

Другие области, такие как шифрование данных для неактивных или других средств шифрования в службе Azure, описаны в статье [Общие сведения о шифровании Azure](https://docs.microsoft.com/azure/security/fundamentals/encryption-overview).

Не следует недооценивать этот этап проекта в планировании. Только при наличии соглашения и правил, связанных с этим разделом, необходимо перейти к следующему шагу, который является планированием сетевой архитектуры, развертываемой в Azure.
...
Предположим, вы хотите переместить определенную систему SAP из локальной среды в Azure. Для этого можно передать в Azure виртуальный жесткий диск, который содержит ОС и двоичные файлы SAP и СУБД, а также дополнительные виртуальные жесткие диски с файлами данных и журналов СУБД. В отличие от [сценария #2 ниже][planning-guide-5.1.2], вы сохраняете имя узла, идентификатор безопасности SAP и учетные записи пользователей SAP на виртуальной машине Azure, так как они были настроены в локальной среде. В этом случае создание универсального образа не требуется. См. главы [Подготовка к перемещению виртуальной машины из локальной среды в Azure с помощью необобщенного диска][planning-guide-5.2.1] этого документа для выполнения локальных шагов подготовки и передачи необобщенных виртуальных машин или виртуальных жестких дисков в Azure. Ознакомьтесь с главой [Сценарий 3. перемещение виртуальной машины из локальной среды с помощью необобщенного виртуального жесткого диска Azure с SAP][deployment-guide-3.4] в [руководстве по развертыванию][deployment-guide] для получения подробных инструкций по развертыванию такого образа в Azure.
...
---
Если вы уже установили содержимое SAP на локальную виртуальную машину (что особенно важно для двухуровневых систем), то параметры системы SAP можно изменить после развертывания виртуальной машины в Azure. Для этого переименуйте экземпляр в SAP Software Provisioning Manager (см. примечание к SAP [1619720]). В разделе [Подготовка виртуальной машины к развертыванию с помощью пользовательского образа для SAP][planning-guide-5.2.2] и [Отправка виртуального жесткого диска из локальной среды в Azure][planning-guide-5.3.2] этого документа для выполнения локальных шагов подготовки и передачи обобщенной виртуальной машины в Azure. Ознакомьтесь с главой [Сценарий 2. развертывание виртуальной машины с помощью пользовательского образа для SAP][deployment-guide-3.3] в [руководстве по развертыванию][deployment-guide] . подробные инструкции по развертыванию такого образа в Azure.

#### <a name="deploying-a-vm-out-of-the-azure-marketplace"></a>Развертывание виртуальной машины из Azure Marketplace

Вы хотите использовать для развертывания виртуальной машины образ виртуальной машины из Azure Marketplace, предоставленный корпорацией Майкрософт или сторонним поставщиком. Когда вы развернете виртуальную машину в Azure, следуйте тем же рекомендациям и инструкциям средств для установки программного обеспечения SAP и/или СУБД, что и в локальной среде. Более подробное описание развертывания см. в разделе [Сценарий 1. развертывание виртуальной машины из Azure Marketplace для SAP][deployment-guide-3.2] в разделе [руководство по развертыванию][deployment-guide].
...
> Настройте Автоподключение дисков для подключенных дисков, как описано в разделе [Настройка автоподключения для подключенных дисков][planning-guide-5.5.3] этого документа.
>
> ![Linux][Logo_Linux] Linux
>
...
Файлы на виртуальном жестком диске, содержащие обобщенную ОС, также хранятся в контейнерах учетных записей хранения Azure или как образы управляемых дисков. Вы можете развернуть новую виртуальную машину из такого образа, обратившись к образу VHD или управляемого диска в качестве источника в файлы шаблона развертывания, как описано в разделе [Сценарий 2. развертывание виртуальной машины с помощью пользовательского образа для SAP][deployment-guide-3.3] в [руководство по развертыванию][deployment-guide].
...
> ![Windows][Logo_Windows] Windows
>
> Убедитесь, что диск D:\ не используется параметр Задать Автоподключение диска для подключенных дисков, как описано в разделе [Настройка автоподключения для подключенных дисков][planning-guide-5.5.3] этого документа.
>
> ![Linux][Logo_Linux] Linux
>
> Убедитесь, что /mnt/resource не используется, и что все диски подключены через uuid. Для диска ОС также убедитесь, что запись загрузчика содержит подключения на основе uuid.
...
Чтобы передать существующую виртуальную машину или виртуальный жесткий диск из локальной сети, необходимо выполнить требования, перечисленные в разделе [Подготовка виртуальной машины к перемещению из локальной среды в Azure с помощью необобщенного диска][planning-guide-5.2.1] этого документа.
...
* Войдите в свою подписку с помощью *Connect-азаккаунт* .
* Настройте подписку контекста с помощью командлета *Set-азконтекст* и параметра SubscriptionId или SubscriptionName — см. <https://docs.microsoft.com/powershell/module/az.accounts/set-Azcontext>
* Отправка виртуального жесткого диска с помощью *Add-азвхд* в учетную запись хранения Azure. см. <https://docs.microsoft.com/powershell/module/az.compute/add-Azvhd>
* Используемых Создайте управляемый диск из виртуального жесткого диска с помощью *New-аздиск* . см. <https://docs.microsoft.com/powershell/module/az.compute/new-Azdisk>
* Настройте диск операционной системы для новой конфигурации виртуальной машины на VHD или управляемый диск с помощью *Set-азвмосдиск* . см. <https://docs.microsoft.com/powershell/module/az.compute/set-Azvmosdisk>
* Создайте новую виртуальную машину из конфигурации виртуальной машины с помощью *New-AzVM* . см. <https://docs.microsoft.com/powershell/module/az.compute/new-Azvm>
* Добавьте диск данных в новую виртуальную машину с помощью *Add-азвмдатадиск* . см. <https://docs.microsoft.com/powershell/module/az.compute/add-Azvmdatadisk>
...
* Отправьте VHD-файл с помощью команды *AZ Storage BLOB upload* — см. раздел [Использование Azure CLI с хранилищем Azure][storage-azure-cli] .
...
* Отправка виртуального жесткого диска с помощью PowerShell или Azure CLI
* Используемых Создайте управляемый диск из виртуального жесткого диска с помощью PowerShell, Azure CLI или портал Azure
...
Чтобы передать существующую виртуальную машину или виртуальный жесткий диск из локальной сети, чтобы использовать их в качестве образа ВИРТУАЛЬНОЙ машины Azure, такой как виртуальная машина или виртуальный жесткий диск, должны соответствовать требованиям, перечисленным в разделе [Подготовка виртуальной машины к развертыванию с помощью пользовательского образа для SAP][planning-guide-5.2.2] этого документа.

* Использование *Sysprep* в Windows или *waagent* . отмените подготовку в Linux для подготовки виртуальной машины. см. [Технический справочник по Sysprep](https://technet.microsoft.com/library/cc766049.aspx) для Windows или [запись виртуальной машины Linux для использования в качестве шаблона диспетчер ресурсов][capture-image-linux-step-2-create-vm-image] для Linux.
* Войдите в свою подписку с помощью *Connect-азаккаунт* .
* Настройте подписку контекста с помощью командлета *Set-азконтекст* и параметра SubscriptionId или SubscriptionName — см. <https://docs.microsoft.com/powershell/module/az.accounts/set-Azcontext>
* Отправка виртуального жесткого диска с помощью *Add-азвхд* в учетную запись хранения Azure. см. <https://docs.microsoft.com/powershell/module/az.compute/add-Azvhd>
* Используемых Создание образа управляемого диска из VHD с помощью *New-азимаже* . см. <https://docs.microsoft.com/powershell/module/az.compute/new-Azimage>
* Задайте в качестве диска ОС в конфигурации новой виртуальной машины:
  * Виртуальный жесткий диск с помощью *Set-азвмосдиск-SourceImageUri-CreateOption fromImage* — см. <https://docs.microsoft.com/powershell/module/az.compute/set-Azvmosdisk>
  * Набор образов управляемого диска *-азвмсаурцеимаже* -см. <https://docs.microsoft.com/powershell/module/az.compute/set-Azvmsourceimage>
* Создайте новую виртуальную машину из конфигурации виртуальной машины с помощью *New-AzVM* . см. <https://docs.microsoft.com/powershell/module/az.compute/new-Azvm>
...
* Использование *Sysprep* в Windows или *waagent* . отмените подготовку в Linux для подготовки виртуальной машины. см. [Технический справочник по Sysprep](https://technet.microsoft.com/library/cc766049.aspx) для Windows или [запись виртуальной машины Linux для использования в качестве шаблона диспетчер ресурсов][capture-image-linux-step-2-create-vm-image] для Linux.
* Отправка виртуального жесткого диска с помощью PowerShell или Azure CLI
* Используемых Создание образа управляемого диска из VHD с помощью PowerShell, Azure CLI или портал Azure
...
Во время загрузки виртуального жесткого диска или управляемого диска его нельзя использовать. А при загрузке диска, подключенного к виртуальной машине, следует отключить и освободить эту виртуальную машину. Если вы хотите загрузить только содержимое базы данных, которое затем следует использовать для настройки новой системы в локальной среде, и если это допустимо, то во время загрузки и установки новой системы, которую система в Azure все еще может работать , можно избежать длительного простоя, выполнив сжатое резервное копирование базы данных на диск и просто скачать этот диск вместо того, чтобы загружать базовую виртуальную машину ОС.
...
* Загрузка виртуального жесткого диска  
  После остановки системы SAP и завершения работы виртуальной машины можно использовать командлет PowerShell Save-Азвхд на локальном целевом компьютере, чтобы загрузить диски VHD обратно в локальный мир. Для этого следует определить URL-адрес виртуального жесткого диска, который можно найти в разделе хранилища на портале Azure (перейдите к учетной записи хранения и найдите тот контейнер хранения, в котором был создан нужный виртуальный жесткий диск). Также нужно определить, куда следует копировать виртуальный жесткий диск.
  ...
  Дополнительные сведения о командлете Save-Азвхд можно узнать здесь <https://docs.microsoft.com/powershell/module/az.compute/save-Azvhd>.
...
**Заключение. чтобы скопировать или сохранить диски, которые являются частью конфигурации системы SAP, необходимо остановить систему SAP, а также завершить работу развернутой виртуальной машины. Только после этого вы можете скопировать или скачать набор дисков, чтобы создать копию системы SAP в Azure или в локальной среде.**
...
Для копирования виртуального жесткого диска можно использовать командлеты Azure PowerShell, как показано в [этой статье][storage-powershell-guide-full-copy-vhd]. Чтобы создать управляемый диск, используйте New-Аздискконфиг и New-Аздиск, как показано в следующем примере.
...
* Создайте контекст учетной записи хранения для **исходной** учетной записи хранения с помощью *New-азсторажеконтекст* — см. <https://docs.microsoft.com/powershell/module/az.storage/new-AzStoragecontext>
* Создайте контекст учетной записи хранения для **целевой** учетной записи хранения с помощью *New-азсторажеконтекст* — см. <https://docs.microsoft.com/powershell/module/az.storage/new-AzStoragecontext>
...
* Один или несколько дисков, которые содержат файл журнала СУБД базы данных SAP и файл журнала для области хранилища СУБД "Temp" (если это поддерживается СУБД). Если журнал базы данных предъявляет высокие требования к скорости операций ввода-вывода, следует чередовать несколько дисков для достижения требуемого числа операций ввода-вывода в секунду.
...
> ![Linux][Logo_Linux] Linux
>
> Разместите файл подкачки Linux в разделе/mnt/mnt/Resource в Linux, как описано в [этой статье][virtual-machines-linux-agent-user-guide]. Настройки файла подкачки можно изменить в файле конфигурации /etc/waagent.conf агента Linux. Добавьте или измените следующие параметры:
>
>
...
Дополнительные рекомендации и дополнительные сведения, в частности для виртуальных машин СУБД, см. в статье о [развертывании СУБД][dbms-guide] .

#### <a name="disk-handling"></a>Управление дисками

В большинстве случаев вам потребуются дополнительные диски для развертывания на виртуальной машине базы данных SAP. Мы говорили о вопросах, касающихся количества дисков в разделе [Структура ВМ и диска для развертываний SAP][planning-guide-5.5.1] этого документа. Портал Azure позволяет подключать и отключать диски после развертывания базовой виртуальной машины. Диски можно подключать и отключать как во время работы виртуальной машины, так и во время ее остановки. При подключении диска портал Azure предлагает подключить пустой или имеющийся диск, который сейчас не подключен к другой виртуальной машине.
...
> ![Windows][Logo_Windows] Windows
>
> [Как подключить диск данных в портал Azure][virtual-machines-linux-attach-disk-portal]
>
> Если диски уже подключены, войдите на виртуальную машину и откройте диспетчер дисков Windows. Если Автоподключение не включено, как рекомендуется в разделе [Настройка автоподключения для подключенных дисков][planning-guide-5.5.3], необходимо перевести вновь подключенный том в оперативный режим и инициализировать его.
>
> ![Linux][Logo_Linux] Linux
>
> Если диски подключены, необходимо войти на виртуальную машину и инициализировать диски, как описано в [этой статье][virtual-machines-linux-how-to-attach-disk-how-to-initialize-a-new-data-disk-in-linux] .
>
>
...
>
> Если диски уже подключены, войдите на виртуальную машину и откройте диспетчер дисков Windows. Если Автоподключение не включено, как рекомендуется в разделе [Настройка автоподключения для подключенных дисков][planning-guide-5.5.3], то вновь подключенный том > необходимо перевести в оперативный режим и инициализировать.
>
> ![Linux][Logo_Linux] Linux
>
> Необходимо инициализировать вновь присоединенный пустой диск, как описано в [этой статье][virtual-machines-linux-how-to-attach-disk-how-to-initialize-a-new-data-disk-in-linux].
...
Сведения о окончательном развертывании и конкретные действия, в особенности развертывание расширения Azure для SAP, см. в [руководстве по развертыванию][deployment-guide].
...
См. раздел различие в архитектуре между классической моделью и ARM, как описано в [этой статье][virtual-machines-azure-resource-manager-architecture].
...
Графический пользовательский интерфейс SAP не подключается сразу к запущенным экземплярам SAP (через порт 32xx), а сначала подключается к процессу сервера сообщений SAP через открытый для него порт (порт 36xx). Ранее этот порт использовался сервером сообщений для внутреннего взаимодействия с экземплярами приложений. Чтобы предотвратить непреднамеренный обмен данными локальных серверов приложений с сервером сообщений в Azure, вы можете изменить порты внутренней связи. Настоятельно рекомендуется изменить внутреннюю связь между сервером сообщений SAP и его экземплярами приложения с другим номером порта в системах, которые были клонированы из локальных систем, таких как клонирование разработки для тестирования проекта и т. д. Это можно сделать с помощью параметра профиля по умолчанию:
...
* Создать новую учетную запись хранения
...
Этот сценарий является расширением установки, описанной в разделе Пример использования [одной виртуальной машины с примером и обучением SAP NetWeaver][planning-guide-7.1] в этом документе. В этом случае в уже существующую группу ресурсов мы добавим другие виртуальные машины. В следующем примере обучающий ландшафт состоит из виртуальных машин SAP ASCS/SCS, виртуальной машины с СУБД и виртуальной машины с экземпляром сервера приложений SAP.
...
Дополнительные сведения о виртуальных сетях Azure и их определении можно найти в [этой статье][virtual-networks-create-vnet-arm-pportal].
...
Другими мерами безопасности при развертывании виртуальных машин в таком сценарии может быть создание [группы безопасности сети][virtual-networks-nsg] для определения правил доступа.
...
* Настройте транспортные маршруты, группы и слои как обычно.
...
Чтобы передать некоторую часть информации о инфраструктуре Azure критически важных систем SAP в экземпляры агента узла SAP, установленные на виртуальных машинах, необходимо установить расширение Azure (VM) для SAP для развернутых виртуальных машин. Поскольку требования SAP относятся только к приложениям SAP, корпорация Майкрософт решила не реализовывать универсальные функции в Azure, но оставить ее для клиентов, чтобы развернуть необходимое расширение и конфигурации виртуальных машин на виртуальных машинах, работающих под управлением в Azure. Однако Управление развертыванием и жизненным циклом расширения виртуальной машины Azure для SAP будет в основном автоматизировано Azure.

#### <a name="solution-design"></a>Архитектура решения

Решение, разработанное для того, чтобы позволить агенту узла SAP получать необходимую информацию, основано на архитектуре агента виртуальной машины Azure и платформы расширения. Агент ВМ и платформа расширений используются для того, чтобы разрешить установку на виртуальной машине программных приложений, доступных в коллекции расширений для виртуальных машин Azure. Идея этой концепции состоит в том, чтобы разрешить (в таких случаях расширение Azure для SAP) развертывание специальных функций в виртуальной машине и настройку такого программного обеспечения во время развертывания.

Агент ВМ Azure, который позволяет управлять на виртуальной машине некоторыми расширениями для виртуальных машин Azure, по умолчанию внедряется в виртуальные машины Windows при создании виртуальной машины на портале Azure. На виртуальных машинах SUSE, Red Hat или Oracle Linux агент и раньше входил в образ, доступный на Azure Marketplace. В случае при передаче виртуальной машины Linux из локальной среды в Azure агент ВМ необходимо установить вручную.

Основные стандартные блоки решения для предоставления сведений о инфраструктуре Azure для агента узла SAP в Azure выглядят следующим образом:

![Компоненты расширения Microsoft Azure][planning-guide-figure-2400]

Как показано на приведенной выше блок-схеме, одна часть решения размещается в образе виртуальной машины Azure и в коллекции расширений Azure, которая является глобально реплицируемым репозиторием, который управляется операциями Azure. Совместная работа группы SAP/MS в реализации SAP для работы с Azure позволяет публиковать новые версии расширения Azure для SAP.

Когда вы развертываете новую виртуальную машину Windows, в эту виртуальную машину автоматически добавляется агент виртуальной машины Azure. Функция этого агента заключается в координации загрузки и настройки расширений Azure для виртуальных машин. На виртуальных машинах Linux агент виртуальной машины Azure и раньше существовал в образе ОС, доступном в Azure Marketplace.

Однако вам все же потребуется выполнить одну процедуру. Это включение и настройка сбора данных о производительности. Процесс конфигурации можно автоматизировать командами интерфейса командной строки или сценарием PowerShell. Скрипт PowerShell можно скачать в Microsoft Azure центре сценариев, как описано в разделе [Deployment Guide][deployment-guide].

Общая архитектура расширения Azure для SAP выглядит следующим образом:

![Расширение Azure для SAP ][planning-guide-figure-2500]

**Чтобы получить точные инструкции и подробные инструкции по использованию этих командлетов PowerShell или команды CLI во время развертывания, следуйте инструкциям, приведенным в [руководстве по развертыванию][deployment-guide].**
...
Для подключения экземпляров SAP через SAProuter необходимо предоставить строку SAProuter и имя узла с любой попыткой подключения.
...
Исходный URI портала — http (s):`<Portalserver`>: 5XX00/IRJ, где порт формируется SAP в <https://help.sap.com/saphelp_nw70ehp1/helpdata/de/a2/f9d7fed2adc340ab462ae159d19509/frameset.htm>.
...
* Высокая доступность серверов приложений (AS) SAP
  ...
  Экземпляры серверов приложений SAP являются избыточными компонентами. Каждый экземпляр SAP AS развертывается на собственной виртуальной машине, которая выполняется в другом домене сбоя и обновления Azure (см. разделы [домены сбоя][planning-guide-3.2.1] и [домены обновления][planning-guide-3.2.2]). Это обеспечивается с помощью групп доступности Azure (см. раздел [группы доступности Azure][planning-guide-3.2.3]). Потенциальная недоступность домена сбоя или обновления Azure (запланированная или незапланированная) приведет к недоступности ограниченного количества виртуальных машин и их экземпляров SAP AS.

  Если каждый экземпляр SAP AS помещается в отдельную учетную запись хранения Azure, потенциальная недоступность одной учетной записи хранения Azure приведет к недоступности только одной виртуальной машины и соответствующего экземпляра SAP AS. Однако учтите, что существует ограничение на число учетных записей хранения Azure в рамках одной подписки Azure. Чтобы обеспечить автоматический запуск экземпляра (A) SCS после перезагрузки виртуальной машины, обязательно установите параметр автозапуска в профиле запуска экземпляра (A) SCS, как описано в разделе [Использование автозапуска для экземпляров SAP][planning-guide-11.5].
  Дополнительные сведения см. в разделе [высокий уровень доступности для серверов приложений SAP][planning-guide-11.4.1] .
...
Чтобы обеспечить высокий уровень доступности системы SAP, необходимо защитить все критические компоненты системы SAP, например избыточные серверы приложений SAP, и уникальные компоненты (например, единую точку отказа), такие как экземпляр SAP (A) SCS и СУБД.
...
В случае с экземплярами серверов приложений или диалогов SAP специальное решение для обеспечения высокой доступности не требуется. Высокая доступность достигается за счет избыточности и, как следствие, достаточного количества серверов на разных виртуальных машинах. Все они должны быть помещены в одну группу доступности Azure, чтобы избежать одновременного обновления виртуальных машин во время запланированного обслуживания. Основные функциональные возможности, основанные на разных доменах обновления и сбоя в рамках единицы масштабирования Azure, уже появились в разделе [домены обновления][planning-guide-3.2.2]. Группы доступности Azure представлены в разделе группы [доступности Azure][planning-guide-3.2.3] этого документа.

Нет бесконечного числа доменов сбоя и обновления, которые могут использоваться в группе доступности Azure в рамках единицы масштабирования Azure. Это означает, что в одну группу доступности помещается несколько виртуальных машин, а более ранние или более поздние версии находятся в одном домене сбоя или обновления.
...
Функции обеспечения высокого уровня доступности и аварийного восстановления для СУБД в целом, а также конкретные СУБД описаны в разделе [руководств по развертыванию СУБД][dbms-guide].
...
##### <a name="windowslogo_windows-ha-on-windows"></a>![Windows][Logo_Windows] Высокая доступность в Windows
...
* Разделение виртуальных машин одной системы SAP на три группы доступности возможно даже при наличии всех виртуальных машин, принадлежащих одной виртуальной сети.
* Каждый слой (например, СУБД, ASCS, серверы приложений) должен использовать выделенную группу доступности.
* Все виртуальные машины, на которых выполняются экземпляры СУБД одной системы SAP, находятся в одной группе доступности. Мы предполагаем, что используется несколько ВМ с экземплярами СУБД на систему, так как используются встроенные функции высокой доступности СУБД (например, SQL Server AlwaysOn или Oracle Data Guard).
...
* Все виртуальные машины, на которых выполняется экземпляр (A) SCS одной системы SAP, находятся в одной группе доступности. Для защиты экземпляра (A)SCS на этих ВМ настроена отказоустойчивая кластеризация Windows Server (WSFC).
...
##### <a name="linuxlogo_linux-ha-on-linux"></a>![Linux][Logo_Linux] Высокая доступность в Linux
...
В этой статье приведен пример создания скрипта для этого процесса в PowerShell: <http://www.westerndevs.com/azure-snapshots/>
...
Резервное копирование СУБД выполняется с помощью специфичных для СУБД методов, как описано в разделе о [СУБД][dbms-guide].
...
* Для уровня СУБД необходимо использовать средства СУБД, для которых не требуется кластер общих дисков. Сведения описаны в документации [по СУБД][dbms-guide].
* Чтобы снизить влияние проблем в доменах сбоя в инфраструктуре Azure или обслуживании узла, следует использовать группы доступности Azure:
  ...
  * НЕ рекомендуется применять ту же группу доступности для виртуальных машин различных систем SAP.
  * Рекомендуется использовать управляемые диски уровня "Премиум".
* Для целей резервного копирования на уровне СУБД SAP ознакомьтесь с [руководством по СУБД][dbms-guide].
