...
При развертывании нескольких виртуальных машин в составе одной системы SAP в Microsoft Azure службах виртуальных машин можно повлиять на контроллер структуры Azure на развертывание приложения в различных доменах сбоя, тем самым соблюдая более высокие требования соглашения об уровне обслуживания. Но при этом вы не можете напрямую контролировать распределение доменов сбоя между единицами масштабирования Azure (наборами из сотен вычислительных узлов, узлов хранения и сетевых узлов) или распределение виртуальных машин между доменам сбоя. Чтобы контроллер структуры Azure мог развернуть набор виртуальных машин в разных доменах сбоя, необходимо назначить виртуальным машинам группу доступности Azure во время развертывания. Дополнительные сведения о группах доступности Azure см. в разделе [группы доступности Azure][planning-guide-3.2.3] в этом документе.
...
> [!NOTE]
> Развертывание "Mv1" виртуальных машин семейства виртуальных машин в качестве виртуальных машин поколения 2 возможно в начале 2020 мая. Благодаря этому можно уменьшить и уменьшении между Mv1 и Mv2 виртуальными машинами.


#### <a name="quotas-in-azure-virtual-machine-services"></a>Квоты в службах виртуальных машин Microsoft Azure
В инфраструктуре Azure служба хранилища Azure и сетевая инфраструктура совместно используются несколькими виртуальными машинами. А так же, как и в собственных центрах обработки данных, чрезмерная подготовка некоторых ресурсов инфраструктуры выполняется в определенной степени. Платформа Microsoft Azure использует квоты на ресурсы диска, ЦП, сети (и некоторые другие ресурсы), чтобы ограничивать потребление ресурсов и поддерживать стабильную и предсказуемую производительность. Различные типы и семейства виртуальных машин (E32s_v3, D64s_v3 и т. д.) имеют разные квоты для количества дисков, ЦП, ОЗУ и сети.
...
Помимо квот ресурсов ЦП и памяти, другие квоты, определенные для номеров SKU виртуальных машин, относятся к:

- Пропускная способность сетевого трафика к виртуальной машине
- Количество операций ввода-вывода для трафика хранилища
- Пропускная способность сетевого трафика

Ограничения пропускной способности для хранилища определяются сетью, поэтому эти шумы могут быть сохранены в абсолютном минимальном объеме. Квота хранилища, связанная с хранилищем, перезаписывает квоты накопленных дисков, которые были подключены (см. также позже в части хранилища). Другими словами, при подключении дисков хранилища, которые будут вычислены, превышают пропускную способность и квоту операций ввода-вывода для ВИРТУАЛЬНОЙ машины, квота виртуальной машины имеет приоритет.

#### <a name="rough-sizing-of-vms-for-sap"></a>Приблизительное изменение размера виртуальных машин для SAP 
...
Изучив часть [этой][virtual-machines-sizes-linux] (Linux) и [этой статьи][virtual-machines-sizes-windows] (Windows), посвященную виртуальным машинам серии DS, вы увидите, что существуют ограничения на объем тома данных для дисков в хранилище уровня Premium в зависимости от детализации уровня виртуальной машины. На разных виртуальных машинах серии DS или GS также существуют разные ограничения числа дисков данных, которые можно подключить. Эти ограничения описаны в упомянутой выше статье. Но по сути они означают, что вы не можете увеличить пропускную способность диска P30 в 32 раза, подключив 32 таких диска к одной виртуальной машине DS14. Пропускная способность ограничивается максимальным уровнем пропускной способности, указанным в статье для этого уровня виртуальной машины.
...
### <a name="microsoft-azure-networking"></a><a name="61678387-8868-435d-9f8c-450b2424f5bd"></a>Microsoft Azure сети
...
* Для виртуальных жестких дисков, которые были подключены к виртуальной машине и должны быть снова подключены к виртуальной машине в Azure, также должен быть фиксированный формат. Для ограничения размера дисков данных ознакомьтесь с [этой статьей (Linux)](../../linux/managed-disks-overview.md) и [этой статьей (Windows)](../../windows/managed-disks-overview.md)). Динамические виртуальные жесткие диски, загружаемые с помощью командлетов PowerShell или интерфейса командной строки, будут преобразованы в статические виртуальные жесткие диски.
...
> ![Windows][Logo_Windows] Windows
>
> На последнем этапе войдите на виртуальную машину с помощью учетной записи администратора. Откройте командное окно Windows от имени *администратора*. Перейдите к папке %windir%\windows\system32\sysprep и запустите программу sysprep.exe.
> Откроется небольшое окно. Важно установить флажок **generalize** (по умолчанию снят флажок) и изменить значение параметра Shutdown с «reboot» на «Shutdown». В этой процедуре предполагается, что процесс sysprep выполняется локально в гостевой ОС виртуальной машины.
...
* Создайте виртуальную машину из конфигурации виртуальной машины с помощью *New-AzVM* — см.<https://docs.microsoft.com/powershell/module/az.compute/new-Azvm>
...
* Выберите подписку с помощью команды *AZ Account Set- `<subscription name or id` -Subscription* .
...
Во время загрузки виртуального жесткого диска или управляемого диска его нельзя использовать. А при загрузке диска, подключенного к виртуальной машине, следует отключить и освободить эту виртуальную машину. Если вы хотите загрузить только содержимое базы данных, что, затем следует использовать для настройки новой системы в локальной среде и, если это допустимо, во время загрузки и установки новой системы, которую система в Azure по-прежнему может работать, можно избежать длительного простоя, выполнив сжатое резервное копирование базы данных на диск и просто скачать этот диск вместо того, чтобы загружать базовую виртуальную машину ОС.
...
* Загрузка виртуального жесткого диска  
  ...
  Дополнительные сведения о командлете Save-Азвхд см. здесь <https://docs.microsoft.com/powershell/module/az.compute/save-Azvhd>.
...
Для копирования виртуального жесткого диска вы можете использовать командлеты Azure PowerShell, как описано в [этой статье][storage-powershell-guide-full-copy-vhd]. Чтобы создать управляемый диск, используйте New-Аздискконфиг и New-Аздиск, как показано в следующем примере.
...
Георепликация Azure работает локально на каждом виртуальном жестком диске в виртуальной машине и не реплицирует операции ввода-вывода в хронологическом порядке на несколько виртуальных жестких дисков на виртуальной машине. Таким образом, виртуальный жесткий диск, представляющий базовую виртуальную машину, и все остальные подключенные к ней диски реплицируются независимо друг от друга. Синхронизация между изменениями на разных виртуальных жестких дисках не соблюдается. Тот факт, что операции ввода-вывода реплицируются независимо от порядка их написания, означает, что Георепликация не является значением для серверов баз данных, которые имеют базы данных, распределенные по нескольким виртуальным жестким дискам. Кроме СУБД, у вас могут быть другие приложения, которые записывают или изменяют данные на нескольких виртуальных жестких дисках и для которых важен порядок этих изменений. Если у вас есть такие приложения, не следует включать георепликацию в Azure. Если в вашей среде нужна георепликация для одного набора виртуальных машин, но не нужна для другого набора, вы можете распределить виртуальные машины и подключенные к ним виртуальные жесткие диски между несколькими учетными записями хранения, для которых георепликация будет включена или отключена.
...
> * [Автоподключения](https://technet.microsoft.com/library/cc753703.aspx)
>
...
1. Для подсети или сетевого интерфейса не определена группа безопасности сети. Сетевой трафик к виртуальным машинам Azure можно защитить с помощью так называемых "групп безопасности сети". Дополнительные сведения см [. в разделе что такое группа безопасности сети (NSG)?][virtual-networks-nsg]
...
>
> * Откройте последовательно управления и безопасность "брандмауэр на **Дополнительные параметры**.
> * Теперь щелкните правой кнопкой мыши Правила для входящих подключений и выберите **новое правило**.
> * В следующем мастере выбрано создание нового правила **порта** .
...
> * В следующей части мастера необходимо оставить элемент Разрешить установленное **Подключение** .
...
> * На последнем шаге мастера Назовите правило и сохраните его, нажав кнопку **Готово**.
>
...
Чтобы передать некоторую часть информации о инфраструктуре Azure критически важных систем SAP в экземпляры агента узла SAP, установленные на виртуальных машинах, необходимо установить расширение Azure (VM) для SAP для развернутых виртуальных машин. Поскольку требования SAP характерны для приложений SAP, корпорация Майкрософт решила не реализовывать в Azure требуемую функциональность, но оставить ее для клиентов, чтобы развернуть необходимое расширение и конфигурации виртуальных машин на виртуальных машинах, работающих в Azure. Однако Управление развертыванием и жизненным циклом расширения виртуальной машины Azure для SAP будет в основном автоматизировано Azure.
...
Исходный URI портала — http (s):`<Portalserver`>:5XX00/IRJ, где порт формируется в соответствии с документацией SAP в <https://help.sap.com/saphelp_nw70ehp1/helpdata/de/a2/f9d7fed2adc340ab462ae159d19509/frameset.htm>.

![Конфигурация конечной точки][planning-guide-figure-2800]
...
* Высокая доступность серверов приложений (AS) SAP
  ...
  Экземпляры серверов приложений SAP являются избыточными компонентами. Каждый экземпляр SAP AS развертывается на отдельной виртуальной машине, которая работает в отдельных домене сбоя и домене обновления Azure (см. разделы [Домены сбоя][planning-guide-3.2.1] и [Домены обновления][planning-guide-3.2.2]). Это обеспечивается с помощью групп доступности Azure (см. раздел [группы доступности Azure][planning-guide-3.2.3]). Потенциальная недоступность домена сбоя или обновления Azure (запланированная или незапланированная) приведет к недоступности ограниченного количества виртуальных машин и их экземпляров SAP AS.
...
#### <a name="high-availability-for-sap-application-servers"></a><a name="5d9d36f9-9058-435d-8367-5ad05f00de77"></a>Высокий уровень доступности для серверов приложений SAP

В случае с экземплярами серверов приложений или диалогов SAP специальное решение для обеспечения высокой доступности не требуется. Высокая доступность достигается за счет избыточности и, как следствие, достаточного количества серверов на разных виртуальных машинах. Все они должны быть помещены в одну группу доступности Azure, чтобы избежать одновременного обновления виртуальных машин во время запланированного обслуживания. Основные функциональные возможности, основанные на разделении доменов обновления и доменов сбоя в рамках единицы масштабирования Azure, уже были описаны выше в разделе [Домены обновления][planning-guide-3.2.2]. Группы доступности Azure представлены в разделе группы [доступности Azure][planning-guide-3.2.3] этого документа.
...
Помимо завершения работы с помощью портал Azure его также можно выполнить с помощью PowerShell или CLI, как описано здесь:<https://azure.microsoft.com/documentation/articles/virtual-machines-deploy-rmtemplates-powershell/>
