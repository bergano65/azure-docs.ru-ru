...
Принцип Зон доступности не применяется к службе HANA [крупных экземпляров HANA](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-overview-architecture). Соглашения об уровне обслуживания для крупных экземпляров HANA можно найти в статье [Соглашение об уровне обслуживания для SAP HANA на крупных экземплярах Azure](https://azure.microsoft.com/support/legal/sla/sap-hana-large/) 
...
Для рабочей нагрузки SAP мы предлагаем только семейства виртуальных машин, которые подходят для рабочей нагрузки SAP и SAP HANA. Чтобы узнать, как подобрать тип виртуальной машины и проверить его пригодность для рабочей нагрузки SAP, ознакомьтесь с документом [Программное обеспечение SAP, поддерживаемое для развертываний Azure](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-supported-product-on-azure). 

> [!NOTE]
> Типы виртуальных машин, сертифицированные для рабочей нагрузки SAP, не имеют избыточной подготовки ресурсов ЦП и памяти.

Вам не только нужно выбрать поддерживаемые виртуальные машины, но и проверить, доступны ли они в определенном регионе, перейдя на сайт [Доступность продуктов по регионам](https://azure.microsoft.com/global-infrastructure/services/). Что еще важнее, необходимо оценить следующее:
...
Инфраструктура хранилища и сети Azure совместно используется виртуальными машинами, на которых запущены разные службы в инфраструктуре Azure. Как и в ваших центрах обработки данных, здесь может происходить чрезмерное использование некоторых инфраструктурных ресурсов. Платформа Microsoft Azure использует квоты на ресурсы диска, ЦП, сети (и некоторые другие ресурсы), чтобы ограничивать потребление ресурсов и поддерживать стабильную и предсказуемую производительность. Разные типы виртуальных машин (E32s_v3, D64s_v3 и т. д.) имеют разные квоты на количество дисков, а также использование ЦП, памяти и сети.
...
Помимо квот ресурсов ЦП и памяти, существуют другие квоты, определенные для номеров SKU виртуальных машин:

- Пропускная способность сетевого трафика к виртуальной машине
- Операции ввода-вывода в секунду для трафика хранилища
- Пропускная способность сетевого трафика

Ограничения пропускной способности для хранилища и сети определены, поэтому соседей с большим количеством трафика можно ограничить абсолютным минимумом. Квота хранилища виртуальной машины переопределяет квоту всех подключенных дисков (см. ниже в разделе о хранилище). Другими словами, при подключении дисков хранилища, которые вместе превысят квоту на пропускную способность и операции ввода-вывода в секунду для виртуальной машины, квота виртуальной машины имеет приоритет.
...
Вы можете передать существующий виртуальный жесткий диск c локального компьютера или создать пустые диски в Azure, а затем подключить эти VHD к развернутым виртуальным машинам.
...
Дополнительные сведения об учетных записях хранения Azure см. в статье [Целевые показатели масштабируемости для учетных записей хранения категории "Стандартный"](../../../storage/common/scalability-targets-standard-account.md) и [Целевые показатели масштабируемости для учетных записей хранения больших двоичных объектов класса Premium](../../../storage/blobs/scalability-targets-premium-page-blobs.md). Из этих статей вы узнаете, что в Azure существуют разные ограничения для учетных записей службы хранилища категории "Стандартный" и Premium. В первую очередь, различается объем данных, которые могут храниться в одной учетной записи хранения. В хранилище категории "Стандартный" объем тома на порядок больше, чем в хранилище класса Premium. С другой стороны, для учетной записи хранения уровня "Стандартный" действует существенное ограничение на число операций ввода-вывода в секунду (см. столбец **Общая частота запросов**). Учетная запись службы хранения Azure уровня "Премиум" не имеет такого ограничения. Подробности и следствия этих различий мы опишем при обсуждении развертывания систем SAP, особенно серверов СУБД.
...
Список вариантов см. в разделе [Предложения для соответствия требованиям от Майкрософт](https://docs.microsoft.com/microsoft-365/compliance/offering-home). 

Другие важные вопросы, например шифрование неактивных или других данных в службе Azure, описаны в статье [Общие сведения о шифровании Azure](https://docs.microsoft.com/azure/security/fundamentals/encryption-overview).
