...
В отличие от стандартного хранилища, где не изменяли возможности зависят от размера диска (или виртуального жесткого диска), хранилище класса Premium в настоящее время имеет три категории дисков, которые приведены в этой статье: <https://azure.microsoft.com/pricing/details/storage/unmanaged-disks/>
...
Дополнительные сведения можно найти здесь: <https://azure.microsoft.com/documentation/services/virtual-network/>
...
Чтобы избежать прямого подключения к Интернету, вы можете настроить принудительное туннелирование для подключения типа "сеть — сеть" между локальной сетью и Azure. Подробное описание функции принудительного туннелирования опубликовано здесь:
...
См. также этот блог и вложенный документ по размерам SAP в Azure:
...
Развернуть расширение для мониторинга Azure для SAP (см. раздел этого документа [Решение для мониторинга в Azure для SAP][planning-guide-9.1]) можно только с помощью PowerShell или интерфейса командной строки. Поэтому при развертывании или администрировании системы SAP NetWeaver в Azure нужно обязательно установить и настроить PowerShell или интерфейс командной строки.  
...
**Вывод: Чтобы скопировать или сохранить диски, которые входят в конфигурацию системы SAP, следует остановить эту систему SAP и завершить работу развернутой виртуальной машины. Только после этого можно копировать или загружать набор дисков, чтобы создать копию системы SAP в Azure или локально.**
...
Для копирования виртуального жесткого диска вы можете использовать командлеты Azure PowerShell, как описано в [этой статье][storage-powershell-guide-full-copy-vhd]. Для создания управляемого диска, используйте New AzDiskConfig и New-AzDisk, как показано в следующем примере.
...
* Создайте контекст учетной записи хранения для **источника** учетной записи хранения с *New AzStorageContext* -см. в разделе <https://docs.microsoft.com/powershell/module/azure.storage/new-AzStoragecontext>
* Создайте контекст учетной записи хранения для **целевой** учетной записи хранения с *New AzStorageContext* -см. в разделе <https://docs.microsoft.com/powershell/module/azure.storage/new-AzStoragecontext>
...
> * [Секреты хранилища Azure и оптимизация ввода-вывода в Linux](https://blogs.msdn.com/b/igorpag/archive/2014/10/23/azure-storage-secrets-and-linux-i-o-optimizations.aspx)
>
>
...
>
> * [Параметры командной строки DiskPart](https://technet.microsoft.com/library/bb490893.aspx)
...
1. Для подсети или сетевого интерфейса не определена группа безопасности сети. Сетевой трафик к виртуальным машинам Azure можно защитить с помощью так называемых "групп безопасности сети". Дополнительные сведения см. в статье [Группы безопасности][virtual-networks-nsg].
...
См. в этой статье, который описывает сведения в этом разделе:
...
* [Простой Windows виртуальной Машины](https://github.com/Azure/azure-quickstart-templates/tree/master/101-vm-simple-windows)
* [Виртуальную Машину из образа](https://github.com/Azure/azure-quickstart-templates/tree/master/101-vm-from-user-image)
...
* [Изменить URL-адрес портала](https://wiki.scn.sap.com/wiki/display/EP/Change+Portal+URL)
* [Изменить номера портов по умолчанию, номера портов портала](https://wiki.scn.sap.com/wiki/display/NWTech/Change+Default++port+numbers%2C+Portal+port+numbers)
...
* [Запуск и остановка SAP вместе с вашей Unix Server запуск и остановка](https://scn.sap.com/community/unix/blog/2012/08/07/startstop-sap-along-with-your-unix-server-startstop)
...
* [Включение автоматического запуска из базы данных HANA](http://www.freehanatutorials.com/2012/10/how-to-enable-auto-start-of-hana.html)
...
Помимо завершение работы с помощью портала Azure можно также сделать с помощью Powershell или интерфейса командной строки, как описано здесь.
...
В этой статье показано пример сценария для этого процесса в Powershell:
