...
И последний шаг — это оценка требований к доступности. Базовая инфраструктура Azure должна время от времени обновляться. Для этого требуется перезапускать узлы, на которых работают виртуальные машины. Корпорация Майкрософт документирует различные варианты [обслуживания виртуальных машин в Azure](https://docs.microsoft.com/azure/virtual-machines/windows/maintenance-and-updates). Чтобы устранить редкие случаи, когда виртуальные машины вынуждены перезагружаться, но, что еще важнее для тех случаев, когда необходимо обновить гостевую ОС или компоненты СУБД, необходимо разработать допустимые концепции высокого уровня доступности для производственных систем SAP. Это требование не отличается от требований, которые вы хотите иметь в локальной среде. Корпорация Майкрософт постоянно перемещает платформу Azure, чтобы сократить время простоя, вызванное изменениями платформы. 
...
Дополнительные сведения о подключении локальной сети к Microsoft Azure см. в [этой статье][vpn-gateway-create-site-to-site-rm-powershell] .
...
>
> Особое планирование систем и регионов должно происходить при использовании нескольких систем SAP, которые тесно интегрированы. Обязательно разверните эти системы как можно ближе друг к другу, чтобы избежать задержки в сети. Примеры высоко интегрированных систем SAP:
> * SAP BW чтение данных из систем SAP OLTP, таких как ERP или CRM или СРМ; ни
> * SAP SLT используется для репликации данных между несколькими системами SPA или даже между SAP и системами, не являющимися SAP; ни
> * SAP S/4 подключен к системе SAP ERP; 123.
...
Не следует недооценивать этот этап проекта в планировании. Только при наличии соглашения и правил для этого раздела необходимо перейти к следующему этапу, который является планированием сетевой архитектуры, развертываемой в Azure.
...
Самым распространенным способом развертывания является перемещение имеющейся виртуальной машины, на которой работает система SAP, из локальной среды в Azure. Такая виртуальная машина и система SAP на ней будут работать в Azure с прежним именем узла и, возможно, с тем же идентификатором безопасности SAP. В этом случае не требуется обобщать гостевую операционную систему виртуальной машины для многочисленных развертываний. Если локальная сеть расширена в Azure (см. раздел распределенное [развертывание одной или нескольких виртуальных машин SAP в Azure с требованием полной интеграции с локальной сетью][planning-guide-2.2] в этом документе), то даже одни и те же учетные записи домена можно использовать на виртуальной машине, так как они использовались до локальной среды.
