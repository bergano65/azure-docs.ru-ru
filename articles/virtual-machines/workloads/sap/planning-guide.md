...
В качестве полезной помощи можно указать предложения по обеспечению [соответствия требованиям Майкрософт](https://docs.microsoft.com/microsoft-365/compliance/offering-home) , чтобы получить список предложений по соответствию, которые корпорация Майкрософт может предоставить. 
...
## <a name="microsoft-azure-virtual-machine-services"></a>Службывиртуальных машин Microsoft Azure
...
С помощью служб виртуальных машин Azure Корпорация Майкрософт позволяет развертывать пользовательские образы сервера в Azure как экземпляры IaaS. Виртуальные машины в Azure основаны на виртуальных жестких дисках (VHD) и могут запускать разные операционные системы в качестве гостевой ОС.
...
Размер и ограничения различных размеров различных виртуальных машин можно просмотреть в таблице в [этой статье (Linux)][virtual-machines-sizes-linux] и в [этой статье (Windows)][virtual-machines-sizes-windows].

Не все различные серии виртуальных машин могут быть предложены в каждом из регионов Azure. Также не забывайте, что не все виртуальные машины и не все серии виртуальных машин сертифицированы для использования с SAP.

> [!IMPORTANT]
> Для использования SAP NetWeaver, S/4HANA и SAP HANA поддерживаются только подмножество типов и конфигураций виртуальных машин, перечисленных в SAP Note [1928533] . Для SAP HANA сертифицированных единиц Azure проверьте [каталог SAP HANA оборудования](https://www.sap.com/dmc/exp/2014-09-02-hana-hardware/enEN/iaas.html#categories=Microsoft%20Azure) .
>

Microsoft Azureная платформа — это платформа с несколькими клиентами. В результате хранилище, сеть и другие ресурсы совместно используются несколькими клиентами. Интеллектуальное регулирование и логика квотирования не позволяют клиентам существенно влиять на производительность, доступную для другого клиента ("шумный сосед"). В частности, для сертификации платформы Azure для SAP HANA корпорации Майкрософт необходимо доказать изоляцию ресурсов в случаях, когда несколько виртуальных машин могут выполняться на одном узле на регулярной основе в SAP. Несмотря на то, что логика в Azure пытается удержать дисперсию пропускной способности небольших, высокообщие платформы обычно применяют большие дисперсии доступности ресурсов или пропускной способности, чем клиенты могут столкнуться с локальными развертываниями. При планировании следует учитывать вероятность того, что система SAP в Azure будет испытывать более существенные отклонения, чем в локальной системе.

Для этого клиентам потребуется знание различных возможностей типов Azure с поддержкой SAP, в том числе:
...
Дополнительные сведения о создании образов из виртуальных машин можно найти в [этой статье (Linux)][virtual-machines-linux-capture-image-resource-manager] и в [этой статье (Windows)][virtual-machines-windows-capture-image-resource-manager].
...
Для копирования виртуального жесткого диска можно использовать Azure CLI, как показано в [этой статье][storage-azure-cli-copy-blobs]. Для создания управляемого диска используйте команду *az disk create*, как показано в следующем примере.
...
Корпорация Майкрософт добавила много новых типов виртуальных машин. Они различаются количеством ЦП, объемом памяти или, что более важно, оборудованием, на котором они работают. Не все эти виртуальные машины поддерживаются SAP (поддерживаемые типы виртуальных машин см. в примечании к SAP [1928533]). Некоторые виртуальные машины работают на базовом оборудовании разных поколений. Новые поколения оборудования развертываются для узлов на уровне единицы масштабирования Azure. В некоторых случаях вы не сможете использовать выбранный тип виртуальной машины для некоторых единиц масштабирования. Группа доступности ограничена возможностью использования единиц масштабирования на разных устройствах.  Например, если уровень СУБД SAP выполняется на E64s_v3, виртуальной машине, которая находится в группе доступности, вместе с виртуальной машиной, в которой выполняется дополнительный экземпляр СУБД, в конфигурации с высоким уровнем доступности вы не сможете просто перезапустить вторичную виртуальную машину в качестве виртуальной машины серии M, так как вам может потребоваться пграде виртуальную машину. Причиной является то, что виртуальные машины серии M и виртуальные машины серии Ev3 работают на разном оборудовании и, следовательно, с разными единицами масштабирования. Вам потребуется создать новую группу доступности, удалить вторичную виртуальную машину серии Ev3 без удаления хранилища и повторно развернуть ВИРТУАЛЬную машину как ВИРТУАЛЬную машину серии M в новой группе доступности.
