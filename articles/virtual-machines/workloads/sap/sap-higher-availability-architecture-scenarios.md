...
* Высокий уровень доступности для серверов приложений SAP
    ...
    Каждый экземпляр сервера приложений SAP помещается в отдельную учетную запись хранения Azure. В случае недоступности одной учетной записи хранения Azure недоступной станет только одна виртуальная машина с одним экземпляром сервера приложений SAP. Однако учтите, что существует ограничение на число учетных записей хранения Azure в одной подписке Azure. Чтобы обеспечить автоматический запуск экземпляра ASCS/SCS после перезагрузки виртуальной машины, установите параметр автозапуска в профиле запуска экземпляра ASCS/SCS, как описано в разделе Использование автоматического [запуска для экземпляров SAP][planning-guide-11.5] .
  
    Дополнительные сведения см. в статье [высокий уровень доступности для серверов приложений SAP][planning-guide-11.4.1].
...
* *Повышение уровня доступности* экземпляра SAP ASCS/SCS
    ...
    Чтобы обеспечить автоматический запуск экземпляра ASCS/SCS после перезагрузки виртуальной машины, установите параметр автозапуска в профиле запуска экземпляра ASCS/SCS, как описано в разделе Использование автоматического [запуска для экземпляров SAP][planning-guide-11.5] . В такой схеме экземпляр ASCS/SCS, выполняемый на одной виртуальной машине, становится единой точкой отказа (SPOF) и определяет доступность всей системы SAP.
