...
Что касается SQL Server, применяются основные соображения развертывания, описанные в разделе [Развертывание СУБД виртуальных машин Azure для SAP NetWeaver ](./dbms_guide_general.md). для SAP HANA соображения упоминаются в этом документе.
...
- [Виртуальные машины Azure в Windows](../../windows/tutorial-manage-vm.md)
- [Виртуальные машины Azure в Linux](../../linux/tutorial-manage-vm.md)
- [Управление сетями Azure и виртуальными сетями с помощью PowerShell](../../windows/tutorial-virtual-network.md)
- [Сеть и виртуальные сети Azure с интерфейсом командной строки](../../linux/tutorial-virtual-network.md)
- [Управление дисками Azure с помощью интерфейса командной строки Azure](../../linux/tutorial-manage-disks.md)

Даже если вы интересуетесь только Business One, документ [SAP NetWeaver на виртуальных машинах Windows. Руководство по планированию и внедрению](./planning-guide.md) может быть полезным источником информации.
...
Более подробный обзор компонентов, выполняемых в клиентской части и часто выполняемых в элементе сервера, см. в [Руководстве администратора SAP Business One](https://help.sap.com/doc/601fbd9113be4240b81d74626439cfa9/10.0/en-US/AdministratorGuide_SQL.pdf) 
...
Что касается размера клиентских виртуальных машин, требования к ресурсам документируются SAP в документе [Руководство по требованиям к оборудованию SAP Business One](https://help.sap.com/doc/bfa9770d12284cce8509956dcd4c5fcb/9.3/en-US/B1_Hardware_Requirements_Guide.pdf). Для Azure необходимо сосредоточиться и вычислить требования, перечисленные в разделе документа 2.4.
...
SAP HANA в качестве серверной части СУБД для бизнеса 1, для HANA поддерживаются только виртуальные машины, перечисленные для бизнеса в HANA в [списке платформы, сертифицированной для Hana](https://www.sap.com/dmc/exp/2014-09-02-hana-hardware/enEN/iaas.html#categories=Microsoft%20Azure%23SAP%20Business%20One) . Эти более сильные ограничения для SAP HANA как системы СУБД не влияют на клиентские компоненты Business One.
...
- Следующий экземпляр – [группа безопасности сети Azure](../../../virtual-network/security-overview.md), которую можно использовать для введения правил маршрутизации и безопасности для виртуальной сети Azure, в которой запускается конфигурация системы SAP Business One.
...
Более сложные версии конфигурации сети Azure основаны на [,описанных в рекомендациях по звездообразной архитектуре](/azure/architecture/reference-architectures/hybrid-networking/hub-spoke) Azure. Шаблон звездообразной архитектуры изменит первую упрощенную конфигурацию, показанную ниже.


![Настройка звездообразной архитектуры сети с помощью Business One](./media/business-one-azure/hub-spoke-network-with-VPN.PNG)

В случаях, когда пользователи подключаются через Интернет без какого-либо частного подключения к Azure, разработка сети в Azure должена соответствовать принципам, описанным в справочной архитектуре Azure для [сети периметра между Azure и Интернетом](/azure/architecture/reference-architectures/dmz/secure-vnet-dmz).

### <a name="business-one-database-server"></a>Сервер базы данных Business One
Для типа базы данных доступны SQL Server и SAP HANA. Независимо от СУБД, необходимо ознакомиться с документацией [по развертыванию СУБД Azure для рабочих нагрузок SAP](./dbms_guide_general.md) , чтобы получить общее представление о развертываниях СУБД в виртуальных машинах Azure, а также связанных разделах о сети и хранилище.
...
Чтобы развернуть SQL Server в качестве СУБД для Business One, см. статью [Развертывание СУБД виртуальных машин в Azure SQL Server для SAP NetWeaver](./dbms_guide_sqlserver.md). 
...
Указанные варианты размеров должны дать представление о том, с чего начать. Возможно, вам потребуется меньше ресурсов, и в этом случае адаптация в Azure будет простой. Переключение между типами виртуальных машин возможно с помощью перезапуска виртуальной машины.

#### <a name="sap-hana-as-dbms"></a>SAP HANA в качестве СУБД
Используя SAP HANA в качестве СУБД в следующих разделах, необходимо следовать указаниям в документе [Руководство по работе SAP HANA в Azure ](./hana-vm-operations.md).

Для настройки высокой доступности и аварийного восстановления вокруг SAP HANA в качестве базы данных для Business One в Azure необходимо прочитать документацию [Обеспечение высокого уровня доступности для SAP HANA на виртуальных машинах Azure](./sap-hana-availability-overview.md) и документацию, указанную в этом документе.

Сведения о резервном копировании и восстановлении стратегий SAP HANA см. в [Руководстве по резервному копированию SAP HANA на виртуальных машинах Azure](./sap-hana-backup-guide.md) и документации, указанной в этом документе.
...
- [Размеры виртуальных машин Windows в Azure](../../windows/sizes.md)
