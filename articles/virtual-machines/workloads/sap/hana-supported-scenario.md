...
- Многоцелевой сайт аварийного восстановления: система на сайте аварийного восстановления, настроенная для использования непроизводственной среды вместе с рабочим экземпляром, настроенным для события аварийного восстановления. 
- Одноэкземплярная SID:  система с одним установленным экземпляром.
- Многоэкземплярная SID: система с несколькими настроенными экземплярами. Также называется средой MCOS.
...
- **Ответ**. Этот интерфейс используется для клиентского доступа.
...
| А | Тип II | >\<виртуальной ЛС тенантно | team0.tenant | Между клиентом и HLI |
| С | Тип II | тенантно\<виртуальной ЛС + 2 > | team0.tenant+2 | Между узлами |
| В | Тип II | Виртуальная ЛС\<тенантно + 1 > | team0.tenant+1 | Между узлом и хранилищем |
| D | Тип II | тенантно\<виртуальной ЛС + 3 > | team0.tenant+3 | STONITH |
...
>[!NOTE]
>Идентификатор безопасности точки\<подключения/usr/SAP/> является символьной ссылкой на точку монтирования/Hana/Shared.
...
| А | Тип II | >\<виртуальной ЛС тенантно | team0.tenant | Между клиентом и HLI |
| С | Тип II | тенантно\<виртуальной ЛС + 2 > | team0.tenant+2 | Настроен, но не используется |
| В | Тип II | Виртуальная ЛС\<тенантно + 1 > | team0.tenant+1 | Между узлом и хранилищем |
| D | Тип II | тенантно\<виртуальной ЛС + 3 > | team0.tenant+3 | Настроен, но не используется |
...
| С | Тип II | тенантно\<виртуальной ЛС + 2 > | team0.tenant+2 | Настроен, но не используется |
| В | Тип II | Виртуальная ЛС\<тенантно + 1 > | team0.tenant+1 | Между узлом и хранилищем |
| D | Тип II | тенантно\<виртуальной ЛС + 3 > | team0.tenant+3 | Используется для STONITH |
...
| D | Тип I | eth4.tenant | eno4.tenant | Настроен, но не используется |
| А | Тип II | >\<виртуальной ЛС тенантно | team0.tenant | Между клиентом и HLI |
| С | Тип II | тенантно\<виртуальной ЛС + 2 > | team0.tenant+2 | Обмен данными между узлами |
| В | Тип II | Виртуальная ЛС\<тенантно + 1 > | team0.tenant+1 | Между узлом и хранилищем |
| D | Тип II | тенантно\<виртуальной ЛС + 3 > | team0.tenant+3 | Настроен, но не используется |
...
### <a name="key-considerations"></a>Основные рекомендации
- /usr/sap/SID является символической ссылкой на /hana/shared/SID.
- Резервный узел: тома и точки подключения настраиваются (помечены как Required for HANA installation (требуются для установки HANA)) для установки экземпляра HANA в резервной единице.
