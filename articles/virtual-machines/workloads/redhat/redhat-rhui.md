...
Для полного списка образов запустите `az vm image list --publisher redhat --all` с помощью Azure CLI.

### <a name="images-connected-to-non-eus-repositories"></a>Образы, подключенные к репозиториям, отличным от ЕУС

При подготовке виртуальной машины из образа RHEL, подключенного к репозиториям, отличным от ЕУС, при запуске `sudo yum update`будет выполнено обновление до последней дополнительной версии RHEL. Например, если вы подготавливаете виртуальную машину из образа RHEL 7,4 PAYG и запускаете `sudo yum update`, вы поработаете с виртуальной машиной RHEL 7,7 (последняя дополнительная версия в семействе RHEL7).
...
При подготовке виртуальной машины из образа RHEL, подключенного к репозиториям ЕУС, вы не будете обновлены до последней версии RHEL во время выполнения `sudo yum update`. Это связано с тем, что образы, подключенные к репозиториям ЕУС, также будут заблокированы до определенной дополнительной версии.
...
На момент написания этой статьи поддержка ЕУС была завершена для RHEL < = 7,4. Дополнительные сведения см. в разделе "дополнительные компоненты поддержки Red Hat Enterprise Linux" в [документации по Red Hat](https://access.redhat.com/support/policy/updates/errata/) .
...
Если вы используете одну из предыдущих версий образа виртуальной машины RHEL, например RHEL 7.4 (URN образа: `RedHat:RHEL:7.4:7.4.2018010506`), будут возникать проблемы с подключением к инфраструктуре RHUI из-за просроченного SSL-сертификата клиента. Отображаемая ошибка может выглядеть следующим образом: _"узел SSL отклонил сертификат с истекшим сроком действия"_ или _"ошибка: не удается получить метаданные репозитория (репомд. XML) для репозитория"... Проверьте его путь и повторите_попытку ". Чтобы решить эту проблему, обновите пакет клиента RHUI на виртуальной машине, используя следующую команду.
...
Если при выполнении `yum update`возникает ошибка 404, попробуйте выполнить следующие действия, чтобы обновить кэш Yum:
```bash
sudo yum clean all;
sudo yum makecache
```
