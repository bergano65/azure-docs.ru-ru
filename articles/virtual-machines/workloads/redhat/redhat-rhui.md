...
1. Заблокировать `releasever` переменную (Запуск от имени привилегированного пользователя):
    ```bash
...
Если вы используете более старую версию образа виртуальной машины RHEL, например RHEL 7,4 (URN образа: `RedHat:RHEL:7.4:7.4.2018010506`), вы будете испытывать проблемы с подключением к RHUI из-за просроченного клиентского сертификата TLS/SSL. Отображаемая ошибка может выглядеть следующим образом: _"узел SSL отклонил сертификат с истекшим сроком действия"_ или _"ошибка: не удается получить метаданные репозитория (репомд. XML) для репозитория"... Проверьте его путь и повторите_попытку ". Чтобы решить эту проблему, обновите пакет клиента RHUI на виртуальной машине, используя следующую команду.
...
Если во время выполнения возникла ошибка 404 `yum update`, попробуйте выполнить следующие действия, чтобы обновить кэш Yum:
```bash
sudo yum clean all;
sudo yum makecache
```
