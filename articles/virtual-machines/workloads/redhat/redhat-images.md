...
Для изображений RHEL 7. x существует несколько различных типов изображений. В следующей таблице показаны различные наборы предоставляемых образов. Чтобы просмотреть полный список, используйте команду `az vm image list --publisher redhat --all`Azure CLI.
...
В этом случае `RedHat:RHEL:7.6:7.6.2019102813` по умолчанию прикрепляется к репозиториям ЕУС. Значение SKU — 7,4. И `RedHat:RHEL:7-LVM:7.6.2019062414` по умолчанию подключен к репозиториям, ОТЛИЧНЫМ от ЕУС. Значение SKU — 7 — LVM.
