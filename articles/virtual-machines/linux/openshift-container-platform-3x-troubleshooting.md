---
title: Устранение неполадок развертывания OpenShift для платформы контейнеров 3,11 в Azure | Документация Майкрософт
...
Файлы журнала (stderr и stdout) для сценариев подготовки узла расположены в `/var/lib/waagent/custom-script/download/0` на всех узлах. Если произошла ошибка во время подготовки узла, просмотрите файлы журналов, чтобы определить ошибку.

Если скрипты подготовки успешно выполнены, необходимо проверить файлы журнала в каталоге `/var/lib/waagent/custom-script/download/1` узла сборник тренировочных заданий ansible. Если ошибка произошла во время фактической установки OpenShift, в файле STDOUT она будет указана. Используйте эту информацию при обращении в службу поддержки за помощью.
...
Закрытый ключ копируется на узел сборник тренировочных заданий ansible-~/.ssh/id_rsa. Убедитесь, что этот файл исправен. Проверьте его, установив SSH-подключение между узлом со сборником схем Ansible и одним из узлов кластера.
...
1. systemctl Status @no__t — 0service >
2. journalctl -xe
