---
title: Развертывание контейнерной платформы OpenShift OpenShift 3.11 в Azure
description: Развертывание контейнерной платформы OpenShift OpenShift 3.11 в Azure.
...
ms.locfileid: "80066589"
---
# <a name="troubleshoot-openshift-container-platform-311-deployment-in-azure"></a>Развертывание контейнерной платформы OpenShift OpenShift 3.11 в Azure
...
Файлы журнала (stderr и stdout) для скриптов подготовки хоста расположены на `/var/lib/waagent/custom-script/download/0` всех узлах. Если произошла ошибка во время подготовки узла, просмотрите файлы журналов, чтобы определить ошибку.

Если сценарии подготовки успешно работали, `/var/lib/waagent/custom-script/download/1` то файлы журнала в каталоге хостатора playbook должны быть изучены. Если ошибка произошла во время фактической установки OpenShift, в файле STDOUT она будет указана. Используйте эту информацию при обращении в службу поддержки за помощью.
...
Вы увидите ошибку, что разрешение было отказано для ssh. ssh к ansible хозяинplay playbook для того чтобы проверить для passphrase на приватном ключе.
...
Частный ключ скопирован в asible playbook хост - q/.ssh/id_rsa. Убедитесь, что этот файл исправен. Проверьте его, установив SSH-подключение между узлом со сборником схем Ansible и одним из узлов кластера.
