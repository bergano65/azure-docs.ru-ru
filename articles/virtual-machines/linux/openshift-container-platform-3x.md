...
| `masterClusterType` | Укажите, использует ли кластер частные или общедоступные главные узлы. Если выбран параметр частный, главные узлы не будут доступны в Интернете через общедоступный IP-адрес. Вместо этого он будет использовать частный IP-адрес, указанный в`masterPrivateClusterIp` | таверна <br> private | таверна |
...
| `routerClusterType` | Укажите, использует ли кластер частные или общедоступные узлы. Если выбран параметр частный, то узлы инфраструктуры не будут доступны через общедоступный IP-адрес. Вместо этого он будет использовать частный IP-адрес, указанный в`routerPrivateClusterIp` | таверна <br> private | таверна |
