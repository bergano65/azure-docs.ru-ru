---
title: Запечатлеть изображение Linux VM с помощью Azure CLI
...
Вы можете использовать сервис **Azure VM Image Builder (Public Preview)** для создания пользовательского изображения, не нужно изучать какие-либо инструменты или настраивать конвейеры, просто предоставляя конфигурацию изображения, и создатель изображения создаст изображение. Для получения дополнительной [Getting Started with Azure VM Image Builder](https://docs.microsoft.com/azure/virtual-machines/linux/image-builder-overview)информации см.
...
## <a name="prefer-a-tutorial-instead"></a>Предпочитаете учебник вместо этого?

Для тестирования, оценки или изучения виртуальных машин в Azure можно использовать упрощенную версию этой статьи: [Руководство. Создание пользовательского образа виртуальной машины Azure с помощью Azure CLI](tutorial-custom-images.md).  В противном случае, продолжайте читать здесь, чтобы получить полную картину.


## <a name="step-1-deprovision-the-vm"></a>Шаг 1. Отзыв виртуальной машины
Сначала отзовите виртуальную машину с помощью агента виртуальной машины Azure, чтобы удалить файлы и данные конкретной машины. На исходной виртуальной машине Linux выполните команду `waagent` с параметром `-deprovision+user`. Для получения дополнительной информации смотрите [руководство пользователя Azure Linux Agent.](../extensions/agent-linux.md)
...
3. Введите **y,** чтобы продолжить. Чтобы предотвратить появление запроса на подтверждение, добавьте параметр `-force` .
4. После выполнения команды введите **выйти**, чтобы закрыть SSH-клиент.  VM будет по-прежнему работает в этой точке.
...
1. Отмените подготовку виртуальной машины, распределение которой вы отменили ранее с помощью команды [az vm deallocate](/cli/azure/vm). Следующий пример выделяет VM под названием *myVM* в ресурсной группе под названием *myResourceGroup*.  
    ...
    Подождите, пока VM полностью разложить, прежде чем двигаться дальше. Этот процесс может занять несколько минут.  VM закрыт во время сделки.

2. Пометьте виртуальную машину как универсальную с помощью команды [az vm generalize](/cli/azure/vm). Следующий пример отмечает VM имени *myVM* в группе ресурсов под названием *myResourceGroup* как обобщенный.
    ...
    Обобщенный VM больше не может быть перезапущен.
...
Эта команда возвращает JSON, описывающий изображение VM. Сохраните этот вывод для последующей ссылки.
