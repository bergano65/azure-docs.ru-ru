...
* [Смена пароля администратора](#password)
* [Настройка для удаленных соединений](#remote)
...
1. В поле поиска введите **S'L Server 2019**и выберите **Enter,** чтобы начать поиск.

1. Ограничьте результаты поиска, выбрав **операционную систему** > **Redhat.**

    ![Фильтр поиска для изображений VM-сервера S'L Server 2019](./media/provision-sql-server-linux-virtual-machine/searchfilter.png)

1. Из результатов поиска выберите изображение Linux сервера 2019. В этом учебном пособии используется **сервер S'L 2019 на RHEL74**.
...
1. Затем введите или выберите следующие значения:
   * **Регион**: Выберите подходящий для вас регион Azure.
   * **Параметры доступности**: Выберите вариант доступности и избыточности, который лучше всего подходит для ваших приложений и данных.
   * **Изменение размера:** Выберите эту опцию, чтобы выбрать размер машины и когда это сделать, **выберите.** Дополнительные сведения о размерах виртуальных машин см. в [этой статье](https://docs.microsoft.com/azure/virtual-machines/virtual-machines-linux-sizes).
   ...
   * **Имя пользователя**: Введите имя администратора для VM.
   * **SSH открытый ключ:** Введите ваш открытый ключ RSA.
   * **Публичные входящие порты**: Выберите **Разрешить выбранные порты** и выбрать порт **SSH (22)** в **списке открытых входящих портов Select.** В этом руководстве это необходимо для подключения и завершения настройки SQL Server. Если вы хотите удаленно подключиться к серверу S'L, вам нужно будет вручную разрешить трафик в порт по умолчанию (1433), используемый microsoft S'L Server для подключения через Интернет после создания виртуальной машины.
...
## <a name="change-the-sa-password"></a><a id="password"></a>Изменение пароля SA
...
1. Выполнить следующие команды, чтобы изменить **PATH** как для сеансов входа, так и для интерактивных/нелогиновых сессий:
