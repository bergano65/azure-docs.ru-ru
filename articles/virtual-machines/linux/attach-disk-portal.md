---
title: Прикрепите диск данных к Linux VM
...
* Размер виртуальной машины определяет, сколько дисков данных к ней можно подключить. Для получения [подробной](sizes.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)информации см.
...
1. Перейдите на [портал Azure,](https://portal.azure.com/) чтобы найти VM. Поиск и выбор **виртуальных машин.**
2. Выберите виртуальную машину из списка.
3. В панели страницы **виртуальных машин,** под **настройками,** выберите **диски.**
...
5. После того, как Azure создает диск и прикрепляет его к виртуальной машине, новый диск указан в настройках диска виртуальной машины под **дисками data.** Так как управляемые диски являются ресурсами верхнего уровня, диск отображается в корне группы ресурсов:
...
Чтобы разбить диск на разделы, отформатировать и подключить новый диск к виртуальной машине Linux, подключитесь к своей виртуальной машине по протоколу SSH. Для получения дополнительной [How to use SSH with Linux on Azure](mac-create-ssh-keys.md)информации см. В следующем примере выполняется подключение к виртуальной машине с помощью общедоступной записи DNS *mypublicdns.westus.cloudapp.azure.com* и имени пользователя *azureuser*: 

```bash
ssh azureuser@mypublicdns.westus.cloudapp.azure.com
```

После подключения к виртуальной машине можно подключить диск. Сначала найдите диск, используя `dmesg` (для обнаружения нового диска можно использовать и другой способ). Следующий пример использует dmesg для фильтрации на дисках *SCSI:*
...
> [!NOTE]
> Рекомендуется использовать последние версии fdisk или расстались, которые доступны для вашего дистро.
...
#### <a name="alternate-method-using-parted"></a>Альтернативный метод с использованием расстались
Утилита fdisk нуждается в интерактивном входе и, следовательно, не идеально подходит для использования в сценариях автоматизации. Тем не менее, [часть утилиты](https://www.gnu.org/software/parted/) может быть сценарий и, следовательно, поддается лучше в сценариях автоматизации. Упрощаемый утилита может использоваться для раздела и форматации диска данных. Для пошагового мы используем новый диск данных /dev/sdc и форматизуем его с помощью файловой системы [XFS.](https://xfs.wiki.kernel.org/)
```bash
sudo parted /dev/sdc --script mklabel gpt mkpart xfspart xfs 0% 100%
partprobe /dev/sdc1
```
Как указано выше, мы используем утилиту [partprobe,](https://linux.die.net/man/8/partprobe) чтобы убедиться, что ядро немедленно известно о новой системе раздела и файловой системе. Неиспользование partprobe может привести к тому, что команды blkid или lslbk немедленно не вернут UUID для новой файловой системы.
...
> [!NOTE]
> Неправильное редактирование **файла /etc/fstab** может привести к незагрузочной системе. Если у вас есть сомнения, см. инструкции по правильному изменению этого файла в документации дистрибутива. Также рекомендуется перед внесением изменений создать резервную копию файла /etc/fstab.
...
```
После этого сохраните *файл /etc/fstab* и перезагрузите систему.
> [!NOTE]
...
> 
> Параметр *nofail* обеспечивает запуск виртуальной машины даже в том случае, если файловая система повреждена или отсутствует диск во время загрузки. Без этой опции, вы можете столкнуться с поведением, как описано в [Cannot SSH к Linux VM из-за ошибок FSTAB](https://blogs.msdn.microsoft.com/linuxonazure/2016/07/21/cannot-ssh-to-linux-vm-after-adding-data-disk-to-etcfstab-and-rebooting/)
