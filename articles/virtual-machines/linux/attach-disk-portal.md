...
3. На боковой панели страницы **Виртуальные машины** в разделе **Параметры** выберите **Диски**.
...
#### <a name="alternate-method-using-parted"></a>Альтернативный метод с использованием Parted
Служебной программе fdisk требуется интерактивный ввод, поэтому она не идеально подходит для использования в скриптах автоматизации. Однако служебная программа [Parted](https://www.gnu.org/software/parted/) поддерживает скрипты, и, следовательно, она лучше подходит для сценариев автоматизации. Служебную программу Parted можно использовать для разделения диска данных на разделы и его форматирования. В этом пошаговом руководстве мы используем новый диск данных /dev/sdc и отформатируем его с помощью файловой системы [XFS](https://xfs.wiki.kernel.org/).
```bash
sudo parted /dev/sdc --script mklabel gpt mkpart xfspart xfs 0% 100%
sudo mkfs.xfs /dev/sdc1
partprobe /dev/sdc1
```
Как показано выше, мы используем служебную программу [partprobe](https://linux.die.net/man/8/partprobe), чтобы убедиться, что ядру известно о новом разделе и файловой системе. Невозможность использовать partprobe может привести к тому, что команды blkid или lslbk не будут немедленно возвращать UUID для новой файловой системы немедленно.
