...
```
> [!NOTE]
> Cloud-init имеет несколько [типов входных данных](https://cloudinit.readthedocs.io/en/latest/topics/format.html). Cloud-init будет использовать первую строку CustomData/UserData для указания того, как она должна обрабатывать входные данные, например указывает, что `#cloud-config` содержимое должно обрабатываться как конфигурация Cloud-init.
