...
Мы активно сотрудничаем с нашими утвержденными партнерами, работающими над дистрибутивами Linux, чтобы образы с поддержкой cloud-init стали доступными в Azure Marketplace. Эти образы обеспечивают бесперебойную работу развертываний и конфигураций cloud-init с виртуальными машинами и масштабируемыми наборами виртуальных машин. Сначала мы сотрудничаем с подтвержденными партнерами Linux дистрибутив и вышестоящими, чтобы обеспечить функции Cloud-init с ОС в Azure, после чего пакеты обновляются и становятся общедоступными в репозиториях пакетов дистрибутив. 
...
| Издатель/версия| Предложение | SKU | Версия | Облако образов — готово к инициализации | Поддержка пакетов Cloud-init в Azure|
|:--- |:--- |:--- |:--- |:--- |:--- |
|Каноническая 18,04 |UbuntuServer |18.04-LTS |latest |да | да |
|Каноническая 16,04|UbuntuServer |16.04-LTS |latest |да | да |
|Каноническая 14,04|UbuntuServer |14.04.5-LTS |latest |да | да |
...
|RedHat 7.7 |RHEL |7-RAW | Н/Д| обновления без образа для запуска февраля 2020| Да — поддержка из версии пакета: *18.5 -3. el7*|
|RedHat 7.7 |RHEL |7-LVM | Н/Д| обновления без образа для запуска февраля 2020| Да — поддержка из версии пакета: *18.5 -3. el7*|
|RedHat 7.7 |RHEL |7,7 | Н/Д| обновления без образа для запуска февраля 2020 | Да — поддержка из версии пакета: *18.5 -3. el7*|
|RedHat 7.7 |RHEL — BYOS | RHEL — lvm77 | Н/Д|обновления без образа для запуска февраля 2020  | Да — поддержка из версии пакета: *18.5 -3. el7*|
