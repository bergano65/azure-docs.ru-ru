...
> - Сейчас виртуальные машины поколения 2 не поддерживают шифрование дисков Azure. Подробнее см. в статье о [поддержке виртуальных машин поколения 2 в Azure](https://docs.microsoft.com/azure/virtual-machines/windows/generation-2).
...
| RedHat | RHEL 7.7 | 7-RAW | RedHat:RHEL:7-RAW:latest | ОС и диск данных (см. примечание ниже) |
...
Для шифрования дисков Azure в системе должны быть установлены модули dm-crypt и vfat. Если вы удалите или отключите vfat от образа по умолчанию, система не сможет считать том ключа и получать ключ, необходимый для разблокировки дисков при последующих перезагрузках. Действия по повышению безопасности системы, связанные с удалением модуля vfat, несовместимы с шифрованием дисков Azure. 
