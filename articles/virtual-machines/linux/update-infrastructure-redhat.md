...
* Azure RHUI — инфраструктура обновления, которая поддерживает все виртуальные машины PAYG RHEL, созданные в Azure. Это препятствует регистрации виртуальных машин RHEL (PAYG) диспетчер подписки или вспомогательных или в другом источнике обновлений, но это с виртуальной Машиной (PAYG) приведет к косвенных двойной выставления счетов. См. в разделе точку следующие сведения.
* В стоимость образа RHEL (PAYG) входит плата за доступ к инфраструктуре RHUI, размещенной в Azure. Отмена регистрации виртуальной машины RHEL (PAYG) в инфраструктуре RHUI в Azure не приведет к ее преобразованию в виртуальную машину с использованием собственной лицензии (BYOL). В случае регистрации одной и той же виртуальной машины в другом источнике обновлений вы можете нести двойные _косвенные_ расходы. Первая плата будет взиматься за использование программного обеспечения Azure RHEL. Вторая плата будет взиматься за подписки Red Hat, которые были приобретены ранее. Если требуется использовать инфраструктуры обновления RHUI, размещенную в Azure, рассмотрите возможность регистрации для использования [образы RHEL BYOS](https://aka.ms/rhel-byos).
* По умолчанию версии rhui выполняется обновление виртуальной Машины RHEL с последним дополнительным номером, при выполнении `sudo yum update`.
    ...
    Чтобы избежать этого, можно переключиться на [каналы расширенной поддержки обновления](#rhel-eus-and-version-locking-rhel-vms) или создать собственный образ, как описано в разделе [Создание и отправка виртуальной машины на основе Red Hat для Azure](redhat-create-upload-vhd.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) статьи. Если вы создаете свой собственный образ, необходимо подключить его к другой инфраструктуре обновления ([непосредственно к Red Hat серверам доставки содержимого](https://access.redhat.com/solutions/253273) или [серверу Red Hat Satellite](https://access.redhat.com/products/red-hat-satellite)).
...
Во время написания этой статьи, EUS более поддерживаются для RHEL < = 7.3. См. в разделе «Red Hat Enterprise Linux больше поддержки надстройки» [документации Red Hat](https://access.redhat.com/support/policy/updates/errata/) для получения дополнительных сведений.
* RHEL 7.4 EUS поддержка прекращается 31 августа 2019 г.
* Прекращение поддержки на RHEL 7.5 EUS, 30 апреля 2020 г.
* RHEL 7.6 EUS поддержка прекращается 31 октября 2020 г.

### <a name="switch-a-rhel-vm-to-eus-version-lock-to-a-specific-minor-version"></a>Переключитесь в виртуальную Машину RHEL EUS (до определенной версии дополнительный номер версии блокировка)
Используйте приведенные ниже инструкции, чтобы заблокировать виртуальную Машину RHEL для определенного дополнительного номера версии (запускается в качестве привилегированного пользователя):
...
1. Заблокировать переменную releasever (запускается в качестве привилегированного пользователя):
    ```bash
...
### <a name="switch-a-rhel-vm-back-to-non-eus-remove-a-version-lock"></a>Снова перевести виртуальную Машину RHEL не EUS (снятия блокировки версии)
Выполните следующую команду в качестве привилегированного пользователя.
1. Удалите файл releasever:
    ```bash
    rm /etc/yum/vars/releasever
     ```

1. Отключите EUS репозиториев:
    ```bash
