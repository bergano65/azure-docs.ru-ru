...
В следующем примере создается субъект-служба и назначаются разрешения участника для группы ресурсов с именем *опеншифтрг*.

Сначала создайте группу ресурсов с именем *опеншифтрг*:
...
Секреты должны быть созданы для закрытого ключа SSH (**сшприватекэй**), секрета клиента Azure AD (**aadClientSecret**), пароля администратора OpenShift (**Опеншифтпассворд**) и пароля диспетчера подписки Red Hat или ключа активации (**рхсмпассвордорактиватионкэй**).  Кроме того, если используются пользовательские сертификаты TLS/SSL, необходимо создать шесть дополнительных секретов — **раутингкафиле**, **раутингцертфиле**, **раутингкэйфиле**, **мастеркафиле**, **мастерцертфиле**и **мастеркэйфиле**.  Эти параметры будут более подробно описаны.
