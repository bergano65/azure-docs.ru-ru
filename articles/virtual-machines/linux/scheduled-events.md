...
| Выполнена | 30 секунд |
| Заканчива | [Настраиваемый пользователем](../../virtual-machine-scale-sets/virtual-machine-scale-sets-terminate-notification.md#enable-terminate-notifications): от 5 до 15 минут |

> [!NOTE] 
> В некоторых случаях Azure может предсказать сбой узла из-за сниженного оборудования и попытается снизить перерывы в работе службы путем планирования миграции. Затронутые виртуальные машины получат запланированное событие с `NotBefore`, которое обычно составляет несколько дней в будущем. Фактическое время зависит от оценки рисков предсказанных сбоев. По возможности Azure пытается предоставить в течение 7 дней предварительное уведомление, но фактическое время меняется и может быть меньше, если прогноз состоит в том, что высокая вероятность неудачи оборудования. Чтобы снизить риски в службе в случае сбоя оборудования до начала процесса миграции, рекомендуется как можно скорее развернуть виртуальную машину самостоятельно.
