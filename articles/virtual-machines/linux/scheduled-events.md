...
- [Платформа инициировала техническое обслуживание](https://docs.microsoft.com/azure/virtual-machines/linux/maintenance-and-updates) (например, перезагрузка VM, живая миграция или сохранение обновлений памяти для хоста)
- Виртуальная машина работает на [деградированных хост аппаратных](https://azure.microsoft.com/blog/find-out-when-your-virtual-machine-hardware-is-degraded-with-scheduled-events) средств, которые, по прогнозам, сбой в ближайшее время
...
- [Spot VM](spot-vms.md) и [Spot шкала набор](../../virtual-machine-scale-sets/use-spot.md) экземпляров выселения.
...
| - | - | - | - | 
| 2019-01-01 | Общедоступная версия | All | <li> Добавлена поддержка виртуальных наборов масштабов машин EventType 'Terminate' |
| 2017-11-01 | Общедоступная версия | All | <li> Добавлена поддержка Spot VM выселения EventType 'Preempt'<br> | 
...
| Упредить | 30 секунд |
| Завершить | [Настраиваемый для пользователя](../../virtual-machine-scale-sets/virtual-machine-scale-sets-terminate-notification.md#enable-terminate-notifications): от 5 до 15 минут |

> [!NOTE] 
> В некоторых случаях Azure может предсказать сбой хоста из-за деградировавого оборудования и попытается смягчить сбои в работе службы путем планирования миграции. Пострадавшие виртуальные машины получат `NotBefore` запланированное событие с тем, что, как правило, несколько дней в будущем. Фактическое время варьируется в зависимости от прогнозируемой оценки риска сбоя. Azure пытается дать 7 дней заблаговременное уведомление, когда это возможно, но фактическое время меняется и может быть меньше, если прогноз, что есть высокая вероятность отказа аппаратного обеспечения в ближайшее время. Чтобы свести к минимуму риск для службы в случае сбоя оборудования до развертывания системы, мы рекомендуем вам как можно скорее самостоятельно перераспределить виртуальную машину.
...
- Узнайте о [планируемом обслуживании виртуальных машин Linux в Azure.](planned-maintenance.md)
