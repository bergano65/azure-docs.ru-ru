---
title: Запланированные события для виртуальных машин Linux в Azure
...
- [Обслуживание, инициированное платформой](https://docs.microsoft.com/azure/virtual-machines/linux/maintenance-and-updates) (например, перезагрузка виртуальной машины, динамическая миграция или сохранение обновлений памяти для узла)
- Виртуальная машина работает на [оборудовании узла с пониженной производительностью](https://azure.microsoft.com/blog/find-out-when-your-virtual-machine-hardware-is-degraded-with-scheduled-events) , прогнозируемое на сбой в ближайшее время
...
- Обнаружение вытеснений экземпляров [виртуальной машины](spot-vms.md) и [плашечного набора](../../virtual-machine-scale-sets/use-spot.md) .
...
| - | - | - | - | 
| 2019-01-01 | Общедоступная версия | Все | <li> Добавлена поддержка для масштабируемых наборов виртуальных машин тип данных EventType "Terminate" |
| 2017-11-01 | Общедоступная версия | Все | <li> Добавлена поддержка для выборочного вытеснения виртуальной машины с вытеснением "вытеснение"<br> | 
...
| 2017-03-01 | Preview (Предварительный просмотр) | Все | <li>Начальный выпуск |
...
| Выполнена | 30 секунд |
| Завершить | [Настраиваемый пользователем](../../virtual-machine-scale-sets/virtual-machine-scale-sets-terminate-notification.md#enable-terminate-notifications): от 5 до 15 минут |

> [!NOTE] 
> В некоторых случаях Azure может предсказать сбой узла из-за сниженного оборудования и попытается снизить перерывы в работе службы путем планирования миграции. Затронутые виртуальные машины получат запланированное событие `NotBefore` , которое, как правило, будет выполняться через несколько дней в будущем. Фактическое время зависит от оценки рисков предсказанных сбоев. По возможности Azure пытается предоставить в течение 7 дней предварительное уведомление, но фактическое время меняется и может быть меньше, если прогноз состоит в том, что высокая вероятность неудачи оборудования. Чтобы снизить риски в службе в случае сбоя оборудования до начала процесса миграции, рекомендуется как можно скорее развернуть виртуальную машину самостоятельно.
...
- Ознакомьтесь с примерами кода Запланированные события в [метаданных экземпляра Azure запланированные события репозитории GitHub](https://github.com/Azure-Samples/virtual-machines-scheduled-events-discover-endpoint-for-non-vnet-vm).
- Дополнительные сведения об API, доступных в [службе метаданных экземпляра](instance-metadata-service.md).
- Узнайте о [плановом обслуживании виртуальных машин Linux в Azure](planned-maintenance.md).
