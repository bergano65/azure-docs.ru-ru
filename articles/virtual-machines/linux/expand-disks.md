---
title: Расширить виртуальные жесткие диски на Linux VM
description: Узнайте, как расширить виртуальные жесткие диски на Linux VM с помощью Azure CLI.
author: roygara
...
> [!WARNING]
> Всегда убедитесь, что файловая система находится в работоспособном состоянии, тип таблицы раздела диска будет поддерживать новый размер и обеспечить резервную копию данных перед выполнением операций по восстановлению диска. Дополнительные сведения см. в статье [Архивация виртуальных машин Linux в Azure](tutorial-backup-vms.md). 
...
1. Для проверки диска данных `df -h`был уменьшен, используйте . В следующем примере выходных данных показано, что теперь размер основного раздела (*/dev/sdc1*) составляет 200 ГБ:
...
* Для получения дополнительной информации [Azure Disk Encryption for Linux VMs](disk-encryption-overview.md)о шифровании дисков см.
