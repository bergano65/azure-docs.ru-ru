...
customData | Просмотреть [пользовательские данные](#custom-data) | 2019-02-01
...
Служба метаданных экземпляра доступна через конечную точку HTTP по адресу 169.254.169.254. Один из сценариев работы Службы метаданных экземпляров — предоставление гарантий, что полученные данные действительно поступили из Azure. Мы подписываем часть этой информации, чтобы гарантировать, что в Azureработает надлежащий образ Marketplace.
