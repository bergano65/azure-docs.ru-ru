...
Сюда входят сведения о номере SKU, хранилище, конфигурации сети и ближайших мероприятиях обслуживания. Полный список доступных данных см. в разделе [API метаданных](#metadata-apis). Служба метаданных экземпляра доступна для виртуальной машины и экземпляров масштабируемого набора виртуальных машин. Используйте метаданные экземпляров для запуска виртуальных машин, созданных или управляемых с помощью [диспетчера ресурсов Azure](https://docs.microsoft.com/rest/api/resources/). 

Служба метаданных экземпляров Azure представляет собой конечную точку REST, доступную по известному IP-адресу без поддержки маршрутизации (`169.254.169.254`), к которому можно получить доступ только в пределах виртуальной машины.
...
> [!NOTE]
> `format=json` не работает для конечных объектов. Для этих запросов необходимо явно задать `format=text`, если по умолчанию используется формат JSON.

### <a name="versioning"></a>Управление версиями

Указание версии службы метаданных экземпляра и версии API в HTTP-запросе является обязательным.

Поддерживаются следующие версии службы: 2017-04-02, 2017-08-01, 2017-12-01, 2018-02-01, 2018-04-02, 2018-10-01, 2019-02-01, 2019-03-11, 2019-04-30, 2019-06-01, 2019-06-04, 2019-08-01, 2019-08-15.

Обратите внимание, что выпуск новой версии для развертывания во всех регионах может занять некоторое время. Сейчас версия 2019-11-01 находится в процессе развертывания и может быть доступна не во всех регионах.
...
customData | Эта функция сейчас отключена, и мы планируем обновить эту документацию, когда она станет доступной | 2019-02-01
...
Профиль службы хранилища виртуальной машины разделен на три категории: ссылки на образ, диск ОС и диски данных.
...
Поле `tags` представляет собой строку с тегами, разделенными точкой с запятой. Это может быть проблемой, если в самих тегах уже используются точки с запятой. Если средство синтаксического анализа настроено для программного извлечения тегов, следует использовать поле `tagsList`, которое является массивом JSON без разделителей, благодаря чему его проще проанализировать.
...
Необязательное поле Nonce содержит 10-значное строковое значение. Если значение не указано, IMDS возвращает вместо него текущую метку времени в формате UTC. Из-за механизма кэширования IMDS может возвращаться ранее кэшированное значение nonce.
...
Большой двоичный объект подписи является версией документа с подписью [pkcs7](https://aka.ms/pkcs7). Он содержит сертификат, который используется для подписи, и некоторые сведения о виртуальной машине, в том числе идентификатор, значение nonce, идентификатор подписки, метку времени создания и время истечения срока действия документа, а также сведения о плане для образа. Сведения о плане заполняются только для образов из Azure Marketplace. Можно извлечь сертификат из ответа и проверить с его помощью, является ли ответ допустимым и поступает ли он из Azure.
...
timestamp/createdOn | Метка времени в формате UTC, обозначающая время создания первого подписанного документа
timestamp/expiresOn | Метка времени в формате UTC, обозначающая время истечения срока действия подписанного документа
...
#### <a name="sample-3-verifying-the-signature"></a>Пример 3. Проверка подписи
...
Пользователь может включить управляемое удостоверение на виртуальной машине, а затем использовать службу метаданных экземпляра для передачи маркера доступа службам Azure. Приложения, запущенные на виртуальной машине, теперь могут запрашивать маркер из конечной точки службы метаданных экземпляров Azure, а затем использовать его для проверки подлинности в облачных службах, включая хранилище ключей.
...
Эта служба является **общедоступной** во всех регионах Azure. В том числе: 
1. [Все общедоступные глобальные регионы Azure](https://azure.microsoft.com/regions/)
2. [Azure для государственных организаций](https://azure.microsoft.com/overview/clouds/government/)  
3. [Azure China 21Vianet](https://www.azure.cn/) 
4. [Azure для Германии](https://azure.microsoft.com/overview/clouds/germany/) 
...
----------------|-------
200 ОК |
400 — недопустимый запрос | Отсутствует заголовок `Metadata: true` или отсутствует формат при запросе конечного объекта
...
9. Почему я получаю запрос о превышении интервала ожидания вызова службы?
   * Вызовы метаданных должны быть сделаны с основного IP-адреса, назначенного основной сетевой карте виртуальной машины. Кроме того, если вы изменили маршруты, то должен быть маршрут для адреса 169.254.0.0/16 из вашей сетевой карты.
