...
Версия 2019-11-01 в настоящее время развертывается и может быть недоступна во всех регионах.
...
410 — потеряно | Повторите попытку через некоторое время в течение 70 секунд
...
экземпляр | См. [API экземпляра](#instance-api) | 2017-04-02
...
customData | Эта функция сейчас отключена, и мы будем обновлять эту документацию, когда она станет доступной | 2019-02-01
...
platformUpdateDomain |  [Обновление домена](manage-availability.md) , в котором запущена виртуальная машина | 2017-04-02
platformFaultDomain | [Домен сбоя](manage-availability.md) , в котором запущена виртуальная машина | 2017-04-02
...
Существует известная ошибка, связанная с сертификатом, используемым для подписания. Сертификаты могут не иметь точного соответствия `metadata.azure.com` для общедоступного облака. Поэтому проверка сертификата должна допускать общее имя из любого `.metadata.azure.com` поддомена.
...
Если промежуточный сертификат не может быть загружен из-за сетевых ограничений во время проверки, можно закрепить промежуточный сертификат. Однако Azure будет переключать сертификаты в соответствии со стандартной практикой PKI. Прикрепленные сертификаты должны быть обновлены, когда происходит смена. При каждом запланированном изменении промежуточного сертификата блог Azure будет обновлен, а клиенты Azure будут уведомлены об этом. Промежуточные сертификаты можно найти [здесь](https://www.microsoft.com/pki/mscorp/cps/default.htm). Промежуточные сертификаты для каждого региона могут отличаться.

> [!NOTE]
>Промежуточный сертификат для Azure китайского центра для Китая будет находиться в глобальном корневом ЦС DigiCert, а не в Baltimore.
Кроме того, если вы зафиксированы промежуточные сертификаты для Azure для Китая в рамках изменения центра корневых цепочек, промежуточные сертификаты должны быть обновлены.
...
- Дополнительные сведения о [запланированные события](scheduled-events.md)
