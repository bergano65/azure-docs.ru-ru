...
`type` — это тип ресурса, который должен быть `"Microsoft.VirtualMachineImages/imageTemplates"`. `apiVersion` со временем будет меняться по мере изменений API, но для предварительной версии должно быть указано значение `"2019-05-01-preview"`.
...
По умолчанию Конструктор образов поддерживает использование скриптов или копирование файлов из нескольких расположений, таких как GitHub и служба хранилища Azure. Чтобы можно было использовать такие расположения, они должны быть общедоступными.

Вы также можете использовать заданное вами управляемое удостоверение Azure, назначаемое пользователем, чтобы разрешить Конструктору образов доступ к службе хранилища Azure, если этому удостоверению предоставлено хотя бы минимальное разрешение "Модуль чтения данных больших двоичных объектов хранилища" в учетной записи хранения Azure. Это означает, что вам не нужно делать большие двоичные объекты хранилища доступными извне или настраивать маркеры безопасности SAS.
...
Полный пример см. в разделе [Использование управляемого удостоверения Azure, назначаемого пользователем, для доступа к файлам в службе хранилища Azure](https://github.com/danielsollondon/azvmimagebuilder/tree/master/quickquickstarts/7_Creating_Custom_Image_using_MSI_to_Access_Storage).

Конструктор образов поддерживает удостоверение, назначаемое пользователем, со следующими ограничениями: • поддерживается только одно удостоверение; • не поддерживаются имена личных доменов.
...
Эта функциональная возможность в Конструкторе образов объявлена нерекомендуемой, так как теперь имеются [образы BYOS RHEL](https://docs.microsoft.com/azure/virtual-machines/workloads/redhat/byos). Ознакомьтесь со следующими сроками:
    * с 31 марта 2020 года шаблоны образов с источниками ISO RHEL больше не будут приниматься поставщиком ресурсов;
    * с 30 апреля 2020 года шаблоны образов, содержащие источники ISO RHEL, больше не будут обрабатываться.
...
> [!NOTE]
> При запуске настройщика оболочки с источником ISO RHEL необходимо убедиться, что первая оболочка настройки обрабатывает регистрацию на сервере предоставления прав Red Hat, прежде чем выполнять какую-либо настройку. После завершения настройки скрипт должен отменить регистрацию на сервере предоставления прав.
...
> [!NOTE]
> Вы можете использовать Конструктор образов Azure в регионе, отличном от того, в котором развернута коллекция, но службе "Конструктор образов виртуальных машин Azure" придется перемещать образ между центрами обработки данных, и это займет больше времени. Конструктор образов автоматически назначает версию образа на основе монотонного целого числа. В настоящее время вы не можете указывать версию. 
