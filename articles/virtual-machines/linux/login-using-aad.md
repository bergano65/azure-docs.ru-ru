---
title: Войти в Linux VM с учетными данными Active Directory Azure
description: Узнайте, как создать и настроить Linux VM для вхлечения в систему с помощью проверки подлинности Active Directory Azure.
author: iainfoulds
...
---
# <a name="preview-log-in-to-a-linux-virtual-machine-in-azure-using-azure-active-directory-authentication"></a>Предварительный просмотр: Вход в виртуальную машину Linux в Azure с помощью проверки подлинности Active Directory Azure
...
> [!IMPORTANT]
> Активная аутентификация Azure Active Directory в настоящее время находится в общедоступном предварительном просмотре.
...
>
> Это расширение не поддерживается в кластерах Azure Kubernetes Service (AKS). Для получения дополнительной [информации](../../aks/support-policies.md)см.
...
Чтобы включить аутентификацию Azure AD для Linux VMs в Azure, необходимо обеспечить, чтобы конфигурация сети VMs позволила получить доступ к следующим конечным точкам по порту TCP 443:
...
> [!NOTE]
> В настоящее время группы сетевой безопасности Azure не могут быть настроены для с врезок с помощью аутентификации Azure AD.
...
> [!NOTE]
> При развертывании этого расширения на ранее созданный VM обеспечить машину, по крайней мере 1 ГБ памяти выделено иначе расширение не сможет установить

Чтобы войти в Linux VM с учетными данными Azure AD, установите расширение входа в VM Active Directory Azure. Расширения виртуальных машин — это небольшие приложения, которые выполняют задачи настройки и автоматизации после развертывания виртуальных машин Azure. С помощью команды [az vm extension set](/cli/azure/vm/extension#az-vm-extension-set) установите расширение *AADLoginForLinux* на виртуальную машину с именем *myVM* в группе ресурсов *myResourceGroup*:
...
Положение о *представлении* *Succeeded* отображается после успешного установки расширения на VM. VM нужен работающий VM-агент для установки расширения. Для получения дополнительной [VM Agent Overview](https://docs.microsoft.com/azure/virtual-machines/extensions/agent-windows)информации см.
...
Войдите на виртуальную машину Azure с Linux, используя учетные данные Azure AD. Параметр `-l` позволяет указать собственный адрес учетной записи Azure AD. Замените пример учетной записи своей собственной. Адреса учетных записей следует вводить в нижнем регистре. Замените пример IP-адреса общедоступным IP-адресом вашего VM из предыдущей команды.

```console
ssh -l azureuser@contoso.onmicrosoft.com 10.11.123.456
```

Вам предлагается войти в Azure AD с одноразовым [https://microsoft.com/devicelogin](https://microsoft.com/devicelogin)кодом использования в. Копировать и вставить одноразовый код на страницу входа устройства.

При появлении запроса введите учетные данные для входа в Azure AD на странице входа. 

Следующее сообщение отображается в веб-браузере, когда вы успешно аутентифицированы:`You have signed in to the Microsoft Azure Linux Virtual Machine Sign-In application on your device.`

Закройте окно браузера, вернитесь к запросу SSH и нажмите клавишу **ВВОД**. 

Теперь вы выполнили вход на виртуальную машину Azure под управлением Linux с разрешениями назначенных ролей, таких как *пользователь виртуальной машины* или *администратор виртуальной машины*. Если учетной записи пользователя назначена роль *виртуального администратора машины,* можно использовать `sudo` для выполнения команд, требующих корневых привилегий.
...
- Проверьте, правильно ли указано имя входа в командной строке SSH. Опечатка в имени входа может вызвать несоответствие между именем входа, указанным в командной строке SSH, и учетной записью, с которой вы вошли в Azure AD. Например, вместо *contoso.onmicrosoft.com лазуритов\@* вы набрали *contoso.onmicrosoft.com azuresuer.\@*
...
Пользователи, которые наследуют права доступа через вложенные группы или назначения ролей, в настоящее время не поддерживаются. Пользователю или группе должны быть непосредственно [назначены обязательные назначения ролей.](#configure-role-assignments-for-the-vm) Например, использование групп управления или вложенных назначений ролей группы не дает правильных разрешений, позволяющих пользователю войти в систему.
