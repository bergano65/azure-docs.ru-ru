...
1.    Необязательно. Зарегистрируйте виртуальные машины, получив преимущество с помощью отдельного источника обновлений (переключение между виртуальными машинами может остаться прикрепленным к [RHUI](../workloads/redhat/redhat-rhui.md) или зарегистрированным через RHSM).
...
|"Параметр для Преимущество гибридного использования Azure не отображается на портале" | Это известная проблема для виртуальных машин RHEL и SLES, созданных из общей коллекции образов, моментальных снимков или захваченных образов PAYG. В этом случае используйте действия CLI, описанные в разделе "[Включение и отключение преимуществ в Azure CLI](#enable-and-disable-the-benefit-in-the-azure-cli)". Чтобы просмотреть состояние АхБ, используйте команду ` az vm get-instance-view -g MyResourceGroup -n MyVm` .|
