...
1. Не создавайте пространство подкачки на диске ОС.
    ...
    Агент Linux для Azure может автоматически настраивать размер области подкачки с использованием локального диска ресурсов, подключенного к виртуальной машине после ее подготовки в Azure. Обратите внимание, что локальный диск ресурса является временным диском и что он может быть опорожнен, если виртуальная машина дезоснащена. После установки агента Linux для Azure (см. предыдущий шаг) измените соответствующим образом следующие параметры в файле /etc/waagent.conf:
...
1. Нажмите **Action** > **Shut Down** в Hyper-V Manager. Виртуальный жесткий диск Linux готов к передаче в Azure.
...
1. Не создавайте пространство подкачки на диске ОС.
    ...
    Агент Linux для Azure может автоматически настраивать размер области подкачки с использованием локального диска ресурсов, подключенного к виртуальной машине после ее подготовки в Azure. Обратите внимание, что локальный диск ресурса является временным диском, и он может быть опорожнен, если виртуальная машина дезпредоставляется. После установки агента Linux для Azure (см. предыдущий шаг) измените соответствующим образом следующие параметры в файле `/etc/waagent.conf`:
...
1.  Создайте файл kickstart, который будет включать содержимое ниже, и сохраните его. Дополнительные сведения об установке kickstart см. [здесь](https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/7/html/Installation_Guide/chap-kickstart-installations.html).
        ...
        PEERDNS=yes
        IPV6INIT=no
    PERSISTENT_DHCLIENT:да NM_CONTROLLED
...
Когда вы используете другую систему виртуализации (т.е. VirtualBox, Xen и т.д.) для подготовки изображения Linux, возможно, потребуется восстановить initrd, чтобы убедиться, что по крайней мере hv_vmbus и hv_storvsc модули ядра доступны на исходном диске оперативной памяти. Это известная проблема по крайней мере в системах на основе предшествующего дистрибутива Red Hat.
...
* Чтобы узнать больше об использовании готовых к производству изображений RHEL BYOS, перейдите на страницу документации для [BYOS](../workloads/redhat/byos.md).
