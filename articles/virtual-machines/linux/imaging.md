...
Azure поддерживает Hyper-V поколения 1 (Gen1) и поколения 2 (Gen2). Gen2 на данный момент является последним поколением и предоставляет больше возможностей, чем Gen1. Например, доступно больше памяти, а также есть поддержка расширений Intel Software Guard и виртуализированной персистентной памяти (vPMEM). Виртуальные машины 2-го поколения, работающие в локальной среде, поддерживают некоторые функции, которые еще не поддерживаются в Azure. Дополнительные сведения см. в разделе "Функции и возможности". Дополнительные сведения см. в этой [статье](../windows/generation-2.md). Создайте образы Gen2, если вам нужны расширенные функции.
