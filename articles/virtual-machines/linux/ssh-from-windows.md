...
* Доступ к Cloud Shell в веб-браузере по адресу [https://shell.azure.com](https://shell.azure.com) или в [портал Azure](https://portal.azure.com). 
...
3. Переместите указатель мыши на пустую область, чтобы обеспечить случайность ключа.

4. После создания открытого ключа при необходимости введите и подтвердите парольную фразу. Она будет запрашиваться при аутентификации на виртуальной машине с помощью вашего закрытого ключа SSH. Если не создать парольную фразу, пользователь, получивший ваш закрытый ключ, сможет войти на любую виртуальную машину или в службу, использующую этот ключ. Мы рекомендуем создать ее. Однако если вы забудете парольную фразу, восстановить ее будет невозможно.

5. Открытый ключ отображается в верхней части окна. Вы можете скопировать весь открытый ключ, а затем вставить его в портал Azure или в шаблон Azure Resource Manager при создании виртуальной машины Linux. Можно также выбрать **Сохранить открытый ключ** , чтобы сохранить копию на компьютере. Обратите внимание, что при сохранении в файл открытый ключ преобразуется в другой формат, [формате RFC4716](https://tools.ietf.org/html/rfc4716). Формат формате RFC4716 может быть несовместим со всеми API. Поэтому для использования в портал Azure рекомендуется скопировать открытый ключ, отображаемый в окне «Вывод».

    ![Сохранение файла PuTTY с открытым ключом](./media/ssh-from-windows/save-public-key.png)

6. При необходимости сохранить закрытый ключ в формате закрытого ключа PuTTy (PPK-файл) щелкните **Save private key** (Сохранить закрытый ключ). PPK-файл требуется, если вы хотите использовать PuTTY позже для установления SSH-подключения к виртуальной машине.

    ![Сохранение файла закрытого ключа PuTTY](./media/ssh-from-windows/save-ppk-file.png)

    Если вы хотите сохранить закрытый ключ в формате OpenSSH, формат закрытого ключа, используемый многими клиентами SSH, выберите **преобразования**  >  **Экспорт ключа OpenSSH**.
...
3. Выберите **Connection**  >  **SSH**  >  категорию**Проверка подлинности** SSH для подключения. Укажите закрытый ключ PuTTY (PPK-файл).
