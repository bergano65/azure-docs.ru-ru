...
Страница результатов откроется с **самостоятельно управляемой платформой контейнера Red Hat OpenShift** в списке. 
...
| Местоположение | Регион Azure для развертывания кластера |
...
| Имя Key Vault | Имя Key Vault, содержащее секретный код с закрытым ключом SSH.  Разрешается использовать только буквы, цифры и дефисы, а их длина должна составлять от 3 до 24 символов. |
