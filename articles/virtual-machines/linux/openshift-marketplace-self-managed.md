...
- **Метрики**. Можно включить hawkular метрики.
- **Ведение журналов.** Можно включить ведение журнала EFK.
...
| Расположение | Регион Azure для развертывания кластера в |
...
| Входной параметр | Описание параметра |
|-----------------------|-----------------|
| Префикс имени кластера OCP | Кластера префикс, используемый для настройки имен узлов для всех узлов. От 1 до 20 символов |
...
| Имя Key Vault | Имя хранилища ключей, которое содержит секрет с ssh закрытый ключ.  Только буквы, цифры и дефисы допускаются и находиться в диапазоне от 3 до 24 символов |
