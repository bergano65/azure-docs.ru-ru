...
Большинство размеров виртуальных машин HPC (HBv2, хб, HC, H16r, H16mr, A8 и A9) имеют сетевой интерфейс для подключения удаленного доступа к памяти (RDMA). Выбрано [N-Series]https://docs.microsoft.com/azure/virtual-machines/nc-series) (размеры, обозначенные "r", такие как конфигурации конфигурация nc24rs (NC24rs_v3, NC24rs_v2 и NC24r), также поддерживают RDMA. Этот интерфейс является дополнением к стандартному сетевому интерфейсу Azure, доступному в других размерах виртуальных машин.
...
- **Масштабируемые наборы виртуальных машин** . в масштабируемом наборе виртуальных машин (VMSS) убедитесь, что развертывание ограничивается одной группой размещения. Например, в шаблоне Resource Manager задайте значение `true` для свойства `singlePlacementGroup`. Обратите внимание, что максимальный размер VMSS, который можно задержать `singlePlacementGroup` с `true` помощью свойства, ограничен на 100 виртуальных машин по умолчанию. Если потребность в масштабе задания HPC превышает 100 виртуальных машин в одном клиенте VMSS, вы можете запросить увеличение, бесплатно [открыв запрос в службу поддержки клиентов в Интернете](../azure-supportability/how-to-create-azure-support-request.md) .
...
- Дополнительные сведения о оптимизации приложения HPC для Azure и некоторые примеры см. в [рабочих нагрузках HPC] (https://docs.microsoft.com/azure/virtual-machines/workloads/hpc/overview) 
