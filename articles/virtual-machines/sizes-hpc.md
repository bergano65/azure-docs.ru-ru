...
Большинство размеров виртуальных машин HPC (HBv2, хб, HC, H16r, H16mr, A8 и A9) имеют сетевой интерфейс для подключения удаленного доступа к памяти (RDMA). Выбранные размеры [серии N](https://docs.microsoft.com/azure/virtual-machines/nc-series) , обозначенные r, такие как конфигурации конфигурация nc24rs (NC24rs_v3, NC24rs_v2 и NC24r), также поддерживают RDMA. Этот интерфейс является дополнением к стандартному сетевому интерфейсу Azure, доступному в других размерах виртуальных машин.
...
> [!NOTE]
> В Azure HPC существует два класса виртуальных машин в зависимости от того, включены ли они в SR-IOV для InfiniBand. В настоящее время для виртуальных машин с поддержкой InfiniBand поддерживается SR-IOV: HBv2, хб, HC и NCv3. На остальных виртуальных машинах с поддержкой InfiniBand не включена поддержка SR-IOV.
...
- **Операционная система** — Linux очень хорошо поддерживается для ВИРТУАЛЬНЫХ машин HPC, дистрибутивов, таких как CENTOS, RHEL, Ubuntu, SUSE, являются общими. В отношении поддержки Windows Windows Server 2016 поддерживается на всех виртуальных машинах серии HPC. Windows Server 2012 R2, Windows Server 2012 также поддерживаются на виртуальных машинах, не использующих SR-IOV.

- **MPI** . размеры виртуальных машин с поддержкой SR-IOV в Azure (HBV2, хб, HC, NCv3) позволяют использовать почти любую версию MPI для использования с Mellanox офед.
...
- **Масштабируемые наборы виртуальных машин** . в масштабируемом наборе виртуальных машин (VMSS) убедитесь, что развертывание выполняется в одну группу размещения для взаимодействия InfiniBand в пределах VMSS. Например, в шаблоне Resource Manager задайте значение `true` для свойства `singlePlacementGroup`. Обратите внимание, что максимальный размер VMSS, который можно задержать `singlePlacementGroup` с `true` помощью свойства, ограничен на 100 виртуальных машин по умолчанию. Если потребность в масштабе задания HPC превышает 100 виртуальных машин в одном клиенте VMSS, вы можете запросить увеличение, бесплатно [открыв запрос в службу поддержки клиентов в Интернете](../azure-supportability/how-to-create-azure-support-request.md) . Ограничение на количество виртуальных машин в одном VMSS может быть увеличено до 300. Обратите внимание, что при развертывании виртуальных машин с использованием групп доступности максимальное ограничение составляет 200 виртуальных машин на группу доступности.
...
- **Квота ядер**. Вам может потребоваться увеличить стандартную квоту на число ядер в подписке Azure. Кроме того, количество ядер, которые можно развернуть для некоторых семейств размеров виртуальных машин (включая серию H), может быть ограничено условиями вашей подписки. Чтобы запросить увеличение квоты, отправьте [запрос в службу поддержки клиентов](../azure-supportability/how-to-create-azure-support-request.md) бесплатно. (Ограничения по умолчанию могут быть разными в зависимости от категории подписки).
...
- Дополнительные сведения о оптимизации приложения HPC для Azure и некоторые примеры на [рабочих нагрузках HPC](https://docs.microsoft.com/azure/virtual-machines/workloads/hpc/overview) 
