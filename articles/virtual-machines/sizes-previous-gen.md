...
<sup>1</sup> Максимально возможная пропускная способность дисков (в операциях ввода-вывода в секунду или Мбит/с) виртуальных машин серии Fs может быть ограничена из-за количества, размера и чередования подключенных дисков.  Дополнительные сведения см. в разделе Разработка для обеспечения высокой производительности [Windows](windows/premium-storage-performance.md) или [Linux](linux/premium-storage-performance.md).  
...
В этом разделе содержатся сведения о старых поколениях размеров виртуальных машин. Эти размеры по-прежнему поддерживаются, но не могут получить дополнительную емкость. Существуют более новые или альтернативные размеры, доступные в целом. Ознакомьтесь с [размером виртуальных машин Linux в Azure](linux/sizes.md) , чтобы выбрать размеры виртуальных машин, которые лучше подходят для ваших нужд.  
...
| Size | vCPU | Память, ГиБ | Временное хранилище (HDD): Гиб | Максимальное число дисков данных | Максимальная пропускная способность дисков данных, операций ввода-вывода в секунду | Максимальное число сетевых карт/ожидаемая пропускная способность сети (Мбит/с) |
| --- | --- | --- | --- | --- | --- | --- |
...
| Size | vCPU | Память, ГиБ | Временное хранилище (HDD): Гиб | Максимальное число дисков данных | Максимальная пропускная способность дисков данных, операций ввода-вывода в секунду | Максимальное число сетевых адаптеров|
|---|---|---|---|---|---|---|
...
**Рекомендуемый новый размер**: [серия Dv3](dv3-dsv3-series.md)
...
## <a name="preview-dc-series"></a>Предварительный просмотр: серия DC

**Рекомендуемый новый размер**: [серия DCsv2](dcv2-series.md)

Хранилище класса Premium: поддерживается

Кэширование хранилища класса Premium: поддерживается

В серии контроллеров домена используется Последнее поколение процессора Intel XEON E-2176G с частотой 3.7 ГГц с технологией SGX, а технология Intel Turbo Boost может работать до 4,7 ГГц. 
...
> [!IMPORTANT]
>
> Виртуальные машины серии DC являются [виртуальными машинами поколения 2](./linux/generation-2.md#creating-a-generation-2-vm) и поддерживают `Gen2` только образы.


### <a name="ds-series"></a>Серия DS  

**Рекомендуемый новый размер**: [серия Dsv3](dv3-dsv3-series.md)
...
| Size | vCPU | Память, ГиБ | Временное хранилище, Гиб | Максимальное число дисков данных | Максимальная пропускная способность для временного хранения (операций ввода-вывода в секунду) | Максимальная пропускная способность некэшированного диска (операций ввода-вывода в секунду) | Максимальное число сетевых карт/ожидаемая пропускная способность сети (Мбит/с) |
|---|---|---|---|---|---|---|---|
...
Максимально возможная пропускная способность дисков виртуальных машин серии Ls может быть ограничена из-за числа, размера и чередования любых подключенных дисков. Дополнительные сведения см. в разделе Разработка для обеспечения высокой производительности [Windows](windows/premium-storage-performance.md) или [Linux](linux/premium-storage-performance.md).
...
| Size | vCPU | Память, ГиБ | Временное хранилище (SSD): ГиБ | Максимальная пропускная способность хранилища: операций ввода-вывода/чтения Мбит/с | Максимальное число дисков данных/пропускная способность: операций ввода-вывода | Максимальное число сетевых карт/ожидаемая пропускная способность сети (Мбит/с) |
|---|---|---|---|---|---|---|
| Standard_G1  | 2  | 28  | 384  | 6000/93/46    | 8 или 8x500   | 2/2000  |
| Standard_G2  | 4  | 56  | 768  | 12000/187/93  | 16 или 16x500 | 2/4000  |
