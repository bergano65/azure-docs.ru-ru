...
Чтобы получить маркер SAS в шаблоне диспетчер ресурсов, используйте функцию **листаккаунтсас** . Пример шаблона см. в разделе [пример функции List](../../azure-resource-manager/resource-group-template-functions-resource.md#list-example).
...
Дополнительные сведения о создании токенов SAS для Центров событий см. на [этой веб-странице](../../event-hubs/event-hubs-authentication-and-security-model-overview.md).
...
условие | Выбирает определенный экземпляр объекта, к которому применяется метрика, или совокупность всех экземпляров этого объекта (необязательно). Дополнительные сведения см. в определениях метрики `builtin`.
...
единица измерения | Должна быть одна из следующих строк: "Count", "Bytes", "Seconds", "Percent", "CountPerSecond", "BytesPerSecond", "Millisecond". Определяет единицу измерения для метрики. Потребители собранных данных ожидают, что значения собранных данных выражены в этой единице. LAD игнорирует это поле.
...
Элемент | Значение
------- | -----
пространство_имен | Пространство имен OMI, в котором должен выполняться запрос (необязательно). Если этот элемент не задан, значение по умолчанию — "root/scx", реализуемое [кроссплатформенными поставщиками System Center](https://scx.codeplex.com/wikipage?title=xplatproviders&referringTitle=Documentation).
...
Частота | Число секунд между выполнениями запроса (необязательно). Значение по умолчанию — 300 (5 минут); минимальное значение — 15 секунд.
