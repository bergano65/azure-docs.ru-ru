...
* **Лазурный CLI**. [Установите среду Azure CLI](https://docs.microsoft.com/cli/azure/install-azure-cli) на компьютере.
...
Заполните правильные значения для переменных в первом разделе перед запуском:
...
Конфигурация образца, загруженная в этих примерах, собирает набор стандартных данных и отправляет их на хранилище таблиц. URL-адрес для конфигурации образца и его содержимого могут быть изменены. В большинстве случаев следует загрузить копию настроек портала JSON и настроить его под свои нужды, а затем иметь любые шаблоны или автоматизацию, которые вы строите, использовать свою собственную версию файла конфигурации, а не загружать этот URL каждый раз.
...
Чтобы получить токен SAS в шаблоне «Менеджер ресурсов», используйте функцию **listAccountSas.** Для примера шаблона см. [пример функции «Список».](../../azure-resource-manager/templates/template-functions-resource.md#list-example)
...
Для получения дополнительной информации о генерации и извлечении информации о токенах SAS для концентраторов событий смотрите [эту веб-страницу](https://docs.microsoft.com/rest/api/eventhub/generate-sas-token#powershell).
...
**ПРИМЕЧАНИЕ**: При использовании портала Azure вместо JSON, правильной формой поля состояния является Name'''''''''''mnt'
...
* [Исследователь хранения данных Microsoft Azure](https://azurestorageexplorer.codeplex.com/ "Обозреватель службы хранилища Azure").
