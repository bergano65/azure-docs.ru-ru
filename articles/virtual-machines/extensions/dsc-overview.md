...
Основным примером использования для расширения Azure Desired State Configuration (DSC) является загрузка VM в [службу state Configuration (DSC) Azure Automation State Configuration (DSC).](../../automation/automation-dsc-overview.md)
Служба предоставляет [преимущества,](/powershell/scripting/dsc/managing-nodes/metaConfig#pull-service) которые включают постоянное управление конфигурацией VM и интеграцию с другими операционными инструментами, такими как Мониторинг Azure.
Использование расширения для регистрации VM в службе обеспечивает гибкое решение, которое работает даже в подписках Azure.

Расширение DSC можно использовать независимо от службы Automation DSC.
Однако это только подтолкнет конфигурацию к VM.
Нет текущей отчетности, за исключением локально в VM.
...
- **Узел**. Целевой объект для конфигурации DSC. В этом документе *узел* всегда ссылается на Azure VM.
...
## <a name="information-for-registering-with-azure-automation-state-configuration-dsc-service"></a>Информация для регистрации в службе государственной конфигурации Azure Automation (DSC)

При использовании расширения DSC для регистрации узла в государственной службе конфигурации необходимо будет предоставить три значения.

- RegistrationUrl - https адрес учетной записи Автоматизации Azure
...
Для имени конфигурации узлов убедитесь, что конфигурация узлов существует в конфигурации состояния Azure.  В противном случае развертывание расширения вернет сбой.  Также убедитесь, что вы используете имя *конфигурации узлов,* а не конфигурацию.
Конфигурация определяется в скрипте, который используется для [компиляции конфигурации узлов (файл MOF).](https://docs.microsoft.com/azure/automation/automation-dsc-compile)
Имя всегда будет Конфигурацией, за `.` которой `localhost` следует период и либо или конкретное имя компьютера.
...
Ниже показан пример простой конфигурации. Сохранить конфигурацию локально как iisInstall.ps1.
...
Следующие команды размещают сценарий iisInstall.ps1 на указанном VM. а также выполняют конфигурацию и возвращают сведения о состоянии.
...
CLI Azure можно использовать для развертывания расширения DSC в существующую виртуальную машину.
...
- **WMF Version** (Версия WMF). Указывает версию Windows Management Framework (WMF), которую необходимо установить на виртуальной машине. Если задать для этого свойства значение latest, будет установлена последняя версия WMF. В настоящее время для этого свойства доступны только такие значения: 4.0, 5.0, 5.1 и latest. Возможные значения зависят от обновлений. Значение по умолчанию является **последним.**

- **Data Collection** (Сбор данных). Определяет, собирает ли расширение данные телеметрии. Дополнительные сведения см. в записи блога [Azure DSC Extension Data Collection](https://blogs.msdn.microsoft.com/powershell/2016/02/02/azure-dsc-extension-data-collection-2/) (Коллекция данных расширения DSC Azure).
