---
title: Расширения и функции Azure VM для Windows
...
- Настройте мониторинг VM с расширением Log Analytics Agent VM. Для получения дополнительной [информации см.](../../log-analytics/log-analytics-azure-vm-extension.md)
...
Для получения информации о поддерживаемых [Azure virtual machine agent](agent-windows.md)операционных системах и инструкциях по установке см.
...
Пакеты расширений можно скачать из репозитория расширений в службе хранилища Azure. В ней также публикуются новости об актуальном состоянии расширений. Если вы используете [поддерживаемую](https://support.microsoft.com/help/4049215/extensions-and-virtual-machine-agent-minimum-version-support) версию агентов, вам не нужно разрешать доступ к хранилищам Azure в регионе VM, так как может использовать агента для перенаправления связи на контроллер ткани Azure для агентской связи (функция HostGAPlugin через привилегированный канал на частном IP [168.63.129.16](https://docs.microsoft.com/azure/virtual-network/what-is-ip-address-168-63-129-16)). Если вы используете неподдерживаемую версию агента, необходимо разрешить исходящий доступ от виртуальной машины к службе хранилище Azure в том же регионе.

> [!IMPORTANT]
> Если вы заблокировали доступ к *168.63.129.16* с помощью гостевого брандмауэра или с помощью прокси-сервера, расширения не сбой, независимо от выше. Требуются порты 80, 443 и 32526.

Агенты можно использовать только для скачивания пакетов расширений и отправки отчетов о состоянии. Например, если для установки расширения требуется скачать сценарий из GitHub (пользовательский сценарий) или требуется доступ к службе хранилища Azure (Azure Backup), откройте дополнительные порты в брандмауэре или в группе безопасности сети (NSG). Разные расширения имеют разные требования, ведь они по сути являются самостоятельными приложениями. Для расширений, требующих доступа к хранилищам Azure или Active Directory Azure, можно разрешить доступ с помощью [тегов службы NG Azure](https://docs.microsoft.com/azure/virtual-network/security-overview#service-tags) для хранения или AzureActiveDirectory.

Агент Windows Guest агент не имеет прокси-сервера поддержки для вас, чтобы перенаправить запросы трафика агента через, что означает, что Windows Гостевой агент будет полагаться на пользовательский прокси (если у вас есть) для доступа к ресурсам в Интернете или на хосте через IP 168.63.129.16.
...
Для получения дополнительной информации смотрите [полный шаблон менеджера ресурсов](https://github.com/Microsoft/dotnet-core-sample-templates/tree/master/dotnet-core-music-windows).
...
На Azure IaaS VM, использующем расширения, в консоли сертификатов можно увидеть сертификаты, которые имеют генератор **_сертификатов Windows Azure CRP._** На классическом RDFE VM эти сертификаты имеют название объекта **_управления службой Windows Azure для расширений._**

Эти сертификаты обеспечивают связь между VM и его хостом во время передачи защищенных настроек (пароль, другие учетные данные), используемых расширениями. Сертификаты построены контроллером ткани Azure и переданы Агенту VM. Если вы останавливаетесь и начинаете VM каждый день, новый сертификат может быть создан контроллером ткани. Сертификат хранится в магазине личных сертификатов компьютера. Эти сертификаты могут быть удалены. Агент VM создает сертификаты, если это необходимо.
...
1. Чтобы проверить журнал агента для гостей Windows, посмотрите на действия, когда ваше расширение было подготовлено в *C: »WindowsAzure»Logs-WaAppAgent.log*
...
После того, как расширение VM было запущено против VM, используйте [Get-AzVM,](https://docs.microsoft.com/powershell/module/az.compute/get-azvm) чтобы вернуть статус расширения. *Substatuses[0]* указывает, что подготовка расширения прошла успешно, а значит оно правильно развернуто на виртуальной машине. Но *Substatuses[1]* указывает, что выполнение расширения на виртуальной машине завершилось сбоем.
...
| Расширение Диагностики Azure |Управляет системой диагностики Azure |[Расширение azure Диагностика](https://azure.microsoft.com/blog/windows-azure-virtual-machine-monitoring-with-wad-extension/) |
| Расширение Azure VM Access |Управляет пользователями и учетными данными. |[Расширение доступа VM для Linux](https://azure.microsoft.com/blog/using-vmaccess-extension-to-reset-login-credentials-for-linux-vm/) |
