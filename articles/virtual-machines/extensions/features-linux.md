...
- Настройка мониторинга виртуальной машины с помощью расширения ВМ Microsoft Monitoring Agent. Дополнительные сведения см. в статье [Мониторинг виртуальных машин Linux в Azure](../linux/tutorial-monitoring.md).
...
Чтобы расширения работали на виртуальной машине, на ней нужно установить агент Azure Linux. Некоторые расширения имеют дополнительные требования, например доступ к определенным ресурсам или зависимости.
...
3. Просмотрите журналы системы. Проверьте наличие других операций, которые могут мешать работе расширения, например длительные установки других приложений, требующие монопольного доступа к диспетчеру пакетов.
...
В некоторых случаях может потребоваться перезапуск расширения ВМ. Можно сначала удалить расширение, затем повторно запустить его любым удобным способом. Чтобы удалить расширение, выполните команду [az vm extension delete](/cli/azure/vm/extension#az-vm-extension-delete) следующим образом:
...
| Расширение службы "Диагностика Azure" |Управляет системой диагностики Azure |[Расширение системы диагностики Azure](https://azure.microsoft.com/blog/windows-azure-virtual-machine-monitoring-with-wad-extension/) |
