...
* verbose Повышение уровня детализации указанной команды
* перевести Пропустить интерактивное подтверждение для некоторых команд

### <a name="commands"></a>Команды
* Справка: Список поддерживаемых команд и флагов.
* deprovision Попытайтесь очистить систему и сделать ее пригодной для повторной инициализации. Следующая операция удаляет:
...
* отменить подготовку и пользователя: Выполняет все действия по отмене подготовки (выше), а также удаляет последнюю подготовленную учетную запись пользователя (полученную из/var/lib/waagent) и связанные с ней данные. Этот параметр используется для отмены подготовки ранее подготовленного образа в Azure с целью его записи и повторного использования.
* Версия: Отображает версию waagent
* сериалконсоле: Настраивает GRUB для пометки ttyS0 (первый последовательный порт) в качестве консоли загрузки. Это гарантирует, что журналы загрузки ядра отправляются на последовательный порт и становятся доступными для отладки.
* управляющей программы Запустите waagent в качестве управляющей программы для управления взаимодействием с платформой. Этот аргумент указывается для waagent в сценарии инициализации waagent.
* запустить Запуск waagent в качестве фонового процесса
...
* Если вам нужно настроить точку подключения диска ресурсов и пространство подкачки в образах облаков Ubuntu во время подготовки, см. следующие ресурсы.
  ...
  * [Вики-сайт Ubuntu: Настройка разделов подкачки](https://go.microsoft.com/fwlink/?LinkID=532955&clcid=0x409)
