...
> [!IMPORTANT]
> Не объявляйте одни и те же общедоступные IP-адреса маршрута в Интернете и через ExpressRoute. Мы настоятельно рекомендуем объявлять более конкретные маршруты через ExpressRoute и более общие маршруты в Интернете для [NAT](expressroute-nat.md). Кроме общедоступного маршрута для NAT, вы также можете объявить через ExpressRoute общедоступные IP-адреса, которые используются сетевыми серверами, взаимодействующими с конечными точками Office 365 в корпорации Майкрософт. 
> 
> 
...
***В глобальные службы Azure входит служба Azure DevOps**
