---
title: 'Azure ExpressRoute: Добавление шлюза в виртуальную сеть: портал'
description: В этой статье описывается добавление шлюза виртуальной сети в уже созданную виртуальную сеть диспетчер ресурсов для ExpressRoute с помощью портал Azure.
...
7. Выберите виртуальную сеть, в которую нужно добавить этот шлюз. Щелкните **Виртуальная сеть** , чтобы открыть колонку **Выбор виртуальной сети** . Выберите виртуальную сеть. Если вы не видите виртуальную сеть, убедитесь, что поле **Расположение** указывает на регион, в котором находится ваша виртуальная сеть.
