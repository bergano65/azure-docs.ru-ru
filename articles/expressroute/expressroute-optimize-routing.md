...
## <a name="path-selection-on-microsoft-and-public-peerings"></a>Выбор пути на Microsoft и публичных пирингах
Важно обеспечить, чтобы при использовании Microsoft или Public peering, что трафик течет по желаемому пути, если у вас есть один или несколько схем ExpressRoute, а также пути к Интернету через Интернет-биржи (IX) или Интернет-провайдера (ISP). BGP использует лучший алгоритм выбора путей, основанный на ряде факторов, включая длинный матч префикса (LPM). Чтобы гарантировать, что трафик, предназначенный для Azure через Microsoft или Public peering, пересекает путь ExpressRoute, клиенты должны реализовать атрибут *Local Preference,* чтобы гарантировать, что путь всегда предпочтительнее на ExpressRoute. 

> [!NOTE]
> Локальные предпочтения по умолчанию обычно 100. Более высокие местные предпочтения являются более предпочтительными. 
>
>
...
В приведенном выше примере, чтобы предпочесть траектории ExpressRoute настроить локальные предпочтения следующим образом. 
...
> [!IMPORTANT]
> Мы удаляем частные номера AS в AS PATH для префиксов, полученных на Microsoft Peering при использовании частного номера AS. Вам нужно заглянуть с общедоступной AS и прибсоздать общедоступные номера AS в AS PATH, чтобы повлиять на маршрутизирование для Microsoft Peering.
> 
> 
