---
title: Базовый уровень безопасности для Azure ExpressRoute
description: Базовый план безопасности Azure для ExpressRoute
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 06/22/2020
...
**Руководство**: неприменимо; ExpressRoute служит парой избыточных цепей для обеспечения высокой доступности. Подключения ExpressRoute не проходят через общедоступный Интернет.
...
**Руководство**: неприменимо; запись пакета поддерживается только для ресурсов вычислений IaaS (виртуальных машин Azure).
...
**Руководство**: неприменимо; Azure ExpressRoute — это не конечная точка, в которой можно фильтровать или разрешать трафик с помощью тегов служб или групп безопасности сети.
...
**Руководство**. Использование тегов для экземпляров Azure ExpressRoute для предоставления метаданных и логической организации.
...
**Руководство**: неприменимо; Корпорация Майкрософт поддерживает источник времени, используемый для ресурсов Azure, таких как Azure Sentinel, для меток времени в журналах.
...
**Руководство**: неприменимо; Azure ExpressRoute не обрабатывает и не создает журналы, связанные с защитой от вредоносных программ.
...
**Руководство**: неприменимо; Azure ExpressRoute не обрабатывает и не создает журналы, связанные с DNS.
...
**Руководство**: неприменимо; единый вход (SSO) обеспечивает безопасность и удобство при входе пользователей в пользовательские приложения в Azure Active Directory (AD). Доступ к плоскости управления Azure ExpressRoute (например, портал Azure) уже интегрирован с Azure Active Directory и доступен через портал Azure, а также Azure Resource Manager REST API.
...
**Руководство**. Включение обратимого удаления в Key Vault для защиты ключей от случайного или вредоносного удаления.
