---
title: 'Налаживать сосуществующие соединения ExpressRoute и S2S VPN: Azure PowerShell'
...
* **Поддерживается только VPN-шлюз на основе маршрутов.** Вы должны использовать [маршрутный VPN шлюз.](../vpn-gateway/vpn-gateway-about-vpngateways.md) Вы также можете использовать маршрутный VPN шлюз с VPN соединением, настроенным для «селекторов трафика на основе политики», как описано в [Connect к нескольким VPN-устройствам на основе политики.](../vpn-gateway/vpn-gateway-connect-multiple-policybased-rm-ps.md)
...
* **Подсеть шлюза должна быть /27 или более короткой префиксой**(например, /26, /25), или вы получите сообщение об ошибке при добавлении виртуального сетевого шлюза ExpressRoute.
...
3. Добавьте подсеть шлюза с префиксом /27 или больше.
   ...
   $vnet = Set-AzVirtualNetwork -VirtualNetwork $vnet
   ```
4. На этом этапе у вас есть виртуальная сеть без шлюзов. Для создания новых шлюзов и настройки соединений используйте следующие примеры:
...
Чтобы добавить конфигурацию "точка — сеть" к VPN-шлюзу при настройке параллельных подключений, выполните следующее. Чтобы загрузить корневой сертификат VPN, необходимо либо установить PowerShell локально на компьютер, либо использовать портал Azure.
...
1. Добавьте пул адресов VPN-клиента.
   ...
   ```
2. Отправьте корневой сертификат VPN в Azure для VPN-шлюза. В этом примере предполагается, что корневой сертификат хранится в локальной машине, где работают следующие cmdlets PowerShell и что вы работаете powerShell локально. Вы также можете загрузить сертификат с помощью портала Azure.
