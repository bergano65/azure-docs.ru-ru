...
3. Добавьте подсеть шлюза с префиксом /27 или больше.
   ...
   $vnet = Set-AzVirtualNetwork -VirtualNetwork $vnet
   ```
4. На этом этапе у вас есть виртуальная сеть без шлюзов. To create new gateways and set up the connections, use the following examples:
...
Чтобы добавить конфигурацию "точка — сеть" к VPN-шлюзу при настройке параллельных подключений, выполните следующее. To upload the VPN root certificate, you must either install PowerShell locally to your computer, or use the Azure portal.
...
1. Добавьте пул адресов VPN-клиента.
   ...
   ```
2. Отправьте корневой сертификат VPN в Azure для VPN-шлюза. In this example, it's assumed that the root certificate is stored in the local machine where the following PowerShell cmdlets are run and that you are running PowerShell locally. You can also upload the certificate using the Azure portal.
