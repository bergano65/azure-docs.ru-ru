...
> [!NOTE]
> [ExpressRoute ценовой категории "Премиум"](https://docs.microsoft.com/azure/expressroute/expressroute-faqs#expressroute-premium) **не** требуется для подключения Dynamics 365 через Azure ExpressRoute.
...
Несколько каналов ExpressRoute из разных расположений пиринга можно подключить к одной виртуальной сети, чтобы обеспечить высокий уровень доступности в случае, если один канал станет недоступным. Затем можно [назначить более высокие веса](https://docs.microsoft.com/azure/expressroute/expressroute-optimize-routing#solution-assign-a-high-weight-to-local-connection) локальному соединению, чтобы отдавалось предпочтение конкретному каналу. Настоятельно рекомендуется, чтобы клиенты настроили хотя бы два канала ExpressRoute, чтобы избежать единой точки отказа. 
...
Клиентам, использующим пиринг Майкрософт для доступа к общедоступным службам Azure, таким как служба хранилища Azure или SQL Azure, а также клиентам, использующим пиринг Майкрософт для Office 365, настоятельно рекомендуется реализовать несколько каналов в разных расположениях пиринга, чтобы избежать единой точки отказа. Клиенты могут объявлять один и тот же префикс на обеих каналах и добавлять в начало [AS PATH](https://docs.microsoft.com/azure/expressroute/expressroute-optimize-routing#solution-use-as-path-prepending) или объявлять разные префиксы для определения пути из локальной среды.
...
* Возможность подключения к Office 365
...
### <a name="can-my-existing-expressroute-circuits-support-connectivity-to-office-365-services"></a>Могут ли мои существующие каналы ExpressRoute поддерживать подключение к службам Office 365?
