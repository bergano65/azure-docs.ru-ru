...
### <a name="if-i-pay-for-an-expressroute-circuit-of-a-given-bandwidth-do-i-have-the-ability-to-use-more-than-my-procured-bandwidth"></a>Если я платите за канал ExpressRoute заданной пропускной способности, у меня есть возможность использовать больше приобретенной пропускной способности?

Да, вы можете использовать в двух случаях ограничение пропускной способности, которое вы выстроили, используя пропускную способность, доступную при дополнительном подключении канала ExpressRoute. Встроенная избыточность канала настраивается с помощью первичного и вторичного подключения, каждой из которых была приобретена пропускная способность, на два пограничных маршрутизатора Microsoft Enterprise (MSEE). Пропускная способность, доступная через вторичное подключение, может использоваться для дополнительного трафика при необходимости. Так как вторичное подключение предназначено для обеспечения избыточности, оно не гарантируется и не должно использоваться для дополнительного трафика в течение длительного периода времени. Дополнительные сведения об использовании обоих подключениях для передачи трафика см. в разделе [использование в качестве пути](./expressroute-optimize-routing.md#solution-use-as-path-prepending).

Если вы планируете использовать только основное подключение для передачи трафика, то пропускная способность для подключения будет исправлена и попытка превышения лимита подписки приведут к увеличению количества сбросов пакетов. Если трафик проходит через шлюз ExpressRoute, пропускная способность для SKU шлюза является фиксированной и не может быть обработана. Сведения о пропускной способности каждого SKU шлюза см. в статье [о шлюзах виртуальной сети ExpressRoute](expressroute-about-virtual-network-gateways.md#aggthroughput).
...
Если канал ExpressRoute включен для пиринга Майкрософт Azure, вы можете получить доступ к [диапазонам общедоступных IP-адресов](../virtual-network/public-ip-addresses.md#public-ip-addresses), используемых в Azure по каналу. Пиринг Майкрософт Azure предоставит доступ к службам, которые в настоящее время размещены в Azure (с географическими ограничениями в зависимости от номера SKU канала). Чтобы проверить доступность конкретной службы, ознакомьтесь с документацией по ней и определите, опубликован ли зарезервированный диапазон для этой службы. Затем найдите диапазоны IP-адресов целевой службы и сравните их с диапазонами, указанными в [XML-файле общедоступного облака с диапазонами IP-адресов Azure и тегами службы](https://www.microsoft.com/download/details.aspx?id=56519). Кроме того, можно открыть запрос в службу поддержки для уточнения.

**Поддерживаются:**

* [Microsoft 365](/microsoft-365/enterprise/azure-expressroute)
* Power BI — предоставляется через региональное сообщество Azure. Сведения о том, как узнать регион клиента Power BI, см. [здесь](/power-bi/service-admin-where-is-my-tenant-located).
...
> [!NOTE]
> [Expressroute Premium](#expressroute-premium) **не** требуется для подключения Dynamics 365 через Azure ExpressRoute, если канал ExpressRoute развернут в том же гео- [регионе](./expressroute-locations-providers.md#expressroute-locations).
...
Дополнительные сведения о выборе пути BGP и распространенных конфигурациях маршрутизатора см. [здесь](./expressroute-optimize-routing.md#path-selection-on-microsoft-and-public-peerings). 
...
См. рекомендации в разделе [Высокий уровень доступности и отработка отказов при использовании Azure ExpressRoute](/microsoft-365/enterprise/network-planning-with-expressroute).
