...
Да. Каналы ExpressRoute настроены так, что позволяют вдвое повысить приобретенную пропускную способность без дополнительной оплаты. Уточните у своего поставщика услуг, поддерживает ли он эту возможность. Это не относится к длительному периоду времени и не гарантируется.  Если трафик проходит через шлюз ExpressRoute, пропускная способность для номера SKU является фиксированной и не может быть обработана.
...
### <a name="how-are-vnets-advertised-on-expressroute-private-peering"></a>Как виртуальных сетей объявляется в частном пиринга ExpressRoute?

Шлюз ExpressRoute объявляет *адресные пространства* виртуальной сети Azure, вы не можете включить или исключить его на уровне подсети. Оно всегда является адресным пространством виртуальной сети, которое объявлено. Кроме того, если используется пиринг виртуальной сети, а в одноранговой виртуальной сети включен параметр "использовать удаленный шлюз", адресное пространство одноранговой виртуальной сети также будет объявлено.

### <a name="how-many-prefixes-can-be-advertised-from-a-vnet-to-on-premises-on-expressroute-private-peering"></a>Сколько префиксов можно объявить из виртуальной сети в локальную для частного пиринга ExpressRoute?

Существует не более 200 префиксов, объявленных для одного подключения ExpressRoute или через пиринг виртуальных сетей с помощью транзитного шлюза. Например, если имеется 199 адресных пространств в одной виртуальной сети, подключенной к каналу ExpressRoute, все 199 этих префиксов будут объявлены в локальной среде. Кроме того, если включена виртуальная сеть, разрешающая транзит шлюза с 1 адресным пространством и 150-лучевой виртуальных сетей с параметром "разрешить удаленный шлюз", то виртуальная сеть, развернутая с помощью шлюза, будет объявлять 151 в локальной среде.
...
Подключение между каналом ExpressRoute и шлюзом (и одноранговым виртуальных сетей с использованием транзитного шлюза, если это применимо) будет отключено. Он будет восстановлен при превышении предельного количества префиксов.  
...
Единственный способ фильтрации и включения маршрутов — на локальном пограничном маршрутизаторе. Определяемые пользователем маршруты можно добавить в виртуальную сеть, чтобы повлиять на определенную маршрутизацию, но это будет статическим, а не частью объявления BGP.
...
ExpressRoute поддерживает [три домена маршрутизации](expressroute-circuit-peerings.md) для различных типов служб: частный пиринг, пиринг Майкрософт и общедоступный пиринг (не рекомендуется).
...
Если канал ExpressRoute включен для пиринга Azure Майкрософт, вы можете получить доступ к [диапазонам общедоступных IP-адресов](../virtual-network/virtual-network-ip-addresses-overview-arm.md#public-ip-addresses) , используемым в Azure по каналу. Пиринг Майкрософт Azure предоставит доступ к службам, которые в настоящее время размещаются в Azure (с географическими ограничениями в зависимости от номера SKU канала). Чтобы проверить доступность конкретной службы, можно проверить документацию по этой службе, чтобы узнать, опубликован ли зарезервированный диапазон для этой службы. Затем найдите диапазоны IP-адресов целевой службы и сравните их с диапазонами, перечисленными в поле [диапазоны IP-адресов Azure и теги службы — общедоступный облачный XML-файл](https://www.microsoft.com/download/details.aspx?id=56519). Кроме того, можно отправить запрос в службу поддержки для уточнения.

**Поддерживается**

* [Office 365.](https://aka.ms/ExpressRouteOffice365)
* Power BI-доступно через региональное сообщество Azure [, Узнайте, как узнать регион](https://docs.microsoft.com/power-bi/service-admin-where-is-my-tenant-located) вашего клиента Power BI.
...
Общедоступный пиринг был отключен в новых каналах ExpressRoute. Теперь службы Azure доступны на пиринга Майкрософт. Если канал, созданный до использования общедоступного пиринга, не является устаревшим, можно выбрать использование пиринга Майкрософт или общедоступного пиринга в зависимости от нужных служб.
...
### <a name="why-i-see-advertised-public-prefixes-status-as-validation-needed-while-configuring-microsoft-peering"></a>Почему при настройке пиринга Майкрософт отображаются сведения о состоянии "объявленные открытые префиксы" (требуется проверка)?
...
> [!NOTE]
> Служба [Expressroute Premium](https://docs.microsoft.com/azure/expressroute/expressroute-faqs#expressroute-premium) **не** требуется для подключения Dynamics 365 через Azure ExpressRoute.
...
Если вы объявляете маршруты по умолчанию, мы будем принудительно отправлять трафик к службам, предлагаемым через пиринг Майкрософт (таким как служба хранилища Azure и база данных SQL) обратно в вашу локальную среду. Вы должны будете настроить свои маршруты для возврата трафика в Azure по пути пиринга Майкрософт или через Интернет. Если вы включили функцию конечной точки службы, трафик, предназначенный для службы, не будет принудительно направляться в локальное расположение. Трафик останется в магистральной сети Azure. Дополнительные сведения о конечных точках службы см. в статье [конечные точки службы виртуальной сети](../virtual-network/virtual-network-service-endpoints-overview.md?toc=%2fazure%2fexpressroute%2ftoc.json) .
...
## <a name="expressroute-direct"></a><a name="expressRouteDirect"></a>Непосредственный ExpressRoute
