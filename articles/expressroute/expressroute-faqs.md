...
№ Вы можете приобрести у своего поставщика услуг VPN-подключение любой скорости. Однако ваше подключение к Azure ограничено пропускной способностью приобретенного вами канала ExpressRoute.
...
Да. Каналы ExpressRoute настроены так, что позволяют вдвое повысить приобретенную пропускную способность без дополнительной оплаты. Уточните у своего поставщика услуг, поддерживает ли он эту возможность. Это не в течение длительного периода времени и не гарантируется. 
...
* Dynamics 365 
* Power BI — доступны через Azure региональных сообщества, см. в разделе [здесь](https://docs.microsoft.com/power-bi/service-admin-where-is-my-tenant-located) для сведения об определении области вашего клиента Power BI. 
...
### <a name="how-do-i-implement-redundancy-on-private-peering"></a>Как реализовать избыточность на частный пиринг?

Несколько каналов ExpressRoute из разных расположениях пиринга можно подключить к одной и той же виртуальной сети для обеспечения высокого уровня доступности в случае, что один канал становится недоступным. После этого можно [назначить высокие весовые коэффициенты](https://docs.microsoft.com/azure/expressroute/expressroute-optimize-routing#solution-assign-a-high-weight-to-local-connection) для локального подключения предпочитать предпочитать определенном канале. Настоятельно рекомендуется настроить что клиентов по крайней мере два канала ExpressRoute, чтобы избежать единых точек отказа. 

См. в разделе [здесь](https://docs.microsoft.com/azure/expressroute/designing-for-high-availability-with-expressroute) при разработке приложений для обеспечения высокой доступности и [здесь](https://docs.microsoft.com/azure/expressroute/designing-for-disaster-recovery-with-expressroute-privatepeering) при разработке приложений для аварийного восстановления.  

### <a name="how-i-do-implement-redundancy-on-microsoft-peering"></a>Как реализовать избыточность на пиринг Майкрософт?

Настоятельно рекомендуется при использовании пиринга для доступа к службам Azure public, как службы хранилища Azure или Azure SQL, а также клиентам, использующим пиринг Майкрософт для Office 365, что они реализуют несколько каналов в другой пиринг Майкрософт Чтобы избежать единых точек faiure расположения. Клиенты могут объявить префикс для обоих каналов и использовать [атрибут AS PATH](https://docs.microsoft.com/azure/expressroute/expressroute-optimize-routing#solution-use-as-path-prepending) или объявлять разные префиксы, чтобы определить путь из локальной.

См. в разделе [здесь](https://docs.microsoft.com/azure/expressroute/designing-for-high-availability-with-expressroute) при разработке приложений для обеспечения высокой доступности.
...
Да. Можно связать одну виртуальную сеть с до четырех каналов ExpressRoute в любом одной или разных расположений пиринга. 
...
Мы не принимаем частные префиксы (RFC1918) для сеанса пиринга BGP Майкрософт. Мы принимаем любого префикс размера (до /32) как для корпорации Майкрософт, так и для частного пиринга.
...
## <a name="expressroute-local"></a>Локальный ExpressRoute
### <a name="what-is-expressroute-local"></a>Что такое локальный ExpressRoute?
Используется значение ExpressRoute Local на SKU для ExpressRoute канала [ExpressRoute Direct](expressroute-erdirect-about.md). Основная функция локальных это, что локальный канал в канал ExpressRoute, расположение пиринга позволяет получить доступ к только для одного или двух регионов Azure в или рядом в одном регионе. Напротив стандартный канал предоставляет доступ к всех регионах геополитической области и канала уровня "премиум" во всех регионах Azure глобально. 

### <a name="what-are-the-benefits-of-expressroute-local"></a>Каковы преимущества локальных ExpressRoute?
Вам нужно оплатить исходящий трафик передачи данных для канала "стандартный" или ExpressRoute "премиум", вы не платите исходящий трафик передачи данных отдельно для канала ExpressRoute локальной. Другими словами цена локальных ExpressRoute включает цены на передачу данных. Локальный ExpressRoute — это наиболее экономичным решением решение, если имеется значительный объем передаваемых данных и может передавать ваши данные через частное подключение к расположению пиринга ExpressRoute практически регионов Azure. 

### <a name="what-features-are-available-and-what-are-not-on-expressroute-local"></a>Какие функции доступны, и каковы не при локальном ExpressRoute?
По сравнению с к каналу Standard ExpressRoute, локального канала имеет тот же набор функции, за исключением:
* Область доступа к регионам Azure, описанных выше
* ExpressRoute глобальным доступом не доступен на локальном узле

Локальный ExpressRoute также имеет те же ограничения на ресурсы (например количество виртуальных сетей на канал) как стандарт. 

### <a name="how-to-configure-expressroute-local"></a>Как настроить локальный ExpressRoute? 
Локальный ExpressRoute доступна только на ExpressRoute Direct. Поэтому сначала необходимо настроить порт ExpressRoute Direct. После создания прямого порта можно создать локального канала в соответствии с инструкциями [здесь](expressroute-howto-erdirect.md).

### <a name="where-is-expressroute-local-available-and-which-azure-regions-is-each-peering-location-mapped-to"></a>Где доступна служба ExpressRoute локальных и в каких регионах Azure каждого расположение пиринга сопоставляется?
Локальный ExpressRoute доступна в расположениях пиринга, наличие одного или двух регионов Azure путем закрытия. Она недоступна в расположении пиринга при наличии не регионом Azure, штата или провинции или своей страны. См. на точное сопоставление [странице расположения](expressroute-locations-providers.md).  
