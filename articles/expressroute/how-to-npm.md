...
3. На странице **Рабочая область Log Analytics** выберите **Создать** и настройте следующие параметры:
...
8. После завершения установки на панели управления появится Microsoft Monitoring Agent. Здесь можно просмотреть конфигурацию и проверить, подключен ли агент к Azure Log Analytics. При подключении агент отображает следующее сообщение: **The Microsoft Monitoring Agent has successfully connected to the Microsoft Operations Management Suite service** (Агент Microsoft Monitoring Agent успешно подключен к службе Microsoft Operations Management Suite).
