---
title: Настройка решения "Монитор производительности сети"для каналов ExpressRoute в Azure | Документация Майкрософт
...
## <a name="configure"></a>Шаг 1. создать рабочую область;
...
3. На **рабочей области Log Analytics** выберите **Create New**, затем настройте следующие параметры:
...
### <a name="download"></a>2.1. Загрузка файла установки агента
...
5. На **параметры установки агента** страницы, вы можете подключить агент к Azure Monitor журналы или Operations Manager. Или можете не выбирать эти варианты, если хотите настроить агент позже. Выбрав нужный вариант, нажмите кнопку **Далее**.
...
8. После завершения установки на панели управления появится Microsoft Monitoring Agent. Можно просмотреть конфигурацию и убедитесь, что агент подключен к журналам Azure Monitor. При подключении агент отображает следующее сообщение: **The Microsoft Monitoring Agent has successfully connected to the Microsoft Operations Management Suite service** (Агент Microsoft Monitoring Agent успешно подключен к службе Microsoft Operations Management Suite).
...
4. В **состояние** столбца, вы должны увидеть что агент успешно подключен к журналам Azure Monitor.
