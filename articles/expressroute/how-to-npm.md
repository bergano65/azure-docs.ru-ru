---
title: 'Azure ExpressRoute: Настройка NPM для цепей'
...
3. На странице **рабочего пространства Log Analytics** выберите **Создать новый**, а затем настроить следующие настройки:
...
3. В разделе **Configure Log Analytics Agents for monitoring using TCP protocol** (Настройка агентов Log Analytics для мониторинга по протоколу TCP) загрузите скрипт PowerShell. С его помощью вы сможете открыть соответствующий порт брандмауэра для транзакций TCP.
   ...
   ![Скрипт PowerShell](./media/how-to-npm/7.png)
...
3. На странице **Лицензионные условия,** читать лицензии, а затем нажмите **Я согласен**.
...
5. На странице **параметры настройки агента** можно подключить агента к журналам Azure Monitor или менеджеру операций. Или можете не выбирать эти варианты, если хотите настроить агент позже. Выбрав нужный вариант, нажмите кнопку **Далее**.

   * Если выбрано подключение к **Azure Log Analytics**, вставьте **идентификатор рабочей области** и **ключ рабочей области** (первичный ключ), скопированные в Блокнот на предыдущих шагах. Затем нажмите кнопку **Далее**.

     ![Идентификатор и ключ](./media/how-to-npm/8.png)
   * Если выбрано подключение к **Operations Manager**, на странице **Management Group Configuration** (Конфигурация группы управления) введите **имя группы управления**, **сервер управления** и **порт сервера управления**. Затем нажмите кнопку **Далее**.

     ![Operations Manager](./media/how-to-npm/9.png)
   * На странице **Учетная запись действия агента** выберите **локальную системную** учетную запись или **учетную запись доменного или локального компьютера**. Затем нажмите кнопку **Далее**.

     ![Учетная запись](./media/how-to-npm/10.png)
6. На странице **Ready to Install** просмотрите свой выбор, а затем нажмите **«Установка».**
7. На странице **Настройка успешно завершена** нажмите кнопку **Готово**.
8. После завершения установки на панели управления появится Microsoft Monitoring Agent. Там можно просмотреть конфигурацию и убедиться, что агент подключен к журналам Azure Monitor. При подключении агент выдает следующее сообщение: **The Microsoft Monitoring Agent has successfully connected to the Microsoft Operations Management Suite service** (Microsoft Monitoring Agent успешно подключен к службе Microsoft Operations Management Suite).
...
4. В столбце **«Статус»** следует увидеть, что агент успешно подключен к журналам Azure Monitor.
...
#### <a name="trend-of-loss-latency-and-throughput"></a><a name="trend"></a>Тенденция потери, задержки и пропускной связи

Диаграммы потерь, задержки и пропускной способности интерактивны. С помощью мыши вы можете увеличить любой фрагмент этих диаграмм. Вы также можете увидеть данные о пропускной способности, задержке и потерях для других интервалов, нажав **На кнопку Дата/Время,** расположенную ниже кнопки «Действия» в левом верхнем углу.
...
### <a name="nodes-view"></a><a name="nodes"></a>Вид узлов
