...
* Отфильтровывать нежелательные префиксы путем применения фильтров маршрутов к сообществам BGP. Это стандартная сетевая практика, которая часто используется во многих сетях.
...
## <a name="prerequisites"></a><a name="workflow"></a>Предварительные требования
...
* Необходимо иметь активный канал ExpressRoute с подготовленным пирингом Майкрософт. Для выполнения этих задач можно использовать следующие инструкции:
  * [Создайте канал ExpressRoute](howto-circuit-cli.md) и включите его на стороне поставщика услуг подключения, прежде чем продолжить. Канал ExpressRoute должен находиться в подготовленном и включенном состоянии.
...
Если фильтр маршрутов уже подключен к каналу, изменения объявлений префикса автоматически распространяются в списке сообщества BGP в ходе установленных сеансов BGP. Вы можете обновить список сообщества BGP вашего фильтра маршрута с помощью следующей команды:
...
## <a name="clean-up-resources"></a><a name="delete"></a>Очистка ресурсов
