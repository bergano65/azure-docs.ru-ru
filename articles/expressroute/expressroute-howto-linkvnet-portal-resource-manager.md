---
title: 'ExpressRoute: Свяжите VNet с схемой: портал Azure'
description: Подключите VNet к схеме Azure ExpressRoute. Пошаговое руководство.
...
* Вам потребуется активный канал ExpressRoute.
  ...
  * Убедитесь, что для вашего канала настроен частный пиринг Azure. Просмотрите [инструкцию Create и модифицировать визирование для](expressroute-howto-routing-portal-resource-manager.md) статьи схемы ExpressRoute для инструкций по пирингу и маршрутированию.
...
1. Убедитесь, что канал ExpressRoute и частный пиринг Azure настроены. Следуйте инструкциям в [Создание схемы ExpressRoute](expressroute-howto-circuit-arm.md) и [создать и изменить вглядываясь для схемы ExpressRoute](expressroute-howto-routing-arm.md). Ваш канал ExpressRoute должен выглядеть, как показано на рисунке ниже.

   [![Снимок экрана канала ExpressRoute](./media/expressroute-howto-linkvnet-portal-resource-manager/routing1.png "Просмотр схемы")](./media/expressroute-howto-linkvnet-portal-resource-manager/routing1-exp.png#lightbox)
2. Теперь можно приступить к подготовке подключения для связи шлюза виртуальной сети с каналом ExpressRoute. Нажмите **Соединение** > **Добавить,** чтобы открыть страницу **подключения Добавить,** а затем настроить значения.

   [![Добавить скриншот соединения](./media/expressroute-howto-linkvnet-portal-resource-manager/samesub1.png "Добавить подключение Скриншот")](./media/expressroute-howto-linkvnet-portal-resource-manager/samesub1-exp.png#lightbox)
...
6. Введите значения **Ключ авторизации** и **Peer circuit URI** (URI однорангового канала) и присвойте подключению имя. Нажмите кнопку **ОК**. **Peer Circuit URI** — это идентификатор ресурсов схемы ExpressRoute (который вы можете найти под панелью настройки свойств цепи ExpressRoute).
