---
title: 'Частный пиринг Azure ExpressRoute: Настройка транспортного режима IPsec - Хосты Windows'
...
## <a name="1-create-a-gpo"></a><a name="creategpo"></a>1. Создание GPO
...
## <a name="2-enable-the-gpo-link"></a><a name="enablelink"></a>2. Включить ссылку На ГПО
...
## <a name="3-define-the-ip-filter-action"></a><a name="filteraction"></a>3. Определить действие IP-фильтра
...
3. В **мастере настройки действий фильтра IP-безопасности** нажмите кнопку **Далее**.
   ...
   [![17]][17]
4. Назовите действие фильтра интуитивно понятным именем, чтобы его можно было найти позже. В этом примере действие называется **myEncryption**. Вы также можете добавить описание. Затем нажмите кнопку **Далее**.
...
## <a name="4-define-an-ip-filter-list"></a><a name="filterlist1"></a>4. Определить список IP-фильтров
...
2. В поле **Имя** введите имя своего списка IP-фильтров. Например, **azure-onpremises-HTTP8080**. Нажмите кнопку **Добавить**.
   ...
   [![25]][25]
3. На странице **Описание IP-фильтра и свойство "Отраженный"** выберите **Отраженный**. Параметр "Отраженный" соответствует пакетам, идущим в обоих направлениях, что позволяет осуществлять двустороннюю связь. Нажмите **Далее**.
...
5. Укажите адрес источника IP-трафика (**IP-адрес или подсеть:**), а затем нажмите кнопку **Далее**.
   ...
   [![28]][28]
6. Укажите **адрес назначения** для IP-адреса или подсети. Затем нажмите кнопку **Далее**.

   [![29]][29]
7. На странице**Тип протокола IP** выберите **TCP**. Затем нажмите кнопку **Далее**.

   [![30]][30]
8. На странице **Порт протокола IP** выберите **Пакеты из любого порта** и **Пакеты на этот порт:**. Введите **8080** в текстовое поле. Эти параметры определяют только HTTP-трафик, который будет зашифрован, на порт назначения 8080. Затем нажмите кнопку **Далее**.
...
## <a name="5-edit-the-ip-filter-list"></a><a name="filterlist2"></a>5. Изыскить список IP-фильтров
...
1. Выберите **политики IPSecurity в Active Directory**, связанные с подразделением. Щелкните правой кнопкой мыши и выберите **Create IP Security Policy** (Создать политику IP-безопасности).
   ...
   [![37]][37]
2. Назовите политику безопасности. Например, **policy-azure-onpremises**. Затем нажмите кнопку **Далее**.
...
## <a name="7-edit-the-ipsec-security-policy"></a><a name="editipsec"></a>7. Отсваивай политику безопасности IPsec
...
## <a name="8-assign-the-ipsec-gpo-to-the-ou"></a><a name="assigngpo"></a>8. Назначить IPsec GPO для OU
