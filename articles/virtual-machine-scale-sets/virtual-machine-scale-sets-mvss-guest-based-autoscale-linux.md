---
title: Используйте автомасштаб Azure с показателями гостей в шаблоне набора шкалы Linux
...
В Azure есть два широких типа метрик, которые собраны из наборов vMs и шкалы: метрики хоста и метрики гостей. На высоком уровне, если вы хотите использовать стандартные показатели процессора, диска и сети, то метрики хоста хорошо подходят. Если, однако, вам нужен больший выбор метрик, то показатели гостя должны быть рассмотрены.

Метрики хоста не требуют дополнительной настройки, поскольку они собираются хост-VM, в то время как гостевые метрики требуют установки [расширения Диагностики Windows Azure](../virtual-machines/windows/extensions-diagnostics-template.md) или [расширения Linux Azure Diagnostics](../virtual-machines/linux/diagnostic-extension.md) в гостевом VM. Одна из основных причин использования метрик гостевой виртуальной машины вместо метрик узла заключается в том, что метрики гостевой виртуальной машины предоставляют больший набор показателей, чем метрики узла. Один из таких примеров — это метрики потребления памяти, которые доступны только через метрики гостевой виртуальной машины. Поддерживаемые метрики узла перечислены [здесь](../azure-monitor/platform/metrics-supported.md), а часто используемые метрики гостевой виртуальной машины — [здесь](../azure-monitor/platform/autoscale-common-metrics.md). В этой статье показано, как изменить [базовый жизнеспособный шаблон набора масштабов,](virtual-machine-scale-sets-mvss-start.md) чтобы использовать правила автомасштабирования, основанные на показателях гостевых для наборов масштабов Linux.

## <a name="change-the-template-definition"></a>Изменение определения шаблона

В [предыдущей статье](virtual-machine-scale-sets-mvss-start.md) мы создали базовый шаблон набора масштабов. Теперь мы будем использовать этот более ранний шаблон и изменить его для создания шаблона, который развертывает набор масштаба Linux с автомасштабом на основе гостевых метрик.
