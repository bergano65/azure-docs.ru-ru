---
title: На портале Azure наборы виртуальной автоматической виртуальной машины
...
5. Давайте создадим правило, в соответствии с которым количество экземпляров виртуальных машин в масштабируемом наборе увеличивается, когда средняя загрузка ЦП превышает 70 % в течение 10 минут. При активации правила количество экземпляров виртуальной машины увеличивается на 20 %. В масштабируемых наборах с небольшим числом экземпляров виртуальных машин можно задать для параметра **Операция** значение *Увеличить счетчик на* и указать значение *1* или *2* для параметра *Число экземпляров*. В этих же наборах с большим числом экземпляров виртуальных машин более оптимальным является увеличение на 10 или 20 % экземпляров.
    ...
    |------------------------|---------------------------------------------------------------------------------------------------------------------|----------------|
    | *Агрегация времени*     | Определяет способ вычисления собранных метрик для анализа.                                                | Среднее        |
    | *Метрическое имя*          | Метрика производительности для мониторинга и применения действий в масштабируемом наборе.                                                   | Percentage CPU |
    ...
    | *Порог*            | Процентное значение, при достижении которого правило автомасштабирования запускает действие.                                                 | 70             |
    | *Длительность*             | Количество времени, в течение которого выполняется отслеживание перед сравнением значений метрик и пороговых значений.                                   | 10 минут.     |
    | *Операции*            | Определяет действие, применяемое к масштабируемому набору при выполнении условий правила (увеличение или уменьшение числа экземпляров), а также шаг приращения.                        | Увеличить процент на |
    | *Количество экземпляров*       | При активации правила должен измениться процент экземпляров виртуальных машин.                                            | 20             |
    | *Охладите вниз (минуты)*  | Время ожидания перед повторным применением правила, чтобы обеспечить время для активации действий автомасштабирования. | 5 мин      |
...
2. Создайте правило, в соответствии с которым количество экземпляров виртуальной машины в масштабируемом наборе уменьшается, когда средняя загрузка ЦП не превышает 30 % в течение 10 минут. При активации правила количество экземпляров виртуальной машины уменьшается на 20 %.
    ...
    | *Порог*            | Процентное значение, при достижении которого правило автомасштабирования запускает действие.                                                 | 30             |
    | *Операции*            | Определяет действие, применяемое к масштабируемому набору при выполнении условий правила (увеличение или уменьшение числа экземпляров), а также шаг приращения.                         | Уменьшить процент на |
    | *Количество экземпляров*       | При активации правила должен измениться процент экземпляров виртуальных машин.                                             | 20             |
...
1. Установите следующие ограничения экземпляров.
    ...
    | Минимальные | Максимальная | Значение по умолчанию|
    |---------|---------|--------|
    | 2       | 10      | 2      |
...
Из этой статьи вы узнали, как использовать правила автомасштабирования для горизонтального масштабирования, а также как увеличивать или уменьшать *число* экземпляров виртуальных машин в масштабируемом наборе. Чтобы увеличить или уменьшить *размер* экземпляров виртуальных машин, вы также можете выполнить вертикальное масштабирование. Для получения дополнительной информации [см.](virtual-machine-scale-sets-vertical-scale-reprovision.md)
