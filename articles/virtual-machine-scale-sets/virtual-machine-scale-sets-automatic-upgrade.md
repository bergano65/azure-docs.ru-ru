...
- Обновляет пакеты экземпляров таким образом, чтобы каждый раз при публикации нового образа издателем.
...
- Работает для всех размеров виртуальных машин, а для образов Windows и Linux.
...
2. Обновление Orchestrator определяет пакет виртуальных машин для обновления, при этом для одного пакета с максимальным числом экземпляров составляет 20% от общего числа экземпляров, подчиняются минимальному размеру пакета на одну виртуальную машину.
...
> [!NOTE]
>Автоматическое обновление ОС не обновляет SKU эталонного образа в масштабируемом наборе. Чтобы изменить номер SKU (например, Ubuntu 16,04-LTS 18,04-LTS), необходимо обновить [модель масштабируемого набора](virtual-machine-scale-sets-upgrade-scale-set.md#the-scale-set-model) непосредственно с помощью номера SKU нужного образа. Издатель и предложение образа нельзя изменить для существующего масштабируемого набора.  

## <a name="supported-os-images"></a>Поддерживаемые образы ОС
В настоящее время поддерживаются только определенные образы платформ ОС. Пользовательские образы [поддерживаются](virtual-machine-scale-sets-automatic-upgrade.md#automatic-os-image-upgrade-for-custom-images) , если масштабируемый набор использует пользовательские образы с помощью [общей коллекции образов](../virtual-machines/shared-image-galleries.md).
...
| MicrosoftWindowsServer  | WindowsServer | 2019 — центр обработки данных — Смаллдиск |
| MicrosoftWindowsServer  | WindowsServer | 2019-Datacenter-with-Containers |
| MicrosoftWindowsServer  | WindowsServer | Datacenter-Core-1903-with-Containers-смаллдиск |
...
- Для масштабируемых наборов, использующих виртуальные машины Windows, начиная с API вычислений версии 2019-03-01, свойство *virtualMachineProfile. osProfile. виндовсконфигуратион. енаблеаутоматикупдатес* должно иметь значение *false* в определении модели масштабируемого набора. Приведенное выше свойство позволяет выполнять обновления в виртуальной машине, когда "Центр обновления Windows" применяет исправления операционной системы без замены диска ОС. Если в масштабируемом наборе включено автоматическое обновление образа ОС, дополнительное обновление с помощью "Центр обновления Windows" не требуется.
...
Автоматическое обновление образа ОС поддерживается для пользовательских образов, развернутых с помощью [общей коллекции образов](../virtual-machines/shared-image-galleries.md). Другие пользовательские образы не поддерживаются при автоматическом обновлении образа ОС.
...
- Экземпляры масштабируемых наборов, настроенных для автоматического обновления образа ОС, будут обновлены до последней версии образа коллекции общих образов, когда новая версия образа публикуется и [реплицируется](../virtual-machines/shared-image-galleries.md#replication) в регион этого масштабируемого набора. Если новый образ не реплицируется в регион, где развернут масштаб, то экземпляры масштабируемых наборов не будут обновлены до последней версии. С помощью многоязыковой репликации образов можно управлять развертыванием нового образа для масштабируемых наборов.
