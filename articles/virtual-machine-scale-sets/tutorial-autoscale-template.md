---
title: Руководство. Автомасштабирование масштабируемого набора с помощью шаблонов Azure
...
## <a name="define-a-rule-to-autoscale-out"></a>Определение правила для автоматического горизонтального увеличения масштаба
...
| *direction*       | Определяет действие, применяемое к масштабируемому набору при выполнении условий правила: горизонтальное увеличение или уменьшение масштаба для числа экземпляров.                                              | Увеличить        |
...
| *value*           | Указывает, сколько экземпляров виртуальных машин следует добавить или удалить, когда применяется правило.                                             | 3               |
...
## <a name="define-a-rule-to-autoscale-in"></a>Определение правила для автоматического горизонтального уменьшения масштаба
...
В следующем примере определяется правило, в соответствии с которым осуществляется горизонтальное уменьшение масштаба и количество экземпляров виртуальной машины уменьшается на один, когда средняя загрузка ЦП не превышает 30 % в течение 5 минут. Это правило будет добавлено в профиль автомасштабирования после предыдущего.
...
Чтобы отслеживать количество экземпляров виртуальных машин в масштабируемом наборе, используйте **watch**. Потребуется 5 минут, чтобы в соответствии с правилами автомасштабирования началось горизонтальное увеличение масштаба с учетом нагрузки на ЦП, созданной программой **stress** на каждом экземпляре виртуальной машины.
...
Дополнительные примеры использования масштабируемых наборов виртуальных машин см. в следующих примерах сценариев для Azure CLI:

> [!div class="nextstepaction"]
> [Примеры сценариев Azure CLI для масштабируемых наборов](cli-samples.md)
