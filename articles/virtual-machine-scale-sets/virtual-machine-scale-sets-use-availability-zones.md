---
title: Создание набора масштабов Azure, использующем зоны доступности
...
Для использования макс распространения, установить *platformFaultDomainCount* до *1*. Чтобы использовать статическое распространение между 5 доменами сбоя, установите для параметра *platformFaultDomainCount* значение *5*. В версии API *2017-12-01,* *platformFaultDomainCount* по умолчанию до *1* для однозональных и кросс-зональных наборов масштаба. В настоящее время поддерживается только статический 5 доменных разложек, поддерживаемых для региональных (незональных) наборов масштабов.

### <a name="placement-groups"></a>Группы размещения

При развертывании масштабируемого набора можно выбрать развертывание с одной или несколькими [группами размещения](./virtual-machine-scale-sets-placement-groups.md) на зону доступности. Для региональных (незональных) наборов масштабов выбор состоит в том, чтобы иметь единую группу размещения в регионе или иметь несколько в регионе. Для большинства рабочих нагрузок рекомендуется несколько групп размещения, что обеспечивает лучшее масштабирование. В версии API *2017-12-01*шкала устанавливает значение по умолчанию для нескольких групп размещения для однозональных и кросс-зональных маштабных наборов, но они по умолчанию по умолчанию для одной группы размещения для региональных (незональных) наборов масштаба.
...
При выборе наилучшей балансировки зон масштабируемый набор пытается выполнять свертывание и развертывание виртуальных машин, поддерживая баланс. Однако, если по какой-то причине это невозможно (например, если одна зона выходит из нее, набор масштабов не может создать новый VM в этой зоне), набор масштабов позволяет временному дисбалансу успешно масштабироваться в или из нее. При последующих попытках масштабирования набор масштабов добавляет ВМ в зоны, которым необходимо больше vMs для сбалансированности шкалы. Аналогичным образом при последующих попытках масштабирования масштабируемый набор удаляет виртуальные машины из зон, где их должно быть меньше. При использовании строгой балансировки зон масштабирование будет завершено ошибкой, если оно приведет к несбалансированному состоянию.

Чтобы использовать вариант наилучшей балансировки зон, установите для параметра *zoneBalance* значение *false*. Это значение установлено по умолчанию в версии API *2017-12-01*. Чтобы использовать строгий баланс зоны, установите *зонуБаланс* на *истину.*
...
Чтобы использовать зоны доступности, масштабируемый набор должен быть создан в [поддерживаемом регионе Azure](../availability-zones/az-overview.md#services-support-by-region). Вы можете создать масштабируемый набор, использующий зоны доступности, с помощью одного из следующих методов:

- [Портал Azure](#use-the-azure-portal)
- Azure CLI
- [Лазурная силаШелл](#use-azure-powershell)
...
При создании общедоступного IP-адреса или подсистемы балансировки нагрузки укажите свойство *"sku": { "name": "Standard" }"* для создания сетевых ресурсов, избыточных между зонами. Необходимо также создать группы безопасности сети и правила, чтобы разрешить любой трафик. Дополнительные сведения см. в разделе [Обзор Azure Load Balancer уровня "Стандартный"](../load-balancer/load-balancer-standard-overview.md) и [Azure Load Balancer уровня "Стандартный" и зоны доступности](../load-balancer/load-balancer-standard-availability-zones.md).
