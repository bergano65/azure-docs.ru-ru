...
Чтобы использовать максимальное распространение, установите *platformFaultDomainCount* в значение *1*. Чтобы использовать статическое распространение между 5 доменами сбоя, установите для параметра *platformFaultDomainCount* значение *5*. В API версии *2017-12-01* *platformFaultDomainCount* по умолчанию имеет значение *1* для масштабируемых наборов с одной зоной и между зонами. В настоящее время для региональных масштабируемых наборов (не зональные) поддерживается только статическое распределение по пяти доменам сбоя.
