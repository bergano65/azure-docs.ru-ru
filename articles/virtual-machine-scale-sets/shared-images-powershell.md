...
Чтобы выполнить пример из этой статьи, вам нужен существующий управляемый образ. Вы можете следить за [учебником: Создайте и используйте пользовательское изображение для виртуальных наборов масштабов машин с Azure PowerShell,](tutorial-use-custom-image-powershell.md) чтобы создать его, если это необходимо. Если управляемое изображение содержит диск данных, размер диска данных не может быть более 1 ТБ.
...
Создайте масштабируемый набор виртуальных машин с помощью командлета [New-AzVmss](/powershell/module/az.compute/new-azvmss). Следующий пример создает набор масштабов из новой версии изображений в центре обработки данных *South Central US.* При появлении запроса задайте собственные административные учетные данные для экземпляров виртуальных машин в масштабируемом наборе:
