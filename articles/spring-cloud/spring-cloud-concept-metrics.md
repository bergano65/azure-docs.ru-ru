...
>| name | Имя метрики пружинного выключателя | Unit | Сведения |
>|----|----|----|------------|
...
>| Число GC Gen 0  | Gen-0-GC-Count | Счетчик        | Число сборок мусора поколения 0 в секунду. |
>| Число GC Gen 1  | Gen-1-GC-Count | Счетчик        | Число сборок мусора поколения 1 в секунду. |
>| Число сборок мусора поколения 2  | Gen-2-GC-Count | Счетчик        | Число сборок мусора поколения 2 в секунду. |
...
>| Число сборок  | число сборок | Счетчик        | Число загруженных сборок. |
>| Число исключений | число исключений | Счетчик       | Число исключений в секунду. |
>| Число потоков пула потоков      | ThreadPool — число потоков              | Счетчик | Число потоков пула потоков. |
>| Мониторинг количества конфликтов блокировок | монитор-блокировка-состязание-количество        | Счетчик | Число конфликтов в секунду при попытке снять блокировку монитора. |
>| Длина очереди пула потоков      | ThreadPool — длина очереди              | Счетчик | Длина очереди рабочих элементов пула потоков. |
>| Число завершенных элементов пула потоков | ThreadPool — завершено — количество элементов | Счетчик | Число завершенных рабочих элементов пула потоков. |
>| Число активных таймеров               | Счетчик "активный — таймер"               | Счетчик | Количество таймеров, которые активны в данный момент. Активным называется таймер, который регистрируется в такте в какой-то момент в будущем и еще не был отменен. |
...
>| name | Имя метрики пружинного выключателя | Unit | Сведения |
>|------|-----------------------------|------|---------|
>| Число запросов в секунду | запросов в секунду | Счетчик | Частота запросов. |
>| Общее количество запросов | Всего запросов | Счетчик | Общее число запросов. |
>| Текущие запросы | текущие запросы | Счетчик | Число текущих запросов. |
>| Неудачные запросы | Неудачные запросы | Счетчик | Количество невыполненных запросов. |
