...
>| аппмеморюсед <br><br>Используемая память приложения (не рекомендуется) | виртуальной машины Java. Memory. used | Байты | Представляет объем памяти, используемый в данный момент в байтах. |
...
>| аппмеморимакс<br><br>Максимальная память приложения (не рекомендуется) | виртуальной машины Java. Memory. Max | Байты | Представляет максимальный объем памяти, который может использоваться для управления памятью. Объем используемой и зафиксированной памяти всегда будет меньше или равен Max, если определено максимальное значение. Выделение памяти может завершиться ошибкой, если она попытается увеличить используемую память, например, которая использовалась > зафиксирована, даже если используется <= Max все равно будет иметь значение true (например, если в системе недостаточно виртуальной памяти). |
...
>| максолдженмеморипулбитес<br><br>Максимальный доступный размер данных старого поколения (не рекомендуется) | виртуальной машины Java. GC. max. Data. size | Байты | Пиковое использование памяти пула памяти старого поколения с момента запуска виртуальной машины Java. |
...
>| олдженмеморипулбитес<br><br>Старый размер данных поколения (не рекомендуется) | виртуальной машины Java. GC. Live. Data. size | Байты | Размер пула памяти старого поколения после полного GC. |
...
>| олдженпромотедбитес<br><br>Повысить до старого размера данных поколения (не рекомендуется) | виртуальной машины Java. GC. Memory. выдвинуто | Байты | Число положительных увеличений размера пула памяти старого поколения до сборки мусора в после сборки мусора. |
...
>| йоунгженпромотедбитес<br><br>Превращение в размер данных создания Титов. (не рекомендуется) | виртуальной машины Java. GC. Memory. распределяемый | Байты | Увеличивается при увеличении размера пула памяти создания Титов после одного сборщика мусора до следующего. |
...
>| гкпаусетоталкаунт<br><br>Счетчик приостановки GC (не рекомендуется) | виртуальной машины Java. GC. Pause (Итого — число) | Счетчик | Общее число GC после запуска этого ЖМВ, включая Титов и Old GC. |
...
>| гкпаусетоталтиме<br><br>Общее время приостановки сборки мусора (не рекомендуется) | виртуальной машины Java. GC. Pause (всего-время) | Миллисекунды | Общее время, затраченное на сборку мусора после запуска ЖМВ, включая Титов и Old GC. |
...
>| Tomcat. threads. config. Max | Tomcat. threads. config. Max | Счетчик | Максимальное число потоков для конфигурации Tomcat |
>| Tomcat. threads. Current | Tomcat. threads. Current | Счетчик | Tomcat текущее число потоков |
>| Tomcat. Global. Request. AVG. Time | Tomcat. Global. Request. AVG. Time | Миллисекунды | Среднее время запроса Tomcat |
...
>| Название | Имя метрики пружинного выключателя | Единицы | Сведения |
>|----|----|----|------------|
>| томкатсентбитес<br><br>Tomcat всего отправленных байтов (не рекомендуется) | Tomcat. Global. Sent | Байты | Объем отправленного веб-сервера Tomcat данных |
>| Tomcat. Global. Sent | Tomcat. Global. Sent | Байты | Объем отправленного веб-сервера Tomcat данных |
>| томкатрецеиведбитес<br><br>Общее число полученных байт Tomcat (не рекомендуется) | Tomcat. Global. Received. | Байты | Объем полученного веб-сервера Tomcat данных |
>| Tomcat. Global. Received. | Tomcat. Global. Received. | Байты | Объем полученного веб-сервера Tomcat данных |
>| томкатрекуесттоталтиме<br><br>Общее время запроса Tomcat (не рекомендуется) | Tomcat. Global. Request (всего-Time) | Миллисекунды | Общее время Tomcat веб-сервера для обработки запросов |
>| томкатрекуесттоталкаунт<br><br>Общее число запросов Tomcat (не рекомендуется) | Tomcat. Global. Request (всего — Count) | Счетчик | Общее число обработанных запросов веб-сервера Tomcat |
...
>| томкатрекуестмакстиме<br><br>Максимальное время запроса Tomcat (не рекомендуется) | Tomcat. Global. Request. Max | Миллисекунды | Максимальное время, в течение которого веб-сервер Tomcat обработать запрос |
...
>| Название | Имя метрики пружинного выключателя | Единицы | Сведения |
>|----|----|----|------------|
>| томкатсессионактивемакскаунт<br><br>Максимальное число активных сеансов Tomcat (не рекомендуется) | Tomcat. Sessions. Active. Max | Счетчик | Максимальное количество сеансов, которые были активными одновременно |
...
>| томкатсессионаливемакстиме<br><br>Максимальное время сеанса Tomcat (не рекомендуется) | Tomcat. Sessions. Alive. Max | Миллисекунды | Наибольшее время (в секундах), в течение которого истек срок действия сеанса |
>| Tomcat. Sessions. Alive. Max | Tomcat. Sessions. Alive. Max | Миллисекунды | Наибольшее время (в секундах), в течение которого истек срок действия сеанса |
>| томкатсессионкреатедкаунт<br><br>Число созданных сеансов Tomcat (не рекомендуется) | Tomcat. Sessions. Created | Счетчик | Число созданных сеансов |
>| Tomcat. Sessions. Created | Tomcat. Sessions. Created | Счетчик | Число созданных сеансов |
>| томкатсессионекспиредкаунт<br><br>Счетчик с истекшим сроком действия сеанса Tomcat (не рекомендуется) | Tomcat. Sessions. Expires | Счетчик | Число просроченных сеансов |
>| Tomcat. Sessions. Expires | Tomcat. Sessions. Expires | Счетчик | Число просроченных сеансов |
>| томкатсессионрежектедкаунт<br><br>Число отклоненных сеансов Tomcat (не рекомендуется) | Tomcat. Sessions. отклонено | Счетчик | Число сеансов, которые не были созданы, так как достигнуто максимальное число активных сеансов. |
