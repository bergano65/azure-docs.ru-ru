...
Чтобы устранить эту ошибку, перейдите `server parameters` к экземпляру MySQL и измените `time_zone` значение с *System* на *+ 0:00*.
...
1. Перейдите к `https://<your application test endpoint>/actuator/env` ответу и проверьте его.  Результат будет выглядеть так:
...
Найдите дочерний узел с `systemEnvironment`именем.  Этот узел содержит переменные среды приложения.
...
Если вы видите метрики из _виртуальной машины Java_ , но не метрики из _Tomcat_, проверьте, включена ли `spring-boot-actuator` зависимость в пакете приложения и что она успешно загружается.
