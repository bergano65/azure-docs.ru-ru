...
* **Подсети**. Виртуальная сеть должна содержать две подсети, выделенные для экземпляра службы Azure Spring Cloud: 
    ...
    * Между этими подсетями и экземпляром службы Azure Spring Cloud существует связь "один к одному". Нельзя совместно использовать несколько экземпляров службы в одной подсети. Для каждого развертываемого экземпляра службы необходимо использовать новую подсеть.
