...
3. Щелкните **Добавить потоковый вход** и выберите **Концентратор событий**. Затем укажите на странице "Новые входные данные" следующие сведения:
   ...
   |---------|---------|---------|
   |Входной псевдоним  |  CallStream   |  Введите имя для определения входных данных задания.   |
...
Чтобы проверить такие случаи, вы можете использовать самосоединение потоковой передачи данных, чтобы выполнить самосоединение потока на основе значения `CallRecTime`. Затем можно найти записи вызовов, в которых `CallingIMSI` значение (исходный номер) совпадает, `SwitchNum` но значение (страна или регион происхождения) не совпадает.
...
4. На панели **Выходные данные** щелкните **Добавить** и выберите **Хранилище BLOB-объектов**. Затем укажите на странице "Новые выходные данные" следующие сведения:
   ...
   |**Параметр**  |**Рекомендуемое значение**  |**Описание**  |
   |---------|---------|---------|
   |Выходной псевдоним  |  CallStream-FraudulentCalls   |  Введите имя для определения выходных данных задания.   |
