...
1. Войдите на портал Azure и нажмите **«Создайте ресурс** в левом верхнем верхнем конце экрана».
...
2. В панели концентратора случая щелкните **общие политики доступа,** а затем нажмите ** + &nbsp;Добавить.**
...
3. В элементе `<appSettings>` сделайте следующее:
   ...
   * Задайте для ключа `Microsoft.ServiceBus.ConnectionString` значение строки подключения. 
   ...
   Раздел `<appSettings>` будет выглядеть следующим примером:
...
1. На портале Azure нажмите **«Создайте работу по поиску ресурсов** > **Internet of Things** > **Stream Analytics».**
...
Для такого преобразования нужна последовательность временных окон, которые не перекрываются. У каждого окна будет дискретный набор данных, которые можно группировать и статистически обрабатывать. Такой тип окна называется *"переворачивающимся" окном*. В окне Tumbling, вы можете получить количество входящих `SwitchNum`вызовов, сгруппированных по , который представляет страну / регион, где вызов возник. 
...
1. Измените запрос в редакторе кода, используя следующие значения:
    ...
    Чтобы указать, что вы хотите использовать окно Tumbling, `GROUP BY` вы используете функцию [TUMBLINGWINDOW](https://docs.microsoft.com/stream-analytics-query/tumbling-window-azure-stream-analytics) в оговорке. В функции укажите единицу времени (от микросекунд до 24 часов) и размер окна (количество единиц). В этом примере окно Tumbling состоит из 5-секундных интервалов, так что вы получите подсчет по странам/регионам за каждые 5 секунд количества звонков.
...
Чтобы проверить такие случаи, вы можете использовать самосоединение потоковой передачи данных, чтобы выполнить самосоединение потока на основе значения `CallRecTime`. Затем можно посмотреть записи `CallingIMSI` вызовов, где значение (исходное `SwitchNum` число) является одинаковым, но значение (страна/регион происхождения) не одно и то же.

При использовании соединения с потоковой передачей данных соединение должно предоставить некоторые ограничения в отношении того, насколько совпадающие строки могут быть разделены по времени. (Как отмечалось ранее, потоковые данные фактически бесконечны.) Границы времени для отношений указаны внутри `ON` положения соединения `DATEDIFF` с использованием функции. В этом случае соединение основано на 5-секундном интервале данных вызова.
