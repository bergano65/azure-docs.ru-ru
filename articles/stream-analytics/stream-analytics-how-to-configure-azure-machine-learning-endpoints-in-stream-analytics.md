...
---
# <a name="azure-machine-learning-studio-classic-integration-in-stream-analytics-preview"></a>Студия машинного обучения Azure (классическая) интеграция в Stream Analytics (Preview)
Stream Analytics поддерживает функции, определяемые пользователями, которые вызывают в Azure Machine Learning Studio (классические) конечные точки. Поддержка этой возможности в REST API описана в статье [Библиотека REST API Stream Analytics](https://msdn.microsoft.com/library/azure/dn835031.aspx). В этой статье приведены дополнительные сведения, необходимые для успешной реализации этой возможности в Stream Analytics. Также было размещено руководство, которое доступно [здесь](stream-analytics-machine-learning-integration-tutorial.md).

## <a name="overview-azure-machine-learning-studio-classic-terminology"></a>Обзор: Терминология Машинного обучения Azure (классическая)
Microsoft Azure Machine Learning Studio (классическая) предоставляет совместный инструмент, инструмент для перетаскивания, который можно использовать для создания, тестирования и развертывания решений для прогностической аналитики на ваших данных. Этот инструмент называется *Azure Machine Learning Studio (классический).* Студия используется для взаимодействия с ресурсами машинного обучения, а также для компилирования и тестирования прототипа с последующими итерациями. Эти ресурсы и их определения приведены ниже.
...
* **Конечная точка**: *Конечные точки* — это объект Azure Machine Learning Studio (классический), используемый для использования функций в качестве ввода, применения указанной модели машинного обучения и возврата набранного вывода.
* **Оценивающая веб-служба**. *Оценивающая веб-служба* — это коллекция конечных точек, упомянутых выше.

Каждая конечная точка имеет API для пакетного и синхронного выполнения. В Stream Analytics используется синхронное выполнение. Конкретная служба называется [Служба запроса/ответа](../machine-learning/studio/consume-web-services.md) в Студии машинного обучения Azure (классическая).
...
Например, следующий пример кода создает scalar UDF под названием *Newudf,* который связывается с azure Machine Learning Studio (классическая) конечная точка. Обратите внимание, что *конечную точку* (URI службы) можно найти на странице справки по API для выбранной службы, а *apiKey* — на главной странице служб.
...
После создания схемы определяемой пользователем функции требуется получить ее полное определение. Конечная точка RetrieveDefaultDefinition поможет получить определение по умолчанию для функции scalar, которая связана с конечная точка Azure Machine Learning Studio (классическая). Для приведенных ниже полезных данных требуется получить определение указанной пользователем функции по умолчанию для скалярной функции, которая привязана к конечной точке машинного обучения Azure. Фактическая конечная точка не указывается, так как она задана во время запроса PUT. Stream Analytics вызывает указанную в запросе конечную точку, если она задана явно. В противном случае будет использоваться конечная точка, указанная изначально. Здесь определяемая пользователем функция (UDF) принимает один строковой параметр (предложение) и возвращает один вывод строкового типа, который указывает на метку "тональность" для этого предложения.
