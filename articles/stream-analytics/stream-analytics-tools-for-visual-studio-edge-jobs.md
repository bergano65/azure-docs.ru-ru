---
title: Задания Stream Analytics Edge в инструментах Azure Stream Analytics для Visual Studio
description: В этой статье описывается, как для разработки, отладки, создания и данные Stream Analytics в IoT Edge заданий с помощью средства Stream Analytics для Visual Studio.
...
В этом руководстве вы узнаете, как использовать средства Stream Analytics для Visual Studio. Вы узнаете, как создавать, отлаживать и создания заданий Stream Analytics Edge. После создания и тестирования задания можно перейти на портал Azure, чтобы развернуть его на устройствах. 
...
![Новый проект Stream Analytics Edge в Visual Studio](./media/stream-analytics-tools-for-visual-studio-edge-jobs/new-stream-analytics-edge-project.png)
...
Задания Stream Analytics, развертывания в средах Edge Интернета вещей Stream Analytics поддерживают большую часть [справочнике по языку запросов Stream Analytics](https://msdn.microsoft.com/azure/stream-analytics/reference/stream-analytics-query-language-reference?f=255&MSPPError=-2147217396). Тем не менее следующие операции еще не поддерживаются для заданий Stream Analytics Edge: 


|**Категория**  | **Команда**  |
|---------|---------|
|Другие операторы | <ul><li>PARTITION BY;</li><li>TIMESTAMP BY OVER</li><li>Определяемые пользователем функции JavaScript</li><li>Определяемые пользователем статистические функции (UDA)</li><li>GetMetadataPropertyValue</li><li>С помощью более 14 статических выражений за один шаг</li></ul>   |

При создании заданий Stream Analytics Edge на портале компилятор автоматически предупредит вас Если вы не используете является поддерживаемым оператором.
...
3. Откроется всплывающее окно. Выберите для обновления существующего задания Stream Analytics Edge или создайте новую. При обновлении существующего задания, она заменит все параметры задания, в этом сценарии, будет опубликовано новое задание. Выберите **Create a New Azure Stream Analytics Job** (Создать задание Azure Stream Analytics), введите имя для этого задания, например **MyASAEdgeJob**, выберите необходимые **подписку**, **группу ресурсов** и **расположение**, а затем нажмите кнопку **Отправить**.
   ...
   Теперь задания Stream Analytics Edge создан. Можно ссылаться на [выполнение заданий в IoT Edge руководстве](stream-analytics-edge.md) чтобы узнать, как развернуть его на устройствах. 

## <a name="manage-the-job"></a>Управление заданием 

Вы можете просмотреть состояние и схему заданий в обозревателе сервера. Из **Stream Analytics** в **обозревателя серверов**, разверните подписку и группу ресурсов, где развернуто задание Stream Analytics Edge. Вы увидите задание MyASAEdgejob с состоянием **Создано**. Разверните узел задания и дважды щелкните его, чтобы открыть представление задания.
