...
---
# <a name="perform-sentiment-analysis-with-azure-stream-analytics-and-azure-machine-learning-studio-classic"></a>Выполняйте анализ настроений с помощью Аналитики спотоков Azure stream и студии машинного обучения Azure (классический)

В этой статье описывается, как быстро настроить простое задание Azure Stream Analytics, интегрируемое в программу Azure Machine Learning Studio (классическая). Вы используете модель машинного обучения для анализа тональности из коллекции Cortana Intelligence для анализа потока текстовых данных, а также определения оценки тональности в реальном времени. С помощью Cortana Intelligence Suite вы сможете выполнить эту задачу, не вникая в особенности создания модели анализа тональности.
...
3. Добавьте модель аналитики настроений из галереи Cortana Intelligence Gallery в рабочее пространство Azure Machine Learning Studio (классическое) и разместите эту модель в качестве веб-сервиса в рабочем пространстве машинного обучения.
...
1. На портале Azure нажмите **«Создайте учетную** > **запись****хранения** > ресурсов».
...
4. На портале Azure выберите учетную запись хранения. В лезвии учетной записи ** + &nbsp;** хранилища щелчкните **Контейнеры,** а затем нажмите Контейнер для создания хранилища капли.
...
2. Нажмите **Создать ресурс** > **Интернет вещей** > **Поток Analytics работу**. 
...
3. Заполните колонку **Хранилище BLOB-объектов** следующими значениями:
   ...
   |Поле  |Значение  |
   |---------|---------|
   |**Вхотливый псевдоним** | Используйте имя `datainput` и выберите **Select blob storage from your subscription** (Выбрать хранилище BLOB-объектов из своей подписки).       |
   |**Учетная запись хранилища**  |  выберите учетную запись хранения, созданную ранее.  |
...
3. Заполните колонку **Хранилище BLOB-объектов** следующими значениями:
   ...
   |Поле  |Значение  |
   |---------|---------|
   |**Выходный псевдоним** | Используйте имя `datamloutput` и выберите **Select blob storage from your subscription** (Выбрать хранилище BLOB-объектов из своей подписки).       |
   |**Учетная запись хранилища**  |  выберите учетную запись хранения, созданную ранее.  |
   |**Контейнер**  | Выберите созданный ранее контейнер (`azuresamldemoblob`).        |
   |**Формат сериализации событий**  |  Выберите **CSV**       |
...
2. Перейдите к функциям > **работы** > **и добавьте** > **AzureML**
