...
2. После того как данные будут распределены в источнике данных, необходимо убедиться в том, что запрос разбит на секции. Это требует от вас использования **PARTITION BY** во всех шагах. Этапов может быть несколько, но на каждом из них должен использоваться один и тот же ключ. При уровне совместимости 1.0 и 1.1 ключ раздела должен быть установлен на **РазделитивИд,** чтобы задание было полностью параллельным. Для заданий с уровнем совместимости 1.2 и выше пользовательский столбец может быть указан как ключ раздела в настройках ввода, и задание будет паралелизовано автоматически даже без положения PARTITION BY. Для вывода концентратора событий свойство "Ключевой столбец раздела" должно быть установлено для использования "PartitionId".

3. Большая часть выходных данных может воспользоваться преимуществами секционирования, однако при использовании типа выходных данных, не поддерживающих секционирование, задание не будет полностью параллельным. Для выводов концентратора событий убедитесь, что **столбец ключа раздела** настроен так же, как и ключ раздела запроса. Дополнительные сведения см. в [этом разделе](#outputs).
...
* Выход: Концентратор событий с 8 разделами ("ключевой столбец раздела" должен быть установлен для использования "PartitionId")
...
### <a name="compatibility-level-12---multi-step-query-with-different-partition-by-values"></a>Уровень совместимости 1.2 - Многоступенчатый запрос с различными значениями PARTITION BY 
* Входные данные — концентратор событий с 8 секциями.
* Выход: Концентратор событий с 8 разделами ("Ключевой столбец раздела" должен быть установлен для использования "TollBoothId")
...
Уровень совместимости 1.2 позволяет параллельно выполнять запрос по умолчанию. Например, запрос из предыдущего раздела будет разделен до тех пор, пока столбец "TollBoothId" установлен в качестве ключа ввода раздела. PartITION BY PartitionId оговорка не требуется.
...
* Запрос в пределах шага должен иметь ключевое слово **PARTITION BY.**
...
| <ul><li>Поток входных данных секционирован по 3.</li><li>Запрос состоит из двух шагов. Входной шаг секционирован, а второй шаг — нет.</li><li>Заявление <strong>SELECT</strong> читается из раздельного ввода.</li></ul> | 24 (18 и 6 секционированных и несекционированных шагов соответственно) |
...
## <a name="achieving-higher-throughputs-at-scale"></a>Достижение более высокой пропускной воды в масштабе

[Неловсомненно параллельная](#embarrassingly-parallel-jobs) работа необходима, но не достаточна для поддержания более высокой пропускной выхажеловки в масштабе. Каждая система хранения данных и соответствующий выход Stream Analytics имеет различия в том, как достичь наилучшей пропускной записи. Как и в любом сценарии в масштабе, есть некоторые проблемы, которые могут быть решены с помощью правильных конфигураций. В этом разделе рассматриваются конфигурации для нескольких общих выходов и приведены образцы для поддержания коэффициентов приема 1K, 5K и 10K событий в секунду.

При следующих наблюдениях используется задание Stream Analytics с запросом без состояния (прохождение), базовый JavaScript UDF, который записывается в Event Hub, Azure S'L DB или Cosmos DB.

#### <a name="event-hub"></a>Концентратор событий

|Коэффициент приема (события в секунду) | Единицы потоковой передачи | Ресурсы вывода  |
|--------|---------|---------|
| 1 тыс.     |    1    |  2 ТУ   |
| 5 тыс.     |    6    |  6 ТУ   |
| 10 тыс.    |    12   |  10 ТУ  |

Решение [Event Hub](https://github.com/Azure-Samples/streaming-at-scale/tree/master/eventhubs-streamanalytics-eventhubs) имеет линейное масштабирование с точки зрения потоковых единиц (SU) и пропускной всей, что делает его наиболее эффективным и эффективным способом анализа и потоковой передачи данных из Stream Analytics. Вакансии могут быть сокращены до 192 SU, что примерно переводится для обработки до 200 МБ / с, или 19 триллионов событий в день.
...
[Azure S'L](https://github.com/Azure-Samples/streaming-at-scale/tree/master/eventhubs-streamanalytics-azuresql) поддерживает запись параллельно под названием Inherit Partition, но она не включена по умолчанию. Однако для достижения более высоких пропускной воды, наряду с полностью параллельным запросом, для достижения более высоких пропускной воды может оказаться достаточное обеспечение. Пропускные данные s'L в значительной степени зависят от конфигурации базы данных S'L Azure и схемы таблицы. В статье, опубликованной в [статье s'L Output Performance,](./stream-analytics-sql-output-perf.md) более подробно описаны параметры, которые могут максимизировать пропускную способность записи. Как отмечено в выходе Azure Stream Analytics в статье [базы данных Azure S'L Database,](./stream-analytics-sql-output-perf.md#azure-stream-analytics) это решение не масштабируется линейно как полностью параллельный конвейер за пределами 8 разделов и, возможно, потребуется переделка перед выходом S'L (см. [INTO).](https://docs.microsoft.com/stream-analytics-query/into-azure-stream-analytics#into-shard-count) Премиум SKUs необходимы для поддержания высоких ставок IO наряду с накладными от резервного копирования журнала происходит каждые несколько минут.
...
[Космический DB](https://github.com/Azure-Samples/streaming-at-scale/tree/master/eventhubs-streamanalytics-cosmosdb) выход из Stream Analytics был обновлен для использования родной интеграции под [уровень совместимости 1.2](./stream-analytics-documentdb-output.md#improved-throughput-with-compatibility-level-12). Уровень совместимости 1.2 обеспечивает значительно более высокую пропускную связь и снижает потребление RU по сравнению с 1,1, что является уровнем совместимости по умолчанию для новых рабочих мест. В решении используются контейнеры CosmosDB, разделенные на /deviceId, а остальная часть решения настроена одинаково.

Все [стриминговые на лазурных образцах scale](https://github.com/Azure-Samples/streaming-at-scale) используют концентратор событий, питаемый загрузкой, имитирующей тестовые клиенты в качестве ввода. Каждое событие ввода представляет собой документ 1KB JSON, который легко переводит настройки коэффициентов приема на скорость пропускной связи (1 МБ/с, 5 МБ/с и 10 МБ/с). События имитируют ioT-устройство, отправляющее следующие данные JSON (в укороченной форме) для устройств до 1K:
...
> [!NOTE]
> Конфигурации могут изменяться из-за различных компонентов, используемых в решении. Для более точной оценки настройте образцы в соответствии с вашим сценарием.
