...
Уровень совместимости 1,2 разрешает выполнение параллельных запросов по умолчанию. Например, запрос из предыдущего раздела будет секционирован до тех пор, пока в качестве входного ключа секции задан столбец «TollBoothId». Предложение PARTITION BY PartitionId не требуется.
