...
Определение задания Stream Analytics включает по крайней мере один потоковый вход, запрос и выходные данные. Входные данные — это точки, откуда задания считывают данные из потока. Запрос используется для преобразования потока входных данных, а выходные данные являются точками, куда направляются результаты задания.

## <a name="partitions-in-inputs-and-outputs"></a>Секции в входных и выходных данных
Секционирование позволяет разделить данные на подмножества на основе [ключа секции](https://docs.microsoft.com/azure/event-hubs/event-hubs-scalability#partitions). Если входные данные (например, концентраторы событий) разделены по ключу, настоятельно рекомендуется указать этот ключ секции при добавлении входных данных в задание Stream Analytics. Масштабирование задания Stream Analytics использует преимущества секций во входных и выходных данных. Задание Stream Analytics может одновременно использовать и записывать разные секции, что повышает пропускную способность. 

### <a name="inputs"></a>Входные данные
Все входные данные в Azure Stream Analytics могут использовать преимущества секционирования:
-   EventHub (если используется уровень совместимости 1,1 или ниже, необходимо явно задать ключ секции с ключевым словом PARTITION BY)
-   Центр Интернета вещей (если используется уровень совместимости 1,1 или ниже, необходимо явно задать ключ секции с ключевым словом PARTITION BY)
...
Задание с *усложненным параллелизмом* является наиболее масштабируемым сценарием в Azure Stream Analytics. Он соединяет один раздел входных данных с одним экземпляром запроса и одним разделом выходных данных. Такой параллелизм имеет следующие требования:

1. Если в логике запроса применяется ключ, который обрабатывается тем же экземпляром запроса, необходимо только проследить за тем, чтобы события попадали в ту же секцию входных данных. При использовании Центров событий или Центра Интернета вещей это означает, что для данных событий должно быть задано значение **PartitionKey**. Кроме того, можно использовать секционированные отправители. Для хранилища BLOB-объектов это означает, что события отправляются в папку той же секции. Примером может служить экземпляр запроса, который объединяет данные по userID, где концентратор событий ввода секционирован с использованием userID в качестве ключа секции. Однако если логика запроса не требует того же ключа для обработки одним и тем же экземпляром запроса, это требование можно игнорировать. В качестве примера такой логики можно привести простой запрос select, project или filter.  

2. Следующий шаг заключается в том, чтобы сделать запрос секционированным. Для заданий с уровнем совместимости 1,2 или выше (рекомендуется) пользовательский столбец может быть указан в качестве ключа секции во входных параметрах, и задание будет паралеллизед автоматически. Для заданий с уровнем совместимости 1,0 или 1,1 необходимо использовать **секционирование по PartitionId** во всех шагах запроса. Этапов может быть несколько, но на каждом из них должен использоваться один и тот же ключ. 

3. Большинство выходов, поддерживаемых в Stream Analytics, могут воспользоваться преимуществами секционирования. Если используется тип выходных данных, который не поддерживает секционирование задания, то параллельное использование не будет *усложнено*. Для выходных данных концентратора событий убедитесь, что для **ключевого столбца секции** задан один и тот же ключ секции, используемый в запросе. Дополнительные сведения см. в [этом разделе](#outputs).
...
Если число входных секций не совпадает с числом выходных секций, топология не будет с усложненным параллелизмом независимо от запроса. Однако по-прежнему можно получить определенный уровень или параллелизации.
...
* Запрос внутри шага должен иметь ключевое слово **Partition by** .
