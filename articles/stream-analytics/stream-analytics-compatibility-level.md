...
**уровень 1,0:** Средство синтаксического анализа JSON автоматически преобразует строковые значения со сведениями о дате, времени или зоне в тип DateTime, а затем преобразует их в формат UTC. Это привело к потере сведений о часовом поясе.

**уровень 1,1:** Автоматическое приведение строковых значений с данными о дате, времени или зоне в тип DateTime больше не поддерживается. В результате информация о часовом поясе сохраняется.
