...
После подтверждения подключения любое задание, выполняемое в кластере Stream Analytics, сможет получить доступ к ресурсу через эту частную конечную точку. В этой статье показано, как создавать и удалять частные конечные точки в кластере Stream Analytics.
