...
ms.locfileid: "80409248"
---
# <a name="troubleshoot-azure-stream-analytics-outputs"></a>Устранение неполадок с выходными данными в Azure Stream Analytics

В этой статье описаны общие проблемы с выходными соединениями Azure Stream Analytics, как устранить проблемы вывода и как устранить проблемы. Многие этапы устранения неполадок требуют включения диагностических журналов для работы Stream Analytics. Если нет включенных диагностических журналов, [см.](stream-analytics-job-diagnostic-logs.md)
...
2.  Посмотрите на [**метрики мониторинга**](stream-analytics-monitoring.md) на вкладке **Монитор.** Поскольку значения агрегируются, метрики задерживаются на несколько минут.
   * Если ввод событий превышает 0, задания может считывать входные данные. Если ввод событий не превышает 0, то возникает проблема с вводом задания. Просмотрите [входные соединения Troubleshoot,](stream-analytics-troubleshoot-input.md) чтобы узнать, как устранить проблемы ввода.
   * Если ошибки преобразования данных превышают 0 и восхождение, см. [ошибки данных Azure Stream Analytics](data-errors.md) для получения подробной информации об ошибках преобразования данных.
   * Если количество ошибок в продолжительности превышает 0, ваша работа может получать данные, но при обработке запроса генерирует ошибки. Чтобы найти ошибки, перейдите к [журналам аудита](../azure-resource-manager/management/view-activity-logs.md) и выполните фильтрацию по состоянию *Сбой*.
   * Если Ввесть больше 0 и Выходы равны 0, то один из следующих примеров:
      * В результате обработки исходящие события не получены.
      * События или поля могут быть пороками, что приводит к нулевому выходу после обработки запросов.
      * Заданию не удалось передать данные в приемник выходных данных из-за проблем с подключением или аутентификацией.

   В указанных случаях дополнительные сведения об этих ошибках можно получить в сообщениях журнала операций (включая сведения о самом событии), за исключением случаев, когда логика запроса отфильтровала все события. Если обработка нескольких событий генерирует ошибки, ошибки агрегируются каждые 10 минут.
