...
2. В левом меню выберите **выходы** в рамках **топологии вакансий.** Затем выберите **и переключите** **Power BI** из меню выпадения.
...
3. **Выберите: Добавить** > **Power BI**. Затем заполните форму следующими сведениями и нажмите кнопку **Авторизовать**:
   ...
   |Имя таблицы |  мошеннические звонки  |
...
* **defaultRetentionPolicy: BasicFIFO** - Данные FIFO, с максимумом 200 000 строк.
* **defaultMode: pushStreaming** - Набор данных поддерживает как потоковые плитки, так и традиционные визуальные эффекты на основе отчетов (также известные как push).
...
2. На странице **запроса** для работы Stream Analytics щелкните точки рядом с `CallStream` вводом, а затем выберите **примерданных данных из ввода.**

3. Укажите, что необходим трехминутный образец данных, и щелкните **ОК**. Дождитесь уведомления о выборке данных.

4. Нажмите **тест** и просмотрите результаты.

## <a name="run-the-job"></a>Запуск задания

1. Убедитесь, что приложение TelcoStreaming работает.

2. Перейдите на страницу **«Обзор»** для выполнения задания Stream Analytics и выберите **«Старт».**
...
2. В рабочем пространстве нажмите ** + &nbsp;«Создать».**
...
* [Ссылка на API управления аналитикой лазурных потоков REST](https://msdn.microsoft.com/library/azure/dn835031.aspx)
