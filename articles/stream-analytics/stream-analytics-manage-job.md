...
3. После создания политики щелкните ее имя, чтобы открыть ее. Найдите поле **Строка подключения — первичный ключ**. Нажмите кнопку **копирования** рядом со строкой подключения.
...
:::image type="content" source="media/stream-analytics-manage-job/authorize-power-bi.png" alt-text="Кнопка авторизации для Power BI":::
...
Далее необходимо создать преобразование, анализирующее данные в режиме реального времени. Определите запрос преобразования с помощью [языка запросов Stream Analytics](/stream-analytics-query/stream-analytics-query-language-reference). Запрос, используемый в этом руководстве, обнаруживает мошеннические вызовы в данных телефона.
...
2. В разделе **Топология задания** задания Stream Analytics выберите вариант **Запрос**. В окне запроса перечислены входные и выходные данные, настроенные для задания, и можно создать запрос, преобразовывающий входной поток.
...
Запрос можно протестировать из редактора запросов. Выполните следующие действия, чтобы проверить запрос:

1. Проверьте, чтобы приложение TelcoGenerator работало и создавало записи телефонных звонков.

2. Выберите **Test** (Тестировать) для проверки запроса. Вы увидите следующие результаты:
