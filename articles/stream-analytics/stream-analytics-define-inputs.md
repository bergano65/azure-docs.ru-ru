...
В следующей таблице приводится описание каждого свойства на **новой странице ввода** в портал Azure для потоковой передачи входных данных из концентратора событий.
...
| **Тип сжатия события** | Тип сжатия используется для чтения таких входящих потоков данных, как None (по умолчанию), GZip или Deflate. |
...
| **Формат даты** (необязательно) | При использовании переменной даты в пути это формат даты, по которому упорядочены файлы. Пример: `YYYY/MM/DD` |
