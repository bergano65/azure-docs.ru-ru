...
* [Подписка на Azure](https://azure.microsoft.com/pricing/free-trial/)
...
| ИД пункта сбора | Время въезда | LicensePlate | Штат | Убедитесь, | Модель | Тип транспортного средства | Вес транспортного средства | Плата | Тег |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| **Толлид** | **Время выезда** | **LicensePlate** |
| --- | --- | --- |
...
2. На странице **обзора** задания в поле **Запрос** просмотрите синтаксис запроса.
   ...
   Как видите, Azure Stream Analytics использует язык запросов, такой же как SQL, и добавляет несколько расширений для указания аспектов запроса, связанных со временем.  См. дополнительные сведения о [конструкциях управления временем](https://docs.microsoft.com/stream-analytics-query/time-management-azure-stream-analytics) и [оконных расширениях](https://docs.microsoft.com/stream-analytics-query/windowing-azure-stream-analytics), используемых в этом запросе.
...
4. Просмотрите выходные данные примера задания TollApp.
   - **Выход Cosmos DB** — это контейнер базы данных Cosmos, который получает события, происходящие в результате. Обратите внимание, что эти выходные данные используются в предложении INTO запроса потоковой передачи.
...
4. Расширьте**платныедокументы** **AppDatabaseCollection** > **.** > 
...
Azure Stream Analytics поддерживает масштабируемость, что позволяет обрабатывать большие объемы данных. Запрос Analytics потоков Azure может использовать пункт **PARTITION BY,** чтобы сообщить системе, что этот шаг масштабируется. **PartitionId** — это специальная колонка, которая добавляет сядр, чтобы соответствовать идентификатору раздела ввода (концентратор событий).
