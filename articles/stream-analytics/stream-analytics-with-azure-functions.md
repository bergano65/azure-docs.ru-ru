...
* [создать экземпляр кэша Azure для Redis](#create-an-azure-redis-cache-instance);  
* [создать функцию в службе "Функции Azure", которая может записывать данные в кэш Azure для Redis](#create-a-function-in-azure-functions-that-can-write-data-to-azure-redis-cache);    
...
* [проверить кэш Azure для Redis на наличие результатов](#check-azure-redis-cache-for-results).  
...
2. Перейдите к функции **run.csx**. Обновите ее, используя следующий код. (Необходимо заменить \<свою строку подключения к кэшу Azure для Redis\> основной строкой подключения кэша Azure для Redis, извлеченной в предыдущем разделе.)  
