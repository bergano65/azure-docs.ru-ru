---
title: Разработка функций .NET Standard для заданий Azure Stream Analytics (Предварительный просмотр)
description: Узнайте, как писать функции, определяемые пользователями, для заданий Stream Analytics.
author: mamccrea
ms.author: mamccrea
...
---
# <a name="develop-net-standard-user-defined-functions-for-azure-stream-analytics-jobs-preview"></a>Разработка функций, определяемых пользователями .NET Standard для заданий Azure Stream Analytics (Preview)
...
Функция с определенным и попринципным пользователем .NET для облачных заданий доступна в:
...
Если вы заинтересованы в использовании этой функции в каком-либо другом регионе, вы можете [запросить доступ.](https://aka.ms/ccodereqregion)
...
В этом примере **UDFTest** — это библиотечный проект класса C, а **ASAUDFDemo** — проект Azure Stream Analytics, который будет ссылаться на **UDFTest.**
...
2. Добавьте ссылку на проект СЗ в проекте ASA. Щелкните правой кнопкой мыши узел "Ссылки" и выберите "Добавить ссылку".
...
8. В окне настройки функции C# выберите **Загрузить из ссылки на проект ASA** и щелкните имена связанных сборки, класса и метода в раскрывающемся списке. Для обозначения методов, типов и функций в запросе Stream Analytics классы должны быть определены как *общедоступные,* а объекты должны быть определены как *статические общедоступные.*
...
После отправки ZIP-файлов пакетов в учетную запись хранения Azure вы можете использовать функции в запросах Azure Stream Analytics. Все, что вам нужно сделать, это включить информацию о хранении в конфигурации работы Stream Analytics. Протестировать функцию локально с помощью этого параметра невозможно, так как инструменты Visual Studio не смогут скачать ваш пакет. Путь к пакету анализируется непосредственно в службе. 
...
Разверните раздел **Настройка пользовательского кода** и укажите следующие предложенные значения в конфигурации:
   ...
   |Источник сборки пользовательского кода|Существующие сборочные пакеты из облака|
...
* [Учебник: Напишите функцию, определяемую пользователем на C'для задания Analytics потоков Azure (Предварительный просмотр)](stream-analytics-edge-csharp-udf.md)
