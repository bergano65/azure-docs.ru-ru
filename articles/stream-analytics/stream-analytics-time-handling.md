...
- **Водяной знак**: маркер времени события, указывающий на то, какие события точки были входящий обработчику потоковой передачи. Метки времени позволяют системе указывать ход выполнения при приеме событий. Входящий поток данных событий никогда не останавливается, поэтому метки времени показывают прогресс до определенной точки в потоке.
...
Затем Stream Analytics извлекает время начала из спецификации запроса. Тем не менее, так как брокер входных событий индексируется только по времени получения, система должна преобразовать начальное время события во время получения. Система может начать обрабатывать события с этого момента в брокере входных событий. С ограничением интервала раннего поступления преобразование не представляет никаких трудностей. Это время события запуска за вычетом окна, в котором раньше поступают пять минут. Кроме того, это вычисление означает, что система удаляет все события, которые отображаются в течение 5 минут, предшествующих по сравнению со временем прибытия.
...
1. Иллюстрация того, как метка времени изменяется вместе с этими событиями:
   ...
   5. При обработке двенадцатого события (3) время прибытия (12:27) составляет 6 минут перед временем события (12:21). Применяется политика позднего получения. Время события скорректировано (12:22) и находится ниже метки времени (12:21), поэтому дальнейшая корректировка не применяется.
