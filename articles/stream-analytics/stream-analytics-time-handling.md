...
- **Метка времени**. Маркер времени событие, указывающее, до момента, какие были входящий потоковой передачи обработчику событий. Метки времени позволяют системе указывать ход выполнения при приеме событий. Входящий поток данных событий никогда не останавливается, поэтому метки времени показывают прогресс до определенной точки в потоке.
