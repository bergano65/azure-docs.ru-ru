...
## <a name="choose-the-best-starting-time"></a>Выбор наиболее подходящего времени запуска
...
Важно использовать метку времени в полезных данных, если используется временная логика для учета задержек в исходной системе или в сети. Время, назначенное событию, доступно в [SYSTEM.TIMESTAMP](/stream-analytics-query/system-timestamp-stream-analytics).
...
- [Рассмотрение порядка событий Azure Stream Analytics]()
