...
description: Узнайте, как работает обработка времени в Azure Stream Analytics, например, как выбрать лучшее время начала, как обрабатывать поздние и ранние события, а также обработку времени метрик.
author: mamccrea
ms.author: mamccrea
...
- **Время события**: Время, когда исходное событие произошло. Например, двигающийся по шоссе автомобиль приближается к пункту взимания дорожных сборов.
...
- **Водяной знак**: Маркер времени события, указывающий до того, в какую точку события были введены в потоковый процессор. Метки времени позволяют системе указывать ход выполнения при приеме событий. Входящий поток данных событий никогда не останавливается, поэтому метки времени показывают прогресс до определенной точки в потоке.
...
2. **Время приложения** (также называется временем события).
   ...
   Время приложения присваивается при создании события и является частью полезных данных события. Для обработки событий по времени подачи заявки используйте **Timestamp по** пункту в выбранном запросе. Если предложение **Timestamp by** отсутствует, то события обрабатываются по времени получения.
...
Затем Stream Analytics извлекает время начала из спецификации запроса. Тем не менее, так как брокер входных событий индексируется только по времени получения, система должна преобразовать начальное время события во время получения. Система может начать обрабатывать события с этого момента в брокере входных событий. С ограничением интервала раннего поступления преобразование не представляет никаких трудностей. Это время начала мероприятия минус 5-минутное раннее прибытие окна. Этот расчет также означает, что система отбрасывает все события, которые рассматриваются как имеющие время события 5 минут ealier, чем время прибытия.
...
1. Иллюстрация того, как метка времени изменяется вместе с этими событиями:
   ...
   5. При обработке двенадцатого события (устройство3) время прибытия (12:27) на 6 минут опережает время события (12:21). Применяется политика позднего получения. Время события скорректировано (12:22) и находится ниже метки времени (12:21), поэтому дальнейшая корректировка не применяется.
