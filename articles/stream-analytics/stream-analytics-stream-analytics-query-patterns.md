...
**Объяснение**. [COUNT(DISTINCT Time)](https://docs.microsoft.com/stream-analytics-query/count-azure-stream-analytics) возвращает количество уникальных значений в столбце "Время" в течение определенного интервала. Затем можно использовать выходные данные этого действия для вычисления среднего для каждого устройства путем удаления дубликатов.
