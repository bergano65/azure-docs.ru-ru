...
Конструктор запросов можно выразить простой сквозной логики, чтобы переместить данные события из первого входного потока в выходных данных хранилища данных, или он может выполнять широкие возможности анализа закономерностей в их сопоставления и временных для вычисления статистических значений за различные периоды времени, как показано на [построения Интернета вещей решение с помощью Stream Analytics](stream-analytics-build-an-iot-solution-using-stream-analytics.md) руководства. Можно объединить данные из нескольких входных данных для объединения события потоковой передачи, и могут выполнять операции поиска по статические эталонные данные для обогащения значений. Можно также записывать данные на несколько выходов.
...
Данные JSON и Avro могут содержать сложные типы, такие как вложенные объекты (записи) или массивы. Дополнительные сведения о работе с эти сложные типы данных, см. [данных синтаксического анализа JSON и AVRO](stream-analytics-parsing-json.md) статьи.

## <a name="query-example-convert-data-types"></a>Пример запроса. Преобразование типов данных

**Описание** Определите типы свойств для входного потока. Например, вес автомобиля поступает по входному потоку как строки и должен быть преобразован к **INT** для выполнения **сумма**.
...
## <a name="query-example-use-likenot-like-to-do-pattern-matching"></a>Пример запроса. Используйте LIKE/NOT как для сопоставления шаблонов
...
**Объяснение**. Используйте инструкцию **LIKE** для проверки значения поля **LicensePlate**. Он должен начинаться с буквы А, затем должна идти любая строка из нуля или более символов и затем заканчиваться номер 9. 
...
**Объяснение**. Выражение **INTO** сообщает Stream Analytics, в какие выходные данные записывать данные из этого оператора. Первый запрос является передачей данных, полученных на выход с именем **ArchiveOutput**. Второй запрос выполняет некоторую простую статистическую обработку и фильтрацию и отправляет результаты нижестоящей системе оповещений, **AlertOutput**.
...
Теперь давайте проблему и найдем первый автомобиль определенной марки за каждые 10 минут.
...
| пользователя | Компонент | Duration |
| --- | --- | --- |
...
**Объяснение**. Этот запрос создает события каждые 5 секунд и выводит последнее событие, полученное ранее. ["Прыгающего" окна](/stream-analytics-query/hopping-window-azure-stream-analytics) длительность определяет, насколько далеко в прошлое запрос выполняет в поисках последнего события (300 секунд в этом примере).
...
**Описание** При выполнении операции, такой как вычисление средних значений событий за заданный период времени, повторяющиеся события должны быть отфильтрованы. В следующем примере второе событие повторяется первого.
