...
### <a name="you-want-to-write-udfs-udas-and-custom-deserializers-in-a-language-other-than-javascript-or-c"></a>Вы хотите писать определяемые пользователем функции, UDA и пользовательские десериализаторы на языке, отличном от JavaScript или C. #

Azure Stream Analytics поддерживает определяемые пользователем функции (UDF) или пользовательские статистические выражения (UDA) в JavaScript для облачных заданий и C# для заданий IoT Edge. Также поддерживаются пользовательские десериализаторы C#. Если вы хотите реализовать десериализатор, определяемую пользователем функцию или UDA на других языках, таких как Java или Python, можно использовать структурированную потоковую передачу Spark. Вы также можете запустить **EventProcessorHost** концентраторов событий на собственных виртуальных машинах для выполнения произвольной потоковой обработки.
