...
Портал Azure внедряет ключевые показатели производительности, которые могут быть использованы для мониторинга и устранения неполадок в ашего запроса и производительности задания. Для просмотра метрик перейдите к нужному заданию Stream Analytics и просмотрите раздел **Мониторинг** на странице "Обзор".  
...
| Полученные входные источники       | Количество сообщений, полученных заданием. Для концентратора событий сообщение является единым EventData. Для Blob сообщение - это одна капля. Обратите внимание, что входные источники учитываются до десериализации. При наличии ошибок десериализации входные источники могут быть больше, чем вводные события. В противном случае, он может быть меньше или равен событиям ввода, так как каждое сообщение может содержать несколько событий. |
...
| Ошибки среды выполнения         | Общее количество ошибок, связанных с обработкой запросов (за исключением ошибок, обнаруженных при глотании событий или результатов outputting) |
