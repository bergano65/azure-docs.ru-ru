...
Ключ раздела, или имя столбца, используемое для разделения входных данных, может содержать буквенно-цифровые символы, дефисы, знаки подчеркивания и пробелы. Использование вложенных полей в качестве ключа раздела не поддерживается, если не используется в сочетании с псевдонимами. Ключ секции должен иметь тип NVARCHAR (MAX), BIGINT, FLOAT или BIT (уровень совместимости 1,2 или выше). Дополнительные сведения см. в разделе [типы данных Azure Stream Analytics](/stream-analytics-query/data-types-azure-stream-analytics).
