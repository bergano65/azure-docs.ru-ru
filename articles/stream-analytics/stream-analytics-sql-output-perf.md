...
- **Таблица в памяти как таблица temp** - [Таблицы in-Memory](/sql/relational-databases/in-memory-oltp/in-memory-oltp-in-memory-optimization) обеспечивают очень высокоскоростные загрузки данных, но данные должны вписываться в память. Тесты производительности демонстрируют, что массовая загрузка из таблицы в памяти в таблицу на диске примерно в 10 раз быстрее, чем во время непосредственной массовой вставки с помощью одного средства записи в дисковой таблице со столбцом идентификаторов и кластеризованным индексом. Чтобы использовать производительность массовой вставки, настройте [задание копирования с использованием службы "Фабрика данных Azure"](../data-factory/connector-azure-sql-database.md), позволяющее копировать данные из таблицы в памяти в таблицу на диске.

## <a name="avoiding-performance-pitfalls"></a>Как избежать ловушек производительности
Массовая вставка данных происходит гораздо быстрее, чем загрузка данных с помощью одиночных вставок, поскольку избегается повторное накладное расходование на передачу данных, разбор оператора вставки, запуск оператора и выдачу записи транзакции. Вместо этого в движок хранения используется более эффективный способ передачи данных. Стоимость установки этого пути, однако, намного выше, чем одна вставка оператора в таблице на основе диска. Точка безубыточности, как правило, около 100 строк, за которой навалочная нагрузка почти всегда более эффективна. 

Если скорость входящих событий низкая, она может легко создавать размеры пакетов ниже 100 строк, что делает неэффективную масс-вставку и использует слишком много дискового пространства. Чтобы обойти это ограничение, вы можете сделать одно из этих действий:
* Создайте [триггер](/sql/t-sql/statements/create-trigger-transact-sql) INSTEAD of, чтобы использовать простую вставку для каждой строки.
* Используйте таблицу Temp Memory, описанную в предыдущем разделе.

Другой такой сценарий возникает при записи в некластерный индекс columnstore (NCCI), где меньшие вставки навалом могут создавать слишком много сегментов, что может привести к сбою индекса. В этом случае рекомендуется использовать вместо этого индекс кластерного columnstore.
