...
| Хранилище BLOB-объектов Azure | Да | Используйте в шаблоне пути токены {date} и {time} {fieldname} из полей событий. Выберите формат даты, например ГГГГ-ММ-ДД, ДД-ММ-ГГГГ, ММ-ДД-ГГГГ. Для времени используется формат ЧЧ. Выходные данные большого двоичного объекта можно секционировать по одному настраиваемому атрибуту события {FieldName} или {DateTime: \<specifier> }. | Соответствует секционированию входных данных для [полностью распараллеливаемых запросов](stream-analytics-scale-jobs.md). |
