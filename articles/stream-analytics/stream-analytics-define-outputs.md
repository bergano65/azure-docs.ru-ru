...
Stream Analytics поддерживает [хранилище озера данных Azure](https://azure.microsoft.com/services/data-lake-store/). Azure Data Lake Store — это гипермасштабируемый репозиторий корпоративного уровня, для рабочих нагрузок анализа больших данных. Data Lake Store можно использовать для хранения данных любого размера, типа и скоростью приема в одном месте для эксплуатационной и исследовательской аналитики. Stream Analytics должен быть авторизован для доступа к Data Lake Store.

Выходные данные Azure Data Lake Store из Stream Analytics в настоящее время недоступны в регионах Azure для Китая (21Vianet) и Azure для Германии (T-Systems International).

### <a name="authorize-an-azure-data-lake-store-account"></a>Авторизация учетной записи Azure Data Lake Store

1. При выборе Data Lake Store в качестве выходных данных на портале Azure, будет предложено авторизовать подключение к существующему экземпляру Data Lake Store.

   ![Авторизовать подключение к Data Lake Store](./media/stream-analytics-define-outputs/06-stream-analytics-define-outputs.png)

2. Если у вас уже есть доступ к Data Lake Store, выберите **авторизовать сейчас**. Указывает, предлагающее страницы **перенаправление для авторизации**. После успешной авторизации вы увидите страницу, которая позволяет настроить выходные данные Data Lake Store.

3. После проверки подлинности учетной записи Data Lake Store, можно настроить свойства для выходных данных Data Lake Store.
...
В следующей таблице перечислены имена свойств и их описания для настройки выходных данных Data Lake Store.   

| Имя свойства | Описание |
| --- | --- |
| Выходной псевдоним | Понятное имя, используемое в запросах для направления выходных данных запроса в Data Lake Store. |
| Учетная запись | Имя учетной записи Data Lake Store, где вы отправляете ваши выходные данные. Вам предоставляется стрелку раскрывающегося списка учетных записей Data Lake Store, доступных в вашей подписке. |
...
| Сервис кодирования | Если вы используете формат CSV или JSON, должна быть указана кодировка. Сейчас UTF-8 является единственным поддерживаемым форматом кодировки.|
...
Необходимо будет повторно аутентифицировать учетную запись Data Lake Store, если с момента создания задания или последней аутентификации пароль был изменен. Если вы не выполните повторную аутентификацию, ваше задание не создает выходных результатов и отображает ошибку, которая указывает на необходимость повторная авторизация в журналах операций. 

В настоящее время токена проверки подлинности необходимо обновлять вручную каждые 90 дней для всех заданий с выходными данными Data Lake Store. Можно обойти это ограничение, [проверки подлинности с помощью управляемого удостоверения (Предварительная версия)](https://docs.microsoft.com/azure/stream-analytics/stream-analytics-managed-identities-adls).

Чтобы обновить авторизацию:

1. Выберите **остановить** для остановки задания.
1. Перейдите к вашей Data Lake Store выходных данных и выберите **обновить авторизацию** ссылку.

   На короткое время указывает всплывающей страницы **перенаправление для авторизации**. Если авторизация выполнена успешно, на странице указано, **авторизация успешно возобновлена** и затем закрывается автоматически. 
   
1. Выберите **Сохранить** в нижней части страницы. Затем можно перезапустить задание со **время последней остановки** во избежание потери данных.

![Обновление авторизации Data Lake Store в выходных данных](./media/stream-analytics-define-outputs/08-stream-analytics-define-outputs.png)
...
| Имя свойства | Описание |
| --- | --- |
| Выходной псевдоним |Понятное имя, которое используется в запросах для направления выходных данных запроса в соответствующую базу данных. |
...
> [!NOTE]
> В настоящее время для выходных данных задания в Stream Analytics поддерживается предложение базы данных SQL Azure. Виртуальной машине Azure под управлением SQL Server с подключенной базой данных не поддерживается. Это может быть изменено в будущих выпусках.
>

## <a name="blob-storage"></a>Хранилище BLOB-объектов
Хранилище BLOB-объектов предлагает экономичное и масштабируемое решение для хранения больших объемов неструктурированных данных в облаке. Общие сведения о хранилище BLOB-объектов и их использование, см. в разделе [способы использования больших двоичных объектов](../storage/blobs/storage-dotnet-how-to-use-blobs.md).

Ниже перечислены имена свойств и их описания для создания выходных данных в BLOB-объектов.

| Имя свойства       | Описание                                                                      |
| ------------------- | ---------------------------------------------------------------------------------|
| Выходной псевдоним        | Понятное имя, которое используется в запросах для направления выходных данных запроса в хранилище BLOB-объектов. |
...
| Имя свойства | Описание |
| --- | --- |
| Выходной псевдоним | Понятное имя, используемое в запросах для направления выходных данных запроса в концентратор событий. |
...
| Формат сериализации событий | Формат сериализации для выходных данных. Поддерживаются форматы JSON, CSV и Avro. |
| Сервис кодирования | В настоящее время единственным поддерживаемым форматом кодирования файлов CSV и JSON является UTF-8. |
| Разделитель | Применимо только для сериализации CSV-файлов. Stream Analytics поддерживает множество общепринятых разделителей для сериализации данных в формате CSV. Поддерживаются такие разделители: запятая, точка с запятой, пробел, табуляция и вертикальная черта. |
| Формат | Применяется только для сериализации JSON. **Строки-разделители** указывает, что выходные данные форматируются по каждый объект JSON, разделенных точкой с новой строки. **Массив** указывает, что выходные данные форматируются как массив объектов JSON. Этот массив будет закрыт только в том случае, если выполнение задания будет остановлено или Stream Analytics перейдет в следующее временное окно. Как правило это предпочтительнее использовать JSON, разделенных на строки, так как он не требует никакой специальной обработки, пока выходного файла по-прежнему записывается. |
...
1. Когда Power BI выбрана в качестве выходных данных на портале Azure, будет предложено авторизовать существующего пользователя Power BI или создать новую учетную запись Power BI.
...
2. Создайте учетную запись, если у вас еще нет один и затем выберите **авторизовать сейчас**. Появляется следующая страница:
...
3. Укажите рабочую или учебную учетную запись для авторизации выходных данных Power BI. Если вы все еще не зарегистрировались для Power BI, выберите **Зарегистрируйтесь сейчас**. Рабочая или учебная учетная запись, используемом для Power BI может отличаться от учетной записи подписки Azure, теперь вы вошли в систему.

### <a name="configure-the-power-bi-output-properties"></a>Настройка свойств выходных данных Power BI
После проверки подлинности учетной записи Power BI, можно настроить свойства для выходных данных Power BI. В следующей таблице перечислены имена свойств и их описания для настройки выходных данных Power BI.

| Имя свойства | Описание |
| --- | --- |
| Выходной псевдоним |Укажите понятное имя, которое используется в запросах для направления выходных данных запроса в этих выходных данных Power BI. |
...
Пошаговое руководство по настройке выходных данных Power BI и панели мониторинга, см. в разделе [Azure Stream Analytics и Power BI](stream-analytics-power-bi-dashboard.md) статьи.
...
Предыдущий и текущий | Int64 | String | DateTime | Double
-----------------|-------|--------|----------|-------
...
Если пароль учетной записи Power BI изменяется после создания или последней проверки подлинности к заданию Stream Analytics, необходимо выполнить проверку подлинности, Stream Analytics. Если в клиенте Azure Active Directory (AAD) настроена Многофакторная идентификация Azure, вам потребуется каждые две недели обновлять авторизацию Power BI. Признаком этой проблемы является отсутствие выходных данных задания и наличие записи «Ошибка проверки подлинности пользователя» в журналах операций:
...
| Имя свойства | Описание |
| --- | --- |
| Выходной псевдоним |Понятное имя, которое используется в запросах для направления выходных данных запроса в табличное хранилище. |
| Учетная запись хранения |Имя учетной записи хранения, где вы отправляете ваши выходные данные. |
...
| Имя свойства | Описание |
| --- | --- |
| Выходной псевдоним |Понятное имя, используемое в запросах для направления выходных данных запроса в этом разделе служебной шины. |
...
| Имя свойства | Описание |
| --- | --- |
| Выходной псевдоним | Псевдоним для ссылки на эти выходные данные в запросе Stream Analytics. |
...
| Имя свойства | Описание |
| --- | --- |
| Приложение функций |Имя приложения функций Azure. |
...
| Максимальный размер пакетов |Свойство, которое позволяет задать максимальный размер каждого пакета выходных данных, передаваемый функции Azure. Объем входных данных задается в байтах. По умолчанию это значение равно 262 144 байт (256 КБ). |
| Максимальное число пакетов  |Свойство, позволяет указать максимальное число событий в каждом пакете, который отправляется в функциях Azure. По умолчанию используется значение 100. |
...
| Раздел служебной шины Azure | Да | Выбран автоматически. Количество разделов [основано на размере и номере SKU служебной шины](../service-bus-messaging/service-bus-partitioning.md). Ключ секции — это уникальное целое значение для каждой секции.| Совпадает с количеством секций в разделе выходных данных.  |
| Очередь служебной шины Azure | Да | Выбран автоматически. Количество разделов [основано на размере и номере SKU служебной шины](../service-bus-messaging/service-bus-partitioning.md). Ключ секции — это уникальное целое значение для каждой секции.| Совпадает с количеством секций в очереди выходных данных. |
| Azure Cosmos DB | Да | Используется маркер {partition} в шаблон имени коллекции. Значение {partition} основано на предложении PARTITION BY в запросе. | Соответствует секционированию входных данных для [полностью распараллеливаемых запросов](stream-analytics-scale-jobs.md). |
...
| Тип вывода | Максимальный размер сообщения | Оптимизация размера пакета |
| :--- | :--- | :--- |
| Azure Data Lake Store | См. в разделе [ограничения хранилища Озера данных](../azure-subscription-service-limits.md#data-lake-store-limits). | Используйте до 4 МБ в операции записи. |
| База данных SQL Azure | 10 000 максимальное число строк в одной инструкции bulk insert.<br />100 минимальному числу строк для одной инструкции bulk insert. <br />См. в разделе [Azure SQL ограничивает](../sql-database/sql-database-resource-limits.md). |  Каждый пакет изначально является неполным, вставленные с помощью максимальный размер пакета. Вы можете разделить пакетной службы в два раза (пока не дойдете до размера минимальное пакета) в зависимости от ошибки, допускающие повторение, от SQL. |
| Хранилище BLOB-объектов Azure | См. в разделе [ограничения службы хранилища Azure](../azure-subscription-service-limits.md#storage-limits). | Размер блока максимальное BLOB-объектов — 4 МБ.<br />Число bock максимальное BLOB-объектов — 50 000. |
