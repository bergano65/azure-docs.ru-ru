---
title: Общие сведения о брандмауэре веб-приложения (WAF) на Шлюзе приложений Azure
...
Веб-приложения все чаще подвергаются вредоносным атакам, использующим общеизвестные уязвимости. Повсеместно используются уязвимости для атак путем внедрения кода SQL и межсайтовых сценариев, и это лишь немногие из них. 
...
Брандмауэр веб-приложения обеспечивает такие возможности на основе правил из [основных наборов правил OWASP](https://www.owasp.org/index.php/Category:OWASP_ModSecurity_Core_Rule_Set_Project) версии 3.0 или 2.2.9. Он автоматически обновляется, чтобы включить защиту от новых уязвимостей, без дополнительной настройки.

![imageURLroute](./media/waf-overview/WAF1.png)

Шлюз приложений работает как контроллер доставки приложений и предлагает функции моста SSL, сходство сеансов на основе файлов cookie, распределение нагрузки по методу циклического перебора, маршрутизацию на основе содержимого, возможность размещения нескольких веб-сайтов и повышенную безопасность.

К усовершенствованиям системы безопасности Шлюза приложений относятся управление политиками SSL и поддержка сквозного режима SSL-подключений. Теперь защита приложений усилена за счет прямой интеграции WAF (брандмауэра веб-приложения) с предложением ADC. Это позволяет легко настроить централизованное управление и защиту веб-приложений от распространенных сетевых уязвимостей.
...
- Запросить размер ограничения — брандмауэр веб-приложения позволяет пользователям задавать предельные размеры запроса в нижнюю и верхнюю границы.
- Списки исключений - списки исключений WAF разрешить пользователям исключать определенные атрибуты запроса на основе оценки WAF. Распространенный пример — вставленные токены Active Directory, которые используются для проверки подлинности или полей пароля.
...
См. дополнительные сведения о [списке правил и соответствующих возможностях защиты](#core-rule-sets).
...
* **Режим обнаружения**. В этом режиме WAF Шлюза приложений обнаруживает и регистрирует все угрозы, сохраняя оповещения в файле журнала. Для шлюза приложений в разделе **Диагностика** нужно включить ведение журнала диагностики. Кроме того, нужно убедиться, что журнал WAF выбран и включен. В режиме обнаружения брандмауэр веб-приложения не блокирует входящие запросы.
...
OWASP имеет два режима решения, ли блокирует трафик или нет. Нет, традиционный режим и режимом оценки аномалий. В стандартном режиме независимо от других правил соответствует ли слишком считается любое правило соответствующий трафик. Хотя проще для понимания, отсутствия на количество правил запускаются конкретного запроса является одним из ограничений в этом режиме. Таким образом был введен режим оценки аномалий, которого стала по умолчанию с OWASP 3.x. 

В режиме оценки аномалий тот факт, что одна из правил, описанных в предыдущем разделе соответствует трафик не означает немедленно что трафик будет блокироваться, при условии, что брандмауэр находится в режиме предотвращения. Правила имеют определенного уровня важности (критические, ошибок, предупреждения и уведомления), и в зависимости от серьезности, но приводят к увеличению числовое значение для запроса вызывается показатель аномалии. Например одно правило сопоставления предупреждение будет передавать значение 3, но одно правило сопоставления критических будет передавать значение 5. 

Нет пороговое значение для показателя аномалий, под которой трафик не удаляется, что пороговое значение — 5. Это означает, одно правило сопоставления критических достаточно, чтобы Azure WAF блокирует запрос в режиме предотвращения (поскольку правило критического показатель аномалии увеличивается на 5 в соответствии с выше). Тем не менее одно правило с уровнем предупреждение будет только увеличивать аномалии оценка на 3. Так как 3 по-прежнему меньше, чем пороговое значение 5, трафик не должны блокироваться, даже если брандмауэр веб-приложения находится в режиме предотвращения. 

Обратите внимание на то, что сообщение в журнале при трафика совпадения правила WAF будет включать action_s поле «Заблокировано», но это не обязательно означает, что трафик фактически была заблокирована. Оценку аномалии, 5 или более поздней версии требуется даже блокировать трафик.  

### <a name="application-gateway-waf-reports"></a>Мониторинг WAF

Важно наблюдение за работоспособностью шлюза приложений. Мониторинг работоспособности брандмауэра веб-приложения и приложений, которые он защищает, обеспечивается за счет ведения журналов и интеграции с Azure Monitor, центром безопасности Azure и журналами Azure Monitor.

![диагностика](./media/waf-overview/diagnostics.png)

#### <a name="azure-monitor"></a>Azure Monitor

Каждый журнал шлюза приложений интегрирован с [Azure Monitor](../monitoring-and-diagnostics/monitoring-overview.md).  Это позволяет отслеживать диагностические сведения, включая журналы и оповещения WAF.  Эта возможность предоставляется в ресурсе шлюза приложений на портале на вкладке **Диагностика** или напрямую через службу Azure Monitor. См. дополнительные сведения о [включении журналов диагностики для Шлюза приложений](application-gateway-diagnostics.md).

#### <a name="azure-security-center"></a>Центр безопасности Azure

[Центр безопасности Azure](../security-center/security-center-intro.md) помогает вам выявлять и предотвращать угрозы, а также принимать ответные меры благодаря более полной информации о состоянии ресурсов Azure и контролю над их безопасностью. Шлюз приложений теперь [интегрируется в центр безопасности Azure](application-gateway-integration-security-center.md). Центр безопасности Azure проверяет среды для обнаружения незащищенных веб-приложений. Теперь он может рекомендовать WAF шлюза приложений для защиты таких уязвимых ресурсов. WAF шлюза приложений можно создать непосредственно в центре безопасности Azure.  Эти экземпляры WAF интегрируются с центром безопасности Azure. Они будут отправлять предупреждения и сведения о работоспособности в центр безопасности Azure для создания отчетов.

![Рисунок 1](./media/waf-overview/figure1.png)
