...
Фильтрация подключений поможет обеспечить корректное удаление элементов серверного пула на протяжении запланированных обновлений службы. Этот параметр включается через параметры HTTP серверной части и может применяться ко всем элементам серверного пула при создании правила. При использовании включенного Шлюза приложений все снятые с регистрации экземпляры серверного пула не получат новых запросов, а существующие запросы будут выполняться в течение установленного времени. Это относится к экземплярами серверного пула, которые явно удалены из него путем изменения пользовательской конфигурации, и которые после проверки работоспособности определены как неработоспособные. Единственным исключением из этого являются запросы, которые используются для отмены регистрации экземпляров, отмененных явным образом, из-за сходства сеансов, управляемых шлюзом, и которые будут и дальше использоваться для отмены регистрации экземпляров.

Дополнительные сведения см. в разделе Connection Draining (Фильтрация подключений) в статье [Application Gateway configuration overview](https://docs.microsoft.com/azure/application-gateway/configuration-overview#connection-draining) (Обзор настройки Шлюза приложений).
