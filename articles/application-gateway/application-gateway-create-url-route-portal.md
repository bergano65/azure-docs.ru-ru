...
2. Выберите **Windows Server 2016 Datacenter** в списке популярных.
3. Введите следующие значения для виртуальной машины:

    - **Группа ресурсов**, выберите **создать**и введите *myResourceGroupAG*.
    - **Имя виртуальной машины**: *myVM1*
    - **Регион**. *СОВМЕСТ Восточная часть США*
    - **Имя пользователя**: *azureuser*
    - **Пароль**. *Azure123456!* —


4. Выберите **Next:Disks** (Далее: диски).
5. Выберите **Далее: сети.**
6. В поле **Виртуальная сеть**выберите **создать** и введите следующие значения для виртуальной сети:

   - *myVNet* — имя виртуальной сети;
   - *10.0.0.0/16* — диапазон адресов виртуальной сети;
   - *myBackendSubnet* для имени первой подсети
   - *10.0.1.0/24* — диапазон адресов подсети.
   - *myAGSubnet* — для второго имени подсети.
   - *10.0.0.0/24* — диапазон адресов подсети.
7. Нажмите кнопку **ОК**.

8. Убедитесь, что в разделе **сетевой интерфейс**выбран параметр **myBackendSubnet** для подсети, а затем **нажмите кнопку Далее. Управление**.
9. Нажмите кнопку **Отключить**, чтобы выключить диагностику загрузки.
10. Щелкните **Проверка + создать**, просмотрите параметры на странице Сводка и нажмите кнопку **создать**.
...
1. На вкладке **Основные сведения** введите значения для следующих параметров шлюза приложений.
   ...
   - **Регион** — выберите **(США) Восточная часть США**.
        ...
        ![Создание шлюза приложений: Основные сведения](./media/application-gateway-create-gateway-portal/application-gateway-create-basics.png)
...
3. Примите значения по умолчанию для других параметров, а затем **нажмите кнопку Далее. интерфейсные серверы**.
...
5. В разделе **Цель** выберите сетевой интерфейс для **myVM1**.
6. Выберите **Добавить**.
7. Повторите, чтобы добавить серверный пул *изображений* с *myVM2* в качестве целевого объекта и серверный пул *видео* с *myVM3* в качестве целевого объекта.
8. Нажмите кнопку **Добавить** , чтобы сохранить конфигурацию серверного пула и вернуться на вкладку "Обратная **Вставка** ".
...
8. В качестве **пути**введите */Images/* \*.
9. В качестве **имени правила пути**введите *Images*.
10. Для **параметра HTTP**выберите **михттпсеттинг** .
11. Для **внутреннего целевого объекта**выберите **изображения**.
12. Нажмите кнопку **Добавить** , чтобы сохранить правило пути и вернуться на вкладку **Добавление правила маршрутизации** .
13. Повторите, чтобы добавить еще одно правило для видео.
14. Выберите **Добавить** , чтобы добавить правило маршрутизации и вернуться на вкладку **Конфигурация** .
15. По завершении выберите **Next: Теги** , а затем **Далее: Отзыв и создание**.

> [!NOTE]
> Для работы с вариантами по умолчанию */* не нужно добавлять настраиваемое правило * для пути. Это автоматически обрабатывается внутренним пулом по умолчанию.
...
2. Скопируйте общедоступный IP-адрес и вставьте его в адресную строку браузера. Например, http:\//52.188.72.175:8080.
   ...
   Прослушиватель на порте 8080 направляет этот запрос в внутренний пул по умолчанию.

3. Измените URL-адрес *на&lt;http://IP-&gt;Address: 8080/Images/Test.htm*, &lt;заменив IP&gt; -адрес на свой IP-адрес, и вы увидите нечто вроде следующего:
   ...
   Прослушиватель на порте 8080 направляет этот запрос в серверный пул *образов* .

4. Измените URL-адрес *на&lt;http://IP-&gt;Address: 8080/Video/Test.htm*, &lt;заменив IP&gt; -адрес на свой IP-адрес, и вы увидите нечто вроде следующего:
   ...
   Прослушиватель на порте 8080 направляет этот запрос в серверный пул *видео* .


## <a name="next-steps"></a>Следующие шаги

- [Включение сквозного режима SSL в шлюзе приложений Azure](application-gateway-backend-ssl.md)
