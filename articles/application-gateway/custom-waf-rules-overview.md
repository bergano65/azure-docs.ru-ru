...
ms.locfileid: "73027580"
---
# <a name="overview-custom-rules-for-web-application-firewall-v2"></a>Обзор: настраиваемые правила для брандмауэра веб-приложения версии 2
...
> [!NOTE]
> Настраиваемые правила недоступны в версии 1 SKU WAF.
...
Чтобы разрешить что-либо, убедитесь, что параметр `-Action` имеет значение **allow**. Чтобы заблокировать что-либо, убедитесь, что параметр `-Action` установлен в **Block**, как показано в следующем коде:
...
- Журнал: позволяет записать правило в журнал и позволяет выполнять оставшиеся правила для оценки. Последующие пользовательские правила оцениваются в порядке приоритета, за которыми следуют управляемые правила.
