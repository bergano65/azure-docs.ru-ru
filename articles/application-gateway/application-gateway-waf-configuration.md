...
Следующие атрибуты могут добавляться в список исключений. Значения выбранного поля не оцениваются относительно правил WAF. Удаление проверки значения поля списки исключений.

* Заголовки запроса
* Объекты cookie запросов
* Имя атрибута запроса (args) можно добавить в качестве исключаемого элемента, например:

   * Имя поля формы
   * XML-сущности
   * Сущность JSON
   * Аргументы строки запроса URL-адрес
