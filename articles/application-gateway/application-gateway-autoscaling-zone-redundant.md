...
- **Key Vault Integration**: Application Gateway v2 поддерживает интеграцию с Key Vault для серверных сертификатов, которые прилагаются к httpS включенным слушателям. Для получения дополнительной [TLS termination with Key Vault certificates](key-vault-certs.md)информации см.
...
- **Повышение производительности**: V2 SKU предлагает до 5X лучше TLS разгрузки производительность по сравнению со стандартом / WAF SKU.
...
Standard_v2 и WAF_v2 SKU доступны в следующих регионах: Северная Центральная ЧАСТЬ США, южная центральная часть США, Западная ЧАСТЬ США, Западная ЧАСТЬ США, Западная ЧАСТЬ США 2, Центральная часть США, Северная Европа, Западная Европа, юго-восточная Азия, Франция Центральная, Запад Великобритании, Япония Восток, Япония Западная, Австралия Восток, Австралия, Южная, Бразилия, Центральная Часть, Канада Восток, Восточная Азия, Центральная, Корея , Великобритания на юге, Центральной Индии, Западной Индии, южной Индии.
...
Standard_v2 Прикладного шлюза предоставляется в течение месяца, с нулевым исходящими минимальными экземплярами, и за это время он получает 25 новых подключений TLS/sec, в среднем 8,88-Мбит/с передачи данных. Предполагая, что соединения недолговечны, ваша цена будет:
...
Standard_v2 прикладного шлюза готовится в течение месяца, с минимум пятью экземплярами, но на этот раз происходит в среднем 125 Мбит/с передачи данных и 25 подключений TLS в секунду. Предполагая, что нет трафика и соединения недолговечны, ваша цена будет:
...
Заявка шлюза WAF_v2 предоставляется в течение месяца. За это время он получает 25 новых подключений TLS/sec, в среднем 8,88-Мбит/с передачи данных и делает 80 запросов в секунду. Предполагая, что соединения недолговечны, и что расчетный удельный расчет приложения поддерживает 10 RPS на вычислительную единицу, ваша цена будет:
...
| Прекращение безопасности транспортного слоя (TLS)/Безопасные розетки Слой (SSL)            | &#x2713; | &#x2713; |
...
|Различие|Сведения|
|--|--|
|Сертификат аутентификации|Не поддерживается.<br>Для получения дополнительной информации смотрите [Обзор конца до конца TLS с помощью шлюза приложений](ssl-overview.md#end-to-end-tls-with-the-v2-sku).|
