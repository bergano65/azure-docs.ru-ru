...
Номера SKU Standard_v2 и WAF_v2 доступны в следующих регионах: Северо-центральный регион США, Юго-Центральный регион, Западная часть США, Запад США 2, восток США, Западная часть США 2, Центральная часть США, Северная Европа, Западная Европа, Юго-Восточная Азия, Центральная французская, западная часть Соединенного Королевства, Восточная Япония, Западная Япония, Восточная Австралия , Юго-Восточная Австралия, Южная Бразилия, Центральная Канада, Восточная Канада, Восточная Азия, Центральная Корея, Южная Корея, Южная Индия, южная часть Соединенного Королевства, Центральная Индия, Западная Индия, Южная Индия.
...
Standard_v2 шлюза приложений подготавливается в течение месяца, с 0 минимальными экземплярами, и в течение этого времени будет получено 25 новых SSL-подключений/с, в среднем — 8,88 Мбит/с. Если подключения имеют небольшой срок действия, Ваша цена будет:
...
Как видите, счет выставляется только за 4 единицы мощности, а не для всего экземпляра. 
...
Standard_v2 шлюза приложений подготавливается в течение месяца, но не менее 5 экземпляров. Если нет времени существования трафика и соединений, Ваша цена будет:
...
В этом случае выставляется счет за все 5 экземпляров, даже если трафик отсутствует.

**Пример 4**

Standard_v2 шлюза приложений подготавливается в течение месяца, в котором не менее 5 экземпляров, но на этот раз в среднем 125 Мбит/с и 25 подключений SSL в секунду. Если нет времени существования трафика и соединений, Ваша цена будет:
...
В этом случае вам выставляются счета за все 5 экземпляров, а также 7 единиц мощности (7/10 экземпляра).  
