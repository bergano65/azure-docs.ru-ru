...
Каждая единица емкости включает не более 1 единицы вычислений, 2500 постоянных подключений или 2,22 Мбит/с пропускной способности.
...
> Количество запросов, которые может обработать единица вычислений, зависит от различных критериев, таких как размер ключа сертификата TLS, алгоритм обмена ключами, перезапись заголовков и, в случае WAF, размера входящего запроса. Рекомендуем выполнить тесты приложений для определения частоты запросов на единицу вычислений. Единица емкости и единица вычислений будут доступны в качестве метрик перед началом выставления счетов.

В следующей таблице показаны примеры цен (они приведены только для наглядности).

**Цены в восточной части США**

|              Имя SKU                             | Фиксированная цена ($/ч)  | Цена за единицу емкости ($/ч ед. емкости)   |
| ------------------------------------------------- | ------------------- | ------------------------------- |
| Standard_v2                                       |    0,20             | 0,0080                          |
| WAF_v2                                            |    0,36             | 0,0144                          |

Дополнительные сведения о ценах см. на [странице цен](https://azure.microsoft.com/pricing/details/application-gateway/). 

**Пример 1**

Шлюз приложений Standard_v2 подготавливается без автомасштабирования в режиме ручного масштабирования с фиксированной емкостью в пять экземпляров.

Фиксированная цена = 744 (ч) * $0,20 = $148,8 <br>
Цена за единицы емкости = 744 (ч) * 10 ед. емкости на экземпляр * 5 экземпляров * $0,008 на ч ед. емкости = $297,6

Общая цена = $148,8 + $297,6 = $446,4

**Пример 2**

Шлюз приложений Standard_v2 подготавливается в течение месяца с нулевым минимальным количеством экземпляров, и в течение этого времени он получает по 25 новых TLS-подключений в секунду со средней пропускной способностью 8,88 Мбит/с. При условии небольшой длительности подключений цена будет следующей:

Фиксированная цена = 744 (ч) * $0,20 = $148,8

Цена за единицы емкости = 744 (ч) * максимум (25/50 ед. вычислений для подключений/с, 8,88/2,22 ед. емкости для пропускной способности) * $0,008 = 744 * 4 * 0,008 = $23,81

Общая цена = $148,8+23,81 = $172,61

Как видите, счет выставляется только для четыре единицы емкости, а не за весь экземпляр. 

> [!NOTE]
> Функция "максимум" возвращает наибольшее значение в паре значений.


**Пример 3**

Шлюз приложений Standard_v2 подготавливается в течение месяца и использует не менее пяти экземпляров. При условии отсутствия трафика и небольшой длительности подключений цена будет следующей:

Фиксированная цена = 744 (ч) * $0,20 = $148,8

Цена за единицы емкости = 744 (ч) * максимум (0/50 ед. вычислений для подключений/с, 0/2,22 ед. емкости для пропускной способности) * $0,008 = 744 * 50 * 0,008 = $297,60

Общая цена = $148,80 + 297,60 = $446,4

В этом случае оплачиваются все пять экземпляров, несмотря на отсутствие трафика.

**Пример 4**

Шлюз приложений Standard_v2 подготавливается в течение месяца и использует минимум пять экземпляров, но на этот раз он получает по 25 TLS-подключений в секунду со средней пропускной способностью 125 Мбит/с. При условии отсутствия трафика и небольшой длительности подключений цена будет следующей:

Фиксированная цена = 744 (ч) * $0,20 = $148,8

Цена за единицы емкости = 744 (ч) * максимум (25/50 ед. вычислений для подключений/с, 125/2,22 ед. емкости для пропускной способности) * $0,008 = 744 * 57 * 0,008 = $339,26

Общая цена = $148,80 + 339,26 = $488,06

В этом случае оплачиваются все пять экземпляров, а также семь единиц емкости (что составляет 7/10 экземпляра).  

**Пример 5**

Шлюз приложений WAF_v2 подготавливается в течение месяца. В течение этого времени он получает по 25 новых TLS-подключений в секунду со средней пропускной способностью 8,88 Мбит/с и отправляет по 80 запросов в секунду. При условии небольшой длительности подключений и поддержки 10 RPS на единицу вычислений при расчете единиц вычислений для приложения цена будет следующей:

Фиксированная цена = 744 (ч) * $0,36 = $267,84

Цена за единицы емкости = 744 (ч) * максимум (ед. вычислений максимум (25/50 для подключений/с, 80/10 WAF RPS), $8,88/2,22 ед. емкости для пропускной способности) * $0.0144 = 744 * 8 * 0,0144 = $85,71
...
|Маршрут, определяемый пользователем в подсети Шлюза приложений|Поддерживается (конкретные сценарии). Предварительная версия.<br> Дополнительные сведения см. в статье [Обзор настройки Шлюза приложений](configuration-overview.md#user-defined-routes-supported-on-the-application-gateway-subnet).|
