...
Внутренний пул используется для перенаправления запросов на внутренних серверах, на которых будет обслуживать запрос. Серверной части могут состоять из сетевых адаптеров, масштабируемых наборов виртуальных машин, общедоступных IP-адресов, внутренних IP-адресов, полное доменное имя (FQDN) и мультитенантные серверные решения как служба приложений Azure. В этом примере мы будем использовать виртуальные машины как целевой серверной части. Мы можно использовать существующие виртуальные машины или создать новые. В этом примере мы создадим две виртуальные машины, Azure использует в качестве внутренних серверов для шлюза приложений. Чтобы сделать это, мы выполним следующее.

1. Создание новой виртуальной Машиной, *myVM* и *myVM2*, который будет использоваться в качестве внутренних серверов.
2. Установите службы IIS на виртуальных машинах, чтобы убедиться, что шлюз приложений успешно создан.
3. Добавьте внутренний пул внутренних серверов.
