...
- **Несколько сайтов**. Эта конфигурация прослушивателя необходима при настройке нескольких веб-приложений на одном экземпляре шлюза приложений. Он позволяет настроить более эффективную топологию для развертываний, добавив до 100 веб-сайтов в один шлюз приложений. Каждый веб-сайт может быть направлен в свой собственный внутренний пул. Например, три поддомены, abc.contoso.com, xyz.contoso.com и pqr.contoso.com, указывают на IP-адрес шлюза приложений. Создайте три прослушивателя с несколькими сайтами и настройте каждый прослушиватель для соответствующего порта и параметра протокола.
    ...
    Дополнительные сведения см. в разделе [Размещение нескольких сайтов](application-gateway-web-app-overview.md).
...
Шлюз приложений обрабатывает прослушиватели в указанном [порядке](configuration-overview.md#order-of-processing-listeners).
...
Дополнительные сведения см. [в статье перезапись заголовков HTTP в шлюзе приложений](rewrite-http-headers.md).
...
- Определите, должен ли сеанс пользователя храниться на том же сервере с помощью [сходства сеансов на основе файлов cookie](overview.md#session-affinity).

- Аккуратное удаление членов серверного пула с помощью функции [сток подключений](overview.md#connection-draining).
