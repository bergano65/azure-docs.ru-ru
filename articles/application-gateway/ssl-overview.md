...
> [!NOTE] 
>
> Чтобы SSL-сертификат был доверенным, этот сертификат должен быть выдан центром сертификации, включенным в доверенное хранилище шлюза разрешение. Если сертификат не был выдан доверенным центром сертификации, шлюз приложений будет проверять Убедитесь, что сертификат выдающего ЦС выдан доверенным центром сертификации, и так далее, пока не обнаружится доверенный центр сертификации (при этом не будет обнаружено надежное безопасное подключение) или не удастся найти доверенный ЦС (в этом случае шлюз разрешение пометит серверную унхе АЛСИ). Поэтому рекомендуется, чтобы сертификат внутреннего сервера содержал как корневой, так и интермидиате ЦС.
