...
1. Измените файл [Helm-config. YAML](#sample-helm-config-file) одним из следующих способов.
   - Удалите ключ `watchNamespace` целиком из [Helm-config. YAML](#sample-helm-config-file) -агик будет наблюдать за всеми пространствами имен
...
Например, рассмотрим следующие дублирующиеся входные ресурсы `staging` , определенные пространства имен, `production` и `www.contoso.com`для:
```yaml
...
Несмотря на два входящих ресурса, требующих направлять `www.contoso.com` трафик в соответствующие пространства имен Kubernetes, только одна серверная часть может обслуживать трафик. АГИК будет создавать конфигурацию для одного из ресурсов на основе первых поступающих. Если одновременно создаются два передает ресурсов, приоритет будет иметь один из них, описанный ранее в алфавите. В приведенном выше примере мы сможем только создавать параметры для `production` входящего трафика. Шлюз приложений будет настроен со следующими ресурсами:
...
Обратите внимание, что за исключением правила *прослушивателя* и *маршрутизации*, созданные ресурсы шлюза приложений включают имя`production`пространства имен (), для которого они были созданы.
...
Например, если вы добавили `staging` сначала, агик настроит шлюз приложений для маршрутизации трафика в промежуточный пул внутренних серверов. На более позднем этапе, `production` посвященном входным параметрам, будет агик перепрограммировать шлюз приложений, который начнет маршрутизацию `production` трафика во внутренний пул.
...
По умолчанию АГИК будет настраивать шлюз приложений на основе пометки входных данных в любом пространстве имен. Если вы хотите ограничить это поведение, вы можете использовать следующие параметры:
  - Ограничьте пространство имен, явно определив пространства имен АГИК, `watchNamespace` с помощью ключа YAML в [Helm-config. YAML](#sample-helm-config-file)
