...
- Необходимо включить исключения для входящего трафика на портах 65503-65534 для SKU шлюза приложений версии 1 и портов 65200-65535 для SKU v2. Такой диапазон портов требуется для обмена данными в рамках инфраструктуры Azure. Эти порты защищены (заблокированы) сертификатами Azure. Внешние сущности, включая клиентов этих шлюзов, не могут инициировать изменения на этих конечных точках без соответствующих сертификатов.
...
1. Разрешите входящий трафик из диапазона IP-адресов источника или IP-адреса.
...
При создании нового прослушивателя выбирается режим " [ *базовый* " и](https://docs.microsoft.com/azure/application-gateway/application-gateway-components#types-of-listeners)"многосайтовый".
...
- Выберите базовый, если хотите перенаправить все запросы к связанному прослушивателю (например *,<i></i>blog.\*contoso.com/)* в один пул серверной части.
...
Выберите необходимый тип перенаправления: *Постоянный (301)* , *временный (307)* , *найденный (302*) или *другой (303)* .
...
Этот параметр добавляет, удаляет или обновляет заголовки HTTP-запросов и ответов во время перемещения пакетов запросов и ответов между клиентом и внутренним пулом. Эту возможность можно настроить только с помощью PowerShell. Поддержка портал Azure и CLI пока недоступна. Дополнительные сведения можно найти в разделе
...
### <a name="connection-draining"></a>Очистка подключений
...
### <a name="request-timeout"></a>Время ожидания запроса истекло
...
### <a id="pick"/></a>Выберите имя узла на внутреннем адресе
