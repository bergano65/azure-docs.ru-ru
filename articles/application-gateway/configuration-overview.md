...
Этот параметр добавляет, удаляет или обновляет заголовки HTTP-запросов и ответов во время перемещения пакетов запросов и ответов между клиентом и внутренним пулом. Дополнительные сведения см. в разделе:
 ...
 - [Общие сведения о перезаписи HTTP-заголовков](rewrite-http-headers.md)
...
Начиная с **17 февраля 2020**, обновление [Chromium](https://www.chromium.org/Home) [V80](https://chromiumdash.appspot.com/schedule) предоставляет требование, где файлы cookie HTTP без атрибута SameSite обрабатываются как SameSite = нестрогие. В случае запросов CORS (совместное использование ресурсов независимо от источника), если файл cookie должен быть отправлен в контексте стороннего разработчика, он должен использовать "SameSite = None; Защита "атрибутов и должна отправляться только по протоколу HTTPS. В противном случае в сценарии HTTP браузер не будет передавать файлы cookie в контексте стороннего разработчика. Целью этого обновления из Chrome является повышение безопасности и предотвращение атак с подделкой межсайтовых запросов (CSRF). 

Для поддержки этого изменения шлюз приложений (все типы SKU) будет внедрять другой идентичный файл cookie с именем **аппликатионгатевайаффинитикорс** в дополнение к существующему файлу cookie **аппликатионгатевайаффинити** , который похож, но теперь этот файл cookie будет иметь еще два атрибута **"SameSite = None;** В него добавляется защита, чтобы можно было поддерживать сеанс прикрепленного сеанса даже для запросов между источниками.

Обратите внимание, что имя файла cookie сопоставления по умолчанию — **аппликатионгатевайаффинити** , и оно может быть изменено пользователями. Если вы используете имя файла cookie для пользовательского сходства, то добавляется дополнительный файл cookie с CORS в качестве суффикса, например **кустомкукиенамекорс**.

> [!NOTE]
> Если атрибут **SameSite = None** установлен, он также должен содержать флаг **Secure** и должен отправляться по **протоколу HTTPS**. Поэтому, если для CORS требуется сходство сеансов, необходимо перенести рабочую нагрузку в HTTPS. Сведения о разгрузке SSL и сквозной документации SSL для шлюза приложений см. в статье [Обзор](ssl-overview.md), [Настройка разгрузки SSL](create-ssl-portal.md), [Настройка сквозного протокола SSL](end-to-end-ssl-portal.md).
...
Этот параметр в сочетании с протоколом HTTPS в прослушивателе поддерживает [сквозной протокол SSL](ssl-overview.md). Это позволяет безопасно передавать конфиденциальные данные, зашифрованные в серверной части. Для каждого серверного сервера в пуле серверной части, для которого включено сквозное подключение SSL, необходимо настроить сертификат, чтобы обеспечить безопасную связь.
