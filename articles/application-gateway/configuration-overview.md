...
На этом рисунке показано приложение с тремя прослушивателями. Первые два являются прослушивателями с несколькими сайтами для `http://acme.com/*` и `http://fabrikam.com/*`соответственно. Прослушивать порт 80. Третий — базовый прослушиватель, который содержит сквозное завершение SSL (SSL).
...
1. Разрешите входящий трафик из исходного IP-адреса или диапазона IP-адресов и назначения в качестве всей подсети шлюза приложений или на конкретный настроенный частный IP-адрес внешнего интерфейса. NSG не работает на общедоступном IP-адресе.
...
- Если требуется перенаправить запросы к различным внутренним пулам на *основе заголовка или имени узла,* выберите многосайтовый прослушиватель, где необходимо также указать имя узла, соответствующее входящему запросу. Это связано с тем, что шлюз приложений использует заголовки узлов HTTP 1,1 для размещения нескольких веб-сайтов на одном и том же общедоступном IP-адресе и порте.

#### <a name="order-of-processing-listeners"></a>Порядок обработки прослушивателей

Для номера SKU v1 запросы сопоставляются в соответствии с порядком правил и типом прослушивателя. Если правило с базовым прослушивателем сначала находится в порядке, оно обрабатывается первым и принимает любой запрос на это сочетание порта и IP-адреса. Чтобы избежать этого, сначала настройте правила с многосайтовыми прослушивателями и отправьте правило с базовым прослушивателем в последнюю из списка.
...
Политика SSL настраивается для управления версиями протокола SSL. Вы можете настроить шлюз приложений для использования версии протокола минимальное для подтверждений TLS из TLS 1.0, TLS 1.1 и TLS 1.2. По умолчанию SSL 2,0 и 3,0 отключены и не могут быть настроены. Дополнительные сведения см. в статье [Общие сведения о политике SSL шлюза приложений](https://docs.microsoft.com/azure/application-gateway/application-gateway-ssl-policy-overview).
...
Этот параметр добавляет, удаляет или обновляет заголовки HTTP-запросов и ответов во время перемещения пакетов запросов и ответов между клиентом и внутренним пулом. Дополнительные сведения см. здесь:

 - [Общие сведения о перезаписи HTTP-заголовков](https://docs.microsoft.com/azure/application-gateway/rewrite-http-headers)
 - [Настройка перезаписи HTTP-заголовка](https://docs.microsoft.com/azure/application-gateway/rewrite-http-headers-portal)
...
Сток подключений помогает аккуратно удалить членов пула серверной части во время плановых обновлений службы. Этот параметр можно применить ко всем элементам пула серверной части во время создания правила. Это гарантирует, что все отменяющие регистрацию экземпляров серверного пула продолжают поддерживать существующие подключения и обслуживать запросы в течение настраиваемого времени ожидания и не получают новые запросы или соединения. Единственным исключением из этого являются запросы, привязанные к экземплярам дерегистринг из-за сходства сеансов, управляемых шлюзом, и будут по-прежнему использоваться в экземплярах дерегистринг. Сток подключений применяется к внутренним экземплярам, которые явным образом удаляются из серверного пула.
...
### <a id="pick"/></a>выбрать имя узла на внутреннем адресе
...
Например, если в параметре **имени узла** указано *www.contoso.com* , то исходный запрос * https://appgw.eastus.cloudapp.azure.com/path1 меняется на * https://www.contoso.com/path1, когда запрос перенаправляется на сервер внутренней части.
