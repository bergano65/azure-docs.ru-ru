...
В случае SKU v1 определяемые пользователем маршруты (Udr) поддерживаются в подсети шлюза приложений, до тех пор, пока они не влияют на end-to-end запрос-ответ.
...
#### <a name="supported-certs"></a>Поддерживаемые сертификаты

Поддерживаются самозаверяющие сертификаты, сертификаты ЦС, групповые и сертификаты высокой Надежности.
...
- #### <a name="redirection-type"></a>Тип перенаправления
  ...
  Выберите тип перенаправления, требуемое от: Постоянный временной, найден "или" см. другие.
...
- Параметры HTTP при присоединении к правилу маршрутизации базовым запросом:
...
- При присоединении параметры HTTP для правила маршрутизации на основе путей запроса:
