...
Шлюз приложений использует один частный IP-адрес на каждый экземпляр, а также еще один, если настроен частный интерфейсный IP-адрес. Кроме того, Azure зарезервирует первые четыре и последний IP-адрес в каждой подсети для внутреннего использования. Например если шлюз приложений имеет значение три экземпляра и без частный внешний IP-адреса, по крайней мере восемь IP-адресов будет требуется в подсеть — пять IP-адресов для внутреннего использования и три IP-адреса для трех экземпляров шлюза приложений. Таким образом, в данном случае/29 требуется подсеть, размер или более поздней версии. Если у вас есть три экземпляра и IP-адрес частный внешний IP-конфигурации, то девять IP-адресов, которые будут необходимы - три IP-адреса для трех экземпляров шлюза приложений, один IP-адрес для частный интерфейсный IP-адрес и пять IP-адресов для внутреннего использования. Таким образом, в данном случае/28 требуется подсеть, размер или более поздней версии.
