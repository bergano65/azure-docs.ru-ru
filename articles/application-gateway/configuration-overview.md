...
- **Версия 2**
  ...
  **Сценарий 3**. UDR for Azure Kubernetes Service кубенет

  Если вы используете кубенет со службой Kubernetes Azure (AKS) и контроллером входящего трафика шлюза приложений (АГИК), необходимо настроить таблицу маршрутов, чтобы разрешить маршрутизацию трафика, отправляемого в модули Pod, на правильный узел. Это не потребуется, если вы используете Azure CNI. 

   Чтобы настроить таблицу маршрутизации, чтобы разрешить кубенет работу, выполните следующие действия.

  1. Создайте ресурс таблицы маршрутов в Azure. 
  2. После создания перейдите на страницу **маршруты** . 
  3. Добавить новый маршрут:
     ...
     - Адресом следующего прыжка должен быть IP-адрес узла, на котором размещаются модули, в диапазоне IP-адресов, определенном в поле префикс адреса. 
...
Этот параметр добавляет, удаляет или обновляет заголовки HTTP-запросов и ответов во время перемещения пакетов запросов и ответов между клиентом и внутренним пулом. Дополнительные сведения можно найти в разделе
...
### <a name="pick-host-name-from-back-end-address"></a><a id="pick"/></a>Выберите имя узла на внутреннем адресе
...
Например, если в *www.contoso.com* параметре **имени узла** указано www.contoso.com, то исходный запрос *`https://appgw.eastus.cloudapp.azure.com/path1` меняется на *`https://www.contoso.com/path1` , когда запрос перенаправляется на сервер фонового сервера.
