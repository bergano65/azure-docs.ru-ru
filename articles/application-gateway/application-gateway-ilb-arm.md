---
title: Использование с внутренним балансом нагрузки - Шлюз приложений Azure
...
* **Пул серверов бэк-энда:** Список IP-адресов серверов бэк-энда. Указанные IP-адреса должны относиться к одной виртуальной сети (но не входить в подсеть шлюза приложений) либо представлять собой общедоступные или виртуальные IP-адреса.
* **Настройки пула серверов бэк-энда:** Каждый пул имеет такие настройки, как порт, протокол и сродство на основе файлов cookie. Эти параметры привязываются к пулу и применяются ко всем серверам в этом пуле.
...
* **Слушатель:** Слушатель имеет передний порт, протокол (http или Https, они чувствительны к случаям) и имя сертификата SSL (при настройке SSL разгрузки).
* **Правило:** Правило связывает слушателя и пул сервера бэк-энда и определяет, на какой бэк-энд сервер должен быть направлен на трафик, когда он попадает к конкретному слушателю. В настоящее время поддерживается только *основное* правило. *Основное* правило предусматривает циклическое распределение нагрузки.
