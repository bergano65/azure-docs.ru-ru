...
Чтобы проверить работоспособность серверного пула, можно использовать страницу **Оценка работоспособности серверной части** на портале Azure. Можно также использовать [Azure PowerShell](https://docs.microsoft.com/powershell/module/az.network/get-azapplicationgatewaybackendhealth?view=azps-2.6.0), [интерфейс командной строки](https://docs.microsoft.com/cli/azure/network/application-gateway?view=azure-cli-latest#az-network-application-gateway-show-backend-health) или [REST API](https://docs.microsoft.com/rest/api/application-gateway/applicationgateways/backendhealth).
...
> [!NOTE]
> Запрос пробы по умолчанию отправляется в формате \<протокол\>://127.0.0.1:\<порт\>/. Например, для пробы HTTP на порте 80 используется запрос http://127.0.0.1:80. Только коды состояния HTTP от 200 до 399 соответствуют работоспособному состоянию. Протокол и порт назначения наследуются от параметров HTTP. Если требуется, чтобы Шлюз приложений выполнял пробу для другого протокола, имени узла или пути и определял другой код состояния как работоспособный, настройте пользовательскую пробу и свяжите ее с параметрами HTTP.
...
**Причина.** После установки TCP-подключения и выполнения подтверждения TLS (если включен протокол TLS) Шлюз приложений отправляет пробу в виде HTTP-запроса GET на внутренний сервер. Как было сказано ранее, проба по умолчанию имеет вид \<протокол\>://127.0.0.1:\<порт\>/ и считает коды состояния ответа от 200 до 399 как соответствующие работоспособному состоянию. Если сервер возвращает любой другой код состояния, он будет помечен как Unhealthy с помощью этого сообщения.
...
| **Error** | **Действия** |
| --- | --- |
| Несоответствие кода состояния пробы: Отображается ошибка 401 | Проверьте, требуется ли для внутреннего сервера аутентификация. На этом этапе пробы Шлюза приложений не могут передавать учетные данные для аутентификации. Либо разрешите код \"HTTP 401\" для состояния пробы, либо отправляйте пробу по пути, в котором сервер не требует аутентификации. | |
...
Чтобы создать пользовательскую пробу, выполните [эти действия](https://docs.microsoft.com/azure/application-gateway/application-gateway-create-probe-portal).
...
Узнайте больше [сопоставлении пробы Шлюза приложений](https://docs.microsoft.com/azure/application-gateway/application-gateway-probe-overview#probe-matching).
...
**Сообщение**. Сертификат сервера, используемый внутренним сервером, не подписан известным центром сертификации (ЦС). Добавьте в список разрешений серверную часть в Шлюзе приложений, передав корневой сертификат сертификата сервера, используемого серверной частью.
...
Дополнительные сведения о том, как извлечь и передать доверенные корневые сертификаты в Шлюз приложений, см. в разделе [Экспорт доверенного корневого сертификата (для SKU версии 2)](https://docs.microsoft.com/azure/application-gateway/certificates-for-backend-authentication#export-trusted-root-certificate-for-v2-sku).
...
1.  Проверьте, не блокирует ли группа безопасности сети доступ из **Интернета** к портам 65503–65534 (номер SKU версии 1) или 65200–65535 (SKU версии 2).
    ...
    е)  Выберите **Сохранить** и убедитесь, что серверная часть отображается в состоянии Healthy. Кроме того, это можно сделать с помощью [PowerShell или интерфейса командной строки](https://docs.microsoft.com/azure/virtual-network/manage-network-security-group).
...
Узнайте больше о [диагностике и ведении журнала для Шлюза приложений](https://docs.microsoft.com/azure/application-gateway/application-gateway-diagnostics).
