...
> [!NOTE]
> Запрос пробы по умолчанию отправляется в формате <protocol>://127.0.0.1:<port>/, например, <http://127.0.0.1/> для проверки HTTP на порту 80 и рассматривает только ответ от кодов состояния HTTP 200-399 в качестве работоспособного ответа. Протокол и порт назначения наследуются от параметров HTTP. Если требуется, чтобы шлюз приложений проверит наличие другого протокола, имени узла или пути и принимал другой код состояния как работоспособный, настройте пользовательскую проверку и свяжите ее с параметрами HTTP.
...
1.  Проверьте, можно ли подключиться к внутреннему серверу на порте, указанном в параметрах HTTP, с помощью браузера или PowerShell, например, можно использовать команду: Test-NetConnection-ComputerName www.bing.com-порт 443
