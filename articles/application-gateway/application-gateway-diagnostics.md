...
* [Метрики](application-gateway-metrics.md). Шлюз приложений имеет несколько метрик, которые помогают убедиться в том, что система работает должным образом.
...
|действие     |  Действие, выполняемое с запросом. Возможные значения: Blocked и Allowed.      |
