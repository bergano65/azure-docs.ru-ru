---
title: Задние медицинские и диагностические журналы
...
---
# <a name="back-end-health-and-diagnostic-logs-for-application-gateway"></a>Задние медицинские и диагностические журналы для шлюза приложения

Вы можете отслеживать ресурсы Azure Application Gateway следующим образом:
...
* [Метрики](application-gateway-metrics.md): Приложение шлюз имеет несколько метрик, которые помогут вам проверить, что ваша система работает как ожидалось.
...
> [!IMPORTANT]
> Если в подсети Application Gateway есть группа сетевой безопасности (NSG), то диапазоны открытых портов 65503-65534 для v1 SKUs и 65200-65535 для v2 SKUs в подсети Application Gateway для входящего трафика. Такой диапазон портов требуется для обмена данными в рамках инфраструктуры Azure. Они защищены (заблокированы) посредством сертификатов Azure. Без подходящих сертификатов внешние сущности, включая клиентов этих шлюзов, не смогут никоим образом изменить конечные точки.


### <a name="view-back-end-health-through-the-portal"></a>Просмотр данных о работоспособности серверной части на портале

На портале данные о работоспособности серверной части предоставляются автоматически. В существующем шлюзе приложения выберите **мониторинг** > **работоспособности Backend.**

На этой странице отображается каждый участник внутреннего пула (сетевой адаптер, IP-адрес или полное доменное имя). На ней показаны имя внутреннего пула, порт, параметры HTTP внутреннего пула и его состояние работоспособности. Допустимые значения для состояния здоровья **являются здоровыми,** **нездоровыми**и **неизвестными.**
...
|sslEnabled| Используется ли связь с пулами бэк-энда TLS/SSL. Допустимые значения: on и off.|
...
|receivedBytes     | Размер полученного пакета (в байтах).        |
|sentBytes| Размер отправленного пакета (в байтах).|
|timeTaken| Продолжительность времени (в **секундах),** что требуется для запроса, который будет обработан и его ответ будет отправлен. Вычисляется как промежуток времени от момента, когда шлюз приложений получает первый байт HTTP-запроса, до завершения отправки ответа. Важно отметить, что в поле Time-Taken обычно указывается время передачи пакетов запросов и ответов по сети. |
...
|sslProtocol| Используется протокол SSL/TLS (если включен TLS).|
...
|action     |  Действие, выполняемое с запросом. Доступные значения совпадают и блокируются.      |
