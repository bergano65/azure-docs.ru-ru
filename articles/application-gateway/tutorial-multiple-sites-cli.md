---
title: Размещение нескольких веб-сайтов с помощью CLI
...
Если вы решили установить и использовать CLI локально, эта статья требует, чтобы вы запускали версию Azure CLI 2.0.4 или позже. Чтобы узнать версию, выполните команду `az --version`. Если вам нужно установить или обновить, [см.](/cli/azure/install-azure-cli)
...
Добавьте пулы бэкэнда, необходимые для хранения серверов бэкэнда, используя [создание адресного пула az network-gateway](/cli/azure/network/application-gateway/address-pool#az-network-application-gateway-address-pool-create)
```azurecli-interactive
...
Правила обрабатываются в указанном порядке. Трафик направляется с использованием первого правила, которое соответствует независимо от специфики. Например, если для одного порта имеется правило с базовым прослушивателем и правило с многосайтовым прослушивателем, для обеспечения правильной работы правило с многосайтовым прослушивателем должно быть указано перед правилом с базовым прослушивателем. 

В этом примере можно создать два новых правила и удалить правило по умолчанию, созданное при развертывании шлюза приложения. Вы можете добавить правило с помощью команды [az network application-gateway rule create](/cli/azure/network/application-gateway/rule#az-network-application-gateway-rule-create).
...
Использование A-записей не рекомендуется, так как VIP может измениться при перезагрузке шлюза приложения.
