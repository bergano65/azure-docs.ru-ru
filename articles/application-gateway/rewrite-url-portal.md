...
4. Укажите имя для набора перезаписи и свяжите его с правилом маршрутизации:
    ...
    a. Введите имя набора перезаписи в поле **имя** .
    ...
    :::image type="content" source="./media/rewrite-url-portal/rewrite-url-portal-2.png" alt-text="Добавить набор перезаписи":::

5. Создайте правило перезаписи:

    a. Выберите **Добавить правило перезаписи**.
    
    :::image type="content" source="./media/rewrite-url-portal/rewrite-url-portal-3.png" alt-text="Добавить набор перезаписи":::
...
6. В этом примере мы будем перезаписывать URL-адрес и строку запроса URL-адреса, только если путь содержит */артикле*. Для этого добавьте условие, определяющее, содержит ли URL-путь */артикле* .
    ...
    a. Выберите **Добавить условие** , а затем выберите поле, содержащее инструкции **If** , чтобы развернуть его.
    ...
    :::image type="content" source="./media/rewrite-url-portal/rewrite-url-portal-4.png" alt-text="Добавить набор перезаписи":::
...
7. Добавление действия для перезаписи URL-адреса и пути URL-адреса
   ...
   е) Щелкните **ОК**.
    ...
    :::image type="content" source="./media/rewrite-url-portal/rewrite-url-portal-5.png" alt-text="Добавить набор перезаписи":::
...
9. Убедитесь, что новый набор перезаписи отображается в списке наборов перезаписи
    ...
    :::image type="content" source="./media/rewrite-url-portal/rewrite-url-portal-6.png" alt-text="Добавить набор перезаписи":::
