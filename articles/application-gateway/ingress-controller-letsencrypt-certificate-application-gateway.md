...
1. Диаграмма Helm
    ...
    Выполните следующий скрипт, чтобы установить диаграмму `cert-manager` Helm. В результате:
...
3. Развертывание приложения
    ...
    Создайте ресурс входящего трафика, чтобы предоставить `guestbook` приложению доступ к нему с помощью шлюза приложений с сертификатом.

    Убедитесь, что шлюз приложений имеет общедоступную IP-конфигурацию с DNS-именем (либо с `azure.com` помощью домена по умолчанию `Azure DNS Zone` , либо подготавливает службу, и назначьте собственный пользовательский домен).
    Обратите внимание `certmanager.k8s.io/cluster-issuer: letsencrypt-staging`на заметку, которая сообщает менеджеру CERT о необходимости обрабатывать ресурс с тегами.
...
4. Рабочий сертификат
    ...
    1. Удалите существующую промежуточную `ClusterIssuer` среду, созданную на предыдущем шаге, и создайте новую, заменив сервер ACME с клустериссуер YAML выше на`https://acme-v02.api.letsencrypt.org/directory`

5. Истечение срока действия сертификата и его продление

    До истечения срока `Lets Encrypt` действия `cert-manager` сертификата будет автоматически обновлять сертификат в хранилище секретов Kubernetes. На этом этапе контроллер входящего трафика шлюза приложений применит обновленный секрет, указанный в входящих ресурсах, который используется для настройки шлюза приложений.
