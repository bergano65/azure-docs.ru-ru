...
description: Шлюз приложений обеспечивает встроенную поддержку WebSocket независимо от размера. Настройки для настройки пользователя не настраиваются.
author: vhorne
ms.author: amsriva
...
Для установления подключения WebSocket между клиентом и сервером происходит обмен определенным рукопожатием на основе HTTP. В случае успешного выполнения протокол прикладного уровня "обновляется" с HTTP до WebSocket, используя ранее установленное соединение TCP. После этого HTTP больше не задействован. Данные могут быть отправлены или получены с использованием протокола WebSocket любой конечной точкой в любое время до завершения подключения WebSocket. 
...
Для поддержки трафика WebSocket можно использовать имеющийся прослушиватель HTTPListener. Ниже приведен фрагмент кода с элементом httpListeners из примера файла шаблона. Для поддержки и защиты трафика WebSocket необходимы прослушиватели HTTP и HTTPS. Аналогичным образом вы можете использовать портал или Azure PowerShell для создания шлюза приложения со слушателями в порту 80/443 для поддержки трафика WebSocket.
...
Для определения серверного пула с серверами с поддержкой WebSocket используется параметр BackendAddressPool. Параметр backendHttpSetting определяется с внутренним портом 80 или 443. Значение тайм-аута запроса в параметрах HTTP также относится к сеансу WebSocket. В правиле разгрома не требуется никаких изменений, которые используются для привязки соответствующего слушателя к соответствующему пулу адресов бэкэнда. 
