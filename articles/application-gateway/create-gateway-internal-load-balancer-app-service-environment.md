...
Для уменьшения издержек можно передать сертификат ILB в параметры HTTP, активировав путь пробы. (Этот шаг предназначен только для списка разрешений. Он не будет использоваться для SSL связи.) Вы можете получить сертификат ILB, обратившись к ILB с ЕГО IP-адресом из браузера на HTTPS, а затем экспортируя сертификат SSL в закодированном формате CER Base-64 и загрузив сертификат на соответствующие настройки HTTP.
