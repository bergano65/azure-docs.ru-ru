...
В этой статье неукоснятся следующие проблемы: сертификат не в белом списке при создании шлюза приложения с помощью внутреннего баланса нагрузки (ILB) вместе с средой службы приложений (ASE) на заднем конце при использовании сквозного TLS в Azure.
...
**502 - Веб-сервер получил недействительный ответ, действуя в качестве шлюза или прокси-сервера.**
...
Для уменьшения издержек можно передать сертификат ILB в параметры HTTP, активировав путь пробы. (Этот шаг предназначен только для списка разрешений. Он не будет использоваться для связи TLS.) Вы можете получить сертификат ILB, обратившись к ILB с ЕГО IP-адресом из браузера на HTTPS, а затем экспортируя сертификат TLS/SSL в закодированном формате CER Base-64 и загрузив сертификат в соответствующих настройках HTTP.
