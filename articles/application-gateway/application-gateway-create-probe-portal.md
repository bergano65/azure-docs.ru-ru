...
1. В колонке **Добавление зонда работоспособности** введите необходимые сведения о пробе, затем нажмите кнопку **ОК**.
   ...
   |**Узел**|Например, contoso.com|Имя узла, используемое для пробы. Применяется только в случае, когда в шлюзе приложений настроено многосайтовое подключение. В противном случае используется значение 127.0.0.1. Это значение отличается от имени узла виртуальной машины.|
   |**Путь**|/ или другой путь|Остальная часть полного URL-адреса пользовательской пробы. Путь должен начинаться с /. Путь по умолчанию http:\//contoso.com просто используйте «/» |
   |**Интервал (сек)**|30|Интервал между выполнением проб работоспособности. Не рекомендуется задавать значение менее 30 секунд.|
   |**Время ожидания (сек)**|30|Время ожидания пробы. Интервал времени ожидания должен быть достаточно большим для выполнения HTTP-вызова, чтобы обеспечить доступность страницы с данными о работоспособности внутреннего сервера.|
