...
Например: Вы настроили шлюз приложений таким образом, что внутренние серверы A, B и C используются для получения сетевого трафика HTTP на порт 80. По умолчанию мониторинг работоспособности осуществляет проверку трех серверов каждые 30 секунд и ожидает получения HTTP-ответа, подтверждающего работоспособность. HTTP-ответ, подтверждающий работоспособность, имеет [код состояния](https://msdn.microsoft.com/library/aa287675.aspx) от 200 до 399.
...
По умолчанию ответ HTTP (S) с кодом состояния от 200 до 399 считается работоспособным. Настраиваемые пробы работоспособности поддерживают два критерия соответствия. Критерий соответствия можно использовать, чтобы дополнительно изменить стандартную интерпретацию содержимого ответа, указывающего на работоспособность.
...
| --- | --- | --- |
| URL-адрес пробы |http://127.0.0.1:\<port\>/ |URL-адрес |
| Interval |30 |Количество времени (в секундах) ожидания перед отправкой следующей пробы работоспособности.|
...
| Порог состояния неработоспособности |3 |Управляет количеством проб, которые нужно отправить в случае сбоя стандартной пробы работоспособности. Эти дополнительные пробы работоспособности отправляются быстро и последовательно, чтобы быстро определить работоспособность серверной части и не дожидаться интервала пробы. Сервер внутреннего пула считается неисправным, когда количество повторных неудачных попыток проверки достигает порогового значения сбоя. |

> [!NOTE]
> Порт совпадает с портом, указанным в параметрах HTTP серверной части.

Проба по умолчанию выполняет поиск только в http:\//127.0.0.1:\<порт\> для определения состояния работоспособности. Если необходимо настроить пробу работоспособности для перехода на настраиваемый URL-адрес или изменить любые другие параметры, используйте пользовательские пробы.
...
| Interval |Интервал проверки в секундах. Это значение означает интервал времени между двумя последовательными проверками. |
...
| Порог состояния неработоспособности |Количество повторных попыток проверки. Сервер внутреннего пула считается неисправным, когда количество повторных неудачных попыток проверки достигает порогового значения сбоя. |
