...
| Пороговое значение сбоя |3 |Управляет количеством проб, которые отправляются в случае сбоя обычной проверки работоспособности. Эти дополнительные пробы работоспособности отправляются в ходе быстрой успешной работы, чтобы быстро определить работоспособность серверной части и не ждать интервала пробы. Это бехаивор только номер SKU версии v1. В случае номера SKU версии 2 пробы работоспособности ожидают интервал. Сервер внутреннего пула считается неисправным, когда количество повторных неудачных попыток проверки достигает порогового значения сбоя. |

> [!NOTE]
> Порт совпадает с портом, указанным в параметрах HTTP серверной части.

Проба по умолчанию просматривает только\/порт\> http:\</127.0.0.1:, чтобы определить состояние работоспособности. Если необходимо настроить пробу работоспособности для перехода на настраиваемый URL-адрес или изменить любые другие параметры, используйте пользовательские пробы.
...
| Path |Относительный путь проверки. Путь должен начинаться с "/". |
...
Необходимо разрешить входящий Интернет-трафик через TCP-порты 65503-65534 для номера SKU шлюза приложений версии 1, а TCP-порты 65200-65535 для SKU v2 с конечной подсетью **и тегом источника AS** **гатевайманажер** Service. Такой диапазон портов требуется для обмена данными в рамках инфраструктуры Azure.

Кроме того, исходящее подключение к Интернету не может быть заблокировано, а входящий трафик, поступающий из тега **AzureLoadBalancer** , должен быть разрешен.
