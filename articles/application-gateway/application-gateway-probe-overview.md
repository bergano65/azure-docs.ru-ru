...
description: Azure Application Gateway отслеживает работоспособность всех ресурсов в пуле бэк-энда и автоматически удаляет из пула любой ресурс, который считается неработоспособным.
...
Источник IP-адреса Application Gateway используется для зондов работоспособности зависит от бэкэнд пула:
 
- Если пул бэкэнда является общедоступной конечной точкой, то исходным адресом является публичный ip-адрес шлюза приложения.
- Если пул бэкэнда является частной конечной точкой, то исходный IP-адрес — это из подсетевого приложения пространства частного IP-адреса.
...
Если проверка работоспособности по умолчанию для сервера А завершается неудачно, шлюз приложений удаляет его из внутреннего пула и сетевой трафик перестает поступать на этот сервер. Проверка работоспособности по умолчанию по-прежнему продолжает проверять сервер A каждые 30 секунд. Когда сервер А успешно отвечает на один запрос от зонда работоспособности по умолчанию, он возвращается как здоровый пул бэк-энда, и трафик снова начинает поступать на сервер.

### <a name="probe-matching"></a>Соответствие проб

По умолчанию ответ HTTP (S) со кодом состояния между 200 и 399 считается здоровым. Настраиваемые пробы работоспособности поддерживают два критерия соответствия. Критерии соответствия могут быть использованы для опционально изменить интерпретацию по умолчанию того, что делает здоровый ответ.
...
- **Соответствие текста ответа HTTP**. Критерий соответствия пробы, позволяющий проверять текст ответа HTTP и сопоставлять его с указанной пользователем строкой. Матч только ищет наличие пользователя указанной строки в ответ тела и не является полной регулярной соответствия выражения.
...
| Пороговое значение сбоя |3 |Управляет, сколько зондов отправить в случае, если есть сбой регулярного зонда здоровья. Эти дополнительные зонды здоровья отправляются в быстрой последовательности, чтобы определить здоровье бэкэнда быстро и не ждать интервал зонда. Это behaivor только v1 SKU. В случае v2 SKU, зонды здоровья ждут интервал. Сервер внутреннего пула считается неисправным, когда количество повторных неудачных попыток проверки достигает порогового значения сбоя. |

> [!NOTE]
> Порт совпадает с портом, указанным в параметрах HTTP серверной части.

Зонд по умолчанию смотрит\/только на http: /127.0.0.1:\<порт\> для определения состояния работы. Если необходимо настроить пробу работоспособности для перехода на настраиваемый URL-адрес или изменить любые другие параметры, используйте пользовательские пробы.
...
Вы должны разрешить входящий интернет-трафик на портах TCP 65503-65534 для приложения Шлюз v1 SKU, и TCP порты 65200-65535 для v2 SKU с направлением подсети, как **любой** и источник, как **GatewayManager** сервисный тег. Такой диапазон портов требуется для обмена данными в рамках инфраструктуры Azure.

Кроме того, исходящие подключения к Интернету не могут быть заблокированы, и входящий трафик, поступающий из тега **AzureLoadBalancer,** должен быть разрешен.

Для получения дополнительной информации смотрите [обзор конфигурации приложения Gateway](configuration-overview.md#network-security-groups-on-the-application-gateway-subnet).
