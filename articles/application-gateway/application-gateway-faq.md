...
Виртуальный IP-адрес может измениться, если вы останавливаете и запускаете шлюз приложений. Но DNS-имя, связанное с шлюзом приложений, не изменяется в течение времени существования шлюза. Так как DNS-имя не меняется, следует использовать псевдоним CNAME и указать его DNS-адрес шлюза приложений.
...
Да. Можно настроить сток подключений, чтобы изменить элементы в внутреннем пуле без прерывания работы. Эта установка позволяет продолжить отправку существующих подключений к их предыдущему назначению до тех пор, пока соединение не будет закрыто или не истечет время ожидания. Сток подключений ожидает завершения только текущих активных подключений. Шлюз приложений не имеет сведений о состоянии сеанса приложения.
...
В поле Host указывается имя для отправки зонда при настройке многосайтового шлюза приложений. В противном случае используйте "127.0.0.1". Это значение отличается от имени узла виртуальной машины. Его формат — \<протокол\>://\<узел\>\>:\<путь кпорту\>.\<
...
Проверьте работоспособность с помощью командлета `Get-AzApplicationGatewayBackendHealth` PowerShell или портала. Дополнительные сведения см. в разделе [Журналы диагностики](application-gateway-diagnostics.md).
