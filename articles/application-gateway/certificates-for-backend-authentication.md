---
title: Сертификаты, необходимые для список разрешений в шлюзе приложений Azure
description: В этой статье приведены примеры того, как SSL-сертификат можно преобразовать в сертификат проверки подлинности и доверенный корневой сертификат, необходимые для внутренних экземпляров список разрешений в шлюзе приложений Azure.
...
Доверенный корневой сертификат необходим для разрешения внутренних экземпляров в SKU шлюза приложений версии 2. Корневой сертификат является Base-64 в кодировке X. 509 (. CER) отформатируйте корневой сертификат из сертификатов внутреннего сервера. В этом примере вы используете SSL-сертификат для серверного сертификата и экспортируете его открытый ключ. Затем вы экспортируете корневой сертификат доверенного ЦС из открытого ключа в формате Base64, чтобы получить доверенный корневой сертификат. 
...
Теперь у вас есть сертификат проверки подлинности или доверенный корневой сертификат в Base-64 с кодировкой X. 509 (. CER). Его можно добавить в шлюз приложений, чтобы обеспечить сквозное шифрование SSL внутренних серверов. См. раздел [Настройка сквозного шифрования SSL](https://docs.microsoft.com/azure/application-gateway/application-gateway-end-to-end-ssl-powershell).
