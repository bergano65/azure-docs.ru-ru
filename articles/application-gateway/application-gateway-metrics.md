...
- **Время соединения серверной части**
  ...
  Сюда входит задержка сети, а также время, затраченное стеком TCP внутреннего сервера для установки новых соединений. В случае SSL также включает время, затраченное на подтверждение. 
...
- **Время последнего ответа серверной части**
  ...
  Это приблизительно соответствует сумме времени *отклика первого байта* в сервере и времени передачи данных (это значение может сильно различаться в зависимости от размера запрошенных объектов и задержки сети сервера).
...
- **Расчетные единицы пропускной способности**

  При использовании SKU v2 модель ценообразования зависит от потребления. Единицы мощности измеряют стоимость на основе потребления, которая будет взиматься в дополнение к фиксированным затратам. *Расчетные единицы пропускной способности* указывают количество единиц мощности, по которым оценивается счет. Это значение вычисляется в виде большего значения между *текущими единицами мощности* (единицами мощности, необходимыми для балансировки нагрузки трафика) и *фиксированными единицами оплачиваемой емкости* (подготовленные элементы минимальной емкости).
...
- **Фиксированные единицы оплачиваемой емкости**

  Минимальное число единиц емкости, подготовленных в соответствии с параметром *минимальной единицы масштабирования* (один экземпляр преобразуется в 10 единиц мощности) в конфигурации шлюза приложений.
