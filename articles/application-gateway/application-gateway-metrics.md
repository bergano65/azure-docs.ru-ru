...
После фильтрации по прослушивателю, если *RTT клиента* превышает общее время, чем *общий доступ к шлюзу приложений*, можно вывести, что задержка, наблюдаемая клиентом, обусловлена сетевым подключением между клиентом и шлюзом приложений. Если обе задержки являются сравнимыми, то высокая задержка может быть вызвана одним из следующих способов: шлюз приложений, сеть между шлюзом приложений и внутренним приложением или производительность внутреннего приложения.
...
Если *Общее время для шлюза приложений* превышает *время последнего ответа сервера за последний байт* для конкретного прослушивателя, то его можно вывести, так как большая задержка может быть вызвана шлюзом приложений. С другой стороны, если две метрики сравнимы, то эта ошибка может быть вызвана сетью между шлюзом приложений и внутренним приложением или производительностью серверного приложения.

- **Время соединения серверной части**

  Время, затраченное на установление соединения с серверным приложением. В случае SSL он включает время, затраченное на подтверждение. Обратите внимание, что эта метрика отличается от других метрик задержки, так как это только измеряет время подключения и поэтому не должно быть напрямую сравниваться с другими задержками. Однако при сравнении шаблона *времени подключения внутреннего сервера* с шаблоном других задержек может быть указано, можно ли увеличить другие задержки из-за вариации в сети между приложением Gateway и внутренним приложением. 
