---
title: Настройка сквозного протокола TLS с помощью шлюза приложений Azure
description: В этой статье описывается, как настроить сквозной протокол TLS для шлюза приложений Azure с помощью PowerShell.
...
ms.locfileid: "81312496"
---
# <a name="configure-end-to-end-tls-by-using-application-gateway-with-powershell"></a>Настройка сквозного TLS с помощью шлюза приложений с PowerShell

## <a name="overview"></a>Обзор

Шлюз приложений Azure поддерживает сквозное шифрование трафика. Шлюз приложений прерывает подключение TLS/SSL на шлюзе приложений. Затем шлюз применяет правила маршрутизации к трафику, повторно шифрует пакет и пересылает его в соответствующую серверную часть согласно определенным правилам маршрутизации. Любой ответ веб-сервера проходит через тот же процесс на пути к пользователю.

Шлюз приложений поддерживает определение настраиваемых параметров TLS. отключение следующих версий протокола: **TLSv1.0**, **TLSv1.1** и **TLSv1.2**. Также шлюз поддерживает определение комплектов шифров для использования и их приоритет. Дополнительные сведения о настраиваемых параметрах TLS см. в разделе [Общие сведения о политике TLS](application-gateway-SSL-policy-overview.md).
...
В этом сценарии вы узнаете, как создать шлюз приложений, используя сквозной протокол TLS с PowerShell.

Вы узнаете:

* Создайте группу ресурсов с именем **appgw-RG**.
* Создайте виртуальную сеть с именем **appgwvnet** и адресным пространством **10.0.0.0/16**.
* как создать две подсети, **appgwsubnet** и **appsubnet**;
* Создайте небольшой шлюз приложений, поддерживающий сквозное шифрование TLS, которое ограничивает версии протокола TLS и комплекты шифров.

## <a name="before-you-begin"></a>Подготовка к работе

[!INCLUDE [updated-for-az](../../includes/updated-for-az.md)]

Чтобы настроить сквозной протокол TLS с помощью шлюза приложений, требуется сертификат для шлюза, а для внутренних серверов требуются сертификаты. Сертификат шлюза используется для получения симметричного ключа в соответствии со спецификацией протокола TLS. Затем используется симметричный ключ шифрования и расшифровки трафика, отправляемого на шлюз. Сертификат шлюза должен быть представлен в формате PFX (файл обмена личной информацией). Этот формат файла позволяет экспортировать закрытый ключ, необходимый шлюзу приложений для шифрования и расшифровки трафика.

Для сквозного шифрования TLS серверная части должна быть явно разрешена шлюзом приложений. Передайте общий сертификат серверных частей в шлюз приложения. Это гарантирует, что шлюз приложений взаимодействует только с известными экземплярами серверной части, тем самым защищая сквозной обмен данными.
...
5. Настройте сертификат для шлюза приложений. Этот сертификат используется для шифрования и расшифровки трафика на шлюзе приложений.
   ...
   > [!NOTE]
   > В этом примере настраивается сертификат, используемый для подключения TLS. Сертификат должен иметь формат PFX, а пароль к сертификату должен содержать от 4 до 12 символов.

6. Создайте прослушиватель HTTP для шлюза приложений. Назначьте IP-конфигурацию внешнего интерфейса, порт и сертификат TLS/SSL для использования.

   ```powershell
   $listener = New-AzApplicationGatewayHttpListener -Name listener01 -Protocol Https -FrontendIPConfiguration $fipconfig -FrontendPort $fp -SSLCertificate $cert
   ```

7. Отправьте сертификат, который будет использоваться в ресурсах пула серверной части с поддержкой TLS.

   > [!NOTE]
   > Проба по умолчанию получает открытый ключ из *стандартной* привязки TLS на IP-адресе серверной части и сравнивает полученное значение открытого ключа со значением открытого ключа, которое вы задаете здесь. 
   > 
   > Если на серверной части используются заголовки узлов и указание имени сервера (SNI), полученный открытый ключ не обязательно будет ключом сайта, на который направляется интернет-трафик. Если вы сомневаетесь, посетите https://127.0.0.1/ серверные серверы, чтобы подтвердить, какой сертификат используется для привязки TLS *по умолчанию* . В этом разделе используйте открытый ключ из этого запроса. Если вы используете заголовки узлов и SNI для привязок HTTPS и не получаете ответ и сертификат от ручного запроса к https://127.0.0.1/ браузеру на внутренних серверах, необходимо настроить на них привязку TLS по умолчанию. Если этого не сделать, пробы будут завершены ошибкой и серверная часть не будет добавлена в список разрешений.
   ...
   Если вы используете номер SKU Шлюза приложений версии 2, создайте доверенный корневой сертификат, а не сертификат проверки подлинности. Дополнительные сведения см. в статье Общие сведения о [сквозном TLS с помощью шлюза приложений](ssl-overview.md#end-to-end-tls-with-the-v2-sku).
...
10. Настройте размер экземпляра шлюза приложений. Доступные размеры: **Standard\_Small**, **Standard\_Medium** и **Standard\_Large**.  Для емкости доступны значения от **1** до **10**.
...
2. Добавьте новый ресурс сертификата из CER – файла, который содержит открытый ключ сертификата, и также может быть тем же сертификатом, добавленным в прослушиватель для завершения TLS в шлюзе приложений.
...
Предыдущие шаги потребовали создания приложения с комплексным протоколом TLS и отключением определенных версий протокола TLS. В следующем примере отключаются определенные политики TLS в существующем шлюзе приложений.
...
2. Определите политику TLS. В следующем примере отключены **tlsv 1.0** и **tlsv 1.1** , а также комплекты шифров **\_TLS\_ECDHE\_ECDSA\_с\_AES\_128\_gcm SHA256**, **\_TLS\_ECDHE\_ECDSA\_с\_AES\_256\_gcm SHA384**, **а\_TLS\_RSA\_с\_AES\_128\_gcm SHA256** — это единственные разрешенные.
...
3. Наконец, обновите шлюз. Этот последний шаг занимает много времени. После этого на шлюзе приложений будет настроен сквозной протокол TLS.
