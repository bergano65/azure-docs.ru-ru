...
Служба Azure Backup может создавать резервные копии виртуальных машин, имеющих до 16 дисков. Поддержка 16 дисков предоставляется в [последней версии](backup-upgrade-to-vm-backup-stack-v2.md) стека резервного копирования виртуальных машин Azure версии 2.
...
Невозможно сделать моментальный снимок диска с поддержкой ускорителя записи. Но служба Azure Backup может исключить диск с поддержкой ускорителя записи из резервной копии. Исключение диска для виртуальных машин с дисками с поддержкой ускорителя записи поддерживается только для подписок, обновленных до стека резервного копирования виртуальных машин Azure версии 2. Чтобы выполнить обновление до стека резервного копирования виртуальных машин Azure версии 2, ознакомьтесь с этой [статьей](backup-upgrade-to-vm-backup-stack-v2.md). Эта функция сейчас доступна в восточной Японии, Северной Европе, Юго-Восточной Азии, восточной части США, западной части США — 2, Западной Европе и восточной части США — 2.
...
Выберите восстановление диска, если вы хотите следующее.
  ...
  * Добавить другие параметры, которые должны быть настроены с помощью PowerShell или шаблона.  w
...
Для повышения производительности восстановления рекомендуется перейти на стек резервного копирования виртуальных машин версии 2 и использовать [функцию мгновенного восстановления](backup-upgrade-to-vm-backup-stack-v2.md).
