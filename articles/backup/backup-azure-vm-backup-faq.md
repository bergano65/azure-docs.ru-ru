...
description: Ответы на часто задаваемые вопросы о резервном копировании виртуальных машин Azure с помощью Azure Backup.
author: sogup
manager: vijayts
ms.service: backup
ms.topic: conceptual
...
Мы используем закрытую предварительную версию для SAP HANA резервного копирования с RPO, равной 15 минутам. Она похожа на резервное копирование базы данных SQL и использует интерфейс backInt для сторонних решений, сертифицированных SAP HANA. Если вы заинтересованы, напишите нам `AskAzureBackupTeam@microsoft.com` по адресу с темой **Регистрация для использования закрытой предварительной версии для резервного копирования SAP HANA на виртуальных машинах Azure**.
...
Запланированное резервное копирование будет запущено в течение 2 часов запланированного времени архивации. Для ex Если 100 виртуальных машин имеют запланированное время начала резервного копирования в 2:00 AM, то по макс. 4:00 все 100VMs будут иметь задание резервного копирования. Если запланированные резервные копии приостановлены из-за сбоя и возобновления или повторной попытки, резервное копирование может начаться за пределами этого запланированного окна часовой.
...
Политика архивации виртуальных машин Azure поддерживает минимальный диапазон хранения в 7 дней до 9999 дней. Любое изменение существующей политики резервного копирования виртуальной машины с менее чем 7 днями потребует обновления в соответствии с минимальным диапазоном хранения, равным 7 дням.
...
Используйте восстановление диска, если нужно сделать следующее:
  ...
  * Добавьте параметры конфигурации, которые не существовали во время резервного копирования.
