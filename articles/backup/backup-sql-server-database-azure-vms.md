...
>[!NOTE]
>Операция **настройки защиты** для баз данных со специальными символами, такими как "+" или "&" в имени, не поддерживается. Можно либо изменить имя базы данных, либо включить **автоматическую защиту**, которая может успешно защитить эти базы данных.
...
5. В поле >  **цель резервного копирования****Обнаружение баз данных в виртуальных машинах**выберите **запустить обнаружение** , чтобы найти незащищенные виртуальные машины в подписке. Поиск может занять некоторое время в зависимости от числа незащищенных виртуальных машин в подписке.
...
8. Azure Backup обнаруживает все базы данных SQL Server на виртуальной машине. Во время обнаружения в фоновом режиме выполняются следующие элементы:
    ...
    * Azure Backup создает учетную запись службы NT Service\AzureWLBackupPluginSvc.
      ...
      * Для NT Сервице\азуревлбаккупплугинсвк требуются разрешения системного администратора SQL. Все SQL Server виртуальные машины, созданные в Marketplace, входят в состав установленной SqlIaaSExtension. Расширение AzureBackupWindowsWorkload использует SQLIaaSExtension для автоматического получения необходимых разрешений.
...
1. На этапе **резервного копирования** > **Шаг 2. Настройка резервного копирования**выберите **настроить резервное копирование**.
