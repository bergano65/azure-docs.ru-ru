...
- **Разрешить IP-адресов центра обработки данных Azure**. Этот параметр позволяет [диапазоны IP-адресов](https://www.microsoft.com/download/details.aspx?id=41653) в загрузку. Чтобы получить доступ к группу безопасности сети (NSG), используйте командлет Set-AzureNetworkSecurityRule. Если все в порядке получателей списке только IP-адреса конкретного региона, необходимо также обновить список надежных получателей тег службы Azure Active Directory (Azure AD), чтобы включить проверку подлинности.
