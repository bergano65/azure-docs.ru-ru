...
| Сообщение об ошибке | Возможные причины | Рекомендованное действие |
|---|---|---|
Виртуальной машине не удается связаться со службой Azure Backup из-за проблем с подключением к Интернету. | Виртуальной машине требуется исходящее подключение к службам Azure Backup, службе хранилища Azure или службам Azure Active Directory.| <li> Если вы используете NSG для ограничения возможностей подключения, то следует использовать тег службы *AzureBackup* , чтобы разрешить исходящий доступ к Azure Backup службе, а также к службам Azure AD (*AzureActiveDirectory*) и службе хранилища Azure (*хранилище*). Чтобы предоставить доступ, выполните следующие [действия](./backup-sql-server-database-azure-vms.md#nsg-tags) . <li> Убедитесь, что DNS разрешает конечные точки Azure. <li> Убедитесь, что виртуальная машина находится за подсистемой балансировки нагрузки, которая блокирует доступ к Интернету. При назначении общедоступного IP-адреса виртуальным машинам обнаружение будет работать. <li> Убедитесь в отсутствии брандмауэра, антивирусной программы или прокси-сервера, блокирующих вызовы указанных выше трех целевых служб.
