...
Значения параметров `{containerName}` и `{protectedItemName}` соответствуют указанным [выше](#responses-1). `{fabricName}` — это Azure. В нашем примере мы получим следующее:
