...
>[!NOTE]
>DPM 2019 UR2 поддерживает SQL Server экземпляров отказоустойчивого кластера (FCI) с помощью общих томов кластера (CSV).
...
* Проблемы при резервном копировании SQL Server 2014 или выше
  * В SQL Server 2014 добавлена новая функция для создания [базы данных для локального экземпляра SQL Server в хранилище BLOB-объектов Azure](/sql/relational-databases/databases/sql-server-data-files-in-microsoft-azure?view=sql-server-ver15). DPM не может использоваться для защиты этой конфигурации.
...
1. Нажмите **Далее**. DPM показывает общее доступное дисковое пространство. Он также показывает потенциальное использование дискового пространства.
...
1. Если вы являетесь администратором, вы можете автоматически переносить эту начальную резервную копию **по сети** и выбрать время передачи. Или выберите **ручной** перенос резервной копии. Затем щелкните **Далее**.
...
1. Выберите время выполнения проверки согласованности. Затем щелкните **Далее**.
...
1. Выберите источники данных для защиты в Azure. Затем щелкните **Далее**.
...
1. DPM отобразит сведения о точке восстановления. Нажмите **Далее**. Чтобы перезаписать базу данных, выберите тип восстановления **Восстановить в исходном экземпляре SQL Server**. Затем щелкните **Далее**.
...
1. На странице **Указание параметров восстановления** можно выбрать параметры восстановления. Например, можно выбрать **регулирование использования полосы пропускания сети** для регулирования пропускной способности, используемой при восстановлении. Затем щелкните **Далее**.
