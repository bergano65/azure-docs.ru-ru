...
* Убедитесь, что [реплики группы доступности настроены как доступные только для чтения](/sql/database-engine/availability-groups/windows/configure-read-only-access-on-an-availability-replica-sql-server?view=sql-server-ver15).
* Необходимо явно добавить системную учетную запись **NTAuthority\System** в группу Sysadmin на SQL Server.
* При выполнении восстановления в альтернативном расположении для частично автономной базы данных необходимо убедиться, что в целевом экземпляре SQL включена функция [автономных баз](/sql/relational-databases/databases/migrate-to-a-partially-contained-database?view=sql-server-ver15#enable) данных.
* При выполнении восстановления в альтернативном расположении для базы данных файлового потока необходимо убедиться, что в целевом экземпляре SQL включена функция [базы данных файлового потока](/sql/relational-databases/blob/enable-and-configure-filestream?view=sql-server-ver15) .
...
* SQL Server 2014 или выше проблемы резервного копирования:
  * В SQL Server 2014 добавлена новая функция для создания [базы данных для локальных SQL Server в хранилище больших двоичных объектов Windows Azure](/sql/relational-databases/databases/sql-server-data-files-in-microsoft-azure?view=sql-server-ver15). DPM не может использоваться для защиты этой конфигурации.
...
1. На странице **выбор краткосрочных целей** включите необходимые входные данные для создания точек резервного копирования на диск.
...
1. Если вы являетесь администратором, вы можете автоматически переносить эту начальную резервную копию **по сети** и выбрать время передачи. Или выберите **ручной** перенос резервной копии. Нажмите кнопку **Далее**.
...
1. Выберите время выполнения проверки согласованности. Нажмите кнопку **Далее**.
...
1. Выберите источники данных для защиты в Azure. Нажмите кнопку **Далее**.
...
1. DPM отобразит сведения о точке восстановления. Выберите **Далее**. Чтобы перезаписать базу данных, выберите тип восстановления **Восстановить в исходном экземпляре SQL Server**. Нажмите кнопку **Далее**.
...
1. На странице **Указание параметров восстановления** можно выбрать параметры восстановления. Например, можно выбрать **регулирование использования полосы пропускания сети** для регулирования пропускной способности, используемой при восстановлении. Нажмите кнопку **Далее**.
