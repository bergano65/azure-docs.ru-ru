...
## <a name="backup-items-in-recovery-services-vault"></a>Архивация элементов в хранилище служб восстановления

Вы можете отслеживать все элементы резервного копирования с помощью хранилища служб восстановления. При переходе к разделу " **архивные элементы** " в хранилище открывается представление с указанием количества элементов резервного копирования каждого типа рабочей нагрузки, связанного с хранилищем. При щелчке любой строки открывается подробное представление с перечнем всех элементов резервного копирования заданного типа рабочей нагрузки с информацией о последнем состоянии резервного копирования для каждого элемента, доступной последней точкой восстановления и т. д.

![Архивные элементы хранилища RS](media/backup-azure-monitoring-laworkspace/backup-items-view.png)

> [!NOTE]
> Для элементов, резервное копирование которых выполняется в Azure с помощью DPM, в списке будут показаны все защищенные источники данных (как диск, так и в сети) с помощью сервера DPM. Если защита для DataSource с сохранением данных резервного копирования остановлена, источник данных по-прежнему будет отображаться на портале. Можно обратиться к сведениям об источнике данных, чтобы узнать, имеются ли точки восстановления на диске, в сети или в обоих случаях. Кроме того, источники данных, для которых оперативная защита остановлена, но данные сохранены, выставление счетов за точки восстановления в сети продолжается до тех пор, пока данные не будут полностью удалены.
>
> Версия DPM должна быть DPM 1807 (5.1.378.0) или DPM 2019 (версия 10.19.58.0 или более поздняя), чтобы элементы резервного копирования отображались на портале хранилища служб восстановления.
...
> [!NOTE]
> Просмотр оповещений в хранилищах в настоящее время не поддерживается в центре архивации. Чтобы просмотреть оповещения для этого хранилища, необходимо перейти к отдельному хранилищу.
