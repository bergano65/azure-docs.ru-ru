---
title: Постоянное копирование Linux VMs
...
Сценарии предварительной обработки вызывают собственные API приложения, которые замораживают операции ввода вывода и высвобождают содержимое памяти на диск. Эти действия обеспечивают согласованность моментального снимка с приложениями. Постскриптам используют нативные AI приложений для оттаивания IOs, которые позволяют приложению возобновить нормальную работу после снимка VM.
...
5. Настройте файл **VMSnapshotScriptPluginConfig.json**, как описано далее.
    ...
    - **timeoutInSeconds**: Укажите отдельные тайм-ауты для предварительного сценария и пост-скрипта (максимальное значение может быть 1800).
    ...
    - **СкриптыExecutionPollTimeSeconds:** Установите время, когда расширение должно спать между каждым опросом до выполнения сценария. Например, если значение 2, расширение проверяет, завершается ли выполнение сценария до/пост каждые 2 секунды. Минимальное и максимальное значение, что может занять 1 и 5 соответственно. Значение должно быть строго бесхопослятие.
