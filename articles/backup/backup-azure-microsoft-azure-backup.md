---
title: Используйте сервер резервного копирования Azure для резервного копирования рабочих нагрузок
description: В этой статье узнайте, как подготовить среду для защиты и резервного копирования рабочих нагрузок с помощью Сервера резервного копирования Microsoft Azure (MABS).
ms.topic: conceptual
ms.date: 11/13/2018
...
ms.locfileid: "79273414"
---
# <a name="install-and-upgrade-azure-backup-server"></a>Установка и обновление Azure Backup Server

> [!div class="op_single_selector"]
>
> * [Сервер резервного копирования Azure](backup-azure-microsoft-azure-backup.md)
> * [SCDPM](backup-azure-dpm-introduction.md)
>
>

> Применяется к: MABS v3. (MABS v2 больше не поддерживается. Если вы используете версию раньше, чем MABS v3, пожалуйста, пересмотрите до последней версии.)
...
> [!NOTE]
> Azure Backup Server позволяет защитить виртуальные машины VMware и предоставляет возможности для повышения безопасности. Установите этот продукт, как описано в следующих разделах, и последнюю версию агента Azure Backup. Дополнительные сведения о резервном копировании серверов VMware с помощью Azure Backup Server см. в статье [Резервное копирование сервера VMware с помощью сервера резервного копирования Azure](backup-azure-backup-server-vmware.md). Чтобы узнать о возможностях безопасности, обратитесь к [документации функций безопасности резервного копирования Azure.](backup-azure-security-feature.md)
>
>

MABS, развернутый в Azure VM, может резервное копирование ВМ в Azure, но они должны быть в том же домене, чтобы включить операцию резервного копирования. Процесс поддержки VM Azure остается таким же, как резервное копирование VMs в помещениях, однако развертывание MABS в Azure имеет некоторые ограничения. Для получения дополнительной информации об ограничении см. [DPM как виртуальная машина Azure](https://docs.microsoft.com/system-center/dpm/install-dpm?view=sc-dpm-1807#setup-prerequisites)

> [!NOTE]
> Azure имеет две модели развертывания для создания и работы с ресурсами: [менеджер ресурсов и классический.](../azure-resource-manager/management/deployment-models.md) Данная статья содержит сведения о восстановлении виртуальных машин, развернутых с использованием модели Resource Manager.
>
>
...
При выборе сервера для запуска сервера резервного копирования Azure рекомендуется начать с галерейного изображения Windows Server 2016 Datacenter или Windows Server 2019 Datacenter. Статья [Создание первой виртуальной машины Windows на портале Azure](../virtual-machines/virtual-machines-windows-hero-tutorial.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json) содержит руководство по началу работы с рекомендуемой виртуальной машиной в Azure. Оно подходит даже тем, кто только начинает знакомиться с Azure. Рекомендуемые минимальные требования к серверу виртуальной машины (VM) должны быть: Standard_A4_v2 с четырьмя ядрами и 8-ГБ оперативной памяти.
...
Если вы не хотите запускать базовый сервер в Azure, можно запустить сервер на виртуальной машине Hyper-V, виртуальной машине VMware или на физическом узле. Рекомендуемые минимальные требования к серверного оборудования составляют два ядра и 8-ГБ оперативной памяти. В следующей таблице перечислены поддерживаемые операционные системы.
...
С помощью дедупликации Windows Server можно выполнить дедупликацию хранилища DPM. См. дополнительные сведения об использовании [DPM и дедупликации](https://docs.microsoft.com/system-center/dpm/deduplicate-dpm-storage?view=sc-dpm-2019) при развертывании на виртуальных машинах Hyper-V.
...
5. В колонке **Backup Goals** (Цели архивации) в меню **Где выполняется рабочая нагрузка?** выберите пункт **Локально**.
    ...
    Из меню **резервного копирования, который выхотите создать резервное копирование?** **OK**
...
7. Выберите все файлы и нажмите кнопку **Далее**. Скачайте все файлы со страницы скачивания Microsoft Azure Backup и поместите их в одну папку.
    ...
    Так как размер загрузки всех файлов вместе > 3G, на 10-Мбит / с скачать ссылку это может занять до 60 минут для загрузки для завершения.
...
> [!WARNING]
> Для извлечения файлов настройки требуется не менее 4 ГБ свободного пространства.
>
>
...
3. Пакет установки Azure Backup Server поставляется в комплекте с соответствующими файлами,необходимыми для сервера S'L Server. При запуске новой установки сервера резервного копирования Azure выберите новую опцию **«Установка нового instanceof s'L Server» с помощью этой настройки** и нажмите кнопку **«Проверка и установка».** После успешной установки необходимых компонентов нажмите кнопку **Далее**.

    >[!NOTE]
    >Если вы хотите использовать свой собственный сервер S'L, поддерживаемые версии сервера S'L являются S'L Server 2014 SP1 или выше, 2016 и 2017.  Все версии SQL Server должны быть 64-разрядными уровня Standard или Enterprise.
    >Azure Backup Server не поддерживает использование удаленного экземпляра SQL Server. Экземпляр, который использует Azure Backup Server, должен быть локальным. Если вы используете существующий сервер S'L для MABS, настройка MABS поддерживает только использование *названных экземпляров* сервера S'L.

    ![Azure Backup Server — проверка SQL](./media/backup-azure-microsoft-azure-backup/sql/01.png)

    В случае сбоя и появления рекомендации перезагрузить компьютер, выполните инструкцию на экране и нажмите кнопку **Проверить снова**. При наличии каких-либо проблем с конфигурацией СЗЛ, перенастройка S'L в том, что касается руководящих принципов S'L, и повторная попытка установить/обновить MABS с использованием существующего экземпляра S'L.
   ...
   **Настройка вручную**
    ...
    **Конфигурация SSRS с SQL 2017**
    ...
    При использовании собственного экземпляра S'L 2017 необходимо вручную настроить SSRS. После настройки SSRS присвойте свойству *IsInitialized* служб SSRS значение *True*. Когда задано значение True, в MABS предполагается, что службы SSRS уже настроены и настройка служб SSRS будет пропущена.

    Для этой конфигурации SSRS используйте следующие значения:
    * Учетная запись обслуживания: «Использование встроенного счета» должно быть сетевой службой
    * URL-адрес веб-сервиса: «Виртуальный\<каталог» должен быть ReportServer_ s'LInstanceName>
    * База данных: DatabaseName\<должен быть ReportServer$ S'LInstanceName>
    * URL-адрес веб-портала: «Виртуальный каталог» должен быть Reports_\<sLInstanceName>
    ...
    > [!NOTE]
    > Лицензирование для сервера S'L, используемого в качестве базы данных для MABS, регулируется [условиями онлайн-услуг Майкрософт](https://www.microsoft.com/licensing/product-licensing/products) (OST). По данным OST, сервер S'L в комплекте с MABS может использоваться только в качестве базы данных для MABS.

4. Укажите расположение для установки файлов сервера Microsoft Azure Backup и нажмите кнопку **Далее**.

    ![Microsoft Azure Backup PreReq2](./media/backup-azure-microsoft-azure-backup/space-screen.png)

    Папка для временных файлов обязательна для резервного копирования в Azure. Убедитесь, что размер папки для временных файлов составляет по крайней мере 5 % объема данных, для которых планируется создать резервную копию в облаке. После завершения установки необходимо настроить защиту отдельных дисков. Дополнительные сведения о пулах носителей см. в статье [Настройка пулов носителей и дискового хранилища](https://docs.microsoft.com/previous-versions/system-center/system-center-2012-R2/hh758075(v=sc.12)).
...
8. Установка выполняется поэтапно. На первом этапе на сервере установлен агент служб восстановления Microsoft Azure. В это время мастер также проверяет наличие подключения к Интернету. При наличии подключения к Интернету можно продолжить установку. В противном случае потребуется указать сведения о прокси-сервере.
...
1. На консоли администратора резервного копирования сервера выберите**агентов** **управления.** > 
...
Ниже приведены действия по перемещению MABS на новый сервер и сохранению хранилища. Это можно сделать только в том случае, если все данные находятся в Современном хранилище резервных копий.
  ...
  > [!IMPORTANT]
  >
  > * Для нового имени сервера используется такое же имя, как и у исходного экземпляра Azure Backup Server. Вы не можете изменить имя нового экземпляра сервера резервного копирования Azure, если вы хотите использовать предыдущий пул хранения и базу данных MABS (DPMDB) для сохранения точек восстановления.
  > * Вы должны иметь резервную резервную базу данных MABS (DPMDB). Вам понадобится восстановить базу данных.

1. На панели отображения выберите клиентские компьютеры, на которых требуется обновить агент защиты.
2. Выключите исходный сервер резервного копирования Azure или снимите его с провода.
...
6. Установка сервера резервного копирования Azure V3 или позже (перемещение дисков пула MABS storage со старого сервера и импорта)
...
10. Из командной строки администратора на компакт-диске нового сервера в Microsoft Azure Backup установите расположение и папку bin.
    ...
    Пример по патакжесу: C:'windows'system32>cd "c:"Файлы программы"Microsoft Azure Backup\"

11. Для резервного копирования Azure выполнить DPMSYNC -SYNC

    Если вы добавили новые диски в пул хранилища DPM вместо перемещения старых, запустите DPMSYNC-Reallocatereplica
...
При этом подписка Azure должна быть активна. Чтобы узнать состояние вашей подписки и управлять ею, войдите на [портал подписки.](https://account.windowsazure.com/Subscriptions)
...
Если у вас есть брандмауэр или прокси, который препятствует доступу к Azure, необходимо разрешить следующие адреса домена в профиле брандмауэра/прокси:
...
Если вы используете спиринга ExpressRoute Microsoft, пожалуйста, выберите следующие службы/регионы:
...
Для получения более подробной информации посетите [требования к маршруту ExpressRoute.](https://docs.microsoft.com/azure/expressroute/expressroute-routing)
...
Состояние подписки Azure можно изменить со *Срок действия истек* или *Отозвана* на *Активная*. Тем не менее, это имеет некоторые последствия для поведения продукта в то время как состояние не *является активным:*
...
5. Теперь можно начать защиту данных. При обновлении до Современного хранилища резервных копий во время защиты также можно выбрать тома, в которых необходимо хранить резервные копии, и проверять наличие неподготовленного пространства. Ознакомьтесь с [дополнительными сведениями](backup-mabs-add-storage.md).
...
Вы можете получить подробную информацию здесь о [подготовке вашей среды для DPM](https://docs.microsoft.com/system-center/dpm/prepare-environment-for-dpm?view=sc-dpm-2019). На этом сайте также содержатся сведения о поддерживаемых конфигурациях, при которых можно развернуть и использовать Azure Backup Server. Вы можете использовать ряд [командлетов PowerShell](https://docs.microsoft.com/powershell/module/dataprotectionmanager/?view=systemcenter-ps-2016) для выполнения различных операций.
