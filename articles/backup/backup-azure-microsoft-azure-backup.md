...
> [!NOTE]
> В Azure есть две модели развертывания для создания ресурсов и работы с ними: [Диспетчер ресурсов и Classic](../azure-resource-manager/management/deployment-models.md). Данная статья содержит сведения о восстановлении виртуальных машин, развернутых с использованием модели Resource Manager.
>
>
...
3. Пакет установки Azure Backup Server входит в состав необходимых SQL Server двоичных файлов. При запуске новой установки Azure Backup Server выберите параметр **установить новый экземпляр SQL Server с этой программой установки** и нажмите кнопку **проверить и установить** . После успешной установки необходимых компонентов нажмите кнопку **Далее**.

    >[!NOTE]
    >Если вы хотите использовать собственный SQL Server, Поддерживаемые версии SQL Server SQL Server 2014 с пакетом обновления 1 (SP1) и более поздних версий, 2016 и 2017.  Все версии SQL Server должны быть 64-разрядными уровня Standard или Enterprise.
    >Azure Backup Server не поддерживает использование удаленного экземпляра SQL Server. Экземпляр, который использует Azure Backup Server, должен быть локальным. Если вы используете существующий SQL Server для MABS, программа установки MABS поддерживает только использование *именованных экземпляров* SQL Server.
...
1. На консоль администратора сервере архивации выберите агенты **управления** > **Agents**.
...
Дополнительные сведения см. в описании [требований к маршрутизации ExpressRoute](https://docs.microsoft.com/azure/expressroute/expressroute-routing).
...
Подробные сведения о [подготовке среды для DPM](https://docs.microsoft.com/system-center/dpm/prepare-environment-for-dpm?view=sc-dpm-2019)можно получить здесь. На этом сайте также содержатся сведения о поддерживаемых конфигурациях, при которых можно развернуть и использовать Azure Backup Server. Вы можете использовать ряд [командлетов PowerShell](https://docs.microsoft.com/powershell/module/dataprotectionmanager/?view=systemcenter-ps-2016) для выполнения различных операций.
