...
3. В пакет установки Azure Backup Server включены нужные двоичные файлы SQL Server. При запуске новой установки Azure Backup Server выберите вариант **Установить новый экземпляр SQL Server во время этой установки** и нажмите кнопку **Проверить и установить**. После успешной установки необходимых компонентов нажмите кнопку **Далее**.
    ...
    В случае сбоя и появления рекомендации перезагрузить компьютер, выполните инструкцию на экране и нажмите кнопку **Проверить снова**. Если возникнут проблемы с конфигурацией SQL, перенастройте SQL согласно рекомендациям для SQL и повторите установку или обновление MABS, используя существующий экземпляр SQL.
   ...
   **Настройка вручную**
    ...
    * URL-адрес веб-службы. Для параметра Virtual Directory (Виртуальный каталог) нужно выбрать ReportServer_\<название_экземпляра_SQL>.
    * База данных: Для параметра DatabaseName (Имя базы данных) нужно выбрать ReportServer$\<имя_экземпляра_SQL>.
    * URL-адрес веб-портала. Для параметра Virtual Directory (Виртуальный каталог) нужно выбрать Reports_\<имя_экземпляра_SQL>.
...
* регион Microsoft Azure, где расположено хранилище Служб восстановления;
* служба хранилища Azure в том же расположении, что и хранилище Служб восстановления.

Подробнее см. в статье [Требования ExpressRoute к маршрутизации](https://docs.microsoft.com/azure/expressroute/expressroute-routing).
