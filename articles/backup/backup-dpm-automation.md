...
Чтобы получить список серверов, на которых установлен агент DPM и которые управляются сервером DPM, воспользуйтесь командлетом [Get-DPMProductionServer](/powershell/module/dataprotectionmanager/get-dpmproductionserver?view=systemcenter-ps-2019) . В этом примере мы отфильтруем и настроим только PowerShell с именем *ProductionServer01* для резервного копирования.
