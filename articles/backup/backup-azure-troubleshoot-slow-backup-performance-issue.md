...
При попытке резервного копирования больших дисков рекомендуется использовать [Azure Data Box](https://docs.microsoft.com/azure/backup/offline-backup-azure-data-box) для первой резервной копии (начальной репликации).  Если вы не можете использовать Data Box, то любые временные сетевые проблемы, происходящие в среде во время длительной передачи данных по сети, могут вызвать сбои резервного копирования.  Для защиты от этих сбоев можно включить несколько папок в начальную резервную копию и постепенно добавлять папки до тех пор, пока все они не будут успешно архивированы в Azure.  Последующие добавочные резервные копии будут создаваться относительно быстро.
