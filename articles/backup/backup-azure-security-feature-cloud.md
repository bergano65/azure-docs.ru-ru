...
Обратимое удаление по умолчанию включено для вновь созданных хранилищ. Если функция безопасности обратимого удаления отключена, данные резервного копирования не будут защищены от случайных или вредоносных удалений. Без функции обратимого удаления все удаления защищенных элементов приведут к немедленному удалению без возможности восстановления. Поскольку данные резервного копирования в состоянии "обратимого" удаления не несут никаких затрат на клиента, отключение этой функции не рекомендуется. Единственное обстоятельство, где следует отключать обратимое удаление, — это планирование перемещения защищенных элементов в новое хранилище и ожидание в течение 14 дней, необходимых перед удалением и повторной защитой (например, в тестовой среде).

### <a name="prerequisites-for-disabling-soft-delete"></a>Предварительные требования для отключения обратимого удаления

- Включение или отключение обратимого удаления для хранилищ (без защищенных элементов) может выполняться только портал Azure. Это относится к следующим:
  - Только что созданные хранилища, которые не содержат защищенные элементы
  - Существующие хранилища, защищенные элементы которых были удалены и истек срок их действия (за период хранения 14 дней);
- Если функция обратимого удаления для хранилища отключена, ее можно включить повторно, но вы не сможете отменить этот вариант и отключить его, если хранилище содержит защищенные элементы.
- Нельзя отключить обратимое удаление для хранилищ, содержащих защищенные элементы или элементы в обратимо удаленном состоянии. Если это необходимо, выполните следующие действия.
  - Отключение защиты удаленных данных для всех защищенных элементов.
  - Подождите, пока не истечет срок хранения в течение 14 дней.
  - Отключить обратимое удаление.

Чтобы отключить обратимое удаление, убедитесь, что выполнены необходимые условия, и выполните следующие действия.
...
Дополнительные сведения см. в статье [Шифрование службы хранилища Azure для неактивных данных](https://docs.microsoft.com/azure/storage/common/storage-service-encryption).
