...
| **Операционная система**                                 | **номер SKU**                                                      |
| -------------------------------------- | ------------------------------------------------------------ |
| **Рабочая станция**                        |                                                              |
...
* * Если вы планируете использовать более 80 ТБ начальных резервных копий данных для одного сервера MARS, обратитесь [AskAzureBackupTeam@microsoft.com](mailto:AskAzureBackupTeam@microsoft.com)к.
...
1. Запустите *файл marsagentinstaller. exe*и выполните *только* шаги по [установке и регистрации агента](https://docs.microsoft.com/azure/backup/install-mars-agent#install-and-register-the-agent) в хранилище служб восстановления, в котором необходимо хранить резервные копии.
...
1. Скачайте PSExec <https://download.sysinternals.com/files/PSTools.zip> с сервера с установленным агентом MARS.
...
1. На странице **обнаружение устройств Data Box** выберите пункт **обнаружить устройство** . Это действие выполняет проверку агента MARS для локально подключенных Azure Data Box дисков и обнаруживает их.
    ...
    >
    >Например, `\\mydomain\myserver\disk1\` если путь к диску — и *disk1* содержит каталог с именем *PageBlob*, путь, ВВЕДЕНный на странице мастера агента Mars, будет `\\mydomain\myserver\disk1\`иметь значение.
    >
...
>[!NOTE]
> Проверьте, не настроено `AzureOfflineBackup_<Azure User Id>` ли другое начальное заполнение в приложении, а также в случае, если другие серверы не зависят от этого приложения. Выберите **Параметры** > **ключи** в разделе **открытые ключи** . Не должны быть добавлены другие открытые ключи. Справочные сведения см. на следующем снимке экрана.
>
...
5. Перейдите к разделу реестра *Computer \ HKEY_LOCAL_MACHINE \Софтваре\микрософт\виндовс Azure баккуп\конфиг\клаудбаккуппровидер.* Щелкните правой кнопкой мыши **клаудбаккуппровидер**и добавьте новое строковое значение с именем `AzureADAppCertThumbprint_<Azure User Id>`.
    ...
    > - Перейдите по пути `Computer\HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows Azure Backup\DbgSettings\OnlineBackup` реестра с именем *куррентусерид*.
...
Для получения любых вопросов и уточнений о любых возникших проблемах обратитесь [AskAzureBackupTeam@microsoft.com](mailto:AskAzureBackupTeam@microsoft.com)к.
