...
Мгновенное восстановление предполагает восстановление компьютера непосредственно из моментального снимка резервной копии, а не копии моментального снимка в хранилище. Мгновенное восстановление выполняется быстрее, чем восстановление из хранилища. Количество доступных точек мгновенного восстановления зависит от длительности хранения, настроенной для моментальных снимков.
...
Восстановление элемента до его состояния в определенный момент времени (СМОЛой).
...
## <a name="recovery-point-restore-point-retention-point"></a>Точка восстановления, точка восстановления и точка хранения
...
RPO указывает максимальную неприемлемую потери данных в сценарии потери данных. Это определяется периодичностью резервного копирования.

## <a name="rto-recovery-time-objective"></a>RTO (целевое время восстановления)

RTO указывает максимально допустимое время, в течение которого данные могут быть восстановлены до последней доступной точки во времени после потери данных.
...
Моментальный снимок — это полная копия виртуального жесткого диска, доступная только для чтения. [Подробнее](https://docs.microsoft.com/azure/virtual-machines/windows/snapshot-copy-managed-disk).
...
Файл учетных данных хранилища — это сертификат, созданный порталом для каждого хранилища. Используется при регистрации сервера в хранилище. [Подробнее](backup-azure-dpm-introduction.md).
