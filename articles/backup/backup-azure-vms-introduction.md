...
## <a name="limited-public-preview-backup-of-vm-with-disk-sizes-up-to-30-tb"></a>Ограниченная общедоступная Предварительная версия: Резервная копия виртуальной машины с размером диска до 30 ТБ

Azure Backup теперь поддерживает ограниченную общедоступную предварительную версию более крупных и более мощных [управляемых дисков Azure](https://azure.microsoft.com/blog/larger-more-powerful-managed-disks-for-azure-virtual-machines/) размером до 30 ТБ. Эта предварительная версия обеспечивает поддержку на уровне рабочего уровня для управляемых виртуальных машин.

Вы можете легко зарегистрироваться в предварительной версии без каких бы то ни было последствий с текущими резервными копиями. После регистрации подписки в предварительной версии необходимо успешно создать резервную копию всех виртуальных машин с размером диска до 30 ТБ. Чтобы зарегистрироваться в предварительной версии:
...
2. Выберите подписку, которую необходимо зарегистрировать для обновления:
...
3. Зарегистрируйте эту подписку в программе предварительной версии: 
    ...
    Подождите 30 минут, пока подписка не будет зарегистрирована в предварительной версии. 

 4. Чтобы проверить состояние, выполните следующие командлеты:

    ```powershell
    PS C:> Get-AzureRmProviderFeature -FeatureName "LargeDiskVMBackupPreview" –ProviderNamespace Microsoft.RecoveryServices 
    ```
5. Когда подписка отобразится как зарегистрированная, выполните следующую команду:
...
> [!NOTE]
> В этой предварительной версии не поддерживаются зашифрованные виртуальные машины с дисками размером более 4 ТБ.
