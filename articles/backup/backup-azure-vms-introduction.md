...
4. Расширение резервного копирования создает моментальный снимок уровня хранилища, согласованный с приложением.
...
Служба пытается выполнить резервное копирование максимально быстро, используя все доступные ресурсы.

- Чтобы максимально ускорить процесс, операция резервного копирования пытается параллельно создавать резервные копии всех дисков виртуальной машины.
- Например, если у виртуальной машины четыре диска, то служба попытается выполнить резервное копирование всех четырех дисков одновременно.
- Количество дисков, для которых выполняется резервное копирование, — это основной фактор, определяющий объем трафика резервного копирования для этой учетной записи хранения.
- Все операции ввода-вывода ограничены *целевой пропускной способностью для одного большого двоичного объекта*. Предельное значение пропускной способности составляет 60 МБ в секунду.
- Для каждого диска, для которого создается резервная копия, служба архивации Azure считывает блоки на диске и сохраняет только измененные данные (добавочное резервное копирование). Используя приведенные ниже усредненные данные по пропускной способности, вы можете примерно рассчитать время архивации для диска заданного размера.

    **Операция** | **Пропускная способность в лучшем случае**
    --- | ---
    Начальное резервное копирование | 160 Мбит/с |
    Добавочное резервное копирование | 640 Мбит/с  <br><br> Пропускная способность существенно снижается, если измененные данные на диске в значительной степени разрозненны.|
...
- Если вы настраиваете политики таким образом, что резервное копирование всех виртуальных машин выполняется одновременно, у вас возникнет перегрузка по трафику, поскольку процесс резервного копирования будет обрабатывать все диски параллельно.
...
- **Начальное резервное копирование нового диска, добавляемого к уже защищенной виртуальной машине**. Если выполняется добавочное резервное копирование виртуальной машины, при добавлении нового диска соглашение об уровне обслуживания, составляющем один день, может быть не соблюдено, в зависимости от размера нового диска.
- **Фрагментированное приложение**. Неправильная конфигурация приложения может сделать его неоптимальным для хранения.
    - Если моментальный снимок содержит много фрагментированных операций записи малого размера, то службе требуется дополнительное время на обработку данных, записанных приложениями.
    - Для приложений, работающих внутри виртуальной машины, рекомендуемый минимальный размер блока-записи составляет 8 КБ. Если приложение использует блок объемом менее 8 КБ, то это негативно влияет на производительность резервного копирования.
- **Перегруженные учетные записи хранения**. Резервное копирование может выполняться по расписанию в то время, когда приложение выполняется в рабочей среде, а также в случаях, когда в одной учетной записи хранения размещается больше чем 5–10 дисков.
- **Режим проверки согласованности**. Для дисков объемом свыше 1 ТБ резервное копирование по ряду причин может выполняться в режиме проверки согласованности.
    - Управляемый диск перемещается в рамках перезапуска виртуальной машины.
    - Преобразование моментального снимка в базовый большой двоичный объект.
...
- Планируйте резервное копирование виртуальных машин на периоды низкой нагрузки. Таким образом, служба резервного копирования получает достаточную скорость ввода-вывода для передачи данных из учетной записи хранения в хранилище.
- Если выполняется резервное копирование управляемых дисков, служба Azure Backup осуществляет управление хранением. Если выполняется резервное копирование неуправляемых дисков
    - Примените политику резервного копирования к виртуальным машинам, распределенным по нескольким учетным записям хранения.
    - В одно расписание резервного копирования не следует включать более 20 дисков из одной учетной записи хранения.
    - Если в вашей учетной записи хранения больше 20 дисков, разделите эти виртуальные машины между несколькими политиками. Так вы обеспечите достаточное количество операций ввода-вывода для этапа передачи данных при резервном копировании.
...
- Для восстановления виртуальных машин из одного хранилища рекомендуется использовать разные учетные записи хранения вместо одной. Это позволяет исключить необходимость регулирования и гарантировать 100%-ю успешность восстановления с хорошей производительностью.
- Восстановление с уровня 1 хранилища будет занимать считаные минуты, тогда как для уровня 2 этот процесс может длиться часы. Для более быстрого восстановления мы рекомендуем использовать [функцию Instant RP](backup-upgrade-to-vm-backup-stack-v2.md). Это относится только к управляемым виртуальным машинам Azure.
...
- Скачайте [электронную таблицу Excel для планирования ресурсов](https://gallery.technet.microsoft.com/Azure-Backup-Storage-a46d7e33) и попробуйте рассчитать объем дисков и расписание резервного копирования.
