...
## <a name="restore-prerequisites"></a>Восстановление предварительных требований
...
- Если на сервере работает несколько экземпляров, все эти экземпляры должны быть настроены и работать. В противном случае сервер не будет отображаться в списке целевых серверов для восстановления базы данных. Дополнительные сведения см. в разделе [действия по устранению неполадок](backup-sql-server-azure-troubleshoot.md#faulty-instance-in-a-vm-with-multiple-sql-server-instances).
