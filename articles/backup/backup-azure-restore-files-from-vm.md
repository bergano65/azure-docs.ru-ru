...
Команда ```lvdisplay``` также показывает, являются ли группы томов активными. Если группа томов помечена как неактивная, ее необходимо повторно активировать для подключения. Если группа томов отображается как неактивная, выполните следующую команду, чтобы активировать ее.
...
Когда имя группы томов будет активным, выполните команду ```lvdisplay``` повторно, чтобы просмотреть все соответствующие атрибуты.
...
> [!NOTE]
>
> - Имя скачанного файла скрипта будет содержать **географическое имя**, которое необходимо указать в URL-адресе. Например: Имя скачанного скрипта начинается с \'имя_виртуальной_машины\'\_\'географическое_имя\'_\'уникальный_идентификатор\', например *ContosoVM_wcus_12345678*.
...
В этом разделе объясняется, как выполнять восстановление файлов из резервных копий виртуальных машин Azure с более чем 16 дисками с объемом каждого диска более 32 ТБ.

Так как в процессе восстановления файлов присоединяются все диски из резервной копии, при использовании большого числа дисков (> 16) или больших дисков (> 32 ТБ каждый) рекомендуется использовать следующие точки действий:
