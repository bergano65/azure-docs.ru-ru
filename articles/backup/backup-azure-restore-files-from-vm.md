...
| Выходные данные exe: *этот скрипт является недопустимым, так как диски были отключены через портал/превышен предел в 12 ч. Скачайте новый скрипт с портала.* |    Диски были отключены от портала или превышено 12-часовое ограничение | Эта копия исполняемого файла больше недействительна, и использовать ее нельзя. Если вы хотите получить доступ к файлам этой точки восстановления, посетите портал для получения нового exe-файла.|
