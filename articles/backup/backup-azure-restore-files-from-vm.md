...
![Рабочий процесс восстановления папки файлов](./media/backup-azure-restore-files-from-vm/file-recovery-1.png)

## <a name="step-1-generate-and-download-script-to-browse-and-recover-files"></a>Шаг 1. Создание и скачивание скрипта для просмотра и восстановления файлов

Чтобы восстановить файлы или папки из точки восстановления, перейдите к виртуальной машине и выполните следующие действия.
...
5. Выберите **скачать исполняемый файл** (для виртуальных машин Windows Azure) или **загрузить скрипт** (для виртуальных машин Azure Linux создается скрипт Python), чтобы загрузить программное обеспечение, используемое для копирования файлов из точки восстановления.
...
## <a name="step-2-ensure-the-machine-meets-the-requirements-before-executing-the-script"></a>Шаг 2. Проверка соответствия компьютера требованиям перед выполнением сценария

После успешной загрузки скрипта убедитесь, что у вас есть правильный компьютер для выполнения этого сценария. Виртуальная машина, в которой вы планируете выполнить сценарий, не должна иметь ни одной из следующих неподдерживаемых конфигураций. Если это так, выберите альтернативный компьютер, предпочтительный в том же регионе, который соответствует требованиям.  

### <a name="dynamic-disks"></a>Динамические диски

Невозможно запустить исполняемый сценарий на виртуальной машине с любой из следующих характеристик:

- Тома, охватывающие несколько дисков (составные и чередующиеся тома).
- Отказоустойчивые тома (тома с зеркальным отображением и томов RAID 5) на динамических дисках.

### <a name="windows-storage-spaces"></a>Дисковое пространство Windows

Невозможно запустить скачанный исполняемый файл на виртуальной машине, настроенной для дисковых пространств Windows.

### <a name="virtual-machine-backups-having-large-disks"></a>Резервные копии виртуальных машин с большими дисками

Если на резервном компьютере имеется большое количество дисков (>16) или больших дисков (каждый > 4 ТБ), не рекомендуется выполнять этот сценарий на том же компьютере для восстановления, так как он оказывает значительное влияние на виртуальную машину. Вместо этого рекомендуется использовать отдельную виртуальную машину только для восстановления файлов (виртуальные машины Azure D2v3 VM), а затем завершать работу, когда это не требуется. 

## <a name="step-3-os-requirements-to-successfully-run-the-script"></a>Шаг 3. требования к ОС для успешного выполнения скрипта

Виртуальная машина, на которой нужно запустить скачанный скрипт, должна соответствовать следующим требованиям.
...
## <a name="step-4-access-requirements-to-successfully-run-the-script"></a>Шаг 4. требования к доступу для успешного выполнения скрипта
...
> [!NOTE]
>
> Файл скрипта, скачанный на шаге 1 [выше](#step-1-generate-and-download-script-to-browse-and-recover-files) , будет содержать **географическое имя** в имени файла. Используйте это **географическое имя** для заполнения URL-адреса. Имя скачанного скрипта будет начинаться с: \' VMname \' \_ \' геоимя \' _ \' GUID \' .<br><br>
...
## <a name="step-5-running-the-script-and-identifying-volumes"></a>Шаг 5. Запуск скрипта и определение томов

### <a name="for-windows"></a>Для Windows

После выполнения всех требований, перечисленных в шаге 2, шаг 3 и 4, скопируйте сценарий из скачанного расположения (обычно это папка downloads), щелкните исполняемый файл или скрипт правой кнопкой мыши и запустите его с учетными данными администратора. При появлении запроса введите пароль или вставьте его из памяти и нажмите клавишу ВВОД. После ввода пароля сценарии подключаются к точке восстановления.
...
#### <a name="for-backed-up-vms-with-large-disks-windows"></a>Для резервных копий виртуальных машин с большими дисками (Windows)
...
Для компьютеров Linux создается скрипт Python. Скачайте сценарий и скопируйте его на соответствующий или совместимый сервер Linux. Вам может потребоваться изменить разрешения, чтобы выполнить его с помощью ```chmod +x <python file name>```. Затем запустите файл Python с помощью ```./<python file name>```.
...
#### <a name="for-backed-up-vms-with-large-disks-linux"></a>Для резервных копий виртуальных машин с большими дисками (Linux) * *

Если процесс восстановления файла зависает после запуска скрипта восстановления файлов (например, если диски не подключены или подключены, но тома не отображаются), выполните следующие действия.

1. В файле /etc/iscsi/iscsid.conf измените настройку с:
    - `node.conn[0].timeo.noop_out_timeout = 5`  Кому `node.conn[0].timeo.noop_out_timeout = 120`
2. После внесения описанных выше изменений перезапустите сценарий. При наличии временных сбоев убедитесь, что промежуток от 20 до 30 минут между повторным выполнением, чтобы избежать появления последовательностей запросов, влияющих на целевую подготовку. Этот интервал между повторными запусками обеспечит готовность целевого объекта к подключению из скрипта.
3. После восстановления файлов убедитесь, что вы вернитесь на портал и выберите **отключить диски** для точек восстановления, в которых не удалось подключить тома. По сути этот шаг позволяет очистить существующие процессы или сеансы и повысить вероятность восстановления.


#### <a name="lvmraid-arrays-for-linux-vms"></a>Массивы LVM/RAID (для виртуальных машин Linux)

В Linux диспетчеры логических томов (LVM) и (или) программные массивы RAID используются для управления логическими томами на нескольких дисках. Если защищенная виртуальная машина Linux использует LVM и/или RAID-массивы, на ней нельзя запустить сценарий.<br>
Вместо этого запустите сценарий на любом другом компьютере с совместимой операционной системы, поддерживающей файловую систему защищенной виртуальной машины.<br>
...
## <a name="step-6-closing-the-connection"></a>Шаг 6. Закрытие подключения
...
После отключения дисков вы получите сообщение. На обновление состояния подключения может потребоваться несколько минут, после чего диски можно удалить.
...
Поток данных между службой восстановления и компьютером защищается путем создания защищенного туннеля TLS по протоколу TCP (на компьютере, где выполняется скрипт [должен поддерживаться TLS 1.2](#step-3-os-requirements-to-successfully-run-the-script)).
