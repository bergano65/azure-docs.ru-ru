...
7. Из расположения скачивания (обычно папка "Загрузки") щелкните исполняемый файл или сценарий правой кнопкой мыши и запустите его с учетными данными администратора. При появлении запроса введите пароль или вставьте его из памяти и нажмите клавишу ВВОД. После ввода пароля сценарии подключаются к точке восстановления.
    ...
    - URL-адреса службы восстановления (географическое имя относится к региону, в котором находится хранилище службы восстановления) - <https://pod01-rec2.geo-name.backup.windowsazure.com> (для Azure Public геообъектов) - <https://pod01-rec2.geo-name.backup.windowsazure.cn> (для Azure для государственных организаций) - <https://pod01-rec2.geo-name.backup.windowsazure.us> (для правительства Azure США) - <https://pod01-rec2.geo-name.backup.windowsazure.de> (для Azure для Германии)
    - исходящий порт 3260.

> [!Note]
>
> - Имя скачанного файла скрипта будет содержать **географическое имя** , которое будет заполнено URL-адресом. Для сценария например, имя скачанного скрипта начинается с \'VMname\'\_\'геоимя\'_\'GUID\', например ContosoVM_wcus_12345678
...
В этом разделе объясняется, как выполнить восстановление файлов из резервных копий виртуальных машин Azure, количество дисков в которых > 16, а размер каждого диска — > 32 ТБ.
...
- Если сервер восстановления является виртуальной машиной Linux
...
- После выполнения следующей операции запустите сценарий еще раз. С учетом этих изменений очень вероятно, что восстановление файла завершилось с ошибкой.
- Каждый раз, когда пользователь скачивает сценарий, Azure Backup инициирует процесс подготовки точки восстановления к скачиванию. При использовании больших дисков это займет значительное время. При наличии последовательных пакетов запросов целевая подготовка будет идти в спирали. Поэтому рекомендуется скачать скрипт с портала, PowerShell или интерфейса командной строки, подождать 20-30 мин (эвристика) и запустить ее. На этот раз целевой объект должен быть готов к подключению из скрипта.
- После восстановления файла убедитесь, что на портале выбран пункт "отключить диски" для точек восстановления, на которых не удалось подключить тома. По сути, этот шаг позволяет очистить существующие процессы и сеансы и повысить вероятность восстановления.
...
| Выходные данные exe: *этот скрипт является недопустимым, так как диски были отключены через портал/превышен предел в 12 ч. Скачайте новый скрипт с портала.* |    Диски отключены через портал или превышено 12-часовое ограничение. | Эта копия исполняемого файла больше недействительна, и использовать ее нельзя. Если вам еще нужен доступ к файлам этой точки восстановления на определенный момент времени, зайдите на портал и скачайте новый EXE-файл.|
...
| В Windows: не удается просмотреть нужные тома | Диски присоединены, но тома не настроены | На экране управления дисками определите дополнительные диски, относящиеся к точке восстановления. Если какой-либо из этих дисков находится в автономном состоянии, попробуйте сделать это в сети, щелкнув диск правой кнопкой мыши и выбрав пункт "в сети".|

## <a name="security"></a>Безопасность

В этом разделе обсуждаются различные меры безопасности, предпринятые для реализации восстановления файлов из резервных копий виртуальных машин Azure, чтобы пользователи осведомлены о аспектах безопасности этой функции.

### <a name="feature-flow"></a>Поток функций

Эта функция была создана для доступа к данным виртуальной машины без необходимости восстановления всей виртуальной машины или дисков виртуальной машины, а также минимальных действий. Доступ к данным виртуальной машины предоставляется сценарием (который монтирует том восстановления при запуске, как показано ниже) и, следовательно, является основой для всех реализаций безопасности.
