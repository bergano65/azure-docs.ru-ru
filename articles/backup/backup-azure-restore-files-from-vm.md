...
Существуют сценарии, в которых имена групп томов могут иметь 2 UUID после выполнения сценария. Это означает, что имена групп томов на компьютере, где выполняется сценарий, и на виртуальной машине, на которой они выполняются, совпадают. Затем необходимо переименовать группы томов с резервными копиями. Рассмотрим приведенный ниже пример.
...
Чтобы выполнить скрипт, требуется пароль, который отображается только для полномочного пользователя во время создания скрипта на портале Azure, в PowerShell или CLI. Это необходимо для того, чтобы полномочный пользователь, загружающий скрипт, также отвечал за выполнение сценария.
