...
5. Щелкните **Скачать исполняемый файл** (для виртуальной машины Windows в Azure) или **Скачать скрипт** (для виртуальной машины Linux в Azure создается скрипт Python), чтобы скачать программное обеспечение для копирования файлов из точки восстановления.
...
> [!Note]
> Обнаружены некоторые проблемы при выполнении сценарий восстановления файлов на компьютерах с ОС SP4 SLES 12. Исследование с группой SLES.
> В настоящее время, запуск сценария восстановления файла работает на компьютерах с версиями SLES 12 SP2 и SP3 ОС.
>
...
| ------------------------ | -------------- | ------------------ |
| Выходные данные EXE-файла: *Exception connecting to the target* (Исключение при подключении к целевому объекту) |Скрипт не может получить доступ к точке восстановления    | Проверьте, соответствует ли компьютер всем перечисленным выше требованиям. |  
...
| Выходные данные EXE-файла: сценарий выполняется успешно, но в его выходных данных нет сообщения New volumes attached (Новые тома подключены). |    Это временная проблема    | Тома, скорее всего, уже подключены. Попробуйте открыть их через проводник. Если вы несколько раз запускали скрипт на одном и том же компьютере, попробуйте перезагрузить компьютер. Список будет отображаться при последующих запусках EXE-файла. |
...
В этом разделе рассматриваются различные меры безопасности, предлагаемые для реализации восстановление файлов из резервных копий виртуальных Машин Azure, таким образом, что пользователи знают аспектам безопасности данного компонента.

### <a name="feature-flow"></a>Функция потока

Эта функция была создана для доступа к данным виртуальной Машины без необходимости восстановить всю виртуальную Машину или виртуальной Машине диски и минимальные действия. Доступ к данным виртуальной Машины предоставляется с помощью сценария (который монтирует том восстановления, при запуске, как показано ниже) и поэтому он формирует краеугольным камнем все реализации безопасности

  ![Поток компонент системы безопасности](./media/backup-azure-restore-files-from-vm/vm-security-feature-flow.png)

### <a name="security-implementations"></a>Реализации безопасности

#### <a name="select-recovery-point-who-can-generate-script"></a>Выберите точку восстановления (который можно создать скрипт)

Этот сценарий предоставляет доступ к данным виртуальной Машины, важно контролировать, кто может создавать в первую очередь. Один выполнить вход на портал Azure и должно быть [RBAC авторизованных](backup-rbac-rs-vault.md#mapping-backup-built-in-roles-to-backup-management-actions) возможность сформировать скрипт.

Восстановление файлов должен тот же уровень авторизации, требуемые для восстановления виртуальной Машины и восстановления дисков. Другими словами только для авторизованных пользователей может представление данных виртуальной Машины можно сформировать скрипт.

Созданный скрипт подписывается с помощью официальный сертификат Microsoft для службы архивации Azure. Любой подделки скрипт означает, что подписи, и любая попытка запустить сценарий выделяется как потенциальный риск Операционной системой.

#### <a name="mount-recovery-volume-who-can-run-script"></a>Том восстановления подключения, (который можно запустить сценарий)

Только администратор может запустить сценарий и следует запускать в режиме с повышенными правами. Только скрипт выполняется предварительно созданный набор шагов и не принимает входные данные из внешних источников.

Для запуска скрипта, один требуется пароль, который доступен только для авторизованного пользователя во время создания сценария в портале Azure или PowerShell или интерфейса командной строки. Это гарантирует, что авторизованного пользователя, который загружает скрипт также отвечает за запуск сценария.

#### <a name="browse-files-and-folders"></a>Просматривать файлы и папки

Чтобы просматривать файлы и папки, скрипт использует инициатор iSCSI на компьютере и подключиться к точке восстановления, который настроен в качестве целевого объекта iSCSI. Здесь можно предположить, что сценарии, где один пытается имитировать подделка либо или всех компонентов.

Мы используем взаимной механизм проверки подлинности CHAP, таким образом, проверяет подлинность каждого компонента, с другой. Это означает, что очень сложно фальшивые инициатора, для подключения к целевому объекту iSCSI и фиктивный целевой на подключенном к компьютеру, где он выполняется.

Поток данных между службой восстановления и компьютер защищен, создав защищенный туннель SSL по TCP ([TLS 1.2 должен поддерживаться](#system-requirements) на компьютере, где выполняется сценарий)

Любой файл элемента управления списка управления Доступом в родительский/резервная копия виртуальной Машины также сохраняются в подключенном файловой системе.

Этот сценарий предоставляет доступ только для чтения до точки восстановления и действителен в течение всего 12 часов. Если пользователь хочет удалить доступ к ранее, затем войдите в портал или PowerShell или интерфейса командной строки Azure и выполнять **отключите диски** для этой определенной точки восстановления. Сразу же станут недействительными скрипт.
