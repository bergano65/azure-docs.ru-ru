...
author: pvrk
manager: shivamg
keywords: PowersShell; Резервное копирование файлов Azure; Восстановление файлов Azure;
ms.service: backup
ms.topic: conceptual
ms.date: 11/12/2018
ms.author: pullabhk
...
Если вы не знакомы со службой архивации Azure, общие сведения о службе см. в статье [Что такое служба архивации Azure?](backup-introduction-to-azure-backup.md) Перед началом убедитесь, что вы приняли к сведению возможности предварительного просмотра резервных копий общих файловых ресурсов Azure, задокументированных [здесь](backup-azure-files.md).
...
2. Чтобы получить список доступных командлетов PowerShell для службы архивации Azure, введите следующую команду:
    ...
    ![Список служб восстановления](./media/backup-azure-afs-automation/list-of-recoveryservices-ps-az.png)
...
В следующем примере с помощью политики dailyafs включается защита для файлового ресурса Azure testAzureFileShare в разделе учетной записи хранения testStorageAcct.
...
Мы используем преимущества моментальных снимков общего ресурса файлов Azure при создании резервных копий и поэтому задание, как правило, завершается к тому времени, когда команда возвращает выходные данные.

### <a name="modify-protection-policy"></a>Изменение политики защиты

Если вы хотите изменить политику, с помощью которой защищен файловый ресурс Azure, используйте **Enable-AzRecoveryServicesBackupProtection** с соответствующим элементом резервного копирования и новую политику защиты.

В следующих примерах политика защиты testAzureFS изменяется с dailyafs на monthlyafs.
...
## <a name="restore-azure-file-sharesazure-files"></a>Восстановление файлов и файловых ресурсов Azure
...
### <a name="fetching-recovery-points"></a>Получение точек восстановления
...
#### <a name="restoring-an-azure-file-share"></a>Восстановление файловых ресурсов Azure
...
* ***TargetFileShareName***. Файловые ресурсы находятся в учетной записи хранения, в которую восстанавливается резервное содержимое.
...
#### <a name="restoring-an-azure-file"></a>Восстановление файла Azure
...
* ***TargetFileShareName***. Файловые ресурсы находятся в учетной записи хранения, в которую восстанавливается резервное содержимое.
* ***TargetFolder***. Папка в файловом ресурсе, в которую восстанавливаются данные. Если содержимое резервной копии необходимо восстановить в корневую папку, укажите значения целевой папки в виде пустой строки.
* ***SourceFilePath***. Абсолютный путь к файлу, который будет восстановлен в общей папке, в виде строки. Этот же путь используется в командлете PS ```Get-AzStorageFile```.
* ***SourceFileType***. Выбирается или каталог, или файл. Принимает "папка" или "файл".
* ***ResolveConflict***. Инструкции в случае конфликта с восстановленными данными. Принимает "перезаписать" или "пропустить".

Как вы видите, дополнительные параметры связаны только с отдельным файлом для восстановления.
...
### <a name="restore-azure-file-shares-to-original-location"></a>Восстановление файловых ресурсов Azure в исходное расположение

В случае восстановления в исходное расположение все параметры, связанные с назначением и целью, указывать не обязательно. Нужно указать только ```ResolveConflict```.
...
По запросу операции резервного копирования и восстановления возвращают задание наряду с идентификатором, как показано [выше](#trigger-an-on-demand-backup). Используйте командлет ```Get-AzRecoveryServicesBackupJobDetails```, чтобы отслеживать выполнение задания и получать дополнительные сведения.
