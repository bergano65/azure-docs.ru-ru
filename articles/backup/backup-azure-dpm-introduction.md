...
[System Center DPM](https://docs.microsoft.com/system-center/dpm/dpm-overview) производит резервное копирование данных файлов и приложений. DPM взаимодействует со службой архивации Azure по описанной ниже схеме.
...
- Убедитесь, что файл сохранен в расположении, к которому можно получить доступ с сервера DPM. Если они сохраняются на файловый ресурс, доступ к которому осуществляется по протоколу SMB, убедитесь, что у вас есть необходимые разрешения на доступ к этому ресурсу.
...
4. В окне **Свойства**  >  **учетные данные архивации**нажмите кнопку **скачать**. Портал создаст учетные данные хранилища, используя сочетание имени хранилища и текущей даты, и предоставит их вам для скачивания.

    ![Скачать](./media/backup-azure-dpm-introduction/vault-credentials.png)
...
7. Щелкните **Зарегистрировать**, чтобы запустить процесс регистрации сервера DPM в хранилище.

Когда регистрация сервера в хранилище успешно завершится, вы можете начинать резервное копирование данных в Microsoft Azure. Для резервного копирования рабочих нагрузок в Azure необходимо настроить группу защиты в консоли DPM. [Узнайте, как](https://docs.microsoft.com/system-center/dpm/create-dpm-protection-groups?view=sc-dpm-2019) развертывать группы защиты.
