...
Чтобы повысить эффективность хранилища резервных копий, в Backup Server версии 2 добавлена поддержка томов. Экземпляры Backup Server версии 1 и 2 поддерживают диски.
...
При добавлении тома в Backup Server вы можете присвоить ему понятное имя. Выберите столбец **Понятное имя** для тома, которому необходимо присвоить имя. При необходимости имя можно изменить позже. Вы также можете использовать PowerShell, чтобы добавлять или изменять понятные имена томов.
...
Чтобы обновить свойства тома в пуле хранилищ Backup Server, используйте командлет PowerShell **Update-DPMDiskStorage**.
...
5. На странице **Выбор элементов группы** в поле **Доступные элементы** перечислены элементы с агентами защиты. Для этого примера выберите тома D:\ и E:\, и добавьте их в область **Выбранные элементы**. Щелкните **Далее**.

  ![Страница "Выбор элементов группы"](./media/backup-mabs-upgrade-to-v2/create-a-protection-group-3.png)

6. На странице **Выбор метода защиты данных** введите имя в поле **Имя группы защиты**, выберите необходимый метод защиты и нажмите кнопку **Далее**. Если требуется краткосрочная защита, выберите метод резервного копирования с помощью **диска**.
...
8. На странице **Проверка выделения дискового накопителя** просмотрите сведения о выбранных источниках данных, включая их размер, значения для подготавливаемого пространства и целевой том хранилища.
...
После установки Backup Server версии 2 или обновления до этой версии, а затем обновления операционной системы до Windows Server 2016, обновите группы защиты, чтобы использовать Modern Backup Storage. По умолчанию группы защиты не изменяются. Они будут работать согласно изначальным настройкам. 
...
3. В списке доступных дисков выберите диски, которые требуется добавить. Выберите **Добавить**, а затем нажмите кнопку **ОК**.
...
В следующих разделах объясняется, как обновить агенты защиты для клиентских компьютеров, подключенных и не подключенных к сети.
...
4. На клиентском компьютере, не подключенном к сети, в столбце **Состояние агента** будет отображаться элемент **Обновить ожидание**, пока компьютер не подключится к сети.
  ...
  Когда клиентский компьютер подключен к сети, в столбце **Обновления агентов** для клиентского компьютера отобразится состояние **Обновление**.
  
### <a name="move-legacy-protection-groups-from-the-old-version-and-sync-the-new-version-with-azure"></a>Перемещение старых групп защиты из старой версии и синхронизация новой версии с помощью Azure

Когда Azure Backup Server и операционная система будут обновлены, все готово для защиты новых источников данных с помощью Modern Backup Storage. Источники данных, которые уже защищены, остаются защищенными и в Azure Backup Server (прежняя версия). Все новые группы защиты используют Modern Backup Storage.

Чтобы перенести источники данных из устаревшего режима защиты в Modern Backup Storage:

1.  Добавьте новые тома в пул хранилищ Data Protection Manager. Можно также назначить понятное имя и выбрать теги для источника данных.

2. Остановите защиту каждого источника данных, который находится в старом режиме. Затем выберите **Сохранить защищенные данные**.  Так можно восстановить старые точки восстановления после миграции.

3. Создайте группу защиты. Затем выберите источники данных, которые необходимо сохранить в новом формате.

  Data Protection Manager сохранит локальную копию реплики старого хранилища резервных копий на томе Modern Backup Storage.
    > [!NOTE] 
    > Создание копии отображается как задание операций, выполняемых после восстановления.

  Все новые точки синхронизации и восстановления будут храниться в Modern Backup Storage.

  После истечения срока действия старые точки восстановления будут удалены. Таким образом, место на диске освободится.

  Когда все старые тома будут удалены из старого хранилища, диск можно удалить из Azure Backup. Затем его можно удалить из системы.

4. Создайте резервную копию базы данных Data Protection Manager.

  > [!IMPORTANT]
  > - Для нового имени сервера используется такое же имя, как и у исходного экземпляра Azure Backup Server. Нельзя изменить имя нового экземпляра Azure Backup Server, если вы хотите использовать предыдущий пул хранилищ и базу данных Data Protection Manager для сохранения точек восстановления.
  > - Нужно иметь резервную копию базы данных Data Protection Manager. Вам понадобится восстановить базу данных.

5. Завершите работу исходного экземпляра Azure Backup Server или переведите сервер в автономный режим.

6. Сбросьте учетную запись компьютера в Active Directory.

7. Установите Windows Server 2016 на новый компьютер. Для имени сервера используется такое же имя компьютера, как и у исходного экземпляра Azure Backup Server.

8. Присоединитесь к домену.

9. Установите Azure Backup Server версии 2. (Удалите диски пула хранилищ Data Protection Manager со старого сервера и импортируйте их на новый.)

10. Восстановите базу данных Data Protection Manager, созданную на шаге 4.

11. Подключите хранилища из исходного сервера Backup Server к новому серверу.

12. В SQL Server восстановите базу данных Data Protection Manager.

13. В командной строке администрирования на новом сервере используйте `cd`, чтобы перейти к расположению папки с установкой Microsoft Azure Backup и папке bin.  

  Пример:  
  путь к Azure Backup — C:\windows\system32>cd "c:\Program Files\Microsoft Azure Backup\DPM\DPM\bin\.

14. Запустите `DPMSYNC -SYNC`.
  
  > [!NOTE]
  > Если вы добавили *новые* диски в пул хранилищ Data Protection Manager вместо перемещения старых, запустите `DPMSYNC -Reallocatereplica`.

## <a name="new-powershell-cmdlets-in-backup-server-v2"></a>Новые командлеты PowerShell в Backup Server версии 2
