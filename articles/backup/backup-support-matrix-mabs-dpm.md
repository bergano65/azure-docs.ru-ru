...
- [Резервную копию сервера-источника DPM можно создать с помощью дополнительного сервера DPM](https://docs.microsoft.com/system-center/dpm/back-up-the-dpm-server?view=sc-dpm-2019). Сервер-получатель будет защищать базы данных сервера-источника и реплики источников данных, хранящиеся на сервере-источнике. В случае сбоя сервера-источника сервер-получатель может продолжать защищать рабочие нагрузки, защищаемые сервером-источником, до тех пор, пока сервер-источник снова не станет доступным.  В MABS такая возможность не предоставляется.
