...
**Deployment** | **Поддержка** | **Сведения**
--- | --- | ---
**В локальной среде** | Физический сервер<br/><br/>Виртуальная машина Hyper-V<br/><br/> Виртуальная машина VMware | Дополнительные сведения см. в [матрице защиты](backup-mabs-protection-matrix.md) . 
...
**Сценарий** | **DPM/MABS**
--- | ---
**MABS на виртуальной машине Azure** |  Windows 2016 Datacenter.<br/><br/> Windows 2019 Datacenter.<br/><br/> Рекомендуется начать с образа из Marketplace.<br/><br/> Минимальное Standard_A4_v2 с четырьмя ядрами и 8 ГБ ОЗУ.
**DPM на виртуальной машине Azure** | System Center 2012 R2 с пакетом обновления 3 или более поздней версии.<br/><br/> Операционная система Windows, [соответствующая требованиям System Center](/system-center/dpm/prepare-environment-for-dpm#dpm-server).<br/><br/> Рекомендуется начать с образа из Marketplace.<br/><br/> Минимальное Standard_A4_v2 с четырьмя ядрами и 8 ГБ ОЗУ.
**MABS в локальной среде** |  MABS v3 и более поздние версии: Windows Server 2016 или Windows Server 2019
...
**.NET Framework в MABS** | Для виртуальной машины MABS требуется .NET Framework 3,3 с пакетом обновления 1 (SP1) или более поздней версии.
...
**Поддерживаемое резервное копирование** | Эти операционные системы поддерживаются для виртуальных машин, для которых требуется создать резервную копию.<br/><br/> Канал Semi-Annual Windows Server (Datacenter, Enterprise, Standard)<br/><br/> Windows Server 2016, Windows Server 2012 R2, Windows Server 2008 R2
...
--- | --- | ---
Подключен | Активна | Создайте резервную копию на диске DPM/MABS.<br/><br/> Резервное копирование в Azure.<br/><br/> Восстановление с диска.<br/><br/> Восстановление из Azure.
Подключен | Срок действия истек или подписка отозвана | Резервное копирование на диск или в Azure невозможно.<br/><br/> Если срок действия подписки истек, можно выполнить восстановление с диска или из Azure.<br/><br/> Если подписка списана, восстановление с диска или Azure невозможно. Точки восстановления Azure удалены.
Отсутствует подключение более 15 дней | Активно | Резервное копирование на диск или в Azure невозможно.<br/><br/> Вы можете выполнить восстановление с диска или из Azure.
...
## <a name="domain-and-domain-trusts-support"></a>Поддержка доверия между доменами и доменами

|Требование |Сведения |
|---------|---------|
|Домен    | Сервер DPM/MABS должен находиться в домене Windows Server 2019, Windows Server 2016, Windows Server 2012 R2, Windows Server 2012.        |
|Доверие в домене   |  DPM/MABS поддерживает защиту данных между лесами, если вы устанавливаете двустороннее доверие на уровне леса между отдельными лесами.   <BR><BR>   DPM/MABS может защищать серверы и рабочие станции между доменами в лесу, имеющем двустороннее отношение доверия с доменом сервера DPM/MABS. Сведения о защите компьютеров в рабочих группах и недоверенных доменах см [. в статье резервное копирование и восстановление рабочих нагрузок в рабочих группах и недоверенных доменах.](/system-center/dpm/back-up-machines-in-workgroups-and-untrusted-domains)  |
...
**Хранение данных MABS на виртуальной машине Azure** | Рекомендуется хранить данные в течение одного дня на диске Azure DPM/MABS и выполнять резервное копирование из DPM/MABS в хранилище для более длительного хранения. Таким образом можно защитить больший объем данных путем их разгрузки в Azure Backup.
...
Сведения о различных серверах и рабочих нагрузках, которые можно защитить с помощью Azure Backup Server, см. в [матрице защиты Azure Backup Server](./backup-mabs-protection-matrix.md#protection-support-matrix).

## <a name="supported-backups-to-dpm"></a>Поддерживаемые типы резервных копий, передаваемые в DPM

Сведения о различных серверах и рабочих нагрузках, которые можно защитить с помощью Data Protection Manager, см. в статье [что может сделать резервное копирование DPM?](/system-center/dpm/dpm-protection-matrix).
...
- [Настройка DPM](/system-center/dpm/install-dpm).
