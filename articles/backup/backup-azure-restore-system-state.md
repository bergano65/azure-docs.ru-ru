...
ms.locfileid: "77602470"
---
# <a name="restore-system-state-to-windows-server"></a>Восстановление состояния системы в Windows Server

В этой статье описано, как восстановить резервные копии состояния системы Windows Server из хранилища служб восстановления Azure. Для восстановления системного состояния необходимо иметь резервную поддержку системного состояния (созданную с помощью инструкций в [резервном состоянии system State)](backup-azure-system-state.md#back-up-windows-server-system-state)и убедиться, что вы установили [последнюю версию агента Microsoft Azure Recovery Services (MARS).](https://aka.ms/azurebackup_agent) Восстановление данных состояния системы Windows Server из хранилища службы восстановления Azure состоит из двух этапов:
...
3. Чтобы восстановить данные на тот же сервер или компьютер, в области **Приступая к работе** выберите **Этот сервер (`<server name>`)** и нажмите кнопку **Далее**.
    ...
    ![Выберите эту опцию сервера для восстановления данных в той же машине](./media/backup-azure-restore-system-state/samemachine.png)
...
10. После успешного восстановления следуйте указаниям в разделе [Применение восстановленного состояния системы в Windows Server](backup-azure-restore-system-state.md), чтобы завершить процесс восстановления состояния системы.
...
11. Проверьте детали восстановления на панели подтверждения и нажмите **Recover.**
...
2. Выполните [последующие](https://docs.microsoft.com/windows-server/identity/ad-ds/manage/ad-forest-recovery-nonauthoritative-restore) шаги здесь, чтобы использовать Windows Server Backup cmdlets для восстановления AD DS.

## <a name="troubleshoot-failed-system-state-restore"></a>Устранение неполадок при сбое восстановления состояния системы

Если предыдущий процесс применения состояния системы не завершился успешно, используйте среду восстановления Windows (Win RE) для восстановления сервера Windows. На следующих шагах описывается, как восстанавливать с помощью Win RE. Используйте эту опцию только в том случае, если Windows Server не загружается нормально после восстановления состояния системы. Будьте осторожны, следующий процесс стирает все несистемные данные.
