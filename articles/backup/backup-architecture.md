...
- Нужно указать способ репликации данных в хранилище для обеспечения избыточности:
  - **Локально избыточное хранилище (LRS)**. для защиты от сбоев в центре обработки данных можно использовать LRS. LRS реплицирует данные в единицу масштабирования хранилища. [Подробнее](https://docs.microsoft.com/azure/storage/common/storage-redundancy-lrs).
  - **Геоизбыточное хранилище (GRS)**. для защиты от простоев в пределах региона можно использовать GRS. GRS реплицирует данные в дополнительный регион. [Подробнее](https://docs.microsoft.com/azure/storage/common/storage-redundancy-grs).
...
--- | --- | ---
**Полный** | Полная резервная копия содержит весь источник данных. Требует больше пропускной способности сети, чем разностное или добавочное резервное копирование. | Используется для начального резервного копирования.
