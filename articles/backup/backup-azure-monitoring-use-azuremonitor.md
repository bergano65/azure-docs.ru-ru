---
title: 'Azure Backup: Мониторинг Azure Backup с помощью Azure Monitor'
...
author: dcurwin
manager: carmonm
keywords: Log Analytics; Azure Backup; Уведомление Параметры диагностики; Группы действий
ms.service: backup
ms.topic: conceptual
ms.date: 06/04/2019
ms.author: dacurwin
...
Вы можете выбрать рабочую область Log Analytics из другой подписки. Чтобы отслеживать хранилища в рамках одной подписки в одном месте, выберите одну и ту же рабочую область Log Analytics для нескольких хранилищ служб восстановления. Чтобы задать для канала всю информацию, связанную с Azure Backup в рабочую область Log Analytics, выберите **установите azurebackupreport** в качестве журнала.
...
- **Azure Monitor**. В разделе **Insights** (аналитика) выберите **More (дополнительно** ) и выберите соответствующую рабочую область.
- **Рабочие области log Analytics**: Выберите соответствующую рабочую область, а затем в разделе **Общие**выберите **Сводка по рабочей области**.
...
- Во всех решениях нерегламентированные задания резервного копирования и задания восстановления помещаются сразу после *завершения*.
...
Здесь ресурс является хранилищем служб восстановления. Необходимо повторить те же действия для всех хранилищ, в которых необходимо получать уведомления с помощью журналов действий. Условие не будет иметь порог, точку или частоту, так как это оповещение основано на событиях. После создания соответствующего журнала действий создается предупреждение.
