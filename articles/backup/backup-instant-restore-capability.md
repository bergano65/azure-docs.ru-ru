...
* Для резервного копирования виртуальных машин, использующих неуправляемые диски класса Premium в учетных записях хранения, с помощью мгновенного восстановления рекомендуется выделить *50%* свободного пространства для общего выделенного пространства, которое требуется **только** для первой резервной копии. 50% свободного места не является обязательным для резервных копий после завершения первой архивации.
...
Если для точки восстановления есть моментальный снимок и эта последняя точка восстановления доступна, он сохраняется до следующего успешного резервного копирования. В соответствии с разметкой "сборка мусора" (GC), в настоящее время которая требует, чтобы по крайней мере одна последняя версия RP всегда присутствовала в случае сбоя всех резервных копий из-за проблемы в виртуальной машине. Обычно точки восстановления удаляются не позднее 24 часов после истечения их срока действия.
