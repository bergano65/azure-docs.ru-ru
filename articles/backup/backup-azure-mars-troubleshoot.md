...
- [Убедитесь в наличии сетевого подключения между агентом MARS и Azure](https://docs.microsoft.com/azure/backup/backup-azure-mars-troubleshoot#the-microsoft-azure-recovery-service-agent-was-unable-to-connect-to-microsoft-azure-backup).
- Убедитесь, что режим MARS выполняется (в консоли службы). Если необходимо, перезапустите и повторите операцию.
- [Убедитесь, что на диске с объемом от 5% до 10% доступно свободное место в папке временных файлов](https://docs.microsoft.com/azure/backup/backup-azure-file-folder-backup-faq#whats-the-minimum-size-requirement-for-the-cache-folder).
- [Проверьте, мешает ли другой процесс или антивирусному программному обеспечению Azure Backup](https://docs.microsoft.com/azure/backup/backup-azure-troubleshoot-slow-backup-performance-issue#cause-another-process-or-antivirus-software-interfering-with-azure-backup).
...
- Если запланированное резервное копирование завершается сбоем, но резервное копирование выполняется вручную, см. раздел [резервное копирование не выполняется в соответствии](https://docs.microsoft.com/azure/backup/backup-azure-mars-troubleshoot#backups-dont-run-according-to-schedule)
...
- Если агент резервного копирования работает на виртуальной машине Azure, см. [эту статью](https://docs.microsoft.com/azure/backup/backup-azure-troubleshoot-slow-backup-performance-issue#cause-backup-agent-running-on-an-azure-virtual-machine).
...
- Ниже перечислены условия, которые могут привести к пропуску файлов резервных копий.
  - Неподдерживаемые атрибуты файла (например, в папке OneDrive сжатый поток, точки повторного анализа). Полный список см. в [матрице поддержки](https://docs.microsoft.com/azure/backup/backup-support-matrix-mars-agent#supported-file-types-for-backup).
...
- Служба архивации помечает эти файлы как неудачные в файле журнала со следующим соглашением об именовании: *ластбаккупфаиледфилескскскскс. txt* в папке *C:\Program Files\Microsoft Azure Recovery Service ажент\темп* .
- Чтобы устранить эту проблему, изучите файл журнала, чтобы понять характер проблемы:
  ...
  | 0x80070002, 0x80070003 | Системе не удается найти указанный файл.         | [Убедитесь, что вспомогательная папка не заполнена](https://docs.microsoft.com/azure/backup/backup-azure-file-folder-backup-faq#manage-the-backup-cache-folder)  <br><br>  Убедитесь, что том, на котором настроена область временных файлов, существует (не удален)  <br><br>   [Убедитесь, что агент MARS исключен из антивирусной программы, установленной на компьютере](https://docs.microsoft.com/azure/backup/backup-azure-troubleshoot-slow-backup-performance-issue#cause-another-process-or-antivirus-software-interfering-with-azure-backup)  |
  | 0x80070005             | Отказано в доступе                                    | [Проверка блокировки доступа антивирусной программы или другого стороннего программного обеспечения](https://docs.microsoft.com/azure/backup/backup-azure-troubleshoot-slow-backup-performance-issue#cause-another-process-or-antivirus-software-interfering-with-azure-backup)     |
...
- С помощью параметра [Добавить правила исключения в существующую политику](https://docs.microsoft.com/azure/backup/backup-azure-manage-mars#add-exclusion-rules-to-existing-policy) можно исключить из политики резервного копирования неподдерживаемые, отсутствующие или удаленные файлы, чтобы обеспечить успешную архивацию.
...
- Убедитесь, что на сервере установлен PowerShell 3,0 или более поздней версии. Чтобы проверить версию PowerShell, выполните следующую команду и убедитесь, что номер `Major` версии — 3 или более поздней:
...
Сообщение об ошибке | Рекомендованное действие
-- | --
Агенту Службы восстановления Microsoft Azure не удалось получить доступ к вспомогательному расположению для инициализации VHD. | Чтобы устранить эту проблему, выполните приведенные ниже действия и перезапустите сервер. <br/> - [Проверьте наличие антивирусной программы или других процессов, блокирующих файлы вспомогательного расположения.](#another-process-or-antivirus-software-blocking-access-to-cache-folder)<br/> - [Проверьте, является ли вспомогательное расположение допустимым и доступным для агента MARS.](backup-azure-file-folder-backup-faq.md#how-to-check-if-scratch-folder-is-valid-and-accessible)
