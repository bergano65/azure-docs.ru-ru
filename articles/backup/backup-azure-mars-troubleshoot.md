...
**Сообщение об ошибке**: указаны недопустимые учетные данные хранилища. Файл либо поврежден, либо не содержит последних учетных данных, связанных со службой восстановления. (ИДЕНТИФИКАТОР: 34513)
...
1. Последовательно выберите **пункты сервис** > **Свойства обозревателя** > **подключения** > **Параметры локальной сети**.
...
| Ошибка | Возможные причины | Рекомендуемые действия |
| ---     | ---     | ---    |
...
- Убедитесь, что на сервере установлен PowerShell 3,0 или более поздней версии. Чтобы проверить версию PowerShell, выполните следующую команду и убедитесь, что номер версии `Major` равен 3 или выше:

  `$PSVersionTable.PSVersion`

- Убедитесь, что этот путь является частью переменной среды `PSMODULEPATH`:

  `<MARS agent installation path>\Microsoft Azure Recovery Services Agent\bin\Modules\MSOnlineBackup`

- Если для политики выполнения PowerShell для `LocalMachine` задано значение `restricted`, командлет PowerShell, запускающий задачу резервного копирования, может завершиться ошибкой. Выполните следующие команды в режиме с повышенными правами, чтобы проверить и установить политику выполнения либо `Unrestricted`, либо `RemoteSigned`.
...
7. Выберите **Диспетчер задач** > **службы (локальные)**  > **Служба инициатора iSCSI (Майкрософт**):
...
-- | --
Агенту Службы восстановления Microsoft Azure не удалось получить доступ к контрольной сумме резервной копии, хранящейся в вспомогательном расположении. | Чтобы устранить эту проблему, выполните приведенные ниже действия и перезапустите сервер. <br/> - [проверить, есть ли антивирусная программа или другие процессы, блокирующие файлы временных расположений](#another-process-or-antivirus-software-blocking-access-to-cache-folder)<br/> - [проверить, является ли вспомогательное расположение допустимым и доступным для агента MARS.](backup-azure-file-folder-backup-faq.md#how-to-check-if-scratch-folder-is-valid-and-accessible)

### <a name="salvhdinitializationerror"></a>SalVhdInitializationError

Сообщение об ошибке | Рекомендуемое действие |
-- | --
Агенту Службы восстановления Microsoft Azure не удалось получить доступ к вспомогательному расположению для инициализации VHD. | Чтобы устранить эту проблему, выполните приведенные ниже действия и перезапустите сервер. <br/> - [проверить, есть ли антивирусная программа или другие процессы, блокирующие файлы временных расположений](#another-process-or-antivirus-software-blocking-access-to-cache-folder)<br/> - [проверить, является ли вспомогательное расположение допустимым и доступным для агента MARS.](backup-azure-file-folder-backup-faq.md#how-to-check-if-scratch-folder-is-valid-and-accessible)

### <a name="sallowdiskspace"></a>салловдискспаце

Сообщение об ошибке | Рекомендуемое действие |
-- | --
Сбой резервного копирования из-за нехватки места в томе, где находится вспомогательная папка | Чтобы устранить эту проблему, проверьте приведенные ниже действия и повторите операцию.<br/>- [Убедитесь, что агент Mars является последним](https://go.microsoft.com/fwlink/?linkid=229525&clcid=0x409)<br/> - [проверить и устранить проблемы с хранилищем, влияющие на рабочую область резервного копирования](#prerequisites)
