...
5. В меню **Цель резервного копирования** выберите **Настройка архивации**.
    ...
    Служба Azure Backup отображает все экземпляры SQL с изолированными базами данных, а также группы доступности SQL Always On. Чтобы просмотреть изолированные базы данных в экземпляре SQL Server, щелкните значок шеврона слева от имени этого экземпляра. Выберите также значок шеврона слева от группы доступности AlwaysOn, чтобы просмотреть список баз данных. На следующем изображении показан пример изолированного экземпляра и группы доступности Always On.
...
6. В списке баз данных выберите все, которые необходимо защитить, и нажмите **ОК**.
    ...
    Кроме того, вы можете включить автоматическую защиту всего экземпляра или группы доступности AlwaysOn, выбрав параметр **ВКЛ** из соответствующего раскрывающегося списка в столбце **АВТОЗАЩИТА**. Функция автоматической защиты не только включает защиту для всех существующих баз данных за один раз, но также распространяется на все новые базы данных, которые будут добавлены к этому экземпляру или группе доступности в дальнейшем.  
      ...
      Если экземпляр или группа доступности уже содержит некоторые из защищенных баз данных, по-прежнему можно **включить** параметр автоматической защиты. В этом случае политика резервного копирования, определенная в следующем шаге, будет применима только для незащищенных баз данных, а уже защищенные базы данных будут по-прежнему защищены с помощью их соответствующих политик.

      Нет ограничений на количество баз данных, выбираемых за один раз с помощью функции автоматической защиты (можно выбрать столько баз данных, сколько их находится в хранилище).  

      Рекомендуем включить автоматическую защиту для всех экземпляров и групп доступности Always On, если нужно, чтобы все базы данных, добавленные в будущем, были автоматически настроены для защиты.
...
### <a name="steps-to-restore-a-database"></a>Действия по восстановлению базы данных
...
7. Чтобы остановить защиту базы данных, выберите **Остановить архивацию**.
  ...
  Следует учитывать, что параметр **Остановить архивацию** не работает для базы данных в экземпляре с автоматической защитой. Единственный способ остановить защиту этой базы данных — отключить на время автоматическую защиту экземпляра, а затем выбрать параметр **Остановить архивацию** в разделе **Элементы архивации** для этой базы данных.  

  Можно отключить автоматическую защиту на экземпляре или в группе доступности Always On в разделе **Настройка архивации**. Щелкните имя экземпляра, чтобы открыть панель сведений справа, в верхней части которой есть пункт **Отключить автозащиту**. Нажмите **Отключить автозащиту** для отключения автоматической защиты этого экземпляра.
...
Нет, автоматическая защита применяется ко всему экземпляру. Невозможно выборочно защитить лишь некоторые базы данных экземпляра с помощью автоматической защиты.
...
Если у вас уже есть некоторые защищенные базы данных в экземпляре, они будут по-прежнему защищены с помощью соответствующих политик даже после **включения** параметра автоматической защиты. Но все незащищенные базы данных вместе с теми, которые будут добавлены в дальнейшем, будут иметь только одну политику, определенную вами в разделе **Настройка архивации** после выбора баз данных. В отличие от других защищенных баз данных, невозможно даже изменить политику в базе данных экземпляра с автоматической защитой.
...
Единственный способ остановить защиту этой базы данных — отключить на время автоматическую защиту экземпляра, а затем выбрать параметр **Остановить архивацию** в разделе **Элементы архивации** для этой базы данных. Затем можно повторно включить автоматическую защиту для этого экземпляра.
...
База данных, недавно добавленная в экземпляр с автоматической защитой, может отобразиться не мгновенно. Это обусловлено тем, что обнаружение обычно выполняется раз в 8 часов. Но пользователь может выполнить обнаружение вручную с помощью параметра **Восстановление баз данных**, чтобы мгновенно обнаружить и защитить новые базы данных, как показано на изображении ниже.
