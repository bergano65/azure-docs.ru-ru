...
ms.locfileid: "56887817"
---
# <a name="back-up-sql-server-databases-on-azure-vms"></a>Создание резервной копии баз данных SQL Server на виртуальных машинах Azure 
...
> [!div class="checklist"]
> * проверка предварительных требований для резервного копирования экземпляра SQL Server;
...
1. Убедитесь, что у вас есть экземпляр SQL Server, работающий в Azure. Вы можете [быстро создать экземпляр SQL Server](../sql-database/sql-database-get-started-portal.md) в marketplace.
2. Просмотрите ограничения общедоступной предварительной версии ниже.
3. Просмотрите сценарий поддержки.
4. [Ознакомьтесь с часто задаваемыми вопросами об этом сценарии](faq-backup-sql-server.md).


## <a name="preview-limitations"></a>Ограничения предварительной версии

В этой общедоступной предварительной версии есть ряд ограничений.
...
- Вы можете создать резервные копии 2000 баз данных SQL Server в хранилище. Если у вас больше баз данных, создайте еще одно хранилище. 
- [Резервное копирование распределенных групп доступности](https://docs.microsoft.com/sql/database-engine/availability-groups/windows/distributed-availability-groups?view=sql-server-2017) работает не полностью.
- Экземпляры отказоустойчивого кластера SQL Server Always On не поддерживаются для резервного копирования.
- Резервное копирование SQL Server необходимо настроить на портале. Сейчас резервное копирование невозможно настроить с помощью Azure PowerShell, интерфейса командной строки или интерфейсов REST API.
...
- Базу данных с большим числом файлов невозможно защитить. Максимальное число поддерживаемых файлов не является детерминированным. Оно не только зависит от количества файлов, но также зависит от длины пути файлов. 

Изучите [часто задаваемые вопросы](faq-backup-sql-server.md) о резервном копировании баз данных SQL Server.
...
**Поддерживаемые географические регионы** | Юго-Восточная Австралия (ASE); Южная Бразилия (BRS); Центральная Канада (CNC); Восточная Канада (CE); центральная часть США (CUS); Восточная Азия (EA); Восточная Австралия (AE); восточная часть США (EUS); восточная часть США 2 (EUS2); Центральная Индия (INC); Южная Индия (INS); Восточная Япония (JPE); Западная Япония (JPW); Центральная Корея (KRC); Южная Корея (KRS); центрально-северная часть США (NCUS); Северная Европа (NE); центрально-южная часть США (SCUS); Юго-Восточная Азия (SEA); южная часть Соединенного Королевства (UKS); западная часть соединенного Королевства (UKW); центрально-западная часть США (WCUS); Западная Европа (WE); западная часть США (WUS); западная часть США 2 (WUS 2)
...
1. Определите или [создайте хранилище Служб восстановления](backup-azure-sql-database.md#create-a-recovery-services-vault) в том же регионе или месте, что и размещение виртуальной машины SQL Server.
2. [Проверьте разрешения виртуальной машины](#fix-sql-sysadmin-permissions), необходимые для резервного копирования баз данных SQL.
3. Убедитесь, что виртуальная машина имеет [сетевое подключение](backup-azure-sql-database.md#establish-network-connectivity).
4. Убедитесь, что базы данных SQL Server именуются в соответствии с [рекомендациями по именованию](backup-azure-sql-database.md) для Azure Backup.
...
- **Разрешение диапазонов IP-адресов центра обработки данных Azure**. Разрешите [диапазоны IP-адресов](https://www.microsoft.com/download/details.aspx?id=41653) при скачивании. Для доступа в группу безопасности сети используйте командлет **Set-AzureNetworkSecurityRule**.
...
Если вы не создавали виртуальную машину SQL Server из Azure Marketplace, может появиться ошибка **UserErrorSQLNoSysadminMembership**. В этом случае [выполните эти инструкции](#fix-sql-sysadmin-permissions).
...
9. Выберите виртуальную машину, на которой выполняется база данных SQL Server, и щелкните **Обнаружить базы данных**. 
10. Azure Backup обнаруживает все базы данных SQL Server на виртуальной машине. Во время обнаружения в фоновом режиме происходит следующее:
...
- Кроме того, вы можете включить автоматическую защиту. Автоматическая защита защищает имеющиеся базы данных и автоматически защищает новые базы данных, добавляемые в экземпляр группы доступности.


Настройте резервное копирование следующим образом:

1. На панели мониторинга хранилища выберите **Архивация**. 
...
4. В раскрывающемся списке **What do you want to back up** (Что необходимо архивировать) выберите **SQL Server в виртуальной машине Azure**.
...
6. Выберите все базы данных, которые нужно защитить, и нажмите кнопку **ОК**.

    ![Защита базы данных](./media/backup-azure-sql-database/select-database-to-protect.png)

    - Отображаются все экземпляры SQL Server (изолированные и в группах доступности).
    - Для фильтрации выберите стрелку вниз слева от имени экземпляра или группы доступности.

7. В меню **Резервное копирование** выберите **Политика архивации**. 

    ![Выбор политики резервного копирования](./media/backup-azure-sql-database/select-backup-policy.png)

8. В меню **Выбрать политику архивации** выберите политику и нажмите кнопку **ОК**.
    ...
    - [определить новую политику](backup-azure-sql-database.md#configure-a-backup-policy) на основе целевой точки восстановления (RPO) и периода хранения.
    - Имеющуюся политику резервного копирования нельзя изменять в течение срока действия предварительной версии.
    
9. В меню **Архивация** выберите **Включить резервное копирование**.

    ![Включение выбранной политики резервного копирования](./media/backup-azure-sql-database/enable-backup-button.png)

10. В области **Уведомления** портала можно отслеживать ход настройки.
...
Включите автоматическую защиту, чтобы автоматически создавалась резервная копия всех имеющихся баз данных и баз данных, которые в будущем будут добавлены в изолированный экземпляр SQL Server или группу доступности Always On SQL Server. 

- При включении автоматической защиты и выборе политики имеющиеся защищенные базы данных будут продолжать использовать предыдущую политику.
- Ограничения на количество баз данных, которые можно выбрать для автоматической защиты за один раз, отсутствуют.

Включите автоматическую защиту следующим образом:
...
Если вам нужно отключить автоматическую защиту, щелкните имя экземпляра в разделе **Настройка резервного копирования** и выберите **Отключить автозащиту** для экземпляра. Резервное копирование всех баз данных продолжится. Однако будущие базы данных не будут автоматически защищены.
...
3. Учетная запись виртуальной службы Windows **NT SERVICE\AzureWLBackupPluginSvc** была создана во время регистрации виртуальной машины и обнаружения баз данных SQL. Введите имя учетной записи, как показано в поле **Введите имя объекта**. Выберите **Проверить имена**, чтобы разрешить имя. Последовательно выберите **ОК**.

    ![Выбор "Проверить имена" для разрешения имени неизвестной службы](./media/backup-azure-sql-database/check-name.png)


4. Убедитесь, что в качестве **роли сервера** выбрана роль **sysadmin**. Последовательно выберите **ОК**. Теперь должны существовать необходимые разрешения.

    ![Убедитесь, что для сервера выбрана роль sysadmin.](./media/backup-azure-sql-database/sysadmin-server-role.png)

5. Теперь свяжите базу данных с хранилищем Служб восстановления. В списке **Защищенные серверы** на портале Azure щелкните правой кнопкой мыши сервер, на котором произошла ошибка, и выберите **Повторно обнаружить базы данных**.

    ![Проверка наличия у сервера соответствующих разрешений](./media/backup-azure-sql-database/check-erroneous-server.png)

6. Проверьте ход выполнения в области **Уведомления**. Когда выбранные базы данных будут найдены, появится сообщение об успешном завершении.

    ![Сообщение об успешном развертывании](./media/backup-azure-sql-database/notifications-db-discovered.png)

Кроме того, вы можете включить [автоматическую защиту](backup-azure-sql-database.md#enable-auto-protection) всего экземпляра или группы доступности Always On, выбрав параметр **ВКЛ** из соответствующего раскрывающегося списка в столбце **АВТОЗАЩИТА**. Возможность [автоматической защиты](backup-azure-sql-database.md#enable-auto-protection) не только включает защиту для всех существующих баз данных за один раз, но также распространяется на все новые базы данных, которые будут добавлены к этому экземпляру или группе доступности в дальнейшем.  
