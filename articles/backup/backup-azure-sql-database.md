...
* Чтобы иметь возможность обнаруживать базы данных на этой виртуальной машине, Azure Backup создает учетную запись `NT SERVICE\AzureWLBackupPluginSvc`. Эта учетная запись используется для резервного копирования и восстановления и требует разрешений системного администратора SQL. `NT SERVICE\AzureWLBackupPluginSvc` — это [учетная запись виртуальной службы](/windows/security/identity-protection/access-control/service-accounts#virtual-accounts), поэтому для нее не требуется управление паролями. Azure Backup будет использовать учетную запись `NT AUTHORITY\SYSTEM` для обнаружения и запроса баз данных, поэтому эта учетная запись должна соответствовать общедоступному имени входа в SQL. Если вы не создали SQL Server виртуальную машину из Azure Marketplace, может появиться сообщение об ошибке **усереррорсклносисадминмембершип**. В этом случае [выполните эти инструкции](#set-vm-permissions).
...
Если вы не создали SQL Server виртуальную машину в Azure Marketplace или используете SQL 2008 и 2008 R2, может возникнуть ошибка **усереррорсклносисадминмембершип** .
