...
Чтобы просмотреть сценарии резервного копирования и восстановления, которые поддерживаются сегодня, см. [таблицу поддержки](backup-azure-sql-database.md#scenario-support).
...
* Чтобы иметь представление о том, сколько баз данных можно защитить на каждом сервере, необходимо рассмотреть такие факторы, как пропускная способность, размер виртуальной машины, частота резервного копирования, размер базы данных и т. д. [Загрузите](https://download.microsoft.com/download/A/B/5/AB5D86F0-DCB7-4DC3-9872-6155C96DE500/SQL%20Server%20in%20Azure%20VM%20Backup%20Scale%20Calculator.xlsx) планировщик ресурсов, который предоставляет приблизительное количество баз данных, которые можно использовать для каждого сервера на основе ресурсов виртуальной машины и политики резервного копирования.
...
Мы советуем, чтобы резервное копирование настраивалось только на одном узле группы доступности. Резервное копирование всегда должно быть настроено в одном регионе с основным узлом. Другими словами, необходимо, чтобы основной узел всегда присутствовал в регионе, в котором вы настраиваете резервное копирование. Если все узлы группы доступности находятся в том же регионе, в котором настроена резервная копия, никаких проблем не возникает.

#### <a name="for-cross-region-ag"></a>Группа доступности перехода в другой регион

* Независимо от настроек резервного копирования, резервные копии не будут происходить с узлов, которые находятся не в том же регионе, где настроена резервная копия. Так как резервное копирование между регионами не поддерживается. Если у вас только два узла, а вторичный узел находится в другом регионе; в этом случае резервные копии будут продолжать выполняться с первичного узла (если только параметр резервного копирования не является "вторичным").
...
* **Параметры резервного копирования: основной**
...
* **Параметр резервного копирования: только вторичный**
...
* **Параметр резервного копирования: дополнительный**
...
9. После предоставления разрешения повторно **Определите баз данных** на портале: хранилище **->** инфраструктура резервного копирования **->** рабочей нагрузки на виртуальной машине Azure:
