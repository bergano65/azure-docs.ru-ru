...
Расширение резервного копирования, выполняемое на виртуальной машине, должно иметь исходящий доступ к общедоступным IP-адресам Azure. Разрешить доступ можно следующим образом.


- **Правила групп безопасности сети (NSG)**. Разрешите [диапазоны IP-адресов центра обработки данных Azure](https://www.microsoft.com/download/details.aspx?id=41653). Можно добавить правило, которое разрешает доступ к службе Azure Backup с помощью [тега службы](../virtual-network/security-overview.md#service-tags), вместо того чтобы по отдельности разрешать каждый диапазон адресов и управлять ими в дальнейшем.
...
3. В поле "Источник" выберите **VirtualNetwork** (виртуальная сеть).
...
2. В поле **Приоритет** присвойте правилу значение приоритета меньше, чем у любых вышестоящих запрещающих правил.
3. Укажите имя и описание правила и нажмите кнопку **ОК**.
...
4. Задайте параметры прокси-сервера.
    ...
    - На компьютерах Windows в настройках браузера укажите, что должен использоваться прокси-сервер. Если вы уже используете прокси-сервер для учетной записи пользователя, чтобы применить настройки к учетной записи на уровне системы, можно использовать этот сценарий.
        ```
...
1. Разрешите входящие подключения в параметрах прокси-сервера.
2. Например, откройте **брандмауэр Windows в режиме повышенной безопасности**.
