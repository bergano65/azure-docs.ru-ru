---
title: Архивация виртуальных машин Azure в хранилище служб восстановления с помощью Azure Backup
description: Описывает, как для резервного копирования виртуальных машин Azure в хранилище служб восстановления с помощью Azure Backup
services: backup
...
В этой статье описывается, как для резервного копирования виртуальных машин Azure в хранилище служб восстановления с помощью [Azure Backup](backup-overview.md) службы. 
...
Как правило не нужно явным образом разрешить исходящий сетевой доступ для виртуальной Машины Azure, поэтому он может обмениваться данными со службой Azure Backup.
Если виртуальные машины не удается подключиться, и в том случае, если появится сообщение об ошибке **ExtensionSnapshotFailedNoNetwork**, следует явным образом разрешить доступ. Расширение резервного копирования может обмениваться данными с Azure общедоступных IP-адресов для трафика резервного копирования.


#### <a name="explicitly-allow-outbound-access"></a>Явным образом разрешить исходящий доступ

Если виртуальную Машину не удается подключиться к службе архивации, явным образом разрешите исходящий доступ с помощью одного из методов, представленных в таблице.

**Параметр** | **Действие** | **Сведения** 
--- | --- | --- 
**Настройка правил группы безопасности сети** | Разрешите [диапазоны IP-адресов центра обработки данных Azure](https://www.microsoft.com/download/details.aspx?id=41653). | Вместо разрешения и управление каждый диапазон адресов, можно добавить правило группы безопасности сети, обеспечивающий доступ к службе архивации Azure с помощью [тег службы](backup-azure-arm-vms-prepare.md#set-up-an-nsg-rule-to-allow-outbound-access-to-azure). [Узнайте больше](../virtual-network/security-overview.md#service-tags).<br/><br/> Существуют без дополнительных затрат.<br/><br/> Правила можно легко управлять с помощью тегов службы.
**Развертывание прокси-сервера** | Разверните прокси-сервер HTTP для маршрутизации трафика. | Этот метод обеспечивает доступ ко всей службе Azure, а не только к хранилищу.<br/><br/> Точное управление URL-адресами хранилища.<br/><br/> Нет используется точка Интернет-доступ для виртуальных машин.<br/><br/> Имеются дополнительные затраты для учетной записи-посредника.
**Настройка брандмауэра для Azure** | Разрешите трафик через брандмауэр Azure на виртуальной Машине, с помощью тега полное доменное имя для службы архивации Azure. |  Этот метод является просто использовать при наличии брандмауэра Azure, которые настроены в подсети виртуальной сети.<br/><br/> Невозможно создать собственные теги полное доменное имя или измените полные доменные имена в теге.<br/><br/> Если вы используете "Управляемые диски Azure", может потребоваться открыть в брандмауэрах дополнительный порт (8443).
...
### <a name="allow-firewall-access-by-using-an-fqdn-tag"></a>Открытие брандмауэра с помощью полного доменного ИМЕНИ тега
...
По умолчанию это [геоизбыточное хранилище (GRS)](https://docs.microsoft.com/azure/storage/common/storage-redundancy-grs). Мы рекомендуем использовать GRS для резервная копия является основной. Можно использовать [локально избыточное хранилище (LRS)](https://docs.microsoft.com/azure/storage/common/storage-redundancy-lrs?toc=%2fazure%2fstorage%2fblobs%2ftoc.json) дешевле параметра.

Измените тип репликации хранилища следующим образом:

1. На портале выберите новое хранилище. В разделе **параметры**выберите **свойства**.
2. В **свойства**в разделе **резервная конфигурация**выберите **обновления**.
3. Выберите тип репликации хранилища и выберите **Сохранить**.
...
1. В хранилище > **Обзор**выберите **и резервное копирование**.
...
2. В разделе **Цель резервного копирования**  >  **Где выполняется рабочая нагрузка?** выберите **Azure**. В раскрывающемся списке **Что необходимо архивировать?** выберите **Виртуальная машина**  >   **ОК**.
...
3. В области **Политика архивации** выберите политику, которую вы хотите связать с хранилищем. Нажмите кнопку **ОК**.
    ...
    - Выберите **Create New** для создания политики. [Подробнее](backup-azure-arm-vms-prepare.md#configure-a-backup-policy) об определении политики.

    ![Области "Архивация" и "Политика архивации"](./media/backup-azure-arm-vms-prepare/select-backup-goal-2.png)

4. В **выбор виртуальных машин** панели выберите виртуальные машины, которые будут использовать указанную политику резервного копирования > **ОК**.

   Выбранные виртуальные машины проверяются. Виртуальные машины можно выбрать только в том же регионе, что и хранилище. Архивация виртуальных машин может выполняться только в одном хранилище.
...
5. В разделе **Архивация** выберите **Включить резервное копирование**.
   ...
   Этот шаг выполняется развертывание политики в хранилище и к виртуальным машинам. Он также устанавливает расширение архивации на агент виртуальной Машины, на виртуальной Машине Azure.
...
На запущенной виртуальной машине проще получить точку восстановления, согласованную с приложениями. Тем не менее резервное копирование виртуальной машины выполняется даже в том случае, если она выключена и нельзя установить расширение. Он называется автономной виртуальной Машиной. В этом случае будет отказоустойчивой точки восстановления.
...
1. В меню хранилища выберите **архивные элементы**.
2. В **архивные элементы**выберите **виртуальной машине Azure**.
3. В **архивные элементы** выберите кнопку с многоточием (**...** ).
4. Выберите **резервную**.
5. В **моментальная Архивация**, использование элементов управления календарем выберите последний день, следует ли хранить точки восстановления > **ОК**.
