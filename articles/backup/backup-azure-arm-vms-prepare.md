...
## <a name="verify-backup-job-status"></a>Проверьте состояние задания резервного копирования

Задание резервного копирования подробной информации о каждой резервной копии виртуальной Машины состоит из 2 этапа **моментального снимка** этап, за которым следует **передача данных в хранилище** этапа.<br/>
Этап создания моментального снимка гарантирует доступность точки восстановления хранятся вместе с дисками для **мгновенное восстановление** и доступны для более 5 дней в зависимости от срок хранения моментальных снимков, заданный пользователем. Передача данных в хранилище создается точка восстановления в хранилище для длительного хранения. Передача данных в хранилище начинается только после завершения фазы моментального снимка.

  ![Состояние задания резервного копирования](./media/backup-azure-arm-vms-prepare/backup-job-status.png)

Существует два **подзадач** в серверной части, один для задания резервного копирования с внешнего интерфейса, который можно проверить с **задание резервного копирования** колонке сведений о как указано ниже:

  ![Состояние задания резервного копирования](./media/backup-azure-arm-vms-prepare/backup-job-phase.png)

**Передача данных в хранилище** этап может занять несколько дней для завершения в зависимости от объема дисков, обновлениях на каждом диске и других факторов.

Состояние задания, зависит от следующих сценариев:
...
Завершено | Сбой | Завершено с предупреждением
Сбой | Сбой | Сбой


Теперь благодаря этой возможности для той же виртуальной Машине, две резервные копии могут выполняться параллельно, но на любом этапе (моментальный снимок, передачи данных в хранилище) может выполняться только один подзадачи. В сценарии были, что задание резервного копирования выполняется привело к резервной копии на следующий день переход на другой позволяет избегать с этим отделение функциональные возможности. Резервные копии в последующий день можно моментального снимка завершенного при **передача данных в хранилище** пропустить, если задание резервного копирования на более ранних день находится в состоянии хода выполнения.
Добавочная точка восстановления создана в хранилище соберет все обновленных данных из последней точки восстановления, созданной в хранилище. Никак не отражается стоимость на пользователя.
