...
2. В качестве **цели** > резервного копирования **, где выполняется рабочая нагрузка?** выберите **Azure**. В **поле что вы хотите архивировать?** выберите **Виртуальная машина** >  **ОК**. При этом расширение виртуальной машины зарегистрируется в хранилище.
...
2. В **расписанию резервного копирования** укажите, когда следует создавать резервные копии. Для виртуальных машин Azure можно создавать ежедневное или еженедельное резервное копирование.
2. В окне **мгновенное восстановление**укажите, как долго следует хранить моментальные снимки локально для мгновенного восстановления.
...
3. В списке **диапазон хранения**укажите, как долго следует хранить ежедневные или еженедельные точки резервного копирования.
4. При **хранении месячной точки резервного копирования**укажите, следует ли хранить ежемесячные резервные копии ежедневных или еженедельных резервных копий.
...
Завершено | Пропущен | Завершено
Завершено | Завершено | Завершено
Завершено | Неудача | Выполнено с предупреждениями
...
- Если у вас возникли проблемы с подключением к виртуальным машинам или если вы видите ошибку **екстенсионснапшотфаиледнонетворк** при попытке подключения, необходимо явно разрешить доступ, чтобы расширение резервного копирования могла взаимодействовать с общедоступными IP-адресами Azure для резервного копирования. трафика. Методы доступа приведены в следующей таблице.
...
6. В поле **Диапазоны портов назначения** выберите порт.
    ...
    - Виртуальная машина, использующая неуправляемые диски с зашифрованной учетной записью хранения: 443 (значение по умолчанию)
    - Виртуальная машина, использующая управляемые диски: 8443
...
3. В списке **тип правила**выберите **Настраиваемый** > **Далее**.
