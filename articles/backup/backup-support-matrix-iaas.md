...
Операций резервного копирования в день (с использованием расширения виртуальных машин Azure) | Одно запланированное резервное копирование в день.<br/><br/>Служба Azure Backup поддерживает до трех резервных копий по запросу в день и одну дополнительную запланированную архивацию.
...
- Azure Backup не поддерживает резервное копирование файлов NFS, которые подключены из хранилища или с любого другого сервера NFS, на компьютеры Linux или Windows. Он выполняет резервное копирование только тех дисков, которые локально подключены к виртуальной машине.
...
Максимальное время окончания срока действия для точки восстановления | Без ограничения (99 лет).
...
Резервное копирование с использованием [параметров диагностики](../azure-monitor/essentials/platform-logs-overview.md)  | Не поддерживается. <br/><br/> Если восстановление виртуальной машины Azure с параметрами диагностики запускается с помощью команды [Создать](backup-azure-arm-restore-vms.md#create-a-vm), оно завершается сбоем.
Восстановление виртуальных машин, прикрепленных к зонам | Поддерживается (для виртуальной машины, для которой создается резервная копия после января 2019 и для которой доступны [зоны доступности](https://azure.microsoft.com/global-infrastructure/availability-zones/) ).<br/><br/>В настоящее время поддерживается восстановление в ту же зону, которая закреплена на виртуальных машинах. Однако если зона недоступна из-за сбоя, восстановление завершится ошибкой.
...
[Выделенный узел Azure](https://docs.microsoft.com/azure/virtual-machines/dedicated-hosts) | Поддерживаются:
Конфигурация дисковых пространств Windows для автономных виртуальных машин Azure | Поддерживаются:

## <a name="vm-storage-support"></a>Поддержка хранилища виртуальной машины

**Компонент** | **Поддержка**
--- | ---
Диски данных виртуальной машины Azure | Поддержка резервного копирования виртуальных машин Azure с емкостью до 32 дисков.<br><br> Поддержка резервного копирования виртуальных машин Azure с неуправляемыми дисками или классических виртуальных машин возможна только при количестве дисков до 16.
...
Диски с включенным ускорителем записи | Начиная с 23 ноября 2020, поддерживается только в регионах "Центральная Корея (Крк)" и "Северная Африка" (SAN) для ограниченного числа подписок. Для этих поддерживаемых подписок Azure Backup будет создавать резервные копии виртуальных машин, на которых включены диски с ускоренной записью (WA) во время резервного копирования.<br><br>Для неподдерживаемых регионов требуется подключение к Интернету на виртуальной машине, чтобы создавать моментальные снимки виртуальных машин с включенным WA.<br><br> **Важное примечание**. в этих неподдерживаемых регионах виртуальные машины с жесткими дисками WA требуют подключения к Интернету для успешной архивации (даже если эти диски исключены из резервной копии).
...
Общее хранилище| Резервное копирование виртуальных машин с помощью общий том кластера (CSV) или Scale-Out файлового сервера не поддерживается. Модули записи общего тома кластера, скорее всего, не будут работать во время резервного копирования. При восстановлении диски, содержащие тома CSV, могут не извлекаться.
[Общие диски](../virtual-machines/disks-shared-enable.md) | Не поддерживается.
SSD (цен. категория "Ультра") диски | Не поддерживается. Дополнительные сведения см. в разделе [ограничения](selective-disk-backup-restore.md#limitations).
[Временные диски](https://docs.microsoft.com/azure/virtual-machines/managed-disks-overview#temporary-disk) | Резервное копирование временных дисков не производится Azure Backup.
