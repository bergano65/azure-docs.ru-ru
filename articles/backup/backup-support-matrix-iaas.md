...
- [Таблица поддержки](backup-support-matrix-mabs-dpm.md) для резервного копирования с помощью сервера Azure Backup или System Center Data Protection Manager (DPM).
...
Резервное копирование виртуальной машины после изменения времени компьютера | Не поддерживается.<br/><br/> Если время компьютера изменилось на дату и время в будущем после включения резервного копирования для виртуальной машины. Однако даже если изменение времени было отменено, успешное резервное копирование не гарантируется.  
...
- Azure Backup не поддерживает 32-разрядные операционные системы.
- Вы можете использовать и другие дистрибутивы Linux, если в них установлен [агент виртуальной машины Azure для Linux](https://docs.microsoft.com/azure/virtual-machines/extensions/agent-linux) и включена поддержка Python.
- Azure Backup не поддерживает виртуальную машину Linux с настроенным прокси-сервером, если в ней не установлена версия Python 2.7.
...
Резервное копирование согласованных виртуальных машин | Azure Backup не обеспечивает согласованность данных и приложений в нескольких виртуальных машинах.
Резервное копирование с использованием [параметров диагностики](https://docs.microsoft.com/azure/azure-monitor/platform/diagnostic-logs-overview)  | Не поддерживается. <br/><br/> Если восстановление виртуальной машины Azure с параметрами диагностики запускается с помощью команды [Создать](backup-azure-arm-restore-vms.md#create-a-vm), оно завершается сбоем.
...
Виртуальные машины 2-го поколения | Поддерживается <br> Служба Azure Backup поддерживает резервное копирование и восстановление [виртуальных машин 2-го поколения](https://azure.microsoft.com/updates/generation-2-virtual-machines-in-azure-public-preview/). При восстановлении этих виртуальных машин из точки восстановления они восстанавливаются как [виртуальные машины 2-го поколения](https://azure.microsoft.com/updates/generation-2-virtual-machines-in-azure-public-preview/).

## <a name="vm-storage-support"></a>Поддержка хранилища виртуальной машины

**Компонент** | **Поддержка**
--- | ---
Диски данных виртуальной машины Azure | Поддержка резервного копирования виртуальных машин Azure с количеством дисков до 32 доступна в [этих регионах](#backup-of-azure-virtual-machines-with-up-to-32-disks) в состоянии общедоступной предварительной версии.<br><br> Поддержка резервного копирования виртуальных машин Azure с неуправляемыми дисками или классических виртуальных машин возможна только при количестве дисков до 16.
...
Диски с включенным ускорителем записи | Не поддерживается.<br/><br/> Azure Backup автоматически исключает диски с включенным ускорителем записи во время резервного копирования. Так как резервное копирование для них не выполняется, восстановить эти диски из точек восстановления виртуальной машины невозможно. <br><br> **Важное примечание**. Виртуальным машинам с дисками с включенным ускорителем записи требуется подключение к Интернету для успешного резервного копирования (даже если эти диски исключены из резервного копирования).
Резервное копирование и восстановление дедуплицированных виртуальных машин и дисков | Azure Backup не поддерживает дедупликацию. Дополнительные сведения см. в [этой статье](https://docs.microsoft.com/azure/backup/backup-support-matrix#disk-deduplication-support). <br/> <br/>  — Azure Backup не выполняет дедупликацию в виртуальных машинах в хранилище Служб восстановления. <br/> <br/>  — Если во время восстановления виртуальные машины находятся в состоянии дедупликации, файлы нельзя восстановить, так как хранилище не распознает такой формат. Однако можно успешно выполнить полное восстановление виртуальных машин.
...
Общее хранилище| Резервное копирование виртуальных машин с помощью общего тома кластера (CSV) или масштабируемого файлового сервера не поддерживается. Модули записи общего тома кластера, скорее всего, не будут работать во время резервного копирования. При восстановлении диски, содержащие тома CSV, могут не извлекаться.
[Общие диски](https://docs.microsoft.com/azure/virtual-machines/windows/disks-shared-enable) | Не поддерживается.

### <a name="backup-of-azure-virtual-machines-with-up-to-32-disks"></a>Резервное копирование виртуальных машин Azure с емкостью до 32 дисков

Azure Backup теперь поддерживает резервное копирование виртуальных машин Azure с количеством подключенных дисков до 32.  Эта функция находится в стадии общедоступной предварительной версии в следующих регионах: Центрально-западная часть США, Центральная Канада, Юго-Восточная Азия, Южная Бразилия, Восточная Канада, Центральная Франция, Южная Франция, Центральная Индия, Южная Индия, Восточная Япония, Западная Япония, Республика Корея (центральный регион), Республика Корея (южный регион), Северная часть ЮАР, Южная часть Соединенного Королевства, Западная часть Соединенного Королевства, Восточная Австралия.  Если вам нужна эта функция в других регионах, зарегистрируйтесь для получения ограниченной предварительной версии, написав нам по адресу AskAzureBackupTeam@microsoft.com.  

>[!NOTE]
>Azure Backup поддерживает до 16 дисков для виртуальных машин Azure с неуправляемыми дисками или классических виртуальных машин.
...
- Для виртуальных машин Azure расширение виртуальной машины считывает данные напрямую из учетной записи хранения Azure по сети хранения данных. Сжатие этого трафика необязательно.
