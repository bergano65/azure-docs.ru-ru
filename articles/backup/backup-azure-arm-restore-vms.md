...
5. В поле **место хранения**укажите учетную запись хранения для виртуальной машины. [Дополнительные сведения](#storage-accounts)
...
3. В разделе **Учетная запись хранения** укажите учетную запись, в которую необходимо скопировать виртуальные жесткие диски. [Дополнительные сведения](#storage-accounts)
...
3. В поле **промежуточное расположение**укажите, где должны сохраняться моментальные снимки текущих управляемых дисков в процессе восстановления. [Дополнительные сведения](#storage-accounts)
...
Чтобы перейти к функции в предварительной версии, ознакомьтесь с [разделом перед началом](https://docs.microsoft.com/azure/backup/backup-create-rs-vault#set-cross-region-restore).
...
1. На портале выберите **хранилище служб восстановления** > **архивные элементы** .
...
1. На портале перейдите к **хранилищу служб восстановления** > **задания резервного копирования** .
2. Щелкните **дополнительный регион** , чтобы просмотреть элементы в дополнительном регионе.
...
**Закрепленные виртуальные машины зоны** | Azure Backup поддерживает резервное копирование и восстановление закрепленных виртуальных машин с зоной. [Дополнительные сведения](https://azure.microsoft.com/global-infrastructure/availability-zones/)
...
- Если виртуальная машина, для которой выполнено резервное копирование, имеет статический IP-адрес, восстановленная виртуальная машина будет иметь динамический IP-адрес, чтобы избежать конфликтов. Для [восстановленной виртуальной машины можно добавить статический IP-адрес](/previous-versions/azure/virtual-network/virtual-networks-reserved-private-ip#how-to-add-a-static-internal-ip-to-an-existing-vm).
...
- Если не удается получить доступ к виртуальной машине после восстановления из-за неработающего отношения виртуальной машины с контроллером домена, выполните следующие действия, чтобы открыть виртуальную машину.
