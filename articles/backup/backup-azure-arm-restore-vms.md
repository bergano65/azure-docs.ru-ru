...
С помощью этой колонки можно разными способами выполнить восстановление. Обратите внимание на то, что в этой колонке отображены только точки восстановления за последние 30 дней.
    ...
    а) Щелкните правой кнопкой мыши точку восстановления в этой колонке (которой менее 30 дней) и инициируйте **восстановление виртуальной машины**.

    б) Чтобы воспользоваться точками восстановления, которым больше 30 дней, можно щелкнуть ссылку "щелкните здесь" в колонке.

    в) С помощью меню **Restore VM** (Восстановить виртуальную машину) в заголовке можно просмотреть и отфильтровать список виртуальных машин по настраиваемым датам на свой выбор.

    Используйте фильтр, чтобы изменить диапазон времени отображения точек восстановления. По умолчанию отображаются точки восстановления всех уровней согласованности. Измените значение фильтра "All restore points" (Все точки восстановления), чтобы выбрать определенный уровень согласованности точек восстановления. Дополнительные сведения о типах точек восстановления см. в разделе [Согласованность данных](backup-azure-vms-introduction.md#data-consistency).
  ...
  ![Точки восстановления](./media/backup-azure-arm-restore-vms/vm-blade1.png)
    ...
    >  [!NOTE]
    > Тип восстановления определяет расположение данных: учетная запись хранения клиента, хранилище или оба эти варианта. Узнайте больше о [точке мгновенного восстановления](https://azure.microsoft.com/blog/large-disk-support/).

8. В колонке **Восстановление** выберите **Точка восстановления**.

    ![Выбор точки восстановления](./media/backup-azure-arm-restore-vms/select-recovery-point1.png)

    В колонке **Восстановление** показано, что точка восстановления выбрана с помощью кнопки **ОК**.
...
2. Эта колонка в настоящее время содержит два параметра: **Create New** (Создать), который используется по умолчанию, и **Replace existing** (Заменить существующие), который позволяет выполнить восстановление на месте, чтобы заменить диски, сохранив только существующие конфигурации и расширения.

> [!NOTE]
> Несколько следующих месяцев мы посвятим разработке полной замены виртуальной машины, включая ее диски, параметры сети, конфигурации и расширения.
>
>

 Для параметра **Create New** (Создать), который позволяет восстановить данные в новую виртуальную машину или на новые диски, доступны два варианта.

 ![Мастер настройки восстановления](./media/backup-azure-arm-restore-vms/restore-configuration-create-new.png)
...
1. В колонке **Restore configuration** (Конфигурация восстановления), упомянутой в предыдущем разделе, введите или выберите значения для каждого из следующих полей.
    ...
    Е. **Расположение хранилища**. Учетные записи хранения используются для промежуточного хранения и обработки. В этом меню перечислены учетные записи хранения в том же расположении, что и хранилище служб восстановления. Учетные записи хранения, избыточные в пределах зоны, не поддерживаются. Если у вас нет учетной записи хранения в одном расположении с хранилищем служб восстановления, ее следует создать до запуска операции восстановления. Тип репликации учетной записи хранения отображается в скобках.

    ![Мастер настройки восстановления](./media/backup-azure-arm-restore-vms/recovery-configuration-wizard1.png)

    > [!NOTE]
    > * Виртуальная сеть является необязательной для классической виртуальной машины и обязательной для виртуальной машины, развернутой с помощью Resource Manager.

    > * Тип хранилища, указанный для учетной записи хранения ценовой категории "Премиум" или "Стандартный" в расположении промежуточного хранения, определяет тип хранилища диска восстановления. Сейчас смешанный режим дисков при восстановлении не поддерживается.
    >
    >
...
Значение типа восстановления **Restore disk** в колонке **Restore configuration** (Конфигурация восстановления) позволяет создать виртуальную машину с помощью настраиваемых конфигураций. При восстановлении дисков выбираемая учетная запись хранения должна находиться в том же расположении, что и хранилище служб восстановления. Если у вас нет учетных записей хранения в одном расположении с хранилищем служб восстановления, требуется создать такую учетную запись. Учетные записи хранения ZRS не поддерживаются. Тип репликации учетной записи хранения отображается в скобках.

После операции восстановления можно выполнить следующее:
...
**Восстановление на месте** можно выполнить с помощью вкладки **Replace Existing** (Заменить существующие).

## <a name="replace-existing-disks-from-a-restore-point"></a>Замена существующих дисков из точки восстановления
Параметр **Replace Existing** (Заменить существующие) позволяет заменить диски в текущей виртуальной машине дисками из выбранной точки восстановления. Эта операция может быть выполнена только при наличии текущей виртуальной машины. Если по какой-либо причине она была удалена, эту операция невозможно будет выполнить. В качестве альтернативы мы рекомендуем **создать** виртуальную машину или диски, чтобы продолжить операции восстановления. Во время замены существующих дисков в качестве меры предосторожности мы выполняем предварительное резервное копирование данных. В результате создается моментальный снимок и точка восстановления в хранилище с периодом хранения, соответствующим запланированному в настроенной политике резервного копирования. Если точка восстановления содержит больше или меньше дисков, чем текущая виртуальная машина, то в виртуальной машине будет отражено только количество дисков в точке восстановления. Вариант **Заменить существующие** для неуправляемых дисков и зашифрованных виртуальных машин в настоящее время не поддерживается. Он также не поддерживается для [генерализованных виртуальных машин](https://docs.microsoft.com/azure/virtual-machines/windows/capture-image-resource) и виртуальных машин, созданных из [пользовательских образов](https://azure.microsoft.com/resources/videos/create-a-custom-virtual-machine-image-in-azure-resource-manager-with-powershell/).  

 В колонке **Restore configuration** (Конфигурация восстановления) следует указать только параметр **Расположение промежуточного хранения**.
 ...
 a. **Тип восстановления**. Диски в существующей виртуальной машине заменяются дисками из выбранной точки восстановления.

 b. **Расположение промежуточного хранения**. Учетные записи хранения используются для промежуточного хранения и обработки управляемых дисков. В этом меню перечислены учетные записи хранения в том же расположении, что и хранилище служб восстановления. Учетные записи хранения, избыточные в пределах зоны, не поддерживаются. Если у вас нет учетной записи хранения в одном расположении с хранилищем служб восстановления, ее следует создать до запуска операции восстановления. Тип репликации учетной записи хранения отображается в скобках.
