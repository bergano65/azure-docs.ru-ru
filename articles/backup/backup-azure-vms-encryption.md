...
При шифровании дисков с помощью ключей, управляемых клиентом (CMK), ключ, используемый для шифрования дисков, хранится в Azure Key Vault и управляется вами. Шифрование службы хранилища (SSE) с использованием CMK отличается от шифрования дисков Azure (ADE). ADE использует средства шифрования операционной системы. SSE шифрует данные в службе хранилища, позволяя использовать любую ОС или образы для виртуальных машин.

Вам не нужно выполнять явные действия для резервного копирования или восстановления виртуальных машин, которые используют ключи, управляемые клиентом, для шифрования дисков. Данные резервного копирования для этих виртуальных машин, хранящихся в хранилище, будут зашифрованы с помощью тех же методов, что и [Шифрование, используемое в хранилище](encryption-at-rest-with-cmk.md).

Дополнительные сведения о шифровании управляемых дисков с помощью управляемых клиентом ключей см. в [этой статье](../virtual-machines/disk-encryption.md#customer-managed-keys).
...
- Вы можете выполнять резервное копирование и восстановление зашифрованных виртуальных машин ADE в одной подписке и регионе.
...
- Вы можете выполнять резервное копирование и восстановление зашифрованных виртуальных машин ADE в той же подписке и регионе, что и хранилище резервных копий служб восстановления.
- Зашифрованные файлы ADE нельзя восстановить на уровне файлов и папок. Чтобы восстановить файлы и папки, необходимо восстановить всю виртуальную машину.
- При восстановлении виртуальной машины нельзя использовать параметр [заменить существующую виртуальную машину](backup-azure-arm-restore-vms.md#restore-options) для зашифрованных ВИРТУАЛЬНЫХ машин ADE. Этот параметр поддерживается только для незашифрованных управляемых дисков.
...
1. Выберите хранилище ключей, связанное с зашифрованной виртуальной машиной, для которой выполняется резервное копирование.
    ...
    >[!TIP]
    >Чтобы указать хранилище ключей, связанное с виртуальной машиной, используйте следующую команду PowerShell. Замените имя группы ресурсов и имя виртуальной машины:
    >
    >`Get-AzVm -ResourceGroupName "MyResourceGroup001" -VMName "VM001" -Status`
    >
    > Найдите имя хранилища ключей в этой строке:
    >
