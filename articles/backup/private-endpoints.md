...
1. В области **Добавление назначения ролей** выберите **участник** в качестве **роли**и используйте **имя** хранилища в качестве **участника**. Выберите хранилище и щелкните **сохранить** по завершении.
...
1. На панели навигации слева щелкните **частные конечные точки**. В области **частные конечные точки** щелкните **+ Добавить** , чтобы начать создание частной конечной точки для хранилища.
...
1. В процессе **создания частной конечной точки** вам потребуется указать сведения для создания подключения к частной конечной точке.
    ...
    1. Перейдите к разделу " **Проверка и создание** " после ввода подробностей. По завершении проверки нажмите кнопку **создать** , чтобы создать частную конечную точку.
...
>[!NOTE]
> Этот шаг не требуется, если используется интегрированная зона DNS. Но если вы создали собственную зону Azure Частная зона DNS или используете пользовательскую частную зону DNS, убедитесь, что записи созданы, как описано в этом разделе.
...
1. Перейдите к **частной зоне DNS** и перейдите к параметру **Обзор** на панели слева. После этого щелкните **+ набор записей** , чтобы начать добавление записей.
...
А) Нет, частные конечные точки можно создавать только для новых резервных хранилищ. Поэтому хранилище не должно иметь каких-либо элементов, защищенных в нем. На самом деле, перед созданием частных конечных точек не попытается защитить какие бы то ни было элементы в хранилище.
...
А) Нет, хранилище не должно иметь каких бы то ни было попыток защитить какие бы то ни было элементы в прошлом.
...
А) Нет, Частная конечная точка для резервного копирования может использоваться только для Azure Backup. Необходимо создать новую закрытую конечную точку для Azure Site Recovery, если она поддерживается службой.
...
А) Не выполнив действия, описанные в статье, и продолжение защиты элементов может привести к тому, что хранилище не сможет использовать частные конечные точки. Поэтому рекомендуется ознакомиться с этим контрольным списком перед тем, как перейти к защите элементов.
...
А) Да, можно использовать собственные DNS-серверы. Однако убедитесь, что все необходимые записи DNS добавлены в этом разделе как предложенные.

У. Нужно ли выполнять дополнительные действия на моем сервере после выполнения процедуры в этой статье?<br>
А) После выполнения процесса, описанного в этой статье, вам не нужно выполнять дополнительную работу, чтобы использовать частные конечные точки для резервного копирования и восстановления.
