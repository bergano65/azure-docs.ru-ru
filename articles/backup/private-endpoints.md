...
Необходимо создать три частные зоны DNS и связать их с виртуальной сетью. Помните, что, в отличие от BLOB и очереди, общедоступные URL-адреса службы резервного копирования не регистрируются в общедоступном DNS-сервере Azure для перенаправления в зоны DNS частной связи. 
