...
1. На портале > > архивные**элементы** **хранилища служб восстановления**выберите защищенные элементы в облаке (например, компьютер азуревиртуал, служба хранилища Azure (службы файлов Azure), SQL на виртуальной машине Azure и т. д.).

    ![выбор типа резервной копии](./media/backup-azure-delete-vault/azure-storage-selected.png)

2. Щелкните правой кнопкой мыши элемент резервного копирования. В зависимости от того, защищен элемент резервного копирования или нет, в меню будет отображаться пункт « **закончить архивацию** » или « **удалить данные резервной**копии».
...
- Вам будет предложено ввести ПИН-код безопасности. Чтобы создать ПИН-код, выполните следующие действия.
  ...
  - В разделе **ПИН-код безопасности** щелкните **Создать**. Скопируйте этот ПИН-код. (Этот ПИН-код действителен только в течение пяти минут.)
...
**Метод 2** Запустите консоль **управления MABS** . В разделе **Выбор метода защиты данных** снимите флажок **я хочу оперативную защиту**.
...
1. Установите шоколад [отсюда](https://chocolatey.org/) и установите ARMClient, выполнив приведенную ниже команду:
