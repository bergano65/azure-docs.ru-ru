...
5. Проверьте значок **уведомления** : значок уведомления ![The. ](./media/backup-azure-delete-vault/messages.png) По завершении процесса служба отобразит следующее сообщение: *Идет остановка резервного копирования и удаление данных резервного копирования для "* Элемент архивации *"* . *Операция успешно завершена*.
...
5. Проверьте значок **уведомления** ![delete Backup Data @ no__t-2. После завершения операции служба отобразит сообщение: *Идет остановка резервного копирования и удаление данных резервного копирования для элемента Backup.* *Операция успешно завершена*.
...
5. Перейдите в **хранилище служб восстановления** > **Параметры**@no__t **-3.**
...
8. На странице **ход изменения резервной копии** появится следующее сообщение: данные резервного копирования *Deleted будут храниться в течение 14 дней. По истечении этого времени данные резервного копирования будут безвозвратно удалены.*  
...
- Отключите защиту и удалите данные для всех элементов, защищенных с резервным копированием в облаке (например, Виртуальная машина Лаас, файловый ресурс Azure и т. д.):
    ...
    ```PowerShell
       ...
       [<CommonParameters>] 
    ```
    [Дополнительные @no__t-](https://docs.microsoft.com/powershell/module/az.recoveryservices/disable-azrecoveryservicesbackupprotection?view=azps-2.6.0&viewFallbackFrom=azps-2.5.0)1about отключает защиту для элемента, защищенного с помощью резервной копии. 
...
1. Выполните следующую команду, используя идентификатор подписки, имя группы ресурсов и имя хранилища. Если у вас нет зависимостей, хранилище удаляется при выполнении следующей команды:
   ...
   ```
2. Если хранилище не пусто, вы получите следующее сообщение об ошибке: *Невозможно удалить хранилище, так как в этом хранилище есть ресурсы.* Чтобы удалить защищенный элемент или контейнер в хранилище, выполните следующую команду:
