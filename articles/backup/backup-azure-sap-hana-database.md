...
> [!NOTE]
> Прокси уровня службы не поддерживается. Это значит, что трафик через прокси-сервер из нескольких или выбранных служб (службы архивации Azure) не поддерживается. Все данные или трафик могут направляться прокси-сервером.
...
1. В меню **Full Backup policy** (Политика полного резервного копирования) для параметра **Частота архивации** выберите **Ежедневно** или **Еженедельно**.
...
1. В разделе **Диапазон хранения** настройте параметры периода удержания для полной резервной копии.
...
1. В меню **Full Backup policy** (Политика полного резервного копирования) щелкните **ОК**, чтобы подтвердить параметры.
1. Чтобы добавить политику разностного резервного копирования, щелкните **Разностная резервная копия**.
1. В меню **Differential Backup policy** (Политика разностной резервной копии) выберите **Включить** для открытия элементов управления периодичностью и хранением.
    ...
    * Максимальный срок хранения разностных резервных копий составляет 180 дней. Если требуется более длительное хранение, необходимо использовать полные резервные копии.
    ...
    > [!NOTE]
    > Теперь в общедоступной предварительной версии поддерживаются добавочные резервные копии. Для ежедневного резервного копирования можно выбрать либо разностное, либо добавочное копирование, но не оба варианта.
1. В меню **Политика добавочного резервного копирования** выберите **Включить**, чтобы открыть элементы управления периодичностью и хранением.
...
1. Для сохранения политики и возврата к главному меню **Политика архивации** нажмите кнопку **ОК**.
1. Чтобы добавить политику резервного копирования журналов транзакций, выберите **Резервная копия журналов**.
...
1. Для сохранения политики и возврата к главному меню **Политика архивации** нажмите кнопку **ОК**.
1. После завершения определения политики резервного копирования нажмите кнопку **ОК**.
...
1. Верните предыдущие значения параметров в Azure.
    ...
    * Задайте  для Catalog_backup_using_backint **значение true**.
