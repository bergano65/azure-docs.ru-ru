...
* **Журнал событий** может показывать ошибки резервного копирования, которые относятся к другим продуктам резервного копирования, например системе архивации данных Windows Server, а также не Azure Backup. Чтобы определить, связан ли сбой с Azure Backup, выполните следующие действия.
  ...
  * Ниже приведен пример ошибки Просмотр событий 517, где Azure Backup работал нормально, но ошибка "cистема архивации данных Windows Server":<br>
...
Эта ошибка возникает из-за того, что модули записи VSS находятся в неправильном состоянии. Расширения Azure Backup взаимодействуют с модулями записи VSS, чтобы создавать моментальные снимки дисков. Проблему можно устранить следующим способом.

Перезапустите модули записи VSS, которые находятся в неисправном состоянии.
- В командной строке с повышенными привилегиями выполните команду ```vssadmin list writers```.
- В выходных данных содержатся все модули записи VSS и их состояние. Для каждого модуля записи VSS с состоянием, которое не является **[1]**, перезапустите соответствующую службу модуля записи VSS. 
...
Если перезапуск модулей записи VSS не решил проблему, но проблема сохраняется из-за истечения времени ожидания, то:
- Выполните следующую команду из командной строки с повышенными привилегиями (от имени администратора), чтобы предотвратить создание потоков для моментальных снимков BLOB-объектов.
...
Эта ошибка возникает из-за неправильного состояния службы VSS. Расширения Azure Backup взаимодействуют со службой VSS для создания моментальных снимков дисков. Проблему можно устранить следующим способом.

Перезапустите службу VSS (теневое копирование томов).
- Перейдите в раздел Services. msc и перезапустите службу теневого копирования томов.<br>
или<br>
...
Если проблема сохраняется, перезапустите виртуальную машину в запланированное время простоя.
...
Чтобы устранить эту проблему, используйте параметр [восстановить диски](https://docs.microsoft.com/azure/backup/backup-azure-arm-restore-vms#restore-disks) во время операции восстановления. Используйте эти диски для создания виртуальной машины из списка [доступных размеров виртуальных машин](https://docs.microsoft.com/azure/backup/backup-support-matrix-iaas#vm-compute-support) с помощью [командлетов PowerShell](https://docs.microsoft.com/azure/backup/backup-azure-vms-automation#create-a-vm-from-restored-disks).
...
Azure Backup поддерживает резервное копирование и восстановление виртуальных машин, доступных в Azure Marketplace. Эта ошибка возникает при попытке восстановить виртуальную машину (с указанным параметром плана или издателя), которая больше не доступна в Azure Marketplace. [Дополнительные сведения](https://docs.microsoft.com/legal/marketplace/participation-policy#offering-suspension-and-removal)см. здесь.
- Чтобы устранить эту проблему, используйте параметр [восстановить диски](https://docs.microsoft.com/azure/backup/backup-azure-arm-restore-vms#restore-disks) во время операции восстановления, а затем используйте [PowerShell](https://docs.microsoft.com/azure/backup/backup-azure-vms-automation#create-a-vm-from-restored-disks) или командлеты [Azure CLI](https://docs.microsoft.com/azure/backup/tutorial-restore-disk) , чтобы создать виртуальную машину с последними сведениями Marketplace, соответствующими виртуальной машине.
