...
10. После включения резервного копирования выполняется [начальное резервное копирование](#run-the-initial-backup). Вы можете немедленно запустить начальное резервное копирование или дождаться его запуска в соответствии с расписанием резервного копирования.
...
> [!NOTE]
> Azure Backup создает отдельную группу ресурсов (отличную от группы ресурсов виртуальной машины) для хранения точек восстановления с форматом именования **AzureBackupRG_geography_number** (пример: AzureBackupRG_northeurope_1). Вам не следует блокировать эту группу ресурсов.
