...
- Отчеты для Базы данных SQL Azure, Data Protection Manager и сервера Azure Backup пока не поддерживаются.
...
- В Power BI частота запланированного обновления для отчетов составляет 24 часа. Кроме этого, отчеты в Power BI можно обновлять вручную. В этом случае для создания отчетов используются данные, которые присутствуют в клиентской учетной записи хранения на момент обновления.
...
2. В списке элементов, которые отображаются в разделе **Мониторинг и отчеты** под хранилищем, щелкните **Отчеты службы архивации**, чтобы настроить для отчетов эту учетную запись хранения.
      ...
      ![Шаг 2. Выбор пункта меню "Отчеты службы архивации"](./media/backup-azure-configure-reports/backup-reports-settings.PNG)
...
> [!NOTE]
> Когда вы настроите отчеты и сохраните учетную запись хранения, *подождите 24 часа*, пока завершится начальная отправка данных. Только после этого можно импортировать пакет содержимого Azure Backup в Power BI. Дополнительные сведения см. в разделе [Часто задаваемые вопросы](#frequently-asked-questions). 
>
>
...
2. Выберите **Получение данных**. В **библиотеке пакетов содержимого** откройте раздел **Службы** и выберите **Получить**. Выполните действия, указанные в статье [Подключение к используемым службам с помощью Power BI](https://powerbi.microsoft.com/documentation/powerbi-content-packs-services/).
...
5. Введите ключ для этой учетной записи хранения. Чтобы [просмотреть или скопировать ключи учетных записей хранения](../storage/common/storage-account-manage.md#access-keys), перейдите в свою учетную запись хранения на портале Azure. 
...
7. После успешного импорта данных пакет содержимого **Azure Backup** появится в колонке **Приложения** в области навигации. Теперь в разделах **Панели мониторинга**, **Отчеты** и **Наборы данных** служба Azure Backup отмечена желтой звездочкой, которая обозначает новые импортированные отчеты.
...
**Как проверить, поступают ли данные отчетов в учетную запись хранения?**
...
**С какой частотой данные отправляются в учетную запись хранения и в пакет содержимого службы Azure Backup в Power BI?**
...
**Как долго я могу хранить отчеты?**

   При настройке учетной записи хранения вы можете выбрать период хранения данных отчетов в учетной записи хранения. Выполните шаг 6 из предыдущего раздела "Настройка учетной записи хранения для отчетов". Кроме того, отчеты можно [анализировать в Excel](https://powerbi.microsoft.com/documentation/powerbi-service-analyze-in-excel/) и сохранять на более длительный период времени, если возникнет такая потребность.

**Увижу ли я все свои данные в отчетах после настройки учетной записи хранения?**
...
**Если для отчетов уже настроена учетная запись хранения, могу ли я изменить конфигурацию и использовать другую учетную запись хранения?**
...
**Могу ли я ли просмотреть отчеты по нескольким хранилищам и подпискам?**
