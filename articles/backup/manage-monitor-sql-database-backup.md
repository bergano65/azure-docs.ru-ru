...
Рекомендуется использовать этот параметр с осторожностью. При запуске на виртуальной Машине с расширением уже работоспособное, эта операция приведет к расширение, чтобы получить перезапуска. Это может привести все задания выполняется переход на другой. Сбое проверьте для одного или нескольких [Симптомы](backup-sql-server-azure-troubleshoot.md#re-registration-failures) перед запуском операции повторной регистрации.
