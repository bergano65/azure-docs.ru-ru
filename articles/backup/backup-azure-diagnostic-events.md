...
При использовании [устаревшего установите azurebackupreport событий](https://docs.microsoft.com/azure/backup/backup-azure-diagnostic-events#legacy-event) рекомендуется переключиться на использование описанных выше событий в самом раннем случае.

Дополнительные сведения см. в разделе [модель данных для событий диагностики Azure Backup](https://docs.microsoft.com/azure/backup/backup-azure-reports-data-model).
...
Теперь можно использовать Azure Backup для отправки данных диагностики хранилища в выделенные Log Analytics таблицы для резервного копирования. Эти таблицы называются [таблицами, зависящими от ресурсов](https://docs.microsoft.com/azure/azure-monitor/platform/resource-logs-collect-workspace#resource-specific).
...
В настоящее время мы продолжаем поддерживать событие установите azurebackupreport для обратной совместимости в случаях, когда у пользователей есть пользовательские запросы к этому событию. Примерами могут быть пользовательские оповещения журнала и пользовательские визуализации. *Рекомендуется как можно раньше перейти к [новым событиям](https://docs.microsoft.com/azure/backup/backup-azure-diagnostic-events#diagnostics-events-available-for-azure-backup-users) *. Новые события:
...
*Устаревшее событие в режиме диагностики Azure в конечном итоге будет устаревшим. Выбор новых событий может помочь избежать сложных миграций позднее*. Наше [решение для создания отчетов](https://docs.microsoft.com/azure/backup/configure-reports) , использующее log Analytics, также останавливает поддержку данных из устаревшего события.
...
1. Выясните, какие хранилища отправляют данные в Log Analytics рабочие области с помощью устаревшего события и подписок, к которым они относятся. Выполните следующие рабочие области для обнаружения этих хранилищ и подписок.
