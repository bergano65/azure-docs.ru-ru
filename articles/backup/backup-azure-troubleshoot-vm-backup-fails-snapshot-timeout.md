...
Эта ошибка возникает, когда один из ошибок расширения переводит виртуальную машину в состояние сбоя подготовки.<br>**Откройте портал Azure > VM > Settings > extensions > Extensions Status** и убедитесь, что все расширения находятся в состоянии **успешной подготовки** .
...
4. Выберите **расширение моментального снимка**.
5. Выберите **Удалить**.

Если на виртуальной машине Linux расширение VMSnapshot не отображается в портал Azure, [Обновите агент Linux для Azure](../virtual-machines/linux/update-agent.md), а затем запустите резервное копирование.
