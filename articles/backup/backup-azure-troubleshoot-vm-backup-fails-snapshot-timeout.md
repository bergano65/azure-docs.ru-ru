...
**Причина 1. [Агент установлен на виртуальной машине, но он не отвечает (для виртуальных машин Windows)](#the-agent-installed-in-the-vm-but-unresponsive-for-windows-vms)**    
**Причина 2. [Агент, установленный на виртуальной Машине является устаревшим (для виртуальных машин Linux)](#the-agent-installed-in-the-vm-is-out-of-date-for-linux-vms)**  
**Причина 3. [Не удалось получить состояние моментального снимка или создать моментальный снимок не может быть](#the-snapshot-status-cannot-be-retrieved-or-a-snapshot-cannot-be-taken)**    
...
* Эта ошибка также может произойти при активации нескольких операций резервного копирования в день. В настоящее время мы рекомендуем только одна резервная копия в день, как точки мгновенное восстановление хранятся в течение 1 – 5 дней согласно сохранения настроенных моментального снимка и только 18 мгновенных RPs можно связать с виртуальной Машиной в любой момент времени. <br>
...
**Шаг 1. [Снять блокировку на группу ресурсов для точки восстановления](#remove_lock_from_the_recovery_point_resource_group)** <br>
**Шаг 2. [Очистить коллекцию точек восстановления](#clean_up_restore_point_collection)**<br>
...
После регистрации виртуальной машины в службе архивации Azure и добавления ее в расписание служба архивации инициирует задание, взаимодействуя с расширением виртуальной машины, чтобы создать моментальный снимок. Любое из указанных ниже условий может помешать активации создания моментального снимка, что может привести к сбою службы Backup. Выполните следующие шаги про устранению неполадок в указанном порядке, а затем повторите операцию:    
**Причина 1. [Не удалось получить состояние моментального снимка или создать моментальный снимок не может быть](#the-snapshot-status-cannot-be-retrieved-or-a-snapshot-cannot-be-taken)**  
...
**Причина 1. [Не удалось получить состояние моментального снимка или создать моментальный снимок не может быть](#the-snapshot-status-cannot-be-retrieved-or-a-snapshot-cannot-be-taken)**  
**Причина 2. [Не удалось обновить или загрузить расширение резервного копирования](#the-backup-extension-fails-to-update-or-load)**  
**Причина 3. [Агент установлен на виртуальной машине, но он не отвечает (для виртуальных машин Windows)](#the-agent-installed-in-the-vm-but-unresponsive-for-windows-vms)**  
**Причина 4. [Агент, установленный на виртуальной Машине является устаревшим (для виртуальных машин Linux)](#the-agent-installed-in-the-vm-is-out-of-date-for-linux-vms)**
...
После регистрации виртуальной машины в службе архивации Azure и добавления ее в расписание служба архивации инициирует задание, взаимодействуя с расширением виртуальной машины, чтобы создать моментальный снимок. Любое из указанных ниже условий может помешать активации создания моментального снимка, что может привести к сбою службы Backup. Выполните следующие шаги про устранению неполадок в указанном порядке, а затем повторите операцию:  
**Причина 1. [Агент, установленный на виртуальной Машине, но он не отвечает (для виртуальных машин Windows)](#the-agent-installed-in-the-vm-but-unresponsive-for-windows-vms)**  
**Причина 2. [Агент, установленный на виртуальной Машине является устаревшим (для виртуальных машин Linux)](#the-agent-installed-in-the-vm-is-out-of-date-for-linux-vms)**  
**Причина 3. [Не удалось получить состояние моментального снимка или создать моментальный снимок не может быть](#the-snapshot-status-cannot-be-retrieved-or-a-snapshot-cannot-be-taken)**  
**Причина 4. [Не удалось обновить или загрузить расширение резервного копирования](#the-backup-extension-fails-to-update-or-load)**  
**Причина 5. Служба Backup не имеет разрешения на удаление старых точек восстановления из-за блокировки группы ресурсов** <br>
...
2. Убедитесь, что запущен агент Azure на виртуальной Машине, выполнив следующую команду: `ps -e`
...
* [Очистка восстановления коллекцию точек, выполнив Нерегламентированное резервное копирование](#clean-up-restore-point-collection-by-running-ad-hoc-backup)<br>
* [Очистка и восстановление точки коллекции на портале Azure](#clean-up-restore-point-collection-from-azure-portal)<br>

#### <a name="clean-up-restore-point-collection-by-running-ad-hoc-backup"></a>Очистка восстановления коллекцию точек, выполнив Нерегламентированное резервное копирование
После удаления блокировки, активируйте резервное копирование ad hoc или вручную. Это обеспечит автоматическую очистку точек восстановления. Ожидается, что эта операция ad hoc и пользовательские, переход на другой первый раз; Тем не менее она обеспечит автоматическую очистку вместо ручного удаления точек восстановления. После очистки следующее запланированное резервное копирование должно быть выполнено успешно.
