---
title: Восстановление данных в Azure на сервере Windows
description: В этой статье узнайте, как восстановить данные, хранящиеся в Azure, на сервере Windows или windows-компьютере с помощью агента Microsoft Azure Recovery Services (MARS).
ms.reviewer: saurse
ms.topic: conceptual
ms.date: 09/07/2018
...
3. На странице **Getting Started,** чтобы восстановить данные на том же сервере или компьютере, выберите **этот сервер (`<server name>`)** > **Далее**.
...
4. В области **Выбор режима восстановления** выберите **Отдельные файлы и папки** > **Далее**.
   ...
   > [!TIP]
   > **Индивидуальные файлы и папки** параметр ы позволяют быстро получить доступ к данным точки восстановления. Параметр подходит для восстановления отдельных файлов с общим размером не более 80 ГБ, а также обеспечивает скорость передачи или копирования до 6 Мбит/с во время восстановления. Параметр **Том** позволяет восстановить все резервные копии на определенном томе. Этот параметр обеспечивает более высокую скорость передачи (до 60 Мбит/с), что идеально подходит для восстановления большого объема данных или целых томов.
...
7. На странице **«Просмотр и восстановление файлов»** выберите **«Прожектор»** и найдите нужные файлы и папки.
...
* *Исходная машина* — оригинальная машина, из которой была взята резервная часть, и которая в настоящее время недоступна.
* *Целевой компьютер* — компьютер, на который восстанавливаются данные.
* *Образец хранилища* - Хранилище служб восстановления, в котором зарегистрированы исходная машина и целевая машина.

> [!NOTE]
> Резервные копии невозможно восстановить на целевой компьютер с более ранней версией операционной системы. Например, резервное копирование, взятое с компьютера Windows 7, может быть восстановлено на компьютере с Windows 7 (или позже). а резервную копию, созданную на компьютере Windows 8, невозможно восстановить на компьютере Windows 7.
>
>

1. Откройте резервное **копирование Microsoft Azure** на целевой машине.
...
6. На странице **Выбор резервного сервера** из списка компьютеров выберите исходный компьютер и укажите парольную фразу. Затем выберите **Следующий**.
...
10. На странице **Browse and Recover Files** выберите **«Просмотреть»** и открыть Windows Explorer и найти нужные файлы и папки.
...
* Найдите [общие вопросы о резервном копировании файлов и папок.](backup-azure-file-folder-backup-faq.md)
