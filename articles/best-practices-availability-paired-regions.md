...
> [!NOTE]
> Назначенные национальные пары Azure нельзя изменить.

Рисунок 1. Пары регионов Azure

| Geography | Пары регионов |  |
|:--- |:--- |:--- |
...
| Germany |Центральная Германия |Северо-восточная Германия |
| Индия |Центральная Индия |Южная Индия |
| Индия |Западная Индия |Южная Индия |
| Япония |Восточная Япония |Западная Япония |
| Korea |Республика Корея, центральный регион |Республика Корея, южный регион |
...
| Южная Африка | Северная часть ЮАР; | Западная Южная Африка
| Великобритания |западная часть Соединенного Королевства |южная часть Соединенного Королевства |
...
![Azure SQL](./media/best-practices-availability-paired-regions/3Green.png) **база данных SQL Azure** . с помощью Георепликации базы данных SQL Azure можно настроить асинхронную репликацию транзакций в любой регион мира. Однако рекомендуется развертывать эти ресурсы в парном регионе для большинства сценариев аварийного восстановления. Дополнительные сведения см. в статье [Обзор: активная георепликация для базы данных SQL Azure](sql-database/sql-database-geo-replication-overview.md).

![диспетчер ресурсов](./media/best-practices-availability-paired-regions/4Green.png) **Azure Resource Manager** -диспетчер ресурсов обеспечивает логическую изоляцию компонентов между регионами. Это означает, что логические сбои в одном регионе едва ли затронут другой регион.
