---
title: Решения Azure VMware (AVS). Настройка высокой доступности из локальной среды в VPN-шлюз AVS
description: В этой статье описывается, как настроить подключение высокого уровня доступности из локальной среды к VPN-шлюзу AVS, для которого включена высокая доступность.
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/14/2019
ms.topic: article
...
---
# <a name="configure-a-high-availability-connection-from-on-premises-to-an-avs-vpn-gateway"></a>Настройка подключения высокой доступности из локальной среды к VPN-шлюзу AVS

Сетевые администраторы могут настроить высокодоступное VPN-подключение типа "сеть — сеть" IPsec из локальной среды к VPN-шлюзу AVS.
...
1. Убедитесь, что в организации [подготовлены](create-nodes.md) необходимые узлы и создано по крайней мере одно частное облако AVS.
2. [Настройте VPN-шлюз типа "сеть — сеть](vpn-gateway.md#set-up-a-site-to-site-vpn-gateway) " между локальной сетью и частным облаком AVS.
...
Для работы VPN-подключения типа "сеть — сеть" необходимо разрешить UDP 500/4500 и ESP (IP-протокол 50) из основного и дополнительного общедоступного IP-адреса (Peer IP) на внешнем интерфейсе локального VPN-шлюза Cisco ASA.
...
Чтобы настроить этап 2 (IPsec), создайте список управления доступом (ACL), определяющий трафик для шифрования и туннелирования. В следующем примере нужного трафика является из туннеля, источником которого является локальная локальной подсети (10.16.1.0/24), в удаленную подсеть частного облака AVS (192.168.0.0/24). Список ACL может содержать несколько записей, если между сайтами есть несколько подсетей.
...
#### <a name="define-the-avs-remote-subnet-as-an-object"></a>Определение удаленной подсети AVS в качестве объекта
...
Настройте набор преобразований (TS), который должен содержать ключевое слово ```ikev1```. Атрибуты шифрования и хеширования, указанные в службах терминалов, должны соответствовать параметрам, перечисленным в [конфигурации по умолчанию для VPN-ШЛЮЗОВ AVS](cloudsimple-vpn-gateways.md#cryptographic-parameters).
...
Чтобы VPN-подключение типа "сеть — сеть" работало, необходимо разрешить UDP 500/4500 и ESP (IP-протокол 50) из основного и дополнительного общедоступного IP-адреса (однорангового узла) на внешнем интерфейсе локального шлюза Palo Alto Networks.
...
* Имя интерфейса. Первое поле заполняется автоматически с помощью ключевого слова "Tunnel". В соседнем поле введите любое число от 1 до 9999. Этот интерфейс будет использоваться в качестве основного интерфейса туннеля для передачи трафика между сайтами между локальным центром обработки данных и частным облаком AVS.
...
### <a name="2-set-up-static-routes-for-avs-private-cloud-subnets-to-be-reached-over-the-site-to-site-vpn"></a>2. Настройте статические маршруты для подсетей с частным облаком AVS, которые будут достигнуты через VPN-подключение типа "сеть — сеть".

Маршруты необходимы для того, чтобы локальные подсети достигли подсетей закрытого облака AVS.

Выберите **сетевые** > **виртуальные маршрутизаторы** >  > **статические маршруты** *по умолчанию* > **Добавить**, настройте следующие поля и нажмите кнопку **ОК**.

* Название Введите любое имя для простоты идентификации назначения маршрута.

* Целевой объект. Укажите подсети с частным облаком AVS, которые будут достигнуты через туннельные интерфейсы S2S из локальной среды.
...
Повторите предыдущие шаги, чтобы создать другой маршрут для подсетей с частным облаком AVS для использования в качестве вторичного или резервного маршрута через дополнительный интерфейс туннелирования. На этот раз выберите другой идентификатор туннеля и метрику более высокого уровня, чем для основного маршрута.
...
Выберите **сетевые** > **разверните узел Сетевые профили** > **Шифрование IKE** > **Добавить**, настройте следующие поля и нажмите кнопку **ОК**.
...
Выберите **сетевые** > **разверните узел Сетевые профили** > **шлюзы IKE** > **Добавить**, настройте следующие поля и нажмите кнопку **ОК**.

Вкладка "Общие":

* Название Введите имя для шлюза IKE, который будет использоваться в качестве пиринга для основного VPN-узла AVS.
...
* Одноранговый адрес. Введите IP-адрес основного однорангового узла AVS.
* аутентификация; Выберите **общий ключ**.
* Предварительный ключ/подтверждение общего ключа. Введите общий ключ в соответствии с ключом VPN-шлюза AVS.
* Локальная идентификация. Введите общедоступный IP-адрес локального брандмауэра Palo Alto.
* Идентификация однорангового узла. Введите IP-адрес основного однорангового узла AVS.
...
Выберите **сетевые** > **разверните узел Сетевые профили** > **шифрование IPSec** > **Добавить**, настройте следующие поля и нажмите кнопку **ОК**.
...
Повторите предыдущие шаги, чтобы создать другой профиль шифрования IPsec, который будет использоваться в качестве дополнительного VPN-узла AVS. Один и тот же профиль шифрования IPSEC можно также использовать как для основного, так и для дополнительного туннелей IPsec (см. следующую процедуру).

### <a name="6-define-monitor-profiles-for-tunnel-monitoring"></a>6. Определение профилей монитора для мониторинга туннеля

Выберите **сетевые** > **разверните узел Сетевые профили** > **монитор** > **Добавить**, настройте следующие поля и нажмите кнопку **ОК**.
...
Выберите **сетевые** > **туннели IPSec** > **добавить**, настройте следующие поля и нажмите кнопку **ОК**.

Вкладка "Общие":

* Название Введите любое имя основного туннеля IPSEC для пиринга с основным одноранговым узлом AVS.
...
* Конечный IP-адрес. Введите любой IP-адрес, принадлежащий подсети "частный облако" AVS, которая разрешена через подключение "сеть — сеть". Убедитесь, что туннельные интерфейсы (такие как Tunnel. 20-10.64.5.2/32 и Tunnel. 30-10.64.6.2/32) в Palo Alto могут обращаться к IP-адресу частного облака AVS через VPN-подключение типа "сеть — сеть". См. следующую конфигурацию для идентификаторов прокси-сервера.
* Профиля. Выберите профиль монитора.

Вкладка "ИД прокси-сервера": щелкните **IPv4** > **Добавить** и настроить следующие параметры:
...
* Локальный. Укажите локальные локальные подсети, которым разрешено взаимодействовать с подсетями частного облака AVS через VPN-подключение типа "сеть — сеть".
* Удаленный. Укажите удаленные подсети в частном облаке AVS, которым разрешено взаимодействовать с локальными подсетями.
* См. Выберите **любой**.

Повторите предыдущие шаги, чтобы создать другой туннель IPsec, который будет использоваться для дополнительного VPN-узла AVS.
