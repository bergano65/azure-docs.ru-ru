...
Встроенный контроллер служб платформы в ВКСА связан с **доменом единого входа vCenter**.  Доменное имя — **клаудсимпле. local**.  Для доступа к vCenter создается **CloudOwner@cloudsimple.com** пользователя по умолчанию.  Вы можете добавить локальные [источники удостоверений](set-vcenter-identity.md)Azure Active Directory для vCenter.
