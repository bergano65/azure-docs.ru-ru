...
* Копирование данных из Azure в частное облако AVS.
...
* Создайте [подключение виртуальной сети Azure](cloudsimple-azure-network-connection.md) из частного облака AVS к виртуальной сети, в которой создана учетная запись хранения, выполнив действия, описанные в статье [подключение виртуальной сети Azure к AVS с помощью ExpressRoute](virtual-network-connection.md).
...
Данные, скопированные на устройство Data Box, будут доступны в учетной записи хранения Azure после того, как состояние заказа Data Box будет показано как завершенное. Теперь данные можно скопировать в решение Azure VMware. Данные в учетной записи хранения должны быть скопированы в хранилище данных vSAN частного облака AVS с помощью протокола NFS. 

Сначала скопируйте данные хранилища BLOB-объектов на управляемый диск в виртуальной машине Linux в Azure с помощью **AzCopy**. Сделайте управляемый диск доступным через NFS, подключите общий ресурс NFS в качестве хранилища данных в частном облаке AVS, а затем скопируйте данные. Этот метод позволяет ускорить копирование данных в частное облако AVS.

### <a name="copy-data-to-your-avs-private-cloud-using-a-linux-virtual-machine-and-managed-disks-and-then-export-as-nfs-share"></a>Копирование данных в частное облако AVS с помощью виртуальной машины Linux и управляемых дисков, а затем Экспорт в качестве общего ресурса NFS

1. Создание [виртуальной машины Linux](../virtual-machines/linux/quick-create-portal.md) в Azure в том же регионе, где создана учетная запись хранения, и подключение виртуальной сети Azure к частному облаку AVS.
...
8. Назначьте разрешения для IP-адресов клиента, чтобы получить доступ к общему ресурсу NFS, изменив файл `/etc/exports`.
    ...
    Введите следующие строки в файл для каждого ESXi узла в частном облаке AVS. Если вы создаете общие папки для нескольких папок, добавьте все папки.
...
### <a name="mount-the-linux-virtual-machine-nfs-share-as-a-datastore-on-an-avs-private-cloud-vcenter-cluster-and-then-copy-data"></a>Подключите общий ресурс NFS виртуальной машины Linux в качестве хранилища данных в кластере vCenter частного облака версии AVS, а затем скопируйте данные.

Общая папка NFS из виртуальной машины Linux должна быть подключена как хранилище данных в кластере vCenter частного облака AVS. После подключения данные можно скопировать из хранилища данных NFS в хранилище vSAN для частного облака AVS.

1. Войдите на свой частный облачный сервер AVS.
...
1. В пользовательском веб-интерфейсе службы "закрытое облако vCenter" перейдите в раздел " **хранилище**". Выберите хранилище данных NFS виртуальной машины Linux, а затем выберите **файлы**.
...
3. Выберите **зарегистрировать** виртуальную машину для регистрации виртуальной машины в частном облаке AVS.
...
После копирования всех данных в частное облако AVS можно удалить хранилище данных NFS из частного облака AVS:
...
3. Отключите хранилище данных виртуальной машины Linux от версии vCenter частного облака "AVS".
...
* Узнайте больше о различных вариантах [переноса рабочих нагрузок в частное облако AVS](migrate-workloads.md).
