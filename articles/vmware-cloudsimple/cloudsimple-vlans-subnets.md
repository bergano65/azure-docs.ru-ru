---
title: Виртуальные ЛС и подсети в решениях Azure VMware (AVS)
description: Сведения о виртуальных ЛС и подсетях в частном облаке AVS
author: sharaths-cs
ms.author: dikamath
ms.date: 08/15/2019
ms.topic: article
...
AVS предоставляет сеть для каждого региона, в котором развернута служба AVS. Сеть — это одно адресное пространство TCP уровня 3 с включенной маршрутизацией по умолчанию. Все частные облака и подсети AVS, созданные в этом регионе, могут взаимодействовать друг с другом без дополнительной настройки. Вы можете создавать группы распределенных портов в vCenter с помощью виртуальных ЛС.

![Топология сети AVS](media/cloudsimple-network-topology.png)

## <a name="vlans"></a>Виртуальные локальные сети

Виртуальная ЛС (сеть уровня 2) создается для каждого частного облака AVS. Трафик уровня 2 остается внутри границы частного облака AVS, что позволяет изолировать локальный трафик в частном облаке AVS. Виртуальную ЛС, созданную в частном облаке AVS, можно использовать для создания распределенных групп портов только в этом частном облаке AVS. Виртуальная ЛС, созданная в частном облаке AVS, автоматически настраивается на всех коммутаторах, подключенных к узлам частного облака AVS.
...
По умолчанию все подсети могут взаимодействовать друг с другом, уменьшая нагрузку на конфигурацию для маршрутизации между частными облаками AVS. Данные Восток-Запад на компьютерах в одном регионе остаются в одной сети уровня 3 и передаются через локальную сетевую инфраструктуру в пределах региона. Для обмена данными между частными облаками AVS в регионе не требуется исходящего трафика. Этот подход устраняет снижение производительности глобальной сети и исходящего трафика при развертывании различных рабочих нагрузок в разных частных облаках AVS.

## <a name="vspherevsan-subnets-cidr-range"></a>диапазон CIDR подсетей vSphere/vSAN

Частное облако AVS создается в виде изолированного стека VMware (ESXi hosts, vCenter, vSAN и НСКС), управляемого сервером vCenter. Компоненты управления развертываются в сети, выбранной для подсетей vSphere/vSAN CIDR. Диапазон CIDR сети делится на разные подсети во время развертывания.
...
Выбор размера диапазона CIDR для подсетей vSphere/vSAN влияет на размер частного облака AVS. В следующей таблице показано максимальное количество узлов, которые можно использовать в зависимости от размера подсетей vSphere/vSAN CIDR.
...
### <a name="management-subnets-created-on-an-avs-private-cloud"></a>Подсети управления, созданные в частном облаке AVS

При создании частного облака AVS создаются следующие подсети управления.
