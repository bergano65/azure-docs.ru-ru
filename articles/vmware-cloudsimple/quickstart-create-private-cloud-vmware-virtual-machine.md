---
title: Краткое руководство. Создание виртуальной машины VMware в частном облаке AVS
description: Описание процесса создания виртуальной машины VMware в частном облаке AVS
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/16/2019
ms.topic: article
...
ms.locfileid: "77020069"
---
# <a name="create-vmware-virtual-machines-on-your-avs-private-cloud"></a>Создание виртуальных машин VMware в частном облаке AVS

Чтобы создать виртуальные машины в частном облаке AVS, начните с доступа к порталу AVS из портал Azure.
...
4. На странице **Обзор** щелкните **Перейти на портал AVS** , чтобы открыть новую вкладку браузера для портала AVS. При появлении запроса войдите с помощью учетных данных для входа в Azure. 
...
Чтобы получить доступ к vCenter, начните с портала AVS. На домашней странице в разделе **Общие задачи**щелкните **запустить клиент vSphere**. Выберите частное облако AVS и щелкните **запустить клиент vSphere** в частном облаке AVS.
...
9. Выберите параметры жесткого диска и сети. Для нового дисковода CD/DVD выберите **ISO-файл хранилища данных**. Если вы хотите разрешить трафик с общедоступного IP-адреса на эту виртуальную машину, выберите сеть в качестве **ВМ-1**.
    ![новую виртуальную машину](media/vcvm08.png)

10. Откроется окно выбора. Выберите файл, который вы ранее перегрузили в папку ISO и Templates, и нажмите кнопку **ОК**.
    ![новую виртуальную машину](media/vcvm10.png)

11. Проверьте параметры и нажмите кнопку **ОК** , чтобы создать виртуальную машину.
    ![новую виртуальную машину](media/vcvm11.png)

Теперь виртуальная машина добавляется в ресурсы для загрузки рабочей нагрузки и готова к использованию. 
![новую виртуальную машину](media/vcvm12.png)

Теперь базовая настройка завершена. Вы можете начать использовать частное облако AVS так же, как и локальную инфраструктуру виртуальных машин.

Следующие разделы содержат дополнительные сведения о настройке DNS и DHCP для рабочих нагрузок частного облака AVS и изменении конфигурации сети по умолчанию.
...
Страницы сети на портале AVS позволяют указать конфигурацию для таблиц брандмауэра и общедоступных IP-адресов для виртуальных машин.

### <a name="allocate-public-ips"></a>Выделение общедоступных IP-адресов

1. Перейдите в раздел **сетевое > общедоступный IP-адрес** на портале AVS.
...
### <a name="default-avs-firewall-rules-for-public-ip"></a>Правила брандмауэра AVS по умолчанию для общедоступного IP-адреса

* VPN-трафик. разрешается весь трафик между (от или до) VPN и всеми сетями рабочей нагрузки и сетью управления.
* Внутренний трафик в частном облаке AVS: разрешен весь трафик между локальными рабочими нагрузками (от/до) и сетью управления (показанным выше).
* Интернет — трафик:
...
* [Настройка VPN-шлюзов в сети AVS](vpn-gateway.md)
