...
Чтобы использовать Azure AD в качестве источника удостоверений с vCenter, необходимо настроить доменные службы Azure AD и Azure AD. Следуйте указаниям, приведенным ниже:
...
2. Собирайте параметры конфигурации, необходимые для настройки источника удостоверений.
    ...
    | **URL-адрес основного сервера** | Сервер LDAP основного контроллера домена для домена.<br><br>Используйте следующий формат: `ldaps://hostname:port`. Обычно для подключений LDAPs используется порт 636. <br><br>Сертификат, устанавливающий отношение доверия для конечной точки LDAPs сервера Active Directory, необходим при использовании `ldaps://` в первичном или вторичном URL-адресе LDAP. |
