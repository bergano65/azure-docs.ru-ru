---
title: Решения Azure VMware (AVS). Использование Azure AD в качестве источника удостоверений в частном облаке AVS
description: В этой статье описывается добавление Azure AD в качестве поставщика удостоверений в частном облаке AVS для проверки подлинности пользователей, обращающихся к AVS из Azure.
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/15/2019
ms.topic: article
...
ms.locfileid: "77015929"
---
# <a name="use-azure-ad-as-an-identity-provider-for-vcenter-on-avs-private-cloud"></a>Использование Azure AD в качестве поставщика удостоверений для vCenter в частном облаке AVS

Вы можете настроить в частном облаке Azure Active Directory для Azure для администраторов VMware доступ к vCenter. После настройки источника удостоверений единого входа пользователь **клаудовнер** может добавить пользователей из источника удостоверений в vCenter.  
...
2. [Как настроить источник удостоверений в частном облаке AVS](#set-up-an-identity-source-on-your-avs-private-cloud-vcenter)
...
## <a name="set-up-an-identity-source-on-your-avs-private-cloud-vcenter"></a>Настройка источника удостоверений в частном облаке AVS

1. [Эскалировать привилегии](escalate-private-cloud-privileges.md) для вашего частного облака AVS.
2. Собирайте параметры конфигурации, необходимые для настройки источника удостоверений.
    ...
    | **Базовое DN для групп** | Базовое различающееся имя для групп. Для Azure AD используйте: `OU=AADDC Users,DC=<domain>,DC=<domain suffix>` пример: `OU=AADDC Users,DC=cloudsimplecustomer,DC=com`|
    ...
    | **Выбор сертификата** | Если вы хотите использовать LDAPs с сервером Active Directory LDAP или источником удостоверений сервера OpenLDAP, после ввода `ldaps://` в текстовом поле URL-адрес появится кнопка **выбрать сертификат** . Дополнительный URL-адрес не требуется. |
...
3. Войдите в службу "частное облако" AVS после повышения привилегий.
