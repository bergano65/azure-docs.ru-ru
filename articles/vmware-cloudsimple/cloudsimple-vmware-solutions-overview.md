...
**Решение VMware в Azure с помощью клаудсимпле** — это полностью управляемая служба, которая позволяет запускать платформу VMware в Azure. Это решение включает vSphere, vCenter, vSAN, НСКС-T и соответствующие инструменты.
Среда VMware работает изначально в инфраструктуре нестандартного состояния Azure в облачных расположениях Azure. Служба включает все функции, необходимые для эффективного и безопасного использования платформ VMware.
...
* Граничные типы сетевых служб, включая VPN, общедоступный IP-адрес и шлюзы Интернета. Эти службы пограничных служб работают в Azure и имеют соответствующие средства защиты и от атак DDoS защиту Azure.
* Резервирование емкости для снижения затрат.
* Высокоскоростное подключение с низкой задержкой к Azure и локальным.
* Архитектуры решений для клиентов, которые используют службы Azure в интегрированной манере, используют преимущества этой уникальной архитектуры "облако VMware в общедоступном облаке". К этим службам Azure относятся Azure AD, хранилище, шлюзы приложений и другие.
* Инфраструктура полностью выделяется для вас и физически изолирована от инфраструктуры других клиентов.
...
* **Непрерывность эксплуатации:** Клаудсимпле предлагает собственный доступ к платформам VMware. Архитектура Клаудсимпле совместима с существующими:
  ...
  * Процессы
* **Без переобучения:** Совместимость платформы VMware позволяет использовать имеющиеся навыки и знания.
