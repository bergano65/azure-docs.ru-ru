---
title: Решения Azure VMware (AVS) — частные облака AVS
description: Сведения о частных облаках и концепциях AVS.
author: sharaths-cs
ms.author: dikamath
ms.date: 08/20/2019
ms.topic: article
...
---
# <a name="avs-private-cloud-overview"></a>Общие сведения о частном облаке AVS

AVS преобразует и распространяет рабочие нагрузки VMware в общедоступные облака за считаные минуты. С помощью службы AVS можно развертывать VMware изначально в инфраструктуре исходного состояния системы Azure. Развертывание находится в расположениях Azure и полностью интегрируется с остальными частями облака Azure.

Решение AVS обеспечивает полную оперативную непрерывность работы VMware. Это решение предоставляет преимущества общедоступного облака:

* Эластичность
* Инновации
* Эффективность

В случае с AVS вы выдаете преимущества от модели использования облака, которая снижает общую стоимость владения. Он также предлагает оптимизацию по требованию, оплату по мере роста и оптимизировать ресурсы.

AVS полностью совместима с:
...
AVS управляет инфраструктурой и всеми необходимыми службами сети и управления. Служба AVS позволяет команде сосредоточиться на следующих:
...
## <a name="avs-private-cloud-environment-overview"></a>Общие сведения о среде частного облака AVS

Частное облако AVS — это изолированный стек VMware, который поддерживает:
...
Частные облака AVS управляются с помощью портала AVS. Они имеют собственный сервер vCenter в собственном домене управления.
...
Вы можете развернуть выделенные узлы в расположениях Azure. Затем вы можете управлять ими с помощью Azure и AVS. Частное облако AVS состоит из одного или нескольких кластеров vSphere, а каждый кластер содержит от 3 до 16 узлов.

Вы можете создать частное облако AVS с помощью приобретенных узлов с оплатой по мере использования или зарезервированных выделенных узлов.

Частное облако AVS можно подключить к локальной среде и сети Azure, используя следующие подключения:

* Безопасность
* Частная VPN
* Azure ExpressRoute

Среда частного облака AVS предназначена для устранения одиночных точек отказа:
...
## <a name="scenarios-for-deploying-an-avs-private-cloud"></a>Сценарии развертывания частного облака AVS

Ниже приведены некоторые примеры вариантов использования развертывания частного облака AVS.
...
* Репликация всех политик и сетевых подключений в облаке с помощью портала AVS и знакомых средств VMware. Репликация сокращает усилия и риск создания и управления реализациями аварийного восстановления и VDI.

### <a name="high-performance-applications-and-databases"></a>Высокопроизводительные приложения и базы данных

* Выполняйте наиболее ресурсоемкие рабочие нагрузки с архитектурой гиперконвергентном, предоставляемой AVS.
...
В следующей таблице перечислены ограничения для ресурсов частного облака AVS.
