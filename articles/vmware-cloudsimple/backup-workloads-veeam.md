---
title: Решения Azure VMware (AVS). Резервное копирование виртуальных машин в частном облаке AVS с помощью Veeam
description: В этой статье описывается, как создать резервную копию виртуальных машин, работающих в частном облаке AVS на основе Azure, с помощью Veeam B & R 9,5.
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/16/2019
ms.topic: article
...
---
# <a name="back-up-workload-vms-on-avs-private-cloud-using-veeam-br"></a>Резервное копирование виртуальных машин рабочей нагрузки в частном облаке AVS с помощью Veeam B & R

В этом руководство описано, как создать резервную копию виртуальных машин, работающих в частном облаке AVS на основе Azure, с помощью Veeam B & R 9,5.
...
Вы можете использовать Azure для предоставления резервного хранилища и целевого объекта хранилища для долгосрочного резервного копирования и архивации. Весь сетевой трафик резервного копирования между виртуальными машинами в частном облаке AVS и репозиторием резервных копий в Azure перемещается по каналу высокой пропускной способности и низкой задержке. Трафик репликации между регионами перемещается по внутренней сети объединительной платы Azure, что снижает затраты на пропускную способность для пользователей.

**Базовое развертывание**

Для сред с менее чем 30 ТБ для резервного копирования AVS рекомендует следующую конфигурацию:

* Veeam Backup Server и прокси-сервер установлены на одной виртуальной машине в частном облаке AVS.
...
Для сред с более чем 30 ТБ резервного копирования AVS рекомендует следующую конфигурацию:

* Один прокси-сервер на узел в кластере vSAN, как рекомендуется для Veeam.
* Основной репозиторий резервных копий на основе Windows в частном облаке AVS для кэширования данных за пять дней для быстрого восстановления.
...
## <a name="requirements-for-veeam-solution-on-avs"></a>Требования к решению Veeam в AVS

Для решения Veeam необходимо выполнить следующие действия.

* Предоставьте собственные лицензии Veeam.
* Развертывание Veeam и управление ими для резервного копирования рабочих нагрузок, работающих в частном облаке AVS.
...
## <a name="install-and-configure-veeam-backups-in-your-avs-private-cloud"></a>Установка и Настройка резервных копий Veeam в частном облаке "AVS"

В следующих разделах описывается установка и настройка решения резервного копирования Veeam для частного облака AVS.

Процесс развертывания состоит из следующих этапов.

1. [Пользовательский интерфейс vCenter: Настройка служб инфраструктуры в частном облаке AVS](#vcenter-ui-set-up-infrastructure-services-in-your-avs-private-cloud)
2. [Портал AVS: Настройка сети для частного облака AVS для Veeam](#avs-private-cloud-set-up-avs-private-cloud-networking-for-veeam)
3. [Портал AVS: эскалация привилегий](#avs-private-cloud-escalate-privileges-for-cloudowner)
4. [Портал Azure: подключение виртуальной сети к частному облаку AVS](#azure-portal-connect-your-virtual-network-to-the-avs-private-cloud)
5. [Портал Azure: Создание репозитория резервных копий в Azure](#azure-portal-connect-your-virtual-network-to-the-avs-private-cloud)
...
7. [Пользовательский интерфейс vCenter для частного облака AVS: install Veeam B & R](#vcenter-console-of-avs-private-cloud-install-veeam-br)
...
9. [Портал AVS: Настройка доступа к Veeam и отмены эскалации привилегий](#avs-portal-set-up-veeam-access-and-de-escalate-privileges)
...
* [Частное облако AVS](create-private-cloud.md) , созданное с помощью портала AVS.  
...
* Veeam 9,5 U3 устанавливаемый носитель (ISO), отправленный в хранилище данных vSAN частного облака AVS

### <a name="vcenter-ui-set-up-infrastructure-services-in-your-avs-private-cloud"></a>Пользовательский интерфейс vCenter: Настройка служб инфраструктуры в частном облаке AVS
...
* Можно добавить внешний поставщик удостоверений, как описано в разделе [Настройка источников удостоверений vCenter для использования Active Directory](set-vcenter-identity.md) если выполняется одно из следующих условий:
  * Вы хотите опознать пользователей из локальной Active Directory (AD) в частном облаке AVS.
  * Вы хотите настроить AD в частном облаке AVS для всех пользователей.
...
### <a name="avs-private-cloud-set-up-avs-private-cloud-networking-for-veeam"></a>Частное облако AVS: Настройка сети для частного облака AVS для Veeam

Получите доступ к порталу AVS, чтобы настроить сеть частного облака AVS для решения Veeam.
...
По умолчанию AVS предоставляет ссылку 1Gbps ExpressRoute. Для более крупных размеров среды может потребоваться ссылка на более высокую пропускную способность. Обратитесь в службу поддержки Azure для получения дополнительных сведений о связях с большей пропускной способностью.

Чтобы продолжить установку, вам потребуется ключ авторизации и URI одноранговой цепи, а также доступ к своей подписке Azure. Эти сведения доступны на странице Подключение к виртуальной сети на портале AVS. Инструкции см. [в статье Получение сведений о пиринга для виртуальной сети Azure с подключением к AVS](virtual-network-connection.md). Если у вас возникли проблемы с получением сведений, [обратитесь в службу поддержки](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest).

### <a name="avs-private-cloud-escalate-privileges-for-cloudowner"></a>Частное облако AVS: эскалация привилегий для **клаудовнер**

Пользователь по умолчанию "клаудовнер" не имеет достаточных привилегий в частном облаке "AVS" для установки VEEAM, поэтому права доступа пользователя vCenter должны быть расширены. Дополнительные сведения см. в разделе [эскалация привилегий](escalate-private-cloud-privileges.md).
...
Подключите виртуальную сеть к частному облаку AVS, следуя инструкциям в статье [Подключение к виртуальной сети Azure с помощью ExpressRoute](azure-expressroute-connection.md).
...
7. Предоставьте/backup1 в качестве точки подключения NFS к резервному серверу Veeam, работающему в частном облаке AVS. Инструкции см. в статье о цифровом океане [как настроить подключение NFS к CentOS 6](https://www.digitalocean.com/community/tutorials/how-to-set-up-an-nfs-mount-on-centos-6). Используйте это имя общего ресурса NFS при настройке репозитория резервного копирования на сервере Veeam Backup Server.
...
### <a name="vcenter-console-of-avs-private-cloud-install-veeam-br"></a>консоль vCenter из частного облака AVS: install Veeam B & R

Получите доступ к vCenter из частного облака AVS, чтобы создать учетную запись службы Veeam, установите Veeam B & R 9,5 и настройте Veeam с помощью учетной записи службы.
...
1. Добавьте VMware vSphere в качестве управляемой серверной среды. При появлении запроса введите учетные данные учетной записи службы Veeam, созданной в начале [консоли VCenter частного облака AVS: install Veeam B & R](#vcenter-console-of-avs-private-cloud-install-veeam-br).
...
### <a name="avs-portal-set-up-veeam-access-and-de-escalate-privileges"></a>Портал AVS: Настройка доступа к Veeam и отмены эскалации привилегий
...
### <a name="avs-references"></a>Ссылки AVS
...
* [Разрешения для частного облака AVS](learn-private-cloud-permissions.md)
