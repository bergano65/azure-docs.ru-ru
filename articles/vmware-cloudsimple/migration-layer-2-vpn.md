---
title: Решения Azure VMware (AVS) — перенос локальной сети уровня 2 в частное облако AVS
description: Описание настройки VPN уровня 2 между НСКС-T в частном облаке AVS и локальным автономным клиентом НСКС.
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/19/2019
ms.topic: article
...
В этом руководство вы узнаете, как использовать VPN уровня 2 (L2VPN) для переноса сети уровня 2 из локальной среды в частное облако AVS. Это решение позволяет выполнять миграцию рабочих нагрузок, работающих в локальной среде VMware, в частное облако AVS в Azure в рамках одного адресного пространства подсети без необходимости повторного IP-адреса рабочих нагрузок.
...
> [!NOTE]
> В этом руководстве рассматривается сценарий, в котором локальные и закрытые Облачные центры данных AVS подключены через VPN типа "сеть — сеть".
...
В этом сценарии развертывания частное облако AVS подключается к локальной среде через VPN-туннель типа "сеть — сеть", который позволяет локальным организациям и подсетям vMotion обмениваться данными с подсетями, связанными с управлением частным облаком AVS и vMotion. Это расположение необходимо для перекрестного vCenter vMotion (КСВК-vMotion). Маршрутизатор НСКС-T Tier0 развертывается как сервер L2VPN в частном облаке AVS.

Автономный НСКСный периметр развертывается в локальной среде как клиент L2VPN и затем связывается с сервером L2VPN. Конечная точка туннеля GRE создается на каждой стороне и настроена для переноса локальной сети уровня 2 в частное облако AVS. Эта конфигурация показана на следующем рисунке.
...
* Найдите сеть уровня рабочей нагрузки 2 для переноса в частное облако AVS.
* Найдите сеть уровня 2 в локальной среде для развертывания клиентского устройства L2VPN.
* [Частное облако AVS уже создано](create-private-cloud.md).
* Версия автономного устройства НСКС-T, совместимая с версией НСКС-T Manager (НСКС-T 2.3.0), используемой в среде частного облака AVS.
...
* Пересылка DNS задается на локальных DNS-серверах, чтобы домен az.cloudsimple.io указывал на DNS-серверы частного облака AVS.
...
------------ | ------------- | ------------ | ------------- | ------------- 
| All | Программа | All | Программа | Да |
...
| vSphere 6.7 UI или 6.5 P03, НСКС-V или версии ниже НСКС-T 2.2, 6.5 P03 или выше | All | All | N — VDS | [Не поддерживается для Ввваре](https://kb.vmware.com/s/article/56991) |

Начиная с версии VMware НСКС-T 2,3:

* Логический коммутатор на стороне частного облака AVS, который растягивается в локальную среду через L2VPN, нельзя маршрутизировать одновременно. Растянутый логический коммутатор не может быть подключен к логическому маршрутизатору.
...
### <a name="avs-private-cloud-ip-schema-for-nsx-t-tier0-router-l2-vpn-serve"></a>IP-схема частного облака AVS для маршрутизатора НСКС-T Tier0 (для виртуальных частных сетей L2)
...
### <a name="avs-private-cloud-network-to-be-mapped-to-the-stretched-network"></a>Сеть "частный облако" AVS будет сопоставлена с растянутой сетью
...
1. Войдите в НСКС-T Manager https://*НСКС-t-Manager-IP-Address* и выберите **Сетевые** > **маршрутизаторы** > **поставщик —**  > **Обзор**. В **режиме высокого уровня доступности выберите режим**" **Активный — ожидание**". Это действие открывает всплывающее окно, в котором отображается пограничная виртуальная машина, на которой в данный момент активен маршрутизатор Tier0.

    ![Выбор режима "активный — резервный"](media/l2vpn-fetch01.png)

2. Выберите **Fabric** > **узлы** > **края**. Запишите IP-адрес активной виртуальной машины (ребра VM1), указанной на предыдущем шаге.

    ![IP-адрес управления заметками](media/l2vpn-fetch02.png)

3. Откройте сеанс SSH с IP-адресом управления пограничной виртуальной машины. Выполните команду ```get logical-router``` с именем пользователя **Admin** и паролем **AVS 123!** .
...
2. Выберите **сетевые** > **Переключение** > **коммутаторов** >  **< коммутатор \логикал\>** **Обзор**. > 
...
1. [Создайте общедоступный IP-адрес](public-ips.md) для интерфейса замыкания на себя НСКС-T Tier0 на портале AVS.
...
1. Создайте маршрут со значением NULL для сети интерфейса замыкания на себя. Войдите в Диспетчер НСКС-T и выберите **сетевые** > **Маршрутизация** > **маршрутизаторы** > **поставщик-LR** > **Маршрутизация** > **статические маршруты**. Нажмите кнопку **Добавить**. В поле **сеть**введите IP-адрес интерфейса замыкания на себя. Для **следующих прыжков**нажмите кнопку **Добавить**, укажите значение NULL для следующего прыжка и выберите значение по умолчанию 1 для параметра Расстояние администратора.

    ![Добавить статический маршрут](media/l2vpn-routing-security01.png)

2. Создайте список префиксов IP-адресов. Войдите в Диспетчер НСКС-T и выберите **сетевые** > **Маршрутизация** > **маршрутизаторы** > **поставщик-LR** > **Маршрутизация** > **списки префиксов IP-адресов**. Нажмите кнопку **Добавить**. Введите имя для указания списка. Для добавления **префиксов**нажмите кнопку **Добавить** дважды. В первой строке введите "0.0.0.0/0" для команды " **сеть** " и "запретить" для **действия**. Во второй строке выберите **любой** для **сети** и **разрешите** **действие**.
...
4. Войдите в Диспетчер НСКС-T и выберите **сетевые** > **Маршрутизация** > **маршрутизаторы** > **поставщик-LR** > **Маршрутизация** > **соседей** **BGP** > . Выберите первый сосед. Щелкните **изменить** **семейства адресов** > . В семействе IPv4 измените столбец **исходящий фильтр** и выберите созданный список IP-префиксов. Выберите команду **Сохранить**. Повторите этот шаг для второго соседа.

    ![присоединить список префиксов IP-адресов](media/l2vpn-routing-security03.png) ![присоединить IP-префикс список 2](media/l2vpn-routing-security04.png)

5. Повторно распространите статический маршрут NULL в BGP. Чтобы объявить маршрут интерфейса замыкания на ундерлай, необходимо повторно распространить статический маршрут NULL в BGP. Войдите в Диспетчер НСКС-T и выберите **сетевые** > **Маршрутизация** > **маршрутизаторы** > **поставщик-LR** > **Маршрутизация** > **соседи** > **перераспределения маршрутов** . Выберите **поставщик-LR-Route_Redistribution** и нажмите кнопку **изменить**. Установите флажок **статический** и нажмите кнопку **сохранить**.
...
Используйте следующий шаблон, чтобы заполнить все сведения о настройке VPN на основе маршрутов на маршрутизаторе НСКС-T Tier0. Идентификаторы UUID в каждом вызове POST необходимы во всех последующих вызовах POST. IP-адреса для интерфейсов замыкания на себя и туннеля для L2VPN должны быть уникальными и не перекрываться с сетями в локальной среде или частном облаке AVS.

IP-адреса, выбранные для интерфейса замыкания на себя и туннеля, используемые для L2VPN, должны быть уникальными и не перекрываться с сетями или частными облаками в локальной сети или AVS. Сеть интерфейса замыкания на себя всегда должна иметь значение/32.
...
Для всех приведенных ниже вызовов API Замените IP-адрес своим IP адресом НСКС-T Manager. Все эти вызовы API можно выполнить из пост-клиента или с помощью команд `curl`.
...
Перед развертыванием убедитесь, что локальные правила брандмауэра разрешают входящий и исходящий трафик UDP 500/4500 от/к общедоступному IP-адресу AVS, зарезервированному ранее для интерфейса замыкания маршрутизатора НСКС-T T0. 
...
2. Перейдите в папку со всеми извлеченными файлами. Выберите все VMDK (НСКС-L2T-Client-Large. MF и NSX-l2t-client-large. ovf для большого размера устройства или НСКС-L2T-Client-Ксларже. MF и NSX-l2t-client-Xlarge. ovf для очень большого размера устройства). Щелкните **Далее**.
    ...
    ![выберите шаблон](media/l2vpn-deploy-client02.png) ![выберите шаблон](media/l2vpn-deploy-client03.png)
...
6. Введите следующие сведения на экране **Настройка шаблона** и нажмите кнопку **Далее**.
    ...
    * **Одноранговый адрес**. Введите IP-адрес, зарезервированный на портале Azure AVS для интерфейса замыкания на себя НСКС-T Tier0.
    ...
    * **Администратор CLI, включение и ввод пароля пользователя root**. Задайте пароль для учетной записи администратора/Enable/root.
      ...
      ![настроить шаблон](media/l2vpn-deploy-client08.png)
      ![настроить шаблон — дополнительно](media/l2vpn-deploy-client09.png)
