...
1. Войдите в НСКС-T Manager https://*НСКС-t-Manager-IP-Address* и выберите **сеть** > **маршрутизаторы** >  **—** **Общие сведения об** > . В **режиме высокого уровня доступности выберите режим**" **Активный — ожидание**". Это действие открывает всплывающее окно, в котором отображается пограничная виртуальная машина, на которой в данный момент активен маршрутизатор Tier0.
...
3. Откройте сеанс SSH с IP-адресом управления пограничной виртуальной машины. Выполните команду ```get logical-router``` с именем пользователя **Admin** и паролем **клаудсимпле 123!** .
...
1. Создайте маршрут со значением NULL для сети интерфейса замыкания на себя. Войдите в НСКС Manager и выберите **сеть** > **Маршрутизация** > **маршрутизаторы** > **provider-LR** > **Маршрутизация** > **статических маршрутов**. Щелкните **Добавить**. В поле **сеть**введите IP-адрес интерфейса замыкания на себя. Для **следующих прыжков**нажмите кнопку **Добавить**, укажите значение NULL для следующего прыжка и выберите значение по умолчанию 1 для параметра Расстояние администратора.

    ![Добавить статический маршрут](media/l2vpn-routing-security01.png)

2. Создайте список префиксов IP-адресов. Войдите в НСКС Manager и выберите **сеть** > **Маршрутизация** > **маршрутизаторы** > **provider-LR** > **Маршрутизация** > .**списки префиксов IP-адресов**. Щелкните **Добавить**. Введите имя для указания списка. Для добавления **префиксов**нажмите кнопку **Добавить** дважды. В первой строке введите "0.0.0.0/0" для команды " **сеть** " и "запретить" для **действия**. Во второй строке выберите **любой** для **сети** и **разрешите** **действие**.
...
4. Войдите в НСКС Manager и выберите **сеть** > **Маршрутизация** > **маршрутизаторы** > **provider-LR** > **Маршрутизация** > **соседей** **BGP**1. Выберите первый сосед. Щелкните **изменить** **семейства адресов** > . В семействе IPv4 измените столбец **исходящий фильтр** и выберите созданный список IP-префиксов. В нижней части страницы нажмите кнопку **Save**. Повторите этот шаг для второго соседа.
...
Для всех приведенных ниже вызовов API Замените IP-адрес своим IP адресом НСКС-T Manager. Вы можете выполнять все эти вызовы API из клиента POST или с помощью команд `curl`.
