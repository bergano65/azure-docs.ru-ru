...
|----------|-----------|----------|--------------|
|<a name="count" />count|Количество результатов для возврата, начиная с индекса определяется `offset` параметра.|Строка|Нет |   
...
|<a name="mkt" />mkt|Рынок, по которому возвращаются результаты. <br /><br />Список возможных значений рынка приведен в разделе "Коды рынка".<br /><br /> **Примечание**. API Local Business Search в настоящее время поддерживает только рынок и язык en-us.<br /><br />|Строка|Yes|
