...
|<a name="mkt" />mkt|Рынок, по которому возвращаются результаты. <br /><br />Список возможных значений рынка приведен в разделе "Коды рынка".<br /><br /> **Примечание**. API Local Business Search в настоящее время поддерживает только рынок и язык en-us.<br /><br />|String|Да|
|<a name="offset"/>offset|Индекс для запуска результатов, заданных параметром `count`.|Integer|Нет|  
