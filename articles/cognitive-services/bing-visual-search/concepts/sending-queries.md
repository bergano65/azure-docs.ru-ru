...
Если вы отправляете Визуальный поиск маркер или URL-адрес образа, в следующем фрагменте кода показан объект JSON, который необходимо включить в тело записи:
...
Объект `imageInfo` должен содержать поле `url` или `imageInsightsToken`, но не оба. В поле `url` укажите URL-адрес изображения, доступного в Интернете. Максимальный поддерживаемый размер изображения составляет 1 МБ.
...
Поле `cropArea` является необязательным. Область кадрирования задает верхний левый угол и правый нижний угол интересующей области. Укажите значения в диапазоне от 0,0 до 1,0. Они представляют собой процент от общей ширины или высоты. Например, приведенный выше пример помечает правую половину изображения как интересующую область. Если вы хотите ограничить запрос аналитики интересующей областью, укажите область обрезки.

Объект `filters` содержит фильтр сайта (см. поле `site`), который можно использовать, чтобы ограничить результаты с подобными изображениями и аналогичными товарами определенным доменом. Например, если изображение имеет контактную книгу, можно задать для `site` значение `www.microsoft.com`.
...
При необходимости можно задать атрибут `enableEntityData` в заголовке, чтобы `true` подробные сведения о главной сущности в загружаемом изображении, включая ссылки на сведения о веб-и соотношении. По умолчанию это поле `false`.
...
При отправке локального образа в следующем фрагменте отображаются данные формы, которые необходимо включить в текст записи. Данные формы должны включать заголовок `Content-Disposition`. Его параметру `name` необходимо присвоить значение "image", а для параметра `filename` можно задать любую строку. Для заголовка `Content-Type` может быть задан любой часто используемый тип MIME изображения. Содержимое формы представляет собой двоичные данные изображения. Максимально допустимый размер отправляемого изображения — 1 МБ. Наибольшее из значений ширины или высоты не должно превышать 1500 пикселей.
...
В следующем фрагменте кода показано, как указать область, представляющую интерес отправленного изображения:
...
В следующем фрагменте показан полный запрос Image Insights, который передает маркер изображения и интересующий регион. Вы получаете токен Insights из предыдущего вызова/имажес/СЕАРЧ:
...
Если для изображения доступны аналитические сведения, ответ содержит один или несколько полей `tags` с такими сведениями. Поле `image` содержит токен Insights для входного изображения:
...
а поле `tags` — отображаемое имя и список действий (аналитики). Один из тегов содержит поле `displayName`, для которого задана пустая строка. Этот тег содержит аналитические сведения по умолчанию, такие как веб-страницы, где содержится изображение, визуально подобные изображения и источники для покупок товаров, найденных в изображении. Так как весь образ представляет интерес, тег Insights по умолчанию не включает ограничивающие рамки для интересующих областей.
...
Список аналитических сведений по умолчанию см. в разделе [Default Insights тег](../default-insights-tag.md).

Остальные теги содержат другие сведения, которые могут представлять интерес для пользователя. Например, если изображение содержит текст, один тег может включать атрибут TextResults, содержащий распознанный текст. Или, если Bing распознает сущность (то есть хорошо известный или популярный пользователь, место или вещь) на изображении, один из тегов может опознать сущность. API для визуального поиска также возвращает широкий набор терминов (тегов), производных от входного изображения. Эти теги позволяют пользователям просматривать основные понятия, обнаруженные в образе. Например, если это фото знаменитого спортсмена, одним из тегов может быть слово "Спорт", содержащее ссылки на связанные изображения.
...
Если изображение содержит текст, распознаваемый службой, один из тегов будет содержать аналитические сведения TextResults (действие). `displayName` Insights содержит распознанный текст:
...
Если изображение содержит известную сущность, например хорошо известное или популярное в культуре лицо, место или вещь, один из тегов может включать в себя объект Entity Insights. Поля `mainEntity` и `data` доступны только в том случае, если атрибут `enableEntityData` в заголовке `Content-Type` имеет значение `true`.
...
- [Учебник. Создание Визуальный поиск одностраничного веб-приложения](../tutorial-bing-visual-search-single-page-app.md)
