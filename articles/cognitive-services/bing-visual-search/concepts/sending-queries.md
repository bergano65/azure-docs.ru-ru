...
Объект `filters` содержит фильтр сайта (см. поле `site`), который можно использовать, чтобы ограничить результаты с подобными изображениями и аналогичными товарами определенным доменом. Например, если изображение имеет контактную книгу, можно задать `site` для значение `www.microsoft.com` .
...
Каждый запрос должен включать `Content-Type` заголовок. Заголовок должен иметь значение: `multipart/form-data; boundary=\<boundary string\>` , где \<boundary string\> — уникальная непрозрачная строка, определяющая границу данных формы. Например, `boundary=boundary_1234-abcd`.
...
При необходимости можно задать `enableEntityData` атрибут в заголовке для получения `true` подробных сведений о основной сущности в загружаемом изображении, включая ссылки на сведения о веб-и соотношении. `false`По умолчанию это поле имеет значение.
...
Если изображение содержит известную сущность, например хорошо известное или популярное в культуре лицо, место или вещь, один из тегов может включать в себя объект Entity Insights. `mainEntity`Поля и `data` доступны только в том случае, если `enableEntityData` атрибут в `Content-Type` заголовке имеет значение `true` .
