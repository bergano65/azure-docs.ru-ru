...
Объект `imageInfo` должен содержать поле `url` или `imageInsightsToken`, но не оба. `url` В поле укажите URL-адрес изображения, доступного в Интернете. Максимальный поддерживаемый размер изображения составляет 1 МБ.
...
Ниже приведены заголовки, которые должны быть указаны в запросе. `User-Agent` `X-MSEdge-ClientID` `X-MSEdge-ClientIP` `X-Search-Location`Заголовки и являютсяединственнымиобязательнымизаголовками,нотакжеследуетвключить,,и.`Ocp-Apim-Subscription-Key` `Content-Type`
