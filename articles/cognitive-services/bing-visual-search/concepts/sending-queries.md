...
| <a name="mkt" />mkt   | Рынок, для которого будут отображаться результаты. <br /><br /> **Примечание.** Необходимо всегда указывать на рынке, если он известен. Таким образом Bing может направить запрос и вернуть соответствующий и оптимальный ответ.<br /><br /> Этот параметр и параметр запроса [cc](#cc) являются взаимоисключающими. Не указывайте оба. | string | Да      |
...
| Заголовок | Description (Описание) |
| --- | --- |
...
Каждый запрос должен включать заголовок `Content-Type`. Заголовок должен иметь значение: `multipart/form-data; boundary=\<boundary string\>`, где \<строка границы\> является уникальной, непрозрачной строкой, определяющей границу данных формы. Пример: `boundary=boundary_1234-abcd`.
...
Если изображение содержит известную сущность, например хорошо известное или популярное в культуре лицо, место или вещь, один из тегов может включать в себя объект Entity Insights.
