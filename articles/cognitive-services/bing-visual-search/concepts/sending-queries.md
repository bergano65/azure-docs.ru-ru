...
Объект `imageInfo` должен содержать поле `url` или `imageInsightsToken`, но не оба. В `url` поле укажите URL-адрес изображения, доступного в Интернете. Максимальный поддерживаемый размер изображения составляет 1 МБ.
...
Объект `filters` содержит фильтр сайта (см. поле `site`), который можно использовать, чтобы ограничить результаты с подобными изображениями и аналогичными товарами определенным доменом. Например, если изображение имеет контактную книгу, можно задать для `site` `www.microsoft.com`значение.
...
Ниже приведены параметры запроса, которые должны быть указаны в запросе. Как минимум, необходимо включить параметр `mkt` запроса:

| Имя | Значение | Тип | Обязательный |
| --- | --- | --- | --- |
...
Каждый запрос должен включать `Content-Type` заголовок. Заголовок должен иметь значение: `multipart/form-data; boundary=\<boundary string\>`, где \<строка\> границы является уникальной, непрозрачной строкой, определяющей границу данных формы. Например, `boundary=boundary_1234-abcd`.
...
При необходимости можно задать `enableEntityData` атрибут в заголовке `true` для получения подробных сведений о основной сущности в загружаемом изображении, включая ссылки на сведения о веб-и соотношении. По умолчанию `false` это поле имеет значение.
...
Если для изображения доступны аналитические сведения, ответ содержит один или несколько полей `tags` с такими сведениями. `image` Поле содержит токен Insights для входного изображения:
...
Если изображение содержит текст, распознаваемый службой, один из тегов будет содержать аналитические сведения TextResults (действие). Insights `displayName` содержит распознанный текст:
...
Если изображение содержит известную сущность, например хорошо известное или популярное в культуре лицо, место или вещь, один из тегов может включать в себя объект Entity Insights. Поля `mainEntity` и `data` доступны только в том случае, `enableEntityData` если атрибут в `Content-Type` заголовке имеет `true`значение.
...
- [Руководство по Создание одностраничного веб-приложения для визуального поиска](../tutorial-bing-visual-search-single-page-app.md)
