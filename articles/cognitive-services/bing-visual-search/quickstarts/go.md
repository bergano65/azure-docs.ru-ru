---
title: Краткое руководство. Получение аналитических сведений об изображениях с помощью REST API и Go — Визуальный поиск Bing
titleSuffix: Azure Cognitive Services
description: Узнайте, как отправить изображение с помощью API Визуального поиска Bing и Go, а затем получить аналитические сведения об этом изображении.
...
Из этого краткого руководства вы узнаете, как сделать первый вызов API Визуального поиска Bing, используя язык программирования Go. Запрос POST передает изображение в конечную точку API. Результаты включают в себя URL-адреса и описательные сведения об изображениях, аналогичных переданному изображению.

## <a name="prerequisites"></a>Предварительные требования

* Установите [двоичные файлы Go](https://golang.org/dl/).
* Для отображения результатов установите принтер глубокой печати go-spew. Чтобы установить go-spew, используйте команду `$ go get -u https://github.com/davecgh/go-spew`.

[!INCLUDE [cognitive-services-bing-visual-search-signup-requirements](../../../../includes/cognitive-services-bing-visual-search-signup-requirements.md)]

## <a name="project-and-libraries"></a>Проект и библиотеки

Создайте проект Go в интегрированной среде разработки или текстовом редакторе. Затем импортируйте `net/http` для запросов, `ioutil` для чтения ответа и `encoding/json` для обработки текста JSON-результатов. Используйте библиотеку `go-spew` для анализа результатов JSON.
...
## <a name="struct-to-format-results"></a>Структура для форматирования результатов
...
1. Убедитесь, что конечная точка указана верно, и замените значение `token` действительным ключом подписки из своей учетной записи Azure. 
...
4. Для `endpoint` можно использовать глобальную конечную точку в следующем коде или конечную точку [личного поддомена](../../../cognitive-services/cognitive-services-custom-subdomains.md), отображаемую на портале Azure для вашего ресурса.
...
## <a name="boundaries-of-post-body"></a>Границы основного текста POST

Для выполнения запроса POST к конечной точке Визуального поиска необходимы начальные и конечные границы данных POST. Эти функции не включены в блок `main()`.

Начальная граница включает в себя номер пакета, идентификатор типа содержимого `Content-Disposition: form-data; name="image"; filename=`, а также имя файла изображения для выполнения запроса POST. 

Конечная граница содержит только номер пакета. 
...
```
## <a name="add-image-bytes-to-post-body"></a>Добавление байтов изображения к основному тексту запроса POST

В этом коде создается запрос POST, который содержит данные изображения.
...
```
> [!NOTE]
> Код для этого примера предоставил Джордано Франческо (Giordano Francesco).
...
> [Краткое руководство. Поиск в Интернете с помощью REST API Bing для поиска в Интернете и Go](../../Bing-Web-Search/quickstarts/go.md)
