...
description: Параметры конфигурации контейнеров Анализа текста.
...
Настройка конфигурации `Billing` определяет URI конечной точки ресурса Анализа текста на Azure, используемый, чтобы отслеживать данные для выставления счетов для контейнера. Для этого параметра конфигурации необходимо задать значение, которое должно быть допустимым URI конечной точки для ресурса Анализа текста в Azure.
