...
Узнайте, как развертывать контейнер распознавания языка. Эта процедура показано, как создать локальный контейнеры Docker, отправьте контейнеров в реестр закрытый контейнер, запуска контейнера в кластере Kubernetes и протестировать его в веб-браузере. С помощью контейнеров можно сместить внимание разработчиков от управления инфраструктурой, чтобы вместо этого сосредоточиться на разработке приложений.
