...
>    * В настоящее время поддерживаются английский (`en`), японский (`ja`), китайский, упрощенное письмо (`zh-Hans`), китайский, традиционное письмо (`zh-Hant`), французский (`fr`), итальянский (`it`), испанский (`es`), голландский (`nl`), португальский (`pt`) и нидерландский (`de`) языки.
...
|Метки тональности и оценка     | Теперь, кроме оценки, API возвращает для текста сведения о категории тональности. Категории: `positive`, `negative`, `neutral` и `mixed`.       |
...
| Параметр версии модели | Необязательный параметр для выбора версии модели Анализа текста, используемой для данных. |
