...
Анализ текста Azure для работоспособности — это Контейнерная служба, которая извлекает и помечает соответствующие медицинские сведения из неструктурированных текстов, таких как заметки Doctor, сводки разрядов, документы клинической практике и электронные записи о работоспособности.  
...
Полный список поддерживаемых сущностей см. в статье [категории сущностей](../named-entity-types.md?tabs=health) , возвращенных анализ текста для сферы здравоохранения.
...
Заполните и отправьте [форму запроса Cognitive Services контейнеров](https://aka.ms/cognitivegate) , чтобы запросить доступ к контейнеру. В настоящее время вам не будет выставлен счет за Анализ текста для использования работоспособности. 
...
1. Использование [портал Azure](text-analytics-how-to-install-containers.md?tabs=healthcare) для создания анализ текста ресурса и использование DOCKER для получения контейнера.
2. Использование приведенных ниже сценариев PowerShell и [Azure CLI](https://docs.microsoft.com/cli/azure/?view=azure-cli-latest) для автоматизации конфигурации контейнера развертывания ресурсов.
