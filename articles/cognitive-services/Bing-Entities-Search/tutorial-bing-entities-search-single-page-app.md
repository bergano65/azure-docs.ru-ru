...
Чтобы выполнить задания, описанные в этом руководстве, требуются ключи подписок для API "Поиск Bing" и API "Карты Bing". Если у вас нет таких ключей, можете использовать [ключ пробной версии](https://azure.microsoft.com/try/cognitive-services/?api=bing-web-search-api) и [базовый ключ Карт Bing](https://www.microsoft.com/maps/create-a-bing-maps-key).
...
HTML-код содержит форму поиска, в которой пользователь вводит запрос и выбирает параметры поиска. Эта форма связана с языком JavaScript, на котором фактически выполняется поиск по атрибуту `<form>` тега `onsubmit`:
