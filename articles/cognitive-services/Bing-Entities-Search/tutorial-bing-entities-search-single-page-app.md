...
Для API Bing для поиска сущностей [требуется, чтобы результаты отображались в указанном порядке](use-display-requirements.md). Так как API может возвращать два типа ответа, недостаточно выполнить итерацию через коллекцию `Entities` или `Places` верхнего уровня в ответе JSON и отобразить полученные результаты. (Если необходим только один тип результата, то используйте параметр запроса `responseFilter`.)
