...
Обработчик `onsubmit` возвращает значение `false`, что препятствует отправке формы на сервер. Код JavaScript фактически осуществляет сбор необходимой информации из формы и выполняет поиск.
...
Например, функция SafeSearch может иметь значения `strict`, `moderate` или `off`, а `moderate` — это значение по умолчанию. Но в нашей форме он представлен флажком, который имеет только два состояния. Код JavaScript преобразует это значение в `strict` или `off` (мы не используем `moderate`).
