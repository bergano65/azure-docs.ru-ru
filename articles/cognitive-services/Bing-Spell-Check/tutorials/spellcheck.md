...
Чтобы выполнить задачи, описанные в этом руководстве, вам потребуется ключ подписки для API Bing для проверки орфографии. Если у вас его нет, вам потребуется следующее:
...
Создайте HTML-форму с текстовым полем. Обработайте событие `onsubmit` и вызовите функцию bingSpellCheck, передав содержимое текстового поля и ключ API Проверки орфографии Bing.
