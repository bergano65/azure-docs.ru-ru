...
## <a name="detect-text-from-images"></a>Определение текста на изображениях

Чтобы извлечь печатный и рукописный текст из изображений в поток символов, пригодный для машинного чтения, можно использовать API Компьютерного зрения (см. [API-интерфейсы определения текста](concept-recognizing-text.md)). API чтения использует обновленные модели и работает с текстом на таких разнообразных поверхностях и фонах, как квитанции, плакаты, визитные карточки, письма и доски. Два API-интерфейса обнаружения текста поддерживают извлечение печатного текста на [нескольких языках](./language-support.md).
