...
ms.locfileid: "57432113"
---
# <a name="what-is-computer-vision"></a>Что собой представляет компьютерное зрение

Компьютерное зрение Azure обеспечивает разработчикам доступ к расширенным алгоритмам обработки изображений и возврата данных. Чтобы проанализировать изображение, нужно передать его или указать его URL-адрес. Алгоритмы обработки изображений могут анализировать содержимое по-разному, в зависимости от интересующих вас визуальных компонентов. Например, API компьютерного зрения может определить, содержит ли изображение содержимое для взрослых или непристойного характера, или найти все лица людей на изображении.
...
Для извлечения текста из изображения в машиночитаемый поток символов с помощью [оптического распознавания символов (OCR)](concept-recognizing-text.md#ocr-optical-character-recognition-api) можно использовать Компьютерное зрение. При необходимости функция OCR выравнивает распознанный текст и предоставляет координаты кадра каждого слова. Функция OCR поддерживает 25 языков и автоматически определяет язык распознанного текста.

Можно также использовать [Read API](concept-recognizing-text.md#read-api) для извлечения как печатного, так и рукописного текста из изображений и текстовых документов. Read API использует обновленные модели и работает с разнообразными объектами с различными поверхностями и фонами, такими как квитанции, плакаты, визитные карточки, письма и доски. В настоящее время Read API находится в режиме предварительного просмотра и единственным поддерживаемым языком является английский.

## <a name="moderate-content-in-images"></a>Модерация содержимого на изображениях

Компьютерное зрение можно использовать для [обнаружения непристойного и оскорбительного содержимого](concept-detecting-adult-content.md) в изображение и оценки достоверности такого содержимого. Фильтр, определяющий содержимое для взрослых и носящее непристойный характер, можно настроить в соответствии с предпочтением пользователя по степени принятия материалов данного характера.
