...
Компьютерное зрение включает возможности [оптического распознавания символов (OCR)](concept-recognizing-text.md). Вы также можете использовать API чтения для извлечения печатного и рукописного текста из изображений и документов. Это средство использует последние модели и работает с текстом на самых разных поверхностях и фонах, таких как чеки, плакаты, визитные карточки, письма и доски. Два API OCR поддерживают извлечение печатного текста на [нескольких языках](./language-support.md).
