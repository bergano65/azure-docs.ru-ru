...
ms.locfileid: "65231659"
---
# <a name="use-computer-vision-features-with-the-rest-api-and-javascript"></a>Используйте возможности компьютерного зрения с REST API и JavaScript

В этом руководстве приведены возможности Azure Cognitive Services API компьютерного зрения REST.
...
Приложение форм JavaScript уже написано, но в нем нет функциональных возможностей API компьютерного зрения. В этом руководстве добавьте код, применяемый к REST API компьютерного зрения компьютера, чтобы завершить работу приложения.

## <a name="prerequisites"></a>Технические условия

### <a name="platform-requirements"></a>Требования платформы

Необходимо выполнить действия, описанные в этом руководстве, с помощью простого текстового редактора.
...
Если вы хотите использовать готовый проект с добавляется учебника код, можно использовать файлы в **завершено** папки.

## <a name="add-tutorial-code-to-the-project"></a>Добавьте код учебника в проект

В приложении JavaScript заданы шесть HTML-файлов, по одному для каждого компонента. Каждый файл демонстрирует другую функцию компьютерного (анализ, распознавание текста и т. д.). Шесть разделов не имеют взаимозависимостей, поэтому можно добавить код учебника один файл, все шесть файлов или только несколько файлов. Коме того, учебный код можно добавлять в файлы в любом порядке.

### <a name="analyze-an-image"></a>Анализ изображения

Функция анализа компьютерного сканирует изображения для тысяч узнаваемых объектов, живых организмов, пейзажей и действий. После завершения анализа эта функция возвращает объект JSON, описывающий изображение с помощью описательных тегов, анализа цвета, подписей и многого другого.

Чтобы выполнить функции анализа приложения, выполните следующие действия:
...
Чтобы выполнить функцию ориентиров приложения, выполните следующие действия:

#### <a name="add-the-event-handler-code-for-the-landmark-button"></a>Добавьте код обработчика событий для кнопки черт лица
...
Чтобы выполнить функцию знаменитостей приложения, выполните следующие действия:
...
Чтобы выполнить функцию эскиз приложения, выполните следующие действия:
...
Чтобы выполнить функцию распознавания текста приложение, выполните следующие действия:
...
Чтобы выполнить функцию распознавания рукописного текста приложения, выполните следующие действия:
...
В этом руководстве вы использовали REST API компьютерного зрения с JavaScript для тестирования многих возможностей анализа доступных образа. Далее см. в справочной документации, чтобы узнать больше об API, участвующих.
