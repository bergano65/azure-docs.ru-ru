...
С помощью [API предварительной версии Read 3,2](https://westus.dev.cognitive.microsoft.com/docs/services/computer-vision-v3-2-preview-2/operations/5d986960601faab4bf452005)для больших многостраничных документов используйте `pages` параметр query, чтобы указать номера страниц или диапазоны страниц для извлечения текста только из этих страниц. Например, в следующем примере показан документ с 10 страницами для обоих вариантов — все страницы (1-10) и выбранные страницы (3-6).

:::image border type="content" source="./Images/ocr-select-pages.png" alt-text="Выходные данные выбранных страниц":::

## <a name="specify-text-line-order-in-the-output"></a>Укажите порядок текстовых строк в выходных данных
С помощью [предварительной версии API Read 3,2 для предварительного просмотра](https://westus.dev.cognitive.microsoft.com/docs/services/computer-vision-v3-2-preview-2/operations/5d986960601faab4bf452005)укажите порядок, в котором текстовые строки выводятся с помощью `read order` параметра запроса. Выберите один из двух параметров: по `basic` умолчанию — слева и сверху вниз, а `natural` для более удобной — удобный для чтения порядок линий. В следующем примере показаны оба набора номеров строк заказа для одного и того же документа с двумя столбцами. Обратите внимание, что изображение справа показывает последовательные номера строк в каждом столбце для представления порядка чтения.

:::image border type="content" source="./Images/ocr-read-order.png" alt-text="Пример порядка чтения с РАСПОЗНАВАНИем":::
...
:::image border type="content" source="./Images/handwritten-text-line.png" alt-text="Пример классификации рукописного ввода с оптическим распознаванием":::
