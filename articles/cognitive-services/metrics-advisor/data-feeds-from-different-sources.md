...
| **азуресклконнектионстринг**| Храните строку подключения AzureSQL в качестве **сущности для проверки подлинности** в помощнике по метрикам и используйте ее непосредственно при подключении данных метрик. Только администраторы сущности проверки подлинности могут просматривать эти учетные данные, но позволяют полномочным пользователям создавать веб-каналы данных, не требуя сведений об этих учетных данных. |
| **DataLakeGen2SharedKey**| Храните ключ учетной записи Data Lake в качестве **сущности для проверки подлинности** в советнике по метрикам и используйте его непосредственно при подключении данных метрик. Только администраторы сущности проверки подлинности могут просматривать эти учетные данные, но позволяют полномочным пользователям создавать веб-канал данных без необходимости знать сведения об учетных данных.|
| **ServicePrincipal**| Сохраните субъект-службу в качестве **сущности для проверки подлинности** в советнике по метрикам и используйте его непосредственно при подключении данных метрик. Только администраторы сущности проверки подлинности могут просматривать учетные данные, но позволяют полномочным пользователям создавать веб-канал данных без необходимости знать сведения об учетных данных.|
| **сервицепринЦипалинкэйваулт**|Сохраните субъект-службу в KeyVault как **сущность проверки подлинности** в советнике по метрикам и используйте его непосредственно при подключении данных метрик. Только администраторы **сущности проверки подлинности** могут просматривать учетные данные, но также не должны иметь возможности создавать канал данных, не зная подробных учетных данных. |
...
|[**Azure Data Lake Storage 2-го поколения**](#adl) | Базовый<br>DataLakeGen2SharedKey<br>ServicePrincipal<br>сервицепринЦипалинкэйваулт<br> |
|[**База данных SQL Azure/SQL Server**](#sql) | Базовый<br>ManagedIdentity<br>ServicePrincipal<br>сервицепринЦипалинкэйваулт<br>азуресклконнектионстринг
...
Создайте **сущность проверки подлинности** и используйте ее для проверки подлинности в источниках данных. В следующих разделах указываются параметры, необходимые для *обычной* проверки подлинности. 
