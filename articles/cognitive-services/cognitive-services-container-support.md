...
ms.locfileid: "79219415"
---
# <a name="container-support-in-azure-cognitive-services"></a>Поддержка контейнеров в Azure Cognitive Services

Поддержка контейнеров в Azure Cognitive Services позволяет разработчикам использовать те же богатые AA, которые доступны в Azure, и обеспечивает гибкость в развертывании и размещении служб, которые поставляются с [контейнерами Docker.](https://www.docker.com/what-container) Контейнерная поддержка в настоящее время доступна для подмножества когнитивных служб Azure, включая части:
...
Ресурсы когнитивных служб доступны в [Microsoft Azure.](https://azure.microsoft.com) Войдите на [портал Azure](https://portal.azure.com/), чтобы создать и изучить ресурсы Azure для этих служб.

## <a name="features-and-benefits"></a>Возможности и преимущества:

- **Неизменяемая инфраструктура**: Включите команды DevOps, чтобы использовать согласованный и надежный набор известных параметров системы, при этом будучи в состоянии адаптироваться к изменениям. Контейнеры обеспечивают гибкость для поворота в пределах предсказуемой экосистемы и избежать дрейфа конфигурации.
- **Контроль над данными**: Позвольте клиентам выбирать, где эти когнитивные службы обрабатывают их данные. Это особенно важно для тех клиентов, у которых есть потребность в технологии Cognitive Services, но нет возможности отправлять данные в облако. Поддержка согласованности в гибридной среде, в том числе данных, управления, идентификатора и безопасности.
...
- **Портативная архитектура**: Включите создание портативной архитектуры приложений, которая может быть развернута на Azure, на территории и краю. Контейнеры можно развертывать непосредственно в [Службе Azure Kubernetes](../aks/index.yml), [Экземплярах контейнеров Azure](../container-instances/index.yml) или в кластере [Kubernetes](https://kubernetes.io/), развернутом в [Azure Stack](/azure-stack/operator). Дополнительные сведения см. в статье [Развертывание Kubernetes в Azure Stack](/azure-stack/user/azure-stack-solution-template-kubernetes-deploy).
- **Высокая пропускная способность / низкая задержка**: Предоставьте клиентам возможность масштабирования для высокой пропускной способности и низких требований задержки, позволяя Когнитивным службам работать физически близко к их логике приложения и данным. Контейнеры не ограничивают число транзакций в секунду (TPS), и их можно увеличивать и уменьшать для удовлетворения спроса, предоставив необходимые аппаратные ресурсы.
- **Масштабируемость**: С постоянно растущей популярностью контейнеризации и программного обеспечения оркестровки контейнеров, таких как Kubernetes; масштабируемость находится на переднем крае технологических достижений. Опираясь на масштабируемую кластерную основу, разработка приложений ориентирована на высокую доступность.
...
| Служба | Поддерживаемый уровень ценообразования | Контейнер | Описание |
|---------|----------|----------|-------------|
...
|[API Компьютерного зрения][cv-containers] |F0, S1|**Прочитать** |Извлекает печатный текст из изображений различных объектов с разными поверхностями и фонами, например чеков, плакатов и визитных карточек. Контейнер Read также обнаруживает *рукописный текст* в изображениях и обеспечивает поддержку PDF/TIFF/multi-page.<br/><br/>**Важно:** Контейнер Read в настоящее время работает только с английским языком.|
...
|[Распознаватель формы][fr-containers] |F0, S0|**Распознаватель документов** |Form Understanding применяет технологию машинного обучения для выявления и извлечения пар и таблиц с ключевымзначением из форм.<br>[Запрос доступа](https://aka.ms/FormRecognizerContainerRequestAccess)|
|[Луис][lu-containers] |F0, S0|**LUIS** ([образ](https://go.microsoft.com/fwlink/?linkid=2043204&clcid=0x409))|Загружает обученную или опубликованную модель Распознавания речи, также известную как приложение LUIS, в контейнер Docker и предоставляет доступ к прогнозам запроса из конечных точек API контейнера. Можно собирать журналы запросов из контейнера и отправлять их обратно на [портал LUIS](https://www.luis.ai) для повышения точности прогнозов приложения.|
|[API службы "Речь"][sp-containers-stt] |F0, S0|**Речево-текст** |Расшифровывает непрерывную речь в режиме реального времени в текстовый формат.|
|[API службы "Речь"][sp-containers-cstt] |F0, S0|**Пользовательские речи к тексту** |Транскрибирует непрерывную речь в реальном времени в текст с помощью пользовательской модели.|
|[API службы "Речь"][sp-containers-tts] |F0, S0|**Текст к речи** |Преобразует текст в естественно звучащую речь.|
|[API службы "Речь"][sp-containers-ctts] |F0, S0|**Пользовательский текст к речи** |Преобразует текст в естественно звучающую речь с помощью пользовательской модели.|
|[Текстовая аналитика][ta-containers-keyphrase] |F0, S|**Извлечение ключевых фраз** ([изображение](https://go.microsoft.com/fwlink/?linkid=2018757&clcid=0x409)) |Извлекает ключевые фразы для определения основных идей текста. Например, для входного текста "Еда была вкусной и персонал был замечательным" API вернет основные тезисы в записи: "еда" и "замечательный персонал". |
|[Текстовая аналитика][ta-containers-language]|F0, S|**Определение языка** ([изображение](https://go.microsoft.com/fwlink/?linkid=2018759&clcid=0x409)) |Решение может определить язык введенного текста (поддерживается более 120 языков) и сообщить один код языка для каждого документа, отправленного в запросе. Код языка сопряжен с показателем, указывающим степень оценки. |
|[Текстовая аналитика][ta-containers-sentiment]|F0, S|**Анализ тональности** ([изображение](https://go.microsoft.com/fwlink/?linkid=2018654&clcid=0x409)) |Анализирует необработанный текст на предмет положительной или отрицательной тональности. Этот API возвращает оценку тональности (0 или 1) для каждого документа, где 1 означает положительную тональность. Модели анализа предварительно обучены с использованием обширного набора текстов и технологий естественного языка корпорации Майкрософт. Для [выбранных языков](./text-analytics/language-support.md) API может анализировать и оценивать любой необработанный текст, напрямую возвращая результаты вызывающему приложению. |

<!--
|[Personalizer](https://go.microsoft.com/fwlink/?linkid=2083923&clcid=0x409) |F0, S0|**Personalizer** ([image](https://go.microsoft.com/fwlink/?linkid=2083928&clcid=0x409))|Azure Personalizer is a cloud-based API service that allows you to choose the best experience to show to your users, learning from their real-time behavior.|
-->

Кроме того, некоторые контейнеры поддерживаются в Когнитивных [**Услугах All-In-One, предлагающих**](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesAllInOne) ключи ресурса. Можно создать единый ресурс Cognitive Services All-In-One и использовать один и тот же ключ выставления счетов в поддерживаемых службах для следующих служб:
...
> [!IMPORTANT]
> В настоящее время вы должны завершить процесс регистрации для доступа к следующим контейнерам, в которых вы заполняете и отправляете анкету с вопросами о вас, вашей компании и случае использования, для которого вы хотите реализовать контейнеры. После того, как вы получите доступ и предоставили учетные данные, вы можете вытащить изображения контейнеров из частного реестра контейнеров, размещенных в реестре контейнеров Azure.
...
> * [Прочитать](computer-vision/computer-vision-how-to-install-containers.md)
> * [От речи к тексту и текст-к-речи](Speech-Service/speech-container-howto.md#request-access-to-the-container-registry)
...
Узнайте о [рецептах контейнеров,](containers/container-reuse-recipe.md) которые можно использовать в Службах Когнитивных Услуг.

Установите и изучите функциональные возможности, предоставляемые контейнерами в Azure Cognitive Services:

* [Контейнеры-детекторы аномалий][ad-containers]
* [Контейнеры компьютерного зрения][cv-containers]
* [Контейнеры для лица][fa-containers]
* [Контейнеры для распознавания форм][fr-containers]
* [Контейнеры для понимания языка (LUIS)][lu-containers]
* [Контейнеры API службы речи][sp-containers]
* [Контейнеры для анализа текста][ta-containers]
