...
|[Компьютерное зрение][cv-containers] |F0, S1|**чтение** |Извлекает печатный текст из изображений различных объектов с разными поверхностями и фонами, например чеков, плакатов и визитных карточек. Контейнер чтения также обнаруживает *рукописный текст* в изображениях и предоставляет поддержку PDF/TIFF/многостраничной поддержки.<br/><br/>**Важно!** Контейнер чтения в настоящее время работает только на английском языке.<br>[Запрос доступа](Computer-vision/computer-vision-how-to-install-containers.md#request-access-to-the-private-container-registry)|
