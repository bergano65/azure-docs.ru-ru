---
title: Краткое руководство. Создание ресурса Azure Cognitive Services с помощью шаблонов ARM | Документация Майкрософт
description: Начало работы с шаблоном Azure Resource Manager для развертывания ресурса Cognitive Services.
...
В этой статье показано, как создать и развернуть ресурс Cognitive Services, используя шаблон Azure Resource Manager (ARM). Этот ресурс для нескольких служб дает вам следующие возможности:
...
* [Microsoft.CognitiveServices/accounts](https://docs.microsoft.com/azure/templates/microsoft.cognitiveservices/accounts): создает ресурс Cognitive Services.
...
> [!NOTE]
> Для создания `az deployment group` требуется Azure CLI 2.6 или более поздней версии. Для отображения версии введите `az --version`. Дополнительные сведения см. в этой [документации](https://docs.microsoft.com/cli/azure/deployment/group).

Выполните следующий сценарий с помощью интерфейса командной строки Azure (CLI) [на локальном компьютере](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest) или в браузере с помощью кнопки **Попробовать**. Введите имя и расположение (например, `centralus`) для новой группы ресурсов, а для развертывания в ней ресурса Cognitive Services будет использоваться шаблон ARM. Запомните или запишите имя, которое вы используете. Вы будете использовать его позже для проверки развертывания.
