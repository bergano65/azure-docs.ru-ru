...
В запросе можно также указать необязательный параметр [mkt](https://docs.microsoft.com/rest/api/cognitiveservices-bingsearch/bing-custom-search-api-v7-reference#mkt), определяющий рынок, для которого будут отображаться результаты. Для списка дополнительных параметров запроса [см.](https://docs.microsoft.com/rest/api/cognitiveservices-bingsearch/bing-custom-search-api-v7-reference#query-parameters)

Страну или регион можно указать с помощью параметра запроса `cc`. Указывая страну или регион, необходимо также задать один или несколько кодов языков с помощью `Accept-Language`-заголовка. Поддерживаемые языки варьируются в зависимости от страны/региона; они даются для каждой страны/региона в таблице **Рынков.**
...
|Гонконг, SAR|Китайский (традиционный)|zh-HK|
