---
title: Добавление аналитики в API Bing для поиска изображений
titleSuffix: Azure Cognitive Services
description: Статистика Bing предоставляет аналитику для API Bing для поиска изображений. Аналитика включает в себя вызов тома, часто используемые строки запроса, географическое распределение и многое другое.
services: cognitive-services
author: swhite-msft
manager: nitinme
ms.service: cognitive-services
ms.subservice: bing-image-search
ms.topic: article
ms.date: 03/04/2019
ms.author: scottwhi
ms.openlocfilehash: 43a6c221a08c926243d0fd5cb0ce97d5f0955f97
ms.sourcegitcommit: 41ca82b5f95d2e07b0c7f9025b912daf0ab21909
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 06/13/2019
ms.locfileid: "60520092"
---
# <a name="add-analytics-to-the-bing-image-search-api"></a>Добавление аналитики в API Bing для поиска изображений

[!INCLUDE [cognitive-services-bing-statistics](../../../includes/cognitive-services-bing-statistics.md)]
