...
При использовании частных конечных точек с голосовыми службами необходимо использовать пользовательскую конечную точку для вызова службы распознавания речи. Нельзя использовать глобальную конечную точку. Конечная точка должна соответствовать этому шаблону: `{account}.{stt|tts|voice|dls}.speech.microsoft.com` .
