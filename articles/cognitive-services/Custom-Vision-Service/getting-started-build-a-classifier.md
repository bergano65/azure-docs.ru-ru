...
1. Затем выберите один из доступных доменов. Каждый домен оптимизирует классификатор для определенных типов изображений, как описано в следующей таблице. Если потребуется, вы сможете изменить этот домен позднее.
    ...
    |Домен|Назначение|
    |---|---|
    |__Универсальный__| Рассчитан на самые разные задачи классификации изображений. Если не подходит ни один из других доменов или вы не уверены, какой домен выбрать, выбирайте Generic. |
    ...
    |__Розничная торговля__|Рассчитан на изображения из каталогов товаров и торговых веб-сайтов. Если требуется высокая точность при распознавании таких объектов, как платья, брюки и рубашки, следует использовать этот домен.|
    |__Совершеннолетние__|Рассчитан на определение материалов для взрослых. Например, если вы хотите блокировать изображения людей в купальниках, этот домен дает возможность создать соответствующий пользовательский классификатор.|
    |__Compact доменов__| Оптимизированы для ограничений классификации в режиме реального времени на мобильных устройствах. Модели, созданные доменами Compact, можно экспортировать для локального запуска.|
...
[Тестирование и повторное Обучение модели](test-your-model.md)
