...
ms.locfileid: "66389687"
---
# <a name="how-to-use-the-transformtext-method"></a>Использование метода TransformText

> [!NOTE]
> Этот метод устарел и больше не поддерживается. Он недоступен в версии API перевода текстов 3.0.
...
Метод представляет собой службу RESTful, использующую метод GET по протоколу HTTP. Он поддерживает простую сериализацию XML и JSON.
...
| language | **Обязательный**. Строка, представляющая код языка. Этот параметр поддерживает только английский язык с именем **en**.|
| category | **Необязательный**. Строка, содержащая категорию (домен) перевода. Этот параметр поддерживает только параметр по умолчанию — **general** (общая).|
| sentence | **Обязательный**. Предложение, которое нужно исправить. |

## <a name="return-value"></a>Возвращаемое значение

Возвращаемое значение содержит преобразованное предложение.
