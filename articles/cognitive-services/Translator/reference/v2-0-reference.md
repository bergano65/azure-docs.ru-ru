...
|Text|(пусто)   |Обязательный элемент. Строка, представляющая текст для перевода. Размер текста не должен превышать 10 000 знаков.|query|строка|
...
|:--|:--|:--|:--|:--|
|appid|(пусто)|Обязательный элемент. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле appid пустым, а потом введите туда строку, содержащую `"Bearer" + " " + "access_token"`.|query|string|
|Text|(пусто)   |Обязательный элемент. Строка, содержащая предложение или предложения на указанном языке, которые необходимо произнести для создания потока формата WAVE. Размер устного текста не должен превышать 2000 знаков.|query|строка|
|language|(пусто)   |Обязательный элемент. Строка, представляющая код языка, на котором произносится текст. Код должен присутствовать в списке кодов, возвращаемых методом `GetLanguagesForSpeak`.|query|строка|
...
|до|(пусто)|Обязательный элемент. Строка, представляющая код языка, на который переводится текст.|query|string|
|рейтинг|(пусто) |Необязательный элемент. Целое число, представляющее оценку качества этой строки. Значение от –10 до 10. По умолчанию равен 1.|query|integer|
...
|Text|(пусто)   |Обязательный элемент. Строка, представляющая текст для перевода, который нужно разделить на предложения. Размер текста не должен превышать 10 000 знаков.|query|строка|
|language   |(пусто)    |Обязательный элемент. Строка, представляющая код языка.|query|строка|
|Авторизация|(пусто)|Требуется, если поле appid или заголовок Ocp-Apim-Subscription-Key не указаны. Маркер авторизации.  "Bearer" + " " + "access_token".    |Верхний колонтитул|string|
|Ocp-Apim-Subscription-Key|(пусто)|Требуется, если поле appid или заголовок авторизации не указаны.|Верхний колонтитул|string|
...
|до |(пусто)    |Обязательный элемент. Строка, представляющая код языка, на который переводится текст.|query|string|
|maxTranslations|(пусто)|Обязательный элемент. Целочисленное значение, представляющее максимальное число возвращаемых переводов.|query|integer|
