...
Чтобы получить доступ к переводчику, необходимо [зарегистрироваться в Microsoft Azure](../translator-how-to-signup.md).
...
|из|(пусто)   |Необязательный параметр. Строка, представляющая код языка для преобразуемого текста. Например, en для английского языка.|query|строка|
|значение|(пусто) |Обязательный. Строка, представляющая код языка, в который преобразуется текст.|query|строка|
|сontentType|(пусто)    |Необязательный параметр. Формат переведенного текста. Поддерживаются форматы `text/plain` (по умолчанию) и  `text/html` . Все HTML-элементы должны быть правильно сформированными, полными элементами.|query|строка|
...
|значение|(пусто)|Обязательный. Строка, представляющая код языка для перевода текста в.|query|строка|
|рейтинг|(пусто) |Необязательный параметр. Целое число, представляющее оценку качества строки. Значение находится в диапазоне от-10 до 10. Значение по умолчанию — 1.|query|Целое число|
|сontentType|(пусто)    |Необязательный параметр. Формат переведенного текста. Поддерживаемые форматы: `text/plain` и `text/html` . Все HTML-элементы должны быть правильно сформированными, полными элементами.    |query|строка|
|категория|(пусто)|Необязательный параметр. Строка, содержащая категорию (домен) перевода. Значение по умолчанию — `general`.|query|строка|
...
|значение |(пусто)    |Обязательный. Строка, представляющая код языка для перевода текста в.|query|строка|
|maxTranslations|(пусто)|Обязательный. Целое число, представляющее максимальное число возвращаемых переводов.|query|Целое число|
