...
Все вызовы API перевода текстов требуют ключ подписки для аутентификации. Этот API поддерживает три режима проверки подлинности:

- Маркер доступа. Используйте ключ подписки, указанный на **шаге** 9, чтобы создать маркер доступа, выполнив запрос POST для службы авторизации. Дополнительные сведения см. в документации службы токенов. Передать маркер доступа у сервиса перевода, с помощью заголовка авторизации или `access_token` параметр запроса. Маркер доступа действителен 10 минут. Получите новый маркер доступа каждые 10 минут и продолжать использовать тот же уровень доступа маркера для повторных запросов во время этих 10 минут.
- Непосредственно ключ подписки. Передать ключ подписки в качестве значения `Ocp-Apim-Subscription-Key` заголовок, включенный в запрос к Translator API. В этом режиме не нужно вызывать службу проверки подлинности маркеров для создания маркера доступа.
- Объект [подписки нескольких служб Cognitive Services](https://azure.microsoft.com/pricing/details/cognitive-services/). Этот режим позволяет использовать один секретный ключ для проверки подлинности запросов для нескольких служб. <br/>
...
|:--|:--|:--|:--|:--|
|appid  |(пусто)    |Обязательный элемент. Если используется заголовок авторизации или заголовок Ocp-Apim-Subscription-Key, оставьте поле appid пустым, или укажите строку, содержащую "Bearer" + " " + "access_token" (маркер носителя и маркер доступа).|query|string|
|Text|(пусто)   |Обязательный элемент. Строка, представляющая текст для перевода. Размер текста не должен превышать 10 000 знаков.|query|string|
|from|(пусто)   |Необязательный элемент. Строка, представляющая код языка текста перевода. Например, en для английского языка.|query|string|
|до|(пусто) |Обязательный элемент. Строка, представляющая код языка, на который переводится текст.|query|string|
|сontentType|(пусто)    |Необязательный элемент. Формат переведенного текста. Поддерживаемые форматы: text/plain (по умолчанию) и text/html. Любой код HTML должен быть полным элементом с правильным форматом.|query|string|
|category|(пусто)   |Необязательный элемент. Строка, содержащая категорию (домен) перевода. Значение по умолчанию: general.|query|string|
|Авторизация|(пусто)  |Требуется, если поле appid или заголовок Ocp-Apim-Subscription-Key не указаны. Маркер авторизации.  "Bearer" + " " + "access_token".|Верхний колонтитул|string|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если поле appid или заголовок авторизации не указаны.|Верхний колонтитул|string|
...
* `options`: Необязательный элемент. Объект `Options`, содержащий перечисленные ниже значения. Все они необязательны и по умолчанию соответствуют наиболее распространенным настройкам. Указанные элементы должны находится в алфавитном порядке.
    - `Category`: Строка, содержащая категорию (домен) перевода. По умолчанию — `general`.
...
Текст запроса включает массив строк, представляющих коды ISO 639-1 языка для извлечения понятных имен. Пример:
...
**Возвращаемое значение:** Строковый массив, содержащий названия языков, поддерживаемых Translator Service, локализованый для запрошенного языка.
...
|Параметр|Значение|Описание|Тип параметра|Тип данных|
|:--|:--|:--|:--|:--|
|appid|(пусто)|Обязательный элемент. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле appid пустым, а потом введите туда строку, содержащую `"Bearer" + " " + "access_token"`.|query|string|
|локаль|(пусто) |Обязательный элемент. Строка, представляющая комбинацию двухбуквенного кода ISO 639 с нижним регистром, связанного с языком, и двухбуквенного кода ISO 3166 с верхним регистром для локализации имен языков или же просто код ISO 639.|query|string|
|Авторизация|(пусто)  |Требуется, если поле appid или заголовок `Ocp-Apim-Subscription-Key` не указаны. Маркер авторизации: `"Bearer" + " " + "access_token"`.|Верхний колонтитул|string|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если поле appid или заголовок `Authorization` не указаны.|Верхний колонтитул|string|
...
|:--|:--|:--|:--|:--|
|appid|(пусто)|Обязательный элемент. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле appid пустым, а потом введите туда строку, содержащую `"Bearer" + " " + "access_token"`.|query|string|
|Text|(пусто)   |Обязательный элемент. Строка, содержащая предложение или предложения на указанном языке, которые необходимо произнести для создания потока формата WAVE. Размер устного текста не должен превышать 2000 знаков.|query|string|
|language|(пусто)   |Обязательный элемент. Строка, представляющая код языка, на котором произносится текст. Код должен присутствовать в списке кодов, возвращаемых методом `GetLanguagesForSpeak`.|query|string|
|свойства|(пусто)|Необязательный элемент. Строка, задающая идентификатор типа содержимого. В настоящее время доступны `audio/wav` и `audio/mp3`. По умолчанию используется значение `audio/wav`.|query|string|
|options|(пусто)    |<ul><li>Необязательный элемент. Строка, указывающая свойства синтезированной речи:<li>`MaxQuality` и `MinSize` доступны для указания качества звуковых сигналов. С `MaxQuality` вы можете получить голоса высочайшего качества, а с `MinSize` — голоса с наименьшим размером. Значение по умолчанию — `MinSize`.</li><li>`female` и `male` доступны для указания голоса нужного пола. Значение по умолчанию — `female`. Используйте вертикальную черту <code>\|</code> для добавления нескольких вариантов. Например `MaxQuality|Male`.</li></li></ul> |query|string|
|Авторизация|(пусто)|Требуется, если поле `appid` или заголовок `Ocp-Apim-Subscription-Key` не указаны. Маркер авторизации: `"Bearer" + " " + "access_token"`.|Верхний колонтитул|string|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если поле `appid` или заголовок `Authorization` не указаны.|Верхний колонтитул|string|
...
|:--|:--|:--|:--|:--|
|appid|(пусто)  |Обязательный элемент. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле appid пустым, а потом введите туда строку, содержащую `"Bearer" + " " + "access_token"`.|query|string|
|Text|(пусто)|Обязательный элемент. Строка, содержащая текст, язык которого нужно определить. Размер текста не должен превышать 10 000 знаков.|query| string|
|Авторизация|(пусто)|Требуется, если поле `appid` или заголовок `Ocp-Apim-Subscription-Key` не указаны. Маркер авторизации: `"Bearer" + " " + "access_token"`.|Верхний колонтитул|string|
|Ocp-Apim-Subscription-Key  |(пусто)    |Требуется, если поле `appid` или заголовок `Authorization` не указаны.|Верхний колонтитул|string|
...
|:--|:--|:--|:--|:--|
|appid|(пусто)|Обязательный элемент. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле appid пустым, а потом введите туда строку, содержащую `"Bearer" + " " + "access_token"`.|query|string|
|OriginalText|(пусто)|Обязательный элемент. Строка, содержащая текст для перевода. Максимальная длина строки – 1000 знаков.|query|string|
|TranslatedText|(пусто) |Обязательный элемент. Строка, содержащая текст, переведенный на целевой язык. Максимальная длина строки – 2000 знаков.|query|string|
|from|(пусто)   |Обязательный элемент. Строка, представляющая код языка текста перевода. en = english, de = german и т. д.|query|string|
|до|(пусто)|Обязательный элемент. Строка, представляющая код языка, на который переводится текст.|query|string|
...
|сontentType|(пусто)    |Необязательный элемент. Формат переведенного текста. Поддерживаемые форматы: text/plain и text/html. Любой код HTML должен быть полным элементом с правильным форматом.   |query|string|
|category|(пусто)|Необязательный элемент. Строка, содержащая категорию (домен) перевода. Значение по умолчанию: general.|query|string|
|user|(пусто)|Обязательный элемент. Строка, используемая для отслеживания источника для отправки.|query|string|
|uri|(пусто)|Необязательный элемент. Строка, содержащая расположения содержимого этого перевода.|query|string|
|Авторизация|(пусто)|Требуется, если поле appid или заголовок `Ocp-Apim-Subscription-Key` не указаны. Маркер авторизации: `"Bearer" + " " + "access_token"`.    |Верхний колонтитул|string|
|Ocp-Apim-Subscription-Key|(пусто)|Требуется, если поле `appid` или заголовок `Authorization` не указаны.|Верхний колонтитул|string|
...
|:--|:--|:--|:--|:--|
|appid|(пусто)  |Обязательный элемент. Если используется заголовок авторизации или заголовок Ocp-Apim-Subscription-Key, оставьте поле appid пустым, или укажите строку, содержащую "Bearer" + " " + "access_token" (маркер носителя и маркер доступа).|query| string|
|Text|(пусто)   |Обязательный элемент. Строка, представляющая текст для перевода, который нужно разделить на предложения. Размер текста не должен превышать 10 000 знаков.|query|string|
|language   |(пусто)    |Обязательный элемент. Строка, представляющая код языка.|query|string|
|Авторизация|(пусто)|Требуется, если поле appid или заголовок Ocp-Apim-Subscription-Key не указаны. Маркер авторизации.  "Bearer" + " " + "access_token".    |Верхний колонтитул|string|
|Ocp-Apim-Subscription-Key|(пусто)|Требуется, если поле appid или заголовок авторизации не указаны.|Верхний колонтитул|string|
...
|appid|(пусто)|Обязательный элемент. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле appid пустым, а потом введите туда строку, содержащую `"Bearer" + " " + "access_token"`.|query|string|
|Text|(пусто)|Обязательный элемент. Строка, представляющая текст для перевода. Размер текста не должен превышать 10 000 знаков.|query|string|
|from|(пусто)|Обязательный элемент. Строка, представляющая код языка текста перевода.|query|string|
|до |(пусто)    |Обязательный элемент. Строка, представляющая код языка, на который переводится текст.|query|string|
...
|Авторизация| (пусто)|Требуется, если поле `appid` или заголовок `Ocp-Apim-Subscription-Key` не указаны. Маркер авторизации: `"Bearer" + " " + "access_token"`.|string| Верхний колонтитул|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если поле `appid` или заголовок `Authorization` не указаны.|Верхний колонтитул|string|
