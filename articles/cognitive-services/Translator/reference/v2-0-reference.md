...
- Маркер доступа. Используйте ключ подписки, чтобы создать маркер доступа, выполнив запрос POST к службе проверки подлинности. Дополнительные сведения см. в документации службы токенов. Передайте маркер доступа службе транслятора с помощью `Authorization` заголовка или параметра запроса `access_token`. Маркер доступа действителен 10 минут. Получите новый маркер доступа каждые 10 минут и используйте один и тот же маркер доступа для повторяющихся запросов в течение 10 минут.
- Ключ подписки, используемый напрямую. Передайте ключ подписки в качестве значения в заголовке `Ocp-Apim-Subscription-Key`, включаемом в запрос API перевода текстов. Если вы используете ключ подписки напрямую, вам не нужно вызывать службу проверки подлинности маркеров для создания маркера доступа.
...
|Marked     |Ненормативные слова будут окружены тегами XML \<ненормативной лексикой > и \</профанити >.       |彼はジャッカスです。 |Он является \<ненормативной лексикой > жаккасс\</профанити >.  |
...
|:--|:--|:--|:--|:--|
|appid  |(пусто)    |обязательный параметр. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле `appid` пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"`.|запрос|строка|
|text|(пусто)   |обязательный параметр. Строка, представляющая текст для преобразования. Текст не может содержать более 10 000 символов.|запрос|строка|
|from|(пусто)   |необязательный параметр. Строка, представляющая код языка для преобразуемого текста. Например, en для английского языка.|запрос|строка|
|значение|(пусто) |обязательный параметр. Строка, представляющая код языка, в который преобразуется текст.|запрос|строка|
|сontentType|(пусто)    |необязательный параметр. Формат переведенного текста. Поддерживаемые форматы: `text/plain` (по умолчанию) и `text/html`. Все HTML-элементы должны быть правильно сформированными, полными элементами.|запрос|строка|
|категория|(пусто)   |необязательный параметр. Строка, содержащая категорию (домен) перевода. Значение по умолчанию — `general`.|запрос|строка|
|Авторизация|(пусто)  |Требуется, если и поле `appid`, и заголовок `Ocp-Apim-Subscription-Key` оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.|Верхний колонтитул|строка|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если и поле `appid`, и заголовок `Authorization` оставлены пустыми.|Верхний колонтитул|строка|
...
* `AppId`: обязательный параметр. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле `AppId` пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"`.
...
* `Options`: необязательный параметр. Объект `Options`, содержащий следующие значения. Все они являются необязательными и по умолчанию используются наиболее распространенными параметрами. Указанные элементы должны находится в алфавитном порядке.
...
* `Texts`: обязательный параметр. Массив, содержащий текст для перевода. Все строки должны быть на одном языке. Общий объем преобразуемого текста не должен превышать 10 000 символов. Максимальное число элементов массива — 2 000.
* `To`: обязательный параметр. Строка, представляющая код языка, в который преобразуется текст.
...
### <a name="implementation-notes"></a>Примечания по реализации
Получает понятные имена для языков, переданных в качестве параметра `languageCodes`, локализованных в переданный `locale`ный язык.
...
|:--|:--|:--|:--|:--|
|appid|(пусто)|обязательный параметр. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле `appid` пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"`.|запрос|строка|
|языковой стандарт|(пусто) |обязательный параметр. Строка, представляющая один из следующих элементов, используемый для локализации названий языков: <ul><li>Сочетание кода языка и региональных параметров в формате ISO 639 2-Letter, связанного с языком, и кодом подкультуры в верхнем регистре ISO 3166 2. <li>Код языка и региональных параметров ISO 639 в нижнем регистре.|запрос|строка|
|Авторизация|(пусто)  |Требуется, если и поле `appid`, и заголовок `Ocp-Apim-Subscription-Key` оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.|Верхний колонтитул|строка|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если и поле `appid`, и заголовок `Authorization` оставлены пустыми.|Верхний колонтитул|строка|
...
|:--|:--|:--|:--|:--|
|appid|(пусто)|обязательный параметр. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле `appid` пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"`.|запрос|строка|
|text|(пусто)   |обязательный параметр. Строка, содержащая одно или несколько предложений для потока на указанном языке. Длина текста не должна превышать 2 000 символов.|запрос|строка|
|язык|(пусто)   |обязательный параметр. Строка, представляющая поддерживаемый код языка языка, на который следует поговорить с текстом. Код должен быть одним из кодов, возвращаемых методом `GetLanguagesForSpeak`.|запрос|строка|
|format|(пусто)|необязательный параметр. Строка, указывающая идентификатор типа содержимого. В настоящее время доступны `audio/wav` и `audio/mp3`. По умолчанию используется значение `audio/wav`.|запрос|строка|
|options|(пусто)    |необязательный параметр. Строка, указывающая свойства синтезированного речевого ввода:<ul><li>`MaxQuality` и `MinSize` укажите качество звукового сигнала. `MaxQuality` обеспечивает наивысший уровень качества. `MinSize` предоставляет наименьший размер файла. Значение по умолчанию — `MinSize`.</li><li>`female` и `male` указать желаемый Пол голоса. Значение по умолчанию — `female`. Используйте вертикальную черту (<code>\|</code>), чтобы включить несколько параметров. Например, `MaxQuality|Male`.</li></li></ul>  |запрос|строка|
|Авторизация|(пусто)|Требуется, если и поле `appid`, и заголовок `Ocp-Apim-Subscription-Key` оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.|Верхний колонтитул|строка|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если и поле `appid`, и заголовок `Authorization` оставлены пустыми.|Верхний колонтитул|строка|
...
|:--|:--|:--|:--|:--|
|appid|(пусто)  |обязательный параметр. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле `appid` пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"`.|запрос|строка|
|text|(пусто)|обязательный параметр. Строка, содержащая текст, язык которого должен быть идентифицирован. Длина текста не должна превышать 10 000 символов.|запрос|  строка|
|Авторизация|(пусто)|Требуется, если и поле `appid`, и заголовок `Ocp-Apim-Subscription-Key` оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.|Верхний колонтитул|строка|
|Ocp-Apim-Subscription-Key  |(пусто)    |Требуется, если и поле `appid`, и заголовок `Authorization` оставлены пустыми.|Верхний колонтитул|строка|
...
### <a name="response-class-status-200"></a>Класс Response (status 200)
`DetectArray` успешно выполнена. Возвращает массив строк, содержащий двухбайтовый код языка для каждой строки входного массива.
...
|:--|:--|:--|:--|:--|
|appid|(пусто)|обязательный параметр. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле `appid` пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"`.|запрос|строка|
|OriginalText|(пусто)|обязательный параметр. Строка, содержащая текст для преобразования. Максимальная длина строки — 1 000 символов.|запрос|строка|
|TranslatedText|(пусто) |обязательный параметр. Строка, содержащая текст, переведенный на целевой язык. Максимальная длина строки — 2 000 символов.|запрос|строка|
|from|(пусто)   |обязательный параметр. Строка, представляющая код языка исходного языка текста. Например, EN для английского языка и de для немецкого.|запрос|строка|
|значение|(пусто)|обязательный параметр. Строка, представляющая код языка для перевода текста в.|запрос|строка|
|рейтинг|(пусто) |необязательный параметр. Целое число, представляющее оценку качества строки. Значение находится в диапазоне от-10 до 10. Значение по умолчанию — 1.|запрос|целое число|
|сontentType|(пусто)    |необязательный параметр. Формат переведенного текста. Поддерживаемые форматы: `text/plain` и `text/html`. Все HTML-элементы должны быть правильно сформированными, полными элементами.    |запрос|строка|
|категория|(пусто)|необязательный параметр. Строка, содержащая категорию (домен) перевода. Значение по умолчанию — `general`.|запрос|строка|
|user|(пусто)|обязательный параметр. Строка, используемая для отслеживания инициатора отправки.|запрос|строка|
|URI|(пусто)|необязательный параметр. Строка, содержащая расположение содержимого перевода.|запрос|строка|
|Авторизация|(пусто)|Требуется, если и поле `appid`, и заголовок `Ocp-Apim-Subscription-Key` оставлены пустыми.  Маркер авторизации: `"Bearer" + " " + "access_token"`.  |Верхний колонтитул|строка|
|Ocp-Apim-Subscription-Key|(пусто)|Требуется, если и поле `appid`, и заголовок `Authorization` оставлены пустыми.|Верхний колонтитул|строка|
...
Добавляет массив переводов в память преобразования. Этот метод представляет собой версию массива `AddTranslation`.
...
* `AppId`: обязательный параметр. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле `AppId` пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"`.
* `From`: обязательный параметр. Строка, содержащая код языка исходного языка. Должен быть на одном из языков, возвращаемых методом `GetLanguagesForTranslate`.
* `To`: обязательный параметр. Строка, содержащая код языка целевого языка. Должен быть на одном из языков, возвращаемых методом `GetLanguagesForTranslate`.
* `Translations`: обязательный параметр. Массив переводов для добавления в память перевода. Каждый перевод должен содержать `OriginalText`, `TranslatedText`и `Rating`. Максимальный размер каждой `OriginalText` и `TranslatedText` составляет 1 000 символов. Сумма всех элементов `OriginalText` и `TranslatedText` не может превышать 10 000 символов. Максимальное число элементов массива — 100.
* `Options`: обязательный параметр. Набор параметров, включая `Category`, `ContentType`, `Uri`и `User`. требуется `User`. `Category`, `ContentType`и `Uri` являются необязательными. Указанные элементы должны находится в алфавитном порядке.
...
|:--|:--|:--|:--|:--|
|appid|(пусто)  |обязательный параметр. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле `appid` пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"`.|запрос| строка|
|text|(пусто)   |обязательный параметр. Строка, представляющая текст, который необходимо разделить на предложения. Максимальный размер текста составляет 10 000 символов.|запрос|строка|
|язык   |(пусто)    |обязательный параметр. Строка, представляющая код языка входного текста.|запрос|строка|
|Авторизация|(пусто)|Требуется, если и поле `appid`, и заголовок `Ocp-Apim-Subscription-Key` оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.   |Верхний колонтитул|строка|
|Ocp-Apim-Subscription-Key|(пусто)|Требуется, если и поле `appid`, и заголовок `Authorization` оставлены пустыми.|Верхний колонтитул|строка|
...
Объект `TranslateOptions` содержит значения из следующего списка. Все они являются необязательными и по умолчанию используются наиболее распространенными параметрами. Указанные элементы должны находится в алфавитном порядке.

* `Category`: строка, содержащая категорию (домен) перевода. Значение по умолчанию — `general`.
* `ContentType`: единственный поддерживаемый параметр и значение по умолчанию — `text/plain`.
* `IncludeMultipleMTAlternatives`: логический флаг, указывающий, следует ли возвратить из модуля MT более одной альтернативы. Допустимые значения: `true` и `false` (с учетом регистра). Значение по умолчанию — `false`, возвращающее только одно альтернативное. Установка флага в `true` позволяет создавать искусственные альтернативы, полностью интегрированные с инфраструктурой совместного преобразования (КТФ). Эта функция позволяет возвращать альтернативы для предложений, которые не имеют переводов в КТФ, добавляя искусственные альтернативы из *n*-лучшего списка декодера.
...
* `Translations`: массив найденных совпадений, хранящийся в `TranslationMatch` объектов (описывается в следующем разделе). Переводы могут включать в себя небольшие варианты исходного текста (нечеткие совпадения). Переводы будут отсортированы: 100% совпадений первыми, нечеткие соответствия Далее.
...
|:--|:--|:--|:--|:--|
|appid|(пусто)|обязательный параметр. Если используется заголовок `Authorization` или `Ocp-Apim-Subscription-Key`, оставьте поле `appid` пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"`.|запрос|строка|
|text|(пусто)|обязательный параметр. Строка, представляющая текст для преобразования. Максимальный размер текста составляет 10 000 символов.|запрос|строка|
|from|(пусто)|обязательный параметр. Строка, представляющая код языка для преобразуемого текста.|запрос|строка|
|значение |(пусто)    |обязательный параметр. Строка, представляющая код языка для перевода текста в.|запрос|строка|
|maxTranslations|(пусто)|обязательный параметр. Целое число, представляющее максимальное число возвращаемых переводов.|запрос|целое число|
|Авторизация| (пусто)|Требуется, если и поле `appid`, и заголовок `Ocp-Apim-Subscription-Key` оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.|строка|  Верхний колонтитул|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если и поле `appid`, и заголовок `Authorization` оставлены пустыми.|Верхний колонтитул|строка|
...
* `AppId`: обязательный параметр. Если используется заголовок `Authorization`, оставьте поле `AppId` пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"`.
* `From`: обязательный параметр. Строка, представляющая код языка для преобразуемого текста.
* `MaxTranslations`: обязательный параметр. Целое число, представляющее максимальное число возвращаемых переводов.
* `Options`: необязательный параметр. Объект `Options`, содержащий следующие значения. Все они являются необязательными и по умолчанию используются наиболее распространенными параметрами. Указанные элементы должны находится в алфавитном порядке.
    - `Category`: строка, содержащая категорию (домен) перевода. Значение по умолчанию — `general`.
    - `ContentType`: единственный поддерживаемый параметр и значение по умолчанию — `text/plain`.
    - `IncludeMultipleMTAlternatives`: логический флаг, указывающий, следует ли возвратить из модуля MT более одной альтернативы. Допустимые значения: `true` и `false` (с учетом регистра). Значение по умолчанию — `false`, возвращающее только одно альтернативное. Установка флага в `true` позволяет создавать искусственные альтернативы в переводе, полностью интегрированные с инфраструктурой совместного преобразования (КТФ). Эта функция позволяет возвращать альтернативы для предложений, которые не имеют альтернативных вариантов в КТФ, добавляя искусственные альтернативы из *n*-лучшего списка декодера.
...
* `Uri`: отфильтруйте результаты по данному URI. Если значение не задано, по умолчанию используется `all`.
* `User`: отфильтруйте результаты по данному пользователю. Если значение не задано, по умолчанию используется `all`.
* `Texts`: обязательный параметр. Массив, содержащий текст для перевода. Все строки должны быть на одном языке. Общий объем преобразуемого текста не должен превышать 10 000 символов. Максимальное число элементов массива — 10.
* `To`: обязательный параметр. Строка, представляющая код языка для перевода текста в.
