---
title: Переводчик версии 2.0
titleSuffix: Azure Cognitive Services
description: Справочная документация по Translator 2.0.
...
ms.locfileid: "83588621"
---
# <a name="translator-v20"></a>Переводчик версии 2.0

> [!IMPORTANT]
> Эта версия транслятора является устаревшей. [Просмотрите документацию по версии 3 транслятора](v3-0-reference.md).

Версию 2 переводчика можно легко интегрировать в ваши приложения, веб-сайты, средства или другие решения, чтобы обеспечить многоязыковую поддержку пользователей. Его можно использовать на любой аппаратной платформе и с любой операционной системой для перевода языка и других задач, связанных с языком, таких как распознавание текста и преобразование текста в речь, согласно отраслевым стандартам. Дополнительные сведения см. в разделе [переводчик](../translator-info-overview.md).

## <a name="getting-started"></a>Начало работы
Чтобы получить доступ к переводчику, необходимо [зарегистрироваться в Microsoft Azure](../translator-text-how-to-signup.md).

## <a name="authentication"></a>Аутентификация 
Для всех вызовов транслятора требуется ключ подписки для проверки подлинности. API поддерживает три метода проверки подлинности:

- Маркер доступа. Используйте ключ подписки, чтобы создать маркер доступа, выполнив запрос POST к службе проверки подлинности. Дополнительные сведения см. в документации службы токенов. Передайте маркер доступа службе переводчика с помощью `Authorization` заголовка или `access_token` параметра запроса. Маркер доступа действителен 10 минут. Получите новый маркер доступа каждые 10 минут и используйте один и тот же маркер доступа для повторяющихся запросов в течение 10 минут.
- Ключ подписки, используемый напрямую. Передайте ключ подписки в качестве значения в `Ocp-Apim-Subscription-Key` заголовке, входящем в запрос транслятора. Если вы используете ключ подписки напрямую, вам не нужно вызывать службу проверки подлинности маркеров для создания маркера доступа.
...
|ProfanityAction    |Действие |Пример исходного кода (японский)  |Пример перевода (английский)  |
|:--|:--|:--|:--|
...
|Marked     |Ненормативные слова будут окружены \< ненормативными XML-тегами> и \< /профанити>.       |彼はジャッカスです。 |Это \< ненормативная лексика>жаккасс \< /профанити>.  |
|Удаленная    |Оскорбительные слова будут удалены из выходных данных без замены.     |彼はジャッカスです。 |Он — .   |

    
## <a name="excluding-content-from-translation"></a>Исключение содержимого из перевода
При переводе содержимого с помощью тегов, например HTML ( `contentType=text/html` ), иногда бывает полезно исключить конкретное содержимое из перевода. С помощью атрибута `class=notranslate` можно указать содержимое, которое должно остаться не переведенным. В следующем примере содержимое первого `div` элемента не будет переведено, но содержимое второго `div` элемента будет переведено.
...
|:--|:--|:--|:--|:--|
|appid  |(пусто)    |Обязательный. Если `Authorization` `Ocp-Apim-Subscription-Key` используется заголовок или, оставьте `appid` поле пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"` .|query|строка|
|текст|(пусто)   |Обязательный. Строка, представляющая текст для преобразования. Текст не может содержать более 10 000 символов.|query|строка|
|из|(пусто)   |Необязательный элемент. Строка, представляющая код языка для преобразуемого текста. Например, en для английского языка.|query|строка|
|в|(пусто) |Обязательный. Строка, представляющая код языка, в который преобразуется текст.|query|строка|
|сontentType|(пусто)    |Необязательный элемент. Формат переведенного текста. Поддерживаются форматы `text/plain` (по умолчанию) и `text/html` . Все HTML-элементы должны быть правильно сформированными, полными элементами.|query|строка|
|категория|(пусто)   |Необязательный элемент. Строка, содержащая категорию (домен) перевода. Значение по умолчанию — `general`.|query|строка|
|Авторизация|(пусто)  |Требуется, если оба `appid` поля и `Ocp-Apim-Subscription-Key` заголовок оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.|заголовок|строка|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если оба `appid` поля и `Authorization` заголовок оставлены пустыми.|заголовок|строка|
...
* `AppId`: обязательный. Если `Authorization` `Ocp-Apim-Subscription-Key` используется заголовок или, оставьте `AppId` поле пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"` .
* `From`: Необязательный параметр. Строка, представляющая код языка для преобразуемого текста. Если оставить это поле пустым, ответ будет содержать результат автоматического определения языка.
* `Options`: Необязательный параметр. `Options`Объект, содержащий следующие значения. Все они являются необязательными и по умолчанию используются наиболее распространенными параметрами. Указанные элементы должны находится в алфавитном порядке.
    ...
    - `Uri`: отфильтруйте результаты по данному URI. Значение по умолчанию: `all`.
    - `User`: отфильтруйте результаты по данному пользователю. Значение по умолчанию: `all`.
* `Texts`: обязательный. Массив, содержащий текст для перевода. Все строки должны быть на одном языке. Общий объем преобразуемого текста не должен превышать 10 000 символов. Максимальное число элементов массива — 2 000.
* `To`: обязательный. Строка, представляющая код языка, в который преобразуется текст.

Можно опустить необязательные элементы. Элементы, являющиеся прямыми дочерними элементами, `TranslateArrayRequest` должны быть перечислены в алфавитном порядке.

`TranslateArray`Метод принимает `application/xml` или `text/xml` для `Content-Type` .

**Возвращаемое значение:** массив `TranslateArrayResponse`. Каждый из них `TranslateArrayResponse` имеет следующие элементы:
...
### <a name="response-class-status-200"></a>Класс Response (status 200)
Успешный ответ включает массив `TranslateArrayResponse` массивов в формате, описанном ранее.
...
### <a name="implementation-notes"></a>Примечания по реализации
Получает понятные имена для языков, переданных в качестве параметра `languageCodes` , локализованного в переданный `locale` язык.

Код URI запроса — `https://api.microsofttranslator.com/V2/Http.svc/GetLanguageNames`.

Текст запроса включает массив строк, который представляет коды языка ISO 639-1, для которых нужно получить понятные имена. Приведем пример:
...
|:--|:--|:--|:--|:--|
|appid|(пусто)|Обязательный. Если `Authorization` `Ocp-Apim-Subscription-Key` используется заголовок или, оставьте `appid` поле пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"` .|query|строка|
|текст|(пусто)   |Обязательный. Строка, содержащая одно или несколько предложений для потока на указанном языке. Длина текста не должна превышать 2 000 символов.|query|строка|
|Язык|(пусто)   |Обязательный. Строка, представляющая поддерживаемый код языка языка, на который следует поговорить с текстом. Код должен быть одним из кодов, возвращаемых методом `GetLanguagesForSpeak` .|query|строка|
|format|(пусто)|Необязательный элемент. Строка, указывающая идентификатор типа содержимого. В настоящее время доступны `audio/wav` и `audio/mp3`. Значение по умолчанию — `audio/wav`.|query|строка|
|options|(пусто)    |Необязательный элемент. Строка, указывающая свойства синтезированного речевого ввода:<ul><li>`MaxQuality`и `MinSize` Укажите качество звукового сигнала. `MaxQuality`обеспечивает наивысший уровень качества. `MinSize`предоставляет наименьший размер файла. Значение по умолчанию — `MinSize` .</li><li>`female`и `male` Укажите желаемый род голоса. Значение по умолчанию — `female`. Используйте вертикальную черту ( <code>\|</code> ), чтобы включить несколько параметров. Например, `MaxQuality|Male`.</li></li></ul>  |query|строка|
|Авторизация|(пусто)|Требуется, если оба `appid` поля и `Ocp-Apim-Subscription-Key` заголовок оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.|заголовок|строка|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если оба `appid` поля и `Authorization` заголовок оставлены пустыми.|заголовок|строка|
...
|:--|:--|:--|:--|:--|
|appid|(пусто)  |Обязательный. Если `Authorization` `Ocp-Apim-Subscription-Key` используется заголовок или, оставьте `appid` поле пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"` .|query|строка|
|текст|(пусто)|Обязательный. Строка, содержащая текст, язык которого должен быть идентифицирован. Длина текста не должна превышать 10 000 символов.|query|  строка|
|Авторизация|(пусто)|Требуется, если оба `appid` поля и `Ocp-Apim-Subscription-Key` заголовок оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.|заголовок|строка|
|Ocp-Apim-Subscription-Key  |(пусто)    |Требуется, если оба `appid` поля и `Authorization` заголовок оставлены пустыми.|заголовок|строка|
...
|:--|:--|:--|:--|:--|
|appid|(пусто)|Обязательный. Если `Authorization` `Ocp-Apim-Subscription-Key` используется заголовок или, оставьте `appid` поле пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"` .|query|строка|
|OriginalText|(пусто)|Обязательный. Строка, содержащая текст для преобразования. Максимальная длина строки — 1 000 символов.|query|строка|
|TranslatedText|(пусто) |Обязательный. Строка, содержащая текст, переведенный на целевой язык. Максимальная длина строки — 2 000 символов.|query|строка|
|из|(пусто)   |Обязательный. Строка, представляющая код языка исходного языка текста. Например, EN для английского языка и de для немецкого.|query|строка|
|в|(пусто)|Обязательный. Строка, представляющая код языка для перевода текста в.|query|строка|
|рейтинг|(пусто) |Необязательный элемент. Целое число, представляющее оценку качества строки. Значение находится в диапазоне от-10 до 10. Значение по умолчанию — 1.|query|целочисленный|
...
|категория|(пусто)|Необязательный элемент. Строка, содержащая категорию (домен) перевода. Значение по умолчанию — `general`.|query|строка|
|пользователь|(пусто)|Обязательный. Строка, используемая для отслеживания инициатора отправки.|query|строка|
...
* `AppId`: обязательный. Если `Authorization` `Ocp-Apim-Subscription-Key` используется заголовок или, оставьте `AppId` поле пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"` .
* `From`: обязательный. Строка, содержащая код языка исходного языка. Должен быть на одном из языков, возвращаемых методом `GetLanguagesForTranslate`.
* `To`: обязательный. Строка, содержащая код языка целевого языка. Должен быть на одном из языков, возвращаемых методом `GetLanguagesForTranslate`.
* `Translations`: обязательный. Массив переводов для добавления в память перевода. Каждый перевод должен содержать `OriginalText` , `TranslatedText` и `Rating` . Максимальный размер каждого из них `OriginalText` `TranslatedText` — 1 000 символов. Сумма всех `OriginalText` `TranslatedText` элементов и не может превышать 10 000 символов. Максимальное число элементов массива — 100.
* `Options`: обязательный. Набор параметров, включая `Category` , `ContentType` , `Uri` и `User` . `User` является обязательным. `Category`, `ContentType` и `Uri` являются необязательными. Указанные элементы должны находится в алфавитном порядке.

### <a name="response-class-status-200"></a>Класс Response (status 200)
`AddTranslationArray`метод прошел. 
...
|:--|:--|:--|:--|:--|
|appid|(пусто)  |Обязательный. Если `Authorization` `Ocp-Apim-Subscription-Key` используется заголовок или, оставьте `appid` поле пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"` .|query| строка|
|текст|(пусто)   |Обязательный. Строка, представляющая текст, который необходимо разделить на предложения. Максимальный размер текста составляет 10 000 символов.|query|строка|
|Язык   |(пусто)    |Обязательный. Строка, представляющая код языка входного текста.|query|строка|
|Авторизация|(пусто)|Требуется, если оба `appid` поля и `Ocp-Apim-Subscription-Key` заголовок оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.   |заголовок|строка|
|Ocp-Apim-Subscription-Key|(пусто)|Требуется, если оба `appid` поля и `Authorization` заголовок оставлены пустыми.|заголовок|строка|
...
### <a name="implementation-notes"></a>Примечания по реализации
Извлекает массив переводов для данной языковой пары из хранилища и системы машинного перевода. `GetTranslations`отличается от `Translate` в том, что он возвращает все доступные переводы.

Код URI запроса — `https://api.microsofttranslator.com/V2/Http.svc/GetTranslations`.

Текст запроса включает необязательный `TranslationOptions` объект, имеющий следующий формат:
...
`TranslateOptions`Объект содержит значения, перечисленные в следующем списке. Все они являются необязательными и по умолчанию используются наиболее распространенными параметрами. Указанные элементы должны находится в алфавитном порядке.

* `Category`: Строка, содержащая категорию (домен) перевода. Значение по умолчанию — `general`.
* `ContentType`: Единственный поддерживаемый параметр и значение по умолчанию — `text/plain` .
* `IncludeMultipleMTAlternatives`: Логический флаг, указывающий, следует ли возвратить из модуля MT более одной альтернативы. Допустимые значения: `true` и `false` (с учетом регистра). Значение по умолчанию — `false` , возвращающее только одно альтернативное. Установка флага для `true` включения возможности создания искусственных альтернатив, полностью интегрированных с инфраструктурой совместного преобразования (КТФ). Эта функция позволяет возвращать альтернативы для предложений, которые не имеют переводов в КТФ, добавляя искусственные альтернативы из *n*-лучшего списка декодера.
    - Тока. Эти оценки применяются следующим образом: 
       ...
       - Автоматически создаваемые (*n*наилучшие) варианты перевода имеют оценку 0 и степень соответствия 100.
    - Число альтернатив. Число возвращаемых альтернатив может быть таким же, как и значение, указанное в параметре `maxTranslations` , но может быть ниже.
...
* `State`: Пользовательское состояние, помогающее сопоставить запрос и ответ. В ответе будет возвращено то же содержимое.
* `Uri`: отфильтруйте результаты по данному URI. Если значение не задано, по умолчанию используется `all` .
* `User`: отфильтруйте результаты по данному пользователю. Если значение не задано, по умолчанию используется `all` .
...
* `Translations`: Массив найденных совпадений, хранящийся в `TranslationMatch` объектах (описанных в следующем разделе). Переводы могут включать в себя небольшие варианты исходного текста (нечеткие совпадения). Переводы будут отсортированы: 100% совпадений первыми, нечеткие соответствия Далее.
* `From`: Если метод не задает `From` язык, это значение будет получено из автоматического определения языка. В противном случае это будет указанный `From` язык.
* `State`: Пользовательское состояние, помогающее сопоставить запрос и ответ. Содержит значение, заданное в `TranslateOptions` параметре.

`TranslationMatch`Объект состоит из следующих значений:
...
### <a name="response-class-status-200"></a>Класс Response (status 200)
`GetTranslationsResponse`Объект в формате, описанном ранее.
...
|:--|:--|:--|:--|:--|
|appid|(пусто)|Обязательный. Если `Authorization` `Ocp-Apim-Subscription-Key` используется заголовок или, оставьте `appid` поле пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"` .|query|строка|
|текст|(пусто)|Обязательный. Строка, представляющая текст для преобразования. Максимальный размер текста составляет 10 000 символов.|query|строка|
|из|(пусто)|Обязательный. Строка, представляющая код языка для преобразуемого текста.|query|строка|
|в |(пусто)    |Обязательный. Строка, представляющая код языка для перевода текста в.|query|строка|
|maxTranslations|(пусто)|Обязательный. Целое число, представляющее максимальное число возвращаемых переводов.|query|целочисленный|
|Авторизация| (пусто)|Требуется, если оба `appid` поля и `Ocp-Apim-Subscription-Key` заголовок оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.|строка|  заголовок|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если оба `appid` поля и `Authorization` заголовок оставлены пустыми.|заголовок|строка|
...
* `AppId`: обязательный. Если `Authorization` используется заголовок, оставьте `AppId` поле пустым. В противном случае включите строку, содержащую `"Bearer" + " " + "access_token"` .
* `From`: обязательный. Строка, представляющая код языка для преобразуемого текста.
* `MaxTranslations`: обязательный. Целое число, представляющее максимальное число возвращаемых переводов.
* `Options`: Необязательный параметр. `Options`Объект, содержащий следующие значения. Все они являются необязательными и по умолчанию используются наиболее распространенными параметрами. Указанные элементы должны находится в алфавитном порядке.
    - `Category`: Строка, содержащая категорию (домен) перевода. Значение по умолчанию — `general`.
    - `ContentType`: Единственный поддерживаемый параметр и значение по умолчанию — `text/plain` .
    - `IncludeMultipleMTAlternatives`: Логический флаг, указывающий, следует ли возвратить из модуля MT более одной альтернативы. Допустимые значения: `true` и `false` (с учетом регистра). Значение по умолчанию — `false` , возвращающее только одно альтернативное. Установка флага для `true` включения создания искусственных альтернатив в переводе, полностью интегрированного с платформой совместного преобразования (КТФ). Эта функция позволяет возвращать альтернативы для предложений, которые не имеют альтернативных вариантов в КТФ, добавляя искусственные альтернативы из *n*-лучшего списка декодера.
...
* `Uri`: отфильтруйте результаты по данному URI. Если значение не задано, по умолчанию используется `all` .
* `User`: отфильтруйте результаты по данному пользователю. Если значение не задано, по умолчанию используется `all` .
* `Texts`: обязательный. Массив, содержащий текст для перевода. Все строки должны быть на одном языке. Общий объем преобразуемого текста не должен превышать 10 000 символов. Максимальное число элементов массива — 10.
* `To`: обязательный. Строка, представляющая код языка для перевода текста в.

Можно опустить необязательные элементы. Элементы, являющиеся прямыми дочерними элементами, `GetTranslationsArrayRequest` должны быть перечислены в алфавитном порядке.
...
> [Миграция на Translator v3](../migrate-to-v3.md)
