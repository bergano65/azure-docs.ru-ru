...
|значение|(пусто) |Обязательный элемент. Строка, представляющая код языка, на который переводится текст.|query|string|
...
|языковой стандарт|(пусто) |Обязательный элемент. Строка, представляющая комбинацию двухбуквенного кода ISO 639 с нижним регистром, связанного с языком, и двухбуквенного кода ISO 3166 с верхним регистром для локализации имен языков или же просто код ISO 639.|query|string|
|Авторизация|(пусто)  |Требуется, если поле appid или заголовок `Ocp-Apim-Subscription-Key` не указаны. Маркер авторизации: `"Bearer" + " " + "access_token"`.|Верхний колонтитул|string|
...
|Язык|(пусто)   |Обязательный элемент. Строка, представляющая код языка, на котором произносится текст. Код должен присутствовать в списке кодов, возвращаемых методом `GetLanguagesForSpeak`.|query|string|
...
|Язык   |(пусто)    |Обязательный элемент. Строка, представляющая код языка.|query|string|
|Авторизация|(пусто)|Требуется, если поле appid или заголовок Ocp-Apim-Subscription-Key не указаны. Маркер авторизации.  "Bearer" + " " + "access_token".    |Верхний колонтитул|string|
|Ocp-Apim-Subscription-Key|(пусто)|Требуется, если поле appid или заголовок авторизации не указаны.|Верхний колонтитул|string|
