...
- Маркер доступа. Используйте ключ подписки, чтобы создать маркер доступа, выполнив запрос POST к службе проверки подлинности. Дополнительные сведения см. в документации службы токенов. Передайте маркер доступа службе переводчика с помощью `Authorization` заголовка или параметра `access_token` запроса. Маркер доступа действителен 10 минут. Получите новый маркер доступа каждые 10 минут и используйте один и тот же маркер доступа для повторяющихся запросов в течение 10 минут.
- Ключ подписки, используемый напрямую. Передайте ключ подписки в качестве значения в `Ocp-Apim-Subscription-Key` заголовке, включаемом в запрос API перевода текстов. Если вы используете ключ подписки напрямую, вам не нужно вызывать службу проверки подлинности маркеров для создания маркера доступа.
...
|Параметр|Применение|Описание    |Тип параметра|тип данных|
|:--|:--|:--|:--|:--|
|appid  |(пусто)    |Обязательный элемент. Если используется `Authorization` заголовок `Ocp-Apim-Subscription-Key` или, оставьте `appid` поле пустым. В противном случае включите строку, `"Bearer" + " " + "access_token"`содержащую.|query|строка|
...
|сontentType|(пусто)    |Необязательный параметр. Формат переведенного текста. Поддерживаются форматы `text/plain` (по умолчанию `text/html`) и. Все HTML-элементы должны быть правильно сформированными, полными элементами.|query|строка|
...
|Авторизация|(пусто)  |Требуется, если оба `appid` поля и `Ocp-Apim-Subscription-Key` заголовок оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.|Верхний колонтитул|строка|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если оба `appid` поля и `Authorization` заголовок оставлены пустыми.|Верхний колонтитул|строка|
...
|500    |Ошибка сервера. Свяжитесь с нами, если ошибка не устранена. Укажите приблизительную дату & время запроса и идентификатор запроса, содержащийся в заголовке `X-MS-Trans-Info`ответа.|
...
* `AppId`: обязательный. Если используется `Authorization` заголовок `Ocp-Apim-Subscription-Key` или, оставьте `AppId` поле пустым. В противном случае включите строку, `"Bearer" + " " + "access_token"`содержащую.
...
* `Options`: Необязательный параметр. `Options` Объект, содержащий следующие значения. Все они являются необязательными и по умолчанию используются наиболее распространенными параметрами. Указанные элементы должны находится в алфавитном порядке.
    ...
    - `ProfanityAction`: Указывает, как обрабатываются ненормативные лексики, как описано выше. Допустимые значения `NoAction` : (по умолчанию `Marked`) `Deleted`, и.
...
`TranslateArray` Метод `application/xml` принимает или `text/xml` для `Content-Type`.

**Возвращаемое значение:** массив `TranslateArrayResponse`. Каждый `TranslateArrayResponse` из них имеет следующие элементы:
...
### <a name="implementation-notes"></a>Примечания по реализации
Получает понятные имена для языков, переданных в качестве `languageCodes`параметра, локализованного в `locale` переданный язык.

Код URI запроса — `https://api.microsofttranslator.com/V2/Http.svc/GetLanguageNames`.

Текст запроса включает массив строк, который представляет коды языка ISO 639-1, для которых нужно получить понятные имена. Пример:
...
* `Translations`: обязательный. Массив переводов для добавления в память перевода. Каждый перевод должен содержать `OriginalText`, `TranslatedText`и `Rating`. Максимальный размер каждого из них `OriginalText` `TranslatedText` — 1 000 символов. Сумма всех `OriginalText` элементов и `TranslatedText` не может превышать 10 000 символов. Максимальное число элементов массива — 100.
* `Options`: обязательный. Набор параметров, `Category`включая, `ContentType`, `Uri`и. `User` `User` является обязательным. `Category`, `ContentType`и `Uri` являются необязательными. Указанные элементы должны находится в алфавитном порядке.
...
* `IncludeMultipleMTAlternatives`: Логический флаг, указывающий, следует ли возвратить из модуля MT более одной альтернативы. Допустимые значения `true` : `false` и (с учетом регистра). Значение по умолчанию — `false`, возвращающее только одно альтернативное. Установка флага для `true` включения возможности создания искусственных альтернатив, полностью интегрированных с инфраструктурой совместного преобразования (КТФ). Эта функция позволяет возвращать альтернативы для предложений, которые не имеют переводов в КТФ, добавляя искусственные альтернативы из *n*-лучшего списка декодера.
    ...
    - Число альтернатив. Число возвращаемых альтернатив может быть таким же, как и значение, указанное `maxTranslations`в параметре, но может быть ниже.
...
* `State`: Пользовательское состояние, помогающее сопоставить запрос и ответ. В ответе будет возвращено то же содержимое.
* `Uri`: отфильтруйте результаты по данному URI. Если значение не задано, по умолчанию `all`используется.
* `User`: отфильтруйте результаты по данному пользователю. Если значение не задано, по умолчанию `all`используется.
...
|Авторизация| (пусто)|Требуется, если оба `appid` поля и `Ocp-Apim-Subscription-Key` заголовок оставлены пустыми. Маркер авторизации: `"Bearer" + " " + "access_token"`.|строка|  Верхний колонтитул|
|Ocp-Apim-Subscription-Key|(пусто)  |Требуется, если оба `appid` поля и `Authorization` заголовок оставлены пустыми.|Верхний колонтитул|строка|
...
* `AppId`: обязательный. Если используется `Authorization` заголовок, оставьте `AppId` поле пустым. В противном случае включите строку, `"Bearer" + " " + "access_token"`содержащую.
