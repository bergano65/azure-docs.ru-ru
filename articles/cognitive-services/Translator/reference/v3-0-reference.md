...
Microsoft Translator обслуживается несколькими центрами обработки данных. В настоящее время находятся в 10 [географических расположений Azure](https://azure.microsoft.com/global-infrastructure/regions):

* **Северная и Южная Америка:** Восточная часть США, Юго-центральный регион США, западно-Центральная часть США и Западная часть США 2 
...
|:--|:--|:--|
|Azure|Global (без привязки к региону)|   api.cognitive.microsofttranslator.com|
...
| 400050| Входной текст является слишком длинным. Представление [ограничения запросов](../request-limits.md).|
...
| 408001| Подготавливается запрошенный система перевода. Повторите попытку через несколько минут.|
| 408002| Запрос, истекло время ожидания на входящем потоке. Клиент не создал запрос в течение времени ожидания подготовленного сервера. Клиент может повторить запрос без внесения изменений в любой момент времени.|
