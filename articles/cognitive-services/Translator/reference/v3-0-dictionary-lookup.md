...
description: Метод поиска по словарю предоставляет альтернативные переводы для слова и небольшое количество идиоматическим фраз.
...
<table width="100%">
  ...
  <tr>
    <td>from</td>
    <td>*Обязательный параметр*.<br/>Определяет язык оригинального текста. Исходный язык должен быть одним из [поддерживаемых языков](./v3-0-languages.md), включенных в область `dictionary`.</td>
  </tr>
  <tr>
    <td>значение</td>
...
<table width="100%">
  ...
  <tr>
    ...
    <td>*Необязательно*.<br/>Созданный клиентом идентификатор GUID, позволяющий уникально идентифицировать запрос. Этот заголовок можно опустить, если в строке запроса указан идентификатор трассировки в параметре с именем `ClientTraceId`.</td>
  </tr>
</table> 
