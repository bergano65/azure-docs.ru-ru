...
Давайте создадим функцию для определения длины предложения. Эта функция будет принимать один аргумент — ключ подписки Переводчика.
...
Теперь давайте создадим URL-адрес. URL-адрес создается с использованием методов `Parse()` и `Query()`. При этом параметры добавляются с помощью метода `Add()`.

Скопируйте следующий код в функцию `breakSentence`.
...
>[!NOTE]
> Дополнительные сведения о конечных точках, маршрутах и параметрах запросов см. в руководстве по [Переводчику 3.0. BreakSentence](https://docs.microsoft.com/azure/cognitive-services/translator/reference/v3-0-break-sentence).
