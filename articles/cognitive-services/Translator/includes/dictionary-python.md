...
Конечная точка Переводчика задается как `endpoint`. Параметр `path` задает маршрут `dictionary/lookup` и определяет, что нужно использовать версию 3 API.
