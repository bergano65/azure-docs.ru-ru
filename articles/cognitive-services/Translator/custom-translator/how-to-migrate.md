...
> [!Note]
> Для проведения обучения пользовательскому переводчику требуется как минимум 10 000 уникальных извлеченных предложений. Настраиваемый переводчик не может выполнить обучение меньше рекомендуемого [минимума](sentence-alignment.md#suggested-minimum-number-of-extracted-and-aligned-sentences).
...
4. На следующей странице выберите любой из этих двух параметров:
    ...
    1\. Deployed Trainings only (Только развернутые обучения). Этот вариант переносит только развернутые учебные системы и связанные с ними документы.
...
|   | Центр | Пользовательский переводчик |
|:-----|:----:|:----:|
