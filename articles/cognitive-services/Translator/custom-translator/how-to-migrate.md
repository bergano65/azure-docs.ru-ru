...
* Имена всех документов и проектов будут перенесены с добавлением префикса "hub_" в имя. Автоматически созданные тесты и данные для настройки получают имена hub_systemtune_\<modelid> или hub_systemtest_\<modelid>.
...
* В любой момент БЛЕУ оценку, перенесенное из центра обучения, можно найти на странице Траинингдетаилс модели в заголовке "Оценка Блеу в концентраторе MT".
...
3. Щелкните ссылку "Перенос" для соответствующего проекта.
    ...
    ![Снимок экрана, на котором выделена кнопка "миграция" для выбранного проекта.](media/how-to/how-to-migrate-from-hub.png)
...
3. На странице "Параметры" щелкните "Migrate Workspace data to Custom Translator" (Перенести данные рабочей области в Custom Translator).
    ...
    ![Снимок экрана, посвященный параметру переноса данных рабочей области в пользовательский переводчик.](media/how-to/how-to-migrate-workspace-from-hub.png)
...
После того как вы запросили миграцию рабочей области или проекта из центра, вы найдете журнал миграции на странице пользовательские параметры переводчика.
