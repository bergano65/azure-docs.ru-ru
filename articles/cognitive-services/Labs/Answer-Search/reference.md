...
-  `q=<URL>` -Запрос, который обозначает объект поиска
...
|-------------|-----------------|----------|  
|<a name="error-code" />code|Код ошибки, определяющий ее категорию. Список возможных кодов см. в разделе [Коды ошибок](#error-codes).|String|  
|<a name="error-message" />message|Текстовое описание ошибки.|String|  
|<a name="error-moredetails" />moreDetails|Подробное описание, которое предоставляет дополнительную информацию об ошибке.|String|  
|<a name="error-parameter" />parameter|Параметр запроса, который вызвал эту ошибку.|String|  
|<a name="error-subcode" />subCode|Код ошибки, по которому определяется ошибка. Например, если `code` имеет значение InvalidRequest, то `subCode` может иметь значение ParameterInvalid или ParameterInvalidValue. |String|  
|<a name="error-value" />value|Недопустимое значение параметра запроса.|String|  
...
|licenseNotice|Лицензия для отображения рядом с целевым полем. Например, "Текст в рамках лицензии CC-BY-SA".<br /><br /> Используйте имя и URL-адрес лицензии в поле `license`, чтобы создать гиперссылки на веб-сайт со сведениями о лицензии. Замените имя лицензии в строке `licenseNotice` (например, CC-BY-SA) гиперссылкой, которая была только что создана.|String|  
|mustBeCloseToContent|Логическое значение, определяющее, следует ли содержимое правила поместить рядом с полем, к которому оно применяется. Если возвращается значение **true**, содержимое нужно расположить в непосредственной близости. Если возвращается значение **false** или этого поля не существует, содержимое может размещаться по усмотрению вызывающего.|Boolean|  
|targetPropertyName|Имя поля, к которому применяется правило.|String|  
...
|url|URL-адрес. Используйте URL-адрес и отображаемый текст для создания гиперссылки.|String|  
...
|ИМЯ|Value|type|  
|----------|-----------|----------|  
|_type|Указание типа, которое имеет значение MediaAttribution.|String|  
|mustBeCloseToContent|Логическое значение, определяющее, следует ли содержимое правила поместить рядом с полем, к которому оно применяется. Если возвращается значение **true**, содержимое нужно расположить в непосредственной близости. Если возвращается значение **false** или этого поля не существует, содержимое может размещаться по усмотрению вызывающего.|Boolean|  
|targetPropertyName|Имя поля, к которому применяется правило.|String|  
|url|URL-адрес, который используется для создания гиперссылки содержимого мультимедиа. Например, если целевой объект — это изображение, URL-адрес используется, чтобы сделать изображение интерактивными.|String|  
...
|ИМЯ|Value|type|  
|----------|-----------|----------|  
|_type|Указание типа, которому задано значение SearchResponse.|String|  
|WebPage|Объект JSON, который определяет предварительную версию|string|  
