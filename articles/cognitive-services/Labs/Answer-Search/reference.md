...
-  `q=<URL>`— Запрос, определяющий объект поиска.
...
|Header|Описание|  
|------------|-----------------|  
|Принимаю|Дополнительный заголовок запроса.<br /><br /> Тип мультимедиа по умолчанию — это приложение или JSON. Чтобы указать, что в ответе используется [JSON-LD](https://json-ld.org/), задайте для заголовка Accept приложение или JSON+LD в качестве значения.|  
...
|-------------|-----------------|----------|  
|<a name="error-code" />code|Код ошибки, определяющий ее категорию. Список возможных кодов см. в разделе [Коды ошибок](#error-codes).|Строковое|  
|<a name="error-message" />message|Текстовое описание ошибки.|Строковое|  
|<a name="error-moredetails" />moreDetails|Подробное описание, которое предоставляет дополнительную информацию об ошибке.|Строковое|  
|<a name="error-parameter" />parameter|Параметр запроса, который вызвал эту ошибку.|Строковое|  
|<a name="error-subcode" />subCode|Код ошибки, по которому определяется ошибка. Например, если `code` имеет значение InvalidRequest, то `subCode` может иметь значение ParameterInvalid или ParameterInvalidValue. |Строковое|  
|<a name="error-value" />value|Недопустимое значение параметра запроса.|Строковое|  
...
|----------|-----------|----------|  
|name|Имя лицензии.|Строковое|  
|url|URL-адрес веб-сайта, где пользователь может получить дополнительные сведения о лицензии.<br /><br /> Используйте имя и URL-адрес для создания гиперссылки.|Строковое|  
...
|Имя|Значение|Type|  
|----------|-----------|----------|  
|_type|Указание типа, который имеет значение LicenseAttribution.|Строковое|  
|Лицензия|Лицензия, по которой может использоваться содержимое.|[Лицензия](#license)|  
|licenseNotice|Лицензия для отображения рядом с целевым полем. Например, "Текст в рамках лицензии CC-BY-SA".<br /><br /> Используйте имя и URL-адрес лицензии в поле `license`, чтобы создать гиперссылки на веб-сайт со сведениями о лицензии. Замените имя лицензии в строке `licenseNotice` (например, CC-BY-SA) гиперссылкой, которая была только что создана.|Строковое|  
|mustBeCloseToContent|Логическое значение, определяющее, следует ли содержимое правила поместить рядом с полем, к которому оно применяется. Если возвращается значение **true**, содержимое нужно расположить в непосредственной близости. Если возвращается значение **false** или этого поля не существует, содержимое может размещаться по усмотрению вызывающего.|логический|  
|targetPropertyName|Имя поля, к которому применяется правило.|Строковое|  
...
|url|URL-адрес. Используйте URL-адрес и отображаемый текст для создания гиперссылки.|Строковое|  
...
|Название|Значение|Тип|  
|----------|-----------|----------|  
|_type|Указание типа, которое имеет значение MediaAttribution.|Строковое|  
|mustBeCloseToContent|Логическое значение, определяющее, следует ли содержимое правила поместить рядом с полем, к которому оно применяется. Если возвращается значение **true**, содержимое нужно расположить в непосредственной близости. Если возвращается значение **false** или этого поля не существует, содержимое может размещаться по усмотрению вызывающего.|логический|  
|targetPropertyName|Имя поля, к которому применяется правило.|Строковое|  
|url|URL-адрес, который используется для создания гиперссылки содержимого мультимедиа. Например, если целевой объект — это изображение, URL-адрес используется, чтобы сделать изображение интерактивными.|Строковое|  
...
|Название|Значение|Type|  
|----------|-----------|----------|  
|name|Имя издателя.|Строковое|  
|url|URL-адрес для веб-сайта издателя.<br /><br /> Обратите внимание, что издатель может не предоставить веб-сайт.|Строковое|  
...
|Имя|Значение|Тип|  
|----------|-----------|----------|
|name|Заголовок страницы, не обязательно заголовок HTML|Строковое|
|url|URL-адрес, который фактически проверялся (запрос может обрабатываться с учетом перенаправлений)|Строковое|  
|description|Краткое описание страницы и содержимого|Строковое|  
|isFamilyFriendly|Наиболее точно для элементов в веб-индексе. В режиме реального времени это обнаружение основано исключительно на URL-адресе, а не на содержимом страницы.|Логический|
|primaryImageOfPage/contentUrl|URL-адрес примера изображения для включения в предварительный просмотр|Строковое| 
...
|Название|Значение|Type|  
|----------|-----------|----------|  
|_type|Указание типа, которому задано значение SearchResponse.|Строковое|  
