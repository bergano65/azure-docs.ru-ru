...
1. Перейдите в раздел **> преобразования речи в текст Пользовательское распознавание речи > обучения**.
2. Нажмите кнопку **обучение модели**.
3. Затем укажите **имя** и **Описание**для обучения.
4. В раскрывающемся меню **сценарий и базовая модель** выберите сценарий, который лучше подходит для вашего домена. Если вы не знаете, какой сценарий выбрать, выберите **Общие**. Базовая модель является отправной точкой для обучения. Если у вас нет предпочтений, вы можете использовать последнюю версию.
5. На странице **выбор обучающих данных** выберите один или несколько наборов данных для транскрипции с метками, которые вы хотите использовать для обучения.
6. После завершения обучения можно выполнить проверку точности для новой обученной модели. Этот шаг не является обязательным.
7. Выберите **создать** , чтобы создать пользовательскую модель.
