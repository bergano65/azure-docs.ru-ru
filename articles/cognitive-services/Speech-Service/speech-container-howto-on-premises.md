---
title: Использование с Kubernetes и Helm
...
| Helm CLI | Как часть [интерфейса командной строки][helm-install] install, you'll also need to initialize Helm, which will install [Tiller][tiller-install]Helm. |
...
| Service | ЦП или контейнер | Память/контейнер |
|--|--|--|
...
| **Преобразование текста в речь** | для одного одновременного запроса требуется минимум 500 миллиардах. `optimizeForTurboMode` Если включен, то требуется 1 000 миллиардах. (по умолчанию: два одновременных запроса) | Обязательно: 1 GB<br> Ограничены 2 ГБ |
