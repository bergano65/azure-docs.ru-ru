...
`certs` Проверьте наличие подкаталога в разделе опенсслдир. В приведенном выше примере это будет `/usr/lib/ssl/certs`.
...
* Если ОПЕНССЛДИР — это что- `/usr/lib/ssl` то еще и/или имеется отдельный файл пакета сертификатов, а не несколько отдельных файлов, необходимо задать соответствующую переменную среды SSL, чтобы указать, где можно найти сертификаты.

## <a name="examples"></a>Примеры

- ОПЕНССЛДИР — `/opt/ssl`. `certs` Существует подкаталог с множеством `.crt` файлов или `.pem` .
Задайте переменную `SSL_CERT_DIR` среды для указания `/opt/ssl/certs` перед запуском программы, ИСПОЛЬЗУЮЩЕЙ речевой пакет SDK. Пример:
```bash
export SSL_CERT_DIR=/opt/ssl/certs
```

- ОПЕНССЛДИР `/etc/pki/tls` (как и в системах на базе RHEL/CentOS). `certs` Например `ca-bundle.crt`, существует подкаталог с файлом пакета сертификатов.
Установите переменную `SSL_CERT_FILE` среды, чтобы она указывала на этот файл, перед запуском программы, использующей РЕЧЕВОЙ пакет SDK. Пример:
```bash
