...
|-----------|-------------|---------------------|
| `language` | Определяет распознаваемую устную речь. См. раздел [Поддерживаемые языки](language-support.md#speech-to-text). | Обязательное |
...
| `Content-type` | Описывает формат и кодек предоставленных аудиоданных. Допустимые значения: `audio/wav; codecs=audio/pcm; samplerate=16000` и `audio/ogg; codecs=opus`. | Обязательное |
...
| Параметр | Описание: | Обязательный или необязательный |
|-----------|-------------|---------------------|
| референцетекст | Текст, по которому будет оцениваться произношение. | Обязательное |
