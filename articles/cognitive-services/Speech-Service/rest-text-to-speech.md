...
> [!NOTE]
> Если у выбранного голоса и формата вывода будет разная скорость передачи звука, то при необходимости звук будет обработан повторно. Тем не менее, не поддерживают голоса 24 кГц `audio-16khz-16kbps-mono-siren` и `riff-16khz-16kbps-mono-siren` форматы выходных данных.
...
| 415 | Неподдерживаемый тип носителя | Возможно, неправильным `Content-Type` был предоставлен. `Content-Type` должно быть присвоено `application/ssml+xml`. |
| 429 | Слишком много запросов | Вы превысили квоту или частоту запросов, разрешенных для вашей подписки. |
| 502 | Недопустимый шлюз | Проблема с сетью или сервером. Может также указывать на недопустимые заголовки. |
