---
title: Создание пользовательской службы "Пробуждение Word — речь"
...
Прежде чем можно будет использовать пользовательское пробуждение с устройством, необходимо создать пробуждение с помощью службы автоматического пробуждения Microsoft Custom Word. После того как вы представите слово для пробуждения, служба создает файл, который развертывается в комплекте средств разработки, чтобы включить пробуждение слова на устройстве.

1. Перейдите на [портал службы пользовательское распознавание речи](https://aka.ms/sdsdk-speechportal) и **Войдите в систему** или если у вас нет подписки на речь, выберите [**создать подписку**](https://go.microsoft.com/fwlink/?linkid=2086754) .

    ![Портал службы Пользовательское распознавание речи](media/speech-devices-sdk/wake-word-4.png)

1. В текстовом слове [настраиваемого пробуждения](https://aka.ms/sdsdk-wakewordportal) по своему усмотрению выберите команду **Добавить пробуждение слова**. У нас есть некоторые [рекомендации](#choose-an-effective-wake-word) по выбору эффективного ключевого слова. В настоящее время поддерживается только язык EN-US.

    ![Ввод выбранного слова для активации](media/speech-devices-sdk/wake-word-5.png)

1. Будут созданы три варианта произношения слова "Пробуждение". Вы можете выбрать все произношение, которое вам нравится. Затем нажмите кнопку **Отправить** , чтобы создать слово пробуждения. Если вы хотите изменить слово "Пробуждение", сначала удалите его, если навести указатель мыши на строку произношения. появится значок удаления.

    ![Проверка слова "Пробуждение"](media/speech-devices-sdk/wake-word-6.png)

1. Создание модели может занять до одной минуты. Вам будет предложено загрузить файл.

    ![Скачивание слова "Пробуждение"](media/speech-devices-sdk/wake-word-7.png)

1. Сохраните ZIP-файл на компьютер. Этот файл потребуется для развертывания пользовательского текстового редактора для пробуждения в наборе средств разработки.

## <a name="next-steps"></a>Следующие шаги

Краткое руководство по тестированию пользовательского пробуждения с помощью [пакета SDK для речевых устройств](https://aka.ms/sdsdk-quickstart).
