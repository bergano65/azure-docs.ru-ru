...
---
# <a name="how-to-configure-a-microphone-array"></a>Настройка массива микрофонов
...
В Windows сведения о геометрических массивах микрофона автоматически получаются из аудио драйвера. Таким образом, свойства `DeviceGeometry` `SelectedGeometry`, и `MicArrayGeometryConfigFile` являются необязательными. Мы используем [файл JSON](https://aka.ms/sdsdk-micarray-json) , предоставленный `MicArrayGeometryConfigFile` с помощью, для получения диапазона беамформинг.
...
В настоящее время пакет SDK для речевых устройств поддерживает только [Рубо v1](speech-devices-sdk-android-quickstart.md) . Поведение аналогично предыдущему выпуску, `MicArrayGeometryConfigFile` за исключением того, что свойство Now можно использовать для указания JSON-файла, содержащего диапазон беамформинг.
