...
ms.locfileid: "85307744"
---
# <a name="set-up-web-endpoints"></a>Настройка конечных точек веб-сайта

Из этой статьи вы узнаете, как настроить конечные веб-точки в приложении настраиваемых команд, которое позволяет выполнять HTTP-запросы из клиентского приложения. Вы выполняете следующие задачи:
...
1. Перейдите в раздел "веб-конечные точки" и щелкните "создать конечную точку веб-сайта".
   ...
   | ------- | --------------- | ----------- |
   | Имя | упдатедевицестате | Имя для конечной веб-точки. |
   ...
   | Метод | ПОМЕСТИТЬ | Разрешенные взаимодействия (например, GET, POST) с конечной точкой.|
   | Заголовки | Ключ: App, Value: уникальное имя приложения. | Параметры заголовка для включения в заголовок запроса.|
...
1. В поле **Конфигурация**введите следующие значения: 
   ...
   | Содержимое текста | Недоступно | Содержимое текста запроса. |
