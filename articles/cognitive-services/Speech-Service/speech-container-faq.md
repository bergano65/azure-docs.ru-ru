...
<details>
<summary>
<b>Вы могли бы помочь в планировании ресурсов и оценке затрат на локальные речевые контейнеры?</b>
</summary>
...
Для оборудования мы используем стандартную виртуальную `DS13_v2` машину Azure в качестве справочника (каждая из ядер должна быть 2,6 ГГц или выше, с включенным набором инструкций AVX2).
...
- 1 конечная точка<sup>St</sup> , возвращающая речь. фрагменты `speech.hypothesis` сообщения вместо сообщений. 3 конечные точки компонента<sup>удаленных рабочих столов</sup> возвращаются для конечной точки диктофона.
- Все краткие руководства по использованию `RecognizeOnce` всех шаблонов (интерактивный режим)
- У углерода есть утверждение о `speech.fragment` том, что для сообщений, которым они не возвращаются в интерактивном режиме.
...
**Ответ.** `RecognizeOnce()` в интерактивном режиме звук обрабатывается только до 15 секунд, так как этот режим предназначен для речевых команд, где фразы продолжительностьюы должны быть короткими. Если вы используете `StartContinuousRecognition()` для диктовки или диалога, ограничение в 15 секунд отсутствует.
...
У *нас есть* неявная и базовая поддержка пунктуации для существующих контейнеров, но она `off` по умолчанию. Это означает, что можно получить `.` символ в примере, но не `。` символ. Чтобы включить эту неявную логику, ниже приведен пример того, как это сделать в Python с помощью нашего пакета SDK для распознавания речи (он будет похож на другие языки):
...
- Для единого распознавания (интерактивного режима) с пользовательской конечной точкой (т. е. `SpeechConfig` с параметром конечной точки) см `speech_recognize_once_from_file_with_custom_endpoint_parameters()`. в разделе.
- Для непрерывного распознавания (режим диалога) и просто изменения для использования пользовательской конечной точки, как описано выше `speech_recognize_continuous_from_file()`, см. раздел.
- Чтобы включить диктовку в образцах, подобных приведенным выше (только если это действительно необходимо), сразу `speech_config`после создания добавьте `speech_config.enable_dictation()`код.
...
Формат для host `protocol://hostname:port` `:port` является необязательным (см. ниже):
...
Примеры Python, приведенные выше, `host` но используйте параметр `endpoint`вместо:
