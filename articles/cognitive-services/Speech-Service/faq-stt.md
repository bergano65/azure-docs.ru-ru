...
**Ответ**. При отправке каждого канала по отдельности (каждый канал в отдельном файле) за каждый файл будет взята дополнительная сумма. Если отправить один файл со всеми коммутаторами, которые были объединены в один канал, будет выставляться счет за длительность одного файла.
