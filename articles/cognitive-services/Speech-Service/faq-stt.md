...
Если вы адаптировали и развернули модель с базовым планом версии 1.0, такое развертывание не будет изменяться. Клиенты могут списать развернутую модель, повторно адаптировать ее с помощью новой версии базового плана и повторного развертывания.
...
Чтобы увеличить параллелизм для базовых моделей, необходимы следующие сведения:

- Регион службы,

и либо

- маркер доступа для вас субрскриптион (см. [здесь](https://docs.microsoft.com/azure/cognitive-services/speech-service/rest-speech-to-text#how-to-get-an-access-token));

или

- Идентификатор ресурса для подписки:
  - Перейдите в https://portal.azure.com,
  ...
  - отобразите свойства этой службы,
...
Ответ **. Если**каждый канал отправляется отдельно (каждый канал в отдельном файле), то за каждый файл будет взята дополнительная сумма. Если отправить один файл со всеми коммутаторами, которые были объединены в один канал, будет выставляться счет за длительность одного файла.
