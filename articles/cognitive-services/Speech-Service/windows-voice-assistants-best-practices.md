...
![снимок экрана с голосовым помощником в Windows перед активацией](media/voice-assistants/windows_voice_assistant/initial_state.png)
...
> [!NOTE]
> Из-за активной проблемы помощники, которые нарисулись над пользовательским интерфейсом блокировки, должны реализовать Виндовсервице. Клосевиндов () для всех отклонений. Это приведет к завершению работы приложения, но позволит устранить технические проблемы и сохранить состояние помощника в чистом состоянии. Кроме того, для поддержания чистого состояния, если для приложения включена описанная выше блокировка голоса, она должна прослушивать изменения состояния блокировки и Виндовсервице. Клосевиндов () при блокировке устройства.
...
![Снимок экрана с голосовым помощником в Windows перед активацией](media/voice-assistants/windows_voice_assistant/above_lock_response.png)

![Снимок экрана с голосовым помощником в Windows перед активацией](media/voice-assistants/windows_voice_assistant/lock_screen2.png)

### <a name="privacy-amp-security-considerations-above-lock"></a>Вопросы &amp; безопасности конфиденциальности при блокировке

Многие компьютеры являются переносимыми, но не всегда находятся в пределах клиента. Они могут ненадолго остаться в гостиницах, рабочих помещениях в самолете или рабочих областях, где у других людей есть физический доступ. Если помощники, включенные выше блокировки, не подготовлены, они могут подвергаться классу так называемых &quot;атак злоумышленников [Маид](https://en.wikipedia.org/wiki/Evil_maid_attack) &quot; .
...
- Помощники должны **реализовать навык список разрешений, чтобы определять навыки, подтверждающие безопасность и безопасность при** блокировке.
...
- Навык список разрешений должен рассматривать три класса действий или навыков:

| **Класс Action** | **Описание** | **Примеры (не полный список)** |
| --- | --- | --- |
| Безопасность без проверки подлинности | Сведения общего назначения или основная команда и управление приложением | &quot;Сколько времени? &quot;, &quot;Воспроизведите следующую запись&quot; |
| Безопасность с ИД динамика | Риск олицетворения, раскрытие персональных данных. | &quot;Что&#39;о следующей встрече? &quot;, &quot;Ознакомьтесь со списком&quot;покупок, &quot;ответьте на звонок.&quot; |
...
Для дальнейшей защиты работы, **связи или другие запуски приложений всегда будут блокироваться Windows, пока клиент не войдет в систему.** В качестве последнего решения проблемы с безопасностью Корпорация Майкрософт оставляет за собой право удалить приложение из список разрешений включенных помощников, если серьезная ошибка безопасности не будет своевременно устранена.
