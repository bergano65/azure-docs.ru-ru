...
В настоящее время поддерживает Speech SDK C# ([здесь подробно](https://aka.ms/csspeech)), Java (Android и индивидуальные устройства), Objective-C (iOS), C++ (Windows и Linux) и JavaScript. Интерфейсы API на всех платформах похожи, что упрощает разработку для нескольких платформ.
