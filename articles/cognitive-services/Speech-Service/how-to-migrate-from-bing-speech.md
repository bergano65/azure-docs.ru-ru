...
Протоколы WebSocket службы "Речь" также совместимы с этими протоколами, используемыми службой "Распознавание речи Bing". Мы рекомендуем для разработки новых приложений использовать пакет SDK для службы "Речь", а не WebSocket. Рекомендуется также перенести существующий код в этот пакет SDK. Однако, как и с REST API, существующий код, использующий службу "Распознавание речи Bing" через WebSocket, требует только изменения конечной точки и обновления ключа.
