...
Аудио отправляется в тексте HTTP-запроса `PUT`. Аудиопоток должен быть в 16-битном формате WAV с одноканальным (моно) PCM на 16 кГц в следующем формате или кодировке.

* Формат WAV кодека PCM
* Формат Ogg кодека OPUS
