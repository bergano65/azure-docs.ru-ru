...
| `Content-type` | Описывает формат и кодек предоставленных аудиоданных. Допустимые значения: `audio/wav; codec=audio/pcm; samplerate=16000` и `audio/ogg; codec=audio/pcm; samplerate=16000`. | Обязательно |
...
Текст отправляется в теле HTTP-запроса `POST`. Это может быть обычный текст (ASCII или UTF-8) или [SSML](speech-synthesis-markup.md) (UTF-8). В запросах с обычным текстом используются голос и язык службы распознавания речи по умолчанию. С помощью SSML можно указать голос и язык.
