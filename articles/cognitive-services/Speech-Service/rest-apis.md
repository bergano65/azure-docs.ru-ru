---
title: REST API службы "Речь"
description: Справочник по REST API службы "Речь".
...
|-|-|-|
|`language`|Обязательно|Идентификатор распознаваемого языка. См. сведения о [поддерживаемых языках](language-support.md#speech-to-text).|
...
>[!NOTE]
>Указанные выше форматы поддерживаются в службе "Речь" с помощью REST API и WebSocket. [Пакет SDK службы "Речь"](/index.yml) в настоящее время поддерживает только формат WAV кодека PCM.
...
В следующем коде показано, как отправлять аудио блоками. Только первый фрагмент данных должен содержать заголовок звукового файла. `request` — это объект HTTPWebRequest, подключенный к соответствующей конечной точке REST. `audioFile` — путь к звуковому файлу на диске.
...
Результаты возвращаются в формате JSON. В зависимости от параметров запроса данные возвращаются в формате `simple` или `detailed`.

#### <a name="the-simple-format"></a>Формат `simple` 

Этот формат включает следующие поля верхнего уровня.

|Имя поля|Содержимое|
|-|-|
|`RecognitionStatus`|Состояние, например, `Success` в случае успешного распознавания. См. эту [таблицу](rest-apis.md#recognitionstatus).|
...
#### <a name="the-detailed-format"></a>Формат `detailed` 

Этот формат включает следующие поля верхнего уровня.

|Имя поля|Содержимое|
|-|-|
|`RecognitionStatus`|Состояние, например, `Success` в случае успешного распознавания. См. эту [таблицу](rest-apis.md#recognition-status).|
|`Offset`|Время (в единицах 100 нс), с которого в звуковом потоке начинается распознанная речь.|
|`Duration`|Длительность (в единицах 100 нс) распознанной речи в звуковом потоке.|
|`NBest`|Список возможных интерпретаций той же речи, упорядоченных от наиболее вероятных к наименее вероятным. См. [описание NBest](rest-apis.md#nbest).|

#### <a name="nbest"></a>NBest

Поле `NBest` — это список возможных интерпретаций той же речи, упорядоченных от наиболее вероятных к наименее вероятным. Первая запись совпадает с основным результатом распознавания. Каждая запись содержит следующие поля.
...
|`Display`| Отображаемая форма распознанного текста с расстановкой знаков препинания и прописных букв.
...
Полный список доступных голосов приведен в разделе [Language and region support for Speech Service API](language-support.md#text-to-speech) (Поддержка языков и регионов API службы "Речь").
