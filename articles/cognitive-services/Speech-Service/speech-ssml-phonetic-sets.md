...
Набор телефонной службы речи подает нагрузку после гласного слога. набор `fr-FR` телефонной службы речи не поддерживает подгрузку IPA "ˌ". Если требуется поднагрузка IPA, следует использовать IPA напрямую.
...
> [!TIP]
> Набор `fr-FR` телефонной службы речи не поддерживает следующие французские лиасионс, `n‿` `t‿`, и `z‿`. Если они необходимы, рекомендуется использовать IPA напрямую.
...
> [!NOTE]
> Нам нужно добавить [номер телефона GS\] между двумя различными гласными, за исключением того, что две гласные являются подлинными дифсонг. Этот устныеный согласный является глоттал остановкой. Дополнительные сведения см. в <a href="http://en.wikipedia.org/wiki/Glottal_stop" target="_blank">разделе глоттал <span class="docon docon-navigate-external x-hidden-focus"> </a> </a>останавливаюте.
...
| Д.      | `e`   | **e**лефанте | в**e**nЧтобы    | Елефант**e** |
...
> [!TIP]
> Набор `es-ES` телефонной службы речи не поддерживает следующие испанские IPA, `β` `ð`, и `ɣ`. Если они необходимы, рекомендуется использовать IPA напрямую.

# <a name="zh-cn"></a>[zh-CN](#tab/zh-CN)

Набор телефонного по службы распознавания `zh-CN` речи для основан на собственном наборе <a href="https://en.wikipedia.org/wiki/Pinyin" target="_blank">ПИН <span class="docon docon-navigate-external x-hidden-focus"></span> </a> -кодов телефона.
...
Набор телефонного по службы распознавания `zh-TW` речи для основан на собственном наборе <a href="https://en.wikipedia.org/wiki/Bopomofo" target="_blank">бопомофо <span class="docon docon-navigate-external x-hidden-focus"></span> </a> для телефона.
...
Набор телефонного по службы распознавания `ja-JP` речи для установлен на основе собственного телефонного набора <a href="https://en.wikipedia.org/wiki/Kana" target="_blank">японской азбуки <span class="docon docon-navigate-external x-hidden-focus"></span> </a> .
