...
| Пример 1 (SES для согласных, начальных слов для гласного) | Пример 2 (Интервокалик для согласных, Word срединного всегда for гласно) | Пример 3 (Coda для согласных, слово final для гласного) | Comments |
|--|--|--|--|
...
| `sapi` | `ipa` | Пример 1 | Пример 2    | Пример 3  |
|--------|-------|-----------|--------------|------------|
| канал     | `tʃ`  | **CH**в  | э**t**ключать   | Атта**CH** |
...
Набор телефонной службы речи подает нагрузку после гласного слога. набор телефонной службы речи `fr-FR` не поддерживает подгрузку IPA "ˌ". Если требуется поднагрузка IPA, следует использовать IPA напрямую.
...
| величин     | `ø`   | **œу**FS      | CR,**Европейский**, SER     | qu (**ЕС** )  |
...
| л      | `v`   | **v**ENT    | в**v**введите  | рê**v**e                         |
| w      | `w`   | **подразделение**i     | Строка**подразделения**f    |                                  |
| y      | `j`   | **y**OD     | p**i**éтинер  | **Илле** режима MARS                    |
...
> [!TIP]
> Набор телефонной службы `fr-FR` речи не поддерживает следующие французские лиасионс, `n‿`, `t‿`и `z‿`. Если они необходимы, рекомендуется использовать IPA напрямую.
...
| величин     | `øː`      | **Ц**НДА                              | АБЛ**ц**стен  | Малм**ц**                          |
...
| канал | `tʃ` | **Тсч**ечиен | ауфжепу**тсч**t | бундесдеу**тсч** |  |
| л | `v` | **w**Инкен | Вопрос**u**Алле | [<sup>9</sup>](#de-c-9) GR**ОО** |  |
...
<a id="de-c-10"></a>
**10** *`x` IPA является жестким "CH" после всех непередних гласных (a, AA,, вероятно, UW и дифсонг AW).*<br>
<a id="de-c-11"></a>
**11** *`ç` IPA — это мягкая "CH" после гласных (IH, ий, EH, AE, Uy, UE, OE, EU также в дифсонгс гг, Oy) и согласных*<br>
<a id="de-c-12"></a>
...
> [!NOTE]
> Нам нужно добавить [GS\] телефон между двумя различными гласными, за исключением того, что две гласные являются подлинными дифсонг. Этот устныеный согласный является глоттал остановкой. Дополнительные сведения см. в разделе <a href="http://en.wikipedia.org/wiki/Glottal_stop" target="_blank">глоттал <span class="docon docon-navigate-external x-hidden-focus"></a></a>.
...
| b      | `b`        | **б**аобаб |                | AM**b**        |
|        | `β`        |            | из**b**AB     | баоба**б**     |
| канал     | `tʃ`       | **CH**екуе | сопутствующие**буквы**e      | Марракуе**CH** |
| d      | `d`        | Эдо **г**   |                | портлан**г**   |
|        | `ð`        |            | de**o**       | Верда**г**     |
...
> [!TIP]
> Набор телефонной службы `es-ES` речи не поддерживает следующие испанские IPA, `β`, `ð`и `ɣ`. Если они необходимы, рекомендуется использовать IPA напрямую.

# <a name="zh-cn"></a>[zh-CN](#tab/zh-CN)

Набор телефонной службы речи для `zh-CN` основан на собственном наборе <a href="https://en.wikipedia.org/wiki/Pinyin" target="_blank">пиньинь <span class="docon docon-navigate-external x-hidden-focus"></span> </a> телефона.
...
Набор телефонной службы речи для `zh-TW` основан на собственном наборе <a href="https://en.wikipedia.org/wiki/Bopomofo" target="_blank">бопомофо <span class="docon docon-navigate-external x-hidden-focus"></span> </a> .
...
Набор телефонной службы речи для `ja-JP` основан на собственном наборе <a href="https://en.wikipedia.org/wiki/Kana" target="_blank">японской азбуки <span class="docon docon-navigate-external x-hidden-focus"></span> </a> телефона.
