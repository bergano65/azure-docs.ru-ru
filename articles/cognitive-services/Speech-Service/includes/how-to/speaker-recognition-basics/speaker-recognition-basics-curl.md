...
Для начала [создайте голосовой профиль](https://docs.microsoft.com/rest/api/speakerrecognition/verification/textdependent/createprofile). Необходимо вставить ключ и регион подписки службы "Речь" в каждую из этих команд в этой статье.
...
Затем вам нужно [зарегистрировать речевой профиль](https://docs.microsoft.com/rest/api/speakerrecognition/verification/textdependent/createenrollment). В качестве значения параметра `--data-binary` укажите звуковой файл на компьютере, который содержит одну из поддерживаемых парольных фраз, например my voice is my passport, verify me (мой голос — мой паспорт, проверь меня). Вы можете записать такой звуковой файл с помощью приложения, например [Запись голоса Windows](https://www.microsoft.com/p/windows-voice-recorder/9wzdncrfhwkn?activetab=pivot:overviewtab), или создать его с помощью [преобразования текста в речь](https://docs.microsoft.com/azure/cognitive-services/speech-service/index-text-to-speech).
...
Теперь вы готовы [сверить пример звуковых данных с речевым профилем](https://docs.microsoft.com/rest/api/speakerrecognition/verification/textdependent/verifyprofile). Этот пример звуковых данных должен содержать ту же парольную фразу, что и примеры, использованные для регистрации речевого профиля.
...
Чтобы завершить, [удалите речевой профиль](https://docs.microsoft.com/rest/api/speakerrecognition/verification/textdependent/deleteprofile).
...
Начните с [создания профиля проверки, не зависящего от текста](https://docs.microsoft.com/rest/api/speakerrecognition/verification/textindependent/createprofile).
...
Затем [зарегистрируете речевой профиль](https://docs.microsoft.com/rest/api/speakerrecognition/verification/textindependent/createenrollment). Опять же, вместо отправки трех примеров звуковых данных необходимо отправить только те примеры, длительность звуковых данных которых составляет 20 секунд.
...
Чтобы завершить, [удалите речевой профиль](https://docs.microsoft.com/rest/api/speakerrecognition/verification/textindependent/deleteprofile).
...
Начните с [создания профиля идентификации, не зависящего от текста](https://docs.microsoft.com/rest/api/speakerrecognition/identification/textindependent/createprofile).
...
Затем вам нужно [зарегистрировать речевой профиль](https://docs.microsoft.com/rest/api/speakerrecognition/identification/textindependent/createenrollment). Опять же, вам необходимо отправить примеры звуковых данных, длительность которых составляет 20 секунд. Эти примеры не обязательно должны содержать парольную фразу.
...
Чтобы завершить, [удалите речевой профиль](https://docs.microsoft.com/rest/api/speakerrecognition/identification/textindependent/deleteprofile).
