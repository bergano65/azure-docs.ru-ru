...
**Ответ. С помощью HTTP-запросов можно обращаться к более чем 80 стандартным голосовым моделям на более чем 45 языках.** Сначала получите [ключ подписки](./overview.md#try-the-speech-service-for-free). Чтобы узнать, как делать вызовы REST к предварительно развернутым голосовым моделям, см. раздел [REST API](./overview.md#reference-docs).
...
На данный момент мы не обеспечиваем поддержку записи в сети и не можем предоставить рекомендации по звукозаписывающим студиям. Сведения о требованиях к формату см. [в статье Подготовка записей и расшифровки](./how-to-custom-voice-create-voice.md).
