...
> [!NOTE]
> В `en-US-AriaRUS` этом примере используется речь. Полный список поддерживаемых голосов см. в разделе [Поддержка языков](language-support.md#text-to-speech).
...
В зависимости от языка речевого пакета SDK вы устанавливаете `"SpeechServiceResponse_Synthesis_WordBoundaryEnabled"` свойство в `false` значение в экземпляре `SpeechConfig` объекта.
...
По умолчанию в службе преобразования текста в речь используется нейтральный стиль для стандартных и нейронных голосов. С помощью нейронных голосов можно настроить стиль речи, чтобы выразить чирфулнесс, сопереживание или тональности с помощью `<mstts:express-as>` элемента. Это необязательный элемент, уникальный для службы распознавания речи.
...
> [!NOTE]
> Используйте этот элемент, чтобы переопределить поведение по умолчанию преобразования текста в речь (TTS) для слова или фразы, если синтезированное речевое выражение для этого слова или фразы звучит неестественно. Задайте `strength` значение `none` , чтобы предотвратить разрыв интонационную, который автоматически вставляется службой преобразования текста в речь.
...
| Атрибут | Описание | Обязательный или необязательный |
|-----------|-------------|---------------------|
| `strength` | Задает относительную продолжительность паузы, используя одно из следующих значений:<ul><li>none</li><li>x-слабый</li><li>безопасные</li><li>Средний (по умолчанию)</li><li>надежный</li><li>строгость x</li></ul> | Необязательный |
| `time` | Задает абсолютную длительность паузы в секундах или миллисекундах. Примеры допустимых значений — `2s` и.`500` | Необязательный |
...
`s` Элемент может содержать текст и `audio`следующие элементы:, `break`, `phoneme`, `prosody`, `say-as` `mstts:express-as`, и. `sub`
...
Иногда TTS не может правильно произношень слова, например названия компании или внешнего имени. Разработчики могут определить считывание этих сущностей в SSML с `phoneme` помощью `sub` тега и, или определить чтение нескольких сущностей, обратившись к пользовательскому файлу лексикона `lexicon` с помощью тега.
...
Каждый `lexeme` элемент является элементом лексикона. `grapheme`содержит текст, описывающий орсограф `lexeme`. Форма "Цветовая подзапись `alias`" может быть указана как. В `phoneme` элементе может быть указана телефонная строка.

`lexicon` Элемент содержит по крайней мере `lexeme` один элемент. Каждый `lexeme` элемент содержит по крайней `grapheme` мере один элемент и один `grapheme`или `alais`несколько элементов `phoneme` , и. `grapheme` Элемент содержит текст, описывающий <a href="https://www.w3.org/TR/pronunciation-lexicon/#term-Orthography" target="_blank">орсографи <span class="docon docon-navigate-external x-hidden-focus"> </span> </a>. `alias` Элементы используются для указания произношения акронима или сокращенного выражения. `phoneme` Элемент предоставляет текст, описывающий, `lexeme` как произносится.
...
В приведенном выше примере мы используем Международный фонетический алфавит, также известный как набор IPA Phone. Мы рекомендуем разработчикам использовать IPA, так как это Международный стандарт. Учитывая,`en-US`что IPA нелегко запомнить, служба распознавания речи определяет фонетический набор для семи языков (, `fr-FR`, `de-DE`, `es-ES` `ja-JP` `zh-CN`,, и `zh-TW`).
...
| `rate` | Указывает интенсивность речи текста. Вы можете выразить `rate` следующим образом:<ul><li>Относительное значение, выраженное в виде числа, которое выступает в качестве множителя по умолчанию. Например, значение *1* приводит к отсутствию изменений в скорости. Значение *0,5* приводит к половине скорости. Значение *3* приводит к утраиваетсяу скорости.</li><li>Значение константы:<ul><li>x-замедляют</li><li>медленный</li><li>средняя</li><li>быстрая</li><li>x-Fast</li><li>default</li></ul></li></ul> | Необязательный |
...
| `format` | Предоставляет дополнительные сведения о точном форматировании текста элемента для типов содержимого, которые могут иметь неоднозначные форматы. SSML определяет форматы для типов содержимого, которые их используют (см. таблицу ниже). | Необязательный |
| `detail` | Указывает уровень детализации для речи. Например, этот атрибут может запросить, чтобы механизм синтеза речи произносит знаки пунктуации. Для `detail`не определено ни одного стандартного значения. | Необязательный |

<!-- I don't understand the last sentence. Don't we know which one Cortana uses? -->

Ниже приведены поддерживаемые типы содержимого для атрибутов `interpret-as` и. `format` Включайте `format` атрибут только в том `interpret-as` случае, если для параметра задано значение Дата и время.
...
Для каждого документа SSML допускается только один фоновый звуковой файл. Тем не менее `audio` Теги можно добавлять в `voice` элемент, чтобы добавить дополнительный звук в документ SSML.
...
| `volume` | Указывает громкость фонового звукового файла. **Допустимые значения** `0` : `100` в включительно. Значение по умолчанию — `1`. | Необязательный |
| `fadein` | Задает время, в течение которого фоновый звук постепенно передается в миллисекундах. Значение по умолчанию `0`—, что эквивалентно отсутствию появления. **Допустимые значения** `0` : `10000` в включительно.  | Необязательный |
| `fadeout` | Указывает длительность фонового исчезновения в миллисекундах. Значение по умолчанию `0`—, что эквивалентно значению без затухания. **Допустимые значения** `0` : `10000` в включительно.  | Необязательный |
