...
| Атрибут | Описание | Обязательный или необязательный |
|-----------|-------------|---------------------|
| `strength` | Определяет относительную продолжительность паузы с использованием одного из следующих значений:<ul><li>Нет</li><li>x-слабый</li><li>Слабым</li><li>средний (по умолчанию)</li><li>надежный</li><li>x-сильный</li></ul> | Необязательно |
| `time` | Определяет абсолютную продолжительность паузы в секундах или миллисекундах. Примеры допустимых `2s` значений`500` | Необязательно |
...
| `format` | Предоставляет дополнительную информацию о точном форматировании текста элемента для типов содержимого, которые могут иметь неоднозначные форматы. SSML определяет форматы для типов контента, которые их используют (см. таблицу ниже). | Необязательно |
| `detail` | Указывает уровень детализации, который нужно произнесь. Например, этот атрибут может потребовать, чтобы двигатель синтеза речи произносил знаки препинания. Стандартные значения не определены для. `detail` | Необязательно |
...
| `volume` | Определяет объем фонового аудиофайла. **Принятые значения:** `0` к инклюзивному. `100` Значение по умолчанию — `1`. | Необязательно |
| `fadein` | Определяет продолжительность фонового звука "угасает" как миллисекунды. Значение по `0`умолчанию, что эквивалентно не исчезают дюйма **Принятые значения:** `0` к инклюзивному. `10000`  | Необязательно |
| `fadeout` | Определяет продолжительность фонового звука за тридцать секунд. Значение по `0`умолчанию, что эквивалентно не исчезают. **Принятые значения:** `0` к инклюзивному. `10000`  | Необязательно |
