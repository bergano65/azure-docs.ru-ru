...
| attribute | Description | Обязательный или необязательный |
|-----------|-------------|---------------------|
...
| `xml:lang` | Указывает язык корневого документа. Значение может содержать строчный, двухбуквенный код языка (например, `en`), языковой код и страну или регион в верхнем регистре (например, `en-US`). | Обязательно |
| `xmlns` | Задает универсальный код ресурса (URI) документа, который определяет словарь разметки (типы элементов и имена атрибутов) документа SSML. Текущий URI — https://www.w3.org/2001/10/synthesis. | Обязательно |
...
В зависимости от языка речевого пакета SDK вы задаете свойству `"SpeechServiceResponse_Synthesis_WordBoundaryEnabled"` значение `false` на экземпляре объекта `SpeechConfig`.

# <a name="c"></a>[C#](#tab/csharp)

Дополнительные сведения см. в <a href="https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechconfig.setproperty?view=azure-dotnet" target="_blank">разделе <span class="docon docon-navigate-external x-hidden-focus"> </span>`SetProperty` </a>.
...
Дополнительные сведения см. в <a href="https://docs.microsoft.com/cpp/cognitive-services/speech/speechconfig#setproperty" target="_blank">разделе <span class="docon docon-navigate-external x-hidden-focus"> </span>`SetProperty` </a>.
...
Дополнительные сведения см. в <a href="https://docs.microsoft.com/java/api/com.microsoft.cognitiveservices.speech.speechconfig.setproperty?view=azure-java-stable#com_microsoft_cognitiveservices_speech_SpeechConfig_setProperty_String_String_" target="_blank">разделе <span class="docon docon-navigate-external x-hidden-focus"> </span>`setProperty` </a>.
...
Дополнительные сведения см. в <a href="https://docs.microsoft.com/python/api/azure-cognitiveservices-speech/azure.cognitiveservices.speech.speechconfig?view=azure-python#set-property-by-name-property-name--str--value--str-" target="_blank">разделе <span class="docon docon-navigate-external x-hidden-focus"> </span>`set_property_by_name` </a>.
...
Дополнительные сведения см. в <a href="https://docs.microsoft.com/javascript/api/microsoft-cognitiveservices-speech-sdk/speechconfig?view=azure-node-latest#setproperty-string--string-" target="_blank">разделе <span class="docon docon-navigate-external x-hidden-focus"> </span>`setProperty` </a>.
...
Дополнительные сведения см. в <a href="https://docs.microsoft.com/objectivec/cognitive-services/speech/spxspeechconfiguration#setpropertytobyname" target="_blank">разделе <span class="docon docon-navigate-external x-hidden-focus"> </span>`setPropertyTo` </a>.
...
По умолчанию в службе преобразования текста в речь используется нейтральный стиль для стандартных и нейронных голосов. С помощью нейронных голосов можно настроить стиль речи, чтобы выразить чирфулнесс, сопереживание или тональности с помощью элемента `<mstts:express-as>`. Это необязательный элемент, уникальный для службы распознавания речи.
...
| Голосовая связь | Тип | Description |
|-------|------|-------------|
...
| | `type="newscast"` | Выражает формальный тон аналогично новостным трансляциям |
| | `type="customerservice"` | Обговорите понятный и прочный способ обслуживания клиентов |
| `pt-BR-FranciscaNeural` | `type="cheerful"` | Выражает положительное и счастливое распознавания эмоций |
...
| attribute | Description | Обязательный или необязательный |
|-----------|-------------|---------------------|
| `strength` | Задает относительную продолжительность паузы, используя одно из следующих значений:<ul><li>none</li><li>x-слабый</li><li>безопасные</li><li>Средний (по умолчанию)</li><li>надежный</li><li>строгость x</li></ul> | Необязательно |
...
| attribute | Description | Обязательный или необязательный |
|-----------|-------------|---------------------|
| `alphabet` | Указывает фонетический алфавит, используемый при синтезировании произношения строки в атрибуте `ph`. Строка, указывающая букву алфавита, должна быть указана в строчных буквах. Ниже приведены возможные алфавиты, которые можно указать.<ul><li>`ipa` &ndash; Международный фонетический алфавит</li><li>`sapi` &ndash; службы речи, фонетический алфавит</li><li>Универсальный набор телефонов `ups` &ndash;</li></ul><br>Алфавит применяется только к `phoneme` в элементе. Дополнительные сведения см. в разделе [Справочник по фонетическому алфавиту](https://en.wikipedia.org/wiki/International_Phonetic_Alphabet). | Необязательно |
| `ph` | Строка, содержащая номера телефонов, задающих произношение слова в элементе `phoneme`. Если указанная строка содержит нераспознаваемые телефоны, служба преобразования текста в речь (TTS) отклоняет весь документ SSML и не выдает ни одного из речевых выходных данных, указанных в документе. | Требуется при использовании фонемы. |
...
В приведенном выше примере мы используем Международный фонетический алфавит, также известный как набор IPA Phone. Мы рекомендуем разработчикам использовать IPA, так как это Международный стандарт. Учитывая, что IPA легко запомнить, служба распознавания речи определяет фонетический набор для семи языков (`en-US`, `fr-FR`, `de-DE`, `es-ES`, `ja-JP`, `zh-CN`и `zh-TW`).

Вы можете использовать `sapi` в качестве значение для атрибута `alphabet` с пользовательскими лексиконами, как показано ниже:
...
| `rate` | Указывает интенсивность речи текста. Вы можете выразить `rate` как:<ul><li>Относительное значение, выраженное в виде числа, которое выступает в качестве множителя по умолчанию. Например, значение *1* приводит к отсутствию изменений в скорости. Значение *0,5* приводит к половине скорости. Значение *3* приводит к утраиваетсяу скорости.</li><li>Значение константы:<ul><li>x-замедляют</li><li>медленный</li><li>средняя</li><li>ускоряет</li><li>x-Fast</li><li>значение по умолчанию</li></ul></li></ul> | Необязательно |
...
| `detail` | Указывает уровень детализации для речи. Например, этот атрибут может запросить, чтобы механизм синтеза речи произносит знаки пунктуации. Для `detail`не определены стандартные значения. | Необязательно |
...
| `date` | dmy, mdy, ГМД, ГДМ, им, My, MD, DM, d, m, y | Текст обговорятся как дата. Атрибут `format` задает формат даты (*d = день, m = месяц и y = year*). Модуль синтеза речи произносит следующее:<br /><br />`Today is <say-as interpret-as="date" format="mdy">10-19-2016</say-as>`<br /><br />Как "Сегодня Октябрь девятнадцатого 2016". |
...
| `volume` | Указывает громкость фонового звукового файла. **Допустимые значения**: `0` для `100` включительно. Значение по умолчанию — `1`. | Необязательно |
| `fadein` | Задает время, в течение которого фоновый звук постепенно передается в миллисекундах. Значение по умолчанию — `0`, что эквивалентно отсутствию появления. **Допустимые значения**: `0` для `10000` включительно.  | Необязательно |
