...
| FadeIn | Указывает длительность фонового исчезновения звука в миллисекундах. Значение по умолчанию `0`—, что эквивалентно отсутствию появления. **Допустимые значения** `0` : `10000` в включительно.  | Необязательный |
