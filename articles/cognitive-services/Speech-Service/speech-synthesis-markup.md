...
При использовании SSML для преобразования текста в синтезированную речь Помните, что, как и в случае с XML, Специальные символы, такие как кавычки, апострофы и квадратные скобки, должны быть экранированы. Дополнительные сведения см. в разделе [Extensible Markup Language (XML) 1,0: Приложение г @ no__t-0.
...
| | Тип =`chat` | Проговаривать на случайном, ослабленном тоновом |
...
| Атрибут | Описание | Обязательный или необязательный |
|-----------|-------------|---------------------|
| интерпретировать как | Указывает тип содержимого текста элемента. Список типов см. в таблице ниже. | Обязательное значение |
...
| Описание | Указывает уровень детализации для речи. Например, этот атрибут может запросить, чтобы механизм синтеза речи произносит знаки пунктуации. Для `detail` не определены стандартные значения. | Необязательный |
...
|--------------|--------|----------------|
| адрес | | Текст обговорятся как адрес. Модуль синтеза речи произносит следующее:<br /><br />`I'm at <say-as interpret-as="address">150th CT NE, Redmond, WA</say-as>`<br /><br />Как «я в 150th суд Северная Восточная часть Редмонд Вашингтон». |
...
Элемент `mstts:backgroundaudio` позволяет добавить фоновый звук в документы SSML (или смешать звуковой файл с текстом в речь). С `mstts:backgroundaudio` можно циклически перебрать звуковой файл в фоновом режиме, появление в начале перевода текста в речь и исчезание в конец текста в речь.
...
Для каждого документа SSML допускается только один фоновый звуковой файл. Однако можно добавить теги `audio` в элемент `voice` для добавления дополнительного звука в документ SSML.
...
| том | Указывает громкость фонового звукового файла. **Допустимые значения**: `0` к `100` включительно. По умолчанию используется значение `1`. | Необязательный |
| FadeIn | Указывает длительность фонового исчезновения звука в миллисекундах. Значение по умолчанию — `0`, что эквивалентно отсутствию появления. **Допустимые значения**: `0` к `10000` включительно.  | Необязательный |
| FadeOut | Указывает длительность фонового исчезновения в миллисекундах. Значение по умолчанию — `0`, что эквивалентно значению без затухания. **Допустимые значения**: `0` к `10000` включительно.  | Необязательный |
