...
* Предотвратить загрузку https прокси агента webpack.

## <a name="speech-sdk-140-2019-april-release"></a>Speech SDK 1.4.0: Выпуск апреля 2019 г.

**новые функции;** 

* Пакет SDK теперь поддерживает службу преобразования текста в речь, бета-версия. Она поддерживается в Windows и Linux рабочего стола с помощью C++, C#и Java. Дополнительные сведения см [преобразования текста в речь Обзор](text-to-speech.md#get-started-with-text-to-speech).
...
* Для получения дополнительной поддержки разработки и отладки можно перенаправить SDK сведения диагностики и ведения журнала в файл журнала (Дополнительные сведения о [здесь](how-to-use-logging.md)).
...
* Создаваемый пакетом SDK идентификатор сессии (SessionId) в некоторых языках&nbsp;и средах не был достаточно случайным. Для устранения этой проблемы добавлена инициализация генератора случайных чисел.
...
* В некоторых случаях объект Connection не освобождался должным образом. Эта ошибка исправлена.
...
* `StartContinuousRecognitionAsync()` поддерживает распознавание выполняющейся длительное время.
