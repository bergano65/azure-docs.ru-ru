...
* Пакет SDK для службы "Речь" поддерживает выбор микрофонного входа с помощью класса AudioConfig. Это позволяет выполнять потоковую передачу аудиоданных в службу "Речь" с любого микрофона, а не только выбранного по умолчанию. Дополнительные сведения см. в документации [о выборе устройств для получения аудио данных](how-to-select-audio-input-devices.md). Эта возможность пока недоступна для JavaScript.
