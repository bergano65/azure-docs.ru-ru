...
* Пакет SDK теперь поддерживает аудиофайлы MP3 и труде/Ogg как поток входных файлов. Эта функция доступна только на платформе Linux с помощью C++ и C# и в настоящее время в бета-версии (Дополнительные сведения о [здесь](how-to-use-compressed-audio-input-streams.md)).
