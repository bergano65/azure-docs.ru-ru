...
* SWIFT и Objective-C:
    * Улучшенные отчеты об ошибках: Методы, которые могут привести к возникновению ошибки присутствуют в двух версиях: Предоставляет `NSError` объект для обработки ошибок, а тот, который приводит к появлению исключения. Первое из них предоставляются Swift. Это изменение требует адаптации в существующий код Swift.
    * Обработка событий улучшенные
...
*   Исправление ошибки, отсутствующие метаданные в пакете framework для macOS
*   Устранить проблемы с `pip install --user` на Windows
...
* Исправьте FromSubscription при использовании с расшифровка дикторского текста для диалога.
* Исправлена ошибка в ключевое слово, распознавание голоса первый виртуальный помощник.
