...
ms.locfileid: "51218599"
---
# <a name="release-notes"></a>Заметки о выпуске

## <a name="speech-service-sdk-110"></a>Пакет SDK для службы "Речь" версии 1.1.0

**Новые функции**

* Поддержка Android x86/x64.
* Поддержка прокси-сервера. В объекте SpeechConfig теперь можно вызвать функцию для вызова сведений о прокси-сервере (имя узла, порт, имя пользователя и пароль). Эта функция недоступна в iOS.
...
* Пакет NuGet: для проектов .NET Framework предотвращает компиляцию с конфигурацией AnyCPU.
...
* Добавлен пример C++ для извлечения потоковой передачи по запросу из [репозитория с примером](https://aka.ms/csspeech/samples) и для отправки в него.
