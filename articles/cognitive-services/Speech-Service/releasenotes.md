...
**Новые функции**
*   Добавлен API Фромхост () для упрощения использования с локальными контейнерами и облаками независимых.
*   Добавлены автоматические распознавание языка источника для распознавания речи (в Java и C++)
*   Добавлен объект Саурцелангуажеконфиг для распознавания речи, используемый для указания ожидаемых исходных языков (в Java C++и)
*   Добавлена поддержка Кэйвордрекогнизер в Windows (UWP), Android и iOS через пакеты NuGet и Unity.
*   Добавлен удаленный диалог Java API для записи разговора в асинхронных пакетах.

**Критические изменения**
*   Функции диалога Транскрибер перемещены в пространстве имен Microsoft. CognitiveServices. Speech. транскрипция.
*   Часть методов диалога Транскрибер перемещается в новый класс CONVERSATION.
*   Удалена поддержка 32-bit (ARMv7 и x86) iOS 

**Исправления ошибок**
*   Устранение сбоя, если локальная Кэйвордрекогнизер используется без допустимого ключа подписки на службу речи.

**Примеры**
*   Пример Xamarin для Кэйвордрекогнизер
*   Пример Unity для Кэйвордрекогнизер
...
* Свойство цели-C: `OutputFormat` было добавлено в Спксспичконфигуратион.
...
* C#: Если асинхронная операция не ожидалась в основном потоке, возможно, распознаватель был удален до завершения асинхронной задачи.
* Java: Исправлена проблема, приводящая к сбою виртуальной машины Java.
* Цель-C: фиксированное сопоставление перечислений; Вместо Рекогнизингинтент был возвращен Рекогнизединтент.
* JavaScript: задайте для параметра формат вывода по умолчанию значение SIMPLE в Спичконфиг.
