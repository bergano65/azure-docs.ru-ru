...
| Параметр | Рекомендуемое значение |
| ----| ---- |
| Параметр Configuration | Принять предопределенные входные данные из внешнего каталога |                               
...
1. В окне **Новая проверка** настройте проверку следующим образом и нажмите кнопку **создать**.
    ...
    | Неудачный ответ |  Простой редактор > первый вариант >`Sorry, I can only set temperature between 60 and 80 degrees` | Запрашивать новое значение, если проверка не пройдена |
