...
Все сущности из v2 можно также найти в API V3 с тем же идентификатором. Где изменилась схема результата (например, транскрипции), результат GET в версии v3 API использует схему v3. Результат получения в версии 2 API использует ту же схему v2. Вновь созданные сущности на **v3 недоступны в** результатах API v2.

## <a name="breaking-changes"></a>Критические изменения

Список критических изменений был отсортирован по величине изменений, необходимых для адаптации. В вызывающем коде требуется внести нетривиальные изменения. Для большинства изменений требуется только изменение имен элементов.
