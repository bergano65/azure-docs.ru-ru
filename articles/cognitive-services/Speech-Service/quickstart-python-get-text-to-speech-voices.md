---
title: Краткое руководство. Перечисление голосовых моделей для преобразований текста в речь на Python — службы "Речь"
...
Для этой конечной точки требуется предоставить маркер доступа для аутентификации. Чтобы получить маркер доступа, нужно выполнить обмен. Этот пример меняет ключ подписки служб "Речь" на маркер доступа через конечную точку `issueToken`.
