...
1. Щелкните **опубликовать** в правой части вновь созданного профиля.
...
> [!IMPORTANT]
> В ресурсе регистрации каналов Bot будет показан глобальный регион, даже если вы выбрали "Западная часть США". Такое поведение является стандартным.
...
1. Откройте решение Visual Studio Ечобот. В обозревателе решений перейдите к разделу **appSettings. JSON** и дважды щелкните его.
...
1. Сохраните файл JSON. Он должен выглядеть примерно так:
```json
{
...
1. Следуйте инструкциям, приведенным в этой статье.
   * Скачайте ZIP-пакет, содержащий исполняемый файл для запуска, или
   * Создайте исполняемый файл самостоятельно, клонировать репозиторий и создав проект.

1. Запустите клиентское приложение и настройте его для подключения к роботу в соответствии с инструкциями в репозитории GitHub.
1. Нажмите кнопку **Повторное соединение** и убедитесь, что сообщение отображается **нажатием кнопки MIC, или введите, чтобы начать говорить с программой Bot**.
