---
title: Учебник. Голоса. Включение программы-робота с помощью речевого пакета SDK
...
ms.locfileid: "68932045"
---
# <a name="tutorial-voice-enable-your-bot-using-the-speech-sdk"></a>Учебник. Голосовая связь — включение программы-робота с помощью речевого пакета SDK
...
2. Звук записывается с микрофона по умолчанию при нажатии кнопки (или постоянно записывается при активации настраиваемого Wake-слова)
3. При необходимости выполняется пользовательское обнаружение слов пробуждения, ограничение потоковой передачи звука в облако
4. С помощью пакета SDK для распознавания речи приложение подключается к каналу голосовой речи и потокам аудио
5. При необходимости выполняется более высокая точность проверки слов при пробуждении по службе
6. Звук передается в службу распознавания речи и расшифрованной в текст
7. Распознанный текст передается в эхо-Bot как действие платформы Bot 
8. Текст ответа включается службой преобразования текста в речь (TTS) и передается обратно в клиентское приложение для воспроизведения.

![тег схемы](media/tutorial-voice-enable-your-bot-speech-sdk/diagram.png "Поток речевого канала")

> [!NOTE]
> Для выполнения действий, описанных в этом руководстве, не требуется платная служба. Как новый пользователь Azure, вы сможете использовать кредиты из бесплатной подписки Azure и бесплатного уровня для работы с этим руководством.
...
> * Добавление пользовательской активации пробуждения по словам
...
2. В левой области навигации выберите **группы ресурсов**. Затем нажмите кнопку **Добавить** , чтобы добавить новую группу ресурсов.
3. Вам будет предложено ввести некоторые сведения:
...
4. Щелкните **проверить и создать**. Вы увидите баннер, **прошедший проверку**на прочтение.
5. Нажмите кнопку **Создать**. Создание группы ресурсов может занять несколько минут.
6. Как и в случае с ресурсами, которые будут созданы далее в этом учебнике, рекомендуется закрепить эту группу ресурсов на панели мониторинга для упрощения доступа. Если вы хотите закрепить эту группу ресурсов, щелкните значок закрепления в верхнем правом углу панели мониторинга.
...
* Прямой канал распознавания речи — это предварительная версия службы. Это может быть ограничено конкретными регионами Azure. Дополнительные сведения о доступных регионах см. в разделе " [Виртуальные помощники](https://docs.microsoft.com/azure/cognitive-services/speech-service/regions#voice-first-virtual-assistants)".
* В канале голосового перевода прямой линии используется служба преобразования текста в речь, которая имеет стандартные и нейронные голоса. Нейронные голоса [ограничены конкретными регионами Azure](https://docs.microsoft.com/azure/cognitive-services/speech-service/regions#standard-and-neural-voices).
...
1. Выберите **создать ресурс** в левой области навигации.
...
4. Вам будет предложено ввести некоторые сведения:
   ...
   * Убедитесь, что для **ценовой**категории выбрано " **стандартный S1** ". Это должно быть значение по умолчанию.
...
| Имя | ТИП  | Расположение |
|------|-------|----------|
| Спичечоботтуториал — AppServicePlan | План служб приложений | Западная часть США |
| Спичечоботтуториал — речь | Cognitive Services | Западная часть США |
...
> [!NOTE]
> Приведенные ниже инструкции, а также дополнительные сведения о программе Echo Bot доступны в [файле сведений примера на сайте GitHub](https://github.com/microsoft/BotBuilder-Samples/blob/master/experimental/directline-speech/csharp_dotnetcore/02.echo-bot/README.md).
...
3. На панели инструментов выберите **файл** > **Открыть** > **проект/решение**и откройте файл проекта программы Echo Bot, настроенный для использования с прямым каналом речи:
   ```
   experimental\directline-speech\csharp_dotnetcore\02.echo-bot\EchoBot.csproj
   ```
4. После загрузки проекта нажмите клавишу `F5` , чтобы запустить проект.

### <a name="test-with-the-bot-framework-emulator"></a>Тестирование с помощью эмулятора Bot Framework

[Эмулятор Bot](https://github.com/microsoft/botframework-emulator) — это классическое приложение, позволяющее Bot-разработчикам тестировать и отлаживать свои программы-роботы локально или удаленно через туннель. Выполните следующие действия, чтобы использовать эмулятор Bot-робота для тестирования эха.
...
3. Введите URL-адрес для Bot. Пример:
   ...
   ```
4. Используйте пользовательский интерфейс для взаимодействия с программой-роботом с помощью введенного текста. Убедитесь, что вы получаете ответ.
...
1. В Visual Studio откройте эхо-робот, настроенный для использования с прямым каналом речи:
   ...
   experimental\directline-speech\csharp_dotnetcore\02.echo-bot\EchoBot.csproj
   ```
2. В **Обозреватель решений**щелкните правой кнопкой мыши решение **Ечобот** и выберите **Опубликовать...**
3. Откроется новое окно **Выбор целевого объекта публикации** .
3. Выберите **служба приложений** в области навигации слева, выберите **создать**и нажмите кнопку **опубликовать**.
5. Когда появится окно **Создание службы приложений** :
...
7. В Visual Studio должно отобразиться сообщение об успешном выполнении, которое выглядит следующим образом:
   ```
   Publish Succeeded.
   Web App was published successfully https://EchoBot20190805125647.azurewebsites.net/
   ```
8. Браузер по умолчанию должен открыть и отобразить страницу, которая считывает: "Ваш робот готов!".
9. На этом этапе проверьте группу ресурсов **спичечоботтуториал-ResourceGroup** в портал Azure и убедитесь в наличии трех ресурсов:
...
2. В левой области навигации выберите **Параметры**, а затем щелкните **Конфигурация**.
...
| EchoBot20190805125647 | Служба приложений | Западная часть США |
| Спичечоботтуториал — AppServicePlan | план службы приложений | Западная часть США |
| Спичечоботтуториал — Ботрегистратион | Регистрация каналов Bot | Глобальный |
| Спичечоботтуториал — речь | Cognitive Services | Западная часть США |
...
2. В левой области навигации выберите **каналы**.
   * Найдите **дополнительные каналы**и щелкните **прямое распознавание речи**.
   * Просмотрите текст на странице с названием **Настройка прямого перевода строки (Предварительная версия)** и нажмите кнопку **сохранить**.
   * В процессе создания были созданы два **секретных ключа** . Эти ключи уникальны для программы Bot. При написании клиентского приложения с помощью [речевого пакета SDK](https://docs.microsoft.com/azure/cognitive-services/speech-service/)вы получите один из этих ключей, чтобы установить подключение между клиентским приложением, каналом голосовой речи прямой линии и службой Bot. В этом руководстве вы будете использовать клиент прямого перевода строки (WPF, C#).
   * Щелкните **Показывать** и скопируйте один из ключей в любом месте, где вы сможете легко получить к нему доступ. Не беспокойтесь, вы всегда можете получить доступ к ключам из портал Azure.
3. В левой области навигации щелкните **Параметры**.
...
> [!TIP]
> Дополнительные сведения см. в статье [Подключение программы-робота к прямой голосовой сети (Предварительная версия)](https://docs.microsoft.com/azure/bot-service/bot-service-channel-connect-directlinespeech?view=azure-bot-service-4.0). На этой странице содержатся дополнительные сведения и известные проблемы.
...
| Error | Что делать? |
|-------|----------------------|
|Ошибка приложения (Дополнительные сведения см. в журнале): Microsoft. CognitiveServices. Speech. CSharp: Значение не может быть равно NULL. Имя параметра: Спичконфиг | Это ошибка клиентского приложения. Убедитесь, что в главном окне приложения у вас есть непустое значение для параметра " *секрет Bot* " (см. раздел [Регистрация программы Bot с помощью прямого голосового канала](#register-the-direct-line-speech-channel)). |
|Ошибка Аусентикатионфаилуре: Сбой обновления WebSocket с ошибкой проверки подлинности (401). Проверьте правильность ключа подписки (или маркера авторизации) и имя региона.| На странице Параметры приложения убедитесь, что вы правильно указали ключ подписки на речь и его регион.<br>Убедитесь, что ваш секрет Bot введен правильно. |
|Ошибка Коннектионфаилуре: Соединение было закрыто удаленным узлом. Код ошибки: 1011. Описание ошибки: Не удалось подключиться к Bot перед отправкой сообщения | Убедитесь, что [установлен флажок "включить конечную точку потоковой передачи"](#register-the-direct-line-speech-channel) и/или [переключаемые **веб-сокеты** ](#enable-web-sockets) .<br>Убедитесь, что служба приложений Azure запущена. Если это так, попробуйте перезапустить службу приложений.|
|Ошибка Коннектионфаилуре: Соединение было закрыто удаленным узлом. Код ошибки: 1011. Описание ошибки: Код состояния отклика не указывает на успешное выполнение: 500 (InternalServerError)| Ваш Bot указал нейронный голос в поле [выходного](https://github.com/microsoft/botframework-sdk/blob/master/specs/botframework-activity/botframework-activity.md#speak) действия, но регион Azure, связанный с ключом подписки на речь, не поддерживает нейронные голоса. См. раздел [Стандартные и нейронные голоса](https://docs.microsoft.com/azure/cognitive-services/speech-service/regions#standard-and-neural-voices).|
|Ошибка Коннектионфаилуре: Соединение было закрыто удаленным узлом. Код ошибки: 1000. Описание ошибки: Превышено значение максимального времени простоя подключения к веб-сокету (> 300000 МС)| Это ожидаемая ошибка, когда подключение к каналу остается открытым и неактивным в течение более пяти минут. |

Если ваша ошибка не устранена в таблице, см [. статью Предварительная версия виртуальных помощников для предварительного просмотра: простом едином входе Azure Active Directory](https://docs.microsoft.com/azure/cognitive-services/speech-service/faq-voice-first-virtual-assistants).

### <a name="view-bot-activities"></a>Просмотр действий программы-робота

Каждый Bot отправляет и получает сообщения о действиях. В окне " **Журнал действий** " в разделе клиентское речевое взаимодействие вы увидите журналы с метками времени с каждым действием, полученным клиентом от Bot. Вы также можете просмотреть действия, отправленные клиентом в Bot с помощью [`DialogServiceConnector.SendActivityAsync`](https://docs.microsoft.com/en-us/dotnet/api/microsoft.cognitiveservices.speech.dialog.dialogserviceconnector.sendactivityasync) метода. При выборе элемента журнала отображаются сведения о связанном действии в формате JSON.
...
- [`DialogServiceConfig`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.dialog.dialogserviceconfig)— Для параметров конфигурации (ключ подписки на речь, область ключей, секрет Bot)
...
## <a name="add-custom-wake-word-activation"></a>Добавление пользовательской активации пробуждения по словам

Пакет SDK для распознавания речи поддерживает пользовательскую активацию пробуждения по словам. Как и в случае с "Привет, Кортана" для помощника Майкрософт, вы можете написать приложение, которое будет постоянно прослушивать пробуждение по своему усмотрению. Помните, что слово «пробуждение» может быть одиночным словом или словосочетанием из нескольких слов.

> [!NOTE]
> Термин " *работа пробуждения* " часто используется с *ключевым словом*, и в документации Майкрософт могут отображаться оба варианта использования.

Обнаружение слов пробуждения выполняется в клиентском приложении. Если используется пробуждение по слову, звук будет передаваться только в канал прямого перевода строки, если обнаружено слово Wake. Канал распознавания речи прямой линии включает в себя компонент, именуемый *проверкой ключевых слов (КВВ)* , который выполняет более сложную обработку в облаке, чтобы убедиться, что выбранное вами вами слово пробуждения находится в начале аудиопотока. Если проверка ключевых слов выполняется, канал будет взаимодействовать с Bot.

Выполните следующие действия, чтобы создать модель пробуждения Word, настроить клиентский речевой режим для использования этой модели и, наконец, проверить ее с помощью программы Bot.

1. Выполните эти инструкции, чтобы [создать пользовательское пробуждение с помощью службы распознавания речи](https://docs.microsoft.com/azure/cognitive-services/speech-service/speech-devices-sdk-create-kws).
2. Распакуйте файл модели, скачанный на предыдущем шаге. Оно должно называться для пробуждения по слову. Выполняется поиск файла с именем `kws.table`.
3. В клиентском речевом клиенте найдите меню **Параметры** (найдите значок шестеренки в правом верхнем углу). Выберите **путь к файлу модели** и введите полный путь `kws.table` к файлу из шага 2.
4. Убедитесь, что установлен флажок **включено**. Это сообщение должно появиться рядом с флажком: "Будет прослушивать пробуждение слова при следующем подключении". Если вы указали неправильный файл или недопустимый путь, появится сообщение об ошибке.
5. Введите **ключ подписки**на речь и **регион ключа подписки**, а затем нажмите кнопку **ОК** , чтобы закрыть меню **параметров** .
6. Выберите **секрет Bot**и нажмите кнопку **Повторное соединение**. Должно отобразиться сообщение, которое считывает: "Новый диалог запущен — введите текст, нажмите кнопку" микрофон "или" Пробуждение ". Теперь приложение постоянно ожидает передачи данных.
7. Проговорите любую фразу, которая начинается с вашего текстового пробуждения. Например: " **{ваше пробуждение}** , какое время это?". Не нужно приостанавливать после уттеринг слова Wake. По завершении происходят две вещи:
...
8. Продолжайте экспериментировать с тремя типами входных данных, которые поддерживает Bot:
   ...
   * Произнести фразу, которая начинается с текстового слова

### <a name="view-the-source-code-that-enables-wake-word"></a>Просмотр исходного кода, включающего пробуждение по словам

В исходном коде клиента прямой строки просмотрите эти файлы, чтобы просмотреть код, используемый для включения обнаружения слов в пробуждении:

1. [`DLSpeechClient\Models.cs`](https://github.com/Azure-Samples/Cognitive-Services-Direct-Line-Speech-Client/blob/master/DLSpeechClient/Models.cs)включает вызов метода [`KeywordRecognitionModel.fromFile()`](https://docs.microsoft.com/en-us/javascript/api/microsoft-cognitiveservices-speech-sdk/keywordrecognitionmodel?view=azure-node-latest#fromfile-string-)Speech SDK, который используется для создания экземпляра модели из локального файла на диске.
1. [`DLSpeechClient\MainWindow.xaml.cs`](https://github.com/Azure-Samples/Cognitive-Services-Direct-Line-Speech-Client/blob/master/DLSpeechClient/MainWindow.xaml.cs)включает в себя вызов метода [`DialogServiceConnector.StartKeywordRecognitionAsync()`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.dialog.dialogserviceconnector.startkeywordrecognitionasync)Speech SDK, который активирует непрерывное обнаружение слов пробуждения.
...
1. Начнем с открытия `\experimental\directline-speech\csharp_dotnetcore\02.echo-bot\Bots\echo-bot.cs`.
2. Затем выберите SSML. Его легко найти, так как он заключен в `<speak></speak>` Теги.
3. В строке `<voice name>` SSML выберите тег, замените его на `<voice name='de-DE-Stefan-Apollo'>`и сохраните. Эта отформатированная строка сообщает службе преобразования текста в речь о необходимости вернуть синтезированный речевой ответ с помощью `de-DE-Stefan-Apollo`голоса, который оптимизирован для немецкого языка.
...
5. Откройте приложение "прямое распознавание речи", нажмите кнопку "Параметры" (значок с правом верхнем углу) и введите `de-de` в поле "язык". Это задает распознавание речевого языка, переопределяя значение по умолчанию `en-us`.
6. Следуйте инструкциям в разделе [Создание](#build-the-direct-line-speech-client) почтового клиента с прямым распознаванием для повторного подключения к вновь развернутому роботу, говорите на новом языке и поговорите о том, что на этом языке я говорю о новом голосе.
...
> [Создание собственного клиентского приложения с помощью пакета SDK для распознавания речи](quickstart-virtual-assistant-csharp-uwp.md)
...
* Создание и развертывание собственного робота с поддержкой голоса:
    ...
    * Изучите существующие [решения для Bot-Framework](https://github.com/microsoft/botframework-solutions): Создание [пользовательского виртуального помощника](https://docs.microsoft.com/azure/cognitive-services/speech-service/voice-first-virtual-assistants) с голосовым подключением и [речевого](https://github.com/microsoft/botframework-solutions/blob/master/docs/howto/assistant/csharp/speechenablement.md) ввода
