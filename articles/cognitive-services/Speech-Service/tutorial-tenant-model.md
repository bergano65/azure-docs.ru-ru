...
description: Автоматическое создание защищенной и соответствующей требованиям клиентской модели (Пользовательское распознавание речи с данными из Microsoft 365), которая использует данные из Microsoft 365 для оптимального распознавания используемых в организации терминов.
...
Клиентская модель (Пользовательское распознавание речи с данными из Microsoft 365) предоставляется корпоративным клиентам Microsoft 365 по запросу и позволяет автоматически создать пользовательскую модель распознавания речи на основе данных Microsoft 365 в организации. Эта модель будет оптимизирована для безопасного и соответствующего требованиям распознавания технических и жаргонных терминов, а также имен сотрудников, используемых в организации.

> [!IMPORTANT]
> Если ваша организация регистрируется через службу клиентской модели, служба "Речь" получает доступ к языковой модели этой организации. Эта модель формируется на основе сообщений электронной почты и документов Microsoft 365, доступных для всех пользователей в организации. Администратор вашей организации может включать и отключать возможность использования языковой модели на уровне организации через портал администрирования.
...
Прежде чем развертывать клиентскую модель, необходимо зарегистрироваться в службе клиентской модели. Эта регистрация выполняется в центре администрирования Microsoft 365 только администратором.
...
После того как администратор включит клиентскую модель для вашей организации, вы сможете создать языковую модель на основе данных из Microsoft 365.
...
В этом руководстве показано, как использовать данные Microsoft 365 для создания собственной модели распознавания речи, ее развертывания и использования с пакетом SDK Речи.
