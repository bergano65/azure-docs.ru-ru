...
description: Автоматическое создание пользовательской модели распознавания речи на основе данных Office365 позволяет обеспечить оптимальное качество распознавания применяемых в организации терминов в речи с высокой степенью безопасности и соответствия требованиям.
...
ms.locfileid: "73504598"
---
# <a name="create-a-tenant-model-preview"></a>Создание клиентской модели (предварительная версия)

Клиентская модель предоставляется корпоративным клиентам Office365 по запросу и позволяет автоматически создавать пользовательскую модель распознавания речи на основе данных Office365 вашей организации. Создаваемая модель будет оптимизирована для безопасного и соответствующего требованиям распознавания технических и жаргонных терминов, а также имен сотрудников, используемых в организации.

> [!IMPORTANT]
> Если ваша организация регистрируется для использования клиентской модели, служба "Речь" сможет получать доступ к ее языковой модели, которая формируется на основе таких ресурсов Office 365, как сообщения электронной почты и документы. Администратор Office 365 вашей организации может включать и отключать использование языковой модели на уровне организации с помощью портала администрирования Office 365.
...
![Схема клиентской модели](media/tenant-language-model/tenant-language-model-diagram.png)
...
2. На панели слева выберите **Параметры** и затем **Приложения**.
   ...
   ![Схема клиентской модели](media/tenant-language-model/tenant-language-model-enrollment.png)

3. Найдите и выберите **службы распознавания речи Azure**.

   ![Схема клиентской модели](media/tenant-language-model/tenant-language-model-enrollment-2.png)
...
3. На этом этапе появится сообщение о том, что вы можете создать клиентскую модель.
   > [!NOTE]
   > Создавать клиентскую модель на английском языке могут корпоративные клиенты Office 365 из Северной Америки. Если вы используете защищенное хранилище клиента (CLB) или ключ клиента (CK), эта возможность будет недоступна. Чтобы определить, используете ли вы защищенное хранилище клиента или ключ клиента, выполните следующие инструкции.
