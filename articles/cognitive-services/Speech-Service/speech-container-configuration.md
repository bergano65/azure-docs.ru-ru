...
Пользовательские модели загружаются при первом запуске новой модели в рамках пользовательской команды DOCKER. Последовательные запуски одного `ModelId` и того же для пользовательского контейнера речевого ввода будут использовать ранее скачанную модель. Если подключение тома не указано, пользовательские модели не могут быть сохранены.
...
| ----------- | ----- | ----------------- |
| **{API_KEY}** | Ключ конечной точки `Speech` ресурса на странице ключей Azure `Speech` .   | `xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`                                                                                  |
