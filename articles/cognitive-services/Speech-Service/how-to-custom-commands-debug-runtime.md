...
| Код ошибки | Сведения |
| ------- | -------- |
| 401 | Аусентикатионфаилуре: сбой обновления WebSocket с ошибкой проверки подлинности |
| 1000 | Превышено максимальное время простоя соединения WebSocket (> 300 000 мс) |
| 1002 | Сервер вернул код состояния "404", когда ожидался код состояния "101". |
...
Неактивные соединения прерываются сервером через 5 минут. Попробуйте повторно подключиться.
...
- Вы пытаетесь получить доступ к настраиваемому командному приложению за пределами вашего речевого ресурса
...
| ------- | --------------- | ----------- |
| макстурнсрешолдреачед | По истечении максимального разрешенного числа выполнений ход выполнения не выполнялся |
| рекогнизеркуотаексцеедед | Превышена квота на использование распознавателя |
| рекогнизерконнектионфаилед | Сбой подключения к распознавателю |
| рекогнизерунаусоризед | Доступ к этому приложению с текущей подпиской невозможен |
| рекогнизеринпутексцеедедалловедленгс | Длина входных данных превышает максимальную поддерживаемую длину распознавателя |
| рекогнизернотфаунд | Распознаватель не найден |
| рекогнизеринвалидкуери | Недопустимый запрос распознавателя |
| рекогнизереррор | Средство распознавания возвращает ошибку |
