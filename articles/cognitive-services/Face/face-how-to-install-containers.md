...
| Контейнер | Минимум | Рекомендуется | Транзакций в секунду<br>(Минимума, максимума)|
|-----------|---------|-------------|--|
...
> [!IMPORTANT]
> Cognitive Services контейнеры не лицензируются для запуска без подключения к Azure для отслеживания использования. Клиентам необходимо включить контейнеры для обмена данными для выставления счетов в службе контроля использования программных продуктов, все время. Контейнеры Cognitive Services не отправляют данные клиента, например изображение или текст, который анализируется, в корпорацию Майкрософт.
