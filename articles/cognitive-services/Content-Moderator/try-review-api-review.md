...
- **Содержимое**: в случае изображения и видео это строка URL-адреса, указывающая на содержимое. Для текстового содержимого это собственно текстовая строка.
...
Нажмите кнопку **Отправить**. Если операция выполнена, **состояние ответа** будет `200 OK`, а в поле **ответное содержимое** отобразится идентификатор проверки. Скопируйте этот идентификатор, чтобы использовать на следующих шагах.
...
В [средстве проверки](https://contentmoderator.cognitive.microsoft.com)выберите **проверка**  > **изображение** /**текст** /**видео** (в зависимости от используемого содержимого). Содержимое, которое вы перегрузили, должно отобразиться для человеческого анализа.
...
Нажмите кнопку **Отправить**. Если операция завершилась с ошибкой, **состояние отклика** будет `200 OK`, а в поле **ответное содержимое** отобразятся сведения о проверке в формате JSON, как показано ниже:
...
- **состояние**
- **ревиеверресулттагс**. отображается, если какие-либо теги были добавлены вручную командой экспертной проверки (отображается поле **createdBy** ).
- **метаданные**. здесь показаны теги, которые изначально были добавлены в проверку перед тем, как группа разработчиков по анализу внес изменения.
