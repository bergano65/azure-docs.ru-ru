...
1. Измените поле **текст запроса** , чтобы ввести запрос JSON с подробными сведениями о **описании** и **типе** (либо `Image`, либо `Text`).
...
Нажмите кнопку **Отправить**. Если операция выполнена с ошибкой, **состояние отклика** — `200 OK`, а в поле **ответное содержимое** отображается рабочий процесс в формате JSON, как показано ниже:
