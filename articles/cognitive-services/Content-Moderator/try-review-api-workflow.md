...
1. Измените поле **текст запроса** , чтобы ввести запрос JSON с подробными сведениями о **описании** и `Image` **типе** (либо `Text`).
...
Выберите **Отправить**. Если операция выполнена, **состояние ответа** — `200 OK`, а в поле **ответное содержимое** отображается рабочий процесс в формате JSON, как показано ниже:
