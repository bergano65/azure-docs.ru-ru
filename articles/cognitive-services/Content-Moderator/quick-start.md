...
- Реализация модерации текста. Используйте [консоль API](try-text-api.md) или использовать [ C# быстрого запуска](text-moderation-quickstart-dotnet.md) проверяемого текстового содержимого для ненормативную лексику, классификация машинная нежелательного текста (Предварительная версия) и личные данные.
