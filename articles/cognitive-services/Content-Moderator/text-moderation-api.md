...
description: Используйте модерацию текста для возможного нежелательного текста, личных данных и пользовательских списков терминов.
...
Используйте машинную модерацию текста и возможности [человеческого обзора](Review-Tool-User-Guide/human-in-the-loop.md) модератора с помощью машины для умерить содержание текста.
...
**Функция машинной классификации текста** С помощью машины модератора поддерживает **только английский язык**и помогает обнаруживать потенциально нежелательный контент. С ее помощью оценивается содержимое, которое может считаться недопустимым в определенном контексте. Она выдает вероятность для каждой категории и может рекомендовать пользовательскую проверку. Она использует обученную модель для выявления лексики оскорбительного, неуважительного или дискриминационного характера. Сюда входят сленговые и сокращенные выражения, оскорбления и специально искаженные слова, которые нужно проверять. 
...
Функция персональных данных определяет потенциальное наличие этой информации:
...
Предположим, что входная версия ("lzay" и "f0x" являются преднамеренными):
...
Проверьте в работе [консоль API для списков терминов](try-text-api.md) и примеры кода для REST API. Кроме того, проверить текст модерации разделе [.NET SDK quickstart,](dotnet-sdk-quickstart.md) если вы знакомы с Visual Studio и C .
