...
QnA Maker использует поддержку CMK из службы поиска Azure. Настройка [CMK в поиске Azure с помощью Azure Key Vault](https://docs.microsoft.com/azure/search/search-security-manage-encryption-keys). Этот экземпляр Azure должен быть связан со службой QnA Maker, чтобы сделать ее CMK включенной.

# <a name="qna-maker-managed-preview-release"></a>[Управляемый QnA Maker (предварительный выпуск)](#tab/v2)

QnA Maker использует [поддержку CMK из службы поиска Azure](https://docs.microsoft.com/azure/search/search-security-manage-encryption-keys)и автоматически связывает предоставленный CMK для шифрования данных, хранящихся в индексе службы поиска Azure.
...
2.  Выберите параметр " **управляемые ключи клиента** ". Укажите сведения о [ключах, управляемых клиентом](https://docs.microsoft.com/azure/storage/common/customer-managed-keys-configure-key-vault?tabs=portal) , и щелкните **сохранить**.
