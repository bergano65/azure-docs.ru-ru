...
**Ответ**. Конечная точка службы QnAMaker полезна в целях отладки при обращении в службу поддержки QnAMaker или UserVoice. Конечная точка является URL-адресом в `https://your-resource-name.azurewebsites.net`следующем формате:.
...
* Имя пользовательского поддомена опубликованной конечной точки базы знаний, `host`известное как, найдено на странице **параметров** после публикации.
