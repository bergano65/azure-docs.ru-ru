...
* [Получение сведений о базе знаний](https://westus.dev.cognitive.microsoft.com/docs/services/5a93fcf85b4ccd136866eb37/operations/knowledgebases_getknowledgebasedetails) 
* [Получение конечных точек базы знаний](https://westus.dev.cognitive.microsoft.com/docs/services/5a93fcf85b4ccd136866eb37/operations/endpointkeys_getendpointkeys)
...
Чтобы обращаться к базе знаний для отправки вопроса и получения наилучшего ответа, программе требуется _узел конечной точки_ из API сведений о базе знаний и _ключ первичной конечной точки_ из API конечных точек. Эти методы описаны в следующих разделах вместе с методом создания ответа. 

В следующей таблице показано использование данных для создания URI:

|Создание шаблона URI ответа|
|--|
|https://**имя_узла**.azurewebsites.net/qnamaker/knowledgebases/**ИД базы знаний**/generateAnswer|
...
|--|--|
|Авторизация|`Endpoint` + **основная конечная точка**<br>Пример: `Endpoint xxxxxxx`<br>Обратите внимание на пробел между текстом `Endpoint` и значением первичной конечной точки. 
...
Добавьте следующий метод, чтобы получить сведения о базе знаний. Эти данные содержат имя узла базы знаний. Имя узла — это имя веб-службы Azure QnA Maker, которое вы ввели при создании ресурса QnA Maker. 
