...
---
# <a name="tutorial-create-a-qna-bot-with-azure-bot-service-v4"></a>Учебник. Создание программы-робота, QnA с помощью Azure Служба программ-роботов v4
...
1. На портале QnA Maker, перейдите к **публикации** страницы и публикации в базе знаний. Выберите **создания программ-роботов**. 
    ...
    ![На портале QnA Maker перейдите на страницу публикации и публикации в базе знаний. Нажмите кнопку Создать программ-роботов.](../media/qnamaker-tutorials-create-bot/create-bot-from-published-knowledge-base-page.png)
...
1.  Введите параметры для создания программ-роботов.
    ...
    |Размещение|`west us`|Расположение ресурсов Azure bot.|
    ...
    |Ключ проверки подлинности QnA|**Не менять**|Это значение заполняется автоматически.|
    |План службы приложений или расположение|**Не менять**|В этом учебнике расположение не имеет значения.|
    |Служба хранилища Azure|**Не менять**|Диалог данные хранятся в таблицах хранилища Azure.|
    |Application Insights|**Не менять**|Ведение журнала, отправляемых в Application Insights.|
    |Microsoft App ID (Идентификатор приложения Майкрософт)|**Не менять**|Active directory пользователя и пароль не требуется.|
