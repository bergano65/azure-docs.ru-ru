...
* **Если нужно давать один ответ на одинаковые вопросы, команды и запросы**. Когда разные пользователи задают одинаковые вопросы, всем им предоставляется один и тот же ответ. 
...
|3|Клиентское приложение использует ответ в формате JSON для принятия решений о том, как продолжить беседу. Таким решением может быть предоставление наиболее подходящего ответа или дополнительных вариантов для уточнения поиска лучшего ответа. |
|||
...
Набор вопросов и ответов содержит все варианты вопроса для определенного ответа, теги метаданных, используемые для фильтрации вариантов ответа во время поиска, и дальнейшие запросы для уточнения поиска.
...
**Шаг 3**. Опубликуйте базу знаний и протестируйте ее из пользовательской конечной точки с помощью [cURL](../quickstarts/get-answer-from-kb-using-curl.md) или [Postman](../quickstarts/get-answer-from-kb-using-postman.md). 

**Шаг 4**. В клиентском приложении программными средствами вызовите конечную точку базы знаний и просмотрите полученный файл JSON с наиболее подходящим ответом на вопрос пользователя.  
...
* Июнь 2019 г.
    * Улучшена модель ранжирования для французского, итальянского, немецкого, испанского и португальского языков.
* Апрель 2019 г.
    * Добавлена возможность извлечения содержимого веб-сайтов.
    * Добавлена поддержка документов SharePoint.
* Март 2019 г.
    * Активное обучение 
    * Улучшенная модель ранжирования и обработки естественного языка для английского языка. 
