...
1. Чтобы просмотреть предлагаемые вопросы, на странице « **изменение** базы знаний» выберите **Просмотр параметров**, а затем выберите **Показать активные предложения по обучению**. 
    ...
    [![On. в разделе "Редактирование" на портале выберите Показать предложения, чтобы просмотреть альтернативы нового вопроса активного обучения.](../media/improve-knowledge-base/show-suggestions-button.png)](../media/improve-knowledge-base/show-suggestions-button.png#lightbox)
...
1. Отфильтруйте базу знаний с помощью пар вопросов и ответов, чтобы отображались только предложения, выбрав **Фильтр по предложениям**.
    ...
    [![Use переключатель фильтровать по предложению для просмотра только альтернативных вариантов вопроса для активного обучения.](../media/improve-knowledge-base/filter-by-suggestions.png)](../media/improve-knowledge-base/filter-by-suggestions.png#lightbox)

1. Каждая пара QnA предлагает новые альтернативные варианты вопроса с галочкой, `✔`, чтобы принять вопрос или `x`, чтобы отклонить предложения. Установите флажок, чтобы добавить вопрос. 

    [![Select или отклоните альтернативные варианты вопросов активного обучения, выбрав зеленую галочку или красную метку удаления.](../media/improve-knowledge-base/accept-active-learning-suggestions.png)](../media/improve-knowledge-base/accept-active-learning-suggestions.png#lightbox)
...
При отправке вопроса в QnA Maker для ответа в свойстве `top` тела JSON задается число возвращаемых ответов. 
...
|Свойство HTTP-запроса|Name|Тип|Цель|
|--|--|--|--|
...
|`userQuestion`|string|Точный текст запроса пользователя. Обязательный параметр.|
...
В клиентском приложении, таком как Bot, можно сохранить данные, а затем отправить много записей в один текст JSON в массиве `feedbackRecords`. 
...
Если в приложении включено активное обучение и вы экспортируете приложение, то в столбце `SuggestedQuestions` в TSV-файле будут сохранены активные данные для обучения. 

Столбец `SuggestedQuestions` представляет собой объект JSON со сведениями о неявном, `autosuggested` и явной обратной связи `usersuggested`. Пример этого объекта JSON для одного отправленного пользователем вопроса `help`:
...
* [ПАКЕТ SDK .NET](https://docs.microsoft.com/dotnet/api/microsoft.azure.cognitiveservices.knowledge.qnamaker.alterationsextensions.getasync?view=azure-dotnet)
