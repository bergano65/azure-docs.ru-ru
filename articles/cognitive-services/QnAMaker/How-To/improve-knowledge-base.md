...
Используйте [образец программ-роботов Azure](https://aka.ms/activelearningsamplebot) чтобы увидеть активное обучение в сценарии end-to-end.
...
|Верхний колонтитул|Content-Type|строка|Тип носителя текста, отправляемого в API. Значение по умолчанию —: `application/json`|
...
`SuggestedQuestions` Столбец представляет собой объект JSON, сведения о неявных (`autosuggested`) и явного (`usersuggested`) обратной связи. Пример объекта JSON для один пользователь отправил вопрос `help` является:
