...
* [Создайте службу QnA Maker](create-publish-knowledge-base.md#create-a-new-qna-maker-knowledge-base) или используйте существующую, в которой будет размещен представленный в этом кратком руководстве примера документа на английском языке.
...
1. На **шаге 4**установите флажок **Enable multi-turn extraction from URLs, .pdf or .docx files** (Включить многоэтапное извлечение из URL-адресов, файлов .pdf или .docx).
1. В поле **Default answer text** (Текст ответа по умолчанию) введите значение `Quickstart - can't find answer`. В рабочей базе знаний эти сведения должны быть более полезными для пользователя, но для этого краткого руководства подойдет простой ответ.
1. В том же **шаге 4** щелкните **+ Add file** (+ Добавить файл) и выберите скачанный файл `.docx`, который указан в списке необходимых компонентов.
