...
|--|--|--|--|
|Имя (ключ)|Здесь<br>буквенно-цифровые (буквы и цифры)<br>`_` (подчеркивание)<br> Не должно содержать пробелов.|`^[a-zA-Z0-9_]+$`|100|
|Значение|Разрешает все, Кроме<br>`:` (двоеточие)<br>`|` (вертикальный канал)<br>Разрешено только одно значение.|`^[^:|]+$`|500|
|||||
