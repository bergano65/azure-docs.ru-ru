...
Можно создать резервную копию базы знаний, экспортировав базу знаний в `.tsv` или `.xls` формате. После экспорта включите этот файл как часть обычной проверки системы управления версиями.
...
### <a name="test-knowledge-base"></a>Проверить базу знаний

*База знаний тестирования* — это версия, которая в настоящий момент редактируется, сохраняется и проверяется на точность и полноту ответов. Изменения, вносимые в базу знаний тестирования, не влияют на конечного пользователя приложения или чата. База знаний теста называется `test` в HTTP-запросе. `test` знания доступны в области интерактивного **теста** портала QnA Maker.
