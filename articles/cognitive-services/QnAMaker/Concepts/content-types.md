...
QnA Maker добавляет содержимое файлов и URL-адресов, преобразуя содержимое в наборы QnA, сохраненные как markdown (`.md`). Содержимое подходит для базы знаний, где содержимое организовано в виде структурированной формы и представлено в четко определенных разделах. Разделы можно разбить на подразделы или подразделы. Извлечение лучше всего работает с документами, которые имеют четкую структуру с иерархической структурой заголовков.

QnA Maker определяет разделы и подразделы и связи в содержимом на основе визуальных подсчетов, таких как размер шрифта, стиль шрифта, нумерация, цвета и т. д. Частично структурированное содержимое включает руководства, часто задаваемые вопросы, рекомендации, политики, буклеты, объявления и многие другие типы файлов.
...
Дополнительные сведения см. в [справочной документации по QnA Maker Markdown](../reference-markdown-format.md).
...
* Сведения о проектировании и управлении [наборами вопросов и ответов (QnA)](question-answer-set.md)