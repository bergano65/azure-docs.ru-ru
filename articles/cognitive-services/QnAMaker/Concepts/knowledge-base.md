...
* **Ответы**: Ответ — это ответ, возвращаемый при сопоставлении пользовательского запроса с соответствующим вопросом.  
...
|4|Этот измененный запрос отправляется в индекс службы поиска Azure, который `top` получает количество результатов. Если нужный ответ не указан в этих результатах, увеличьте значение `top` немного. Как правило, значение 10 для `top` Works в 90% запросов.|
|5|QnA Maker применяет расширенные Добавление признаков, чтобы определить правильность полученного результатов поиска Azure для пользовательского запроса. |
