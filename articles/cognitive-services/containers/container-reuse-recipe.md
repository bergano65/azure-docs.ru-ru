...
description: Используйте эти рецепты контейнеров для создания Cognitive Services контейнеров, которые можно использовать повторно. Контейнеры можно построить с помощью некоторых или всех параметров конфигурации, чтобы они не требовались при запуске контейнера. После создания нового слоя контейнера (с параметрами) и его локального тестирования можно сохранить контейнер в реестре контейнеров. При запуске контейнера потребуются только те параметры, которые в данный момент не хранятся в контейнере.
...
* `docker run`в примерах можно использовать каталог `c:` на диске, чтобы избежать конфликтов разрешений в Windows. Если вам нужен конкретный каталог для входных данных, может потребоваться предоставить соответствующие разрешения службе Docker.
...
* Задает конечную точку `{BILLING_ENDPOINT}` выставления счетов из ключа среды узла с `ENV`помощью.
* Задает ключ `{ENDPOINT_KEY}` API выставления счетов из ключа среды узла с помощью "env.
...
* Контейнер LUIS поддерживает более одной модели. Если все модели хранятся в одной и той же папке, все они должны `COPY` иметь одну инструкцию.
* Запустите файл DOCKER из относительного родителя входного каталога модели. В следующем примере выполните `docker build` команды и `docker run` `/input`из относительного родителя. `/input` Первый`COPY` в команде — это каталог главного компьютера. Второй `/input` — каталог контейнера.
...
1. `Dockerfile` Создайте с помощью рецепта "текст из повторного использования". `Dockerfile` Не имеет расширения.
...
1. Создайте файл в образе в командной строке или терминале, используя следующую команду. Замените значения в угловых скобках `<>`на собственное имя контейнера и тег.  
    ...
    Параметр тега, `-t`— это способ добавления сведений о том, что было изменено для контейнера. Например, имя `modified-LUIS` контейнера указывает, что исходный контейнер был слоем. Имя `with-billing-and-model` тега показывает, как был изменен контейнер Language Understanding (Luis).
...
1. Войдите в свой частный реестр с Azure CLI из консоли.
    ...
    Замените значения в угловых скобках `<my-registry>`на собственное имя реестра.  
...
1. Пометьте контейнер с частным расположением реестра. Замените значения в угловых скобках `<my-registry>`на собственное имя реестра. 
