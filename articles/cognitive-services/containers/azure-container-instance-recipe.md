...
* Один контейнер Cognitive Services на локальном узле (ваш компьютер). Убедитесь, что вы можете:
  * Вытяните образ с помощью команды `docker pull`.
  * Запустите локальный контейнер, чтобы успешно выполнить все необходимые параметры конфигурации с помощью команды `docker run`.
