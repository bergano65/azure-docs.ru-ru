...
Если приложение имеет от 10 до 20 фраз с разной длиной предложения, другим порядком слов и даже разными словами (синонимы "подчиненные", "управлять", "отчет"), LUIS может вернуть низкий показатель точности. Создайте шаблон, чтобы помочь понять важность порядок слов, LUIS. 
...
|Пример вложенных необязательно utterance|Пояснение|
|--|--|
...
**Группирования** синтаксис со скобками, может быть двух уровней вложенности. Например, `(({Entity1.RoleName1} | {Entity1.RoleName2} ) | {Entity2} )`. Это позволяет любой из трех сущностей для сопоставления. 
...
|Пример utterance вложенные группировки|Пояснение|
|--|--|
