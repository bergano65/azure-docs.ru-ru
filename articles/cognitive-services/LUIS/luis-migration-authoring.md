...
## <a name="troubleshooting-the-migration-process-for-luis-authoring"></a>Устранение неполадок в процессе миграции для создания LUIS

* Ключи разработки LUIS видны только на портале LUIS после завершения процесса миграции. При создании ключей создания, например с помощью интерфейса командной строки LUIS, пользователю по-прежнему потребуется выполнить процесс миграции на портале LUIS.
...
* Если неперенесенный пользователь не является владельцем каких-либо приложений, но является членом совместной работы с другими приложениями, принадлежащими другим пользователям, и эти владельцы прошли процесс миграции, этот пользователь должен будет выполнить миграцию, чтобы получить доступ к приложениям.
...
В процессе миграции возникает ошибка, если:
* Ваша подписка не разрешает вам создавать ресурсы Cognitive Services
* Миграция негативно влияет на любую среду выполнения приложений. При миграции все участники совместной работы удаляются из приложений и удаляются в качестве участника совместной работы из других приложений. Этот процесс означает, что назначенные ключи также будут удалены. Миграция блокируется, если вы назначили ключи в других приложениях. Перед миграцией удалите ключ, который был назначен безопасно. Если известно, что назначенный ключ не используется в среде выполнения, необходимо удалить его, чтобы иметь возможность выполнить миграцию.

Получите доступ к списку ресурсов Azure приложения, используя следующий формат URL-адреса:
