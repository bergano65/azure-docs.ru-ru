...
Миграция — это процесс изменения проверки подлинности с учетной записи электронной почты на ресурс Azure. После миграции ваша учетная запись будет связана с подпиской Azure и ресурсом разработки Azure. *Все пользователи LUIS (владельцы приложений и участники совместной работы), в конечном итоге, потребуют миграции.*
...
## <a name="notes-before-you-migrate"></a>Примечания перед миграцией

* Миграция не может быть отменена.
...
* После миграции владельца приложения будут скрыты от учетной записи участника совместной работы.
...
* Приложения не будут переноситься вместе с вами, если вы являетесь членом программы совместной работы. Однако участники совместной работы получат запрос на экспорт необходимых приложений.
* Владелец не может понять, перенесены ли участники совместной работы.
...
* После назначения совместной работы ресурсу Azure им потребуется выполнить миграцию, прежде чем они смогут получить доступ к приложениям. В противном случае у них не будет доступа для создания приложений.
...
* Вам нужно создать ресурс Azure LUIS Authoring на портале LUIS или [портал Azure](https://portal.azure.com/#create/Microsoft.CognitiveServicesLUISAllInOne). Создание ресурса-разработчика на портале LUIS является частью процесса миграции, описанного в следующем разделе.
