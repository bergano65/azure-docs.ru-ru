...
|Прошедшая машинное обучение|Можно пометить|Руководство|Пример<br>Ответ|Тип сущности|Назначение|
|--|--|--|--|--|--|
...
## <a name="machine-learned-entities-use-context"></a>Контекст использования сущностей результаты обучения компьютера

Результаты обучения компьютера сущности сведения из контекста в utterance. Это придает вариант размещения в пример фразы. 

## <a name="non-machine-learned-entities-dont-use-context"></a>Сущности не узнали машины не используют контекст

Следующая не машина узнал сущностей учитывает контекст utterance при сопоставлении сущностей: 

* [Предварительно созданные сущности](#prebuilt-entity)
* [Объекты регулярных выражений](#regular-expression-entity)
* [Сущности списка](#list-entity) 

Эти сущности не требуют добавления метки или обучения модели. Как только при добавлении или настройке сущности, извлекаются сущности. Недостаток заключается в том, можно, эти сущности быть overmatched, с которой контекста была создана в учетную запись, соответствие бы не были ли внесены. 

Это осуществляется с помощью списка сущностей на новые модели часто. Время сборки и тестирования модели с сущностью списка, но при публикации модели и получать запросы из конечной точки, вы поймете, что модель overmatching из-за отсутствия контекста. 

Если вы хотите совпадение слов или фраз и с учетом контекста, можно двумя способами. Первый — для использования простого объекта в паре с списка фраз. Список фразы не будет использоваться для сопоставления, но вместо поможет относительно схож слова сигнала (взаимозаменяемыми список). Если необходимо иметь точное соответствие вместо списка фраз вариации, используйте объект списка с ролью, описанных ниже.

### <a name="context-with-non-machine-learned-entities"></a>Контекст с сущностями, узнали без машин

Если требуется контекст utterance значимыми для известных сущностей без машин, следует использовать [ролей](luis-concept-roles.md).

Если у вас есть сущности, узнали без машин, таких как [предварительно созданных сущностей](#prebuilt-entity), [regex](#regular-expression-entity) сущностей или [списка](#list-entity) сущности, которые соответствовал далеко за рамки экземпляра, требуется, рассмотрите возможность Создание одной сущности с двумя ролями. Одна роль будет записан, что вы ищете и одна роль будет записан, что вы не ищете. Обе версии необходимо задавать метки в пример фразы.  
...
* Редко изменяются. Если вам нужно изменить список часто или должен самостоятельно разверните список, простой сущности, увеличивающиеся со фразы списком является лучшим выбором. 
* Набор не превышает максимальные [ограничения](luis-boundaries.md) LUIS для этого типа сущности.
* Текст в высказывании в точности совпадает с синонимом или каноническим именем. LUIS не использует список вне точных текстовых совпадений. Нечеткое соответствие, многостроковое, морфологического поиска, множественного числа и другие варианты с сущностью списка не разрешаются. Для управления вариантами рекомендуется применять [шаблон](luis-concept-patterns.md#syntax-to-mark-optional-text-in-a-template-utterance) с синтаксисом, имеющим необязательный текст.
...
|Фраза|
|--|
|Книга The Man Who Mistook His Wife for a Hat and Other Clinical Tales (Человек, который принял жену за шляпу, и другие истории из врачебной практики) была написана американским автором в этом году?<br><br>Книга **The Man Who Mistook His Wife for a Hat and Other Clinical Tales (Человек, который принял жену за шляпу, и другие истории из врачебной практики)** была написана американским автором в этом году?|
|Книга Half Asleep in Frog Pajamas (Сонные глазки и пижама в лягушечку) была написана американским автором в этом году?<br><br>Книга **Half Asleep in Frog Pajamas (Сонные глазки и пижама в лягушечку)** была написана американским автором в этом году?|
|Роман The Particular Sadness of Lemon Cake (Особая печаль лимонного торта) был написан американским автором в этом году?<br><br>Роман **The Particular Sadness of Lemon Cake (Особая печаль лимонного торта)**  был написан американским автором в этом году?|
|Книга There's A Wocket In My Pocket! (Кто живет в моем кармане?) была написана американским автором этом году?<br><br>Книга **There's A Wocket In My Pocket! (Кто живет в моем кармане?)** была написана американским автором этом году?|
||
...
Регулярные выражения могут соответствовать более, чем ожидалось в соответствии с. Примером этого является числовой word, такие как сопоставление `one` и `two`. Например, следующему регулярному выражению, который совпадает с числом, `one` вместе с другими числами:

```javascript
(plus )?(zero|one|two|three|four|five|six|seven|eight|nine)(\s+(zero|one|two|three|four|five|six|seven|eight|nine))*
``` 

Это выражение регулярное выражение также соответствует слов, которые заканчиваются эти числа, такие как `phone`. Чтобы устранить подобные проблемы, убедитесь, что регулярное выражение соответствует учитывает границы слов в учетной записи. Регулярное выражение для использования границ слов в этом примере используется в следующему регулярному выражению:
