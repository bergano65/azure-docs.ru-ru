...
* Следующие предварительно созданные сущности имеют разные ответы JSON:
    ...
    * Имя ключа измеряемого блока `units` из в`unit`
...
|--|--|--|
|V3|GET|HTTPS://<b>{регион}</b>. API.Cognitive.Microsoft.com/Luis/<b>PREDICTION</b>/<b>v 3.0</b>/АППС/<b>{App-ID}</b>/Слотс/<b>{имя слота}</b>/предикт? запрос =<b>{запрос}</b>|
|V3|POST|HTTPS://<b>{регион}</b>. API.Cognitive.Microsoft.com/Luis/<b>PREDICTION</b>/<b>v 3.0</b>/АППС/<b>{App-ID}</b>/Слотс/<b>{слот-Name}</b>/предикт|
|V2|GET|HTTPS://<b>{регион}</b>. API.Cognitive.Microsoft.com/Luis/<b>PREDICTION</b>/<b>v 3.0</b>/АППС/<b>{App-ID}</b>/версионс/<b>{версия-ID}</b>/предикт? запрос =<b>{Query}</b>|
|V2|POST|HTTPS://<b>{регион}</b>. API.Cognitive.Microsoft.com/Luis/<b>PREDICTION</b>/<b>v 3.0</b>/АППС/<b>{App-ID}</b>/версионс/<b>{версия-ID}</b>/предикт|
...
Если требуются метаданные сущности, то в строке запроса необходимо использовать `verbose=true` флаг, а ответ содержит метаданные в `$instance` объекте. Примеры показаны в ответах JSON в следующих разделах.
