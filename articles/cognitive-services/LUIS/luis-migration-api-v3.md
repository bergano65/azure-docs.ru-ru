...
|`query`|строка|Только версия 3|Обязательный элемент.|**В версии 2**utterance для прогнозирования находится в параметре `q`. <br><br>**В версии 3**функциональные возможности передаются в параметр `query`.|
