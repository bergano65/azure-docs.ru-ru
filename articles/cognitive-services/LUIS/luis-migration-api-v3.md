...
## <a name="endpoint-url-changes-by-slot-name"></a>Изменение URL-адреса конечной точки, имя слота

Формат вызова конечной точки HTTP V3 изменился.

|МЕТОД|URL-адрес|
|--|--|
|GET|https://<b>{REGION}</b>.api.cognitive.microsoft.com/luis/<b>Предварительная версия v3.0</b>/apps/<b>{APP-ID}</b>/slots/<b>{имя СЛОТА}</b>/ прогнозирования? запрос =<b>{QUERY}</b>|
|POST|https://<b>{REGION}</b>.api.cognitive.microsoft.com/luis/<b>Предварительная версия v3.0</b>/apps/<b>{APP-ID}</b>/slots/<b>{имя СЛОТА}</b>/ прогнозирования|
|||

## <a name="endpoint-url-changes-by-version-id"></a>Изменение URL-адреса конечной точки, идентификатор версии

Если вы хотите запрашивать по версии, необходимо сначала [публикацию через API](https://westus.dev.cognitive.microsoft.com/docs/services/5890b47c39e2bb17b84a55ff/operations/5890b47c39e2bb052c5b9c3b) с `"directVersionPublish":true`. Запрос ссылается на идентификатор версии, а не имя слота конечной точки.
...
|POST|https://<b>{REGION}</b>.api.cognitive.microsoft.com/luis/<b>Предварительная версия v3.0</b>/apps/<b>{APP-ID}</b>/versions/<b>{ИД}</b>/ Прогнозирование|
|||
...
|Имя param|type|Version|значение по умолчанию|Назначение|
|--|--|--|--|--|
|`log`|boolean|V2 И V3|false|Query Store в файле журнала.| 
|`query`|string|Только версия 3|По умолчанию - это необходимо в запросе GET|**В версии 2**, utterance в качестве прогнозируемых находится в `q` параметра. <br><br>**В версии 3**, переданный функции `query` параметра.|
|`show-all-intents`|boolean|Только версия 3|false|Возвращает все объекты Intent с соответствующую оценку в **prediction.intents** объекта. Объекты Intent возвращаются в виде объектов в родительской `intents` объекта. Это обеспечивает программный доступ без необходимости поиска цель в массиве: `prediction.intents.give`. В версии 2 они были возвращены в массиве. |
|`verbose`|boolean|V2 И V3|false|**В версии 2**, если задано значение true, все прогнозируемые намерения были возвращены. Все прогнозируемые объекты Intent, используйте параметр V3 типа `show-all-intents`.<br><br>**В версии 3**, этот параметр предоставляет только сущности сведения о метаданных сущности прогноза.  |
...
|Свойство|type|Version|значение по умолчанию|Назначение|
|--|--|--|--|--|
|`dynamicLists`|массив|Только версия 3|Необязательно.|[Динамические списки](#dynamic-lists-passed-in-at-prediction-time) позволяют расширять обученной и опубликованы в списке сущность, уже в приложении LUIS.|
|`externalEntities`|массив|Только версия 3|Необязательно.|[Внешние сущности](#external-entities-passed-in-at-prediction-time) предоставить приложением LUIS возможность идентифицировать и помечать сущности во время выполнения, который можно использовать в качестве функции для существующих сущностей. |
|`options.datetimeReference`|строка|Только версия 3|по умолчанию|Используется для определения [datetimeV2 смещение](luis-concept-data-alteration.md#change-time-zone-of-prebuilt-datetimev2-entity).|
|`options.overridePredictions`|boolean|Только версия 3|false|Указывает, если пользователя [внешней сущности (с тем же именем, что существующая сущность)](#override-existing-model-predictions) используется или существующую сущность в модели используется для прогнозирования. |
|`query`|строка|Только версия 3|Обязательный элемент.|**В версии 2**, utterance в качестве прогнозируемых находится в `q` параметра. <br><br>**В версии 3**, переданный функции `query` параметра.|
...
### <a name="override-existing-model-predictions"></a>Переопределить существующие модели прогнозов

`overridePredictions` Параметры свойство указывает, что если пользователь отправляет внешнюю сущность, перекрывается с прогнозируемое сущности с одинаковым именем, LUIS выбирает передаваемая сущность или сущность, существующих в модели. 

Например, рассмотрим запрос `today I'm free`. Обнаруживает LUIS `today` как datetimeV2 следующим образом:
...
Если пользователь отправляет внешней сущности:
...
Если `overridePredictions` присваивается `false`, LUIS возвращает ответ, как если бы не отправлены внешней сущности. 
...
Если `overridePredictions` присваивается `true`, LUIS возвращает ответ, содержащий:
...
Все API версии 2 не будет считаться устаревшим для по крайней мере 9 месяцев после предварительной версии V3. 
