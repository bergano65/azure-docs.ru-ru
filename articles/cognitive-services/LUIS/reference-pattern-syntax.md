...
|или| \| — Вертикальная черта (вертикальная черта)<br><br>На вертикальных панелях (или) в одной группе имеется ограничение 2 |-|Где имеет форму ({Form-Name-Short} &#x7c; {форма-Name-Long} &#x7c; {форма-номер})|
...
**Необязательный** синтаксис с квадратными скобками может быть вложен на два уровня. Например, так: `[[this]is] a new form`. Этот пример позволяет выполнять следующие фразы продолжительностью:
...
Синтаксис **группирования** с круглыми скобками может быть вложен двумя уровнями. Например, так: `(({Entity1.RoleName1} | {Entity1.RoleName2} ) | {Entity2} )`. Эта функция позволяет сопоставлять любую из трех сущностей.
