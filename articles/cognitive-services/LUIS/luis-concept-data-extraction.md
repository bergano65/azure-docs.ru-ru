...
description: Извлекайте данные из текста высказывания с намерениями и сущностями. Узнайте, какие данные можно извлечь из Language Understanding (LUIS).
author: diberry
ms.topic: conceptual
ms.date: 01/23/2020
...
ms.locfileid: "79221087"
---
# <a name="extract-data-from-utterance-text-with-intents-and-entities"></a>Извлекайте данные из текста высказывания с намерениями и сущностями
...
Если ваш чат-бот или ПРИЛОЖЕНИЕ для вызова LUIS принимает решение, основанное на нескольких оценках намерений, верните все баллы умысла.


#### <a name="v2-prediction-endpoint-response"></a>[Отклик конечной точки прогнозирования V2](#tab/V2)

Установите параметр запроса, `verbose=true`. Ответ конечной точки:
...
Установите параметр запроса, `show-all-intents=true`. Ответ конечной точки:
...
|--|--|--|--|--|
|Техническое оборудование|Блокировка с намерением|Строка|intents[0].intent|"<b>Utilities</b>.ShowNext"|
|Обмен данными|Блокировка с намерением|Строка|intents[1].intent|<b>Communication</b>.StartOver"|
...
Просмотрите [поддержку токенов](luis-language-support.md#tokenization) в LUIS.
...
Композитная [сущность](reference-entity-composite.md) состоит из других сущностей, таких как предварительно построенные сущности, простые, регулярные выражения и объекты списка. Эти отдельные сущности формируют единую сущность.

## <a name="list-entity-data"></a>Данные сущности списка

[Объекты списка](reference-entity-list.md) представляют собой фиксированный, закрытый набор связанных слов вместе с их синонимами. LUIS не обнаруживает дополнительные значения для сущностей списка. Посмотреть предложения для новых слов на основе текущего списка можно с помощью функции **Рекомендовать**. Если имеется несколько сущностей списка с одним и тем же значением, в запросе конечной точки возвращается каждая сущность.
...
Без параметра запроса: `verbose=true`
...
С параметром запроса: `verbose=true`
...
[Регулярная сущность выражения](reference-entity-regular-expression.md) извлекает сущность на основе предоставляемого вами шаблона регулярного выражения.
...
В зависимости от языка и региональных параметров формат имен людей может незначительно отличаться. Используйте либо заранее построенную **[сущность personName,](luis-reference-prebuilt-person.md)** либо **[простую сущность](luis-concept-entity-types.md#simple-entity)** с [ролями](luis-concept-roles.md) имени и фамилии.

Если вы используете простую сущность, не забудьте привести примеры, которые используют имя и фамилию в разных частях высказывания, в высказываниях различной длины, и высказывания во всех намерениях, включая намерения None. Регулярно [просматривайте](luis-how-to-review-endoint-utt.md) высказывания конечной точки, чтобы помечать имена, которые были неправильно спрогнозированы.

### <a name="names-of-places"></a>Имена мест

Имена местоположений устанавливаются и известны, такие как города, округа, штаты, провинции и страны/регионы. Используйте заранее построенную **[сущность geographyV2](luis-reference-prebuilt-geographyv2.md)** для извлечения информации о местоположении.
...
Сущность `Location`имя , с `Origin` `Destination`двумя ролями, и .
...
В V3 **имя роли** является основным названием объекта.
...
[Pattern.any](reference-entity-pattern-any.md) — это заполнитель переменной длины, используемый только в высказывании шаблона шаблона для обозначения того, где сущность начинается и заканчивается.
...
Конечная точка LUIS может обнаружить одни и те же данные в разных сущностях.
...
Без `verbose=true` параметра запроса.
...
С `verbose=true` в качестве параметра запроса.
...
Без `verbose=true` строки запроса:
