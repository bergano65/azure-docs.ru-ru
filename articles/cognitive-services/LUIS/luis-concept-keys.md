...
ms.locfileid: "70256955"
---
# <a name="authoring-and-runtime-keys"></a>Создание и ключи среды выполнения
...
|Ключ|Цель|Служба для переприятия`kind`|Служба для переприятия`type`|
|--|--|--|--|
...
Создание ключей разработки в портал Azure позволяет управлять разрешениями для ресурсов разработки путем назначения пользователей [роли участника](#contributions-from-other-authors). Для добавления участников требуется разрешение на уровне подписки Azure. 
...
Конечная точка, используемая для доступа к среде выполнения, использует поддомен, уникальный для региона ресурса, `{region}` который указан в следующей таблице. 
...
**Для [создания перенесенных приложений ресурсов](luis-migration-authoring.md)** : 
