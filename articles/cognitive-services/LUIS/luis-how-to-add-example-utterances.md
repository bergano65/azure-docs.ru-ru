...
1. Выберите `SQL server` в высказывании, чтобы отметить его как простую сущность. В окне раскрывающего списка сущностей можно выбрать имеющийся объект или добавить новую сущность. Чтобы добавить новую сущность, введите ее имя `Job` в текстовом поле, а затем выберите **создать новую сущность**.
...
При условии, что utterance, `Does John Smith work in Seattle?`, составной utterance может вернуть сведения о сущности для имени сотрудника `John Smith`и расположение `Seattle` в составной сущности. Дочерние сущности должны уже существовать в приложении и быть помечены в примере utterance перед созданием составной сущности.
