...
Пакетная диаграмма отображает четыре квадранта результатов. В правой части диаграммы находится фильтр. По умолчанию фильтр устанавливается в первом намерении в списке. Фильтр содержит все объекты и только простые и составные сущности. При выборе [раздела диаграммы](luis-concept-batch-test.md#batch-test-results) или точки внутри диаграммы соответствующие высказывания отображаются под диаграммой. 
...
Чтобы проверить сущности в пакетном тесте, они должны быть помечены в пакетном файле в формате JSON. Используются только сущности, изученные компьютером: простые и составные сущности. Не добавляйте машинно-необученные сущности, потому что они всегда встречаются либо с помощью регулярных выражений, либо с явным текстовым совпадением.
