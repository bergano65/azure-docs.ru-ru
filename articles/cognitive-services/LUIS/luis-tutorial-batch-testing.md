...
* Без дубликатов. 
* Типы сущностей, которые разрешено: только сущности, генерируемое системой машинам, простого и составного. Пакетное тестирование полезно только для машинно-обученных намерений и сущностей.
...
Пакетная диаграмма отображает четыре квадранта результатов. В правой части диаграммы находится фильтр. По умолчанию фильтр устанавливается в первом намерении в списке. Фильтр содержит намерения и сущности только для простых и составных. При выборе [раздела диаграммы](luis-concept-batch-test.md#batch-test-results) или точки внутри диаграммы соответствующие высказывания отображаются под диаграммой. 
...
Чтобы проверить сущности в пакетном тесте, они должны быть помечены в пакетном файле в формате JSON. Используются только результаты обучения компьютера сущности: простых и составных сущностей. Не добавляйте машинно-необученные сущности, потому что они всегда встречаются либо с помощью регулярных выражений, либо с явным текстовым совпадением.
...
Диаграмма открывается со всеми намерениями, которые были правильно спрогнозированы. Прокрутите вниз справа фильтр, чтобы найти сущность прогнозы с ошибками. 

1. Выберите сущность **Job** в фильтре.

    ![Ошибка сущности прогнозов в фильтре](./media/luis-tutorial-batch-testing/hr-entities-filter-errors.png)
