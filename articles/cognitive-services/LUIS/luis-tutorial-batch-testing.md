...
ms.locfileid: "69637451"
---
# <a name="tutorial-batch-test-data-sets"></a>Учебник. Пакетное тестирование наборов данных
...
* Без дубликатов. 
* Допустимые типы сущностей: только машинные объекты простого и составного уровня. Пакетное тестирование полезно только для машинно-обученных намерений и сущностей.
...
5. Назовите набор данных `intents only` и выберите **Готово**.
    ...
    ![Выберите файл](./media/luis-tutorial-batch-testing/hr-import-new-dataset-ddl.png)
...
Пакетная диаграмма отображает четыре квадранта результатов. В правой части диаграммы находится фильтр. Фильтр содержит сведения и сущности. При выборе [раздела диаграммы](luis-concept-batch-test.md#batch-test-results) или точки внутри диаграммы соответствующие высказывания отображаются под диаграммой. 
...
Результаты теста **GetJobInformation**, отображаемые в фильтре, показывают, что 2 из четырех прогнозов были успешными. Выберите в нижней левой квадранте имя **false (ложь** ), чтобы увидеть фразы продолжительностью под диаграммой. 

Используйте сочетание клавиш CTRL + E, чтобы перейти к представлению меток, чтобы увидеть точный текст пользователя utterance. 

Utterance `Is there a database position open in Los Colinas?` помечается как _жетжобинформатион_ , но текущая модель прогнозирует utterance как _апплифоржоб_. 

Существует почти три раза столько раз, сколько примеров для **апплифоржоб** , чем **жетжобинформатион**. Это неравномерность примера фразы продолжительностьюа в пользу **апплифоржоб** намерения, что приводит к неправильному прогнозу. 
...
## <a name="how-to-fix-the-app"></a>Как исправить это приложение
...
Исправление заключается в добавлении дополнительных фразы продолжительностью в **жетжобинформатион**. Не забывайте менять длину utterance, выбор слов и компоновку слов, сохраняя при этом цель поиска сведений о задании, а _не_ применение для этого задания.
...
1. Нажмите кнопку с многоточием (***...***) справа от имени пакета и выберите **Run (выполнить**). Дождитесь завершения пакетного теста. Обратите внимание, что кнопка **See results** (Просмотреть результаты) теперь зеленого цвета. Это означает, что весь пакет успешно выполнен.
...
Диаграмма открывается со всеми намерениями, которые были правильно спрогнозированы. Прокрутите вниз в правой части фильтра, чтобы найти прогнозы сущностей с ошибками. 

1. Выберите сущность **Job** в фильтре.

    ![Прогнозирование сущностей ошибок в фильтре](./media/luis-tutorial-batch-testing/hr-entities-filter-errors.png)
