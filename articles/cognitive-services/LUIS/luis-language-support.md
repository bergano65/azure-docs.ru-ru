...
|Нидерландский<br>`de-de`|1.0.1|Разделяет слова, разбивая их на пробелы.<br> Если пользователь вводит `Ik ga naar de kleuterschool` в качестве utterance, он остается единственным маркером. Таким образом `kleuterschool` , помечается как единая сущность. |
