...
|--|--|--|
|Немецкий<br>`de-de`|1.0.0|Разделяет слова, разделив их с помощью токена на основе машинного обучения, который пытается разбить составные слова на отдельные компоненты.<br>Если пользователь введет `Ich fahre einen krankenwagen` в качестве utterance, он будет включен `Ich fahre einen kranken wagen`. Разрешение пометки `kranken` и `wagen` независимо от того, как разные сущности.|
|Немецкий<br>`de-de`|1.0.2|Разделяет слова, разбивая их на пробелы.<br> Если пользователь вводит `Ich fahre einen krankenwagen` в качестве utterance, он остается единственным маркером. Таким `krankenwagen` образом, помечается как единая сущность. |
|Нидерландский<br>`de-de`|1.0.0|Разделяет слова, разделив их с помощью токена на основе машинного обучения, который пытается разбить составные слова на отдельные компоненты.<br>Если пользователь введет `Ik ga naar de kleuterschool` в качестве utterance, он будет включен `Ik ga naar de kleuter school`. Разрешение пометки `kleuter` и `school` независимо от того, как разные сущности.|
|Нидерландский<br>`de-de`|1.0.1|Разделяет слова, разбивая их на пробелы.<br> Если пользователь вводит `Ik ga naar de kleuterschool` в качестве utterance, он остается единственным маркером. Таким `kleuterschool` образом, помечается как единая сущность. |
