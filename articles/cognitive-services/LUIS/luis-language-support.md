...
|--|--|--|
|Немецкий<br>`de-de`|1.0.0|Разделяет слова, разделив их с помощью токена на основе машинного обучения, который пытается разбить составные слова на отдельные компоненты.<br>Если пользователь введет `Ich fahre einen krankenwagen` в качестве utterance, он будет `Ich fahre einen kranken wagen`включен. Разрешение пометки `kranken` и `wagen` независимо от того, как разные сущности.|
