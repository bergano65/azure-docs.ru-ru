...
Параметры нормализации utterance по умолчанию отключены. Эти параметры включают:
...
Нормализация **форм Word** игнорирует различия в словах, которые выходят за пределы корня. Например, слова `run` , `running` и `runs` изменяются в зависимости от глагола.
