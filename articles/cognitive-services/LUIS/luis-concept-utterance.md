---
title: Хороший пример высказываний - LUIS
...
Основной термин здесь, "компьютер", не отличается. Но пользователь может применять другие слова: настольный компьютер, ноутбук, рабочая станция или просто машина. LUIS может разумно вывести синонимы из контекста, но когда вы создаете высказывания для обучения, всегда лучше их изменять.

## <a name="example-utterances-in-each-intent"></a>Примеры высказываний по каждому намерению

Для каждого намерения следует предоставить не менее 15 примеров высказываний. Если по какому-то намерению не будет ни одного примера высказываний, обучить LUIS будет невозможно. Если у вас есть намерение с одним или очень немногими примерами высказываний, LUIS может не точно предсказать намерение. 
...
## <a name="utterance-normalization"></a>Нормализация высказывания

Нормализация высказывания – это процесс игнорирования последствий знаков препинания и диакритических эффектов во время обучения и прогнозирования.

## <a name="utterance-normalization-for-diacritics-and-punctuation"></a>Нормализация высказывания для диакритических и пунктуаций

Нормализация высказывания определяется при создании или импорте приложения, поскольку оно является параметром в файле приложения JSON. Настройки нормализации высказывания отключены по умолчанию. 

Диакритика – это знаки или знаки в тексте, такие как: 

```
İ ı Ş Ğ ş ğ ö ü
```

Если приложение включает сярализацию, баллы в **тестовом** стекле, пакетных тестах и запросах конечных точек изменятся для всех высказываний с помощью диакритики или знаков препинания.

Включите нормализацию высказываний для диакритики или пунктуации `settings` к файлу приложения LUIS JSON в параметре.
...
Нормализация **знаков препинания** означает, что до того, как ваши модели будут обучены и до того, как будут предсказаны запросы на конечные точки, пунктуация будет удалена из высказываний. 

Нормализация **диакритических** заменяет персонажей диакритикой в высказываниях с обычными персонажами. Например: `Je parle français` `Je parle francais`становится . 

Нормализация не означает, что вы не увидите пунктуации и диакритические знаки в вашем примере высказывания или прогноз ответы, просто, что они будут проигнорированы во время обучения и прогнозирования.


### <a name="punctuation-marks"></a>Знаки препинания

В LUIS пунктуация является отдельным маркером. Высказывание, содержащее период в конце, по сравнению с высказыванием, которое не содержит период в конце, состоит из двух отдельных высказываний и может получить два разных прогноза. 

Если пунктуация не нормализуется, LUIS не игнорирует знаки препинания по умолчанию, поскольку некоторые клиентские приложения могут присваивать значение этим знакам. Обязательно предоставляйте примеры высказываний со знаками препинания и без них, чтобы оба варианта возвращали приблизительно одинаковый результат оценки. 
...
Если пунктуация не имеет конкретного значения в клиентском приложении, рассмотрите [возможность игнорирования знаков препинания](#utterance-normalization) путем нормализации знаков препинания. 

### <a name="ignoring-words-and-punctuation"></a>Игнорирование слов и пунктуации

Если вы хотите игнорировать конкретные слова или пунктуацию [pattern](luis-concept-patterns.md#pattern-syntax) в шаблонах, используйте шаблон `[]`с _проигнорированным_ синтаксисом квадратных скобок. 
