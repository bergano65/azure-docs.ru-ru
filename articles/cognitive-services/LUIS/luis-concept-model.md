...
Сущность представляет собой единицу данных, которые необходимо извлечь из utterance. Сущность, занятая компьютером, — это сущность верхнего уровня, содержащая подсущности, которые также являются сущностями, предназначенными для компьютера.
...
* [сущности, изученные компьютером](reference-entity-machine-learned-entity.md)
    * вложенные сущности (также элементы, занятые компьютером)
        * [особенностей](#features)
            * [список фраз](luis-concept-feature.md)
            * [такие сущности](luis-concept-feature.md) , как [регулярные выражения](reference-entity-regular-expression.md), [списки](reference-entity-list.md)и предварительно [подготовленные объекты](luis-reference-prebuilt-entities.md) , которые не изменяются компьютером.
