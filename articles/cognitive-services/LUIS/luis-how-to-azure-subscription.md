...
ms.locfileid: "79219976"
---
# <a name="using-authoring-and-runtime-resource-keys"></a>Использование ключей ресурсов авторизации и времени выполнения
...
Когда вы будете готовы опубликовать свою конечную точку прогноза, создайте и назначьте ключи авторизации и прогнозирования времени выполнения, чтобы заменить функциональность стартового ключа. 

## <a name="create-resources-in-the-azure-portal"></a>Создание ресурсов на портале Azure

1. Используйте [эту ссылку,](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesLUISAllInOne) чтобы открыть портал Azure для создания ресурсов.
1. Выберите **оба** для создания автора и ключа сионный график прогнозирования. 
1. Введите информацию, необходимую для создания ресурса, затем выберите **Создать** для завершения процесса.

    ![Создание ресурса понимания языка](./media/luis-how-to-azure-subscription/create-resource-in-azure.png)

    |name|Назначение|
    |--|--|
    |Имя ресурса| Пользовательское имя, которое вы выбираете, используется как часть URL-адреса для запросов на авторство и прогнозирование конечных точек.|
    |имя подписки;| подписка, которая будет выставлен счет за ресурс.|
    |Группа ресурсов| Пользовательское имя группы ресурсов, которое вы выбираете или создаете. Группы ресурсов позволяют группировать ресурсы Azure для доступа и управления в одном регионе.|
    |Место разработки|Область, связанная с вашей моделью.|
    |Ценовая категория для разработки|Уровень ценообразования определяет максимальную транзакцию в секунду и месяц.|
    |Расположение среды выполнения|Область, связанная с моментом выполнения опубликованной точки прогнозирования.|
    |Ценовая категория среды выполнения|Уровень ценообразования определяет максимальную транзакцию в секунду и месяц.|

    После создания обоих ресурсов назначаем ресурсы на портале LUIS.
...
1. На странице **настроек пользователей** выберите **ресурс автором добавления,** а затем выберите существующий ресурс авторизации. Нажмите кнопку **Сохранить**. 
