...
|Key|Цель|Служба для переприятия`kind`|Служба для переприятия`type`|
|--|--|--|--|
...
|Ключ прогнозирования запроса| Запросы конечной точки прогнозирования запросов. Создайте ключ прогнозирования LUIS, прежде чем клиентское приложение запросит прогнозы за пределами 1 000 запросов, предоставленных начальным ресурсом. |`LUIS`|`Cognitive Services`|
...
|Ключ для начала работы|Бесплатная разработка (без управления доступом на основе ролей) через портал LUIS или API (включая пакеты SDK), бесплатные 1 000 запросов к конечной точке прогнозирования в месяц с помощью браузера, API или пакетов SDK.|-|Не является ресурсом Azure|

После завершения процесса создания ресурсов Azure [назначьте ключ](#assign-a-resource-to-an-app) приложению на портале Luis.
...
1. На странице **Параметры пользователя** выберите **Добавить ресурс для разработки** и выберите существующий ресурс разработки. Нажмите кнопку **Сохранить**.

## <a name="assign-a-resource-to-an-app"></a>Назначение ресурса для приложения

Вы можете назначить приложение для приложения с помощью следующей процедуры.
