...
**Вопрос. Почему не удалось объединить два последних примера высказываний в отдельный шаблон высказываний?** Шаблон не поддерживает синтаксис OR. Чтобы захватить версию `in` и `on`, каждая из них должна быть отдельным высказыванием шаблона.
