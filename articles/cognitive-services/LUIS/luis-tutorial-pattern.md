...
1. Импортируйте JSON-файл в новое приложение на [портале LUIS предварительной версии](https://preview.luis.ai). На странице **Мои приложения** выберите пункт **+ New app for conversation** (Создать приложение для беседы), а затем выберите **Import as JSON** (Импортировать как JSON). Выберите файл, скачанный на предыдущем шаге.
