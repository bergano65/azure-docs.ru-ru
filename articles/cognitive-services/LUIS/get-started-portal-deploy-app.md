...
`q=` в конце URL-адреса — это сокращение от **query** (запрос). Здесь речевой фрагмент пользователя добавляется в запрос GET. После `q=` введите речевой фрагмент пользователя, используемый в конце предыдущего краткого руководства:
