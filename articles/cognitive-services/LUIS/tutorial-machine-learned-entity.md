...
1. В списке примеров речевых фрагментов выберите указанный ниже фрагмент.
    ...
    Щелкните и перетащите курсор к тексту первого примера намерения. В появившемся меню введите имя сущности как `Order`. Теперь выберите из списка элемент `Order Create new entity`.
...
1. Задайте имя сущности `SizeListentity` и тип **List**, чтобы ее можно было легко распознать при сравнении со списком `SizePhraselist`, создание которого описано в предыдущем разделе.
...
* [Руководство по намерениям](./tutorial-intents-only.md)
...
> [Добавление готовой сущности keyPhrase](./luis-reference-prebuilt-keyphrase.md)