...
Чтобы определить сущность, вам нужно создать ее и добавить метку к тексту, который представляет эту сущность в примерах речевых фрагментов со всеми намерениями. Примеры с метками информируют LUIS о том, какая сущность используется и где ее можно найти в речевом фрагменте.
...
В этом примере вы создадите сущность, прошедшую машинное обучение, которая будет представлять собой заказ в приложении для заказа пиццы. Этот заказ должен содержать все элементы, необходимые для его выполнения. В начале работы сущность извлечет текст, связанный с заказом, его размер и количество.

Речевой фрагмент для `Please deliver one large cheese pizza to me` должен извлекать `one large cheese pizza` в качестве заказа, а затем — `1` и `large`.

Вы можете дополнительно разложить текст на части, создав подкомпоненты для начинки и коржей. После ознакомления с этим руководством вы сможете уверенно добавлять эти подкомпоненты к существующей сущности `Order`.
...
1. В поле **Choose an entity type** (Выберите тип сущности) выберите **Add Structure** (Добавить структуру), а затем **Далее**. Структура необходима для добавления подкомпонентов, таких как размер и количество.
...
1. Чтобы добавить **дескриптор**, выберите значение `+` в области **Descriptors** (Дескрипторы), а затем щелкните **Create new phrase list** (Создать новый список фраз).
...
1. После обучения добавьте новый пример речевого фрагмента к намерению, чтобы понять, насколько хорошо LUIS понимает прошедшую машинное обучение сущность.
    ...
    Здесь сущность самого верхнего уровня `Order` и подкомпонент `Size` обозначены пунктирными линиями.
...
## <a name="add-prebuilt-number-to-help-extract-data"></a>Добавление предварительно заданного числа для извлечения данных
...
Сущность `Order` должна включать подкомпонент `Quantity`, который будет определять количество элементов в заказе. Количество должно быть ограничено числом, чтобы извлеченные данные сразу были готовы к использованию в клиентском приложении и отсортированы по имени.
...
При использовании ограничения будет извлекаться лишь текст, соответствующий этому ограничению.

1. Щелкните **Сущности** и выберите сущность `Order`.
1. Выберите **+ Add Component** (+ Добавить компонент) и введите имя `Quantity`, а затем нажмите клавишу ВВОД, чтобы добавить новый подкомпонент в сущность `Order`.
...
Щелкните **Обучить**, чтобы обучить приложение новым речевым фрагментам. После обучения подкомпонент `Quantity` правильно прогнозируется в компоненте `Order`. Правильный прогноз обозначается сплошной линией.
