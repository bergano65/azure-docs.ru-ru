...
|Требования к упаковке|Подробная информация|
|--|--|
...
|Параметр запроса|введите|Назначение|
|--|--|--|
|`query`|строка|Фраза пользователя.|
|`verbose`|логическое значение|Логическое значение, указывающее, следует ли возвращать все метаданные для прогнозируемых моделей. Значение по умолчанию — false.|
|`log`|логическое значение|Записывает запросы в журнал, который затем можно использовать для [активного обучения](luis-how-to-review-endpoint-utterances.md). Значение по умолчанию — false.|
|`show-all-intents`|логическое значение|Логическое значение, указывающее, следует ли возвращать только цели или цель наибольшей оценки. Значение по умолчанию — false.|
...
|`verbose`|логическое значение|Возвращает все намерения и их оценки, если задано значение true. По умолчанию задано значение false, при котором возвращается только верхнее намерение.|
|`staging`|логическое значение|Возвращает запрос из результатов промежуточной среды, если задано значение true. |
|`log`|логическое значение|Записывает запросы в журнал, который затем можно использовать для [активного обучения](luis-how-to-review-endpoint-utterances.md). Значение по умолчанию — true.|
