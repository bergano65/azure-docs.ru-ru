...
## <a name="intent-prediction-errors"></a>Ошибок в прогнозировании намерений 

Пример utterance в объект intent возможно, ошибка намерений прогноза между целью utterance пример, в настоящее время находится в и целью прогноза, определяется во время обучения. 

Чтобы найти utterance ошибок в прогнозировании и исправлять их, используйте **фильтра** параметра **оценки** параметры неверно и Unclear в сочетании с **представление** параметр **Полное и подробное представление**. 

![Чтобы найти utterance ошибок в прогнозировании и исправлять их, используйте параметр фильтра.](./media/luis-how-to-add-intents/find-intent-prediction-errors.png)

Когда применяются фильтры и представления, и есть пример фразы с ошибками, пример utterance отобразятся фразы и о возникших проблемах.

![! [Когда применяются фильтры и представления, и есть пример фразы с ошибками, в примере utterance списке показаны фразы и о возникших проблемах.] (. / media/luis-how-to-add-intents/find-errors-in-utterances.png)](./media/luis-how-to-add-intents/find-errors-in-utterances.png#lightbox)

Каждая строка содержит оценку прогноза текущего обучения для utterance пример, ближайшего конкурент рейтингу, который разница в эти две оценки. 

### <a name="fixing-intents"></a>Исправление намерений

Чтобы узнать, как устранить ошибки намерений прогноза, используйте [сводную панель мониторинга](luis-how-to-use-dashboard.md). Сводная панель мониторинга предоставляет analysis для активной версии последнего обучения, а также самых востребованных функций для устранения вашей модели.  
...
См. в разделе [добавьте сущность utterance](luis-how-to-add-example-utterances.md) для получения дополнительных сведений.
...
Когда один или несколько пример фразы выбраны в списке, пометив флажки слева от utterance панели инструментов над списком utterance позволяет выполнять следующие действия:
