...
* В процессе **разработки**фразы продолжительностью создаются и сохраняются в намерениях. Для успешного LUIS приложения требуются фразы продолжительностью на целях. После публикации и получения приложением запросов на конечной точке строка запроса конечной точки, `log=false`, определяет, хранится ли конечная точка utterance. Если конечная точка хранится, она становится частью активного фразы продолжительностью обучения, найденного в разделе **Build** на портале, в разделе **Проверка конечной точки фразы продолжительностью** . 
