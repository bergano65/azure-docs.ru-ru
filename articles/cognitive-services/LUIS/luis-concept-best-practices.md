...
После [публикации](luis-how-to-publish-app.md)приложения используйте цикл разработки, чтобы добавить компоненты, опубликовать и проверить конечную точку. Не начинайте очередной цикл разработки с добавления новых примеров высказываний. Это не позволит LUIS обучить вашу модель с помощью высказываний настоящих пользователей. 
...
* Учебник. [Создание приложения LUIS для определения намерений пользователя](luis-quickstart-intents-only.md)
...
Лучше всего не использовать эти методы, пока ваше приложение не будет протестировано. Прежде чем добавлять [списки фраз](luis-concept-feature.md) и [шаблоны](luis-concept-patterns.md) , необходимо понять, как приложение ведет себя, так как эти функции имеют взвешенное значение более интенсивно, чем пример фразы продолжительностью и будут распознавать достоверность. 
...
* Учебник. [Учебник. Исправление неточных прогнозов с помощью проверки высказываний конечной точки](luis-tutorial-review-endpoint-utterances.md)
