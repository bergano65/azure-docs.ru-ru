...
Начните с определения полных концепций данных, которые необходимо извлечь в utterance. Это ваша сущность, занятая компьютером. Затем разбивает фразу на ее части. Это включает определение подсущностей и функций.

Например, если требуется извлечь адрес, можно вызвать наиболее частную сущность, предназначенную для машинного обучения `Address` . При создании адреса необходимо указать некоторые из его подсущностей, такие как адрес улицы, город, штат и почтовый индекс.
