...
|-------|------|-----------|-------------|
|Обязательно для заполнения| `Input` | String | Цель входного подключения. Значение по умолчанию — `/input`.    <br><br>Пример:<br>`--mount type=bind,src=c:\input,target=/input`|
|Обязательно для заполнения| `Output` | String | Цель выходного подключения. Значение по умолчанию — `/output`.  <br><br>Пример:<br>`--mount type=bind,src=c:\output,target=/output`|
