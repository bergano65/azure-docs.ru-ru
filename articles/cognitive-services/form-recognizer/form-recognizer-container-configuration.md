...
> [!IMPORTANT]
> , [`ApiKey`](#apikey-configuration-setting) [`Billing`](#billing-configuration-setting)и [`Eula`](#eula-setting) настройки используются вместе. Для всех трех параметров необходимо указать допустимые значения. В противном случае контейнер не запустится. Дополнительные сведения об использовании этих параметров конфигурации для создания экземпляра контейнера см. в разделе [Выставление счетов](form-recognizer-container-howto.md#billing).
...
Используйте подключения привязок для чтения данных из контейнера и записи в него. Можно указать входним крепление или выходное крепление, указав опцию `--mount` в [ `docker run` команде.](https://docs.docker.com/engine/reference/commandline/run/)
...
* **Персонаж продолжения линии:** Команды Докера в следующих\\разделах используют заднюю слэш () в качестве символа продолжения линии. Замените или удалите ее в соответствии с требованиями вашей операционной системы.
...
| **{COMPUTER_VISION_ENDPOINT_URI}** | Конечная точка выставления счетов. При использовании облачного ресурса Компьютерного зрения значение URI доступно на странице общих сведений об API Компьютерного зрения на портале Azure. Если вы используете контейнер *распознавания когнитивных услуг,* используйте URL-адрес биллинга, который `docker run` передается контейнеру в команде. |

Просмотреть [сбор необходимых параметров](form-recognizer-container-howto.md#gathering-required-parameters) для получения подробной информации о том, как получить эти значения.
