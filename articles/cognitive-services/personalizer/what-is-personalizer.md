...
ms.locfileid: "74707952"
---
# <a name="what-is-personalizer"></a>Что такое служба "Персонализатор"?

Персонализатор Azure — это облачная служба API, которая позволяет приложению выбрать максимально удобный режим работы для пользователей, изучая их коллективное поведение в реальном времени.
...
Ознакомьтесь с демонстрацией [принципа работы Персонализатора](https://personalizercontentdemo.azurewebsites.net/).
...
1. Используйте [API ранжирования](https://westus2.dev.cognitive.microsoft.com/docs/services/personalizer-api/operations/Rank) для вызова Персонализатора с информацией (_функциями_) о пользователях и содержимом (_действиях_). Перед использованием службы "Персонализатор" нет необходимости предоставлять очищенные, маркированные данные. Интерфейсы API можно вызывать напрямую или с помощью пакетов SDK, доступных для различных языков программирования.
1. Покажите пользователю в клиентском приложении действие, выбранное службой "Персонализатор".
1. Используйте [API вознаграждения](https://westus2.dev.cognitive.microsoft.com/docs/services/personalizer-api/operations/Reward), чтобы оставить отзыв в Персонализаторе с указанием того, выбрал ли пользователь действие, предложенное службой. Это _[оценка вознаграждения](concept-rewards.md)_ .
...
* *Rank*. Используйте Rank API, чтобы определить _действие_, которое следует отобразить в текущем _контексте_. Действия отправляются в виде массива объектов JSON с идентификатором и сведениями (_функции_) о каждой из них; контекст отправляется в виде другого объекта JSON. API возвращает параметр actionId, который приложение затем должно преобразовать для просмотра пользователем.
* *Награда*: После взаимодействия пользователя с приложением вам нужно определить качество работы персонализации, используя число от 0 до 1, и отправить его в виде [оценки вознаграждения](concept-rewards.md). 
