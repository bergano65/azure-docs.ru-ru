---
title: 'Функции: действие и контекст — Персонализация'
...
* _Пользовательское_ лицо, например `Sports_Shopper` . Это не должен быть идентификатор отдельного пользователя. 
...
Персонализатор поддерживает признаки строковых, числовых и логических типов. Вполне вероятно, что в основном приложение будет использовать строковые функции с некоторыми исключениями.
...
* **Строки**. для строковых типов каждое сочетание ключа и значения рассматривается как функция One-Hot (например, Жанр: "сЦиенцефиктион" и Жанр: "документальные" создаст две новых функции ввода для модели машинного обучения.
* **Числовые**. Если число является величиной, которая должна пропорционально повлиять на результат персонализации, следует использовать числовые значения. Это очень зависимый сценарий. В упрощенном примере, например при персонализации розничной торговли, Нумберофпетсовнед может быть функцией, которая является числовой, так как может потребоваться, чтобы люди с 2 или 3 pets могли повлиять на результат персонализации дважды или чаще так, как если бы у них было 1 Pet. Функции, основанные на числовых единицах, но если значение не линейное, например возраст, температура или высота пользователя, лучше кодируются как строки. Например, DayOfMonth будет строкой с символами "1", "2"... "31". Если имеется много категорий, можно улучшить качество функций с помощью диапазонов. Например, Age можно закодировать как «Age»: «0-5», «Age»: «6-10» и т. д.
...
> [!Note]
> В настоящее время настоятельно рекомендуется использовать имена для пространств имен компонентов, которые основаны на кодировке UTF-8, и начинаться с разных букв. Например,, `user` `state` и `device` начинаются с `u` , `s` и `d` . В настоящее время пространства имен с одинаковыми первыми символами могут привести к конфликтам в индексах, используемых для машинного обучения.
...
### <a name="restrictions-in-character-sets-for-namespaces"></a>Ограничения в наборах символов для пространств имен

Строка, используемая для именования пространства имен, должна соответствовать некоторым ограничениям: 
* Оно не может быть в Юникоде.
* Вы можете использовать некоторые печатные символы с кодами < 256 для имен пространств имен. 
* Нельзя использовать символы с кодами < 32 (не печатные), 32 (пробел), 58 (двоеточие), 124 (pipe) и 126 – 140.
* Оно не должно начинаться с символа подчеркивания "_", или функция будет пропущена.
...
* Используйте категории категорий и строковые типы для функций, которые не являются величиной. 
...
Сведения о расположении обычно имеют преимущества от создания более широких классификаций. Например, Latitude-Longitudeная координата, например Широта: 47,67402 ° N, Long: 122,12154 ° W является слишком точной, и заставляет модель изучать широту и долготу как отдельные измерения. При попытке персонализации на основе сведений о расположении она помогает сгруппировать сведения о расположении в больших секторах. Простой способ сделать это — выбрать соответствующую точность округления для Lat-Long чисел и объединить широту и долготу в "области", сделав их одной строкой. Например, хорошим способом представления 47,67402 ° N, Long: 122,12154 ° W в регионах приблизительно в нескольких километрах, будет "Location": "34.3, 12,1".
...
* [Связывание сущностей](../text-analytics/index.yml)
* [Анализ текста](../text-analytics/overview.md)
* [эмоция.](../face/overview.md)
* [Компьютерное зрение](../computer-vision/overview.md)
...
* Имеет идентификатор _события_ . Если у вас уже есть идентификатор события, следует отправить его. Если у вас нет идентификатора события, не отправляйте его, а Персонализация создает для вас и возвращает его в ответ на запрос ранжирования. Идентификатор связан с событием Rank, а не с пользователем. При создании идентификатора GUID лучше подходит. 
