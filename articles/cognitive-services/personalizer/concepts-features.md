...
> [!Note]
> В настоящее время настоятельно рекомендуется использовать имена для пространств имен компонентов, которые основаны на кодировке UTF-8, и начинаться с разных букв. `user` `device` `u` `s`Например,, и начинаются с, и `d` `state` В настоящее время пространства имен с одинаковыми первыми символами могут привести к конфликтам в индексах, используемых для машинного обучения.
