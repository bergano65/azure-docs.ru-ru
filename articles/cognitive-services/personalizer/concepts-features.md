...
В приведенном ниже коде JSON `user`, `state` и `device` — это пространства имен признаков. Примечание общедоступной предварительной версии. в настоящее время настоятельно рекомендуется использовать имена для пространств имен компонентов, которые основаны на кодировке UTF-8, и начинаться с разных букв. Например, `user`, `state`и `device` начинаются с `u`, `s`и `d`. В настоящее время пространства имен с одинаковыми первыми символами могут привести к конфликтам в индексах, используемых для машинного обучения.
