...
Personalizer не предписывает, ограничить или устранить какие возможности вы можете отправлять для действия и контекст:
...
* Персонализатор попытается выбрать действия, наиболее подходящие для всех пользователей в любое время.
...
### <a name="how-choice-of-feature-type-affects-machine-learning-in-personalizer"></a>Как машинное обучение в Personalizer влияет на выбор типа компонента

* **Строки**: Для строковых типов каждой комбинации ключа и значения создает новый веса в Personalizer модели машинного обучения. 
* **Числовые**: Числовые значения следует использовать, когда номер пропорционально влияют на результат персонализации. Это очень сценарий зависимых. В упрощенном примере например когда Персонализация Розничная возникают, NumberOfPetsOwned может быть компонентом, который является числовым, как вы можете влияют на результат персонализации дважды или thrice как наличие 1 pet людям с домашними животными 2 или 3. Функции, которые основаны на числовых единиц, но когда значение не линейной - например возраст, температуры или высота Person - лучше всего кодируются как строки и функция качества, обычно может быть повышена путем использования диапазонов. Например, возраст кодируются как «Возраст»: «0-5", «Age»:" 6-10" и т. д.
* **Логическое** значения, отправленные со значением «false» act, как, если они еще не были отправлены вообще.
...
Можно присваивать пространствам имен любые имена, которые являются допустимыми ключами JSON. Пространства имен используются для организации компонентов в различных наборов и для устранения неоднозначности функций с одинаковыми именами. Можно рассматривать пространства имен «префикс» добавляется к имена компонентов. Пространства имен не могут быть вложенными.


В приведенном ниже коде JSON `user`, `state` и `device` — это пространства имен признаков. Примечание общедоступной предварительной версии. В настоящее время мы настоятельно рекомендуем использовать имена для пространства имен, на основе UTF-8 и начинаться с разные буквы. Например `user`, `state`, и `device` начинаются с `u`, `s`, и `d`. Возникла пространств имен с тем же первых символов может привести к конфликту в индексах, используемым для машинного обучения.

Объекты JSON могут включать вложенные объекты JSON и простые свойства и значения. Массив может быть включен только в том случае, если элементы массива являются числами. 
