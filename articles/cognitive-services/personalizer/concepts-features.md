---
title: 'Особенности: Действие и контекст - Персонализатор'
...
* Пользователь _персоной,_ таких `Sports_Shopper`как . Это не должно быть индивидуальным идентификатором пользователя. 
...
Personalizer не предписывает, не ограничивает и не исправляет, какие функции можно отправлять для действий и контекста:
...
* Для категориальных объектов не нужно определять возможные значения, и вам не нужно заранее определять диапазоны для численных значений.
...
### <a name="how-choice-of-feature-type-affects-machine-learning-in-personalizer"></a>Как выбор типа функций влияет на машинное обучение в Personalizer

* **Строки**: Для типов строк каждая комбинация ключа и ценности создает новые веса в модели машинного обучения Personalizer. 
* **Число:** Вы должны использовать числовые значения, когда число должно пропорционально влиять на результат персонализации. Это очень зависит от сценария. В упрощенном примере, например, при персонализации розничного опыта, NumberOfPetsOwned может быть функция, которая является числовым, как вы можете люди с 2 или 3 домашних животных, чтобы повлиять на результат персонализации в два раза или три жды столько, сколько с 1 домашнее животное. Функции, основанные на числовых единицах, но где значение не является линейным - такие как возраст, температура или высота человека - лучше всего кодируются как строки, и качество функции, как правило, может быть улучшено с помощью диапазонов. Например, Возраст может быть закодирован как "Возраст":"0-5", "Возраст":"6-10" и т.д.
* **Boolean** значения послал со значением "ложных" действовать, как если бы они не были отправлены на всех.
...
Можно присваивать пространствам имен любые имена, которые являются допустимыми ключами JSON. Пространства имен используются для организации функций в отдельные наборы и для дисамбии с аналогичными именами. Пространства имен можно считать «префиксом», который добавляется к именам объектов. Пространства имен не могут быть вложены.


В приведенном ниже коде JSON `user`, `state` и `device` — это пространства имен признаков. 

> [!Note]
> В настоящее время мы настоятельно рекомендуем использовать имена для областей имен, которые utF-8 основаны и начать с разных букв. Например, `user` `state`, `device` , `u`и `s`начать `d`с , и . В настоящее время наличие пространства имен с теми же первыми символами может привести к столкновениям в индексах, используемых для машинного обучения.

Объекты JSON могут включать вложенные объекты JSON и простые свойства/ценности. Массив может быть включен только в том случае, если элементы массива являются числами. 
...
### <a name="restrictions-in-character-sets-for-namespaces"></a>Ограничения в наборах символов для пространства имен

Строка, используемая для именования пространства имен, должна следовать некоторым ограничениям: 
* Это не может быть универсальным кодом.
* Некоторые символы, на которых можно посмотреть печать, с кодами < 256 для имен пространства имен. 
* Нельзя использовать символы с кодами < 32 (не для печати), 32 (пространство), 58 (колон), 124 (труба) и 126-140.
* Она не должна начинаться с подчеркивателя «Я» или функция будет проигнорирована.
...
Информация о местоположении также обычно выигрывает от создания более широких классификаций. Например, координаты широты и долготы, такие как Лат: 47.67402 "N", Long: 122.12154 "W является слишком точным, и заставляет модель изучать широту и долготу в качестве различных размеров. Когда вы пытаетесь персонализировать на основе информации о местоположении, это помогает группировать информацию о местоположении в больших секторах. Простой способ сделать это заключается в том, чтобы выбрать соответствующую точность округления для лат-длинных чисел, и объединить широту и долготу в "области", сделав их в одну строку. Например, хороший способ представить 47.67402 "N, Длинные: 122.12154 W в регионах примерно в несколько километров в ширину будет " расположение":"34,3 , 12,1".
...
* [Связь с сущностью](../entitylinking/home.md)
* [Текстовая аналитика](../text-analytics/overview.md)
* [API распознавания эмоций](../emotion/home.md)
* [API Компьютерного зрения](../computer-vision/home.md)
...
* Имеет идентификатор _события._ Если у вас уже есть идентификатор события, вы должны представить его. Если у вас нет идентификатора события, не отправляйте его, Personalizer создает его для вас и возвращает его в ответ на запрос Rank. Идентификатор связан с событием ранга, а не с пользователем. При создании идентификатора графический интерфейс работает лучше всего. 
