...
[Исходный код примеров пакета SDK для API "Поиск видео Bing" для Java](https://github.com/Azure-Samples/cognitive-services-java-sdk-samples/tree/master/Search/BingVideoSearch) доступен на сайте GitHub.
...
Установите зависимости пакета SDK для API "Поиск видео Bing" с помощью Maven, Gradle или другой системы управления зависимостями. Для файла POM Maven требуется объявление:
```
  <dependencies>
    <dependency>
...
```
Реализуйте клиент **VideoSearchAPIImpl**. Для этого требуется экземпляр класса **ServiceClientCredentials**.
```
...
```
Выполните поиск видео по запросу "SwiftKey". Проверьте число результатов. Выведите идентификатор, название и URL-адрес для первого результата.
```
...
```
Выполните поиск видео по запросу "Bellevue Trailer". Отфильтруйте результаты поиска с помощью параметров *free*, *short* и *1080p resolution*. Проверьте число результатов. Выведите идентификатор, название и URL-адрес для первого результата.
```
...
```
Выполните поиск видео, набирающих популярность. Проверьте параметры **bannerTiles** и **categories**.
```
...
```
Выполните поиск видео по запросу "Bellevue Trailer", а затем с помощью поиска найдите сведения о первом результате.
```
