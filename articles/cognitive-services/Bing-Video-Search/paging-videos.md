...
Чтобы получить предполагаемое количество найденных результатов поиска, используйте поле [totalEstimatedMatches](https://docs.microsoft.com/rest/api/cognitiveservices/bing-video-api-v7-reference#videos-totalestimatedmatches) в ответе, представленном в формате JSON.   
...
Чтобы разбить на страницы список доступных видео, используйте в запросе параметры [count](https://docs.microsoft.com/rest/api/cognitiveservices/bing-video-api-v7-reference#count) и [offset](https://docs.microsoft.com/rest/api/cognitiveservices/bing-video-api-v7-reference#offset).  
...
Если вы используете значение по умолчанию для параметра [count](https://docs.microsoft.com/rest/api/cognitiveservices/bing-video-api-v7-reference#count), необходимо указать только параметр `offset`, как показано в следующем примере запроса.  
