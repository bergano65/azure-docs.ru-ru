...
| sessionID | Строковый тип | Глобальный уникальный идентификатор для этого сеанса, используемый для отладки. |
...
| Параметр | Тип | Описание |
| ------- | ---- | ----------- |
| code | Строковый тип | Один из набора кодов ошибок. См. статью о [кодах ошибок](#error-codes). |
| сообщение | Строковый тип | Понятное представление ошибки. |
...
| Имя | Тип | Описание |
| ---- | ---- |------------ |
| содержимое | Строковый тип | Строка, содержащая содержимое, отправленное в иммерсивное средство чтения. |
| lang | Строковый тип | Язык текста, значение находится в формате языкового тега IETF BCP 47, например en, ES-ES. Язык будет автоматически обнаружен, если он не указан. См. сведения о [поддерживаемых языках](#supported-languages). |
...
| Имя | Тип | Описание |
| ---- | ---- |------------ |
| уиланг | Строковый тип | Язык пользовательского интерфейса, значение находится в формате языкового тега IETF BCP 47, например en, ES-ES. По умолчанию используется язык браузера, если он не указан. |
...
| чувствительн | Строковый тип | Строка, возвращаемая из Онпреференцесчанжед, представляющая предпочтения пользователя в иммерсивное средство чтения. Дополнительные [сведения см.](#settings-parameters) в статьях параметры и [инструкции по хранению пользовательских настроек](./how-to-store-user-preferences.md) . |
...
| customDomain | Строковый тип | Зарезервировано для внутреннего использования. Личный домен, в котором размещается иммерсивное средство чтения webapp (по умолчанию — NULL). |
...
| Имя | Тип | Описание |
| ---- | ---- |------------ |
| voice | Строковый тип | Voice: "женщина" или "папа". Обратите внимание, что не все языки поддерживают оба пола. |
...
| Имя | Тип | Описание |
| ---- | ---- |------------ |
| Язык | Строковый тип | Задает язык перевода, значение находится в формате языкового тега IETF BCP 47, например fr-FR, es-MX, zh-Ханс-CN. Требуется для автоматического включения перевода слов или документов. |
...
| fontFamily | Строковый тип | Задает выбранный шрифт ("Calibri", "Комиксанс" или "Ситка"). |
...
| тема | Строковый тип | Задает выбранную тему (например, "светлая", "темная"...). |
...
| наунхигхлигхтингколор | Строковый тип | Задает выбранный цвет для выделения существительных. |
| вербхигхлигхтинженаблед | Логическое | Задает включение или выключение выделения глагола. |
| вербхигхлигхтингколор | Строковый тип | Задает цвет выделения для выбранного глагола. |
| аджективехигхлигхтинженаблед | Логическое | Задает включение или выключение выделения прилагательного. |
| аджективехигхлигхтингколор | Строковый тип | Задает выбранный цвет выделения прилагательных. |
| адвербхигхлигхтинженаблед | Логическое | Задает включение или выключение выделения модификаторов. |
| адвербхигхлигхтингколор | Строковый тип | Задает выбранный цвет выделения модификаторов. |
