...
| timeout | Номер | Длительность (в миллисекундах) до сбоя [лаунчасинк](#launchasync) с ошибкой времени ожидания (значение по умолчанию — 15000 МС). Это время ожидания применяется только к начальному запуску страницы читатель, где успешно отображается при открытии страницы читатель и запуске счетчика. Коррекция времени ожидания не требуется. |
| уизиндекс | Номер | Z-индекс создаваемого IFRAME (по умолчанию — 1000). |
...
| Скорость | Номер | Скорость воспроизведения должна составлять от 0,5 до 2,5 включительно. |
...
| Имя | Тип | Описание |
| ---- | ---- |------------ |
| textSize | Номер | Задает размер выбранного текста. |
...
| textSize | Номер | Задает размер выбранного текста. |
| fontFamily | Строка | Задает выбранный шрифт ("Calibri", "Комиксанс" или "Ситка"). |
| текстспаЦинг | Номер | Задает, включается ли или выключается текстовый зазор. |
