...
Брандмауэр веб-приложения Azure (WAF) со службой "Передняя дверца" позволяет управлять доступом к веб-приложениям на основе определенных условий. Пользовательское правило WAF состоит из номера приоритета, типа правила, условий соответствия и действия. Существует два типа настраиваемых правил: правила соответствия и правила ограничения скорости. Правило соответствия управляет доступом на основе условий сопоставления, в то время как правило ограничения скорости управляет доступом на основе условий сопоставления и скорости входящих запросов. Вы можете отключить пользовательское правило, чтобы предотвратить его оценку, но сохранить конфигурацию. В этой статье рассматриваются правила соответствия, основанные на параметрах HTTP.

## <a name="priority-match-conditions-and-action-types"></a>Приоритет, условия соответствия и типы действий
Вы можете управлять доступом с помощью настраиваемого правила WAf, определяющего номер приоритета, тип правила, условия соответствия и действие. 

- **Priority** — уникальное целое число, описывающее порядок вычисления правил WAF. Правила с более низкими значениями оцениваются перед правилами с более высокими значениями
...
- **Действие:** определяет способ маршрутизации запроса при сопоставлении правила WAF. Можно выбрать одно из следующих действий, которое будет применено, если запрос соответствует настраиваемому правилу.
    ...
    - *Log* -WAF регистрирует запись в журналах WAF и продолжит оценивать следующее правило.
    - *Redirect* -WAF перенаправляет запрос на указанный URI, записывает запись в журналы WAF и завершает работу.

- **Условие соответствия:** определяет переменную соответствия, оператор и значение match. Каждое правило может содержать несколько условий соответствия. Условие соответствия может основываться на приведенных ниже *переменных соответствия*.
    - Ремотеаддр (IP-адрес клиента)
    - рекуестмесод
    - QueryString
    - Аргументы
    - RequestUri
...
- **Оператор:** List включает следующее:
    - Any: часто используется для определения действия по умолчанию, если правила не совпадают. Any является оператором Match ALL.
    - Ипматч: определение ограничения IP-адресов для переменной Ремотеаддр
    - Геосоответствие: определение географической фильтрации для переменной Ремотеаддр
    - Равно
    - Содержит
...
Можно задать условие *отрицания* , равное true, если результат условия должен быть инвертирован.

*Значение match* определяет список возможных значений соответствия.
