---
title: Передняя дверь Azure - Балансировка нагрузки с набором доставки приложений Azure Документы Майкрософт
description: Эта статья поможет вам узнать о том, как Azure рекомендует балансировать с пакетом загрузки с набором доставки приложений
services: frontdoor
documentationcenter: ''
author: sharad4u
...
Microsoft Azure предоставляет несколько глобальных и региональных служб для управления распределением сетевого трафика и сбалансированной загрузкой: «Менеджер трафика», «Передняя дверь», «Шлюз приложений» и «Балансер нагрузки».  Совместное использование этих служб в множестве регионов Azure и разных зональных архитектурах позволяет создавать надежные, масштабируемые, высокопроизводительные приложения.

![Набор служб для предоставления приложений ][1]
 
Эти службы разбиваются на две категории.
1. **Глобальные службы балансировки нагрузки,** такие как Traffic Manager и Front Door, распределяют трафик от конечных пользователей через региональные бэкэнды, через облака или даже гибридные предприимчивые службы. Глобальная балансировка нагрузки направляет трафик на ближайшую серверную часть службы и реагирует на изменения в надежности или производительности службы, чтобы поддерживать постоянную работу и максимальную производительность для ваших пользователей. 
2. **Региональные службы балансировки нагрузки,** такие как Standard Load Balancer или Application Gateway, предоставляют возможность распределения трафика в виртуальных сетях (VNET) между вашими виртуальными машинами (VMs) или зональными конечными точками обслуживания в регионе.

Объединение глобальных и региональных служб в приложении обеспечивает сквозной надежный, эффективный и безопасный способ маршрутирования трафика к пользователям и от ваших пользователей к вашим службам IaaS, PaaS или в предприимчивых услугах. В следующем разделе описывается каждая из этих служб.
...
- Приоритетная рассылание, чтобы направить весь трафик на бэкэнд, с другими бэкэндами в качестве резервного копирования.
...
- географическая маршрутизация, которая обеспечивает направление запрашиваемых сторон, расположенных в определенных географических регионах, в серверную часть, сопоставленную с этими регионами (например, все запросы от Испании должны быть направлены в регион Azure "Центральная Франция");
...
Клиент подключается напрямую к указанной серверной части. Если серверная часть неработоспособна, диспетчер трафика Azure обнаружит это и перенаправит клиентов на другой работающий экземпляр. Для получения дополнительной информации об услуге обратитесь к документации [Azure Traffic Manager Manager.](../traffic-manager/traffic-manager-overview.md)

**Azure Front Door** обеспечивает динамическое ускорение веб-сайта (DSA), включая глобальное балансирование нагрузки HTTP.  Она просматривает входящие HTTP-запросы, направленные в ближайшую серверную часть или регион для указанного имени узла, URL-адреса и настроенных правил.  
...
Шлюз приложений предоставляет контроллер доставки приложений (ADC) как услугу, предлагая для приложения разные функции балансировки нагрузки уровня 7. Это позволяет клиентам оптимизировать производительность веб-фермы, разгрузив процессор-интенсивный TLS прекращения в приложение шлюз. Кроме того, пользователи получают другие возможности маршрутизации уровня 7, включая циклическое распределение входящего трафика, соответствие сеансу на основе файлов cookie, маршрутизацию на основе URL-путей и возможность размещения нескольких веб-сайтов за одним шлюзом приложений. Шлюз приложений можно настроить как шлюз с доступом в Интернет, внутренний шлюз или же как сочетание этих вариантов. Шлюз приложений полностью управляется Azure. Он является масштабируемым и обладает высоким уровнем доступности. Для более эффективного управления предоставляется широкий набор возможностей ведения журнала и диагностики.
...
</br>В следующей таблице описаны различия между диспетчером трафика и передней дверью Azure:</br>

| Диспетчер трафика | Azure Front Door |
| --------------- | ------------------------ |
|**Любой протокол.** Так как диспетчер трафика работает на уровне DNS, вы можете передавать любой тип трафика: HTTP, TCP, UDP и т. д. | **Ускорение HTTP:** С Front Door трафик proxied на краю сети Microsoft.  Из-за этого в запросах HTTP(S) увидены улучшения задержки и пропускной связи, снижающие задержку для переговоров TLS и использующие горячие соединения от AFD к приложению.|
|**Локтая повыка:** При размытии на уровне DNS трафик всегда идет от точки к точке.  Маршрутизирование из вашего филиала в ваш центр обработки данных может идти прямым путем; даже на вашей собственной сети с помощью менеджера трафика. | **Независимое масштабирование.** Так как Front Door работает с HTTP-запросами, запросы к разным URL-адресам могут быть перенаправлены в другую серверную часть или пул региональных служб (микрослужб) на основе правил и работоспособности каждой микрослужбы приложения.|
...
Рекомендуется, чтобы все веб-сайты, API и службы были геоизбыточными и по мере возможности доставляли трафик пользователям из ближайшего (с минимальной задержкой) расположения.  Объединение услуг от Traffic Manager, Front Door, Application Gateway и Load Balancer позволяет создавать географически и зонально избыточные, чтобы максимизировать надежность, масштаб и производительность.
