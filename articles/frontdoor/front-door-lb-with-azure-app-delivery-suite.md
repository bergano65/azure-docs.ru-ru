...
1. **Глобальные службы балансировки нагрузки** , такие как диспетчер трафика и передняя дверца, распределяют трафик от конечных пользователей по региональным серверам, по облакам или даже к гибридным локальным службам. Глобальная балансировка нагрузки направляет трафик на ближайшую серверную часть службы и реагирует на изменения в надежности или производительности службы, чтобы поддерживать постоянную работу и максимальную производительность для ваших пользователей. 
2. **Региональные службы балансировки нагрузки** , такие как Load Balancer (цен. Категория "Стандартный") или шлюз приложений, обеспечивают возможность распределять трафик между виртуальными сетями (виртуальных сетей) между виртуальными машинами или конечными точками службы зональные в пределах региона.

Объединение глобальных и региональных служб в приложении обеспечивает сквозной надежный, производительный и безопасный способ маршрутизации входящего и исходящего трафика пользователей в IaaS, PaaS или локальные службы. В следующем разделе описывается каждая из этих служб.
...
- Приоритетная маршрутизация для направления всего трафика на серверную часть с другими незавершенными резервными копиями.
...
Рекомендуется, чтобы все веб-сайты, API и службы были геоизбыточными и по мере возможности доставляли трафик пользователям из ближайшего (с минимальной задержкой) расположения.  Объединение служб из диспетчера трафика, передней дверцы, шлюза приложений и Load Balancer позволяет создавать географические и зоналли избыточные, чтобы обеспечить максимальную надежность, масштабируемость и производительность.
