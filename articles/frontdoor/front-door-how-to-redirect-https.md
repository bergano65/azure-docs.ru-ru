...
Добавьте запись CNAME для сопоставления поддомена с интерфейсным узлом по умолчанию на передней дверце`<name>.azurefd.net`( `<name>` , где — имя профиля передней дверцы).

В качестве `www.contoso.com` примера для домена добавьте запись CNAME, которая сопоставляет имя `www` с. `<name>.azurefd.net`
...
> [!WARNING]
> Включение HTTPS для личного домена может занять несколько минут, а также зависит от проверки владельца домена, если запись CNAME не сопоставлена напрямую с узлом `<name>.azurefd.net`передней дверцы. Узнайте больше о [том, как включить протокол HTTPS для личного домена](./front-door-custom-domain-https.md).
