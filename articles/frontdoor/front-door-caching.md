...
- **Пропускать строки запроса**. в этом режиме Передняя дверца передает строки запроса от запрашивающей стороны в серверный первый запрос и кэширует ресурс. Все последующие запросы к этому ресурсу, которые обслуживаются в среде Front Door, будут пропускать строки запроса до истечения срока действия кэшированного ресурса.
...
Заголовки ответов Cache-Control, указывающие, что ответ не будет кэшироваться, например Cache-Control: закрытый, Cache-Control: un-Cache и Cache-Control: No-Store, не учитывается.  Если нет управления кэшем, поведение по умолчанию заключается в том, что АФД кэширует ресурс в течение X-интервала времени, в течение которого X выбирается случайным образом в диапазоне от 1 до 3 дней.
