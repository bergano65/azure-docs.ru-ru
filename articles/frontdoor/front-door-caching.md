...
1. **Очистка одного пути**. Очистите отдельные активы, указав полный путь к ресурсу (без протокола и домена) с расширением файла, например/пиктурес/страсбаург.ПНГ;
2. **Очистка по подстановочным знакам.** В качестве подстановочного знака можно использовать звездочку (\*). Очистите все папки, вложенные папки и файлы в конечной точке с помощью\* пути или удалите все вложенные папки и файлы в указанной папке, указав папку, а затем/\*, например,/Pictures/.\*
...
1. Cache-Control: s-maxage =\<секунды>
2. Cache-Control: max-age =\<seconds>
3. Срок действия истекает: \<HTTP-Дата>
