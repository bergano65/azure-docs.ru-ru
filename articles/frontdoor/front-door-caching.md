---
title: Лазурная передняя дверь - кэширование Документы Майкрософт
...
ms.locfileid: "79471750"
---
# <a name="caching-with-azure-front-door"></a>Кэшинг с лазурной передней дверью
В данном документе описывается поведение Front Door с правилами маршрутизации, в которых включено кэширование. Front Door является современной сетью доставки контента (CDN) и таким образом наряду с динамическим ускорением сайта и балансировкой нагрузки, она также поддерживает поведение кэширования, как и любой другой CDN.

## <a name="delivery-of-large-files"></a>Доставка больших файлов
Передняя дверь Azure обеспечивает большие файлы без крышки на размер файла. Front Door использует метод фрагментирования объекта. При запросе большого файла Front Door получает небольшие фрагменты этого файла из серверной части. После получения запроса полного файла или диапазона байтов среда Front Door запрашивает файл из серверной части в виде фрагментов по 8 МБ каждый.
...
- [Бротли](https://en.wikipedia.org/wiki/Brotli)
...
1. **Очистка одного пути**: Очистка индивидуального актива (ы), указав полный путь актива (без протокола и домена), с расширением файла, например, /pictures/strasbourg.png;
2. **Очистка по подстановочным знакам.** В качестве подстановочного знака можно использовать звездочку (\*). Очистите все папки, субфалеры и файлы\* под конечную точку с / в пути или очистить все subfolders\*и файлы под\*определенной папкой, указав папку следуют / , например, / фотографии / .
...
1. Кэш-контроль: s-maxage'\<секунд>
2. Кэш-контроль: максимальный\<возраст секунд>
3. Срок действия: \<http-date>

Заголовки ответов Cache-Control, указывающие на то, что ответ не будет кэшироваться, например, Cache-Control: приватный, кэш-контроль: без кэша и кэш-контроль: не хранится. Тем не менее при отправке нескольких запросов на лету к серверу POP для одного и того же URL-адреса они могут вернуть одинаковый ответ. Если нет кэш-контроля присутствует поведение по умолчанию является то, что AFD будет кэшировать ресурс для X количество времени, где X случайным образом выбрали между 1 до 3 дней.
