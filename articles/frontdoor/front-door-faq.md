...
### <a name="can-i-configure-azure-cdn-behind-my-front-door-profile-or-vice-versa"></a>Можно ли настроить Azure CDN за профилем передней дверцы или наоборот?

Передняя дверца Azure и Azure CDN не могут быть настроены вместе, так как обе службы используют одни и те же веб-сайты Azure при ответе на запросы. 
...
Да, передняя дверца Azure поддерживает разгрузку TLS/SSL и сквозной протокол TLS, который повторно шифрует трафик на серверную часть. На самом деле, поскольку подключения к серверной части происходят через общедоступный IP-адрес, рекомендуется настроить переднюю дверцу для использования протокола HTTPS в качестве протоколов пересылки.
