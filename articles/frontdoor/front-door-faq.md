...
Служба Azure двери — глобально распределенная служба несколькими клиентами. Таким образом инфраструктура двери является общим для всех клиентов. Тем не менее, путем создания входной двери вы определяете конкретного конфигурацию, необходимую для приложения, и 

### <a name="is-http-https-redirection-supported"></a>Поддерживается ли перенаправление протоколов HTTP -> HTTPS?

Входной двери в настоящее время не поддерживает перенаправление URL-адреса.
