...
Да. На самом деле служба двери Azure поддерживает узла, путь и перенаправление строки запроса, а также частью URL-адрес перенаправления. Дополнительные сведения о [URL-адрес перенаправления](front-door-url-redirect.md). 
...
### <a name="how-do-i-lock-down-the-access-to-my-backend-to-only-azure-front-door"></a>Как заблокировать доступ к моей для только Azure двери серверной части?

Чтобы заблокировать приложение для приема трафика только из вашей конкретных двери, необходимо будет настроить списки управления доступом IP-адрес для серверной части, и затем ограничить набор допустимых значений для заголовка «X-Forwarded-узел» отправлено передовой линии с Azure. Эти действия описаны ниже:

- Настройка IP-адрес управления доступом по адресам для вашей серверных систем для приема трафика из пространства IP-адресов Azure передней дверцы серверной части и только служб инфраструктуры Azure. Сейчас мы работаем над Интеграция [диапазоны IP-адресов Azure и тегами служб](https://www.microsoft.com/download/details.aspx?id=56519) , но теперь вы можете см. в статье диапазоны IP-адресов, как показано ниже:
 
    - Передней дверцы **IPv4** пространства IP-адрес серверной части: `147.243.0.0/16`
    - Передней дверцы **IPv6** пространства IP-адрес серверной части: `2a01:111:2050::/44`
    - Azure [базовые услуги инфраструктуры](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) через виртуальные IP-адреса: `168.63.129.16` и `169.254.169.254`

    > [!WARNING]
    > Пространство IP-адрес серверной части передней дверцы может измениться позже, однако мы следит за тем, пока это не произошло, что мы бы были интегрированы с [диапазоны IP-адресов Azure и тегами служб](https://www.microsoft.com/download/details.aspx?id=56519). Мы рекомендуем подписаться на [диапазоны IP-адресов Azure и тегами служб](https://www.microsoft.com/download/details.aspx?id=56519) любого изменения или обновления.

-   Фильтр по значениям для входящий заголовок "**X-Forwarded-Host**" Отправленные двери. Только допустимые значения для заголовка должен быть всех узлов внешнего интерфейса, как определено в файле config двери. На самом деле даже более конкретно, только имена узлов для которых требуется принимать трафик, в этой конкретной серверной ваше.
    - Пример – давайте предположим, что файл config двери имеет следующими узлами frontend _`contoso.azurefd.net`_ (A), _`www.contoso.com`_ (B), _ (C), и _`notifications.contoso.com`_ (D). Предположим, что у вас есть две серверные системы X и Y. 
    - X серверной части должен занять только трафик из имен узлов A и B. Y серверной части могут получить трафик A, C и D.
    - Таким образом, в серверной части X соглашаться следует, только трафик, который имеет заголовок "**X-Forwarded-Host**" задать либо _`contoso.azurefd.net`_ или _`www.contoso.com`_ . Для всего остального серверной части X следует отклонять трафик.
    - Аналогичным образом в серверной части Y соглашаться следует, только трафик, который имеет заголовок "**X-Forwarded-Host**" задать либо _`contoso.azurefd.net`_ , _`api.contoso.com`_ или  _`notifications.contoso.com`_ . Для всего остального серверной части Y следует отклонять трафик.
...
### <a name="does-front-door-support-autorotation-of-certificates"></a>Поддерживает ли двери autorotation сертификатов?

Для параметра сертификата двери управляемые сертификаты, autorotated по двери. Если вы используете управляемый сертификат двери и увидеть, что дата окончания срока действия сертификата является менее чем через 60 дней, файл запрос в службу поддержки.
</br>Autorotation не поддерживается для собственных пользовательского SSL-сертификата. Как и в том, как он был настроен в первый раз для данного пользовательского домена, может потребоваться для входной двери точки до версии правильного сертификата в хранилище ключей и убедитесь, что субъект-службу для входной двери по-прежнему имеет доступ к хранилищу ключей. Эта операция развертывания обновленного сертификата с дверью atomic и не вызывает какого-либо влияния рабочей среде, указано имя субъекта или по сети SAN для сертификата не изменяется.
...
Нет, сейчас не поддерживает двери, чтобы запретить конкретные версии TLS, нельзя также задать минимальную версию TLS. 
