...
Azure Front Door поддерживает динамическое ускорение сайта (DSA), разгрузку SSL и отогивание до конца SSL, веб-приложение Firewall, сродство сеансов на основе файлов cookie, маршрутизирование на основе URL-адреса, бесплатные сертификаты и управление несколькими доменами и другие. Полный список поддерживаемых функций можно ознакомьтесь с [обзором передней двери Azure.](front-door-overview.md)
...
- С помощью шлюза приложений, стоящего за AFD, можно достичь 100% Разгрузки SSL и маршрутировать только запросы HTTP в пределах своей виртуальной сети (VNET).
...
Примечание - Пользовательские обновления сертификатов SSL занимают около 30 минут, чтобы быть развернуты во всем мире.
...
Front Door управляемый вариант положения стандартный сертификат SSL через Digicert и хранится в ключе Убежище переднего двери. Если вы решите использовать свой собственный сертификат, то вы можете на борту сертификата из поддерживаемого CA и может быть стандартным SSL, расширенным сертификатом проверки или даже сертификатом подстановочного знака. Самоподписанные сертификаты не поддерживаются. [Узнайте, как включить HTTPS для пользовательского домена.](https://aka.ms/FrontDoorCustomDomainHTTPS)
...
Нет, настройка Front Door для конкретных наборов шифров не поддерживается. Тем не менее, вы можете получить свой собственный пользовательский сертификат SSL от вашего сертификата органа (скажем, Verisign, поручение, или Digicert) и иметь конкретные шифр люксы отмечены на сертификат, когда у вас есть он генерируется. 
...
Да, Azure Front Door поддерживает Разгрузку SSL и заканчивается SSL, который повторно шифрует трафик в бэкэнд. В самом деле, так как соединения с бэкэндом происходят над его общедоступной IP, рекомендуется настроить front Door для использования HTTPS в качестве протокола пересылки.
...
2. **Frontend**: Вы не можете использовать самоподписанные сертификаты при использовании собственного пользовательского SSL-сертификата для включения HTTPS в пользовательский домен.
...
1. **Несоответствие имени объекта сертификата**: Для соединений HTTPS Front Door ожидает, что ваш бэкэнд представляет сертификат от действительного CA с именем предмета (ы), соответствующим именем бэкэнда. Например, если ваше бэкэнд-хост-имя `myapp-centralus.contosonews.net` установлено и сертификат, который ваш бэкэнд представляет во время рукопожатия SSL, ни имеет, `myapp-centralus.contosonews.net` ни `*myapp-centralus*.contosonews.net` в названии объекта, передняя дверь откажется от соединения и приведет к ошибке. 
