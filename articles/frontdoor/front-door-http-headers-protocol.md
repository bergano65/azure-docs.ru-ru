...
| ------------- | ------------- |
| Через |  С помощью: 1.1 azure </br> Версия HTTP клиента а затем добавляет входной двери *Azure* как значение заголовка Via. Это указывает на версию клиента HTTP, а этот входной двери был получателю промежуточных для запроса между клиентом и серверной части.  |
