...
В базе данных Azure для MyS'L существует два варианта доступа к медленным журналам запросов: локальное хранилище серверов или диагностические журналы Azure Monitor. Это устанавливается `log_output` с использованием параметра.

Для локального хранилища серверов можно перечислить и загрузить журналы медленных запросов с помощью портала Azure или Azure CLI. На портале Azure перейдите на сервер на портале Azure. В разделе **Мониторинг** найдите страницу **Журналы сервера**. Для получения дополнительной информации о Azure CLI [см. Найдите и получите доступ к медленным журналам запросов с помощью Azure CLI.](howto-configure-server-logs-in-cli.md) 
