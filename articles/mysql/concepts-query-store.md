...
| **Параметр** | **Описание** | **По умолчанию** | **Разнообраз** |
|---|---|---|---|
...
| query_store_capture_utility_queries | Включение или отключение для сбора всех запросов служебной программы, которые выполняются в системе. | NO | ДА, НЕТ |
...
| **Параметр** | **Описание** | **По умолчанию** | **Разнообраз** |
|---|---|---|---|
| query_store_wait_sampling_capture_mode | Позволяет включать и выключать статистику ожидания. | None | НЕТ, ВСЕ |
...
| `query_id`| bigint (20) | NO| Уникальный идентификатор, сформированный для конкретного запроса. Если тот же запрос выполняется в другой схеме, будет создан новый идентификатор |
| `timestamp_id` | TIMESTAMP| NO| Отметка времени выполнения запроса. Это основано на конфигурации query_store_interval|
| `query_digest_text`| longtext| NO| Нормализованный текст запроса после удаления всех литералов|
| `query_sample_text` | longtext| NO| Первый внешний вид фактического запроса с литералами|
| `query_digest_truncated` | bit| YES| Был ли текст запроса усечен. Если длина запроса превышает 1 КБ, будет указано значение Да.|
| `execution_count` | bigint (20)| NO| Количество выполнений запроса для этого идентификатора метки времени/в течение заданного периода времени|
| `warning_count` | bigint (20)| NO| Число предупреждений, созданных во время внутреннего запроса|
| `error_count` | bigint (20)| NO| Количество ошибок, созданных этим запросом в течение интервала|
| `sum_timer_wait` | double| YES| Общее время выполнения этого запроса в течение интервала|
| `avg_timer_wait` | double| YES| Среднее время выполнения для этого запроса в течение интервала|
...
| `sum_lock_time` | bigint (20)| NO| Общий объем времени, затраченного на все блокировки для выполнения этого запроса в течение этого периода времени|
| `sum_rows_affected` | bigint (20)| NO| количество затронутых строк.|
| `sum_rows_sent` | bigint (20)| NO| Число строк, отправленных клиенту|
| `sum_rows_examined` | bigint (20)| NO| Число проверенных строк.|
| `sum_select_full_join` | bigint (20)| NO| Число полных соединений|
| `sum_select_scan` | bigint (20)| NO| Число просмотров SELECT |
| `sum_sort_rows` | bigint (20)| NO| Число отсортированных строк|
...
| `sum_created_tmp_disk_tables` | bigint (20)| NO| Общее число временных таблиц, созданных на диске (создает операции ввода-вывода)|
| `first_seen` | TIMESTAMP| NO| Первое вхождение (UTC) запроса во время выполнения статистического окна|
| `last_seen` | TIMESTAMP| NO| Последнее вхождение (UTC) запроса во время этого окна агрегирования|
...
| **Имя**| **Тип данных** | **IS_NULLABLE** | **Описание** |
|---|---|---|---|
| `interval_start` | TIMESTAMP | NO| Начало интервала (15-минутное приращение)|
| `interval_end` | TIMESTAMP | NO| Окончание интервала (15-минутное приращение)|
| `query_id` | bigint (20) | NO| Созданный уникальный идентификатор для нормализованного запроса (из хранилища запросов)|
| `query_digest_id` |  varchar(32) | NO| Нормализованный текст запроса после удаления всех литералов (из хранилища запросов) |
| `query_digest_text` | longtext | NO| Первый внешний вид фактического запроса с литералами (из хранилища запросов) |
