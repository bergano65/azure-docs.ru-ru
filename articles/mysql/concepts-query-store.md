...
Используйте [портала Azure](howto-server-parameters.md) или [Azure CLI](howto-configure-server-parameters-using-cli.md) требуется получить или задать другое значение для параметра.

## <a name="views-and-functions"></a>Представления и функции

Просмотр и управление хранилищем запросов осуществляеются с помощью следующих представлений и функций. Любой пользователь в [выберите привилегии роли public](howto-create-users.md#how-to-create-additional-admin-users-in-azure-database-for-mysql) эти представления можно использовать для просмотра данных в Store запроса. Эти представления доступны только в **mysql** базы данных.
