...
Область **применения:**  Azure база данных для MySQL 5,7
...
1. Выберите параметры **сервера** in **Параметры** section меню.
...
|Ожидания с большим числом операций ввода-вывода буфера | Найдите в хранилище запросов запросы с большим числом физических операций чтения. Если они соответствуют запросам с высокими ожиданиями ввода-вывода, рассмотрите возможность введения индекса в базовую сущность для поиска вместо проверок. Это позволит свести к минимуму затраты на операции ввода-вывода запросов. Проверьте **рекомендации по повышению производительности** for на сервере на портале, чтобы узнать, есть ли рекомендации по индексам для этого сервера, которые оптимизируют запросы. |
|Ожидания с высокой загрузкой памяти | Найдите в хранилище запросов запросы, использующие больше всего памяти. Эти запросы, скорее всего, дополнительно задерживают выполнение соответствующих запросов. Проверьте **рекомендации по повышению производительности** for на сервере на портале, чтобы узнать, есть ли рекомендации по индексам, которые оптимизируют эти запросы.|
...
Используйте [портал Azure](howto-server-parameters.md) or [Azure CLI](howto-configure-server-parameters-using-cli.md) to Get или задайте другое значение для параметра.

## <a name="views-and-functions"></a>Представления и функции

Просмотр и управление хранилищем запросов осуществляеются с помощью следующих представлений и функций. Все пользователи в [открытой роли выбор привилегий](howto-create-users.md#how-to-create-additional-admin-users-in-azure-database-for-mysql) могут использовать эти представления для просмотра данных в хранилище запросов. Эти представления доступны только в **mysql** database.
...
- Если у сервера MySQL имеется параметр `default_transaction_read_only` в, хранилище запросов не может записывать данные.
