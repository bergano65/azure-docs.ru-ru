---
title: Частная ссылка для базы данных Azure для MySQL (Предварительная версия)
...
ms.locfileid: "75897511"
---
# <a name="private-link-for-azure-database-for-mysql-preview"></a>Частная ссылка для базы данных Azure для MySQL (Предварительная версия)
...
После того как администратор сети создаст закрытую конечную точку (PE), администратор MySQL может управлять подключением к базе данных Azure для MySQL с помощью подключения к частной конечной точке (PEC).

> [!NOTE]
> Сейчас база данных Azure для MySQL поддерживает только автоматическое утверждение для частной конечной точки.
