---
title: Ограничения - База данных Azure для MyS'L
...
Минимальные и максимальные значения нескольких популярных параметров сервера определяются уровнем ценообразования и vCores. Обратитесь к нижеприведенной таблице для ограничений.

### <a name="max_connections"></a>max_connections

|**Уровень цен**|**Виртуальные ядра**|**Значение по умолчанию**|**Мин значение**|**Максимальная стоимость**|
|---|---|---|---|---|
|Basic|1|50|10|50|
|Basic|2|100|10|100|
...
> [!IMPORTANT]
> Для лучшего опыта, мы рекомендуем вам использовать пулер соединения, как ProxyS'L для эффективного управления соединениями.

Создание новых клиентских подключений к MyS'L требует времени, и после создания эти соединения занимают ресурсы базы данных, даже при простое. Большинство приложений запрашивают много недолговечных соединений, что усугубляет эту ситуацию. В результате меньше ресурсов, доступных для фактической рабочей нагрузки, приводит к снижению производительности. Пулер соединения, который уменьшает простоя соединения и повторно использует существующие соединения, поможет избежать этого. Чтобы узнать о настройке ProxyS'L, посетите наш [блог](https://techcommunity.microsoft.com/t5/azure-database-for-mysql/load-balance-read-replicas-using-proxysql-in-azure-database-for/ba-p/880042).

### <a name="query_cache_size"></a>query_cache_size

Кэш запроса выключен по умолчанию. Для включения кэша запроса `query_cache_type` наверстойнете параметр. 

Просмотрите [документацию MyS'L,](https://dev.mysql.com/doc/refman/5.7/en/server-system-variables.html#sysvar_query_cache_size) чтобы узнать больше об этом параметре.

> [!NOTE]
> Кэш запроса обесценился по состоянию на MyS'L 5.7.20 и был удален в MyS'L 8.0

|**Уровень цен**|**Виртуальные ядра**|**Значение по умолчанию**|**Мин значение**|**Максимальная стоимость**|
|---|---|---|---|---|
|Basic|1|Не настраивается в базовом уровне|Н/Д|Н/Д|
|Basic|2|Не настраивается в базовом уровне|Н/Д|Н/Д|
...
Просмотрите [документацию MyS'L,](https://dev.mysql.com/doc/refman/5.7/en/server-system-variables.html#sysvar_sort_buffer_size) чтобы узнать больше об этом параметре.
...
Просмотрите [документацию MyS'L,](https://dev.mysql.com/doc/refman/5.7/en/server-system-variables.html#sysvar_join_buffer_size) чтобы узнать больше об этом параметре.
...
Просмотрите [документацию MyS'L,](https://dev.mysql.com/doc/refman/5.7/en/server-system-variables.html#sysvar_max_heap_table_size) чтобы узнать больше об этом параметре.
...
Просмотрите [документацию MyS'L,](https://dev.mysql.com/doc/refman/5.7/en/server-system-variables.html#sysvar_tmp_table_size) чтобы узнать больше об этом параметре.
...
Таблицы часового пояса могут быть `mysql.az_load_timezone` заселены, вызывая сохраненную процедуру с такого инструмента, как командная строка MyS'L или MyS'L Workbench. Обратитесь к [статьям портала Azure](howto-server-parameters.md#working-with-the-time-zone-parameter) или [Azure CLI,](howto-configure-server-parameters-using-cli.md#working-with-the-time-zone-parameter) чтобы вызвать сохраненную процедуру и установить глобальные или часовые пояса сессии.

## <a name="storage-engine-support"></a>Поддержка подсистем хранилища

### <a name="supported"></a>Поддерживается
- [InnoDB](https://dev.mysql.com/doc/refman/5.7/en/innodb-introduction.html);
- [Памяти](https://dev.mysql.com/doc/refman/5.7/en/memory-storage-engine.html)
...
- Привилегия SUPER: Также ограничена [привилегия SUPER.](https://dev.mysql.com/doc/refman/5.7/en/privileges-provided.html#priv_super)
- DEFINER: Требуется супер привилегий для создания и ограничен. Если импортируются данные с помощью резервной копии, удалите команды `CREATE DEFINER` вручную или с помощью команды `--skip-definer` при выполнении mysqldump.
...
- Пожалуйста, обратитесь к [уровню ценообразования](concepts-pricing-tiers.md) для пределов размера хранилища на ценовом уровне.
...
- [Что доступно на каждом уровне обслуживания](concepts-pricing-tiers.md)
