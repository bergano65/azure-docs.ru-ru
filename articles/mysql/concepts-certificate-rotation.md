...
Все приложения, использующие SSL/TLS и проверяющие корневой сертификат, должны обновить корневой сертификат для подключения к базе данных Azure для MySQL. Если вы сейчас не используете SSL/TLS, это не повлияет на доступность вашего приложения. Вы можете проверить, пытается ли клиентское приложение использовать режим SSL с предопределенным доверенным центром сертификации (CA) [здесь](concepts-ssl-connection-security.md#ssl-default-settings).
...
*   Создайте объединенное хранилище сертификатов ЦС с сертификатами **BaltimoreCyberTrustRoot** и **DigiCertGlobalRootG2** .
    ...
    *   Для пользователей .NET (соединители MySQL или NET, MySQLConnector) убедитесь, что **BaltimoreCyberTrustRoot** и **DigiCertGlobalRootG2** существуют в хранилище сертификатов Windows — доверенные корневые центры сертификации. Если какие либо сертификаты не существуют, импортируйте недостающий сертификат.
        ...
        ![Сертификат .NET для базы данных Azure для MySQL](media/overview/netconnecter-cert.png)
    ...
    *   Для других пользователей (клиент MySQL, MySQL Workbench/C/C++/го/Писон/Руби/ФП/нодежс/Перл/Свифт) можно выполнить слияние двух файлов сертификатов ЦС, таких как приведенный ниже формат.</b>
...
Если вы используете сертификат, выданный в базе данных Azure для MySQL, как описано здесь, доступность приложения может быть прервана, так как база данных не будет доступна. В зависимости от приложения могут быть получены различные сообщения об ошибках, включая, но не ограниченные:
...
Если вы не обновите корневой сертификат до 30 ноября 2020, ваши приложения, подключающиеся по протоколу SSL/TLS и выполняющие проверку корневого сертификата, не смогут взаимодействовать с сервером базы данных MySQL, и приложение будет испытывать проблемы с подключением к серверу базы данных MySQL.
...
###  <a name="6-if-i-create-a-new-server-after-nov-30-2020-will-i-be-impacted"></a>6. Если я создам новый сервер после 30 ноября 2020, будет ли это затронуто?
