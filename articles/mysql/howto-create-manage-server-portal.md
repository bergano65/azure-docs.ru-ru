...
description: В этой статье описывается, как быстро создать новую базу данных Azure для сервера MySQL и управлять сервером с помощью портал Azure.
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: conceptual
...
> [!NOTE]
> Масштабирование поддерживается только из общего назначения в оптимизированные для памяти уровни служб и наоборот. Обратите внимание, что переход на базовую ценовую категорию и из нее после создания сервера не поддерживается в базе данных Azure для MySQL.
> 
