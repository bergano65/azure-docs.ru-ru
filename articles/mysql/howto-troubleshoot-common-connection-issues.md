...
* Конфигурация брандмауэра сервера: Убедитесь, что база данных Azure для серверного брандмауэра MyS'L настроена для включения в систему подключения от вашего клиента, включая прокси-серверы и шлюзы.
* Конфигурация клиентского брандмауэра: Брандмауэр на вашем клиенте должен разрешить подключение к серверу базы данных. В некоторых брандмауэрах необходимо разрешить IP-адреса и порты сервера, к которым нельзя подключиться, а также приложения, например MySQL.
