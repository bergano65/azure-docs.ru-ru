...
* Для использования ключа, управляемого клиентом, ключ должен иметь следующие атрибуты.
  ...
  * не отключено;
  * возможность выполнять операции _получения_, _упаковки ключа_ и _распаковки ключа_.
...
2. Выберите **разрешения для ключа**, а затем — **получить**, **обернуть**, **Распереносить** , а в качестве **участника**— имя сервера MySQL. Если сервер-участник не найден в списке существующих участников, его необходимо зарегистрировать, попытайтесь настроить шифрование данных в первый раз, что приведет к сбою.
