...
Одним из сценариев, в которых изменяется IP-адрес, является ситуация, когда используется избыточность в пределах зоны и происходит отработка отказа между первичным и вторичным сервером. Использование полного доменного имени означает, что вы можете легко повторять соединения с одной и той же строкой подключения.

Пример
* Такую `hostname = servername.mysql.database.azure.com`
* Избегайте использования `hostname = 10.0.0.4` (частного адреса) или `hostname = 40.2.45.67` (общедоступный IP-адрес)
