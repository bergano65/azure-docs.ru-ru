---
title: Импорт и экспорт - База данных Azure для MyS'L
...
- MyS'L Workbench [MyS'L Workbench Скачать](https://dev.mysql.com/downloads/workbench/) или другой сторонний myS'L инструмент, чтобы сделать импорт / экспорт.

## <a name="use-common-tools"></a>Использование распространенных инструментов
Используйте общие утилиты и инструменты, такие как MyS'L Workbench или mysqldump, для удаленного подключения и импорта или экспорта данных в базу данных Azure для MyS'L. 
...
1. Выберите все схемы, которые необходимо экспортировать. При необходимости выберите из каждой схемы конкретные объекты или таблицы и выполните экспорт. Параметры конфигурации позволяют выполнить экспорт в папку проекта или в самодостаточный файл SQL, выполните дамп сохраненных процедур и событий или пропустите данные таблицы. 
   ...
   Кроме того, используйте **Export a Result Set** для экспорта определенного результата, установленного в редакторе S'L, в другой формат, например CSV, JSON, HTML и XML. 
