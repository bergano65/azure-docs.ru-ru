---
title: Руководство по проектированию сервера Базы данных Azure для MySQL с помощью портала Azure
...
2. Выберите **Базы данных** > **База данных Azure для MySQL**. Если сервер MySQL не отображается в категории **Базы данных** , щелкните **Показать все** , чтобы просмотреть все доступные службы баз данных. Для быстрого поиска службы вы можете также ввести в поле поиска **База данных Azure для MySQL**.
   
   :::image type="content" source="./media/tutorial-design-database-using-portal/1-Navigate-to-MySQL.png" alt-text="Переход к службе MySQL":::
...
4. Щелкните **Review + create** (Просмотреть и создать). Вы можете нажать кнопку **Уведомления** на панели инструментов, чтобы отслеживать процесс развертывания. Развертывание может занять до 20 минут.
...
2. Заполните форму **Восстановление** , указав следующие сведения.
   ...
   - **Расположение.** Регион совпадает с регионом исходного сервера и не может быть изменен.
   - **Ценовая категория**. Ценовая категория совпадает с категорией исходного сервера и не может быть изменена.
...
Если вы ожидаете, что в будущем эти ресурсы вам не понадобятся, их можно удалить, удалив группу ресурсов или сам сервер MySQL. Чтобы удалить группу ресурсов, выполните следующие шаги.
