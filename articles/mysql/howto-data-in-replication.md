...
В этой статье описывается, как настроить Репликация входных данных в службе "база данных Azure для MySQL", настроив главные и репликовые серверы. В этой статье предполагается, что у вас есть опыт работы с серверами и базами данных MySQL.

Чтобы создать реплику в службе "база данных Azure для MySQL", Репликация входных данных синхронизирует данные с главного сервера MySQL в локальной среде, на виртуальных машинах или в облачных службах баз данных.
...
1. Проверьте состояние репликации.
   ...
   Если для параметра `Slave_IO_Running` и `Slave_SQL_Running` задано значение "Yes", а для `Seconds_Behind_Master` параметра равно "0", репликация работает правильно. `Seconds_Behind_Master` указывает величину задержки на реплике. Если значение не равно "0", это означает, что реплика обрабатывает обновления. 
