...
description: Шифрование данных в базе данных Azure для MySQL с помощью ключа, управляемого клиентом, позволяет создание собственных ключей (BYOK) для защиты данных в неактивных целях. Он также позволяет организациям реализовать разделение обязанностей в управлении ключами и данными.
author: kummanish
ms.author: manishku
ms.service: mysql
ms.topic: conceptual
...
ms.locfileid: "77048322"
---
# <a name="azure-database-for-mysql-data-encryption-with-a-customer-managed-key"></a>Шифрование данных базы данных Azure для MySQL с помощью управляемого клиентом ключа
...
Шифрование данных в базе данных Azure для MySQL с помощью ключа, управляемого клиентом, позволяет создание собственных ключей (BYOK) для защиты данных в неактивных целях. Он также позволяет организациям реализовать разделение обязанностей в управлении ключами и данными. При шифровании, управляемом клиентом, вы несете ответственность за полный контроль над жизненным циклом ключа, разрешениями на использование ключа и аудитом операций с ключами.

Для базы данных Azure для MySQL задается шифрование данных на уровне сервера. При такой форме шифрования данных ключ используется при шифровании ключа шифрования данных (DEK). DEK — это управляемый клиентом асимметричный ключ, хранящийся в принадлежащем заказчике и управляемом пользователем экземпляре [Azure Key Vault](../key-vault/key-Vault-secure-your-key-Vault.md) . DEK подробно описывается далее в этой статье.
...
> [!NOTE]
> Эта функция доступна во всех регионах Azure, где база данных Azure для MySQL поддерживает ценовые категории "общего назначения" и "оптимизировано для памяти".

## <a name="benefits"></a>Преимущества

Шифрование данных для базы данных Azure для MySQL предоставляет следующие преимущества.

* Повышенная прозрачность, детализированный контроль и управление для ключа шифрования.
...
* Возможность отделения управления ключами от управления данными в Организации, поэтому администратор Key Vault может отозвать разрешения на доступ к ключам, чтобы сделать зашифрованную базу данных недоступной.
* Более высокий уровень доверия для конечных пользователей, так как корпорация Майкрософт не может просматривать или извлекать ключи шифрования в Key Vault.
...
![Схема, на которой показан обзор создание собственных ключей](media/concepts-data-access-and-security-data-encryption/mysqloverview.png)

Чтобы сервер MySQL использовал управляемые клиентом ключи, хранящиеся в Key Vault для шифрования DEK, администратор Key Vault предоставляет следующие права доступа к серверу:
...
* Key Vault и база данных Azure для MySQL должны принадлежать одному клиенту Azure Active Directory (Azure AD). Межклиентские Key Vault и взаимодействия с сервером не поддерживаются. Перемещение ресурсов после этого требует перенастройки шифрования данных.
...
* Предоставьте службе "база данных Azure для MySQL" доступ к хранилищу ключей с разрешениями Get, wrapKey и unwrapKey, используя уникальное управляемое удостоверение. В портал Azure уникальный идентификатор создается автоматически при включении шифрования данных в MySQL. Подробные пошаговые инструкции по использованию портал Azure см. в статье [Настройка шифрования данных для MySQL](howto-data-encryption-portal.md) .
