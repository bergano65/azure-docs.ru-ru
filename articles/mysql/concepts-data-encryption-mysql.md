...
> [!NOTE]
> В настоящее время для использования этой возможности требуется запросить соответствующие права доступа. Для этого обратитесь по адресу AskAzureDBforMySQL@service.microsoft.com.
...
* Key Vault и База данных Azure для MySQL должны принадлежать одному клиенту Azure Active Directory (Azure AD). Взаимодействие между Key Vault и сервером, размещенными в разных клиентах, не поддерживается. При последующем перемещении ресурсов требуется перенастроить шифрование данных.
...
* Предоставьте отдельной Базе данных Azure для MySQL доступ к хранилищу ключей с разрешениями get, wrapKey и unwrapKey, используя его уникальное управляемое удостоверение. Это удостоверение автоматически создается на портале Azure при включении шифрования данных в MySQL. Подробные пошаговые инструкции по использованию портала Azure см. в статье о [настройке шифрования данных для MySQL](howto-data-encryption-portal.md).
...
* Запустите процесс восстановления или создания реплики чтения в главной Базе данных Azure для MySQL.
