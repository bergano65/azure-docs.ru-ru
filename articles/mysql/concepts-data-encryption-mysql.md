---
title: Шифрование данных базы данных Azure для MySQL с помощью управляемого клиентом ключа
description: Шифрование данных базы данных Azure для MySQL с помощью управляемого клиентом ключа
author: kummanish
ms.author: manishku
ms.service: mysql
ms.topic: conceptual
...
Шифрование данных для базы данных Azure для MySQL предоставляет следующие преимущества.
* Повышенная прозрачность, детальный контроль и управление для ключа шифрования 
...
Ключи шифрования данных, зашифрованные с помощью ключей шифрования ключей, хранятся отдельно, и только сущность с доступом к ключу шифрования ключей может расшифровывать эти ключи шифрования данных. Дополнительные сведения см. в статье [безопасность при неактивных шифрования](https://docs.microsoft.com/azure/security/azure-security-encryption-atrest).
...
![Обзор собственных ключей](media/concepts-data-access-and-security-data-encryption/mysqloverview.png)


Чтобы сервер MySQL мог использовать управляемые клиентом ключи, хранящиеся в AKV для шифрования DEK, администратор Key Vault должен предоставить серверу следующие права доступа, используя его уникальное удостоверение:
...
* Key Vault и база данных Azure для MySQL должны принадлежать одному клиенту Azure Active Directory (AAD). Межклиентские Key Vault и взаимодействия с сервером не поддерживаются. Перемещение ресурсов после этого требует перенастройки шифрования данных. Дополнительные сведения о перемещении ресурсов.
...
* Предоставьте службе "база данных Azure для MySQL" доступ к Key Vault с разрешениями **Get, wrapKey, unwrapKey,** используя уникальное управляемое удостоверение. При использовании портал Azure уникальный идентификатор автоматически создается при включении шифрования данных в MySQL. Подробные пошаговые инструкции по использованию портал Azure см. в статье [Настройка шифрования данных для MySQL](howto-data-encryption-portal.md) .
...
### <a name="requirements-for-configuring-customer-key"></a>Требования к настройке ключа клиента
...
* Убедитесь, что Key Vault и база данных Azure для MySQL находятся в одном регионе, чтобы обеспечить более быстрый доступ к операциям переноса и распаковки DEK. 
...
После того как база данных Azure для MySQL шифруется с помощью управляемого ключа клиента, хранящегося в Key Vault, любая вновь созданная копия сервера, как и локальное, либо геовосстановление, или реплики чтения, также шифруется с помощью управляемого ключа клиента. Однако их можно изменить в соответствии с управляемым ключом нового клиента для шифрования. При изменении ключа, управляемого клиентом, старые резервные копии сервера начнут использовать последний ключ.

Чтобы избежать проблем во время установки шифрования данных, управляемого клиентом, во время восстановления или создания реплики, важно выполнить следующие действия на сервере-образце и восстановлении или на реплике:

* Инициируйте процесс восстановления или чтения реплики из базы данных master Azure для MySQL.
...
* На восстановленном сервере-реплике повторно проверьте ключ, управляемый клиентом, в параметрах шифрования данных, чтобы убедиться в том, что только что созданный сервер получает разрешения на кодирование и распаковку для ключа, хранящегося в AKV.
...
Узнайте, как настроить шифрование данных с помощью управляемого клиентом ключа для базы данных Azure для MySQL, используя [портал Azure](howto-data-encryption-portal.md).
