...
В подмножестве [регионов Azure](/azure/mysql/concepts-pricing-tiers#storage)все новые подготовленные серверы могут поддерживать хранилище общего назначения вплоть до 16 ТБ. Иными словами, хранилище размером до 16 ТБ является хранилищем общего назначения по умолчанию для всех [регионов](concepts-pricing-tiers.md#storage) , в которых он поддерживается. Резервное копирование на этих 16-ТБ серверах хранилища основано на моментальных снимках. Первая полная резервная копия моментального снимка планируется сразу же после создания сервера. Первая полная резервная копия моментальных снимков сохраняется как базовая резервная копия сервера. Последующие резервные копии моментальных снимков — это только разностные резервные копии.

В подмножестве [регионов Azure](concepts-pricing-tiers.md#storage)все новые подготовленные серверы могут поддерживать хранилище общего назначения вплоть до 16 ТБ. Иными словами, хранилище размером до 16 ТБ является хранилищем общего назначения по умолчанию для всех [регионов](concepts-pricing-tiers.md#storage) , в которых он поддерживается. Резервное копирование на этих 16-ТБ серверах хранилища основано на моментальных снимках. Первая полная резервная копия моментального снимка планируется сразу же после создания сервера. Первая полная резервная копия моментальных снимков сохраняется как базовая резервная копия сервера. Последующие резервные копии моментальных снимков — это только разностные резервные копии.
...
Долгосрочное хранение резервных копий за период до 35 дней в настоящее время не поддерживается службой. Вы можете использовать mysqldump, чтобы создавать резервные копии и хранить их для долгосрочного хранения. Наша группа поддержки [посвящена пошаговым](https://techcommunity.microsoft.com/t5/azure-database-for-mysql/automate-backups-of-your-azure-database-for-mysql-server-to/ba-p/1791157) инструкциям, которые помогут вам получить доступ к этой статье.

### <a name="backup-redundancy-options"></a>Типы избыточности для резервного копирования

В службе "База данных Azure для MySQL" вы можете выбрать локально избыточное или геоизбыточное хранилище резервных копий в ценовой категории "Общего назначения" или "С оптимизацией для операций в памяти". Резервные копии в геоизбыточном хранилище не только хранятся в регионе, где размещен сервер, но и реплицируются в [связанный центр обработки данных](../best-practices-availability-paired-regions.md). Эта географическая избыточность обеспечивает лучшую защиту и возможность восстановления сервера в другом регионе в случае аварии. На уровне "Базовый" предоставляется только локально избыточное хранилище резервных копий.
...
> [!IMPORTANT]
>Если географическое восстановление выполняется для вновь созданного сервера, то Начальная синхронизация резервных копий может занять более 24 часов в зависимости от размера данных, так как время создания первоначального полного резервного копирования моментальных снимков значительно выше. Последующие резервные копии моментальных снимков — это добавочное копирование, поэтому восстановление выполняется быстрее по истечении 24 часов после создания сервера. Если вы оцениваете географические восстановления, чтобы определить RTO, рекомендуем подождать и оценить геовосстановление **только после 24 часов** создания сервера для лучшей оценки.
