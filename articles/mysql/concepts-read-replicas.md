...
> [!IMPORTANT]
> База данных Azure для MySQL использует двоичное ведение журнала на основе **строк** . Если в таблице отсутствует первичный ключ, то все строки в таблице проверяются на наличие операций DML. Это приводит к увеличению задержки репликации. Чтобы реплика могла поддерживать изменения в источнике, рекомендуется добавить первичный ключ в таблицы на исходном сервере перед созданием сервера реплики или повторного создания сервера реплики, если он уже имеется.
