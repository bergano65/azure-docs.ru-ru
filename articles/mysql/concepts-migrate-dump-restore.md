---
title: Мигрировать с помощью дампа и восстановления - База данных Azure для MyS'L
...
- [Создание сервера базы данных Azure для MySQL — портал Azure](quickstart-create-mysql-server-database-using-azure-portal.md)
- утилита командной линии [mysqldump](https://dev.mysql.com/doc/refman/5.7/en/mysqldump.html) установлена на машине.
- MyS'L Workbench [MyS'L Workbench Download](https://dev.mysql.com/downloads/workbench/) или другой сторонний инструмент MyS'L для сброса и восстановления команд.

## <a name="use-common-tools"></a>Использование распространенных инструментов
Используйте общие утилиты и инструменты, такие как MyS'L Workbench или mysqldump, для удаленного подключения и восстановления данных в базу данных Azure для MyS'L. Используйте эти инструменты на своем клиентском компьютере, подключенном к Интернету, чтобы подключиться к базе данных Azure для MySQL. Для обеспечения безопасности используйте подключение с SSL-шифрованием, а также см. статью [SSL-соединения в базе данных Azure для MySQL](concepts-ssl-connection-security.md). При переносе в базу данных Azure для MySQL не нужно перемещать файлы дампа в особое облачное расположение. 
...
-   Используйте `skip-definer` опцию в mysqlpump, чтобы опустить положения определения и S'L SECURITY из инструкций для представления и сохраненных процедур.  При перезагрузке файла дампа создается объекты, которые используют значения DEFINER и S'L SECURITY.
...
3. Нажмите на ссылку **Экспорт.** Появится страница для просмотра дампа базы данных.
...
5. Используйте кнопку **просмотра,** чтобы найти файл базы данных. 
