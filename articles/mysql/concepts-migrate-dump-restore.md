...
-   Используйте параметр `skip-definer` в mysqlpump, чтобы опустить предложения for parallelism и SQL SECURITY из инструкций CREATE для представлений и хранимых процедур.  При перезагрузке файла дампа он создает объекты, использующие значения по умолчанию и безопасность SQL.
