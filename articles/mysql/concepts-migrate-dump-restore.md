...
Вы можете использовать служебные программы MySQL, такие как **mysqldump** и **mysqlpump** , для дампа и загрузки баз данных в базу данных Azure MySQL в нескольких распространенных сценариях. В других сценариях вместо этого подхода вы можете использовать [импорт и экспорт](concepts-migrate-import-export.md).

- **При переносе всей базы данных используйте дампы базы данных**. Мы рекомендуем использовать этот способ при перемещении большого количества данных MySQL, или если вы не хотите прерывать работу службы для динамичных сайтов или приложений.
-  **Используйте дамп базы данных, если все таблицы в базе данных используют подсистему хранилища InnoDB**. База данных Azure для MySQL поддерживает только подсистему хранилища InnoDB и не поддерживает другие подсистемы хранилища. Если таблицы настроены с помощью других подсистем хранилища, преобразуйте их в формат ядра InnoDB перед перемещением в базу данных Azure для MySQL.
