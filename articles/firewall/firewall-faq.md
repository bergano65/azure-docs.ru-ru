...
description: Вопросы и ответы по брандмауэру Azure. Управляемая облачная служба сетевой безопасности, которая защищает ресурсы виртуальной сети Azure.
services: firewall
author: vhorne
ms.service: firewall
ms.topic: conceptual
...
## <a name="are-network-security-groups-nsgs-supported-on-the-azurefirewallsubnet"></a>Поддерживаются ли группы безопасности сети (группы безопасности сети) в Азурефиреваллсубнет?

Брандмауэр Azure — это управляемая служба с несколькими уровнями защиты, включая защиту платформы с помощью NIC уровня группы безопасности сети (недоступная для просмотра).  Уровень подсети группы безопасности сети не требуется для Азурефиреваллсубнет и отключены для обеспечения прерывания работы службы.
...
Для обеспечения безопасного доступа к службам PaaS рекомендуем использовать конечные точки. Вы можете включить конечные точки служб в подсети службы "Брандмауэр Azure" и отключить их в подключенных периферийных виртуальных сетях. Таким образом вы выдаете преимущества обеих функций: безопасность конечной точки службы и централизованное ведение журнала для всего трафика.
...
Поддерживается принудительное туннелирование. Дополнительные сведения см. в статье [принудительное туннелирование брандмауэра Azure (Предварительная версия)](forced-tunneling.md). 
...
## <a name="how-does-connection-draining-work"></a>Как работает сток подключений?

Для любого планового обслуживания логика очистки подключения корректно обновляет внутренние узлы. Брандмауэр Azure ожидает закрытия существующих подключений через 90 с. При необходимости клиенты могут автоматически повторно установить подключение к другому внутреннему узлу.
...
Начальная пропускная способность брандмауэра Azure составляет 2,5 – 3 Гбит/с и масштабируется до 30 Гбит/с. Он автоматически масштабируется в зависимости от загрузки ЦП и пропускной способности.

## <a name="how-long-does-it-take-for-azure-firewall-to-scale-out"></a>Сколько времени займет развертывание брандмауэра Azure?

Брандмауэр Azure постепенно масштабируется, когда средняя пропускная способность или загрузка ЦП составляет 60%. Масштабирование занимает от пяти до семи минут. При тестировании производительности убедитесь, что тестируется не менее 10 – 15 минут, и инициируйте новые подключения, чтобы воспользоваться преимуществами только что созданных узлов брандмауэра.
...
## <a name="why-can-a-tcp-ping-and-similar-tools-successfully-connect-to-a-target-fqdn-even-when-no-rule-on-azure-firewall-allows-that-traffic"></a>Почему можно успешно подключиться к целевому полному доменному имени по протоколу TCP ping и аналогичным средствам, даже если в брандмауэре Azure нет правила для этого трафика?

TCP-проверка связи на самом деле не подключается к целевому полному доменному имени. Это происходит потому, что прозрачный прокси-сервер брандмауэра Azure прослушивает порт 80/443 для исходящего трафика. Проверка связи TCP устанавливает подключение к брандмауэру, который затем удаляет пакет и записывает в журнал подключение. Такое поведение не влияет на безопасность. Тем не менее, чтобы избежать путаницы, мы изучаем потенциальные изменения в этом поведении.

## <a name="are-there-limits-for-the-number-of-ip-addresses-supported-by-ip-groups"></a>Существуют ли ограничения для количества IP-адресов, поддерживаемых группами IP?

Да. Дополнительные сведения см. в статье [Подписка Azure, пределы, квоты и ограничения службы](../azure-resource-manager/management/azure-subscription-service-limits.md#azure-firewall-limits) .
