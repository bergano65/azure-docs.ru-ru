...
В настоящее время подстановочные знаки можно использовать только в левой части полного доменного имени. Например, **_. contoso.com_* и **_contoso.com_*.

Если вы настроили **_. contoso.com_*, он допускает *анивалуе*. contoso.com, но не contoso.com (домен вершине). Если вы хотите разрешить вершину домена, вы должны явным образом настроить его как целевое FQDN.
...
Брандмауэр Azure масштабируется постепенно, когда средняя пропускная способность или загрузка ЦП составляет 60 %. Максимальная пропускная способность развертывания по умолчанию составляет примерно 2,5 – 3 Гбит/с и начинает масштабирование при достижении 60% этого числа. Процесс масштабирования занимает от пяти до семи минут. 

При тестировании производительности обязательно выполните тестирование по крайней мере с 10 до 15 минут и запустите новые подключения, чтобы воспользоваться преимуществами только что созданных узлов брандмауэра.
...
Проверка связи по протоколу TCP фактически не подключается к целевому FQDN. Это происходит потому, что прозрачный прокси-сервер Брандмауэра Azure прослушивает исходящий трафик порта 80/443. Проверка связи TCP устанавливает подключение к брандмауэру, который затем удаляет пакет. Такое поведение не влияет на безопасность. Однако во избежание путаницы мы изучаем возможные изменения в этом поведении.
