...
Начальная пропускная способность Azure Firewall составляет 2,5 - 3 Гбит/с и составляет 30 Гбит/с. Он масштабируется на основе использования процессора и пропускной их пропускной всей. Обратитесь в службу поддержки, чтобы увеличить пропускную способность брандмауэра.

## <a name="how-long-does-it-take-for-azure-firewall-to-scale-out"></a>Сколько времени занимает масштаб масштаб azure Firewall?

Масштабирование Брандмауэра Azure Firewall занимает от пяти до семи минут. Обратитесь в службу поддержки, чтобы увеличить начальную пропускную способность брандмауэра, если у вас есть всплески, которые требуют более быстрого автомасштаба.

При тестировании автомасштабирования брандмауэра следует учитывать следующие моменты:

- Однократная производительность потока TCP ограничена 1,4 Гбит/с. Таким образом, тест производительности должен установить несколько потоков TCP.
- Инструменты производительности должны постоянно устанавливать новые соединения для их подключения к уменьшенным экземплярам Firewall. Если тест устанавливает соединения один раз в начале, то они будут только связаны с первоначальными экземплярами бэкэнда. Даже если брандмауэр масштабируется, вы не увидите повышенной производительности, поскольку соединения связаны с первоначальными экземплярами.
