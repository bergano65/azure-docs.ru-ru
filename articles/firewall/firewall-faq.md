...
* *Правила сети*. Настройте правила, содержащие исходные адреса, протоколы, порты назначения и адреса назначения.
...
Если для вашей конфигурации требуется принудительное туннелирование в локальной сети и вы можете определить конечные префиксы IP-адресов для назначений в Интернете, эти диапазоны можно настроить с помощью локальной сети в качестве следующего прыжка через определяемый пользователем маршрут на Азурефиреваллсубнет. Для определения этих маршрутов можно также использовать BGP.
...
## <a name="what-does-provisioning-state-failed-mean"></a>*Состояние подготовки: Не* удалось выполнить среднее значение?
