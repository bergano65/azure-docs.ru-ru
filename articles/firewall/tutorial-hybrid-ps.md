---
title: Развертывание & настроить брандмауэр Azure в гибридной сети с помощью PowerShell
...
Для этой статьи вы создаете три виртуальные сети:
...
- **VNet-Onprem** — локальная виртуальная сеть, которая представляет локальную среду. При фактическом развертывании ее можно подключить либо через соединение VPN, либо через ExpressRoute. Для простоты в этой статье используется подключение vpn-шлюза, а виртуальная сеть, расположенная в Azure, используется для представления предприимчивой сети.
...
Если вместо этого вы хотите использовать портал Azure для завершения этого учебника, [см.](tutorial-hybrid-portal.md)

[!INCLUDE [updated-for-az](../../includes/updated-for-az.md)]

## <a name="prerequisites"></a>Предварительные требования

Эта статья требует, чтобы вы запускали PowerShell локально. Необходимо установить модуль Azure PowerShell. Чтобы узнать версию, выполните команду `Get-Module -ListAvailable Az`. Если вам необходимо выполнить обновление, ознакомьтесь со статьей, посвященной [установке модуля Azure PowerShell](https://docs.microsoft.com/powershell/azure/install-Az-ps). После проверки версии PowerShell выполните командлет `Login-AzAccount`, чтобы создать подключение к Azure.

Чтобы этот сценарий работал правильно, есть три ключевых требования:

- Определяемый пользователем маршрут (UDR) в периферийной подсети, указывающий на IP-адрес Брандмауэра Azure как шлюз по умолчанию. Распространение маршрута виртуального сетевого шлюза должно быть **отключено** в этой таблице маршрутов.
...
В этой статье смотрите раздел [«Создание маршрутов»,](#create-the-routes) чтобы узнать, как создаются эти маршруты.
...
>
>Брандмауэр Azure может быть настроен для поддержки принудительного туннелирования. Для получения дополнительной [Azure Firewall forced tunneling](forced-tunneling.md)информации см.
...
В следующем примере декларируются переменные, использующие значения для этой статьи. В ряде случаев для работы со своей подпиской требуется заменить некоторые значения собственными. Измените переменные (при необходимости), а затем скопируйте и вставьте их в консоль PowerShell.
...
Во-первых, создайте группу ресурсов для хранения ресурсов для этой статьи:
...
Откройте веб-браузер на **VM-Onprem**\<и просмотрите http:// VM-spoke-01 private IP.\>
