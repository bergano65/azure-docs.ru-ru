...
Предпочтительная конструкция заключается в интеграции внутреннего баланса нагрузки с брандмауэром Azure, так как это гораздо более простой дизайн. Вы можете использовать общественный балансик нагрузки, если у вас уже есть один развернуты, и вы хотите сохранить его на месте. Однако необходимо учитывать проблему асимметричной маршрутизации, которая может нарушить работу функции в случае применения общедоступной подсистемы балансировки нагрузки.
...
### <a name="route-table-example"></a>Пример таблицы маршрута

Например, следующие маршруты для брандмауэра на общедоступный IP-адрес 20.185.97.136, и частный IP-адрес 10.0.1.4.
...
В следующем примере правило NAT переводит трафик RDP в брандмауэр на 20.185.97.136 к балансу нагрузки на 20.42.98.220:
...
Помните, что веб-служба должна работать на хостах в пуле балансов нагрузки, если вы используете зонды работоспособности TCP для переноса 80 или зондов HTTP/HTTPS.
...
Для получения дополнительной информации [Security groups](../virtual-network/security-overview.md)о НСГ см.
