...
description: В этой статье вы узнаете, как развертывать и настраивать брандмауэр Azure с помощью Azure CLI.
services: firewall
author: vhorne
ms.service: firewall
ms.date: 08/29/2019
...
* Правила приложений, определяющие полные доменные имена (FQDN), к которым можно получить доступ из подсети. Кроме того, в [фз-УДО могут входить экземпляры S'L.](sql-fqdn-filtering.md)
...
Для этой статьи создается упрощенный единый VNet с тремя подсетями для простого развертывания. Для развертывания производственных систем рекомендуется [использовать концентратор и модель спиной.](https://docs.microsoft.com/azure/architecture/reference-architectures/hybrid-networking/hub-spoke) Брандмауэр находится в собственном VNet. Серверы рабочей нагрузки размещены в одноранговых виртуальных сетях в одном регионе с одной или несколькими подсетями.
...
Если вы предпочитаете, вы можете завершить эту процедуру с помощью [портала Azure](tutorial-firewall-deploy-portal.md) или [Azure PowerShell.](deploy-ps.md)
...
Если вы решили установить и использовать CLI локально, вам потребуется Azure CLI 2.0.4 или более поздней версии. Чтобы найти версию, запустите **az --версию**. Дополнительные сведения об установке или обновлении см. [здесь]( /cli/azure/install-azure-cli).
...
При запросе введите пароль для виртуальной машины.
...
Создайте NIC для Srv-Work с определенными IP-адресами сервера DNS и без публичного IP-адреса для тестирования.
...
Теперь создайте рабочую нагрузку виртуальной машины.
При запросе введите пароль для виртуальной машины.
...
Связать таблицу маршрутов с подсетью
