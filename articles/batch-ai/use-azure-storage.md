...
- [портала Azure](../storage/files/storage-how-to-use-files-portal.md)
...
Для сокращения путей входных данных в виде переменной среды используйте свойство `inputDirectories` файла `job.json` (или `models.JobCreateParamters.input_directories` при использовании пакета SDK Batch AI). Схема `inputDirectories` выглядит следующим образом:
...
Для сокращения путей выходных данных в виде переменной среды используйте свойство `outputDirectories` файла `job.json` (или `models.JobCreateParamters.output_directories` при использовании пакета SDK Batch AI). С помощью этого метода можно упростить пути к выходным файлам. Схема `outputDirectories` выглядит следующим образом:
