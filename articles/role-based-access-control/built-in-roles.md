...
description: В этой статье описаны встроенные роли Azure для управления доступом на основе ролей Azure (RBAC). В нем перечислены действия, действия, действия данных и notDataActions.
...
[Контроль доступа на основе ролей Azure (RBAC)](overview.md) имеет несколько встроенных ролей Azure, которые можно назначить пользователям, группам, директорам служб и управляемым идентификаторам. Назначение ролей является способом управления доступом к ресурсам Azure. Если встроенные роли не отвечают конкретным потребностям организации, можно создать собственные [пользовательские роли Azure.](custom-roles.md)
...
> | [Администратор доступа пользователя](#user-access-administrator) | Позволяет управлять доступом пользователей к ресурсам Azure. | 18d7d88d-d35e-4fb5-a5c3-7773c20a72d9 |
> | **Службы вычислений** |  |  |
> | [Классический виртуальный вклад машины](#classic-virtual-machine-contributor) | Позволяет управлять классическими виртуальными машинами, но не доступом к ним и не учетной записью виртуальной сети или хранения, к которой они подключены. | d73bb868-a0df-4d4d-bd69-98a00b01fccb |
> | [Виртуальная машина Администратор Войти](#virtual-machine-administrator-login) | Позволяет просматривать виртуальные машины на портале и входить в систему с правами администратора. | 1c0163c0-47e6-4577-8991-ea5c82e286e4 |
...
> | [Виртуальная машина Пользователь Войти](#virtual-machine-user-login) | Позволяет просматривать виртуальные машины на портале и входить в систему с правами обычного пользователя. | fb879df8-f326-4884-b1cf-06f3ad86be52 |
> | **Сеть** |  |  |
> | [Участник конечных точек CDN](#cdn-endpoint-contributor) | Можно управлять конечными точками CDN, но не может предоставить доступ другим пользователям. | 426e0c7f-0c7e-4658-b36f-ff54d6c29b45 |
> | [Читатель конечной точки CDN](#cdn-endpoint-reader) | Можно просматривать конечные точки CDN, но не может вносить изменения. | 871e35f6-b5c1-49cc-a043-bde969a0f2cd |
> | [Участник профиля CDN](#cdn-profile-contributor) | Можно управлять профилями CDN и их конечными точками, но не может предоставить доступ другим пользователям. | ec156ff8-a8d1-4d15-830c-5b80698ca432 |
> | [Читатель данных профиля CDN](#cdn-profile-reader) | Можно просматривать профили CDN и их конечные точки, но не может вносить изменения. | 8f96442b-4075-438f-813d-ad51ab4019af |
> | [Классический сетевой вклад](#classic-network-contributor) | Позволяет управлять классическими сетями, но не доступом к ним. | b34d265f-36f7-4a0d-a4d4-e158ca92e90f |
...
> | [Участник учетной записи хранения](#storage-account-contributor) | Разрешает управление учетными записями хранения. Предоставляет доступ к ключу учетной записи, который может быть использован для доступа к данным через авторизацию Shared Key. | 17d1049b-9a84-46fb-8f53-869881c3d3ab |
> | [Роль службы оператора ключей учетных записей хранения](#storage-account-key-operator-service-role) | Разрешает листинг и регенерацию ключей доступа к учетной записи хранилища. | 81a9662b-bebf-436f-a333-f67b29880f12 |
> | [участник данных BLOB-объектов хранилища](#storage-blob-data-contributor); | Прочитайте, напишите и удалите контейнеры и капли для хранения Azure. Чтобы узнать, какие действия необходимы для данной операции данных, [см.](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations) | ba92f5b4-2d11-453d-a403-e96b0029c9fe |
> | [владелец данных BLOB-объектов хранилища](#storage-blob-data-owner); | Обеспечивает полный доступ к контейнерам и данным Azure Storage, включая назначение контроля доступа POSIX. Чтобы узнать, какие действия необходимы для данной операции данных, [см.](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations) | b7e6dc6d-f1e8-4753-8033-0f276bb0955b |
> | [читатель данных больших двоичных объектов хранилища](#storage-blob-data-reader). | Прочитайте и перечислите контейнеры и капли для хранения Azure. Чтобы узнать, какие действия необходимы для данной операции данных, [см.](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations) | 2a2b9908-6ea1-4ae2-8e65-a410df84e7d1 |
> | [Хранилище Blob Делегатор](#storage-blob-delegator) | Получите ключ делегации пользователя, который затем может быть использован для создания общей подписи доступа для контейнера или капли, подписанных с учетными данными Azure AD. Для получения дополнительной информации [см.](https://docs.microsoft.com/rest/api/storageservices/create-user-delegation-sas) | db58b8e5-c6ad-4a2a-8342-4190687cbf4a |
> | [Вкладчик обмена файлами хранения файлов](#storage-file-data-smb-share-contributor) | Позволяет читать, писать и удалять доступ к файлам/каталогам в файлах Azure. Эта роль не имеет встроенного эквивалента на файловых серверах Windows. | 0c867c2a-1d8c-454a-a3db-ab2ea1bdc8bb |
> | [Хранение файла данных SMB Доля Повышенный вкладчик](#storage-file-data-smb-share-elevated-contributor) | Позволяет читать, писать, удалять и изменять АКЛ в файлах/каталогах в файлах Azure. Эта роль эквивалентна aCL файла изменения на серверах файлов Windows. | a7264617-510b-434b-a828-9731dc254ea7 |
> | [Хранение файла данных SMB Доля Читателя](#storage-file-data-smb-share-reader) | Позволяет читать доступ к файлам/каталогам в файлах Azure. Эта роль эквивалентна файлу ACL чтения на серверах файлов Windows. | aba4ae5f-2193-4029-9191-0cb91df5e314 |
> | [Вкладчик очереди хранения](#storage-queue-data-contributor) | Прочитайте, записывайте и удалите очереди и сообщения азового хранилища. Чтобы узнать, какие действия необходимы для данной операции данных, [см.](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations) | 974c5e8b-45b9-4653-ba55-5f855dd0fb88 |
...
> | **Контейнеры** |  |  |
> | [AcrDelete](#acrdelete) | acr удалить | c2f4ef07-c644-48eb-af81-4b1b4947fb11 |
...
> | [Вкладчик СЗЛ ДБ](#sql-db-contributor) | Позволяет управлять базами данных SQL, но не доступом к ним. Кроме того, не позволяет управлять их политиками безопасности или родительскими серверами SQL Server. | 9b7fa17d-e63e-47b0-bb0a-15c516ac86ec |
...
> | [Оператор кластера HDInsight](#hdinsight-cluster-operator) | Позволяет читать и изменять конфигурации кластеров HDInsight. | 61ed4efc-fab3-44fd-b111-e24485cc132a |
...
> | **Блокчейн** |  |  |
> | [Доступ к узлову Blockchain (Предварительный просмотр)](#blockchain-member-node-access-preview) | Позволяет получить доступ к узтам члена Blockchain | 31a002a1-acaf-453e-8a5b-297c9ca1ea24 |
...
> | [Читатель данных когнитивных служб (Предварительный просмотр)](#cognitive-services-data-reader-preview) | Позволяет читать данные Cognitive Services. | b59867f0-fa02-499b-be73-45a86b5b3e1c |
...
> | [Владелец конфигурации приложений](#app-configuration-data-owner) | Позволяет получить полный доступ к данным конфигурации приложений. | 5ae67dd6-50cb-40e7-96ff-dc2bfa4b606b |
> | [Читатель данных конфигурации приложений](#app-configuration-data-reader) | Позволяет читать доступ к данным конфигурации приложения. | 516239f1-63e1-4d78-a4de-a74fb236a071 |
> | [Владелец шины Azure](#azure-service-bus-data-owner) | Предоставляет полный доступ к ресурсам Azure Service Bus. | 090c5cfd-751d-490a-894a-3ce6f10941919191919 |
> | [Получатель данных об обслуживании сервиса Azure](#azure-service-bus-data-receiver) | Позволяет получить доступ к ресурсам Azure Service Bus. | 4f6d3b9b-027b-4f4c-9142-0e5a2a2247e0 |
> | [Отправитель данных шины службы Azure](#azure-service-bus-data-sender) | Позволяет отправлять доступ к ресурсам Azure Service Bus. | 69a216fc-b8fb-44d8-bc22-1f3c2cd27a39 |
...
> | **Безопасность** |  |  |
> | [Вкладчик Azure Sentinel](#azure-sentinel-contributor) | Вкладчик Azure Sentinel | ab8e14d6-4a74-4a29-9ba8-549422addade |
> | [Лазурный Страж Читатель](#azure-sentinel-reader) | Лазурный Страж Читатель | 8d289c81-5878-46d4-8554-54e1e3d8b5cb |
> | [Лазурный Сентинел Ответчик](#azure-sentinel-responder) | Лазурный Сентинел Ответчик | 3e150937-b8fe-4cfb-8069-0eaf05ecd056 |
> | [Ключевой вкладчик Vault](#key-vault-contributor) | Позволяет управлять хранилищами ключей, но не доступом к ним. | f25e0fa2-a7c8-4377-a976-54943a77a395 |
> | [Админ безопасности](#security-admin) | Может просматривать политики безопасности и состояния безопасности, изменять политики безопасности, просматривать оповещения и рекомендации, а также закрывать предупреждения и рекомендации. | fb1c8493-542b-48eb-b624-b4c8fea62acd |
...
> | [Автор трудовых книжки](#workbook-contributor) | Можно сохранить общие трудовые книжки. | e8ddcd69-c73f-4f9f-9844-4100522f16ad |
> | [Читатель трудовых книжки](#workbook-reader) | Можно читать трудовые книжки. | b279062a-9be3-42a0-92ae-8b3cf002ec4d |
...
> | [Лазурная подключенная машина на борту](#azure-connected-machine-onboarding) | Может на борту Azure Connected Machines. | b64e21ea-ac4e-4cdf-9dc9-5b892992bee7 |
> | [Администратор ресурсов подключенных машин Azure](#azure-connected-machine-resource-administrator) | Можно читать, писать, удалять и повторно на борту Azure Connected Machines. | cd570a14-e51a-42ad-bac8-bafd67325302 |
> | [Биллинг Читатель](#billing-reader) | Разрешает читать данные выставления счетов. | fa23ad8b-c56e-40d8-ac0c-ce449e1d2c64 |
...
> | [Оператор схемы](#blueprint-operator) | Может назначать существующие опубликованные чертежи, но не может создавать новые чертежи. Обратите внимание, что это работает только в том случае, если назначение выполнено с помощью управляемого ипотечно-удостоверения, назначенного пользователем. | 437d2ced-4a38-4302-8479-ed2bcb43d090 |
> | [Вкладчик по управлению затратами](#cost-management-contributor) | Позволяет просматривать расходы и управлять конфигурацией затрат (например, бюджеты, экспорты) | 434105ed-43f6-45c7-a02f-909b2ba83430 |
...
> | [Роль автора управляемых приложений](#managed-application-contributor-role) | Позволяет создавать управляемые ресурсы приложения. | 641177b8-a67a-45b9-a033-47bc880bb21e |
...
> | [Назначение зависотки управляемых служб Удалить роль](#managed-services-registration-assignment-delete-role) | Роль удаления управленных служб Registration Assignment Delete Role позволяет управляющим пользователям-арендаторам удалить назначение регистрации, назначенное их арендатору. | 91c1777a-f3dc-4fae-b103-61d183457e46 |
...
> | [Автор данных по вопросам политики (Предварительный просмотр)](#policy-insights-data-writer-preview) | Позволяет читать политики ресурсов и писать доступ к событиям политики компонентов ресурса. | 66bb4e9e-b016-4a94-8249-4c0511c2be84 |
> | [Участник политики ресурсов](#resource-policy-contributor) | Пользователи с правами на создание/изменение политики ресурсов, создание билета поддержки и чтение ресурсов/иерархии. | 36243c78-bf99-498c-9df9-86d9f8d28608 |
...
> | [Тег Вкладчик](#tag-contributor) | Позволяет управлять тегами на сущностях, не предоставляя доступа к самим объектам. | 4a9ae827-6dc8-4573-8ac7-8239d42aa03f |
> | **Прочее** |  |  |
...
> | **Недействия** |  |
> | Microsoft.Authorization/*/Delete | Удаление ролей, назначений политик, определений политик и определений, установленных политик |
> | Microsoft.Authorization/*/Write | Создание ролей, назначений ролей, назначений политики, определений политик и определений, установленных политик |
...
> | Microsoft.Blueprint/blueprintAssignments/write | Создание или обновление любых назначений чертежей |
> | Microsoft.Blueprint/blueprintAssignments/delete | Удаление любых назначений чертежа |
> | **Действия с данными** |  |
> | *отсутствуют* |  |
> | **NotDataActions** |  |
> | *отсутствуют* |  |
...
> | Microsoft.Insights/alertRules/* | Создание правил оповещения Insights и управление ими |
> | Microsoft.Network/applicationGateways/backendAddressPools/join/action | Присоединяется к пулу адресов бэкэнда шлюза приложения. Не оповещенный. |
> | Microsoft.Network/loadBalancers/backendAddressPools/join/action | Присоединяется к пулу адресного адреса балансера нагрузки. Не оповещенный. |
> | Microsoft.Network/loadBalancers/inboundNatPools/join/action | Присоединяется к входящим пулу NAT-баланса нагрузки. Не предупреждаю. |
> | Microsoft.Network/loadBalancers/inboundNatRules/join/action | Присоединяется к правилу входящего нат нат нагрузки. Не оповещенный. |
> | Microsoft.Network/loadBalancers/probes/join/action | Разрешение использования зондов подсистемы балансировки нагрузки. Например, при наличии этого разрешения свойство healthProbe в масштабируемом наборе виртуальных машин может ссылаться на конкретный зонд. Не предупреждаю. |
> | Microsoft.Network/loadBalancers/read | Возвращает определение подсистемы балансировки нагрузки. |
...
> | Microsoft.Network/networkSecurityGroups/join/action | Присоединяется к группе сетевой безопасности. Не оповещенный. |
> | Microsoft.Network/networkSecurityGroups/read | Возвращает определение группы безопасности сети. |
> | Microsoft.Network/publicIPAddresses/join/action | Присоединяется к общедоступному IP-адресу. Не оповещенный. |
> | Microsoft.Network/publicIPAddresses/read | Возвращает определение общедоступного IP-адреса. |
> | Microsoft.Network/virtualNetworks/read | Возвращает определение виртуальной сети. |
> | Microsoft.Network/virtualNetworks/subnets/join/action | Присоединяется к виртуальной сети. Не оповещенный. |
...
Можно управлять конечными точками CDN, но не может предоставить доступ другим пользователям.
...
Можно просматривать конечные точки CDN, но не может вносить изменения.
...
Можно управлять профилями CDN и их конечными точками, но не может предоставить доступ другим пользователям.
...
Можно просматривать профили CDN и их конечные точки, но не может вносить изменения.
...
Можно создавать и управлять кластером Avere vFXT.
...
> | Microsoft.Network/virtualNetworks/subnets/join/action | Присоединяется к виртуальной сети. Не оповещенный. |
> | Microsoft.Network/virtualNetworks/subnets/joinViaServiceEndpoint/action | Присоединение к подсети ресурса, например учетной записи хранения или базы данных SQL. Не предупреждаю. |
> | Microsoft.Network/networkSecurityGroups/join/action | Присоединяется к группе сетевой безопасности. Не оповещенный. |
> | Microsoft.Resources/deployments/* | Создание развертываний группы ресурсов и управление ими |
> | Microsoft.Insights/alertRules/* | Создание правил оповещения Insights и управление ими |
> | Microsoft.Resources/subscriptions/resourceGroups/read | Возвращает группы ресурсов или выводит их список. |
> | Microsoft.Хранение//прочитайте |  |
> | Microsoft.Storage/storageAccounts/* | Создание учетных записей хранения и управление ими |
> | Microsoft.Support/* | Создание запросов в службу поддержки и управление ими |
> | Microsoft.Resources/подписки/ресурсгруппы/ресурсы/читать | Возвращает ресурсы группы ресурсов. |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
...
Используется кластером Avere vFXT для управления кластером
...
Позволяет управлять службой резервного копирования, но не может создавать хранилища и давать доступ к другим
...
> | Microsoft.Databox/места/проверкиВходы/действия | Этот метод выполняет все типы проверки. |
> | Microsoft.Databox/места/регионКонфигурация/действие | Этот метод возвращает конфигурации для региона. |
> | Microsoft.Databox/места/validateAddress/action | Проверяет адрес доставки и предлагает альтернативные адреса, если они доступны. |
> | Microsoft.ResourceHealth/availabilityStatuses/read | Возвращает состояния доступности для всех ресурсов в указанной области. |
> | Microsoft.Support/* | Создание запросов в службу поддержки и управление ими |
> | **Недействия** |  |
> | *отсутствуют* |  |
...
Разрешает управление учетными записями хранения. Предоставляет доступ к ключу учетной записи, который может быть использован для доступа к данным через авторизацию Shared Key.
...
Разрешает листинг и регенерацию ключей доступа к учетной записи хранилища.
...
Прочитайте, напишите и удалите контейнеры и капли для хранения Azure. Чтобы узнать, какие действия необходимы для данной операции данных, [см.](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)

> [!div class="mx-tableFixed"]
> |  |  |
> | --- | --- |
> | **Действия** |  |
> | Microsoft.Storage/storageAccounts/blobServices/containers/delete | Удалить контейнер. |
> | Microsoft.Storage/storageAccounts/blobServices/containers/read | Верните контейнер или список контейнеров. |
> | Microsoft.Storage/storageAccounts/blobServices/containers/write | Измените метаданные или свойства контейнера. |
> | Microsoft.Storage/storageAccounts/blobServices/generateUserDelegationKey/action | Возвращает ключ делегации пользователя для службы Blob. |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
...
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | Microsoft.Storage/storageAccounts/blobServices/containers/blobs/* | Полные разрешения на капли. |
> | **NotDataActions** |  |
> | *отсутствуют* |  |
...
Прочитайте и перечислите контейнеры и капли для хранения Azure. Чтобы узнать, какие действия необходимы для данной операции данных, [см.](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)
...
### <a name="storage-blob-delegator"></a>Хранилище Blob Делегатор

Получите ключ делегации пользователя, который затем может быть использован для создания общей подписи доступа для контейнера или капли, подписанных с учетными данными Azure AD. Для получения дополнительной информации [см.](https://docs.microsoft.com/rest/api/storageservices/create-user-delegation-sas)
...
### <a name="storage-file-data-smb-share-contributor"></a>Вкладчик обмена файлами хранения файлов

Позволяет читать, писать и удалять доступ к файлам/каталогам в файлах Azure. Эта роль не имеет встроенного эквивалента на файловых серверах Windows.
...
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | Microsoft.Storage/storageAccounts/fileServices/fileshares/files/read | Возвращает файл/папку или список файлов/папок. |
> | Microsoft.Storage/storageAccounts/fileServices/fileshares/files/write | Возвращает результат написания файла или создания папки. |
> | Microsoft.Storage/storageAccounts/fileServices/fileshares/files/delete | Возвращает результат удаляния файла/папки. |
> | **NotDataActions** |  |
> | *отсутствуют* |  |
...
### <a name="storage-file-data-smb-share-elevated-contributor"></a>Хранение файла данных SMB Доля Повышенный вкладчик

Позволяет читать, писать, удалять и изменять АКЛ в файлах/каталогах в файлах Azure. Эта роль эквивалентна aCL файла изменения на серверах файлов Windows.
...
> | Microsoft.Storage/storageAccounts/fileServices/fileshares/files/write | Возвращает результат написания файла или создания папки. |
> | Microsoft.Storage/storageAccounts/fileServices/fileshares/files/delete | Возвращает результат удаляния файла/папки. |
> | Microsoft.Storage/storageAccounts/fileServices/fileshares/files/modifypermissions/action | Возвращает результат изменения разрешения в файл/папку. |
> | **NotDataActions** |  |
> | *отсутствуют* |  |
...
### <a name="storage-file-data-smb-share-reader"></a>Хранение файла данных SMB Доля Читателя

Позволяет читать доступ к файлам/каталогам в файлах Azure. Эта роль эквивалентна файлу ACL чтения на серверах файлов Windows.
...
### <a name="storage-queue-data-contributor"></a>Вкладчик очереди хранения

Прочитайте, записывайте и удалите очереди и сообщения азового хранилища. Чтобы узнать, какие действия необходимы для данной операции данных, [см.](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)

> [!div class="mx-tableFixed"]
> |  |  |
> | --- | --- |
> | **Действия** |  |
> | Microsoft.Storage/storageAccounts/queueServices/queues/delete | Удалите очередь. |
> | Microsoft.Storage/storageAccounts/queueServices/queues/read | Верните очередь или список очередей. |
> | Microsoft.Storage/storageAccounts/queueServices/queues/write | Изменение метаданных очереди или свойств. |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | Microsoft.Storage/storageAccounts/queueServices/queues/messages/delete | Удалите одно или несколько сообщений из очереди. |
> | Microsoft.Storage/storageAccounts/queueServices/queues/messages/read | Пик или получить одно или несколько сообщений из очереди. |
...
### <a name="storage-queue-data-message-processor"></a>Обработка сообщений о очередях хранения данных

Изгоняй, извлечение и удаление сообщения из очереди хранения Azure. Чтобы узнать, какие действия необходимы для данной операции данных, [см.](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)
...
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | Microsoft.Storage/storageAccounts/queueServices/queues/messages/read | Подглядывай сообщение. |
> | Microsoft.Storage/storageAccounts/queueServices/queues/messages/process/action | Извлекать и удалять сообщение. |
> | **NotDataActions** |  |
> | *отсутствуют* |  |
...
### <a name="storage-queue-data-message-sender"></a>Отправитель сообщений о очередях хранилища

Добавление сообщений в очередь хранения Azure. Чтобы узнать, какие действия необходимы для данной операции данных, [см.](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)
...
### <a name="storage-queue-data-reader"></a>Читатель очередей хранения данных

Прочитайте и перечислите очереди и сообщения азового хранения. Чтобы узнать, какие действия необходимы для данной операции данных, [см.](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)
...
Предоставляет доступ к данным, связанным с чтением карты, из учетной записи карт Azure.
...
> | Microsoft.Web/hostingEnvironments/Join/Action | Присоединяется к среде службы приложений |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | *отсутствуют* |  |
...
> | --- | --- |
> | **Действия** |  |
> | Microsoft.ContainerRegistry/регистраторы/артефакты/удаление | Удаление артефакта в реестре контейнеров. |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | *отсутствуют* |  |
...
> | --- | --- |
> | **Действия** |  |
> | Microsoft.ContainerRegistry/регистраторы/карантин/читай | Извлечение или получение помещенных в карантин образов из реестра контейнеров. |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | *отсутствуют* |  |
...
> | **Действия** |  |
> | Microsoft.ContainerRegistry/регистраторы/карантин/читай | Извлечение или получение помещенных в карантин образов из реестра контейнеров. |
> | Microsoft.ContainerRegistry/регистраторы/карантин/запись | Запись и изменение состояния карантина образов, помещенных в карантин. |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | *отсутствуют* |  |
...
### <a name="cosmos-db-operator"></a>Оператор Космос ДБ

Позволяет управлять dB-учетными записями Azure Cosmos, но не получать доступ к данным в них. Предотвращает доступ к ключам учетной записи и строкам подключения.
...
> | Microsoft.Insights/metricDefinitions/read | Считывает определения метрик. |
> | **Недействия** |  |
> | Microsoft.Sql/управляемыеInstances/databases/currentSensitivityLabels/» |  |
> | Microsoft.Sql/управляемыеInstances/databases/recommendedSensitivityLabels/» |  |
> | Microsoft.Sql/managedInstances/databases/schemas/tables/columns/sensitivityLabels/ |  |
> | Microsoft.Sql/управляемыеInstances/databases/securityAlertPolicies/» |  |
> | Microsoft.Sql/управляемыеInstances/databases/sensitivityLabels/» |  |
> | Microsoft.Sql/managedInstances/databases/vulnerabilityAssessments/* |  |
> | Microsoft.Sql/управляемыеInstances/securityAlertПолитика/з. |  |
...
> | Microsoft.Sql/серверы/базы данных/текущиеЧувствительностьLabels/» |  |
> | Microsoft.Sql/servers/databases/dataMaskingPolicies/* | Изменение политик маскирования данных. |
> | Microsoft.Sql/servers/databases/extendedAuditingSettings/* |  |
> | Microsoft.Sql/серверы/базы данных/рекомендуемаяЧувствительностьLabels/» |  |
...
### <a name="sql-managed-instance-contributor"></a>Автор управляемых экземпляров S'L

Позволяет управлять управляемыми экземплярами и требуемой конфигурацией сети, но не может предоставить доступ к другим.
...
> | Microsoft.Sql/locations/*/read |  |
> | Microsoft.Sql/управляемыеInstances/» |  |
> | Microsoft.Support/* | Создание запросов в службу поддержки и управление ими |
...
> | Microsoft.Sql/managedInstances/databases/vulnerabilityAssessments/* |  |
> | Microsoft.Sql/управляемыеInstances/securityAlertПолитика/з. |  |
> | Microsoft.Sql/управляемыеInstances/базы данных/прозрачныеШифрование данных/» |  |
> | Microsoft.Sql/managedInstances/vulnerabilityAssessments/* |  |
...
> | Microsoft.Sql/servers/databases/schemas/tables/columns/read | Получить столбец базы данных. |
> | Microsoft.Sql/servers/databases/schemas/tables/columns/sensitivityLabels/* |  |
> | Microsoft.Sql/servers/databases/schemas/tables/read | Получить таблицу базы данных. |
...
> | Microsoft.Sql/servers/databases/sensitivityLabels/* |  |
> | Microsoft.Sql/серверы/базы данных/прозрачноешифрование данных/» |  |
> | Microsoft.Sql/servers/databases/vulnerabilityAssessments/* |  |
> | Microsoft.Sql/servers/databases/vulnerabilityAssessmentScans/* |  |
> | Microsoft.Sql/servers/databases/vulnerabilityAssessmentSettings/* |  |
...
Позволяет управлять серверами и базами данных, но не получать к ним доступ, а не к политикам, связанным с безопасностью.
...
### <a name="azure-event-hubs-data-owner"></a>Владелец концентраторов событий Azure

Позволяет получить полный доступ к ресурсам Azure Event Hubs.
...
### <a name="azure-event-hubs-data-receiver"></a>Получатель данных концентраторов azure event

Позволяет получить доступ к ресурсам Azure Event Hubs.
...
### <a name="azure-event-hubs-data-sender"></a>Отправитель концентраторов событий Azure

Позволяет отправлять доступ к ресурсам Azure Event Hubs.
...
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | Microsoft.EventHub//отправить/действия |  |
> | **NotDataActions** |  |
> | *отсутствуют* |  |
...
> | Microsoft.Resources/subscriptions/resourceGroups/read | Возвращает группы ресурсов или выводит их список. |
> | Microsoft.Support/* | Создание запросов в службу поддержки и управление ими |
> | Microsoft.EventGrid/eventSubscriptions/write | Создание или обновление подписки на события |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | *отсутствуют* |  |
...
Позволяет читать и изменять конфигурации кластеров HDInsight.
...
> | Microsoft.HDInsight/кластеры/getGatewaySettings/action | Получите настройки шлюза для кластера HDInsight |
> | Microsoft.HDInsight/кластеры/обновлениеGatewaySettings/action | Обновление настроек шлюза для кластера HDInsight |
> | Microsoft.HDInsight/кластеры/конфигурации/з |  |
> | Microsoft.Resources/subscriptions/resourceGroups/read | Возвращает группы ресурсов или выводит их список. |
...
### <a name="blockchain-member-node-access-preview"></a>Доступ к узлову Blockchain (Предварительный просмотр)

Позволяет получить доступ к узтам члена Blockchain

> [!div class="mx-tableFixed"]
> |  |  |
> | --- | --- |
> | **Действия** |  |
> | Microsoft.Blockchain/blockchainMembers/transactionNodes/read | Получает или перечисляет существующий узл транзакций участника Blockchain (ы). |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | Microsoft.Blockchain/blockchainMembers/transactionNodes/connect/action | Подключается к узлам транзакций участника Blockchain. |
> | **NotDataActions** |  |
> | *отсутствуют* |  |
...
### <a name="cognitive-services-data-reader-preview"></a>Читатель данных когнитивных служб (Предварительный просмотр)

Позволяет читать данные Cognitive Services.
...
### <a name="spatial-anchors-account-contributor"></a>Вкладчик счета пространственных якорей

Позволяет управлять пространственными якорями в учетной записи, но не удалять их
...
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | Microsoft.MixedReality/SpatialAnchorsAccounts/create/action | Создание пространственных якорей |
> | Microsoft.MixedReality/SpatialAnchorsAccounts/discovery/read | Откройте для себя близлежащие пространственные якоря |
> | Microsoft.MixedReality/SpatialAnchorsAccounts/свойства/читай | Получить свойства пространственных якорей |
> | Microsoft.MixedReality/SpatialAnchorsAccounts/query/read | Найдите пространственные якоря |
> | Microsoft.MixedReality/SpatialAnchorsAccounts/submitdiag/read | Отправка диагностических данных для улучшения качества службы пространственных якорей Azure |
> | Microsoft.MixedReality/SpatialAnchorsAccounts/write | Обновление свойств пространственных якорей |
> | **NotDataActions** |  |
> | *отсутствуют* |  |
...
### <a name="spatial-anchors-account-owner"></a>Владелец счета пространственных якорей

Позволяет управлять пространственными якорями в вашем аккаунте, включая их удалять
...
> | **Действия с данными** |  |
> | Microsoft.MixedReality/SpatialAnchorsAccounts/create/action | Создание пространственных якорей |
> | Microsoft.MixedReality/SpatialAnchorsAccounts/delete | Удаление пространственных якорей |
> | Microsoft.MixedReality/SpatialAnchorsAccounts/discovery/read | Откройте для себя близлежащие пространственные якоря |
> | Microsoft.MixedReality/SpatialAnchorsAccounts/свойства/читай | Получить свойства пространственных якорей |
> | Microsoft.MixedReality/SpatialAnchorsAccounts/query/read | Найдите пространственные якоря |
> | Microsoft.MixedReality/SpatialAnchorsAccounts/submitdiag/read | Отправка диагностических данных для улучшения качества службы пространственных якорей Azure |
...
### <a name="spatial-anchors-account-reader"></a>Читатель учетной записи пространственных якорей

Позволяет найти и прочитать свойства пространственных якорей в вашем аккаунте
...
### <a name="app-configuration-data-owner"></a>Владелец конфигурации приложений

Позволяет получить полный доступ к данным конфигурации приложений.
...
> | Microsoft.AppConfiguration/configurationStores/'/'удаляем |  |
> | **NotDataActions** |  |
> | *отсутствуют* |  |
...
Позволяет читать доступ к данным конфигурации приложения.
...
### <a name="azure-service-bus-data-owner"></a>Владелец шины Azure

Предоставляет полный доступ к ресурсам Azure Service Bus.
...
### <a name="azure-service-bus-data-receiver"></a>Получатель данных об обслуживании сервиса Azure

Позволяет получить доступ к ресурсам Azure Service Bus.

> [!div class="mx-tableFixed"]
> |  |  |
> | --- | --- |
> | **Действия** |  |
> | Microsoft.ServiceBus//очереди/читай |  |
> | Microsoft.ServiceBus//темы/читай |  |
...
### <a name="azure-service-bus-data-sender"></a>Отправитель данных шины службы Azure

Позволяет отправлять доступ к ресурсам Azure Service Bus.
...
> | --- | --- |
> | **Действия** |  |
> | Microsoft.AzureStack/регистрации/продукты/действия |  |
...
> | **Действия** |  |
> | Microsoft.Authorization/*/read | Чтение ролей и назначений ролей |
> | Microsoft.EventGrid/eventSubscriptions/read | Читать событиеПодписка |
> | Microsoft.EventGrid/topicTypes/eventSubscriptions/read | Создание списка подписок на глобальные события по типу темы |
> | Microsoft.EventGrid/locations/eventSubscriptions/read | Создание списка подписок на события в регионе |
> | Microsoft.EventGrid/locations/topicTypes/eventSubscriptions/read | Создание списка подписок на события в регионе по типу темы |
> | Microsoft.Resources/subscriptions/resourceGroups/read | Возвращает группы ресурсов или выводит их список. |
...
Позволяет управлять логическими приложениями, но не изменять доступ к ним.
...
> | Microsoft.Web/serverFarms/join/action |  |
> | Microsoft.Web/serverFarms/read | Возвращает свойства плана службы приложений. |
> | Microsoft.Web/sites/functions/listSecrets/action | Список секретов функции. |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | *отсутствуют* |  |
...
Позволяет читать, включать и отключать логические приложения, но не редактируть и не обновлять их.
...
### <a name="azure-sentinel-contributor"></a>Вкладчик Azure Sentinel

Вкладчик Azure Sentinel
...
> | Microsoft.Insights/рабочие книги/ |  |
> | Microsoft.Authorization/*/read | Чтение ролей и назначений ролей |
> | Microsoft.Insights/alertRules/* | Создание правил оповещения Insights и управление ими |
> | Microsoft.Resources/deployments/* | Создание развертываний группы ресурсов и управление ими |
> | Microsoft.Resources/subscriptions/resourceGroups/read | Возвращает группы ресурсов или выводит их список. |
...
### <a name="azure-sentinel-reader"></a>Лазурный Страж Читатель

Лазурный Страж Читатель
...
> | Microsoft.OperationalInsights/workspaces/query//read |  |
> | Microsoft.OperationalInsights/workspaces/dataSources/read | Возвращает источники данных в рабочей области. |
> | Microsoft.Insights/workbooks/read | Читать трудовую книжку |
> | Microsoft.Authorization/*/read | Чтение ролей и назначений ролей |
> | Microsoft.Insights/alertRules/* | Создание правил оповещения Insights и управление ими |
> | Microsoft.Resources/deployments/* | Создание развертываний группы ресурсов и управление ими |
> | Microsoft.Resources/subscriptions/resourceGroups/read | Возвращает группы ресурсов или выводит их список. |
...
### <a name="azure-sentinel-responder"></a>Лазурный Сентинел Ответчик

Лазурный Сентинел Ответчик
...
Может просматривать политики безопасности и состояния безопасности, изменять политики безопасности, просматривать оповещения и рекомендации, а также закрывать предупреждения и рекомендации.
...
### <a name="security-assessment-contributor"></a>Вкладчик по оценке безопасности

Позволяет продвигать оценки в Центр безопасности

> [!div class="mx-tableFixed"]
> |  |  |
> | --- | --- |
> | **Действия** |  |
> | Microsoft.Security/оценки/запись | Создание или обновление оценок безопасности по подписке |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | *отсутствуют* |  |
...
Это устаревшая роль. Пожалуйста, используйте вместо этого админ безопасности.
...
Можно просматривать рекомендации и оповещения, просматривать политики безопасности, просматривать состояния безопасности, но не может вносить изменения.
...
> | Microsoft.DevTestLab/labs/ensureCurrentUserProfile/action | Убедитесь, что текущий пользователь имеет действительный профиль в лаборатории. |
...
> | Microsoft.Network/networkInterfaces/join/action | Присоединяется к виртуальной машине с сетевым интерфейсом. Не оповещенный. |
> | Microsoft.Network/networkInterfaces/read | Возвращает определение сетевого интерфейса.  |
> | Microsoft.Network/networkInterfaces/write | Создает новый сетевой интерфейс или обновляет существующий.  |
...
> | Microsoft.LabServices/labAccounts/getPricingAndAvailability/action | Получите цены и доступность комбинаций размеров, географических регионов и операционных систем для лабораторного счета. |
> | Microsoft.LabServices/labaccounts/getRestrictionsandUsage/action | Получить основные ограничения и использование для этой подписки |
> | Microsoft.Resources/subscriptions/resourceGroups/read | Возвращает группы ресурсов или выводит их список. |
> | Microsoft.Support/* | Создание запросов в службу поддержки и управление ими |
> | **Недействия** |  |
> | *отсутствуют* |  |
...
> | **Действия** |  |
> | Microsoft.Authorization/*/read | Чтение ролей и назначений ролей |
> | Microsoft.Insights/alertRules/* | Создание и управление классическими правилами оповещения |
> | Microsoft.Insights/metricAlerts/ | Создание и управление новыми правилами оповещения |
> | Microsoft.Insights/components/* | Создание компонентов Insights и управление ими |
> | Microsoft.Insights/webtests/* | Создание и управление веб-тестами Insights |
> | Microsoft.ResourceHealth/availabilityStatuses/read | Возвращает состояния доступности для всех ресурсов в указанной области. |
> | Microsoft.Resources/deployments/* | Создание развертываний группы ресурсов и управление ими |
> | Microsoft.Resources/subscriptions/resourceGroups/read | Возвращает группы ресурсов или выводит их список. |
> | Microsoft.Support/* | Создание запросов в службу поддержки и управление ими |
...
> | Microsoft.Insights/webtests/* | Создание и управление веб-тестами Insights |
> | Microsoft.Insights/рабочие книги/ |  |
> | Microsoft.OperationalInsights/workspaces/intelligencepacks/* | Прочитайте/напишите/удалите пакеты решений для аналитики журналов. |
> | Microsoft.OperationalInsights/workspaces/savedSearches/* | Аналитика чтения/записи/удаления сохранена поисков. |
> | Microsoft.OperationalInsights/workspaces/search/action | Выполняет поисковый запрос. |
...
> | Microsoft.OperationalInsights/workspaces/storageinsightconfigs/* | Прочитайте/напишите/удалите конфигурации анализа хранения данных журнала. |
> | Microsoft.Support/* | Создание запросов в службу поддержки и управление ими |
> | Microsoft.WorkloadMonitor/monitors/* |  |
> | Microsoft.WorkloadMonitor/notificationSettings/* |  |
> | Microsoft.AlertsManagement/smartDetectorAlertRules/ |  |
> | Microsoft.AlertsУправление/действияПравила/з. |  |
...
### <a name="workbook-contributor"></a>Автор трудовых книжки

Можно сохранить общие трудовые книжки.

> [!div class="mx-tableFixed"]
> |  |  |
> | --- | --- |
> | **Действия** |  |
> | Microsoft.Insights/workbooks/write | Создание или обновление трудовой книжки |
> | Microsoft.Insights/рабочие книги/удалить | Удаление книги |
> | Microsoft.Insights/workbooks/read | Читать трудовую книжку |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
...
### <a name="workbook-reader"></a>Читатель трудовых книжки

Можно читать трудовые книжки.
...
## <a name="management--governance"></a>Управление и управление
...
### <a name="azure-connected-machine-onboarding"></a>Лазурная подключенная машина на борту

Может на борту Azure Connected Machines.

> [!div class="mx-tableFixed"]
> |  |  |
> | --- | --- |
> | **Действия** |  |
> | Microsoft.HybridCompute/машины/читай | Ознакомьте любые машины Azure Arc |
> | Microsoft.HybridCompute/машины/запись | Пишет машины Azure Arc |
...
### <a name="azure-connected-machine-resource-administrator"></a>Администратор ресурсов подключенных машин Azure

Можно читать, писать, удалять и повторно на борту Azure Connected Machines.
...
> | Microsoft.HybridCompute/машины/читай | Ознакомьте любые машины Azure Arc |
> | Microsoft.HybridCompute/машины/запись | Пишет машины Azure Arc |
> | Microsoft.HybridCompute/машины/удаление | Удаляет машины Azure Arc |
> | Microsoft.HybridCompute/машины/воссоединение/действие | Повторное подключение машин Azure Arc |
...
> | **Действия** |  |
> | Microsoft.Authorization/*/read | Чтение ролей и назначений ролей |
> | Microsoft.Blueprint/blueprints/ | Создавайте и управляйте определениями чертежей или артефактами чертежей. |
> | Microsoft.Resources/subscriptions/resourceGroups/read | Возвращает группы ресурсов или выводит их список. |
> | Microsoft.Resources/deployments/* | Создание развертываний группы ресурсов и управление ими |
> | Microsoft.Support/* | Создание запросов в службу поддержки и управление ими |
> | **Недействия** |  |
...
Может назначать существующие опубликованные чертежи, но не может создавать новые чертежи. Обратите внимание, что это работает только в том случае, если назначение выполнено с помощью управляемого ипотечно-удостоверения, назначенного пользователем.
...
> | **Действия** |  |
> | Microsoft.Authorization/*/read | Чтение ролей и назначений ролей |
> | Microsoft.Blueprint/BlueprintНазначенияs/) | Создавайте и управляйте назначениями чертежей. |
> | Microsoft.Resources/subscriptions/resourceGroups/read | Возвращает группы ресурсов или выводит их список. |
> | Microsoft.Resources/deployments/* | Создание развертываний группы ресурсов и управление ими |
> | Microsoft.Support/* | Создание запросов в службу поддержки и управление ими |
> | **Недействия** |  |
...
### <a name="managed-application-contributor-role"></a>Роль автора управляемых приложений

Позволяет создавать управляемые ресурсы приложения.
...
> | **Действия** |  |
> | */чтение | Чтение ресурсов всех типов, кроме секретов. |
> | Microsoft.Решения/приложения/я |  |
> | Microsoft.Solutions/register/action | Регистрация в решениях. |
> | Microsoft.Ресурсы/подписки/ресурсгруппы/з |  |
> | Microsoft.Resources/deployments/* | Создание развертываний группы ресурсов и управление ими |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
...
### <a name="managed-services-registration-assignment-delete-role"></a>Назначение зависотки управляемых служб Удалить роль

Роль удаления управленных служб Registration Assignment Delete Role позволяет управляющим пользователям-арендаторам удалить назначение регистрации, назначенное их арендатору.

> [!div class="mx-tableFixed"]
> |  |  |
> | --- | --- |
> | **Действия** |  |
> | Microsoft.ManagedServices/registrationНазначения/читай | Извлекает список задания регистрации управляемых служб. |
> | Microsoft.ManagedServices/registrationНазначения/удаление | Удаляет назначение регистрации управляемых служб. |
...
### <a name="policy-insights-data-writer-preview"></a>Автор данных по вопросам политики (Предварительный просмотр)

Позволяет читать политики ресурсов и писать доступ к событиям политики компонентов ресурса.
...
> | *отсутствуют* |  |
> | **Действия с данными** |  |
> | Microsoft.PolicyInsights/checkDataPolicyCompliance/action | Проверьте состояние соответствия данному компоненту в отношении политик и данных. |
> | Microsoft.PolicyInsights/policyEvents/logDataEvents/action | Зарегистрируй события политики компонента ресурса. |
> | **NotDataActions** |  |
> | *отсутствуют* |  |
...
Пользователи с правами на создание/изменение политики ресурсов, создание билета поддержки и чтение ресурсов/иерархии.
...
> | Microsoft.Resources/subscriptions/resourceGroups/read | Возвращает группы ресурсов или выводит их список. |
> | Microsoft.Storage/storageAccounts/read | Возвращает список учетных записей хранения или свойства указанной учетной записи хранения. |
> | Microsoft.RecoveryServices/vaults/replicationOperationStatus/read | Ознакомьте состояние операции репликации Убежища |
> | Microsoft.Support/* | Создание запросов в службу поддержки и управление ими |
> | **Недействия** |  |
> | *отсутствуют* |  |
> | **Действия с данными** |  |
...
### <a name="tag-contributor"></a>Тег Вкладчик

Позволяет управлять тегами на сущностях, не предоставляя доступа к самим объектам.
