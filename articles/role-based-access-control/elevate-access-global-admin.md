...
---
# <a name="elevate-access-to-manage-all-azure-subscriptions-and-management-groups"></a>Повысить доступ к управлению всеми подписными группами и управленческими группами Azure
...
## <a name="how-does-elevated-access-work"></a>Как работает повышенный доступ?
...
1. Найдите и выберите **Azure Active Directory**.
   ...
   ![Выберите активный каталог Azure - скриншот](./media/elevate-access-global-admin/search-for-azure-active-directory.png)

1. Под **управлением**, выберите **Свойства**.

   ![Выберите свойства для свойств Активного каталога Azure - скриншот](./media/elevate-access-global-admin/azure-active-directory-properties.png)
...
1. В разделе **Управление доступом для ресурсов Azure** установите значение **Да**.
   ...
   При установке переключателя на **Yes,** вам присваивается роль администратора пользовательского доступа в Azure RBAC в корневой области (/). Это предоставляет разрешение на назначение ролей во всех подписках и группах управления Azure, связанных с данным каталогом Azure AD. Этот переключатель доступен только пользователям, которым назначена роль глобального администратора в Azure AD.
   ...
   Если для переключателя задать значение **Нет**, роль администратора доступа пользователей в Azure RBAC будет удалена из вашей учетной записи. Вы больше не сможете назначать роли во всех подписках и группах управления Azure, связанных с данным каталогом Azure AD. Вы сможете просматривать (и управлять ими) только подписки и группы управления Azure, к которым вам был предоставлен доступ.
    ...
    > [!NOTE]
    > Если вы используете [Azure AD Privileged Identity Management (PIM),](../active-directory/privileged-identity-management/pim-configure.md)деактивация назначения ролей не изменяет этот переключатель на **No.** Для поддержания наименее привилегированных доступа мы рекомендуем настроить этот переключатель в **No,** прежде чем деактивировать назначение ролей.
...
1. Выйдите из системы и войдите снова, чтобы обновить доступ.
    ...
    Теперь у вас есть доступ ко всем подпискам и группам управления в своем каталоге. При просмотре панели управления доступа (IAM) вы заметите, что вам была назначена роль администратора пользовательского доступа в корневой области.
...
Чтобы удалить назначение ролей администратора пользовательского доступа в корневой области ( ),`/`выполните следующие действия.

1. Войти в систему как тот же пользователь, который использовался для повышения доступа.

1. В списке навигации выберите **Azure Active Directory**, а затем нажмите кнопку **Свойства**.

1. Установите **управление доступом для ресурсов Azure,** переключайтесь обратно в **No.** Так как это параметр для отдельного пользователя, необходимо войти как тот же пользователь, который использовался для повышения доступа.

    Если вы попытаетесь удалить назначение ролей администратора пользовательского доступа на панели управления доступом (IAM), вы увидите следующее сообщение. Чтобы удалить назначение ролей, необходимо установить переключение обратно в **No** или использовать Azure PowerShell, Azure CLI или API REST.

    ![Удалить назначения ролей с корневой областью](./media/elevate-access-global-admin/iam-root-remove.png)

## <a name="azure-powershell"></a>Azure PowerShell

[!INCLUDE [az-powershell-update](../../includes/updated-for-az.md)]

### <a name="list-role-assignment-at-root-scope-"></a>Список назначения роли в корневой области (/)

Чтобы перечислить назначение роли администратора пользовательского`/`доступа для пользователя в корневой области (), используйте команду [Get-AzRoleAssignment.](/powershell/module/az.resources/get-azroleassignment)
...
Чтобы удалить назначение ролей администратора пользовательского доступа для`/`себя или другого пользователя в корневой области (), выполните следующие действия.

1. Войдите в систему как пользователь, который может удалить повышенный уровень доступа. Это может быть тот же пользователь, который использовался для повышения доступа, или другой Глобальный Администратор с повышенным доступом в корневой области.
...
Чтобы перечислить назначение роли администратора пользовательского`/`доступа для пользователя в корневой области ( ), используйте команду [списка назначения роли АЗ.](/cli/azure/role/assignment#az-role-assignment-list)
...
1. Используйте команду [удаления назначения роли АЗ,](/cli/azure/role/assignment#az-role-assignment-delete) чтобы удалить назначение роли администратора пользовательского доступа.
...
1. Использование REST, `elevateAccess`вызова, который предоставляет вам роль администратора пользовательского доступа в корневой области ().`/`

   ```http
   POST https://management.azure.com/providers/Microsoft.Authorization/elevateAccess?api-version=2016-07-01
   ```

1. Создание [назначение роли](/rest/api/authorization/roleassignments), чтобы назначить любую роль в любой области. В следующем примере показаны свойства для присвоения роли`/`«roleDefinitionID» в корневой области ( ):
...
1. В то время как администратор пользовательского доступа, вы`/`также можете удалить назначения ролей в корневой области ().

1. Удалите привилегии администратора доступа пользователей, пока они не понадобятся вновь.

### <a name="list-role-assignments-at-root-scope-"></a>Список назначений ролей в корневой области (/)
...
### <a name="list-deny-assignments-at-root-scope-"></a>Список отрицать назначения в корневой области (/)

Вы можете перечислить все задания отказа для`/`пользователя в корневой области ().
...
При вызове `elevateAccess`вы создаете назначение ролей для себя, поэтому для отмены этих привилегий необходимо удалить`/`назначение роли администратора пользовательского доступа для себя в корневой области ()
...
1. Предыдущие вызовы возвращают список назначения ролей. Найдите назначение роли, у которого задана область `"/"` и `roleDefinitionId` заканчивается значением ИД имени роли, найденным на шаге 1, а `principalId` совпадает со значением objectId администратора каталога. 
    ...
    Опять же, сохранить `name` Идентификатор от параметра, в данном случае 11111111-1111-1111-1111-11111111111111111111.
