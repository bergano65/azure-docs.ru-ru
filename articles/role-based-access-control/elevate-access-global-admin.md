...
Если вы являетесь глобальным администратором, могут быть случаи, когда вы хотите сделать следующие действия:
...
1. Установите **управление доступом для ресурсов Azure,** переключайтесь обратно в **No.** Так как это параметр для отдельного пользователя, необходимо войти как тот же пользователь, который использовался для повышения доступа.
    ...
    Если вы попытаетесь удалить назначение ролей администратора пользовательского доступа на панели управления доступом (IAM), вы увидите следующее сообщение. Чтобы удалить назначение ролей, необходимо настроить переключение на **No** или использовать Azure PowerShell, Azure CLI или API REST.
...
При вызове `elevateAccess`вы создаете для себя назначение ролей, поэтому для отмены этих привилегий необходимо удалить`/`назначение роли администратора пользовательского доступа для себя в корневой области ().
