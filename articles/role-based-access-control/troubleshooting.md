...
- Если не удается добавить назначение роли в портал Azure **управления доступом (IAM)** , так как параметр **Добавить** > **добавить назначение ролей** отключен или получено сообщение об ошибке "у клиента с идентификатором объекта нет прав на выполнение действия", убедитесь, что вы вошли в систему с помощью пользователя, которому назначена роль с разрешениями `Microsoft.Authorization/roleAssignments/write`, такими как [владелец](built-in-roles.md#owner) или [администратор доступа пользователей](built-in-roles.md#user-access-administrator) в области, которой вы пытаетесь назначить роль.
- Если вы получаете сообщение об ошибке "больше не удается создать назначения ролей (код: Ролеассигнментлимитексцеедед)" при попытке назначить роль, попробуйте уменьшить число назначений ролей, назначив вместо них роли группам. Azure поддерживает до **2000** назначений ролей на подписку. Это ограничение назначений ролей является фиксированным и не может быть увеличено.
...
- Если появляется сообщение об ошибке, похожее на "клиент имеет разрешение на выполнение действия" Microsoft. Authorization/roleDefinitions/Write "в области"/субскриптионс/{субскриптионид} ", но связанная подписка не найдена при попытке обновить пользовательскую роль, проверьте, не была ли удалена одна или несколько [назначаемых областей](role-definitions.md#assignablescopes) в клиенте. Если область удалена, создайте запрос в службу поддержки, так как способа самостоятельного решения этой проблемы пока не существует.
...
- Инструкции по переносу подписки на другой клиент Azure AD см. [в статье перемещение прав владения подпиской Azure в другую учетную запись](../cost-management-billing/manage/billing-subscription-transfer.md).
...
- Если у вас возникли проблемы с администратором служб или соадминистратором, см. статью [Добавление или изменение администраторов Подписки Azure](../cost-management-billing/manage/add-change-subscription-administrator.md) и [ролей администратора классической подписки, ролей RBAC Azure и РОЛЕЙ администратора Azure AD](rbac-and-directory-admin-roles.md).
...
Если назначить роль участнику безопасности (пользователь, группа, субъект-служба или управляемое удостоверение), а затем удалить этот субъект безопасности без удаления назначения роли, то тип субъекта безопасности для назначения роли будет отображаться как **неизвестный**. На следующем снимке экрана приведен пример на портале Azure. Имя субъекта безопасности указано в списке **удалено удостоверение** и **удостоверение больше не существует**. 

![Группа ресурсов веб-приложений](./media/troubleshooting/unknown-security-principal.png)

Если вы перечислите назначение ролей с помощью Azure PowerShell, вы увидите пустой `DisplayName`, а `ObjectType` задано значение Unknown. Например, командлет [Get-азролеассигнмент](/powershell/module/az.resources/get-azroleassignment) Возвращает назначение роли, похожее на следующее:
...
Аналогично, если вы перечислите назначение ролей с помощью Azure CLI, вы увидите пустой `principalName`. Например, [AZ Role назначений List](/cli/azure/role/assignment#az-role-assignment-list) Возвращает назначение роли, похожее на следующее:
...
Не существует проблем с назначениями ролей, но их можно удалить с помощью действий, аналогичных другим назначениям ролей. Сведения об удалении назначений ролей см. в разделе [портал Azure](role-assignments-portal.md#remove-a-role-assignment), [Azure PowerShell](role-assignments-powershell.md#remove-a-role-assignment)или [Azure CLI](role-assignments-cli.md#remove-a-role-assignment)
