...
- Если вы получаете сообщение об ошибке «можно создать дополнительные назначения ролей (код: RoleAssignmentLimitExceeded)» при попытке назначить роль, попробуйте уменьшить число назначений ролей путем назначения ролей группам, вместо этого. Azure поддерживает до **2000** назначений ролей на подписку.
...
- Если вы не сможете удалить настраиваемую роль и выводится сообщение об ошибке «существуют назначения ролей, которые ссылаются на роль (код: RoleDefinitionHasAssignments)», то существуют назначения ролей, по-прежнему с помощью настраиваемой роли. Удалите эти назначения ролей и попытайтесь снова удалить пользовательскую роль.
- Если вы получаете сообщение об ошибке «роли Превышено ограничение на определение. Можно создать дополнительные определения ролей (код: RoleDefinitionLimitExceeded)» при попытке создать новую настраиваемую роль, удалить все пользовательские роли, которые не используются. Azure поддерживает до **2000** пользовательских ролей в клиенте.
- Если отобразится сообщение об ошибке аналогичную «у клиента есть разрешение на выполнение действия «Microsoft.Authorization/roleDefinitions/write» в области «/ subscriptions / {subscriptionid}», однако связанная подписка не найден» при попытке обновить пользовательскую роль, проверка является ли один или несколько [назначаемых областей](role-definitions.md#assignablescopes) были удалены в клиенте. Если область была удалена, затем создайте запрос в службу поддержки, как самостоятельного решения нет доступных в данный момент.
...
- Если передать подписку в другой Azure AD клиентом, все назначения ролей, удаляются без возможности восстановления из исходного Azure AD клиента и не переносятся в целевом клиенте Azure AD. Необходимо повторно создать назначения ролей в целевом клиенте.
...
- Если возникает ошибка разрешения «клиент с идентификатором объекта, не авторизован для выполнения действия с областью (код: AuthorizationFailed)» при попытке создать ресурс, проверьте, что вы сейчас вошли в систему пользователя, который назначается роль с разрешением на запись к ресурсу в выбранной области. Например, для управления виртуальными машинами в группе ресурсов, должны иметь [участник виртуальных машин](built-in-roles.md#virtual-machine-contributor) роли для группы ресурсов (или родительской области). Список разрешений для каждой встроенной роли, см. в разделе [встроенных ролей ресурсов Azure](built-in-roles.md).
