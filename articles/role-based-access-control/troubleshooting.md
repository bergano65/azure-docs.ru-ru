...
Если вы перечислите назначение этой роли с помощью Azure PowerShell, то может `DisplayName` отобразиться пустое `ObjectType` значение, а для параметра задано **неизвестное**. Например, командлет [Get-азролеассигнмент](/powershell/module/az.resources/get-azroleassignment) Возвращает назначение роли, похожее на следующие выходные данные:
...
Аналогично, если вы перечислите назначение ролей с помощью Azure CLI, может отобразиться `principalName`пустое значение. Например, [AZ Role назначений List](/cli/azure/role/assignment#az-role-assignment-list) Возвращает назначение роли, похожее на следующие выходные данные:
