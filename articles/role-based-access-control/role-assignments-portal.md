...
description: Узнайте, как управлять доступом пользователей, групп, субъектов-служб и управляемых удостоверений с помощью управления доступом на основе ролей (RBAC) и портала Azure. Сюда также входят сведения о том, как перечислять, предоставлять и удалять права доступа.
...
ms.locfileid: "52634869"
---
# <a name="manage-access-using-rbac-and-the-azure-portal"></a>Управление доступом с помощью RBAC и портала Azure

[Управление доступом на основе ролей (RBAC)](overview.md) — это способ управления доступом к ресурсам в Azure. В этой статье описывается, как управлять доступом пользователей, групп, субъектов-служб и управляемых удостоверений с помощью RBAC и портала Azure.
...
Определение роли — это коллекция разрешений, используемых для назначения ролей. В Azure насчитывается более 70 [встроенных ролей](built-in-roles.md). Выполните следующие действия, чтобы просмотреть роли и разрешения, которые можно выполнять в плоскостях управления и данных.
...
1. Щелкните **Добавить назначение роли**, чтобы открыть панель для добавления назначения роли.
   ...
   ![Область "Добавить назначение роли"](./media/role-assignments-portal/add-role-assignment.png)
