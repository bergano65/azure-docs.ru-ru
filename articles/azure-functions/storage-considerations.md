...
Вы можете подключить существующие общие папки файлов Azure к приложениям функции Linux. Подключив общую папку к приложению-функции Linux, вы можете использовать существующие модели машинного обучения или другие данные в своих функциях. Вы можете использовать [`az webapp config storage-account add`](/cli/azure/webapp/config/storage-account#az-webapp-config-storage-account-add) команду, чтобы подключить существующую общую папку к приложению функции Linux. 

В этой команде `share-name` — это имя существующей общей папки службы файлов Azure и `custom-id` может быть любой строкой, которая уникально определяет общий ресурс при подключении к приложению-функции. Кроме того `mount-path` , — это путь, из которого к общей папке осуществляется доступ в приложении-функции. `mount-path`значение должно быть в формате `/dir-name`и не может начинаться с `/home`.
...
В настоящее время поддерживается `storage-type` только `AzureFiles` в. Вы можете подключить только пять общих папок к конкретному приложению-функции. Подключение общей папки может увеличить время холодного запуска по крайней мере на 200-300 мс или даже больше, если учетная запись хранения находится в другом регионе.

Подключенный общий ресурс доступен для кода функции в `mount-path` указанном. Например, если `mount-path` имеет значение `/path/to/mount`, доступ к целевому каталогу можно получить с помощью API-интерфейсов файловой системы, как показано в следующем примере Python:
