...
Для отключения функции мы рекомендуем использовать параметр приложения в формате `AzureWebJobs.<FUNCTION_NAME>.Disabled`. Вы можете создавать и изменять такой параметр приложения разными способами, в том числе через [Azure CLI](/cli/azure/) или на вкладке **Управление** на [портале Azure](https://portal.azure.com). 
...
Также можно использовать кнопки **Включить** и **Отключить** на странице **Обзор** для функции. Эти кнопки по сути позволяют создать и удалить параметр приложения `AzureWebJobs.<FUNCTION_NAME>.Disabled`.
