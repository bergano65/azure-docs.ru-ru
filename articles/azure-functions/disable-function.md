...
ms.locfileid: "83115577"
---
# <a name="how-to-disable-functions-in-azure-functions"></a>Способы отключения функций в решении "Функции Azure"

В этой статье объясняется, как отключить функцию в решении "Функции Azure". *Отключение* функции означает, что среда выполнения будет игнорировать автоматически запускаемый триггер, определенный для функции. Это позволяет предотвратить выполнение определенной функции без остановки всего приложения-функции.
...
> [!NOTE]  
> При отключении функции, активируемой HTTP, с помощью методов, описанных в этой статье, конечная точка по-прежнему может быть доступна при запуске на локальном компьютере.  

## <a name="use-the-azure-cli"></a>Использование Azure CLI

В Azure CLI используется [`az functionapp config appsettings set`](/cli/azure/functionapp/config/appsettings#az-functionapp-config-appsettings-set) команда для создания и изменения параметра приложения. Следующая команда отключает функцию с именем `QueueTrigger` , создавая параметр приложения с именем и присвоив `AzureWebJobs.QueueTrigger.Disabled` ему значение `true` . 
...
Можно также использовать кнопки **включить** и **Отключить** на странице **Обзор** функции. Эти кнопки работают, создавая и удаляя `AzureWebJobs.<FUNCTION_NAME>.Disabled` параметр приложения.

![Переключатель состояния функции](media/disable-function/function-state-switch.png)

> [!NOTE]  
> Функция тестирования, интегрированная с порталом, игнорирует `Disabled` параметр. Это означает, что отключенная функция по-прежнему запускается при запуске из окна **тестов** на портале. 
...
В функции библиотеки классов можно также использовать `Disable` атрибут, чтобы предотвратить срабатывание функции. Можно использовать атрибут без параметра конструктора, как показано в следующем примере:
...
В версии 1. x можно также использовать `disabled` свойство файла *Function. JSON* , чтобы указать среде выполнения не запускать функцию. Этот метод работает только для языков сценариев, таких как скрипты C# и JavaScript. `disabled`Свойству может быть присвоено значение `true` или имя параметра приложения:
...
Файл можно изменить в портал Azure или воспользоваться кнопками **включить** и **Отключить** на странице **обзора** функции. Переключатель на портале работает путем изменения файла *function.json*.
