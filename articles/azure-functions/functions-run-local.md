...
+ На [Портал Azure]. Перейдите к учетной записи хранения, выберите **Ключи доступа** в разделе **Параметры**, а затем скопируйте одно из значений **Строка подключения**.
...
| **`--build [-b]`** | Выполняет действие сборки при развертывании в приложении-функции Linux. (принимает: удаленный, локальный) |
