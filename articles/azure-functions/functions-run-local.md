...
> [!IMPORTANT]
> При включении расширения пакетов в файле host.json проекта, необходимо установить .NET Core 2.x SDK. Дополнительные сведения см. в разделе [локальной разработки с помощью основных инструментов функций Azure и пакетов расширения ](functions-bindings-register.md#local-development-with-azure-functions-core-tools-and-extension-bundles). Пакеты расширения требуется версия 2.6.1071 основные средства, или более поздней версии.
...
| Параметр      | ОПИСАНИЕ                            |
| ------------ | -------------------------------------- |
| **`IsEncrypted`** | Если задано значение `true`, все значения шифруются с помощью ключа локального компьютера. Используется с командами `func settings`. По умолчанию имеет значение `true`. Когда `true`, все параметры, которые добавляются с помощью `func settings add` шифруются с помощью ключа локального компьютера. Это отражает, как параметры приложения-функции хранятся в параметрах приложения в Azure. Шифрование значения локальных параметров предоставляет обеспечения дополнительной защиты для ценных данных следует local.settings.json быть открытым.  |
...
| **`--enableAuth`** | Включите полную проверку подлинности, конвейер обработки.|
