...
Для записи выходных данных в [потоковые журналы](../app-service/web-sites-enable-diagnostic-log.md) в F# в функции следует использовать аргумент типа [ILogger](https://docs.microsoft.com/dotnet/api/microsoft.extensions.logging.ilogger). Для согласованности мы советуем назвать этот аргумент `log`. Например: 
