...
- Измените тип параметра журнала из `TraceWriter` для `ILogger`
- Изменение `log.Info` для `log.LogInformation`
...
Следующая функция Java использует `@ServiceBusQueueTrigger` заметки [Java функции библиотеки времени выполнения](/java/api/overview/azure/functions/runtime) для описания конфигурации для триггера очереди служебной шины. Функция извлекает сообщения, помещенные в очередь и добавляет его в журналы.
...
Функции Java также может запускаться, когда сообщение добавляется в раздел служебной шины. В следующем примере используется `@ServiceBusTopicTrigger` заметку для описания конфигурации триггера.
...
Java функции также могут записывать в раздел служебной шины. В следующем примере используется `@ServiceBusTopicOutput` заметку для описания конфигурации для привязки выходных данных. 
...
* `out T paramName` - `T` может быть любым JSON-сериализуемым типом. Если при выходе из функции параметр имеет значение NULL, то служба "Функции" создает сообщение с пустым объектом.
* `out string` – Если значение параметра равно null, при выходе из функции, функции не создает сообщение.
* `out byte[]` – Если значение параметра равно null, при выходе из функции, функции не создает сообщение.
* `out BrokeredMessage` – Если значение параметра равно null, при выходе из функции, функции не создает сообщение.
* `ICollector<T>` или `IAsyncCollector<T>` — создать несколько сообщений. Сообщение создается при вызове метода `Add` .
...
|---|---|
| Служебная шина Azure | [Коды ошибок служебной шины](https://docs.microsoft.com/azure/service-bus-messaging/service-bus-messaging-exceptions) |
| Служебная шина Azure | [Ограничения служебной шины](https://docs.microsoft.com/azure/service-bus-messaging/service-bus-quotas) |
