...
5. Используйте NuGet, чтобы добавить ссылку из тестового приложения на [Microsoft.AspNetCore.Mvc](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc/)
...
Класс `ListLogger` реализует `ILogger` интерфейс и содержит внутренний список сообщений для оценки во время теста.

**Нажмите на** приложение *Functions.Test* и выберите **Добавить > класса,** назовите его **NullScope.cs** и введите следующий код:
...
Далее, **правое нажатие** на *приложение Functions.Test* и выберите **Добавить > класса**, назовите его **ListLogger.cs** и введите следующий код:
...
- **BeginScope**: Области добавляют контекст в журнал. В этом случае тест просто указывает на `NullScope` статический экземпляр в классе, чтобы позволить тесту функционировать.

- **IsEnabled**: Значение `false` по умолчанию предоставляется.

- **Войти**: Этот метод `formatter` использует предоставленную функцию для формата `Logs` сообщения, а затем добавляет полученный текст в коллекцию.
...
- **Данные**: Это свойство возвращает [IEnumerable](https://docs.microsoft.com/dotnet/api/system.collections.ienumerable) сбор выборочных данных. Пары "ключ-значение" представляют собой значения, которые передаются в строку запроса.

- **CreateDictionary**: Этот метод принимает пару ключей/значений `Dictionary` в качестве `QueryCollection` аргументов и возвращает новое, используемое для создания для представления значений строки запроса.

- **CreateHttpRequest**: Этот метод создает запрос HTTP, инициализированный с параметрами строки заданного запроса.

- **CreateLogger**: На основе типа регистратора этот метод возвращает класс регистратора, используемый для тестирования. `ListLogger` следит за сообщениями в журнале, доступными для оценки в тестах.
...
- **Http_trigger_should_return_known_string**: Этот тест создает запрос со значениями строки запроса `name=Bill` для функции HTTP и проверяет, что ожидаемый ответ возвращается.

- **Http_trigger_should_return_string_from_member_data**: Этот тест использует атрибуты xUnit для предоставления выборочных данных функции HTTP.

- **Timer_should_log_message**: Этот тест `ListLogger` создает экземпляр и передает его функциям таймер. После выполнения функции журнал будет проверен на наличие ожидаемого сообщения.

Если вы хотите получить доступ к настройкам приложений в тестах, вы можете использовать [System.Environment.GetEnvironmentVariable](./functions-dotnet-class-library.md#environment-variables).
...
Этот модуль реализует свойство `IsPastDue`, чтобы установить его как ложный экземпляр таймера. Конфигурации таймер, как ncRONTAB выражения не требуется здесь, как тест использовать просто вызова функции непосредственно для проверки результата.
