...
1.  Используйте следующую [`az webapp config appsettings set`](/cli/azure/webapp/config/appsettings#az-webapp-config-appsettings-set) команду, чтобы создать параметр `WEBSITE_RUN_FROM_PACKAGE` приложения со значением в качестве Key Vault ссылки на внешний URL-адрес:
...
- Запуск приложения из пакета развертывания делает `wwwroot/` его доступным только для чтения. Приложение получает сообщение об ошибке при попытке записи в этот каталог.
