...
В следующей команде вместо заполнителя `<zip_file_path>` укажите путь к расположению ZIP-файла. Кроме того, `<app_name>` замените на уникальное имя приложения функции и замените `<resource_group>` на имя группы ресурсов.
