...
1. В Eclipse выберите меню **файла,** затем выберите **&gt; Новый - Maven Project.** 
...
1. Выберите **Add Archetype** (Добавить архетип) и добавьте записи для [azure-functions-archetype](https://mvnrepository.com/artifact/com.microsoft.azure/azure-functions-archetype).
    ...
    ![Eclipse Maven создать](media/functions-create-first-java-eclipse/functions-create-eclipse.png)  
1. Нажмите кнопку **ОК**, а затем — **Далее**.  Не забудьте заполнить значения для всех `resourceGroup`полей, включая , `appName`и `appRegion` (пожалуйста, используйте различные appName, кроме **fabrikam-функция-20170920120101928**), и в конечном итоге **закончить**.
    ![Затмение Maven создать2](media/functions-create-first-java-eclipse/functions-create-eclipse2.png)  
...
В конфигурации **Run As,** настроенной `azure-functions:run` на `azure-functions:run -DenableDebug` предыдущем этапе, измените и запустите обновленную конфигурацию, чтобы запустить приложение функции в режиме отладки.
