...
1. На вкладке **Основные сведения** используйте параметры виртуальной машины, как указано в таблице под изображением.
    ...
    | **Authentication type (Тип проверки подлинности)** | Пароль | Чтобы использовать проверку подлинности с помощью пароля, необходимо также указать **имя пользователя**, защищенный **пароль**, а затем **подтвердить пароль**. В этом руководстве вам не потребуется входить в виртуальную машину, если не требуется устранение неполадок. |
...
1. В окне **Создание виртуальной сети**используйте параметры, приведенные в таблице под изображением.
    ...
    | **Диапазон адресов** (подсеть) | 10.10.1.0/24   | Размер подсети определяет, сколько интерфейсов можно добавить в подсеть. Эта подсеть используется сайтом WordPress.  `/24` Подсеть предоставляет 254 адресов узлов. |
...
1. В новом приложении функции выберите **функции** > платформы**сеть**.
...
1. В поле **состояние компонента сети**используйте параметры, приведенные в таблице под изображением.
    ...
    | **Диапазон адресов** | 10.10.2.0/24   | Размер подсети позволяет ограничивать общее число экземпляров, на которые может масштабироваться приложение функции плана Premium. В `/24` этом примере используется подсеть с 254 доступных адресов узлов. Эта подсеть не подготовлена, но ее легко вычислить. |
...
1. В приложении-функции выберите **прокси** > **+**, а затем используйте параметры прокси-сервера в таблице под изображением.
    ...
    | **Внутренний URL-адрес** | http://<YOUR_VM_IP>/ВП-контент/семес/твентисевентин/Ассетс/имажес/хеадер.ЖПГ | Замените `<YOUR_VM_IP>` IP-адресом созданной ранее виртуальной машины WordPress. Это сопоставление возвращает один файл с сайта. |
