...
| ------------- | --------- | ------------------------------------------------------------ |
| `Outputs`     | объект    | Содержит значения ответа, определенные `bindings` массивом в *function.json*.<br /><br />Например, если для функции настроена выходная привязка очереди с именем "Микуеуеаутпут", то `Outputs` содержит ключ с именем `myQueueOutput` , который задается настраиваемым обработчиком для сообщений, отправляемых в очередь. |
