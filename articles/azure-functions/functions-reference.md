...
Для получения дополнительной [Azure Functions triggers and bindings concepts](functions-triggers-bindings.md)информации см.
...
Приложение-функция предоставляет контекст выполнения в Azure, в котором выполняются функции. Таким образом, это подразделение развертывания и управления для ваших функций. Приложение-функция состоит из одной или нескольких независимых функций, которые управляются, развертываются и масштабируются вместе. Все функции в функциональном приложении имеют один и тот же план ценообразования, метод развертывания и версию времени выполнения. Приложение-функцию можно рассматривать как способ упорядочения функций и совместного управления ими. Чтобы узнать больше, смотрите [Как управлять приложением функции](functions-how-to-use-azure-function-app-settings.md). 
...
Версию среды выполнения Функций можно настроить с помощью параметра приложения `FUNCTIONS_EXTENSION_VERSION`. Например, значение "No3" указывает на то, что ваше приложение функции будет использовать 3.x в качестве основной версии. При выпуске приложения-функции обновляются до версии с новым дополнительным номером. Дополнительные сведения, в том числе инструкции по просмотру точной версии приложения-функции см. в статье [Выбор целевых версий среды выполнения Функций Azure](set-runtime-version.md).
...
* [Ссылка разработчика Функций Azure Node.js](functions-reference-node.md)
