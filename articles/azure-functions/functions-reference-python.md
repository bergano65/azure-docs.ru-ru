...
Чтобы асинхронно запустить функцию, используйте инструкцию `async def`, которая запускает функцию непосредственно с помощью [асинЦио](https://docs.python.org/3/library/asyncio.html) :
...
Функция без ключевого слова `async` выполняется автоматически в пуле потоков асинЦио:
