...
|-------------|---------|---------------|----------------|
| Приходит новое сообщение очереди, которая выполняет функцию для записи в другую очередь. | очереди<sup>*</sup> | *Нет* | очереди<sup>*</sup> |
...
| Объект webhook, который использует Microsoft Graph, чтобы обновить лист Excel. | HTTP | *Нет* | Microsoft Graph |
...
- Для триггеров направление всегда равно `in`
- Использовать входные и выходные привязки `in` и `out`
