...
* **backend.request.querystring.\<имя_параметра\>** — параметр строки запроса, который можно задать для вызова внутреннего сервера. Замените *\<имя_параметра\>* именем параметра, который вы собираетесь задать. Обратите внимание, что если указана пустая строка, параметр по-прежнему включается в запрос серверной части.
