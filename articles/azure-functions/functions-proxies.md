---
title: Работа с прокси-функциями в Azure Functions
...
## <a name="create-a-proxy"></a><a name="create"></a>Создание прокси-сервера
...
По умолчанию ответ клиента инициализируется в качестве копии ответа внутреннего сервера. Вы можете изменить код состояния, описание, заголовки и текст ответа. Измененные значения могут ссылаться на [параметры приложения,] [параметры исходного запроса клиента]и параметры от [обратного ответа.]
...
>[!Note]  
>Если функция использует уровень авторизации *функции, администратора или системы*, вам нужно указать код и идентификатор клиента в соответствии с исходным URL-адресом функции. В этом случае ссылка `"backendurl": "https://localhost/api/httptriggerC#1?code=<keyvalue>&clientId=<keyname>"` будет выглядеть следующим образом: Мы рекомендуем хранить эти ключи в [настройках приложения] и ссылки на те, в ваших прокси. Это позволяет избежать хранения секретов в исходном коде. 
...
* **{request.headers.\<имя_заголовка\>}.** Заголовок, который можно считать из исходного запроса. Замените * \<HeaderName\> * с именем заголовка, который вы хотите прочитать. Если заголовок не включен в запрос, в качестве значения будет отображаться пустая строка.
* **{request.querystring.\<имя_параметра\>}.** Параметр строки запроса, который можно считать из исходного запроса. Замените * \<\> ParameterName* с именем параметра, который вы хотите прочитать. Если параметр не включен в запрос, в качестве значения будет отображаться пустая строка.
...
* **{backend.response.headers.\<имя_заголовка\>}.** Заголовок, который можно считать из ответа внутреннего сервера. Замените * \<HeaderName\> * с именем заголовка, который вы хотите прочитать. Если заголовок не включен в ответ, в качестве значения будет отображаться пустая строка.

### <a name="reference-application-settings"></a><a name="use-appsettings"></a>Ссылки на параметры приложения

Вы также можете ссылаться на [настройки приложения, определенные для приложения функции,](https://docs.microsoft.com/azure/azure-functions/functions-how-to-use-azure-function-app-settings) окружая имя настройки с знаками процента (%).

Например, обратный URL-адрес *https://%ORDER_PROCESSING_HOST%/api/orders* будет заменен "%ORDER_PROCESSING_HOST%" значением настройки ORDER_PROCESSING_HOST.
...
### <a name="application-settings"></a><a name="applicationSettings"></a>Настройки приложения
...
* **backend.request.querystring.\<имя_параметра\>** — параметр строки запроса, который можно задать для вызова внутреннего сервера. Замените * \<\> ParameterName* с именем параметра, который вы хотите установить. Обратите внимание, что если пустая строка предоставлена, параметр по-прежнему включен в запрос на задний конец.
* **backend.request.headers.\<имя_заголовка\>** — заголовок, который можно задать для вызова внутреннего сервера. Замените * \<HeaderName\> * с именем заголовка, который вы хотите установить. Если указана пустая строка, заголовок не включается в запрос внутреннего сервера.
...
* **response.headers.\<имя_заголовка\>.** Заголовок, который можно задать для ответа клиенту. Замените * \<HeaderName\> * с именем заголовка, который вы хотите установить. Если указана пустая строка, заголовок не включается в ответ.
