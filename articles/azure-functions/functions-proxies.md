...
* **backend.request.headers.\<имя_заголовка\>** — заголовок, который можно задать для вызова внутреннего сервера. Замените * \<хеадернаме\> * именем заголовка, который необходимо задать. Если указана пустая строка, заголовок не включается в запрос внутреннего сервера.
