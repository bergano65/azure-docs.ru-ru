...
| Название | Тип | Положение | Описание |
| ---- | ---- |  -------- | ----------- |
...
Функции PowerShell поддерживают управление модулями Azure службой. Изменяя Host. JSON и устанавливая для свойства Манажеддепенденци Enabled значение true, файл требований. PSD1 будет обработан. Последние модули Azure будут автоматически скачаны и станут доступными для функции.
...
Использование собственных модулей или модулей из [коллекция PowerShell](https://powershellgallery.com) немного отличается от того, как это можно сделать обычным образом.
...
Чтобы использовать пользовательские модули или модули PowerShell из коллекция PowerShell, можно разместить модули, от которых зависит `Modules` папка. Из этой папки модули автоматически становятся доступными для среды выполнения функций. Все функции в приложении функции могут использовать эти модули.

Чтобы воспользоваться этой функцией, создайте `Modules` папку в корне приложения-функции. Сохраните модули, которые вы хотите использовать в функциях в этом расположении.
...
Используйте `Save-Module` , чтобы сохранить все модули, используемые функциями, или скопируйте в `Modules` папку собственные пользовательские модули. В папке modules приложение-функция должно иметь следующую структуру папок:
...
По умолчанию PowerShell является _отдельным потоковым_ языком сценариев. Однако параллелизм можно добавить с помощью нескольких пространств выполнения PowerShell в одном процессе. Эта функция работает в среде выполнения PowerShell для функций Azure.

Существует ряд недостатков этого подхода.

#### <a name="concurrency-is-only-as-good-as-the-machine-its-running-on"></a>Параллелизм работает так же хорошо, как и компьютер, на котором он работает.

Если приложение-функция работает в [плане службы приложений](functions-scale.md#app-service-plan) , которое поддерживает только одно ядро, то параллелизм практически не будет работать. Это обусловлено тем, что для балансировки нагрузки нет дополнительных ядер. В этом случае производительность может варьироваться, когда одному ядру требуется переключение между пространствами выполнения.

[План потребления](functions-scale.md#consumption-plan) выполняется с использованием только одного ядра, поэтому нельзя использовать параллелизм. Если вы хотите воспользоваться всеми преимуществами параллелизма, следует развернуть функции в приложении-функции, работающем в выделенном плане службы приложений с достаточными ядрами.
