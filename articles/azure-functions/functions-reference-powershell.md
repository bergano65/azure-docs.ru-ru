...
| имя_метода | Имя функции, которая была активирована     | строка   |
| RandGuid   | Уникальный идентификатор guid для этого выполнения функции | строка   |
...
| Сведения | **`Write-Information`** <br/> **`Write-Host`** <br /> **`Write-Output`**      | Сведения | Записывает _сведения_ уровня ведения журнала. |
| Отладка | **`Write-Debug`** |
...
| **`ContentType`** | Короткий руки для задав тип содержимого для ответа. | строка                    |
