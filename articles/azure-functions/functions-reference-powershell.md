...
Каждый тип триггера имеет другой набор метаданных. `$TriggerMetadata` Например, для `QueueTrigger` содержит `InsertionTime`,, `Id` `DequeueCount`и, помимо прочего,. Дополнительные сведения о метаданных триггера очереди см. в [официальной документации по триггерам очереди](functions-bindings-storage-queue-trigger.md#message-metadata). Ознакомьтесь с документацией по [триггерам](functions-triggers-bindings.md) , с которыми вы работаете, чтобы узнать, что входит в метаданные триггера.
...
Привязки триггера и ввода считываются как параметры, передаваемые в функцию. Для `in` входных привязок `direction` задано значение в Function. JSON. `name` Свойство, определенное в `function.json` , является именем параметра в `param` блоке. Так как PowerShell использует именованные параметры для привязки, порядок параметров не имеет значения. Однако рекомендуется следовать порядку привязок, определенных в `function.json`.
...
В функциях для `out` выходной привязки в Function `direction` . JSON задано значение. Запись в выходную привязку можно выполнить с помощью `Push-OutputBinding` командлета, который доступен для среды выполнения функций. `name` Во всех случаях свойство привязки, определенное `function.json` в, соответствует `Name` параметру `Push-OutputBinding` командлета.
...
| Данные | **`Write-Information`** <br/> **`Write-Host`** <br /> **`Write-Output`**      | Данные | Выполняет запись в журнал на уровне _информации_ . |
| Отладка | **`Write-Debug`** |
...
> [!IMPORTANT]
> Использование `Write-Verbose` командлетов `Write-Debug` или не является достаточным для просмотра подробных сведений и ведения журнала на уровне отладки. Кроме того, необходимо настроить пороговое значение уровня ведения журнала, которое объявляет, какой уровень журналов вы в действительности интересуют. Дополнительные сведения см. в статье [Настройка уровня ведения журнала приложения функции](#configure-the-function-app-log-level).
...
Если вы используете приложение-функция локально для разработки, ведет журнал по умолчанию в файловой системе. Чтобы просмотреть журналы в консоли, задайте для `AZURE_FUNCTIONS_ENVIRONMENT` `Development` переменной среды значение перед запуском приложение-функция.
...
Функции позволяют использовать [коллекцию PowerShell](https://www.powershellgallery.com) для управления зависимостями. С включенным управлением зависимостями файл требований. PSD1 используется для автоматического скачивания необходимых модулей. Это поведение можно включить, задав для `managedDependency` `true` свойства значение в корневом каталоге [файла host. JSON](functions-host-json.md), как показано в следующем примере:
...
| **`MDMinBackgroundUpgradePeriod`**      | `1.00:00:00`(1 день)     | Чтобы избежать чрезмерного обновления модулей при частых перезапусках рабочих ролей, проверка обновления модулей не выполняется, когда любой рабочий процесс уже инициировал эту проверку `MDMinBackgroundUpgradePeriod`в последней. |

Использование собственных пользовательских модулей немного отличается от того, как это можно сделать обычным образом.

На локальном компьютере модуль устанавливается в одну из глобально доступных папок в `$env:PSModulePath`. При работе в Azure у вас нет доступа к модулям, установленным на вашем компьютере. Это означает, что `$env:PSModulePath` объект для приложения-функции PowerShell отличается `$env:PSModulePath` от в регулярном скрипте PowerShell.
...
> [!NOTE]
> Модули, указанные в файле требований. PSD1, автоматически загружаются и включаются в путь, поэтому их не нужно включать в папку Modules. Они хранятся локально в `$env:LOCALAPPDATA/AzureFunctions` папке и в `/data/ManagedDependencies` папке при запуске в облаке.
...
При запуске приложения-функции языковой рабочий процесс PowerShell добавляет эту `Modules` папку в, `$env:PSModulePath` чтобы вы могли полагаться на автозагрузку модуля точно так же, как в обычном сценарии PowerShell.
...
В этом примере конфигурация `myFunction` включает `scriptFile` свойство, которое ссылается `PSFunction.psm1`на модуль PowerShell в другой папке.  `entryPoint` Свойство ссылается на `Invoke-PSTestFunc` функцию, которая является точкой входа в модуле.
...
В этой конфигурации объект `Invoke-PSTestFunc` выполняется точно так же, как `run.ps1` и.
