---
title: Используйте функции Azure для выполнения задачи очистки базы данных
...
В этой статье показано, как использовать функции Azure для создания запланированного задания, которое подключается к базе данных Azure S'L или управляемому экземпляру Azure S'L. Код функции очищает строки в таблице базы данных. Новая функция СЗ создана на основе заранее определенного шаблона триггера таймер в Visual Studio 2019. Для выполнения этого сценария необходимо также задать строку подключения к базе данных как параметр приложений в приложении-функции. Для управляемого экземпляра Azure S'L необходимо [включить публичную конечную точку](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-public-endpoint-configure) в доступе к функциям Azure. В этом сценарии к базе данных применяется массовая операция. 
...
+ Завершите этапы в статье [Создайте свою первую функцию с помощью Visual Studio](functions-create-your-first-function-visual-studio.md) для создания локального приложения функции, которое нацелено на версию 2.x или более позднюю версию времени выполнения. Кроме того, опубликуйте свой проект в приложении-функции в Azure.
...
1. Щелкните **Строки подключения** в разделе **Параметры** и полностью скопируйте строку подключения **ADO.NET**. Для строки подключения к копиям Azure S'L для общедоступной конечных точек.
...
1. В Solution Explorer нажмите правой кнопкой мыши на проект приложения функции и выберите настройки **службы** > **приложений Publish Ed.** Выберите **Добавить параметр**, а затем в окне **Имя нового параметра приложения** введите `sqldb_connection` и щелкните **ОК**.
...
1. Откройте проект локального функционального приложения в Visual Studio 2019.
...
1. В Solution Explorer нажмите правой кнопкой мыши на проект приложения функции и выберите **функцию «Добавить** > **новую azure».**
