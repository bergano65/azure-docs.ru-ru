...
Служба "Функции" позволяет использовать ключи, чтобы затруднить несанкционированный доступ к конечным точкам функции HTTP во время развертывания.  Если уровень авторизации HTTP для функции, активируемой HTTP, не имеет значение `anonymous`, запросы должны включать ключ API в запрос. 
