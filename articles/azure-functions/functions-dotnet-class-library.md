...
Версии среды выполнения функций работают с конкретными версиями .NET. В следующей таблице показан самый высокий уровень .NET Core, платформа .NET Framework и .NET Core, которые можно использовать с определенной версией функций в проекте. 
...
Цель этого файла — предоставить сведения контроллеру масштабирования, который будет использоваться для [принятия решений о масштабировании в плане потребления](event-driven-scaling.md). По этой причине файл содержит только сведения о триггерах, но не о входных или выходных привязках.
...
К зависимостям пакетов `Sdk` относятся триггеры и привязки. Проект 1. x ссылается на триггеры и привязки 1. x, так как эти триггеры и привязки предназначены для платформа .NET Framework, а триггеры и привязки 2. x предназначены для .NET Core.
