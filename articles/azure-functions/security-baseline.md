...
**Руководство**: неприменимо; приложения-функции не обрабатывают и не создают журналы, связанные с защитой от вредоносных программ.
...
**Руководство**: неприменимо; приложения-функции не обрабатывают и не создают доступные для пользователя журналы, связанные с DNS.
...
**Руководство**. Использование Azure Active Directory (AD) в качестве центральной системы проверки подлинности и авторизации для приложений-функций. Для учета поведения учетной записи для входа в плоскость управления (портал Azure) используйте функции защиты идентификации Azure Active Directory (AD) и средства обнаружения рисков, чтобы настроить автоматические ответы на обнаруженные подозрительные действия, связанные с удостоверениями пользователей. Вы также можете включить данные в Azure Sentinel для дальнейшего изучения.
...
**Руководство**. для обеспечения изоляции используйте отдельные подписки и группы управления для приложений с конфиденциальными или большими рисками.

Развертывайте приложения-функции с высоким риском в своей виртуальной сети (VNet). Безопасность периметра для приложений-функций достигается через виртуальных сетей. Функции, выполняемые в плане Premium или Среда службы приложений (ASE), можно интегрировать с виртуальных сетей. Выберите оптимальную архитектуру для вашего варианта использования.
