...
> [!NOTE]
> В этой статье приводятся рекомендации для приложений функций, предназначенных для Устойчивые функции 1. x. Она еще не обновлена, чтобы учитывать изменения, появившиеся в Устойчивые функции 2. x. Дополнительные сведения о различиях между версиями расширений см. в статье [устойчивые функции версии](durable-functions-versions.md) .
...
Маршрутизатор направляет запросы на развертывание и оркестрации соответствующему приложению функции на основе `version`, отправленного с запросом, игнорируя версию исправления.
