...
| ------------ | ------ | ---------- |
| Даты и время  | API, возвращающие текущую дату или время, являются недетерминированными, поскольку возвращаемое значение отличается для каждого воспроизведения. | Используйте`CurrentUtcDateTime` API в .NET или `currentUtcDateTime` API в JavaScript, который является надежным для воспроизведения. |
