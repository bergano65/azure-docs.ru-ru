...
| ------------ | ------ | ---------- |
| Значения даты и времени  | API, возвращающие текущую дату или время, являются недетерминированными, поскольку возвращаемое значение отличается для каждого воспроизведения. | Используйте API`CurrentUtcDateTime` в .NET или API `currentUtcDateTime` в JavaScript, который является надежным для воспроизведения. |
