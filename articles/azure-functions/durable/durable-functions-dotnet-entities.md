...
В следующем `Counter` примере реализована сущность, которая хранит одно значение типа Integer, и предлагает четыре операции `Add`: `Reset`, `Get`, и. `Delete`
...
`Run` Функция содержит шаблон, необходимый для использования синтаксиса на основе классов. Это должна быть *статическая* функция Azure. Он выполняется один раз для каждого сообщения операции, обрабатываемого сущностью. Когда `DispatchAsync<T>` вызывается и сущность еще не находится в памяти, она конструирует объект типа `T` и заполняет его поля из последнего сохраненного JSON-файла, найденного в хранилище (если таковой имеется). Затем он вызывает метод с соответствующим именем.
...
Классы сущностей — это POCO (обычные старые объекты CLR), для которых не требуются специальные классы, интерфейсы или атрибуты. Но:
...
Операции также имеют доступ к функциональным возможностям, предоставляемым `Entity.Current` контекстом:

* `EntityName`: имя выполняемой в данный момент сущности.
* `EntityKey`— ключ выполняемой в данный момент сущности.
* `EntityId`: идентификатор выполняющейся в данный момент сущности (включает имя и ключ).
...
> [!NOTE]
> Объект, возвращаемый `ReadEntityStateAsync` функцией, — это только локальная копия, то есть моментальный снимок состояния сущности с более ранней точки во времени. В частности, это может быть устаревшим, и изменение этого объекта не влияет на фактическую сущность. 
...
Клиентский код может `SignalEntityAsync<TEntityInterface>` использовать для отправки сигналов в сущности, `TEntityInterface`реализующие. Пример:
...
В этом примере `proxy` параметр является динамически создаваемым экземпляром `ICounter`, который внутренне преобразует вызов `Delete` в сигнал.

> [!NOTE]
> `SignalEntityAsync` API-интерфейсы можно использовать только для односторонних операций. Даже если операция возвращает `Task<T>`, значение `T` параметра всегда будет равно null или `default`, а не фактический результат.
Например, не имеет смысла подать сигнал `Get` операции, так как значение не возвращается. Вместо этого клиенты могут использовать либо `ReadStateAsync` для прямого доступа к состоянию счетчика, либо для запуска функции Orchestrator, которая `Get` вызывает операцию. 
...
Неявным образом возвращаются все возвращаемые `void` операции и любые операции, которые возвращают `Task` или `Task<T>` вызывают. Одно из них может изменить это поведение по умолчанию и операции с сигналами, даже если они `SignalEntity<IInterfaceType>` возвращают задачу, используя метод явным образом.
...
Если указан только ключ сущности и не удается найти уникальную реализацию во время выполнения, `InvalidOperationException` возникает исключение. 
...
* Методы интерфейса сущности должны `void`возвращать `Task`, или`Task<T>` 

Если какое-либо из этих правил нарушается, `InvalidOperationException` во время выполнения создается исключение, когда интерфейс используется в качестве аргумента типа `SignalEntity` для `CreateProxy`или. В сообщении об исключении объясняется, какое правило было разорвано.

> [!NOTE]
> Возвращаемые `void` методы интерфейса могут быть только сигнальными (односторонние), но не вызванными (двусторонними). Методы интерфейса, `Task` возвращающие или `Task<T>` , могут быть либо вызванными, либо сигнальными. При вызове они возвращают результат операции или повторно создают исключения, созданные операцией. Однако при появлению сигнала они не возвращают фактический результат или исключение из операции, а только значение по умолчанию.

## <a name="entity-serialization"></a>Сериализация сущностей

Так как состояние сущности надежно материализованным, класс сущности должен быть сериализуемым. Среда выполнения Устойчивые функции использует библиотеку [JSON.NET](https://www.newtonsoft.com/json) для этой цели, которая поддерживает ряд политик и атрибутов для управления процессом сериализации и десериализации. Наиболее часто используемые типы данных C# (включая массивы и типы коллекций) уже являются сериализуемыми и могут быть легко использованы для определения состояния устойчивых сущностей.
...
- Мы `[JsonObject(MemberSerialization.OptIn)]` добавляем аннотацию к классу, чтобы напомнить нам, что класс должен быть сериализуемым, а также для сохранения только тех элементов, которые явно помечены как свойства JSON.
-  Мы замечаем поля, которые будут сохраняться в, `[JsonProperty("name")]` чтобы напомнить нам, что поле является частью состояния сохраненной сущности, и указать имя свойства, которое будет использоваться в представлении JSON.

Однако эти атрибуты не требуются. другие соглашения или атрибуты могут быть разрешены при условии, что они работают с Json.NET. Например, один из них может `[DataContract]` использовать атрибуты или вообще не иметь атрибутов:
...
По умолчанию имя класса *не* сохраняется как часть представления JSON: то есть используется `TypeNameHandling.None` в качестве значения по умолчанию. Это поведение по умолчанию можно переопределить с `JsonObject` помощью `JsonProperty` атрибутов или.

### <a name="making-changes-to-class-definitions"></a>Внесение изменений в определения классов

При внесении изменений в определение класса после запуска приложения необходимо учитывать некоторые меры, поскольку сохраненный объект JSON может больше не соответствовать новому определению класса. Тем не менее часто можно правильно работать с измененными форматами данных, если один из них понимает процесс десериализации, `JsonConvert.PopulateObject`используемый.
...
Доступ к функциональным возможностям, относящимся к сущностям, можно получить `IDurableEntityContext`через объект контекста типа. Этот объект контекста доступен в качестве параметра функции сущности и через свойство `Entity.Current`Async-Local.
...
* `DeleteState()`: удаляет состояние сущности, если она существует. 

Если состояние, возвращенное `GetState` , является объектом, его можно напрямую изменить с помощью кода приложения. Нет необходимости повторно вызываться `SetState` в конце (но также без ущерба). Если `GetState<TState>` вызывается несколько раз, необходимо использовать один и тот же тип.
