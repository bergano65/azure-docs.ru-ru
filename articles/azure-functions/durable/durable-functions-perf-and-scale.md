...
Максимальная задержка опроса настраивается с `maxQueuePollingInterval` помощью свойства в [файле host. JSON](../functions-host-json.md#durabletask). Установка более высокого значения может привести к увеличению задержек при обработке сообщения. Более высокие задержки будут ожидаться только после периодов бездействия. Если присвоить этому параметру более низкое значение, это может привести к увеличению затрат на хранение данных из-за увеличения транзакций хранилища.
...
Очереди, таблицы и BLOB-объекты, используемые Устойчивые функции, создаются в настроенной учетной записи хранения Azure. Используемая учетная запись может быть задана с помощью параметра `durableTask/azureStorageConnectionStringName` в файле **host.json**.
