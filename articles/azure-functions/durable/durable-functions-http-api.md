...
`systemKey` — Это ключ авторизации, автоматически сформированный узлом функций Azure. В частности, он предоставляет доступ к API расширения устойчивых задач и им можно управлять так же, как и [другими ключами авторизации](https://github.com/Azure/azure-webjobs-sdk-script/wiki/Key-management-API). Можно создавать URL-адреса, содержащие правильные `taskHub` `connection` `systemKey` значения строк запросов, и, используя API [привязки клиента оркестрации](durable-functions-bindings.md#orchestration-client) , такие как API- `CreateCheckStatusResponse` `CreateHttpManagementPayload` интерфейсы и в .NET, а также `createCheckStatusResponse` API- `createHttpManagementPayload` интерфейсы и в JavaScript.
...
* **HTTP 202 (принято)**: для указанной функции Orchestrator запланирован запуск. `Location`Заголовок ответа содержит URL-адрес для опроса состояния оркестрации.
...
* **Location**: URL-адрес конечной точки состояния. Этот URL-адрес содержит то же значение, что и `statusQueryGetUri` поле.
* **Retry-After**: количество секунд ожидания между операциями опроса. Значение по умолчанию — `10`.
...
Если задать значение маркера продолжения в заголовке следующего запроса, можно получить следующую страницу результатов. Это имя заголовка запроса также является `x-ms-continuation-token` .
...
| Поле                  | Тип данных | Описание |
|------------------------|-----------|-------------|
| **`instancesDeleted`** | Целое число   | Число удаленных экземпляров. В случае с одним экземпляром это значение всегда должно быть равно `1` . |
...
Ниже приведен пример запроса, который отправляет определяемое пользователем сообщение Add в `Counter` сущность с именем `steps` . Содержимое сообщения — это значение `5` . Если сущность еще не существует, она будет создана этим запросом:
...
> [!NOTE]
> По умолчанию для [сущностей на основе классов в .NET](durable-functions-dotnet-entities.md#defining-entity-classes)указание `op` значения `delete` приведет к удалению состояния сущности. Если сущность определяет операцию с именем `delete` , то вместо нее будет вызываться определяемая пользователем операция.
...
* **HTTP 400 (** недопустимый запрос): содержимое запроса не было типа `application/json` , не является допустимым JSON или имело недопустимое `entityKey` значение.
* **HTTP 404 (не найдено)**: указанный объект `entityName` не найден.
...
Если `Counter` сущность просто содержала несколько действий, сохраненных в `currentValue` поле, содержимое ответа может выглядеть следующим образом (отформатировано для удобочитаемости):
...
## <a name="list-entities"></a>Список сущностей

Можно запросить несколько сущностей по имени сущности или дате последней операции.
...
| Поле                       | Тип параметра  | Описание |
|-----------------------------|-----------------|-------------|
| **`entityName`**            | URL-адрес             | Необязательный параметр. Если указано, фильтрует список возвращенных сущностей по имени сущности (без учета регистра). |
| **`fetchState`**            | Строка запроса    | Необязательный параметр. Если задано значение `true` , состояние сущности будет включаться в полезные данные ответа. |
| **`lastOperationTimeFrom`** | Строка запроса    | Необязательный параметр. При указании фильтрует список возвращенных сущностей, которые обрабатывали операции после заданной метки времени ISO8601. |
| **`lastOperationTimeTo`**   | Строка запроса    | Необязательный параметр. При указании фильтрует список возвращенных сущностей, которые обрабатывали операции перед заданной ISO8601 отметкой времени. |
| **`top`**                   | Строка запроса    | Необязательный параметр. Если указано, ограничивает количество сущностей, возвращаемых запросом. |


### <a name="response"></a>Ответ

Успешный ответ HTTP 200 содержит сериализованный в JSON массив сущностей и, по желанию, состояние каждой сущности.

По умолчанию операция возвращает первые 100 сущностей, соответствующих условиям запроса. Вызывающий объект может указать значение параметра строки запроса для `top` , чтобы получить другое максимальное число результатов. Если больше результатов существует, чем возвращается, маркер продолжения также возвращается в заголовке ответа. Имя заголовка — `x-ms-continuation-token`.

Если задать значение маркера продолжения в заголовке следующего запроса, можно получить следующую страницу результатов. Это имя заголовка запроса также является `x-ms-continuation-token` .

### <a name="example---list-all-entities"></a>Пример. Перечисление всех сущностей

В следующем примере HTTP-запроса перечислены все сущности в концентраторе задач:

```http
GET /runtime/webhooks/durabletask/entities
```

Формат JSON ответа может выглядеть следующим образом (отформатировано для удобочитаемости):
...
### <a name="example---filtering-the-list-of-entities"></a>Пример фильтрации списка сущностей

В следующем примере HTTP-запрос перечисляет только первые две сущности типа `counter` , а также извлекает свое состояние:
