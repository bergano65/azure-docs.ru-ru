...
| **`purgeHistoryDeleteUri`** |«Очистить журнал» URL-адрес экземпляра оркестрации. |
| **`rewindPostUri`**         |(Предварительная версия) URL-адрес «назад» экземпляра оркестрации. |
...
| Поле                  | Тип данных | Описание |
|------------------------|-----------|-------------|
| **`instancesDeleted`** | integer   | Количество удаленных экземпляров. В случае одного экземпляра это значение всегда должно быть `1`. |
...
|-------------------|-----------------|-------------|
| **`instanceId`**  | URL             | Идентификатор экземпляра оркестрации. |
| **`reason`**      | Строка запроса    | Необязательный. Причина перемотки экземпляра оркестрации. |
...
### <a name="signal-entity-preview"></a>Сущность сигнала (Предварительная версия)

Отправляет сообщение односторонней операцией [устойчивых сущности](durable-functions-types-features-overview.md#entity-functions). Если сущность не существует, он будет создан автоматически.

#### <a name="request"></a>Запрос

HTTP-запрос имеет формат (для ясности показаны несколько строк) следующим образом:
...
| **`op`**          | Строка запроса    | Необязательный. Имя вызываемой операции определяемые пользователем. |
| **`{content}`**   | Содержимое запроса | Полезные данные события в формате JSON. |

Ниже приведен пример запроса, который отправляет определяемое пользователем сообщение «Добавить» для `Counter` сущность с именем `steps`. Содержимое сообщения является значением `5`. Если сущность еще не существует, он будет создан этим запросом:
...
Эта операция имеет несколько возможные ответы:

* **HTTP 202 (Accepted)** (HTTP 202 (принято)): Операция сигнала был принят для асинхронной обработки.
* **HTTP 400 (Bad Request)** (HTTP 400 (недопустимый запрос)): Содержимое запроса не типа `application/json`, не является допустимым документом JSON или был указан недопустимый `entityKey` значение.
* **HTTP 404 (Not Found)** (HTTP 404 (не найдено)): Указанный `entityType` не найден.

Успешный запрос HTTP не содержит все содержимое в ответ. Сбой HTTP-запроса может содержать сведения об ошибке формата JSON в содержимое ответа.
...
Эта операция имеет две возможные ответы:

* **HTTP 200 (OK)** (HTTP 200 (ОК)): Существует указанной сущности.
* **HTTP 404 (Not Found)** (HTTP 404 (не найдено)): Указанная сущность не найдена.

Успешный ответ содержит состояние сериализации JSON объекта, как его содержимое.

#### <a name="example"></a>Пример
Ниже приведен пример запроса HTTP, который получает состояние существующего `Counter` сущность с именем `steps`:

```http
GET /runtime/webhooks/durabletask/entities/Counter/steps
```

Если `Counter` сущности просто содержится несколько этапов, сохраненные в `currentValue` поле, содержимое ответа может выглядеть следующим образом (в формате для удобства чтения):
