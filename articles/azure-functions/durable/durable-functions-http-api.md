...
| Поле                  | Тип данных | Description |
|------------------------|-----------|-------------|
| **`instancesDeleted`** | integer   | Число удаленных экземпляров. В случае с одним экземпляром это значение всегда должно быть `1`. |
...
| Поле                   | Тип данных | Description |
|-------------------------|-----------|-------------|
| **`instancesDeleted`**  | integer   | Число удаленных экземпляров. |
...
> [!NOTE]
> По умолчанию для [сущностей на основе классов в .NET](durable-functions-dotnet-entities.md#defining-entity-classes)указание `op` значение `delete` удалит состояние сущности. Если сущность определяет операцию с именем `delete`, то вместо нее будет вызываться определяемая пользователем операция.
...
| **`fetchState`**            | Строка запроса    | Необязательный параметр. Если задано значение `true`, то состояние сущности будет включаться в полезные данные ответа. |
...
Следующий пример HTTP-запроса перечисляет только первые две сущности типа `counter`, а также извлекает их состояние:
