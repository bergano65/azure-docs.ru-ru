...
В этом примере реализуется система проверки номера телефона на основе SMS. Эти типы потоков часто используются при проверке номера телефона клиента или для многофакторной проверки подлинности. Это мощный пример, потому что вся реализация выполняется с использованием нескольких небольших функций. Внешнее хранилище данных, например база данных, не требуется.
...
* `E4_SmsPhoneVerification`: [Функция оркестратора,](durable-functions-bindings.md#orchestration-trigger) которая выполняет процесс проверки телефона, включая управление тайм-аутами и повторами.
* `E4_SendSmsChallenge`: [Функция активности,](durable-functions-bindings.md#activity-trigger) отправляя код через текстовое сообщение.

### <a name="e4_smsphoneverification-orchestrator-function"></a>E4_SmsPhoneVerification функция оркестратора
...
> [!NOTE]
> Сперва это может показаться неочевидным, но эта функция оркестратора полностью детерминированная. Это детерминированно, `CurrentUtcDateTime` потому что свойство используется для расчета времени истечения времени таймер, и он возвращает то же значение на каждом воспроизведении на данный момент в коде оркестратора. Такое поведение важно для `winner` обеспечения того, `Task.WhenAny`чтобы те же результаты от каждого повторного вызова .
...
> [!NOTE]
> Сперва это может показаться неочевидным, но эта функция оркестратора полностью детерминированная. Это детерминированно, `currentUtcDateTime` потому что свойство используется для расчета времени истечения времени таймер, и он возвращает то же значение на каждом воспроизведении на данный момент в коде оркестратора. Такое поведение важно для `winner` обеспечения того, `context.df.Task.any`чтобы те же результаты от каждого повторного вызова .
...
Пользователи получают SMS-сообщение с 4-значным кодом. У них есть 90 секунд, чтобы отправить тот же четырехзначный код обратно в экземпляр функции оркестратора для завершения процесса проверки. Если пользователи отправляют неверный код, у них есть еще три попытки (в пределах тех же 90 секунд).
...
## <a name="e4_sendsmschallenge-activity-function"></a>функция активности E4_SendSmsChallenge

Функция **E4_SendSmsChallenge** использует привязку Twilio для отправки SMS-сообщения с четырехзначным кодом конечному пользователю.
...
А вот код, который генерирует четырехзначный код вызова и отправляет SMS-сообщение:
...
Чтобы ответить с кодом, вы можете использовать `{eventName}` `SmsChallengeResponse` [ `RaiseEventAsync` (.NET) или `raiseEvent` (JavaScript)](durable-functions-instance-management.md) внутри другой функции или вызвать **sendEventUrl** HTTP POST webhook ссылка ссылка выше, заменив с названием события, :
