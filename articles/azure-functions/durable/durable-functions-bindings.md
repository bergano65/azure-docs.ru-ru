...
Кроме того, функции .NET можно привязать `IAsyncCollector<T>` к, где `T` — `StartOrchestrationArgs` или `JObject`.
...
> [!NOTE]
> Предыдущий код предназначен для Устойчивые функции 2. x. Для Устойчивые функции 1. x необходимо использовать тип `DurableOrchestrationClient` параметра вместо. `IDurableOrchestrationClient` Дополнительные сведения о различиях между версиями см. в статье [устойчивые функции версии](durable-functions-versions.md) .
...
При использовании средств Visual Studio для функций Azure триггер сущности настраивается с помощью атрибута `EntityTriggerAttribute` .NET.
...
* **TState-\<состояния> ()**: Возвращает текущее состояние сущности. Если он еще не существует, он создается и инициализируется в `default<TState>`. `TState` Параметр должен быть примитивным или сериализуемым типом JSON. 
* **TState-\<состояния> (инитфунктион)**: Возвращает текущее состояние сущности. Если он еще не существует, он создается путем вызова предоставленного `initfunction` параметра. `TState` Параметр должен быть примитивным или сериализуемым типом JSON. 
...
* **Тинпут ввода\<-> ()**: получает входные данные для текущей операции. Параметр `TInput` типа должен быть примитивным или сериализуемым типом JSON.
...
* **Сигналентити (EntityId, счедуледтимеутк, Operation, input)**: отправляет одностороннее сообщение в сущность. `operation` Параметр должен быть строкой, отличной от NULL, которая `scheduledTimeUtc` должна быть значением времени в формате UTC, в которое вызывается операция, а `input` параметр должен быть примитивным или сериализуемым объектом JSON.
...
Доступ `IDurableEntityContext` к объекту, переданному в функцию сущности, можно `Entity.Current` получить с помощью свойства Async-Local. Этот подход удобен при использовании модели программирования на основе классов.
...
Если вы используете Visual Studio, можно выполнить привязку к клиенту сущности с помощью атрибута `DurableClientAttribute` .NET.

> [!NOTE]
> Также `[DurableClientAttribute]` можно использовать для привязки к [клиенту оркестрации](#orchestration-client).
...
Затем клиентский код может `SignalEntityAsync<ICounter>` использовать для создания строго типизированного прокси-сервера:
...
`proxy` Параметр является динамически создаваемым экземпляром `ICounter`, который внутренне преобразует вызов `Add` в эквивалентный (нетипизированный) вызов `SignalEntityAsync`.
...
В частности, не имеет смысла подать сигнал `Get` операции, так как значение не возвращается. Вместо этого клиенты могут использовать либо `ReadStateAsync` для прямого доступа к состоянию счетчика, либо для запуска функции Orchestrator, которая `Get` вызывает операцию.
