---
title: Шаблоны и технические концепции Устойчивых функций в Azure
description: Предоставляет сведения о принципах работы Устойчивых функций в Azure для включения преимуществ выполнения кода с отслеживанием состояния в облаке.
services: functions
author: kashimiz
manager: jeconnoc
keywords: ''
...
В этом разделе описываются типичные шаблоны приложения, в которых можно эффективно использовать Устойчивые функции.
...
![Схема цепочки функций](./media/durable-functions-concepts/function-chaining.png)
...
Параметр `context` [DurableOrchestrationContext] \(.NET\) и объект `context.df` (JavaScript) предоставляют методы для вызова других функций по имени, передачи параметров и возврата выходных данных функции. Каждый раз, когда код вызывает `await` (C#) или `yield` (JavaScript), платформа Устойчивых функций создает *контрольные точки* выполнения текущего экземпляра функции. Если процесс или виртуальная машина перезапускается во время выполнения, экземпляр функции возобновляется из предыдущего вызова `await` или `yield`. Подробнее об этом поведении перезапуска вы узнаете позже.

> [!NOTE]
> Объект `context` в JavaScript представляет не [DurableOrchestrationContext], а [контекст функции](../functions-reference-node.md#context-object) в целом.

### <a name="fan-in-out"></a>Шаблон 2. Развертывание/вход
...
![Схема "Развертывание и объединение"](./media/durable-functions-concepts/fan-out-fan-in.png)
...
![Схема API HTTP](./media/durable-functions-concepts/async-http-api.png)
...
![Схема мониторинга](./media/durable-functions-concepts/monitor.png)
...
![Схема участия пользователя](./media/durable-functions-concepts/approval.png)
...
Ниже приведен пример того, как выглядят события отслеживания устойчивых функций на портале Application Insights с использованием [аналитики Application Insights](../../application-insights/app-insights-analytics.md).
...
![Поле customDimensions в запросе App Insights](./media/durable-functions-concepts/app-insights-2.png)
...
![Снимок экрана обозревателя службы хранилища](./media/durable-functions-concepts/storage-explorer.png)
