...
После изучения этой статьи вы научитесь использовать инструменты Visual Studio 2017 для службы "Функции Azure" и локально тестировать функцию hello world.  Эта функция будет организовывать и объединять в цепочку вызовы других функций. Затем вы опубликуете код функции в Azure. Эти средства доступны как часть рабочей нагрузки Azure для разработки в Visual Studio 2017.
...
3. Используйте настройки, указанные в таблице под изображением.
    ...
    | **Шаблон** | Empty | При этом создается пустое приложение-функция. |
...
4. Нажмите кнопку **ОК**, чтобы создать проект пустой функции.

## <a name="add-functions-to-the-app"></a>Добавление функций в приложение

Visual Studio создает проект пустого приложения-функции.  Он содержит необходимые для приложения файлы базовой конфигурации, но еще не содержит никаких функций.  В проект необходимо добавить шаблон устойчивой функции.
...
Новая устойчивая функция будет добавляться в приложение.  Откройте новый файл, чтобы просмотреть содержимое.  Эта устойчивая функция является простым примером цепочки функции.  

* Метод `RunOrchestrator` связан с помощью функции оркестратора.  Эта функция запустит и создаст список, а также добавит в него результат трех вызовов функций.  Список возвращается после завершения вызовов трех функций.  Вызываемая функция — это метод `SayHello` (по умолчанию он будет называться <NameOfFile>_Hello).
* Функция `SayHello` вернет hello.
* Метод `HttpStart` описывает функцию, которая будет запускать экземпляры оркестрации.  [Триггер HTTP](../functions-bindings-http-webhook.md), который связан с ним, запустит новый экземпляр оркестратора и вернет ответ состояния проверки.
...
> [Обзор устойчивых функций](durable-functions-overview.md)
