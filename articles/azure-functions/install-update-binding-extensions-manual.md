...
1. Выберите вкладку **Функции платформы** в разделе **Инструменты разработки**, а затем — **Дополнительные инструменты (Kudu)**. Конечная точка KUDU ( `https://<APP_NAME>.scm.azurewebsites.net/` ) открывается в новом окне.

1. В окне KUDU выберите **консоль отладки**  >  **cmd**.  

1. Чтобы удалить папку в окне командной строки, перейдите к `D:\home\site\wwwroot` и выберите значок удаления рядом с `bin`. Нажмите кнопку **ОК** , чтобы подтвердить удаление.
