...
1. Выберите вкладку **Функции платформы** в разделе **Инструменты разработки**, а затем — **Дополнительные инструменты (Kudu)**. Конечная точка Куду ()`https://<APP_NAME>.scm.azurewebsites.net/`открывается в новом окне.

1. В окне Kudu выберите **консоль** > Debug**CMD.**  

1. Чтобы удалить папку в окне командной строки, перейдите к `D:\home\site\wwwroot` и выберите значок удаления рядом с `bin`. Выберите **OK** для подтверждения удаления.
