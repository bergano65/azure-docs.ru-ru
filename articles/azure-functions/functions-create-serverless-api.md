...
Откройте портал Azure. Для этого войдите на портал [https://portal.azure.com](https://portal.azure.com) с помощью своей учетной записи Azure.
...
1. Используйте настройки триггера HTTP, как указано в таблице.
    ...
    | Поле | Образец значения | Description |
    |---|---|---|
...
При этом добавляется новый прокси, GetUserByName, без свойства backendUri. Вместо вызова другого ресурса он изменяет ответ по умолчанию от прокси-серверов, используя переопределение ответа. Переопределение запроса и ответа может также использоваться в сочетании с URL-адресом внутреннего сервера. Это особенно полезно при создании прокси-сервера для устаревшей системы, где может потребоваться изменить заголовки, параметры запроса и т. д. Дополнительные сведения о переопределениях запросов и ответов см. в разделе [изменение запросов и ответов в учетных записях-посредниках](https://docs.microsoft.com/azure/azure-functions/functions-proxies).
