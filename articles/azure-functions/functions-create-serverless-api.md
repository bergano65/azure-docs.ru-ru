---
title: Настройка конечная точка HTTP в функциях Azure
...
ms.locfileid: "75769274"
---
# <a name="customize-an-http-endpoint-in-azure-functions"></a>Настройка конечная точка HTTP в функциях Azure

В этой статье вы узнаете, как функции Azure позволяют создавать высокомасштабируемые AAP. Azure Functions поставляется с набором встроенных триггеров и привязок HTTP, которые упрощает авторизут конечную точку на различных языках, включая Node.js, C и многое другое. В этой статье вы будете настраивать триггер HTTP для обработки конкретных действий в дизайне API. Вы также подготовите ваш API к увеличению размеров, интегрировав его с прокси-серверами Функций Azure и установив макет API. Все это будет выполнено в бессерверной вычислительной среде Функций, так что вам не придется беспокоиться о масштабировании ресурсов, вы просто сможете сосредоточиться на логике вашего API.

## <a name="prerequisites"></a>Предварительные требования 

[!INCLUDE [Previous quickstart note](../../includes/functions-quickstart-previous-topics.md)]

Полученная функция будет использоваться для остальной части этой статьи.

### <a name="sign-in-to-azure"></a>Вход в Azure

Откройте портал Azure. Для этого вопийте [https://portal.azure.com](https://portal.azure.com) в учетной записи Azure.
...
При этом добавляется новый прокси, GetUserByName, без свойства backendUri. Вместо вызова другого ресурса он изменяет ответ по умолчанию от прокси-серверов, используя переопределение ответа. Переопределение запроса и ответа может также использоваться в сочетании с URL-адресом внутреннего сервера. Это особенно полезно при прокси-сервере в устаревшую систему, где может потребоваться изменение заголовков, параметров запроса и т.д. Чтобы узнать больше об переопределениях запросов и ответов, [см.](https://docs.microsoft.com/azure/azure-functions/functions-proxies)
...
В этой статье вы узнали, как создавать и настраивать API на функциях Azure. а также объединять несколько API, включая макеты, в единую поверхность API. Вы можете использовать эти методы для создания API любой сложности, работая в бессерверной вычислительной модели, предоставляемой Функциями Azure.
