...
Дополнительные сведения о настройке функций HTTP см. в статье [привязки HTTP в функциях Azure](https://docs.microsoft.com/azure/azure-functions/functions-bindings-http-webhook).
...
Повторите шаги для [создания приложения-функции](https://docs.microsoft.com/azure/azure-functions/functions-create-first-azure-function#create-a-function-app), чтобы создать приложение-функцию, в котором вы создадите свой прокси-сервер. Этот URL-адрес нового приложения выступает в качестве внешнего интерфейса для нашего API, а приложение-функция, которое вы ранее редактировали, выступает в качестве серверной части.
...
Затем добавьте макет API. Замените файл прокси. JSON следующим кодом:
