...
Триггер очереди хранилища определяется в *Function. JSON* , где *тип* имеет значение `queueTrigger`.
...
В коде * _ \_init_\_. Корректировка* объявляется параметр `func.ServiceBusMessage`, который позволяет считывать сообщение очереди в функции.
...
В следующем примере Java показана функция триггера очереди хранилища, которая записывает в журнал активируемое сообщение `myqueuename`, помещенное в очередь.
...
В следующей таблице описаны свойства конфигурации привязки, заданные в файле *Function. JSON* и `QueueTrigger` атрибуте.
...
|**двух**| н/д | Только в файле *function.json*. Нужно задать значение `in`. Это свойство задается автоматически при создании триггера на портале Azure. |
...
Получите доступ к данным сообщения с помощью параметра метода, например `string paramName`. `paramName` — Это значение, указанное в `name` свойстве файла *Function. JSON*. Вы можете выполнить привязку к одному из следующих типов:
...
Полезные данные элемента очереди доступны через `context.bindings.<NAME>` , где `<NAME>` соответствует имени, определенному в *Function. JSON*. Если полезная нагрузка — JSON, значение десериализуется в объект.
...
|--------|----|-----------|
|`QueueTrigger`|`string`|Полезные данные очереди (если это допустимая строка). Если полезная нагрузка сообщения очереди является строкой, `QueueTrigger` то она имеет то же значение, что и переменная `name` , именованная свойством в *Function. JSON*.|
...
В файле [host.json](functions-host-json.md#queues) содержатся параметры, управляющие поведением очереди триггера. Дополнительные сведения о доступных параметрах см. в разделе [Параметры Host. JSON](functions-bindings-storage-queue-output.md#hostjson-settings) .
