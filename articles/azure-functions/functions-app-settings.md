...
В версии 2. x и более поздних версиях среды выполнения функций параметры приложения могут переопределять параметры [Host. JSON](functions-host-json.md) в текущей среде. Эти переопределения выражаются в виде параметров `AzureFunctionsJobHost__path__to__setting`приложения с именем. Дополнительные сведения см. в разделе [Переопределение значений Host. JSON](functions-host-json.md#override-hostjson-values).
...
|Key|Образец значения|
|---|------------|
|Режим\_совместимости\_\_функций v2|true|
