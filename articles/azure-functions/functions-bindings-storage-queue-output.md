...
`QueueOutput` Аннотация позволяет записывать сообщение в качестве выходных данных функции. В следующем примере показана функция, активируемая по протоколу HTTP, которая создает сообщение очереди.
