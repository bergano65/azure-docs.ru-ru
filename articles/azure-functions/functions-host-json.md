...
В следующем примере файла *Host. JSON* для версии 2. x + указаны все возможные параметры (за исключением всех, которые предназначены только для внутреннего использования).
...
Полную структуру JSON см. в предыдущем [примере файла host. JSON](#sample-hostjson-file).

> [!NOTE]
> Выборка журналов может привести к тому, что некоторые выполнения не будут отображаться в колонке монитора Application Insights. Чтобы избежать выборки журналов, добавьте `samplingExcludedTypes: "Request"` к значению `applicationInsights`.
...
| самплинжексклудедтипес | null | Разделенный точками с запятой список типов, которые не нужно выделять. Распознаваемые типы: Dependency, Event, Exception, PageView, Request, Trace. Передаются все экземпляры указанных типов; типы, которые не указаны, являются примерами. |
| самплингинклудедтипес | null | Разделенный точками с запятой список типов, для которых необходимо выполнить выборку. пустой список подразумевает все типы. Тип, указанный в `samplingExcludedTypes` переопределяемых типов перечислений. Распознаваемые типы: Dependency, Event, Exception, PageView, Request, Trace. Передаются все экземпляры указанных типов; типы, которые не указаны, являются примерами. |
...
|Свойство | По умолчанию | Description |
| --------- | --------- | --------- | 
| енаблехттптригжерекстендединфоколлектион | Да | Включает или отключает расширенные сведения о HTTP-запросе для триггеров HTTP: заголовки корреляции входящих запросов, поддержку ключей многоплатформенного инструментирования, метод HTTP, путь и ответ. |
...
| хандлеунтраккедексцептионс | Да | Включает или отключает отслеживание исключений, которые не отслеживаются с помощью Application Insights телеметрии. |
...
| исенабледвхенпрофилинг | Да | Включает или отключает создание моментального снимка, даже если Application Insights Profiler собирает подробный сеанс профилирования. |
| исексцептионснаппоинтсенаблед | false | Включает или отключает фильтрацию исключений. |
| исловприоритиснапшотуплоадер | Да | Определяет, следует ли запускать процесс Снапшотуплоадер с нормальным приоритетом. |
...
| максимумснапшотсрекуиред | 3 | Максимальное количество моментальных снимков, собранных для одной проблемы, в диапазоне от 1 до 999. Проблему можно рассматривать как отдельную инструкцию throw в приложении. После того, как количество моментальных снимков, собираемых для проблемы, достигает этого значения, для этой проблемы больше не будет собираться моментальные снимки, пока не будут сброшены счетчики проблем (см. `problemCounterResetInterval`) и достигнуто ограничение `thresholdForSnapshotting`. |
...
Это значение указывает версию схемы файла host. JSON. Строка версии `"version": "2.0"` требуется для приложения-функции, предназначенного для среды выполнения v2, или более поздней версии. Нет изменений в схеме Host. JSON между v2 и v3.
