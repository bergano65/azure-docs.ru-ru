...
|Свойство  |значение по умолчанию | Описание |
|---------|---------|---------| 
|enabled|Да|Указывает, включена ли функция. | 
...
Управляемые зависимости является компонентом предварительной версии, в настоящее время поддерживаются только в функции на основе PowerShell. Он позволяет зависимости, чтобы автоматически управляет служба. Если включено свойство имеет значение true, [requirements.psd1](functions-reference-powershell.md#dependency-management) файл будет обработан. Зависимости будут обновляться при выходе любой дополнительный номер версии.
