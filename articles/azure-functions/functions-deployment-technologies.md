...
+ [Потребление](consumption-plan.md)
+ [Премиальный](functions-premium-plan.md)
+ [Выделенная (служба приложений)](dedicated-plan.md)
...
По умолчанию как [Azure functions Core Tools](functions-run-local.md) , так и [расширение функций Azure для Visual Studio Code](./create-first-function-vs-code-csharp.md#publish-the-project-to-azure) выполнять удаленные сборки при развертывании в Linux. По этой причине оба средства автоматически создают эти параметры в Azure.
...
Приложения-функции, работающие в Linux в [специальном плане (служба приложений)](dedicated-plan.md) и [план Premium](functions-premium-plan.md) , также имеют ограниченный сайт SCM или KUDU.
...
>При развертывании с помощью ZIP-развертывания можно настроить приложение для [запуска из пакета](run-functions-from-deployment-package.md). Чтобы запустить из пакета, задайте `WEBSITE_RUN_FROM_PACKAGE` для параметра [] (функции-app-settings. md # website_run_from_package параметр приложения значение `1` . Рекомендуется использовать ZIP-развертывание. Это позволяет ускорить загрузку приложений, и это значение по умолчанию для VS Code, Visual Studio и Azure CLI.
...
### <a name="cloud-sync"></a>Облачная синхронизация
...
>__Как его использовать:__ Чтобы иметь возможность изменять функции в портал Azure, необходимо [создать функции на портале](./functions-get-started.md). Чтобы сохранить один источник истинности, использование любого другого метода развертывания сделает функцию только для чтения и не позволит продолжить редактирование на портале. Чтобы вернуться к состоянию, в котором можно изменить файлы в портал Azure, можно вручную снова включить режим редактирования `Read/Write` и удалить все параметры приложения, связанные с развертыванием (например [`WEBSITE_RUN_FROM_PACKAGE`](functions-app-settings.md#website_run_from_package) ,.
...
При развертывании приложения-функции в Azure можно выполнить развертывание в отдельном слоте развертывания, а не непосредственно в рабочей среде. Дополнительные сведения о слотах развертывания см. в документации по [слотам развертывания функций Azure](functions-deployment-slots.md) .
