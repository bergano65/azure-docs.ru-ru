...
| Технология развертывания | Использование Windows | Windows Premium (Предварительная версия) | Выделенные Windows  | Использование Linux | Linux Premium (Предварительная версия) | Выделенные Linux |
|-----------------------|:-------------------:|:-------------------------:|:------------------:|:---------------------------:|:-------------:|:---------------:|
...
#### <a name="remote-build-on-linux-preview"></a>Удаленная сборка в Linux (Предварительная версия)
...
> [!NOTE]
> В настоящее время удаленная сборка в специальном плане Linux (служба приложений) поддерживается только для Node. js и Python.
...
Кроме того, можно указать VS Code выполнить удаленную сборку при развертывании, добавив флаг "Азурефунктионс. Скмдобуилддурингдеплоймент". Чтобы узнать, как добавить флаг к VS Code, ознакомьтесь с инструкциями на [вики-сайте расширения функций Azure](https://github.com/microsoft/vscode-azurefunctions/wiki).

>При развертывании с помощью ZIP-развертывания можно настроить приложение для [запуска из пакета](run-functions-from-deployment-package.md). Чтобы запустить из пакета, задайте `WEBSITE_RUN_FROM_PACKAGE` для `1`параметра приложения значение. Рекомендуется использовать ZIP-развертывание. Это позволяет ускорить загрузку приложений, и это значение по умолчанию для VS Code, Visual Studio и Azure CLI. 
