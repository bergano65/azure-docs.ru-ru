...
4. Чтобы проверить функцию, щелкните **Тест** в правой области сверху. После этого откроется вкладка "Тест". В разделе **Текст запроса`0.2` введите значение `0.2`, а затем нажмите кнопку **Запуск**. В тексте ответа возвратится значение **RED**. 
