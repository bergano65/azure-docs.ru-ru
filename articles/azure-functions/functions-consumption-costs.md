...
Рассмотрим случай, когда объем памяти, используемый функцией, остается постоянным. В этом случае вычисление затрат является простым умножением. Например, предположим, что функция заняла 0,5 ГБ в течение 3 секунд. Затем стоимость выполнения составляет `0.5GB * 3s = 1.5 GB-seconds`. 
...
+ **Триггеры и привязки**: Время, затраченное на чтение и запись выходных данных в [привязки функций](functions-triggers-bindings.md) , учитывается как время выполнения. Например, если функция использует выходную привязку для записи сообщения в очередь службы хранилища Azure, то время выполнения включает время, затраченное на запись сообщения в очередь, которое включается в вычисление стоимости функции. 
...
1. В левой части выберите **метрики** > **выберите ресурс**, а затем используйте параметры под изображением, чтобы выбрать приложение-функцию.
    ...
    |---------|---------|---------|
    | Subscription    |  Ваша подписка  | Подписка с приложением функции.  |
...
На этой диаграмме показан общий объем 1 110 000 000 `Function Execution Units`, потребляемый в течение двух часов, измеряется в МБ-миллисекундах. Чтобы преобразовать в GB-seconds, разделите на 1024000. В этом примере используется приложение-функция @no__t – 0 ГБ-секунд. Вы можете взять это значение и умножить его на текущую стоимость времени выполнения на [цен на странице][странице цен] ценообразования, что дает вам затраты на эти два часа, предполагая, что вы уже использовали любые бесплатные гранты времени выполнения. 
...
Не забудьте заменить `<AZURE_SUBSCRIPTON_ID>` на идентификатор подписки Azure, выполняющий команду.
...
```
В этом конкретном ответе показано, что от `2019-09-11T21:46` до `2019-09-11T23:18`, в течение которого приложение заняло 1110000000 МБ-миллисекунд (1083,98 ГБ/с).
...
|----------------------------|---------------|-------------|
| 9/12/2019, 1:05:14 @ NO__T-0947 AM | Байты исключительного пользования | 209 932 288 |
| 9/12/2019, 1:06:14 @ NO__T-0994 AM | Байты исключительного пользования | 212 189 184 |
| 9/12/2019, 1:06:30 @ NO__T-0010 AM | Байты исключительного пользования | 231 714 816 |
| 9/12/2019, 1:07:15 @ NO__T-0040 AM | Байты исключительного пользования | 210 591 744 |
| 9/12/2019, 1:12:16 @ NO__T-0285 AM | Байты исключительного пользования | 216 285 184 |
| 9/12/2019, 1:12:31 @ NO__T-0376 AM | Байты исключительного пользования | 235 806 720 |
...
|----------------------------|-----------------------------|
| QueueTrigger Авгдуратионмс | 16 @ no__t — 0087                     |
| QueueTrigger Максдуратионмс | 90 @ no__t — 0249                     |
| QueueTrigger Миндуратионмс | 8 @ no__t — 0522                      |
