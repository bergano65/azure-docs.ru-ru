...
ms.locfileid: "67447714"
---
# <a name="develop-azure-functions-using-visual-studio"></a>Разработка Функций Azure с помощью Visual Studio  

Средства функций Azure — это расширение для Visual Studio, которое служит для разработки, тестирования и развертывания C# функции в Azure. Если вы впервые сталкиваетесь с функциями Azure, дополнительные сведения см. в статье [Общие сведения о Функциях Azure](functions-overview.md).
...
Другие ресурсы, которые используются, например учетной записи хранения Azure создаются в вашей подписке во время публикации.
...
* **local.settings.json**: содержит параметры, используемые при выполнении функций локально. Эти параметры не используются при работе в Azure. Дополнительные сведения см. в разделе [файл с локальными параметрами](#local-settings-file).
...
Параметры в файле local.settings.json не передаются автоматически при публикации проекта. Чтобы убедиться в том, что эти параметры также существуют в приложении-функции в Azure, необходимо передать их после публикации проекта. Дополнительные сведения см. в разделе [параметры приложения-функции](#function-app-settings).
...
При публикации из Visual Studio, один из двух методов развертывания используются:

* [Веб-развертывание](functions-deployment-technologies.md#web-deploy-msdeploy): пакеты и развертывание приложений Windows к любому серверу IIS.
* [ZIP-развертывание с помощью выполнения-с-пакет включен](functions-deployment-technologies.md#zip-deploy): рекомендуется выбирать для развертывания функций Azure.

Следуйте инструкциям ниже, чтобы опубликовать проект для приложения-функции в Azure.
