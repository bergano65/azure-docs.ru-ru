...
Ограничения доступа доступны в службе " [премиум](functions-premium-plan.md)", " [потребление](functions-scale.md#consumption-plan)" и " [служба приложений](functions-scale.md#app-service-plan)".
...
1. Включите конечные точки службы или закрытую конечную точку для учетной записи хранения.  
    * Не забудьте включить подсеть, выделенную для приложений-функций, при использовании конечной точки службы.
    * Не забудьте создать запись DNS и настроить приложение для [работы с конечными точками частной конечной](#azure-dns-private-zones) точки, если используется частная конечная точка.  Учетной записи хранения потребуется частная конечная точка для `file` и `blob` подресурсов.  При использовании определенных возможностей, таких как Устойчивые функции, также требуются `queue` и `table` доступны через подключение частной конечной точки.
