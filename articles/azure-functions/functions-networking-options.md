...
|[Предварительный просмотр интеграции виртуальной сети (Azure ExpressRoute и исходящего трафика конечных точек службы)](#preview-version-of-virtual-network-integration)|❌No|✅Yes|✅Yes|✅Yes|
|[гибридных подключений](#hybrid-connections)|❌No|❌No|✅Yes|✅Yes|
|[доступ к частным сайтам](#private-site-access)|❌No| ✅Yes|✅Yes|✅Yes|
...
## <a name="outbound-ip-restrictions"></a>Ограничения исходящего IP-адресов

Ограничения исходящего IP-адресов доступны только для функций, развернутых в среде службы приложений. Вы можете настроить ограничения исходящих подключений для виртуальной сети, в которой развертывается среда службы приложений.
...
Доступ к частным сайтам подразумевает приложения доступны только из частной сети как из в пределах виртуальной сети Azure. 
* Доступ к частным сайтам доступны для уровня "премиум" и службы приложений планированию времени **конечные точки службы** настроены. Дополнительные сведения см. в разделе [конечных точек службы виртуальной сети](../virtual-network/virtual-network-service-endpoints-overview.md)
    * Имейте в виду, что конечные точки служб, функция по-прежнему оказывает полную исходящий доступ к Интернету, даже с настроена интеграция виртуальной сети.
* Доступ к частным сайтам доступна только в среде службы приложений, настроены с помощью внутреннего балансировщика нагрузки (ILB). Дополнительные сведения см. в разделе [Создание и использование внутреннего балансировщика нагрузки со средой службы приложений](../app-service/environment/create-ilb-ase.md).
