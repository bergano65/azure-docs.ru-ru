...
Параметры в файле host. JSON применяются ко всем функциям в приложении в рамках *одного экземпляра* функции. Например, если у вас есть приложение-функция с двумя функциями HTTP, а для запросов [`maxConcurrentRequests`](functions-bindings-http-webhook-output.md#hostjson-settings) задано значение 25, запрос к любому триггеру HTTP будет считаться общим 25 одновременных запросов.  Если это приложение-функция масштабируется до 10 экземпляров, то две функции эффективно допускают 250 параллельных запросов (10 экземпляров * 25 одновременных запросов на экземпляр). 
