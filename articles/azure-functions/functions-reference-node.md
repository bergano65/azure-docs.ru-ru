...
Входные данные в Функциях Azure делятся на две категории: входные данные от триггера и дополнительные входные данных. Привязки триггера и другие привязки для ввода (привязки `direction === "in"`) могут считываться функцией тремя способами.
 - **_[Рекомендуется.]_ Как параметры, передаваемые функции.** Они передаются в функцию в том же порядке, в каком они определены в файле *function.json*. Свойство, определенное в *Function. JSON* , не обязано совпадать с именем параметра, хотя оно должно. `name`
...
+ **_[Только ответ.]_ Путем вызова `context.done()`.** Специальный тип привязки HTTP возвращает ответ, который передается `context.done()` в метод. Следующая привязка вывода HTTP определяет параметр вывода `$return`:
...
| 2.x  | _Active LTS_ и _обслуживание LTS_ версии Node. js (рекомендуется использовать 8.11.1 и 10.14.1). Установите версию, используя [параметр приложения](functions-how-to-use-azure-function-app-settings.md#settings) WEBSITE_NODE_DEFAULT_VERSION.|
...
При отладке с помощью VS Code `--inspect` параметр автоматически добавляется `port` с использованием значения в файле Launch. JSON проекта.
...
Файлы TypeScript (. TS) преобразуются в файлы JavaScript (JS) в `dist` выходном каталоге. Шаблоны TypeScript используют [ `scriptFile` параметр](#using-scriptfile) в `function.json` , чтобы указать расположение соответствующего JS файла в `dist` папке. Расположение выходных данных задается шаблоном с помощью `outDir` параметра `tsconfig.json` в файле. Если изменить этот параметр или имя папки, среда выполнения не сможет найти код для выполнения.
...
- Выполните команду, выберите `node` в качестве стека языка, а затем выберите `typescript`. `func init`
...
При написании функций Azure в JavaScript следует писать код с помощью `async` ключевых слов и. `await` Написание кода `async` с `await` помощью и вместо обратных `.then` вызовов `.catch` или и с помощью обещания помогает избежать двух распространенных проблем:
...
Использование ключевых слов `await`ипомогает избежать обеих этих ошибок. `async` Следует использовать функцию [`util.promisify`](https://nodejs.org/api/util.html#util_util_promisify_original) служебной программы Node. js для включения функций в стиле обратного вызова с ошибками в ожидающие функции.

В приведенном ниже примере все необработанные исключения, вызванные во время выполнения функции, не вызывают ошибку при отдельном вызове, вызвавшем исключение. Ключевое слово означает, что `readFileAsync` следующие шаги выполняются `readFile` только после завершения. `await` В `async` и `await` также`context.done()` не требуется вызывать обратный вызов.
