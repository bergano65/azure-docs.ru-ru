...
Возвращает именованный объект, используемый для чтения свойства или присвоения привязки данных. Входные данные и привязка данных триггера может осуществляться путем чтения свойства на `context.bindings`. Выходные данные привязки могут назначаться путем добавления данных к `context.bindings`
...
В Azure Functions воспользуйтесь методами `context.log` для записи выходных данных трассировки в консоль. В Функциях версии 2.x трассировка выходных данных с помощью `console.log` регистрируется на уровне приложения-функции. Это означает, что выходные данные из `console.log` не привязаны к вызова определенной функции и они не отображаются в журналах определенной функции. Тем не менее они распространяются в Application Insights. В Функциях версии 1.x для записи в консоль нельзя использовать метод `console.log`.
