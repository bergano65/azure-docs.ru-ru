...
Функция JavaScript — это экспортированная `функция` , которая `function` выполняется при активации ([триггеры настраиваются в файле function.json](functions-triggers-bindings.md)). Первый аргумент, передаваемый каждая функция является `context` объект, который используется для получения и отправки привязки данных, ведение журналов и взаимодействия со средой выполнения.

В этой статье предполагается, что вы уже прочли [руководство для разработчиков по Функциям Azure](functions-reference.md). Выполнить быстрый запуск функций для создания первой функции с помощью [Visual Studio Code](functions-create-first-function-vs-code.md) или [на портале](functions-create-first-azure-function.md).

В этой статье также поддерживает [TypeScript разрабатываемым](#typescript).
...
Входные данные в Функциях Azure делятся на две категории: входные данные от триггера и дополнительные входные данных. Привязки триггера и другие привязки для ввода (привязки `direction === "in"`) могут считываться функцией тремя способами.
 - **_[Рекомендуется]_  Как параметры, передаваемые в функции.** Они передаются в функцию в том же порядке, в каком они определены в файле *function.json*. `name` Свойство, определенное в *function.json* не должны совпадать с именем параметра, несмотря на то, что он должен.
 
   ```javascript
   module.exports = async function(context, myTrigger, myInput, myOtherInput) { ... };
   ```
   
 - **Как члены [ `context.bindings` ](#contextbindings-property) объекта.** Каждый элемент назван по свойству `name`, определенному в файле *function.json*.
 ...
 - **В качестве входных данных, с помощью JavaScript [ `arguments` ](https://msdn.microsoft.com/library/87dw3w1k.aspx) объекта.** Этот способ по сути идентичен передаче входных данных в качестве параметров, но позволяет динамически обрабатывать входные данные.
...
- **_[Рекомендуется для нескольких выходов]_  Возврата объекта.** Если вы используете async/обещание, возвращая функции, может возвращать объект с назначенного выходных данных. В приведенном ниже примере выходные привязки в файле *function.json* называются httpResponse и queueOutput.
...
- **_[Рекомендуется для одного выходного]_  Возврат значения непосредственно и с помощью $return имя привязки.** Этот метод работает для функций возврата (асинхронной или функции обещаний). См. пример в разделе [Экспорт асинхронной функции](#exporting-an-async-function). 
...
| **warn (_сообщение_)**    | Записывает сообщение в журнал на уровне предупреждений или более низком. |
| **Info (_сообщение_)**    | Записывает сообщение в журнал на уровне сведений или более низком.    |
| **verbose (_сообщение_)** | Записывает сообщение в журнал на уровне детализации.           |
...
| Свойство      | ОПИСАНИЕ                                                    |
| ------------- | -------------------------------------------------------------- |
| _текст_        | Объект, содержащий текст запроса.               |
...
| Свойство  | ОПИСАНИЕ                                               |
| --------- | --------------------------------------------------------- |
| _текст_    | Объект, содержащий текст ответа.         |
...
+ **С помощью именованных входных и выходных привязок.** В этом случае триггер и привязки HTTP работают как любая другая привязка. В следующем примере объект ответа задается с помощью именованной привязки `response`: 
    ...
    ```
+ **_[Только ответ]_ Путем вызова `context.res.send(body?: any)`.** HTTP-ответ создается с входными данными `body` в качестве текста ответа. `context.done()` неявно вызывается.
...
`scriptFile` можно использовать для получения структуру папок, которая выглядит как в следующем примере:
...
Если целевая версия 2.x среды выполнения функций, оба [функций Azure для Visual Studio Code](functions-create-first-function-vs-code.md) и [основных инструментов функций Azure](functions-run-local.md) позволяют создавать приложения-функции с помощью шаблона, которые поддерживают Проекты приложений функция TypeScript. Шаблон создает `package.json` и `tsconfig.json` файлы проекта, которые упрощают для транскомпиляции, запуска и публикации функций JavaScript из кода TypeScript с помощью этих средств.

Созданный `.funcignore` файл используется для указания, какие файлы будут удалены при публикации проекта в Azure.  

Файлы TypeScript (.ts), транскомпилированный в файлы JavaScript (с расширением JS) `dist` выходной каталог. Используйте шаблоны TypeScript [ `scriptFile` параметр](#using-scriptfile) в `function.json` чтобы указать расположение соответствующего файла с расширением JS в `dist` папку. Расположение выходных данных задается с помощью шаблона с помощью `outDir` параметр в `tsconfig.json` файл. Если изменить этот параметр или имя папки, среда выполнения не сможет найти для запуска кода.

> [!NOTE]
> Экспериментальная поддержка TypeScript существует версия 1.x среды выполнения функций. Transpiles экспериментальной версии файлов TypeScript в файлы JavaScript при вызове функции. В версии 2.x, этот экспериментальную поддержку был заменен методом на основе инструментов, выполняющий транспилирования до инициализации узла и во время развертывания.
...
При развертывании приложения-функции в Azure с помощью **развернуть приложение-функцию...**  кнопки, расширение функций Azure сначала создает готовый к выпуску сборки файлов JavaScript из исходных файлов TypeScript.
...
Чтобы выполнять код функции приложения, локально с помощью основных инструментов, используйте `npm start` , вместо команды `func host start`. `npm start` Команда эквивалентна следующие команды:
...
Прежде чем использовать [ `func azure functionapp publish` ] для развертывания в Azure, сначала необходимо запустить `npm run build:production` команды. Эта команда создает сборку готовых файлов JavaScript из исходных файлов TypeScript, которые могут быть развернуты с помощью [ `func azure functionapp publish` ].
