...
1. В [GitHub](https://github.com)найдите репозиторий, выберите **Параметры** > **секреты** > **добавьте новый секрет**.

   ![Добавить секрет](media/functions-how-to-github-actions/add-secret.png)

1. Используйте `AZURE_CREDENTIALS` в качестве **имени** и скопированного результата команды для **value**, если затем выберите **Добавить секрет**. Если используется профиль публикации, для параметра **имя** и содержимое файла укажите **значение**`SCM_CREDENTIALS`.
