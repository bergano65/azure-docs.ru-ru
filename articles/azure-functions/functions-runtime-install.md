...
1. После того как вы прочитали условия **EULA**, **проверить окно,** чтобы принять условия и нажмите **Далее,** чтобы продвинуться.
...
1. Инструмент конфигурации **Runtime функций Azure** показывает, какие роли установлены на вашей машине.
...
1. Нажмите на вкладку **«Учетные данные».**  Здесь необходимо создать два новых учетных данных для использования с долей файлов для размещения всех функциональных приложений.  Укажите **имя пользователя** и **пароль** для **владельца общей папки** и **пользователя общей папки**, а затем выберите **Применить**.

    ![Учетные данные предварительной версии среды выполнения службы "Функции Azure"][9]

1. Нажмите на вкладку **«Поделиться файлами».**  Здесь необходимо указать детали расположения общего файла.  Его можно создать автоматически или вы можете использовать имеющийся общий файловый ресурс, щелкнув **Применить**.  Если вы выберете новое расположение общей папки, укажите каталог для использования в среде выполнения Функций Azure.

    ![Предварительная версия среды выполнения службы "Функции Azure" — общая папка][10]

1. Нажмите на вкладку **IIS.**  На этой вкладке отображается подробная информация об веб-сайтах в IIS, которую создает инструмент конфигурации Runtime функций Azure.  Здесь вы можете указать пользовательское DNS-имя для портала предварительной версии среды выполнения службы "Функции Azure".  Нажмите **Применить** для завершения.

    ![Предварительная версия среды выполнения службы "Функции Azure" — IIS][11]

1. Нажмите на вкладку **Службы.**  На этой вкладке отображается состояние служб в инструменте конфигурации Runtime функций Azure.  Если **служба активации узла службы "Функции Azure"** не запущена после первоначальной настройки, щелкните **Запустить службу**.
...
1. `https://<machinename>.<domain>` Например, `https://mycomputer.mydomain.com`посетите портал **Runtime функций Azure.**
...
1. Для создания приложений-функций необходимо создать подписку.  В верхнем левом углу портала щелкните опцию **+** рядом с подписками.
