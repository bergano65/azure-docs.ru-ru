...
> [!NOTE]
> Эта статья предназначена для службы "Функции Azure" версии 1.x.  Справочные сведения о host.json в функциях 2. x и более поздних версий см. в разделе [host.jsпо Справочнику по функциям Azure 2. x](functions-host-json.md).
...
|Свойство  |По умолчанию | Описание |
|---------|---------|---------| 
|Включено|верно|Указывает, включена ли функция. | 
...
|autoComplete|верно|Если значение равно true, то триггер автоматически завершит обработку сообщения при успешном выполнении операции. Если значение равно "false", функция должна завершить сообщение перед возвратом.|
