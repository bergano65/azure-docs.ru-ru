...
В следующем примере [показана функция C-звезды,](functions-dotnet-class-library.md) выполняемая каждый раз, когда минуты имеют значение, делится на пять (например, если функция начинается в 18:57:00, следующее исполнение будет в 19:00:00). Объект [`TimerInfo`](https://github.com/Azure/azure-webjobs-sdk-extensions/blob/master/src/WebJobs.Extensions/Extensions/Timers/TimerInfo.cs) передается в функцию.
...
В следующем примере показаны привязка триггера таймера в файле *function.json* и [функция JavaScript](functions-reference-node.md), которая использует эту привязку. Эта функция выполняет запись в журнал, указывая, когда ее вызов выполняется из-за пропущенного запуска по расписанию. Объект [таймера](#usage) передается в функцию.
...
В следующем примере используется связывание триггера таймера, конфигурация которого описана в файле *function.json.* Фактическая [функция Python,](functions-reference-python.md) использующая привязку, описана в файле * __init__.py.* Объект, передаваемый в функцию, имеет тип [azure.functions.TimerRequest object.](/python/api/azure-functions/azure.functions.timerrequest) Логика функции записывает в журналы, указывая, является ли текущий вызов из-за пропущенного возникновения расписания. 
...
Конструктор атрибута принимает выражение CRON или `TimeSpan`. Использовать его `TimeSpan` можно только в том случае, если приложение функции работает в плане Службы Приложения. `TimeSpan`не поддерживается для функций потребления или упругой премиум-класса.
...
В следующей таблице объясняется свойства связывающей конфигурации, `TimerTrigger` установленные в файле *function.json* и атрибуте.

|свойство function.json | Свойство атрибута |Описание|
|---------|---------|----------------------|
|**тип** | Недоступно | Этому свойству необходимо присвоить значение "timerTrigger". Это свойство задается автоматически при создании триггера на портале Azure.|
...
|**Расписание**|**ScheduleExpression**|Значение [выражения CRON](#ncrontab-expressions) или [TimeSpan](#timespan). `TimeSpan` можно использовать только для приложения-функции, которая работает в плане службы приложений. Вы можете поместить выражение расписания в настройку приложения и установить это свойство на имя настройки приложения, обернутое в **%** знаки, как в этом примере: "%ScheduleAppSetting%". |
|**runOnStartup**|**RunOnStartup**|Если настроено значение `true`, функция вызывается при запуске среды выполнения. Например, среда выполнения запускается, когда приложение-функция выходит из спящего режима (в который она перешла из-за отсутствия активности), когда приложение функции перезапускается из-за изменений функции, и когда приложение функции масштабируется. Так **runOnStartup** должны редко, если `true`когда-либо быть установлен на , особенно в производстве. |
|**useMonitor**|**UseMonitor**|Установите значение `true` или`false`, чтобы указать, следует ли отслеживать расписание. При мониторинге расписания его экземпляры сохраняются, чтобы обеспечить его корректную обработку даже при перезапуске экземпляров приложения-функции. Если не установить явно, `true` по умолчанию для графиков, которые имеют интервал повторения больше, чем или равна 1 минуте. Для расписаний, выполняющихся чаще одного раза в минуту, значением по умолчанию является `false`.

[!INCLUDE [app settings to local.settings.json](../../includes/functions-app-settings-local.md)]

> [!CAUTION]
> Мы не рекомендуем задавать параметр **runOnStartup** для `true` в рабочей среде. Использование этого параметра заставляет код выполняться в очень непредсказуемое время. В определенных рабочих ситуациях эти дополнительные выполнения могут привести значительно более высоким затратам для приложений, размещенных в планах потребления. Например, при включенном **runOnStartup** триггер вызывается при масштабировании приложения функции. Убедитесь, что вы полностью понимаете рабочее поведение своих функций перед включением **runOnStartup** в рабочей среде.   

## <a name="usage"></a>Использование

При вызове функции триггера таймера объект таймера передается в функцию. Далее представлен JSON в качестве примера объекта таймера.
...
Azure Functions использует библиотеку [NCronTab](https://github.com/atifaziz/NCrontab) для интерпретации выражений NCRONTAB. Выражение NCRONTAB похоже на выражение CRON, за исключением того, что оно включает в себя дополнительное шестое поле в начале для использования для точности времени в секундах:
...
|Значение интервала (оператор `/`)|<nobr>"0 */5 * * * *"</nobr>|в hh:00:00, hh:05:00, hh:10:00 и так далее через hh:55:00, где hh каждый час (12 раз в час)|

[!INCLUDE [functions-cron-expressions-months-days](../../includes/functions-cron-expressions-months-days.md)]

### <a name="ncrontab-examples"></a>Примеры NCRONtab

Вот несколько примеров выражений NCRONTAB, которые можно использовать для триггера таймера в Azure Functions.
...
Например, *восточное поясное время* — UTC-05:00. Чтобы ваш таймер вызвать пожар в 10:00 утра EST каждый день, используйте следующее выражение NCRONTAB, который приходится на часовой пояс UTC:

```
"0 0 15 * * *"
``` 

Или создайте параметр приложения с именем `WEBSITE_TIME_ZONE` для приложения-функции и задайте **Восточное время США** в качестве значения.  Затем использует следующее выражение NCRONTAB: 
...
Если вы делитесь учетными записями хранения в функциональных приложениях, которые не развернуты в службе приложений, возможно, потребуется явно назначить идентификатор хоста каждому приложению.
...
Можно опустить идентифицирующую ценность или вручную установить конфигурацию идентифицирующей функции каждой функции на другое значение.

Триггер таймер использует блокировку хранилища, чтобы гарантировать, что есть только один экземпляр таймер, когда приложение функции масштабируется до нескольких экземпляров. Если два функциональных приложения имеют одну и ту же идентифицирующую конфигурацию и каждый использует триггер таймер, выполняется только один таймер.
