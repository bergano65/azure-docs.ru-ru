...
Например, приложение-функция JavaScript ограничено ограничением памяти по умолчанию в Node. js. Чтобы увеличить этот фиксированный предел памяти, добавьте параметр `languageWorkers:node:arguments` приложения со значением. `--max-old-space-size=<max memory in MB>`
