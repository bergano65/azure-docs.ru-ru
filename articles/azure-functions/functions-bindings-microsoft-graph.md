...
|**direction**| Недоступно |Обязательное. Необходимо задать значение `in`.|
|**Фильтрация**|**Фильтр**| Если задано значение `userFromRequest`, привязка будет получать только подписки, принадлежащие вызывающему пользователю (применяется только с [триггером HTTP]).| 
