...
|**direction**| Недоступно |Обязательное. Необходимо задать значение `in`.|
|**Фильтрация**|**Filter**| Если задано значение `userFromRequest`, привязка будет получать только подписки, принадлежащие вызывающему пользователю (применяется только с [триггером HTTP]).| 
