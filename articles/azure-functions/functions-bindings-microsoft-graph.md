...
ms.locfileid: "88962848"
---
# <a name="microsoft-graph-bindings-for-azure-functions"></a>Привязки Microsoft Graph для службы "Функции Azure"

> [!IMPORTANT]
> Привязки Microsoft Graph Preview теперь не рекомендуются. Дополнительные сведения об использовании Microsoft Graph с функциями Azure см. в руководстве по [созданию функций Azure с помощью Microsoft Graph](https://docs.microsoft.com/graph/tutorials/azure-functions) .
...
### <a name="enabling-functions-20"></a>Включение функций 2,0

Расширения привязки доступны только для функций Azure 2,0. 

Сведения о том, как настроить приложение-функцию для использования версии 2,0 среды выполнения функций, см. [в разделе Назначение целевых версий среды выполнения функций Azure](set-runtime-version.md).
...
> [!Note] 
> Процесс установки на портале может занять до 10 минут в плане потребления.
...
|**Ресурс**|**ресурсов**|Обязательное. URL-адрес ресурса Azure AD, для которого запрашивается маркер.|
...
|**type**| Недоступно |Обязательное. Необходимо задать значение `excel`.|
|**direction**| Недоступно |Обязательное. Необходимо задать значение `in`.|
|**identity**|**Идентификация**|Обязательное. Удостоверение, которое будет использоваться для выполнения действия. Может иметь одно из следующих значений:<ul><li><code>userFromRequest</code> — применяется только с [триггером HTTP]. Позволяет использовать удостоверение вызывающего пользователя.</li><li><code>userFromId</code> — позволяет использовать удостоверение пользователя, ранее вошедшего в систему, с указанным идентификатором. См. описание свойства <code>userId</code>.</li><li><code>userFromToken</code> — позволяет использовать удостоверение, представленное указанным токеном. См. описание свойства <code>userToken</code>.</li><li><code>clientCredentials</code> — позволяет использовать удостоверение приложения-функции.</li></ul>|
|**userId**|**UserId**  |Требуется, только если для свойства _identity_ задано значение `userFromId`. Идентификатор субъекта-пользователя, связанный с пользователем, ранее вошедшим в систему.|
|**userToken**|**UserToken**|Требуется, только если для свойства _identity_ задано значение `userFromToken`. Токен, предназначенный для приложения-функции. |
|**путь**|**Путь**|Обязательное. Путь к книге Excel в OneDrive.|
|**worksheetName**|**воркшитнаме**|Лист, на котором находится таблица.|
...
|**worksheetName**|**воркшитнаме**|Лист, на котором находится таблица.|
|**tableName**|**TableName**|Имя таблицы. Если это свойство не указано, будет использоваться содержимое листа.|
|**updateType**|**UpdateType**|Обязательное. Тип изменения, которое требуется внести в таблицу. Может иметь одно из следующих значений:<ul><li><code>update</code> — замена содержимого таблицы в OneDrive.</li><li><code>append</code> — добавление полезных данных в конец таблицы в OneDrive путем создания новых строк.</li></ul>|
...
|**type**| Недоступно |Обязательное. Необходимо задать значение `onedrive`.|
|**direction**| Недоступно |Обязательное. Необходимо задать значение `in`.|
|**identity**|**Идентификация**|Обязательное. Удостоверение, которое будет использоваться для выполнения действия. Может иметь одно из следующих значений:<ul><li><code>userFromRequest</code> — применяется только с [триггером HTTP]. Позволяет использовать удостоверение вызывающего пользователя.</li><li><code>userFromId</code> — позволяет использовать удостоверение пользователя, ранее вошедшего в систему, с указанным идентификатором. См. описание свойства <code>userId</code>.</li><li><code>userFromToken</code> — позволяет использовать удостоверение, представленное указанным токеном. См. описание свойства <code>userToken</code>.</li><li><code>clientCredentials</code> — позволяет использовать удостоверение приложения-функции.</li></ul>|
|**userId**|**UserId**  |Требуется, только если для свойства _identity_ задано значение `userFromId`. Идентификатор субъекта-пользователя, связанный с пользователем, ранее вошедшим в систему.|
|**userToken**|**UserToken**|Требуется, только если для свойства _identity_ задано значение `userFromToken`. Токен, предназначенный для приложения-функции. |
...
* [Использование](#outlook-output---usage)
...
|**type**| Недоступно |Обязательное. Необходимо задать значение `outlook`.|
|**direction**| Недоступно |Обязательное. Необходимо задать значение `out`.|
|**identity**|**Идентификация**|Обязательное. Удостоверение, которое будет использоваться для выполнения действия. Может иметь одно из следующих значений:<ul><li><code>userFromRequest</code> — применяется только с [триггером HTTP]. Позволяет использовать удостоверение вызывающего пользователя.</li><li><code>userFromId</code> — позволяет использовать удостоверение пользователя, ранее вошедшего в систему, с указанным идентификатором. См. описание свойства <code>userId</code>.</li><li><code>userFromToken</code> — позволяет использовать удостоверение, представленное указанным токеном. См. описание свойства <code>userToken</code>.</li><li><code>clientCredentials</code> — позволяет использовать удостоверение приложения-функции.</li></ul>|
|**userId**|**UserId**  |Требуется, только если для свойства _identity_ задано значение `userFromId`. Идентификатор субъекта-пользователя, связанный с пользователем, ранее вошедшим в систему.|
|**userToken**|**UserToken**|Требуется, только если для свойства _identity_ задано значение `userFromToken`. Токен, предназначенный для приложения-функции. |
...
|---------|---------|----------------------|
|**name**| Недоступно |Обязательное. Имя переменной, используемое в коде функции для сообщения электронной почты. Дополнительные сведения см. в разделе [Использование выходной привязки сообщений Outlook в коде](#outlook-output-code).|
|**type**| Недоступно |Обязательное. Необходимо задать значение `graphWebhookSubscription`.|
|**direction**| Недоступно |Обязательное. Необходимо задать значение `in`.|
...
|**userId**|**UserId**  |Требуется, только если для свойства _identity_ задано значение `userFromId`. Идентификатор субъекта-пользователя, связанный с пользователем, ранее вошедшим в систему.|
|**userToken**|**UserToken**|Требуется, только если для свойства _identity_ задано значение `userFromToken`. Токен, предназначенный для приложения-функции. |
|**action**|**Действие**|Обязательное. Указывает действие, которое необходимо выполнить с помощью привязки. Может иметь одно из следующих значений:<ul><li><code>create</code> — регистрация новой подписки.</li><li><code>delete</code> — удаление указанной подписки.</li><li><code>refresh</code> — обновление указанной подписки. Позволяет предотвратить истечение срока действия.</li></ul>|
|**subscriptionResource**|**субскриптионресаурце**|Требуется, только если для свойства _action_ задано значение `create`. Указывает ресурс Microsoft Graph, изменения которого будут отслеживаться. Дополнительные сведения см. в статье [Работа с веб-перехватчиками в Microsoft Graph]. |
