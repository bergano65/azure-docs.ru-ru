...
Независимо от используемого средства необходимо создать представление JSON для объекта панели мониторинга. Этот объект содержит сведения о плитках на панели мониторинга. Сюда входят размеры, позиции, ресурсы, к которым они привязаны, и любые пользовательские настройки.
...
После настройки панели мониторинга выполните следующие действия, чтобы опубликовать панель мониторинга с помощью команды **Share** , а затем использовать обозреватель ресурсов для выборки JSON.
...
Публикация занимает несколько секунд.  После этого следующий шаг — переход к [Обозреватель ресурсов](https://portal.azure.com/#blade/HubsExtension/ArmExplorerBlade) для выборки JSON.

![Обзор обозреватель ресурсов](./media/azure-portal-dashboards-create-programmatically/search-resource-explorer.png)

В обозреватель ресурсов перейдите к выбранной подписке и группе ресурсов. Затем выберите только что опубликованный ресурс панели мониторинга для отображения JSON.

![Просмотр JSON в обозреватель ресурсов](./media/azure-portal-dashboards-create-programmatically/resource-explorer-json-detail.png)
...
Azure предлагает возможность оркестрации развертывания нескольких ресурсов. Создайте шаблон развертывания, который выражает набор развертываемых ресурсов и связи между ними.  Для каждого ресурса используется единый формат JSON, как и при их последовательном создании. Разница заключается в том, что язык шаблона добавляет несколько концепций, таких как переменные, параметры, основные функции и многое другое. Этот расширенный синтаксис поддерживается только в контексте развертывания шаблона. Он не работает при использовании с императивными API, которые обсуждались ранее. Дополнительные сведения см. [в разделе сведения о структуре и синтаксисе шаблонов Azure Resource Manager](../azure-resource-manager/resource-group-authoring-templates.md).
...
Полный рабочий шаблон можно увидеть в конце этого документа.
...
Мы представляем две версии примера панели мониторинга в формате JSON. Первая — версия, экспортированная с портала, которая уже была привязана к ресурсу. Вторая — это версия шаблона, которая может быть программно привязана к любой виртуальной машине и развернута с помощью Azure Resource Manager.
