...
description: Пройдите через структуру JSON панели мониторинга Azure с помощью примерной панели мониторинга. Включает ссылку на свойства ресурсов.
...
Идентификатор ресурса Azure, в соответствии с [конвенциями именования ресурсов Azure.](/azure/architecture/best-practices/resource-naming) Когда портал создает панель мониторинга, он обычно выбирает идентификатор в виде гида, но при создании их программно вы можете использовать любое действительное имя. 

### <a name="the-name-property"></a>Свойство name
Имя — это сегмент идентификатора ресурсов, который не включает информацию о подписке, типе ресурса или группе ресурсов. По сути, это последний сегмент идентификатора ресурса.
...
Объект свойств содержит два свойства: __lenses__ и __metadata__. Свойство __линз__ содержит информацию о плитках на приборной панели.  Свойство __metadata__ предназначено для потенциальных будущих возможностей.
...
Объект 0 содержит два свойства — __order__ и __parts__.  В текущей версии панелей мониторинга __order__ всегда равен 0. Свойство __частей__ содержит объект, определяющий отдельные части (также называемые плитками) на панели мониторинга.
...
Объект inputs обычно содержит сведения, которые привязывают плитку к экземпляру ресурса.  Виртуальная часть машины в нашей панели мониторинга образца содержит один вход, который использует идентификатор ресурса Azure для выражения привязки.  Этот идентификатор ресурсов согласован во всех ресурсах Azure.
...
Для плиток, связанных с управляемыми объектами портала первого класса (ресурсами), связь выражается через объект asset.  В нашем примере панели мониторинга плитка виртуальной машины содержит такое описание ресурса.  Свойство __idInputName__ сообщает порталу, что ввод идентификатора содержит уникальный идентификатор для актива, в данном случае идентификатор ресурса. Для большинства типов ресурсов Azure ресурсы определены на портале.
