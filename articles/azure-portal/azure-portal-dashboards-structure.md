...
Для плиток, связанных с управляемыми объектами портала первого класса (ресурсами), связь выражается через объект asset.  В нашем примере панели мониторинга плитка виртуальной машины содержит такое описание ресурса.  Свойство __идинпутнаме__ сообщает порталу, что входные данные идентификатора содержат уникальный идентификатор для ресурса, в данном случае идентификатор ресурсов. В большинстве типов ресурсов Azure ресурсы определены на портале.
