...
description: Настройка и использование эмулятора хранилища — утилиты, имитирующей службы хранения данных Blob, Queue и Table, доступные в Azure на локальной машине разработки.
...
Можно настроить автоматический запуск эмулятора хранения в Visual Studio при запуске или отладке службы в Visual Studio. В обозревателе решений откройте контекстное меню своего проекта **Azure** и выберите **Свойства**. На вкладке **Разработка** в списке **Запуск эмулятора хранения Azure** выберите **True** (если это значение еще не задано).  Некоторые типы проектов не имеют вкладки **«Разработка».** Если это так, можно включить или отключить запуск эмулятора хранилища, `StartDevelopmentStorage` установив элемент в файле проекта. Установите его на **Истину,** чтобы включить его, или **ложно,** чтобы отключить его.  Например, в проекте Azure Functions откройте файл проекта для редактирования и измените код XML следующим образом:
