...
description: В этой статье представлен концептуальный обзор потока аттестации TPM с помощью службы обеспечения устройств IoT (DPS).
author: nberdy
ms.author: nberdy
ms.date: 04/04/2019
ms.topic: conceptual
...
Эта статья относится только к устройствам, в которых используется TPM 2.0 с поддержкой ключа HMAC, и их ключам подтверждения. Она не предназначена для устройств, в которых используются сертификаты X.509 для аутентификации. TPM является отраслевым стандартом ISO от Trusted Computing Group, и вы можете прочитать больше о TPM на [полной спецификации TPM 2.0](https://trustedcomputinggroup.org/tpm-library-specification/) или [спецификации ISO/IEC 11889](https://www.iso.org/standard/66510.html). В этой статье также предполагается, что вы знакомы с публичными и частными парами ключей, а также с тем, как они используются для шифрования.
