...
description: В этой статье приводятся общие сведения о потоке аттестации доверенного платформенного модуля с помощью службы подготовки устройств Интернета вещей (DPS).
author: nberdy
ms.author: nberdy
ms.date: 04/04/2019
ms.topic: conceptual
...
В этой статье описывается процесс подтверждения личности при использовании доверенный платформенный модуль (TPM) (TPM). TPM — это тип аппаратного модуля безопасности (HSM). В этой статье предполагается, что используется дискретный, микропрограммный или интегрированный модуль TPM. Эмулированные программными средствами модули TPM хорошо подходят для прототипирования или тестирования. Но они не обеспечивают такой же уровень безопасности, как дискретные, микропрограммные или интегрированные TPM. Мы не рекомендуем использовать программные модули TPM в рабочей среде. Дополнительные сведения о типах доверенных платформенных модулей см. в статье [A Brief Introduction to TPM](https://trustedcomputinggroup.org/wp-content/uploads/TPM-2.0-A-Brief-Introduction.pdf) (Краткое введение в доверенный платформенный модуль).

Эта статья относится только к устройствам, в которых используется TPM 2.0 с поддержкой ключа HMAC, и их ключам подтверждения. Она не предназначена для устройств, в которых используются сертификаты X.509 для аутентификации. TPM — это общедоступный стандарт ISO из организация TCG, и вы можете ознакомиться с дополнительными сведениями о TPM по [полной спецификации tpm 2,0](https://trustedcomputinggroup.org/tpm-library-specification/) или [спецификации ISO/IEC 11889](https://www.iso.org/standard/66510.html). В этой статье также предполагается, что вы знакомы с парой открытого и закрытого ключей и как они используются для шифрования.
