...
description: Описывает концепции предоставления услуг, специфичных для устройств с Службой обеспечения устройств (DPS) и IoT Hub
author: nberdy
ms.author: nberdy
ms.date: 09/18/2019
ms.topic: conceptual
...
Это группа устройств, совместно использующих определенный механизм аттестации. Группы регистрации поддерживают как X.509, так и симметричные. Все устройства в группе регистрации X.509 представляют сертификаты X.509, подписанные тем же корневым или промежуточным органом по сертификации (CA). Каждое устройство в группе симметричных ключевых моментов представления токенов SAS, полученных из симметричного ключа группы. Имя группы регистрации и сертификата должно состоять из букв и цифр нижнего регистра и может содержать дефисы.
