---
title: Служба обеспечения устройств Azure IoT - Концепции безопасности
description: Описывает концепции обеспечения безопасности, характерные для устройств с службой обеспечения устройств (DPS) и IoT Hub
author: nberdy
ms.author: nberdy
ms.date: 04/04/2019
ms.topic: conceptual
...
* **Симметричный ключ,** основанный на [маркерах безопасности](../iot-hub/iot-hub-devguide-security.md#security-tokens)общей подписи доступа (SAS), которые включают хэшированную подпись и встроенный срок действия. Дополнительные сведения см. в разделе [Аттестация симметричного ключа](concepts-symmetric-key-attestation.md).
...
Сертификаты листа, используемые при [индивидуальной](./concepts-service.md#individual-enrollment) регистрации, имеют требование о том, что **имя субъекта** должно быть установлено в идентификаторе регистрации индивидуальной регистрации. Сертификаты листа, используемые при входе в [группу регистрации,](./concepts-service.md#enrollment-group) должны иметь набор идентификатора **имени объекта** для нужного идентификатора устройства, который будет отображаться в регистрационных **отчетах** для проверенного устройства в группе регистрации.
