...
1. Следуя инструкциям статьи [Регистрация открытой части сертификата X.509 и получение кода проверки](how-to-verify-certificates.md#register-the-public-part-of-an-x509-certificate-and-get-a-verification-code), отправьте корневой сертификат и получите код проверки от DPS.
...
2. Получив от DPS код проверки для корневого сертификата, выполните следующую команду из рабочего каталога скриптов сертификата, чтобы создать сертификат проверки.
    ...
    Этот скрипт создает сертификат, подписанный корневым сертификатом с кодом проверки в качестве имени субъекта. Этот сертификат позволяет DPS проверить наличие доступа к закрытому ключу корневого сертификата. Обратите внимание на расположение сертификата проверки в выходных данных скрипта.
