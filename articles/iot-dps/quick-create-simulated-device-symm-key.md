---
title: Краткое руководство по подготовке устройств с использованием симметричного ключа в Центре Интернета вещей Azure с помощью C
description: В этом кратком руководстве описано, как с помощью пакета SDK C для устройств подготовить устройство, которое использует симметричный ключ, в Службе подготовки устройств к добавлению в Центр Интернета вещей Azure (DPS).
author: wesmc7777
ms.author: wesmc
ms.date: 01/14/2020
ms.topic: quickstart
...
ms.locfileid: "96746511"
---
# <a name="quickstart-provision-a-device-with-symmetric-keys"></a>Краткое руководство. Подготовка устройства с использованием симметричных ключей

Из этого краткого руководства вы узнаете, как запустить код подготовки устройства на компьютере для разработки с ОС Windows, чтобы подключить его к Центру Интернета вещей в качестве устройства Интернета вещей. Вы настроите в устройстве использование симметричного ключа для прохождения аутентификации в экземпляре службы подготовки устройств и назначения Центру Интернета вещей. Для подготовки устройства будет использоваться пример кода из [пакета SDK для Интернета вещей Azure](https://github.com/Azure/azure-iot-sdk-c). Устройство будет распознано по отдельной регистрации в экземпляре службы подготовки и назначено Центру Интернета вещей.
...
Если вы не знакомы с процессом автоматической подготовки, см. раздел [Роли и учетные записи Azure](about-iot-dps.md#provisioning-process). 
...
В пакет SDK входит пример кода подготовки для устройств. Этот код предпримет попытку подготовки устройства во время последовательности его загрузки.
...
5. Выполните приведенную ниже команду, чтобы создать версию пакета SDK для используемой клиентской платформы разработки. В каталоге `cmake` будет создано решение Visual Studio для кода подготовки устройств. 
...
## <a name="run-the-provisioning-code-for-the-device"></a>Выполнение кода подготовки для устройства
...
8. В меню Visual Studio выберите **Отладка** > **Запуск без отладки**, чтобы запустить решение. При появлении запроса на перестройку проекта щелкните **Да**, чтобы перестроить его перед запуском.
...
9. На портале перейдите в Центр Интернета вещей, которому назначено устройство, и щелкните вкладку **Устройства Интернета вещей**. Когда устройство будет успешно подготовлено для центра, в колонке **Устройства Интернета вещей** появится его код со значением **Включено** в столбце *Состояние*. Возможно, вам потребуется нажать кнопку **Обновить** в верхней области. 
...
Из этого краткого руководства вы узнали, как запустить код подготовки устройства на компьютере Windows.  Устройство прошло проверку подлинности и подготовлено в центре Интернета вещей с использованием симметричного ключа. Чтобы узнать, как подготавливать устройства с сертификатами X.509, перейдите к краткому руководству по устройствам X.509. 

> [!div class="nextstepaction"]
> [Краткое руководство Azure по подготовке устройства X.509 с помощью пакета SDK C для Интернета вещей Azure](quick-create-simulated-device-x509.md)