...
3. В разделе **Add enrollment** (Добавить регистрацию) введите сведения ниже, а затем нажмите кнопку **Сохранить**.
    ...
    - **Ключ подтверждения**. Введите *ключ подтверждения*, созданный для устройства TPM путем запуска проекта *tpm_device_provision*.
    - **Идентификатор регистрации**. Введите *идентификатор регистрации*, созданный для устройства TPM путем запуска проекта *tpm_device_provision*.
    - **Устройство IoT Edge**. Выберите **Отключить**.
...
> [Краткое руководство по Azure. Регистрация устройства доверенного платформенного модуля (TPM) в Службе подготовки устройств к добавлению в Центр Интернета вещей](quick-enroll-device-tpm-java.md)
