---
title: Используйте различные механизмы аттестации с помощью службы обеспечения устройств устройств Azure IoT
description: Azure Как - Как использовать различные механизмы аттестации с службой обеспечения устройств (DPS) Клиент SDK в Azure
author: robinsh
ms.author: robinsh
ms.date: 03/30/2018
ms.topic: conceptual
...
Служба обеспечения поставляется с эмулятором device Identity Composition Engine (DICE), который генерирует сертификат **X.509** для проверки подлинности устройства. Чтобы включить проверку подлинности **X.509,** запустите следующую команду: 
...
Услуга обеспечения может быть использована с **X.509** на другом оборудовании. Для обмена данными между оборудованием и пакетом SDK требуется интерфейс. Сведения об интерфейсе можно получить у производителя HSM.
...
- Есть много [вариантов конфигурации CMake,](https://cmake.org/cmake/help/v3.6/manual/cmake.1.html) доступных для создания SDK. Например, можно отключить один из доступных стеков протоколов, добавив аргумент в команду создания проекта CMake:
    ```
    cmake -Duse_amqp=OFF ..
    ```
...
2. Войдите на портал Azure, нажмите кнопку **Все ресурсы** в меню слева и откройте службу подготовки устройств.
   - **X.509 Индивидуальная регистрация**: На сводке резюме службы, выберите **Управление регистрацией.** Выберите вкладку **«Индивидуальные регистрации»** и нажмите кнопку **«Добавить»** вверху. Выберите **X.509** в качестве *механизма*удостоверения личности, загрузите сертификат листа, как того требует лезвие. Затем нажмите кнопку **Сохранить**. 
   - **X.509 Группа Регистрация**: На предоставление услуг резюме лезвие, выберите **Управление регистрации**. Выберите вкладку **Групповые регистрации** и нажмите кнопку **Добавить** вверху. Выберите **X.509** в качестве *механизма*подтверждения идентификации, введите название группы и имя сертификации, загрузите сертификат CA/Intermediate в соответствии с требованиями лезвия. Затем нажмите кнопку **Сохранить**. 
...
После того, как устройство было подготовлено службой обеспечения, этот API использует указанный режим аутентификации **(X.509** или TPM) для подключения к IoT Hub: 
  ```
