---
title: Предоставление устаревших устройств с использованием симметричных ключей - Служба обеспечения устройств Azure IoT
description: Как использовать симметричные ключи для предоставления устаревших устройств в экземпляре Службы обеспечения устройств (DPS)
author: wesmc7777
ms.author: wesmc
ms.date: 04/10/2019
ms.topic: conceptual
...
> [!NOTE]
> Образец, используемый в этой статье, написан на c. Существует также [устройство, подготовленное с симметричным образцом ключей.](https://github.com/Azure-Samples/azure-iot-samples-csharp/tree/master/provisioning/Samples/device/SymmetricKeySample) Чтобы использовать этот образец, загрузите или клоните репозиторий [azure-iot-ssharp-csharp](https://github.com/Azure-Samples/azure-iot-samples-csharp) и следуйте инструкциям в строке кода. Вы можете следовать инструкциям в этой статье, чтобы создать симметричную группу регистрации ключей с помощью портала и найти область идентификатора и первичные и вторичные ключи группы регистрации, необходимые для выполнения образца. Вы также можете создавать индивидуальные регистрации с помощью образца.
...
8. В меню Visual Studio выберите **Отек** > **Start без отладки** для запуска решения. При появлении запроса перестроить проект щелкните **Да**, чтобы перестроить его перед запуском.
...
9. На портале перейдите к концентратору IoT, к которым было назначено имитированное устройство, и нажмите на вкладку **IoT Devices.** При успешном подготовке смоделированного концентратора идентификатор устройства отображается на лезвии **IoT Devices** с **включенным** *STATUS.* Возможно, вам потребуется нажать кнопку **Обновить** в верхней области. 
