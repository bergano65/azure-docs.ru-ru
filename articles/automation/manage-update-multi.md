...
- Доступ к репозиторию обновления для Linux VMs на борту решения.
...
- **Без оценки** — данные оценки обновления не были получены от компьютера в течение ожидаемого отрезка времени. Для компьютеров Linux, ожидаемый таймфрейм находится в последний час. Для компьютеров Windows это последние 12 часов.
...
- **Не пытался**: Обновление не было установлено, потому что недостаточное время было доступно на основе определенного окна обслуживания.
- **Успешно**: Обновление удалось.
- **Не удалось**: Обновление не удалось.
