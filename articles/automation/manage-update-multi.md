...
description: В этой статье описывается, как управлять обновлениями для виртуальных машин Azure и non-Azure.
services: automation
...
Управление обновлениями Azure Automation можно использовать для управления обновлениями и исправлениями для виртуальных компьютеров Windows и Linux. В учетной записи [службы автоматизации Azure](automation-offering-get-started.md) вы можете сделать следующее:

- подключить виртуальные машины;
- оценить состояние доступных обновлений;
- запланировать установку необходимых обновлений;
- Просмотрите результаты развертывания, чтобы убедиться, что обновления были успешно применены ко всем виртуальным машинам, для которых включено управление обновлением.

Чтобы узнать о системных требованиях к управлению обновлениями, [см.](automation-update-management.md#clients)

## <a name="prerequisites"></a>Предварительные требования

* Виртуальная машина или компьютер, где установлена одна из поддерживаемых операционных систем.
* Доступ к репозиторию обновлений для Linux VMs на борту для управления обновлениями.
...
Агент Log Analytics для Windows и Linux должен быть установлен на вдыхать на вашей корпоративной сети или другой облачной среде, чтобы включить их в управление обновлением. Чтобы узнать системные требования и поддерживаемые методы развертывания агента [Overview of the Log Analytics agent](../azure-monitor/platform/log-analytics-agent.md)в машинах, размещенных за пределами Azure, см.
...
Если управление обновлениями недавно включено для компьютеров, возможно, они еще не оценивались. Состояние соответствия для `Not assessed`этих компьютеров . Вот список возможных значений состояния соответствия:

- `Compliant`: Компьютеры, которые не пропускают критические или обновления безопасности.
- `Non-compliant`: Компьютеры, которые отсутствуют по крайней мере одно критическое или обновление безопасности.
- `Not assessed`: Данные оценки обновления не были получены с компьютера в течение ожидаемого периода времени. Для компьютеров Linux ожидаемый таймфрейм – последний час. Для компьютеров Windows ожидаемый таймфрейм составляет последние 12 часов.

Чтобы просмотреть состояние агента, выберите ссылку в столбце **готовности агента обновления.** Откроется область гибридной рабочей роли со сведениями о состоянии этой роли. Ниже приведен пример агента, который долгое время не был подключен к решению "Управление обновлениями".
...
После включения управления обновлением открывается панель управления обновлением. На вкладке **Отсутствующие обновления** можно просмотреть список отсутствующих обновлений.
...
После того, как компьютер завершает сканирование для соответствия обновлению, агент перенаправляет информацию оптом в журналы Azure Monitor. На компьютере с Windows проверка на соответствие по умолчанию выполняется каждые 12 часов.
...
Для компьютера Linux сканирование соответствия выполняется каждый час по умолчанию. При перезапуске агента MMA проверка соответствия инициируется в течение 15 минут.
...
- **Группы для обновления:** Определите запрос на основе комбинации подписок, групп ресурсов, местоположений и тегов для создания динамической группы Виртуальных групп Azure для включения в развертывание. Для не-Azure VMs сохраненные поиски используются для создания динамической группы для включения в развертывание. Чтобы узнать больше см., [Динамические группы](automation-update-management-groups.md).
- **Компьютеры, на которые нужно установить обновления**. Выберите "Сохраненные поисковые запросы", группу "Импортировано" или выберите "Компьютеры", чтобы выбрать компьютеры, которые требуется обновить.
   ...
   >[!NOTE]
   >Выбор опции Сохраненный поиск не возвращает идентификаторы машины, только их имена. Если у вас есть несколько ВМ с одинаковым именем в нескольких группах ресурсов, они возвращаются в результатах. Использование **групп для обновления** опции рекомендуется, чтобы убедиться, что вы включаете уникальные виртуальные, соответствующие вашим критериям.

   Если вы выбираете **машины,** готовность машины отображается в колонке **готовности агента обновления.** Вы можете просмотреть состояние работоспособности компьютера перед планированием развертывания обновлений. Чтобы узнать о различных методах создания компьютерных групп [Computer groups in Azure Monitor logs](../azure-monitor/platform/computer-groups.md) в журналах Azure Monitor, см.

  ![Область "Новое развертывание обновления"](./media/manage-update-multi/update-select-computers.png)

- **Классификации обновлений**. Выберите типы программного обеспечения, которые будут включены в развертывание обновления. Для описания типов классификации [см.](automation-view-update-assessments.md#update-classifications) Ниже приведены типы классификации:
...
- **Обновления для включения или исключения**. После этого откроется страница включения и исключения. Обновления, которые можно включить или исключить, находятся на отдельных вкладках. Для получения дополнительной информации о том, как обрабатывается включение, [см.](automation-tutorial-update-management.md#schedule-an-update-deployment)
...
> [!NOTE]
> Нельзя указывать обновления, которые были заменены для включения в развертывание обновления.
>
...
- `Not attempted`: Обновление не было установлено из-за недостаточного времени, основанного на определенном окне обслуживания.
- `Succeeded`: Обновление удалось.
...
Чтобы узнать больше о журналах управления обновлением, выводах и ошибках, смотрите [записи обновления запроса для управления обновлением.](automation-update-management-query-logs.md)
