...
| `Credential` | Необязательный | Учетные данные, используемые для входа в среду Azure. |
...
| `TenantID` | Необязательный | Идентификатор организации клиента, связанной с учетной записью службы автоматизации. |
...
В оболочке командной строки PowerShell перейдите к папке, содержащей скачанный скрипт. Измените значения для параметров `AutomationAccountName`, `AAResourceGroupName` `OMSResourceGroupName` `HybridGroupName` `SubscriptionID`,,, и. `WorkspaceName` Затем выполните сценарий.
...
2. Убедитесь, что агент правильно скачал решение службы автоматизации. 
...
При настройке агента для отправки отчетов в Log Analytics рабочую область решение службы автоматизации отправляет модуль `HybridRegistration` PowerShell, который содержит `Add-HybridRunbookWorker` командлет. Используйте этот командлет для установки среды Runbook на компьютере и его регистрации в службе автоматизации Azure.
...
Теперь выполните `Add-HybridRunbookWorker` командлет, используя следующий синтаксис.

```powershell-interactive
Add-HybridRunbookWorker –GroupName <String> -EndPoint <Url> -Token <String>
```

Данные, необходимые для этого командлета, можно найти на странице Управление ключами на портале Azure. Откройте эту страницу, выбрав **ключи** на странице "Параметры" в учетной записи службы автоматизации.

![Страница «Управление ключами»](media/automation-hybrid-runbook-worker/elements-panel-keys.png)

* В `GroupName` качестве параметра используйте имя группы гибридных рабочих ролей Runbook. Если эта группа уже существует в учетной записи службы автоматизации, то к ней добавляется текущий компьютер. Если эта группа не существует, она добавляется.
* Для `EndPoint` параметра используйте запись **URL-адреса** на странице Управление ключами.
* Для `Token` параметра используйте запись **первичный ключ доступа** на странице Управление ключами.
...
Поскольку основной целью гибридной рабочей роли Runbook является управление локальными ресурсами, скорее всего, потребуется установить модули, поддерживающие эти ресурсы, в частности `PowerShellGet` модуль. Сведения об установке модулей Windows PowerShell см. в разделе [Windows PowerShell](https://docs.microsoft.com/powershell/scripting/developer/windows-powershell).

Установленные модули должны находиться в расположении, на которое ссылается переменная `PSModulePath` среды, чтобы Гибридная Рабочая роль могла автоматически импортировать их. Дополнительные сведения см. [в разделе Install modules in PSModulePath](https://docs.microsoft.com/powershell/scripting/developer/module/installing-a-powershell-module?view=powershell-7).
