...
### <a name="server-onboarding-for-management-with-state-configuration-dsc"></a>Подключение сервера для управления с помощью конфигурации состояния (DSC)

Сведения о подключении серверов для управления с помощью настройки состояния (DSC) см. в статье подключение [компьютеров для управления с помощью настройки состояния (DSC)](automation-dsc-onboarding.md).

Включение [Управление обновлениями](automation-update-management.md) автоматически настраивает любой компьютер Windows, подключенный к рабочей области log Analytics, как гибридную рабочую роль Runbook для поддержки обновлений Runbook. Однако этот рабочий процесс не зарегистрирован ни в одной группе гибридных рабочих ролей Runbook, которая уже определена в учетной записи службы автоматизации.
...
Рабочий компьютер можно добавить в группу гибридных рабочих ролей Runbook в учетной записи службы автоматизации. Обратите внимание, что необходимо поддерживать модули Runbook службы автоматизации, если вы используете одну и ту же учетную запись как для функции службы автоматизации Azure, так и для членства в группе гибридной рабочей роли Runbook. Эта функция добавлена в версии 7.2.12024.0 гибридной рабочей роли Runbook.
...
### <a name="step-2---add-an-azure-automation-feature-to-the-log-analytics-workspace"></a>Шаг 2. Добавление функции службы автоматизации Azure в рабочую область Log Analytics

Функция автоматизации добавляет функции службы автоматизации Azure, включая поддержку гибридной рабочей роли Runbook. При добавлении решения в рабочую область Log Analytics оно автоматически отправляет на компьютер агента рабочие компоненты, которые вы устанавливаете, как описано на следующем шаге.
...
3. Откройте сеанс PowerShell в режиме администратора и выполните следующую команду со значениями URL-адреса и первичного ключа доступа. Используйте `Verbose` параметр для подробного журнала процесса удаления. Чтобы удалить устаревшие машины из группы гибридной рабочей роли, используйте необязательный параметр `machineName`.
...
Чтобы удалить группу гибридных рабочих ролей Runbook, сначала необходимо удалить гибридную рабочую роль Runbook с каждого компьютера, который является членом группы. Затем выполните следующие действия, чтобы удалить группу.

1. На портале Azure откройте учетную запись службы автоматизации.
2. Выберите **группы гибридных рабочих ролей** в разделе **Автоматизация процессов**. Выберите группу, которую нужно удалить. Откроется страница свойств этой группы.
