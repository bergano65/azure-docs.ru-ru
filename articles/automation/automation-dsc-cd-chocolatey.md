...
На самом деле он хранится дважды: один раз в виде обычного текста и после компиляции в виде MOF-файла (для тех, кто знаком с этими объектами). На портале MOF — это "Конфигурация узла" (а не просто "Конфигурация"). Это связанный с узлом артефакт, сообщающий узлу его конфигурацию. Ниже объясняется, как назначить узлу конфигурацию узла.
...
Некоторые параметры вам придется настроить самостоятельно. Но это не очень сложно, и оно выполняется только один раз для каждого ресурса (если вы не хотите обновлять его в будущем). Дополнительные сведения о создании модулей интеграции PowerShell см. в этой статье: [Создание модулей интеграции для службы автоматизации Azure](https://azure.microsoft.com/blog/authoring-integration-modules-for-azure-automation/)
