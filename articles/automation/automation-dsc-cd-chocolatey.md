...
Начиная с самого верха, вы пишете свой код, строите его, тестируете его, а затем создаете пакет установки. Chocolatey может обрабатывать пакеты установки различных типов, например MSI, MSU и ZIP. И у вас есть полная мощность PowerShell, чтобы сделать фактическую установку, если родные возможности Chocolatey не до него. Поместите пакет в какое-то место, достижимое - хранилище пакетов. Он может располагаться где угодно. Например, в этом примере используется общая папка в учетной записи хранилища больших двоичных объектов Azure. Chocolatey работает непосредственно с серверами NuGet и некоторыми другими серверами в рамках управления метаданными пакета. Возможные варианты описаны в [этой статье](https://github.com/chocolatey/choco/wiki/How-To-Host-Feed). Пример использования использует NuGet. Nuspec — это метаданные о пакетах. Информация Nuspec компилируется в NuPkg и хранится на сервере NuGet. Когда конфигурация запрашивает пакет по имени и ссылается на сервер NuGet, ресурс Chocolatey DSC на VM захватывает пакет и устанавливает его. Можно также запросить определенную версию пакета.

В левом нижнем углу изображения есть шаблон управления ресурсами Azure. В этом примере использования расширение VM регистрирует VM с сервером Azure Automation State Configuration в качестве узла. Конфигурация хранится на сервере тяги дважды: один раз как простой текст и один раз компилируется как файл MOF. На портале Azure MOF представляет собой конфигурацию узлов, в отличие от простой конфигурации. Это артефакт, связанный с узлам, чтобы узла знала его конфигурацию. Ниже объясняется, как назначить узлу конфигурацию узла.

Создание Nuspec, компиляция и хранение на сервере NuGet — это мелочь. Кроме того, вы уже управляете виртуальными машинами. 

Для непрерывного развертывания требуется один раз настройка сервера вытягивания, регистрация узлов с ним один раз и создание и хранение начальной конфигурации на сервере. По мере обновления и развертывания пакетов в репозитории необходимо обновить конфигурацию и конфигурацию узлов на сервере вытягивания.

Если вы не начинаете с шаблона «Менеджер ресурсов», это нормально. Есть команды PowerShell, которые помогут вам зарегистрировать ваши VMs с сервером тяги. Для получения дополнительной информации см. [Onboarding машины для управления Azure Automation State Configuration](automation-dsc-onboarding.md).
...
1. Установка [платформы управления Windows v5](https://aka.ms/wmf5latest) (не требуется для Windows 10).

2. Установка интеграционного модуля.

    ```azurepowershell-interactive
    Install-Module –Name MODULE-NAME`    <—grabs the module from the PowerShell Gallery
    ```

3. Копируйте папку модуля из **c: »Программные файлы »WindowsPowerShell»Modules-MODULE-NAME** на временную папку.

4. Удалить образцы и документацию из основной папки.

5. Застегните основную папку, назвав файл с именем папки.

6. Поместите файл «ЗИП» в доступное место http, например хранилище кабр в учетной записи хранения Azure.
...
Вот скрипт **New-ConfigurationScript.ps1** (изменен для использования модуля Az):
...
Эти шаги приводят к тому, что на сервере притяжения помещается новая конфигурация узлов под названием **ISVBoxConfig.isvbox.** Имя конфигурации узла `configurationName.nodeName`построено как.
