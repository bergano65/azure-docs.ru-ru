...
Все модули Runbook в службе автоматизации Azure являются рабочими процессами Windows PowerShell. Графические модули Runbook и графические модули Runbook рабочих процессов PowerShell создают код PowerShell, который выполняется, но их нельзя просматривать или изменять. Графический модуль Runbook можно преобразовать в графический модуль Runbook рабочего процесса PowerShell и наоборот. Однако эти модули Runbook нельзя преобразовать в текстовый модуль Runbook. Кроме того, графический редактор автоматизации не может импортировать существующий текстовый Runbook.
...
Элемент управления Canvas позволяет проектировать модуль Runbook. Вы можете добавить действия из узлов в элемент управления "Библиотека" в модуль Runbook и соединить их со ссылками, чтобы определить логику модуля Runbook. В нижней части холста есть элементы управления, позволяющие увеличить и уменьшить масштаб.
...
Набор параметров определяет обязательные и необязательные параметры, которые принимают значения для определенного командлета. Все командлеты имеют по крайней мере один набор параметров, а некоторые имеют несколько наборов. Если командлет имеет несколько наборов параметров, необходимо выбрать, какой из них следует использовать, прежде чем можно будет настроить параметры. Набор параметров, используемый действием, можно изменить, выбрав **набор параметров** и выбрав другой набор. В этом случае все значения параметров, которые уже настроены, теряются.
...
Например, следующий код выполняет вычисление дат с помощью входной переменной runbook — $NumberOfDays. Затем вычисленное значение DateTime отправляется в качестве выходных данных для использования последующими действиями в модуле Runbook.
...
При указании условия для ссылки действие назначения выполняется только в том случае, если условие разрешается в значение true. В условии обычно используют переменную $ActivityOutput для получения выходных данных исходного действия.
...
Второй механизм доступа к данным извлекает выходные данные действия в источнике данных выражения PowerShell или из действия сценария рабочего процесса с переменной ActivityOutput, используя приведенный ниже синтаксис. Если выходным объектом является объект, модуль Runbook может указать одно свойство.
...
При создании модуля Runbook можно получить доступ к его входным параметрам, нажав кнопку **Вход и выход** на панели инструментов Runbook. Откроется элемент управления **Вход и выход**, в котором можно изменить существующий входной параметр или создать новый, нажав кнопку **Добавить входные данные**.
...
| Свойство | Description |
|:--- |:--- |
| Имя |Уникальное имя параметра. Это имя может содержать только буквенно-цифровые символы и не может содержать пробелы. |
| Description |Необязательное описание входного параметра. |
...
| Обязательный |Параметр, указывающий, должно ли быть указано значение для параметра. Запустить Runbook не удастся, если не указать значение для каждого обязательного параметра, у которого нет определенного значения по умолчанию. |
| Значение по умолчанию |Значение, используемое для параметра, если оно не указано. Присвойте этому параметру значение null или конкретное значение. |
...
Следующие команды создают строку из текущей даты и присваивают ее переменной. Код отправляет содержимое переменной в выходные данные.
...
Вы можете объединить несколько условий в модуле Runbook с помощью [логического оператора](https://technet.microsoft.com/library/hh847789.aspx), такого как **-и** или **-или**. Например, следующее условие проверяет, находится ли виртуальная машина в предыдущем примере в состоянии "остановлена" или "остановлена".
...
В службе автоматизации Azure для модулей runbook, которые управляют ресурсами Azure, требуется проверка подлинности. [Учетная запись запуска от имени](automation-create-runas-account.md), также называемая субъектом-службой, является механизмом по умолчанию, который Runbook службы автоматизации использует для доступа к ресурсам Azure Resource Manager в вашей подписке. Эту функцию можно добавить в графический модуль Runbook, добавив ресурс подключения AzureRunAsConnection, использующий командлет PowerShell [Get-AutomationConnection](https://technet.microsoft.com/library/dn919922%28v=sc.16%29.aspx) , и командлет [Connect-азаккаунт](/powershell/module/az.profile/connect-azaccount) на холст. Этот сценарий показан в следующем примере.
