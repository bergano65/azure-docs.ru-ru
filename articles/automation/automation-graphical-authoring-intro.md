...
Для всех командлетов можно предоставить дополнительные параметры. Это общие параметры PowerShell или другие пользовательские параметры. Графический редактор представляет текстовое поле, в котором можно указать параметры с помощью синтаксиса PowerShell. Например, чтобы использовать `Verbose` общий параметр, необходимо указать. `-Verbose:$True`
...
Например, следующий код выполняет вычисления с датами, используя входную переменную Runbook `NumberOfDays`с именем. Затем вычисленное значение DateTime отправляется в качестве выходных данных для использования последующими действиями в модуле Runbook.
...
При указании условия для ссылки действие назначения выполняется только в том случае, если условие разрешается в значение true. Обычно `ActivityOutput` переменная используется в условии для получения выходных данных от исходного действия.

Для связи конвейера необходимо указать условие для одного объекта. Модуль Runbook оценивает условие для каждого выходного объекта в исходном действии. Затем выполняется целевое действие для каждого объекта, удовлетворяющего условию. Например, с помощью исходного действия можно использовать `Get-AzVM`следующий синтаксис для связи условного конвейера, чтобы получить только виртуальные машины в группе ресурсов с именем group1.
...
Модуль Runbook использует три различные последовательные ссылки, которые проверяют значения `VMName` входных `ResourceGroupName` параметров и определяют необходимое действие. Возможные действия: запуск одной виртуальной машины, запуск всех виртуальных машин в группе ресурсов или запуск всех виртуальных машин в подписке. Для связи Sequence между `Connect to Azure` и `Get single VM`ниже приведена логика условия.
...
Например, `Start-AzVM` действие в модуле Runbook, указанное ниже, запускает все виртуальные машины. Действие включает две условные связи. Первая условная ссылка использует выражение `$ActivityOutput['Start-AzVM'].IsSuccessStatusCode -eq $true` для фильтрации при успешном `Start-AzVM` завершении действия. Вторая условная ссылка использует выражение `$ActivityOutput['Start-AzVM'].IsSuccessStatusCode -ne $true` для фильтрации, если `Start-AzVm` действию не удается запустить виртуальную машину.

![Пример условной связи](media/automation-graphical-authoring-intro/runbook-conditional-links.png)

Любое действие, следующее за первой ссылкой и использующая выходные данные `Get-AzureVM` действия, получает только те виртуальные машины, которые были запущены `Get-AzureVM` в момент запуска. Любое действие, следующее за второй ссылкой, получает только те виртуальные машины, которые были остановлены в `Get-AzureVM` момент запуска. Действие, следующее по третьей связи, получит все виртуальные машины вне зависимости от того, запущены они или нет.
...
То, как данные записываются в шину данных, зависит от типа связи действия. Для связи конвейера данные выводятся в виде нескольких объектов. Для последовательной связи данные выводятся в виде массива. Если имеется только одно значение, оно выводится как одноэлементный массив.
...
Второй механизм доступа к данным извлекает выходные данные действия в источнике данных выражения PowerShell или действие сценария рабочего процесса с `ActivityOutput` переменной, используя приведенный ниже синтаксис. Если выходным объектом является объект, модуль Runbook может указать одно свойство.
...
При создании модуля Runbook можно получить доступ к его входным параметрам, щелкнув **Ввод и вывод** на панели инструментов Runbook. Откроется элемент управления Вход и выход, в котором можно изменить существующий входной параметр или создать новый, нажав кнопку **Добавить входные данные**.
...
| Описание |Необязательный параметр. Описание назначения для входного параметра. |
| Type | Необязательный параметр. Предполагаемый тип данных значения параметра. Портал Azure предоставит соответствующий элемент управления для типа данных каждого параметра при запросе ввода его значения. Поддерживаются параметры строкового, логического, объектного типа, а также типа Int32, Int64, DateTime и типа десятичного числа. Если тип данных не выбран, то по умолчанию используется значение строковоготипа.|
| Обязательный | Необязательный параметр. Параметр, указывающий, должно ли быть указано значение для параметра. При выборе `yes`значения необходимо указать значение при запуске модуля Runbook. Если выбрать `no`, то при запуске модуля Runbook значение не требуется, и можно будет использовать значение по умолчанию. Модуль Runbook не может быть запущен, если не указано значение для каждого обязательного параметра, для которого не определено значение по умолчанию. |
| Значение по умолчанию | Необязательный параметр. Значение, используемое для параметра, если оно не передается при запуске модуля Runbook. Чтобы задать значение по умолчанию, `Custom`выберите. Выберите `None` , если не нужно указывать значение по умолчанию. |
...
Чтобы использовать выходные данные предыдущего действия в модуле Runbook, используйте `ActivityOutput` переменную со следующим синтаксисом.
...
Например, следующее условие определяет, остановлена ли виртуальная машина из действия с `Get-AzureVM` именем.
...
Вы можете объединить несколько условий в модуле Runbook с помощью [логического оператора](https://technet.microsoft.com/library/hh847789.aspx), например `-and` или `-or`. Например, в следующем условии проверяется, находится ли виртуальная машина в предыдущем примере в состоянии остановка или остановка.
...
В следующем примере используются выходные данные действия, вызываемого `Get Twitter Connection` для заполнения хэш-таблицы.
...
Некоторые Подписчики создают учетную запись службы автоматизации с помощью [учетной записи пользователя Azure AD](automation-create-aduser-account.md) для управления классическим развертыванием Azure или для Azure Resource Managerных ресурсов. Чтобы обеспечить обратную совместимость для этих подписчиков, механизм проверки подлинности, используемый в `Add-AzureAccount` модуле Runbook, является командлетом с [ресурсом учетных данных](automation-credentials.md). Ресурс представляет Active Directory пользователя с доступом к учетной записи Azure.

Вы можете включить эту функцию для графического модуля Runbook, добавив ресурс учетных данных на холст, а затем `Add-AzureAccount` действие, которое использует ресурс учетных данных для своих входных данных. См. указанный ниже пример.

![Действия аутентификации](media/automation-graphical-authoring-intro/authentication-activities.png)

Модуль Runbook должен пройти проверку подлинности в начале и после каждой контрольной точки. Поэтому необходимо использовать `Add-AzureAccount` действие после любого `Checkpoint-Workflow` действия. Не нужно использовать дополнительные действия с учетными данными.
