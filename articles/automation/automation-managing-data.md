...
| ресурсы |Окончательно удаляются через 90 дней после удаления актива пользователем или через 90 дней после удаления пользователем учетной записи, содержащей актив. |
...
Однако если необходимо хранить данные в течение более длительного периода времени, журналы заданий Runbook можно пересылать в Azure Monitor журналы.  Дополнительные сведения см. [в журналах пересылки данных заданий службы автоматизации Azure в журналы Azure Monitor](automation-manage-send-joblogs-log-analytics.md).   

## <a name="backing-up-azure-automation"></a>Создание резервной копии службы автоматизации Azure
