...
Runbook Converter реализован как модуль PowerShell с `ConvertFrom-SCORunbook` cmdlet называется, что выполняет преобразования.  После установки средство создаст ярлык для сеанса PowerShell, загружающий командлет.   
...
Все действия Orchestrator в [модуле стандартных действий](#standard-activities-module) будут преобразованы.  В то же время некоторые стандартные действия Orchestrator не входят в этот модуль и не подвергаются преобразованию.  Например, `Send Platform Event` нет эквивалента Автоматизации Azure, так как событие специфично для Orchestrator.
...
Любое действие из [конвертат-пакета](#integration-pack-converter) будет преобразовано, если `modules` вы предоставите путь к модулю интеграции с параметром. Для пакетов интеграции System Center можно использовать [модули интеграции System Center Orchestrator](#system-center-orchestrator-integration-modules).
...
Runbooks in Orchestrator принимают параметры `Initialize Data` ввода с действием.  Если преобразуемый модуль Runbook включает такое действие, для каждого параметра этого действия в модуле Runbook в службе автоматизации Azure создается [входной параметр](automation-graphical-authoring-intro.md#runbook-input-and-output) .  В преобразованном модуле Runbook создается действие [управления сценарием рабочего процесса](automation-graphical-authoring-intro.md#activities) , извлекающее и возвращающее каждый параметр.  Результаты выполнения этого действия используются всеми действиями в модуле Runbook, использующими входной параметр.
...
Runbooks в Orchestrator начать другие runbooks с деятельностью. `Invoke Runbook` Если преобразованная запущенная книга включает `Wait for completion` в себя это действие и параметр установлен, то для нее создается действие runbook в преобразованном runbook.  Если `Wait for completion` опция не установлена, создается действие Workflow Script, которое использует [Start-AzAutomationRunbook](https://docs.microsoft.com/powershell/module/az.automation/start-azautomationrunbook?view=azps-3.7.0) для запуска runbook. После импорта преобразованного модуля Runbook в службу автоматизации Azure измените это действие с учетом указанной в этом действии информации.
...
* [Гибридная рабочая роль Runbook](automation-hybrid-runbook-worker.md)
