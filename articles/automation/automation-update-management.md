...
После выхода пакета занимает 2 – 3 часа для исправления отображались для компьютеров Linux для оценки. Исправление для оценки компьютеров под управлением Windows появиться через 12–15 часов после выпуска.
...
Запланированное развертывание определяет, какие целевые компьютеры получать применимые обновления, явно указав компьютеров или выбрав [группы компьютеров](../azure-monitor/platform/computer-groups.md) , основанный на поиск по журналам определенного набора компьютеров, или [Запрос azure](#azure-machines) , динамически выбирает виртуальные машины Azure, на основании заданных критериев. Эти группы отличаются от [конфигурацию области](../azure-monitor/insights/solution-targeting.md), который используется только для определения, какие машины получение пакетов управления, включите решение. 

Вы также можете указать расписание для назначения и подтверждения периода времени, в течение которого могут быть установлены обновления. Этот период времени, называется окна обслуживания. Десять минут периода обслуживания зарезервирован для перезагрузки, если требуется перезагрузка, и вы выбрали параметр, соответствующий перезагрузки. Если имеется менее десяти минут периода обслуживания, установки исправлений занимает больше времени, чем ожидалось, не произойдет перезагрузка.
...
Чтобы убедиться, что в группе управления Operations Manager взаимодействует с журналами Azure Monitor, см. в разделе [интеграции проверки Operations Manager с помощью журналов Azure Monitor](../azure-monitor/platform/om-agents.md#validate-operations-manager-integration-with-azure-monitor).
...
Чтобы создать развертывание обновления, выберите **Запланировать развертывание обновлений**. **Новое развертывание обновления** откроется страница. Введите значения для свойств, описанных в следующей таблице и нажмите **Создать**:
...
Управление обновлениями использует репозиторий локально настроенные обновления на исправление поддерживаемых системах Windows. Это WSUS или обновления Windows. Такие средства, как [System Center Updates Publisher](/sccm/sum/tools/updates-publisher
) (Updates Publisher) позволяют публиковать пользовательские обновления в WSUS. Этот сценарий позволяет управление обновлениями для исправления компьютеров, использующих System Center Configuration Manager в качестве репозитория их обновления с помощью стороннего программного обеспечения. Чтобы узнать, как настроить Updates Publisher, см. статью [Установка Updates Publisher](/sccm/sum/tools/install-updates-publisher).
...
Управление обновлениями предоставляет возможность использования на динамическую группу Azure или виртуальных машин Non-Azure для развертывания обновлений. Эти группы вычисляются во время развертывания, поэтому вам не нужно редактировать вашего развертывания, чтобы добавить компьютеры.

> [!NOTE]
> При создании развертывания обновлений, необходимо иметь соответствующие разрешения. Дополнительные сведения см. в разделе [установить обновления](#install-updates).

### <a name="azure-machines"></a>Машины Azure

Эти группы определяются по запросу. Когда начинается развертывание обновлений, оцениваются участники этой группы. Динамические группы не работают с классическим виртуальным машинам. При определении запроса следующие элементы могут использоваться для совместного заполнения динамической группы.
...
### <a name="non-azure-machines"></a>Виртуальных машин, отличные от Azure

Для Azure без машин, сохраненные поисковые запросы также называются группы компьютеров используются для создания динамической группы. Чтобы узнать, как создать сохраненный поиск, см. в разделе [создания группы компьютеров](../azure-monitor/platform/computer-groups.md#creating-a-computer-group). После создания группы ее можно выбрать из списка сохраненных поисковых запросов. Нажмите кнопку **предварительной версии** для предварительного просмотра компьютеров в сохраненный поиск в это время.
...
## <a name="remove-a-vm-from-update-management"></a>Удалите виртуальную Машину из управления обновлениями
