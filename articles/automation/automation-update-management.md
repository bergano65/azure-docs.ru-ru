...
Шаблон Azure [Resource Manager](automation-update-management-deploy-template.md) доступен, чтобы помочь вам развернуть решение **управления обновлением** в новом или существующем рабочем пространстве автоматизации и анализа журналов в подписке.
...
Для создания развертываний обновлений и управления ими требуются определенные разрешения. Чтобы узнать об этих [Role-based access – Update Management](automation-role-based-access-control.md#update-management-permissions)разрешениях, см.
...
Каждая машина Windows, управляемая решением, указана в панели гибридных групп работников в качестве гибридной рабочей группы системы для учетной записи Automation. Решения используют `Hostname FQDN_GUID` конвенцию именования. Вы не можете выбрать эти группы с помощью модулей Runbook в своей учетной записи. Если вы попытаетесь, попытка не удается. Эти группы призваны поддерживать только это решение управления.
...
> [!NOTE]
> Если у вас есть группа управления Operations Manager 1807 или 2019, подключенная к рабочей области Log Analytics с `IsAutoRegistrationEnabled` агентами, настроенными в группе управления для сбора данных журнала, вам необходимо переопределить параметр и установить его в правиле **Microsoft.IntelligencePacks.AzureAutomation.HybridAgent.Init.**
...
> [!NOTE]
> Для того, чтобы машины с агентом Operations Manger полностью управлялись системой управления обновлениями, агент должен быть обновлен до агента Log Analytics для Windows или агента Log Analytics для Linux. Чтобы узнать, как обновить агент, см. [эту статью](https://docs.microsoft.com/system-center/scom/deploy-upgrade-agents). В средах, в нее используется менеджер операций, необходимо запустить диспетчер атлетового центра 2012 R2 UR 14 или позже.
