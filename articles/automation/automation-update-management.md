...
После того как компьютер выполнит проверку соответствия обновлениям, агент переадресовывает сведения в Azure Log Analytics в пакетном режиме. На компьютере с Windows проверка на соответствие по умолчанию выполняется через каждые 12 часов.
