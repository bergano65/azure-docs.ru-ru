...
После того как компьютер выполнит проверку соответствия обновлениям, агент переадресовывает сведения в Azure Log Analytics в пакетном режиме. На компьютере с Windows проверка на соответствие по умолчанию выполняется каждые 12 часов.
...
Регистрация компьютера для Управления обновлениями в нескольких рабочих областях Log Analytics (множественная адресация) не поддерживается.
...
Дополнительные сведения по установке агента Log Analytics для Linux и скачиванию последней версии см. [здесь](https://github.com/microsoft/oms-agent-for-linux). Дополнительные сведения по установке агента Log Analytics для Windows см. в статье [Подключение компьютеров Windows к службе Log Analytics в Azure](../log-analytics/log-analytics-windows-agent.md).
...
Если агент не может взаимодействовать с Log Analytics и настроен на взаимодействие с Интернетом через брандмауэр или прокси-сервер, вам необходимо подтвердить корректную настройку брандмауэра или прокси-сервера. Чтобы узнать, как проверить это, ознакомьтесь со статьей [Подключение компьютеров Windows к службе Log Analytics в Azure](../azure-monitor/platform/agent-windows.md) или [Настройка агента Log Analytics для компьютеров Linux в гибридной среде](../log-analytics/log-analytics-agent-linux.md).
...
О том, как подтвердить, что группа управления Operations Manager взаимодействует с Log Analytics, описывается в разделе [Проверка интеграции Operations Manager с OMS](../azure-monitor/platform/om-agents.md#validate-operations-manager-integration-with-log-analytics).
...
| Компьютеры, на которые нужно установить обновления |Щелкните "Сохраненные условия поиска", "Импортировать группу" или выберите "Компьютер" в раскрывающемся списке и выберите нужные компьютеры. Если выберете **Компьютеры**, готовность к обновлению будет показана в столбце **Готовность к обновлению агента**.</br> Дополнительные сведения о различных способах создания группы компьютеров в Log Analytics, см. в разделе [Использование групп компьютеров при поиске по журналам Log Analytics](../azure-monitor/platform/computer-groups.md). |
...
## <a name="firstparty-predownload"></a>Установка собственных исправлений и предварительная загрузка
...
По умолчанию Центр обновления Windows предоставляет обновления только для ОС Windows. Если вы зададите параметр **Give me updates for other Microsoft products when I update Windows** (Предоставлять обновления для других продуктов Майкрософт при обновлении Windows), вы будете получать обновления для других продуктов, в том числе исправления безопасности для SQL Server и другого нашего собственного программного обеспечения. Этот параметр нельзя настроить с помощью групповой политики. Выполните следующие команды PowerShell в системах, для которых вы хотите включить установку наших собственных исправлений, и решение "Управление обновлениями" будет учитывать это параметр.
...
Важно знать, что исключения переопределяют включения. Например, при определении правила исключения `*` ни исправления, ни пакеты не устанавливаются, так как они все будут исключены. В компьютерах Linux, если пакет включен, но имеет зависимый пакет, который был исключен, пакет не устанавливается.
