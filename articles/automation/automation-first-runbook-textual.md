...
1. В этом случае будет создан [Runbook рабочего процесса PowerShell](automation-runbook-types.md#powershell-workflow-runbooks). Поэтому выберите **Рабочий процесс PowerShell** для **типа Runbook**.
...
Можно либо ввести код непосредственно в модуль Runbook, либо выбрать командлеты, модули Runbook и ресурсы из элемента управления "Библиотека" и добавить их в модуль Runbook с любыми связанными параметрами. В этом пошаговом руководстве введите код непосредственно в модуль Runbook.

1. В настоящее время модуль Runbook пуст только с обязательным ключевым словом **рабочего процесса** , именем модуля Runbook и фигурными скобками, добавлен весь рабочий процесс.
...
1. После завершения задания Runbook на панели "тест" отображаются выходные данные. В этом случае отображается «Hello World».
...
1. Прокрутите влево, чтобы просмотреть модуль Runbook в области **модули Runbook** , и обратите внимание, что для параметра **Состояние создания** установлено значение Опубликовано.
1. Прокрутите экран вправо, чтобы просмотреть панель MyFirstRunbook-Workflow.
...
1. Когда состояние модуля Runbook отобразится, нажмите кнопку **выходные данные**. Откроется страница выходных данных, где может появиться сообщение "Hello World".
...
1. Щелкните **Все журналы** , чтобы открыть область "Потоки" для задания Runbook. в потоке вывода должно отображаться только "Hello World". Обратите внимание, что на панели «потоки» могут отображаться другие потоки для задания Runbook, такие как потоки подробных сведений и ошибок, если Runbook выполняет запись в них.
...
Вы протестировали и опубликовали свой модуль runbook, но пока он не выполняет никаких полезных действий. Нужно, чтобы он управлял ресурсами Azure. Это не может сделать это, если только не будет выполнена проверка подлинности с использованием учетных данных для подписки. При проверке подлинности используется командлет **Connect-азаккаунт** .
...
Теперь, когда модуль runbook прошел аутентификацию в подписке Azure, вы можете управлять ресурсами. Добавим команду для запуска виртуальной машины. Вы можете выбрать любую виртуальную машину в подписке Azure. Теперь вы прописано это имя в Runbook. Если вы управляете ресурсами в нескольких подписках, необходимо использовать параметр *азконтекст* с командлетом [Get-азконтекст](/powershell/module/az.accounts/get-azcontext) .
...
Сейчас модуль Runbook запускает виртуальную машину, которая жестко закодирована в модуле Runbook. Это будет более полезно, если вы можете указать виртуальную машину при запуске модуля Runbook. Добавьте входные параметры в модуль Runbook, чтобы предоставить эти функциональные возможности.

1. Добавьте значения для *VMName* и *ResourceGroupName* в модуль Runbook и используйте связанные переменные с командлетом **Start-AzVM** , как показано ниже.
