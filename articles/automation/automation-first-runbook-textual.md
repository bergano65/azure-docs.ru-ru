...
Для начала вы создадите простой модуль runbook, выводящий на экран текст *Hello World*.
...
1. При этом создается [задание Runbook](automation-runbook-execution.md) и отображается его состояние.
   ...
   Состояние задания начнется с *постановкой в очередь* , что означает, что он ожидает, пока Рабочая роль Runbook в облаке станет доступной. Как только рабочая роль затребует задание, оно получит состояние *Запущено*, а с началом фактического выполнения модуля runbook — состояние *Выполняется*.
...
1. Если сейчас прокурить экран влево, чтобы просмотреть модуль runbook в области **Модули Runbook**, для параметра **Состояние создания** отобразится значение **Опубликовано**.
