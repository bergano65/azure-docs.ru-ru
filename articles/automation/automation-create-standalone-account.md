...
* **Выполнить как учетную запись**. Она выполняет следующие задачи:
...
* Для создания учетной записи Автоматизации учетная запись пользователя Azure AD должна быть добавлена к роли с разрешениями, эквивалентными роли владельца **для корпорации Майкрософт. Ресурсы автоматизации.** Дополнительные сведения см. в статье [Управление доступом на основе ролей в службе автоматизации Azure](automation-role-based-access-control.md).
...
Если вы не являетесь участником экземпляра Active Directory, прежде чем быть добавлены к роли глобального администратора/администратора подписки, вы добавлены в Active Directory в качестве гостя. В этом сценарии на странице **учетной** записи Add Automation можно увидеть это сообщение: «У вас нет разрешений на создание».
...
1. В области **Добавление учетной записи службы автоматизации** в поле **Имя** введите имя новой учетной записи службы автоматизации. Это имя не может быть изменено после добавления. *Названия учетных записей автоматизации уникальны для одной из регионов и ресурсной группы. Имена для учетных записей автоматизации, которые были удалены, могут быть доступны не сразу.*
