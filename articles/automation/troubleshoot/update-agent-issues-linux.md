...
Может быть несколько причин, по которым компьютер не отображается как готовый (работоспособный) в Управление обновлениями. Вы можете проверить работоспособность агента гибридной рабочей роли Runbook Linux, чтобы определить базовую проблему. Ниже приведены три состояния готовности для компьютера.
...
В этой статье описывается, как запустить средство устранения неполадок для компьютеров Azure с портал Azure и с компьютеров, не относящихся к Azure, в [автономном сценарии](#troubleshoot-offline). 
