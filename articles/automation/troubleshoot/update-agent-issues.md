...
Есть много причин, почему компьютер Azure может не отображать состояние **Готово** в службе "Управление обновлениями Azure". В службе "Управление обновлениями" можно проверить работоспособность агента гибридной рабочей роли, чтобы определить первоначальную проблему. В этой статье описывается, как запустить средство устранения неполадок службы "Управление обновлениями" на портале Azure и в автономных сценариях.

## <a name="start-the-troubleshooter"></a>Запуск средства устранения неполадок

На портале Azure на странице **Troubleshoot Update Agent** (Устранение неполадок с агентом обновления) отображаются проблемы с агентом. Там есть ссылка на эту статью, которая может помочь в устранении неполадок. Чтобы перейти на страницу **Troubleshoot Update Agent** (Устранение неполадок с агентом обновления), выберите ссылку **Устранение неполадок** в столбце **Update Agent Readiness** (Готовность к обновлению агента).
...
Чтобы переустановить Microsoft Monitoring Agent, ознакомьтесь с [этой статьей](../../log-analytics/log-analytics-quick-collect-windows-computer.md#install-the-agent-for-windows).
...
Средство устранения неполадок можно использовать в гибридной рабочей роли в автономном режиме, запустив скрипт локально. Скрипт [Troubleshoot-WindowsUpdateAgentRegistration](https://www.powershellgallery.com/packages/Troubleshoot-WindowsUpdateAgentRegistration) можно найти в коллекции PowerShell. Выходные данные этого скрипта выглядят так:
