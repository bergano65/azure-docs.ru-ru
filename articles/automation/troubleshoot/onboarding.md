...
### <a name="missing-write-permissions"></a>Сценарий: сбой адаптации с сообщением — не удается включить решение
...
Убедитесь, что у вас есть разрешение на подключение виртуальной машины. Просмотрите [разрешения, необходимые для подключения компьютеров](../automation-role-based-access-control.md#onboarding), и попробуйте еще раз подключить решение. Если вы получаете сообщение об ошибке `The solution cannot be enabled on this VM because the permission to read the workspace is missing`, убедитесь, что у вас есть разрешение `Microsoft.OperationalInsights/workspaces/read`, чтобы иметь возможность найти виртуальную машину в рабочей области.
...
Проверьте уведомления в правом верхнем углу портал Azure или перейдите к группе ресурсов, содержащей учетную запись службы автоматизации, и выберите **развертывания** в разделе **Параметры** , чтобы просмотреть неудачное развертывание. Дополнительные сведения о политике Azure см. в [обзоре политики Azure](../../governance/policy/overview.md?toc=%2fazure%2fautomation%2ftoc.json).
...
* Запуск и остановка виртуальных машин в нерабочее время
  ...
  * Дополнительные действия по удалению Запуск и остановка виртуальных машин в нерабочее времяного решения см. в статье [Запуск и завершение виртуальной машины в нерабочее время](../automation-solution-vm-management.md##remove-the-solution).
