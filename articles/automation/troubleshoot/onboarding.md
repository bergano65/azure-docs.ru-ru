...
description: Сведения об устранении ошибок адаптации решения.
services: automation
author: mgoedtel
ms.author: magoedte
ms.date: 05/22/2019
...
При подключении решения Управление обновлениями или Отслеживание изменений и инвентаризации могут возникать ошибки. В этой статье описаны различные ошибки, которые могут возникнуть, и способы их устранения.
...
### <a name="scenario-re-signing-certificates-via-https-proxy-is-not-supported"></a><a name="resigning-cert"></a>Сценарий: повторное подписание сертификатов через прокси-сервер HTTPS не поддерживается
...
Убедитесь, что у вас есть [необходимые разрешения для подключения к компьютерам](../automation-role-based-access-control.md#onboarding-permissions) , а затем повторите попытку подключения решения. Если вы получаете сообщение об `The solution cannot be enabled on this VM because the permission to read the workspace is missing`ошибке, убедитесь, что `Microsoft.OperationalInsights/workspaces/read` у вас есть разрешение на возможность найти виртуальную машину в рабочей области.
...
Эта ошибка может возникать, если ценовая категория не соответствует модели выставления счетов подписки. См. раздел [мониторинг использования и оценка затрат в Azure Monitor](https://aka.ms/PricingTierWarning).
...
Этот код ошибки означает, что сохраненный запрос группы поисковых компьютеров, используемый для решения, отформатирован неправильно. 
...
Вы можете удалить запрос для решения, а затем снова подключить решение, которое воссоздаст запрос. Запрос можно найти в рабочей области в разделе **сохраненные условия поиска**. Имя запроса — **микрософтдефаулткомпутерграуп**, а Категория запроса — имя связанного решения. Если включено несколько решений, запрос **микрософтдефаулткомпутерграуп** отображает несколько раз при **сохраненном поиске**.
...
При использовании этих решений из рабочей области удалите следующие решения:

* Управление обновлениями
* Отслеживание изменений и инвентаризация
* Запуск и остановка виртуальных машин в нерабочее время

После удаления решений можно удалить связь с рабочей областью. Важно очистить все существующие артефакты из этих решений из рабочей области и учетной записи службы автоматизации. 

* Для Управление обновлениями удалите развертывания обновлений (расписания) из учетной записи службы автоматизации.
* Для запуск и остановка виртуальных машин в нерабочее время удалите все блокировки на компоненты решения в учетной записи службы автоматизации в разделе **Параметры** > **блокировки**. См. раздел [удаление запуск и остановка виртуальных машин в нерабочее время решения](../automation-solution-vm-management.md#remove-the-solution).
...
### <a name="scenario-install-failed-because-of-a-transient-environment-issues"></a><a name="transient-environment-issue"></a>Сценарий: сбой установки из-за временных проблем среды

Сбой установки расширения Log Analytics для Windows во время развертывания из-за другой установки или действия, блокирующего установку
