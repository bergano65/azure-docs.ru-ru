...
### <a name="child-runbook-auth-failure"></a>Сценарий: сбой дочернего модуля runbook при работе с несколькими подписками
...
1. Предельный объем памяти. Действуют [ограничения](../../azure-subscription-service-limits.md#automation-limits) на объем памяти, выделяемой для песочницы службы автоматизации, поэтому задание может завершиться ошибкой, если оно использует более 400 МБ памяти.
...
1. Модуль несовместим. Такая ситуация может возникнуть, если зависимости модуля заданы неправильно. В этом случае runbook обычно возвращает сообщение об ошибке "Команда не найдена" или "Не удается привязать параметр".
...
Гибридные рабочие роли не ограничены тремя часами [справедливого распределения](../automation-runbook-execution.md#fair-share), как песочницы Azure. Хотя гибридные рабочие роли Runbook не ограничиваются тремя часами справедливого распределения, модули runbook, выполняемые в гибридных рабочих ролях Runbook, все равно следует разрабатывать с поддержкой перезапуска при возникновении непредвиденных проблем локальной инфраструктуры.

Еще один вариант — оптимизировать runbook с помощью [дочерних модулей runbook](../automation-child-runbooks.md). Если runbook циклически выполняет одну и ту же функцию для ряда ресурсов, например определенную операцию в нескольких базах данных, такую функцию можно переместить в дочерний runbook. Все эти дочерние модули runbook параллельно выполняются в отдельных процессах. В результате общее время выполнения родительского модуля runbook уменьшается.
...
если веб-перехватчик отключен, его можно повторно включить на портале Azure. Если истек срок действия веб-перехватчика, его необходимо удалить или создать заново. Если срок действия еще не истек, можно только [обновить веб-перехватчик](../automation-webhooks.md#renew-webhook).
