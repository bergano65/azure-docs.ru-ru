...
2. Сведения об определенном сообщении см. в разделе о [потоках предупреждений и ошибок](https://docs.microsoft.com/azure/automation/automation-runbook-output-and-messages#runbook-output) runbook и сравните сообщение с перечисленными ниже ошибками.
...
### <a name="if-the-runbook-is-suspended-or-unexpectedly-failed"></a>Если работа модуля Runbook приостановлена или произошла непредвиденная ошибка

Существует несколько причин приостановки или сбоя модуля runbook.

* [Состояния заданий](https://docs.microsoft.com/azure/automation/automation-runbook-execution#job-statuses) определяют состояние Runbook и некоторые возможные причины.
...
### <a name="login-azurerm"></a>Сценарий. Выполните команду Login-AzureRMAccount для входа.
...
1. Теперь можно использовать следующий код Runbook для проверки разрешений учетной записи службы автоматизации в другой подписке.
    ...
    Замените "\<CertificateThumbprint\>" значением, скопированным на шаге #1 и значением "\<SubscriptionName\>", которое было скопировано на шаге #4.
...
При работе с командлетом `Add-AzureAccount` или `Connect-AzureRmAccount` произошла следующая ошибка.
...
### <a name="get-serializationsettings"></a>Сценарий. Появляется сообщение об ошибке в потоках задания о методе get_SerializationSettings
...
* Необходимые модули не импортированы или устарели.
* Для работы скрипта может потребоваться участие пользователя.
* Для работы некоторых модулей требуются библиотеки, которые (предположительно) есть на компьютерах под управлением Windows. Эти библиотеки могут отсутствовать в песочнице.
* Для работы некоторых модулей требуется версия .NET, которая отличается от доступной в песочнице.

#### <a name="resolution"></a>Разрешение

Для решения этой проблемы подходит любой из следующих способов.

* Убедитесь, что вы правильно [прошли проверку подлинности для входа в Azure](../manage-runas-account.md).
* Убедитесь, что модули [Azure импортированы правильно и не устарели](../automation-update-azure-modules.md).
* Убедитесь, что ни один из ваших командлетов не запрашивает информацию. Это поведение не поддерживается в модулях runbook.
* Убедитесь, что для элементов, которые являются частью вашего модуля, не заданы зависимости с элементами, которые не входят в этот модуль.
* В песочницах Azure используется .NET Framework 4.7.2. Если в модуле используется более поздняя версия платформы, он не сможет работать. В этом случае необходимо использовать [гибридную рабочую роль runbook](../automation-hybrid-runbook-worker.md)
...
## <a name="other"></a>: Моя проблема не указана в этой статье
...
### <a name="runbook-job-completed-but-with-unexpected-results-or-errors"></a>Задание Runbook завершено, но с непредвиденными результатами или ошибками

Ниже перечислены конкретные проблемы и способы их решения, но мы настоятельно рекомендуем сначала выполнить следующие два действия по устранению неполадок:

* Попробуйте [запустить модуль Runbook локально,](https://docs.microsoft.com/azure/automation/troubleshoot/runbooks#runbook-fails) прежде чем запускать его в службе автоматизации Azure. Это может помочь, если проблема возникла в модуле runbook, а не в службе автоматизации Azure.
* Сведения об определенном сообщении см. в разделе о [потоках предупреждений и ошибок](https://docs.microsoft.com/azure/automation/automation-runbook-output-and-messages#runbook-output) runbook и сравните сообщение с перечисленными ниже ошибками.
* Добавьте [дополнительные выходные данные](https://docs.microsoft.com/azure/automation/automation-runbook-output-and-messages#message-streams) в модуль Runbook, чтобы найти место возникновения ошибки.
...
### <a name="switching-between-multiple-subscriptions-in-a-runbook"></a>Переключение между несколькими подписками в модуле runbook

Следуйте указаниям в [работе с несколькими подписками](https://docs.microsoft.com/azure/automation/automation-runbook-execution#working-with-multiple-subscriptions).

### <a name="runbook-fails-with-error-the-subscription-cannot-be-found"></a>Сбой Runbook с ошибкой: Не удается найти подписку

Эта проблема может возникать, когда в модуле runbook не используется учетная запись запуска от имени для доступа к ресурсам Azure. Чтобы устранить проблему, выполните действия, описанные в разделе [Сценарий: не удается найти подписку Azure](https://docs.microsoft.com/azure/automation/troubleshoot/runbooks#unable-to-find-subscription)

### <a name="error-your-azure-credentials-have-not-been-set-up-or-have-expired-please-run-connect-azurermaccount-to-set-up-your-azure-credentials"></a>Ошибка: Учетные данные Azure не настроены или истек срок их действия. выполните команду Connect-azureRmAccount, чтобы настроить учетные данные Azure.

Эта ошибка может возникнуть, если вы не используете учетную запись запуска от имени или срок ее действия истек. См. раздел [Управление учетными записями запуска от имени Azure Automation](https://docs.microsoft.com/azure/automation/manage-runas-account).

### <a name="error-run-login-azurermaccount-to-login"></a>Ошибка: Выполните команду Login-AzureRmAccount для входа.

Эта ошибка может возникнуть, если вы не используете учетную запись запуска от имени или срок ее действия истек. См. раздел [Управление учетными записями запуска от имени Azure Automation](https://docs.microsoft.com/azure/automation/manage-runas-account).

### <a name="runbook-fails-with-error-strong-authentication-enrollment-is-required"></a>Сбой Runbook с ошибкой: Требуется надежная регистрация проверки подлинности

См. статью [Проверка подлинности в Azure, так как многофакторная идентификация включена](https://docs.microsoft.com/azure/automation/troubleshoot/runbooks#auth-failed-mfa) в разделе Руководство по устранению неполадок Runbook.

### <a name="runbook-fails-with-the-errors-no-permission-forbidden-403-or-some-variation"></a>Сбой Runbook с ошибками: Нет разрешения, запрещено, 403 или какой либо вариант
...
### <a name="error-the-term-is-not-recognized"></a>Ошибка: Термин не распознан

Выполните действия, описанные в разделе [не распознано](https://docs.microsoft.com/azure/automation/troubleshoot/runbooks#cmdlet-not-recognized) в руководстве по устранению неполадок Runbook.

### <a name="errors-about-typedata"></a>Ошибки о TypeData

Если вы получаете ошибки, связанные с TypeData, это значит, что вы запустили рабочий процесс PowerShell с модулями, которые не поддерживают этот рабочий процесс. Вам нужно изменить тип модуля runbook на PowerShell. Дополнительные сведения см. в разделе [типы модулей Runbook](https://docs.microsoft.com/azure/automation/automation-runbook-types#powershell-runbooks) .
