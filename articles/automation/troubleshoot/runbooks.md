...
Вы получите следующую ошибку при вызове дочернего модуля runbook с `-Wait` коммутатора и в выходной поток содержит и объектов:
...
4. Модуль runbook попытка вызвать исполняемый файл или subprocess в модуле runbook, который работает в изолированной среды Azure. В песочницах Azure этот сценарий не поддерживается.

5. Модуль runbook попытка записи слишком много данных исключение в исходящий поток.
...
* В поток выходных данных заданий ограничено 1 МБ. Убедитесь, что вызовы исполняемый файл или подпроцесс заключить в блок try/catch. Если вызывают исключение, записи сообщения из этого исключения в переменную службы автоматизации. Это будет препятствовать его записи в поток выходных данных задания.
...
### <a name="cannot-invoke-method"></a>Сценарий. Задание PowerShell завершается ошибкой: Не удается вызвать метод

#### <a name="issue"></a>Проблема

При запуске задания PowerShell в модуле runbook, запущенных в Azure, вы получите следующее сообщение об ошибке:
...
Эта ошибка может возникать при запуске PowerShell, запущено задание в модуле runbook в Azure. Это может происходить, потому что модули Runbook запускались в Azure "песочницы" может не запускаться на [полноязыковом режиме](/powershell/module/microsoft.powershell.core/about/about_language_modes)).

#### <a name="resolution"></a>Способы устранения:

Эту проблему можно решить двумя способами:

* Вместо использования `Start-Job`, использовать `Start-AzureRmAutomationRunbook` запуска модуля runbook
* Если runbook содержит следующее сообщение об ошибке, запустите его на гибридной рабочей роли Runbook

Дополнительные сведения об этом поведении и другие поведения модулей Runbook службы автоматизации Azure, см. в разделе [поведение Runbook](../automation-runbook-execution.md#runbook-behavior).
