...
Первый шаг преобразования сценария PowerShell в рабочий процесс PowerShell заключается в его заключении с `Workflow` ключевым словом.  Рабочий процесс начинается с ключевого слова, `Workflow` за которым следует текст скрипта, заключенный в фигурные скобки. Имя рабочего процесса соответствует `Workflow` ключевому слову, как показано в следующем синтаксисе:
...
Имя рабочего процесса должно соответствовать имени модуля Runbook в службе автоматизации. При импорте модуля Runbook имя файла должно совпадать с именем рабочего процесса и должно оканчиваться на *. ps1*.

Чтобы добавить параметры в рабочий процесс, используйте `Param` ключевое слово так же, как в скрипте.
...
`InlineScript` Действие полезно, если необходимо выполнить одну или несколько команд как традиционный сценарий PowerShell вместо рабочего процесса PowerShell.  Пока команды в рабочем процессе отправляются в Windows Workflow Foundation для обработки, команды в блоке InlineScript обрабатываются при помощи Windows PowerShell.
...
`Parallel` Ключевое слово можно использовать для создания блока сценария с несколькими командами, выполняемыми одновременно. Для этого используется описанный ниже синтаксис. В этом случае действия Activity1 и Activity2 запускаются одновременно. Действие Activity3 запускается только после завершения действий Activity1 и Activity2.
...
*Контрольная точка* представляет собой моментальный снимок текущего состояния рабочего процесса, который включает текущие значения переменных и любые выходные данные, созданные для этой точки. Если рабочий процесс завершается ошибкой или приостанавливается, при следующем запуске он будет выполняться не с начала, а с последней контрольной точки.  Вы можете задать контрольную точку в рабочем процессе `Checkpoint-Workflow` с действием. Служба автоматизации Azure имеет функцию " [справедливое предоставление общего доступа](automation-runbook-execution.md#fair-share)", при которой любой модуль Runbook, работающий в течение 3 часов, выгружается, чтобы разрешить выполнение других модулей Runbook. В конечном итоге, выгруженный модуль Runbook будет перезагружен, и когда это произойдет, он возобновит выполнение с последней контрольной точки, сделанной в модуле Runbook. Чтобы гарантировать, что модуль Runbook в конечном итоге будет выполнен, необходимо добавить контрольные точки с интервалами менее 3 часов. Если при каждом запуске добавляется новая контрольная точка, и если модуль Runbook будет удален через 3 часа из-за ошибки, то модуль Runbook возобновит работу неограниченное время.
...
Поскольку учетные данные имени пользователя не сохраняются после вызова действия [приостановки или рабочего процесса](https://technet.microsoft.com/library/jj733586.aspx) или после последней контрольной точки, необходимо задать учетные данные со значением NULL, а затем извлечь их из хранилища ресурсов `Suspend-Workflow` после вызова или контрольной точки.  В противном случае может появиться следующее сообщение об ошибке:`The workflow job cannot be resumed, either because persistence data could not be saved completely, or saved persistence data has been corrupted. You must restart the workflow.`
