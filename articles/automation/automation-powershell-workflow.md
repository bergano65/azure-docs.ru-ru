...
Первый шаг к преобразованию скрипта PowerShell в рабочий `Workflow` процесс PowerShell заключается в том, чтобы приложить его к ключевому слову.  Рабочий процесс начинается с ключевого `Workflow` слова, за которым следует тело скрипта, заключенное в скобки. Название рабочего процесса следует `Workflow` за ключевым словом, показанным в следующем синтаксисе:
...
Имя рабочего процесса должно соответствовать имени модуля Runbook в службе автоматизации. Если книга runbook импортируется, то имя файла должно совпадать с именем рабочего процесса и должно заканчиваться в *.ps1*.

Чтобы добавить параметры в рабочий `Param` процесс, используйте ключевое слово так же, как в скрипте.
...
Действие`InlineScript` полезно, когда вам нужно запустить одну или несколько команд в качестве традиционного скрипта PowerShell вместо рабочего процесса PowerShell.  Пока команды в рабочем процессе отправляются в Windows Workflow Foundation для обработки, команды в блоке InlineScript обрабатываются при помощи Windows PowerShell.
...
Ключевое `Parallel` слово можно использовать для создания блока скриптов с несколькими командами, которые работают одновременно. Для этого используется описанный ниже синтаксис. В этом случае действия Activity1 и Activity2 запускаются одновременно. Действие Activity3 запускается только после завершения действий Activity1 и Activity2.
...
*Контрольная точка* представляет собой моментальный снимок текущего состояния рабочего процесса, который включает текущие значения переменных и любые выходные данные, созданные для этой точки. Если рабочий процесс завершается ошибкой или приостанавливается, при следующем запуске он будет выполняться не с начала, а с последней контрольной точки.  Вы можете установить контрольно-пропускной `Checkpoint-Workflow` пункт в рабочем процессе с действием. Azure Automation имеет функцию, называемую [справедливой долей,](automation-runbook-execution.md#fair-share)где любая книга, которая работает в течение 3 часов, выгружается, чтобы позволить другим запускам работать. В конце концов, разгруженная книга будет перезагружена, и когда она будет, она возобновит выполнение с последнего контрольно-пропускного пункта, взятого в беговой книге. Для того, чтобы гарантировать, что runbook в конечном итоге будет завершена, вы должны добавить контрольно-пропускные пункты с интервалами, которые работают менее 3 часов. Если во время каждого запуска добавляется новый контрольно-пропускной пункт, и если запуск получает выселили через 3 часа из-за ошибки, то runbook будет возобновлен на неопределенный срок.
...
Поскольку учетные данные имени пользователя не сохраняются после вызова действия [приостановки рабочего процесса](https://technet.microsoft.com/library/jj733586.aspx) или после последнего контрольно-пропускного `Suspend-Workflow` пункта, необходимо установить учетные данные, чтобы свести на нет, а затем получить их снова из хранилища активов после вызова или вызова контрольно-пропускного пункта.  В противном случае вы можете получить следующее сообщение об ошибке:`The workflow job cannot be resumed, either because persistence data could not be saved completely, or saved persistence data has been corrupted. You must restart the workflow.`
