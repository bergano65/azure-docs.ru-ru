...
*Контрольная точка* представляет собой моментальный снимок текущего состояния рабочего процесса, который включает текущие значения переменных и любые выходные данные, созданные для этой точки. Если рабочий процесс завершается ошибкой или приостанавливается, при следующем запуске он будет выполняться не с начала, а с последней контрольной точки.  Можно установить контрольную точку для рабочего процесса при помощи действия **Checkpoint-Workflow** . Служба автоматизации Azure есть функция [долевого](automation-runbook-execution.md#fair-share), где будет выгружен любой модуль runbook, который выполняется в течение 3 часов, чтобы разрешить выполнение других модулей Runbook. Со временем, выгружен runbook будет перезагружен, и когда, возобновит выполнение с последней контрольной точки, в модуль runbook. Чтобы гарантировать, что модуль runbook конечном счете будет выполнена, необходимо добавить контрольные точки с интервалами, которые выполняются для менее 3 часов. Во время каждого запуска добавляется новую контрольную точку, если модуль runbook получает удаления после 3 часов из-за ошибки, затем модуль runbook будет возобновлена неопределенное время.
