...
> [!NOTE]
> `Connect-AzAccount -Identity`работает для гибридного Runbook Worker, используя установленную системой идентификацию и одиночное удостоверение, назначенное пользователем. Если вы используете несколько идентификаторов, назначенных пользователем, в `Connect-AzAccount` Гибридном Работнике Runbook, в runbook необходимо указать параметр *AccountId* для выбора определенного удостоверения, назначенного пользователем.
...
3. Отоверьте runbook, изменив `Password` значение переменной о свой собственный пароль. 
4. Опубликуйте книгу.
...
Используйте `RunOn` параметр `Start-AzureAutomationRunbook` с cmdlet. В следующем примере Windows PowerShell используется для запуска книги по тестированию под названием **Test-Runbook** в группе Hybrid Runbook Worker под названием MyHybridGroup.
...
> [!NOTE]
> Параметр `RunOn` был `Start-AzureAutomationRunbook` добавлен в версию 0.9.1 Microsoft Azure PowerShell. Если у вас установлена более ранняя версия, следует [загрузить последнюю версию](https://azure.microsoft.com/downloads/) . Установите эту версию только на рабочей станции, где вы начинаете runbook от PowerShell. Вам не нужно устанавливать его на гибридный компьютер Runbook Worker, если вы не собираетесь начать runbooks с этого компьютера.
...
Подписанный runbook `<runbook name>.asc`называется .
