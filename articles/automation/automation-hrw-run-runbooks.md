...
). Либо же вы можете указать учетную запись запуска от имени, чтобы предоставить контекст пользователя для всех модулей Runbook.

### <a name="runbook-authentication"></a>Проверка подлинности модуля Runbook

По умолчанию модули Runbook работают в контексте учетной записи локальной системы для Windows и специальной учетной записи пользователя **nxautomation** для Linux на локальном компьютере, поэтому они должны предоставить свою собственную аутентификацию при доступе к ресурсам.
...
По завершении предыдущих этапов можно использовать `Connect-AzureRmAccount -Identity` в модуле Runbook для аутентификации ресурсов Azure. Это снижает потребность использовать учетную запись запуска от имени и управлять ее сертификатом.
