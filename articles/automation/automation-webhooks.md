...
| Свойство | Описание |
|:--- |:--- |
| Name |Для веб-перехватчика можете указать любое имя, так как оно не предоставляется клиенту. Имя используется для идентификации модуля Runbook в службе автоматизации Azure. <br> Рекомендуется задать веб-перехватчику имя, связанное с клиентом, который будет его использовать. |
