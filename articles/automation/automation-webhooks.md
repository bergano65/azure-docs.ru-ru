...
|:--- |:--- |
| Название |Для веб-перехватчика можете указать любое имя, так как оно не предоставляется клиенту. Имя используется для идентификации модуля Runbook в службе автоматизации Azure. <br> Рекомендуется задать веб-перехватчику имя, связанное с клиентом, который будет его использовать. |
...
| Enabled |При создании веб-перехватчика он по умолчанию включается. Если его отключить, клиенты не смогут его использовать. Свойство **Включен** можно задать при создании веб-перехватчика или в любое время после его создания. |
