...
> [!NOTE]
> Для Отслеживание изменений и инвентаризации используется одно и то же решение. Если один из них включен, также включается и другой.

На рисунке ниже выбрано управление обновлениями. Отслеживание изменений и инвентаризации имеют одинаковый макет и поведение.
...
При подключении нескольких компьютеров могут быть компьютеры, которые отображаются как `Cannot enable`. У этого могут быть разные причины. В следующих разделах показаны возможные причины `Cannot enable` состояния виртуальной машины при попытке подключения.
...
**Решение**. Щелкните **использовать как конфигурацию** , чтобы изменить целевую учетную запись службы автоматизации и log Analytics рабочую область.
...
**Решение:** См. [список поддерживаемых клиентов](automation-update-management.md#supported-client-types).
