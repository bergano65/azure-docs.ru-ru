...
В этой статье приведены сведения о настройке решения "Отслеживание изменений", позволяющего легко обнаруживать изменения в среде. Решение отслеживает следующие изменения конфигурации, которые помогут выявить проблемы в работе:

- Программное обеспечение Windows
- Программное обеспечение Linux (пакеты)

    >[!NOTE]
    >Отслеживание изменений отслеживает только программное обеспечение, которое управляется диспетчером пакетов распространения.
...
Сведения о подключении компьютеров к решению см. в статье [Onboard Update Management, Change Tracking, and Inventory solutions](automation-onboard-solutions-from-automation-account.md) (Подключение решений "Управление обновлениями", "Отслеживание изменений" и "Инвентаризация"). После подключения машины с помощью решения Отслеживание изменений и инвентаризации можно настроить элементы для трассировки. Если для трассировки включен новый файл или раздел реестра, он включается как для Отслеживание изменений, так и для инвентаризации.
...
> [!NOTE]
> Включение полного Отслеживание измененийного решения может привести к дополнительным расходам. Дополнительные сведения см. в разделе [цены на автоматизацию](https://azure.microsoft.com/pricing/details/automation/).
...
* Разные методы установки не отписываются
* файлы *. exe не отписываются для Windows
...
* Управляющие программы Linux могут показывать измененное состояние, несмотря на отсутствие изменений. Это происходит из-за того, как захватывается `SvcRunLevels` поле.
...
|ConfigurationChange <br>&#124;где RegistryKey = = @ "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Куалитикомпат"| Полезно для отслеживания изменений в важных антивирусных ключах|
|ConfigurationChange <br>&#124;где RegistryKey содержит @ "HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\SharedAccess\\параметры\\Фиреваллполици"| Полезно для отслеживания изменений в параметрах брандмауэра|
