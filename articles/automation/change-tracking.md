...
Отслеживание изменений и Инвентаризация получают данные из Azure Monitor. Виртуальные машины, подключенные к Log Analytics рабочим областям, используют агенты Log Analytics для получения данных об изменениях в установленном программном обеспечении, службах Майкрософт, реестре Windows и файлах, а также на любых управляющих программах Linux на наблюдаемых серверах. Когда данные доступны, агенты отправляют их в Azure Monitor для обработки. Azure Monitor применяет логику к полученным данным, записывает их и делает доступной. 

Функция Отслеживание изменений и инвентаризации позволяет работать с функциональными областями отслеживания изменений и инвентаризации в службе автоматизации Azure. Так как в обеих областях используется один и тот же агент Log Analytics, процесс добавления виртуальной машины в любой функциональной области будет одинаковым. 

> [!NOTE]
> Чтобы использовать функции Отслеживание изменений и инвентаризации, необходимо разместить все виртуальные машины в одной подписке и регионе учетной записи службы автоматизации.
...
* Если вы соберете более 2500 файлов в течение 30-минутного цикла сбора, производительность отслеживания изменений и инвентаризации может быть снижена.
...
Отслеживание изменений и Inventory поддерживаются во всех операционных системах, отвечающих требованиям агента Log Analytics. Версии операционной системы Windows, которые поддерживаются официально, являются Windows Server 2008 с пакетом обновления 1 (SP1) или более поздней версии и Windows 7 SP1 или более поздней версии. Также поддерживается ряд операционных систем Linux. См. раздел [Общие сведения об агенте log Analytics](https://docs.microsoft.com/azure/azure-monitor/platform/log-analytics-agent). 
...
Используйте Отслеживание изменений и инвентаризацию в портал Azure, чтобы просмотреть сводку изменений для наблюдаемых компьютеров. Чтобы получить доступ к этой функции, выберите один из вариантов: добавить виртуальные машины для **отслеживания изменений** или **инвентаризации** в разделе **Управление конфигурацией** в учетной записи службы автоматизации.  
...
Среднее Log Analytics использования данных для компьютера, использующего Отслеживание изменений и инвентаризацию, составляет примерно 40 МБ в месяц в зависимости от среды. С помощью функции использования и оценки затрат рабочей области Log Analytics можно просмотреть данные, принимаемые Отслеживание изменений и инвентаризации на диаграмме использования. Это представление данных можно использовать для анализа использования данных и определения того, как они влияют на ваш счет. Ознакомьтесь со статьей анализ [использования и оценка затрат](https://docs.microsoft.com/azure/azure-monitor/platform/manage-cost-storage#understand-your-usage-and-estimate-costs).  
