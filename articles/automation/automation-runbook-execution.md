...
description: В этой статье содержатся общие сведения об обработке модулей runbook в службе автоматизации Azure.
services: automation
...
[Агент Log Analytics для Linux](../azure-monitor/platform/agent-linux.md) работает аналогично агенту для Windows, но подключает к Azure Monitor компьютеры Linux. Агент устанавливается с учетной записью пользователя **nxautomation**, позволяющей выполнять команды, для которых требуются корневые разрешения, например, в гибридной рабочей роли Runbook. Учетная запись **nxautomation** — это системная учетная запись, которая не требует пароля.
