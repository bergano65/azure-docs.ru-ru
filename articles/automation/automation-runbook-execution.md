...
|Установка модуля, для которого требуется установщик|Гибридная рабочая роль Runbook|Модули для "песочницы" должно быть отображается|
...
|Сценарии, требующие повышения прав|Гибридная рабочая роль Runbook|"Песочницы" не допускают повышение прав. Чтобы решить эту проблему, используйте гибридной рабочей роли Runbook, и можно отключить контроль учетных Записей и использование `Invoke-Command` при запуске команды требуется повышение прав|
|Сценарии, требующие доступа к инструментарию WMI|Гибридная рабочая роль Runbook|Заданий, выполняемых в "песочницах" в облаке [не имеют доступа к инструментарию WMI](#device-and-application-characteristics)|
...
Запустили PowerShell заданий, запущенных из модуля Runbook в Azure "песочницы" не может выполняться в полноязыковом режиме. Дополнительные сведения о режимах языка PowerShell, см. в разделе [режимы языка PowerShell](/powershell/module/microsoft.powershell.core/about/about_language_modes). Дополнительные сведения о том, как взаимодействовать с заданиями в службе автоматизации Azure см. в разделе [Получение состояния задания с помощью PowerShell](#retrieving-job-status-using-powershell)
...
> [!NOTE]
> Местное время на процесс песочница Azure присваивается время в формате UTC. Вычисления даты и времени в модулях Runbook должны это учитывать.
