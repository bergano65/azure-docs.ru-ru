...
| Характеристика  | Модули Runbook PowerShell | Модули Runbook рабочих процессов PowerShell |
| ------ | ----- | ----- |
| Speed | Запускаются быстро, так как они не используют шаг компиляции. | Выполняется медленнее. |
| Контрольные точки | Не поддерживают контрольные точки. Модуль Runbook PowerShell может возобновить операцию только с начала. | Используйте контрольные точки, которые позволяют книге возобновлять операции с любой точки. |
| Выполнение команды | Поддерживает только последовательное выполнение. | Поддержка как последовательного, так и параллельного выполнения.|
| Пространство выполнения | Одно пространство выполнения выполняет все функции в скрипте. | Для действия, команды или блока скрипта можно использовать отдельное пространство выполнения. |

Помимо этих различий модули Runbook PowerShell имеют некоторые [синтаксические отличия](https://technet.microsoft.com/magazine/dn151046.aspx) от модулей Runbook рабочих процессов PowerShell.
...
4. После завершения задания Runbook на панели "тест" отображаются выходные данные. В этом случае вы увидите `Hello World`.
...
1. После того как в модуле Runbook отобразится `Completed`, нажмите кнопку **выходные данные** , чтобы открыть страницу вывод, на которой можно увидеть отображаемые `Hello World`.
...
Как показано в примере ниже, подключение запуска от имени устанавливается с помощью командлета [Connect-азаккаунт](https://docs.microsoft.com/powershell/module/az.accounts/connect-azaccount?view=azps-3.5.0) . При управлении ресурсами в нескольких подписках необходимо использовать параметр `AzContext` с командлетом [Get-азконтекст](https://docs.microsoft.com/powershell/module/Az.Accounts/Get-AzContext?view=azps-3.5.0).
...
1. Добавьте в скрипт Runbook командлет [Start-AzVM](https://docs.microsoft.com/powershell/module/Az.Compute/Start-AzVM?view=azps-3.5.0) , чтобы запустить виртуальную машину. Как показано ниже, командлет запускает виртуальную машину с именем `VMName` и с группой ресурсов с именем `ResourceGroupName`.
