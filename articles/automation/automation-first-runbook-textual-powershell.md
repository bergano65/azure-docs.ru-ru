...
* Вам может потребоваться [обновить модули Azure](automation-update-azure-modules.md) на основе используемых командлетов.
...
Модуль runbook, который вы только что создали, все еще находится в режиме черновика. Прежде чем запустить модуль в рабочей среде, его нужно опубликовать.  При публикации модуля Runbook существующая опубликованная версия перезаписывается черновой версией. В вашем случае опубликованной версии не существует, так как модуль runbook создан только что.
...
1. Сохраните runbook, затем щелкните **Тестовая область**, чтобы протестировать модуль.
