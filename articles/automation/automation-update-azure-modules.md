...
Так как модули регулярно обновляются специалистами группы продуктов, то изменения могут затронуть и входящие в них командлеты. Данное действие может негативно повлиять на работу модулей Runbook. Это зависит от типа изменения. Например, возможно переименование параметра или полное прекращение поддержки командлета. Чтобы избежать негативного влияния на модули Runbook и автоматизируемые ими процессы, выполните предварительное тестирование и проверку. Если у вас нет выделенной учетной записи службы автоматизации, предназначенной для этой цели, то рекомендуется ее создать, чтобы протестировать различные сценарии в процессе разработки модулей Runbook. Тестирование должно включать повторяющиеся изменения, такие как обновление модулей PowerShell. После проверки результатов и применения необходимых изменений можно перейти к процессу переноса их в рабочую среду.
...
Обновить модули можно путем импорта модуля Runbook [Update-AzureModule.ps1](https://github.com/azureautomation/runbooks/blob/master/Utility/ARM/Update-AzureModule.ps1) в учетную запись службы автоматизации и его запуска. Этот процесс может завершиться сбоем, если версии, которые вы пытаетесь импортировать из коллекции, несовместимы со службами Azure, которые в настоящее время развернуты в целевой среде Azure. Вам нужно будет проверить, указаны ли совместимые версии модулей в параметрах модуля Runbook.
