...
Элемент управления "Библиотека" в левой части редактора позволяет добавлять действия в модуль Runbook. Вы добавите командлет `Write-Output` для вывода текста из модуля Runbook.
...
1. По умолчанию поле **метки** имеет имя командлета, но его можно изменить на более понятное. Измените его на `Write Hello World to output`.
1. Щелкните **Параметры**, чтобы указать значения параметров командлета.

   Некоторые командлеты имеют несколько наборов параметров, и необходимо выбрать, какой из них следует использовать. В этом случае `Write-Output` имеет только один набор параметров.

1. Выберите параметр `InputObject`. Это параметр, который используется для указания текста, отправляемого в поток вывода.
1. Раскрывающееся меню **источник данных** содержит источники, которые можно использовать для заполнения значения параметра. В этом меню выберите **выражение PowerShell**. 
   ...
   Выходные данные таких источников можно использовать в качестве другого действия, ресурса службы автоматизации или выражения PowerShell. В этом случае выходные данные просто `Hello World`. Для этого воспользуйтесь выражением PowerShell и укажите строку.<br>

1. В поле **выражение** введите `Hello World` а затем дважды нажмите кнопку **ОК** , чтобы вернуться на холст.
...
1. После завершения задания Runbook на панели "тест" отображаются выходные данные. В этом случае вы увидите `Hello World`.<br> ![Привет, мир!](media/automation-first-runbook-graphical/runbook-test-results.png)
...
1. Откроется область задания для задания Runbook, которое было создано. Убедитесь, что в поле **состояние задания** указано значение **завершено**.
1. Нажмите кнопку **выходные данные** , чтобы открыть страницу вывод, на которой можно увидеть `Hello World`.
...
Вы протестировали и опубликовали модуль Runbook, но пока не все полезно для управления ресурсами Azure. Прежде чем настраивать модуль Runbook для проверки подлинности, необходимо создать переменную для хранения идентификатора подписки, настроить действие для проверки подлинности, а затем ссылаться на переменную. Включение ссылки на контекст подписки позволяет легко работать с несколькими подписками.
...
1. Запись `Write Hello World to output` больше не нужна. Просто нажмите кнопку с многоточием и выберите **Удалить**.
...
1. В элементе управления "Библиотека" введите `Connect-AzAccount` в поле поиска.
1. Добавьте `Connect-AzAccount` на холст.
1. Наведите указатель мыши на элемент **Get Run As Connection** (Получить подключение запуска от имени), чтобы в нижней части элемента появился кружок. Щелкните окружность и перетащите стрелку на `Connect-AzAccount`, чтобы сформировать ссылку. Модуль Runbook начинается с `Get Run As Connection`, а затем запускается `Connect-AzAccount`.<br> ![Создание связи между действиями](media/automation-first-runbook-graphical/runbook-link-auth-activities.png)
1. На холсте выберите `Connect-AzAccount`. В области управления конфигурацией в поле **Метка** введите **имя входа в Azure** .
1. Щелкните **Параметры**, после чего появится страница Конфигурация параметров действия.
1. Командлет `Connect-AzAccount` имеет несколько наборов параметров, и необходимо выбрать один из них перед указанием значений параметров. Щелкните **набор параметров** , а затем выберите **ServicePrincipalCertificate**.
...
1. В элементе управления "Библиотека" введите `Set-AzContext` в поле поиска.
1. Добавьте `Set-AzContext` на холст.
1. Выберите `Set-AzContext` на холсте. В области управления конфигурацией введите `Specify Subscription Id` в поле **Метка** .
1. Щелкните **Параметры**, чтобы открыть страницу "Конфигурация параметров действия".
1. Командлет `Set-AzContext` имеет несколько наборов параметров, и необходимо выбрать один из них перед указанием значений параметров. Щелкните **набор параметров** , а затем выберите **SubscriptionId**.
...
1. Наведите указатель мыши на `Login to Azure`, пока в нижней части фигуры не появится окружность. Щелкните окружность и перетащите стрелку, чтобы `Specify Subscription Id`. 
...
Теперь необходимо добавить действие `Start-AzVM` для запуска виртуальной машины. Вы можете выбрать любую виртуальную машину в подписке Azure. Теперь вы прописано свое имя в командлет [Start-AzVM](https://docs.microsoft.com/powershell/module/az.compute/start-azvm?view=azps-3.5.0) .

1. В элементе управления "Библиотека" введите `Start-Az` в поле поиска.
2. Добавьте `Start-AzVM` на холст, а затем щелкните и перетащите его под `Specify Subscription Id`.
1. Наведите указатель мыши на `Specify Subscription Id`, пока в нижней части фигуры не появится окружность. Щелкните окружность и перетащите стрелку, чтобы `Start-AzVM`.
1. Выберите `Start-AzVM`. Щелкните **Параметры** , а затем **набор параметров** , чтобы просмотреть наборы для действия.
1. Выберите **ресаурцеграупнамепараметерсетнаме** для набора параметров. Поля **ResourceGroupName** и **Name** содержат восклицательные знаки рядом с ними, чтобы указать, что они являются обязательными параметрами. Обратите внимание, что оба поля предполагают строковые значения.
...
1. Создайте второй обязательный входной параметр с именем `ResourceGroupName`, а затем нажмите кнопку **ОК** , чтобы закрыть область вход и выход.<br> ![Входные параметры Runbook](media/automation-first-runbook-graphical/start-azurermvm-params-outputs.png)
1. Выберите действие `Start-AzVM` и щелкните **Параметры**.
...
Теперь можно изменить модуль Runbook таким образом, чтобы он попытался запустить виртуальную машину только в том случае, если она еще не запущена. Для этого добавьте командлет [Get-AzVM](https://docs.microsoft.com/powershell/module/Az.Compute/Get-AzVM?view=azps-3.5.0) , который получает состояние на уровне экземпляра виртуальной машины. Затем можно добавить модуль кода рабочего процесса PowerShell с именем `Get Status` с фрагментом кода PowerShell, чтобы определить состояние виртуальной машины: запущена или остановлена. Условная ссылка из модуля `Get Status` выполняется только `Start-AzVM` в случае остановки текущего состояния выполнения. В конце этой процедуры модуль Runbook использует командлет `Write-Output` для вывода сообщения о том, что виртуальная машина успешно запущена.

1. Откройте **MyFirstRunbook-Graphical** в графическом редакторе.
1. Удалите связь между `Specify Subscription Id` и `Start-AzVM`, щелкнув ее и нажав клавишу **Delete**.
1. В элементе управления "Библиотека" введите `Get-Az` в поле поиска.
1. Добавьте `Get-AzVM` на холст.
1. Выберите `Get-AzVM` и щелкните **набор параметров** , чтобы просмотреть наборы для командлета. 
1. Выберите набор параметров **GetVirtualMachineInResourceGroupNameParamSet** . Рядом с полями **ResourceGroupName** и **Name** есть восклицательные знаки, указывающие, что они указывают обязательные параметры. Обратите внимание, что оба поля предполагают строковые значения.
...
1. Создайте ссылку из `Specify Subscription Id` для `Get-AzVM`.
1. В элементе управления "Библиотека" разверните **элемент Управление Runbook** и добавьте **код** на холст.  
1. Создайте ссылку из `Get-AzVM` для `Code`.  
1. Щелкните `Code` и в области Конфигурация измените метку, чтобы **получить состояние**.
1. Выберите `Code` и откроется страница редактор кода.  
...
1. Создайте ссылку из `Get Status` для `Start-AzVM`.<br> ![Runbook с модулем кода](media/automation-first-runbook-graphical/runbook-startvm-get-status.png)  
...
1. Дважды добавьте `Write-Output` на холст.
1. Для первого элемента управления `Write-Output` щелкните **Параметры** и измените значение **метки** , чтобы **уведомить о запуске ВМ**.
1. Для параметра **InputObject**измените значение в поле **источник данных** на **выражение PowerShell**и введите выражение `$VMName successfully started.`.
1. На втором элементе управления `Write-Output` щелкните **Параметры** и измените значение **метки** , чтобы **УВЕДОМИТЬ о сбое запуска виртуальной машины**.
1. Для параметра **InputObject**измените значение в поле **источник данных** на **выражение PowerShell**и введите выражение `$VMName could not start.`.
1. Создайте ссылки с `Start-AzVM` на `Notify VM Started` и `Notify VM Start Failed`.
1. Выберите ссылку, чтобы `Notify VM Started` и изменить **условие применения** на true.
