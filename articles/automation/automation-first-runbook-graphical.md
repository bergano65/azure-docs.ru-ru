...
ms.locfileid: "78226496"
---
# <a name="my-first-graphical-runbook"></a>Мой первый графический модуль Runbook
...
В этом руководстве описывается создание [графического модуля Runbook](automation-runbook-types.md#graphical-runbooks) в службе автоматизации Azure. Начните с простого модуля Runbook, который тестирует и публикует, и изучите, как отвести состояние задания Runbook. Затем измените модуль Runbook, чтобы он фактически управлял ресурсами Azure, в этом случае запускает виртуальную машину Azure. Выполните инструкции из руководства, чтобы сделать модуль Runbook более надежным, добавив параметры Runbook и условные ссылки.
...
1. В элементе управления "Библиотека" щелкните поле поиска и введите **запись-вывод**. Результаты поиска показаны на следующем рисунке. <br> ![Microsoft.PowerShell.Utility](media/automation-first-runbook-graphical/search-powershell-cmdlet-writeoutput.png)
...
1. По умолчанию поле **метки** имеет имя командлета, но его можно изменить на более понятное. Введите в это поле значение **Вывод текста "Привет, мир!"** .
...
1. Раскрывающееся меню **источник данных** содержит источники, которые можно использовать для заполнения значения параметра. В этом меню выберите **выражение PowerShell**. 
   ...
   Выходные данные таких источников можно использовать в качестве другого действия, ресурса службы автоматизации или выражения PowerShell. В нашем случае нужно вывести только текст **Hello World**. Для этого воспользуйтесь выражением PowerShell и укажите строку.<br>
...
1. Вам больше не требуется **Hello World записи для вывода** . Просто нажмите кнопку с многоточием и выберите **Удалить**.
...
1. На холсте выберите **Connect-азаккаунт**. В области управления конфигурацией в поле **Метка** введите **имя входа в Azure** .
1. Щелкните **Параметры**, после чего появится страница Конфигурация параметров действия.
1. Командлет **Connect-азаккаунт** имеет несколько наборов параметров, и необходимо выбрать один из них перед указанием значений параметров. Щелкните **Набор параметров** и выберите набор параметров **ServicePrincipalCertificate**.
...
1. На странице значение параметра внесите следующие параметры и нажмите кнопку **ОК**.
   ...
   * **Путь к полю** --тип **applicationId**. Вы указываете имя свойства для пути к полю, так как действие выводит объект с несколькими свойствами.
...
1. В элементе управления "Библиотека" введите **Set-азконтекст** в поле поиска.
1. Добавьте **Set-азконтекст** на холст.
1. На холсте выберите **Set-азконтекст** . В области управления конфигурацией введите в поле **Метка** параметр **указать идентификатор подписки** .
1. Щелкните **Параметры**, чтобы открыть страницу "Конфигурация параметров действия".
1. Командлет **Set-азконтекст** имеет несколько наборов параметров, и необходимо выбрать один из них перед указанием значений параметров. Щелкните **Набор параметров** и выберите набор параметров **SubscriptionId**.
...
1. Выберите набор параметров **ResourceGroupNameParameterSetName** . Поля **ResourceGroupName** и **Name** содержат восклицательные знаки рядом с ними, чтобы указать, что они являются обязательными параметрами. Обратите внимание, что оба поля предполагают строковые значения.
...
1. Выберите **Get-AzVM** , а затем **параметр SET** , чтобы просмотреть наборы для командлета. 
