...
|---------|---------|
|Windows     | [PowerShell](automation-windows-hrw-install.md#automated-deployment)<br>[Manual](automation-windows-hrw-install.md#manual-deployment) (Вручную)        |
|Linux     | [Python](automation-linux-hrw-install.md#installing-a-linux-hybrid-runbook-worker)        |

Рекомендуемый метод установки заключается в том, чтобы полностью автоматизировать процесс настройки компьютера Windows с помощью модуля Runbook службы автоматизации. Второй способ — выполнить пошаговую процедуру установки и настройки роли вручную. Для компьютеров с Linux для установки агента запустите скрипт Python.
...
## <a name="a-nameremove-a-hybrid-runbook-workerremove-a-hybrid-runbook-worker-from-an-on-premises-computer"></a><a name="remove-a-hybrid-runbook-worker">Удаление гибридной рабочей роли Runbook с локального компьютера

Вы можете удалить гибридную рабочую роль Runbook с локального компьютера, как описано в этом разделе для Windows и Linux.

### <a name="remove-the-worker-on-windows"></a>Удаление рабочей роли в Windows
...
### <a name="remove-the-worker-on-linux"></a>Удаление рабочей роли в Linux
...
Чтобы Гибридная Рабочая роль Runbook подключались к службе автоматизации Azure и зарегистрировалась ею, она должна иметь доступ к номеру порта и URL-адресам, описанным в этом разделе. Этот доступ расположен поверх [портов и URL-адресов, необходимых для подключения агента log Analytics](../azure-monitor/platform/agent-windows.md) к Azure Monitorным журналам.

[!INCLUDE [azure-monitor-log-analytics-rebrand](../../includes/azure-monitor-log-analytics-rebrand.md)]

Если для обмена данными между агентом и службой автоматизации Azure используется прокси-сервер, убедитесь, что соответствующие ресурсы доступны. Время ожидания запросов от гибридной рабочей роли Runbook и служб автоматизации составляет 30 секунд. После 3 попыток запрос завершится ошибкой. Если доступ к Интернету ограничивается брандмауэром, вам нужно изменить его настройки, чтобы разрешить доступ к OMS. Если в качестве прокси-сервера используется шлюз Log Analytics, он должен быть настроен для гибридных рабочих ролей. Инструкции см. в разделе [Настройка поддержки гибридных рабочих ролей службы автоматизации](https://docs.microsoft.com/azure/log-analytics/log-analytics-oms-gateway).
