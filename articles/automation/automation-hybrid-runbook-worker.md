...
Каждый гибридный компонент Runbook Worker является членом группы гибридных компонентов Runbook Worker, которая указывается при установке агента. Группа может включать одного агента, но в нее можно установить несколько агентов для обеспечения высокого уровня доступности. Каждая машина может разместить один гибридный работник отчетности на одну учетную запись автоматизации.

Когда вы запускаете модуль runbook в гибридной рабочей роли Runbook, вы выбираете группу для его выполнения. Каждый компонент Worker в группе опрашивает службу автоматизации, чтобы узнать, доступны ли какие-либо задания. Если задание доступно, его получает первая рабочая роль. Время обработки очереди задач зависит от профиля оборудования гибридной рабочей роли и загрузки. Указание конкретной рабочей роли невозможно. К гибридным рабочим ролям Runbook не применяются многие ограничения, которые имеются в песочницах Azure. К ним не применяются те же ограничения дискового пространства, памяти и сетевых сокетов. Гибридные рабочие роли Runbook ограничиваются только собственными ресурсами. Кроме того, Hybrid Runbook Workers не разделяют 180-минутный лимит времени [справедливого обмена,](automation-runbook-execution.md#fair-share) который есть у песочниц Azure. Чтобы узнать больше об ограничениях служб для песочниц Azure [limits](../azure-resource-manager/management/azure-subscription-service-limits.md#automation-limits)и гибридных работников Runbook, см.

## <a name="install-a-hybrid-runbook-worker"></a>Установка гибридной рабочей роли Runbook Worker

Процесс установки гибридного Runbook Worker зависит от операционной системы. В таблице ниже определяются типы развертывания.
...
Рекомендуемый метод установки заключается в использовании runbook автоматизации, чтобы полностью автоматизировать процесс настройки компьютера Windows. Второй метод заключается в том, чтобы следовать пошаговой процедуре, чтобы вручную установить и настроить роль. Для компьютеров с Linux для установки агента запустите скрипт Python.

> [!NOTE]
> Для управления конфигурацией серверов, поддерживающих роль Hybrid Runbook Worker с помощью конфигурации состояния (DSC), необходимо добавить серверы в качестве узлов DSC. Дополнительные сведения о развертывании этих узлов для управления с помощью DSC см. в статье [Подключение компьютеров для управления с помощью Azure Automation DSC](automation-dsc-onboarding.md).
>
...
## <a name="a-nameremove-a-hybrid-runbook-workerremove-a-hybrid-runbook-worker-from-an-on-premises-computer"></a><a name="remove-a-hybrid-runbook-worker">Удалить гибридный Runbook Worker с предприимченного компьютера

С предприимчеза того, что описано в этом разделе для Windows и Linux, можно удалить Hybrid Runbook Worker с предприимчеза.

### <a name="remove-the-worker-on-windows"></a>Удалить сотрудника в Windows
...
3. Откройте сеанс PowerShell в режиме администратора и запустите следующую команду с вашим URL и значениями основных ключевых значений доступа. Используйте `Verbose` параметр для подробного журнала процесса удаления. Чтобы удалить устаревшие машины из группы гибридной рабочей роли, используйте необязательный параметр `machineName`.

```powershell-interactive
Remove-HybridRunbookWorker -url <URL> -key <PrimaryAccessKey> -machineName <ComputerName>
```

### <a name="remove-the-worker-on-linux"></a>Удалить работника на Linux

Для получения идентификатора рабочего пространства можно использовать команду `ls /var/opt/microsoft/omsagent` в гибридном Runbook Worker. Создается папка, которая называется с идентификатором рабочего пространства.
...
Чтобы удалить группу Hybrid Runbook Worker, сначала необходимо удалить hybrid Runbook Worker с каждого компьютера, который является членом группы. Затем используйте следующие шаги, чтобы удалить группу:

1. На портале Azure откройте учетную запись службы автоматизации.
2. Выберите **гибридные группы работников** **в рамках автоматизации процессов.** Выберите группу, которую нужно удалить. Откроется страница свойств этой группы.
...
## <a name="configure-your-network"></a><a name="network-planning"></a>Настройка сети
...
При определении исключений рекомендуется использовать адреса из списка. Для IP-адресов можно загрузить [IP Ranges Ip Ranges Центра обработки данных Microsoft Azure.](https://www.microsoft.com/en-us/download/details.aspx?id=56519) Файл обновляется еженедельно и содержит развернутые в настоящее время диапазоны и все предстоящие изменения диапазонов IP-адресов.
