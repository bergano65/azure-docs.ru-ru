...
> [!NOTE]
> Избегайте использования переменных в параметре *Name* командлета **Get-AutomationVariable** в модуле Runbook или конфигурации DSC. Использование этого параметра может усложнить обнаружение зависимостей между модулями Runbook или конфигурациями DSC и переменными автоматизации во время разработки.
...
Скрипт использует командлет **New-азаутоматионвариабле** , чтобы создать новую переменную и задать ее начальное значение. Затем он может получить значение с помощью **Get-азаутоматионвариабле**. Если значение является простым типом, то этот же тип извлекается. Если это сложный тип, то извлекается тип **PSCustomObject** .
...
Используйте действие **Set-AutomationVariable**, чтобы задать значение переменной службы автоматизации в модуле Runbook PowerShell или конфигурации DSC, и действие **Get-AutomationVariable**, чтобы получить его. Не следует использовать командлеты **Set-азаутоматионвариабле** и **Get-азаутоматионвариабле** или их эквиваленты для модуля AzureRM в модуле Runbook или конфигурации DSC, так как они менее эффективны, чем действия рабочего процесса. 

Обратите внимание, что невозможно получить значение безопасной переменной с помощью **Get-азаутоматионвариабле** или его эквивалента модуля AzureRM. 

Единственный способ создать новую переменную из модуля Runbook или конфигурации DSC — использовать командлет **New-азаутоматионвариабле** .
