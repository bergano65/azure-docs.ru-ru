...
Ниже перечислены типы ошибок PowerShell, которые могут возникать при выполнении модуля Runbook: прекращение ошибок и устранимые ошибки.
...
Это поведение можно также обобщить для использования во многих модулях Runbook, поместив эти два действия в отдельный модуль обработки ошибок, следуя рекомендациям, приведенным выше. Прежде чем исходный модуль Runbook вызовет эту ошибку при обработке модуля Runbook, он может создать пользовательское сообщение из его данных, а затем передать его в качестве параметра в Runbook обработки ошибок.

## <a name="how-to-use-error-handling"></a>Способы использования обработки ошибок

Каждое действие в модуле Runbook имеет параметр конфигурации, который включает исключения в неустранимые ошибки. По умолчанию этот параметр отключен. Рекомендуется включить этот параметр для всех действий, в которых модуль Runbook обрабатывает ошибки. Включение этой конфигурации гарантирует, что модуль Runbook будет обрабатывать как завершающие, так и неустранимые ошибки в действии как неустранимые ошибки, используя ссылку на ошибку.  
