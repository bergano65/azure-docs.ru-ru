...
| Обязательный |Необязательный параметр. Значение Boolean, определяющее, требует ли параметр значения. Если вы установите это до **подлинного,** значение должно быть предоставлено при запуске. Если вы установите это **на ложное,** значение является необязательным. Если вы не указываете значение для **свойства Обязательного,** PowerShell считает параметр ввода необязательным по умолчанию. |
...
Обратите внимание, что runbooks PowerShell и PowerShell Workflow поддерживают все простые типы и сложные типы, такие как **объект** ы или **PSCredential** для параметров ввода. Если в вашей книге runbook есть параметр ввода объекта, необходимо использовать хэш-таблицу PowerShell с парами именных значений для передачи значения. Например, в runbook есть следующий параметр.
...
4. Создайте два параметра со следующими свойствами, которые будут использоваться в активности **Get-AzVM,** а затем нажмите **OK.**
...
> [!NOTE]
> Параметры строки поддерживают пустые значения типа String. Если ввести **[EmptyString]** в текстовое поле входного параметра, ему передастся пустая строка. Кроме того, параметры строки не поддерживают Null. Если вы не передаете какое-либо значение параметру строки, PowerShell интерпретирует его как Null.
...
* *subscriptionId*: Ваш идентификатор подписки Azure.  
* *resourceGroupName*: Название группы ресурсов для учетной записи Автоматизация.
* *automationAccountName*: Название учетной записи Автоматизации, размещенной в указанном облачном сервисе.  
...
Если вы хотите запустить runbook **Get-AzureVMTextual,** созданный ранее с *VMName* и *resourceGroupName* в качестве параметров, используйте следующий формат JSON для органа запроса.
...
При выполнении runbook с помощью веб-крючка отправляется предопределенный параметр ввода *[WebhookData](automation-webhooks.md)* вместе с определенными параметрами ввода. 
...
Введите следующий код в текстовом файле и сохраните его как-то `test.json` на локальном компьютере.
...
1. Войти в Azure, как показано на рисунке. После этого вам будет предложено ввести учетные данные Azure.
   ...
   ```
    ...
    >[!NOTE]
    >Для runbooks PowerShell **Add-AzAccount** и **Add-AzureRMAccount** являются псевдонимами **Connect-AzAccount.** Обратите внимание, что эти псевдонимы не доступны для графических runbooks. Графический runbook может использовать только **сам Connect-AzAccount.**

1. Получите содержимое сохраненного файла JSON и преобразуйте его в строку. `JsonPath` — это путь, по которому был сохранен файл JSON.
...
1. Создайте хэш-таблицу для параметров **Start-AzAutomationRunbook.** 
   ...
   Обратите внимание, что значение *параметров* устанавливается для объекта PowerShell, содержащего значения из файла JSON.
