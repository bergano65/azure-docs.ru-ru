---
title: Используйте управление обновлением Azure с клиентами configuration Manager
description: Эта статья предназначена для настройки менеджера конфигурации Microsoft Endpoint с помощью этого решения для развертывания обновлений программного обеспечения для клиентов ConfigMgr.
services: automation
...
---
# <a name="deploy-updates-to-microsoft-endpoint-configuration-manager-clients-with-update-management"></a>Развертывание обновлений для клиентов Microsoft Endpoint Configuration Manager с помощью управления обновлением

Клиенты, которые инвестировали в Microsoft Endpoint Configuration Manager для управления ПК, серверами и мобильными устройствами, также полагаются на его силу и зрелость в управлении обновлениями программного обеспечения в рамках цикла управления обновлениями программного обеспечения (SUM).

Можно получать отчеты и обновлять управляемые серверы Windows путем создания и предварительного промежуточного размещения развертываемых обновлений программного обеспечения в Configuration Manager, а также получать подробные сведения о состоянии завершенных развертываний обновлений с помощью [решения "Управление обновлениями"](automation-update-management.md). Если вы используете Configuration Manager для отчетности об обновлении, но не для управления развертыванием обновлений с серверами Windows, вы можете продолжать отчитываться перед менеджером конфигурации, пока обновления безопасности управляются с помощью решения Update Management.
...
* Серверы Windows, которыми в настоящее время управляет среда configuration Manager, также должны отчитываться перед рабочим пространством Log Analytics, в котором также включено решение управления обновлением.
* Эта функция включена в текущей ветвке Configuration Manager версии 1606 и выше. Чтобы интегрировать центральный сайт управления конфигурацией или автономный основной сайт с журналами Azure Monitor и коллекциями импорта, просмотрите [менеджер конфигурации Connect в журналы Azure Monitor.](../azure-monitor/platform/collect-sccm.md)  
