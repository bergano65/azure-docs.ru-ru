...
* Создает ресурс сертификата службы автоматизации с `AzureRunAsCertificate` именем в указанной учетной записи службы автоматизации. Ресурс сертификата содержит закрытый ключ сертификата, используемый приложением Azure AD.
  
* Создает ресурс подключения службы автоматизации с `AzureRunAsConnection` именем в указанной учетной записи службы автоматизации. Ресурс подключения содержит идентификатор приложения, идентификатор клиента, идентификатор подписки и отпечаток сертификата.
...
Классическая учетная запись запуска от имени Azure выполняет следующие задачи.
  ...
  * Создает ресурс сертификата службы автоматизации с `AzureClassicRunAsCertificate` именем в указанной учетной записи службы автоматизации. Этот ресурс содержит закрытый ключ сертификата, используемый в сертификате управления.

  * Создает ресурс подключения службы автоматизации с `AzureClassicRunAsConnection` именем в указанной учетной записи службы автоматизации. Ресурс подключения содержит имя подписки, идентификатор подписки и имя ресурса сертификата.

>[!NOTE]
>Классическая учетная запись запуска от имени Azure по умолчанию не создается одновременно при создании учетной записи службы автоматизации. Эта учетная запись создается отдельно после выполнения действий, описанных далее в этой статье.
...
Чтобы создать или обновить учетную запись запуска от имени, необходимо иметь привилегии и разрешения. Администратор приложения в Azure Active Directory и владелец в подписке могут выполнять все задачи. В ситуации, когда у вас есть разделение обязанностей, в следующей таблице показан список задач, эквивалентный командлет и необходимые разрешения.
...
### <a name="permissions-to-configure-classic-run-as-accounts"></a><a name="permissions-classic"></a>Разрешения для настройки классической учетной записи запуска от имени
...
Выполните следующие действия, чтобы обновить учетную запись службы автоматизации Azure в портал Azure. Создайте учетные записи запуска от имени и классической модели запуска от имени по отдельности. Если вам не нужно управлять классическими ресурсами, достаточно создать учетную запись запуска от имени Azure.
...
6. В зависимости от интересующей учетной записи используйте панель **Добавить** **классическую учетную запись запуска от имени Azure или добавить классический** . После просмотра сведений о обзоре нажмите кнопку **создать**.
...
* Учетная запись службы автоматизации, на которую ссылаются как на значения `AutomationAccountName` параметров `ApplicationDisplayName` и.
* Разрешения, эквивалентные тем, которые перечислены в списке [необходимые разрешения для настройки учетных записей запуска от имени](#permissions).

Чтобы получить значения для `SubscriptionId`параметров, `ResourceGroupName`и, которые являются обязательными параметрами для скрипта PowerShell, выполните следующие действия.
...
1. В оболочке командной строки с повышенными привилегиями перейдите в папку, содержащую скрипт.
1. Выполните скрипт, используя значения параметров для необходимой конфигурации.
...
После выполнения сценария вам будет предложено выполнить аутентификацию в Azure. Войдите с помощью учетной записи, которая является членом роли администраторов Подписки и соадминистратором подписки.
...
Если вы создали классическую учетную запись запуска от имени с общедоступным файлом сертификата предприятия (**CER** -файл), используйте этот сертификат. См. раздел [отправка сертификата API управления в портал Azure](../azure-api-management-certs.md).
...
Если вы создали классическую учетную запись запуска от имени с самозаверяющим открытым сертификатом (**CER** -файл), скрипт создаст и сохранит его в папке временных файлов на компьютере. Его можно найти в профиле `%USERPROFILE%\AppData\Local\Temp`пользователя, который использовался для выполнения сеанса PowerShell.
...
## <a name="renewing-a-self-signed-certificate"></a><a name="cert-renewal"></a>Продление срока действия самозаверяющего сертификата

Срок действия самозаверяющего сертификата, созданного для учетной записи запуска от имени, истекает один год с даты создания. В некоторый момент до истечения срока действия учетной записи запуска от имени необходимо продлить сертификат. Его можно продлить в любое время до истечения срока действия. 
...
## <a name="setting-up-automatic-certificate-renewal-with-an-automation-runbook"></a><a name="auto-cert-renewal"></a>Настройка автоматического продления сертификатов с помощью модуля Runbook службы автоматизации
...
Если необходимо ограничить, что еще может делать субъект-служба запуска от имени, можно добавить другие типы ресурсов в `NotActions` элемент определения пользовательской роли. В следующем примере доступ к ограничен `Microsoft.Compute/*`. Если добавить этот тип ресурса в `NotActions` для определения роли, роль не сможет получить доступ к ресурсам вычислений. Дополнительные сведения об определениях ролей см. в статье Общие сведения о [определениях ролей для ресурсов Azure](../role-based-access-control/role-definitions.md).
...
Определить, используется ли субъект-служба, используемый вашей учетной записью запуска от имени, можно в определении роли участника или в другой. 
...
Вы можете разрешить службе автоматизации Azure проверять, использует ли Key Vault и субъект-службу учетной записи запуска от имени определение пользовательской роли. Необходимо сделать следующее:

* Предоставьте разрешения на Key Vault.
* Задайте политику доступа.

Вы можете использовать сценарий [екстенд-аутоматионрунасаккаунтролеассигнменттокэйваулт. ps1](https://aka.ms/AA5hugb) в коллекция PowerShell, чтобы предоставить учетной записи запуска от имени разрешения Key Vault. Дополнительные сведения о настройке разрешений на Key Vault см. в разделе [предоставление приложениям доступа к хранилищу ключей](../key-vault/general/group-permissions-for-apps.md) .
...
Некоторые элементы конфигурации, необходимые для запуска от имени или классической учетной записи запуска от имени, могут быть удалены или неправильно созданы во время начальной установки. Возможные экземпляры неконфигурации включают:
...
Для таких экземпляров с недопустимыми настройками учетная запись службы автоматизации обнаруживает изменения и `Incomplete` отображает состояние в области Свойства учетных записей запуска от имени для учетной записи.
