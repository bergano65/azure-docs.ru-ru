...
ms.locfileid: "51515766"
---
# <a name="asdk-release-notes"></a>Заметки о выпуске ASDK  
Статья содержит сведения об улучшениях, исправлениях ошибок и известных проблемах в пакете средств разработки Azure Stack (ASDK). Если вы не знаете, какая версия используется, проверьте ее [с помощью портала](.\.\azure-stack-updates.md#determine-the-current-version).
...
- Следующие проблемы с управляемыми дисками исправлены в версии 1809, а также в версии 1808 [(исправление для Azure Stack версии 1.1808.7.113)](https://support.microsoft.com/help/4471992/): 

   <!--  2966665 – IS, ASDK --> 
   - Исправлена проблема, при которой присоединение дисков SSD к виртуальным машинам с управляемым диском ценовой категории "Премиум" (DS, DSv2, Fs, Fs_V2) завершается ошибкой: *Failed to update disks for the virtual machine ‘vmname’ Error: Requested operation cannot be performed because storage account type ‘Premium_LRS’ is not supported for VM size ‘Standard_DS/Ds_V2/FS/Fs_v2* (Не удалось обновить диски для виртуальной машины "имя_виртуальной_машины". Ошибка: запрошенная операция не может быть выполнена, так как тип учетной записи хранилища Premium_LRS не поддерживается для размеров виртуальной машины Standard_DS/Ds_V2/FS/Fs_v2). 
   
   - Создание виртуальной машины с управляемым диском с помощью **createOption**: **Attach** завершается следующей ошибкой: *Long running operation failed with status "Failed". Additional Info: "An internal execution error occurred".*
...
<!-- 3090289 – IS, ASDK --> 
- Исправлена проблема, когда после применения обновления 1808 могут возникнуть следующие проблемы при развертывании виртуальных машин с управляемыми дисками:
   ...
   2. Если вы настроили среды с несколькими клиентами, при развертывании виртуальных машин в подписке, которая связана с гостевым каталогом, может произойти сбой с сообщением о внутренней ошибке. Чтобы устранить ошибку, выполните следующие действия.
      1. Примените [Исправление 1808 Azure Stack](https://support.microsoft.com/help/4471992).
...
<!-- 3164607 – IS, ASDK -->
- Повторное присоединение отключенного диска к той же виртуальной машине с тем же именем и LUN завершается такой ошибкой: **Не удается подключить диск данных datadisk к виртуальной машине vm1**. Эта ошибка возникает, если отключение диска еще выполняется или последняя операция отключения завершилась сбоем. Подождите, пока диск будет полностью отключен, и повторите попытку. Либо снова удалите или отключите диск явным образом. Чтобы решить эту проблему, повторно подключите диск с другим именем или LUN. 
...
- <!-- ASDK --> **Масштабируемые наборы виртуальных машин теперь имеют встроенные элементы коллекции**.  Элементы коллекции масштабируемого набора виртуальных машин теперь доступны на порталах пользователей и администраторов без необходимости скачивания. 

- <!-- IS, ASDK --> **Масштабирование масштабируемого набора виртуальных машин**.  Можно использовать портал, чтобы [масштабировать масштабируемый набор виртуальных машин](/azure/azure-stack/azure-stack-compute-add-scalesets.md#scale-a-virtual-machine-scale-set).   
...
- <!-- 3099544 – IS, ASDK --> Если при создании виртуальной машины с помощью портала Azure Stack выбрать размер виртуальной машины, в столбце цены в долларах США за месяц отображается сообщение **Недоступно**. Этот столбец не должен отображаться. Azure Stack теперь поддерживает отображение столбца цены на виртуальную машину.
