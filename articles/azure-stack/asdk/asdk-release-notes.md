...
ms.locfileid: "53724748"
---
# <a name="asdk-release-notes"></a>Заметки о выпуске ASDK 
 
Эта статья содержит сведения об улучшениях, исправлениях ошибок и известных проблемах в пакете средств разработки Azure Stack (ASDK). Если вы не знаете, какая версия используется, проверьте ее [с помощью портала](../azure-stack-updates.md#determine-the-current-version).
...
<!-- 3060156 - IS ASDK --> 
- На обоих порталах (администратора и пользователя) устранена проблема, при которой выбор элемента **Удалить все настройки и личные панели мониторинга** в параметрах портала не работал должным образом. 

<!-- 2930799 - IS ASDK --> 
- На порталах администратора и пользователя устранена проблема, при которой в разделе **Все службы** отображался ресурс **Планы защиты от атак DDoS**, который не входит в Azure Stack.
 
<!--2760466 – IS  ASDK --> 
- Устранена проблема, при которой при установке новой среды Azure Stack в ней могло не отображаться предупреждение **Требуется активация**. Сейчас оно отображается.

<!--1236441 – IS  ASDK --> 
- Устранена проблема, которая препятствовала применению политик RBAC к группе пользователя при использовании ADFS.

<!--3463840 - IS, ASDK --> 
- Устранена проблема со сбоем резервного копирования инфраструктуры из-за недоступности файлового сервера из открытой сети VPN. Это исправление переносит службу резервного копирования инфраструктуры обратно в общедоступную сеть инфраструктуры. Если вы применили последнюю версию [исправления Azure Stack для версии 1809](#azure-stack-hotfixes), которое устраняет эту проблему, то исправление 1811 не будет вносить никаких дальнейших изменений. 
...
<!--  2873083 - IS ASDK --> 
- Устранена проблема, при которой при использовании браузера Internet Explorer на портале для создания масштабируемого набора виртуальных машин раскрывающийся список *размера экземпляра* не загружался должным образом. Теперь в этом браузере все работает правильно.  

<!-- 3190553 - IS ASDK -->
- Устранена проблема, при которой выдавались оповещения о том, что экземпляр роли инфраструктуры недоступен, или о том, что узел единицы масштабирования находится в автономном режиме.
