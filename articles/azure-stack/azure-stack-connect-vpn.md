...
* Развертывание (с несколькими узлами) интегрированных систем Azure Stack с прямым соединением с Интернетом. Доступ к внешнему общедоступному диапазону IP-адресов можно получить напрямую из общедоступного Интернета.
...
2. Выберите **+ Create a resource** (+ Создать ресурс) на портале пользователя.
...
1. Выберите **+ Create a resource** (+ Создать ресурс) на портале Azure.  
...
1. Выберите **+ Create a resource** (+ Создать ресурс) на портале пользователя.
2. Щелкните **Marketplace**, а затем выберите **Сети**.
...
9. В поле **Shared Key (PSK)** (Общий ключ (PSK)) введите **12345**. Нажмите кнопку **ОК**.
...
Чтобы проверить VPN-подключение, необходимо создать две виртуальные машины: в Azure и в Azure Stack. После создания этих виртуальных машин их можно использовать для отправки и получения данных через туннель VPN.
...
Кроме того, следует выполнить более тщательное тестирование передачи данных. Например, путем копирования файлов разных размеров в обоих направлениях.
