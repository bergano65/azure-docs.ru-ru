...
Номер сборки обновления 1901 для Azure Stack — **1.1901.0.95**.
...
- **AzureRm 2.4.0**
   ...
   * **AzureRm.Compute**  
         Добавлены простые наборы параметров в `New-AzureRMVM` и `NewAzureRMVMSS`. Параметр `-ImageName` поддерживает указание пользовательских образов.  
   * **AzureRm.Insights**  
...
- <!-- 3235634 – IS, ASDK --> Исправлена проблема, из-за которой развертываемые виртуальные машины с размерами, содержащими суффикс **v2**, например **Standard_A2_v2**, требовали указания суффикса в следующем виде: **Standard_A2_v2** (строчная v). Как и в глобальной среде Azure, теперь можно использовать **Standard_A2_V2** (V прописными буквами).
...
- В настоящее время в Azure Stack есть расширения, которые успешно развертываются самостоятельно. Их не нужно скачивать через синдикацию marketplace. В следующих версиях эти расширения будут удалены. Операторы Azure Stack теперь должны явно синдицировать эти расширения из Azure Stack Marketplace.
   ...
   Рекомендуется, чтобы при развертывании расширения пользователь Azure Stack установил для поля `autoUpgradeMinorVersion` значение **true**. Например: 
...
- Существует новый подход для точного планирования емкости Azure Stack. Мы установили ограничения на общее количество виртуальных машин, которые могут быть развернуты в Azure Stack, чтобы обеспечить соответствие всех наших внутренних служб с масштабами, в которых работают клиенты. Ограничение составляет 60 виртуальных машин на один узел, максимум 700 для всей метки (если достигнуто ограничение в 60 виртуальных машин на узел). Дополнительные сведения см. в разделе о [новом выпуске планировщика ресурсов](http://aka.ms/azstackcapacityplanner).
