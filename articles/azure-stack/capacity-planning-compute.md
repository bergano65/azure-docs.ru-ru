...
Чтобы добиться высокого уровня доступности рабочих систем с несколькими виртуальными машинами в Azure, виртуальные машины размещаются в группе доступности и распределяются между несколькими доменами сбоя. Это означает, что виртуальные машины, помещенные в группу доступности, физически изолированы друг от друга в стойках, что позволяет обеспечить отказоустойчивость, как показано на следующей схеме:

![Домены сбоя и обновления](media\azure-stack-capacity-planning\domains.png)
...
Ниже приведена формула расчета общего объема доступной памяти для размещения виртуальных машин в клиенте. Этот объем памяти предназначен для всей единицы масштабирования Azure Stack.
  ...
  Доступный объем памяти для размещения виртуальных машин = Общий объем памяти на сервере – Резерв для устойчивости – Накладные расходы на инфраструктуру Azure Stack <sup>1</sup>

  Резерв для устойчивости = H + R * (N-1) + V * (N-2)
...
> - V = самая большая виртуальная машина в единице масштабирования
  ...
  <sup>1</sup> Накладные расходы на инфраструктуру Azure = 208 ГБ
