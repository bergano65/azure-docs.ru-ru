...
- Чтобы повысить общую надежность и доступность основных служб инфраструктуры во время обновления, собственный поставщик ресурсов обновления при необходимости будет определять и вызывать автоматические глобальные исправления как часть плана действий по обновлению. Рабочие процессы глобального исправления включают следующее:
    - Инфраструктура проверяется на наличие в виртуальных машин, которые находятся не в оптимальном состоянии, и выполняется попытка устранить обнаруженные проблемы (при необходимости). 
    - Выполняется проверка на наличие проблем со службой SQL в рамках плана управления и попытка устранить их (при необходимости).
    - Проверяется служба подсистемы балансировки нагрузки программного обеспечения (SLB) в сетевом контроллере (NC) и выполняется попытка устранить выявленные проблемы (при необходимости).
    - Проверяется состояние сетевого контроллера (NC) и выполняется попытка устранить выявленные проблемы (при необходимости).
    - Проверяется состояние узлов Service Fabric со службой агента аварийного восстановления (ERCS), при необходимости выявленные проблемы устраняются.
    - Проверяется состояние узлов Service Fabric со службой XRP, при необходимости выявленные проблемы устраняются.
    - Проверяется состояние узлов Service Fabric со службой Azure Consistent Storage (ACS), при необходимости выявленные проблемы устраняются.
