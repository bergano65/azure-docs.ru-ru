...
7.  Укажите имя пула IP-адресов.  Это имя служит лишь для того, чтобы упростить определение пула IP-адресов, поэтому оно может быть любым.  Рекомендуется использовать в качестве имени диапазон адресов, но это необязательно.
...
## <a name="update-the-acls-on-your-top-of-rack-switches"></a>Обновление списков управления доступом в стоечных коммутаторах верхнего уровня
Последнее, что нужно сделать, чтобы добавленный диапазон IP-адресов заработал, — обновить списки управления доступом (ACL) в стоечных коммутаторах верхнего уровня (ToR).  Списки ACL в коммутаторах ToR блокируются так, что подключения извне Azure Stack к новому диапазону IP-адресов не будут работать, пока этот диапазон не будет добавлен в списки ACL в коммутаторе.  

Чтобы обновить списки ACL в коммутаторах ToR, обратитесь к изготовителю оборудования.  У него есть необходимые для этого средства.
