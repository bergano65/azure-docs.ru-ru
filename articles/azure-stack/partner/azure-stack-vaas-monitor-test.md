...
ms.locfileid: "49956669"
---
# <a name="monitor-and-manage-tests-in-the-vaas-portal"></a>Мониторинг теста с помощью проверки как услуги Azure Stack

[!INCLUDE [Azure_Stack_Partner](./includes/azure-stack-partner-appliesto.md)]

После планирования тестов для решения Azure Stack модель "проверка как услуга" (VaaS) начнет сообщать о состоянии выполнения теста. Эта информация доступна на портале VaaS, как и определенные действия, например изменение расписания и отмена тестов.
...
Каждый тип рабочего процесса позволяет изменить [параметры теста](azure-stack-vaas-parameters.md#test-parameters), указанные во время создания рабочего процесса.
...
Каждый экземпляр теста выводит список доступных действий, выполняемых при щелчке его контекстного меню **[...]** в таблице экземпляров теста.
...
В контекстном меню выберите **Скачать журналы**, чтобы скачать файл `.zip` выходных данных журнала во время выполнения теста. Это действие доступно только для завершенных тестов, то есть тестов с состоянием `Cancelled`, `Failed`, `Aborted` или `Succeeded`.
...
Запланированный тест можно отменить, если его состояние `Pending` или `Running`.  

1. В контекстном меню выберите **Отмена**, чтобы открыть командную строку и отменить выполнение тестового экземпляра.
