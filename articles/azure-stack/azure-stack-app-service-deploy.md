...
9. На следующей странице установщика службы приложений выполните следующие действия.
   ...
   4.3. В поле **Azure Resource Manager root certificate file** (Файл корневого сертификата Azure Resource Manager) введите расположение файла сертификата или перейдите к нему.
...
11. Введите сведения об SQL Server для экземпляра сервера, используемого для размещения баз данных поставщика ресурсов службы приложений, и выберите **Далее**. Установщик проверит свойства подключения SQL.
    ...
    > [!NOTE]
    > Установщик попытается проверить связь с SQL Server, прежде чем продолжить. Но если вы выполняете развертывание в существующей виртуальной сети, этот тест подключения может завершиться сбоем. Вам придет предупреждение и приглашение продолжить. Если информация SQL Server верна, продолжайте развертывание.
    >
