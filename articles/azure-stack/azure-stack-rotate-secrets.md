...
## <a name="pre-steps-for-secret-rotation"></a>Предварительные шаги для смены секретов
   ...
   > Смена секретов выполняется через [привилегированную конечную точку](https://docs.microsoft.com/azure/azure-stack/azure-stack-privileged-endpoint) и требует учетные данные оператора Azure Stack.
