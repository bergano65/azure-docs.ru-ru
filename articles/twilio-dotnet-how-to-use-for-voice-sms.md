...
## <a name="create-an-azure-application"></a><a id="create_app"></a>Создание приложения Azure
Приложение Azure, на котором размещается приложение с поддержкой Twilio, ничем не отличается от любого другого приложения Azure. Добавьте библиотеку Twilio для .NETи настройте роль так, чтобы она могла использовать такие библиотеки.
Сведения о создании начального проекта Azure с помощью Visual Studio см. на [этой странице][vs_project].

## <a name="configure-your-application-to-use-twilio-libraries"></a><a id="configure_app"></a>Нанастройка приложения для использования библиотек Twilio
...
По умолчанию Microsoft Visual Studio 2010 устанавливает NuGet версии 1.2. Для установки библиотек Twilio требуется версия NuGet 1.6 или выше. Для получения информации об установке [https://nuget.org/][nuget]или обновлении NuGet см.
...
### <a name="to-add-the-twilio-libraries-to-your-visual-studio-project"></a><a id="use_nuget"></a>Чтобы добавить библиотеки Twilio в проект Visual Studio, выполните указанные ниже действия.
...
На следующем снимке экрана показано, как отправить SMS-сообщение с использованием класса **MessageResource**. Номер **предоставляется** Twilio для пробных учетных записей для отправки SMS-сообщений. Номер **To** (Кому) для учетной записи Twilio необходимо проверить перед выполнением кода.
...
## <a name="how-to-provide-twiml-responses-from-your-own-website"></a><a id="howto_provide_twiml_responses"></a>Как: Предоставить TwiML Ответы с вашего собственного веб-сайта
...
> [!NOTE]
> Хотя TwiML предназначается для использования веб-службами, TwiML можно также просмотреть в браузере. Например, [https://twimlets.com/message][twimlet_message_url] нажмите кнопку, чтобы увидеть пустой `<Response>` элемент; в качестве еще [https://twimlets.com/message?Message%5B0%5D=Hello%20World](https://twimlets.com/message?Message%5B0%5D=Hello%20World) одного `<Response>` примера нажмите кнопку, чтобы увидеть элемент, содержащий элемент &lt;Say.&gt;
>
...
Для получения дополнительной информации [https://www.twilio.com/docs/api/twiml](https://www.twilio.com/docs/api/twiml)о TwiML см.
