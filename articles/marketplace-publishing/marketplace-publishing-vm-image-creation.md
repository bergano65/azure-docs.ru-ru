...
* [Установка классического интерфейса командной строки Azure (Azure Classic CLI)](../cli-install-nodejs.md)
...
**Классический Azure CLI (рекомендуется для непрерывной интеграции и интеграции со сторонними решениями (не Windows))**
...
3.  Создайте файл PowerShell (или другой исполняемый скрипт) со следующим кодом и сохраните его на локальном компьютере.
    ...
    c. **`<Permission Start Date>`**: чтобы сохранить время в формате UTC, выберите дату начала за день до текущей даты. Например, если сегодня 26 октября 2016 г., выберите значение 25.10.2016. Если используется Azure CLI версии 2.0 или более поздней, укажите дату и время для дат начала и окончания, например 10-25-2016T00:00:00Z.

    d. **`<Permission End Date>`**: выберите дату, которая наступит через 3 недели после даты **Start Time** (Время начала). Нужно использовать значение **11.02.2016**. Если используется Azure CLI версии 2.0 или более поздней, укажите дату и время для дат начала и окончания, например 11-02-2016T00:00:00Z.
