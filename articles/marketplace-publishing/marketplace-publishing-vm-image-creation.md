...
Скачав VHD с операционной системой, используйте клиент Hyper-V и настройте виртуальную машину, чтобы начать создание номера SKU. Подробные инструкции можно найти по следующей ссылке на сайт TechNet: [Установка Hyper-V и настройка виртуальной машины](https://technet.microsoft.com/library/hh846766.aspx).
...
10. Выполните следующие действия, чтобы создать URI подписанного URL-адреса для VHD-образа.
    ...
    b. **Access permitted to** (Доступ разрешен до). Выберите дату, которая наступит через 3 недели после даты **Access permitted from** (Доступ разрешен с).
...
7.  Обновите время начала, время окончания срока действия и разрешения, как показано ниже.
    ...
    b.  **Expiry Time:** (Время окончания срока действия). Выберите дату, которая наступит через 3 недели после даты **Start Time** (Время начала).
...
3.  Создайте файл Bash (или другой эквивалентный исполняемый скрипт) с приведенным ниже кодом и сохраните его на локальном компьютере.
    ...
    b. **`<VHD Blob Name>`**: укажите имя большого двоичного объекта VHD.

    Выберите дату, которая наступит не ранее чем через 3 недели после даты начала (по умолчанию — время создания маркера SAS). Например, **2018-10-11T23:56Z**.

    Ниже приведен пример кода после обновления нужных параметров.     export AZURE_STORAGE_ACCOUNT=vhdstorage1ba78dfb6bc2d8     EXPIRY=$(date -d "3 weeks" '+%Y-%m-%dT%H:%MZ')     CONTAINER_SAS=$(az storage container generate-sas -n vhds --permissions rl --expiry $EXPIRY -otsv)     BLOB_URL=$(az storage blob url -c vhds -n osdisk_1ba78dfb6b.vhd -otsv)     echo $BLOB_URL\?$CONTAINER_SAS
