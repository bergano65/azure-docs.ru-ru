...
* Портал Azure: https://portal.azure.com/#gallery/{PublisherNamespace}.{OfferIdentifier}{SKUIDdentifier}  
...
В следующем разделе приводятся рекомендации по созданию образа виртуальной машины Linux для Azure Marketplace. Пошаговые инструкции см. в статье о [создании настраиваемого образа виртуальной машины Linux](../virtual-machines/linux/create-upload-generic.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json).
...
Используя портал Microsoft Azure, вы можете создать виртуальную машину на основе одобренного базового образа всего за несколько простых шагов. В следующем списке приведен обзор этого процесса:
...
3. Следуйте инструкциям на экране, чтобы создать VM с помощью выбранного базового образа. Укажите имя узла (компьютера), имя пользователя (зарегистрированного как администратор), а также пароль для виртуальной машины.
...
6. Выберите **расположение** для развертывания.
    ...
    b.    Если вы планируете разработку образа в Azure, рекомендуем с самого начала использовать один из регионов Microsoft Azure в США. Это позволит ускорить копирование VHD, которое выполняет Майкрософт от вашего имени при отправке образа для сертификации.
...
Дополнительные сведения об RDP можно найти на сайте MSDN в статье [Подключение к виртуальной машине Azure с помощью RDP или SSH](https://msdn.microsoft.com/library/azure/dn535788.aspx).

**Настройка виртуальной машины и создание номера SKU**

Скачав VHD с операционной системой, используйте клиент Hyper-V и настройте виртуальную машину, чтобы начать создание номера SKU. Подробные инструкции можно найти на сайте TechNet в статье об [установке Hyper-V и настройке виртуальной машины](https://technet.microsoft.com/library/hh846766.aspx).
...
* Вы можете запустить следующую команду из каталога %windir%\System32\Sysprep.
  ...
  Рекомендации по подготовке операционной системы к использованию команды sysprep приведены на шаге 1 статьи MSDN [Отправка универсального диска VHD и создание виртуальных машин с его помощью в Azure](../virtual-machines/windows/classic/createupload-vhd.md?toc=%2fazure%2fvirtual-machines%2fwindows%2fclassic%2ftoc.json).

## <a name="4-deploy-a-vm-from-your-vhds"></a>4. Развертывание виртуальной машины из дисков VHD
Передав виртуальные жесткие диски (подготовленный VHD с ОС и от нуля до нескольких VHD данных) в учетную запись хранения Azure, их можно зарегистрировать как пользовательский образ виртуальной машины. Затем можно приступать к тестированию образа. Так как VHD с ОС является обобщенным, вы не можете напрямую развертывать виртуальную машину, указав URL-адрес этого VHD.
...
### <a name="set-up-the-necessary-tools-powershell-and-azure-classic-cli"></a>Настройка необходимых средств, PowerShell и классического интерфейса Azure CLI
...
Рекомендации по записи виртуальной машины с помощью API, PowerShell или Azure CLI см. в указанных ниже источниках.
...
Развернуть большую виртуальную машину из только что созданного универсального образа можно с помощью приведенных ниже командлетов.
...
Приведенные ниже инструкции применимы только для неуправляемых дисков, которые являются единственным поддерживаемым типом.
...
Рекомендуемые средства перечислены ниже (три варианта):
...
10. Выполните следующие действия, чтобы создать URI подписанного URL-адреса для VHD-образа.
    ...
    b. **Access permitted to** (Доступ разрешен до). Выберите дату, которая наступит не ранее чем через три недели после даты **Access permitted from** (Доступ разрешен с).
    ...
    Е. Проверьте содержимое поля **Созданный URI подписанного URL-адреса для контейнера**согласно следующим условиям.
       ...
       - В конце подписи должны быть символы **=rl**. Это значение показывает, что права на чтение и создание списков успешно предоставлены.
       - В середине подписи должны быть символы **sr=c**. Это значение показывает, что у вас есть доступ на уровне контейнера.
...
12. Скопируйте URI подписанного URL-адреса. Вставьте этот URI на портал публикации.
...
4.  Свяжите Обозреватель службы хранилища Microsoft Azure с подпиской. Для этого войдите в учетную запись.
...
7.  Обновите время начала, время окончания срока действия и разрешения, как показано ниже.
    ...
    a.  **Время начала.** Чтобы сохранить время в формате UTC, выберите дату начала за день до текущей даты. Например, если сегодня 6 октября 2014 г., выберите 05.10.2014.

    b.  **Время окончания срока действия.** Выберите дату, которая наступит не ранее чем через три недели после **времени начала**.

    c.  **Разрешения.** Выберите разрешения на **создание списков** и **чтение**.
...
8.  Скопируйте подписанный URL-адрес контейнера.
    ...
    - URI должен включать имя файла образа и расширение **.vhd**.
    - В середине подписи должны быть символы **sp=rl**. Это значение показывает, что права на чтение и создание списков успешно предоставлены.
    - В середине подписи должны быть символы **sr=c**. Это значение показывает, что у вас есть доступ на уровне контейнера.

9.  Проверьте работу созданного URI подписанного URL-адреса в браузере. Должен начаться процесс скачивания.

10. Скопируйте URI подписанного URL-адреса. Вставьте этот URI на портал публикации.

11. Повторите эти шаги для каждого виртуального жесткого диска в номере SKU.

**Azure CLI 2.0 (рекомендуется для непрерывной интеграции и интеграции со сторонними решениями)**

Ниже перечислены шаги по созданию URL-адреса SAS с помощью классического Azure CLI.

[!INCLUDE [outdated-cli-content](../../includes/contains-classic-cli-content.md)]

1.  Скачайте Microsoft Azure CLI [отсюда](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest). Также можно использовать ссылки для **[Windows](https://docs.microsoft.com/cli/azure/install-azure-cli-windows?view=azure-cli-latest)** и **[MAC OS](https://docs.microsoft.com/cli/azure/install-azure-cli-macos?view=azure-cli-latest)**.

2.  Установите инструмент после скачивания.

3.  Создайте файл Bash (или другой эквивалентный исполняемый скрипт) с приведенным ниже кодом и сохраните его на локальном компьютере.
    ...
    a. **`<Storage Account Name>`**. Укажите имя учетной записи хранения.

    b. **`<VHD Blob Name>`**. Укажите имя BLOB-объекта VHD.

    Выберите дату, которая наступит не ранее чем через три недели после даты начала (по умолчанию — время создания маркера SAS). Например, `2018-10-11T23:56Z`.

    Пример кода после обновления нужных параметров:     export AZURE_STORAGE_ACCOUNT=vhdstorage1ba78dfb6bc2d8     EXPIRY=$(date -d "three weeks" '+%Y-%m-%dT%H:%MZ')     CONTAINER_SAS=$(az storage container generate-sas -n vhds --permissions rl --expiry $EXPIRY -otsv)     BLOB_URL=$(az storage blob url -c vhds -n osdisk_1ba78dfb6b.vhd -otsv)     echo $BLOB_URL\?$CONTAINER_SAS

4.  Выполните скрипт. Вы получите URL-адрес SAS для доступа на уровне контейнера.

5.  Проверьте URL-адрес SAS.
...
9.  Скопируйте URI подписанного URL-адреса. Вставьте этот URI на портал публикации.
...
|---|---|---|---|
|Сбой при копировании образов: символ "?" не найден в URL-адресе SAS.|сбой: Идет копирование образов. Не удалось скачать BLOB-объект с помощью указанного URI SAS.|Обновите URL-адрес SAS с помощью рекомендуемых средств.|[https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/](https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/)|
|Сбой при копировании образов: параметры st и se отсутствуют в URL-адресе SAS.|сбой: Идет копирование образов. Не удалось скачать BLOB-объект с помощью указанного URI SAS.|Обновите URL-адрес SAS, указав правильные даты начала и окончания срока действия.|[https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/](https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/)|
|Сбой при копировании образов: параметр sp=rl отсутствует в URL-адресе SAS.|сбой: Идет копирование образов. Не удалось скачать BLOB-объект с помощью указанного URI SAS.|Обновите URL-адрес SAS, указав разрешения на чтение и создание списка.|[https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/](https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/)|
|Сбой при копировании образов: URL-адрес SAS в имени VHD-файла содержит пробелы.|сбой: Идет копирование образов. Не удалось скачать BLOB-объект с помощью указанного URI SAS.|Обновите URL-адрес, удалив пробелы.|[https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/](https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/)|
|Сбой при копировании образов: ошибка авторизации URL-адреса SAS.|сбой: Идет копирование образов. Не удалось скачать BLOB-объект из-за ошибки авторизации.|Создайте URL-адрес SAS повторно.|[https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/](https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/)|
|Сбой при копировании изображений означает, что параметры URL-адреса SAS "st" и "se" не имеют полной спецификации даты и времени|сбой: Идет копирование образов. Не удалось загрузить большой двоичный объект из-за неправильного URL-адреса SAS |Параметры URL-адреса SAS даты начала и окончания ("st", "se") должны иметь полную спецификацию даты и времени, например 11-02-2017T00:00:00Z, а не только дату или сокращенную версию для времени. Этот сценарий можно встретить, используя Azure CLI версии 2.0 или более поздней. Обязательно укажите полную спецификацию даты и времени и повторно создайте URL-адрес SAS.|[https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/](https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/)|

## <a name="next-step"></a>Дальнейшие действия
Закончив с настройкой данных SKU, перейдите к статье [Завершение создания предложения с маркетинговыми материалами][link-pushstaging]. На этом этапе публикации следует предоставить маркетинговые материалы, цены и другие сведения до выполнения **шага 3 (тестирование предложения виртуальной машины в промежуточной среде)**. На шаге 3 тестируются различные варианты использования перед развертыванием предложения в Azure Marketplace для предоставления общего доступа и возможности приобретения.  
