...
Для работы с этим руководством вам потребуется приложение Azure IoT Central. Если вы завершили работу с руководством по [созданию приложения Azure IoT Central](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json), вы можете повторно использовать созданное в нем приложение. В противном случае выполните следующие шаги для создания пустого приложения Azure IoT Central:
...
2. Введите адрес электронной почты и пароль, которые вы используете для доступа к подписке Azure:
    ...
    ![Ввод учетной записи организации](./media/tutorial-define-device-type-experimental/sign-in.png)

3. Чтобы начать создание приложения Azure IoT Central, щелкните **Новое приложение**:

    ![Страница диспетчера приложений Azure IoT Central](./media/tutorial-define-device-type-experimental/iotcentralhome.png)
...
4. Чтобы создать приложение Azure IoT Central, сделайте следующее:
    ...
    * Выберите **Пробная версия**. Для создания приложения в рамках пробной версии вам не потребуется подписка Azure.
       ...
       Дополнительные сведения о каталогах и подписках см. в статье [Создание приложения Azure IoT Central](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json).
    ...
    * Нажмите кнопку **Создать**.
    ...
    ![Страница создания приложения Azure IoT Central](./media/tutorial-define-device-type-experimental/iotcentralcreate.png)

    Дополнительные сведения см. в [кратком руководстве по созданию приложения](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json).
...
Чтобы добавить новый шаблон устройства в приложение, вам нужно перейти на страницу **шаблонов устройств**. Для этого нажмите кнопку **Шаблоны устройств** в меню навигации слева.

![Страница шаблонов устройств](./media/tutorial-define-device-type-experimental/devicetemplates.png)
...
1. На странице **Шаблоны устройств** нажмите кнопку **+ Создать**:

    ![Страница шаблонов устройств, создание шаблонов устройств](./media/tutorial-define-device-type-experimental/newtemplate.png)

3. На странице **Настраиваемый шаблон устройства** введите **подключенный кондиционер** в качестве имени вашего устройства, а затем нажмите кнопку **Создать**. Можно также передать изображение устройства, которое будет видимым для операторов в обозревателе устройства:

    ![Пользовательское устройство](./media/tutorial-define-device-type-experimental/createcustomdevice.png)
...
4. В шаблоне устройства **Подключенный кондиционер** откройте вкладку **Измерения**, где вы определяете телеметрию. Каждый шаблон устройства, который вы определяете, имеет отдельные вкладки, где вы можете:
    ...
    * настроить _панель мониторинга_ устройства для операторов.
    ...
    ![Измерения кондиционера](./media/tutorial-define-device-type-experimental/airconmeasurements.png)

    > [!NOTE]
    > Чтобы изменить имя шаблона устройства, щелкните имя шаблона вверху страницы.

5. Чтобы добавить данные телеметрии температуры, щелкните **New Measurement** (Новое измерение). Затем выберите **Телеметрия** как тип измерения:

    ![Измерения подключенного кондиционера](./media/tutorial-define-device-type-experimental/airconmeasurementsnew.png)

6. Каждый тип телеметрии, который вы определяете для шаблона устройства, включает в себя [параметры конфигурации](howto-set-up-template-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json), такие как:
    ...
    * параметры имитации.
    ...
    ![Настройка моделирования температуры](./media/tutorial-define-device-type-experimental/temperaturesimulation.png)
...
7. Через некоторое время на вкладке **измерений** отобразится диаграмма телеметрии температуры с вашего имитированного подключенного кондиционера. Используйте элементы управления для настройки видимости, агрегации или для редактирования определения телеметрии:
    ...
    ![Просмотр моделирования температуры](./media/tutorial-define-device-type-experimental/viewsimulation.png)

8. Вы также можете настроить диаграмму, используя элементы управления **График**, **С накоплением** и **Изменить диапазон времени**:

    ![Настройка диаграммы](./media/tutorial-define-device-type-experimental/customizechart.png)
...
1. Чтобы добавить измерение события **ошибки вентилятора**, щелкните **+ New Measurement** (+ Новое измерение). Затем выберите **Событие** как тип измерения:

    ![Измерения подключенного кондиционера](./media/tutorial-define-device-type-experimental/eventnew.png)

2. Каждый тип события, которое вы определяете для шаблона устройства, включает в себя [параметры конфигурации](howto-set-up-template-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json), такие как:
   ...
   * серьезность.
    ...
    ![Настройка измерения событий](./media/tutorial-define-device-type-experimental/eventconfiguration.png)
...
3. Через некоторое время на вкладке **измерений** отобразится диаграмма случайно созданных событий с вашего имитированного подключенного кондиционера. Используйте элементы управления для настройки видимости или для редактирования определения события:
    ...
    ![Просмотр имитации события](./media/tutorial-define-device-type-experimental/eventview.png)

1. Чтобы просмотреть дополнительные сведения о событии, щелкните его имя на диаграмме.

    ![Просмотр подробностей о событии](./media/tutorial-define-device-type-experimental/eventviewdetail.png)
...
1. Чтобы добавить измерение состояния **Режим вентилятора**, щелкните **+ New Measurement** (+ Новое измерение). Затем выберите **Состояние** как тип измерения:

    ![Измерения состояния подключенного кондиционера](./media/tutorial-define-device-type-experimental/statenew.png)

2. Каждый тип состояния, который вы определяете для шаблона устройства, включает в себя [параметры конфигурации](howto-set-up-template-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json), такие как:
   ...
   * цвет для каждого значения.
    ...
    ![Настройка измерения состояния](./media/tutorial-define-device-type-experimental/stateconfiguration.png)
...
3. Через некоторое время на странице **измерений** отобразится диаграмма произвольно созданных состояний с вашего имитированного подключенного кондиционера. Используйте элементы управления для настройки видимости или для редактирования определения состояния:
    ...
    ![Просмотр имитации состояния](./media/tutorial-define-device-type-experimental/stateview.png)

4. Если в течение небольшого периода времени количество отправленных устройством точек данных слишком велико, измерение состояния отображается с другими визуальными элементами. Щелкните диаграмму, чтобы увидеть все точки данных за этот период времени, которые будут отображаться в хронологическом порядке. Вы также можете сузить диапазон времени, чтобы увидеть подробные сведения об измерениях.
...
* Вы можете использовать _свойства_, чтобы определить метаданные, связанные с устройством. Существует две категории свойств:
    ...
    * Используйте _свойства устройства_, чтобы позволить устройству отправлять значения свойств в ваше приложение. Сообщаемые свойства могут обновляться только устройством. Для оператора свойства устройства доступны только для чтения. В сценарии подключения кондиционера версия встроенного ПО и серийный номер устройства — это свойства, сообщаемые устройством.
    ...
    Дополнительные сведения см. в разделе [Свойства](howto-set-up-template-experimental.md#properties) руководства по настройке шаблона устройства.
...
2. Вы можете создавать параметры различных типов, например цифровые или текстовые. Щелкните **Номер**, чтобы добавить параметр номера для устройства.
...
3. Для настройки параметра **Set Temperature** (Установить температуру) используйте сведения в следующей таблице:
    ...
    ![Настройка параметра установки температуры](./media/tutorial-define-device-type-experimental/configuresetting.png)
...
4. Вы можете настроить макет вкладки **Параметры**, перемещая и изменяя размеры плиток параметров:
    ...
    ![Настройка макета параметров](./media/tutorial-define-device-type-experimental/settingslayout.png)
...
1. Вы можете создавать свойства устройства разных типов, например цифровые или текстовые. Чтобы добавить свойство расположения в шаблон устройства, выберите **Расположение**. Для настройки свойства расположения используйте сведения из следующей таблицы:
    ...
    ![Настройка свойств устройства](./media/tutorial-define-device-type-experimental/configureproperties.png)
...
1. Для настройки свойства даты последнего обслуживания используйте сведения из следующей таблицы:
    ...
    ![Настройка свойств устройства](./media/tutorial-define-device-type-experimental/configureproperties2.png)
...
1. Чтобы настроить версию встроенного ПО, используйте сведения в следующей таблице:
    ...
    ![Настройка версии встроенного ПО](./media/tutorial-define-device-type-experimental/configureproperties3.png)
...
1. Чтобы настроить серийный номер, используйте информацию в следующей таблице:
    ...
    ![Настройка серийного номера](./media/tutorial-define-device-type-experimental/configureproperties4.png)
...
1. Для настройки новой команды используйте сведения в следующей таблице:
    ...
    ![Подготовка к добавлению параметра](./media/tutorial-define-device-type-experimental/commandsecho1.png)
...
1. Настройте компонент **Линейная диаграмма**, используя информацию в следующей таблице:
    ...
    | Показатели     | Температура (щелкните значок **видимости** рядом с параметром **температуры**) |

    ![Параметры линейной диаграммы](./media/tutorial-define-device-type-experimental/linechartsettings.png)

    Нажмите кнопку **Сохранить**.

1. Щелкните **Журнал событий**, используя информацию в следующей таблице:
    ...
    ![Параметры линейной диаграммы](./media/tutorial-define-device-type-experimental/dashboardeventchartsetting.png)
...
1. Настройте компонент **State History** (История состояния), используя информацию в следующей таблице:
    ...
    ![Параметры линейной диаграммы](./media/tutorial-define-device-type-experimental/dashboardstatechartsetting.png)

    Нажмите кнопку **Сохранить**.

1. Чтобы добавить на панель мониторинга параметры и свойства устройства, щелкните **Settings and Properties** (Параметры и свойства). Щелкните **Add/Remove** (Добавить/удалить), чтобы добавить параметры или свойства, которые должны отображаться на панели мониторинга.
...
1. Настройте компонент **Settings and Properties** (Параметры и свойства), используя сведения в следующей таблице:
    ...
    ![Настройка свойства температуры](./media/tutorial-define-device-type-experimental/propertysettings4.png)
...
1. Теперь вы видите смоделированные данные для подключенного кондиционера на панели мониторинга. Вы можете изменить плитки и макет для панели мониторинга:
    ...
    ![Просмотр панели мониторинга](./media/tutorial-define-device-type-experimental/dashboard.png)
