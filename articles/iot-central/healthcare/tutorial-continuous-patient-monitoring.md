...
Группы устройств позволяют логически группировать набор устройств, чтобы затем выполнять массовые запросы или операции с ними. 

Если щелкнуть вкладку группы устройств, вы увидите, что для каждого из шаблонов устройств в приложении созданы группы устройств по умолчанию. Вы заметите, что также были созданы два дополнительных примера групп устройств под названием "Подготовка устройств" и "Устройства с устаревшим встроенным ПО". Эти примеры групп устройств будут использоваться в качестве входных данных для выполнения некоторых [заданий](#jobs).
...
Задания позволяют выполнять массовые операции в наборе устройств, используя [группы устройств](#device-groups) в качестве входных данных. Мы добавили в шаблон приложения два примера заданий, которые оператору решения может потребоваться выполнить в некоторый момент жизненного цикла устройства:
* **Обновление встроенного ПО шарнирного бандажа на колено** Это задание найдет устройства в группе устройств с устаревшим встроенным ПО и запустит команду, чтобы обновить эти устройства до последней версии встроенного ПО шарнирного бандажа на колено. В этом примере задания предполагается, что устройства имеют возможности получать команду Update и файлы встроенного ПО из облака напрямую.  

* **Повторная подготовка устройств** Если у вас есть набор устройств, которые недавно были возвращены в больницы, и их необходимо повторно подготовить к работе со следующим набором пациентов, можно запустить это задание, чтобы выполнить массовое обновление этих устройств подготовки. В этом случае мы берем все устройства из группы устройств с именем "Подготовка устройства" и выполним команду для их инициализации. 
...
Экспорт данных позволяет непрерывно экспортировать данные устройства IoT Central в другие службы Azure, включая [Azure API для FHIR](concept-continuous-patient-monitoring-architecture.md#export-to-azure-api-for-fhir).
