...
- Создайте шаблон устройства в IoT Central а затем реализуйте его модель возможностей устройства в коде устройства.
- Импортируйте модель возможностей устройства из [каталога устройств сертификации Azure для IOT](https://aka.ms/iotdevcat) , а затем добавьте свойства облака, настройки и панели мониторинга, необходимые для приложения IOT Central.
- Создание модели возможностей устройства с помощью Visual Studio Code. Реализуйте код устройства из модели. Вручную импортируйте модель возможностей устройства в приложение IoT Central, а затем добавьте свойства облака, настройки и панели мониторинга, необходимые для приложения IoT Central.
- Создание модели возможностей устройства с помощью Visual Studio Code. Реализуйте код устройства на основе модели и подключите реальное устройство к IoT Centralному приложению, используя подключение "устройство — первое". IoT Central находит и импортирует модель возможностей устройства из общедоступного репозитория. Затем можно добавить любые облачные свойства, настройки и панели мониторинга, необходимые IoT Central приложении к шаблону устройства.
...
| Поле | Описание |
| ----- | ----------- |
| Название | Отображаемое имя для значения телеметрии, используемое на панелях мониторинга и формах. |
| Название | Имя поля в сообщении телеметрии. IoT Central создает значение для этого поля из отображаемого имени, но при необходимости можно выбрать собственное значение. |
...
| Схема | Тип данных телеметрии, например Double, String или Vector. Доступные варианты определяются семантическим типом. Схема недоступна для типов семантических событий и состояний. |
| severity | Доступно только для семантического типа события. **Ошибка**, **информация**или **предупреждение**. |
...
| Добавление примечаний | Любые комментарии о возможности телеметрии. |
...
| Поле | Описание |
| ----- | ----------- |
| Название | Отображаемое имя значения свойства, используемого на панелях мониторинга и формах. |
| Название | Имя свойства. IoT Central создает значение для этого поля из отображаемого имени, но при необходимости можно выбрать собственное значение. |
...
| Схема | Тип данных свойства, например Double, String или Vector. Доступные варианты определяются семантическим типом. Схема недоступна для типов семантических событий и состояний. |
| Записываемый | Если свойство недоступно для записи, устройство может передавать значения свойств в IoT Central. Если свойство доступно для записи, устройство может передавать значения свойств в IoT Central и IoT Central может передавать обновления свойств на устройство.
| severity | Доступно только для семантического типа события. **Ошибка**, **информация**или **предупреждение**. |
| Значения состояния | Доступно только для семантического типа состояния. Определите возможные значения состояния, каждое из которых имеет отображаемое имя, имя, тип перечисления и значение. |
| Единица измерения | Единица измерения для значения свойства, например **миль/ч**, **%** или  **&deg;C**. |
| Единица дисплея | Отображаемая единица для использования на панелях мониторинга и формах. |
| Добавление примечаний | Любые комментарии о возможности свойства. |
...
| Поле | Описание |
| ----- | ----------- |
| Название | Отображаемое имя команды, используемой на панелях мониторинга и формах. |
| Название | Имя команды. IoT Central создает значение для этого поля из отображаемого имени, но при необходимости можно выбрать собственное значение. |
| Тип возможности | Command |
| Command | Синчронаусексекутионтипе. |
| Добавление примечаний | Любые комментарии о возможности команды. |
| Описание | Описание возможности команды. |
| Запрос | Если параметр включен, определение параметра запроса включает в себя: имя, отображаемое имя, схему, единицу и единицу показа. |
| Отклик | Если параметр включен, определение ответа команды включает в себя: имя, отображаемое имя, схему, единицу и единицу показа. |
...
## <a name="add-cloud-properties"></a>Добавить свойства облака
...
| Поле | Описание |
| ----- | ----------- |
| Название | Отображаемое имя для значения облачного свойства, используемого на панелях мониторинга и формах. |
| Название | Имя облачного свойства. IoT Central создает значение для этого поля из отображаемого имени, но при необходимости можно выбрать собственное значение. |
| Семантический тип | Семантический тип свойства, например "температура", "состояние" или "событие". Выбор семантического типа определяет, какие из следующих полей доступны. |
| Схема | Тип данных облачного свойства, например Double, String или Vector. Доступные варианты определяются семантическим типом. |

## <a name="add-customizations"></a>Добавление настроек

Настройки следует использовать, когда необходимо изменить импортированный интерфейс или добавить к возможности функции, относящиеся к IoT Central. Можно настраивать только поля, которые не нарушают совместимость интерфейса. Например, вы можете просматривать:

- Настройте отображаемое имя и единицы возможности.
- Добавьте цвет по умолчанию, используемый при отображении значения на диаграмме.
- Укажите начальные, минимальные и максимальные значения для свойства.

Нельзя настроить имя возможности или тип возможности. Если есть изменения, которые нельзя внести в раздел " **Настройка** ", вам потребуется версия шаблона устройства и интерфейса, чтобы изменить возможность.

### <a name="generate-default-views"></a>Создание представлений по умолчанию

Создание представлений по умолчанию — это быстрый способ приступить к визуализации важной информации об устройстве. Для шаблона устройства будет создано до трех представлений по умолчанию:
...
После публикации шаблона устройства можно вносить только ограниченные изменения в модель возможностей устройства. Чтобы изменить интерфейс, необходимо [создать и опубликовать новую версию](./howto-version-device-template-pnp.md).
...
> [Создание новой версии](howto-version-device-template-pnp.md?toc=/azure/iot-central-pnp/toc.json&bc=/azure/iot-central-pnp/breadcrumb/toc.json)
> шаблона устройства[Подключение устройства MXChip IOT DevKit к приложению Azure IOT Central](howto-connect-devkit-pnp.md?toc=/azure/iot-central-pnp/toc.json&bc=/azure/iot-central-pnp/breadcrumb/toc.json)
