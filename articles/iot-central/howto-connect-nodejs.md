...
| Название | Имя поля  | Units | Min | Max | Число десятичных знаков |
| ------------ | ----------- | ----- | --- | --- | -------------- |
...
### <a name="location-measurements"></a>Расположение измерения

Добавьте следующие измерения расположение на **измерения** страницы:

| Название | Имя поля  |
| ------------ | ----------- |
| Location     | location    |

Расположение измерения, тип данных состоит из двух с плавающей запятой для широты и долготы и необязательно с плавающей запятой для Высота над уровнем моря.

Введите имена полей из таблицы в шаблон устройства. Если имена полей не совпадают имена свойств в соответствующий код устройства, расположение невозможно отобразить в приложении.
...
| Название    | Имя поля     | Units | Десятичные знаки | Min | Max  | Initial |
| --------------- | -------------- | ----- | -------- | --- | ---- | ------- |
...
1. Чтобы отправить данные телеметрии, состояние, события и измерения расположение в приложение Azure IoT Central, добавьте следующую функцию в файл:
...
* Просмотр расположения на **измерения** страницы:

    ![Просмотр расположения измерения](media/howto-connect-nodejs/viewlocation.png)
