...
1. Добавьте следующие объявления переменных в файл:
  ...
  > [!NOTE]
  > Решение Azure IoT Central переведено на использование Службы подготовки устройств к добавлению в центр Интернета вещей Azure (DPS) для подключения всех устройств. Выполните инструкции по [получению строки подключения устройства](concepts-connectivity.md#getting-device-connection-string) и продолжите работу с оставшейся частью руководства. Подробные инструкции см. в разделе [Подготовка клиентского кода](tutorial-add-device.md#prepare-the-client-code) руководства по добавлению устройств.
