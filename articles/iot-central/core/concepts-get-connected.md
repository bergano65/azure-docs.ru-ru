...
IoT Central автоматически связывает устройство с шаблоном устройства при подключении устройства. Устройство отправляет идентификатор модели при подключении. IoT Central использует идентификатор модели для идентификации шаблона устройства для конкретной модели устройства. Процесс обнаружения выполняется следующим образом.

1. Если шаблон устройства уже опубликован в IoT Central приложении, устройство связывается с шаблоном устройства.
1. Если шаблон устройства еще не опубликован в IoT Central приложении, IoT Central ищет модель устройства в репозитории общедоступной модели. Если IoT Central находит модель, она использует ее для создания базового шаблона устройства.
...
- Ознакомьтесь с примером кода, в котором показано, как использовать маркеры SAS в [руководстве. Создание и подключение клиентского приложения к приложению IoT Central Azure (учебник-Connect-Device-nodejs.md).
