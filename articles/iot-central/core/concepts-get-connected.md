...
Дополнительные сведения см. в руководстве по [созданию и подключению клиентского приложения Node. js к приложению Azure IOT Central](./tutorial-connect-device-nodejs.md) .
...
Перед подключением устройства к сертификату X. 509 добавьте и проверьте промежуточный или корневой сертификат X. 509 в приложении. Устройства должны использовать конечные сертификаты X. 509, созданные на основе корневого или промежуточного сертификата.
...
Перейдите в раздел **администрирование > подключение устройства > управление основным сертификатом** и добавьте корневой или промежуточный сертификат X. 509, который вы используете для создания сертификатов устройств.

![Параметры подключения](media/concepts-get-connected/manage-x509-certificate.png)

Проверка владельца сертификата гарантирует, что у пользователя, который отправляет сертификат, закрытый ключ сертификата. Чтобы проверить сертификат, сделайте следующее:

  1. Нажмите кнопку рядом с **кодом проверки** , чтобы создать код.
  1. Создайте сертификат проверки X. 509 с кодом проверки, созданным на предыдущем шаге. Сохраните сертификат как CER-файл.
  1. Отправьте подписанный сертификат проверки и нажмите кнопку " **проверить**". Сертификат помечается как **проверенный** при успешной проверке.
...
Создание конечных сертификатов X. 509 для устройств с помощью переданного корневого или промежуточного сертификата. Используйте **идентификатор устройства** в качестве `CNAME` значения в конечных сертификатах. Код устройства нуждается в значении **области идентификатора** для приложения, **идентификатора устройства**и соответствующего сертификата устройства.

#### <a name="sample-device-code"></a>Пример кода устройства

В следующем примере из [пакета SDK для Node. js для Azure IOT](https://github.com/Azure/azure-iot-sdk-node/blob/master/provisioning/device/samples/register_x509.js) показано, как клиентское устройство Node. js использует конечный сертификат X. 509 и службу DPS для регистрации в приложении IOT Central:
...
- [Средства для пакета SDK для устройства подготовки устройств Azure IOT](https://github.com/Azure/azure-iot-sdk-node/blob/master/provisioning/tools/readme.md): коллекция средств Node. js, которые можно использовать для создания и проверки сертификатов и ключей X. 509.
...
1. Скопируйте первичный ключ группы IoT Central приложения:

    ![Первичный ключ SAS группы приложений](media/concepts-get-connected/group-sas-keys.png)
...
1. [Добавьте и проверьте корневой или промежуточный сертификат X. 509](#connect-devices-using-x509-certificates) для приложения IOT Central.

1. Создайте конечные сертификаты для устройств, используя корневой или промежуточный сертификат, добавленный в приложение IoT Central. Используйте идентификаторы устройств `CNAME` в нижнем регистре в качестве в конечных сертификатах.
...
- **Сертификаты X. 509:** Чтобы создать отдельную регистрацию с помощью сертификатов X. 509, откройте страницу **Подключение устройства** , в качестве метода подключения выберите вариант **отдельная регистрация** , а в качестве механизма — **Certificates (X. 509)** . Сертификаты устройств, используемые с отдельной записью регистрации, имеют требование, что для издателя и субъекта CN задано значение идентификатора устройства.
    ...
    > [!TIP]
    > Для тестирования можно использовать [инструменты для пакета SDK устройства подготовки устройств Azure IOT для Node. js](https://github.com/Azure/azure-iot-sdk-node/tree/master/provisioning/tools) , чтобы создать самозаверяющий сертификат.`node create_test_cert.js device "mytestdevice"`
