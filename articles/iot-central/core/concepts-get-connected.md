...
Чтобы выполнить блочное подключение устройств с помощью сертификатов X. 509, сначала зарегистрируйте устройства в приложении, используя CSV-файл для [импорта идентификаторов и имен](howto-manage-devices.md#import-devices)устройств. ИДЕНТИФИКАТОР устройства может содержать буквы, цифры и `-` символ.
...
1. Создайте конечные сертификаты для устройств, используя корневой или промежуточный сертификат, добавленный в приложение IoT Central. Используйте идентификаторы устройств в качестве `CNAME` в конечных сертификатах. ИДЕНТИФИКАТОР устройства может содержать буквы, цифры и `-` символ.
