...
- Подключать устройства с использованием стандартных отраслевых сертификатов X.509.
...
1. В приложении IoT Central _добавьте и проверьте промежуточный или корневой сертификат X. 509_ , который вы используете для создания сертификатов устройств.
    ...
    - Перейдите в раздел **администрирование > подключение устройства > сертификаты (X. 509)** и добавьте корневой или промежуточный сертификат X. 509, который вы используете для создания сертификатов конечных устройств.
      ...
      ![Параметры подключения](media/concepts-get-connected/connection-settings.png)
    ...
    - Проверка принадлежности сертификата гарантирует, что у отправки сертификата есть закрытый ключ сертификата. Чтобы проверить сертификат, сделайте следующее:
        ...
        - Отправьте подписанный сертификат проверки и нажмите кнопку " **проверить**".
          ...
          ![Параметры подключения](media/concepts-get-connected/verify-cert.png)
...
![Параметры подключения](media/concepts-get-connected/device-connection-flow1.png)
...
1. Настройте параметры подключения:
    ...
    ![Параметры подключения SAS](media/concepts-get-connected/connection-settings-sas.png)
...
1.  При использовании DPS для подключения устройств к IoT Central убедитесь, что строка подключения устройства (центр Интернета вещей) не сохранена или кэширована. Чтобы повторно подключить устройства, пройдите через обычный поток регистрации устройств DPS, чтобы получить правильную строку подключения устройства. Если строка подключения кэшируется, программное обеспечение устройства работает с риском наличия устаревшей строки подключения в сценариях, где IoT Central обновил базовый центр Интернета вещей Azure. 
