...
1. Используйте `az iot central device compute-device-key` команду, чтобы создать ключи SAS устройства. Используйте групповой первичный ключ из предыдущего шага. Идентификаторы устройств должны быть в нижнем регистре:
