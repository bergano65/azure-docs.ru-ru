...
- [Подключение устройств с помощью функций Plug and Play Интернета вещей (Предварительная версия)](#connect-devices-with-iot-plug-and-play-preview)
...
1. При переключении на устройство оно сначала подключается к DP для получения сведений о регистрации IoT Central.
    ...
    > [!NOTE]
    > Сведения о том, как автоматически связать устройство с шаблоном устройства, см. в статье [Подключение устройств с помощью Интернета вещей Plug and Play (Предварительная версия)](#connect-devices-with-iot-plug-and-play-preview).
...
## <a name="connect-devices-with-iot-plug-and-play-preview"></a>Подключение устройств с помощью Plug and Play Интернета вещей (Предварительная версия)

Одной из основных функций Интернета вещей Plug and Play (Предварительная версия) с IoT Central является возможность связывать шаблоны устройств автоматически при подключении устройства. Наряду с учетными данными устройства устройства теперь могут отправить **капабилитимоделид** как часть вызова регистрации устройства. Эта возможность позволяет IoT Central обнаруживать и связывать шаблон устройства с устройством. Процесс обнаружения выполняется следующим образом.
...
> [!NOTE]
> Обратите внимание, что параметр **автоматического утверждения** на странице **Администрирование > подключение к устройству** должен быть включен для автоматического подключения устройств, обнаружения шаблона устройства и начала отправки данных.
