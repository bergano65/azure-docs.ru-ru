...
1. На **странице основной сертификат**отправьте свой основной сертификат X. 509. Это корневой или промежуточный сертификат:
    ...
    :::image type="content" source="media/concepts-get-connected/upload-primary-certificate.png" alt-text="Снимок экрана: Добавление группы регистрации X. 509":::

1. Щелкните **создать код проверки** , чтобы создать код для инструмента, который вы используете для создания сертификата проверки. Затем выберите **проверить** , чтобы отправить сертификат проверки.

1. После успешной проверки вы увидите следующее подтверждение:

    :::image type="content" source="media/concepts-get-connected/verified-primary-certificate.png" alt-text="Снимок экрана: Добавление группы регистрации X. 509":::
...
1. Скопируйте первичный ключ группы из группы регистрации **SAS-IOT-Devices** :
    ...
    :::image type="content" source="media/concepts-get-connected/group-primary-key.png" alt-text="Снимок экрана: Добавление группы регистрации X. 509" параметр **автоматического утверждения** определяет, нужно ли вручную утвердить устройство, прежде чем оно сможет начать отправку данных.
