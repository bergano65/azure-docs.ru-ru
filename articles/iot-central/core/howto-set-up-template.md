...
description: В этой статье, как и разработчик, показано, как создать новый шаблон устройства Azure IoT в приложении Azure IoT Central. Вы определите данные телеметрии, состояние, свойства и команды для своего типа.
author: dominicbetts
ms.author: dobett
ms.date: 12/06/2019
ms.topic: how-to
...
[IoT Plug and Play (предварительный просмотр)](../../iot-pnp/overview-iot-plug-and-play.md) позволяет IoT Central интегрировать устройства без написания какого-либо встроенного кода устройства. В основе IoT Plug and Play (предварительная версия) лежит схема модели возможностей устройства, которая описывает возможности устройства. В приложении IoT Central шаблоны устройств используют эти модели возможностей устройств IoT Plug and Play (предварительная версия).
...
1. Перейдите на страницу **Шаблоны устройств** в приложении IoT Central.
1. Выберите **новый** > **пользовательский**.
...
1. Перейдите к модели возможностей устройства и выберите **интерфейс .**
...
1. На странице **Выбор интерфейса** можно выполнить следующие действия:
    ...
    - Выберите один из стандартных интерфейсов, таких как интерфейс **Device Information.** Стандартные интерфейсы определяют возможности, общие для многих устройств. Эти интерфейсы публикуются с помощью Azure IoT и не поддерживают управление версиями или изменения.
...
| Единицы | Единица для значения телеметрии, например ** &deg;** миль / **ч,** **%** или C . |
...
| Severity | Доступно только для семантического типа события. Возможны такие степени серьезности: **Ошибка**, **Сведения** или **Предупреждение**. |
| Значения состояния | Доступно только для семантического типа состояния. Определите возможные значения состояния, для каждого из которых есть отображаемое имя, название, тип перечисления и значение. |
| Единицы | Единица для стоимости свойства, **%** например, миль / **ч,** или ** &deg;C**. |
| Единица отображения | Отображаемая единица для использования на панелях мониторинга и в формах. |
...
1. Перейдите к шаблону устройства и выберите **Просмотры**.
...
1. Чтобы участок нескольких значений телеметрии на одной плитке диаграммы, выберите значения телеметрии, а затем выберите **Combine**.
...
Чтобы опубликовать шаблон устройства, перейдите к вам шаблон устройства и выберите **Publish**.
...
* создание шаблона устройства Интернета вещей;
...
> [Подключение устройства](howto-connect-devkit.md)
