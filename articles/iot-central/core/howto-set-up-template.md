...
Настройки следует использовать, когда необходимо изменить импортированный компонент или добавить функции, относящиеся к IoT Central, к возможности. Можно настраивать только поля, которые не нарушают совместимость компонентов. Например, с их помощью можно выполнять следующее.
