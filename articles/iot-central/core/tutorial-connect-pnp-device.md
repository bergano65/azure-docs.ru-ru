...
> [!div class="checklist"]
> * Создание устройства IoT Plug and Play (предварительная версия) с использованием DCM в Visual Studio Code.
...
1. Откройте командную строку. Выполните указанную ниже команду, чтобы установить Vcpkg.
...
1. Войдите в созданное приложение IoT Central с помощью шаблона **Пользовательское приложение** в кратком руководстве [Создание приложения Azure IoT Central](./quick-deploy-iot-central.md).

1. Перейдите на страницу **​​Администрирование** и выберите **Подключение устройства**.

1. Запишите **Область идентификатора**. Данное значение понадобится далее в этом руководстве.

1. Выберите группу регистраций **SAS-IoT-Devices**. Запишите значение поля **Первичный ключ**. Данное значение понадобится далее в этом руководстве.
...
Из этого учебника вы узнали, как подключить устройство IoT Plug and Play (предварительная версия), созданное на основе модели DCM, хранящейся в общедоступном репозитории.
