---
title: Экспорт данных из IoT Central Azure (Предварительная версия) | Документация Майкрософт
...
> [!Note]
> В этой статье описаны функции предварительной версии экспорта данных в IoT Central.
>
...
> - Дополнительные сведения о различиях в предварительной версии экспорта данных и функциях экспорта устаревших данных см. в [таблице сравнения](#comparison-of-legacy-data-export-and-preview-data-export) ниже.

В этой статье описывается, как использовать новую функцию предварительной версии экспорта данных в Azure IoT Central. Используйте эту функцию для непрерывного экспорта отфильтрованных и обогащенных данных IoT из приложения IoT Central. Экспорт данных позволяет отправлять изменения практически в реальном времени в другие части облачного решения для получения подробных сведений, аналитики и хранилища для теплого пути.
...
Чтобы использовать функции предварительной версии для экспорта данных, необходимо иметь [приложение v3](howto-get-app-info.md), и необходимо иметь разрешение на [Экспорт данных](howto-manage-users-roles.md) .
...
1. На левой панели выберите **Экспорт данных (Предварительная версия)**.

    > [!Tip]
    > Если вы не видите элемент **Экспорт данных (Предварительная версия)** в левой области, у вас нет разрешений на настройку экспорта данных в приложении. Обратитесь к администратору, чтобы настроить экспорт данных.
...
1. При необходимости добавьте фильтры, чтобы уменьшить объем экспортируемых данных. Существует несколько типов фильтров, доступных для каждого типа экспорта данных:
    ...
    Чтобы отфильтровать телеметрию, используйте:

    - **Фильтр возможностей**. при выборе элемента телеметрии в раскрывающемся списке **имя** экспортированный поток содержит только данные телеметрии, соответствующие условию фильтра. При выборе элемента свойств устройства или облака в раскрывающемся списке **имя** экспортированный поток содержит только данные телеметрии с устройств со свойствами, соответствующими условию фильтра.
...
## <a name="comparison-of-legacy-data-export-and-preview-data-export"></a>Сравнение экспорта устаревших данных и предварительный просмотр данных

В следующей таблице показаны различия между функциями экспорта и предварительного экспорта данных [предыдущих версий](howto-export-data-legacy.md) .
