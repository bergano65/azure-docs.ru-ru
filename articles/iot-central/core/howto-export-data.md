...
ms.locfileid: "88008531"
---
# <a name="export-iot-data-to-destinations-in-azure-using-data-export-legacy"></a>Экспорт данных IoT в назначения в Azure с помощью экспорта данных (прежние версии)

> [!Note]
> Существует новый способ экспорта данных в IoT Central. Вы можете использовать новый экспорт данных, чтобы отфильтровать и расширить экспортированные данные, а затем экспортировать их в новые назначения, такие как конечные точки веб-перехватчика. Сведения о новом экспорте данных можно узнать [здесь](./howto-use-data-export.md). Дополнительные сведения о различиях между новым экспортом данных и экспортом устаревших данных см. в [таблице сравнения](./howto-use-data-export.md#comparison-of-legacy-data-export-and-new-data-export).
...
6. (Необязательно.) Если вы выбрали **Enter a connection string** (Ввести строку подключения), открывается новое окно, в которое нужно вставить строку подключения. Чтобы получить строку подключения, сделайте следующее:
    ...
    Вставьте строку подключения. Введите имя экземпляра или **контейнера**, учитывая, что учитывается регистр.
