...
description: Как разработчик устройства вы узнаете, как подключить устройство MXChip IoT DevKit к приложению IoT Central Azure с помощью Интернета вещей Plug and Play (Предварительная версия).
author: liydu
ms.author: liydu
ms.date: 12/03/2019
ms.topic: how-to
...
В этой статье показано, как подключить устройство MXChip IoT DevKit (DevKit) к приложению Azure IoT Central. Устройство использует сертифицированную модель Plug and Play IoT (Предварительная версия) для настройки подключения устройства DevKit к IoT Central.
...
- Приложение IoT Central. Вы можете выполнить действия, описанные в разделе [Создание приложения IOT Central](./quick-deploy-iot-central.md).

## <a name="get-device-connection-details"></a>Получение сведений о подключении устройства

1. В приложении IoT Central Azure перейдите на вкладку **шаблоны устройств** и выберите **+ создать**. В разделе **Использование шаблона предварительно настроенного устройства**выберите **MXChip IOT DevKit**.

    ![Шаблон устройства для MXChip IoT DevKit](media/howto-connect-devkit/device-template.png)

1. Нажмите кнопку **Далее: настроить** , а затем **создать**.

1. Выберите вкладку **устройства** . В списке устройства выберите **MXChip IOT DevKit** и щелкните **+ создать** , чтобы создать новое устройство на основе шаблона.

    ![Новое устройство](media/howto-connect-devkit/new-device.png)

1. Во всплывающем окне введите **идентификатор устройства** `SampleDevKit` и **имя устройства** как `MXChip IoT DevKit - Sample`. Убедитесь, что параметр **смоделированный** отключен. Щелкните **Создать**.

    ![ИДЕНТИФИКАТОР и имя устройства](media/howto-connect-devkit/device-id-name.png)

1. Выберите созданное устройство и нажмите кнопку **подключить**. Запишите значение **области идентификатор**, **идентификатор устройства**и **первичный ключ**. Эти значения понадобятся позже в этой статье.
...
1. Скачайте последнюю [предварительную версию встроенного по Azure IoT Central Plug and Play (Предварительная версия)](https://github.com/Azure-Samples/mxchip-iot-devkit-pnp/raw/master/bin/iotc_devkit.bin) для устройства DevKit из GitHub.
...
1. Откройте веб-браузер и перейдите по [http://192.168.0.1/](http://192.168.0.1/)адресу. Откроется следующая веб-страница:
