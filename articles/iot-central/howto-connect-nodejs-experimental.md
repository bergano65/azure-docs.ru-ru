...
1. Добавьте следующие объявления переменных в файл:
    ...
    > [!NOTE]
    > Решение Azure IoT Central переведено на использование Службы подготовки устройств к добавлению в центр Интернета вещей Azure (DPS) для подключения всех устройств. Выполните инструкции по [получению строки подключения устройства](concepts-connectivity-experimental.md#getting-device-connection-string) и продолжите работу с оставшейся частью руководства. Подробные инструкции см. в разделе [Подготовка клиентского кода](tutorial-add-device-experimental.md#prepare-the-client-code) руководства по добавлению устройств.
...
* Просматривать данные телеметрии на странице **Measurements** (Измерения).
    ...
    ![Просмотр телеметрии](media/howto-connect-nodejs-experimental/viewtelemetry.png)

* Просматривать значения свойств устройства, отправленные из устройства, на странице **Свойства**. Если подключение установлено успешно, плитки со свойствами устройств обновляются. 

    ![Просмотр свойств устройства](media/howto-connect-nodejs-experimental/viewproperties.png)

* Задавать скорость вращения вентилятора и целевую температуру на странице **Параметры**. Если подключение установлено успешно, значения параметров синхронизируются. 

    ![Установка скорости вращения вентилятора](media/howto-connect-nodejs-experimental/setfanspeed.png)
...
* [Подготовка и подключение Raspberry Pi](howto-connect-raspberry-pi-python.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)
