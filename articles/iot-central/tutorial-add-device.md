...
3. Введите идентификатор устройства (**должен состоять из символов нижнего регистра**) или используйте предложенный идентификатор устройства. Вы также можете ввести имя нового устройства.  
...
2. На странице **Свойства** нового реального подключенного кондиционера задайте для параметра **Серийный номер** значение **10001**, а для **версии встроенного ПО** — 9.75. Затем нажмите кнопку **Сохранить**:
...
## <a name="get-connection-details-for-real-device-from-application"></a>Получение сведений о подключении для реального устройства из приложения

Разработчику устройства необходимо внедрить *сведения о подключении устройства* для вашего реального устройства в код, который выполняется на устройстве. Строка подключения позволяет устройству безопасно подключаться к вашему приложению Azure IoT Central. В следующих шагах показано, как найти строку подключения для экземпляра устройства в приложении:
...
2. На странице **подключения** скопируйте **идентификатор области, идентификатор устройства, первичный ключ** и сохраните их.

   ![Сведения о подключении](media/tutorial-add-device/device-connect.PNG)

   Используйте программу командной строки ниже, чтобы получить строку подключения устройства.  
