...
### <a name="analyze-usage-for-a-sql-database-reserved-instance-purchase"></a>Анализ потребления для покупки зарезервированного экземпляра Базы данных SQL

Зарезервированная емкость применяется к ценам на виртуальные ядра вычислений в Базе данных SQL. Она не применяется к ценам на основе DTU, стоимости лицензий SQL и любых других расходов, кроме вычислений.

Чтобы сузить оценку до подходящего потребления ресурсов Базы данных SQL, примените следующие фильтры по данным о потреблении.
...
### <a name="analysis-for-sql-data-warehouse"></a>Анализ потребления для хранилища данных SQL

Зарезервированная емкость применяется к потреблению DWU в хранилище данных SQL и приобретается с шагом приращения в 100 DWU. Чтобы сузить оценку до подходящего потребления хранилища SQL, примените следующие фильтры к данным о потреблении.
...
В поле **ResourceLocation** получите сведения о потреблении для хранилища данных SQL в нужном регионе.

Потребление хранилища данных SQL может существенно увеличиваться и (или) уменьшаться в течение дня. Проанализируйте эти данные со специалистами, отвечающими за обслуживание экземпляра хранилища данных SQL, чтобы правильно определить базовое потребление.

Перейдите на портале Azure к разделу "Резервирования" и приобретите зарезервированную емкость для Хранилища данных SQL с нужным количеством сотен DWU.
