...
Срок действия | Тип учетной записи | Description
--- | --- | ---
...
Затраты | EA, PAYG | См. CostInBillingCurrency.
...
Частота | Все | Указывает, ожидается ли повторная оплата. Плата может взиматься один раз (**OneTime**), повторяться ежемесячно или ежегодно (**Recurring**) или основываться на использовании (**UsageBased**).
...
IsAzureCreditEligible | Все | Указывает, можно ли платить за использование кредитов Azure (значения: true, false).
...
Срок действия | Все | Отображает срок действия предложения. Например: в случае с зарезервированными экземплярами в качестве термина отображается 12 месяцев. Для однократных или повторяющихся покупок срок действия составляет 1 месяц (SaaS, поддержка Marketplace). Это не применяется к использованию Azure.
