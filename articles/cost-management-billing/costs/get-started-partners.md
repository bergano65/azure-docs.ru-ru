...
Служба "Управление затратами Azure" изначально доступна для прямых партнеров, которые подключили своих клиентов к Клиентскому соглашению Майкрософт и [приобрели план Azure](/partner-center/purchase-azure-plan). В этой статье объясняется использование партнерами функций [Управление затратами Azure](../index.yml) для просмотра затрат на подписки в плане Azure. Здесь также содержатся сведения о том, как партнеры обеспечивают доступ к Управлению затратами для своих клиентов.

Для прямых партнеров и непрямых поставщиков глобальный администратор и агенты администрирования непрямого поставщика могут получить доступ к Управлению затратами в арендаторе партнера. Торговые посредники и клиенты могут получить доступ к Управлению затратами в арендаторе клиента и просматривать затраты для подписок, в которых затраты рассчитываются и отображаются в розничных тарифах. Но у таких торговых посредников и клиентов должен быть доступ RBAC к подписке в арендаторе клиента для просмотра затрат. Поставщику следует включить политику видимости затрат для арендатора клиента.
...
![Пример, показывающий затраты для всех клиентов](./media/get-started-partners/customer-costs1.png)

Ниже приведен пример, показывающий затраты для одного клиента.
![Пример, показывающий затраты для одного клиента](./media/get-started-partners/customer-costs2.png)
...
Чтобы получить доступ к службе "Управление затратами Azure" в рамках подписки, любой пользователь, имеющий доступ RBAC к подписке, может просмотреть расходы по розничным (по мере использования) ставкам. Однако для этого следует включить политику видимости затрат для арендатора клиента. Полный список поддерживаемых типов учетных записей см. в статье [Understand Cost Management data](understand-cost-mgt-data.md) (Интерпретация данных службы "Управление затратами").


## <a name="how-cost-management-uses-scopes"></a>Как используются области в Управлении затратами

Области — это места, где вы управляете данными выставления счетов, пользуетесь ролями, относящимися к платежам, рассматриваете счета и проводите общее управление учетными записями. Роли для выставления счетов и учетных записей управляются отдельно из областей для управления ресурсами, использующих RBAC. Чтобы четко отличать намерение отдельных областей (включая различия в управлении доступом), они называются областями выставления счетов и областями RBAC соответственно.

Сведения об областях выставления счетов и ограничениях RBAC, а также о том, как управление затратами работает с областями, см. в статье [Understand and work with scopes](understand-work-scopes.md) (Общие сведения об областях и управление ими).
...
Независимо от того, в каких валютах клиенты оплачивают счета, партнеры используют область Учетной записи выставления счетов для составления бюджетов и управления затратами в долларах США для своих клиентов, подписок, ресурсов и ресурсных групп.

В представлении анализа затрат партнеры также фильтруют затраты в определенной валюте выставления счетов по клиентам. Выберите список **Фактические затраты** для просмотра затрат в поддерживаемых валютах выставления счетов клиента.
...
В настоящее время валюта для выставления счета клиенту является валютой по умолчанию при просмотре расходов в области профиля выставления счетов. Бюджеты, заданные в области профиля выставления счетов, относятся к валюте выставления счетов.
...
## <a name="enable-cost-management-in-the-customer-tenant"></a>Включение управления затратами в арендаторе клиента

Партнеры могут разрешить доступ к Управлению затратами после подключения клиентов к Клиентскому соглашению Майкрософт. Затем партнеры могут включить политику, позволяющую клиентам просматривать свои затраты, вычисленные в розничных ставках с оплатой по мере использования. Затраты отображаются в валюте выставления счетов клиента для использования в областях подписки RBAC и групп ресурсов.

При включении партнером политики видимости затрат, любой пользователь с доступом к подписке с помощью Azure Resource Manager сможет управлять затратами и анализировать их в соответствии со ставками с оплатой по мере использования. По сути, просматривать расходы могут торговые посредники и клиенты, имеющие соответствующий доступ RBAC к подпискам Azure.

Партнеры могут также просматривать расходы независимо от политики, если у них есть доступ к подписке и группе ресурсов.

### <a name="enable-the-policy-to-view-azure-usage-charges"></a>Включение политики для просмотра расходов на использование Azure

Чтобы включить политику для просмотра расходов на использование Azure для своих клиентов партнеры используют приведенные ниже сведения.

Войдите в арендатор партнера на портале Azure и выберите **Cost Management + Billing** (Управление затратами и выставление счетов). Выберите соответствующую учетную запись для выставления счетов в рамках Соглашения с партнерами Майкрософт, а затем выберите **Клиенты**. Список клиентов связан с учетной записью выставления счетов.
...
Если политика видимости затрат включена, все службы, имеющие использование подписок, будут показывать затраты при ставках с оплатой по мере использования. Использование на резервирования отображается с нулевыми расходами за фактические и амортизированные затраты. Покупки и права не связаны с определенной подпиской. Поэтому покупки не отображаются в области подписки.

Чтобы просмотреть затраты для арендатора клиента, откройте **Cost Management + Billing** (Управление затратами и выставление счетов), а затем соответствующую учетную запись выставления счетов в рамках Соглашения с партнерами Майкрософт.

![Выбор учетной записи выставления счетов](./media/get-started-partners/select-billing-account.png)

В разделе **Выставление счетов** выберите **подписки Azure**, а затем клиента.

![Выбор клиента подписки Azure](./media/get-started-partners/subscriptions-select-customer.png)

Выберите **Анализ затрат** и начните просмотр затрат.
Анализ затрат, бюджеты и оповещения доступны для подписок и группы ресурсов областей RBAC по ставкам, основанным на оплате по мере использования.

![Просмотр анализа затрат в качестве пользователя ](./media/get-started-partners/customer-tenant-view-cost-analysis.png)

Представления для амортизированных и фактических расходов для зарезервированных экземпляров в области RBAC показывают нулевые расходы. Расходы на зарезервированные экземпляры отображаются только в тех областях выставления счетов, в которых были сделаны покупки.

## <a name="analyze-costs-in-cost-analysis"></a>Анализ затрат в функции анализа затрат

Партнеры, имеющие доступ к областям выставления счетов в арендаторах партнеров, могут изучать и анализировать расходы по счетам в рамках анализа затрат по клиентам для конкретного клиента или по счету. В представлении [Анализ затрат](quick-acm-cost-analysis.md) можно также [сохранять представления](quick-acm-cost-analysis.md#saving-and-sharing-customized-views) и экспортировать данные в [файлы CSV и PNG](quick-acm-cost-analysis.md#automation-and-offline-analysis).

Пользователи RBAC, имеющие доступ к подписке в арендаторе клиента, могут также анализировать розничные расходы на подписку в клиентском арендаторе, сохранять представления и экспортировать данные в файлы CSV и PNG.
...
| billingCurrency | Валюта, определяющая фактурную стоимость. | Валюта географического региона клиента. |
...
| **paygCostInBillingCurrency** | Показывает затраты, если цены указаны в розничных ценах. Показывает цены по принципу "с оплатой по мере использования" в валюте выставления счетов. Доступно только для областей RBAC. | Недоступно |
| **paygCostInUSD** | Показывает затраты, если цены указаны в розничных ценах. Показывает цены с оплатой по мере использования в долларах США. Доступно только для областей RBAC. | Недоступно |
...
Партнеры, имеющие доступ к областям выставления счетов в арендаторе партнера, могут экспортировать свои данные о затратах и использовании в большой двоичный объект службы хранилища Azure. Большой двоичный объект должен быть на подписке в арендаторе партнера, который не является подпиской на общую папку или подпиской клиента. Чтобы включить экспорт данных о затратах, мы рекомендуем установить в арендаторе партнера независимую подписку с оплатой по мере использования для размещения экспортированных данных о затратах. Учетная запись хранения для экспорта создается в большом двоичном объекте службы хранилища Azure, размещенном в подписке с оплатой по мере использования. В зависимости от области, в которой партнер создает экспорт, соответствующие данные автоматически экспортируются в учетную запись хранения на периодической основе.

Пользователи с RBAC-доступом к подписке могут также экспортировать данные о затратах в большой двоичный объект службы хранилища Azure, размещенном в любой подписке арендатора клиента.
...
Для пользователей RBAC в арендаторе клиента выберите подписку в арендаторе клиента. Создайте учетную запись хранения Azure, используя подписку.
