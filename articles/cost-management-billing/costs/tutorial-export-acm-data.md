...
Чтобы создать или запланировать операцию экспорта либо же просмотреть сведения о ней, откройте требуемую область на портале Azure и выберите в меню **Анализ затрат**. Например, перейдите к разделу **Подписки**, выберите подписку из списка и щелкните в меню **Анализ затрат**. В верхней части страницы анализа затрат выберите **Параметры**, затем выберите **Экспортируемые элементы** и параметр экспорта.
...
[![Новый пример создания задачи экспорта, в котором показан тип экспорта](./media/tutorial-export-acm-data/basics_exports.png)](./media/tutorial-export-acm-data/basics_exports.png#lightbox)
...
1. Создайте группу управления и назначьте ей подписки.
1. В окне разделе "Экспортируемые элементы" выберите **Область**.
1. Выберите **Select this management group** (Выбрать эту группу управления).
