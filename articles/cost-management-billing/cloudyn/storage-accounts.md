---
title: Настройка учетных записей хранения для Cloudyn в Azure | Документация Майкрософт
description: В этой статье описывается, как настроить учетные записи хранения Azure и контейнеры хранилища AWS для Cloudyn.
keywords: ''
author: bandersmsft
ms.author: banders
ms.date: 01/24/2020
...
ms.locfileid: "77117621"
---
# <a name="configure-storage-accounts-for-cloudyn"></a>Настройка учетных записей хранения для Cloudyn

<!--- intent: As a Cloudyn user, I want to configure Cloudyn to use my cloud service provider storage account to store my reports. -->

Отчеты Cloudyn можно сохранять на портале Cloudyn, в службе хранилища Azure или контейнерах хранилища AWS. За сохранение отчетов на портале Cloudyn не требуется плата. Тем не менее сохранение отчетов в хранилище вашего поставщика облачных служб является необязательным и требует дополнительных затрат. Эта статья поможет вам настроить учетные записи хранения Azure и контейнеры хранилища Amazon Web Services (AWS) для хранения отчетов.

## <a name="prerequisites"></a>Предварительные требования

Необходима учетная запись хранения Azure или контейнер хранилища Amazon.

Если у вас нет учетной записи хранения Azure, то вам потребуется ее создать. Сведения о создании учетной записи хранения Azure см. в разделе [Создайте учетную запись хранения](../../storage/common/storage-account-create.md).
...
Настроить службу хранилища Azure для использования в Cloudyn нетрудно. Соберите сведения об учетной записи хранения и скопируйте их на портале Cloudyn.
...
4. Скопируйте **имя учетной записи хранения** и **строку подключения** для ключа key1.  
   ![Скопируйте имя учетной записи хранения и строку подключения](./media/storage-accounts/azure-storage-access-keys.png).  
5. Откройте портал Cloudyn с портала Azure или перейдите по адресу [https://azure.cloudyn.com](https://azure.cloudyn.com) и войдите.
...
7. Щелкните **Add new +** (Добавить новый) и убедитесь, что выбрана служба Microsoft Azure. Вставьте имя учетной записи хранения Azure в области **Имя**. Вставьте **строку подключения** в соответствующей области. Введите имя контейнера и нажмите кнопку **Сохранить**.  
   ![Вставьте имя учетной записи хранения Azure и строку подключения в поле добавления нового хранилища отчетов](./media/storage-accounts/azure-cloudyn-storage.png)

   Новая запись хранилища отчетов Azure отобразится в списке учетных записей хранения.  
    ![Новая запись хранилища отчетов Azure в списке](./media/storage-accounts/azure-storage-entry.png)
...
Cloudyn использует существующие учетные данные AWS (пользователя или роли), чтобы сохранять отчеты в контейнере. Для проверки доступа Cloudyn пытается сохранить небольшой текстовый файл _check-bucket-permission.txt_ в контейнер.
...
6. Щелкните **Review policy** (Просмотреть политику).  
    ![Политика AWS JSON с отображением сведений о примере](./media/storage-accounts/aws-policy.png)  
...
**Для ролей:**
  ...
  3. Найдите созданную вами политику и выберите ее, затем щелкните **Attach Policy** (Привязать политику).
    ![Пример политики, подключенной к роли Cloudyn](./media/storage-accounts/aws-attach-policy-role.png)
...
4. Найдите созданную политику и выберите ее, а затем щелкните **Next: Review** (Далее: проверка).
5. На странице "Add permissions to role name" (Добавление разрешений для имени роли) щелкните **Add permissions** (Добавить разрешения).  
    ![Пример политики, подключенной к пользователю Cloudyn](./media/storage-accounts/aws-attach-policy-user.png)


### <a name="optional-set-permission-with-bucket-policy"></a>Необязательное действие: Настройка разрешения с помощью политики контейнеров
...
4. Выберите учетную запись и контейнер хранилища. Имя контейнера хранилища AWS будет указано автоматически.  
    ![Сведения о примере в поле добавления нового хранения отчетов](./media/storage-accounts/aws-cloudyn-storage.png)  
5. Нажмите кнопку **Save** (Сохранить), затем — кнопку **ОК**.
    ...
    ![Новая запись хранилища отчетов AWS отобразится в списке учетных записей хранения.](./media/storage-accounts/aws-storage-entry.png)
...
- Ознакомьтесь с разделом [Сведения об отчетах по управлению затратами](understanding-cost-reports.md), чтобы изучить базовую структуру и функции отчетов Cloudyn.
