---
title: Подключение учетной записи Amazon Web Services к Cloudyn в Azure
...
4. В диалоговом окне **Add AWS Account** (Добавление учетной записи AWS) скопируйте **внешний идентификатор** и сохраните его для использования на шагах по созданию роли AWS (в следующем разделе). Внешний идентификатор уникален для вашей учетной записи. На следующем изображении примером внешнего идентификатора является _Contoso_, за которым следует номер. Ваш идентификатор отличается.  
    ![Внешний идентификатор, отображаемый в поле добавления учетной записи AWS](./media/connect-aws-account/external-id.png)

### <a name="add-aws-read-only-role-based-access"></a>Предоставление доступа только для чтения на основе ролей AWS

1. Войдите в консоль AWS по ссылке [https://console.aws.amazon.com/iam/home](https://console.aws.amazon.com/iam/home) и выберите **Roles** (Роли).
...
6. В поле **IAM Role** (Роль IAM) вставьте скопированное ранее значение и щелкните **Save** (Сохранить).  
    ![Вставьте ARN роли в поле добавления учетной записи AWS](./media/connect-aws-account/add-aws-account-box.png)
...
![Состояние учетной записи AWS, отображаемое на странице управления учетными записями](./media/connect-aws-account/aws-account-status01.png)
...
1. Войдите в консоль AWS по ссылке [https://console.aws.amazon.com/iam/home](https://console.aws.amazon.com/iam/home) и выберите **Users** (Пользователи).
...
4. В качестве **типа доступа** выберите **Programmatic access** (Программный доступ) и щелкните **Next: Permissions** (Далее: разрешения).  
    ![Введите имя пользователя на странице добавления пользователя](./media/connect-aws-account/add-user01.png)
...
6. В разделе **Attach permissions policies** (Подключение политик разрешений) в поле поиска фильтра **Policy type** (Тип политики) введите `ReadOnlyAccess`, выберите **ReadOnlyAccess**, а затем щелкните **Next: Review** (Далее: отзыв).  
    ![Выберите ReadOnlyAccess, чтобы задать разрешения для пользователя](./media/connect-aws-account/set-permission-for-user.png)
...
9. Щелкните **Download .csv** (Скачать CSV-файл) и сохраните файл credentials.csv в безопасном месте.  
    ![Нажмите кнопку скачивания CSV-файла, чтобы сохранить учетные данные](./media/connect-aws-account/download-csv.png)
...
1. Войдите в консоль AWS по ссылке [https://console.aws.amazon.com](https://console.aws.amazon.com) и выберите **Services** (Службы).
...
4. В мастере создания контейнеров выберите имя и регион для контейнера и нажмите кнопку **Next** (Далее).  
    ![Пример сведений на странице создания контейнера](./media/connect-aws-account/create-bucket.png)
...
9. Выберите команду **Сохранить**.  
    ![Нажмите кнопку сохранения в редакторе политики контейнеров](./media/connect-aws-account/bucket-policy-editor.png)
...
3. Выберите все четыре параметра степени детализации отчета, а затем нажмите кнопку **Save preferences** (Сохранить параметры).  
    ![Выберите степень детализации, чтобы включить отчеты](./media/connect-aws-account/enable-reports.png)
...
![Состояние консолидации, отображаемое на вкладке учетных записей AWS](./media/connect-aws-account/consolidated-status.png)
