...
> Admin1 может быть Microsoft Cloud поставщиком решений или Соглашение Enterprise пользователем. Их организация готова начать использовать Cloudyn.  Администратор регистрируется на портале Azure и выполняет вход на портал Cloudyn. Когда пользователь, регистрирующий службу Cloudyn, подписывается на портал Cloudyn, admin1 станет *основным администратором*. Администратор1 не создает какие-либо пользовательские учетные записи. Однако с помощью портала Cloudyn они создают учетные записи Azure и настроили иерархию сущностей. Admin1 информирует Admin2, администратор клиента, что им нужно зарегистрироваться в Cloudyn и войти на портал Cloudyn.
>
> Администратор2 регистрируется через портал Azure. Однако при попытке войти на портал Cloudyn появляется сообщение об ошибке, сообщающее о том, что учетная запись **приостановлена**. Главный администратор (администратор1) получает уведомление о приостановке учетной записи. Администратор1 должен активировать учетную запись администратора2 и предоставить *доступ к соответствующим сущностям от имени администратора*. Он также должен разрешить доступ к управлению пользователями и активировать учетную запись пользователя.
...
Если изменить адрес электронной почты в Cloudyn с адреса по умолчанию в Azure, ваша учетная запись может быть заблокирована. Может отображаться состояние "Инитиаллисуспендед". Если ваша учетная запись заблокирована, обратитесь к другому администратору для сброса учетной записи.
