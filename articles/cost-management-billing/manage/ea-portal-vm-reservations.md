...
### <a name="partial-refunds"></a>Частичное возмещение

Мы будем частично возмещать средства при возврате резервирований клиентами в рамках EA, приобретенных за счет избытка, а не за счет предоплаты Azure (прежнее название — денежное обязательство).

Возмещение будет отражено на портале EA в виде отрицательной корректировки за предыдущий месяц и положительной корректировки за текущий месяц. Эти данные отображаются аналогично сведениям, касающимся обмена резервированиями. В кредит-ноте будет указан номер исходного счета. Чтобы согласовать первоначальную покупку с кредит-нотой, обратитесь к исходному номеру счета.
