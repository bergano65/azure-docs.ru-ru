...
description: Узнайте, как создать дополнительные подписки Azure программными средствами.
author: bandersmsft
...
ms.locfileid: "91371445"
---
# <a name="programmatically-create-azure-subscriptions-preview"></a>Программное создание подписок Azure (предварительная версия)

Клиенты с учетной записью выставления счетов по [Соглашению Enterprise Azure (EA)](https://azure.microsoft.com/pricing/enterprise-agreement/), [Клиентскому соглашению Майкрософт (MCA)](https://azure.microsoft.com/pricing/purchase-options/microsoft-customer-agreement/) или [Соглашению с партнером Майкрософт (MPA)](https://www.microsoft.com/licensing/news/introducing-microsoft-partner-agreement) могут создавать подписки программными средствами. В этой статье Вы узнаете как создавать подписки программно с помощью Azure Resource Manager.

Когда вы создаете подписки Azure программными средствами, они регулируются соглашением, в соответствии с которым вы получили службы Azure от корпорации Майкрософт или уполномоченного торгового посредника. Дополнительные сведения см. на странице [Юридическая информация Службы Microsoft Azure](https://azure.microsoft.com/support/legal/).
...
* Текущий владелец учетной записи регистрации может [предоставить вам права доступа](grant-access-to-create-subscription.md). Аналогично, если вы хотите использовать субъект-службу для создания подписки с EA, необходимо [предоставить ему возможность создания подписки](grant-access-to-create-subscription.md).
...
```
Используйте свойство `principalName`, чтобы указать учетную запись для оплаты подписок. Скопируйте значение `ObjectId` этой учетной записи. Например, если вы хотите создать подписки в учетной записи регистрации SignUpEngineering@contoso.com, скопируйте ```747ddfe5-xxxx-xxxx-xxxx-xxxxxxxxxxxx```. Вставьте этот идентификатор объекта в любое место, чтобы его можно было использовать на следующем шаге в качестве `enrollmentAccountObjectId`.
...
Выполните следующий запрос, заменив `<enrollmentAccountObjectId>` на `name`, скопированный на первом шаге (```747ddfe5-xxxx-xxxx-xxxx-xxxxxxxxxxxx```). Если вы хотите указать владельцев, ознакомьтесь с [инструкциями по получению идентификаторов объектов пользователя](grant-access-to-create-subscription.md#userObjectId).
...
| `owners`      | нет       | Строка | Идентификатор объекта любого пользователя, которого вы хотите добавить в качестве владельца Azure RBAC для подписки при ее создании.  |
...
Воспользуйтесь приведенной ниже командой [New-AzSubscription](/powershell/module/az.subscription), заменив `<enrollmentAccountObjectId>` на значение `ObjectId`, полученное на первом шаге (```747ddfe5-xxxx-xxxx-xxxx-xxxxxxxxxxxx```). Если вы хотите указать владельцев, ознакомьтесь с [инструкциями по получению идентификаторов объектов пользователя](grant-access-to-create-subscription.md#userObjectId).
...
| `OwnerObjectId`      | нет       | Строка | Идентификатор объекта любого пользователя, которого вы хотите добавить в качестве владельца Azure RBAC для подписки при ее создании.  |
| `OwnerSignInName`    | нет       | Строка | Адрес электронной почты любого пользователя, которого вы хотите добавить в качестве владельца Azure RBAC для подписки при ее создании. Можно использовать этот параметр вместо `OwnerObjectId`.|
| `OwnerApplicationId` | нет       | Строка | Идентификатор приложения любого субъекта-службы, которого вы хотите добавить в качестве владельца Azure RBAC для подписки при ее создании. Можно использовать этот параметр вместо `OwnerObjectId`. При использовании этого параметра служба должна иметь [доступ на чтение к каталогу](/powershell/azure/active-directory/signing-in-service-principal?view=azureadps-2.0#give-the-service-principal-reader-access-to-the-current-tenant-get-azureaddirectoryrole).|
...
Воспользуйтесь приведенной ниже командой [az account create](/cli/azure/ext/subscription/account?view=azure-cli-latest#-ext-subscription-az-account-create), заменив `<enrollmentAccountObjectId>` на значение `name`, скопированное на первом шаге (```747ddfe5-xxxx-xxxx-xxxx-xxxxxxxxxxxx```). Если вы хотите указать владельцев, ознакомьтесь с [инструкциями по получению идентификаторов объектов пользователя](grant-access-to-create-subscription.md#userObjectId).
...
| `owner-object-id`      | нет       | Строка | Идентификатор объекта любого пользователя, которого вы хотите добавить в качестве владельца Azure RBAC для подписки при ее создании.  |
| `owner-upn`    | нет       | Строка | Адрес электронной почты любого пользователя, которого вы хотите добавить в качестве владельца Azure RBAC для подписки при ее создании. Можно использовать этот параметр вместо `owner-object-id`.|
...
В приведенном ниже примере используются REST API. В настоящее время PowerShell и Azure CLI не поддерживаются.
...
Отправьте указанный ниже запрос, чтобы получить список всех учетных записей выставления счетов.
...
| `owners`      | нет       | Строка | Идентификатор объекта любого пользователя или субъекта-службы, которого вы хотите добавить в качестве владельца Azure RBAC для подписки при ее создании.  |
...
```
Используйте свойство `description`, чтобы определить торгового посредника, который будет связан с подпиской. Скопируйте значение `resellerId` для торгового посредника. Например, если необходимо связать `Wingtip`, скопируйте `3xxxxx`. Вставьте это значение в любое место, чтобы его можно было использовать на следующем шаге.

### <a name="create-a-subscription-for-a-customer"></a>Создание подписки для клиента

В следующем примере создается подписка с именем *Dev Team subscription* для *Fabrikam toys*, которая затем связывается с торговым посредником *Wingtip*. T
