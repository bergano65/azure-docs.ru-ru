...
7.    После того как runbook завершит процесс импорта, выберите **Изменить**, чтобы отобразить графический редактор runbook и параметр публикации.
    ...
    ![Изменение графических модулей runbook в Azure](./media/cost-management-budget-scenario/billing-cost-management-budget-scenario-01.png)
...
1. На странице **Модули Runbook** на [портале Azure](https://portal.azure.com/) щелкните модуль runbook **StopAzureV2Vm**, который отображается в колонке обзора модулей runbook.
...
4. Для параметра **Имя** веб-перехватчика задайте значение **Дополнительно**. Для свойства **Включено** необходимо выбрать значение **Да**. Значение **Срок действия истекает** менять не требуется. Дополнительные сведения о свойствах веб-перехватчиков см. [здесь](../../automation/automation-webhooks.md#webhook-properties).
...
1.    На [портале Azure](https://portal.azure.com/) выберите **Создать ресурс** > **Интеграция** > **Приложение логики**.
    ...
    ![Выбор ресурса приложения логики в Azure](./media/cost-management-budget-scenario/billing-cost-management-budget-scenario-03.png)
2.    В колонке **Создание приложения логики** укажите сведения, необходимые для создания приложения логики, выберите **Закрепить на панели мониторинга** и нажмите кнопку **Создать**.
    ...
    ![Создание приложения логики в Azure](./media/cost-management-budget-scenario/billing-cost-management-budget-scenario-03a.png)
...
6.    Выберите **Использовать пример полезной нагрузки, чтобы создать схему** в диалоговом поле **Анализ JSON**.
    ...
    ![Использование примера полезных данных JSON для создания схемы в приложении логики на портале Azure](./media/cost-management-budget-scenario/billing-cost-management-budget-scenario-07.png)
...
20.    Выполните поиск по запросу "отправка сообщения" и выберите действие *отправки сообщения электронной почты* в зависимости от используемой службы электронной почты.
    ...
    ![Действие отправки электронной почты в приложении логики на портале Azure](./media/cost-management-budget-scenario/billing-cost-management-budget-scenario-17.png)
...
14.    Заполните текстовые поля **Кому**, **Тема** и **Текст** для сообщения электронной почты, которое уведомляет получателя о завершении работы дополнительных виртуальных машин. Используйте динамическое содержимое **BudgetName** и **NotificationThresholdAmount**, чтобы заполнить поля "Тема" и "Текст".
    ...
    ![Сведения об отправке электронной почты в приложении логики на портале Azure](./media/cost-management-budget-scenario/billing-cost-management-budget-scenario-24.png)
