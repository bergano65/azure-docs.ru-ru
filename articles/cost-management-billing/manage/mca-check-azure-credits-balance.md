...
Вы можете проверить кредитный баланс Azure для счета на выставление счетов для соглашения клиента Майкрософт в портал Azure или с помощью интерфейсов API.

В учетной записи выставления счетов для Клиентского соглашения Майкрософт кредиты назначаются профилю выставления счетов. Каждый профиль выставления счетов имеет собственные кредиты, которые автоматически применяются к расходам на счет. Для просмотра кредитного баланса Azure профиля выставления счетов необходимо иметь роль "владелец", "участник", "читатель" или "менеджер счетов" для профиля выставления счетов, или роль "владелец", "участник", "читатель" для учетной записи выставления счетов. Подробные сведения о ролях см. в разделе [Сведения об административных ролях клиентских соглашений Майкрософт в Azure](understand-mca-roles.md).
...
3. На странице области выставления счетов выберите счет выставления счетов, для которого необходимо отрегулировать кредитный баланс. Счет выставления счетов должен иметь тип " **Соглашение клиента Майкрософт**".
    ...
    > [!NOTE]
    >
    > Портал Azure запоминает последнюю доступную область выставления счетов и отображает область при следующем обращении к странице Управление затратами и выставление счетов. Если вы посещали управление затратами и выставление счетов ранее, вы не увидите страницу области выставления счетов. Если это так, убедитесь, что вы находится в [правильной области](#check-access-to-a-microsoft-customer-agreement). В противном случае [переключите область](view-all-accounts.md#switch-billing-scope-in-the-azure-portal) , чтобы выбрать счет выставления счетов для соглашения клиента Майкрософт.

3. Выберите **методы оплаты** с левой стороны, а затем щелкните **кредиты Azure**.

   ![Снимок экрана: баланс по кредиту для профиля выставления счетов](./media/mca-check-azure-credits-balance/mca-payment-methods.png)

4. На странице кредиты Azure содержатся следующие подразделы.

   #### <a name="balance"></a>Balance

   В разделе баланс отображается сводка по кредитовому балансу Azure.

   ![Снимок экрана: баланс по кредиту для профиля выставления счетов](./media/mca-check-azure-credits-balance/mca-credit-balance.png)
   ...
   В разделе "список кредитов" отображается список кредитов Azure.
   ...
   В разделе Transactions (транзакции) отображаются все транзакции, которые затронули баланс кредита.

   ![Снимок экрана с кредитными транзакциями для профиля выставления счетов](./media/mca-check-azure-credits-balance/mca-credits-transactions.png)

   | Срок действия | Определение |
   |---|---|
   | Дата транзакции | Дата возникновения транзакции |
   | Description | Описание транзакции |
   | Объем| Сумма транзакции |
   | Balance | Баланс после транзакции |

    > [!NOTE]
    >
    > Если вы не видите кредиты Azure на странице способы оплаты, у вас нет кредитов или вы не выбрали нужную область. Выберите счет выставления счетов, который содержит кредиты или один из профилей выставления счетов. Сведения об изменении областей см. [в разделе переключение областей выставления счетов в портал Azure](view-all-accounts.md#switch-billing-scope-in-the-azure-portal).

5. Если вы просматриваете кредиты Azure в области счетов выставления счетов, а счет выставления счетов имеет несколько профилей выставления счетов, на странице кредиты Azure отобразится таблица со сведениями о кредитах Azure для каждого профиля выставления счетов. Выберите профиль выставления счетов из списка, выберите методы оплаты, а затем кредиты Azure, чтобы просмотреть сведения для профиля выставления счетов.

    ![Снимок экрана: список кредита для счета выставления счетов](./media/mca-check-azure-credits-balance/mca-account-credit-list.png)
