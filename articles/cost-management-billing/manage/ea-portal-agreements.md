...
Поставщик решений по лицензированию (LSP) указывает одно процентное значение на EA Portal.  Вся коммерческая информация на портале будет приподнята в процентах, предоставляемых LSP. Пример:
...
- В примере ниже показано, как коммерческая информация будет отображаться для клиента:
    - Денежный баланс: USD 110 000.
    - Ставка по показателям для службы A: USD 11 в час.
    - Сведения об использовании и размещении для службы A при использовании в течение 100 часов: USD 1 100.
    - Денежный баланс, доступный клиенту, после вычета использования службы A: USD 108 900.
...
**Шаг 1. добавление разметки цены**
...
Чтобы проверить, не опубликована ли в регистрации разметка, щелкните **Управление** в левой области навигации и перейдите на вкладку **Регистрация** . Выберите поле регистрации для проверки и просмотрите состояние разметки в разделе _сведения о регистрации_. Отобразится текущее состояние функции наценки для этого EA: "Отключено", "Предварительная версия" или "Опубликовано".
...
| Хранилище Microsoft Azure | 5 учетных записей хранения, максимальный размер каждой из которых составляет 100 ТБ. | Количество учетных записей хранения можно увеличить до 20 на одну подписку. Если требуются дополнительные учетные записи хранения, добавьте дополнительные подписки. |
| SQL Azure | 149 баз данных любого типа (т. е., выпуска Web Edition или Business Edition). |   |
| Access Control | 50 пространств имен на учетную запись. 100 млн транзакций контроля доступа в месяц. |   |
...
Чтобы убедиться, что развертывание выполняется в правильной регистрации, можно проверить сведения о включаемых единицах в прейскуранте. Войдите как администратор предприятия и щелкните **отчеты** в левой области навигации и выберите вкладку **прайс-лист** . Щелкните значок скачивания в правом верхнем углу и найдите соответствующие номера SKU плана с фильтром по столбцу "Включенный объем" и выберите значения больше "0".
...
Если включенные единицы для служб в прайс – листе не совпадают с развернутыми данными, например с помощью службы оперативной аналитики Premium и проанализированных данных в службе оперативной аналитики, это означает, что вы могли развернуть службу, которая не охватывает этот план, обратитесь в службу поддержки Azure Enterprise Portal по адресу [https://aka.ms/AzureEntSupport](https://aka.ms/AzureEntSupport) , чтобы мы могли помочь вам дальше.
