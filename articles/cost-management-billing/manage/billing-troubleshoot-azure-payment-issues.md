...
Если вы получаете сообщение об ошибке: *платеж просрочен. Возникла проблема с методом оплаты* *, и мы не можем сохранить информацию. Закройте браузер и повторите попытку.* в этом случае на карточке имеется ожидающая оплата, так как карта была отклонена финансовым учреждением.
