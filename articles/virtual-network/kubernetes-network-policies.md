...
При реализации безопасности для кластера используйте группы безопасности сети (группы безопасности сети), чтобы отфильтровать трафик North-South, т. е. трафик, поступающий в подсеть кластера и исходящий из нее, и использовать политики сети Kubernetes для East-West трафика, то есть трафика между модулями Pod в кластере.
