...
Чтобы приступить к работе, обратитесь к [документации по виртуальной сети](./index.yml) . В ней представлены описание и информация о развертывании для всех компонентов виртуальной сети.
...
* PowerShell
* Azure CLI
* Файл конфигурации сети (netcfg — только для классических виртуальных сетей). См. статью [Настройка виртуальной сети с помощью файла конфигурации сети](/previous-versions/azure/virtual-network/virtual-networks-using-network-configuration-file).
...
Да. Дополнительные сведения см. в статье [Перемещение виртуальной машины или экземпляра роли в другую подсеть](/previous-versions/azure/virtual-network/virtual-networks-move-vm-role-to-subnet).
...
Да. Вы можете применить [группы безопасности сети](./network-security-groups-overview.md) для отдельных подсетей в виртуальной сети и (или) для подключенных к виртуальной сети сетевых адаптеров.
...
### <a name="do-virtual-networks-store-customer-data"></a>Хранятся ли в виртуальных сетях данные клиентов?
Нет. В виртуальных сетях не хранятся данные клиентов. 
...
Да. В [Azure Resource Manager](/rest/api/virtual-network) и [классической](/previous-versions/azure/ee460799(v=azure.100)) модели развертывания можно использовать интерфейсы API-интерфейсов RESTful для виртуальных сетей.
...
### <a name="can-i-peer-a-vnet-to-two-different-vnets-with-the-the-use-remote-gateway-option-enabled-on-both-the-peerings"></a>Можно ли выполнить одноранговую виртуальную сеть с двумя разными виртуальных сетей с включенным параметром "использовать удаленный шлюз" на обоих узлах?
Нет. Параметр "использовать удаленный шлюз" можно включить только на одном одноранговом узле с одним из виртуальных сетей.
...
Первым шагом является операция на стороне сети и вторым – операция на стороне службы ресурсов. Оба действия могут выполняться одним и тем же администратором или разными администраторами на основе разрешений RBAC Azure, предоставленных роли администратора. Мы рекомендуем сначала включить конечные точки службы виртуальной сети перед настройкой списков управления доступом в виртуальной сети на стороне службы Azure. Поэтому эти действия необходимо выполнить в приведенной выше последовательности, чтобы настроить конечные точки службы виртуальной сети.
...
### <a name="do-service-endpoints-work-with-icmp"></a>Работают ли конечные точки службы с ICMP?
Нет, ICMP-трафик, источником которого является подсеть с включенными конечными точками службы, не будет принимать путь туннеля службы к требуемой конечной точке. Конечные точки службы будут работать только с TCP-трафиком. Это означает, что если вы хотите протестировать задержку или подключение к конечной точке через конечные точки службы, такие средства, как ping и tracert, не будут показывать истинный путь, необходимый для ресурсов в подсети.
