...
Виртуальная сеть Azure — это представление сети в облаке. Это логическая изоляция облака Azure, выделенного для вашей подписки. С помощью виртуальной сети можно подготовить VPN к работе в Azure и управлять ею. Кроме того, при необходимости можно связать виртуальную сеть с другими виртуальными сетями в Azure или с локальной ИТ-инфраструктурой для создания гибридных или распределенных решений. Каждый создаваемый VNet имеет свой собственный блок CIDR и может быть связан с другими VNets и сетевыми сетями до тех пор, пока блоки CIDR не пересекаются. Кроме того, вы можете изменять настройки DNS-серверов для виртуальных сетей и разбивать виртуальные сети на подсети.

Вы можете использовать виртуальные сети для указанных ниже целей.

* Создайте специальный частный облачный VNet. Иногда для вашего решения не нужна распределенная конфигурация. При создании виртуальной сети подключенные к ней службы и виртуальные машины могут безопасно обмениваться данными непосредственно друг с другом в облаке. В рамках вашего решения вы все еще можете настроить подключения конечных точек для виртуальных машин и служб, которым необходимо подключение к Интернету.

* Безопасно расширьте свой центр обработки данных. С помощью виртуальных сетей можно создавать традиционные VPN типа «сеть-сеть» (S2S) для безопасного масштабирования емкости центра обработки данных. Для создания безопасного подключения между корпоративным VPN-шлюзом и Azure в VPN-подключениях S2S используется IPsec.

* Включить сценарии гибридных облаков. Виртуальные сети предоставляют гибкие возможности для использования целого ряда сценариев гибридного облака. Вы можете безопасно подключать облачные приложения к локальным системам любого типа, например к большим ЭВМ и системам под управлением ОС Unix.

### <a name="how-do-i-get-started"></a>Как начать работу?
Посетите [виртуальную сетевую документацию,](https://docs.microsoft.com/azure/virtual-network/) чтобы начать работу. В ней представлены описание и информация о развертывании для всех компонентов виртуальной сети.
...
- x.x.x.2, x.x.x.3: Зарезервировано Azure для отображения I-образных инологов Azure DNS в пространстве VNet
- x.x.x.255: адрес сетевой трансляции
...
Наименьшая поддерживаемая подсеть IPv4 - /29, а самая большая - /8 (с использованием определений подсети CIDR).  Подсети IPv6 должны быть точно /64 по размеру.  
...
Да, VNets может быть IPv4-только или двойной стек (IPv4'IPv6).  Для получения [подробной информации смотрите обзор IPv6 для виртуальных сетей Azure](./ipv6-overview.md).
...
Да. Вы можете в любое время изменить список DNS-серверов для виртуальной сети. Если вы измените список серверов DNS, необходимо выполнить продление аренды DHCP на всех затронутых ВМ в VNet, чтобы новые настройки DNS вступили в силу. Для VMs под управлением Windows OS `ipconfig /renew` вы можете сделать это, введя непосредственно на VM. Для других типов ОС, обратитесь к DHCP продление аренды документации для конкретного типа ОС. 
...
Nothing. IP-адреса (общедоступный виртуальный, общедоступный или частный) остаются назначенными для слота развертывания облачной службы или для виртуальной машины.
...
Да. Вы можете развертывать веб-приложения внутри VNet с помощью ASE (App Service Environment), подключать бэкэнд ваших приложений к Вашим VNets с помощью интеграции VNet и блокировать входящий трафик в ваше приложение конечными точками обслуживания. Дополнительные сведения см. в следующих статьях:

* [Функции сети Службы Приложений](../app-service/networking-features.md)
...
* [Ограничения доступа службы приложений](../app-service/app-service-ip-restrictions.md)
...
### <a name="can-i-connect-a-virtual-machine-scale-set-to-a-vnet"></a>Могу ли я подключить виртуальную шкалу машины к VNet?
Да. Необходимо подключить виртуальную шкалу машины к VNet.
...
### <a name="how-can-i-restrict-access-to-azure-paas-resources-from-a-vnet"></a>Как ограничить доступ к ресурсам Azure PaaS из VNet?

Ресурсы, развернутые через некоторые службы Azure PaaS (такие как База данных Azure Storage и База данных Azure S'L), могут ограничивать доступ к сети VNet с помощью виртуальных конечных точек сетевого обслуживания или частной ссылки Azure. Для получения подробной информации смотрите [обзор конечных точек виртуального сетевого обслуживания,](virtual-network-service-endpoints-overview.md) [обзор Azure Private Link](../private-link/private-link-overview.md)
...
Да. В [диспетчере ресурсов Azure](/rest/api/virtual-network) и [классических](https://go.microsoft.com/fwlink/?LinkId=296833) моделях развертывания можно использовать API REST для VNets.
...
Да. Глобальный пиринг виртуальных сетей позволяет создавать пиринговую связь между виртуальными сетями в разных регионах. Глобальное внедеворство VNet доступно во всех публичных регионах Azure, облачных регионах Китая и облачных регионах правительства. Нельзя глобально использовать из общедоступных регионов Azure в национальные облачные регионы.

### <a name="what-are-the-constraints-related-to-global-vnet-peering-and-load-balancers"></a>Каковы ограничения, связанные с глобальными балансами вобласти и нагрузкой VNet?
Если две виртуальные сети в двух разных регионах просматриваются над Global VNet Peering, вы не можете подключиться к ресурсам, которые находятся за балансом basic Load balancer через IP-адрес Front End of the Load Balancer. Это ограничение не существует для балансомера стандартной нагрузки.
Следующие ресурсы могут использовать основные балансы нагрузки, что означает, что вы не можете достичь их через IP-адрес Front End грузоотза через Global VNet Peering. Однако вы можете использовать global VNet пиринг для достижения ресурсов непосредственно через свои частные I-use VNet, если это разрешено. 
- VMs за основные балансы нагрузки
- Виртуальные наборы масштабов машин с балансироворами базовой нагрузки 
- кэш Redis; 
- Приложение шлюз (v1) SKU
- Service Fabric
- СЗЛ MI
- Управление API
- Служба доменов active Directory (ADDS)
- Logic Apps
- HDInsight
-   Пакетная служба Azure
- Среда службы приложений

Вы можете подключиться к этим ресурсам через ExpressRoute или VNet-to-VNet через шлюзы VNet.
...
Если подключение к внимательному ключу находится в *состоянии Инициированного,* это означает, что вы создали только одну ссылку. Чтобы установить успешное подключение, необходимо создать двунаправленную связь. Например, чтобы установить пиринговое подключение виртуальной сети А к виртуальной сети В, нужно создать связь из сети VNetA к сети VNetB и из сети VNetB к сети VNetA. Создание обеих ссылок изменит состояние на *Connected.*
...
Если вонючее соединение VNet находится в *отключенном* состоянии, это означает, что одна из созданных ссылок была удалена. Для восстановления одноранговой связи необходимо удалить ссылку и воссоздать ее.
...
### <a name="why-is-my-peering-connection-in-a-disconnected-state"></a>Почему мое пиринговое соединение находится в *отключенном состоянии?*
...
### <a name="how-can-i-troubleshoot-vnet-peering-issues"></a>Как устранить проблемы VNet Peering?
Вот [руководство по устранению неполадок,](https://support.microsoft.com/en-us/help/4486956/troubleshooter-for-virtual-network-peering-issues) которые вы можете попробовать.
...
Предварительный просмотр виртуальной сети TAP доступен во всех регионах Azure. Отслеживаемые сетевые интерфейсы, ресурс TAP виртуальной сети и сборщик или решение для аналитики должны быть развернуты в одном регионе.
...
У отслеживаемого сетевого интерфейса может быть только одна конфигурация TAP. Проверьте с индивидуальным [партнером решение](virtual-network-tap-overview.md#virtual-network-tap-partner-solutions) для возможности потоковой передачи нескольких копий трафика TAP на инструменты аналитики по вашему выбору.
...
Существует два шага для обеспечения безопасности ресурса службы Azure через конечные точки службы:
1. Включите конечные точки для службы Azure.
2. Настройте списки управления доступом виртуальной сети в службе Azure.

Первым шагом является операция на стороне сети и вторым – операция на стороне службы ресурсов. Оба действия выполняются либо с помощью администратора, либо разными администраторами на основании разрешений RBAC, предоставленных ролей администратора. Мы рекомендуем сначала включить конечные точки службы виртуальной сети перед настройкой списков управления доступом в виртуальной сети на стороне службы Azure. Таким образом, шаги должны выполняться в последовательности, указанной выше, для настройки конечных точек службы VNet.
...
Нет, не все службы Azure находятся в виртуальной сети клиента. Большинство служб данных Azure, таких как Azure Storage, Azure S'L и Azure Cosmos DB, являются мультитенантными службами, к которым можно получить доступ по общедоступным IP-адресам. Подробнее об интеграции виртуальных сетей для служб Azure можно узнать [здесь](virtual-network-for-azure-services.md). 
...
Функция конечных точек службы VNet (включение конечной точки службы VNet на стороне сети и настройка соответствующих АКЛ VNet на стороне службы Azure) ограничивает доступ службы Azure к разрешенной VNet и подсети, обеспечивая тем самым безопасность уровня сети и изолирования трафика службы Azure. Весь трафик, использующий конечные точки службы виртуальной сети, проходит через магистраль Майкрософт, обеспечивая тем самым еще один уровень изоляции от общедоступного Интернета. Кроме того, клиенты могут выбрать полностью отказаться от открытого доступа в Интернет для ресурсов служб Azure и разрешить трафик только из своей виртуальной сети, используя IP-брандмауэры и списки управления доступом для виртуальной сети, защищая, таким образом, ресурсы служб Azure от несанкционированного доступа.      
...
### <a name="can-an-on-premises-devices-ip-address-that-is-connected-through-azure-virtual-network-gateway-vpn-or-expressroute-gateway-access-azure-paas-service-over-vnet-service-endpoints"></a>Может ли IP-адрес на месте устройства, подключенный через шлюз виртуальной сети Azure (VPN) или шлюз ExpressRoute, получить доступ к службе Azure PaaS через конечные точки службы VNet?
...
### <a name="can-i-use-vnet-service-endpoint-feature-to-secure-azure-service-to-multiple-subnets-within-a-virtual-network-or-across-multiple-virtual-networks"></a>Могу ли я использовать функцию VNet Service Endpoint для обеспечения безопасности службы Azure для нескольких подсетей в виртуальной сети или в нескольких виртуальных сетях?
Чтобы обеспечить безопасность служб Azure для нескольких подсетей в виртуальной сети или в нескольких виртуальных сетях, включите конечные точки обслуживания на стороне сети на каждой из подсетей самостоятельно, а затем защитите ресурсы службы Azure для всех подсетей путем настройки соответствующие АКЛ VNet на стороне службы Azure.
...
### <a name="what-happens-when-you-access-an-azure-service-account-that-has-a-virtual-network-access-control-list-acl-enabled-from-outside-the-vnet"></a>Что происходит при доступе к учетной записи службы Azure, которая имеет виртуальный список управления доступом к сети (ACL), включенный из-за пределов VNet?
...
### <a name="can-an-azure-service-have-both-a-vnet-acl-and-an-ip-firewall"></a>Может ли служба Azure иметь как VNet ACL, так и брандмауэр IP??
Да, vNet ACL и IP-брандмауэр могут сосуществовать. Обе эти функции дополняют друг друга для обеспечения изоляции и безопасности.
...
Удаление виртуальных сетей и подсетей являются независимыми операциями и поддерживаются даже в том случае, если конечные точки включены для служб Azure. В тех случаях, когда службы Azure настраивают АКПлы VNet для тех VNets и подсетей, информация VNet ACL, связанная с этой службой Azure, отключена при удалении vNet или подсети, в которой включена конечная точка службы VNet.
 
### <a name="what-happens-if-an-azure-service-account-that-has-a-vnet-service-endpoint-enabled-is-deleted"></a>Что произойдет, если удаляется учетная запись службы Azure с включенной конечной точкой службы VNet?
Удаление учетной записи службы Azure является независимой операцией и поддерживается даже тогда, когда конечная точка службы включена на стороне сети и акфолы VNet настроены на стороне службы Azure. 
...
При включении конечных точек служб для виртуальной сети исходные IP-адреса ресурсов в подсети виртуальной сети будут переключаться с использования общедоступных адресов IPv4 на частные адреса виртуальной сети Azure для трафика в службу Azure. Обратите внимание, что это может привести к сбою в сбое в определенных брандмауэрах IP-адресов IPV4, которые ранее были общедоступными ipV4 в службах Azure. 

### <a name="does-the-service-endpoint-route-always-take-precedence"></a>Всегда ли приоритет имеет маршрут конечных точек обслуживания?
Конечные точки обслуживания добавляют системный маршрут, который имеет приоритет над маршрутами BGP и обеспечивает оптимальную маршрутизирующую для трафика конечных точек службы. Конечные точки службы всегда направляют трафик служб непосредственно из виртуальной сети в службу в магистральной сети Microsoft Azure. Для получения дополнительной информации о том, как Azure выбирает маршрут, [см.](virtual-networks-udr-overview.md)
...
Пользователь с правами на запись в виртуальной сети может настроить конечные точки служб в виртуальных сетях независимо друг от друга. Чтобы защитить ресурсы службы Azure к VNet, пользователь должен иметь разрешение **Microsoft.Network/virtualNetworks/subnets/joinViaServiceEndpoint/action** для добавленных подсетей. Это разрешение по умолчанию включено во встроенную роль администратора служб и может быть изменено при создании настраиваемых ролей. Узнайте больше о встроенных ролях и назначении разрешений, определенных для [настраиваемых ролей](https://docs.microsoft.com/azure/role-based-access-control/custom-roles?toc=%2fazure%2fvirtual-network%2ftoc.json).
...
### <a name="does-azure-active-directory-azure-ad-support-vnet-service-endpoints"></a>Поддерживает ли Active Directory Azure (Azure AD) конечные точки службы VNet?

Активный каталог Azure (Azure AD) не поддерживает конечные точки службы. Полный список служб Azure, поддерживающих конечные точки службы VNet, можно посмотреть [здесь.](https://docs.microsoft.com/azure/virtual-network/virtual-network-service-endpoints-overview) Обратите внимание, что тег "Microsoft.AzureActiveDirectory", указанный в конечных точках службы поддержки, используется для поддержки конечных точек службы ADLS Gen 1. Для ADLS Gen 1 виртуальная сетевая интеграция для Azure Data Lake Storage Gen1 использует безопасность виртуальной сетевой системы между виртуальной сетью и Active Directory (Azure AD) для генерации дополнительных требований безопасности в маркере доступа. Эти утверждения используются для проверки подлинности виртуальной сети в учетной записи ADLS 1-го поколения и для предоставления доступа. Узнайте больше об [интеграции Azure Data Lake Store Gen 1 VNet](../data-lake-store/data-lake-store-network-security.md?toc=%2fazure%2fvirtual-network%2ftoc.json)
...
Общее количество конечных точек служб в виртуальной сети не ограничено. Для ресурса службы Azure (например, учетная запись хранения Azure) службы могут назаять ограничения на количество подсетей, используемых для обеспечения безопасности ресурса. В следующей таблице приведены примеры некоторых ограничений. 
