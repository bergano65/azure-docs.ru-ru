...
- Вы можете связать не более одной группы безопасности сети с каждой подсетью в виртуальной сети. Ту же или другую группу безопасности сети можно связать с каждой подсетью. Каждая группа безопасности сети содержит правила, которые разрешают или запрещают входящий и исходящий трафик из источников и назначений. Подробнее о [группах сетевой безопасности.](#traffic-filtering)
...
Вы можете просматривать образцы конструкций для реализации сети периметра (также известной как ДМЗ) между Azure и Интернетом с помощью [NVA.](/azure/architecture/reference-architectures/dmz/secure-vnet-dmz?toc=%2Fazure%2Fvirtual-network%2Ftoc.json)
...
- Необходимо, чтобы трафик между подсетями проходил через виртуальный сетевой модуль. Чтобы узнать больше о том, как [настроить таблицы маршрутов, чтобы заставить трафик через NVA.](tutorial-create-route-table-portal.md)
...
Вы можете объединить пиринг и VPN шлюз для создания [концентратора и спицовых сетей,](/azure/architecture/reference-architectures/hybrid-networking/hub-spoke?toc=%2fazure%2fvirtual-network%2ftoc.json)где виртуальные сети подключаются к виртуальной сети концентратора, а концентратор подключается к приложечной сети, например.
...
Политики применяются к следующей иерархии: группе управления, подписке и группе ресурсов. Узнайте больше о [политике Azure](../governance/policy/overview.md?toc=%2fazure%2fvirtual-network%2ftoc.json) или разверните некоторые примеры [шаблона политики](policy-samples.md).
