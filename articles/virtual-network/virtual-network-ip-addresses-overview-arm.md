...
* **Общедоступные IP-адреса**: используются для связи с Интернетом, включая общедоступные службы Azure.
