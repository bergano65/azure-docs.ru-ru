...
4. Введите или выберите значения для перечисленных ниже параметров в разделе **Создать общедоступный IP-адрес**, а затем щелкните **Создать**.
   ...
   |Subscription|ДА|Ресурс в пределах одной и той же [подписки](../azure-glossary-cloud-terminology.md?toc=%2fazure%2fvirtual-network%2ftoc.json#subscription), с которым будет связан общедоступный IP-адрес.|
   ...
   |Location|ДА|Ресурс в пределах одного и того же [расположения](https://azure.microsoft.com/regions), с которым будет связан общедоступный IP-адрес.|
