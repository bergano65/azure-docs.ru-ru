...
4. Введите или выберите значения для перечисленных ниже параметров в разделе **Создать общедоступный IP-адрес**, а затем щелкните **Создать**.
   ...
   |Имя (только видимо, если вы выберете IP-версию **обоих)**|Да, если вы выберете IP-версию **обоих**|Имя должно отличаться от того, что вы ввели для первого **имени** в этом списке. Если вы решили создать IPv4- и IPv6-адрес, портал создаст два отдельных ресурса общедоступных IP-адресов, каждому из которых назначена своя версия.|
   |Назначение IP-адреса (только видимо, если вы выберете IP-версию **обоих)**|Да, если вы выберете IP-версию **обоих**|Те же ограничения, как назначение IP-адреса выше|
   |Подписка|Да|Должна существовать в той же [подписке,](../azure-glossary-cloud-terminology.md?toc=%2fazure%2fvirtual-network%2ftoc.json#subscription) что и ресурс, к которому вы будете ассоциировать общественные IP-адреса.|
   |Группа ресурсов|Да|Может существовать в той же или другой [группе ресурсов,](../azure-glossary-cloud-terminology.md?toc=%2fazure%2fvirtual-network%2ftoc.json#resource-group) как ресурс, к которому вы будете ассоциировать общественные IP..|
   |Расположение|Да|Должен существовать в том же [месте](https://azure.microsoft.com/regions), также называют регионом, как ресурс, к которому вы будете ассоциировать общественного IP's.|
...
Хотя на портале предоставляется возможность создания двух ресурсов общедоступных IP-адресов (IPv4 и IPv6), следующие команды PowerShell и интерфейса командной строки создают один ресурс с адресом для одной или другой версии IP. Если требуется два общедоступных ресурса IP-адреса, по одному для каждой IP-версии, вы должны запустить команду дважды, указав разные имена и IP-версии для общедоступных ресурсов IP-адреса.
...
|PowerShell|[Get-AzPublicIpAddress](/powershell/module/az.network/get-azpublicipaddress) для получения общедоступного объекта IP-адреса и просмотра его настроек, [Set-AzPublicIpAddress](/powershell/module/az.network/set-azpublicipaddress) для обновления настроек; [Удалить-AzPublicipAddress](/powershell/module/az.network/remove-azpublicipaddress) для удаления|
