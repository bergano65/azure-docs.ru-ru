...
Помимо пропускной способности, количество сетевых подключений, присутствующих на VM в любой момент времени, может повлиять на производительность сети. Сетевой стек Azure поддерживает состояние для каждого направления соединения TCP/UDP в структурах данных, называемых «потоками». Типичное соединение TCP/UDP будет иметь 2 потока, одно для входящего и другое для исходящего направления. 

Передача данных между конечными точками требует создания нескольких потоков в дополнение к тем, которые выполняют передачу данных. Некоторые примеры представляют потоки, созданные для разрешения DNS и потоки, созданные для зондов работоспособности баланса нагрузки. Также обратите внимание, что сетевые виртуальные приборы (NVA), такие как шлюзы, прокси, брандмауэры, будут видеть потоки создаются для подключения, прекращенные на приборе и возникнув прибором. 

![Подсчет потока для разговора TCP через передокающий прибор](media/virtual-machine-network-throughput/flow-count-through-network-virtual-appliance.png)

## <a name="flow-limits-and-recommendations"></a>Ограничения и рекомендации потока

Сегодня сетевой стек Azure поддерживает 250K общий поток сети с хорошей производительностью для VMs с более чем 8 процессорных ядер и 100k общий поток с хорошей производительностью для VMs с менее чем 8 ядер процессора. Прошлый этот предел производительности сети грациозно ухудшается для дополнительных потоков до жесткого предела 500K общий поток, 250K входящих и 250K исходящих, после чего дополнительные потоки будут удалены.

||VMs с <8 процессорных ядер|VMs с 8 "процессорных ядер|
|---|---|---|
|<b>Хорошая производительность</b>|100K Потоки |250K Потоки|
|<b>Деградированная производительность</b>|Выше 100k Потоки|Выше 250K Потоки|
|<b>Предел потока</b>|500K Потоки|500K Потоки|

В [Azure Monitor](../azure-monitor/platform/metrics-supported.md#microsoftcomputevirtualmachines) доступны метрики для отслеживания количества сетевых потоков и скорости создания потока в экземплярах VM или VMSS.

![лазурный монитор-поток-метрики.png](media/virtual-machine-network-throughput/azure-monitor-flow-metrics.png)

Установление подключения и коэффициенты прекращения могут также влиять на производительность сети в качестве создания соединения и прекращения акций процессора с процедурами обработки пакетов. Мы рекомендуем сопоставить рабочие нагрузки с ожидаемыми схемами трафика и соответствующим образом масштабировать рабочие нагрузки в соответствии с потребностями производительности. 
