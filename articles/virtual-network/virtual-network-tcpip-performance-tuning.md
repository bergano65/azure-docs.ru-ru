...
Вы можете задать начальный размер окна TCP и коэффициент масштабирования TCP в Windows с помощью `Set-NetTCPSetting` команды PowerShell. Дополнительные сведения см. в разделе  [Set-нетткпсеттинг](/powershell/module/nettcpip/set-nettcpsetting?view=win10-ps).
