...
ms.locfileid: "77431423"
---
# <a name="what-is-virtual-network-nat-public-preview"></a>Что такое NAT виртуальной сети (общедоступная предварительная версия)
...
- [Подсистема балансировки нагрузки](../load-balancer/load-balancer-overview.md)
- [Общедоступный IP-адрес](../virtual-network/virtual-network-ip-addresses-overview-arm.md#public-ip-addresses)
...
## <a name="configurable-idle-timeout"></a>Настраиваемое время ожидания простоя

По умолчанию время ожидания простоя равно 4 минутам, но его можно увеличить до 120 минут. Любое действие с потоком также может повлечь сброс таймера простоя, в том числе проверка активности TCP-подключения.
...
Сейчас NAT доступно в следующих регионах:
...
## <a name = "enable-preview"></a>Участие в использовании общедоступной предварительной версии

Чтобы разрешить участвовать в использовании общедоступной предварительной версии, соответствующие подписки должны быть зарегистрированы.  Участие в пользовании предполагает обязательный двухэтапный процесс и соблюдение инструкций, приведенных ниже для Azure CLI и Azure PowerShell.  Активация может занять несколько минут.
...
1. Зарегистрируйте подписку для общедоступной предварительной версии:

    ```azurecli-interactive
      az feature register --namespace Microsoft.Network --name AllowNatGateway
    ```

2. Активируйте регистрацию:
...
Во время действия общедоступной предварительной версии предоставляется скидка в размере 50 %.
...
- Если в виртуальной сети есть несколько подсетей, для каждой подсети можно настроить отдельное NAT.
