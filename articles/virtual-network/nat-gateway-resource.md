...
Зональное обязательство не действует, если a) зона экземпляра виртуальной машины и зоны зонального шлюза NAT не согласованы; б) ресурс регионального шлюза NAT используется с зональными экземплярами виртуальных машин.
...
Каждый шлюз NAT может поддерживать 64 000 подключений на каждый назначенный исходящий IP-адрес.  Дополнительные сведения см. в следующем разделе "Преобразование исходных сетевых адресов", а также ознакомьтесь со статьей об [устранении неполадок](https://docs.microsoft.com/azure/virtual-network/troubleshoot-nat), чтобы получить рекомендации по решению конкретных проблем.
