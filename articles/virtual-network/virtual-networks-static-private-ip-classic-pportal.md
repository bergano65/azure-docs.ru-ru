...
2. Выберите **NEW** > **Compute** > **Windows Server 2012 R2 Datacenter**, обратите внимание, что **список моделей развертывания** Select уже показывает **Классический,** а затем выберите **Создать.**
...
4. Выберите **Дополнительная конфигурационная** > **сеть** > **Виртуальной сети,** а затем выберите **TestVNet.** Если сеть **TestVNet** недоступна, убедитесь, что вы используете расположение *Центральная часть США* и создали тестовую среду, описанную в начале этой статьи.
...
1. На портале Azure выберите **BROWSE ALL Виртуальные** > **машины (классические)** > **DNS01** > **Все настройки** > **IP-адреса** и обратите внимание на назначение IP-адреса и IP-адрес, как показано ниже.
