...
1. Войти на [портал Azure](https://portal.azure.com) как UserA. У учетной записи, используемой для входа, должны быть необходимые разрешения для создания пиринга виртуальных сетей. Список разрешений см. в разделе [Создание, изменение и удаление пиринга в виртуальной сети](virtual-network-manage-peering.md#permissions).
...
8. В поле **Select** выберите UserB или введите адрес электронной почты UserB для его поиска. Отобразится список пользователей из того же клиента Azure Active Directory, с которым связана виртуальная сеть, с которой вы устанавливаете пиринг. Щелкните пользователя B, когда он появится в списке.
9. Нажмите **Сохранить**.
...
15. В вертикальном списке параметров в левой части отобразившейся колонки **myVnetB** щелкните **Свойства**. Скопируйте **Идентификатор ресурса**, он понадобится нам позже. Идентификатор ресурса аналогичен следующему примеру:`/subscriptions/<Subscription ID>/resourceGroups/myResourceGroupB/providers/Microsoft.ClassicNetwork/virtualNetworks/myVnetB`
...
5. Скопируйте следующий сценарий в текстовый редактор на своем компьютере. Замените `<SubscriptionB-Id>` идентификатором своей подписки. Если вам неизвестен идентификатор подписки, введите команду `az account show`. Значение **для идентификатора** в выходных выводах — это ваш идентификатор `Enter`подписки, копирование измененного скрипта, вставьте его на сеанс CLI, а затем нажмите.
...
8. Создайте пиринг между двумя виртуальными сетями, созданными с помощью разных моделей развертывания. Скопируйте следующий сценарий в текстовый редактор на своем компьютере. Замените `<SubscriptionB-id>` свой идентификатор подписки. Если вы не знаете свой идентификатор подписки, введите `az account show` команду. Значение **для идентификатора** в выходе — это подписка Id. Azure создала виртуальную сеть (классическую), созданную на шаг 4 в группе ресурсов под названием *Default-Networking.* Вставьте измененный сценарий в окно сеанса интерфейса командной строки и нажмите клавишу `Enter`.
...
11. **Необязательно**. Чтобы удалить ресурсы, созданные в этом руководстве, выполните инструкции, описанные в разделе [Удаление ресурсов](#delete-cli) этой статьи.
...
### <a name="azure-cli"></a><a name="delete-cli"></a>Лазурный CLI
