...
* **VirtualNetwork** (для развертываний Resource Manager) или **VIRTUAL_NETWORK** (для классических развертываний). Этот тег включает адресное пространство виртуальной сети (все диапазоны CIDR, определенные для виртуальной сети), все адресное пространство локальной, подключенных и [пиринговые](virtual-network-peering-overview.md) виртуальные сети или виртуальной сети, подключенные к [виртуальной шлюз сети](../vpn-gateway/vpn-gateway-about-vpngateways.md?toc=%2fazure%2fvirtual-network%2ftoc.json) и префиксы, используемые на [определяемые пользователем маршруты](virtual-networks-udr-overview.md).
...
* **Sql** (только для развертываний с помощью Resource Manager). Этот тег определяет префиксы адресов базы данных SQL Azure, база данных Azure для MySQL, база данных Azure для PostgreSQL и службы хранилища данных SQL Azure. Значение *Sql* отвечает за разрешение или запрет трафика в SQL. Чтобы разрешить доступ к SQL только в определенном [регионе](https://azure.microsoft.com/regions), можно указать этот регион. Например, чтобы разрешить доступ только к базе данных SQL Azure в регионе "Восточная часть США", вы можете указать *Sql.EastUS* как тег службы. Тег представляет службу, но не определенные экземпляры службы. Например, тег представляет службу "База данных SQL Microsoft Azure", но не определенную базу данных или сервер SQL Azure. 
...
* **ApiManagement** (только для развертываний с помощью Resource Manager). Этот тег определяет префиксы адресов для службы "Управление API Azure". Если указать *ApiManagement* для значения, трафик разрешен или запрещен в интерфейсе управления ApiManagement.  
