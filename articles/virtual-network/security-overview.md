...
|Протокол     | TCP, UDP или значение "Любой", которое также включает TCP, UDP и ICMP. Нельзя указать только ICMP, поэтому если требуется ICMP, используйте значение "Любой". |
...
Указанные ниже теги служб доступны для использования в [правила групп безопасности сети](https://docs.microsoft.com/azure/virtual-network/security-overview#security-rules). Теги служб звездочкой в конце (т. е. AzureCloud *) также может использоваться в [брандмауэр Azure Сетевые правила](https://docs.microsoft.com/azure/firewall/service-tags). 

* **VirtualNetwork** (для развертываний Resource Manager) или **VIRTUAL_NETWORK** (для классических развертываний). Этот тег включает адресное пространство виртуальной сети (все диапазоны CIDR, определенные для виртуальной сети), все адресное пространство локальной, подключенных [пиринговые](virtual-network-peering-overview.md) виртуальные сети или виртуальной сети, подключенные к [виртуальной шлюз сети](../vpn-gateway/vpn-gateway-about-vpngateways.md?toc=%2fazure%2fvirtual-network%2ftoc.json) и префиксы, используемые на [определяемые пользователем маршруты](virtual-networks-udr-overview.md). Имейте в виду, что этот тег может содержать маршрут по умолчанию. 
...
* **AzureCloud*** (только для Resource Manager): Этот тег определяет пространство IP-адресов для Azure, включая все [общедоступные IP-адреса центра обработки данных](https://www.microsoft.com/download/details.aspx?id=41653). Значение *AzureCloud* отвечает за разрешение или запрет трафика к общедоступным IP-адресам Azure. Если вы хотите разрешить доступ к AzureCloud в определенном [регион](https://azure.microsoft.com/regions), можно указать этот регион, в следующем формате AzureCloud. [ имя региона]. Этот тег рекомендуется для правила безопасности для исходящего трафика. 
* **AzureTrafficManager*** (только для Resource Manager): Этот тег определяет диапазон IP-адресов для IP-адресов пробы диспетчера трафика Azure. Дополнительные сведения об IP-адресах пробы диспетчера трафика см. в статье [Диспетчер трафика Azure: вопросы и ответы](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs). Этот тег рекомендуется для правила безопасности для входящего трафика.  
* **Хранилище*** (только для Resource Manager): Этот тег определяет пространство IP-адресов для службы хранилища Azure. Если указать значение *Storage*, трафик будет разрешен или запрещен в хранилище. Только если вы хотите разрешить доступ к хранилищу в определенном [регион](https://azure.microsoft.com/regions), можно указать этот регион, в следующем формате хранения. [ имя региона]. Тег представляет службу, но не определенные экземпляры службы. Например, тег представляет службу хранилища Azure, но не определенную учетную запись хранения Azure. Этот тег рекомендуется для правила безопасности для исходящего трафика. 
* **SQL*** (только для Resource Manager): Этот тег определяет префиксы адресов базы данных SQL Azure, база данных Azure для MySQL, база данных Azure для PostgreSQL и службы хранилища данных SQL Azure. Значение *Sql* отвечает за разрешение или запрет трафика в SQL. Если вы хотите разрешить доступ к Sql в определенном [регион](https://azure.microsoft.com/regions), можно указать этот регион, в следующем формате Sql. [регион]. Тег представляет службу, но не определенные экземпляры службы. Например, тег представляет службу "База данных SQL Microsoft Azure", но не определенную базу данных или сервер SQL Azure. Этот тег рекомендуется для правила безопасности для исходящего трафика. 
* **AzureCosmosDB*** (только для Resource Manager): Этот тег определяет префиксы адресов для службы базы данных Azure Cosmos. Значение *AzureCosmosDB* отвечает за разрешение или запрет трафика к Azure Cosmos DB. Если вы хотите только разрешить доступ к Azure Cosmos DB в определенном [регионе](https://azure.microsoft.com/regions), укажите этот регион в формате AzureCosmosDB.[имя региона]. Этот тег рекомендуется для правила безопасности для исходящего трафика. 
* **AzureKeyVault*** (только для Resource Manager): Этот тег определяет префиксы адресов для службы Azure Key Vault. Значение *AzureKeyVault* отвечает за разрешение или запрет трафика к Azure Key Vault. Если вы хотите только разрешить доступ к Azure Key Vault в определенном [регионе](https://azure.microsoft.com/regions), укажите этот регион в формате AzureKeyVault.[имя региона]. Этот тег имеет зависимость от **azureactivedirectory возникла ошибка** тега. Этот тег рекомендуется для правила безопасности для исходящего трафика.  
* **EventHub*** (только для Resource Manager): Этот тег определяет префиксы адресов для службы концентратора событий Azure. Значение *EventHub* отвечает за разрешение или запрет трафика в концентратор событий. Если вы хотите только разрешить доступ к концентратору событий в определенном [регионе](https://azure.microsoft.com/regions), укажите этот регион в формате "EventHub.[имя региона]". Этот тег рекомендуется для правила безопасности для исходящего трафика. 
* **ServiceBus*** (только для Resource Manager): Этот тег определяет префиксы адресов службы служебной шины Azure с помощью уровня службы "премиум". Значение *ServiceBus* отвечает за разрешение или запрет трафика в Служебную шину. Если вы хотите разрешить доступ к Служебной шине только в определенном [регионе](https://azure.microsoft.com/regions), укажите этот регион в формате "ServiceBus.[имя региона]". Этот тег рекомендуется для правила безопасности для исходящего трафика. 
* **MicrosoftContainerRegistry*** (только для Resource Manager): Этот тег определяет префиксы адресов для службы "Реестр контейнеров Майкрософт". Значение *MicrosoftContainerRegistry* отвечает за разрешение или запрет трафика в Реестр контейнеров Майкрософт. Если вы хотите разрешить доступ к Реестру контейнеров Майкрософт только в определенном [регионе](https://azure.microsoft.com/regions), укажите этот регион в формате "MicrosoftContainerRegistry.[имя региона]". Этот тег рекомендуется для правила безопасности для исходящего трафика. 
* **AzureContainerRegistry*** (только для Resource Manager): Этот тег определяет префиксы адресов для службы "Реестр контейнеров Azure". Значение *AzureContainerRegistry* отвечает за разрешение или запрет трафика в Реестр контейнеров Azure. Если вы хотите разрешить доступ к Реестру контейнеров Azure только в определенном [регионе](https://azure.microsoft.com/regions), укажите этот регион в формате "AzureContainerRegistry.[имя региона]". Этот тег рекомендуется для правила безопасности для исходящего трафика. 
* **AppService*** (только для Resource Manager): Этот тег определяет префиксы адресов для Службы приложений Azure. Значение *AppService* отвечает за разрешение или запрет трафика в Службу приложений. Если вы хотите разрешить доступ к Службе приложений только в определенном [регионе](https://azure.microsoft.com/regions), укажите этот регион в формате "AppService.[имя региона]". Этот тег рекомендуется для правила безопасности для исходящего трафика на серверы переднего плана веб-приложений.  
* **AppServiceManagement*** (только для Resource Manager): Этот тег определяет префиксы адресов управления трафиком для развертываний выделенной среды службы приложений. Значение *AppServiceManagement* отвечает за разрешение или запрет трафика в службу "Управление Службой приложений". Этот тег рекомендуется для правила входящего и исходящего безопасности. 
* **ApiManagement*** (только для Resource Manager): Этот тег определяет префиксы адресов трафик управления для службы управления API, выделенные развертываний. Значение *ApiManagement* отвечает за разрешение или запрет трафика в службу управления API. Этот тег рекомендуется для правила входящего и исходящего безопасности. 
* **AzureConnectors*** (только для Resource Manager): Этот тег определяет префиксы адресов соединителей приложений логики для проверки и серверных подключений. Значение *AzureConnectors* отвечает за разрешение или запрет трафика в Соединители Azure. Если вы хотите разрешить доступ к Соединителям Azure только в определенном [регионе](https://azure.microsoft.com/regions), укажите этот регион в формате "AzureConnectors.[имя региона]". Этот тег рекомендуется для правила безопасности для входящего трафика. 
* **GatewayManager** (только для развертываний с помощью Resource Manager). Этот тег определяет префиксы адресов трафика управления для развертываний выделенные шлюзы VPN или приложения. Значение *GatewayManager* отвечает за разрешение или запрет трафика в Диспетчер шлюзов. Этот тег рекомендуется для правила безопасности для входящего трафика. 
* **AzureDataLake*** (только для Resource Manager): Этот тег определяет префиксы адресов для службы Azure Data Lake. Значение *AzureDataLake* отвечает за разрешение или запрет трафика в Azure Data Lake. Этот тег рекомендуется для правила безопасности для исходящего трафика. 
* **Azureactivedirectory возникла ошибка*** (только для Resource Manager): Этот тег определяет префиксы адресов для службы Azure Active Directory. Значение *AzureActiveDirectory* отвечает за разрешение или запрет трафика в Azure Active Directory. Этот тег рекомендуется для правила безопасности для исходящего трафика.
* **AzureMonitor*** (только для Resource Manager): Этот тег определяет префиксы адресов Log Analytics, Application Insights, AzMon и пользовательские метрики (конечные точки ГБ ОЗУ). Значение *AzureMonitor* отвечает за разрешение или запрет трафика в AzureMonitor. Для Log Analytics, этот тег имеет зависимость от **хранения** тега. Этот тег рекомендуется для правила безопасности для исходящего трафика.
* **ServiceFabric*** (только для Resource Manager): Этот тег определяет префиксы адресов для службы ServiceFabric. Значение *ServiceFabric* отвечает за разрешение или запрет трафика в службе ServiceFabric. Этот тег рекомендуется для правила безопасности для исходящего трафика. 
* **AzureMachineLearning*** (только для Resource Manager): Этот тег определяет префиксы адресов для службы AzureMachineLearning. Значение *AzureMachineLearning* отвечает за разрешение или запрет трафика в службе AzureMachineLearning. Этот тег рекомендуется для правила безопасности для исходящего трафика. 
* **BatchNodeManagement*** (только для Resource Manager): Этот тег определяет префиксы адресов трафик управления для развертывания выделенного пакетной службы Azure. Если указать *BatchNodeManagement* для значения, трафик разрешен или запрещен из пакетной службы на вычислительных узлах. Этот тег рекомендуется для правила входящего и исходящего безопасности. 
* **AzureBackup*** (только для Resource Manager): Этот тег определяет префиксы адресов службы AzureBackup. Если указать *AzureBackup* для значения, трафик разрешен или запрещен для AzureBackup. Этот тег имеет зависимость от **хранения** и **azureactivedirectory возникла ошибка** тега. Этот тег рекомендуется для правила безопасности для исходящего трафика. 
* **AzureActiveDirectoryDomainServices*** (только для Resource Manager): Этот тег определяет префиксы адресов управления трафиком для выделенного развертывания доменных служб Azure Active Directory. Если указать *AzureActiveDirectoryDomainServices* для значения, разрешен или запрещен для AzureActiveDirectoryDomainServices трафика. Этот тег рекомендуется для правила входящего и исходящего безопасности.  
* **SqlManagement*** (только для Resource Manager): Этот тег определяет префиксы адресов трафик управления для SQL, выделенные развертываний. Если указать *SqlManagement* для значения, разрешен или запрещен для SqlManagement трафика. Этот тег рекомендуется для правила входящего и исходящего безопасности. 
* **CognitiveServicesManagement** (только для Resource Manager): Этот тег определяет префиксы адресов трафика для Cognitive Services. Если указать *CognitiveServicesManagement* для значения, разрешен или запрещен для CognitiveServicesManagement трафика. Этот тег рекомендуется для правила безопасности для исходящего трафика.  
* **Dynamics365ForMarketingEmail** (только для Resource Manager): Этот тег определяет префиксы адресов маркетинговой электронной почты службы Dynamics 365. Если указать *Dynamics365ForMarketingEmail* для значения, разрешен или запрещен для Dynamics365ForMarketingEmail трафика. Если вы хотите разрешить доступ к Dynamics365ForMarketingEmail в определенном [регион](https://azure.microsoft.com/regions), можно указать этот регион, в следующем формате Dynamics365ForMarketingEmail. [ имя региона].
* **AzurePlatformDNS** (только для Resource Manager): Этот тег обозначает DNS, которая является службой базовой инфраструктуры. Если указать *AzurePlatformDNS* в качестве значения по умолчанию можно отключить [рассмотрения платформы Azure](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) для DNS. Пройдите этот тег с осторожностью. Тестирование рекомендуется перед использованием этого тега. 
* **AzurePlatformIMDS** (только для Resource Manager): Этот тег обозначает IMDS, которая является службой базовой инфраструктуры. Если указать *AzurePlatformIMDS* в качестве значения по умолчанию можно отключить [рассмотрения платформы Azure](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) для IMDS. Пройдите этот тег с осторожностью. Тестирование рекомендуется перед использованием этого тега. 
* **AzurePlatformLKM** (только для Resource Manager): Этот тег обозначает лицензирования Windows или служба управления ключами. Если указать *AzurePlatformLKM* в качестве значения по умолчанию можно отключить [рассмотрения платформы Azure](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) для лицензирования. Пройдите этот тег с осторожностью. Тестирование рекомендуется перед использованием этого тега. 
...
### <a name="service-tags-in-on-premises"></a>Теги служб в локальной  
Вы можете скачать и интегрировать с брандмауэр подключения к локальной список тегов службы с помощью префикса сведения о следующих еженедельных публикаций для Azure [открытый](https://www.microsoft.com/download/details.aspx?id=56519), [для государственных организаций США](https://www.microsoft.com/download/details.aspx?id=57063), [Китая ](https://www.microsoft.com/download/details.aspx?id=57062), и [Германии](https://www.microsoft.com/download/details.aspx?id=57064) облаков.

Можно также программно получить эти сведения с помощью **API обнаружения тега службы** (Предварительная версия) — [REST](https://aka.ms/discoveryapi_rest), [Azure PowerShell](https://aka.ms/discoveryapi_powershell), и [ Azure CLI](https://aka.ms/discoveryapi_cli). 

> [!NOTE]
> Следуя еженедельных публикаций (старая версия) для Azure [открытый](https://www.microsoft.com/en-us/download/details.aspx?id=41653), [Китая](https://www.microsoft.com/en-us/download/details.aspx?id=42064), и [Германии](https://www.microsoft.com/en-us/download/details.aspx?id=54770) облаках будет считаться устаревшим с 30 июня 2020 г. Запустите с помощью обновленных публикаций, как описано выше. 
...
|Приоритет|Source|Исходные порты|Назначение|Конечные порты|Протокол|Access|
|---|---|---|---|---|---|---|
|65000|VirtualNetwork|0-65535|VirtualNetwork|0-65535|Все|РАЗРЕШИТЬ|
...
> [!NOTE]
> Группы безопасности сети связаны с подсетями или виртуальные машины и облачные службы, развернутые в классической модели развертывания и подсетям или сетевым интерфейсам в модели развертывания Resource Manager. Дополнительные сведения о моделях развертывания Azure см. в статье [Развертывание с помощью Azure Resource Manager и классическое развертывание: сведения о моделях развертывания и состоянии ресурсов](../azure-resource-manager/resource-manager-deployment-model.md?toc=%2fazure%2fvirtual-network%2ftoc.json).
