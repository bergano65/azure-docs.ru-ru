...
Общие настройки менеджера ресурсов Azure: Чтобы узнать больше о общих настройках диспетчера ресурсов Azure, см. [журнал активности,](../azure-monitor/platform/platform-logs-overview.md) [контроль доступа (IAM),](../role-based-access-control/overview.md) [Теги,](../azure-resource-manager/management/tag-resources.md?toc=%2fazure%2fvirtual-network%2ftoc.json) [Замки](../azure-resource-manager/management/lock-resources.md?toc=%2fazure%2fvirtual-network%2ftoc.json)и [Сценарий автоматизации.](../azure-resource-manager/templates/export-template-portal.md)
...
|PowerShell|[Get-AzNetworkInterface](/powershell/module/az.network/get-aznetworkinterface) для просмотра сетевых интерфейсов в настройках подписки или просмотра для сетевого интерфейса|
...
4. Выберите один из вариантов:
   ...
   - **Custom** (Пользовательские). Вы можете настроить собственный DNS-сервер для разрешения имен в нескольких виртуальных сетях. Введите IP-адрес сервера, который будет использоваться в качестве DNS-сервера. Указанный адрес DNS-сервера назначается только этому сетевому интерфейсу и переопределяет любые параметры DNS для виртуальной сети, которой назначен сетевой интерфейс.
     >[!Note]
     >Если VM использует NIC, который является частью набора доступности, все DNS-серверы, указанные для каждого из VMs из всех NICs, которые являются частью набора доступности, будут унаследованы.
...
|PowerShell|[Set-AzNetworkИнтерфейс](/powershell/module/az.network/set-aznetworkinterface)|
...
2. В разделе **Параметры** выберите **Сеть**.  Выберите **группы безопасности приложений,** затем **настройте группы безопасности приложений,** которые необходимо добавить в сетевой интерфейс, или не выберите группы безопасности приложений, из которых требуется удалить сетевой интерфейс, а затем выберите **Сохранить.** Только сетевые интерфейсы, находящиеся в одной виртуальной сети, можно добавить в одну группу безопасности приложений. Группа безопасности приложений должна находиться в том же расположении, что и сетевой интерфейс.
...
1. В верхней части портала Azure в поле с текстом *Поиск ресурсов* введите *сетевые интерфейсы*. Когда **сетевые интерфейсы** появятся в результатах поиска, выберите их.
2. Выберите сетевой интерфейс в списке, который вы хотите удалить.
3. Под **обзорОм** **Выберите Удалить**.
...
|PowerShell|[Удалить-AzNetworkИнтерфейс](/powershell/module/az.network/remove-aznetworkinterface)|
...
2. Выберите **сеть** в **соответствии с SETTINGS**.
