...
3. Введите или выберите значения для перечисленных ниже параметров, а затем щелкните **Создать**.
    ...
    |---|---|---|
    |Имя|Да|Имя должно быть уникальным в пределах выбранной группы ресурсов. Со временем в вашей подписке Azure скорее всего появится несколько сетевых интерфейсов. Ознакомьтесь с разделом [Правила именования и ограничения](/azure/cloud-adoption-framework/ready/azure-best-practices/naming-and-tagging#resource-naming), чтобы изучить рекомендации по созданию соглашения об именовании, упрощающего управление несколькими сетевыми интерфейсами. Имя невозможно изменить после создания сетевого интерфейса.|
...
3. Для выбранного сетевого интерфейса отображаются следующие элементы:
   - **Обзор.** Здесь содержатся сведения о сетевом интерфейсе, такие как назначенные IP-адреса, виртуальная сеть и подсеть, к которой назначен сетевой интерфейс, и виртуальная машина, к которой подключен сетевой интерфейс (если таковая имеется). На следующем рисунке показаны общие параметры для сетевого интерфейса с именем **mywebserver256**: ![ Общие сведения о сетевом интерфейсе.](./media/virtual-network-network-interface/nic-overview.png)

     Вы можете переместить сетевой интерфейс в другую группу ресурсов или подписку, выбрав (**изменить**) рядом с именем **группы ресурсов** или **подписки**. При перемещении сетевого интерфейса в новую подписку необходимо переместить все ресурсы, связанные с сетевым интерфейсом. Если сетевой интерфейс подключен, например, к виртуальной машине, также необходимо переместить виртуальную машину и другие ресурсы, связанные с ней. Сведения о перемещении сетевого интерфейса см. в статье [Перемещение ресурсов в новую группу ресурсов или подписку](../azure-resource-manager/management/move-resource-group-and-subscription.md?toc=%2fazure%2fvirtual-network%2ftoc.json#use-the-portal). В ней указаны необходимые условия для перемещения, а также процедура перемещения с использованием портала Azure, PowerShell и Azure CLI.
...
Общие параметры Azure Resource Manager. Дополнительные сведения о стандартных параметрах Azure Resource Manager см. в разделе [Журнал действий](../azure-monitor/essentials/platform-logs-overview.md), [Управление доступом (IAM)](../role-based-access-control/overview.md), [теги](../azure-resource-manager/management/tag-resources.md?toc=%2fazure%2fvirtual-network%2ftoc.json), [блокировки](../azure-resource-manager/management/lock-resources.md?toc=%2fazure%2fvirtual-network%2ftoc.json)и [скрипт автоматизации](../azure-resource-manager/templates/export-template-portal.md).

<a name="view-settings-commands"></a>**Меню**
...
|Средство|Get-Help|
|---|---|
|CLI|[az network nic list](/cli/azure/network/nic) для просмотра сетевых интерфейсов в подписке, [az network nic show](/cli/azure/network/nic) для просмотра параметров сетевого интерфейса.|
|PowerShell|[Get-азнетворкинтерфаце](/powershell/module/az.network/get-aznetworkinterface) , чтобы просмотреть сетевые интерфейсы в подписке или просмотреть параметры для сетевого интерфейса|
...
4. Выберите один из вариантов:
   ...
   - **Custom** (Пользовательские). Вы можете настроить собственный DNS-сервер для разрешения имен в нескольких виртуальных сетях. Введите IP-адрес сервера, который будет использоваться в качестве DNS-сервера. Указанный адрес DNS-сервера назначается только этому сетевому интерфейсу и переопределяет любые параметры DNS для виртуальной сети, которой назначен сетевой интерфейс.
     >[!Note]
     >Если виртуальная машина использует сетевую карту, которая входит в группу доступности, все DNS-серверы, указанные для каждой из виртуальных машин из всех сетевых карт, входящих в группу доступности, будут унаследованы.
...
|PowerShell|[Set-Азнетворкинтерфаце](/powershell/module/az.network/set-aznetworkinterface)|
...
5. Нажмите кнопку **Сохранить**. Новые динамические адреса назначаются из диапазона адресов новой подсети. После назначения сетевого интерфейса в новую подсеть ей можно будет назначить статический IPv4-адрес из диапазона адресов новой подсети, если это требуется. Дополнительные сведения о добавлении, изменении и удалении IP-адресов для сетевого интерфейса см. в [этой статье](virtual-network-network-interface-addresses.md).
...
|PowerShell|[Set-Азнетворкинтерфацеипконфиг](/powershell/module/az.network/set-aznetworkinterfaceipconfig)|
...
2. В разделе **Параметры** выберите **сеть**.  Выберите **группы безопасности приложений** , затем **Настройте группы** безопасности приложений, чтобы выбрать группы безопасности приложений, в которые нужно добавить сетевой интерфейс, или снимите флажки для групп безопасности приложений, из которых необходимо удалить сетевой интерфейс, а затем нажмите кнопку **сохранить**. Только сетевые интерфейсы, находящиеся в одной виртуальной сети, можно добавить в одну группу безопасности приложений. Группа безопасности приложений должна находиться в том же расположении, что и сетевой интерфейс.
...
1. В верхней части портала Azure в поле с текстом *Поиск ресурсов* введите *сетевые интерфейсы*. Когда **сетевые интерфейсы** появятся в результатах поиска, выберите их.
2. Выберите в списке сетевой интерфейс, который требуется удалить.
3. В разделе **Обзор** выберите **Удалить**.
...
|PowerShell|[Remove-Азнетворкинтерфаце](/powershell/module/az.network/remove-aznetworkinterface)|
...
Действующие правила безопасности для каждого сетевого интерфейса, подключенного к виртуальной машине, представляют собой сочетание правил, которые вы создали в группе безопасности сети, и [правил безопасности по умолчанию](./network-security-groups-overview.md#default-security-rules). Общие сведения о действующих правилах безопасности сетевого интерфейса могут помочь определить, почему не удается подключиться к виртуальной машине или из нее. Вы можете просмотреть действующие правила для любого сетевого интерфейса, подключенного к работающей виртуальной машине.
...
5. Просмотрите список действующих правил безопасности, чтобы определить, имеются ли корректные правила для требуемых входящих и исходящих подключений. Дополнительные сведения о том, что отображается в списке, можно найти в разделе [Безопасность сети](./network-security-groups-overview.md).
...
- PowerShell: [Get-азеффективенетворксекуритиграуп](/powershell/module/az.network/get-azeffectivenetworksecuritygroup)
