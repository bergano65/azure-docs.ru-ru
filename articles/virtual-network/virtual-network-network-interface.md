...
3. Введите или выберите значения для перечисленных ниже параметров, а затем щелкните **Создать**.
    ...
    |Resource group|Да|Выберите существующую [группу ресурсов](../azure-glossary-cloud-terminology.md?toc=%2fazure%2fvirtual-network%2ftoc.json#resource-group) или создайте новую. Сетевой интерфейс может находится в одной группе ресурсов с виртуальной машиной, к которой он подключен, и с виртуальной сетью или в разных.|
    |Местоположение|Да|Виртуальная машина и виртуальная сеть, к которой подключен сетевой интерфейс, должна находится в одном [расположении](https://azure.microsoft.com/regions) (регионе).|
...
|PowerShell|[Get-AzNetworkInterface](/powershell/module/az.network/get-aznetworkinterface) для просмотра сетевых интерфейсов в подписке или параметров сетевого интерфейса|
...
4. Выберите один из вариантов:
   ...
   - **Настраиваемая**. Вы можете настроить собственный DNS-сервер для разрешения имен в нескольких виртуальных сетях. Введите IP-адрес сервера, который будет использоваться в качестве DNS-сервера. Указанный адрес DNS-сервера назначается только этому сетевому интерфейсу и переопределяет любые параметры DNS для виртуальной сети, которой назначен сетевой интерфейс.
     >[!Note]
     >Если виртуальная машина использует сетевой Адаптер, который входит в группу доступности, все DNS-серверы, указанные для каждой виртуальной машины из всех сетевых адаптеров, которые являются частью группы доступности будут наследоваться.
