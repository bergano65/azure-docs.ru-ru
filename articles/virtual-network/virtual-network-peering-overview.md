...
Транзитный шлюз поддерживается как для пиринга виртуальной сети, так и для пиринга глобальной виртуальной сети. Транзит шлюза между виртуальными сетями, созданными с помощью разных моделей развертывания (диспетчер ресурсов и классическая модель), поддерживается только в том случае, если шлюз находится в виртуальной сети (диспетчер ресурсов). См. дополнительные сведения о [настройке VPN-шлюза для передачи данных с помощью пиринга между виртуальными сетями](../vpn-gateway/vpn-gateway-peering-gateway-transit.md?toc=%2fazure%2fvirtual-network%2ftoc.json).
...
- Ресурсы в одной виртуальной сети не могут взаимодействовать с интерфейсным IP-адресом базового внутреннего балансировщика нагрузки в глобальной виртуальной сети. Поддержка базовых Load Balancer существует только в пределах одного региона. Поддержка Load Balancer (цен. категория "Стандартный") существует как для пиринга виртуальной сети, так и для пиринга глобальной виртуальной сети. В [этой статье](virtual-networks-faq.md#what-are-the-constraints-related-to-global-vnet-peering-and-load-balancers) описаны службы, использующие базовый балансировщик нагрузки, который не будет работать через глобальную виртуальную сеть.
...
>[!NOTE]
> В предыдущей версии этого документа было сказано, что плата за пиринг виртуальных сетей не будет применяться к транзитному шлюзу. Это Обновлено для отражения точных цен на страницу цен.
