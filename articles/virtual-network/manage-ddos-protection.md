...
План защиты от атак DDoS определяет набор виртуальных сетей, в которых включен стандарт защиты от атак DDoS, для подписок. Вы можете настроить один план защиты от атак DDoS для своей организации и привязать к нему виртуальные сети из нескольких подписок. Сам план защиты от атак DDoS также связан с подпиской, выбранной во время его создания. План защиты DDoS работает в разных регионах и подписках. Пример - вы можете создать план в регионе Восток-США и ссылку на подписку #1 в вашем арендаторе. Тот же план может быть связан с виртуальными сетями из других подписок в разных регионах, через ваш арендатор. Для подписки, с которой связан план, выставляется ежемесячный счет за его использование, а также за избыточное использование в случае, если количество защищенных общедоступных IP-адресов превышает 100. Дополнительные сведения о ценах на защиту от атак DDoS приведены [здесь](https://azure.microsoft.com/pricing/details/ddos-protection/).
...
1. Щелкните **Создать ресурс** в верхнем левом углу окна портала Azure.
2. Выберите **Сеть**, а затем выберите **Виртуальную сеть.**
...
5. Выберите **стандарт**. В разделе **DDoS protection plan** (План защиты от атак DDoS) выберите существующий план защиты от атак DDoS или план, созданный на шаге 1, а затем щелкните **Сохранить**. Выбираемый план не обязательно должен находиться в той же подписке, что и виртуальная сеть, но обе подписки должны быть связаны с одним клиентом Azure Active Directory.

**Команды** 
- Azure CLI: [аз сети ddos-защиты создать](https://docs.microsoft.com/cli/azure/network/ddos-protection?view=azure-cli-latest#az-network-ddos-protection-create)
- Powershell: [Новый-AzDdosProtectionPlan](https://docs.microsoft.com/powershell/module/Az.Network/New-AzDdosProtectionPlan?view=azps-2.8.0)
...
**Команды** 
- Azure CLI: [аз сети ddos-защиты удаления](https://docs.microsoft.com/cli/azure/network/ddos-protection?view=azure-cli-latest#az-network-ddos-protection-delete)
- Мощность [оболочки: Удалить-AzDdosProtectionPlan](https://docs.microsoft.com/powershell/module/az.network/remove-azddosprotectionplan?view=azps-3.2.0)
...
1. В левой верхней части портала выберите **Все службы**.
2. Введите *Monitor* в поле **Фильтр**. Когда в результатах поиска появится элемент **Monitor**, щелкните его.
3. Выберите **метрики,** в рамках **SHARED SERVICES**.
...
7. Выберите тип **агрегации** как **Max**
...
Служба "Защита от атак DDoS" уровня "Стандартный" применяет три автоматически настраиваемые политики устранения атак (TCP SYN, TCP и UDP) для каждого общедоступного IP-адреса защищенного ресурса в виртуальной сети с включенной защитой от атак DDoS. Можно просмотреть пороговые значения политики, выбрав **входные пакеты TCP для запуска пакетов DDoS** и **входящих пакетов UDP для запуска** метрик смягчения DDoS с типом **агрегирования** как "Max", как показано на следующем рисунке:
...
6. Выберите **Turn on diagnostics to collect the DDoSMitigationReports log** (Включить диагностику для сбора журнала DDoSMitigationReports) и выберите необходимые параметры из перечисленных ниже.
    ...
    - **Отправка в журнал Analytics**: Записывает журналы в службу Мониторинга Azure. Чтобы узнать больше об этой опции, [см. журналы collect для использования в журналах Azure Monitor.](../azure-monitor/platform/collect-azure-metrics-logs.md?toc=%2fazure%2fvirtual-network%2ftoc.json)
...
## <a name="view-ddos-protection-alerts-in-azure-security-center"></a>Просмотр предупреждений о защите DDoS в Центре безопасности Azure

Центр безопасности Azure предоставляет список [предупреждений безопасности](/azure/security-center/security-center-managing-and-responding-alerts)с информацией, которая поможет исследовать и устранять проблемы. С помощью этой функции вы получаете единое представление оповещений, включая оповещения, связанные с DDoS-атаками, и действия, предпринятые для смягчения атаки в ближайшее время.
Есть два конкретных оповещения, которые вы увидите для обнаружения и смягчения DDoS-атак:

- **DDoS-атака, обнаруженная для общедоступной IP:** Это оповещение генерируется, когда служба защиты DDoS обнаруживает, что один из ваших общедоступных IP-адресов является объектом DDoS-атаки.
- **DDoS-атака смягчена для общедоступного IP:** Это оповещение генерируется при смягчении атаки на общедоступный IP-адрес.
Чтобы просмотреть оповещения, откройте **Центр безопасности** на портале Azure. Под **защитой от угроз**выберите оповещения **безопасности.** На следующем скриншоте показан пример оповещений о DDoS-атаке.

![DDoS-оповещение в Центре безопасности Azure](./media/manage-ddos-protection/ddos-alert-asc.png)

Предупреждения включают общую информацию об общедоступном IP-адресе, который находится под атакой, информацию о гео- и угрозах, а также шаги по устранению последствий.
