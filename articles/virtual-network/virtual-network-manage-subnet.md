...
4. В диалоговом окне **Добавление подсети** введите значения для следующих параметров.
    ...
    | Параметр | Описание |
    | --- | --- |
    | **Name** | Имя должно быть уникальным в пределах виртуальной сети. Для обеспечения максимальной совместимости с другими службами Azure рекомендуется использовать в качестве первого символа имени букву. Например, шлюз приложений Azure не удастся развернуть в подсети с именем, которое начинается с цифры. |
