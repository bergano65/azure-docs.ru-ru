...
- Поддержка [стандартных общедоступных Load Balancer IPv6](virtual-network-ipv4-ipv6-dual-stack-standard-load-balancer-powershell.md) для создания устойчивых масштабируемых приложений, в том числе:
    ...
    - Примечание. Балансировка нагрузки не выполняет преобразование протокола (без NAT64). 
...
- IPv6 для виртуальной сети Azure доступен во всех глобальных коммерческих регионах Azure с помощью всех методов развертывания.  Развертывание в облаке правительства США временно ограничено шаблоном ARM (JSON), интерфейсом командной строки (CLI) и PowerShell.  Поддержка IPv6 на облачном портале для государственных организаций США будет доступна в ближайшее время.  
- Шлюзы ExpressRoute можно использовать для трафика только с IPv4 в виртуальной сети с включенным протоколом IPv6.  Поддержка трафика IPv6 находится в нашей стратегии.   
- VPN-шлюзы нельзя использовать в виртуальной сети с включенным протоколом IPv6 либо напрямую, либо с помощью "UseRemoteGateway".
