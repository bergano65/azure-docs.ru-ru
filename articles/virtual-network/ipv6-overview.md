...
Преимущества виртуальной сети Azure для IPv6:
...
IPv6 для виртуальной сети включает следующие возможности.
...
- Двойные виртуальные сети (IPv4 и IPv6) с двумя подсетями стека позволяют приложениям подключаться к ресурсам IPv4 и IPv6 в своей виртуальной сети или в Интернете.
    > [!IMPORTANT]
    > Размер подсетей для IPv6 должен быть ровно/64.  Это гарантирует совместимость, если вы решите включить маршрутизацию подсети в локальную сеть, так как некоторые маршрутизаторы могут принимать только маршруты IPv6/64.  
...
- Поддержка общедоступной службы Load Balancer ценовой категории "Стандартный" IPv6 для создания устойчивых и масштабируемых приложений. В поддержку входит:
...
- Общедоступный IP-адрес уровня экземпляра предоставляет возможность подключения к Интернету через IPv6 напрямую к отдельным виртуальным машинам.
- Простое добавление IPv6-подключения к существующим развертываниям только по протоколу IPv4 с обновлением на месте.
- Создавайте приложения с двойным стеком, которые автоматически масштабируются до нагрузки с помощью масштабируемых наборов виртуальных машин.
- Поддержка портала для предварительной версии включает в себя интерактивное создание, изменение и удаление виртуальных сетей и подсетей с двойным стеком (IPv4 и IPv6), правила групп безопасности сети IPv6, определяемые пользователем маршруты IPv6 и общедоступные IP-адреса IPv6.  

## <a name="limitations"></a>Ограничения
Предварительный выпуск IPv6 для виртуальной сети Azure имеет следующие ограничения.
- IPv6 для виртуальной сети Azure (Предварительная версия) доступен во всех глобальных регионах Azure, но только в глобальной службе Azure — не в правительственных облаках.
- Поддержка Load Balancer (цен. категория "Стандартный") компонентов на портале доступна только для просмотра.  Однако полная поддержка и документация (с примерами) доступны для Load Balancer (цен. категория "Стандартный") развертываний с помощью Azure PowerShell и интерфейса командной строки (CLI).   
- Поддержка службы "наблюдатель за сетями" для предварительной версии ограничена NSG журналами потоков и записью сетевых пакетов.
- Пиринг виртуальных сетей (с региональными или глобальными) не поддерживается в предварительной версии.
- При использовании стандартного внешнего Load Balancer IPv6 применяются следующие ограничения. 
  - Правила исходящего трафика могут ссылаться на несколько интерфейсных общедоступных IP-адресов, но могут **не** ссылаться на общедоступный префикс IPv6. Общедоступный префикс IP-адреса поддерживает только префиксы IPv4.
  - Правила балансировки нагрузки IPv6 **не** могут использовать функцию *плавающего IP-адреса* . Повторное использование портов на внутренних экземплярах поддерживается только с IPv4.
- Резервирование блока IPv6-адресов с выходом в Интернет не поддерживается компонентом префикса общедоступного IP-адреса Azure.
...
- Узнайте, как [развернуть приложение с двумя стеками IPv6 с помощью Azure PowerShell](virtual-network-ipv4-ipv6-dual-stack-powershell.md).
- Узнайте, как [развернуть приложение с двумя стеками IPv6 с помощью Azure CLI](virtual-network-ipv4-ipv6-dual-stack-cli.md).
