...
Распространение маршрутов ExpressRoute и VPN-шлюзов можно отключить для подсети с помощью свойства в таблице маршрутов. Если обмен маршрутами с Azure выполняется по протоколу BGP, маршруты не добавляются в таблицу маршрутов для всех подсетей при отключении распространения маршрутов в шлюзе виртуальной сети. Активировать VPN-подключения можно с помощью [пользовательских маршрутов](#custom-routes) и *VPN-шлюза* с типом следующего прыжка. **Распространение маршрутов BGP не должно быть отключено для GatewaySubnet. Шлюз не будет работать, если этот параметр отключен.** Дополнительные сведения см. в разделе [Создание, изменение и удаление таблицы маршрутов](manage-route-table.md#create-a-route-table).
...
Дополнительные сведения об использовании шлюзов виртуальной сети между Интернетом и Azure см. в статье [Сеть периметра между Azure и локальным центром обработки данных](/azure/architecture/reference-architectures/dmz/secure-vnet-hybrid?toc=%2fazure%2fvirtual-network%2ftoc.json).
...
|По умолчанию |Активна |10.0.0.0/8          |None                      |                   |
|По умолчанию |Активна |100.64.0.0/10       |None                      |                   |
