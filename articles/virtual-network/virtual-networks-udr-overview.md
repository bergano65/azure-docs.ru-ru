...
2. Настройте в локальной сети безопасный обмен данными с обеими виртуальными сетями через Интернет с помощью туннеля VPN. *Кроме того можно использовать подключение ExpressRoute, но в этом примере используется VPN-подключение.*
...
- [Создайте таблицу маршрутов, определяемых пользователем маршрутов и сетевого виртуального модуля](tutorial-create-route-table-portal.md)
- [Настройка BGP для VPN-шлюз Azure](../vpn-gateway/vpn-gateway-bgp-resource-manager-ps.md?toc=%2fazure%2fvirtual-network%2ftoc.json)
