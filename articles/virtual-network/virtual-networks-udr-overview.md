...
|Пиринговая связь между виртуальными сетями         |Пиринговая связь между виртуальными сетями                                    |Неприменимо|
...
Распространение маршрутов ExpressRoute и VPN-шлюзов можно отключить для подсети с помощью свойства в таблице маршрутов. При обмене маршрутами с Azure с использованием протокола BGP маршруты не добавляются в таблицу маршрутов всех подсетей при отключенном распространении BGP. Активировать VPN-подключения можно с помощью [пользовательских маршрутов](#custom-routes) и *VPN-шлюза* с типом следующего прыжка. См. дополнительные сведения об [отключении распространения маршрутов BGP](manage-route-table.md#create-a-route-table).
...
|ИД  |Источник |Состояние  |Префиксы адресов    |Тип следующего прыжка          |Следующий IP-адрес прыжка|Название определяемого пользователем маршрута| 
|----|-------|-------|------              |-------                |--------           |--------      |
|1   |значение по умолчанию|Недействительно|10.0.0.0/16         |Виртуальная сеть        |                   |              |
|2   |Пользователь   |Активен |10.0.0.0/16         |Виртуальное устройство      |10.0.100.4         |В пределах виртуальной сети (VNet) 1  |
|3   |Пользователь   |Активен |10.0.0.0/24         |Виртуальная сеть        |                   |В пределах подсети 1|
|4.   |значение по умолчанию|Недействительно|10.1.0.0/16         |Пиринговая связь между виртуальными сетями           |                   |              |
|5   |значение по умолчанию|Недействительно|10.2.0.0/16         |Пиринговая связь между виртуальными сетями           |                   |              |
|6   |Пользователь   |Активен |10.1.0.0/16         |Нет                   |                   |К виртуальной сети 2 — 1 —прервать|
|7   |Пользователь   |Активен |10.2.0.0/16         |Нет                   |                   |К виртуальной сети 2 — 2 —прервать|
|8   |значение по умолчанию|Недействительно|10.10.0.0/16        |Шлюз виртуальной сети|[X.X.X.X]          |              |
|9   |Пользователь   |Активен |10.10.0.0/16        |Виртуальный модуль      |10.0.100.4         |К локальной среде    |
|10  |значение по умолчанию|Активен |[X.X.X.X]           |VirtualNetworkServiceEndpoint    |         |              |
|11  |значение по умолчанию|Недействительно|0.0.0.0/0           |Интернет               |                   |              |
|12  |Пользователь   |Активен |0.0.0.0/0           |Виртуальное устройство      |10.0.100.4         |Виртуальный сетевой модуль по умолчанию   |
...
|Источник  |Состояние  |Префиксы адресов    |Тип следующего прыжка             |Следующий IP-адрес прыжка|
|------- |-------|------              |-------                   |--------           
|значение по умолчанию |Активен |10.0.0.0/16         |Виртуальная сеть           |                   |
|значение по умолчанию |Активен |10.1.0.0/16         |Пиринговая связь между виртуальными сетями              |                   |
|значение по умолчанию |Активен |10.2.0.0/16         |Пиринговая связь между виртуальными сетями              |                   |
|значение по умолчанию |Активен |10.10.0.0/16        |Шлюз виртуальной сети   |[X.X.X.X]          |
|значение по умолчанию |Активен |0.0.0.0/0           |Интернет                  |                   |
|значение по умолчанию |Активен |10.0.0.0/8          |Нет                      |                   |
|значение по умолчанию |Активен |100.64.0.0/10       |Нет                      |                   |
|значение по умолчанию |Активен |172.16.0.0/12       |Нет                      |                   |
|значение по умолчанию |Активен |192.168.0.0/16      |Нет                      |                   |
