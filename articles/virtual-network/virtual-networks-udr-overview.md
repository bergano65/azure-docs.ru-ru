...
Распространение маршрутов ExpressRoute и VPN-шлюзов можно отключить для подсети с помощью свойства в таблице маршрутов. При обмене маршрутами с Azure с помощью BGP маршруты не добавляются в таблицу маршрутов всех подсетей с отключением маршрутов маршрутов виртуальной сети. Активировать VPN-подключения можно с помощью [пользовательских маршрутов](#custom-routes) и *VPN-шлюза* с типом следующего прыжка. **Распространение маршрута не должно быть отключено на GatewaySubnet. Шлюз не будет функционировать с этим параметром отключен.** Для получения подробной информации, см [Как отключить виртуальную сеть шлюз маршрут распространения](manage-route-table.md#create-a-route-table).
