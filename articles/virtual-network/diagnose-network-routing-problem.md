...
3. Под **настройками** слева выберите **Networking**и перейдите на ресурс сетевого интерфейса, выбрав его имя.
     ![Просмотр сетевых интерфейсов](./media/diagnose-network-routing-problem/view-nics.png)
4. Слева выберите **эффективные маршруты.** Эффективные маршруты для сетевого интерфейса под названием **myVMNic1** показаны на следующей картинке: ![Просмотр эффективных маршрутов](./media/diagnose-network-routing-problem/view-effective-routes.png)
...
Получите эффективные маршруты для сетевого интерфейса с [Get-AzEffectiveRouteTable](/powershell/module/az.network/get-azeffectiveroutetable). Следующий пример получает эффективные маршруты для сетевого интерфейса под названием *myVMNic1*, то есть в группе ресурсов под названием *myResourceGroup*:
...
В предыдущем выходе название сетевого интерфейса *myVMNic1*.
...
- Для правильной работы пирингового трафика виртуальной сети системный маршрут с типом следующего прыжка *Пиринг VNet* должен действовать в диапазоне префиксов пиринговой виртуальной сети. Если такого маршрута не существует, а виртуальная сетевая вглядывающая **ссылка подключена:**
