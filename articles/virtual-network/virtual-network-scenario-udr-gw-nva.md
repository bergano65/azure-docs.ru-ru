...
Это стандартный (также knowns как DMZ) сеть периметра с сетью Периметра и защищенной сети. Такие сценарии можно реализовать в Azure с помощью групп безопасности сети, виртуальные устройства брандмауэров или их сочетание. В следующей таблице приведены преимущества и недостатки NSG и виртуальных устройств брандмауэров.
...
В описанном ниже решении используются виртуальные устройства брандмауэров сети периметра (DMZ) / защищенные сценарии сети.
...
| Место назначения | Следующий прыжок | Объяснение |
| --- | --- | --- |
...
* **маршрут**: Весь трафик на 10.0.0.0/16 (**azurevnet**) должны быть отправлены через туннель **ONPREMAZURE**.
* **Политика**. Разрешить любого двунаправленного трафика между **port2** и **ONPREMAZURE**.

### <a name="azf1"></a>AZF1
AZF1 — это виртуальное устройство Azure со следующими правилами:

* **Политика**. Разрешить любого двунаправленного трафика между **port1** и **port2**.

### <a name="azf2"></a>AZF2
AZF2 — это виртуальное устройство Azure со следующими правилами:

* **маршрут**: Весь трафик на 10.0.0.0/16 (**onpremvnet**) должны быть отправлены шлюза Azure IP-адрес (т. е. 10.0.0.1) через **port1**.
* **Политика**. Разрешить любого двунаправленного трафика между **port1** и **port2**.
...
5. После подготовки всех ресурсов, входить **onpremvm2** и проверьте связь с адресом 10.0.3.101, чтобы проверить возможность подключения между **onpremsn2** и **azsn3**.
