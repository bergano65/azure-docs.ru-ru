...
5. <a name="add-peering"></a>Введите или выберите значения описанных ниже параметров.
    ...
    - **Разрешить перенаправленный трафик**. Установите этот флажок, чтобы разрешить трафик, *переадресованный* виртуальным сетевым модулем в виртуальной сети (то есть который не был изначально передан из виртуальной сети) в поток для этой виртуальной сети с помощью пиринга. Например, рассмотрим три виртуальные сети: Spoke1, Spoke2 и Hub. Пиринг существует между каждой виртуальной сеть Spoke и виртуальной сетью Hub, но между виртуальными сетями Spoke пиринг отсутствует. Виртуальный сетевой модуль развернут в виртуальной сети Hub, и к каждой виртуальной сети Spoke применяются определяемые пользователем маршруты, направляющие трафик между подсетями через виртуальный сетевой модуль. Если этот флажок не проверяется на предмет вглядывания между каждой виртуальной сетью и виртуальной сетью концентратора, трафик не проходит между виртуальными сетями, поскольку концентратор не пересылает трафик между виртуальными сетями. Если эта возможность активирована, перенаправленный трафик передается с помощью пирингового подключения, но при этом определяемые пользователем маршруты или виртуальные модули сетей не создаются. Определяемые пользователем маршруты или виртуальные модули сетей создаются отдельно. Дополнительные сведения об [определяемых пользователем маршрутах](virtual-networks-udr-overview.md#user-defined). Этот флажок не нужно устанавливать, если трафик между виртуальными сетями переадресовывается через VPN-шлюз Azure.
    ...
    - **Использовать удаленные шлюзы.** Установите этот флажок, чтобы трафик из исходной виртуальной сети проходил через шлюз, подключенный к виртуальной сети, с которой устанавливается пиринг. Например, к виртуальной сети, с которой устанавливается пиринг, подключен VPN-шлюз, обеспечивающий обмен данными с локальной сетью.  После установки этого флажка трафик из исходной виртуальной сети проходит через VPN-шлюз, подключенный к пиринговой виртуальной сети. Для этого к пиринговой виртуальной сети должен быть подключен шлюз, а также необходимо установить флажок **Разрешить транзит шлюзов**. Если не устанавливать этот флажок (настройка по умолчанию), трафик из пиринговой виртуальной сети по-прежнему поступает в исходную виртуальную сеть, но не проходит через подключенный к ней шлюз.
      ...
      Вы не можете использовать удаленные шлюзы, если у вас уже есть шлюз, настроенный в вашей виртуальной сети. См. дополнительные сведения о [настройке VPN-шлюза для передачи данных с помощью пиринга между виртуальными сетями](../vpn-gateway/vpn-gateway-peering-gateway-transit.md?toc=%2fazure%2fvirtual-network%2ftoc.json).
        
    > [!NOTE]
    > Если вы используете виртуальный сетевой шлюз для отправки на месте трафика транзитом в заглянул VNet, заглянул VNet IP диапазон для наместераровательного устройства VPN должны быть установлены на "интересный" трафик. В противном случае ваши ресурсы не смогут общаться с ресурсами в загляну vNet.
...
- **PowerShell**: [Адд-AzVirtualNetworkPeering](/powershell/module/az.network/add-azvirtualnetworkpeering)
...
5. Измените соответствующий параметр. Прочитайте о параметрах для каждого параметра в [шаге 5](#add-peering) Создать пиринг.
...
- **PowerShell**: [Get-AzVirtualNetworkPeering](/powershell/module/az.network/get-azvirtualnetworkpeering) для извлечения параметров вглядывания представления и [настройки-AzVirtualNetworkPeering](/powershell/module/az.network/set-azvirtualnetworkpeering) для изменения настроек.
...
- **PowerShell**: [Удалить-AzVirtualNetworkPeering](/powershell/module/az.network/remove-azvirtualnetworkpeering)

## <a name="requirements-and-constraints"></a>Требования и ограничения

- <a name="cross-region"></a>Пиринг можно создать между виртуальными сетями в одном или в разных регионах. Виртуальные сети в разных регионах также называют *Global VNet Peering.* 
- При создании глобального пиринга виртуальные сети с пирингом могут существовать в любом регионе общедоступного облака Azure или в облачных регионах Китая или в областях облаков правительства. Вы не можете заглянуть в облака. Например, VNet в общедоступном облаке Azure не может быть приглянен к VNet в облаке Azure China.
- Ресурсы в одной виртуальной сети не могут взаимодействовать с IP-адресом внешнего интерфейса внутренней базовой подсистемы балансировки нагрузки в глобальной пиринговой виртуальной сети. Поддержка базового балансировщика нагрузки существует только в пределах одного региона. Поддержка Load Balancer (цен. категория "Стандартный") существует как для пиринга виртуальной сети, так и пиринга глобальной виртуальной сети. Здесь описаны службы, которые используют балансируев основной нагрузки, который не будет работать над Global VNet [Peering.](virtual-networks-faq.md#what-are-the-constraints-related-to-global-vnet-peering-and-load-balancers)
- Вы можете использовать удаленные шлюзы или разрешить транзит шлюза в глобально заглянунных виртуальных сетях и локально заглянунных виртуальных сетях.
- Виртуальные сети могут быть в одной или разных подписках. Если вы устанавливаете пиринг виртуальных сетей в разных подписках, обе подписки могут быть связаны с одним и тем же или с разными клиентами Azure Active Directory. Если у вас еще нет арендатора AD, его можно [создать.](../active-directory/develop/quickstart-create-new-tenant.md?toc=%2fazure%2fvirtual-network%2ftoc.json-a-new-azure-ad-tenant) Поддержка пиринга между виртуальными сетями из подписок, связанных с разными клиентами Azure Active Directory, на портале недоступна. Вы можете использовать интерфейс командной строки, PowerShell или шаблоны.
...
- Если пиринговая связь устанавливается между двумя виртуальными сетями, созданными при помощи Resource Manager, ее нужно настроить для каждой из этих виртуальных сетей. Отображается состояние пиринга одного из следующих типов. 
  - *Инициировано:* При создании вглядывающейся во вторую виртуальную сеть из первой виртуальной сети *инициируется*статус пиринга. 
  - *Подключено:* При создании пиринга из второй виртуальной сети в первую виртуальную сеть, ее состояние пиринга *Connected*. Если просмотреть состояние пиринга для первой виртуальной сети, вы увидите, что ее статус изменен с *инициированного* на *Connected.* Пиринг не будет успешно установлен до тех пор, пока не *будет подключен*статус пиринга для обоих виртуальных сетевых пирингов.
...
- За входящий и исходящий трафик по пиринговой связи между виртуальными сетями взимается номинальная плата. Для получения дополнительной информации смотрите [страницу ценообразования](https://azure.microsoft.com/pricing/details/virtual-network).
...
- Узнайте, как создать [концентратор и говорить топологии сети](/azure/architecture/reference-architectures/hybrid-networking/hub-spoke?toc=%2fazure%2fvirtual-network%2ftoc.json)
