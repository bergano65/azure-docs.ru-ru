...
* **Ubuntu 14.04 в ядре linux в azure**
* **Ubuntu 16.04 или более поздней версии** 
* **SLES 12 SP3 или более поздней версии** 
* **RHEL 7.4 или более поздней версии**
* **CentOS 7.4 или более поздней версии**
* **CoreOS Linux**
* **Debian "Stretch" с бэкпортированным ядром**
* **Oracle Linux 7.4 и более поздние версии с Red Hat совместимый ядра (RHCK)**
* **Oracle Linux 7.5 и более поздние версии с UEK версии 5**
...
## <a name="handle-dynamic-binding-and-revocation-of-virtual-function"></a>Дескриптор динамическую привязку и отзыва виртуальных функций 
Приложения должны выполняться через искусственные сетевой Адаптер, который предоставляется в виртуальной Машине. Если приложение выполняется непосредственно на сетевой Адаптер VF, он не получает **все** пакеты, предназначенные для виртуальной Машины, так как некоторые пакеты отображаются через интерфейс искусственных.
При запуске приложения через синтетическую NIC, это гарантирует, что приложение получает **все** пакеты, предназначенные к нему. Она также обеспечивает, приложение продолжает выполняться, даже если VF отменяется в том случае, когда выполняется обслуживание узла. Привязка к синтетическую NIC приложений является **обязательные** требование для всех приложений, используя преимущества **ускоренной сети**.
