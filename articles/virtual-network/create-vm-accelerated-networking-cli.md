---
title: Создание Azure VM с ускоренной сетью с помощью Azure CLI
...
* **SLES12 SP3 или позже** 
* **RHEL 7.4 или более поздно**
* **CentOS 7.4 или более поздние**
* **CoreOS Linux**
* **Debian "Stretch" с бэкпортированным ядром**
* **Oracle Linux 7.4 и позже с Red Hat совместимым ядром (RHCK)**
* **Oracle Linux 7.5 и позже с версией UEK 5**
...
Если вы используете пользовательское изображение, и ваше изображение поддерживает ускоренную сеть, пожалуйста, убедитесь, что необходимые драйверы для работы с Mellanox ConnectX-3 и ConnectX-4 Lx NICs на Azure.
...
В этой статье описано, как создать виртуальную машину с ускорением работы в сети. Но с помощью Azure CLI вы также можете [создать аналогичную виртуальную машину на портале Azure](../virtual-machines/linux/quick-create-portal.md?toc=%2fazure%2fvirtual-network%2ftoc.json). При создании виртуальной машины на портале, в **создании виртуального лезвия машины,** выберите вкладку **Networking.**  В этой вкладке есть возможность для **ускорения сетей.**  Если вы выбрали [поддерживаемую операционную систему](#supported-operating-systems) и [размер VM,](#supported-vm-instances)эта опция автоматически заселится в "On".  Если нет, то он будет заполнить "Off" вариант для ускоренной сети и дать пользователю причину, почему она не будет включена.   
...
После создания виртуальной машины можно подтвердить, что ускоренная сеть включена, следуя инструкциям в [Confirm, что ускоренная сеть включена.](#confirm-that-accelerated-networking-is-enabled)

## <a name="cli-creation"></a>Создание CLI
...
Создайте виртуальную сеть с [az сети Vnet создать](/cli/azure/network/vnet). В следующем примере создаются виртуальная сеть *myVnet* и одна подсеть.
...
Создайте VM с [az vm создать.](/cli/azure/vm) В следующем примере создается виртуальная машина *myVM* с образом UbuntuLTS и размером, поддерживающим ускоренную сеть (*Standard_DS4_v2*):
...
## <a name="handle-dynamic-binding-and-revocation-of-virtual-function"></a>Обработка динамического связывания и отзыва виртуальной функции 
Приложения должны работать над синтетическим NIC, который подвергается в VM. Если приложение работает непосредственно над VF NIC, оно не получает **все** пакеты, предназначенные для VM, так как некоторые пакеты отображаются над синтетическим интерфейсом.
Если вы запустите приложение над синтетическим NIC, это гарантирует, что приложение получит **все** пакеты, которые предназначены для него. Он также гарантирует, что приложение продолжает работать, даже если VF отменяется при обслуживании хоста. Приложения, привязывающие к синтетическому NIC, являются **обязательным** требованием для всех приложений, пользующихся **преимуществами ускоренной сети.**
