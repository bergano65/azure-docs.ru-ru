---
title: Создание пиринга виртуальных сетей с использованием разных подписок
titlesuffix: Azure Virtual Network
description: Узнайте, как создать пиринг между виртуальными сетями, созданными с помощью Resource Manager в разных подписках Azure, которые размещены в одном или в разных арендаторах Azure Active Directory.
...
В этом руководстве вы узнаете, как установить пиринг между виртуальными сетями, развернутыми с помощью Resource Manager. Виртуальные сети существуют в разных подписках, которые могут принадлежать разным арендаторам Azure Active Directory (Azure AD). Установление пиринга между двумя виртуальными сетями позволяет ресурсам в разных виртуальных сетях взаимодействовать друг с другом с такими же пропускной способностью и задержкой, как если бы эти ресурсы находились в одной виртуальной сети. Узнайте больше о [пиринге виртуальных сетей](virtual-network-peering-overview.md).
...
10. В вертикальном списке вариантов слева в колонке **myVnetA — управление доступом (IAM)** щелкните **Свойства**. Скопируйте **Идентификатор ресурса**, он понадобится нам позже. ИД ресурса будет выглядеть примерно так: `/subscriptions/<Subscription Id>/resourceGroups/myResourceGroupA/providers/Microsoft.Network/virtualNetworks/myVnetA`.
...
14. В вертикальном списке вариантов слева в колонке **myVnetB** щелкните **Свойства**. Скопируйте **Идентификатор ресурса**, он понадобится нам позже. ИД ресурса будет выглядеть примерно так: `/subscriptions/<Subscription ID>/resourceGroups/myResourceGroupB/providers/Microsoft.ClassicNetwork/virtualNetworks/myVnetB`.
...
21. В колонке **Добавить пиринг** введите или выберите следующие значения, а затем нажмите кнопку **ОК**:
     ...
     - **Модель развертывания виртуальной сети**:  Выберите **Диспетчер ресурсов**.
...
28. **Необязательно**: хотя в этом руководстве не рассматривается создание виртуальных машин, можно создать по виртуальной машине в каждой виртуальной сети и подключить эти виртуальные машины друг к другу, чтобы проверить связь.
29. **Необязательно**: чтобы удалить ресурсы, созданные при работе с этим руководством, выполните инструкции, описанные в разделе [Удаление ресурсов](#delete-portal) этой статьи.
...
11. **Необязательно**: хотя в этом руководстве не рассматривается создание виртуальных машин, можно создать по виртуальной машине в каждой виртуальной сети и подключить эти виртуальные машины друг к другу, чтобы проверить связь.
...
13. **Необязательно**: хотя в этом руководстве не рассматривается создание виртуальных машин, можно создать по виртуальной машине в каждой виртуальной сети и подключить эти виртуальные машины друг к другу, чтобы проверить связь.
14. **Необязательно**: чтобы удалить ресурсы, созданные при работе с этим руководством, выполните инструкции, описанные в разделе [Удаление ресурсов](#delete-powershell) этой статьи.

## <a name="create-peering---resource-manager-template"></a><a name="template"></a>Создание пиринга с помощью шаблона Resource Manager
...
3. Войдите в Azure в качестве пользователя A и разверните шаблон с помощью [портала](../azure-resource-manager/templates/deploy-portal.md?toc=%2fazure%2fvirtual-network%2ftoc.json#deploy-resources-from-custom-template), [PowerShell](../azure-resource-manager/templates/deploy-powershell.md?toc=%2fazure%2fvirtual-network%2ftoc.json#deploy-local-template) или [Azure CLI](../azure-resource-manager/templates/deploy-cli.md?toc=%2fazure%2fvirtual-network%2ftoc.json#deploy-local-template). Укажите имя файла, в котором вы сохранили текст JSON на шаге 2.
