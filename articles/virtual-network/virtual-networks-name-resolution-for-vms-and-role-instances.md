...
* Если виртуальная сеть связана с [Azure DNS частными зонами](../dns/private-dns-overview.md) в качестве виртуальной сети регистрации, обратные запросы DNS будут возвращать две записи. Одна запись будет иметь форму \[vmname.\] [приатеднсзоненаме] и другие могут иметь форму \[vmname\]. internal.cloudapp.NET
