...
> [!NOTE]
> В зависимости от сценария может потребоваться использовать Azure DNS частные зоны. Дополнительные сведения см. в статье [Использование Azure DNS для частных доменов](../dns/private-dns-overview.md).
>
...
| Обратный поиск DNS для внутренних IP-адресов. |[Разрешение имен с помощью собственного DNS-сервера](#name-resolution-that-uses-your-own-dns-server). |Неприменимо |
