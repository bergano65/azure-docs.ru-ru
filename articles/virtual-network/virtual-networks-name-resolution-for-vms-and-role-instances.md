...
* Если виртуальная сеть связана с [Azure DNS частными зонами](../dns/private-dns-overview.md) в качестве виртуальной сети регистрации, обратные запросы DNS будут возвращать две записи. Одна запись будет иметь форму \[ vmname \] . [ приватеднсзоненаме] и другие могут иметь форму \[ vmname \] . internal.cloudapp.NET
...
* В классических моделях развертывания суффикс можно определить, вызвав [API получения развертывания](https://msdn.microsoft.com/library/azure/ee460804.aspx) или выполнив командлет [Get-AzureVM -Debug](/powershell/module/servicemanagement/azure.service/get-azurevm).
