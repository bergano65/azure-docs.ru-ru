...
| --- | --- | --- |
| Разрешение имен между виртуальными машинами, расположенными в одной виртуальной сети, или между экземплярами роли облачных служб, расположенными в одной облачной службе. | [Частные зоны Azure DNS](../dns/private-dns-overview.md) или [разрешение имени, предоставленное Azure](#azure-provided-name-resolution) |Имя узла или полное доменное имя |
| Разрешение имен между виртуальными машинами, расположенными в разных виртуальных сетях, или экземплярами ролей, расположенными в разных облачных службах. |[Частные зоны Azure DNS](../dns/private-dns-overview.md) или, управляемые клиентами DNS-серверы, перенаправляют запросы между виртуальными сетями для разрешения Azure (прокси DNS). Смотрите [разрешение имен с помощью собственного DNS-сервера.](#name-resolution-that-uses-your-own-dns-server) |только полное доменное имя |
...
| Разрешение имен локального компьютера и службы из экземпляров роли или виртуальных машин в Azure. |Управляемые клиентами DNS-серверы (например, локальный контроллер домена, локальный контроллер домена только для чтения или DNS-сервер, вторично синхронизированный с помощью передач зоны). Смотрите [разрешение имен с помощью собственного DNS-сервера.](#name-resolution-that-uses-your-own-dns-server) |только полное доменное имя |
...
| Обратный поиск DNS для внутренних IP-адресов. |[Частные зоны Azure DNS](../dns/private-dns-overview.md) или [разрешение имени, предоставленное Azure,](#azure-provided-name-resolution) или [разрешение имен с помощью собственного DNS-сервера.](#name-resolution-that-uses-your-own-dns-server) |Неприменимо |
...
Разрешение имени, предоставленное Azure, обеспечивает только основные авторитетные возможности DNS. При использовании этой опции имена и записи зоны DNS будут автоматически управляться Azure, и вы не сможете контролировать имена зон DNS или жизненный цикл записей DNS. Если вам нужно полнофункциональный DNS-решение для виртуальных сетей, вы должны использовать [частные зоны Azure DNS](../dns/private-dns-overview.md) или [управляемые клиентами DNS-серверы.](#name-resolution-that-uses-your-own-dns-server)

Вместе с разрешением общедоступных DNS-имен Azure предоставляет разрешение внутренних имен для виртуальных машин и экземпляров ролей, которые находятся в той же виртуальной сети или облачной службе. Виртуальные машины и экземпляры в облачной службе используют одинаковый DNS-суффикс (поэтому имени узла достаточно). Но в виртуальных сетях, использующих классическую модель развертывания, разные облачные службы используют разные DNS-суффиксы. В этом случае для разрешения имен между разными облачными службами требуется полное доменное имя. В виртуальных сетях, развернутых с использованием модели развертывания Azure Resource Manager, суффикс DNS соответствует всем виртуальным машинам в виртуальной сети, поэтому F-DN не нужен. DNS-имена могут назначаться виртуальным машинам и сетевым интерфейсам. Несмотря на то, что разрешение имен Azure не требует настройки, как указано в предыдущей таблице, это не самый лучший выбор для всех сценариев развертывания.
...
* Если вы хотите отключить обратную функцию DNS в виртуальной сети, вы можете сделать это, создав обратную зону поиска, используя [частные зоны Azure DNS](../dns/private-dns-overview.md) и свяжет эту зону с виртуальной сетью. Например, если пространство IP-адреса вашей виртуальной сети составляет 10.20.0.0/16, то можно создать пустую частную зону DNS 20.10.in-addr.arpa и связать ее с виртуальной сетью. При привязке зоны к виртуальной сети следует отключить авторегистрацию по ссылке. Эта зона перекроет зоны обратного поиска по умолчанию для виртуальной сети, и так как эта зона пуста, вы получите NXDOMAIN для обратных DNS-запросов. Подробнее о том, как создать частную DNS-зону и связать ее с виртуальной сетью, читайте в нашем руководстве по [быстрому](https://docs.microsoft.com/azure/dns/private-dns-getstarted-portal) запуску.
...
* Настроите исходный DNS-сервер в настройках исходной виртуальной сети.
