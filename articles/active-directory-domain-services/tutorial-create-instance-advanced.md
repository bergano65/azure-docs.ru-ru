...
При работе с Azure AD DS вы не получаете разрешения *администратора домена* или *администратора предприятия* для управляемого домена. Эти разрешения зарезервированы службой и не предоставляются для пользователей в клиенте. Вместо этого для некоторых операций с повышенным уровнем доступа можно применять группу *Администраторы AAD DC*. Сюда относятся вхождение в группу администраторов на виртуальных машинах, присоединенных к домену, и настройка групповой политики.
