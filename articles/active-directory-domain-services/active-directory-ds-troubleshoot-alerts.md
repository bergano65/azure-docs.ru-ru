...
| AADDS108 | *Ресурс, используемый для управляемого домена, удален. Чтобы доменные службы Azure AD продолжали правильно работать, требуется этот ресурс.* | [Ресурс удален](#aadds108-resources-for-your-managed-domain-cannot-be-found) |
| AADDS109 | *Подсеть, выбранная для развертывания доменных служб Azure AD, переполнена и не обладает достаточным местом для контроллера домена, который необходимо создать.* | [Подсеть переполнена](#aadds109-the-subnet-associated-with-your-managed-domain-is-full) |
| AADDS110 | *Определено, что подсеть виртуальной сети этого домена обладает недостаточным количеством IP-адресов. Для доменных служб Azure AD требуется не менее двух доступных IP-адресов, включенных в подсеть. Рекомендуется наличие не менее 3–5 свободных IP-адресов в подсети. Такая ситуация может возникать, если в подсети развернуты другие виртуальные машины, использующие доступные IP-адреса, или ограничено количество доступных IP-адресов в подсети.* | [Недостаточное количество IP-адресов](#aadds110-not-enough-ip-address-in-the-managed-domain) |
| AADDS111 | *Один или несколько сетевых ресурсов, используемых в управляемом домене, не могут быть использованы, так как целевая область была заблокирована.* | [Ресурсы заблокированы](#aadds111-resources-are-locked) |
| AADDS112 | *Один или несколько сетевых ресурсов, используемых в управляемом домене, не могут быть использованы из-за ограничений политики.* | [Ресурсы недоступны для использования](#aadds112-resources-are-unusable) |
...
| AADDS114 | * Контроллерам домена доменных служб Azure AD не удается получить доступ к порту 443. С помощью данного порта можно обновить управляемый домен и управлять им. * | [Порт 443 заблокирован](#aadds114-port-443-blocked) |
...
## <a name="aadds108-resources-for-your-managed-domain-cannot-be-found"></a>AADDS108. Ресурсы для управляемого домена не найдены
...
1.  Откройте страницу работоспособности доменных служб Azure AD.
  1.    Перейдите на [страницу доменных служб Azure AD]() на портале Azure.
...
2.  Проверьте предупреждения, чтобы убедиться, что оно появилось менее четырех часов назад.
  1.    На странице работоспособности щелкните предупреждение с идентификатором **AADDS108**.
...
## <a name="aadds109-the-subnet-associated-with-your-managed-domain-is-full"></a>AADDS109. Подсеть, связанная с управляемым доменом, переполнена
...
## <a name="aadds110-not-enough-ip-address-in-the-managed-domain"></a>AADDS110. Недостаточно IP-адресов в управляемом домене
...
## <a name="aadds111-resources-are-locked"></a>AADDS111. Ресурсы заблокированы
...
## <a name="aadds112-resources-are-unusable"></a>AADDS112. Ресурсы недоступны для использования
...
## <a name="aadds114-port-443-blocked"></a>AADDS114. Порт 443 заблокирован

**Оповещение.**

*Контроллерам домена доменных служб Azure AD не удается получить доступ к порту 443. С помощью данного порта можно обновить управляемый домен и управлять им.*

**Способы устранения:**

Разрешите доступ входящего трафика для доменных служб Azure AD через порт 443 группы безопасности сети.
