...
| notificationSettings    | Если в управляемом домене поддерживается генерация оповещений, можно отправлять уведомления по электронной почте. <br />Для этих уведомлений можно *включить* *глобальных администраторов* клиента Azure и членов группы *Администраторы контроллера домена AAD*.<br /> Если это необходимо, вы можете добавить дополнительных получателей уведомлений при наличии важных предупреждений.|
