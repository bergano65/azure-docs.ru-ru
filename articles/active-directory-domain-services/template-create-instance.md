...
* **Ограничения префикса домена:** Нельзя создать управляемый домен с префиксом длиннее 15 символов. Длина указанного префикса доменного имени (например, *contoso* в доменном имени *contoso.com*) не должна превышать 15 символов.
...
В приведенном ниже определении уплотненных параметров показано, как объявляются эти значения. Будет создан лес пользователей с именем *aadds.contoso.com* , в котором все пользователи из Azure AD синхронизированы с управляемым доменом Azure AD DS.
