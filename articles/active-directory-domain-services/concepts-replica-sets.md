...
При создании управляемого домена Azure Active Directory доменных служб (Azure AD DS) определяется уникальное пространство имен. Это пространство имен — это доменное имя, например *aaddscontoso.com*, а два контроллера домена (DCS) развертываются в выбранном регионе Azure. Это развертывание контроллеров домена называется набором реплик.

Вы можете развернуть управляемый домен для нескольких наборов реплик на клиент Azure AD. Наборы реплик можно добавить в любую одноранговую виртуальную сеть в любом регионе Azure, поддерживающем AD DS Azure. Дополнительные наборы реплик в разных регионах Azure предоставляют географическое аварийное восстановление для устаревших приложений, если регион Azure переходит в автономный режим.

Сейчас наборы реплик доступны в предварительной версии.
...
При создании управляемого домена, например *aaddscontoso.com*, создается первоначальный набор реплик. Дополнительные наборы реплик используют одно и то же пространство имен и конфигурацию. Изменения AD DS Azure, включая конфигурацию, удостоверение пользователя и учетные данные, группы, объекты групповой политики, объекты компьютеров и другие изменения, применяются ко всем наборам реплик в управляемом домене с помощью репликации AD DS.
...
Наборы реплик являются общедоступной функцией предварительной версии в доменных службах Azure AD. Вы можете использовать рабочий управляемый домен, но помните о различиях в поддержке, которые существуют для компонентов, которые еще находятся в предварительной версии. Дополнительные сведения о [предварительных выAzure Active Directoryах соглашения об уровне обслуживания](https://azure.microsoft.com/support/legal/preview-supplemental-terms/).
