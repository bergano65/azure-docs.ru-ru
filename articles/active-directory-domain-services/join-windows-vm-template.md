...
1. На странице **Настраиваемое развертывание** введите следующие сведения, чтобы создать виртуальную машину Windows Server и присоединить ее к управляемому домену Azure AD DS.
    ...
    | Имя пользователя домена           | Учетная запись пользователя в управляемом домене Azure AD DS, которую следует использовать для приподключения виртуальной машины к управляемому домену, например `contosoadmin@aadds.contoso.com`. Эта учетная запись должна быть членом группы *администраторов контроллера домена Azure AD* . |
...
1. На странице **Настраиваемое развертывание** введите следующие сведения, чтобы ПРИсоединить виртуальную машину к управляемому домену Azure AD DS.
    ...
    | Имя пользователя присоединение к домену     | Учетная запись пользователя в управляемом домене Azure AD DS, которую следует использовать для приподключения виртуальной машины к управляемому домену, например `contosoadmin@aadds.contoso.com`. Эта учетная запись должна быть членом группы *администраторов контроллера домена Azure AD* . |
