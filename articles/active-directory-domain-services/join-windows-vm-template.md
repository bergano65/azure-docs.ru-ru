...
1. На странице **Настраиваемое развертывание** введите следующие сведения, чтобы создать виртуальную машину Windows Server и присоединить ее к управляемому домену Azure AD DS.
    ...
    | Домен для присоединение            | DNS-имя управляемого домена AD DS Azure, например *aaddscontoso.com*. |
    | Имя пользователя домена           | Учетная запись пользователя в управляемом домене Azure AD DS, которую следует использовать для приподключения виртуальной машины к управляемому домену, например `contosoadmin@aaddscontoso.com`. Эта учетная запись должна быть частью управляемого домена AD DS Azure. |
...
> Файл параметров шаблона запрашивает пароль для учетной записи пользователя, которая является частью управляемого домена AD DS Azure. Не вводите значения вручную в этот файл и оставьте его доступным в общих папках или в других общих расположениях.
...
1. На странице **Настраиваемое развертывание** введите следующие сведения, чтобы ПРИсоединить виртуальную машину к управляемому домену Azure AD DS.
    ...
    | Имя пользователя присоединение к домену     | Учетная запись пользователя в управляемом домене Azure AD DS, которую следует использовать для приподключения виртуальной машины к управляемому домену, например `contosoadmin@aaddscontoso.com`. Эта учетная запись должна быть частью управляемого домена AD DS Azure. |
