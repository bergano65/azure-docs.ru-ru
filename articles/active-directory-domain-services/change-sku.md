...
После развертывания управляемого домена можно изменить номера SKU. Однако если вы используете лес ресурсов (сейчас находится в предварительной версии) и создали односторонние отношения доверия исходящего трафика из Azure AD DS в локальную среду AD DS, существуют некоторые ограничения для операции изменения номера SKU. SKU " *премиум* " и " *Корпоративный* " определяют ограничение на число создаваемых доверий. Вы не можете перейти на номер SKU с более низким пределом, чем настроено в настоящее время.
