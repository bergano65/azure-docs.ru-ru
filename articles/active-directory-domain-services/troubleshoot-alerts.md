...
| AADDS502 | *Срок действия защищенного сертификата LDAP для управляемого домена истекает [дата].* | [Истечение срока действия безопасного сертификата LDAP](alert-ldaps.md#aadds502-secure-ldap-certificate-expiring) |
...
Эта ошибка является неисправимой. Чтобы устранить ее, необходимо [удалить имеющийся управляемый домен](delete-aadds.md) и повторно создать его в новом каталоге. Если у вас возникли трудности с удалением, свяжитесь с командой разработки продукта, отвечающей за доменные службы Azure Active Directory [для поддержки](contact-us.md).
...
1. [Удалите свой управляемый домен](delete-aadds.md) из имеющегося каталога.
...
> [!WARNING]
> Если ваш управляемый домен блокируется в течение продолжительного времени, существует риск того, что он будет удален. Рекомендуем снять блокировку как можно скорее. Дополнительные сведения см. в [этой статье](suspension.md).
