...
ФГПП влияет только на пользователей, созданных непосредственно в Azure AD DS. Параметры сложности пароля не затрагивают облачных пользователей и пользователей домена, синхронизированных с управляемым доменом AD DS Azure из Azure AD. ФГПП распространяется с помощью сопоставления групп в управляемом домене Azure AD DS, а все вносимые изменения применяются при следующем входе пользователя в систему. Изменение политики не разблокирует уже заблокированную учетную запись пользователя.
...
1. Войдите на виртуальную машину Windows, которая используется для администрирования управляемого домена (должен быть как минимум Windows Server 2012 R2). Если у вас ее нет, следуйте инструкциям по [управлению доменом доменных служб Azure AD](manage-domain.md).
...
9. В разделе **Directly Applies To** (Применяется напрямую к) нажмите кнопку **Добавить**. В диалоговом окне **Select Users or Groups** (Выбор пользователей или групп) нажмите кнопку **Расположения**.
   ...
   ![Выбрать пользователей и группы](./media/how-to/fgpp-applies-to.png)
