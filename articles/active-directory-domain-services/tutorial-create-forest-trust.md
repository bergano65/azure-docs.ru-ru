...
* Управляемый домен Azure AD DS, созданный с использованием леса ресурсов и настроенный в клиенте Azure AD.
    * Если потребуется, [создайте и настройте экземпляр доменных служб Azure Active Directory][create-azure-ad-ds-instance-advanced].
    ...
    > [!IMPORTANT]
    > Убедитесь, что вы создали управляемый домен Azure AD DS с использованием леса *ресурсов*. Параметр по умолчанию создает лес *пользователя*. Только леса ресурсов могут создавать отношения доверия к локальным средам AD DS. Для управляемого домена также необходимо использовать как минимум SKU уровня *Корпоративный*. При необходимости [измените номер SKU для управляемого домена Azure AD DS][howto-change-sku].
...
Чтобы правильно разрешить управляемый домен Azure AD DS из локальной среды, может потребоваться добавить серверы пересылки на существующие DNS-серверы. Если в локальной среде не настроено взаимодействие с управляемым доменом Azure AD DS, выполните следующие действия на рабочей станции управления с локальным доменом AD DS:
...
1. Добавьте IP-адреса управляемого домена Azure AD DS, например *10.0.2.4* и *10.0.2.5*.

## <a name="create-inbound-forest-trust-in-the-on-premises-domain"></a>Создание входящего доверия леса в локальном домене

Для локального домена AD DS требуется входящее доверие леса для управляемого домена Azure AD DS. Это доверие необходимо настроить вручную в локальном домене AD DS. Его нельзя создать на портале Azure.
...
1. Выберите параметр для создания доверия для **This domain only** (Только для этого домена). На следующем шаге вы создадите доверие на портале Azure для управляемого домена Azure AD DS.
...
Теперь, когда в локальном домене AD DS настроено разрешение управляемого домена Azure AD DS и создано входящее доверие леса, создайте исходящее доверие леса. Это последний шаг в создании отношения доверия между локальным доменом AD DS и управляемым доменом Azure AD DS.

Чтобы создать исходящее доверие для управляемого домена Azure AD DS на портале Azure, выполните действия, указанные ниже.
...
1. В меню в левой части управляемого домена Azure AD DS выберите **Отношения доверия**, а затем нажмите кнопку **+ Добавить**, чтобы добавить доверие.

   > [!NOTE]
   > Если пункт меню **Отношения доверия** не отображается, найдите в разделе **Свойства** элемент *Тип леса*. Создавать доверия можно только для *лесов ресурсов*. Если тип леса — *Пользователь*, создание доверий невозможно. Сейчас нет возможности изменять тип леса управляемого домена Azure AD DS. Необходимо удалить и повторно создать управляемый домен в качестве леса ресурсов.
