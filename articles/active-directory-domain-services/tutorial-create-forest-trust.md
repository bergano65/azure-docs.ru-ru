...
1. Выберите вкладку **доверия** , затем **новое отношение доверия** .
   ...
   > [!NOTE]
   > Если пункт меню " **доверия** " не отображается, установите флажок в разделе " **Свойства** " для *типа леса*. Только леса *ресурсов* могут создавать отношения доверия. Если тип леса — *User*, то нельзя создавать отношения доверия. Сейчас нет способа изменить тип леса управляемого домена AD DS Azure. Необходимо удалить и повторно создать управляемый домен в качестве леса ресурсов.

1. Введите имя в доменном имени Azure AD DS, например *aaddscontoso.com*, а затем нажмите кнопку **Далее** .
...
1. В портал Azure найдите и выберите **доменные службы Azure AD**, а затем выберите управляемый домен, например *aaddscontoso.com* .
...
1. С помощью **проводника Windows**подключитесь к созданной общей папке, используя полное имя узла и общую папку, например `\\fs1.aaddscontoso.com\CrossforestShare`.
