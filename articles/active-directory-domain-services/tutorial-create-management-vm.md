...
* Узел бастиона Azure, развернутый в виртуальной сети Azure AD DS.
    * При необходимости [создайте узел бастиона для Azure][azure-bastion].
...
1. В области **Обзор** виртуальной машины выберите **подключить**, а затем **бастиона**.

    ![Подключение к виртуальной машине Windows с помощью бастиона в портал Azure](./media/join-windows-vm/connect-to-vm.png)

1. Введите учетные данные для виртуальной машины, а затем нажмите кнопку **подключить**.

   ![Подключитесь через узел бастиона в портал Azure](./media/join-windows-vm/connect-to-bastion.png)
...
1. Чтобы просмотреть управляемый домен AD DS Azure, выберите имя домена в левой области, например *aaddscontoso.com*. Первыми в списке указаны два контейнера с именами *Компьютеры AADDC* и *Пользователи AADDC*.
