...
* Убедитесь, что [включена синхронизация паролей](tutorial-create-instance.md#enable-user-accounts-for-azure-ad-ds) в соответствии с шагами, описанными на странице «Приступая к работе».
...
* **Синхронизированные учетные записи**. Если учетные записи пользователей синхронизируются из локального каталога, убедитесь в следующем.
  ...
  * Вы включили в Azure AD Connect [выполнение полной синхронизации](tutorial-create-instance.md#enable-user-accounts-for-azure-ad-ds).
...
* **Убедитесь, что учетная запись пользователя активна**: Если учетная запись пользователя заблокирована, она не сможет выполнить вход, пока ее учетная запись не будет активна снова. При пяти недействительных попытках ввода пароля в течение 2 минут в управляемом домене происходит блокировка учетной записи пользователя на 30 минут. Через 30 минут учетная запись пользователя будет автоматически разблокирована.
  * Недопустимые попытки ввода пароля в управляемом домене не блокируют учетную запись пользователя в Azure AD. Учетная запись пользователя заблокирована только в управляемом домене доменных служб Azure AD. Проверьте состояние учетной записи пользователя с помощью консоли администрирования Active Directory (ADAC) для управляемого домена AD DS Azure, а не в Azure AD.
  * Можно также [настроить подробные политики паролей, которые изменяют порог и длительность блокировки по умолчанию](https://docs.microsoft.com/azure/active-directory-domain-services/password-policy).
