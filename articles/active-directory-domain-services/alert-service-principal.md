...
[Субъекты-службы](../active-directory/develop/app-objects-and-service-principals.md) — это приложения, которые платформа Azure использует для обновления и обслуживания управляемого домена Azure AD DS, а также для управления им. При удалении субъекта-службы изменяется функциональность управляемого домена Azure AD DS.
...
При удалении обязательного субъекта-службы платформа Azure не сможет выполнять автоматизированные задачи управления. Возможно, управляемый домен Azure AD DS будет неправильно применять обновления или создавать резервные копии.

### <a name="check-for-missing-service-principals"></a>Проверка отсутствующих субъектов-служб

Чтобы узнать, какие субъекты-службы отсутствуют и требуют повторного создания, выполните следующие действия:
...
1. Выполните поиск по идентификатору приложения. Если существующее приложение не найдено, выполните инструкции из раздела *Решение*, чтобы создать субъект-службу или повторно зарегистрировать пространство имен.
...
Если в каталоге Azure AD отсутствует приложение с идентификатором *2565bd9d-da50-47d4-8b85-4c97f669dc36*, выполните следующие действия в Azure AD PowerShell. Дополнительные сведения см. в статье об [установке модуля Azure AD PowerShell](/powershell/azure/active-directory/install-adv2).

1. Установите модуль Azure AD PowerShell и импортируйте его, выполнив следующее:
...
Сведения о работоспособности управляемого домена Azure AD DS автоматически обновятся в течение двух часов, и оповещение исчезнет.
...
1. Выберите подписку, связанную с управляемым доменом Azure AD DS.
...
После удаления обоих приложений платформа Azure автоматически восстановит их и попытается возобновить синхронизацию паролей. Сведения о работоспособности управляемого домена Azure AD DS автоматически обновятся в течение двух часов, и оповещение исчезнет.
