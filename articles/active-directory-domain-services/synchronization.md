...
* **Структуры подразделений**. Подразделения, определенные в локальном домене, не синхронизируются с управляемым доменом. В управляемом домене есть два встроенных подразделения. По умолчанию структура подразделений в управляемом домене одноуровневая. Однако вы можете [создавать в управляемом домене пользовательские подразделения](create-ou.md).
