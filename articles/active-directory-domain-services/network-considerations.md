...
[Группа безопасности сети (NSG)](../virtual-network/virtual-networks-nsg.md) содержит список правил, которые разрешают или запрещают передачу сетевого трафика в виртуальную сеть Azure. Группа безопасности сети создается при развертывании управляемого домена, содержащего набор правил, которые позволяют службе использовать функции проверки подлинности и управления. Эта группа безопасности сети по умолчанию связана с подсетью виртуальной сети, в которой развернут управляемый домен.
...
| Номер порта | Протокол | Источник                             | Назначение | Действие | Обязательное значение | Цель |
|:-----------:|:--------:|:----------------------------------:|:-----------:|:------:|:--------:|:--------|
