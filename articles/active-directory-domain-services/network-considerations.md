...
* Виртуальная сеть не может полагаться на службы DNS, кроме тех, которые предоставляются AD DS Azure.
    ...
    * Нельзя использовать настраиваемые параметры DNS-сервера для направления запросов к другим DNS-серверам, включая виртуальные машины. Ресурсы в виртуальной сети должны использовать службу DNS, предоставляемую AD DS Azure.
...
* Для AD DS Azure требуется от пяти до семи IP-адресов. Убедитесь, что диапазон IP-адресов подсети может предоставлять такое количество адресов.
...
| Номер порта | Протокол | Source                             | Назначение | Action | Обязательное значение | Цель |
|:-----------:|:--------:|:----------------------------------:|:-----------:|:------:|:--------:|:--------|
| 443         | TCP      | азуреактиведиректоридомаинсервицес | Any         | РАЗРЕШИТЬ  | Да      | Синхронизация с клиентом Azure AD. |
| 3389        | TCP      | корпнетсав                         | Any         | РАЗРЕШИТЬ  | Да      | Управление доменом. |
| 5986        | TCP      | азуреактиведиректоридомаинсервицес | Any         | РАЗРЕШИТЬ  | Да      | Управление доменом. |
| 636         | TCP      | Any                                | Any         | РАЗРЕШИТЬ  | Нет       | Включается только при настройке защищенного протокола LDAP (LDAPs). |
