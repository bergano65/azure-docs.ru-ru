...
В этой статье показано, как настроить ограниченное делегирование Kerberos для Resource-басд в управляемом домене Azure AD DS.
...
Ограниченное делегирование Kerberos (KCD) ограничивает службы или ресурсы, к которым может подключаться указанный сервер или приложение при олицетворении другого удостоверения. Традиционные KCD требуются права администратора домена для настройки учетной записи домена для службы и ограничивают учетную запись для запуска в одном домене. В традиционных KCD также есть несколько проблем. Например, в более ранних операционных системах администратор служб не имел удобного способа выяснить, какие из интерфейсных служб делегируются службам ресурсов, которыми они владеют. Любая интерфейсная служба, которая может делегировать службу ресурсов, является потенциальной точкой атаки. Если сервер, на котором размещена интерфейсная служба, настроенная для делегирования служб ресурсов, был скомпрометирован, то службы ресурсов также могут быть скомпрометированы.
