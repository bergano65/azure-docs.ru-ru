...
ms.locfileid: "79082365"
---
# <a name="tutorial-create-and-configure-an-azure-active-directory-domain-services-instance"></a>Учебник. Создание и настройка Azure Active Directory экземпляра доменных служб
...
1. Выберите группу ресурсов, например *myResourceGroup*, а затем выберите экземпляр AD DS Azure в списке ресурсов Azure, например *aaddscontoso.com*. На вкладке **Обзор** можно увидеть, что сейчас выполняется *развертывание* управляемого домена. Настроить управляемый домен можно только после его полной подготовки.
...
1. На вкладке **Обзор** для управляемого домена отображаются некоторые **требуемые шаги конфигурации**. Первый из них — обновление параметров DNS-сервера для виртуальной сети. После правильной настройки параметров DNS этот шаг исчезает из списка.
    ...
    Указанные здесь адреса принадлежат контроллерам домена для использования в виртуальной сети. В этом примере эти адреса являются *10.0.1.4* и *10.0.1.5*. Эти IP-адреса вы можете найти на вкладке **Свойства**.
