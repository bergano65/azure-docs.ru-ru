---
title: Масштабируемость - Концентраторы событий Azure Документы Майкрософт
description: В этой статье приводится информация о том, как масштабировать концентраторы событий Azure с помощью разделов и блоков пропускной связи.
services: event-hubs
...
Пропускная способность концентраторов событий контролируется *блоками пропускной способности.* Единицы пропускной способности — это предварительно приобретенные единицы емкости. Единая пропускная часть позволяет:
...
Для получения дополнительной информации о функции автоматического раздувания [см.](event-hubs-auto-inflate.md)
