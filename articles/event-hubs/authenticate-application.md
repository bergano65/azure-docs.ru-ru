...
После регистрации приложения субъект-служба приложения назначается роли Azure AD концентраторов событий, описанной в разделе [сборки — роли для концентраторов событий Azure](#built-in-roles-for-azure-event-hubs) . 
...
1. На странице **Добавление назначения ролей** выполните следующие действия.
    ...
    1. Найдите **участника безопасности** (пользователя, группы, субъекта-службы), которому нужно назначить роль. Выберите **зарегистрированное приложение** из списка. 
    ...
    4. Перейдите на вкладку **назначения ролей** и Подтвердите назначение ролей. Например, на следующем рисунке показано, что **myWebApp** находится в роли **отправителя данных концентраторов событий Azure** . 
        
        ![Пользователь в списке](./media/authenticate-application/user-in-list.png)

Вы можете выполнить аналогичные действия, чтобы назначить роль для пространства имен концентраторов событий, группы ресурсов или подписки. Определив роль и ее область, вы можете проверить это поведение с помощью примеров [в этом расположении GitHub](https://github.com/Azure/azure-event-hubs/tree/master/samples/DotNet/Microsoft.Azure.EventHubs/Rbac). Дополнительные сведения об управлении доступом к ресурсам Azure с помощью Azure RBAC и портал Azure см. в [этой статье](..//role-based-access-control/role-assignments-portal.md). 
...
- Чтобы узнать, как назначать назначения ролей Azure и управлять ими с помощью Azure PowerShell, Azure CLI или REST API, см. следующие статьи:
    ...
    - [Добавление назначений ролей Azure с помощью шаблона Azure Resource Manager](../role-based-access-control/role-assignments-template.md)
