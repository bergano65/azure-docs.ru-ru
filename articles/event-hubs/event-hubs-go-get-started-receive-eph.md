...
В этом кратком руководстве вы создали приложение Go, которое получает сообщения из концентратора событий. Чтобы узнать, как отправлять события в концентратор событий с помощью Go, изучите статью [Отправка событий в Центры событий Azure с помощью Go](event-hubs-go-get-started-send.md).
