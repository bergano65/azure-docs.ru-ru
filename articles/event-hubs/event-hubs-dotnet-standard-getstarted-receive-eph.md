...
2. На портале в области навигации слева выберите **Создать ресурс** и **Хранилище** в категориях, а затем щелкните **Учетная запись хранения — BLOB-объект, файл, таблица, очередь**.  
3. Заполните поля в окне **Создание учетной записи хранения** и выберите **Просмотреть и создать**. 

    ![Создать учетную запись хранения][1]

4. На странице **Просмотр и создание** проверьте значения в полях и выберите **Создать**. 
5. Получив сообщение об **успешном выполнении развертывания**, выберите имя новой учетной записи хранения. 
6. В окне **Основные компоненты** выберите **BLOB-объекты**. 
7. Выберите **+Контейнер** вверху. Укажите имя контейнера.  
8. Выберите **Ключи доступа** в окне слева и скопируйте имя контейнера хранения, а также значение **key1**. 

    Сохраните на время эти значения в Блокноте или любом другом месте.
