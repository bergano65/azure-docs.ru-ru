...
1. Нагрузка неравномерно распределяется между всеми секциями в концентраторе событий, и в одной секции превышается локальное ограничение пропускной способности.
    ...
    Способы устранения: пересмотреть стратегию распределения секций или попробовать воспользоваться [EventHubClient.Send(eventDataWithOutPartitionKey)](/dotnet/api/microsoft.servicebus.messaging.eventhubclient#Microsoft_ServiceBus_Messaging_EventHubClient_Send_Microsoft_ServiceBus_Messaging_EventData_).
