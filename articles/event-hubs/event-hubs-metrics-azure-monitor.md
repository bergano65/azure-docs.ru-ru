...
Метрики включены по умолчанию, и вы можете получить доступ к данным за последние 30 дней. Если необходимо хранить данные в течение более длительного периода времени, можно архивировать данные метрик в учетную запись хранения Azure. Этот параметр можно настроить в [параметрах диагностики](../azure-monitor/essentials/diagnostic-settings.md) в Azure Monitor.
...
Можно также открыть метрики непосредственно с помощью пространства имен. Для этого выберите пространство имен и щелкните **метрики**. Чтобы отобразить метрики, отфильтрованные по области концентратора событий, выберите концентратор событий, а затем щелкните **метрики**.
...
Использование метрик в Azure Monitor пока бесплатно. Однако если вы используете другие решения, которые принимают данные метрик, вам может быть выставлен счет за эти решения. Например, при архивации данных метрик в учетную запись хранения Azure плата будет взиматься службой хранилища Azure, Вы также оплачиваете Azure при потоковой передаче данных метрик в журналы Azure Monitor для расширенного анализа.
...
Список метрик, поддерживаемых службой, см. в статье [концентраторы событий Azure](../azure-monitor/essentials/metrics-supported.md#microsofteventhubnamespaces) .

> [!NOTE]
> При возникновении ошибки пользователя концентраторы событий Azure обновляют метрику **ошибок пользователя** , но не заносить в журнал другие диагностические сведения. Поэтому необходимо собрать сведения об ошибках пользователей в приложениях. Кроме того, можно преобразовать данные телеметрии, созданные при отправке или получении сообщений в Application Insights. Пример см. в разделе [Отслеживание с помощью Application Insights](../service-bus-messaging/service-bus-end-to-end-tracing.md#tracking-with-azure-application-insights).

## <a name="azure-monitor-integration-with-siem-tools"></a>Интеграция Azure Monitor с SIEM Tools
Маршрутизация данных мониторинга (журналы действий, журналы диагностики и т. д.) в концентратор событий с Azure Monitor позволяет легко интегрироваться со средствами управления сведениями о безопасности и событиями (SIEM). Дополнительные сведения см. в следующих статьях и записях блога:

- [Потоковая передача данных мониторинга Azure в концентратор событий для потребления внешним инструментом](../azure-monitor/essentials/stream-monitoring-data-event-hubs.md)
