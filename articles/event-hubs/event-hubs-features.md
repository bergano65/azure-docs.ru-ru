...
Благодаря этой интеграции не нужно запустить кластеры Kafka или управлять ими с помощью Zookeeper. Это также позволяет работать с некоторыми из самых требовательных функций концентраторов событий, такими как Сбор, Автоматическое расширение и Геоизбыточное аварийное восстановление.
...
Хотя секции можно идентифицировать и отправлять напрямую, это не рекомендуется делать. Вместо этого можно использовать конструкции более высокого уровня, описанные в [издатель событий](#event-publishers) и разделах емкости. 
