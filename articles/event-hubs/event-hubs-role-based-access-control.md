...
Azure предоставляет следующие встроенные роли RBAC для авторизации доступа к пространству имен концентраторов событий:

Роль [владельца данных концентраторов событий Azure (Предварительная версия)](../role-based-access-control/built-in-roles.md#azure-event-hubs-data-owner-preview) обеспечивает доступ к данным в пространстве имен концентраторов событий и его сущностях (очереди, разделы, подписки и фильтры).

>[!IMPORTANT]
> Ранее мы поддерживали Добавление управляемого удостоверения в роль **владельца** или **участника** . Однако права доступа к данным для роли **владельца** и **участника** больше не учитываются. Если вы используете роль **владельца** или **участника** , перейдите на роль **владельца данных концентраторов событий Azure (Предварительная версия)** .
...
В этом введении описывается простое консольное приложение, [код для которого находится на сайте GitHub](https://github.com/Azure/azure-event-hubs/tree/master/samples/DotNet/Microsoft.Azure.EventHubs/Rbac/AzureEventHubsSDK).
...
Затем [Создайте пространство имен концентраторов событий](event-hubs-create.md). 
