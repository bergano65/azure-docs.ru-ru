...
[Владельца данных концентраторов событий Azure (Предварительная версия)](../role-based-access-control/built-in-roles.md#azure-event-hubs-data-owner-preview) роль обеспечивает доступ к данным в пространство имен концентраторов событий и его сущностей (очереди, разделы, подписки и фильтры)

>[!IMPORTANT]
> Ранее поддерживается добавление управляемое удостоверение для **владельца** или **участник** роли. Однако права для доступа к данным **владельца** и **участник** роли больше не учитываются. Если вы используете **владельца** или **участник** роль в переходе на использование **владельца данных концентраторов событий Azure (Предварительная версия)** роли.
