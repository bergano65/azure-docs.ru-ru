...
Каждому клиенту Центров событий назначается уникальный маркер, который передается в клиент. Маркеры создаются таким, что каждый уникальный маркер предоставляет доступ к другому уникальному издателю. Клиент, владеющий маркером, может отсылаться только одному издателю и не иметь другого издателя. Если несколько клиентов совместно используют один маркер, они также совместно используют издатель.

Всем токенам назначаются ключи SAS. Как правило, все маркеры должны быть подписаны тем же ключом. Клиенты не знают о ключе, что предотвращает применение маркеров производства другими клиентами. Клиенты работают с теми же маркерами до истечения срока их действия.
...
3. Предоставьте маркер клиенту издателя, который может отправить только сущности, доступ к которой предоставляет маркер.
4. После истечения срока действия маркера клиент теряет доступ к отправке и публикации в сущности. 


> [!NOTE]
> Хотя это не рекомендуется, для устройств можно использовать маркеры, которые предоставляют доступ к концентратору событий. Любое устройство, содержащее этот маркер, может передавать сообщения непосредственно в этот концентратор событий. Кроме того, устройство нельзя внести в список блокировок, чтобы запретить отправку данных в концентратор событий.
> 
> Описанное выше поведение можно наблюдать, когда один и тот же токен распространяется на несколько устройств, которые обеспечивают доступ на уровне пространства имен. В этом случае устройство или издатель Rouge нельзя изолировать и отозвать. Рекомендуется всегда предоставлять определенные и детализированные области.
...
> 
> Если маркер украден злоумышленником, злоумышленник может действовать от имени клиента, маркер которого был украден. Если издатель занесен в список блокировок, он будет непригоден для использования, пока он не получит новый маркер, использующий другого издателя.


## <a name="authenticating-event-hubs-consumers-with-sas"></a>Проверка подлинности потребителей концентраторов событий с SAS 
Для проверки подлинности серверных приложений, использующих данные, созданные производителями концентраторов событий, для аутентификации на основе токенов концентраторов событий требуется, чтобы клиенты имели права **управления** или прослушивают права доступа, назначенные его концентраторам событий. пространство имен или экземпляр концентратора событий или раздел. Данные потребляются из концентраторов событий с помощью групп потребителей. Хотя политика SAS обеспечивает детализированную область, эта область определяется только на уровне сущности, а не на уровне потребителя. Это означает, что привилегии, определенные на уровне пространства имен или на уровне экземпляра или раздела концентратора событий, будут применяться к группам потребителей этой сущности.
