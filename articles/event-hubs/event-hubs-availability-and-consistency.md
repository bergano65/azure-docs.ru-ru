...
> [!NOTE]
> Термин **Секция** используется в разных контекстах в концентраторах событий и теорема Cap. 
> - **Концентраторы событий** упорядочивают события в одну или несколько секций. Секции независимы и содержат собственную последовательность данных, они часто увеличиваются по различным тарифам. Дополнительные сведения см. в разделе [Секция](event-hubs-features.md#partitions).
> - В **теоремае Cap** Секция — это разрыв связи между узлами в распределенной системе.
...
При создании нового объекта **[евенсубпродуцерклиент](/dotnet/api/azure.messaging.eventhubs.producer.eventhubproducerclient)** и использовании метода **[SendAsync](/dotnet/api/azure.messaging.eventhubs.producer.eventhubproducerclient.sendasync)** события автоматически распределяются между секциями в концентраторе событий. Такое поведение обеспечивает наивысший показатель времени непрерывной работы.

#### <a name="microsoftazureeventhubs-410-or-earlier"></a>[Microsoft. Azure. EventHubs (4.1.0 или более ранняя версия)](#tab/old)
Если вы создадите объект **[EventHubClient](/dotnet/api/microsoft.azure.eventhubs.eventhubclient)** и используете метод **[Send](/dotnet/api/microsoft.azure.eventhubs.eventhubclient.sendasync#Microsoft_Azure_EventHubs_EventHubClient_SendAsync_Microsoft_Azure_EventHubs_EventData_)** (Отправить), то события автоматически распределятся между секциями в концентраторе событий. Такое поведение обеспечивает наивысший показатель времени непрерывной работы.
