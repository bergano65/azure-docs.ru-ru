...
> [!NOTE]
> Эта статья относится к старой версии SDK центров событий Azure. Чтобы узнать, как перенести код в новую версию SDK, см. 
...
>
> Кроме того, см. [нагрузку раздела баланса в нескольких экземплярах приложения.](event-processor-balance-partition-load.md)
...
- **hostName.** Имя каждого экземпляра потребителя. Каждый экземпляр **EventProcessorHost** должен иметь уникальное значение для этой переменной в группе потребителей, поэтому не старайтесь кодировать это значение.
...
Рекомендуется выполнять это относительно быстро, то есть делать как можно меньше обработки. Вместо этого используйте группы потребителей. Если вам нужно написать в хранилище и сделать некоторые области разгрома, лучше использовать две группы потребителей и два [IEventProcessor](/dotnet/api/microsoft.azure.eventhubs.processor.ieventprocessor) реализации, которые работают отдельно.
...
- [ВызовProcessorAfterReceiveTimeout](/dotnet/api/microsoft.azure.eventhubs.processor.eventprocessoroptions.invokeprocessorafterreceivetimeout): Если этот параметр **является правдой,** [ProcessEventsAsync](/dotnet/api/microsoft.azure.eventhubs.processor.ieventprocessor.processeventsasync) называется, когда основной вызов для получения событий на разперегородки. Этот метод полезен для принятия временных действий в периоды бездействия на разделе.
...
Epoch — это уникальный идентификатор (значение эпохи), который использует служба для обеспечения раздела/аренды. Вы создаете приемник на основе epoch с помощью метода [CreateEpochReceiver.](https://docs.microsoft.com/dotnet/api/microsoft.azure.eventhubs.eventhubclient.createepochreceiver?view=azure-dotnet) Этот метод создает приемник на основе эпохи. Приемник создается для определенного раздела концентратора событий из указанной группы потребителей.

Функция эпохи предоставляет пользователям возможность гарантировать, что есть только один приемник на группу потребителей в любой момент времени, со следующими правилами:

- Если в группе потребителей нет существующего приемника, пользователь может создать приемник с любой эпохной ценностью.
- Если есть приемник с эпохальное значение e1 и новый приемник создается с эпохальное значение e2, где e1 <e2, приемник с e1 будет отключен автоматически, приемник с e2 создается успешно.
- Если есть приемник с эпохальное значение e1 и новый приемник создается с эпохным значением e2, где e1 > e2, то создание e2 с ошибкой: Приемник с эпохой e1 уже существует.

### <a name="no-epoch"></a>Нет эпохи
Вы создаете приемник на основе эпохи с помощью метода [CreateReceiver.](https://docs.microsoft.com/dotnet/api/microsoft.azure.eventhubs.eventhubclient.createreceiver?view=azure-dotnet) 

Существует несколько сценариев обработки потоков, в которых пользователи хотели бы создать несколько приемников в одной группе потребителей. Для поддержки таких сценариев, у нас есть возможность создать приемник без эпохи, и в этом случае мы позволяем до 5 одновременных приемников на группу потребителей.

### <a name="mixed-mode"></a>Смешанный режим
Мы не рекомендуем использовать приложения, где вы создаете приемник с эпохой, а затем переключиться на не-эпохи или наоборот на той же группе потребителей. Однако, когда такое поведение происходит, служба обрабатывает его, используя следующие правила:

- Если есть приемник, уже созданный с эпохой e1 и активно получает события и новый приемник создается без эпохи, создание нового приемника не удастся. Приемники эпохи всегда имеют приоритет в системе.
- Если был приемник, уже созданный с эпохой e1 и отключился, и новый приемник создается без эпохи на новом MessagingFactory, создание нового приемника будет успешным. Существует оговорка здесь, что наша система будет обнаруживать "приемник отключения" после 10 минут.
- Если есть один или несколько приемников, созданных без эпохи, и новый приемник создается с эпохой e1, все старые приемники отключаются.


> [!NOTE]
> Мы рекомендуем использовать различные группы потребителей для приложений, использующих эпохи, и для тех, кто не использует эпохи, чтобы избежать ошибок. 
...
* [Образцы концентраторов событий на GitHub](https://github.com/Azure/azure-event-hubs/tree/master/samples)
