...
Вы можете добавить управляемое удостоверение в роль **владельца данных концентраторов событий** для пространства имен концентраторов событий. Эта роль предоставляет удостоверению полный доступ (для управления и операций с данными) ко всем сущностям в пространстве имен.

>[!IMPORTANT]
> Ранее мы поддерживали Добавление управляемого удостоверения в роль **владельца** или **участника** . Однако права доступа к данным для роли **владельца** и **участника** больше не учитываются. Если вы используете роль **владельца** или **участника** , переключитесь на использование роли **владельца данных концентраторов событий** .

Чтобы использовать новую встроенную роль, выполните следующие действия. 

1. Перейдите на [портал Azure](https://portal.azure.com).
2. Перейдите к пространству имен концентраторов событий.
3. На странице **пространство имен концентраторов событий** выберите **Управление доступом (IAM)** в меню слева.
4. На странице **Управление доступом (IAM)** выберите **Добавить** в разделе **Добавление назначения роли** . 

    ![Кнопка добавления назначения роли](./media/event-hubs-managed-service-identity/add-role-assignment-button.png)
5. На странице **Добавление назначения ролей** выполните следующие действия. 
    1. В качестве **роли**выберите **владелец данных концентраторов событий Azure**. 
    2. Выберите **удостоверение** , которое будет добавлено к роли.
...
6. Перейдите на страницу **назначения ролей** и убедитесь, что пользователь добавлен в роль **владельца данных концентраторов событий Azure** . 

    ![Подтверждение добавления пользователя в роль](./media/event-hubs-managed-service-identity/role-assignments.png)
...
Теперь можно изменить страницу по умолчанию созданного вами приложения ASP.NET. Вы также можете использовать код веб-приложения из этого [репозитория GitHub](https://github.com/Azure/azure-event-hubs/tree/master/samples/DotNet/Microsoft.Azure.EventHubs/Rbac/ManagedIdentityWebApp). 
