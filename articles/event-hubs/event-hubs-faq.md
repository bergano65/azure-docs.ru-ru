...
* [Выделяем](event-hubs-capture-overview.md)
...
Чтобы подключиться к концентратору событий, необходимо обновить конфигурации клиента Kafka. Для этого нужно создать пространство имен концентраторов событий и получить [строку подключения](event-hubs-get-connection-string.md). Измените bootstrap.servers, чтобы он содержат полное доменное имя Центров событий и порт 9093. Обновите SASL. JAAS. config, чтобы направить клиент Kafka в конечную точку концентратора событий (которая является полученной строкой подключения) с правильной проверкой подлинности, как показано ниже:
...
### <a name="what-is-the-messageevent-size-for-event-hubs"></a>Каков размер сообщения или события для концентраторов событий?
Максимальный размер сообщения, допустимый для концентраторов событий, составляет 1 МБ.
