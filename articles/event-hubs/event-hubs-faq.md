...
### <a name="when-do-i-create-a-new-namespace-vs-use-an-existing-namespace"></a>Когда я создаю новое пространство имен и использует существующее пространство имен?
Распределение емкости[(единицы пропускной способности (ТУ)](#throughput-units)выставляется счетом на уровне пространства имен. Пространство имен также связано с регионом.

Вместо существующего в одном из следующих сценариев может потребоваться создать новое пространство имен: 

- Вам нужен концентратор событий, связанный с новым регионом.
- Вам нужен концентратор событий, связанный с другой подпиской.
- Вам нужен концентратор событий с определенным распределением емкости (т.е. потребность в емкости пространства имен с добавленным концентратором событий превысит порог 40 TU, и вы не хотите идти на выделенный кластер)  
...
* Более одной [группы потребителей](event-hubs-features.md#consumer-groups)
* [Захвата](event-hubs-capture-overview.md)
...
Центры событий (цен. категория "Стандартный") в настоящее время поддерживают максимальный срок хранения в семь дней. Концентраторы событий не предназначены в качестве постоянного хранилища данных. Периоды удержания более 24 часов предназначены для сценариев, в которых удобно воспроизводить поток событий в однии и те же системы; например, обучить или проверить новую модель машинного обучения на существующих данных. Если требуется хранить сообщения дольше семи дней, включите функцию [Сбор](event-hubs-capture-overview.md) в Центрах событий, чтобы извлекать из концентратора событий данные в учетную запись хранения или учетную запись службы Azure Data Lake по своему выбору. Включение записи повлечет за собой затраты в соответствии с приобретенной единицей пропускной способности.

Можно настроить период хранения захваченных данных в учетной записи хранилища. Функция **управления жизненным циклом** Azure Storage предлагает богатую политику, основанную на правилах, для учетных записей общего назначения v2 и blob. Эти политики позволяют переводить данные на новые уровни доступа и удалять их по завершении жизненного цикла. Для получения дополнительной информации [см.](../storage/blobs/storage-lifecycle-management-concepts.md) 
...
### <a name="what-ports-do-i-need-to-open-on-the-firewall"></a>Какие порты нужно открыть на брандмауэре? 
Для отправки и получения сообщений можно использовать следующие протоколы с помощью Azure Service Bus:

- Протокол AMQP
- HTTP
- Apache Kafka

Ознакомьтесь со следующей таблицей для исходящих портов, которые необходимо открыть для использования этих протоколов для связи с концентраторами событий Azure. 

| Протокол | порты; | Сведения | 
| -------- | ----- | ------- | 
| AMQP | 5671 и 5672 | Смотрите [руководство по протоколу АМЗП](../service-bus-messaging/service-bus-amqp-protocol-guide.md) | 
| HTTP, HTTPS | 80, 443 |  |
| Kafka | 9093 | Посмотреть [центры использования событий из приложений Kafka](event-hubs-for-kafka-ecosystem-overview.md)

### <a name="what-ip-addresses-do-i-need-to-whitelist"></a>Какие IP-адреса мне нужны для белого списка?
Чтобы найти нужные IP-адреса для «белого списка» для ваших подключений, выполните следующие действия:

1. Выполнить следующую команду из запроса команды: 

    ```
    nslookup <YourNamespaceName>.servicebus.windows.net
    ```
2. Заметьте, что IP-адрес вернулся в `Non-authoritative answer`. Единственный раз, когда она изменится, это восстановление пространства имен в другом кластере.

Если вы используете избыточность зоны для пространства имен, вам нужно сделать несколько дополнительных шагов: 

1. Во-первых, вы запустите nslookup на пространстве имен.

    ```
    nslookup <yournamespace>.servicebus.windows.net
    ```
2. Запишите имя в **разделе неавторитетный ответ,** который находится в одном из следующих форматов: 
    ...
    ```
3. Запустите nslookup для каждого из них с суффиксами s1, s2 и s3, чтобы получить IP-адреса всех трех экземпляров, работающих в трех зонах доступности, 
...
Чтобы подключиться к концентратору событий, необходимо обновить конфигурацию клиента Kafka. Это делается путем создания пространства имен event Hubs и получения [строки соединения.](event-hubs-get-connection-string.md) Измените bootstrap.servers, чтобы он содержат полное доменное имя Центров событий и порт 9093. Обновление sasl.jaas.config, чтобы направить клиента Kafka к вашей конечной точке событий концентратора (которая является строкой соединения вы получили), с правильной аутентификацией, как показано ниже:
...
Примечание: Если sasl.jaas.config не поддерживается конфигурацией в вашей структуре, найдите конфигурации, которые используются для установки имени пользователя sASL и пароля, и используйте их вместо этого. В качестве имени пользователя задайте $ConnectionString, а в качестве пароля — строки подключения для Центров событий.

### <a name="what-is-the-messageevent-size-for-event-hubs"></a>Каков размер сообщения/события для концентраторов событий?
Максимальный размер сообщения, разрешенный для концентраторов событий, составляет 1 МБ.
...
С этой функцией не связаны **затраты.** 
...
Ценовая категория "Выделенный" для Центров событий позволяет создавать развертывания с одним клиентом для самых требовательных пользователей. Это предложение создает кластер с привязкой к объему ресурсов, который не ограничен единицами пропускной способности. Это означает, что вы можете использовать кластер для глотания и потоковой передачи данных в зависимости от процессора и использования памяти кластера. Дополнительные сведения см. в статье [Обзор Центров событий ценовой категории "Выделенный"](event-hubs-dedicated-overview.md).

### <a name="how-much-does-a-single-capacity-unit-let-me-achieve"></a>Что мне предоставит одна единица емкости?
Для выделенного кластера, сколько вы можете глотать и поток зависит от различных факторов, таких как ваши производители, потребители, скорость, с которой вы глотания и обработки, и многое другое. 
...
Чтобы создать кластер Центров событий ценовой категории "Выделенный", отправьте [запрос в службу поддержки на увеличение квоты](https://portal.azure.com/#create/Microsoft.Support) или свяжитесь с [группой разработчиков Центров событий](mailto:askeventhubs@microsoft.com). Развертывание кластера и передача пользователю, то есть вам, обычно занимает около двух недель. Этот процесс является временным до тех пор, пока полное самообслуживание не будет доступно через шаблоны портала Azure или Azure Resource Manager, развертывание кластера занимает около двух часов.

## <a name="best-practices"></a>Рекомендации

### <a name="how-many-partitions-do-i-need"></a>Сколько секций мне нужно?
Количество секций определяется во время создания. Рабочий диапазон — от 2 до 32. Количество разделов не изменяется, поэтому при настройке количества разделов следует учитывать долгосрочную шкалу. Секции — это способ организации данных, который соотносится со степенью параллелизма подчиненных элементов, требуемой для работы потребляющих приложений. Поэтому количество секций в концентраторе событий непосредственно связано с предполагаемым числом параллельных модулей чтения. Для получения дополнительной информации [Partitions](event-hubs-features.md#partitions)о разделах см.
...
В Центрах событий предусмотрен один модуль чтения секции на каждую группу потребителей. В большинстве вариантов использования значение по умолчанию (четыре секции) является достаточным. Если вы хотите масштабировать обработку событий, вы можете рассмотреть возможность добавления дополнительных разделов. На перегородке нет определенного ограничения пропускной всей перевалки, однако совокупная пропускная информация в пространстве имен ограничена количеством блоков пропускной выхасти. Увеличивая количество единиц пропускной способности в пространстве имен, вы можете увеличить количество секций, чтобы в параллельных модулях чтения также достигалась максимальная пропускная способность.
