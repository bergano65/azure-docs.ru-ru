...
### <a name="when-do-i-create-a-new-namespace-vs-use-an-existing-namespace"></a>Если создать новое пространство имен или использование существующего пространства имен?
Выделение емкости ([единиц пропускной способности (единицы пропускной способности)](#throughput-units)) выставляются на уровне пространства имен. Пространство имен также связана с регионом.

Вы можете создать новое пространство имен вместо использования существующих в одном из следующих сценариев: 

- Вам потребуется концентратор событий, связанных с новой области.
- Вам потребуется концентратор событий, связанных с другой подпиской.
- Вам потребуется концентратор событий с выделением отдельных емкости (то есть емкость требуются для пространства имен с концентратором событий добавлена превысит пороговое значение 40 единиц пропускной Способности, и вы не хотите обратиться за выделенный кластер)  
...
| HTTP, HTTPS | 80, 443 |  |
| Kafka | 9093 | См. в разделе [использование концентраторов событий из приложений Kafka](event-hubs-for-kafka-ecosystem-overview.md)
...
1. Выполните следующую команду из командной строки: 
    ...
    ```
2. Запишите IP-адрес, возвращаемый в `Non-authoritative answer`. Единственный момент времени, при этом изменяются является его восстановить пространство имен на другом кластере.
