...
> [!NOTE]
> Функция восстановления гео-катастрофы доступна только для [стандартных и специализированных SKUs.](https://azure.microsoft.com/pricing/details/event-hubs/)  
...
Функция восстановления гео-катастроф доступен только для [стандартных и специализированных SKUs.](https://azure.microsoft.com/pricing/details/event-hubs/) Вам не нужно изменять какие-либо строки подключения, так как соединение выполняется через псевдоним.
...
## <a name="supported-namespace-pairs"></a>Поддерживаемые пары пространства имен
Поддерживаются следующие комбинации первичных и вторичных областей имен:  

| Первичное пространство имен | Вторичное пространство имен | Поддерживается | 
| ----------------- | -------------------- | ---------- |
...
> [!NOTE]
> Нельзя объединять пространства имен, которые находятся в одном выделенном кластере. Можно паратировать пространства имен, которые находятся в отдельных кластерах. 
...
1. По замыслу, геоаварийное восстановление Event Hubs не воспроизводит данные, и поэтому вы не можете повторно использовать старое смещенное значение основного концентратора событий в вторичном концентраторе событий. Мы рекомендуем перезапустить приемник событий с одним из следующих:

- *EventPosition.FromStart ()* - Если вы хотите прочитать все данные о вашем вторичном центре событий.
- *EventPosition.FromEnd ()* - Если вы хотите прочитать все новые данные с момента подключения к вашему вторичному концентратору событий.
- *EventPosition.FromEnqueuedTime (dateTime)* - Если вы хотите прочитать все данные, полученные в вашем вторичном центре событий, начиная с заданной даты и времени.
