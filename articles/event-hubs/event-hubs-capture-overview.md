...
Концентраторы событий Azure позволяют автоматически захватывать потоковые данные в концентрах событий в [хранилище Azure Blob](https://azure.microsoft.com/services/storage/blobs/) или учетной записи [Azure Data Lake Storage Gen 1 или Gen 2](https://azure.microsoft.com/services/data-lake-store/) по вашему выбору с дополнительной гибкостью указания интервала времени или размера. Настройка функции "Сбор" выполняется быстро, ее использование не влечет дополнительных административных расходов, а масштабирование осуществляется автоматически на основе [единиц пропускной способности](event-hubs-scalability.md#throughput-units) Центров событий. Функция "Сбор" в Центрах событий — это самый удобный способ передачи потоковых данных в Azure. Он позволяет сосредоточиться на обработке данных, а не на их записи.
...
Захват концентраторов событий позволяет указать собственную учетную запись хранения и контейнер Azure Blob, а также учетную запись хранения данных Azure Data Lake, которая используется для хранения захваченных данных. Эти учетные записи могут находиться в том же регионе, что и концентратор событий, или в другом. Это дает гибкие возможности использования функции "Сбор" в Центрах событий.
...
- [Включить концентраторы событий захват с помощью портала Azure](event-hubs-capture-enable-through-portal.md)
...
[Apache дрель: Azure Blob хранения плагин][Apache Drill: Azure Blob Storage Plugin]
...
[Потоковая передача по шкале: Захват концентраторов событий]

### <a name="use-apache-spark"></a>Использование Apache Spark

[Apache Spark][Apache Spark] — это "единый аналитический механизм для крупномасштабной обработки данных". Он поддерживает разные языки, включая SQL, и может легко связываться с хранилищем BLOB-объектов Azure. Существует несколько вариантов запуска Apache Spark в Azure, и каждый из них обеспечивает легкий доступ к хранению Azure Blob:

- [HDInsight: Адресные файлы в хранилище Azure][HDInsight: Address files in Azure storage]
- [Лазурные databricks: хранилище Azure Blob][Azure Databricks: Azure Blob Storage]
- [Служба Azure Kubernetes](https://docs.microsoft.com/azure/aks/spark-job) 
...
Apache Avro предоставляет руководства по началу работы для платформ [Java][Java] и [Python][Python]. Вы также можете прочитать Начало работы со статьей [Event Hubs Capture.](event-hubs-capture-python.md)
...
Обратите внимание, что Capture не потребляет квоту на выход, так как выставляется отдельно. 
...
Узнайте, как включить эту функцию с помощью портала Azure и шаблона управления ресурсами Azure:

- [Использование портала Azure для включения функции "Сбор" в Центрах событий](event-hubs-capture-enable-through-portal.md)
- [Используйте шаблон менеджера ресурсов Azure для включения в комплектует концентраты событий](event-hubs-resource-manager-namespace-event-hub-enable-capture.md)
...
[Потоковая передача по шкале: Захват концентраторов событий]:https://github.com/yorek/streaming-at-scale/tree/master/event-hubs-capture
