...
Собранные данные записываются [Apache Avro][Apache Avro] формат: — сжатый быстрый двоичный формат, обеспечивающий эффективную структуру данных с помощью встроенной схемы. Этот формат широко используется в экосистеме Hadoop, Stream Analytics и фабрике данных Azure. Работа с Avro более подробно описана далее в этой статье.
...
В случае, если большой двоичный объект хранилища Azure, временно недоступна, записи концентраторов событий будет хранить ваши данные для срока хранения данных, настроенным в концентраторе событий и фоновая заливка данные после своей учетной записи хранения снова станет доступным.

### <a name="scaling-to-throughput-units"></a>Масштабирование единиц пропускной способности

Трафик Центров событий контролируется с помощью [единиц пропускной способности](event-hubs-scalability.md#throughput-units). Одна единица пропускной способности разрешает передачу до 1 МБ/с или 1000 событий/с для входящих данных или до 2 МБ/с или 2000 событий/с для исходящих данных. Концентраторы событий Standard можно настроить от 1 до 20 единиц пропускной способности, и вы можете приобрести больше увеличение квоты [запрос в службу поддержки][support request]. Использование единиц пропускной способности свыше приобретенного количества регулируется. Функция "Сбор" в Центрах событий копирует данные непосредственно из внутреннего хранилища Центров событий. При этом выполняется обход квоты на единицы пропускной способности для исходящего трафика, а этот трафик сохраняется для других средств обработки, например Stream Analytics или Spark.
...
Простой способ просмотра файлов Avro — с помощью [средств Avro][Avro Tools] jar from Apache. You can also use [Apache Drill][Apache Drill] для упрощенного взаимодействия на основе SQL или [Apache Spark][Apache Spark] для выполнения Комплексное распределенную обработку на данные, полученные. 

### <a name="use-apache-drill"></a>Использование Apache Drill

[Apache Drill][Apache Drill] является «открытый модуль SQL-запросов для анализа больших данных», можно запросить структурированных и полуструктурированных данных, везде, где. Этот механизм может работать как отдельный узел или как огромный кластер для высокой производительности.
...
[Apache Drill. Подключаемый модуль службы хранилища BLOB-объектов Azure][Apache Drill: Azure Blob Storage Plugin]
...
[Apache Spark][Apache Spark] — это «единой analytics обрабатывать большие объемы данных.» Он поддерживает разные языки, включая SQL, и может легко связываться с хранилищем BLOB-объектов Azure. Существует два варианта для запуска Apache Spark в Azure, и оба обеспечивают легкий доступ к хранилищу BLOB-объектов Azure.

- [HDInsight. Обращение к файлам в службе хранилища Azure][HDInsight: Address files in Azure storage]
- [Azure Databricks. Хранилище BLOB-объектов][Azure Databricks: Azure Blob Storage]

### <a name="use-avro-tools"></a>Использование средств Avro

[Средств Avro][Avro Tools] доступны в виде JAR-пакет. После того как вы загрузили этот JAR-файл, чтобы просмотреть схему определенного файла Avro, выполните следующую команду:
...
Apache Avro предусмотрены руководства по началу работы для [Java][Java] and [Python][Python]. Дополнительные сведения см. в статье [Пошаговое руководство. Использование функции "Сбор" в Центрах событий с Python](event-hubs-capture-python.md).
...
Обратите внимание на то, что записи не потреблять квоты исходящих сообщений, как начисляется отдельно. 
