...
По умолчанию функция, которая обрабатывает события, вызывается последовательно для данной секции. Последующие события и вызовы этой функции из одной и той же очереди секционирования по мере того, как конвейер событий продолжит выполняться в фоновом режиме в других потоках. Обратите внимание, что события из разных секций могут обрабатываться параллельно, и любое общее состояние, доступное в секциях, должно быть синхронизировано.
