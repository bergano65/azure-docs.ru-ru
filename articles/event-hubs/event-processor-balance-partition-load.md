...
В этой статье описывается пример сценария для использования нескольких экземпляров для чтения событий из концентратора событий, а затем дать вам подробную информацию об особенностях клиента процессора событий, что позволяет получать события из нескольких разделов одновременно и баланс нагрузки с другими потребителей, которые используют один и тот же концентратор событий и группу потребителей.
