...
* Кластер Вфкст и виртуальная машина контроллера кластера должны находиться в одной и той же виртуальной сети (vnet) в одной группе ресурсов и использовать одну и ту же учетную запись хранения. Шаблон автоматического создания кластера обрабатывает это в большинстве случаев.
...
* Шаблон создания кластера может создавать большинство необходимых ресурсов инфраструктуры для кластера, включая группы ресурсов, виртуальные сети, подсети и учетные записи хранения. Если вы хотите использовать уже существующие ресурсы, убедитесь, что они соответствуют требованиям, приведенным в этой таблице. 

  | Resource | Использовать существующий? | Требования |
  |----------|-----------|----------|
...
Кэш диска на каждый узел настраивается и может быть размером от 1000 до 8000 ГБ. Рекомендуемый размер кэша для узлов Standard_E32s_v3 — 4 ТБ на узел.
