...
Существуют некоторые обходные пути, позволяющие не владельцам создавать Авере Вфкст для кластера Azure. Эти сценарии подразумевают ограниченность ресурсов и назначение создателям дополнительных ролей управления доступом на основе ролей (RBAC). Во всех этих случаях владелец подписки также должен заранее [принять условия использования программного обеспечения Авере вфкст](#accept-software-terms) .
