...
![схема с виртуальной сетью, содержащей дополнительное хранилище BLOB-объектов, и подсетью, содержащей три сгруппированные виртуальные машины с метками узлов vFXT и кластера vFXT и одну виртуальную машину с меткой контроллера кластера](media/avere-vfxt-deployment-500px.png)
...
Посетите страницу выходных данных, чтобы собрать сведения, необходимые для кластера. Дополнительные сведения см. в разделе [Сбор необходимых входных данных](#inputs-needed-for-cluster-creation).
...
> [!NOTE] 
> В течение первой недели, когда выйдет общедоступная версия (с 31 октября по 7 ноября 2018 года), нужно использовать параметр командной строки, чтобы принять условия для двух образов программного обеспечения вместо выполнения этой процедуры. Выполните инструкции, приведенные в разделе [Принятие условий программного обеспечения заранее](avere-vfxt-prereqs.md#accept-software-terms-in-advance). 
...
* **Учетная запись администратора**: укажите способ входа в контроллер кластера. 
  * Выберите метод "Имя пользователя и пароль" или "Открытый ключ SSH" (рекомендуется).
    ...
    > [!TIP] 
    > Ключ SSH более безопасен. Дополнительные сведения см. в статье [Краткая инструкция: создание и использование пары из открытого и закрытого ключей SSH для виртуальных машин Linux в Azure](https://docs.microsoft.com/azure/virtual-machines/linux/mac-create-ssh-keys). 
...
## <a name="gather-needed-inputs"></a>Сбор необходимых входных данных

Для создания кластера вам потребуются следующие сведения. 

Если вы создали узел контроллера с помощью шаблона Resource Manager, вы можете [получить сведения из выходных данных шаблона](#finding-template-output). 

Для подключения к контроллеру необходимы следующие данные: 
...
Для создания кластера потребуются следующие данные: 

* Имя группы ресурсов
* Расположение Azure. 
* Имя виртуальной сети
* Имя подсети
* Имя роли узла кластера.
* Имя учетной записи хранения, если создается контейнер больших двоичных объектов.

Вы также можете найти недостающие сведения, перейдя на страницу сведений о виртуальной машине контроллера. Например, выберите **Все ресурсы** и найдите имя контроллера, а затем щелкните его, чтобы просмотреть подробные сведения.

### <a name="finding-template-output"></a>Поиск выходных данных шаблона

Чтобы найти эти сведения в выходных данных шаблона Resource Manager, выполните следующую процедуру:

1. На верхней панели портала Azure щелкните значок уведомлений и нажмите кнопку **Перейти к группе ресурсов**. Откроется новая группа ресурсов, содержащая контроллер и виртуальную сеть.

   ![Раздел уведомлений портала с сообщением "Развертывание прошло успешно" и кнопками "Перейти к группе ресурсов" и "Закрепить на панели мониторинга"](media/avere-vfxt-browse-to-rg.png)
...
1. Способ подключения к контроллеру кластера зависит от вашей настройки.
   ...
   * Если у контроллера нет общедоступного IP-адреса, установите [ExpressRoute](https://docs.microsoft.com/azure/expressroute/) или VPN-подключение к вашей виртуальной сети.
