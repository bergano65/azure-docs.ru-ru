...
Найдите на портале Azure нужный шаблон создания, выполнив поиск по строке Avere и выбрав Avere vFXT for Azure Deployment (Avere vFXT для развертывания Azure). <!-- xxx update if that name changes xxx --> 

<!-- **[XXX need production image of template deploy in search and/or entry page of template deploy XXX]** -->

Щелкните **Создать**, чтобы начать развертывание. 
...
* **Resource group** (Группа ресурсов). Выберите группу ресурсов для кластера Avere vFXT или щелкните Create new (Создать) и введите имя новой группы ресурсов. 
...
* **Use blob storage** (Использовать хранилище BLOB-объектов). Выберите, нужно ли создать новый контейнер BLOB-объектов Azure и настроить его в качестве серверного хранилища для нового кластера Avere vFXT. Если вы решили создать новый контейнер, предоставьте для него учетную запись хранения. Если вы не будете создавать новый контейнер больших двоичных объектов, к созданному кластеру необходимо присоединить уже существующее хранилище (см. [инструкции по настройке хранилища](avere-vfxt-add-storage.md)). Задайте этому полю значение **false**, если вы не хотите создавать контейнер.

* **Storage account** (Учетная запись хранения). Если вы создаете новый контейнер больших двоичных объектов Azure, введите здесь имя учетной записи хранения. Это должна быть стандартная учетная запись хранения общего назначения версии 2, для которой настроены локально избыточное хранилище и горячий уровень доступа. См. дополнительные сведения о [требованиях к учетной записи хранения](avere-vfxt-add-storage.md#azure-storage-cloud-core-filer).
