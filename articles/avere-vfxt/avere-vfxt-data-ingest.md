...
1. Определите общее число копируемых файлов и каталогов.
   ...
   Например, используйте служебную программу ``prime.py`` авере с аргументами ```prime.py --directory /path/to/some/directory``` (доступны для скачивания <https://github.com/Azure/Avere/blob/master/src/clientapps/dataingestor/prime.py>URL-адреса).
