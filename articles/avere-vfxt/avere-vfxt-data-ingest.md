...
После создания нового кластера vFXT первой задачей может стать перемещение данных на новый объем хранилища в Azure. Однако, если вы будете использовать обычный метод — выполнение простой команды копирования из одного клиента — вероятно, скорость копирования будет низкой. Однопотечное копирование не является хорошим вариантом для копирования данных в бэк-энд-хранилище кластера Avere vFXT.

Поскольку кластер Avere vFXT для Azure является масштабируемым кэшем с несколькими клиентами, самый быстрый и эффективный способ копирования данных — это несколько клиентов. Этот метод параллелизует прием файлов и объектов.
...
Здесь также описываются некоторые служебные программы, которые могут помочь. Утилита ``msrsync`` может быть использована для частичной автоматизации процесса деления ``rsync`` набора данных на ведра и использования команд. Сценарий ``parallelcp`` — это другая служебная программа, которая считывает исходный каталог и выдает команды копирования автоматически. Кроме того, ``rsync`` инструмент может быть использован в два этапа, чтобы обеспечить более быструю копию, которая по-прежнему обеспечивает согласованность данных.
...
* [Двухфазный пример rsync](#use-a-two-phase-rsync-process)
...
При разработке стратегии копирования данных параллельно следует понимать компромиссы в размере файла, подсчете файлов и глубине каталога.
...
Каждый процесс копирования имеет пропускную способность и скорость передачи файлов, которые можно измерять по времени команды копирования с учетом размера и количества файлов. Объяснение того, как измерять ставки выходит за рамки этого документа, но важно понять, будете ли вы иметь дело с небольшими или большими файлами.
...
После того как будет достаточно параллельных потоков, направленных к одной точке подключения файловой системы, наступит момент, когда добавление потоков не обеспечит больше пропускной способности. (Пропуск будет измеряться в файлах/секундах или байтах в секунду, в зависимости от типа данных.) Или, что еще хуже, чрезмерное резьба иногда может привести к деградации пропускной связи.
...
## <a name="use-a-two-phase-rsync-process"></a>Используйте двухфазный процесс rsync

Стандартная ``rsync`` утилита не работает хорошо для заполнения облачного хранилища через систему Avere vFXT для системы Azure, поскольку она генерирует большое количество файлов, создавая и переименовывая операции, чтобы гарантировать целостность данных. Тем не менее, вы ``--inplace`` можете ``rsync`` безопасно использовать опцию, чтобы пропустить более тщательную процедуру копирования, если вы будете следовать, что со вторым запуском, который проверяет целостность файла.

Стандартная ``rsync`` операция копирования создает временный файл и заполняет его данными. Если передача данных завершается успешно, временный файл переименовывался в исходное имя файла. Этот метод гарантирует согласованность, даже если файлы доступны во время копирования. Но этот метод генерирует больше операций записи, что замедляет движение файлов через кэш.

Опция ``--inplace`` записывает новый файл непосредственно в его окончательном месте. Файлы не гарантированно быть последовательными во время передачи, но это не важно, если вы грунтовки системы хранения для использования позже.

Вторая ``rsync`` операция служит проверкой согласованности первой операции. Поскольку файлы уже скопированы, второй этап представляет собой быстрое сканирование, чтобы убедиться, что файлы назначения соответствуют файлам источника. Если какие-либо файлы не совпадают, они копируются повторно.

Вы можете выпустить оба этапа вместе в одной команде:

```bash
rsync -azh --inplace <source> <destination> && rsync -azh <source> <destination>
```

Этот метод является простым и эффективным по времени методом для наборов данных до количества файлов, которые может обрабатывать диспетчер внутреннего каталога. (Обычно это 200 миллионов файлов для кластера 3-узлов, 500 миллионов файлов для кластера из шести узлов и так далее.)

## <a name="use-the-msrsync-utility"></a>Использование утилиты msrsync

Инструмент ``msrsync`` также может быть использован для перемещения данных в основной файлер бэк-энда для кластера Avere. Это средство предназначено для оптимизации использования пропускной способности путем запуска нескольких параллельных процессов ``rsync``. Оно доступно на сайте GitHub по адресу <https://github.com/jbd/msrsync>.
...
Вы также можете ``--inplace`` использовать ``msrsync`` аргумент с командами. Если вы используете эту опцию, рассмотрите возможность запуска второй команды (как в случае с [rsync,](#use-a-two-phase-rsync-process)описанной выше) для обеспечения целостности данных.

``msrsync``можно писать только в местные тома и из нее. Источник и назначение должны быть доступны как локальные точки подключения в виртуальной сети кластера.

Чтобы ``msrsync`` использовать облачный том Azure кластером Avere, следуйте следующим инструкциям:

1. Установка ``msrsync`` и ее предпосылки (rsync и Python 2.6 или более поздней позже)
1. Определите общее число копируемых файлов и каталогов.

   Например, используйте утилиту ``prime.py`` Avere с ```prime.py --directory /path/to/some/directory``` аргументами <https://github.com/Azure/Avere/blob/master/src/clientapps/dataingestor/prime.py>(доступно при загрузке URL).
...
1. Выдать ``msrsync`` команду для копирования файлов:
   ...
   При ``--inplace``использовании добавьте второе выполнение без возможности проверить, правильно ли скопированы данные:
...
Скрипт ``parallelcp`` также может быть полезен для передачи данных в бэк-энд-хранилище кластера vFXT.
