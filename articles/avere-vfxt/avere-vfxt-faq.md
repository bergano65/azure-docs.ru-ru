...
Следующая диаграмма может помочь вам ответить на этот вопрос. Чем ближе рабочий процесс к правому верхнему углу, тем больше вероятность того, что решение avere vFXT для кэширования Azure подходит для вашей среды.

![График диаграммы, показывающие, что читать тяжелые нагрузки с тысячами клиентов лучше подходят для Avere vFXT](media/avere-vfxt-fit-assessment.png)
...
Avere vFXT для Azure — это кэш. В ней данные не хранятся. В нем используется сочетание ОЗУ и дисков SSD для хранения кэшированных данных. Данные постоянно хранятся в системе серверного хранилища (например, система запоминающего устройства, подключаемого к сети NetApp, или контейнер больших двоичных объектов). Система Avere vFXT не владеет сведениями об объеме хранимых данных, из которого они кэшируются. Avere vFXT кэширует только подмножество данных, которое требуется клиентам.  

### <a name="what-regions-are-supported"></a>Какие регионы поддерживаются?

Avere vFXT для Azure поддерживается во всех регионах, за исключением суверенных регионов (Китай, Германия). Убедитесь, что область, которую вы хотите использовать, может поддерживать большое количество вычислительных ядер и экземпляры виртуальных машин, необходимых для создания кластера Avere vFXT.

### <a name="how-do-i-get-help-with-avere-vfxt"></a>Как получить помощь по работе с Avere vFXT?

Специализированная группа вспомогательного персонала предлагает помощь в avere vFXT для Azure. Чтобы подать запрос в службу поддержки на портале Azure, следуйте инструкциям в разделе [Open a support ticket for your Avere vFXT](avere-vfxt-open-ticket.md#open-a-support-ticket-for-your-avere-vfxt) (Подача запроса на поддержку по работе с Avere vFXT).
...
> [!NOTE]
> Плата за программное обеспечение взимается с использования Avere vFXT с aWS или Google Cloud. Дополнительная плата за использование хранилища Azure blob не взимается.

## <a name="technical-compute"></a>Технический: Вычислить
...
Кластер Avere vFXT для Azure использует практические компьютеры Microsoft Azure E32s_v3.
...
## <a name="technical-disks"></a>Технический: Диски
...
## <a name="technical-networking"></a>Технические: Сеть

### <a name="what-network-is-recommended"></a>Какая сеть рекомендуется?

Если вы используете на месте хранения с Avere vFXT, вы должны иметь 1-Гбит/с или лучшее сетевое соединение между хранилищем и кластером. Если объем данных небольшой и перед запуском заданий необходимо скопировать данные в облако, VPN-подключения может быть достаточно.

> [!TIP]
> Чем медленнее сетевое звено, тем медленнее будет исходное "холодное" сообщение. Медленные операции чтения увеличивают задержку рабочего конвейера.
...
Да. Avere vFXT работает строго как кластер высокой доступности (HA) и требует нескольких IP-адресов для работы. Если кластер находится в собственной подсети, вы можете избежать конфликтов IP-адресов, которые могут привести к проблемам с установкой и перебоям в работе. Подсеть кластера может находиться в виртуальной сети, используемой другими ресурсами, до тех пор, пока IP-адреса не пересекаются.
...
### <a name="can-i-restrict-internet-access-from-my-clusters-virtual-network"></a>Могу ли я ограничить доступ в Интернет из виртуальной сети моего кластера?

Как правило, вы можете настроить дополнительную безопасность в виртуальной сети по мере необходимости, но некоторые ограничения могут помешать работе кластера.

Например, ограничение исходящего доступа в Интернет из виртуальной сети создает проблемы для кластера, если вы также не добавите правило, которое явно позволяет получить доступ к AzureCloud. Эта ситуация описана в [дополнительной документации на GitHub](https://github.com/Azure/Avere/tree/master/src/vfxt/internet_access.md).

Для справки с настраиваемыми безопасности, обратитесь в службу поддержки, описанную в [Get help with your system.](avere-vfxt-open-ticket.md#open-a-support-ticket-for-your-avere-vfxt)

## <a name="technical-back-end-storage-core-filers"></a>Техническое: Резервное хранилище (основные регистраторы)
...
Для получения более подробной информации о требованиях к учетной записи хранилища читайте в материале [«Облачный файл хранения блоков Azure Blob».](avere-vfxt-add-storage.md#azure-blob-storage-cloud-core-filer)
...
Хранилище blob Cool tier обычно не рекомендуется для файла Avere vFXT для основного файлера Azure. Cool tier предлагает более низкие затраты на хранение, но более высокие эксплуатационные расходы. (См. [Цены Block blob](<https://azure.microsoft.com/pricing/details/storage/blobs/>) для получения более подробной информации.) Если данные будут доступны и изменены или удалены часто, пожалуйста, рассмотрите возможность использования горячего уровня.

[Уровни доступа](https://docs.microsoft.com/azure/storage/blobs/storage-blob-storage-tiers#cool-access-tier) дают больше информации о том, когда имеет смысл использовать хранилище Cool уровня в качестве основного файлера vFXT.
...
* [Обзор развертывания](avere-vfxt-deploy-overview.md)
