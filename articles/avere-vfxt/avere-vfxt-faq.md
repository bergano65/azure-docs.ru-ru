...
Да. Авере Вфкст работает строго как высокодоступный кластер (HA) и требует для работы нескольких IP-адресов. Если кластер находится в собственной подсети, вы можете избежать конфликтов IP-адресов, которые могут привести к проблемам с установкой и перебоям в работе. Подсеть кластера может находиться в имеющейся виртуальной сети при условии, что IP-адреса не перекрываются.
...
* NetApp ONTAP (кластеризованный режим 9.4, 9.3, 9.2, 9.1P1 8.0-8.3) и (7-Mode 7.*, 8.0-8.3);
  ...
  > [!NOTE]
  > Azure NetApp Files в настоящее время не поддерживается.
...
[Уровни доступа](https://docs.microsoft.com/azure/storage/blobs/storage-blob-storage-tiers#cool-access-tier) предоставляют дополнительные сведения о том, когда может быть целесообразно использовать хранилище "холодного уровня" в качестве вфкст Core фильтр.
