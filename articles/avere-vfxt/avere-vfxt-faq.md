...
Avere vFXT для кластера Azure использует виртуальные машины E32s_v3 или D16s_v3 Microsoft Azure. 
...
Да. Avere vFXT выполняется исключительно в качестве кластера высокого уровня доступности, поэтому для работы требуется несколько IP-адресов. Если кластер находится в собственной подсети, вы можете избежать конфликтов IP-адресов, которые могут привести к проблемам с установкой и перебоям в работе. Подсеть кластера может находиться в имеющейся виртуальной сети при условии, что IP-адреса не перекрываются.
