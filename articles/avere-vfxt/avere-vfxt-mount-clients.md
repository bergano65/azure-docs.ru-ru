...
> [!NOTE]
> Если при создании кластера Avere vFXT не был создан новый контейнер Blob, добавьте системы хранения, описанные в [хранении,](avere-vfxt-add-storage.md) прежде чем пытаться смонтировать клиентов.
...
Команда крепления имеет три элемента:

* vFXT путь - сочетание IP-адреса и пути пересечения пространства имен на кластере 9описано ниже)
* локальный путь (путь в клиенте);
* параметры команды крепления - перечисленные в [аргументах команды Маунт](#mount-command-arguments)
...
Если ваш кластер был создан с помощью хранилища Blob, путь пространства имен к этому контейнеру`/msazure`

Например, ``mount 10.0.0.12:/msazure /mnt/vfxt``.

При добавлении хранилища после создания кластера путь соединения пространства имен — это значение, установленное в **пути Namespace** при создании соединения. Например, если вы использовали ``/avere/files`` в качестве пути к пространству имен, ваши клиенты подключат путь *IP-адрес*:/avere/files к своей локальной точке подключения.
...
После установки клиентов их можно использовать для копирования данных в новый контейнер для хранения данных Blob в кластере. Если вам не нужно заполнять новые хранилища, прочитайте другие ссылки, чтобы узнать о дополнительных задачах настройки:

* [Перемещение данных в ядро кластера файлера](avere-vfxt-data-ingest.md) - Как использовать несколько клиентов и потоков для эффективной загрузки данных в новый основной файлер
