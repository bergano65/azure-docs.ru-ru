...
1. Создание кластера Avere vFXT 
   ...
   * Создание *контроллера кластера*.  
     ...
     При создании новой виртуальной сети во время развертывания контроллер будет иметь общедоступный IP-адрес. Это означает, что контроллер может служить узлом переходов для подключения к кластеру извне виртуальной сети.
