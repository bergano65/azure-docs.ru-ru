...
В какой-то момент жизненного цикла кластера Avere vFXT для кластера Azure может потребоваться добавить кластерные узлы или запустить или перезагрузить кластер. Когда проект будет завершен, вам нужно будет знать, как остановить кластер и удалить его навсегда.

В этой статье объясняется, как добавить или удалить кластерные узлы и другие основные операции кластера. Если необходимо изменить настройки кластера или контролировать его работу, используйте [панель управления Avere.](avere-vfxt-cluster-gui.md)

В зависимости от задачи управления может потребоваться использовать один из трех различных инструментов: панель управления Avere, vfxt.py сценарий управления кластером командной строки и портал Azure.
...
На портале Azure *остановленный* узел (который может быть перезапущен) отображает состояние, **остановленное** на портале Azure. *Удаленный* узла показывает **состояние остановленного (разложено)** и больше не несет платы за вычисления или хранение.
...
Панель управления Avere отопроводит приоритеты целостности данных, поэтому она пытается записывать измененные данные в резервное хранилище перед возможной разрушительной операцией. Это делает его более безопасным вариантом, чем портал Azure.
...
Когда кластер начинает выключаться, он публикует сообщения состояния на вкладку **панели мониторинга.** Через несколько мгновений сообщения останавливаются, и в конечном итоге сессия Панели управления Avere прекращает отвечать, что означает, что кластер выключен.
...
Как и панель управления Avere, операции vfxt.py пытаются обеспечить постоянное хранение измененных данных во внутреннем хранилище перед завершением работы или уничтожением кластера или узла. Это делает его более безопасным вариантом, чем портал Azure.
...
* Название группы ресурсов для кластера, а также для сетевых и хранилищных ресурсов, если они не находятся в той же группе ресурсов, что и кластер
* расположение кластера;
* сеть и подсеть кластера;
* Роль доступа к кластерным узлам (использовать встроенную роль [Оператора Avere)](../role-based-access-control/built-in-roles.md#avere-operator)
...
Опция ``--quick-destroy`` может быть использована, если не требуется сохранять измененные данные из кэша кластера.
...
Если вы создали дополнительные ресурсы специально для кластера vFXT, может потребоваться удалить их в процессе удаления кластера. Не уничтожайте элементы, содержащие нужные данные, или любые элементы, которыми делятся другие проекты.
...
* Виртуальные сети
* Контейнеры для хранения и учетные записи хранения данных **(только** если они не содержат важных данных)
* Группа доступности
