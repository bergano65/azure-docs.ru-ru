...
* **Учетная запись Хранения озера данных Gen1**. Следуйте инструкциям в [Get started с Azure Data Lake Storage 1 с помощью портала Azure.](data-lake-store-get-started-portal.md) Предположим, у вас есть учетная запись Data Lake Storage 1-го поколения **myadlsg1**. 
...
2. В области слева щелкните **Задания Stream Analytics**, а затем нажмите кнопку **Добавить**.
    ...
    ![Создание вакансии по аналитике потоков](./media/data-lake-store-stream-analytics/create.job.png "Создание задания Stream Analytics")
...
1. Для выполнения задания Stream Analytics необходимо выполнить запрос из вкладки **«Запрос».** Для этого руководства вы можете запустить запрос образца, заменив заполнителей входными и выходными псевдонимами, как показано на снимке экрана ниже.
