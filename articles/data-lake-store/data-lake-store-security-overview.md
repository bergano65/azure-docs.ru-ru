...
| Владелец |All |All |Владелец — это привилегированный пользователь. Он может управлять всем и имеет полный доступ к данным. |
...
![Параметры брандмауэра и доступ по IP-адресу](./media/data-lake-store-security-overview/firewall-ip-access.png "Настройки брандмауэра и IP-адрес")

Тегов службы поддержки виртуальных сетей Azure (VNet) для Data Lake Gen 1. Тег службы представляет собой группу префиксов IP-адресов из данной службы Azure. Корпорация Майкрософт управляет адресными префиксами, охватываемыми тегом службы, и автоматически обновляет тег службы при изменении адресов. Для получения дополнительной информации смотрите [обзор тегов службы Azure](../virtual-network/service-tags-overview.md).
...
![Шифрование в хранении данных озера Gen1](./media/data-lake-store-security-overview/adls-encryption.png "Шифрование в хранении данных озера Gen1")
...
Вы можете включить аудит доступа к данным и диагностическую систему на портале Azure и отправить журналы в учетную запись хранения Azure Blob, концентратор событий или журналы Azure Monitor.
