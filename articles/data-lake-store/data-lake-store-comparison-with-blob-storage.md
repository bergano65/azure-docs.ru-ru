...
|  | Хранилище Azure Data Lake Storage 1-го поколения | хранилище BLOB-объектов Azure |
| --- | --- | --- |
...
| Операции с данными — аудит |Доступен. Смотрите [здесь](data-lake-store-diagnostic-logs.md) для получения информации. |Доступно |
...
| Ограничения размера |Нет ограничений на размер учетных записей, размер и количество файлов |Для конкретных ограничений см. [целевые показатели масштабируемости для стандартных учетных записей хранения данных](../storage/common/scalability-targets-standard-account.md) и [целевые показатели масштабируемости и производительности для хранения Blob.](../storage/blobs/scalability-targets.md) Чтобы увеличить предельные значения ограничений для учетной записи, обратитесь в службу [поддержки Azure](https://azure.microsoft.com/support/faq/) |
| Геоизбыточность |Локальная избыточность (несколько копий данных в одном регионе Azure) |Локальная избыточность (LRS), избыточность в пределах зоны (ZRS), глобальная избыточность (GRS), геоизбыточность с доступом на чтение (RA-GRS). Дополнительную информацию можно узнать [здесь](../storage/common/storage-redundancy.md) |
| Состояние службы |Общедоступная версия |Общедоступная версия |
| Доступность по регионам |Смотрите [здесь](https://azure.microsoft.com/regions/#services) |Доступны во всех регионах Azure |
