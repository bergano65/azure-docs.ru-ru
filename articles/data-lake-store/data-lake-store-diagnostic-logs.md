...
| operationName |Строковый тип |Имя операции, добавленной в журнал. Например, getfilestatus. |
...
| correlationId |Строковый тип |Идентификатор журнала, который можно использовать для формирования набора связанных записей журнала. |
...
| operationName |Строковый тип |Имя операции, добавленной в журнал. Например, getfilestatus. |
| resultType |Строковый тип |Состояние операции. Например, 200. |
| resultSignature |Строковый тип |Дополнительные сведения об операции. |
| correlationId |Строковый тип |Идентификатор журнала, который можно использовать для формирования набора связанных записей журнала. |
| удостоверение |Объект |Идентификатор, для которого был создан журнал. |
| properties |JSON |Дополнительные сведения см. ниже. |
...
| Имя | Тип | Описание |
| --- | --- | --- |
| StreamName |Строковый тип |Путь выполнения операции. |

## <a name="samples-to-process-the-log-data"></a>Примеры обработки данных журнала
При отправке журналов из Azure Data Lake Storage 1-го поколения в журнал Azure Monitor (см. статью [Просмотр или анализ данных, собранных с помощью Azure Monitor журналов Поиск](../azure-monitor/learn/tutorial-viewdata.md) сведений об использовании журналов Azure Monitor). следующий запрос возвращает таблицу, содержащую список отображаемых имен пользователей, время событий и количество событий для времени события вместе с визуальной диаграммой. Его можно легко изменить, чтобы отображался идентификатор GUID для пользователя или другие атрибуты:
