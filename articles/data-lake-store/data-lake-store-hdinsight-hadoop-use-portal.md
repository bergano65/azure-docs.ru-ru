---
title: Создание кластеров Azure HDInsight с помощью Data Lake Storage Gen1 - портал
...
* **Учетная запись Хранения озера данных Gen1**. Следуйте инструкциям в статье [Начало работы с Azure Data Lake Storage 1-го поколения с помощью портала Azure](data-lake-store-get-started-portal.md). В учетной записи также необходимо создать корневую папку.  В этой статье используется корневая папка под названием __/кластеры.__
* **Субъект-служба Azure Active Directory**. В этом руководстве содержится инструкция по созданию основного сервиса в Active Directory Azure (Azure AD). Чтобы создать субъект-службу, необходимо быть администратором Azure AD. Если вы администратор, вы можете пропустить это условие и продолжить.

>[!NOTE]
>Создать основной службы можно только в том случае, если вы являетесь администратором Azure AD. Администратор Azure AD должен сначала создать субъект-службу, после чего вы сможете создать кластер HDInsight, использующий Data Lake Storage 1-го поколения. При создании субъекта-службы также необходимо использовать сертификат, как описано в разделе [Создание субъекта-службы с использованием сертификата](../active-directory/develop/howto-authenticate-service-principal-powershell.md#create-service-principal-with-self-signed-certificate).
>

## <a name="create-an-hdinsight-cluster"></a>Создание кластера HDInsight

В этом разделе вы создадите кластер HDInsight с учетными записями Data Lake Storage 1-го поколения в качестве хранилища по умолчанию или дополнительного хранилища. Эта статья посвящена только части настройки учетных записей Data Lake Storage Gen1. Общие сведения о создании кластеров и соответствующие процедуры см. в статье [Создание кластеров Hadoop в HDInsight](../hdinsight/hdinsight-hadoop-provision-linux-clusters.md).
...
Для создания кластера HDInsight с учетной записью Data Lake Storage Gen1 в качестве учетной записи хранилища по умолчанию:
...
Для создания кластера HDInsight с учетной записью Data Lake Storage Gen1 в качестве дополнительной учетной записи для хранения данных:
...
3. В колонке **Хранилище** в разделе **Тип первичного хранилища** выберите **Хранилище Azure**, а затем введите указанные ниже сведения.
    ...
    * **Метод выбора** - Указать учетную запись хранения, которая является частью подписки Azure, выберите **мои подписки,** а затем выберите учетную запись хранилища. Чтобы указать учетную запись хранения, не входящую в вашу подписку Azure, выберите **Ключ доступа**, а затем введите данные внешней учетной записи хранения.

    * **Контейнер по умолчанию** - Используйте либо значение по умолчанию, либо укажите свое имя.
    * **Дополнительные учетные записи хранения** - Добавить больше учетных записей хранения Azure в качестве дополнительного хранилища.
    * **Доступ к магазину Data Lake Store** - Налаживание доступа между учетной записью Data Lake Storage Gen1 и кластером HDInsight. Для получения инструкций [см.](#configure-data-lake-storage-gen1-access)
...
Чтобы создать основной сервис с портала Azure:

1. Выберите **доступ в магазин Data Lake Store** из лезвия хранилища.
1. На лезвии **доступа Data Lake Storage Gen1** выберите Создать **новый**.
1. Выберите **основной сервис,** а затем следуйте инструкциям по созданию основного обслуживания.
...
1. Выберите **Access** для настройки доступа к папке.  См. раздел [Настройка разрешений для файлов](#configure-file-permissions).

Чтобы воспользоваться существующим принципом обслуживания с портала Azure:

1. Выберите **доступ к магазину Data Lake Store**.
1. На лезвии **доступа Data Lake Storage Gen1** выберите **Существующий вариант использования.**
1. Выберите **основной сервис,** а затем выберите основной службы.
...
Конфигурация отличается в зависимости от того, используется ли учетная запись в качестве хранилища по умолчанию или дополнительной учетной записи хранилища:
...
Назначить разрешение на корневом уровне учетной записи Data Lake Storage Gen1:

1. На лезвии **доступа Data Lake Storage Gen1** выберите **Access**. Откроется колонка **Выбор разрешений для файла**. В ней отображены все учетные записи Data Lake Storage 1-го поколения в вашей подписке.
1. Наведите указатель мыши (но не щелкайте) на имя учетной записи Data Lake Storage 1-го поколения, чтобы отобразился флажок, а затем установите этот флажок.
   ...
   По умолчанию, __READ__, __WRITE__, и __EXECUTE__ все выбраны.

1. В нижней части страницы щелкните **Выбрать**.
1. Выберите **Выполнить** для присвоения разрешения.
1. Выберите **готовое**.

Чтобы назначить разрешение на уровне корневой группы HDInsight:

1. На лезвии **доступа Data Lake Storage Gen1** выберите **Access**. Откроется колонка **Выбор разрешений для файла**. В ней отображены все учетные записи Data Lake Storage 1-го поколения в вашей подписке.
1. Из лезвия **разрешений файлов Select** выберите имя учетной записи Data Lake Storage Gen1, чтобы показать его содержимое.
...
![Назначить основные разрешения службы кластеру HDInsight](./media/data-lake-store-hdinsight-hadoop-use-portal/hdi.adl.3-1.png "Назначить основные разрешения службы кластеру HDInsight")

## <a name="verify-cluster-setup"></a><a name="verify-cluster-set-up"></a>Проверка настройки кластера

После завершения настройки кластера проверьте результат в колонке кластера, выполнив одно или оба указанных ниже действия.

* Чтобы убедиться, что связанное хранилище для кластера — это указанная вами учетная запись Data Lake Storage Gen1, выберите **учетные записи хранилища** в левом стеле.
...
* Чтобы убедиться, что основной сервис правильно связан с кластером HDInsight, выберите **доступ Data Lake Storage Gen1** в левом стеле.
...
После настройки кластера с data Lake Storage Gen1 в качестве хранилища смотрите следующие примеры использования кластера HDInsight для анализа данных, хранящихся в Data Lake Storage Gen1.
...
Кластер Spark можно использовать для выполнения заданий Spark с данными, хранящимися в Data Lake Storage 1-го поколения. Для получения дополнительной информации [см.](../hdinsight/spark/apache-spark-use-with-data-lake-store.md)
