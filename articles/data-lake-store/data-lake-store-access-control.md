...
|            |    Файл     |   Папка |
|------------|-------------|----------|
| **Разрешение на чтение (R)** | Чтение содержимого файла | Требуется **прочитать** и **выполнить,** чтобы перечислить содержимое папки|
...
Когда создается файл или папка, значение umask используется для изменения того, как списки ACL по умолчанию устанавливаются для дочернего элемента. umask — это 9-битное значение на родительских папках, которое содержит значение RWX для **владения пользователем,** **группы владельцев**и **других.**

Umask для хранения azure Data Lake Storage1 — это постоянное значение, установленное для 007. Это значение преобразуется в
...
> [!NOTE]
> Для удаления файлов в папках не требуются разрешения на запись. Кроме того, корневая папка "/" **никогда не** может быть удалена.
>
>
