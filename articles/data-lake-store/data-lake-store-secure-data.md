...
2. В колонке учетной записи Data Lake Storage 1-го поколения щелкните **Управление доступом (IAM)**. В колонке по умолчанию перечисляются владельцы подписки под заголовком "Владелец".
    ...
    ![Назначить группу безопасности учетной записи Azure Data Lake Storage Gen1](./media/data-lake-store-secure-data/adl.select.user.icon1.png "Назначить группу безопасности учетной записи Azure Data Lake Storage Gen1")
...
1. В колонке учетной записи Data Lake Storage 1-го поколения щелкните **Azure Data Explorer**.
    ...
    ![Просмотр данных через Data Explorer](./media/data-lake-store-secure-data/adl.start.data.explorer.png "Просмотр данных через Data Explorer")
2. В колонке **Обозреватель данных** выберите папку, для которой нужно настроить список управления доступом, а затем нажмите кнопку **Доступ**. Чтобы назначить списки управления доступом для файла, выберите файл для предварительного просмотра и щелкните **Доступ** из колонки **Предварительный просмотр файла**.
    ...
    ![Установка ACL в файловой системе хранения данных Озера Gen1](./media/data-lake-store-secure-data/adl.acl.1.png "Установка ACL в файловой системе хранения данных Озера Gen1")
...
4. Щелкните значок **Добавить**, чтобы открыть колонку **Назначение разрешений**. В этой колонке нажмите **Выберите пользователя или группу**, а затем в колонке **Выберите пользователя или группу** найдите созданную ранее группу безопасности в Azure Active Directory. Если у вас много групп для поиска, воспользуйтесь текстовым полем в верхней части для фильтрации по имени группы. Выберите группу, которую необходимо добавить, и нажмите кнопку **Выбрать**.
    ...
    ![Добавить группу](./media/data-lake-store-secure-data/adl.acl.3.png "Добавление группы")
...
![Параметры брандмауэра и доступ по IP-адресу](./media/data-lake-store-secure-data/firewall-ip-access.png "Настройки брандмауэра и IP-адрес")
...
1. В колонке учетной записи Data Lake Storage 1-го поколения щелкните **Управление доступом (IAM)**. 
    ...
    ![Назначить группу безопасности учетной записи Data Lake Storage Gen1](./media/data-lake-store-secure-data/adl.select.user.icon.png "Назначить группу безопасности учетной записи Data Lake Storage Gen1")
...
1. В колонке учетной записи Data Lake Storage 1-го поколения щелкните **Azure Data Explorer**.
    ...
    ![Создание каталогов в учетной записи Data Lake Storage Gen1](./media/data-lake-store-secure-data/adl.start.data.explorer.png "Создание каталогов в учетной записи Data Lake Storage Gen1")
