...
Чтобы отделить сервер обработки от пула запросов, используйте [Set-азаналисиссервицессервер](https://docs.microsoft.com/powershell/module/az.analysisservices/set-azanalysisservicesserver). Укажите необязательный `-DefaultConnectionMode` параметр для использования `Readonly` .
