...
- **Скачивание и запуск программы установки.** На этом шаге устанавливается служба шлюза на компьютер организации. Войдите в Azure с помощью учетной записи своего [клиента](/previous-versions/azure/azure-services/jj573650(v=azure.100)#BKMK_WhatIsAnAzureADTenant) Azure AD. Учетные записи Azure B2B (гостевые) не поддерживаются.
...
**Ответ**: № Шлюз необходим для подключения только к локальным источникам данных.

**Вопрос**: Нужно ли устанавливать шлюз на том же компьютере, на котором находится источник данных? <br/>
**Ответ**: № Для шлюза требуется только возможность подключения к серверу (как правило, в пределах той же сети).
...
**Ответ**: № Шлюз использует исходящие подключения к служебной шине Azure.
...
**Ответ**: № Службе Windows требуется действительная учетная запись Windows. По умолчанию служба будет выполняться с идентификатором безопасности службы NT SERVICE\PBIEgwService.
...
**Вопрос**: Как добиться высокого уровня доступности?  
**Ответ**: Можно установить шлюз на другом компьютере и создать таким образом кластер. Дополнительные сведения см. в разделе [Кластеры высокой доступности для локального шлюза данных](https://docs.microsoft.com/power-bi/service-gateway-high-availability-clusters) в документации к Power BI Gateway.
