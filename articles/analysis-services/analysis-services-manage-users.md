...
Visual Studio подключается к Azure Analysis Services, используя Active Directory универсальную проверку подлинности с поддержкой MFA. Пользователям предлагается выполнить вход в Azure при первом развертывании. Пользователи должны войти в Azure с помощью учетной записи с разрешениями администратора сервера на соответствующем сервере. При первом входе в Azure назначается маркер. Маркер кэшируется в памяти для последующих повторного подключения.
