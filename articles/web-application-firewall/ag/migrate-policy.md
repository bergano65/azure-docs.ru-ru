---
title: Перенос политик WAF для шлюза приложений Azure
description: Узнайте, как перенести политики Брандмауэра веб-приложений Azure с помощью Azure PowerShell.
...
Этот скрипт упрощает переход от конфигурации WAF или политики WAF только для пользовательских правил, к полной политике WAF. Вы можете увидеть предупреждение на портале, в котором говорится, *что переход на политику WAF,* или вы можете захотеть новый публичный предварительный просмотр функций WAF, таких как пользовательские правила Geomatch, политика для сайтов и за URI WAF, или набор правил смягчения последствий ботов. Чтобы использовать любую из этих функций, вам нужна полная политика WAF, связанная с шлюзом приложения. 

Для получения дополнительной информации о создании новой политики WAF [см.](create-waf-policy-ag.md) Для получения информации о миграции [см.](create-waf-policy-ag.md#migrate-to-waf-policy)

## <a name="to-migrate-to-waf-policy-using-the-migration-script"></a>Переход на политику WAF с помощью скрипта миграции

Используйте следующие шаги для выполнения сценария миграции: 

1. Откройте следующее окно облакной оболочки или откройте его из нутри портала.
