---
title: Настройка политик WAF на уровне сайта с помощью PowerShell
titleSuffix: Azure Web Application Firewall
description: Узнайте, как настроить политики брандмауэра веб-приложения для каждого сайта в шлюзе приложений с помощью Azure PowerShell.
...
ms.locfileid: "77444697"
---
# <a name="configure-per-site-waf-policies-using-azure-powershell"></a>Настройка политик WAF на уровне сайта с помощью Azure PowerShell

Параметры брандмауэра веб-приложения (WAF) содержатся в политиках WAF, а также для изменения конфигурации WAF, которая изменяет политику WAF.

При сопоставлении с шлюзом приложений политики и все параметры отображаются глобально. Поэтому, если у вас есть пять сайтов за WAF, все пять сайтов будут защищены одной и той же политикой WAF. Это замечательно, если вам нужны одинаковые параметры безопасности для каждого сайта. Но вы также можете применить политики WAF к отдельным прослушивателям, чтобы разрешить настройку WAF для конкретного сайта.

Применяя политики WAF к прослушивателю, можно настроить параметры WAF для отдельных сайтов, не затрагивая изменения на каждом сайте. Наиболее конкретная политика имеет приоритет. Если имеется глобальная политика и политика для каждого сайта (политика WAF, связанная с прослушивателем), то политика для каждого сайта переопределяет глобальную политику WAF для этого прослушивателя. Другие прослушиватели без собственных политик будут затронуты только глобальной политикой WAF.
...
> * Применение политики WAF глобально, на уровне сайта и на основе URI
...
Создайте две политики WAF, одну глобальную и одну на сайт, а затем добавьте настраиваемые правила. 

Политика для каждого сайта ограничивает размер отправляемых файлов до 5 МБ. Все остальное то же самое.
...
### <a name="apply-a-per-uri-policy"></a>Применение политики для каждого URI

Чтобы применить политику на основе URI, просто создайте новую политику и примените ее к конфигурации правила пути. 
...
Вы можете использовать командлет [Get-AzPublicIPAddress](/powershell/module/az.network/get-azpublicipaddress), чтобы получить общедоступный IP-адрес шлюза приложений. Затем используйте этот IP-адрес для перелистывания в (замените 1.1.1.1, показанный ниже). 
