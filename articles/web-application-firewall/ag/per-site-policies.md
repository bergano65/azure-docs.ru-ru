---
title: Настройка политик waF на сайте с использованием PowerShell
titleSuffix: Azure Web Application Firewall
description: Узнайте, как настроить политики веб-приложений на веб-сайте на шлюзе приложений с помощью Azure PowerShell.
...
---
# <a name="configure-per-site-waf-policies-using-azure-powershell"></a>Настройка политик WAF на сайте с помощью Azure PowerShell

Параметры Web Application Firewall (WAF) содержатся в политиках WAF, и для изменения конфигурации WAF вы измените политику WAF.

При общении с шлюзом приложений политики и все параметры отражаются во всем мире. Таким образом, если у вас есть пять сайтов за WAF, все пять сайтов защищены той же политикой WAF. Это здорово, если вам нужны те же настройки безопасности для каждого сайта. Но вы также можете применить политики WAF для отдельных слушателей, чтобы обеспечить конфигурацию WAF для конкретного сайта.

Применяя политики WAF к слушателю, вы можете настроить настройки WAF для отдельных сайтов без изменений, затрагивающих каждый сайт. Самая конкретная политика имеет прецедент. Если есть глобальная политика и политика в отношении сайтов (политика WAF, связанная со слушателем), то политика в отношении одного сайта отменяет глобальную политику WAF для этого слушателя. Другие слушатели, не имеющие собственной политики, будут затронуты только глобальной политикой WAF.
...
> * Применение политики WAF глобально, на сайте и в
...
Создайте две политики WAF, одну глобальную и одну на сайте, и добавьте пользовательские правила. 

Политика в отношении одного сайта ограничивает лимит загрузки файлов до 5 МБ. Все остальное то же самое.
...
### <a name="apply-a-per-uri-policy"></a>Применение полиса для per-URI

Чтобы применить политику per-URI, просто создайте новую политику и примените ее к конфигурации правила пути. 
...
Вы можете использовать командлет [Get-AzPublicIPAddress](/powershell/module/az.network/get-azpublicipaddress), чтобы получить общедоступный IP-адрес шлюза приложений. Затем используйте этот IP-адрес, чтобы свернуться против (заменить 1.1.1.1 показано ниже). 
