...
> [!NOTE]
> В настоящее время портал Azure поддерживает только проверку HMAC для доступа к данным конфигурации приложения. Проверка подлинности Azure AD не поддерживается. Поэтому пользователям портал Azure требуется, чтобы роль *участника* получала ключи доступа ресурса конфигурации приложения. Предоставление ролей для *модуля чтения данных конфигурации приложения* или *владельца данных конфигурации приложения* не влияет на доступ через портал.
