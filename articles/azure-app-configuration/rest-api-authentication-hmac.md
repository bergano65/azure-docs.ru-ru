...
ms.locfileid: "93424415"
---
# <a name="hmac-authentication---rest-api-reference"></a>Проверка подлинности HMAC — Справочник по REST API

Запросы HTTP могут проходить проверку подлинности с помощью схемы аутентификации **HMAC-SHA256** . Эти запросы должны передаваться по протоколу TLS.
...
Значения учетных данных (также называемые "ID") и секрет (также называемое "значение") должны быть получены из экземпляра конфигурации приложения Azure, что можно сделать с помощью [портал Azure](https://portal.azure.com) или [Azure CLI](https://docs.microsoft.com/cli/azure/?view=azure-cli-latest&preserve-view=true).
...
| --------------- | ------------ |
| **Узел** | Узел Интернета и номер порта. См. раздел  [3.2.2](https://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html#sec3.2.2) |
| **Дата** | Дата и время порождения запроса. Оно не может быть больше 15 минут от текущего GMT. Значением является дата HTTP-Date, как описано в разделе [3.3.1](https://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html#sec3.3.1)
| **x-ms-date** | То же, что и ```Date``` выше. Его можно использовать, если агент не может напрямую обращаться к ```Date``` заголовку запроса, или прокси-сервер изменяет его. Если ```x-ms-date``` ```Date``` указаны и, ```x-ms-date``` имеет приоритет. |
...
| **Авторизация** | Сведения о проверке подлинности, необходимые для схемы **HMAC-SHA256** . Ниже описаны формат и сведения. |
...
|  Аргумент | Описание  |
| ------ | ------ |
| **HMAC-SHA256** | Схема авторизации _(обязательная)_ |
...
Имена заголовков HTTP-запросов, разделенных точкой с запятой, необходимые для подписания запроса. Эти заголовки HTTP также должны быть правильно предоставлены вместе с запросом. **Не используйте пробелы**.
...
HMACSHA256 хэш **строки в** кодировке Base64 с помощью ключа доступа, определенного параметром `Credential` .
...
|  Аргумент | Описание  |
| ------ | ------ |
| **HTTP_METHOD** | Имя метода HTTP в верхнем регистре, используемое с запросом. См. [раздел 9](https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html) |
|**path_and_query** | Объединение абсолютного пути URI запроса и строки запроса. См. [раздел 3,3](https://tools.ietf.org/html/rfc3986#section-3.3).
| **signed_headers_values** | Значения всех заголовков HTTP-запросов, перечисленных в **сигнедхеадерс** , разделенные точкой с запятой. Формат соответствует семантике **сигнедхеадерс** . |
...
**Причина:** ```Date``` или ```x-ms-date``` заголовок запроса больше 15 минут от текущего времени GMT.
...
**Причина:** Отсутствующий или недопустимый ```Date``` ```x-ms-date``` заголовок запроса или
...
**Причина:** Отсутствует обязательный параметр в ```Authorization``` заголовке запроса
...
**Решение:** Проверьте ```Credential``` параметр ```Authorization``` заголовка запроса и убедитесь, что он является ДОПУСТИМЫм идентификатором ключа доступа. Убедитесь, что ```Host``` Заголовок указывает на зарегистрированную учетную запись.
...
**Решение:** Убедитесь в ```String-To-Sign``` правильности. Убедитесь в правильности ```Secret``` и правильности использования (декодировать base64 перед использованием). См. раздел " **примеры** ".
