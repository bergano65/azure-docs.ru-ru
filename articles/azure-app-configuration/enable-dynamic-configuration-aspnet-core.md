...
1. Откройте файл *Program.cs* и обновите метод `CreateWebHostBuilder`, чтобы добавить метод `config.AddAzureAppConfiguration()`.
    ...
    Метод `ConfigureRefresh` используется, чтобы указать параметры, используемые для обновления данных конфигурации в хранилище конфигураций приложения при запуске операции обновления. Чтобы непосредственно запускать операцию обновления, для приложения необходимо настроить ПО промежуточного слоя, чтобы при любых изменениях выполнить обновление данных конфигурации.
...
4. Войдите на [портале Azure](https://portal.azure.com). Щелкните **Все ресурсы** и выберите экземпляр хранилища конфигураций приложений, который вы создали по инструкциям из краткого руководства.
...
6. Обновите страницу браузера, чтобы просмотреть новые параметры конфигурации. Чтобы отразить изменения, может потребоваться более одного обновления страницы браузера.
    ...
    > [!NOTE]
    > Поскольку параметры конфигурации кэшируются со стандартным сроком действия 30 секунд, любые изменения, внесенные в параметры в хранилище конфигураций приложения, будут отражены в веб-приложении только после истечения срока действия кэша.
