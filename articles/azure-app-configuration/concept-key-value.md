...
description: Общие сведения о том, как данные конфигурации хранятся в службе "Конфигурация приложений Azure"
...
ms.locfileid: "56884985"
---
# <a name="key-value-store"></a>Хранилище ключ-значение

Служба "Конфигурация приложений Azure" хранит данные конфигурации в виде пар "ключ — значение", что дает простой и гибкий механизм для представления разных параметров приложения, с которым хорошо знакомы разработчики.

## <a name="keys"></a>ключей

Ключи выполняют роль имени в парах "ключ — значение" и используются для сохранения и извлечения нужных значений. Традиционно ключи размещаются в иерархическом пространстве имен и разделяются определенным символом (например, `/` или `:`) в соответствии с некоторым соглашением, которое вы сочтете оптимальным для приложения. Конфигурация приложений всегда обрабатывает ключи целиком. Она не выполняет синтаксический анализ ключей для изучения структуры имен или применения к ним любых правил.
...
Ключи хранятся в Конфигурации приложений в формате строк Юникода с учетом регистра. Например, *app1* и *App1* в хранилище Конфигурации приложений будут разными ключами. Учитывайте это виду при использовании параметров конфигурации в приложении, так как некоторые платформы не учитывают регистр для ключей конфигурации. Например, система конфигурации ASP.NET Core обрабатывает все ключи как строки без учета регистра. Чтобы избежать непредсказуемых результатов при обращении к Конфигурации приложений из приложения ASP.NET Core, следует отказаться от использования ключей, которые отличаются только регистром.

В именах ключей в Конфигурации приложений можно использовать любой символ Юникода, за исключением зарезервированных символов `*`, `,` и `\`. Если вам нужно использовать зарезервированный символ, экранируйте его с помощью `\{Reserved Character}`. На размер каждой пары "ключ — значение" налагается общее ограничение в 10 тысяч символов. Сюда входят все символы ключа, значения и связанных с ними необязательных атрибутов. В пределах этого ограничения можно создать многоуровневую иерархию ключей.

### <a name="designing-key-namespaces"></a>Проектирование пространства имен ключей

Существует два принципиальных подхода к именованию ключей для данных конфигурации: плоская или иерархическая структура. С точки зрения использования в приложении эти методы очень похожи, но второй дает ряд дополнительных преимуществ.

* Более удобен для чтения. В отличие от простой длинной последовательности символов, иерархические имена ключей с разделителями воспринимаются как предложения из слов, разделенных пробелами.
...
### <a name="versioning-key-values"></a>Управление версиями для пар "ключ — значение"
...
Конфигурация приложений не выполняет автоматическое управление версиями при изменении пар "ключ — значение". Вы можете применять метки для создания нескольких версий любой пары "ключ — значение". Например, в метки можно заносить номера версий приложения или идентификаторы фиксации Git, чтобы отслеживать пары "ключ — значение" для определенной версии или сборки программного обеспечения.

В метках ключей можно использовать любой символ Юникода, за исключением зарезервированных символов `*`, `,` и `\`. Если вам нужно использовать зарезервированный символ, экранируйте его с помощью `\{Reserved Character}`.

### <a name="querying-key-values"></a>Запрос пар "ключ — значение"

Каждая пара "ключ — значение" уникально идентифицируется по сочетанию ключа и метки (которая может принимать значение `null`). Запрос пар "ключ — значение" из хранилища конфигураций выполняется по шаблонам. Хранилище конфигураций возвращает все пары "ключ — значение", которые соответствуют указанному шаблону, и все относящиеся к ним значения и атрибуты. В вызовах к REST API Конфигурации приложений можно использовать следующие шаблоны ключей:
...
Можно также добавить следующие шаблоны меток.
...
Присваиваемые ключам значения также являются строками Юникода. Для значений можно использовать любые символы Юникода. Для каждого значения можно указать опциональный **тип содержимого**, определяемый пользователем. Этот атрибут позволяет хранить дополнительные данные о значении (например, о схеме кодирования), которые помогут приложению правильно обработать это значение.

В хранилище Конфигурации приложений все данные конфигурации, в том числе все ключи и значения, шифруются при хранении и передаче. Но Конфигурацию приложений не следует считать заменой Azure Key Vault. В этой службе не следует хранить секреты приложения.

## <a name="next-steps"></a>Дополнительная информация

* [Концепция. Моментальный снимок на определенный момент времени](concept-point-time-snapshot.md)  
