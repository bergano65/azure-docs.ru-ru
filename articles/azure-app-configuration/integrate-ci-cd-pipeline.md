...
Ваше приложение может не запуститься, если оно зависит от службы "Конфигурация приложений Azure" и не может получить к ней доступ. Чтобы повысить устойчивость приложения, упакуйте данные конфигурации в файл, который развертывается вместе с приложением и загружается локально во время запуска приложения. Такой подход гарантирует, что при запуске приложение будет получать стандартные значения параметров. Эти значения будут перезаписаны более новыми изменениями из хранилища Конфигурации приложений, когда оно станет доступно.
