...
> [!NOTE]
> Функция закрытых конечных точек теперь общедоступна во всех регионах, *Кроме* Центральной Индии. В **центральном регионе Индии** конфигурация приложений Azure позволяет использовать частные конечные точки в качестве общедоступной предварительной версии. Предложения общедоступной предварительной версии позволяют клиентам экспериментировать с новыми функциями до официального выпуска.  Общедоступные предварительные версии функций и служб не предназначены для использования в рабочей среде.
...
Владельцы учетных записей служб могут управлять запросами на согласие и закрытыми конечными точками через `Private Endpoints` вкладку хранилища конфигураций в [портал Azure](https://portal.azure.com).
...
> [!IMPORTANT]
> Используйте ту же строку подключения для подключения к хранилищу конфигураций приложений с помощью частных конечных точек, как и для общедоступной конечной точки. Не подключайтесь к хранилищу, используя его `privatelink` URL-адрес поддомена.

## <a name="dns-changes-for-private-endpoints"></a>Изменения DNS для частных конечных точек

При создании частной конечной точки запись ресурса DNS CNAME для хранилища конфигурации обновляется до псевдонима в поддомене с префиксом `privatelink` . Azure также создает [закрытую зону DNS](../dns/private-dns-overview.md) `privatelink` , соответствующую поддомену, с записями ресурсов DNS a для частных конечных точек.

При разрешении URL-адреса конечной точки из виртуальной сети, в которой размещается частная конечная точка, она разрешается в частную конечную точку хранилища. При разрешении из внешней сети URL-адрес конечной точки разрешается в общедоступную конечную точку. При создании частной конечной точки открытая конечная точка отключается.
...
> [!TIP]
> При использовании пользовательского или локального DNS-сервера следует настроить DNS-сервер для разрешения имени хранилища в `privatelink` поддомене на IP-адрес частной конечной точки. Это можно сделать, делегируя `privatelink` поддомен частной зоне DNS виртуальной сети, или настроив зону DNS на DNS-сервере и добавив записи A DNS.
