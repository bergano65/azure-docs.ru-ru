...
В следующем примере показано, как отправлять сообщения электронной почты с веб-страницы, размещенной в Azure, с помощью SendGrid. Полученное приложение подскажет пользователю значения электронной почты, как показано на следующем скриншоте.

![Формат адреса электронной почты][emailform]

Полученная электронная почта будет похожа на следующий скриншот.
...
В дополнение к отправке электронной почты, emailform.jsp обеспечивает результат для пользователя; примером является следующий скриншот:
