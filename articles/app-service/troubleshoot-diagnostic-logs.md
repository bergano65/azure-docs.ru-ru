...
|введите|платформа|Место проведения|ОПИСАНИЕ|
|-|-|-|-|
...
- Приложения ASP.NET могут использовать класс [System.Diagnostics.Trace](/dotnet/api/system.diagnostics.trace) для записи информации в журнал диагностики приложений. Например,
...
| Тип журнала | Поддержка Windows | Поддержка Linux | ОПИСАНИЕ |
|-|-|-|
| аппсервицеконсолелогс | тба | Yes | Стандартный вывод и стандартная ошибка |
| аппсервицехттплогс | Yes | Yes | Журналы веб-сервера |
| аппсервицеенвиронментплатформлогс | Yes | Yes | Среда службы приложений: масштабирование, изменение конфигурации и журналы состояния|
| аппсервицеаудитлогс | Yes | Yes | Действия входа через FTP и KUDU |
| аппсервицефилеаудитлогс | тба | тба | Изменения файлов с помощью FTP и KUDU |
| аппсервицеапплогс | тба | Java SE & Tomcat | Журналы приложений |
