---
title: Вопросы и ответы о производительности приложений
description: Получите ответы на часто задаваемые вопросы о доступности, производительности и проблемах с приложениями в службе приложений Azure.
author: genlin
...
> [!NOTE]
> Некоторые из приведенных ниже рекомендаций могут работать только с службами приложений Windows или Linux. Например, службы приложений Linux по умолчанию работают в 64-разрядном режиме.
>
...
На снижение производительности приложения могут влиять несколько факторов. Подробные инструкции по устранению неполадок см. в разделе [Устранение причин низкой производительности веб-приложения в службе приложений Azure](troubleshoot-performance-degradation.md).
...
4. Выберите **загрузить дамп памяти**  >  **полный дамп**.
...
Чтобы определить причину ошибки и устранить ее, выполните указания из записи блога [Azure Web Apps: Error 403 — This web app is stopped](/archive/blogs/waws/azure-web-apps-error-403-this-web-app-is-stopped) (Ошибка 403 "Это веб-приложение остановлено" в веб-приложениях Azure).
...
Сведения о квотах и ограничениях см. в [этом разделе](../azure-resource-manager/management/azure-subscription-service-limits.md#app-service-limits). 
...
15. Выберите **инструменты**  >  **KUDU**  >  **Go**.
...
Дополнительные сведения см. в статье [Настройка веб-приложений в службе приложений Azure](configure-common.md).
...
Веб-задания предназначены для фоновой обработки. Ограничения на выполнение фоновой обработки в веб-задании отсутствуют. Дополнительные сведения о веб-заданиях см. в статье [Выполнение фоновых задач с веб-заданиями](webjobs-create.md).
...
Эта проблема исправлена в Kestrel версии 1.0.2. Эта версия входит в состав обновления ASP.NET Core 1.0.3. Чтобы устранить эту проблему, обновите зависимости приложения для использования Kestrel 1.0.2. Кроме того, эту проблему можно решить одним из двух способов, описанных в записи блога [ASP.NET Core 1.0 slow perf issues in App Service web apps](/archive/blogs/waws/asp-net-core-slow-perf-issues-on-azure-websites) (Проблемы низкой производительности ASP.NET Core 1.0 в веб-приложениях службы приложений).
...
Чтобы определить, используется ли локальный кэш, перейдите на вкладку **Параметры приложения** службы приложений. Если используется локальный кэш, параметр приложения `WEBSITE_LOCAL_CACHE_OPTION` имеет значение `Always` .
...
Дополнительные сведения об исходящих подключениях веб-приложений см. в [этой записи блога](https://www.freekpaans.nl/2015/08/starving-outgoing-connections-on-windows-azure-web-sites/).
...
Пошаговые указания по выполнению отладки веб-приложения с помощью Visual Studio см. в [этой записи блога](/archive/blogs/benjaminperkins/remote-debug-your-azure-app-service-web-app).