...
Чтобы определить, используется ли локальный кэш, перейдите на вкладку **Параметры приложения** службы приложений. Если используется локальный кэш, то параметру приложения `WEBSITE_LOCAL_CACHE_OPTION` присваивается значение `Always`.
