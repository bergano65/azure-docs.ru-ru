...
Чтобы определить, используется ли локальный кэш, перейдите на вкладку **Параметры приложения** службы приложений. Если используется локальный кэш, параметр `WEBSITE_LOCAL_CACHE_OPTION` приложения имеет значение. `Always`
