...
> * Пример кода для библиотек классов C# функций Azure аналогичен коду пакета SDK для веб-заданий, за исключением того, что в проекте пакета SDK для заданий @no__t — 1 не требуется атрибут.
...
Версия 2. *x* пакета SDK позволяет использовать собственные имена для этих строк подключения или сохранять их в других местах. Вы можете задать имена в коде, используя [`JobHostConfiguration`], как показано ниже:
...
Все исходящие HTTP-запросы, внесенные из функции с помощью @no__t – 0, проходят через `ServicePointManager`. После достижения значения, установленного в `DefaultConnectionLimit`, `ServicePointManager` начинает ставить запросы в очередь перед их отправкой. Предположим, что для ограничения `DefaultConnectionLimit` установлено значение 2 и код включает 1000 HTTP-запросов. Первоначально в ОС разрешено только два запроса. Остальные 998 поставлены в очередь, пока не появится место для них. Это означает, что время ожидания `HttpClient` может исключаться, так как оно кажется, что запрос был выполнен, но запрос никогда не отправлялся операционной системой на целевой сервер. Таким образом, вы можете столкнуться с поведением, которое не имеет смысла: локальному клиенту `HttpClient` потребуется 10 секунд, чтобы выполнить запрос, но служба будет возвращать каждый запрос за 200 мс. 

Значение по умолчанию для приложений ASP.NET — `Int32.MaxValue`. это, скорее всего, подойдет для веб-заданий, выполняющихся в базовом или более высоком плане службы приложений. Веб-заданиям обычно требуется параметр Always On и они поддерживаются только планами службы приложений "базовый" и "более высокий".

Если ваше задание WebJob работает в бесплатном или общем плане службы приложений, приложение ограничено песочницей службы приложений, которая в настоящее время имеет такое [ограничение подключений — 300](https://github.com/projectkudu/kudu/wiki/Azure-Web-App-sandbox#per-sandbox-per-appper-site-numerical-limits). При несвязанном ограничении подключений в `ServicePointManager` скорее всего будет достигнуто пороговое значение подключения "песочницы", и сайт завершит работу. В этом случае установка более низкого значения параметра `DefaultConnectionLimit`, например 50 или 100, может не допустить события выше и обеспечить достаточную пропускную способность.
...
Функции должны быть открытыми методами и должны иметь один атрибут Trigger или атрибут [`NoAutomaticTrigger`](#manual-triggers) .
...
Пакет `Microsoft.Azure.WebJobs.Extensions`, упомянутый ранее, также предоставляет специальный тип привязки, который вы можете зарегистрировать, вызвав метод `UseCore`. Эта привязка позволяет определить параметр [`ExecutionContext`] в сигнатуре функции, которая включается следующим образом:
...
* **Версия 3. *x*:** Настройте конфигурацию при вызове метода `Add<Binding>` в `ConfigureWebJobs`.
...
Чтобы добавить сопоставитель, нужно вызвать метод расширения [`ConfigureServices`] в [`HostBuilder`](/dotnet/api/microsoft.extensions.hosting.hostbuilder), как показано в следующем примере:
...
Передайте в объект `JobHost` класс `NameResolver`, как показано ниже:
...
Каждую категорию можно независимо отфильтровать с определенной [`LogLevel`](/dotnet/api/microsoft.extensions.logging.loglevel). Например, вы можете просмотреть все журналы для обработки триггера большого двоичного объекта, но только `Error` и выше для всего остального.
...
В версии 2. *x* пакета SDK для управления фильтрацией используется класс `LogCategoryFilter`. @No__t-0 имеет свойство `Default` с начальным значением `Information`, что означает, что все сообщения на уровнях `Information`, `Warning`, `Error` или `Critical` записываются, но все сообщения на уровнях `Debug` или `Trace` отфильтровываются.
