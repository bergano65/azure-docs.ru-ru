...
> * Пример кода для библиотек классов C# функций Azure подобен коду пакета SDK для веб-заданий, за исключением `FunctionName` того, что в проекте пакета SDK веб-заданий не требуется атрибут.
...
Версия 3. *x* использует стандартные ASP.NET Core API. Вызовите [`HostBuilder`](/dotnet/api/microsoft.extensions.hosting.hostbuilder) метод для экземпляра. [`UseEnvironment`](/dotnet/api/microsoft.extensions.hosting.hostinghostbuilderextensions.useenvironment) Передайте строку с `development`именем, как в следующем примере:
...
В классе `JobHostConfiguration` имеется метод `UseDevelopmentSettings`, который включает режим разработки.  В следующем примере показано, как использовать параметры разработки. Чтобы возвратить `true` при локальном запуске, установите локальную переменную среды с `AzureWebJobsEnv` именем и значением `Development`. `config.IsDevelopment`
...
Атрибут сообщает среде выполнения о необходимости вызова функции при каждом появлении сообщения очереди `myqueue-items` в очереди. `QueueTrigger` Атрибут указывает среде выполнения использовать сообщение очереди для чтения большого двоичного объекта в контейнере *Sample-WorkItems.* `Blob` Содержимое сообщения очереди, переданное в функцию в `myQueueItem` параметре, является именем большого двоичного объекта.
...
В версии 3. *x*, привязки к хранилищу включены в `Microsoft.Azure.WebJobs.Extensions.Storage` пакет. Вызовите метод `ConfigureWebJobs`расширенияв методе, как показано ниже: `AddAzureStorage`
...
Вызовите метод `ConfigureWebJobs`расширенияв методе, как показано ниже: `AddExecutionContextBinding`
...
* **Версия 3. *x*:** Настройте конфигурацию при `Add<Binding>` вызове метода в `ConfigureWebJobs`.
* **Версия 2. *x*:** Задайте конфигурацию, задав свойства в объекте конфигурации, который передается `JobHost`в.
...
Сопоставитель добавляется путем вызова [`ConfigureServices`] метода расширения в [`HostBuilder`](/dotnet/api/microsoft.extensions.hosting.hostbuilder), как показано в следующем примере:
...
Передайте `JobHost` класс в объект, как показано ниже: `NameResolver`
...
Если необходимо выполнить некоторую работу в функции перед использованием `Queue`атрибута привязки, например `Blob`, или `Table`, можно использовать `IBinder` интерфейс.
...
* [Примеры](../azure-functions/functions-bindings-storage-queue.md#trigger---example). Примеры кода. Пример C# библиотеки классов применяется к пакету SDK для веб-заданий. Просто опустите `FunctionName` атрибут.
...
В следующем примере, если параметр `Disable_TestJob` приложения имеет `1` значение или `True` (без учета регистра), функция не будет выполняться. В этом случае среда выполнения создает сообщение журнала о том, что *функция Functions.TestJob отключена*.
...
Триггеры очередей и больших двоичных объектов автоматически запрещают функции обрабатывать сообщения очереди или большие двоичные объекты более одного раза; функции не обязательно должны быть идемпотентными.
...
Каждую категорию [`LogLevel`](/dotnet/api/microsoft.extensions.logging.loglevel)можно отфильтровать отдельно. Например, вы можете просмотреть все журналы для обработки триггера большого двоичного объекта, но только `Error` и выше для всего остального.
...
В версии 2. *x* пакета SDK для управления фильтрацией используется `LogCategoryFilter` класс. `Warning` `Debug` `Information` `Critical` `Error`Имеет свойство с начальным значением ,означающее,чтовсесообщениянауровнях,,илизаписываютсявжурнал,новсесообщенияв`Information` `Default` `LogCategoryFilter` или `Trace` уровни отфильтровываются.
...
В версии 3. *x*, вам больше не нужно очищать диск [`TelemetryClient`] при остановке узла. Система внедрения зависимостей .NET Core автоматически удаляет зарегистрированный `ApplicationInsightsLoggerProvider`, который очищает [`TelemetryClient`].

#### <a name="version-2x"></a>Версия 2. *x*

В версии 2. *x*, [`TelemetryClient`] созданный внутренним поставщиком Application Insights для пакета SDK [`ServerTelemetryChannel`](https://github.com/Microsoft/ApplicationInsights-dotnet/blob/develop/src/ServerTelemetryChannel/ServerTelemetryChannel.cs)веб-заданий. Когда конечная точка Application Insights недоступна или регулирует входящие запросы, этот канал [сохраняет запросы в файловой системе веб-приложения и позже повторно отправляет их](https://apmtips.com/blog/2015/09/03/more-telemetry-channels).
...
Если вы хотите изменить любую часть конвейера Application Insights, вы можете указать свою фабрику `ITelemetryClientFactory`, и узел будет использовать ваш класс для создания клиента [`TelemetryClient`]. Например, этот код переопределяет `DefaultTelemetryClientFactory` для изменения `ServerTelemetryChannel`свойства:
