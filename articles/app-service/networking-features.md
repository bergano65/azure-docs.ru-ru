...
В настоящее время некоторые из этих служб не поддерживаются, но возможны из ASE. Вот несколько примеров:
