...
| Ограничить доступ к моему приложению из ресурсов в виртуальной сети | Конечные точки службы </br> Среда ASE с ILB </br> Частные конечные точки |
...
| Защита приложения с помощью брандмауэра веб-приложения (WAF) | Шлюз приложений + ILB ASE </br> Шлюз приложений с частными конечными точками </br> Шлюз приложений с конечными точками службы </br> Передняя дверца Azure с ограничениями доступа |
...
Функция ограничений доступа позволяет фильтровать **Входящие** запросы на основе исходного IP-адреса. Действие фильтрации выполняется на внешних ролях, которые находятся в потоке от рабочих ролей, в которых выполняются приложения. Так как внешние роли являются вышестоящими от рабочих ролей, функции ограничений доступа можно рассматривать как защиту на уровне сети для приложений. Эта функция позволяет создать список блоков адресов allow и Deny, которые оцениваются в порядке приоритета. Она аналогична функции группы безопасности сети (NSG), которая существует в сети Azure.  Эту функцию можно использовать в ASE или в службе с несколькими клиентами. При использовании с ILB ASE можно ограничить доступ из частных блоков адресов.
...
Многоуровневое приложение — это приложение, в котором серверные приложения API могут получать доступ только с интерфейсного уровня. Существует два способа создания многоуровневого приложения. Начните с использования интеграции с виртуальной сетью для подключения клиентского веб-приложения к подсети в виртуальной сети. Это позволит веб-приложению выполнять вызовы в виртуальной сети. После подключения внешнего приложения к виртуальной сети необходимо выбрать способ блокировки доступа к приложению API.  Можно сделать следующее:
