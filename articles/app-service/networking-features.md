---
title: Функции сетей
description: Узнайте о компонентах сети в службе приложений Azure, а также о возможностях, необходимых для обеспечения безопасности или функциональности сети.
author: ccompy
...
Существует два основных типа развертывания для службы приложений Azure. Существует общедоступная служба, которая размещает планы службы приложений в категории "бесплатный", "общий", "базовый", "Стандартный", "Премиум", "категории премиум v2" и "Premiumv3". Затем имеется один клиентский Среда службы приложений (ASE), который размещает планы службы приложений изолированного SKU непосредственно в виртуальной сети Azure. Используемые функции будут зависеть от того, находится ли вы в службе с несколькими клиентами или в ASE. 
...
| Варианты использования входящих подключений | Компонент |
|---------------------|-------------------|
...
| Ограничить доступ к моему приложению из ресурсов в виртуальной сети | Конечные точки служб </br> Среда ASE с ILB </br> Частные конечные точки |
| Предоставить мое приложение на частном IP-адресе в виртуальной сети | Среда ASE с ILB </br> Частные конечные точки </br> Частный IP-адрес для входящего трафика в шлюзе приложений с конечными точками службы |
| Защита приложения с помощью брандмауэра веб-приложения (WAF) | Шлюз приложений + ILB ASE </br> Шлюз приложений с частными конечными точками </br> Использование Шлюза приложений с конечными точками служб </br> Передняя дверца Azure с ограничениями доступа |
...
| Варианты использования исходящих подключений | Компонент |
|---------------------|-------------------|
...
| Доступ к ресурсам через каналы ExpressRoute | интеграция с виртуальной сетью; </br> ASE | 
...
Единицы масштабирования службы приложений Azure поддерживают множество клиентов в каждом развертывании. Планы бесплатных и общих номеров SKU размещают рабочие нагрузки клиентов на рабочих процессах с несколькими клиентами. Базовые и вышеуказанные планы размещают рабочие нагрузки клиентов, предназначенные только для одного плана службы приложений (ASP). Если у вас есть стандартный план службы приложений, все приложения в этом плане будут выполняться в одном рабочем процессе. При горизонтальном масштабировании рабочей роли все приложения в этой среде ASP будут реплицированы на новый рабочий процесс для каждого экземпляра в ASP. 

#### <a name="outbound-addresses"></a>Исходящие адреса

Рабочие виртуальные машины разбиваются в большую часть по тарифам службы приложений. Свободные, общие, базовые, стандартные и Premium используют один и тот же тип рабочей машины. Категории премиум v2 находится на другом типе виртуальной машины. Premiumv3 — это еще один тип виртуальной машины. При каждом изменении семейства виртуальных машин существует другой набор исходящих адресов. При масштабировании с Standard на категории премиум v2 исходящие адреса изменятся. При масштабировании с категории премиум v2 на Premiumv3 исходящие адреса изменятся. Существуют более старые единицы масштабирования, которые будут изменять как входящие, так и исходящие адреса при масштабировании от Standard до категории премиум v2. Существует несколько адресов, используемых для выполнения исходящих вызовов. Исходящие адреса, используемые приложением для выполнения исходящих вызовов, перечислены в свойствах приложения. Эти адреса являются общими для всех приложений, работающих в одном семействе рабочих виртуальных машин в этом развертывании службы приложений. Если вы хотите просмотреть все возможные адреса, которые приложение может использовать в этой единице масштабирования, существует еще одно свойство с именем Поссиблеаутбаундаддрессес, в котором они будут перечислены. 

![Свойства приложения](media/networking-features/app-properties.png)

Служба приложений имеет ряд конечных точек, которые используются для управления службой.  Эти адреса публикуются в отдельном документе и также находятся в теге службы Аппсервицеманажемент IP. Тег Аппсервицеманажемент используется только с Среда службы приложений, где необходимо разрешить такой трафик. Входящие адреса службы приложений записываются в теге AppService IP Service. Отсутствует тег службы IP-адресов, содержащий исходящие адреса, используемые службой приложений. 
...
Вы можете узнать, как задать адрес в приложении с помощью руководства по [добавлению сертификата TLS/SSL в службу приложений Azure][appassignedaddress]. 

### <a name="access-restrictions"></a>Ограничения доступа 

Функция ограничений доступа позволяет фильтровать **Входящие** запросы. Действие фильтрации выполняется на внешних ролях, которые находятся в потоке от рабочих ролей, в которых выполняются приложения. Так как внешние роли являются вышестоящими от рабочих ролей, функции ограничений доступа можно рассматривать как защиту на уровне сети для приложений. Эта функция позволяет создать список разрешенных и отклоненных правил, которые оцениваются в порядке приоритета. Она аналогична функции группы безопасности сети (NSG), которая существует в сети Azure.  Эту функцию можно использовать в ASE или в службе с несколькими клиентами. При использовании с конечной точкой ILB ASE или Private можно ограничить доступ из частных блоков адресов.
> [!NOTE]
...
Функция ограничений доступа на основе IP-адресов помогает в сценариях, где необходимо ограничить IP-адреса, которые можно использовать для доступа к приложению. Поддерживаются форматы IPv4 и IPv6. Ниже перечислены варианты использования этой функции.

* Ограничение доступа к приложению из набора четко определенных адресов 
* Ограничение доступа через службу балансировки нагрузки, например на переднюю дверцу Azure

![Ограничения доступа с помощью передней дверцы](media/networking-features/access-restrictions-afd.png)

Узнайте, как включить эту функцию с помощью учебника по [настройке ограничений доступа][iprestrictions].

#### <a name="service-endpoint-based-access-restriction-rules"></a>Правила ограничения доступа на основе конечной точки службы

Конечные точки службы позволяют заблокировать **входящий** доступ к приложению, чтобы исходный адрес был получен из набора подсетей, которые вы выбрали. Эта функция работает совместно с ограничениями доступа по IP. Конечные точки службы несовместимы с удаленной отладкой. Чтобы использовать удаленную отладку в приложении, клиент не может находиться в подсети с включенными конечными точками службы. Конечные точки службы задаются в том же пользовательском интерфейсе, что и ограничения доступа по IP-адресу. Вы можете создать список разрешенных и запрещенных правил доступа, включающий в себя общедоступные адреса и подсети в виртуальных сетей. Эта функция поддерживает такие сценарии, как:
...
[Использование частных конечных точек для веб-приложения Azure][privateendpoints]

Частные конечные точки позволяют выполнять такие сценарии, как:

* Ограничить доступ к моему приложению из ресурсов в виртуальной сети 
* Предоставить мое приложение на частном IP-адресе в виртуальной сети 
* Защита приложения с помощью WAF 

Частные конечные точки предотвращают утечка данных как единственную вещь, к которой можно получить доступ через частную конечную точку — это приложение, с помощью которого она настроена. 
...
* Защита всего исходящего трафика 
...
Дополнительные сведения об этой функции см. в статье [Интеграция службы приложений с виртуальной][vnetintegration]сетью.
...
Многоуровневое приложение — это приложение, в котором серверные приложения API могут получать доступ только с интерфейсного уровня. Существует два способа создания многоуровневого приложения. Начните с использования интеграции с виртуальной сетью для подключения клиентского веб-приложения к подсети в виртуальной сети. Это позволит веб-приложению выполнять вызовы в виртуальной сети. После подключения внешнего приложения к виртуальной сети необходимо выбрать способ блокировки доступа к приложению API.  Вы можете:

* размещение приложений интерфейса и приложения API в одном ILB ASE и предоставление интерфейсного приложения в Интернете с помощью шлюза приложений
* размещение внешнего интерфейса в службе с несколькими клиентами и серверной части в ILB ASE
* размещение приложений интерфейса и приложения API в службе с несколькими клиентами

Если вы размещаете клиентское приложение и приложение API для многоуровневого приложения, вы можете:

Предоставление доступа к приложению API с частными конечными точками в виртуальной сети

![частые конечные точки приложения с двумя уровнями](media/networking-features/multi-tier-app-private-endpoint.png)
...
![защищенное приложение конечных точек службы](media/networking-features/multi-tier-app.png)

Ниже приведены компромиссы между двумя методами.

* с конечными точками службы необходимо только защитить трафик к приложению API в подсети интеграции. Это обеспечивает безопасность приложения API, но вы по-прежнему можете иметь возможность утечка данных из внешнего приложения в другие приложения в службе приложений.
* с частными конечными точками в играют две подсети. Это увеличивает сложность. Кроме того, Частная конечная точка является ресурсом верхнего уровня и расширяет возможности управления. Преимущество использования частных конечных точек заключается в том, что у вас нет возможности утечка данных. 

Любой из этих методов будет работать с несколькими внешними интерфейсами. В небольших масштабах использование конечных точек служб гораздо проще, так как вы просто включаете конечные точки службы для приложения API в подсети интеграции переднего плана. При добавлении дополнительных интерфейсных приложений необходимо настроить все приложения API, чтобы они имели конечные точки служб с подсетью интеграции. С частными конечными точками у вас больше сложностей, но при настройке частной конечной точки не нужно изменять что-либо в приложениях API. 

### <a name="line-of-business-applications"></a>бизнес-приложения;

Бизнес-приложения — это внутренние приложения, которые обычно не предоставляются для доступа из Интернета. Эти приложения вызываются из корпоративных сетей, в которых доступ можно контролировать. Если вы используете ILB ASE, вы легко размещаете бизнес-приложения. При использовании службы с несколькими клиентами можно использовать частные конечные точки или конечные точки служб, Объединенные с шлюзом приложений. Существует две причины использования шлюза приложений с конечными точками службы вместо частных конечных точек.

* требуется WAF защита бизнес-приложений
* Вы хотите распределить нагрузку между несколькими экземплярами бизнес-приложений

Если ни один из вариантов не подходит, лучше использовать частные конечные точки. Если в службе приложений доступны частные конечные точки, вы можете предоставлять свои приложения на частных адресах в виртуальной сети. Частная конечная точка, размещенная в виртуальной сети, может быть достигнута через подключения ExpressRoute и VPN. При настройке частных конечных точек ваши приложения будут предоставляться по частному адресу, но необходимо настроить DNS таким образом, чтобы они достигли этого адреса из локальной среды. Чтобы выполнить эту работу, необходимо перенаправить Azure DNS частную зону, содержащую частные конечные точки, на локальные DNS-серверы. Azure DNS частные зоны не поддерживают переадресацию зон, но их можно поддерживать с помощью DNS-сервера для этой цели. Этот шаблон, [DNS-сервер пересылки](https://azure.microsoft.com/resources/templates/301-dns-forwarder/), упрощает пересылку Azure DNS частной зоны на локальные DNS-серверы.

## <a name="app-service-ports"></a>Порты службы приложений

При сканировании службы приложений вы увидите несколько портов, доступных для входящих подключений. Невозможно заблокировать или контролировать доступ к этим портам в многоклиентской службе. Предоставляются следующие порты:
