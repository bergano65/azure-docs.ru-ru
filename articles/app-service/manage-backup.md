...
description: Узнайте, как создавать резервные копии приложений в службе приложений Azure. Запуск ручного или запланированного резервного копирования. Настраивайте резервные копирования, включив прилагаемую базу данных.
...
ms.locfileid: "81535763"
---
# <a name="back-up-your-app-in-azure"></a>Резервное копирование приложения в Azure
Функция архивации и восстановления в [службе приложений Azure](overview.md) позволяет легко создавать резервные копии приложений вручную или по расписанию. Можно настроить резервные отправления, которые будут сохранены до неопределенного периода времени. Вы можете восстановить приложение до моментального снимка предыдущего состояния, перезаписав существующее приложение или восстановив другое.
...
* Резервное копирование базы данных Azure, включенной TLS для MyS'L, не поддерживается. Если резервное копирование настроено, резервные копии будут неисправными.
* Резервное копирование tLS включено Лазурная база данных для PostgreS'L не поддерживается. Если резервное копирование настроено, резервные копии будут неисправными.
...
2. На странице **резервного копирования** **не настроена настройка резервного копирования. Нажмите здесь, чтобы настроить резервную подготовку для вашего приложения.**

    ![Щелкните Настроить.](./media/manage-backup/configure-start.png)

3. На странице **конфигурации резервного копирования** нажмите **Кнопка Хранилище, не настроенная** для настройки учетной записи хранилища.
...
7. На странице **резервного копирования** нажмите **кнопку «Резервное копирование».**
...
2. Налажить график резервного копирования по желанию и выбрать **OK.**
...
> [!NOTE]
> Индивидуальные базы данных в резервной копировании могут быть 4 ГБ макс, но общий максимальный размер резервного копирования составляет 10 ГБ
...
Вы можете получить доступ к файлам, перемещаясь в `https://<app-name>.scm.azurewebsites.net/DebugConsole`. Если отобразится запрос на вход в учетную запись Azure, выполните его.
...
Отправьте файл `_backup.filter` в каталог `D:\home\site\wwwroot\` своего сайта с помощью протокола [FTP](deploy-ftp.md) или любым другим способом. При желании можно создать файл непосредственно `DebugConsole` с помощью Kudu и вставить содержимое.
