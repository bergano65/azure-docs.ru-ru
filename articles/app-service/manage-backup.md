...
* Резервное копирование базы данных Azure с поддержкой TLS для MySQL не поддерживается. Если резервное копирование настроено, резервные копии будут неисправными.
* Резервное копирование базы данных Azure для PostgreSQL с поддержкой TLS не поддерживается. Если резервное копирование настроено, резервные копии будут неисправными.
...
Доступ к файлам можно получить, перейдя по `https://<app-name>.scm.azurewebsites.net/DebugConsole`адресу. Если отобразится запрос на вход в учетную запись Azure, выполните его.
