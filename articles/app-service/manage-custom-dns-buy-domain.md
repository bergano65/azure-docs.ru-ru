...
> [!NOTE]
> Небезопасная метка для личного домена означает, что он еще не привязан к SSL-сертификату, а любой запрос HTTPS из браузера в личный домен получит ошибку или предупреждение в зависимости от браузера. Сведения о настройке привязки SSL см. в статье [приобретение и Настройка SSL-сертификата для службы приложений Azure](web-sites-purchase-ssl-web-site.md).
>
...
Перейдите по указанным именам узлов в браузере. В примере на предыдущем снимке экрана попробуйте перейти к _abc.kontoso.net_.
...
* Если автоматическое продление включено: В день после истечения срока действия домена Azure попытается выставить счет за продление доменного имени.
