---
title: Приобретение личного доменного имени в Службе приложений Azure
...
Сведения о ценах на домены службы приложений Azure, посетите [странице цен на службу приложений](https://azure.microsoft.com/pricing/details/app-service/windows/) и прокрутите вниз до домена службы приложений.
...
|-|-|-|
|Защита личных сведений | Включить | Дайте согласие на защиту личных сведений, которая входит в стоимость покупки _бесплатно_. Некоторыми доменами верхнего уровня управляют регистраторы, которые не поддерживают защиту личных сведений. Они перечислены на странице **Защита личных сведений**. |
| Назначить имена узлов по умолчанию | **www** и **\@** | При необходимости выберите нужные привязки имен узлов. После завершения операции покупки домена доступ к приложению может осуществляться с помощью выбранных имен узлов. Если приложение находится за [диспетчером трафика Azure](https://azure.microsoft.com/services/traffic-manager/), вы не увидите пункт назначения корневого домена (@), так как диспетчер трафика не поддерживает записи типа A. После завершения приобретения домена можно внести изменения в назначения имен узлов. |
...
> [!NOTE]
> Домены службы приложений используют GoDaddy для регистрации домена и Azure DNS для размещения доменов. Помимо платы за регистрацию домена взимается плата за использование Azure DNS. Дополнительные сведения см. на странице [цен на Azure DNS](https://azure.microsoft.com/pricing/details/dns/).
>
>
...
Если вы решили не назначить один или несколько имен узлов по умолчанию для приложения во время покупки, или необходимо назначить имя узла не указан, имя узла можно назначить в любое время.
...
Чтобы вручную обновить домен, нажмите кнопку **Продление домена**. Тем не менее, эта кнопка не активна до [90 дней до истечения срока действия домена](#when-domain-expires).

Если ваш продление домена прошла успешно, вы получите уведомление по электронной почте в течение 24 часов.

## <a name="when-domain-expires"></a>После истечения срока действия домена

Azure имеет дело с истекающим сроком действия или истечения срока действия домены службы приложений следующим образом:

* Если автоматическое обновление отключено: 90 дней до истечения срока действия домена, вам отправляется электронное письмо с уведомлением обновления и **продление домена** активируется кнопка на портале.
* Если включено автоматическое продление: В день после даты окончания срока действия домена Azure пытается выставлять вам счета для обновления имени домена.
* При возникновении ошибки во время автоматического обновления (например, вашей карты истек срок действия), или если автоматическое обновление отключено и разрешить домена истекает, Azure уведомит вас об окончания срока действия домена и parks ваше доменное имя. Вы можете [вручную обновите](#renew-the-domain) вашего домена.
* В день 4 и 12 дней после истечения срока действия Azure отправляет дополнительные уведомления по электронной почте. Вы можете [вручную обновите](#renew-the-domain) вашего домена.
* На 19 день после истечения срока действия домена остается на удержании, но подпадала под действие плату активации. Вы можете вызывать службу поддержки, чтобы обновить имя домена, соблюдать все применимые обновления и активации сборов.
* 25-го после истечения срока действия Azure помещает доменом аукционного лота в службе домена имя отрасли аукциона. Вы можете вызывать службу поддержки, чтобы обновить имя домена, соблюдать все применимые обновления и активации сборов.
* На 30-й день после истечения срока действия вы больше не смогут использовать вашего домена.
