---
title: Руководство. Доступ из веб-приложения к хранилищу с помощью управляемых удостоверений | Azure
description: Из этого руководства вы узнаете, как получить доступ к службе хранилища Azure от имени приложения с помощью управляемых удостоверений.
...
ms.locfileid: "94428454"
---
# <a name="tutorial-access-azure-storage-from-a-web-app"></a>Руководство. Доступ к хранилищу Azure из веб-приложения

Узнайте, как с помощью управляемых удостоверений получить доступ к хранилищу Azure не от имени вошедшего в систему пользователя, а от имени веб-приложения, выполняемого в Службе приложений Azure.

:::image type="content" alt-text="Доступ к хранилищу" source="./media/scenario-secure-app-access-storage/web-app-access-storage.svg" border="false":::

Предположим, что вам нужен доступ из веб-приложения к плоскости данных Azure (к службе хранилища Azure, Azure SQL, Azure Key Vault или другим службам).  Вы могли бы использовать общий ключ, но это создаст дополнительные проблемы с оперативной безопасностью, то есть с распределением прав на создание, развертывание секрета и управление им.  Также этот ключ может случайно оказаться отправленным в GitHub, где злоумышленники смогут его найти. Есть более безопасный способ предоставить веб-приложению доступ к данным — применить [управляемое удостоверение](/azure/active-directory/managed-identities-azure-resources/overview). Управляемое удостоверение из Azure Active Directory позволяет службам приложений получать доступ к ресурсам через управление доступом на основе ролей (RBAC) без учетных данных приложения. Когда вы назначаете веб-приложению управляемое удостоверение, Azure создает и распространяет сертификат.  Вам не придется беспокоиться об управлении секретами или учетными данными приложения.
...
> * создать учетную запись хранения и контейнер больших двоичных объектов;
...
:::image type="content" alt-text="Системное удостоверение" source="./media/scenario-secure-app-access-storage/create-system-assigned-identity.png":::

Это действие создает новый идентификатор объекта, который отличается от идентификатора приложения, созданного в колонке **Проверка подлинности и авторизация**.  Скопируйте идентификатор объекта для управляемого удостоверения, назначаемого системой, так как он потребуется позже.

## <a name="create-a-storage-account-and-blob-storage-container"></a>Создание учетной записи хранения и контейнера больших двоичных объектов

Теперь вы готовы создать учетную запись хранения и контейнер больших двоичных объектов.
...
Чтобы создать контейнер для хранения большого двоичного объекта в службе хранилища Azure, выполните следующие действия.
...
Чтобы создать учетную запись хранения общего назначения версии 2 и контейнер для хранения большого двоичного объекта, выполните следующий скрипт. Укажите имя группы ресурсов, которая содержит веб-приложение. Введите имя учетной записи хранения. Выбранное вами имя должно быть уникальным в Azure. Также имя должно содержать от 3 до 24 символов и может состоять только из цифр и строчных букв. Укажите расположение для учетной записи хранения.  Чтобы просмотреть список доступных расположений для вашей подписки, выполните ```Get-AzLocation | select Location```. Все знаки в имени контейнера должны быть строчными. Оно должно начинаться с буквы или цифры и может содержать только буквы, цифры и дефисы (-).
...
Чтобы создать учетную запись хранения общего назначения версии 2 и контейнер для хранения большого двоичного объекта, выполните следующий скрипт. Укажите имя группы ресурсов, которая содержит веб-приложение. Введите имя учетной записи хранения. Выбранное вами имя должно быть уникальным в Azure. Также имя должно содержать от 3 до 24 символов и может состоять только из цифр и строчных букв. Укажите расположение для учетной записи хранения.  Все знаки в имени контейнера должны быть строчными. Оно должно начинаться с буквы или цифры и может содержать только буквы, цифры и дефисы (-).
...
# <a name="portal"></a>[Портал](#tab/azure-portal)
На [портале Azure](https://portal.azure.com) откройте страницу учетной записи хранения, чтобы предоставить доступ веб-приложению.  На панели навигации слева выберите **Управление доступом (IAM)** , а затем щелкните **Назначение ролей**.  Вы увидите список субъектов, которым разрешен доступ к учетной записи хранения.  Теперь вам нужно добавить назначение роли для робота, то есть службы приложений, которая будет обращаться к этой учетной записи хранения.  Выберите **Добавить**->**Добавить назначение ролей**.

В поле **Роль** выберите **Участник для данных BLOB-объектов хранилища**, чтобы веб-приложение получило доступ на чтение данных из больших двоичных объектов хранилища.  В поле **Назначить доступ к** выберите **Служба приложений**.  В поле **Подписка** выберите нужную подписку.  Теперь выберите службу приложений, к которой вы намерены предоставить доступ.  Нажмите кнопку **Сохранить**.

:::image type="content" alt-text="Добавление назначения роли" source="./media/scenario-secure-app-access-storage/add-role-assignment.png":::
...
## <a name="access-blob-storage-net"></a>Доступ к хранилищу больших двоичных объектов (.NET)
...
Установите [пакет NuGet для хранилища больших двоичных объектов](https://www.nuget.org/packages/Azure.Storage.Blobs/), чтобы организовать работу со службой хранилища больших двоичных объектов, и [пакет NuGet клиентской библиотеки службы удостоверений Azure для .NET](https://www.nuget.org/packages/Azure.Identity/), чтобы выполнять проверку подлинности с учетными данными Azure AD.  Для установки клиентских библиотек можно использовать интерфейс командной строки .NET Core или консоль диспетчера пакетов в Visual Studio.
