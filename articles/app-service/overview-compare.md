...
Существует несколько способов размещения веб-сайтов в Azure: [Служба приложений Azure][Azure App Service], [виртуальные машины][Virtual Machines], [Service Fabric][Service Fabric] и [облачные службы][Cloud Services]. Данная статья поможет вам понять имеющиеся возможности и сделать правильный выбор веб-приложения.
...
В следующей таблице сравниваются возможности службы приложений, облачных служб, виртуальных машин и Service Fabric, что поможет вам сделать лучший выбор. Ознакомиться с текущей информацией, касающейся Соглашений об уровнях обслуживания (SLA) для каждого из вариантов, можно в разделе [Соглашения об уровнях обслуживания для Azure](https://azure.microsoft.com/support/legal/sla/).
...
| Управление автоматическим обновлением ОС |X |X | | |Реализовано частично с помощью приложения для управления исправлениями. В будущем планируется полная реализация. |
...
| Средний уровень узла многоуровневой архитектуры |X |X |X |X |В веб-приложениях службы приложений можно легко разместить средний уровень REST API, а компонент [WebJobs](https://go.microsoft.com/fwlink/?linkid=390226) может размещать задания фоновой обработки. Веб-задания можно выполнять на выделенном веб-сайте, что позволит обеспечить независимое масштабирование для этого уровня. |
| Интегрированная поддержка MySQL как службы |X |X | | | |
| Поддержка ASP.NET, классической ASP, Node.js, PHP, Python |X |X |X |X |Service Fabric поддерживает создание веб-интерфейса на основе [ASP.NET 5](../service-fabric/service-fabric-reliable-services-communication-aspnetcore.md). Также вы можете развернуть приложение любого типа (Node.js, Java и т. д.) в качестве [гостевого исполняемого файла](../service-fabric/service-fabric-guest-executables-introduction.md). |
| Масштабирование на несколько экземпляров без повторного развертывания |X |X |X |X |Виртуальные машины можно масштабировать до нескольких экземпляров, но службы, работающие на этих машинах, должны быть записаны для реализации этого масштабирования. Вам нужно настроить подсистему балансировки нагрузки для распределения запросов между компьютерами, и убедиться, что у вас есть более чем один экземпляр виртуальной машины в [группе доступности](../virtual-machines/windows/manage-availability.md). |
...
| Установка любых MSI-файлов по выбору пользователя | |X |X |X |Service Fabric позволяет разместить любой исполняемый файл как [гостевой исполняемый файл](../service-fabric/service-fabric-guest-executables-introduction.md). Более того, на виртуальных машинах можно установить любое приложение. |
