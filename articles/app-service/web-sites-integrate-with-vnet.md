...
1. Одна версия обеспечивает интеграцию с виртуальными сетями в одном регионе. Эта форма функции требуется подсеть в виртуальной сети в одном регионе
2. Другая версия поддерживает интеграцию с виртуальными сетями в других регионах или с классическими виртуальными сетями. Эта версия компонента требует развертывания шлюза виртуальной сети в виртуальной сети.
...
Эта функция доступна в предварительной версии, но она поддерживается для производственных рабочих нагрузок со следующими ограничениями:
...
* Приложения и виртуальной сети должны находиться в том же регионе
* Один адрес, используемый для каждого экземпляра плана службы приложений. Так как размер подсети невозможно изменить после назначения, используйте подсеть, которая может превышать максимальный размер масштабов. 32-разрядные адреса /27 являются рекомендуемым размером, так как они способны вместить план Службы приложений, который масштабируется до 20 экземпляров.
* Не удается удалить виртуальную сеть с помощью интеграции приложения. Сначала необходимо удалить интеграцию 

Для использования интеграции с виртуальной сетью с виртуальной сетью Resource Manager, в том же регионе:
...
Эта функция поддерживает только один виртуальный интерфейс каждой рабочей роли.  Один виртуальный интерфейс каждой рабочей роли означает, что один виртуальный интерфейс на план службы приложений. Все приложения в одном плане службы приложений можно использовать же интеграция с виртуальной сетью, но если необходимо подключиться к дополнительной виртуальной сети, необходимо создать другой план службы приложений. Виртуальный интерфейс, используемый не является ресурсом, который клиенты имеют прямой доступ к.
