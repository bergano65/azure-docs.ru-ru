...
ms.locfileid: "51262086"
---
# <a name="integrate-your-app-with-an-azure-virtual-network"></a>Интеграция приложения с виртуальной сетью Azure
В этом документе описана функция службы приложений Azure — интеграция с виртуальной сетью. Кроме того, приведены инструкции по ее настройке для приложений в [службе приложений Azure](https://go.microsoft.com/fwlink/?LinkId=529714). Если вы не знакомы с виртуальными сетями Azure, вот краткое описание. Эта функция позволяет размещать множество ресурсов Azure в сети, недоступной из Интернета, а также самостоятельно управлять доступом к ним. Эти сети можно подключать к локальным сетям с помощью разных технологий VPN. Начать изучение виртуальных сетей Azure лучше всего со статьи [Обзор виртуальной сети][VNETOverview]. 
...
Если для виртуальной сети не настроен шлюз или не активировано подключение типа "точка — сеть", это нужно сделать до интеграции. Чтобы выполнить эту задачу для классической виртуальной сети, перейдите на портал и откройте список "Виртуальные сети (классические)". Выберите сеть, интеграцию с которой вы хотите выполнить. Выберите VPN-подключения. Здесь вы можете создать VPN-подключение типа "точка — сеть" и настроить шлюз. Настройка шлюза занимает примерно 30 минут.
...
В представлении интеграции с виртуальной сетью на уровне приложения можно выполнить только одну операцию — отключить приложение от той виртуальной сети, к которой оно сейчас подключено. Чтобы отключить приложение от виртуальной, выберите команду "Отключить" в верхней части окна. Это действие не изменяет настройки виртуальной сети. Конфигурация виртуальной сети, включая настроенные шлюзы, останется неизменной. Если вы хотите удалить виртуальную сеть, необходимо сначала удалить все ее ресурсы, включая шлюзы. 

В представлении плана службы приложений доступны несколько дополнительных операций. Доступ к этому представлению осуществляется не через приложение. Чтобы открыть сетевой пользовательский интерфейс ASP, зайдите в основной пользовательский интерфейс ASP и прокрутите страницу вниз. Выберите "Состояние компонента сети" чтобы открыть пользовательский интерфейс этой сети. Выберите "Щелкните здесь для управления", чтобы перечислить интеграции виртуальной сети в ASP.
...
Это новая версия интеграции виртуальной сети, которая не зависит от VPN-подключения типа "точка — сеть". Эта новая предварительная версия. Новые возможности интеграции виртуальной сети имеют следующие характеристики.

- Новой функции требуется неиспользуемая подсеть в виртуальной сети Resource Manager
- Один адрес используется для каждого экземпляра плана Службы приложений. Так как размер подсети нельзя изменить после назначения, используйте подсеть, которая может превышать максимальный размер масштабов. 32-разрядные адреса /27 являются рекомендуемым размером, так как они способны вместить план Службы приложений, который масштабируется до 20 экземпляров.
- Вы можете использовать защищенные ресурсы конечной точки службы с помощью новой функции интеграции с виртуальной сетью. Разрешите доступ из подсети, назначенной вашему приложению, для настройки конечных точек службы.
- Доступ к ресурсам можно получить с помощью подключения ExpressRoute без дополнительной настройки.
...
- Новая возможность доступна только из единиц масштабирования для новой Службы приложений Azure. Портал сообщит об использовании приложением новой функции интеграции виртуальной сети. 
- Приложения и виртуальная сеть должны находиться в одном регионе.

Новая функция интеграции виртуальной сети доступна изначально только в регионах "Северная Европа" и "Восток США".
