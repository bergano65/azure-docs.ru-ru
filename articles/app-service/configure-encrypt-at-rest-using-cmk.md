...
Сначала [Создайте учетную запись хранения Azure](../storage/common/storage-account-create.md) и [зашифруйте ее с помощью управляемых клиентом ключей](../storage/common/storage-service-encryption.md#customer-managed-keys-with-azure-key-vault). После создания учетной записи хранения используйте [Обозреватель службы хранилища Azure](../vs-azure-tools-storage-manage-with-storage-explorer.md) для отправки файлов пакетов.
...
Когда вы отправите файл в хранилище BLOB-объектов и получите для него URL SAS, задайте для параметра приложения `WEBSITE_RUN_FROM_PACKAGE` значение URL-адрес SAS. В следующем примере он выполняется с помощью Azure CLI:
...
Теперь можно заменить значение параметра `WEBSITE_RUN_FROM_PACKAGE` приложения на Key Vault ссылку на URL-адрес в кодировке SAS. Это обеспечит шифрование URL-адреса SAS в Key Vault, что обеспечивает дополнительный уровень безопасности.

1. Используйте следующую команду [`az keyvault create`](/cli/azure/keyvault#az-keyvault-create) , чтобы создать экземпляр Key Vault.       
...
1. Используйте следующую команду [`az keyvault secret set`](/cli/azure/keyvault/secret#az-keyvault-secret-set) , чтобы добавить внешний URL-адрес в качестве секрета в хранилище ключей:   

    ```azurecli    
    az keyvault secret set --vault-name "Contoso-Vault" --name "external-url" --value "<SAS-URL>"    
    ```    

1.  Используйте следующую команду [`az webapp config appsettings set`](/cli/azure/webapp/config/appsettings#az-webapp-config-appsettings-set) , чтобы создать параметр `WEBSITE_RUN_FROM_PACKAGE` приложения со значением в качестве Key Vault ссылки на внешний URL-адрес:
...
- Запуск приложения из пакета развертывания делает `wwwroot/` только для чтения. Приложение получает сообщение об ошибке при попытке записи в этот каталог.
