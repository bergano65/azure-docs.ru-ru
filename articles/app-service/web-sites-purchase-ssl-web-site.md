...
| Имя узла незащищенного домена | Если вы укажете здесь корневой домен, вы получите сертификат, который защищает *оба домена*: и корневой домен, и домен с поддоменом `www`. Чтобы защитить только поддомен, укажите здесь полное доменное имя вместе с поддоменом (например, `mysubdomain.contoso.com`). |
| Subscription | Центр обработки данных, где размещается веб-приложение. |
| Resource group | Группа ресурсов, в которой содержится сертификат. Например, можно использовать новую группу ресурсов или ту же группу ресурсов, что и для приложения службы приложений. |
...
| Условия использования | Щелкните, чтобы подтвердить, что вы принимаете условия использования. Сертификаты, полученные из GoDaddy. |
...
| Resource group | Рекомендуется выбирать группу ресурсов, идентичную сертификату службы приложений. |
| Местоположение | Предназначено для выбора расположения, идентичного тому, где находится приложение службы приложений. |
...
## <a name="rekey-certificate"></a>Повторное создание ключей сертификата

Если вы считаете, что сертификат, закрытый ключ становится известен посторонним, вы сможете повторно создать сертификат. Выберите сертификат в [сертификаты службы приложений](https://portal.azure.com/#blade/HubsExtension/Resources/resourceType/Microsoft.CertificateRegistration%2FcertificateOrders) странице, а затем выберите **повторное создание ключей и синхронизация** в левой области навигации.

Нажмите кнопку **повторное создание ключей** для запуска процесса. Этот процесс может занять от 1 до 10 минут.
...
После завершения операции повторного создания ключей, нажмите кнопку **синхронизации**. Операции синхронизации автоматически обновляет привязки имен узлов для сертификата в службе приложений, не вызывая простоев для ваших приложений.
...
![автоматически обновить сертификат](./media/app-service-web-purchase-ssl-web-site/auto-renew.png)
...
После завершения операции обновления, нажмите кнопку **синхронизации**. Операции синхронизации автоматически обновляет привязки имен узлов для сертификата в службе приложений, не вызывая простоев для ваших приложений.
