...
| Имя узла незащищенного домена | Укажите корневой домен здесь. Выданный сертификат защищает как корневой домен, *так* и поддомен `www`. В выданном сертификате поле Common Name (общее имя) содержит корневой домен, а поле альтернативное имя субъекта содержит домен `www`. Чтобы защитить только поддомен, укажите здесь полное доменное имя вместе с поддоменом (например, `mysubdomain.contoso.com`).|
...
Чтобы создать новое хранилище или выбрать существующее, на странице **Состояние Key Vault** щелкните **Репозиторий Key Vault**. Если вы решили создать новое хранилище, воспользуйтесь следующей таблицей, чтобы настроить хранилище и нажать кнопку **создать**.
...
## <a name="rekey-certificate"></a>Сертификат смены ключей
