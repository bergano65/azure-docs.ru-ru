...
| Параметр | Описание |
|-|-|
| Название | Понятное имя сертификата службы приложений. |
| Имя узла незащищенного домена | Укажите корневой домен здесь. Выданный сертификат защищает как корневой, *так* и `www` поддомен. В выданном сертификате поле Common Name (общее имя) содержит корневой домен, а поле альтернативное имя `www` субъекта содержит домен. Чтобы защитить только поддомен, укажите здесь полное доменное имя вместе с поддоменом (например, `mysubdomain.contoso.com`).|
...
| Условия использования | Щелкните, чтобы подтвердить, что вы принимаете условия использования. Сертификаты получаются из GoDaddy. |
...
| Параметр | Описание |
|-|-|
| Название | Уникальное имя, содержащее буквы, цифры и дефисы. |
...
## <a name="verify-domain-ownership"></a>Проверить права на владение доменом
...
## <a name="rekey-certificate"></a>Повторно создать ключи для сертификата

Если вы считаете, что закрытый ключ сертификата скомпрометирован, вы можете сменить его на сертификат. Выберите сертификат на странице [Сертификаты службы приложений](https://portal.azure.com/#blade/HubsExtension/Resources/resourceType/Microsoft.CertificateRegistration%2FcertificateOrders) , а затем выберите параметр **сменить ключ и синхронизировать** в левой области навигации.

Щелкните **Сменить** ключ, чтобы запустить процесс. Этот процесс может занять от 1 до 10 минут.
...
После завершения операции смены ключей нажмите кнопку **синхронизировать**. Операция синхронизации автоматически обновляет привязки имен узлов для сертификата в службе приложений, не вызывая каких бы то ни было простоев в приложениях.

> [!NOTE]
> Если не щелкнуть **синхронизировать**, служба приложений автоматически синхронизирует сертификат в течение 48 часов.
...
![продлить сертификат автоматически](./media/app-service-web-purchase-ssl-web-site/auto-renew.png)
...
После завершения операции обновления нажмите кнопку **синхронизировать**. Операция синхронизации автоматически обновляет привязки имен узлов для сертификата в службе приложений, не вызывая каких бы то ни было простоев в приложениях.
