...
1. Выберите **параметры** > **Проверка подлинности и авторизация** в левой области и **Убедитесь, что** **Проверка подлинности службы приложений** включена.
...
1. В поле **URI перенаправления**выберите **веб** и введите URL-адрес приложения службы приложений и добавьте путь `/.auth/login/aad/callback`. Например, `https://contoso.azurewebsites.net/.auth/login/aad/callback`. 
...
1. Выберите **предоставить API** > **набор**. Вставьте URL-адрес приложения службы приложений и нажмите кнопку **сохранить**.
...
1. В **режиме управления**выберите **Дополнительно** и настройте проверку подлинности службы приложений в соответствии со следующей таблицей.
    ...
    |Идентификатор издателя| Используйте `https://login.microsoftonline.com/<tenant-id>` и замените *\<tenant-id >* **идентификатором каталога (клиента)** регистрации приложения. |
...
1. На странице **Регистрация приложения** введите **имя** для регистрации приложения.
1. В поле **URI перенаправления**выберите **общедоступный клиент (мобильный & Рабочий стол)** и введите URL-адрес приложения службы приложений и добавьте путь `/.auth/login/aad/callback`. Например, `https://contoso.azurewebsites.net/.auth/login/aad/callback`.
1. Нажмите кнопку **Создать**.
...
1. Выберите **разрешения API** > **Добавить разрешение** > **Мои интерфейсы API**.
