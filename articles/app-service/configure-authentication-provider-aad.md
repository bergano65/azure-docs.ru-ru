...
7. Теперь щелкните **URL-адреса ответа**, измените **URL-адрес ответа**, вставьте URL-адрес приложения (из шага 1), а затем добавьте его в конец URL-адреса */.AUTH/LOGIN/AAD/callback* (например, `https://contoso.azurewebsites.net/.auth/login/aad/callback`). Нажмите кнопку **Сохранить**.

   > [!NOTE]
   > Вы можете использовать одну регистрацию приложения для нескольких доменов, добавив дополнительные **URL-адреса ответа**. Убедитесь, что каждый экземпляр службы приложений моделируется собственной регистрацией, поэтому у него есть собственные разрешения и согласие. Также рекомендуется использовать отдельные регистрации приложений для отдельных слотов сайта. Это позволяет избежать совместного использования разрешений в разных средах, чтобы ошибка в новом тестируемом коде не влияла на рабочую среду.
...
2. В конфигурации аутентификации Active Directory щелкните **Дополнительно** в разделе **Режим управления**. Вставьте идентификатор приложения в поле идентификатора клиента (из шага 8) и вставьте URL-адрес (из шага 9) для URL-адреса издателя. Затем нажмите кнопку **ОК**.
