...
description: Узнайте, как настроить активную проверку подлинности Каталог azure как поставщика идентификационных данных для приложения App Service.
...
ms.locfileid: "80046443"
---
# <a name="configure-your-app-service-app-to-use-azure-ad-login"></a>Настройте приложение Службы app для использования входа в Azure AD

[!INCLUDE [app-service-mobile-selector-authentication](../../includes/app-service-mobile-selector-authentication.md)]

В этой статье показано, как настроить службу приложений Azure для использования активного каталога Azure (Azure AD) в качестве поставщика аутентификации.
...
1. Выберите раздел **Фирменная символика**. В **URL-адресе главной страницы**введите URL-адрес приложения App Service и выберите **Сохранить.**
...
1. В **режиме управления**выберите **Расширенную** и настройте аутентификацию Службы приложений в соответствии со следующей таблицей:
    ...
    |Идентификатор эмитента| Используйте `https://login.microsoftonline.com/<tenant-id>`и замените * \<>-идентификатор омнивка* клиента с **идентификатором каталога (арендатора)** регистрации приложения. |
...
Вы можете зарегистрировать нативных клиентов, чтобы разрешить аутентификацию с помощью клиентской библиотеки, такой как **Библиотека подлинности Active Directory.**
...
1. В **Перенаправлении URI**выберите **публичного клиента (мобильный & рабочий стол)** и введите URL. `<app-url>/.auth/login/aad/callback` Например, `https://contoso.azurewebsites.net/.auth/login/aad/callback`.
    ...
    > [!NOTE]
    > Для приложения Windows используйте [пакет SID](../app-service-mobile/app-service-mobile-dotnet-how-to-use-client-library.md#package-sid) в качестве URI.
1. Выберите **Создать**.
