...
Чтобы настроить управление трафиком службы приложений, создайте профиль в диспетчере трафика Azure, использующий, один из четырех нагрузки описанных выше методов распределения и затем добавить конечные точки (в данном случае служба приложений), для которых требуется для контроля трафика профиль. Сведения о состоянии приложения (запущено, остановлено или удалено) регулярно передаются в профиль. Это позволяет диспетчеру трафика Azure правильно перенаправлять трафик.
