...
Чтобы получить URL-адрес для включения локального развертывания Git для существующего приложения, выполните [`az webapp deployment source config-local-git`](/cli/azure/webapp/deployment/source#az-webapp-deployment-source-config-local-git) в Cloud Shell. Замените \<app-name> и \<group-name> именем приложения и его группой ресурсов Azure.
...
Чтобы создать приложение с поддержкой Git, выполните команду [`az webapp create`](/cli/azure/webapp#az-webapp-create) в Cloud Shell с `--deployment-local-git` параметром. Замените \<app-name> , \<group-name> и \<plan-name> именами нового приложения Git, его группой ресурсов Azure и планом службы приложений Azure.
...
1. В локальном окне терминала добавьте удаленное хранилище Azure в локальный репозиторий Git. В команде замените \<url> URL-адресом репозитория Git, полученным на предыдущем шаге.
