---
title: Локальный кэш
description: Узнайте, как локальный кэш работает в службе приложений Azure, а также как включить, изменить размер и заставить заставить задания о состоянии локального кэша приложения.
tags: optional
...
> [!NOTE]
> Локальный кэш не поддерживается в приложениях функции или контейнеризированных приложениях Службы Приложений, таких как в [контейнерах Windows](app-service-web-get-started-windows-container.md) или в [Службе приложений на Linux.](containers/app-service-linux-intro.md)
...
* При публикации нового содержимого происходит явное обновление содержимого общей папки. Вы можете сразу же просмотреть одно и то же содержимое через веб-сайт SCM и запущенное приложение (обычно некоторые технологии, такие как ASP.NET инициировать перезагрузку приложения на некоторые изменения файлов, чтобы получить последнее содержимое).
