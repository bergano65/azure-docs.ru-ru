...
* При публикации нового содержимого происходит явное обновление содержимого общей папки. Можно сразу просмотреть то же содержимое, через веб-сайт SCM и выполнения приложения (обычно некоторые технологии, такие как ASP.NET инициировать перезапуск приложения на некоторых изменениях файлов, чтобы получить обновленное содержимое).
...
* На [потоковую передачу журналов](troubleshoot-diagnostic-logs.md#streamlogs) влияет оптимальное копирование. Вы можете наблюдать задержку в потоковой передаче журналов около одной минуты.
