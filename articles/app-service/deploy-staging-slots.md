...
1. Выполните действия, описанные в разделе [Переключение слотов развертывания](#Swap) , но выберите **выполнить переключение с помощью предварительной версии**.
    ...
    ![Переключить с предварительным просмотром](./media/web-sites-staged-publishing/SwapWithPreview.png)
...
> [!NOTE]
> `<applicationInitialization>`является частью каждого запуска приложения, где эти два параметра приложения применяются только к переключениям слотов.
