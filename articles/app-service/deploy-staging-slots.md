...
Чтобы включить несколько слотов развертывания, приложение должно выполняться на уровне **Стандартный**, **Премиум** или *Изолированный*.
...
2. Выберите параметр **Слоты развертывания** и щелкните **Добавить слот**.
   ...
   > [!NOTE]
   > Если приложение не выполняется в категории **Стандартный**, **Премиум** или *Изолированный*, появится сообщение со списком поддерживаемых ценовых категорий, в которых можно использовать промежуточную публикацию. На этом этапе вы можете выбрать команду **Обновить** и перейти на вкладку **Масштаб** приложения, а затем продолжить процесс.
   > 
   > 
...
5. Щелкните URL-адрес приложения в колонке слота. Обратите внимание, что слот развертывания имеет собственное имя узла и также является динамическим приложением. Сведения о том, как запретить общий доступ к слоту развертывания, см. в статье [Веб-приложения в службе приложений — блокирование веб-доступа к непроизводственным слотам развертывания](https://ruslany.net/2014/04/azure-web-sites-block-web-access-to-non-production-deployment-slots/).

В созданном слоте развертывания изначально нет содержимого. Вы можете выполнить развертывание в область из другой ветви репозитория или вообще из другого репозитория. Вы также можете изменить конфигурацию области. Для обновлений содержимого используйте профиль публикации или учетные данные развертывания, связанные с областью развертывания.  Например, вы можете выполнить [публикацию в эту область с помощью Git](deploy-local-git.md).
...
В некоторых случаях операция переключения требует значительного времени, например, если переключаемое приложение долго подготавливается к запуску. Дополнительные сведения об операциях переключения вы можете получить из [журнала действий](../azure-monitor/platform/activity-logs-overview.md) на [портале Azure](https://portal.azure.com).
...
[Веб-приложения в службе приложений — блокирование веб-доступа к непроизводственным слотам развертывания](https://ruslany.net/2014/04/azure-web-sites-block-web-access-to-non-production-deployment-slots/)  
