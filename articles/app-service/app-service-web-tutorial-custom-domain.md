...
![Выбор служб приложений](./media/app-service-web-tutorial-custom-domain/app-services.png)
...
Чтобы добавить личный домен в приложение, необходимо проверить его принадлежность к домену, добавив идентификатор проверки в качестве записи в формате TXT в поставщике домена. В левой области навигации страницы приложения щелкните **Обозреватель ресурсов** в разделе **Средства разработки**, а затем щелкните **Перейти**.

В представлении JSON свойств приложения найдите `customDomainVerificationId`и скопируйте его значение, заключив в двойные кавычки. Этот идентификатор проверки подлинности необходим для следующего шага.
...
> [!NOTE]
> Метка **Не защищено** у личного домена означает, что он еще не привязан к TLS/SSL-сертификату, и HTTPS-запросы из браузера к нему будут возвращать ошибку или предупреждение, в зависимости от браузера. Сведения о добавлении привязки TLS см. в статье [Защита пользовательского доменного имени с помощью привязки TLS/SSL в Службе приложений Azure](configure-ssl-bindings.md).
...
| TXT | `asuid` | [Идентификатор проверки, полученный ранее](#get-domain-verification-id) | Служба приложений обращается к записи в формате TXT `asuid.<subdomain>`, чтобы проверить владение личным доменом. В качестве корневого домена используйте `asuid`. |
...
Перейдите к следующему руководству, чтобы научиться привязывать пользовательский TLS/SSL-сертификат к веб-приложению.
