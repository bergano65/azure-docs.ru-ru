---
title: Руководство. Сопоставьте существующее настраиваемое DNS-имя
...
ms.locfileid: "78373650"
---
# <a name="tutorial-map-an-existing-custom-dns-name-to-azure-app-service"></a>Руководство. Привязка существующего настраиваемого DNS-имени к службе приложений Azure
...
> [!NOTE]
> Метка **Не защищено** у личного домена означает, что он еще не привязан к SSL-сертификату, и HTTPS-запросы из браузера к нему будут возвращать ошибку или предупреждение, в зависимости от браузера. Сведения о добавлении привязки SSL см. в разделе [Защита пользовательского доменного имени с помощью привязки SSL в Службе приложений Azure](configure-ssl-bindings.md).
...
| Тип записи | Host | Значение |
| - | - | - |
| А | `@` | IP-адрес из раздела [Копирование IP-адреса приложения](#info). |
...
> | Тип записи | Host | Значение |
> | - | - | - |
> | А | `www` | IP-адрес из раздела [Копирование IP-адреса приложения](#info). |
