---
title: Создание ASE с помощью ARM
description: Узнайте, как создать внешнюю среду службы приложений ILB с помощью шаблона Azure Resource Manager.
author: ccompy
...
2. После создания ILB ASE будет загружен сертификат TLS/SSL, который соответствует вашему домену ILB ASE.

3. Загруженный сертификат TLS/SSL присваивается ILB ASE в качестве «по умолчанию» TLS/SSL-сертификата.  Этот сертификат используется для TLS/SSL трафика для приложений на ILB ASE, когда они используют общий `https://someapp.mycustomrootdomain.com`корневой домен, который назначен ASE (например, ).
...
## <a name="upload-and-configure-the-default-tlsssl-certificate"></a>Загрузить и настроить "по умолчанию" сертификат TLS/SSL
Сертификат TLS/SSL должен быть связан с ASE как "по умолчанию" TLS/SSL-сертификат, который используется для создания подключений TLS к приложениям. Если суффикс DNS по умолчанию ASE `https://some-random-app.internal-contoso.com` *internal-contoso.com,* подключение к требует сертификата TLS/SSL, который действителен для q *.internal-contoso.com*. 

Получить действительный сертификат TLS/SSL, используя внутренние органы сертификации, приобретая сертификат у внешнего эмитента или используя сертификат, подписанный самостоятельно. Независимо от источника сертификата TLS/SSL, следующие атрибуты сертификата должны быть настроены должным образом:

* **Тема**: Этот атрибут должен быть установлен на *.your-root-domain-here.com*.
* **Тема Альтернативное название**: Этот атрибут должен включать в себя как *.your-root-domain-here.com,* так и *.scm.your-root-domain-here.com*. Соединения TLS с сайтом SCM/Kudu, связанные с каждым приложением, используют адрес формы *your-app-name.scm.your-root-domain-here.com.*

При действительном сертификате TLS/SSL в руках необходимы два дополнительных подготовительных шага. Конвертировать/сохранить сертификат TLS/SSL в виде файла .pfx. PFX-файл должен содержать все промежуточные и корневые сертификаты. Защитите его паролем.

Файл .pfx должен быть преобразован в строку base64, поскольку сертификат TLS/SSL загружается с помощью шаблона Resource Manager. Такое преобразование требуется, так как шаблоны Resource Manager представлены в виде текстовых файлов. Это позволит добавить PFX-файл в качестве параметра шаблона.
...
После успешного сгенерирования и преобразования сертификата TLS/SSL в базовую строку используйте шаблон ресурсного менеджера На [настройку sSL-сертификата по умолчанию][quickstartconfiguressl] на GitHub. 

Ниже перечислены параметры, содержащиеся в файле *azuredeploy.parameters.json*.

* *appServiceEnvironmentName*: Имя iLB ASE настраивается.
* *existingAseLocation*: Текстовая строка, содержащая область Azure, где был развернут ILB ASE.  Например, центрально-южная часть США.
...
* *certificateThumbprint*: Отпечаток пальца сертификата. Если вы получите это значение из PowerShell (например, значение *$certificate.Thumbprint* в приведенном выше фрагменте кода), его можно использовать без каких-либо изменений. Если вы скопируете это значение в диалоговом окне сертификатов Windows, в нем нужно удалить лишние пробелы. *СертификатБольшой отпечаток* должен выглядеть примерно как AF3143EB61D43F677784115BB7F17BBCECACACAE.
* *name*: Дружественный идентификатор строки по вашему выбору используется для идентификации сертификата. Имя используется как часть уникального идентификатора менеджера ресурсов для *сущности Microsoft.Web/сертификатов,* представляющей сертификат TLS/SSL. Имя *должно* заканчиваться таким суффиксом: \_имя_ASE_InternalLoadBalancingASE. Этот суффикс сообщает порталу Azure о том, что для обеспечения безопасности ASE с поддержкой ILB используется сертификат.
...
Когда шаблон будет реализован, к приложениям в среде ASE с внутренним балансировщиком нагрузки можно будет получать доступ по протоколу HTTPS, Соединения закрепляются с помощью сертификата TLS/SSL по умолчанию. Сертификат TLS/SSL по умолчанию используется, когда приложения на ILB ASE рассматриваются с помощью комбинации имени приложения плюс имя хоста по умолчанию. Например, `https://mycustomapp.internal-contoso.com` использует сертификат TLS/SSL по умолчанию для*q .internal-contoso.com*.

Так же как и для приложений в общедоступной мультитенантной службе, разработчики могут настроить для отдельных приложений пользовательские имена узлов Они также могут настроить уникальные привязки сертификатов SNI TLS/SSL для отдельных приложений.
