...
3. Переданный SSL-сертификат назначается ASE с внутренним балансировщиком нагрузки в качестве SSL-сертификата по умолчанию.  Этот сертификат используется для SSL трафика для приложений на ILB ASE, когда они используют общий `https://someapp.mycustomrootdomain.com`корневой домен, который назначен ASE (например, ).
...
* **Тема Альтернативное название**: Этот атрибут должен включать в себя как *.your-root-domain-here.com,* так и *.scm.your-root-domain-here.com*. Для SSL-подключений к сайту SCM или Kudu, связанному с каждым приложением, используется адрес в формате *имя_приложения.scm.имя_корневого_домена.com*.
