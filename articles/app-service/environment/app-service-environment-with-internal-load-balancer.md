...
После выбора внутреннего виртуального IP-адреса возможность добавлять дополнительные IP-адреса для ASE исчезнет, и вместо этого необходимо будет предоставить поддомен ASE. В ASE с внешним виртуальным IP-адресом имя среды службы приложений указывается в поддомене для приложений, создаваемых в этой среде. Если ASE имеет имя **_ContosoTest_* _ и приложение в этом ASE называется _*_myTest_*_, то поддомен имеет формат _*_ContosoTest.p.azurewebsites.NET_*_ , а URL-адрес этого приложения — _*_myTest.ContosoTest.p.azurewebsites.NET_*_. Если указать внутренний виртуальный IP-адрес, то имя среды ASE не используется в ее поддомене. В этом случае поддомен указывается явным образом. Если ваш поддомен _*_contoso.Corp.NET_*_ и вы сделали приложение в этой ASE с именем _*_timereporting_*_, URL-адрес этого приложения будет _*_timereporting.contoso.Corp.NET_*_.
...
1. В портал Azure выберите _ *создать ресурс-> Интернет и мобильные устройства-> Web** или **Mobile** или **приложение API**.
...
6. Выберите **Создать**. Установите флажок **Закрепить на панели мониторинга**, чтобы отобразить приложение на панели мониторинга. 
