...
description: Создание и использование ASE с ILB. Этот документ предоставляется только для клиентов, которые используют наследие v1 ASE.
author: ccompy
...
6. Выберите **виртуальную сеть/месторасположение - > конфигурацию VNet** и установите VIP тип на внутренний.
7. Введите имя поддомена (он будет использоваться для приложений, созданных в этой ASE).
8. Выберите **OK,** а затем **создать**.
...
1. Перейдите на uI ASE после создания **ASE -> Настройки - > сертификаты ILB.**
...
3. Получите адрес ILB для вашего ASE **(ASE -> Свойства - > виртуальный IP-адрес).**
