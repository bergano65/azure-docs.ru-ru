---
title: Автомасштабирование v1
description: Autoscaling и App Service Окружающей среды v1. Этот документ предоставляется только для клиентов, которые используют наследие v1 ASE.
author: btardif
...
Frank — это сисадмин для предприятия, которое перекочешно переносит часть рабочих нагрузок, которым они управляют, в среду службы приложений.
...
Федор хорошо знаком с приложением. Они знают, что часы пик для нагрузки между 9:00 утра и 6:00 вечера, потому что это линия бизнеса (LOB) приложение, которое сотрудники используют, пока они находятся в офисе. В конце рабочего дня показатели использования падают. В этот период сохраняется некоторая рабочая нагрузка, так как пользователи могут использовать приложение удаленно с мобильных устройств или домашних компьютеров. Производственный план службы приложений уже настроен на автоматическое масштабирование в соответствии с загрузкой ЦП и на основе следующих правил:
...
| **Продолжительность:** 10 минут |**Длительность:** 15 минут |
| **Агрегация по времени:** среднее |**Агрегация по времени:** среднее |
