---
title: Интеграция Среды службы приложений Azure с внутренней системой балансировки нагрузки (ILB) со Шлюзом приложения
...
Брандмауэры веб-приложения защищают веб-приложения, проверяя входящий трафик Интернета и блокируя атаки с использованием кода SQL и межсайтовых скриптов, передачи вредоносных программ, атаки DDoS на приложения и другие атаки. Также они проверяют ответы от внутренних веб-серверов для предотвращения потери данных (DLP). Можно получить устройство WAF из Azure Marketplace или использовать [шлюз приложений Azure][appgw].
...
Дополнительные сведения о создании Среда службы приложений ILB см. в разделе [Создание и использование среда службы приложений ilB][ilbase].
...
2. В области **Основные**:
   ...
   1\. В поле **Имя** введите имя шлюза приложений.

   2\. В области **Уровень** выберите **WAF**.

   В. В поле **Подписка** выберите подписку, которую использует виртуальная сеть среды службы приложений.

   Г. В области **Группа ресурсов** создайте или выберите группу ресурсов.
...
3. В области **Параметры**:
   ...
   1\. В разделе **Виртуальная сеть** выберите виртуальную сеть среды службы приложений.

   2\. В поле **Подсеть** укажите подсеть, в которой будет развернут шлюз приложений. Не указывайте подсеть GatewaySubnet, иначе будет невозможно создание VPN-шлюзов.

   В. В разделе **Тип IP-адреса** выберите **Общедоступный**.

   Г. В разделе общедоступного IP-адреса выберите **Общедоступный IP-адрес**. Если его у вас нет, создайте его.
...
Сведения о настройке пользовательских доменных имен для веб-приложений см. в статье [Настройка пользовательских доменных имен для веб-приложения][custom-domain]. Однако для приложения в среде службы приложений с внутренней подсистемой балансировки нагрузки нет никакой проверки доменного имени. Так как вам принадлежит служба DNS, которая управляет конечными точками приложения, вы можете добавить все, что посчитаете нужным. Добавляемое в этом случае имя личного домена не обязательно должно находиться в вашей службе DNS, но его все равно нужно настроить для вашего приложения. 
