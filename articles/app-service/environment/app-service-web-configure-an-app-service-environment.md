...
description: Конфигурация, управление и мониторинг среды службы приложений v1. Этот документ предоставляется только для клиентов, которые используют наследие v1 ASE.
author: ccompy
...
**Параметры**Свойства : Лезвие **настроек** автоматически открывается, когда вы воспитываете лезвие ASE.**Properties** >  В верхней части колонки находится раздел **Свойства**. Наряду с элементами, которые повторяют данные из раздела **Основные компоненты**, здесь также отображены и другие полезные сведения, включая **виртуальный IP-адрес** и **исходящий IP-адрес**.

![Колонка "Параметры" и "Свойства"][4]

**Настройки** > **IP-адресов:** При создании приложения IP Secure Sockets Layer (SSL) в вашем ASE вам нужен IP SSL-адрес. Чтобы получить его, в среде ASE должны быть адреса SSL на основе IP, которые можно выделить. Для этой цели в ASE добавляется 1 адрес SSL на основе IP, но позже можно добавить дополнительные адреса. Дополнительные адреса SSL на основе IP предоставляются на платной основе, как указано на странице [цен на службу приложений][AppServicePricing] в разделе "SSL-подключения". Дополнительные цена — цена за SSL на основе IP.

**Настройки** > **Front End Pool** / **Worker Pool:** Каждый из этих лопастей пула ресурсов предлагает возможность видеть информацию только в этом пуле ресурсов, в дополнение к предоставлению элементов управления для полного масштабирования этого пула ресурсов.  
...
Чтобы использовать возможности руководства или автомасштабного масштаба в определенном пуле ресурсов, перейдите в**пулы работников пулов** **Настройки** > **Front End.** /  Затем откройте пул, который требуется изменить. Перейти к **настройкам** > **масштабирование** или**масштабирование** **настроек.** >  Колонка **Горизонтальное масштабирование** позволяет контролировать число экземпляров. **Увеличение масштаба** позволяет контролировать размер ресурса.  
...
Чтобы начать работу с средой службы приложений, [см. Как создать среду службы приложений](app-service-web-how-to-create-an-app-service-environment.md).
