...
description: Настройка, управление и мониторинг Среда службы приложений v1. Этот документ предоставляется только для клиентов, использующих устаревшую версию ASE (версию 1).
author: ccompy
...
**Параметры**  >  **Свойства**. колонка **параметров** автоматически открывается при открытии колонки ASE. В верхней части колонки находится раздел **Свойства**. Наряду с элементами, которые повторяют данные из раздела **Основные компоненты**, здесь также отображены и другие полезные сведения, включая **виртуальный IP-адрес** и **исходящий IP-адрес**.

![Колонка "Параметры" и "Свойства"][4]

**Параметры**  >  **IP-адреса**. при создании приложения ip-SSL (SSL) в ASE вам потребуется SSL на основе IP адрес. Чтобы получить его, в среде ASE должны быть адреса SSL на основе IP, которые можно выделить. Для этой цели в ASE добавляется 1 адрес SSL на основе IP, но позже можно добавить дополнительные адреса. Дополнительные адреса SSL на основе IP предоставляются на платной основе, как указано на странице [цен на службу приложений][AppServicePricing] в разделе "SSL-подключения". Дополнительные цена — цена за SSL на основе IP.

**Параметры**  >  Пул внешних интерфейсов **Front End Pool**  /  **Пулы рабочих ролей**. Каждая из этих колонок пула ресурсов позволяет просматривать сведения только в этом пуле ресурсов, а также предоставлять элементы управления для полного масштабирования пула ресурсов.  
...
Чтобы использовать возможности ручного или автомасштабирования в определенном пуле ресурсов, последовательно выберите **Параметры**  >  **интерфейсные**  /  **Пулы пул рабочих ролей** . Затем откройте пул, который требуется изменить. Последовательно выберите **Параметры**  >  **Scale out** или **Параметры**  >  **увеличить масштаб**. Колонка **Горизонтальное масштабирование** позволяет контролировать число экземпляров. **Увеличение масштаба** позволяет контролировать размер ресурса.  
...
Чтобы приступить к работе со средами службы приложений, см. статью [создание среда службы приложений](app-service-web-how-to-create-an-app-service-environment.md).
