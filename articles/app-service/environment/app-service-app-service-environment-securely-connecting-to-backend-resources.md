...
Поскольку Среда службы приложений всегда **создается в виртуальной** сети Azure Resource Manager **или** в классической [модели развертывания][virtualnetwork], исходящие подключения между среда службы приложений и другими внутренними ресурсами может передаваться только для виртуальной сети.  В связи с последним изменением от июня 2016 г. среды ASE можно также развертывать в виртуальных сетях, использующих либо диапазоны общедоступных адресов, либо адресные пространства RFC1918 (т. е. частные адреса).  
...
* [Списки управления доступом к сети][NetworkAccessControlLists] (Сетевые списки ACL)
