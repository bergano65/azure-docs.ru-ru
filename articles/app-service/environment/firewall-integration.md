...
Брандмауэр Azure может отправлять журналы в службу хранилища Azure, концентратор событий или Log Analytics. Чтобы интегрировать приложение с любым поддерживаемым местом назначения, перейдите в раздел "Журналы диагностики"на портале Брандмауэра Azure и включите журналы для требуемого расположения. При интеграции с Log Analytics вы сможете просматривать журналы с любым трафиком, отправляемым в Брандмауэр Azure. Чтобы просмотреть отклоняемый трафик, откройте раздел "Журналы"на портале Log Analytics и введите запрос такого рода. 

    AzureDiagnostics | where msg_s contains "Deny" | where TimeGenerated >= ago(1h)
 
Интеграция Брандмауэра Azure с Log Analytics очень полезна при первом рабочем запуске приложения, когда вам известны не все зависимости приложения. См. дополнительные сведения об [анализе данных Log Analytics в Azure Monitor](https://docs.microsoft.com/azure/azure-monitor/log-query/log-query-overview).
...
- Зависимости IP-адреса задаются для трафика, не использующего протокол HTTP/HTTPS.
