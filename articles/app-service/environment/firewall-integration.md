...
Если в подсети ASE еще нет назначенного NSG, создайте ее. В NSG установите первое правило, разрешающее трафик из тега службы с именем Аппсервицеманажемент на портах 454, 455. Это все, что требуется из общедоступных IP-адресов для управления ASE. Адреса, расположенные за этим тегом службы, используются только для администрирования службы приложений Azure. Трафик управления, который проходит через эти подключения, шифруется и защищается с помощью сертификатов проверки подлинности. Типичный трафик на этом канале включает такие действия, как инициированные клиентом команды и пробы работоспособности. 
