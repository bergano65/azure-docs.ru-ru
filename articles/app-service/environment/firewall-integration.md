...
Развертывание ASE с помощью исходящий трафик, проходящий через аппаратный брандмауэр требует изменения маршрутов в подсети ASE. Маршруты работают на уровне IP-адрес. Если не соблюдать осторожность при определении маршрутов, вы можете принудительно ответный трафик TCP на источник, с другого адреса. Это называется асимметричной маршрутизации и будет нарушено TCP.

Должно существовать маршруты, чтобы входящий трафик ASE можно отвечать обратно пришел так же, как трафик. Это касается запросов входящего трафика управления, и он имеет значение true, если для запросов приложений от входящего трафика.

Трафик к и из ASE должны соблюдаться следующие соглашения

* Трафик к Azure SQL, хранилища и концентратора событий не поддерживает использование устройства брандмауэра. Этот трафик должен отправляться непосредственно к этим службам. Чтобы реализовать — Настройка конечных точек служб для этих трех служб. 
* Правила таблиц маршрутов должны быть определены, которые отправляют входящий трафик управления от его источник.
* Правила таблиц маршрутов должны быть определены, которые отправляют трафик приложений от входящего трафика из его источник. 
* Весь трафик, оставляя ASE могут отправляться на ваше устройство брандмауэра с правилом таблицы маршрутов.
...
| 40.77.24.27:80 | Требуется монитор и предупреждение о проблемах ASE |
| 40.77.24.27:443 | Требуется монитор и предупреждение о проблемах ASE |
| 13.90.249.229:80 | Требуется монитор и предупреждение о проблемах ASE |
| 13.90.249.229:443 | Требуется монитор и предупреждение о проблемах ASE |
| 104.45.230.69:80 | Требуется монитор и предупреждение о проблемах ASE |
| 104.45.230.69:443 | Требуется монитор и предупреждение о проблемах ASE |
| 13.82.184.151:80 | Требуется монитор и предупреждение о проблемах ASE |
| 13.82.184.151:443 | Требуется монитор и предупреждение о проблемах ASE |
