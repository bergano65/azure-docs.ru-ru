...
5. При выборе существующей виртуальной сети необходимо создать подсеть для размещения ASE. Задайте для подсети достаточно большой размер с учетом увеличения среды службы приложений в будущем. Рекомендуемый размер — `/24`. Такая подсеть содержит 256 адресов и может обслуживать среду службы приложений максимального размера и любые потребности масштабирования. 
...
1. Выберите используемую операционную систему. 
    ...
    * Если вы хотите создать приложение Linux с помощью настраиваемого контейнера Docker, можно просто использовать собственный контейнер, следуя приведенным [здесь][linuxapp] инструкциям. 

1. Выбор или создание плана службы приложений Если необходимо создать план службы приложений, выберите свою среду ASE в качестве расположения. Выберите рабочий пул, в котором будет создан этот план. При создании плана службы приложений следует выбрать ASE в качестве расположения и рабочий пул. При указании имени приложения домен в имени вашего приложения заменяется доменом ASE.

1. Нажмите кнопку **Создать**. Если требуется отобразить приложение на панели мониторинга, следует установить флажок **Закрепить на панели мониторинга**.
...
* **Субъект** — для этого атрибута необходимо задать значение *.имя_корневого_домена.
* **Альтернативное имя субъекта** — этот атрибут должен содержать два значения: **.имя_корневого_домена* и **.scm.имя_корневого_домена*. Для SSL-соединения с сайтом SCM или Kudu, связанным с каждым приложением, будет использоваться адрес в формате *имя_приложения.scm.имя_корневого_домена*.
...
1. Задайте сертификат ILB, выбрав PFX-файл сертификата, и введите пароль. На выполнение этой операции потребуется некоторое время. Появится сообщение о том, что выполняется отправка.

1. Получите адрес ILB для ASE. Выберите **ASE** > **Свойства** > **Виртуальный IP-адрес**.

2. Создайте приложение в среде ASE после ее создания.

3. Создайте виртуальную машину в этой виртуальной сети (если вы этого еще не сделали).
...
4. Задайте DNS для домена среды ASE. Можно указать в DNS подстановочные знаки с именем домена. Чтобы выполнить несколько простых проверок, можно изменить файл hosts на виртуальной машине, чтобы в качестве имени приложения задать виртуальный IP-адрес.

    a. Если доменное имя вашей среды ASE — _.ilbase.com_ и вы создали приложение с именем _mytestapp_, его адрес — _mytestapp.ilbase.com_. Позднее вы настроите разрешение имени _mytestapp.ilbase.com_ в адрес ILB. (В Windows файл hosts находится в папке _C:\Windows\System32\drivers\etc\\_.)
...
6. Воспользуйтесь браузером на этой виртуальной машине и перейдите по адресу https://mytestapp.ilbase.com. Если используется самозаверяющий сертификат, учтите, что уровень безопасности будет снижен.
...
- Добавьте сертификат в хранилище доверенных сертификатов. Это позволит разблокировать Microsoft Edge и Internet Explorer.
