...
* **Субъект**: для этого атрибута необходимо задать значение *\.корневой_домен.
* **Альтернативное имя субъекта**: этот атрибут должен содержать два значения: *.корневой_домен* и *.scm.корневой_домен*. Для SSL-соединения с сайтом SCM или Kudu, связанным с каждым приложением, будет использоваться адрес в формате *имя_приложения.scm.имя_корневого_домена*.
