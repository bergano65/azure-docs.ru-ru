...
Когда среда службы приложений (ASE) использует общедоступный виртуальный IP-адрес для приложений, на него поступает весь входящий трафик.  Он включает в себя трафик HTTP и HTTPS приложений, а также прочий трафик FTP, удаленной отладки и операций управления Azure.  Полный список конкретных портов (обязательных и необязательных), доступных в общедоступном виртуальном IP-адресе, см. в статье [Управление входящим трафиком][controllinginboundtraffic] в среда службы приложений. 
...
Поскольку среда службы приложений развертывается в подсети, группы безопасности сети можно использовать для управления входящим трафиком подсети.  Дополнительные сведения о том, как управлять входящим трафиком к Среда службы приложений, см. в следующей [статье][controllinginboundtraffic].

Дополнительные сведения о том, как разрешить исходящее подключение к Интернету из Среда службы приложений, см. в следующей статье о работе с [Express Route][ExpressRoute].  Метод, описанный в статье, применяется также при работе с подключением «сеть — сеть» и при использовании принудительного туннелирования.
...
Сведения о входящих портах, используемых средами службы приложений и использовании групп безопасности сети для управления входящим трафиком, доступны [здесь][controllinginboundtraffic].

В этой [статье][ExpressRoute]содержатся сведения об использовании определяемых пользователем маршрутов для предоставления исходящего доступа в Интернет к средам службы приложений. 
