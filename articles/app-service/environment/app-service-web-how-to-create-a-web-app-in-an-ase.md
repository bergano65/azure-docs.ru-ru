...
description: Узнайте, как создавать веб-приложения в среде службы приложений v1. Этот документ предоставляется только для клиентов, которые используют наследие v1 ASE.
author: ccompy
...
3. Выберите или создайте группу ресурсов.
    ...
    *Группы ресурсов* позволяют управлять связанными ресурсами Azure как единое целое и полезны при установлении правил *управления доступом на основе ролей* (RBAC) для ваших приложений. Для получения дополнительной информации смотрите [обзор менеджера ресурсов Azure][ResourceGroups]. 
...
При создании в среде ASE плана ASP выбранные рабочие роли будут отличаться, так как в среде ASE нет общих рабочих ролей.  Работники, которые вы должны использовать, это те, которые были выделены asE админом.  Это означает, что для создания нового плана необходимо выделить в пул работников ASE больше сотрудников, чем общее количество экземпляров во всех ваших планах, уже наданных в этом пуле работников.  Если в рабочем пуле среды ASE недостаточно рабочих ролей для создания плана ASP, обратитесь к администратору среды ASE, чтобы их добавить.
