...
description: Узнайте, как создать среду Службы приложений с приложением в ней, или создать автономный (пустой) ASE.
author: ccompy
...
> [!NOTE]
> Каждая среда службы приложений имеет виртуальный IP (VIP), который может быть использован для контакта с средой службы приложений.
...
1. На [портале Azure](https://portal.azure.com/)выберите **«Создайте ресурс** > **веб- и мобильное** > **веб-приложение».**
...
1. Выберите виртуальную сеть и расположение. Вы можете создать виртуальную сеть или выбрать имеющуюся: 
    ...
    * Если выбрать имеющуюся виртуальную сеть, при создании ASE создается подсеть. *Вы не можете использовать заранее созданную подсеть на портале. Вы можете создать ASE с существующей подсетью, если вы используете шаблон менеджера ресурсов.* Сведения о создании среды ASE из шаблона см. в статье [Создание среды службы приложений с внутренней подсистемой балансировки нагрузки с помощью шаблонов Azure Resource Manager][MakeASEfromTemplate].
