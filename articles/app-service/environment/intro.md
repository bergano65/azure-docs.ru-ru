---
title: Введение
description: Узнайте, как Среды службы приложений Azure помогают масштабировать, защищать и оптимизировать приложения в полностью изолированной и выделенной среде.
author: ccompy
...
* Среды службы приложений можно развернуть в Зонах доступности (AZ) с использованием привязки зоны.  Дополнительные сведения см. в статье [App Service Environment Support for Availability Zones][ASEAZ] (Поддержка Среды службы приложений для Зон доступности).
...
Функция ASE позволяет развернуть службу приложений Azure непосредственно в виртуальной сети Azure Resource Manager клиента. Дополнительные сведения о виртуальных сетях Azure см. в статье [Виртуальная сеть Azure: часто задаваемые вопросы](../../virtual-network/virtual-networks-faq.md). Так как среда ASE всегда существует в виртуальной сети, а точнее в подсети виртуальной сети, можно использовать средства безопасности виртуальных сетей для управления как входящими, так и исходящими подключениями в сети для приложений.
...
В ASEv1 необходимо вручную управлять всеми ресурсами. включая внешние интерфейсы, рабочие процессы и IP-адреса, использованные для SSL на основе IP. Перед горизонтальным увеличением масштаба плана службы приложений необходимо горизонтально увеличить масштаб пула рабочих процессов, в котором необходимо разместить план.
