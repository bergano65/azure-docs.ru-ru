...
description: Конфигурация сети для среды службы приложений для PowerApps с Azure ExpressRoute. Этот документ предоставляется только для клиентов, которые используют наследие v1 ASE.
author: stefsch
...
* Исходящие сетевые подключения к доменам ocsp.msocsp.com, mscrl.microsoft.com и crl.microsoft.com. Подключение к этим доменам необходимо для поддержки функциональности TLS.
...
* Скачайте Azure PowerShell со страницы Azure [Загрузки][AzureDownloads]. Выберите загрузку с датой июнь 2015 г. или позже. В соответствии с **командными инструментами** > **Windows PowerShell,** выберите **Установить** для установки новейших смлетов PowerShell.
...
### <a name="step-3-associate-the-table-to-the-subnet"></a>Шаг 3: Связать таблицу с подсетью
