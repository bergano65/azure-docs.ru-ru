...
Внешняя ASE позволяет назначать IP-адреса отдельным приложениям. В ASE с ILB такая возможность не предусмотрена. Для получения дополнительной информации о том, как настроить приложение с собственным IP-адресом, смотрите [Безопасное пользовательское имя DNS с привязкой SSL в Azure App Service.](../configure-ssl-bindings.md)
