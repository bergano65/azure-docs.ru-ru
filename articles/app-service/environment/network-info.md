...
|  Разрешение входящего трафика Azure Load Balancer | Балансировщик нагрузки Azure | Подсеть ASE: Все порты
...
Для взаимодействия между балансировщиком нагрузки Azure и подсетью ASE необходимо открыть как минимум порты 454, 455 и 16001. Порт 16001 используется для трафика проверки активности между балансировщиком нагрузки и ASE. Если вы используете ASE с ILB, то вы можете заблокировать трафик 454, 455 и 16001 порты.  При использовании внешней среды ASE, необходимо принять во внимание порты доступа обычное приложение.  Если вы используете адреса, присвоенные приложению, необходимо открыть его для всех портов.  Если адрес присваивается определенному приложению, балансировщик нагрузки будет заранее использовать неизвестные порты для отправки трафика HTTP и HTTPS в ASE.

Если вы используете IP-адреса, присвоенные приложению, необходимо разрешить трафик с IP-адресов, назначенных приложений к подсети ASE.
...
| 53 | DNS |
| 123 | NTP. |
| 80/443 | Список отзыва Сертификатов, обновления Windows, Linux зависимости, служб Azure |
| 1433 | Azure SQL | 
| 12000 | Мониторинг |
