...
- **Внешняя среда ASE.** Предоставляет приложения, размещенные в ASE, по доступному в Интернете IP-адресу. Дополнительные сведения см. [в разделе Создание внешнего ASE][MakeExternalASE].
- **Среда ASE с ILB.** Предоставляет приложения в ASE по IP-адресу в вашей виртуальной сети Azure. Внутренняя конечная точка является внутренним балансировщиком нагрузки (ILB). Поэтому среда называется ASE с ILB. Дополнительные сведения см. [в статье Создание и использование ilB ASE][MakeILBASE].
...
В среде ASE есть несколько IP-адресов, на которые следует обратить внимание. Подробные сведения.
...
Конечные точки службы позволяют ограничить доступ к мультитенантным службам для набора виртуальных сетей и подсетей. Дополнительные сведения о конечных точках службы см. в документации по конечным точкам [службы виртуальной сети][serviceendpoints] . 
