...
description: Узнайте о функциях службы приложений «Среда v1». Этот документ предоставляется только для клиентов, которые используют наследие v1 ASE.
author: stefsch
...
Среда службы приложений — это опция [премиум-плана][PremiumTier] обслуживания [Службы приложений Azure,](../overview.md) которая обеспечивает полностью изолированную и выделенную среду для надежного запуска приложений Azure App Service в больших масштабах, включая Web Apps, Мобильные приложения и Приложения API.  
...
Для получения обзора того, как среды служб ы приложений обеспечивают высокий масштаб и безопасный доступ к сети, смотрите в среде службы приложений [AzureCon Deep Dive!][AzureConDeepDive]
...
Фронтовой пул содержит вычислительные ресурсы, ответственные за прекращение TLS, а также автоматическое балансирование нагрузки запросов приложений в среде службы приложений.
