---
title: Многоуровневая архитектура безопасности в средах Службы приложений Azure
...
Основная цель заключается в том, чтобы сделать невозможным общий доступ к внутреннему приложению API из Интернета, разрешив вызывать API только вышестоящими веб-приложениями.  [Группы безопасности сети (группы безопасности сети)][NetworkSecurityGroups] можно использовать в подсетях, содержащих среды службы приложений, чтобы ограничить общий доступ к приложениям API.
...
Так как [группы безопасности сети (группы безопасности сети)][NetworkSecurityGroups] применяются к подсетям, а среды службы приложений развертываются в подсетях, правила, содержащиеся в NSG, применяются ко **всем** приложениям, выполняемым на среда службы приложений.  Использование приведенного в этой статье примера архитектуры предполагает следующее. После того как группа безопасности сети будет применена к подсети, содержащей apiase, все приложения, выполняющиеся в среде службы приложений apiase, будут защищены этим же набором правил безопасности. 

* **Определение исходящего IP-адреса вышестоящих вызывающих объектов.**  Какой IP-адрес или какие IP-адреса у вышестоящих вызывающих объектов?  Эти адреса нужны для явного разрешения доступа в NSG.  Вызовы между средами службы приложений считаются интернет-вызовами, поэтому исходящему IP-адресу, назначенному каждой из трех вышестоящих сред службы приложений, в NSG должен быть разрешен доступ для подсети apiase.   Дополнительные сведения об определении исходящего IP-адреса для приложений, выполняющихся в Среда службы приложений, см. в статье Обзор [архитектуры сети][NetworkArchitecture] .
...
Сначала добавляется явное правило разрешения для инфраструктуры управления Azure, как указано в статье о [входящем трафике][InboundTraffic] для сред службы приложений.
...
Общие сведения об [исходящих IP-адресах][NetworkArchitecture] и средах службы приложений.

[Сетевые порты][InboundTraffic] , используемые средами службы приложений.
