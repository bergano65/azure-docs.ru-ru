...
По умолчанию CI, Интернет-систем, таких как GitHub и DevOps в Azure, не работают с ASE с ILB, так как не конечная точка публикации недоступна через Интернет. Для разработки и операций Azure это можно обойти, установив агент резидентной выпуска в вашей внутренней сети, где он достигнет внутреннего балансировщика Нагрузки. Кроме того можно также использовать системы непрерывной Интеграции, которая использует модель по запросу, например Dropbox.
