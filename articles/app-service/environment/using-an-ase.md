...
При использовании внешнего ASE приложения, сделанные в ASE, регистрируются в Azure DNS. С помощью ILB ASE необходимо управлять собственными DNS. 

Чтобы настроить DNS с помощью ILB ASE, выполните следующие действия.
...
У ASE есть 1 ТБ хранилища для всех приложений в ASE. План службы приложений в SKU "изолированная ценовая категория" имеет ограничение в 250 ГБ по умолчанию. Если у вас пять или более планов службы приложений, будьте внимательны, чтобы не превышать ограничение в 1 ТБ ASE. Если вам требуется больше ограничений в пределах 250 ГБ в одном плане службы приложений, обратитесь в службу поддержки, чтобы настроить ограничение для плана службы приложений на максимум 1 ТБ. Если предельный размер плана изменяется, по всем планам службы приложений в ASE остается ограничение в 1 ТБ.
...
| ASE приближается к общему ограничению экземпляров | Указанный ASE приближается к общему ограничению экземпляра ASE. В настоящее время {0} он содержит экземпляры плана службы приложений максимум 201 экземпляров. |
...
| Сбой операций масштабирования | Не удалось масштабировать план службы{0}приложений (). Текущее состояние: {1} {2} исполнители. |
