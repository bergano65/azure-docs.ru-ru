...
- **Рабочие**: Ресурсы, размещающие ваши приложения
- **База данных**: Содержит информацию, определяющую среду
- **Хранение**: Используется для размещения приложений, опубликованных заказчиком
...
Служба приложений может выделить специальному IP-адресу приложения. Эта возможность доступна после настройки SSL на основе IP, как описано в [Свяжите существующий пользовательский сертификат TLS/SSL в Azure App Service.][ConfigureSSL] В ILB ASE нельзя добавлять больше IP-адресов, которые будут использоваться для SSL на основе IP.
...
При использовании внешнего ASE приложения, сделанные в вашем ASE, регистрируются в Azure DNS. С ILB ASE, вы должны управлять своим собственным DNS. 

Для настройки DNS с iLB ASE:
...
Настройки DNS для суффикса домена ASE по умолчанию не ограничивают ваши приложения только этими именами. Вы можете установить пользовательское доменное имя без проверки на ваших приложениях в ILB ASE. Если вы хотите создать зону, названную *contoso.net,* вы можете сделать это и указать его на IP-адрес ILB. Пользовательское доменное имя работает для запросов приложений, но не для сайта scm. Сайт scm доступен только в * &lt;&gt;приложении&lt; .scm. asename&gt;.appserviceenvironment.net*. 

Зона названа *.&lt; asename&gt;.appserviceenvironment.net* является глобально уникальным. До мая 2019 года клиенты смогли указать суффикс домена ILB ASE. Если вы хотите использовать *.contoso.com* для суффикса домена, то вы могли сделать так и то включило бы место scm. Были проблемы с этой моделью в том числе; управление сертификатом SSL по умолчанию, отсутствие единого ввоза с сайтом scm и требование об использовании сертификата подстановочного знака. Процесс обновления сертификата ILB ASE по умолчанию также был разрушительным и вызвал перезагрузку приложения. Чтобы решить эти проблемы, поведение ILB ASE было изменено на использование суффикса домена на основе названия ASE и ссуффиксом, принадлежащим Microsoft. Изменение поведения ILB ASE влияет только на ILB ASEs, сделанные после мая 2019 года. Уже существующие ILB ASEs должны по-прежнему управлять сертификатом по умолчанию ASE и их конфигурацией DNS.
...
С ILB ASE, публикация конечных точек доступны только через ILB. Внутренняя подсистема балансировки нагрузки расположена на частном IP-адресе в подсети ASE в виртуальной сети. Если у вас нет сетевого доступа к ILB, вы не можете публиковать какие-либо приложения на этом ASE. Как отмечено в [Создании и использовании ILB ASE,][MakeILBASE]вы должны настроить DNS для приложений в системе. Это требование включает конечную точку СКМ. Если конечные точки не определены должным образом, вы не можете опубликовать. Ваши IDEs также должны иметь сетевой доступ к ILB для публикации непосредственно к нему.
...
Если вы интегрируетесь с Log Analytics, вы можете увидеть журналы, выбрав **журналы** с портала ASE и создав запрос против **AppServiceEnvironmentPlatformLogs.** Входы излучаются только тогда, когда ваш ASE имеет событие, которое вызовет его. Если у вас ASE нет такого события, не будет никаких журналов. Чтобы быстро увидеть пример журналов в рабочем пространстве Log Analytics, выполните масштабную операцию с одним из планов службы приложений в вашем ASE. Затем можно запустить запрос на **AppServiceEnvironmentPlatformLogs,** чтобы просмотреть эти журналы. 

**Создание оповещения**

Чтобы создать оповещение о своих журналах, следуйте инструкциям в [Create, view and manage log alerts с помощью Azure Monitor.][logalerts] Краткое описание

* Откройте страницу оповещения на портале ASE
* Выберите **новое правило оповещения**
* Выберите ресурс в качестве рабочего пространства analytics журналов
* Установите условие с помощью пользовательского поиска журнала, чтобы использовать запрос типа : "AppServiceEnvironmentPlatformLogs где ResultDescription содержит "начало масштабирования" или что вы хотите. Установите порог по мере необходимости. 
* Добавить или создать группу действий по желанию. Группа действий — это место, где вы определяете ответ на оповещение, например отправку электронного письма или SMS-сообщения
* Назовите ваше оповещение и сохраните его.
