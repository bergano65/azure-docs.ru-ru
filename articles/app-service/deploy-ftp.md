...
description: Узнайте, как развернуть приложение в службе приложений Azure с помощью FTP или FTPS. Улучшить безопасность веб-сайта, отключив незашифрованную FTP.
...
2. Выберите веб-приложение, развертывание которых необходимо развернуть.

    ![Выберите приложение.](media/app-service-continuous-deployment/select-your-app.png)

3. Выберите **центр** > развертывания**FTP** > **Dashboard**.
...
В панели мониторинга FTP выберите **Copy,** чтобы скопировать конечную точку FTPS и учетные данные приложений.
...
> [!NOTE]
> Проверка подлинности на конечную точку FTP/FTPS с использованием учетных данных пользователя, требующего имени пользователя в следующем формате: 
>
>`<app-name>\<user-name>`
>
...
Чтобы отключить незашифрованную FTP, выберите **FTPS только** в **состоянии FTP.** Чтобы полностью отключить FTP и FTPS, выберите **Инвалиды.** По завершении щелкните **Сохранить**. При использовании **FTPS Only,** вы должны обеспечить TLS 1.2 или выше, перемещаясь по лезвию **настроек TLS/SSL** вашего веб-приложения. Протоколы TLS 1.0 и 1.1 не поддерживаются для параметра **Только FTPS**.
...
- [Я не могу FTP и опубликовать свой код. Как я могу решить проблему?](#im-not-able-to-ftp-and-publish-my-code-how-can-i-resolve-the-issue)
