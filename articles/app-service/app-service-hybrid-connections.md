...
1. Войдите, используя учетную запись Azure для получения доступных с вашими подписками гибридных подключений. HCM не продолжать использовать учетную запись Azure, кроме этого. 
1. Выберите подписку.
...
В службе приложений **tcpping** средство командной строки, которые могут вызываться из консоли Дополнительные инструменты (Kudu). Этот инструмент может сообщить, имеется ли доступ к конечной точке TCP, но не сообщает, имеется ли доступ к конечной точке гибридного подключения. Используя этот инструмент в консоли для конечной точки гибридного подключения, можно только подтвердить, что оно использует сочетание "узел:порт".  

Если у вас есть клиент командной строки для конечной точки, можно проверить связь с консоли приложений. Например можно проверить доступ к конечным точкам web server с помощью curl.
