...
Любой пользователь `Reader` , имеющий доступ к ретранслятору, сможет _увидеть_ гибридное подключение при попытке добавить его в веб-приложение на портале Azure, но не сможет _добавить_ его, так как у них отсутствуют разрешения на получение строки подключения, используемой для установления подключения ретранслятора. Чтобы успешно добавить гибридное подключение, у них должно быть `listKeys` разрешение (`Microsoft.Relay/namespaces/hybridConnections/authorizationRules/listKeys/action`). `Contributor` Роль или любая другая роль, которая включает это разрешение для ретранслятора, позволит пользователям использовать гибридное подключение и добавлять его в собственные веб-приложения.
