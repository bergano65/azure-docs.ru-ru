...
ms.locfileid: "74671511"
---
# <a name="azure-app-service-hybrid-connections"></a>Гибридные подключения к службе приложений Azure #

Гибридные подключения — это и служба в Azure, и функция в службе приложений Azure. В качестве службы их применение и возможности шире, чем функциональные возможности в службе приложений Azure. Дополнительные сведения о гибридные подключения и их использовании вне службы приложений см. в разделе [Azure Relay гибридные подключения][HCService].
...
### <a name="prerequisites"></a>Технические условия ###
 - Требуется служба приложений Windows. Она доступна только в Windows.  

## <a name="add-and-create-hybrid-connections-in-your-app"></a>Добавление и создание гибридных подключений в приложении ##

Чтобы создать гибридное подключение, перейдите к [портал Azure][portal] и выберите свое приложение. Выберите **Сети** > **Настройте конечные точки гибридного подключения**. Здесь можно увидеть гибридные подключения, которые были настроены для приложения.  
...
Для работы гибридных подключений в сети, в которой размещена конечная точка гибридного подключения, необходим агент ретрансляции. Он называется Hybrid Connection Manager (HCM). Чтобы скачать HCM, в приложении на [портал Azure][portal]выберите **сети** > **настроить конечные точки гибридного подключения**.  
...
Если необходимо, чтобы кто-либо вне вашей подписки разместил экземпляр HCM для заданного гибридного подключения, предоставьте ему строку подключения к шлюзу для этого гибридного подключения. Строку подключения к шлюзу можно увидеть в свойствах гибридного подключения в [портал Azure][portal]. Чтобы использовать эту строку, нажмите кнопку **Ввести вручную** в HCM и вставьте строку подключения к шлюзу.
...
Одним из способов использования этой информации является armclient, который можно получить из проекта [armclient][armclient] GitHub. Ниже приведен пример установки уже существующего гибридного подключения к вашему приложению. Создайте JSON-файл, используя приведенную выше схему, например:
...
Существующее гибридное подключение можно добавить в другие веб-приложения службы приложений любым пользователем, имеющим достаточные разрешения на основе ретранслятора служебной шины Azure. Это означает, что если необходимо предотвратить повторное использование этого же гибридного подключения (например, если целевой ресурс является службой, не имеющей каких-либо дополнительных мер безопасности для предотвращения несанкционированного доступа), необходимо заблокировать доступ к Azure. Ретранслятор служебной шины.

Любой пользователь, имеющий `Reader` доступ к ретранслятору, сможет _увидеть_ гибридное подключение при попытке добавить его в свое веб-приложение на портале Azure, но не сможет _Добавить_ его, так как у них отсутствуют разрешения на получение строки подключения, которая используется для установки подключения ретранслятора. Для успешного добавления гибридного подключения у него должно быть разрешение `listKeys` (`Microsoft.Relay/namespaces/hybridConnections/authorizationRules/listKeys/action`). Роль `Contributor` или любая другая роль, которая включает это разрешение для ретранслятора, позволит пользователям использовать гибридное подключение и добавлять его в собственные веб-приложения.
