...
description: Узнайте, как создавать и использовать гибридные соединения в Службе приложений Azure для доступа к ресурсам в разрозненных сетях.
author: ccompy
...
### <a name="prerequisites"></a>Предварительные требования ###
 - Требуется служба Windows App. Он доступен только в Windows.  

## <a name="add-and-create-hybrid-connections-in-your-app"></a>Добавление и создание гибридных подключений в приложении ##

Чтобы создать гибридное подключение, перейдите на [портал Azure][portal] и выберите свое приложение. Выберите **сетевые** > **настройки конечных точек гибридного соединения.** Здесь можно увидеть гибридные подключения, которые были настроены для приложения.  
...
1. Вопийте с учетной записью Azure, чтобы получить гибридные соединения доступными с подписками. HCM не продолжает использовать вашу учетную запись Azure после этого. 
1. Выберите подписку.
...
Перечисленные ниже интерфейсы API позволяют напрямую управлять гибридными подключениями к приложениям. 
...
Эти сведения можно использовать с помощью ARMClient, который вы можете получить из проекта [ARMClient][armclient] на сайте GitHub. Ниже приведен пример установки уже существующего гибридного подключения к вашему приложению. Создайте JSON-файл, используя приведенную выше схему, например:
...
## <a name="secure-your-hybrid-connections"></a>Защищайте гибридные соединения ##

Существующее гибридное соединение может быть добавлено в другие веб-приложения службы App любым пользователем, имеющим достаточное количество разрешений на базовый перевалку автобусов Azure. Это означает, что если необходимо запретить другим пользователям повторно использовать то же гибридное соединение (например, когда целевой ресурс является службой, которая не имеет каких-либо дополнительных мер безопасности для предотвращения несанкционированного доступа), необходимо заблокировать доступ к Azure Сервисная автобусная эстафета.

Любой `Reader` человек, у которого есть доступ к Ретрансити, сможет _увидеть_ гибридное соединение при попытке добавить его в web-приложение в порталaz, но они не смогут _добавить_ его, поскольку у них нет разрешений на получение строки соединения, которая используется для установления соединения реле. Для того, чтобы успешно добавить гибридное соединение, они должны иметь `listKeys` разрешение (`Microsoft.Relay/namespaces/hybridConnections/authorizationRules/listKeys/action`). Роль `Contributor` или любая другая роль, которая включает это разрешение в Реле, позволит пользователям использовать гибридное соединение и добавлять его в свои собственные веб-приложения.
