...
В портал Azure можно просматривать *квоты* и *метрики* для приложения и плана службы приложений, а также настраивать *оповещения* и *Автомасштабирование* , основанные на метриках.
...
| Метрика | Description |
| --- | --- |
| **Среднее время ответа** | Среднее время, затрачиваемое приложением на обслуживание запросов в секундах. |
...
| **Длина дисковой очереди** | среднее число запросов на чтение и запись, поставленных в очередь в хранилище. Высокая длина очереди диска — это указание приложения, которое может замедлить работу из-за чрезмерного ввода/вывода на диск. |
...
* Метрики с детализацией по **минутам** хранятся в течение 30 часов.
* Метрики степени детализации по **часам** хранятся в течение 30 дней.
* Метрики детализации по **дням** хранятся в течение 30 дней.
...
![диаграмма метрики в портал Azure][metrics] можно получить доступ к метрикам непосредственно на странице **обзора** ресурсов. Здесь вы увидите диаграммы, представляющие некоторые метрики приложений.
