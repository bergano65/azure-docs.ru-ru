---
title: Проблема домена и TLS / SSL сертификаты
description: Найдите решения общих проблем, с которыми можно столкнуться при настройке домена или сертификата TLS/SSL в службе приложений Azure.
author: genlin
...
ms.locfileid: "80668025"
---
# <a name="troubleshoot-domain-and-tlsssl-certificate-problems-in-azure-app-service"></a>Проблема устранения проблем и проблем с сертификатами TLS/SSL в службе приложений Azure

В этой статье перечислены общие проблемы, с которыми можно столкнуться при настройке домена или сертификата TLS/SSL для веб-приложений в Службе приложений Azure. Кроме того, здесь описаны возможные причины этих проблем и способы их устранения.
...
### <a name="you-cant-add-a-tlsssl-certificate-binding-to-an-app"></a>Вы не можете добавить привязку сертификата TLS/SSL к приложению 

#### <a name="symptom"></a>Симптом

При добавлении привязки TLS вы получаете следующее сообщение об ошибке:
...
"Невозможно удалить сертификат, поскольку он в настоящее время используется в привязке TLS/SSL. Привязка TLS должна быть удалена, прежде чем вы сможете удалить сертификат".
...
- Служба приложений имеет план уровня "Бесплатный" или "Общий". Эти уровни ценообразования не поддерживают TLS. 
...
Да. При доступе к пользовательским доменотам и лезвию TLS на портале Azure вы видите приобретенные домены. Вы можете настроить приложение, чтобы использовать любой из этих доменов.
