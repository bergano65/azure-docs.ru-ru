...
Добавьте параметр приложения с именем `WEBSITE_LOAD_CERTIFICATES` и присвойте ему значение отпечатка сертификата. Чтобы предоставить доступ к нескольким сертификатам сразу, укажите несколько значений отпечатков, разделив их запятыми. Чтобы сделать доступными все сертификаты, установите здесь значение `*`. Обратите внимание, что это поместит сертификат в хранилище `CurrentUser\My`.
