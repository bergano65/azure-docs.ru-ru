...
3. Присвойте проекту имя *вебжобссдксампле*, а затем выберите **создать**.
   ...
   ![Диалоговое окно «Создание проекта»](./media/webjobs-sdk-get-started/new-project.png)
...
1. Запустите проект снова.
   ...
   Поскольку в функции `QueueTrigger` использовался атрибут `ProcessQueueMessage`, среда выполнения пакета SDK WebJobs прослушивает запускаемые сообщения в очереди. Она находит новое сообщение в очереди с именем *queue* и вызывает функцию.
