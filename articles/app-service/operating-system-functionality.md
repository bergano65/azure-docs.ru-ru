...
description: Узнайте о функциональности ОС в службе приложений Azure на Windows. Узнайте, какие типы доступа к файлам, сети и реестру получает ваше приложение.
...
- Развертывание из Azure DevOps или `ERROR_NOT_ENOUGH_DISK_SPACE: Web deployment task failed. (Web Deploy detected insufficient space on disk)`Visual Studio может вывести из строя.
...
Код приложения может использовать протоколы на базе TCP/IP и UDP для установки исходящих сетевых подключений к доступным из Интернета конечным точкам, предоставляющим внешние службы. Приложения могут использовать эти же протоколы для подключения к службам в Azure, например, путем создания httpS-соединений к базе данных S'L.
