...
| 2. Последующая проверка подлинности | Поставщик перенаправляет клиент к `/.auth/login/<provider>/callback`. | Код клиента [отправляет маркер от поставщика](app-service-authentication-how-to.md#validate-tokens-from-providers) в `/.auth/login/<provider>` для проверки. |
