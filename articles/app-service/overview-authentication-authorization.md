...
description: Узнайте о встроенной поддержке аутентификации и авторизации в службах приложений Azure и функциях Azure, а также о том, как она может помочь обезопасить приложение от несанкционированного доступа.
...
### <a name="userapplication-claims"></a>Претензии пользователя/приложения

Для всех языковых инфраструктур Служба App Service делает утверждения в входящих маркерах (будь то от аутентифицированного конечнома пользователя или клиентского приложения) доступными для вашего кода, вводя их в заголовки запросов. Для приложений ASP.NET 4.6 служба приложений заполняет свойство [ClaimsPrincipal.Current](/dotnet/api/system.security.claims.claimsprincipal.current) утверждениями пользователя, прошедшими проверку подлинности, поэтому вы можете следовать стандартным шаблонам кода .NET, включая атрибут `[Authorize]`. Аналогичным образом для приложений PHP служба приложений заполняет переменную `_SERVER['REMOTE_USER']`. Для Java-приложений заявки [доступны из сервлета Tomcat.](containers/configure-language-java.md#authenticate-users-easy-auth)

Для [Azure](../azure-functions/functions-overview.md) `ClaimsPrincipal.Current` Functions , не заполняется для кода .NET, но вы все равно можете `ClaimsPrincipal` найти требования пользователя в заголовках запросов, или получить объект из контекста запроса или даже через обязательный параметр. Дополнительную информацию можно узнать [о работе с идентификационными данными клиента.](../azure-functions/functions-bindings-http-webhook-trigger.md#working-with-client-identities)
...
Токены идентификаторов, маркеры доступа и обновления кэшируются для проверки подлинности сеанса и доступны только ассоциированному пользователю.  
...
> [!NOTE]
> Звонки из надежного приложения браузера в Службе App в другой REST API в App Service или [Azure Functions](../azure-functions/functions-overview.md) могут быть проверены с помощью потока, направленного на сервер. Дополнительные сведения см. в статье [Настройка проверки подлинности и авторизации в Службе приложений Azure](app-service-authentication-how-to.md).
>
