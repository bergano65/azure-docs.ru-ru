...
> [!IMPORTANT]
> Вы не обязаны использовать службу приложений для AuthN и Аусо. Многие веб-платформы оснащены функциями безопасности, и при необходимости вы можете использовать их. Если требуется больше гибкости, чем предоставляет служба приложений, можно также написать собственные служебные программы.  
>
> Однако при использовании любого из вариантов службы, не относящейся к приложению, для удаленной проверки подлинности следует помнить, что [Chrome 80 вносит критические изменения в свою реализацию SameSite для файлов cookie](https://www.chromestatus.com/feature/5088147346030592) (Дата выпуска около марта 2020), и механизм проверки подлинности приложения может прерываться при обновлении клиентских браузеров. В ASP.NET Core документации содержатся сведения о том, как устранить эту эту работу в приложении, в [http: Browser SameSite изменяет влияние на проверку подлинности](/dotnet/core/compatibility/3.0-3.1#http-browser-samesite-changes-impact-authentication). Он содержит полезные рекомендации по тестированию критических изменений для основных браузеров, независимо от того, используется ASP.NET Core или нет.
>
