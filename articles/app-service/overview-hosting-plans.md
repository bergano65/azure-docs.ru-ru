...
description: Узнайте, как работают планы App Service в Службе приложений Azure, как они выставляются клиенту и как масштабировать их для ваших нужд.
...
- Ценовой уровень (бесплатный, общий, базовый, стандартный, премиум, PremiumV2, Изолированный)
...
- **Изолированный:** Этот уровень запускает выделенные Виртуальные ММ Azure на выделенных виртуальных сетях Azure. Он обеспечивает изоляцию сети поверх изоляции вычислений для ваших приложений. Обеспечивается максимальное горизонтальное масштабирование.

[!INCLUDE [app-service-dev-test-note](../../includes/app-service-dev-test-note.md)]

Каждый уровень также предоставляет подмножество функций службы приложений. Эти функции включают пользовательские домены и сертификаты TLS/SSL, автоматическое масштабирование, слоты для развертывания, резервные копирования, интеграцию менеджера трафика и многое другое. Чем выше уровень, тем больше доступно функций. Чтобы узнать, какие функции поддерживает каждая ценовая категория, ознакомьтесь с [подробными сведениями о планах службы приложений](https://azure.microsoft.com/pricing/details/app-service/plans/).

<a name="new-pricing-tier-premiumv2"></a>

> [!NOTE]
> В новой ценовой категории **Премиум V2** реализованы [виртуальные машины серии Dv2](../virtual-machines/dv2-dsv2-series.md) с более быстрыми процессорами, хранилищами на базе SSD и двойным соотношением ресурсов памяти и ядра по сравнению с категорией **Стандартный**. Категория **Премиум V2** также отличается более высокой масштабируемостью благодаря увеличенному числу экземпляров, предоставляя при этом все расширенные функции, доступные в рамках плана "Стандартный". Все функции, доступные в существующей категории **Премиум**, включены в категорию **Премиум V2**.
>
...
В **бесплатных** и **общих** уровнях приложение получает минуты процессора в общем экземпляре VM и не может масштабироваться. В других уровнях приложение работает и масштабируется следующим образом.
...
- В специализированных вычислительных уровнях **(Основные,** **Стандартные,** **Премиум**, **PremiumV2)** план Службы приложений определяет количество экземпляров VM, на которые масштабируются приложения, поэтому _каждый экземпляр VM_ в плане Службы Приложения имеет почасовую плату. Плата за экземпляры виртуальной машины является фиксированной, независимо от количества запущенных в них приложений. Чтобы избежать непредвиденных расходов, ознакомьтесь с разделом об [удалении плана службы приложений](app-service-plan-manage.md#delete).
...
Вы не получаете плату за использование функций Службы App, которые доступны для вас (настройка пользовательских доменов, tLS / SSL сертификаты, слоты развертывания, резервные копирования и т.д.). Однако имеются исключения.
...
- IP-соединения TLS на основе IP - За каждое ip-ip-сообщение TLS взимается почасовая плата, но некоторые **стандартные** стандарты или выше бесплатно дают одно IP-соединение TLS. Соединения на основе SNI TLS бесплатны.
