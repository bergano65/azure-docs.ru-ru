---
title: Перенос активного DNS-имени
...
При предварительной привязке пользовательского домена можно выполнить оба следующих действия, прежде чем вносить любые изменения в существующие записи DNS.
...
Чтобы проверить принадлежность домена, добавьте запись типа TXT для проверки домена. Имя узла для записи типа TXT зависит от типа записи DNS, которую необходимо сопоставлять. См. следующую таблицу ( `@` обычно представляет корневой домен):
...
1. В левой области навигации страницы приложения на [портале Azure](https://portal.azure.com) выберите **Личные домены**. 
...
1. Введите полное доменное имя, которое требуется перенести, соответствующее создаваемой записи TXT, например `contoso.com` , `www.contoso.com` или `*.contoso.com` . Выберите **Проверить**.

    Активируется кнопка **Добавить личный домен**. 

1. Убедитесь, что в качестве значения параметра **Тип записи имени узла** задан тип записи DNS, которую необходимо перенести. Выберите **Добавить имя узла**.
...
## <a name="migrate-domain-from-another-app"></a>Миграция домена из другого приложения
