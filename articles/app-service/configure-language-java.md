...
Чтобы задать выделенную память или другие параметры среды выполнения ВИРТУАЛЬНОЙ машины Java, создайте [параметр приложения](configure-common.md#configure-app-settings) с именем `JAVA_OPTS` с параметрами. Служба приложений передает этот параметр в среду выполнения Java при запуске в качестве переменной среды.
...
Наконец, мы поместим драйвер JAR в путь к классам Tomcat и перезапустите службу приложений. Убедитесь, что файлы драйвера JDBC доступны для Tomcat класслоадер, поместив их в каталог */Хоме/томкат/либ* . (Создайте этот каталог, если он еще не существует.) Чтобы передать эти файлы в экземпляр службы приложений, выполните следующие действия.
