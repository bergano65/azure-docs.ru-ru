...
Чтобы настроить приложение для получения сертификатов клиента, можно переключиться **на** "требовать входящий сертификат", выбрав **Конфигурация** > **Общие параметры** на портале Azure или `clientCertEnabled` задав для приложения параметр. `true` Чтобы задать параметр, выполните следующую команду в [Cloud Shell](https://shell.azure.com).
...
Пути исключения можно настроить, выбрав**Общие параметры** **конфигурации** > и определив путь исключения. В этом примере все, что `/public` находится в разделе Path для вашего приложения, не запрашивает сертификат клиента.
...
В службе приложений завершение запроса TLS происходит в интерфейсной подсистеме балансировки нагрузки. При пересылке запроса в код приложения с [включенными сертификатами клиента](#enable-client-certificates)служба приложений вставляет заголовок `X-ARR-ClientCert` запроса с сертификатом клиента. Служба приложений не выполняет никаких действий с этим сертификатом клиента, кроме перенаправления его в приложение. Код приложения отвечает за проверку сертификата клиента.

Для ASP.NET сертификат клиента можно получить с помощью свойства **HttpRequest. clientcertificate** .

Для других стеков приложений (Node. js, PHP и т. д.) сертификат клиента доступен в приложении через значение в заголовке `X-ARR-ClientCert` запроса в кодировке Base64.
