...
В [портале Azure]найдите и выберите **службы приложений**, а затем выберите свое приложение. 
...
В [портале Azure]найдите и выберите **службы приложений**, а затем выберите свое приложение. В меню слева приложения выберите **конфигурация** > **Параметры приложения**.

![Параметры приложения](./media/configure-common/open-ui.png)

Для разработчиков ASP.NET и ASP.NET Core установка строк подключения в службе приложений выполняется так же, как и в `<connectionStrings>` в *файле Web. config*, но значения, заданные в службе приложений, переопределяются в *файле Web. config*. Вы можете безопасно использовать параметры разработки (например, файл базы данных) в файле *Web. config* и рабочих секретах (например, учетные данные базы данных SQL) в службе приложений. Этот же код использует параметры разработки при локальной отладке и использует свои рабочие секреты при развертывании в Azure.
...
В [портале Azure]найдите и выберите **службы приложений**, а затем выберите свое приложение. В меню слева приложения выберите **конфигурация** > **Общие параметры**.
...
В [портале Azure]найдите и выберите **службы приложений**, а затем выберите свое приложение. В левом меню приложения выберите **Configuration** > **документы по умолчанию**.
...
В [портале Azure]найдите и выберите **службы приложений**, а затем выберите свое приложение. В меню слева приложения выберите **Конфигурация** **сопоставления путей** > .
