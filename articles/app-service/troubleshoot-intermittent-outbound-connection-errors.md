...
Если назначением является служба Azure, которая поддерживает конечные точки службы, можно избежать проблем нехватки портов SNAT с помощью [интеграции виртуальной сети](https://docs.microsoft.com/azure/app-service/web-sites-integrate-with-vnet) и конечных точек службы. При использовании интеграции с виртуальной сетью и размещении конечных точек службы в подсети интеграции исходящий трафик приложения к этим службам не будет иметь исходящих ограничений на порты SNAT.
...
* Чтобы реализовать проверку активности для приложений Node.js, изучите [мое приложение узла, которое делает избыточные исходящие вызовы](https://docs.microsoft.com/azure/app-service/app-service-web-nodejs-best-practices-and-troubleshoot-guide#my-node-application-is-making-excessive-outbound-calls).
