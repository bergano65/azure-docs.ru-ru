...
Популярные веб-платформы позволяют получить доступ к информации `X-Forwarded-*` в стандартном шаблоне приложения. В [Express](https://expressjs.com/), можно использовать [доверия прокси-серверы](http://expressjs.com/guide/behind-proxies.html). Например: 
