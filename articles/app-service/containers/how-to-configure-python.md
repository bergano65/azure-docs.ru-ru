...
Вы можете задать поведение при запуске контейнера, указав пользовательскую команду запуска Gunicorn. Например, если у вас есть приложение Flask, главным модулем которого является *hello.py*, и объект приложения Flask называется `myapp`, значит команда будет выглядеть следующим образом:
