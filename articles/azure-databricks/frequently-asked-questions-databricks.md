...
Дополнительные сведения см. в разделе [использование Azure Data Lake Storage с Azure Databricks](/azure/databricks/data/data-sources/azure/azure-datalake).
...
### <a name="issue-this-subscription-is-not-registered-to-use-the-namespace-microsoftdatabricks"></a>Проблема. Подписка не зарегистрирована для использования пространства имен "Microsoft. кирпичные"

#### <a name="error-message"></a>Сообщение об ошибке

"Эта подписка не зарегистрирована для использования пространства имен" Microsoft. кирпичные ". Чтобы узнать больше о регистрации подписок, перейдите на страницу https://aka.ms/rps-not-found . (Код: MissingSubscriptionRegistration)
...
* Эта ошибка также может возникнуть, если доменное имя из используемого адреса электронной почты назначено многим каталогам в Azure Active Directory. Чтобы обойти эту проблему, создайте нового пользователя в каталоге, который содержит подписку с рабочей областью Databricks.
    ...
    . Войдите на портал Azure с учетными данными нового пользователя и найдите нужную рабочую область Databricks.
...
"Cloud Provider Launch Failure: A cloud provider error was encountered while setting up the cluster" (Сбой запуска поставщика облачных служб: при настройке кластера произошла ошибка поставщика облачных служб). Дополнительные сведения см. в руководстве по Databricks. Код ошибки Azure: PublicIPCountLimitReached. Сообщение об ошибке Azure: не удалось создать более 10 общедоступных IP-адресов для этой подписки в этом регионе. "

#### <a name="background"></a>Фон

Кластеры кирпичей используют один общедоступный IP-адрес для каждого узла (включая узел драйвера). Подписки Azure имеют [ограничения общедоступных IP-адресов](/azure/azure-resource-manager/management/azure-subscription-service-limits#publicip-address) для каждого региона. Таким образом, операции создания кластера и масштабирования могут завершиться ошибкой, если это приведет к превышению предельного числа общедоступных IP-адресов, выделенных для этой подписки в этом регионе. Это ограничение также включает общедоступные IP-адреса, выделенные для использования модулями, не являющимися данными, например настраиваемые пользовательские виртуальные машины.

Как правило, кластеры используют только общедоступные IP-адреса, когда они активны. Однако `PublicIPCountLimitReached` ошибки могут продолжаться в течение короткого периода времени даже после завершения работы других кластеров. Это связано с тем, что модульные источники служб временно кэшируют ресурсы Azure при завершении работы кластера. Кэширование ресурсов — это проектирование, поскольку оно значительно сокращает задержку запуска кластера и автоматическое масштабирование во многих распространенных сценариях.

#### <a name="solution"></a>Решение

Если ваша подписка уже достигла предельного значения общедоступного IP-адреса для данного региона, то следует выполнить одно из следующих действий.

- Создайте новые кластеры в другой рабочей области. Другая рабочая область должна находиться в регионе, в котором вы не достигли ограничения общедоступного IP-адреса вашей подписки.
- [Запрос на увеличение предельного числа общедоступных IP-адресов](https://docs.microsoft.com/azure/azure-portal/supportability/resource-manager-core-quotas-request). Выберите значение **Квоты** для параметра **Тип проблемы** и значение **Сети: ARM** для параметра **Тип квоты**. В области **Сведения** сформулируйте просьбу увеличить квоту на общедоступные IP-адреса. Например, если установлен лимит 60, а вам нужно создать кластер на 100 узлов, попросите увеличить лимит до 160.
