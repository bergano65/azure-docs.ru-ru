...
4. Перейдите к созданной учетной записи хранения и выберите **BLOB-объекты** в разделе **Службы** на странице **Обзор**. Щелкните **+ Container** (+ Контейнер) и введите имя контейнера. Щелкните **ОК**.
...
2. Ресурс Key Vault выбирается автоматически. Нажмите кнопку **создания**.
...
2. В разделе **Служба Azure Databricks** укажите следующие значения, чтобы создать рабочую область Databricks:
   ...
   |Расположение     | Предназначено для выбора расположения, идентичного тому, где находится Azure Key Vault. Другие доступные регионы см. в статье о [доступности служб Azure по регионам](https://azure.microsoft.com/regions/services/).        |
