...
---
# <a name="tutorial-query-a-sql-server-linux-docker-container-in-a-virtual-network-from-an-azure-databricks-notebook"></a>Учебник. Запрос контейнера Docker в Linux SQL Server в виртуальной сети с Azure Databricks notebook
...
7. Добавьте правило для открытия порта 22 для SSH. Используйте следующие параметры:
    ...
    |-------|---------------|-----------|
    |Исходный код|IP-адреса|IP-адреса указывает, что входящий трафик из определенного источника, IP-адреса будут разрешены или запрещены этим правилом.|
    ...
    |Конечный файл|IP-адреса|IP-адреса указывает, что исходящий трафик для определенного источника, IP-адреса будут разрешены или запрещены этим правилом.|
    ...
    |Название|ssh-databricks-tutorial-vm|Введите имя правила.|
...
8. Добавьте правило, чтобы открыть порт 1433 для SQL со следующими параметрами:
    ...
    |Название|sql-databricks-tutorial-vm|Введите имя правила.|
...
## <a name="create-a-sql-database"></a>Создать базу данных SQL
