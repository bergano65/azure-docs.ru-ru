...
7. Добавьте правило для открытия порта 22 для SSH. Используйте следующие параметры:
    ...
    |-------|---------------|-----------|
    |source|IP-адреса|IP-адреса указывает, что входящий трафик из определенного источника, IP-адреса будут разрешены или запрещены этим правилом.|
...
8. Добавьте правило, чтобы открыть порт 1433 для SQL со следующими параметрами:
    ...
    |Name|sql-databricks-tutorial-vm|Введите имя правила.|
