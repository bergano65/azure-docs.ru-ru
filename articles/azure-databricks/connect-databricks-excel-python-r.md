...
5. В диалоговом окне **Simba Spark ODBC Driver** (Драйвер ODBC для Simba Spark) введите следующие значения.
    ...
    |**Узлы (-ы)**     | Укажите скопированное из рабочей области Databricks значение *имени узла сервера*.        |
    |**порт**.     | Введите *443*.        |
    |**Механизм проверки подлинности** > **Mechanism**     | Выберите *Имя пользователя и пароль*.        |
    |**User name**     | Введите *маркер*.        |
...
* Если вы используете RStudio для рабочего стола в качестве интегрированной среды разработки, [https://aka.ms/rclient/](https://aka.ms/rclient/)также установите Microsoft R Client из. 
