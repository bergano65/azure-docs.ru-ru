---
title: Схема безопасности и соответствия требованиям Azure. Трехуровневое веб-приложение IaaS для UK OFFICIAL
description: Схема безопасности и соответствия требованиям Azure. Трехуровневое веб-приложение IaaS для UK OFFICIAL
services: security
author: jomolesk
...
## <a name="overview"></a>Обзор
 ...
 Текущая архитектура и соответствующие ей шаблоны Azure Resource Manager поддерживаются технической документацией корпорации Майкрософт, [14 Cloud Security Controls for UK cloud Using Microsoft Azure](https://gallery.technet.microsoft.com/14-Cloud-Security-Controls-670292c1) (14 методов управления безопасностью облака в Соединенном Королевстве с использованием Microsoft Azure). В этой статье вы узнаете, как службы Azure соответствуют принципам облачной безопасности Великобритании NCSC 14, тем самым позволяя организациям быстро отменять свои возможности в соответствии с обязательствами по соответствию, используя облачные службы глобально и в Великобритании на Microsoft Azure вычислений.
...
![Схема эталонной архитектуры трехуровневого веб-приложения IaaS для UK OFFICIAL](images/ukofficial-iaaswa-architecture.png?raw=true "Схема эталонной архитектуры трехуровневого веб-приложения IaaS для UK OFFICIAL")
...
**Шлюз Интернета и общедоступный IP-адрес**. Шлюз Интернета предоставляет службы приложений пользователям через Интернет. Безопасность трафика, который обращается к этим службам, обеспечивается за счет возможностей маршрутизации и балансировки нагрузки 7 уровня [шлюза приложений](../../application-gateway/overview.md) и защиты брандмауэра веб-приложения (WAF).
...
**Группы безопасности сети**. Группы [NSG](../../virtual-network/virtual-network-vnet-plan-design-arm.md) содержат списки управления доступом, которые разрешают или запрещают трафик в виртуальной сети. Группы безопасности сети можно использовать для защиты трафика на уровне отдельных виртуальных машин или подсети.
...
**Мониторинг**. [Azure Monitor](../../azure-monitor/overview.md) — это служба платформы, которая предоставляет один источник для мониторинга журнала действий, метрик и журналов диагностики всех ресурсов Azure. Azure Monitor можно настроить для визуализации, запроса, маршрутизации, архивирования и работы с метриками и журналами, поступающими из ресурсов в Azure. Рекомендуется использовать управление доступом на основе ресурсов для защиты журнала аудита, чтобы пользователи не могли изменять журналы.

**Журналы действий**. Настройте [журналы действий Azure](../../azure-monitor/platform/activity-logs-overview.md), чтобы предоставить информацию об операциях, которые выполнялись в ресурсах в подписке.

**Журналы диагностики**. [Это](../../azure-monitor/platform/resource-logs-overview.md) все журналы, создаваемые ресурсами. Эти журналы могут включать системные журналы событий Windows, журналы больших двоичных объектов, таблиц и очередей.
...
**Архивация журналов**. Хранилище данных журнала можно настроить для записи в централизованную учетную запись хранения Azure для архивации и определенного срока хранения. Журналы можно обрабатывать с помощью Azure Monitor журналов или сторонних SIEM систем.
...
[Расширение доменных служб Active Directory в Azure](/azure/architecture/reference-architectures/identity/adds-extend-domain).
...
**Обеспечение безопасности сети**. [Группы безопасности сети](../../virtual-network/virtual-network-vnet-plan-design-arm.md) рекомендуется использовать в каждой подсети, чтобы обеспечить второй уровень защиты от входящего трафика, проходящего через неправильно настроенный или отключенный шлюз. Пример. [Шаблон Resource Manager для развертывания группы безопасности сети](https://github.com/mspnp/template-building-blocks/tree/v1.0.0/templates/buildingBlocks/networkSecurityGroups).

**Обеспечение безопасности общедоступных конечных точек**. Шлюз Интернета предоставляет службы приложений пользователям через Интернет. Трафик, получающий доступ к этим службам, защищен с помощью [шлюза приложений](../../application-gateway/overview.md), который предоставляет брандмауэр веб-приложения и протокол управления HTTPS.
...
**Ограничение контроля доступа**. Используйте [управление доступом на основе ролей](../../role-based-access-control/role-assignments-portal.md) (RBAC), чтобы управлять ресурсами приложения с помощью [пользовательских ролей](../../role-based-access-control/custom-roles.md) RBAC, позволяющих ограничить операции, которые DevOps может выполнять на каждом из уровней. При предоставлении разрешений используйте [принцип минимальных привилегий](https://msdn.microsoft.com/library/hdb58b2f(v=vs.110).aspx#Anchor_1). Ведите журнал всех административных операций и проводите регулярный аудит, чтобы следить за тем, все ли изменения конфигурации были запланированы.

**Доступ к Интернету**. Эта Эталонная архитектура использует [шлюз приложений Azure](../../application-gateway/overview.md) в качестве шлюза с выходом в Интернет и балансировщика нагрузки. В качестве альтернативы [шлюзу приложений Azure](../../application-gateway/overview.md) некоторые клиенты могут попробовать использовать сетевые виртуальные модули сторонних производителей, чтобы получить дополнительные уровни сетевой безопасности.
...
В [таблице ответственности клиентов](https://aka.ms/ukofficial-crm) (книга Excel) перечислены все 14 принципов облачной безопасности с указанием того, кто отвечает за реализацию принципа (или его части): корпорация Майкрософт, клиент или они оба.

В [таблице реализации принципов](https://aka.ms/ukofficial-iaaswa-pim) (книга Excel) перечислены все 14 принципов облачной безопасности. Для каждого принципа (или его части), за который согласно таблице ответственности клиентов отвечает клиент, указано: (1) реализует ли служба автоматизации схемы этот принцип, и (2) описание того, как реализация согласовывается с требованиями к принципу.
...
Есть два метода, которые пользователи могут использовать для развертывания этой автоматизации схемы. В первом методе используется скрипт PowerShell, тогда как второй метод использует портал Azure для развертывания эталонной архитектуры. Подробные инструкции по развертыванию см. в [этой статье](https://aka.ms/ukofficial-iaaswa-repo).
