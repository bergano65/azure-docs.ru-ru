...
## <a name="overview"></a>Обзор
 ...
 Текущая архитектура и соответствующие ей шаблоны Azure Resource Manager поддерживаются технической документацией корпорации Майкрософт, [14 Cloud Security Controls for UK cloud Using Microsoft Azure](https://gallery.technet.microsoft.com/14-Cloud-Security-Controls-670292c1) (14 методов управления безопасностью облака в Соединенном Королевстве с использованием Microsoft Azure). В этом документе категорий о службах Azure соответствуют Великобритании 14 принципов облачной безопасности NCSC, тем самым позволяя организациям быстро отслеживать способность в соответствии с их выполнение обязательств по соответствию с помощью облачных служб глобально, так и в Соединенном Королевстве в Microsoft Azure облако.
...
**Журналы действий**. Настройте [журналы действий Azure](https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitoring-overview-activity-logs), чтобы предоставить информацию об операциях, которые выполнялись в ресурсах в подписке.

**Журналы диагностики**. [Это](https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitoring-overview-of-diagnostic-logs) все журналы, создаваемые ресурсами. Эти журналы могут включать системные журналы событий Windows, журналы больших двоичных объектов, таблиц и очередей.
...
**Архивация журналов**. Хранилище данных журнала можно настроить для записи в централизованную учетную запись Azure для архивирования и в течение определенного срока. Журналы могут быть обработаны с помощью журналов Azure Monitor или с системами SIEM сторонних производителей.
...
**Обеспечение безопасности сети**. [Группы безопасности сети](https://docs.microsoft.com/azure/virtual-network/virtual-networks-nsg) рекомендуется использовать в каждой подсети, чтобы обеспечить второй уровень защиты от входящего трафика, проходящего через неправильно настроенный или отключенный шлюз. Пример. [Шаблон Resource Manager для развертывания группы безопасности сети](https://github.com/mspnp/template-building-blocks/tree/v1.0.0/templates/buildingBlocks/networkSecurityGroups).
...
**Доступ к Интернету**. Эта Эталонная архитектура использует [шлюза приложений Azure](https://docs.microsoft.com/azure/application-gateway/application-gateway-introduction) как шлюз и балансировщика нагрузки для Интернета. В качестве альтернативы [шлюзу приложений Azure](https://docs.microsoft.com/azure/application-gateway/application-gateway-introduction) некоторые клиенты могут попробовать использовать сетевые виртуальные модули сторонних производителей, чтобы получить дополнительные уровни сетевой безопасности.
...
Есть два метода, которые пользователи могут использовать для развертывания этой автоматизации схемы. Первый метод используется сценарий PowerShell, а второй метод использует портал Azure для развертывания эталонной архитектуры. Подробные инструкции по развертыванию см. в [этой статье](https://aka.ms/ukofficial-iaaswa-repo).
