---
title: Схема безопасности и соответствия требованиям Azure. Аналитика данных для FFIEC
description: Схема безопасности и соответствия требованиям Azure. Аналитика данных для FFIEC
services: security
author: meladie
...
ms.locfileid: "71257265"
---
# <a name="azure-security-and-compliance-blueprint-analytics-for-ffiec-financial-services"></a>Схема безопасности и соответствия требованиям Azure. Аналитика для финансовых служб FFIEC

## <a name="overview"></a>Краткое описание

Эта схема безопасности и соответствия требованиям Azure является руководством по развертыванию архитектуры аналитики данных в Azure, подходящей для сбора, хранения и получения финансовых данных, регулируемых Федеральным советом по надзору за финансовыми учреждениями (Federal Financial Institution Examination Council, FFIEC).

Это руководство по реализации эталонной архитектуры и модель рисков являются основополагающими документами для клиентов, помогающими обеспечить соответствие требованиям FFIEC. В этом решении представлен базовый план, с помощью которого клиенты могут развертывать рабочие нагрузки в Azure в соответствии со стандартами FFIEC. Он не должен использоваться "как есть" в рабочей среде, так как требуется дополнительная настройка.
...
![Схема эталонной архитектуры аналитики для FFIEC](images/ffiec-analytics-architecture.png "Схема эталонной архитектуры аналитики для FFIEC")
...
**Машинное обучение Azure служба**: [машинное обучение Azure](https://docs.microsoft.com/azure/machine-learning/service/) — это методика обработки и анализа данных, которая позволяет компьютерам использовать существующие данные для прогнозирования будущих поведений, результатов и тенденций.

**Каталог данных Azure**. Благодаря [каталогу данных](../../data-catalog/overview.md) источники данных легко обнаруживаются и являются понятными для пользователей, которые управляют данными. Общие источники данных можно регистрировать, снабжать тегами и использовать для поиска финансовых данных. Эти данные остаются в существующем расположении, однако копия этих метаданных добавляется в каталог данных вместе со ссылкой на расположение источника данных. Кроме того, чтобы облегчить обнаружение каждого источника данных с помощью функции поиска и сделать их доступными для пользователей, метаданные индексируются.
...
- Журналы Azure Monitor подключены к [журналам диагностики группы&#39;безопасности сети](https://github.com/krnese/AzureDeploy/blob/master/AzureMgmt/AzureMonitor/nsgWithDiagnostics.json) .
...
- **Журналы действий.** [Журналы действий](../../azure-monitor/platform/activity-logs-overview.md) содержат информацию об операциях, которые выполнялись с ресурсами в подписке. Журналы действий помогают определить инициатора операции, время события и состояние.
- **Журналы диагностики.** [Журналы диагностики](../../azure-monitor/platform/resource-logs-overview.md) включают в себя все журналы, создаваемые каждым ресурсом. К этим журналам относятся системные журналы событий Windows, журналы службы хранилища Azure, журналы аудита Key Vault, а также журналы брандмауэра и доступа к шлюзу приложений. Все журналы диагностики обеспечивают запись данных в централизованную и зашифрованную учетную запись Azure для архивирования. В соответствии с требованиями конкретной организации пользователь может настроить срок хранения до 730 дней.

**Журналы Azure Monitor**. Эти журналы консолидируются в [журналы Azure Monitor](https://azure.microsoft.com/services/log-analytics/) для обработки, хранения и отчетов панели мониторинга. Собранные данные объединяются в отдельные таблицы для каждого типа данных в рабочих областях Log Analytics, что позволяет анализировать все данные независимо от первичного источника. Кроме того, центр безопасности Azure интегрируется с журналами Azure Monitor, что позволяет клиентам использовать запросы Kusto для доступа к данным событий безопасности и объединения их с данными из других служб.

В состав этой архитектуры входят следующие [решения мониторинга](../../monitoring/monitoring-solutions.md) Azure:
-   [Оценка Active Directory](../../azure-monitor/insights/ad-assessment.md). Решение "Проверка работоспособности Active Directory" регулярно оценивает риски и работоспособность серверных сред и предоставляет список рекомендаций, относящихся к развернутой серверной инфраструктуре, в порядке приоритетности.
...
-   [Аналитика журнала действий](../../azure-monitor/platform/collect-activity-logs.md). Решение "Аналитика журнала действий" помогает анализировать журналы действий Azure во всех подписках Azure для клиента.
...
Схему потока данных для этой эталонной архитектуры можно [скачать](https://aka.ms/ffiec-analytics-tm) или просмотреть ниже. Используя эту модель, клиенты могут найти зоны потенциального риска в инфраструктуре системы при внесении изменений.

![Аналитика для модели угроз FFIEC](images/ffiec-analytics-threat-model.png "Аналитика для модели угроз FFIEC")

## <a name="compliance-documentation"></a>Документация по соответствию

В документе [Схема безопасности и соответствия требованиям Azure. Таблица ответственности клиента согласно FFIEC](https://aka.ms/ffiec-crm) указаны все нормативы FFIEC. В этой таблице указано, кто отвечает за реализацию каждого норматива: корпорация Майкрософт, клиент или все вместе.

В документе [Схема безопасности и соответствия требованиям Azure. Таблица ответственности клиента согласно FFIEC](https://aka.ms/ffiec-analytics-cim) содержатся сведения о том, какие нормативы FFIEC отражены в архитектуре хранилища данных, включая подробное описание того, как реализация соответствует требованиям каждого затронутого норматива.
