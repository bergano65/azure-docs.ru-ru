...
**Log Analytics**. Эти журналы объединяются в [Log Analytics](https://azure.microsoft.com/services/log-analytics/) для обработки, хранения данных и создания отчетов на панелях мониторинга. После сбора данные объединяются в отдельные таблицы для каждого типа данных, что позволяет анализировать все данные независимо от исходного источника. Кроме того, центр безопасности Azure интегрируется с Log Analytics, в результате чего клиенты могут использовать запросы Log Analytics для доступа к своим данным событий безопасности и сочетать их с данными из других служб.
