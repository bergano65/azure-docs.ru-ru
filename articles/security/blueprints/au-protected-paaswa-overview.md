...
В каждой группе безопасности сети открыты определенные порты и протоколы, чтобы решение могло работать безопасно и правильно. Кроме того, для каждой группы безопасности сети настроены следующие конфигурации:
  ...
  - Журналы Azure Monitor подключен к [средствами диагностики для группы безопасности сети](https://github.com/krnese/AzureDeploy/blob/master/AzureMgmt/AzureMonitor/nsgWithDiagnostics.json)
...
Кроме того все транзакции в Azure на портале управления Azure происходит через HTTPS с использованием TLS версии 1.2.
