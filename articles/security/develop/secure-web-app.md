...
В примере приложения используется сквозное SSL-шифрование для транзитных данных, входящих в сеть и выходящих из нее. Шлюз настроен с сертификатом, подписанным самостоятельно.
> [!IMPORTANT]
...
Для получения дополнительной информации о том, как настроить SSL и сертификат органа (CA) проверки для PostgreS'L, [см.](/azure/postgresql/concepts-ssl-connection-security)
...
Подробнее о том, как настроить SSL безопасности для PostgreS'L здесь [Нанастройка SSL безопасности подключения](/azure/postgresql/concepts-ssl-connection-security).
...
5. На **странице Разъемов данных Azure Sentinel - Под** **конфигурацией**выберите **разъемы данных.** Вы видите массив служб Azure, которые можно связать с экземпляром хранения журналов Analytics для анализа в Azure Sentinel.
   ...
   4. На странице **«Диагностические настройки»** выберите созданное вами рабочее пространство «Аналитика журнала», а затем выберите все метрики, которые вы хотите собрать и отправить в Azure Sentinel. Нажмите кнопку **Сохранить**.
