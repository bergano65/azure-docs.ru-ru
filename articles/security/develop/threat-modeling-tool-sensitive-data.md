...
| **Базы данных** | <ul><li>[Реализуйте динамическую маскировку данных, чтобы уменьшить шансы раскрытия конфиденциальных данных для непривилегированных пользователей](#dynamic-users)</li><li>[Обеспечьте хранение паролей в формате хэша с использованием случайной строки данных](#salted-hash)</li><li>[Обеспечить шифрование конфиденциальных данных в столбцах баз данных](#db-encrypted)</li><li>[Обеспечить включение шифрования на уровне базы данных (TDE)](#tde-enabled)</li><li>[Зашифруйте резервные копии базы данных](#backup)</li></ul> | 
| **Веб-API** | <ul><li>[В хранилище браузера не должны храниться конфиденциальные данные, относящиеся к веб-API](#api-browser)</li></ul> | 
| Azure DocumentDB | <ul><li>[Шифрование конфиденциальных данных, хранящихся в Cosmos DB](#encrypt-docdb)</li></ul> | 
| **Граница доверия виртуальной машины IaaS Azure** | <ul><li>[Используйте шифрование дисков Azure для шифрования дисков, используемых виртуальными машинами](#disk-vm)</li></ul> | 
| **Граница доверия Service Fabric** | <ul><li>[Шифрование секретов в приложениях Service Fabric](#fabric-apps)</li></ul> | 
...
| **Мобильный клиент** | <ul><li>[Шифруйте конфиденциальные данные или личные данные, записываемые в локальное хранилище телефонов](#pii-phones)</li><li>[Запутать генерируемые файлы перед распространением среди конечных пользователей](#binaries-end)</li></ul> | 
| **WCF** | <ul><li>[Установка clientCredentialType на сертификат или Windows](#cert)</li><li>[Не включен режим безопасности WCF](#security)</li></ul> | 

## <a name="ensure-that-binaries-are-obfuscated-if-they-contain-sensitive-information"></a><a id="binaries-info"></a>Двоичные файлы, содержащие конфиденциальные данные, должны быть замаскированы
...
## <a name="consider-using-encrypted-file-system-efs-is-used-to-protect-confidential-user-specific-data"></a><a id="efs-user"></a>Используйте шифрование на уровне файловой системы (EFS) для защиты конфиденциальных данных пользователя
...
## <a name="ensure-that-sensitive-data-stored-by-the-application-on-the-file-system-is-encrypted"></a><a id="filesystem"></a>Настройте шифрование конфиденциальных данных, сохраняемых приложением в файловой системе
...
## <a name="ensure-that-sensitive-content-is-not-cached-on-the-browser"></a><a id="cache-browser"></a>Конфиденциальное содержимое не должно кэшироваться в браузере
...
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | Недоступно  |
| **Шаги** | Браузеры могут сохранять информацию для кэширования и ведения истории. Эти кэшированные файлы хранятся в папке, подобной папке временных файлов Интернета в Internet Explorer. При повторном обращении к страницам браузер отображает их из кэша. Если конфиденциальная информация отображается пользователю (например, его адрес, данные кредитной карты, номер социального страхования или имя пользователя), то эта информация может храниться в кэше браузера и, следовательно, извлекаться путем изучения кэша браузера или просто нажав кнопку "Назад" браузера. Установите значение заголовка ответа управления кэшем на "нехранилище" для всех страниц. |
...
## <a name="encrypt-sections-of-web-apps-configuration-files-that-contain-sensitive-data"></a><a id="encrypt-data"></a>Зашифруйте разделы файлов конфигурации веб-приложения, содержащие конфиденциальные данные
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Руководство по шифрованию разделов конфигурации в ASP.NET 2.0 с помощью DPAPI](https://msdn.microsoft.com/library/ff647398.aspx), [сведения об указании поставщика защищенной конфигурации](https://msdn.microsoft.com/library/68ze1hb2.aspx), [использование Azure Key Vault для защиты секретов приложения](https://azure.microsoft.com/documentation/articles/guidance-multitenant-identity-keyvault/) |
...
## <a name="explicitly-disable-the-autocomplete-html-attribute-in-sensitive-forms-and-inputs"></a><a id="autocomplete-input"></a>Явным образом отключите автозаполнение атрибута HTML в конфиденциальных формах и входных данных
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Сведения об атрибуте autocomplete на сайте MSDN](https://msdn.microsoft.com/library/ms533486(VS.85).aspx), [использование функции автозаполнение в формах HTML](https://msdn.microsoft.com/library/ms533032.aspx), [Бюллетень по безопасности (Майкрософт) MS10-071 — критическое](https://technet.microsoft.com/security/bulletin/MS10-071), [запись блога о функции автозаполнения](https://blog.mindedsecurity.com/2011/10/autocompleteagain.html) |
...
## <a name="ensure-that-sensitive-data-displayed-on-the-user-screen-is-masked"></a><a id="data-mask"></a>Замаскируйте конфиденциальные данные, отображаемые на экране пользователя
...
## <a name="implement-dynamic-data-masking-to-limit-sensitive-data-exposure-non-privileged-users"></a><a id="dynamic-users"></a>Реализуйте динамическую маскировку данных, чтобы уменьшить шансы раскрытия конфиденциальных данных для непривилегированных пользователей
...
| **Ссылки**              | [Динамическая маскировка данных](https://msdn.microsoft.com/library/mt130841) |
...
## <a name="ensure-that-passwords-are-stored-in-salted-hash-format"></a><a id="salted-hash"></a>Обеспечьте хранение паролей в формате хэша с использованием случайной строки данных
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Хеширование паролей с использованием API .NET для шифрования](https://docs.asp.net/en/latest/security/data-protection/consumer-apis/password-hashing.html) |
...
## <a name="ensure-that-sensitive-data-in-database-columns-is-encrypted"></a><a id="db-encrypted"></a>Шифруйте конфиденциальные данные в столбцах базы данных
...
| **Ссылки**              | [Контрольный список. Шифрование конфиденциальных данных](https://technet.microsoft.com/library/ff848751(v=sql.105).aspx), [Шифрование столбца данных](https://msdn.microsoft.com/library/ms179331), [ENCRYPTBYCERT (Transact-SQL)](https://msdn.microsoft.com/library/ms188061) |
...
## <a name="ensure-that-database-level-encryption-tde-is-enabled"></a><a id="tde-enabled"></a>Включите шифрование уровня базы данных (TDE)
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Основные сведения о прозрачном шифровании данных (TDE)](https://technet.microsoft.com/library/bb934049(v=sql.105).aspx) |
...
## <a name="ensure-that-database-backups-are-encrypted"></a><a id="backup"></a>Зашифруйте резервные копии базы данных
...
| **Ссылки**              | [Шифрование резервной копии](https://msdn.microsoft.com/library/dn449489) |
...
## <a name="ensure-that-sensitive-data-relevant-to-web-api-is-not-stored-in-browsers-storage"></a><a id="api-browser"></a>В хранилище браузера не должны храниться конфиденциальные данные, относящиеся к веб-API
...
## <a name="encrypt-sensitive-data-stored-in-cosmos-db"></a><a id="encrypt-docdb"></a>Шифрование конфиденциальных данных, хранящихся в Cosmos DB
...
## <a name="use-azure-disk-encryption-to-encrypt-disks-used-by-virtual-machines"></a><a id="disk-vm"></a>Шифруйте диски, используемые виртуальными машинами, с помощью шифрования дисков Azure
...
| **Ссылки**              | [Управление секретами в приложениях Service Fabric](https://azure.microsoft.com/documentation/articles/service-fabric-application-secret-management/) |
...
## <a name="perform-security-modeling-and-use-business-unitsteams-where-required"></a><a id="modeling-teams"></a>Выполняйте моделирование безопасности, при необходимости привлекая рабочие подразделения и группы
...
## <a name="minimize-access-to-share-feature-on-critical-entities"></a><a id="entities"></a>Ограничьте доступ к общим функциям критически важных сущностей
...
## <a name="train-users-on-the-risks-associated-with-the-dynamics-crm-share-feature-and-good-security-practices"></a><a id="good-practices"></a>Информируйте пользователей о рисках, связанных с использованием функции "Поделиться" Dynamics CRM, и предоставьте им рекомендации безопасности
...
## <a name="include-a-development-standards-rule-proscribing-showing-config-details-in-exception-management"></a><a id="exception-mgmt"></a>Включите стандартные правила разработки, запрещающие показ данных о конфигурации в управлении исключениями
...
## <a name="use-azure-storage-service-encryption-sse-for-data-at-rest-preview"></a><a id="sse-preview"></a>Используйте шифрование службы хранилища Azure (SSE) для неактивных данных (предварительная версия)
...
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Создание политики конфигурации](https://docs.microsoft.com/mem/intune/configuration/), [сведения об обслуживании цепочки ключей](https://components.xamarin.com/view/square.valet) |
| **Шаги** | <p>Перед записью в локальную файловую систему персональные данные пользователя (электронная почта, номер телефона, имя, фамилия, параметры и т. д.), которые приложение записывает в файловую систему мобильного устройства, должны быть зашифрованы. Если приложение является корпоративным, рассмотрите возможности публикации приложения с помощью Windows Intune.</p>|
...
## <a name="obfuscate-generated-binaries-before-distributing-to-end-users"></a><a id="binaries-end"></a>Маскируйте созданные двоичные файлы перед их распространением конечным пользователям
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Crypto Obfuscator для .NET](https://www.ssware.com/cryptoobfuscator/obfuscator-net.htm) |
...
## <a name="set-clientcredentialtype-to-certificate-or-windows"></a><a id="cert"></a>Установите для параметра clientCredentialType значение Certificate или Windows
...
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Fortify](https://vulncat.fortify.com/en/detail?id=desc.config.dotnet.wcf_misconfiguration_weak_token) |
...
## <a name="wcf-security-mode-is-not-enabled"></a><a id="security"></a>Не включен режим безопасности WCF
...
| **Ссылки**              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx), [Fortify Kingdom](https://vulncat.fortify.com/en/detail?id=desc.config.dotnet.wcf_misconfiguration_weak_class_reference), [основы безопасности WCF](https://www.codemag.com/article/0611051) |
