...
| --------------- | ------- |
| **Отношения доверия между компьютерами** | <ul><li>[Убедитесь, что соответствующие списки ACL настроены для ограничения несанкционированного доступа к данным на устройстве.](#acl-restricted-access)</li><li>[Конфиденциальное пользовательское содержимое приложения должно храниться в каталоге профиля пользователя](#sensitive-directory)</li><li>[Убедитесь, что развернутые приложения запускаются с минимальными привилегиями.](#deployed-privileges)</li></ul> |
| **Веб-приложение** | <ul><li>[Принудительный порядок последовательного шага при обработке потоков бизнес-логики](#sequential-logic)</li><li>[Реализуйте ограничение частоты транзакций, чтобы предотвратить перечисление](#rate-enumeration)</li><li>[Проверка правильности авторизации и соблюдение принципа минимальных привилегий](#principle-least-privilege)</li><li>[Бизнес-логика и принятие решений об авторизации доступа к ресурсам не должны основываться на параметрах входящего запроса](#logic-request-parameters)</li><li>[Убедитесь, что содержимое и ресурсы не могут быть перечислены или доступны путем несанкционированного просмотра](#enumerable-browsing)</li></ul> |
| **База данных** | <ul><li>[Убедитесь, что для подключения к серверу базы данных используются учетные записи с минимальными привилегиями доступа](#privileged-server)</li><li>[Реализация безопасность на уровне строк RLS для предотвращения доступа клиентов к данным друг друга](#rls-tenants)</li><li>[Роль системного администратора должна быть только у необходимых действительных пользователей](#sysadmin-users)</li></ul> |
...
| **концентратору событий Azure** | <ul><li>[Для создания маркеров устройства используйте ключ SAS с разрешениями только для отправки](#sendonly-sas)</li><li>[Не используйте маркеры доступа, которые предоставляют прямой доступ к концентратору событий](#access-tokens-hub)</li><li>[Подключайтесь к концентратору событий, используя ключи SAS с минимальными необходимыми разрешениями](#sas-minimum-permissions)</li></ul> |
| **Azure DocumentDB** | <ul><li>[Использование маркеров ресурса (по возможности) для подключения к Cosmos DB](#resource-docdb)</li></ul> |
| **Граница доверия Azure** | <ul><li>[Обеспечение детального управления доступом к подписке Azure с помощью RBAC](#grained-rbac)</li></ul> |
...
| **Ссылки**              | [Управление доступом к центру Интернета вещей](https://azure.microsoft.com/documentation/articles/iot-hub-devguide/#Security) |
...
| **Шаги** | В некоторых бизнес-сценариях для хранения конфиденциальных данных, предназначенных для различных сторон, может потребоваться хранилище таблиц Azure. Например конфиденциальные данные, относящиеся к разным странам и регионам. В таких случаях сигнатуры SAS можно создать, указав диапазоны ключей секций и строк, чтобы пользователь мог получить доступ к данным, относящимся к конкретной стране или региону.| 
