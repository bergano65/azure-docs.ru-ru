...
ms.locfileid: "72244632"
---
# <a name="security-frame-authentication--mitigations"></a>Механизм безопасности. Проверка подлинности | Устранение рисков 

| Продукт или служба | Статья |
| --------------- | ------- |
| **Веб-приложение**    | <ul><li>[Рассмотрите возможность использования стандартного механизма аутентификации для проверки подлинности веб-приложений](#standard-authn-web-app)</li><li>[Приложения должны надежно обрабатывать сценарии неудачной проверки подлинности](#handle-failed-authn)</li><li>[Включить усиление или адаптивную аутентификацию](#step-up-adaptive-authn)</li><li>[Интерфейсы администрирования должны быть заблокированы соответствующим образом](#admin-interface-lockdown)</li><li>[Безопасно реализуйте запоминание функций паролей](#forgot-pword-fxn)</li><li>[Обеспечить выполнение политики пароля и учетной записи](#pword-account-policy)</li><li>[Реализация элементов управления для предотвращения перечисления имени пользователя](#controls-username-enum)</li></ul> |
| **Базы данных** | <ul><li>[Когда это возможно, используйте аутентификацию Windows для подключения к серверу S'L](#win-authn-sql)</li><li>[При возможности используйте активную проверку подлинности каталогов Azure для подключения к базе данных S'L](#aad-authn-sql)</li><li>[Если используется режим проверки подлинности SQL, обеспечьте применение политики учетных записей и паролей на сервере SQL Server](#authn-account-pword)</li><li>[Не используйте проверку подлинности СЗЛ в содержащихся базах данных](#autn-contained-db)</li></ul> |
...
| **Граница доверия Service Fabric** | <ul><li>[Ограничьте анонимный доступ к кластеру Service Fabric](#anon-access-cluster)</li><li>[Убедитесь, что сертификат от клиента к узлам Service Fabric отличается от сертификата узла к узлам](#fabric-cn-nn)</li><li>[Используйте AAD для аутентификации клиентов для обслуживания кластеров тканей](#aad-client-fabric)</li><li>[Нужно использовать сертификаты Service Fabric, полученные из утвержденного Центра сертификации](#fabric-cert-ca)</li></ul> |
| **Сервер удостоверений** | <ul><li>[Используйте стандартные сценарии проверки подлинности, поддерживаемые Identity Server](#standard-authn-id)</li><li>[Переопределить кэш маркера маркера сервера идентификации по умолчанию с помощью масштабируемой альтернативы](#override-token)</li></ul> |
| **Отношения доверия между компьютерами** | <ul><li>[Двоичные файлы развернутого приложения должны иметь цифровую подпись](#binaries-signed)</li></ul> |
| **WCF** | <ul><li>[Включить аутентификацию при подключении к очередям МСМЗ в WCF](#msmq-queues)</li><li>[WCF: не присваивайте атрибуту message clientCredentialType значение None](#message-none)</li><li>[WCF-Не устанавливайте Транспорт clientCredentialType ни к одному](#transport-none)</li></ul> |
| **Веб-API** | <ul><li>[Убедитесь, что стандартные методы аутентификации используются для обеспечения безопасности Web-апогей](#authn-secure-api)</li></ul> |
| **Azure AD** | <ul><li>[Используйте стандартные сценарии проверки подлинности, поддерживаемые Azure Active Directory](#authn-aad)</li><li>[Переопределить кэш маркера ADAL по умолчанию с помощью масштабируемой альтернативы](#adal-scalable)</li><li>[Для предотвращения воспроизведения маркеров проверки подлинности ADAL нужно использовать свойство TokenReplayCache](#tokenreplaycache-adal)</li><li>[Используйте библиотеки ADAL для управления запросами токенов от клиентов OAuth2 до AAD (или на территории AD)](#adal-oauth2)</li></ul> |
...
| **Облачный шлюз Интернета вещей** | <ul><li>[Устройства, подключаемые к облачному шлюзу, должны проходить проверку подлинности](#authn-devices-cloud)</li><li>[Использование учетных данных аутентификации на устройстве](#authn-cred)</li></ul> |
...
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | Недоступно  |
| Сведения | <p>Политика паролей и учетных записей должна быть реализована в соответствии с политикой организации и рекомендациями.</p><p>Для защиты от атак методом прямого подбора и подбора на основе словаря необходимо реализовать политику надежных паролей, чтобы пользователи создавали сложные пароли (например, длиной не менее 12 символов, а также содержащие алфавитно-цифровые или специальные символы).</p><p>Политики блокировки учетных записей можно реализовать следующим образом:</p><ul><li>**Мягкая блокировка.** Может быть хорошим вариантом для защиты пользователей от атак методом прямого подбора. Например, всякий раз, когда пользователь вводит неправильный пароль три раза приложение может заблокировать учетную запись на минуту, чтобы замедлить процесс грубой заставить их пароль делает его менее выгодным для злоумышленника, чтобы продолжить. Если бы вы реализовали жесткие контрмеры блокировки для этого примера, вы бы достигли "DoS", навсегда блокируя учетные записи. Кроме того, приложение может создать одноразовый пароль и отправить его пользователю по внештатному каналу (по электронной почте, SMS и т. д.). Как вариант, можно реализовать проверку CAPTCHA по достижении порогового значения числа неудачных попыток.</li><li>**Твердый локаут:** Этот тип блокировки должен применяться всякий раз, когда вы обнаружите, пользователь атакует ваше приложение и противостоять им с помощью постоянной блокировки своей учетной записи, пока группа реагирования не успела сделать свою экспертизу. После этого процесса вы можете принять решение вернуть пользователю свою учетную запись или принять дальнейшие юридические меры против них. Такой подход не позволит злоумышленнику продолжить атаки на приложение и инфраструктуру.</li></ul><p>Для защиты от атак на прогнозируемые учетные записи и учетные записи по умолчанию все ключи и пароли должны быть заменяемыми, а также быть созданы или заменены с момента их создания.</p><p>Если приложение автоматически создает пароли, они должны создаваться случайным образом и иметь высокий уровень энтропии.</p>|
...
| **Ссылки**              | [Выбор режима проверки подлинности](https://msdn.microsoft.com/library/ms144284.aspx) |
...
| **Ссылки**              | [Подключение к базе данных СЗЛ с помощью активной аутентификации каталогов Azure](https://azure.microsoft.com/documentation/articles/sql-database-aad-authentication/) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Политика паролей сервера S'L](https://technet.microsoft.com/library/ms161959(v=sql.110).aspx) |
...
| **Ссылки**              | [Рекомендации по защите автономных баз данных](https://msdn.microsoft.com/library/ff929055.aspx) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Что такое Многофакторная идентификация Azure?](https://azure.microsoft.com/documentation/articles/multi-factor-authentication/) |
...
| **Ссылки**              | [Сценарии защиты кластера Service Fabric](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security) |
...
| **Ссылки**              | [Сценарии защиты кластера Service Fabric](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security/#_client-to-node-certificate-security), [Безопасное подключение к кластеру](https://azure.microsoft.com/documentation/articles/service-fabric-connect-to-secure-cluster/) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Среда: Azure |
| **Ссылки**              | [Рекомендации по обеспечению безопасности](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security/#security-recommendations) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Среда: Azure |
| **Ссылки**              | [Сертификаты X.509 и Service Fabric](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security/#x509-certificates-and-service-fabric) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [IdentityServer3. Общая картина](https://identityserver.github.io/Documentation/docsv2/overview/bigPicture.html) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Развертывание сервера удостоверений. Кэширование](https://identityserver.github.io/Documentation/docsv2/advanced/deployment.html) |
...
| **Атрибуты**              | Недоступно |
| **Ссылки**              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx) |
...
| **Ссылки**              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx), [Fortify](https://community.microfocus.com/t5/UFT-Discussions/UFT-API-Test-with-WCF-wsHttpBinding/m-p/600927) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Проверка подлинности и авторизация в веб-API ASP.NET](https://www.asp.net/web-api/overview/security/authentication-and-authorization-in-aspnet-web-api), [сведения о внешних службах проверки подлинности с веб-API ASP.NET (C#)](https://www.asp.net/web-api/overview/security/external-authentication-services) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Сценарии аутентификации в Azure Active Directory](https://azure.microsoft.com/documentation/articles/active-directory-authentication-scenarios/), [Примеры кода Azure Active Directory](https://azure.microsoft.com/documentation/articles/active-directory-code-samples/), [Руководство разработчика по Azure Active Directory](https://azure.microsoft.com/documentation/articles/active-directory-developers-guide/) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Современная проверка подлинности с помощью Azure Active Directory для веб-приложений](https://blogs.msdn.microsoft.com/microsoft_press/2016/01/04/new-book-modern-authentication-with-azure-active-directory-for-web-applications/), [запись блога об использовании Redis в качестве кэша маркеров ADAL](https://blogs.msdn.microsoft.com/mrochon/2016/09/19/using-redis-as-adal-token-cache/)  |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Современная проверка подлинности с помощью Azure Active Directory для веб-приложений](https://blogs.msdn.microsoft.com/microsoft_press/2016/01/04/new-book-modern-authentication-with-azure-active-directory-for-web-applications/) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [ADAL](https://azure.microsoft.com/documentation/articles/active-directory-authentication-libraries/) |
...
| **Ссылки**              | N/A, [концентратор Azure IoT с .NET](https://azure.microsoft.com/documentation/articles/iot-hub-csharp-csharp-getstarted/), [Начало работы с концентратором IoT и узлом JS,](https://azure.microsoft.com/documentation/articles/iot-hub-node-node-getstarted) [Обеспечение IoT с SAS и сертификатами](https://azure.microsoft.com/documentation/articles/iot-hub-sas-tokens/), [репозиторий Git](https://github.com/Azure/azure-iot-sdks/tree/master/node) |
...
* Создание концентратора IoT на лазурном газе
...
| **Ссылки**              | [Маркеры безопасности Центра Интернета вещей Azure](https://azure.microsoft.com/documentation/articles/iot-hub-sas-tokens/) |
