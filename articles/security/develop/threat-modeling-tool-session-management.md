...
| **Сервер удостоверений** | <ul><li>[Реализация надлежащего выхода при использовании Identity Server](#proper-logout)</li></ul> |
| **Веб-приложение** | <ul><li>[Используйте защищенные файлы cookie в приложениях, использующих протокол HTTPS](#https-secure-cookies)</li><li>[Укажите атрибут httpOnly в определении файла cookie всех приложений на основе протокола HTTP](#cookie-definition)</li><li>[Уменьшите риск атак с подделкой межсайтовых запросов на веб-страницах ASP.NET](#csrf-asp)</li><li>[Настройка сеанса на срок действия](#inactivity-lifetime)</li><li>[Внедрение надлежащего выхода из приложения](#proper-app-logout)</li></ul> |
| **Веб-API** | <ul><li>[Уменьшите риск атак с подделкой межсайтовых запросов в веб-интерфейсах API ASP.NET](#csrf-api)</li></ul> |
...
| **Шаги** | IdentityServer поддерживает возможность создавать федерацию с внешними поставщиками удостоверений. Когда пользователь выходит из поставщика идентификационных данных в зависимости от используемого протокола, возможно, можно получить уведомление, когда пользователь выйдет из него. Это позволяет IdentityServer уведомлять своих клиентов, чтобы они могли также подписать пользователя. Проверьте документацию в разделе ссылок на детали реализации.|
...
| **Шаги** | Значение свойства RequireSSL задается в файле конфигурации web.config для приложения ASP.NET с помощью атрибута requireSSL. Этот атрибут также позволяет указать, нужно ли использовать протокол SSL для возвращения билета проверки подлинности (файла cookie) на сервер.|
...
| **Шаги** | Время сеанса представляет событие, возникающее, когда пользователь не выполняет никаких действий на веб-сайте в течение интервала (определяется веб-сервером). Это событие на стороне сервера изменяет состояние сеанса пользователя на "Недопустимый" (например, "больше не используется") и дает указание веб-серверу уничтожить его (удалить все данные сеанса). Пример кода ниже задает для атрибута времени ожидания сеанса значение 15 минут в файле web.config.|
...
| **Применимые технологии** | Веб-формы |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Элемент формы для проверки подлинности (ASP.NET настройки Schema)](https://msdn.microsoft.com/library/1d3t3c61(v=vs.100).aspx) |
