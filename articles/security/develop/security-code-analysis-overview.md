...
Обнаружение угроз безопасности Майкрософт (МСРД) — это облачная служба для тестирования методом Монте-Карло. Он определяет уязвимые ошибки безопасности в программном обеспечении. Для этой службы требуется отдельная подписка и активация. Дополнительные сведения см. в [центре разработчиков мсрд](https://docs.microsoft.com/security-risk-detection/).
