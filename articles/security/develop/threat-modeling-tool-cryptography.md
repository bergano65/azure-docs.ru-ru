...
| --------------- | ------- |
| **Веб-приложение** | <ul><li>[Используйте только утвержденные симметричные шифры блоков и длины ключей](#cipher-length)</li><li>[Используйте утвержденные режимы блочного шифрования и векторы инициализации для симметричных шифров](#vector-ciphers)</li><li>[Используйте утвержденные асимметричные алгоритмы, длину ключей и схему дополнения](#padding)</li><li>[Используйте утвержденные генераторы случайных чисел](#numgen)</li><li>[Не используйте симметричные шифры потока](#stream-ciphers)</li><li>[Использование утвержденных алгоритмов MAC/HMAC/ключового хэша](#mac-hash)</li><li>[Используйте только утвержденные криптографические хэш-функции](#hash-functions)</li></ul> |
| **Базы данных** | <ul><li>[Используйте надежные алгоритмы шифрования для шифрования данных в базе данных](#strong-db)</li><li>[Пакеты SSIS должны быть зашифрованы и подписаны в цифровом виде](#ssis-signed)</li><li>[Добавление цифровой подписи в критически важные базы данных](#securables-db)</li><li>[Для защиты ключей шифрования используйте сервер EKM](#ekm-keys)</li><li>[Используйте функцию AlwaysEncrypted, если ключи шифрования не должны быть раскрыты в движке базы данных](#keys-engine)</li></ul> |
| **Устройство IoT** | <ul><li>[Хранение криптографических ключей надежно на Устройстве IoT](#keys-iot)</li></ul> | 
| **Облачный шлюз Интернета вещей** | <ul><li>[Создание случайного симметричного ключа достаточной длины для проверки подлинности в IoT Hub](#random-hub)</li></ul> | 
| **Мобильный клиент Dynamics CRM** | <ul><li>[Обеспечить наличие политики управления устройствами, которая требует использования PIN-кода и позволяет удаленное вытирание](#pin-remote)</li></ul> | 
| **Клиент Outlook для Dynamics CRM** | <ul><li>[Убедитесь, что политика управления устройством действует, что требует БЛОКИРОВКи PIN/password/auto и шифрует все данные (например, BitLocker)](#bitlocker)</li></ul> | 
| **Сервер удостоверений** | <ul><li>[Убедитесь, что ключи подписи будут перенесены при использовании Identity Server](#rolled-server)</li><li>[Убедитесь, что криптографически сильный идентификатор клиента, секрет клиента используются в Identity Server](#client-server)</li></ul> | 
...
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | Недоступно  |
| **Шаги** | <p>Продукты должны использовать семейство алгоритмов хэширования SHA-2 (SHA256, SHA384 и SHA512). Если требуется хэш меньшей длины, например выходные данные длиной 128 бит, чтобы вместить структуру данных, разработанную для MD5-хэша небольшой длины, группы разработки продуктов могут усечь один из хэшей SHA2 (обычно SHA256). Обратите внимание, что SHA384 — это усеченная версия SHA512. Усечение криптографических хэшей до менее 128 бит в целях безопасности не допускается. Новый код не должен использовать хэш-алгоритмы MD2, MD4, MD5, SHA-0, SHA-1 или RIPEMD. Конфликты хэшей подходят для этих алгоритмов с точки зрения вычислений, благодаря чему они эффективно устраняются.</p><p>Разрешенные хэш-алгоритмы .NET для управляемого шифрования (в порядке предпочтения):</p><ul><li>SHA512Cng (совместим со стандартом FIPS);</li><li>SHA384Cng (совместим со стандартом FIPS);</li><li>SHA256Cng (совместим со стандартом FIPS);</li><li>SHA512Managed (не-FIPS-совместимый) (использовать SHA512 в качестве имени алгоритма в вызовах на HashAlgorithm.Create или CryptoConfig.CreateFromName)</li><li>SHA384Managed (не-FIPS-совместимый) (использовать SHA384 в качестве имени алгоритма в вызовах на HashAlgorithm.Create или CryptoConfig.CreateFromName)</li><li>SHA256Managed (не-FIPS-совместимый) (использовать SHA256 в качестве имени алгоритма в вызовах на HashAlgorithm.Create или CryptoConfig.CreateFromName)</li><li>SHA512CryptoServiceProvider (совместим со стандартом FIPS);</li><li>SHA256CryptoServiceProvider (совместим со стандартом FIPS);</li><li>SHA384CryptoServiceProvider (совместим со стандартом FIPS).</li></ul>| 
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Выбор алгоритма шифрования](https://technet.microsoft.com/library/ms345262(v=sql.130).aspx) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Определение источника пакетов с помощью цифровых подписей](https://msdn.microsoft.com/library/ms141174.aspx), [Предотвращение угроз и уязвимости для безопасности (службы Integration Services)](https://msdn.microsoft.com/library/bb522559.aspx) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [ADD SIGNATURE (Transact-SQL)](https://msdn.microsoft.com/library/ms181700) |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Расширенное управление ключами (Extensible Key Management)](https://msdn.microsoft.com/library/bb895340), [Расширенное управление ключами с помощью хранилища ключей Azure (SQL Server)](https://msdn.microsoft.com/library/dn198405) |
...
| **Ссылки**              | [Всегда зашифрован (Движок базы данных)](https://msdn.microsoft.com/library/mt163865) |
...
| **Ссылки**              | [Доверенный платформенный модуль в Windows 10 IoT Core](https://developer.microsoft.com/windows/iot/docs/tpm), [настройка доверенного платформенного модуля в Windows 10 IoT Core](https://docs.microsoft.com/windows/iot-core/secure-your-device/setuptpm), [доверенный платформенный модуль пакета SDK для устройств Azure IoT](https://github.com/Azure/azure-iot-hub-vs-cs/wiki/Device-Provisioning-with-TPM) |
...
## <a name="ensure-a-device-management-policy-is-in-place-that-requires-a-pinpasswordauto-lock-and-encrypts-all-data-eg-bitlocker"></a><a id="bitlocker"></a>Убедитесь, что политика управления устройством действует, что требует БЛОКИРОВКи PIN/password/auto и шифрует все данные (например, BitLocker)
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Identity Server - Ключи, подписи и криптография](https://identityserver.github.io/Documentation/docsv2/configuration/crypto.html) |
...
## <a name="ensure-that-cryptographically-strong-client-id-client-secret-are-used-in-identity-server"></a><a id="client-server"></a>Убедитесь, что криптографически сильный идентификатор клиента, секрет клиента используются в Identity Server
