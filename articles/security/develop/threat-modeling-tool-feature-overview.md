...
| Имя набора элементов                               | Подробнее      |
| --------------------------------------- | ------------ |
...
Наведите указатель мыши на элемент **Open a Model** (Открыть модель), чтобы отобразились два параметра: **Open From this Computer** (Открыть с этого компьютера) и **Open from OneDrive** (Открыть из OneDrive). Первый параметр открывает экран **File Open** (Открытие файла). Второй параметр позволяет выполнить входа в OneDrive. После успешной аутентификации можно выбрать папки и файлы.
...
- **Перетаскивание**. Перетащите нужный элемент на сетку. Затем используйте свойства этого элемента, чтобы указать дополнительные сведения.
...
- **Щелчок+SHIFT**. Щелкните первый элемент (отправляющий данные), нажмите и удерживайте клавишу SHIFT, а затем выберите второй элемент (получающий данные). Щелкните правой кнопкой мыши и выберите **Connect** (Соединить). При использовании двунаправленного потока данных порядок не столь важен.
...
![Сообщить](./media/threat-modeling-tool-feature-overview/report.png)
