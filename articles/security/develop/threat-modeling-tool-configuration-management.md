---
title: Управление конфигурацией для инструмента моделирования угроз Майкрософт
titleSuffix: Azure
...
| **Полевой шлюз Интернета вещей** | <ul><li>[Шифрование ОС и дополнительных перегородок IoT Field Gateway с бик-запираемым](#field-bit-locker)</li><li>[Измените учетные данные для входа в систему полевого шлюза по умолчанию во время установки](#default-change)</li></ul> |
...
| **Отношения доверия между компьютерами** | <ul><li>[Убедитесь, что устройства имеют элементы управления безопасности конечных точек, настроенные в рамках организационных политик](#controls-policies)</li></ul> |
...
```
Эта политика позволяет скриптам загружать сяог только с сервера веб-приложения и сервера Google Analytics. Все остальные скрипты отклоняются. Если на веб-сайте включена политика CSP, некоторые функции отключаются автоматически (см. ниже). Это позволяет предотвратить выполнение межсайтовых сценариев. 
...
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | Недоступно  |
| **Шаги** | Используйте сторонние скрипты JavaScript только из доверенных источников с конечными точками SSL. |

## <a name="ensure-that-authenticated-aspnet-pages-incorporate-ui-redressing-or-click-jacking-defenses"></a><a id="ui-defenses"></a>Убедитесь, что проверенные ASP.NET страницы включают защиту uI Redressing или click-jacking
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [OWASP кликов jacking обороны Cheat лист](https://cheatsheetseries.owasp.org/cheatsheets/Clickjacking_Defense_Cheat_Sheet.html), [IE Внутренние - Борьба нажмите-джекинг с X-Frame-Опции](https://blogs.msdn.microsoft.com/ieinternals/2010/03/30/combating-clickjacking-with-x-frame-options/) |
...
### <a name="example"></a>Пример
Заголовок X-FRAME-OPTIONS можно установить через IIS web.config. Фрагмент кода Web.config для сайтов, которые никогда не должны быть оформлены: 
```csharp
    <system.webServer>
        <httpProtocol>
            <customHeader>
...
## <a name="configure-a-windows-firewall-for-database-engine-access"></a><a id="firewall-db"></a>Настройка брандмауэра Windows для доступа к компоненту Database Engine
...
## <a name="encrypt-os-and-additional-partitions-of-iot-field-gateway-with-bit-locker"></a><a id="field-bit-locker"></a>Шифрование ОС и дополнительных перегородок IoT Field Gateway с бик-запираемым
