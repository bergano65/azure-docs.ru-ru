...
Очень удобно определять жестко закодированные секреты и устранять риски. Но предотвращение возврата секретов в любом случае еще лучше.

Чтобы помочь этому в этом, корпорация Майкрософт выпустила предварительную версию анализатора кода сканера учетных данных как часть [расширения Microsoft DevLabs](https://marketplace.visualstudio.com/items?itemName=VSIDEDevOpsMSFT.ContinuousDeliveryToolsforVisualStudio) для Visual Studio. Анализатор — это выпуск на раннем этапе предварительной версии. Он предоставляет разработчикам встроенный интерфейс для обнаружения потенциальных секретов в своем коде. Таким образом, анализатор также дает разработчикам возможность исправить эти проблемы в режиме реального времени.
...
Попытайтесь получить путь к журналу ошибок, который указан в параметре **Csc. exe/еррорлог** . Параметр и путь отображаются в журнале для задачи сборки Roslyn Analyzer. Они могут выглядеть примерно так, как **/еррорлог:\_ф:\тс-сервицес-123 work\456\s\Some\Project\Code\Code.csproj.Sarif**
