...
ms.locfileid: "81687899"
---
# <a name="security-frame-input-validation--mitigations"></a>Механизм безопасности: проверки входных данных | Снижение риска 
| Продукт или служба | Статья |
| --------------- | ------- |
| **Веб-приложение** | <ul><li>[Отключить скрипты XSLT для всех преобразований с использованием ненадежных листов стилей](#disable-xslt)</li><li>[Убедитесь, что каждая страница, на которой может быть содержимое, управляемое пользователем, не использует автоматическое обнаружение типов MIME](#out-sniffing)</li><li>[Твердые или отключить XML Разрешение сущности](#xml-resolution)</li><li>[Приложения, используя http.sys, выполняют проверку канонизации URL](#app-verification)</li><li>[Убедитесь, что при приеме файлов от пользователей обеспечиваются соответствующие меры безопасности](#controls-users)</li><li>[Убедитесь, что в веб-приложении для доступа к данным используются параметры типобезопасности](#typesafe)</li><li>[Используйте отдельные классы привязки моделей или списки привязки фильтров для устранения уязвимости массового назначения MVC](#binding-mvc)</li><li>[Кодируйте ненадежные выходные веб-данные перед обработкой](#rendering)</li><li>[Выполните проверку и фильтрацию входных данных на всех свойствах модели типа строки](#typemodel)</li><li>[Санитизация должна применяться на полях форм, которые принимают все символы, например, богатый текстовый редактор](#richtext)</li><li>[Не назначайте элементы DOM раковинам, которые не имеют встроенного кодирования](#inbuilt-encode)</li><li>[Проверка всех перенаправлений в приложении закрыта или сделана безопасно](#redirect-safe)</li><li>[Внедрите проверку входных данных для всех параметров строкового типа, принимаемых методами контроллера](#string-method)</li><li>[Установите верхний предел тайм-аута для регулярной обработки выражений, чтобы предотвратить DoS из-за плохих регулярных выражений](#dos-expression)</li><li>[Избегайте использования html.Raw в представлениях Razor](#html-razor)</li></ul> | 
| **Базы данных** | <ul><li>[Не используйте динамические запросы в сохраненных процедурах](#stored-proc)</li></ul> |
| **Веб-интерфейс API** | <ul><li>[Убедитесь, что проверка модели выполняется на методах Web API](#validation-api)</li><li>[Осуществление вхотливной проверки на всех параметрах типа строк, принятых методами Web API](#string-api)</li><li>[Убедитесь, что параметры, безопасные для типов, используются в Web API для доступа к данным](#typesafe-api)</li></ul> | 
| **Azure DocumentDB** | <ul><li>[Использование параметризованных SQL-запросов для Azure Cosmos DB](#sql-docdb)</li></ul> | 
| **WCF** | <ul><li>[Проверка ввода WCF через связывание Schema](#schema-binding)</li><li>[Проверка входных данных WCF с помощью инспекторов параметров](#parameters)</li></ul> |

## <a name="disable-xslt-scripting-for-all-transforms-using-untrusted-style-sheets"></a><a id="disable-xslt"></a>Отключайте скрипты XSLT для всех операций преобразования, использующих ненадежные таблицы стилей
...
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Безопасность XSLT](https://msdn.microsoft.com/library/ms763800(v=vs.85).aspx), [XsltSettings.EnableScript Property](https://msdn.microsoft.com/library/system.xml.xsl.xsltsettings.enablescript.aspx) (Свойство XsltSettings.EnableScript) |
...
## <a name="ensure-that-each-page-that-could-contain-user-controllable-content-opts-out-of-automatic-mime-sniffing"></a><a id="out-sniffing"></a>Убедитесь, что каждая страница, на которой может быть содержимое, управляемое пользователем, не использует автоматическое обнаружение типов MIME
...
## <a name="harden-or-disable-xml-entity-resolution"></a><a id="xml-resolution"></a>Усильте защиту для разрешения сущностей XML или отключите его
...
## <a name="applications-utilizing-httpsys-perform-url-canonicalization-verification"></a><a id="app-verification"></a>Приложения, использующие драйвер http.sys, выполняют проверку нормализации URL-адресов
...
## <a name="ensure-that-type-safe-parameters-are-used-in-web-application-for-data-access"></a><a id="typesafe"></a>Убедитесь, что в веб-приложении для доступа к данным используются параметры типобезопасности
...
## <a name="use-separate-model-binding-classes-or-binding-filter-lists-to-prevent-mvc-mass-assignment-vulnerability"></a><a id="binding-mvc"></a>Используйте отдельные классы привязки моделей или списки привязки фильтров для устранения уязвимости массового назначения MVC
...
## <a name="perform-input-validation-and-filtering-on-all-string-type-model-properties"></a><a id="typemodel"></a>Проверяйте и фильтруйте входные данные по всем свойствам модели строкового типа
...
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Adding Validation](https://www.asp.net/mvc/overview/getting-started/introduction/adding-validation) (Добавление проверки), [Validating Model Data in an MVC Application](https://msdn.microsoft.com/library/dd410404(v=vs.90).aspx) (Проверка данных модели в приложении MVC), [Основные принципы ваших приложений ASP.NET MVC](https://msdn.microsoft.com/magazine/dd942822.aspx) |
| **Шаги** | <p>Перед использованием в приложении все входные параметры нужно проверять, чтобы убедиться, что приложение защищено от вредоносных входных данных. Проверьте входные значения, используя регулярные выражения на стороне сервера и стратегию проверки по списку разрешений. Если передать в метод непроверенные входные данные или параметры, приложение подвергается риску атаки путем внедрения кода.</p><p>Точки входа для веб-приложений также включают поля форм, строки запросов, файлы cookie, заголовки HTTP и параметры веб-службы.</p><p>После привязки модели необходимо выполнить следующие проверки входных данных:</p><ul><li>Свойства модели нужно пометить с помощью аннотации RegularExpression, чтобы принимались допустимые знаки, а также применялась максимально допустимая длина.</li><li>Методы контроллера должны выполнить проверку ModelState.</li></ul>|

## <a name="sanitization-should-be-applied-on-form-fields-that-accept-all-characters-eg-rich-text-editor"></a><a id="richtext"></a>Санитизация должна применяться на полях форм, которые принимают все символы, например, богатый текстовый редактор
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Кодирование небезопасных входных данных](https://msdn.microsoft.com/library/ff647397.aspx#paght000003_step3), [HTML Sanitizer](https://github.com/mganss/HtmlSanitizer) (Библиотека HtmlSanitizer) |
| **Шаги** | <p>Определите все статические теги разметки, которые нужно использовать. Зачастую при форматировании используются только безопасные элементы HTML, такие как `<b>` (жирный шрифт) и `<i>` (курсив).</p><p>Прежде чем записывать данные, закодируйте их в HTML. Таким образом, любой вредоносный скрипт становится безопасным, так как он обрабатывается как текст, а не как исполняемый код.</p><ol><li>Отключите проверку запросов ASP.NET, добавив атрибут ValidateRequest="false" в страничную директиву \@.</li><li>Закодируйте строковые входные данные с помощью метода HtmlEncode.</li><li>Используйте StringBuilder и вызовите его метод Replace, чтобы выборочно удалить кодировку элементов HTML, которые необходимо разрешить.</li></ol><p>Страница в ссылках отключает проверку запросов ASP.NET, задав атрибут `ValidateRequest="false"`. Она кодирует входные данные с помощью HTML и выборочно разрешает использовать элементы `<b>` и `<i>`. Кроме того, для очистки HTML можно использовать библиотеку .NET.</p><p>HtmlSanitizer — это библиотека .NET для очистки фрагментов и документов HTML от конструкций, которые могут привести к атаке с использованием межсайтовых сценариев. Она использует AngleSharp для анализа и обработки HTML и CSS, а также для управления. HtmlSanitizer можно установить в виде пакета NuGet. Входные данные пользователя можно передать через соответствующие методы очистки HTML или CSS на стороне сервера. Обратите внимание, что очистка — это крайняя мера безопасности.</p><p>Самые эффективные меры безопасности — это проверка входных данных и кодирование выходных данных.</p> |

## <a name="do-not-assign-dom-elements-to-sinks-that-do-not-have-inbuilt-encoding"></a><a id="inbuilt-encode"></a>Не назначайте элементы модели DOM приемникам без встроенных возможностей кодирования
...
## <a name="validate-all-redirects-within-the-application-are-closed-or-done-safely"></a><a id="redirect-safe"></a>Убедитесь, что операции перенаправления в приложении закрыты или выполняются безопасно
...
| **Применимые технологии** | Универсальные, MVC 5, MVC 6 |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Validating Model Data in an MVC Application](https://msdn.microsoft.com/library/dd410404(v=vs.90).aspx) (Проверка данных модели в приложении MVC), [Основные принципы ваших приложений ASP.NET MVC](https://msdn.microsoft.com/magazine/dd942822.aspx) |
...
## <a name="set-upper-limit-timeout-for-regular-expression-processing-to-prevent-dos-due-to-bad-regular-expressions"></a><a id="dos-expression"></a>Задайте верхний предел времени ожидания для обработки регулярных выражений во избежание атаки типа "отказ в обслуживании" из-за недопустимого регулярного выражения
...
| **Применимые технологии** | Универсальные, веб-формы, MVC 5, MVC 6  |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Недвижимость defaultRegexMatchOut](https://msdn.microsoft.com/library/system.web.configuration.httpruntimesection.defaultregexmatchtimeout.aspx) |
...
## <a name="avoid-using-htmlraw-in-razor-views"></a><a id="html-razor"></a>Не используйте метод Html.Raw в представлениях Razor
...
## <a name="do-not-use-dynamic-queries-in-stored-procedures"></a><a id="stored-proc"></a>Не используйте динамические запросы в хранимых процедурах
...
## <a name="ensure-that-model-validation-is-done-on-web-api-methods"></a><a id="validation-api"></a>Убедитесь, что в методах веб-API выполнена проверка моделей
...
| **Применимые технологии** | MVC 5, MVC 6 |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Проверка модели в ASP.NET Web API](https://www.asp.net/web-api/overview/formats-and-model-binding/model-validation-in-aspnet-web-api) |
...
## <a name="implement-input-validation-on-all-string-type-parameters-accepted-by-web-api-methods"></a><a id="string-api"></a>Внедрите проверку входных данных для всех параметров строкового типа, принимаемых методами веб-API
...
## <a name="ensure-that-type-safe-parameters-are-used-in-web-api-for-data-access"></a><a id="typesafe-api"></a>Убедитесь, что в веб-API для доступа к данным используются параметры типобезопасности
...
## <a name="use-parameterized-sql-queries-for-cosmos-db"></a><a id="sql-docdb"></a>Использование параметризованных SQL-запросов для Cosmos DB
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Announcing SQL Parameterization in Azure Cosmos DB](https://azure.microsoft.com/blog/announcing-sql-parameterization-in-documentdb/) (Объявление о параметризации SQL в Azure Cosmos DB) |
| **Шаги** | Несмотря на то что Azure Cosmos DB поддерживает исключительно запросы только на чтение, атака путем внедрения кода SQL по-прежнему возможна, если создавать запросы, объединяя их с входными данными пользователя. Возможно, пользователь может получить доступ к данным, к которым он не должен получить доступ в рамках одной коллекции, создав вредоносные запросы s'L. Используйте параметризованные SQL-запросы, если запросы создаются на основе входных данных пользователя. |

## <a name="wcf-input-validation-through-schema-binding"></a><a id="schema-binding"></a>Проверка входных данных WCF путем привязки к схеме
...
| **Применимые технологии** | Универсальные, .NET Framework 3 |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [MSDN](https://msdn.microsoft.com/library/ff647875.aspx) |
