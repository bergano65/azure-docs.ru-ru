...
| --------------- | ------- |
| **Веб-приложение** | <ul><li>[Отключить скрипты XSLT для всех преобразований, использующих недоверенные таблицы стилей](#disable-xslt)</li><li>[Убедитесь, что каждая страница, на которой может быть содержимое, управляемое пользователем, не использует автоматическое обнаружение типов MIME](#out-sniffing)</li><li>[Защита или отключение разрешения сущностей XML](#xml-resolution)</li><li>[Приложения, использующие HTTP. sys, выполняют проверку канонизации URL-адресов](#app-verification)</li><li>[Убедитесь, что при приеме файлов от пользователей обеспечиваются соответствующие меры безопасности](#controls-users)</li><li>[Убедитесь, что в веб-приложении для доступа к данным используются параметры типобезопасности](#typesafe)</li><li>[Используйте отдельные классы привязки моделей или списки привязки фильтров для устранения уязвимости массового назначения MVC](#binding-mvc)</li><li>[Кодируйте ненадежные выходные веб-данные перед обработкой](#rendering)</li><li>[Выполнить проверку входных данных и фильтрацию по всем свойствам модели строкового типа](#typemodel)</li><li>[Очистка должна применяться к полям формы, принимающим все символы, например редактор форматированного текста](#richtext)</li><li>[Не назначайте элементы DOM приемникам, у которых нет встроенной кодировки](#inbuilt-encode)</li><li>[Проверка того, что все перенаправления в приложении закрыты или выполняются безопасно](#redirect-safe)</li><li>[Внедрите проверку входных данных для всех параметров строкового типа, принимаемых методами контроллера](#string-method)</li><li>[Установка времени ожидания верхнего предела для обработки регулярных выражений с целью предотвращения DoS из-за недопустимых регулярных выражений](#dos-expression)</li><li>[Избегайте использования HTML. RAW в представлениях Razor](#html-razor)</li></ul> | 
| **База данных** | <ul><li>[Не используйте динамические запросы в хранимых процедурах](#stored-proc)</li></ul> |
| **Веб-интерфейс API** | <ul><li>[Убедитесь, что проверка модели выполняется в методах веб-API.](#validation-api)</li><li>[Реализация проверки входных данных для всех параметров строкового типа, принимаемых методами веб-API](#string-api)</li><li>[Убедитесь, что в веб-API для доступа к данным используются строго типизированные параметры](#typesafe-api)</li></ul> | 
| **Azure DocumentDB** | <ul><li>[Использование параметризованных SQL-запросов для Azure Cosmos DB](#sql-docdb)</li></ul> | 
| **WCF** | <ul><li>[Проверка входных данных WCF с помощью привязки схемы](#schema-binding)</li><li>[Проверка входных данных WCF с помощью инспекторов параметров](#parameters)</li></ul> |
...
## <a name="sanitization-should-be-applied-on-form-fields-that-accept-all-characters-eg-rich-text-editor"></a><a id="richtext"></a>Очистка должна применяться к полям формы, принимающим все символы, например редактор форматированного текста
...
| **Применимые технологии** | Универсальные, веб-формы, MVC 5, MVC 6  |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Дефаултрежексматчтимеаут, свойство](https://msdn.microsoft.com/library/system.web.configuration.httpruntimesection.defaultregexmatchtimeout.aspx) |
...
| **Применимые технологии** | MVC 5, MVC 6 |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Проверка модели в веб-API ASP.NET](https://www.asp.net/web-api/overview/formats-and-model-binding/model-validation-in-aspnet-web-api) |
...
| **Шаги** | Несмотря на то что Azure Cosmos DB поддерживает исключительно запросы только на чтение, атака путем внедрения кода SQL по-прежнему возможна, если создавать запросы, объединяя их с входными данными пользователя. Пользователь может получить доступ к данным, к которым они не должны обращаться в пределах одной коллекции, создавая вредоносные SQL-запросы. Используйте параметризованные SQL-запросы, если запросы создаются на основе входных данных пользователя. |
