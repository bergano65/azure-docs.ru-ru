...
| **Веб-API** | <ul><li>[Настройте принудительную передачу всего трафика к веб-интерфейсам API через HTTPS-подключение](#webapi-https)</li></ul> |
| **Лазурный кэш для Redis** | <ul><li>[Убедитесь, что связь с Azure Cache для Redis будет сверх SSL](#redis-ssl)</li></ul> |
