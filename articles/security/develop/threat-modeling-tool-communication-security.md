---
title: Безопасность связи для инструмента моделирования угроз Майкрософт
titleSuffix: Azure
...
| **Фабрика данных Azure**. | <ul><li>[Используйте шлюз управления данными при подключении внутреннего сервера S'L к фабрике данных Azure](#sqlserver-factory)</li></ul> |
| **Сервер удостоверений** | <ul><li>[Убедитесь, что весь трафик на Identity Server находится в неподключении HTTPS](#identity-https)</li></ul> |
...
| **Базы данных** | <ul><li>[Обеспечить шифрование и проверку подключения к серверу сервера S'L](#sqlserver-validation)</li><li>[Настройте принудительное шифрование подключений к SQL Server](#encrypted-sqlserver)</li></ul> |
| **Хранилище Azure** | <ul><li>[Настройте подключение к службе хранилища Azure по протоколу HTTPS](#comm-storage)</li><li>[Проверяйте MD5-хэш после скачивания большого двоичного объекта по протоколу HTTP](#md5-https)</li><li>[Используйте совместимый клиент SMB 3.0 для обеспечения шифрования данных в пути в Акции файлов Azure](#smb-shares)</li></ul> |
| **Мобильный клиент** | <ul><li>[Реализация Сертификата Закрепление](#cert-pinning)</li></ul> |
...
| **Кэш Azure для Redis** | <ul><li>[Убедитесь, что связь с Azure Cache для Redis будет сверх SSL](#redis-ssl)</li></ul> |
| **Полевой шлюз Интернета вещей** | <ul><li>[Безопасное устройство для связи Field Gateway](#device-field)</li></ul> |
...
## <a name="use-data-management-gateway-while-connecting-on-premises-sql-server-to-azure-data-factory"></a><a id="sqlserver-factory"></a>Используйте шлюз управления данными при подключении внутреннего сервера S'L к фабрике данных Azure
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Связанные типы служб - Azure и on-premises |
| **Ссылки**              |[Перемещение данных между натерритории и фабрикой данных Azure,](https://azure.microsoft.com/documentation/articles/data-factory-move-data-between-onprem-and-cloud/#create-gateway) [шлюзом управления данными](https://azure.microsoft.com/documentation/articles/data-factory-data-management-gateway/) |
...
## <a name="configure-ssl-certificate-for-custom-domain-in-azure-app-service"></a><a id="ssl-appservice"></a>Настройте сертификат SSL для личного домена в службе приложений Azure
...
| **Ссылки**              | [Включение протокола HTTPS для приложения в службе приложений Azure](../../app-service/configure-ssl-bindings.md) |
...
| **Ссылки**              | [Включение зашифрованных соединений для ядра СУБД](https://msdn.microsoft.com/library/ms191192)  |
| **Шаги** | Шифрование SSL позволяет повысить безопасность данных, передаваемых между экземплярами SQL Server и приложениями, расположенными в разных сетях. |
...
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Привязка сертификата и открытого ключа](https://owasp.org/www-community/controls/Certificate_and_Public_Key_Pinning) |
...
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx), [Fortify Kingdom](https://vulncat.fortify.com/en/detail?id=desc.config.dotnet.wcf_misconfiguration_transport_security_enabled) |
...
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Информация о работе с SSL в веб-интерфейсе API](https://www.asp.net/web-api/overview/security/working-with-ssl-in-web-api) |
...
## <a name="ensure-that-communication-to-azure-cache-for-redis-is-over-ssl"></a><a id="redis-ssl"></a>Настройка подключения к кэшу Azure для Redis по протоколу SSL
...
| **Шаги** | В отличии от кэша Azure для Redis сервер Redis не имеет встроенной поддержки SSL. Если вы подключаетесь к кэшу Azure для Redis и ваш клиент поддерживает протокол SSL, например StackExchange.Redis, то следует использовать SSL. По умолчанию порт без SSL отключен для новых экземпляров кэша Azure для Redis. Эти стандартные параметры не следует изменять, если клиенты Redis зависят от поддержки SSL. |
...
| **Применимые технологии** | Универсальный шаблон |
| **Атрибуты**              | Недоступно  |
| **Ссылки**              | [Выберите протокол связи](https://azure.microsoft.com/documentation/articles/iot-hub-devguide/#messaging) |
