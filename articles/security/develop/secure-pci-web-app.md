...
- Разверните веб-приложение Azure с помощью среды службы приложений, которая является выделенной изолированной с доступом к брандмауэру внешнего интерфейса.
...
- Настройте политику Azure и схемы Azure, чтобы оценить соответствие требованиям.
...
- Защита идентификации Azure Active Directory обнаруживает потенциальные уязвимости, влияющие на удостоверения Организации, настраивает автоматические ответы на обнаруженные подозрительные действия, связанные с удостоверениями организации, и изучает подозрительные инциденты на выполнение соответствующих действий по их устранению.
...
- [Оценка Active Directory](https://docs.microsoft.com/azure/log-analytics/log-analytics-ad-assessment): решение проверки работоспособности Active Directory оценивает риск и работоспособность серверных сред через регулярные промежутки времени и предоставляет список рекомендаций, относящихся к развернутой серверной инфраструктуре.
