...
Шифрование дисков Azure доступно на виртуальных машинах, соответствующие этим требованиям минимальный объем памяти:

| Виртуальная машина | Минимальные требования к памяти |
|--|--|
| Виртуальные машины Windows | 2 ГБ |
| Если только шифрование тома данных виртуальных машин Linux| 2 ГБ |
| Виртуальные машины Linux при шифровании данных и тома операционной системы и где использование корневой (/) файловой системы — 4 ГБ или меньше | 8 ГБ |
| При шифровании данных и томов ОС виртуальных машин Linux и где использование файловой системы корневой (/) больше, чем 4 ГБ | Использование корневой файловой системы * 2. Например 16 ГБ использование корневой файловой системы требует по крайней мере 32 ГБ ОЗУ |

По завершении процесса шифрования диска ОС на виртуальных машинах Linux, виртуальную Машину можно настроить для запуска с меньшим объемом памяти. 

> [!NOTE]
> Шифрование диска ОС Linux не доступен для [масштабируемые наборы виртуальных машин](../virtual-machine-scale-sets/index.yml).

Шифрование дисков Azure также доступна для виртуальных машин с хранилищем класса premium. 
...
#### <a name="additional-prerequisites-for-linux-iaas-vms"></a>Дополнительные требования для виртуальных машин IaaS Linux 
...
- Убедитесь, что параметры /etc/fstab для подключения настроены правильно. Чтобы настроить эти параметры, запустите команду mount -a или перезагрузите виртуальную машину и подключите ее заново таким образом. Как только это будет завершено, проверьте выходные данные команды lsblk, чтобы убедиться, что нужный диск все еще подключен. 
  ...
  - До начала шифрования, не забудьте остановить все службы и процессы, которые могут выполнять запись для подключенных дисков данных с последующим и отключить их, чтобы они не перезапускать автоматически после перезагрузки. Это может оставить файлы открытыми на этих секций и препятствует шифрования необходимо повторно подключить их, что приводит к сбою шифрования. 
