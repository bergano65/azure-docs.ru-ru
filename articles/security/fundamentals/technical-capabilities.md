...
description: Введение в службы безопасности в Azure, которые помогут защитить данные, ресурсы и приложения в облаке.
services: security
...
ms.locfileid: "76845339"
---
# <a name="azure-security-technical-capabilities"></a>Технические возможности обеспечения безопасности Azure
В этой статье содержится знакомство с службами безопасности в Azure, которые помогут защитить данные, ресурсы и приложения в облаке и удовлетворить потребности в безопасности вашего бизнеса.
...
## <a name="security-technical-capabilities-to-fulfil-your-responsibility"></a>Технические возможности безопасности для выполнения вашей ответственности

Microsoft Azure предоставляет услуги, которые помогут вам удовлетворить ваши потребности в безопасности, конфиденциальности и соответствии требованиям. Следующая картинка помогает объяснить различные службы Azure, доступные для создания безопасной и соответствующей инфраструктуре приложений на основе отраслевых стандартов.
...
## <a name="manage-and-control-identity-and-user-access"></a>Управление и управление идентификацией и доступом пользователей
...
Решения Microsoft identity и access management помогают ИТ-отделу защитить доступ к приложениям и ресурсам в корпоративном центре обработки данных и в облаке, позволяя дополнительные уровни проверки, такие как многофакторная аутентификация и условный доступ Политики. Наблюдение за подозрительной активностью с использованием расширенных отчетов, аудита и предупреждений помогает смягчить потенциальные риски безопасности. [Azure Active Directory Premium](../../active-directory/active-directory-whatis.md) предоставляет единый вход на тысячи облачных приложений и доступ к веб-приложениям, которые вы работаете в местных отелях.
...
Портал [Active Directory Azure](https://aad.portal.azure.com/) доступен в рамках портала Azure. Из этой панели мониторинга можно получить обзор состояния организации и легко управлять каталогом, пользователями или доступом к приложениям.

![Azure Active Directory](./media/technical-capabilities/azure-security-technical-capabilities-fig2.png)

Ниже приведены основные возможности управления идентификацией Azure:
...
[Единый вход (SSO)](https://azure.microsoft.com/documentation/videos/overview-of-single-sign-on/) означает возможность доступа ко всем приложениям и ресурсам, которые вам необходимы для ведения бизнеса, при входе только один раз с помощью одной учетной записи пользователя. После входа пользователю доступны все необходимые приложения без повторной проверки подлинности (например, ввода пароля).

Многие организации полагаются на программное обеспечение в качестве сервисных приложений (SaaS), таких как Office 365, Box и Salesforce для повышения производительности конечных пользователей. Обычно ИТ-специалистам приходилось отдельно создавать и обновлять учетные записи пользователей в каждом приложении SaaS, а пользователям нужно было запоминать новые пароли для каждого такого приложения.

[Azure AD расширяет доступ к облаку active Directory,](../../active-directory/manage-apps/what-is-single-sign-on.md)позволяя пользователям использовать свою основную организационную учетную запись не только для вхотворки в свои устройства, связанные с доменом, и на ресурсы компании, но и на все приложения для работы в Интернете и SaaS, необходимые для их работы.

Теперь пользователям не нужно управлять несколькими наборами учетных данных, состоящих из имени и пароля. Доступ к приложениям предоставляется или отменяется автоматически на основе членства в группах организации и статуса конкретного сотрудника. [Azure AD вводит элементы управления безопасностью и доступом,](../../active-directory/active-directory-enterprise-apps-manage-sso.md) которые позволяют централизованно управлять доступом пользователей в приложениях SaaS.

#### <a name="multi-factor-authentication"></a>Многофакторная Идентификация

[Azure Multi-Factor Authentication (MFA)](../../active-directory/authentication/multi-factor-authentication.md) — это метод аутентификации, который требует использования более одного метода проверки и добавляет критический второй уровень безопасности для входе в систему и транзакций пользователя. [МИД помогает защитить](../../active-directory/authentication/concept-mfa-howitworks.md) доступ к данным и приложениям при одновременном удовлетворении спроса пользователей на простой процесс вхлечения. Она обеспечивает строгую проверку подлинности разными способами — с помощью телефонного звонка, текстового сообщения, уведомления в мобильном приложении, кода подтверждения или маркеров OAuth сторонних поставщиков.
...
На портале Azure или [портале Azure Active Directory](https://aad.portal.azure.com/)[отчеты](../../active-directory/active-directory-reporting-azure-portal.md) упорядочены следующим образом.
...
- Интегрированные отчеты о приложениях — дает представление о том, как облачные приложения используются в вашей организации. Azure Active Directory обеспечивает интеграцию с множеством облачных приложений.

- Отчеты об ошибках: указывают на ошибки, которые возникают при подготовке учетных записей для внешних приложений.

- Отчеты, отображаемые для пользователя, — отображение устройства и вписаться в данные активности для конкретного пользователя.
...
[Регистрация устройств Azure AD](../../active-directory/devices/overview.md) является основой для сценариев [условного доступа](../../active-directory/devices/overview.md) на основе устройств. В ходе регистрации служба регистрации устройств Azure Active Directory назначает устройству идентификатор, который используется для аутентификации устройства при входе пользователя. Подлинное устройство и атрибуты устройства могут быть использованы для обеспечения соблюдения политики условного доступа для приложений, размещенных в облаке и на месте.

В сочетании с решением управления [мобильным устройством (MDM),](https://www.microsoft.com/itshowcase/Article/Content/588/Mobile-device-management-at-Microsoft) таким как Intune, атрибуты устройства в Active Directory Azure обновляются с дополнительной информацией об устройстве. Это позволяет создавать правила условного доступа, которые обеспечивают доступ с устройств в соответствии со стандартами безопасности и соответствия требованиям.

#### <a name="privileged-identity-management"></a>Управление привилегированными пользователями (PIM)

[Azure Active Directory (AD) Привилегированное управление идентификацией](../../active-directory/privileged-identity-management/pim-configure.md) позволяет управлять, контролировать и контролировать привилегированные идентификаторы и доступ к ресурсам в Azure AD, а также к другим онлайн-сервисам Microsoft, таким как Office 365 или Microsoft Intune.
...
[Azure AD Identity Protection](../../active-directory/identity-protection/overview.md) — это служба безопасности, которая обеспечивает консолидированное представление об обнаружении рисков и потенциальных уязвимостях, влияющих на идентификаторы вашей организации. Identity Protection использует существующие возможности обнаружения аномалий Azure Active Directory (доступны через отчеты anomalous Activity Reports Azure AD) и вводит новые типы обнаружения рисков, которые могут обнаруживать аномалии в режиме реального времени.

## <a name="secure-resource-access"></a>Безопасный доступ к ресурсам

Управление доступом в Azure начинается с оплаты. Владельцем учетной записи Azure, к которым можно получить доступ, посещая [центр учетной записи Azure,](https://account.windowsazure.com/subscriptions)является Администратор учетной записи (AA). Подписки — это контейнеры для выставления счетов, которые в то же время обеспечивают защиту: у каждой подписки есть администратор службы, который может добавлять, удалять и изменять ресурсы Azure в этой подписке на портале Azure. По умолчанию администратором службы является администратор учетной записи. Но администратор учетной записи может изменить администратора службы в Центре управления учетной записью Azure.
...
Ориентированным на информационную безопасность компаниям следует сосредоточиться на предоставлении сотрудникам именно тех разрешений, которые им необходимы. Слишком большое число разрешений делает учетную запись уязвимой для злоумышленников. Недостаточное число разрешений мешает эффективной работе сотрудников. [Управление доступом на основе ролей (RBAC) Azure](../../role-based-access-control/overview.md) помогает решить эту задачу, обеспечивая точное управление доступом для Azure.
...
- В пути: Когда данные передаются между компонентами, местоположениями или программами, например, по сети, через автобус обслуживания (от предварительного к облачному и наоборот, включая гибридные соединения, такие как ExpressRoute), или во время процесса ввода/вывода, считается, что они неподвижны.
...
Цель шифрования в состоянии покоя заключается в шифровании всех данных. При этом исключается возможность потери важных данных или всех сохраненных расположений. Укажите все данные, хранящиеся в приложении.
...
Для каждого магазина, который вы используете, используйте существующее шифрование в поддержку отдыха.

- Дополнительные сведения о шифровании в службе хранилища Azure для неактивных данных см. в [этой статье](../../storage/common/storage-service-encryption.md).
...
- Дополнительные сведения о шифровании на виртуальных машинах и хранилищах на локальных дисках см. в статье [Дисковое шифрование Azure для виртуальных машин IaaS под управлением Windows и Linux](../azure-security-disk-encryption-overview.md).
...
Службы, реализующие шифрование в Rest, как правило, по-прежнему подвержены ключам шифрования или данным, оставленным незашифрованными на диске хоста (например, в файле страницы хост-ОС). Поэтому службы должны обеспечить шифрование объема узла для их услуг. Для упрощения этой задачи группа специалистов по вычислениям включила развертывание шифрования узла, использующего предохранитель сетевого ключа [BitLocker](https://technet.microsoft.com/library/dn306081.aspx) и расширение службы DCM, а также агент для шифрования тома узла.

Большинство служб внедряются на стандартных виртуальных машинах Azure. Такие службы должны получать [шифрование узла](../azure-security-disk-encryption-overview.md) автоматически, когда служба вычислений включает его. Для служб, работающих в кластере, управляемом службой вычислений, шифрование узла включится автоматически, как только будет развернут Windows Server 2016.
...
Организации, которые не защищают данные в пути, более восприимчивы к [атакам «человек в середине»,](https://technet.microsoft.com/library/gg195821.aspx) [подслушиванию](https://technet.microsoft.com/library/gg195641.aspx)и угону сеансов. Такие атаки могут стать первым шагом в ходе получения доступа к конфиденциальным данным.
...
## <a name="secure-your-application"></a>Защищайте свое приложение
...
[Брандмауэр веб-приложений (WAF)](../../application-gateway/waf-overview.md) — это функция [Gateway application,](../../application-gateway/overview.md) обеспечивающая централизованную защиту веб-приложений от распространенных уязвимостей и уязвимостей.
...
> [!Note]
> Для более подробного списка правил и их [Core rule sets](../../application-gateway/waf-overview.md)защиты см.
...
Служба приложений Azure использует ту же антивредоносную программу, что и облачные службы Azure и виртуальные машины. Чтобы узнать больше об этом, обратитесь к нашей [документации Antimalware](antimalware.md).

## <a name="secure-your-network"></a>Защищайте свою сеть
...
[Сетевая инфраструктура Azure](../../virtual-machines/windows/infrastructure-example.md) позволяет безопасно подключать ресурсы Azure друг к другу с [помощью виртуальных сетей (VNets).](../../virtual-network/virtual-networks-overview.md) Виртуальная сеть — это представление вашей собственной сети в облаке. Это логическая изоляция облачной сети Azure, выделенной для вашей подписки. Виртуальные сети можно также подключать к локальным сетям.

![Обеспечение безопасности сети (защита)](./media/technical-capabilities/azure-security-technical-capabilities-fig6.png)

Если вам нужен базовый контроль доступа к сети (на основе IP-адреса и протоколов TCP или UDP), то вы можете использовать [группы сетевой безопасности.](../../virtual-network/virtual-network-vnet-plan-design-arm.md) Группа безопасности сети (NSG) — это базовый брандмауэр для фильтрации пакетов с отслеживанием состояния, который позволяет контролировать доступ по [5 кортежам](https://www.techopedia.com/definition/28190/5-tuple).

Сетевые функции Azure поддерживают возможность настройки поведения маршрутизации для сетевого трафика в виртуальных сетях Azure. Это можно сделать, нанастройки [маршрутов, определяемых пользователем,](../../virtual-network/virtual-networks-udr-overview.md) в Azure.

[Принудительное туннелирование](https://www.petri.com/azure-forced-tunneling) — это механизм, который можно использовать для обеспечения того, чтобы ваши службы не могли инициировать подключение к устройствам в Интернете.

Azure поддерживает подключение к локальной сети по выделенному каналу связи WAN и к виртуальной сети Azure с помощью [ExpressRoute](../../expressroute/expressroute-introduction.md). Для связи между Azure и вашим сайтом будет использоваться выделенное подключение вне общедоступного Интернета. Если приложение Azure работает в нескольких центрах обработки данных, можно использовать [azure Traffic Manager](../../traffic-manager/traffic-manager-overview.md) для интеллектуального направления запросов пользователей в экземплярах приложения. Вы также можете перенаправлять трафик в службы, не запущенные в Azure, если они доступны в Интернете.
...
[Резервное копирование Azure](../../backup/backup-overview.md) — это масштабируемое решение, которое защищает данные приложения с нулевыми капитальными вложениями и минимальными эксплуатационными расходами. Ошибки приложений могут повредить данные, а ошибки пользователей — привести к ошибкам в коде приложений. Служба архивации Azure защитит виртуальные машины Windows и Linux.
...
## <a name="ensure-compliance-cloud-services-due-diligence-checklist"></a>Обеспечить соответствие требованиям: Контрольный список служб облачных услуг
...
## <a name="azure-infrastructure-and-application-security-validation"></a>Проверка инфраструктуры и безопасности приложений Azure

[Azure Operational Security](operational-security.md) относится к службам, элементам управления и функциям, доступным пользователям для защиты их данных, приложений и других ресурсов в Microsoft Azure.
...
[Azure Monitor](../../azure-monitor/index.yml) — это решение для управления ИТ для гибридного облака. Используемые в одиночку или для расширения существующего развертывания System Center журналы Azure Monitor дают максимальную гибкость и контроль для управления инфраструктурой на основе облачных технологий.

![Azure Monitor](./media/technical-capabilities/azure-security-technical-capabilities-fig8.png)

С Azure Monitor вы можете управлять любым экземпляром в любом облаке, включая на территории, Azure, AWS, Windows Server, Linux, VMware и OpenStack, по более низкой цене, чем конкурентоспособные решения. Созданная для облачных технологий, Azure Monitor предлагает новый подход к управлению предприятием, который является самым быстрым и экономичным способом решения новых бизнес-задач и размещения новых рабочих нагрузок, приложений и облачных сред.
...
[Центр безопасности Azure](../../security-center/security-center-intro.md) помогает вам выявлять и предотвращать угрозы, а также принимать ответные меры благодаря более полной информации о состоянии ресурсов Azure и контролю над их безопасностью. Она включает встроенные функции мониторинга безопасности и управления политиками для подписок Azure, помогает выявлять угрозы, которые в противном случае могли бы оказаться незамеченными, и взаимодействует с широким комплексом решений по обеспечению безопасности.
...
- **Пользовательские события и метрики,** которые вы пишете сами в клиентском или серверном коде, для отслеживания бизнес-событий, таких как проданные товары или выигранные игры.

Обычно инфраструктура приложения состоит из ряда компонентов, например из виртуальной машины, учетной записи хранения и виртуальной сети или веб-приложения, базы данных, сервера базы данных и служб сторонних поставщиков. Эти компоненты не отображаются как отдельные сущности, вместо этого они воспринимаются как связанные и взаимозависимые части одной сущности. Их нужно развертывать, контролировать и отслеживать в виде группы. [Менеджер ресурсов Azure](../../azure-resource-manager/management/overview.md) позволяет работать с ресурсами в вашем решении в группе.
...
> [!Note]
> Диспетчер ресурсов предоставляет новый способ развертывания ваших решений и управления ими. Если вы использовали предыдущую модель развертывания и хотите [Understanding Resource Manager Deployment and classic deployment](../../azure-resource-manager/management/deployment-models.md)узнать об изменениях, см.

## <a name="next-steps"></a>Дальнейшие действия

Дополнительные сведения о безопасности можно узнать, ознакомившись с указанными далее подробными статьями.

- [Аудит и лесозаготовки](https://www.microsoft.com/en-us/trustcenter/security/auditingandlogging)
...
- [Управление идентификацией и доступом](https://www.microsoft.com/en-us/trustcenter/security/identity)

- [Сетевая безопасность](https://www.microsoft.com/en-us/trustcenter/security/networksecurity)
