...
ms.locfileid: "81454685"
---
# <a name="azure-data-encryption-at-rest"></a>Шифрование неактивных данных в Azure

Microsoft Azure включает в себя инструменты для защиты данных в соответствии с потребностями вашей компании в области безопасности и соответствия требованиям. В этой статье основное внимание уделяется следующим вопросам:
...
- Ключи должны храниться в безопасном месте с применением доступа на основе удостоверений и политик аудита. Ключи шифрования данных часто шифруются ключом шифрования в Azure Key Vault для дальнейшего ограничения доступа.
...
Шифрование в состоянии покоя также может потребоваться из-за необходимости организации в управлении данными и соблюдении нормативных требований. В отраслевых и государственных нормативах, например HIPAA, PCI и FedRAMP, описываются меры по обеспечению безопасности на основе требований к шифрованию и защите данных. Шифрование при хранении является обязательной мерой, необходимой для соблюдения некоторых из этих нормативов. Для получения дополнительной информации о подходе корпорации Майкрософт [Federal Information Processing Standard (FIPS) Publication 140-2](https://docs.microsoft.com/microsoft-365/compliance/offering-fips-140-2)к проверке FIPS 140-2 см. 

Шифрование неактивных данных не только позволяет выполнить нормативные требования и требования соответствия, но и предоставляет расширенную защиту для платформы. Microsoft Azure предоставляет соответствующую стандартам платформу для служб, приложений и данных. Также предоставляется поддержка на уровне оборудования для физической защиты, управления доступом к данным и аудита. Тем не менее, важно обеспечить дополнительные "перекрывающиеся" меры безопасности в случае, если одна из других мер безопасности не удается и шифрование в состоянии обеспечить такую меру безопасности.
...
При реализации шифрования неактивных данных используется несколько ключей шифрования. Хранение ключа шифрования в Azure Key Vault обеспечивает безопасный доступ к ключам и центральное управление ключами. Тем не менее, локальный доступ к ключам шифрования более эффективен для массового шифрования и расшифровки, чем взаимодействие с Key Vault для каждой операции данных, что позволяет более эффективное шифрование и более высокую производительность. Ограничение использования ключа шифрования снижает риск его компрометации, а также затраты на повторное шифрование при необходимости замены ключа. Для удовлетворения всех этих потребностей в моделях отдыха используются ключевые иерархии, состоящие из следующих типов ключей:
...
Для сценариев, в которых требуется шифровать неактивные данные и управлять ключами шифрования, можно использовать шифрование на стороне сервера с помощью управляемых пользователем ключей в хранилище ключей. Некоторые службы могут хранить корневой ключ шифрования ключей в Azure Key Vault и хранить зашифрованный ключ шифрования данных во внутреннем расположении рядом с данными. В таком сценарии пользователи могут переносить свои собственные ключи (BYOK) в хранилище ключей или создать ключи и использовать их для шифрования нужных ресурсов. В то время как Поставщик ресурсов выполняет операции шифрования и расшифровки, он использует настроенный ключ шифрования в качестве основного ключа для всех операций шифрования.

Потеря ключей шифрования означает потерю данных. По этой причине ключи не должны быть удалены. Ключи должны быть резервного копирования при создании или повороте. [Soft-Delete](https://docs.microsoft.com/azure/key-vault/key-vault-ovw-soft-delete) должен быть включен в любое хранилище, где хранятся ключи шифрования. Вместо того, чтобы удалять ключ, набор включен в ложные или установить дату истечения.
...
- Полный контроль над используемыми ключами – ключи шифрования управляются в Key Vault клиента под контролем клиента.
...
Данные клиента Platform as a Service (PaaS) обычно находятся в службе хранения данных, такой как Blob Storage, но также могут быть кэшированы или сохранены в среде выполнения приложения, например, в виртуальной машине. Чтобы просмотреть доступные варианты шифрования неактивных данных, ознакомьтесь с таблицей ниже для используемых хранилищ и платформ приложений.
...
Все управляемые диски, моментальные снимки и изображения шифруются с помощью шифрования служб хранения с помощью ключа, управляемого службой. Более полное шифрование в решении Rest гарантирует, что данные никогда не сохраняются в незашифрованном виде. При обработке данных на виртуальной машине данные могут сохраняться в файле страницы Windows или файле своп Linux, сбою в сбое или в журнале приложений. Чтобы убедиться, что эти данные шифруются при хранении, приложения IaaS могут использовать службу "Шифрование дисков Azure" на виртуальной машине Azure IaaS (Windows или Linux) и виртуальном диске.
...
Любой пользователь, который использует функции инфраструктуры как услуги (IaaS) Azure, может включить шифрование неактивных данных на виртуальных машинах IaaS и дисках с помощью шифрования дисков Azure. Для получения дополнительной информации о шифровании Azure Disk можно ознакомиться с [документацией по шифрованию azure Disk.](../azure-security-disk-encryption-overview.md)

#### <a name="azure-storage"></a>Хранилище Azure

Все службы хранения данных Azure (хранилище Blob, хранение очередей, хранение таблицы и файлы Azure) поддерживают шифрование на стороне сервера в состоянии покоя; некоторые службы дополнительно поддерживают ключи, управляемые клиентами, и шифрование на стороне клиента. 
...
|----------------------------------|--------------------|-----------------------------------------|--------------------|
|                                  | **На стороне сервера с помощью управляемого службой ключа**     | **Серверная сторона с помощью ключа, управляемого клиентом**             | **Клиент-сторона с помощью ключа, управляемого клиентом**      |
...
| Исследования приложений Azure Monitor | Да                | Да                | -                  |
| Аналитика журналов Azure Monitor      | Да                | Да                | -                  |
...
| База данных Azure S'L для MariaDB   | Да                | -                  | -                  |
| База данных Azure S'L для MyS'L     | Да                | Да                | -                  |
| База данных Azure S'L для PostgreS'L | Да               | Да                | -                  |
...
| Подготовка устройств Концентратора IoT      | Да                | Да                | -                  |
| **Управление**    |                    |                    |                    |
...
| Премиум-хранилище Blob             | Да                | Да, 2048-разрядный RSA  | Да                |
| Хранилище дисков                     | Да                | Да                | -                  |
| Ультра-хранилище дисков               | Да                | Да                | -                  |
| Управляемое хранение дисков             | Да                | Да                | -                  |
| Хранилище файлов                     | Да                | Да, 2048-разрядный RSA  | -                  |
| Хранение файлов Премиум             | Да                | Да, 2048-разрядный RSA  | -                  |
...
\*Эта служба не сохраняет данные. Переходные кэши, если таковые имеются, шифруются ключом Майкрософт.

\*\*Эта служба поддерживает хранение данных в собственном ключевом хранилище, учетной записи хранения или других службах хранения данных, которая уже поддерживает шифрование сервера с помощью ключей, управляемых клиентом.

## <a name="conclusion"></a>Заключение

Защита данных пользователя, хранимых в службах Azure, представляет особую важность для Майкрософт. Все службы, размещенные в Azure, обязуются предоставлять параметры шифрования неактивных данных. Службы Azure поддерживают либо управляемые сервисом ключи, либо ключи, управляемые клиентом, либо шифрование на стороне клиента. Службы Azure в целом повышают шифрование при доступности rest, и в ближайшие месяцы планируется создать новые опции для предварительного просмотра и общей доступности.
