...
Цель написания этой статьи — предоставить общий обзор более надежной системы безопасности после развертывания, посвященной нашим контрольному списку «[5 шагов по обеспечению безопасности инфраструктуры идентификации](steps-secure-identity.md)», в котором описываются некоторые основные функции и Обслуживание.
...
* Планирование регулярных улучшений безопасности
...
**Рекомендации**: центрирование управления безопасностью и обнаружение идентификаторов пользователей и служб.
**Сведения**: используйте Azure AD для совместного использования элементов управления и удостоверений.
...
**Рекомендации**. Создайте один экземпляр Azure AD. Согласованность и единые заслуживающие доверия источники увеличивают ясность и снижают риски безопасности от ошибок и сложности настройки.
...
**Рекомендации**. для разработки новых приложений используйте Azure AD для проверки подлинности.
...
**Сведения**: настройте [Условный доступ](/azure/active-directory/active-directory-conditional-access-azure-portal) Azure AD на основе группы, расположения и чувствительности приложения для приложений SaaS и приложений, подключенных к Azure AD.

**Рекомендации**: блокировать устаревшие протоколы проверки подлинности.
**Сведения**: злоумышленники используют слабые места в старых протоколах каждый день, особенно для атак методом распыления паролей. Настройка условного доступа для блокировки устаревших протоколов. Дополнительные сведения см. в видео [Azure AD: Do and «против](https://www.youtube.com/watch?v=wGk0J4z90GI) ».

## <a name="plan-for-routine-security-improvements"></a>Планирование регулярных улучшений безопасности

Безопасность всегда развивается, и важно встроить в облако и инфраструктуру управления удостоверениями способ регулярного отображения и обнаружения новых способов защиты среды.

Оценка безопасности идентификации — это набор рекомендуемых средств управления безопасностью, которые корпорация Майкрософт публикует, чтобы предоставить вам числовой показатель для совместного измерения уровня безопасности и планирования будущих улучшений безопасности. Вы также можете просматривать баллы по сравнению с другими отраслями, а также со временем.

**Рекомендации**. Планируйте процедуры проверки безопасности и улучшения в соответствии с рекомендациями в отрасли.
**Сведения**: используйте функцию оценки безопасности идентификации для ранжирования улучшений с течением времени.
...
**Вариант 1**. Включение MFA для всех пользователей и методов входа с использованием **преимуществ**по умолчанию безопасности Azure AD. Этот параметр позволяет легко и быстро применять MFA для всех пользователей в вашей среде с помощью строгой политики:

* Запрос административных учетных записей и административных механизмов входа
* Требовать запрос MFA через Microsoft Authenticator для всех пользователей
* Ограничьте устаревшие протоколы проверки подлинности.

Этот метод доступен для всех уровней лицензирования, но не может быть смешан с существующими политиками условного доступа. Дополнительные сведения см. в подокнах по умолчанию для безопасности Azure AD.

**Вариант 2**. [включите многофакторную проверку подлинности, изменив пользовательское состояние](../../active-directory/authentication/howto-mfa-userstates.md).   
...
**Вариант 3**. [включите многофакторную проверку подлинности с политикой условного доступа](/azure/active-directory/authentication/howto-mfa-getstarted).
...
**Вариант 4**. Включите многофакторную проверку подлинности с политиками условного доступа, оценивая риск для пользователя и входа в [Защита идентификации Azure AD](/azure/active-directory/authentication/tutorial-risk-based-sspr-mfa).   
...
**Рекомендации**: разделение обязанностей внутри команды и предоставление пользователям доступа только для тех пользователей, которые им необходимы для выполнения своих задач. Вместо того чтобы предоставлять всем неограниченные разрешения в подписке или ресурсах Azure, разрешите только определенные действия в определенной области.
...
> [!Note]
> Определенные разрешения создают ненужные сложности и путаницу, накапливаясь в "устаревшей" конфигурации, которую трудно исправить, не опасаясь нарушения. Избегайте использования разрешений для конкретных ресурсов. Вместо этого используйте группы управления для разрешений в масштабе предприятия и групп ресурсов для разрешений в рамках подписок. Избегайте разрешения для конкретных пользователей. Вместо этого назначьте доступ к группам в Azure AD.

**Рекомендации**. Предоставьте группам безопасности доступ к ресурсам Azure, чтобы получить возможность оценивать и исправлять риски.
...
**Рекомендации**. Убедитесь, что все критические учетные записи администратора являются управляемыми учетными записями Azure AD.
...
**Рекомендации**. Убедитесь, что все критические роли администратора имеют отдельную учетную запись для административных задач, чтобы избежать фишинга и других атак на нарушение прав администратора.
...
**Рекомендации**. в случае аварийной ситуации в случае аварии следует приостанавливать процесс «стекла».
...
**Рекомендация**. требовать, чтобы все критические учетные записи администратора были меньше пароля (предпочтительно) или требовать многофакторную проверку подлинности.
...
**Рекомендации**. отмените предоставление учетных записей администратора, когда сотрудники покидают вашу организацию.
**Подробно**: Поставьте процесс, который отключает или удаляет учетные записи администратора, когда сотрудники оставляют свою организацию.

**Рекомендации**. регулярно тестируйте учетные записи администратора с помощью текущих методов атак.
