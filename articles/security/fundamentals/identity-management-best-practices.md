...
Многие считают удостоверение основным периметром безопасности. Это связано с изменением взглядов на обеспечение безопасности сети. Периметры сети становятся все более уязвимыми, а традиционная защита уже не так эффективна, как до распространения устройств [BYOD](/mem/intune/fundamentals/byod-technology-decisions) и облачных приложений.
...
**Сведения**. Синхронизируйте свой локальный каталог с облачным каталогом с помощью [Azure AD Connect](../../active-directory/hybrid/whatis-hybrid-identity.md).
...
Дополнительные сведения см. в статье о [реализации синхронизации хэша паролей с помощью службы синхронизации Azure AD Connect](../../active-directory/hybrid/how-to-connect-password-hash-synchronization.md).
...
**Сведения**. Используйте правильные возможности для поддержки проверки подлинности:
  ...
  - [Azure AD B2B](../../active-directory/external-identities/index.yml) для гостевых пользователей и внешних партнеров;
...
**Сведения**. Azure AD [расширяет локальную службу Active Directory](../../active-directory/hybrid/whatis-hybrid-identity.md) в облако. Пользователи могут использовать свои основные рабочие или учебные учетные записи для присоединенных к домену устройств, ресурсов компании и всех веб- и SaaS-приложений, необходимых для решения поставленных задач. Пользователям не нужно помнить несколько наборов учетных данных. Доступ к приложениям предоставляется или отменяется автоматически на основе членства в группах организации и статуса конкретного сотрудника. Вы можете также управлять доступом к приложениям из коллекции или к собственным локальным приложениям, разработанным и опубликованным с помощью [прокси приложения Azure AD](../../active-directory/manage-apps/application-proxy.md).

Предоставьте пользователям возможность единого входа, чтобы они могли получать доступ к [приложениям SaaS](../../active-directory/manage-apps/what-is-single-sign-on.md), используя рабочую или учебную учетную запись в Azure AD. Это применимо не только к приложениям SaaS корпорации Майкрософт, но и к другим приложениям, таким как [Google Apps](../../active-directory/saas-apps/google-apps-tutorial.md) и [Salesforce](../../active-directory/saas-apps/salesforce-tutorial.md). Приложение можно настроить для использования Azure AD в качестве поставщика [удостоверений на основе SAML](../../active-directory/fundamentals/active-directory-whatis.md). Azure AD из соображений безопасности не выдает токен, который позволяет войти в приложение, если у пользователя нет права на доступ к приложению в Azure AD. Вы можете предоставить пользователю такое право напрямую или через одну из групп, в которые он входит.
...
Этот метод доступен для всех уровней лицензирования, но его нельзя сочетать с существующими политиками условного доступа. Дополнительные сведения см. в подокнах [по умолчанию для безопасности Azure AD](../../active-directory/fundamentals/concept-fundamentals-security-defaults.md) .
...
**Преимущество**. Это традиционный метод применения двухфакторной проверки подлинности. Он работает как с [многофакторной проверкой подлинности Azure AD в облаке, так и в Azure сервер многофакторной идентификации](../../active-directory/authentication/concept-mfa-howitworks.md). Использование этого метода требует, чтобы пользователи выполняли двухфакторную проверку подлинности при каждом входе в учетную запись, и переопределяет политики условного доступа.

Чтобы определить, где должна быть включена многофакторная проверка подлинности, Узнайте, [какая версия Azure AD MFA подходит для моей организации](../../active-directory/authentication/concept-mfa-howitworks.md).

**Вариант 3**. [Включение Многофакторной идентификации с политикой условного доступа](../../active-directory/authentication/howto-mfa-getstarted.md).
...
Это наиболее гибкий способ включить двухфакторную проверку подлинности для пользователей. Включение политики условного доступа работает только для многофакторной идентификации Azure AD в облаке и является функцией Azure AD уровня "Премиум". Дополнительные сведения об этом методе см. в этой статьи в этой [службе](../../active-directory/authentication/howto-mfa-getstarted.md).
...
Этот метод использует оценку рисков защиты идентификации Azure AD, чтобы определить необходимость требования двухфакторной проверки подлинности на основе риска пользователя и входа для всех облачных приложений. Этот метод требует лицензирования Azure Active Directory P2. Дополнительные сведения об этом методе см. в статье [Защита идентификации Azure Active Directory](../../active-directory/identity-protection/overview-identity-protection.md).

> [!Note]
> Вариант 2. Включение многофакторной проверки подлинности путем изменения пользовательской среды, переопределяет политики условного доступа. Поскольку параметры 3 и 4 используют политики условного доступа, вы не можете использовать для них параметр 2.
...
Управление доступом к облачным ресурсам является критически важной функцией в любой организации, использующей облако. [Управление доступом на основе ролей Azure (Azure RBAC)](../../role-based-access-control/overview.md) позволяет управлять доступом к ресурсам Azure, их действиям и тем, к каким областям они имеют доступ.
...
С помощью [Azure RBAC](../../role-based-access-control/overview.md) можно назначать разрешения пользователям, группам и приложениям в определенной области. Областью назначения роли может быть подписка, группа ресурсов или отдельный ресурс.
...
**Сведения**: используйте [встроенные роли Azure](../../role-based-access-control/built-in-roles.md) в Azure для назначения привилегий пользователям.
...
**Сведения**: предоставьте группам безопасности роль читателя Azure RBAC [Security](../../role-based-access-control/built-in-roles.md#security-reader) . Вы можете использовать корневую группу управления или группу управления сегментом в зависимости от области ответственности:
...
**Сведения**: добавьте группы безопасности с этими потребностями в роль [администратора безопасности](../../role-based-access-control/built-in-roles.md#security-admin) Azure RBAC, чтобы они могли просматривать политики безопасности, просматривать состояния безопасности, изменять политики безопасности, просматривать оповещения и рекомендации, а также отклонять предупреждения и рекомендации. Для этого можно использовать корневую группу управления или группу управления сегментом в зависимости от области ответственности.
...
- [Рабочие станции с привилегированным доступом](https://4sysops.com/archives/understand-the-microsoft-privileged-access-workstation-paw-security-model/) предоставляют выделенную операционную систему для выполнения конфиденциальных задач, защищенную от атак из Интернета и векторов угроз.
...
Можно воспользоваться [Azure Resource Manager](../../azure-resource-manager/management/overview.md) и создать политики безопасности с определениями, описывающими действия или ресурсы, которые следует отклонять в первую очередь. Эти определения политик назначаются для нужной области, такой как подписка, группа ресурсов или отдельный ресурс.

> [!NOTE]
> Политики безопасности не совпадают с политиками Azure RBAC. Они фактически используют Azure RBAC для авторизации пользователей при создании этих ресурсов.
>
>
...
- попытки входа [без трассировки](../../active-directory/reports-monitoring/howto-find-activity-reports.md);
- [атаки методом перебора](../../active-directory/reports-monitoring/howto-find-activity-reports.md), направленные на определенную учетную запись;
- попытки входа из нескольких расположений;
- попытки входа с [зараженных устройств](../../active-directory/reports-monitoring/howto-find-activity-reports.md);
- подозрительные IP-адреса.

**Сведения**. Используйте [отчеты об аномалиях](../../active-directory/reports-monitoring/overview-reports.md) в Azure AD Premium. Предусмотрите процессы и процедуры, которые ИТ-администраторам следует выполнять для ежедневного составления этих отчетов или их создания по требованию (обычно в сценарии реагирования на инциденты).
...
**Сведения**. Используйте [защиту идентификации Azure AD](../../active-directory/identity-protection/overview-identity-protection.md), которая помечает текущие риски на панели мониторинга и отправляет ежедневные сводные уведомления по электронной почте. Для защиты удостоверений организации можно настроить политики на основе рисков, которые автоматически реагируют на обнаруженные проблемы в случае достижения указанного уровня риска.
