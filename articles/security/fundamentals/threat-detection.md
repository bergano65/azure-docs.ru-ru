...
ms.locfileid: "75981468"
---
# <a name="azure-advanced-threat-detection"></a>Расширенное обнаружение угроз Azure

Azure предлагает встроенные в расширенную функциональность обнаружения угроз через такие службы, как Активный каталог Azure (Azure AD), журналы Azure Monitor и Центр безопасности Azure. С помощью этого набора служб и функций безопасности можно легко и быстро понять, что произошло в развертываниях Azure.
...
[Azure AD Identity Protection](../../active-directory/identity-protection/overview.md) — это функция [Azure Active Directory Premium P2,](../../active-directory/active-directory-whatis.md) которая предоставляет обзор обнаружения рисков и потенциальных уязвимостей, которые могут повлиять на идентификаторы организации. Identity Protection использует существующие возможности обнаружения аномалий Azure AD, доступные через [отчеты anomalous Activity Reports Azure AD,](../../active-directory/active-directory-reporting-azure-portal.md)и вводит новые типы обнаружения рисков, которые могут обнаруживать аномалии в реальном времени.

![Схема защиты идентификации Azure AD](./media/threat-detection/azure-threat-detection-fig1.png)

Identity Protection использует адаптивные алгоритмы машинного обучения и эвристику для обнаружения аномалий и обнаружений рисков, которые могут указывать на то, что личность была скомпрометирована. Используя эти данные, Identity Protection генерирует отчеты и оповещения, чтобы вы могли исследовать эти обнаружения рисков и принять соответствующие меры по исправлению или смягчению последствий.

Защита идентификации Azure Active Directory — это больше, чем просто инструмент для мониторинга и создания отчетности. На основе обнаружения рисков Identity Protection вычисляет уровень риска пользователя для каждого пользователя, чтобы можно было настроить политики, основанные на риске, чтобы автоматически защитить идентификаторы вашей организации.

Эти политики, основанные на риске, в дополнение к другим [элементам управления условным доступом,](../../active-directory/active-directory-conditional-access-azure-portal.md) предоставляемым Azure Active Directory и [EMS,](../../active-directory/active-directory-conditional-access-azure-portal.md)могут автоматически блокировать или предлагать адаптивные действия по исправлению положения, включающие сбросы паролей и многофакторную проверку подлинности.

### <a name="identity-protection-capabilities"></a>Возможности защиты идентификации

Защита идентификации Azure Active Directory — это больше, чем просто инструмент для мониторинга и создания отчетности. Для защиты удостоверений организации можно настроить политики на основе рисков, которые автоматически реагируют на обнаруженные проблемы в случае достижения указанного уровня риска. Эти политики, в дополнение к другим элементам управления условным доступом, предоставляемым Azure Active Directory и EMS, могут либо автоматически блокировать, либо инициировать адаптивные действия по исправлению положения, включая сбросы паролей и многофакторную проверку подлинности.

Примеры способов защиты удостоверений Azure, которые могут помочь защитить учетные записи и удостоверения:

[Обнаружение рисков и рискованных учетных записей](../../active-directory/identity-protection/overview.md)
...
[Исследование обнаружения рисков](../../active-directory/identity-protection/overview.md)
-   Отправка уведомлений об обнаружении рисков.
-   Исследуйте обнаружение рисков с использованием соответствующей и контекстной информации.
-   Базовые рабочие процессы для отслеживания исследований.
-   Легкий доступ к действиям по исправлению, таким как сброс пароля.

[Политики условного доступа на основе рисков](../../active-directory/identity-protection/overview.md)
...
[Журналы Azure Monitor](../../azure-monitor/index.yml) — это облачное решение для управления ИТ- иТ- иТ- ит-инфраструктурой, которое помогает управлять и защищать свою инфраструктуру в помещениях и облачном помещении. Поскольку журналы Azure Monitor реализованы как облачная служба, вы можете быстро работать с минимальными инвестициями в инфраструктурные службы. Новые возможности безопасности предоставляются автоматически, что сокращает затраты на текущее обслуживание и обновление.

В дополнение к самостоятельному предоставлению ценных услуг журналы Azure Monitor могут интегрироваться с компонентами System Center, такими как [System Center Operations Manager,](https://blogs.technet.microsoft.com/cbernier/2013/10/23/monitoring-windows-azure-with-system-center-operations-manager-2012-get-me-started/)для расширения существующих инвестиций в управление безопасностью в облаке. Системный центр и журналы Azure Monitor могут работать вместе, чтобы обеспечить полный опыт управления гибридным.

### <a name="holistic-security-and-compliance-posture"></a>Комплексное состояние безопасности и соответствия

[Панель мониторинга "Безопасность и аудит" в Log Analytics](../../security-center/security-center-intro.md) позволяет всесторонне изучать данные об информационной безопасности в вашей организации. Для этого применяются встроенные поисковые запросы, которые позволяют определять важные моменты, требующие вашего внимания. Панель мониторинга безопасности и аудита — это домашний экран для всего, что связано с безопасностью в журналах Azure Monitor. Она содержит обобщенное представление состояния безопасности компьютеров организации. Она также позволяет просматривать все события за последние 24 часа, 7 дней или другие настраиваемые промежутки времени.

Журналы Azure Monitor помогут вам быстро и легко понять общую позицию безопасности любой среды, все в контексте ИТ-операций, включая оценку обновления программного обеспечения, оценку антивредоносного ПО и базовые настройки конфигурации. Вы можете изучить готовые данные журнала безопасности, чтобы оптимизировать процессы аудита безопасности и соответствия.

![Панель мониторинга "Безопасность и аудит" в Log Analytics](./media/threat-detection/azure-threat-detection-fig3.jpg)

Данные на панели мониторинга "Безопасность и аудит" в Log Analytics подразделяются на четыре основные категории:
...
В центре [журналов Azure Monitor](../../log-analytics/log-analytics-queries.md) находится репозиторий, который размещается Azure.
...
![Панель мониторинга мониторинга Azure](./media/threat-detection/azure-threat-detection-fig5.png)
...
Большая часть взаимодействия с журналами Azure Monitor осуществляется через портал Azure, который работает в любом браузере и предоставляет вам доступ к настройкам конфигурации и нескольким инструментам для анализа и принятия действий по сбору данных. На портале вы можете использовать:
* [Поиск по журналам](../../log-analytics/log-analytics-queries.md), при котором вы формируете запросы для анализа собранных данных.
* [Панели мониторинга](../../azure-monitor/learn/tutorial-logs-dashboards.md), которые можно настраивать, используя графические представления большинства значимых поисковых запросов.
* [Решения](../../monitoring/monitoring-solutions.md), которые обеспечивают дополнительные функциональные возможности и инструменты анализа.

![Инструменты анализа](./media/threat-detection/azure-threat-detection-fig6.png)

Решения добавляют функциональность в журналы Azure Monitor. Они в основном работают в облаке и обеспечивают анализ данных, собранных в репозитории аналитики журналов. Решения могут также определять новые типы записей, которые могут быть собраны с помощью поиска журнала или с помощью дополнительного пользовательского интерфейса, который предлагается в панели мониторинга аналитики журнала.
...
Центр безопасности Azure помогает защитить ваши ресурсы Azure. Он включает в себя встроенные функции мониторинга безопасности и управления политиками для подписок Azure. В этой службе можно определить политики как для подписок Azure, так и для [групп ресурсов](../../azure-resource-manager/management/manage-resources-portal.md) для повышения степени детализации.

![Схема для центра безопасности Azure](./media/threat-detection/azure-threat-detection-fig8.png)
...
![Обнаружение угроз в центре безопасности Azure](./media/threat-detection/azure-threat-detection-fig9.jpg)
...
Некоторые примеры:
-   **Подозрительное выполнение процесса:** Злоумышленники используют несколько методов для выполнения вредоносного программного обеспечения без обнаружения. Например, злоумышленник может дать вредоносной программе то же имя, что и у обычного системного файла, при этом разместив вредоносные файлы в альтернативных расположениях, использовать имя, очень похожее на имя безвредного файла, или замаскировать настоящее расширение файла. Чтобы обнаруживать подобные выбросы, центр безопасности моделирует поведение процессов и отслеживает их выполнение.

-   **Скрытые вредоносные программы и попытки эксплуатации:** Сложные вредоносные программы могут уклоняться от традиционных антивирусных продуктов, либо никогда не писать на диск или шифрования компонентов программного обеспечения, хранящихся на диске. Тем не менее такую программу может обнаружить анализ памяти, так как в ходе своей работы она не может не оставлять следов в памяти. Когда программа дает сбой, аварийный дамп записывает часть памяти в момент сбоя. Анализируя память в аварийном дампе, центр безопасности Azure может выявить методы, с помощью которых хакер использует уязвимости ПО, получает доступ к конфиденциальным данным и тайно пользуется скомпрометированным компьютером, не снижая его производительность.
...
-   **Мониторинг разведки угроз**: Разведка угроз включает в себя механизмы, индикаторы, последствия и рекомендации по существующим или возникающим угрозам. Эта информация размещается в сообществе специалистов по безопасности, и корпорация Майкрософт непрерывно отслеживает каналы аналитики угроз из внутренних и внешних источников.
...
-   **Настройка обнаружения:** Алгоритмы работают в обход реальных наборов данных клиентов, и исследователи безопасности работают с клиентами для проверки результатов. Чтобы улучшать алгоритмы машинного обучения, специалисты анализируют ложные и истинные срабатывания.
...
[Microsoft antimalware](antimalware.md) for Azure — это одноагентное решение для приложений и сред для клиентов, предназначенное для работы в фоновом режиме без вмешательства человека. Вы можете развернуть систему защиты, соответствующую рабочим нагрузкам своих приложений, и использовать базовую конфигурацию защиты (по умолчанию) или расширенную настраиваемую конфигурацию, включающую отслеживание вредоносных программ. Антивредоносная программа Azure — это компонент безопасности для виртуальных машин Azure, который автоматически устанавливается на всех виртуальных машинах PaaS в Azure.
...
-   **Защита в режиме реального времени:** отслеживает активность в облачных сервисах и виртуальных машинах для обнаружения и блокирования выполнения вредоносных программ.
...
[Брандмауэр веб-приложения](../../app-service/environment/app-service-app-service-environment-web-application-firewall.md) — это функция [шлюза приложений Azure](../../application-gateway/application-gateway-web-application-firewall-overview.md), которая обеспечивает защиту веб-приложений, использующих шлюз приложений для стандартных функций [управления доставкой приложений](https://kemptechnologies.com/in/application-delivery-controllers). Принцип работы брандмауэра веб-приложения заключается в защите приложений от [большинства из основных 10 уязвимостей OWASP](https://www.owasp.org/index.php/Top_10_2010-Main).
...
-   **Гибкое и надежное обнаружение**: Модели обнаружения аномалий позволяют пользователям настраивать настройки чувствительности и обнаруживать аномалии между сезонными и несезонными наборами данных. Пользователи могут настроить модель обнаружения аномалий, чтобы повысить или понизить чувствительность API обнаружения в соответствии с потребностями. Это будет означать обнаружение большего или меньшего числа видимых аномалий в данных с сезонными закономерностями и без них.

-   **Масштабируемое и своевременное обнаружение**: Традиционный способ мониторинга с нынешними пороговыми значениями, установленными знаниями экспертов в области домена, является дорогостоящим и не масштабируемым для миллионов динамически изменяющихся наборов данных. Модели обнаружения аномалий в данном API обучаются и настраиваются автоматически на основе данных журналов и данных, получаемых в режиме реального времени.

-   **Проактивное и действенное**обнаружение: для раннего выявления аномалий можно применять медленное обнаружение тренда и изменения уровня. Обнаруженные ранние аномальные сигналы могут использоваться для привлечения специалистов к изучению проблемных областей и принятию ответных мер. Кроме того, модели анализа первопричин и инструменты оповещения могут разрабатываться на основе этой службы API обнаружения аномалий.
...
- [Реагирование на современные угрозы:](../../security-center/security-center-alerts-overview.md#respond-threats)Помогает выявлять активные угрозы, нацеленные на ресурсы Azure, и предоставляет информацию, необходимую для быстрого реагирования.
