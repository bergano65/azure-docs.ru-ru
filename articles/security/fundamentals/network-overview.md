...
Более удачным вариантом может быть создание VPN-подключения типа "сеть — сеть" между двумя виртуальными сетями. Этот метод использует тот же протокол [туннельного режима IPSec](https://technet.microsoft.com/library/cc786385.aspx) , что и локальное VPN-подключение типа "сеть — сеть", упомянутое выше.
