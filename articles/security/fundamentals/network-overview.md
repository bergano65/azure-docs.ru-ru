...
> [!NOTE]
> Дополнительные сведения о брандмауэрах хранилища см. в разделе [Общие сведения о безопасности службы хранилища Azure](storage-overview.md).
...
[Центр безопасности Azure](../../security-center/security-center-intro.md) может управлять группами безопасности сети на виртуальных машинах и блокировать доступ к виртуальной машине, пока доступ не будет запрошен пользователем с соответствующими разрешениями управления доступом на основе ролей [RBAC](/azure/role-based-access-control/overview). Когда пользователь успешно авторизован, ASC вносит изменения в группу NSG, чтобы разрешить доступ к выбранным портам на указанное время. По истечении времени группы безопасности сети вернутся в исходное состояние защиты.
...
* [Конечные точки обслуживания](../../virtual-network/virtual-network-service-endpoints-overview.md#secure-azure-services-to-virtual-networks)
...
* [Что такое пользовательские маршруты и IP-пересылка](../../virtual-network/virtual-networks-udr-overview.md)
...
Доступ к этим улучшенным функциям сетевой безопасности осуществляется с помощью партнерского решения Azure. Вы можете найти самые актуальные решения для сетевой безопасности партнеров Azure, посетив [Azure Marketplace](https://azure.microsoft.com/marketplace/)и испоиска «безопасность» и «сетевую безопасность».
...
* [Общие сведения о Брандмауэре Azure](/azure/firewall/overview)
...
VPN-подключение "точка — сеть" позволяет настроить частное и безопасное подключение между пользователем и виртуальной сетью. Когда VPN-подключение уже установлено, пользователь может установить RDP- или SSH-подключение к любой виртуальной машине в виртуальной сети через канал VPN. (Это предполагает, что пользователь может аутентифицировать подлинность и авторизован.) От точка к сайту VPN поддерживает:

* SSTP (Secure Socket Tunneling Protocol) — разработанный корпорацией Майкрософт VPN-протокол на основе SSL. Решение SSL VPN может проникать в брандмауэры, так как большинство брандмауэров открывают порт TCP 443, который использует TLS/SSL. Протокол SSTP поддерживается только на устройствах Windows. Azure поддерживает все версии Windows с протоколом SSTP (Windows 7 и более поздние версии).
...
* [О VPN шлюзе](../../vpn-gateway/vpn-gateway-about-vpngateways.md)
...
Более удачным вариантом может быть создание VPN-подключения типа "сеть — сеть" между двумя виртуальными сетями. Этот метод использует тот же [протокол режима туннеля IPSec,](https://technet.microsoft.com/library/cc786385.aspx) что и упомянутое выше подключение от сайта к сайту VPN.
...
* [Налаживать соединение VNet-to-VNet с помощью менеджера ресурсов Azure и PowerShell](../../vpn-gateway/vpn-gateway-vnet-vnet-rm-ps.md)
...
* Разгрузка TLS. Когда клиент подключается к балансулизатору нагрузки, этот сеанс шифруется с помощью протокола HTTPS (TLS). Однако для повышения производительности можно установить соединение между подсистемой балансировки нагрузки и веб-сервером в ее зоне действия с использованием протокола HTTP (без шифрования). Это называется "Разгрузка TLS", потому что веб-серверы, стоящие за балансером нагрузки, не испытывают накладных расходов на процессор, связанных с шифрованием. Это позволяет веб-серверам быстрее обслуживать запросы.
...
* [Обзор шлюза приложений](/azure/application-gateway/application-gateway-introduction)
...
* [Доступная в Интернете подсистема балансировки нагрузки, используемая для нескольких виртуальных машин или служб](/azure/load-balancer/load-balancer-internet-overview)
* [Обзор внутренней подсистемы балансировки нагрузки](/azure/load-balancer/load-balancer-internal-overview)
...
* [Облачные службы Майкрософт и сетевая безопасность](network-best-practices.md)
...
* **Адаптивная настройка:** Интеллектуальное профилирование трафика узнает трафик вашего приложения с течением времени, а также выбирает и обновляет профиль, который является наиболее подходящим для вашего сервиса. Этот профиль корректируется с изменением трафика с течением времени. Защита для уровней от 3 до 7: при использовании брандмауэром веб-приложения предоставляется полная комплексная защита от атак DDoS.
...
> [!NOTE]
> Для получения самых актуальных уведомлений о доступности и статусе этой службы проверьте [страницу обновлений Azure.](https://azure.microsoft.com/updates/?product=network-watcher)
...
* [Журналы активности](../../azure-monitor/platform/platform-logs-overview.md). С помощью этих журналов можно просмотреть все операции, отправленные для ваших подписок Azure. Эти журналы включены по умолчанию и доступны на портале Azure. Ранее они назывались журналами аудита или операционными журналами.
...
* [Журналы Azure Monitor для групп сетевой безопасности (НСГ)](../../virtual-network/virtual-network-nsg-manage-log.md)
