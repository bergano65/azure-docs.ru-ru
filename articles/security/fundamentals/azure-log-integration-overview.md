...
Служба "Интеграция журналов данных Azure" собирает события из журналов средства просмотра событий Windows, [журналов действий Azure](/azure/azure-monitor/platform/activity-logs-overview), [оповещений центра безопасности Azure](/azure/security-center/security-center-intro) и [журналов диагностики Azure](/azure/azure-monitor/platform/diagnostic-logs-overview), которые поступают из ресурсов Azure. Эта интеграция позволит создать в решении SIEM единую панель мониторинга для всех локальных и облачных ресурсов. Панели мониторинга можно использовать для получения, статистической обработки, сопоставления и анализа предупреждений о событиях безопасности.
...
![Процесс интеграции журналов данных Azure](media/azure-log-integration-overview/azure-log-integration.png)
...
* **Журналы управления и контроля**: содержат информацию об операциях CREATE, UPDATE и DELETE в [Azure Resource Manager](/azure/azure-resource-manager/resource-group-overview). К этому типу относятся, например, журналы действий Azure.
...
|**QRadar** | Начните использовать соединитель QRadar, как описано в последнем разделе статьи [Потоковая передача данных мониторинга Azure в концентратор событий для потребления внешним инструментом](/azure/azure-monitor/platform/stream-monitoring-data-event-hubs). | Используйте соединитель QRadar, который описан в последнем разделе статьи [Потоковая передача данных мониторинга Azure в концентратор событий для потребления внешним инструментом](/azure/azure-monitor/platform/stream-monitoring-data-event-hubs). |
|**ArcSight** | Продолжайте использовать интегратор журналов Azure до тех пор, пока не будет доступен соединитель, а затем перейдите на решение на основе соединителя.  | В качестве альтернативы рекомендуется использовать журналы Azure Monitor. Не подключайтесь к службе "Интеграция журналов данных Azure", если вы не готовы выполнить миграцию при появлении соответствующего соединителя. |
...
Если вам нужна помощь, создайте [запрос в службу поддержки](/azure/azure-supportability/how-to-create-azure-support-request). Для этой службы выберите **Интеграция журнала**.
...
* [Интеграция журналов Azure с ведением журнала системы диагностики Azure и пересылкой событий Windows](azure-log-integration-get-started.md). Это руководство содержит пошаговые инструкции по установке службы "Интеграция журналов данных Azure". Также в нем описана интеграция журналов из хранилища Диагностики Azure для Windows (WAD), журналов действий Azure, оповещений центра безопасности Azure и журналов аудита Azure Active Directory.
* [Часто задаваемые вопросы об интеграции журналов Azure](azure-log-integration-faq.md). Эта статья содержит ответы на часто задаваемые вопросы о службе "Интеграция журналов данных Azure".
