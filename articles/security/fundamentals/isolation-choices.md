...
### <a name="vlan-isolation"></a>Изоляция виртуальной ЛС
В каждом кластере существуют три виртуальные локальные сети.

![Изоляция виртуальной ЛС](./media/isolation-choices/azure-isolation-fig8.jpg)
...
Поэтому служба хранилища Azure выполняется на отдельном оборудовании без сетевого подключения к службе вычислений Azure, за исключением логических подключений. Это означает, что при создании виртуального диска место на диске не выделяется для всей емкости. Вместо этого создается таблица, сопоставляющая адреса на виртуальном диске с областями на физическом диске, и эта таблица изначально пуста. **Когда клиент впервые записывает данные на виртуальный диск, на физическом диске выделяется область и указатель нее помещается в таблицу.**
...
-   [шифрование подключения](../../storage/blobs/security-recommendations.md), например шифрование SMB 3.0 для файловых ресурсов Azure;

-   [шифрование на стороне клиента](../../storage/blobs/security-recommendations.md), которое шифрует данные перед их передачей в хранилище и расшифровывает данные после получения из хранилища.
...
-   [Шифрование на стороне клиента](../../storage/blobs/security-recommendations.md) также обеспечивает шифрование при хранении.
