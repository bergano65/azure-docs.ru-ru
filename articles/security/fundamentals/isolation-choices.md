...
description: Узнайте, как Azure обеспечивает изоляцию как от вредоносных, так и незлостных пользователей и предлагает архитекторам различные варианты изоляции.
services: security
...
Для диагностики и обслуживания требуется и используется рабочая модель, которая использует JIT-систему повышения привилегий. Azure AD Privileged Identity Management (PIM) вводит концепцию подходящего админа. [Подходящими админами](../../active-directory/privileged-identity-management/pim-configure.md) должны быть пользователи, которым нужен привилегированный доступ время от времени, но не каждый день. Эта роль будет неактивной, пока пользователю не потребуется доступ. При необходимости пользователь выполнит активацию и станет активным администратором на заранее определенный период времени.
...
- **Автор** может создавать и управлять всеми типами ресурсов Azure, но не может предоставить доступ к другим.
...
В дополнение к изолированным узлам, описанным в предыдущем разделе, Azure также предлагает специальные хосты. Выделенные хосты в Azure — это служба, предоставляющая физические серверы, которые могут размещать одну или несколько виртуальных машин и которые предназначены для одной подписки Azure. Выделенные хоста обеспечивают изоляцию оборудования на физическом уровне сервера. Никакие другие VMs не будут размещены на ваших хостовах. Выделенные узлы развертываются в тех же центрах обработки данных и имеют ту же сеть и базовую инфраструктуру хранения данных, что и другие, неизолированные узлы. Для получения дополнительной информации смотрите подробный обзор [выделенных хостов Azure.](https://docs.microsoft.com/azure/virtual-machines/windows/dedicated-hosts)
...
-   [Шифрование проводов,](../../storage/blobs/security-recommendations.md)например шифрование SMB 3.0 для акций Azure File.

-   [Шифрование на стороне клиента,](../../storage/blobs/security-recommendations.md)для шифрования данных до их передачи в хранилище и расшифровки данных после их передачи из хранилища.

#### <a name="encryption-at-rest"></a>Шифрование при хранении
Для многих организаций [шифрование данных в состоянии покоя](isolation-choices.md) является обязательным шагом на пути к конфиденциальности, соответствию требованиям конфиденциальности и суверенитету данных. В Azure есть три функции, обеспечивающие шифрование неактивных данных.

-   [Шифрование службы хранилища](../../storage/blobs/security-recommendations.md) позволяет настроить автоматическое шифрование данных, записываемых в службу хранилища Azure.

-   [Шифрование на стороне клиента](../../storage/blobs/security-recommendations.md) также обеспечивает функцию шифрования в состоянии покоя.
...
-   Включение шифрования на Linux VMs, настроенных с помощью полоски диска (RAID) с помощью [mdadm](https://en.wikipedia.org/wiki/Mdadm)
...
База данных [Microsoft SQL Azure](../../sql-database/sql-database-single-database-get-started.md) — это облачная служба реляционных баз данных, созданная на основе технологий SQL Server. Она предоставляет высокодоступную, масштабируемую и мультитенантную службу баз данных, размещаемую корпорацией Майкрософт в облаке.
...
![Модель приложения SQL Azure](./media/isolation-choices/azure-isolation-fig10.png)
...
![Изоляции с помощью топологии сети](./media/isolation-choices/azure-isolation-fig12.png)
...
SQL Azure состоит из служб, использующих различные функции компьютеров. S'L Azure делится на "бэкэнд" Облачная база данных и "передний конец" (Gateway /Управление) среды, с общим принципом трафика только вдаваясь в бэк-энд, а не из. Фронтовая среда может общаться с внешним миром других служб и в целом, имеет только ограниченные разрешения в бэк-энде (достаточно, чтобы позвонить в точки входа, которые он должен вызвать).
...
- Узнайте о [вариантах сетевой изоляции для машин в виртуальных сетях Windows Azure.](https://azure.microsoft.com/blog/network-isolation-options-for-machines-in-windows-azure-virtual-networks/) Это включает в себя классический фронт-энд сценарий, где машины в определенной сети бэк-энда или подсети могут только позволить некоторым клиентам или другим компьютерам подключиться к определенной конечной точке на основе разрешаемого списка IP-адресов.

- Узнайте о [виртуальной изоляции машины в Azure](../../virtual-machines/windows/isolation.md). Azure Compute предлагает виртуальные размеры машин, которые изолированы от определенного типа оборудования и предназначены для одного клиента.
