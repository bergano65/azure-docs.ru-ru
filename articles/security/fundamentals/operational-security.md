...
description: Узнайте о журналах Microsoft Azure Monitor, его службах и о том, как они работают.
services: security
...
6.  [Каталог Azure Active](../../active-directory/fundamentals/active-directory-whatis.md)


## <a name="microsoft-azure-monitor-logs"></a>Журналы мониторов Microsoft Azure

Журналы Microsoft Azure Monitor — это решение для управления ИТ для гибридного облака. Используемые в одиночку или для расширения существующего развертывания System Center журналы Azure Monitor дают максимальную гибкость и контроль для управления инфраструктурой на основе облачных технологий.

![Журналы Azure Monitor](./media/operational-security/azure-operational-security-fig1.png)

С помощью журналов Azure Monitor вы можете управлять любым экземпляром в любом облаке, включая на территории, Azure, AWS, Windows Server, Linux, VMware и OpenStack, по более низкой цене, чем конкурентоспособные решения. Созданные для облачных журналов, журналы Azure Monitor предлагают новый подход к управлению предприятием, который является самым быстрым и экономичным способом решения новых бизнес-задач и размещения новых рабочих нагрузок, приложений и облачных сред.

### <a name="azure-monitor-services"></a>Службы Azure Monitor

Основная функциональность журналов Azure Monitor обеспечивается набором служб, запускаемых в Azure. Каждая служба предоставляет определенную функцию управления, благодаря чему можно комбинировать службы для получения различных сценариев управления.
...
Служба Azure Monitor безопасно управляет облачными данными, используя следующие методы:
-   разделение данных;
-   хранение данных;
...
[Резервное копирование Azure](https://azure.microsoft.com/documentation/services/backup) обеспечивает резервное копирование и восстановление служб данных и входит в набор продуктов и служб Azure Monitor.
...
[Azure Monitor](../../security-center/security-center-intro.md) — это облачное решение корпорации Майкрософт по управлению ИТ, которое помогает управлять и защищать свою инфраструктуру в помещениях и облачную инфраструктуру.


[Решения для управления](../../monitoring/monitoring-solutions.md) представляют собой готовые наборы логики, которые реализуют определенный сценарий управления с помощью одного или нескольких служб Azure Monitor. От корпорации Майкрософт и партнеров доступны различные решения, которые можно легко добавить в подписку Azure, чтобы увеличить стоимость ваших инвестиций в Azure Monitor. Как партнер, вы можете создавать собственные решения для поддержки приложений и служб и предоставлять их пользователям через шаблоны Azure Marketplace или quickstart.


![Решения для управления](./media/operational-security/azure-operational-security-fig4.png)

Хорошим примером решения, которое использует несколько служб для обеспечения дополнительных функциональных возможностей, является [решение для управления обновлениями](../../automation/automation-update-management.md). Это решение использует агент [журналов Azure Monitor](../../log-analytics/log-analytics-queries.md) для Windows и Linux для сбора информации о необходимых обновлениях на каждом агенте. Он записывает эти данные в репозиторий журналов Azure Monitor, где их можно проанализировать с помощью включенной панели мониторинга.

При создании развертывания для установки необходимых обновлений используются книги запусков в [Azure Automation.](../../automation/automation-intro.md) Всем процессом можно управлять на портале, не заботясь о деталях.
...
Решение [Журналов безопасности и аудита Azure Monitor](../../security-center/security-center-monitoring.md) позволяет ИТ-отделам активно контролировать все ресурсы, что может помочь свести к минимуму последствия инцидентов безопасности. В журналах Azure Monitor Security and Audit есть домены безопасности, которые можно использовать для мониторинга ресурсов. Домен безопасности предоставляет быстрый доступ к параметрам. Мы более подробно рассмотрим следующие домены, которые имеют отношение к мониторингу безопасности:
...
-   **<a href="https://docs.microsoft.com/azure/network-watcher/network-watcher-next-hop-overview">Следующий прыжок</a>** - Определяет следующий переход для пакетов, маршрутизированных в ткани сети Azure, что позволяет диагностировать любые неправильно настроенные маршруты, определяемые пользователем.

-   **<a href="https://docs.microsoft.com/azure/network-watcher/network-watcher-security-group-view-overview">Представление группы безопасности</a>** - получает эффективные и применяемые правила безопасности, которые применяются на VM.
...
> [!Note]
> Для получения дополнительной информации об [Scalability targets for standard storage accounts](../../storage/common/scalability-targets-standard-account.md)ограничениях учетной записи хранилища см.
...
-   Улучшение безопасности приложений с помощью многофакторной аутентификации Azure AD и условного доступа.
...
> [!Note]
> Для получения дополнительной информации [Azure Active Directory Report Retention Policies](../../active-directory/reports-monitoring/reference-reports-data-retention.md)о сохранении отчетов см.

Для клиентов, заинтересованных в хранении [событий аудита](../../active-directory/active-directory-reporting-activity-audit-logs.md) в течение более длительных периодов хранения, API отчетности может использоваться для регулярного использования событий аудита в отдельный хранилище данных.
...
-   Как данные собираются, обрабатываются и защищены в наборе Azure Monitor.
