...
Предпочтительным методом интеграции журналов Azure является использование соединителя Azure Monitor от поставщика SIEM и выполнение следующих [инструкций](/azure/azure-monitor/platform/stream-monitoring-data-event-hubs). Тем не менее, если поставщик SIEM не предоставляет соединитель для Azure Monitor, в качестве временного решения можно использовать службу "Интеграция журналов данных Azure" (если она поддерживает вашу систему SIEM).
...
![XML-файл события](media/azure-log-integration-faq/event-xml.png)
...
Ошибка:
  ...
  *"Warning creating Role Assignment - AuthorizationFailed" (Предупреждение при создании назначения роли — AuthorizationFailed): Клиент жанедо\@Microsoft.com "с идентификатором объекта" fe9e03e4-4dad-4328-910f-fd24a9660bd2 "не имеет авторизации для выполнения действия" Microsoft. Authorization/roleAssignments/Write "для области"/Subscriptions/ 70d95299-d689-4c97-b971-0d8ff0000000'.*
...
* [Просмотр журналов действий для аудита действий с ресурсами](/azure/azure-resource-manager/resource-group-audit);
...
См. статью [Управление оповещениями безопасности в Центре безопасности Azure и реагирование на них](/azure/security-center/security-center-managing-and-responding-alerts).
...
Сведения о том, как получать, изменять и настраивать конфигурацию системы диагностики Azure, см. в статье [Включение системы диагностики Azure на виртуальной машине под управлением Windows с помощью PowerShell](/azure/virtual-machines/windows/ps-extensions-diagnostics?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json). 
...
При наличии проблем во время установки и настройки создайте [запрос на поддержку](/azure/azure-supportability/how-to-create-azure-support-request). Выберите **Интеграция с журналом** в качестве службы, в которую необходимо отправить запрос поддержки.
