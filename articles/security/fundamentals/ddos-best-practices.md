---
title: Разработка устойчивых решений с помощью DDoS-защиты Azure
...
ms.locfileid: "80548685"
---
# <a name="azure-ddos-protection---designing-resilient-solutions"></a>Защита от DDoS Azure - Разработка устойчивых решений
...
Например, в политике службы "Защита от атак DDoS" указывается, какой должен быть *установлен* объем трафика. (То есть, трафик арендатора должен быть направлен через очистку приборов.) Затем политика определяет, как приборы очистки должны *смягчить* атаку.
...
Служба "Защита от атак DDoS" уровня "Стандартный" предоставляет расширенную телеметрию с помощью [Azure Monitor](/azure/azure-monitor/overview) во время атаки DDoS. Вы можете настроить получение оповещений для любой метрики Azure Monitor, которую использует защита от атак DDoS. Вы можете интегрировать журналы с Splunk (Azure Event Hubs), журналами Azure Monitor и хранилищем Azure для расширенного анализа через интерфейс Azure Monitor Diagnostics.

##### <a name="ddos-mitigation-policies"></a>Политики устранения атак DDoS

На портале Azure выберите **Monitor** > **Metrics**. На панели **Метрики** выберите группу ресурсов, выберите тип ресурса **Общедоступный IP-адрес** и свой общедоступный IP-адрес Azure. Метрики DDoS видны на панели **Доступные метрики**.
...
Мониторинг приложений самостоятельно. и распознавать нормальное поведение приложения. Подготовьтесь действовать, если приложение во время атаки DDoS не будет вести себя так, как ожидалось.
...
"Защита от атак DDoS Azure" уровня "Стандартный" определяет и устраняет атаки DDoS без вмешательства пользователя. Чтобы получить уведомление о активном смягчении для защищенного публичного IP-адреса, можно [настроить оповещение](/azure/virtual-network/ddos-protection-manage-portal) по метрике **под DDoS-атакой или нет.** Вы можете создать оповещения для других метрик атак DDoS, чтобы изучить масштабирование атаки, снижающийся трафик и другие сведения.
...
- Если вам нужно разрешить список IP или IP диапазона от Azure DDoS стандарт защиты. Распространенным сценарием является разбивка IP-адреса, если трафик перенаправляется из внешнего облака WAF в Azure. 
...
Стандарт Защиты DDoS предназначен [для служб, развернутых в виртуальной сети.](/azure/virtual-network/virtual-network-for-azure-services) Для остальных служб применяется "Защита от атак DDoS" уровня "Базовый". Следующие эталонные архитектуры упорядочены по сценариям. Образцы архитектуры сгруппированы.
...
Эталонная архитектура демонстрирует запуск приложения службы приложений Azure в одном регионе. Эта архитектура показывает набор проверенных методов для веб-приложения, использующем  [службу приложений Azure](https://azure.microsoft.com/documentation/services/app-service/)и [базу данных Azure S'L.](https://azure.microsoft.com/documentation/services/sql-database/)
