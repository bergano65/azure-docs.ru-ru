...
- [Кэш Azure для Redis](/azure/azure-cache-for-redis) останется в КОРНЕВОМ центре сертификации Baltimore CyberTrust, но их промежуточные ЦС изменятся. [Щелкните здесь, чтобы узнать больше.](../../azure-cache-for-redis/cache-whats-new.md)
> [!IMPORTANT]
