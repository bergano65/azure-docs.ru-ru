...
-   [Шифрование на уровне ячейки](https://msdn.microsoft.com/library/ms179331.aspx) для шифрования определенных столбцов или даже ячеек данных с различными ключами шифрования.
...
В строке подключения приложения необходимо добавить параметры для шифрования подключения и указать не доверять сертификату сервера. (Это делается для вас, если вы копируете строку соединения из портала Azure.) В противном случае соединение не будет проверять личность сервера и будет подвержено атакам «человек в середине». Например, для драйвера ADO.NET это параметры строки подключения `Encrypt=True` и `TrustServerCertificate=False`.
...
Одним из решений может стать шифрование конфиденциальных данных в базе данных и защита ключей, используемых при шифровании, с помощью сертификата. Это решение не позволит никому использовать данные без ключей, но такой тип защиты следует планировать заранее.
...
Если база данных находится в отношении GeoDR, она защищается различными ключами на разных серверах. Если две базы данных подключены к одному серверу, используется один встроенный сертификат. Майкрософт автоматически меняет эти сертификаты каждые 90 дней. 

Для получения дополнительной информации [см.](/sql/relational-databases/security/encryption/transparent-data-encryption-tde)
...
-   **Аутентификация Azure Active Directory**: [аутентификация Azure AD](/azure/sql-database/sql-database-aad-authentication) — это механизм подключения к Базе данных SQL Azure и хранилищу данных SQL с помощью удостоверений в Azure AD. Этот механизм позволяет централизованно управлять удостоверениями пользователей базы данных.
...
[Динамическая маскировка данных](/sql/relational-databases/security/dynamic-data-masking) помогает предотвратить несанкционированный доступ к конфиденциальным данным, позволяя определить, сколько конфиденциальных данных выявить с минимальным воздействием на слой приложения. Эта функция безопасности на основе политики скрывает конфиденциальные данные в результирующем наборе запроса в заданных полях базы данных, а данные в базе данных не меняются.
...
- [Классификация & обнаружения данных](/azure/sql-database/sql-database-data-discovery-and-classification)
- [Оценка уязвимости](/azure/sql-database/sql-vulnerability-assessment)  
...
- [Защитите базу данных Azure S'L](/azure/sql-database/sql-database-security-tutorial)
