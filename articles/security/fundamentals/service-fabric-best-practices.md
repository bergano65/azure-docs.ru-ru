...
-   Настройка TLS для Service Fabric Azure.
...
-   Service Fabric управления доступом на основе ролей (Service Fabric RBAC). в этом сценарии используются отдельные удостоверения (сертификаты, Azure AD и т. д.) для каждой роли администратора и клиентского пользователя, обращающегося к кластеру. Удостоверения роли указываются при создании кластера.
...
Чтобы настроить TLS для приложения, сначала необходимо получить сертификат SSL/TLS, подписанный центром сертификации. Центр сертификации — это доверенная третья сторона, которая выдает сертификаты для обеспечения безопасности TLS. Если у вас еще нет сертификата SSL/TLS, необходимо получить его из компании, которая продает сертификаты SSL/TLS.

Сертификат должен соответствовать следующим требованиям к сертификатам SSL/TLS в Azure:
...
-   Имя субъекта сертификата должно совпадать с доменным именем, которое используется для обращения к облачной службе.
    ...
    >[!NOTE]
    >Невозможно получить сертификат SSL/TLS из центра сертификации для домена __cloudapp__**.NET** .
...
Дополнительные сведения об использовании сертификатов SSL/TLS см. в статье [Настройка TLS для приложения в Azure](../../cloud-services/cloud-services-configure-ssl-certificate-portal.md).
...
Сертификат устанавливает связь между [Azure Key Vault](../../key-vault/general/secure-your-key-vault.md), кластером Service Fabric и поставщиком ресурсов, который использует сертификаты. При создании кластера сведения о связи сертификата сохраняются в хранилище ключей.
...
Чтобы узнать больше о том, как настроить хранилище ключей, ознакомьтесь со статьей об [ Azure Key Vault](../../key-vault/general/overview.md).
...
>[!NOTE]
> Дополнительные сведения об использовании ролей в Service Fabric см. в разделе [Service Fabric управление доступом на основе ролей для клиентов Service Fabric](../../service-fabric/service-fabric-cluster-security-roles.md).
...
- [Контрольный список для обеспечения безопасности Azure Service Fabric](../../service-fabric/service-fabric-best-practices-security.md)
