---
title: Рекомендации по безопасности службы Azure Fabric
...
-   Настройте TLS для сервиса Azure Fabric.
...
Чтобы настроить TLS для приложения, сначала необходимо получить сертификат SSL/TLS, подписанный CA. CA является надежной третьей стороной, которая выдает сертификаты для целей безопасности TLS. Если у вас еще нет сертификата SSL/TLS, вам необходимо получить его от компании, которая продает сертификаты SSL/TLS.

Сертификат должен соответствовать следующим требованиям для сертификатов SSL/TLS в Azure:
...
-   Имя субъекта сертификата должно совпадать с доменным именем, которое используется для обращения к облачной службе.
    ...
    >[!NOTE]
    >Вы не можете получить сертификат SSL/TLS в ЦС для домена __cloudapp__**.net.**
...
Подробнее об использовании сертификатов SSL/TLS можно узнать в [Azure.](../../cloud-services/cloud-services-configure-ssl-certificate-portal.md)
...
После создания приложений для представления кластера отосваживайте пользователям роли, поддерживаемые Service Fabric: только для чтения и админ. Вы можете назначить эти роли с помощью портала Azure.
