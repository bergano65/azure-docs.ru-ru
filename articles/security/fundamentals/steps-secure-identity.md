...
* Используйте облачную аналитику.
* Включение самообслуживания конечных пользователей.

Убедитесь, что вы отслеживаете, какие функции и шаги были выполнены при чтении этого контрольного списка.
...
Рекомендации в этом документе сопоставляются с [оценкой безопасности удостоверений](../../active-directory/fundamentals/identity-secure-score.md), автоматизированной оценкой конфигурации безопасности удостоверений клиента Azure AD. Организации могут использовать страницу "Оценка безопасного удостоверения" на портале Azure AD для поиска пропусков в текущей конфигурации безопасности, чтобы обеспечить соблюдение текущих [рекомендаций Майкрософт по обеспечению](identity-management-best-practices.md) безопасности. Реализация каждой рекомендации на странице оценки безопасности увеличит ваш рейтинг и позволит вам отслеживать свой прогресс, а также поможет вам сравнить свою реализацию с реализациями других организаций аналогичного размера или отрасли.

![Оценка безопасности удостоверений](./media/steps-secure-identity/azure-ad-sec-steps0.png)

> [!NOTE]
> Многие описываемые здесь функции нуждаются в Azure AD Premium подписке, а некоторые — бесплатно. Дополнительные сведения см. в наших [ценах на Azure Active Directory](https://azure.microsoft.com/pricing/details/active-directory/) и [контрольном списке развертывания Azure AD](https://docs.microsoft.com/azure/active-directory/fundamentals/active-directory-deployment-checklist-p2) .
...
Злоумышленники, получившие доступ к привилегированным учетным записям, могут причинить значительный ущерб, поэтому очень важно в первую очередь защитить именно эти учетные записи. Включите и запросите [многофакторную идентификацию Azure](../../active-directory/authentication/multi-factor-authentication.md) (MFA) для всех администраторов в Организации, используя [Параметры безопасности Azure AD по умолчанию](../../active-directory/fundamentals/concept-fundamentals-security-defaults.md) или [Условный доступ](../../active-directory/conditional-access/plan-conditional-access.md). Если вы еще не реализовали многофакторную идентификацию, сделайте это сейчас! Это очень важно.
...
### <a name="make-sure-your-organization-uses-strong-authentication"></a>Убедитесь, что ваша организация использует строгую проверку подлинности.
...
Чтобы легко включить базовый уровень безопасности идентификации, можно использовать включение одним щелчком с помощью [параметров безопасности Azure AD](../../active-directory/fundamentals/concept-fundamentals-security-defaults.md). Параметры безопасности по умолчанию применяют Azure MFA для всех пользователей в клиенте и блокируют вход из устаревших протоколов на уровне клиента.
...
* В случае сбоя в локальной среде (например, при атаке от атаки через атаку) вы можете переключиться на использование [облачной аутентификации с помощью синхронизации хэшей паролей](choose-ad-authn.md). Этот метод проверки подлинности позволяет продолжить доступ к приложениям, настроенным для проверки подлинности, с Azure Active Directory, включая Office 365. В этом случае ИТ-специалистам не потребуется использовать личные учетные записи электронной почты для обмена данными до тех пор, пока не будет устранен локальный сбой.
...
Используя концепцию предполагаемого нарушения, вы должны уменьшить влияние скомпрометированных учетных данных, когда таковые появятся. Для каждого приложения в вашей среде рассмотрите применимые варианты использования: какие группы, сети, устройства и другие элементы следует авторизовать, а затем заблокируйте остальные. С помощью [условного доступа Azure AD](../../active-directory/conditional-access/overview.md)вы можете управлять доступом полномочных пользователей к приложениям и ресурсам в зависимости от определенных условий.

### <a name="restrict-user-consent-operations"></a>Ограничение операций согласия пользователя

Важно понимать различные принципы работы с [согласия приложения Azure AD](https://docs.microsoft.com/azure/active-directory/develop/application-consent-experience), [типы разрешений и согласия](https://docs.microsoft.com/azure/active-directory/develop/v2-permissions-and-consent), а также их влияние на уровень безопасности вашей организации. По умолчанию все пользователи в Azure AD могут предоставлять приложениям, которые используют платформу Microsoft Identity, доступ к данным вашей организации. Хотя предоставление пользователям разрешения на предоставление пользователям возможности легко получать полезные приложения, которые интегрируются с Microsoft 365, Azure и другими службами, это может представлять риск, если он не используется и не отслеживается осторожно.

Корпорация Майкрософт рекомендует [Отключить будущие операции предоставления согласия пользователя](https://docs.microsoft.com/azure/active-directory/manage-apps/methods-for-removing-user-access#i-want-to-disable-all-future-user-consent-operations-to-any-application) , чтобы сократить контактную зону и снизить риск. Если согласие с конечным пользователем отключено, предыдущие разрешения на согласие по-прежнему будут учитываться, но все последующие операции согласия должны выполняться администратором. Согласие администратора может запрашиваться пользователями через встроенный [Рабочий процесс запроса согласия администратора](https://docs.microsoft.com/azure/active-directory/manage-apps/configure-admin-consent-workflow) или с помощью собственных процессов поддержки. Перед отключением согласия конечного пользователя используйте наши [рекомендации](https://docs.microsoft.com/azure/active-directory/manage-apps/manage-consent-requests) для планирования изменений в Организации. Для приложений, к которым вы хотите разрешить доступ всем пользователям, рассмотрите возможность [предоставления согласия от имени всех пользователей](https://docs.microsoft.com/azure/active-directory/develop/v2-admin-consent), убедившись, что пользователи, которые еще не предоставляли по отдельности, получат доступ к приложению. Если вы не хотите, чтобы эти приложения были доступны всем пользователям во всех сценариях, используйте [назначение приложений](https://docs.microsoft.com/azure/active-directory/manage-apps/methods-for-assigning-users-and-groups) и [Условный доступ](https://docs.microsoft.com/azure/active-directory/conditional-access/overview) , чтобы ограничить доступ пользователей к приложениям.

Убедитесь, что пользователи могут запрашивать одобрение администратора для новых приложений, чтобы уменьшить трение пользователя, свести к минимальному объему поддержки и предотвратить регистрацию пользователей для приложений, использующих учетные данные не из Azure AD. После регулирования операций согласия администраторы должны регулярно выполнять аудит приложения и разрешения на их основе.
...
При развертывании процесса привилегированной учетной записи следуйте [рекомендациям по созданию по крайней мере двух учетных записей аварийной ситуации](../../active-directory/users-groups-roles/directory-admin-roles-secure.md) , чтобы убедиться, что у вас по-прежнему есть доступ к Azure AD, если вы заблокировали вас.
...
Риск для пользователя указывает на вероятность того, что удостоверение пользователя было скомпрометировано и рассчитывается на основе [обнаружений рисков пользователей](../../active-directory/identity-protection/overview.md) , связанных с удостоверением пользователя. Политика риска пользователя — это политика условного доступа, которая оценивает уровень риска для конкретного пользователя или группы. На основании низкого, среднего и высокого уровней риска можно настроить политику для блокировки доступа или требования изменить надежный пароль с помощью многофакторной идентификации. Корпорация Майкрософт рекомендует требовать изменения надежного пароля для пользователей с высоким уровнем риска.
...
Риск при входе представляет собой вероятность попытки входа с использованием удостоверения, предпринимаемой кем-то другим, но не владельцем учетной записи. [Политика риска входа](../../active-directory/identity-protection/overview.md) — это политика условного доступа, которая оценивает уровень риска для конкретного пользователя или группы. На основе уровня риска (высокий, средний, низкий) можно настроить политику для блокировки доступа или принудительного выполнения многофакторной идентификации. Многофакторную идентификацию необходимо применять при входах с уровнем риска "Средний" или выше.

![Вход с анонимных IP-адресов](./media/steps-secure-identity/azure-ad-sec-steps2.png)

## <a name="step-4---utilize-cloud-intelligence"></a>Шаг 4. Использование службы "облачная аналитика"
...
[Защита идентификации Azure Active Directory](../../active-directory/identity-protection/overview.md) — это средство уведомления, мониторинга и отчетности, которое можно использовать для обнаружения потенциальных уязвимостей, влияющих на удостоверения вашей организации. Он обнаруживает обнаружение рисков, таких как утечки учетных данных, невозможность перемещения и вход с зараженных устройств, анонимные IP-адреса, IP-адреса, связанные с подозрительными действиями, и неизвестные расположения. Включите оповещения с уведомлениями для получения сообщений от пользователей, подверженных риску, и (или) сообщение электронной почты еженедельного дайджеста.
...
Пользователи могут переходить к скомпрометированному веб-узлу или приложениям, которые будут получать доступ к сведениям о профиле и пользовательским данным, таким как электронная почта. Злоумышленник может использовать полученные разрешения для шифрования содержимого почтового ящика и потребовать выкуп за восстановление ваших данных почтового ящика. [Администраторы должны просматривать и проверять](https://docs.microsoft.com/office365/securitycompliance/detect-and-remediate-illicit-consent-grants) разрешения, предоставляемые пользователями, или отключать возможность пользователей предоставлять согласие по умолчанию.

Помимо аудита разрешений, предоставленных пользователями, можно [размещать рискованные или нежелательные приложения OAuth](https://docs.microsoft.com/cloud-app-security/investigate-risky-oauth) в средах Premium.

## <a name="step-5---enable-end-user-self-service"></a>Шаг 5. Включение самообслуживания для конечных пользователей
...
[Самостоятельный сброс пароля (SSPR)](../../active-directory/authentication/quickstart-sspr.md) в Azure AD предоставляет ИТ-администраторам простые средства для сброса или снятия блокировки паролей или учетных записей без участия пользователя службы технической поддержки или администратора. Система включает подробные отчеты, которые отслеживают, когда пользователи сбрасывают свои пароли, а также уведомления о неправильном использовании или нарушении.

### <a name="implement-self-service-group-and-application-access"></a>Реализация самостоятельного доступа к группам и приложениям

Azure AD предоставляет возможность не администраторам управлять доступом к ресурсам с помощью групп безопасности, групп Office 365, ролей приложений и доступа к каталогам пакетов.  [Самостоятельное управление группами](../../active-directory/users-groups-roles/groups-self-service-management.md) позволяет владельцам групп управлять собственными группами без необходимости назначать административную роль. Пользователи также могут создавать группы Office 365 и управлять ими, не полагаясь на администраторов для обработки своих запросов, и срок действия неиспользуемых групп истекает автоматически.  Управление назначениями [Azure AD](../../active-directory/governance/entitlement-management-overview.md) дополнительно обеспечивает делегирование и видимость с помощью комплексных рабочих процессов запросов на доступ и автоматического истечения срока действия.  Вы можете делегировать пользователям, не являющимся администраторами, возможность настраивать собственные пакеты доступа для групп, групп, приложений и сайтов SharePoint Online, которыми они владеют, с пользовательскими политиками, которые необходимы для утверждения доступа, включая настройку руководителей сотрудников и спонсоров бизнес-партнеров в качестве утверждающих лиц.

### <a name="implement-azure-ad-access-reviews"></a>Реализация проверок доступа Azure AD

С помощью [проверок доступа Azure AD](../../active-directory/governance/access-reviews-overview.md)можно управлять членством в пакетах и группах, доступом к корпоративным приложениям и назначениями привилегированных ролей, чтобы обеспечить поддержку стандарта безопасности.  Регулярное понимание самих пользователей, владельцы ресурсов и другие проверяющие гарантируют, что пользователи не будут хранить доступ в течение продолжительного периода времени, когда они больше не нужны.
...
Если вы уверены, что все эти действия готовы, используйте [оценку безопасности удостоверений](../../active-directory/fundamentals/identity-secure-score.md)Майкрософт, которая позволит вам в курсе [последних рекомендаций](identity-management-best-practices.md) и угроз безопасности.
