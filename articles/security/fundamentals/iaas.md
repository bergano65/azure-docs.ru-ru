...
> [!NOTE]
> Чтобы повысить безопасность Linux VMs на Azure, можно интегрироваться с аутентификацией Azure AD. При использовании [аутентификации Azure AD для Linux VMs](/azure/virtual-machines/linux/login-using-aad)вы централизованно контролируете и обеспечиваете соблюдение политик, которые позволяют или отказывают в доступе к им.
>
>
...
В целях обнаружения и удаления вирусов, шпионских и других вредоносных программ необходимо установить средства защиты от вредоносных программ. Вы можете установить [Microsoft Antimalware](antimalware.md) или решение защиты конечных точек партнера Майкрософт[(Trend Micro](https://help.deepsecurity.trendmicro.com/Welcome.html), [Broadcom,](https://www.broadcom.com/products) [McAfee,](https://www.mcafee.com/us/products.aspx) [Windows Defender](https://www.microsoft.com/windows/comprehensive-security)и System [Center Endpoint Protection).](/configmgr/protect/deploy-use/endpoint-protection)
...
**Лучшая практика**: Быстро применять обновления безопасности для VMs.   
**Деталь**: Включить Центр безопасности Azure (бесплатный уровень или стандартный уровень) для [выявления недостающих обновлений безопасности и их применения.](../../security-center/security-center-apply-system-updates.md)
...
[Шифрование azure Disk](../azure-security-disk-encryption-overview.md) помогает шифровать ваши windows и Linux IaaS виртуальные диски машины. Для шифрования дисков Azure используются стандартные для отрасли функции ([BitLocker](https://technet.microsoft.com/library/cc732774.aspx) в Windows и [DM-Crypt](https://en.wikipedia.org/wiki/Dm-crypt) в Linux), которые обеспечивают шифрование томов дисков ОС и дисков данных. Решение интегрировано с [Azure Key Vault,](https://azure.microsoft.com/documentation/services/key-vault/) чтобы помочь вам управлять и управлять ключами и секретами шифрования дисков в подписке на хранилище ключей. Данное решение обеспечивает шифрование всех неактивных данных на дисках виртуальной машины в службе хранилища Azure.
...
Мониторинг и ограничение прямого подключения К Интернету VM. Злоумышленники постоянно сканируют общедоступные диапазоны IP-адресов облаков для открытых портов управления и пытаются «легкие» атаки, такие как общие пароли и известные неисправленные уязвимости. В следующей таблице перечислены рекомендации по защите от этих атак:

**Лучшая практика**: Предотвращение непреднамеренного воздействия сетевой иохранительной системы безопасности.   
**Деталь**: Используйте RBAC, чтобы гарантировать, что только центральная сетевая группа имеет разрешение на сетевые ресурсы.

**Лучшая практика**: Выявление и устранение открытых ВМ, которые позволяют получить доступ с "любого" исходного IP-адреса.   
**Деталь**: Используйте Центр безопасности Azure. Центр безопасности порекомендует ограничить доступ через конечные точки, обращенные в Интернет, если какая-либо из ваших групп сетевой безопасности имеет одно или несколько входящих правил, которые позволяют получить доступ с "любого" исходного IP-адреса. Центр безопасности порекомендует вам отсеить эти входящие правила, чтобы [ограничить доступ к](../../security-center/security-center-network-recommendations.md) IP-адресам источника, которые действительно нуждаются в доступе.

**Лучшая практика**: Ограничить порты управления (RDP, SSH).   
**Деталь**: [Просто в срок (JIT) VM доступ](../../security-center/security-center-just-in-time.md) может быть использован для блокировки входящего трафика на ваш Azure VMs, снижение подверженности атакам при одновременном обеспечении легкого доступа к вс-чам. Когда JIT включен, Центр безопасности блокирует входящий трафик на ваши vMs-сообщения Azure, создавая правило группы безопасности сети. Вы выбираете порты на виртуальной машине, для которых будет заблокирован входящий трафик. Эти порты контролируются решением JIT.
