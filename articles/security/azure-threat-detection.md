...
ms.locfileid: "60611238"
---
# <a name="azure-advanced-threat-detection"></a>Расширенное обнаружение угроз Azure

Azure предлагает встроенные функции расширенного обнаружения угроз через службы, такие как Azure Active Directory (Azure AD), журналы Azure Monitor и центр безопасности Azure. С помощью этого набора служб и функций безопасности можно легко и быстро понять, что произошло в развертываниях Azure.
...
[Журналы Azure Monitor](https://docs.microsoft.com/azure/operations-management-suite/operations-management-suite-overview) — это Облачное решение Майкрософт управления ИТ, помогает управлять и защищать в локальной и облачной инфраструктурой. Так как Azure Monitor журналы реализуется как облачная служба, возможно он приступить к работе быстро с минимальными затратами на службы инфраструктуры. Новые возможности безопасности предоставляются автоматически, что сокращает затраты на текущее обслуживание и обновление.

Помимо предоставления ценных служб отдельно, Azure Monitor журналы можно интегрировать с компонентами System Center, такими как [System Center Operations Manager](https://blogs.technet.microsoft.com/cbernier/2013/10/23/monitoring-windows-azure-with-system-center-operations-manager-2012-get-me-started/), для расширения безопасности существующие инвестиции управления в облако. Журналы System Center и Azure Monitor могут работать совместно для обеспечения возможности полностью гибридной формы управления.

### <a name="holistic-security-and-compliance-posture"></a>Комплексное состояние безопасности и соответствия

[Панель мониторинга "Безопасность и аудит" в Log Analytics](https://docs.microsoft.com/azure/operations-management-suite/oms-security-getting-started) позволяет всесторонне изучать данные об информационной безопасности в вашей организации. Для этого применяются встроенные поисковые запросы, которые позволяют определять важные моменты, требующие вашего внимания. Панели мониторинга безопасности и аудита — это начальный экран все ресурсы, связанные с безопасностью в Azure Monitor журналы. Она содержит обобщенное представление состояния безопасности компьютеров организации. Она также позволяет просматривать все события за последние 24 часа, 7 дней или другие настраиваемые промежутки времени.

Azure Monitor регистрирует помогают быстро и легко понять общее состояние безопасности любой среды, все в контексте ИТ-операций, включая оценку обновлений программного обеспечения, оценки вредоносных программ и шаблонов базовой конфигурации. Вы можете изучить готовые данные журнала безопасности, чтобы оптимизировать процессы аудита безопасности и соответствия.

![Панель мониторинга "Безопасность и аудит" в Log Analytics](./media/azure-threat-detection/azure-threat-detection-fig3.jpg)
...
В центре [журналы Azure Monitor](https://docs.microsoft.com/azure/log-analytics/log-analytics-overview) занимает репозиторий, который размещается в Azure.
...
![Панель мониторинга журналов Azure Monitor](./media/azure-threat-detection/azure-threat-detection-fig5.png)
...
Большая часть взаимодействия с журналами Azure Monitor — через портал Azure, который работает в любом браузере и обеспечивает доступ к параметрам конфигурации и многочисленным средствам для анализа и использования собранных данных. На портале вы можете использовать:
...
Решения расширяют функциональные возможности журналы Azure Monitor. В первую очередь они выполняются в облаке и обеспечивают анализ данных, которые собираются в репозиторий log analytics. Решения также может определять новые типы записей собирать, которые могут анализироваться по журналам или дополнительный пользовательский интерфейс, который предоставляет решения на панели мониторинга log analytics.
...
| Управление | Снижайте риск, задав политики и оповещения, чтобы получить максимальный контроль над сетевым трафиком облака. Используйте Cloud App Security, чтобы перенести пользователей в альтернативные варианты облачных приложений, санкционированные системой безопасности. |
...
| Управление | Снижайте риск, задав политики и оповещения, чтобы получить максимальный контроль над сетевым трафиком облака. Используйте Cloud App Security, чтобы перенести пользователей в альтернативные варианты облачных приложений, разрешенные системой безопасности. |
| | |
