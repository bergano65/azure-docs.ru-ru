...
Azure предлагает встроенные функции расширенного обнаружения угроз в таких службах, как Azure Active Directory (Azure AD), Azure Log Analytics и центр безопасности Azure. С помощью этого набора служб и функций безопасности можно легко и быстро понять, что произошло в развертываниях Azure.
...
Ключевым компонентом [Log Analytics](https://docs.microsoft.com/azure/log-analytics/log-analytics-overview) является репозиторий, который размещен в Azure.
...
![Панель мониторинга Log Analytics ](./media/azure-threat-detection/azure-threat-detection-fig5.png)
...
Решения добавляют функциональные возможности в службу Log Analytics. Они выполняются преимущественно в облаке и обеспечивают анализ данных, собираемых в репозитории Log Analytics. В решениях также могут определяться новые типы записей, которые нужно собирать и можно анализировать, используя запросы поиска по журналам или дополнительный пользовательский интерфейс, предоставляемый решением на панели мониторинга Log Analytics.
