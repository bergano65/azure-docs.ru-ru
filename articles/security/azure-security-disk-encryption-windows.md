...
В этой статье приведены инструкции по включению Microsoft Azure шифрования дисков для виртуальных машин IaaS Windows. Прежде чем можно будет использовать шифрование дисков, необходимо выполнить [Предварительные требования к шифрованию дисков Azure](../security/azure-security-disk-encryption-prerequisites.md). 

Также настоятельно рекомендуется [создать моментальный снимок](../virtual-machines/windows/snapshot-copy-managed-disk.md) и (или) резервную копию дисков перед шифрованием. Резервные копии обеспечивают возможность восстановления в случае любого непредвиденного сбоя во время шифрования. Для виртуальных машин с управляемыми дисками необходимо создать резервную копию до начала шифрования. После создания резервной копии можно использовать [командлет Set-азвмдискенкриптионекстенсион](/powershell/module/az.compute/set-azvmdiskencryptionextension) для шифрования управляемых дисков, указав параметр-skipVmBackup. Дополнительные сведения о резервном копировании и восстановлении зашифрованных виртуальных машин см. в статье [резервное копирование и восстановление зашифрованной виртуальной машины Azure](../backup/backup-azure-vms-encryption.md) .

>[!WARNING]
> - Если вы уже использовали [шифрование дисков Azure с помощью приложения Azure AD](azure-security-disk-encryption-prerequisites-aad.md) для шифрования этой виртуальной машины, этот способ нужно применять и далее для шифрования виртуальной машины. Вы не можете использовать [Шифрование дисков Azure](azure-security-disk-encryption-prerequisites.md) на этой ЗАШИФРОВАННОЙ виртуальной машине, так как это не поддерживаемый сценарий, что означает, что переключение из приложения AAD для этой ЗАШИФРОВАННОЙ виртуальной машины пока не поддерживается. 
...
Шифрование можно включить с помощью шаблона, командлетов PowerShell или команд интерфейса командной строки. Сведения о схеме для расширения виртуальной машины см. в статье [Шифрование дисков Azure для Windows ](../virtual-machines/extensions/azure-disk-enc-windows.md).
...
- **Отключение шифрования:** чтобы отключить шифрование, используйте команду [az vm encryption disable](/cli/azure/vm/encryption#az-vm-encryption-disable). Отключение шифрования диска данных на виртуальной машине Windows, если шифрование дисков операционной системы и данных не работает должным образом. В этом случае отключите шифрование на всех дисках.
  ...
  > [!NOTE]
  >Нужно обязательно создать образ или резервную копию экземпляра виртуальной машины на основе управляемого диска вне шифрования дисков Azure и перед включением этой функции. Моментальный снимок управляемого диска можно создать с портала или использовать для этого [Azure Backup](../backup/backup-azure-vms-encryption.md). Резервные копии обеспечивают возможность восстановления при любом непредвиденном сбое во время шифрования. После создания резервной копии командлет Set-Азвмдискенкриптионекстенсион можно использовать для шифрования управляемых дисков, указав параметр-skipVmBackup. Эта команда будет завершаться ошибкой при использовании с виртуальными машинами на основе управляемых дисков, пока не будет сделана резервная копия и указан определенный параметр. 
  >
  >Шифрование или отключение шифрования может привести к перезагрузке виртуальной машины. 
...
Используйте командлет [Set-азвмссдискенкриптионекстенсион](/powershell/module/az.compute/set-azvmssdiskencryptionextension) , чтобы включить шифрование в масштабируемом наборе виртуальных машин Windows. Группа ресурсов, масштабируемый набор виртуальных машин и хранилище ключей уже должны быть созданы в качестве необходимых компонентов.
...
- **Получение состояния шифрования для масштабируемого набора виртуальных машин:** Используйте командлет [Get-азвмссдискенкриптион](/powershell/module/az.compute/get-azvmssdiskencryption) .
