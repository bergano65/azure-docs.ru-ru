...
В этой статье инструкции по включению Microsoft дискового шифрования для Windows виртуальных машин IaaS Azure (ВМ). Перед использованием шифрования дисков, необходимо сначала выполнить [предварительных требованиях шифрования дисков Azure](../security/azure-security-disk-encryption-prerequisites.md). 

Также настоятельно рекомендуется, [создать моментальный снимок](../virtual-machines/windows/snapshot-copy-managed-disk.md) и (или) резервную копию дисков перед шифрованием. Резервные копии обеспечивают возможность восстановления в случае любого непредвиденного сбоя во время шифрования. Для виртуальных машин с управляемыми дисками необходимо создать резервную копию до начала шифрования. Как только будет установлено резервной копии, можно использовать [командлет Set-AzVMDiskEncryptionExtension](/powershell/module/az.compute/set-azvmdiskencryptionextension) зашифровать управляемые диски, указав параметр - skipVmBackup. Дополнительные сведения о том, как резервное копирование и восстановление зашифрованных виртуальных машин см. в разделе [резервного копирования и восстановления зашифрованных виртуальных Машин Azure](../backup/backup-azure-vms-encryption.md) статьи.

>[!WARNING]
> - Если вы уже использовали [шифрование дисков Azure с помощью приложения Azure AD](azure-security-disk-encryption-prerequisites-aad.md) для шифрования этой виртуальной машины, этот способ нужно применять и далее для шифрования виртуальной машины. Нельзя использовать [шифрования дисков Azure](azure-security-disk-encryption-prerequisites.md) на эту зашифрованную виртуальную Машину так, как это не является поддерживаемым сценарием, значение, переключаясь из приложения AAD для это зашифрованных виртуальных Машин сейчас не поддерживается. 
...
Вы можете включить шифрование с помощью шаблона, командлетов PowerShell или команд интерфейса командной строки. Сведения о схеме для расширения виртуальной машины см. в статье [Шифрование дисков Azure для Windows ](../virtual-machines/extensions/azure-disk-enc-windows.md).
...
- **Отключение шифрования дисков:** Чтобы отключить шифрование, используйте [Disable AzVMDiskEncryption](/powershell/module/az.compute/disable-azvmdiskencryption) командлета. Отключение шифрования диска данных на виртуальной Машине Windows в том случае, когда для шифрования дисков операционной системы и данных не работает должным образом. В этом случае отключите шифрование на всех дисках.
...
- **Отключение шифрования:** чтобы отключить шифрование, используйте команду [az vm encryption disable](/cli/azure/vm/encryption#az-vm-encryption-disable). Отключение шифрования диска данных на виртуальной Машине Windows в том случае, когда для шифрования дисков операционной системы и данных не работает должным образом. В этом случае отключите шифрование на всех дисках.
...
Вы можете отключить шифрование с помощью Azure PowerShell, Azure CLI или шаблона Resource Manager. Отключение шифрования диска данных на виртуальной Машине Windows в том случае, когда для шифрования дисков операционной системы и данных не работает должным образом. В этом случае отключите шифрование на всех дисках.
