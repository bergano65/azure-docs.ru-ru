...
* [Сеть сеть VPN](../vpn-gateway/vpn-gateway-howto-multi-site-to-site-resource-manager-portal.md). Это доверенная, надежная и установленная технология, но подключение выполняется через Интернет. Пропускная способность ограничена до 200 Мбит/с. Сеть сеть VPN является наиболее предпочтительным режимом, в некоторых сценариях.
