---
title: Технические возможности обеспечения безопасности в Azure, Microsoft Azure
description: Общие сведения о безопасности служб в Azure, которые помогут вам защитить данные, ресурсы и приложения в облаке.
services: security
...
ms.locfileid: "67127192"
---
# <a name="azure-security-technical-capabilities"></a>Технические возможности обеспечения безопасности Azure
В этой статье содержатся общие сведения о безопасности служб в Azure, которые помогут вам защитить данные, ресурсы и приложения в облаке и соответствии с потребностями безопасности вашей компании.
...
## <a name="security-technical-capabilities-to-fulfil-your-responsibility"></a>Технические возможности обеспечения безопасности, необходимое для выполнения вы отвечаете за

Microsoft Azure предоставляет службы, которые помогут вам требованиям безопасности, конфиденциальности и соответствия требованиям. Следующий рисунок поможет разобраться в различных служб Azure вы можете создать инфраструктуру безопасность и совместимость приложений на основе отраслевых стандартов.
...
## <a name="manage-and-control-identity-and-user-access"></a>Управление и управления удостоверениями и доступом пользователей
...
Идентификация и доступ управления решения Майкрософт помогают ИТ защиты доступа к приложениям и ресурсам в корпоративных центрах обработки данных и в облаке, предоставляют такие дополнительные уровни проверки, как многофакторная проверка подлинности и условного доступа политики. Наблюдение за подозрительной активностью с использованием расширенных отчетов, аудита и предупреждений помогает смягчить потенциальные риски безопасности. [Azure Active Directory Premium](https://docs.microsoft.com/azure/active-directory/active-directory-editions) предоставляет единый вход для тысяч облачных приложений и доступ к веб-приложений на предприятии.
...
[Портал Azure Active Directory](https://aad.portal.azure.com/) доступен как часть портала Azure. Из этой панели мониторинга можно получить общие сведения о состоянии вашей организации и легко управлять каталога, пользователей или доступ к приложениям.

![Azure Active Directory](media/azure-security-technical-capabilities/azure-security-technical-capabilities-fig2.png)

Ниже приведены основные возможности управления удостоверениями в Azure.
...
Во многих организациях зависят от программного обеспечения как услуги (SaaS) приложения, такие как Office 365, Box и Salesforce для повышения производительности работы конечных пользователей. Обычно ИТ-специалистам приходилось отдельно создавать и обновлять учетные записи пользователей в каждом приложении SaaS, а пользователям нужно было запоминать новые пароли для каждого такого приложения.
...
- Отчеты об интегрированных приложениях — получить представление о том, как облачные приложения используются в вашей организации. Azure Active Directory обеспечивает интеграцию с множеством облачных приложений.

- Отчеты об ошибках: указывают на ошибки, которые возникают при подготовке учетных записей для внешних приложений.

- Отчеты о пользователях — отображает устройство и войти в данные о действиях для конкретного пользователя.
...
[Регистрация устройств в Azure AD](https://docs.microsoft.com/azure/active-directory/device-management-introduction) лежит в основе устройств [условного доступа](https://docs.microsoft.com/azure/active-directory/active-directory-device-registration-on-premises-setup) сценариев. В ходе регистрации служба регистрации устройств Azure Active Directory назначает устройству идентификатор, который используется для аутентификации устройства при входе пользователя. Аутентифицированное устройство и атрибуты устройства, затем можно применять политики условного доступа для приложений, размещенных в облаке и локальной.
...
## <a name="secure-resource-access"></a>Защита доступа к ресурсам
...
- Во время передачи: Данные, передаваемые между компонентами, расположениями или программами, например по сети, по служебной шине (из локальной сети в облако или наоборот, в том числе с использованием гибридных подключений, например ExpressRoute), а также в процессе ввода-вывода, то есть при любом перемещении.
...
| Шифрование на стороне сервера с помощью управляемых службой ключей | Шифрование на стороне сервера с помощью управляемых клиентом ключей в Azure Key Vault | Шифрование на стороне сервера, с помощью в локальной среде клиента управляемых ключей |
| • Поставщики ресурсов Azure выполняют операции шифрования и расшифровки <br> • Ключами управляет Майкрософт <br>• Все возможности облака | • Поставщики ресурсов Azure выполняют операции шифрования и расшифровки<br>• Клиент управляет ключами через Azure Key Vault<br>• Все возможности облака | • Поставщики ресурсов Azure выполняют операции шифрования и расшифровки <br>• Пользователь управляет ключами в локальной <br> • Все возможности облака| • Расшифрованные данные недоступны службам Azure <br>• Клиенты хранят ключи в локальной среде или других безопасных хранилищах. Ключи недоступны для служб Azure <br>• Возможности облака ограничены|
...
Цель шифрования неактивных данных — для шифрования всех данных. При этом исключается возможность потери важных данных или всех сохраненных расположений. Укажите все данные, хранящиеся в приложении.
...
Для каждого магазина, которые можно использовать используйте существующий поддержку шифрования неактивных.
...
## <a name="secure-your-application"></a>Обеспечить безопасность приложения
...
## <a name="azure-infrastructure-and-application-security-validation"></a>Azure инфраструктуры и приложений проверки безопасности
...
[Azure Monitor](https://docs.microsoft.com/azure/operations-management-suite/operations-management-suite-overview) — решение для ИТ-управления для гибридного облака. Использовать отдельно или для расширения в существующее развертывание System Center, журналы Azure Monitor предоставляет максимальную гибкость и возможность управления для управления облачной инфраструктуры.

![Azure Monitor](media/azure-security-technical-capabilities/azure-security-technical-capabilities-fig8.png)

С помощью Azure Monitor можно управлять любым экземпляром в любом облаке, включая локальных, Azure, AWS, Windows Server, Linux, VMware и OpenStack по более низкой цене, чем у конкурентных решений. Создана для мире господства облачных технологий, Azure Monitor предлагает новый подход к управлению предприятием, это самый быстрый и выгодный способ решения новых бизнес-задач и обработки новых рабочих нагрузок, приложений и облачных сред.
...
![Журналы Azure Monitor](media/azure-security-technical-capabilities/azure-security-technical-capabilities-fig9.png)
