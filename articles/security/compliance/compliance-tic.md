...
#### <a name="option-b-use-virtual-network-service-endpoints-service-tunnel"></a>Вариант 2. Использование конечных точек службы виртуальной сети (туннелирование службы)
...
[Политика Azure](../../governance/policy/overview.md) — это служба Azure, которая предоставляет организации расширенные возможности аудита и реализации инициатив по соответствию требованиям. Но клиенты могут уже сейчас разрабатывать и проверять правила службы "Политика Azure", которые в будущем позволят им обеспечить соответствие требованиям TIC.
...
|---------|---------|---------|
|Применение определяемой пользователем таблицы маршрутов. | Маршрут по умолчанию во всех точках виртуальных сетей ведет к надежному шлюзу виртуальной сети для передачи трафика в локальную сеть.    | Используйте [этот шаблон](../../governance/policy/samples/no-user-defined-route-table.md). |
|Проверка запрета службы "Наблюдатель за сетями" для региона.  | Обязательно включите службу "Наблюдатель за сетями" для всех используемых регионов.  | Используйте [этот шаблон](../../governance/policy/samples/network-watcher-not-enabled.md). |
|Конкретная группа безопасности сети для каждой подсети.  | Примените ко всем подсетям в каждой виртуальной сети группу безопасности сети (или набор утвержденных групп безопасности сети) для блокировки интернет-трафика. | Используйте [этот шаблон](../../governance/policy/samples/nsg-on-subnet.md). |
|Конкретная группа безопасности сети для каждого сетевого адаптера. | Примените ко всем сетевым адаптерам на каждой виртуальной машине группу безопасности сети для блокировки интернет-трафика. | Используйте [этот шаблон](../../governance/policy/samples/nsg-on-nic.md). |
|Используйте утвержденную виртуальную сеть для сетевых интерфейсов виртуальных машин.  | Разместите все сетевые адаптеры в утвержденной виртуальной сети. | Используйте [этот шаблон](../../governance/policy/samples/use-approved-vnet-vm-nics.md). |
|Допустимые расположения. | Развертывайте все ресурсы в регионах, где конфигурация виртуальных сетей и службы "Наблюдатель за сетями" соответствует требованиям.  | Используйте [этот шаблон](../../governance/policy/samples/allowed-locations.md). |
|Недопустимые типы ресурсов, например **общедоступные IP-адреса**. | Запретите развертывание типов ресурсов, для которых нет плана обеспечения соответствия. К примеру, с помощью этой политики можно запретить развертывание ресурсов общедоступного IP-адреса. Даже если входящий сетевой трафик эффективно заблокирован правилами для групп безопасности сети, запрет на использование общедоступных IP-адресов дополнительно уменьшает область направления атак.   | Используйте [этот шаблон](../../governance/policy/samples/not-allowed-resource-types.md).  |

### <a name="network-watcher-traffic-analytics"></a>Аналитика трафика в службе "Наблюдатель за сетями"

Решение [аналитики трафика](https://azure.microsoft.com/blog/traffic-analytics-in-preview/) в службе "Наблюдатель за сетями" получает данные из журнала потоков и других журналов и формирует обобщенную аналитику по сетевому трафику. Эти данные будут полезны для аудита соответствия TIC и выявления проблемных мест. С помощью панелей мониторинга высокого уровня вы сможете быстро определить, какие виртуальные машины взаимодействуют с Интернетом, и получить конкретный список рекомендаций для маршрутизации TIC.
