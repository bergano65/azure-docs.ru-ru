...
> [!NOTE]
> Для повышения безопасности виртуальных машин Linux в Azure, можно интегрировать с аутентификацией Azure AD. При использовании [проверки подлинности Azure AD для виртуальных машин Linux](../virtual-machines/linux/login-using-aad.md), централизованного управления и применять политики, которые разрешают или запрещают доступ к виртуальным машинам.
>
>
...
**Рекомендация**. Быстро применить обновления для системы безопасности для виртуальных машин.   
**Сведения**. Включите центр безопасности Azure (уровень "бесплатный" или "стандартный") для [выявления отсутствующих обновлений безопасности и применить их](../security-center/security-center-apply-system-updates.md).
...
Центр безопасности сохраняет данные в [журналы Azure Monitor](../log-analytics/log-analytics-overview.md). Журналы Azure Monitor предоставляет подсистему запросов языка и аналитики, которая позволяет получить сведения о работе приложений и ресурсов. Данные также собираются из [Azure Monitor](../monitoring-and-diagnostics/monitoring-overview.md), решений по управлению и агентов, установленных на виртуальных машинах в локальной среде или в облаке. Такие общие функции позволяют составить полную картину всей среды.
...
## <a name="restrict-direct-internet-connectivity"></a>Ограничить прямое подключение к Интернету
Отслеживать и ограничивать прямое подключение к Интернету на виртуальной Машине. Злоумышленники постоянно проверять диапазоны IP-адрес общедоступного облака для открытые порты управления и попытайтесь «простой» атак, таких как распространенных простых паролей и известных Неисправленные уязвимости. В следующей таблице перечислены рекомендуемые способы защиты от таких атак:

**Рекомендация**. Предотвращения случайного прямого доступа к сети, маршрутизацию и безопасность.   
**Сведения**. Убедитесь, что только центральной сети группа имеет разрешение на сетевые ресурсы с помощью RBAC.

**Рекомендация**. Выявлять и устранять предоставляемых виртуальных машин, которые разрешают доступ из «any» исходный IP-адрес.   
**Сведения**. В центре безопасности Azure. Центр безопасности будет рекомендовать ограничить доступ через конечные точки с выходом в Интернет, если какой-либо групп безопасности сети имеет одно или несколько входящих правил, разрешающих доступ с «any» исходный IP-адрес. Центр безопасности будет рекомендовать изменить эти правила входящего трафика для [ограничить доступ](../security-center/security-center-restrict-access-through-internet-facing-endpoints.md) для исходных IP-адресов, которые фактически требуется доступ.

**Рекомендация**. Ограничьте порты управления (RDP, SSH).   
**Сведения**. [Доступ к виртуальной Машине just-in-time (JIT)](../security-center/security-center-just-in-time.md) можно использовать для блокировки входящего трафика к виртуальным машинам Azure, снизить уязвимость к атакам и обеспечить удобный доступ к виртуальным машинам, когда требуется. Если JIT включен, Центр безопасности блокирует входящий трафик к виртуальным машинам Azure, создав правило группы безопасности сети. Вы выбираете порты на виртуальной машине, для которых будет заблокирован входящий трафик. Эти порты управляет решение JIT.
