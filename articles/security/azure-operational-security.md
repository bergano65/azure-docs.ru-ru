...
description: Дополнительные сведения о журналы Microsoft Azure Monitor, его службы и как это работает.
services: security
...
Журналы Microsoft Azure Monitor — это решение управления ИТ для гибридного облака. Использовать отдельно или для расширения в существующее развертывание System Center, журналы Azure Monitor предоставляет максимальную гибкость и возможность управления для управления облачной инфраструктуры.

![Журналы Azure Monitor](./media/azure-operational-security/azure-operational-security-fig1.png)

С помощью журналов Azure Monitor можно управлять любым экземпляром в любом облаке, включая локальных, Azure, AWS, Windows Server, Linux, VMware и OpenStack по более низкой цене, чем у конкурентных решений. Создана для мире господства облачных технологий, журналы Azure Monitor предлагает новый подход к управлению предприятием, это самый быстрый и выгодный способ решения новых бизнес-задач и обработки новых рабочих нагрузок, приложений и облачных сред.

### <a name="azure-monitor-services"></a>Службы Azure Monitor

Основные функциональные возможности Azure Monitor журналов предоставляется с помощью набора служб, работающих в Azure. Каждая служба предоставляет определенную функцию управления, благодаря чему можно комбинировать службы для получения различных сценариев управления.
...
[Журналы Azure Monitor](https://azure.microsoft.com/documentation/services/log-analytics) предоставляет службы мониторинга, собирая данные из управляемых ресурсов в центральном репозитории. Эти данные могут содержать события, данные о производительности или пользовательские данные, полученные с помощью API. Собранные данные доступны для оповещения, анализа и экспорта.
...
![Журналы Azure Monitor](./media/azure-operational-security/azure-operational-security-fig2.png)

Служба Azure Monitor безопасно управляет данными на основе облака с помощью следующих методов:
-   разделение данных;
-   хранение данных;
...
[Служба архивации Azure](https://azure.microsoft.com/documentation/services/backup) предоставляет данные резервного копирования и восстановления служб и является частью Azure Monitor набор продуктов и служб.
...
[Azure Monitor](https://docs.microsoft.com/azure/operations-management-suite/oms-security-getting-started) является корпорации Майкрософт Облачное решение для управления, помогает управлять и защищать в локальной и облачной инфраструктурой.


[Решения по управлению](https://docs.microsoft.com/azure/operations-management-suite/operations-management-suite-solutions) представляют собой предварительно упакованные наборы логики, реализующие определенный сценарий управления с помощью одной или нескольких служб Azure Monitor. Доступны различные решения корпорации Майкрософт и партнеров, что можно легко добавить к своей подписке Azure, чтобы увеличить значение вложений в Azure Monitor. Как партнер вы можете создавать собственные решения для поддержки приложений и служб и предоставлять их пользователям через Azure Marketplace или c помощью шаблонов быстрого запуска.


![Решения для управления](./media/azure-operational-security/azure-operational-security-fig4.png)

Хорошим примером решения, которое использует несколько служб для обеспечения дополнительных функциональных возможностей, является [решение для управления обновлениями](https://docs.microsoft.com/azure/operations-management-suite/oms-solution-update-management). Это решение использует [журналы Azure Monitor](https://docs.microsoft.com/azure/log-analytics/log-analytics-overview) агент для Windows и Linux собирать сведения о необходимых обновлениях на каждом агенте. Он записывает эти данные в хранилище журналов Azure Monitor, где их можно анализировать с помощью встроенной панели мониторинга.
...
-   **Разделение данных.** Данные логическим путем отделяются для каждого компонента службы. Все данные отмечаются тегами по организациям. Эти теги существуют в течение всего жизненного цикла данных и используются на каждом уровне службы.
...
-   **Ваши учетные записи хранения.** Учетная запись хранения указывается для каждого региона, в котором работают виртуальные машины. Таким образом, данные можно хранить в том же регионе, где расположена виртуальная машина, из которой выполняется сбор данных.
...
[Монитор Azure журналы безопасности](https://docs.microsoft.com/azure/operations-management-suite/oms-security-monitoring-resources) и аудита решение позволяет ИТ-администраторам активно отслеживать все ресурсы, которые помогут свести к минимуму влияние нарушений безопасности. Azure Monitor журналы безопасности и аудита имеют домены безопасности, которые могут использоваться для наблюдения за ресурсами. Домен безопасности предоставляет быстрый доступ к параметрам. Мы более подробно рассмотрим следующие домены, которые имеют отношение к мониторингу безопасности:
...
-   Повышение безопасности приложений с Azure AD, многофакторная проверка подлинности и условного доступа.
...
Отчеты содержат такие сведения: имя события, выполнивший действие субъект, подвергшийся изменению целевой ресурс, дата и время (в формате UTC). Список событий аудита для службы Azure Active Directory можно получить на [портале Azure](https://portal.azure.com/). Дополнительные сведения см. в статье [Что такое отчеты в Azure Active Directory](https://docs.microsoft.com/azure/active-directory/active-directory-reporting-azure-portal). Ниже приведен список включенных отчетов.
...
-   Как данные собираются, обрабатываются и закрепления в наборе Azure Monitor.
...
- [Журналы Azure Monitor | Безопасность и соответствие требованиям](https://www.microsoft.com/cloud-platform/security-and-compliance)
