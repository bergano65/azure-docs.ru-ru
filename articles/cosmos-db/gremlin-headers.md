...
Ниже перечислены наиболее распространенные коды, возвращаемые `x-ms-status-code` сервером для атрибута Status.
...
| **1009** | Операция не завершилась за отведенное время и была отменена сервером. Оптимизируйте обходы для быстрого выполнения, отфильтровывая вершины или края при каждом прыжке к области поиска с узким диапазоном. Время ожидания запроса по умолчанию составляет **60 секунд**. |
