...
Все журналы ресурсов в Azure Monitor имеют те же поля, за которыми следуют поля, зависящие от службы. Общая схема описана в [Azure Monitor схеме журнала ресурсов](../azure-monitor/platform/diagnostic-logs-schema.md#top-level-resource-logs-schema). Список типов журналов ресурсов, собранных для Azure Cosmos DB, см. в разделе [мониторинг Azure Cosmos DB Справочник по данным] ((Monitor-Cosmos-DB-справочник. md # Resource-Logs).  
