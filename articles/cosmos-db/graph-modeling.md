...
| Microsoft Edge | ID | Строка | Уникально применяется на секцию. Автоматически создается по умолчанию. Как правило, нет необходимости в уникальном получении ребер по идентификатору. |
| Microsoft Edge | метка | Строка | Это свойство используется для определения типа связи двух вершин. |
| Microsoft Edge | properties | Строка, логическое значение, числовое значение | Список отдельных свойств, которые хранятся в виде пар "ключ — значение", в каждом ребре. |
