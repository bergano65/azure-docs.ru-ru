...
| --- | --- | --- |  --- |
| Вершина | ID | Строковый тип | Уникально применяется на секцию. Если значение не предоставлено при вставке, будет храниться автоматически созданный идентификатор GUID. |
| Вершина | метка | Строковый тип | Это свойство используется для определения типа сущности, которую представляет вершина. Если значение не указано, будет использоваться значение по умолчанию (vertex). |
...
| Пограничный случай | ID | Строковый тип | Уникально применяется на секцию. Автоматически создается по умолчанию. Как правило, нет необходимости в уникальном получении ребер по идентификатору. |
| Пограничный случай | метка | Строковый тип | Это свойство используется для определения типа связи двух вершин. |
| Пограничный случай | properties | Строка, логическое значение, числовое значение | Список отдельных свойств, которые хранятся в виде пар "ключ — значение", в каждом ребре. |
...
:::image type="content" source="./media/graph-modeling/graph-modeling-2.png" alt-text="Модель сущности с вершинами для свойств." border="false":::
...
:::image type="content" source="./media/graph-modeling/graph-modeling-3.png" alt-text="Модель сущности с вершинами для свойств." border="false":::
