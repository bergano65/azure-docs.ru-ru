...
* **Свойства на основе Vertex**: В этом подходе сущность использует три отдельных вершины и два края для описания своих свойств. Хотя при таком подходе может уменьшиться избыточность, он увеличивает сложность модели. Это может привести к дополнительной задержке, увеличению сложности запросов и повышению вычислительных затрат. Эта модель также может вызвать трудности при секционировании.

![Модель сущности с вершинами для свойств.](./media/graph-modeling/graph-modeling-1.png)

* **Встроенные свойства :** Этот подход использует список пар ключей, чтобы представить все свойства сущности внутри вершины. Такой подход уменьшает сложности модели, что приведет к упрощению запросов и более экономически выгодным обходам.
