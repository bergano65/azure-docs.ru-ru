...
2. Вы выполняете переход с однотенантной базы данных, предназначенной для использования на виртуальных машинах, размещенных в IaaS или в локальной среде (например, NoSQL или реляционные базы данных), на базу данных Azure Cosmos DB. И если имеется много коллекций и таблиц и диаграмм и не требуется вносить изменения в модель данных. Обратите внимание на то, возможно, злоумышленнику часть преимуществ, предлагаемых Azure Cosmos DB, если вы не обновляете модель данных при миграции из локальной базы данных. Рекомендуется всегда выполнять повторную оценку модели данных для получения максимальной производительности, а также для оптимизации затрат. 
...
**Рассмотрим подготовку пропускной способности на отдельный контейнер в том случае, если:**
...
**Примите во внимание гибрид выше две стратегии.**
...
3. Другой способ получения операций и их стоимость в RUs — путем включения журналов Azure Monitor, в которых даст вам разбивку/длительность операции и единиц запроса. В Azure Cosmos DB плата за запросы начисляется для каждой операции, поэтому сведения о плате за каждую операцию можно сохранить из ответа и затем использовать для анализа. 
