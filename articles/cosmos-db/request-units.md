...
Чтение элемента размером 1 КБ соответствует 1 единице запроса (1 ЕЗ). Для хранения каждого 1 ГБ данных требуется не менее 10 единиц запросов в секунду. Цены на остальные операции с базой данных выражаются в ЕЗ аналогичным образом. Независимо от того, какие API вы используете для взаимодействия с контейнером Azure Cosmos, затраты всегда измеряются в ЕЗ. Независимо от типа операции базы данных (запись, чтение или запрос) затраты всегда измеряются в ЕЗ.
