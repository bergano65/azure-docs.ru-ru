...
ms.locfileid: "48248760"
---
# <a name="throughput-and-request-units-in-azure-cosmos-db"></a>Пропускная способность и единицы запросов в Azure Cosmos DB

Ресурсы Azure Cosmos DB оплачиваются, исходя из подготовленной пропускной способности и хранения. Пропускная способность Azure Cosmos DB выражается в **единицах запросов в секунду (ЕЗ/с)**. Azure Cosmos DB поддерживает разные интерфейсы API с разным набором операций — от простых операций чтения и записи до сложных запросов графа. Каждый запрос потребляет единицы запросов на основе объема вычислений, необходимых для обслуживания запроса. Количество единиц запроса для конкретной операции является детерминированным. Количество единиц запросов, потребленных любой операцией в Azure Cosmos DB, можно определить по заголовку ответа. Чтобы обеспечить прогнозируемую производительность, пропускную способность необходимо резервировать блоками по 100 единиц запросов в секунду. Вы можете оценить требуемую вам пропускную способность с помощью [калькулятора единиц запросов](https://www.documentdb.com/capacityplanner) Azure Cosmos DB.

В Azure Cosmos DB можно обеспечить пропускную способность с двумя деталями. 

1. **Контейнер Azure Cosmos DB.** Пропускная способность, предоставляемая для контейнера, зарезервирована только для этого конкретного контейнера. При назначении пропускной способности (ЕЗ/с) на уровне контейнера могут создаваться **фиксированные** или **неограниченные** контейнеры. 

  Контейнеры фиксированного размера имеют максимальный предел пропускной способности 10 000 ЕЗ/с и предел хранения 10 ГБ. Чтобы создать контейнер неограниченного размера, необходимо указать минимальную пропускную способность в 1000 ЕЗ в секунду и [ключ раздела](partition-data.md). Так как ваши данные могут быть разделены между несколькими разделами, выбирайте ключ раздела с большим количеством элементов (от 100 до миллионов уникальных значений). Выбрав ключ раздела с большим количеством уникальных значений, Azure Cosmos DB гарантирует равномерное масштабирование запросов к коллекции, таблице и графу. 

2. **База данных Azure Cosmos DB.** Пропускная способность, предоставляемая для базы данных, используется всеми контейнерами в базе данных. При обеспечении пропускной способности на уровне базы данных вы можете явно исключить определенные контейнеры и подготовить пропускную способность для этих контейнеров на уровне контейнера. Пропускная способность уровня базы данных требует создания всех коллекций с помощью ключа раздела. При назначении пропускной способности на уровне базы данных контейнеры, принадлежащие этой базе данных, должны быть созданы с помощью ключа раздела, потому что каждая коллекция является **неограниченным** контейнером.  

На основе подготовленной пропускной способности Azure Cosmos DB будет выделять физические разделы для размещения контейнеров и разделять данные между секциями по мере увеличения их объема. На следующем рисунке показано, как выполняется подготовка пропускной способности на разных уровнях:
