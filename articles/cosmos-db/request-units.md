...
Стоимость чтения элемента по 1 КБ составляет 1 единица запроса (или 1 RU). Для хранения каждого 1 ГБ данных требуется не менее 10 единиц запросов в секунду. Всем остальным операциям с базой данных также назначаются затраты с помощью RUs. Независимо от того, какие API вы используете для взаимодействия с контейнером Azure Cosmos, затраты всегда измеряются в ЕЗ. Независимо от типа операции базы данных (запись, чтение или запрос) затраты всегда измеряются в ЕЗ.
