...
* Ссылка Azure синапсе для Azure Cosmos DB поддерживается API SQL и Azure Cosmos DB API для MongoDB. Он не поддерживается для API Gremlin, API Cassandra и API таблиц. 
...
* Сетевая изоляция для аналитического хранилища Azure Космсо DB с использованием управляемых частных конечных точек в Azure синапсе Analytics сейчас не поддерживается.
