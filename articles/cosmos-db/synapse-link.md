...
* Ссылка Azure синапсе поддерживается для API Azure Cosmos DB SQL (Core) и API Azure Cosmos DB для MongoDB. Поддержка API Cassandra в настоящее время находится в предварительной версии. Чтобы запросить доступ к селективной предварительной версии, отправьте сообщение электронной почты [команде Azure Cosmos DB](mailto:cosmosdbsynapselink@microsoft.com).
