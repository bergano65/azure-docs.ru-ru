...
![Схема архитектуры для интеграции Azure Synapse Analytics с Azure Cosmos DB](./media/synapse-link/synapse-analytics-cosmos-db-architecture.png)
...
* Инженер бизнес-аналитики, начинающий моделирование и публикацию отчета о доступе к операционным данным в Azure Cosmos DB непосредственно через Synapse SQL.
...
* В общедоступном предварительном выпуске Azure Synapse Link поддерживается только для API Azure Cosmos DB SQL (Core). Поддержка API Azure Cosmos DB для MongoDB и API Cassandra в настоящее время находится в предварительной версии. Чтобы запросить доступ к селективной предварительной версии, отправьте сообщение электронной почты [команде Azure Cosmos DB](mailto:cosmosdbsynapselink@microsoft.com).

* В настоящее время аналитическое хранилище можно включить только для новых контейнеров (как в новых, так и в имеющихся учетных записях Azure Cosmos DB).
...
Модель выставления счетов по Azure Synapse Link преобразует затраты в затраты, связанные с использованием аналитического хранилища Azure Cosmos DB и среды выполнения Synapse. Дополнительные сведения см. в статьях [Цены на аналитическое хранилище Azure Cosmos DB](analytical-store-introduction.md#analytical-store-pricing) и [Цены на Azure Synapse Analytics]().
...
* [Что поддерживается в среде выполнения Azure Synapse Analytics]()
