...
* На данный момент Azure Synapse Link для Azure Cosmos DB поддерживается для API SQL и API Azure Cosmos DB для MongoDB. Эта возможность не поддерживается для API Gremlin и API таблиц. Поддержка API Cassandra находится в закрытой предварительной версии. Дополнительные сведения можно получить у [команды Azure синапсе Link](mailto:cosmosdbsynapselink@microsoft.com).  
