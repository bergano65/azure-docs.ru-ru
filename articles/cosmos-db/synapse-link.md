...
* Ссылка на Azure синапсе для Azure Cosmos DB поддерживается для API SQL и Azure Cosmos DB API для MongoDB. Он не поддерживается для API Gremlin и API таблиц. Поддержка API Cassandra находится в закрытой предварительной версии. Дополнительные сведения можно получить у [команды Azure синапсе Link](mailto:cosmosdbsynapselink@microsoft.com).  
...
* Доступ к аналитическому хранилищу Azure Cosmos DB с бессерверным Synapse SQL в настоящее время находится на этапе предварительной версии. Чтобы запросить доступ, отправьте сообщение по электронной почте [команде Azure синапсе Link](mailto:cosmosdbsynapselink@microsoft.com).

* Для контейнеров с включенным аналитическим хранилищем в настоящее время автоматическое резервное копирование и восстановление данных в аналитическом хранилище не поддерживается. Если в учетной записи базы данных включена ссылка синапсе, Azure Cosmos DB продолжит автоматически [выполнять резервное копирование](https://docs.microsoft.com/azure/cosmos-db/online-backup-and-restore) данных в хранилище транзакций (только) контейнеров с запланированным интервалом резервного копирования, как всегда. Важно отметить, что при восстановлении контейнера с включенным аналитическим хранилищем в новую учетную запись контейнер будет восстановлен только с хранилищем транзакций и без включенного аналитического хранилища. 
