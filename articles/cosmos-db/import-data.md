---
title: Учебник. средство миграции баз данных для Azure Cosmos DB
description: Учебник. Узнайте, как использовать средства переноса данных с открытым кодом Azure Cosmos DB для импорта данных в Azure Cosmos DB из различных источников, включая MongoDB, SQL Server, хранилище таблиц, Amazon DynamoDB, CSV и JSON-файлы. Преобразование CSV в JSON.
author: deborahc
...
ms.locfileid: "78387944"
---
# <a name="tutorial-use-data-migration-tool-to-migrate-your-data-to-azure-cosmos-db"></a>Учебник. Перенос данных в Azure Cosmos DB с помощью средства переноса данных
...
*{"DomainInfo": {"Domain_Name": "ACUS.GOV", "Domain_Name_Address": "https:\//www.ACUS.GOV"}, "Федеральное агентство": "административная конференция США", "RedirectInfo": {"перенаправление": "0", "Redirect_Destination": ""}, "ID": "9cc565c5-EBCD-1c03-ebd3-cc3e2ecd814d"}*
...
5. Number of Retries on Failure (Количество повторных попыток при сбое): указывает частоту повторных попыток подключения к Azure Cosmos DB при временных сбоях (например, при прерывании сетевого подключения).
...
4. Number of Retries on Failure (Количество повторных попыток при сбое): указывает частоту повторных попыток подключения к Azure Cosmos DB при временных сбоях (например, при прерывании сетевого подключения).
