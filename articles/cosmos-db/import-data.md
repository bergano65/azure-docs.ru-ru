...
* **[API MongoDB](mongodb-introduction.md)**. Сейчас средство переноса данных не поддерживает API MongoDB для Azure Cosmos DB в качестве источника или целевого объекта. Инструкции по переносу данных в коллекции API MongoDB в Azure Cosmos DB или из них см. в статье [Azure Cosmos DB: импорт данных MongoDB](mongodb-migrate.md). Вы по-прежнему можете экспортировать данные из MongoDB в коллекции API SQL в Azure Cosmos DB, чтобы использовать их с этим интерфейсом, с помощью средства переноса данных.
