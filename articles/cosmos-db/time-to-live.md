...
Срок жизни в Azure Cosmos DB отсчитывается в секундах от последнего изменения документа. Для этого используется поле `_ts`, которое имеется в каждом документе. В поле _ts отмечается дата и время в формате UNIX-времени. Поле `_ts` обновляется при каждом изменении документа. 
...
| --- |:--- |:--- |:--- |
| Свойство TTL на уровне документа отсутствует |Переопределение на уровне документа не происходит, так как свойство TTL не применяется ни для коллекции, ни для документа. |Документы в этой коллекции не устаревают. |Документы в этой коллекции устаревают по истечении интервала n. |
| Свойство TTL на уровне документа имеет значение -1 |Переопределение на уровне документа не происходит, так как для коллекции не определено свойство DefaultTTL. Система не учитывает свойство TTL, определенное для документа. |Документы в этой коллекции не устаревают. |Документ с TTL=-1 в этой коллекции никогда не устаревает. Все прочие документы устаревают по истечении интервала n. |
...
По умолчанию срок жизни отключен для всех коллекций Cosmos DB и для всех документов. Настроить TTL можно программно или с помощью портала Azure. Чтобы настроить TTL с помощью портала Azure, сделайте следующее:

1. Войдите на [портал Azure](https://portal.azure.com/) и перейдите в учетную запись Azure Cosmos DB.  

2. Перейдите в коллекцию, для которой нужно задать значение TTL, и откройте область **Масштаб и параметры**. Вы увидите, что срок жизни по умолчанию **отключен**. Задайте для TTL параметр **Включен (по умолчанию)** или **Включен**.

   **Отключен** — документы не удаляются автоматически.  
   **Включен (по умолчанию)** — позволяет задать для TTL значение –1 (бесконечно), что означает, что срок жизни документов по умолчанию не ограничен.  
   **Включен** — документы удаляются через n секунд после последнего изменения.  

   ![Настройка срока жизни](./media/time-to-live/set-ttl-in-portal.png)
...
TTL применяется ко всему документу. Если вы хотите, чтобы устаревала только часть документа, мы рекомендуем извлечь эту часть из основного документа в отдельный связанный документ, для которого и применить TTL.
