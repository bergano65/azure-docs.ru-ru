...
* В настоящее время поддерживаются типы операций `insert`, `update` и `replace`. Однако операция удаления или другие события пока не поддерживаются.
