...
> [!NOTE]
> Чтобы использовать потоки изменений, создайте учетную запись с API Azure Cosmos DB версии 3,6 для MongoDB или более поздней версии. Если запустить примеры потока изменений для более ранней версии, может появиться сообщение об ошибке `Unrecognized pipeline stage name: $changeStream`. 
