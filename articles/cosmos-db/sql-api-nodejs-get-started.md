...
ms.locfileid: "52160959"
---
# <a name="tutorial-build-a-nodejs-console-app-with-javascript-sdk-to-manage-azure-cosmos-db-sql-api-data"></a>Руководство. Создание консольного приложения Node.js с помощью пакета SDK JavaScript для управления данными API SQL для Azure Cosmos DB
...
В этом руководстве показано, как создать консольное приложение Node.js, которое создает ресурсы Azure Cosmos DB, и запросить эти ресурсы.
...
У вас нет времени, чтобы создать приложение? Не беспокойтесь! Полное решение доступно на [GitHub](https://github.com/Azure-Samples/azure-cosmos-db-sql-api-nodejs-getting-started ). Краткие инструкции см. в разделе [Получение полного решения для руководства по Node.js](#GetSolution) в этой статье.
...
1. Скопируйте и вставьте приведенный ниже код, чтобы задать идентификатор базы данных и идентификатор контейнера. По этим идентификаторам клиент Azure Cosmos DB найдет нужные базу данных и контейнер.
   ...
   Вы можете создать базу данных с помощью метода [createIfNotExists](/javascript/api/%40azure/cosmos/databases) или функции [create](/javascript/api/%40azure/cosmos/databases) класса **Databases**. База данных представляет собой логический контейнер элементов, разделенных между контейнерами. 
...
Контейнер можно создать с помощью метода [createIfNotExists](/javascript/api/%40azure/cosmos/containers) или функции [create](/javascript/api/%40azure/cosmos/containers) из класса **Containers**.  Контейнер состоит из элементов (которые в случае API SQL являются документами JSON) и связанной логики приложения JavaScript.
...
Azure Cosmos DB поддерживает [полнофункциональные запросы](how-to-sql-query.md) к документам JSON, хранящимся в каждом контейнере. В следующем примере кода показан выполняемый запрос к документам в контейнере.

1. Скопируйте и вставьте функцию **queryContainer** после функции **createFamilyItem** в файле app.js. Azure Cosmos DB поддерживает SQL-подобные запросы, как показано ниже. Дополнительные сведения о создании сложных запросов см. на [площадке для запросов](https://www.documentdb.com/sql/demo) и в [документации по запросам](how-to-sql-query.md).
