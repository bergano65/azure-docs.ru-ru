...
Пространственные функции могут использоваться для выполнения запросов близости к пространственным данным. Например, ниже приведен запрос, возвращающий все документы семейства, которые находятся в пределах 30 км указанного расположения с помощью встроенной функции `ST_DISTANCE`.
...
`ST_WITHIN` можно использовать для проверки того, находится ли точка в пределах многоугольника. Многоугольники обычно используются для представления границ, например почтовых зон, границ штатов или границ природных образований. При включении пространственного индексирования в политику индексирования "запросы нахождения внутри" будут эффективно обрабатываться с помощью индекса.
...
для проверки допустимости пространственного объекта можно использовать `ST_ISVALID` и `ST_ISVALIDDETAILED`. Например, следующий запрос проверяет действительность точки, значение широты для которой выходит за пределы допустимого диапазона (-132.8). `ST_ISVALID` возвращает только логическое значение, а `ST_ISVALIDDETAILED` возвращает значение типа Boolean и строку, содержащую причину, по которой она считается недопустимой.
...
Эти функции также могут использоваться для проверки многоугольников. Например, здесь мы используем `ST_ISVALIDDETAILED` для проверки многоугольника, который не закрыт.
...
Ниже приведен пример запроса LINQ, который находит все документы в контейнере Cosmos для Azure, значение `location` находится в радиусе 30 км указанной точки с помощью LINQ.
...
Аналогично, ниже приведен запрос на поиск всех документов, `location` в пределах указанного бокса или многоугольника.
