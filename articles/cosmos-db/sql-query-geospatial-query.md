...
Аргументы многоугольника `ST_WITHIN` в могут содержать только одно кольцо, то есть многоугольники не должны содержать отверстий в них.
...
Ниже приведен пример запроса LINQ, который находит все документы в контейнере Cosmos для Azure, значение `location` которых находится в радиусе 30 км указанной точки с помощью LINQ.
