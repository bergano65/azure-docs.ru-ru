...
В этих примерах показано, как настроить контейнер с пользовательской политикой разрешения конфликтов, использующей хранимую процедуру для решения конфликтов. Эти конфликты не будут отображаться в веб-канале конфликтов, если в хранимой процедуре нет ошибки. После создания политики с использованием контейнера необходимо создать хранимую процедуру. См. пример пакета SDK для .NET. Эту политику поддерживает только API Core (SQL).
