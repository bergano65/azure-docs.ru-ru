...
1. В списке параметров выберите **Подключения к частной конечной точке**, а затем выберите **Частная конечная точка**.
   ...
   ![Выбор параметров для создания частной конечной точки на портале Azure](./media/how-to-configure-private-endpoints/create-private-endpoint-portal.png)
...
![Выходные данные развертывания шаблона Resource Manager](./media/how-to-configure-private-endpoints/resource-manager-template-deployment-output.png)
...
* Если вы не настраиваете никакого общего трафика или конечной точки службы, но создаете частные конечные точки, учетная запись Azure Cosmos будет доступна только через частные конечные точки. Если вы не настроите общедоступный трафик или конечную точку службы, то после того, как все утвержденные частные конечные точки будут отклонены или удалены, учетная запись станет открыта для всей сети.
...
* Если вы используете Приватный канал с учетной записью Azure Cosmos с помощью прямого подключения, то можно использовать только протокол TCP. Протокол HTTP в настоящее время не поддерживается.
...
* Сведения о настройке брандмауэра для Azure Cosmos DB см. в статье о [поддержке брандмауэра](firewall-support.md).
