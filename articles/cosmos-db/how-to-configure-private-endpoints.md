...
> [!NOTE]
> Частная поддержка конечных точек в настоящее время обычно доступна только для режима подключения шлюза. Для прямого режима он доступен в качестве функции предварительного просмотра.
...
1. В **создать частную конечную точку - Основы** панели, введите или выберите следующие детали:
    ...
    | **Детали инстанции** |  |
    | Имя | Введите любое имя для вашей частной конечной точки. Если это имя будет принято, создайте уникальное. |
...
1. При **создании частной конечной точки - Конфигурация**введите или выберите эту информацию:
    ...
    |Интеграция с частной зоной DNS |Выберите **Да**. <br><br/> Чтобы подключиться к частной части с вашей частной конечной точкой, вам нужна запись DNS. Мы рекомендуем вам интегрировать вашу частную конечную точку с частной зоной DNS. Вы также можете использовать свои собственные DNS-серверы или создавать DNS-записи, используя файлы-хоста на виртуальных машинах. |
...
## <a name="blocking-public-network-access-during-account-creation"></a>Блокирование общедоступного доступа к сети во время создания учетной записи

Как описано в предыдущем разделе, и если не были установлены конкретные правила брандмауэра, добавление частной конечной точки делает вашу учетную запись Azure Cosmos доступной только через частные конечные точки. Это означает, что учетная запись Azure Cosmos может быть достигнута из общественного трафика после ее создания и до добавления частной конечной точки. Чтобы убедиться, `Disabled` что доступ к общедоступной сети отключен еще `publicNetworkAccess` до создания частных конечных точек, можно настроить флаг во время создания учетной записи. Например, в [этом шаблоне управления ресурсами Azure](https://azure.microsoft.com/resources/templates/101-cosmosdb-private-endpoint/) можно ознакомиться с примером использования этого флага.
