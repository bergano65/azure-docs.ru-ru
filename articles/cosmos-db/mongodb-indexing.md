...
Для индексирования дополнительных полей применяются команды управления индексами MongoDB. Как и в MongoDB, API Azure Cosmos DB для MongoDB автоматически индексирует `_id` только поле. Эта политика индексирования по умолчанию отличается от Azure Cosmos DB API SQL, который по умолчанию индексирует все поля.
