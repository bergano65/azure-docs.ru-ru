...
- Если нет, выберите его и щелкните "зарегистрировать":
...
- Щелкните "+ добавить политику доступа".
- В раскрывающемся меню "разрешения для ключа" выберите "получить", "разворачивать ключ" и "ключ переноса":

    ![Выбор нужных разрешений](./media/how-to-setup-cmk/portal-akv-add-ap-perm2.png)

- В разделе "Выбор субъекта" щелкните "нет выбранных", найдите и выберите участника "Azure Cosmos DB", а затем нажмите кнопку "выбрать" в нижней части (если не удается найти субъект "Azure Cosmos DB", возможно, потребуется повторно зарегистрировать ресурс Microsoft. DocumentDB. поставщик на шаге 2):
...
- Скопируйте ключ "идентификатор ключа", кроме части, которая находится после последней косой черты:
...
При создании новой учетной записи Azure Cosmos DB из портал Azure выберите "ключ, управляемый клиентом" на шаге "шифрование". В поле "URI ключа" передайте универсальный код ресурса (URI) Azure Key Vault ключа, скопированного на шаге 4.
...
- передайте URI Azure Key Vaultого ключа, скопированного на шаге 4, в свойстве "Кэйваулткэйури" в "Пропертйобжект".
...
- Передайте универсальный код ресурса (URI) Azure Key Vault ключа, скопированный на шаге 4 в свойстве "Кэйваулткэйури" объекта "Properties".
- обязательно используйте "2019-12-12" в качестве версии API
