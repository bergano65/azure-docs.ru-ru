...
![Включение обратимого удаления и защиты от очистки для нового экземпляра Azure Key Vault](./media/how-to-setup-cmk/portal-akv-prop.png)
...
1. На портале Azure перейдите к экземпляру Azure Key Vault, который планируется использовать для размещения ключей шифрования. В меню слева выберите **Политики доступа**.
   ...
   ![Пункт "Политики доступа" в меню слева](./media/how-to-setup-cmk/portal-akv-ap.png)
...
1. Скопируйте **идентификатор ключа**, кроме части после последней косой черты:
   ...
   ![Копирование идентификатора ключа](./media/how-to-setup-cmk/portal-akv-keyid.png)
...
Вы можете программно получить сведения об учетной записи Azure Cosmos и проверить наличие свойства `keyVaultKeyUri`. Сведения о том, как это можно сделать [в PowerShell](#using-powershell) и [с помощью Azure CLI](#using-azure-cli), см. выше.
...
### <a name="how-do-i-rotate-an-encryption-key"></a>Как сменить ключ шифрования?

Смена ключа выполняется путем создания его новой версии в Azure Key Vault.

![Создание новой версии ключа](./media/how-to-setup-cmk/portal-akv-rot.png)

Предыдущая версия может быть отключена через 24 часа или после того, как в [журнал аудита Azure Key Vault](../key-vault/general/logging.md) перестанут добавляться сведения о действиях Azure Cosmos DB с этой версией.
