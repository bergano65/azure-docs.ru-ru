...
В этой статье описывается вложенные запросы SQL и их часто используется в Azure Cosmos DB. Все примеры запросов в этом документе может выполняться для сельского набор данных, предварительно загружается на [Azure Cosmos DB площадку для запросов](https://www.documentdb.com/sql/demo).
