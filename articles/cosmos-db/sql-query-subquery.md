...
Следующий запрос выполняет `GetMaxNutritionValue` UDF дважды:
...
### <a name="example-rewriting-array_contains-and-join-as-exists"></a>Пример: перезапись ARRAY_CONTAINS и присоединение как существует
...
Рассмотрим следующий запрос, который фильтруется на основе свойств Units и `nutritionValue` в массиве: 
