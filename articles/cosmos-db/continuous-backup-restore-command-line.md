...
ms.locfileid: "99527728"
---
# <a name="configure-and-manage-continuous-backup-and-point-in-time-restore---using-azure-cli"></a>Настройка непрерывного резервного копирования и восстановление на момент времени и управление ими с помощью Azure CLI
...
Следующая команда показывает пример учетной записи с одним регионом с именем `pitracct3` с политикой непрерывного резервного копирования, которая создала регион "Западная часть США" в группе ресурсов "myrg":
...
Используйте следующую команду, чтобы получить список всех версий контейнеров SQL. Эта команда работает только с учетными записями Live. Параметр "databaseRid" — это "ResourceId" базы данных, которую необходимо восстановить. Это значение атрибута «Овнерресаурцеид», найденное в ответе `az cosmosdb sql restorable-database list` команды.
