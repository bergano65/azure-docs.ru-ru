...
Процесс ротации главного ключа прост. 
...
:::image type="content" source="./media/secure-access-to-data/nosql-database-security-master-key-rotate-workflow.png" alt-text="Ротация главного ключа на портале Azure: демонстрация безопасности базы данных NoSQL" border="false":::

### <a name="code-sample-to-use-a-master-key"></a>Пример кода для использования главного ключа

В следующем примере кода показано, как использовать конечную точку учетной записи Cosmos DB и главный ключ для создания экземпляра DocumentClient и создания базы данных.
...
В следующем примере кода показано, как использовать конечную точку учетной записи Azure Cosmos DB и главный ключ для создания экземпляра `CosmosClient` объекта:
