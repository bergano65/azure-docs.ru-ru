...
ms.locfileid: "79251821"
---
# <a name="secure-access-to-data-in-azure-cosmos-db"></a>Защита доступа к данным в Azure Cosmos DB

В этой статье содержится обзор обеспечения доступа к данным, хранящимся в [Microsoft Azure Cosmos DB.](https://azure.microsoft.com/services/cosmos-db/)
...
Мастер-ключи предоставляют доступ ко всем административным ресурсам для учетной записи базы данных. Главные ключи.
...
## <a name="resource-tokens"></a>Токены ресурсов<a id="resource-tokens"></a>
...
- имеют ограничение по времени (настраиваемый срок действия). Действительный срок действия по умолчанию составляет один час. Однако продолжительность действия маркера можно задать явным образом. Ее значение не должно превышать пять часов;
...
Пользователи Azure Cosmos DB связаны с базой данных Cosmos.  Каждая база данных может содержать несколько пользователей Cosmos DB или не содержать ни одного. Следующий пример кода показывает, как создать пользователя Cosmos DB с помощью [Azure Cosmos DB .NET SDK v3.](https://github.com/Azure/azure-cosmos-dotnet-v3/tree/master/Microsoft.Azure.Cosmos.Samples/Usage/UserManagement)
...
> [!NOTE]
> Каждый пользователь Cosmos DB имеет метод ReadAsync() который может быть использован для получения списка [разрешений, связанных](#permissions) с пользователем.

## <a name="permissions"></a>Разрешения<a id="permissions"></a>

Ресурс разрешения связан с пользователем и присваивается в контейнере, а также на уровне ключа раздела. Каждый пользователь может содержать ноль или более разрешений. Ресурс разрешения обеспечивает доступ к маркеру безопасности, который необходим пользователю при попытке получить доступ к определенному контейнеру или данным в определенном ключе раздела. Существует два уровня доступа, предоставляемых ресурсом разрешения:
...
> [!NOTE]
> Для выполнения сохраненных процедур пользователь должен иметь все разрешения на контейнер, в котором будет запущена сохраненная процедура.
...
### <a name="code-sample-to-read-permission-for-user"></a>Образец кода для чтения разрешения для пользователя

Следующий фрагмент кода показывает, как получить разрешение, связанное с пользователем, созданным выше, и мгновенно вывести нового CosmosClient от имени пользователя, приобщенный к одному ключу раздела.
...
4. В **присваивайте доступ к коробке,** выберите **пользователя Azure AD, группу или приложение.**
...
- Чтобы узнать больше о безопасности базы данных Cosmos, [см.](database-security.md)
...
- Образцы управления пользователями с пользователями и разрешениями, [образцы управления пользователями .NET SDK v3](https://github.com/Azure/azure-cosmos-dotnet-v3/blob/master/Microsoft.Azure.Cosmos.Samples/Usage/UserManagement/UserManagementProgram.cs)
