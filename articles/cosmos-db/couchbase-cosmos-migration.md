...
Где *_repo* — объект репозитория, а *doc* — объект класса POJO. Можно использовать `.save` для вставки или Upsert (если найден документ с указанным идентификатором). В следующем фрагменте кода показано, как вставить или обновить объект doc:
...
|-------------------|-------------------|
|Выберите META (`TravelDocument`). ID в качестве идентификатора `TravelDocument`,. * `TravelDocument` из `_type` Where = "com. XX. XX. XX. xxx. xxx. XXXX" и Country = "Индия", а любой m в Висас удовлетворяет m. Type = = "Multi-Entry" и m. Country в ["Индия", Бутан "] Order ` Validity` by DESC порог 25 offset 0   | Выберите c. ID, c из c JOIN m в c. Country = "Индия" WHERE c. _type = "com. XX. XX. XX. xxx. xxx. XXXX" и c. Country = "Индия" и m. Type = "Multi-Entry" и m. Country в ("Индии", "Бутан") порядок по c. смещение DESC OFFSET 0 |
...
1. Создайте объект соединения для Azure Cosmos DB с помощью `ConnectionBuilder` метода, как показано в следующем примере. Убедитесь, что вы поместили это объявление в Bean таким образом, что следующий код должен быть выполнен только один раз:
