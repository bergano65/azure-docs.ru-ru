---
title: Запись сохраненных процедур, триггеров и UDF в Azure Cosmos DB
...
> [!Tip]
> Cosmos поддерживает развертывание контейнеров с сохраненными процедурами, триггерами и функциями, определяемыми пользователем. Для получения дополнительной информации [см. Создать контейнер Azure Cosmos DB с функциональностью на стороне сервера.](manage-sql-with-resource-manager.md#create-sproc)
...
### <a name="pre-triggers"></a><a id="pre-triggers"></a>Триггеры предварительного выполнения
...
### <a name="post-triggers"></a><a id="post-triggers"></a>Триггеры последующего выполнения
...
При использовании сохраненной процедуры, триггеров или пользовательских `console.log()` функций можно регистрировать шаги с помощью команды. Эта команда сконцентрирует строку для отладки, когда `EnableScriptLogging` она будет верной, как показано в следующем примере:
