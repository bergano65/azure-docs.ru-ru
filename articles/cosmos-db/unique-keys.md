...
description: Узнайте, как определить и использовать уникальные ключи для базы данных Azure Cosmos. В этой статье также описывается, как уникальные ключи добавляют слой целостности данных.
author: markjbrown
ms.author: mjbrown
...
После создания контейнера с уникальной ключевой политикой предотвращается создание нового или обновления существующего элемента, в результате чего дубликат в логическом разделе предотвращается, как это определено уникальным ограничением ключа. Ключ секции в сочетании с уникальным ключом гарантирует уникальность элемента в области контейнера.
...
* В именах уникальных ключей учитывается регистр. Например, рассмотрим контейнер с уникальным набором `/address/zipcode`ключей. Если у ваших данных есть поле с именем, `ZipCode`Azure Cosmos `zipcode` DB вставляет "null" в качестве уникального ключа, потому что это не то же самое, что `ZipCode`. Учет регистра в этой ситуации приводит к тому, что другие записи со значением ZipCode добавлены не будут, ведь новое значение null нарушит требование уникальности ключа.
