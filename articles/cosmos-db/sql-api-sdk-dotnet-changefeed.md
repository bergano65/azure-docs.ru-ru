...
* Улучшения стабильности и диагностики:
  * Добавлена поддержка для обнаружения чтения веб-канала изменений в течение длительного времени. Если он занимает больше времени, чем указано в свойстве `ChangeFeedProcessorOptions.ChangeFeedTimeout`, выполняются следующие действия.
  ...
  * Добавлено новое общее значение перечисления: `Monitoring.MonitoredOperation.ReadChangeFeed`. Если для параметра `HealthMonitoringRecord.Operation` задано значение `Monitoring.MonitoredOperation.ReadChangeFeed`, это указывает на то, что неполадка работоспособности связана с чтением веб-канала изменений.
