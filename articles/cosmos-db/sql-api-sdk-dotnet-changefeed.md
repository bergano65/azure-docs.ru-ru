...
> * [REST](/rest/api
> * [Поставщик ресурсов REST](/rest/api
...
* Небольшое критическое изменение: методы интерфейса IChangeFeedDocumentClient и класса ChangeFeedDocumentClient были изменены для включения параметров RequestOptions и CancellationToken. IChangeFeedDocumentClient — это точка расширяемости с дополнительными возможностями, которая позволяет создавать собственную реализацию клиента документов для использования с обработчиком канала изменений, например для декорирования клиента документов и перехвата всех вызовов к нему для дополнительной трассировки, обработки ошибок и т. д. После этого обновления в код, который реализует IChangeFeedDocumentClient, необходимо будет включить новые параметры.
...
> [!WARNING]
> После 31 августа 2022 г. Azure Cosmos DB больше не будет получать исправления ошибок, новые функции и поддержку версий 1.x пакета SDK Azure Cosmos DB для .NET или для .NET Core для API SQL. Даже если вы не выполните обновление, запросы, отправляемые из версий 1.х пакета SDK, будут обрабатываться службой Azure Cosmos DB.
