---
title: Azure Cosmos DB API обработчика веб-канала изменений .NET, заметки о выпуске пакета SDK
...
|   |   |
|---|---|
|**Загрузка пакета SDK**|[NuGet](https://www.nuget.org/packages/Microsoft.Azure.DocumentDB.ChangeFeedProcessor/)|
...
* Добавлен новый метод `ChangeFeedProcessorBuilder.WithCheckpointPartitionProcessorFactory` и соответствующий открытый интерфейс `ICheckpointPartitionProcessorFactory`. Это позволяет реализации `IPartitionProcessor` интерфейса использовать встроенный механизм создания контрольных точек. Новая фабрика аналогична существующей `IPartitionProcessorFactory`, за исключением того, `Create` что ее метод также `ILeaseCheckpointer` принимает параметр.
* Для одного и того же `ChangeFeedProcessorBuilder.WithPartitionProcessorFactory` `ChangeFeedProcessorBuilder.WithCheckpointPartitionProcessorFactory` `ChangeFeedProcessorBuilder` экземпляра можно использовать только один из двух методов: или.

### <a name="228"></a><a name="2.2.8"/>2.2.8
* Улучшения стабильности и диагностики:
  * Добавлена поддержка для обнаружения чтения веб-канала изменений в течение длительного времени. Если он занимает больше времени, чем указано `ChangeFeedProcessorOptions.ChangeFeedTimeout` свойством, выполняются следующие шаги.
    * Операция чтения канала изменений в проблемной секции прервана.
    * Экземпляр обработчика веб-канала изменений удаляет владение проблемной аренды. Удаленная Аренда будет получена во время следующего шага получения аренды, которая будет выполнена тем же или другим экземпляром обработчика веб-канала изменений. Таким образом, чтение веб-канала изменений начнется заново.
    * В монитор работоспособности передается сообщение о проблемах. Монитор работоспособности по умолчанию отправляет все обнаруженные проблемы в журнал трассировки.
  * Добавлено новое общедоступное свойство `ChangeFeedProcessorOptions.ChangeFeedTimeout`:. Значение этого свойства по умолчанию равно 10 минутам.
  * Добавлено новое общее значение перечисления: `Monitoring.MonitoredOperation.ReadChangeFeed`. Если для `HealthMonitoringRecord.Operation` `Monitoring.MonitoredOperation.ReadChangeFeed`параметра задано значение, это указывает на то, что проблемы с работоспособностью связаны с чтением веб-канала изменений.

### <a name="227"></a><a name="2.2.7"/>2.2.7
* Улучшена стратегия балансировки нагрузки для сценария, когда получение всех аренд занимает больше времени, чем истекает срок действия аренды, например из-за проблем с сетью:
  * В этом сценарии используется алгоритм балансировки нагрузки, с использованием которого можно ошибочно рассмотреть аренду с истекшим сроком действия, что приводит к краже аренды от активных владельцев. Это может вызвать ненужное повторное распределение большого количества аренд.
  * Эта проблема исправлена в этом выпуске, избегая повторных попыток конфликтов при получении аренды с истекшим сроком действия, когда владелец не изменился и поспонинг получение аренды с истекшим сроком действия для следующей итерации балансировки нагрузки.
...
* Небольшое критическое изменение: методы интерфейса IChangeFeedDocumentClient и класса ChangeFeedDocumentClient были изменены для включения параметров RequestOptions и CancellationToken. Ичанжефиддокументклиент — это расширенная точка расширения, которая позволяет предоставить пользовательскую реализацию клиента документа для использования с обработчиком веб-канала изменений, например, украшение DocumentClient и перехват всех вызовов к нему для выполнения дополнительной трассировки, обработки ошибок и т. д. В этом обновлении код, реализующий Ичанжефиддокументклиент, потребуется изменить, чтобы включить в реализацию новые параметры.
...
* Улучшение стабильности.
  * Исправление для обработки отмененных задач, которые могут привести к остановке наблюдателей в некоторых разделах.
