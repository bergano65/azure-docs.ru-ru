...
|**Документация по API**|[Справочная документация по изменению API библиотеки обработчика веб-каналов](/dotnet/api/microsoft.azure.documents.changefeedprocessor?view=azure-dotnet&preserve-view=true)|
...
* Добавлена совместимость хранилища аренды с пакетом SDK для версии 3, который обеспечивает пути для оперативной миграции. Приложение может перейти на пакет SDK v3 и выполнить миграцию обратно в библиотеку обработчика веб-канала изменений без потери состояния.
...
* Улучшена стратегия балансировки нагрузки для сценария, когда получение всех аренд занимает больше времени, чем истечение срока действия аренды, например из-за проблем с сетью.
  * В этом сценарии используется алгоритм балансировки нагрузки, который позволяет ошибочно рассмотреть аренду с истекшим сроком действия, что приводит к краже аренды от активных владельцев. Это может вызвать ненужное повторное распределение множества аренд.
  * Эта проблема исправлена в этом выпуске, избегая повторных попыток конфликта при получении аренды с истекшим сроком действия, когда владелец не изменился и не откладывает получение аренды с истекшим сроком до следующей итерации балансировки нагрузки.
...
* Добавлена поддержка обработки разбиения коллекций, использующих пропускную способность общей базы данных.
  * В этом выпуске устранена проблема, которая может возникнуть во время разделения в коллекциях с использованием пропускной способности общей базы данных при повторном распределении секций с одним созданным диапазоном ключей дочернего раздела, а не двумя. В таких ситуациях обработчик канала изменений может зависнуть, удаляя аренду для старого диапазона ключей раздела и не создавая новые аренды. В этом выпуске эта проблема исправлена.
...
* Фиксированный расчет средства оценки для учетных записей с несколькими регионами записи и новым форматом токена сеанса.
...
### <a name="202-prerelease"></a><a id="2.0.2-prerelease"></a>Предварительный выпуск 2.0.2:
