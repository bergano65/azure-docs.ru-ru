...
A. Да, теперь эта функция доступна для новых учетных записей Cosmos и должна выполняться во время создания учетной записи. Дополнительные сведения см. в статье [Настройка ключей, управляемых клиентом, для учетной записи Azure Cosmos с помощью Azure Key Vault](https://docs.microsoft.com/azure/cosmos-db/how-to-setup-cmk).
