...
Чтобы использовать этот шаг, просто добавьте вызов `executionProfile()` функции в конец запроса Gremlin. **Запрос Gremlin будет выполнен** , и результат операции вернет объект ответа JSON с профилем выполнения запроса.
...
> [!NOTE]
> На шаге Ексекутионпрофиле будет выполнен запрос Gremlin. Сюда входят шаги `addV` или `addE`, что приведет к созданию и зафиксировать изменения, указанные в запросе. В результате будут также оплачены единицы запроса, созданные запросом Gremlin.
...
`Coalesce`| На этом шаге выполняется вычисление двух операций при каждом выполнении `coalesce()` шага Gremlin.
...
- Если взглянуть `storeOps` на объект, можно увидеть, что `fanoutFactor` имеет `5`значение, а это означает, что эта операция осуществляла доступ к [5 секциям](https://docs.microsoft.com/azure/cosmos-db/partition-data) .
