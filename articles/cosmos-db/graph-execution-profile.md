...
Чтобы использовать этот шаг, просто добавьте вызов функции `executionProfile()` в конце запроса Gremlin. **Запрос Gremlin будет выполнен** , и результат операции вернет объект ответа JSON с профилем выполнения запроса.
...
После вызова шага `executionProfile()` ответ будет объектом JSON, который включает в себя выполненный шаг Gremlin, общее время, затраченное на выполнение инструкции, а также массив операторов среды выполнения Cosmos DB, на которые привела инструкция.
...
> [!NOTE]
> На шаге Ексекутионпрофиле будет выполнен запрос Gremlin. Сюда входят `addV` или `addE`шаги, которые приведут к созданию и зафиксируют изменения, указанные в запросе. В результате будут также оплачены единицы запроса, созданные запросом Gremlin.
...
В ответе функции Ексекутионпрофиле () будет выдаваться иерархия объектов JSON со следующей структурой:
  - **Объект операции Gremlin**: представляет всю выполненную операцию Gremlin. Содержит следующие свойства.
    ...
    - `metrics`: массив, содержащий все операторы среды выполнения Cosmos DB, которые были выполнены для выполнения запроса. Этот список сортируется в порядке выполнения.
  ...
  - **Cosmos DB операторы времени выполнения**: представляет каждый из компонентов всей операции Gremlin. Этот список сортируется в порядке выполнения. Каждый объект содержит следующие свойства:
    ...
    - `time`: количество времени (в миллисекундах), затраченное данным оператором.
    ...
    - `counts`: количество объектов, возвращенных этим оператором из уровня хранилища. Он содержится в `counts.resultCount` скалярного значения в.
...
Cosmos DB оператор среды выполнения Gremlin|Description
---|---
...
`Coalesce`| На этом шаге выполняется вычисление двух операций при каждом выполнении шага `coalesce()` Gremlin.
...
- Судя от метрики `time`, задержка этого запроса кажется высокой, так как она [больше 10 мс для одной операции чтения точки](https://docs.microsoft.com/azure/cosmos-db/introduction#guaranteed-low-latency-at-99th-percentile-worldwide).
- Если взглянуть на объект `storeOps`, можно увидеть, что `fanoutFactor` `5`, что означает, что эта операция была обращена к [5 секциям](https://docs.microsoft.com/azure/cosmos-db/partition-data) .

В конце этого анализа мы можем определить, что первый запрос обращается к большему числу секций, чем необходимо. Это можно устранить, указав ключ секционирования в запросе в виде предиката. Это приведет к снижению задержки и снижению затрат на запрос. Узнайте больше о [секционировании графов](graph-partitioning.md). Более оптимальный запрос будет `g.V('tt0093640').has('partitionKey', 't1001')`.
...
- Значение `metrics[0].time` находится выше в первом ответе, что означает, что этот один шаг занял больше времени для устранения.
- `metrics[0].counts.resultsCount` значение выше, чем в первом ответе, что означает, что начальный рабочий набор данных был больше.
