...
- **Приоритет последней записи (LWW)** . При применении этой политики по умолчанию используется свойство метки времени, назначенное системой (на основе протокола синхронизации времени). При использовании API SQL можно указать любое другое настраиваемое числовое свойство (например, собственное представление метки времени), которое будет использоваться для разрешения конфликтов. Настраиваемое числовое свойство также называется *путем разрешения конфликтов*. 
  ...
  > [!NOTE]
  > Приоритет последней записи — это политика разрешения конфликтов по умолчанию. Он доступен для следующих API: SQL, MongoDB, Cassandra, Gremlin и Table.
