...
Пространственные индексы можно использовать в правильно отформатированных геообъектах [JSON](geospatial.md) . Точки, LineString и многоугольники в настоящее время поддерживаются.
...
> [!NOTE]
> Предложение, которое упорядочивается по отдельному свойству, всегда нуждается в индексе диапазона и завершается ошибкой, если путь, на который он ссылается, не имеет такого значения.  `ORDER BY` Аналогичным образом, `ORDER BY` нескольким запросам *всегда* требуется составной индекс.
