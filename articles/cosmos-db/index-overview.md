...
До тех пор, пока один предикат фильтра использует один из индекса рода, движок запроса будет оценивать, что сначала перед сканированием остальных. Например, если у вас есть запрос на`SELECT * FROM c WHERE c.firstName = "Andrew" and CONTAINS(c.lastName, "Liu")`
