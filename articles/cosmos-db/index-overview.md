...
До тех пор, пока один предикат фильтра использует один из типов индексов, обработчик запросов будет оценивать это перед проверкой остальных. Например, если имеется SQL-запрос, например`SELECT * FROM c WHERE c.firstName = "Andrew" and CONTAINS(c.lastName, "Liu")`
