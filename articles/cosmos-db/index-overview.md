...
В статье описан способы индексирования данных в Azure Cosmos DB и использования индексов для повышения производительности запросов. Перед изучением процедуры настройки [политик индексирования](index-policy.md)рекомендуется пройти по этому разделу.
...
До тех пор, пока один предикат фильтра использует для типа индекса, обработчик запросов оценивает это перед продолжением поиска. Например, если имеется SQL-запрос, например `SELECT * FROM c WHERE c.firstName = "Andrew" and CONTAINS(c.lastName, "Liu")`
...
> [!NOTE]
> Предложение `ORDER BY`, которое упорядочивается по отдельному свойству, *всегда* нуждается в индексе диапазона и завершается ошибкой, если путь, на который он ссылается, не имеет такого значения. Аналогичным образом `ORDER BY` запрос, который упорядочивает по нескольким свойствам, *всегда* должен иметь составной индекс.
