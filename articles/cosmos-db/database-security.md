...
- Возможность выполнить отработку отказа из одного ЦОД в другой
...
- Установка исправлений на серверы, которые находятся в актуальном состоянии
...
|Интеграция с Active Directory (RBAC)| Кроме того, можно предоставить или ограничить доступ к учетной записи Cosmos, базы данных, контейнера и предложения (пропускная способность), с помощью управления доступом (IAM) на портале Azure. IAM обеспечивает управление доступом на основе ролей и интегрируется с Active Directory. Можно использовать встроенные или пользовательские роли для отдельных пользователей и групп. См. в разделе [интеграции Active Directory](role-based-access-control.md) для получения дополнительной информации.|
