...
В настоящее время просмотр документов, содержащих UUID, не поддерживается в Data Explorer. Это не влияет на сборы, только просмотр отдельных документов или запросов, которые включают эти документы. Для просмотра и управления этими документами пользователи должны продолжать использовать инструмент, который первоначально использовался для создания этих документов.
