...
description: Узнайте, как использовать Azure Cosmos DB, чтобы перемещать существующие приложения или создавать новые, используя средства CQL и драйверы Cassandra.
author: kanshiG
ms.author: govindk
ms.reviewer: sngun
...
> [!NOTE]
> В API Cassandra Azure Cosmos DB теперь доступен [режим бессерверной емкости](serverless.md).
...
**Стандарт с открытым исходным кодом.** Хотя это полностью управляемая служба, API Cassandra по-прежнему поддерживает возможности собственного [сетевого протокола Apache Cassandra](cassandra-support.md), позволяя создавать приложения в соответствии с широко используемым облачным стандартом с открытым исходным кодом.
...
**Эластичность пропускной способности и хранилища**. Azure Cosmos DB обеспечивает пропускную способность между регионами с возможностью ее масштабирования в рамках операций, выполняемых на портале Azure, а также в PowerShell или интерфейсе командной строки. Вы можете [эластично масштабировать](manage-scale-cassandra.md) память и пропускную способность для таблиц по мере необходимости на основе предсказуемой производительности.
...
* Просмотрите [часто задаваемые вопросы](cassandra-faq.md).
