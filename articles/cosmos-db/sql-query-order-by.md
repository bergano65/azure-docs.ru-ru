...
В дополнительном положении ORDER BY указывается порядок сортировки результатов, возвращенных запросом.
...
- `<sort_specification>`  
   ...
   Можно указать несколько свойств. Названия свойств должны быть уникальными. Последовательность свойств сортировки в пункте ORDER BY определяет организацию отсортированного набора результатов. То есть результирующий набор сортируется по первому свойству, а затем упорядоченный список сортируется по второму свойству и т. д.  
  
   Имена свойств, упомянутые в оговорке ORDER BY, должны соответствовать либо свойству в избранном списке, либо свойству, определенному в коллекции, указанному в пункте FROM, без каких-либо двусмысленностей.  
