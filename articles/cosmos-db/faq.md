...
Да, API таблиц использует платформу службы Azure Cosmos DB для резервного копирования. Резервные копии создаются автоматически. См. дополнительные сведения об [оперативном резервном копировании и восстановлении с помощью Azure Cosmos DB](../synapse-analytics/sql-data-warehouse/backup-and-restore.md).
...
### <a name="what-is-the-protocol-version-supported-by-azure-cosmso-db-cassandra-api-is-there-a-plan-to-support-other-protocols"></a>Что такое протокольная версия, поддерживаемая Azure Cosmso DB Cassandra API? Планируется ли поддержка других протоколов?
