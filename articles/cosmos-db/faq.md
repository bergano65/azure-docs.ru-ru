...
Служба Azure Cosmos DB доступна на портале Azure. Во-первых, вам нужно зарегистрироваться для получения подписки Azure. Если вы зарегистрировались, можно добавить учетную запись Azure Cosmos DB в свою подписку Azure.
...
### <a name="ive-set-up-my-container-to-use-lazy-indexing-i-see-that-my-queries-dont-return-expected-results"></a>Для моего контейнера настроено использование отложенного индексирования, и я вижу, что запросы не возвращают ожидаемые результаты.
...
API Azure Cosmos DB для MongoDB — это слой совместимости протокола связи, который позволяет приложениям легко и прозрачно взаимодействовать с ядром собственной СУБД Azure Cosmos DB с помощью существующих и поддерживаемых сообществом пакетов SDK и драйверов для MongoDB. Разработчики теперь могут применять существующие цепочки инструментов MongoDB и уже наработанные навыки для создания приложений, пользующихся преимуществами Azure Cosmos DB. Azure Cosmos DB предлагает разработчикам такие уникальные функции, как глобальное распределение с многомастерной репликацией, автоматическая индексация, поддержка резервного копирования, финансово подкрепленные соглашения об уровне обслуживания и т. д.

### <a name="how-do-i-connect-to-my-database"></a>Как подключиться к базе данных?

Чтобы быстро подключиться к базе данных Cosmos через API Azure Cosmos DB для MongoDB, откройте [портал Azure](https://portal.azure.com). Войдите в свою учетную запись и выберите в меню навигации слева **Быстрый запуск**. Эта функция позволяет оперативно получить фрагменты кода для подключения к базе данных.
...
В этих выходных данных показано, сколько времени понадобилось на получение объектов (вершин, ребер), а также указан размер рабочего набора данных. На основе этих значений и формируется стоимость запросов Azure Cosmos DB.
