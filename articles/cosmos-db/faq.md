...
API DocumentDB или API-интерфейс SQL (DocumentDB) для Azure Cosmos DB теперь называется API SQL для Azure Cosmos DB. Вам не нужно ничего менять, чтобы продолжать работу с приложениями, созданными с помощью API DocumentDB. Их функциональность остается прежней.
...
* До трех коллекций на каждую подписку для учетных записей MongoDB.
...
* Пропускная способность минимальное 20 ЕЗ/с при подготовке на уровне базы данных.
...
| OPTIONS | /<table-resource-name> | [Запрос таблицы предварительного CORS](https://docs.microsoft.com/rest/api/storageservices/preflight-table-request) | Этот метод является частью CORS и сейчас не поддерживается в Azure Cosmos DB. |
| ПОЛУЧЕНИЕ | /?restype=service@comp=stats | [Получение статистики службы таблиц](https://docs.microsoft.com/rest/api/storageservices/get-table-service-stats) | Предоставляет сведения о том, как быстро данные копируются между первичными и вторичными базами данных. Этот метод не требуется в Cosmos DB, так как репликация является частью операций записи. |
...
* [Мнение пользователей](https://feedback.azure.com/forums/263030-azure-cosmos-db)
* [Форум MSDN](https://social.msdn.microsoft.com/Forums/en-US/home?forum=azurecosmosdb)
* [Stack Overflow](https://stackoverflow.com/questions/tagged/azure-cosmosdb). Stack Overflow лучше всего подходит, чтобы задать вопросы о программировании. Ваш вопрос должен быть [по теме](https://stackoverflow.com/help/on-topic) и [максимально точным и подробным, чтобы на него можно было ответить по существу](https://stackoverflow.com/help/how-to-ask).
...
Собственные драйверы Tinkerpop Gremlin позволяют использовать словарь параметров для выполнения запросов. См. пример с использованием [Gremlin.Net](https://tinkerpop.apache.org/docs/3.2.7/reference/#gremlin-DotNet) и [Gremlin-Javascript](https://github.com/Azure-Samples/azure-cosmos-db-graph-nodejs-getting-started/blob/master/app.js).
