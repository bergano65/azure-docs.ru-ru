...
* Разрешение на доступ к учетной записи может предоставляться только личным учетным записям Майкрософт (MSA). Не используйте учетные записи Active Directory (AAD) или учетные записи, принадлежащие корпоративным арендаторам AAD, так как для них могут распространяться ограничения, блокирующие доступ.
...
Главный ключ — это токен безопасности для доступа ко всем ресурсам учетной записи. Пользователи с этим ключом имеют доступ на чтение и запись для всех ресурсов в пределах учетной записи базы данных. Будьте осторожны при распространении главных ключей. Первичный и вторичный главные ключи см. в колонке **Ключи** на [портале Azure][azure-portal]. Дополнительные сведения о ключах см. в разделе [Просмотр, копирование и повторное создание ключей доступа](manage-with-cli.md#list-account-keys).
