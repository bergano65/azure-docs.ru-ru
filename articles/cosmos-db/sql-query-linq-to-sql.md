...
- **Математические функции**: поддерживают преобразование из `Abs`.NET `Acos`, `Asin`, `Atan`, `Ceiling`, `Cos`, `Exp`, `Floor`, `Log`, `Log10`, `Pow` `Round` `Sign` `Sin` `Sqrt` `Tan`,,,,, и `Truncate` в эквивалентные встроенные функции SQL.
- **Строковые функции**: поддерживает преобразование из `Concat`.NET `Contains`, `Count`, `EndsWith`,`IndexOf` `Replace` `Reverse` `StartsWith`,,,, `SubString`, `ToLower`, `ToUpper` `TrimEnd`,, и `TrimStart` в эквивалентные встроенные функции SQL.
- **Функции массивов**: поддерживает преобразование из `Concat`.NET `Contains`, и `Count` в эквивалентные встроенные функции SQL.
- **Функции геопространственных расширений**: поддерживает преобразование из `Distance`методов-заглушек `IsValid`, `IsValidDetailed`, и `Within` в эквивалентные встроенные функции SQL.
...
Синтаксис: `input(.|.SelectMany())(.Select()|.Where())*`. Объединенный запрос может начинаться с необязательного `SelectMany` запроса, за которым `Select` следует `Where` несколько операторов или.
...
`input.SelectMany(x=>x.Q())` Синтаксис — `Q` это оператор `Select`, `SelectMany`или. `Where`
