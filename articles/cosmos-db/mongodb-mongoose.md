...
[Node.js](https://nodejs.org/) версии 0.10.29 или более поздней.
...
- **Хранение каждой объектной модели в отдельной коллекции:** Рекомендуем [создать базу данных со выделенной пропускной стоимостью.](set-throughput.md#set-throughput-on-a-database) Использование этой модели емкости даст вам более высокую эффективность затрат.

    :::image type="content" source="./media/mongodb-mongoose/db-level-throughput.png" alt-text="Учебник Node.js - Скриншот портала Azure, показывающий, как создать базу данных в Data Explorer для учетной записи Azure Cosmos DB для использования с модулем Mongoose Node":::

- **Хранение всех моделей объектов в одной коллекции Cosmos DB:** Если вы предпочитаете хранить все модели в одной коллекции, можно просто создать новую базу данных без выбора опции Просвливания. Использование этой модели емкости создаст каждую коллекцию со своей собственной пропускной способностью для каждой модели объекта.

После создания базы данных вы будете использовать `COSMOSDB_DBNAME` имя в переменной среды ниже.
...
5. Добавьте строку подключения и имя Cosmos DB в файл ```.env```. Замените заполнителей «космос-имя-аккаунт» и «dbname» с вашим собственным именем учетной записи Cosmos и именем базы данных, без символов скобки.
...
## <a name="best-practices-for-using-mongoose-with-cosmos-db"></a>Лучшие практики для использования мангуста с Космос DB

Для каждой создаваемой вами модели Mongoose создает новую коллекцию. Лучше всего это решать с помощью [опции пропускной связи уровня базы данных,](set-throughput.md#set-throughput-on-a-database)которая обсуждалась ранее. Чтобы использовать единую коллекцию, необходимо использовать [mongoose Discriminators.](https://mongoosejs.com/docs/discriminators.html) "Дискриминаторы" — это механизм наследования схемы. Он позволяет иметь несколько моделей с пересекающимися схемами поверх той же базовой коллекции MongoDB.

Вы можете хранить различные модели данных в одной и той же коллекции, а затем использовать предложения фильтра во время запроса, чтобы извлекать только необходимые данные. Давайте пройдемся по каждой из моделей.

### <a name="one-collection-per-object-model"></a>Одна коллекция на объектную модель

В этом разделе описано, как это сделать с помощью API Azure Cosmos DB для MongoDB. Этот метод является нашим рекомендуемым подходом, поскольку он позволяет контролировать затраты и емкость. В результате количество единиц запросов в базе данных не зависит от количества моделей объектов. Это операционная модель по умолчанию для Mongoose, так что, вы можете быть знакомы с этим.
...
1. Теперь при входе на портал Azure вы увидите две коллекции, созданные в Cosmos DB.
    ...
    ![Учебник Node.js - Скриншот портала Azure, показывающий учетную запись Azure Cosmos DB с несколькими именами коллекций - база данных узлов][multiple-coll]
...
1. Теперь, если вернуться на портал Azure, вы заметите, что у вас есть лишь одна коллекция ```alldata``` с данными Family и VacationDestinations.
    ...
    ![Учебник Node.js - Скриншот портала Azure, показывающий учетную запись Azure Cosmos DB с выделенным именем коллекции - база данных узлов][alldata]
