...
Создайте консольное приложение .NET в Visual Studio. Ниже показано, как создать консольное приложение в Visual Studio 2017 г. Эти же действия можно выполнить и в других версиях Visual Studio. Вы можете использовать библиотеку таблиц Azure Cosmos DB в любом приложении .NET, в том числе облачной службе Azure, веб-приложении Azure, а также классическом или мобильном приложении. Для упрощения в этом руководстве мы будем использовать консольное приложение.

1. Выберите **Файл** > **Создать** > **Проект**.

1. Выберите **Установлено** > **Visual C#**  > **Консольное приложение (.NET Core)**.

1. В поле **Имя** введите имя приложения, например **CosmosTableSamples** (вы можете выбрать любое другое имя).
...
Класс [CloudTableClient](https://docs.microsoft.com/dotnet/api/microsoft.windowsazure.storage.table.cloudtableclient?redirectedfrom=MSDN&view=azure-dotnet) позволяет получать таблицы и сущности, хранящиеся в хранилище таблиц. Так как в учетной записи API таблиц Cosmos DB пока нет ни одной таблицы, давайте добавим метод `CreateTableAsync` для их создания в класс **Common.cs**:
