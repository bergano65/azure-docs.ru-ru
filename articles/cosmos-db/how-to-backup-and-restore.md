...
* Если вы случайно удалили или испортили свои данные, следует связаться со [службой поддержки Azure](https://azure.microsoft.com/support/options/) в течение 8 часов, чтобы команда Azure Cosmos DB могла помочь вам восстановить данные из резервных служб.
  ...
  * Если вы случайно удалили базу данных или контейнер, откройте чехол поддержки Sev B или Sev C Azure. 
  * Если вы случайно удалили или испортили некоторые документы в контейнере, откройте дело поддержки Sev A. 

При повреждении данных, а также при изменении либо удалении документов в контейнере нужно **удалить контейнер как можно скорее**. Удалив контейнер, вы можете избежать перезаписи Azure Cosmos DB резервными копиями. Если по какой-либо причине удаление невозможно, следует как можно скорее отправить запрос в службу. Помимо имени учетной записи Azure Cosmos, имен баз данных, имен контейнеров следует указать момент, в который данные могут быть восстановлены. Важно действовать с максимальной точностью, чтобы помочь нам определить самые лучшие резервные копии, доступные на этот момент. Важно также указать время в формате UTC. 
