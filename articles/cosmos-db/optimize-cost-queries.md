---
title: Оптимизация затрат и RU/s для выполнения запросов в Azure Cosmos DB
...
Количество единиц запроса, расходуемых на запрос, зависит от нескольких факторов. Например, количество загруженных/возвращенных элементов Azure Cosmos, количество запросов по отношению к индексу, время компиляции запроса и т.д. Azure Cosmos DB гарантирует, что один и тот же запрос к одним и тем же данным всегда будет иметь одинаковую стоимость в единицах запроса. Профиль запроса, полученный по метрикам выполнения запроса, дает хорошее представление о затратах единиц запроса.  
...
* Узнайте больше о [зарезервированной емкости Azure Cosmos DB](cosmos-db-reserved-capacity.md)
