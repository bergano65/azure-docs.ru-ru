...
* Размер индекса может увеличиваться в следующих случаях:

  * Длительность разбиения секций — пространство индекса освобождается после завершения разбиения секции.
  * При разбиении секции пространство индекса будет временно увеличиваться во время разбиения секции. 
...
- Для запросов с обоими статистическими выражениями (например, COUNT или SUM) и фильтрами также выгодны составные индексы.
...
* Если фильтр запроса имеет несколько свойств, фильтры равенства должны быть первыми свойствами в `ORDER BY` предложении.
