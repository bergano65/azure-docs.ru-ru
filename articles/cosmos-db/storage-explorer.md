...
1. После установки **Azure Storage Explorer**щелкните значок **плагина** слева, как показано на следующем изображении:
...
2. В **поле диалога Azure Sign в** диалоговом окне выберите **вход,** а затем введите учетные данные Azure.

    ![Войти](./media/storage-explorer/sign-in.png)
...
2. Сейчас поддерживаются только API SQL и API службы таблиц. Выберите API, вставьте **строку подключения**, введите **метку для учетной записи** и нажмите кнопку **Далее**. Теперь проверьте сводные данные и нажмите кнопку **Подключиться**, чтобы подключиться к учетной записи Azure Cosmos DB. Для получения информации о получении основной строки соединения, [см.](manage-with-powershell.md#list-keys)
...
1. Нажмите на вашу базу данных, выберите **Создать коллекцию**, а затем предоставить следующую информацию, как **идентификатор коллекции,** **емкость хранения**и т.д. Нажмите **OK,** чтобы закончить. 
...
+ Вы находитесь за *прозрачным прокси,* что означает, что кто-то (например, ваш ИТ-отдел) перехватывает трафик HTTPS, расшифровывает его, а затем шифрует его с помощью сертификата, подписанного самостоятельно.

+ Вы работаете с программным обеспечением, таким как антивирусное программное обеспечение, которое вводит самоподписанные сертификаты TLS/SSL в получаемые сообщения HTTPS.
...
6.  Открыть исследователь хранения, а затем перейти к **отожобья** > **SSL Сертификаты** > **Импортные сертификаты**. Используя средство выбора файлов, найдите, выберите и откройте созданные CER-файлы.
...
- Если вы хотите удалить подключенные ресурсы хранилища SAS, удалите:
  ...
  - **Вам придется повторно ввести все ваши учетные данные,** если вы удалите эти файлы
...
Если ни одно из решений не работает для вас, отправьте электронное письмо в Azure Cosmos DB Dev Tooling Team ()[cosmosdbtooling@microsoft.com](mailto:cosmosdbtooling@microsoft.com)с подробной информацией о проблеме, для устранения проблем.
