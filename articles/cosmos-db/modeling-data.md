...
Внедрение данных отлично подходит для многих случаях, однако существуют сценарии, когда денормализации данных вызовет больше проблем, не стоит потраченных усилий. Что же нам делать теперь?

Связи между сущностями можно задавать не только в реляционных базах данных. В базе данных документов может иметь сведения в одном документе, к которому относится к данным в других документах. Мы не рекомендуем создавать системы, которые лучше подходят для реляционной базы данных в Azure Cosmos DB или любой другой базы данных документов, но простые связи легко реализуются и может быть полезно.
