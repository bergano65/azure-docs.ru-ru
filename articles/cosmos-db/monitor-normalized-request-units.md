...
**Нормализованная метрика потребления единиц** измерения используется для того, чтобы увидеть степень насыщенности диапазонов ключей секций относительно трафика. Azure Cosmos DB равномерно распределяет пропускную способность по всем диапазонам ключей секций. Эта метрика обеспечивает в секунду представление максимального использования пропускной способности для диапазона ключей секций. Используйте эту метрику для вычисления использования единиц запросов/s в диапазоне ключей секций для заданного контейнера. Используя эту метрику, если вы видите высокий процент использования единиц запросов для всех диапазонов ключей разделов в Azure Monitor, необходимо увеличить пропускную способность в соответствии с потребностями рабочей нагрузки. 
