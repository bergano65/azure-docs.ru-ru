...
* CREATE TABLE 
* Создание индекса (без указания имени индекса, а полные замороженные индексы пока не поддерживаются)
