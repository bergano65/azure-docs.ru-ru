---
title: Настройка учетной записи Azure Cosmos DB с периодическим резервным копированием
description: В этой статье описывается, как настроить учетные записи Azure Cosmos DB с периодическим резервным копированием с интервалом резервного копирования. и хранения. Также как обратиться в службу поддержки для восстановления данных.
author: kanshiG
...
ms.locfileid: "99527784"
---
# <a name="configure-azure-cosmos-db-account-with-periodic-backup"></a>Настройка учетной записи Azure Cosmos DB с периодическим резервным копированием
[!INCLUDE[appliesto-all-apis](includes/appliesto-all-apis.md)]

Azure Cosmos DB автоматически копирует все ваши данные с регулярными интервалами. Автоматическое резервное копирование не влияет на производительность или доступность операций баз данных. Для обеспечения устойчивости в случае региональной аварии все резервные копии хранятся отдельно в службе хранилища, а также глобально реплицируются. Azure Cosmos DB обеспечивает высокую избыточность и устойчивость к региональным авариям не только ваших данных, но и их резервных копий. Ниже описано, как Azure Cosmos DB выполняет резервное копирование данных.
...
* Чтобы гарантировать низкую задержку, моментальный снимок резервной копии хранится в хранилище BLOB-объектов Azure в том же регионе, что и текущий регион записи (или **один** из регионов записи, если у вас есть конфигурация записи в нескольких регионах). Чтобы можно было обеспечить устойчивость к региональным авариям, каждый моментальный снимок данных резервной копии в хранилище BLOB-объектов Azure повторно реплицируется в другой регион через геоизбыточное хранилище (GRS). Регион, в который реплицируется резервная копия, зависит от региона источника и региона, который составляет с ним пару. Чтобы узнать больше, ознакомьтесь со [списком геоизбыточных пар регионов Azure](../best-practices-availability-paired-regions.md). У вас нет прямого доступа к этой резервной копии. Команда Azure Cosmos DB восстановит резервную копию при получении запроса на поддержку.
  ...
  :::image type="content" source="./media/configure-periodic-backup-restore/automatic-backup.png" alt-text="Периодическое полное резервное копирование всех Cosmos DB сущностей в хранилище GRS Azure." lightbox="./media/configure-periodic-backup-restore/automatic-backup.png" border="false":::

* Резервное копирование не влияет на производительность или доступность приложения. Azure Cosmos DB выполняет резервное копирование данных в фоновом режиме без использования дополнительной подготовленной пропускной способности (RUs) или влияя на производительность и доступность базы данных.
...
1. Перейдите к учетной записи Azure Cosmos и откройте панель **архивация & восстановление** . Обновите интервал резервного копирования и срок хранения резервных копий по мере необходимости.
   ...
   * **Копии хранящихся данных** . по умолчанию предлагаются две резервные копии данных без оплаты. Дополнительная плата взимается, если требуется более двух копий. Чтобы узнать точную цену за дополнительные копии, см. раздел Использование хранилища на [странице "цены](https://azure.microsoft.com/pricing/details/cosmos-db/) ".

   :::image type="content" source="./media/configure-periodic-backup-restore/configure-backup-interval-retention.png" alt-text="Настройте интервал резервного копирования и период удержания для существующей учетной записи Azure Cosmos." border="true":::
...
:::image type="content" source="./media/configure-periodic-backup-restore/configure-periodic-continuous-backup-policy.png" alt-text="Настройте политику периодической или непрерывной архивации для новых учетных записей Azure Cosmos." border="true":::

## <a name="request-data-restore-from-a-backup"></a><a id="request-restore"></a>Запрос восстановления данных из резервной копии
...
Чтобы восстановить конкретный моментальный снимок резервной копии, Azure Cosmos DB требует, чтобы данные были доступны во время цикла резервного копирования для этого моментального снимка.
...
* При удалении одной или нескольких баз данных необходимо предоставить учетную запись Azure Cosmos, а также имена баз данных Azure Cosmos и указать, существует ли новая база данных с тем же именем.
...
На снимке экрана ниже показано, как создать запрос в службу поддержки для контейнера (коллекции, графа, таблицы) на восстановление данных с помощью портала Azure. Укажите другие сведения, такие как тип данных, назначение восстановления, время удаления данных, чтобы помочь нам определить приоритет запроса.

:::image type="content" source="./media/configure-periodic-backup-restore/backup-support-request-portal.png" alt-text="Создайте запрос на поддержку резервного копирования с помощью портал Azure." border="true":::
...
## <a name="required-permissions-to-change-retention-or-restore-from-the-portal"></a>Необходимые разрешения для изменения срока хранения или восстановления с портала
Участники, которые являются частью роли [космосдббаккупоператор](../role-based-access-control/built-in-roles.md#cosmosbackupoperator), владелец или участник, могут запросить восстановление или изменить срок хранения.

## <a name="understanding-costs-of-extra-backups"></a>Общие сведения о затратах на дополнительные операции резервного копирования
Две резервные копии предоставляются бесплатно, и дополнительные резервные копии начисляются в соответствии с ценами на основе региона для хранилища резервных копий, описанных в разделе [цены на хранилище резервных копий](https://azure.microsoft.com/en-us/pricing/details/cosmos-db/). Например, если срок хранения резервных копий настроен на 240 часов, то 10 дней и интервал резервного копирования — 24 часа. Это подразумевает 10 копий данных резервных копий. Предполагая, что 1 ТБ данных в Западная часть США 2, для хранилища резервных копий в определенном месяце будет 1000 * 0,12 ~ $120. 
...
* Чтобы выполнить запрос на восстановление, обратитесь в службу поддержки Azure, отправьте [билет из портал Azure](https://portal.azure.com/?#blade/Microsoft_Azure_Support/HelpAndSupportBlade).
