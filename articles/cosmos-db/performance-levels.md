...
- [Что делать, если мне нужно более 20 ГБ памяти?](#more-storage-needed)
...
## <a name="what-if-i-need-more-than-20-gb-of-storage"></a>Что делать, если мне нужно более 20 ГБ памяти?

Если у вас есть коллекция с уровнем производительности S1, S2 или S3 или есть единая коллекция разделов, все из которых имеют 20 ГБ памяти, вы можете использовать инструмент миграции данных Azure Cosmos DB, чтобы перенести свои данные в раздельную коллекцию практически с раздельным сбором неограниченное хранение. Сведения о преимуществах секционированной коллекции см. в статье [Секционирование, ключи секции и масштабирование в Azure Cosmos DB](sql-api-partition-data.md). 
...
3.  [Запрос единиц](request-units.md). Узнайте, как потребляется пропускная способность в зависимости от типа операции, например чтение, запись или запрос.
