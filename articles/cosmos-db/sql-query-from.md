...
* Контейнер может иметь псевдоним, например `SELECT f.id FROM Families AS f` или просто `SELECT f.id FROM Families f`. Ниже `f` приведен псевдоним для `Families`. Ключевое слово AS является необязательным для [псевдонима](sql-query-aliasing.md) идентификатора.  
