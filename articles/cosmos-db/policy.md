...
Ниже приведен пример определения политики для проверки того, превышает ли подготовленная пропускная способность базы данных SQL в Azure Cosmos DB максимально допустимое ограничение (400 единиц запросов в секунду). Пользовательское определение политики включает два правила: одно для проверки псевдонима свойства определенного типа, и второе — для определенного свойства этого типа. Оба правила используют псевдонимы.
...
На следующем снимке экрана приведены два примера назначений политики. Одно из этих назначений основано на встроенном определении политики и проверяет, что ресурсы Azure Cosmos DB развертываются только в разрешенных регионах Azure. Другое назначение основано на пользовательском определении политики. Это назначение проверяет, что подготовленная пропускная способность ресурсов Azure Cosmos DB не превышает заданных ограничений.

После развертывания назначений политики результаты оценки отображаются на панели мониторинга соответствия. Обратите внимание, что на это может потребоваться до 30 минут после развертывания назначения политики.

На этом снимке экрана представлен пример результатов оценки соответствия:

- В указанной области есть одна учетная запись Azure Cosmos DB, и из них ноль соответствуют назначению политики, которое проверяет регионы развертывания ресурсов.
- В указанной области есть два ресурса баз данных или коллекций Azure Cosmos DB, и из них один соответствует назначению политики, которое проверяет ограничение по подготовленной пропускной способности.

:::image type="content" source="./media/policy/compliance.png" alt-text="Поиск встроенных определений политик для Azure Cosmos DB":::

Чтобы исправить несоответствующие политике ресурсы, воспользуйтесь [этой статьей](../governance/policy/how-to/remediate-resources.md).

## <a name="next-steps"></a>Next Steps

- [Изучите примеры пользовательских определений политик для Azure Cosmos DB](https://github.com/Azure/azure-policy/tree/master/samples/CosmosDB)
