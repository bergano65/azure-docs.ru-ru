...
* **Режим соединения: Используйте прямой режим**
<a id="direct-connection"></a>
    
    То, как клиент подключается к DB Azure Cosmos, имеет важные последствия для производительности, особенно с точки зрения задержки клиентов. *ConnectionMode* — это ключевая настройка конфигурации, доступная для настройки клиента *ConnectionPolicy.* Для Async Java SDK, два доступных ConnectionModes являются:  
      
    * [Gateway](/java/api/com.microsoft.azure.cosmosdb.connectionmode) ("Шлюз", используется по умолчанию);  
    * [Прямой](/java/api/com.microsoft.azure.cosmosdb.connectionmode)

    Режим шлюза поддерживается на всех платформах SDK и является настроенным вариантом по умолчанию. Если приложения работают в корпоративной сети со строгими ограничениями брандмауэра, режим Gateway является лучшим выбором, поскольку он использует стандартный порт HTTPS и одну конечную точку. Но режим шлюза не обеспечивает максимальную производительность, поскольку задействует дополнительный сетевой узел при каждой операции чтения или записи данных в Azure Cosmos DB. Из-за этого режим Direct обеспечивает лучшую производительность из-за меньшего количества сетевого хмеля.

    *ConnectionMode* настроен во время построения экземпляра *DocumentClient* с параметром *ConnectionPolicy.*
...
* **Collocate клиентов в том же регионе Azure для производительности**<a id="same-region"></a>
...
* **Настройка ConnectionPolicy**
    ...
    По умолчанию запросы Direct mode Cosmos DB выполняются по TCP при использовании Async Java SDK. Внутри SDK использует специальную архитектуру режима Direct для динамического управления сетевыми ресурсами и получения наилучшей производительности.

    В Async Java SDK режим Direct является лучшим выбором для повышения производительности базы данных с наибольшим количеством рабочих нагрузок. 

    * ***Обзор прямого режима***

        ![Иллюстрация архитектуры прямого режима](./media/performance-tips-async-java/rntbdtransportclient.png)

        Архитектура клиентской стороны, используемая в режиме Direct, обеспечивает предсказуемое использование сети и мультиплексированный доступ к репликам Azure Cosmos DB. На приведенной выше диаграмме показано, как режим Direct маршрутирует запросы клиентов на реплики в бэкэнде Cosmos DB. Архитектура прямого режима выделяет до 10 **каналов** на стороне клиента на реплику DB. Канал — это соединение TCP, которому предшествует буфер запроса, глубина которого составляет 30 запросов. Каналы, принадлежащие к реплике, динамически распределяются по мере необходимости **в endpoint службы**реплики. Когда пользователь выдает запрос в режиме Direct, **TransportClient** направляет запрос в соответствующую конечную точку обслуживания на основе ключа раздела. Буферы **очереди запросов** запросы перед конечной точкой службы.

    * ***Параметры конфигурации ConnectionPolicy для прямого режима***

        В качестве первого шага используйте следующие рекомендуемые настройки конфигурации ниже. Пожалуйста, свяжитесь с [командой Azure Cosmos DB,](mailto:CosmosDBPerformanceSupport@service.microsoft.com) если у вас возникли проблемы по данной теме.

        Если вы используете Azure Cosmos DB в качестве баз данных эталонов (т.е. база данных используется для многих операций чтения по точкам и нескольких операций записи), может быть приемлемым установить *idleEndpointTimeout* до 0 (т.е. нет тайм-аута).
        ...
        | maxBufferЕмкость          | 8388608    |
        | maxChannelsPerEndpoint     | 10         |
        | maxRequestsPerChannel      | 30         |
        | receiveHangDetectionTime   | "PT1M5S"   |
        | запросExpiryInterval      | "PT5S"     |
        | requestTimeout             | "PT1M"     |
        | запросTimerРезолюция     | "PT0.5S"   |
        | sendHangDetectionTime      | "PT10S"    |
        | выключениеTimeout            | "PT15S"    |

    * ***Советы по программированию для прямого режима***

        Просмотрите статью Azure Cosmos DB [Async Java SDK Troubleshooting](troubleshoot-java-async-sdk.md) в качестве основы для решения любых проблем Async Java SDK.

        Некоторые важные советы программирования при использовании прямого режима:

        + **Используйте многопоточность в приложении для эффективной передачи данных TCP** - После запроса приложение должно подписаться на получение данных по другому потоку. Невыполнение этого решения заставляет непреднамеренную операцию «полудуплекса», и последующие запросы блокируются в ожидании ответа предыдущего запроса.

        + **Выполняйте вычислительные нагрузки на выделенном потоке** - По теме, что было бы по предыдущему совету, операции, такие как комплексная обработка данных, лучше всего помещаются в отдельный поток. Запрос, который вытягивает данные из другого хранилища данных (например, если поток использует магазины данных Azure Cosmos DB и Spark одновременно), может испытывать повышенную задержку, и рекомендуется породить дополнительный поток, который ждет ответа от другого хранилище данных.

            + Базовая сеть IO в Async Java SDK управляется Netty, см. эти [советы для избежания шаблонов кодирования, которые блокируют потоки Netty IO.](troubleshoot-java-async-sdk.md#invalid-coding-pattern-blocking-netty-io-thread)

        + **Моделирование данных** - Azure Cosmos DB SLA предполагает размер документа менее 1КБ. Оптимизация модели данных и программирования в пользу меньшего размера документа, как правило, приводит к снижению задержки. Если вам потребуется хранение и поиск документов больше 1кБ, рекомендуемый подход заключается в том, чтобы документы ссылались на данные в Хранилище Azure Blob.
...
* **Настройка параллельных запросов для секционированных коллекций**
    ...
    * ***Настройка набораMaxBufferedItemCount\:***
...
* **Использование надлежащего планировщика (избегайте перехвата потоков цикла обработки событий ввода-вывода netty)**
    ...
    Следует использовать подходящий планировщик RxJava в зависимости от типа рабочей нагрузки. Читайте [``Schedulers``](http://reactivex.io/RxJava/1.x/javadoc/rx/schedulers/Schedulers.html)здесь .
...
* **Отключить лесозаготовки нетти**

    Регистрация библиотек Netty болтлива и должна быть выключена (подавления знака в конфигурации может быть недостаточно), чтобы избежать дополнительных затрат на процессор. Если вы не используете режим отладки, полностью отключите ведение журналов netty. Поэтому, если вы используете log4j для сокращения дополнительных затрат на ЦП в связи с ``org.apache.log4j.Category.callAppenders()`` из netty, добавьте следующую строку в базу кода:

    ```java
    org.apache.log4j.Logger.getLogger("io.netty").setLevel(org.apache.log4j.Level.OFF);
    ```

 * **Os Открытый лимит ресурсов файлов**
 
    Некоторые системы Linux (например, Red Hat) имеют верхний предел на количество открытых файлов и, таким образом, общее количество соединений. Чтобы узнать текущие ограничения, выполните следующую команду:
...
* **Используйте нативную реализацию SSL для нетти**

    Netty может использовать OpenSSL непосредственно для стека реализации SSL для достижения более высокой производительности. Если такая конфигурация отсутствует, netty вернется к реализации SSL по умолчанию в Java.
...
* **Измерение и настройка расхода единиц запроса/повторного использования**
    ...
    Чтобы оценить расходы на любую операцию (создание, обновление или удаление), проверьте значение заголовка [x-ms-request-charge](/rest/api/cosmos-db/common-cosmosdb-rest-request-headers). Это значение содержит число единиц запроса, потребляемых соответствующей операцией. Вы также можете посмотреть на эквивалентную\<свойство RequestCharge в> ResourceResponse T или FeedResponse\<T>.
