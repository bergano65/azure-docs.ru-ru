...
* **Настройка ConnectionPolicy**
  ...
  * ***Параметры конфигурации ConnectionPolicy для прямого режима** _
    ...
    Если в качестве эталонной базы данных используется Azure Cosmos DB (то есть база данных используется для многих операций чтения точки и несколько операций записи), может быть приемлемым задание _idleEndpointTimeout * в 0 (то есть без времени ожидания).
...
* ***Советы по программированию для прямого режима** _
  ...
  _ **Использование многопоточности в приложении для эффективного приема данных TCP** . После выполнения запроса приложение должно подписываться на получение данных в другом потоке. Если этого не сделать, это вызовет непредвиденную операцию "полудуплекс" и последующие запросы блокируются в ожидании ответа предыдущего запроса.
...
* **Настройка параллельных запросов для секционированных коллекций**
  ...
  * ***Настройка setMaxDegreeOfParallelism \:** _
  ...
  _ * **Настройка setMaxBufferedItemCount \:** _
...
_ **Реализовать отходы через getRetryAfterInMilliseconds интервалы**
...
* **Увеличение скорости выполнения операций записи посредством исключения неиспользуемых путей из индексирования**
    ...
    Дополнительные сведения см. в статье [Политики индексации Azure Cosmos DB](/azure/cosmos-db/index-policy).
