...
* **Настройка ConnectionPolicy**
    ...
    * ***Советы по программированию для прямого режима***
        ...
        + **Выполняйте вычислительные нагрузки на выделенном потоке** - По теме, что было бы по предыдущему совету, операции, такие как комплексная обработка данных, лучше всего помещаются в отдельный поток. Запрос, который вытягивает данные из другого хранилища данных (например, если поток использует данные Azure Cosmos DB и Spark одновременно), может испытывать повышенную задержку, и рекомендуется породить дополнительный поток, который ждет ответа от другого хранилища данных.
...
* **Используйте нативную реализацию TLS/SSL для нетти**

    Netty может использовать OpenSSL непосредственно для стека реализации TLS для достижения более высокой производительности. При отсутствии этой конфигурации netty вернется к реализации TLS по умолчанию Java.
