...
|[Функции даты и времени](#date-time-functions)|Функции даты и времени позволяют получить текущую дату и время в формате UTC в двух формах. числовая метка времени, значение которой является эпохой UNIX в миллисекундах или в виде строки, которая соответствует формату ISO 8601.|
...
|---------|----------|
| Математические функции | ABS, CEILING, EXP, ЭТАЖ, ЖУРНАЛ, LOG10, ПИТАНИЕ, КРУГЛЫЙ, ЗНАК, SQRT, КВАДРАТНЫЙ, TRUNC, ACOS, ASIN, ATAN, ATN2, COS, COT, ГРАДУСЫ, PI, РАДИАНЫ, RAND, SIN, TAN |
| Функции проверки типов | IS_ARRAY, IS_BOOL, IS_NULL, IS_NUMBER, IS_OBJECT, IS_STRING, IS_DEFINED, IS_PRIMITIVE |
...
| Функции даты и времени | ЖЕТКУРРЕНТДАТЕТИМЕ, ЖЕТКУРРЕНТТИМЕСТАМП,  |
...
## <a id="type-checking-functions"></a>Функции проверки типов

Функции проверки типов позволяют проверить тип выражения в SQL-запросе. Функции проверки типов можно использовать для определения типов свойств в элементах на лету, когда они являются переменными или неизвестными. Ниже приведена таблица поддерживаемых встроенных функций проверки типов:
...
|[RTRIM](#bk_rtrim)|[STARTSWITH](#bk_startswith)|[стрингтоаррай](#bk_stringtoarray)|
|[стрингтобулеан](#bk_stringtoboolean)|[стрингтонулл](#bk_stringtonull)|[стрингтонумбер](#bk_stringtonumber)|
|[стрингтубжект](#bk_stringtoobject)|[SUBSTRING](#bk_substring)|[ToString](#bk_tostring)|
...
```  
  ...
  ####  <a name="bk_stringtoarray"></a>стрингтоаррай  
...
- `expr`  
   ...
   Любое допустимое скалярное выражение, вычисляемое как выражение массива JSON. Обратите внимание, что вложенные строковые значения должны быть написаны с помощью двойных кавычек. Дополнительные сведения о формате JSON см. в разделе [JSON.org](https://json.org/)
  ...
  **Примеры**  
  ...
  В следующем примере показано, как Стрингтоаррай ведет себя между различными типами. 
...
####  <a name="bk_stringtoboolean"></a>стрингтобулеан  
...
- `expr`  
   ...
   Любое допустимое скалярное выражение, вычисляемое как логическое выражение.  
  ...
  **Примеры**  
  ...
  В следующем примере показано, как Стрингтобулеан ведет себя между различными типами. 
...
####  <a name="bk_stringtonull"></a>стрингтонулл  
...
- `expr`  
   ...
   Любое допустимое скалярное выражение, вычисляемое как выражение null.
  ...
  **Примеры**  
  ...
  В следующем примере показано, как Стрингтонулл ведет себя между различными типами. 
...
####  <a name="bk_stringtonumber"></a>стрингтонумбер  
...
- `expr`  
   ...
   Любое допустимое скалярное выражение, вычисляемое как выражение числа JSON. Числа в JSON должны быть целым числом или плавающей запятой. Дополнительные сведения о формате JSON см. в разделе [JSON.org](https://json.org/)  
  ...
  **Примеры**  
  ...
  В следующем примере показано, как Стрингтонумбер ведет себя между различными типами. 
...
####  <a name="bk_stringtoobject"></a>стрингтубжект  
...
- `expr`  
   ...
   Любое допустимое скалярное выражение, вычисляемое как выражение объекта JSON. Обратите внимание, что вложенные строковые значения должны быть написаны с помощью двойных кавычек. Дополнительные сведения о формате JSON см. в разделе [JSON.org](https://json.org/)  
  ...
  **Примеры**  
  ...
  В следующем примере показано, как Стрингтубжект ведет себя между различными типами. 
...
```  
## <a id="date-time-functions"></a>Функция даты и времени
...
|||
|-|-|
|[ЖЕТКУРРЕНТДАТЕТИМЕ](#bk_get_current_date_time)|[ЖЕТКУРРЕНТТИМЕСТАМП](#bk_get_current_timestamp)||

####  <a name="bk_get_current_date_time"></a>ЖЕТКУРРЕНТДАТЕТИМЕ
 Возвращает текущую дату и время в формате UTC как строку ISO 8601.
...
```
  ...
  **Типы возвращаемого значения**
  ...
  Возвращает текущее значение строки ISO 8601 для даты и времени в формате UTC. 

  Это значение указывается в формате гггг-мм-DDThh: mm: SS. СССЗ, где:
...
####  <a name="bk_get_current_timestamp"></a>ЖЕТКУРРЕНТТИМЕСТАМП
