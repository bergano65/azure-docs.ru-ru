...
## <a name="step-2-connect-to-your-azure-cosmos-database"></a><a id="connect"></a>Шаг 2: Подключитесь к базе данных Azure Cosmos
...
1. В окне **Azure Cosmos DB ODBC Driver SDN Setup** (Настройка имени источника данных драйвера ODBC для Azure Cosmos DB) укажите следующие сведения: 
    ...
    ![Страница ключей Azure Cosmos DB](./media/odbc-driver/odbc-cosmos-account-keys.png)
...
1.  Нажмите кнопку **Дополнительные параметры** и задайте следующие значения:
    *  **REST API Версия**: Выберите [версию REST API](https://docs.microsoft.com/rest/api/cosmos-db/) для ваших операций. Дефолт 2015-12-16. Если у вас есть контейнеры с [большими ключами раздела](large-partition-keys.md) и требуется версия REST API 2018-12-31:
        - Тип в **2018-12-31** для версии REST API
        - В меню **«Пуск»** введите «regedit», чтобы найти и открыть приложение **редактора реестра.**
        - В редакторе реестра перейдите по пути: **КомпьютерHKEY_LOCAL_MACHINE-ПРОГРАММА,ODBC-ODBC. ИНИ**
        - Создайте новый подключ с тем же названием, что и Ваш DSN, например, "Contoso Account ODBC DSN".
        - Перейдите к подключке "Contoso Account ODBC DSN".
        - Нажмите правой кнопкой мыши, чтобы добавить новое значение **строки:**
            - Название значения: **IgnoreSessionToken**
            - Данные значений: **1**
            ![настройки редактора реестра](./media/odbc-driver/cosmos-odbc-edit-registry.png)
    ...
    - **Schema File** (Файл схемы). Здесь можно выбрать один из нескольких вариантов.
        - По умолчанию, оставляя эту запись как (пустая), драйвер сканирует первую страницу данных для всех контейнеров, чтобы определить схему каждого контейнера. Это называется картированиеконтейнеров. Так как файл схемы не определен, драйверу приходится выполнять сканирование для каждого сеанса. При этом запуск приложения с использованием имени источника данных может стать дольше. Мы рекомендуем связывать файл схемы для имени источника данных.
        - Если у вас уже есть файл схемы (возможно, тот, который вы создали с помощью редактора Schema), вы можете нажать **Просмотр,** перейти к файлу, нажмите **Сохранить**, а затем нажмите **OK**.
...
## <a name="step-3-create-a-schema-definition-using-the-container-mapping-method"></a><a id="#container-mapping"></a>Шаг 3: Создание определения схемы с помощью метода картирования контейнеров

Существует два типа методов выборки, которые можно использовать: **картирование контейнеров** или **разгранители таблицы.** Сессия выборки может использовать оба метода отбора проб, но каждый контейнер может использовать только конкретный метод выборки. Приведенные ниже шаги создают схему для данных в одном или нескольких контейнерах с использованием метода картирования контейнеров. Этот метод выборки извлекает данные на странице контейнера для определения структуры данных. Он перекладывает контейнер на стол на стороне ODBC. Этот метод выборки является эффективным и быстрым, когда данные в контейнере однородны. Если контейнер содержит неоднородный тип данных, мы рекомендуем использовать [метод картирования столовых делимитеров,](#table-mapping) поскольку он обеспечивает более надежный метод выборки для определения структур данных в контейнере. 

1. После выполнения шагов 1-4 в [подключении к базе данных Azure Cosmos](#connect)щелкните **редактор Schema** в окне **настройки Драйвера DSBC.**
...
1. В окне **Редактор схем** нажмите кнопку **Создать**.
    Окно **Generate Schema** отображает все контейнеры в учетной записи Azure Cosmos DB. 

1. Выберите один или несколько контейнеров для образца, а затем нажмите **Образец.** 
...
1. После того как вы закончили определение схемы, нажмите **Файл** | **Сохранить**, перейти к каталогу, чтобы сохранить схему, а затем нажмите **Сохранить**.

1. Чтобы использовать эту схему с DSN, откройте **окно настройки Azure Cosmos DB ODBC Driver DSN** (через администратор апогея ODBC Data Source Administrator), нажмите **Расширенные параметры,** а затем в поле **Schema File** перейдите к сохраненной схеме. При сохранении файла схемы в существующем имени источника данных изменяется DSN-соединение в соответствии с данными и структурой, определенными схемой.
...
Существует два типа методов выборки, которые можно использовать: **картирование контейнеров** или **разгранители таблицы.** Сессия выборки может использовать оба метода отбора проб, но каждый контейнер может использовать только конкретный метод выборки. 

Следующие шаги создают схему для данных в одном или нескольких контейнерах с помощью метода картирования **столовых делимитеров.** Мы рекомендуем использовать этот метод выборки, когда контейнеры содержат неоднородный тип данных. С помощью этого метода можно ограничить выборку набором атрибутов и соответствующими значениями. Например, если документ содержит свойство "Тип", можно ограничить выборку значениями этого свойства. В результате выборки вы получите набор таблиц для каждого значения выбранного типа. Например, если выбрать значение "Автомобиль", будет создана таблица со сведениями об автомобилях, а если выбрать значение "Самолет", будет создана таблица со сведениями о самолетах.
...
1. Выберите контейнер на вкладке **«Представление примера»** в столбце **определения карт для** контейнера, нажмите **«Отображвите».** Затем в окне **Mapping Definition** (Определение сопоставления) выберите метод **Table Delimiters** (Таблицы-разделители). После этого выполните описанные ниже действия.
...
1. После заполнения определений отображения для контейнеров, которые вы хотите отобразить, в окне **редактора Schema** щелкните **Образец**.
     Вы можете изменить имя, тип и длину SQL (если применимо), масштаб (если применимо), точность (если применимо) и параметр, допускающий значение null, для каждого столбца.
    - Для параметра **Скрыть столбец** можно задать значение **true**, если необходимо исключить этот столбец из результатов запроса. Столбцы, для параметра "Скрыть столбец" которых задано значение true, не возвращаются для выбора и проекции, несмотря на то, что они по-прежнему являются частью схемы. Например, можно скрыть все обязательные системные свойства Azure Cosmos DB, начинающиеся со знака `_`.
    - Столбец **Идентификатор** — это единственное поле, которое нельзя скрыть, так как оно используется в качестве первичного ключа в нормализованной схеме. 
...
Чтобы запросить связанную базу данных, введите запрос SSMS. В этом примере запрос выбирает из таблицы `customers`в контейнере с именем:
...
Чтобы создать представление для данных в окне **редактора Schema** в столбце **View Definitions,** нажмите **Добавить** строку контейнера для выборки. 
