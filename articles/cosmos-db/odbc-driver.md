...
1.  Нажмите кнопку **Дополнительные параметры** и задайте следующие значения:
    *  **Версия REST API**: Выберите [версию REST API](https://docs.microsoft.com/rest/api/cosmos-db/) для операций. Значение по умолчанию 2015-12-16. Если у вас есть контейнеры с [большими ключами разделов](large-partition-keys.md) и требуется REST API версии 2018-12-31:
        - Введите **2018-12-31** для версии REST API
        - В меню " **Пуск** " введите "regedit", чтобы найти и открыть приложение **редактора реестра** .
        - В редакторе реестра перейдите по пути: **Computer\HKEY_LOCAL_MACHINE\SOFTWARE\ODBC\ODBC. ССЫЛКИ**
        - Создайте новый подраздел с тем же именем, что и у имени DSN, например "Учетная запись ODBC DSN в Contoso".
        ...
        - Щелкните правой кнопкой мыши, чтобы добавить новое **строковое** значение:
            ...
            - Данные значения: **1**
             @ No__t-2Registry параметры редактора @ no__t-3
