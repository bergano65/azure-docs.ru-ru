...
1. Откройте приложение **Администратор источников данных ODBC** на своем компьютере. Это можно сделать, введя в поле поиска Windows **Источники данных ODBC**. 
    ...
    :::image type="content" source="./media/odbc-driver/odbc-driver.png" alt-text="Администратор источников данных ODBC для Azure Cosmos DB":::

## <a name="step-2-connect-to-your-azure-cosmos-database"></a><a id="connect"></a>Шаг 2. подключение к базе данных Cosmos Azure
...
1. В окне **Azure Cosmos DB ODBC Driver SDN Setup** (Настройка имени источника данных драйвера ODBC для Azure Cosmos DB) укажите следующие сведения: 
    ...
    :::image type="content" source="./media/odbc-driver/odbc-driver-dsn-setup.png" alt-text="Окно настройки имени источника данных драйвера ODBC для Azure Cosmos DB":::
    ...
    :::image type="content" source="./media/odbc-driver/odbc-cosmos-account-keys.png" alt-text="Страница ключей Azure Cosmos DB":::
...
1.  Нажмите кнопку **Дополнительные параметры** и задайте следующие значения:
    *  **Версия REST API** : выберите [версию REST API](/rest/api/cosmos-db/) для своих операций. Значение по умолчанию 2015-12-16. Если у вас есть контейнеры с [большими ключами разделов](large-partition-keys.md) и требуется REST API версии 2018-12-31:
        - Введите **2018-12-31** для версии REST API
        - В меню " **Пуск** " введите "regedit", чтобы найти и открыть приложение **редактора реестра** .
        - В редакторе реестра перейдите по пути: **Computer\HKEY_LOCAL_MACHINE\SOFTWARE\ODBC\ODBC.INI**
        - Создайте новый подраздел с тем же именем, что и у имени DSN, например "Contoso Account ODBC DSN".
        - Перейдите к подразделу "имя пользователя ODBC для учетной записи Contoso".
        - Щелкните правой кнопкой мыши, чтобы добавить новое **строковое** значение:
            - Имя значения: **игноресессионтокен**
            - Данные значения: **1** 
             :::image type="content" source="./media/odbc-driver/cosmos-odbc-edit-registry.png" alt-text="Параметры редактора реестра":::
    ...
    - **Schema File** (Файл схемы). Здесь можно выбрать один из нескольких вариантов.
        - По умолчанию, если оставить эту запись как пустую, драйвер сканирует первую страницу данных для всех контейнеров, чтобы определить схему каждого контейнера. Это называется сопоставлением контейнеров. Так как файл схемы не определен, драйверу приходится выполнять сканирование для каждого сеанса. При этом запуск приложения с использованием имени источника данных может стать дольше. Мы рекомендуем связывать файл схемы для имени источника данных.
        - Если у вас уже есть файл схемы (возможно, созданный с помощью редактора схемы), можно нажать кнопку **Обзор** , перейти к файлу, нажать кнопку **сохранить** , а затем нажать кнопку **ОК**.
...
Существует два типа методов выборки, которые можно использовать: **сопоставление контейнеров** или **табличные разделители**. В сеансе выборки могут использоваться оба метода выборки, но каждый контейнер может использовать только конкретный метод выборки. Приведенные ниже действия создают схему для данных в одном или нескольких контейнерах с помощью метода сопоставления контейнеров. Этот метод выборки извлекает данные на странице контейнера для определения структуры данных. Он переставит контейнер в таблицу на стороне ODBC. Этот метод выборки эффективен и ускоряется, когда данные в контейнере являются однородными. Если контейнер содержит разнородный тип данных, рекомендуется использовать [метод сопоставления таблиц-разделителей](#table-mapping) , так как он предоставляет более надежный метод выборки для определения структур данных в контейнере. 
...
1. Выберите один или несколько контейнеров для выборки, а затем щелкните **Пример**. 
...
1. Завершив определение схемы, щелкните **файл**  |  **сохранить** , перейдите в каталог, чтобы сохранить схему, а затем нажмите кнопку **сохранить**.

1. Чтобы использовать эту схему с именем DSN, откройте **окно установки Azure Cosmos DB ODBC Driver DSN** (с помощью администратора источников данных ODBC), щелкните **Дополнительные параметры** , а затем в поле **файл схемы** перейдите к сохраненной схеме. При сохранении файла схемы в существующем имени источника данных изменяется DSN-соединение в соответствии с данными и структурой, определенными схемой.

## <a name="step-4-create-a-schema-definition-using-the-table-delimiters-mapping-method"></a><a id="table-mapping"></a>Шаг 4. Создание определения схемы с использованием метода таблиц-разделителей

Существует два типа методов выборки, которые можно использовать: **сопоставление контейнеров** или **табличные разделители**. В сеансе выборки могут использоваться оба метода выборки, но каждый контейнер может использовать только конкретный метод выборки. 

Следующие шаги создают схему для данных в одном или нескольких контейнерах с помощью метода сопоставления **таблиц и разделителей** . Рекомендуется использовать этот метод выборки, если контейнеры содержат разнородный тип данных. С помощью этого метода можно ограничить выборку набором атрибутов и соответствующими значениями. Например, если документ содержит свойство "Тип", можно ограничить выборку значениями этого свойства. В результате выборки вы получите набор таблиц для каждого значения выбранного типа. Например, если выбрать значение "Автомобиль", будет создана таблица со сведениями об автомобилях, а если выбрать значение "Самолет", будет создана таблица со сведениями о самолетах.
...
1. Выберите контейнер на вкладке " **Образец представления** " в столбце **Определение сопоставления** для контейнера и нажмите кнопку **изменить**. Затем в окне **Mapping Definition** (Определение сопоставления) выберите метод **Table Delimiters** (Таблицы-разделители). После этого выполните описанные ниже действия.
    ...
    b. Если требуется ограничить выборку только конкретными значениями введенного атрибута, выберите атрибут в списке, введите значение в поле **Значение** (например "Сиэтл") и нажмите клавишу ВВОД. Вы можете добавить еще несколько значений для атрибутов. При вводе значения убедитесь, что выбран правильный атрибут.
...
:::image type="content" source="./media/odbc-driver/odbc-driver-linked-server-ssms.png" alt-text="Связанный сервер в среде SSMS":::

### <a name="query-linked-database"></a>Запрос к связанной базе данных

Чтобы запросить связанную базу данных, введите запрос SSMS. В этом примере запрос выбирает из таблицы в контейнере с именем `customers` :

```sql
SELECT * FROM OPENQUERY(DEMOCOSMOS, 'SELECT *  FROM [customers].[customers]')
```

выполнение запроса. Результат должен выглядеть следующим образом:
...
Чтобы создать представление для данных, в окне **Редактор схемы** в столбце **определения представления** нажмите кнопку **добавить** в строке контейнера для выборки. 

:::image type="content" source="./media/odbc-driver/odbc-driver-create-view.png" alt-text="Создание представления данных":::
...
1. В окне **Изменить представление** введите запрос Azure Cosmos DB. Это должен быть [SQL-запрос Azure Cosmos DB](./sql-query-getting-started.md), например `SELECT c.City, c.EmployeeName, c.Level, c.Age, c.Manager FROM c WHERE c.City = "Seattle"`. Затем нажмите кнопку **ОК**.
    ...
    :::image type="content" source="./media/odbc-driver/odbc-driver-create-view-2.png" alt-text="Добавление запроса при создании представления":::
...
1. Щелкните **Получить данные**.
    ...
    :::image type="content" source="./media/odbc-driver/odbc-driver-power-bi-get-data.png" alt-text="Получение данных в Power BI Desktop":::

1. В окне **Получить данные** щелкните **Другие** | **ODBC** | **Подключиться**.

    :::image type="content" source="./media/odbc-driver/odbc-driver-power-bi-get-data-2.png" alt-text="Выбор источника данных ODBC в окне получения данных Power BI":::
...
1. В окне **Из ODBC** выберите созданное имя источника данных и нажмите кнопку **ОК**. Поля на вкладке **Дополнительные параметры** можно не заполнять.
   ...
   :::image type="content" source="./media/odbc-driver/odbc-driver-power-bi-get-data-3.png" alt-text="Выбор имени источника данных (DSN) в окне получения данных Power BI":::
...
1. В окне **Навигатор** на панели слева разверните базу данных, схему и выберите таблицу. В область результатов данные добавляются по созданной схеме.
    ...
    :::image type="content" source="./media/odbc-driver/odbc-driver-power-bi-get-data-4.png" alt-text="Выбор таблицы в окне получения данных Power BI":::

1. Чтобы визуализировать данные в Power BI Desktop, установите флажок рядом с именем таблицы и щелкните **Загрузить**.

1. :::image type="icon" source="./media/odbc-driver/odbc-driver-data-tab.png":::Чтобы подтвердить, что данные были импортированы, в Power BI Desktop слева выберите вкладку данные. 

1. Теперь вы можете создавать визуальные элементы с помощью Power BI, щелкнув вкладку Отчет :::image type="icon" source="./media/odbc-driver/odbc-driver-report-tab.png"::: , щелкнув **создать визуальный** элемент, а затем настроив плитку. Дополнительные сведения о создании визуализаций в Power BI Desktop см. в статье [Типы визуализаций в Power BI](https://powerbi.microsoft.com/documentation/powerbi-service-visualization-types-for-reports-and-q-and-a/). 
