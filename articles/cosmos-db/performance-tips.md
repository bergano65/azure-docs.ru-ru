...
1. **Политика подключения: использование режима прямого подключения**
   ...
   * режим прямого подключения.
     ...
     * HTTPS
     ...
     |Напрямую    |     TCP    |  Пакет SDK для .NET    | Порты в диапазоне 10 000–20 000 |
...
1. **Оптимизация производительности посредством настройки размера страницы для запросов и каналов чтения**
   ...
   > [!NOTE] 
   > Свойство maxItemCount не должен использоваться только в целях разбиение на страницы. Это основной использования его, чтобы повысить производительность запросов за счет снижения максимальное количество элементов, возвращаемых на одной странице.  

   Можно также задать размер страницы, с помощью доступных пакетов SDK Azure Cosmos DB. [MaxItemCount](/dotnet/api/microsoft.azure.documents.client.feedoptions.maxitemcount?view=azure-dotnet) свойство в FeedOptions позволяет задать максимальное число элементов, возвращаемых в операции enmuration. Когда `maxItemCount` имеет значение-1, пакет SDK автоматически определяет наиболее оптимальное значение в зависимости от размера документа. Пример:
    
   ```csharp
    IQueryable<dynamic> authorResults = client.CreateDocumentQuery(documentCollection.SelfLink, "SELECT p.Author FROM Pages p WHERE p.Title = 'About Seattle'", new FeedOptions { MaxItemCount = 1000 });
   ```
    
   При выполнении запроса, полученные данные отправляются в пределах пакета TCP. Если указать слишком низкое значение `maxItemCount`, количество поездок, необходимые для отправки данных в пределах пакета TCP высоки, который влияет на производительность. Таким образом, если вы не уверены, какое значение задать для `maxItemCount` свойство, лучше всего равным -1 и позволить пакету SDK выберите значение по умолчанию. 
