...
1. **Политика подключения: использование режима прямого подключения**
   ...
   * режим прямого подключения.
     ...
     В режиме прямого подключения поддерживается обмен данными через протоколы TCP и HTTPS. Если вы используете последнюю версию пакета SDK для .NET, то в .NET Standard 2,0 и .NET Framework поддерживается режим прямого подключения. Если вы используете режим прямого подключения, вам доступны два варианта протоколов:
