...
5. **Настройка параллельных запросов для секционированных коллекций**
    ...
    (a) ***степень\:***  параллелизма параллельных запросов работает путем параллельного запроса нескольких секций. Однако данные из отдельных секций извлекаться последовательно по отношению к запросу. Установка в пакете [SDK версии 2](sql-api-sdk-dotnet.md) или `MaxConcurrency` в [пакете SDK v3](sql-api-sdk-dotnet-standard.md) значения, равного количеству секций, имеет максимальную шансы на достижение наиболее возможного запроса, при условии, что все остальные системные условия остаются прежними. `MaxDegreeOfParallelism` Если число секций неизвестно, можно задать для степени параллелизма большое число, а система выберет минимальное (число секций, предоставленных пользователем входных данных) в качестве степени параллелизма.
