...
1.  **Для рабочих нагрузок, интенсивно использующих запросы, используйте Windows 64-bit вместо обработки узлов Linux или Windows 32**

    Для повышения производительности рекомендуется использовать Windows 64-разрядную обработку узлов. Пакет SDK для SQL содержит собственную ServiceInterop. dll для локального анализа и оптимизации запросов, а также поддерживается только на платформе Windows x64. Для Linux и других неподдерживаемых платформ, в которых недоступна библиотека ServiceInterop. dll, она выполняет дополнительный сетевой вызов шлюза для получения оптимизированного запроса. В следующих типах приложений по умолчанию используется 32-разрядный хост-процесс. Чтобы изменить его на 64-разрядный, выполните описанные ниже действия в зависимости от типа приложения.

    - Для исполняемых приложений это можно сделать, задав для [целевой платформы платформу](https://docs.microsoft.com/visualstudio/ide/how-to-configure-projects-to-target-platforms?view=vs-2019) **x64** в окне **Свойства проекта** на вкладке **Сборка** .
    ...
    > [!NOTE] 
    > Visual Studio по умолчанию применяет новые проекты к любому ЦП. Рекомендуется задать для проекта значение x64, чтобы избежать переключения на x86. Любой проект ЦП может легко переключиться на x86, если добавляется любая зависимость, которая является только x86.<br/>
    > Файл ServiceInterop. dll должен находиться в той же папке, что и DLL-файл пакета SDK, из которого выполняется. Это необходимо сделать только для пользователей, которые вручную скопировав библиотеки DLL или имеют пользовательские системы сборки и развертывания.
    
2. **Включить сборку мусора на стороне сервера (GC)**
...
3. **Развертывание рабочей нагрузки клиента**
...
1. **Политика подключения: использование режима прямого подключения**
   ...
   * Прямой режим
     ...
     Режим прямого доступа поддерживает подключение по протоколу TCP, а режим подключения по умолчанию — при использовании [Microsoft. Azure. Cosmos/.NET v3 SDK](sql-api-sdk-dotnet-standard.md).
     ...
     |Прямой доступ    |     TCP    |  Пакет SDK для .NET    | Порты в диапазоне 10 000–20 000 |

     Azure Cosmos DB предлагает простую и открытую модель программирования RESTful поверх HTTPS. Кроме того, предлагается эффективный протокол TCP, который также является RESTful в своей коммуникационной модели и доступен через клиентский пакет SDK для .NET. Протокол TCP использует SSL для начальной проверки подлинности и шифрования трафика. Чтобы добиться лучшей производительности, рекомендуется по возможности использовать протокол TCP.

     Для пакета SDK v3 режим подключения настраивается во время создания экземпляра Космосклиент в составе Космосклиентоптионс, помните, что в качестве режима по умолчанию используется режим Direct.
...
5. **Настройка параллельных запросов для секционированных коллекций**
    ...
    (a) ***степень параллелизма\:*** параллельный запрос работает путем параллельного запроса нескольких секций. Однако данные из отдельных секций извлекаться последовательно по отношению к запросу. Установка `MaxDegreeOfParallelism` в [пакете SDK v2](sql-api-sdk-dotnet.md) или `MaxConcurrency` в [пакете SDK v3](sql-api-sdk-dotnet-standard.md) на число секций максимально вероятной для достижения наиболее возможного запроса, при условии, что все остальные системные условия остаются прежними. Если число секций неизвестно, можно задать для степени параллелизма большое число, а система выберет минимальное (число секций, предоставленных пользователем входных данных) в качестве степени параллелизма.
...
7. **Кэширование URI документов для снижения задержки чтения**

    Чтобы повысить производительность операций чтения, по возможности выполняйте кэширование URI документов. Необходимо определить логику для кэширования идентификатора ресурса при создании ресурса. Поиск на основе идентификатора ресурса выполняется быстрее, чем уточняющие запросы на основе имен, поэтому кэширование этих значений повышает производительность. 
...
1. **Измерение и настройка расхода единиц запроса/повторного использования**
    ...
    Чтобы измерить издержки любой операции (создания, обновления или удаления), просмотрите заголовок [x-MS-Request-расход](https://docs.microsoft.com/rest/api/cosmos-db/common-cosmosdb-rest-response-headers) (или эквивалентное свойство RequestCharge в ResourceResponse\<t > или FeedResponse\<t > в пакете SDK .NET) для измерения количества единиц запросов, потребляемых этими операциями.
