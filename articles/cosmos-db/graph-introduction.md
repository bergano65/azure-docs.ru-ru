...
* **Гибко масштабируемые пропускная способность и хранилище**
  ...
  На практике графы должны масштабироваться за пределы емкости одного сервера. Azure Cosmos DB поддерживает горизонтально масштабируемые базы данных графа, которые могут иметь практически неограниченный размер с точки зрения предоставляемой пропускной способности. По мере увеличения масштаба базы данных графа, данные будут автоматически распределяться с использованием [секционирования данных графа](https://docs.microsoft.com/azure/cosmos-db/graph-partitioning).

* **Репликация между несколькими регионами**

  Azure Cosmos DB может автоматически реплицировать данные графа в любой регион Azure. Репликация упрощает разработку приложений, которым требуется глобальный доступ к данным. Помимо минимизации задержки чтения, Azure Cosmos DB предоставляет региональный механизм отработки отказа, который может обеспечить непрерывность работы вашего приложения в редких случаях прерывания обслуживания в регионе. 

* **Быстрые запросы и обходы с использованием наиболее распространенного стандарта запросов графа**

  Храните разные типы вершин и ребер, а также выполняйте запросы этих документов через знакомый синтаксис Gremlin. Gremlin — это императивный, функциональный язык запросов, который предоставляет комфортный интерфейс для реализации общих алгоритмов графов. 
...
* **Полностью управляемая база данных графа**
  ...
  В полностью управляемой службе Microsoft Azure нет необходимости управлять виртуальными машинами, обновлять программное обеспечение среды выполнения, управлять сегментированием или репликацией или заниматься обновлениями уровня комплексных данных. Каждый граф автоматически сохраняется и защищается от региональных сбоев. Эти гарантии позволяют разработчикам сосредоточиться на обеспечении ценности приложения вместо эксплуатации своих баз данных и управления ими. 
...
Базы данных графов часто относят к категории базу данных NoSQL или нереляционных баз данных, поскольку в них нет зависимости от схемы или ограничений модели данных. Это отсутствие схемы позволяет естественно и эффективно моделировать и хранить связанные структуры. 
