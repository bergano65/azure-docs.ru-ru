...
| `MaxDegreeOfParallelism` | Получает или устанавливает количество параллельных операций, запускаемых в сторону клиента во время параллельного выполнения запроса в службе базы данных Azure Cosmos. Положительное значение свойства ограничивает число параллельных операций до заданного значения. Если оно имеет значение меньше 0, система автоматически определяет число параллельных операций для выполнения. |
...
По умолчанию Azure Cosmos DB будет применять автоматическую индексацию ко всем данным. Для сценариев высокой производительности вставки следует исключить пути, так как это снизит стоимость RU для каждой операции вставки. 
