---
title: Метрики SQL-запросов для API SQL в Azure Cosmos DB
...
| `MaxDegreeOfParallelism` | Возвращает или задает число одновременных операций, выполняемых на стороне клиента во время выполнения параллельных запросов в службе базы данных Azure Cosmos. Положительное значение свойства ограничивает число параллельных операций до заданного значения. Если оно имеет значение меньше 0, система автоматически определяет число параллельных операций для выполнения. |
...
#### <a name="max-item-count"></a>Максимальное число элементов
...
По умолчанию Azure Cosmos DB будет применять автоматическое индексирование ко всем данным. В сценариях вставки с высокой производительностью рассмотрите возможность исключения путей, так как это снизит затраты на ЕДИНИЦу для каждой операции вставки. 
...
| ------ | -----| ----------- |
| `totalExecutionTimeInMs` | миллисекунд | Время выполнения запроса | 
| `queryCompileTimeInMs` | миллисекунд | Время компиляции запроса  | 
| `queryLogicalPlanBuildTimeInMs` | миллисекунд | Время на создание логического плана запроса | 
| `queryPhysicalPlanBuildTimeInMs` | миллисекунд | Время на создание физического плана запроса | 
| `queryOptimizationTimeInMs` | миллисекунд | Время, затраченное на оптимизацию запроса | 
| `VMExecutionTimeInMs` | миллисекунд | Время, затраченное в среде выполнения запроса | 
| `indexLookupTimeInMs` | миллисекунд | Время, затраченное на физическом уровне индекса | 
| `documentLoadTimeInMs` | миллисекунд | Время, затраченное на загрузку документов  | 
...
| `userFunctionExecuteTimeInMs` | миллисекунд | Общее время, затраченное на выполнение определяемых пользователем функций (в миллисекундах) | 
| `retrievedDocumentCount` | количество | Общее число полученных документов  | 
| `retrievedDocumentSize` | Байты | Общий размер полученных документов в байтах  | 
| `outputDocumentCount` | количество | Количество выходных документов | 
| `writeOutputTimeInMs` | миллисекунд | Время выполнения запроса в миллисекундах | 
