...
Основная модель взаимодействия с этими ресурсами заключается в использовании глаголов `GET`HTTP, `PUT` `POST`, и `DELETE`с их стандартной интерпретацией. Используйте `POST` для создания нового ресурса, выполнения хранимой процедуры или запроса Cosmos DB. Запросы всегда включают только операции чтения без побочных эффектов.

В следующих примерах показан `POST` запрос API SQL к примерам элементов. Запрос имеет простой фильтр для свойства JSON `name` . Типы `x-ms-documentdb-isquery` и Content-Type: `application/query+json` заголовки обозначают, что операция является запросом. Замените `mysqlapicosmosdb.documents.azure.com:443` на универсальный код ресурса (URI) для учетной записи Cosmos DB.
...
Если результаты запроса не помещаются на одной странице, REST API возвращает маркер продолжения через заголовок `x-ms-continuation-token` ответа. Клиенты могут разбивать результаты на страницы, включая заголовок в последующих результатах. Можно также управлять количеством результатов на страницу с помощью заголовка `x-ms-max-item-count` Number.
...
Подробные метрики для выполнения запроса можно получить, задав для `x-ms-documentdb-populatequerymetrics` `true`заголовка значение. Дополнительные сведения см. в статье [Tuning query performance with Azure Cosmos DB](sql-api-query-metrics.md) (Настройка производительности запросов с помощью Azure Cosmos DB).
