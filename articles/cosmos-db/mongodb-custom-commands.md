...
Используя API Azure Cosmos DB для MongoDB, вы можете наслаждаться преимуществами Cosmos DB, такими как глобальное распространение, автоматический, высокая доступность, гарантии задержки, автоматическое, шифрование в состоянии покоя, резервные копирования и многое другое, сохраняя при этом свои инвестиции в приложение MongoDB.

## <a name="mongodb-protocol-support"></a>Поддержка протокола MongoDB

По умолчанию API API Azure Cosmos DB для MongoDB совместим с версией сервера MongoDB 3.2, для получения более подробной информации [см.](mongodb-feature-support.md) Функции или операторы запросов, добавленные в версию MongoDB 3.4, в настоящее время доступны в качестве предварительного просмотра в API API Azure Cosmos DB для MongoDB. Следующие команды расширения поддерживают специфическую функциональность Azure Cosmos DB при выполнении операций CRUD на данных, хранящихся в API API Azure Cosmos DB для MongoDB:
...
|---------|---------|---------|---------|
| customAction | строка | Обязательно | Название пользовательской команды. Должно быть "СоздатьКоллекцию".|
| коллекция | строка | Обязательно | Имя коллекции. Специальные символы не допускаются.|
| offerThroughput | INT | Необязательно | Установленная пропускная информация для установки в базе данных. Если этот параметр не предусмотрен, он будет по умолчанию до минимума, 400 RU/s. Для указания пропускной вылимза за 10 `shardKey` 000 руб./с требуется параметр.|
| shardKey | строка | Необязательно | Путь к осколок для осколки коллекции. Этот параметр необходим, если вы установите более 10 `offerThroughput`000 RU/s в.  Если это указано, все вставленные документы потребуют этого значения. |
...
Для создания осколки коллекции с названием "testCollection" и прослеживаемым пропускными данными 1000 rUs и свойством осколка "a.b" используйте следующую команду:
