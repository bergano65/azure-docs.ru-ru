---
title: Пользовательские команды для управления данными, хранящимися в Azure Cosmos DB API для MongoDB
description: В этой статье описывается, как использовать пользовательские команды для управления данными, хранящимися в Azure Cosmos DB API для MongoDB.
author: SnehaGunda
...
ms.locfileid: "58807056"
---
# <a name="use-custom-commands-to-manage-data-stored-in-azure-cosmos-dbs-api-for-mongodb"></a>Использование пользовательских команд для управления данными, хранящимися в Azure Cosmos DB API для MongoDB 
...
По умолчанию, API Azure Cosmos DB для MongoDB совместим с MongoDB server версии 3.2, Дополнительные сведения см. в разделе [поддерживается возможностей и синтаксиса](mongodb-feature-support.md). Функции и операторы запросов, добавленные в MongoDB версии 3.4 в настоящее время доступны в предварительной версии в Azure Cosmos DB API для MongoDB. Следующие пользовательские команды поддержки определенных функциональных возможностей Azure Cosmos DB, при выполнении операций CRUD в данные, хранящиеся в Azure Cosmos DB API для MongoDB:
...
Пользовательские команды создания базы данных создает новую базу данных MongoDB. Имя базы данных используется в контексте базы данных, по которой выполняется команда. Формат команды CreateDatabase выглядит следующим образом:
...
Пользовательские команды обновления базы данных обновляет свойства, связанные с указанной базы данных. В настоящее время можно обновить только свойство «offerThroughput».
...
Команда пользовательские базы данных get возвращает объект базы данных. Имя базы данных используется в контексте базы данных, по которой выполняется команда.
...
Пользовательские команды создания коллекции создает новую коллекцию MongoDB. Имя базы данных используется в контексте базы данных, по которой выполняется команда. Формат команды CreateCollection выглядит следующим образом:
...
Пользовательские команды update коллекции обновляет свойства, связанные с указанной коллекции.
