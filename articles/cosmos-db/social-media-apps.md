---
title: Конструктивный шаблон Cosmos DB. Приложения социальных сетей
...
[Машинное обучение Azure](https://azure.microsoft.com/services/machine-learning/), входящее в состав [Cortana Intelligence Suite](https://social.technet.microsoft.com/wiki/contents/articles/36688.introduction-to-cortana-intelligence-suite.aspx), — это полностью управляемая облачная служба, которая позволяет создавать рабочие процессы с помощью алгоритмов, используя простой интерфейс с возможностью перетаскивания, а также позволяет программировать собственные алгоритмы на [R](https://en.wikipedia.org/wiki/R_\(programming_language\)) или использовать некоторые встроенные и готовые к использованию API, например API [текстовой аналитики](https://gallery.cortanaanalytics.com/MachineLearningAPI/Text-Analytics-2), API Content Moderator или [API рекомендаций](https://gallery.azure.ai/Solution/Recommendations-Solution).
...
Служба Cosmos DB изначально поддерживает [динамическое секционирование](https://azure.microsoft.com/blog/10-things-to-know-about-documentdb-partitioned-collections/). Она автоматически создает секции в соответствии с определенным **ключом секции**, заданным в качестве атрибута документов. Определять правильный ключ секции необходимо во время разработки. Дополнительные сведения см. в разделе [Выбор ключа секции](partitioning-overview.md#choose-partitionkey).

Для социального взаимодействия необходимо согласовать стратегию секционирования со способом отправки запросов и записи. (Например, желательно считывать данные в рамках одной секции и избегать образования "горячих точек", распределяя записываемые данные между различными секциями.) Секции могут определяться временным ключом (днем, месяцем или неделей), категорией содержимого, географическим регионом или пользователем. На самом деле все зависит от того, как вы будете запрашивать данные и отображать их при социальном взаимодействии.
...
Дело в том, что универсального решения для таких сценариев не существует. Это сочетание отличных служб, позволяющих создавать превосходные решения: скорость и свобода Azure Cosmos DB для создания отличного приложения социальных сетей; интеллектуальные функции первоклассного решения для поиска (например, Поиска Azure); гибкость служб приложений Azure для размещения не только не зависящих от языка приложений, но и мощных фоновых процессов; расширяемая служба хранилища Azure и База данных SQL Azure для хранения больших объемов данных; большие аналитические возможности Машинного обучения Azure для формирования знаний и аналитики, позволяющих получать отзывы о ваших процессах и доставлять пользователям именно то содержимое, которое им нужно.
