---
title: 'Шаблон проектирования Azure Cosmos DB: приложения для социальных сетей'
...
Поскольку вы используете Azure Cosmos DB, вы можете легко реализовать поисковую систему с помощью [Azure Cognitive Search](https://azure.microsoft.com/services/search/) в течение нескольких минут без ввода какого-либо кода, кроме процесса поиска и uI.

Почему этот процесс так прост?

Azure Cognitive Search реализует то, что они называют [Индексаторами,](https://msdn.microsoft.com/library/azure/dn946891.aspx)фоновые процессы, которые закрепляются в репозиториях данных и автоматически добавляют, обновляют или удаляют объекты в индексах. Они поддерживают [индексаторы базы данных SQL Azure](https://blogs.msdn.microsoft.com/kaevans/2015/03/06/indexing-azure-sql-database-with-azure-search/), [индексаторы больших двоичных объектов Azure](../search/search-howto-indexing-azure-blob-storage.md) и, к счастью, [индексаторы Azure Cosmos DB](../search/search-howto-index-documentdb.md). Переход информации из Cosmos DB в Azure Cognitive Search прост. Обе технологии хранят данные в формате JSON, поэтому вам достаточно [создать индекс](../search/search-create-index-portal.md) и сопоставить индексируемые атрибуты документов. Это все! В зависимости от размера данных, все содержимое станет доступно для поиска в течение нескольких минут благодаря лучшему решению "поиск как служба" в облачной инфраструктуре.

Для получения дополнительной информации о Azure Cognitive Search вы можете посетить [руководство автостопом по поиску.](https://blogs.msdn.microsoft.com/mvpawardprogram/2016/02/02/a-hitchhikers-guide-to-search/)
...
[Azure Machine Learning](https://azure.microsoft.com/services/machine-learning/), часть [Cortana Intelligence Suite](https://social.technet.microsoft.com/wiki/contents/articles/36688.introduction-to-cortana-intelligence-suite.aspx), это полностью управляемый облачный сервис, который позволяет создавать рабочие процессы с помощью алгоритмов в простом интерфейсе перетаскивания, кодировать свои собственные алгоритмы в [R,](https://en.wikipedia.org/wiki/R_\(programming_language\))или использовать некоторые из уже построенных и готовых к использованию AA, таких как: [Text Analytics,](https://gallery.cortanaanalytics.com/MachineLearningAPI/Text-Analytics-2)«Модератор содержимого» или [Рекомендации.](https://gallery.azure.ai/Solution/Recommendations-Solution)
...
Для социального взаимодействия необходимо согласовать стратегию секционирования со способом отправки запросов и записи. (Например, считывания в пределах одного раздела являются желательными, и избежать "горячих точек", распространяя пишет на нескольких разделах.) Некоторые варианты: разделы на основе временного ключа (день/месяц/неделя), по категории контента, по географическому региону или по пользователю. На самом деле все зависит от того, как вы будете запрашивать данные и отображать их при социальном взаимодействии.
...
Дело в том, что универсального решения для таких сценариев не существует. Это синергия, созданная сочетанием больших услуг, которые позволяют нам создавать большой опыт: скорость и свободу Azure Cosmos DB, чтобы обеспечить большое социальное приложение, интеллект за первоклассное решение поиска, как Azure Cognitive Search, гибкость служб приложений Azure для размещения даже не языковых агностиковых приложений, а мощных фоновых процессов и расширяемой базы данных по хранению данных Azure и Azure S'L для хранения огромных объемов данных и аналитической мощности машинного обучения Azure создавать знания и интеллект, которые могут обеспечить обратную связь с вашими процессами и помочь нам доставить правильный контент для нужных пользователей.
