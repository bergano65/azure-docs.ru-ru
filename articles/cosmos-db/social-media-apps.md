...
Как специалисты и разработчики, вы могли задаваться вопросом, как эти сети хранят и связывают ваши данные, или даже самостоятельно участвовать в создании или проектировании новой социальной сети для какой-нибудь специфической ниши рынка. Именно тогда возникает важный вопрос: как хранятся все эти данные?

Предположим, что вы создаете новую яркую социальную сеть, где ваши пользователи могут публиковать статьи с сопутствующими материалами, например изображениями, видео или даже музыкой. Пользователи могут комментировать записи и оценивать их с помощью очков. В сети будет веб-канал записей, которые пользователи смогут видеть, также они смогут взаимодействовать с целевой страницей основного веб-сайта. Этот метод сначала выглядит несложным, но для простоты давайте остановимся на этом (можно углубиться в настраиваемые пользовательские веб-каналы, на которые влияют связи, но это не входит в задачи этой статьи).

Итак, как все это хранить и где?

Возможно вы обладаете опытом работы с базами данных SQL или имеете понятие о [реляционном моделировании данных](https://en.wikipedia.org/wiki/Relational_model) и можете начать рисовать примерно следующее.
...
Почему SQL — не наилучший вариант для этого сценария? Рассмотрим структуру одной записи. Чтобы показать эту запись на веб-сайте или в приложения, мне потребовалось бы сделать запрос с… путем объединения восьми таблиц (!) только для того, чтобы показать одну запись. Теперь представьте поток записей, которые динамически загружаются и отображаются на экране, и вы поймете о чем я.

Вы могли бы использовать огромный экземпляр SQL с вычислительной мощностью, достаточной для обработки тысяч запросов с большим количеством соединений, чтобы обслуживать ваше содержимое, но для чего тогда существует более простое решение?

## <a name="the-nosql-road"></a>Путь NoSQL
Эта статья содержит рекомендации по экономичному моделированию данных социальной платформы с помощью базы данных NoSQL Azure в [Azure Cosmos DB](https://azure.microsoft.com/services/cosmos-db/) и использованию других возможностей базы данных Azure Cosmos DB, например [API Gremlin](../cosmos-db/graph-introduction.md). Используя [NoSQL](https://en.wikipedia.org/wiki/NoSQL), храня данные в формате JSON и применяя [денормализацию](https://en.wikipedia.org/wiki/Denormalization), запись, которая ранее представлялась сложной, можно преобразовать в один [документ](https://en.wikipedia.org/wiki/Document-oriented_database).
...
Azure Cosmos DB обеспечивает индексирование всех свойств с помощью автоматического индексирования, которое даже можно [настроить](indexing-policies.md). Эта методика без использования схемы позволяет хранить документы с различными динамическими структурами. Возможно, завтра вам понадобятся записи со списком категорий или хэштегами, связанными с ними. Cosmos DB будет обрабатывать новые документы с помощью добавленных атрибутов, и для этого вам не придется ничего делать дополнительно.
...
Вы можете создать поток "Последние" с сообщениями, упорядоченными по дате создания, а также поток "Популярные" с записями, которые понравились наибольшему числу пользователей за последние 24 часа. Вы даже можете реализовать настраиваемый поток для каждого пользователя, например основанный на логике, учитывающей число подписчиков и интересы, и все равно это будет список записей. Важно то, как создаются эти списки, а производительность чтения остается неизменной. Получив один из этих списков, вы выполните один запрос к Cosmos DB с помощью [оператора IN](sql-api-sql-query.md#WhereClause), чтобы получить сразу несколько страниц записей.
...
Это может подойти для пользователя с несколькими тысячами читателей, но если к вам присоединится какая-либо знаменитость, то при данном подходе размер документа увеличится и вы все равно натолкнетесь на ограничение размера.

Чтобы решить эту задачу, можно применить смешанный подход. В документе статистики пользователей вы можете хранить количество читателей.
...
Фактический граф подписчиков можно хранить с помощью [API Gremlin](../cosmos-db/graph-introduction.md) базы данных Azure Cosmos DB. При этом создаются [вершины](http://mathworld.wolfram.com/GraphVertex.html) для каждого пользователя и [грани](http://mathworld.wolfram.com/GraphEdge.html), поддерживающие отношения "А подписан на Б". API Gremlin позволяет вам не только получать подписчиков конкретного пользователя, но и создавать более сложные запросы, предлагающие пользователей с некоторыми общими характеристиками. Если добавить в граф категории содержимого, которые "нравятся" пользователям, вы сможете добавить возможности связывания, включая интеллектуальное обнаружение содержимого, предложение содержимого, интересующего пользователей, на которых вы подписаны, или поиск людей, с которыми вы можете иметь общие интересы.
...
И в случае изменения, затрагивающего один из атрибутов блока, несложно найти требуемые документы с помощью запросов, которые указывают на индексированные атрибуты (SELECT * FROM posts p WHERE p.createdBy.id == "edited_user_id"), а затем обновить эти блоки.
...
[Машинное обучение Azure](https://azure.microsoft.com/services/machine-learning/), являющееся частью [Cortana Intelligence Suite](https://social.technet.microsoft.com/wiki/contents/articles/36688.introduction-to-cortana-intelligence-suite.aspx), — это полностью управляемая облачная служба, которая позволяет создавать рабочие процессы с помощью алгоритмов, используя простой интерфейс с перетаскиванием, а также позволяет программировать собственные алгоритмы на [R](https://en.wikipedia.org/wiki/R_\(programming_language\)) или использовать некоторые встроенные и готовые к использованию API, например API [текстовой аналитики](https://gallery.cortanaanalytics.com/MachineLearningAPI/Text-Analytics-2), API Content Moderator или [API рекомендаций](https://gallery.azure.ai/Solution/Recommendations-Solution).
...
Со временем у вас, очевидно, увеличится трафик и потребление ресурсов (измеряемое в единицах запроса, [ЕЗ](request-units.md)). Операции чтения и записи станут более частыми по мере роста вашей базы пользователей и начнут создавать и потреблять больше содержимого. Поэтому возможность **масштабирования пропускной способности** крайне важна. Увеличить количество ЕЗ легко. Это можно сделать, нажав несколько кнопок на портале Azure или [выполнив команды через интерфейс API](https://docs.microsoft.com/rest/api/cosmos-db/replace-an-offer).
...
Эта статья проливает свет на возможности создания социальных сетей полностью на платформе Azure с помощью недорогих служб, что дает превосходные результаты, поощряя к использованию решения для многоуровневого хранения данных и распределения данных, которое называется "Лестница".
