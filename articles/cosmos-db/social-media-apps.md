...
Почему SQL — не наилучший вариант для этого сценария? Рассмотрим структуру одной записи. Чтобы показать эту запись на веб-сайте или в приложения, мне потребовалось бы сделать запрос с… восьми соединениями таблиц (!) только для того, чтобы показать одну запись. Теперь представьте поток записей, которые динамически загружаются и отображаются на экране, и вы поймете, о чем я.

Конечно, вы могли бы использовать огромный экземпляр SQL с вычислительной мощностью, достаточной для обработки тысяч запросов с этим количеством соединений, чтобы обслуживать ваше содержимое, но для чего тогда существует более простое решение?
