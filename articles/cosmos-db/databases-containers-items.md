...
ms.locfileid: "51684864"
---
# <a name="working-with-azure-cosmos-databases-containers-and-items"></a>Работа с базами данных, контейнерами и элементами Azure Cosmos
...
| **Сущность Azure Cosmos DB** | **API SQL** | **API Cassandra** | **API MongoDB** | **API Gremlin** | **API таблиц** |
| --- | --- | --- | --- | --- | --- |
|База данных Azure Cosmos | База данных | Пространство ключей | База данных | Нет данных | Нет данных |

> [!NOTE]
> При создании графа или таблицы с помощью API Gremlin и API таблиц база данных по умолчанию создается автоматически в рамках учетной записи Azure Cosmos.
...
| **Операция** | **Интерфейс командной строки Azure**|**API SQL** | **API Cassandra** | **API MongoDB**; | **API Gremlin** | **API таблицы**; |
| --- | --- | --- | --- | --- | --- | --- |
...
Контейнеры Cosmos Azure поддерживают эластичное масштабирование независимо от выбранного режима подготовленной пропускной способности (выделенной или общей). Это значит, что они могут иметь неограниченный объем хранилища и подготовленной пропускной способности.  
...
| **Системные свойства** | **Устанавливаемое пользователем или генерируемое системой** | **Назначение** | **API SQL** | **API Cassandra** | **API MongoDB**; | **API Gremlin** | **API таблицы**; |
| --- | --- | --- | --- | --- | --- | --- | --- |
|__rid | Генерируемое системой | Уникальный идентификатор контейнера. | Yes | Нет  | Нет  | Нет  | Нет  |
|__etag | Генерируемое системой | Тег сущности, используемый для управления оптимистической блокировкой. | Yes | Нет  | Нет  | Нет  | Нет  |
|__ts | Генерируемое системой | Метка времени последнего обновления контейнера. | Yes | Нет  | Нет  | Нет  | Нет  |
|__self | Генерируемое системой | Адресуемый URI контейнера. | Yes | Нет  | Нет  | Нет  | Нет  |
...
| **Сущность Azure Cosmos DB** | **API SQL** | **API Cassandra** | **API MongoDB**; | **API Gremlin** | **API таблицы**; |
| --- | --- | --- | --- | --- | --- |
...
|**Системные свойства** | **Устанавливаемое пользователем или генерируемое системой**| **Назначение** | **API SQL** | **API Cassandra** | **API MongoDB**; | **API Gremlin** | **API таблицы**; |
| --- | --- | --- | --- | --- | --- | --- | --- |
|__id | Генерируемое системой | Уникальный идентификатор элемента. | Yes | Нет  | Нет  | Нет  | Нет  |
|__etag | Генерируемое системой | Тег сущности, используемый для управления оптимистической блокировкой. | Yes | Нет  | Нет  | Нет  | Нет  |
|__ts | Генерируемое системой | Метка времени последнего обновления элемента. | Yes | Нет  | Нет  | Нет  | Нет  |
|__self | Генерируемое системой | Адресуемый URI элемента. | Yes | Нет  | Нет  | Нет  | Нет  |
