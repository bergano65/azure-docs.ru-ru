...
Если отображается необычно большая задержка для операций с точками, например:

* Операция Get или Set с ключом секции и ИДЕНТИФИКАТОРом в режиме прямого подключения
* Операция чтения или записи или
* Запрос

Чтобы узнать размер возвращаемых данных, можно выполнить поиск в журнале диагностики. Если вы видите постоянную высокую задержку операций запросов, можно найти журнал диагностики, чтобы узнать размер возвращаемых данных, [пропускной способности или единиц запросов в секунду](cosmosdb-monitor-resource-logs.md#diagnostic-queries) или количество таких операций за определенный период. Таким образом можно отладить проблемы задержки на стороне сервера.
