...
- **Единицы запросов**: каждая операция базы данных имеет связанные единицы запроса (RUs) в Azure Cosmos DB. При исполнении это значение вычитается из доступного уровня единиц запросов в заданной секунде. Если для запроса требуется больше, чем в настоящее время выделено два варианта, увеличивается количество запросов на повышение (или ожидание до начала следующей секунды), а затем выполняется повтор операции.
...
- **Автоматическое сегментирование**: Azure Cosmos DB предоставляет автоматическую систему секционирования, для которой требуется только ключ сегмента (или секционирования). [Механизм автоматического секционирования](partition-data.md) совместно используется всеми Azure Cosmos DBными API-интерфейсами и обеспечивает бесперебойность данных и масштабирование через горизонтальное распределение.
