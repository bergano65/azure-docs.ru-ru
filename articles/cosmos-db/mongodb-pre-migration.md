...
## <a name="migration-options-for-azure-cosmos-dbs-api-for-mongodb"></a><a id="options"></a>Параметры миграции для API Azure Cosmos DB для MongoDB
...
|Справка в Интернете|[Azure Database Migration Service](../dms/tutorial-mongodb-cosmos-db-online.md)|&bull;Полностью управляемая служба миграции.<br/>&bull;Предоставляет решения для размещения и мониторинга для задачи миграции. <br/>&bull;Подходит для больших наборов данных и отвечает за репликацию динамических изменений <br/>&bull;Работает только с другими источниками MongoDB|


## <a name="estimate-the-throughput-need-for-your-workloads"></a><a id="estimate-throughput"></a>Оценка потребности в пропускной способности для рабочих нагрузок
...
По умолчанию Azure Cosmos DB обеспечивает автоматическое индексирование всех вставленных данных. Возможности индексирования, предоставляемые Azure Cosmos DB, включают добавление составных индексов, уникальных индексов и индексов срока жизни (TTL). Интерфейс управления индексами сопоставляется с `createIndex()` командой. Дополнительные сведения см. [в статье индексирование в API Azure Cosmos DB для MongoDB](mongodb-indexing.md).
