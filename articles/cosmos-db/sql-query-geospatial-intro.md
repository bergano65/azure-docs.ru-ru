...
API-интерфейс SQL Azure Cosmos DB поддерживает тип данных **Geography** . Пространственный тип данных **geography** представляет данные в системе координат для сферической Земли.
...
### <a name="points-in-geography-coordinate-system"></a>Точки в географической системе координат
...
При работе с API SQL можно использовать классы `Point`, `LineString`, `Polygon`и `MultiPolygon` в пространстве имен `Microsoft.Azure.Cosmos.Spatial` для внедрения сведений о расположении в объекты приложения. Эти классы помогают упростить сериализацию и десериализацию пространственных данных в GeoJSON.
