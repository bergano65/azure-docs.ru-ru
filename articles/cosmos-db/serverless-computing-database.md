...
* Создание **триггера функций Azure**, управляемых событиями, для Cosmos DB. Этот триггер использует потоки [канала изменений](change-feed.md) для мониторинга изменений в контейнере Azure Cosmos DB. При внесении изменений в контейнер поток канала изменений отправляется в триггер, который вызывает функцию Azure.
...
* Входную привязку к контейнеру Azure Cosmos DB можно использовать в той же функции, что и триггер функций Azure для Cosmos DB, и может использоваться с выходной привязкой или без нее. С помощью такого сочетания можно передавать актуальные данные курса валют (полученные с помощью входной привязки к контейнеру данных обмена валют) в канал изменений для новых заказов в службе корзины для покупок. Обновленную итоговую сумму для корзины для покупок, в которой учтена конвертация валют, можно записать в третий контейнер с помощью выходной привязки.
