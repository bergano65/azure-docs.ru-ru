---
title: Вычисления без серверных баз данных с функциями Azure Cosmos DB и Azure
...
* Создайте триггер **Azure Функции, управляемый событиями, для Cosmos DB.** Этот триггер зависит от потоков [подачи изменений](change-feed.md) для мониторинга контейнера Azure Cosmos для внесения изменений. При внесении изменений в контейнер поток канала изменений отправляется в триггер, который вызывает функцию Azure.
* Кроме того, свяжите функцию Azure с контейнером Azure Cosmos, используя **связывание ввода.** Входные привязки считывают данные из контейнера при выполнении функции.
* Привязать функцию к контейнеру Azure Cosmos с помощью **выходной привязки.** Выходные привязки записывают данные в контейнер после завершения выполнения функции.

> [!NOTE]
> В настоящее время триггер Azure Functions, привязки входных данных и выходные привязки для Cosmos DB поддерживаются для использования только с API S'L. Для всех других API Azure Cosmos DB доступ к базе данных из функции должен осуществляться с использованием статического клиента для API.
...
Триггер Azure Functions, привязка входных данных и выходное привязка для Azure Cosmos DB могут быть использованы в следующих комбинациях:

* Триггер Azure Functions для Cosmos DB может быть использован с выходной привязкой к другому контейнеру Azure Cosmos. После того, как функция выполнит действие с элементом в канале изменений, его можно будет записать в другой контейнер (если записать его в тот же контейнер, из которого он поступил, то это может привести к созданию рекурсивного цикла). Или можно использовать триггер Azure Functions для Cosmos DB для эффективного переноса всех измененных элементов из одного контейнера в другой контейнер с использованием выходной привязки.
...
* Привязка входных данных к контейнеру Azure Cosmos может использоваться в той же функции, что и триггер Azure Functions для Cosmos DB, и может использоваться с привязкой выхода или без нее. С помощью такого сочетания можно передавать актуальные данные курса валют (полученные с помощью входной привязки к контейнеру данных обмена валют) в канал изменений для новых заказов в службе корзины для покупок. Обновленную итоговую сумму для корзины для покупок, в которой учтена конвертация валют, можно записать в третий контейнер с помощью выходной привязки.
...
### <a name="iot-use-case---azure-functions-trigger-and-output-binding-for-cosmos-db"></a>IoT случай использования - Триггер и выходной привязки функций Azure для Cosmos DB
...
**Реализация:** Используйте триггерно-выходной привязки функций Azure для Cosmos DB

1. **Триггер Azure Functions для Cosmos DB** используется для запуска событий, связанных с автомобильными оповещениями, таких как свет контрольного двигателя, поступающий в подключенном автомобиле.
2. Когда загорается индикатор проверки двигателя, данные датчика отправляются в Azure Cosmos DB.
3. Azure Cosmos DB создает или обновляет новые документы данных датчиков, а затем эти изменения передаются на триггер Azure Functions для Cosmos DB.
...
7. **Выходная связка** функции обновляет запись автомобиля в другом контейнере Azure Cosmos для хранения информации о событии проверки двигателя.

На рисунке ниже приведен код, написанный на портале Azure для этого триггера.

![Создание триггера Azure Functions для Cosmos DB на портале Azure](./media/serverless-computing-database/cosmos-db-trigger-portal.png)
...
1. Используя [триггер таймер,](../azure-functions/functions-bindings-timer.md)вы можете получить информацию о балансе банковского счета, хранящуюся в контейнере Azure Cosmos, с помощью **связывания ввода.**
...
### <a name="gaming-use-case---azure-functions-trigger-and-output-binding-for-cosmos-db"></a>Случай использования игр - Триггер и выход Azure Functions для Cosmos DB 
...
1. Используя [базу данных](graph-introduction.md) графикaz Cosmos DB для хранения всех пользователей, можно создать новую функцию с триггером Azure Functions для Cosmos DB. 
...
**Реализация:** Несколько функций Azure триггеры для Космос DB прослушивания одного контейнера

1. Можно создать несколько функций Azure, добавив триггеры Azure Functions для Cosmos DB для каждого из них, которые слушают одну и ту же ленту изменений данных корзины. Обратите внимание, если несколько функций прослушивают один канал изменений, для каждой функции требуется новая коллекция аренды. Дополнительные сведения о коллекциях аренды см. в разделе [Основные сведения о библиотеке обработчика канала изменений](change-feed-processor.md).
2. Всякий раз, когда пользователь добавляет товар в корзину для покупок, каждая функция независимо вызывается каналом изменений из контейнера корзины для покупок.
   ...
   * Еще одна функция может отправлять сведения об определенных товарах, выбранных клиентом, в отдел маркетинга, который рассылает покупателям рекламу. 
     ...
     Любой отдел может создать функции Azure для Cosmos DB, слушая ленту изменений, и быть уверенным, что они не будут задерживать критические события обработки заказов в процессе.

Во всех этих случаях использования, поскольку функция отделила само приложение, вам не нужно постоянно вращать новые экземпляры приложений. Вместо этого служба "Функции Azure" по мере необходимости запускает отдельные функции для выполнения отдельных процессов.

## <a name="tooling"></a>Инструменты

Интеграция между Azure Cosmos DB и Azure Functions доступна на портале Azure и в Visual Studio 2019.

* На портале Azure Functions можно создать триггер. Для инструкций по быстрому запуску на [портале Azure смотрите триггер Azure Functions для Cosmos DB.](../azure-functions/functions-create-cosmos-db-triggered-function.md)
* На портале Azure Cosmos DB можно добавить триггер Azure Functions для Cosmos DB в существующее приложение Azure Function в той же группе ресурсов.
* В Visual Studio 2019 можно создать триггер с помощью [инструментов Функции Azure:](../azure-functions/functions-develop-vs.md)
...
Служба "Функции Azure" дает возможность создавать масштабируемые единицы работы, или компактные элементы логики, которые могут выполняться по требованию, без подготовки ресурсов или управления инфраструктурой. Используя функции Azure, вам не нужно создавать полноценное приложение для реагирования на изменения в базе данных Azure Cosmos, можно создавать небольшие многоразовые функции для определенных задач. Кроме того, можно использовать данные Azure Cosmos DB как входные или выходные данные функции Azure, передаваемые в ответ на событие, например HTTP-запросы или триггер таймера.
...
* [Создание триггера Azure Functions для Cosmos DB на портале Azure](../azure-functions/functions-create-cosmos-db-triggered-function.md)
