...
ms.locfileid: "78674306"
---
# <a name="understand-your-azure-cosmos-db-bill"></a>Расшифровка счета за использование Azure Cosmos DB

Azure Cosmos DB — это полностью управляемая оптимизированная для облака служба баз данных с упрощенной моделью выставления счетов. Плата взимается только за подготовленную пропускную способность и использованный объем хранилища. Не взимается дополнительная плата за лицензию, оборудование, служебные затраты или стоимость оборудования по сравнению с альтернативными вариантами или способами, размещаемыми в IaaS. При рассмотрении возможностей Azure Cosmos DB в нескольких регионах служба базы данных обеспечивает существенное снижение затрат по сравнению с существующими локальными или решениями IaaS.
...
> [!NOTE]
> Выставление счетов осуществляется для любой части часового часа, а не за 60 минут.
...
* В течение 720 часов (24 часа * 30 дней), если для подготовленной пропускной способности в 500 ч было 1 200 единиц запросов в секунду, а оставшиеся пропускная способность в 220 часа была 22 200 долларов/с, то ежемесячный счет показывает: 500 x $0.096/час + 220 x $1.776/Hour = $438.72/месяц.
...
* Ваш счет изменится на: 1 550 * $0,008 = $12.40/час.  

* В течение 720 часов, если для подготовленной пропускной способности 300 часов была 120-K единиц запросов в секунду, а оставшиеся пропускная способность 420 часа была равна 155-K единиц запросов в секунду, ежемесячный счет будет содержать: 300 x $9.60/час + 420 x $12.40/час = $2 880 + $5 208 = $8088/month. 
...
*Кроме того, предположим, что вы передаете 100 ГБ данных каждый месяц из контейнера в западной части США для репликации данных в восточной части США, Северной Европе и Восточная Азия. Плата взимается за исходящий трафик в соответствии с тарифами на передачу данных.*
...
| | |Счет пропускной способности для двух дополнительных регионов: Восточная часть США, Северная Европа (все регионы доступны для записи)  |`(2 + 1) * (60 K RU/sec /100 * $0.016) * 100 hours = $2,880`  |2880 долл. США  |
...
| | |Счет пропускной способности для двух дополнительных регионов: Восточная часть США, Северная Европа (все регионы доступны для записи)  |`(2 + 1) * (120 K RU/sec /100 * $0.016) * 100 hours = $5,760`  |5760 долл. США  |
...
| | |Счет пропускной способности для двух дополнительных регионов: Восточная часть США, Северная Европа (все регионы доступны для записи)  |`(2 + 1) * (140 K RU/sec /100 * $0.016-) * 100 hours = $6,720` |6720 долл. США |
...
| | |Счет пропускной способности для двух дополнительных регионов: Восточная часть США, Северная Европа (все регионы доступны для записи)  |`(1 + 1) * (90 K RU/sec /100 * $0.016) * 100 hours = $2,880`  |2880 долл. США  |
...
| | |Счет пропускной способности для двух дополнительных регионов: Восточная часть США, Северная Европа (все регионы доступны для записи)  |`(1 + 1) * (40 K RU/sec /100 * $0.016) * 100 hours = $1,280`  |1280 долл. США  |
...
| | |Счет пропускной способности для двух дополнительных регионов: Восточная часть США, Северная Европа (все регионы доступны для записи)  |`(1 + 1) * (120 K RU/sec /100 * $0.016) * 200 hours = $1,280`  |7680 долл. США  |
...
| | |Счет пропускной способности для двух дополнительных регионов: Восточная часть США, Северная Европа (все регионы доступны для записи)  |`(1 + 1) * (70 K RU/sec /100 * $0.016) * 20 hours = $448`  |224 долл. США  |
|| |**Общая ежемесячная стоимость**  | |**38 688 долл. США**   |

## <a name="billing-examples-with-free-tier-accounts"></a>Примеры выставления счетов с учетными записями бесплатных уровней
С Azure Cosmos DB уровня "бесплатный" вы получите первые 400 единиц запросов в секунду и 5 ГБ в своей учетной записи, которые будут использоваться на уровне учетной записи. За период между единицами запросов в секунду и хранилищем свыше 400 единиц запросов в секунду и 5 ГБ будет взиматься плата по стандартным ценам на странице цен. В счете не будет вычислена плата за бесплатную 400 единиц запросов в секунду и 5 ГБ, а только единиц запросов в секунду и хранилище, за исключением того, что распространяется на бесплатный уровень. 400 единиц запросов в секунду относится к любым типам подготовленных пропускной способности единиц запросов в секунду, Автопилот (Предварительная версия) и нескольким главным службам.  

### <a name="billing-example---container-or-database-with-provisioned-throughput"></a>Пример выставления счетов — контейнер или база данных с подготовленной пропускной способностью
- Предположим, что мы создаем базу данных или контейнер в учетной записи уровня Free с 400 единиц запросов в секунду и 5 ГБ хранилища.
- Плата за этот ресурс не будет отображаться. Каждый час и ежемесячная стоимость будут $0.
- Теперь предположим, что в той же учетной записи мы добавили еще одну базу данных или контейнер с 1000 единиц запросов в секунду и 10 ГБ хранилища.
- Теперь в счете будет показана плата за 1000 единиц запросов в секунду и 10 ГБ хранилища. 

### <a name="billing-example---container-or-database-with-autopilot-throughput-preview"></a>Пример выставления счетов — контейнер или база данных с пропускной способностью автопилота (Предварительная версия)
- Предположим, что в учетной записи бесплатного уровня мы создаем базу данных или контейнер с включенным автопилотом с максимальным числом единиц запросов в секунду (4000 единиц запроса в секунду). Этот ресурс будет автоматически масштабироваться между 400 единиц запросов в секунду и 4000 единиц запросов в секунду. 
- Предположим, что в час с 1 по час 10 ресурс равен минимуму 400 единиц запросов в секунду. В течение часа 11 ресурс масштабируется до 1000 единиц запросов в секунду, а затем в течение часа выполняется резервное копирование на 400 единиц запросов в секунду.
- В часах с 1 по 10 будет взиматься плата за пропускную способность $0, так как 400 единиц запросов в секунду охватывает уровень "бесплатный". 
- В час 11 будет взиматься плата за действующий 1000 единиц запросов в секунду, 400 единиц запросов в секунду, равную 600 единиц запросов в секунду в течение часа. В течение часа это будет иметь 6 единиц, 100 единиц запросов в секунду, поэтому общая стоимость пропускной способности в час будет составлять 6 штук * $0,012 = $0,072. 
- Плата за хранилище, превышающее первые 5 ГБ, будет взиматься по стандартным тарифам на хранение. 

### <a name="billing-example---multi-region-single-write-region-account"></a>Пример выставления счетов — учетная запись в нескольких регионах, одна область записи
- Предположим, что в учетной записи бесплатного уровня мы создаем базу данных или контейнер с 1200 единиц запросов в секунду и 10 ГБ хранилища. Мы выполним репликацию учетной записи в 3 региона, и у нас есть учетная запись с одним главным узлом (с одной записью).
- В общем, без уровня "бесплатный" плата будет взиматься за 3 * 1200 единиц запросов в секунду на 3600 единиц запроса в секунду и 3 * 10 ГБ = 30 ГБ хранилища.
- За счет скидки на бесплатный уровень после удаления 400 единиц запросов в секунду и 5 ГБ хранилища плата будет взиматься за действующий 3200 единиц запросов в секунду (32 единицы) подготовленной пропускной способности в пределах одной области записи и 25 ГБ хранилища.
- Ежемесячные затраты на единицу запросов в секунду: 32 единиц * $0,008 * 24 часа * 31 день = $190,46. Ежемесячные затраты на хранение: 25 ГБ * 0,25/ГБ = $6,25. Общая стоимость составит $190,46 + $6,25 = $196,71.
- Примечание. Если цена за единицу для единиц запросов в секунду или хранилища различается в регионах, уровень Free 400 единиц запросов в секунду и 5 ГБ будет отражать тарифы учетной записи, в которой был создан регион.

### <a name="billing-example---multi-region-multi-master-multiple-write-region-account"></a>Пример выставления счетов — учетная запись с несколькими регионами (несколько регионов для записи)

Этот пример отражает [цены на несколько хозяев](https://azure.microsoft.com/pricing/details/cosmos-db/) для учетных записей, созданных после 1 декабря 2019 г. 
- Предположим, что в учетной записи бесплатного уровня мы создаем базу данных или контейнер с 1200 единиц запросов в секунду и 10 ГБ хранилища. Мы реплицируем учетную запись в 3 региона, и у нас есть учетная запись с несколькими хозяевами (регион для нескольких операций записи). 
- В общем, без уровня "бесплатный" плата будет взиматься за 3 * 1200 единиц запросов в секунду на 3600 единиц запроса в секунду и 3 * 10 ГБ = 30 ГБ хранилища.
- За счет скидки на бесплатный уровень после удаления 400 единиц запросов в секунду и 5 ГБ хранилища плата будет взиматься за действующий 3200 единиц запросов в секунду (32 единицы) для подготовленной пропускной способности в пределах нескольких регионов записи и 25 ГБ хранилища.
- Ежемесячные затраты на единицу запросов в секунду: 32 единиц * $0,016 * 24 часа * 31 день = $380,93. Ежемесячные затраты на хранение: 25 ГБ * 0,25/ГБ = $6,25. Общая стоимость составит $380,93 + $6,25 = $387,18.
...
|----|----|----|----|
|Тип операции| Число запросов в секунду| СР. единиц запросов на запрос| Необходимо ЕЗ|
|Запись| 100 | 5 | 500|
|Чтение| 400| 1| 400|

Всего единиц запросов в секунду: 500 + 400 = 900 Почасовая стоимость: 900/100 * $0,008 = $0,072 ожидаемые ежемесячные затраты на пропускную способность (при условии 31 дня): $0,072 * 24 * 31 = $53,57
...
*Цены могут отличаться в зависимости от региона. Актуальные цены см. на [странице с ценами](https://azure.microsoft.com/pricing/details/cosmos-db/).*
...
|----|----|----|----|----|
|Восток США|0,008 долл. США |50 тыс.|4 долл. США|2880 долл. США |
...
* Стоимость пропускной способности (с оплатой по мере использования): 100 000 единиц запросов в секунду/100 * $0.008/час * 8760 часов в году = $70 080 
