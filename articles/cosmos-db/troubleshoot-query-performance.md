...
- Для достижения наилучшей производительности следуйте рекомендациям по [повышению производительности](performance-tips.md).
    > [!NOTE] 
    > Для повышения производительности рекомендуется использовать Windows 64-разрядную обработку узлов. Пакет SDK для SQL содержит собственную ServiceInterop. dll для локального анализа и оптимизации запросов, а также поддерживается только на платформе Windows x64. Для Linux и других неподдерживаемых платформ, в которых недоступна библиотека ServiceInterop. dll, она выполняет дополнительный сетевой вызов шлюза для получения оптимизированного запроса. 
- Запрос Cosmos DB не поддерживает минимальное число элементов.
    - Код должен поддерживать любой размер страницы от 0 до максимального числа элементов
    - Количество элементов на странице может изменяться и меняться без уведомления.
- Для запросов ожидаются пустые страницы, которые могут отображаться в любое время. 
    - Причина, по которой пустые страницы отображаются в пакетах SDK, позволяет больше возможностей отменить запрос. Также ясно, что пакет SDK выполняет несколько сетевых вызовов.
    - Пустые страницы могут отображаться в существующих рабочих нагрузках, так как физическая Секция разделяется в Cosmos DB. Первая секция теперь имеет 0 результатов, что приводит к пустой странице.
    - Пустые страницы вызываются с помощью серверной части, занимающего запрос, поскольку запрос занимает больше определенного количества времени на серверной части для получения документов. Если Cosmos DB загружает запрос, он вернет маркер продолжения, который позволит продолжить выполнение запроса. 
- Убедитесь, что запрос полностью остановлен. Просмотрите примеры пакета SDK и используйте цикл while на `FeedIterator.HasMoreResults` для очистки всего запроса.
...
При наличии большого количества подготовленных единиц запросов (свыше 30 000) или большого объема хранимых данных (свыше примерно 100 ГБ) у вас, вероятно, будет достаточно большого размера контейнера, чтобы увидеть значительное снижение расходов на запросы единиц запросов.
