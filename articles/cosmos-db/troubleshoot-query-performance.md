...
- [Убедитесь, что политика индексирования включает необходимые пути](#ensure-that-the-indexing-policy-includes-necessary-paths)

- [Сведения о том, какие системные функции используют индекс](#understand-which-system-functions-utilize-the-index)

- [Оптимизация запросов с помощью предложения Filter и ORDER BY](#optimize-queries-with-both-a-filter-and-an-order-by-clause)

- [Оптимизация запросов, использующих DISTINCT](#optimize-queries-that-use-distinct)
...
- [Оптимизация запросов, имеющих фильтр по нескольким свойствам](#optimize-queries-that-have-a-filter-on-multiple-properties)
...
- [Повышение близости между приложением и Azure Cosmos DB](#improving-proximity-between-your-app-and-azure-cosmos-db)

- [Увеличение подготовленной пропускной способности](#increasing-provisioned-throughput)

- [Повышение Максконкурренци](#increasing-maxconcurrency)

- [Повышение MaxBufferedItemCount](#increasing-maxbuffereditemcount)

## <a name="optimizations-for-queries-where-retrieved-document-count-significantly-exceeds-output-document-count"></a>Оптимизации для запросов, в которых количество полученных документов значительно превышает число выходных документов:
...
## <a name="ensure-that-the-indexing-policy-includes-necessary-paths"></a>Убедитесь, что политика индексирования включает необходимые пути
...
## <a name="optimize-queries-with-both-a-filter-and-an-order-by-clause"></a>Оптимизация запросов с помощью предложения Filter и ORDER BY
...
## <a name="optimize-queries-that-use-distinct"></a>Оптимизация запросов, использующих DISTINCT

Более эффективным будет поиск `DISTINCT` набора результатов, если дублирующиеся результаты являются последовательными. Добавление в запрос предложения `ORDER BY` и составного индекса обеспечит последовательную последовательность повторяющихся результатов. Если необходимо `ORDER BY` несколько свойств, добавьте составной индекс. Вы можете наблюдать за последствиями выполнения запроса к набору данных [информации питании](https://github.com/CosmosDB/labs/blob/master/dotnet/setup/NutritionData.json) .
...
## <a name="optimizations-for-queries-where-retrieved-document-count-is-approximately-equal-to-output-document-count"></a>Оптимизации для запросов, в которых количество полученных документов приблизительно равно числу выходных документов:
...
## <a name="optimize-queries-that-have-a-filter-on-multiple-properties"></a>Оптимизация запросов, имеющих фильтр по нескольким свойствам
...
## <a name="common-optimizations-that-reduce-query-latency-no-impact-on-ru-charge"></a>Распространенные оптимизации, сокращающие задержку запросов (без влияния на ЕДИНИЦу оплаты):
...
## <a name="improving-proximity-between-your-app-and-azure-cosmos-db"></a>Повышение близости между приложением и Azure Cosmos DB
...
## <a name="increasing-provisioned-throughput"></a>Увеличение подготовленной пропускной способности
...
## <a name="increasing-maxconcurrency"></a>Повышение Максконкурренци
...
## <a name="increasing-maxbuffereditemcount"></a>Повышение MaxBufferedItemCount
