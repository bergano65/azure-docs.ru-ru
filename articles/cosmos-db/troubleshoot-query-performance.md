...
Оптимизацию запросов в Azure Cosmos DB можно разделить на следующие широкие категории.

- Оптимизация, снижающая затраты единиц запроса (RU) на запрос.
- Оптимизация, которая просто сокращает задержку.

Сокращение количества единиц запроса также почти наверняка сокращает задержку.

В этой статье приведены примеры, которые можно повторно создать с помощью набора данных [о питании](https://github.com/CosmosDB/labs/blob/master/dotnet/setup/NutritionData.json).
...
- Для достижения оптимальной производительности выполните следующие [Советы по повышению производительности](performance-tips.md).
    > [!NOTE]
...
- Пакет SDK позволяет задать `MaxItemCount` для запросов, но не позволяет указать минимальное число элементов.
    ...
    - Количество элементов на странице всегда будет меньше или равно указанному `MaxItemCount`. Однако `MaxItemCount` является только максимумом, и количество результатов может быть меньше этого значения.
...
- У всех запросов есть токен продолжения, позволяющий продолжить выполнение запроса. Не забудьте полностью очистить запрос. Взгляните на примеры пакета SDK и используйте цикл `while` на `FeedIterator.HasMoreResults` для очистки всего запроса.
...
Вы можете в любое время добавлять свойства в политику индексации без влияния на доступность или производительность записи. Если добавить в индекс новое свойство, то запросы, использующие это свойство, немедленно станут использовать новый доступный индекс. Запрос будет использовать новый индекс в процессе построения последнего. Поэтому результаты запроса могут быть несогласованными, пока выполняется перестроение индекса. Если индексируется новое свойство, запросы, использующие только существующие индексы, не будут затронуты при перестроении индекса. Можно [отслеживать ход преобразования индекса](https://docs.microsoft.com/azure/cosmos-db/how-to-manage-indexing-policy#use-the-net-sdk-v3).

### <a name="understand-which-system-functions-use-the-index"></a>Узнайте, какие системные функции используют индекс.

Если выражение может быть преобразовано в диапазон строковых значений, значит, оно может использовать индекс. В противном случае оно этого не может.

Вот список некоторых распространенных строковых функций, которые могут использовать индекс:
...
Другие части запроса могут по-прежнему использовать индекс, даже если системные функции этого не делают.
