...
- DB-запросы Azure Cosmos DB не поддерживают минимальное количество элементов.
    - Код должен обрабатывать любой размер страницы, от нуля до максимального количества элементов.
    - Количество элементов на странице может и будет меняться без предварительного уведомления.
- Пустые страницы, как ожидается, для запросов и могут отображаться в любое время.
    - Пустые страницы разоблачаются в SDK, поскольку эта экспозиция предоставляет больше возможностей для отмены запроса. Это также дает понять, что SDK выполняет несколько сетевых вызовов.
    - Пустые страницы могут отображаться в существующих рабочих нагрузках, поскольку физическая часть разделена в Azure Cosmos DB. Первая разделбудет будет иметь нулевые результаты, что вызывает пустую страницу.
    - Пустые страницы вызваны упреждения упреждающего запроса, потому что запрос занимает больше, чем некоторое фиксированное количество времени на бэкэнд для получения документов. Если Azure Cosmos DB предвосхищает запрос, он возвращает маркер продолжения, который позволит продолжить запрос.
- Не забудьте полностью истощить запрос. Посмотрите на образцы SDK `while` и `FeedIterator.HasMoreResults` используйте цикл, чтобы истощить весь запрос.
...
Вот список функций строки, которые могут использовать индекс:
