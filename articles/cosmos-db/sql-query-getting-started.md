...
Второй элемент использует `givenName` , `familyName` а вместо `firstName` и `lastName`.
...
Следующий запрос переформатирует выходные данные JSON в другую форму. Запрос проецирует новый объект JSON `Family` с двумя выбранными `Name` полями и `City`, когда город адреса совпадает с состоянием. «Москва» соответствует этому варианту.
...
* Так как API SQL работает с значениями JSON, он взаимодействует с сущностями в форме дерева, а не со строками и столбцами. Вы можете ссылаться на узлы дерева в любой произвольной глубине, `Node1.Node2.Node3…..Nodem`например, аналогично ссылке из `<table>.<column>` двух частей в ANSI SQL.
