...
`givenName` Второй элемент использует `familyName` , а вместо `firstName` и. `lastName`
...
Следующий запрос переформатирует выходные данные JSON в другую форму. Запрос проецирует новый объект JSON `Family` с двумя выбранными полями `Name` и `City`, когда город адреса совпадает с состоянием. «Москва» соответствует этому варианту.
