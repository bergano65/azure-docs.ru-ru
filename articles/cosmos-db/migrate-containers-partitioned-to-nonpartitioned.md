...
При выполнении запроса из пакета SDK v3 для элементов, которые вставляются с помощью пакета SDK v2, или элементов, вставленных с помощью пакета SDK v3 с параметром `PartitionKey.None`, запрос Count может потреблять больше единиц запросов/с, если параметр `PartitionKey.None` указан в FeedOptions. Рекомендуется не указывать параметр `PartitionKey.None`, если другие элементы не вставляются с ключом секции.

Если новые элементы вставляются с разными значениями для ключа секции, запрос на количество таких элементов путем передачи соответствующего ключа в `FeedOptions` не будет иметь каких-либо проблем. Если после вставки новых документов с ключом секции необходимо выполнить запрос только к количеству документов без значения ключа секции, этот запрос может вызвать больше единиц запросов в секунду, аналогичных обычным секционированным коллекциям.
