...
При запросе из пакета SDK v3 для элементов, которые вставляются с помощью пакета SDK v2, или элементов, вставленных с помощью пакета SDK v3 `PartitionKey.None` с параметром, запрос Count может ПОТРЕБЛЯТЬ больше единиц запросов/с `PartitionKey.None` , если параметр указан в FeedOptions. Рекомендуется не указывать `PartitionKey.None` параметр, если никакие другие элементы не вставляются с ключом секции.
