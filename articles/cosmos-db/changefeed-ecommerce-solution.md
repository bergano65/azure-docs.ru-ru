...
description: В этой статье описывается, как канал изменений может быть использован розничной компанией для понимания шаблонов пользователей, выполнения анализа и визуализации данных в режиме реального времени
author: SnehaGunda
...
2. **Космос DB:** Генерируемые данные хранятся в контейнере Azure Cosmos.  

3. **Изменение ленты:** В ленте изменений будут прослушиваться изменения в контейнере Azure Cosmos. Каждый раз, когда новый документ добавляется в коллекцию (то есть когда происходит такое событие, как просмотр товара пользователем, добавление товара в корзину и его покупка), канал изменений вызывает [функцию Azure](../azure-functions/functions-overview.md).  
...
1. Перейдите на [портал Azure](https://portal.azure.com/) и найдите **dB-аккаунт Azure Cosmos,** созданный развертыванием шаблонов.  
...
При создании нового документа или изменении текущего документа в контейнере Cosmos канал изменения автоматически добавляет этот измененный документ в историю изменений коллекции. Сейчас вы создадите и запустите функцию Azure, которая обрабатывает канал изменений. При создании или изменении документа в созданной коллекции канал изменений активирует функцию Azure. Затем функция Azure отправит измененный документ в концентратор событий.
...
7. Если вы переходите на [портал Azure,](https://portal.azure.com/) затем на учетную запись Cosmos DB в вашей группе ресурсов, затем в **Data Explorer,** вы увидите рандомизированные данные, импортированные в вашем **changefeedlabcollection.**
...
1. С [портала Azure](https://portal.azure.com/)перейдите в группу ресурсов, затем в **streamjob1** (задание аналитики потоков, созданное в прелабе).  
...
6. В поле **Тип визуализации** выберите **Линейчатая диаграмма с группировкой** в раскрывающемся меню. В разделе **Оси** добавьте действие. Оставьте поле **Условные обозначения** без изменений. Затем, под следующим разделом под названием **Значение,** добавить **avg**. Выберите **Следующий,** затем название диаграммы, и выберите **Применить**. Вы должны увидеть новую диаграмму на панели мониторинга.  
...
Теперь вы увидите, как вы можете использовать свой новый инструмент анализа данных для подключения к реальному сайту электронной коммерции. Чтобы создать сайт электронной коммерции, используйте базу данных Azure Cosmos для хранения списка категорий товаров (женский, мужской, Unisex), каталога продуктов и списка наиболее популярных товаров.

1. Перейдите назад к [порталу Azure](https://portal.azure.com/), затем к **вашей учетной записи Cosmos DB,** затем к **Data Explorer.**  

   Добавьте две коллекции в соответствии с**продуктами** и категориями **changefeedlabdatabase** - и **категориями** с фиксированной емкостью хранения.
...
Чтобы удалить ресурсы, созданные в этой лаборатории, перейдите к группе ресурсов на [портале Azure,](https://portal.azure.com/)затем выберите **группу ресурсов Delete** из меню в верхней части страницы и следуйте инструкциям.
