...
3. **Канал изменений:** Веб-канал изменений будет прослушивать изменения в контейнере Azure Cosmos. Каждый раз, когда новый документ добавляется в коллекцию (то есть когда происходит такое событие, как просмотр товара пользователем, добавление товара в корзину и его покупка), канал изменений вызывает [функцию Azure](../azure-functions/functions-overview.md).  
...
6. **Azure Stream Analytics:** Azure Stream Analytics определяет запросы для обработки событий и анализа данных в реальном времени. Затем эти данные отправляются в [Microsoft Power BI](https://docs.microsoft.com/power-bi/desktop-what-is-desktop).  

7. **Power BI:** Power BI используется для визуализации данных, отправленных Azure Stream Analytics. Вы можете создать панель мониторинга, чтобы увидеть, как метрики изменяются в реальном времени.  
...
1. Перейдите на [портал Azure](https://portal.azure.com/) и найдите **учетную запись Azure Cosmos DB**, созданную при развертывании шаблона.  
...
3. Добавьте имена **коллекции** и **базы данных**. (Эти имена должны быть **changefeedlabcollection** и **changefeedlabdatabase**, если вы не решите назвать их по-другому.)
   ...
   ![Обновить строки подключения](./media/changefeed-ecommerce-solution/update-connection-string.png)
...
7. Если вы перейдете на [портал Azure](https://portal.azure.com/), а затем в учетную запись Cosmos DB в группе ресурсов и **обозреватель данных**, вы увидите случайные данные, импортированные в **changefeedlabcollection**.
...
1. На [портале Azure](https://portal.azure.com/) перейдите к своей группе ресурсов, а затем в **streamjob1** (задание потоковой аналитики, которое вы создали при настройке задания).  
...
7. Теперь, если вы хотите визуализировать больше метрики, можете вернуться к **streamjob1** и создать еще три набора выходных данных со следующими полями.
   ...
   1\. **Выходной псевдоним** — incomingRevenueOutput, имя набора данных — incomingRevenue, имя таблицы — incomingRevenue.  
   2\. **Выходной псевдоним** — top5Output, имя набора данных — top5, имя таблицы — top5.  
   В. **Выходной псевдоним** — uniqueVisitorCountOutput, имя набора данных — uniqueVisitorCount, имя таблицы — uniqueVisitorCount.
...
1. Вернитесь на [портал Azure](https://portal.azure.com/), перейдите в вашу **базу данных Cosmos DB**, а затем в **обозреватель данных**.  
...
Чтобы удалить ресурсы, созданные в ходе выполнения этого задания, перейдите к группе ресурсов на [портале Azure](https://portal.azure.com/), а затем щелкните **Удалить группу ресурсов** в меню в верхней части страницы и следуйте инструкциям.
