...
### <a name="high-network-latency"></a>Высокая задержка сети
...
Если [время ожидания](#request-timeouts) отсутствует, и диагностика отображают отдельные запросы, где большая задержка очевидна по разнице между `ResponseTime` и `RequestStartTime`, например (> 300 миллисекунд в этом примере):
...
### <a name="request-rate-too-large"></a>Слишком большая частота запросов
