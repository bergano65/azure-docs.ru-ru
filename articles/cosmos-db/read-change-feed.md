...
ms.locfileid: "83697344"
---
# <a name="reading-azure-cosmos-db-change-feed"></a>Чтение канала изменений Azure Cosmos DB

Вы можете работать с веб-каналом изменений Azure Cosmos DB, используя модель принудительной отправки или модель извлечения. При использовании модели отправки клиент запрашивает работу с сервера и содержит бизнес-логику для обработки изменений. Однако сложность проверки изменений и сохранения состояния для последних обработанных изменений обрабатывается на сервере.

При использовании модели извлечения сервер запрашивает работу, зачастую из центральной рабочей очереди. В данном случае клиент не только содержит бизнес-логику для обработки изменений, но также сохраняет состояние для последнего обработанного изменения, выполняя балансировку нагрузки для нескольких клиентов, обрабатывающих изменения параллельно, и обрабатывая ошибки.
