...
* **MongoRequests**: Выберите эту опцию для входа в запросы, инициированные пользователем, с переднего конца для обслуживания запросов на API Azure Cosmos DB для MongoDB, этот тип журнала недоступен для других учетных записей API. Запросы MongoDB будут отображаться в MongoRequests, а также DataPlaneRequests. Ключевые свойства, `Requestcharge`чтобы `opCode`отметить: .
