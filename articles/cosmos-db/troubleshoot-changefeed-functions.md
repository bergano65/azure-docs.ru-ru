...
## <a name="consuming-the-cosmos-db-sdk-separately-from-the-trigger-and-bindings"></a>Использование пакета SDK для Cosmos DB отдельно от триггера и привязки
...
> **Триггер Azure Cosmos DB, по умолчанию не будет повторно пакет изменений, если возникло необработанное исключение** во время выполнения вашего кода. Это означает, что изменения не поступало в месте назначения, так как не удалось обработать их.
