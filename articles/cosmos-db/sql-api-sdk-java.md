---
title: 'Azure Космос DB: API Java, SDK & ресурсы'
...
|**Документация API**|[Справочная документация по API Java](/java/api/com.microsoft.azure.documentdb)|
...
* Исправлена проблема тайм-аута пула подключения.
* Исправления auth маркер обновления на внутренних retries.

### <a name="246"></a><a name="2.4.6"/>2.4.6
* Обновлено правильное тег политики реплики стороны клиента на databaseAccount и сделано databaseAccount конфигурация читает из кэша.

### <a name="245"></a><a name="2.4.5"/>2.4.5
* Избегая повторной попытки на недействительной ошибке диапазона ключа раздела, если пользователь предоставляет pkRangeId.

### <a name="244"></a><a name="2.4.4"/>2.4.4
* Оптимизированный кэш ключа раздела обновляется.
* Исправлен сценарий, при котором SDK не развлекает щелочное сплит-подсказку с сервера и приводит к неправильному обновлению кэша маршрутификации стороны клиента.

### <a name="242"></a><a name="2.4.2"/>2.4.2
* Оптимизированный кэш коллекции обновляется.

### <a name="241"></a><a name="2.4.1"/>2.4.1
* Добавлена поддержка для получения внутреннего сообщения исключения из строки диагностики запроса.

### <a name="240"></a><a name="2.4.0"/>2.4.0
* Представлена версия api на PartitionKeyDefinition.

### <a name="230"></a><a name="2.3.0"/>2.3.0
* Добавлена отдельная поддержка тайм-аута для прямого режима.

### <a name="223"></a><a name="2.2.3"/>2.2.3
* Потребление сообщения об ошибке с нулевой из службы и создание исключения клиента документа.

### <a name="222"></a><a name="2.2.2"/>2.2.2
* Улучшение соединения розетки, добавляя SoKeepAlive по умолчанию верно.

### <a name="220"></a><a name="2.2.0"/>2.2.0
* Добавлена поддержка строки диагностики диагностики запроса.
...
* Исправлена ошибка в глобальном менеджере конечных точек, чтобы заставить обновить.
* Исправлена ошибка для upserts с предварительными условиями в прямом режиме.

### <a name="211"></a><a name="2.1.1"/>2.1.1
* Исправлена ошибка в кэше адреса шлюза.
...
* Исправлена ошибка, чтобы гарантировать, что queryIteror не возвращает нули.
...
* Исправлена ошибка, при которой запрос в режиме DirectHttps может перестать отвечать при возникновении сбоев дроссельной заслонки.
...
* Проверка свойства идентификатора для всех ресурсов. Идентификаторы ресурсов не могут содержать знаки ?, /, #, \, или заканчиваться пробелом.
...
> [!WARNING]
> Все версии **1.x** S'L SDK для Java будут выведены из эксплуатации **30 мая 2020 года.**
> 
>
