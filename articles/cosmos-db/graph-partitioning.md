---
title: Секционирование в API Gremlin для Azure Cosmos DB | Документация Майкрософт
...
- **И вершины, и ребра хранятся в виде документов JSON** внутри контейнера API Gremlin Azure Cosmos DB.
...
- **В запросах графа необходимо задавать ключ секции**. Чтобы воспользоваться всеми преимуществами горизонтального секционирования в Azure Cosmos DB, ключ секции необходимо по возможности указывать всякий раз, когда выбирается одна вершина. Ниже приведены запросы для выбора одной или нескольких вершин в секционированном графе:
    ...
    - `/id` и `/label` не поддерживаются в качестве ключей разделов для контейнера в Gremlin API.
...
В этой статье мы предоставили обзор концепций и рекомендации для секционирования с использованием API Gremlin Azure Cosmos DB. 
