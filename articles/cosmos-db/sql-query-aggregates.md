...
API SQL поддерживает следующие агрегатные функции. `SUM`и `AVG` используют числовые значения, и `COUNT`, `MIN`и `MAX` работают с числами, строками, логическими значениями и значениями NULL.
...
Эти статистические системные функции получают преимущества от [индекса диапазона](index-policy.md#includeexclude-strategy). `COUNT`Если предполагается `SUM`использовать свойство,, `MIN`, `MAX`или `AVG` , необходимо [включить соответствующий путь в политику индексирования](index-policy.md#includeexclude-strategy).
