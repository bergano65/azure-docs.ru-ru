...
- Документы C-CDA преобразуются в [ресурсы HL7 FHIR](https://hl7.org/fhir/2017Jan/resourcelist.html) в формате JSON.
...
2. **Приложение EHR**: отправляет запрос в репозиторий FHIR в Azure Cosmos DB и сохраняет ответ в очередь служебной шины. Это приложение логики использует [приложение API](#api-app) для получения новых и измененных документов.
...
2. **Приложение EHR**: отправляет запрос в репозиторий FHIR в Azure Cosmos DB и сохраняет ответ в очередь служебной шины. Ниже приводится код приложения GetNewOrModifiedFHIRDocuments.
...
- целое число: Number of documents returned (Число обработанных документов).

**Outputs**
- Выполнение: код состояния: 200, ответ: список документов (массив JSON);
- сбой: код состояния: 404, ответ: No Documents found for '*resource name'* Resource Type" (Не найдены документы с типом ресурса "имя_ресурса").
...
На следующем рисунке показано, как приложение Swagger использовалось для тестирования [FhirNotificationApi](#api-app-source).
...
Дополнительные сведения о базе данных Azure Cosmos DB см. на [ее главной странице](https://azure.microsoft.com/services/cosmos-db/). Подробную информацию о приложениях Azure Logic Apps см. [здесь](https://azure.microsoft.com/services/logic-apps/).
