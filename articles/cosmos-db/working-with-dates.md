...
Azure Cosmos DB поддерживает типы JSON, такие как строка, число, логическое значение, null, массив, объект. Он не поддерживает непосредственно тип DateTime. В настоящее время Azure Cosmos DB не поддерживает локализацию дат. Поэтому необходимо хранить значения DateTime в виде строк. Рекомендуемый формат для строк DateTime в Azure Cosmos DB соответствует `YYYY-MM-DDThh:mm:ss.sssZ` стандарту ISO 8601 UTC. Рекомендуется хранить все даты в Azure Cosmos DB как время в формате UTC. Преобразование строк даты в этот формат позволит сортировать даты лексикографически. Если хранятся даты, отличные от времени в формате UTC, логика должна обрабатываться на стороне клиента. Чтобы преобразовать локальную дату и время в формат UTC, смещение должно быть известно или сохранено как свойство в JSON, а клиент может использовать смещение для расчета значения DateTime в формате UTC.
