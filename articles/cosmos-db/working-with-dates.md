...
Помимо основных типов, многим приложениям требуется тип DateTime для представления дат и отметок времени. В этой статье описывается, как разработчики могут хранить, извлекать и искать даты в Azure Cosmos DB с помощью пакета SDK для .NET.

## <a name="storing-datetimes"></a>Хранение значений даты и времени

Azure Cosmos DB ограничивается типами JSON, которые не включают тип DateTime. Таким образом, в Azure Cosmos DB даты должны храниться в виде строк. В настоящее время Azure Cosmos DB не поддерживает локализацию дат. Рекомендуемый формат для строк DateTime в Azure Cosmos DB соответствует `YYYY-MM-DDThh:mm:ss.sssZ` стандарту ISO 8601 UTC. Форматирование строк в этом формате позволит сортировать даты лексикографически. Логика для обработки дат, отличных от времени в формате UTC, должна быть определена клиентом. Многим приложениям вполне достаточно используемого по умолчанию строкового представления дат и времени, поскольку оно:
