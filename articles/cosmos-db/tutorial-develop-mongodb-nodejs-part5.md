---
title: Создание приложения MongoDB на Node.Js с помощью Angular (часть 5)
...
ms.locfileid: "53134137"
---
# <a name="create-a-mongodb-app-with-angular-and-azure-cosmos-db---part-5-connect-to-azure-cosmos-db"></a>Создание приложения MongoDB с помощью Angular и Azure Cosmos DB, часть 5 Подключение к Azure Cosmos DB 

Из этого руководства в нескольких частях вы узнаете, как создать приложение Node.js с использованием Express и Angular и подключить его к учетной записи [API MongoDB для Azure Cosmos DB](mongodb-introduction.md).
...
> * Получение сведений о строке подключения Cosmos DB.
...
Просмотрите следующий видеоролик, чтобы быстро изучить шаги, описанные в этом документе: 
...
1. Установите модуль npm mongoose (программный интерфейс, который используется для обмена данными с MongoDB).

    ```bash
    npm i mongoose --save
    ```

2. Теперь создайте файл с именем **mongo.js** в папке **server**. Затем добавьте в этот файл сведения о подключении учетной записи Cosmos DB.
...
3. Скопируйте код ниже в **mongo.js**. Этот код выполняет следующие действия:
    ...
    * Вызывает файл env, который позволяет задавать определенные настройки в зависимости от среды — промежуточной, производственной или среды разработки. Вы создадите этот файл при работе со следующим разделом.

    * Содержит строку подключения MongoDB, которая задается в файле env.
...
2. Скопируйте код ниже в **hero.model.js**. Этот код предоставляет следующие функциональные возможности:
...
3. В **hero.service.js** обновите функцию getHeroes, чтобы она принимала параметры `req` и `res`, как показано ниже:
    ...
    Давайте выделим пару минут на проверку и просмотрим цепочку вызовов. Цепочка начинается с файла `index.js`, в котором настраивается сервер узла, и записи, которая настраивает и определяет маршруты. Затем файл routes.js обращается к службе Hero и дает ей команду получить функции, например getHeroes, а также передать запрос и ответ. Теперь наступает черед файла hero.service.js. Он захватывает модель и подключается к Mongo. При вызове он выполняет команду getHeroes и возвращает ответ 200. Затем он занимает свое место в цепочке. 
...
3. Теперь в браузере откройте окно инструментов для разработчиков, выберите вкладку сети и перейдите по адресу http://localhost:3000. Вот и наше приложение.
...
В этой части руководства вы выполнили следующие задачи:
