...
1. API etcd в Azure Cosmos DB в настоящее время находится в режиме предварительной версии. Зарегистрируйтесь,перейдя по адресу https://aka.ms/cosmosetcdapi-signup, чтобы использовать предварительную версию. После отправки формы ваша подписка будет добавлена в утвержденный список для использования API etcd Azure Cosmos. 
...
1. При наличии нескольких подписок переключитесь на подписку, внесенную в утвержденный список API etcd Azure Cosmos DB. Вы можете переключиться на нужную подписку с помощью следующей команды:
