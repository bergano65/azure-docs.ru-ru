...
1. Разверните кластер Azure Kubernetes, использующий Azure Cosmos DB, выполнив следующую команду:
   ...
   * **подписка-id:** Идентификатор подписки Azure с включенным API API Azure Cosmos DB и т.д.
   * **клиент-id:** AppId директора службы. Идентификатор `appId` возвращен в качестве выходных данных на шаге 4.
   * **Клиент-секрет:** Пароль директора службы или случайно сгенерированный пароль. Это значение было возвращено в качестве выходных данных в параметре password на шаге 4. 
   * **dnsPrefix:** Уникальное название DNS. Это значение будет частью имени узла (например, myprod1, staging).
   * **местонахождение:**  Место, где кластер должен быть развернут, в настоящее время поддерживается только "центрик".

   > [!Note]
   > В настоящее время API etcd Azure Cosmos доступен для развертывания только в регионе centralus. 
 
   * **апи-модель:** Полностью квалифицированный путь к файлу шаблона.
   * **принудительного перезаписи:** Эта опция используется для автоматического перезаписи существующих файлов в каталоге вывода.
