...
> [!NOTE]
> Табличное хранилище REST API операции также возвращают `ETag` значение, производное от LMT. В этом документе термины ETag и LMT используются взаимозаменяемыми, так как они ссылаются на одни и те же базовые данные.  
> 
> 
...
Выбранный вариант `PartitionKey` и `RowKey` является фундаментальным для хорошего проектирования таблиц. Каждая сущность, хранящаяся в таблице, должна иметь уникальное `PartitionKey` сочетание `RowKey`и. Как `PartitionKey` и в случае с ключами в таблице реляционной базы `RowKey` данных, значения и индексируются для создания кластеризованного индекса, который обеспечивает быстрый поиск. Однако хранилище таблиц не создает вторичных индексов, поэтому это единственное два индексированных свойства (некоторые закономерности, описанные ниже, показывают, как можно обойти это очевидное ограничение).  

Таблица состоит из одной или нескольких секций, и многие из решений по проектированию будут выбирать подходящий `PartitionKey` вариант и `RowKey` оптимизировать решение. Решение может состоять только из одной таблицы, содержащей все сущности, упорядоченные по секциям, но обычно решение имеет несколько таблиц. Таблицы помогают логически упорядочивать сущности, а также помогают управлять доступом к данным с помощью списков управления доступом. Можно удалить всю таблицу с помощью одной операции хранилища.  
...
* **Укажите `PartitionKey` и в `RowKey` запросах, и.** *Запросы на указание* , такие как, являются наиболее эффективными запросами к хранилищу таблиц.  
...
* **Используйте значения составного ключа.** Доступны `PartitionKey` только ключи и `RowKey`. Например, значения составного ключа можно использовать для включения альтернативных путей доступа с ключами к сущностям.  
...
### <a name="how-your-choice-of-partitionkey-and-rowkey-affects-query-performance"></a>Выбор `PartitionKey` и `RowKey` влияние на производительность запросов
В следующих примерах предполагается, что хранилище таблиц хранит сущности сотрудников со следующей структурой (в большинстве примеров `Timestamp` это свойство не используется для ясности):  
...
* *Запрос к точке* является наиболее эффективным поиском, и его рекомендуется использовать для операций поиска большого объема или уточняющих запросов, для которых требуется наименьшая задержка. Такой запрос может использовать индексы для эффективного поиска отдельной сущности, указывая значения `PartitionKey` и. `RowKey` Например: `$filter=(PartitionKey eq 'Sales') and (RowKey eq '2')`.  
* Второй лучший вариант — *запрос Range*. Он использует `PartitionKey`фильтры и для диапазона `RowKey` значений, чтобы вернуть более одной сущности. `PartitionKey` Значение определяет конкретную секцию, а `RowKey` значения определяют подмножество сущностей в этой секции. Например: `$filter=PartitionKey eq 'Sales' and RowKey ge 'S' and RowKey lt 'T'`.  
* Третье лучше — *Просмотр секций*. Он использует фильтры `PartitionKey`, и для другого неключевого свойства и может возвращать более одной сущности. `PartitionKey` Значение определяет конкретную секцию и значения свойств, которые выбираются для подмножества сущностей в этой секции. Например: `$filter=PartitionKey eq 'Sales' and LastName eq 'Smith'`.  
* *Просмотр таблицы* не включает `PartitionKey`, и является неэффективным, поскольку он выполняет поиск всех секций, составляющих таблицу, для любых совпадающих сущностей. Он выполняет просмотр таблицы независимо от того, использует ли фильтр `RowKey`. Например: `$filter=LastName eq 'Jones'`.  
* Запросы хранилища таблиц Azure, которые возвращают несколько сущностей `PartitionKey` , `RowKey` сортируют их в и порядке. Чтобы избежать перестановки сущностей в клиенте, выберите `RowKey` объект, который определяет наиболее распространенный порядок сортировки. Результаты запроса, возвращенные API таблиц Azure в Azure Cosmos DB, не сортируются по ключу секции или ключу строки. Дополнительные сведения см. в ответе на вопрос [Чем API таблиц отличается от хранилища таблиц Azure?](table-api-faq.md#table-api-vs-table-storage).

Использование "**или**" для указания фильтра на основе `RowKey` значений приводит к просмотру секции и не рассматривается как запрос диапазона. Поэтому избегайте запросов, использующих фильтры, например `$filter=PartitionKey eq 'Sales' and (RowKey eq '121' or RowKey eq '322')`:.  
...
Ваш вариант `PartitionKey` должен сбалансировать потребность в использовании EGT (чтобы обеспечить согласованность) в соответствии с требованием распределения сущностей по нескольким секциям (для обеспечения масштабируемого решения).  
...
Идеальный вариант `PartitionKey` позволяет использовать эффективные запросы и имеет достаточно секций, чтобы обеспечить масштабируемость решения. Как правило, вы обнаружите, что сущности будут иметь подходящее свойство, которое распределяет сущности по достаточному количеству секций.

> [!NOTE]
> Например, в системе, в которой хранятся сведения о пользователях или сотрудниках, `UserID` может быть `PartitionKey`хорошим. У вас может быть несколько сущностей, которые `UserID` используют определенный в качестве ключа секции. Каждая сущность, хранящая данные о пользователе, сгруппирована в одну секцию. Доступ к этим сущностям можно получить с помощью EGT, сохраняя при этом высокую масштабируемость.
> 
> 

Существуют дополнительные рекомендации по выбору `PartitionKey` , которые связаны с тем, как вставлять, обновлять и удалять сущности. Дополнительные сведения см. в разделе [Разработка для изменения данных](#design-for-data-modification) далее в этой статье.  

### <a name="optimize-queries-for-table-storage"></a>Оптимизация запросов к хранилищу таблиц
Табличное хранилище автоматически индексирует сущности с помощью значений `PartitionKey` и `RowKey` в одном кластеризованном индексе. Именно по этой причине запросы к точкам являются наиболее эффективными для использования. Однако в кластеризованном индексе в `PartitionKey` и `RowKey`не существует индексов, отличных от этого.
...
* [Шаблон вторичного индекса внутри раздела](#intra-partition-secondary-index-pattern). Хранение нескольких копий каждой сущности с использованием разных `RowKey` значений (в одной секции). Это обеспечивает быстрый и эффективный поиск, а также альтернативные порядки сортировки с использованием `RowKey` различных значений.  
* [Шаблон вторичного индекса в разных секциях](#inter-partition-secondary-index-pattern). Хранение нескольких копий каждой сущности с использованием `RowKey` различных значений в отдельных секциях или в отдельных таблицах. Это обеспечивает быстрый и эффективный поиск, а также альтернативные порядки сортировки с использованием `RowKey` различных значений.  
...
Табличное хранилище возвращает результаты запроса, отсортированные в возрастающем порядке `PartitionKey` , на основе `RowKey`и затем по.

> [!NOTE]
> Результаты запроса, возвращенные API таблиц Azure в Azure Cosmos DB, не сортируются по ключу секции или ключу строки. Дополнительные сведения см. в ответе на вопрос [Чем API таблиц отличается от хранилища таблиц Azure?](table-api-faq.md#table-api-vs-table-storage).

Ключи в хранилище таблиц — это строковые значения. Чтобы обеспечить правильную сортировку числовых значений, необходимо преобразовать их в фиксированную длину и заполнить их нулями. Например, если значение идентификатора сотрудника, используемое в `RowKey` качестве, является целочисленным значением, необходимо преобразовать идентификатор сотрудника **123** в **00000123**. 
...
* [Шаблон вторичного индекса внутри раздела](#intra-partition-secondary-index-pattern). Хранение нескольких копий каждой сущности с использованием разных `RowKey` значений (в одной секции). Это обеспечивает быстрый и эффективный поиск, а также альтернативные порядки сортировки с использованием `RowKey` различных значений.  
* [Шаблон вторичного индекса](#inter-partition-secondary-index-pattern)в разных разделах. Хранение нескольких копий каждой сущности с использованием различных `RowKey` значений в отдельных секциях в отдельных таблицах. Это обеспечивает быстрый и эффективный поиск, а также альтернативные порядки сортировки с использованием `RowKey` различных значений.
* [Шаблон заключительного фрагмента журнала](#log-tail-pattern): получение *n* сущностей, недавно добавленных в секцию `RowKey` , с использованием значения, которое сортируется в обратную дату и время.  
...
Чтобы обновить или удалить сущность, необходимо иметь возможность ее определять с помощью значений `PartitionKey` и. `RowKey` В этом отношении выбор `PartitionKey` и `RowKey` изменение сущностей должны удовлетворять аналогичным условиям для поддержки запросов точек. Необходимо, чтобы сущности определялись как можно эффективнее. Вы не хотите использовать неэффективное сканирование секций или таблиц для поиска сущности, чтобы обнаружить значения `PartitionKey` и `RowKey` , которые необходимо обновить или удалить.  
...
* [Шаблон составного ключа](#compound-key-pattern). Используйте `RowKey` составные значения, чтобы разрешить клиенту выполнять поиск связанных данных с помощью одноточечного запроса.  
* [Шаблон заключительного фрагмента журнала](#log-tail-pattern): получение *n* сущностей, недавно добавленных в секцию `RowKey` , с использованием значения, которое сортируется в обратную дату и время.  
...
Что касается таблиц, то в дополнение к политике шифрования пользователи должны указать свойства, которые необходимо зашифровать. Либо укажите `EncryptProperty` атрибут (для сущностей POCO, производных от `TableEntity`), либо укажите сопоставитель шифрования в параметрах запроса. Сопоставитель шифрования — это делегат, который принимает ключ секции, ключ строки и имя свойства и возвращает логическое значение, указывающее, должно ли это свойство быть зашифровано. Во время шифрования клиентская библиотека использует эти сведения, чтобы решить, следует ли шифровать свойство при записи в канал. Делегат также обеспечивает возможность логики в отношении того, как шифруются свойства. (Например, если X, то зашифруйте свойство A; в противном случае зашифруйте свойства A и B.) Нет необходимости предоставлять эти сведения при чтении или запросе сущностей.
...
В этом примере показана неявная связь «один ко многим» между типами на основе `PartitionKey` значения. В каждом отделе может работать много сотрудников.  
...
Если клиентское приложение использует набор классов, образующих часть отношения наследования для представления бизнес-сущностей, можно легко сохранить эти сущности в хранилище таблиц. Например, у вас может быть следующий набор классов, определенных в клиентском приложении, где `Person` является абстрактным классом.
...
Храните несколько копий каждой сущности, используя разные `RowKey` значения (в одной секции). Это обеспечивает быстрый и эффективный поиск, а также альтернативные порядки сортировки с использованием `RowKey` различных значений. Обновления между копиями можно хранить согласованно с помощью EGT.  

#### <a name="context-and-problem"></a>Контекст и проблема
Табличное хранилище автоматически индексирует сущности с помощью `PartitionKey` значений `RowKey` и. Это позволяет клиентскому приложению эффективно извлекать сущность с помощью этих значений. Например, с помощью следующей табличной структуры клиентское приложение может использовать запрос к точке для получения отдельной сущности Employee, используя название отдела и идентификатор сотрудника (значения `PartitionKey` и `RowKey` ). Клиент также может извлекать сущности, отсортированные по идентификатору сотрудника в каждом отделе.

![Изображение сущности Employee][6]

Если вы также хотите найти сущность Employee на основе значения другого свойства, например адреса электронной почты, необходимо использовать менее эффективное сканирование секций для поиска совпадения. Это связано с тем, что хранилище таблиц не предоставляет вторичные индексы. Кроме того, нет возможности запрашивать список сотрудников, отсортированных в порядке, отличном от `RowKey` Order.  

#### <a name="solution"></a>Решение
Чтобы обойти недостаток вторичных индексов, можно сохранить несколько копий каждой сущности, при этом каждая копия будет использовать другое `RowKey` значение. При хранении сущности со следующими структурами можно эффективно получать сущности сотрудников на основе адреса электронной почты или идентификатора сотрудника. Значения префикса для `RowKey`, `empid_`и `email_` позволяют запросить одного сотрудника или диапазона сотрудников, используя диапазон адресов электронной почты или идентификаторов сотрудников.  
...
* Вам не обязательно дублировать все свойства сущности. Например, если запросы, которые ищут сущности с помощью адреса электронной почты в, `RowKey` никогда не нуждаются в возрасте сотрудника, эти сущности могут иметь следующую структуру:
...
Однако не превышайте ограничения масштабируемости секций при выполнении поиска сущностей с использованием различных `RowKey` значений.  
...
Храните несколько копий каждой сущности, используя разные `RowKey` значения в отдельных секциях или в отдельных таблицах. Это обеспечивает быстрый и эффективный поиск, а также альтернативные порядки сортировки с использованием `RowKey` различных значений.  
...
Чтобы найти сущность сотрудника на основе другого свойства (например, адрес электронной почты), необходимо использовать менее эффективную функцию просмотра раздела. Это связано с тем, что хранилище таблиц не предоставляет вторичные индексы. Кроме того, нет возможности запрашивать список сотрудников, отсортированных в порядке, отличном от `RowKey` Order.  
...
#### <a name="solution"></a>Решение
Чтобы обойти недостаток вторичных индексов, можно сохранить несколько копий каждой сущности, при этом каждая копия будет использовать разные `PartitionKey` значения `RowKey` и. При хранении сущности со следующими структурами можно эффективно получать сущности сотрудников на основе адреса электронной почты или идентификатора сотрудника. Значения префикса для `PartitionKey`, `empid_`и `email_` позволяют указать, какой индекс следует использовать для запроса.  
...
Используйте этот шаблон, если вы хотите избежать превышения ограничений масштабируемости секций при выполнении поиска сущностей с использованием различных `RowKey` значений.  
...
#### <a name="context-and-problem"></a>Контекст и проблема
Табличное хранилище автоматически индексирует сущности с помощью `PartitionKey` значений `RowKey` и. Это позволяет клиентскому приложению эффективно извлекать сущность с помощью запроса к точке. Например, с помощью следующей структуры таблицы клиентское приложение может эффективно извлекать отдельные сущности сотрудников с помощью имени отдела и идентификатора сотрудника ( `PartitionKey` и `RowKey`).  
...
`EmployeeIDs` Свойство содержит список идентификаторов сотрудников для сотрудников с фамилией, `RowKey`хранящимся в.  
...
1. Получите сущность индекса со `PartitionKey` значением "Sales" и `RowKey` значением "Jones". Сохраните ETag это сущности для использования на шаге 2.  
2. Создайте транзакцию группы сущностей (то есть пакетную операцию), которая вставляет новую сущность Employee`PartitionKey` (значение "Sales" `RowKey` и Value "000152") и обновляет сущность индекса (`PartitionKey` значение "Sales" и `RowKey` value "Jones"). ТРАНЗАКЦИИ группы СУЩНОСТЕЙ делает это, добавляя новый идентификатор сотрудника в список в поле EmployeeID. Дополнительные сведения о EGT см. в разделе [транзакции группы сущностей](#entity-group-transactions).  
...
1. Получите сущность индекса со `PartitionKey` значением "Sales" и `RowKey` значением "Jones".  
2. Проанализируйте список идентификаторов сотрудников в `EmployeeIDs` поле.  
3. Если вам нужны дополнительные сведения о каждом из этих сотрудников (например, их адреса электронной почты), извлеките каждую сущность сотрудников, `PartitionKey` используя значение "продажи", `RowKey` и значения из списка сотрудников, полученных на шаге 2.  
...
* Для этого решения требуются как минимум два запроса для получения совпадающих сущностей: один для запроса сущностей индекса для получения `RowKey` списка значений, а затем запросы для получения каждой сущности в списке.  
...
Используйте составные `RowKey` значения, чтобы разрешить клиенту выполнять поиск связанных данных с помощью одноточечного запроса.  
...
Обратите внимание `RowKey` , что теперь представляет собой составной ключ, состоящие из идентификатора сотрудника и года данных проверки. Это позволяет получить данные о производительности и проверке данных сотрудника с помощью одного запроса для одной сущности.  
...
* Следует использовать подходящий символ-разделитель, который упрощает анализ `RowKey` значения: например, **000123_2012**.  
...
Извлечение *n* сущностей, недавно добавленных в секцию, с `RowKey` помощью значения, которое сортируется в обратную дату и время.  

> [!NOTE]
> Результаты запроса, возвращенные API таблиц Azure в Azure Cosmos DB, не сортируются по ключу секции или ключу строки. Таким же подходом, хотя этот шаблон подходит для хранилища таблиц, он не подходит для Azure Cosmos DB. Подробный список различий функций см. [в статье различия между API таблиц в Azure Cosmos DB и хранилищем таблиц Azure](table-api-faq.md#table-api-vs-table-storage).

#### <a name="context-and-problem"></a>Контекст и проблема
Общее требование заключается в необходимости иметь возможность извлекать последние созданные сущности. Например, десять последних заявок на возмещение сотрудникам. Запросы таблиц поддерживают операцию `$top` запроса для возврата первых *n* сущностей из набора. Нет эквивалентной операции запроса для возврата последних *n* сущностей в наборе.  

#### <a name="solution"></a>Решение
Храните сущности с помощью `RowKey` , который естественным образом сортирует в обратный порядок даты и времени, поэтому самая последняя запись всегда является первой в таблице.  

Например, чтобы извлечь десять самых последних заявок на возмещение расходов, отправленных сотрудниками, можно использовать обратное значение тика, являющееся производным от текущей даты и времени. В следующем примере кода C# показан один из способов создания подходящего "инвертированных тактов" для объекта `RowKey` , который сортирует от самого последнего к самому старому:  
...
Одна из возможных архитектур — использовать дату и время запроса на вход в `RowKey`:  
...
Обратите внимание, что в этом `PartitionKey` примере `RowKey` используются составные ключи и. `PartitionKey` Использует как отдел, так и идентификатор сотрудника для распределения журнала по нескольким секциям.  
...
#### <a name="context-and-problem"></a>Контекст и проблема
Распространенным вариантом использования данных журнала является получение выбранных записей журнала для определенного диапазона дат и времени. Например, необходимо найти все ошибки и критические сообщения, которые приложение зарегистрировало между 15:04 и 15:06, на определенную дату. Вы не хотите использовать дату и время сообщения журнала, чтобы определить секцию, в которой сохраняются сущности журнала. Это приводит к тому, что в любой момент времени все сущности журнала будут иметь одно и то же `PartitionKey` значение (см. [отпечаток в начале или в конце](#prepend-append-anti-pattern)). Например, следующая схема сущности для сообщения журнала приводит к постановке в горячую секцию, так как приложение записывает все сообщения журнала в секцию для текущей даты и часа:  

![Рисунок объекта сообщения журнала][28]

В этом примере `RowKey` включает дату и время сообщения журнала, чтобы обеспечить сортировку сообщений журнала в порядке даты и времени. `RowKey` Также включает идентификатор сообщения, если несколько сообщений журнала имеют одинаковую дату и время.  

Другой подход заключается в использовании объекта `PartitionKey` , который гарантирует, что приложение записывает сообщения в диапазон секций. Например, если источник сообщения журнала предоставляет способ распределения сообщений по нескольким секциям, можно использовать следующую схему сущности:  
...
Самый простой способ выполнить запрос к точке — использовать операцию **извлечения** таблицы. Как показано в следующем фрагменте кода C#, эта операция извлекает сущность со `PartitionKey` значением "Sales" и `RowKey` значением "212":  
...
Обратите внимание, что в запросе `RowKey` указаны как `PartitionKey` , так и, чтобы обеспечить лучшую производительность.  
...
> [!NOTE]
> В примере вложены несколько `CombineFilters` методов для включения трех условий фильтра.  
> 
> 

#### <a name="retrieve-large-numbers-of-entities-from-a-query"></a>Получение большого количества сущностей из запроса
Оптимальный запрос возвращает отдельную сущность на основе `PartitionKey` значения и `RowKey` значения. Однако в некоторых сценариях может потребоваться возврат множества сущностей из одной и той же секции или даже из нескольких секций. В таких случаях следует полностью протестировать производительность приложения.  
...
Если вы используете клиентскую библиотеку хранилища, она может автоматически выполнять маркеры продолжения, так как она возвращает сущности из хранилища таблиц. Например, следующий пример кода C# автоматически обрабатывает маркеры продолжения, если хранилище таблиц возвращает их в ответе:  
...
Обратите внимание `RowKey` , что значение доступно, несмотря на то, что оно не включено в список извлекаемых свойств.  
...
`Replace` Метод `TableOperation` класса всегда заменяет полную сущность в хранилище таблиц. Если не включить в запрос свойство, если оно существует в хранимой сущности, запрос удаляет это свойство из хранимой сущности. Каждое свойство требуется в включать в запрос до тех пор, пока оно явным образом не будет удалено из хранимой сущности.  

`Merge` Метод `TableOperation` класса можно использовать для сокращения объема данных, отправляемых в хранилище таблиц, когда требуется обновить сущность. `Merge` Метод заменяет все свойства в хранимой сущности значениями свойств из сущности, входящей в запрос. Этот метод оставляет без изменений все свойства в хранимой сущности, которые не включаются в запрос. Это полезно, если у вас есть крупные сущности, и требуется только обновить небольшое количество свойств в запросе.  
...
Каждая сущность по- `PartitionKey`прежнему `RowKey`должна иметь `Timestamp` значения, и, но может иметь любой набор свойств. Кроме того, нет ничего указывать тип сущности, если вы не решите хранить эту информацию где-то еще. Существует два варианта определения типа сущности.  

* Добавьте в начало типа сущности `RowKey` (или, возможно, `PartitionKey`значение). Например, `EMPLOYEE_000123` или `DEPARTMENT_SALES` в качестве `RowKey` значений.  
...
Первый параметр, добавляющий тип сущности в `RowKey`, полезен, если существует вероятность того, что две сущности разных типов могут иметь одно и то же значение ключа. Кроме того, он позволяет объединять сущности одного типа в раздел.  
...
Если известно, какой тип сущности хранится с конкретными `RowKey` значениями и `PartitionKey` , то при извлечении сущности можно указать тип сущности. Это было показано в двух предыдущих примерах, в которых извлекаются сущности типа `EmployeeEntity`: [выполните запрос к точке с помощью клиентской библиотеки хранилища](#run-a-point-query-by-using-the-storage-client-library) и [Извлеките несколько сущностей с помощью LINQ](#retrieve-multiple-entities-by-using-linq).  

Второй вариант — использовать `DynamicTableEntity` тип (контейнер свойств) вместо конкретного типа сущности POCO. Этот параметр также может повысить производительность, так как нет необходимости выполнять сериализацию и десериализовать сущность в типы .NET. Следующий код C# потенциально извлекает из таблицы несколько сущностей различных типов, но возвращает все сущности в виде `DynamicTableEntity` экземпляров. Затем для определения типа `EntityType` каждой сущности используется свойство.  
...
Чтобы получить другие свойства, необходимо использовать `TryGetValue` метод для `Properties` свойства `DynamicTableEntity` класса.  

Третьим вариантом является объединение с использованием `DynamicTableEntity` типа и `EntityResolver` экземпляра. Он позволяет выполнять разрешение в несколько типов POCO в рамках одного запроса. В этом примере `EntityResolver` делегат использует `EntityType` свойство, чтобы различать два типа сущности, возвращаемые запросом. `Resolve` Метод `resolver` использует делегат для разрешения `DynamicTableEntity` экземпляров в `TableEntity` экземпляры.  
...
Вам не нужно знать тип сущности для ее удаления, и вы всегда узнаете тип сущности при его вставке. Однако можно использовать `DynamicTableEntity` тип для обновления сущности, не зная ее тип, и без использования класса сущности POCO. Следующий пример кода извлекает одну сущность и проверяет, существует ли это `EmployeeCount` свойство, прежде чем обновлять его.  
...
Можно создать маркер SAS, который предоставляет доступ к подмножеству сущностей в таблице. По умолчанию создается маркер SAS для всей таблицы. Но можно также указать, чтобы маркер SAS предоставил доступ либо к диапазону `PartitionKey` значений, либо к диапазону значений `PartitionKey` и. `RowKey` Вы можете создать маркеры SAS для отдельных пользователей системы, чтобы маркер SAS каждого пользователя допускает доступ только к собственным сущностям в хранилище таблиц.  
...
В экземпляре клиента можно повысить пропускную способность, выполняя операции хранилища асинхронно. Клиентская библиотека хранилища упрощает написание асинхронных запросов и изменений. Например, вы можете начать с синхронного метода, который получает все сущности в секции, как показано в следующем коде C#:  
...
* Сигнатура метода теперь включает `async` модификатор и возвращает `Task` экземпляр.  
* Вместо вызова `ExecuteSegmented` метода для получения результатов метод теперь вызывает `ExecuteSegmentedAsync` метод. Метод использует `await` модификатор для асинхронного получения результатов.  

Клиентское приложение может вызывать этот метод несколько раз с разными значениями `department` параметра. Каждый запрос выполняется в отдельном потоке.  

В `Execute` `TableQuery` классе нет асинхронной версии метода, так как `IEnumerable` интерфейс не поддерживает асинхронное перечисление.  
...
* Сигнатура метода теперь включает `async` модификатор и возвращает `Task` экземпляр.  
* Вместо вызова `Execute` метода для обновления сущности метод теперь вызывает `ExecuteAsync` метод. Метод использует `await` модификатор для асинхронного получения результатов.  
