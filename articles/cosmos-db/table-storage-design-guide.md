...
* Запросы хранилища таблиц Azure, которые возвращают несколько сущностей `PartitionKey` , `RowKey` сортируют их в и порядке. Чтобы избежать перестановки сущностей в клиенте, выберите `RowKey` объект, который определяет наиболее распространенный порядок сортировки. Результаты запроса, возвращенные API таблиц Azure в Azure Cosmos DB, не сортируются по ключу секции или ключу строки. Дополнительные сведения см. в ответе на вопрос [Чем API таблиц отличается от хранилища таблиц Azure?](faq.md#where-is-table-api-not-identical-with-azure-table-storage-behavior).
