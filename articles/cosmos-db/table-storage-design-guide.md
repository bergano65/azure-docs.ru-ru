---
title: Разработка таблиц Azure Cosmos DB для поддержки масштабирования и производительности
description: Руководство по проектированию таблиц службы хранилища Azure. Проектирование масштабируемых и производительных таблиц в Azure Cosmos DB и таблице службы хранилища Azure
...
* Запросы службы таблиц Azure, возвращающие несколько сущностей, возвращают их отсортированными по порядку **PartitionKey** и **RowKey**. Чтобы избежать повторной сортировки сущностей в клиенте, выберите свойство **RowKey**, которое определяет самый распространенный порядок сортировки. Результаты запроса, возвращенные API таблиц Azure в Azure Cosmos DB, не сортируются по ключу секции или ключу строки. Дополнительные сведения см. в ответе на вопрос [Чем API таблиц отличается от хранилища таблиц Azure?](faq.md#where-is-table-api-not-identical-with-azure-table-storage-behavior).
...
![Сущность отдела и сотрудника][1]
...
![ER-диаграмма отношений наследования][3]
...
![Диаграмма сущности клиента и сущности сотрудника][4]
...
![Образ шаблонов для разработки таблицы][5]
...
#### <a name="solution"></a>Решение
Чтобы решить проблему отсутствия вторичных индексов, можно сохранить несколько копий каждой сущности с каждой копией, используя различные значения **PartitionKey** и **RowKey**. При хранении сущности с показанными ниже структурами можно эффективно получать сущности сотрудников на основе адреса электронной почты или идентификатора сотрудника. Значения префикса для **PartitionKey**, "empid_" и "email_" позволяют указать, какой индекс следует использовать для запроса.  

![Сущность сотрудника с первичным индексом и сущность сотрудника с вторичным индексом][10]
...
* Дублировать все свойства сущности не требуется. Например, если для запросов, которые ищут сущности по электронному адресу в **RowKey**, не требуется возраст сотрудника, то эти сущности могут иметь следующую структуру:
  ...
  ![Сущность сотрудника с вторичным индексом][11]
...
* Хранилище сущностей в службе таблиц, которое еще индексируется с помощью службы Когнитивный поиск Azure.  
...
![Сущность сотрудника со строкой, содержащей список идентификаторов сотрудников с одинаковыми фамилиями][14]
...
![Сущность сотрудника со строкой, содержащей список идентификаторов сотрудников с одинаковыми фамилиями][15]
...
![Сущность просмотра сотрудника][19]
...
![Сущность сотрудника с составным ключом][20]
...
![Сущность статистики сообщений][22]
...
![Сущность статистики сообщений с разделенными свойствами][23]
...
![Сущность статистики сообщений с Rowkey 01 и Сущность состояния сообщений с Rowkey 02][24]
...
![Сущность сотрудника со строкой для фотографии, указывающей на хранилище BLOB-объектов][25]
...
![Сущность сотрудника со свойством RowKey, объединяющим год, месяц, день, время и идентификатор события][27]
...
Методики, рассматриваемые в этом разделе, особенно актуальны для темы [Отношения наследования](#inheritance-relationships), которая обсуждалась ранее в этой статье в разделе "Моделирование отношений".  
