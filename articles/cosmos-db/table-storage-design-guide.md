...
description: Руководство по разработке таблиц хранилища Azure. Разработка масштабируемых и производительных таблиц в Azure Cosmos DB и таблице хранилища Azure
...
* Запросы службы таблиц Azure, возвращающие несколько сущностей, возвращают их отсортированными по порядку **PartitionKey** и **RowKey**. Чтобы избежать повторной сортировки сущностей в клиенте, выберите свойство **RowKey**, которое определяет самый распространенный порядок сортировки. Результаты запросов, возвращаемые API таблиц Azure в Azure Cosmso DB, не сортируются по ключу раздела или ключу строки. Дополнительные сведения см. в ответе на вопрос [Чем API таблиц отличается от хранилища таблиц Azure?](faq.md#where-is-table-api-not-identical-with-azure-table-storage-behavior).
...
Методики, рассматриваемые в этом разделе, особенно актуальны для темы [Отношения наследования](#inheritance-relationships), которая обсуждалась ранее в статье [Руководство по разработке табличных хранилищ Azure: разработка масштабируемых и высокопроизводительных таблиц](#modelling-relationships).  
