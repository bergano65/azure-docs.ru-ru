...
В общедоступной предварительной версии ссылка синапсе поддерживается для API Azure Cosmos DB SQL (Core) и для Azure Cosmos DB API для MongoDB. 
...
|Синапсе SQL Server    | Чтение, просмотр |
