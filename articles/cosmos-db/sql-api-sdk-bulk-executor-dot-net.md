...
|**Загрузка пакета SDK**| [NuGet](https://www.nuget.org/packages/Microsoft.Azure.CosmosDB.BulkExecutor/) |
...
|**Начало работы**|[Приступая к работе с пакетом SDK для .NET библиотеки пакетной среды](bulk-executor-dot-net.md)|
...
### <a name="241-preview"></a><a name="2.4.1-preview"/>2.4.1 — Предварительная версия

* Исправлена Тоталелапседтиме в ответе Булкделете для правильного измерения общего времени, включая повторные попытки.

### <a name="240-preview"></a><a name="2.4.0-preview"/>2.4.0 — Предварительная версия
...
* Исправлена проблема с Монгобулкексекутор, которая была неожиданно увеличила размер документа путем добавления заполнения и в некоторых случаях, при превышении максимального размера документа.

### <a name="187"></a><a name="1.8.7"/>1.8.7

* Исправлена проблема с Булкделетеасинк, когда коллекция имеет вложенные пути к ключам секций.
...
* Теперь трассировка использует TraceSource. Пользователи могут определять прослушиватели для `BulkExecutorTrace` источника.
