---
title: Данные о модели и перегородке на Azure Cosmos DB на примере реального мира
...
- **(C1)** Создание/отсечение пользователя
- **(No1)** Получение пользователя
- **[C2]**  — создание или изменение записи;
- **[Q2]**  — получение записи;
- **(No3)** Перечислите публикации пользователя в короткой форме
- **(C3)** Создание комментария
- **(No4)** Перечислите комментарии поста
...
На этом этапе еще не заходила речь о подробных сведениях для каждой сущности (пользователь, запись и т. д.). Этот шаг, как правило, одним из первых, которые будут решаться при проектировании против реляционных магазин, потому что мы должны выяснить, как эти лица будут переводить с точки зрения таблиц, столбцов, иностранных ключей и т.д. Это гораздо меньше проблем с базой данных документов, которая не обеспечивает соблюдение какой-либо схемы в записи.
...
| **Задержки** | **Стоимость в ЕЗ** | **Производительность** |
| --- | --- | --- |
| 7 мс | 5,71 ЕЗ | ✅ |
...
Еще раз, наш первоначальный запрос не фильтрует на `posts` ключе раздела контейнера, который вызывает дорогостоящий вентилятор-аут. Это еще хуже, как мы ориентируемся на гораздо `ORDER BY` больший набор результатов и сортировать результаты с оговоркой, что делает его более дорогим с точки зрения единиц запроса.
...
## <a name="v2-introducing-denormalization-to-optimize-read-queries"></a>V2: Введение денормализации для оптимизации запросов чтения
...
## <a name="v3-making-sure-all-requests-are-scalable"></a>V3: Обеспечение масштабируемой масштабирования всех запросов
...
- [Work with databases, containers, and items](databases-containers-items.md) (Работа с базами данных, контейнерами и элементами)
