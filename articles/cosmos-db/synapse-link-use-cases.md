...
![Azure Synapse Link для Azure Cosmos DB в аналитике цепочки поставок ](./media/synapse-link-use-cases/supply-chain-analytics.png)
...
![Azure Synapse Link для Azure Cosmos DB при персонализации в режиме реального времени](./media/synapse-link-use-cases/real-time-personalization.png)
...
![Azure Synapse Link для Azure Cosmos DB в прогнозном обслуживании Интернета вещей](./media/synapse-link-use-cases/iot-predictive-maintenance.png)
...
На следующем рисунке показаны шаблоны рабочих нагрузок, использующих Azure Cosmos DB: ![Шаблоны рабочих нагрузок Azure Synapse Link для Azure Cosmos DB](./media/synapse-link-use-cases/synapse-link-workload-patterns.png)

Давайте рассмотрим пример международной компании CompanyXYZ, занимающейся электронной коммерцией, которую осуществляет в 20 странах, чтобы продемонстрировать преимущества выбора Azure Cosmos DB как единой базы данных в режиме реального времени, поддерживающей транзакционные и аналитические требования к платформе управления запасами.
...
* Основной бизнес CompanyXYZ зависит от системы управления запасами, поэтому доступность и надежность являются для нее основными базовыми требованиями. Преимущества использования Azure Cosmos DB:
  ...
  * Благодаря тесной интеграции с инфраструктурой Azure и прозрачной глобальной репликации с несколькими владельцами Azure Cosmos DB предоставляет ведущую в отрасли [доступность уровня 99,999 %](high-availability.md) на фоне региональных простоев.
...
* Партнеры по цепочке поставок CompanyXYZ могут находиться в разных географических расположениях, но для поддержки их локальных операций может потребоваться единое представление о запасах продуктов по всему миру. Сюда входит возможность чтения обновлений, внесенных другими партнерами по цепочке поставок, в режиме реального времени. А также возможность вносить обновления, не беспокоясь о конфликтах с другими партнерами, сохраняя высокую пропускную способность. Преимущества использования Azure Cosmos DB:
  ...
  * С помощью уникального протокола репликации с несколькими владельцами и хранилища транзакций, оптимизированного для записи, Azure Cosmos DB обеспечивает менее 10 мс глобальной задержки для индексированных операций чтения и записи на уровне 99-го процентиля.
  ...
  * Транзакционное хранилище Azure Cosmos DB предоставляет на три варианта больше, чем два уровня согласованности (общей и на уровне событий), для достижения [компромисса в отношении доступности и производительности](consistency-levels-tradeoffs.md), максимального приближенного к потребностям бизнеса.
...
* [Использование бессерверного SQL / SQL по запросу (предварительная версия) в Azure Synapse Analytics](../synapse-analytics/sql/on-demand-workspace-overview.md).
