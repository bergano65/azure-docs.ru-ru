...
description: Узнайте, как подключить приложение MongoDB к Azure Cosmos DB, получив строку соединения с портала Azure
author: timsander1
ms.author: tisande
...
1. В интернет-браузере вопием на [портал Azure](https://portal.azure.com).
...
4. Откроется колонка **Строка подключения**. Она содержит все необходимые сведения для подключения к учетной записи с помощью драйвера для MongoDB, включая автоматически сформированную строку подключения.
   ...
   [![Лезвие](./media/connect-mongodb-account/ConnectionStringBlade.png) строки соединения](./media/connect-mongodb-account/ConnectionStringBlade.png#lightbox)
...
* Имя пользователя (обязательно): Имя учетной записи Cosmos.
* Пароль (требуется): Пароль учетной записи Cosmos.
* Хост (требуется): ФЗДН учетной записи Космоса.
