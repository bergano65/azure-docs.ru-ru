...
Пропускная способность, подготовленная для контейнера, равномерно распределяется между физическими секциями и предполагается, что ключ секции, который распределяет логические секции равномерно между физическими секциями, также распределяется равномерно по всем логические секции контейнера. Нельзя выборочно указывать пропускную способность для логических секций. Так как один логический раздел контейнера или несколько размещаются в физическом разделе, физические разделы относятся исключительно к данному контейнеру и поддерживают пропускную способность, подготовленную для контейнера. 
...
Все контейнеры, созданные в базе данных с подготовленной пропускной способностью, должны быть созданы с [ключом секции](partition-data.md). В любой момент времени пропускная способность, выделенная контейнеру в базе данных, распределяется по всем логическим секциям этого контейнера. При наличии контейнеров, использующих подготовленную пропускную способность, настроенную в базе данных, невозможно выборочно применить пропускную способность к конкретному контейнеру или логической секции. 
...
Пропускная способность, подготовленная к базе данных, может совместно использоваться контейнерами в этой базе данных. Для каждого нового контейнера с общей пропускной способностью уровня базы данных потребуется 100 единиц запросов в секунду. При подготовке контейнеров с предложением общей базы данных:

* Каждые 25 контейнеров группируются в набор секций, а пропускная способность базы данных (D) совместно используется контейнерами в наборе секций. Если в базе данных есть до 25 контейнеров и в любой момент времени, если используется только один контейнер, то этот контейнер может использовать максимальную пропускную способность.

* Для каждого нового контейнера, созданного после 25 контейнеров, создается новый набор секций, а пропускная способность базы данных разделяется между созданными наборами секций (т. е. D/2 для 2 наборов разделов, D/3 для трех наборов разделов...). В любой момент времени, если вы используете только один контейнер из базы данных, он может использовать максимум (D/2, D/3, D/4... пропускная способность) соответственно. Учитывая сниженную пропускную способность, рекомендуется создать не более 25 контейнеров в одной базе данных.

**Пример**

* При создании базы данных с именем MyDB с подготовленной пропускной способностью 10000 единиц запросов в секунду.

* При подготовке 25 контейнеров в разделе "MyDB" все контейнеры группируются в набор секций. В любой момент времени, если вы используете только один контейнер из базы данных, он может использовать не более 10000 единиц запросов в секунду (D).

* При подготовке контейнера 26th создается новый набор разделов, и пропускная способность разделяется между обоими наборами секций. Поэтому в любой момент времени, если вы используете только один контейнер из базы данных, он может использовать не более 5 КБ единиц запросов в секунду (D/2). Поскольку существует два набора разделов, коэффициент общей пропускной способности разделяется на D/2.

   На следующем рисунке показан пример графического изображения:

   ![Коэффициент совместного использования при пропускной способности уровня базы данных](./media/set-throughput/database-level-throughput-shareability-factor.png)


Если рабочие нагрузки предполагают удаление и повторное создание всех коллекций в базе данных, рекомендуется удалить пустую базу данных и создать новую базу данных перед созданием коллекции. На следующем рисунке показано, как в физическом разделе можно разместить один логический раздел или несколько, которые относятся к разным контейнерам в базе данных.
