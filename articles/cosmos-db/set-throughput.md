...
Контейнеры в базе данных с общей пропускной способностью совместно используют пропускную способность (единиц запросов в секунду), выделенную этой базе данных. В базе данных можно использовать до четырех контейнеров с минимум 400 единиц запросов в секунду. Для каждого нового контейнера после первых четырех потребуется не менее 100 единиц запросов в секунду. Например, если у вас есть общая пропускная способность базы данных с восемью контейнерами, минимальный единицей запросов в секунду будет 800 единиц запросов в секунду.

> [!NOTE]
> В феврале 2020 мы представили изменение, которое позволяет иметь не более 25 контейнеров в общей пропускной способности, что обеспечивает совместное использование пропускной способности в контейнерах. После первых 25 контейнеров можно добавить дополнительные контейнеры в базу данных, только если они [подготовлены с выделенной пропускной способностью](#set-throughput-on-a-database-and-a-container), которая отделена от общей пропускной способности базы данных.<br>
Если учетная запись Azure Cosmos DB уже содержит общую пропускную способность с > = 25 контейнерами, учетная запись и все остальные учетные записи в той же подписке Azure будут исключены из этого изменения. Если у вас есть отзывы или вопросы, [обратитесь в службу поддержки продукта](https://portal.azure.com/?#blade/Microsoft_Azure_Support/HelpAndSupportBlade) . 
...
* Текущее число контейнеров Azure Cosmos в базе данных с общей пропускной способностью. 
