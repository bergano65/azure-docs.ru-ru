...
В следующем примере соединение является перекрестным произведением между двумя объектами JSON, корнем `id` элемента и `children` подкорнем. Тот факт, `children` что является массивом, не эффективен в соединении, так как он работает с одним корнем `children` , который является массивом. Результат содержит только два результата, поскольку перекрестное произведение каждого элемента с массивом дает только один элемент.
...
2. Примените перекрестное произведение с корнем `f` элемента с каждым дочерним элементом `c` , сведенным к первому шагу.
3. Наконец, следует проецировать только свойство `f` `id` корневого объекта.

Первый элемент, `AndersenFamily`, содержит только один `children` элемент, поэтому результирующий набор содержит только один объект. Второй элемент, `WakefieldFamily`,, содержит два `children`, поэтому перекрестный продукт создает два объекта, по одному `children` для каждого элемента. Корневые поля в обоих этих элементах будут такими же, как и ожидаемые при векторном произведении.
...
В следующем примере имеется дополнительный фильтр для `pet`, исключающий все кортежи, в которых нет `Shadow`имени Pet. Можно создавать кортежи из массивов, фильтровать по любому элементу кортежа и проецировать любое сочетание элементов.
