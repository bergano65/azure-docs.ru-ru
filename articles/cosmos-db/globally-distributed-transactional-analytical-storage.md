...
|Максимальный объем хранилища на ключ логической секции   |   10 GB      |   Без ограничений      |
...
|Цена за ГБ    |   0,25 долл. США за ГБ      |  $0,02/ГБ       |
...
1. **Настройка контейнера исключительно для аналитических рабочих нагрузок (с бесконечным хранением)**
   ...
   Вы можете настроить контейнер Azure Cosmos исключительно для аналитических рабочих нагрузок. Эта конфигурация имеет преимущество, когда вам не нужно платить за хранилище транзакций. Если ваша цель — использовать контейнер только для аналитических рабочих нагрузок, можно отключить хранилище транзакций, установив `DefaultTimeToLive` в 0 в контейнере Cosmos, а также включить аналитическое хранилище с бесконечным хранением, установив `ColumnStoreTimeToLive` в значение-1.
...
1. **Настройка контейнера исключительно для транзакционных рабочих нагрузок (с неограниченным сроком хранения)**
   ...
   Вы можете настроить контейнер Cosmos для Azure исключительно для транзакционных рабочих нагрузок. Вы можете отключить аналитическое хранилище, задав `ColumnStoreTimeToLive` равным 0 в контейнере, а также включить аналитическое хранилище с бесконечным хранением, установив `DefaultTimeToLive` в значение-1.
...
1. **Настройка контейнера для транзакционных и аналитических рабочих нагрузок (с неограниченным сроком хранения)**
   ...
   Вы можете настроить контейнер Azure Cosmos для транзакционных и аналитических рабочих нагрузок с полной изоляцией производительности между ними. Вы можете включить аналитическое хранилище, установив значение `ColumnStoreTimeToLive` в значение-1 и включив хранилище транзакций с бесконечным хранением, установив `DefaultTimeToLive ` в значение-1.
...
1. **Настройка контейнера для транзакционных и аналитических рабочих нагрузок с использованием уровней хранилища**
   ...
   Можно включить хранилище транзакций с бесконечным хранением, установив `DefaultTimeToLive` в < значение 1 > и включив аналитическое хранилище, установив `ColumnStoreTimeToLive` в < значение 2 >. Azure Cosmos DB обеспечит, что < значение 2 > всегда больше, чем < значение 1 >.
