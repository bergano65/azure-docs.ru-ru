...
Конечное свойство `streetName` внутри вложенного объекта `address` будет представлено в схеме аналитического хранилища в виде столбца `address.object.streetName.int32` . Тип данных добавляется в качестве суффикса к столбцу. Таким образом, если в хранилище транзакций добавляется другой документ, где значение конечного свойства `streetNo` равно "123" (Обратите внимание, что это строка), схема аналитического хранилища автоматически изменяется без изменения типа ранее записанного столбца. Новый столбец, добавляемый в аналитическое хранилище как `address.object.streetName.string` место, где хранится значение "123".
