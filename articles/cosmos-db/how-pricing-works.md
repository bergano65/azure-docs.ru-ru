...
- **Операции с базой данных**. способ оплаты за операции с базой данных зависит от типа используемой учетной записи Azure Cosmos.
  ...
  - **Подготовленная пропускная способность**. [подготовленная](set-throughput.md) пропускная способность (также называемая зарезервированной пропускной способностью) гарантирует высокую производительность в любом масштабе. Вы указываете необходимую пропускную способность в [единицах запросов](request-units.md) в секунду, а Azure Cosmos DB выделяет ресурсы, необходимые для обеспечения настроенной пропускной способности. Вы можете [подготовить пропускную способность для базы данных или контейнера](set-throughput.md). В зависимости от потребностей рабочей нагрузки можно масштабировать пропускную способность в любое время или использовать [Автомасштабирование](provision-throughput-autoscale.md) (хотя для базы данных или контейнера требуется минимальная пропускная способность, чтобы гарантировать соглашение об уровне обслуживания). С вас взимается плата за максимальную подготовленную пропускную способность в течение каждого часа.
