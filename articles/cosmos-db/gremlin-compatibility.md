...
* в настоящее время не поддерживается ***```property(set, 'xyz', 1)```*** задание количества элементов. Используйте вместо этого ```property(list, 'xyz', 1)```.
...
* ***Типы JSON, отличные от примитивов*** , не поддерживаются. Используйте типы ```string```, ```number``` или ```true``` @ no__t-3 @ no__t-4. значения ```null``` не поддерживаются. 
