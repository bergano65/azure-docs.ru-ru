---
title: Часто задаваемые вопросы о режиме автомасштабирования в Azure Cosmos DB
...
При подготовленной пропускной способности в режиме автомасштабирования Azure Cosmos DB будет автоматически управлять единицами запросов в контейнере или базе данных и масштабировать их в зависимости от их использования. В этой статье содержатся ответы на часто задаваемые вопросы об автомасштабировании.

## <a name="frequently-asked-questions"></a>Часто задаваемые вопросы

### <a name="is-autoscale-mode-supported-for-all-apis"></a>Поддерживается ли режим автомасштабирования для всех API?
Да, режим автомасштабирования поддерживается для всех API-интерфейсов: Core (SQL), Gremlin, Table, Cassandra и API для MongoDB.

### <a name="is-autoscale-mode-supported-for-multi-master-accounts"></a>Поддерживается ли режим автомасштабирования для учетных записей с несколькими хозяевами?
Да, режим автомасштабирования поддерживается для учетных записей с несколькими хозяевами. В каждом регионе, который добавляется в учетную запись Cosmos, доступны максимальные единицы запросов в секунду. 
...
Режим автомасштабирования можно включить для новых контейнеров и баз данных, созданных с помощью портал Azure. 

### <a name="is-there-cli-or-sdk-support-to-create-containers-or-databases-with-autoscale-mode"></a>Поддерживается ли интерфейс командной строки или пакет SDK для создания контейнеров или баз данных с режимом автомасштабирования?
В настоящее время в портал Azure можно создавать только ресурсы с режимом автомасштабирования. Поддержка интерфейса командной строки и пакета SDK пока недоступна.
...
В настоящее время можно включить Автомасштабирование для новых контейнеров и баз данных при их создании. Поддержка включения режима автомасштабирования в существующих контейнерах и базах данных пока недоступна. Вы можете перенести существующие контейнеры в новый контейнер с помощью [фабрики данных Azure](../data-factory/connector-azure-cosmos-db.md) или [веб-канала изменений](change-feed.md). 

### <a name="can-i-turn-off-autoscale-mode-on-a-container-or-database"></a>Можно ли отключить режим автомасштабирования в контейнере или базе данных?
Да, вы можете отключить Автомасштабирование, перейдя к параметру "вручную" для подготовленной пропускной способности. В текущем выпуске после переключения из режима автомасштабирования в режим «вручную» невозможно снова включить Автомасштабирование для того же ресурса. 

### <a name="is-autoscale-mode-supported-for-shared-throughput-databases"></a>Поддерживается ли режим автомасштабирования для баз данных с общей пропускной способностью?
Да, режим автомасштабирования поддерживается для баз данных с общей пропускной способностью. Чтобы включить эту функцию, выберите режим автомасштабирования и параметр **пропускная способность** при создании базы данных. 
...
Для баз данных с общим пропускной способностью с включенным режимом автомасштабирования число разрешенных коллекций: MIN (25, Max единиц запросов в секунду для базы данных/1000). Например, если максимальная пропускная способность базы данных составляет 20 000 единиц запросов в секунду, то база данных может иметь минимум (25, 20 000 единиц запросов в секунду/1000) = 20 коллекций. 
...
В режиме автомасштабирования можно мгновенно увеличивать или уменьшать масштаб единиц запросов/с в пределах минимального и максимального диапазона единиц запросов в секунду на основе входящего трафика. Выставление счетов выполняется через 1-часовой уровень детализации, где за определенный час взимается плата за наибольший единиц запросов в секунду.

### <a name="can-i-specify-a-custom-max-throughput-rus-value-for-autoscale-mode"></a>Можно ли указать настраиваемое значение максимальной пропускной способности (единиц запросов в секунду) для режима автомасштабирования?
...
* Узнайте о [преимуществах подготовленной пропускной способности в режиме автомасштабирования](provision-throughput-autoscale.md#autoscale-benefits).
