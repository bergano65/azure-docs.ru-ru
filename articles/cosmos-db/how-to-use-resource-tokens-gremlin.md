...
Одна учетная запись Gremlin позволяет выпускать неограниченное количество маркеров. Однако на протяжении 1 часа можно использовать до 100 маркеров одновременно. Если приложение превышает лимит токенов в час, запрос на проверку подлинности отклоняется, и вы получаете следующее сообщение об ошибке: "Превышен допустимым ограничением маркера ресурса в 100, который может быть использован одновременно". Это не работает для закрытия активных соединений, использующих определенные маркеры, чтобы освободить слоты для новых маркеров. Ядро базы данных Azure Cosmos DB Gremlin отслеживает уникальные маркеры в течение часа непосредственно перед запросом аутентификации.
...
* [Узнайте, как обеспечить доступ к данным](secure-access-to-data.md) в Azure Cosmos DB
