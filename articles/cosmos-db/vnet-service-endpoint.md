---
title: Безопасный доступ к dB-счетам Azure Cosmos с помощью виртуальной конечней точки службы Сети
...
### <a name="are-additional-rbac-permissions-needed-for-azure-cosmos-accounts-with-vnet-service-endpoints"></a>Нужны ли дополнительные разрешения RBAC для учетных записей Azure Cosmos с конечными точками службы VNET?

После добавления конечных точек службы VNet в учетную запись Azure Cosmos для `Microsoft.Network/virtualNetworks/subnets/joinViaServiceEndpoint/action` внесения любых изменений в настройки учетной записи необходим доступ к действию для всех VNET, настроенных на учетную запись Azure Cosmos. Это разрешение требуется, поскольку процесс авторизации проверяет доступ к ресурсам (например, база данных и виртуальные сетевые ресурсы) перед оценкой любых свойств.
 
Авторизация проверяет разрешение на действие ресурса VNet, даже если пользователь не указывает AcL VNET с помощью Azure CLI. В настоящее время плоскость управления учетной записью Azure Cosmos поддерживает установление полного состояния учетной записи Azure Cosmos. Одним из параметров для вызова `virtualNetworkRules`диспетчерской плоскости является. Если этот параметр не указан, Azure CLI делает вызов `virtualNetworkRules` базы данных для получения и использования этого значения в вызове обновления.
...
В настоящее время для учетной записи Azure Cosmos разрешено не более 256 подсетей.

### <a name="can-i-enable-access-from-vpn-and-express-route"></a>Можно ли включить доступ для VPN и Express Route? 
Для доступа к учетной записи Azure Cosmos по маршруту Express из помещений необходимо включить вонючую майкрософт. После размещения правил доступа к брандмауэру IP-адресов или виртуальной сети можно добавить общедоступные IP-адреса, используемые для пиринга Майкрософт в брандмауэре учетной записи Azure Cosmos, чтобы разрешить локальным службам доступ к учетной записи. 
...
Нет, только виртуальные сети Azure Resource Manager могут использовать конечную точку службы. Классические виртуальные сети не поддерживают конечные точки обслуживания.
...
Это требуется только в том случае, если вы хотите, чтобы к учетной записи Azure Cosmos DB были доступны другие службы первой стороны Azure, такие как фабрика данных Azure, Azure Cognitive Search или любая служба, развернутая в данном регионе Azure.
