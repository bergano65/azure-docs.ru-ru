...
4. Выберите **Подписка**, из которой требуется добавить виртуальную сеть Azure. Выберите **виртуальную сеть** и **подсети** Azure, которым необходимо предоставить доступ к учетной записи Azure Cosmos DB. Затем выберите **Включить**, чтобы включить выбранные сети с конечными точками службы для "Microsoft.AzureCosmosDB". После завершения работы мастера выберите **Добавить**.  
   ...
   > [!NOTE]
   > Если конечная точка службы для Azure Cosmos DB не была ранее настроена для выбранных виртуальных сетей и подсетей Azure, ее можно настроить в рамках этой операции. Включение доступа займет до 15 минут. Обязательно отключите брандмауэр IP-адресов, когда сохраните содержимое ACL брандмауэра, чтобы повторно включить его позже. 
...
> [!NOTE]
> Если у вас уже есть брандмауэр IP-адресов, настроенный для вашей учетной записи Azure Cosmos DB, запишите его конфигурацию, удалите брандмауэр IP-адресов, а затем включите конечную точку службы. Если включить конечную точку службы, но не отключить брандмауэр, трафик из указанного диапазона IP-адресов утратит идентификатор виртуального IP-адреса и будет удален (появится сообщение об ошибке фильтрации IP-адресов). Чтобы избежать этой ошибки, всегда следует придерживаться такой последовательности действий: отключить правила брандмауэра, скопировать их, включить конечную точку службы из подсети и указать ACL для подсети из Cosmos DB. Настроив конечную точку службы и добавив список ACL, вы можете повторно включить брандмауэр IP-адресов, если это необходимо.
...
### <a name="what-is-the-relationship-between-service-endpoint-and-network-security-group-nsg-rules"></a>Какова взаимосвязь между конечной точкой службы и правилами группы безопасности сети (NSG)?  

Правила NSG в Azure Cosmos DB позволяют ограничить доступ к определенным диапазонам IP-адресов Azure Cosmos DB. Если нужно разрешить доступ к экземпляру Azure Cosmos DB в определенном [регионе](https://azure.microsoft.com/global-infrastructure/regions/), укажите этот регион в следующем формате: 

    AzureCosmosDB.<region name>

Дополнительные сведения о тегах NSG см. в статье о [тегах службы виртуальной сети](../virtual-network/security-overview.md#service-tags). 
