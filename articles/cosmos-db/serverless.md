...
- **Задержка**. После того как бессерверное предложение становится общедоступным, задержка бессерверных контейнеров будет заключаться в том, что в течение 10 миллисекунд или меньше для операций чтения точки и 30 миллисекунд или меньше для записи. Операции чтения по точкам включают в себя выборку одного элемента по его ИДЕНТИФИКАТОРу и значению ключа секции.
...
- [Выбор между подготовленной пропускной способностью и бессерверным](throughput-serverless.md)
