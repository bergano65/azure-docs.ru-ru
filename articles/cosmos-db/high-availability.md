...
- **Учетные записи в нескольких регионах с одним регионом для записи (сбой области записи):** 
  * будут сохранять высокий уровень доступности для операций чтения при сбое региона записи. Однако для операций записи необходимо **включить автоматический переход** на другой ресурс в учетной записи Cosmos, чтобы отработка отказа затронутого региона в другом регионе. Отработка отказа будет выполняться в указанном порядке приоритетов регионов. 
  * Когда затронутый регион возвращается в режим «в сети», нереплицированные данные, находящиеся в затронутом регионе записи во время сбоя, становятся доступными через [канал конфликтов](how-to-manage-conflicts.md#read-from-conflict-feed). Приложения могут читать канал конфликтов, разрешать конфликты на основе логики конкретного приложения и записывать обновленные данные обратно в контейнер Cosmos. 
...
При добавлении региона в новые или существующие учетные записи Cosmos Azure можно включить избыточность зоны. В настоящее время избыточность зоны может быть включена только с помощью шаблонов портал Azure, PowerShell и Azure Resource Manager. Чтобы включить избыточность зоны в учетной записи Azure Cosmos, установите `isZoneRedundant` флаг в `true` для определенного расположения. Этот флаг можно установить в свойстве Locations. Например, следующий фрагмент кода PowerShell включает избыточность зоны для региона "Юго-Восточная Азия":
