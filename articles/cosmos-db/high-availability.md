...
Региональные сбои случаются нечасто, и Azure Cosmos DB гарантирует, что ваша база данных всегда высокодоступна. Следующие сведения записи Cosmos DB поведение во время сбоя, в зависимости от конфигурации учетной записи Cosmos:
...
|Задержка чтения    |  CROSS регион    |   CROSS регион   |    Низкий  |
|Задержка записи    |   CROSS регион   |  CROSS регион    |   Низкий   |
...
> [!NOTE] 
> Чтобы включить поддержку зоне доступности с несколькими региона учетной записи Azure Cosmos, учетная запись должна иметь операциях записи включено.


Вы можете включить избыточность зон, при добавлении области в новые или существующие учетные записи Azure Cosmos. В настоящее время можно включить только избыточность зон с помощью Azure portal, шаблонов Azure Resource Manager и PowerShell. Чтобы обеспечить избыточность зон в вашей учетной записи Azure Cosmos, следует задать `isZoneRedundant` флаг `true` в определенном расположении. Можно установить этот флаг в свойство расположения. Например следующий фрагмент кода powershell позволяет избыточности зон для региона «Юго-Восточная Азия»:
...
Вы можете включить зоны доступности с помощью портала Azure, при создании учетной записи Azure Cosmos. При создании учетной записи, не забудьте включить **геоизбыточности**, **записывает в нескольких регионах**и выберите регион, когда поддерживаются зоны доступности: 
