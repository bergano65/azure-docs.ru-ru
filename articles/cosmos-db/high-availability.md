...
* Учетные записи Cosmos Azure, использующие строгую согласованность с тремя или меньшим числом регионов (одна запись, два чтения), потеряют доступность записи во время сбоя региона чтения. Однако клиенты с четырьмя или более общими регионами могут использовать динамические кворумы чтения, отправив запрос в службу поддержки. Учетные записи, которые поддерживают по крайней мере два региона чтения в этой конфигурации, будут поддерживать доступность для записи.
...
|Региональный сбой — потери данных | Потеря данных |  Потеря данных | Зависит от уровня согласованности. Дополнительные сведения см. в статье о [согласованности, доступности и производительности](consistency-levels-tradeoffs.md) . | Зависит от уровня согласованности. Дополнительные сведения см. в статье о [согласованности, доступности и производительности](consistency-levels-tradeoffs.md) .
