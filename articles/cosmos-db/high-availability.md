...
* Если Azure Cosmos DB, то перед тем как операция записи будет подтверждена клиенту, данные надежно фиксируются кворумом реплик в пределах региона, принимающего операции записи. Дополнительные сведения см. в разделе [уровни согласованности и пропускная способность](consistency-levels-tradeoffs.md#consistency-levels-and-throughput) .
...
* [Шаблоны диспетчера ресурсов Azure](manage-sql-with-resource-manager.md)
