...
- Учетные записи в нескольких регионах, настроенные с несколькими регионами или несколькими хозяевами, будут высокодоступными для операций записи и чтения. Региональные отработки отказа происходят мгновенно и не требуют изменений приложения.
...
- Учетные записи Cosmos Azure, использующие строгую согласованность с двумя или менее регионами чтения (включая область чтения & записи), потеряют доступность записи во время сбоя региона чтения, но будут поддерживать доступность чтения для оставшихся регионов.
...
Эта функция доступна в: *Южная часть Соединенного Королевства, Юго-Восточная Азия, Восточная часть США 2, Центральная часть США, Западная Европа, Западная часть США 2, Восточная Япония, Северная Европа, центральные регионы Франции* .
...
:::image type="content" source="./media/high-availability/enable-availability-zones-using-portal.png" alt-text="Включение Зоны доступности с помощью портал Azure"::: 
