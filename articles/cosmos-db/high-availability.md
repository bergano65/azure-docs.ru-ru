...
### <a name="multi-region-accounts-with-a-single-write-region-write-region-outage"></a>Многорегиональные учетные записи с одноязычным регионом (сбой региона записи)
...
- Когда ранее затронутый регион возвращается в онлайн, любые данные записи, которые не были воспроизведены, когда регион не удалось, становится доступным через [конфликты корма](how-to-manage-conflicts.md#read-from-conflict-feed). Приложения могут читать ленту конфликтов, разрешать конфликты на основе конкретной логики приложения и записывать обновленные данные в контейнер Azure Cosmos по мере необходимости.
- После восстановления ранее затронутый регион записи становится автоматически доступным как регион чтения. Вы можете вернуться в восстановленный регион в качестве региона записи. Вы можете переключить регионы с помощью [портала PowerShell, Azure CLI или Azure.](how-to-manage-database-account.md#manual-failover) Нет **потери данных или доступности** до, во время или после переключения региона записи, и ваше приложение продолжает быть очень доступным.

> [!IMPORTANT]
> Настоятельно рекомендуется настроить учетные записи Azure Cosmos, используемые для производственных нагрузок, чтобы **включить автоматическое сбой.** Ручной сбой требует подключения между вторичным и первичным регионом записи для завершения проверки согласованности, чтобы убедиться, что во время сбоя нет потери данных. Если основная область недоступна, эта проверка согласованности не может завершиться, а сбой в руководстве не удастся, что приведет к потере доступности записи.

### <a name="multi-region-accounts-with-a-single-write-region-read-region-outage"></a>Многорегиональные учетные записи с однозначным регионом (читай-

- Во время сбоя в области чтения учетные записи Cosmos, использующие любой уровень согласованности или сильную согласованность с тремя или более прочитанными регионами, будут оставаться доступными для чтения и записей.
...
- Для обработки сбоя региона чтения изменения в коде приложения не требуются. Когда область чтения, повлияваена на него, она автоматически синхронизируется с текущим регионом записи и будет снова доступна для обслуживания запросов на чтение.
...
> [!IMPORTANT]
> Учетные записи Azure Cosmos, использующие сильную согласованность с двумя или менее регионами чтения, теряют доступность записи во время сбоя в сбое в сбое в считывании, но сохранят доступность чтения для остальных регионов.
...
- Даже если ваша учетная запись Azure Cosmos очень доступна, приложение может быть неправильно разработано, чтобы оставаться очень доступным. Чтобы проверить сквозную высокую доступность приложения, как часть тестирования приложения или аварийного восстановления (DR) сверла, временно отключить автоматическое сбой для учетной записи, вызвать [руководство неудачи с помощью PowerShell, Azure CLI или Azure портала](how-to-manage-database-account.md#manual-failover), а затем контролировать сбой приложения. После завершения, вы можете выйти из строя обратно в основной области и восстановить автоматическое сбой для учетной записи.
