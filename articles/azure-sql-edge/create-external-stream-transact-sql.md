...
Azure SQL ребр в настоящее время поддерживает только следующие входные и выходные данные потока.

**Потоковый вход**: определяет соединения с источником данных для чтения потока данных.
- Центр Edge
- Kafka (поддержка входных данных Kafka в настоящее время доступна только в версиях Intel/AMD64 Azure SQL ребр.)

**Потоковый выход**: определяет соединения с источником данных для записи потока данных. 
- Центр Edge
