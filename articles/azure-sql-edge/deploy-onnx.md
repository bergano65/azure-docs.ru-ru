---
title: Развертывание и создание прогнозов с помощью ONNX в SQL Azure для пограничных вычислений (предварительная версия)
description: Узнайте, как обучить модель, преобразовать ее в ONNX, развернуть в SQL Azure для пограничных вычислений (предварительная версия), а затем выполнить собственную функцию прогноза (PREDICT) с данными с помощью переданной модели ONNX.
...
ms.locfileid: "85476739"
---
# <a name="deploy-and-make-predictions-with-an-onnx-model-in-azure-sql-edge-preview"></a>Развертывание и создание прогнозов с помощью модели ONNX в SQL Azure для пограничных вычислений (предварительная версия)

Из этого краткого руководства вы узнаете, как обучить модель, преобразовать ее в ONNX, развернуть в SQL Azure для пограничных вычислений (предварительная версия), а затем выполнить собственную функцию прогноза (PREDICT) с данными с помощью переданной модели ONNX. Дополнительные сведения см. в статье [Машинное обучение и ИИ с применением ONNX в SQL для пограничных вычислений (предварительная версия)](onnx-overview.md).
...
* Если вы еще не развернули модуль SQL Azure для пограничных вычислений, выполните действия, описанные в статье об [развертывании SQL для пограничных вычислений (предварительная версия) с помощью портала Azure](deploy-portal.md).
...
* Откройте Azure Data Studio и выполните следующие действия, чтобы установить пакеты, необходимые для этого краткого руководства:
    ...
    1. Щелкните **Управление пакетами**, и в разделе **Добавить новый** найдите пакет **scikit-learn** и установите его. 
    1. Кроме того, установите пакеты **setuptools**, **numpy**, **onnxmltools**, **onnxruntime**, **skl2onnx**, **pyodbc** и **sqlalchemy**.
...
Загрузите данные в SQL Azure для пограничных вычислений.
...
Когда модель подготовлена в SQL Azure для пограничных вычислений, выполните собственную функцию PREDICT с данными, используя переданную модель ONNX.
